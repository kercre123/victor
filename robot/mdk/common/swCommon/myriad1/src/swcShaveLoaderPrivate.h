...
typedef struct
{
   ...
   //andreil: these are reversed (compared to original definition) to cope with Leon shit [caches-ON] "featrure"
    unsigned short    shEntrySize;      // Program header table entry count
    unsigned short    phCount;          // Section header table entry size
    unsigned short    shStringIndex;    // Section header table entry count
    unsigned short    shCount;          // Section header string table index
} tMofFileHeader;
