...
/* metadata must always be big-endian !!! */
#ifndef INTERNAL_FILEDATA_SECTOR_BUFFER_TYPE
#define INTERNAL_FILEDATA_SECTOR_BUFFER_TYPE le_pointer
#endif
/* for various reasons you might redefine INTERNAL_FILEDATA_SECTOR_BUFFER_TYPE
 * in the Makefile
 */
...
typedef struct {
	...
	pointer_type win_pt;        /* This holds the data type of the information that is stored in the window (DT_FILEDATA/DT_METADATA) */
} FATFS;
...
FRESULT f_read (FIL*, void*, UINT, UINT*, pointer_type);			/* Read data from a file */
...
FRESULT f_write (FIL*, const void*, UINT, UINT*, pointer_type);	/* Write data to a file */
