...
typedef struct {
	...
	//andreil: short is enough
    //int scale_factor[MAX_SCFAC_BANDS];
	short scale_factor[MAX_SCFAC_BANDS];
	...
	//andreil: char is enough, as there are just 12 CBs...
    //int book_vector[MAX_SCFAC_BANDS];
	char book_vector[MAX_SCFAC_BANDS]; 
...
typedef struct {
  ...
  //andriel: char is enough.
  char num_cb_long;
  char num_cb_short;
  char cb_width_long[NSFB_LONG];
  char cb_width_short[NSFB_SHORT];
} SR_INFO;
