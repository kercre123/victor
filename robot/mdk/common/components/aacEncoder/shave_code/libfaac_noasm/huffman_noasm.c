...
// this area will be shared a number of times
unsigned char g_WBUFF[1024*16];
...
void HuffmanInit(CoderInfo *coderInfo, unsigned int numChannels)
{
	...
	//andreil:
    // #error: check if data array is accessed above FRAME_LEN

    for (channel = 0; channel < numChannels; channel++)
	{
        //coderInfo[channel].data = (int*)AllocMemory(1*FRAME_LEN*sizeof(int), "coderInfo[channel].data");
		////andreil: made it short
        //coderInfo[channel].len = (short*)AllocMemory(1*FRAME_LEN*sizeof(short),  "coderInfo[channel].len");

		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        //andrei: map these on g_WBUFF: data is 4KB, len is 2KB (6KB alltogether)
		coderInfo[channel].data =   (int*)&g_WBUFF[channel*(6*1024)+0];
        coderInfo[channel].len  = (short*)&g_WBUFF[channel*(6*1024)+4096];
    }
}
...

{
	...
	char* book_vector = coderInfo -> book_vector;

    levels = (int) ((FAAC_log((float)coderInfo->nr_of_sfb)/FAAC_log((float)2.0))+1);
...

{
	...
	//WARNING andreil: input requires > 16 bits to represent... so input/16 should be > 1
	// so it should enter this loop at least once, thus make N>=0... in this case 
	// I could use the shifts instead of pows ???
    while (x >= 1) {
        N++;
        x = x/2;
    }
    ...
    *len_esc_sequence = 2*N + 5;  /* the length of the escape sequence in bits */
	...
	//andreil: can replace the pows with shifts ???
	//if(N<1)
	//	 printf("can't use shifts...");

    output = (int)((FAAC_pow(2,N) - 1)*FAAC_pow(2,N+5) + y - FAAC_pow(2,N+4));
	//output = (int)( ((2<<N) - 1)*(2<<(N+5)) + y - (2<<(N+4)));
...

{
	...
	short* scale_factors = coderInfo->scale_factor;
