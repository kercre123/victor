 ...
 *
 * @brief SPI, SEND LUDP DATA, send ludp data to an existing connection
 *
 * @section Description
 ...
 * @param[in]   uint8  protocol, LUDP
 * @param[in]   uint8 *targetIp, Remote target IP address
 * @param[in]   uint8 *targetPort, Remote target port
 * @param[out]  none
 * @return      errCode
 *              -1 = SPI busy / Timeout
 *              -2 = SPI Failure
              ...
              uint32 payloadLen,uint8 protocol, uint8 *destIp, uint16 destPort)
{
	...
	*(uint16 *)uSendFrame.sendFrameLudpSnd.sendDataOffsetSize = headerLen;
#else
	rsi_uint16To2Bytes(uSendFrame.sendFrameLudpSnd.sendDataOffsetSize, headerLen);
#endif
	...
	*(uint16 *)uFrameDscFrame.frameDscDataSnd.dataFrmBodyLen = (uint16)(frameLen & 0x7fff);
	...
	uSendFrame.sendFrameLudpSnd.sendCmd[0] = rsi_dFrmSend[0];
	uSendFrame.sendFrameLudpSnd.sendCmd[1] = rsi_dFrmSend[1];
#ifdef RSI_LITTLE_ENDIAN
	*(uint16 *)uSendFrame.sendFrameLudpSnd.socketDescriptor = socketDescriptor;
	*(uint16 *)uSendFrame.sendFrameLudpSnd.sendBufLen = payloadLen;
   *(uint16 *)uSendFrame.sendFrameLudpSnd.destPort = destPort;
#else
	rsi_uint16To2Bytes(uSendFrame.sendFrameLudpSnd.socketDescriptor,socketDescriptor);
	rsi_uint16To2Bytes(uSendFrame.sendFrameLudpSnd.sendBufLen,payloadLen);
   rsi_uint16To2Bytes(uSendFrame.sendFrameLudpSnd.destPort,destPort);
#endif
   memcpy(uSendFrame.sendFrameLudpSnd.destIp,destIp,RSI_IP_ADD_LEN);
	...
	retval = rsi_SpiFrameDataWr(hdrLen,(uint8 *)&uSendFrame.uSendBuf,
			(payloadLen),(uint8 *)payload,RSI_C2DATA);
	if (retval != 0x00) 
	{
#ifdef RSI_DEBUG_PRINT
