...
//     void Convolution1x7_asm(unsigned char** in, unsigned char** out, half* conv, unsigned long inWidth);
//
...
class Convolution1x7KernelTest : public ::testing::TestWithParam< unsigned long > {
protected:
...
TEST_F(Convolution1x7KernelTest, TestUniformInputLinesWidthAll0)
{
	...
	float* convFilterC = inputGen.GetLineFloat(7, 1.0f);
	half* convFilterAsm = inputGen.GetLineFloat16(7, 1.0);

	Convolution1x7(inLines, outLinesC, convFilterC, width);
	Convolution1x7_asm_test(inLines, outLinesAsm, convFilterAsm, width);
...
TEST_F(Convolution1x7KernelTest, TestIndividualValues)
{
	...
	float convFilterC[7] = {-0.1, 1.5, -5.0, 14.0, -5.0, 1.5, -0.1};
	half convFilterAsm[7] = {-0.1, 1.5, -5.0, 14.0, -5.0, 1.5, -0.1};

	inLines[0][0] =   4;  inLines[0][1] =  23; inLines[0][2] =    0;
	inLines[0][3] =   8;  inLines[0][4] =  16; inLines[0][5] =   25;
	inLines[0][6] =  10;  inLines[0][7] = 255; inLines[0][8] =   16;
	inLines[0][6] =   0;  inLines[0][7] =   2; inLines[0][8] =    1;

	Convolution1x7(inLines, outLinesC, convFilterC, width);
	Convolution1x7_asm_test(inLines, outLinesAsm, convFilterAsm, width);
	RecordProperty("CyclePerPixel", convolution1x7CycleCount / width);

	EXPECT_EQ(103, outLinesAsm[0][0]);
	EXPECT_EQ( 56, outLinesAsm[0][1]);
...
TEST_F(Convolution1x7KernelTest, TestUniformInputRandomFilter)
{
	...
	outLinesAsm = inputGen.GetEmptyLines(width, 1);
	inputGen.SelectGenerator("random");
	float* convFilterC = inputGen.GetLineFloat(7, -3.0f, 10.0f);
	half convFilterAsm[7];
	for(int i = 0; i < 7; i++) {
