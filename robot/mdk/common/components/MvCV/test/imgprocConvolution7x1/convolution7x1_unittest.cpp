...
//     void Convolution7x1_asm(unsigned char** in, unsigned char** out, half* conv, unsigned long inWidth);
//
...
class Convolution7x1KernelTest : public ::testing::TestWithParam< unsigned long > {
protected:
...
TEST_F(Convolution7x1KernelTest, TestUniformInputLinesWidthAll0)
{
	...
	Convolution7x1(inLines, outLinesC, convFilterC, width);
	Convolution7x1_asm_test(inLines, outLinesAsm, convFilterAsm, width);
...
TEST_F(Convolution7x1KernelTest, TestUniformInputDifferentFilter)
{
	...
	outLinesC = inputGen.GetEmptyLines(width, 1);
	outLinesAsm = inputGen.GetEmptyLines(width, 1);
	float convFilterC[KERNEL_SIZE] = {-0.1, 1.5, -5.0, 14.0, -5.0, 1.5, -0.1};
	half convFilterAsm[KERNEL_SIZE] = {-0.1, 1.5, -5.0, 14.0, -5.0, 1.5, -0.1};
