...
//     void Convolution1x9_asm(unsigned char** in, unsigned char** out, half* conv, unsigned long inWidth);
//
...
class Convolution1x9KernelTest : public ::testing::TestWithParam< unsigned long > {
protected:
...
TEST_F(Convolution1x9KernelTest, TestUniformInputLinesWidthAll0)
{
	...
	float* convFilterC = inputGen.GetLineFloat(9, 1.0f);
	half* convFilterAsm = inputGen.GetLineFloat16(9, 1.0);

	Convolution1x9(inLines, outLinesC, convFilterC, width);
	Convolution1x9_asm_test(inLines, outLinesAsm, convFilterAsm, width);
...
TEST_F(Convolution1x9KernelTest, TestUniformInputDifferentFilter)
{
	...
	outLinesC = inputGen.GetEmptyLines(width, 1);
	outLinesAsm = inputGen.GetEmptyLines(width, 1);
	float convFilterC[9] = {0.1, -2.5, 4.0, -6.0, 18.0, -6.0, 4.0, -2.5, 0.1};
	half convFilterAsm[9] = {0.1, -2.5, 4.0, -6.0, 18.0, -6.0, 4.0, -2.5, 0.1};
...
TEST_F(Convolution1x9KernelTest, TestUniformInputRandomFilter)
{
	...
	outLinesAsm = inputGen.GetEmptyLines(width, 1);
	inputGen.SelectGenerator("random");
	float* convFilterC = inputGen.GetLineFloat(9, -3.0f, 10.0f);
	half convFilterAsm[9];
	for(int i = 0; i < 9; i++) {
