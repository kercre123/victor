...
TestRunner convolution7x1TestRunner(APP_PATH, APP_ELFPATH, DBG_INTERFACE);
...
void Convolution7x1_asm_test(unsigned char** in, unsigned char** out, half* conv, unsigned long width)
{
	...
	convolution7x1TestRunner.SetInput("input", in, width, maxWidth, 7, 0);
	convolution7x1TestRunner.SetInput("conv", conv, 7, 0);
	convolution7x1TestRunner.SetInput("width", (unsigned int)width, 0);

	convolution7x1TestRunner.Run(0);
	...
	convolution7x1TestRunner.GetOutput(string("output"), 0, width, maxWidth, 1, out);
}
