...
#include <filters/medianFilter9x9/medianFilter9x9.h>
...
void medianFilter9x9(UInt32 widthLine, UInt8 **outLine, UInt8 ** inLine)
{
	UInt32 i = 0;
	UInt8 *out;
    int j = 0;
...
void svuMedianFilter9x9(SippFilter *fptr, int svuNo, int runNo)
{
    UInt8 *output;
    UInt8 *iline[9];
...
#ifdef SIPP_USE_MVCV
	medianFilter9x9_asm(fptr->sliceWidth, &output, iline);
#else
  	medianFilter9x9(fptr->sliceWidth, &output, iline);
