...
#include <filters/medianFilter5x5/medianFilter5x5.h>
...
void medianFilter5x5(UInt32 widthLine, UInt8 **outLine, UInt8 ** inLine)
{
	UInt32 i = 0;
	UInt8 *out;
    int j = 0;
...
void svuMedianFilter5x5(SippFilter *fptr, int svuNo, int runNo)
{
    UInt8 *output;
    UInt8 *iline[5];
  ...
  //the output line
	...
	medianFilter5x5_asm(fptr->sliceWidth, &output, iline);//aplying the kernel

#else

    medianFilter5x5(fptr->sliceWidth, &output, iline);//aplying the kernel
