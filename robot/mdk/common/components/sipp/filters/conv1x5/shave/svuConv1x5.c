...
void Convolution1x5(UInt8 **in, UInt8 **out, float *conv, UInt32 inWidth)
{
    UInt32   i, y;
    float sum = 0.0;
    UInt8    *input  = *in;
...
void svuConv1x5(SippFilter *fptr, int svuNo, int runNo)
{
    UInt8 *output;
    UInt8 *iline[1];
...
#ifdef SIPP_USE_MVCV
   Convolution1x5_asm(iline, &output, param->cMat, fptr->sliceWidth);
#else
   Convolution1x5(iline, &output, param->cMat, fptr->sliceWidth);
#endif
}
