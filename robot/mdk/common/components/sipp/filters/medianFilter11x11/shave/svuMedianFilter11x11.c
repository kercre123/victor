...
#include <filters/medianFilter11x11/medianFilter11x11.h>

void medianFilter11x11(UInt32 widthLine, UInt8 **outLine, UInt8 ** inLine)
{
	uint32_t i = 0;
	UInt8 *out;
    int j = 0;
...
void svuMedianFilter11x11(SippFilter *fptr, int svuNo, int runNo)
{
    UInt8 *output;
    UInt8 *iline[11];
...
//the output line
	...
	medianFilter11x11_asm(fptr->sliceWidth, &output, iline);//aplying the kernel



#else
    
	medianFilter11x11(fptr->sliceWidth, &output, iline);
