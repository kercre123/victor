...
#include <filters/medianFilter15x15/medianFilter15x15.h>
...
void medianFilter15x15(UInt32 widthLine, UInt8 **outLine, UInt8 ** inLine)
{
	UInt32 i = 0;
	UInt8 *out;
    int j = 0;
...
void svuMedianFilter15x15(SippFilter *fptr, int svuNo, int runNo)
{
    UInt8 *output;
    UInt8 *iline[15];
    ...
    iline[13]=(UInt8 *)WRESOLVE((void*)fptr->dbLinesIn[0][runNo&1][13], svuNo);
    iline[14]=(UInt8 *)WRESOLVE((void*)fptr->dbLinesIn[0][runNo&1][14], svuNo);
...
#ifdef SIPP_USE_MVCV
	medianFilter15x15_asm(fptr->sliceWidth, &output, iline);
#else
  	medianFilter15x15(fptr->sliceWidth, &output, iline);
