...
//#############################################################################
void svufP406RandNoise(SippFilter *fptr, int svuNo, int runNo)
{
//    UInt32 i;
  ...
  //Input Y line
    UInt16 *inY  = (UInt16 *)WRESOLVE((void*)fptr->dbLinesIn[0][runNo&1][0], svuNo);
  //Output Y line
    UInt16 *outY = (UInt16 *)WRESOLVE(fptr->dbLineOut[runNo&1],       svuNo);

    memcpy(outY, inY, fptr->sliceWidth * 2);
  //Noise 
    //for(i=0; i<fptr->sliceWidth; i++)
    //{
    //    outY[i] = clampU8((inY[i] + (getRandom(mW, mZ) - 0.5f)*inY[i] * param->strength));
    //}
} 
