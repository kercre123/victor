...
void DrvIcbConfigureIrq( u32 source, u32 priority, u32 type ) {
    ...
    // config trap trigger
    SET_REG_WORD( getIcbBase() +( source << 2 ), ( priority << 2 )| type );
...
void DrvIcbEnableIrq( u32 source ) {
  ...
  SET_REG_WORD(getIcbBase() +  ICB_ENABLE_0_OFFSET + t, GET_REG_WORD_VAL( getIcbBase() +  ICB_ENABLE_0_OFFSET + t) |( 1 << (source - t*8) ));
}
...
void DrvIcbIrqClear( u32 source ) {
  u32 t =( source & 0x20 )? 4 : 0;
  SET_REG_WORD( getIcbBase() + ICB_CLEAR_0_OFFSET +  t, 1 << (source - t*8) );
}

void DrvIcbIrqTrigger( u32 source ) {
  u32 t =( source & 0x20 )? 4 : 0;
  SET_REG_WORD(getIcbBase() + ICB_SETINT_0_OFFSET + t, 1 << (source - t*8) );
}
