...
static u32 calculatePllConfigValue(u32 osc_in, u32 sys_clk_desired,u32 * pAchievedFreqKhz);
...
static u32 calculatePllConfigValue(u32 osc_in, u32 sys_clk_desired,u32 * pAchievedFreqKhz)
{
    u32 pll_ctrl_val, freq;
    u32 F_ref;
    u32 F_vco;
    u32 diff, diff_bk;

    u32 in_div, feedback_div, out_div, band;
    u32 in_div_bk = 0, feedback_div_bk = 0, out_div_bk = 0, band_bk = 0;

    diff_bk = 1000000; // 1 GHz difference

    for (in_div = 1 ; in_div < 33 ; ++in_div)
    {
        F_ref = osc_in / in_div ; 
        if ((F_ref >= 10000) && (F_ref <= 50000))
        {
            band = 0;   
            for (out_div = 0 ; out_div < 4 ; ++out_div)
            {
                F_vco = sys_clk_desired  * (1<<out_div);
                if ((F_vco <= 600000) && (F_vco >= 300000))
                {
                    feedback_div = (sys_clk_desired * (1 << out_div) * in_div) / osc_in;
                    if ((feedback_div >= 6) && (feedback_div <= 100))
                    {
                        freq = osc_in * feedback_div / ((1<<out_div) * in_div);

                        if (sys_clk_desired > freq)
                            diff = sys_clk_desired - freq;
                        else
                            diff = freq - sys_clk_desired;

                        if (diff < diff_bk)
                        {
                            in_div_bk = in_div;
                            feedback_div_bk = feedback_div;
                            out_div_bk = out_div;
                            band_bk = band;
                            diff_bk = diff;
                        }
    ...
    pll_ctrl_val = ( (in_div_bk - 1) << 6)       |
                   ( (feedback_div_bk - 1)<< 11) | 
                   ( out_div_bk << 4 )         |
                   ( band_bk << 2);

    freq = osc_in * feedback_div_bk / ((1<<out_div_bk) * in_div_bk);
...
int DrvCprTemporaryInitPLL()
{
    u32 fb,idiv,odiv,bs,bp,pd;
    u32 data;
    u32 refsrc;
    u32 exp_data,count;
    u32 data_v128;
...
//    data={0x0,((fb >> 0) & ((1 << (6 - 0 + 1)) - 1)),((idiv >> 0) & ((1 << (4 - 0 + 1)) - 1)),((odiv >> 0) & ((1 << (1 - 0 + 1)) - 1)),((bs >> 0) & ((1 << (1 - 0 + 1)) - 1)),bp,pd};
    data = (pd & 1) | ((bp&1) << 1) | ((bs & 3)<<2) | ((odiv & 3)<<4) | ((idiv & 31)<<6) | ((fb & ((1<<7)-1))<<11);
...
//     unitTestAssert(GET_REG_WORD_VAL(CPR_RETENTION6_ADR) == data_v128);
    ...
    // Start up the PLL
     ...
     //data={0x0,((refsrc >> 0) & ((1 << (2 - 0 + 1)) - 1)),((fb >> 0) & ((1 << (6 - 0 + 1)) - 1)),((idiv >> 0) & ((1 << (4 - 0 + 1)) - 1)),((odiv >> 0) & ((1 << (1 - 0 + 1)) - 1)),((bs >> 0) & ((1 << (1 - 0 + 1)) - 1)),bp,pd};
     data = 0;
     DrvRegSetBitField(&data,18,3,refsrc);
     DrvRegSetBitField(&data,11,7,fb);
     DrvRegSetBitField(&data, 6,7,idiv);
     DrvRegSetBitField(&data, 4,5,odiv);
     DrvRegSetBitField(&data, 2,2,bs);
     DrvRegSetBitField(&data, 1,1,bp);
     DrvRegSetBitField(&data, 0,1,pd);
...
//
    ...
    while ((!((((exp_data >> 0) & 1) == 1)|(((exp_data >> 2) & 1) == 1))) & (count < 100)) {
       SleepTicks(100);count++;
       exp_data = GET_REG_WORD_VAL(CPR_PLL_STAT0_ADR); 
    }
    ...
    SET_REG_WORD(CPR_AUX_CLK_RST_ADR  , 0x00000000);
     data_v128 = 0x00000000; DrvRegSetBitField(&data_v128, CSS_AUX_DDR_CORE, 1, 1);
