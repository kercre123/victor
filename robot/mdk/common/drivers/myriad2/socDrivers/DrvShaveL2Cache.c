...
#define SHAVE_L2_OP_FLUSH_INVLD 0x1
#define SHAVE_L2_OP_INVLD       0x2
#define SHAVE_L2_OP_FLUSH       0x4
...
typedef struct partitions_users{
	...
	drvShaveL2CachePartitionSizes_enum_t size[MAX_PARTITIONS];
}partitions_cfg_t;
...
static partitions_cfg_t partitions_configuration={
		0,
		{SHAVEPART16KB,SHAVEPART16KB,SHAVEPART16KB,SHAVEPART16KB,SHAVEPART16KB,SHAVEPART16KB,SHAVEPART16KB,SHAVEPART16KB}
};
...
void DrvShaveL2CacheSetupDMAPartitionsNumber (u32 numParts)
{
    ...
    while ((GET_REG_WORD_VAL(L2C_BUSY_ADR) & 0x1));      /* Wait for idle */
    ...
    for(i = 0; i < numParts; i++)
    {
        ...
        DrvShaveL2CacheSetDMAPartId(i, i);

		/* Set the slice LSU L2 Cache partition IDs */
        DrvShaveL2CacheSetLSUPartId(i, i);
...
void PartitionUtil (u32 partNo, u32 operation)
{
   ...
   while ((GET_REG_WORD_VAL(L2C_BUSY_ADR) & 0x2));         //Wait for DONE
...
unsigned int DrvShaveL2CacheAllocateSetPartitions(void)
{
	...
	//We need to keep track of the partition start address inside L2cache
	...
	while ( (i<partitions_configuration.partitions_asked) && (result==0) )
	{
		if (size16KBsectionsLeft<=0){
			...
			unsigned int partitionWord=0x0;
			...
			//This is built using data from the 55_L2_cache.doc Myriad2 document
			switch (partitions_configuration.size[i]){
			case SHAVEPART256KB:{
...
//			SET_REG_WORD(  SLICE_NWN_CPC(partitions_configuration.slices[i]) , partitionWord);
	...
	//And just to make sure everything is all right
	// setup master IDs
