...
int mv171MipiBridgesCount = sizeof(mv171MipiBridges)/sizeof(mv171MipiBridges[0]);
...
// I2C1
static tyI2cConfig i2c1Mv171DefaultConfiguration =
    {
     .device                = GPIO_BITBASH,
     .sclPin                 = I2C1_SCL,
     .sdaPin                = I2C1_SDA,
...
// I2C2
static tyI2cConfig i2c2Mv171DefaultConfiguration =
    {
     .device                = IIC2_DEVICE,
     .sclPin                = I2C2_SCL,
     .sdaPin                = I2C2_SDA,
...
int brd171InitialiseI2C(tyI2cConfig * i2cCfg, I2CM_Device ** i2cDev)
{
    ...
    if (i2cCfg == NULL)
        i2cCfg = &i2c2Mv171DefaultConfiguration;
...
{
        unsigned int i;
        for (i=0;i<(sizeof(mv171MipiBridges)/sizeof(TMipiBridge));i++) {
            icTC358746AssignI2c(&mv171MipiBridges[i], i2cDev);
            icTC358746Reset(&mv171MipiBridges[i]);
        }
        return 0; 
}

static int brd171ReasignMipiBridgesI2cModule(I2CM_Device * i2cDev)
{
        unsigned int i;
        for (i=0;i<(sizeof(mv171MipiBridges)/sizeof(TMipiBridge));i++) {
            icTC358746AssignI2c(&mv171MipiBridges[i], i2cDev);
...
void brd171GpioToI2cDevice(TBrd171I2cDevicesEnum tsh, I2CM_Device ** i2cDev)
{
    ...
    case ToshibaChip1:
         ...
         brd171InitialiseI2C(&i2c2Mv171DefaultConfiguration, i2cDev);
         
         // assign the handler to the i2c modules
         brd171ReasignMipiBridgesI2cModule(*i2cDev);
    ...
    case ToshibaChip3:
     ...
     case ToshibaCam1:
          ...
          brd171InitialiseI2C(&i2c1Mv171DefaultConfiguration, i2cDev);
          break;
