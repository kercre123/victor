...
/// 
/// Low level driver controlling the functions of the Myriad
...
void DrvGpioIrqResetAll()
{
    ...
    for (i = 0; i < 142; i++)
    {
        DrvGpioMode( i, DrvGpioGetMode(i) & (~(D_GPIO_IRQ_SRC_0 | D_GPIO_IRQ_SRC_1)));
    }
    gpioIrqReset++;
}

void DrvGpioIrqConfig( u32 gpio, u32 irSrc, u32 irqType, u32 priority, irq_handler callback)
{
    if( !gpioIrqReset )// if gpio irq reset wasn't done
      DrvGpioIrqResetAll();//by default all gpios will trigger both irqSrc

    if( irSrc & D_GPIO_IRQ_SRC_1 )
      DrvIcbSetupIrq( IRQ_GPIO_1, priority, irqType, callback);
    if( irSrc & D_GPIO_IRQ_SRC_0 )
      DrvIcbSetupIrq( IRQ_GPIO_0, priority, irqType, callback);

    DrvGpioMode( gpio, D_GPIO_MODE_7 | D_GPIO_DIR_IN | irSrc | D_GPIO_WAKEUP_OFF);
}
...
void DrvGpioSetPin(int gpioNum,int value)
{
    ...
    DrvGpioMode(gpioNum, D_GPIO_DIR_OUT | D_GPIO_MODE_7 | D_GPIO_WAKEUP_OFF | D_GPIO_IRQ_SRC_NONE); // Always ensure that GPIO is enabled
...
{
    u32 originalGpioMode;

    originalGpioMode = GET_REG_WORD_VAL(GPIO_MODE0_ADR + 4*gpioNum);

    // Clear the bits we intend to change in order to ensure pin is correctly set as input, 
    // without changing bits we don't want to interfere with
    originalGpioMode &= ~(D_GPIO_MODE_7 | D_GPIO_DIR_IN | D_GPIO_DATA_INV_ON);

    // This function takes full responsibility for making sure that the GPIO is 
    // configured in Direct Mode as input
    DrvGpioMode(gpioNum, originalGpioMode | ( D_GPIO_DIR_IN | D_GPIO_MODE_7) ); // Always ensure that GPIO is enabled
...
void DrvGpioInitialiseRange(const drvGpioInitArrayType initArray)
{
    ...
    if (padsNeedUpdate)
    {
        ...
        SET_REG_WORD(GPIO_PAD_CTRL_ADR, GPIO_SCAN_XFER_CFGREG_TO_SCANREG);         

        for (i=0;i< (totalGpios+regPerCycle);i+=2)
        {
            // Shift out the first two gpio_pad configs into the data register
            SET_REG_WORD(GPIO_PAD_CTRL_ADR, GPIO_SCAN_SHIFT_TWO_REG | GPIO_SCAN_TRIGGER_SHIFT); 
            waitForGpioScanComplete();
            ...
            valGpioOutA = (GET_REG_WORD_VAL(GPIO_PAD_CFG_RD_WR_DATA_ADR) >> 0 ) & GPIO_SCAN_REG_MASK ;
            valGpioOutB = (GET_REG_WORD_VAL(GPIO_PAD_CFG_RD_WR_DATA_ADR) >> GPIO_SCAN_BITS_PER_REGISTER ) & GPIO_SCAN_REG_MASK ;
            ...
            // Write back the these register after optionally patching them
             SET_REG_WORD(GPIO_PAD_CFG_RD_WR_DATA_ADR, (valGpioOutB << GPIO_SCAN_BITS_PER_REGISTER) | valGpioOutA ); 
        }
        // If we are still shifting, wait until that finishes
        waitForGpioScanComplete();
        // Finally write the scan chain back into the CFG registers to make it all active
        SET_REG_WORD(GPIO_PAD_CTRL_ADR, GPIO_SCAN_XFER_SCANREG_TO_CFGREG);         
    }
...
static inline void waitForGpioScanComplete(void)
{
    ...
    GET_REG_WORD_VAL(GPIO_PAD_CTRL_ADR);
    // Then wait for bit to clear
    while (GET_REG_WORD_VAL(GPIO_PAD_CTRL_ADR) & GPIO_SCAN_IN_PROGRESS);
}
...
void GpioPadSetAllTable(u32 table[])
{
    ...
    {
         waitForGpioScanComplete();
         SET_REG_WORD(GPIO_PAD_CFG_RD_WR_DATA_ADR, table[GPIO_NUM - i - 1] & 0xFFF); // load 
         SET_REG_WORD(GPIO_PAD_CTRL_ADR, GPIO_SCAN_SHIFT_ONE_REG | GPIO_SCAN_TRIGGER_SHIFT); // shift in register
    }
    waitForGpioScanComplete();
    SET_REG_WORD(GPIO_PAD_CTRL_ADR, GPIO_SCAN_XFER_SCANREG_TO_CFGREG);          // latch in
}
...
void GpioPadGetAllTable(u32 table[])
{
    int i;
    waitForGpioScanComplete();
    SET_REG_WORD(GPIO_PAD_CTRL_ADR, GPIO_SCAN_XFER_CFGREG_TO_SCANREG);          // latch out
    SET_REG_WORD(GPIO_PAD_CTRL_ADR, GPIO_SCAN_SHIFT_ONE_REG | GPIO_SCAN_TRIGGER_SHIFT);  // shift first register
    for (i=0;i<GPIO_NUM;++i) 
    {
        waitForGpioScanComplete();
        // The first register is now in the upper 12 bits of the CFG_RD register, read it out
        table[GPIO_NUM - i - 1] = (GET_REG_WORD_VAL(GPIO_PAD_CFG_RD_WR_DATA_ADR) >> GPIO_SCAN_BITS_PER_REGISTER) & 0xFFF ; 
        SET_REG_WORD(GPIO_PAD_CTRL_ADR, GPIO_SCAN_SHIFT_ONE_REG | GPIO_SCAN_TRIGGER_SHIFT);              // shift again
    }                                                                                
}
...
void GpioPadSet(u32 gpioNo, u32 val)
{
    ...
    unsigned int oldVal;
    waitForGpioScanComplete();
    SET_REG_WORD(GPIO_PAD_CTRL_ADR, 0x004);                                         // latch out all the values 
    for (i=0 ; i<GPIO_NUM - gpioNo + (CFG_SCAN_LEN-1) ;++i)
    {
        waitForGpioScanComplete();
        SET_REG_WORD(GPIO_PAD_CTRL_ADR, 0xC01);                                     // shift 
    }

    waitForGpioScanComplete();
    oldVal = GET_REG_WORD_VAL(GPIO_PAD_CFG_RD_WR_DATA_ADR) & 0xfff000;             // avoid destriong the neigbour values (24b now)
    oldVal |= val & 0xfff;
    SET_REG_WORD(GPIO_PAD_CFG_RD_WR_DATA_ADR, oldVal);                             // load 

    for (i=0 ; i < gpioNo + 1 ;++i)
    {
        waitForGpioScanComplete();
        SET_REG_WORD(GPIO_PAD_CTRL_ADR, 0xC01);                                     // shift back
    }    
    waitForGpioScanComplete();                                                      // wait for the previous shift to complete        
    SET_REG_WORD(GPIO_PAD_CTRL_ADR, 0x002);                                         // latch in all the values 
}
...
//--------------------------------------------------------------------------------------------------------------------------------
    ...
    {    
         waitForGpioScanComplete();                                             // wait for the previous shift to complete    
         SET_REG_WORD(GPIO_PAD_CTRL_ADR, 0xF001);                               // shift with 20 registers (20*12=240)
         noShft -= 20;
    }
    while (noShft >= 5)
    {    
         waitForGpioScanComplete();                                             // wait for the previous shift to complete
         SET_REG_WORD(GPIO_PAD_CTRL_ADR, 0x3C01);                               // shift with 5 registers (5*12=60)
         noShft -= 5;
    }
    for (i=0 ; i < noShft ;++i)
    {
...
//--------------------------------------------------------------------------------------------------------------------------------    
    waitForGpioScanComplete();                                                  // wait for the previous shift to complete    
    oldVal = GET_REG_WORD_VAL(GPIO_PAD_CFG_RD_WR_DATA_ADR) & 0xfff000;          // avoid destriong the neigbour values
    oldVal |= val & 0xfff;
    SET_REG_WORD(GPIO_PAD_CFG_RD_WR_DATA_ADR, oldVal);                          // load 
//--------------------------------------------------------------------------------------------------------------------------------    
    noShft = gpioNo +1;
...
//--------------------------------------------------------------------------------------------------------------------------------    
    ...
    maskSet &= 0xfff;    
    waitForGpioScanComplete();                                                   // wait for the previous shift to complete    
    SET_REG_WORD(GPIO_PAD_CFG_RD_WR_DATA_ADR, (GET_REG_WORD_VAL(GPIO_PAD_CFG_RD_WR_DATA_ADR) & (~maskClear)) | maskSet );  // load 
//--------------------------------------------------------------------------------------------------------------------------------    
    noShft = gpioNo + 1;
    while (noShft >= 20)
    {    
         waitForGpioScanComplete();                                              // wait for the previous shift to complete    
         SET_REG_WORD(GPIO_PAD_CTRL_ADR, 0xF001);                                // shift with 20 registers
         noShft -= 20;
    }
    while (noShft >= 5)
    {    
         waitForGpioScanComplete();                                              // wait for the previous shift to complete    
         SET_REG_WORD(GPIO_PAD_CTRL_ADR, 0x3C01);                                // shift with 5 registers
         noShft -= 5;
    }
    for (i=0 ; i < noShft ;++i)
    {
...
{  
   ...
   {
       waitForGpioScanComplete();                                                // wait for the previous shift to complete    
       SET_REG_WORD(GPIO_PAD_CTRL_ADR, 0xC01);                                   // shift to the right register 
   }
   waitForGpioScanComplete();                                                    // wait for the previous shift to complete       
   PAD = GET_REG_WORD_VAL(GPIO_PAD_CFG_RD_WR_DATA_ADR) & 0xfff;                  // get the register
   return PAD;
}
...
//--------------------------------------------------------------------------------------------------------------------------------    
    for (i=0 ; i<(gpioEnd-gpioStart+1) ; ++i)
    {
         ...
         oldVal |= val & 0xfff;
         SET_REG_WORD(GPIO_PAD_CFG_RD_WR_DATA_ADR, oldVal);                     // load    
         SET_REG_WORD(GPIO_PAD_CTRL_ADR, 0x0C01);                                // shift 
    }
//--------------------------------------------------------------------------------------------------------------------------------    
    noShft = gpioStart;
