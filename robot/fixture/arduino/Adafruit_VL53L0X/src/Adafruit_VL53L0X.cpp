...
*/
/**************************************************************************/
boolean Adafruit_VL53L0X::calibrate(uint8_t i2c_addr, boolean debug, TwoWire *i2c,
                                    VL53L0X_Sense_config_t vl_config) {
    // Initialize Comms
    pMyDevice->I2cDevAddr = VL53L0X_I2C_ADDR;  // default
    pMyDevice->comms_type = 1;
...
boolean Adafruit_VL53L0X::isAverageReadingOkay(uint16_t avgReading) {
    uint16_t calibThreshMin = kTargetDistance - kCalibrationThreshold;
    uint16_t calibThreshMax = kTargetDistance + kCalibrationThreshold;
    return ((avgReading >= calibThreshMin) &&
            (avgReading <= calibThreshMax));
}