...
$Header:
//source/qcom/qct/platform/uefi/workspaces/pweber/apps/8x26_emmcdl/emmcdl/main/latest/inc/dload.h#2
...
#define CMD_MEM_UNFRAMED_READ_REQ \
  0x14 /* Memory unframed read request   (received)      */
#define CMD_MEM_UNFRAMED_READ_RESP \
  0x15 /* Memory unframed read response  (transmitted)   */
...
#define CMD_QPST_COOKIE_READ_REQ \
  0x19 /* QPST cookie read request      (received)    */
#define CMD_QPST_COOKIE_READ_RESP \
  0x1A /* QPST cookie read response     (transmitted) */
...
#define EHOST_SIMPLE_READ_REQ \
  0x03  // Read some number of bytes from phone memory
...
#define EHOST_CLOSE_RSP \
  0x16  // Response to close and flush last partial write to Flash
#define EHOST_SECURITY_REQ \
  0x17  // Send Security mode to use for programming images
#define EHOST_SECURITY_RSP 0x18  // Response to Send Security mode
#define EHOST_PARTITION_REQ \
  0x19  // Send partition table to use for programming images
#define EHOST_PARTITION_RSP 0x1A  // Response to send partition table
#define EHOST_OPEN_MULTI_REQ \
  0x1B  // Open for writing image (Multi-image mode only)
...
class Dload {
 public:
  ...
  int ConnectToFlashProg(unsigned char ver);  // ver=2 for byte addressing and
                                              // ver=3 for sector addressing
