...
$Header:
//source/qcom/qct/platform/uefi/workspaces/pweber/apps/8x26_emmcdl/emmcdl/main/latest/inc/serialport.h#3
...
class SerialPort {
 public:
  ...
  int Flush();
  int SendSync(unsigned char *out_buf, int out_length, unsigned char *in_buf,
               int *in_length);
 ...
 private:
  int HDLCEncodePacket(unsigned char *in_buf, int in_length,
                       unsigned char *out_buf, int *out_length);
  int HDLCDecodePacket(unsigned char *in_buf, int in_length,
                       unsigned char *out_buf, int *out_length);
