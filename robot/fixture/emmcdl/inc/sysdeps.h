...
static __inline__ bool emmcdl_thread_create(emmcdl_thread_func_t func,
                                            void* arg) {
...
static __inline__ int emmcdl_open_mode(const char* path, int options,
                                       int mode) {
...
static __inline__ void emmcdl_sleep_ms(int mseconds) { Sleep(mseconds); }

extern int emmcdl_socket_accept(int serverfd, struct sockaddr* addr,
                                socklen_t* addrlen);

#undef accept
#define accept ___xxx_accept

extern int emmcdl_setsockopt(int fd, int level, int optname, const void* optval,
                             socklen_t optlen);
...
static __inline__ int emmcdl_socket_setbufsize(int fd, int bufsize) {
  int opt = bufsize;
  return emmcdl_setsockopt(fd, SOL_SOCKET, SO_RCVBUF, (const void*)&opt,
                           sizeof(opt));
}
...
static __inline__ int emmcdl_open_mode(const char* pathname, int options,
                                       int mode) {
...
static __inline__ int emmcdl_close(int fd) { return close(fd); }
#undef close
#define close ____xxx_close
...
static __inline__ int emmcdl_unlink(const char* path) { return unlink(path); }
#undef unlink
#define unlink ___xxx_unlink
...
static __inline__ int emmcdl_socket_accept(int serverfd, struct sockaddr* addr,
                                           socklen_t* addrlen) {
  int fd;
...
static __inline__ bool emmcdl_thread_create(emmcdl_thread_func_t start,
                                            void* arg) {
  pthread_attr_t attr;
...
static __inline__ int emmcdl_setsockopt(int fd, int level, int optname,
