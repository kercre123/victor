...
$Header:
//source/qcom/qct/platform/uefi/workspaces/pweber/apps/8x26_emmcdl/emmcdl/main/latest/inc/sparse.h#1
...
typedef struct _SPARSE_HEADER {
  ...
  uint16_t
      wVerMajor;  // 0x1 Major version, reject version with higher major version
  uint16_t
      wVerMinor;  // 0x0 Minor version, accept version with higher minor version
  ...
  uint32_t dwImageChecksum;  // CRC32 checksum of the original data counting do
                             // not care 802.3 polynomial
} SPARSE_HEADER;
...
typedef struct _CHUNK_HEADER {
  ...
  uint32_t dwTotalSize;  // Number of bytes in input file including chunk header
