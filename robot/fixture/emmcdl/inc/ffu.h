...
$Header:
//source/qcom/qct/platform/uefi/workspaces/pweber/apps/8x26_emmcdl/emmcdl/main/latest/inc/ffu.h#7
...
typedef struct _STORE_HEADER {
  ...
  uint16_t FullFlashMajorVersion,
      FullFlashMinorVersion;  // FFU version, i.e. the image format
  char szPlatformId[192];     // string which indicates what device this FFU is
                              // intended to be written to
  uint32_t dwBlockSizeInBytes;  // size of an image block in bytes � the
                                // device�s actual sector size may differ
  uint32_t
      dwWriteDescriptorCount;  // number of write descriptors to iterate through
  uint32_t dwWriteDescriptorLength;  // total size of all the write descriptors,
                                     // in bytes (included so they can be read
                                     // out up front and interpreted later)
  uint32_t
      dwValidateDescriptorCount;  // number of validation descriptors to check
  uint32_t dwValidateDescriptorLength;  // total size of all the validation
                                        // descriptors, in bytes
  uint32_t dwInitialTableIndex;  // block index in the payload of the initial
                                 // (invalid) GPT
  uint32_t
      dwInitialTableCount;  // count of blocks for the initial GPT, i.e. the GPT
                            // spans blockArray[idx..(idx + count -1)]
  uint32_t dwFlashOnlyTableIndex;  // first block index in the payload of the
                                   // flash-only GPT (included so safe flashing
                                   // can be accomplished)
...
enum DISK_ACCESS_METHOD { DISK_BEGIN = 0, DISK_SEQ = 1, DISK_END = 2 };
...
class FFUImage {
 ...
 private:
  ...
  int AddEntryToRawProgram(char *szRawProgram, char *szFileName,
                           uint64_t ui64FileOffset, int64_t i64StartSector,
                           uint64_t ui64NumSectors);
  int ReadGPT(void);
