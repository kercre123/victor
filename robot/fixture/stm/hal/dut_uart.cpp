...
static volatile int framing_err_cnt = 0, overflow_err_cnt = 0,
                    drop_char_cnt = 0;
}  // namespace DUT_UART
...
#define USART USART2
    ...
    ConsolePrintf(
        "  HCLK=%u SYSCLK=%u PCLK1=%u PCLK2=%u\n",
        RCC_ClocksStatus.HCLK_Frequency, RCC_ClocksStatus.SYSCLK_Frequency,
    ...
    ConsolePrintf("  CR1=0x%04x CR2=0x%04x CR3=0x%04x BRR=0x%04x\n", USART->CR1,
                  USART->CR2, USART->CR3, USART->BRR);
    ConsolePrintf(
        "  UE=%u TE=%u RE=%u HDSEL=%u\n", (USART->CR1 & USART_CR1_UE) > 0,
        (USART->CR1 & USART_CR1_TE) > 0, (USART->CR1 & USART_CR1_RE) > 0,
        (USART->CR3 & USART_CR3_HDSEL) > 0);
#endif

    dut_uart_delay_bit_time_(
        2, baud);  // wait at least 1 bit-time for uart peripheral to spin up
    dut_uart_delay_bit_time_(
        20, baud);  // 2 byte times on the wire for receiver to sync line change
    current_baud = baud;
    rx.enabled = 1;
  }
}
...
void DUT_UART::putchar(char c) {
  if (current_baud > 0)  // initialized
  {
    ...
    // volatile int junk = USART2->SR; //TC bit is cleared by 'read from the
    // USART_SR register followed by a write to the USART_DR register'
    USART2->DR = c;
    // while( !(USART2->SR & USART_SR_TXE) ); //dat moved to shift register (ok
    // to write a new data) while( !(USART2->SR & USART_SR_TC)  ); //wait for
    // transmit complete
  }
}
...
static int dut_uart_getchar_(void) {
  ...
  {
    if( USART2->SR & (USART_SR_ORE | USART_SR_FE) ) { //framing and/or overrun
  error junk = USART2->DR; //flush the dr & shift register junk = USART2->DR;
  ...
  */
  ...
  if (DUT_UART::rx.len > 0) {
    ...
    if (++DUT_UART::rx.r >= DUT_UART::rx_fifo_size) DUT_UART::rx.r = 0;
    DUT_UART::rx.len--;
  } else
    c = -1;  // no data
...
char* DUT_UART::getline(char* buf, int bufsize, int timeout_us, int* out_len) {
  ...
  do  // read 1 char (even if timeout==0)
  {
    c = dut_uart_getchar_();
    if (c > 0 &&
        c < 0x80) {  // ignore null and non-ascii; messes with ascii parser
      if (c == '\r' || c == '\n')
        eol = 1;
      else if (len < maxlen)
        buf[len++] = c;
...
extern "C" void USART2_IRQHandler(void) {
  ...
  {
    if (status & USART_SR_ORE) DUT_UART::overflow_err_cnt++;
    if (status & USART_SR_FE) DUT_UART::framing_err_cnt++;
