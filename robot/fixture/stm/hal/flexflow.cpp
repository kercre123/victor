...
void FLEXFLOW::putchar(char c) {
  if (current_baud > 0)  // initialized
  {
    ...
    volatile int junk =
        USART1->SR;  // TC bit is cleared by 'read from the USART_SR register
                     // followed by a write to the USART_DR register'
    USART1->DR = c;
    // while( !(USART1->SR & USART_SR_TXE) ); //dat moved to shift register (ok
    // to write a new data) while( !(USART1->SR & USART_SR_TC)  ); //wait for
    // transmit complete
  }
