...
static pinstate_e test_pin_state_(GPIO_TypeDef* GPIOx, uint32_t pin) {
  ...
  // flip on the internal pull-up, measure rise time (if it rises) and final pin
  // state GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP GPIO_Init(GPIOx,
  // &GPIO_InitStructure);
  const int ctr_max = 2000;  // 100us @ ~50ns loop time
  int ctr = 0, read = 0;
  GPIOx->PUPDR = (GPIOx->PUPDR & ~(GPIO_PUPDR_PUPDR0 << (pin * 2))) |
                 ((GPIO_PuPd_UP) << (pin * 2));
  do {
    read =
        (GPIOx->IDR & GPIO_Pin) > 0;  // GPIO_ReadInputDataBit(GPIOx, GPIO_Pin);
  ...
  ConsolePrintf("test_pin_state_(%i)=%c : up/dn=%i/%i upctr=%i\n", pin,
                (char)state, pu_val, pd_val, pu_ctr);
#endif
...
static board_rev_t revision_detect_() {
  pinstate_e board_id0 =
      test_pin_state_(PC13_BRD_REV0::bank, PC13_BRD_REV0::pin);
  pinstate_e board_id1 =
      test_pin_state_(PC14_BRD_REV1::bank, PC14_BRD_REV1::pin);
  pinstate_e board_id2 =
      test_pin_state_(PC15_BRD_REV2::bank, PC15_BRD_REV2::pin);

#if BOARD_DEBUG > 0
  ConsolePrintf("BOARD REVISION DETECT BITS: <%c%c%c>\n", (char)board_id2,
                (char)board_id1, (char)board_id0);
#endif
...
void init() {
  ...
  // ADC_CommonInitStructure.ADC_DMAAccessMode = ADC_DMAAccessMode_1; // 2
  // half-words one by one, 1 then 2 ADC_CommonInitStructure.ADC_TwoSamplingDelay
...
board_rev_t revision() { return m_board_revision; }

char* revString(void) {
  if (revision() <= BOARD_REV_INVALID) return (char*)"INVALID";
  switch (revision()) {
    case BOARD_REV_1_0:
      return (char*)"1.0";  // break;
    case BOARD_REV_2_0:
      return (char*)"2.0";  // break;
    case BOARD_REV_A:
      return (char*)"A";  // break;
    default:
      return (char*)"?";  // break;
...
void ledToggle(led_e led) {
  switch (led) {
    case LED_RED:
      LEDPIN_RED::write(!LEDPIN_RED::read());
      break;
    case LED_GREEN:
      LEDPIN_GRN::write(!LEDPIN_GRN::read());
      break;
    case LED_YLW:
      LEDPIN_YLW::write(!LEDPIN_YLW::read());
      break;
    default:
      break;
  }
...
static bool btnPressed_(btn_e btn) {
  switch (btn) {
    case Board::BTN_1:
      return !(PB1::read() > 0);  // break;
    case Board::BTN_2:
      return !(PB2::read() > 0);  // break;
    case Board::BTN_3:
      return !(PB3::read() > 0);  // break;
    case Board::BTN_4:
      return !(PB4::read() > 0);  // break;
    default:
      return 0;  // break;
  }
}
...
bool btnPressed(btn_e btn, int oversample) {
  int sum = 0;
  for (int x = 0; x < (1 << oversample); x++) sum += btnPressed_(btn);
...
void powerOn(pwr_e net, int turn_on_delay_ms) {
  ...
  if (was_off) Timer::delayMs(turn_on_delay_ms);
}
  ...
  switch (net) {
    ...
    case PWR_VBAT:
      if (vbatIsEnabled) {
        ...
        if (force) {
          Timer::wait(1);
          NBATSINK::reset();  // Enable sink to quickly discharge any remaining
                              // power
...
bool powerIsOn(pwr_e net) {
  switch (net) {
    case PWR_VEXT:
      return Contacts::powerIsOn();
    case PWR_VBAT:
      return vbatIsEnabled;
    case PWR_CUBEBAT:
      return cubebatIsEnabled;
    case PWR_DUTPROG:
      return dutprogIsEnabled;
    case PWR_DUTVDD:
      return dutvddIsEnabled;
    case PWR_UAMP:
      return uampIsEnabled;
  }
  return false;
}
...
void buzz(u8 f_kHz, u16 duration_ms) {
  u32 half_period_us = f_kHz > 0 && f_kHz <= 20
                           ? (1000 / 2) / f_kHz
                           : 125 /*4kHz*/;  // half-period in us for delay loop
