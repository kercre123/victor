...
void on(uint8_t n);  // turn on led<n>, n={0..num-1}. Note, only 1 LED can be on
                     // at a time
...
//@param display_time_us - time to light each enabled led (total frame time ~=
//display_time_us * num)
...
int getDx(uint8_t n);  // return D{1..4} to indicate which physical component
                       // led<n> is part of
