...
enum adc_chan_e {
  ...
  ADC_DUT_RX = ADC_Channel_2,  // PA2   XXX: Cozmo Fix 1.5 D1_BP4, GrabADC(), BP
                               // LED testing
  ADC_DUT_TX = ADC_Channel_3,  // PA3   XXX: Cozmo Fix 1.5 D3_BP3, GrabADC(), BP
                               // LED testing
  ADC_DUT_CS = ADC_Channel_4,    // PA4
  ADC_DUT_SCK = ADC_Channel_5,   // PA5
  ADC_DUT_MISO = ADC_Channel_6,  // PA6   XXX: Cozmo Fix 1.5 D2_BP2, GrabADC(),
                                 // BP LED testing
  ADC_DUT_MOSI = ADC_Channel_7,  // PA7   XXX: Cozmo Fix 1.5 D4_BP1, GrabADC(),
                                 // BP LED testing
  ...
  ADC_DUT_ADC2 = ADC_Channel_13,  // PC3   XXX: Cozmo Fix 1.5 ADC_ENCA,
                                  // QuickADC(), ReadEncoder()
  ADC_DUT_ADC3 = ADC_Channel_14,  // PC4   XXX: Cozmo Fix 1.5 ADC_ENCB,
                                  // QuickADC(), ReadEncoder()
...
bool btnPressed(btn_e btn,
                int oversample = 6);  // true if button is pressed. 2^oversample
                                      // for average/glitch filter

// poll for debounce and edge detect. Includes hysteresis, min edge time is
// sample period * sample limit sample period/limit determine filtering
// heuristics. < 0 for either param resets the state machine.
//@return -1 released (edge), 1 pressed (edge), 0 no change
int btnEdgeDetect(btn_e btn, int sample_period_us = 5000,
                  int sample_limit = 10);

void powerOn(pwr_e net, int turn_on_delay_ms =
                            2);  // Wait 'turn_on_delay_ms' for voltage to
                                 // stabilize (delay ignored if power already on)
void powerOff(
    pwr_e net, int turn_off_delay_ms = 50,
    bool force = 1);  // Wait 'turn_off_delay_ms' for power to drain out (delay
                      // ignored if power already off). if force=1, drive
                      // available current sinks (if any) to discharge quickly.
...
// read ADC[mV]. 2^oversample = # samples (averaged). pin_chg_us = charge delay
// pin.cfg->sample (-1 skips pin cfg/restore)
