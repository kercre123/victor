...
void init() {
  ...
  I2C_Send16(
      INA220_VEXT_BUS_ADDR, 0,
      7 + (1 << 3));  // Measure voltage(9-bit,84us) +
                      // current(10-bit,148uS,+/-40mV -> 2A @ 0.02R full-scale)
  I2C_Send16(INA220_VEXT_BUS_ADDR, 5,
             0);  // No calibration - we can do our own math

  // init VBAT monitor IC
  I2C_Send16(
      INA220_VBAT_BUS_ADDR, 0,
      7 + (1 << 3));  // Measure voltage(9-bit,84us) +
                      // current(10-bit,148uS,+/-40mV -> 2A @ 0.02R full-scale)
  I2C_Send16(INA220_VBAT_BUS_ADDR, 5,
             0);  // No calibration - we can do our own math

  // init CUBEBAT monitor IC
  // I2C_Send16(INA220_CBAT_BUS_ADDR, 0, 7 + (1<<3) + (3<<11)); // Measure
  // voltage(9-bit,84us) + current(10-bit,148uS,GAIN/8,+/-320mV -> 106.666mA @ 3R
  // full-scale)
  I2C_Send16(INA220_CBAT_BUS_ADDR, 0,
             7 + (1 << 3) + (1 << 11));  // Measure voltage(9-bit,84us) +
                                         // current(10-bit,148uS,GAIN/2,+/-80mV
                                         // -> 156.863mA @ 0.51R full-scale)
  I2C_Send16(INA220_CBAT_BUS_ADDR, 5,
             0);  // No calibration - we can do our own math

  // init UAMP monitor IC
  I2C_Send16(
      INA220_UAMP_BUS_ADDR, 0,
      7 + (1 << 3));  // Measure voltage(9-bit,84us) +
                      // current(10-bit,148uS,+/-40mV -> 4mA @ 10R full-scale)
  I2C_Send16(INA220_UAMP_BUS_ADDR, 5,
             0);  // No calibration - we can do our own math
}
...
static const int sample_time_us =
    sample_time_voltage_us +
    sample_time_current_us;  // V+I sampling active (Mode 7)
...
static inline int32_t iscale_ma_(int32_t shunt_reg, uint8_t address) {
  switch (address) {
    case INA220_VEXT_BUS_ADDR:
      return shunt_reg >> 1;  // R=0.02: FS 40mV  = reg val 4000 -> 2000mA
    case INA220_VBAT_BUS_ADDR:
      return shunt_reg >> 1;  // R=0.02: FS 40mV  = reg val 4000 -> 2000mA
    // case INA220_CBAT_BUS_ADDR: return shunt_reg / 300;  //R=3.0 : FS 320mV =
    // reg val 32000 -> 106.666mA
    case INA220_CBAT_BUS_ADDR:
      return shunt_reg / 51;  // R=0.51 : FS 80mV  = reg val 8000 -> 156.863mA
    // case INA220_UAMP_BUS_ADDR: return shunt_reg / 1000; //R=10  : FS 40mV  =
    // reg val 4000 -> 4mA
    case INA220_UAMP_BUS_ADDR:
      return shunt_reg;  // R=10  : FS 40mV  = reg val 4000 -> 4000uA -- PSYCH!
                         // CONVERT THIS ONE TO UA
  }
  return 0;
}
...
int32_t getVoltageMv(pwr_e net, int oversample) {
  switch (net) {
    ...
    case PWR_CUBEBAT:
      return getVoltageMv_(oversample, INA220_CBAT_BUS_ADDR);
    case PWR_UAMP:
      return getVoltageMv_(oversample, INA220_UAMP_BUS_ADDR);
    case PWR_DUTVDD:
      return Board::getAdcMv(ADC_DUT_VDD_IN, oversample)
             << 1;  // this input has a resistor divider
    case PWR_DUTPROG:
      break;
  }
  return 0;
...
int32_t getCurrentMa(pwr_e net, int oversample) {
  switch (net) {
    ...
    case PWR_CUBEBAT:
      return getCurrentMa_(oversample, INA220_CBAT_BUS_ADDR);
    case PWR_UAMP:
      return getCurrentMa_(oversample, INA220_UAMP_BUS_ADDR) /
             1000;  // UAMP scales to uA
    case PWR_DUTPROG:
    case PWR_DUTVDD:
      break;
  }
...
int32_t getCurrentUa(pwr_e net, int oversample) {
  if (net == PWR_UAMP)
    return getCurrentMa_(oversample, INA220_UAMP_BUS_ADDR);  // UAMP scales to
                                                             // uA
  return 0;
}

void setDoubleSpeed(bool enable) { CLOCK_WAIT = enable ? 0 : 1; }
