...
void Opto::tick(void)
{
  ...
  tof_status = readReg(0, TOF_SENSOR_ADDRESS, RESULT_RANGE_STATUS);
  tof_reading      = readReg16(0, TOF_SENSOR_ADDRESS, RESULT_RANGE_STATUS + 10);
  tof_signal_rate  = readReg16(0, TOF_SENSOR_ADDRESS, RESULT_RANGE_STATUS + 6);
  tof_ambient_rate = readReg16(0, TOF_SENSOR_ADDRESS, RESULT_RANGE_STATUS + 8);
  tof_spad_count   = readReg16(0, TOF_SENSOR_ADDRESS, RESULT_RANGE_STATUS + 2);
  ...
  writeReg(0, TOF_SENSOR_ADDRESS, SYSRANGE_START, 0x01);
}

tof_dat_t* Opto::read(void)
{
  /*static uint32_t Tlast = 0;
  if( Timer::elapsedUs(Tlast) > 5000 ) {
    Tlast = Timer::get();
    Opto::tick();
  }*/
  ...
  Timer::wait(5000); //wait 1 tick for sensor read
  ...
  static tof_dat_t tof;
  memset( &tof, 0, sizeof(tof_dat_t) );
  tof.status = tof_status;
  tof.reading = tof_reading;
  tof.signal_rate = tof_signal_rate;
  tof.ambient_rate = tof_ambient_rate;
  tof.spad_count = tof_spad_count;
...
{
  if( optoActive ) return;
  I2C::init(); //I2C::capture();
