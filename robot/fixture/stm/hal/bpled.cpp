...
static const bpled_t bpled[BPLED::num] = {
    ...
    {2, BPLED::COLOR_RED,
     (SR_BIT_BPA1 | 0 | SR_BIT_BPGN | SR_BIT_BPBL | SR_BIT_BLUE | SR_BIT_RED)},
    {2, BPLED::COLOR_GRN,
     (SR_BIT_BPA1 | SR_BIT_BPRD | 0 | SR_BIT_BPBL | SR_BIT_BLUE | SR_BIT_RED)},
    {2, BPLED::COLOR_BLU,
     (SR_BIT_BPA1 | SR_BIT_BPRD | SR_BIT_BPGN | 0 | SR_BIT_BLUE | SR_BIT_RED)},
    {3, BPLED::COLOR_RED,
     (SR_BIT_BPA2 | 0 | SR_BIT_BPGN | SR_BIT_BPBL | SR_BIT_BLUE | SR_BIT_RED)},
    {3, BPLED::COLOR_GRN,
     (SR_BIT_BPA2 | SR_BIT_BPRD | 0 | SR_BIT_BPBL | SR_BIT_BLUE | SR_BIT_RED)},
    {3, BPLED::COLOR_BLU,
     (SR_BIT_BPA2 | SR_BIT_BPRD | SR_BIT_BPGN | 0 | SR_BIT_BLUE | SR_BIT_RED)},
    {4, BPLED::COLOR_RED,
     (SR_BIT_BPA3 | 0 | SR_BIT_BPGN | SR_BIT_BPBL | SR_BIT_BLUE | SR_BIT_RED)},
    {4, BPLED::COLOR_GRN,
     (SR_BIT_BPA3 | SR_BIT_BPRD | 0 | SR_BIT_BPBL | SR_BIT_BLUE | SR_BIT_RED)},
    {4, BPLED::COLOR_BLU,
     (SR_BIT_BPA3 | SR_BIT_BPRD | SR_BIT_BPGN | 0 | SR_BIT_BLUE | SR_BIT_RED)}};
...
void enable() {
  if (!bpled_enabled) {
    ...
    TestCommon::powerOnProtected(PWR_VBAT, 100 /*ms*/, 50 /*mA*/,
                                 2);  // power on + short circuit check
    writeShiftReg(0);
  }
  bpled_enabled = 1;
}
...
void on(uint8_t n) { writeShiftReg(n < BPLED::num ? bpled[n].srval : 0); }

void off() { writeShiftReg(0); }
...
int getDx(uint8_t n) { return n < BPLED::num ? bpled[n].dx : 0; }
...
static inline char* colorString_(color_e c) {
  switch (c) {
    case COLOR_RED:
      return (char*)"RED";  // break;
    case COLOR_GRN:
      return (char*)"GRN";  // break;
    case COLOR_BLU:
      return (char*)"BLU";  // break;
    default:
      return (char*)"???";  // break; //COLOR_INVALID
  }
}

char* description(uint8_t n) {
  static char s[10];
  int len = sprintf(s, "D%u.%s", (n < BPLED::num ? bpled[n].dx : 255),
                    colorString_(getColor(n)));
  s[len] = '\0';
  return s;
}
