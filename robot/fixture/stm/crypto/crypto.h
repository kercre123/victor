...
#define _CRYPTO_H_
/*
** Crypto implementation based on public domain code.  Original author:  Doug
*Whiting, Hifn, 2005
**
** The code was optimized by Nathan Monson to work efficiently with ARM
** The optimized version is Copyright (c) 2011 Nathan Monson and released to
*Anki without restrictions or royalty
*/
...
#ifndef FIXTURE
#define TARGET_IMAGE_SIZE \
  (56 * 1024)  // 56KB is all the flash we have (minus bootloader)
...
#define SAFE_PAYLOAD_SIZE \
  2048  // Must be a power of 2!	Does not need to match flash block size.
...
// It must be exactly 32 bytes, because there is no secure way to place anything
// but a 16-byte nonce and 16-byte mac here All parameters in the nonce (first
// 16-bytes) are cryptographically secured by the mac while still being
// cleartext readable
typedef struct {
  u32b taga;  // Set to SAFE_PLATFORM_TAG - prevents the boot loader from
              // loading the wrong platform's binaries
  u32b tagb;  // Could be used for future expansion - this is the only available
              // space
  u32b guid;  // MUST NEVER REPEAT!  Low 16-bits are build number, high 16-bits
              // are effectively random
  u32b blockFlags;  // Flags for this block, of the form BLOCK_FLAG_XXX - unique
                    // for each block
  u32b mac[4];  // 16-byte message authentication code, indicating this block
                // has not been tampered with
} safe_header;

// The ota_header structure appears just once in the OTA output
// It must be exactly 32 bytes, because there is no secure way to place anything
// but a 16-byte nonce and 16-byte mac here
typedef struct {
  u32b tag;  // Set to OTA_PLATFORM_TAG - prevents the boot loader from loading
             // incompatible binaries
  u16b srclen;   // Source (compressed/encrypted) length in bytes - excluding
                 // header
  u16b destlen;  // Destination length in 16-bit words
  u32b guid;  // MUST NEVER REPEAT!  Unique identifier for this image - usually
              // set to gettimeofday() - in hopes we won't repeat
...
// Block flag bits
#define BLOCK_FLAG_BLOCK \
  0x1FF  // Mask for this block's address - 1MB ought to be enough for anyone!
#define BLOCK_FLAG_LAST 0x4000  // Set on the last block
#define BLOCK_FLAG_SHORT_LAST \
  0x8000  // Set on the last block for files <= 128KB (old fixtures)
...
#define CRYPTO_X1_BUMP \
  128 /* computed as keySize/2 + 256*(macSize % CRYPTO_MAC_SIZE) */
...
void CryptoEncryptBytes(CryptoContext *ctx, const u08p plaintext,
                        u08p ciphertext, u32b msgLen);
void CryptoDecryptBytes(CryptoContext *ctx, const u08p ciphertext,
                        u08p plaintext, u32b msgLen);
