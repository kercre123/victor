...
static bool swd_da1458x_chipinit(void) {
  ...
  SwdDebugPrintf("  IDCODE=0x%08x version=%u PARTNO=%u Designer=%u\n",
                 (uint32_t)value, IDCODE_GET_VERSION(value),
                 IDCODE_GET_PARTNO(value), IDCODE_GET_DESIGNER(value));

  // power up and reset
  swd_write(DEBUG_PORT, SWDP_REG_W_SELECT,
            0x0);  // Set AP bank 0x0 on AP 0x0. CTRLSEL=0
  swd_write(DEBUG_PORT, SWDP_REG_RW_CTRLSTAT,
            SWDP_CTRLSTAT_CSYSPWRUPREQ | SWDP_CTRLSTAT_CDBGPWRUPREQ |
                SWDP_CTRLSTAT_CDBGRSTREQ);             // Power up + DP reset
  swd_read(DEBUG_PORT, SWDP_REG_RW_CTRLSTAT, &value);  // Check powered up
  const uint32_t CTRLSTAT_verify =
      SWDP_CTRLSTAT_CSYSPWRUPACK | SWDP_CTRLSTAT_CSYSPWRUPREQ |
      SWDP_CTRLSTAT_CDBGPWRUPACK | SWDP_CTRLSTAT_CDBGPWRUPREQ |
      SWDP_CTRLSTAT_CDBGRSTREQ;  // 0xf4 << 24
  bool pwrup_rst_ok = (value & 0xFF000000) == CTRLSTAT_verify;
  SwdDebugPrintf("  CTRLSTAT=0x%08x %s\n", (uint32_t)value,
                 (pwrup_rst_ok ? "ok" : "FAIL"));
...
int swd_da1458x_load_bin(const uint8_t* bin_start, const uint8_t* bin_end) {
  // const uint32_t ram_load_addr = 0x08000000; //XXX: what's the ram start
  // address?
  int e = ERROR_OK;
  ...
  SwdDebugPrintf("swd_da1458x_load_bin 0x%08x-0x%08x (%u)\n", bin_start,
                 bin_end, bin_end - bin_start);
  if (!bin_start || !bin_end || bin_end <= bin_start) {
    ConsolePrintf(
        "BAD_ARG: swd_da1458x_load_bin() start=%06x end=%06x size=%i\n",
  ...
  swd_phy_reset();
  if (!swd_da1458x_chipinit()) return ERROR_ACK1;

  // uh...
  volatile int ignore = e;
  ignore = r;
