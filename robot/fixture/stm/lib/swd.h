...
#define SWDP_REG_RW_WCR \
  0x4  // CTRLSEL=1 in the SELECT register (optional register)
...
int swd_stm32_init();  // initialize the swd interface and connect to mcu's
                       // debug port
int swd_stm32_deinit();  // teardown & de-init

int swd_stm32_erase(void);
int swd_stm32_flash(uint32_t flash_addr, const uint8_t* bin_start,
                    const uint8_t* bin_end,
                    bool verify = true);  // return 0 (success) or error code
int swd_stm32_verify(uint32_t flash_addr, const uint8_t* bin_start,
                     const uint8_t* bin_end);
...
// Returns 0 on success, 1 on erase failure (chip will survive), 2 on program
// failure (dead)
...
int swd_da1458x_load_bin(
    const uint8_t* bin_start,
    const uint8_t* bin_end);  // return 0 (success) or error code
