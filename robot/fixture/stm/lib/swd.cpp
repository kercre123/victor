...
// debug config
#define SWD_DEBUG 0
#define SWD_DEBUG_PRINT_RW \
  0 /*low level r/w ops usually too verbose, clog up the debug console*/
...
static inline char *resp_str(swd_resp_t swd_response) {
  switch (swd_response) {
    case SWD_ACK:
      return (char *)"OK";  // break;
    case SWD_WAIT:
      return (char *)"WAIT";  // break;
    case SWD_FAULT:
      return (char *)"FAULT";  // break;
    case SWD_PARITY:
      return (char *)"PARITY";  // break;
  }
...
// 80 would occasionally fail a board, 150 seems enough margin - tested
// empirically
#define delay_data_setup() \
  {                        \
    volatile int x = 15;   \
    while (x--)            \
      ;                    \
  }
#define delay_clock_high() \
  {                        \
    volatile int x = 15;   \
    while (x--)            \
      ;                    \
...
static swd_resp_t swd_write_(char APnDP, int A, unsigned long data) {
  ...
  // Bit 0 = 1, Bit 1 = AP, Bit 2 = Read, Bit 3-4 = A 2-3 (A is 0, 4, 8, C), Bit
  // 5 = parity
  wpr = 0x81 /*0b10000001*/ | (APnDP << 1) |
        (A << 1);                   // A is a 32-bit address bits 0,1 are 0.
  if (count_ones_(wpr & 0x1E) % 2)  // odd number of 1s
    wpr |= 1 << 5;                  // set parity bit

  if (SWD_DEBUG_PRINT_RW)
    SwdDebugPrintf("    swd_write(Port=%s, Addr=%d, data=%08x, wpr=%02x) = ",
                   (APnDP ? "Access" : "Debug"), A, data, wpr);
  ...
  if (SWD_DEBUG_PRINT_RW) SwdDebugPrintf("OK\n");
...
static swd_resp_t swd_read_(char APnDP, int A, volatile unsigned long *data) {
  ...
  wpr = 0x85 /*0b10000101*/ | (APnDP << 1) |
        (A << 1);                   // A is a 32-bit address bits 0,1 are 0.
  if (count_ones_(wpr & 0x1E) % 2)  // odd number of 1s
    wpr |= 1 << 5;                  // set parity bit

  if (SWD_DEBUG_PRINT_RW)
    SwdDebugPrintf("    swd_read(Port=%s, Addr=%d, wpr=%02x) = ",
                   (APnDP ? "Access" : "Debug"), A, wpr);
  ...
  if (response != SWD_ACK) {
    if (SWD_DEBUG_PRINT_RW)
      SwdDebugPrintf("%s (%u)\n", (response == SWD_WAIT ? "WAIT" : "FAULT"),
                     response);
    // else //only print faults
    //  SwdDebugPrintf("    %s (%u) swd_read(Port=%s, Addr=%d, wpr=%02x)\n",
    //  (response == SWD_WAIT ? "WAIT" : "FAULT"), response, (APnDP ? "Access" :
    //  "Debug"), A, wpr);
  ...
  if (count_ones_(wpr) % 2) {             // odd number of 1s
    if (wpr != 1) response = SWD_PARITY;  // bad parity
  } else {
    if (wpr != 0) response = SWD_PARITY;  // bad parity
  }

  if (SWD_DEBUG_PRINT_RW)
    SwdDebugPrintf("%08x%s\n", *data,
                   (response == SWD_PARITY ? " PARITY ERROR" : ""));
  // else if(response != SWD_ACK) //only print faults
  //  SwdDebugPrintf("    %s (%u) swd_read(Port=%s, Addr=%d, wpr=%02x) =
  //  %08x\n", (response == SWD_PARITY ? " PARITY ERROR" : "ERROR"), response,
  //  (APnDP ? "Access" : "Debug"), A, wpr, *data );
...
swd_resp_t swd_read(char APnDP, int A, volatile unsigned long *data) {
  ...
  if (SWD_DEBUG && !SWD_DEBUG_PRINT_RW) {
    if (r != SWD_ACK) {
      if (r != SWD_PARITY) *data = 0;  // didn't read anything. don't print junk
