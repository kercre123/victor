...
static int swd_stm32_write16(uint32_t addr, uint16_t data) {
  ...
  if (r != SWD_ACK) SwdDebugPrintf("  FAILED, resp=%u\n", r);
...
static bool swd_stm32_chipinit(void) {
  ...
  // swd_read(1, 0xC, &value);   // Read AP (not debug port), register 0xFC
  // (IDCODE) swd_read(0, 0xC, &value);   // Read back result from RB registsr
...
static int swd_stm32_init_(void) {
  ...
  if (!swd_stm32_chipinit()) return ERROR_SWD_CHIPINIT;
...
int swd_stm32_deinit() {
  if (swd_initialized) {
    // int r = swd_write32(0xE000EDF0, 0xA05F0003);    // Halt core, just in
    // case if( r != SWD_ACK )
...
int swd_stm32_erase(void) {
  ...
  if (!swd_initialized) THROW_RETURN(ERROR_INVALID_STATE);
  ...
  if (r != SWD_ACK) THROW_RETURN(ERROR_SWD_WRITE_FAULT);
  ...
  r |= swd_write32(0x40022010, 0x00000200);  // Finished!
  SwdDebugPrintf("\n");
  if (r != SWD_ACK) THROW_RETURN(ERROR_SWD_FLASH_ERASE);

  SwdPrintf("erase %s in %ums (r=%u)\n",
            (r == SWD_ACK ? "completed" : "FAILED"),
            (Timer::get() - start) / 1000, r);
  if (timeout < 1) THROW_RETURN(ERROR_SWD_FLASH_ERASE);

  return ERROR_OK;
}

int swd_stm32_read(uint32_t flash_addr, uint8_t* out_buf, int size) {
  SwdPrintf("flash read %i bytes @ 0x%08x-0x%08x...", size, flash_addr,
            flash_addr + size - 1);
...
int swd_stm32_verify(uint32_t flash_addr, const uint8_t* bin_start,
  ...
  SwdPrintf("flash verify, %u bytes (%u pages) @ 0x%08x-0x%08x\n", size,
            CEILDIV(size, pagesize), flash_addr, flash_addr + size - 1);
  ...
  for (int addr = 0; addr < size; addr += linesize) {
    int len = addr + linesize < size ? linesize
                                     : size - addr;  // last line probably short
    ...
    for (int i = 0; i < len; i++) {  // error check this line
      if (page_buffer[i] != bin_start[addr + i]) eline++;
    }
    ...
    // handle errors
    if (eline) {
      if (printlen) SwdPrintf("\n");
      printlen = 0;

      // print binary vs read data to compare
      SwdPrintf("err @ %08x bin : ", flash_addr + addr);
      for (int i = 0; i < len; i++) SwdPrintf("%02x", bin_start[addr + i]);
      SwdPrintf("\n               read: ");
      for (int i = 0; i < len; i++) SwdPrintf("%02x", page_buffer[i]);
      SwdPrintf("\n");
  ...
  SwdPrintf("flash verify %s in %ums\n", !ecnt ? "completed" : "FAILED",
            (Timer::get() - start) / 1000);
  if (ecnt > 0) THROW_RETURN(ERROR_FLASH_VERIFY);

  return ERROR_OK;
}

int swd_stm32_flash(uint32_t flash_addr, const uint8_t* bin_start,
                    const uint8_t* bin_end, bool verify) {
  int e = ERROR_OK;
  ...
  // For reasons I don't understand, must flash one page at a time
  {
    ...
    for (int page = 0; size > 0; page++, size -= pagesize) {
      ...
      memcpy(page_buffer, &bin_start[page * pagesize],
             pagesize);     // buffer next page
      ...
      SwdDebugPrintf(".swd_stm32_flash_page 0x%08x with %u bytes\n",
                     flash_addr + (page * pagesize),
                     (size > pagesize ? pagesize : size));
      int r1 = swd_stm32_flash_page(flash_addr + (page * pagesize),
                                    (u16*)page_buffer);
      // SwdDebugPrintf("...%s\n", !r1 ? "ok" : "failed");
      r |= r1;

      // when debug is turned off, print each page write/status AFTER op
      // completes (includes result, not complicated by internal debug prints)
      if (!SWD_STM32_DEBUG)
        SwdPrintf(".swd_stm32_flash_page 0x%08x with %u bytes: %s\n",
                  flash_addr + (page * pagesize),
                  (size > pagesize ? pagesize : size),
                  (r1 == SWD_ACK ? "ok" : " FAILED"));
    }

    SwdPrintf("flash write %s in %ums (r=%u)\n",
              (r == SWD_ACK ? "completed" : "FAILED"),
              (Timer::get() - start) / 1000, r);
    if (r != SWD_ACK) THROW_RETURN(ERROR_SWD_FLASH_WRITE);
  }

  Timer::delayMs(10);

  // Readback verify
  if (verify) e = swd_stm32_verify(flash_addr, bin_start, bin_end);

  // r = swd_write32(0xE000EDF0, 0xA05F0003);    // Halt core, just in case
  // if( r != SWD_ACK )
  //  SwdPrintf("core halt FAILED\n");

  if (e != ERROR_OK) THROW_RETURN(e);
  return ERROR_OK;
}
...
int swd_stm32_lock_jtag(void) {
  ...
  r |= swd_write32(0x40022010,
                   0x200 + 0x40 + 0x20);  // START option byte ERASE w/OPTWRE
  ...
  {
    if (wait-- <= 0) THROW_RETURN(1);
    Timer::wait(1000);
  }

  r |= swd_write32(0x40022010,
                   0x200 + 0x10);  // Allow option byte program w/OPTWRE
  r |=
      swd_stm32_write16(0x1FFFF800, 0xCC);  // RDP mode 2 - irreversible disable
