...
// Soft Resets the MCU. User data is stored in no-init ram for recovery at boot.
// dat==NULL clears stored data.
void nvReset(u8 *dat, u16 len);

//@return valid data size (bytes) from last nvReset. -1 if no data exists (e.g.
//POR).
int nvResetGetLen(void);

// Try get data stored by the last nvReset() call. Data is written to user
// buffer (if non-NULL), up to the user's max len (safe).
