...
void fixtureInit(void) {
  ...
  if (g_flashParams.fixtureTypeOverride > FIXMODE_NONE &&
      g_flashParams.fixtureTypeOverride < g_num_fixmodes) {
...
const char* fixtureName(void) {
  const char* name = g_fixmode >= 0 && g_fixmode < g_num_fixmodes
                         ? g_fixmode_info[g_fixmode].name
                         : NULL;
  return name ? name : "NA";
}
...
bool fixtureDetect(int min_delay_us) {
  ...
  bool (*Detect)(void) = g_fixmode >= 0 && g_fixmode < g_num_fixmodes
                             ? g_fixmode_info[g_fixmode].Detect
                             : NULL;
...
void fixtureCleanup(void) {
  void (*Cleanup)(void) = g_fixmode >= 0 && g_fixmode < g_num_fixmodes
                              ? g_fixmode_info[g_fixmode].Cleanup
                              : NULL;
...
TestFunction* fixtureGetTests(void) {
  ...
  TestFunction* (*GetTests)(void) = g_fixmode >= 0 && g_fixmode < g_num_fixmodes
                                        ? g_fixmode_info[g_fixmode].GetTests
                                        : NULL;  // check array boundaries
...
// development tool: validate the fixmode_info array contents - hopefully catch
...
bool fixtureValidateFixmodeInfo(bool print) {
  ...
  for (int n = 0; n < g_num_fixmodes; n++) {
    ...
    if (fixnfo->name != NULL &&
        fixnfo->mode != n)  // verify array index matches 'mode' field
      v = false;
    if (fixnfo->name == NULL &&
        (fixnfo->Detect != NULL || fixnfo->GetTests != NULL ||
         fixnfo->mode > -1))  // validate empty entries
      v = false;

    //#if FIXTURE_VALIDATE_PRINT > 0
    if (print)
      ConsolePrintf("  %02d [%02i] %-10s %8x %8x %s\n", n, fixnfo->mode,
                    (fixnfo->name ? fixnfo->name : "-"), fixnfo->Detect,
                    fixnfo->GetTests, (v ? "" : "<--INVALID"));
    //#endif
...
uint32_t fixtureReadSerial(void) {
  ...
  return (serial << 20) |
         (GetSequence(0) &
          0x0Fffff);  // read only, no sequence changes or flash update
}
...
/*char* unixtimestr_(time_t time) {
  ...
  snprintf(str,sizeof(str)-1, "%0.2d:%0.2d:%0.2d", time/3600, (time%3600)/60,
time%60 ); str[sizeof(str)-1] = '\0'; return str;
}*/

char* rtcstr_(RTC_DateTypeDef* date, RTC_TimeTypeDef* time) {
  static char str[32];
  int len = snprintf(str, sizeof(str), "%u %02u/%02u/%02u %02u:%02u:%02u",
                     date->RTC_WeekDay, date->RTC_Date, date->RTC_Month,
                     date->RTC_Year, time->RTC_Hours, time->RTC_Minutes,
                     time->RTC_Seconds);
  str[len] = '\0';
  return str;
}
...
static int RTC_SetDateTime_(RTC_DateTypeDef* RTC_DateStruct,
...
static void RTC_GetDateTime_(RTC_DateTypeDef* RTC_DateStruct,
                             RTC_TimeTypeDef* RTC_TimeStruct) {
...
int fixtureSetTime(time_t time) {
  int e = 0;
  bool rtc_inited =
      RTC_ReadBackupRegister(RTC_BACKUP_REG_INIT) == RTC_INIT_VALUE;
  struct {
  ...
  // unix -> stm rtc format
  struct tm bdtime =
      *localtime(&time);  // intermediate "broken-down time" (clib)
...
#if FIXTURE_RTC_DEBUG > 0
  char* ltime = asctime(&bdtime);
  ltime[24] = '\0';  // rm newline
  ConsolePrintf("fixtureSetTime,%i,%010u,%s,%s,%u\n", fixtureTimeIsValid(),
                time, ltime, rtcstr_(&rtc.date, &rtc.time), century);
#endif

  if (rtc_inited) {
    e |= RTC_SetDateTime_(&rtc.date, &rtc.time);
    RTC_WriteBackupRegister(RTC_BACKUP_REG_CENTURY,
                            e == 0 ? century : 0);  // invalidate on error
...
#if FIXTURE_RTC_DEBUG > 0
    if ((e & (RTC_ERR_VERIFY_DATE | RTC_ERR_VERIFY_TIME |
              RTC_ERR_VERIFY_CENTURY)) != 0)
...
time_t fixtureGetTime(void) {
  ...
  bdtime.tm_year =
      valid ? century - 1900 + rtc.date.RTC_Year : rtc.date.RTC_Year;
  ...
  if (valid) time = mktime(&bdtime);  //"broken-down time" (clib) -> unix

#if FIXTURE_RTC_DEBUG > 0
  char* ltime = asctime(&bdtime);
  ltime[24] = '\0';  // rm newline
  ConsolePrintf("fixtureGetTime,%i,%010u,%s,%s,%u\n", valid, time, ltime,
                rtcstr_(&rtc.date, &rtc.time), century);
#endif
...
#if FIXTURE_RTC_DEBUG > 0
  ConsolePrintf("fixtureGetSetTime,%i,%i,%010u,%s", fixtureTimeIsValid(),
                settime_valid, time, ctime(&time));
#endif
...
static void rtc_init_(void) {
  ...
  if (RTC_ReadBackupRegister(RTC_BACKUP_REG_INIT) != RTC_INIT_VALUE) {
    ...
    volatile int lseWaitCnt = 0;
    const int lseWaitMax = 40 * 1000000;  // rocky estimate: 4M waitCnt/s. DS
                                          // spec >2s max LSE startup time
    while (RCC_GetFlagStatus(RCC_FLAG_LSERDY) == RESET &&
           ++lseWaitCnt < lseWaitMax)
      ;

    if (FIXTURE_RTC_DEBUG > 0)
      ConsolePrintf("%s (cnt=%i)\n",
    ...
    RTC_InitStructure.RTC_AsynchPrediv =
        0x7F;  // clock prescaler for 32768 -> 1Hz
...
void testprint_(const char* prefix, time_t time, const char* post) {
  char* ltime = ctime(&time);
  ltime[24] = '\0';  //"Sun Sep 16 01:03:52 1973\n\0"
  ConsolePrintf("%s,%i,%010u,%s,%s,\n", prefix ? prefix : "",
                fixtureTimeIsValid(), time, ltime, post ? post : "");
}
