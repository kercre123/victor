...
void SetFixtureText(bool reinit) {
  ...
  if (!inited) helperLcdClear();

  // current mode
  bool is_fixmode_head =
      g_fixmode == FIXMODE_HEAD1 || g_fixmode == FIXMODE_HEAD1_OL ||
      g_fixmode == FIXMODE_HEAD2 || g_fixmode == FIXMODE_HELPER1;
  bool is_fixmode_packout =
      g_fixmode == FIXMODE_PACKOUT || g_fixmode == FIXMODE_PACKOUT_OL;

  // different colors for debug builds
  char color = 'b';
  if (!g_isReleaseBuild) color = m_last_error == ERROR_OK ? 'g' : 'r';
  if (g_isReleaseBuild && is_fixmode_packout &&
      !fixtureTimeIsValid())  // packout (release build) indicate if RTC is
                              // invalid
    color = 'r';
  ...
  if (is_fixmode_head)
    helperLcdSetLine(1,
                     snformat(b, bz, "prev esn: 0x%08x", TestHeadGetPrevESN()));
  else if (is_fixmode_packout)
    helperLcdSetLine(
        1, fixtureTimeStr(RtcDisplayTime));  // e.g. "Sun Sep16 01:03 1973"
  ...
  if (!g_isReleaseBuild)
    helperLcdSetLine(2, snformat(b, bz, "DEBUG e%03i %u.%03us", m_last_error,
                                 m_last_time_ms / 1000, m_last_time_ms % 1000));
  ...
  /*tempC = tempC < -9 ? -9 : (tempC > 99 ? 99 : tempC); //bounded for 2-char
  display space if( tempC < 0 ) { //read error helperLcdSetLine(4,
  snformat(b,bz,"                   -") ); helperLcdSetLine(5, snformat(b,bz,"
  %u", (-1)*tempC) ); } else { helperLcdSetLine(4, snformat(b,bz," %u",
  tempC/10) ); helperLcdSetLine(5, snformat(b,bz,"                   %u",
  tempC%10) );
  }//-*/
  int temp_line =
      is_fixmode_head
          ? 6
          : 7;  // shift up for head modes (make space for os version info)
  ...
  if (!inited)
    helperLcdSetLine(
        8, snformat(b, bz, "%-15s v%03u", BUILD_INFO, g_fixtureReleaseVersion));
...
void WaitForDeviceOff(bool error, int debounce_ms) {
  ...
  while (g_isDevicePresent) {
    ...
    if (!fixtureDetect(1000)) {
      if (++debounce >= debounce_ms)  // 500 checks * 1ms = 500ms delay showing
                                      // error post removal
        g_isDevicePresent = 0;
    } else
      debounce = 0;
...
static void RunTests() {
  ...
  cmdSend(CMD_IO_HELPER, "logstart", CMD_DEFAULT_TIMEOUT,
          CMD_OPTS_DEFAULT & ~(CMD_OPTS_EXCEPTION_EN | CMD_OPTS_LOG_ALL));
  ...
  try {
    ...
    error = e == ERROR_OK ? ERROR_THROW_0 : e;  // don't allow test to throw
                                                // 'OK'
  }
  ...
  m_last_time_ms = Timer::elapsedUs(Tstart) /
                   1000;  // Note: may be less than actual time, if test isn't
                          // using the Timer (requires polling for accuracy)
  ConsolePrintf("[TEST:END]\n", error);
  cmdSend(CMD_IO_HELPER, "logstop", CMD_DEFAULT_TIMEOUT,
          CMD_OPTS_DEFAULT & ~CMD_OPTS_EXCEPTION_EN);
  ...
  // flush console. lots of stuff can back up while we've been busy running a
  // test
  ConsoleFlushLine();
...
static bool IsDevicePresent(void) {
  ...
  if (fixtureDetect(1000)) {
    if (++s_debounce >= 300) {  // 300 checks * 1ms = 300ms to be sure the board
                                // is reliably in contact
      s_debounce = 0;
      return TRUE;
    }
  } else
...
static bool TryToRunTests(void) {
  ...
  // If/when we add testport support - use ToggleContacts and then repeatedly
  // call TryToEnterDiagnosticMode
  g_isDevicePresent = 1;
  RunTests();
  return TRUE;
}
...
inline void dbgBtnHandler(void) {
  ...
  int x = USE_START_BTN > 0
              ? Board::BTN_2
              : Board::BTN_1;  // skip start btn if used for testing
...
void helper_temp_rtc_manage(bool force_update) {
  ...
  int update_interval = last_update_successful
                            ? 10 * 1000 * 1000
                            : 60 * 1000 * 1000;  // throttle down for debug if
                                                 // not connected to helper head
  ...
  if (ConsoleGetIndex_() > 0 && !force_update) {
    ...
    if (Timer::elapsedUs(TtempUpdate) >
        180 * 1000 * 1000) {  // something got stuck
...
static void MainExecution() {
  ...
  if (g_forceStart) g_forceStart = 0, start = 1;
  ...
  helper_temp_rtc_manage(
      start);  // force immediate temp read +clock update after test completion
...
void MainErrorLoop(void) {
  ...
  while (1) {
    ...
    ConsoleUpdate();  // Keep the comm chanel open so we can bootload out of
                      // this corner
  }
}
...
int main(void) {
  ...
  g_app_reset.valid =
      sizeof(g_app_reset) == nvResetGet((u8*)&g_app_reset, sizeof(g_app_reset));

  Timer::init();
  FLEXFLOW::init();
  FetchParams();  // g_flashParams = flash backup (saved via 'setmode' console
                  // cmd)
  InitConsole();
  InitRandom();
  Board::init();
  ...
  if (!fixtureValidateFixmodeInfo()) {
    ...
    fixtureValidateFixmodeInfo(
        true);  // print the info array to console, hilighting invalid entries
    ConsolePrintf("\n\n");
  ...
  try {
    cubebootSignature(0); /*supress dbg print*/
  } catch (int e) {
    cubeErr = e;
  }
  if (cubeErr != ERROR_OK) {
    try {
      cubebootSignature(1); /*print error detail*/
    } catch (int e) {
    }
