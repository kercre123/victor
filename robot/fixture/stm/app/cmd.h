...
#define CMD_OPTS_LOG_RSP_TIME \
  0x0040  // print-log append elapsed time to logged rsp line
#define CMD_OPTS_LOG_ASYNC 0x0080  // print-log async line (:async)
#define CMD_OPTS_LOG_OTHER \
  0x0100  // print-log 'other' rx'd line (informational,uncategorized)
...
#define CMD_OPTS_DBG_PRINT_ENTRY \
  0x1000  // debug: print function entry with parsed params
#define CMD_OPTS_DBG_PRINT_RX_PARTIAL \
  0x2000  // debug: print any unexpected chars, partial line left in rx buffer
          // at cmd end
#define CMD_OPTS_DBG_SYSCON_OVF_ERR \
  0x4000  // debug: throw error if syscon tx overflow detected
#define CMD_OPTS_DEFAULT                                                     \
  (CMD_OPTS_EXCEPTION_EN | CMD_OPTS_REQUIRE_STATUS_CODE | CMD_OPTS_LOG_ALL | \
   CMD_OPTS_DBG_PRINT_RX_PARTIAL)
...
// e.g. send(IO_DUT, snformat(x,x,"lcdshow %u %s /"Victor DVT/"", solo=0,
// color="ib"), timeout=100 );
//@param async_handler: user handler, any line beginning with ASYNC_PREFIX
//@param dbuf: if provided, rx dat between cmd and response lines will be copied
//here (raw char stream, not null terminated)
char* cmdSend(cmd_io io, const char* scmd, int timeout_ms = CMD_DEFAULT_TIMEOUT,
              int opts = CMD_OPTS_DEFAULT, void (*async_handler)(char*) = 0,
              cmd_dbuf_t* dbuf = 0);
int cmdStatus();  // parsed rsp status of most recent cmdSend(). status=1st arg,
                  // INT_MIN if !exist or bad format
uint32_t cmdTimeMs();  // time it took for most recent cmdSend() to finish

// DEBUG:
int cmdSysconOvfCnt();  // get syscon tx overflow cnt from most recent
                        // cmdSend(CMD_IO_CONTACTS) [reported by syscon debug
                        // builds]
...
//@return u32 value of input hex string (e.g. 'a235dc01'). 0 on parse error +
//errno set to -1
...
char* cmdGetArg(char* s, int n, char* out_buf = 0,
                int buflen = 0);  // overload out_buf/buflen to provide user
                                  // buffer to hold the argument
