...
int cmdStatus() { return m_status; }

static uint32_t m_time_ms;  // runtime for most recent command
uint32_t cmdTimeMs() { return m_time_ms; }

static int syscon_tx_ovf_cnt;
int cmdSysconOvfCnt() { return syscon_tx_ovf_cnt; }
...
static void get_errs_(cmd_io io, io_err_t* ioe) {
  ioe->rxDroppedChars =
      io == CMD_IO_DUT_UART
          ...
          : (io == CMD_IO_CONTACTS ? Contacts::getRxDroppedChars() : 0);
  ioe->rxOverflowErrors =
      io == CMD_IO_DUT_UART
          ? DUT_UART::getRxOverflowErrors()
          : (io == CMD_IO_CONTACTS ? Contacts::getRxOverflowErrors() : 0);
  ioe->rxFramingErrors =
      io == CMD_IO_DUT_UART
          ? DUT_UART::getRxFramingErrors()
          : (io == CMD_IO_CONTACTS ? Contacts::getRxFramingErrors() : 0);
}
...
static char* getline_(int c) {
  if (c == '\r' || c == '\n') {
    ...
    return m_line;
             ...
             m_line_len < line_maxlen)  // ascii (ignore null)
    m_line[m_line_len++] = c;
...
static int getchar_(cmd_io io) {
  switch (io) {
    case CMD_IO_DUT_UART:
      return DUT_UART::getchar();  // break;
    case CMD_IO_CONSOLE:
      return ConsoleGetCharNoWait();  // break;
    case CMD_IO_CONTACTS: {
      int c = Contacts::getchar();
      if (c == 0x88) {  // syscon DEBUG_TX_OVF_DETECT=1 inserts overflow
                        // indicator in the stream
...
static void write_(cmd_io io, const char* s1, const char* s2 = NULL,
                   const char* s3 = NULL);
...
char* cmdSend(cmd_io io, const char* scmd, int timeout_ms, int opts,
              void (*async_handler)(char*), cmd_dbuf_t* dbuf) {
  char b[80];
  const int bz = sizeof(b);  // str buffer
  ...
  if (io != CMD_IO_CONSOLE &&
      opts & CMD_OPTS_LOG_CMD)  // echo to log BEFORE the DUT write or we'll
                                // miss response chars
  ...
  while (Timer::elapsedUs(Twait) < timeout_ms * 1000) {
    ...
    if (dbuf && c > 0 && c < 128) {
      // if( io != CMD_IO_CONSOLE && opts & CMD_OPTS_LOG_OTHER )
      // ConsolePutChar(c); //DEBUG inspect
      if (dbuf->wlen < dbuf->size)
        dbuf->p[dbuf->wlen++] = c;
      else {
        if (opts & CMD_OPTS_LOG_ERRORS &&
            dbuf->wlen == dbuf->size)  // one-shot report
          write_(CMD_IO_CONSOLE, LOG_RSP_PREFIX, "DBUF_OVERFLOW\n");
        if (opts & CMD_OPTS_EXCEPTION_EN) throw ERROR_BUFFER_TOO_SMALL;
      }
    }
    ...
    if (rsp != NULL) {
      ...
      if (rspLen > sizeof(RSP_PREFIX) - 1 &&
          !strncmp(rsp, RSP_PREFIX,
                   sizeof(RSP_PREFIX) - 1))  // response prefix detected
      {
        ...
        if (opts & CMD_OPTS_LOG_RSP) {
          write_(CMD_IO_CONSOLE,
                 io == CMD_IO_CONSOLE ? RSP_PREFIX : LOG_RSP_PREFIX,
                 rsp);  // differentiate prefix depending on the sender
        ...
        if (m_ioerr.rxOverflowErrors > 0 || m_ioerr.rxDroppedChars > 0) {
          if (opts & CMD_OPTS_LOG_ERRORS)
            write_(CMD_IO_CONSOLE,
                   snformat(b, bz, "IO ERROR ovfl=%i dropRx=%i frame=%i",
                            m_ioerr.rxOverflowErrors, m_ioerr.rxDroppedChars,
                            m_ioerr.rxFramingErrors));
        ...
        if (syscon_tx_ovf_cnt > 0) {
          if (opts & CMD_OPTS_LOG_ERRORS)
            write_(CMD_IO_CONSOLE,
                   snformat(b, bz, "SYSCON ERROR: TX OVERFLOW CNT = %i\n",
                            syscon_tx_ovf_cnt));
        ...
        if (nargs < 1 ||
            strcmp(cmdGetArg((char*)scmd, 0, b, bz), cmdGetArg(rsp, 0)) != 0) {
        ...
        if (nargs >= 2) {
          ...
          if (val != INT_MIN) m_status = val;  // record parsed value
        }

        // check for status code errors (if not allowed)
        if (nargs >= 2 && m_status != 0 &&
            !(opts & CMD_OPTS_ALLOW_STATUS_ERRS)) {
        ...
        // passed all checks
        return rsp;
      } else if (rspLen > sizeof(ASYNC_PREFIX) - 1 &&
                 !strncmp(rsp, ASYNC_PREFIX, sizeof(ASYNC_PREFIX) - 1)) {
        if (io != CMD_IO_CONSOLE &&
            opts & CMD_OPTS_LOG_ASYNC)  // echo to log, unchanged
      ...
      {
        if (io != CMD_IO_CONSOLE &&
            opts & CMD_OPTS_LOG_OTHER)  // echo to log, unchanged
          write_(CMD_IO_CONSOLE, rsp, "\n");
      }
  ...
  if (opts &
      CMD_OPTS_DBG_PRINT_RX_PARTIAL)  // see what's leftover in the rx buffer
  {
    if (m_line_len > 0) {
...
int32_t cmdParseInt32(char* s) {
  ...
  if (s) {
    ...
    // check conversion errs, limit numerical range, and verify entire arg was
    // parsed (stops at whitespace, invalid chars...)
    if (errno == 0 && val <= INT_MAX && val >= INT_MIN /*+1*/ && endptr > s &&
        *endptr == '\0')
      return val;
  }
...
char* cmdGetArg(char* s, int n, char* out_buf, int buflen) {
  ...
  while (n--) {                                  // seek to nth arg
    if ((s = nextArg_(s)) == NULL) return NULL;  // there is no spoon
  }
...
void cmdDbgParseTestbench(void) {
  ...
  for (int x = 0; x < sizeof(samples) / sizeof(char*); x++) {
    ...
    for (int n = 0; n < numargs + 2;
         n++)  // check a few indices past valid - make sure parser is rejecting
               // out-of-bounds
    {
      char* arg = cmdGetArg((char*)samples[x], n);
      if (n < numargs) {
        // these should all be valid args. valid arg ptr, anyway. 0 length is
        // valid!
        if (arg)
