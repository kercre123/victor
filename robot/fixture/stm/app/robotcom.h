...
void helperLcdShow(bool solo, bool invert, char color_rgbw,
...
const int ccr_sr_cnt[12] = {
    0, 2, 4, 2, 2, 2, 2,
    4, 2, 1, 1, 4};  // number of CCC sensor fields per line for each type
                     // (internal:parsing & error checking)
...
// data conversion
#define RCOM_BAT_RAW_TO_MV(raw) \
  (((raw)*2800) >> 11) /*robot_sr_t::bat.raw (adc) to millivolts*/
...
#define RCOM_PRINT_LEVEL_CMD_DAT_RSP \
  (RCOM_PRINT_LEVEL_CMD | RCOM_PRINT_LEVEL_DAT | RCOM_PRINT_LEVEL_RSP)
#define RCOM_PRINT_LEVEL_ALL                                            \
  (RCOM_PRINT_LEVEL_CMD | RCOM_PRINT_LEVEL_DAT | RCOM_PRINT_LEVEL_RSP | \
   RCOM_PRINT_LEVEL_NFO)
...
typedef union {
  ...
  struct {
    int32_t raw;
    int32_t temp;
  } bat;  // battery: raw-adc, temperature (2x int16)
  struct {
    int32_t fL;
    int32_t fR;
    int32_t bL;
    int32_t bR;
  } cliff;  // cliff sensors: front/back L/R (4x uint16)
  struct {
    int32_t pos;
    int32_t speed;
    int32_t delta;
  } enc;  // encoder: position, speed, [delta,spine-only] (2[3]x int32)
  struct {
    int32_t rangeMM;
    int32_t spadCnt;
    int32_t signalRate;
    int32_t ambientRate;
  } prox;  // proximity,TOF (4x uint16)
  struct {
    int32_t touch;
    int32_t btn;
  } btn;  // touch & button (2x uint16)
  struct {
    int32_t rssi;
  } fccRssi;  // FCC mode RSSI (int8)
  struct {
    int32_t pktCnt;
  } fccRx;  // Fcc mode packet rx (int32)
} robot_sr_t;

void rcomSetTarget(bool spine_nCCC);  // select charge contacts or spine for
                                      // command target (sticky)
...
void rcomLed(
    uint8_t* leds12,
    int printlvl =
        RCOM_PRINT_LEVEL_DEFAULT);  // leds[12] {led0R,G,B,led1R,G,B...}
robot_sr_t* rcomMot(uint8_t NN, uint8_t sensor, int8_t treadL, int8_t treadR,
                    int8_t lift, int8_t head,
                    int printlvl = RCOM_PRINT_LEVEL_DEFAULT);
robot_sr_t* rcomGet(
    uint8_t NN, uint8_t sensor,
    int printlvl = RCOM_PRINT_LEVEL_DEFAULT);  // NN = #drops (sr vals). returns
                                               // &sensor[0] of [NN-1]
int rcomRlg(uint8_t idx, char* buf, int buf_max_size,
            int printlvl =
                RCOM_PRINT_LEVEL_DEFAULT);  // read log 'idx' into buf (NOT
                                            // null-terminated). return num chars
                                            // written to buf [e.g. strlen(buf)]
