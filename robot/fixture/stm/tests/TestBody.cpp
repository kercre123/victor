...
// syscon has a 4-word header that is mangled by the normal build signing
// process
#define SYSCON_HEADER_SIZE 16
#define SYSCON_EVIL \
  0x4F4D3243 /*first word of syscon header, indicates valid app*/
...
static void mcu_flash_program_(uint32_t flash_addr, const uint8_t* bin,
                               const uint8_t* binEnd, bool verify,
                               const char* name) {
  mcu_swd_init_();
...
static void mcu_flash_verify_(uint32_t flash_addr, const uint8_t* bin_start,
                              const uint8_t* bin_end) {
  try {
    swd_stm32_verify(flash_addr, bin_start, bin_end);
  } catch (int e) {
    throw e;
  }
}
...
bool TestBodyDetect(void) {
  ...
  DUT_TRX::init(MODE_INPUT, PULL_NONE);  // prevent pu from doing strange things
...
static void BodyShortCircuitTest(void) {
  ...
  int ima_limit_VEXT = g_fixmode == FIXMODE_BODY3
                           ? 1200
                           : 350;  // BODY3 may pull charging current
  Board::powerOff(PWR_VEXT);
...
static void BodyTryReadSerial(void) {
  ...
  if (!BODYID_HWREV_IS_VALID(bodyid.hwrev) ||
      !BODYID_MODEL_IS_VALID(bodyid.model) || ein1 != BODYID_ESN_EMPTY ||
      ein2 != BODYID_ESN_EMPTY || ein3 != BODYID_ESN_EMPTY) {
    bodyid.hwrev = BODYID_HWREV_EMPTY;
    bodyid.model = BODYID_MODEL_EMPTY;
...
static void BodyLoadTestFirmware(void) {
  ...
  // Erase and flash
  mcu_power_up_();
  mcu_flash_program_(FLASH_ADDR_TEST_APP, g_BodyTest, g_BodyTestEnd, 1,
                     "test firmware");
  mcu_power_down_();

  // Power up and test comms
  Board::powerOn(PWR_VEXT, 0);  // must provide VEXT to wake up the mcu
  if (g_fixmode < FIXMODE_BODY3) Board::powerOn(PWR_VBAT, 0);
  Timer::delayMs(
      150);  // wait for systest to boot into main and enable battery power
  ...
  // test tread encoders
  cmdSend(CMD_IO_CONTACTS, "encoders", CMD_DEFAULT_TIMEOUT,
  ...
  if (g_fixmode < FIXMODE_BODY1) {
    ...
    ConsolePrintf("cliff = fL:%i fR:%i bR:%i bL:%i\n", cliff.cliff.fL,
                  cliff.cliff.fR, cliff.cliff.bR, cliff.cliff.bL);
  }
  ...
  if (g_fixmode < FIXMODE_BODY1) {
    ...
    ConsolePrintf("battery = %i.%03iV\n",
                  RCOM_BAT_RAW_TO_MV(bat.bat.raw) / 1000,
                  RCOM_BAT_RAW_TO_MV(bat.bat.raw) % 1000);
  }
...
STATIC_ASSERT(APP_GLOBAL_BUF_SIZE >= STM32F030XX_PAGESIZE,
              body_buffer_size_check);
STATIC_ASSERT(APP_GLOBAL_BUF_SIZE >= BodyBootMaxSize,
              body_boot_buffer_size_check);
...
static void BodyLoadProductionFirmware(void) {
  ...
  if (bodyid.hwrev == BODYID_HWREV_EMPTY && g_fixmode == FIXMODE_BODY1)
    bodyid.hwrev =
        CURRENT_BODY_HW_REV;  // only set valid hwrev in production mode
  ...
  *((uint32_t*)(bodyboot + 24)) = bodyid.esn;
  ...
  ConsolePrintf("load: ESN %08x, hwrev %u, model %u\n", bodyid.esn,
                bodyid.hwrev, bodyid.model);
  mcu_power_up_();
  mcu_flash_program_(FLASH_ADDR_SYSBOOT, bodyboot, bodyboot + bodybootSize, 1,
                     "bootloader");
...
static void BodyVerifyProductionFirmware(void) {
  ...
  if (g_fixmode ==
      FIXMODE_BODY1_OL)  // Offline mode doesn't write production firmware
    return;
  ...
  Timer::delayMs(
      150);  // wait for mcu to boot into main and enable battery power
  Board::powerOff(PWR_VEXT);
  ...
  while (1) {
    if (Timer::elapsedUs(Tstart) >
        4 * 1000 * 1000) {  // nominal app boot delay is ~2.6s
      ConsolePrintf("\ntimeout waiting for syscon boot\n");
      throw ERROR_BODY_NO_BOOT_MSG;
    }

    // Debug: shortcut outta here
    if (ConsoleReadChar() > -1 && allow_skip) break;

    // wait for current to be in the range we expect
    int ibat_ma =
        Meter::getCurrentMa(PWR_VBAT, 8);  // large oversample for averaging
...
a key to skip" : "") );
  ...
  while(1)
  {
    if( Timer::elapsedUs(Tstart) > 4*1000*1000 ) { //nominal delay is ~2.5s
      ...
      if( g_fixmode == FIXMODE_BODY3 ) //XXX: debug, can't guarantee reset
timing (battery installed) break; throw ERROR_BODY_NO_BOOT_MSG;
    }
...
static void BodyCheckProgramLockout(void) {
  ...
  if (err == ERROR_SWD_CHIPINIT || err == ERROR_SWD_INIT_FAIL) {
    ...
    // if we don't detect production firmware, assume unprogrammed and throw the
    // original swd error
    try {
    ...
    throw ERROR_BODY_PROGRAMMED;  // detected boot msg. this board is
                                  // programmed/locked
  }
...
static void BodyChargeContactElectricalDebug(void) {
  ...
  while (1) {
    ...
    cmdSend(CMD_IO_CONTACTS, "getvers", CMD_DEFAULT_TIMEOUT,
            CMD_OPTS_DEFAULT & ~CMD_OPTS_EXCEPTION_EN);
    cmdSend(CMD_IO_CONTACTS, "getvers", CMD_DEFAULT_TIMEOUT,
            CMD_OPTS_DEFAULT & ~CMD_OPTS_EXCEPTION_EN);
    Timer::delayMs(50);
    cmdSend(CMD_IO_CONTACTS,
            "testcommand  extra  long  string  to  measure  gate  rise  abcd  "
            "1234567890",
            CMD_DEFAULT_TIMEOUT, CMD_OPTS_DEFAULT & ~CMD_OPTS_EXCEPTION_EN);
    Timer::delayMs(50);
    ...
    Timer::delayMs(50);
    cmdSend(CMD_IO_CONTACTS,
            "getvers plus some extra ignored params to create a really really "
            "long transmit cyclce",
            CMD_DEFAULT_TIMEOUT, CMD_OPTS_DEFAULT & ~CMD_OPTS_EXCEPTION_EN);
...
TestFunction* TestBody0GetTests(void) {
  ...
  static TestFunction m_tests_0[] = {
      BodyShortCircuitTest,       BodyCheckProgramLockout,
      BodyTryReadSerial,          BodyLoadTestFirmware,
      BodyLoadProductionFirmware, BodyVerifyProductionFirmware,
      BodyFlexFlowReport,         NULL,
  };
...
TestFunction* TestBody2GetTests(void) {
  static TestFunction m_tests[] = {
      BodyShortCircuitTest,
      BodyCheckProgramLockout,
      BodyTryReadSerial,  // skip serial read to force blank state (generate new
                          // ESN)
      // BodyLoadTestFirmware,
