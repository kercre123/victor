...
uint32_t TestHeadGetPrevESN(void) {
  ...
  if (m_previous_esn & 0x80000000) {
    if (g_fixmode == FIXMODE_HEAD1) {
      m_previous_esn = fixtureReadSerial();  // readSerial returns the next s/n
                                             // to be allocated
...
bool TestHeadDetect(void) {
  ...
  bool detected =
      !DUT_RESET::read();  // true if signal is pulled down by dut circuit
  DUT_RESET::init(MODE_INPUT, PULL_NONE);  // prevent pu from doing strange
                                           // things to mcu (phantom power?)
...
void TestHeadDutProgram(void) {
  const int timeout_s =
      90 + 60;  // XXX: DVT4-RC clocked in at ~87s. Add some margin.
  char b[50];
  int bz = sizeof(b);
  ...
  //  FIXMODE_HEAD1_OL  : dutprogram timeout ESN-DBG  HWREV-DBG  MODEL-DBG
  //  nocert nos //inhibit cloud cert + os write (just test USB connectivity)
  //  FIXMODE_HEAD2     : dutprogram timeout ESN-DBG  HWREV-DBG  MODEL-DBG
  //  nocert FIXMODE_HELPER1   : dutprogram timeout ESN-DBG  HWREV-DBG MODEL-DBG
  //  nocert helper

  // provision ESN
  headnfo.esn = g_fixmode == FIXMODE_HEAD1 ? fixtureGetSerial() : 0x00100000;
  m_previous_esn =
      headnfo.esn;  // even if programming fails, report the (now unusable) ESN
  int hwrev = g_isReleaseBuild && g_fixmode == FIXMODE_HEAD1
                  ? CURRENT_HEAD_HW_REV
                  : HEADID_HWREV_DEBUG;
  ...
  switch (g_fixmode) {
    case FIXMODE_HEAD1:
      suffix = "";
      break;
    case FIXMODE_HEAD1_OL:
      suffix = "nocert nos";
      break;
    case FIXMODE_HEAD2:
      suffix = "nocert";
      break;
    case FIXMODE_HELPER1:
      suffix = "nocert helper";
      break;
    default:
      suffix = "nocert nos";
      break;
  ...
  snformat(b, bz, "dutprogram %u %08x %04x %04x %s", timeout_s, headnfo.esn,
           hwrev, model, suffix);
  cmdSend(CMD_IO_HELPER, b, (timeout_s + 10) * 1000,
          CMD_OPTS_DEFAULT | CMD_OPTS_ALLOW_STATUS_ERRS);
  if (cmdStatus() >= ERROR_HEADPGM &&
      cmdStatus() < ERROR_HEADPGM_RANGE_END)  // headprogram exit code range
    throw cmdStatus();
  else if (cmdStatus() != 0)
...
TestFunction* TestHelper1GetTests(void) { return TestHead1GetTests(); }

TestFunction* TestHead2GetTests(void) { return TestHead1GetTests(); }
