...
void mic_print_stream(char *stream, int MKx, bool odd_nEven) {
  ...
  for (int i = odd_nEven; i < mux_stream_len; i += 2) {
    if (((i - odd_nEven) / 2) % linesize == 0) ConsolePrintf("\n");
...
static inline void analyze_mic_stream_(char *bit, int even_MKx, int odd_MKx) {
  ...
  for (int i = 0; i < mux_stream_len - 1; i += 2) {
    if (bit[i] == bit[i + 1]) ++oddfollow;  // odd follows the preceding even
  }
  for (int i = 1; i < mux_stream_len - 1; i += 2) {
    if (bit[i] == bit[i + 1]) ++evenfollow;  // even follows the preceding odd
  }
}
...
static void TestMics(void) {
  ...
  for (int x = 0; x < 4; x++) {
    ...
    mic[x].pass =
        (mic[x].bitPct > 20 && mic[x].bitPct < 80) && (mic[x].followPct < 90);

    ConsolePrintf(
        "MK%u bit-ratio 1:0 -> %i:%i (%i%%) -- follower %i/%u (%i%%) [%s]\n",
        x + 1, mic[x].bitCnt, bitCntTotal - mic[x].bitCnt, mic[x].bitPct,
...
static const uint32_t led_frame_[] = {
    0, BF_D1_RED | BF_D2_RED | BF_D3_RED, 0, BF_D1_GRN | BF_D2_GRN | BF_D3_GRN,
    0, BF_D1_BLU | BF_D2_BLU | BF_D3_BLU,
};
...
void TestButton(void) {
  uint32_t btn_start,
      cnt_compare = 50;  // adjust LPF count for different sample timing
