...
void TestDebugInit(void) { srand(Timer::get()); }

void TestDebugShellScriptTimeout(void) {
  const uint32_t esn =
      0x80000000 | ((rand() & 0xffff) << 16) | (rand() & 0xffff);  // 0xabcd1234
  ...
  char b[60];
  const int bz = sizeof(b);
  snformat(b, bz, "dutprogram %u %08x %u", shell_timeout_s, esn,
           script_delay_s);
  cmdSend(CMD_IO_HELPER, b, (shell_timeout_s + 10) * 1000,
          CMD_OPTS_DEFAULT | CMD_OPTS_ALLOW_STATUS_ERRS);
}
...
void TestDebugShellArgParsing(void) {
  ...
  snformat(b, bz, "dutprogram %u %08x   %i  param2  param3  ", shell_timeout_s,
           esn, param1);
  cmdSend(CMD_IO_HELPER, b, (shell_timeout_s + 10) * 1000,
          CMD_OPTS_DEFAULT | CMD_OPTS_ALLOW_STATUS_ERRS);
}
...
TestFunction* TestDebugGetTests(void) {
  static TestFunction m_tests[] = {TestDebugInit,
                                   // TestDebugShellScriptTimeout,
                                   TestDebugShellArgParsing, NULL};
  return m_tests;
}
