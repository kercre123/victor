...
static void dbg_test_all_(void) {
  ...
  rcomMot(100, RCOM_SENSOR_MOT_LEFT, 127, 0, 0, 0);
  ConsolePutChar('\n');
  rcomMot(100, RCOM_SENSOR_MOT_RIGHT, 0, -127, 0, 0);
  ConsolePutChar('\n');
  rcomMot(50, RCOM_SENSOR_MOT_LIFT, 0, 0, 100, 0);
  ConsolePutChar('\n');
  rcomMot(75, RCOM_SENSOR_MOT_LIFT, 0, 0, -100, 0);
  ConsolePutChar('\n');
  rcomMot(50, RCOM_SENSOR_MOT_HEAD, 0, 0, 0, 100);
  ConsolePutChar('\n');
  rcomMot(75, RCOM_SENSOR_MOT_HEAD, 0, 0, 0, -100);
  ConsolePutChar('\n');

  rcomGet(1, RCOM_SENSOR_BATTERY);
  ConsolePutChar('\n');
  rcomGet(3, RCOM_SENSOR_BATTERY);
  ConsolePutChar('\n');
  rcomGet(5, RCOM_SENSOR_BATTERY);
  ConsolePutChar('\n');
  rcomGet(1, RCOM_SENSOR_CLIFF);
  ConsolePutChar('\n');
  ...
  rcomGet(1, RCOM_SENSOR_PROX_TOF);
  ConsolePutChar('\n');
  rcomGet(1, RCOM_SENSOR_BTN_TOUCH);
  ConsolePutChar('\n');
  rcomGet(1, RCOM_SENSOR_RSSI);
  ConsolePutChar('\n');
  rcomGet(1, RCOM_SENSOR_RX_PKT);
  ConsolePutChar('\n');
  rcomGet(1, RCOM_SENSOR_DEBUG_INC);
  ConsolePutChar('\n');
  rcomRlg(0, 0, 0);
  rcomRlg(1, 0, 0);
}
...
static void dbg_test_emr_(bool blank_only, bool dont_clear) {
  static uint32_t m_emr[256];
  int idx;
  ConsolePrintf("EMR READ/WRITE TEST\n");

  // reset EMR to blank
  for (idx = 1; idx < 256; idx++) rcomSmr(idx, 0);
  ...
  for (idx = 0; idx < 256; idx++) {
    ...
    if (idx == EMR_FIELD_OFS(PLAYPEN_READY_FLAG) ||
        ...
        (idx >= EMR_FIELD_OFS(playpen[0]) &&
         idx <= EMR_FIELD_OFS(playpen[7]))) {
      val = 0;
    }
  ...
  // results!
  ConsolePrintf("EMR test %s: %u errors\n", mismatch > 0 ? "FAILED" : "passed",
                mismatch);
  Timer::delayMs(
      2000);  // so we can see it before blanking emr (which might fail...)
...
static void run_debug(int arg[]) {
  if (arg[0] == 1) dbg_test_all_();
  if (arg[0] == 2) dbg_test_comm_loop_(arg[1], arg[2], arg[3]);
  if (arg[0] == 3) dbg_test_readlogs_(arg[1], arg[2]);
  if (arg[0] == 4) dbg_test_leds_(arg[1], arg[2]);

  if (arg[0] == 14) dbg_test_emr_(arg[1], arg[2]);
}
...
const char* DBG_cmd_substitution(const char* line, int len) {
  ...
  if (!strcmp(line, "esn")) return ">>esn 00 00 00 00 00 00";
  if (!strcmp(line, "bsv")) return ">>bsv 00 00 00 00 00 00";
  if (!strcmp(line, "mot")) return ">>mot ff 03";
  if (!strcmp(line, "get")) return ">>get 01 00";
  if (!strncmp(line, "eng", 3) || !strncmp(line, "pwr", 3) ||
      !strncmp(line, "rlg", 3))
    return snformat(buf, sizeof(buf), ">>%c%c%c %02x %02x %02x %02x %02x %02x",
                    line[0], line[1], line[2], larg[0], larg[1], larg[2],
                    larg[3], larg[4], larg[5]);
  if (!strncmp(line, "smr", 3) || !strncmp(line, "gmr", 3)) {
    int ix = larg[0];
    int val = larg[1];
    return snformat(buf, sizeof(buf), ">>%c%c%c %02x %02x %02x %02x %02x 00",
                    line[0], line[1], line[2], ix, val & 0xFF,
                    (val >> 8) & 0xff, (val >> 16) & 0xff, (val >> 24) & 0xff);
  }
  if (!strncmp(line, "debug", 5)) {
    int dbgErr = 0;
    ConsolePrintf("========== DEBUG %i %i %i %i %i %i ==========\n", larg[0],
                  larg[1], larg[2], larg[3], larg[4], larg[5]);
    try {
      run_debug(larg);
    } catch (int e) {
      dbgErr = e;
    ...
    return "\n";  // sends the debug line to charge contacts (ignored by robot,
                  // but keeps cmd in recall buffer)
  }
  return 0;
}
...
#define SYSCON_CHG_PWR_DELAY_MS \
  250 /*delay from robot's on-charger detect until charging starts*/

#define IS_FIXMODE_ROBOT1() \
  (g_fixmode == FIXMODE_ROBOT1 || g_fixmode == FIXMODE_ROBOT1_OL)
#define IS_FIXMODE_ROBOT3() \
  (g_fixmode == FIXMODE_ROBOT3 || g_fixmode == FIXMODE_ROBOT3_OL)
#define IS_FIXMODE_PACKOUT() \
  (g_fixmode == FIXMODE_PACKOUT || g_fixmode == FIXMODE_PACKOUT_OL)
#define IS_FIXMODE_OFFLINE()                                           \
  (g_fixmode == FIXMODE_ROBOT1_OL || g_fixmode == FIXMODE_ROBOT3_OL || \
...
bool TestRobotDetect(void) {
  // on test cleanup/exit, let charger kick back in so we can properly detect
  // removal
  int chargeDelay = (g_isDevicePresent && !Board::powerIsOn(PWR_VEXT))
                        ? SYSCON_CHG_PWR_DELAY_MS
                        : 0;  // condition from app::WaitForDeviceOff()
  ...
  return i_ma > DETECT_CURRENT_MA ||
         (i_ma > PRESENT_CURRENT_MA && g_isDevicePresent);
}
...
void TestRobotCleanup(void) {
  ...
  if (!cleanup_preserve_vext) Board::powerOff(PWR_VEXT);
...
void read_robot_info_(void) {
  if (IS_FIXMODE_ROBOT1()) {
    ...
    uint32_t esnCmd =
        rcomEsn();  // not really necesary anymore since GMR[0] is esn
    flexnfo.bsv = *rcomBsv();
    ...
    uint32_t playpenTouchSensorMinValid =
        rcomGmr(EMR_FIELD_OFS(playpenTouchSensorMinValid));
    uint32_t playpenTouchSensorMaxValid =
        rcomGmr(EMR_FIELD_OFS(playpenTouchSensorMaxValid));
    uint32_t playpenTouchSensorRangeThresh =
        rcomGmr(EMR_FIELD_OFS(playpenTouchSensorRangeThresh));
    uint32_t playpenTouchSensorStdDevThresh =
        rcomGmr(EMR_FIELD_OFS(playpenTouchSensorStdDevThresh));
    uint32_t playpenTestDisableMask =
        rcomGmr(EMR_FIELD_OFS(playpenTestDisableMask));

#define U32FLOAT(u) (*((float*)&(u)))

    ConsolePrintf("EMR[%u] esn         :%08x [%08x]\n", EMR_FIELD_OFS(ESN),
                  flexnfo.esn, esnCmd);
    ConsolePrintf("EMR[%u] hwver       :%u\n", EMR_FIELD_OFS(HW_VER),
                  flexnfo.hwver);
    ConsolePrintf("EMR[%u] model       :%u\n", EMR_FIELD_OFS(MODEL),
                  flexnfo.model);
    ConsolePrintf("EMR[%u] lotcode     :%u\n", EMR_FIELD_OFS(LOT_CODE),
                  lotcode);
    ...
    ConsolePrintf("EMR[%u] packedout   :%u\n", EMR_FIELD_OFS(PACKED_OUT_FLAG),
                  packedout);
    ConsolePrintf("EMR[%u] packout-date:%u\n", EMR_FIELD_OFS(PACKED_OUT_DATE),
                  flexnfo.packoutdate);
    ConsolePrintf("EMR[%u] playpenTouchSensorMinValid:%u\n",
                  EMR_FIELD_OFS(playpenTouchSensorMinValid),
                  playpenTouchSensorMinValid);
    ConsolePrintf("EMR[%u] playpenTouchSensorMaxValid:%u\n",
                  EMR_FIELD_OFS(playpenTouchSensorMaxValid),
                  playpenTouchSensorMaxValid);
    ConsolePrintf("EMR[%u] playpenTouchSensorRangeThresh :%f\n",
                  EMR_FIELD_OFS(playpenTouchSensorRangeThresh),
                  U32FLOAT(playpenTouchSensorRangeThresh));
    ConsolePrintf("EMR[%u] playpenTouchSensorStdDevThresh:%f\n",
                  EMR_FIELD_OFS(playpenTouchSensorStdDevThresh),
                  U32FLOAT(playpenTouchSensorStdDevThresh));
    ConsolePrintf("EMR[%u] playpenTestDisableMask:%08x\n",
                  EMR_FIELD_OFS(playpenTestDisableMask),
                  playpenTestDisableMask);
  }
}
...
static int robot_get_batt_mv(int* out_raw = 0, bool sanity_check = true,
                             int printlvl = RCOM_PRINT_LEVEL_DEFAULT);
...
void TestRobotInfo(void) {
  ...
  if (g_fixmode == FIXMODE_ROBOT0)
    TestCommon::consoleBridge(TO_CONTACTS, 0, 0, BRIDGE_OPT_LINEBUFFER,
                              DBG_cmd_substitution);
  //-*/
...
void TestRobotSensors(void) {
  ...
  // BATTERY (ADC input)
  {
    ...
    ConsolePrintf("bat,raw,%i,mv,%i\n", bat.bat.raw,
                  RCOM_BAT_RAW_TO_MV(bat.bat.raw));
  }

  // CLIFF
  {
    // per software team, intermittent 0 values are common. Sensor is 12?-bit
    // max
    ...
    for (int x = 0; x < NN_cliff; x++) {
      ...
      if (psr[x].cliff.fL >= cliff_min && psr[x].cliff.fL <= cliff_max)
        cnt.fL++;
      if (psr[x].cliff.fR >= cliff_min && psr[x].cliff.fR <= cliff_max)
        cnt.fR++;
      if (psr[x].cliff.bL >= cliff_min && psr[x].cliff.bL <= cliff_max)
        cnt.bL++;
      if (psr[x].cliff.bR >= cliff_min && psr[x].cliff.bR <= cliff_max)
        cnt.bR++;
    }
    robot_sr_t cliff = psr[NN_cliff >> 1];
    ConsolePrintf("cliff,fL,%i,fR,%i,bL,%i,bR,%i\n", cliff.cliff.fL,
                  cliff.cliff.fR, cliff.cliff.bL, cliff.cliff.bR);
    ConsolePrintf("..cnt,fL,%i,fR,%i,bL,%i,bR,%i\n", cnt.fL, cnt.fR, cnt.bL,
                  cnt.bR);
  ...
  // TOUCH btn
  {
    ...
    for (int x = 0; x < NN_touch; x++) {
      const int touch_min = 100,
                touch_max = 1200;  // playpen uses range 500-700 (off charger).
                                   // Ours will be noisier.
      if (psr[x].btn.touch >= touch_min && psr[x].btn.touch <= touch_max)
        validCnt++;
    }
    robot_sr_t btn = psr[NN_touch >> 1];
    ConsolePrintf("btn,touch,%i,btn,%i,cnt,%i\n", btn.btn.touch, btn.btn.btn,
                  validCnt);

    if (validCnt < NN_ok) throw ERROR_SENSOR_TOUCH;
  }
  ...
  // TOF
  {
    ...
    for (int x = 0; x < NN_tof; x++) {
      ...
      if (psr[x].prox.rangeMM < 1 || psr[x].prox.signalRate < 1 ||
          psr[x].prox.spadCnt < 1 || psr[x].prox.ambientRate < 1)
        ecount++;
    }
    robot_sr_t prox = psr[NN_tof >> 1];
    ConsolePrintf("prox,mm,%i,sigRate,%i,spad,%i,ambientRate,%i\n",
                  prox.prox.rangeMM, prox.prox.signalRate, prox.prox.spadCnt,
                  prox.prox.ambientRate);

    if (ecount > emax) throw ERROR_SENSOR_TOF;
  }
}
...
void print_tread_dat(robot_tread_dat_t* dat, const char* treadname) {
  ...
  ConsolePrintf("tread %s FWD speed:%+i avg:%+i travel:%+i\n", name,
                dat->fwd_mid, dat->fwd_avg, dat->fwd_travel);
  ConsolePrintf("tread %s REV speed:%+i avg:%+i travel:%+i\n", name,
                dat->rev_mid, dat->rev_avg, dat->rev_travel);
}
...
static robot_tread_dat_t* robot_tread_test_(uint8_t sensor, int8_t power) {
  ...
  const int printlvl =
      g_isReleaseBuild ? RCOM_PRINT_LEVEL_CMD : RCOM_PRINT_LEVEL_CMD_DAT_RSP;
  ...
  // Forward
  int start_pos =
      rcomGet(1, sensor, printlvl)[0].enc.pos;  // get the idle start position
  psr = rcomMot(100, sensor, pwrL, pwrR, 0, 0, printlvl);
  test.fwd_mid = psr[49].enc.speed;
  for (int x = 10; x < 90; x++) test.fwd_avg += psr[x].enc.speed;
  test.fwd_avg /= (90 - 10);
  ...
  for (int x = 10; x < 90; x++) test.rev_avg += psr[x].enc.speed;
  test.rev_avg /= (90 - 10);
...
#warning "TREAD TEST"
  ...
  if (!IS_FIXMODE_PACKOUT()) TestRobotTreads_(75, 750, 300);
...
typedef struct {
  int start_active,
      start_passive;  // starting positions: active = motor is pushing to
                      // absolute limit, passive = with motor idle
...
void print_range_dat(robot_range_dat_t* dat, const char* sensorname) {
  ...
  ConsolePrintf("%s POS start:%i passive:%i delta:%i\n", name,
                dat->start_active, dat->start_passive,
                dat->start_active - dat->start_passive);
  ConsolePrintf("%s UP  speed:%+i avg:%+i travel:%+i\n", name, dat->up_mid,
                dat->up_avg, dat->up_travel);
  ConsolePrintf("%s DN  speed:%+i avg:%+i travel:%+i\n", name, dat->dn_mid,
                dat->dn_avg, dat->dn_travel);
}

// measure speed and range of motion
static robot_range_dat_t* robot_range_test_(uint8_t sensor, uint8_t NNtest,
                                            int8_t power) {
  power = ABS(power);
  ...
  if (!IS_FIXMODE_ROBOT1()) {
    if (DEBUG_PRINT)
      ConsolePrintf("%s move to starting position [%i,%i]\n",
                    lift ? "LIFT" : "HEAD", -liftPwrMax, -headPwrMax);
    test.start_active = rcomMot(NNstart, sensor, 0, 0, -liftPwrMax, -headPwrMax,
                                printlvl)[NNstart - 1]
                            .enc.pos;
  }
  if (DEBUG_PRINT)
    ConsolePrintf("%s get passive start position\n", lift ? "LIFT" : "HEAD");
  test.start_passive = rcomGet(NNsettle, sensor, printlvl)[NNsettle - 1]
                           .enc.pos;  // allow time for mechanics to settle
  if (IS_FIXMODE_ROBOT1()) test.start_active = test.start_passive;

  // move up
  if (DEBUG_PRINT)
    ConsolePrintf("%s move up [%i,%i]\n", lift ? "LIFT" : "HEAD", liftPwr,
                  headPwr);
  psr = rcomMot(NNtest, sensor, 0, 0, liftPwr, headPwr, printlvl);
  int avg_cnt = 0;
  for (int x = 1; x < NNtest; x++) {
    if (psr[x].enc.speed > 0) {
      avg_cnt++;
      test.up_avg += psr[x].enc.speed;
    }
  }
  ...
  // "HEAD", liftPwrMax, headPwrMax); const uint8_t NNadjust = (power >= 100 ? 15
  // : 30); rcomMot(NNadjust, sensor, 0, 0, liftPwrMax, headPwrMax, printlvl );

  // move down
  if (DEBUG_PRINT)
    ConsolePrintf("%s move down [%i,%i]\n", lift ? "LIFT" : "HEAD", -liftPwr,
                  -headPwr);
  psr = rcomMot(NNtest, sensor, 0, 0, -liftPwr, -headPwr, printlvl);
  avg_cnt = 0;
  for (int x = 1; x < NNtest; x++) {
    psr[x].enc.speed *= (-1);  // XXX: workaround for syscon bug, reports
                               // ABS(speed) for head/lift
    if (psr[x].enc.speed < 0) {
      avg_cnt++;
      test.dn_avg += psr[x].enc.speed;
    }
  }
  ...
  // "HEAD"); rcomGet(NNsettle, sensor, printlvl);
...
void TestRobotRange(robot_range_t* testlift, robot_range_t* testhead) {
  robot_range_dat_t lift =
      *robot_range_test_(RCOM_SENSOR_MOT_LIFT, testlift->NN, testlift->power);
  robot_range_dat_t head =
      *robot_range_test_(RCOM_SENSOR_MOT_HEAD, testhead->NN, testhead->power);
...
#warning "RANGE ERROR CHECKING DISABLED"
  ...
  lift.dn_travel *= -1;
  lift.dn_avg *= -1;  // positive comparisons
  ...
  else if (lift.up_travel < testlift->travel_min ||
           lift.dn_travel < testlift->travel_min)
    throw ERROR_MOTOR_LIFT_RANGE;  // moves, but not enough...
  else if (lift.up_travel > testlift->travel_max ||
           lift.dn_travel > testlift->travel_max)
    throw ERROR_MOTOR_LIFT_NOSTOP;  // moves too much!
  else if (testlift->speed_min > 0 && (lift.up_avg < testlift->speed_min ||
                                       lift.dn_avg < testlift->speed_min))
  ...
  head.dn_travel *= -1;
  head.dn_avg *= -1;  // positive comparisons
  ...
  else if (head.up_travel < testhead->travel_min ||
           head.dn_travel < testhead->travel_min)
    throw ERROR_MOTOR_HEAD_RANGE;  // moves, but not enough...
  else if (head.up_travel > testhead->travel_max ||
           head.dn_travel > testhead->travel_max)
    throw ERROR_MOTOR_HEAD_NOSTOP;  // moves too much!
  else if (testhead->speed_min > 0 && (head.up_avg < testhead->speed_min ||
                                       head.dn_avg < testhead->speed_min))
...
#warning "RANGE TESTING"
  for (int pwr = 100; pwr >= 45; pwr -= 5) {
    if (pwr >= 100 || (pwr < 71 && pwr > 39)) {  // DEBUG limited range of
                                                 // values
      ConsolePrintf("RANGE TEST pwr = %i\n", pwr);
      uint8_t NNlift = pwr < 75 ? 85 : 55;
      uint8_t NNhead = pwr < 90 ? 125 : 70;
      robot_range_t lift = {NNlift, /*power*/ pwr, /*travel_min*/ 0,
                            /*travel_max*/ 99999, /*speed_min*/ 0};
      robot_range_t head = {NNhead, /*power*/ pwr, /*travel_min*/ 0,
                            /*travel_max*/ 99999, /*speed_min*/ 0};
  ...
  if (IS_FIXMODE_ROBOT1()) {
    ...
    robot_range_t lift = {/*NN*/ 55, /*power*/ 75, /*travel_min*/ 400,
                          /*travel_max*/ 9999, /*speed_min*/ 1800};
    robot_range_t head = {/*NN*/ 70, /*power*/ 100, /*travel_min*/ 700,
                          /*travel_max*/ 9999, /*speed_min*/ 2300};
    TestRobotRange(&lift, &head);
  } else if (!IS_FIXMODE_PACKOUT()) {
    // lift: travel 195-200, speed 760-1600
    // head: travel 550-560, speed 2130-2400
    robot_range_t lift = {/*NN*/ 55, /*power*/ 75, /*travel_min*/ 170,
                          /*travel_max*/ 230, /*speed_min*/ 650};
    robot_range_t head = {/*NN*/ 70, /*power*/ 100, /*travel_min*/ 520,
                          /*travel_max*/ 590, /*speed_min*/ 1700};
    TestRobotRange(&lift, &head);
  }
  ...
  if (IS_FIXMODE_ROBOT1()) {
    ...
    robot_range_t lift = {/*NN*/ 85, /*power*/ 50, /*travel_min*/ 400,
                          /*travel_max*/ 9999, /*speed_min*/ 800};
    robot_range_t head = {/*NN*/ 125, /*power*/ 55, /*travel_min*/ 550,
                          /*travel_max*/ 9999, /*speed_min*/ 700};
    TestRobotRange(&lift, &head);
  } else {
    // lift: travel 190-200, speed 580-1520
    // head: travel 545-555, speed 900-1230
    robot_range_t lift = {/*NN*/ 85, /*power*/ 65, /*travel_min*/ 170,
                          /*travel_max*/ 230, /*speed_min*/ 400};
    robot_range_t head = {/*NN*/ 125, /*power*/ 60, /*travel_min*/ 520,
                          /*travel_max*/ 590, /*speed_min*/ 700};
    TestRobotRange(&lift, &head);
  }
...
#define U32FLOAT(u) (*((float*)&(u)))
    ...
    rcomSmr(EMR_FIELD_OFS(playpenTouchSensorRangeThresh),
            FLOAT2U32(rangeThres));
    float stdDevThres = 3.1f;  // default 1.8
    rcomSmr(EMR_FIELD_OFS(playpenTouchSensorStdDevThresh),
            FLOAT2U32(stdDevThres));

    // disable some playpen errors
    const uint32_t PlaypenTestMask =
        0  // from clad/src/clad/types/factoryTestTypes.clad
...
void RobotPowerDown(void) {
  ...
  if (!IS_FIXMODE_ROBOT1()) {
    ...
    cleanup_preserve_vext =
        1;  // leave power on for removal detection (no cleanup pwr cycle)

    /*/DEBUG:
    ConsolePrintf("delay for manual powerdown check. press a key to skip\n");
    uint32_t Tstart = Timer::get();
    while( ConsoleReadChar() > -1 );
    while( Timer::elapsedUs(Tstart) < 10*1000*1000 ) { if( ConsoleReadChar() >
    -1 ) break; }
    //-*/
  } else {
    ...
    cleanup_preserve_vext =
        1;  // leave power on for removal detection (no cleanup pwr cycle)
    Timer::delayMs(SYSCON_CHG_PWR_DELAY_MS +
                   100);  // wait for charger to kick in (known state)
    ...
    if (spine_mv > 250) throw ERROR_BODY_CANNOT_POWER_OFF;

    /*/DEBUG
    ConsolePrintf("Power Off Debug:\n");
    for(int wait=5; wait >0; wait--) {
      ConsolePrintf("%is spine_mv=%i VEXT=%s\n", wait,
    Meter::getVoltageMv(PWR_DUTVDD,5), Contacts::powerIsOn()?"on":"off");
      Timer::delayMs(1000);
    }//-*/
  }
}
...
static void led_manage_(int reset, int frame_period = 250 * 1000,
                        int printlvl = RCOM_PRINT_LEVEL_NONE);
...
void DEBUG_TestRobotLeds(void) {
  ...
  const int framerate = 1000 * 1000;
  const int testtime = 20 * 1000 * 1000;
  uint32_t Tstart = Timer::get();
  led_manage_(1, framerate, RCOM_PRINT_LEVEL_CMD);
  while (Timer::elapsedUs(Tstart) < testtime) {
    led_manage_(0, framerate, RCOM_PRINT_LEVEL_CMD);
  }
  led_manage_(1, framerate, RCOM_PRINT_LEVEL_CMD);
}
...
static void btn_sample_(int* press_cnt, int* release_cnt) {
  ...
  if (Timer::elapsedUs(Tsample) > 30 * 1000) {
    ...
    if (press_cnt != NULL)
      *press_cnt =
          !pressed ? 0 : (*press_cnt >= 65535 ? 65535 : *press_cnt + 1);
    if (release_cnt != NULL)
      *release_cnt =
          pressed ? 0 : (*release_cnt >= 65535 ? 65535 : *release_cnt + 1);
  }
}
...
static inline int charge1_(uint16_t timeout_s, uint16_t i_done_ma,
                           bool dbgPrint) {
  ...
  uint32_t Tdlatch =
      Timer::get() -
      (8 * 1000 *
       1000);  // first latch after ~2s. Waits for charger to kick in.

  // Turn on charging power
  Board::powerOn(PWR_VEXT, 0);
  Timer::delayMs(SYSCON_CHG_PWR_DELAY_MS);  // delay for syscon to enable
                                            // charger
  ...
  while (Timer::elapsedUs(Tstart) < (timeout_s * 1000 * 1000)) {
    ...
    // Debug: print real-time current usage in console (erasing behavior can
    // cause problems with some terminals, logging etc)
  ...
  if (print_len) ConsolePutChar('\n');

  Board::powerOff(PWR_VEXT);
  return status;
}

// recharge cozmo. parameterized conditions: time, battery voltage, current
// threshold
int m_Recharge(uint16_t max_charge_time_s, uint16_t bat_limit_mv,
               uint16_t i_done_ma, bool dbgPrint) {
  const u8 BAT_CHECK_INTERVAL_S =
      90;  // interrupt charging this often to test battery level
  ConsolePrintf("recharge,%dcV,%dmA,%ds\r\n", bat_limit_mv, i_done_ma,
                max_charge_time_s);
  ...
  while (bat_limit_mv == 0 || batt_mv < bat_limit_mv) {
    if (max_charge_time_s > 0 &&
        Timer::elapsedUs(Tstart) >= ((uint32_t)max_charge_time_s * 1000 * 1000))
    ...
    if (chgStat != RECHARGE_STATUS_TIMEOUT) return chgStat;
  }
...
void Recharge(void) {
  ...
  const u16 BAT_FULL_I_THRESH_MA =
      200;  // current threshold for charging complete (experimental)
  int status;
  ...
  if (g_fixmode == FIXMODE_RECHARGE0)
    status = m_Recharge(2 * BAT_MAX_CHARGE_TIME_S, 0, BAT_FULL_I_THRESH_MA,
                        dbgPrint);  // charge to full battery
  else
    status = m_Recharge(BAT_MAX_CHARGE_TIME_S, VBAT_CHARGE_LIMIT_MV, 0,
                        dbgPrint);  // charge to specified voltage

  if (status == RECHARGE_STATUS_TIMEOUT) throw ERROR_TIMEOUT;

  if (status == RECHARGE_STATUS_OK)
    cmdSend(CMD_IO_CONTACTS, "powerdown", 50,
            CMD_OPTS_DEFAULT & ~(CMD_OPTS_LOG_ERRORS | CMD_OPTS_EXCEPTION_EN));
}
...
//@param bat_overvolt_mv - battery too full voltage level. special failure
//handling above this threshold.
...
#define CHARGE_TEST_DEBUG(x) x
  ...
  while (Timer::elapsedUs(Twait) < 5 * 1000 * 1000) {
    ...
    CHARGE_TEST_DEBUG({
      ...
      if (ABS(current_ma - ibase_ma) >= IDIFF_MA ||
          ABS(avg - ibase_ma) >= IDIFF_MA ||
          Timer::elapsedUs(Tprint) > 500 * 1000 ||
          (avgCnt >= NUM_SAMPLES && avg >= i_done_ma)) {
        ibase_ma = current_ma;
        Tprint = Timer::get();
        ConsolePrintf("%04umV %03d/%03d ", voltage_mv, avg, current_ma);
        for (int x = 1; x <= (avg > current_ma ? avg : current_ma);
             x += DISP_MA_PER_CHAR)
          ConsolePrintf(x <= avg && x <= current_ma ? "="
                                                    : x > avg ? "+" : "-");
        ConsolePrintf("\n");
      }
    });
    ...
    if (avgCnt >= NUM_SAMPLES && avg >= i_done_ma) break;

    // error out quickly if robot removed from charge base
    if ((offContact = current_ma < PRESENT_CURRENT_MA ? offContact + 1 : 0) >
        20) {
    ...
    if (voltage_mv < undervolt || voltage_mv > overvolt) {
      ...
      throw voltage_mv < undervolt ? ERROR_OUTPUT_VOLTAGE_LOW
                                   : ERROR_OUTPUT_VOLTAGE_HIGH;
    }
  }
  ...
  ConsolePrintf("charge-current-dbg,avgMax,%d,%d,iMax,%d,%d\r\n", avgMax,
                avgMaxTime, iMax, iMaxTime);
  if (avgCnt >= NUM_SAMPLES && avg >= i_done_ma) return;  // OK

  if (batt_mv >= bat_overvolt_mv)
    throw ERROR_BAT_OVERVOLT;  // error prompts operator to put robot aside for
                               // a bit
...
static void RobotLogCollect(void) {
  ...
  for (int i = 0; i < numlogs; i++) {
    ...
    error_t e = ERROR_OK;
    try {
      len = rcomRlg(i, &logbuf[ofs], logbufsize - 1 - ofs);
    } catch (int err) {
      e = err;
      len = 0;
    }
...
static void RobotFlexFlowPackoutReport(void) {
  ...
  FLEXFLOW::printf("<flex> log packout_%08x_fix.log\n", flexnfo.esn);
  {
    ...
    FLEXFLOW::printf("packout-date %010u %s", flexnfo.packoutdate,
                     ctime(&flexnfo.packoutdate));
    ...
    FLEXFLOW::printf("body-app-vers %08x %08x %08x %08x ", bsv->app_version[0],
                     bsv->app_version[1], bsv->app_version[2],
                     bsv->app_version[3]);
    for (int x = 0; x < 16; x++) {
  ...
  bool valid_head_esn = !(flexnfo.esn == 0 || flexnfo.esn == 0xFFFFffff ||
                          (flexnfo.esn & 0xFFF00000) == 0);
  if (!valid_head_esn) throw ERROR_ROBOT_INVALID_ESN;

  bool valid_body_ein =
      !(flexnfo.bsv.ein[0] == 0 || flexnfo.bsv.ein[0] == 0xFFFFffff ||
        (flexnfo.bsv.ein[0] & 0xFFF00000) == 0);
  if (!valid_body_ein) {
    ConsolePrintf("--- ERROR_ROBOT_INVALID_BODY_EIN ---\n");
    if (g_isReleaseBuild) throw ERROR_ROBOT_INVALID_BODY_EIN;
  }

  // sanity check known logs
  for (int i = 0; i < numlogs; i++) {
    int len_min =
        i == 0 ? 750 : 250;  // 0=playpen (~1450 bytes), 1=cloud (~445 bytes)
    if (!flexnfo.log[i] || flexnfo.loglen[i] < len_min) {
      ConsolePrintf("--- ERROR_ROBOT_BAD_LOGFILE[%i] len %i ---\n", i,
                    flexnfo.loglen[i]);
      if (g_isReleaseBuild) throw ERROR_ROBOT_BAD_LOGFILE;
    }
  }
  ...
  if (!flexnfo.bat_mv && !g_isReleaseBuild) {
    ConsolePrintf(
        "----------\nBAD_ARG: PackoutReport() esn=%08x ein=%08x "
        "bat_mv=%i\n----------\n",
...
static void BatteryCheck(void) {
  ...
  if (flexnfo.bat_mv < VBAT_MV_MINIMUM) throw ERROR_BAT_UNDERVOLT;

  if (IS_FIXMODE_PACKOUT()) {
    if (flexnfo.bat_mv > VBAT_MV_MAXIMUM) throw ERROR_BAT_OVERVOLT;
  }
}
...
void LogDownload(void) {
  ...
  for (int i = 0; true; i++) {
    ...
    int len = 0;
    int printlvl =
        RCOM_PRINT_LEVEL_ALL;  // RCOM_PRINT_LEVEL_CMD | RCOM_PRINT_LEVEL_RSP;
    try {
      len = rcomRlg(i, &logbuf[0], logbufsize - 1, printlvl);
    } catch (int err) {
      e = err;
      len = 0;
    }
...
void SweatinToTheOldies(void) {
  ...
  while (1) {
    // feel the burn
    try {
      int8_t treadPwrL = 100;
      int8_t treadPwrR = 100;
      int8_t liftPwr = 70;
      int8_t headPwr = 70;
      int printlvl =
          RCOM_PRINT_LEVEL_CMD;  // | RCOM_PRINT_LEVEL_NFO; //
                                 // RCOM_PRINT_LEVEL_DAT | RCOM_PRINT_LEVEL_RSP
      rcomMot(120, RCOM_SENSOR_MOT_LEFT, treadPwrL, -treadPwrR, liftPwr,
              -headPwr, printlvl);
      rcomMot(120, RCOM_SENSOR_MOT_RIGHT, -treadPwrL, treadPwrR, -liftPwr,
              headPwr, printlvl);
    } catch (...) {
    }
    ...
    if (batt_mv <= batt_mv_cutoff) break;
    //-*/
    ...
    // int voltage_mv = Meter::getVoltageMv(PWR_VEXT,4);
    if (current_ma < PRESENT_CURRENT_MA) break;
    //-*/
  }
}
...
TestFunction* TestRobot3GetTests(void) {
  static TestFunction m_tests[] = {
      TestRobotButton,  TestRobotInfo,
      EmrChecks,     // check previous test results and reset status flags
      BatteryCheck,  // sensor/motors may act strange if battery is low
      TestRobotSensors, TestRobotTreads,
      TestRobotRange,   ChargeTest,
