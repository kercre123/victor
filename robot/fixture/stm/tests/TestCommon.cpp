...
void powerOnProtected(pwr_e net, uint16_t time_ms, int ilimit_ma,
                      int oversample) {
  ...
  DBG_VERBOSE(for (int i = 0; i < icnt; i++) {
    if (i && !(i % 16)) ConsolePrintf("\n");
    ConsolePrintf("%03i,", ilog[i]);
  } ConsolePrintf("\n"););

  if (ima > ilimit_ma) throw ERROR_POWER_SHORT;
}
}  // namespace TestCommon
...
static inline const char* which_s_(bridge_target_e which) {
  switch (which) {
    case TO_DUT_UART:
      return "DUT UART";  // break;
    case TO_CONTACTS:
      return "Charge Contacts";  // break;
    default:
      return "???";  // break;
  }
}
...
static inline int bridge_getchar_(bridge_target_e which) {
  switch (which) {
    ...
    case TO_CONTACTS:
      return !Contacts::powerIsOn() ? Contacts::getchar() : -1;  // break;
    default:
      return -1;  // break;
  }
}
...
static inline void bridge_get_errs_(bridge_target_e which, int e[]) {
  e[0] = which == TO_DUT_UART ? DUT_UART::getRxDroppedChars()
                              : Contacts::getRxDroppedChars();
  e[1] = which == TO_DUT_UART ? DUT_UART::getRxOverflowErrors()
                              : Contacts::getRxOverflowErrors();
  e[2] = which == TO_DUT_UART ? DUT_UART::getRxFramingErrors()
                              : Contacts::getRxFramingErrors();
}

void consoleBridge(bridge_target_e which, int inactivity_delay_ms,
                   int timeout_ms, int opts,
                   bridge_hook_sendline_t hook_sendline) {
  const char cmd_exit[5] = "exit";
  int exit_idx = 0;
  const char cmd_echo[5] = "echo";
  int echo_idx = 0;
  const char cmd_buff[5] = "buff";
  int buff_idx = 0;
  const char cmd_stats[6] = "stats";
  int stats_idx = 0;
  const char cmd_charge[7] = "charge";
  int charge_idx = 0;
  ...
  ConsolePrintf(
      "Console Bridge <-> %s for %ims,inactive:%ims [or type 'exit']\n",
  ...
  while (1) {
    ...
    if (Timer::elapsedUs(Tstart) >= 1000) {
      Tstart = Timer::get();
      if (timeout_ms > 0 && --timeout_ms == 0) break;
      if (inactivity_delay_ms > 0 && --inactivity_delay_ms == 0) break;
      if (which == TO_CONTACTS && charge_en) {
        if (++idle_ms >= charge_idle_ms && !Contacts::powerIsOn()) {
          // ConsolePrintf("idle timeout. charging enabled\n");
          Contacts::powerOn();  // Note: prevents rx from contacts until console
                                // is re-actviated
    ...
    do {
      if ((c = bridge_getchar_(which)) > -1) {
        if (c > 0 && c <= 0x7f) {  // limit to ascii, ignore null. getline()
                                   // methods usually do this.
          idle_ms = 0;
          ConsolePutChar(c);
        }
      }
    ...
    if ((c = ConsoleReadChar()) > -1) {
        ...
        if (c > 0 && c < 0x80)  // ignore null and non-ascii
        {
          if (c == '\r' || c == '\n') {  // enter or return
            ...
            if (line_len > 0) {
              ...
              if (hook_sendline !=
                  NULL)  // hook for external alias/substitutions
              {
                ...
                if (sub_line /*&& sub_len > 0*/) {
                  sub_len = sub_len > line_maxlen
                                ? line_maxlen
                                : sub_len;  // limit to our buffer size
                  ...
                  for(int x=0; x<line_len; x++) {
                    if( line[x] == '\n' ) ConsolePrintf("/n");
                    else ConsolePutChar( line[x] >= ' ' && line[x] < '~' ?
                  line[x] : '*'); } ConsolePrintf("' (%i)\n", line_len);
                  //-*/

                  allow_send = 0;  // don't send this now; leave in console for
                                   // user to modify
                  for (int x = 0; x < line_len;
                       x++) {  // be helpful and type the substitute line into
                               // the console
              ...
              if (allow_send) {
                for (int x = 0; x < line_len; x++)
                  bridge_putchar_(
                      which, line[x],
                      1);  // dump the entire line (hold TX mode in half-duplex)
            ...
            if (recall_len > 0) {
              for (int x = 0; x < recall_len; x++)
                ConsolePutChar(line[x]);  // re-print last command on the
                                          // console
      ...
      if (esc_cnt >= 5) {
        ...
        while (Timer::elapsedUs(Tanki) < 15 * 1000 * 1000) ConsoleUpdate();
      ...
      if ((c == '\r' || c == '\n') &&
        ...
        ConsolePrintf("drop:%i ovf:%i frame:%i\n", e.dropped_chars, e.overflow,
                      e.framing);
        stats_idx = 0;
      } else
        stats_idx =
            (c == cmd_stats[stats_idx]) ? stats_idx + 1 : 0;  // index track
      ...
      if ((c == '\r' || c == '\n') &&
        ...
        charge_idx =
            (c == cmd_charge[charge_idx]) ? charge_idx + 1 : 0;  // index track
