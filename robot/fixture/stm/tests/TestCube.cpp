...
uint32_t cubebootSignature(bool dbg_print, int* out_cubeboot_size) {
  ...
  if (!crc_init)  // cache crc. hard-coded bins won't change at runtime, I
                  // pinky-swear
  {
    // OTP stub contains nested cubeboot.bin application. location/size info is
    // reported at hard-coded addresses...
    const uint32_t cubeboot_magic = ((uint32_t*)g_CubeStub)[40];
    const uint32_t cubeboot_start =
        ((uint32_t*)g_CubeStub)[41] &
        0x0FFFffff;  // ignore dialog's OTP/Ram section placements
    const uint32_t cubeboot_end =
        ((uint32_t*)g_CubeStub)[42] &
        0x0FFFffff;  // ignore dialog's OTP/Ram section placements
    const int cubeboot_size = cubeboot_end - cubeboot_start;
    const int cubestub_size = g_CubeStubEnd - g_CubeStub;

    // validate header
    bool bad_header = cubeboot_magic != 0xc0beb007 || cubeboot_size < 4000 ||
                      cubeboot_size >= cubestub_size ||
                      cubeboot_start >= cubestub_size ||
                      cubeboot_end >= cubestub_size;
#if TESTCUBE_DEBUG > 0
    if (bad_header || dbg_print) {
      ...
      ConsolePrintf("  g_CubeBoot: %08x-%08x (%i) magic %08x\n", cubeboot_start,
                    cubeboot_end - 1, cubeboot_size, cubeboot_magic);
    }
#endif
    if (bad_header) {
#if TESTCUBE_DEBUG > 0
...
// DEBUG: byte-for-byte compare to actual binary
    ...
    if (cubeboot_raw_size >
        0)  // must be included by 'binaries.s'. Development check.
    {
      int mismatch_cnt = 0;
      ConsolePrintf("CubeStub[CubeBoot] binary compare to cubeboot.bin:\n");
      ConsolePrintf(".size: %u, %u(raw) %s\n", cubeboot_size, cubeboot_raw_size,
                    cubeboot_size != cubeboot_raw_size ? "--MISMATCH--" : "");
      ...
      for (int x = 0; x < MIN(cubeboot_size, cubeboot_raw_size); x++) {
        if (g_CubeStub[cubeboot_start + x] != g_CubeBoot[x]) {
          ConsolePrintf(".MISMATCH @ %08x: %02x,%02x\n", x,
                        g_CubeStub[cubeboot_start + x], g_CubeBoot[x]);
          if (++mismatch_cnt > 20) break;
        }
      }
  ...
  if (out_cubeboot_size) *out_cubeboot_size = m_cubeboot_size;
  if (dbg_print) ConsolePrintf("cubeboot crc32=%08x\n", crc);
  return crc;
}
...
static void da14580_load_program_(const uint8_t* bin, int size,
                                  const char* name) {
  ConsolePrintf("da14580 load %s: %ikB (%u)\n", (name ? name : "program"),
                CEILDIV(size, 1024), size);
  TestCubeCleanup();
  ...
  while (1) {
    ...
    if (ABS(vbat3v_mv - vbat3v_last) > 25 &&
        Timer::elapsedUs(Tupdate) > 50 * 1000)  // detect changes, rate limited
    {
      vbat3v_last = vbat3v_mv;
      Tupdate = Timer::get();
      ConsolePrintf("%u,", vbat3v_mv);
    }
    if (Timer::elapsedUs(Tupdate) > 250 * 1000 ||
        Timer::elapsedUs(Tstart) > 3000 * 1000)  // stable or timeout
      break;
  }
  ConsolePrintf("%u\n", vbat3v_mv);  // print final value

  if (vbat3v_mv < 2500) throw ERROR_CUBE_BAD_POWER;  // ERROR_BAT_UNDERVOLT;
  if (vbat3v_mv > 2900) throw ERROR_BAT_OVERVOLT;    // ERROR_CUBE_BAD_POWER
  ...
  while (1) {
    ...
    {
      // Send boot header (quick! need to engage bootloader. take time to print
      // after...)
      ...
      if (ack != 0x06)                  // Note: 0x15 == NACK
        throw ERROR_CUBE_CANNOT_WRITE;  // bootloader rejected length, or
                                        // protocol error etc.
      ...
      if (crc != crc_local) throw ERROR_CUBE_VERIFY_FAILED;

      // send ACK to run the image
      DUT_UART::putchar(0x06);  // ACK
      Timer::wait(
          1000);  // make sure ack clears the output SR before disabling UART
      // ConsolePrintf("pgm time: %ums\n", Timer::elapsedUs(Tstart)/1000);
      // //XXX: this doesn't work. Timer isn't updated if not polling...
    ...
    if (c == 0xB2)  // sync char from programmed cube
    {
      ...
      int len = 0;
      char* rsp =
          DUT_UART::getline(buf, sizeof(buf), 50 * 1000,
                            &len);  // discards non-ascii (repeat sync chars)
      if (len > 0) {
        ConsolePrintf("rx '%s'\n", buf);  // includes incomplete lines...
        int nargs = !rsp ? 0 : cmdNumArgs(rsp);

        if (rsp && nargs >= 6 &&
            !strcmp(cmdGetArg(rsp, 0), "cubeboot"))  // cube POR boot message
        {
          ...
          uint32_t expected_model = g_fixmode == FIXMODE_CUBE2
                                        ? CUBEID_MODEL_CUBE2
                                        : CUBEID_MODEL_CUBE1;
          if (info.hwrev != CURRENT_CUBE_HW_REV ||
              info.model != expected_model || info.crc != crc_current) {
    ...
    if (Timer::elapsedUs(Tstart) >
        500 * 1000)  // timeout waiting for bootloader start char
...
void DbgPowerWaitTest(void) {
  ...
  cmdSend(
      CMD_IO_DUT_UART, "echo on", CMD_DEFAULT_TIMEOUT,
      (CMD_OPTS_DEFAULT | CMD_OPTS_ALLOW_STATUS_ERRS) & ~CMD_OPTS_EXCEPTION_EN);
...
bool TestCubeDetect(void) {
  ...
  DUT_SWC::init(MODE_INPUT, PULL_NONE);  // prevent pu from doing strange things
                                         // to mcu (phantom power?)
...
static void CubeTest(void) {
  ...
  for (int n = 0; n < sizeof(ledtest) / sizeof(led_test_t); n++) {
    cmdSend(CMD_IO_DUT_UART, snformat(b, bz, "leds 0x%x %s", ledtest[n].bits,
                                      (ledtest[n].duty <= 1 ? "static" : "")));
    Timer::delayMs(20);
    ...
    do {
      ledtest[n].i_meas = Meter::getCurrentMa(PWR_CUBEBAT, oversample++);
             ...
             !ledtest_pass(&ledtest[n], ledtest[n].i_meas - i_base));
  ...
  for (int n = 0; n < sizeof(ledtest) / sizeof(led_test_t); n++) {
    ...
    ConsolePrintf("%s current %imA %s\n", ledtest[n].name, i_test,
                  pass ? "ok" : "--FAIL--");
    if (!pass) e = ledtest[n].e;
  }
  ...
  // cmdSend(CMD_IO_DUT_UART, "leds 0x1 static"); //pull some led current for
  // typical usecase cmdSend(CMD_IO_DUT_UART, "vled 1");
  Timer::delayMs(50);  // wait for VLED boost reg to stabilize
  cmdSend(CMD_IO_DUT_UART, "accel", 250,
          CMD_OPTS_DEFAULT | CMD_OPTS_ALLOW_STATUS_ERRS);
  if (cmdStatus() == 10) throw ERROR_CUBE_ACCEL_PWR;
  if (cmdStatus() != 0)  // == 11
...
static void OTPbootloader(void) {
  ...
  // Generate bd address
  bdaddr_t bdaddr;
  bdaddr_generate(&bdaddr,
                  GetRandom);  // use RNG peripheral for proper randomness

  // prepare hwardware ids
  cubeid.esn = CUBEID_ESN_INVALID;
  cubeid.hwrev = CURRENT_CUBE_HW_REV;
  cubeid.model = (g_fixmode == FIXMODE_CUBE1)
                     ? CUBEID_MODEL_CUBE1
                     : ((g_fixmode == FIXMODE_CUBE2) ? CUBEID_MODEL_CUBE2
                                                     : CUBEID_MODEL_INVALID);
  ...
  {
    char* cmd =
        snformat(b, bz, "otp write %s %08x %u %u %08x", bdaddr2str(&bdaddr),
                 cubeid.esn, cubeid.hwrev, cubeid.model, crc);

    // only burn OTP for valid cube types
    if (g_fixmode == FIXMODE_CUBE1 || g_fixmode == FIXMODE_CUBE2) {
      cmdSend(CMD_IO_DUT_UART, cmd, 60 * 1000,
              (CMD_OPTS_DEFAULT | CMD_OPTS_ALLOW_STATUS_ERRS) &
                  ~CMD_OPTS_EXCEPTION_EN);
  ...
  if (write_result != 0) throw ERROR_CUBE_VERIFY_FAILED;
}

static void burnFCC(void) {
  da14580_load_program_(g_CubeStubFcc, g_CubeStubFccEnd - g_CubeStubFcc,
                        "cube fcc");
  Timer::delayMs(250);  // wait for ROM+app reboot sequence
  DUT_UART::init(57600);
  ...
  Board::powerOn(PWR_DUTPROG);
  {
    cmdSend(CMD_IO_DUT_UART, "otp write fcc", 60 * 1000,
            (CMD_OPTS_DEFAULT | CMD_OPTS_ALLOW_STATUS_ERRS) &
                ~CMD_OPTS_EXCEPTION_EN);
    write_result = cmdStatus();
  }
...
TestFunction* TestCube1GetTests(void) {
  static TestFunction m_tests[] = {
      ShortCircuitTest, CubeTest, OTPbootloader, CubeFlexFlowReport, NULL,
  };
  return m_tests;
}
...
bool TestBlockDetect(void) { return false; }
...
TestFunction* TestBlock2GetTests(void) { return TestBlock1GetTests(); }
