...
#define USART_RCC_EN(x) \
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART2, ((x) ? ENABLE : DISABLE))
#define GPIO_RCC_EN(x) \
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOA, ((x) ? ENABLE : DISABLE))
...
#define USART_RCC_EN(x) \
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART6, ((x) ? ENABLE : DISABLE));
#define GPIO_RCC_EN(x) \
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOC, ((x) ? ENABLE : DISABLE))
#else
...
static inline void putchar_(char c) {
  ...
  while (!(USART->ISR & USART_ISR_TXE))
    ;  // dat moved to shift register (ok to write a new data)
  while (!(USART->ISR & USART_ISR_TC))
    ;  // wait for transmit complete
#elif TARGET_FIXTURE
  volatile int junk =
      USART->SR;  // TC bit is cleared by 'read from the USART_SR register
...
void Contacts::init(void) {
  if (mode != MODE_UNINITIALIZED) return;
...
// Config uart pins
  ...
  USART->CR2 = 0;
  USART->CR3 = TARGET_SYSCON ? USART_CR3_HDSEL : 0; //syscon=half-duplex.
  duplexing is external in fixture hw USART->CR1 = USART_CR1_UE | USART_CR1_TE |
...
int Contacts::printf(const char* format, ...) {
  if (mode == MODE_UNINITIALIZED) return 0;
...
int Contacts::get(void) {
  int c;
  if (mode == MODE_UNINITIALIZED) return -1;
...
#if TARGET_SYSCON
  if (USART->ISR &
      (USART_ISR_ORE | USART_ISR_FE)) {  // framing and/or overrun error
...
#elif TARGET_FIXTURE
  if (USART->SR &
      (USART_SR_ORE | USART_SR_FE)) {  // framing and/or overrun error
