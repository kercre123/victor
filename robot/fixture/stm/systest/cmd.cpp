...
char* console_getarg(char* s, int n, char* buf, int buflen) {
  ...
  while (n--) {                                   // seek to nth arg
    if ((s = next_arg_(s)) == NULL) return NULL;  // there is no spoon
  }
...
int cmd_process(char* s) {
  ...
  if (!s) return STATUS_NULL;

  // is this a command?
  int len = strlen(s);
  if (!strncmp(s, CMD_PREFIX, sizeof(CMD_PREFIX) - 1) &&
      len > sizeof(CMD_PREFIX) - 1)  // valid cmd formatting
    s += sizeof(CMD_PREFIX) - 1,
        len -= sizeof(CMD_PREFIX) - 1;  // skip the prefix for parsing
  else
  ...
  if (!strcmp(cmd, "leds")) {
    ...
    int ledbf = strtol(console_getargl(s, 1), 0,
                       0);  // hex ok - must have 0x prefix for cstd parser
  ...
  if (!strcmp(cmd, "delay")) {
    ...
    int ms = strtol(console_getargl(s, 1), 0,
                    0);  // hex ok - must have 0x prefix for cstd parser
  ...
  if (!strcmp(cmd, "vmain")) {
    ...
    if (nargs < 2) return respond_(cmd, STATUS_ARG_NA, "missing-args");
  ...
  if (!strcmp(cmd, "encoders")) {
    ...
    if (!rtok) return respond_(cmd, ERROR_BODY_TREAD_ENC_RIGHT, 0);
    if (!ltok) return respond_(cmd, ERROR_BODY_TREAD_ENC_LEFT, 0);
  ...
  if (!strcmp(cmd, "get")) {
    int nargs = console_num_args(s);
    if (nargs < 3) return respond_(cmd, STATUS_ARG_NA, "missing-args");

    int NN = strtol(console_getargl(s, 1), 0, 0);
    int sr = strtol(console_getargl(s, 2), 0, 0);
    if (NN < 1 || NN > 255) return respond_(cmd, STATUS_ARG_NA, "NN-invalid");

    //-------------------------------------
    // Get Battery [:batAdc temp]
    //-------------------------------------
    if (sr == RCOM_SENSOR_BATTERY) {
#define VIN_RAW_TO_MV(raw) \
  (((raw)*2800) >> 11) /*robot_sr_t::bat.raw (adc) to millivolts*/
      ...
      // XXX
      writes_(snformat(
          b, bz, "WARNING: adc disabled. reporting dummy data for debug\n"));
...
#define VALUE(value) 1, (void*)value
      ...
      // I2C::capture();
      {
        ...
        const I2C_Operation I2C_LOOP[] = {
            {I2C_REG_READ, 0, DROP_SENSOR_ADDRESS, PS_DATA_0,
             TARGET(cliffSense[3])},
            {I2C_REG_READ, 1, DROP_SENSOR_ADDRESS, PS_DATA_0,
             TARGET(cliffSense[2])},
            {I2C_REG_READ, 2, DROP_SENSOR_ADDRESS, PS_DATA_0,
             TARGET(cliffSense[1])},
            {I2C_REG_READ, 3, DROP_SENSOR_ADDRESS, PS_DATA_0,
             TARGET(cliffSense[0])},
            {I2C_DONE},
        };
        ...
        for (int i = 0; i < 4; i++) {
          ...
          I2C::writeReg(i, DROP_SENSOR_ADDRESS, PS_MEAS_RATE, 3 | (3 << 3) |
        0x40); I2C::writeReg(i, DROP_SENSOR_ADDRESS, PS_CAN_0, 0);
          I2C::writeReg(i, DROP_SENSOR_ADDRESS, PS_CAN_1, 0);
        }*/

        // XXX
        writes_(snformat(b, bz,
                         "WARNING: cliff sensors disabled. reporting dummy "
                         "data for debug\n"));
        ...
        for (int n = 0; n < NN; n++) {
          ...
          writes_(snformat(b, bz, ":%i %i %i %i\n", cliffSense[0],
                           cliffSense[1], cliffSense[2], cliffSense[3]));
          Timer::delayMs(5);
        }
