/** Clad definition file for messages sent between Application processor and
 * Hardware processor.
 *
...
/* All spine messages follow the following format:
    ...
    Huge majority of messages will be
       ...
       *Important*: Payload Type can not contain any of the Sync characters.
                    This allows us to avoid rescanning them when we get invalid
   ones. BYTECOUNT = uint16: bytes  to follow (excluding checksum). PAYLOAD one
   of the message types below CHECKSUM is uint32 bytewise sum of payload.
*/
...
enum { RUNNING_FLAGS_SENSORS_VALID = 1 };
...
struct BatteryState {
  int16_t main_voltage;  // This is battery voltage when the charger is
                         // unpowered / disconnected from VEXT
  int16_t charger;
  int16_t temperature;
  BatteryFlags flags;
  int16_t _unused[2];
...
struct BodyToHead {
  ...
  uint16_t
      micError[2];  // Raw bits from a segment of mic data (stuck bit detect)
