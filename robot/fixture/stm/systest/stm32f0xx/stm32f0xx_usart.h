...
typedef struct {
  ...
  uint32_t USART_WordLength; /*!< Specifies the number of data bits transmitted
...
#define IS_USART_ALL_PERIPH(PERIPH)                                        \
  (((PERIPH) == USART1) || ((PERIPH) == USART2) || ((PERIPH) == USART3) || \
   ((PERIPH) == USART4) || ((PERIPH) == USART5) || ((PERIPH) == USART6) || \
   ((PERIPH) == USART7) || ((PERIPH) == USART8))
...
#define USART_WordLength_7b                                              \
  ((uint32_t)0x10001000) /*!< only available for STM32F072 and STM32F030 \
                            devices */
 ...
 * @note     These parameters are only available for STM32F051 and STM32F072
 * devices
 * @{
 */
...
#define USART_FLAG_RWU \
  USART_ISR_RWU /*!< Not available for  STM32F030 devices */
...
#define USART_FLAG_EOB \
  USART_ISR_EOBF /*!< Not available for  STM32F030 devices */
...
#define USART_FLAG_LBD \
  USART_ISR_LBD /*!< Not available for  STM32F030 devices */
...
#define USART_IT_WU \
  ((uint32_t)0x00140316) /*!< Not available for  STM32F030 devices */
#define USART_IT_CM ((uint32_t)0x0011010E)
#define USART_IT_EOB \
  ((uint32_t)0x000C011B) /*!< Not available for  STM32F030 devices */
...
#define USART_IT_LBD \
  ((uint32_t)0x00080206) /*!< Not available for  STM32F030 devices */
...
#define IS_USART_CLEAR_IT(IT)                                                 \
  (((IT) == USART_IT_TC) || ((IT) == USART_IT_PE) || ((IT) == USART_IT_FE) || \
   ((IT) == USART_IT_NE) || ((IT) == USART_IT_ORE) ||                         \
   ((IT) == USART_IT_IDLE) || ((IT) == USART_IT_LBD) ||                       \
   ((IT) == USART_IT_CTS) || ((IT) == USART_IT_RTO) ||                        \
   ((IT) == USART_IT_EOB) || ((IT) == USART_IT_CM) || ((IT) == USART_IT_WU))
/**
 * @}
 */
...
void USART_SetPrescaler(
    ...
    uint8_t USART_Prescaler); /* Not available for STM32F030 devices */
...
void USART_LINBreakDetectLengthConfig(
    USART_TypeDef* USARTx,
    uint32_t
        USART_LINBreakDetectLength); /* Not available for STM32F030 devices */
void USART_LINCmd(
    USART_TypeDef* USARTx,
    FunctionalState NewState); /* Not available for STM32F030 devices */
...
void USART_SetGuardTime(
    USART_TypeDef* USARTx,
    uint8_t USART_GuardTime); /* Not available for STM32F030 devices */
void USART_SetAutoRetryCount(
    USART_TypeDef* USARTx,
    uint8_t USART_AutoCount); /* Not available for STM32F030 devices */
void USART_SetBlockLength(
    USART_TypeDef* USARTx,
    uint8_t USART_BlockLength); /* Not available for STM32F030 devices */

/* IrDA mode functions ********************************************************/
void USART_IrDAConfig(
    USART_TypeDef* USARTx,
    uint32_t USART_IrDAMode); /* Not available for STM32F030 devices */
void USART_IrDACmd(
    USART_TypeDef* USARTx,
    FunctionalState NewState); /* Not available for STM32F030 devices */
