...
typedef struct {
  ...
  uint16_t
      SPI_BaudRatePrescaler; /*!< Specifies the Baud Rate prescaler value which
                                ...
                                the master
                                        clock. The slave clock does not need to
                                be set. */
...
#define IS_SPI_ALL_PERIPH(PERIPH) (((PERIPH) == SPI1) || ((PERIPH) == SPI2))
...
#define IS_SPI_DATA_SIZE(SIZE)                                     \
  (((SIZE) == SPI_DataSize_4b) || ((SIZE) == SPI_DataSize_5b) ||   \
...
#define IS_SPI_CRC_LENGTH(LENGTH) \
  (((LENGTH) == SPI_CRCLength_8b) || ((LENGTH) == SPI_CRCLength_16b))
/**
 * @}
 */
...
#define IS_SPI_I2S_DMA_REQ(REQ) \
  ((((REQ) & (uint16_t)0xFFFC) == 0x00) && ((REQ) != 0x00))
/**
 * @}
 */
...
void I2S_Init(SPI_TypeDef* SPIx,
              I2S_InitTypeDef*
                  I2S_InitStruct); /*!< Not applicable for STM32F030 devices */
void SPI_StructInit(SPI_InitTypeDef* SPI_InitStruct);
void I2S_StructInit(I2S_InitTypeDef* I2S_InitStruct); /*!< Not applicable for
                                                         STM32F030 devices */
