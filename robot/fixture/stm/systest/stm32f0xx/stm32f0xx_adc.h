...
typedef struct {
  ...
  FunctionalState
      ADC_ContinuousConvMode; /*!< Specifies whether the conversion is performed
                                 in Continuous or Single mode. This parameter
                                 can be set to ENABLE or DISABLE. */

  uint32_t ADC_ExternalTrigConvEdge; /*!< Selects the external trigger Edge and
                                        enables the trigger of a regular group.
                                        This parameter can be a value of @ref
  ...
  uint32_t
      ADC_ExternalTrigConv; /*!< Defines the external trigger used to start the
                               ...
                               This parameter can be a value of @ref
                               ADC_external_trigger_sources_for_channels_conversion
                             */
  ...
  uint32_t ADC_ScanDirection; /*!< Specifies in which direction the channels
                                 will be scanned in the sequence.
                                   This parameter can be a value of @ref
                                 ADC_Scan_Direction */
} ADC_InitTypeDef;
 ...
 * @{
 */
/* These defines are obsolete and maintained for legacy purpose only. They are
 * replaced  by the ADC_ClockMode */
#define ADC_JitterOff_PCLKDiv2 ADC_CFGR2_JITOFFDIV2
#define ADC_JitterOff_PCLKDiv4 ADC_CFGR2_JITOFFDIV4

#define IS_ADC_JITTEROFF(JITTEROFF) \
  (((JITTEROFF)&0x3FFFFFFF) == (uint32_t)RESET)
 ...
 * @{
 */
#define ADC_ClockMode_AsynClk \
  ((uint32_t)0x00000000) /*!< ADC Asynchronous clock mode */
#define ADC_ClockMode_SynClkDiv2 \
  ADC_CFGR2_CKMODE_0 /*!<  Synchronous clock mode divided by 2 */
#define ADC_ClockMode_SynClkDiv4 \
  ADC_CFGR2_CKMODE_1 /*!<  Synchronous clock mode divided by 4 */
#define IS_ADC_CLOCKMODE(CLOCK)             \
  (((CLOCK) == ADC_ClockMode_AsynClk) ||    \
...
#define ADC_ExternalTrigConv_T3_TRGO \
  ((uint32_t)(ADC_CFGR1_EXTSEL_0 | ADC_CFGR1_EXTSEL_1))
...
#define IS_ADC_DMA_MODE(MODE) \
  (((MODE) == ADC_DMAMode_OneShot) || ((MODE) == ADC_DMAMode_Circular))
/**
 * @}
 */
...
#define ADC_AnalogWatchdog_Channel_10 \
  ((uint32_t)0x28000000) /*!< Not available for STM32F031 devices */
#define ADC_AnalogWatchdog_Channel_11 \
  ((uint32_t)0x2C000000) /*!< Not available for STM32F031 devices */
#define ADC_AnalogWatchdog_Channel_12 \
  ((uint32_t)0x30000000) /*!< Not available for STM32F031 devices */
#define ADC_AnalogWatchdog_Channel_13 \
  ((uint32_t)0x34000000) /*!< Not available for STM32F031 devices */
#define ADC_AnalogWatchdog_Channel_14 \
  ((uint32_t)0x38000000) /*!< Not available for STM32F031 devices */
#define ADC_AnalogWatchdog_Channel_15 \
  ((uint32_t)0x3C000000) /*!< Not available for STM32F031 devices */
...
#define ADC_Channel_10 \
  ADC_CHSELR_CHSEL10 /*!< Not available for STM32F031 devices */
#define ADC_Channel_11 \
  ADC_CHSELR_CHSEL11 /*!< Not available for STM32F031 devices */
#define ADC_Channel_12 \
  ADC_CHSELR_CHSEL12 /*!< Not available for STM32F031 devices */
#define ADC_Channel_13 \
  ADC_CHSELR_CHSEL13 /*!< Not available for STM32F031 devices */
#define ADC_Channel_14 \
  ADC_CHSELR_CHSEL14 /*!< Not available for STM32F031 devices */
#define ADC_Channel_15 \
  ADC_CHSELR_CHSEL15 /*!< Not available for STM32F031 devices */
#define ADC_Channel_16 ADC_CHSELR_CHSEL16
#define ADC_Channel_17 ADC_CHSELR_CHSEL17
#define ADC_Channel_18 \
  ADC_CHSELR_CHSEL18 /*!< Not available for STM32F030 devices */

#define ADC_Channel_TempSensor ((uint32_t)ADC_Channel_16)
#define ADC_Channel_Vrefint ((uint32_t)ADC_Channel_17)
#define ADC_Channel_Vbat \
  ((uint32_t)ADC_Channel_18) /*!< Not available for STM32F030 devices */

#define IS_ADC_CHANNEL(CHANNEL)      \
  (((CHANNEL) != (uint32_t)RESET) && \
   (((CHANNEL)&0xFFF80000) == (uint32_t)RESET))
...
#define IS_ADC_CONFIG_IT(IT) \
  (((IT) != (uint32_t)RESET) && (((IT)&0xFFFFFF60) == (uint32_t)RESET))

#define IS_ADC_GET_IT(IT)                                                      \
  (((IT) == ADC_IT_ADRDY) || ((IT) == ADC_IT_EOSMP) || ((IT) == ADC_IT_EOC) || \
   ((IT) == ADC_IT_EOSEQ) || ((IT) == ADC_IT_OVR) || ((IT) == ADC_IT_AWD))

#define IS_ADC_CLEAR_IT(IT) \
  (((IT) != (uint32_t)RESET) && (((IT)&0xFFFFFF60) == (uint32_t)RESET))
...
#define IS_ADC_CLEAR_FLAG(FLAG) \
  (((FLAG) != (uint32_t)RESET) && (((FLAG)&0xFFFFFF60) == (uint32_t)RESET))
...
void ADC_JitterCmd(ADC_TypeDef* ADCx, uint32_t ADC_JitterOff,
...
void ADC_VbatCmd(
    FunctionalState NewState); /*!< Not applicable for STM32F030 devices */

/* Channels Configuration functions *******************************************/
void ADC_ChannelConfig(ADC_TypeDef* ADCx, uint32_t ADC_Channel,
