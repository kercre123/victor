...
// configure uart pin(s)
  ...
  if ((int)Board::revision >= BOARD_REV_2_0) {
    CHGPWR::init(MODE_OUTPUT, PULL_NONE,
                 TYPE_PUSHPULL);  // always drive, to override CHGTX_4V control
  }
...
void Contacts::setModePower(void) {
  if (mode == MODE_POWER) return;
  if (mode == MODE_UNINITIALIZED) Contacts::init();
...
void Contacts::setModeTx(void) {
  ...
  delay_bit_time_(2);  // wait at least 1 bit-time for uart peripheral to spin
                       // up
  ...
  if ((int)Board::revision >= BOARD_REV_2_0) {
    ...
    // Drive CHGRX low for the other half of push-pull (pulls VEXT down through
    // 330R)
    CHGRX::reset();
    CHGRX::mode(MODE_OUTPUT);
  }
  ...
  delay_bit_time_(
      20);  // 2 byte times on the wire for receiver to sync line change
  mode = MODE_TX;
}
...
char* line_process_char_(
    char c, int* out_len)  // return ptr to full line, when available
{
  switch (c) {
    case '\r':  // Return, EOL
    case '\n':
...
char* Contacts::getlinebuffer(int* out_len) {
  if (out_len) *out_len = line_len;  // report length
  return m_line;
}

int Contacts::flushRx(void) {
  if (mode != MODE_RX) return 0;
...
extern "C" void USART_IRQHandler(void) {
  ...
  {
    if (status & USART_ISR_ORE) Contacts::overflow_err_cnt++;
    if (status & USART_ISR_FE) Contacts::framing_err_cnt++;
  ...
  {
    if (status & USART_SR_ORE) Contacts::overflow_err_cnt++;
    if (status & USART_SR_FE) Contacts::framing_err_cnt++;
