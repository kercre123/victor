...
void Timer::init(void) {
  ...
  TIM_TimeBaseStructure.TIM_Prescaler =
      59;  // should be 29 for 30 MHz (assuming peripheral clock 1)... WTF
...
#elif USE_SYSTICK > 0
  SysTick->LOAD = SysTickReloadVal;  // set reload such that 1 counter cycle is
                                     // exactly 65,536us (16-bit us counter)
  SysTick->VAL = SysTickReloadVal;
  SysTick->CTRL =
      SysTick_CTRL_CLKSOURCE_Msk |  // 1 = processor clock
                                    // SysTick_CTRL_TICKINT_Msk |    //1 =
                                    // counting down to zero asserts the SysTick
                                    // exception request
...
#elif USE_SYSTICK
  // reload value (e.g. max count) is calculated such that resulting range is
  // 0..65k (16-bit)
  uint32_t count =
      SysTickReloadVal - SysTick->VAL;  // make count-down into count-up
