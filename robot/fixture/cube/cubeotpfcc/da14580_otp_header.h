...
#define OTP_HEADER_PACKAGE_QFN56_SC14439 0x00000033  // QFN56 + SC14439
                                                     // (DA14582)
...
// OTP Header format
// Note: fields beginning with "FAC_" are protected; burned by manufacturer.
// "Invalid value may destroy the chip"
typedef struct {
  uint32_t app_flag_1;  // ofs 0x00      0: Empty OTP, 0x1234A5A5: Application
                        // is in OTP
  uint32_t app_flag_2;  // ofs 0x04      0: Empty OTP, 0xA5A51234: Application
                        // is in OTP
  uint32_t
      FAC_rf_trim;  // ofs 0x08      Bits[31:16]=RF_TRIM1 ,Bits[15:0]=RF_TRIM2
  uint32_t
      FAC_crc;  // ofs 0x0C      CRC16 of the calibration data (fields:
                // 0x08,78,7C,84,88,90) -> {FAC_rf_trim, cal_flags, FAC_trim_LNA,
                // FAC_trim_bandgap, FAC_trim_RC16, FAC_trim_VCO}
  ...
  uint32_t
      FAC_tester;  // ofs 0x68      Bits[7:2]=Tester_ID, Bits[1:0]=Tester_Site
  uint32_t position;  // ofs 0x6C      Byte3: LOT, Byte2: Wafer, Byte1: Y coord,
                      // Byte0: X coord
  uint32_t package;  // ofs 0x70      0x00=WLCSP34, 0xAA=QFN40, 0x55=QFN48,
                     // 0x99=KGD, 0x11=QFN40 0.5mm pitch, 0x22=QFN40 + flash
                     // (DA14583-00), 0x33=QFN56 + SC14439 (DA14582)
  uint32_t src_32KHz;  // ofs 0x74      32kHz source selection - 0x00:
                       // XTAL32KHz, 0xAA: RC32KHz
  uint32_t
      cal_flags;  // ofs 0x78      Bit[31:16]: 0xA5A5, Bit5: Trim_VCO_Cal, Bit4:
                  // Trim_XTAL16_Cal, Bit3: Trim_RC16_Cal, Bit2:
                  // Trim_Bandgap_Cal, Bit1: Trim_RFIO_Cal, Bit0: Trim_LNA_Cal
  ...
  uint32_t code_signature[15];  // ofs 0x94-0xCC Customer Code Signature - Hash
                                // Signature of Code
  uint32_t sig_algorithm;  // ofs 0xD0      Signature Algorithm - 0x00: None,
                           // 0xAA: MD5, 0x55: SHA-1, 0xFF: CRC32
  uint32_t dev_unique_id[2];  // ofs 0xD4-0xD8 Device unique ID - Device number
                              // (written as a string of bytes, i.e. left-most
                              // byte will be burned at 0x7FD4)
  uint32_t custom_field[6];  // ofs 0xDC-0xF0 Custom Fields
  uint32_t remap_flag;  // ofs 0xF4      Remapping Flag - 0x00: SRAM remapped to
                        // 0, 0xAA: OTP remapped to 0
  uint32_t dma_len;  // ofs 0xF8      DMA Length - Number of 32-bit words. <
                     // 0x2000 words. (>= APPLICATION LEN FOR OTP MIRRORING)
  uint32_t jtag_enable;  // ofs 0xFC      JTAG enable flag - 0x0: Enable, 0xAA:
                         // Disable
