 ...
 *
 * It handles messages from lower and higher layers not related to an ongoing
 *connection.
 *
 * @{
 ****************************************************************************************
...
enum gapm_msg_id {
  ...
  GAPM_PEER_NAME_IND,
  /// Confirm connection to a specific device (Connection Operation in Selective
  /// mode)
  GAPM_CONNECTION_CFM,
...
struct gapm_white_list_mgt_cmd {
  ...
  uint8_t nb;
  /// Device address information that can be used to add or remove element in
  /// device list.
...
struct gapm_adv_host {
  ...
  /// - ADV_ALLOW_SCAN_ANY_CON_ANY: Allow both scan and connection requests from
  /// anyone
  /// - ADV_ALLOW_SCAN_WLST_CON_ANY: Allow both scan req from White List devices
  /// only and
...
struct gapm_start_scan_cmd {
  ...
  /// - SCAN_ALLOW_ADV_WLST: Allow advertising packets from White List devices
  /// only
  uint8_t filt_policy;
...
struct gapm_start_connection_cmd {
  ...
  /// - GAPM_CONNECTION_NAME_REQUEST: Name Request operation (requires to start
  /// a direct
  ///   connection)
  struct gapm_air_operation op;
  ...
  ///  Shall be 1 for GAPM_CONNECTION_DIRECT or GAPM_CONNECTION_NAME_REQUEST
  ///  operations Shall be greater than 0 for other operations
  uint8_t nb_peers;
