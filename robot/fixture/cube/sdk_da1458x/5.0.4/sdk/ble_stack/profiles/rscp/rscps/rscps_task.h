...
/// Maximum number of Running Speed and Cadence Profile Sensor role task
/// instances
...
enum rscps_msg_id {
  ...
  /// Inform the application that the profile has been disabled after a
  /// disconnection
  RSCPS_DISABLE_IND,
...
struct rscps_create_db_req {
  /**
   * RSC Feature Value - Not supposed to be modified during the lifetime of the
   * device This value is used to decide if the SC Control Point Characteristic
   * is part of the Running Speed and Cadence service.
   */
  uint16_t rsc_feature;
  /**
   * Indicate if the Sensor Location characteristic is supported. Note that if
   * the Multiple Sensor Location feature is set has supported in the
   * rsc_feature parameter, the characteristic will be added (mandatory).
   */
