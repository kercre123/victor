 ...
 *
 * The SPOTARTASK is responsible for handling the API messages from the
...
struct spotar_enable_req {
  ...
  /// Security level
  uint8_t sec_lvl;
  /// MSbyte indicates Memory device to apply patch. The rest of the word is the
  /// base address of the patch area
  uint32_t mem_dev;
  /// Most sicnificant word (MSW) is the existing patch size, LSW is the entire
  /// memory
