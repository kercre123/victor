 ...
 *
 * The BLPSTASK is responsible for handling the messages coming in and out of
 *the
...
enum {
  ...
  BLPS_MEAS_SEND_REQ,
  /// Send blood pressure measurement value confirm to APP so stable values can
  /// be erased if correctly sent.
  BLPS_MEAS_SEND_CFM,
...
struct blps_enable_req {
  ...
  /// b3=authorized; b1 or b2 and b3 can go together it matches the permission
  /// bytes order of settings
  uint8_t sec_lvl;
...
struct blps_cfg_indntf_ind {
  ...
  uint16_t cfg_val;
  /// Own code for differentiating between Blood Pressure Measurement, and
  /// Intermediate
