 ...
 * is the term used by this profile to describe a device that can perform
 *Glucose measurement and notify about on-going measurement and indicate final
...
#define GLPS_IS(FLAG) \
  ((glps_env.flags & (1 << (GLPS_##FLAG))) == (1 << (GLPS_##FLAG)))
...
#define GLPS_HANDLE(idx)                                                   \
  (glps_env.shdl + (idx) -                                                 \
   ((!(GLPS_IS(MEAS_CTX_SUPPORTED)) && ((idx) > GLS_IDX_MEAS_CTX_NTF_CFG)) \
        ? (3)                                                              \
        : (0)))

/// Get database attribute index
#define GLPS_IDX(hdl)                                     \
  (((((hdl)-glps_env.shdl) > GLS_IDX_MEAS_CTX_NTF_CFG) && \
    !(GLPS_IS(MEAS_CTX_SUPPORTED)))                       \
       ? ((hdl)-glps_env.shdl + 3)                        \
       : ((hdl)-glps_env.shdl))
...
enum {
  ...
  /// Flag used to know if there is an on-going record access control point
  /// request
  GLPS_RACP_ON_GOING,
...
uint8_t glps_pack_meas_value(uint8_t* packed_meas,
                             const struct glp_meas* meas_val, uint16_t seq_num);
 ...
 *
 * @param[in] racp_rsp Record Access Control Response value
 * @param[in] racp_ind_src Application that requests to send RACP response
 *indication.
 *
