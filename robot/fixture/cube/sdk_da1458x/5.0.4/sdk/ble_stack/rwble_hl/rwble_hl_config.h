...
#define BLE_GATTM 1
#define BLE_GATTC 1
#define BLE_GATTC_HEAP_ENV_SIZE \
  (sizeof(struct gattc_env_tag) + KE_HEAP_MEM_RESERVED)
#define BLE_L2CC_HEAP_ENV_SIZE \
...
#if (BLE_CENTRAL || BLE_PERIPHERAL)
#define BLE_SMPC 1
#define BLE_SMPC_HEAP_ENV_SIZE \
  (sizeof(struct smpc_env_tag) + KE_HEAP_MEM_RESERVED)
#else  //(BLE_CENTRAL || BLE_PERIPHERAL)
#define BLE_SMPC 0
...
#define BLE_ATTS 1
#define BLE_ATTS_HEAP_ENV_SIZE \
  (sizeof(struct atts_env_tag) + KE_HEAP_MEM_RESERVED)
#else
#define BLE_ATTS 0
...
#define BLE_HOST_TASK_SIZE                                           \
  (BLE_GAPM + BLE_GAPC + BLE_L2CM + BLE_L2CC + BLE_SMPM + BLE_SMPC + \
   BLE_ATTM + BLE_ATTC + BLE_ATTS + BLE_GATTM + BLE_GATTC)

/// Number of BLE profiles tasks
#define BLE_PRF_TASK_SIZE                                                   \
  (BLE_PROX_MONITOR + BLE_PROX_REPORTER + BLE_BM_SERVER + BLE_BM_CLIENT +   \
   BLE_STREAMDATA_DEVICE + BLE_STREAMDATA_HOST + BLE_FINDME_LOCATOR +       \
   BLE_FINDME_TARGET + BLE_HT_COLLECTOR + BLE_HT_THERMOM + BLE_DIS_CLIENT + \
   BLE_DIS_SERVER + BLE_BP_COLLECTOR + BLE_BP_SENSOR + BLE_TIP_CLIENT +     \
   BLE_TIP_SERVER + BLE_HR_COLLECTOR + BLE_HR_SENSOR + BLE_SP_CLIENT +      \
   BLE_SP_SERVER + BLE_BAS_CLIENT + BLE_BAS_SERVER + BLE_HID_DEVICE +       \
   BLE_HID_BOOT_HOST + BLE_HID_REPORT_HOST + BLE_GL_COLLECTOR +             \
   BLE_GL_SENSOR + BLE_NEB_SERVER + BLE_NEB_CLIENT + BLE_RSC_COLLECTOR +    \
   BLE_RSC_SENSOR + BLE_CSC_COLLECTOR + BLE_CSC_SENSOR + BLE_CP_COLLECTOR + \
   BLE_CP_SENSOR + BLE_LN_COLLECTOR + BLE_LN_SENSOR + BLE_AN_CLIENT +       \
   BLE_AN_SERVER + BLE_ANC_CLIENT + BLE_PAS_CLIENT + BLE_PAS_SERVER +       \
   BLE_ACCEL + BLE_WPT_CLIENT + BLE_UDS_SERVER + BLE_UDS_CLIENT +           \
   BLE_BCS_SERVER + BLE_BCS_CLIENT + BLE_WSS_SERVER + BLE_WSS_COLLECTOR +   \
   BLE_CTS_SERVER + BLE_CTS_CLIENT)
...
#define BLEHL_HEAP_ENV_SIZE                                                   \
  (BLE_GAPC_HEAP_ENV_SIZE + BLE_SMPC_HEAP_ENV_SIZE + BLE_ATTS_HEAP_ENV_SIZE + \
   BLE_GATTC_HEAP_ENV_SIZE + BLE_L2CC_HEAP_ENV_SIZE)
#endif
