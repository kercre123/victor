 ...
 * @addtogroup HCI HCI
 * @ingroup ROOT
 * @brief HCI module handling communication between lower and higher layers in
 *split architecture.
 *
 *@{
 * This is the following convention for HCI transport functions:
 *     - Functions referring to Commands/events to be sent through HCI transport
 *layer contain 'send' in their name.
 *
 *     - Functions referring to Commands/events to be from HCI transport layer
 *reception to FW targets contain 'dispatch' in their name.
 *
 *     - Suffixes 'pk' and 'unpk' are used for command/event packing and
 *unpacking functions, which generally bear a similar name to the command name
 *in the SIG specification.
 *
 *     - All commands and events are described by structures, defined in
 *llm/c_task.h. The structures may sometimes present padded space for alignment,
 *therefore for those messages described with such structures, specific
 *packing/unpacking functions can be found. All the rest are handled by 'basic'
 *or 'aligned' named functions.
 *
 ****************************************************************************************
 */
...
/// Event types for HCI message handler to determine pointer to start of packed
/// event in the kernel message space.
enum {
  /// Command Complete Event type will have packed event starting at
  /// ke_msg->dest_id +1
  TYPE_CCEVT = -5,
  /// Command Status Event type will have packed event starting at
  /// ke_msg->dest_id +1
  TYPE_CSEVT = -5,
...
struct hci_evt_pk_util {
  /// Event type: Command Complete or other, for pointer position in
  /// msg_send_handler
  int8_t evt_type;
...
struct hci_acl_hdr {
  /// Connection handle
  uint16_t hdl;
  /// Broadcasts Flag (always 0) and Packet Boundary Flag - first packet or
  /// fragment ind
  uint8_t bcpb_flag;
...
struct hci_env_tag {
  ...
  /// Allowed number of commands from Host to controller (sent from controller
  /// to host)
 ...
 * HCI_Write_Stored_Link_Key, HCI_Write_Location_Data), and to avoid processing
 *variable sizes as specific cases, received size is considered ok if less or
 *equal maximum size defined by standard.
 *
 ...
 *
 * This function is the algorithm to check that received byte stream in out of
 *sync state corresponds to HCI_reset command.
 *
...
/**
 ****************************************************************************************
 * @brief Function called in HCIH EIF reception routine to allocate a KE message
 *for a command.
 *
...
/**
 ****************************************************************************************
 * @brief Function called in HCIH EIF reception routine to dispatch a KE message
 *for a command with no parameters.
 *
 * Uses the hci_env command header OGF and OCF components.
 *****************************************************************************************
 */
 ...
 ****************************************************************************************
 * @brief Function called in HCIH EIF reception routine to dispatch a KE message
 *for data reception, once payload reception has been completed (for max
 *BLE_DATA_LEN).
 *
 * ***************************************************************************************
 */
