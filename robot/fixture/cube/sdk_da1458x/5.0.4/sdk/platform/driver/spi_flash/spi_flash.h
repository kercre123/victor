...
typedef struct {
  ...
  uint32_t
      jedec_id_matching_bitmask;  // bitmask of the JEDEC ID to derive matching
  uint32_t flash_size;            // the total size in bytes
  uint32_t page_size;             // the page size in bytes
  uint8_t memory_protection_bitmask;  // the memory protection-related bits of
                                      // the status register
 ...
 ****************************************************************************************
 */
void spi_flash_init(uint32_t spi_flash_size_param,
                    uint32_t spi_flash_page_size_param);

/**
 ****************************************************************************************
 * @brief Detect the SPI flash device, based on the JEDEC manufacturer id and
 *the manufacturer-defined data(2 bytes) which is retrieved when the command 9Fh
 *        is issued. If the device is successfully identified, the total memory
 *size and page size are retrieved from a lookup table.
 * @return the index of the device in the
 *SPI_FLASH_KNOWN_DEVICES_PARAMETERS_LIST or
 ...
 ****************************************************************************************
 */
int32_t spi_flash_read_data(uint8_t *rd_data_ptr, uint32_t address,
                            uint32_t size);

/**
 ****************************************************************************************
 * @brief Program page (up to <SPI Flash page size> bytes) starting at given
 *address.
 * @param[in] *wr_data_ptr  Pointer to the data to be written
 * @param[in] address       Starting address of data to be written
 * @param[in] size          Size of the data to be written (should not be larger
 *than SPI Flash page size)
 * @return error code or success (ERR_OK)
 ****************************************************************************************
 */
int32_t spi_flash_page_program(uint8_t *wr_data_ptr, uint32_t address,
                               uint16_t size);

/**
****************************************************************************************
* @brief Issue a comamnd to Erase a given address.
* @param[in] address        Address that belongs to the block64/block32/sector
*range
* @param[in] spiEraseModule BLOCK_ERASE_64, BLOCK_ERASE_32, SECTOR_ERASE
* @return error code or success (ERR_OK)
****************************************************************************************
*/
int8_t spi_flash_block_erase(uint32_t address,
                             SPI_erase_module_t spiEraseModule);

/**
 ****************************************************************************************
 * @brief Erase chip.
 * @note In order for the erasure to succeed, all locking options must be
 *disabled.
 * @return error code or success (ERR_OK)
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief Write data to flash across page boundaries and at any starting
 *address.
 * @param[in] *wr_data_ptr Pointer to the data to be written
 * @param[in] address      Starting address of page to be written (must be a
 *multiple of SPI Flash page size)
 * @param[in] size         Size of the data to be written (can be larger than
 *SPI Flash page size)
 * @return Number of bytes actually written
 ****************************************************************************************
 */
int32_t spi_flash_write_data(uint8_t *wr_data_ptr, uint32_t address,
                             uint32_t size);

/**
****************************************************************************************
* @brief Sends the Power-Down instruction.
* Remark: The function spi_flash_release_from_powerdown() is used to enable the
*IC again
* @return error code or success (ERR_OK)
****************************************************************************************
*/
 ...
 ****************************************************************************************
 */
int32_t spi_flash_configure_memory_protection(
    uint8_t spi_flash_memory_protection_setting);
 ...
 *1-byte value starting at given address.
 * @param[in] value         Value used to fill memory
 * @param[in] address       Starting address
 * @param[in] size          Size of the area to be filled (should not be larger
 *than SPI Flash page size)
 * @return error code or success (ERR_OK)
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief Fill memory with a 1-byte value, across page boundaries and at any
 *starting address.
 * @param[in] value    The value with which memory will be filled
 * @param[in] address  Starting address of page to be written (must be a
 *multiple of SPI Flash page size)
 * @param[in] size     Size of the area to be filled (can be larger than SPI
 *Flash page size)
 * @return  Number of bytes actually written
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief Initializes spi and spi_flash drivers, discovers jedec id and releases
 *from power down.
