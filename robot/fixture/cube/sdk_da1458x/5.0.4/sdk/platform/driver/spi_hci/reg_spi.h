 ...
 *
 * @return SPI_INT_BIT value: 1 if there are received data available, 0
 *otherwise.
 ****************************************************************************************
 */
 ...
 ****************************************************************************************
 */
__INLINE void spi_int_bit_clear(void) { SetWord16(SPI_CLEAR_INT_REG, 0x01); }
...
__INLINE void spi_write_byte(uint8_t wr_byte) {
  while (GetBits16(SPI_CTRL_REG, SPI_TXH) == 1)
    ;                                         // Wait if SPI Tx FIFO is full
