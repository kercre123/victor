...
//#define REG_BLE_EM_RX_ADDR_GET(idx) (jump_table_struct[offset_em_rx] + (idx) *
//REG_BLE_EM_RX_SIZE)
#define REG_BLE_EM_RX_ADDR_GET(idx) \
  (jump_table_struct[offset_em_rx] + (idx)*REG_BLE_EM_RX_SIZE)
...
__INLINE void ble_rxcntl_pack(int elt_idx, uint8_t rxdone, uint16_t rxnextptr) {
  ...
  EM_BLE_WR(BLE_RXCNTL_ADDR + elt_idx * REG_BLE_EM_RX_SIZE,
            ((uint16_t)rxdone << 15) | ((uint16_t)rxnextptr << 0));
}

__INLINE void ble_rxcntl_unpack(int elt_idx, uint8_t* rxdone,
                                uint16_t* rxnextptr) {
...
__INLINE void ble_rxdone_setf(int elt_idx, uint8_t rxdone) {
  ASSERT_ERR((((uint16_t)rxdone << 15) & ~((uint16_t)0x00008000)) == 0);
  EM_BLE_WR(BLE_RXCNTL_ADDR + elt_idx * REG_BLE_EM_RX_SIZE,
            (EM_BLE_RD(BLE_RXCNTL_ADDR + elt_idx * REG_BLE_EM_RX_SIZE) &
             ~((uint16_t)0x00008000)) |
                ((uint16_t)rxdone << 15));
}
...
__INLINE void ble_rxstat_pack(int elt_idx, uint8_t bdaddrmatch, uint8_t nesnerr,
                              uint8_t snerr, uint8_t micerr, uint8_t crcerr,
                              uint8_t lenerr, uint8_t typeerr,
                              uint8_t syncerr) {
  ...
  EM_BLE_WR(BLE_RXSTAT_ADDR + elt_idx * REG_BLE_EM_RX_SIZE,
            ((uint16_t)bdaddrmatch << 7) | ((uint16_t)nesnerr << 6) |
                ((uint16_t)snerr << 5) | ((uint16_t)micerr << 4) |
                ((uint16_t)crcerr << 3) | ((uint16_t)lenerr << 2) |
                ((uint16_t)typeerr << 1) | ((uint16_t)syncerr << 0));
}

__INLINE void ble_rxstat_unpack(int elt_idx, uint8_t* bdaddrmatch,
                                uint8_t* nesnerr, uint8_t* snerr,
                                uint8_t* micerr, uint8_t* crcerr,
                                uint8_t* lenerr, uint8_t* typeerr,
                                uint8_t* syncerr) {
...
__INLINE void ble_bdaddr_match_setf(int elt_idx, uint8_t bdaddrmatch) {
  ASSERT_ERR((((uint16_t)bdaddrmatch << 7) & ~((uint16_t)0x00000080)) == 0);
  EM_BLE_WR(BLE_RXSTAT_ADDR + elt_idx * REG_BLE_EM_RX_SIZE,
            (EM_BLE_RD(BLE_RXSTAT_ADDR + elt_idx * REG_BLE_EM_RX_SIZE) &
             ~((uint16_t)0x00000080)) |
...
__INLINE void ble_rxphce_pack(int elt_idx, uint8_t rxlen, uint8_t rxmd,
                              uint8_t rxsn, uint8_t rxnesn, uint8_t rxllid) {
  ...
  EM_BLE_WR(BLE_RXPHCE_ADDR + elt_idx * REG_BLE_EM_RX_SIZE,
            ((uint16_t)rxlen << 8) | ((uint16_t)rxmd << 4) |
                ((uint16_t)rxsn << 3) | ((uint16_t)rxnesn << 2) |
                ((uint16_t)rxllid << 0));
}

__INLINE void ble_rxphce_unpack(int elt_idx, uint8_t* rxlen, uint8_t* rxmd,
                                uint8_t* rxsn, uint8_t* rxnesn,
                                uint8_t* rxllid) {
...
__INLINE void ble_rxlen_setf(int elt_idx, uint8_t rxlen) {
  ASSERT_ERR((((uint16_t)rxlen << 8) & ~((uint16_t)0x00001F00)) == 0);
  EM_BLE_WR(BLE_RXPHCE_ADDR + elt_idx * REG_BLE_EM_RX_SIZE,
            (EM_BLE_RD(BLE_RXPHCE_ADDR + elt_idx * REG_BLE_EM_RX_SIZE) &
             ~((uint16_t)0x00001F00)) |
                ((uint16_t)rxlen << 8));
}
...
__INLINE void ble_rxphadv_pack(int elt_idx, uint8_t rxadvlen, uint8_t rxrxadd,
                               uint8_t rxtxadd, uint8_t rxtype) {
  ...
  EM_BLE_WR(BLE_RXPHADV_ADDR + elt_idx * REG_BLE_EM_RX_SIZE,
            ((uint16_t)rxadvlen << 8) | ((uint16_t)rxrxadd << 7) |
                ((uint16_t)rxtxadd << 6) | ((uint16_t)rxtype << 0));
}

__INLINE void ble_rxphadv_unpack(int elt_idx, uint8_t* rxadvlen,
                                 uint8_t* rxrxadd, uint8_t* rxtxadd,
                                 uint8_t* rxtype) {
  uint16_t localVal =
      EM_BLE_RD(BLE_RXPHADV_ADDR + elt_idx * REG_BLE_EM_RX_SIZE);
...
__INLINE void ble_rxchass_pack(int elt_idx, uint8_t usedchidx, uint8_t rssi) {
  ...
  EM_BLE_WR(BLE_RXCHASS_ADDR + elt_idx * REG_BLE_EM_RX_SIZE,
            ((uint16_t)usedchidx << 8) | ((uint16_t)rssi << 0));
}

__INLINE void ble_rxchass_unpack(int elt_idx, uint8_t* usedchidx,
                                 uint8_t* rssi) {
  uint16_t localVal =
      EM_BLE_RD(BLE_RXCHASS_ADDR + elt_idx * REG_BLE_EM_RX_SIZE);
...
__INLINE uint16_t ble_rxbuf_getf(int elt_idx, int reg_idx) {
  ASSERT_ERR(reg_idx <= 18);
  uint16_t localVal =
      EM_BLE_RD(BLE_RXBUF_ADDR + elt_idx * REG_BLE_EM_RX_SIZE + reg_idx * 2);
...
__INLINE void ble_rxbuf_setf(int elt_idx, int reg_idx, uint16_t rxbuf) {
  ...
  EM_BLE_WR(BLE_RXBUF_ADDR + elt_idx * REG_BLE_EM_RX_SIZE + reg_idx * 2,
            (uint16_t)rxbuf << 0);
}
