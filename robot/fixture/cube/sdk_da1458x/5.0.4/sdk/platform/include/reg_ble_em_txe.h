...
//#define REG_BLE_EM_TXE_ADDR_GET(idx) (EM_BLE_TXE_OFFSET + (idx) *
//REG_BLE_EM_TXE_SIZE)
#define REG_BLE_EM_TXE_ADDR_GET(idx) \
  (jump_table_struct[offset_em_txe] + (idx)*REG_BLE_EM_TXE_SIZE)
...
#define BLE_TXECNTL_ADDR (0x80000 + jump_table_struct[offset_em_txe])
...
__INLINE void ble_txecntl_pack(int elt_idx, uint8_t txedone, uint8_t txerdy,
                               uint16_t txenextptr) {
  ...
  EM_BLE_WR(BLE_TXECNTL_ADDR + elt_idx * REG_BLE_EM_TXE_SIZE,
            ((uint16_t)txedone << 15) | ((uint16_t)txerdy << 14) |
                ((uint16_t)txenextptr << 0));
}

__INLINE void ble_txecntl_unpack(int elt_idx, uint8_t* txedone, uint8_t* txerdy,
                                 uint16_t* txenextptr) {
  uint16_t localVal =
      EM_BLE_RD(BLE_TXECNTL_ADDR + elt_idx * REG_BLE_EM_TXE_SIZE);
...
__INLINE void ble_txephce_pack(int elt_idx, uint8_t txelen, uint8_t txemd,
                               uint8_t txesn, uint8_t txenesn,
                               uint8_t txellid) {
  ...
  EM_BLE_WR(BLE_TXEPHCE_ADDR + elt_idx * REG_BLE_EM_TXE_SIZE,
            ((uint16_t)txelen << 8) | ((uint16_t)txemd << 4) |
                ((uint16_t)txesn << 3) | ((uint16_t)txenesn << 2) |
                ((uint16_t)txellid << 0));
}

__INLINE void ble_txephce_unpack(int elt_idx, uint8_t* txelen, uint8_t* txemd,
                                 uint8_t* txesn, uint8_t* txenesn,
                                 uint8_t* txellid) {
  uint16_t localVal =
      EM_BLE_RD(BLE_TXEPHCE_ADDR + elt_idx * REG_BLE_EM_TXE_SIZE);
