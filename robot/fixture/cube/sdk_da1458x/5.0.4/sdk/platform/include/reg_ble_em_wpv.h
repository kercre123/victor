...
//#define REG_BLE_EM_WPV_ADDR_GET(idx) (EM_BLE_WPV_OFFSET + (idx) *
//REG_BLE_EM_WPV_SIZE)
#define REG_BLE_EM_WPV_ADDR_GET(idx) \
  (jump_table_struct[offset_em_wpv] + (idx)*REG_BLE_EM_WPV_SIZE)
...
#define BLE_WLPRV_ADDR (0x80000 + jump_table_struct[offset_em_wpv])
...
__INLINE uint16_t ble_wlprv_get(int elt_idx, int reg_idx) {
  ...
  return EM_BLE_RD(BLE_WLPRV_ADDR + elt_idx * REG_BLE_EM_WPV_SIZE +
                   reg_idx * 2);
}

__INLINE void ble_wlprv_set(int elt_idx, int reg_idx, uint16_t value) {
  ASSERT_ERR(reg_idx <= 2);
  EM_BLE_WR(BLE_WLPRV_ADDR + elt_idx * REG_BLE_EM_WPV_SIZE + reg_idx * 2,
            value);
}
...
__INLINE uint16_t ble_wlprv_getf(int elt_idx, int reg_idx) {
  ASSERT_ERR(reg_idx <= 2);
  uint16_t localVal =
      EM_BLE_RD(BLE_WLPRV_ADDR + elt_idx * REG_BLE_EM_WPV_SIZE + reg_idx * 2);
