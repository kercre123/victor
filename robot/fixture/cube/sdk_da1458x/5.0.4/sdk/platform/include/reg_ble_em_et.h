...
//#define REG_BLE_EM_ET_ADDR_GET(idx) (jump_table_struct[offset_em_et] + (idx) *
//REG_BLE_EM_ET_SIZE)
#define REG_BLE_EM_ET_ADDR_GET(idx) \
  (jump_table_struct[offset_em_et] + (idx)*REG_BLE_EM_ET_SIZE)
...
__INLINE void ble_extab_pack(int elt_idx, uint8_t etstatus, uint8_t excptrnrdy,
                             uint16_t csptr) {
  ...
  EM_BLE_WR(BLE_EXTAB_ADDR + elt_idx * REG_BLE_EM_ET_SIZE,
            ((uint16_t)etstatus << 14) | ((uint16_t)excptrnrdy << 13) |
                ((uint16_t)csptr << 0));
}

__INLINE void ble_extab_unpack(int elt_idx, uint8_t* etstatus,
                               uint8_t* excptrnrdy, uint16_t* csptr) {
...
__INLINE void ble_etstatus_setf(int elt_idx, uint8_t etstatus) {
  ASSERT_ERR((((uint16_t)etstatus << 14) & ~((uint16_t)0x0000C000)) == 0);
  EM_BLE_WR(BLE_EXTAB_ADDR + elt_idx * REG_BLE_EM_ET_SIZE,
            (EM_BLE_RD(BLE_EXTAB_ADDR + elt_idx * REG_BLE_EM_ET_SIZE) &
             ~((uint16_t)0x0000C000)) |
                ((uint16_t)etstatus << 14));
}
