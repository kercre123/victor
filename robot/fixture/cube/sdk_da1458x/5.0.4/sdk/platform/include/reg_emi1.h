...
__INLINE void emi1_extmemcntl1_pack(int reg_idx, uint8_t iftype1,
                                    uint8_t setupwr1, uint8_t holdwr1,
                                    uint8_t wswr1, uint8_t setuprd1,
                                    uint8_t holdrd1, uint8_t wsrd1) {
  ASSERT_ERR(reg_idx <= 1);
  ...
  REG_PL_WR(EMI1_EXTMEMCNTL1_ADDR + reg_idx * 4,
            ((uint32_t)iftype1 << 10) | ((uint32_t)setupwr1 << 9) |
                ((uint32_t)holdwr1 << 8) | ((uint32_t)wswr1 << 5) |
                ((uint32_t)setuprd1 << 4) | ((uint32_t)holdrd1 << 3) |
                ((uint32_t)wsrd1 << 0));
}

__INLINE void emi1_extmemcntl1_unpack(int reg_idx, uint8_t* iftype1,
                                      uint8_t* setupwr1, uint8_t* holdwr1,
                                      uint8_t* wswr1, uint8_t* setuprd1,
                                      uint8_t* holdrd1, uint8_t* wsrd1) {
  ASSERT_ERR(reg_idx <= 1);
...
__INLINE void emi1_if_type1_setf(int reg_idx, uint8_t iftype1) {
  ...
  ASSERT_ERR((((uint32_t)iftype1 << 10) & ~((uint32_t)0x00000C00)) == 0);
  REG_PL_WR(EMI1_EXTMEMCNTL1_ADDR + reg_idx * 4,
            (REG_PL_RD(EMI1_EXTMEMCNTL1_ADDR + reg_idx * 4) &
             ~((uint32_t)0x00000C00)) |
                ((uint32_t)iftype1 << 10));
}
