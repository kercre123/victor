...
/**
 ****************************************************************************************
 * @brief       Disable all sleep modes. The system operates in active / idle
 *modes only.
 * @param       void
 * @return      void
 ****************************************************************************************
...
/**
 ****************************************************************************************
 * @brief       Activates extended sleep mode. The system operates in idle /
 *active / extended sleep modes.
 * @param       void
 * @return      void
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief       Activates deep sleep mode. The system operates in idle / active
 * / deep sleep modes.
 * @param       void
 * @return      void
 ****************************************************************************************
...
/**
 ****************************************************************************************
 * @brief       Restore the sleep mode to what it was before disabling. App
 *should not modify the sleep mode directly.
 * @param       void
...
/**
 ****************************************************************************************
 * @brief       Disable sleep but save the sleep mode status. Store the sleep
...
/**
 ****************************************************************************************
 * @brief       Puts the BLE core to permanent sleep. Only an external event can
 *wake it up.\ BLE sleeps forever waiting a forced wakeup. After waking up from
 *an external event, if the system has to wake BLE up it needs to restore the
 *default mode of operation by calling app_ble_ext_wakeup_off() or the BLE won't
 *be able to wake up in order to serve BLE events!
 * @param       void
 * @return      void
 ...
 *              if rwip_env.ext_wakeup_enable is 0 since it wouldn't be possible
 *to wake-up the BLE core in this case.
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief       Takes the BLE core out of the permanent sleep mode.Restore BLE
 *cores' operation to default mode. The BLE core will wake up every 10sec even
 *if no BLE events are scheduled. If an event is to be scheduled earlier, then
 *BLE will wake up sooner to serve it.
 * @param       void
 * @details
 * @return      void
 ****************************************************************************************
 ...
 *              Returns the current mode of operation of the BLE core (external
 *wakeup or default).
 * @param       void
...
/**
 ****************************************************************************************
 * @brief       Wake the BLE core via an external request. If the BLE core is
 *sleeping (permanently or not and external wake-up is enabled) then this
 *function wakes it up. A call to app_ble_ext_wakeup_off() should follow if the
 *BLE core was in permanent sleep.
 * @param       void
 * @return      status
...
/**
 ****************************************************************************************
 * @brief       Modifies the system startup sleep delay. It can be called in
 *app_init() to modify the default delay (2 sec);
...
/**
 ****************************************************************************************
 * @brief Creates sw cursor in power profiler tool. Used for Development/
 *Debugging
 *
 * @return void
 ****************************************************************************************
...
/**
 ****************************************************************************************
 * @brief Calculates the minimum time between the beginning of two consecutive
ADV_IND PDUs within an advertising event. Used to optimise the power profile of
advertisisng event.
 *
...
typedef enum { GOTO_SLEEP = 0, KEEP_POWERED } arch_main_loop_callback_ret_t;

/*
 * Main loop callback functions' structure. Developer must set application's
 *callback functions in the initialization of the structure in
...
/**
 ****************************************************************************************
 * @brief Returns a pointer to ble metrics structure keeping device's
 statistics.
 *
 * @param[in]  void
 *
 ...
 ****************************************************************************************
 */
__INLINE arch_ble_metrics_t *arch_ble_metrics_get(void) { return &metrics; }
