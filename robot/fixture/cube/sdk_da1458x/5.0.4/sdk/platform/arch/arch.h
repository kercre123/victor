 ...
 *are architecture dependent.  The implementation of those is implemented in the
...
#if ((EXTERNAL_WAKEUP) && \
     (!BLE_APP_PRESENT))  // external wake up, only in full embedded designs
#include "gpio.h"
...
#error \
    "CFG_MEM_MAP_DEEP_SLEEP defined with CFG_MEM_MAP_EXT_SLEEP! Select one of them"
#endif
#endif
...
/*
 * Deep sleep threshold. Application specific. Control if during deep sleep the
 *system RAM will be powered off and if OTP copy will be required.
 ****************************************************************************************
 */
...
#define MAX_SLEEP_DURATION_PERIODIC_WAKEUP \
  MS_TO_SLOTS_CONVERT(CFG_MAX_SLEEP_DURATION_PERIODIC_WAKEUP_MS)
/// Sleep Duration Value in non external wake-up mode
#define MAX_SLEEP_DURATION_EXTERNAL_WAKEUP \
  MS_TO_SLOTS_CONVERT(CFG_MAX_SLEEP_DURATION_EXTERNAL_WAKEUP_MS)
 ...
 ****************************************************************************************
 */
#define XTAL16_TRIM_DELAY_SETTING \
  (0xA2)  // TRIM_CTRL_REG value ==> 14 cycles of 250usec each = 3.5ms (note:
          // RC16 is used)

/*
 * XTAL16 settling time measured in low power clock cycles.
 * Cannot be less than 1.9ms due to the OTP copy in deep sleep, which may take
 *quite long if RC16 is too low (i.e. 10MHz).
 ****************************************************************************************
 */
#define XTAL16M_SETTLING_IN_XTAL32_CYCLES \
  (95)  // count for 95 LP cycles, (x 30.5usec = ~2.9ms)
...
#define MINIMUM_SLEEP_DURATION \
  (1250)  // 2 slots (3 or more slots will be allowed (note: not for timer
          // wakeups))
#define LP_ISR_TIME_USEC \
  (3357)  // 110 * (1000000/32768); must be greater than
          // XTAL16M_SETTLING_IN_USEC
...
#define CLK_TRIM_WAIT_CYCLES \
  (1000)  // How many 16MHz cycles we will wait before applying the correct FREQ
          // TRIM value

/* Note
 * --------------------------
 * After XTAL16M settling, the code needs ~10usec + 1/2 LP cycle to power-up BLE
 * and pop BLE registers. This translates to ~26usec for XTAL32 and ~57usec for
 * RCX.
 *
 * Thus, LP_ISR_TIME_USEC - XTAL16 settling time = 3357 - 2900 = 457usec and 457
 * - 57 = 400usec are left as a safety margin in the worst case (RCX) (best case
 * is 3357 - 2900 - 26 = 431 for XTAL32).
 *
 * In principle, this time can be reduced to be at least 2 low power cycles.
 *
 * If the remaining time is not enough for periph_init() to complete then an
 * ASSERTION will hit in BLE_WAKEUP_LP_Handler (when DEVELOPMENT_DEBUG is 1). In
 * this case, since the settling time of the XTAL16M is fixed, the user should
 * increase the LP_ISR_TIME_USEC (and, consequently, the
...
#define TWEXT_VALUE_RCX \
  (TWIRQ_RESET_VALUE + 2)  // ~190usec (LP ISR processing time) / 85usec (LP clk
                           // period) = 2.23 => 3 LP cycles
#define TWEXT_VALUE_XTAL32 \
  (TWIRQ_RESET_VALUE + 6)  // ~190usec (LP ISR processing time) / 30usec (LP clk
                           // period) = 6.3 => 7 LP cycles

// The times below have been measured for RCX. For XTAL32 will be less. But this
// won't make much difference...
#define SLP_PROC_TIME (60)  // 60 usec for SLP ISR to request Clock compensation
#define SLEEP_PROC_TIME \
  (30)  // 30 usec for rwip_sleep() to program Sleep time and drive
        // DEEP_SLEEP_ON

// Change this if the application needs to increase the sleep delay limit from
// 9375usec that is now.
 ...
 * @param[in] polarity The polarity of the external wake up interrupt. 0=active
 *low. 1=active high
 * @return void
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief Compute size of SW stack used. This function is compute the maximum
 *size stack used by SW.
...
/**
 ****************************************************************************************
 * @brief Re-boot FW. This function is used to re-boot the FW when error has
 *been detected, it is the end of the current FW execution. After waiting
 *transfers on UART to be finished, and storing the information that FW has
 *re-booted by itself in a non-loaded area, the FW restart by branching at FW
 *entry point.
...
/**
 ****************************************************************************************
 * @brief Wrapper of the platform reset. It will be invoked before a software
 *reset is issued from the stack. Possible reasons will be included in the error
 *field.
...
/**
 ****************************************************************************************
 * @brief Selects convertion function (XTAL32 or RCX20) for low power cycles to
 *us.
...
/**
 ****************************************************************************************
 * @brief Selects convertion function (XTAL32 or RCX20) for us to low power
 *cycles.
...
/**
 ****************************************************************************************
 * @brief Initialise random number generator seed using random bits acquired
 *from TRNG
 * @return void
 ****************************************************************************************
 */
...
// required to define GLOBAL_INT_** macros as inline assembly. This file is
// included after definition of ASSERT macros as they are used inside ll.h
