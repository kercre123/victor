 ...
 * @file co_bt.h
 *
 * @brief This file contains the common Bluetooth defines, enumerations and
 *structures definitions for use by all modules in RW stack.
 *
 ...
 * definitions, mathematical functions, help functions, list and buffer
 *definitions.
 *
 * @{
 ****************************************************************************************
...
/* Packet and buffer sizes. These sizes do not include payload header (except
 * for FHS packet where there is no payload header) since payload header is
 * written or read by
...
/* Counter for train length, Npage (N*16 slots) depends on the slave page scan
 * mode and the number of active SCO: | SR mode |  no SCO  |  one SCO  |  two
 * SCO | |   R0    |   >=1    |    >=2    |    >=3   | |   R1    |   >=128  |
 * >=256  |    >=384 | |   R2    |   >=256  |    >=512  |    >=768 |   */
...
#define BT_DEFAULT_BDADDR                       \
  {                                             \
    { 0x01, 0x23, 0x45, 0x67, 0x89, RIPPLE_ID } \
  }
...
#define HCI_CCEVT_RDLOCCODECS_RETPAR_LEN \
  0x01  //+ SUPP_CODECS_LEN (variable length)
...
enum adv_filter_policy {
  ...
  /// Allow both scan req from White List devices only and connection req from
  /// anyone
  ADV_ALLOW_SCAN_WLST_CON_ANY,
  /// Allow both scan req from anyone and connection req from White List devices
  /// only
