...
#define KE_MSG_HANDLER(module, fname)                                         \
  static int module##_##fname##_handler(                                      \
...
#define KE_STATE_HANDLER(hdl) \
  { hdl, sizeof(hdl) / sizeof(struct ke_msg_handler) }
 ...
 *
 * In this function we also handle the SAVE service: when a task state changes
 *we try to activate all the messages currently saved in the save queue for the
 *given task identifier.
 *
 * @param[in]  id          Identifier of the task instance whose state is going
 *to be modified
...
/**
 ****************************************************************************************
 * @brief Generic message handler to consume message without handling it in the
 *task.
 *
