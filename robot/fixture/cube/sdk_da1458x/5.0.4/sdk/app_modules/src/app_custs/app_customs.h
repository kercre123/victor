...
/**
 ****************************************************************************************
 * @brief It is called to validates the characteristic value before it is
 *written to profile database.
 * @param[in] att_idx   Attribute index to be validated before it is written to
 *database
 * @param[in] last      last request of a multiple prepare write request.
 *                      When "true" it means the entire value has been received
 * @param[in] offset    offset at which the data has to be written (used only
 *for values that are greater than MTU)
 ...
 ****************************************************************************************
 */
uint8 app_custs1_val_wr_validate(uint16_t att_idx, bool last, uint16_t offset,
 ...
 ****************************************************************************************
 */
uint8 app_custs2_val_wr_validate(uint16_t att_idx, bool last, uint16_t offset,
                                 uint16_t len, uint8_t* value);
