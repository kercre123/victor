...
#define UUID_128_LE(b15, b14, b13, b12, b11, b10, b9, b8, b7, b6, b5, b4, b3, \
                    b2, b1, b0)                                               \
...
enum {
  ...
  SPOTA_STATUS_IMG_STARTED =
      0x10,  // SPOTA started for downloading image (SUOTA application)
};
...
void app_discover_all_char(uint16_t conidx, uint16_t start_handle,
                           uint16_t end_handle);

void app_characteristic_write(uint16_t conidx, uint16_t char_val_handle,
                              uint8_t *value, uint8_t value_length);

void app_characteristic_read(uint16_t conidx, uint16_t char_val_handle);

void app_discover_char_desc(uint16_t conidx, uint16_t start_handle,
                            uint16_t end_handle);
