 ...
 * @file app_spotar.h
 *
 * @brief Header file for application handlers for spotar ble events and
 *responses.
 *
...
typedef struct {
  ...
  uint8_t suota_image_bank;  // Image bank to upgrade. 0x00: oldest, 0x01: Image
                             // 1, 0x02: Image 2
  uint32_t suota_block_idx;  // The chuck index of the block. Has size 0 to
                             // new_patch_len
 ...
 *
 * @param[in]   mem_info: Patch memory info. Number of patches and overall patch
 *length.
 *
 * @return      void.
 ****************************************************************************************
...
/**
 ****************************************************************************************
 * @brief This function is called when the entire image has been received
 *successfully to set the valid flag field on the image header and make the
 *image valid for the bootloader.
 *
 * @param[in]   void
 *
 ...
 ****************************************************************************************
 */
int spotar_create_db_cfm_handler(ke_task_id_t const dest_id,
                                 struct spotar_create_db_cfm *param);

/**
 ****************************************************************************************
 * @brief Reads memory device and writes memory info.
 *
 * @param[in]   mem_dev: MSbyte holds the Memory device type, rest is the base
 *address.
 * @param[in]   mem_info: 16MSbits hold number of patches, 16LSbits hold overall
 *mem len.
 *
 * @return      void
 ****************************************************************************************
 ...
 *
 * @param[in] *rd_data_ptr:	Points to the position the read data will be
 *stored
 ...
 ****************************************************************************************
 */
int app_read_ext_mem(uint8_t *rd_data_ptr, FILE *fp, uint32_t address,
                     uint32_t size);
 ...
 ****************************************************************************************
 */
int app_write_ext_mem(uint8_t *rd_data_ptr, FILE *fp, uint32_t address,
                      uint32_t size);
 ...
 ****************************************************************************************
 */
int spotar_patch_mem_dev_ind_handler(ke_task_id_t const dest_id,
                                     struct spotar_patch_mem_dev_ind *param);
 ...
 ****************************************************************************************
 */
int spotar_patch_len_ind_handler(ke_task_id_t const dest_id,
 ...
 *        Firstly, the image header is extracted from the first block, then the
 *external memmory is checked to determine where to write this new image and
 *finally the header and the first image data are written to external memory.
 *
 * @param[in]   bank:      User can select bank=1 or bank=2 to force where to
 *write the image bypassing the check to update the oldest image in the ext
 *memory data:      Points to the first data block received over SUOTAR protocol
 ...
 ****************************************************************************************
 */
int app_read_image_headers(uint8_t image_bank, uint8_t *data,
                           uint32_t data_len);
 ...
 ****************************************************************************************
 */
int spotar_patch_data_ind_handler(ke_task_id_t const dest_id,
                                  struct spotar_patch_data_ind const *param);

/**
 ****************************************************************************************
 * @brief This function is called to find the older image of the two in the
 *external memmory
 *
...
/**
 ****************************************************************************************
 * @brief This function is called to get the correct image id to write to the
 *image header
 *
 ...
 *				upgraded image has been saved to it or after an error
 *on the suota process.
 *
 ...
 *			Otherwise it adds 0 bytes as long as the size of the image
 *header.
 *
 * @param[in]   void
 *
