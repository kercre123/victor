...
static void (*const app_process_catch_rest_cb)(
    ke_msg_id_t const msgid, void const *param, ke_task_id_t const dest_id,
    ke_task_id_t const src_id) = NULL;

static const struct arch_main_loop_callbacks user_app_main_loop_callbacks = {
    .app_on_init = default_app_on_init,

    // By default the watchdog timer is reloaded and resumed when the system
    // wakes up. The user has to take into account the watchdog timer handling
    // (keep it running, freeze it, reload it, resume it, etc), when the
    // app_on_ble_powered() is being called and may potentially affect the main
    // loop.
    ...
    // wakes up. The user has to take into account the watchdog timer handling
    // (keep it running, freeze it, reload it, resume it, etc), when the
    // app_on_system_powered() is being called and may potentially affect the
    // main loop.
...
// place in this structure the app_<profile>_db_create and app_<profile>_enable
// functions for SIG profiles that do not have this function already implemented
// in the SDK or if you want to override the functionality. Check the prf_func
// array in the SDK for your reference of which profiles are supported.
