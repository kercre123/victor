...
/****************************************************************************************************************/
/* If defined the OTP header data are read from SysRAM. The data are copied by
 * bootrom code.                    */
/* Otherwise the OTP Header data are read from OTP memory. */
/* It must be undefined during development phase and for applications booting
 * from external interfaces (UART,   */
/* SPI , I2C) */
/****************************************************************************************************************/
#undef CFG_BOOT_FROM_OTP

/****************************************************************************************************************/
/* If defined NVDS structure is initialized with hardcoded values. If not
 * defined the NDS structure is          */
/* padded with 0. The second option is used for production builds where the NVDS
 * area in OTP or non-volatile    */
...
/*      -LP_CLK_FROM_OTP    Use the selection in the corresponding field of OTP
 * Header                          */
/****************************************************************************************************************/
#define CFG_LP_CLK LP_CLK_XTAL32

/****************************************************************************************************************/
/* If defined the application uses a hadrcoded value for XTAL16M trimming.
 * Should be disabled for devices       */
/* where XTAL16M is calibrated and trim value is stored in OTP. */
/* Important note. The hardcoded value is the average value of the trimming
 * values giving the optimal results   */
...
/****************************************************************************************************************/
/* Enables True Random number Generator. A random number is generated at system
 * initialization and used to seed */
...
/* If CFG_MEM_MAP_EXT_SLEEP or no sleep mode is selected the heap memory sizes
 * are automatically configured     */
/* If CFG_MEM_MAP_DEEP_SLEEP is selected heap memory sizes are controlled by
 * setting the values of the following*/
/* directives. Developer can use UM-B-011: Memory map excel tool to calculate
 * the optimal values.               */
/*  -   USE_MEMORY_MAP  Memory configuration based on sleep mode. Determines if
 * specific data areas can be      */
/*      located in SysRAM or not. */
/*  -   REINIT_DESCRIPT_BUF  Reinitialize BLE messages' descriptors at wakeup
...
#define REINIT_DESCRIPT_BUF \
  0  // 0: keep in RetRAM, 1: re-init is required (set to 0 when Extended Sleep
     // is used)
#define USE_MEMORY_MAP EXT_SLEEP_SETUP
#else
#define REINIT_DESCRIPT_BUF \
  1  // 0: keep in RetRAM, 1: re-init is required (set to 0 when Extended Sleep
     // is used)
#define USE_MEMORY_MAP DEEP_SLEEP_SETUP
#define DB_HEAP_SZ 768
...
/****************************************************************************************************************/
/* NVDS configuration */
/* - CFG_NVDS_TAG_BD_ADDRESS            Default bdaddress. If bdaddress is
 * written in OTP header this value is  */
/*                                      ignored */
/* - CFG_NVDS_TAG_LPCLK_DRIFT           Low power clock drift. Permitted values
 * in ppm are:                     */
/*      + DRIFT_20PPM */
/*      + DRIFT_30PPM */
/*      + DRIFT_50PPM */
...
/* - CFG_NVDS_TAG_BLE_CA_TIMER_DUR      Channel Assessment Timer duration
 * (Multiple of 10ms)                    */
/* - CFG_NVDS_TAG_BLE_CRA_TIMER_DUR     Channel Reassessment Timer duration
 * (Multiple of CA timer duration)     */
/* - CFG_NVDS_TAG_BLE_CA_MIN_RSSI       Minimal RSSI Threshold */
/* - CFG_NVDS_TAG_BLE_CA_NB_PKT         Number of packets to receive for
 * statistics                             */
/* - CFG_NVDS_TAG_BLE_CA_NB_BAD_PKT     Number  of bad packets needed to remove
 * a channel                       */
/****************************************************************************************************************/
#define CFG_NVDS_TAG_BD_ADDRESS \
  { 0x05, 0x00, 0x00, 0xCA, 0xEA, 0x80 }
...
/****************************************************************************************************************/
/* Enables the logging of heap memories usage. The feature can be used in
 * development/debug mode                */
/* Application must be executed in Keil debugger environment. Developer must
 * stop execution and type            */
/* disp_memlog in debugger's command window. Heap memory statistics will be
 * displayed on window                 */
/****************************************************************************************************************/
...
/****************************************************************************************************************/
/* Used in DA14583. Enables reading the bdaddress from internal SPI flash. If
 * non DA14583 applications the      */
/* feature is disabled. */
/****************************************************************************************************************/
