...
// <o> DK selection <0=> As in da1458x_periph_setup.h <1=> Basic <2=> Pro <3=>
// Expert <4=> USB Dongle
#define HW_CONFIG (0)

#define HW_CONFIG_BASIC_DK \
  ((HW_CONFIG == 0 && SDK_CONFIG == 1) || HW_CONFIG == 1)
#define HW_CONFIG_PRO_DK ((HW_CONFIG == 0 && SDK_CONFIG == 2) || HW_CONFIG == 2)
#define HW_CONFIG_EXPERT_DK \
  ((HW_CONFIG == 0 && SDK_CONFIG == 3) || HW_CONFIG == 3)
...
#define I2C_SPEED_MODE \
  I2C_FAST  // 1: standard mode (100 kbits/s), 2: fast mode (400 kbits/s)
#define I2C_ADDRESS_MODE \
  I2C_7BIT_ADDR  // 0: 7-bit addressing, 1: 10-bit addressing
#define I2C_ADDRESS_SIZE \
  I2C_2BYTES_ADDR  // 0: 8-bit memory address, 1: 16-bit memory address, 3:
...
#define SPI_PHA_MODE \
  1  // If same with SPI_POL_MODE, data are valid on clk high edge, else on low
...
#define EXTERNAL_WAKEUP_GPIO_POLARITY \
  (GPIO_GetPinStatus(HCI_EIF_SELECT_PORT, HCI_EIF_SELECT_PIN) == 1) ? 1 : 0
#else
