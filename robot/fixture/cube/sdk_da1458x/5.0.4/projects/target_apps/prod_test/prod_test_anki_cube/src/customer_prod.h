...
// so this msg is confirmation of the message, so packages are not transmitted
// yet after packages are transmitted there's an additional message
// 'HCI_TX_PACKAGES_SEND_READY'.
...
enum op_audio_test { CMD__AUDIO_TEST_OP_START = 1, CMD__AUDIO_TEST_OP_STOP };
