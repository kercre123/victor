...
class MCUProxyServer(BaseSubServer):
    ...
    def stop(self):
        self.log("Closing MCUProxyServer\n")
    ...
    def step(self):
        ...
        while len(self.rawSerData):
            ...
            if self.v > 9: self.log("\t length = %d\n" % length)
            if length > 1500: # Should never happen, means corrupt data
                self.log("MCU bad length: %d\n" % length)
            ...
            if msgID == messages.PrintText.ID:
                self.log("M4: %s" % (self.rawSerData[1:length],)) # Print statement
