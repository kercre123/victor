 ...
 *              Mostly duplicated from util/logging/DAS.h so as not have to
 *include all of Util in vic-robot
 *
...
#error \
    "This header may not be included by other headers. We rely on GCC __INCLUDE_LEVEL__ to enforce this restriction."
#error \
    "If your compiler does not define __INCLUDE_LEVEL__, please add an appropriate check for your compiler."
#endif

#if (__INCLUDE_LEVEL__ > 1)
#error \
    "This header must be included directly from your CPP file. This header may not be included by other headers."
#error \
    "If this header is included by another header, doxygen macros will not expand correctly."
#endif
...
// DAS message macros
//
// These macros are used to ensure that developers provide some description of
// each event defined. Note these macros are expanded two ways!  In normal
// compilation, they are expanded into C++ variable declarations and logging
// calls.  When processed by doxygen, they are are expanded into syntactically
// invalid C++ that contains magic directives to produce readable documentation.
//
#ifndef DOXYGEN

#define DASMSG(ezRef, eventName, documentation) \
  {                                             \
    Anki::Vector::RobotInterface::DasMsg __DAS_msg(eventName);
#define DASMSG_SET(dasEntry, value, comment) \
  __DAS_msg.dasEntry = Anki::Vector::RobotInterface::DasItem(value);
#define DASMSG_SEND()                                \
  Anki::Vector::RobotInterface::sLogInfo(__DAS_msg); \
  }
#define DASMSG_SEND_WARNING()                           \
  Anki::Vector::RobotInterface::sLogWarning(__DAS_msg); \
  }
#define DASMSG_SEND_ERROR()                           \
  Anki::Vector::RobotInterface::sLogError(__DAS_msg); \
  }
#define DASMSG_SEND_DEBUG()                           \
  Anki::Vector::RobotInterface::sLogDebug(__DAS_msg); \
  }
