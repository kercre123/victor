...
constexpr auto MOTOR_LEFT_WHEEL =
    EnumToUnderlyingType(MotorID::MOTOR_LEFT_WHEEL);
constexpr auto MOTOR_RIGHT_WHEEL =
    EnumToUnderlyingType(MotorID::MOTOR_RIGHT_WHEEL);
...
constexpr auto NUM_BACKPACK_LEDS =
    EnumToUnderlyingType(LEDId::NUM_BACKPACK_LEDS);
...
const u32 batteryUpdateRate_tics_ =
    50;  // How often to update the simulated battery voltage (in robot ticks)
...
constexpr uint32_t kMicSendWindow_tics =
    6;  // Roughly equivalent to every anim process tic
...
Util::FixedCircularBuffer<RawChunkArray, kMicSendWindow_tics *
                                             kMaxNumMicMsgsAllowedPerSendWindow>
    micData_;
...
float HeadPowerToAngSpeed(float power) { return power * 2 * M_PI_F; }
...
void MotorUpdate() {
  ...
  for (int i = 0; i < MOTOR_COUNT; i++) {
    if (motors_[i]) {
      ...
      // Update speed
      motorSpeeds_[i] =
          (posDelta * ONE_OVER_CONTROL_DT) * (1.0 - motorSpeedCoeffs_[i]) +
          motorSpeeds_[i] * motorSpeedCoeffs_[i];
...
void UpdateSimBatteryVolts() {
  ...
  // Compute delta volts
  const float updateTime_sec =
      Util::MilliSecToSec((float)batteryUpdateRate_tics_ * ROBOT_TIME_STEP_MS);
  const float batteryDeltaVolts =
      (batteryIncreaseRate_voltsPerMin / 60.f) * updateTime_sec;
  ...
  batteryVolts = Util::Clamp(batteryVolts, minBatteryVolts, maxBatteryVolts);

  batteryVoltsField_->setSFFloat(batteryVolts);
}

void AudioInputCallback(const AudioUtil::AudioSample* data,
                        uint32_t numSamples) {
  std::lock_guard<std::mutex> lock(micDataMutex_);
  micData_.push_back();
  auto* newData = micData_.back().data();

  // Duplicate our mono channel input across 4 interleaved channels to simulate
  // 4 mics
...
Result HAL::Init(const int* shutdownSignal) {
  ...
  leftWheelPosSensor_ =
      webotRobot_.getPositionSensor("LeftWheelMotorPosSensor");
  rightWheelPosSensor_ =
      webotRobot_.getPositionSensor("RightWheelMotorPosSensor");
  ...
  // Backpack button
  backpackButtonPressedField_ =
      webotRobot_.getSelf()->getField("backpackButtonPressed");
  ...
  // Touch sensor
  touchSensorTouchedField_ =
      webotRobot_.getSelf()->getField("touchSensorTouched");
  ...
  DEV_ASSERT(batteryVoltsField_ != nullptr,
             "simHAL.Init.MissingBatteryVoltsField");

  batteryChargeRateField_ =
      webotRobot_.getSelf()->getField("batteryChargeRate_voltsPerMin");
  DEV_ASSERT(batteryChargeRateField_ != nullptr,
             "simHAL.Init.MissingBatteryChargeRateField");

  batteryDischargeRateField_ =
      webotRobot_.getSelf()->getField("batteryDischargeRate_voltsPerMin");
  DEV_ASSERT(batteryDischargeRateField_ != nullptr,
             "simHAL.Init.MissingBatteryDischargeRateField");

  // Audio Input
  audioCaptureSystem_.SetCallback(std::bind(
      &AudioInputCallback, std::placeholders::_1, std::placeholders::_2));
  audioCaptureSystem_.Init();

  if (audioCaptureSystem_.IsValid()) {
    audioCaptureSystem_.StartRecording();
  } else {
    PRINT_NAMED_WARNING("HAL.Init",
                        "AudioCaptureSystem not supported on this platform. No "
                        "mic data input available.");
  }
...
bool HAL::IsInitialized(void) { return isInitialized; }
...
bool HAL::IMUReadData(HAL::IMU_DataStructure& IMUData) {
  ...
  // Compute estimated IMU temperature based on measured data from Victor
  // prototype
  ...
  if (kSimulateGyroBias) {
    ...
    const float initialBias_dps[3] = {
        0.f, 0.f, 0.f};  // inital zero-rate offset at startup. worst case is
                         // +/- 10 deg/sec
    const float biasChangeDueToTemp_dps_per_degC =
        0.08f;  // zero-rate offset change as temperature changes. worst case
                // 0.08 deg/sec per degC
    const float biasDueToTemperature_dps =
        (IMUData.temperature_degC - T_initial) *
        biasChangeDueToTemp_dps_per_degC;

    for (int i = 0; i < 3; i++) {
      IMUData.gyro[i] +=
          DEG_TO_RAD(initialBias_dps[i] + biasDueToTemperature_dps);
    }
  }
...
float HAL::MotorGetCalibPower(MotorID motor) {
  ...
  switch (motor) {
    ...
    case MotorID::MOTOR_HEAD:
      ...
      break;
    default:
      PRINT_NAMED_ERROR("simHAL.MotorGetCalibPower.UndefinedType", "%d",
                        EnumToUnderlyingType(motor));
      break;
  }
  return power;
}
...
void HAL::MotorSetPower(MotorID motor, f32 power) {
  switch (motor) {
    ...
    case MotorID::MOTOR_HEAD:
      ...
      break;
    default:
      PRINT_NAMED_ERROR("simHAL.MotorSetPower.UndefinedType", "%d",
                        EnumToUnderlyingType(motor));
      return;
  }
}
...
void HAL::MotorResetPosition(MotorID motor) {
  if (motor >= MotorID::MOTOR_COUNT) {
    PRINT_NAMED_ERROR("simHAL.MotorResetPosition.UndefinedType", "%d",
                      EnumToUnderlyingType(motor));
    return;
  }
...
f32 HAL::MotorGetSpeed(MotorID motor) {
  switch (motor) {
    ...
    default:
      PRINT_NAMED_ERROR("simHAL.MotorGetSpeed.UndefinedType", "%d",
                        EnumToUnderlyingType(motor));
      break;
  }
  return 0;
...
f32 HAL::MotorGetPosition(MotorID motor) {
  switch (motor) {
    ...
    default:
      PRINT_NAMED_ERROR("simHAL.MotorGetPosition.UndefinedType", "%d",
                        EnumToUnderlyingType(motor));
      return 0;
  }
...
Result HAL::Step(void) {
  if (webotRobot_.step(Vector::ROBOT_TIME_STEP_MS) == -1) {
    ...
    // Always display ground truth pose:
    {
      ...
      webotRobot_.setLabel(robotID_, poseString, 0.5, robotID_*.05, .05,
    0xff0000, 0.);
    }
     */
...
ProxSensorDataRaw HAL::GetRawProxData() {
  ...
  if (PowerGetMode() == POWER_MODE_ACTIVE) {
    ...
    // Note: These fields are spoofed with simple defaults for now, but should
    // be computed to reflect the actual behavior of the sensor once we do some
...
u16 HAL::GetButtonState(const ButtonID button_id) {
  switch (button_id) {
    ...
    default: {
      AnkiError("sim_hal.GetButtonState.UnexpectedButtonType",
                "Button ID=%d does not have a sensible return value",
                button_id);
      return 0;
    }
  }
...
f32 HAL::BatteryGetVoltage() { return batteryVoltsField_->getSFFloat(); }

bool HAL::BatteryIsCharging() { return (chargeContact_->getPresence() == 1); }
...
u8 HAL::BatteryGetTemperature_C() { return 40; }

bool HAL::BatteryIsLow() { return (BatteryGetVoltage() < 3.6f); }
...
HAL::PowerState HAL::PowerGetDesiredMode() { return powerState_; }

HAL::PowerState HAL::PowerGetMode() { return powerState_; }

bool HAL::AreEncodersDisabled() { return false; }

bool HAL::IsHeadEncoderInvalid() { return false; }

bool HAL::IsLiftEncoderInvalid() { return false; }
