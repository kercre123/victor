...
// Spins until valid frame of `type` is recieved, or `timeout_ms` elapses.
// returns whole frame
const void* hal_get_frame(uint16_t type, int32_t timeout_ms);

// Spins until any valid frame is recieved, or `timeout_ms` elapses. returns
// whole frame
...
enum RobotMode {  // todo: mode is a dummy value. If ever needed, this should be
                  // in clad file.
  RobotMode_IDLE,
  RobotMode_RUN,
};
