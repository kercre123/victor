...
static const size_t SPINE_B2H_FRAME_LEN = sizeof(struct SpineMessageHeader) +
                                          sizeof(struct BodyToHead) +
                                          sizeof(struct SpineMessageFooter);

static const size_t SPINE_CCC_FRAME_LEN = sizeof(struct SpineMessageHeader) +
                                          sizeof(struct BodyToHead) +
                                          sizeof(struct SpineMessageFooter);
...
ssize_t spine_write_h2b_frame(spine_ctx_t spine,
                              const struct HeadToBody* h2b_payload);

// write message to set lights
// Only useful if not already sending h2b frames which
// contains light commands. (i.e. calm mode)
ssize_t spine_set_lights(spine_ctx_t spine,
                         const struct LightState* light_state);
...
// On success, returns the length of the complete frame and copies data into
// outbuf. On failure, returns 0 if not enough data has been buffered (partial
// frame) or -1 if buffered data is invalid
ssize_t spine_parse_frame(spine_ctx_t spine, void* out_buf, size_t out_buf_len,
                          size_t* out_len);
...
ssize_t spine_parse_header(const void* inbuf, size_t inbuf_len,
                           struct SpineMessageHeader* out_header);

// parse buffer into b2h data frame
ssize_t spine_parse_b2h_frame(const void* inbuf, size_t inbuf_len,
                              struct spine_frame_b2h* out_frame);

// create H2B frame for sending data to body
ssize_t spine_make_h2b_frame(const struct HeadToBody* h2b_payload,
                             struct spine_frame_h2b* out_frame);
