...
void ProcessIMUEvents() {
  ...
  if (imu_read_samples < 0) {
    ...
    if (!sentDAS) {
      sentDAS = true;
      DASMSG(imu_failure, "robot.imu_failure",
             "Indicates that we failed to read/write to the IMU and it may not "
  ...
  for (int i = 0; i < imu_read_samples; i++) {
    for (int j = 0; j < 3; j++) {
      ...
      imuData.gyro[j] =
          rawData[i].gyro[j] * IMU_GYRO_SCALE_DPS * RADIANS_PER_DEGREE;
    }
...
void ProcessLoop() {
  ...
  hdr_init(IMU_HDR_MIN_VALUE, IMU_HDR_MAX_VALUE, IMU_HDR_SIG_FIG, &_hdr);
#endif
...
bool HAL::IMUReadData(HAL::IMU_DataStructure& imuData) {
  ...
  while (PopIMU(imuData)) {
  };  // Just to pop queue
  static TimeStamp_t lastIMURead = 0;
  TimeStamp_t now = HAL::GetTimeStamp();
  if (now - lastIMURead > 4) {
    // TEMP HACK: Send 0s because on my Nexus 5x, the gyro values are kinda
    // crazy.
    imuData.accel[0] = 0.f;
    imuData.accel[1] = 0.f;
