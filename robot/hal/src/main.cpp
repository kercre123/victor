...
// For development purposes, while HW is scarce, it's useful to be able to run
// on phones
...
int run() {
  ...
  if (result != Result::RESULT_OK) {
    AnkiError("robot.main.InitFailed", "Unable to initialize (result %d)",
              result);
  ...
  // After Init, all memory we need has been initialized and the IMU thread (if
  // used) has been instantiated, lock our pages
  ...
  for (;;) {
    // HAL::Step should never return !OK, but if it does, best not to trust its
    // data.
    ...
    std::chrono::duration<double, std::micro> sleepTime =
        std::chrono::milliseconds(5) - elapsed;
    ...
    if (wasPackedOutAtBoot && shutdownSignal == 0 &&
