...
//
// Robot DAS doesn't link with ankiutils, so we have to duplicate parts of
// Anki::Util::DAS
//
...
void DasLogEvent(int prio, const DasMsg& dasMsg) {
  ...
  static_assert(Anki::Vector::RobotInterface::DAS::EVENT_MARKER == '@',
                "DAS event marker does not match declarations");
  static_assert(Anki::Vector::RobotInterface::DAS::FIELD_MARKER == '\x1F',
                "DAS field marker does not match declarations");
  static_assert(Anki::Vector::RobotInterface::DAS::FIELD_COUNT == 9,
  ...
  __android_log_print(
      ...
      dasMsg.i2.c_str(), dasMsg.i3.c_str(), dasMsg.i4.c_str(), UptimeMS());
}
