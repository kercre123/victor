...
Result InitRadio() {
  ...
  AnkiInfo("HAL.InitRadio.StartListening", "Start listening at %s",
           server_path.c_str());
  if (!server.StartListening(server_path.c_str())) {
    AnkiError("HAL.InitRadio.UDPServerFailed", "Unable to listen at %s",
              server_path.c_str());
...
bool HAL::RadioIsConnected(void) { return server.HasClient(); }
...
bool HAL::RadioSendPacket(const void* buffer, const size_t length) {
  if (server.HasClient()) {
    ...
    if (bytesSent < (ssize_t)length) {
      AnkiError("HAL.RadioSendPacket.FailedToSend",
                "Failed to send msg contents (%zd/%zu sent)", bytesSent,
                length);
