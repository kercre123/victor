...
static_assert(EnumToUnderlyingType(CLAD_VECTOR(MotorID)::MOTOR_LEFT_WHEEL) == MOTOR_LEFT, "Robot/Spine CLAD Mimatch");
static_assert(EnumToUnderlyingType(CLAD_VECTOR(MotorID)::MOTOR_RIGHT_WHEEL) == MOTOR_RIGHT, "Robot/Spine CLAD Mimatch");
static_assert(EnumToUnderlyingType(CLAD_VECTOR(MotorID)::MOTOR_LIFT) == MOTOR_LIFT, "Robot/Spine CLAD Mimatch");
static_assert(EnumToUnderlyingType(CLAD_VECTOR(MotorID)::MOTOR_HEAD) == MOTOR_HEAD, "Robot/Spine CLAD Mimatch");
...
float HAL::MotorGetCalibPower(CLAD_VECTOR(MotorID) motor)
{
  f32 power = 0.f;
  switch (motor) 
  {
    case CLAD_VECTOR(MotorID)::MOTOR_LIFT:
      power = HAL_LIFT_MOTOR_CALIB_POWER;
      break;
    case CLAD_VECTOR(MotorID)::MOTOR_HEAD:
...
void HAL::MotorSetPower(CLAD_VECTOR(MotorID) motor, f32 power)
{
...
void HAL::MotorResetPosition(CLAD_VECTOR(MotorID) motor)
{
  const auto m = EnumToUnderlyingType(motor);
  assert(m < MOTOR_COUNT);
...
f32 HAL::MotorGetSpeed(CLAD_VECTOR(MotorID) motor)
{
  const auto m = EnumToUnderlyingType(motor);
  assert(m < MOTOR_COUNT);
...
f32 HAL::MotorGetPosition(CLAD_VECTOR(MotorID) motor)
{
  const auto m = EnumToUnderlyingType(motor);
  assert(m < MOTOR_COUNT);
...
void PrintConsoleOutput(void)
{
#if REALTIME_CONSOLE_OUTPUT > 0
  {
    ...
    printf("Pos: %d %d %d %d", bodyData_->motor[(int)CLAD_VECTOR(MotorID)::MOTOR_HEAD].position,
                               bodyData_->motor[(int)CLAD_VECTOR(MotorID)::MOTOR_LIFT].position,
                               bodyData_->motor[(int)CLAD_VECTOR(MotorID)::MOTOR_LEFT_WHEEL].position,
                               bodyData_->motor[(int)CLAD_VECTOR(MotorID)::MOTOR_RIGHT_WHEEL].position);
    // printf("pos = %f ", HAL::MotorGetPosition(CLAD_VECTOR(MotorID)::MOTOR_OF_INTEREST));
    // printf("spd = %f ", HAL::MotorGetSpeed(CLAD_VECTOR(MotorID)::MOTOR_OF_INTEREST));
    // printf("pow = %f ", internalData_.motorPower[(int)CLAD_VECTOR(MotorID)::MOTOR_OF_INTEREST]);
