...
static_assert(EnumToUnderlyingType(MotorID::MOTOR_LEFT_WHEEL) == MOTOR_LEFT,
              "Robot/Spine CLAD Mimatch");
static_assert(EnumToUnderlyingType(MotorID::MOTOR_RIGHT_WHEEL) == MOTOR_RIGHT,
              "Robot/Spine CLAD Mimatch");
static_assert(EnumToUnderlyingType(MotorID::MOTOR_LIFT) == MOTOR_LIFT,
              "Robot/Spine CLAD Mimatch");
static_assert(EnumToUnderlyingType(MotorID::MOTOR_HEAD) == MOTOR_HEAD,
              "Robot/Spine CLAD Mimatch");

// From hal.cpp
extern BodyToHead* bodyData_;  // buffers are owned by the code that fills them.
                               // Spine owns this one
...
static f32 HAL_MOTOR_POSITION_SCALE[MOTOR_COUNT] = {
    ((0.96 * 29.0 * 0.25 * 3.14159265359) /
     172.3),  // Left Tread mm (Magic 96% corrective fudge factor, 29mm wheel
              // diameter)
...
static f32 HAL_MOTOR_DIRECTION[MOTOR_COUNT] = {1.0, -1.0, 1.0, 1.0};
...
float HAL::MotorGetCalibPower(MotorID motor) {
  ...
  switch (motor) {
    ...
    case MotorID::MOTOR_HEAD:
      ...
      break;
    default:
      AnkiError("HAL.MotorGetCalibPower.InvalidMotorType", "%s",
                EnumToString(motor));
      assert(false);
      break;
  }
...
void HAL::MotorSetPower(MotorID motor, f32 power) {
  ...
  headData_.motorPower[m] = HAL_MOTOR_POWER_OFFSET + HAL_MOTOR_POWER_SCALE *
                                                         power *
                                                         HAL_MOTOR_DIRECTION[m];
}
...
f32 HAL::MotorGetSpeed(MotorID motor) {
  ...
  if (bodyData_->motor[m].time != 0) {
    float countsPerTick =
        (float)bodyData_->motor[m].delta / bodyData_->motor[m].time;
...
f32 HAL::MotorGetPosition(MotorID motor) {
  const auto m = EnumToUnderlyingType(motor);
  assert(m < MOTOR_COUNT);
  return (bodyData_->motor[m].position - internalData_.motorOffset[m]) *
         HAL_MOTOR_POSITION_SCALE[m];
}
...
void PrintConsoleOutput(void) {
#if REALTIME_CONSOLE_OUTPUT > 0
  {
    ...
    printf("Pos: %d %d %d %d",
           bodyData_->motor[(int)MotorID::MOTOR_HEAD].position,
    ...
    // printf("pow = %f ",
    // internalData_.motorPower[(int)MotorID::MOTOR_OF_INTEREST]); printf("cliff
    // = %d %d% d% d
    // ",bodyData_->cliffSense[0],bodyData_->cliffSense[1],bodyData_->cliffSense[2],bodyData_->cliffSense[3]);
    // printf("prox = %d %d ",bodyData_->proximity.rangeStatus,
    // bodyData_->proximity.rangeMM);
    printf("\n");
  }
#endif
}
