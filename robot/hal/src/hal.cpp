...
BodyToHead* bodyData_;  // buffers are owned by the code that fills them. Spine
                        // owns this one
...
BodyToHead dummyBodyData_ = {.cliffSense = {800, 800, 800, 800}};
#endif
...
// so we cache the last non-0xFFFF value and return this as the latest touch
// sensor reading
...
static const TimeStamp_t MAX_POWER_MODE_SWITCH_TIME_MS[2] = {
    100,          // Calm->Active timeout
...
BodyToHead BootBodyData_ = {
    // dummy data for boot stub frames
    .framecounter = 0,
...
const u32 SPINE_GET_FRAME_TIMEOUT_MS =
    1000 * SELECT_TIMEOUT_SEC * (SELECT_TIMEOUT_ATTEMPTS + 1);
...
extern "C" {
ssize_t spine_write_frame(spine_ctx_t spine, PayloadId type, const void* data,
                          int len);
...
bool check_spine_readable(spine_ctx_t spine) {
  ...
  if (nfds < 0) {
    AnkiWarn("HAL.check_spine_readable.error", "select error %s",
             strerror(errno));
    return false;
  }
  ...
  if (nfds == 0) {
    selectTimeoutCount++;
    AnkiWarn("HAL.check_spine_readable.timeout", "select timeout %u",
             selectTimeoutCount);
...
void populate_boot_body_data(const struct SpineMessageHeader* hdr) {
  if (!haveValidSyscon_) {
    ...
    uint8_t button_pressed =
        ((struct MicroBodyToHead*)(hdr + 1))->buttonPressed;
    BootBodyData_.touchLevel[1] = button_pressed ? 0xFFFF : 0x0000;
    BootBodyData_.micError[0] =
        ~BootBodyData_.micError[0];  // prevent stuck bits
...
void das_log_version_info(const VersionInfo* versionInfo) {
  ...
  const int ein_arr_len =
      sizeof(versionInfo->ein) / sizeof(versionInfo->ein[0]);
  ...
  const int app_version_arr_len =
      sizeof(versionInfo->app_version) / sizeof(versionInfo->app_version[0]);
...
void handle_syscon_version(const VersionInfo* versionInfo) {
  if (memcmp(&_sysconVersionInfo, versionInfo, sizeof(_sysconVersionInfo)) !=
      0) {
...
Result spine_wait_for_first_frame(spine_ctx_t spine,
  ...
  while (!initialized && *shutdownSignal == 0) {
    ...
    ssize_t r =
        spine_parse_frame(spine, &frameBuffer_, sizeof(frameBuffer_), NULL);

    if (r < 0) {
      continue;
    } else if (r > 0) {
      const struct SpineMessageHeader* hdr =
          (const struct SpineMessageHeader*)frameBuffer_;
      if (hdr->payload_type == PAYLOAD_DATA_FRAME) {
        initialized = true;
        const struct spine_frame_b2h* frame =
            (const struct spine_frame_b2h*)frameBuffer_;
...
Result HAL::Init(const int* shutdownSignal) {
...
#ifndef HAL_DUMMY_BODY
  {
    ...
    nextInvalidProxDataReportSendTime_ms_ =
        GetTimeStamp() + INVALID_PROX_DATA_REPORT_PERIOD_MS;

    desiredPowerMode_ = POWER_MODE_ACTIVE;

    spine_init(&spine_);
    struct spine_params params = {.devicename = SPINE_TTY,
                                  .baudrate = SPINE_BAUD};
...
Result spine_get_frame() {
  ...
  do {
    ...
    if (r < 0) {
      ...
      const struct SpineMessageHeader* hdr =
          (const struct SpineMessageHeader*)frame_buffer;
      if (hdr->payload_type == PAYLOAD_DATA_FRAME) {
        handle_payload_data(
            frame_buffer);  // payload starts immediately after header
        result = RESULT_OK;
      } else if (hdr->payload_type == PAYLOAD_CONT_DATA) {
        ...
        populate_boot_body_data(hdr);
        // N.B. All the data in the resulting fake frame is 0, except for
        // buttonPressed & micError. This will only happen if body is in
        // bootloader & nonfunctional, so it shouldn't matter, but be aware.
        result = RESULT_OK;
      } else {
        LOGD("Unknown Frame Type %x\n", hdr->payload_type);
      }
...
void ReportRecentInvalidProxDataReadings() {
  ...
  if ((invalidProxSensorStatusCounts_[(int)RangeStatus::SIGMA_FAIL] +
    ...
    DASMSG(hal_invalid_prox_reading_report, "hal.invalid_prox_reading_report",
           "Report the recent number of minor status failures");
    DASMSG_SET(i1, timeSinceBoot_ms, "Time (ms) since last boot")
    DASMSG_SET(i2, invalidProxSensorStatusCounts_[(int)RangeStatus::SIGMA_FAIL],
               "Number of recent sigma failures");
    DASMSG_SET(i3,
               invalidProxSensorStatusCounts_[(int)RangeStatus::SIGNAL_FAIL],
               "Number of recent signal failures");
    DASMSG_SET(i4, invalidProxSensorStatusCounts_[(int)RangeStatus::PHASE_FAIL],
               "Number of recent phase failures");
    DASMSG_SEND();
  }
  ...
  if ((invalidProxSensorStatusCounts_[(int)RangeStatus::MIN_RANGE_FAIL] +
    ...
    DASMSG(hal_severe_invalid_prox_reading_report,
           "hal.severe_invalid_prox_reading_report",
           "Report of recent number of severe status failures");
    DASMSG_SET(i1, timeSinceBoot_ms, "Time (ms) since last boot")
    DASMSG_SET(i2,
               invalidProxSensorStatusCounts_[(int)RangeStatus::MIN_RANGE_FAIL],
               "Number of recent min range failures");
    DASMSG_SET(i3,
               invalidProxSensorStatusCounts_[(int)RangeStatus::HARDWARE_FAIL],
               "Number of recent hardware failures");
    DASMSG_SET(i4, noUpdateProxSensorStatusCount_,
               "Number of recent missing updates");
    DASMSG_SEND();
  }

  nextInvalidProxDataReportSendTime_ms_ += INVALID_PROX_DATA_REPORT_PERIOD_MS;
  memset(invalidProxSensorStatusCounts_, 0,
         sizeof(invalidProxSensorStatusCounts_));
  noUpdateProxSensorStatusCount_ = 0;
}

extern "C" ssize_t spine_write_ccc_frame(spine_ctx_t spine,
                                         const struct ContactData* ccc_payload);
...
Result HAL::Step(void) {
  ...
  if (now - last_packet_send >= MIN_CCC_XMIT_SPACING_US) {
    ...
    // struct HeadToBody* h2bp = (commander_is_active) ? ccc_data_get_response()
    // : &headData_;
    ...
    if (currPowerMode != desiredPowerMode_) {
      if (((lastPowerSetModeTime_ms_ == 0) && reportUnexpectedPowerMode_) ||
          ((lastPowerSetModeTime_ms_ > 0) &&
           ((now_ms - lastPowerSetModeTime_ms_ >
             MAX_POWER_MODE_SWITCH_TIME_MS[desiredPowerMode_])))) {
        AnkiWarn("HAL.Step.UnexpectedPowerMode",
                 "Curr mode: %u, Desired mode: %u, now: %ums, lastSetModeTime: "
                 "%ums, lastH2BSendTime: %ums",
                 currPowerMode, desiredPowerMode_, now_ms,
                 lastPowerSetModeTime_ms_, lastH2BSendTime_ms_);
    ...
    // Send DAS msg every 24 hours to report the number of invalid prox sensor
    // readings
  ...
  do {
    ...
    // Timeout is tuned to accommodate worst case back-to-back spine select
    // timeouts
    const u32 timeSinceStartOfGetFrame_ms =
        GetTimeStamp() - startSpineGetFrameTime_ms;
    ...
    if (*shutdownSignal_ != 0 &&
        timeSinceStartOfGetFrame_ms > ROBOT_TIME_STEP_MS) {
  ...
  if (bodyData_ != nullptr) {
    ...
    if (currTemp > 50 && currTemp != lastReportedBodyTemp_C &&
  ...
  // return a fail code if commander is active to prevent robotics from getting
  // confused
...
void ProcessMicError() {
  ...
  for (int i = 0; i < 32; ++i) {
    ...
    if (sameBitsArr[i] > 254) {
      ...
      sameBitsArr[i] = 0;  // prevent this channel from warning again for
                           // ~1.25s.
    }
  }
  ...
  if (!sentDAS && whichChannelsStuck > 0) {
    ...
    AnkiError("HAL.ProcessMicError.StuckBitDetected", "0x%x",
              whichChannelsStuck);

    DASMSG(mic_stuck_bit, "robot.stuck_mic_bit",
           "Indicates that one or more of the microphones is not functioning "
           "properly");
    DASMSG_SET(
        i1, whichChannelsStuck,
...
inline u16 FlipBytes(u16 v) { return ((((v)&0x00FF) << 8) | ((v) >> 8)); }
...
inline RangeStatus ConvertToApiRangeStatus(const u8 deviceRangeStatus) {
  ...
  // For a more detailed explanation of the failure codes, refer to the VL53L0X
  // API:
  ...
  switch (internal_device_range_status) {
    ...
    // NOTE: This mapping assumes that all of the internal sensor checks (sigma
    // limit, signal ref clip limit, etc.) are DISABLED. If this assumption is
    // not valid at some point in the future due to a firmware configuration
    // change, the results of these checks will need to be incorporated into
    // this mapping.
    case 1:
    case 2:
    case 3:
    ...
    default:
      ...
      // AnkiWarn("HAL.ConvertToApiRangeStatus.InvalidStatus", "0x%02x",
      // deviceRangeStatus);
      return RangeStatus::NO_UPDATE;
  }
}

ProxSensorDataRaw HAL::GetRawProxData() { return proxData_; }
...
void ProcessProxData() {
  ...
  if (HAL::PowerGetMode() == POWER_MODE_CALM) {
    ...
    proxData_.rangeStatus =
        ConvertToApiRangeStatus(bodyData_->proximity.rangeStatus);
    // Track the occurrences of invalid prox sensor readings, reported on a
    // periodic basis
    switch (proxData_.rangeStatus) {
      ...
      case RangeStatus::NO_UPDATE:
        ...
        break;
      default:
        AnkiWarn("HAL.ProcessProxData.UnhandledStatus", "%s",
                 EnumToString(proxData_.rangeStatus));
        break;
    }

    proxData_.distance_mm = FlipBytes(bodyData_->proximity.rangeMM);
    // Signal/Ambient Rate are fixed point 9.7, so convert to float:
    proxData_.signalIntensity =
        static_cast<float>(FlipBytes(bodyData_->proximity.signalRate)) / 128.f;
    proxData_.ambientIntensity =
        static_cast<float>(FlipBytes(bodyData_->proximity.ambientRate)) / 128.f;
    // SPAD count is fixed point 8.8, so convert to float:
    proxData_.spadCount =
        static_cast<float>(FlipBytes(bodyData_->proximity.spadCount)) / 256.f;
    proxData_.timestamp_ms = HAL::GetTimeStamp();
...
bool HAL::BatteryIsLow() { return bodyData_->battery.flags & POWER_IS_TOO_LOW; }
...
u8 HAL::BatteryGetTemperature_C() {
  if (bodyData_->battery.temperature > 0xff) {
    AnkiWarn("HAL.BatteryGetTemperature_C.InvalidTemp", "%u",
             bodyData_->battery.temperature);
    return 0;
  }
    ...
    if (++_bodyDataPrintCounter >= _bodyDataPrintPeriod_tics) {
      if (_bodyDataPrintMotors) {
        ...
        AnkiInfo("HAL.BodyData.Motors",
                 ...
                 "WL: (pos %d, dlt %d, tm %u), "
                 "WR: (pos %d, dlt %d, tm %u)",
                 bodyData_->flags, head.position, head.delta, head.time,
                 lift.position, lift.delta, lift.time, left.position,
                 left.delta, left.time, right.position, right.delta,
                 right.time);
      }
      if (_bodyDataPrintProx) {
        ...
        AnkiInfo("HAL.BodyData.Prox",
                 "Status 0x%02x, "
                 "Cliff: %4u %4u %4u %4u, "
                 "Prox: range %u, sig %u, amb %u, spadCnt %u, sampCnt %u, "
                 "calibRes %u",
                 bodyData_->flags, cliff[0], cliff[1], cliff[2], cliff[3],
                 prox.rangeMM, prox.signalRate, prox.ambientRate,
                 prox.spadCount, prox.sampleCount, prox.calibrationResult);
      }
      if (_bodyDataPrintBattery) {
        ...
        AnkiInfo("HAL.BodyData.Battery",
                 ...
                 bodyData_->flags, batt.main_voltage, batt.charger,
                 batt.temperature, batt.flags);
      }

      // TODO: Add more later. Maybe with filter flags so you don't always have
      // to print everything!
...
void HAL::PowerSetDesiredMode(const PowerState state) {
  ...
  DASMSG_SET(i1, state == POWER_MODE_ACTIVE,
             "Active mode (1) or calm mode (0)");
...
HAL::PowerState HAL::PowerGetDesiredMode() { return desiredPowerMode_; }

HAL::PowerState HAL::PowerGetMode() {
  if (bodyData_ == nullptr) {
    return POWER_MODE_ACTIVE;
  }
  return (bodyData_->flags & RUNNING_FLAGS_SENSORS_VALID) ? POWER_MODE_ACTIVE
                                                          : POWER_MODE_CALM;
}
...
void hal_terminate(void) { Anki::Vector::HAL::Shutdown(); }
}
