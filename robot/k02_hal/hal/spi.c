...
void spi_init(void) {
  ...
  SPI0_CTAR0 = SPI_CTAR_LSBFE_MASK | SPI_CTAR_BR(7) | SPI_CTAR_FMSZ(15) /*| SPI_CTAR_CPOL_MASK | SPI_CTAR_CPHA_MASK*/;
  ...
  for(;;) {
    ...
    {
      SPI0_PUSHR = SPI_PUSHR_CONT_MASK | SPI_PUSHR_PCS((tx_idx & 1) ? 2 : 0) | 0x5500; //spi_buff[tx_idx];
