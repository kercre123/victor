...
static void SendKill() {
	static const uint8_t KILL_PACKET[] = {
		'T', 'x', 1, 0,	// Write Flash
		0x00, 0x80, 0x01, 0x00, // Address 0x18000
	...
	int test = sizeof(KILL_PACKET);
	
	int index = 0;
	while (index < sizeof(KILL_PACKET)) {
		while (UART0_TCFIFO < uart_fifo_size) {
			UART0_D = KILL_PACKET[index++];
		}
	}
}
