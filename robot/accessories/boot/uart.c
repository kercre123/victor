// Simple 2m baud bit-bang UART on P10 (if you can get a wire on it)
#include "hal.h"

// Send a character out P10 - assumes P10 is already set for output
void PrintChar(u8 tx)
{
  ACC = tx;           // Get TX into A, inform compiler tx is not dead
  EA = 0;             // Can't handle interrupts because they stretch the output
  P1DIR = 255-2-1;    // Turn on P10 (and leave P11/CSB on) 
#pragma asm
