...
const f32 ENCODER_FILTERING_COEFF =
    0.f;  // TODO: If this seems to be ok at 0, get rid of filteredWheelSpeeds
...
void SetGains(const f32 kp, const f32 ki, const f32 maxIntegralError) {
  AnkiInfo("WheelController.SetGains", "New gains: kp=%f, ki=%f, maxSum=%f", kp,
           ki, maxIntegralError);
  Kp_ = kp;
  Ki_ = ki;
...
void Run() {
  ...
  if (!skipActiveControl) {
    ...
    AnkiDebug("WheelController", "speeds: %f (L), %f (R)   (Curr: %d, %d)",
              filterWheelSpeedL_, filterWheelSpeedR_, measuredWheelSpeedL_,
              measuredWheelSpeedR_);
...
#if (DEBUG_WHEEL_CONTROLLER)
    AnkiDebug("WheelController",
              "error: %f (L), %f (R)   error_sum: %f (L), %f (R)", errorL,
              errorR, error_sumL_, error_sumR_);
#endif

    /*
     // If commanded speed is 0 and current speed is pretty slow, suppress motor
     drive and don't let error accumulate.
     // This was contributing to brief slow crawls after pushing or sudden
     stops. if (GetUserCommandedCurrentVehicleSpeed() == 0 &&
     (GetCurrentMeasuredVehicleSpeed() < WHEEL_DEAD_BAND_MM_S ||
     (desiredWheelSpeedL < 0 && desiredWheelSpeedR < 0))) { error_suml_ = 0;
     error_sumr_ = 0;
     outl = 0;
     outr = 0;
    ...
    // Sum the error (integrate it). But ONLY, if we are not commading max
    // output already This should prevent the integral term to become to huge
    ...
    if (ABS(power_r_) < Vector::HAL::MOTOR_MAX_POWER) {
      ...
      error_sumR_ =
          CLIP(error_sumR_, -MAX_ERROR_SUM_RIGHT, MAX_ERROR_SUM_RIGHT);
    }
  } else {
...
void EncoderSpeedFilterIteration(void) {
  ...
  filterWheelSpeedL_ =
      (measuredWheelSpeedL_ * (1.0f - ENCODER_FILTERING_COEFF) +
       (filterWheelSpeedL_ * ENCODER_FILTERING_COEFF));
  filterWheelSpeedR_ =
      (measuredWheelSpeedR_ * (1.0f - ENCODER_FILTERING_COEFF) +
...
void SetDesiredWheelSpeeds(f32 leftws, f32 rightws) {
  if (enable_) {
    ...
    AnkiDebug("WheelController.SetDesiredWheelSpeeds.Disabled",
              "Ignoring speeds %f and %f while disabled", leftws, rightws);
  }
}
...
void utilSetVehicleOLTrajectory(u16 radius, u16 vspeed) {
  // if the radius is zero, we can't compute the speeds and return without doing
  // anything
  if (radius == 0) return;
...
bool AreWheelsPowered() { return (power_l_ != 0 || power_r_ != 0); }
