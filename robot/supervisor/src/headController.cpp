...
// If defined, angle is calibrated while power is still being applied, versus a
// short period of time after motor is "relaxed"
...
// Keep track of why we started a calibration, so that we can report this to DAS
...
// True if encoder was reported as invalid by HAL and has not been calibrated
// since
...
bool IsCalibrating() { return calState_ != HCS_IDLE; }

void ClearCalibration() { isCalibrated_ = false; }
...
void Stop() { SetAngularVelocity(0); }
...
void OnMotorCalibrated() {
  ...
  AnkiInfo("HeadController.Calibrated",
           ...
           EnumToString(calibrationReason_), angleError_deg);

  // Log DAS, but not if this is a calibration due to normal startup
  const u32 timeUncalibrated_ms =
      encoderInvalidStartTime_ms_ > 0
          ? HAL::GetTimeStamp() - encoderInvalidStartTime_ms_
          : 0;
  if (calibrationReason_ != MotorCalibrationReason::Startup) {
    DASMSG(head_motor_calibrated, "head_motor_calibrated",
           "The robot's head motor has just completed a calibration");
    DASMSG_SET(s1, EnumToString(calibrationReason_),
               "Reason for triggering calibration");
    DASMSG_SET(i1, 1000.f * angleError_deg,
               "Angular error (millidegrees). This represents how far out of "
               "calibration the motor was.");
    DASMSG_SET(i2, timeUncalibrated_ms,
               "Amount of time motor was uncalibrated according to syscon "
               "(ms). If syscon didn't know then 0.")
    DASMSG_SEND();
  }
}
...
void CalibrationUpdate() {
  if (!isCalibrated_) {
      ...
      case HCS_WAIT_FOR_STOP:
        ...
        if (!IsMoving()) {
          if (HAL::GetTimeStamp() - lastHeadMovedTime_ms > HEAD_STOP_TIME) {
            ...
            // Set timestamp to be used in next state to wait for motor to
            // "relax"
            lastHeadMovedTime_ms = HAL::GetTimeStamp();
    ...
    if (IsCalibrating() && calState_ >= HCS_WAIT_FOR_STOP) {
      ...
      if (currAngle - lowHeadAngleDuringCalib_rad_ >
          UPWARDS_HEAD_MOTION_FOR_CALIB_ABORT_RAD) {
        if (firstCalibration_) {
          AnkiWarn(
              "HeadController.CalibrationUpdate.RestartingCalib",
              "Someone is probably messing with head (low: %fdeg, curr: %fdeg)",
              RAD_TO_DEG(lowHeadAngleDuringCalib_rad_), RAD_TO_DEG(currAngle));
          calState_ = HCS_LOWER_HEAD;
        } else {
          AnkiInfo(
              "HeadController.CalibrationUpdate.Abort",
              "Someone is probably messing with head (low: %fdeg, curr: %fdeg)",
              RAD_TO_DEG(lowHeadAngleDuringCalib_rad_), RAD_TO_DEG(currAngle));
...
f32 GetAngleRad() { return currentAngle_.ToFloat(); }

void SetAngleRad(f32 angle) { currentAngle_ = angle; }

f32 GetLastCommandedAngle() { return desiredAngle_.ToFloat(); }

void GetCamPose(f32 &x, f32 &z, f32 &angle) {
  f32 cosH = cosf(currentAngle_.ToFloat());
  f32 sinH = sinf(currentAngle_.ToFloat());

  x = HEAD_CAM_POSITION[0] * cosH - HEAD_CAM_POSITION[2] * sinH +
      NECK_JOINT_POSITION[0];
  z = HEAD_CAM_POSITION[2] * cosH + HEAD_CAM_POSITION[0] * sinH +
...
void PoseAndSpeedFilterUpdate() {
  ...
  radSpeed_ = (measuredSpeed * (1.0f - SPEED_FILTERING_COEFF) +
  ...
  AnkiDebug("HeadController",
            "HEAD FILT: speed %f, speedFilt %f, currentAngle %f, currHalPos "
            "%f, prevPos %f, pwr %f",
            measuredSpeed, radSpeed_, currentAngle_.ToFloat(),
            HAL::MotorGetPosition(MOTOR_HEAD), prevHalPos_, power_);
#endif
  prevHalPos_ = HAL::MotorGetPosition(MotorID::MOTOR_HEAD);
}

f32 GetAngularVelocity() { return radSpeed_; }

void SetAngularVelocity(const f32 speed_rad_per_sec,
...
void SetMaxSpeedAndAccel(const f32 max_speed_rad_per_sec,
                         const f32 accel_rad_per_sec2) {
...
// TODO: There is common code with the other SetDesiredAngle() that can be
// pulled out into a shared function.
void SetDesiredAngle_internal(f32 angle, f32 acc_start_frac, f32 acc_end_frac,
                              f32 duration_seconds, f32 speed_rad_per_sec,
                              f32 accel_rad_per_sec2, bool useVPG) {
  if (!enable_ || bracing_) {
    return;
  }
  ...
  if (inPosition_ && (angle == desiredAngle_) &&
      (fabsf((desiredAngle_ - currentAngle_).ToFloat()) < HEAD_ANGLE_TOL)) {
#if (DEBUG_HEAD_CONTROLLER)
    AnkiDebug("HeadController", "Already at desired angle %f degrees",
  ...
  if (duration_seconds > 0) {
    res = vpg_.StartProfile_fixedDuration(
        startRad, startRadSpeed, acc_start_frac * duration_seconds,
        desiredAngle_.ToFloat(), acc_end_frac * duration_seconds,
        MAX_HEAD_SPEED_RAD_PER_S, MAX_HEAD_ACCEL_RAD_PER_S2, duration_seconds,
        CONTROL_DT);

    if (!res) {
      AnkiInfo("HeadController.SetDesiredAngle.VPGFixedDurationFailed",
               "startVel %f, startPos %f, acc_start_frac %f, acc_end_frac %f, "
               "endPos %f, duration %f. Trying VPG without fixed duration.",
               startRadSpeed, startRad, acc_start_frac, acc_end_frac,
               desiredAngle_.ToFloat(), duration_seconds);
    }
  }
  if (!res) {
    ...
    vpg_.StartProfile(startRadSpeed, startRad, vpgSpeed, vpgAccel, 0,
                      desiredAngle_.ToFloat(), CONTROL_DT);
  }

#if (DEBUG_HEAD_CONTROLLER)
  AnkiDebug("HeadController",
            "VPG (fixedDuration): startVel %f, startPos %f, acc_start_frac %f, "
            "acc_end_frac %f, endPos %f, duration %f",
            startRadSpeed, startRad, acc_start_frac, acc_end_frac,
            desiredAngle_.ToFloat(), duration_seconds);
#endif
...
void SetDesiredAngleByDuration(f32 angle_rad, f32 acc_start_frac,
  ...
  SetDesiredAngle_internal(angle_rad, acc_start_frac, acc_end_frac,
                           duration_seconds, MAX_HEAD_SPEED_RAD_PER_S,
                           MAX_HEAD_ACCEL_RAD_PER_S2, true);
}

void SetDesiredAngle(f32 angle_rad, f32 speed_rad_per_sec,
                     f32 accel_rad_per_sec2, bool useVPG) {
  SetDesiredAngle_internal(angle_rad, DEFAULT_START_ACCEL_FRAC,
                           DEFAULT_END_ACCEL_FRAC, 0, speed_rad_per_sec,
                           accel_rad_per_sec2, useVPG);
}

bool IsInPosition(void) { return inPosition_; }

// Check for conditions that could lead to motor burnout.
// If motor is powered at greater than BURNOUT_POWER_THRESH for more than
// BURNOUT_TIME_THRESH_MS, stop it! Assuming that motor is mis-calibrated and
// it's hitting the low or high hard limit. Do calibration. Returns true if a
// protection action was triggered.
bool MotorBurnoutProtection() {
  ...
  } else if (HAL::GetTimeStamp() - potentialBurnoutStartTime_ms_ >
             BURNOUT_TIME_THRESH_MS) {
    if (IsInPosition() || IMUFilter::IsBeingHeld() ||
      ...
      AnkiWarn("HeadController.MotorBurnoutProtection",
               "Recalibrating (power = %f)", power_);
      const bool autoStarted = true;
      StartCalibrationRoutine(
          autoStarted, MotorCalibrationReason::HeadMotorBurnoutProtection);
    }
    return true;
  }
  return false;
...
bool IsBracing() { return bracing_; }
...
Result Update() {
  ...
  if (not inPosition_) {
    ...
    // Compute power value
    power_ = (Kp_ * angleError_) +
             (Kd_ * (angleError_ - prevAngleError_) * CONTROL_DT) +
             (Ki_ * angleErrorSum_);
...
#if (DEBUG_HEAD_CONTROLLER)
    PERIODIC_PRINT(100,
                   "HEAD: currA %f, curDesA %f, desA %f, err %f, errSum %f, "
                   "pwr %f, spd %f\n",
                   currentAngle_.ToFloat(), currDesiredAngle_,
                   desiredAngle_.ToFloat(), angleError_, angleErrorSum_, power_,
                   radSpeed_);
...
void SetGains(const f32 kp, const f32 ki, const f32 kd,
  ...
  AnkiInfo("HeadController.SetGains",
           "New head gains: kp = %f, ki = %f, kd = %f, maxSum = %f", Kp_, Ki_,
           Kd_, MAX_ERROR_SUM);
}

bool IsEncoderInvalid() { return encoderInvalidStartTime_ms_ > 0; }
