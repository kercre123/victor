...
// Sets/Gets the speed the user wants the vehicle to drive at (eventually, after
// acceleration phase)
void SetUserCommandedDesiredVehicleSpeed(s16 ucspeed);
s16 GetUserCommandedDesiredVehicleSpeed(void);

// Set/Get the current target speed, while we are ramping up according to the
...
// This is for special case handling when we want the vehicle to reach the
// desired speed ASAP Ignores acceleration completely, and just sets the
// userCommandedCurrentVehicleSpeed
...
// This function assumes that userCommandedAcceleration is ALWAYS a positive
// value
void RunAccelerationUpdate(void);

// Runs one iteration of the vheicle speed controller to compute the commanded
// vehicle speed
void Manage(void);
