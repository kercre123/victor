...
Result Initialize() {
  ...
  matlabViz_.EvalStringEcho(
      "h_fig  = figure('Name', 'VisionSystem'); "
...
Result ResetFiducialDetection(const Array<u8>& image) {
  ...
  matlabViz_.EvalStringEcho(
      "set(h_img, 'CData', detectionImage); "
...
Result SendFiducialDetection(const Quadrilateral<f32>& corners,
  ...
  matlabViz_.EvalStringEcho(
      "detectedQuad = double(detectedQuad); "
...
Result SendTrackInit(const Array<u8>& image, const Quadrilateral<f32>& quad) {
  ...
  matlabViz_.EvalStringEcho(
      "h_template = axes('Pos', [0 0 .33 .33], 'Tag', 'TemplateAxes'); "
      "imagesc(detectionImage, 'Parent', h_template); hold on; "
      "plot(templateQuad([1 2 4 3 1],1), "
      "     templateQuad([1 2 4 3 1],2), 'r', "
      "     'LineWidth', 2, "
      "     'Parent', h_template); "
      "set(h_template, 'XLim', [0.9*min(templateQuad(:,1)) "
      "1.1*max(templateQuad(:,1))], "
      "                'YLim', [0.9*min(templateQuad(:,2)) "
      "1.1*max(templateQuad(:,2))]);");
  ...
  if (saveTrackingResults_) {
    ...
    matlabViz_.EvalStringEcho(
        "saveDir = fullfile('~', 'temp', '%s', '%s'); "
        "if ~isdir(saveDir), mkdir(saveDir); end, "
        "fid = fopen(fullfile(saveDir, 'quads.txt'), 'wt'); saveCtr = 0; "
        "fileCloser = onCleanup(@()fclose(fid)); "
        "imwrite(detectionImage, fullfile(saveDir, sprintf('image_%%.5d.png', "
        "saveCtr))); "
...
Result SendTrackInit(const Array<u8> &image,
                         ...
                         MemoryStack scratch)
{
  return SendTrackInit(image,
tracker.get_transformation().get_transformedCorners(scratch));
}
*/

Result SendTrack(const Array<u8>& image, const Quadrilateral<f32>& quad,
  ...
  //                                          "sprintf('~/temp/trackingImage%.3d.png',
  //                                          imageCtr)); " "imageCtr = imageCtr
  //                                          + 1;");
  matlabViz_.EvalStringEcho(
      "set(h_img, 'CData', trackingImage); "
  ...
  if (saveTrackingResults_) {
    matlabViz_.EvalStringEcho(
        "saveCtr = saveCtr + 1; "
        "imwrite(trackingImage, fullfile(saveDir, sprintf('image_%%.5d.png', "
        "saveCtr))); "
        "fprintf(fid, '[%%d] (%%f,%%f) (%%f,%%f) (%%f,%%f) (%%f,%%f)\\n', "
        "        saveCtr, "
  ...
  if (converged) {
    matlabViz_.EvalStringEcho(
        "title(h_axes, 'Tracking Succeeded', 'FontSize', 16);");
  } else {
...
{
#if USE_MATLAB_VISUALIZATION
  return SendTrack(image,
tracker.get_transformation().get_transformedCorners(scratch), converged); #else
  return RESULT_OK;
#endif
}
*/
...
#if USE_MATLAB_VISUALIZATION
  matlabViz_.EvalStringEcho(
      "h = subplot(1,2,%d, 'Parent', h_fig_tform), "
      "hold(h, 'off'), imagesc(img, 'Parent', h), axis(h, 'image'), hold(h, "
      "'on'), "
      "plot(quad([1 2 4 3 1],1), quad([1 2 4 3 1],2), 'r', 'LineWidth', 2, "
      "'Parent', h); "
      "title(h, '%s');",
      subplotNum, titleStr);
#endif
