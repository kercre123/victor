 ...
 *
 * Usage:
 *   1) Start profiler.  e.g. START_TIME_PROFILE(MyProfiler, FIRST) where FIRST
 *is the desired profile name of the following block of code up until the next
 *call to MARK_NEXT_TIME_PROFILE or END_TIME_PROFILE.
 *
 *   2) Mark the end of the previous profile and the start of new one
 *(Optional). e.g. MARK_NEXT_TIME_PROFILE(MyProfiler, SECOND)
 *
 *   3) Mark end of profiling  e.g. END_TIME_PROFILE(MyProfiler)
 *
 *      NB: All calls to START, MARK, and END should be in the same scope, or
 *profiles may be corrupted!
 *
...
#define START_TIME_PROFILE(profilerName, firstProfileName)       \
  static TimeProfiler timeProfiler##profilerName(#profilerName); \
  timeProfiler##profilerName.StartProfiling(#firstProfileName);
#define MARK_NEXT_TIME_PROFILE(profilerName, nextProfileName) \
  timeProfiler##profilerName.MarkNextProfile(#nextProfileName);
#define END_TIME_PROFILE(profilerName) \
  timeProfiler##profilerName.EndProfiling();
