 ...
 *   Orientation of gyro axes is assumed to be identical to that of robot when
 *the head is at 0 degrees. i.e. x-axis points forward, y-axis points to robot's
 *left, z-axis points up.
 *
...
// Roll angle:
f32 roll_ = 0.f;
const f32 ROLL_FILT_COEFF =
    0.75f;  // Filter to combine gyro and accel for smooth roll estimation
            // The higher this value, the slower it approaches accel-based roll,
            // but the less noisy it is.
const f32 ROLL_FILT_COEFF_MOVING =
    0.9f;  // Same as above, but used when the robot's wheels are moving

// Pitch angle: Approaches angle of accelerometer wrt gravity horizontal
f32 pitch_ = 0;
const f32 PITCH_FILT_COEFF =
    0.98f;  // Filter to combine gyro and accel for smooth pitch estimation
            // The higher this value, the slower it approaches accel-based
            // pitch, but the less noisy it is.
const f32 PITCH_FILT_COEFF_MOVING =
    0.998f;  // Same as above, but used when the robot's wheels are moving
const f32 UNINIT_HEAD_ANGLE =
    10000;  // Just has to be some not physically possible value
...
f32 gyro_robot_frame_filt[3] = {
    0};  // Filtered gyro measurements in robot frame
const f32 RATE_FILT_COEFF =
    1.f;  // IIR low-pass filter coefficient (1 == disable filter)

f32 gyro_bias_filt[3] = {0};  // Filtered gyro bias
const f32 GYRO_BIAS_FILT_COEFF_NORMAL =
    0.0005f;  // IIR low-pass filter coefficient (1 == disable filter).
              // Relatively slow once we're sure calibration is reasonably good
              // since it shouldn't be changing that fast.

const f32 GYRO_BIAS_FILT_COEFF_TEMP_CHANGING =
    0.0025f;  // Gyro bias filter coefficient used while the IMU temperature is
              // changing (due to initial warming up)

const f32 GYRO_BIAS_FILT_COEFF_PRECALIB =
    0.017f;  // Gyro bias filter coefficient. Relatively fast before
             // calibration.
...
// During initial gyro bias calibration, store recent bias estimates in a queue.
// This allows us to compute a min and max of recent bias estimates, to ensure
// that all readings are within a small band before declaring the gyro bias
// 'calibrated'.
std::array<Util::MinMaxQueue<f32>, 3> gyroBiasCalibValues_;

// Max difference between the max and min of recent gyro bias estimates. If any
// recent readings that fall outside this band, then the gyro bias is not yet
// calibrated.
const f32 BIAS_FILT_ALLOWED_BAND = DEG_TO_RAD_F32(0.35f);

// Number of consecutive gyro bias estimates required to fall within a small
// min/max window before declaring the gyro 'calibrated'
const u16 BIAS_FILT_COMPLETE_COUNT = 200;

bool gyro_sign[3] = {false};  // true is negative, false is positive

// Circular buffer of robot-frame gyro readings corresponding to every IMU
// reading that we have received. These get sent to engine as part of RobotState
// and are used for image de-warping, etc.
ImuDataBufferType imuDataBuffer_;

f32 accel_filt[3] = {0};  // Filtered accelerometer measurements
f32 accel_robot_frame[3] = {
    0};  // Unfiltered accelerometer measurements in robot frame
f32 accel_robot_frame_filt[3] = {
    0};  // Filtered accelerometer measurements in robot frame
f32 abs_accel_robot_frame_filt[3] = {0};  // Absolute value of accelerations
const f32 ACCEL_FILT_COEFF =
    0.1f;  // IIR low-pass filter coefficient (1 == disable filter)

f32 accelMagnitudeSqrd_ = 9810 * 9810;

const f32 HP_ACCEL_FILT_COEFF =
    0.5f;  // IIR high-pass filter coefficient (0 == no-pass)
f32 accel_robot_frame_high_pass[3] = {0};

// These values used to check if IMU temperature is changing (see usage in
// IMUFilter::Update())
...
const f32 PICKUP_WHILE_WHEELS_NOT_MOVING_GYRO_THRESH =
    DEG_TO_RAD_F32(10.f);                                           // rad/s
...
const f32 GYRO_MOTION_THRESHOLD =
    DEG_TO_RAD_F32(0.5f);  // Gyro motion threshold post-calibration
const f32 GYRO_MOTION_PRECALIB_THRESHOLD = DEG_TO_RAD_F32(
    10.f);  // Gyro motion threshold pre-calibration
...
bool fallStarted_ =
    false;  // Indicates that falling is detected by accelerometer, but not
            // necessarily for long enough to trigger falling_ flag
...
// note:
//
// In brief, there are three events used to determine if an impact was
// experienced:
//  BeingHeld, Freefall, and AccelerationSpike.
// The last one is when the measured acceleration norm from the imu shows high
// force which we know occurs when the robot hits the ground, or when is snapped
// or jerked during handling, or forcibly put onto a table. We care about not
// firing fall-impact events for the latter two cases, as a result we use a
// combination of heuristics on BeingHeld and Freefall detection to have
// confidence we experience a true-fall.
//
// Empirical testing shows that both BeingHeld and Freefall are prone to false
// results as well as inconsistent timing of detection. For example:
// - the robot may detect teetering right before rolling off of the edge of a
// table
//   as being held
// - during very fast tumbling mid-air, the robot may misclassify the Freefall
// state
// - the robot may register as being held either instantly, or up to a second
// later
//
// To combat this, we buffer the AccelerationSpike event with a rolling-window,
// which means when an impact is detected there is an active period during which
// we may catch other pertinent events like BeingHeld and Freefall to determine
// if it was a true-fall.
//
// Examples of how this is applied are:
//
// - suppressing impact-detection if being-held was recently true prevents
// handling
//   and moving the robot around as registering as a true-positive fall.
//
// - suppressing impact-detection if being-held is true for a minimum duration
// means
//   we can be more confident in the being-held state. The flipside is that it
//   takes longer to be certain, which necessitates buffering the acceleration
//   spike event in order to correlate it correctly.
//
// - free-fall detection has some cases (i.e. tumbling) that causes the robot to
//   misdetect this case. To combat this, we use a timeout that is conditioned
//   on freefall state variables to catch this case and thus check for impact
//   more aggressively (thus reducing false-negatives where we ignore impacts
//   because there was no confirmed fall)

// compute the max acceleration in a sliding-window for detecting the impact
// spike
...
// while the robot is being held, the minimum duration of a hold that suppresses
// impact-detection from activating
const TimeStamp_t MIN_TIME_HELD_SUPPRESS_IMPACT_MS = 500;

// after the robot is being putdown, suppress impacts for a duration of time
// (helps ignore being put down forcefully)
const TimeStamp_t MIN_TIME_SINCE_PUTDOWN_SUPPRESS_IMPACT_MS = 500;

// we expect to see a spike in acceleration in excess of 1.0g's at the time of
// impact
...
//        NaN in accelerometer normalisation) if(!((ax == 0.0f) && (ay == 0.0f)
//        && (az == 0.0f))) {
//
...
//          _4q2 * az; s3 = 4.0f * q1q1 * q3 - _2q1 * ax + 4.0f * q2q2 * q3 -
//          _2q2 * ay; recipNorm = invSqrt(s0 * s0 + s1 * s1 + s2 * s2 + s3 *
//          s3); // normalise step magnitude s0 *= recipNorm; s1 *= recipNorm;
//          s2 *= recipNorm;
//          s3 *= recipNorm;
//
//          // Apply feedback step
...
//        q0 += qDot1 * (1.0f * CONTROL_DT);   // q0 += qDot1 * (1.0f /
//        sampleFreq); q1 += qDot2 * (1.0f * CONTROL_DT); q2 += qDot3 * (1.0f *
//        CONTROL_DT); q3 += qDot4 * (1.0f * CONTROL_DT);
//
...
void ResetPickupVars() {
  ...
  external_accel_disturbance_cnt[0] = external_accel_disturbance_cnt[1] =
      external_accel_disturbance_cnt[2] = 0;
}
...
// Applies low-pass filtering to 3-element input, storing result to 3-element
// output assuming output is passed in with previous timestep's filter values.
...
// Returns low pass filtered output given single input and previous timestep's
// output
f32 LowPassFilter_single(const f32 prev_output, const f32 input,
                         const f32 coeff) {
  return input * coeff + prev_output * (1.f - coeff);
}

// Applies high-pass filtering to 3-element input and prev_input,
// storing result to 3-element output assuming output is passed in with previous
// timestep's filter values.
void HighPassFilter(f32* output, const f32* input, const f32* prev_input,
                    const f32 coeff) {
...
void UpdateGyroBiasFilter(const float rawGyro[3]) {
  ...
  //
  // During initial calibration, we maintain a filtered gyro bias estimate for
  // each of the three axes. Once the filtered bias estimates converge on a
  // near-constant value, we declare the gyro bias filter 'complete'. The
  // criterion for convergence is that the filtered bias estimate has remained
  // within a small window (BIAS_FILT_ALLOWED_BAND) for the required number of
  // ticks (BIAS_FILT_COMPLETE_COUNT).
  //
  // Once the initial calibration is complete (i.e. IsBiasFilterComplete() ==
  // true), we switch to a much slower filter which keeps track of slow drifting
  // of gyro zero-rate bias over time and temperature changes.

  if (!isMotionDetected_) {
    // Update gyro bias offset while not moving. If the gyro bias value queue is
    // just starting, then set the filtered gyro bias to the actual gyro value
    // to initialize it. Otherwise, pass it through the low pass filter.
    for (int i = 0; i < 3; i++) {
      if (!IsBiasFilterComplete() && gyroBiasCalibValues_[i].empty()) {
        gyro_bias_filt[i] = rawGyro[i];
      } else {
        gyro_bias_filt[i] =
            LowPassFilter_single(gyro_bias_filt[i], rawGyro[i], gyroBiasCoeff_);
      }
    }
    ...
    if (!IsBiasFilterComplete()) {
      ...
      // Only declare the bias filter complete if we have enough filtered bias
      // readings, and they all fall within the required window.
      biasFilterComplete_ = std::all_of(
          gyroBiasCalibValues_.begin(), gyroBiasCalibValues_.end(),
      ...
      if (IsBiasFilterComplete()) {
        ...
        AnkiInfo("IMUFilter.GyroCalibrated", "%f %f %f (deg/sec)",
                 gyro_bias_deg[0], gyro_bias_deg[1], gyro_bias_deg[2]);

        DASMSG(imu_filter_gyro_calibrated, "imu_filter.gyro_calibrated",
               "Gyro calibration values and duration");
        DASMSG_SET(i1, 1000 * gyro_bias_deg[0],
                   "X-axis gyro bias (milli-degrees)");
        DASMSG_SET(i2, 1000 * gyro_bias_deg[1],
                   "Y-axis gyro bias (milli-degrees)");
        DASMSG_SET(i3, 1000 * gyro_bias_deg[2],
                   "Z-axis gyro bias (milli-degrees)");
        DASMSG_SET(i4, HAL::GetTimeStamp() - lastMotionDetectedTime_ms,
                   "Time since last motion detected (ms)");
        DASMSG_SEND();

        gyroBiasCoeff_ = GYRO_BIAS_FILT_COEFF_NORMAL;
        // No longer need to keep the gyro bias calibration values, so discard
        // them to save memory
        ClearGyroBiasCalibValues();
      }
    }
  } else if (!IsBiasFilterComplete()) {
    // Since we're moving, clear out the initial gyro bias value queues so that
    // we restart gyro bias calibration
    ClearGyroBiasCalibValues();
  }
}
...
void DetectFalling() {
  ...
  const f32 FALLING_THRESH_LOW_MMPS2_SQRD =
      6000 * 6000;  // If accMag falls below this, fallStarted is triggered...
  const f32 FALLING_THRESH_HIGH_MMPS2_SQRD =
      9000 * 9000;  // ...and not untriggered until it rises above this.
  const f32 STOPPED_TUMBLING_THRESH =
      50.f;  // accelerometer value corresponding to "no longer tumbling on the
             // ground after a fall"

  // Fall detection timing:
  const TimeStamp_t now = HAL::GetTimeStamp();
  const TimeStamp_t fallDetectionTimeout_ms =
      150;  // fallStarted flag must be set for this long in order for 'bracing'
            // to occur.

  // "Bracing manuever" timing
  const TimeStamp_t bracingTime_ms =
      250;  // this much time (minimum) is allowed for the bracing maneuver to
            // complete.
  ...
  if (falling_) {
    if (accelMagnitudeSqrd_ > FALLING_THRESH_HIGH_MMPS2_SQRD) {
      ...
      // Check for high-freq activity on x-axis (this could easily be any other
      // axis since the threshold is so small) to determine when the robot is
      // definitely no longer moving.
      if ((fabsf(accel_robot_frame_high_pass[0]) < STOPPED_TUMBLING_THRESH) &&
        ...
        DASMSG(imu_filter_falling_event, "imu_filter.falling_event",
               "Robot experienced a fall");
    ...
    if (fallStarted_) {
      // If fallStarted has been set for long enough, set the global falling
      // flag and brace.
      if (now - fallStartedTime_ > fallDetectionTimeout_ms) {
        ...
        fallStarted_ = (accelMagnitudeSqrd_ < FALLING_THRESH_HIGH_MMPS2_SQRD) &&
                       ProxSensors::IsAnyCliffDetected();
      }
...
void DetectImpact() {
  ...
  if (maxAccelerationMagnitudeBuffer_.size() >
      MAX_ACCEL_MAGNITUDE_BUFFER_SIZE) {
    maxAccelerationMagnitudeBuffer_.pop();
  }
  f32 maxAccMagnitude = maxAccelerationMagnitudeBuffer_.max();

  // compute whether the robot's being-held state should suppress the impact
  // detector
  ...
  const bool isCurrentlyHeld =
      currRobotBeingHeldState &&
      ((now - timeOfStartBeingHeld_) > MIN_TIME_HELD_SUPPRESS_IMPACT_MS);
  const bool wasRecentlyHeld =
      !currRobotBeingHeldState &&
      ((now - timeOfEndBeingHeld_) < MIN_TIME_SINCE_PUTDOWN_SUPPRESS_IMPACT_MS);
  const bool beingHeldSuppressImpact = (wasRecentlyHeld || isCurrentlyHeld);

  // determine if we should query for impact detection, because of a possible
  // fall
  ...
  const bool shouldCheckForImpact =
      (timeSinceFallingDetected < IMPACT_DETECTION_WINDOW_AFTER_FALL_MS);

  // throttle sending the impact message if we've sent it already
  const bool canSendMessage =
      (timeSinceImpactMsg > IMPACT_MESSAGE_THROTTLE_TIME_MS);

  if (canSendMessage) {
    const bool didAccelerationSpike =
        (maxAccMagnitude > MIN_ACCEL_MAG_SQUARED_FOR_IMPACT_MMPS2);
    const bool detectedImpact = shouldCheckForImpact &&
                                !beingHeldSuppressImpact &&
                                didAccelerationSpike;

    if (detectedImpact) {
      RobotInterface::SendMessage(RobotInterface::FallImpactEvent());

      DASMSG(imu_filter_fall_impact_event, "imu_filter.fall_impact_event",
             "Robot experienced a freefall for a minimum duration and then a "
             "sudden impact");
      DASMSG_SEND();
...
bool AreMotorsMoving() {
  return WheelController::AreWheelsMoving() || HeadController::IsMoving() ||
         LiftController::IsMoving();
}

// Checks for conditions to enter/exit picked up state
void DetectPickup() {
  if (!pickupDetectEnabled_) return;
  ...
  bool cliffBasedPickupDetect = numCliffSensorsDetectingChange >=
                                NUM_CLIFF_SENSORS_FOR_CHANGE_DETECT_PICKUP;

  if (IsPickedUp()) {
    // Picked up flag is reset only when the robot has
    // stopped moving, detects no cliffs, and has been set upright.
    if (!cliffBasedPickupDetect && !isMotionDetected_ &&
    ...
    // Different detection thresholds are used depending on whether or not the
    // wheels are moving.
    bool gyroZBasedMotionDetect = false;
    if (!AreMotorsMoving()) {
      // As long as wheels aren't moving, we can also check for Z-axis gyro
      // motion
      gyroZBasedMotionDetect = ABS(gyro_robot_frame_filt[2]) >
                               PICKUP_WHILE_WHEELS_NOT_MOVING_GYRO_THRESH;

    } else {
      // Is the robot turning at a radically different speed than what it should
      // be experiencing given current wheel speeds?
      // UNEXPECTED_ROTATION_SPEED_THRESH is being used as a multipurpose margin
      // here. Because GetCurrNoSlipBodyRotSpeed() is based on filtered wheel
      // speeds there's a little delay which permits measuredBodyRotSpeed to be
      // a little faster than maxPossibleBodyRotSpeed.
      const f32 maxPossibleBodyRotSpeed =
          WheelController::GetCurrNoSlipBodyRotSpeed();
      const f32 measuredBodyRotSpeed = IMUFilter::GetRotationSpeed();
      gyroZBasedMotionDetect =
          (((maxPossibleBodyRotSpeed > UNEXPECTED_ROTATION_SPEED_THRESH) &&
            ((measuredBodyRotSpeed < -UNEXPECTED_ROTATION_SPEED_THRESH) ||
             (measuredBodyRotSpeed >
              maxPossibleBodyRotSpeed + UNEXPECTED_ROTATION_SPEED_THRESH))) ||
           ((maxPossibleBodyRotSpeed < -UNEXPECTED_ROTATION_SPEED_THRESH) &&
            ((measuredBodyRotSpeed > UNEXPECTED_ROTATION_SPEED_THRESH) ||
             (measuredBodyRotSpeed <
    ...
    // Sensitive check
    // If motors aren't moving, any motion is because a person was messing with
    // it!
    if (!AreMotorsMoving()) {
      // Sufficient gyro motion is evidence of pickup
      bool gyroBasedMotionDetected =
          (ABS(gyro_robot_frame_filt[0]) >
           PICKUP_WHILE_WHEELS_NOT_MOVING_GYRO_THRESH) ||
          (ABS(gyro_robot_frame_filt[1]) >
           PICKUP_WHILE_WHEELS_NOT_MOVING_GYRO_THRESH) ||
          (ABS(gyro_robot_frame_filt[2]) >
           PICKUP_WHILE_WHEELS_NOT_MOVING_GYRO_THRESH);
      ...
      // Only evaluating the horizontal axes. Z-acceleration is sensitive to
      // surface vibrations, plus z-motion should be captured more reliably by
      // the cliff sensor. Keeping track of sign of disturbance because if it
      // crosses 0 this is more indicative of vibration versus steady motion.
      for (u8 i = 0; i < 2; ++i) {
        if (ABS(accel_robot_frame_high_pass[i]) >
            ACCEL_DISTURBANCE_MOTION_THRESH) {
          s8 incr = accel_robot_frame_high_pass[i] > 0 ? 1 : -1;
          if (accel_robot_frame_high_pass[i] > 0 ==
      ...
      bool accelBasedMotionDetected = (ABS(external_accel_disturbance_cnt[0]) >
                                       PICKUP_COUNT_WHILE_MOTIONLESS) ||
                                      (ABS(external_accel_disturbance_cnt[1]) >
                                       PICKUP_COUNT_WHILE_MOTIONLESS);

      if (potentialPickupCnt_ > PICKUP_COUNT_WHILE_MOTIONLESS ||
          accelBasedMotionDetected) {
        AnkiInfo("IMUFilter.PDWhileStationary",
                 "acc (%f, %f, %f), gyro (%f, %f, %f), cliff %d",
                 accel_robot_frame_filt[0], accel_robot_frame_filt[1],
                 accel_robot_frame_filt[2], gyro_robot_frame_filt[0],
                 gyro_robot_frame_filt[1], gyro_robot_frame_filt[2],
      ...
      if (CheckPickupWhileMoving() || cliffBasedPickupDetect ||
        ...
        if (++potentialPickupCnt_ > PICKUP_COUNT_WHILE_MOVING) {
          SetPickupDetect(true);
          AnkiInfo("IMUFilter.PickupDetected",
                   "accX %f, accY %f, accZ %f, cliff %d, gyroZ %d",
                   accel_robot_frame_filt[0], accel_robot_frame_filt[1],
                   accel_robot_frame_filt[2], cliffBasedPickupDetect,
                   gyroZBasedMotionDetect);
        }
      } else {
        potentialPickupCnt_ = 0;
...
void DetectMotion() {
  ...
  if (AreMotorsMoving() ||
      ...
      (IsBiasFilterComplete() && (ABS(gyro_[0]) > GYRO_MOTION_THRESHOLD ||
  ...
  // TODO: Gyro seems to be sensitive enough that it's sufficient for detecting
  // motion, but if
  //       that's not the case, check for changes in gravity vector.
  // ...

  isMotionDetected_ =
      (lastMotionDetectedTime_ms + MOTION_DETECT_TIMEOUT_MS) >= currTime;
}
...
void DetectPoke() {
  ...
  // Do nothing if motors are moving, as detecting pokes while the robot is
  // attempting to move is far more challenging. Also don't attempt to detect
  // pokes if a pickup was detected.
  // TODO(GB): Poke detection should still run if the robot is being held
  // upright in the user's hand.
  if (!AreMotorsMoving() && !pickedUp_) {
    ...
    if (fabsf(gyro_robot_frame_filt[2]) > PEAK_GYRO_THRESHOLD) {
      ...
      if ((_peakGyroMaxTime > _peakGyroStartTime) &&
          (_peakGyroMaxTime - _peakGyroStartTime < MAX_GYRO_PEAK_DURATION_MS)) {
...
// This pitch measurement isn't precise to begin with, but it's extra imprecise
// when the head is moving so be careful relying on it when the head is moving!
void UpdatePitch() {
  f32 headAngle = HeadController::GetAngleRad();

  if (prevHeadAngle_ != UNINIT_HEAD_ANGLE) {
    const f32 accelBasedPitch =
        atan2f(imu_data_.accel[0], imu_data_.accel[2]) - headAngle;
    const f32 gyroBasedPitch = pitch_ - (gyro_robot_frame[1] * CONTROL_DT) -
                               (headAngle - prevHeadAngle_);

    // Complementary filter to mostly trust gyro integration for current pitch
    // in the short term but always approach accelerometer-based pitch in the
    // "long" term. Because of things like keepaway and pounce which require
    // fast and somewhat accurate measurements of pitch, use a different
    // coefficient while the wheels are moving.
    const f32 coeff =
        (WheelController::AreWheelsPowered() ||
         WheelController::AreWheelsMoving() || HeadController::IsMoving())
  ...
  // AnkiDebugPeriodic(50, "RobotPitch", "%f deg (motion %d, gyro %f)",
  // RAD_TO_DEG_F32(pitch_), MotionDetected(), gyro_robot_frame_filt[1]);
}

void UpdateRoll() {
  const f32 headAngle = HeadController::GetAngleRad();
  const f32 accelBasedRoll =
      atan2f(imu_data_.accel[1], imu_data_.accel[2] * cosf(headAngle));
  const f32 gyroBasedRoll = roll_ - (gyro_robot_frame[0] * CONTROL_DT);

  // Complementary filter to mostly trust gyro integration for current roll in
  // the short term but always approach accelerometer-based roll in the "long"
  // term.
  const bool isMoving = WheelController::AreWheelsPowered() ||
                        WheelController::AreWheelsMoving() ||
                        HeadController::IsMoving();
  ...
  //                         "Filtered, accel-based, gyro-based: %f %f %f, (In
  //                         motion: %d)", RAD_TO_DEG_F32(roll_),
...
Result Update() {
  ...
  while (HAL::IMUReadData(imu_data_)) {
    ...
    //
    // Gyro zero-rate bias changes with IMU temperature, so a more aggressive
    // gyro bias filter coefficient is required for times when IMU temperature
    // is changing 'rapidly'. If IMU temperature has changed a lot in the past x
    // seconds, apply a more aggressive gyro bias filter coef. Use a faster rate
    // for playpen, since it needs to log IMU temperature changes.
    ...
    if (curTime > timeOfLastImuTempSample_ms_ + imuTempReadingRate_ms &&
        IsBiasFilterComplete()) {
      const bool temperatureChanging =
          fabsf(lastImuTempSample_degC_ - imu_data_.temperature_degC) >
          temperatureDiffThresh_degC;
      gyroBiasCoeff_ = temperatureChanging ? GYRO_BIAS_FILT_COEFF_TEMP_CHANGING
...
#ifndef SIMULATOR
    // VECTOR: Correct for observed sensitivity error on z axis of gyro
    // (VIC-285) It has been observed that the z axis gyro usually reports about
    // a 1.03% higher rate than it is actually experiencing, so simply scale it
    // here.
    gyro_[2] *= 0.989f;
#endif
    ...
    const f32 headAngle =
        HeadController::GetAngleRad();  // TODO: Use encoders or accelerometer
                                        // data? If encoders, may need to use
                                        // accelerometer data anyway for when
                                        // it's on ramps.
    ...
    //
    // Rotation in robot frame = D * [dr/dt, dp/dt, dy,dt] where the latter
    // vector is given by gyro readings. In our case, we only care about yaw. In
    // other words, it's always true that r = y = 0. (NOTE: This is true as long
    // as we don't start turning on ramps!!!) So the result simplifies to...
    gyro_robot_frame[0] = gyro_[0] + gyro_[2] * tanf(headAngle);
    gyro_robot_frame[1] = gyro_[1];
    gyro_robot_frame[2] = gyro_[2] / cosf(headAngle);
    // TODO: We actually only care about gyro_robot_frame_filt[2]. Any point in
    ...
    const f32 xzAccelMagnitude =
        std::hypot(imu_data_.accel[0], imu_data_.accel[2]);
    const f32 accel_angle_imu_frame =
        atan2_fast(imu_data_.accel[2], imu_data_.accel[0]);
    ...
    HighPassFilter(accel_robot_frame_high_pass, accel_robot_frame_filt,
                   prev_accel_robot_frame_filt, HP_ACCEL_FILT_COEFF);
...
#if (DEBUG_IMU_FILTER)
    PERIODIC_PRINT(200, "Accel angle %f %f\n", accel_angle_imu_frame,
                   accel_angle_robot_frame);
    ...
    //                      imu_data_.accel[0], imu_data_.accel[1],
    //                      imu_data_.accel[2]);
    ...
    if (isRecording_) {
      ...
      if (++recordDataIdx_ == IMU_CHUNK_SIZE) {
        ...
        if (imuChunkMsg_.chunkId == imuChunkMsg_.totalNumChunks) {
          AnkiDebug("IMUFilter.IMURecording.Complete", "time %dms",
                    HAL::GetTimeStamp());
          isRecording_ = false;
        }
      }
      ...
      if (sentIMUDataMsgs_ == totalIMUDataMsgsToSend_) {
        AnkiDebug("IMUFilter.IMURecording.CompleteRaw", "time %dms",
                  HAL::GetTimeStamp());
        isRecording_ = false;
      ...
      for (int i = 0; i < 3; i++) {
        ...
        imuRawDataMsg_.g[i] =
            (int16_t)1000.f * imu_data_.gyro[i];  // millirad/sec
      }
...
HAL::IMU_DataStructure GetLatestRawData() { return imu_data_; }

const f32* GetBiasCorrectedGyroData() { return gyro_; }

ImuDataBufferType& GetImuDataBuffer() { return imuDataBuffer_; }
...
f32 GetRotationSpeed() { return rotSpeed_; }

f32 GetPitch() { return pitch_; }

f32 GetRoll() { return roll_; }

bool IsPickedUp() { return pickedUp_ || falling_; }

bool IsBeingHeld() { return IsPickedUp() && isMotionDetected_; }

bool IsMotionDetected() { return isMotionDetected_; }

bool IsFalling() { return falling_; }

bool IsBiasFilterComplete() { return biasFilterComplete_; }

const f32* GetGyroBias() { return gyro_bias_filt; }
...
void RecordAndSend(const u32 length_ms) {
  ...
  imuChunkMsg_.totalNumChunks =
      length_ms / (ROBOT_TIME_STEP_MS * IMU_CHUNK_SIZE);
#else
