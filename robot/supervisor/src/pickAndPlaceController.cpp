...
// Used for placement only when the lift tends to get stuck due to block
// friction.
...
const u32 HIGH_DOCK_POINT_OF_NO_RETURN_DIST_MM =
    ORIGIN_TO_HIGH_LIFT_DIST_MM + 40;
...
bool robotHasStartedPitching_ =
    false;  // becomes true once the robot has begun to tilt forward as a result
            // of driving up the charger ramp.

// When charger docking, we must first tilt this amount before enabling cliff
// sensor correction
...
// Amount of time that Cozmo pitch needs to exceed TILT_FAILURE_ANGLE_RAD in
// order to fail at backing up on charger
...
// Whether to delay sending the pickup success message until after the lift is
// in position
bool _sendPickupSuccessAfterLiftUp = false;

// During cube pickup, whether or not to raise the lift at the same time that we
// begin backing up
bool _backUpWhileLiftingCube = false;

// Deep roll action params
// Note: These are mainly for tuning the deep roll and can be removed once it's
// locked down.
f32 _rollLiftHeight_mm =
    35;  // The lift height to command when engaging the block for roll
f32 _rollDriveSpeed_mmps =
    50;  // The forward driving speed while engaging the block for roll
f32 _rollDriveAccel_mmps2 =
    40;  // The forward driving accel while engaging the block for roll
u32 _rollDriveDuration_ms =
    1500;  // The forward driving duration while engaging the block for roll
f32 _rollBackupDist_mm =
    100;  // The amount to back up once the lift has engaged the block for roll

// Deep roll actions const params for a scooch adjustment
// Seems to help avoid treads rubbing up against cube corners causing roll to
// fail
...
typedef enum { INIT, ALIGNING, ALIGNED } CliffAlignState;
...
DockAction GetCurAction() { return action_; }
...
void SendCliffAlignCompleteMessage(const CliffAlignResult result) {
  AnkiInfo("PAP.SendCliffAlignCompleteMessage.Result", "%s",
           EnumToString(result));
...
static void StartBackingOut() {
  ...
  AnkiInfo("PAP.StartBackingOut.Dist",
           "Starting %.1fmm backout (%.2fsec duration)", backoutDist_mm,
           backoutTime_sec);

  transitionTime_ = HAL::GetTimeStamp() + (backoutTime_sec * 1e3f);

  SteeringController::ExecuteDirectDrive(-BACKOUT_SPEED_MMPS,
                                         -BACKOUT_SPEED_MMPS);
  ...
  switch (mode_) {
      ...
      switch (action_) {
        ...
        case DockAction::DA_FACE_PLANT:
          LiftController::SetDesiredHeight(LIFT_HEIGHT_LOWDOCK,
                                           DEFAULT_LIFT_SPEED_RAD_PER_SEC,
                                           DEFAULT_LIFT_ACCEL_RAD_PER_SEC2);
        ...
        case DockAction::DA_ALIGN_SPECIAL:
          ...
          break;
        case DockAction::DA_POST_DOCK_ROLL:
          // Skip docking completely and go straight to Setting lift for Post
          // Dock
        ...
        case DockAction::DA_BACKUP_ONTO_CHARGER_USE_CLIFF:
          ...
          SteeringController::ExecuteDirectDrive(kChargerDockingSpeedHigh,
                                                 kChargerDockingSpeedHigh);
          transitionTime_ = HAL::GetTimeStamp() + 7000;
          useCliffSensorAlignment_ =
              (action_ == DockAction::DA_BACKUP_ONTO_CHARGER_USE_CLIFF);
    ...
    case MOVING_LIFT_PREDOCK: {
      if (LiftController::IsInPosition() && HeadController::IsInPosition()) {
        if (action_ == DockAction::DA_PLACE_LOW_BLIND) {
          DockingController::StartDockingToRelPose(
              dockSpeed_mmps_, dockAccel_mmps2_, dockDecel_mmps2_,
              dockOffsetDistX_, dockOffsetDistY_, dockOffsetAng_);
        } else {
          // Set the distance to the marker beyond which
          // we should ignore docking error signals since the lift occludes our
          // view anyway.
          ...
          DockingController::StartDocking(
              dockSpeed_mmps_, dockAccel_mmps2_, dockDecel_mmps2_,
              dockOffsetDistX_, dockOffsetDistY_, dockOffsetAng_,
              pointOfNoReturnDist, useFirstErrorSignalOnly);
        }
        mode_ = DOCKING;
#if (DEBUG_PAP_CONTROLLER)
    ...
    case DOCKING: {
      if (!DockingController::IsBusy()) {
        if (DockingController::DidLastDockSucceed()) {
          ...
          // Docking failed for some reason. Probably couldn't see block
          // anymore.
          ...
          switch (action_) {
            case DockAction::DA_PICKUP_LOW:
            case DockAction::DA_PICKUP_HIGH: {
              SendPickAndPlaceResultMessage(false,
                                            BlockStatus::BLOCK_PICKED_UP);
#ifdef SIMULATOR
              // Disengage the 'gripper' here since we have failed to dock with
              // the cube
      ...
      switch (action_) {
        ...
        case DockAction::DA_PLACE_LOW_BLIND: {
          LiftController::SetDesiredHeight(LIFT_HEIGHT_LOWDOCK,
                                           ...
                                           DEFAULT_LIFT_ACCEL_RAD_PER_SEC2);
          transitionTime_ =
              HAL::GetTimeStamp() + START_BACKOUT_PLACE_LOW_TIMEOUT_MS;
          break;
        }
        case DockAction::DA_PICKUP_LOW:
        case DockAction::DA_PICKUP_HIGH: {
          ...
          // When a block is picked up we want an "animation" to play where
          // Cozmo moves forwards and backwards a little bit while picking up
          // the block, gives a sense of momentum
          ...
          if (_backUpWhileLiftingCube) {
            // If we are to back up while lifting the cube, then skip the
            // "pickup anim". Set the transition time to now so that we
            // immediately begin driving backward in the WAITING_TO_MOVE state.
            ...
            PathFollower::AppendPathSegment_Line(
                ...
                PICKUP_ANIM_SPEED_MMPS, PICKUP_ANIM_ACCEL_MMPS2,
        ...
        case DockAction::DA_PLACE_HIGH: {
          LiftController::SetDesiredHeight(LIFT_HEIGHT_HIGHDOCK,
                                           ...
                                           DEFAULT_LIFT_ACCEL_RAD_PER_SEC2);
          transitionTime_ =
              HAL::GetTimeStamp() + START_BACKOUT_PLACE_HIGH_TIMEOUT_MS;
          break;
        }
        case DockAction::DA_ROLL_LOW:
        case DockAction::DA_DEEP_ROLL_LOW:
        case DockAction::DA_POST_DOCK_ROLL: {
          ...
          // TODO: Can these be replaced with DEFAULT_LIFT_SPEED_RAD_PER_SEC and
          // DEFAULT_LIFT_ACCEL_RAD_PER_SEC2?
          const f32 LIFT_SPEED_FOR_ROLL = 0.75f;
          const f32 LIFT_ACCEL_FOR_ROLL = 100.f;

          if (action_ == DockAction::DA_DEEP_ROLL_LOW) {
            LiftController::SetDesiredHeight(
                _rollLiftHeight_mm, LIFT_SPEED_FOR_ROLL, LIFT_ACCEL_FOR_ROLL);
            SteeringController::ExecuteDirectDrive(
                _rollDriveSpeed_mmps, _rollDriveSpeed_mmps,
            ...
            LiftController::SetDesiredHeight(
                LIFT_HEIGHT_LOWDOCK, LIFT_SPEED_FOR_ROLL, LIFT_ACCEL_FOR_ROLL);
        ...
        case DockAction::DA_POP_A_WHEELIE:
          ...
          SpeedController::SetUserCommandedAcceleration(
              100);  // TODO: Restore this accel afterwards?
    ...
    case MOVING_LIFT_FOR_ROLL: {
      if (LiftController::GetHeightMM() <= LIFT_HEIGHT_LOW_ROLL) {
        ...
        // In case lift has trouble getting to low position, we don't want to
        // back up forever.
    ...
    case MOVING_LIFT_FOR_DEEP_ROLL: {
      if (HAL::GetTimeStamp() > transitionTime_ ||
          IMUFilter::GetPitch() > DEG_TO_RAD_F32(35.f)) {
        ...
        SteeringController::ExecuteDirectDrive(0, 0);
        LiftController::SetDesiredHeight(
            _rollLiftHeight_mm + _kRollLiftHeightScoochOffset_mm,
            _kRollLiftScoochSpeed_rad_per_sec,
            _kRollLiftScoochAccel_rad_per_sec_2);
    ...
    case FACE_PLANTING_BACKOUT: {
      if (IMUFilter::GetPitch() < _facePlantStartBackupPitch_rad) {
        SteeringController::ExecuteDirectDrive(_facePlantDriveSpeed_mmps,
                                               _facePlantDriveSpeed_mmps);
    ...
    case FACE_PLANTING: {
      if (HAL::GetTimeStamp() > transitionTime_) {
        SendPickAndPlaceResultMessage(
            IMUFilter::GetPitch() < _facePlantStartBackupPitch_rad,
    ...
    case MOVING_LIFT_POSTDOCK: {
      if (LiftController::IsInPosition() ||
        ...
        switch (action_) {
          case DockAction::DA_PICKUP_LOW:
          case DockAction::DA_PICKUP_HIGH: {
            // For pickup, delay sending the success message until after lift
            // has reached its target height. This prevents the camera from
            // immediately re-observing the cube before the lift has raised
            // enough.
          ...
          default:
            AnkiError("PAP.MOVING_LIFT_POSTDOCK.InvalidAction", "%hhu",
                      action_);
        }  // switch(action_)
    ...
    case PICKUP_ANIM: {
      switch (pickupAnimMode_) {
        case (WAITING_TO_MOVE): {
          // Once enough time passes start driving backwards for the backwards
          // part of the "animation"
          if (HAL::GetTimeStamp() > transitionTime_) {
            SteeringController::ExecuteDirectDrive(-PICKUP_ANIM_SPEED_MMPS,
                                                   -PICKUP_ANIM_SPEED_MMPS);
            transitionTime_ =
                HAL::GetTimeStamp() + PICKUP_ANIM_TRANSITION_TIME_MS;
            pickupAnimMode_ = MOVING_BACK;
          }
          break;
        }
        case (MOVING_BACK): {
          // We have been driving backwards for long enough and the "animation"
          // is complete
          if (HAL::GetTimeStamp() > transitionTime_) {
            SteeringController::ExecuteDirectDrive(0, 0);
            mode_ = MOVING_LIFT_POSTDOCK;
        ...
        case (NOTHING): {
          // If we have finished doing the forwards part of the "animation"
          // transition to waiting to do the backwards part
    ...
    case BACKUP_ON_CHARGER: {
      if (HAL::GetTimeStamp() > transitionTime_) {
        ...
        Reset();
      } else if (ProxSensors::GetCliffDetectedFlags() ==
                 ((1 << HAL::CLIFF_BL) | (1 << HAL::CLIFF_BR))) {
        // Cliff detection is disabled now, so double check that we are not
        // seeing a cliff on BOTH rear cliff sensors (which may indicate a real
        // cliff and not a spurious trip)
        ...
        } else if (HAL::GetTimeStamp() - tiltedOnChargerStartTime_ >
                   TILT_FAILURE_DURATION_MS) {
        ...
        if (!robotHasStartedPitching_ &&
            (IMUFilter::GetPitch() <
             pitchAngleAtStartOfBackup_rad_ -
                 kChargerDockingMinPitchAngleChange_rad)) {
          robotHasStartedPitching_ = true;
          AnkiInfo("PAP.BACKUP_ON_CHARGER.StartedPitching", "pitch: %.2f deg",
                   RAD_TO_DEG(IMUFilter::GetPitch()));
        }

        // Slow down one of the sides if it's seeing white, but only if we have
        // first seen black at some point on that sensor, AND only if we have
        // begun driving up onto the charger platform (based on pitch angle).
        // This is to ensure that the robot does not veer away from the charger
        // on initial approach with light-colored tables or tables with funky
        // textures like the dreaded dark wood grain.
        if (robotHasStartedPitching_) {
    ...
    case CLIFF_ALIGN_TO_WHITE: {
      ...
      switch (cliffAlignState_) {
        case INIT: {
          ...
          // Check that at least one of the front cliff sensors is detecting
          // white
          if (white_l && white_r) {
            ...
            SendCliffAlignCompleteMessage(
                CliffAlignResult::CLIFF_ALIGN_FAILURE_NO_WHITE);
            Reset();
          }
          break;
        }
        case ALIGNING: {
          ...
          if (fabsf((cliffAlignStartAngle_ - Localization::GetCurrPose_angle())
                        .ToFloat()) > CLIFF_ALIGN_MAX_ANGLE_RAD) {
            AnkiInfo("PAP.CLIFF_ALIGN_TO_WHITE.TurnedTooMuch", "");
            SendCliffAlignCompleteMessage(
                CliffAlignResult::CLIFF_ALIGN_FAILURE_OVER_TURNING);
            Reset();
            break;
          }

          if (HAL::GetTimeStamp() - cliffAlignStartTime_ms_ >
              CLIFF_ALIGN_TIMEOUT_MS) {
            AnkiInfo("PAP.CLIFF_ALIGN_TO_WHITE.Timeout", "");
            SendCliffAlignCompleteMessage(
                CliffAlignResult::CLIFF_ALIGN_FAILURE_TIMEOUT);
            Reset();
            break;
          }
          ...
          if (NEAR(gyroZ_radps, 0, CLIFF_ALIGN_ZERO_GYRO_TOL)) {
            ...
            if (cliffAlignNearZeroGyroCount_ >
                CLIFF_ALIGN_MAX_COUNT_ZERO_GYRO) {
              AnkiInfo("PAP.CLIFF_ALIGN_TO_WHITE.NoGyroMovement", "");
              SendCliffAlignCompleteMessage(
                  CliffAlignResult::CLIFF_ALIGN_FAILURE_NO_TURNING);
              Reset();
              break;
            }
          } else {
...
bool IsCarryingBlock() { return carryState_ != CarryState::CARRY_NONE; }
...
void SetCarryState(CarryState state) { carryState_ = state; }

CarryState GetCarryState() { return carryState_; }

void DockToBlock(const DockAction action, const bool doLiftLoadCheck,
                 const f32 speed_mmps, const f32 accel_mmps2,
                 const f32 decel_mmps2, const f32 rel_x, const f32 rel_y,
                 const f32 rel_angle, const u8 numRetries) {
#if (DEBUG_PAP_CONTROLLER)
...
void PlaceOnGround(const f32 speed_mmps, const f32 accel_mmps2,
                   const f32 decel_mmps2, const f32 rel_x, const f32 rel_y,
                   const f32 rel_angle) {
  DockToBlock(DockAction::DA_PLACE_LOW_BLIND, speed_mmps, accel_mmps2,
              decel_mmps2, rel_x, rel_y, rel_angle);
}
...
void StopCliffAlignToWhite() {
  if (mode_ != IDLE && action_ == DockAction::DA_CLIFF_ALIGN_TO_WHITE) {
    SendCliffAlignCompleteMessage(
        CliffAlignResult::CLIFF_ALIGN_FAILURE_STOPPED);
    Reset();
  }
}

void SetRollActionParams(const f32 liftHeight_mm, const f32 driveSpeed_mmps,
                         const f32 driveAccel_mmps2, const u32 driveDuration_ms,
                         const f32 backupDist_mm) {
  AnkiDebug("PAP.SetRollActionParams",
            "liftHeight: %f, speed: %f, accel: %f, duration %d, backupDist %f",
            liftHeight_mm, driveSpeed_mmps, driveAccel_mmps2, driveDuration_ms,
            backupDist_mm);
...
void SetBackUpWhileLiftingCube(const bool b) { _backUpWhileLiftingCube = b; }
