...
// If we are in this deadband, we won't command anythign to the wheels if we are
// trying to slow down
...
// This is an upper bound on the actual rotation speed since with treads there
// is significant slip.
f32 GetCurrNoSlipBodyRotSpeed();

// This function will command a wheel speed to the left and right wheel so that
// the vehicle follows a trajectory This will only work if the steering
// controller does not overwrite the values.
