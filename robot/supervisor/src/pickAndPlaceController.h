...
// transmitted from cozmo-engine immediately after calling this. (Except for
// DA_PLACE_LOW_BLIND) If DA_PLACE_LOW_BLIND, rel_* parameters are wrt to
// current robot pose. Otherwise, rel_* params are ignored.
void DockToBlock(const DockAction action, const bool doLiftLoadCheck,
                 const f32 speed_mmps, const f32 accel_mmps2,
                 const f32 decel_mmps2, const f32 rel_x = 0,
                 const f32 rel_y = 0, const f32 rel_angle = 0,
...
// When set, this will cause the robot to begin backing up at the same time that
// he begins to raise the lift during cube pickup. This prevents cube pickup
// from failing when the cube is pressed up against a wall or other rigid
// surface.
