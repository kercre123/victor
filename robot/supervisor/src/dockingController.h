 ...
 * Description:
 *
 *   Controller for docking to a block, where 'docking' does not necessarily
 *mean the forklift is engaging the block but rather following some sort of
 *approach path based on a specific block's relative location.
 *
...
// Returns true if robot is the process of looking for a block or docking to a
// block
bool IsBusy();
...
// The robot will follow a docking path generated from an error signal to a
// marker that it expects to be receiving from cozmo-engine immediately after
// this is called. dockOffsetDistX: Distance along normal from block face that
// the robot should "dock" to. dockOffsetDistY: Horizontal offset from block
// face center that the robot should "dock" to.
//                  +ve = left of robot.
//                  e.g. To place a block on top of two other blocks, the robot
//                  would need to "dock" to
//                       one of the blocks at some horizontal offset.
// dockOffsetAngle: Docking offset angle. +ve means block is facing robot's
// right side.
void StartDocking(const f32 speed_mmps, const f32 accel_mmps,
                  const f32 decel_mmps, const f32 dockOffsetDistX,
                  const f32 dockOffsetDistY = 0, const f32 dockOffsetAngle = 0,
                  const u32 pointOfNoReturnDistMM = 0,
                  const bool useFirstErrSignalOnly = false);

// Goes to a pose such that if the robot were to lower a block that it was
// carrying once it were in that pose, the block face facing the robot would be
// aligned with the pose specified relative to the pose of the robot at the time
// "docking" started. No vision markers are required as this is a "blind
// docking" maneuver.
...
// Returns the distance between the current robot origin and the last computed
// location of the marker that it is/was docking to. Use carefully!
