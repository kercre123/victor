 ...
 *
 * Description:
 *   Class for returning ideal speed and velocity given start velocity, start
 *position, end velocity, end position, acceleration (same value used for
 *deceleration), maximum speed, and the update timestep.
 *
 *   Useful for feeding a controller a progressive target rather than just the
 *final one.
 *
 **/
...
class VelocityProfileGenerator {
 public:
  ...
  void StartProfile(float startVel, float startPos, float maxSpeed, float accel,
                    float endVel, float endPos, float timeStep);

  // Starts a trajectory for a target velocity only. No target position.
  void StartProfile(float startVel, float startPos, float endVel, float accel,
                    float timeStep);

  /*
  bool StartProfile_fixedDuration(float startPos, float startVel, float
  startAccel, float endPos, float endVel, float endAccel, float duration, float
  timeStep);
   */
  ...
  // acc_start_duration: Initial time period during which we accelerate to some
  // velocity (i.e. vel_mid)
  //            pos_end: End position
  //   acc_end_duration: Final time period following the period in which we're
  //   coasting at vel_mid to approach a final speed of 0.
  //            vel_max: Max velocity permitted. Fails if any velocity in the
  //            profile exceeds this amount. acc_max: Max acceleration
  //            permitted. Fails if any acceleration in the profile exceeds this
  //            amount.
  //           duration: Time period that the entire traversal from pos_start to
  //           pos_end should take. timeStep: Time resolution at which
  //           subsequent calls to Step() will compute position and velocity.
  bool StartProfile_fixedDuration(float pos_start, float vel_start,
                                  float acc_start_duration, float pos_end,
                                  float acc_end_duration, float vel_max,
                                  float acc_max, float duration,
                                  float timeStep);
  ...
  //
  // TODO: Assuming that timestep is small enough that we don't need to care
  // about
