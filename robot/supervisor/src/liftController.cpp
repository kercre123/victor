...
// How long the lift needs to stop moving for before it is considered to be
// limited.
...
const f32 LIFT_ANGLE_LOW_LIMIT_RAD =
    ConvertLiftHeightToLiftAngleRad(LIFT_HEIGHT_LOWDOCK);
const f32 LIFT_ANGLE_HIGH_LIMIT_RAD =
    ConvertLiftHeightToLiftAngleRad(LIFT_HEIGHT_CARRY);

// If the lift angle falls outside of the range defined by these thresholds, do
// not use D control. This is to prevent vibrating that tends to occur at the
// physical limits.
const f32 NO_D_TERM_LIFT_ANGLE_LOW_RANGE_MAX_RAD =
    LIFT_ANGLE_LOW_LIMIT_RAD + DEG_TO_RAD(5.f);
const f32 NO_D_TERM_LIFT_ANGLE_LOW_RANGE_MIN_RAD = LIFT_ANGLE_LOW_LIMIT_RAD;
const f32 NO_D_TERM_LIFT_ANGLE_HIGH_RANGE_MIN_RAD =
    LIFT_ANGLE_HIGH_LIMIT_RAD - DEG_TO_RAD(5.f);
...
// If it exceeds this value, applied power should decay to this value when in
// position. This value should be slightly less than the motor burnout
// protection threshold (POWER_THRESHOLD[]) in syscon's motors.cpp since the
// actual applied power can be slightly more than this.
...
// On the real robot, this is the angle between the lower lift joint on the
// robot body and the lower lift joint on the forklift assembly.
...
// Checking for cube on lift by lowering power and seeing if there's lift
// movement
...
#ifdef SIMULATOR
    // SetDesiredHeight might engage the gripper, but we don't want it engaged
    // right now.
...
bool IsCalibrating() { return calState_ != LCS_IDLE; }
...
void OnMotorCalibrated() {
  ...
  if (calibrationReason_ != MotorCalibrationReason::Startup) {
    DASMSG(lift_motor_calibrated, "lift_motor_calibrated",
...
void CalibrationUpdate() {
  if (!isCalibrated_) {
      ...
      case LCS_WAIT_FOR_STOP:
        ...
        if (!IsMoving()) {
          if (HAL::GetTimeStamp() - lastLiftMovedTime_ms > LIFT_STOP_TIME_MS) {
            // Turn off motor
            SetPower(
                0.f);  // Not strong enough to lift motor, but just enough to
                       // unwind backlash. Not sure if this is actually helping.
    ...
    if (IsCalibrating()) {
      ...
      if (currentAngle_rad_ - lowLiftAngleDuringCalib_rad_ >
          UPWARDS_LIFT_MOTION_FOR_CALIB_ABORT_RAD) {
        ...
        if (liftAngleHigherThanCalibAbortAngleCount_ >=
            UPWARDS_LIFT_MOTION_FOR_CALIB_ABORT_CNT) {
          if (firstCalibration_) {
            AnkiWarn("LiftController.CalibrationUpdate.RestartingCalib",
                     "Someone is probably messing with lift (low: %fdeg, curr: "
                     "%fdeg)",
                     RAD_TO_DEG(lowLiftAngleDuringCalib_rad_),
                     RAD_TO_DEG(currentAngle_rad_));
            calState_ = LCS_LOWER_LIFT;
          } else {
...
f32 GetHeightMM() { return ConvertLiftAngleToLiftHeightMM(currentAngle_rad_); }

f32 GetAngleRad() { return currentAngle_rad_; }
...
void PoseAndSpeedFilterUpdate() {
  ...
  AnkiDebug("LiftController",
            "LIFT FILT: speed %f, speedFilt %f, currentAngle %f, currHalPos "
            "%f, prevPos %f, pwr %f\n",
            measuredSpeed, radSpeed_, currentAngle_rad_,
            HAL::MotorGetPosition(MotorID::MOTOR_LIFT), prevHalPos_, power_);
#endif
  prevHalPos_ = currHalPos;
}

void SetDesiredAngle_internal(const f32 angle_rad, const f32 acc_start_frac,
                              const f32 acc_end_frac,
                              const f32 duration_seconds,
                              const f32 speed_rad_per_sec,
                              const f32 accel_rad_per_sec2, bool useVPG) {
  ...
  if (!HAL::IsGripperEngaged()) {
    ...
    if (newDesiredAngle < desiredAngle_rad_ &&
        (newDesiredAngle == MIN_LIFT_ANGLE ||
         newDesiredAngle ==
             ConvertLiftHeightToLiftAngleRad(LIFT_HEIGHT_HIGHDOCK))) {
      disengageGripperAtDest_ = true;
      disengageAtAngle_ = ConvertLiftHeightToLiftAngleRad(
          ConvertLiftAngleToLiftHeightMM(newDesiredAngle) +
          3.f * LIFT_FINGER_HEIGHT);
    } else {
  ...
  if (inPosition_ && (newDesiredAngle == desiredAngle_rad_) &&
  ...
  if (duration_seconds > 0) {
    res = vpg_.StartProfile_fixedDuration(
        ...
        MAX_LIFT_SPEED_RAD_PER_S, MAX_LIFT_ACCEL_RAD_PER_S2, duration_seconds,
        CONTROL_DT);
    ...
    if (!res) {
      AnkiInfo("LiftController.SetDesiredAngle.VPGFixedDurationFailed",
               ...
               desiredAngle_rad_, duration_seconds);
    }
  }
  if (!res) {
...
void SetDesiredHeight_internal(f32 height_mm, f32 acc_start_frac,
  ...
  SetDesiredAngle_internal(angle_rad, acc_start_frac, acc_end_frac,
                           duration_seconds, speed_rad_per_sec,
...
void SetDesiredAngleByDuration(f32 angle_rad, f32 acc_start_frac,
                               f32 acc_end_frac, f32 duration_seconds) {
  SetDesiredAngle_internal(angle_rad, acc_start_frac, acc_end_frac,
                           duration_seconds, MAX_LIFT_SPEED_RAD_PER_S,
                           MAX_LIFT_ACCEL_RAD_PER_S2, true);
}
...
void SetDesiredHeightByDuration(f32 height_mm, f32 acc_start_frac,
                                f32 acc_end_frac, f32 duration_seconds) {
  SetDesiredHeight_internal(height_mm, acc_start_frac, acc_end_frac,
                            duration_seconds, MAX_LIFT_SPEED_RAD_PER_S,
                            MAX_LIFT_ACCEL_RAD_PER_S2, true);
}

void SetDesiredHeight(f32 height_mm, f32 speed_rad_per_sec,
                      f32 accel_rad_per_sec2, bool useVPG) {
  SetDesiredHeight_internal(height_mm, DEFAULT_START_ACCEL_FRAC,
                            DEFAULT_END_ACCEL_FRAC, 0, speed_rad_per_sec,
                            accel_rad_per_sec2, useVPG);
}
...
// BURNOUT_TIME_THRESH_MS, stop it! If the lift was in position, assuming that
// someone is messing with the motor. If the lift was not in position, assuming
// that it's mis-calibrated and it's hitting the low or high hard limit. Do
// calibration. Returns true if a protection action was triggered.
  ...
  if (potentialBurnoutStartTime_ms_ == 0) {
    ...
    if (IsInPosition() || IMUFilter::IsBeingHeld() ||
      ...
      StartCalibrationRoutine(
          autoStarted, MotorCalibrationReason::LiftMotorBurnoutProtection);
    }
    return true;
  }
...
Result Update() {
  ...
  if (checkingForLoadStartTime_ > 0) {
    ...
    } else if (currentAngle_rad_ < checkingForLoadStartAngle_ -
                                       CHECKING_FOR_LOAD_ANGLE_DIFF_THRESH) {
      AnkiInfo("LiftController.Update.LoadDetected", "in %d ms",
               currTime - checkingForLoadStartTime_);
      checkForLoadWhenInPosition_ = false;
      checkingForLoadStartTime_ = 0;
      if (checkForLoadCallback_) {
  ...
  const bool inPiLowRange =
      (IN_RANGE(currentAngle_rad_, NO_D_TERM_LIFT_ANGLE_LOW_RANGE_MIN_RAD,
                NO_D_TERM_LIFT_ANGLE_LOW_RANGE_MAX_RAD) &&
       IN_RANGE(currDesiredAngle_rad_, NO_D_TERM_LIFT_ANGLE_LOW_RANGE_MIN_RAD,
                NO_D_TERM_LIFT_ANGLE_LOW_RANGE_MAX_RAD));
  const bool inPiHighRange =
      (IN_RANGE(currentAngle_rad_, NO_D_TERM_LIFT_ANGLE_HIGH_RANGE_MIN_RAD,
                NO_D_TERM_LIFT_ANGLE_HIGH_RANGE_MAX_RAD) &&
       IN_RANGE(currDesiredAngle_rad_, NO_D_TERM_LIFT_ANGLE_HIGH_RANGE_MIN_RAD,
                NO_D_TERM_LIFT_ANGLE_HIGH_RANGE_MAX_RAD));
  ...
  if ((ABS(angleError) < LIFT_ANGLE_TOL) &&
      (desiredAngle_rad_ == currDesiredAngle_rad_)) {
    ...
    if (ABS(power_) > maxPowerInPosition) {
      ...
      AnkiInfo("LiftController.Update.CheckingForLoad", "%d",
               checkingForLoadStartTime_);
      power_ = 0;
    }
    ...
    if (lastInPositionTime_ms_ == 0) {
      ...
      inPosition_ = true;
#if (DEBUG_LIFT_CONTROLLER)
      AnkiDebug("LiftController", " LIFT HEIGHT REACHED (%f mm)",
                GetHeightMM());
#endif
    }
  } else {
...
#if (DEBUG_LIFT_CONTROLLER)
  AnkiDebugPeriodic(50, "LiftController.Update.Values",
                    "LIFT: currA %f, curDesA %f, currVel %f, desA %f, err %f, "
                    "errSum %f, inPos %d",
                    currentAngle_rad_, currDesiredAngle_rad_, radSpeed_,
                    desiredAngle_rad_, angleError, angleErrorSum_,
                    inPosition_ ? 1 : 0);
  AnkiDebugPeriodic(
      50, "LiftController.Update.Power", "P: %f, I: %f, D: %f, total: %f",
      (Kp_ * angleError), (Ki_ * angleErrorSum_),
      (Kd_ * (angleError - prevAngleError_) * CONTROL_DT), power_);
#endif
...
void SetGains(const f32 kp, const f32 ki, const f32 kd,
  ...
  AnkiInfo("LiftController.SetGains",
           "New lift gains: kp = %f, ki = %f, kd = %f, maxSum = %f", Kp_, Ki_,
           Kd_, MAX_ERROR_SUM);
}
