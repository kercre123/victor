...
// Manage() must be called regularly regardless of which mode the controller is
// in. The default mode is SM_PATH_FOLLOW which uses the fidx passed in via
// Manage to determine steering. The other modes have a one-shot behavior and
// fall back to SM_PATH_FOLLOW when they complete.
typedef enum {
...
// left_accel: Acceleration with which to approach left_vel in mm/s^2. (Sign is
// ignored.) right_accel: Acceleration with which to approach right_vel in
// mm/s^2. (Sign is ignored.)
void ExecuteDirectDrive(f32 left_vel, f32 right_vel, f32 left_accel = 0,
                        f32 right_accel = 0);
...
// useShortestDir: If true, ignores the sign of maxAngularVel and takes the
// shortest path to targetAngle. numHalfRevolutions: The number of
// half-rotations the robot should perform. Used for relative turns of
//   angles greater than 180 degrees, for example. Ignored if useShortestDir is
//   true.
void ExecutePointTurn(f32 targetAngle, f32 maxAngularVel, f32 angularAccel,
                      f32 angularDecel, f32 angleTolerance,
                      bool useShortestDir = true,
                      uint16_t numHalfRevolutions = 0);
...
// Function to init steering controller when we expect to feed it a
// discontinuous follow line index so that it doesn't compare it to the previous
// follow line index.
void ReInit(void);
...
//                     If curvatureRadius_mm == 0, the speed is in rad/s where
//                     +ve means CCW rotation.
// accel:              Acceleration to approach target speed in mm/sec^2 (or
// rad/s^2 if curvatureRadius_mm == 0)
//                     0 == Max acceleration
