...
// Resets localization pose to (0,0,0) every time a relative block pose update
// is received. Recalculates the start pose of the path that is at the same
// position relative to the block as it was when tracking was initiated. If
// encoder-based localization is reasonably accurate, this probably won't be
// necessary.
#define RESET_LOC_ON_BLOCK_UPDATE 0

// Limits how quickly the dockPose angle can change per docking error message
// received. The purpose being to mitigate the error introduced by rolling
// shutter on marker pose.
// TODO: Eventually, accurate stamping of images with time and gyro data should
// make it
//       possible to do rolling shutter correction on the engine. Then we can
//       take this stuff out.
#define DOCK_ANGLE_DAMPING 0

// Whether or not to use Dubins path. If 0, just uses marker normal vector as
// approach path.
#define USE_DUBINS_PATH 0

// Whether or not to adjust the head angle to try to look towards the block
// based on our error signal
...
enum Mode { IDLE, LOOKING_FOR_BLOCK, APPROACH_FOR_DOCK };
...
// Whether or not to use only the first error signal received for docking
// purposes. Results in smoother, albeit less accurate, docking. Useful for
// charger docking since the marker pose estimation isn't great.
bool useFirstErrorSignalOnly_ = false;

TimeStamp_t lastDockingErrorSignalRecvdTime_ = 0;

// If error signal not received in this amount of time, tracking is considered
// to have failed.
const u32 STOPPED_TRACKING_TIMEOUT_MS = 1000;

// If an initial track cannot start for this amount of time, block is considered
// to be out of view and docking is aborted.
...
f32 LATERAL_DOCK_TOLERANCE_AT_DOCK_MM =
    BLOCK_ON_GROUND_LATERAL_DOCK_TOLERANCE_MM;
...
// The pose of the block as we're docking (note: this is actually the _marker_
// pose)
...
// the block is at least START_LIFT_TRACKING_DIST_MM close and
// START_LIFT_TRACKING_HEIGHT_MM high
...
// For Hybrid Docking: the previous block pose used to determine if the block or
// we have moved weirdly so we should acquire a new docking error signal and use
// it to plan a new path
...
// The docking speed for the final path segment right in front of the thing we
// are docking with Allows us to slow down when we are close to the dockPose
const f32 finalDockSpeed_mmps_ = 20;
#endif

// When hybrid docking the hanns maneuver tolerance for rel_x needs to be
// increased since we didn't push the block
const u8 HYBRID_REL_X_TOL_MM = 10;

// If the robot's pose relative to the dockPose is more than these tolerances
// off it is not in position and should backup and retry docking
...
// Tolerances for doing the Hanns Maneuver when we are checking our position
// with the last docking error message
const f32 ERRMSG_HM_ANGLE_TOL = DEG_TO_RAD_F32(16);
const u8 ERRMSG_HM_LATERAL_DOCK_TOL_MM = 18;
const u8 ERRMSG_HM_DIST_TOL = 15;

// Tolerances for doing the Hanns Maneuver when we are checking our position
// with our pose relative to dockPose
const u8 POSE_HM_DIST_TOL = 10;
const u8 POSE_HM_HORZ_TOL_MM = 10;
const f32 POSE_HM_ANGLE_TOL = DEG_TO_RAD_F32(10);

// Angle tolerances for checking whether or not we are in position based on what
// we are using to check pose
...
// If the block and cozmo are on the same plane the dock err signal z_height
// will be below this value Normally it is ~22mm
const u8 BLOCK_ON_GROUND_DOCK_ERR_Z_HEIGHT_MM = 25;

const u8 DOCK_OFFSET_DIST_X_OFFSET = 10;

// The different failure cases that can occur when cozmo is not in position
// during docking
enum FailureMode { NO_FAILURE, BACKING_UP, HANNS_MANEUVER };
...
bool IsHybridDocking() {
  return (dockingMethod_ == DockingMethod::HYBRID_DOCKING) ||
         (dockingMethod_ == DockingMethod::HYBRID_DOCKING_BEELINE);
}

bool IsBusy() { return (mode_ != IDLE); }
...
f32 GetVerticalFOV() {
  AnkiConditionalError(headCamFOV_ver_ != 0.f,
                       "DockingController.GetVerticalFOV.ZeroFOV", "");
  return headCamFOV_ver_;
}

f32 GetHorizontalFOV() {
  AnkiConditionalError(headCamFOV_hor_ != 0.f,
                       "DockingController.GetHorizontalFOV.ZeroFOV", "");
  return headCamFOV_hor_;
}

DockingResult GetDockingResult() { return dockingResult_; }
...
void SetMaxRetries(u8 numRetries) { maxDockRetries_ = numRetries; }
...
void ApplyDockingSteeringGains() {
  ...
  SteeringController::SetGains(DOCKING_K1, DOCKING_K2,
...
void RestoreSteeringGains() {
  if (appliedGains_) {
    SteeringController::SetGains(prevK1_, prevK2_, prevPathDistOffsetCap_,
                                 prevPathAngOffsetCap_);
    appliedGains_ = false;
  }
}
...
bool IsMarkerInFOV(const Anki::Embedded::Pose2d& markerPose,
                   const f32 markerWidth) {
  // Half fov of camera at center horizontal.
  // 0.36 radians is roughly the half-FOV of the camera bounded by the lift
  // posts.
  ...
  Radians angleToMarkerCenter =
      atan2_fast(markerCenterY - y, markerCenterX - x);

  // Compute angle to marker edges
  // (For now, assuming marker faces the robot.)
  // TODO: Use marker angle
  f32 distToMarkerCenter = sqrtf((markerCenterY - y) * (markerCenterY - y) +
                                 (markerCenterX - x) * (markerCenterX - x));
  Radians angleToMarkerLeftEdge =
      angleToMarkerCenter + atan2_fast(0.5f * markerWidth, distToMarkerCenter);
  Radians angleToMarkerRightEdge =
      angleToMarkerCenter - atan2_fast(0.5f * markerWidth, distToMarkerCenter);
...
// TODO: Should this be in some kinematics module where we have functions to get
// things wrt other things?
f32 GetCamFOVLowerHeight() {
  ...
  // TODO: This is really rough computation approximating with a fixed
  ...
  // z, angle, lowerCamFOVangle, liftH);

  return CLIP(liftH, LIFT_HEIGHT_LOWDOCK, LIFT_HEIGHT_CARRY);
}

// If docking to a high block, assumes we're trying to pick it up!
// Gradually lift block from a height of START_LIFT_HEIGHT_MM to
// LIFT_HEIGHT_HIGH_DOCK over the marker distance ranging from
// START_LIFT_TRACKING_DIST_MM to dockOffsetDistX_.
void HighDockLiftUpdate() {
  ...
  if (failureMode_ != BACKING_UP &&
    ...
    // If the height of the block is higher than our normal docking height (the
    // block is off the ground) then we are doing high docking so adjust our
    // lateral dock tolerance
    if (dockingErrSignalMsg_.z_height > BLOCK_ON_GROUND_DOCK_ERR_Z_HEIGHT_MM) {
      ...
      LATERAL_DOCK_TOLERANCE_AT_DOCK_MM =
          BLOCK_NOT_ON_GROUND_LATERAL_DOCK_TOLERACNCE_MM;
    } else {
    ...
    // The ending lift height is made smaller the heigher the block is so that
    // the lift doesn't rise above the block which it can if just using z_height
    const f32 liftApproachSlope =
        (dockingErrSignalMsg_.z_height -
         fabsf(dockingErrSignalMsg_.z_height -
               BLOCK_ON_GROUND_DOCK_ERR_Z_HEIGHT_MM) /
             10.f) /
        (START_LIFT_TRACKING_DIST_MM - dockOffsetDistX_);
    ...
    if (estDistToMarker < START_LIFT_TRACKING_DIST_MM) {
      ...
      f32 liftHeight =
          liftApproachSlope * (START_LIFT_TRACKING_DIST_MM - estDistToMarker);

      // Keep between current desired height and high dock height
      liftHeight =
          CLIP(liftHeight, lastCommandedHeight, LIFT_HEIGHT_HIGHDOCK_PICKUP);

      // Apply height
      LiftController::SetDesiredHeight(liftHeight);
#ifdef SIMULATOR
      // If we are picking up a block, engage the gripper now, so that we engage
      // with the block as soon as we encounter it. Otherwise, the block may
      // 'slide' to the side as we begin pushing it. Note: This is slightly more
      // realistic, since the real-life forks "hook into" the cube, preventing
      // lateral movement.
...
void SetCameraFieldOfView(f32 horizontalFOV, f32 verticalFOV) {
  AnkiDebug("DockingController.SetCameraFieldOfView.Values", "H: %f, V: %f",
...
Result Update() {
  // If we failed to dock and are now backing up ignore error signals until we
  // are DIST_TO_BACKUP_ON_FAILURE_MM away from our initial backing up pose
  if (failureMode_ == BACKING_UP) {
#if (TRACK_BLOCK)
    if (dockingErrSignalMsgReady_) {
      // Keep looking at marker while backing up
      f32 desiredHeadAngle =
          atan_fast((dockingErrSignalMsg_.z_height - NECK_JOINT_POSITION[2]) /
                    dockingErrSignalMsg_.x_distErr);
      HeadController::SetDesiredAngle(desiredHeadAngle);
    }
#endif

    if (Localization::GetDistTo(backupPose_.x(), backupPose_.y()) >
        DIST_TO_BACKUP_ON_FAILURE_MM) {
      failureMode_ = NO_FAILURE;

      // If we are doing hybrid docking then we want to backup when not in
      // position and then fail so if action retries we are close to predock
      // pose
      if (IsHybridDocking()) {
      ...
      // While we are backing up we don't want to time out due to ignoring error
      // signals so fake time
  ...
  while (dockingErrSignalMsgReady_) {
    ...
    // Note 10/9/2018 [MAM]: Commenting this out since it seems to trigger
    // spuriously and does not seem necessary as long as we assume the block is
    // not moving. If we were to re-enable this logic, it should be moved
    // somewhere after or within SetRelDockPose() so that the distance to the
    // updated block pose is used instead of the historical distance of the
    // docking error signal (which is from the image capture time). Check if we
    // are beyond point of no return distance
    //          if (pastPointOfNoReturn_) {
    //            // If we believe we are past the point of no return distance
    //            but are actually not if(dockingErrSignalMsg_.x_distErr >
    //            pointOfNoReturnDistMM_)
    //            {
    //#if(DEBUG_DOCK_CONTROLLER)
    //              AnkiInfo(
    //              "DockingController.Update.NoLongerPastPointOfNoReturn", "(%f
    //              > %d)",
    //                        dockingErrSignalMsg_.x_distErr,
    //                        pointOfNoReturnDistMM_);
    //#endif
...
#if (DEBUG_DOCK_CONTROLLER)
    AnkiInfo("DockingController.Update.ReceivedErrorSignal",
             "time=%d, x_distErr=%f, y_horErr=%f, z_height=%f, angleErr=%fdeg",
             dockingErrSignalMsg_.timestamp, dockingErrSignalMsg_.x_distErr,
             dockingErrSignalMsg_.y_horErr, dockingErrSignalMsg_.z_height,
             RAD_TO_DEG_F32(dockingErrSignalMsg_.angleErr));
#endif

    // Check that error signal is plausible
    // If not, treat as if tracking failed.
    // TODO: Get tracker to detect these situations and not even send the error
    // message here.
    if (dockingErrSignalMsg_.x_distErr > 0.f &&
        ABS(dockingErrSignalMsg_.angleErr) < 0.75f * M_PI_2_F) {
      ...
      SetRelDockPose(
          ...
          dockingErrSignalMsg_.angleErr, dockingErrSignalMsg_.timestamp);
      ...
      // When repeatedly commanding a low head angle head calibration likes to
      // kick in so this is to prevent that from happening
  ...
  if (markerOutOfFOV_) {
    ...
    if (lastMarkerPoseObservedIsSet_) {
      ...
      distToMarker = lastMarkerPoseObservedInValidFOV_.GetTranslation().Dist(
          currPose.GetTranslation());

      if (PathFollower::IsTraversingPath() &&
          !IsMarkerInFOV(lastMarkerPoseObservedInValidFOV_,
                         MARKER_WIDTH - 5.f) &&
  ...
  if (!pastPointOfNoReturn_ && (pointOfNoReturnDistMM_ != 0) &&
      (distToMarker < pointOfNoReturnDistMM_)) {
#if (DEBUG_DOCK_CONTROLLER)
    AnkiInfo("DockingController.Update.PointOfNoReturn", "(%f < %d) mode:%i",
             distToMarker, pointOfNoReturnDistMM_, mode_);
#endif
  ...
  // There are some special cases for aligning with a block (rolling is
  // basically aligning)
  const bool isAligning =
      PickAndPlaceController::GetCurAction() == DockAction::DA_ALIGN ||
  ...
  switch (mode_) {
    ...
    case LOOKING_FOR_BLOCK:
      ...
      if ((!pastPointOfNoReturn_) && !markerOutOfFOV_ &&
          (HAL::GetTimeStamp() - lastDockingErrorSignalRecvdTime_ >
           GIVEUP_DOCKING_TIMEOUT_MS)) {
        StopDocking(DockingResult::DOCK_FAILURE_TOO_LONG_WITHOUT_BLOCKPOSE);
        AnkiInfo(
            "DockingController.LOOKING_FOR_BLOCK.TooLongWithoutErrorSignal",
            "currTime %d, lastErrSignal %d, Giving up.", HAL::GetTimeStamp(),
            lastDockingErrorSignalRecvdTime_);
      }
      break;
    case APPROACH_FOR_DOCK: {
      ...
      if (dockingMethod_ != DockingMethod::EVEN_BLINDER_DOCKING) {
        ...
        if (!markerlessDocking_ && (!pastPointOfNoReturn_) &&
            (!useFirstErrorSignalOnly_) && !markerOutOfFOV_ &&
            (HAL::GetTimeStamp() - lastDockingErrorSignalRecvdTime_ >
             STOPPED_TRACKING_TIMEOUT_MS)) {
          PathFollower::ClearPath();
          SpeedController::SetUserCommandedDesiredVehicleSpeed(0);
          mode_ = LOOKING_FOR_BLOCK;
          AnkiInfo(
              "DockingController.APPROACH_FOR_DOCK.TooLongWithoutErrorSignal",
      ...
      // If we did the Hanns maneuver assume it succedded since we should only
      // be doing when we believe it will work
      if (failureMode_ == HANNS_MANEUVER && createdValidPath_ &&
          !PathFollower::IsTraversingPath()) {
        StopDocking(DockingResult::DOCK_SUCCESS_HANNS_MANEUVER);
        break;
      }

      // If this is DA_BACKUP_ONTO_CHARGER*, don't bother with all the checks
      // below and just assume we got to where we want. (Charger marker pose
      // signal is too noisy to trust. We just need to get roughly in front of
      // the thing so we can turn around to back into it.
      if (createdValidPath_ && !PathFollower::IsTraversingPath() &&
          (PickAndPlaceController::GetCurAction() ==
      ...
      if (createdValidPath_ && !PathFollower::IsTraversingPath() &&
        ...
        if (dockingMethod_ != DockingMethod::BLIND_DOCKING &&
            dockingMethod_ != DockingMethod::EVEN_BLINDER_DOCKING) {
          f32 rel_angle_to_block = relPose.GetAngle().ToFloat();

          // Depending on when our last docking error signal was we will either
          // use it or the robot's relative distances to the dock pose to
          // determine if we are in position. Note: we only do these checks if
          // we are not beelining, since the beeline path may be angled a bit,
          // causing these tolerances to be inappropriate.
          const bool isBeelineDocking =
              (dockingMethod_ == DockingMethod::HYBRID_DOCKING_BEELINE);
          if (!markerlessDocking_ && !isBeelineDocking) {
            // Since hybrid docking doesn't plan into the block the error signal
            // will be slightly larger when we finish traversing the path so
            // increase the x_dist_err tolerance
            f32 rel_x_tol_mm = (dockingMethod_ == DockingMethod::HYBRID_DOCKING
                                    ? HYBRID_REL_X_TOL_MM
                                    : 0);
            if ((HAL::GetTimeStamp() - dockingErrSignalMsg_.timestamp) <=
                    TIME_SINCE_LAST_ERRSIG &&
                (dockingErrSignalMsg_.x_distErr >
                     pointOfNoReturnDistMM_ + rel_x_tol_mm ||
                 ABS(dockingErrSignalMsg_.y_horErr) >
                     LATERAL_DOCK_TOLERANCE_AT_DOCK_MM ||
                 ABS(dockingErrSignalMsg_.angleErr) >
                     ERRMSG_NOT_IN_POSITION_ANGLE_TOL)) {
              AnkiInfo(
                  "DockingController.Update.FinishedPathButErrorSignalTooLarge_"
                  "1",
                  "x: %f, y: %f, angle: %f", dockingErrSignalMsg_.x_distErr,
                  dockingErrSignalMsg_.y_horErr, dockingErrSignalMsg_.angleErr);
              inPosition = false;

              // If we aren't in position but are relatively close then we want
              // to do the Hanns maneuver The x_distErr can be fairly large due
              // it not being that recent so reduce it by a ratio of how long
              // ago the error was and the x_distErr at that time
              if (dockingErrSignalMsg_.x_distErr -
                          ((HAL::GetTimeStamp() -
            ...
            // If we can't use the last docking error signal then check if the
            // robot's pose is within some tolerances of the dockPose
            else if ((HAL::GetTimeStamp() - dockingErrSignalMsg_.timestamp) >
                         TIME_SINCE_LAST_ERRSIG &&
                     (rel_vert_dist_block > VERT_DOCK_TOL_MM ||
                      ABS(rel_horz_dist_block) > HORZ_DOCK_TOL_MM ||
                      ABS(rel_angle_to_block) >
                          POSE_NOT_IN_POSITION_ANGLE_TOL)) {
              AnkiInfo(
                  "DockingController.Update.FinishedPathButErrorSignalTooLarge_"
                  "2",
                  "ver: %f, hor: %f, angle: %f", rel_vert_dist_block,
                  rel_horz_dist_block, rel_angle_to_block);
              inPosition = false;
        ...
        // If we know we are not in position and we are not currently backing up
        // due to an already recognized failure then fail this dock and either
        // backup or do Hanns maneuver. We are only able to fail docking this
        // way if the docking involves markers
        if (!markerlessDocking_ && !inPosition) {
          if (dockingMethod_ != DockingMethod::BLIND_DOCKING &&
              dockingMethod_ != DockingMethod::EVEN_BLINDER_DOCKING) {
            // If we have determined we should do the Hanns maneuver and we
            // aren't currently doing it, are docking to a block on the ground,
            // and not carrying a block
            if (doHannsManeuver && failureMode_ != HANNS_MANEUVER &&
                dockingToBlockOnGround &&
                !PickAndPlaceController::IsCarryingBlock() && !isAligning) {
              SendDockingStatusMessage(Status::STATUS_DOING_HANNS_MANEUVER);
              AnkiInfo("DockingController.Update.ExecutingHannsManeuver", "");

              // Hanns Maneuver was accidentally tuned to work with specific
              // gains so apply them
              ...
              f32 turnAngle = 0;
              // Depending on which source of information we used to determine
              // we are not in position use the same one to get the turn angle
              if ((HAL::GetTimeStamp() - dockingErrSignalMsg_.timestamp) <=
                  TIME_SINCE_LAST_ERRSIG) {
                turnAngle =
                    (dockingErrSignalMsg_.y_horErr >= 0 ? -HM_TURN_ANGLE_RAD
                                                        : HM_TURN_ANGLE_RAD);
              } else {
                turnAngle = (rel_horz_dist_block >= 0 ? -HM_TURN_ANGLE_RAD
                                                      : HM_TURN_ANGLE_RAD);
              }

              PathFollower::AppendPathSegment_PointTurn(
                  x, y, a.ToFloat(), a.ToFloat() - turnAngle,
                  HM_ROT_SPEED_RADPS, HM_ROT_ACCEL_MMPS2, HM_ROT_ACCEL_MMPS2,
                  0.1f, true);

              PathFollower::AppendPathSegment_Line(
                  x, y, x + (HM_DIST_MM)*cosf(a.ToFloat() - turnAngle / 2.f),
                  y + (HM_DIST_MM)*sinf(a.ToFloat() - turnAngle / 2.f),
                  HM_SPEED_MMPS, HM_ACCEL_MMPS2, HM_ACCEL_MMPS2);
            ...
            // will occur if we are in position for hanns maneuver then we won't
            // do it and just succeed
            else if (doHannsManeuver &&
                     (isAligning || !dockingToBlockOnGround)) {
              StopDocking(DockingResult::DOCK_SUCCESS);
            }
            // Otherwise we are not in position and should just back up
              ...
              if (numDockingFails_++ >= maxDockRetries_) {
                StopDocking(DockingResult::DOCK_FAILURE_RETRY);
                AnkiInfo("DockingController.Update.TooManyDockingFails",
                         "MaxRetries %d", maxDockRetries_);
                break;
              }
              ...
              SteeringController::ExecuteDirectDrive(BACKUP_SPEED_MMPS,
                                                     BACKUP_SPEED_MMPS);
              dockSpeed_mmps_ = DOCKSPEED_ON_FAILURE_MMPS;
              dockDecel_mmps2_ = DOCKDECEL_ON_FAILURE_MMPS2;

              // Raise the lift if rolling, don't do anything to the lift if
              // aligning or carrying a block, everything else we want to lower
              // the lift when backing up
              if (PickAndPlaceController::GetCurAction() ==
                      DockAction::DA_ROLL_LOW ||
                  PickAndPlaceController::GetCurAction() ==
                      DockAction::DA_DEEP_ROLL_LOW) {
                LiftController::SetDesiredHeight(LIFT_HEIGHT_CARRY);
              } else if (!isAligning &&
                         !PickAndPlaceController::IsCarryingBlock() &&
                         PickAndPlaceController::GetCurAction() !=
                             DockAction::DA_ALIGN_SPECIAL) {
                LiftController::SetDesiredHeightByDuration(LIFT_HEIGHT_LOWDOCK,
                                                           0.25, 0.25, 1);
              }
              dockingToBlockOnGround = true;
        ...
        // Otherwise we are in position and docking can succeed
          ...
          AnkiInfo("DockingController.Update.Success",
                   "vert:%f horz:%f rel_x:%f rel_y:%f t:%d",
                   rel_vert_dist_block, ABS(rel_horz_dist_block),
                   dockingErrSignalMsg_.x_distErr,
...
void SetRelDockPose(f32 rel_x, f32 rel_y, f32 rel_rad, TimeStamp_t t) {
  ...
  if (rel_x < 0.f || ABS(rel_rad) > 0.75f * M_PI_2_F) {
    AnkiWarn("DockingController.SetRelDockPose.OORErrorSignal",
             "x: %f, y: %f, rad: %f", rel_x, rel_y, rel_rad);
    return;
  }
  ...
  // Ignore error signals while doing the Hanns Manuever so that we don't try to
  // acquire a new error signal because the block is moving
  ...
  if (mode_ == LOOKING_FOR_BLOCK) {
    ...
    Localization::GetDriveCenterPose(approachStartPose_.x(),
                                     approachStartPose_.y(),
                                     approachStartPose_.angle());
    ...
    // PRINT("Approach start delta: distToBlock: %f, angleToBlock: %f,
    // blockAngleRelToRobot: %f\n", approachPath_dist,approachPath_dtheta,
    // approachPath_dOrientation);
#endif

    followingBlockNormalPath_ = false;
  }

  // DOCK_OFFSET_DIST_X_OFFSET is added to the dockOffsetDistX_ here because
  // when the robot is docked with the block rel_x is generally greater than
  // dockOffsetDistX_
  if (rel_x <= dockOffsetDistX_ + DOCK_OFFSET_DIST_X_OFFSET &&
      ABS(rel_y) <= LATERAL_DOCK_TOLERANCE_AT_DOCK_MM) {
#if (DEBUG_DOCK_CONTROLLER)
    AnkiInfo("DockingController.SetRelDockPose.DockPoseReached",
             "dockOffsetDistX = %f",
             dockOffsetDistX_ + DOCK_OFFSET_DIST_X_OFFSET);
#endif
    return;
  }

  // This error signal is with respect to the pose the robot was at at time
  // dockMsg.timestamp Find the pose the robot was at at that time and transform
  // it to be with respect to the robot's current pose
  ...
  AnkiInfo("DockingController.SetRelDockPose",
           "HistPose %f %f %f (t=%d), currPose %f %f %f (t=%d)", histPose.x(),
           histPose.y(), histPose.angle().getDegrees(), t, currPose.x(),
           currPose.y(), currPose.angle().getDegrees(), HAL::GetTimeStamp());
#endif

  // Compute absolute block pose using error relative to pose at the time image
  // was taken.
  f32 distToBlock = sqrtf((rel_x * rel_x) + (rel_y * rel_y));
  f32 angle_to_block = atan2_acc(rel_y, rel_x);
  f32 blockPose_x =
      histPose.x() +
      distToBlock * cosf(angle_to_block + histPose.GetAngle().ToFloat());
  f32 blockPose_y =
      histPose.y() +
      distToBlock * sinf(angle_to_block + histPose.GetAngle().ToFloat());
  ...
  if (IsHybridDocking() && prev_blockPose_x_ != -1) {
    if (fabsf(prev_blockPose_x_ - blockPose_x) > DELTA_BLOCKPOSE_X_TOL_MM ||
      ...
      AnkiInfo("DockingController.SetRelDockPose.AcquireNewSignal", "%f %f %f",
               prev_blockPose_x_ - blockPose_x, prev_blockPose_y_ - blockPose_y,
  ...
  // Ignore error signal if blind docking or hybird docking and we dont need to
  // acquire a new signal
  if (PathFollower::IsTraversingPath() &&
      (dockingMethod_ == DockingMethod::BLIND_DOCKING ||
       dockingMethod_ == DockingMethod::EVEN_BLINDER_DOCKING ||
       (IsHybridDocking() &&
        numErrSigToAcquireNewSignal_ < CONSEC_ERRSIG_TO_ACQUIRE_NEW_SIGNAL))) {
    return;
  }
  numErrSigToAcquireNewSignal_ = 0;

  // Create new path that is aligned with the normal of the block we want to
  // dock to. End point: Where the robot origin should be by the time the robot
  // has docked. Start point: Projected from end point at specified rad.
  ...
  if (markerOutOfFOV_) {
    ...
    if (IsMarkerInFOV(blockPose_, MARKER_WIDTH)) {
      AnkiInfo("DockingController.SetRelDockPose.MarkerInsideFOV", "");
      markerOutOfFOV_ = false;
    } else {
      AnkiInfo("DockingController",
  ...
  // approachStartPose_.x(), approachStartPose_.y(),
  // approachStartPose_.angle.ToFloat());
#endif

  // Compute dock pose
  dockPose_.x() =
      blockPose_.x() - dockOffsetDistX_ * cosf(blockPose_.GetAngle().ToFloat());
  dockPose_.y() =
      blockPose_.y() - dockOffsetDistX_ * sinf(blockPose_.GetAngle().ToFloat());
#if (DOCK_ANGLE_DAMPING)
  ...
  do {
    ...
    if (++endRadiiIdx == NUM_END_RADII) {
      AnkiInfo("DockingController.SetRelDockPose.DubinsPathFailed", "");
      followingBlockNormalPath_ = true;
      break;
    }

    numPathSegments = PathFollower::GenerateDubinsPath(
        x, y, a.ToFloat(), dockPose_.x(), dockPose_.y(),
        dockPose_.GetAngle().ToFloat(), DOCK_PATH_START_RADIUS_MM,
        DOCK_PATH_END_RADII_MM[endRadiiIdx], dockSpeed_mmps_, dockAccel_mmps2_,
        dockDecel_mmps2_, FINAL_APPROACH_STRAIGHT_SEGMENT_LENGTH_MM,
        &path_length);
    ...
    for (u8 s = 0; s < numPathSegments; ++s) {
      if (path.GetSegmentConstRef(s).GetType() == Planning::PST_ARC) {
        f32 segSweepRad =
            fabsf(path.GetSegmentConstRef(s).GetDef().arc.sweepRad);
  ...
  // PRINT("numPathSegments: %d, path_length: %f, distToBlock: %f,
  // followBlockNormalPath: %d\n",
  //      numPathSegments, path_length, distToBlock, followingBlockNormalPath_);

#else
  // Skipping Dubins path since the straight line path seems to work fine as
  // long as the steeringController gains are set appropriately according to
  // docking speed.
  followingBlockNormalPath_ = true;
#endif
  ...
  if (followingBlockNormalPath_) {
...
#endif
    {
      ...
      f32 x_end_mm = 0.f;
      f32 y_end_mm = 0.f;
      // Plan just a little bit into the block to reduce the chance that we
      // don't quite get close enough and completely whiff picking up the
      // object. If we are rolling, push the block by the configured amount.
      const DockAction curAction = PickAndPlaceController::GetCurAction();
      const bool isRolling = (curAction == DockAction::DA_ROLL_LOW) ||
                             (curAction == DockAction::DA_DEEP_ROLL_LOW);
      f32 distIntoBlock_mm = isRolling ? PATH_END_DIST_INTO_BLOCK_MM : 5;
      if (dockingMethod_ == DockingMethod::HYBRID_DOCKING_BEELINE) {
        // If we are beeline docking, we should just draw a line from the
        // robot's pose to the block pose, and follow that line.
        const f32 absAngleToBlock =
            atan2_fast(blockPose_.y() - Localization::GetCurrPose_y(),
                       blockPose_.x() - Localization::GetCurrPose_x());
        const f32 absAngleToBlockCos = cosf(absAngleToBlock);
        const f32 absAngleToBlockSin = sinf(absAngleToBlock);

        // Note: We start the path behind the robot a bit to ensure that it is
        // _before_ the end point.
        x_start_mm = Localization::GetCurrPose_x() -
                     3 * distToBlock * absAngleToBlockCos;
        y_start_mm = Localization::GetCurrPose_y() -
                     3 * distToBlock * absAngleToBlockSin;

        // If the block is rotated, we should drive further into it to ensure
        // that the lift fully engages. Note: The 20 corresponds to roughly half
        // the width of the block
        const Radians angleDifference = absAngleToBlock - blockPose_.angle();
        distIntoBlock_mm +=
            20 * sinf(angleDifference.getAbsoluteVal().ToFloat());

        // Path follower expects a drive center pose, so take that into account
        // here
        f32 endDistanceOffset_mm = distIntoBlock_mm - dockOffsetDistX_ -
                                   ABS(Localization::GetDriveCenterOffset());

        x_end_mm = blockPose_.x() + endDistanceOffset_mm * absAngleToBlockCos;
        y_end_mm = blockPose_.y() + endDistanceOffset_mm * absAngleToBlockSin;
      } else {
        // Compute new starting point for path
        // HACK: Feeling lazy, just multiplying path by some scalar so that it's
        // likely to be behind the current robot pose.
      ...
      PathFollower::ClearPath();
      PathFollower::AppendPathSegment_Line(x_start_mm, y_start_mm, x_end_mm,
                                           y_end_mm, dockSpeed_mmps_,
                                           dockAccel_mmps2_, dockAccel_mmps2_);

      AnkiInfo("DockingController",
               "Computing straight line path (%f, %f) to (%f, %f)\n",
               x_start_mm, y_start_mm, x_end_mm, y_end_mm);
    }
#if ALLOW_TRACKER_DOCKING
    else {
      // Distance it takes in order to decelerate from dockSpeed to 0 with the
      // given dock deceleration This is for the final path segment so we get a
      // nice smooth ease-in when close to the block
      f32 distToDecel =
          (-(dockSpeed_mmps_ * dockSpeed_mmps_)) / (-2 * dockDecel_mmps2_);

      // If distToDecel is larger than our max decel distance calculate a new
      // deceleration such that distToDecel will be equivalent to our max decel
      // distance
      if (distToDecel > MAX_DECEL_DIST_MM) {
        dockDecel_mmps2_ =
            (-(dockSpeed_mmps_ * dockSpeed_mmps_)) / (-2 * MAX_DECEL_DIST_MM);
        distToDecel = MAX_DECEL_DIST_MM;
      }

      // If we need to do a point turn in order to turn towards the block than
      // this padding will ensure we are DIST_AWAY_FROM_BLOCK_FOR_PT_MM away
      // from the block when we turn. This is so that we don't turn and collide
      // with the block
      f32 distToDecelPadding = 0;
      if (distToDecel < DIST_AWAY_FROM_BLOCK_FOR_PT_MM &&
          distToBlock > DIST_AWAY_FROM_BLOCK_FOR_PT_MM) {
      ...
      f32 x_inFrontOfBlock =
          dockPose_.x() - (distToDecelPadding + distToDecel) * dockPoseAngleCos;
      f32 y_inFrontOfBlock =
          dockPose_.y() - (distToDecelPadding + distToDecel) * dockPoseAngleSin;
      ...
      // If we are more than DIST_AWAY_FROM_BLOCK_FOR_ADDITIONAL_PATH_MM away
      // from the block or the blocks angle reletive to robot is greater than
      // ANGLE_FROM_BLOCK_FOR_ADDITIONAL_PATH_RAD plan a path to the point
      // DIST_AWAY_FROM_BLOCK_FOR_PT_MM directly in front of the block
      if (distToBlock > DIST_AWAY_FROM_BLOCK_FOR_ADDITIONAL_PATH_MM ||
          ABS(rel_rad) > ANGLE_FROM_BLOCK_FOR_ADDITIONAL_PATH_RAD) {
        PathFollower::AppendPathSegment_Line(
            0, x, y, x_inFrontOfBlock, y_inFrontOfBlock, dockSpeed_mmps_,
            dockAccel_mmps2_, dockDecel_mmps2_);

        // If the angle of the block relative to the robot is greater than
        // ANGLE_FROM_BLOCK_FOR_POINT_TURN_RAD we will want to do a point turn
        // in order face the block
        if (ABS(rel_rad) > ANGLE_FROM_BLOCK_FOR_POINT_TURN_RAD) {
          PathFollower::AppendPathSegment_PointTurn(
              0, x_inFrontOfBlock, y_inFrontOfBlock,
              dockPose_.GetAngle().ToFloat(), 3.0, 1.0, 1.0, 0.1, true);
        }

        // If we added padding to place us 60mm away from the block then we need
        // to add a path segment to get us from that point to the point where we
        // will start to decelerate otherwise we will already be at the point of
        // deceleration
        if (distToDecelPadding > 0) {
          PathFollower::AppendPathSegment_Line(
              0, x_inFrontOfBlock, y_inFrontOfBlock,
              dockPose_.x() - (distToDecel)*dockPoseAngleCos,
              dockPose_.y() - (distToDecel)*dockPoseAngleSin, dockSpeed_mmps_,
              dockAccel_mmps2_, dockDecel_mmps2_);
        }
      }
      // Else we are close enough and roughly directly in front of the block
      // that we can just go straight to the deceleration point
      else if (distToBlock > distToDecel) {
        PathFollower::AppendPathSegment_Line(
            0, x, y, dockPose_.x() - (distToDecel)*dockPoseAngleCos,
            dockPose_.y() - (distToDecel)*dockPoseAngleSin, dockSpeed_mmps_,
            dockAccel_mmps2_, dockDecel_mmps2_);
      }

      // Finally add a segment from our deceleration point to a point
      // PATH_END_DIST_INTO_BLOCK_MM (or if docking to a block off the ground
      // 0mm) inside of the block Placing the point inside the block causes us
      // to push the block and maybe it will slide sideways onto the lift if we
      // are ever so slightly off the marker due to camera extrinsics
      f32 distIntoBlock = ((!dockingToBlockOnGround ||
                            PickAndPlaceController::IsCarryingBlock() ||
                            PickAndPlaceController::GetCurAction() == DA_ALIGN)
                               ? 0
                               : PATH_END_DIST_INTO_BLOCK_MM);

      PathFollower::AppendPathSegment_Line(
          0, dockPose_.x() - (distToDecel)*dockPoseAngleCos,
          ...
          finalDockSpeed_mmps_, dockAccel_mmps2_, dockDecel_mmps2_);
    }
#endif
  }
...
void StartDocking_internal(const f32 speed_mmps, const f32 accel_mmps,
                           const f32 decel_mmps) {
  dockingStarted = true;
  ...
  // Change gains for tracker docking to smooth pathfollowing due to constantly
  // changing path
...
void StartDocking(const f32 speed_mmps, const f32 accel_mmps,
                  const f32 decel_mmps, const f32 dockOffsetDistX,
                  const f32 dockOffsetDistY, const f32 dockOffsetAngle,
...
void StartDockingToRelPose(const f32 speed_mmps, const f32 accel_mmps,
                           const f32 decel_mmps, const f32 rel_x,
                           const f32 rel_y, const f32 rel_angle) {
  StartDocking_internal(speed_mmps, accel_mmps, decel_mmps);
...
const Anki::Embedded::Pose2d& GetLastMarkerAbsPose() { return blockPose_; }
