...
#define GET_RED(color)                                  \
  ((color & EnumToUnderlyingType(LEDColor::LED_RED)) >> \
   EnumToUnderlyingType(LEDColorShift::LED_RED_SHIFT))
#define GET_GRN(color)                                    \
  ((color & EnumToUnderlyingType(LEDColor::LED_GREEN)) >> \
   EnumToUnderlyingType(LEDColorShift::LED_GRN_SHIFT))
#define GET_BLU(color)                                   \
  ((color & EnumToUnderlyingType(LEDColor::LED_BLUE)) >> \
   EnumToUnderlyingType(LEDColorShift::LED_BLU_SHIFT))

namespace Anki {
namespace Vector {
inline u32 AlphaBlend(const u32 onColor, const u32 offColor,
                      const float alpha) {
  ...
  return (int(onRed * alpha + offRed * invAlpha))
             << EnumToUnderlyingType(LEDColorShift::LED_RED_SHIFT) |
         (int(onGrn * alpha + offGrn * invAlpha))
             << EnumToUnderlyingType(LEDColorShift::LED_GRN_SHIFT) |
         (int(onBlu * alpha + offBlu * invAlpha))
             << EnumToUnderlyingType(LEDColorShift::LED_BLU_SHIFT);
}

u32 GetCurrentLEDcolor(const LightState& ledParams,
                       const TimeStamp_t currentTime,
                       const TimeStamp_t phaseTime, const u32 msPerFrame) {
  AnkiAssert(currentTime >= phaseTime,
             "LedController.GetCurrentLEDcolor.InvalidPhaseTime");
  ...
  const u32 totalTime_ms =
      ledParams.transitionOnPeriod_ms + ledParams.onPeriod_ms +
      ledParams.transitionOffPeriod_ms + ledParams.offPeriod_ms;
  ...
  if (phaseTime_ms < ledParams.transitionOnPeriod_ms) {
    ...
    newColor = AlphaBlend(
        ledParams.onColor, ledParams.offColor,
        float(phaseTime_ms) / float(ledParams.transitionOnPeriod_ms));
  } else if (phaseTime_ms <
             (ledParams.transitionOnPeriod_ms + ledParams.onPeriod_ms)) {
    // In the "on" period
    ...
    const u16 offPhase = phaseTime_ms - (ledParams.transitionOnPeriod_ms +
                                         ledParams.onPeriod_ms);
    newColor =
        AlphaBlend(ledParams.offColor, ledParams.onColor,
                   float(offPhase) / float(ledParams.transitionOffPeriod_ms));
  } else {
    // In the "off" period
