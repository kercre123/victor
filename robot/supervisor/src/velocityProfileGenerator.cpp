...
VelocityProfileGenerator::VelocityProfileGenerator() { targetReached_ = true; }

void VelocityProfileGenerator::StartProfile(float startVel, float startPos,
                                            float endVel, float accel,
                                            float timeStep) {
  noTargetMode_ = true;
  ...
  deltaVelPerTimeStepStart_ =
      accel_ * timeStep_ * (endVel_ - startVel_ > 0 ? 1.f : -1.f);

  targetReached_ = false;

  // Set values that don't really apply in noTargetMode to 0, even though 0 is
  // wrong, but at least they're not random values from the last time
  // StartProfile was called.
...
void VelocityProfileGenerator::StartProfile(float startVel, float startPos,
  ...
  float d = ((2 * maxVel_ * maxVel_) - (endVel_ * endVel_) -
             (startVel_ * startVel_)) /
            (2 * accel_);
  ...
  if (fabsf(d) > fabsf(totalDistToTarget_)) {
    maxReachableVel_ = sqrtf(((2 * accel * fabsf(totalDistToTarget_)) +
                              (endVel_ * endVel_) + (startVel_ * startVel_)) *
                             0.5f);
  ...
  // Compute distance required to decelerate from maxReachableVel_ (Vs) to
  // endVel_ (Vf). d = 1/2 a*t^2 + Vs*t , where t=(Vf-Vs)/a But a is actually
  // negative because we're decelering so t=(Vs-Vf)/a and d = 1/2 * -a * ((Vs -
  // Vf)/a)^2 + Vs * (Vs - Vf)/a d = (Vs*Vs - Vf*Vf)/(2a)
  decelDistToTarget_ = 0;
  if (fabsf(maxReachableVel_) > fabsf(endVel_)) {
    decelDistToTarget_ =
        (maxReachableVel_ * maxReachableVel_ - endVel_ * endVel_) /
        (2 * accel_);
  }
  ...
  startAccelDist_ = fabsf(0.5f * (maxReachableVel_ - startVel_) /
                          deltaVelPerTimeStepStart_ * timeStep_) *
                    (maxReachableVel_ >= 0 ? 1 : -1);
  endAccelDist_ = fabsf(0.5f * (endVel_ - maxReachableVel_) /
                        deltaVelPerTimeStepEnd_ * timeStep_) *
                  (maxReachableVel_ >= 0 ? 1 : -1);

#if (DEBUG_VPG)
  AnkiDebug("VPG", "startVel %f, startPos %f, maxSpeed %f, accel %f", startVel_,
            startPos_, maxVel_, accel_);
  AnkiDebug("VPG", "endVel %f, endPos %f, timestep %f", endVel_, endPos_,
            timeStep_);
  AnkiDebug("VPG",
            "deltaVel %f, maxReachableVel %f, totalDist %f, decelDistToTarget "
            "%f, dir %f",
            deltaVelPerTimeStepStart_, maxReachableVel_, totalDistToTarget_,
            decelDistToTarget_, direction);
#endif
}

/*
  // sign of acc_start and acc_end matter!
  bool VelocityProfileGenerator::StartProfile_fixedDuration(float pos_start,
  float vel_start, float acc_start, float pos_end, float vel_end, float acc_end,
                                                            float duration,
                                                            float timeStep)
  {
    ...
    //
    // and that for the case where vel_start, vel_mid, and vel_end all have the
  same sign
    ...
    float C = vel_start*b + 0.5*acc_start*b*b + vel_end*duration - vel_end*b -
  dist;
...
// There are 3 types of profiles, and their mirror images across the time axis,
// that this can generate:
//
// 1) vel_start and vel_mid are of the same sign, vel_mid is of greater
// magnitude than vel_start. 2) vel_start and vel_mid are of the same sign,
// vel_mid is of lesser magnitude than vel_start. 3) vel_start and vel_mid are
// of opposite signs.
bool VelocityProfileGenerator::StartProfile_fixedDuration(
    float pos_start, float vel_start, float acc_start_duration, float pos_end,
    float acc_end_duration, float vel_max, float acc_max, float duration,
    float timeStep) {
  noTargetMode_ = false;
  ...
  AnkiConditionalWarnAndReturnValue(
      (acc_start_duration + acc_end_duration <= duration), false,
      "VPG.StartProfile_fixedDuration.AccDurationsExceedTotal",
      "acc_start_duration + acc_end_duration exceeds total duration (%f + %f > "
      "%f)",
  ...
  // Figure out if vel_mid is +ve or -ve by setting it to 0 and seeing what
  // distance is covered
  float distWhenVelMid0 = 0.5f * (vel_start * acc_start_duration);
  bool isVelMidGT0 = (dist >= distWhenVelMid0);

  // If accelerating to a vel_mid of 0 for the acc_start phase actually passes
  // the end_pos, then return false as this is probably not an intended profile.
  if (((dist > 0) == (distWhenVelMid0 > 0)) &&
      (fabsf(distWhenVelMid0) > fabsf(dist))) {
    // end_pos reached during starting acc phase. Consider reducing acc_start
    // duration or decreasing vel_start magnitude

    // TODO: Consider shorter acceleration/deceleration periods that could make
    // this profile work?
  ...
  float distWhenVelMidIsVelStart = vel_start * (duration - acc_end_duration) +
                                   0.5f * (vel_start * acc_end_duration);
  ...
  if (accStartCrosses0) {
    ...
    AnkiConditionalWarnAndReturnValue(
        (discr >= 0), false,
        "VPG.StartProfile_fixedDuration.NegativeDiscriminant",
        "A = %f, B = %f, C = %f", A, B, C);
    ...
    AnkiConditionalWarnAndReturnValue(
        vm <= 0, false, "VPG.StartProfile_fixedDuration.NegativeVm",
        "vm > 0  (A = %f, B = %f, C = %f)", A, B, C);
  ...
  AnkiConditionalWarnAndReturnValue(
      !((fabsf(vs) > vel_max) || (fabsf(vm) > vel_max)), false,
      "VPG.StartProfile_fixedDuration.VelExceedsMax",
      "vs = %f, vm = %f, vel_max = %f", vs, vm, vel_max);
  ...
  if ((acc_start > acc_max) || (acc_end > acc_max)) {
    AnkiWarn("VPG.StartProfile_fixedDuration.AccelMaxExceeded",
             "acc_start = %f, acc_end = %f, acc_max = %f", acc_start, acc_end,
             acc_max);
    return false;
  }
  ...
  startAccelDist_ = fabsf(0.5f * (maxReachableVel_ - startVel_) * ts) *
                    (maxReachableVel_ >= 0 ? 1 : -1);
  endAccelDist_ = fabsf(0.5f * (endVel_ - maxReachableVel_) * te) *
                  (maxReachableVel_ >= 0 ? 1 : -1);

#if (DEBUG_VPG)
  AnkiDebug("VPG", "startVel %f, startPos %f, endVel %f, endPos %f", startVel_,
            startPos_, endVel_, endPos_);
  AnkiDebug("VPG", "ts %f, tm %f, te %f, total duration %f", ts, tm, te,
            duration);
  AnkiDebug("VPG", "deltaVelStart %f, deltaVelEnd %f, maxReachableVel %f",
            deltaVelPerTimeStepStart_, deltaVelPerTimeStepEnd_,
            maxReachableVel_);
  AnkiDebug("VPG", "totalDist %f, decelDistToTarget %f", totalDistToTarget_,
            decelDistToTarget_);
#endif
...
float VelocityProfileGenerator::Step(float &currVel, float &currPos) {
  ...
  if (isDecel_) {
    ...
    if (currVel_ != endVel_) {
      ...
      const bool currVelHasPassedEndVelFromStartVel =
          (maxReachableVel_ == endVel_) &&
          ((currVel_ > endVel_) == (currVel_ > startVel_));

      // If maxReachableVel != endVel, currVel approaches endVel from
      // maxReachableVel
      const bool currVelHasPassedEndVelFromMaxReachableVel =
          (maxReachableVel_ != endVel_) &&
          ((currVel_ > endVel_) == (currVel_ > maxReachableVel_));

      if (currVelHasPassedEndVelFromStartVel ||
          currVelHasPassedEndVelFromMaxReachableVel) {
        currVel_ = endVel_;
      }
    }
  ...
  if (((currDistToTarget > 0) != (totalDistToTarget_ > 0)) ||
      (isDecel_ && (currVel_ == endVel_))) {
    targetReached_ = true;
    currVel_ = endVel_;
    currPos_ = endPos_;
  }
...
#if (DEBUG_VPG)
  AnkiDebug("VPG.Step",
            "currVel %f, currPos %f, currDistToTarget %f, isDecel %d", currVel_,
            currPos_, currDistToTarget, isDecel_);
#endif
...
bool VelocityProfileGenerator::TargetReached() { return targetReached_; }
