...
// At low speeds Cozmo was stuttering/struggling while following a path which
// was due to resetting wheelcontroller's integral gains, this enables or
// disables resetting those gains. If Cozmo suddenly has trouble following paths
...
s8 realPathSegment_ = -1;  // Segment index of the global path. Reset only on
                           // StartPathTraversal().
...
const f32 COAST_VELOCITY_RADPS =
    0.4f;  // Same as POINT_TURN_TERMINAL_VEL_RAD_PER_S
...
void ClearPath(bool didCompletePath) {
  if (IsTraversingPath()) {
    ...
    if (!didCompletePath && currentPathIsExternal_) {
      // Send a path interruption event iff we were following an
      // externally-specified path when cleared
...
// Add path segment
bool AppendPathSegment_Line(f32 x_start_mm, f32 y_start_mm, f32 x_end_mm,
                            f32 y_end_mm, f32 targetSpeed, f32 accel,
                            f32 decel) {
  TrimPath();
...
bool AppendPathSegment_Arc(f32 x_center_mm, f32 y_center_mm, f32 radius_mm,
                           f32 startRad, f32 sweepRad, f32 targetSpeed,
                           f32 accel, f32 decel) {
  TrimPath();
  return path_.AppendArc(x_center_mm, y_center_mm, radius_mm, startRad,
                         sweepRad, targetSpeed, accel, decel);
}

bool AppendPathSegment_PointTurn(f32 x, f32 y, f32 startAngle, f32 targetAngle,
                                 f32 targetRotSpeed, f32 rotAccel, f32 rotDecel,
                                 f32 angleTolerance, bool useShortestDir) {
  TrimPath();
  return path_.AppendPointTurn(x, y, startAngle, targetAngle, targetRotSpeed,
                               rotAccel, rotDecel, angleTolerance,
                               useShortestDir);
}

u8 GenerateDubinsPath(f32 start_x, f32 start_y, f32 start_theta, f32 end_x,
                      f32 end_y, f32 end_theta, f32 start_radius,
                      f32 end_radius, f32 targetSpeed, f32 accel, f32 decel,
                      f32 final_straight_approach_length, f32 *path_length) {
  return Planning::GenerateDubinsPath(
      path_, start_x, start_y, start_theta, end_x, end_y, end_theta,
      start_radius, end_radius, targetSpeed, accel, decel,
      final_straight_approach_length, path_length);
}
...
u8 GetClosestSegment(const f32 x, const f32 y, const f32 angle) {
  ...
  for (u8 i = 0; i < path_.GetNumSegments(); ++i) {
    ...
    AnkiDebug("PathFollower.GetClosestSegment.PathDist", "%f  (res=%d)",
              distToSegment, res);
#endif
    if (ABS(distToSegment) < distToClosestSegment &&
        (res == Planning::IN_SEGMENT_RANGE ||
         res == Planning::OOR_NEAR_START)) {
      closestSegId = i;
      distToClosestSegment = ABS(distToSegment);
#if (DEBUG_PATH_FOLLOWER)
      AnkiDebug("PathFollower.GetClosestSegment",
                " New closest seg: %d, distToSegment %f (res=%d)", i,
                distToSegment, res);
#endif
    }
  }
...
bool StartPathTraversal(u16 path_id) {
  // If we were already following an externally-specified path, send an
  // interruption event.
  ...
  if (path_.GetNumSegments() > 0) {
    ...
    AnkiConditionalErrorAndReturnValue(
        path_.CheckContinuity(CONTINUITY_TOL_MM2), false,
        "PathFollower.StartPathTraversal.PathIsDiscontinuous", "");
    ...
    // Set speed
    // (Except for point turns whose speeds are handled at the steering
    // controller level)
    f32 targetSpeed = GetCurrentSegmentTargetSpeed();
    if (path_[currPathSegment_].GetType() != Planning::PST_POINT_TURN) {
      SpeedController::SetUserCommandedDesiredVehicleSpeed(targetSpeed);
      SpeedController::SetUserCommandedAcceleration(
          path_[currPathSegment_].GetAccel());
      SpeedController::SetUserCommandedDeceleration(
          path_[currPathSegment_].GetDecel());
    }

    AnkiDebug("PathFollower.StartPathTraversal",
              "Start segment %d, speed = %f, accel = %f, decel = %f",
              currPathSegment_, targetSpeed, path_[currPathSegment_].GetAccel(),
              path_[currPathSegment_].GetDecel());
  ...
  if (currentPathIsExternal_) {
    // update the lastPathID and send an event that we're starting a new
    // externally-specified path
    lastPathID_ = path_id;
...
Planning::SegmentRangeStatus ProcessPathSegment(f32 &shortestDistanceToPath_mm,
                                                f32 &radDiff) {
  ...
  AnkiAssert(Planning::PST_LINE == path_[currPathSegment_].GetType() ||
                 Planning::PST_ARC == path_[currPathSegment_].GetType(),
             "");
  ...
  Planning::SegmentRangeStatus status =
      path_[currPathSegment_].GetDistToSegment(
          lookaheadX, lookaheadY, angle.ToFloat(), shortestDistanceToPath_mm,
          radDiff, &distToEnd);
  ...
  if ((currPathSegment_ == path_.GetNumSegments() - 1) ||
    ...
    if (status == Planning::OOR_NEAR_END) {
      if (LOOK_AHEAD_DIST_MM != 0) {
        ...
        status = path_[currPathSegment_].GetDistToSegment(
            x, y, angle.ToFloat(), junk_mm, junk_rad, &distToEnd);
        // PRINT("PATH-OOR: status %d (distToEnd %f, currCmdSpeed %d mm/s,
        // currSpeed %d mm/s)\n", status, distToEnd, (s32)
        //      SpeedController::GetUserCommandedCurrentVehicleSpeed(),
        //      (s32)SpeedController::GetCurrentMeasuredVehicleSpeed());
      }
    } else {
      // For purposes of determining if we should decelerate we should consider
      // the distance remaining between the segment end and the robot's drive
      // center rather than the lookahead point.
      distToEnd += LOOK_AHEAD_DIST_MM;
    }

    // 2) Check if time to decelerate
    if (!startedDecelOnSegment_) {
      // See if the current deceleration rate is sufficient to stop the robot at
      // the end of the path
      ...
      if (distToStopIfDecelNow >= distToEnd) {
        // Compute the deceleration necessary to stop robot at end of segment in
        // the remaining distance Since we're actually decelerating to
        // END_OF_PATH_TARGET_SPEED_MMPS, this is just an approximation.
        ...
        SpeedController::SetUserCommandedDesiredVehicleSpeed(
            copysign(END_OF_PATH_TARGET_SPEED_MMPS,
                     path_[currPathSegment_].GetTargetSpeed()));
        startedDecelOnSegment_ = true;
        // AnkiDebug( "PathFollower", "PathFollower: Decel to end of segment %d
        // (of %d) at %d mm/s^2 from speed of %d mm/s (meas %d mm/s) over %f
        // mm\n",
        //      currPathSegment_, path_.GetNumSegments(), requiredDecel,
        //      currSpeed,
        //      (s32)SpeedController::GetCurrentMeasuredVehicleSpeed(),
        //      distToEnd);
      }
      ...
      AnkiDebug("PathFollower.ProcessPathSegment.Decel",
...
Planning::SegmentRangeStatus ProcessPathSegmentPointTurn(
    f32 &shortestDistanceToPath_mm, f32 &radDiff) {
  const Planning::PathSegmentDef::s_turn *currSeg =
      &(path_[currPathSegment_].GetDef().turn);

#if (DEBUG_PATH_FOLLOWER)
  Radians currOrientation = Localization::GetCurrPose_angle();
  AnkiDebug("PathFollower.ProcessPathSegmentPointTurn",
...
#endif
    SteeringController::ExecutePointTurn(
        currSeg->targetAngle, path_[currPathSegment_].GetTargetSpeed(),
        path_[currPathSegment_].GetAccel(), path_[currPathSegment_].GetDecel(),
        currSeg->angleTolerance, currSeg->useShortestDir);
  ...
  // For point turns, ignore how far off we may be from the ideal point turn
  // location
...
void PathComplete() {
  ...
  // Pass in "true" to signify that we _did_ complete the path (so don't send an
  // Interruption event, since we just sent a Completed event above).
  ClearPath(true);
...
Result Update() {
  ...
  switch (path_[currPathSegment_].GetType()) {
    ...
    case Planning::PST_POINT_TURN:
      ...
      break;
    default:
      AnkiWarn("PathFollower.Update.InvalidSegmentType",
               "Segment %d has invalid type %d", currPathSegment_,
               path_[currPathSegment_].GetType());
      break;
  }

#if (DEBUG_PATH_FOLLOWER)
  AnkiDebug("PathFollower.Update.DistToPath",
            "%f mm, %f deg, segRes %d, segType %d, currSeg %d", distToPath_mm_,
            RAD_TO_DEG_F32(radToPath_), segRes,
            path_[currPathSegment_].GetType(), currPathSegment_);
#endif
...
#if (DEBUG_PATH_FOLLOWER)
    AnkiDebug("PathFollower.Update.SegmentSpeed",
  ...
  if (!DockingController::IsBusy()) {
    ...
    if (ABS(distToPath_mm_) > TOO_FAR_FROM_PATH_DIST_MM) {
      AnkiWarn("PathFollower.Update.StartingErrorTooLarge", "%f mm",
               distToPath_mm_);
      ClearPath();
      return RESULT_FAIL;
    }
...
void PrintPath() { path_.PrintPath(); }

void PrintPathSegment(s16 segment) { path_.PrintSegment(segment); }

u16 GetLastPathID() { return lastPathID_; }

const Planning::Path &GetPath() { return path_; }

bool DriveStraight(f32 dist_mm, f32 acc_start_frac, f32 acc_end_frac,
                   f32 duration_sec) {
  ...
  if (acc_start_frac < 0 || acc_end_frac < 0) {
    AnkiWarn("PathFollower.DriveStraight.NegativeFraction",
             "start: %f, end: %f", acc_start_frac, acc_end_frac);
    return false;
  }
  ...
  if (!vpg.StartProfile_fixedDuration(
          0, currSpeed, acc_start_frac * duration_sec, dist_mm,
          acc_end_frac * duration_sec, MAX_SAFE_WHEEL_SPEED_MMPS,
          MAX_WHEEL_ACCEL_MMPS2, duration_sec, CONTROL_DT)) {
  ...
  // NOTE: PathFollower actually transitions to the speed settings of the
  // following segment
  ...
  AnkiInfo("PathFollower.DriveStraight.Params",
           "total dist %f, startDist %f, endDist %f", dist_mm, startAccelDist,
           endAccelDist);

  // Get intermediate poses: (1) after starting accel phase and (2) before
  // ending accel phase
  ...
  AnkiDebug("PathFollower.DriveStraight.Points",
            "(%f, %f) to (%f, %f) to (%f, %f) to (%f, %f)\n", curr_x, curr_y,
            int_x1, int_y1, int_x2, int_y2, dest_x, dest_y);
  ClearPath();
  AppendPathSegment_Line(curr_x, curr_y, int_x1, int_y1, maxReachableVel,
                         startAccel, startAccel);
  AppendPathSegment_Line(int_x1, int_y1, int_x2, int_y2, maxReachableVel,
                         startAccel, startAccel);
  AppendPathSegment_Line(
      int_x2, int_y2, dest_x, dest_y,
      dist_mm > 0 ? COAST_VELOCITY_MMPS : -COAST_VELOCITY_MMPS, endAccel,
      endAccel);
  StartPathTraversal();
...
bool DriveArc(f32 sweep_rad, f32 radius_mm, f32 acc_start_frac,
              f32 acc_end_frac, f32 duration_sec) {
  VelocityProfileGenerator vpg;
  ...
  Localization::GetDriveCenterPose(curr_x, curr_y, curr_angle);
  f32 currAngSpeed =
      -SpeedController::GetCurrentMeasuredVehicleSpeed() / radius_mm;

  // Check for valid fractions
  if (acc_start_frac < 0 || acc_end_frac < 0) {
    AnkiWarn("PathFollower.DriveArc.NegativeFraction", "start: %f, end: %f",
             acc_start_frac, acc_end_frac);
    return false;
  }
  ...
  if (!vpg.StartProfile_fixedDuration(
          0, currAngSpeed, acc_start_frac * duration_sec, sweep_rad,
          acc_end_frac * duration_sec, MAX_BODY_ROTATION_SPEED_RAD_PER_SEC,
          MAX_BODY_ROTATION_ACCEL_RAD_PER_SEC2, duration_sec, CONTROL_DT)) {
  ...
  AnkiDebug("PathFollower.DriveArc",
            "curr_x,y  (%f, %f), center x,y (%f, %f), radius %f", curr_x,
            curr_y, x_center, y_center, radius_mm);
  AnkiDebug("PathFollower.DriveArc",
            "start + sweep1 = ang1 (%f + %f = %f), end + sweep2 = ang2 ang2 "
            "(%f - %f = %f)",
            startRad, startAccelSweep, int_ang1, startRad + sweep_rad,
            endAccelSweep, int_ang2);
  AnkiDebug("PathFollower.DriveArc",
            "targetSpeed %f, startAccel %f, endAccel %f", targetSpeed,
            startAccel, endAccel);

  // Create 3-segment path
  ClearPath();
  AppendPathSegment_Arc(x_center, y_center, absRadius, startRad,
                        startAccelSweep, targetSpeed, startAccel, startAccel);
  AppendPathSegment_Arc(x_center, y_center, absRadius, int_ang1,
                        int_ang2 - int_ang1, targetSpeed, startAccel,
                        startAccel);
  AppendPathSegment_Arc(
      x_center, y_center, absRadius, int_ang2, endAccelSweep,
      drivingFwd > 0 ? COAST_VELOCITY_MMPS : -COAST_VELOCITY_MMPS, endAccel,
      endAccel);
...
bool DrivePointTurn(f32 sweep_rad, f32 acc_start_frac, f32 acc_end_frac,
                    f32 angleTolerance, f32 duration_sec) {
  VelocityProfileGenerator vpg;
  ...
  // Check for valid fractions
  if (acc_start_frac < 0 || acc_end_frac < 0) {
    AnkiWarn("PathFollower.DrivePointTurn.NegativeFraction",
             "start: %f, end: %f", acc_start_frac, acc_end_frac);
    return false;
  }
  ...
  if (!vpg.StartProfile_fixedDuration(
          0, 0, acc_start_frac * duration_sec, sweep_rad,
          acc_end_frac * duration_sec, MAX_BODY_ROTATION_SPEED_RAD_PER_SEC,
          MAX_BODY_ROTATION_ACCEL_RAD_PER_SEC2, duration_sec, CONTROL_DT)) {
    AnkiWarn(
        "PathFollower.DrivePointTurn.VPGFail",
  ...
  AnkiDebug("PathFollower.DrivePointTurn",
            "start %f, int_ang1 %f, int_ang2 %f, dest %f", curr_angle.ToFloat(),
            int_ang1, int_ang2, dest_ang);
  AnkiDebug("PathFollower.DrivePointTurn",
            "targetRotSpeed %f, startRotAccel %f, endRotAccel %f", targetRotVel,
            startAngAccel, endAngAccel);

  // Create 3-segment path
  ClearPath();
  AppendPathSegment_PointTurn(curr_x, curr_y, curr_angle.ToFloat(), int_ang1,
                              targetRotVel, startAngAccel, startAngAccel,
                              angleTolerance, false);
  AppendPathSegment_PointTurn(curr_x, curr_y, int_ang1, int_ang2, targetRotVel,
                              startAngAccel, startAngAccel, angleTolerance,
                              false);
  AppendPathSegment_PointTurn(
      curr_x, curr_y, int_ang2, dest_ang,
      sweep_rad > 0 ? COAST_VELOCITY_RADPS : -COAST_VELOCITY_RADPS, endAngAccel,
      endAngAccel, angleTolerance, false);
