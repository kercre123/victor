...
//////// DriveTest /////////
bool enableToggleDir_ =
    false;  // false: Only drive forward
            // true: Switch between driving forward and reverse

bool enableDirectHALTest_ =
    false;  // false: Test WheelController
            // true:  Test direct drive via HAL::MotorSetPower

bool enableCycleSpeedsTest_ =
    false;  // false: Drive motors at the commanded speed/power
            // true: Cycle through different power levels increasing by
            // wheelPowerStep each time

const u32 WHEEL_TOGGLE_DIRECTION_PERIOD_MS = 2000;

// Acceleration
const f32 accel_mmps2 =
    0;  // 0 for infinite acceleration.
...
//////// DockPathTest /////////
enum { DT_STOP, DT_STRAIGHT, DT_LEFT, DT_STRAIGHT2, DT_RIGHT };
...
const LEDColor LEDColorList_[LED_COLOR_LIST_SIZE] = {
    LEDColor::LED_RED, LEDColor::LED_GREEN, LEDColor::LED_BLUE,
    LEDColor::LED_OFF};
...
TestMode GetMode() { return testMode_; }
...
Result PlaceOnGroundTestInit(s32 x_offset_mm, s32 y_offset_mm,
                             s32 angle_offset_degrees) {
  AnkiInfo("TestModeController.PlaceOnGroundTestInit",
           "xOffset %d mm, yOffset %d mm, angleOffset %d degrees", x_offset_mm,
           y_offset_mm, angle_offset_degrees);
  ticCnt_ = 0;
  PickAndPlaceController::PlaceOnGround(50, 100, 100, x_offset_mm, y_offset_mm,
...
Result DockPathTestUpdate() {
  ...
  if (ticCnt_++ >= DOCK_PATH_TOGGLE_TIME_S * ONE_OVER_CONTROL_DT) {
    ...
    AnkiInfo("TestModeController.DockPathTestUpdate", "Test state: %d",
             dockPathState_);
    ticCnt_ = 0;
  }
...
Result PathFollowTestUpdate() {
  ...
  if (!pathStarted_ && HAL::GetTimeStamp() > startDriveTime_ms) {
    ...
    PathFollower::AppendPathSegment_Arc(-radius, 0, radius, 0,    M_PI_F, speed,
    PF_ACCEL_MMPS2, PF_DECEL_MMPS2);
    PathFollower::AppendPathSegment_Arc(-radius, 0, radius, M_PI_F, M_PI_F,
    speed, PF_ACCEL_MMPS2, PF_DECEL_MMPS2);
    ...
    */
    ...
    Localization::SetDriveCenterPose(
        0, 0, M_PI_2_F * (PATH_FOLLOW_BACKWARDS ? 1 : -1));

    // PathFollower::AppendPathSegment_PointTurn(0, 0, -M_PI_2_F,
    // -1.5f, 2.f, 2.f);

    float arc1_radius = sqrt((float)5000);  // Radius of sqrt(50^2 + 50^2)
    f32 sweepAng = atan_fast((350 - arc1_radius) / 250);

    PathFollower::AppendPathSegment_Arc(arc1_radius, 0, arc1_radius, -M_PI_F,
                                        sweepAng, PF_TARGET_SPEED_MMPS,
                                        PF_ACCEL_MMPS2, PF_DECEL_MMPS2);
    ...
    PathFollower::AppendPathSegment_Line(
        firstConnectionPt_x, firstConnectionPt_y, secondConnectionPt_x,
        secondConnectionPt_y, PF_TARGET_SPEED_MMPS, PF_ACCEL_MMPS2,
        PF_DECEL_MMPS2);

    PathFollower::AppendPathSegment_Arc(
        350, -250, arc1_radius, -M_PI_F + sweepAng, M_PI_F - sweepAng,
        PF_TARGET_SPEED_MMPS, PF_ACCEL_MMPS2, PF_DECEL_MMPS2);
#else
    PathFollower::AppendPathSegment_Line(0.0, 0.0, 300, -300,
                                         PF_TARGET_SPEED_MMPS, PF_ACCEL_MMPS2,
                                         PF_DECEL_MMPS2);
    float arc1_radius = sqrt((float)5000);  // Radius of sqrt(50^2 + 50^2)
    PathFollower::AppendPathSegment_Arc(350, -250, arc1_radius, -0.75f * PI,
                                        0.75f * PI, PF_TARGET_SPEED_MMPS,
                                        PF_ACCEL_MMPS2, PF_DECEL_MMPS2);
#endif
    PathFollower::AppendPathSegment_Line(
        350 + arc1_radius, -250, 350 + arc1_radius, 200, PF_TARGET_SPEED_MMPS,
        PF_ACCEL_MMPS2, PF_DECEL_MMPS2);
    float arc2_radius = sqrt((float)20000);  // Radius of sqrt(100^2 + 100^2)
    // PathFollower::AppendPathSegment_Arc(0.35 + arc1_radius - arc2_radius,
    // 0.2, arc2_radius, 0, M_PI_2_F);
    PathFollower::AppendPathSegment_Arc(
        350 + arc1_radius - arc2_radius, 200, arc2_radius, 0, 3 * M_PI_2_F,
        PF_TARGET_SPEED_MMPS, PF_ACCEL_MMPS2, PF_DECEL_MMPS2);
    PathFollower::AppendPathSegment_Arc(
        350 + arc1_radius - arc2_radius, 200 - 2 * arc2_radius, arc2_radius,
        M_PI_2_F, -3 * M_PI_2_F, PF_TARGET_SPEED_MMPS, PF_ACCEL_MMPS2,
        PF_DECEL_MMPS2);

    PathFollower::AppendPathSegment_Line(
        350 + arc1_radius - 2 * arc2_radius, 200 - 2 * arc2_radius,
        350 + arc1_radius - 2 * arc2_radius, 0, PF_TARGET_SPEED_MMPS,
        PF_ACCEL_MMPS2, PF_DECEL_MMPS2);
  ...
  if (!PathFollower::IsTraversingPath()) {
    switch (pfcfState_) {
      case PFCF_DRIVE_STRAIGHT:
        AnkiInfo(
            "TestModeController.PathFollowConvenienceFuncTestUpdate."
            "DriveStraight",
            "Started");
      ...
      case PFCF_DRIVE_ARC:
        AnkiInfo(
            "TestModeController.PathFollowConvenienceFuncTestUpdate.DriveArc",
            "Started");
      ...
      case PFCF_DRIVE_POINT_TURN:
        AnkiInfo(
            ...
            "DrivePointTurn",
            "Started");
        if (!PathFollower::DrivePointTurn(-M_PI_2_F, 0.25, 0.25,
                                          DEG_TO_RAD_F32(1.f), 1)) {
          AnkiInfo(
              "TestModeController.PathFollowConvenienceFuncTestUpdate."
              "DrivePointTurn",
              "FAILED");
...
//                   (Only applies if DTF_ENABLE_DIRECT_HAL_TEST and
//                   DTF_ENABLE_CYCLE_SPEED_TESTS are set)
// wheelSpeedOrPower: Wheel speed to command in mm/s (when
// DTF_ENABLE_DIRECT_HAL_TEST is not set)
//                    or power to command as a percent (when
//                    DTF_ENABLE_DIRECT_HAL_TEST is set)
Result DriveTestInit(s32 flags, s32 powerStepPercent, s32 wheelSpeedOrPower) {
  AnkiInfo("TestModeController.DriveTestInit", "%x, %d, %d", flags,
           powerStepPercent, wheelSpeedOrPower);
  enableDirectHALTest_ =
      flags & EnumToUnderlyingType(DriveTestFlags::DTF_ENABLE_DIRECT_HAL_TEST);
  enableCycleSpeedsTest_ =
      flags &
      EnumToUnderlyingType(DriveTestFlags::DTF_ENABLE_CYCLE_SPEEDS_TEST);
  enableToggleDir_ =
      flags & EnumToUnderlyingType(DriveTestFlags::DTF_ENABLE_TOGGLE_DIR);
  wheelPowerStep_ =
      powerStepPercent != 0 ? powerStepPercent : DEFAULT_WHEEL_POWER_STEP;
  wheelSpeed_mmps_ =
      wheelSpeedOrPower != 0 ? wheelSpeedOrPower : WHEEL_SPEED_CMD_MMPS;
  wheelPower_ = 0;
...
Result DriveTestUpdate() {
  ...
  if (ticCnt_++ >= WHEEL_TOGGLE_DIRECTION_PERIOD_MS / ROBOT_TIME_STEP_MS) {
    ...
    if (enableDirectHALTest_) {
      AnkiInfo("TestModeController.DriveTestUpdate",
               "Applying %.3f power (currSpeed %.2f %.2f, filtSpeed %.2f %.2f)",
               wheelTargetPower_ * 0.01f, lSpeed, rSpeed, lSpeed_filt,
               rSpeed_filt);
    } else {
      AnkiInfo("TestModeController.DriveTestUpdate",
  ...
  if (enableDirectHALTest_) {
    ...
    SteeringController::ExecuteDirectDrive(wheelSpeed_mmps_, wheelSpeed_mmps_,
...
Result LiftTestInit(s32 mode, s32 noddingCycleTime_ms, s32 powerPercent) {
  AnkiInfo("TestModeController.LiftTestInit", "flags = %d, powerPercent = %d",
           mode, powerPercent);
  ticCnt_ = 0;
  ticCnt2_ = 0;
  ...
  if (liftTestMode_ == LiftTestFlags::LiftTF_TEST_POWER ||
      liftTestMode_ == LiftTestFlags::LiftTF_DISABLE_MOTOR) {
...
Result LiftTestUpdate() {
  switch (liftTestMode_) {
    case LiftTestFlags::LiftTF_NODDING: {
      ...
      const f32 thetaStep =
          2 * M_PI_F / liftNodCycleTime_ms_ * ROBOT_TIME_STEP_MS;
    ...
    case LiftTestFlags::LiftTF_TEST_HEIGHTS: {
      ...
      if (ticCnt_++ >= 1000 / ROBOT_TIME_STEP_MS) {
        if (liftTestMode_ == LiftTestFlags::LiftTF_TEST_HEIGHTS) {
          up = !up;
          if (up) {
            AnkiInfo("TestModeController.LiftTestUpdate",
                     "Lift HIGH %f mm (maxSpeed %f rad/s)",
                     LIFT_DES_HIGH_HEIGHT, maxLiftSpeed_radPerSec_);
            LiftController::SetDesiredHeight(LIFT_DES_HIGH_HEIGHT);
          } else {
            AnkiInfo("TestModeController.LiftTestUpdate",
                     "Lift LOW %f mm (maxSpeed %f rad/s)", LIFT_DES_LOW_HEIGHT,
                     maxLiftSpeed_radPerSec_);
          ...
          if (up) {
            AnkiInfo("TestModeController.LiftTestUpdate",
                     "Lift UP %f power (maxSpeed %f rad/s)", liftPower_,
                     maxLiftSpeed_radPerSec_);
            HAL::MotorSetPower(MotorID::MOTOR_LIFT, liftPower_);
          } else {
            AnkiInfo("TestModeController.LiftTestUpdate",
                     "Lift DOWN %f power (maxSpeed %f rad/s)", -liftPower_,
                     maxLiftSpeed_radPerSec_);
            HAL::MotorSetPower(MotorID::MOTOR_LIFT, -liftPower_);
          }
    ...
    case LiftTestFlags::LiftTF_DISABLE_MOTOR:
      break;
    default: {
      AnkiWarn("TestModeController.LiftTestUpdate",
               "WARN: Unknown lift test mode %hhu", liftTestMode_);
      Reset();
    }
  }
  ...
  if (ABS(lSpeed) > 0.001f) {
    ...
    AnkiInfo("TestModeController.LiftTestUpdate",
             "Avg lift speed %f rad/s, height change %f mm", avgLiftSpeed_,
             LiftController::GetHeightMM() - startLiftHeightMM_);
    avgLiftSpeed_ = 0;
    ticCnt2_ = 0;
  }
...
Result LiftToggleTestUpdate() {
  ...
  if (LiftController::IsInPosition()) {
    if (inPositionCycles++ > 500) {
      ...
      AnkiInfo("TestModeController.LiftToggleTestUpdate", "SET LIFT TO %f mm",
               liftHeight);
  ...
  if (ticCnt2_++ >= printCyclePeriod_) {
    ...
    AnkiInfo("TestModeController.LiftToggleTestUpdate",
             "Lift speed %f rad/s, height %f mm", lSpeed, lPos);
    ticCnt2_ = 0;
...
Result HeadTestInit(s32 mode, s32 noddingCycleTime_ms, s32 powerPercent) {
  AnkiInfo("TestModeController.HeadTestInit", "flags = %d, powerPercent = %d",
           mode, powerPercent);
  ticCnt_ = 0;
  ticCnt2_ = 0;
  printCyclePeriod_ = 250 / ROBOT_TIME_STEP_MS;
...
Result HeadTestUpdate() {
  switch (headTestMode_) {
    ...
    case HeadTestFlags::HTF_TEST_ANGLES: {
      ...
      if (ticCnt_++ >= headNodCycleTime_ms_ / ROBOT_TIME_STEP_MS) {
        if (headTestMode_ == HeadTestFlags::HTF_TEST_ANGLES) {
          up = !up;
          if (up) {
            AnkiInfo("TestModeController.HeadTestUpdate",
                     "Head HIGH %f rad (maxSpeed %f rad/s)",
                     HEAD_DES_HIGH_ANGLE, maxHeadSpeed_radPerSec_);
            HeadController::SetDesiredAngle(HEAD_DES_HIGH_ANGLE);
          } else {
            AnkiInfo("TestModeController.HeadTestUpdate",
                     "Head LOW %f rad (maxSpeed %f rad/s)", HEAD_DES_LOW_ANGLE,
                     maxHeadSpeed_radPerSec_);
          ...
          if (up) {
            AnkiInfo("TestModeController.HeadTestUpdate",
                     "Head UP %f power (maxSpeed %f rad/s)", headPower_,
                     maxHeadSpeed_radPerSec_);
            HAL::MotorSetPower(MotorID::MOTOR_HEAD, headPower_);
          } else {
            AnkiInfo("TestModeController.HeadTestUpdate",
                     "Head DOWN %f power (maxSpeed %f rad/s)", -headPower_,
                     maxHeadSpeed_radPerSec_);
            HAL::MotorSetPower(MotorID::MOTOR_HEAD, -headPower_);
          }
    ...
    case HeadTestFlags::HTF_NODDING: {
      ...
      const f32 thetaStep =
          2 * M_PI_F / headNodCycleTime_ms_ * ROBOT_TIME_STEP_MS;
    ...
    default:
      AnkiWarn("TestModeController.HeadTestUpdate",
               "Unknown head test mode %hhu", headTestMode_);
      break;
  }
  ...
  if (ticCnt2_++ >= printCyclePeriod_) {
    ...
    if (ABS(hSpeed) > 0 || ABS(hSpeed_filt) > 0) {
      AnkiInfo("TestModeController.HeadTestUpdate",
               "Head speed %f rad/s (filt %f rad/s), angle %f rad", hSpeed,
               hSpeed_filt, hPos);
    }
    ticCnt2_ = 0;
...
Result IMUTestUpdate() {
  if (imuTestDoTurns_) {
    if (SteeringController::GetMode() != SteeringController::SM_POINT_TURN) {
      if (IT_turnLeft) {
        ...
        SteeringController::ExecutePointTurn(IT_TARGET_ANGLE, IT_MAX_ROT_VEL,
                                             IT_ROT_ACCEL, IT_ROT_ACCEL,
                                             IT_ROT_TOL, false);
      } else {
        // Turn right 180 degrees
        AnkiInfo("TestModeController.IMUTestUpdate", "Turning to 0");
        SteeringController::ExecutePointTurn(0.f, -IT_MAX_ROT_VEL, IT_ROT_ACCEL,
                                             IT_ROT_ACCEL, IT_ROT_TOL, false);
      }
  ...
  if (++ticCnt_ >= 200 / ROBOT_TIME_STEP_MS) {
    ...
    AnkiInfo("TestModeController.IMUTestUpdate",
             "Gyro (%f,%f,%f) rad/s, (%f,%f,%f) mm/s^2", data.gyro[0],
             data.gyro[1], data.gyro[2], data.accel[0], data.accel[1],
             data.accel[2]);
...
Result LightTestInit(s32 flags, s32 ledID, s32 color) {
  AnkiInfo("TestModeController.LightTestInit",
           "flags = %x, ledID = %d, color = %x", flags, ledID, color);
...
Result LightTestUpdate() {
  ...
  if (ticCnt_++ > 2000 / ROBOT_TIME_STEP_MS) {
    AnkiInfo("TestModeController.LightTestUpdate",
             "LED channel %hhu, color 0x%x", ledID_,
             LEDColorList_[ledColorIdx_]);
    // HAL::SetLED(ledID_, LEDColorList_[ledColorIdx_]);
    (void)LEDColorList_;  // Prevent compiler error. Don't need this when SetLED
                          // call restored.
...
Result StopTestUpdate() {
  ...
  if (ST_slowingDown) {
    if (HAL::MotorGetPosition(MotorID::MOTOR_LEFT_WHEEL) == ST_prevLeftPos &&
      ...
      AnkiInfo("TestModeController.StopTestUpdate",
               "STOPPED: (%f, %f) mm in %d tics", ST_prevLeftPos,
               ST_prevRightPos, ST_slowDownTics);
      ST_slowingDown = false;
    }
...
Result MaxPowerTestInit() {
  ...
  // Disable all controllers so that they can be overridden with HAL power
  // commands
  WheelController::Disable();
  LiftController::Disable();
...
Result MaxPowerTestUpdate() {
  ...
  if (ticCnt_++ > 5000 / ROBOT_TIME_STEP_MS) {
    AnkiInfo("TestModeController.MaxPowerTestUpdate", "SWITCHING POWER: %f",
             pwr);
...
Result Update() {
  ...
  if (HeadController::IsCalibrated() && LiftController::IsCalibrated() &&
