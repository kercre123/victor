...
#include "clad/robotInterface/messageEngineToRobot_switch_from_0x01_to_0x4F.def"
    ...
    default:
      AnkiWarn("Messages.ProcessBadTag_EngineToRobot.Recvd",
               "Received message with bad tag %x", msg.tag);
  }
}  // ProcessMessage()
...
void UpdateRobotStateMsg() {
  ...
  WheelController::GetFilteredWheelSpeeds(robotState_.lwheel_speed_mmps,
                                          robotState_.rwheel_speed_mmps);
  ...
  robotState_.backpackTouchSensorRaw =
...
#define SET_STATUS_BIT(expr, bit) \
  robotState_.status |=           \
      ((expr) ? EnumToUnderlyingType(RobotStatusFlag::bit) : 0)
  const bool areWheelsMoving =
      WheelController::AreWheelsMoving() ||
      SteeringController::GetMode() == SteeringController::SM_POINT_TURN;
  const bool isMoving = HeadController::IsMoving() ||
                        LiftController::IsMoving() || areWheelsMoving;
...
#undef SET_STATUS_BIT
  ...
  const s32 messagePeriod =
      calmMode_ ? STATE_MESSAGE_FREQUENCY_CALM : STATE_MESSAGE_FREQUENCY;
...
RobotState const& GetRobotStateMsg() { return robotState_; }
...
void Process_shutdown(const RobotInterface::Shutdown& msg) { HAL::Shutdown(); }

void Process_calmPowerMode(const RobotInterface::CalmPowerMode& msg) {
  // NOTE: This used to actually enable calm mode in syscon, but since "quiet"
  // mode
  //       was implemented in syscon where encoders are "off" when the motors
  //       are not being driven leaving minimal difference between calm mode and
  //       active mode in terms of battery life, this now only throttles
  //       RobotState messages being sent to engine since it still results in a
  //       10+% reduction in CPU consumption. Not going into syscon calm mode
  //       also means that motor calibrations are no longer necessary as a
  //       precaution when leaving calm mode.
...
void Process_absLocalizationUpdate(
    const RobotInterface::AbsoluteLocalizationUpdate& msg) {
  ...
  /*Result res =*/Localization::UpdatePoseWithKeyframe(
      msg.origin_id, msg.pose_frame_id, msg.timestamp, currentMatX, currentMatY,
      currentMatHeading.ToFloat());

  /*
  AnkiInfo( "Messages.Process_absLocalizationUpdate.Recvd", "Result %d,
  currTime=%d, updated frame time=%d: (%.3f,%.3f) at %.1f degrees (frame =
  %d)\n", res, HAL::GetTimeStamp(), msg.timestamp, currentMatX, currentMatY,
        currentMatHeading.getDegrees(),
        Localization::GetPoseFrameId());
   */
}  // ProcessAbsLocalizationUpdateMessage()

void Process_forceDelocalizeSimulatedRobot(
    const RobotInterface::ForceDelocalizeSimulatedRobot& msg) {
#ifdef SIMULATOR
...
void Update() {
  ...
  if (!syncRobotAckSent_) {
    if (syncRobotReceived_ && IMUFilter::IsBiasFilterComplete() &&
        LiftController::IsCalibrated() && HeadController::IsCalibrated()) {
      ...
      // Since the bias is typically calibrate before the robot is even
      // connected, this is the time when the data can actually be sent up to
      // engine.
  ...
  if (calmModeEnabledByEngine_) {
    ...
    } else if (timeToEnableCalmMode_ms_ != 0 &&
               timeToEnableCalmMode_ms_ < now_ms) {
      calmMode_ = true;
  ...
  while ((dataLen = HAL::RadioGetNextPacket(pktBuffer_)) > 0) {
    ...
    if (!msgBuf.IsValid()) {
      AnkiWarn("Receiver.ReceiveData.Invalid", "Receiver got %02x[%d] invalid",
               pktBuffer_[0], dataLen);
    } else if (msgBuf.Size() != dataLen) {
      AnkiWarn("Receiver.ReceiveData.SizeError",
               "Parsed message size error %d != %d", dataLen, msgBuf.Size());
    } else {
...
void Process_appendPathSegArc(const RobotInterface::AppendPathSegmentArc& msg) {
  PathFollower::AppendPathSegment_Arc(
      msg.x_center_mm, msg.y_center_mm, msg.radius_mm, msg.startRad,
      msg.sweepRad, msg.speed.target, msg.speed.accel, msg.speed.decel);
}

void Process_appendPathSegLine(
    const RobotInterface::AppendPathSegmentLine& msg) {
  PathFollower::AppendPathSegment_Line(
      msg.x_start_mm, msg.y_start_mm, msg.x_end_mm, msg.y_end_mm,
      msg.speed.target, msg.speed.accel, msg.speed.decel);
}

void Process_appendPathSegPointTurn(
    const RobotInterface::AppendPathSegmentPointTurn& msg) {
  PathFollower::AppendPathSegment_PointTurn(
      msg.x_center_mm, msg.y_center_mm, msg.startRad, msg.targetRad,
      msg.speed.target, msg.speed.accel, msg.speed.decel, msg.angleTolerance,
      msg.useShortestDir);
}
...
void Process_executePath(const RobotInterface::ExecutePath& msg) {
  ...
  AnkiInfo("Messages.Process_executePath.Result", "id=%d result=%d", msg.pathID,
           result);
}

void Process_dockWithObject(const DockWithObject& msg) {
  AnkiInfo("Messages.Process_dockWithObject.Recvd",
           "action %hhu, dockMethod %hhu, doLiftLoadCheck %d, "
           "backUpWhileLiftingCube %d, speed %f, accel %f, decel %f",
           msg.action, msg.dockingMethod, msg.doLiftLoadCheck,
           msg.backUpWhileLiftingCube, msg.speed_mmps, msg.accel_mmps2,
           msg.decel_mmps2);
  ...
  PickAndPlaceController::DockToBlock(msg.action, msg.doLiftLoadCheck,
                                      msg.speed_mmps, msg.accel_mmps2,
                                      msg.decel_mmps2, 0, 0, 0, msg.numRetries);
}

void Process_placeObjectOnGround(const PlaceObjectOnGround& msg) {
  // AnkiInfo( "Messages.Process_placeObjectOnGround.Recvd", "");
  PickAndPlaceController::PlaceOnGround(msg.speed_mmps, msg.accel_mmps2,
                                        msg.decel_mmps2, msg.rel_x_mm,
                                        msg.rel_y_mm, msg.rel_angle);
}

void Process_startMotorCalibration(
    const RobotInterface::StartMotorCalibration& msg) {
...
void Process_drive(const RobotInterface::DriveWheels& msg) {
  ...
  // AnkiInfo( "Messages.Process_drive.Executing", "left=%f mm/s, right=%f
  // mm/s", msg.lwheel_speed_mmps, msg.rwheel_speed_mmps);

  // PathFollower::ClearPath();
  SteeringController::ExecuteDirectDrive(
      msg.lwheel_speed_mmps, msg.rwheel_speed_mmps, msg.lwheel_accel_mmps2,
      msg.rwheel_accel_mmps2);
}
...
void Process_liftHeight(const RobotInterface::SetLiftHeight& msg) {
  // AnkiInfo( "Messages.Process_liftHeight.Recvd", "height %f, maxSpeed %f,
  // duration %f", msg.height_mm, msg.max_speed_rad_per_sec, msg.duration_sec);
  if (msg.duration_sec > 0) {
    LiftController::SetDesiredHeightByDuration(msg.height_mm, 0.1f, 0.1f,
                                               msg.duration_sec);
  } else {
    LiftController::SetDesiredHeight(msg.height_mm, msg.max_speed_rad_per_sec,
...
void Process_setLiftAngle(const RobotInterface::SetLiftAngle& msg) {
  ...
  if (msg.duration_sec > 0) {
    LiftController::SetDesiredAngleByDuration(msg.angle_rad, 0.1f, 0.1f,
                                              msg.duration_sec);
  } else {
    LiftController::SetDesiredAngle(msg.angle_rad, msg.max_speed_rad_per_sec,
                                    msg.accel_rad_per_sec2);
  }
  AckMotorCommand(msg.actionID);
}
...
void Process_headAngle(const RobotInterface::SetHeadAngle& msg) {
  // AnkiInfo( "Messages.Process_headAngle.Recvd", "angle %f, maxSpeed %f,
  // duration %f", msg.angle_rad, msg.max_speed_rad_per_sec, msg.duration_sec);
  if (msg.duration_sec > 0) {
    HeadController::SetDesiredAngleByDuration(msg.angle_rad, 0.1f, 0.1f,
                                              msg.duration_sec);
  } else {
    HeadController::SetDesiredAngle(msg.angle_rad, msg.max_speed_rad_per_sec,
                                    msg.accel_rad_per_sec2);
  }
  AckMotorCommand(msg.actionID);
}
...
void Process_setBodyAngle(const RobotInterface::SetBodyAngle& msg) {
  SteeringController::ExecutePointTurn(
      msg.angle_rad, msg.max_speed_rad_per_sec, msg.accel_rad_per_sec2,
      msg.accel_rad_per_sec2, msg.angle_tolerance, msg.use_shortest_direction,
...
void Process_turnInPlaceAtSpeed(const RobotInterface::TurnInPlaceAtSpeed& msg) {
  // AnkiInfo( "Messages.Process_turnInPlaceAtSpeed.Recvd", "speed %f rad/s,
  // accel %f rad/s2", msg.speed_rad_per_sec, msg.accel_rad_per_sec2);
  SteeringController::ExecutePointTurn(msg.speed_rad_per_sec,
                                       msg.accel_rad_per_sec2);
}
...
void Process_rollActionParams(const RobotInterface::RollActionParams& msg) {
  PickAndPlaceController::SetRollActionParams(
      msg.liftHeight_mm, msg.driveSpeed_mmps, msg.driveAccel_mmps2,
      msg.driveDuration_ms, msg.backupDist_mm);
}
...
void Process_setControllerGains(const RobotInterface::ControllerGains& msg) {
  switch (msg.controller) {
    ...
    case ControllerChannel::controller_steering: {
      SteeringController::SetGains(msg.kp, msg.ki, msg.kd,
                                   msg.maxIntegralError);  // Coopting structure
      break;
    }
    case ControllerChannel::controller_pointTurn: {
      SteeringController::SetPointTurnGains(msg.kp, msg.ki, msg.kd,
                                            msg.maxIntegralError);
      break;
    }
    default: {
      AnkiWarn("Messages.Process_setControllerGains.InvalidController",
               "controller: %hhu", msg.controller);
    }
  }
}

void Process_setMotionModelParams(
    const RobotInterface::SetMotionModelParams& msg) {
...
void Process_enableMotorPower(const RobotInterface::EnableMotorPower& msg) {
  switch (msg.motorID) {
    ...
    default: {
      AnkiWarn("Messages.enableMotorPower.UnhandledMotorID", "%hhu",
               msg.motorID);
      break;
    }
  }
...
void Process_cliffAlignToWhiteAction(
    const RobotInterface::CliffAlignToWhiteAction& msg) {
  if (msg.start) {
...
void Process_enableBraceWhenFalling(
    const RobotInterface::EnableBraceWhenFalling& msg) {
...
void Process_turnToRecordedHeading(
    RobotInterface::TurnToRecordedHeading const& msg) {
  SteeringController::ExecutePointTurnToRecordedHeading(
      DEG_TO_RAD_F32(msg.offset_deg), DEG_TO_RAD_F32(msg.speed_degPerSec),
      DEG_TO_RAD_F32(msg.accel_degPerSec2),
      DEG_TO_RAD_F32(msg.decel_degPerSec2), DEG_TO_RAD_F32(msg.tolerance_deg),
      msg.numHalfRevs, msg.useShortestDir);
}
...
Result SendMicDataFunction(const s16* latestMicData, uint32_t numSamples) {
  ...
  /*
  Deinterlace the audio before sending it to Engine/Anim. Coming into this
  method, we have chunks of 80 samples each, but the data for 4 mics are
  interleaved;
  ...
  Since the recipient doesn't process data any more frequently than every 10ms,
  we're minimizing the message overhead by doing the deinterlacing here and
  sending the 10ms message instead of 2 5ms messages.
  */

  int chunkOffset = chunkID * samplesPerChunk;
  for (size_t channel = 0; channel < numChannels; ++channel) {
    for (size_t sample = 0; sample < samplesPerChunk; ++sample) {
      const auto sampleBufferIndex =
          channel * samplesPerDeinterlacedChunk + chunkOffset + sample;
  ...
  chunkID = chunkID ? 0 : 1;
  if (chunkID == 0) {
    memcpy(micData.data, sampleBuffer,
           numChannels * samplesPerDeinterlacedChunk * sizeof(s16));
...
bool ReceivedInit() { return syncRobotReceived_; }
