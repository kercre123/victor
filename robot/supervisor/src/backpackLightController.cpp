 ...
 * Description:
 *
 *   Controller for the LEDs on the backpack (i.e. the "health" or "directional"
 *indicator).
 *
...
RobotInterface::SetBackpackLights
    _ledParams[(int)BackpackLightLayer::BPL_NUM_LAYERS];

RobotInterface::SetSystemLight _sysLedParams;

BackpackLightLayer _layer;  // Currently animated layer
TimeStamp_t _ledPhases[(
    int)LEDId::NUM_BACKPACK_LEDS];  // Time phase of current animation
...
void SetParams(const RobotInterface::SetBackpackLights& params) {
  if (params.layer >= (int)BackpackLightLayer::BPL_NUM_LAYERS) {
    AnkiWarn("BackpackLightController.SetParams.InvalidLayer",
             "Layer %d is invalid", (int)params.layer);
    return;
  }
...
void EnableLayer(const BackpackLightLayer layer, bool forceUpdate) {
  if (layer >= BackpackLightLayer::BPL_NUM_LAYERS) {
    AnkiWarn("BackpackLightController.EnableLayer.InvalidLayer",
             "Layer %d is invalid", (int)layer);
    return;
  }
...
Result Init() {
  memset(&_ledParams[(int)BackpackLightLayer::BPL_USER], 0,
         sizeof(_ledParams[(int)BackpackLightLayer::BPL_USER]));
  ...
  for (u8 i = 0; i < (u8)LEDId::NUM_BACKPACK_LEDS; i++) {
    _ledParams[(int)BackpackLightLayer::BPL_USER].lights[i] = {
        ...
        .offPeriod_ms = static_cast<u16>(
            (kTimeDiff_ms * ((u8)LEDId::NUM_BACKPACK_LEDS - 1 - i))),
        .transitionOnPeriod_ms = 300,
        .transitionOffPeriod_ms = 300,
        .offset_ms = static_cast<s16>(
            (kTimeDiff_ms * ((u8)LEDId::NUM_BACKPACK_LEDS - 1 - i)))};
  }
...
Result Update() {
  ...
  for (int i = 0; i < (int)LEDId::NUM_BACKPACK_LEDS; ++i) {
    const u32 newColor = GetCurrentLEDcolor(_ledParams[(int)_layer].lights[i],
                                            currentTime, _ledPhases[i]);
    HAL::SetLED((HAL::LEDId)i, newColor);
  }  // for each LED

  const u32 newColor =
      GetCurrentLEDcolor(_sysLedParams.light, currentTime, _sysLedPhase);
