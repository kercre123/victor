...
using ImuDataBufferType =
    Util::FixedCircularBuffer<IMUDataFrame,
                              IMUConstants::IMU_FRAMES_PER_ROBOT_STATE>;
...
// Angle around gravity horizontal, "roll", in radians. When the robot is on its
// right side, the angle should be exactly +pi/2 radians (+90 deg), and -pi/2
// (-90 deg) when on its left side.
f32 GetRoll();

// Starts recording a buffer of data for the specified time and sends it to
// basestation
...
// Whether or not we have finished accumulating enough readings of the gyro
// offset while the robot is not moving. SyncRobotAck is blocked until this
// completes!
