...
const std::array<ProxSensorLookupEntry, 2> kProxSensorRawDistLUT_mm{
    {{100, -30}, {300, -22}}};
#else
const std::array<ProxSensorLookupEntry, 1> kProxSensorRawDistLUT_mm{{{0, 0}}};
#endif
#endif
...
u16 _whiteDetectThreshold[4] = {
    MIN_CLIFF_STOP_ON_WHITE_VAL_HIGH, MIN_CLIFF_STOP_ON_WHITE_VAL_HIGH,
    MIN_CLIFF_STOP_ON_WHITE_VAL_HIGH, MIN_CLIFF_STOP_ON_WHITE_VAL_HIGH};

u16 _cliffDetectThresh[4] = {
    CLIFF_SENSOR_THRESHOLD_DEFAULT, CLIFF_SENSOR_THRESHOLD_DEFAULT,
    CLIFF_SENSOR_THRESHOLD_DEFAULT, CLIFF_SENSOR_THRESHOLD_DEFAULT};
...
u8 GetCliffDetectedFlags() { return _cliffDetectedFlags; }

u8 GetWhiteDetectedFlags() { return _whiteDetectedFlags; }

u16 GetCliffValue(u32 ind) {
  AnkiConditionalErrorAndReturnValue(ind < HAL::CLIFF_COUNT, 0,
                                     "ProxSensors.GetCliffValue.InvalidIndex",
                                     "Index %d is not valid", ind);
...
ProxSensorDataRaw GetProxData() {
  ...
  if (rawDist_mm <= lut.front().rawDistance_mm) {
    ...
    for (auto low = lut.begin(); low != lut.end(); ++low) {
      ...
      if ((rawDist_mm >= x0) && (rawDist_mm <= x1)) {
...
void UpdateCliff() {
  ...
  // Compute bounds on cliff detect/undetect thresholds which may be adjusted
  // according to the intensity of ambient light as measured by the LED-off
  // level.
  ...
  for (int i = 0; i < _nCliffSensors; i++) {
    ...
    } else if (whiteAlreadyDetected &&
               (_cliffVals[i] <
                _whiteDetectThreshold[i] - CLIFF_STOP_ON_WHITE_HYSTERSIS)) {
      _whiteDetectedFlags &= ~(1 << i);
    }
  }
  ...
  const bool isDriving = (ABS(leftSpeed) + ABS(rightSpeed)) >
                         WheelController::WHEEL_SPEED_CONSIDER_STOPPED_MM_S;

  // Check for whether or not wheels are already stopping.
  // When reversing and stopping fast enough it's possible for the wheels to
  // report forward speeds. Not sure if because the speed change is too fast for
  // encoders to register properly or if the wheels are actually moving forward
  // very briefly, but in any case, if the wheels are already stopping there's
  // no need to report StoppingDueToCliff. Currently, this only seems to happen
  // during the backup for face plant.
  f32 desiredLeftSpeed, desiredRightSpeed;
  WheelController::GetDesiredWheelSpeeds(desiredLeftSpeed, desiredRightSpeed);
  const bool alreadyStopping =
      (desiredLeftSpeed == 0.f) && (desiredRightSpeed == 0.f);

  // If side cliffs are detected while driving stop (possibly again)
  // because this is a precarious situation.
  const bool sideCliffsDetected =
      (_cliffDetectedFlags == ((1 << HAL::CLIFF_FL) | (1 << HAL::CLIFF_BL))) ||
      (_cliffDetectedFlags == ((1 << HAL::CLIFF_FR) | (1 << HAL::CLIFF_BR)));

  const bool frontCliffsDetectingWhite =
      ProxSensors::IsWhiteDetected(HAL::CLIFF_FL) ||
      ProxSensors::IsWhiteDetected(HAL::CLIFF_FR);

  const bool possibleCliffStop =
      (IsAnyCliffDetected() && !_wasAnyCliffDetected) || sideCliffsDetected;
  const bool possibleWhiteStop =
      frontCliffsDetectingWhite && !_wasAnyWhiteDetected;
  ...
  if (_enableCliffDetect && !IMUFilter::IsPickedUp() && !_putdownOnCliff &&
    ...
    if (stoppingBecauseCliff || stoppingBecauseWhite) {
      ...
      AnkiInfo("ProxSensors.UpdateCliff.StoppingDueToCliff",
               "stopOnCliff: %d, stopOnWhite %d, sideCliffsDetected: %d, "
               "possibleCliff: %d, possibleWhite: %d",
               _stopOnCliff, _stopOnWhite, sideCliffsDetected,
               possibleCliffStop, possibleWhiteStop);
  ...
  if (_pendingUncliffEvent != 0 &&
      HAL::GetTimeStamp() >= _pendingUncliffEvent) {
...
bool IsAnyCliffDetected() { return _cliffDetectedFlags != 0; }

bool IsCliffDetected(u32 ind) { return _cliffDetectedFlags & (1 << ind); }

bool IsAnyWhiteDetected() { return _whiteDetectedFlags != 0; }

bool IsWhiteDetected(u32 ind) { return _whiteDetectedFlags & (1 << ind); }
...
void SetWhiteDetectThreshold(u32 ind, u16 level) {
  AnkiConditionalError(ind < HAL::CLIFF_COUNT,
                       "ProxSensors.SetWhiteDetectThreshold.InvalidIndex",
                       "Index %d is not valid", ind);
  AnkiInfo("ProxSensors.SetWhiteDetectThreshold.NewLevel",
           "cliff sensor %d, threshold %d", ind, level);
  _whiteDetectThreshold[ind] = level;
}

void SetCliffDetectThreshold(u32 ind, u16 level) {
  AnkiConditionalError(ind < HAL::CLIFF_COUNT,
                       "ProxSensors.SetCliffDetectThreshold.InvalidIndex",
                       "Index %d is not valid", ind);
  if (level > CLIFF_SENSOR_THRESHOLD_MAX) {
    AnkiWarn("ProxSensors.SetCliffDetectThreshold.TooHigh",
             "cliff sensor %d, threshold %d", ind, level);
  } else if (level < CLIFF_SENSOR_THRESHOLD_MIN) {
    AnkiWarn("ProxSensors.SetCliffDetectThreshold.TooLow",
             "cliff sensor %d, threshold %d", ind, level);
  } else {
    AnkiDebug("ProxSensors.SetCliffDetectThreshold.NewLevel",
              "cliff sensor %d, threshold %d", ind, level);
