...
static int triggerFakeHwClock() {
  ...
  if (fifo == NULL) {
    AnkiError("CozmoBot.triggerFakeHwClock", "Failed to open fifo (errno %d)\n",
              errno);
    return -1;
  }
...
const u32 MAIN_TOO_LATE_TIME_THRESH_USEC =
    ROBOT_TIME_STEP_MS * 1500;  // Normal cycle time plus 50% margin
...
Result Init(const int* shutdownSignal) {
  ...
  AnkiConditionalErrorAndReturnValue(lastResult == RESULT_OK, lastResult,
                                     "CozmoBot.InitFail.HAL", "");

  lastResult = BackpackLightController::Init();
  AnkiConditionalErrorAndReturnValue(
      lastResult == RESULT_OK, lastResult,
      "CozmoBot.InitFail.BackpackLightController", "");

  lastResult = Messages::Init();
  AnkiConditionalErrorAndReturnValue(lastResult == RESULT_OK, lastResult,
                                     "CozmoBot.InitFail.Messages", "");

  lastResult = Localization::Init();
  AnkiConditionalErrorAndReturnValue(lastResult == RESULT_OK, lastResult,
                                     "CozmoBot.InitFail.Localization", "");

  lastResult = PathFollower::Init();
  AnkiConditionalErrorAndReturnValue(lastResult == RESULT_OK, lastResult,
                                     "CozmoBot.InitFail.PathFollower", "");

  lastResult = IMUFilter::Init();
  AnkiConditionalErrorAndReturnValue(lastResult == RESULT_OK, lastResult,
                                     "CozmoBot.InitFail.IMUFilter", "");

  lastResult = DockingController::Init();
  AnkiConditionalErrorAndReturnValue(lastResult == RESULT_OK, lastResult,
                                     "CozmoBot.InitFail.DockingController", "");

  // Before liftController?!
  lastResult = PickAndPlaceController::Init();
  AnkiConditionalErrorAndReturnValue(lastResult == RESULT_OK, lastResult,
                                     "CozmoBot.InitFail.PickAndPlaceController",
                                     "");

  lastResult = LiftController::Init();
  AnkiConditionalErrorAndReturnValue(lastResult == RESULT_OK, lastResult,
                                     "CozmoBot.InitFail.LiftController", "");
  ...
  // If there was rampost error, check if battery is too hot or too low and
  // shutdown if so. The appropriate icon should already be on the face from
  // rampost.
  if (HasRampostError()) {
...
// Note: Motors are disabled automatically by syscon after 25ms of spine sync
// loss
void Destroy() {
  if (initialized_) {
...
void SendPrepForShutdown(ShutdownReason reason) {
  ...
  const u32 timeSinceOnChargerStateChanged_ms =
      lastOnChargerChangedTime_ms_ > 0
          ? HAL::GetTimeStamp() - lastOnChargerChangedTime_ms_
          : 0;
  DASMSG(vectorbot_prep_for_shutdown, "vectorbot.prep_for_shutdown",
         "Signalling to prepare for imminent shutdown");
  DASMSG_SET(i1, static_cast<u32>(HAL::BatteryGetVoltage() * 1000),
             "Current raw battery voltage (mV)");
  DASMSG_SET(i2, HAL::BatteryIsOnCharger(),
             "Whether the battery is on charger");
  DASMSG_SET(i3, timeSinceOnChargerStateChanged_ms,
             "Time since on charger state changed (ms)");
  DASMSG_SET(i4, HAL::BatteryGetTemperature_C(), "Battery temperature (C)");
  DASMSG_SET(s1, EnumToString(reason), "Reason for shutdown");
  DASMSG_SET(s2, lowBattOnStart_ ? "LowBattOnStart" : "",
             "Battery was low when vic-robot started");
  DASMSG_SET(s3, hotBattOnStart_ ? "HotBattOnStart" : "",
             "Battery was overheating when vic-robot started");
  DASMSG_SEND();
}
...
void CheckForOverheatingBatteryShutdown() {
  ...
  if (HAL::BatteryIsOverheated() || hotBattOnStart_) {
    if (shutdownTime_ms == 0) {
      ...
      shutdownTime_ms = now_ms + (hotBattOnStart_ ? HAL_SHUTDOWN_DELAY_SHORT_MS
                                                  : HAL_SHUTDOWN_DELAY_MS);
...
void CheckForCriticalBatteryShutdown() {
  ...
  if (HAL::IsShutdownImminent() || lowBattOnStart_) {
    if (shutdownTime_ms == 0) {
      ...
      shutdownTime_ms = now_ms + (lowBattOnStart_ ? HAL_SHUTDOWN_DELAY_SHORT_MS
                                                  : HAL_SHUTDOWN_DELAY_MS);
      shutdownInProgress_ = true;
...
void CheckForShutdown() {
  ...
  // sync(). The assumption is that whoever is handling the PrepForShutdown
  // message is able to do so within this amount of time
    ...
    case NONE:
      if (buttonIsPressed) {
        ...
        // If button has been held down for more than SHUTDOWN_TIME_MS
...
#if FACTORY_TEST
          // In factory, don't send PrepForShutdown because processes don't
          // shutdown cleanly and we get a fault code (i.e. 915 - NO_ENGINE)
          // appear right before the power is pulled. Instead just call sync and
          // let syscon pull power.
          AnkiWarn("CozmoBot.CheckForButtonHeld.PossiblyShuttingDown",
                   "Syncing");
          timeMark_ms = curTime_ms;
          sync();
#else
          ...
          AnkiInfo("CozmoBot.CheckForButtonHeld.Shutdown",
                   "Sending PrepForShutdown");
  ...
  switch (state) {
    ...
    case PREP: {
      if (IMUFilter::IsBiasFilterComplete()) {
        ...
        const TimeStamp_t timeDif_ms =
            HAL::GetTimeStamp() - syncReceivedTime_ms;
        if (timeDif_ms > GYRO_NOT_CALIB_PREP_FOR_SHUTDOWN_MS) {
          // Send a shutdown message to anim/engine
          AnkiInfo("CozmoBot.CheckForGyroCalibShutdown.Shutdown",
                   "Sending PrepForShutdown");
...
Result step_MainExecution() {
  ...
  if (lastCycleStartTime_usec_ != 0) {
    ...
    if (timeBetweenCycles > MAIN_TOO_LATE_TIME_THRESH_USEC) {
      ...
      avgMainTooLateTime_usec_ =
          (u32)((f32)(avgMainTooLateTime_usec_ * (mainTooLateCnt_ - 1) +
                      timeBetweenCycles)) /
          mainTooLateCnt_;
  ...
  */
  ...
  // Now that the robot state msg has been updated, send mic data (which uses
  // some of robot state)
  ...
  if (cycleTime > MAIN_TOO_LONG_TIME_THRESH_USEC) {
    ...
    avgMainTooLongTime_usec_ =
        (u32)((f32)(avgMainTooLongTime_usec_ * (mainTooLongCnt_ - 1) +
                    cycleTime)) /
        mainTooLongCnt_;
  ...
  if (nextMainCycleTimeErrorReportTime_usec_ < cycleEndTime) {
    const bool reportTooLate =
        (mainTooLateCnt_ >= MIN_TOO_LATE_COUNT_PER_REPORTING_PERIOD) ||
        (maxMainTooLateTime_usec_ >= INSTANT_WARNING_TOO_LATE_TIME_THRESH_USEC);
    const bool reportTooLong =
        (mainTooLongCnt_ >= MIN_TOO_LONG_COUNT_PER_REPORTING_PERIOD) ||
    ...
    if (reportTooLate || reportTooLong) {
      AnkiWarn("CozmoBot.MainCycleTimeError",
               "TooLate: %d tics, avg: %d us, max: %d us, TooLong: %d tics, "
               "avg: %d us, max: %d us",
               mainTooLateCnt_, avgMainTooLateTime_usec_,
               maxMainTooLateTime_usec_, mainTooLongCnt_,
               avgMainTooLongTime_usec_, maxMainTooLongTime_usec_);

      if (reportTooLate) {
        DASMSG(vectorbot_main_cycle_too_late, "vectorbot.main_cycle_too_late",
               "Report upto once/sec when robot tick is too late");
        DASMSG_SET(
            i1, mainTooLateCnt_,
            "Number of ticks that exceed MAIN_TOO_LATE_TIME_THRESH_USEC");
        DASMSG_SET(i2, avgMainTooLateTime_usec_,
                   "Of the late ticks, average time since start of previous "
                   "tick (usec)");
        DASMSG_SET(i3, maxMainTooLateTime_usec_,
                   "Of the late ticks, maximum time since start of previous "
                   "tick (usec)");
        DASMSG_SEND_WARNING();
      }
      if (reportTooLong) {
        DASMSG(vectorbot_main_cycle_too_long, "vectorbot.main_cycle_too_long",
               "Report upto once/sec when robot tick is too long");
        DASMSG_SET(
            i1, mainTooLongCnt_,
            "Number of ticks that exceed MAIN_TOO_LONG_TIME_THRESH_USEC");
        DASMSG_SET(i2, avgMainTooLongTime_usec_,
                   "Average duration of too long ticks (usec)");
        DASMSG_SET(i3, maxMainTooLongTime_usec_,
                   "Maximum duration of too long ticks (usec)");
        DASMSG_SEND_WARNING();
      }
    }
    ...
    nextMainCycleTimeErrorReportTime_usec_ =
        cycleEndTime + MAIN_CYCLE_ERROR_REPORTING_PERIOD_USEC;
  }
