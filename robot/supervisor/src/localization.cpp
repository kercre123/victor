...
// 2 == Assumes total dist travelled per tic is discounted more the sharper you
// turn
...
const u8 POSE_HISTORY_SIZE =
    2000 /
    (ROBOT_TIME_STEP_MS * POSE_HISTORY_RES_IN_CYCLES);  // 2000ms of history
...
// Interpolates the pose at time targetTime which should be between historical
// pose1 time and historical pose2 time. Puts result in history at index
// poseResult_idx.
Result InterpolatePose(int pose1_idx, int pose2_idx, f32 targetTime,
                       int poseResult_idx) {
  PoseStamp *p1 = &(hist_[pose1_idx]);
  PoseStamp *p2 = &(hist_[pose2_idx]);

  if (p1->t > p2->t) {
    AnkiWarn("Localization.InterpolatePose.PosesOutOfOrder",
             "pose2 is older than pose1");
    return RESULT_FAIL;
  }

  if (targetTime < p1->t || targetTime > p2->t) {
    AnkiWarn("Localization.InterpolatePose.TargetTimeOOR",
...
Result UpdatePoseWithKeyframe(PoseOriginID_t originID, PoseFrameID_t frameID,
                              TimeStamp_t t, const f32 x, const f32 y,
                              const f32 angle) {
  // Update frameID
  frameId_ = frameID;
  originId_ = originID;
  ...
  u16 i;
  if (t == 0) {
    // If t==0, this is considered to be a command to just update the current
    // pose
    AnkiInfo("Localization.UpdatePoseWithKeyFrame.SettingPose",
             "x= %f, y= %f, angle= %f", x, y, angle);
    SetCurrPose(x, y, angle);
    return RESULT_OK;
  } else if (GetHistIdx(t, i) == RESULT_FAIL) {
    AnkiWarn(
        "Localization.UpdatePoseWithKeyFrame.TimeNotFound",
...
// timestamped older than lastKeyFrameUpdate.
    ...
    AnkiInfo("Localization.UpdatePoseWithKeyFrame.IgnoreOldKeyframe",
             "Ignoring keyframe %d at time %d\n", frameID, t);
#endif
    return RESULT_OK;
  }

  // If origins match, then chain historical poses per frame up to the current
  // pose
  if (hist_[i].originID == originID) {
    // Start at the absolute localization pose we have recieved and add any pose
    // information added to history since its timestamp. This is done by
    // chaining together the total pose tranformations within each pose frame in
    // history (time t to current).
    ...
    // History is stored in a circular buffer so make sure idxAfterEnd will be
    // valid
    const u16 idxAfterEnd = (hEnd_ == POSE_HISTORY_SIZE - 1 ? 0 : hEnd_ + 1);

    // Starting at time t go through history and find the starting and ending
    // pose for each frameID in history
    while (idx != idxAfterEnd) {
      ...
      // Increment idx until frameIDs in history change or we reach the end of
      // history
      ...
      // Otherwise use the pose at the end of this frame
        ...
        // idx is pointing to the entry that has the incremented frameID so we
        // need to subtract one to point to the last entry for this frame. We
        // also have to account for the fact that history is stored in a
        // circular buffer
        const u16 idxAtEndOfFrame =
            (idx == 0 ? POSE_HISTORY_SIZE - 1 : idx - 1);
      ...
      chainedPose =
          poseAtStartOfFrame.GetWithRespectTo(poseAtEndOfFrame) * chainedPose;
    }
  ...
  // We need to rely on t because we can receive several messages for the same t
  // with minor HAL timer differences. We want to process those messages as if
  // they were of the same time (t)
...
Result GetHistPoseAtTime(TimeStamp_t t, Anki::Embedded::Pose2d &p) {
  ...
  if (hist_[hStart_].t > t) {
    AnkiWarn("Localization.GetHistPoseAtTime.TimeTooOld",
             "History starts at time %d, pose requested at time %d. Returning "
             "oldest pose.\n",
             hist_[hStart_].t, t);
  ...
  if (hist_[hEnd_].t < t) {
    AnkiWarn("Localization.GetHistPoseAtTime.TimeTooNew",
             "History ends at time %d, pose requested at time %d. Returning "
             "newest pose.\n",
             hist_[hEnd_].t, t);
...
void Update() {
  ...
  // Compute new pose based on encoders and gyros, but only if there was any
  // motion.
  ...
  if (movement) {
    ...
    printf("\ncurrWheelPos (%f, %f)   prevWheelPos (%f, %f)\n",
           currLeftWheelPos, currRightWheelPos, prevLeftWheelPos_,
           prevRightWheelPos_);
#endif

    f32 lRadius, rRadius;  // Radii of each wheel arc path (+ve radius means
                           // origin of arc is to the left)
...
#if (DEBUG_LOCALIZATION)
    printf(
        "lRadius %f, rRadius %f, lDist %f, rDist %f, cTheta %f, cDist %f, "
        "cRadius %f\n",
    ...
    if (ABS(cRadius) >= BIG_RADIUS) {
      ...
      */
      ...
      // Clip according to expected cTheta since there's no way it can be more
      // than that
      ...
      /*
       ...
       // measured rotation (cTheta_meas) / the expected rotation based on
       encoders (cTheta)
      ...
      // Drawing a straight line from the previous position to the new position
      // forms a chord in the circle defined by the turning radius as determined
      // by the incremental wheel motion this tick. The angle of this circle
      // that this chord spans is cTheta. The angle of the chord relative to the
      // robot's previous trajectory is cTheta / 2.
      f32 cThetaHalf = 0.f;
      /*
  ...
  if (shouldUpdateDisplay && HAL::GetTimeStamp() % 100 == 0) {
    ...
    SetText(CURR_EST_POSE, "Est. Pose: (x,y)=(%.4f, %.4f) at deg=%.1f", x_, y_,
...
f32 GetCurrPose_x() { return x_; }

f32 GetCurrPose_y() { return y_; }

const Radians &GetCurrPose_angle() { return orientation_; }
...
void ConvertToDriveCenterPose(const Anki::Embedded::Pose2d &robotOriginPose,
  ...
  driveCenterPose.x() =
      robotOriginPose.GetX() + GetDriveCenterOffset() * cosf(angle);
  driveCenterPose.y() =
      robotOriginPose.GetY() + GetDriveCenterOffset() * sinf(angle);
...
void ConvertToOriginPose(const Anki::Embedded::Pose2d &driveCenterPose,
  ...
  robotOriginPose.x() =
      driveCenterPose.GetX() - GetDriveCenterOffset() * cosf(angle);
  robotOriginPose.y() =
      driveCenterPose.GetY() - GetDriveCenterOffset() * sinf(angle);
  robotOriginPose.angle() = driveCenterPose.GetAngle();
}

PoseFrameID_t GetPoseFrameId() { return frameId_; }

PoseOriginID_t GetPoseOriginId() { return originId_; }
...
void SetMotionModelParams(f32 slipFactor) { slipFactor_ = slipFactor; }
