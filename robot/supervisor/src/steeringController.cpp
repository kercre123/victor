...
// For checking if robot is not turning despite maxed integral power being
// applied
...
// If angular distance to target is less than this value, zero the P and I
// terms, and clear error sum (to combat stiction). This effectively limits the
// accuracy of the turn to this amount (keep it small)
const f32 POINT_TURN_CONTROLLER_PI_DEADBAND_RAD = DEG_TO_RAD_F32(0.5f);

// Integrator only accumulates if the angular distance to target is within this
// value, and is zeroed if not
const f32 POINT_TURN_INTEGRATOR_THRESH_RAD = DEG_TO_RAD_F32(5.f);

// Amount by which to reduce P and D gains for low speed turns, to prevent
// controller 'stuttering' The thresholds below determine when this is active.
// (this should be between 0 and 1)
const f32 POINT_TURN_LOW_SPEED_GAIN_REDUCTION_FACTOR = 0.5f;

// For turns that remain below this speed the entire time (or if the commanded
// velocity is below this) the P and D gains are reduced to avoid controller
// stuttering
const f32 POINT_TURN_REDUCE_GAINS_SPEED_THRESH_RAD_PER_S = DEG_TO_RAD_F32(40.f);

// For turns with a commanded accel below this threshold, reduce the P and D
// gains if the commanded velocity is below a threshold (to prevent controller
// stuttering at low speeds)
const f32 POINT_TURN_REDUCE_GAINS_ACCEL_THRESH_RAD_PER_S2 =
    DEG_TO_RAD_F32(100.f);
...
void SetGains(f32 k1, f32 k2, f32 dockPathDistOffsetCap_mm,
              f32 dockPathAngularOffsetCap_rad) {
  AnkiInfo("SteeringController.SetGains",
           "New gains: k1 %f, k2 %f, dist_cap %f mm, ang_cap %f rad", k1, k2,
           dockPathDistOffsetCap_mm, dockPathAngularOffsetCap_rad);
  K1_ = k1;
  K2_ = k2;
...
f32 GetK1Gain() { return K1_; }

f32 GetK2Gain() { return K2_; }

f32 GetPathDistOffsetCap() { return PATH_DIST_OFFSET_CAP_MM; }

f32 GetPathAngOffsetCap() { return PATH_ANGULAR_OFFSET_CAP_RAD; }
...
void SetPointTurnGains(f32 kp, f32 ki, f32 kd, f32 maxIntegralError) {
  ...
  */
  AnkiInfo("SteeringController.SetPointTurnGains",
           "New gains: kp %f, ki %f, kd %f, maxSum %f", kp, ki, kd,
           maxIntegralError);
}

SteerMode GetMode() { return currSteerMode_; }

// This manages at a high level what the steering controller needs to do (steer,
// use open loop, etc.)
void Manage() {
  if (!enable_) {
    return;
...
f32 GetRotationSpeedLimit() { return maxRotationWheelSpeedDiff; }

void DisableRotationSpeedLimit() { maxRotationWheelSpeedDiff = 0.f; }

// 1) If either wheel is going faster than possible, shift both speeds down by
// an offset to preserve curvature. 2) If wheel speeds will cause robot to turn
// faster than permitted, shift both wheel speed towards each other.
  ...
  if (maxRotationWheelSpeedDiff > 0) {
    ...
    if (wheelSpeedDiff > maxRotationWheelSpeedDiff) {
      ...
      *lowerWheelSpeed += speedAdjust;
      //          AnkiInfo( "SteeringController", "  Wheel speed adjust: (%f,
      //          %f), adjustment %f", *higherWheelSpeed, *lowerWheelSpeed,
      //          speedAdjust);
    }
  }
}
...
/**
 * @brief     Crosstrack steering controller. Modification of original
 * controller provided by Gabe Hoffmann for Drive.
 * @details   This control law uses the crosstrack error, heading error, and
 * vehicle speed to determine appropriate left and right wheel pwm commands to
 * converge on zero crosstrack error and zero heading error. The controller
 * operates by attempting to turn the vehicle to have a heading w.r.t the path
 * that is the arctan of a gain times the crosstrack error, normalized by speed.
 * Due to the normalization by speed, it should converge at a constant rate for
 * a given gain.
 *
 * @author    Gabe Hoffmann (gabe.hoffmann@gmail.com), Hanns Tappeiner, Kevin
 * Yoon
 * @copyright This original code is provided to Anki, Inc under royalty-free
 * non-exclusive license per terms of consulting agreement. Gabe Hoffmann
 * retains ownership of previous work herein: the control law, clipping logic,
 * and filter.
 * @param offsetError_mm    The crosstrack location (or shortest distance to the
 * currently followed path segment) of the robot
 * @param headingError_rad  The angular error between the robot's current
 * orientation and the tangent of the closest point on the current path.
 */
void RunLineFollowControllerNL(f32 offsetError_mm, float headingError_rad) {
  ...
  // Get the controller desired speed for the current tic. This speed should
  // approach user desired speed.
  ...
  // If moving backwards, modify distance and angular error such that proper
  // curvature is computed below.
  if (desspeed < 0) {
    offsetError_mm *= -1;
  ...
  if (WheelController::AreWheelsMoving() &&
      ABS(desspeed) > SpeedController::SPEED_CONSIDER_VEHICLE_STOPPED_MM_S) {
    steering_active = TRUE;
  }

  // Disable steering while we are changing direction (going from forwards to
  // backwards or backwards to forwards)
  if ((currspeed > 0 && userDesiredSpeed < 0) ||
      (currspeed < 0 && userDesiredSpeed > 0)) {
    steering_active = FALSE;
  }

  // Desired behavior?  We probably only want the robot to actively steering
  // when it's attempting to follow a path. When it's not following a path, you
  // should be able to push it around freely.

  // Deactivate steering if: We are not really moving and the commanded speed is
  // zero (or smaller than 0+eps)
  if (!WheelController::AreWheelsMoving() &&
      ABS(desspeed) <= SpeedController::SPEED_CONSIDER_VEHICLE_STOPPED_MM_S) {
    steering_active = false;
  }
  ...
  if (steering_active == TRUE) {
    curvature =
        -K1_ * (atan_fast(K2_ * offsetError_mm / (ABS(currspeed) + 200)) -
                headingError_rad);
  } else {
    curvature = 0;
  }
...
#if (DEBUG_STEERING_CONTROLLER)
  AnkiDebug(
      "SteeringController.RunLineFollowControllerNL.Errors",
      "offsetError_mm: %f, headingError_rad: %f, curvature: %f, currSpeed: %d",
      offsetError_mm, headingError_rad, curvature, currspeed);
#endif

  // We are moving along a circle, so let's compute the speed for the single
  // wheels Let's interpret the delta_speed as a curvature: Curvature is 1/radius
  // Commanded speeds to wheels are desired speed + offsets for curvature

  // if delta speed is positive, the left wheel is supposed to turn slower, it
  // becomes the INNER wheel
  float leftspeed = (float)desspeed - WHEEL_DIST_HALF_MM * curvature * desspeed;

  // if delta speed is positive, the right wheel is supposed to turn faster, it
  // becomes the OUTER wheel
  float rightspeed =
      (float)desspeed + WHEEL_DIST_HALF_MM * curvature * desspeed;
...
#if (DEBUG_STEERING_CONTROLLER)
  AnkiDebug("SteeringController.RunLineFollowControllerNL.Speeds",
            "%d (L), %d (R)", wleft, wright);
#endif
...
void SetPathFollowMode() { currSteerMode_ = SM_PATH_FOLLOW; }
  ...
  if (PathFollower::IsTraversingPath()) {
    ...
    if (gotError) {
      ...
      if (DockingController::IsBusy()) {
        ...
        fidx =
            CLIP(fidx, -PATH_DIST_OFFSET_CAP_MM,
                 PATH_DIST_OFFSET_CAP_MM);  // TODO: Loosen this up the closer
                                            // we get to the block?????
        pathRadErr = CLIP(pathRadErr, -PATH_ANGULAR_OFFSET_CAP_RAD,
                          PATH_ANGULAR_OFFSET_CAP_RAD);
      }

      // PERIODIC_PRINT(1000,"fidx: %f, distErr %f, radErr %f\n", fidx,
      // pathDistErr, pathRadErr); PRINT("fidx: %f, distErr %f, radErr %f\n",
...
void ExecuteDirectDrive(f32 left_vel, f32 right_vel, f32 left_accel,
                        f32 right_accel) {
  if (!enable_) {
    AnkiDebug("SteeringController.ExecuteDirectDrive.Disabled",
              "lspeed: %f mm/s, rspeed: %f mm/s", left_vel, right_vel);
    return;
  }

  // AnkiDebug( "DIRECT DRIVE", "%d: %f   %f  (%f %f)", HAL::GetTimeStamp(),
  // left_vel, right_vel, left_accel, right_accel);
  ...
  if (currLeftVel > targetLeftVel_) leftAccelPerCycle_ *= -1;
  if (currRightVel > targetRightVel_) rightAccelPerCycle_ *= -1;
}
...
void ExecutePointTurn_Internal(f32 maxAngularVel, f32 angularAccel) {
  ...
  angularAccel_ = NEAR_ZERO(angularAccel) ? MAX_BODY_ROTATION_ACCEL_RAD_PER_SEC2
                                          : angularAccel;
  ...
  if (ABS(maxAngularVel_) < POINT_TURN_TERMINAL_VEL_RAD_PER_S) {
    maxAngularVel_ =
        POINT_TURN_TERMINAL_VEL_RAD_PER_S * (maxAngularVel_ > 0 ? 1 : -1);
    AnkiWarn("SteeringController.ExecutePointTurn_Internal.PointTurnTooSlow",
...
void ExecutePointTurn(f32 angularVel, f32 angularAccel) {
  if (!enable_) {
    AnkiDebug("SteeringController.ExecutePointTurn.Disabled", "Speed: %f rad/s",
              angularVel);
    return;
  }

  AnkiDebug("SteeringController.ExecutePointTurn_2.Params",
            "%d: vel %f, accel %f", HAL::GetTimeStamp(),
            RAD_TO_DEG_F32(angularVel), RAD_TO_DEG_F32(angularAccel));

  // Check for nan params
  if (isnan(angularVel) || isnan(angularAccel)) {
    AnkiWarn("SteeringController.ExecutePointTurn_2.NanParamFound", "%f, %f",
             angularVel, angularAccel);
    return;
  }

  if (fabsf(angularVel) > MAX_BODY_ROTATION_SPEED_RAD_PER_SEC) {
    AnkiWarn("SteeringController.ExecutePointTurn_2.PointTurnTooFast",
  ...
  vpg_.StartProfile(IMUFilter::GetRotationSpeed(), currAngle, maxAngularVel_,
                    angularAccel_, CONTROL_DT);
}
...
void ExecutePointTurn(f32 targetAngle, f32 maxAngularVel, f32 angularAccel,
                      f32 angularDecel, f32 angleTolerance, bool useShortestDir,
                      uint16_t numHalfRevolutions) {
  if (!enable_) {
    AnkiDebug("SteeringController.ExecutePointTurn.Disabled",
              "targetAngle: %f rad, Speed: %f rad/s", targetAngle,
              maxAngularVel);
    return;
  }

  AnkiDebug("SteeringController.ExecutePointTurn.Params",
            "%d: target %f, vel %f, accel %f, decel %f, tol %f, shortestDir "
            "%d, numHalfRevs %d",
            HAL::GetTimeStamp(), RAD_TO_DEG_F32(targetAngle),
            RAD_TO_DEG_F32(maxAngularVel), RAD_TO_DEG_F32(angularAccel),
            RAD_TO_DEG_F32(angularDecel), RAD_TO_DEG_F32(angleTolerance),
            useShortestDir, numHalfRevolutions);

  // Check for nan params
  if (isnan(targetAngle) || isnan(maxAngularVel) || isnan(angularAccel) ||
      isnan(angularDecel) || isnan(angleTolerance)) {
    AnkiWarn("SteeringController.ExecutePointTurn.NanParamFound",
             "%f, %f, %f, %f, %f", targetAngle, maxAngularVel, angularAccel,
             angularDecel, angleTolerance);
    return;
  }

  if (fabsf(maxAngularVel) > MAX_BODY_ROTATION_SPEED_RAD_PER_SEC) {
    AnkiWarn("SteeringController.ExecutePointTurn.PointTurnTooFast",
             "Speed of %f deg/s exceeds limit of %f deg/s. Clamping.",
             RAD_TO_DEG_F32(maxAngularVel),
             MAX_BODY_ROTATION_SPEED_DEG_PER_SEC);
    maxAngularVel =
        copysignf(MAX_BODY_ROTATION_SPEED_RAD_PER_SEC, maxAngularVel);
  }
  ...
  if (useShortestDir) {
    angularDistExpected_ =
        (targetAngle - Localization::GetCurrPose_angle()).ToFloat();
    // Update destAngle and maxAngularVel_ so that the shortest turn is executed
    // to reach the goal
    maxAngularVel_ = copysignf(maxAngularVel_, angularDistExpected_);
    destAngle = currAngle + angularDistExpected_;
  } else {
    // Compute target angle that is on the appropriate side of currAngle given
    // the maxAngularVel which determines the turning direction.
    ...
    while (fabsf(destAngle - currAngle) <
           M_PI_F * (numHalfRevolutions - 0.5f)) {
      destAngle += copysignf(2 * M_PI_F, maxAngularVel);
    }

    while (fabsf(destAngle - currAngle) >
           M_PI_F * (numHalfRevolutions + 1.5f)) {
      destAngle -= copysignf(2 * M_PI_F, maxAngularVel);
    }
  ...
  // Exit early if we're already within tolerance of the target and not
  // currently moving. (If wheels are moving we might be outside of tolerance by
  // the time they stop.)
  ...
  vpg_.StartProfile(IMUFilter::GetRotationSpeed(), currAngle, maxAngularVel_,
                    angularAccel_,
                    maxAngularVel_ > 0 ? POINT_TURN_TERMINAL_VEL_RAD_PER_S
                                       : -POINT_TURN_TERMINAL_VEL_RAD_PER_S,
                    destAngle, CONTROL_DT);
}
...
void ManagePointTurn() {
  ...
  // The only exception is that when the robot is being held, we don't care if
  // the pitch is out of the acceptable range, since some behaviors like the
  // HeldInPalm or the WhileInAir reactions can have actions that execute point
  // turns while the robot is tilted in mid-air, on a user's palm, etc.
  ...
  if (currPitch.getAbsoluteVal() > MAX_PITCH_POINT_TURN_RAD && !isBeingHeld) {
    ...
    AnkiInfo("SteeringController.ManagePointTurn.StoppingDueToPitch",
             "Pitch magnitude of %f [deg] is higher than max allowed pitch of "
             "%f [deg]",
  ...
  s16 arcVel = (s16)(currDesiredAngularVel * WHEEL_DIST_HALF_MM *
                     POINT_TURN_SLIP_COMP_FACTOR);  // mm/s
  ...
  if (isPointTurnWithTarget_) {
    ...
    if (fabsf(angularDistExpected_ - angularDistTraversed_) > M_PI_F) {
      // we still have at least 180 deg left in the turn, so don't compare
      // angles yet
    ...
    if (absAngularDistToTarget < pointTurnAngTol_) {
      if (inPositionStartTime_ == 0) {
        AnkiDebug("SteeringController.ManagePointTurn.InRange",
                  "distToTarget %f, currAngle %f, currDesired %f (currTime %d, "
                  "inPosTime %d)",
                  RAD_TO_DEG_F32(angularDistToTarget), currAngle.getDegrees(),
                  RAD_TO_DEG_F32(currDesiredAngle), HAL::GetTimeStamp(),
                  inPositionStartTime_);
        inPositionStartTime_ = HAL::GetTimeStamp();
      } else if (HAL::GetTimeStamp() - inPositionStartTime_ >
                     IN_POSITION_THRESHOLD_MS ||
                 !WheelController::AreWheelsMoving()) {
#if (DEBUG_STEERING_CONTROLLER)
        ...
        AnkiDebug("SteeringController.ManagePointTurn.Stopping",
                  "currAngle %f, currDesired %f, currVel %f, distTraversed %f, "
                  "distExpected %f,  wheelSpeeds %f %f, desSpeeds %f %f",
                  currAngle.getDegrees(), RAD_TO_DEG_F32(currDesiredAngle),
                  RAD_TO_DEG_F32(currDesiredAngularVel),
                  RAD_TO_DEG_F32(angularDistTraversed_),
                  RAD_TO_DEG_F32(angularDistExpected_), 0, 0, 0, 0);
#endif
        ExitPointTurn();
        return;
      }
    } else {
      AnkiDebugPeriodic(100, "SteeringController.ManagePointTurn.OOR",
                        "distToTarget %f, currAngle %f, currDesired %f "
                        "(currTime %d, inPosTime %d)",
                        RAD_TO_DEG_F32(angularDistToTarget),
      ...
      if (fabsf(pointTurnAngleErrorSum_) == pointTurnMaxIntegralError_) {
        if (pointTurnIntegralPowerMaxedStartTime_ == 0 ||
            fabsf((pointTurnIntegralPowerMaxedStartAngle_ - currAngle)
                      .ToFloat()) < POINT_TURN_STUCK_THRESHOLD_RAD) {
          pointTurnIntegralPowerMaxedStartTime_ = HAL::GetTimeStamp();
          pointTurnIntegralPowerMaxedStartAngle_ = currAngle;
        } else if (HAL::GetTimeStamp() - pointTurnIntegralPowerMaxedStartTime_ >
                   POINT_TURN_STUCK_THRESHOLD_MS) {
          AnkiInfo("SteeringController.ManagePointTurn.StoppingCuzStuck",
                   "currAngle %f, currDesired %f", currAngle.getDegrees(),
    ...
    const f32 angularDistToCurrDesiredAngle =
        (Radians(currDesiredAngle) - currAngle).ToFloat();
    f32 p = angularDistToCurrDesiredAngle * pointTurnKp_;
    f32 i = pointTurnAngleErrorSum_ * pointTurnKi_;
    f32 d = (angularDistToCurrDesiredAngle - prevPointTurnAngleError_) *
            pointTurnKd_;

    // Stiction correction - zero P and I terms if close to set point (prevents
    // limit cycle due to stiction) See Section 3.2 of
    ...
    // At low speeds, scale back the P and D terms to prevent controller
    // 'stuttering'. Only apply this to either turns that remain at low speed
    // the entire time, or if the accel is low enough for stuttering to be
    // noticeable.
    if ((fabsf(vpg_.GetMaxReachableVel()) <
         POINT_TURN_REDUCE_GAINS_SPEED_THRESH_RAD_PER_S) ||
        (fabsf(angularAccel_) <
             POINT_TURN_REDUCE_GAINS_ACCEL_THRESH_RAD_PER_S2 &&
         fabsf(currDesiredAngularVel) <
             POINT_TURN_REDUCE_GAINS_SPEED_THRESH_RAD_PER_S)) {
    ...
    if (absAngularDistToTarget < POINT_TURN_INTEGRATOR_THRESH_RAD) {
      pointTurnAngleErrorSum_ =
          pointTurnAngleErrorSum_ + angularDistToCurrDesiredAngle;
      pointTurnAngleErrorSum_ =
          CLIP(pointTurnAngleErrorSum_, -pointTurnMaxIntegralError_,
               pointTurnMaxIntegralError_);
    } else {
      pointTurnAngleErrorSum_ = 0.f;
    }

    //        AnkiDebug( "SteeringController.ManagePointTurn.Controller",
    //        "timestamp %d, currAngle %.1f, desAngle %.1f, currSpeed %.1f,
    //        desSpeed %.1f, arcVel %d, ff %.1f, p %.1f, i %.1f, d %.1f, errSum
    //        %.1f",
    //                  HAL::GetTimeStamp(),
    //                  currAngle.getDegrees(),
    //                  RAD_TO_DEG_F32(currDesiredAngle),
    ...
    arcVel += (s16)(angularSpeedError * pointTurnSpeedKp_ +
                    (angularSpeedError - prevPointTurnAngleError_) *
                        pointTurnSpeedKd_ +
    ...
    if (ABS(arcVel) < MAX_WHEEL_SPEED_MMPS) {
      ...
      pointTurnAngleErrorSum_ =
          CLIP(pointTurnAngleErrorSum_, -pointTurnSpeedMaxIntegralError_,
               pointTurnSpeedMaxIntegralError_);
    }

    // AnkiDebug( "PointTurnSpeed", "des %f deg/s, meas: %f deg/s, arcVel %d
    // mm/s, errorSum %f", RAD_TO_DEG_F32(currDesiredAngularVel),
    // RAD_TO_DEG_F32(currAngularSpeed), arcVel, pointTurnAngleErrorSum_ );
  }

#if (DEBUG_STEERING_CONTROLLER)
  AnkiDebug("SteeringController.ManagePointTurn.DistToTarget",
...
void ExecuteDriveCurvature(f32 speed, f32 curvatureRadius_mm, f32 accel) {
  if (!enable_) {
    AnkiDebug("SteeringController.ExecuteDriveCurvature.Disabled",
              "Speed: %f mm/s, curvature: %f mm", speed, curvatureRadius_mm);
    return;
  }
  ...
  if (curvatureRadius_mm == s16_MAX || curvatureRadius_mm == s16_MIN) {
    ...
    if (!NEAR_ZERO(accel)) {
      ...
      f32 leftSpeed_instant =
          avgWheelSpeed * (1.0f - WHEEL_DIST_HALF_MM / curvatureRadius_mm);
      f32 rightSpeed_instant =
          avgWheelSpeed * (1.0f + WHEEL_DIST_HALF_MM / curvatureRadius_mm);
      WheelController::SetDesiredWheelSpeeds(leftSpeed_instant,
                                             rightSpeed_instant);
      ...
      if (FLT_NEAR(speed, avgWheelSpeed)) {
        ...
        // Compute what the necessary accelerations are for the individual
        // wheels In the amount of time it should take to accelerate from
        // avgWheelSpeed to speed_mmps at a rate of accel, it should take the
        // same amount of time for leftSpeed_instant to accelerate to leftSpeed
        // at a rate of leftAccel, and rightSpeed_instant to accelerate to
        // rightSpeed at a rate of rightAccel e.g. (speed_mmps - avgWheelSpeed)
        // / accel == (leftSpeed - leftSpeed_instant) / leftAccel
        const f32 oneOverSpeedDiff = 1.f / (speed - avgWheelSpeed);
        leftAccel = accel * (leftSpeed - leftSpeed_instant) * oneOverSpeedDiff;
        rightAccel =
            accel * (rightSpeed - rightSpeed_instant) * oneOverSpeedDiff;
      }
    }
  }
...
void ExecutePointTurnToRecordedHeading(
    f32 offset_rad, f32 maxAngularVel_radPerSec, f32 angularAccel_radPerSec2,
    f32 angularDecel_radPerSec2, f32 angleTolerance_rad,
    uint16_t numHalfRevolutions, bool useShortestDir) {
  ExecutePointTurn(recordedHeading_ + offset_rad, maxAngularVel_radPerSec,
                   angularAccel_radPerSec2, angularDecel_radPerSec2,
                   angleTolerance_rad, useShortestDir, numHalfRevolutions);
}
