...
class SendHelperHeaderEmitter(ast.NodeVisitor):
    ...
    def visit_UnionDecl(self, node):
        for member in node.members():
            self.output.write('inline bool SendMessage(const {member_type}& msg, const bool reliable=true, const bool hot=false) {{ return Anki::Cozmo::HAL::RadioSendMessage(msg.GetBuffer(), msg.Size(), {member_tag}, reliable, hot); }}\n'.format(
