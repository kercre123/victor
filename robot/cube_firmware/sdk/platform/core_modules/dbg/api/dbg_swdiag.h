 ...
 *
 * SW profiling is a debug feature that provides user a configurable way to
 *analyze SW execution performance or behavior, such as timings, state machines,
 *bit field values and so on. It manages the SW state representation over HW
 *digital diagnostic signals (diagports), from the signals selection to the
 *toggling of real HW signals. HW signals representing the SW execution could
 *then be watched on a logic analyzer in parallel with HW internal signals, that
 *could be very useful for low level debugging.
 *
 * @{
 ****************************************************************************************
 */
...
#define DBG_SWDIAG(bank, field, value)                                  \
  if (sw_to_hw[DBG_SWDIAG_##bank] != DBG_SWDIAG_UNDEFINED) {            \
    SW_DIAG_SET(                                                        \
        ((SW_DIAG_GET()) & (~(DBG_SWDIAG_##bank##_##field##_MASK        \
                              << (8 * sw_to_hw[DBG_SWDIAG_##bank])))) | \
        ((((value << DBG_SWDIAG_##bank##_##field##_OFFSET) &            \
           DBG_SWDIAG_##bank##_##field##_MASK)                          \
          << (8 * sw_to_hw[DBG_SWDIAG_##bank]))));                      \
  }
