 ...
 *
 * @return              Pointer to the element found and removed (NULL
 *otherwise).
 ****************************************************************************************
 */
struct co_list_hdr *ke_queue_extract(
    struct co_list *const queue,
 ...
 *
 * This primitive use a comparison function from the parameter list to select
 *where the element must be inserted.
 *
 * @param[in]  queue    Pointer to the queue.
 * @param[in]  element  Pointer to the element to insert.
 * @param[in]  cmp      Comparison function (return true if first element has to
