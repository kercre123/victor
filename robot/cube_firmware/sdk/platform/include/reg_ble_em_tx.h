...
//#define REG_BLE_EM_TX_ADDR_GET(idx) (jump_table_struct[offset_em_tx] + (idx) *
//REG_BLE_EM_TX_SIZE)
#define REG_BLE_EM_TX_ADDR_GET(idx) \
  (jump_table_struct[offset_em_tx] + (idx)*REG_BLE_EM_TX_SIZE)
...
#define BLE_TXCNTL_ADDR (0x80000 + jump_table_struct[offset_em_tx])
...
__INLINE void ble_txcntl_pack(int elt_idx, uint8_t txdone, uint8_t txrdy,
                              uint16_t nextptr) {
  ...
  EM_BLE_WR(BLE_TXCNTL_ADDR + elt_idx * REG_BLE_EM_TX_SIZE,
            ((uint16_t)txdone << 15) | ((uint16_t)txrdy << 14) |
                ((uint16_t)nextptr << 0));
}

__INLINE void ble_txcntl_unpack(int elt_idx, uint8_t* txdone, uint8_t* txrdy,
                                uint16_t* nextptr) {
...
__INLINE void ble_txdone_setf(int elt_idx, uint8_t txdone) {
  ASSERT_ERR((((uint16_t)txdone << 15) & ~((uint16_t)0x00008000)) == 0);
  EM_BLE_WR(BLE_TXCNTL_ADDR + elt_idx * REG_BLE_EM_TX_SIZE,
            (EM_BLE_RD(BLE_TXCNTL_ADDR + elt_idx * REG_BLE_EM_TX_SIZE) &
             ~((uint16_t)0x00008000)) |
                ((uint16_t)txdone << 15));
}
...
__INLINE void ble_txphce_pack(int elt_idx, uint8_t txlen, uint8_t txmd,
                              uint8_t txsn, uint8_t txnesn, uint8_t txllid) {
  ...
  EM_BLE_WR(BLE_TXPHCE_ADDR + elt_idx * REG_BLE_EM_TX_SIZE,
            ((uint16_t)txlen << 8) | ((uint16_t)txmd << 4) |
                ((uint16_t)txsn << 3) | ((uint16_t)txnesn << 2) |
                ((uint16_t)txllid << 0));
}

__INLINE void ble_txphce_unpack(int elt_idx, uint8_t* txlen, uint8_t* txmd,
                                uint8_t* txsn, uint8_t* txnesn,
                                uint8_t* txllid) {
...
__INLINE void ble_txlen_setf(int elt_idx, uint8_t txlen) {
  ASSERT_ERR((((uint16_t)txlen << 8) & ~((uint16_t)0x00001F00)) == 0);
  EM_BLE_WR(BLE_TXPHCE_ADDR + elt_idx * REG_BLE_EM_TX_SIZE,
            (EM_BLE_RD(BLE_TXPHCE_ADDR + elt_idx * REG_BLE_EM_TX_SIZE) &
             ~((uint16_t)0x00001F00)) |
                ((uint16_t)txlen << 8));
}
...
__INLINE void ble_txphadv_pack(int elt_idx, uint8_t txadvlen, uint8_t txrxadd,
                               uint8_t txtxadd, uint8_t txtype) {
  ...
  EM_BLE_WR(BLE_TXPHADV_ADDR + elt_idx * REG_BLE_EM_TX_SIZE,
            ((uint16_t)txadvlen << 8) | ((uint16_t)txrxadd << 7) |
                ((uint16_t)txtxadd << 6) | ((uint16_t)txtype << 0));
}

__INLINE void ble_txphadv_unpack(int elt_idx, uint8_t* txadvlen,
                                 uint8_t* txrxadd, uint8_t* txtxadd,
                                 uint8_t* txtype) {
  uint16_t localVal =
      EM_BLE_RD(BLE_TXPHADV_ADDR + elt_idx * REG_BLE_EM_TX_SIZE);
...
__INLINE uint16_t ble_txbuf_getf(int elt_idx, int reg_idx) {
  ASSERT_ERR(reg_idx <= 18);
  uint16_t localVal =
      EM_BLE_RD(BLE_TXBUF_ADDR + elt_idx * REG_BLE_EM_TX_SIZE + reg_idx * 2);
...
__INLINE void ble_txbuf_setf(int elt_idx, int reg_idx, uint16_t txbuf) {
  ...
  EM_BLE_WR(BLE_TXBUF_ADDR + elt_idx * REG_BLE_EM_TX_SIZE + reg_idx * 2,
            (uint16_t)txbuf << 0);
}
