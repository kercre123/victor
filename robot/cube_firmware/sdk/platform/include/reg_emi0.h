...
__INLINE void emi0_extmemcntl0_pack(int reg_idx, uint8_t iftype0,
                                    uint8_t setupwr0, uint8_t holdwr0,
                                    uint8_t wswr0, uint8_t setuprd0,
                                    uint8_t holdrd0, uint8_t wsrd0) {
  ASSERT_ERR(reg_idx <= 1);
  ...
  REG_PL_WR(EMI0_EXTMEMCNTL0_ADDR + reg_idx * 4,
            ((uint32_t)iftype0 << 10) | ((uint32_t)setupwr0 << 9) |
                ((uint32_t)holdwr0 << 8) | ((uint32_t)wswr0 << 5) |
                ((uint32_t)setuprd0 << 4) | ((uint32_t)holdrd0 << 3) |
                ((uint32_t)wsrd0 << 0));
}

__INLINE void emi0_extmemcntl0_unpack(int reg_idx, uint8_t* iftype0,
                                      uint8_t* setupwr0, uint8_t* holdwr0,
                                      uint8_t* wswr0, uint8_t* setuprd0,
                                      uint8_t* holdrd0, uint8_t* wsrd0) {
  ASSERT_ERR(reg_idx <= 1);
...
__INLINE void emi0_if_type0_setf(int reg_idx, uint8_t iftype0) {
  ...
  ASSERT_ERR((((uint32_t)iftype0 << 10) & ~((uint32_t)0x00000C00)) == 0);
  REG_PL_WR(EMI0_EXTMEMCNTL0_ADDR + reg_idx * 4,
            (REG_PL_RD(EMI0_EXTMEMCNTL0_ADDR + reg_idx * 4) &
             ~((uint32_t)0x00000C00)) |
                ((uint32_t)iftype0 << 10));
}
