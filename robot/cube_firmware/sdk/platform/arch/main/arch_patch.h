...
/**
 * @brief Registers a task for receiving ATT read request indications
 * (ATTS_READ_REQ_IND).
...
/**
 * @brief Un-registers a task from receiving ATT read request indications
 * (ATTS_READ_REQ_IND).
 * @param[in] taskid    Id of the task.
 * @return void
 */
 ...
 * @param[in] conidx        The index of the connection that this reply applies
 * to.
 * @param[in] handle        The handle of the read response.
 * @param[in] status_code   If status_code = ATT_ERR_NO_ERROR then an ATT read
 * response is sent after reading the DB otherwise an ATT error response with
 * this status_code  is sent.
 * @return void
 *
