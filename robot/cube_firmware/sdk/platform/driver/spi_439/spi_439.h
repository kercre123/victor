...
#define SC14439_GPIO_PF_DEBOUNCE_REG \
  (0x122C) /* GPIO DEBOUNCE CONTROL REGISTER */

#define SC14439_CLASSD_CTRL_REG (0x1230) /* CLASSD CONTROL REGISTER */
#define SC14439_CLASSD_CLEAR_INT_REG \
  (0x1232)                                 /* CLASSD CLEAR INTERRUPT REGISTER */
#define SC14439_CLASSD_BUZZER_REG (0x1234) /* CLASSD BUZZER REGISTER */
#define SC14439_CLASSD_TEST_REG (0x1236)   /* CLASSD TEST REGISTER */

#define SC14439_DMA0_A_STARTL_REG \
  (0x1250) /* START ADDRESS LOW A OF DMA CHANNEL 0 */
#define SC14439_DMA0_A_IDX_REG \
  (0x1254) /* DMA RECEIVE INDEX A OF DMA CHANNEL 0 */
#define SC14439_DMA0_B_STARTL_REG \
  (0x1256) /* START ADDRESS LOW B OF DMA CHANNEL 0 */
#define SC14439_DMA0_B_IDX_REG \
  (0x125A) /* DMA RECEIVE INDEX B OF DMA CHANNEL 0 */
#define SC14439_DMA0_LEN_REG \
  (0x125E) /* DMA RECEIVE LENGTH REGISTER CHANNEL 0 */
#define SC14439_DMA0_CTRL_REG \
  (0x1260) /* CONTROL REGISTER FOR DMA CHANNEL 0    */
...
#define SC14439_CODEC_OFFSET1_REG \
  (0x128A) /* CODEC OFFSET ERROR AND COMPENSATION REGISTER */
#define SC14439_CODEC_TEST_CTRL_REG (0x128C) /* CODEC TEST CONTROL REGISTER */
#define SC14439_CODEC_OFFSET2_REG \
  (0x128E) /* CODEC OFFSET COMPENSATION REGISTER */

#define SC14439_PCM0_IN_OUT_REG \
  (0x12B0) /* PCM CHANNEL 0 INPUT/OUTPUT REGISTER */
#define SC14439_PCM1_IN_OUT_REG \
  (0x12B2)                             /* PCM CHANNEL 1 INPUT/OUTPUT REGISTER */
...
#define SC14439_PHASE_INFO_REG \
  (0x12EA) /* PHASE DIFFERENCE BETWEEN PCM_FSC AND 8/16KHZ */
...
****************************************************************************************
*/
uint32_t spi_439_getword(uint32_t dataToSend);
void spi_439_setword(uint32_t address, uint32_t data);
 ...
 *
 * Note that we do 32 bits SPI processing, so the 580 SPI thinks it is loading
 *32 bits values, and the 439 thinks it is doing 2 x 16 bits values.
 *
 * @return void
 ****************************************************************************************
 */
