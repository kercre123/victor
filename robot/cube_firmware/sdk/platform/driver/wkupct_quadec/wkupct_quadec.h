...
/* Important note: If, upon reception of interrupt from the wakeup timer or the
 * quadrature decoder, the system resumes from sleep mode and you wish to resume
 * peripherals functionality , it is necessary to include in your interrupt
 * handler function(s)
 *                 - the ones you register using wkupct_register_callback()
 * and/or quad_decoder_register_callback() - the following lines:
 *
 *                    // Init System Power Domain blocks: GPIO, WD Timer, Sys
 * Timer, etc.
...
enum {
  WKUPCT_PIN_POLARITY_HIGH =
      0,  // the event counter is incremented if the respective input goes HIGH
  WKUPCT_PIN_POLARITY_LOW =
      1,  // the event counter is incremented if the respective input goes LOW
};

/**
 ****************************************************************************************
 * @brief Helper macro for the "pin selection" bitmask expected by function
 *wkupct_enable_irq().
 * @param[in] port    GPIO port
 * @param[in] pin     GPIO pin
 * @return The bitmask value that selects the given GPIO
 * @note When the macro is used with constant arguments an optimizing compiler
 *will collapse it to a constant value. If the macro arguments contain an
 *illegal port/pin pair then a compilation error will be generated.
 ****************************************************************************************
 */
#define WKUPCT_PIN_SELECT(port, pin)                                       \
  (1U << (port == GPIO_PORT_0 && pin <= GPIO_PIN_7                         \
              ? pin                                                        \
              : port == GPIO_PORT_1 && pin <= GPIO_PIN_5                   \
                    ? 8 + pin                                              \
                    : port == GPIO_PORT_2 && pin == GPIO_PIN_8             \
                          ? 14                                             \
                          : port == GPIO_PORT_2 && pin == GPIO_PIN_9       \
                                ? 15                                       \
                                : port == GPIO_PORT_2 && pin <= GPIO_PIN_7 \
                                      ? 16 + pin                           \
                                      : port == GPIO_PORT_3 &&             \
                                                pin <= GPIO_PIN_7          \
                                            ? 24 + pin                     \
...
/**
 ****************************************************************************************
 * @brief Helper macro for the "pin polarity" bitmask expected by function
 *wkupct_enable_irq().
 * @param[in] port        GPIO port
 * @param[in] pin         GPIO pin
 * @param[in] polarity    The required polarity. Can be either
 *WKUPCT_PIN_POLARITY_HIGH or  WKUPCT_PIN_POLARITY_LOW.
 ...
 ****************************************************************************************
 */
#define WKUPCT_PIN_POLARITY(port, pin, polarity)                             \
  ((polarity) << (port == GPIO_PORT_0 && pin <= GPIO_PIN_7                   \
                      ? pin                                                  \
                      : port == GPIO_PORT_1 && pin <= GPIO_PIN_5             \
                            ? 8 + pin                                        \
                            : port == GPIO_PORT_2 && pin == GPIO_PIN_8       \
...
typedef void (*quad_encoder_handler_function_t)(int16_t qdec_xcnt_reg,
                                                int16_t qdec_ycnt_reg,
                                                int16_t qdec_zcnt_reg);
...
/**
 ****************************************************************************************
 * @brief Internal function to disable WKUP_QUADEC_IRQn only when both
 *Quadrature Decoder and Wakeup Timer have unregistered from using it.
 ...
 ****************************************************************************************
 */
void wkupct_enable_irq(uint32_t sel_pins, uint32_t pol_pins,
                       uint16_t events_num, uint16_t deb_time);

/**
 ****************************************************************************************
 * @brief Unregisters WKUP_QUADEC_IRQn use by Wakeup Timer and calls
 * wkupct_quad_disable_IRQ.
 * @note WKUP_QUADEC_IRQn interrupt will be disabled, only if also the
 *Quadrature Decoder has unregistered from usin the IRQ.
 ...
 * wkupct_quad_disable_IRQ.
 * @note WKUP_QUADEC_IRQn interrupt will be disabled, only if also the Wakeup
 *Timer has unregisterd from usin the IRQ.
 * @return WKUPCT_QUADEC_ERR_OK, WKUPCT_QUADEC_ERR_RESERVED
 ****************************************************************************************
 */
