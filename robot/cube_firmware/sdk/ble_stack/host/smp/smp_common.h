...
* @file smp_common.h
*
* @brief Header file - Security Manager Protocol Common Definitions and
*Functions.
*
...
/// Extract the Pairing Failed reason value from a masked Pairing Failed reason
/// value.
...
enum smp_pair_fail_reason {
   ...
   * The user input of passkey failed, for example, the user cancelled the
   * operation.
   */
...
enum smp_prop_error {
   ...
   * The request sent by the HL cannot be handled for some reasons (unauthorized
   * source task, role, ...)
   */
  SMP_ERROR_REQ_DISALLOWED =
      ((SMP_PAIR_FAIL_REASON_MASK | SMP_ERROR_INVALID_PARAM) + 1),
  /**
  ...
  /**
   * Timeout (0x6F)
   * The command cannot be executed because a SMP timeout has been raised during
   * the connection.
   */
  SMP_ERROR_TIMEOUT,
  /**
  ...
  SMP_ERROR_ENC_KEY_MISSING =
      ((SMP_PAIR_FAIL_REASON_REM_MASK | SMP_ERROR_INVALID_PARAM) + 1),
  /**
