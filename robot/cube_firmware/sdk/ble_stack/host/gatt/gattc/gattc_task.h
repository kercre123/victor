 ...
 * The task includes services and characteristic discovery, configuration
 *exchanges and attribute value access operations (reading, writing,
 *notification and indication).
 *
 * Messages may originate from @ref ATTC "ATTC", @ref ATTS "ATTS", @ref GAP
 *"GAP" and Application.
 *
 * @{
 ****************************************************************************************
...
/// Read by UUID: search UUID and read it's characteristic value
/// (GATTC_READ_BY_UUID) Note: it doesn't perform an automatic read long.
...
struct gattc_write_cmd {
  ...
  /// (if false, an ATT Prepare Write will be used this shall be use for
  /// reliable write)
  bool auto_execute;
  /// Attribute handle
  uint16_t handle;
...
/// Confirm modification of database from upper layer when requested by peer
/// device.
struct gattc_write_cmd_cfm {
  ...
  uint8_t status;
  /// Does a response should be sent back to peer device (@see
  /// gattc_write_cmd_ind)
  bool response;
};
...
/// Parameters for @ref GATTC_SVC_CHANGED_CFG_IND and @ref
/// GATTC_SVC_CHANGED_SET_CFG_REQ message
struct gattc_svc_changed_cfg {
  /**
   * Current value of the Client Characteristic Configuration descriptor for the
   * Service Changed characteristic
   */
  uint16_t ind_cfg;
};

/// Inform that a valid read request of database has been requested by peer
/// device.
