 ...
 *
 * It handles messages from lower and higher layers related to an ongoing
 *connection.
 *
 * @{
 ****************************************************************************************
...
enum gapc_msg_id {
  ...
  /// Master confirm or not that parameters proposed by slave are accepted or
  /// not
...
struct gapc_connection_cfm {
  ...
  uint8_t auth;
  /// Peer device is authorized to access database parameters requiring
  /// authorizations.
...
struct gapc_param_update_cmd {
  ...
  uint8_t operation;
  /// Internal parameter used to manage internally l2cap packet identifier for
  /// signaling
  uint8_t pkt_id;
#ifndef __DA14581__
...
struct gapc_bond_ind {
  ...
  union gapc_bond_data {
    ...
    struct gapc_irk irk;
    /// Connection Signature Resolving Key information (if info =
    /// GAPC_CSRK_EXCH)
    struct gap_sec_key csrk;
  } data;
};
