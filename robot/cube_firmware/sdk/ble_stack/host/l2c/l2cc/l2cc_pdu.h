...
struct l2cc_update_param_rsp {
  ...
  /// data length
  uint16_t length;
  /// Result field indicates the response to the Connection Parameter Update
  /// Request
...
struct l2cc_att_sign_wr_cmd {
  ...
  /// Authentication signature for the Attribute Upload, Attribute Handle and
 ...
 * @param[in|out]  p_offset    Pointer to address where the next part of PDU
 *will be read
 * @param[in|out]  p_length    Pointer to the length of payload that has been
 *packed
 * @param[in|out]  p_buffer    Pointer to address where the next part of PDU
 *will be written
 ...
 ****************************************************************************************
 */
uint8_t l2cc_pdu_pack(struct l2cc_pdu *p_pdu, uint16_t *p_offset,
                      uint16_t *p_length, uint8_t *p_buffer, uint8_t conidx,
                      uint8_t llid);

/**
 ****************************************************************************************
 * Unpack L2Cap PDU in a generic format that can be used by firmware using
 generic method.

 * @param[in]      p_pdu       Pointer to the unpacked PDU structure
 * @param[in|out]  p_offset    Pointer to address where the next part of PDU
 will be written
 * @param[in|out]  p_rem_len   Pointer to the remaining length value of PDU to
 receive
 * @param[in|out]  p_buffer    Pointer to address where the next part of PDU
 will be read
 ...
 ****************************************************************************************
 */
uint8_t l2cc_pdu_unpack(struct l2cc_pdu *p_pdu, uint16_t *p_offset,
                        uint16_t *p_rem_len, const uint8_t *p_buffer,
                        uint16_t pkt_length, uint8_t llid);
