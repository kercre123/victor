...
#define ATT_CHAR(prop, handle, type)                \
  {(prop),                                          \
   {(uint8_t)((handle)), (uint8_t)((handle) >> 8)}, \
   {(uint8_t)((type)), (uint8_t)((type) >> 8)}};

/// macro to fill an attribute characteristic
#define ATT_CHAR128(prop, handle, type) \
  {(prop), {(uint8_t)(handle), (uint8_t)(handle >> 8)}, (type)};

/// macro to fill characteristic format
#define ATT_FORMAT(form, exp, unit, name_space, desc) \
  {                                                   \
      (form),                                         \
      (exp),                                          \
      (uint8_t)(unit),                                \
...
// 4 last max length bit is kept to determine if attribute has a 128 bits UUID
// or not. and if it's a BT UUID or not
