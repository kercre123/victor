...
#define PERM(access, right)                           \
  (((PERM_RIGHT_##right) << (PERM_ACCESS_##access)) & \
   (PERM_ACCESS_MASK_##access))
...
#define ATT_GET_SEC_LVL(att_right, svc_right) \
  co_max(((att_right)&PERM_RIGHT_MASK_AUTH),  \
         ((svc_right)&PERM_RIGHT_MASK_AUTH));

/// Retrieve attribute authorization from attribute right and service right
#define ATT_GET_AUTHZ(att_right, svc_right) \
  ((((att_right)&PERM_RIGHT_MASK_AUTHZ) |   \
    ((svc_right)&PERM_RIGHT_MASK_AUTHZ)) == PERM_RIGHT_AUTHZ)
...
enum {
  ...
  TYPE_UUID_32,
  /// 128 Bits BT UUID (takes 2 bytes in ATTS_DB, only significant bits are
  /// stored in
  ///                   attribute database).
  TYPE_UUID_128_BT,
...
struct attm_elmt {
  ...
  /// 2 MSB bits are used to know if UUID is a 128 bits UUID and if its a BT
  /// UUID max length = 2^14 bytes
...
struct attm_env_tag {
   ...
   *
   * The Attribute database is a list of attribute services sorted by handle
   *number. This database shall be initiate by GAP, GATT, profiles and
   *application process at startup and must not change during runtime.
   *
   * Database initialization shall be deterministic in order to always have
   *service handle at same position in database during all product life-cycle.
   *This is required since database client can save position of services in
   *database to not perform service discovery at each connection.
   ***************************************************************************************
   */
 ...
 ****************************************************************************************
 */
bool attm_uuid_match(uint8_t *uuid_a, uint8_t uuid_a_len, uint8_t *uuid_b,
                     uint8_t uuid_b_len);
