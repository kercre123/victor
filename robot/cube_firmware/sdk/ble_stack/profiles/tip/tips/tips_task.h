 ...
 *
 * The TIPSTASK is responsible for handling the messages coming in and out of
 *the
...
enum {
  ...
  /// Inform APP about modification of Current Time Characteristic Client.
  /// Charact. Cfg
  TIPS_CURRENT_TIME_CCC_IND,
  /// Update Time Update State Request from APP
  TIPS_UPD_TIME_UPD_STATE_REQ,
  /// Inform APP about modification of Time Update Control Point Characteristic
  /// Value
...
struct tips_enable_req {
  /// Connection handle
  uint16_t conhdl;
  /// security level: b0= nothing, b1=unauthenticated, b2=authenticated,
  /// b3=authorized; b1 or b2 and b3 can go together
  uint8_t sec_lvl;
  /// Type of connection - will someday depend on button press length; can be
  /// CFG or NORMAL
  uint8_t con_type;
...
struct tips_upd_curr_time_req {
  ...
  /**
   * Indicate if the new Current Time value can be sent if the
   * current_time_ntf_en parameter is set to 1. (0 - Disable; 1- Enable)
   *
   * If the time of the Current Time Server is changed because of reference time
   * update (adjust reason) then no notifications shall be sent to the Current
   * Time Service Client within the 15 minutes from the last notification,
   * unless one of both of the two statements below are true :
   *         - The new time information differs by more than 1min from the
   * Current Time Server time previous to the update
   *         - The update was caused by the client (interacting with another
   * service)
   */
