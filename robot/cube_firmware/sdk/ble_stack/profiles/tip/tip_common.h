 ...
 * The Time Update Control Point Characteristic enables a client to issue a
 * command request to update the time in the time server. 0x01 = Get Reference
 * Update = Forces the state machine to Update Pending and starts the time
 * update procedure 0x02 = Cancel Reference Update = Forces the state machine to
 * idle and stops the attempt to receive a time update.
 */
