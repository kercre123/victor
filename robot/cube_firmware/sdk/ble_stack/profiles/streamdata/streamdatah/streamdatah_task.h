 ...
 *
 * The StreamData Host Task is responsible for handling the API messages
 *received from either the Application or internal tasks.
 *
 * @{
 ****************************************************************************************
 */
...
enum {
  ...
  /// Inform APP that the profile client role has been disabled after a
  /// disconnection
...
struct streamdatah_wr_alert_lvl_req {
  /// Connection handle
  uint16_t conhdl;
  /// 0=LLS or 1=IAS, code for the service in which the alert level should be
  /// written
  uint8_t svc_code;
  /// Alert level
  uint8_t lvl;
...
struct streamdatah_rd_char_rsp {
  /// Connection handle
  uint16_t conhdl;
  /// Read characteristic response status code, may be GATT code or ATT error
  /// code.
  uint8_t status;
  /// Char. Code
  uint8_t char_code;
...
struct streamdatah_wr_char_rsp {
  /// Connection handle
  uint16_t conhdl;
  /// Write characteristic response status code, may be GATT code or ATT error
  /// code.
  uint8_t status;
};
...
extern const struct ke_state_handler
    streamdatah_state_handler[STREAMDATAH_STATE_MAX];
