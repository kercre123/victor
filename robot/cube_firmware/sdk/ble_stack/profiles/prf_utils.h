 ...
 * @ingroup PROFILE
 *
 * @brief Definitions of shared profiles functions that can be used by several
 *profiles
 *
 * @{
 ****************************************************************************************
...
#define PRF_CLIENT_ENABLE(con_info, param, type)                            \
  (prf_client_enable((prf_env_struct***)&type##_envs, &(con_info), (param), \
                     sizeof(struct type##_env_tag)))
...
#define PRF_CLIENT_RESET(envs, type)                                      \
  (prf_client_reset((prf_env_struct***)&(envs), TASK_##type, type##_IDLE, \
                    &TASK_DESC_##type, type##_CLEANUP_FNCT))
...
#define PRF_CLIENT_GET_ENV(prf_id, type)                                     \
  ((struct type##_env_tag*)prf_client_get_env((prf_env_struct**)type##_envs, \
                                              prf_id))
...
#define PRF_CLIENT_DISABLE_IND_SEND(envs, task_id, type, conhdl)             \
  (prf_client_disable_ind_send((prf_env_struct***)&envs, type##_DISABLE_IND, \
                               task_id, type##_IDLE, (conhdl),               \
                               type##_CLEANUP_FNCT))
...
#define PRF_CLIENT_ENABLE_ERROR(envs, prf_task_id, type)          \
  (prf_client_enable_error((prf_env_struct***)&envs, prf_task_id, \
                           type##_DISCOVERING, type##_IDLE,       \
                           type##_CLEANUP_FNCT))
 ...
 *
 * @param con_info Pointer to connection information (connection handle, app
 *task id, profile task id)
 *
 ...
 ****************************************************************************************
 */
void prf_read_char_send(struct prf_con_info* con_info, uint16_t shdl,
 ...
 ****************************************************************************************
 */
void prf_register_atthdl2gatt(struct prf_con_info* con_info,
                              struct prf_svc* svc);
 ...
 ****************************************************************************************
 */
void prf_unregister_atthdl2gatt(struct prf_con_info* con_info,
                                struct prf_svc* svc);
 ...
 *
 * It will request discovery of all characteristic descriptors for the giben
 *service
 *
 * @param con_info pointer to connection information (connection handle, app
 *task id, profile task id)
 ...
 ****************************************************************************************
 */
void prf_disc_char_desc_send(struct prf_con_info* con_info,
 ...
 *
 * @param[in] con_info pointer to connection information (connection handle, app
 *task id, profile task id)
 ...
 ****************************************************************************************
 */
void prf_gatt_write(struct prf_con_info* con_info, uint16_t handle,
                    uint8_t* value, uint16_t length, uint8_t req_type);

/**
 ****************************************************************************************
 * @brief Modify peer client configuration descriptor using GATT
 *
 * It will request write modification of peer client configuration descriptor
 *handle
 *
 * @param[in] con_info pointer to connection information (connection handle, app
 *task id, profile task id)
 ...
 *
 * If some handles are not present, it checks if they shall be present or they
 *are optional.
 *
 * @param nb_chars   Number of Characteristics in the service
 * @param chars      Characteristics values (char handles, val handles,
 *properties)
 ...
 *
 * If some handles are not present, according to characteristic properties it
 *verify if descriptor is optional or not.
 *
 ...
 ****************************************************************************************
 */
uint8_t prf_check_svc_char_desc_validity(
    uint8_t descs_size, const struct prf_char_desc_inf* descs,
 ...
 *
 * @param svc_ehdl         End handle of the service for which characteristics
 *are discovered
 ...
 * @param last_found_char  Characteristics code of the last characteristic that
 *has been found.
 ****************************************************************************************
 */
 ...
 * @param last_char_code   Characteristics code of the characteristic for which
 *the gatt_disc_char_desc_req has been sent
 ****************************************************************************************
 */
void prf_search_descs(uint8_t nb_descs, struct prf_char_desc_inf* descs,
                      const struct prf_char_desc_def* descs_req,
...
/**
 ****************************************************************************************
 * @brief This function is called from a profile client role task to allocate an
 *environment for a specified connection (p_con_info). If the prf_id parameter
 *doesn't match the connection index, the [PRF]_ENABLE_REQ message (p_param) is
 *forwarded to the good profile task id.
 *
 * An environment whose size is env_size will be added in the pool of
 *environments (p_envs). This pool is allocated if it didn't exist.
 *
 ...
 *
 * @return  PRF_ERR_OK                      If the environment has been
 *successfully allocated. PRF_ERR_REQ_DISALLOWED          If the allocation has
 *failed because of a lack of memory. PRF_ERR_INVALID_PARAM           The
 *profile is already enable for this connection. PRF_ERR_FEATURE_NOT_SUPPORTED
 *The message has been forwarded to another task instance.
 ****************************************************************************************
 */
uint8_t prf_client_enable(prf_env_struct*** p_envs,
                          struct prf_con_info* p_con_info, void const* p_param,
                          uint16_t env_size);

/**
 ****************************************************************************************
 * @brief This function is called from a profile client role task to free an
 *environment for a specified connection (idx). If the removed environment was
 *the last one for the profile, the pool of environment is free.
 *
 * @param[in/out] p_envs    Pointer to the profile environment variable
 * @param[in] idx           Connection Index
 * @param[in] cleanup       Clean-up function used to clean environment
 *variables allocated by profile.
 *
 * @return PRF_ERR_OK               If the operation succeed.
 * @return PRF_ERR_INVALID_PARAM    If the environment for the provided
 *connection index doesn't exist.
 ****************************************************************************************
 */
 ...
 * All environments are removed and the pool of environment is removed. The
 *state of all task is set to state.
 *
 * @param[in/out] p_envs        Pointer to the profile environment variable
...
/**
 ****************************************************************************************
 * @brief Return the address of the profile client role environment linked to
 *the profile task instance whose id is task_id.
 *
 * @param[in/out] p_envs        Pointer to the profile environment variable
 ...
 ****************************************************************************************
 */
prf_env_struct* prf_client_get_env(prf_env_struct** p_envs,
                                   ke_task_id_t task_id);

/**
 ****************************************************************************************
 * @brief The function is used in a profile client role task to inform the
 *application that the profile has been disabled after a disconnection.
 *
 * @param[in/out] p_envs        Pointer to the profile environment variable
 ...
 * @param conhdl                Connection Handle TODO[FBE] remove it as soon as
 *conhdl remove for external API
 * @param[in] cleanup           Clean-up function used to clean environment
 *variables allocated by profile.
 ****************************************************************************************
 */
void prf_client_disable_ind_send(prf_env_struct*** p_envs, ke_msg_id_t msg_id,
                                 ...
                                 uint16_t conhdl, prf_cleanup_cb cleanup);

/**
 ****************************************************************************************
 * @brief The function is used when an error has been raised during the enabling
 *of a profile client role task. Its purpose is to check the current state of
 *the task in order to come back to the initial configuration if needed
 *
 * @param[in/out] p_envs        Pointer to the profile environment variable
...
/**
 ****************************************************************************************
 * @brief The function is used in a profile server role task to inform the
 *application that an error has occurred in the msg_id message handler.
 *
 * @param p_env                 Profile server role task environment
 * @param status                Error status
 * @param ind_msg_id            [PRF]_ERROR_IND message id
 * @param msg_id                Message ID of the message for which an error has
 *been raised
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief The function is used in a profile server role task to trigger an event
 *to peer device (indication/notification) about new attribute value
 *modification.
 *
 * @param p_env                 Profile server role task environment
 * @param indication            True if it's an indication, False if it's a
 *notification
