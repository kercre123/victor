...
#define UDS_IS(FLAG) \
  ((udss_env.flags & (1 << (UDS_##FLAG))) == (1 << (UDS_##FLAG)))
 ...
 ****************************************************************************************
 */
uint8_t udss_pack_ucp_rsp(uint8_t* packed_val, struct uds_ucp_rsp* ucp_rsp);
 ...
 * @param[in] ucp_ind_src Application that requests to send UCP response
 *indication.
 * @return PRF_ERR_IND_DISABLED if indication disabled by peer device,
 *PRF_ERR_OK else.
 ****************************************************************************************
 */
