 ...
 *
 * An actual thermometer device does not exist on current platform, so
 *measurement values that would come from a driver are replaced by simple
 *counters sent at certain intervals following by the profile attributes
 *configuration. When a measurement interval has been set to a non-zero value in
 *a configuration connection, once reconnected, TH will send regular measurement
 *INDs if Temp Meas Char Cfg is set to indicate and using the Meas Intv Value.
 *The INDs will continue until meas interval is set to 0 or connection gets
 *disconnected by C. Measurements should be stored even so, until profile is
 *disabled.(?)
 *
 * If the measurement interval has been set to 0, then if Intermediate Temp is
 *set to be notified and Temp Meas to be indicated, then a timer of fixed length
 *simulates sending several NTF before and indication of a "stable" value. This
 *fake behavior should be replaced once a real driver exists. If Intermediary
 *Temp cannot be notified, just send the indication, if neither can be sent (the
 *configuration connection should never leave this like this) then disconnect.
 *
 *
 * @{
 ****************************************************************************************
 ...
 * The Measurement Interval characteristic has two optional descriptors. In the
 *database, the Client Characteristic Configuration descriptor will always be
 *placed just after the characteristic value. Thus, this function checks if the
 *CCC descriptor has been added.
 * @return     0 if Measurement Interval Char. is not writable (no Valid Range
 *descriptor) 1 if Measurement Interval Char. doesn't support indications (no
 *CCC descriptor) 2 otherwise
 ****************************************************************************************
 */
