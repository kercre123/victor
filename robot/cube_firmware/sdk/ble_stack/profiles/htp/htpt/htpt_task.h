 ...
 *
 * The HTPTTASK is responsible for handling the messages coming in and out of
 *the
...
enum {
  ...
  HTPT_TEMP_SEND_REQ,
  /// Send temperature value confirm to APP so stable values can be erased if
  /// correctly sent.
  HTPT_TEMP_SEND_CFM,
...
enum { HTPT_THERM_TEMP_SEND, HTPT_CENTRAL_IND_CFM };
...
struct htpt_cfg_indntf_ind {
  ...
  uint16_t cfg_val;
  /// Own code for differentiating between Temperature Measurement, Intermediate
  /// Temperature and Measurement Interval chars
  uint8_t char_code;
};
