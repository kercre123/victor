 ...
 *
 * Blood pressure sensor (BPS) profile provides functionalities to upper layer
 *module application. The device using this profile takes the role of Blood
 *pressure sensor.
 *
 ...
 *
 * Profile didn't manages multiple users configuration and storage of offline
 *measurements. This must be handled by application.
 *
 * Blood Pressure Profile Sensor. (BLPS): A BLPS (e.g. PC, phone, etc)
 * is the term used by this profile to describe a device that can perform blood
 *pressure measurement and notify about on-going measurement and indicate final
 *result to a peer BLE device.
 *
 * @{
 ****************************************************************************************
 */
...
#define BLPS_IS_SUPPORTED(mask) (((blps_env.features & mask) == mask))

#define BLPS_IS_ENABLED(mask) (((blps_env.evt_cfg & mask) == mask))
...
/// Indication/notification configuration (put in feature flag to optimize
/// memory usage)
enum {
