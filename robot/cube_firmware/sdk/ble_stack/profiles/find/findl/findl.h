 ...
 *
 * The Find Me Profile defines the functionality required in a device that
 *allows the user to find a peer device by setting its alarm level.
 *****************************************************************************************
 */
 ...
 *
 * The FINDL is responsible for providing Find Me profile Locator
 *functionalities to upper layer module or application. The device using this
 *profile takes the role of find me locator.
 *
 * Find Me Locator (LOC): A LOC (e.g. PC, phone, etc)
 * is the term used by this profile to describe a device that can set an alarm
 *level value in the peer Find Me target Device (TG), causing the TG to start a
 *sound or flashing light or other type of signal allowing it to be located.
 *
 ...
 *  - Discover Alert Level Characteristic in the IAS range(from APP) and Send
 *Result (to APP)
 *  - Set alert level in Target (from APP)
 *  - Error indications (to APP)
 *
 *  The application should remember the discovered IAS range and Alert Level
 *Char. handles for the next connection to a known peer (that may also advertise
 *it supports Find Me Profile.) This allows the setting of the alert level to be
 *faster.
 *
 ...
 ****************************************************************************************
 */
void findl_enable_cfm_send(struct findl_env_tag *findl_env,
