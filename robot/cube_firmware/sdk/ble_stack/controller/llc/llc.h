...
struct llc_env_tag {
  ...
  /**
   * Channel Assessment - Number of packets received with a RSSI greater than
   * the min RSSI threshold and without found synchronization on each channel
   */
  ...
  /// New channel map - Will be applied at instant when a channel map update is
  /// pending
...
#define LLC_LE_CNTL_PKT_BASE_IDX \
  (BLE_TX_BUFFER_CNT - BLE_TX_BUFFER_CNTL - BLE_TX_BUFFER_ADV)
 ...
 *
 * This function set the state of the task, the initiating link supervision time
 *out, the feature used as well as the environment of the LLC and send the
 *connection completed event
 *
 * @param[in] param         Pointer on the structure which contains all the
 *parameters needed to create and maintain the link.
 ...
 ****************************************************************************************
 */
void llc_start(struct llc_create_con_req_ind const *param,
               struct lld_evt_tag *evt);
 ...
 *
 * This function clears the state of the task, the environment of the LLC and
 *send the connection completed event
 *
 * @param[in] conhdl        Connection handle on which the connection is created
 ****************************************************************************************
 */
 ...
 ****************************************************************************************
 */
void llc_discon_event_complete_send(ke_task_id_t src_id, uint8_t status,
                                    uint8_t linkhdl, uint8_t reason);
 ...
 ****************************************************************************************
 */
void llc_create_con_cmd_complete_send(
    uint8_t status, uint16_t conhdl,
 ...
 *
 * This function notify the host that the update of the connection's parameters
 *happened.
 *
 * @param[in] status        Status on the completion of the update connection.
 * @param[in] conhdl        Connection handle on which the update connection
 *happened.
 * @param[in] evt           Pointer to the event structure linked to the
 *connection.
 ****************************************************************************************
 */
 ...
 * @param[in] conhdl        Connection handle on which the command has been
 *processed.
 ****************************************************************************************
 */
 ...
 * @param[in] conhdl        Connection handle on which the command will be
 *processed.
 ****************************************************************************************
 */
 ...
 *
 * @param[in] conhdl        Connection handle on which the packet has been
 *acknowledged
 ...
 * @param[in] conhdl        Connection handle on which the remote features have
 *been read
 * @param[in] feats         Read remote features
 *
 ****************************************************************************************
 */
void llc_feats_rd_event_send(uint8_t status, uint16_t conhdl,
 ...
 *
 * @param[in] status        Status of the event
 * @param[in] conhdl        Connection handle on which the remote version have
 *been read
 *
 ****************************************************************************************
 */
 ...
 *
 * @param[in] conhdl        Connection handle on which the status of the
 ...
 ****************************************************************************************
 */
void llc_common_enc_change_evt_send(uint16_t conhdl, uint8_t enc_status,
                                    uint8_t status);
...
__INLINE void llc_common_flush_occurred_send(uint16_t conhdl) {
  ...
  struct llc_flush_occurred_evt *event = KE_MSG_ALLOC(
      LLC_FLUSH_OCCUR_EVT, l2cc_taskid, llc_taskid, llc_flush_occurred_evt);
  event->conhdl = conhdl;
  ke_msg_send(event);
}
...
__INLINE void llc_common_flush_comp_evt_send(uint16_t conhdl) {
  ...
  ke_task_id_t llc_taskid = KE_BUILD_ID(TASK_LLC, conhdl);
  // allocates the message to send
  struct llc_flush_cmd_complete *event = KE_MSG_ALLOC(
      LLC_FLUSH_CMP_EVT, l2cc_taskid, llc_taskid, llc_flush_cmd_complete);
  event->conhdl = conhdl;
 ...
 *
 * @param[in] conhdl        Connection handle on which the key has been
 *refreshed.
 ****************************************************************************************
 */
__INLINE void llc_common_enc_key_ref_comp_evt_send(uint16_t conhdl,
                                                   uint8_t status) {
  // allocates the message to send
  struct llc_enc_key_ref_cmp_evt *event =
      KE_MSG_ALLOC(LLC_ENC_KEY_REFRESH_CMP_EVT, HOST_SMPC_TASK,
                   KE_BUILD_ID(TASK_LLC, conhdl), llc_enc_key_ref_cmp_evt);
  event->conhdl = conhdl;
  event->status = status;
  ke_msg_send(event);
}
 ...
 * @param[in] evt_new       Pointer to the new LLD event that is used for this
 *connection.
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief Compute the instant when the LL_CHANNEL_MAP_REQ PDU will be sent and
 *program it
 *
...
/**
 ****************************************************************************************
 * @brief Randomly add a channel that had been previously removed (e.g.: by the
 *channel assessment mechanism) based on the channel classification set by the
 *host.
 *
 * @param[in] conhdl        Connection handle
