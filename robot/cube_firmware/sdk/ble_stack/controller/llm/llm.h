 ...
 *
 * The link layer manager contains the modules allowing the physical link
 *establishment, and all the non-connected states.
 * @{
 ****************************************************************************************
 */
...
#define BLE_STATES_BYTE0                                               \
  (BLE_NON_CON_ADV_STATE | BLE_DISC_ADV_STATE | BLE_CON_ADV_STATE |    \
   BLE_DIRECT_ADV_STATE | BLE_PASS_SCAN_STATE | BLE_ACTIV_SCAN_STATE | \
...
#define BLE_STATES_BYTE3                                      \
  (BLE_PASS_SCAN_MASTER_STATE | BLE_ACTIV_SCAN_MASTER_STATE | \
...
// byte14
#define BLE_CMDS_BYTE14 \
  (BLE_RD_LOC_VERS_CMD | BLE_RD_LOC_SUP_FEAT_CMD | BLE_RD_BUF_SIZE_CMD)
// byte15
#define BLE_CMDS_BYTE15 (BLE_RD_BD_ADDR_CMD | BLE_RD_RSSI_CMD)
// byte25
#define BLE_CMDS_BYTE25                                    \
  (BLE_LE_SET_EVT_MSK_CMD | BLE_LE_RD_BUF_SIZE_CMD |       \
...
#define BLE_CMDS_BYTE26                                                    \
  (BLE_LE_SET_SC_RSP_DATA_CMD | BLE_LE_SET_ADV_EN_CMD |                    \
   BLE_LE_SET_SC_PARAM_CMD | BLE_LE_SET_SC_EN_CMD | BLE_LE_CREAT_CNX_CMD | \
   BLE_LE_CREAT_CNX_CNL_CMD | BLE_LE_RD_WL_SIZE_CMD | BLE_LE_CLEAR_WL_CMD)
// byte27
#define BLE_CMDS_BYTE27                                                    \
  (BLE_LE_ADD_DEV_WL_CMD | BLE_LE_REM_DEV_WL_CMD | BLE_LE_CNX_UPDATE_CMD | \
   BLE_LE_SET_HL_CH_CLASS_CMD | BLE_LE_RD_CH_MAP_CMD |                     \
   BLE_LE_RD_REM_USED_FEAT_CMD | BLE_LE_ENCRYPT_CMD | BLE_LE_RAND_CMD)
// byte28
#define BLE_CMDS_BYTE28                                      \
  (BLE_LE_START_ENC_CMD | BLE_LE_LTK_REQ_RPLY_CMD |          \
...
enum { INIT_DIS = 0x00, INIT_EN, INIT_EN_END };
/// Le Direct test types
enum { TEST_TX = 0x00, TEST_RX, TEST_RXTX, TEST_END };
 ...
 *
 * This function initializes the the LLC task, as well as the environment of the
 *LLM
 *
 ****************************************************************************************
 */
 ...
 *
 * This function sends the No Operation command completed event to the host when
 ...
 *
 * This function checks the parameters , fulfill the advertising packet with the
 *useful parameters
 *
 * @param[in] param        Pointer on the structure which contains all the
 *parameters needed to setup the advertising mode.
 *
 ...
 *
 * @param[in] param        Pointer on the structure which contains all the
 *parameters needed to setup the advertising data.
 *
 * @return The status of the parameters validity
 *
 ****************************************************************************************
 ...
 *
 * This function checks the parameters , starts or stops the requested
 *advertising mode.
 *
 * @param[in] param        Pointer on the structure which contains all the
 *parameters needed to start or stop the advertising mode.
 *
 * @return The status of the parameters validity
 *
 ****************************************************************************************
 ...
 *
 * This function checks the parameters , fulfill the scanning packet with the
 *useful parameters
 *
 * @param[in] param        Pointer on the structure which contains all the
 *parameters needed to setup the scanning mode.
 *
 * @return The status of the parameters validity
 *
 ****************************************************************************************
 ...
 *
 * This function checks the parameters , starts or stops the requested scanning
 *mode.
 *
 * @param[in] param        Pointer on the structure which contains all the
 *parameters needed to start or stop the scanning mode.
 *
 * @return The status of the parameters validity
 *
 ****************************************************************************************
 ...
 *
 * @param[in] param        Pointer on the structure which contains all the
 *parameters needed to setup the scanning response data.
 *
 * @return The status of the parameters validity
 *
 ****************************************************************************************
 ...
 *
 * This function checks the parameters , set the transmit mode parameters, turn
 *on the mode and set the LLM state.
 *
 * @param[in] param        Pointer on the structure which contains all the
 *parameters needed to setup the transmit test mode.
 *
 * @return The status of the parameters validity
 *
 ****************************************************************************************
 ...
 *
 * This function checks the parameters , set the receive mode parameters, turn
 *on the mode and set the LLM state.
 *
 * @param[in] param        Pointer on the structure which contains all the
 *parameters needed to setup the receive test mode.
 *
 * @return The status of the parameters validity
 *
 ****************************************************************************************
 ...
 *
 * This function adds the device in the white list, according to the type
 *(public or private)
 *
 * @param[in] param        Pointer on the structure which contains the type and
 ...
 *
 * This function removes the device in the white list, according to the type
 *(public or private)
 *
 * @param[in] param        Pointer on the structure which contains the type and
 ...
 *
 * This function checks the parameters, fulfill the connect_req pdu with the
 *useful parameters, changes the state of the LLM task and requests the LLD to
 *schedule the connection initiation.
 *
 * @param[in] param        Pointer on the structure which contains all the
 *parameters needed to create the link.
 *
 * @return The status of the parameters validity
 *
 ****************************************************************************************
 ...
 *
 * This function sets the key and the data to be encrypted in the encryption
 *engine and start it.
 *
 * @param[in] param        Pointer on the structure which contains all the
 *parameters needed to star tthe encryption engine.
 *
 ****************************************************************************************
 */
 ...
 *
 * This function provide to the host or the LLC the data encrypted by the
 *engine.
 *
 ****************************************************************************************
 */
 ...
 ****************************************************************************************
 */
void llm_common_cmd_complete_send(ke_msg_id_t msg_id, ke_task_id_t dest_id,
 ...
 ****************************************************************************************
 */
void llm_common_cmd_status_send(ke_msg_id_t msg_id, ke_task_id_t dest_id,
                                ke_task_id_t src_id, uint8_t status);
 ...
 *
 * This function gets the information from the received advertising packet and
 *sets the values in the advertising report event.
 *
 ...
 ****************************************************************************************
 */
void llm_adv_report_set(struct llm_le_adv_report_evt *event,
                        struct co_buf_rx_desc *desc);
 ...
 * associated actions (new pdu generation, new state , parameters update,
 *etc...).
 *
 ...
 *
 * This function extracts the parameters from the advertising packet received
 *and generates the event if needed
 *
 ...
 *
 * This function sends the connection complete event to the host and start the
 *LLC task for the dedicated conhdl.
 *
 * @param[in] rxdesc         Pointer on the received advertising packet who has
 *generated the sending of the connect_req pdu.
 *
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief This function returns the number of packets to receive on a channel
 *before being able to disable a channel.
 *
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief This function returns the number of bad packets to receive on a
 *channel before being able to disable a channel.
 *
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief This function returns the minimal RSSI value used in the Channel
