...
/// Constant nibble to use in AA to get more than 1 bit different from
/// Advertising AA
 ...
 *
 * This function checks if advertising or data channel map contains at least 1
 *channel.
 *
 ...
 *
 * This function checks if the bd address if present in the WL and return the
 *status and position.
 *
 ...
 ****************************************************************************************
 */
bool llm_util_bd_addr_in_wl(struct bd_addr const *bd_address,
                            uint8_t bd_addr_type, uint16_t *position);

/**
 ****************************************************************************************
 * @brief Checks the validity of the device address
 *
 * This function checks if the address type and value are available, no NULL,
 *available in the WL, not the local one.
 *
 * @param[in] bd_address           Pointer on the device address to be checked.
 ...
 ****************************************************************************************
 */
uint8_t llm_util_check_address_validity(struct bd_addr *bd_address,
                                        uint8_t addr_type);
 ...
 *
 * This function applies the BD address corresponding to the current address
 *type on the device.
 *
 ...
 *
 * This function checks in case where the filtering is enabled if the address is
 *already listed.
 *
 * @param[in] adv_bd_addr         Pointer on the device address to be checked.
 * @param[in] adv_type             Type of the advertising report (ADV_IND,
 *ADV., SCANRSP)
 *
 * @return If the device address has been found or not.
 *
 ****************************************************************************************
 */
__INLINE bool llm_util_check_adv_report_list(struct bd_addr *adv_bd_addr,
                                             uint8_t adv_type) {
  ...
  for (uint8_t idx = 0; idx < BLE_DUPLICATE_FILTER_MAX; idx++) {
    ...
    if (llm_le_env_lpus.adv_list[idx].adv_type == 0x00) {
      ...
      memcpy(&llm_le_env_lpus.adv_list[idx].adv_addr.addr[0],
             &adv_bd_addr->addr[0], BD_ADDR_LEN);
      // save the type of the device address
      llm_le_env_lpus.adv_list[idx].adv_type |= (1 << adv_type);
      return true;
    }
    if (co_bdaddr_compare(adv_bd_addr,
                          &llm_le_env_lpus.adv_list[idx].adv_addr)) {
...
* |Byte 3           | Byte 2           |  Byte 1            | Byte 0 | |CT1 |
*PRAND_4MSB | CT2 | PRAND_4LSB | CT2 | INCRAND_4MSB | CT2 | INCRAND_4LSB|
*
...
*
* Considering the formats of CT1 and CT2, the rules for AA generation are
*respected:
...
*
*  - CT2 and CT1 ensure that the newly generated address is different and at
*least 1 bit different from the advertising access address
*
*  - since CT1 != CT2 , the four bytes of the AA can never be equal
*
*  - Since in a 32 bit number there are 31 transitions, and the neighboring
*equal bits in CT1 and CT2 represent 7 transitions, there can never be more than
*31-7=24 transitions in the AA
*
*  - The format of CT1 ensures at least two transitions in the most significant
*6 bits.
*
...
__INLINE void llm_util_aa_gen(uint8_t *acc_addr) {
  ...
  acc_addr[0] =
      (LLM_AA_CT2[llm_le_env.aa.ct2_idx] << 4) | (llm_le_env.aa.intrand & 0x0F);
  acc_addr[1] =
      (LLM_AA_CT2[llm_le_env.aa.ct2_idx] << 4) | (llm_le_env.aa.intrand >> 4);
...
__INLINE void llm_util_set_param_adv_dft(void) {
  ...
  txdesc->txheader =
      (uint16_t)(((llm_le_env.advertising_params.type << BLE_TXTYPE_LSB) &
                  BLE_TXTYPE_MASK) |
                 ((0x6 << BLE_TXADVLEN_LSB) & BLE_TXADVLEN_MASK));
  // sets the scan response
  txdesc = co_buf_tx_desc_get(LLM_LE_SCAN_RSP_IDX);
  txdesc->txheader =
      (uint16_t)(((LLM_SCAN_RSP << BLE_TXTYPE_LSB) & BLE_TXTYPE_MASK) |
 ...
 *
 * This function sends an event to the host when the TX or RX test mode is
 *finished
 *
 ****************************************************************************************
 */
__INLINE void llm_util_chk_tst_mode(void) {
  ...
  if ((llm_le_env.test_mode.directtesttype != TEST_END) &&
    ...
    event = KE_MSG_ALLOC(LLM_LE_TEST_END_CMP_EVT, HOST_TESTER_TASK, TASK_LLM,
                         llm_test_end_cmd_complete);
