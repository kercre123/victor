...
#define BLE_TX_BUFFER_CNT_USER \
  (BLE_TX_BUFFER_DATA_USER + BLE_TX_BUFFER_CNTL_USER + BLE_TX_BUFFER_ADV_USER)

/// Number of receive buffers in the RX ring. This number defines the interrupt
/// rate during a connection event. An interrupt is asserted every
/// BLE_RX_BUFFER_CNT/2 reception. This number has an impact on the size of the
/// exchange memory. This number may have to be increased when CPU is very slow
/// to free the received data, in order not to overflow the RX ring of buffers.
...
#define RWIP_HEAP_ENV_SIZE_USER \
  ((BLE_HEAP_ENV_SIZE + BLEHL_HEAP_ENV_SIZE) * BLE_CONNECTION_MAX_USER)

#define BLE_HEAP_MSG_SIZE_USER \
  (256 * (BLE_CONNECTION_MAX_USER + 1) + 96 * (2 * BLE_CONNECTION_MAX_USER + 1))
#define BLEHL_HEAP_MSG_SIZE_USER (256 + 256 * BLE_CONNECTION_MAX_USER)
#define RWIP_HEAP_MSG_SIZE_USER \
  (BLE_HEAP_MSG_SIZE_USER + BLEHL_HEAP_MSG_SIZE_USER)
