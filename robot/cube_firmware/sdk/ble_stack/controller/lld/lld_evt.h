 ...
 ****************************************************************************************
 */
/// Size of the LLD event table. It shall be equal to the max number of
/// supported connections * 2 + 1 for the scanning/advertising
...
#define MAX_INTERVAL_TIME \
  3193600  // Max number of 625us periods for a connection interval
...
/// Instant actions
enum { LLD_NO_ACTION, LLD_PARAM_UPDATE, LLD_CHMAP_UPDATE };
...
struct lld_evt_tag {
  ...
  /// List of TX descriptors programmed for transmission (i.e. chained with the
  /// CS)
  struct co_list tx_prog;
  /// List of TX descriptors ready for transmission (i.e. not yet chained with
  /// the CS)
  ...
  /// Restart policy
  uint8_t restart_pol;
  /// Flag indicating if the event is currently programmed in the exchange table
  /// or not
  bool prog;
...
struct lld_evt_env_tag {
  ...
  /// Pointer to the scanning event. If scan is not active, this pointer is
  /// NULL.
 ...
 *
 * The absolute time difference between time1 and time2 is supposed to be less
 *than the maximum interval time
 *
 ...
 ****************************************************************************************
 */
__INLINE bool lld_evt_time_cmp(uint32_t time1, uint32_t time2) {
...
/**
 ****************************************************************************************
 * @brief Computes the compensation that has to be applied to the supervision
 *timeout, according to the drift between master and slave
 *
...
/**
 ****************************************************************************************
 * @brief Update the TX power field of the event passed as parameter with the
 *requested value of TX power
 *
 ...
 *
 * @param[in] conhdl      Connection handle for which the corresponding event is
 *searched
 *
 * @return The pointer to the found event (NULL if no event is attached to this
 *handle)
 *
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief Create a connection or scanning event and chain it in the scheduling
 *and interval lists
 *
 * @param[in] handle      Connection handle for which the event is created
 *(LLD_ADV_HDL if non-connected event)
 * @param[in] duration    Expected duration of the event, in units of 625us.
 *This duration can be reduced if scheduling of other events is requiring that.
 ...
 ****************************************************************************************
 */
struct lld_evt_tag *lld_evt_scan_create(uint16_t handle, uint16_t duration,
 ...
 * @param[in] restart_pol Requested restart policy, i.e. LLD_ADV_RESTART or
 *LLD_NO_RESTART
 *
 * @return The pointer to the event just created
 *
 ****************************************************************************************
 */
struct lld_evt_tag *lld_evt_adv_create(uint16_t handle, uint16_t mininterval,
 ...
 * @param[out] upd_par     Computed updated parameters, to be put in the LLCP
 *frame
 *
 ...
 * @param[in]  evt_adv     Pointer to the advertising event that triggered the
 *connection
 ...
 ****************************************************************************************
 */
struct lld_evt_tag *lld_evt_move_to_slave(
    struct llc_create_con_req_ind const *con_par,
    struct llm_pdu_con_req_rx *con_req_pdu, struct lld_evt_tag *evt_adv,
    uint16_t conhdl);
...
/**
 ****************************************************************************************
 * @brief Indicates to the LLD to move from initiating to master connected
 *state.
 *
 ...
 * In case the slave event passed as parameter is far enough in the future (e.g.
 *due to slave latency), the event is canceled and replaced with the earliest
 *possible one. This function is called when a data has been pushed for
 *transmission in order to send it as soon as possible even if slave latency is
 *used.
 *
 * @param[in] evt        Event to be deleted
 ****************************************************************************************
 ...
 *
 * This function is a deferred action of the BLE wake up interrupt. It programs
 *the data Tx/Rx exchange in the control structure corresponding to the event,
 *and programs the target time of the next event to be scheduled.
 *
 ****************************************************************************************
 */
 ...
 *
 * This function is a deferred action of the BLE end of event interrupt. It
 *flushes the data Tx/Rx exchange corresponding to the event, and programs the
 ...
 *
 * This function is called under interrupt when an end of sleep interrupt is
 *generated by the BLE core. It sets the associated kernel event in order to
 *perform the handling as a deferred action in background context.
 *
 ****************************************************************************************
 */
 ...
 *
 * This function is called under interrupt when an end of event interrupt is
 *generated by the BLE core. It sets the associated kernel event in order to
 *perform the handling as a deferred action in background context.
 *
 ****************************************************************************************
 ...
 * the BLE core. It sets the associated kernel event in order to perform the
 ...
 *
 * This function is called under interrupt when a general purpose timer
 *interrupt is generated by the BLE core. It sets the associated kernel event in
 *order to perform the handling as a deferred action in background context.
 *
 ****************************************************************************************
 */
 ...
 *
 * @param[in] evt  Pointer to the event for which the acknowledgment was
 *received
 *
 ****************************************************************************************
 */
 ...
 *
 * @param[in] evt     Pointer to the event for which the update has to be
 *programmed
 *
