 ...
 *
 * The LLD is responsible for programming the HW for event scheduling. It
 provides an API
 * to the @ref LLD and the @ref LLC for event programming (@ref LLDEVT)and data
 * transmission/reception (@ref LLDDATA). This API takes two forms depending on
 the
 * direction:
...
#define LLD_RX_IRQ_THRES \
  jump_table_struct[lld_rx_irq_thres]  //(BLE_RX_BUFFER_CNT/2)
 ...
 *
 * This function initializes the BLE HW core, as well as the environment of the
 *LLD
 *
 ...
 *
 * This function stops any pending activity of the BLE core, resets it and
 *disables it. It is called upon reception of a HCI reset command.
 *
 ****************************************************************************************
 */
void lld_reset(void);
 ...
 *
 * This function creates an event for advertising, set the control structure
 *according to the parameters, and program the required data for transmission.
 *
 * @param[in] adv_par        Pointer to the structure containing the advertising
 *                           parameters
 * @param[in] adv_pdu        Pointer to the buffer containing the advertising
 *data
 * @param[in] scan_rsp_pdu   Pointer to the buffer containing the scan response
 *data (should be set to NULL is no scan response has to be transmitted)
 ...
 *
 * This function creates an event for scanning, set the control structure
 *according to the parameters, and program the required data for transmission.
 *
 * @param[in] param          Pointer to the structure containing the scanning
 *parameters
 * @param[in] scan_req_pdu   Pointer to the buffer containing the scan request
 *data (should be set to NULL if passive scanning is required)
 *
 ...
 * @param[in] con_req_pdu  Pointer to the data buffer containing the connection
 *request
...
/**
 ****************************************************************************************
 * @brief Indicates to LLD that it can program the first connection event for
 *this handle
 *
 * This function is called by the @ref LLM once it receives the confirmation
 *that the connection request has been transmitted to the future slave. The LLD
 *then programs the first connection event for this slave.
 * @note The @ref lld_con_start function has to be called previously to start
 *the initiating process, otherwise lld_move_to_master has unpredictable
 *behavior.
 *
 * @param[in] evt     The pointer to the event on which the connection will
 *occur
 * @param[in] conhdl  Connection handle on which connection is initiated
 *
 ****************************************************************************************
 ...
 *
 * This function is called by the @ref LLM once it receives a CONNECT_REQ. The
 *LLD then programs the first connection event according to the parameters
 *received in the CONNECT_REQ frame.
 *
 ...
 ****************************************************************************************
 */
struct lld_evt_tag *lld_move_to_slave(
 ...
 *
 * This function requests to the LLD to program a channel map update with a
 *specific slave (designated by the event pointer passed as parameter). The LLD
 *then computes the instant time for the update and returns it to the caller.
 *
 * @param[in] evt   The pointer to the event on which the channel map has to be
 *changed.
 *
 * @return The instant as computed by the LLD at which the channel map update
 *will occur.
 *
 ****************************************************************************************
 */
 ...
 *
 * This function indicates to the slave that to channel map update request has
 *been received from the master, and at which time it will occur.
 *
 * @param[in] evt     The pointer to the event on which the channel map has to
 *be changed
 * @param[in] instant The connection event count on which the update has to be
 *applied
 *
 ****************************************************************************************
 */
 ...
 *                       interval, window offset and size, timeout and the
 *instant
 * @param[out] param_pdu Pointer on the structure which contains the latency,
 *                       interval, window offset and size, timeout and the
 *instant dedicated to create the pdu
 *
 ****************************************************************************************
 */
 ...
 *                       interval, window offset and size, timeout and the
 *instant received from the connection update pdu
 ****************************************************************************************
 */
 ...
 *
 * This function request to the HW to start the transmit test mode, by setting
 *the format synchronization word (0x94826E8E), CRC(0x555555) and the transmit
 *frequency in the CS and the address of the tx descriptor.
 *
 * @param[in] txdesc        Pointer on the TX descriptor containing the length,
 *type and payload.
 ...
 ****************************************************************************************
 */
struct lld_evt_tag *lld_test_mode_tx(struct co_buf_tx_node *txdesc,
                                     uint8_t tx_freq);
 ...
 *
 * This function request to the HW to start the transmit test mode, by setting
 *the format and the receive frequency in the CS.
 *
