...
struct app_callbacks {
  void (*app_on_connection)(const uint8_t,
                            struct gapc_connection_req_ind const *);
...
#if (BLE_APP_SEC)
  void (*app_on_pairing_request)(const uint8_t,
                                 struct gapc_bond_req_ind const *);
  void (*app_on_tk_exch_nomitm)(const uint8_t,
                                struct gapc_bond_req_ind const *);
  ...
  void (*app_on_encrypt_req_ind)(const uint8_t,
                                 struct gapc_encrypt_req_ind const *);
...
struct profile_callbacks {
  ...
  void (*on_batt_level_ntf_cfg_ind)(uint8_t connection_idx, uint16_t ntf_cfg,
                                    uint8_t bas_instance);
  void (*on_findt_alert_ind)(uint8_t connection_idx, uint8_t alert_lvl);
  void (*on_proxr_level_upd_ind)(uint8_t connection_idx, uint8_t alert_lvl,
                                 uint8_t char_code);
...
#define EXECUTE_PROFILE_CALLBACK_VOID(func)                                 \
  {                                                                         \
    if (user_profile_callbacks.func != NULL) user_profile_callbacks.func(); \
  }

#define EXECUTE_PROFILE_CALLBACK_PARAM(func, param) \
  {                                                 \
    if (user_profile_callbacks.func != NULL)        \
      user_profile_callbacks.func(param);           \
  }
#define EXECUTE_PROFILE_CALLBACK_PARAM1_PARAM2(func, param1, param2) \
  {                                                                  \
    if (user_profile_callbacks.func != NULL)                         \
      user_profile_callbacks.func(param1, param2);                   \
  }

#define EXECUTE_PROFILE_CALLBACK_PARAM1_PARAM2_PARAM3(func, param1, param2, \
                                                      param3)               \
  {                                                                         \
    if (user_profile_callbacks.func != NULL)                                \
...
#define EXECUTE_CALLBACK_VOID(fnc)                                \
  {                                                               \
    if (user_app_callbacks.fnc != NULL) user_app_callbacks.fnc(); \
  }

#define EXECUTE_CALLBACK_PARAM(func, param)                              \
  {                                                                      \
    if (user_app_callbacks.func != NULL) user_app_callbacks.func(param); \
  }

#define EXECUTE_CALLBACK_PARAM1_PARAM2(func, param1, param2) \
  {                                                          \
    if (user_app_callbacks.func != NULL)                     \
