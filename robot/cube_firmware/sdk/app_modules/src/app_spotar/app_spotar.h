...
#if defined(CFG_SPOTAR_I2C_DISABLE)
#define SPOTAR_I2C_DISABLE 1
#else
#define SPOTAR_I2C_DISABLE 0
#endif
...
#if defined(CFG_SPOTAR_PATCH_DISABLE)
#define SPOTAR_PATCH_DISABLE 1
#else
#define SPOTAR_PATCH_DISABLE 0
#endif

#if (SPOTAR_PATCH_DISABLE && SPOTAR_UPDATE_DISABLE)
#error "SPOTAR application is enabled but both SPOTA and SUOTA are disabled"
#endif
#if (SPOTAR_SPI_DISABLE && SPOTAR_I2C_DISABLE && !SPOTAR_UPDATE_DISABLE)
#error "SUOTA application is enabled but both I2C and SPI are disabled"
#endif
...
typedef struct {
  uint8_t signature[2];
  uint8_t
      validflag;    // Set to STATUS_VALID_IMAGE at the end of the image update
 ...
 ****************************************************************************************
 */
void app_spotar_enable(uint16_t conhdl);
...
/**
 ****************************************************************************************
 * @brief Handles patch execution. Should be called at system start up and after
 *deep sleep.
 *
 * @param[in]   void
 *
...
#if (!SPOTAR_PATCH_DISABLE)
/**
 ****************************************************************************************
 * @brief SPOTA data handler. Validates patch and stores patch data to memory
 *device.
 *
 * @param[in]   void
 *
 ...
 * @param[in]   Points to the intermediate buffer address which will be used to
 *temporary store the patch payload fetched from external Non Volatile memories.
 *Only valid if I2C or SPI
 * @param[in]   patch_length: defines the size of the patch to be applied (in
 *32bit words)
 *
 * @return      void
 *
 ****************************************************************************************
 */
void exec_patching_spota(WORD mem_dev, WORD gpio_map, WORD* ptr,
                         WORD patch_length);
