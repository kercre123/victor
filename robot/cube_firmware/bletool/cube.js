	...
	upload(data) {
		...
		let next = (err) => {
			...
			if (offset < data.length) {
				let target = new Buffer([offset & 0xFF, offset >> 8]);
				let chunk = data.slice(offset, offset + MAX_BYTES_PER_PACKET);
				let packet = Buffer.concat([target, chunk]);

				this._ota_target.write(packet, false, next);
				console.log(packet);
				offset += chunk.length;
			} else {
				let packet = new Buffer([0xFF, 0xFF, 0, 0, 0, 0, 0, 0]);
				packet.writeUInt16LE(data.length, 2);
				packet.writeUInt32LE(checksum(data), 4);

				this._ota_target.write(packet);
