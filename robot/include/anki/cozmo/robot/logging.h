 ...
 * non-disclosure agreement with Anki, inc. File created Dec 2015
 *
 * API is intended to be similar to previous errorHandling and DAS libraries but
 * with low overhead appropriate to Cozmo's real time and network processors.
 *
 * Implementations of SendLog will vary for the different processors:
 * On the WiFi processor, SendLog will accept a level and a format string and
 * variable arguments for payload On the Real time processor, SendLog will
 * accept a level and a RtipTrace enum from robotLogging.clad along with the
...
#define console_printf(fmt, ...) \
  __android_log_print(ANDROID_LOG_INFO, "vic-robot", fmt, ##__VA_ARGS__)
#define log_assert(line, file, expr, fmt, ...)                        \
  __android_log_assert(expr, "vic-robot",                             \
                       fmt ": failed at line " line " in file " file, \
                       ##__VA_ARGS__)
#define log_error(name, fmt, ...)                                    \
  __android_log_print(ANDROID_LOG_ERROR, "vic-robot", name ": " fmt, \
                      ##__VA_ARGS__)
#define log_warn(name, fmt, ...)                                    \
  __android_log_print(ANDROID_LOG_WARN, "vic-robot", name ": " fmt, \
                      ##__VA_ARGS__)
#define log_info(name, fmt, ...)                                    \
  __android_log_print(ANDROID_LOG_INFO, "vic-robot", name ": " fmt, \
                      ##__VA_ARGS__)
#define log_debug(name, fmt, ...)                                    \
  __android_log_print(ANDROID_LOG_DEBUG, "vic-robot", name ": " fmt, \
                      ##__VA_ARGS__)
...
#define log_assert(line, file, expr, fmt, ...)                         \
  console_printf("[Assert] " fmt ": \"" expr "\" failed at line " line \
                 " in file " file "\r\n",                              \
                 ##__VA_ARGS__)
#define log_error(name, fmt, ...) \
  console_printf("[Error] " name ": " fmt "\r\n", ##__VA_ARGS__)
#define log_warn(name, fmt, ...) \
  console_printf("[Warn] " name ": " fmt "\r\n", ##__VA_ARGS__)
#define log_info(name, fmt, ...) \
  console_printf("[Info] " name ": " fmt "\r\n", ##__VA_ARGS__)
#define log_debug(name, fmt, ...) \
  console_printf("[Debug] " name ": " fmt "\r\n", ##__VA_ARGS__)

#endif

// Keil doesn't seem to reliably error on these not being defined below so
// trigger explicitly.
...
#define AnkiConditionalErrorAndReturnValue(expression, returnValue,    \
                                           nameString, fmtString, ...) \
  if (!(expression)) {                                                 \
    log_error(nameString, fmtString, ##__VA_ARGS__);                   \
    return returnValue;                                                \
  }
...
#define AnkiConditionalWarnAndReturnValue(expression, returnValue, nameString, \
                                          fmtString, ...)                      \
  if (!(expression)) {                                                         \
    log_warn(nameString, fmtString, ##__VA_ARGS__);                            \
    return returnValue;                                                        \
...
#define AnkiConditionalWarnAndReturn(expression, ...) \
  if (!(expression)) return
#define AnkiConditionalWarnAndReturnValue(expression, returnValue, ...) \
  if (!(expression)) return returnValue
#endif
...
#define AnkiAssert(expression, fmtString, ...)                        \
  if (!(expression)) {                                                \
    log_assert(STRINGIZE(__LINE__), __FILE__, #expression, fmtString, \
               ##__VA_ARGS__);                                        \
    exit(-1);                                                         \
  }
#else
