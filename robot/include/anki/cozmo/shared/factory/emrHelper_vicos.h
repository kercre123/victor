...
static Factory::EMR* OpenEMR(int openFlags, int protFlags) {
  ...
  if (fd == -1) {
    LOG_ERROR("Factory.CheckEMRForPackout.OpenFailed", "%d %s", errno,
              kEMRFile);
    return nullptr;  // exit instead? will probably end up crashing in WriteEMR
                     // or GetEMR will return null
  }
...
static void ReadEMR() {
  ...
  if (_emr == nullptr) {
    ...
    // If this is the factory test and the emr is null (doesn't exist) or we
    // have not been packed out then /factory should be writable so close the
    // emr and reopen as writable
...
static void WriteEMR(size_t offset, void* data, size_t len) {
  ...
  if (FACTORY_TEST) {
    ...
    LOG_WARNING("Factory.WriteEMR.FailedWrite",
                "Failed to write to EMR not factory test");
  }
}
