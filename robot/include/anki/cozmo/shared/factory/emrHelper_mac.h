 ...
 * Date:   1/30/2018
 *
 * Description: Electronic medical(manufacturing) record written to the robot at
 *the factory DO NOT MODIFY Do not include this file directly, instead include
 *emrHelper.h
 *
...
namespace {
// The simulated EMR is created at runtime by webotsCtrlRobot2.cpp which allows
// us to determine if the simulated robot is Whiskey or Vector. As such it needs
// to be created in a shared location so all webots controllers that need access
// to it can easily access it. We already create and write files to the
// playbackLogs dir so it felt like the best place to stick the EMR
...
static Factory::EMR* OpenEMR(int openFlags, int protFlags) {
  ...
  if (fd == -1) {
    PRINT_NAMED_INFO("Factory.CheckEMRForPackout.OpenFailed", "%d %s", errno,
                     kEMRFile);
    return &_fakeEMR;
  }
  ...
  // Memory map the file as shared since multiple processes have access to these
  // functions
  Factory::EMR* emr =
      (Factory::EMR*)(mmap(nullptr, sizeof(EMR), protFlags, MAP_SHARED, fd, 0));
  ...
  // Close our fd, mmap will add a reference to it so the mapping will still be
  // valid
  close(fd);
...
// Ex: WriteEMR(offsetof(Factory::EMR, playpen)/sizeof(uint32_t), buf,
// sizeof(buf));
...
static void SetWhiskey(bool whiskey) {
  WriteEMR(offsetof(Factory::EMR::Fields, HW_VER) / sizeof(uint32_t),
           (whiskey ? 7 : 6));
}
