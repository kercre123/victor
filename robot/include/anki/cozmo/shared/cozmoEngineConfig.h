...
const f32 MAX_MARKER_NORMAL_ANGLE_FOR_SHOULD_BE_VISIBLE_CHECK_RAD =
    DEG_TO_RAD(45.f);
...
// Amount to recede the front boundary of the robot when we don't want to
// include the lift. TEMP: Applied by default to robot bounding box params below
// mostly for demo purposes
//       so we don't prematurely delete blocks, but we may eventually want to
//       apply it conditionally (e.g. when carrying a block)
const f32 ROBOT_BOUNDING_X_LIFT = 19.7f;

const f32 ROBOT_BOUNDING_X =
    100.7f - ROBOT_BOUNDING_X_LIFT;  // including gripper fingers
const f32 ROBOT_BOUNDING_Y = 60.0f;
const f32 ROBOT_BOUNDING_X_FRONT =
    34.2f - ROBOT_BOUNDING_X_LIFT;   // distance from robot origin to front of
                                     // bounding box
const f32 ROBOT_BOUNDING_Z = 69.3f;  // from ground to top of head

// Apply a conservative negative padding when checking for collisions with known
// objects to see if their pose should be marked as "dirty". (A negative value
// means it _really_ has to intersect in order for the object to be dirtied.)
...
// Mostly used by TurnInPlaceAction, but also used by planner in some places.
// Maybe planner should keep its own constants?
...
// A different default used for replanning (while we are already following a
// path)
const f32 DEFAULT_MAX_PLANNER_REPLAN_COMPUTATION_TIME_S = 1.f;

// The closest distance to use for docking (in case we are already close to the
// cube)
const f32 DEFAULT_MIN_PREDOCK_POSE_DISTANCE_MM = 65.0f;

// Default length of the preDock pose line (the line on which preDock poses can
// fall)
...
// When getting a preaction pose for an offset dock, this is the amount by which
// the preaction pose is offset relative to the specified docking offset. (0 <
// val < 1)
const f32 PREACTION_POSE_OFFSET_SCALAR = 1.0f;

// Scalar for preAction pose x distance threshold. We really only care if we are
// aligned in y so the x distance is multiplied by this scalar
const f32 PREACTION_POSE_X_THRESHOLD_SCALAR = 2.0f;

// For things like docking, we want to not turn away too much if we can avoid
// it. This is a threshold in radians. If the starting point is close (in
// euclidean distance) and also the robot angle is within this threshold of the
// final goal angle
