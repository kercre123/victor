...
const f32 WHEEL_RAD_TO_MM =
    WHEEL_DIAMETER_MM / 2.f;  // or HALF_WHEEL_CIRCUM / PI;
const f32 WHEEL_DIST_MM =
    46.f;  // approx distance b/w the center of the front treads
const f32 WHEEL_DIST_HALF_MM = WHEEL_DIST_MM / 2.f;
const f32 WHEEL_BASE_MM = 48.f;

// Tolerances on commanded target angles
// i.e. If motor is within this tolerance of the target angle, it is done
// moving.
...
#ifdef SIMULATOR
// In simulation, the drive center pose always seems to be this value, whether
// carrying a block or not
...
// Forward distance sensor measurements (TODO: finalize these dimensions on
// production robot)
const float kProxSensorTiltAngle_rad = DEG_TO_RAD(
    6.5f);  // Angle that the prox sensor is tilted (upward is positive)
const float kProxSensorPosition_mm[3] = {10.f, 0.f,
                                         16.f};  // With respect to robot origin
const float kProxSensorFullFOV_rad =
    DEG_TO_RAD(25.f);  // Full Field of View (FOV) of the sensor cone
...
const f32 LIFT_HEIGHT_LOWDOCK =
    32.f;  // For interfacing with a cube that is on the ground.
const f32 LIFT_HEIGHT_HIGHDOCK = 76.f;  // For interfacing with a cube that is
                                        // stacked on top of another cube.
const f32 LIFT_HEIGHT_CARRY = 92.f;     // Cube carrying height.
const f32 LIFT_HEIGHT_LOW_ROLL =
    68.f;  // For rolling a cube that is on the ground.
const f32 LIFT_HEIGHT_ABOVE_PROX =
    59.f;  // Just above prox sensor. Useful for driving in cluttered spaces.

// Distance between the lift shoulder joint and the lift "wrist" joint where arm
// attaches to fork assembly
...
// The lift height is defined as the height of the upper lift arm's wrist joint
// plus this offset.
...
// Lift xbar height from top (where tool codes are) to bottom (the other side,
// the one at the bottom)
const f32 LIFT_XBAR_HEIGHT =
    8.5f;  // measured manually (not in dimensions sheet)
// Width of cross bar inside the two vertical sides (approximate)
const f32 LIFT_XBAR_WIDTH = 30.f;
// The height of the bottom of the lift crossbar with respect to the wrist
// joint. This is also the lift's overall bottom wrt wrist
const f32 LIFT_XBAR_BOTTOM_WRT_WRIST_JOINT =
    LIFT_XBAR_HEIGHT_WRT_WRIST_JOINT - LIFT_XBAR_HEIGHT;

// The distance along the x axis from the wrist joint to the front of the lifter
// plate
const f32 LIFT_FRONT_WRT_WRIST_JOINT = 4.f;
// The distance along the x axis from the wrist joint to the back of the lifter
// plate
const f32 LIFT_BACK_WRT_WRIST_JOINT = -1.f;  // rsam: approx

// I added this var because I have seen that physical robots have some slack
// that causes their lifts to fall a little with respect to the position they
// should hold. This fact prevented accurate calculation of where the borders of
// the lift would be in the camera. Applying this offset to the bottom of the
// lift, provides a threshold where we expect the lift to be. This variable was
// calculated by hand, so it's up to tweaks if different evidence is found.
...
const f32 LIFT_BASE_POSITION[3] = {
    -41.0f, 0.f, 30.5f + WHEEL_RAD_TO_MM};  // relative to robot origin
...
#define ConvertLiftHeightToLiftAngleRad(height_mm)                  \
  asinf((CLIP(height_mm, LIFT_HEIGHT_LOWDOCK, LIFT_HEIGHT_CARRY) -  \
         LIFT_BASE_POSITION[2] - LIFT_FORK_HEIGHT_REL_TO_ARM_END) / \
        LIFT_ARM_LENGTH)
#define ConvertLiftAngleToLiftHeightMM(angle_rad)                \
  ((sinf(angle_rad) * LIFT_ARM_LENGTH) + LIFT_BASE_POSITION[2] + \
   LIFT_FORK_HEIGHT_REL_TO_ARM_END)
...
// deadband around the min and max lift-angles beyond which we are considered
// not calibrated
...
const f32 MIN_CAMERA_GAIN =
    0.25f;  // Real min should be 0.1, but using 0.25 as a bandaid for VIC-6653
...
// Cliff detection thresholds (these come from testing with DVT1 robots - will
// need to be adjusted for production hardware)
#if FACTORY_TEST
...
// Cliff sensor value must rise this far above the threshold to 'untrigger'
// cliff detection.
const u16 CLIFF_DETECT_HYSTERESIS = 30;

// V2 cliff sensors (assumes 4 cliff sensors are arranged in a rectangle
// symmetric about the robot x axis) NOTE: These values are approximate and
// should be verified for final V2 design.
const f32 kCliffSensorYOffset_mm =
    14.f;  // y (lateral) distance from robot origin to the cliff sensors
const f32 kCliffSensorXOffsetFront_mm =
    2.f;  // x (longitudinal) offset from robot origin to front cliff sensors
const f32 kCliffSensorXOffsetRear_mm =
    -50.f;  // x (longitudinal) offset from robot origin to rear cliff sensors
...
// It can actually spin closer to 360 deg/s, but we use a conservative limit to
// be sure the robot can actually achieve this top speed. If it can't, point
// turns could look jerky because the robot can't keep up with the rotation
// profile. Ideally, speed, in radians, should be (MAX_WHEEL_SPEED_MMPS /
// WHEEL_DIST_HALF_MM), but tread slip makes this not true.
const f32 MAX_BODY_ROTATION_SPEED_DEG_PER_SEC = 300;
const f32 MAX_BODY_ROTATION_SPEED_RAD_PER_SEC =
    DEG_TO_RAD(MAX_BODY_ROTATION_SPEED_DEG_PER_SEC);
const f32 MAX_BODY_ROTATION_ACCEL_RAD_PER_SEC2 =
    MAX_BODY_ROTATION_SPEED_RAD_PER_SEC * MAX_WHEEL_ACCEL_MMPS2 /
    MAX_WHEEL_SPEED_MMPS;
const f32 MAX_BODY_ROTATION_ACCEL_DEG_PER_SEC2 =
    RAD_TO_DEG(MAX_BODY_ROTATION_ACCEL_RAD_PER_SEC2);
...
// how long there is between stopping the motors and issuing a cliff event
// (because we have decided there isn't a pickup event)
...
const s32 ANIM_OVERTIME_WARNING_THRESH_US =
    ANIM_OVERTIME_WARNING_THRESH_MS * 1000;
...
// TODO: Do we need this?  Only used in simulation I think? (Add #ifdef
// SIMULATOR?)
...
// Defines whether animation process should broadcast the final face image
// (after processing/scan line application etc) back to engine so that it can
// maintain an accurate reperesentation of robot state THIS IS CURRENTLY USED
// FOR R&D ONLY - CONSULT KEVIN YOON ABOUT PERFORMANCE BEFORE TURNING THIS ON IN
// MASTER
