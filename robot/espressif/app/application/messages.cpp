    ...
    namespace Messages {
      ...
      static void SendNVOpResult(const NVStorage::NVOpResult* report, const NVStorage::NVReportDest dest)
      {
        switch (dest)
        {
          ...
          default:
          {
            AnkiError( 151, "Messages.SendNVOpResult", 445, "Unhandled report destination %d", 1, dest);
          }
        }
      }
      ...
      static void NVReadDoneCB(NVStorage::NVStorageBlob* entry, const NVStorage::NVResult result)
      {
        ...
        else // Successful read, send data
        {
          switch(nvOpReportTo)
          {
            ...
            default:
            {
              AnkiError( 152, "Messages.NVReadDoneCB", 445, "Unhandled report destination %d", 1, nvOpReportTo);
            }
          }
        }
      }
