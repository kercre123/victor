...
bool SendMessage(RobotInterface::EngineToRobot& msg)
{
  ...
  AnkiConditionalErrorAndReturnValue((msg.Size() + sentThisTick) <= BYTES_PER_TICK,  false, 30, "RTIP", 197, "SendMessage: Not enough buffer on RTIP\r\n", 0);
  AnkiConditionalErrorAndReturnValue(msg.Size() <= DROP_TO_RTIP_MAX_VAR_PAYLOAD,     false, 30, "RTIP", 198, "SendMessage: Message too large for RTIP, %d > %d\r\n", 2, msg.Size(), DROP_TO_RTIP_MAX_VAR_PAYLOAD);
  AnkiConditionalErrorAndReturnValue(i2spiQueueMessage(msg.GetBuffer(), msg.Size()), false, 30, "RTIP", 199, "SendMessage: Couldn't forward message to RTIP\r\n", 0);
  return true;
}
