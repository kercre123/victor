...
namespace Face {
  ...
  namespace Draw {
    ...
    bool Copy(u64* frame, const u32* image, const int imgCols, const int x, const int y)
    {
      if ((imgCols + x) > COLS)
      {
        AnkiWarn( 197, "Face.Draw.Copy.TooWide", 506, "%d + %d > %d", 3, imgCols, x, COLS);
        return false;
      }
    ...
    bool Number(u64* frame, int digits, u32 value, int x, int y)
    {
      ...
      {
        AnkiWarn( 198, "Face.Draw.Number.TooWide", 506, "%d + %d > %d", 3, x, number_width, COLS);
        return false;
      }
    ...
    bool Print(u64* frame, const char* text, const int characters, const int x, const int y)
    {
      ...
      {
        AnkiWarn( 199, "Face.Draw.Print.TooWide", 506, "%d + %d > %d", 3, x, text_width, COLS);
        return false;
      }
