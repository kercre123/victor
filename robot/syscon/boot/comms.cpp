...
static void check_buffer() {
  ...
  uint32_t expected_crc =
      crc(&inbound.raw_payload, inbound.header.bytes_to_follow);
  SpineMessageFooter* message_end =
      (SpineMessageFooter*)(&inbound
                                 .raw_payload[inbound.header.bytes_to_follow]);
  ...
  switch (inbound.header.payload_type) {
    ...
    case PAYLOAD_VALIDATE:
      if (APP->fingerPrint != COZMO_APPLICATION_FINGERPRINT) {
        ...
        Flash::writeFlash(&APP->fingerPrint, &COZMO_APPLICATION_FINGERPRINT,
                          sizeof(APP->fingerPrint));
        g_exitRuntime = true;
      }
      break;
    ...
    case PAYLOAD_DFU_PACKET: {
      const uint32_t word_size =
          inbound.writeDFU.wordCount * sizeof(inbound.writeDFU.data[0]);
      const uint32_t* dst =
          (uint32_t*)&APP->certificate[inbound.writeDFU.address];
