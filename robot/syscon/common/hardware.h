...
// Whiskey's in the field. MSB = REV2, LSB = REV
// Currently Xray is the only robot with REV2 > 0

#define IS_WHISKEY ((*HW_REVISION & 0xFF) >= WHISKEY_MIN_VERSION)
#define IS_XRAY (((*HW_REVISION >> 8) >= XRAY_MIN_VERSION) && (((*HW_REVISION >> 8) <= XRAY_MAX_VERSION)))
