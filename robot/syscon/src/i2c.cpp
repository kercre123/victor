...
static void kickOff() {
  ...
  switch (i2c_op->op) {
    ...
    case I2C_REG_READ:
      ...
      I2C2->CR2 = (1 << 16) | I2C_CR2_START | i2c_op->slave;
      break;
    case I2C_REG_WRITE_VALUE:
      setupChannel((uint8_t*)&i2c_op->data);

      I2C2->CR1 &= ~I2C_CR1_TCIE;
      I2C2->CR2 = ((i2c_op->length + 1) << 16) | I2C_CR2_START |
                  I2C_CR2_AUTOEND | i2c_op->slave;
      break;
    case I2C_REG_WRITE:
      setupChannel((uint8_t*)i2c_op->data);
...
extern "C" void I2C2_IRQHandler(void) {
  ...
  if (I2C2->ISR & I2C_ISR_TC) {
    I2C2->CR2 = (i2c_op->length << 16) | I2C_CR2_START | I2C_CR2_RD_WRN |
                I2C_CR2_AUTOEND | i2c_op->slave;
    return;
  }
...
bool I2C::multiOp(I2C_Op func, uint8_t channel, uint8_t slave, uint8_t reg,
                  int size, void* data) {
  int max_retries = 15;
  do {
    ...
    I2C_Operation opTable[] = {{func, channel, slave, reg, size, data},
                               {I2C_HOLD}};
    i2c_op = opTable;
    kickOff();
...
bool I2C::writeReg16(uint8_t channel, uint8_t slave, uint8_t reg,
                     uint16_t data) {
  return multiOp(I2C_REG_WRITE, channel, slave, reg, sizeof(data), &data);
}

bool I2C::writeReg32(uint8_t channel, uint8_t slave, uint8_t reg,
                     uint32_t data) {
  return multiOp(I2C_REG_WRITE, channel, slave, reg, sizeof(data), &data);
}
...
int I2C::readReg(uint8_t channel, uint8_t slave, uint8_t reg) {
  uint8_t data;
  if (multiOp(I2C_REG_READ, channel, slave, reg, sizeof(data), &data))
    return -1;
  return data;
}
...
void I2C::init(void) {
  ...
  I2C2->CR1 = 0;
  I2C2->TIMINGR = 0 | (5 << 20)  // Data setup time
                  | (15 << 16)   // Data hold time
                  | (30 << 8)    // SCL high
                  | (63)         // SCL low
      ;

  I2C2->CR1 = 0 | I2C_CR1_ANFOFF | I2C_CR1_RXIE | I2C_CR1_STOPIE | I2C_CR1_TXIE;
