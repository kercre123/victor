...
void Analog::tick(void) {
  ...
  bool enable_watchdog = watchdog_counter >= 5;
  bool vext_now = adc_values[ADC_VEXT] >= TRANSITION_POINT;

  handleButton();
  handleLowBattery();

  charge_cutoff = handleTemperature() 
    || disable_charger 
    || (vext_debounce >= MAX_CHARGE_TIME);
  ...
  on_charger = vext_debounce > MINIMUM_ON_CHARGER;

  if (!shouldPowerRobot()) {
    NVIC_DisableIRQ(ADC1_IRQn);

    // Powered off charger (let charger power body)
    nCHG_PWR::reset();
    POWER_EN::pull(PULL_NONE);
    POWER_EN::mode(MODE_INPUT);
