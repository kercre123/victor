...
const int BOXCAR_WIDTH =
    39;  // 50&60Hz integer divisor: 15625/80/39 = 5.008Hz (good enough)
...
void Touch::init(void) {
  ...
  TIM16->CCMR1 = 0 | (TIM_CCMR1_IC1F_0 * 4)  // Four sample debounce
                 | (TIM_CCMR1_IC1PSC_0 * 0)  // No-prescalar
                 | (TIM_CCMR1_CC1S_0 * 1)    // TI1 input mode
      ;

  TIM16->CCER = 0 | (TIM_CCER_CC1P * 0)  // Rising edge
                | TIM_CCER_CC1E          // Enable Capture 1
      ;

  TIM16->DIER = 0 | TIM_DIER_UIE;
...
extern "C" void TIM16_IRQHandler(void) {
  if (TIM16->SR & TIM_SR_CC1IF)
    sumandcount += ((TIM16->CCR1 << COUNT_BITS) | 1);
  TIM16->SR = 0;

  __disable_irq();
  CAPI::modenoirq(MODE_OUTPUT);
  CAPI::modenoirq(MODE_OUTPUT);  // ~10 cycles (100:1 discharge:charge, 1000
                                 // worst case TIM16 discharge)
  TIM16->CR1 = 0 | TIM_CR1_OPM   // One pulse mode
               | TIM_CR1_CEN     // Enabled
      ;
