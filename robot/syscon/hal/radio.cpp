...
#define TIME_PER_CUBE (int)(COUNT_PER_MS * 33.0f / MAX_CUBES)

extern GlobalDataToHead g_dataToHead;
extern GlobalDataToBody g_dataToBody;

uint8_t           currentCube = 0;
const uint8_t     cubePipe[] = {2,3,4,1};
...
void uesb_event_handler()
{
  ...
  if(rf_interrupts & UESB_INT_RX_DR_MSK)
  {
    ...
    memcpy((uint8_t*)&cubeRx[currentCube], rx_payload.data, sizeof(AcceleratorPacket));
    cubeRx[currentCube].timestamp = GetCounter();
...
void Radio::manage() {
  ...
  if (now - count >= TIME_PER_CUBE)
  {
    count = now;
    
    currentCube = (currentCube + 1) % MAX_CUBES;
