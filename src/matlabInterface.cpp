...
mxClassID Anki::ConvertToMatlabType(const char *typeName, size_t byteDepth)
{
...
std::string Anki::ConvertToMatlabTypeString(const char *typeName, size_t byteDepth)
{
  if(typeName[0] == 'u') { //unsigned
    if(byteDepth == 1) {
...
Anki::Matlab::Matlab(bool clearWorkspace)
{
...
std::string Anki::Matlab::EvalString(const char * const format, ...)
{
  if(!this->ep) {
...
std::string Anki::Matlab::EvalStringEcho(const char * const format, ...)
{
  if(!this->ep) {
    DASError("Anki.", "Matlab engine is not started/connected");
    return "";
  ...
  EvalString("if length(lastAnkiCommandBuffer)==%d lastAnkiCommandBuffer=lastAnkiCommandBuffer(2:end); end; lastAnkiCommandBuffer{end+1}=lastAnkiCommand;", Anki::Matlab::COMMAND_BUFFER_SIZE);
...
std::string Anki::Matlab::EvalStringExplicit(const char * buffer)
{
  if(!this->ep) {
    DASError("Anki.", "Matlab engine is not started/connected");
    return "";
...
std::string Anki::Matlab::EvalStringExplicitEcho(const char * buffer)
{
  if(!this->ep) {
    DASError("Anki.", "Matlab engine is not started/connected");
    return "";
...
mxArray* Anki::Matlab::GetArray(const std::string name)
{
  if(!this->ep) {
    DASError("Anki.", "Matlab engine is not started/connected");
    return NULL;
...
Anki::MatlabVariableType Anki::Matlab::GetType(const std::string name)
{
  if(!this->ep) {
    DASError("Anki.", "Matlab engine is not started/connected");
    return Anki::TYPE_UNKNOWN;
  }
  ...
  Anki::MatlabVariableType type;

  if(types[0] == 1){type=Anki::TYPE_INT8;}
  else if(types[1] == 1){type=Anki::TYPE_UINT8;}
  else if(types[2] == 1){type=Anki::TYPE_INT16;}
  else if(types[3] == 1){type=Anki::TYPE_UINT16;}
  else if(types[4] == 1){type=Anki::TYPE_INT32;}
  else if(types[5] == 1){type=Anki::TYPE_UINT32;}
  else if(types[6] == 1){type=Anki::TYPE_INT64;}
  else if(types[7] == 1){type=Anki::TYPE_UINT64;}
  else if(types[8] == 1){type=Anki::TYPE_SINGLE;}
  else if(types[9] == 1){type=Anki::TYPE_DOUBLE;}
  else{type = Anki::TYPE_UNKNOWN;}
...
s32 Anki::Matlab::GetCvMat(const CvMat *matrix, std::string name)
{
  if(!this->ep) {
    DASError("Anki.", "Matlab engine is not started/connected");
    return -1;
...
CvMat* Anki::Matlab::GetCvMat(const std::string name)
{
  CvMat* matrix = 0;
...
s32 Anki::Matlab::PutCvMat(const CvMat *matrix, std::string name)
{
  if(!this->ep) {
    DASError("Anki.", "Matlab engine is not started/connected");
    return -1;
...
s32 Anki::Matlab::PutString(const char * characters, u32 nValues, const std::string name)
{
  if(!this->ep) {
    DASError("Anki.", "Matlab engine is not started/connected");
    return -1;
...
#if defined(ANKI_USE_OPENCV)
s32 Anki::Matlab::GetIplImage(IplImage *im, const std::string name)
{
  if(!this->ep) {
    DASError("Anki.", "Matlab engine is not started/connected");
    return -1;
...
s32 Anki::Matlab::PutIplImage(const IplImage *im, const std::string name, bool flipRedBlue)
{
  if(!this->ep) {
    DASError("Anki.", "Matlab engine is not started/connected");
    return -1;
...
s32 Anki::Matlab::SetVisible(bool isVisible)
{
  if(!this->ep) {
    DASError("Anki.", "Matlab engine is not started/connected");
    return -1;
...
bool Anki::Matlab::DoesVariableExist(const std::string name)
{
  if(!this->ep) {
    DASError("Anki.", "Matlab engine is not started/connected");
    return false;
