...
static const char* crashTypeNames[NumCrashTypes] = {
    "null", "abort", "stackoverflow", "SIGABRT", "SIGFPE", "SIGILL", "SIGSEGV"};
...
void DoSIGABRT() { raise(SIGABRT); }
__attribute__((noinline)) void DoSIGFPE() { raise(SIGFPE); }
__attribute__((noinline)) void DoSIGILL() { raise(SIGILL); }
__attribute__((noinline)) void DoSIGSEGV() { raise(SIGSEGV); }
__attribute__((noinline))

static void
CrashMe(int crashType, int cmdLineSpecialCount) {
  switch (crashType) {
    case CT_NullPointer: {
      DoNullPointer();
    } break;
...
int main(int argc, char* argv[]) {
  ...
  if (argc < 2) {
    printf(
        "Usage:  vic-testcrash <crashType> [<secsBeforeCrash>] "
        "[<secsTimeout>]\n");
    ...
    printf(
        "\nsecsBeforeCrash: float number of seconds to wait before crashing "
        "(defaults to %f)\n",
        kDefaultCrashTime_s);
    printf(
        "secsTimeout: float number of seconds before program ends normally "
        "(defaults to %f)\n",
        kDefaultTimeout_s);
    exit(1);
  }
  ...
  while (!gShutdown) {
    ...
    const auto remaining_us =
        duration_cast<microseconds>(targetEndFrameTime - tickNow);
