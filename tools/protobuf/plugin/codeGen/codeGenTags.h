...
class CodeGenTags : public CodeCreator {
 public:
  CodeGenTags(const std::string& protoFile, const std::string& nameSpace,
              const TagsInfo& tags)
    ...
    // cpp-ify namespace: Anki.Cozmo.external_interface ==> namespace Anki {
    // namespace ... }
 ...
 protected:
  virtual bool WriteCode(const std::string& filename,
    ...
    printer->Print(
        ("#ifndef PROTOBUF_" + fileWithoutExt + "Tags_2eproto__INCLUDED\n")
            .c_str());
    printer->Print(
        ("#define PROTOBUF_" + fileWithoutExt + "Tags_2eproto__INCLUDED\n\n")
            .c_str());
    ...
    for (const auto& entry : _tags) {
      ...
      printer->Print({{"tagClass", entry.first}, {"underlying", type}},
                     "enum class $tagClass$ : $underlying$\n");
      printer->Print("{\n");

      printer->Indent();
      for (const auto& value : entry.second) {
        printer->Print(
            {{"name", value.first}, {"value", std::to_string(value.second)}},
            "$name$ = $value$,\n");
      }
      printer->Outdent();
