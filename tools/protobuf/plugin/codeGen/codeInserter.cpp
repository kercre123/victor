 ...
 * Created: Jun 24 2018
 *
 * Description:   Inserts code in an existing file based on placeholder
 * locations
 *
...
CodeInserter::CodeInserter(const std::string& messageName,
                           const std::string insertionName)
...
bool CodeInserter::Write(const string& filename,
                         compiler::GeneratorContext* context) {
  std::unique_ptr<io::ZeroCopyOutputStream> output(
      context->OpenForInsert(filename, _insertionPoint));
