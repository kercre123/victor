 ...
 * Created: Jun 24 2018
 *
 * Description:   Creates constructor defs and declarations based on a list of
 * params
 *
...
class CodeGenConstructors : public CodeInserter {
 public:
  CodeGenConstructors(const std::string& fullName,
                      ...
                      const std::vector<CtorInfo>& varList, bool separateCtors,
                      bool isDeclaration)
 ...
 private:
  void GenOneCtor(google::protobuf::io::Printer* printer) {
    ...
    for (int i = 0; i < _varList.size(); ++i) {
      ...
      printer->Print({{"Type", entry.type}, {"Name", entry.varName}},
                     " $Type$ $Name$");
    ...
    if (_isDeclaration) {
      ...
      for (int i = 0; i < _varList.size(); ++i) {
        ...
        if (entry.isMessageType) {
          printer->Print({{"Name", entry.varName}},
                         "set_allocated_$Name$( $Name$ );\n");
        } else {
