 ...
 * Created: Jun 24 2018
 *
 * Description: plugin for giving the proto generated code some nice features
 * from CLAD
 *
...
void FindAndReplaceAll(std::string& str, const std::string& searchStr,
...
bool AnkiProtoGenerator::Generate(const FileDescriptor* file,
  ...
  for (int i = 0; i < file->message_type_count(); i++) {
    ...
    // Todo: we might want to consider generating custom code only if the
    // message is annotated in some way. custom protobuf options (extentions)
    // don't play well with optimize_for LITE_RUNTIME, but we could annotate
    // messages with "// @@anki_gen_code=true" and look for that in
    // SourceLocation::leading_comments. For now, generate code for all the
    // messages!
    ...
    // and no repeated fields (we could support repeated fields with vectors,
    // but forcing users to use the pure protobuf methods will result in fewer
    // copies)
    ...
    if (shouldGenTags) {
      ...
      std::string tagClass;
      const bool success =
          GenerateTagAccessors(file, message, context, tagClass, tags);
      if (!success) {
        return false;
      } else {
...
bool AnkiProtoGenerator::GenerateConstructors(
    const FileDescriptor* file, const Descriptor* message,
  ...
  for (int i = 0; i < fieldCount; ++i) {
    ...
    info.isMessageType =
        (message->field(i)->cpp_type() == FieldDescriptor::CPPTYPE_MESSAGE);
  }
  const bool separateCtors = (numOneOfs > 0);
  const bool isDecl = true;
  CodeGenConstructors ctorDeclWriter{message->full_name(), messageName, list,
                                     separateCtors, isDecl};
  ...
  CodeGenConstructors ctorDefWriter{message->full_name(), messageName, list,
                                    separateCtors, !isDecl};
...
bool AnkiProtoGenerator::GenerateTagAccessors(
    const FileDescriptor* file, const Descriptor* message,
    compiler::GeneratorContext* context, std::string& outTagClass,
  ...
  if (wrote) {
    ...
    for (int i = 0; i < fieldCount; ++i) {
      ...
      std::string camelCase =
          message->oneof_decl(0)->field(i)->camelcase_name();
    ...
    outTagList[fieldCount].second =
        0;  // proto3 will guarantee that this is not used already
  }
  return wrote;
}

bool AnkiProtoGenerator::GenerateTags(
    const FileDescriptor* file, const std::string& nameSpace,
    const TagsInfo& tags,
...
std::string GetVariableType(const FieldDescriptor* message) {
  ...
  if (cppType == FieldDescriptor::CPPTYPE_MESSAGE) {
    ...
    return std::string{"::google::protobuf::"} +
           FieldDescriptor::CppTypeName(cppType);
  } else {
