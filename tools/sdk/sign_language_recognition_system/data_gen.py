        ...
        while True:
            ...
            if (ord("a") <= key <= ord("z")) or (key == ord(" ")):
                ...
                if original_image:
                    ...
                    original_image = cv2.cvtColor(np.array(original_image), cv2.COLOR_BGR2GRAY)
                    images = []
                    rotation_axes = [1, 1, 0]
                    ...
                    # Character
                    print(f"Recorded images for {chr(key)}", end="\n\r")
    finally:
        curses.nocbreak()
        curses.echo()
        curses.endwin()
...
def main():
    ...
    if not args.dataset_root_folder:
        ...
        print("No data folder defined, saving to {}".format(args.dataset_root_folder))
