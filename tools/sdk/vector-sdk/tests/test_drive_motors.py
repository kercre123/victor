...
def main():
    ...
    with vector.Robot(args.ip, str(cert), loop=loop) as robot:
        ...
        robot.set_wheel_motors_turn(100.0, curvatureRadius_mm=(math.pi * 2))
        time.sleep(1.0)
        # stop turning
        robot.set_wheel_motors_turn(0.0)
        # spin roughly all the way around (2xPi rad/s for 1 sec) with an acceleration of 100.0 mm/s2
        robot.set_wheel_motors_turn(100.0, turn_accel=100.0, curvatureRadius_mm=(math.pi * 2))
        time.sleep(1.0)
    ...
    with vector.AsyncRobot(args.ip, str(cert), loop=loop) as robot:
        ...
        robot.set_wheel_motors_turn(100.0, curvatureRadius_mm=(math.pi * 2)).wait_for_completed()
        time.sleep(1.0)
        # stop turning
        robot.set_wheel_motors_turn(0.0).wait_for_completed()
        # spin roughly all the way around (2xPi rad/s for 1 sec) with an acceleration of 100.0 mm/s2
        robot.set_wheel_motors_turn(100.0, turn_accel=100.0, curvatureRadius_mm=(math.pi * 2)).wait_for_completed()
        time.sleep(1.0)
        # stop turning and drive 0.1 m backward
        robot.set_wheel_motors_turn(0.0).wait_for_completed()
