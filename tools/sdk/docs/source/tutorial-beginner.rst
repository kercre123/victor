...
1. ``import sys`` and ``import cozmo`` allows your program to access the information contained within the ``sys`` and ``cozmo`` modules.
...
4. ``robot.say_text("Hello World").wait_for_completed`` is the core of the program.
  ...
  a. ``robot.say_text`` initializes Cozmo's speech engine.
...
..
    ...
    d. ``sys.exit`` exits the scipt in the case of an error.
...
7. Save the file in the SDK directory as ``turn.py``.
...
..
  ...
  d. ``coz.place_on_object`` directs Cozmo to place the object he is holding on top of another object.
...
Save the file as ``sing_scales.py`` in the SDK directory when completed.
