...
void ShowBlanks(Vision::FaceTracker& faceTracker,
                s32 kNumBlankFramesBetweenPeople, s32 numRows, s32 numCols,
                bool doDisplay) {
  ...
  for (s32 iBlank = 0; iBlank < kNumBlankFramesBetweenPeople; ++iBlank) {
    ...
    DEV_ASSERT(RESULT_OK == result,
               "RecognizeFaces.FaceTrackerBlankUpdateFailed");
...
void LiveEnroll(Vision::FaceTracker& faceTracker, const std::string& name,
  ...
  if (RESULT_OK != loadResult) {
    PRINT_NAMED_WARNING("LiveEnroll.LoadAlbumFail",
                        "Could not load album named '%s'", albumName.c_str());
  }

  for (auto const& nameAndID : loadedFaces) {
    if (nameAndID.name == name) {
      PRINT_NAMED_WARNING("LiveEnroll.ReplacingUser",
                          "User '%s' already exists. Will re-enroll.",
                          name.c_str());
  ...
  auto sayInstructions = [&instructions, &enrollPose, &lastModeChangeTime_ms,
                          &frame]() {
    ...
    if (success) {
      if (frame.IsEmpty()) {
        ...
        if (ready) {
          ...
          if (!faces.empty()) {
            ...
            if (face.GetName().empty()) {
              ...
              if (newEnrollPose != enrollPose) {
                faceTracker.SetFaceEnrollmentMode(
                    Vision::FaceEnrollmentPose::Disabled);

                if (frame.GetTimestamp() - lastModeChangeTime_ms >
                    kMinTimePerEnrollMode_ms) {
              ...
              snprintf(temp, 127, "ID=%d D=%.1f R=%.1f, P=%.1f, Y=%.1f",
                       face.GetID(), face.GetIntraEyeDistance(),
          ...
          frame.DrawText({1, frame.GetNumRows() - 20},
                         instructions.at(enrollPose), NamedColors::RED, 0.5f);
          frame.DrawText({1, frame.GetNumRows() - 1},
...
void LiveRecognize(Vision::FaceTracker& faceTracker,
                   const std::string& albumName) {
  ...
  bool done = false;
  std::function<void()> SetDone = [&done]() { done = true; };
  ...
  while (!done) {
    ...
    if (success) {
      if (frame.IsEmpty()) {
        ...
        if (!faces.empty()) {
          face = faces.front();
          frame.DrawText({1, frame.GetNumRows() - 1}, face.GetName(),
                         NamedColors::RED, 0.7f);
          frame.DrawRect(face.GetRect(), NamedColors::RED, 2);

          if (!face.GetName().empty() &&
              frame.GetTimestamp() > nextSayNameTime) {
            std::thread sayNameThread(sayName);
            sayNameThread.detach();

            nextSayNameTime = frame.GetTimestamp() +
                              rng.RandIntInRange(kMinSayNameSpacing_ms,
                                                 kMaxSayNameSpacing_ms);
          }
        }
...
void CannedRun(const std::string& mode, Vision::FaceTracker& faceTracker,
               const std::string& cwdPath, const std::string& inputPath) {
  using namespace Anki::Util;

  static const std::vector<const char*> imageFileExtensions = {"jpg", "jpeg",
                                                               "png"};
  ...
  const s32 kTestBlankingFrequency =
      15;  // blank out every this-many frames during test to restart tracker
  const std::string albumName(
      FileUtils::FullFilePath({cwdPath, "trainedAlbum"}));
  ...
  if (mode == "Train") {
    ...
    for (s32 iGroup = 0; iGroup < groupDirs.size(); ++iGroup) {
      ...
      FileUtils::ListAllDirectories(
          FileUtils::FullFilePath({inputPath, groupDirs[iGroup]}), peopleDirs);

      for (s32 iPerson = 0; iPerson < peopleDirs.size(); ++iPerson) {
        auto imageFiles = FileUtils::FilesInDirectory(
            FileUtils::FullFilePath(
                {inputPath, groupDirs[iGroup], peopleDirs[iPerson]}),
        ...
        do {
          ...
          const std::string fullImageFile(FileUtils::FullFilePath(
              {inputPath, groupDirs[iGroup], peopleDirs[iPerson],
               imageFiles[iFile]}));
          img.Load(fullImageFile);
          if (img.IsEmpty()) {
            PRINT_NAMED_WARNING("RecognizeFaces.ImageLoadFailure",
                                "Image %d: %s", iFile, fullImageFile.c_str());
            ++iFile;
            continue;
          }
          ...
          DEV_ASSERT(RESULT_OK == result,
                     "RecognizeFaces.FaceTrackerUpdateFailed");

          if (faces.size() > 0) {
            if (faces.size() > 1) {
              PRINT_NAMED_WARNING("RecognizeFaces.MultipleFaces",
                                  "Found %lu faces in %s. Using first.",
          ...
          if (kDoDisplay) {
            ...
            if (!faces.empty()) {
              ...
              dispImg.DrawText(
                  faceIter->GetRect().GetTopLeft(),
                  "N=" + std::to_string(faceIter->GetNumEnrollments()),
                  NamedColors::RED, .75f);
              ++faceIter;
            ...
            dispImg.DrawText({0, dispImg.GetNumRows()}, imageFiles[iFile],
                             NamedColors::RED, .5f);
        ...
        } while (++iFile < imageFiles.size() &&
                 face.GetNumEnrollments() < kNumEnrollmentsNeeded);
        ...
        if (face.GetNumEnrollments() >= kNumEnrollmentsNeeded) {
          ...
          PRINT_NAMED_INFO("RecognizeFaces.EnrollmentCompleted",
                           "Enrolled ID %d with name %s", face.GetID(),
                           peopleDirs[iPerson].c_str());
        ...
        ShowBlanks(faceTracker, kNumBlankFramesBetweenPeople, img.GetNumRows(),
                   img.GetNumCols(), kDoDisplay);
    ...
    for (s32 iGroup = 0; iGroup < groupDirs.size(); ++iGroup) {
      ...
      for (s32 iPerson = 0; iPerson < peopleDirs.size(); ++iPerson) {
        ...
        if (saveFaceCropSize > 0) {
          saveDir = FileUtils::FullFilePath(
              {inputPath, groupDirs[iGroup], peopleDirs[iPerson], "faceCrop"});
        ...
        for (s32 iFile = 0; iFile < imageFiles.size(); ++iFile) {
          ...
          if (iFile % kTestBlankingFrequency == 0) {
            ShowBlanks(faceTracker, kNumBlankFramesBetweenPeople,
                       img.GetNumRows(), img.GetNumCols(), kDoDisplay);
            fakeTime_ms += kNumBlankFramesBetweenPeople * kFramePeriod_ms;
          }
          ...
          if (faces.size() > 0) {
            ...
            if (saveFaceCropSize > 0) {
              ...
              Rectangle<s32> roiRect(rectF32.GetX(), rectF32.GetY(),
                                     rectF32.GetWidth(), rectF32.GetHeight());
              Vision::Image faceROI = img.GetROI(roiRect);
              faceROI.Resize(saveFaceCropSize, saveFaceCropSize);
              cv::imwrite(FileUtils::FullFilePath({saveDir, imageFiles[iFile]}),
                          faceROI.get_CvMat_());
            }
          ...
          if (kDoDisplay) {
            ...
            if (!faces.empty()) {
              ...
              dispImg.DrawText({0, dispImg.GetNumRows() - 1},
                               faceIter->GetName() + " Score=" +
                                   std::to_string(faceIter->GetScore()),
                               *drawColor, .75f);
              ++faceIter;
              while (faceIter != faces.end()) {
                dispImg.DrawRect(faceIter->GetRect(), NamedColors::CYAN, 2);
    ...
    for (auto const& detection : detections) {
      fileBody << detection.detectedID << " " << detection.correctID << " "
               << detection.score << std::endl;
    }
    FileUtils::WriteFile(
        FileUtils::FullFilePath({inputPath, "detectionResults.txt"}),
        fileBody.str());

  } else {
    PRINT_NAMED_ERROR("RecognizeFaces.CannedData.UnknownMode", "%s",
                      mode.c_str());
  }
...
int main(int argc, char* argv[]) {
  ...
  if (argc < 2 || (argc > 1 && std::string(argv[1]) == "help")) {
    ...
    printf("\n");
    printf(
        "   LiveEnroll:    Enroll a new person for 'live' recognition with USB "
        "camera.\n");
    printf(
        "                  Use [option] to specify the name for the "
        "enrollee.\n");
    printf(
        "                  Will overwrite existing user of same name, if "
        "any.\n");
    printf("                  Click in window to start.\n");
    printf("\n");
    printf(
        "   LiveRecognize: Open USB camera and recognize enrolled faces in "
        "each frame.\n");
    printf("                  Click in window to stop.\n");
    printf("\n");
    printf(
        "   CannedTrain:   Enroll faces from saved images from Cozmo's "
        "camera.\n");
    printf(
        "                  Use [option] to specify the root path for the "
        "data.\n");
    printf("\n");
    printf(
        "   CannedTest:    Complement to CannedTrain for testing the enrolled "
        "album.\n");
    printf(
        "                  Use [option] to specify the root path for the "
        "data.\n");
  ...
  Util::Data::DataPlatform dataPlatform(filesPath, cachePath, externalPath,
                                        resourcePath);
  ...
  const std::string dataPath(cozmoContext.GetDataPlatform()->pathToResource(
      Util::Data::Scope::Resources, "/config/engine/vision"));
