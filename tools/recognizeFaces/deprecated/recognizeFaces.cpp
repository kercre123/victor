...
static const char *luxandLicense =
    "d1mT6FU+dbiXs+gfPa3iFp2KcWHh/"
    "FsIN5khjZ3UkfvJswLajGu0+"
    "OxP8Q0Aatw7oqqEvXV6nXs0tl1lh0F4tPQX4uq5PwxmMzYFhfUyfLN01N+"
    "zCq4iZMdKLYPCWSbDWQH5+yMbqyQcoW1bvus67WKaaLG8/RZsxf7wm5bMdJw=";
...
static HImage OpencvToFsdk(const cv::Mat_<u8> &in) {
  ...
  FSDK_LoadImageFromBuffer(&fsdkImage, in.ptr(), in.cols, in.rows, in.step[0],
...
Result LoadDatabase(const char *filename) {
  ...
  if (filename) {
    int loadResult;
    if (FSDKE_OK !=
        (loadResult = FSDK_LoadTrackerMemoryFromFile(&tracker, filename))) {
...
Result SetRecognitionParameters(const bool handleArbitraryRotations,
                                const int internalResizeWidth,
                                const int faceDetectionThreshold) {
  Result lastResult;
  if ((lastResult = InitializeTracker()) != RESULT_OK) {
    return RESULT_FAIL;
  }

  // FSDK_SetFaceDetectionParameters(handleArbitraryRotations, false,
  // internalResizeWidth);
  ...
  if (FSDK_SetTrackerParameter(tracker, "HandleArbitraryRotations,",
                               parameterBuffer) != FSDKE_OK) {
    printf("set/TrackerPameter fail 1\n");
    result = RESULT_FAIL;
  }

  snprintf(parameterBuffer, 1024, "%d", internalResizeWidth);
  if (FSDK_SetTrackerParameter(tracker, "InternalResizeWidth",
                               parameterBuffer) != FSDKE_OK) {
  ...
  if (FSDK_SetTrackerParameter(tracker, "FaceDetectionThreshold",
                               parameterBuffer) != FSDKE_OK) {
  ...
  // int errorPosition = -1;
  // const int setResult = FSDK_SetTrackerMultipleParameters(tracker,
  // "HandleArbitraryRotations=true; DetermineFaceRotationAngle=false;
  // InternalResizeWidth=960; FaceDetectionThreshold=1;", &errorPosition);
  // printf("setResult:%d ErrorPosition: %d ", setResult, errorPosition);

  FSDK_GetTrackerParameter(tracker, "HandleArbitraryRotations",
                           &parameterBuffer[0], 1024);
  printf("HandleArbitraryRotations:%s ", parameterBuffer);

  FSDK_GetTrackerParameter(tracker, "DetermineFaceRotationAngle",
                           &parameterBuffer[0], 1024);
  printf("DetermineFaceRotationAngle:%s ", parameterBuffer);

  FSDK_GetTrackerParameter(tracker, "InternalResizeWidth", &parameterBuffer[0],
                           1024);
  printf("InternalResizeWidth:%s ", parameterBuffer);

  FSDK_GetTrackerParameter(tracker, "FaceDetectionThreshold",
                           &parameterBuffer[0], 1024);
...
Result RecognizeFaces(const cv::Mat_<u8> &image, std::vector<Face> &faces,
  ...
  FSDK_GetTrackerParameter(tracker, "HandleArbitraryRotations",
  &parameterBuffer[0], 1024); printf("HandleArbitraryRotations:%s ",
  parameterBuffer);

  FSDK_GetTrackerParameter(tracker, "DetermineFaceRotationAngle",
  &parameterBuffer[0], 1024); printf("DetermineFaceRotationAngle:%s ",
  parameterBuffer);

  FSDK_GetTrackerParameter(tracker, "InternalResizeWidth", &parameterBuffer[0],
  1024); printf("InternalResizeWidth:%s ", parameterBuffer);

  FSDK_GetTrackerParameter(tracker, "FaceDetectionThreshold",
  &parameterBuffer[0], 1024); printf("FaceDetectionThreshold:%s ",
  parameterBuffer);
  */
