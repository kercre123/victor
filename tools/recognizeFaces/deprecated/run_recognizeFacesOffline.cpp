...
int main(int argc, const char *argv[]) {
  ...
  const char *inputFilenamePattern =
      "/Users/pbarnum/Documents/datasets/cozmoPlaytests/Cozmo - Bianca.MP4";
  ...
  if (argc >= 9 && argc <= 9) {
    ...
    printf(
        "Usage:\n"
        "run_recognizeFacesOffline inputFilenamePattern outputFilenamePattern "
        "firstFileIndex lastFileIndex imageWidth imageHeight "
        "faceDetectionThreshold handleArbitraryRotations\n"
        "Examples:\n"
        "run_recognizeFacesOffline "
        "/Volumes/PeterMac/cozmoPlaytests/Cozmo_Bianca/"
        "Cozmo_Bianca_%%SB05d.png "
        "/Users/pbarnum/Documents/tmp/cozmoPlaytestsResults/Cozmo_Bianca/"
        "Cozmo_Bianca_%%05d.png 1 500 960 540 5 1\n"
        "run_recognizeFacesOffline "
        "\"/Users/pbarnum/Documents/datasets/cozmoPlaytests/Cozmo - "
        "Bianca.MP4\" "
        "/Volumes/PeterMac/cozmoPlaytestsResults/Cozmo_Bianca/"
        "Cozmo_Bianca_%%05d.png 1 10000000 960 540 5 0\n");

    return -1;
  }

  printf("Running with %s %s %d %d (%d,%d) %d %d\n", inputFilenamePattern,
         outputFilenamePattern, firstFileIndex, lastFileIndex, imageSize.width,
         imageSize.height, faceDetectionThreshold, handleArbitraryRotations);

  bool isVideo = true;
  const char *inputSuffix =
      inputFilenamePattern + (strlen(inputFilenamePattern) - 3);
  if (!strcmp("png", inputSuffix) || !strcmp("pgm", inputSuffix) ||
      !strcmp("jpg", inputSuffix) || !strcmp("PNG", inputSuffix) ||
      !strcmp("PGM", inputSuffix) || !strcmp("JPG", inputSuffix)) {
    isVideo = false;
  ...
  SetRecognitionParameters(handleArbitraryRotations, imageSize.width,
  ...
  for (s32 iImage = firstFileIndex; iImage <= lastFileIndex; iImage++) {
    ...
    cv::Mat lastImageClipped =
        lastImageRaw(
            cv::Range(0, lastImageRaw.rows / 2),
            cv::Range(lastImageRaw.cols / 4, (3 * lastImageRaw.cols) / 4))
            .clone();
    ...
    for (size_t iFace = 0; iFace < faces.size(); iFace++) {
      cv::rectangle(toShowImage,
                    cv::Point(faces[iFace].xc - faces[iFace].w / 2,
                              faces[iFace].yc - faces[iFace].w / 2),
                    cv::Point(faces[iFace].xc + faces[iFace].w / 2,
                              faces[iFace].yc + faces[iFace].w / 2),
                    cv::Scalar(0, 255, 0), 5);

      char name[1024];
      snprintf(name, 1024, "%d %s (%dpix)", faces[iFace].faceId,
               faces[iFace].name.data(), faces[iFace].w);

      cv::putText(toShowImage, name,
                  cv::Point(faces[iFace].xc, faces[iFace].yc),
    ...
    cv::resize(toShowImage, toShowImageSmall, toSizeSize, 0, 0,
               cv::INTER_LINEAR);
