...
"/Users/pbarnum/Documents/Anki/products-cozmo-large-files/faceRecognition/ankiPeople/";
...
int main(int argc, const char *argv[]) {
  ...
  /*  std::vector<std::vector<cv::Mat_<u8> > > trainingImages =
    GetTrainingImages();

    for(s32 iPerson=0; iPerson<trainingImages.size(); iPerson++) {
      for(s32 iImage=0; iImage<trainingImages.size(); iImage++) {
        const Result addFaceResult =
    AddFaceToDatabase(trainingImages[iPerson][iImage], iPerson);
  ...
  while (isRunning) {
    ...
    for (size_t iFace = 0; iFace < faces.size(); iFace++) {
      ...
      char name[1024];
      snprintf(name, 1024, "%d %s", faces[iFace].faceId,
               faces[iFace].name.data());
    ...
    if (ABS(Anki::Embedded::GetTimeF64() - trainingStartTime) >
        numSecondsToTrain) {
      trainingOn = false;
    }
    ...
    if (trainingOn) {
      ...
      cv::putText(toShowImage, name, cv::Point(30, 30), cv::FONT_HERSHEY_PLAIN,
