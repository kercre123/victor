...
Function to capture images as quickly as possible, when a high and constant
frame rate is critical. You should ideally compile this as RelWithDebInfo or
Release. It only depends on the OpenCV libraries, not all the normal Anki
libraries.
...
static double GetTimeF64() {
  ...
  const double timeInSeconds =
      (double)(time.tv_sec - startSeconds) + ((double)time.tv_usec / 1000000.0);
#else  // Generic Unix
  timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
  const double timeInSeconds =
      (double)(ts.tv_sec) + (double)(ts.tv_nsec) * (1.0 / 1000000000.0);
#endif
...
namespace Anki {
int CaptureImages(const int cameraId, const int numImages,
                  const cv::Size2i imageSize,
                  std::vector<cv::Mat> &capturedImages, double &captureTime,
                  bool &wasQuitPressed, const bool startCaptureImmediately,
                  const bool showPreview, const bool showCrosshair,
                  const bool cameraSettingsGui) {
  ...
  for (int i = 1; i < numImages; i++) {
    // capturedImages[i].create(capturedImages[0].rows, capturedImages[0].cols,
    // capturedImages[0].type());
  ...
  const cv::Point2i center((imageSize.width + 1) / 2,
  ...
  while (curFrame < numImages) {
    ...
    if (showPreview) {
      if (showCrosshair) {
        ...
        toShowImage(cv::Rect(0, center.y - crosshairBlackHalfWidth,
                             imageSize.width, 2 * crosshairBlackHalfWidth + 1))
            .setTo(0);
        toShowImage(cv::Rect(center.x - crosshairBlackHalfWidth, 0,
                             2 * crosshairBlackHalfWidth + 1, imageSize.height))
            .setTo(0);

        toShowImage(cv::Rect(0, center.y - crosshairWhiteHalfWidth,
                             imageSize.width, 2 * crosshairWhiteHalfWidth + 1))
            .setTo(255);
        toShowImage(cv::Rect(center.x - crosshairWhiteHalfWidth, 0,
                             2 * crosshairWhiteHalfWidth + 1, imageSize.height))
            .setTo(255);
      ...
      if ((pressedKey & 0xFF) == 'h') {
        printf(
            "Hold 'c' to start capture, 's' to change camera settings (windows "
            "only), or 'q' to quit.\n");
