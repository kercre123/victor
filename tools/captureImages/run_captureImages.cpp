...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    printf("Running with default parameters %d %d (%d,%d) %s %d %d %d %d\n",
           cameraId, numImages, imageSize.width, imageSize.height,
           filenamePattern, startCaptureImmediately, showPreview, showCrosshair,
           captureOnce);
    ...
    if (argc >= 8) showPreview = static_cast<bool>(strtol(argv[7], &pEnd, 10));

    if (argc >= 9)
      showCrosshair = static_cast<bool>(strtol(argv[8], &pEnd, 10));

    if (argc >= 10) captureOnce = static_cast<bool>(strtol(argv[9], &pEnd, 10));
  } else {
    printf(
        "Usage:\n"
        "run_captureImages cameraId numImages imageWidth imageHeight "
        "filenamePattern <startCaptureImmediately> <showPreview> "
        "<showCrosshair> <captureOnce> \n"
        "Examples:\n"
        "run_captureImages 1 100 640 480 "
        "\"/Users/pbarnum/tmp/images_%%05d.png\"\n"
        "run_captureImages 1 100 640 480 "
        "\"/Users/pbarnum/tmp/images_%%05d.png\" 0 1 1 1s\n");
    return -1;
  }
  ...
  while (!wasQuitPressed) {
    // captureTime is the time right between right before the first frame to
    // right after the last frame
    captureImagesResult = Anki::CaptureImages(
        cameraId, numImages, imageSize, capturedImages, captureTime,
        wasQuitPressed, startCaptureImmediately, showPreview, showCrosshair,
        cameraSettingsGui);

    printf("Captured %d images at %0.2f FPS. Saving ",
           static_cast<int>(capturedImages.size()),
           1.0 / (captureTime / capturedImages.size()));
