...
int init(const int windowWidth, const int windowHeight) {
  ...
  if (initSDL(windowWidth, windowHeight) < 0) return -1;

  if (initGL(windowWidth, windowHeight) < 0) return -2;
...
static int initSDL(const int windowWidth, const int windowHeight) {
  ...
  sdlWindow = SDL_CreateWindow(
      "Voxels", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, windowWidth,
...
int close() {
  ...
  if (voxelMutex) SDL_DestroyMutex(voxelMutex);
...
static int render() {
  ...
  const float camX = cameraZ * -sinf(cameraTheta * (M_PI / 180)) *
                     cosf((cameraPhi) * (M_PI / 180));
  const float camY = cameraZ * -sinf((cameraPhi) * (M_PI / 180));
  const float camZ = -cameraZ * cosf((cameraTheta) * (M_PI / 180)) *
                     cosf((cameraPhi) * (M_PI / 180));
...
int gameMain(f64 numSecondsToRun) {
  ...
  while (isRunning) {
    if (numSecondsToRun > 0 &&
        (Anki::Embedded::GetTimeF64() - startTime) > numSecondsToRun) {
      isRunning = false;
      continue;
    }
