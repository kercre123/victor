...
def copy_all_recordings(robot_ip, recordings_path, store_path):
    ...
    command_text = eval("['scp', '-r', '{}@{}:{}', '{}']".format(ROBOT_USERNAME, robot_ip, recordings_path, store_path))
...
def get_folder_size(folder_path, robot_ip = ""):
    if robot_ip == "":
        ...
        command_text = eval("['ssh', '{}@{}', 'du -sh {}']".format(ROBOT_USERNAME, robot_ip, folder_path))
        folder_size = get_command_output(command_text).split("\t")[0]
    return folder_size
...
def get_created_date_of_recordings_folder(robot_ip, recordings_folder_path):
    command_text = eval("['ssh', '{}@{}', 'stat -c \"%y\" {}']".format(ROBOT_USERNAME, robot_ip, recordings_folder_path))
