...
#define REGISTER_COZMO_SIM_TEST_CLASS(CLASS) \
  static CozmoSimTestRegistrar<CLASS> registrar(#CLASS);
...
#define CST_EXPECT(x, errorStreamOutput)                                       \
  if (!(x)) {                                                                  \
    PRINT_STREAM_WARNING("CST_EXPECT", "(" << #x << "): " << errorStreamOutput \
                                           << "(" << __FILE__ << "."           \
...
#define CST_ASSERT(x, errorStreamOutput)                                       \
  if (!(x)) {                                                                  \
    PRINT_STREAM_WARNING("CST_ASSERT", "(" << #x << "): " << errorStreamOutput \
                                           << "(" << __FILE__ << "."           \
                                           << __FUNCTION__ << "." << __LINE__  \
                                           << ")");                            \
    _result = -1;                                                              \
...
#define CONDITION_WITH_TIMEOUT_ASSERT(cond, start_time, timeout)   \
  (IsTrueBeforeTimeout(cond, #cond, start_time, timeout, __FILE__, \
                       __FUNCTION__, __LINE__))
...
#define IF_CONDITION_WITH_TIMEOUT_ASSERT(cond, timeout)                       \
  static double COMBINE(startTime, __LINE__) = GetSupervisor().getTime();     \
  if (IsTrueBeforeTimeout(cond, #cond, COMBINE(startTime, __LINE__), timeout, \
                          __FILE__, __FUNCTION__, __LINE__))

#define IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(timeout, ...)                 \
  static double COMBINE(startTime, __LINE__) = GetSupervisor().getTime();   \
  if (AllTrueBeforeTimeout({__VA_ARGS__}, {PP_STRINGIZE_X(__VA_ARGS__)},    \
                           COMBINE(startTime, __LINE__), timeout, __FILE__, \
                           __FUNCTION__, __LINE__))

// Derived classes should create an enum class called TestState, and a variable
// called _testState. They should follow the pattern of modifying test state via
// this macro instead of directly.
#define SET_TEST_STATE(s)                                                     \
  {                                                                           \
    PRINT_NAMED_INFO("CozmoSimTestController.TransitionTestState", "%s", #s); \
...
class CozmoSimTestController : public UiGameController {
 ...
 protected:
  ...
  bool IsTrueBeforeTimeout(bool cond, const char* condAsString,
                           double start_time, double timeout, const char* file,
  ...
  // Use to take regular screenshots - on the build server this is preferable to
  // recording movies
  ...
  void SetBlockDebugPrintInterval(double interval_s) {
    _printInterval_s = interval_s;
  }

  double _nextPrintTime = -1.0f;
  double _printInterval_s = 1.0;

  bool IsRobotPoseCorrect(const Point3f& distThreshold,
                          const Radians& angleThreshold,
                          const Pose3d& transform = Pose3d()) const;

  bool IsObjectPoseWrtRobotCorrect(s32 objectID, const Pose3d& actualPose,
  ...
  // Hiding UiGameController's implementation in order to add asserts on send
  // failure
...
// Factory for creating and registering tests derived from
// CozmoSimTestController
class CozmoSimTestFactory {
 ...
 protected:
  std::map<std::string, std::function<CozmoSimTestController*(void)>>
...
class CozmoSimTestRegistrar {
 public:
  CozmoSimTestRegistrar(std::string className) {
    ...
    CozmoSimTestFactory::getInstance()->RegisterFactoryFunction(
        className, [](void) -> CozmoSimTestController* { return new T(); });
  }
};
