 ...
 * Date:
 * Description:   Any UI/Game to be run as a Webots controller should be derived
 * from this class. Author: Modifications:
 */
...
class UiGameController {
 public:
  ...
  ///
  // @brief      Cycles the viz origin between all observed cubes and the robot
  // itself.
  //
  void CycleVizOrigin();
 ...
 protected:
  ...
  virtual void HandleRobotDeletedLocatedObject(
      const ExternalInterface::RobotDeletedLocatedObject& msg){};
  virtual void HandleUiDeviceAvailable(
      const ExternalInterface::UiDeviceAvailable& msgIn){};
  virtual void HandleUiDeviceConnected(
  ...
  virtual void HandleActiveObjectAccel(
      const ExternalInterface::ObjectAccel& msg){};
  virtual void HandleActiveObjectAvailable(
  ...
  virtual void HandleDefinedCustomObject(
      const ExternalInterface::DefinedCustomObject& msg){};
  virtual void HandleRobotDeletedAllCustomObjects(
      const ExternalInterface::RobotDeletedAllCustomObjects& msg){};
  virtual void HandleRobotDeletedCustomMarkerObjects(
      const ExternalInterface::RobotDeletedCustomMarkerObjects& msg){};
  virtual void HandleRobotDeletedFixedCustomObjects(
  ...
  uint32_t SendTurnInPlace(
      const f32 angle_rad, const f32 speed_radPerSec = 0.f,
      const f32 accel_radPerSec2 = 0.f,
      const f32 tol_rad =
          0.f,  // 0: Use default tolerance of POINT_TURN_ANGLE_TOL
      const bool isAbsolute = false,
      const QueueActionPosition queueActionPosition = QueueActionPosition::NOW);

  // Queue a generic action, automatically set robot id, id tag, and position to
  // run now
  void SendAction(const ExternalInterface::QueueSingleAction& msg_in);

  void SendTurnInPlaceAtSpeed(const f32 speed_rad_per_sec,
  ...
  void SendMoveLiftToHeight(const f32 mm, const f32 speed, const f32 accel,
                            const f32 duration_sec = 0.f);
  void SendMoveLiftToAngle(const f32 angle_rad, const f32 speed,
                           const f32 accel, const f32 duration_sec = 0.f);
  ...
  void SendExecutePathToPose(const Pose3d& p, PathMotionProfile motionProf);

  void SendGotoObject(const s32 objectID, const f32 distFromObjectOrigin_mm,
                      PathMotionProfile motionProf,
                      const bool usePreDockPose = false);

  void SendAlignWithObject(const s32 objectID, const f32 distFromMarker_mm,
                           PathMotionProfile motionProf,
  ...
  void SendPickupObject(const s32 objectID, PathMotionProfile motionProf,
                        const bool usePreDockPose,
                        const bool useApproachAngle = false,
  ...
  void SendPlaceOnObject(const s32 objectID, PathMotionProfile motionProf,
                         const bool usePreDockPose,
                         const bool useApproachAngle = false,
                         const f32 approachAngle_rad = 0);
  ...
  void SendPlaceRelObject(const s32 objectID, PathMotionProfile motionProf,
                          const bool usePreDockPose,
  ...
  void SendRollObject(const s32 objectID, PathMotionProfile motionProf,
                      const bool doDeepRoll, const bool usePreDockPose,
                      const bool useApproachAngle = false,
                      const f32 approachAngle_rad = 0);
  ...
  void SendPopAWheelie(const s32 objectID, PathMotionProfile motionProf,
                       const bool usePreDockPose,
                       const bool useApproachAngle = false,
                       const f32 approachAngle_rad = 0);

  void SendFacePlant(const s32 objectID, PathMotionProfile motionProf,
                     const bool usePreDockPose,
                     const bool useApproachAngle = false,
                     const f32 approachAngle_rad = 0);

  void SendMountCharger(const s32 objectID, PathMotionProfile motionProf,
  ...
  void SendControllerGains(ControllerChannel channel, f32 kp, f32 ki, f32 kd,
                           f32 maxErrorSum);
  void SendRollActionParams(f32 liftHeight_mm, f32 driveSpeed_mmps,
                            f32 driveAccel_mmps2, u32 driveDuration_ms,
                            f32 backupDist_mm);
  ...
  void SendCubeAnimation(const u32 objectID,
                         const CubeAnimationTrigger cubeAnimTrigger);
  void SendStopCubeAnimation(const u32 objectID,
                             const CubeAnimationTrigger cubeAnimTrigger);
  void SendAnimation(const char* animName, u32 numLoops,
                     bool throttleMessages = false);
  void SendAnimationGroup(const char* animGroupName, u32 numLoops,
                          bool throttleMessages = false);
  void SendAnimationTrigger(const char* animTriggerName, u32 numLoops,
                            bool throttleMessages = false);
  ...
  uint32_t SendQueuePlayAnimAction(const std::string& animName, u32 numLoops,
  ...
  void SendCameraCalibration(f32 focalLength_x, f32 focalLength_y, f32 center_x,
                             f32 center_y);
  ...
  void SendSetAllActiveObjectLEDs(
  ...
  const ExternalInterface::RobotState& GetRobotState() const {
    return _robotStateMsg;
  }
  ...
  void AddOrUpdateObject(s32 objID, ObjectType objType,
                         ...
                         const uint32_t observedTimestamp, const bool isActive);
  ...
  ///
  // @brief      Iterates through _lightCubes and removes the one of the given
  // ObjectType
  //             (should be unique).
  ...
  // @param[in]  factoryID  FactoryID of cube to be added. (If 0, then factoryID
  // is auto-generated based on ObjectType)
  ...
  ///
  // @brief      Gets the node by definition name defined in the webots world
  // files (.wbt).
  ...
  // @return     The pose of the webots node; translation units are in
  // millimeters.
  //
  ...
  ///
  // @brief      Determines if x seconds passed since the first time this
  // function was called.
  //             Useful in the CST test controllers where the same code block in
  //             each state can be called many times repeatedly as
  //             UpdateInternal is called on every tick and waiting by blocking
  //             the thread is not an option as the time will not advance if the
  //             thread is blocked.
  // @param[in]  xSeconds   Number of seconds to wait for.
  // @return     True if has x seconds passed since the first call of the
  ...
  // @param[in]  defName  The defName of the node to apply force to.
  // (https://www.cyberbotics.com/reference/def-and-use.php)
  // @param[in]  xForce   The x force
  // @param[in]  yForce   The y force
  // @param[in]  zForce   The z force
  //
  void SendApplyForce(const std::string& defName, int xForce, int yForce,
                      int zForce);
 ...
 private:
  ...
  void HandleRobotDelocalizedBase(
      const ExternalInterface::RobotDelocalized& msg);
  void HandleRobotOffTreadsStateChangedBase(
      const ExternalInterface::RobotOffTreadsStateChanged& msg);
  void HandleRobotObservedObjectBase(
      const ExternalInterface::RobotObservedObject& msg);
  void HandleRobotObservedFaceBase(
      const ExternalInterface::RobotObservedFace& msg);
  void HandleRobotObservedPetBase(
      const ExternalInterface::RobotObservedPet& msg);
  void HandleRobotDeletedLocatedObjectBase(
      const ExternalInterface::RobotDeletedLocatedObject& msg);
  void HandleUiDeviceAvailableBase(
      const ExternalInterface::UiDeviceAvailable& msg);
  void HandleUiDeviceConnectedBase(
      const ExternalInterface::UiDeviceConnected& msg);
  void HandleRobotConnectedBase(
      const ExternalInterface::RobotConnectionResponse& msg);
  void HandleRobotCompletedActionBase(
      const ExternalInterface::RobotCompletedAction& msg);
  void HandleImageChunkBase(const ImageChunk& msg);
  void HandleActiveObjectAccelBase(const ExternalInterface::ObjectAccel& msg);
  void HandleActiveObjectAvailableBase(
      const ExternalInterface::ObjectAvailable& msg);
  void HandleActiveObjectConnectionStateBase(
      const ExternalInterface::ObjectConnectionState& msg);
  void HandleActiveObjectMovedBase(const ExternalInterface::ObjectMoved& msg);
  void HandleActiveObjectStoppedMovingBase(
      const ExternalInterface::ObjectStoppedMoving& msg);
  void HandleActiveObjectTappedBase(const ExternalInterface::ObjectTapped& msg);
  void HandleActiveObjectUpAxisChangedBase(
      const ExternalInterface::ObjectUpAxisChanged& msg);
  void HandleLocatedObjectStatesBase(
      const ExternalInterface::LocatedObjectStates& msg);
  void HandleConnectedObjectStatesBase(
      const ExternalInterface::ConnectedObjectStates& msg);
  void HandleAnimationAvailableBase(
      const ExternalInterface::AnimationAvailable& msg);
  void HandleAnimationAbortedBase(
      const ExternalInterface::AnimationAborted& msg);
  void HandleBehaviorTransitionBase(
      const ExternalInterface::BehaviorTransition& msg);
  ...
  void HandleEngineErrorCodeBase(
      const ExternalInterface::EngineErrorCodeMessage& msg);
  void HandleDefinedCustomObjectBase(
      const ExternalInterface::DefinedCustomObject& msg);
  void HandleRobotDeletedAllCustomObjectsBase(
      const ExternalInterface::RobotDeletedAllCustomObjects& msg);
  void HandleRobotDeletedCustomMarkerObjectsBase(
      const ExternalInterface::RobotDeletedCustomMarkerObjects& msg);
  void HandleRobotDeletedFixedCustomObjectsBase(
      const ExternalInterface::RobotDeletedFixedCustomObjects& msg);

  void UpdateActualObjectPoses();

  ///
  // @brief      Iterates through _lightCubes and returns the first light cube
  // with the given ID
  //             (should be unique).
  ...
  typedef enum { UI_WAITING_FOR_GAME = 0, UI_RUNNING } UI_State_t;
