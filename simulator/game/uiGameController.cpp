...
void UiGameController::AddOrUpdateObject(s32 objID, ObjectType objType,
                                         const PoseStruct3d& poseStruct,
                                         const uint32_t observedTimestamp,
                                         const bool isActive) {
  // If an object with the same ID already exists in the map, make sure that its
  // type hasn't changed and update its observed time
  auto it = std::find_if(
      _observedObjects.begin(), _observedObjects.end(),
      [&objID](const ObservedObject& obj) { return (obj.id == objID); });
  if (it != _observedObjects.end()) {
    if (it->type != objType) {
      PRINT_NAMED_WARNING(
          "UiGameController.HandleRobotObservedObjectBase.ObjectChangedType",
          "");
    }
...
void UiGameController::HandleRobotStateUpdateBase(
    const ExternalInterface::RobotState& msg) {
  ...
  const bool hasChangedLocalization =
      (_robotStateMsg.localizedToObjectID != msg.localizedToObjectID);
...
void UiGameController::HandleRobotDelocalizedBase(
...
void UiGameController::HandleRobotObservedObjectBase(
    const ExternalInterface::RobotObservedObject& msg) {
  AddOrUpdateObject(msg.objectID, msg.objectType, msg.pose, msg.timestamp,
                    msg.isActive);

  HandleRobotObservedObject(msg);
}

void UiGameController::HandleRobotObservedFaceBase(
...
void UiGameController::HandleRobotObservedPetBase(
    const ExternalInterface::RobotObservedPet& msg) {
  HandleRobotObservedPet(msg);
}

void UiGameController::HandleLoadedKnownFaceBase(
    const Vision::LoadedKnownFace& msg) {
...
void UiGameController::HandleSetCliffDetectThresholdsBase(
    const SetCliffDetectThresholds& msg) {
  HandleSetCliffDetectThresholds(msg);
}

void UiGameController::HandleEngineErrorCodeBase(
    const ExternalInterface::EngineErrorCodeMessage& msg) {
  HandleEngineErrorCode(msg);
}

void UiGameController::HandleRobotDeletedLocatedObjectBase(
    const ExternalInterface::RobotDeletedLocatedObject& msg) {
  PRINT_NAMED_INFO("UiGameController.HandleRobotDeletedObjectBase",
                   "Robot reported deleting object %d", msg.objectID);

  _observedObjects.erase(
      std::remove_if(_observedObjects.begin(), _observedObjects.end(),
...
void UiGameController::HandleUiDeviceAvailableBase(
    const ExternalInterface::UiDeviceAvailable& msgIn) {
  ...
  PRINT_NAMED_INFO("UiGameController.HandleUiDeviceAvailableBase",
                   "Sending message to command connection to %s device %d.",
                   EnumToString(msgIn.connectionType), msgIn.deviceID);
  ExternalInterface::ConnectToUiDevice msgOut(msgIn.connectionType,
                                              msgIn.deviceID);
...
void UiGameController::HandleUiDeviceConnectedBase(
    const ExternalInterface::UiDeviceConnected& msg) {
  if (msg.connectionType == UiConnectionType::UI) {
    ...
    if (nullptr != redirectVizField) {
      if (redirectVizField->getSFBool()) {
        ...
        PRINT_NAMED_INFO("UiGameController.Init.RedirectingViz", "%u.%u.%u.%u",
...
void UiGameController::HandleRobotConnectedBase(
  ...
  if (msg.result == RobotConnectionResult::Success) {
    ...
    PRINT_NAMED_WARNING("UiGameController.HandleRobotConnectedBase.ConnectFail",
                        "* * * * * * %s * * * * * *", EnumToString(msg.result));
  }
}

void UiGameController::HandleRobotCompletedActionBase(
    const ExternalInterface::RobotCompletedAction& msg) {
  switch ((RobotActionType)msg.actionType) {
    case RobotActionType::PICKUP_OBJECT_HIGH:
    case RobotActionType::PICKUP_OBJECT_LOW: {
      const ObjectInteractionCompleted info =
          msg.completionInfo.Get_objectInteractionCompleted();
      printf("Robot %s picking up object with ID: %d ",
             ActionResultToString(msg.result), info.objectID);
      printf("[Tag=%d]\n", msg.idTag);
    } break;
    ...
    case RobotActionType::PLAY_ANIMATION: {
      const AnimationCompleted info =
          msg.completionInfo.Get_animationCompleted();
      PRINT_NAMED_INFO(
          "UiGameController.HandleRobotCompletedActionBase",
          "Robot finished playing animation %s with result %s. [Tag=%d]",
          info.animationName.c_str(), ActionResultToString(msg.result),
          msg.idTag);
    } break;

    default: {
      PRINT_NAMED_INFO("UiGameController.HandleRobotCompletedActionBase",
                       "Robot completed %s action with result %s [Tag=%d].",
                       EnumToString(msg.actionType),
                       ActionResultToString(msg.result), msg.idTag);
    }
  }
...
void UiGameController::HandleActiveObjectAccelBase(
    const ExternalInterface::ObjectAccel& msg) {
  // PRINT_NAMED_INFO("HandleActiveObjectAccel", "ObjectID %d, timestamp %d,
  // accel {%.2f, %.2f, %.2f}",
  //                 msg.objectID, msg.timestamp, msg.accel.x, msg.accel.y,
  //                 msg.accel.z);
  HandleActiveObjectAccel(msg);
}

void UiGameController::HandleActiveObjectAvailableBase(
    const ExternalInterface::ObjectAvailable& msg) {
  HandleActiveObjectAvailable(msg);
}

void UiGameController::HandleActiveObjectConnectionStateBase(
    const ExternalInterface::ObjectConnectionState& msg) {
  PRINT_NAMED_INFO("HandleActiveObjectConnectionState",
                   "ObjectID %d (factoryID %s): %s", msg.objectID,
                   msg.factoryID.c_str(),
                   msg.connected ? "CONNECTED" : "DISCONNECTED");
  HandleActiveObjectConnectionState(msg);
}

void UiGameController::HandleActiveObjectMovedBase(
    const ExternalInterface::ObjectMoved& msg) {
  PRINT_NAMED_INFO("HandleActiveObjectMovedWrapper",
                   "Received message that object %d moved", msg.objectID);

  HandleActiveObjectMoved(msg);
}

void UiGameController::HandleActiveObjectStoppedMovingBase(
    const ExternalInterface::ObjectStoppedMoving& msg) {
  PRINT_NAMED_INFO("HandleActiveObjectStoppedMoving",
                   "Received message that object %d stopped moving",
                   msg.objectID);

  HandleActiveObjectStoppedMoving(msg);
}

void UiGameController::HandleActiveObjectTappedBase(
    const ExternalInterface::ObjectTapped& msg) {
  PRINT_NAMED_INFO("HandleActiveObjectTapped",
                   "Received message that object %d was tapped.", msg.objectID);

  HandleActiveObjectTapped(msg);
}

void UiGameController::HandleActiveObjectUpAxisChangedBase(
    const ExternalInterface::ObjectUpAxisChanged& msg) {
  PRINT_NAMED_INFO(
      "HandleActiveObjectUpAxisChanged",
      "Received message that object %d's UpAxis has changed (new UpAxis = %s).",
      msg.objectID, UpAxisToString(msg.upAxis));

  HandleActiveObjectUpAxisChanged(msg);
}

void UiGameController::HandleConnectedObjectStatesBase(
    const ExternalInterface::ConnectedObjectStates& msg) {
  for (auto& objectState : msg.objects) {
    ...
    // TODO How do we visualize connected only objects since they don't have a
    // pose?
  }
...
void UiGameController::HandleLocatedObjectStatesBase(
    const ExternalInterface::LocatedObjectStates& msg) {
  PRINT_NAMED_INFO("HandleObjectStates",
                   "Clearing all objects before updating with %zu new objects",
                   msg.objects.size());
  ...
  for (const auto& objectState : msg.objects) {
    PRINT_NAMED_INFO(
        "HandleLocatedObjectStates",
        "Received message about known object %d (type: %s, poseState: %hhu)",
        objectState.objectID, EnumToString(objectState.objectType),
        objectState.poseState);

    // observed timestamp of 0 indicates that we are not actually observing it
    // here
    const uint32_t observedTimestamp = 0;

    AddOrUpdateObject(objectState.objectID, objectState.objectType,
                      objectState.pose, observedTimestamp,
...
void UiGameController::HandleAnimationAvailableBase(
    const ExternalInterface::AnimationAvailable& msg) {
  PRINT_CH_INFO(
      "Animations",
      "UiGameController.HandleAnimationAvailableBase.HandleAnimationAvailable",
      "Animation available: %s", msg.animName.c_str());

  HandleAnimationAvailable(msg);
}

void UiGameController::HandleAnimationAbortedBase(
    const ExternalInterface::AnimationAborted& msg) {
  PRINT_NAMED_INFO("HandleAnimationAborted", "Tag: %u", msg.tag);

  HandleAnimationAborted(msg);
}

void UiGameController::HandleFactoryTestResultEntryBase(
    const FactoryTestResultEntry& msg) {
  PRINT_NAMED_INFO("HandleFactoryTestResultEntry", "Test result: %s",
                   EnumToString(msg.result));

  HandleFactoryTestResultEntry(msg);
}

void UiGameController::HandleEndOfMessageBase(
    const ExternalInterface::EndOfMessage& msg) {
  PRINT_NAMED_INFO("HandleEndOfMessage", "messageType: %s",
                   EnumToString(msg.messageType));

  HandleEndOfMessage(msg);
}

void UiGameController::HandleBehaviorTransitionBase(
    const ExternalInterface::BehaviorTransition& msg) {
  /**PRINT_NAMED_INFO("HandleBehaviorTransition", "Received message that
     behavior changed from %s to %s", msg.oldBehaviorID, msg.newBehaviorID);**/

  HandleBehaviorTransition(msg);
}

void UiGameController::HandleRobotOffTreadsStateChangedBase(
    const ExternalInterface::RobotOffTreadsStateChanged& msg) {
  PRINT_NAMED_INFO("HandleRobotOfftreadsStateChanged",
                   "Received RobotPickedUp message.");
  HandleRobotOffTreadsStateChanged(msg);
  UpdateVizOriginToRobot();
}

void UiGameController::HandleDefinedCustomObjectBase(
    const ExternalInterface::DefinedCustomObject& msg) {
  HandleDefinedCustomObject(msg);
}

void UiGameController::HandleRobotDeletedAllCustomObjectsBase(
    const ExternalInterface::RobotDeletedAllCustomObjects& msg) {
  HandleRobotDeletedAllCustomObjects(msg);
}

void UiGameController::HandleRobotDeletedCustomMarkerObjectsBase(
    const ExternalInterface::RobotDeletedCustomMarkerObjects& msg) {
  HandleRobotDeletedCustomMarkerObjects(msg);
}

void UiGameController::HandleRobotDeletedFixedCustomObjectsBase(
    const ExternalInterface::RobotDeletedFixedCustomObjects& msg) {
...
UiGameController::UiGameController(s32 step_time_ms)
    : _webotsOrigin("WebotsOrigin"), _firstRobotPoseUpdate(true) {
...
void UiGameController::Init() {
  ...
  _physicsControllerClient.Connect(
      "127.0.0.1", (uint16_t)VizConstants::WEBOTS_PHYSICS_CONTROLLER_PORT);
  ...
  if (!_gameComms) {
    ...
    _gameComms =
        new GameComms(deviceID, UI_MESSAGE_SERVER_LISTEN_PORT, engineIP.c_str(),
  ...
  _msgHandler.RegisterCallbackForMessage([this](
    ...
    switch (message.GetTag()) {
      ...
      case ExternalInterface::MessageEngineToGame::Tag::ImageChunk:
        ...
        break;
      case ExternalInterface::MessageEngineToGame::Tag::
          RobotDeletedLocatedObject:
        HandleRobotDeletedLocatedObjectBase(
            message.Get_RobotDeletedLocatedObject());
        break;
      ...
      case ExternalInterface::MessageEngineToGame::Tag::ObjectConnectionState:
        HandleActiveObjectConnectionStateBase(
            message.Get_ObjectConnectionState());
        break;
      ...
      case ExternalInterface::MessageEngineToGameTag::BehaviorTransition:
        ...
        break;
      case ExternalInterface::MessageEngineToGameTag::
          RobotOffTreadsStateChanged:
        HandleRobotOffTreadsStateChangedBase(
            message.Get_RobotOffTreadsStateChanged());
        break;
      ...
      case ExternalInterface::MessageEngineToGameTag::SetCliffDetectThresholds:
        HandleSetCliffDetectThresholdsBase(
            message.Get_SetCliffDetectThresholds());
        break;
      ...
      case ExternalInterface::MessageEngineToGameTag::
        ...
        HandleRobotDeletedAllCustomObjectsBase(
            message.Get_RobotDeletedAllCustomObjects());
        break;
      case ExternalInterface::MessageEngineToGameTag::
          RobotDeletedCustomMarkerObjects:
        HandleRobotDeletedCustomMarkerObjectsBase(
            message.Get_RobotDeletedCustomMarkerObjects());
        break;
      case ExternalInterface::MessageEngineToGameTag::
          RobotDeletedFixedCustomObjects:
        HandleRobotDeletedFixedCustomObjectsBase(
            message.Get_RobotDeletedFixedCustomObjects());
        break;
      default:
        // ignore
        break;
  ...
  switch (_uiState) {
    case UI_WAITING_FOR_GAME: {
      if (!_gameComms->HasClient()) {
        ...
        // Call step() here with a large-ish time to give engine time to
        // initialize before calling OnEngineLoaded()
...
void UiGameController::UpdateActualObjectPoses() {
  ...
  if (_robotNode == nullptr) {
    auto cozmoBotNodeInfo = WebotsHelpers::GetFirstMatchingSceneTreeNode(
        GetSupervisor(), "CozmoBot");
    if (cozmoBotNodeInfo.nodePtr == nullptr) {
      // If there's no Vector, look for a Whiskey
      cozmoBotNodeInfo = WebotsHelpers::GetFirstMatchingSceneTreeNode(
          GetSupervisor(), "WhiskeyBot");
    }

    DEV_ASSERT(cozmoBotNodeInfo.nodePtr != nullptr,
               "UiGameController.UpdateActualObjectPoses.NoCozmoBot");
    DEV_ASSERT(
        cozmoBotNodeInfo.type == webots::Node::ROBOT,
        "UiGameController.UpdateActualObjectPoses.CozmoBotNotSupervisor");

    PRINT_NAMED_INFO("UiGameController.UpdateActualObjectPoses",
                     "Found robot with name %s",
                     cozmoBotNodeInfo.typeName.c_str());
    _robotNode = cozmoBotNodeInfo.nodePtr;

    // Find any LightCube nodes in the world
    const auto& lightCubes =
        WebotsHelpers::GetMatchingSceneTreeNodes(GetSupervisor(), "LightCube");
    ...
    for (const auto& lightCubeNodeInfo : lightCubes) {
      ...
      PRINT_NAMED_INFO("UiGameController.UpdateActualObjectPoses",
                       "Found LightCube with name %s",
                       lightCubeNodeInfo.typeName.c_str());
    }
  }
  ...
  // if it's the first time that we set the proper pose for the robot, update
  // the visualization origin to the robot, since debug render expects to be
  ...
  if (_firstRobotPoseUpdate) {
    PRINT_NAMED_INFO("UiGameController.UpdateVizOrigin",
                     ...
                     _robotPoseActual.GetTranslation().x(),
                     _robotPoseActual.GetTranslation().y(),
                     _robotPoseActual.GetTranslation().z());
...
void UiGameController::CycleVizOrigin() {
  ...
  if (_robotStateMsg.localizedToObjectID >= 0 && !_lightCubes.empty()) {
    ...
    if (_lightCubeOriginIter != _lightCubes.end()) {
      // If we haven't iterated through all the observed light cubes yet,
      // localize to the newly iterated light cube.
      LOG_INFO("UiGameController.UpdateVizOrigin",
               "Aligning viz to match next known LightCube to object %d",
               _robotStateMsg.localizedToObjectID);

      correctionPose =
          GetPose3dOfNode(*_lightCubeOriginIter) *
          GetObjectPoseMap()[_robotStateMsg.localizedToObjectID].GetInverse();
      UpdateVizOrigin(correctionPose);
    } else {
      // We have cycled through all the available light cubes, so localize to
      // robot now.
...
void UiGameController::SetDataPlatform(
...
Result UiGameController::SendMessage(
...
void UiGameController::SendDriveWheels(const f32 lwheel_speed_mmps,
...
void UiGameController::SendDriveArc(const f32 speed, const f32 accel,
                                    const s16 curvature_mm) {
...
void UiGameController::SendDriveStraight(f32 speed_mmps, f32 dist_mm,
...
uint32_t UiGameController::SendTurnInPlace(
    const f32 angle_rad, const f32 speed_radPerSec, const f32 accel_radPerSec2,
    const f32 tol_rad, const bool isAbsolute,
    const QueueActionPosition queueActionPosition) {
  ExternalInterface::QueueSingleAction m;
  m.idTag = ++_queueActionIdTag;
  m.position = queueActionPosition;
  m.numRetries = 1;
  m.action.Set_turnInPlace(ExternalInterface::TurnInPlace(
      angle_rad, speed_radPerSec, accel_radPerSec2, tol_rad, isAbsolute));
  ExternalInterface::MessageGameToEngine message;
...
void UiGameController::SendAction(
    const ExternalInterface::QueueSingleAction& msg_in) {
...
void UiGameController::SendTurnInPlaceAtSpeed(const f32 speed_rad_per_sec,
...
void UiGameController::SendMoveHeadToAngle(const f32 rad, const f32 speed,
                                           const f32 accel,
...
void UiGameController::SendMoveLiftToHeight(const f32 mm, const f32 speed,
                                            const f32 accel,
                                            const f32 duration_sec) {
...
void UiGameController::SendMoveLiftToAngle(const f32 angle_rad, const f32 speed,
                                           const f32 accel,
                                           const f32 duration_sec) {
  ExternalInterface::SetLiftAngle m;
  m.angle_rad = angle_rad, m.max_speed_rad_per_sec = speed;
  m.accel_rad_per_sec2 = accel;
  m.duration_sec = duration_sec;
  ExternalInterface::MessageGameToEngine message;
...
void UiGameController::SendSaveImages(ImageSendMode imageMode,
                                      const std::string& path,
                                      const int8_t qualityOnRobot,
                                      const bool removeRadialDistortion) {
  using namespace ExternalInterface;
  SendMessage(MessageGameToEngine(
      SaveImages(imageMode, qualityOnRobot, removeRadialDistortion, path)));
}
...
void UiGameController::SendPlaceObjectOnGroundSequence(
    const Pose3d& p, PathMotionProfile motionProf,
...
void UiGameController::SendPickupObject(const s32 objectID,
  ...
  m.objectID = objectID, m.motionProf = motionProf;
...
void UiGameController::SendPickupSelectedObject(PathMotionProfile motionProf,
                                                ...
                                                const bool useApproachAngle,
                                                const f32 approachAngle_rad) {
  SendPickupObject(-1, motionProf, usePreDockPose, useApproachAngle,
                   approachAngle_rad);
}
...
void UiGameController::SendPlaceOnSelectedObject(PathMotionProfile motionProf,
                                                 ...
                                                 const bool useApproachAngle,
                                                 const f32 approachAngle_rad) {
  SendPlaceOnObject(-1, motionProf, usePreDockPose, useApproachAngle,
                    approachAngle_rad);
}
...
void UiGameController::SendPlaceRelSelectedObject(PathMotionProfile motionProf,
                                                  ...
                                                  const bool useApproachAngle,
                                                  const f32 approachAngle_rad) {
  SendPlaceRelObject(-1, motionProf, usePreDockPose, placementOffsetX_mm,
                     useApproachAngle, approachAngle_rad);
}
...
void UiGameController::SendRollObject(const s32 objectID,
  ...
  m.usePreDockPose = usePreDockPose;
  m.useApproachAngle = useApproachAngle,
  m.approachAngle_rad = approachAngle_rad, m.objectID = -1;
  ExternalInterface::MessageGameToEngine message;
...
void UiGameController::SendRollSelectedObject(PathMotionProfile motionProf,
                                              ...
                                              const bool useApproachAngle,
                                              const f32 approachAngle_rad) {
  SendRollObject(-1, motionProf, doDeepRoll, usePreDockPose, useApproachAngle,
                 approachAngle_rad);
}
...
void UiGameController::SendMountSelectedCharger(
    PathMotionProfile motionProf, const bool useCliffSensorCorrection) {
  SendMountCharger(-1, motionProf, useCliffSensorCorrection);
}

BehaviorClass UiGameController::GetBehaviorClass(
    const std::string& behaviorClass) const {
...
void UiGameController::SendControllerGains(ControllerChannel channel, f32 kp,
                                           f32 ki, f32 kd, f32 maxErrorSum) {
...
void UiGameController::SendRollActionParams(f32 liftHeight_mm,
                                            f32 driveSpeed_mmps,
                                            f32 driveAccel_mmps2,
                                            u32 driveDuration_ms,
                                            f32 backupDist_mm) {
...
void UiGameController::SendStartTestMode(TestMode mode, s32 p1, s32 p2,
                                         s32 p3) {
...
void UiGameController::SendCubeAnimation(
    const u32 objectID, const CubeAnimationTrigger cubeAnimTrigger) {
...
void UiGameController::SendStopCubeAnimation(
    const u32 objectID, const CubeAnimationTrigger cubeAnimTrigger) {
...
void UiGameController::SendAnimation(const char* animName, u32 numLoops,
  ...
  if (!throttleMessages || _supervisor.getTime() > lastSendTime_sec + 0.5f) {
    ...
    PRINT_NAMED_INFO("SendAnimation",
                     "Ignoring duplicate SendAnimation keystroke.");
  }
}

void UiGameController::SendAnimationGroup(const char* animGroupName,
                                          u32 numLoops, bool throttleMessages) {
  static double lastSendTime_sec = -1e6;
  // Don't send repeated animation commands within a half second
  if (!throttleMessages || _supervisor.getTime() > lastSendTime_sec + 0.5f) {
    ...
    PRINT_NAMED_INFO("SendAnimationGroup",
                     "Ignoring duplicate SendAnimation keystroke.");
  }
}

void UiGameController::SendAnimationTrigger(const char* animTriggerName,
                                            u32 numLoops,
                                            bool throttleMessages) {
  static double lastSendTime_sec = -1e6;
  // Don't send repeated animation commands within a half second
  if (!throttleMessages || _supervisor.getTime() > lastSendTime_sec + 0.5f) {
    ...
    ExternalInterface::PlayAnimationTrigger m(
        numLoops, AnimationTriggerFromString(animTriggerName), false, false,
        false, false);
    ExternalInterface::MessageGameToEngine message;
    ...
    PRINT_NAMED_INFO("SendAnimationTrigger",
                     "Ignoring duplicate SendAnimation keystroke.");
  }
}
...
uint32_t UiGameController::SendQueuePlayAnimAction(const std::string& animName,
                                                   u32 numLoops,
                                                   QueueActionPosition pos) {
  ExternalInterface::QueueSingleAction msg;
  msg.idTag = ++_queueActionIdTag;
  msg.position = pos;
  msg.action.Set_playAnimation(ExternalInterface::PlayAnimation(
      numLoops, animName, false, false, false));
...
void UiGameController::SendCameraCalibration(f32 focalLength_x,
                                             f32 focalLength_y, f32 center_x,
                                             f32 center_y) {
...
void UiGameController::SendSetPreferredCube(
    const std::string& preferredCubeFactoryId) {
  using namespace ExternalInterface;
...
void UiGameController::SendSetActiveObjectLEDs(
    const u32 objectID, const u32 onColor, const u32 offColor,
    const u32 onPeriod_ms, const u32 offPeriod_ms,
    const u32 transitionOnPeriod_ms, const u32 transitionOffPeriod_ms,
    const s32 offset, const bool rotate, const f32 relativeToX,
    const f32 relativeToY, const WhichCubeLEDs whichLEDs,
    const MakeRelativeMode makeRelative, const bool turnOffUnspecifiedLEDs) {
  ExternalInterface::SetActiveObjectLEDs m(
      objectID, onColor, offColor, onPeriod_ms, offPeriod_ms,
      transitionOnPeriod_ms, transitionOffPeriod_ms, offset, relativeToX,
      relativeToY, rotate, whichLEDs, makeRelative, turnOffUnspecifiedLEDs);

  SendMessage(ExternalInterface::MessageGameToEngine(std::move(m)));
}

void UiGameController::SendSetAllActiveObjectLEDs(
    const u32 objectID, const std::array<u32, 4> onColor,
    const std::array<u32, 4> offColor, const std::array<u32, 4> onPeriod_ms,
    ...
    const std::array<s32, 4> offset, const bool rotate, const f32 relativeToX,
    const f32 relativeToY, const MakeRelativeMode makeRelative) {
  ExternalInterface::SetAllActiveObjectLEDs m(
      objectID, onColor, offColor, onPeriod_ms, offPeriod_ms,
      transitionOnPeriod_ms, transitionOffPeriod_ms, offset, relativeToX,
      relativeToY, rotate, makeRelative);

  SendMessage(ExternalInterface::MessageGameToEngine(std::move(m)));
}

void UiGameController::SendPushDrivingAnimations(
    const std::string& lockName, const AnimationTrigger& startAnim,
    const AnimationTrigger& loopAnim, const AnimationTrigger& endAnim) {
...
void UiGameController::SendRemoveDrivingAnimations(
...
s32 UiGameController::GetStepTimeMS() const { return _stepTimeMS; }

webots::Supervisor& UiGameController::GetSupervisor() { return _supervisor; }

const Pose3d& UiGameController::GetRobotPose() const { return _robotPose; }
...
std::vector<s32> UiGameController::GetAllObjectIDsByType(
...
Result UiGameController::GetObjectType(s32 objectID, ObjectType& type) const {
  auto it = std::find_if(
      _observedObjects.begin(), _observedObjects.end(),
      [&objectID](const ObservedObject& obj) { return (obj.id == objectID); });
  if (it != _observedObjects.end()) {
    type = it->type;
    return RESULT_OK;
  }
...
void UiGameController::ClearAllKnownObjects() { _observedObjects.clear(); }
...
UiGameController::ObservedObject UiGameController::GetLastObservedObject()
    const {
...
void UiGameController::PressBackpackButton(bool pressed) {
  if (_backpackButtonPressedField == nullptr) {
    if (_robotNode == nullptr) {
      ...
      _backpackButtonPressedField =
          _robotNode->getField("backpackButtonPressed");
    }
  }
...
void UiGameController::TouchBackpackTouchSensor(bool touched) {
  if (_touchSensorTouchedField == nullptr) {
    if (_robotNode == nullptr) {
      PRINT_NAMED_ERROR(
          "UiGameController.TouchBackpackTouchSensor.NullRobotNode", "");
      return;
    } else {
...
void UiGameController::StartFreeplayMode() {
  using namespace ExternalInterface;
  SendMessage(MessageGameToEngine(
      SetDebugConsoleVarMessage("DevDispatchAfterShake", "1")));
}
...
void UiGameController::SetNodePose(webots::Node* node, const Pose3d& newPose) {
  if (node != nullptr) {
    ...
    const double rotation[4] = {Rvec.GetAxis().x(), Rvec.GetAxis().y(),
                                Rvec.GetAxis().z(), Rvec.GetAngle().ToFloat()};
    rotField->setSFRotation(rotation);

    const double translation[3] = {MM_TO_M(newPose.GetTranslation().x()),
...
void UiGameController::SetLightCubePose(ObjectType lightCubeType,
...
const Pose3d UiGameController::GetLightCubePoseActual(
    ObjectType lightCubeType) {
  webots::Node* lightCube = GetLightCubeByType(lightCubeType);
...
const std::string UiGameController::GetAnimationTestName() const {
  ...
  WebotsHelpers::GetFieldAsString(*_robotNode, "animationTestName",
                                  animTestName);
  return animTestName;
}
...
const Pose3d UiGameController::GetPose3dOfNode(webots::Node* node) const {
  ...
  pose.SetTranslation({static_cast<f32>(M_TO_MM(transActual[0])),
                       static_cast<f32>(M_TO_MM(transActual[1])),
                       static_cast<f32>(M_TO_MM(transActual[2]))});

  pose.SetRotation({static_cast<f32>(orientationActual[0]),
...
webots::Node* UiGameController::GetLightCubeByType(ObjectType inType) const {
  ...
  DEV_ASSERT_MSG(false, "UiGameController.GetLightCubeByType",
                 "Can't find the light cube with type '%s' in the world",
                 ObjectTypeToString(inType));
  return nullptr;
}
...
bool UiGameController::RemoveLightCubeByType(ObjectType inType) {
  ...
  DEV_ASSERT_MSG(false, "UiGameController.RemoveLightCubeById",
                 "Can't find the light cube of ObjectType '%s' in the world",
                 ObjectTypeToString(inType));
  return false;
}

bool UiGameController::AddLightCubeByType(ObjectType inType, const Pose3d& p,
  ...
  for (auto lightCube : _lightCubes) {
    ...
    if (type && (ObjectTypeFromString(type->getSFString()) == inType)) {
      PRINT_NAMED_WARNING(
          "UiGameController.AddLightCubeByType.ObjectTypeAlreadyExists", "%s",
          ObjectTypeToString(inType));
      return false;
    }
  }
  ...
  std::stringstream ss;
  ss << "LightCube { "
     << " objectType " << ObjectTypeToString(inType) << " factoryID "
     << factoryID << " translation " << 0.001f * p.GetTranslation().x() << " "
     << 0.001f * p.GetTranslation().y() << " "
     << 0.001f * p.GetTranslation().z() << " "
     << " rotation " << p.GetRotationAxis().x() << " "
     << p.GetRotationAxis().y() << " " << p.GetRotationAxis().z() << " "
...
void SetChargerPluggedIn(webots::Node* chargerNode, const bool pluggedIn) {
  DEV_ASSERT(chargerNode != nullptr,
             "UiGameController.SetChargerPluggedIn.NullNode");
  auto* isPluggedInField = chargerNode->getField("isPluggedIn");
  DEV_ASSERT(isPluggedInField != nullptr,
             "UiGameController.SetChargerPluggedIn.NoIsPluggedInField");
...
webots::Node* UiGameController::GetNodeByDefName(
    const std::string& defName) const {
  return _supervisor.getFromDef(defName);
}

void UiGameController::SendApplyForce(const std::string& defName, int xForce,
                                      int yForce, int zForce) {
