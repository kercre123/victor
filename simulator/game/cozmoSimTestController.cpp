...
// Also may want/need to change starting webots world viewpoint so that you can
// see everything from a good pose
#define RECORD_TEST 0
const static std::string kBuildDirectory = "../../../build/";
// This folder is created with a build script on teamCity - make the folder
// locally manually for now
const static std::string kScreenShotsPath =
    kBuildDirectory + "mac/Debug/webots_screenshots/";
...
bool CozmoSimTestController::IsTrueBeforeTimeout(
    bool cond, const char* condAsString, double start_time, double timeout,
    const char* file, const char* func, int line) {
  if (GetSupervisor().getTime() - start_time > timeout) {
    if (!cond) {
      PRINT_STREAM_WARNING("CONDITION_WITH_TIMEOUT_ASSERT",
                           "(" << condAsString << ") still false after "
...
bool CozmoSimTestController::AllTrueBeforeTimeout(
    const std::vector<bool>& conditionBools,
    const std::vector<std::string>& conditionStrings, double start_time,
    double timeout, const char* file, const char* func, int line) {
  const bool allTrue =
      !std::any_of(conditionBools.begin(), conditionBools.end(),
  ...
  if (GetSupervisor().getTime() - start_time > timeout) {
    DEV_ASSERT(conditionStrings.size() == conditionBools.size(),
               "CozmoSimTestController.AllTrueBeforeTimeout."
               "NumberOfConditionsMismatch");

    std::ostringstream msg;
    msg << "Conditions: \n\n";
    for (int i = 0; i < conditionBools.size(); i++) {
      msg << (conditionBools[i] ? "<TRUE>   " : "<FALSE>  ")
          << conditionStrings[i] << "\n";
    }

    msg << "\nAbove conditions were still false after " << timeout
        << " seconds (started at " << start_time << ")\n";
    msg << "File: \"" << file << "\", line " << line << ", in function \""
        << func << "()\"";

    PRINT_STREAM_WARNING("ALL_CONDITIONS_WITH_TIMEOUT_ASSERT",
                         msg.str().c_str());
    _result = 255;
...
void CozmoSimTestController::HandleRobotConnected(
...
s32 CozmoSimTestController::UpdateInternal() {
  ...
  if (_screenshotInterval > 0) {
    ...
    if ((simTime - _timeOfLastScreenshot) > _screenshotInterval) {
      ...
      std::stringstream ss;
      ss << kScreenShotsPath << _screenshotID << "_" << timeString << "_"
         << _screenshotNum << ".png";
      GetSupervisor().exportImage(ss.str(), 80);

      PRINT_NAMED_INFO("CozmoSimTestController.UpdateInternal.TookScreenshot",
                       "ID:%s Num:%d Time:%s", _screenshotID.c_str(),
                       _screenshotNum, timeString);
...
void CozmoSimTestController::ExitTest() {
  if (_quitWebotsAfterTest) {
    ...
    QuitController(
        _result);  // Just quit the controller, but keep Webots running
  }
}

// Only runs if #define RECORD_TEST 1, use for local testing
void CozmoSimTestController::StartMovieConditional(const std::string& name,
                                                   int speed) {
  if (RECORD_TEST) {
    time_t t;
    time(&t);
...
void CozmoSimTestController::StartMovieAlways(const std::string& name,
                                              int speed) {
  time_t t;
  time(&t);
  std::stringstream ss;
  ...
  _isRecording =
      GetSupervisor().getMovieStatus() == webots::Supervisor::MOVIE_RECORDING;
  PRINT_NAMED_INFO("Is Movie Recording?", "_isRecording:%d", _isRecording);
}

void CozmoSimTestController::StopMovie() {
  if (_isRecording &&
      GetSupervisor().getMovieStatus() == GetSupervisor().MOVIE_RECORDING) {
    GetSupervisor().movieStopRecording();
    PRINT_NAMED_INFO("CozmoSimTestController.StopMovie",
                     "Movie Stop Command issued");

    while (GetSupervisor().movieIsReady()) {
    }

    if (GetSupervisor().movieFailed()) {
      PRINT_NAMED_ERROR("CozmoSimTestController.StopMovie",
                        "Movie failed to save properly");
    }
...
void CozmoSimTestController::TakeScreenshotsAtInterval(
    const std::string& screenshotID, f32 interval) {
  if (interval <= 0) {
    PRINT_NAMED_ERROR(
        "CozmoSimTestController.TakeShotsAtInterval.InvalidInterval",
        "Interval passed in: %f", interval);
    return;
  }
  ...
  PRINT_NAMED_INFO(
      "CozmoSimTestController.TakeScreenshotsAtInterval.SettingInterval",
      "Interval:%f Path:%s", _screenshotInterval, kScreenShotsPath.c_str());
}

Result CozmoSimTestController::SendMessage(
...
void CozmoSimTestController::DisableRandomPathSpeeds() {
  SendMessage(ExternalInterface::MessageGameToEngine(
      ExternalInterface::SetEnableSpeedChooser(false)));
}
...
void CozmoSimTestController::PrintPeriodicBlockDebug() {
  ...
  if (_nextPrintTime < 0 || currTime_s >= _nextPrintTime) {
    ...
    for (const auto& objPair : GetObjectPoseMap()) {
      ...
      PRINT_NAMED_INFO(
          "CozmoSimTest.BlockDebug.Known",
          "object %d: (%f, %f, %f) theta_z=%fdeg, upAxis=%s%s", objId,
          estPose.GetTranslation().x(), estPose.GetTranslation().y(),
          estPose.GetTranslation().z(),
          estPose.GetRotationAngle<'Z'>().getDegrees(),
          AxisToCString(
              estPose.GetRotationMatrix().GetRotatedParentAxis<'Z'>()),
...
bool CozmoSimTestController::IsRobotPoseCorrect(const Point3f& distThreshold,
                                                const Radians& angleThreshold,
                                                const Pose3d& transform) const {
  ...
  const bool isSame =
      robotPose.IsSameAs(robotPoseActual, distThreshold, angleThreshold);
  return isSame;
}

bool CozmoSimTestController::IsObjectPoseWrtRobotCorrect(
    s32 objectID, const Pose3d& actualPose, const Point3f& distThresh_mm,
    const Radians& angleThresh, const char* debugStr) const {
  const PoseOrigin fakeOrigin;  // declare before any poses that use it as
                                // parent so it destructs last!
  ...
  if (RESULT_OK != GetObjectPose(objectID, objectPoseWrtRobot)) {
    PRINT_NAMED_WARNING("CozmoSimTestController.IsObjectPoseWrtRobotCorrect",
                        "%s: Could not get object %d's pose", debugStr,
                        objectID);
    return false;
  }

  if (false ==
      objectPoseWrtRobot.GetWithRespectTo(GetRobotPose(), objectPoseWrtRobot)) {
    PRINT_NAMED_WARNING("CozmoSimTestController.IsObjectPoseWrtRobotCorrect",
  ...
  if (false ==
      actualPose.GetWithRespectTo(robotPoseActual, actualObjectPoseWrtRobot)) {
    PRINT_NAMED_WARNING(
        "CozmoSimTestController.IsObjectPoseWrtRobotCorrect",
  ...
  // a common origin and see if the represent the same pose relative to that
  // common origin
  ...
  Vec3f Tdiff(0, 0, 0);
  Radians angleDiff(0);
  if (!objectPoseWrtRobot.IsSameAs(actualObjectPoseWrtRobot, distThresh_mm,
                                   angleThresh, Tdiff, angleDiff)) {
    PRINT_NAMED_WARNING("CozmoSimTestController.IsObjectPoseWrtRobotCorrect",
                        "%s: object %d's observed and actual poses do not "
                        "match [Tdiff=(%.1f,%.1f,%.1f) angleDiff=%.1fdeg]",
                        debugStr, objectID, Tdiff.x(), Tdiff.y(), Tdiff.z(),
...
std::shared_ptr<CozmoSimTestController> CozmoSimTestFactory::Create(
    std::string name) {
...
void CozmoSimTestFactory::RegisterFactoryFunction(
    std::string name,
