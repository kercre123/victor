...
struct CozmoBotVizParams {
  bool Valid() {
    return (trans != nullptr) && (rot != nullptr) && (liftAngle != nullptr) &&
           (headAngle != nullptr);
  }
...
struct VizObjectInfo {
  ...
  int webotsNodeId = -1;  // Optional webots node identifier for 3D objects that
                          // are dynamically added to the scene tree.
};
...
class VizControllerImpl {
 ...
 private:
  void SetRobotPose(CozmoBotVizParams& vizParams, const Pose3d& pose,
                    const f32 headAngle, const f32 liftAngle);

  void DrawText(webots::Display* disp, u32 lineNum, u32 color,
                const char* text);
  void DrawText(webots::Display* disp, u32 lineNum, const char* text);
  void ProcessVizSetRobotMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizSetLabelMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizDockingErrorSignalMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizVisionMarkerMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizCameraQuadMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizCameraRectMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizCameraLineMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizCameraOvalMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizCameraTextMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizImageChunkMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizTrackerQuadMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizRobotStateMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizCurrentAnimation(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessCameraParams(const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessBehaviorStackDebug(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVisionModeDebug(const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessEnabledVisionModes(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  ...
  void ProcessVizSetOriginMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);

  void ProcessVizMemoryMapMessageBegin(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizMemoryMapMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizMemoryMapMessageEnd(
      const AnkiEvent<VizInterface::MessageViz>& msg);

  void ProcessVizObjectMessage(const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizEraseObjectMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizShowObjectsMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);

  void ProcessVizLineSegmentMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizEraseLineSegmentsMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);

  void ProcessVizQuadMessage(const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizEraseQuadMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);

  void ProcessVizAppendPathSegmentLineMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizAppendPathSegmentArcMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizSetPathColorMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);
  void ProcessVizErasePathMessage(
      const AnkiEvent<VizInterface::MessageViz>& msg);

  void DisplayBufferedCameraImage(const RobotTimeStamp_t timestamp);
  void DisplayCameraInfo(const RobotTimeStamp_t timestamp);

  void EraseVizObjects(
      const uint32_t lowerBoundId = 0,
      const uint32_t upperBoundId = std::numeric_limits<uint32_t>::max());

  void EraseVizSegments(const std::string& identifier);

  void EraseVizQuads(const VizQuadType quadType, const uint32_t quadId);
  ...
  void Subscribe(const VizInterface::MessageVizTag& tagType,
                 std::function<void(const AnkiEvent<VizInterface::MessageViz>&)>
                     messageHandler) {
  ...
  struct DebugImage {
    ...
    DebugImage(webots::Display* display)
        : imageDisplay(display), imageRef(nullptr) {}
  };
  ...
  // Store the nodeID of the camera node inside the simulated robot (if any).
  // This is to be able to make the viz displays and objects invisible to the
  // robot's camera using webots::Node::setVisibility().
  ...
  // Objects to visualize (e.g. cubes, charger, poses, etc.). Map keyed on viz
  // object ID
