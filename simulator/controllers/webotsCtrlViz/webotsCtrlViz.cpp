 ...
 * Date:          03-19-2014
 * Description:   Interface for basestation to all visualization functions in
 * Webots including cozmo_physics draw functions, display window text printing,
 * and other custom display methods. Author:        Kevin Yoon Modifications:
 */
...
int main(int argc, char** argv) {
  ...
  const Anki::Util::Data::DataPlatform& dataPlatform =
      WebotsCtrlShared::CreateDataPlatformBS(argv[0], "webotsCtrlViz");
  // initialize logger
  ...
  // If we are using Webots R2018b or later, then openGL support is removed and
  // we cannot use the PhysVizController to draw 3D objects. Instead the
  // VizController should draw such objects.
  ...
  // Setup client to forward relevant commands to cozmo_physics plugin (but only
  // if we need it for drawing objects)
  UdpClient physicsClient;
  if (!vizShouldDrawObjects) {
    physicsClient.Connect("127.0.0.1",
                          (uint16_t)VizConstants::PHYSICS_PLUGIN_SERVER_PORT);
  }
  ...
  while (vizSupervisor.step(Anki::Vector::BS_TIME_STEP_MS) != -1) {
    ...
    while ((numBytesRecvd = server.Recv((char*)data, maxPacketSize)) > 0) {
      ...
      vizController.ProcessMessage(
          VizInterface::MessageViz(data, numBytesRecvd));
    }  // while server.Recv
