...
#ifdef SIMULATOR
Anki::Vision::CameraCalibration GetCameraCalibration(
    const webots::Camera* camera) {
  ...
  return Anki::Vision::CameraCalibration(nrows, ncols, f, f, center_x, center_y,
                                         skew);
}
#endif
...
int main(int argc, char** argv) {
  ...
  if (argc < (NUM_POSE_VALS + 1)) {
    fprintf(stderr,
            "Not enough controllerArgs to specify a single robot pose.\n");
    return -1;
  } else if (((argc - 1) % NUM_POSE_VALS) != 0) {
    fprintf(stderr,
            "Robot poses should be specified in groups of 8 values "
            "(Xaxis,Yaxis,Zaxis,Angle,Tx,Ty,Tz,HeadAngle).\n");
    return -1;
  }
  ...
  webots::PositionSensor* headPosSensor_ =
      webotRobot_.getPositionSensor("HeadPosSensor");
  webots::PositionSensor* liftPosSensor_ =
      webotRobot_.getPositionSensor("LiftPosSensor");
  ...
  for (int i_node = 0; i_node < numNodes; ++i_node) {
    ...
    if (nameField != NULL &&
      ...
      if (!objectType.empty()) {
        ...
        const double* objectTrans_m =
            child->getField("translation")->getSFVec3f();
        const double* objectRot = child->getField("rotation")->getSFRotation();
        for (int i = 0; i < 3; ++i) {
          jsonObject["ObjectPose"]["Translation"].append(
              M_TO_MM(objectTrans_m[i]));
      ...
      if (checkPoseStr.back() == '0') {
        ...
        CORETECH_THROW(
            "Unexpected character when looking for CheckRobotPose "
  ...
  // PlatformPathManager::getInstance()->PrependPath(PlatformPathManager::Test,
  // "basestation/test/blockWorldTests/") + root["WorldTitle"].asString();
  std::string outputPath("basestation/test/blockWorldTests/");
  outputPath += root["WorldTitle"].asString();

  for (int i_pose = 0; i_pose < numPoses; ++i_pose, rotIndex += NUM_POSE_VALS,
           transIndex += NUM_POSE_VALS, headAngleIndex += NUM_POSE_VALS) {
    // Move to next pose
    const double translation_m[3] = {atof(argv[transIndex]),
                                     atof(argv[transIndex + 1]),
                                     atof(argv[transIndex + 2])};

    const double rotation[4] = {atof(argv[rotIndex]), atof(argv[rotIndex + 1]),
    ...
    fprintf(stdout,
            "Moving robot '%s' to (%.3f,%.3f,%.3f), "
            "%.1fdeg @ (%.3f,%.3f,%.3f), with headAngle=%.1fdeg\n",
            robotName.c_str(), translation_m[0], translation_m[1],
            translation_m[2], RAD_TO_DEG(rotation[3]), rotation[0], rotation[1],
            rotation[2], RAD_TO_DEG(headAngle));
    ...
    do {
      webotRobot_.step(TIME_STEP);
      headErr =
          fabs(headPosSensor_->getValue() - headMotor_->getTargetPosition());
      liftErr =
          fabs(liftPosSensor_->getValue() - liftMotor_->getTargetPosition());
    ...
    headCam_->saveImage(PlatformPathManager::GetInstance()->PrependPath(
                            PlatformPathManager::Test, imgFilename),
                        100);
    ...
    matlab.EvalStringEcho(
        ...
        PlatformPathManager::GetInstance()
            ->PrependPath(PlatformPathManager::Test, imgFilename)
            .c_str(),
        PlatformPathManager::GetInstance()
            ->PrependPath(PlatformPathManager::Test, imgFilename)
            .c_str());
    ...
    for (int i_marker = 0; i_marker < numMarkers; ++i_marker) {
      ...
      using namespace Anki;
      Vision::MarkerType orientedMarkerCode = static_cast<Vision::MarkerType>(
          mxGetScalar(matlab.GetArray("code")) - 1);

      msg.markerType = static_cast<u16>(
          Embedded::VisionMarker::RemoveOrientation(orientedMarkerCode));

      /*
      mxArray* mxByteArray = matlab.GetArray("byteArray");
      CORETECH_ASSERT(mxGetNumberOfElements(mxByteArray) ==
      VISION_MARKER_CODE_LENGTH); const u8* code = reinterpret_cast<const
      u8*>(mxGetData(mxByteArray)); std::copy(code, code +
      VISION_MARKER_CODE_LENGTH, msg.code.begin());
      */
...

    {
      ...
      const Result result = DetectFiducialMarkers(
          image, markers, homographies,
          detectionParams.scaleImage_numPyramidLevels,
          parameters.scaleImage_thresholdMultiplier,
          detectionParams.component1d_minComponentWidth,
          parameters.component1d_maxSkipDistance,
          detectionParams.component_minimumNumPixels,
          parameters.component_maximumNumPixels,
          detectionParams.component_sparseMultiplyThreshold,
          parameters.component_solidMultiplyThreshold,
          detectionParams.component_minHollowRatio,
          detectionParams.quads_minQuadArea,
          parameters.quads_quadSymmetryThreshold,
          parameters.quads_minDistanceFromImageEdge,
          detectionParams.decode_minContrastRatio,
          detectionParams.maxConnectedComponentSegments,
          detectionParams.maxExtractedQuads,
          detectionParams.quadRefinementIterations, false, ccmScratch,
    ...
    for (s32 i_marker = 0; i_marker < numMarkers; ++i_marker) {
      ...
      // in the mailbox to be picked up and sent out by main execution)
      {
        ...
        msg.x_imgLowerLeft =
            crntMarker.corners[Quadrilateral<f32>::BottomLeft].x;
        msg.y_imgLowerLeft =
            crntMarker.corners[Quadrilateral<f32>::BottomLeft].y;
        ...
        msg.x_imgUpperRight =
            crntMarker.corners[Quadrilateral<f32>::TopRight].x;
        msg.y_imgUpperRight =
            crntMarker.corners[Quadrilateral<f32>::TopRight].y;

        msg.x_imgLowerRight =
            crntMarker.corners[Quadrilateral<f32>::BottomRight].x;
        msg.y_imgLowerRight =
            crntMarker.corners[Quadrilateral<f32>::BottomRight].y;
    ...
    for (auto& marker : markers) {
      ...
      // retraining)
      {
        ...
        const Vision::MarkerType markerType =
            static_cast<Vision::MarkerType>(jsonMarker["markerType"].asInt());
        jsonMarker["markerType"] = Vision::MarkerTypeStrings[markerType];
      }

      fprintf(stdout,
              "Creating JSON for marker type %s with corners (%.1f,%.1f), "
              "(%.1f,%.1f), "
              "(%.1f,%.1f), (%.1f,%.1f)\n",
              jsonMarker["markerType"]
                  .asString()
                  .c_str(),  // Vision::MarkerTypeStrings[marker.markerType],
  ...
  std::string jsonFilename = PlatformPathManager::GetInstance()->PrependPath(
                                 PlatformPathManager::Test, outputPath) +
                             ".json";
