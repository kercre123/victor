...
WebotsDevLogController::WebotsDevLogController(int32_t stepTime_ms)
    : _stepTime_ms(stepTime_ms),
      ...
      _savingImages(false) {
  DEV_ASSERT(nullptr != _selfNode,
             "WebotsDevLogController.Constructor.SelfNodeMissing");
  _supervisor->getKeyboard()->enable(stepTime_ms);
  _vizConnection->Connect(
      "127.0.0.1", Util::EnumToUnderlying(VizConstants::VIZ_SERVER_PORT));
...
void WebotsDevLogController::UpdateStatusText(bool jumping) {
  ...
  if (GetDirectoryPath().empty()) {
    ...
    snprintf(text, kMaxStatusStrLen + 1, "%s %6.2fx",
             _isPaused ? "PAUSE" : "PLAY ", _fastForwardFactor);
  }
...
void WebotsDevLogController::UpdateCurrTimeText(uint32_t time_ms) {
  ...
  snprintf(str, kMaxCurrTimeStrLen + 1, "%7ums %2d:%06.3fs", time_ms, mins,
           secs);

  const int width = kMaxCurrTimeStrLen * kFontWidth;
  const int top = _disp->getHeight() - kFontHeight;
  const int left =
      _disp->getWidth() / 2 - (kMaxCurrTimeStrLen / 2) * kFontWidth;
...
void WebotsDevLogController::UpdateCurrTimeRender(uint32_t time_ms,
                                                  uint32_t targetJumpTime_ms) {
  // first update text
  ...
  if (_totalLogLength_ms > 0) {
    ...
    if (targetJumpTime_ms > 0) {
      const int markLeft =
          kPadding + kInnerPadding +
          targetJumpTime_ms * maxInnerWidth / _totalLogLength_ms - 1;
...
int32_t WebotsDevLogController::Update() {
  ...
  if (_devLogProcessor && !_isPaused) {
    if (_devLogProcessor->AdvanceTime(
            std::round(_fastForwardFactor * _stepTime_ms))) {
  ...
  // don't use fast forward factor here. This allows us to advance the log
  // playback faster than the sim
...
void WebotsDevLogController::EnableSaveImagesIfChecked() {
  const webots::Field* saveImagesField =
      _selfNode->getField(kSaveImagesFieldName);
  if (nullptr == saveImagesField) {
    PRINT_NAMED_ERROR(
        "WebotsDevLogController.ToggleImageSaving.MissingSaveImagesField",
...
void WebotsDevLogController::EnableSaveImages(bool enable) {
  ...
  std::string path = Util::FileUtils::FullFilePath(
      {_devLogProcessor->GetDirectoryName(), "savedImages"});
  ...
  if (_vizConnection->Send((const char*)buffer, numPacked) <= 0) {
    PRINT_NAMED_WARNING(
        "VizManager.SendMessage.Fail", "Send vizMsgID %s of size %zd failed",
  ...
  for (auto key : _lastKeysPressed) {
    ...
    switch (key) {
      case (int)'I': {
        ...
        // Make field in object tree match new state (EnableSaveImages changes
        // _savingImages)
        webots::Field* saveImagesField =
            _selfNode->getField(kSaveImagesFieldName);
        if (nullptr == saveImagesField) {
          PRINT_NAMED_ERROR(
              "WebotsDevLogController.ToggleImageSaving.MissingSaveImagesField",
...
void WebotsDevLogController::SetLogCallbacks() {
  _devLogProcessor->SetVizMessageCallback(std::bind(
      &WebotsDevLogController::HandleVizData, this, std::placeholders::_1));
  _devLogProcessor->SetPrintCallback(std::bind(
      &WebotsDevLogController::HandlePrintLines, this, std::placeholders::_1));
}
...
void WebotsDevLogController::JumpToMS(uint32_t targetTime_ms,
                                      bool dropMessages) {
  if (!_devLogProcessor) {
    return;
  }
  ...
  PRINT_NAMED_INFO("WebotsDevLogController.JumpToMS.Complete", "jump complete");
}
...
void WebotsDevLogController::JumpByMS(uint32_t jump_ms, bool dropMessages) {
  ...
  while (currJump < jump_ms) {
    ...
    uint32_t thisJump =
        jumpLeft > kMaxJumpInterval_ms ? kMaxJumpInterval_ms : jumpLeft;
    currJump += thisJump;
...
void WebotsDevLogController::InitDevLogProcessor(
    const std::string& directoryPath) {
  // We only init the dev log processor when we don't have one and we've been
  // given a valid path. It would be nice to handle loading a new log after
  // having run one already, but the VizController is stateful and we don't yet
  // have a way to clear it before going through another log
  if (_devLogProcessor) {
    PRINT_NAMED_INFO("WebotsDevLogController.InitDevLogProcessor",
                     "DevLogProcessor already exists. Ignoring.");
    return;
  }

  if (directoryPath.empty() ||
      !Util::FileUtils::DirectoryExists(directoryPath)) {
    PRINT_NAMED_INFO("WebotsDevLogController.InitDevLogProcessor",
                     "Input directory %s not found.", directoryPath.c_str());
    return;
  }

  PRINT_NAMED_INFO("WebotsDevLogController.InitDevLogProcessor",
                   "Loading directory %s", directoryPath.c_str());
  ...
  PRINT_NAMED_INFO("WebotsDevLogController.InitDevLogProcessor.TotalLength",
                   "max log timestamp is %d", _totalLogLength_ms);
...
void WebotsDevLogController::HandleVizData(
    const DevLogReader::LogData& logData) {
  if (_vizConnection && _vizConnection->IsConnected()) {
    _vizConnection->Send(reinterpret_cast<const char*>(logData._data.data()),
                         logData._data.size());
  }
}

void WebotsDevLogController::HandlePrintLines(
    const DevLogReader::LogData& logData) {
...
int main(int argc, char** argv) {
  // Note: we don't allow logFiltering in DevLog like we do in the other
  // controllers because this controller is meant to show all logs.
  ...
  Anki::Vector::WebotsDevLogController webotsCtrlDevLog(
      Anki::Vector::kDevLogStepTime_ms);

  // If log directory is already specified when we start, just go ahead and use
  // it, without needing to press 'L' key
