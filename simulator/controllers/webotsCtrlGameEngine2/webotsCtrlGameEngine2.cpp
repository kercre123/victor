...
int main(int argc, char** argv) {
  ...
  // create platform.
  // Unfortunately, CozmoAPI does not properly receive a const DataPlatform, and
  // that change is too big of a change, since it involves changing down to the
  // context, so create a non-const platform
  // const Anki::Util::Data::DataPlatform& dataPlatform =
  // WebotsCtrlShared::CreateDataPlatformBS(argv[0]);
  Anki::Util::Data::DataPlatform dataPlatform =
      WebotsCtrlShared::CreateDataPlatformBS(argv[0], "webotsCtrlGameEngine2");

  // Set RobotID
  OSState::getInstance()->SetRobotID(
      engineSupervisor.getSelf()->getField("robotID")->getSFInt32());
...
#if ANKI_DEV_CHEATS
  if (createDevLoggers) {
    DevLoggingSystem::CreateInstance(
        dataPlatform.pathToResource(Util::Data::Scope::CurrentGameLog,
                                    "devLogger"),
        "mac");
  } else {
    LOG_WARNING("webotsCtrlGameEngine.main.SkippingDevLogger",
                "RobotID: %d - Only DEFAULT_ROBOT_ID may create loggers",
                robotID);
  }
#endif
  ...
  Util::PrintfLoggerProvider* printfLoggerProvider =
      new Util::PrintfLoggerProvider(Anki::Util::LOG_LEVEL_WARN,
...
#if ANKI_DEV_CHEATS
  if (createDevLoggers) {
    loggerVec.push_back(new DevLoggerProvider(
        DevLoggingSystem::GetInstance()->GetQueue(),
        Util::FileUtils::FullFilePath(
            {DevLoggingSystem::GetInstance()->GetDevLoggingBaseDirectory(),
             DevLoggingSystem::kPrintName})));
  }
#endif
  ...
  auto* eventProvider =
      new Anki::Util::EventProviderLoggingAdapter(&loggerProvider);
  ...
  if (params.filterLog) {
    ...
    static const std::string& consoleFilterConfigPath =
        "config/engine/console_filter_config.json";
    if (!dataPlatform.readAsJson(Util::Data::Scope::Resources,
                                 consoleFilterConfigPath,
                                 consoleFilterConfig)) {
      LOG_ERROR("webotsCtrlGameEngine.main.loadConsoleConfig",
                "Failed to parse Json file '%s'",
                consoleFilterConfigPath.c_str());
    }

    // initialize console filter for this platform
    const std::string& platformOS = dataPlatform.GetOSPlatformString();
    const Json::Value& consoleFilterConfigOnPlatform =
        consoleFilterConfig[platformOS];
...
#if ANKI_DEV_CHEATS
    {
      // Disable the Clad logger by default - prevents it sending the log
      // messages, otherwise anyone with a config set to spam every message
      // could run into issues with the amount of messages overwhelming the
      // socket on engine startup/load. Anyone who needs the log messages can
      // enable it afterwards via Unity, SDK or Webots
      Anki::Vector::kEnableCladLogger = false;
    }
#endif  // ANKI_DEV_CHEATS
  } else {
    LOG_INFO("webotsCtrlGameEngine.main.noFilter",
             "Console will not be filtered due to program args");
  }
  ...
  if (!dataPlatform.readAsJson(Util::Data::Scope::Resources,
    ...
    LOG_ERROR("webotsCtrlGameEngine.main.loadConfig",
              "Failed to parse Json file config/engine/configuration.json");
  }
  ...
  webots::Field* numUIsField =
      engineSupervisor.getSelf()->getField("numUIDevicesToWaitFor");
  if (numUIsField) {
    numUIDevicesToWaitFor = numUIsField->getSFInt32();
  } else {
    LOG_WARNING("webotsCtrlGameEngine.main.MissingField",
                "numUIDevicesToWaitFor not found in BlockworldComms");
  }
  ...
  if (!initSuccess) {
    LOG_ERROR("webotsCtrlGameEngine.main",
              "Failed in creation/initialization of CozmoAPI");
  } else {
    ...
    while (engineSupervisor.step(BS_TIME_STEP_MS) != -1) {
      ...
      const double currTimeNanoseconds =
          Util::SecToNanoSec(engineSupervisor.getTime());
      const bool tickSuccess = myVictor.Update(
          Util::numeric_cast<BaseStationTime_t>(currTimeNanoseconds));
      ...
      static const float kTargetDuration_ms =
          Util::numeric_cast<float>(BS_TIME_STEP_MS);
      ...
      myVictor.RegisterEngineTickPerformance(time_ms, engineFreq_ms,
                                             sleepTime_ms, sleepTimeActual_ms);
