...
int main(int argc, char** argv) {
  ...
  Util::Data::DataPlatform dataPlatform =
      WebotsCtrlShared::CreateDataPlatformBS(argv[0], "webotsCtrlAnim");

  // - create and set logger
  Util::IFormattedLoggerProvider* printfLoggerProvider =
      new Util::PrintfLoggerProvider(Anki::Util::LOG_LEVEL_WARN,
                                     params.colorizeStderrOutput);
  Util::MultiFormattedLoggerProvider loggerProvider({printfLoggerProvider});
  ...
  if (params.filterLog) {
    ...
    if (!dataPlatform.readAsJson(Util::Data::Scope::Resources,
                                 ...
                                 consoleFilterConfig)) {
      LOG_ERROR("webotsCtrlAnim.main.loadConsoleConfig",
    ...
    LOG_INFO("webotsCtrlAnim.main.noFilter",
  ...
  if (result != RESULT_OK) {
    LOG_ERROR("webotsCtrlAnim.main",
              "Failed in creation/initialization of AnimEngine");
  } else {
    LOG_INFO("webotsCtrlAnim.main", "AnimEngine created and initialized.");

    OSState::getInstance()->SetRobotID(
        animSupervisor.getSelf()->getField("robotID")->getSFInt32());
    ...
    while (animSupervisor.step(ANIM_TIME_STEP_MS) != -1) {
      ...
      const double currTimeNanoseconds =
          Util::SecToNanoSec(animSupervisor.getTime());
      animEngine.Update(
      ...
      static const float kTargetDuration_ms =
          Util::numeric_cast<float>(ANIM_TIME_STEP_MS);
      ...
      animEngine.RegisterTickPerformance(time_ms, animFreq_ms, sleepTime_ms,
                                         sleepTimeActual_ms);
    }  // End tick loop
  }
