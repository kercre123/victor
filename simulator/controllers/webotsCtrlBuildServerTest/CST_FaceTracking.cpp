...
class CST_FaceTracking : public CozmoSimTestController {
 ...
 private:
  ...
  void HandleRobotObservedFace(
      ExternalInterface::RobotObservedFace const& msg) override;
};
...
s32 CST_FaceTracking::UpdateSimInternal() {
  auto ZeroVelocityAfterXSeconds = [this](webots::Node* node, double xSeconds,
                                          TestState nextState) {
    IF_CONDITION_WITH_TIMEOUT_ASSERT(HasXSecondsPassedYet(xSeconds),
                                     xSeconds + 1) {
  ...
  switch (_testState) {
    ...
    case TestState::WaitToObserveFace: {
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          5, HasXSecondsPassedYet(1), _faceIsObserved,
          NEAR(GetRobotHeadAngle_rad(), headLookupAngle_rad,
               headAngleTolerance_rad)) {
    ...
    case TestState::StopFace1: {
      ZeroVelocityAfterXSeconds(
          _face, 0.5, TestState::VerifyTranslationThenTranslateFaceIn3d);
      break;
    }

    case TestState::VerifyTranslationThenTranslateFaceIn3d: {
      // Somewhere between (670, 250, 385) and (703, 261, 393) is the appx.
      // position of the face after translating for 0.5 seconds. Average
      // position is ~ (686, 255, 389)

      const Vec3f expectedFaceTranslation(686, 255, 389);
      const f32 margin = 20;
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          2,
          NEAR(_facePose.GetTranslation().x(), expectedFaceTranslation.x(),
               margin),
          NEAR(_facePose.GetTranslation().y(), expectedFaceTranslation.y(),
               margin),
          NEAR(_facePose.GetTranslation().z(), expectedFaceTranslation.z(),
               margin)) {
    ...
    case TestState::StopFace2: {
      // 0.15 seconds is allows the face to move across the camera view but is
      // not so long that the face will go out of view
      ZeroVelocityAfterXSeconds(_face, 0.15, TestState::Exit);
      break;
    }

    case TestState::Exit: {
      // (332, -102, 136) is the appx. position of the face after translating
      // for 0.15 seconds.
...
void CST_FaceTracking::HandleRobotObservedFace(
    ExternalInterface::RobotObservedFace const& msg) {
