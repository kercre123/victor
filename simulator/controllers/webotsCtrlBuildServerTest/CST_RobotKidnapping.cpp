    ...
    case TestState::ReSeeObjectA: {
      IF_CONDITION_WITH_TIMEOUT_ASSERT(
        ...
        const Pose3d poseA_actual(0, Z_AXIS_3D(), {150.f, 0.f, 22.f},
                                  poseA.GetParent());
        const Pose3d poseB_actual(0, Z_AXIS_3D(), {300.f, -150.f, 0.f},
                                  poseB.GetParent());
        CST_ASSERT(
            poseA.IsSameAs(poseA_actual, _poseDistThresh_mm, _poseAngleThresh),
            "First object's pose incorrect after re-localization.");

        CST_ASSERT(
            poseB.IsSameAs(poseB_actual, _poseDistThresh_mm, _poseAngleThresh),
...
void CST_RobotKidnapping::HandleRobotStateUpdate(
...
void CST_RobotKidnapping::HandleActiveObjectConnectionState(
