 ...
 * Created: 2/3/2017
 *
 * Description: 1. Define two custom objects (cube and wall) successfully, two
 * unsuccessfully.
...
class CST_CustomObjects : public CozmoSimTestController {
 ...
 private:
  ...
  virtual void HandleDefinedCustomObject(
      const ExternalInterface::DefinedCustomObject& msg) override;
  virtual void HandleRobotDeletedCustomMarkerObjects(
      const ExternalInterface::RobotDeletedCustomMarkerObjects& msg) override;

  void GetDimension(const webots::Node* node, const std::string& name,
                    f32& dim);
  void DefineObjects();
  void CheckPoses();
  void CheckPoseHelper(const ObservableObject* customObj,
...
CST_CustomObjects::CST_CustomObjects()
    : kPoseOrigin(0, Z_AXIS_3D(), {0.f, 0.f, 0.f}),
      ...
      _wallPose2(Radians(-2.15f), Z_AXIS_3D(), {60.f, 310.f, 60.f},
                 kPoseOrigin) {}
...
s32 CST_CustomObjects::UpdateSimInternal() {
  switch (_testState) {
    case TestState::Init: {
      ...
      CST_ASSERT(_cubeSize_mm == otherCubeSize_mm,
                 "CST_CustomObjects.Init.Cube2SizeMismatch");
      CST_ASSERT(_cubeMarkerSize_mm == otherCubeMarkerSize_mm,
                 "CST_CustomObjects.Init.Cube2MarkerSizeMismatch");

      GetDimension(_cube3, "width", otherCubeSize_mm);
      GetDimension(_cube3, "markerWidth", otherCubeMarkerSize_mm);

      CST_ASSERT(_cubeSize_mm == otherCubeSize_mm,
                 "CST_CustomObjects.Init.Cube3SizeMismatch");
      CST_ASSERT(_cubeMarkerSize_mm == otherCubeMarkerSize_mm,
                 "CST_CustomObjects.Init.Cube3MarkerSizeMismatch");
    ...
    case TestState::LookAtObjects: {
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          kDefaultTimeout_sec, _numDefinesReceived == kNumDefinitions,
        ...
        const std::vector<RobotActionUnion> kActions{
            RobotActionUnion(TurnInPlace(DEG_TO_RAD(90), 0.f, 0.f,
                                         POINT_TURN_ANGLE_TOL, false)),
            RobotActionUnion(SetHeadAngle(MIN_HEAD_ANGLE, 100.f, 100.f, 0.f)),
        };

        SendMessage(MessageGameToEngine(QueueCompoundAction(
            kIdTag, kNumRetries, kIsParallel, kPosition, kActions)));
    ...
    case TestState::TurnAndLookDown: {
      const Radians& currentOrientation =
          GetRobotPose().GetRotation().GetAngleAroundZaxis();

      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          kDefaultTimeout_sec, !IsRobotStatus(RobotStatusFlag::IS_MOVING),
    ...
    case TestState::LookBackUp: {
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          kDefaultTimeout_sec, !IsRobotStatus(RobotStatusFlag::IS_MOVING),
          NEAR(GetRobotHeadAngle_rad(), 0, HEAD_ANGLE_TOL),
          GetNumObjects() == 5, IsLocalizedToObject(),
          HasXSecondsPassedYet(
              2.0))  // Allow some time to observe the wall in its new pose
      {
        CheckPoses();
    ...
    case TestState::Kidnap: {
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          kDefaultTimeout_sec, !IsLocalizedToObject(), GetNumObjects() == 0) {
    ...
    case TestState::Undefine: {
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          kDefaultTimeout_sec, GetNumObjects() == 0, _numDefinesReceived == 0) {
        // TODO: Add test state where we look at a custom object but no longer
        // instantiate it?
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - -- - - - - - -
void CST_CustomObjects::GetDimension(const webots::Node* node,
                                     const std::string& name, f32& dim_mm) {
  ...
  CST_ASSERT(Util::IsFltGT(dim_mm, 0.f),
             "CST_CustomObjects.GetDimension.ZeroDimension");
}
...
void CST_CustomObjects::DefineObjects() {
  ...
  DefineCustomCube defineCube(ObjectType::CustomType00,
                              CustomObjectMarker::Circles2, _cubeSize_mm,
                              _cubeMarkerSize_mm, _cubeMarkerSize_mm, false);
  ...
  DefineCustomWall bogusWall(ObjectType::CustomType02,
                             CustomObjectMarker::Circles2, 1.f, 1.f, 1.f, 1.f,
                             false);

  SendMessage(MessageGameToEngine(std::move(bogusWall)));

  // This one should also fail: bad type specified (can't overwrite built-in
  // types)
  DefineCustomWall bogusCube(ObjectType::Block_LIGHTCUBE1,
                             CustomObjectMarker::Triangles3, 1.f, 1.f, 1.f, 1.f,
                             false);

  SendMessage(MessageGameToEngine(std::move(bogusCube)));

  // This definition should succeed
  DefineCustomWall defineWall(
      ObjectType::CustomType01, CustomObjectMarker::Diamonds4, _wallWidth_mm,
      _wallHeight_mm, _wallMarkerWidth_mm, _wallMarkerHeight_mm, true);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - -- - - - - - -
void CST_CustomObjects::CheckPoseHelper(const ObservableObject* object,
                                        const ObjectID& objectID) {
  Pose3d observedPose;
  const Result result = GetObjectPose(objectID, observedPose);

  CST_ASSERT(RESULT_OK == result,
             "CST_CustomObjects.ChechPoses.FailedToGetObjectPose");
  ...
  Point3f Tdiff;
  Radians angleDiff;
  const bool isPoseSame = truePose.IsSameAs_WithAmbiguity(
      observedPose, object->GetRotationAmbiguities(),
      Point3f(kDistTolerance_mm), DEG_TO_RAD(kAngleTolerance_deg), Tdiff,
      angleDiff);

  if (!isPoseSame) {
    PRINT_NAMED_ERROR("CST_CustomObjects.CheckPoses.PoseMismatch",
                      "%s %d: Tdiff=(%.2f,%.2f,%.2f) (Thresh=%.1f), "
                      "AngleDiff=%.1fdeg (Thresh=%.1f)",
...
void CST_CustomObjects::CheckPoses() {
  // Check wall:
  {
    ...
    CST_ASSERT(wallIDs.size() == 1,
               "CST_CustomObjects.CheckPoses.ExpectingOneWall");

    CustomObject* customObj = CustomObject::CreateWall(
        ObjectType::CustomType01, CustomObjectMarker::Diamonds4, _wallWidth_mm,
        _wallHeight_mm, _wallMarkerWidth_mm, _wallMarkerHeight_mm, true);
    ...
    if (_wallID.IsUnknown()) {
      ...
      // Remaining times, verify the ID has remained consistent, since it is
      // marked unique
      CST_ASSERT(_wallID == wallIDs.front(),
                 "CST_CustomObject.CheckPoses.WallIDChanged");
    }
  ...
  // Check cube:
  {
    ...
    std::vector<PoseAndID> posesAndIDs{
        {.cubePose = &_cubePose1, .objectID = customCubeIDs[0]}};

    if (_testState >= TestState::LookBackUp) {
      posesAndIDs.emplace_back(
          PoseAndID{.cubePose = &_cubePose2, .objectID = customCubeIDs[1]});
    }

    for (auto& poseAndID : posesAndIDs) {
      CustomObject* customCube = CustomObject::CreateCube(
          ObjectType::CustomType00, CustomObjectMarker::Circles2, _cubeSize_mm,
          _cubeMarkerSize_mm, _cubeMarkerSize_mm, false);
...
void CST_CustomObjects::HandleDefinedCustomObject(
    const ExternalInterface::DefinedCustomObject& msg) {
  _defineResults[_numDefinesReceived] = msg.success;
  ++_numDefinesReceived;
}

void CST_CustomObjects::HandleRobotDeletedCustomMarkerObjects(
