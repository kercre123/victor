...
enum class TestState { StartUpFreeplayMode, FreePlay, TestDone };
...
s32 CST_BehaviorTracker::UpdateSimInternal() {
  switch (_testState) {
    ...
    case TestState::TestDone: {
      ...
      for (stateChangeIter = _stateChangeList.begin();
           stateChangeIter != _stateChangeList.end(); ++stateChangeIter) {
        float elapsedTime = stateChangeIter->elapsedTime;

        // Track the total time per behavior
        if (stateChangeIter != _stateChangeList.begin()) {
          std::vector<BehaviorStateChange>::iterator prev =
              std::prev(stateChangeIter);
      ...
      // Currently deactivated until we know we will hit all behaviors in the
      // specified time
      /**for(const auto& behavior: _freeplayBehaviorList){
        CST_EXPECT(, "Behavior not played during freeplay");
      }**/

      for (timeMapIter = timeMap.begin(); timeMapIter != timeMap.end();
           ++timeMapIter) {
        PRINT_NAMED_INFO(
            "Webots.BehaviorTracker.TestData",
      ...
      for (countMapIter = countMap.begin(); countMapIter != countMap.end();
           ++countMapIter) {
        PRINT_NAMED_INFO(
            "Webots.BehaviorTracker.TestData",
...
void CST_BehaviorTracker::HandleBehaviorTransition(
