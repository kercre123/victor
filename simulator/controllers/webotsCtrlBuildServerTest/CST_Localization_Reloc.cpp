 ...
 * Created: 3/4/19
 *
 * Description: Localize to the charger, turn and deloc, see another object,
 * then see the charger again and ensure we re-localize and bring origins
 * together.
 *
...
enum class TestState {
  ...
  TurnAndObserveCustomCube,  // Turn to see the custom cube in the new
                             // delocalized origin
  TurnAndLocalizeToCharger,  // Now turn and see just the charger. We should
                             // re-localize and rejigger origins.
  TestDone
};
  ...
  switch (_testState) {
    ...
    case TestState::ObserveChargerAndCube: {
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
        ...
        // Turn away from objects so that we can delocalize while not seeing
        // anything
    ...
    case TestState::WaitForDeloc: {
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          DEFAULT_TIMEOUT, GetNumObjects() == 0, !IsLocalizedToObject()) {
    ...
    case TestState::TurnAndObserveCustomCube: {
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          DEFAULT_TIMEOUT, !IsRobotStatus(RobotStatusFlag::IS_MOVING),
          GetNumObjects() == 1) {
        // Turn back toward the charger and observe it again. This should cause
        // a re-localization, and both origins should be brought together such
        // that we know about all 3 objects.
    ...
    case TestState::TurnAndLocalizeToCharger: {
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          DEFAULT_TIMEOUT, !IsRobotStatus(RobotStatusFlag::IS_MOVING),
          GetNumObjects() == 3, IsLocalizedToObject()) {
        // The robot's estimate of the charger position and the cube position
        // should be the same as in the beginning, since we've gone back into
        // the original origin.
        Pose3d chargerPose;
        GetObjectPose(_chargerId, chargerPose);
        CST_ASSERT(chargerPose.IsSameAs(_initialChargerPose, kDistThreshold_mm,
                                        kAngleThreshold_rad),
...
void CST_Localization_Reloc::HandleRobotObservedObject(
...
void CST_Localization_Reloc::DefineCustomObject() {
  const float cubeSize_mm =
      M_TO_MM(_customCubeNode->getField("width")->getSFFloat());
  const float cubeMarkerSize_mm =
      M_TO_MM(_customCubeNode->getField("markerWidth")->getSFFloat());

  using namespace ExternalInterface;
  DefineCustomCube defineCube(ObjectType::CustomType00,
                              CustomObjectMarker::Circles2, cubeSize_mm,
                              cubeMarkerSize_mm, cubeMarkerSize_mm, false);
