 ...
 * Created: 2/16/2018
 *
 * Description: Plays a few test "cube light animations" and verifies that the
 * simulated cube LED colors change as expected based on the animations' json
 * definitions.
 *
...
class CST_LightCubeAnims : public CozmoSimTestController {
 ...
 private:
  ...
  const CubeAnimationTrigger _testAllLedsTrigger =
      CubeAnimationTrigger::TestAllLeds;
  const CubeAnimationTrigger _testOffsetTrigger =
      CubeAnimationTrigger::TestOffset;
  const CubeAnimationTrigger _testRotationTrigger =
      CubeAnimationTrigger::TestRotation;
  ...
  switch (_testState) {
    ...
    case TestState::WaitForHeadUp: {
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          15, _id >= 0, NEAR(GetRobotHeadAngle_rad(), 0.f, HEAD_ANGLE_TOL)) {
        SendCubeAnimation(_id, _testAllLedsTrigger);
        // Add BS_TIME_STEP_MS because the engine will take one tick to start
        // the anim
    ...
    case TestState::VerifyAllLeds: {
      ...
      const double curExpectedLedValue =
          (double)_expectedLedPattern.EvaluateY(timeIntoAnimation_sec);
      ...
      AssertLedValues({{{{curExpectedLedValue, 0.0, 0.0}},
                        {{0.0, curExpectedLedValue, 0.0}},
                        {{0.0, 0.0, curExpectedLedValue}},
                        {{0.0, 0.0, 0.0}}}},
                      "CST_LightCubeAnims.VerifyAllLeds.WrongLedColor");

      // Transition to next state as soon as we're past the expected curve's x
      // range
      const auto& lastNode =
          _expectedLedPattern.GetNode(_expectedLedPattern.GetNumNodes() - 1);
    ...
    case TestState::VerifyOffset: {
      ...
      const double curExpectedLedValueRed =
          (double)_expectedLedPattern.EvaluateY(timeIntoAnimation_sec);
      const double curExpectedLedValueGreen =
          (double)_expectedLedPattern.EvaluateY(timeIntoAnimation_sec -
                                                kLedOffset_sec);
      const double curExpectedLedValueBlue =
          (double)_expectedLedPattern.EvaluateY(timeIntoAnimation_sec -
                                                2.f * kLedOffset_sec);

      AssertLedValues({{{{curExpectedLedValueRed, 0.0, 0.0}},
    ...
    case TestState::VerifyRotation: {
      ...
      // Midway through each repetition of the animation, test that the expected
      // colors are correct
      if (NEAR(timeIntoAnimation_sec, 0.5f * kRotationAnimLength_sec,
               timeTol_sec)) {
        // Should be in the 0th rotation phase (LED0:red, LED1:green, and
        // LED2:blue all on)
        AssertRotationLedsCorrect(0);
      } else if (NEAR(timeIntoAnimation_sec, 1.5f * kRotationAnimLength_sec,
                      timeTol_sec)) {
        // Should be in the first rotation phase (LED1:red, LED2:green, and
        // LED3:blue all on)
        AssertRotationLedsCorrect(1);
      } else if (NEAR(timeIntoAnimation_sec, 2.5f * kRotationAnimLength_sec,
                      timeTol_sec)) {
        AssertRotationLedsCorrect(2);
      } else if (NEAR(timeIntoAnimation_sec, 3.5f * kRotationAnimLength_sec,
                      timeTol_sec)) {
...
void CST_LightCubeAnims::SetupExpectedLedPattern(
    Util::GraphEvaluator2d& graphEvaluator) {
...
void CST_LightCubeAnims::AssertRotationLedsCorrect(const u16 rotationPhase) {
  // Rotated LED0:red, LED1:green, and LED2:blue should be at full value, and
  // rest should be at 0.
  std::array<LEDValue, 4> expectedLedValues = {{{{kMaxLedValue, 0.0, 0.0}},
...
CST_LightCubeAnims::LEDValue CST_LightCubeAnims::GetLedColor(
    const u16 ledIndex) {
  DEV_ASSERT(ledIndex <= 4, "CST_LightCubeAnims.GetLedColor.InvalidIndex");

  static const webots::Field* colorField =
      GetNodeByDefName("cube")->getField("ledColors");
  DEV_ASSERT(colorField != nullptr,
             "CST_LightCubeAnims.GetLedColor.NullColorField");

  const double* colorVec = colorField->getMFVec3f(ledIndex);
  return {{colorVec[0], colorVec[1], colorVec[2]}};
}

void CST_LightCubeAnims::AssertLedValues(
    const std::array<LEDValue, 4> expectedLedVals, const char* errorStr,
    ...
    for (int colorInd = 0; colorInd < 3; colorInd++) {
      ...
      if (!NEAR(expectedLedVal, actualLedVal, tolerance)) {
        PRINT_NAMED_ERROR("CST_LightCubeAnims.AssertLedValues.WrongValue",
                          "Actual LED value of %f is not near expected value "
                          "of %f (tol %f) for ledIndex %d, colorIndex %d",
                          actualLedVal, expectedLedVal, kLedValueTol, ledIndex,
                          colorInd);
...
void CST_LightCubeAnims::HandleActiveObjectConnectionState(
