...
s32 CST_MotionCommands::UpdateSimInternal() {
  switch (_testState) {
    ...
    case TestState::MovingLift: {
      IF_CONDITION_WITH_TIMEOUT_ASSERT(
          NEAR(GetLiftHeight_mm(), LIFT_HEIGHT_HIGHDOCK, 2.f),
          DEFAULT_TIMEOUT) {
        SendMoveLift(0.f);

        // Drive backwards
        SendDriveWheels(-driveSpeed_mmps, -driveSpeed_mmps, driveAccel,
                        driveAccel);
    ...
    case TestState::Stopping1: {
      IF_CONDITION_WITH_TIMEOUT_ASSERT(
          !IsRobotStatus(RobotStatusFlag::IS_MOVING), DEFAULT_TIMEOUT) {
    ...
    case TestState::DrivingStraight: {
      IF_CONDITION_WITH_TIMEOUT_ASSERT(
          NEAR(GetRobotPose().GetTranslation().x(), 0, 10), DEFAULT_TIMEOUT) {
        SendStopAllMotors();
    ...
    case TestState::DrivingArc: {
      IF_CONDITION_WITH_TIMEOUT_ASSERT(
          GetRobotPose().GetRotation().GetAngleAroundZaxis().IsNear(
              DEG_TO_RAD(90.f), DEG_TO_RAD(5.f)),
          DEFAULT_TIMEOUT) {
