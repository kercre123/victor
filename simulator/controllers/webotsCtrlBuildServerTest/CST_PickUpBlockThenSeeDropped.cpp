 ...
 * Created: 2/16/17
 *
 * Description: Check that Cozmo properly detaches a cube from the lift when it
 * observers the cube somewhere else. This should work regardless of move
 * messages, since lifted cubes produce move messages when the robot moves, and
 * we can potentially pick up unconnected blocks.
 *
...
enum class TestState { Init, PickupObject, TeleportObject, MoveBack, TestDone };
...
void CST_PickUpBlockThenSeeDropped::DropCube() {
  ...
  // rsam: Experimentally this causes the cube to drop by causing a
  // DisengageGripper() call, but we should have a proper message for that. I
  // think Matt was working on that.
  f32 liftSpeedDegPerSec = DEG_TO_RAD(120);
  f32 liftAccelDegPerSec2 = DEG_TO_RAD(600);
  f32 liftDurationSec = 0;
  SendMoveLiftToHeight(LIFT_HEIGHT_HIGHDOCK, liftSpeedDegPerSec,
                       liftAccelDegPerSec2, liftDurationSec);
}
  ...
  switch (_testState) {
    ...
    case TestState::PickupObject: {
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
        ...
        m.action.Set_pickupObject(ExternalInterface::PickupObject(
            _cubeId, _defaultTestMotionProfile, 0, false, true));
    ...
    case TestState::TeleportObject: {
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          20, !IsRobotStatus(RobotStatusFlag::IS_MOVING),
          NEAR(GetRobotPose().GetRotation().GetAngleAroundZaxis().getDegrees(),
               0, ROBOT_ANGLE_TOL_DEG),
    ...
    case TestState::MoveBack: {
      const float kCubeHalfHeight_mm =
          44 * 0.5f;  // I don't see a way in webots to get size of node, and
                      // other CST have this hardcoded
      const float cubeZ = GetLightCubePoseActual(ObjectType::Block_LIGHTCUBE1)
                              .GetTranslation()
                              .z() -
                          kCubeHalfHeight_mm;
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          ...
          GetCarryingObjectID() == _cubeId,
          NEAR(cubeZ, robotZ,
               BLOCK_Z_TOL_MM))  // near Z (cube should be resting on floor)
      {
        // move back
        ...
        m.action.Set_driveStraight(
            ExternalInterface::DriveStraight(200, -dist_mm, true));
        ExternalInterface::MessageGameToEngine message;
        message.Set_QueueSingleAction(m);
        SendMessage(message);
...
void CST_PickUpBlockThenSeeDropped::HandleRobotCompletedAction(
