 ...
 *              origins when being delocalized and then re-seeing existing light
 * cubes.
 *
...
enum class TestState {
  ...
  SeeObjectAWithoutLocalizing,  // Kidnap to R3, see A, but don't localize (too
                                // far)
  ReLocalizeToObjectB,  // Turn towards C, relocalize and bring A into frame
  ReLocalizeToObjectC,  // Turn towards B, relocalize and bring A and C into
                        // frame
...
class CST_RobotKidnappingComplex : public CozmoSimTestController {
 ...
 private:
  ...
  // Message handlers
  virtual void HandleRobotStateUpdate(
      ExternalInterface::RobotState const& msg) override;
  ...
  virtual void HandleActiveObjectConnectionState(
...
CST_RobotKidnappingComplex::CST_RobotKidnappingComplex()
    : _kidnappedPose1(-M_PI_2_F, Z_AXIS_3D(), {0, -_gridSpacing_mm, 0}),
      _kidnappedPose2(M_PI_F, Z_AXIS_3D(),
                      {-2 * _gridSpacing_mm, _gridSpacing_mm, 0}),
      _kidnappedPose3(M_PI_2_F, Z_AXIS_3D(), {-_gridSpacing_mm, 0, 0}),
      _poseA_actual(0, Z_AXIS_3D(), {_gridSpacing_mm, 0.f, 22.f}, _fakeOrigin),
      _poseB_actual(0, Z_AXIS_3D(), {-_gridSpacing_mm, -_gridSpacing_mm, 22.f},
                    _fakeOrigin),
      _poseC_actual(0, Z_AXIS_3D(), {-2 * _gridSpacing_mm, 0.f, 22.f},
                    _fakeOrigin),
...
s32 CST_RobotKidnappingComplex::UpdateSimInternal() {
  switch (_testState) {
    ...
    case TestState::NotifyKidnap: {
      // Sending the delocalize message one tic after actually moving the robot
      // to be sure that no images from the previous pose are processed after
      // the delocalization.
      SendForceDelocalize();
    ...
    case TestState::Kidnap: {
      ...
      if (CONDITION_WITH_TIMEOUT_ASSERT(!IsLocalizedToObject(),
                                        _kidnapStartTime, 2)) {
    ...
    case TestState::FinishTurn: {
      if (CONDITION_WITH_TIMEOUT_ASSERT(_turnInPlaceDone && !_isMoving,
                                        _kidnapStartTime, 6)) {
    ...
    case TestState::ReSeeObjectA: {
      IF_CONDITION_WITH_TIMEOUT_ASSERT(
          _robotState.localizedToObjectID == _objectID_A, 3) {
        CST_ASSERT(IsRobotPoseCorrect(_poseDistThresh_mm, _poseAngleThresh),
    ...
    case TestState::SeeObjectAWithoutLocalizing: {
      IF_CONDITION_WITH_TIMEOUT_ASSERT(!_objectsSeen.empty(), 3) {
        CST_ASSERT(_robotState.localizedToObjectID < 0,
                   "SeeObjectAWithoutLocalizing: Should not localize to object "
                   "A - should be too far");

        // We should only know about one object now: Objects A
        CST_ASSERT(
            CheckObjectPoses({_objectID_A}, "SeeObjectAWithoutLocalizing"),
    ...
    case TestState::ReLocalizeToObjectC: {
      IF_CONDITION_WITH_TIMEOUT_ASSERT(
        ...
        CST_ASSERT(
            CheckObjectPoses({_objectID_A, _objectID_C}, "RelocalizeToObjectC"),
    ...
    case TestState::ReLocalizeToObjectB: {
      IF_CONDITION_WITH_TIMEOUT_ASSERT(
          _robotState.localizedToObjectID == _objectID_B, 3) {
        // We should know about all three objects now
        CST_ASSERT(CheckObjectPoses({_objectID_A, _objectID_B, _objectID_C},
                                    "RelocalizeToObjectB"),
                   "RelocalizeToObjectC: Object pose checks failed");
...
bool CST_RobotKidnappingComplex::CheckObjectPoses(std::vector<int>&& IDs,
                                                  const char* debugStr) {
  ...
  for (auto& objectID : IDs) {
    if (!IsObjectPoseWrtRobotCorrect(
            objectID, *_objectPosesActual[_objectIDToIdx[objectID]],
...
void CST_RobotKidnappingComplex::HandleRobotStateUpdate(
...
void CST_RobotKidnappingComplex::HandleRobotObservedObject(
    ExternalInterface::RobotObservedObject const& msg) {
  _objectsSeen.insert(msg.objectID);
}

void CST_RobotKidnappingComplex::HandleRobotCompletedAction(
...
void CST_RobotKidnappingComplex::HandleActiveObjectConnectionState(
