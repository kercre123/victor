...
CST_NavMap::CST_NavMap()
    : _init(false), _map(new MemoryMap()), _vizm(new VizManager()) {}
...
s32 CST_NavMap::UpdateSimInternal() {
  if (!_init) {
    ...
    _vizm->DrawSegment("inputLine", p1, p2, NamedColors::BLACK, true,
                       kVizPlanarHeight_mm);
    ...
    auto integralP1 =
        GetIntegralCoordinateOfNode(p1, center, precision, height);
    auto integralP2 =
        GetIntegralCoordinateOfNode(p2, center, precision, height);
    auto reprojP1 =
        GetCartesianCoordinateOfNode(integralP1, center, precision, height);
    auto reprojP2 =
        GetCartesianCoordinateOfNode(integralP2, center, precision, height);
    auto addrP1 = GetAddressForNodeCenter(integralP1, height);
    auto addrP2 = GetAddressForNodeCenter(integralP2, height);

    PRINT_NAMED_INFO("CST_NavMap", "P1 %11s coordinate = %s", "input",
                     p1.ToString().c_str());
    PRINT_NAMED_INFO("CST_NavMap", "P1 %11s coordinate = %s", "integral",
                     integralP1.ToString().c_str());
    PRINT_NAMED_INFO("CST_NavMap", "P1 %11s coordinate = %s", "reprojected",
                     reprojP1.ToString().c_str());
    PRINT_NAMED_INFO("CST_NavMap", "P1 address = %s", ToString(addrP1).c_str());

    PRINT_NAMED_INFO("CST_NavMap", "P2 %11s coordinate = %s", "input",
                     p2.ToString().c_str());
    PRINT_NAMED_INFO("CST_NavMap", "P2 %11s coordinate = %s", "integral",
                     integralP2.ToString().c_str());
    PRINT_NAMED_INFO("CST_NavMap", "P2 %11s coordinate = %s", "reprojected",
                     reprojP2.ToString().c_str());
    PRINT_NAMED_INFO("CST_NavMap", "P2 address = %s", ToString(addrP2).c_str());

    PRINT_NAMED_INFO("CST_NavMap", "Max Tree Height = %d", (int)height);

    // generate the rays emanating from the robot position, sweeping some
    // degrees around the recently inserted line in the NavMap
...
// visualizing a single ray
#if (0)
    {
      ...
      auto rayStart = QuadTreeTypes::GetIntegralCoordinateOfNode(
          {0.f, 0.f}, center, precision, height);
      auto rayEnd = QuadTreeTypes::GetIntegralCoordinateOfNode(
          testRayPoints[rayIdx], center, precision, height);
      auto bresPoints = GetBresenhamLine(rayStart, rayEnd, true);

      PRINT_NAMED_INFO("CST_NavMap", "start coordinate = (%s)",
                       rayStart.ToString().c_str());
      PRINT_NAMED_INFO("CST_NavMap", "final coordinate = (%s)",
                       rayEnd.ToString().c_str());
      PRINT_NAMED_INFO("CST_NavMap", "num raster points = %zd",
                       bresPoints.size());

      auto projRayStart =
          GetCartesianCoordinateOfNode(rayStart, center, precision, height);
      auto projRayEnd =
          GetCartesianCoordinateOfNode(rayEnd, center, precision, height);
      _vizm->DrawFrameAxes(
          "start_point", Pose3d(0.0, Z_AXIS_3D(), getPoint3(projRayStart)), 30);
      _vizm->DrawFrameAxes("final_point",
                           Pose3d(0.0, Z_AXIS_3D(), getPoint3(projRayEnd)), 30);

      bool toggle = false;
      for (int i = 1; i < bresPoints.size(); ++i) {
        _vizm->DrawSegment("bresenhamLine",
                           getPoint3(GetCartesianCoordinateOfNode(
                               bresPoints[i - 1], center, precision, height)),
                           getPoint3(GetCartesianCoordinateOfNode(
                               bresPoints[i], center, precision, height)),
      ...
      for (auto& p : bresPoints) {
        ...
        PRINT_NAMED_INFO("CST_NavMap", "NodeAddress for p(%s) = %s",
                         p.ToString().c_str(), ToString(testAddr).c_str());
        ...
        auto cellCenter = QuadTreeTypes::GetCartesianCoordinateOfNode(
            p, center, precision, height);
        _vizm->DrawQuad(VizQuadType::VIZ_QUAD_GENERIC_2D, quadCount++, cellQuad,
                        kVizPlanarHeight_mm, NamedColors::GREEN);
        _vizm->DrawFrameAxes(
            "testFrame", Pose3d(0.f, Z_AXIS_3D(), getPoint3(cellCenter)), 20);
      }

      _vizm->DrawSegment(
          "testRay", Point3f{0.f, 0.f, 0.f},
          Point3f{testRayPoints[rayIdx].x(), testRayPoints[rayIdx].y(), 0.f},
          NamedColors::BLACK, true, kVizPlanarHeight_mm);
    }
#endif
    ...
    std::vector<bool> collisionCheckResults =
        _map->AnyOf({0.f, 0.f}, testRayPoints, collisionCheckFun);
    const auto memo_time_us =
        double((std::chrono::system_clock::now() - start_memo).count()) /
        testRayPoints.size();

    size_t countMismatch = 0;
    for (int rayIdx = 0; rayIdx <= 300; ++rayIdx) {
      const auto start = std::chrono::system_clock::now();
      bool inCollision1Ray = _map->AnyOf(
          FastPolygon({{0.f, 0.f}, testRayPoints[rayIdx]}), collisionCheckFun);
      const auto basic_time_us =
      ...
      //                    Point3f{0.f, 0.f, 0.f},
      //                    Point3f{testRayPoints[i].x(), testRayPoints[i].y(),
      //                    0.f}, collisionCheckResults[i] ? NamedColors::RED :
      //                    NamedColors::BLUE, false, kVizPlanarHeight_mm);

      if (collisionCheckResults[rayIdx] != inCollision1Ray) {
        PRINT_NAMED_INFO(
            "CST_NavMap", "Collision Results do not match (%d) (got=%d exp=%d)",
    ...
    PRINT_NAMED_INFO("CST_NavMap", "Normal = %6.6f, Memo+Raster = %6.6f",
                     single_time_us, memo_time_us);
    CST_EXPECT(FLT_LE(memo_time_us, single_time_us),
               "Ray Checking slower than checking single rays.");
  }
  ...
  if (doViz) {
    ...
    _vizm->SendVizMessage(
        MessageViz(MemoryMapMessageVizBegin(0, data.mapInfo)));
    for (u32 seqNum = 0;
         seqNum * kFullQuadsPerMessage < data.quadInfoFull.size(); seqNum++) {
      auto start = seqNum * kFullQuadsPerMessage;
      auto end =
          std::min(data.quadInfoFull.size(), start + kFullQuadsPerMessage);
      _vizm->SendVizMessage(MessageViz(MemoryMapMessageViz(
          0, QuadInfoFullVector(data.quadInfoFull.begin() + start,
                                data.quadInfoFull.begin() + end))));
    }
