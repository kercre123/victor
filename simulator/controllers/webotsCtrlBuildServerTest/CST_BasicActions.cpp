...
class CST_BasicActions : public CozmoSimTestController {
 private:
  ...
  // Message handlers
  virtual void HandleRobotCompletedAction(
      const ExternalInterface::RobotCompletedAction& msg) override;
};
  ...
  switch (_testState) {
    ...
    case TestState::MoveLiftDown: {
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          DEFAULT_TIMEOUT, _lastActionResult == ActionResult::SUCCESS,
    ...
    case TestState::DriveForwards: {
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
        ...
        m.idTag = 2;
        m.action.Set_driveStraight(
            ExternalInterface::DriveStraight(200, 50, true));
    ...
    case TestState::TurnLeft: {
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          ...
          NEAR(GetRobotPose().GetRotation().GetAngleAroundZaxis().getDegrees(),
               0, 10)) {
    ...
    case TestState::PanAndTilt: {
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
        ...
        m.position = QueueActionPosition::NOW;
        m.idTag = 6;
        m.action.Set_panAndTilt(
            ExternalInterface::PanAndTilt(M_PI_F, M_PI_F, true, true));
        ExternalInterface::MessageGameToEngine message;
        message.Set_QueueSingleAction(m);
        SendMessage(message);
    ...
    case TestState::FacePose: {
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          DEFAULT_TIMEOUT, _lastActionResult == ActionResult::SUCCESS,
          !IsRobotStatus(RobotStatusFlag::IS_MOVING),
          GetRobotPose().GetRotation().GetAngleAroundZaxis().IsNear(
              DEG_TO_RAD(180.f), DEG_TO_RAD(10.f)),
          NEAR(GetRobotHeadAngle_rad(), MAX_HEAD_ANGLE, HEAD_ANGLE_TOL)) {
        ...
        m.action.Set_turnTowardsPose(ExternalInterface::TurnTowardsPose(
            GetRobotPose().GetTranslation().x(),
            GetRobotPose().GetTranslation().y() + -1000, NECK_JOINT_POSITION[2],
            M_PI_F, 0, 0, 0, 0, 0, 0));
        ExternalInterface::MessageGameToEngine message;
        message.Set_QueueSingleAction(m);
        SendMessage(message);
    ...
    case TestState::VisuallyVerifyNoObjectAtPose: {
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
        ...
        m.position = QueueActionPosition::NOW;
        m.idTag = 9;
        m.action.Set_visuallyVerifyNoObjectAtPose(
            ExternalInterface::VisuallyVerifyNoObjectAtPose(
                GetRobotPose().GetTranslation().x(),
                GetRobotPose().GetTranslation().y() + 100,
                NECK_JOINT_POSITION[2], kVisuallyVerifyTolerance_mm,
                kVisuallyVerifyTolerance_mm, kVisuallyVerifyTolerance_mm));
        ExternalInterface::MessageGameToEngine message;
        message.Set_QueueSingleAction(m);
        SendMessage(message);
    ...
    case TestState::VisuallyVerifyObjectAtPose: {
      // Verify robot is not seeing any objects at pose ~(0,100,0) which means
      // the VisuallyVerifyNoObjectAtPose succeeded
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          DEFAULT_TIMEOUT, _lastActionResult == ActionResult::SUCCESS,
          !IsRobotStatus(RobotStatusFlag::IS_MOVING),
          NEAR(GetRobotPose().GetRotation().GetAngleAroundZaxis().getDegrees(),
        ...
        m.action.Set_visuallyVerifyNoObjectAtPose(
            ExternalInterface::VisuallyVerifyNoObjectAtPose(
                _poseToVerify.x(), _poseToVerify.y(), _poseToVerify.z(),
                kVisuallyVerifyTolerance_mm, kVisuallyVerifyTolerance_mm,
    ...
    case TestState::TurnLeftRelative_540: {
      // Verify robot is seeing an object at pose ~(190,0,22) which means the
      // VisuallyVerifyNoObjectAtPose action failed
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          DEFAULT_TIMEOUT, !IsRobotStatus(RobotStatusFlag::IS_MOVING),
          NEAR(GetRobotPose().GetRotation().GetAngleAroundZaxis().getDegrees(),
               0, 10),
    ...
    case TestState::FaceObject: {
      ...
      const Radians currAngle =
          GetRobotPoseActual().GetRotation().GetAngleAroundZaxis();
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
        ...
        std::vector<s32> lightCubeIDs =
            GetAllObjectIDsByType(ObjectType::Block_LIGHTCUBE1);
        CST_ASSERT(!lightCubeIDs.empty(),
                   "Found no cubes of type Block_LIGHTCUBE1");
        CST_ASSERT(lightCubeIDs.size() == 1,
                   "Found too many cubes of type Block_LIGHTCUBE1");

        m.action.Set_turnTowardsObject(ExternalInterface::TurnTowardsObject(
            lightCubeIDs[0], M_PI_F, 0, 0, 0, 0, 0, 0, true, false));
        ExternalInterface::MessageGameToEngine message;
        message.Set_QueueSingleAction(m);
        SendMessage(message);
    ...
    case TestState::TurnAbsolute_90: {
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          ...
          NEAR(_angularDistTraversed_deg, -540.f, 10.f)) {
        StartingAction(RobotActionType::TURN_IN_PLACE);
        SendTurnInPlace(DEG_TO_RAD(90.f), DEG_TO_RAD(150), 0,
...
void CST_BasicActions::StartingAction(const RobotActionType& actionType) {
  // Ensure that HandleRobotCompletedAction has reset _lastActionType to
  // UNKNOWN. Otherwise
  //  we may be trying to start an action before the previous one's completion
  //  was handled.
  CST_ASSERT(_lastActionType == RobotActionType::UNKNOWN,
             "_lastActionType was never reset to UNKNOWN!");
...
void CST_BasicActions::HandleRobotCompletedAction(
    const ExternalInterface::RobotCompletedAction& msg) {
  PRINT_NAMED_INFO("CST_BasicActions.HandleRobotCompletedAction",
                   "completed action %s, result %s",
                   EnumToString(msg.actionType), EnumToString(msg.result));
  ...
  if (msg.actionType == _lastActionType) {
    ...
    PRINT_NAMED_WARNING("CST_BasicActions.HandleRobotCompletedAction",
                        "An unexpected action completed. msg.actionType = %s, "
                        "_lastActionType = %s",
