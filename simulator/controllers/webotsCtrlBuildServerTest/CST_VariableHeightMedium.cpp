...
enum class TestState { Init, PickupMedium, TestDone };
  ...
  switch (_testState) {
    ...
    case TestState::PickupMedium: {
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
        ...
        auto objectsWithType =
            GetAllObjectIDsByType(ObjectType::Block_LIGHTCUBE2);
        CST_ASSERT(objectsWithType.size() == 1,
                   "Expecting 1 object of type LIGHTCUBE2");
        _id = objectsWithType.front();
        m.action.Set_pickupObject(ExternalInterface::PickupObject(
            _id, _defaultTestMotionProfile, 0, false, true));
