...
int main(int argc, char** argv) {
  // Note: we don't allow logFiltering in BuildServerTest like we do in the
  // other controllers because this controller is meant to show all logs.

  // create platform
  const Anki::Util::Data::DataPlatform& dataPlatform =
      WebotsCtrlShared::CreateDataPlatformTest(argv[0],
                                               "webotsCtrlBuildServer");
  ...
  if (nullptr == cstCtrl) {
    PRINT_NAMED_ERROR("WebotsCtrlBuildServerTest.main.TestNotFound",
                      "'%s' test not found", testName.c_str());
    QuitWebots(-1);
  }

  // Check for flag indicating whether or not webots should continue running
  // after the test is complete.
  const bool quitAfterTest =
      (argc >= 3) && (0 == strcmp(argv[2], "--quitWebotsAfterTest"));
  cstCtrl->SetQuitWebotsAfterTest(quitAfterTest);

  PRINT_NAMED_INFO("WebotsCtrlBuildServerTest.main.StartingTest", "%s",
                   testName.c_str());
