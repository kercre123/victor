 ...
 * Created: 3/4/19
 *
 * Description: Localize to a charger, move the charger, and ensure that poses
 * are updated properly (e.g., ensure that the robot appropriately adjusts the
 * _charger_ pose vs. its _own_ pose)
 *
...
enum class TestState {
  ...
  ObserveChargerAndCube,    // See the charger and cube, and localize to the
                            // charger.
  ObserveChargerInNewPose,  // Move the charger and allow the robot to observe
                            // it in its new position. We should update the
                            // _charger's_ position (not the robot's position)
                            // since the robot has not moved at all.
  MoveRobot,  // Now move the robot to a new position (and also moving its head)
  ObserveChargerAgain,  // Observe the charger again. This time, since the robot
                        // has moved, we should update the _robot's_ position,
                        // and the estimated charger pose should remain the
                        // same.
  TestDone
};
...
class CST_Localization_PoseUpdate : public CozmoSimTestController {
  ...
  virtual void HandleRobotObservedObject(
      ExternalInterface::RobotObservedObject const& msg) override;
  ...
  switch (_testState) {
    ...
    case TestState::ObserveChargerAndCube: {
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          ...
          GetNumObjects() == 2, IsLocalizedToObject()) {
        ...
        actualChargerPose.SetTranslation(actualChargerPose.GetTranslation() +
                                         kMoveTranslation);
    ...
    case TestState::ObserveChargerInNewPose: {
      ...
      expectedChargerPose.SetTranslation(expectedChargerPose.GetTranslation() +
                                         kMoveTranslation);

      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          DEFAULT_TIMEOUT,
          chargerPose.IsSameAs(expectedChargerPose, kDistThreshold_mm,
                               kAngleThreshold_rad)) {
        // The estimated charger pose has moved as expected. The robot's
        // estimate of the cube pose and its own pose should be exactly the same
        // as before.
        CST_ASSERT(GetRobotPose().IsSameAs(_initialRobotPose, kDistThreshold_mm,
                                           kAngleThreshold_rad),
                   "Esimated robot pose has changed");

        Pose3d cubePose;
        GetObjectPose(_cubeId, cubePose);
        CST_ASSERT(cubePose.IsSameAs(_initialCubePose, kDistThreshold_mm,
                                     kAngleThreshold_rad),
                   "Estimated cube pose has changed");

        // Move the robot's head up so that it cannot see the charger/cube, and
        // so that it will register the camera as having moved.
    ...
    case TestState::MoveRobot: {
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          DEFAULT_TIMEOUT, !IsRobotStatus(RobotStatusFlag::IS_MOVING)) {
        // Teleport the robot forward and move the head level again so we see
        // the charger.
        Pose3d actualRobotPose = GetRobotPoseActual();
        actualRobotPose.SetTranslation(actualRobotPose.GetTranslation() +
                                       kMoveTranslation);
    ...
    case TestState::ObserveChargerAgain: {
      // We expect the robot's pose estimate to have adjusted by virtue of
      // localizing to the charger.
      Pose3d expectedRobotPose = _initialRobotPose;
      expectedRobotPose.SetTranslation(expectedRobotPose.GetTranslation() +
                                       kMoveTranslation);

      // We also expect the charger pose and the cube pose to _not_ have changed
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          DEFAULT_TIMEOUT,
          GetRobotPose().IsSameAs(expectedRobotPose, kDistThreshold_mm,
                                  kAngleThreshold_rad),
          chargerPose.IsSameAs(expectedChargerPose, kDistThreshold_mm,
                               kAngleThreshold_rad),
          cubePose.IsSameAs(_initialCubePose, kDistThreshold_mm,
                            kAngleThreshold_rad)) {
...
void CST_Localization_PoseUpdate::HandleRobotObservedObject(
    ExternalInterface::RobotObservedObject const& msg) {
