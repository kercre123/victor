...
class CST_IntroducedObstacleAvoidance : public CozmoSimTestController {
 ...
 private:
  ...
  void HandleRobotCompletedAction(
...
s32 CST_IntroducedObstacleAvoidance::UpdateSimInternal() {
  ...
  const Pose3d kObstructingPose = {
      M_PI_2_F, {1, 0, 0}, {200, 0, kDuckHeight_mm}, _webotsOrigin};
  ...
  switch (_testState) {
    case TestState::Init: {
      ...
      SendPushDrivingAnimations("webots_test", AnimationTrigger::Count,
    ...
    case TestState::ExecuteStraightPath: {
      // Note that this does not drive cozmo to the ground truth pose of
      // kRobotDestination, but rather it will be relative to where it starts.
      // That is, if cozmo starts at x=-200mm and the translation in the pose is
      // x=+500mm, he will travel to a ground truth position of +300mm
    ...
    case TestState::IntroduceObstacle: {
      ...
      const bool result = ComputeDistanceBetween(
          GetRobotPoseActual(), kObstructingPose, distanceToObstructingPose_mm);
      CST_ASSERT(
          result,
          "Failed computing distance between robot pose and obstructing pose");
      IF_CONDITION_WITH_TIMEOUT_ASSERT(distanceToObstructingPose_mm < 150.0,
                                       10) {
        // Put the rubber duck in the way of the robot path.
        SetNodePose(_duckNode, kObstructingPose);
        _driveToPoseSucceeded = false;  // reset var just before we check for it
                                        // in the next stage just in case
    ...
    case TestState::VerifyDriveToPoseCompleted: {
      // Takes about 12 seconds for robot to complete the path, set timeout at
      // 20 for some leeway.
    ...
    case TestState::VerifyObstacleAvoidance: {
      ...
      CST_ASSERT(obstaclePoseActual.IsSameAs(
                     kObstructingPose, kDistanceThreshold, kAngleThreshold),
                 "The rubber duck was moved when it should have been avoided "
                 "by the robot.")

      const Pose3d robotPoseActual = GetRobotPoseActual();

      CST_ASSERT(robotPoseActual.IsSameAs(kRobotDestination, kDistanceThreshold,
                                          kAngleThreshold),
                 "The robot didn't reach its destination: expected "
                     << kRobotDestination.GetTranslation().ToString().c_str()
                     << ", got "
                     << robotPoseActual.GetTranslation().ToString());
...
void CST_IntroducedObstacleAvoidance::HandleRobotCompletedAction(
