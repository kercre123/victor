...
bool EqualityCStyleArray(const double* arr1, const double* arr2,
                         int arraySize) {
  for (int i = 0; i < arraySize; i++) {
    if (arr1[i] != arr2[i]) {
      PRINT_NAMED_DEBUG("CST_LEDColor.EqualityCStyleArray",
                        "These two were not equal: arr1 %f, arr2 %f", arr1[i],
                        arr2[i]);
      return false;
    }
  }
...
class CST_LEDColor : public CozmoSimTestController {
 ...
 private:
  ...
  void HandleActiveObjectConnectionState(
      const ExternalInterface::ObjectConnectionState& msg) override;
  ...
  // We pick 240 for on/off period here because it is a multiple of
  // BS_TIME_STEP_MS (currently 60ms) which makes it a lot easier to detect the
  // on/off timing correctly
  ...
  // relative x, y are garbage values since MakeRelativeMode =
  // RELATIVE_LED_MODE_OFF; see COZMO-3049
  const f32 kRelativeToX = 0;
  const f32 kRelativeToY = 0;
  const MakeRelativeMode kMakeRelative =
...
s32 CST_LEDColor::UpdateSimInternal() {
  switch (_testState) {
    ...
    case TestState::WaitForHeadUp: {
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          15, _id >= 0,
    ...
    case TestState::SetRGB: {
      ...
      const std::array<u32, 4> onColor = {
          {kRedColor, kGreenColor, kBlueColor, kBlackColor}};
      const std::array<u32, 4> onPeriod_ms = {
          {kOnPeriod_ms, kOnPeriod_ms, kOnPeriod_ms, kOnPeriod_ms}};
      const std::array<u32, 4> offPeriod_ms = {
          {kOffPeriod_ms, kOffPeriod_ms, kOffPeriod_ms, kOffPeriod_ms}};
      const std::array<u32, 4> transitionOnPeriod_ms = {
          {kTransitionOnPeriod_ms, kTransitionOnPeriod_ms,
           kTransitionOnPeriod_ms, kTransitionOnPeriod_ms}};
      const std::array<u32, 4> transitionOffPeriod_ms = {
          {kTransitionOffPeriod_ms, kTransitionOffPeriod_ms,
           kTransitionOffPeriod_ms, kTransitionOffPeriod_ms}};

      SendSetAllActiveObjectLEDs(
          _id, onColor, onColor, onPeriod_ms, offPeriod_ms,
          transitionOnPeriod_ms, transitionOffPeriod_ms,
          {{kOffset_ms, kOffset_ms, kOffset_ms, kOffset_ms}}, kRotate,
          kRelativeToX, kRelativeToY, kMakeRelative);
    ...
    case TestState::VerifyLEDColors: {
      const webots::Field* colorField =
          GetNodeByDefName("cube")->getField("ledColors");
      const double kLed0Color[3] = {colorField->getMFVec3f(0)[0],
                                    colorField->getMFVec3f(0)[1],
                                    colorField->getMFVec3f(0)[2]};  // Back
      const double kLed1Color[3] = {colorField->getMFVec3f(1)[0],
                                    colorField->getMFVec3f(1)[1],
                                    colorField->getMFVec3f(1)[2]};  // Left
      const double kLed2Color[3] = {colorField->getMFVec3f(2)[0],
                                    colorField->getMFVec3f(2)[1],
                                    colorField->getMFVec3f(2)[2]};  // Front
      const double kLed3Color[3] = {colorField->getMFVec3f(3)[0],
                                    colorField->getMFVec3f(3)[1],
                                    colorField->getMFVec3f(3)[2]};  // Right

      //      PRINT_NAMED_INFO("colorField", "%0llx", (u64)colorField);
      //      PRINT_NAMED_INFO("VerifyColors0", "%f %f %f", kLed0Color[0],
      //      kLed0Color[1], kLed0Color[2]); PRINT_NAMED_INFO("VerifyColors1",
      //      "%f %f %f", kLed1Color[0], kLed1Color[1], kLed1Color[2]);
      //      PRINT_NAMED_INFO("VerifyColors2", "%f %f %f", kLed2Color[0],
      //      kLed2Color[1], kLed2Color[2]); PRINT_NAMED_INFO("VerifyColors3",
      //      "%f %f %f", kLed3Color[0], kLed3Color[1], kLed3Color[2]);

      // We only check if there are any color in each channel at all because
      // there are some post- processing that happens inside engine with the
      // color information sent from game like white balanching which means the
      // LED won't have the exact color that was sent.
    ...
    case TestState::SetLEDAnimation: {
      SendSetActiveObjectLEDs(
          _id, kRedColor, kBlackColor, kOnPeriod_ms, kOffPeriod_ms,
          kTransitionOnPeriod_ms, kTransitionOffPeriod_ms, kOffset_ms, kRotate,
          kRelativeToX, kRelativeToY, WhichCubeLEDs::ALL, kMakeRelative, true);
    ...
    case TestState::VerifyLEDAnimation: {
      // Considered correct if there are correct number of frames in the on
      // period, and the correct number of frames in the off period,
      // back-to-back.
      ...
      if (kOnPeriod_ms % BS_TIME_STEP_MS != 0 ||
          kOffPeriod_ms % BS_TIME_STEP_MS != 0) {
        PRINT_NAMED_WARNING("CST_LEDColor.VerifyLEDAnimation",
                            "If on or off period is not divisible by "
                            "BS_TIME_STEP_MS it is going to be"
                            "difficult to verify the on or off period because "
                            "this update loop will"
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(5, _onFramesMatched,
                                            _offFramesMatched) {
        SET_TEST_STATE(Exit);
      }
    } break;
...
void CST_LEDColor::HandleActiveObjectConnectionState(
