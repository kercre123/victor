...
enum class TestState { Init, ShiftChargerSlightly, TestDone };
...
s32 CST_ChargerDocking::UpdateSimInternal() {
  switch (_testState) {
    case TestState::Init: {
      // Start freeplay mode. The robot's proto in the
      // testWorldChargerDocking.wbt world has the battery level set to a 'low'
      // level, so the robot should immediately begin trying to dock with the
      // charger.
      StartFreeplayMode();
    ...
    case TestState::ShiftChargerSlightly: {
      ...
      auto* chargerNode = WebotsHelpers::GetFirstMatchingSceneTreeNode(
                              GetSupervisor(), "VictorCharger")
                              .nodePtr;
      auto chargerPose = GetPose3dOfNode(chargerNode);
      const auto& robotPose = GetRobotPoseActual();
      float distanceAway_mm = 0.f;
      const bool result =
          ComputeDistanceBetween(chargerPose, robotPose, distanceAway_mm);
      CST_ASSERT(
          result,
          "Failed computing distance between charger pose and robot pose");
      const float angleBetween_deg = (chargerPose.GetRotationAngle<'Z'>() -
                                      robotPose.GetRotationAngle<'Z'>())
                                         .getDegrees();
      if (distanceAway_mm < 180.f && NEAR(angleBetween_deg, -90.f, 10.f)) {
