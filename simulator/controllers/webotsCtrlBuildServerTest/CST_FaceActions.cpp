...
s32 CST_FaceActions::UpdateSimInternal() {
  switch (_testState) {
    case TestState::SetupVisionMode: {
      // enable the correct vision modes (using the console var message for this
      // will also ensure the right schedule is used as well)
    ...
    case TestState::TurnToFace: {
      ...
      m.action.Set_turnInPlace(ExternalInterface::TurnInPlace(
          -M_PI_F / 2, DEG_TO_RAD(100), 0, POINT_TURN_ANGLE_TOL, false));
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
        ...
        m.action.Set_turnTowardsLastFacePose(
            ExternalInterface::TurnTowardsLastFacePose(
                M_PI_F, 0, 0, 0, 0, 0,
                DEG_TO_RAD(kTestDoneGoalTiltTol_deg) /
                    2,  // action's tilt tolerance
                false, AnimationTrigger::Count, AnimationTrigger::Count));
        ExternalInterface::MessageGameToEngine message;
        message.Set_QueueSingleAction(m);
        SendMessage(message);
    ...
    case TestState::TestDone: {
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          DEFAULT_TIMEOUT, !IsRobotStatus(RobotStatusFlag::IS_MOVING),
          NEAR(GetRobotHeadAngle_rad(), DEG_TO_RAD(kTestDoneGoalTilt_deg),
               DEG_TO_RAD(kTestDoneGoalTiltTol_deg)),
          NEAR(GetRobotPose().GetRotation().GetAngleAroundZaxis().getDegrees(),
               -90, 10),
          _prevFaceSeenTime < _faceSeenTime, _prevFaceSeenTime != 0) {
        CST_EXIT();
      }
      break;
    }
...
void CST_FaceActions::HandleRobotCompletedAction(
...
void CST_FaceActions::HandleRobotObservedFace(
