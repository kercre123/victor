...
enum class TestState {
  ...
  WaitingForObjectAvailable,  // We should receive ObjectAvailable messages from
                              // all three cubes
  WaitingForConnectToCubeA,   // We should connect to the closest cube, which is
                              // cube A
  WaitingForDisconnectFromCubeA,
  WaitingForConnectToCubeA_again,  // We should connect to cube A again, since
                                   // we have not reset the preferred cube
  WaitingForDisconnectFromCubeA_again,
  WaitingForConnectToCubeB,  // We should connect to the closest cube, which is
                             // now cube B
  WaitingForDisconnectFromCubeB,
  WaitingForConnectToCubeC,  // We should connect to the preferred cube, which
                             // we have set as cube C
  WaitingForUnexpectedDisconnect,  // After zapping cube C from the world, we
                                   // should get a disconnection message
  Exit
};
...
class CST_CubeConnection : public CozmoSimTestController {
 ...
 private:
  ...
  void HandleActiveObjectAvailable(
      const ExternalInterface::ObjectAvailable& msg) override;
...
s32 CST_CubeConnection::UpdateSimInternal() {
  switch (_testState) {
    case TestState::RequestCubeConnection: {
      CST_ASSERT(!IsAnyCubeConnected(),
                 "Should not be connected to any cubes initially");
    ...
    case TestState::WaitingForObjectAvailable: {
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          5, _cubes[kCubeA].objectAvailableCnt > 0,
    ...
    case TestState::WaitingForConnectToCubeA: {
      ...
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(5, IsAnyCubeConnected(),
    ...
    case TestState::WaitingForConnectToCubeA_again: {
      // We should connect to cube A again (even though it's not the closest
      // cube) since we did _not_ reset the preferred cube.
    ...
    case TestState::WaitingForConnectToCubeC: {
      ...
      IF_CONDITION_WITH_TIMEOUT_ASSERT(GetConnectedCube() == kCubeC, 5) {
        // Remove cube C from the world, which should trigger a disconnection
        // message
...
void CST_CubeConnection::HandleActiveObjectAvailable(
...
void CST_CubeConnection::HandleActiveObjectConnectionState(
    const ExternalInterface::ObjectConnectionState& msg) {
  CST_ASSERT(_cubes.find(msg.factoryID) != _cubes.end(),
             "Received ObjectConnectionState from unknown cube");
