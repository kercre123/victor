...
class CST_RollBlockBehavior : public CozmoSimTestController {
 private:
  ...
  virtual void HandleBehaviorTransition(
      const ExternalInterface::BehaviorTransition& msg) override;
  ...
  switch (_testState) {
    ...
    case TestState::VerifyObject: {
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          DEFAULT_TIMEOUT, _moveHeadToAngleResult == ActionResult::SUCCESS,
          GetNumObjects() == 1, IsLocalizedToObject()) {
    ...
    case TestState::TurnAway: {
      IF_CONDITION_WITH_TIMEOUT_ASSERT(
          _turnInPlaceResult == ActionResult::SUCCESS, 10) {
        // Make sure we are still localized (to an object) before sending deloc
        CST_ASSERT(IsLocalizedToObject(),
                   "Should be localized to object before we deloc");
        SendForceDelocalize();
    ...
    case TestState::DontStartBehavior: {
      CST_ASSERT(!_startedBehavior,
                 "Behavior shouldnt start because we delocalized");
    ...
    case TestState::TurnBack: {
      // At some point (possibly before we stop moving) the behavior should
      // become activatable and start on it's own.
      //  The behavior kicking in may cause the TurnInPlace to be CANCELLED.
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          10,
          (_turnInPlaceResult == ActionResult::SUCCESS ||
           _turnInPlaceResult == ActionResult::CANCELLED_WHILE_RUNNING),
          _startedBehavior) {
    ...
    case TestState::Rolling: {
      // Verify that the behavior has stopped and that the cube has been
      // flipped.
      Pose3d pose = GetLightCubePoseActual(ObjectType::Block_LIGHTCUBE1);
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          25, _stoppedBehavior, !IsRobotStatus(RobotStatusFlag::IS_MOVING),
        ...
        //  The behavior will trigger again automatically once the robot sees
        //  it.
    ...
    case TestState::PushBlockBackward: {
      ...
      const bool result =
          ComputeDistanceBetween(robotPose, cubePose, distToCube_mm);
      ...
      // Still driving?
      const bool wheelsMoving =
          IsRobotStatus(RobotStatusFlag::ARE_WHEELS_MOVING);

      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(25, _startedBehavior, nearBlock,
                                            !wheelsMoving) {
    ...
    case TestState::PushBlockToSide: {
      // Cozmo should fail to roll the block, move backward, and approach block
      // to retry.

      // The roll behavior should never have stopped:
      CST_ASSERT(
          !_stoppedBehavior,
          "Roll behavior should not be finished yet! Still need to retry.")

      // Wait for robot to get close to block, then give it a shove to the side
      // such that it is out of view.
      Pose3d robotPose = GetRobotPoseActual();
      Pose3d cubePose = GetLightCubePoseActual(ObjectType::Block_LIGHTCUBE1);
      float distToCube_mm = 0.f;
      const bool result =
      ...
      double currTime = GetSupervisor().getTime();
      IF_ALL_CONDITIONS_WITH_TIMEOUT_ASSERT(
          15, currTime - _pushedBlockTime > timeToWait_s, nearBlock,
          !_stoppedBehavior) {
...
void CST_RollBlockBehavior::HandleRobotCompletedAction(
    const ExternalInterface::RobotCompletedAction& msg) {
  PRINT_NAMED_INFO("CST_RollBlockBehavior.HandleRobotCompletedAction",
...
void CST_RollBlockBehavior::HandleBehaviorTransition(
