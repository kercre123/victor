 ...
 * cubes one at a time and then all together (from pose R3).
 *
...
inline bool CST_MultiObjectLocalization::HasRelocalizedTo(
    const ObjectID& objectID) const {
  const bool done = (_turnInPlaceDone && _moveHeadDone && objectID.IsSet() &&
  ...
  switch (_testState) {
    ...
    case TestState::Kidnap: {
      ...
      if (CONDITION_WITH_TIMEOUT_ASSERT(!IsLocalizedToObject(),
        ...
        SendMoveHeadToAngle(DEG_TO_RAD(_headAngle_deg), DEG_TO_RAD(360),
                            DEG_TO_RAD(1000));
    ...
    case TestState::LocalizeToObjectC: {
      IF_CONDITION_WITH_TIMEOUT_ASSERT(HasRelocalizedTo(_objectID_C), 3) {
        ...
        //          CST_ASSERT(checkPose.IsSameAs(GetRobotPoseActual(),
        //          _poseDistThresh_mm, _poseAngleThresh),
...
bool CST_MultiObjectLocalization::CheckObjectPoses(std::vector<int>&& IDs,
                                                   const char* debugStr) {
...
void CST_MultiObjectLocalization::HandleRobotObservedObject(
...
void CST_MultiObjectLocalization::HandleRobotCompletedAction(
