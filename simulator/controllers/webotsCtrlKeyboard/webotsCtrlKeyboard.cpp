...
double poseMarkerColor_[2][3] = {
    {0.1, 0.8, 0.1}  // Goto pose color
    ,
...
std::set<u32> streamingAccelObjIds;  // ObjectIDs of objects that are currently
...
std::vector<int>
    _keyRegistrationOrder;  // For printing help menu in order of insertion
...
void WebotsKeyboardController::HandleImageChunk(const ImageChunk& msg) {
  ...
  if (isImageReady) {
    ...
    for (s32 i = 0; i < cvImg.rows; ++i) {
      if (i % 2 == 0) {
        ...
        for (s32 j = 0; j < cvImg.cols; ++j) {
          img_i[3 * j + outputColor] = std::max(
              std::max(img_i[3 * j], img_i[3 * j + 1]), img_i[3 * j + 2]);

          img_i[3 * j + (3 - outputColor)] /= 2;
          img_i[3 * j] = 0;  // kill red channel

          // [Optional] Add a bit of noise
          f32 noise = 20.f * static_cast<f32>(std::rand()) /
                          static_cast<f32>(RAND_MAX) -
                      0.5f;
          img_i[3 * j + outputColor] = static_cast<u8>(std::max(
              0.f,
              std::min(255.f,
                       static_cast<f32>(img_i[3 * j + outputColor]) + noise)));
        }
      }
    }
    ...
    if (uiCamDisplay_ != nullptr) {
      ...
      img_ = uiCamDisplay_->imageNew(cvImg.cols, cvImg.rows, cvImg.data,
                                     webots::Display::RGB);
...
void WebotsKeyboardController::HandleRobotObservedObject(
  ...
  if (uiCamDisplay_ != nullptr) {
    ...
    std::string dispStr(
        "Type=" + std::string(ObjectTypeToString(msg.objectType)) +
        "\nID=" + std::to_string(msg.objectID));
    uiCamDisplay_->drawText(
        dispStr, msg.img_rect.x_topLeft + msg.img_rect.width / 4 + 1,
...
void WebotsKeyboardController::HandleRobotObservedFace(
...
void WebotsKeyboardController::HandleRobotObservedPet(
    const ExternalInterface::RobotObservedPet& msg) {
  // Record centroid of observation in image
  _lastObservedImageCentroid.SetFromMessage(msg);
}

void WebotsKeyboardController::HandleLoadedKnownFace(
    const Vision::LoadedKnownFace& msg) {
  printf(
      "HandleLoadedKnownFace: '%s' (ID:%d) first enrolled %lld seconds ago, "
      "last updated %lld seconds ago, last seen %lld seconds ago\n",
      msg.name.c_str(), msg.faceID, msg.secondsSinceFirstEnrolled,
      msg.secondsSinceLastUpdated, msg.secondsSinceLastSeen);
}

void WebotsKeyboardController::HandleEngineErrorCode(
...
void WebotsKeyboardController::WaitOnKeyboardToConnect() {
  ...
  if (autoConnectField == nullptr) {
    PRINT_NAMED_ERROR(
        "WebotsKeyboardController.MissingField",
  ...
  LOG_INFO("WebotsKeyboardController.WaitForStart",
           "Press Shift+Enter to start the engine");

  const int EnterKey = 4;  // tested experimentally... who knows if this will
  ...
  while (!start && !_shouldQuit) {
    ...
    while ((key = GetSupervisor().getKeyboard()->getKey()) >= 0 &&
      ...
      if (!start && key == ShiftEnterKey) {
        start = true;
        LOG_INFO("WebotsKeyboardController.StartEngine",
...
void WebotsKeyboardController::ToggleImageSaving() {
  ...
  PRINT_NAMED_INFO("ToggleImageSaving", "Mode: %s",
                   EnumToString(_imageStreamSavingMode));
...
void WebotsKeyboardController::GotoPoseMarker() {
  if (poseMarkerMode_ == 0) {
    ...
    Pose3d markerPose_inEngineFrame =
        GetRobotPose() * GetRobotPoseActual().GetInverse() * goalMarkerPose;
    ...
    // For placeOn and placeOnGround, specify whether or not to use the
    // exactRotation specified
    bool useExactRotation =
        root_->getField("useExactPlacementRotation")->getSFBool();

    // Indicate whether or not to place object at the exact rotation specified
    // or just use the nearest preActionPose so that it's merely aligned with
    // the specified pose.
    printf("Setting block on ground at rotation %f rads about z-axis (%s)\n",
           goalMarkerPose.GetRotationAngle<'Z'>().ToFloat(),
           useExactRotation ? "Using exact rotation"
                            : "Using nearest preActionPose");

    SendPlaceObjectOnGroundSequence(goalMarkerPose, pathMotionProfile_,
                                    useExactRotation);
...
void WebotsKeyboardController::SearchForNearbyObject() {
  ...
  ExternalInterface::SearchForNearbyObject searchAction{
      ...
      Util::numeric_cast<f32>(
          DEG_TO_RAD(Util::EnumToUnderlying(SFNOD::HeadAngle_deg)))};
...
void WebotsKeyboardController::ToggleCliffSensorEnable() {
  ...
  msg.Set_EnableCliffSensor(
      ExternalInterface::EnableCliffSensor{enableCliffSensor});
  SendMessage(msg);
...
void WebotsKeyboardController::ToggleTestBackpackLights() {
  ...
  if (!backpackLightsOn) {
    ...
    msg.onColor[(uint32_t)LEDId::LED_BACKPACK_MIDDLE] =
...
void WebotsKeyboardController::ToggleTrackToObject() {
  ...
  if (trackingObject) {
    ...
    printf("Telling robot to track %sto the currently observed object %d\n",
           headOnly ? "its head " : "", GetLastObservedObject().id);
...
void WebotsKeyboardController::ToggleTrackToFace() {
  ...
  if (trackingFace) {
    ...
    printf("Telling robot to track %sto the currently observed face %d\n",
           headOnly ? "its head " : "", (u32)GetLastObservedFaceID());
...
void WebotsKeyboardController::ExecuteBehavior() {
  ...
  if (!EnumFromString(behaviorName, behaviorId)) {
    PRINT_NAMED_ERROR(
        "WebotsKeyboardController.ExecuteBehavior.InvalidBehaviorID",
        "'%s' is not a valid behavior ID", behaviorName.c_str());
    return;
  }
  ...
  if (behaviorId == BehaviorID::LiftLoadTest) {
    SendMessage(ExternalInterface::MessageGameToEngine(
        ExternalInterface::SetLiftLoadTestAsActivatable()));
  }
...
void WebotsKeyboardController::FakeUserIntent() {
  ...
  SendMessage(ExternalInterface::MessageGameToEngine(
      ExternalInterface::FakeUserIntent(userIntent)));
}
...
void WebotsKeyboardController::SetEmotion() {
  ...
  if (emotionValField == nullptr) {
    printf(
        "ERROR: No emotionValField field found in "
        "WebotsKeyboardController.proto\n");
    return;
  }
  ...
  SendMessage(ExternalInterface::MessageGameToEngine(
      ExternalInterface::MoodMessage(ExternalInterface::MoodMessageUnion(
...
void WebotsKeyboardController::TriggerEmotionEvent() {
  ...
  SendMessage(ExternalInterface::MessageGameToEngine(
      ExternalInterface::MoodMessage(ExternalInterface::MoodMessageUnion(
          ExternalInterface::TriggerEmotionEvent(emotionEvent)))));
}
...
void WebotsKeyboardController::PickOrPlaceObject() {
  ...
  if (placeOnGroundAtOffset) {
    placementOffsetX_mm =
        root_->getField("placeOnGroundOffsetX_mm")->getSFFloat();
  }

  // Exact rotation to use if useExactRotation == true
  const double* rotVals =
      root_->getField("exactPlacementRotation")->getSFRotation();
  Rotation3d rot(rotVals[3],
                 {static_cast<f32>(rotVals[0]), static_cast<f32>(rotVals[1]),
                  static_cast<f32>(rotVals[2])});
  printf("Rotation %f\n", rot.GetAngleAroundZaxis().ToFloat());

  if (GetCarryingObjectID() < 0) {
    // Not carrying anything so pick up!
    SendPickupSelectedObject(pathMotionProfile_, usePreDockPose,
                             useApproachAngle, approachAngle_rad);
  } else {
    if (placeOnGroundAtOffset) {
      SendPlaceRelSelectedObject(pathMotionProfile_, usePreDockPose,
                                 placementOffsetX_mm, useApproachAngle,
                                 approachAngle_rad);
    } else {
      SendPlaceOnSelectedObject(pathMotionProfile_, usePreDockPose,
                                useApproachAngle, approachAngle_rad);
    }
  }
}
...
void WebotsKeyboardController::MountSelectedCharger() {
  ...
  SendMountSelectedCharger(pathMotionProfile_, useCliffSensorCorrection);
}
...
void WebotsKeyboardController::TeleportOntoCharger() {
  if (_chargerNode == nullptr) {
    ...
    const auto& chargerNodeInfo =
        WebotsHelpers::GetFirstMatchingSceneTreeNode(GetSupervisor(), nodeName);
    if (chargerNodeInfo.nodePtr == nullptr) {
      PRINT_NAMED_WARNING(
          "WebotsKeyboardController.TeleportOntoCharger.NoChargerNode",
          "can't find node '%s'", nodeName);
      return;
    }
  ...
  const bool transformOK =
      targetPose.GetWithRespectTo(_webotsOrigin, targetPose);
  if (!transformOK) {
    PRINT_NAMED_WARNING(
        "WebotsKeyboardController.TeleportOntoCharger.PoseChainError",
...
void WebotsKeyboardController::PopAWheelie() {
  bool usePreDockPose = !_shiftKeyPressed;
  SendPopAWheelie(-1, pathMotionProfile_, usePreDockPose, useApproachAngle,
                  approachAngle_rad);
}
...
void WebotsKeyboardController::RollObject() {
  ...
  SendRollSelectedObject(pathMotionProfile_, doDeepRoll, usePreDockPose,
                         useApproachAngle, approachAngle_rad);
}
...
void WebotsKeyboardController::SetControllerGains() {
  if (root_) {
    if (_shiftKeyPressed) {
      ...
      f32 steerDistOffsetCap =
          root_->getField("steerDistOffsetCap_mm")->getSFFloat();
      f32 steerAngOffsetCap =
          root_->getField("steerAngOffsetCap_rad")->getSFFloat();
      printf(
          "New steering gains: k1 %f, k2 %f, distOffsetCap %f, angOffsetCap "
          "%f\n",
          steer_k1, steer_k2, steerDistOffsetCap, steerAngOffsetCap);
      SendControllerGains(ControllerChannel::controller_steering, steer_k1,
                          steer_k2, steerDistOffsetCap, steerAngOffsetCap);
      ...
      printf("New pointTurn gains: kp=%f ki=%f kd=%f maxErrorSum=%f\n", kp, ki,
             kd, maxErrorSum);
      SendControllerGains(ControllerChannel::controller_pointTurn, kp, ki, kd,
                          maxErrorSum);
      ...
      SendControllerGains(ControllerChannel::controller_wheel, kp, ki, kd,
                          maxErrorSum);
      ...
      printf("New head gains: kp=%f ki=%f kd=%f maxErrorSum=%f\n", kp, ki, kd,
             maxErrorSum);
      SendControllerGains(ControllerChannel::controller_head, kp, ki, kd,
                          maxErrorSum);
      ...
      printf("New lift gains: kp=%f ki=%f kd=%f maxErrorSum=%f\n", kp, ki, kd,
             maxErrorSum);
      SendControllerGains(ControllerChannel::controller_lift, kp, ki, kd,
                          maxErrorSum);
    }
  } else {
...
void WebotsKeyboardController::SetActiveObjectLights() {
  if (_shiftKeyPressed && _altKeyPressed) {
    ...
    Json::Value jsonMsg;
    Json::Reader reader;
    std::string jsonFilename("../webotsCtrlGameEngine/SetBlockLights_" +
                             std::to_string(jsonMsgCtr++) + ".json");
    std::ifstream jsonFile(jsonFilename);

    if (jsonFile.fail()) {
      jsonMsgCtr = 0;
      jsonFilename = "../webotsCtrlGameEngine/SetBlockLights_" +
                     std::to_string(jsonMsgCtr++) + ".json";
    ...
    for (u32 iLED = 0; iLED < 4; ++iLED) {
      ...
      msg.transitionOnPeriod_ms[iLED] =
          jsonMsg["transitionOnPeriod_ms"][iLED].asUInt();
      msg.transitionOffPeriod_ms[iLED] =
          jsonMsg["transitionOffPeriod_ms"][iLED].asUInt();
    }

    ExternalInterface::MessageGameToEngine msgWrapper;
    msgWrapper.Set_SetAllActiveObjectLEDs(msg);
    SendMessage(msgWrapper);
  } else if (GetLastObservedObject().id >= 0 &&
             GetLastObservedObject().isActive) {
    // Proof of concept: cycle colors
    const s32 NUM_COLORS = 4;
    const ColorRGBA colorList[NUM_COLORS] = {
        ::Anki::NamedColors::RED, ::Anki::NamedColors::GREEN,
        ::Anki::NamedColors::BLUE, ::Anki::NamedColors::BLACK};
    ...
    if (_shiftKeyPressed) {
      ...
      printf("Turning edge %d new color %d (%x)\n", edgeIndex, colorIndex,
             u32(colorList[colorIndex]));
      ...
      printf("Cycling active block %d color from (%d,%d,%d) to (%d,%d,%d)\n",
             ...
             colorList[colorIndex].r(), colorList[colorIndex].g(),
...
void WebotsKeyboardController::AlignWithObject() {
  f32 distToMarker =
      root_->getField("alignWithObjectDistToMarker_mm")->getSFFloat();
  SendAlignWithObject(-1,  // tell game to use blockworld's "selected" object
                      distToMarker, pathMotionProfile_, true, useApproachAngle,
                      approachAngle_rad);
}
...
void WebotsKeyboardController::TurnTowardsObject() {
  ...
  msg.objectID =
      std::numeric_limits<u32>::max();  // HACK to tell game to use blockworld's
                                        // "selected" object
...
void WebotsKeyboardController::GotoObject() {
  SendGotoObject(-1,  // tell game to use blockworld's "selected" object
                 sqrtf(2.f) * 44.f, pathMotionProfile_);
}

void WebotsKeyboardController::RequestIMUData() { SendIMURequest(2000); }
...
void WebotsKeyboardController::AssociateNameWithCurrentFace() {
  ...
  SetFaceToEnroll setFaceToEnroll(userName, observedID, enrollToID,
                                  saveFaceToRobot, sayName, useMusic);
  SendMessage(MessageGameToEngine(std::move(setFaceToEnroll)));

  // todo: currently we send both the SetFaceToEnroll and the cloud intent for
  // meet victor. This will change, but since we don't know what SetFaceToEnroll
  // will be replaced by yet, both messages are being send for now. Eventually
  // there should be one "meet_victor" message and one "I'm changing the name,
  // but don't restart meet victor"
  const std::string json(
      "{\"intent\": \"intent_names_username_extend\", "
      "\"parameters\": \"{\\\"username\\\": \\\"" +
      userName + "\\\"}\" }");
  SendMessage(ExternalInterface::MessageGameToEngine(
      ExternalInterface::FakeCloudIntent(json)));
}
...
void WebotsKeyboardController::TurnTowardsFace() {
  ...
  if (faceID == 0) {
    ...
    SendMessage(
        ExternalInterface::MessageGameToEngine(std::move(turnTowardsFace)));
  }
}
...
void WebotsKeyboardController::EraseLastObservedFace() {
  using namespace ExternalInterface;
  SendMessage(
      MessageGameToEngine(EraseEnrolledFaceByID(GetLastObservedFaceID())));
}
...
#define GET_POINT_TURN_PARAMS()                                             \
  f32 pointTurnAngle =                                                      \
      std::fabs(root_->getField("pointTurnAngle_deg")->getSFFloat());       \
  f32 pointTurnSpeed =                                                      \
      std::fabs(root_->getField("pointTurnSpeed_degPerSec")->getSFFloat()); \
  f32 pointTurnAccel =                                                      \
      std::fabs(root_->getField("pointTurnAccel_degPerSec2")->getSFFloat());

void WebotsKeyboardController::TurnInPlaceCCW() {
  GET_POINT_TURN_PARAMS();
  if (_altKeyPressed) {
    SendTurnInPlaceAtSpeed(DEG_TO_RAD(pointTurnSpeed),
                           DEG_TO_RAD(pointTurnAccel));
  } else {
    SendTurnInPlace(DEG_TO_RAD(pointTurnAngle), DEG_TO_RAD(pointTurnSpeed),
                    DEG_TO_RAD(pointTurnAccel));
  }
}
...
void WebotsKeyboardController::SendRandomProceduralFace() {
  ...
  static_assert(std::tuple_size<decltype(faceParams.leftEye)>::value ==
                    (size_t)Param::NumParameters,
                "LeftEye parameter array is the wrong length");
  static_assert(std::tuple_size<decltype(faceParams.rightEye)>::value ==
                    (size_t)Param::NumParameters,
                "RightEye parameter array is the wrong length");

  Util::RandomGenerator rng;

  faceParams.leftEye[static_cast<s32>(Param::UpperInnerRadiusX)] =
      rng.RandDblInRange(0., 1.);
  faceParams.leftEye[static_cast<s32>(Param::UpperInnerRadiusY)] =
      rng.RandDblInRange(0., 1.);
  faceParams.leftEye[static_cast<s32>(Param::LowerInnerRadiusX)] =
      rng.RandDblInRange(0., 1.);
  faceParams.leftEye[static_cast<s32>(Param::LowerInnerRadiusY)] =
      rng.RandDblInRange(0., 1.);
  faceParams.leftEye[static_cast<s32>(Param::UpperOuterRadiusX)] =
      rng.RandDblInRange(0., 1.);
  faceParams.leftEye[static_cast<s32>(Param::UpperOuterRadiusY)] =
      rng.RandDblInRange(0., 1.);
  faceParams.leftEye[static_cast<s32>(Param::LowerOuterRadiusX)] =
      rng.RandDblInRange(0., 1.);
  faceParams.leftEye[static_cast<s32>(Param::LowerOuterRadiusY)] =
      rng.RandDblInRange(0., 1.);
  faceParams.leftEye[static_cast<s32>(Param::EyeCenterX)] =
      rng.RandIntInRange(-20, 20);
  faceParams.leftEye[static_cast<s32>(Param::EyeCenterY)] =
      rng.RandIntInRange(-20, 20);
  faceParams.leftEye[static_cast<s32>(Param::EyeScaleX)] =
      rng.RandDblInRange(0.8f, 1.2f);
  faceParams.leftEye[static_cast<s32>(Param::EyeScaleY)] =
      rng.RandDblInRange(0.8f, 1.2f);
  faceParams.leftEye[static_cast<s32>(Param::EyeAngle)] =
      0;  // rng.RandIntInRange(-10,10);
  faceParams.leftEye[static_cast<s32>(Param::LowerLidY)] =
      rng.RandDblInRange(0., .25);
  faceParams.leftEye[static_cast<s32>(Param::LowerLidAngle)] =
      rng.RandIntInRange(-20, 20);
  faceParams.leftEye[static_cast<s32>(Param::LowerLidBend)] =
      0;  // rng.RandDblInRange(0, 0.2);
  faceParams.leftEye[static_cast<s32>(Param::UpperLidY)] =
      rng.RandDblInRange(0., .25);
  faceParams.leftEye[static_cast<s32>(Param::UpperLidAngle)] =
      rng.RandIntInRange(-20, 20);
  faceParams.leftEye[static_cast<s32>(Param::UpperLidBend)] =
      0;  // rng.RandDblInRange(0, 0.2);
  faceParams.leftEye[static_cast<s32>(Param::Lightness)] =
      rng.RandDblInRange(0.5f, 1.f);
  faceParams.leftEye[static_cast<s32>(Param::Saturation)] =
      rng.RandDblInRange(0.5f, 1.f);
  faceParams.leftEye[static_cast<s32>(Param::GlowSize)] =
      rng.RandDblInRange(0.f, 1.f);
  faceParams.leftEye[static_cast<s32>(Param::HotSpotCenterX)] =
      rng.RandDblInRange(-0.8f, 0.8f);
  faceParams.leftEye[static_cast<s32>(Param::HotSpotCenterY)] =
      rng.RandDblInRange(-0.8f, 0.8f);

  faceParams.rightEye[static_cast<s32>(Param::UpperInnerRadiusX)] =
      rng.RandDblInRange(0., 1.);
  faceParams.rightEye[static_cast<s32>(Param::UpperInnerRadiusY)] =
      rng.RandDblInRange(0., 1.);
  faceParams.rightEye[static_cast<s32>(Param::LowerInnerRadiusX)] =
      rng.RandDblInRange(0., 1.);
  faceParams.rightEye[static_cast<s32>(Param::LowerInnerRadiusY)] =
      rng.RandDblInRange(0., 1.);
  faceParams.rightEye[static_cast<s32>(Param::UpperOuterRadiusX)] =
      rng.RandDblInRange(0., 1.);
  faceParams.rightEye[static_cast<s32>(Param::UpperOuterRadiusY)] =
      rng.RandDblInRange(0., 1.);
  faceParams.rightEye[static_cast<s32>(Param::LowerOuterRadiusX)] =
      rng.RandDblInRange(0., 1.);
  faceParams.rightEye[static_cast<s32>(Param::LowerOuterRadiusY)] =
      rng.RandDblInRange(0., 1.);
  faceParams.rightEye[static_cast<s32>(Param::EyeCenterX)] =
      rng.RandIntInRange(-20, 20);
  faceParams.rightEye[static_cast<s32>(Param::EyeCenterY)] =
      rng.RandIntInRange(-20, 20);
  faceParams.rightEye[static_cast<s32>(Param::EyeScaleX)] =
      rng.RandDblInRange(0.8, 1.2);
  faceParams.rightEye[static_cast<s32>(Param::EyeScaleY)] =
      rng.RandDblInRange(0.8, 1.2);
  faceParams.rightEye[static_cast<s32>(Param::EyeAngle)] =
      0;  // rng.RandIntInRange(-15,15);
  faceParams.rightEye[static_cast<s32>(Param::LowerLidY)] =
      rng.RandDblInRange(0., .25);
  faceParams.rightEye[static_cast<s32>(Param::LowerLidAngle)] =
      rng.RandIntInRange(-20, 20);
  faceParams.rightEye[static_cast<s32>(Param::LowerLidBend)] =
      rng.RandDblInRange(0., 0.2);
  faceParams.rightEye[static_cast<s32>(Param::UpperLidY)] =
      rng.RandDblInRange(0., .25);
  faceParams.rightEye[static_cast<s32>(Param::UpperLidAngle)] =
      rng.RandIntInRange(-20, 20);
  faceParams.rightEye[static_cast<s32>(Param::UpperLidBend)] =
      rng.RandDblInRange(0, 0.2);
  faceParams.rightEye[static_cast<s32>(Param::Lightness)] =
      rng.RandDblInRange(0.5f, 1.f);
  faceParams.rightEye[static_cast<s32>(Param::Saturation)] =
      rng.RandDblInRange(0.5f, 1.f);
  faceParams.rightEye[static_cast<s32>(Param::GlowSize)] =
      rng.RandDblInRange(0.f, 0.75f);
  faceParams.rightEye[static_cast<s32>(Param::HotSpotCenterX)] =
      rng.RandDblInRange(-0.8f, 0.8f);
  faceParams.rightEye[static_cast<s32>(Param::HotSpotCenterY)] =
      rng.RandDblInRange(-0.8f, 0.8f);
...
void WebotsKeyboardController::PlayAnimation() {
  ...
  if (!WebotsHelpers::GetFieldAsString(*root_, "animationToSendName",
                                       animToSendName)) {
    return;
  }
...
void WebotsKeyboardController::PlayAnimationTrigger() {
  // Send whatever animation trigger is specified in the animationToSendName
  // field
...
void WebotsKeyboardController::RunDebugConsoleFunc() {
  ...
  std::string funcArgs;
  if (!WebotsHelpers::GetFieldAsString(*root_, "consoleVarValue", funcArgs,
                                       false)) {
    return;
  }
  ...
  if (_altKeyPressed) {
    ...
    SendMessage(MessageGameToEngine(
        RunAnimDebugConsoleFuncMessage(funcName, funcArgs)));
  } else {
    // Normal: Send to Engine process
    SendMessage(
        MessageGameToEngine(RunDebugConsoleFuncMessage(funcName, funcArgs)));
  }
}
...
void WebotsKeyboardController::SetDebugConsoleVar() {
  ...
  if (_altKeyPressed) {
    // Alt: Send to Anim process
    SendMessage(
        MessageGameToEngine(SetAnimDebugConsoleVarMessage(varName, tryValue)));
  } else {
    // Normal: Send to Engine process
    SendMessage(
        MessageGameToEngine(SetDebugConsoleVarMessage(varName, tryValue)));
  }
}
...
void WebotsKeyboardController::PlayCubeAnimation() {
  if (_altKeyPressed) {
    // Send whatever cube animation trigger is specified in the
    // animationToSendName field
    ...
    if (!EnumFromString(cubeAnimTriggerStr, cubeAnimTrigger)) {
      LOG_ERROR(
          "WebotsKeyboardController.PlayCubeAnimation."
          "InvalidCubeAnimationTrigger",
...
void WebotsKeyboardController::TogglePowerMode() {
  ...
  LOG_INFO("WebotsKeyboardController.TogglePowerMode", "Calm: %d",
           enableCalmPower);
  using namespace ExternalInterface;
  SendMessage(MessageGameToEngine(RunDebugConsoleFuncMessage(
      "EnableCalmPowerMode", enableCalmPower ? "true" : "false")));
...
void WebotsKeyboardController::SetCameraSettings() {
  ...
  settings.enableAutoExposure =
      root_->getField("enableAutoExposure")->getSFBool();
  ExternalInterface::MessageGameToEngine message;
...
void WebotsKeyboardController::SayText() {
  ...
  // TODO: Add ability to set action style, voice style, duration scalar and
  // pitch from KB controller
  using AudioTtsProcessingStyle =
      Anki::AudioMetaData::SwitchState::Robot_Vic_External_Processing;
  sayTextMsg.voiceStyle =
      (_altKeyPressed ? AudioTtsProcessingStyle::Default_Processed
  ...
  printf("Saying '%s' in voice style '%s' w/ duration scalar %f\n",
         sayTextMsg.text.c_str(), EnumToString(sayTextMsg.voiceStyle),
...
void WebotsKeyboardController::QuitKeyboardController() { _shouldQuit = true; }
...
f32 WebotsKeyboardController::GetLiftSpeed_radps() {
  f32 liftSpeed =
      DEG_TO_RAD(root_->getField("liftSpeedDegPerSec")->getSFFloat());
  if (_shiftKeyPressed) {
    liftSpeed *= 0.5;
  }
  return liftSpeed;
...
f32 WebotsKeyboardController::GetHeadSpeed_radps() {
  f32 headSpeed =
      DEG_TO_RAD(root_->getField("headSpeedDegPerSec")->getSFFloat());
  if (_shiftKeyPressed) {
    headSpeed *= 0.5;
  }
  return headSpeed;
...
void WebotsKeyboardController::MoveLiftToLowDock() {
  SendMoveLiftToHeight(LIFT_HEIGHT_LOWDOCK, GetLiftSpeed_radps(),
                       GetLiftAccel_radps2(), GetLiftDuration_sec());
}

void WebotsKeyboardController::MoveLiftToHighDock() {
  SendMoveLiftToHeight(LIFT_HEIGHT_HIGHDOCK, GetLiftSpeed_radps(),
                       GetLiftAccel_radps2(), GetLiftDuration_sec());
}

void WebotsKeyboardController::MoveLiftToCarryHeight() {
  SendMoveLiftToHeight(LIFT_HEIGHT_CARRY, GetLiftSpeed_radps(),
                       GetLiftAccel_radps2(), GetLiftDuration_sec());
}

void WebotsKeyboardController::MoveLiftToAngle() {
  f32 targetAngle_rad =
      DEG_TO_RAD(root_->getField("liftTargetAngleDeg")->getSFFloat());
  SendMoveLiftToAngle(targetAngle_rad, GetLiftSpeed_radps(),
                      GetLiftAccel_radps2(), GetLiftDuration_sec());
}

void WebotsKeyboardController::MoveHeadToLowLimit() {
  SendMoveHeadToAngle(MIN_HEAD_ANGLE, GetHeadSpeed_radps(),
                      GetHeadAccel_radps2(), GetHeadDuration_sec());
}

void WebotsKeyboardController::MoveHeadToHorizontal() {
  SendMoveHeadToAngle(0, GetHeadSpeed_radps(), GetHeadAccel_radps2(),
                      GetHeadDuration_sec());
}

void WebotsKeyboardController::MoveHeadToHighLimit() {
  SendMoveHeadToAngle(MAX_HEAD_ANGLE, GetHeadSpeed_radps(),
                      GetHeadAccel_radps2(), GetHeadDuration_sec());
}
...
void WebotsKeyboardController::DriveForward() { ++_throttleDir; }

void WebotsKeyboardController::DriveBackward() { --_throttleDir; }

void WebotsKeyboardController::DriveLeft() { --_steeringDir; }

void WebotsKeyboardController::DriveRight() { ++_steeringDir; }
    ...
    if (_currKey >= '0' && _currKey <= '9') {
      ...
      switch (m) {
        case TestMode::TM_DIRECT_DRIVE:
          ...
          // p2: wheelPowerStepPercent (only applies if
          // DTF_ENABLE_DIRECT_HAL_TEST is set) p3: wheelSpeed_mmps (only
          // applies if DTF_ENABLE_DIRECT_HAL_TEST is not set)
        ...
        case TestMode::TM_LIFT:
          ...
          p2 = root_->getField("liftTest_nodCycleTimeMS")
                   ->getSFInt32();  // Nodding cycle time in ms (if
                                    // LiftTF_NODDING flag is set)
          p3 = root_->getField("liftTest_powerPercent")
                   ->getSFInt32();  // Power to run motor at. If 0, cycle
                                    // through increasing power. Only used
                                    // during LiftTF_TEST_POWER.
          break;
        case TestMode::TM_HEAD:
          p1 = root_->getField("headTest_flags")->getSFInt32();
          p2 = root_->getField("headTest_nodCycleTimeMS")
                   ->getSFInt32();  // Nodding cycle time in ms (if HTF_NODDING
                                    // flag is set)
          p3 = root_->getField("headTest_powerPercent")
                   ->getSFInt32();  // Power to run motor at. If 0, cycle
                                    // through increasing power. Only used
                                    // during HTF_TEST_POWER.
          break;
        ...
        case TestMode::TM_LIGHTS:
          ...
          // p2: The LED channel to activate (applies if LTF_CYCLE_ALL not
          // enabled) p3: The color to set it to (applies if LTF_CYCLE_ALL not
          // enabled)
...
void WebotsKeyboardController::CycleConnectionFlowState() {
  ...
  status++;
  if (status >=
      static_cast<u8>(SwitchboardInterface::ConnectionStatus::COUNT)) {
    status = 0;
  }
...
#define REGISTER_KEY_FCN(key, modifier, fcn, help_msg)                   \
  if (!RegisterKeyFcn(key, modifier,                                     \
                      std::bind(&WebotsKeyboardController::fcn, this),   \
                      help_msg)) {                                       \
    PRINT_NAMED_ERROR(                                                   \
        "WebotsKeyboardController.RegisterKeyFcn.DuplicateRegistration", \
        "Key: '%c' (0x%x), Modifier: 0x%x, Fcn: %s", key, key, modifier, \
        #fcn);                                                           \
  }
...
#define REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(key, modifier, fcn,       \
                                                   help_msg, display_string) \
  if (!RegisterKeyFcn(key, modifier,                                         \
                      std::bind(&WebotsKeyboardController::fcn, this),       \
                      help_msg, display_string)) {                           \
    PRINT_NAMED_ERROR(                                                       \
...
// Only MOD_NONE and MOD_ALT are valid modifiers since MOD_SHIFT is already
// implied.
#define REGISTER_SHIFTED_KEY_FCN(key, modifier, fcn, help_msg)                \
  if (modifier & MOD_SHIFT) {                                                 \
    PRINT_NAMED_ERROR(                                                        \
        "WebotsKeyboardController.RegisterKeyFcn.InvalidModifier",            \
        "Can't use shift modifier because it's already implied in key '%c' "  \
        "(0x%x)",                                                             \
        key, key);                                                            \
  } else if (!RegisterKeyFcn(key, modifier | MOD_SHIFT,                       \
                             std::bind(&WebotsKeyboardController::fcn, this), \
                             help_msg)) {                                     \
    PRINT_NAMED_ERROR(                                                        \
        "WebotsKeyboardController.RegisterKeyFcn.DuplicateRegistration",      \
...
WebotsKeyboardController::WebotsKeyboardController(s32 step_time_ms)
  ...
  REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(
      webots::Keyboard::UP, MOD_NONE, DriveForward, "Drive forward", "↑");
  REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(
      webots::Keyboard::UP, MOD_ALT, DriveForward,
      "Drive forward (turbo speed)", "↑");
  REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::UP, MOD_SHIFT,
                                             DriveForward,
                                             "Drive forward (half speed)", "↑");
  REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(
      webots::Keyboard::DOWN, MOD_NONE, DriveBackward, "Drive backward", "↓");
  REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(
      webots::Keyboard::DOWN, MOD_ALT, DriveBackward,
      "Drive backward (turbo speed)", "↓");
  REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(
      webots::Keyboard::DOWN, MOD_SHIFT, DriveBackward,
      "Drive backward (half speed)", "↓");
  REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::LEFT, MOD_NONE,
                                             DriveLeft, "Turn left", "←");
  REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::LEFT, MOD_ALT,
                                             DriveLeft,
                                             "Turn left (turbo speed)", "←");
  REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::LEFT, MOD_SHIFT,
                                             DriveLeft,
                                             "Turn left (half speed)", "←");
  REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::RIGHT, MOD_NONE,
                                             DriveRight, "Turn right", "→");
  REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::RIGHT, MOD_ALT,
                                             DriveRight,
                                             "Turn right (turbo speed)", "→");
  REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::RIGHT, MOD_SHIFT,
                                             DriveRight,
                                             "Turn right (half speed)", "→");
  //      REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::PAGEUP,
  //      MOD_NONE,      , "", "<PageUp>");
  //      REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::PAGEUP,
  //      MOD_ALT,       , "", "<PageUp>");
  //      REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::PAGEUP,
  //      MOD_SHIFT,     , "", "<PageUp>");
  //      REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::PAGEUP,
  //      MOD_ALT_SHIFT, , "", "<PageUp>");
  //      REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::PAGEDOWN,
  //      MOD_NONE,      , "", "<PageDown>");
  //      REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::PAGEDOWN,
  //      MOD_ALT,       , "", "<PageDown>");
  //      REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::PAGEDOWN,
  //      MOD_SHIFT,     , "", "<PageDown>");
  //      REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::PAGEDOWN,
  //      MOD_ALT_SHIFT, , "", "<PageDown>");
  REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::HOME, MOD_NONE,
                                             PressBackButton,
                                             "Press backpack button", "<Home>");
  REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(
      webots::Keyboard::HOME, MOD_ALT, TouchBackSensor,
      "Touch backpack touch sensor", "<Home>");
  //      REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::HOME,
  //      MOD_SHIFT,       , "", "<Home>");
  //      REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::HOME,
  //      MOD_ALT_SHIFT, , "", "<Home>");
  //      REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::END,
  //      MOD_NONE,      , "", "<End>");
  //      REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::END,
  //      MOD_ALT,       , "", "<End>");
  //      REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(webots::Keyboard::END,
  ...
  REGISTER_KEY_FCN('1', MOD_NONE, MoveLiftToLowDock,
                   "Move lift to low dock height");
  REGISTER_KEY_FCN('1', MOD_ALT, ExecuteRobotTestMode,
                   "Start robot test mode 1");
  REGISTER_KEY_FCN('1', MOD_ALT_SHIFT, ExecuteRobotTestMode,
                   "Start robot test mode 11");
  REGISTER_KEY_FCN('2', MOD_NONE, MoveLiftToHighDock,
                   "Move lift to high dock height");
  REGISTER_KEY_FCN('2', MOD_ALT, ExecuteRobotTestMode,
                   "Start robot test mode 2");
  REGISTER_KEY_FCN('2', MOD_ALT_SHIFT, ExecuteRobotTestMode,
                   "Start robot test mode 12");
  REGISTER_KEY_FCN('3', MOD_NONE, MoveLiftToCarryHeight,
                   "Move lift to carry height");
  REGISTER_KEY_FCN('3', MOD_ALT, ExecuteRobotTestMode,
                   "Start robot test mode 3");
  //      REGISTER_KEY_FCN('3', MOD_ALT_SHIFT, ,   "");
  REGISTER_KEY_FCN('4', MOD_NONE, MoveHeadToLowLimit,
                   "Move head all the way down");
  REGISTER_KEY_FCN('4', MOD_ALT, ExecuteRobotTestMode,
                   "Start robot test mode 4");
  //      REGISTER_KEY_FCN('4', MOD_ALT_SHIFT, ,   "");
  REGISTER_KEY_FCN('5', MOD_NONE, MoveHeadToHorizontal,
                   "Move head to 0 degrees");
  REGISTER_KEY_FCN('5', MOD_ALT, ExecuteRobotTestMode,
                   "Start robot test mode 5");
  REGISTER_KEY_FCN('6', MOD_NONE, MoveHeadToHighLimit,
                   "Move head all the way up");
  REGISTER_KEY_FCN('6', MOD_ALT, ExecuteRobotTestMode,
                   "Start robot test mode 6");
  REGISTER_KEY_FCN('7', MOD_NONE, MoveLiftToAngle,
                   "Move lift to targetAngle_deg");
  REGISTER_KEY_FCN('7', MOD_ALT, ExecuteRobotTestMode,
                   "Start robot test mode 7");
  //      REGISTER_KEY_FCN('8', MOD_NONE,      , "");
  REGISTER_KEY_FCN('8', MOD_ALT, ExecuteRobotTestMode,
                   "Start robot test mode 8");
  //      REGISTER_KEY_FCN('9', MOD_NONE,      , "");
  REGISTER_KEY_FCN('9', MOD_ALT, ExecuteRobotTestMode,
                   "Start robot test mode 9");
  REGISTER_KEY_FCN('0', MOD_NONE, TurnTowardsImagePoint,
                   "Turn towards last observed centroid");
  REGISTER_KEY_FCN('0', MOD_ALT, ExecuteRobotTestMode,
                   "Start robot test mode 0");
  REGISTER_KEY_FCN('0', MOD_ALT_SHIFT, ExecuteRobotTestMode,
                   "Start robot test mode 10");
  REGISTER_KEY_FCN('-', MOD_NONE, SayText,
                   "Say string 'sayString' unprocessed");
  REGISTER_KEY_FCN('-', MOD_ALT, SayText,
                   "Say string 'sayString' cozmo-processed");
  //      REGISTER_KEY_FCN('=', MOD_NONE,      , "");
  //      REGISTER_KEY_FCN('=', MOD_ALT,       , "");
  //      REGISTER_KEY_FCN('[', MOD_NONE,      , "");
  //      REGISTER_KEY_FCN('[', MOD_ALT,       , "");
  REGISTER_KEY_FCN(']', MOD_NONE, SetDebugConsoleVar,
                   "Set debug console variable in engine process");
  REGISTER_KEY_FCN(']', MOD_ALT, SetDebugConsoleVar,
                   "Set debug console variable in anim process");
  //      REGISTER_KEY_FCN('\\', MOD_NONE,      , "");
  //      REGISTER_KEY_FCN('\\', MOD_ALT,       , "");
  //      REGISTER_KEY_FCN(';', MOD_NONE,      , "");
  //      REGISTER_KEY_FCN(';', MOD_ALT,       , "");
  ...
  //      REGISTER_KEY_FCN('.', MOD_ALT,      , "");
  REGISTER_KEY_FCN('/', MOD_NONE, StartFreeplayMode,
                   "Start 'freeplay' mode (as if robot was shaken)");
  //      REGISTER_KEY_FCN('/', MOD_ALT,       , "");

  REGISTER_SHIFTED_KEY_FCN(
      '~', MOD_NONE, PlayAnimationTrigger,
      "Play animation trigger specified in 'animationToSendName'");
  REGISTER_SHIFTED_KEY_FCN(
      '~', MOD_ALT, PlayAnimationGroup,
      "Play animation group specified in 'animationToSendName'");
  //      REGISTER_SHIFTED_KEY_FCN('!', MOD_NONE, , "");
  //      REGISTER_SHIFTED_KEY_FCN('!', MOD_ALT, , "");
  //      REGISTER_SHIFTED_KEY_FCN('@', MOD_NONE, , "");
  REGISTER_SHIFTED_KEY_FCN('@', MOD_ALT, ExecutePlaypenTest,
                           "Execute playpen test");
  //      REGISTER_SHIFTED_KEY_FCN('#', MOD_NONE, ,                      "");
  //      REGISTER_SHIFTED_KEY_FCN('#', MOD_ALT, , "");
  REGISTER_SHIFTED_KEY_FCN('$', MOD_NONE, SendSaveCalibrationImage,
                           "Save calibration image");
  REGISTER_SHIFTED_KEY_FCN('$', MOD_ALT, SendClearCalibrationImages,
                           "Clear calibration images");
  REGISTER_SHIFTED_KEY_FCN(
      '%', MOD_NONE, SendComputeCameraCalibration,
      "Compute camera calibration from calibration images");
  //      REGISTER_SHIFTED_KEY_FCN('%', MOD_ALT, , "");
  REGISTER_SHIFTED_KEY_FCN(
      '^', MOD_NONE, PlayAnimation,
      "Plays animation specified in 'animationToSendName'");
  //      REGISTER_SHIFTED_KEY_FCN('^', MOD_ALT,  ,                 "");
  //      REGISTER_SHIFTED_KEY_FCN('&', MOD_NONE, , "");
  //      REGISTER_SHIFTED_KEY_FCN('&', MOD_ALT,  , "");
  REGISTER_SHIFTED_KEY_FCN('*', MOD_NONE, SendRandomProceduralFace,
                           "Draws random procedural face");
  REGISTER_SHIFTED_KEY_FCN('*', MOD_ALT, SetFaceDisplayHue,
                           "Sets face hue to 'faceHue'");
  //      REGISTER_SHIFTED_KEY_FCN('(', MOD_NONE, , "");
  //      REGISTER_SHIFTED_KEY_FCN('(', MOD_ALT,  , "");
  //      REGISTER_SHIFTED_KEY_FCN(')', MOD_NONE, , "");
  //      REGISTER_SHIFTED_KEY_FCN(')', MOD_ALT,  , "");
  REGISTER_SHIFTED_KEY_FCN('_', MOD_NONE, SetCameraSettings,
                           "Set camera settings");
  //      REGISTER_SHIFTED_KEY_FCN('_', MOD_ALT, , "");
  //      REGISTER_SHIFTED_KEY_FCN('+', MOD_NONE,  , "");
  REGISTER_SHIFTED_KEY_FCN('+', MOD_ALT, TogglePowerMode,
                           "Toggle (syscon) power mode");
  //      REGISTER_SHIFTED_KEY_FCN('{', MOD_NONE, , "");
  //      REGISTER_SHIFTED_KEY_FCN('{', MOD_ALT,  , "");
  REGISTER_SHIFTED_KEY_FCN(
      '}', MOD_NONE, RunDebugConsoleFunc,
      "Run debug console function with args in engine process");
  REGISTER_SHIFTED_KEY_FCN(
      '}', MOD_ALT, RunDebugConsoleFunc,
      "Run debug console function with args in anim process");
  //      REGISTER_SHIFTED_KEY_FCN('|', MOD_NONE, , "");
  //      REGISTER_SHIFTED_KEY_FCN('|', MOD_ALT, , "");
  REGISTER_SHIFTED_KEY_FCN(':', MOD_NONE, SetRollActionParams,
                           "Set parameters for roll action");
  //      REGISTER_SHIFTED_KEY_FCN(':', MOD_ALT, , "");
  REGISTER_SHIFTED_KEY_FCN('"', MOD_NONE, PlayCubeAnimation,
                           "Play 'Flash' cube animation on selected cube");
  REGISTER_SHIFTED_KEY_FCN('"', MOD_ALT, PlayCubeAnimation,
                           "Play cube animation trigger specified in "
                           "'animationToSendName' on selected cube");
  REGISTER_SHIFTED_KEY_FCN('<', MOD_NONE, TurnInPlaceCCW,
                           "Turn in place CCW by 'pointTurnAngle_deg'");
  REGISTER_SHIFTED_KEY_FCN('<', MOD_ALT, TurnInPlaceCCW,
                           "Turn in place CCW forever");
  REGISTER_SHIFTED_KEY_FCN('>', MOD_NONE, TurnInPlaceCW,
                           "Turn in place CW by 'pointTurnAngle_deg'");
  REGISTER_SHIFTED_KEY_FCN('>', MOD_ALT, TurnInPlaceCCW,
                           "Turn in place CW forever");
  ...
  //    REGISTER_KEY_FCN('A', MOD_ALT,       SendReadAnimationFile, "Re-load
  ...
  REGISTER_KEY_FCN('C', MOD_NONE, LogCliffSensorData,
                   "Request cliff sensor log");
  REGISTER_KEY_FCN('C', MOD_SHIFT, ExecuteBehavior,
                   "Execute behavior in 'behaviorName'");
  REGISTER_KEY_FCN('C', MOD_ALT, ToggleCameraCaptureFormat,
                   "Toggle camera capture format between RGB and YUV");
  ...
  REGISTER_KEY_FCN('D', MOD_ALT_SHIFT, SendForceDelocalize,
                   "Force robot delocalization");

  REGISTER_KEY_FCN('E', MOD_NONE, SaveSingleImage, "Save single image");
  REGISTER_KEY_FCN('E', MOD_SHIFT, ToggleImageSaving,
                   "Toggle image saving (in viz) mode");
  REGISTER_KEY_FCN('E', MOD_ALT_SHIFT, ToggleImageAndStateSaving,
                   "Toggle image and robot state saving (in viz) mode");

  REGISTER_KEY_FCN('F', MOD_NONE, ToggleFaceDetection, "Toggle face detection");
  REGISTER_KEY_FCN('F', MOD_SHIFT, AssociateNameWithCurrentFace,
                   "Assign 'userName' to current face");
  REGISTER_KEY_FCN('F', MOD_ALT, TurnTowardsFace,
                   "Turn towards face 'faceIDToTurnTowards' or last face if 0");
  REGISTER_KEY_FCN('F', MOD_ALT_SHIFT, EraseLastObservedFace,
                   "Erase last observed face");

  REGISTER_KEY_FCN('G', MOD_NONE, GotoPoseMarker,
                   "Goto/place object at pose marker");
  REGISTER_KEY_FCN('G', MOD_SHIFT, TogglePoseMarkerMode,
                   "Toggle pose marker mode");
  //      REGISTER_KEY_FCN('G', MOD_ALT,       , "");
  //      REGISTER_KEY_FCN('G', MOD_ALT_SHIFT, , "");

  REGISTER_KEY_FCN('H', MOD_NONE, FakeCloudIntent,
                   "Fake clound intent with contents of 'intent' (either a "
                   "name or valid json)");
  REGISTER_KEY_FCN('H', MOD_SHIFT, FakeUserIntent,
                   "Fake user intent with the contents of 'intent'");
  //      REGISTER_KEY_FCN('H', MOD_ALT,       , "");
  REGISTER_KEY_FCN('H', MOD_ALT_SHIFT, SendFakeTriggerWordDetect,
                   "Send fake trigger word detect");

  REGISTER_KEY_FCN('I', MOD_NONE, ToggleImageStreamingToGame,
                   "Toggle image streaming");
  //      REGISTER_KEY_FCN('I', MOD_SHIFT,     , "");
  //      REGISTER_KEY_FCN('I', MOD_ALT,       , "");
  //      REGISTER_KEY_FCN('I', MOD_ALT_SHIFT, , "");

  REGISTER_KEY_FCN('J', MOD_NONE, CycleConnectionFlowState,
                   "Cycle connection flow states");
  //      REGISTER_KEY_FCN('J', MOD_SHIFT,     , "");
  //      REGISTER_KEY_FCN('J', MOD_ALT,       , "");
  //      REGISTER_KEY_FCN('J', MOD_ALT_SHIFT, , "");

  REGISTER_KEY_FCN('K', MOD_NONE, SetControllerGains,
                   "Set wheel/head/lift gains");
  REGISTER_KEY_FCN('K', MOD_SHIFT, SetControllerGains,
                   "Set steering and point turn gains");
  // REGISTER_KEY_FCN('K', MOD_ALT,       , "");
  // REGISTER_KEY_FCN('K', MOD_ALT_SHIFT, , "");

  REGISTER_KEY_FCN('L', MOD_NONE, ToggleTestBackpackLights,
                   "Toggles a test pattern on backpack lights");
  REGISTER_KEY_FCN('L', MOD_SHIFT, SearchForNearbyObject,
                   "Search for nearby object");
  REGISTER_KEY_FCN('L', MOD_ALT, ToggleCliffSensorEnable,
                   "Toggles cliff sensor enable");
  REGISTER_KEY_FCN('L', MOD_ALT_SHIFT, ToggleEngineLightComponent,
                   "Toggle engine light component");

  REGISTER_KEY_FCN('M', MOD_NONE, SetEmotion,
                   "Set 'emotionName' to 'emotionVal'");
  REGISTER_KEY_FCN('M', MOD_SHIFT, TriggerEmotionEvent,
                   "Trigger 'emotionEvent'");
  ...
  REGISTER_KEY_FCN('O', MOD_SHIFT, TurnTowardsObject,
                   "Turn torwards selected object");
  REGISTER_KEY_FCN('O', MOD_ALT, GotoObject, "Go to selected object");
  REGISTER_KEY_FCN('O', MOD_ALT_SHIFT, AlignWithObject,
                   "Align with selected object");

  REGISTER_KEY_FCN('P', MOD_NONE, PickOrPlaceObject,
                   "Pickup or place on selected object from predock pose");
  REGISTER_KEY_FCN('P', MOD_SHIFT, PickOrPlaceObject,
                   "Pickup or place on selected object from current pose");
  REGISTER_KEY_FCN('P', MOD_ALT, PickOrPlaceObject,
                   "Pickup or place relative to selected object at offset "
                   "'placeOnGroundOffsetX_mm' from predock pose");
  REGISTER_KEY_FCN('P', MOD_ALT_SHIFT, PickOrPlaceObject,
                   "Pickup or place relative to selected object at offset "
                   "'placeOnGroundOffsetX_mm' from current pose");

  REGISTER_KEY_FCN('Q', MOD_NONE, SendAbortPath, "Cancel current path");
  REGISTER_KEY_FCN('Q', MOD_SHIFT, SendAbortAll,
                   "Cancel everything (paths, animations, docking, etc.)");
  REGISTER_KEY_FCN('Q', MOD_ALT, SendCancelAction, "Cancel current action");
  //      REGISTER_KEY_FCN('Q', MOD_ALT_SHIFT, , "");

  REGISTER_KEY_FCN('R', MOD_NONE, MountSelectedCharger,
                   "Dock to charger using cliff sensor correction");
  REGISTER_KEY_FCN('R', MOD_SHIFT, MountSelectedCharger,
                   "Dock to charger without using cliff sensor correction");
  REGISTER_KEY_FCN('R', MOD_ALT, FlipSelectedBlock, "Flips the selected cube");
  REGISTER_KEY_FCN('R', MOD_ALT_SHIFT, TeleportOntoCharger,
                   "Teleport the robot onto the charger");
  ...
  REGISTER_KEY_FCN(
      'S', MOD_ALT_SHIFT, DoCliffAlignToWhite,
      "If one front sensor is detecting white (> MIN_CLIFF_STOP_ON_WHITE_VAL) "
      "then rotate until other front sensor detects it as well.");
  ...
  REGISTER_KEY_FCN('U', MOD_NONE, RequestSingleImageToGame,
                   "Requests single image to game");
  REGISTER_KEY_FCN('U', MOD_SHIFT, ToggleImageStreamingToGame,
                   "Toggle image streaming to game mode");
  //      REGISTER_KEY_FCN('U', MOD_ALT,       , "");
  //      REGISTER_KEY_FCN('U', MOD_ALT_SHIFT, , "");

  REGISTER_KEY_FCN('V', MOD_NONE, SetRobotVolume,
                   "Set robot volume to 'robotVolume'");
  REGISTER_KEY_FCN('V', MOD_SHIFT, ToggleVisionWhileMoving,
                   "Toggle vision-while-moving enable");
  //      REGISTER_KEY_FCN('V', MOD_ALT,       , "");
  //      REGISTER_KEY_FCN('V', MOD_ALT_SHIFT, , "");

  REGISTER_KEY_FCN('W', MOD_NONE, RollObject,
                   "Roll selected object from predock pose");
  REGISTER_KEY_FCN('W', MOD_SHIFT, RollObject,
                   "Roll selected object without using predock pose");
  REGISTER_KEY_FCN('W', MOD_ALT, PopAWheelie,
                   "Pop-a-wheelie off of selected object from predock pose");
  REGISTER_KEY_FCN(
      'W', MOD_ALT_SHIFT, PopAWheelie,
      "Pop-a-wheelie off of selected object without using predock pose");
  ...
  REGISTER_KEY_FCN('X', MOD_ALT_SHIFT, QuitKeyboardController,
                   "Quit keyboard controller");

  // REGISTER_KEY_FCN('Y', MOD_NONE,      ToggleKeepFaceAliveEnable,     "Toggle
  // keep face alive enable"); // TODO:(bn) new way to do this
  ...
  REGISTER_KEY_FCN_WITH_SPECIAL_DISPLAY_CHAR(' ', MOD_NONE, SendStopAllMotors,
                                             "Stops all motors", "<Space>");
}

bool WebotsKeyboardController::RegisterKeyFcn(int key, int modifier,
                                              std::function<void()> fcn,
                                              const char* help_msg,
  ...
  auto it =
      find(_keyRegistrationOrder.begin(), _keyRegistrationOrder.end(), key);
...
void WebotsKeyboardController::ProcessKeyPressFunction(int key, int modifier) {
  ...
  PRINT_NAMED_WARNING(
      "WebotsKeyboardController.ProcessKeyPressFunction.KeyNotRegistered",
...
void WebotsKeyboardController::PrintHelp() {
  ...
  for (const auto key : _keyRegistrationOrder) {
    ...
    for (const auto& fcn_map : mod_map) {
      ...
      std::string keyComboStr = modifierString + std::string("'") +
                                fcn_map.second.displayString.c_str() +
                                std::string("'");
...
void WebotsKeyboardController::ProcessKeystroke() {
  ...
  for (auto key : keysPressed) {
    ...
    */
    ...
    const f32 dockAccel_mmps2 =
        root_->getField("dockAccel_mmps2")->getSFFloat();
    const f32 dockDecel_mmps2 =
        root_->getField("dockDecel_mmps2")->getSFFloat();

    // Path speeds
    const f32 pathSpeed_mmps = root_->getField("pathSpeed_mmps")->getSFFloat();
    const f32 pathAccel_mmps2 =
        root_->getField("pathAccel_mmps2")->getSFFloat();
    const f32 pathDecel_mmps2 =
        root_->getField("pathDecel_mmps2")->getSFFloat();
    const f32 pathPointTurnSpeed_radPerSec =
        root_->getField("pathPointTurnSpeed_radPerSec")->getSFFloat();
    const f32 pathPointTurnAccel_radPerSec2 =
        root_->getField("pathPointTurnAccel_radPerSec2")->getSFFloat();
    const f32 pathPointTurnDecel_radPerSec2 =
        root_->getField("pathPointTurnDecel_radPerSec2")->getSFFloat();
    const f32 pathReverseSpeed_mmps =
        root_->getField("pathReverseSpeed_mmps")->getSFFloat();

    // If any of the pathMotionProfile fields are different than the default
    // values use a custom profile
    if (pathMotionProfile_.speed_mmps != pathSpeed_mmps ||
        ...
        pathMotionProfile_.pointTurnSpeed_rad_per_sec !=
            pathPointTurnSpeed_radPerSec ||
        pathMotionProfile_.pointTurnAccel_rad_per_sec2 !=
            pathPointTurnAccel_radPerSec2 ||
        pathMotionProfile_.pointTurnDecel_rad_per_sec2 !=
            pathPointTurnDecel_radPerSec2 ||
    ...
    approachAngle_rad =
        DEG_TO_RAD(root_->getField("approachAngle_deg")->getSFFloat());
    ...
    WebotsHelpers::GetFieldAsString(*root_, "drivingStartAnim",
                                    drivingStartAnim, failOnEmptyString);
    WebotsHelpers::GetFieldAsString(*root_, "drivingLoopAnim", drivingLoopAnim,
                                    failOnEmptyString);
    WebotsHelpers::GetFieldAsString(*root_, "drivingEndAnim", drivingEndAnim,
                                    failOnEmptyString);
...
void WebotsKeyboardController::TestLightCube() {
  static std::vector<ColorRGBA> colors = {
      {::Anki::NamedColors::RED, ::Anki::NamedColors::GREEN,
       ::Anki::NamedColors::BLUE, ::Anki::NamedColors::CYAN,
       ::Anki::NamedColors::ORANGE, ::Anki::NamedColors::YELLOW}};
  static std::vector<WhichCubeLEDs> leds = {
      {WhichCubeLEDs::BACK, WhichCubeLEDs::LEFT, WhichCubeLEDs::FRONT,
       WhichCubeLEDs::RIGHT}};
...
void WebotsKeyboardController::ToggleCameraCaptureFormat() {
  ...
  LOG_INFO("ToggleCameraCaptureFormat", "Switching to %s", yuv ? "YUV" : "RGB");
  msg.format =
      (yuv ? Vision::ImageEncoding::YUV420sp : Vision::ImageEncoding::RawRGB);
  yuv = !yuv;
...
void WebotsKeyboardController::HandleRobotConnected(
...
int main(int argc, char** argv) {
  ...
  const Anki::Util::Data::DataPlatform& dataPlatform =
      WebotsCtrlShared::CreateDataPlatformBS(argv[0], "webotsCtrlKeyboard");
  // initialize logger
