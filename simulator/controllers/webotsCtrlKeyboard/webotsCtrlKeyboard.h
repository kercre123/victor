...
class WebotsKeyboardController : public UiGameController {
 public:
  ...
  // Called before WaitOnKeyboardToConnect (and also before Init), sets up the
  // basics needed for WaitOnKeyboardToConnect, including enabling the keyboard
  void PreInit();

  // if the corresponding proto field is set to true, this function will wait
  // until the user presses Shift+enter to return.This can be used to allow
  // unity to connect. If we notice another connection attempt, we will exit the
  // keyboard controller. This is called before Init
  void WaitOnKeyboardToConnect();

 protected:
  bool RegisterKeyFcn(int key, int modifier, std::function<void()> fcn,
                      const char* help_msg, std::string display_string = "");
  ...
  virtual void HandleRobotObservedObject(
      const ExternalInterface::RobotObservedObject& msg) override;
  virtual void HandleRobotObservedFace(
      const ExternalInterface::RobotObservedFace& msg) override;
  virtual void HandleRobotObservedPet(
      const ExternalInterface::RobotObservedPet& msg) override;
  virtual void HandleLoadedKnownFace(
      const Vision::LoadedKnownFace& msg) override;
  virtual void HandleEngineErrorCode(
      const ExternalInterface::EngineErrorCodeMessage& msg) override;
  virtual void HandleRobotConnected(
      const ExternalInterface::RobotConnectionResponse& msg) override;
