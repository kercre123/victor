...
const u32 kObjectAvailableMessagePeriod_cycles =
    kObjectAvailableMessagePeriod_ms / CUBE_TIME_STEP_MS;

// Length of time in between transmission of battery voltage messages
const u32 kBatteryVoltageMessagePeriod_ms = 1000;
const u32 kBatteryVoltageMessagePeriod_cycles =
    kBatteryVoltageMessagePeriod_ms / CUBE_TIME_STEP_MS;

// To convert between battery voltage and the cube firmware's raw ADC counts
// (used to simulate how the physical cube sends battery voltage to engine). The
// raw ADC value follows the equation: actualVolts = railVoltageCnts * 3.6 /
// 1024
const float kBatteryVoltsToRawCnts = 1024.f / 3.6f;

constexpr int kNumCubeLeds =
    Util::EnumToUnderlying(CubeConstants::NUM_CUBE_LEDS);
...
// Pointer to webots field which contains the current battery voltage of the
// cube (this is to be able to simulate a low cube battery condition)
...
void SendMessageHelper(webots::Emitter* emitter, T&& msg) {
  ...
  // Construct a MessageCubeToEngine union from the passed-in msg (uses
  // MessageCubeToEngine specialized rvalue constructors)
...
inline void SetLED_helper(u32 index, u32 rgbColor) {
  DEV_ASSERT((rgbColor & 0xFF000000) == 0,
             "ActiveBlock.SetLedHelper.InvalidRgbColor");
  ...
  // Store the RGB value, then only send it to Webots once per time step (in
  // Update())
...
Result Init() {
  ...
  if (nullptr == typeField) {
    PRINT_NAMED_ERROR("ActiveBlock.Init.NoObjectType",
                      "Failed to find lightCubeType");
    return RESULT_FAIL;
  }
  ...
  if (factoryID_.empty()) {
    ...
    for (int i = 0; i < 6; i++) {
      const int rand =
          randGen_.RandIntInRange(0, std::numeric_limits<uint8_t>::max());
      ss << std::setw(2) << std::setfill('0') << std::hex << (int)rand << ":";
    }
    factoryID_ = ss.str();
    factoryID_.pop_back();  // pop off the trailing ":"
  }
  PRINT_NAMED_INFO("ActiveBlock", "Starting active object (factoryID %s)",
                   factoryID_.c_str());
  ...
  const int emitterChannel =
      (int)(std::hash<std::string>{}(factoryID_)&0x3FFFFFFF);
  ...
  if (accelBufferSize_ == MAX_ACCEL_BUFFER_SIZE) {
    ...
    for (u8 axis = 0; axis < 3 && !tapDetected; ++axis) {
      ...
      for (u32 i = 1; i < MAX_ACCEL_BUFFER_SIZE; ++i) {
        ...
        f32 currAccelFiltVal =
            alpha * (prevAccelFiltVal + accelBuffer_[axis][idx] - prevAccelVal);
        prevAccelVal = accelBuffer_[axis][idx];
        prevAccelFiltVal = currAccelFiltVal;

        if (currAccelFiltVal > TAP_DETECT_THRESH) {
          PRINT_NAMED_INFO("ActiveBlock", "TapDetected: axis %d, val %f", axis,
                           currAccelFiltVal);
          tapDetected = true;

          // Fast forward in buffer so that we don't allow tap detection again
          // until TAP_DETECT_WINDOW_MS later.
          u32 idxOffset = i + (TAP_DETECT_WINDOW_MS / CUBE_TIME_STEP_MS);
          accelBufferStartIdx_ =
              (accelBufferStartIdx_ + idxOffset) % MAX_ACCEL_BUFFER_SIZE;
...
Result Update() {
  if (active_object_controller.step(CUBE_TIME_STEP_MS) != -1) {
    ...
    static u32 objAvailableSendCtr =
        (u32)randGen_.RandIntInRange(0, kObjectAvailableMessagePeriod_cycles);
    if (objAvailableSendCtr-- == 0) {
      SendMessageHelper(discoveryEmitter_, ExternalInterface::ObjectAvailable(
                                               factoryID_, objectType_, 0));
    ...
    if (batteryVoltageSendCtr-- == 0) {
      ...
      CubeVoltageData msg;
      msg.railVoltageCnts = static_cast<decltype(msg.railVoltageCnts)>(
          batteryVolts * kBatteryVoltsToRawCnts);
    ...
    // For cube accel message, cube firmware buffers ACCEL_FRAMES_PER_MSG of
    // them before sending (due to BLE message rate limits)
    ...
    for (int i = 0; i < 3; i++) {
      ...
      const double scaledAccelValue =
          accel_g * std::numeric_limits<int16_t>::max() / 4.0;
      thisRawAccel.accel[i] =
          Util::numeric_cast_clamped<int16_t>(scaledAccelValue);
    }
    ...
    // Set any pending LED color fields. This must be done here since setMFVec3f
    // can only be called once per simulation time step for a given field (known
    // Webots R2018a bug)
    // TODO: Remove?
