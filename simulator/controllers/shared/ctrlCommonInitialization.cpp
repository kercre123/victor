 ...
 * Date:  07/08/16
 *
 * Description: A few functions that all webots controllers share to initialize.
 *Originally created to refactor the way we set filters for logging.
 *
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Anki::Util::Data::DataPlatform CreateDataPlatformBS(
    const std::string& runningPath, const std::string& platformID) {
  ...
  const std::string outputPath =
      path + kRootDirectory + kBuildPath + platformID;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Anki::Util::Data::DataPlatform CreateDataPlatformTest(
    const std::string& runningPath, const std::string& platformID) {
#if defined(_WIN32) || defined(WIN32)
  size_t pos = runningPath.rfind('\\');
#else
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AutoGlobalLogger::Initialize(
    Util::IFormattedLoggerProvider* loggerProvider,
    const Util::Data::DataPlatform& dataPlatform, bool loadLoggerFilter) {
  ...
  if (loadLoggerFilter) {
    ...
    if (!dataPlatform.readAsJson(Anki::Util::Data::Scope::Resources,
                                 ...
                                 consoleFilterConfig)) {
      PRINT_NAMED_ERROR("AutoGlobalLogger.Initialize",
    ...
    PRINT_CH_INFO("LOG", "AutoGlobalLogger.Initialize",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
DefaultAutoGlobalLogger::DefaultAutoGlobalLogger(
    const Util::Data::DataPlatform& dataPlatform, bool loadLoggerFilter,
    bool colorizeStderrOutput)
    : AutoGlobalLogger(new Util::PrintfLoggerProvider(), dataPlatform,
                       loadLoggerFilter) {
  // assert in case the logger created (passed to base constructor) is not a
  // printfLogger, since we do a static_cast instead of dynamic_cast in shipping
  // for performance.
  DEV_ASSERT(dynamic_cast<Util::PrintfLoggerProvider*>(_provider),
             "DefaultAutoGlobalLogger.DefaultLoggerIsNotPrintf");
  Util::PrintfLoggerProvider* provider =
      static_cast<Util::PrintfLoggerProvider*>(_provider);
