...
std::vector<RootNodeInfo> GetAllSceneTreeNodes(
    const webots::Supervisor& supervisor) {
  const auto* rootNode = supervisor.getRoot();
  DEV_ASSERT(rootNode != nullptr,
             "WebotsHelpers.GetAllSceneTreeNodes.NullSupervisorRoot");

  const auto* rootChildren = rootNode->getField("children");
  DEV_ASSERT(rootChildren != nullptr,
             "WebotsHelpers.GetAllSceneTreeNodes.NullRootChildren");
...
std::vector<RootNodeInfo> GetMatchingSceneTreeNodes(
    const webots::Supervisor& supervisor, const std::string& typeNameToMatch) {
...
bool GetFieldAsString(const webots::Node& parentNode,
                      const std::string& fieldName, std::string& outputStr,
  ...
  if (field == nullptr) {
    PRINT_NAMED_ERROR("WebotsHelpers.GetFieldAsString.NullField",
                      ...
                      fieldName.c_str(), parentNode.getTypeName().c_str());
    return false;
    ...
    PRINT_NAMED_ERROR("WebotsHelpers.GetFieldAsString.WrongFieldType",
                      ...
                      field->getTypeName().c_str(), fieldName.c_str());
    return false;
  }
  ...
  if (failOnEmptyString && outputStr.empty()) {
    PRINT_NAMED_WARNING("WebotsHelpers.GetFieldAsString.EmptyString",
                        "Empty string for field name %s", fieldName.c_str());
    return false;
  }
...
int AddSceneTreeNode(const webots::Supervisor& supervisor,
  ...
  const auto* rootNode = supervisor.getRoot();
  DEV_ASSERT(rootNode != nullptr,
             "WebotsHelpers.AddSceneTreeNode.NullSupervisorRoot");

  auto* rootChildren = rootNode->getField("children");
  DEV_ASSERT(rootChildren != nullptr,
             "WebotsHelpers.AddSceneTreeNode.NullRootChildren");
  ...
  DEV_ASSERT(newNode != nullptr,
             "WebotsHelpers.AddSceneTreeNode.FailedToAddNode");

  const int newNodeId = newNode->getId();
  return newNodeId;
}

void GetWebotsTranslation(const Pose3d& poseIn, double* webotsTranslationOut,
...
void GetWebotsRotation(const Pose3d& poseIn, double* webotsRotationOut) {
...
Pose3d ConvertTranslationRotationToPose(const double* transIn,
  ...
  Pose3d outPose(rotIn[3], Vec3f(rotIn[0], rotIn[1], rotIn[2]),
