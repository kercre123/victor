 ...
 * Date:   1/29/2018
 *
 * Description: Implements Webots-specific INeuralNetMain interface and main()
 *to create the WebotsCtrlNeuralNets "controller" for use in the simulator.
 *
...
class WebotsCtrlNeuralNet : public NeuralNets::INeuralNetMain {
 ...
 protected:
  virtual bool ShouldShutdown() override { return _shouldStop; }

  virtual Util::ILoggerProvider* GetLoggerProvider() override {
    const bool colorizeStderrOutput =
        false;  // TODO: Get from Webots proto in simulation?
    _logger.reset(new Util::PrintfLoggerProvider(Util::LOG_LEVEL_DEBUG,
                                                 colorizeStderrOutput));
    return _logger.get();
  }

  virtual int GetPollPeriod_ms(const Json::Value& config) const override {
    return _webotsSupervisor.getSelf()
        ->getField("pollingPeriod_ms")
        ->getSFInt32();
  }
...
int main(int argc, char** argv) {
  if (argc < 3) {
    ...
    std::cout << std::endl
              << "Usage: " << argv[0]
              << " <configFile>.json modelPath cachePath" << std::endl;
    std::cout << std::endl
              << " Will poll cachePath for neuralNetImage.png to process"
              << std::endl;
    return -1;
  }
