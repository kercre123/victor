...
void PhysicsController::Init() {
  ...
  Subscribe(PhysicsInterface::MessageSimPhysicsTag::ApplyForce,
            std::bind(&PhysicsController::ProcessApplyForceMessage, this,
                      std::placeholders::_1));

  _server.StopListening();
  _server.StartListening(
      (uint16_t)VizConstants::WEBOTS_PHYSICS_CONTROLLER_PORT);
}
...
void PhysicsController::Update() {
  ...
  while ((numBytesRecvd = _server.Recv((char*)data, maxPacketSize)) > 0) {
    ProcessMessage(
        PhysicsInterface::MessageSimPhysics(data, (size_t)numBytesRecvd));
  }
}

void PhysicsController::Cleanup() { _server.StopListening(); }

void PhysicsController::ProcessMessage(
    PhysicsInterface::MessageSimPhysics&& message) {
  dWebotsConsolePrintf(
      "Processing msgs from game controller: Got msg %s\n",
  ...
  _eventManager.Broadcast(
      AnkiEvent<PhysicsInterface::MessageSimPhysics>(type, std::move(message)));
}

void PhysicsController::ProcessApplyForceMessage(
  ...
  dBodyAddForce(GetdBodyID(payload.DefName), payload.xForce, payload.yForce,
                payload.zForce);
}
...
const dBodyID PhysicsController::GetdBodyID(const std::string& objectName) {
  if (_dBodyIDMap.find(objectName) == _dBodyIDMap.end()) {
    ...
    if (body == NULL) {
      dWebotsConsolePrintf(
          "ERROR: ODE couldn't find the body with name `%s`. There is no Solid "
          "(or derived) node with"
          "the specified DEF name or the physics field of the Solid node is "
          "undefined",
          objectName.c_str());
