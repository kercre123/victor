...
class PhysVizController {
 ...
 private:
  ...
  void DrawRamp(float platformLength, float slopeLength, float width,
                float height);
  void DrawHead(float width, float height, float depth);
  void DrawTetrahedronMarker(const float x, const float y, const float z,
                             const float length_x, const float length_y,
                             const float length_z);
  void DrawRobot(Anki::Vector::VizRobotMarkerType type);
  void DrawPredockPose();
  void DrawQuad(const float xUpperLeft, const float yUpperLeft,
                const float zUpperLeft, const float xLowerLeft,
                const float yLowerLeft, const float zLowerLeft,
                const float xUpperRight, const float yUpperRight,
                const float zUpperRight, const float xLowerRight,
                const float yLowerRight, const float zLowerRight);
  void DrawQuadFill(const float xUpperLeft, const float yUpperLeft,
                    const float zUpperLeft, const float xLowerLeft,
                    const float yLowerLeft, const float zLowerLeft,
                    const float xUpperRight, const float yUpperRight,
                    const float zUpperRight, const float xLowerRight,
  ...
  std::unordered_map<uint32_t,
                     std::unordered_map<uint32_t, VizInterface::Quad> >
      _quadMap;

  struct Segment {
    Segment() : color(0) {}
    Segment(uint32_t c, const std::array<float, 3>& o,
