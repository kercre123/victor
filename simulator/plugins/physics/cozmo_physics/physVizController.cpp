...
void PhysVizController::Init() {
  ...
  Subscribe(VizInterface::MessageVizTag::Object,
            std::bind(&PhysVizController::ProcessVizObjectMessage, this,
                      std::placeholders::_1));
  Subscribe(VizInterface::MessageVizTag::Quad,
            std::bind(&PhysVizController::ProcessVizQuadMessage, this,
                      std::placeholders::_1));
  Subscribe(VizInterface::MessageVizTag::EraseObject,
            std::bind(&PhysVizController::ProcessVizEraseObjectMessage, this,
                      std::placeholders::_1));
  Subscribe(VizInterface::MessageVizTag::EraseQuad,
            std::bind(&PhysVizController::ProcessVizEraseQuadMessage, this,
                      std::placeholders::_1));
  Subscribe(
      VizInterface::MessageVizTag::AppendPathSegmentLine,
      std::bind(&PhysVizController::ProcessVizAppendPathSegmentLineMessage,
                this, std::placeholders::_1));
  Subscribe(VizInterface::MessageVizTag::AppendPathSegmentArc,
            std::bind(&PhysVizController::ProcessVizAppendPathSegmentArcMessage,
                      this, std::placeholders::_1));
  Subscribe(VizInterface::MessageVizTag::SetPathColor,
            std::bind(&PhysVizController::ProcessVizSetPathColorMessage, this,
                      std::placeholders::_1));
  Subscribe(VizInterface::MessageVizTag::ErasePath,
            std::bind(&PhysVizController::ProcessVizErasePathMessage, this,
                      std::placeholders::_1));
  Subscribe(VizInterface::MessageVizTag::ShowObjects,
            std::bind(&PhysVizController::ProcessVizShowObjectsMessage, this,
                      std::placeholders::_1));
  Subscribe(VizInterface::MessageVizTag::SetVizOrigin,
            std::bind(&PhysVizController::ProcessVizSetOriginMessage, this,
                      std::placeholders::_1));

  // line segments
  Subscribe(VizInterface::MessageVizTag::LineSegment,
            std::bind(&PhysVizController::ProcessVizLineSegmentMessage, this,
                      std::placeholders::_1));
  Subscribe(VizInterface::MessageVizTag::EraseLineSegments,
            std::bind(&PhysVizController::ProcessVizEraseLineSegmentsMessage,
                      this, std::placeholders::_1));
...
void PhysVizController::Draw(int pass, const char* view) {
  ...
  if (pass == 1 && view == NULL) {
    ...
    // Set default color
    glColor4ub(
        ::Anki::NamedColors::DEFAULT.r(), ::Anki::NamedColors::DEFAULT.g(),
        ::Anki::NamedColors::DEFAULT.b(), ::Anki::NamedColors::DEFAULT.alpha());

    // Set global offset
    glPushMatrix();
    glTranslatef(_globalTranslation[0], _globalTranslation[1],
                 _globalTranslation[2]);
    glRotatef(_globalRotation[0], _globalRotation[1], _globalRotation[2],
              _globalRotation[3]);

    // Draw path
    for (auto pathMapIt = _pathMap.begin(); pathMapIt != _pathMap.end();
         pathMapIt++) {
      ...
      if (_pathColorMap.find(pathID) != _pathColorMap.end()) {
        ...
        glColor4ub(pathColor.r(), pathColor.g(), pathColor.b(),
                   pathColor.alpha());
      }

      // Draw the path
      glBegin(GL_LINE_STRIP);
      for (auto pathIt = pathMapIt->second.begin();
           pathIt != pathMapIt->second.end(); pathIt++) {
    ...
    for (auto objIt = _objectMap.begin(); objIt != _objectMap.end(); ++objIt) {
      ...
      glRotatef(obj->rot_deg, obj->rot_axis_x, obj->rot_axis_y,
                obj->rot_axis_z);
      ...
      switch (obj->objectTypeID) {
        ...
        case VizObjectType::VIZ_OBJECT_CUBOID: {
          ...
          // Object ID label
          std::string idString = std::to_string(
              obj->objectID - Anki::Vector::VizObjectBaseID[(
                                  int)VizObjectType::VIZ_OBJECT_CUBOID]);
          DrawTextAtOffset(idString, 0.6f * obj->x_size_m, 0.6f * obj->y_size_m,
                           0.6f * obj->z_size_m);
          DrawTextAtOffset(idString, -0.6f * obj->x_size_m,
                           -0.6f * obj->y_size_m, -0.6f * obj->z_size_m);
        ...
        case VizObjectType::VIZ_OBJECT_CHARGER: {
          ...
          std::string idString = std::to_string(
              obj->objectID - Anki::Vector::VizObjectBaseID[(
                                  int)VizObjectType::VIZ_OBJECT_CHARGER]);
          DrawTextAtOffset(idString, 0, 0.6f * obj->y_size_m,
                           0.6f * obj->z_size_m);
    ...
    for (const auto& quadsByType : _quadMap) {
      for (const auto& quadsByID : quadsByType.second) {
        ...
        glColor4ub(quadColor.r(), quadColor.g(), quadColor.b(),
                   quadColor.alpha());
    ...
    for (const auto& segmentVectorPerIdentifier : _lineSegments) {
      for (const auto& segmentInVector : segmentVectorPerIdentifier.second) {
        ...
        glColor4ub(segmentColor.r(), segmentColor.g(), segmentColor.b(),
                   segmentColor.alpha());

        // draw segment
        glVertex3f(segmentInVector.origin[0], segmentInVector.origin[1],
                   segmentInVector.origin[2]);
        glVertex3f(segmentInVector.dest[0], segmentInVector.dest[1],
                   segmentInVector.dest[2]);
      }
    }
    glEnd();
...
void PhysVizController::ProcessVizObjectMessage(
  ...
  PRINT("Processing DrawObject %d %d (%f %f %f) (%f %f %f %f) %d \n",
        payload.objectID, payload.objectTypeID, payload.x_trans_m,
        payload.y_trans_m, payload.z_trans_m, payload.rot_deg,
        payload.rot_axis_x, payload.rot_axis_y, payload.rot_axis_z,
        payload.color);

  _objectMap[payload.objectID] = VizInterface::Object(payload);
}

void PhysVizController::ProcessVizLineSegmentMessage(
    const AnkiEvent<VizInterface::MessageViz>& msg) {
  ...
  _lineSegments[payload.identifier].emplace_back(payload.color, payload.origin,
                                                 payload.dest);
...
void PhysVizController::ProcessVizQuadMessage(
    const AnkiEvent<VizInterface::MessageViz>& msg) {
...
void PhysVizController::ProcessVizEraseObjectMessage(
    const AnkiEvent<VizInterface::MessageViz>& msg) {
  ...
  if (payload.objectID == (uint32_t)VizConstants::ALL_OBJECT_IDs) {
    ...
    if (lowerIt == _objectMap.end()) return;
...
void PhysVizController::ProcessVizEraseLineSegmentsMessage(
    const AnkiEvent<VizInterface::MessageViz>& msg) {
...
void PhysVizController::ProcessVizEraseQuadMessage(
    const AnkiEvent<VizInterface::MessageViz>& msg) {
...
void PhysVizController::ProcessVizAppendPathSegmentLineMessage(
    const AnkiEvent<VizInterface::MessageViz>& msg) {
  const auto& payload = msg.GetData().Get_AppendPathSegmentLine();
  PRINT("Processing AppendLine\n");

  PathPoint startPt(payload.x_start_m, payload.y_start_m, payload.z_start_m,
                    true);
...
void PhysVizController::ProcessVizAppendPathSegmentArcMessage(
    const AnkiEvent<VizInterface::MessageViz>& msg) {
...
void PhysVizController::ProcessVizSetPathColorMessage(
    const AnkiEvent<VizInterface::MessageViz>& msg) {
...
void PhysVizController::ProcessVizErasePathMessage(
    const AnkiEvent<VizInterface::MessageViz>& msg) {
...
void PhysVizController::ProcessVizShowObjectsMessage(
    const AnkiEvent<VizInterface::MessageViz>& msg) {
...
void PhysVizController::ProcessVizSetOriginMessage(
    const AnkiEvent<VizInterface::MessageViz>& msg) {
  const auto& payload = msg.GetData().Get_SetVizOrigin();

  _globalRotation[0] = RAD_TO_DEG(
      payload.rot_rad);  // Note that global rotation angle is in degrees!
  ...
  PRINT(
      "Processing SetVizOrigin: %.1fdeg @(%.1f %.1f %.1f), (%.1f %.1f %.1f)\n",
      _globalRotation[0], _globalRotation[1], _globalRotation[2],
      _globalRotation[3], _globalTranslation[0], _globalTranslation[1],
      _globalTranslation[2]);
}

void PhysVizController::DrawTextAtOffset(std::string s, float x_off,
...
void PhysVizController::DrawRamp(float platformLength, float slopeLength,
...
void PhysVizController::DrawTetrahedronMarker(const float x, const float y,
                                              const float z,
...
void PhysVizController::DrawRobot(Anki::Vector::VizRobotMarkerType type) {
  ...
  switch (type) {
    ...
    case VizRobotMarkerType::VIZ_ROBOT_MARKER_BIG_TRIANGLE:
      x += 0.03f;  // Move tip of marker to come forward, roughly up to lift
                   // position.
      l = 0.062f;
      w = 0.08f;
      h = 0.01f;
...
void PhysVizController::DrawQuad(
