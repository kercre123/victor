...
static_assert(Anki::Util::DAS::FIELD_COUNT == 9,
              "DAS field count does not match declarations");
...
// Optional numeric fields are parsed as strings because they may be omitted
// from the log record. See also: util/logging/victorLogger_vicos.cpp
//
// static constexpr const char * EVENT_FORMAT =
...
static inline void serialize(std::ostream& ostr, const std::string& key,
...
DASManager::DASManager(const DASConfig& dasConfig) {
  ...
  _logFilePath =
      Util::FileUtils::FullFilePath({_dasConfig.GetStoragePath(), "das.log"});
}
...
bool DASManager::PostToServer(const std::string& pathToLogFile) {
  ...
  if (success) {
    LOG_DEBUG("DASManager.PostToServer.UploadSuccess",
              "Uploaded json of length %zu", json.length());
    ++_workerSuccessCount;
    if (!(_workerSuccessCount % 10)) {
      DASMSG(dasmgr_upload_success, "dasmgr.upload.stats",
             "Sent after every 10 successful uploads");
    ...
    LOG_ERROR("DASManager.PostToServer.UploadFailed",
              "Failed to upload json of length %zu", json.length());
    ++_workerFailCount;
    DASMSG(dasmgr_upload_failed, "dasmgr.upload.failed",
           "Sent after each failed upload");
...
void DASManager::PostLogsToServer() {
  const auto& directories = {_dasConfig.GetStoragePath(),
                             _dasConfig.GetBackupPath()};
  ...
  for (const auto& dir : directories) {
    ...
    for (const auto& jsonFile : jsonFiles) {
      ...
      if (!posted) {
        LOG_ERROR("DASManager.PostLogsToServer", "Failed to upload %s",
                  jsonFile.c_str());
        return;
      }
  ...
  for (const auto& jsonFile : jsonFiles) {
    ...
    if (!Util::FileUtils::CreateDirectory(backupPath, false, true, S_IRWXU)) {
      LOG_ERROR("DASManager.BackupLogFiles.CreateBackupDir",
                "Failed to create backup path %s", backupPath.c_str());
      return;
    }
    if (Util::FileUtils::GetDirectorySize(backupPath) > (ssize_t)backupQuota) {
      LOG_INFO("DASManager.BackupLogFiles.QuotaExceeded",
               "Exceeded quota for %s", backupPath.c_str());
      return;
    }
    LOG_DEBUG("DASManager.BackupLogFiles.MovingFile", "Moving %s into %s",
              jsonFile.c_str(), backupPath.c_str());
...
void DASManager::EnforceStorageQuota() {
  ...
  LOG_DEBUG("DASManager.EnforceStorageQuota", "Enforce quota %zd on path %s",
            quota, path.c_str());
  ...
  if (directorySize + fileThresholdSize > quota) {
    ...
    while (directorySize + fileThresholdSize > quota && !jsonFiles.empty()) {
      LOG_DEBUG("DASManager.EnforceQuota", "Delete %s",
                jsonFiles.front().c_str());
...
void DASManager::SetAllowUpload(bool allow_upload) {
  ...
  if (allow_upload && !FileUtils::FileExists(kAllowUploadFile)) {
    ...
    if (!FileUtils::TouchFile(kAllowUploadFile)) {
      LOG_ERROR("DASManager.SetAllowUpload", "Unable to create %s",
                kAllowUploadFile);
    }
...
std::string DASManager::ConvertLogEntryToJson(const AndroidLogEntry& logEntry) {
  ...
  DEV_ASSERT(logEntry.message != nullptr,
             "DASManager.ParseLogEntry.InvalidMessage");
  ...
  if (values.size() < Anki::Util::DAS::FIELD_COUNT) {
    LOG_ERROR("DASManager.ConvertLogEntry",
              "Unable to parse %s from %s (%zu != %d)", logEntry.message,
              logEntry.tag, values.size(), Anki::Util::DAS::FIELD_COUNT);
    return "";
  }
  ...
  //
  // If magic event names change, this code should be reviewed for
  // compatibility.
  //
...
void DASManager::ProcessLogEntry(const AndroidLogEntry& logEntry) {
  ...
  if (!_logFile.is_open()) {
    _logFile.open(_logFilePath,
                  std::ios::out | std::ofstream::binary | std::ofstream::ate);
  }
...
void DASManager::RollLogFile() {
  ...
  // Enqueue quota task?
  if (!_exiting) {
    auto quotaTask = [this]() { EnforceStorageQuota(); };
...
void DASManager::ProcessStats() {
  LOG_DEBUG("DASManager.ProcessStats.QueueStats",
            ...
            _entryCount, _eventCount, (uint32_t)_workerSuccessCount,
            (uint32_t)_workerFailCount, (uint32_t)_workerDroppedCount);
  ...
  if (getrusage(RUSAGE_SELF, &ru) == 0) {
    ...
    LOG_DEBUG("DASManager.ProcessStats.MemoryStats",
              "maxrss=%ld ixrss=%ld idrss=%ld isrss=%ld", ru.ru_maxrss,
...
uint32_t DASManager::GetSecondsSinceLastFlush() {
  ...
  return (uint32_t)std::chrono::duration_cast<std::chrono::seconds>(diff)
      .count();
}
...
uint32_t DASManager::GetNextIndexForJsonFile() {
  uint32_t index = 0;
  const auto& storagePaths = {_dasConfig.GetStoragePath(),
                              _dasConfig.GetBackupPath()};
  ...
  for (const auto& path : storagePaths) {
    ...
    if (!jsonFiles.empty()) {
      ...
      const std::string& filename =
          Util::FileUtils::GetFileName(lastFile, true, true);
...
void DASManager::LoadGlobalState() {
  ...
  // Get persistent values from OS
  {
    ...
    if (osState->HasValidEMR()) {
      ...
      LOG_ERROR("DASManager.LoadGlobalState.InvalidEMR",
                "INVALID EMR - NO ESN");
    }
  ...
  const std::string& transient_globals_path =
      _dasConfig.GetTransientGlobalsPath();
  if (!transient_globals_path.empty() &&
      Util::FileUtils::FileExists(transient_globals_path)) {
  ...
  const std::string& persistent_globals_path =
      _dasConfig.GetPersistentGlobalsPath();
  if (!persistent_globals_path.empty() &&
      Util::FileUtils::FileExists(persistent_globals_path)) {
  ...
  // timestamps may drift backward when system clock is adjusted.  If this
  // happens during service restart, events may be dropped.
  //
  if (_last_event_ts != 0) {
  ...
  LOG_DEBUG("DASManager.LoadGlobalState",
            "robot_id=%s robot_version=%s boot_id=%s sequence=%llu "
            "profile_id=%s allow_upload=%d",
            _robot_id.c_str(), _robot_version.c_str(), _boot_id.c_str(), _seq,
            _profile_id.c_str(), _allow_upload);
}
...
static void SaveGlobals(const Json::Value& json, const std::string& path) {
  ...
  try {
    ...
    LOG_ERROR("DASManager.SaveGlobals", "Unable to write %s (%s)", tmp.c_str(),
              ex.what());
    return;
  }
  ...
  if (!ok) {
    LOG_ERROR("DASManager.SaveGlobals", "Unable to move %s to %s", tmp.c_str(),
              path.c_str());
  }
}
...
Result DASManager::Run(const bool& shutdown) {
  ...
  if (!Util::FileUtils::CreateDirectory(storagePath, false, true, S_IRWXU)) {
    LOG_ERROR("DASManager.Run.CreateStoragePathFailure",
              "Failed to create storage path %s", storagePath.c_str());
    return RESULT_FAIL_INVALID_PARAMETER;
  }
  ...
  LOG_INFO("DASManager.Run",
           "robot_id=%s robot_version=%s boot_id=%s feature_run_id=%s",
           _robot_id.c_str(), _robot_version.c_str(), _boot_id.c_str(),
           _feature_run_id.c_str());
  ...
  // Open the log buffer
  struct logger_list* log =
      android_logger_list_open(LOG_ID_MAIN, ANDROID_LOG_RDONLY, 0, 0);
  if (log == nullptr) {
    // If we can't open the log buffer, return an appropriate error code.
    LOG_ERROR("DASManager.Run", "Unable to open android logger (errno %d)",
              errno);
  ...
  while (true) {
    ...
    if (rc <= 0) {
      ...
      LOG_ERROR("DASManager.Run", "Log read error %d (%s)", rc,
                strerror(errno));
    ...
    if (rc != 0) {
      ...
      LOG_ERROR("DASManager.Run", "Unable to process log buffer (error %d)",
                rc);
      continue;
    }
    ...
    // If we are allowed to upload and have gone over the flush interval, roll
    // the log file now. If we are NOT allowed to upload, let the file keep
    // growing to avoid fragmentation.
    //
    bool rollNow = false;
    ...
    if (_purge_backup_files) {
      auto purgeTask = [this]() { PurgeBackupFiles(); };
