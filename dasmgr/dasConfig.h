...
class DASConfig {
 public:
  DASConfig() {}
  DASConfig(const std::string& url, size_t file_threshold_size,
            uint32_t flush_interval, const std::string& storage_path,
            size_t storage_quota, const std::string& backup_path,
            size_t backup_quota, const std::string& persistent_globals_path,
  ...
  // How many seconds should we collect events before trying to upload to the
  // server
  ...
  // How much space can we use for our JSON log files before we must start
  // dropping events.  This should only happen in extreme cases where we can't
  // reach the DAS server.
  ...
  // How much space can we use at the backup location before we have to stop
  // accepting new log files
  ...
  const std::string& GetPersistentGlobalsPath() const {
    return _persistent_globals_path;
  }
  ...
  const std::string& GetTransientGlobalsPath() const {
    return _transient_globals_path;
  }
