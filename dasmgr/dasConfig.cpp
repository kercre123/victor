...
DASConfig::DASConfig(const std::string& url, size_t file_threshold_size,
                     uint32_t flush_interval, const std::string& storage_path,
                     size_t storage_quota, const std::string& backup_path,
                     size_t backup_quota,
...
std::unique_ptr<DASConfig> DASConfig::GetDASConfig(const Json::Value& json) {
  ...
  if (!file_threshold_size.isUInt()) {
    LOG_ERROR("DASConfig.GetDASConfig.InvalidFileThresholdSize",
              "Invalid file_threshold_size attribute");
    return nullptr;
  }

  const auto& flush_interval = dasConfig["flush_interval"];
  if (!flush_interval.isUInt()) {
    LOG_ERROR("DASConfig.GetDASConfig.InvalidFlushInterval",
              "Invalid flush_interval attribute");
    return nullptr;
  }

  const auto& storage_path = dasConfig["storage_path"];
  if (!storage_path.isString()) {
    LOG_ERROR("DASConfig.GetDASConfig.InvalidStoragePath",
              "Invalid storage_path attribute");
    return nullptr;
  }

  const auto& storage_quota = dasConfig["storage_quota"];
  if (!storage_quota.isUInt()) {
    LOG_ERROR("DASConfig.GetDASConfig.InvalidStorageQuota",
              "Invalid storage_quota attribute");
    return nullptr;
  }

  const auto& backup_path = dasConfig["backup_path"];
  if (!backup_path.isString()) {
    LOG_ERROR("DASConfig.GetDASConfig.InvalidBackupPath",
              "Invalid backup_path attribute");
    return nullptr;
  }

  const auto& backup_quota = dasConfig["backup_quota"];
  if (!backup_quota.isUInt()) {
    LOG_ERROR("DASConfig.GetDASConfig.InvalidBackupQuota",
              "Invalid backup_quota attribute");
    return nullptr;
  }

  const auto& persistent_globals_path = dasConfig["persistent_globals_path"];
  if (!persistent_globals_path.isString()) {
    LOG_ERROR("DASConfig.GetDASConfig.InvalidPersistentGlobalsPath",
              "Invalid persistent_globals_path attribute");
    return nullptr;
  }

  const auto& transient_globals_path = dasConfig["transient_globals_path"];
  if (!transient_globals_path.isString()) {
    LOG_ERROR("DASConfig.GetDASConfig.InvalidTransientGlobalsPath",
              "Invalid transient_globals_path attribute");
    return nullptr;
  }

  return std::make_unique<DASConfig>(
      url.asString(), file_threshold_size.asUInt(), flush_interval.asUInt(),
      storage_path.asString(), storage_quota.asUInt(), backup_path.asString(),
      backup_quota.asUInt(), persistent_globals_path.asString(),
...
std::unique_ptr<DASConfig> DASConfig::GetDASConfig(const std::string& path) {
  ...
  if (!reader.parse(ifstream, json)) {
    LOG_ERROR("DASConfig.GetDASConfig.InvalidJsonFile",
              "Unable to parse json from %s", path.c_str());
    return nullptr;
  }
