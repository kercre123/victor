...
CONSOLE_VAR_ENUM(int, kWebvizUpdatePeriod, "OSState.Webviz", 0,
                 "Off,10ms,100ms,1000ms,10000ms");
...
CONSOLE_VAR_RANGED(u32, kHighMemPressureMultiple, "OSState.MemoryInfo", 10, 0,
                   100);

// When memory pressure > this, report yellow alert
CONSOLE_VAR_RANGED(u32, kMediumMemPressureMultiple, "OSState.MemoryInfo", 5, 0,
                   100);

// When disk pressure > this, report red alert
CONSOLE_VAR_RANGED(u32, kHighDiskPressureMultiple, "OSState.DiskInfo", 10, 0,
                   100);

// When disk pressure > this, report yellow alert
CONSOLE_VAR_RANGED(u32, kMediumDiskPressureMultiple, "OSState.DiskInfo", 5, 0,
                   100);
...
const char* kNominalCPUFreqFile =
    "/sys/devices/system/cpu/cpu0/cpufreq/scaling_max_freq";
const char* kCPUFreqFile =
    "/sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_cur_freq";
const char* kCPUFreqSetFile =
    "/sys/devices/system/cpu/cpu0/cpufreq/scaling_setspeed";
const char* kCPUGovernorFile =
    "/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor";
const char* kTemperatureFile =
...
std::string GetProperty(const std::string& key) {
  ...
  if (rc <= 0) {
    LOG_WARNING("OSState.GetProperty.FailedToFindProperty",
                "Property %s not found", key.c_str());
  }
...
OSState::OSState() {
  ...
  if (file.is_open()) {
    ...
    LOG_INFO("OSState.Constructor.NominalCPUFreq", "%dkHz",
             kNominalCPUFreq_kHz);
    file.close();
  } else {
    LOG_ERROR("OSState.Constructor.FailedToOpenNominalCPUFreqFile", "%s",
              kNominalCPUFreqFile);
  }
  ...
  if (Util::FileUtils::FileExists("/etc/os-version")) {
    ...
    if (tokens.size() == 4) {
      try {
        ...
        LOG_WARNING("OSState.GetOSBuildVersion.UnableToParseVersionString",
                    "%s", osv.c_str());
        _majorVersion = -1;
        _minorVersion = -1;
  ...
  const bool versionsValid = _majorVersion >= 0 && _minorVersion >= 0 &&
                             _incrementalVersion >= 0 && _buildVersion >= 0;
  DEV_ASSERT_MSG(versionsValid, "OSState.MajorMinorIncVersionInvalid",
                 "maj %d, min %d, inc %d, build %d", _majorVersion,
                 _minorVersion, _incrementalVersion, _buildVersion);
}

OSState::~OSState() {}

RobotID_t OSState::GetRobotID() const { return DEFAULT_ROBOT_ID; }
...
void OSState::Update(BaseStationTime_t currTime_nanosec) {
  ...
  if (kWebvizUpdatePeriod != 0 && _webServiceCallback) {
    if (_currentTime_ms - _lastWebvizUpdateTime_ms >
        kPeriodEnumToMS[kWebvizUpdatePeriod]) {
      UpdateCPUTimeStats();
...
void OSState::SendToWebVizCallback(
...
void OSState::UpdateCPUFreq_kHz() const {
  ...
  if (_cpuFile.is_open()) {
    ...
    LOG_ERROR("OSState.UpdateCPUFreq_kHz.FailedToOpenCPUFreqFile", "%s",
              kCPUFreqFile);
  }
}

void OSState::SetDesiredCPUFrequency(DesiredCPUFrequency freq) {
  const std::string desiredGovernor = (freq == DesiredCPUFrequency::Automatic)
                                          ? kAutomaticGovernor
                                          : kManualGovernor;

  // write governor mode
  const bool ok1 =
      Util::FileUtils::WriteFile(kCPUGovernorFile, desiredGovernor);
  if (!ok1) {
    LOG_ERROR("OSState.SetDesiredCPUFrequency.SetGovernor.Failed",
              "Failed to write governor value '%s' to file '%s'",
              desiredGovernor.c_str(), kCPUGovernorFile);
    return;
  }

  if (freq != DesiredCPUFrequency::Automatic) {
    // if Automatic, we're done once we set the governor. Otherwise we also need
    // to write the desired freq

    // write frequency
    const unsigned int freqVal = Util::EnumToUnderlying(freq);
    if (freqVal <= 0) {
      LOG_ERROR("OSState.SetDesiredCPUFrequency.InvalidFrequency",
                "Can't set frequency to %d", freqVal);
      return;
    }

    const bool ok2 =
        Util::FileUtils::WriteFile(kCPUFreqSetFile, std::to_string(freqVal));
    if (!ok2) {
      LOG_ERROR("OSState.SetDesiredCPUFrequency.SetFrequency.Failed",
                "Failed to write frequency value '%d' to file '%s'", freqVal,
                kCPUFreqSetFile);
      return;
    }

    LOG_INFO("OSState.SetDesiredCPUFrequency.Manual",
             "Set to manual cpu frequency %u", freqVal);
  } else {
    LOG_INFO("OSState.SetDesiredCPUFrequency.Automatic",
             "Set to automatic cpu frequency management");
  }

  // NOTE: not returning success / fail because all I know is that the file got
  // written to. It's up to the OS to actually change the frequency, and that
  // could take some time or be overruled by something else
}
...
void OSState::UpdateTemperature_C() const {
  ...
  if (_temperatureFile.is_open()) {
    ...
    LOG_ERROR("OSState.UpdateTemperature_C.FailedToOpenTemperatureFile", "%s",
              kTemperatureFile);
  }
}
...
void OSState::UpdateUptimeAndIdleTime() const {
  ...
  if (_uptimeFile.is_open()) {
    ...
    LOG_ERROR("OSState.UpdateUptimeAndIdleTime.FailedToOpenUptimeFile", "%s",
              kUptimeFile);
  }
}
...
void OSState::UpdateMemoryInfo() const {
  ...
  if (_memInfoFile.is_open()) {
    std::string discard;
    _memInfoFile >> discard >> _totalMem_kB >> discard >> discard >>
        _freeMem_kB >> discard >> discard >> _availMem_kB;
    _memInfoFile.close();
  } else {
    LOG_ERROR("OSState.UpdateMemoryInfo.FailedToOpenMemInfoFile", "%s",
              kMemInfoFile);
  }
}
...
void OSState::UpdateCPUTimeStats() const {
  ...
  if (_cpuTimeStatsFile.is_open()) {
    ...
    LOG_ERROR("OSState.UpdateCPUTimeStats.FailedToOpenCPUTimeStatsFile", "%s",
              kCPUTimeStatsFile);
  }
}
...
uint32_t OSState::GetCPUFreq_kHz() const {
  ...
  if ((_currentTime_ms - lastUpdate_ms > _updatePeriod_ms) ||
      (_updatePeriod_ms == 0)) {
    UpdateCPUFreq_kHz();
...
void OSState::GetMemoryInfo(MemoryInfo& info) const {
  ...
  info.alert = GetAlert(info.pressure, kMediumMemPressureMultiple,
...
bool OSState::GetDiskInfo(const std::string& path, DiskInfo& info) const {
  ...
  if (statfs(path.c_str(), &fsinfo) != 0) {
    LOG_ERROR("OSState::GetDiskInfo",
              "Unable to get disk info for %s (errno %d)", path.c_str(), errno);
    return false;
  }
  ...
  info.alert = GetAlert(info.pressure, kMediumDiskPressureMultiple,
...
const std::string& OSState::GetSerialNumberAsString() {
  if (_serialNumString.empty()) {
    std::stringstream ss;
    ss << std::hex << std::setfill('0') << std::setw(8) << std::uppercase
...
void OSState::GetOSBuildVersion(int& major, int& minor, int& incremental,
                                int& build) const {
  major = _majorVersion;
  minor = _minorVersion;
...
const std::string& OSState::GetBuildSha() { return _buildSha; }
...
static std::string GetIPV4AddressForInterface(const char* if_name) {
  ...
  if (ifa != nullptr) {
    ...
    if (s != 0) {
      LOG_ERROR("OSState.GetIPAddress.GetNameInfoFailed", "%s",
                gai_strerror(s));
...
static std::string GetWiFiSSIDForInterface(const char* if_name) {
  ...
  if (ioctl(fd, SIOCGIWESSID, &req) == -1) {
    LOG_INFO("OSState.UpdateWifiInfo.FailedToGetSSID",
             "iface = %s , errno = %s", if_name, strerror(errno));
...
bool OSState::IsValidIPAddress(const std::string& ip) const {
  ...
  if (result != 0) {
    const bool isLinkLocalIP =
        (ip.length() > 7) && ip.compare(0, 7, "169.254") == 0;
...
std::string OSState::GetMACAddress() const {
  ...
  if (macFile.is_open()) {
    ...
    LOG_ERROR("OSState.GetMACAddress.FailedToOpenMACAddressFile", "%s",
              kMACAddressFile);
  }
  return "";
}
...
const std::string& OSState::GetBootID() {
  if (_bootID.empty()) {
    ...
    if (_bootID.empty()) {
      LOG_ERROR("OSState.GetBootID", "Unable to read boot ID from %s",
                kBootIDFile);
    }
  }
  return _bootID;
...
bool OSState::IsWallTimeSynced() const {
  ...
  if (adjtimex(&txc) < 0) {
    LOG_ERROR("OSState.IsWallTimeSynced.CantGetTimex",
              "Invalid return from adjtimex");
    return false;
  }
...
bool OSState::HasTimezone() const {
  ...
  if (!Util::FileUtils::FileExists(kUniversalTimeFile)) {
    LOG_ERROR("OSState.HasTimezone.NoUniversalTimeFile",
              "Unable to find universal time file '%s', cant check for "
              "timezone (assuming none)",
              kUniversalTimeFile);
    return false;
  }
  ...
  if (!Util::FileUtils::FileExists(kLocalTimeFile)) {
    LOG_ERROR("OSState.HasTimezone.NoLocalTimeFile",
              "Missing local time file '%s'", kLocalTimeFile);
    return false;
  }

  // local time should be a symlink to something, either Universal (meaning we
  // don't have a timezone) or a specific timezone
  struct stat linkStatus;
  const int ok = lstat(kLocalTimeFile, &linkStatus);
  if (ok < 0) {
    LOG_ERROR("OSState.HasTimezone.CantStatLink",
              "lstat(%s) returned %d, error %s", kLocalTimeFile, ok,
              strerror(errno));
    return false;
  }
  ...
  // the path string length can be variable. Rather than dynamically allocating
  // it, just make sure our statically allocated buffer is large enough
  ...
  if (linkStatus.st_size >= linkPathLen) {
    LOG_ERROR("OSState.HasTimezone.LinkNameTooLong",
              ...
              linkStatus.st_size, linkPathLen);
    // this means it can't be pointing to kUniversalTimeFile (we static assert
    // that that path will fit within the buffer), so it must be some really
    // long file. It seems likely that this is a timezone with a long name, so
    // return true, but it technically could be pointing to any file
    return true;
  }
  ...
  if (written < 0 || written >= linkPathLen) {
    LOG_ERROR("OSState.HasTimezone.CantReadLink",
              "File '%s' looks like a symlink, but can't be read (returned %d, "
              "error %s)",
              kLocalTimeFile, written, strerror(errno));
    return false;
  }

  linkPath[written] = '\0';

  // if timezone isn't set, path is either kUniversalTimeFile or
  // ../../kUniversalTimeFile
  ...
  if (found != linkPath) {
    ...
    if (found != linkPath + 5 || strstr(linkPath, "../../") != linkPath) {
