...
// check more often if we aren't synced (so we get the accurate time after a
// sync)
...
void PrintWallTimeToLog(ConsoleFunctionContextRef context) {
  ...
  PRINT_NAMED_INFO("WallTime.DEBUG.OSState.IsSynced", "%s",
                   OSState::getInstance()->IsWallTimeSynced() ? "yes" : "no");
  PRINT_NAMED_INFO("WallTime.DEBUG.OSState.HasTimezone", "%s",
                   OSState::getInstance()->HasTimezone() ? "yes" : "no");
  {
    ...
    if (got) {
      PRINT_NAMED_INFO("WallTime.DEBUG.OSState.UTCTime", "%s: %s", t.c_str(),
                       asctime(&time));
    } else {
...

  {
    ...
    if (got) {
      PRINT_NAMED_INFO("WallTime.DEBUG.OSState.LocalTime", "%s: %s", t.c_str(),
                       asctime(&time));
    } else {
...
void SetFakeWallTime24HourUTC(ConsoleFunctionContextRef context) {
  ...
  if (time >= 0 && time < 2400 && time % 100 < 60) {
    ...
    PRINT_NAMED_WARNING(
        "WallTime.SetFakeWallTime.InvalidTime",
        "time %d is invalid, set in 24 hour format in UTC (e.g. 1830)", time);
  }
}
...
bool WallTime::GetApproximateUTCTime(struct tm& utcTime) {
  ...
  PRINT_NAMED_ERROR("WallTime.UTC.Invalid", "gmtime returned null. Error: %s",
                    strerror(errno));
  return false;
}
...
bool WallTime::GetApproximateLocalTime(struct tm& localTime) {
  ...
  PRINT_NAMED_ERROR("WallTime.Local.Invalid",
                    "localtime returned null. Erro`r: %s", strerror(errno));
  return false;
}
...
bool WallTime::IsTimeSynced() {
  ...
  // Use base station timer because it's cheap and good enough here, goal is
  // just to not hit the syscall too frequently if this function is called often
  const float currTime_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();

  const float checkPeriod_s = _wasSynced ? kSyncCheckPeriodWhenSynced_s
                                         : kSyncCheckPeriodWhenNotSynced_s;
...
bool WallTime::AreTimePointsInSameDay(const TimePoint_t& a,
                                      const TimePoint_t& b) {
  tm aTime;
  tm bTime;

  {
    ...
    if (a_tm == nullptr) {
      ...
      // need to return something, possibly better to assume it's the same day
      // to avoid a big reaction or stats bump
      return true;
    } else {
      aTime = *a_tm;
    }
