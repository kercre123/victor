...
RobotID_t OSState::GetRobotID() const { return _robotID; }

void OSState::SetRobotID(RobotID_t robotID) { _robotID = robotID; }
...
void OSState::UpdateTemperature_C() const {
  ...
  // 65C: randomly chosen temperature at which throttling does not appear to
  // occur on physical robot
  _cpuTemp_C = 65;
}
...
void OSState::UpdateMemoryInfo() const {
  ...
  const auto kerr = host_statistics(mach_host_self(), HOST_VM_INFO,
                                    (host_info_t)&vmstat, &count);
  if (kerr == KERN_SUCCESS) {
    const auto totalPages = vmstat.active_count + vmstat.inactive_count +
                            vmstat.free_count + vmstat.wire_count;
    const auto availPages =
        vmstat.active_count + vmstat.inactive_count + vmstat.free_count;
...
void OSState::UpdateCPUTimeStats() const {
  ...
  if (kerr == KERN_SUCCESS) {
    ...
    for (natural_t i = 0; i < numCPUs; ++i) {
      char temp[79 + 1];
      snprintf(temp, sizeof(temp), "CPU%d %d %d %d %d 0 0 0 0 0 0", i,
      ...
      total[CPU_STATE_SYSTEM] +=
          cpuInfo[(CPU_STATE_MAX * i) + CPU_STATE_SYSTEM];
      total[CPU_STATE_IDLE] += cpuInfo[(CPU_STATE_MAX * i) + CPU_STATE_IDLE];
    }

    char temp[79 + 1];
    snprintf(temp, sizeof(temp), "CPU %d %d %d %d 0 0 0 0 0 0",
             total[CPU_STATE_USER], total[CPU_STATE_NICE],
             total[CPU_STATE_SYSTEM], total[CPU_STATE_IDLE]);
...
bool OSState::IsCPUThrottling() const { return false; }
...
const std::string& OSState::GetOSBuildVersion() { return _osBuildVersion; }
...
const std::string& OSState::GetRobotVersion() { return _robotVersion; }

const std::string& OSState::GetBuildSha() { return _buildSha; }

std::string OSState::GetMACAddress() const { return "00:00:00:00:00:00"; }

const std::string& OSState::GetIPAddress(bool update) { return _ipAddress; }

const std::string& OSState::GetSSID(bool update) { return _ssid; }
...
uint64_t OSState::GetWifiTxBytes() const { return 0; }

uint64_t OSState::GetWifiRxBytes() const { return 0; }
...
bool OSState::IsInRecoveryMode() { return false; }

bool OSState::RebootedForMaintenance() const { return kFakeIsReboot; }

bool OSState::HasValidEMR() const { return false; }
...
bool OSState::IsUserSpaceSecure() { return true; }

bool OSState::IsAnkiDevRobot() { return false; }
