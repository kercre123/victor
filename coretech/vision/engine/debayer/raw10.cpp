 ...
 */
inline void RGGBtoRGB(const std::array<u8, 8>& values, u8* outBufferPtr1,
                      u8* outBufferPtr2) {
  // RGGB8 -> RGB format
 ...
 */
inline void RGGBtoY(const std::array<u8, 8>& values, u8* outBufferPtr1,
                    u8* outBufferPtr2) {
  // RGGB8 -> Y8 format
...
Result RAW10toRGB24::operator()(const Debayer::InArgs& inArgs,
  ...
  for (u32 row = 0; row < outArgs.height; row += rowStep) {
    for (u32 col = 0; col < outArgs.width; col += colStep) {
      // Extract RAW10 RGGB10 rows and convert to Gamma Corrected eight RGGB8
      // values
      values[0] =
          _gammaLUT[(inBufferPtr1[0] << 2) | ((inBufferPtr1[4] & 0x03))];
      values[1] =
          _gammaLUT[(inBufferPtr1[1] << 2) | ((inBufferPtr1[4] & 0x0C) >> 2)];
      values[2] =
          _gammaLUT[(inBufferPtr1[2] << 2) | ((inBufferPtr1[4] & 0x30) >> 4)];
      values[3] =
          _gammaLUT[(inBufferPtr1[3] << 2) | ((inBufferPtr1[4] & 0xC0) >> 6)];
      values[4] =
          _gammaLUT[(inBufferPtr2[0] << 2) | ((inBufferPtr2[4] & 0x03))];
      values[5] =
          _gammaLUT[(inBufferPtr2[1] << 2) | ((inBufferPtr2[4] & 0x0C) >> 2)];
      values[6] =
          _gammaLUT[(inBufferPtr2[2] << 2) | ((inBufferPtr2[4] & 0x30) >> 4)];
      values[7] =
          _gammaLUT[(inBufferPtr2[3] << 2) | ((inBufferPtr2[4] & 0xC0) >> 6)];
...
Result RAW10toY8::operator()(const Debayer::InArgs& inArgs,
                             Debayer::OutArgs& outArgs) const {
  u8 outChannels = 1;
  u8 inChannels = 1;
