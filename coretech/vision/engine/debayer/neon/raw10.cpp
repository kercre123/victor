...
Result RAW10toRGB24::operator()(const Debayer::InArgs& inArgs,
  ...
  auto extract = [&](uint8x8_t aa, uint8x8_t bb) -> uint8x8_t {
    uint8x8_t buffer = vext_u8(
        vreinterpret_u8_u64(vshl_n_u64(vreinterpret_u64_u8(aa), 32)), bb, 4);
  ...
  for (u32 row = 0; row < outArgs.height; row += rowStep) {
    for (u32 col = 0; col < outArgs.width;
         col +=
         2 * colStep)  // we're doing 2 blocks of 8 (16 columns and 2 rows)
    {
      uint8x8_t aa, bb;
