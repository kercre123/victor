...
// NOTE: It appears the returned track confidence is set to the fixed value of
// whatever
//   the OKAO detection threshold is set to when in default tracking accuracy
//   mode, so this parameter will have no effect unless the high-accuracy
//   tracker is used
...
// Assuming a max face detection of 3m, focal length of 300 and
// distanceBetweenEyes_mm of 62 then the smallest distance between eyes in
// pixels will be ~6
...
CONSOLE_VAR(bool, kUseUndistortionForFacePose, "Vision.FaceDetectorCommon",
            true);
CONSOLE_VAR(bool, kAdjustEyeDistByYaw, "Vision.FaceDetectorCommon", true);
CONSOLE_VAR(bool, kKeepUndistortedFaceFeatures, "Vision.FaceDetectorCommon",
            false);

namespace DetectParams {
// Parameters common to all face detection modes
CONSOLE_VAR_RANGED(s32, kMaxDetectedFaces, "Vision.FaceDetectorCommon", 10, 1,
                   1023);
CONSOLE_VAR_RANGED(s32, kMinFaceSize, "Vision.FaceDetectorCommon", 48, 20,
                   8192);
CONSOLE_VAR_RANGED(s32, kMaxFaceSize, "Vision.FaceDetectorCommon", 640, 20,
                   8192);
CONSOLE_VAR_ENUM(s32, kPoseAngle, "Vision.FaceDetectorCommon",
                 Okao::GetIndex(Okao::PoseAngle::Front),
                 Okao::GetConsoleString<Okao::PoseAngle>().c_str());
CONSOLE_VAR_ENUM(s32, kRollAngle, "Vision.FaceDetectorCommon",
                 Okao::GetIndex(Okao::RollAngle::UpperPm45),
                 Okao::GetConsoleString<Okao::RollAngle>().c_str());
CONSOLE_VAR_ENUM(s32, kSearchDensity, "Vision.FaceDetectorCommon",
                 Okao::GetIndex(Okao::SearchDensity::Normal),
                 Okao::GetConsoleString<Okao::SearchDensity>().c_str());
CONSOLE_VAR_RANGED(s32, kFaceDetectionThreshold, "Vision.FaceDetectorCommon",
                   500, 1, 1000);
CONSOLE_VAR_ENUM(s32, kDetectionMode, "Vision.FaceDetectorCommon",
                 Okao::GetIndex(Okao::DetectionMode::Movie),
                 Okao::GetConsoleString<Okao::DetectionMode>().c_str());

// Movie only
CONSOLE_VAR_RANGED(s32, kSearchInitialCycle, "Vision.FaceDetectorMovie", 2, 1,
                   45);
CONSOLE_VAR_RANGED(s32, kSearchNewCycle, "Vision.FaceDetectorMovie", 2, 1, 45);
CONSOLE_VAR_RANGED(s32, kSearchNewInterval, "Vision.FaceDetectorMovie", 5, -1,
                   45);
...
CONSOLE_VAR_RANGED(s32, kSteadinessPosition, "Vision.FaceDetectorMovie", 10, 0,
                   30);
CONSOLE_VAR_RANGED(s32, kSteadinessSize, "Vision.FaceDetectorMovie", 10, 0, 30);
CONSOLE_VAR_RANGED(s32, kTrackingSwapRatio, "Vision.FaceDetectorMovie", 400,
                   100, 10000);
CONSOLE_VAR_RANGED(s32, kDelayCount, "Vision.FaceDetectorMovie", 1, 0, 10);
CONSOLE_VAR_ENUM(s32, kTrackingAccuracy, "Vision.FaceDetectorMovie",
                 Okao::GetIndex(Okao::TrackingAccuracy::High),
                 Okao::GetConsoleString<Okao::TrackingAccuracy>().c_str());
...
// while tracking. The nPose field in the DetectionInfo struct was sometimes
// "HEAD" (meaning back of head). From the Omron team:
//   It returned "Head" because you set bUseHeadTracking as TRUE of
//   OKAO_DT_MV_SetPoseExtension(). (It's default value is FALSE.) Face
//   Detection engine output "Head" only by tracking, not from the first frame
//   or Still Mode. It is good for keeping tracking, but not good for Facial
//   Parts Detection. If you give priority to Facial Parts Detection over
//   tracking, you should turn bUseHeadTracking off or skip the face.
...
FaceTracker::Impl::Impl(const Camera& camera, const std::string& modelPath,
...
Result FaceTracker::Impl::Init() {
  ...
  LOG_INFO("FaceTrackerImpl.Init.FaceLibVersion",
           "Initializing with FaceLib version %d.%d", okaoVersionMajor,
           okaoVersionMinor);
  ...
  LOG_INFO("FaceTrackerImpl.Init.FaceDetectorVersion",
           "Initializing with FaceDetector version %d.%d", okaoVersionMajor,
           okaoVersionMinor);
  ...
  LOG_INFO("FaceTrackerImpl.Init.PartDetectorVersion",
           "Initializing with PartDetector version %d.%d", okaoVersionMajor,
           okaoVersionMinor);
  ...
  switch (Okao::GetEnum<Okao::DetectionMode>(DetectParams::kDetectionMode)) {
    case Okao::DetectionMode::Movie: {
      _okaoDetectorHandle =
          OKAO_DT_CreateHandle(_okaoCommonHandle, DETECTION_MODE_MOVIE,
                               DetectParams::kMaxDetectedFaces);
      if (NULL == _okaoDetectorHandle) {
        LOG_ERROR(
            "FaceTrackerImpl.Init.FaceLibDetectionHandleAllocFail.VideoMode",
            "");
        return RESULT_FAIL_MEMORY;
      }

      // Adjust some detection parameters
      okaoResult = OKAO_DT_MV_SetSearchCycle(
          _okaoDetectorHandle, DetectParams::kSearchInitialCycle,
          DetectParams::kSearchNewCycle, DetectParams::kSearchNewInterval);
      ...
      okaoResult = OKAO_DT_MV_SetSteadinessParam(
          _okaoDetectorHandle, DetectParams::kSteadinessPosition,
      ...
      okaoResult = OKAO_DT_MV_SetTrackingSwapParam(
          _okaoDetectorHandle, DetectParams::kTrackingSwapRatio);
      if (OKAO_NORMAL != okaoResult) {
        LOG_ERROR("FaceTrackerImpl.Init.FaceLibSetSwapRatioFailed", "");
        return RESULT_FAIL_INVALID_PARAMETER;
      }

      okaoResult = OKAO_DT_MV_SetDelayCount(
          _okaoDetectorHandle,
          DetectParams::kDelayCount);  // have to see faces for more than one
                                       // frame
      if (OKAO_NORMAL != okaoResult) {
      ...
      okaoResult = OKAO_DT_MV_SetAccuracy(_okaoDetectorHandle,
                                          Okao::GetOkao<Okao::TrackingAccuracy>(
                                              DetectParams::kTrackingAccuracy));
      if (OKAO_NORMAL != okaoResult) {
        LOG_ERROR("FaceTrackerImpl.Init.FaceLibSetAccuracyFailed", "");
        return RESULT_FAIL_INVALID_PARAMETER;
      }

      okaoResult = OKAO_DT_MV_SetAngleExtension(
          _okaoDetectorHandle, DetectParams::kEnableAngleExtension);
      if (OKAO_NORMAL != okaoResult) {
        LOG_ERROR("FaceTrackerImpl.Init.FaceLibSetAngleExtensionFailed", "");
        return RESULT_FAIL_INVALID_PARAMETER;
      }

      okaoResult = OKAO_DT_MV_SetPoseExtension(
          _okaoDetectorHandle, DetectParams::kEnablePoseExtension,
      ...
      okaoResult = OKAO_DT_MV_SetDirectionMask(_okaoDetectorHandle,
                                               DetectParams::kDirectionMask);
      if (OKAO_NORMAL != okaoResult) {
    ...
    case Okao::DetectionMode::Still: {
      _okaoDetectorHandle =
          OKAO_DT_CreateHandle(_okaoCommonHandle, DETECTION_MODE_STILL,
                               DetectParams::kMaxDetectedFaces);
      if (NULL == _okaoDetectorHandle) {
        LOG_ERROR(
            "FaceTrackerImpl.Init.FaceLibDetectionHandleAllocFail.StillMode",
            "");
        return RESULT_FAIL_MEMORY;
      }
      break;
  ...
  okaoResult =
      OKAO_DT_SetSizeRange(_okaoDetectorHandle, DetectParams::kMinFaceSize,
  ...
  okaoResult = OKAO_DT_SetAngle(
      _okaoDetectorHandle,
      Okao::GetOkao<Okao::PoseAngle>(DetectParams::kPoseAngle),
  ...
  okaoResult = OKAO_DT_SetSearchDensity(
      _okaoDetectorHandle,
      Okao::GetOkao<Okao::SearchDensity>(DetectParams::kSearchDensity));
  if (OKAO_NORMAL != okaoResult) {
    LOG_ERROR("FaceTrackerImpl.Init.FaceLibSetSearchDensityFailed", "");
    return RESULT_FAIL_INVALID_PARAMETER;
  }

  okaoResult = OKAO_DT_SetThreshold(_okaoDetectorHandle,
                                    DetectParams::kFaceDetectionThreshold);
  if (OKAO_NORMAL != okaoResult) {
  ...
  _okaoPartDetectionResultHandle =
      OKAO_PT_CreateResultHandle(_okaoCommonHandle);
...
void FaceTracker::Impl::Deinit() {
  ...
  if (NULL != _okaoGazeBlinkDetectHandle) {
    if (OKAO_NORMAL != OKAO_GB_DeleteHandle(_okaoGazeBlinkDetectHandle)) {
      LOG_ERROR(
          "FaceTrackerImpl.Destructor.FaceLibGazeBlinkDetectHandleDeleteFail",
          "");
    }
  }
  ...
  if (NULL != _okaoGazeBlinkResultHandle) {
    if (OKAO_NORMAL != OKAO_GB_DeleteResultHandle(_okaoGazeBlinkResultHandle)) {
      LOG_ERROR(
          "FaceTrackerImpl.Destructor.FaceLibGazeBlinkResultHandleDeleteFail",
          "");
    }
  }
  ...
  if (NULL != _okaoExpressionResultHandle) {
    if (OKAO_NORMAL !=
        OKAO_EX_DeleteResultHandle(_okaoExpressionResultHandle)) {
      LOG_ERROR(
          "FaceTrackerImpl.Destructor.FaceLibExpressionResultHandleDeleteFail",
          "");
    }
  }
  ...
  if (NULL != _okaoEstimateExpressionHandle) {
    if (OKAO_NORMAL != OKAO_EX_DeleteHandle(_okaoEstimateExpressionHandle)) {
      LOG_ERROR(
          "FaceTrackerImpl.Destructor."
          "FaceLibEstimateExpressionHandleDeleteFail",
          "");
    }
  }
  ...
  if (NULL != _okaoPartDetectionResultHandle) {
    if (OKAO_NORMAL !=
        OKAO_PT_DeleteResultHandle(_okaoPartDetectionResultHandle)) {
      LOG_ERROR(
          "FaceTrackerImpl.Destructor."
          "FaceLibPartDetectionResultHandle1DeleteFail",
          "");
    }
  }
  ...
  if (NULL != _okaoPartDetectorHandle) {
    if (OKAO_NORMAL != OKAO_PT_DeleteHandle(_okaoPartDetectorHandle)) {
      LOG_ERROR(
          "FaceTrackerImpl.Destructor.FaceLibPartDetectorHandleDeleteFail", "");
    }
  }
  ...
  if (NULL != _okaoDetectionResultHandle) {
    if (OKAO_NORMAL != OKAO_DT_DeleteResultHandle(_okaoDetectionResultHandle)) {
      LOG_ERROR(
          "FaceTrackerImpl.Destructor.FaceLibDetectionResultHandleDeleteFail",
          "");
    }
  }
...
void FaceTracker::Impl::ClearAllowedTrackedFaces() { Reset(); }
...
bool FaceTracker::Impl::DetectFaceParts(INT32 nWidth, INT32 nHeight,
                                        RAWIMAGE* dataPtr, INT32 detectionIndex,
                                        Vision::TrackedFace& face) {
  INT32 okaoResult = OKAO_PT_SetPositionFromHandle(
      _okaoPartDetectorHandle, _okaoDetectionResultHandle, detectionIndex);
  ...
  okaoResult = OKAO_PT_DetectPoint_GRAY(_okaoPartDetectorHandle, dataPtr,
                                        nWidth, nHeight, GRAY_ORDER_Y0Y1Y2Y3,
                                        _okaoPartDetectionResultHandle);
  ...
  okaoResult =
      OKAO_PT_GetResult(_okaoPartDetectionResultHandle, PT_POINT_KIND_MAX,
...
Result FaceTracker::Impl::EstimateExpression(INT32 nWidth, INT32 nHeight,
                                             RAWIMAGE* dataPtr,
                                             Vision::TrackedFace& face) {
  INT32 okaoResult = OKAO_EX_SetPointFromHandle(_okaoEstimateExpressionHandle,
                                                _okaoPartDetectionResultHandle);
  if (OKAO_NORMAL != okaoResult) {
  ...
  okaoResult = OKAO_EX_Estimate_GRAY(_okaoEstimateExpressionHandle, dataPtr,
                                     nWidth, nHeight, GRAY_ORDER_Y0Y1Y2Y3,
                                     _okaoExpressionResultHandle);
  if (OKAO_NORMAL != okaoResult) {
    if (OKAO_ERR_PROCESSCONDITION == okaoResult) {
    ...
    okaoResult = OKAO_EX_GetResult(_okaoExpressionResultHandle,
                                   EX_EXPRESSION_KIND_MAX, _expressionValues);
    if (OKAO_NORMAL != okaoResult) {
    ...
    static const FacialExpression
        TrackedFaceExpressionLUT[EX_EXPRESSION_KIND_MAX] = {
            FacialExpression::Neutral, FacialExpression::Happiness,
            FacialExpression::Surprise, FacialExpression::Anger,
            FacialExpression::Sadness};

    for (INT32 okaoExprIndex = 0; okaoExprIndex < EX_EXPRESSION_KIND_MAX;
         ++okaoExprIndex) {
      face.SetExpressionValue(TrackedFaceExpressionLUT[okaoExprIndex],
                              Util::numeric_cast<TrackedFace::ExpressionValue>(
                                  _expressionValues[okaoExprIndex]));
    }
  }
...
Result FaceTracker::Impl::DetectSmile(INT32 nWidth, INT32 nHeight,
                                      RAWIMAGE* dataPtr,
                                      Vision::TrackedFace& face) {
  INT32 okaoResult = OKAO_SM_SetPointFromHandle(_okaoSmileDetectHandle,
                                                _okaoPartDetectionResultHandle);
  if (OKAO_NORMAL != okaoResult) {
  ...
  okaoResult = OKAO_SM_Estimate(_okaoSmileDetectHandle, dataPtr, nWidth,
                                nHeight, _okaoSmileResultHandle);
  if (OKAO_NORMAL != okaoResult) {
  ...
  INT32 confidence = 0;
  okaoResult =
      OKAO_SM_GetResult(_okaoSmileResultHandle, &smileDegree, &confidence);
  if (OKAO_NORMAL != okaoResult) {
  ...
  face.SetSmileAmount(static_cast<f32>(smileDegree) * 0.01f,
                      static_cast<f32>(confidence) * 0.001f);

  return RESULT_OK;
}

Result FaceTracker::Impl::DetectGazeAndBlink(INT32 nWidth, INT32 nHeight,
                                             RAWIMAGE* dataPtr,
                                             Vision::TrackedFace& face) {
  INT32 okaoResult = OKAO_GB_SetPointFromHandle(_okaoGazeBlinkDetectHandle,
                                                _okaoPartDetectionResultHandle);
  if (OKAO_NORMAL != okaoResult) {
  ...
  okaoResult = OKAO_GB_Estimate(_okaoGazeBlinkDetectHandle, dataPtr, nWidth,
                                nHeight, _okaoGazeBlinkResultHandle);
  if (OKAO_NORMAL != okaoResult) {
  ...
  if (_detectGaze) {
    ...
    okaoResult = OKAO_GB_GetGazeDirection(_okaoGazeBlinkResultHandle,
                                          &gazeLeftRight_deg, &gazeUpDown_deg);
    if (OKAO_NORMAL != okaoResult) {
  ...
  if (_detectBlinks) {
    ...
    okaoResult = OKAO_GB_GetEyeCloseRatio(_okaoGazeBlinkResultHandle,
                                          &blinkDegreeLeft, &blinkDegreeRight);
    if (OKAO_NORMAL != okaoResult) {
    ...
    face.SetBlinkAmount(static_cast<f32>(blinkDegreeLeft) * 0.001f,
                        static_cast<f32>(blinkDegreeRight) * 0.001f);
  }
...
bool FaceTracker::Impl::DetectEyeContact(const TrackedFace& face,
  ...
  DEV_ASSERT(face.IsTranslationSet(),
             "FaceTrackerImpl.DetectEyeContact.FaceTranslationNotSet");
...
static Vec3f GetTranslation(const Point2f& leftEye, const Point2f& rightEye,
                            const f32 intraEyeDist,
                            const CameraCalibration& scaledCalib) {
  // Get unit vector along camera ray from the point between the eyes in the
  // image
...
Result FaceTracker::Impl::SetFacePoseWithoutParts(const s32 nrows,
                                                  const s32 ncols,
                                                  TrackedFace& face,
                                                  f32& intraEyeDist) {
  // Without face parts detected (which includes eyes), use fake eye centers for
  // finding pose
  auto const& rect = face.GetRect();
  DEV_ASSERT(rect.Area() > 0,
             "FaceTrackerImpl.SetFacePoseWithoutParts.InvalidFaceRectangle");
  ...
  intraEyeDist =
      std::max((rightEye - leftEye).Length(), MinDistBetweenEyes_pixels);

  const CameraCalibration& scaledCalib =
      _camera.GetCalibration()->GetScaled(nrows, ncols);

  // Use the eye positions and raw intra-eye distance to compute the head's
  // translation
  ...
  // We don't know anything about orientation without parts, so don't update it
  // and assume _not_ facing the camera (without actual evidence that we are)
  face.SetIsFacingCamera(false);

  return RESULT_OK;
}

Result FaceTracker::Impl::SetFacePoseFromParts(const s32 nrows, const s32 ncols,
                                               TrackedFace& face,
                                               f32& intraEyeDist) {
  // Init outputs to zero in case anything goes wrong
  intraEyeDist = 0.f;

  if (!ANKI_VERIFY(_camera.IsCalibrated(),
                   "FaceTrackerImpl.SetFacePoseFromParts.CameraNotCalibrated",
                   "")) {
    return RESULT_FAIL;
  }

  // Little local (likely inlineable) helper to check if the offset has been set
  // yet, for readability purposes
  ...
  if (!IsFirstPointOffsetSet(kFirstPointOffset)) {
    // The first time through, empirically determime where the first point is in
    // the void* data structure we get from OKAO. We have already called
    // OKAO_PT_GetResult to populate _facialParts, so we can look for where
    // those values are located within the data structure. The reason we don't
    // just hard-code this, is that we observed a difference b/w the Mac and
    // Android/ARM OKAO libs in how this data structure is organized/packed, so
    // it feels safer to have this as "self documenting" code.
    const INT32* pt = (INT32*)_okaoPartDetectionResultHandle;
    for (INT32 i = 0; i < PT_POINT_KIND_MAX; ++i) {
      if (!IsFirstPointOffsetSet(kFirstPointOffset)) {
        // Not set yet: see if we have found the first point in the HPTRESULT
        // yet
        if ((pt[i] == _facialParts[0].x) && (pt[i + 1] == _facialParts[0].y)) {
          LOG_INFO("FaceTrackerImpl.SetFacePoseFromParts.SetFirstPointOffset",
                   "kFirstPointOffset=%d", i);
          kFirstPointOffset = i;
          break;
        }
      }
    ...
    if (ANKI_DEV_CHEATS && IsFirstPointOffsetSet(kFirstPointOffset)) {
      // Sanity check the FirstPointOffset we just found (all following
      // facialParts points should match too)
      pt = (INT32*)_okaoPartDetectionResultHandle + kFirstPointOffset;
      for (INT32 i = 0; i < PT_POINT_KIND_MAX; ++i, pt += 2) {
        if (!ANKI_VERIFY(
                (*pt == _facialParts[i].x) && (*(pt + 1) == _facialParts[i].y),
                "FaceTrackerImpl.SetFacePoseFromParts.PointMisMatch",
                "Point in HPTRESULT data structure (%d,%d) does not match "
                "expected (%d,%d)",
  ...
  if (!ANKI_VERIFY(
          IsFirstPointOffsetSet(kFirstPointOffset),
          "FaceTrackerImpl.SetFacePoseFromParts.FirstPointOffSetNotSet", "")) {
    return RESULT_FAIL;
  }

  // What I'm about to do is terrible. But OKAO forced my hand by making their
  // HPTRESULT a void* and having it be the only way to get the roll, pitch, and
  // yaw of the face. I'm going to undistort the points internally so that I can
  // pass an undistorted HPTRESULT to GetFaceDirection in order to estimate an
  // improved set of rotation angles.
  ...
  for (INT32 i = 0; i < PT_POINT_KIND_MAX; ++i) {
    ...
    // This is to check that we have the kFirstPointOffset set correctly (as
    // well as the assumption that the rest of the x/y entries are contiguous
    // after that)
    DEV_ASSERT((distortedPoint.x == _facialParts[i].x) &&
                   (distortedPoint.y == _facialParts[i].y),
               "FaceTrackerImpl.SetFacePoseFromParts.BadPointIndexing");
  }

  // Undistort the part locations
  // TODO: consider using Vision::Undistorter for this (challenge: here relying
  // on vectors of cv::Point2f)
  ...
  // Fill in head orientation, using undistorted landmark locations so we are
  // more accurate
  INT32 roll_deg = 0, pitch_deg = 0, yaw_deg = 0;
  INT32 okaoDirResult = OKAO_PT_GetFaceDirection(
      _okaoPartDetectionResultHandle, &pitch_deg, &yaw_deg, &roll_deg);
  ...
  // Put back the original distorted points since the remainder of their usage
  // also needs corresponding image data, which we have _not_ undistorted (to
  ...
  if (OKAO_NORMAL != okaoDirResult) {
    LOG_WARNING(
        "FaceTrackerImpl.SetFacePoseFromParts.FaceLibGetFaceDirectionFail",
  ...
  face.SetHeadOrientation(DEG_TO_RAD(roll_deg), DEG_TO_RAD(pitch_deg),
                          DEG_TO_RAD(yaw_deg));
  ...
  const Point2f leftEye(undistortedParts[PT_POINT_LEFT_EYE].x,
                        undistortedParts[PT_POINT_LEFT_EYE].y);
  const Point2f rightEye(undistortedParts[PT_POINT_RIGHT_EYE].x,
                         undistortedParts[PT_POINT_RIGHT_EYE].y);
  intraEyeDist =
      std::max((leftEye - rightEye).Length(), MinDistBetweenEyes_pixels);
  ...
  // Use the eye positions and yaw-adjusted intra-eye distance to compute the
  // head's translation
  const Vec3f& T = GetTranslation(leftEye, rightEye, intraEyeDist, calib);
  Pose3d headPose = face.GetHeadPose();
  headPose.SetTranslation(T);

  // The okao coordindate system is based around the face instead of around the
  // robot and is different than the anki coordindate system. Specifically the
  // x-axis points out of the detected faces nose, the z-axis points of the top
  // of the detected faces head, and the y-axis points out of the left ear of
  // the detected face. Thus the Yaw and Roll angles map without change onto our
  // coordinate system, while the pitch needs to be negated to map correctly
  // from the okao coordinate system to the anki coordindate system.
  const RotationMatrix3d faceRotation(-face.GetHeadPitch(), face.GetHeadRoll(),
                                      face.GetHeadYaw());
  ...
  // This works very similar to the way that the face angles from okao work. The
  // gaze angles are relative to the image plane and are independent of the head
  // rotation angles. Thus to set this in our pose tree and world space we only
  // need update the default eye pose rotation matrix (which is looking
  // orthogonal towards the image plane) and the translation. For right now roll
  // angles are ignored, since that isn't a natural movement of the eye.
  // However, this could occur by the head rotation but since okao doesn't
  // handle this case, neither do we.
  Pose3d eyePose = face.GetEyePose();
  eyePose.SetTranslation(T);
...
void FaceTracker::Impl::SetCroppingMask(const INT32 nWidth, const INT32 nHeight,
                                        const float cropFactor) {
  DEV_ASSERT(Util::IsFltGTZero(cropFactor),
             "FaceTrackerImpl.SetCroppingMask.ZeroCropFactor");
  ...
  if (Util::IsFltLT(cropFactor, 1.f)) {
    ...
    rcEdgeMask.left =
        std::max(0, (INT32)std::round(0.5 * (1.f - cropFactor) * nWidth));
  ...
  if (OKAO_NORMAL != okaoResult) {
    LOG_WARNING("FaceTrackerImpl.SetCroppingMask.FaceLibSetEdgeMaskFail",
                "FaceLib Result Code=%d, Rect=[%d %d %d %d]", okaoResult,
                rcEdgeMask.left, rcEdgeMask.top, rcEdgeMask.right,
                rcEdgeMask.bottom);
  }

  if (DetectParams::kDetectionMode ==
      Okao::GetIndex(Okao::DetectionMode::Movie)) {
    // Tracking edge mask only applies in Movie mode
    okaoResult =
        OKAO_DT_MV_SetTrackingEdgeMask(_okaoDetectorHandle, rcEdgeMask);
    if (OKAO_NORMAL != okaoResult) {
      LOG_WARNING(
          "FaceTrackerImpl.SetCroppingMask.FaceLibSetTrackingEdgeMaskFail",
          "FaceLib Result Code=%d, Rect=[%d %d %d %d]", okaoResult,
          rcEdgeMask.left, rcEdgeMask.top, rcEdgeMask.right, rcEdgeMask.bottom);
    }
  }
}
...
Result FaceTracker::Impl::Update(const Vision::Image& frameOrig,
  ...
  DEV_ASSERT(frameOrig.IsContinuous(),
             "FaceTrackerImpl.Update.NonContinuousImage");
  ...
  okaoResult =
      OKAO_DT_Detect_GRAY(_okaoDetectorHandle, dataPtr, nWidth, nHeight,
  ...
  okaoResult =
      OKAO_DT_GetResultCount(_okaoDetectionResultHandle, &numDetections);
  if (OKAO_NORMAL != okaoResult) {
  ...
  // so that we can choose to run recognition more selectively in the loop
  // below, effectively prioritizing those we don't already recognize
  std::vector<INT32> detectionIndices(numDetections);
  std::set<INT32> skipRecognition;

  for (INT32 detectionIndex = 0; detectionIndex < numDetections;
       ++detectionIndex) {
    detectionIndices[detectionIndex] = detectionIndex;

    DETECTION_INFO detectionInfo;
    okaoResult = OKAO_DT_GetRawResultInfo(_okaoDetectionResultHandle,
                                          detectionIndex, &detectionInfo);
    ...
    const bool isEnrollmentTrackID =
        (_recognizer.GetEnrollmentTrackID() == detectionInfo.nID);
  ...
  for (auto const& detectionIndex : detectionIndices) {
    ...
    face.SetIsBeingTracked(detectionInfo.nDetectionMethod !=
                           DET_METHOD_DETECTED_HIGH);

    POINT ptLeftTop, ptRightTop, ptLeftBottom, ptRightBottom;
    okaoResult = OKAO_CO_ConvertCenterToSquare(
        detectionInfo.ptCenter, detectionInfo.nHeight, 0, &ptLeftTop,
    ...
    const bool doRecognition = _isRecognitionEnabled &&
                               !(skipRecognition.count(detectionInfo.nID) > 0);
    const bool doPartDetection =
        (_detectEmotion || _detectSmiling || _detectGaze || _detectBlinks ||
         doRecognition);
    ...
    if (doPartDetection) {
      ...
      facePartsFound =
          DetectFaceParts(nWidth, nHeight, dataPtr, detectionIndex, face);
      Toc("FacePartDetection");
    }

    // Will be computed from detected eyes if face parts are found, or "faked"
    // using face detection rectangle otherwise;
    ...
    if (facePartsFound) {
      ...
      if (_detectEmotion) {
        ...
        if (RESULT_OK != expResult) {
          LOG_WARNING("FaceTrackerImpl.Update.EstimateExpressionFailed",
                      "Detection index %d of %d.", detectionIndex,
                      numDetections);
        }
      }  // if(_detectEmotion)
      ...
      if (_detectGaze || _detectBlinks)  // In OKAO, gaze and blink are part of
                                         // the same detector
      {
      ...
      if (_detectGaze) {
        ...
        // There is a assert in there that should catch if the pose is
        // uninitialized but won't catch on going cases of the dependence.
      ...
      if (doRecognition) {
        ...
        // Second if we don't have the tracking id in the recognizer yet,
        // indicating we haven't recognized the face yet. If we don't have any
        ...
        _recognizer.SetNextFaceToRecognize(frameOrig, detectionInfo,
                                           _facialParts, _facialPartConfs,
                                           enableEnrollment);
        ...
        //                            "TrackingID %d already known and there are
        //                            %d faces detected", -detectionInfo.nID,
        //                            numDetections);
        //        }
      }

    } else {
      // NOTE: Without parts, we do not do eye contact, gaze, face recognition,
      // etc.

      SetFacePoseWithoutParts(nHeight, nWidth, face, intraEyeDist);
    }

    // Get whatever is the latest recognition information for the current
    // tracker ID
    s32 enrollmentCompleted = 0;
    auto recognitionData = _recognizer.GetRecognitionData(
        detectionInfo.nID, enrollmentCompleted, debugImages);

    face.SetBestGuessName(
        _recognizer.GetBestGuessNameForTrackingID(detectionInfo.nID));
    ...
    if (recognitionData.WasFaceIDJustUpdated()) {
      ...
      UpdatedFaceID update{
          .oldID = (recognitionData.GetPreviousFaceID() == UnknownFaceID
                        ? -detectionInfo.nID
                        : recognitionData.GetPreviousFaceID()),
    ...
    if (recognitionData.GetFaceID() != UnknownFaceID &&
        recognitionData.GetTrackingID() !=
            recognitionData.GetPreviousTrackingID()) {
      ...
      UpdatedFaceID update{.oldID = -recognitionData.GetTrackingID(),
                           .newID = recognitionData.GetFaceID(),
                           .newName = recognitionData.GetName()};

      // Don't send this update if it turns out to contain the same info as
      // the last one (even if for different reasons)
      if (updatedIDs.empty() || (update.oldID != updatedIDs.back().oldID &&
    ...
    if (UnknownFaceID == recognitionData.GetFaceID()) {
      ...
      DEV_ASSERT(detectionInfo.nID > 0,
                 "FaceTrackerImpl.Update.InvalidTrackerID");
...
Result FaceTracker::Impl::AssignNameToID(FaceID_t faceID,
...
void FaceTracker::Impl::EraseAllFaces() { _recognizer.EraseAllFaces(); }

std::vector<Vision::LoadedKnownFace> FaceTracker::Impl::GetEnrolledNames()
    const {
...
Result FaceTracker::Impl::RenameFace(
...
bool FaceTracker::Impl::IsEnrollable(const DETECTION_INFO& detectionInfo,
                                     const TrackedFace& face,
                                     const f32 intraEyeDist) {
#define DEBUG_ENROLLABILITY 0

  if (detectionInfo.nConfidence > FaceEnrollParams::kMinDetectionConfidence &&
      detectionInfo.nPose == POSE_YAW_FRONT && face.IsFacingCamera() &&
    ...
    LOG_DEBUG("FaceTrackerImpl.IsEnrollable.NotLookingStraight",
              "EyeDist=%.1f (vs. %.1f)", intraEyeDist,
              FaceEnrollParams::kFarDistanceBetweenEyesMin);
  }
...
Result FaceTracker::Impl::SetSerializedData(
...
Result FaceTracker::Impl::DevAddFaceToAlbum(const Image& img,
                                            const TrackedFace& face,
                                            int albumEntry) {
  return _recognizer.DevAddFaceToAlbum(img, face, albumEntry);
}

Result FaceTracker::Impl::DevFindFaceInAlbum(const Image& img,
                                             const TrackedFace& face,
                                             int& albumEntry,
                                             float& score) const {
...
Result FaceTracker::Impl::DevFindFaceInAlbum(
...
float FaceTracker::Impl::DevComputePairwiseMatchScore(int faceID1,
                                                      int faceID2) const {
  return _recognizer.DevComputePairwiseMatchScore(faceID1, faceID2);
}

float FaceTracker::Impl::DevComputePairwiseMatchScore(
...
#if ANKI_DEV_CHEATS
void FaceTracker::Impl::SaveAllRecognitionImages(
