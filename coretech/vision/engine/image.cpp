...
template <typename T>
void ResizeKeepAspectRatioHelper(const cv::Mat_<T>& src, cv::Mat_<T>& dest,
                                 s32 desiredCols, s32 desiredRows, int method,
                                 bool onlyReduceSize) {
  const double ratio =
      double(src.rows) /
      double(src.cols);  // without the double it's the nastiest bug!!
  ...
  if (onlyReduceSize && src.rows < desiredSize.height &&
      src.cols < desiredSize.width) {
    // Source is already smaller than the desired size in both dimensions, and
    // onlyReduceSize was specifed, so don't resize (i.e. don't make small src
    // bigger)
    dest = src;
  } else {
    try {
      ...
      PRINT_NAMED_ERROR("ResizeKeepAspectRatioHelper.CvResizeException",
                        "Error while resizing image: %s,"
                        "ratio %f, rows: %d, cols: %d, desiredSize: (%d, %d)",
                        e.what(), ratio, src.rows, src.cols, desiredSize.width,
                        desiredSize.height);
      return;
    }
  }
...
Result ImageBase<T>::Load(const std::string& filename) {
  ...
  if (ext == "bmp" || ext == "BMP") {
    if (GetNumChannels() == 3) {
      // Converts to RGB internally (not BGR), so no need to use the
      // SetFromShowableFormat call below
    ...
    try {
      showableImage =
          cv::imread(filename, (GetNumChannels() == 1 ? CV_LOAD_IMAGE_GRAYSCALE
                                                      : CV_LOAD_IMAGE_COLOR));
    } catch (const cv::Exception& e) {
      PRINT_NAMED_ERROR("ImageBase.Load.CvImreadFailed", "OpenCV Error: %s",
                        e.what());
      return RESULT_FAIL;
    }
  ...
  try {
    ...
    if (!success) {
      PRINT_NAMED_WARNING("ImageBase.Save.ImwriteFailed",
                          "Failed writing %dx%d image to %s", GetNumCols(),
                          GetNumRows(), filename.c_str());
      return RESULT_FAIL;
    }
  } catch (cv::Exception& ex) {
    PRINT_NAMED_WARNING("ImageBase.Save.ImwriteException",
                        "Failed writing to %s: %s", filename.c_str(),
                        ex.what());
    return RESULT_FAIL;
  }
...
template <typename T>
void ImageBase<T>::DrawCircle(const Point2f& center, const ColorRGBA& color,
                              const s32 radius, const s32 thickness) {
  cv::circle(this->get_CvMat_(), center.get_CvPoint_(), radius,
             GetCvColor(color), thickness);
}

template <typename T>
void ImageBase<T>::DrawFilledCircle(const Point2f& center,
                                    const ColorRGBA& color, const s32 radius) {
  cv::circle(this->get_CvMat_(), center.get_CvPoint_(), radius,
             GetCvColor(color), CV_FILLED);
}

template <typename T>
void ImageBase<T>::DrawRect(const Rectangle<f32>& rect, const ColorRGBA& color,
                            const s32 thickness) {
  cv::rectangle(this->get_CvMat_(), rect.get_CvRect_(), GetCvColor(color),
                thickness);
}

template <typename T>
void ImageBase<T>::DrawFilledRect(const Rectangle<f32>& rect,
...
template <typename T>
void ImageBase<T>::DrawRect(const Rectangle<s32>& rect, const ColorRGBA& color,
                            const s32 thickness) {
  cv::rectangle(this->get_CvMat_(), rect.get_CvRect_(), GetCvColor(color),
                thickness);
}
...
template <typename T>
void ImageBase<T>::DrawFilledRect(const Rectangle<s32>& rect,
                                  const ColorRGBA& color) {
  cv::rectangle(this->get_CvMat_(), rect.get_CvRect_(), GetCvColor(color),
                CV_FILLED);
}
...
template <typename T>
void ImageBase<T>::DrawQuad(const Quad2f& quad, const ColorRGBA& color,
                            const s32 thickness) {
  using namespace Quad;
  DrawLine(quad[CornerName::TopLeft], quad[CornerName::TopRight], color,
           thickness);
  DrawLine(quad[CornerName::TopLeft], quad[CornerName::BottomLeft], color,
           thickness);
  DrawLine(quad[CornerName::TopRight], quad[CornerName::BottomRight], color,
           thickness);
  DrawLine(quad[CornerName::BottomLeft], quad[CornerName::BottomRight], color,
           thickness);
}
...
static void DrawPolyHelper(ImageBase<T>* img, const cv::Mat& ptsMat,
  ...
  try {
    ...
    PRINT_NAMED_ERROR("ImageBase.DrawPolyHelper.OpenCvPolylinesFailed", "%s",
                      e.what());
    return;
  }
}
...
template <typename T>
void ImageBase<T>::DrawPoly(const Poly2f& poly, const ColorRGBA& color,
                            const s32 thickness, const bool closed) {
...
template <typename T>
void ImageBase<T>::DrawPoly(const Poly2i& poly, const ColorRGBA& color,
                            const s32 thickness, const bool closed) {
...
template <typename T>
void ImageBase<T>::DrawFilledConvexPolygon(const std::vector<Point2i> points,
                                           const ColorRGBA& color) {
...
void ImageBase<T>::DrawText(const Point2f& position, const std::string& str,
  ...
  if (centered) {
    const cv::Size textSize =
        cv::getTextSize(str, CV_FONT_NORMAL, scale, thickness, nullptr);
  ...
  if (dropShadow) {
    ...
    shadowPos.x += 1;
    shadowPos.y += 1;
    cv::putText(this->get_CvMat_(), str, shadowPos, CV_FONT_NORMAL, scale,
                GetCvColor(NamedColors::BLACK), thickness);
  }
  cv::putText(this->get_CvMat_(), str, alignedPos.get_CvPoint_(),
              CV_FONT_NORMAL, scale, GetCvColor(color), thickness);
}

template <typename T>
Vec2f ImageBase<T>::GetTextSize(const std::string& str, f32 scale,
                                int thickness) {
...
template <typename T>
void ImageBase<T>::MakeTextFillImageWidth(const std::string& text, int font,
                                          int thickness, int imageWidth,
                                          cv::Size& textSize, float& scale) {
  scale = 0.1f;
  cv::Size prevTextSize(0, 0);
  // TODO Use binary search instead
  for (; scale < 3.f; scale += 0.05f) {
    textSize = cv::getTextSize(text, font, scale, 1, nullptr);
...
template <typename T>
void ImageBase<T>::DrawTextCenteredHorizontally(
    const std::string& text, int font, float scale, int thickness,
    const ColorRGBA& color, int verticalPos, bool drawTwiceToMaybeFillGaps) {
  int baseline = 0;
  cv::Size textSize = cv::getTextSize(text, font, scale, thickness, &baseline);

  Point2f p((this->GetNumCols() - textSize.width) / 2, verticalPos);

  this->DrawText(p, text, color, scale, false, thickness);
...
void ImageBase<T>::Resize(f32 scaleFactor, ResizeMethod method) {
  cv::resize(this->get_CvMat_(), this->get_CvMat_(), cv::Size(), scaleFactor,
             scaleFactor, GetOpenCvInterpMethod(method));
}
...
template <typename T>
void ImageBase<T>::Resize(ImageBase<T>& resizedImage,
                          ResizeMethod method) const {
  if (resizedImage.IsEmpty()) {
    PRINT_NAMED_ERROR("Image.Resize.EmptyImage",
                      "Output image should already have the desired size");
  } else {
    const cv::Size desiredSize(resizedImage.GetNumCols(),
                               resizedImage.GetNumRows());
...
template <typename T>
void ImageBase<T>::ResizeKeepAspectRatio(s32 desiredRows, s32 desiredCols,
                                         ResizeMethod method, bool onlyReduce) {
  if (desiredRows != GetNumRows() || desiredCols != GetNumCols()) {
    ResizeKeepAspectRatioHelper(this->get_CvMat_(), this->get_CvMat_(),
...
template <typename T>
void ImageBase<T>::ResizeKeepAspectRatio(ImageBase<T>& resizedImage,
                                         ResizeMethod method) const {
  if (resizedImage.IsEmpty()) {
    PRINT_NAMED_ERROR("Image.ResizeKeepAspectRatio.EmptyImage",
                      "Output image should already have the desired size");
    return;
  }
  ...
  ResizeKeepAspectRatioHelper(this->get_CvMat_(), resizedImage.get_CvMat_(),
                              desiredCols, desiredRows,
                              GetOpenCvInterpMethod(method), false);
  resizedImage.SetTimestamp(this->GetTimestamp());
  resizedImage.SetImageId(this->GetImageId());
}
...
void ImageBase<T>::CopyTo(ImageBase<T>& otherImage) const {
  ...
  otherImage.SetTimestamp(
      GetTimestamp());  // Make sure timestamp gets copied too
...
void ImageBase<T>::BoxFilter(ImageBase<T>& filtered, u32 size) const {
  try {
    cv::boxFilter(this->get_CvMat_(), filtered.get_CvMat_(), -1,
                  cv::Size(size, size));
  } catch (cv::Exception& e) {
    PRINT_NAMED_ERROR("ImageBase.BoxFilter.OpenCvBoxFilterFailed", "%s",
                      e.what());
    this->CopyTo(
        filtered);  // Just put the unfiltered source into the destination
  }
}

template <typename T>
void ImageBase<T>::Undistort(const CameraCalibration& calib,
                             ImageBase<T>& undistortedImage) const {
  DEV_ASSERT((this != &undistortedImage) &&
                 (this->GetDataPointer() != undistortedImage.GetDataPointer()),
  ...
  const CameraCalibration scaledCalib =
      calib.GetScaled(GetNumRows(), GetNumCols());
  try {
    ...
    PRINT_NAMED_ERROR("ImageBase.Undistort.OpenCvUndistortFailed", "%s",
                      e.what());
  }
}
...
Image::Image() : ImageBase<u8>() {}

Image::Image(s32 nrows, s32 ncols) : ImageBase<u8>(nrows, ncols) {}
...
Image::Image(const Array2d<u8>& array2d) : ImageBase<u8>(array2d) {}

#if ANKICORETECH_USE_OPENCV
Image::Image(cv::Mat_<u8>& cvMat) : ImageBase<u8>(cvMat) {}
#endif
...
s32 Image::GetConnectedComponents(Array2d<s32>& labelImage) const {
  const s32 count =
      cv::connectedComponents(this->get_CvMat_(), labelImage.get_CvMat_());
  return count;
}  // GetConnectedComponents()

s32 Image::GetConnectedComponents(
    Array2d<s32>& labelImage,
    std::vector<ConnectedComponentStats>& stats) const {
  cv::Mat cvStats, cvCentroids;
  const s32 count = cv::connectedComponentsWithStats(
      this->get_CvMat_(), labelImage.get_CvMat_(), cvStats, cvCentroids);
  for (s32 iComp = 0; iComp < count; ++iComp) {
    ...
    ConnectedComponentStats stat{
        ...
        .centroid =
            Point2f(Util::Clamp(compCentroid[0], 0.0, (f64)(GetNumCols() - 1)),
                    Util::Clamp(compCentroid[1], 0.0, (f64)(GetNumRows() - 1))),
        .boundingBox = Rectangle<s32>(
            compStats[cv::CC_STAT_LEFT], compStats[cv::CC_STAT_TOP],
...
inline cv::Scalar Image::GetCvColor(const ColorRGBA& color) const {
  ...
  u8 gray = static_cast<u8>(0.2989f * color.r()) + (0.5870f * color.g()) +
            (0.1140f * color.b());
...
void Image::SetFromShowableFormat(const cv::Mat& showImg) {
  DEV_ASSERT(showImg.channels() == 1,
             "Image.SetFromShowableFormat.UnexpectedNumChannels");
...
#define FILTER_ROW(row1Ptr, row2Ptr, row3Ptr, outputPtr)                 \
  {                                                                      \
    ...
    for (; c < GetNumCols() - 1; c++) {                                  \
      ...
      row3Ptr++;                                                         \
                                                                         \
      *outputPtr = (u8)(((f32) * (row1Ptr - 1) + (f32) * (row1Ptr) +     \
                         (f32) * (row1Ptr + 1) + (f32) * (row2Ptr - 1) + \
                         (f32) * (row2Ptr) + (f32) * (row2Ptr + 1) +     \
                         (f32) * (row3Ptr - 1) + (f32) * (row3Ptr) +     \
                         (f32) * (row3Ptr + 1)) *                        \
                        (1 / 9.f));                                      \
                                                                         \
      outputPtr++;                                                       \
  ...
  // Filter the left and right edge of the image using mirroring
  {
    ...
    filteredRow[0] = (u8)(((f32)(curRow[0]) + (f32)(curRow[1] * 2) +
                           (f32)(nextRow[0] * 2) + (f32)(nextRow[1] * 4)) *
                          (1 / 9.f));

    const u32 lastCols = GetNumCols() - 1;
    filteredRow[lastCols] =
        (u8)(((f32)(curRow[lastCols]) + (f32)(curRow[lastCols - 1] * 2) +
              (f32)(nextRow[lastCols] * 2) + (f32)(nextRow[lastCols - 1] * 4)) *
             (1 / 9.f));
    ...
    for (; r < GetNumRows() - 1; r++) {
      ...
      filteredRow[0] = (u8)(((f32)(curRow[0]) + (f32)(prevRow[0]) +
                             (f32)(nextRow[0]) + (f32)(curRow[1] * 2) +
                             (f32)(prevRow[1] * 2) + (f32)(nextRow[1] * 2)) *
                            (1 / 9.f));

      filteredRow[lastCols] =
          (u8)(((f32)(curRow[lastCols]) + (f32)(prevRow[lastCols]) +
                (f32)(nextRow[lastCols]) + (f32)(curRow[lastCols - 1] * 2) +
                (f32)(prevRow[lastCols - 1] * 2) +
                (f32)(nextRow[lastCols - 1] * 2)) *
               (1 / 9.f));
    }
    ...
    filteredRow[0] = (u8)(((f32)(curRow[0]) + (f32)(curRow[1] * 2) +
                           (f32)(prevRow[0] * 2) + (f32)(prevRow[1] * 4)) *
                          (1 / 9.f));

    filteredRow[lastCols] =
        (u8)(((f32)(curRow[lastCols]) + (f32)(curRow[lastCols - 1] * 2) +
              (f32)(prevRow[lastCols] * 2) + (f32)(prevRow[lastCols - 1] * 4)) *
             (1 / 9.f));
  }
}
...
void Image::ConvertV2RGB565(u8 hue, u8 sat, ImageRGB565& output) {
  ...
  // from V, p, q, or t. index holds which sector each pixel's hue is in and is
  // used to index into the tables held by rp,rq,rt gp,gq,gt and bp,bq,bt. The
  // result of the look up will either be all 0s or all 1s. If all 1s, then for
  // this sector the variable corresponding to the table should be used for this
  // channel. Ex: HSV = (0, 1, 1) which is sector 0 so r = V, g = t, b = p rp[0]
  // == 0s, rq[0] == 0s, rt[0] == 0s  r is set from V gp[0] == 0s, gq[0] == 0s,
  // gt[0] == 1s  g is set from t bp[0] == 1s, bq[0] == 0s, bt[0] == 0s  b is
  // set from p
...
#ifdef __ARM_NEON__
    for (; c < numCols - (kNumElementsProcessedPerLoop - 1);
         c += kNumElementsProcessedPerLoop) {
      ...
      // Depending on which of the 6 sectors hue falls in r,g,b may either be
      // set from V, p, q, or t. index holds which sector each pixel's hue is in
      // and is used to index into the tables held by rp,rq,rt gp,gq,gt and
      // bp,bq,bt. The result of the look up will either be all 0s or all 1s. If
      // all 1s, then for this sector the variable corresponding to the table
      // should be used for this channel. Ex: HSV = (0, 1, 1) which is sector 0
      // so r = V, g = t, b = p rp[0] == 0s, rq[0] == 0s, rt[0] == 0s  r is set
      // from V gp[0] == 0s, gq[0] == 0s, gt[0] == 1s  g is set from t bp[0] ==
      // 1s, bq[0] == 0s, bt[0] == 0s  b is set from p
      ...
      // Shift green bits by 5 and insert into red vector this is the RG56 of
      // RGB565
...
ImageRGBA::ImageRGBA() : ImageBase<PixelRGBA>() {}
...
void ImageRGBA::FillGray(Image& grayImage) const {
  grayImage.SetTimestamp(
      GetTimestamp());  // Make sure timestamp gets transferred!
...
ImageRGBA& ImageRGBA::SetFromRGB565(const ImageRGB565& rgb565, const u8 alpha) {
  // Note: not equivalent to cv::cvtColor(rgb565.get_CvMat_(),
  // this->get_CvMat_(), cv::COLOR_BGR5652RGBA);
...
ImageRGB::ImageRGB() : ImageBase<PixelRGB>() {}

ImageRGB::ImageRGB(s32 nrows, s32 ncols) : ImageBase<PixelRGB>(nrows, ncols) {}
...
ImageRGB& ImageRGB::SetFromRGB565(const ImageRGB565& rgb565) {
  // Similar to how COLOR_BGR5652BGR appears to be swapping R and B in
  // ConvertToShowableFormat(), COLOR_BGR5652RGB here appears not to, which is
  // what we want.
...
void ImageRGB::FillGray(Image& grayOut,
                        const ImageRGB::RGBToGrayMethod method) const {
  switch (method) {
    case RGBToGrayMethod::DoubleGreen:
...
void ImageRGB::FillGrayFromDoubleGreen(Anki::Vision::Image& grayImage) const {
  ...
  for (u32 i = 0; i < numRows; i++) {
    ...
    for (; j < kNumIterations; j += kNumElementsProcessedPerLoop) {
      ...
      uint8x8_t avg = vshrn_n_u16(
          out, 2);  // Narrowing shift right (divide by 4 and convert to u8)
    ...
    const Vision::PixelRGB* imageRGBPtr =
        reinterpret_cast<const Vision::PixelRGB*>(imagePtr);

    for (; j < numCols; j++) {
      *grayPtr = (((u16)imageRGBPtr->r() + (((u16)imageRGBPtr->g()) << 1) +
                   (u16)imageRGBPtr->b()) >>
                  2);
...
void ImageRGB::FillGrayFromGreenChannel(Image& grayImage) const {
  ...
  try {
    cv::extractChannel(get_CvMat_(), cvGrayImage,
                       1);  // gets green channel from BGR (or RGB!)
  } catch (const cv::Exception& e) {
...
Image ImageRGB::Threshold(u8 value, bool anyChannel) const {
  std::function<u8(const PixelRGB&)> thresholdFcn =
      [value, anyChannel](const PixelRGB& p) {
...
void ImageRGB::GetNormalizedColor(ImageRGB& imgNorm,
                                  Array2d<s32>* workingArray) const {
  this->CopyTo(
      imgNorm);  // makes data continuous, which is required for reshape

  DEV_ASSERT(imgNorm.IsContinuous(),
             "ImageRGB.GetNormalizedColor.NotContinuous");
  ...
  try {
    ...
    PRINT_NAMED_ERROR("ImageRGB.GetNormalizedColor.OpenCvReshapeFailed", "%s",
                      e.what());
    return;
  }
  ...
  try {
    ...
    PRINT_NAMED_ERROR("ImageRGB.GetNormalizedColor.OpenCvReduceFailed", "%s",
                      e.what());
    return;
  }

  // Scale each row by 255 and divide by the sum, placing the result directly
  // into output data
  // TODO: Avoid the repeat?
  try {
    cv::divide(imageVector, cv::repeat(imageSum, 1, imageVector.cols),
               imageVector, 255.0, CV_8UC1);
  } catch (cv::Exception& e) {
    PRINT_NAMED_ERROR("ImageRGB.GetNormalizedColor.OpenCvDivideFailed", "%s",
                      e.what());
    return;
  }
}
...
void ImageRGB::SetFromShowableFormat(const cv::Mat& showImg) {
  DEV_ASSERT(showImg.channels() == 3,
             "ImageRGB.SetFromShowableFormat.UnexpectedNumChannels");
...
ImageRGB565::ImageRGB565() : ImageBase<PixelRGB565>() {}
...
ImageRGB565& ImageRGB565::SetFromImageRGB(const ImageRGB& imageRGB) {
  ...
  // Similar to how COLOR_BGR5652BGR appears to be swapping R and B in
  // ConvertToShowableFormat(), COLOR_RGB2BGR565 here appears not to, which is
  // what we want.
...
ImageRGB565& ImageRGB565::SetFromImageRGB(const ImageRGB& imageRGB,
                                          const std::array<u8, 256>& gammaLUT) {
  Allocate(imageRGB.GetNumRows(), imageRGB.GetNumCols());

  std::function<PixelRGB565(const PixelRGB&)> convertFcn =
      [&gammaLUT](const PixelRGB& pixRGB) {
        PixelRGB565 pixRGB565(gammaLUT[pixRGB.r()], gammaLUT[pixRGB.g()],
                              gammaLUT[pixRGB.b()]);
        return pixRGB565;
      };
...
ImageRGB565& ImageRGB565::SetFromVector(const std::vector<u16>& externalData) {
  ...
  DEV_ASSERT(externalData.size() == pixelCount,
             "ImageRGB565.SetFromShowableFormat.UnexpectedNumChannels");
