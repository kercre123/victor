...
class Undistorter {
 public:
  ...
  // Pre-cache undistortion maps for a given image size (does nothing if they
  // already exist)
  Result CacheUndistortionMaps(s32 nrows, s32 ncols);

  // TODO: Provide ability to load from / save to disk (question: how to store
  // per calibration?)
  // Result Save(const std::string& filename);
  // Result Load(const std::string& filename);

  // Will compute and cache distortion maps if needed, or used pre-cached ones
  // if available
  ...
  // Undistort one or more points from an (nrows x ncols) image. Scales
  ...
  // Static versions if you just want to pass in the calibration instead of
  // having an instantiated Undistorter laying around
 ...
 private:
  ...
  template <class T>
  Result UndistortImageHelper(const ImageBase<T>& img,
...
inline Result Undistorter::UndistortPoint(const s32 nrows, const s32 ncols,
  ...
  return Undistorter::UndistortPoint(_calib, nrows, ncols, distortedPointIn,
                                     undistortedPointOut);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
inline Result Undistorter::UndistortPoints(
    const s32 nrows, const s32 ncols,
    const std::vector<Point2f>& distortedPointsIn,
    std::vector<Point2f>& undistortedPointsOut) {
  // Just call static version with member calibration
  return Undistorter::UndistortPoints(_calib, nrows, ncols, distortedPointsIn,
                                      undistortedPointsOut);
}
