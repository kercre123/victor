 ...
 * Date:   7/26/2016
 *
 * Description: Tracks total and recent frame drops and prints statistics for
 *them.
 *
  ...
  void Update(bool isDroppingFrame) {
    ...
    if (_numRecentFrames == _recentN) {
      ...
      Util::sInfoF("robot.vision.dropped_frame_overall_count",
                   {{DDATA, std::to_string(_numFrames).c_str()}}, "%u",
                   _numTotalDrops);

      Util::sInfoF("robot.vision.dropped_frame_recent_count",
                   {{DDATA, std::to_string(_numRecentFrames).c_str()}}, "%u",
                   _numRecentDrops);

      if (_numRecentDrops > 0) {
        PRINT_CH_INFO(
            _channelName, "DroppedFrameStats",
            "Dropped %u of %u total images (%.1f%%), %u of last %u (%.1f%%)",
            _numTotalDrops, _numFrames,
            (f32)_numTotalDrops / (f32)_numFrames * 100.f, _numRecentDrops,
            _numRecentFrames,
