 ...
 * Date:   8/20/2015
 *
 * Description: A container for a tracked face and any features (e.g. eyes,
 *mouth, ...) related to it.
 *
...
class TrackedFace {
 public:
  ...
  // in some situations when we're willing to be less sure about someone's
  // identity.
  ...
  void SetFeature(FeatureName whichFeature, Feature&& points,
                  FeatureConfidence&& confidences);

  void SetEyeCenters(Point2f&& leftCen, Point2f&& rightCen);

  // Returns true if eye centers have been specified via SetEyeCenters, false
  // otherwise
  bool HasEyes() const { return _eyesDetected; }

  // Returns true if the face has eyes set and populates the left and right
  // centers Returns false and does not change leftCen/rightCen if eyes were
  // never set GetIntraEyeDistance will undistort the eye locations according to
  // the camera's calibration (if available)
  ...
  // These are w.r.t. the original observer (i.e. the camera at observation
  // time)
  ...
  // This is the gaze direction computed using the head pose rotation matrix,
  // and then projected onto the ground plane
  ...
  using ExpressionValues =
      std::array<ExpressionValue, (size_t)FacialExpression::Count>;
  const ExpressionValues& GetExpressionValues() const;

  // Return the expression with highest value (and optionally, its score if
  // valuePtr != nullptr) (If the returned expression is Unknown, the returned
  // value will be -1)
  FacialExpression GetMaxExpression(ExpressionValue* valuePtr = nullptr) const;

  // Set a particular expression value
  void SetExpressionValue(FacialExpression whichExpression,
...
inline bool TrackedFace::IsBeingTracked() const { return _isBeingTracked; }

inline void TrackedFace::SetIsBeingTracked(bool tf) { _isBeingTracked = tf; }

inline TimeStamp_t TrackedFace::GetTimeStamp() const { return _timestamp; }

inline void TrackedFace::SetTimeStamp(TimeStamp_t timestamp) {
  _timestamp = timestamp;
}

inline const Rectangle<f32>& TrackedFace::GetRect() const { return _rect; }

inline float TrackedFace::GetScore() const { return _score; }

inline void TrackedFace::SetScore(float score) { _score = score; }

inline FaceID_t TrackedFace::GetID() const { return _id; }

inline void TrackedFace::SetID(FaceID_t newID) { _id = newID; }

inline s32 TrackedFace::GetNumEnrollments() const { return _numEnrollments; }

inline void TrackedFace::SetNumEnrollments(s32 N) { _numEnrollments = N; }
...
inline void TrackedFace::SetFeature(FeatureName whichFeature, Feature&& points,
                                    FeatureConfidence&& confs) {
  DEV_ASSERT(points.size() == confs.size(),
             "TrackedFace.SetFeature.MisMatchedPointsAndConfidences");
  _features[whichFeature] = points;
  _featureConfidences[whichFeature] = confs;
}

inline const TrackedFace::Feature& TrackedFace::GetFeature(
    TrackedFace::FeatureName whichFeature) const {
  return _features[whichFeature];
}

inline const TrackedFace::FeatureConfidence& TrackedFace::GetFeatureConfidence(
    FeatureName whichFeature) const {
  return _featureConfidences[whichFeature];
}

inline void TrackedFace::AddPointToFeature(FeatureName whichFeature,
                                           Point2f&& point) {
...
inline bool TrackedFace::GetEyeCenters(Point2f& leftCen,
                                       Point2f& rightCen) const {
  if (HasEyes()) {
    leftCen = _leftEyeCen;
...
inline void TrackedFace::SetRect(Rectangle<f32>&& rect) { _rect = rect; }

inline Radians TrackedFace::GetHeadYaw() const { return _yaw; }

inline Radians TrackedFace::GetHeadPitch() const { return _pitch; }

inline Radians TrackedFace::GetHeadRoll() const { return _roll; }

inline void TrackedFace::SetHeadOrientation(Radians roll, Radians pitch,
                                            Radians yaw) {
  _roll = roll;
  _pitch = pitch;
  _yaw = yaw;
  // PRINT_NAMED_INFO("TrackedFace.SetHeadOrientation", "Roll=%.1fdeg,
  // Pitch=%.1fdeg, Yaw=%.1fdeg",
  //                 _roll.getDegrees(), _pitch.getDegrees(),
  //                 _yaw.getDegrees());
}

inline const Pose3d& TrackedFace::GetHeadPose() const { return _headPose; }
...
inline const Pose3d& TrackedFace::GetEyePose() const { return _eyePose; }

inline void TrackedFace::SetEyePose(Pose3d& pose) { _eyePose = pose; }

inline const std::string& TrackedFace::GetName() const { return _name; }

inline bool TrackedFace::HasName() const { return !_name.empty(); }

inline bool TrackedFace::IsFacingCamera() const { return _isFacingCamera; }

inline void TrackedFace::SetIsFacingCamera(bool tf) { _isFacingCamera = tf; }
...
inline void TrackedFace::SetRecognitionDebugInfo(
    const std::list<FaceRecognitionMatch>& info) {
  _debugRecognitionInfo = info;
}

inline const std::list<FaceRecognitionMatch>&
TrackedFace::GetRecognitionDebugInfo() const {
...
inline void TrackedFace::SetGazeDirectionStable(
    const bool isGazeDirectionStable) {
