...
class FaceRecognizer : public Profiler {
 public:
  ...
  bool SetNextFaceToRecognize(
      const Vision::Image& img, const DETECTION_INFO& detectionInfo,
  ...
  // Note that this will take effect on the next call to GetRecognitionData
  // below (I.e., this just "queues" the clear to help prevent race conditions
  ...
  EnrolledFaceEntry GetRecognitionData(
      TrackingID_t forTrackingID, s32& enrollmentCountReached,
  ...
  // Populates the list of names and IDs recovered from the serialized data on
  // success
  Result SetSerializedData(const std::vector<u8>& albumData,
                           const std::vector<u8>& enrollData,
                           std::list<LoadedKnownFace>& loadedFaces);

  bool GetFaceIDFromTrackingID(const TrackingID_t trackingID,
                               FaceID_t& faceID) const;

  std::string GetBestGuessNameForTrackingID(
  ...
  // Adds a face to the album using its already-extracted features/confidences,
  // using the next available slot for the given album entry. Fails if album or
  // specific entry is full.
  Result DevAddFaceToAlbum(const Image& img, const TrackedFace& face,
                           int albumEntry);

  // Identify the given face, using enrollments present in the album and puts
  // matched ID and score in 'albumEntry' and 'score'. Note: will always find a
  // match (in the event of RESULT_OK); it's the caller's job to compare to a
  // threshold.
  Result DevFindFaceInAlbum(const Image& img, const TrackedFace& face,
                            int& albumEntry, float& score) const;

  // Same as above, but returns up to maxMatches pairs of album entries and
  // corresponding scores, in decreasing score order
  Result DevFindFaceInAlbum(const Image& img, const TrackedFace& face,
  ...
  float DevComputePairwiseMatchScore(int faceID1, const Image& img,
                                     const TrackedFace& face) const;
  float DevComputePairwiseMatchScore(const Image& img1,
  ...
  // /path/<filename_prefix>_<face_id>_<album_entry_id>_<timestamp>.jpg.
  // Currently the debug images are cropped to only contain the region of the
  // image where there was a face detection. The container of images is
  // populated by enabling kGatherDebugEnrollmentImages which by default saves
  // the croppped images at the same resolution that recognition occurs at. If
  // kDisplayDebugEnrollmentImages is enabled the cropped images with be saved
  // at kEnrollmentThumbnailSize.
 ...
 private:
  ...
  Result UpdateExistingAlbumEntry(AlbumEntryID_t albumEntry, HFEATURE& hFeature,
  ...
  Result SelectiveMergeHelper(EnrollmentData::iterator keepIter,
  ...
  static Result SetSerializedAlbum(HCOMMON okaoCommonHandle,
                                   const std::vector<u8>& serializedAlbum,
                                   HALBUM& album);

  Result GetSerializedEnrollData(std::vector<u8>& serializedEnrollData);

  static Result SetSerializedEnrollData(
  ...
  static Result SanityCheckBookkeeping(
  ...
  // TODO: Using Util::numeric_cast here would be nice, but its not (always)
  // constexpr...
  static constexpr s32 kMaxNamedFacesInAlbum =
      (s32)FaceRecognitionConstants::MaxNumFacesInAlbum;
  static constexpr s32 kMaxAlbumEntriesPerFace =
      (s32)FaceRecognitionConstants::MaxNumAlbumEntriesPerFace;
  static constexpr s32 kMaxEnrollDataPerAlbumEntry =
      (s32)FaceRecognitionConstants::MaxNumEnrollDataPerAlbumEntry;
  ...
  static_assert((kMaxNamedFacesInAlbum + kMinSessionOnlyFaces) *
                        kMaxAlbumEntriesPerFace <=
                    kMaxTotalAlbumEntries,
                "Combination of min/max face parameters too large for OKAO "
                "Library (Max is 1000).");
  static_assert(
      kMaxEnrollDataPerAlbumEntry <= 10,
  ...
  // Which face we are allowed to add enrollment data for (UnknownFaceID ==
  // "any" face), and how many enrollments we are allowed to add ( <0 means as
  // many as we want)
  ...
  std::map<AlbumEntryID_t,
           std::array<Vision::Image, kMaxEnrollDataPerAlbumEntry>>
      _enrollmentImages;
  void SetEnrollmentImage(AlbumEntryID_t albumEntry, s32 dataEntry);
  void DisplayEnrollmentImages(
      DebugImageList<CompressedImage>& debugImages) const;
  void DisplayMatchImages(
      const INT32 resultNum,
      const std::vector<AlbumEntryID_t>& matchingAlbumEntries,
      const std::vector<RecognitionScore>& scores,
      DebugImageList<CompressedImage>& debugImages);

  static Result ComputeFeaturesFromFace(const Image& img,
