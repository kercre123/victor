 ...
 * Date:   10/20/2016
 *
 * Description: Implements a histogram sprecifically for image brightness: 256
 *bins.
 *
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - -
Result ImageBrightnessHistogram::FillFromImage(const Image& img, s32 subSample,
                                               TransformFcn transformFcn) {
  if (subSample <= 0) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - -
Result ImageBrightnessHistogram::FillFromImage(const Image& img,
                                               const Image& weights,
                                               s32 subSample,
                                               TransformFcn transformFcn) {
  if (weights.GetNumRows() != img.GetNumRows() ||
    ...
    PRINT_NAMED_ERROR(
        "ImageBrightnessHistogram.FillFromImage.InvalidWeightMaskSize",
        "Weight mask [%dx%d] not same size as image [%dx%d]",
        weights.GetNumCols(), weights.GetNumRows(), img.GetNumCols(),
        img.GetNumRows());
    return RESULT_FAIL;
  }
  ...
  if (subSample <= 0) {
    PRINT_NAMED_ERROR(
        "ImageBrightnessHistogram.FillFromImage.InvalidSubSampleWithWeights",
        "%d not > 0. Will use 1 instead.", subSample);
    subSample = 1;
  }
...
u8 ImageBrightnessHistogram::ComputePercentile(const f32 p) const {
  if (_totalCount == 0) {
    PRINT_NAMED_WARNING(
        "ImageBrightnessHistogram.ComputePercentile.ZeroTotalCount",
        "Returning 0 for p=%f because histogram is empty", p);
    return 0;
  }

  const s32 desiredCount =
      std::round(CLIP(p, 0.f, 1.f) * static_cast<f32>(_totalCount));
  u8 value = 0;
  s32 cumSum = 0;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - -
std::vector<u8> ImageBrightnessHistogram::ComputePercentiles(
    const std::set<f32>& p_list) const {
  std::vector<u8> values;

  if (_totalCount == 0) {
    PRINT_NAMED_WARNING(
        "ImageBrightnessHistogram.ComputePercentiles.ZeroTotalCount",
  ...
  DEV_ASSERT(values.size() == p_list.size(),
             "ImageBrightnessHistogram.ComputePercentile.SizeMismatch");
...
Image ImageBrightnessHistogram::GetDisplayImage(const s32 maxCount) const {
  ...
  if (maxCount <= 0) {
    ...
    histImg.Allocate((s32)_counts.size(),
                     maxCount);  // start transposed, for efficiency
    histImg.FillWith(255);

    // Shift is the bit-shift amount to achieve scaling the count values such
    // that a perfectly uniform distribution of counts would be 1/4 the image
    // height
    s32 shift = std::log2f(static_cast<f32>(_totalCount * 4) /
                           static_cast<f32>(_counts.size() * maxCount));
