...
class ImageCache {
 public:
  ...
  // Invalidate all cached data and start with new image data at
  // ImageCacheSize::Full. Will use 'method' when resizing the image May not
  // release associated image memory.
  void Reset(const Image& imgGray, ResizeMethod method = ResizeMethod::Linear);

  void Reset(const ImageRGB& imgColor,
             ResizeMethod method = ResizeMethod::Linear);
  ...
  // HasColor is true iff Reset was called with an ImageRGB (even if cached
  // "colorized gray" data exists)
  ...
  // NOTE: Does not actually create a cache entry for the specified size, if
  // none exists yet. (I.e., like a "peek" op)
  ...
  // NOTE: subsample = 1 always means "Full" processing resolution, irrespective
  // of the relationship
  //       between Full and Sensor resolution. Size::Sensor will never be
  //       returned.
  ...
  // If Reset with a color image, the gray version will be computed by averaging
  // (and cached). If Reset with a gray image, the "RGB" version will be
  // computed by replicating channels (and cached). Thus you will always get a
  // valid, non-empty image for both of these calls. If you want to do different
  // things depending on whether there is actually color data available (not
  // just replicated gray channels), use HasColor() above to decide which of
  // these to call. Notes:
  //  * The result of HasColor is not changed by calling GetRGB.
  //  * These are non-const because they could compute a resized version on
  //  demand.
  static constexpr ImageCacheSize GetDefaultImageCacheSize() {
    return ImageCacheSize::Half;
  }
  const Image& GetGray(ImageCacheSize size = GetDefaultImageCacheSize(),
                       GetType* getType = nullptr);
  const ImageRGB& GetRGB(ImageCacheSize size = GetDefaultImageCacheSize(),
                         GetType* getType = nullptr);
