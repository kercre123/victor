...
// NOTE: this currently only affects the ComputeObjectPose() that takes in
// quads.
...
Camera::Camera(void)
    : _camID(0), _calibration(nullptr) {}  // Constructor: Camera()

Camera::Camera(const CameraID_t ID) : _camID(ID), _calibration(nullptr) {}
...
bool Camera::SetCalibration(std::shared_ptr<CameraCalibration> calib) {
  if (IsCalibrated()) {
    ...
    // If Camera is already calibrated and the new one is the same, then nothing
    // is actually changing.
    if (calib == _calibration) {
      return false;
    }
  } else if (calib == nullptr) {
    // The Camera isn't already calibrated and a null calibration is being
    // passed in, nothing is actually changing.
    return false;
  }
...
#if ANKICORETECH_USE_OPENCV
Pose3d Camera::ComputeObjectPoseHelper(
  ...
  const CameraCalibration::DistortionCoeffs& distCoeffs =
      _calibration->GetDistortionCoeffs();
  cv::Mat_<f32> distortionCoeffs(1, (s32)distCoeffs.size(),
                                 const_cast<f32*>(distCoeffs.data()));

  cv::solvePnP(cvObjPoints, cvImagePoints, calibMatrix.get_CvMatx_(),
               distortionCoeffs, cvRvec, cvTranslation, false,
...
Pose3d Camera::ComputeObjectPose(const std::vector<Point2f>& imgPoints,
  ...
  if (this->IsCalibrated() == false) {
    CORETECH_THROW(
        "Camera::ComputeObjectPose() called before calibration set.");
  }
...
Result Camera::ComputeObjectPose(const Quad2f& imgQuad, const Quad3f& worldQuad,
                                 Pose3d& pose) const {
  if (this->IsCalibrated() == false) {
    CORETECH_THROW(
        "Camera::ComputeObjectPose() called before calibration set.");
  ...
  const SmallSquareMatrix<3, WORKING_PRECISION> invK =
      this->GetCalibration().GetInvCalibrationMatrix<WORKING_PRECISION>();
  ...
  for (Quad::CornerName i_corner = Quad::FirstCorner;
    ...
    //   imgRay = K^(-1) * [u v 1]^T
    imgRays[i_corner].x() =
        static_cast<WORKING_PRECISION>(imgQuad[i_corner].x());
    imgRays[i_corner].y() =
        static_cast<WORKING_PRECISION>(imgQuad[i_corner].y());
    ...
    */
    ...
    //       imgRays[i_corner].x(), imgRays[i_corner].y(),
    //       imgRays[i_corner].z());

    // cast each world quad into working precision quad
    worldPoints[i_corner].x() =
        static_cast<WORKING_PRECISION>(worldQuad[i_corner].x());
    worldPoints[i_corner].y() =
        static_cast<WORKING_PRECISION>(worldQuad[i_corner].y());
    worldPoints[i_corner].z() =
        static_cast<WORKING_PRECISION>(worldQuad[i_corner].z());
  }
  ...
  for (s32 i = 0; i < 4; ++i) {
    ...
    Result lastResult = P3P::computePossiblePoses(
        worldPoints[cornerList[1]], worldPoints[cornerList[2]],
        worldPoints[cornerList[3]], imgRays[cornerList[1]],
        imgRays[cornerList[2]], imgRays[cornerList[3]], possiblePoses);
    ...
    for (s32 i_solution = 0; i_solution < 4; ++i_solution) {
      ...
      if (possiblePoses[i_solution].GetTranslation().z() > 0) {
        ...
        this->Project3dPoint(possiblePoses[i_solution] * worldQuad[i_validate],
                             projectedPoint);
...
bool Camera::IsWithinFieldOfView(const Point2f& projectedPoint,
  ...
  return (not std::isnan(projectedPoint.x()) &&
          ...
          projectedPoint.x() <
              static_cast<f32>(_calibration->GetNcols() - xBorderPad) &&
          projectedPoint.y() <
              static_cast<f32>(_calibration->GetNrows() - yBorderPad));

}  // Camera::IsVisible()

bool Camera::Project3dPoint(const Point3f& objPoint, Point2f& imgPoint) const {
  const f32 BEHIND_OR_OCCLUDED = std::numeric_limits<f32>::quiet_NaN();

  if (this->IsCalibrated() == false) {
    PRINT_NAMED_WARNING("Camera.Project3dPoint.NoCalibration",
                        "Called before calibration set");
  ...
  if (objPoint.z() <= 0.f) {
    ...
    if (!k.empty()) {
      ...
      // Uses OpenCV model here:
      // http://docs.opencv.org/2.4/doc/tutorials/calib3d/camera_calibration/camera_calibration.html

      DEV_ASSERT_MSG(k.size() == 4 || k.size() == 5 || k.size() == 8,
                     "Camera.Project3dPoints.UnsupportedNumDistCoeffs", "N=%zu",
                     k.size());
      ...
      imgPoint.x() = imgPoint.x() * cdist + k[2] * a1 +
                     k[3] * a2;  // TODO: numCoeffs > 8: + k[8]*r2+k[9]*r4;
      imgPoint.y() = imgPoint.y() * cdist + k[2] * a3 +
                     k[3] * a1;  // TODO: numCoeffs > 8: + k[10]*r2+k[11]*r4;
    }
...
void Camera::Project3dPoints(const Quad3f& objPoints, Quad2f& imgPoints) const {
  for (Quad::CornerName i_corner = Quad::FirstCorner;
       i_corner < Quad::NumCorners; ++i_corner) {
...
void Camera::ClearOccluders() { _occluderList.Clear(); }
...
bool Camera::ProjectObject(const ObservableObject& object,
  ...
  if (object.GetPose().GetWithRespectTo(_pose, objectPoseWrtCamera) == false) {
    PRINT_NAMED_ERROR("Camera.AddOccluder.ObjectDoesNotShareOrigin",
                      "Object must be in the same pose tree as the camera to "
                      "add it as an occluder");
    return false;
  } else {
...
void Camera::ComputePanAndTiltAngles(const Point2f& imgPoint,
                                     Radians& relPanAngle,
  ...
  relTiltAngle =
      std::atan(-centeredPoint.y() / _calibration->GetFocalLength_y());
  relPanAngle =
      std::atan(-centeredPoint.x() / _calibration->GetFocalLength_x());
}
