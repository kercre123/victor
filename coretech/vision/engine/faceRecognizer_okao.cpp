...
namespace Vision {
  ...
  Result FaceRecognizer::Init(HCOMMON okaoCommonHandle)
  {
    ...
    PRINT_NAMED_INFO("FaceRecognizer.Init.Version",
                                              ...
                                              bool enableEnrollment)
  {
    ...
    if(ProcessingState::Idle == _state && anythingToDo)
    {
      ...
      _mutex.lock();
      img.CopyTo(_img); // TODO: Copy only the ROI of the image where the face is (VIC-12784)
                                                 ...
                                                 HFEATURE featureHandle)
  {
    if(!face.HasEyes())
    {
      PRINT_NAMED_WARNING("FaceRecognizer.ComputeFeaturesFromFace.NoFeatures", "");
      return RESULT_FAIL;
    }
