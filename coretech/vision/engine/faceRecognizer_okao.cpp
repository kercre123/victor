 ...
 * Date:   2/4/2016
 *
 * Description: Wrapper for OKAO Vision face recognition library, which runs on
 *a thread.
 *
...
static const u16 VersionNumber = Util::numeric_cast<u16>(
    FaceRecognitionConstants::EnrolledFaceStorageVersionNumber);
...
// Face must be recognized with score above this threshold to be considered a
// match
CONSOLE_VAR_RANGED(s32, kFaceRecognitionThreshold, "Vision.FaceRecognition",
                   575, 0, 1000);

// Lower threshold for setting the "best guess" if no match is found above the
// higher threshold
CONSOLE_VAR_RANGED(s32, kFaceRecognitionGuessThreshold,
                   "Vision.FaceRecognition", 350, 0, 1000);

// Non-match must be this much less than the recognition threshold to trigger
// registering a new session-only user
CONSOLE_VAR_RANGED(s32, kFaceRecognitionThresholdMarginForAdding,
                   "Vision.FaceRecognition", 200, 0, 1000);

// Second-best named match when top match is session-only must be greater than
// FaceRecognition threshold minus this margin to be used. This makes it a
// little easier to use 2nd best entry. If there is 3rd best match that is also
// named but with different ID, it must also be this margin _below_ the 2nd best
// match's score.
CONSOLE_VAR_RANGED(s32, kFaceRecognitionThresholdMarginForUsing2ndBest,
                   "Vision.FaceRecognition", 50, 0, 1000);

// When enabled, this will merge the individual data elements of the
// session-only album entries for two face records being merged. I don't believe
// this is a good idea anymore because you can create a single album entry with
// very different features in it, which is apparently not good for the OKAO
// libraries.
CONSOLE_VAR(bool, kEnableMergingOfSessionOnlyAlbumEntries,
            "Vision.FaceRecognition", false);

// Time between adding enrollment data for an existing user.
CONSOLE_VAR(f32, kTimeBetweenFaceEnrollmentUpdates_sec,
            "Vision.FaceRecognition", 0.5f);

// If true, gets enrollment time from the image timestamp instead of system
// time. This is especially useful for testing using image files, which may be
// processed faster than real time.
CONSOLE_VAR(bool, kGetEnrollmentTimeFromImageTimestamp,
            "Vision.FaceRecognition", false);

// If true, we can add new enrollment data even after the initial slots are
// full, where we replace the oldest. Otherwise, the initial enrollment data is
// the final record for a person.
CONSOLE_VAR(bool, kEnableEnrollmentAfterFull, "Vision.FaceRecognition", false);

CONSOLE_VAR_RANGED(u8, kFaceRecMaxDebugResults, "Vision.FaceRecognition", 3, 2,
                   10);

CONSOLE_VAR(bool, kFaceRecognitionExtraDebug, "Vision.FaceRecognition", false);

// For simulating slow processing (e.g. on a device)
CONSOLE_VAR(u32, kFaceRecognitionSimulatedDelay_ms, "Vision.FaceRecognition",
            0);

CONSOLE_VAR(s32, kEnrollmentThumbnailSize, "Vision.FaceRecognition", 64);

CONSOLE_VAR(bool, kGatherDebugEnrollmentImages, "Vision.FaceRecognition",
            false);
CONSOLE_VAR(bool, kDisplayDebugEnrollmentImages, "Vision.FaceRecognition",
            false);
...
inline static s64 GetSecondsSince(
    std::chrono::time_point<std::chrono::system_clock> now,
    EnrolledFaceEntry::Time t) {
  using namespace std::chrono;
  const s64 secondsSince =
      Util::numeric_cast<s64>(duration_cast<seconds>(now - t).count());
  return secondsSince;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - sometimes the face is a list, other
// times it's a vector, so make it work with any container todo: clean that up
...
inline void EmplaceLoadedKnownFace(const EnrolledFaceEntry& entry,
  ...
  const auto secSinceEpoch =
      duration_cast<seconds>(lastSeenTime.time_since_epoch()).count();
  const s64 secSinceEnrolled =
      GetSecondsSince(nowTime, entry.GetEnrollmentTime());
  const s64 secSinceUpdated =
      GetSecondsSince(nowTime, entry.GetLastUpdateTime());
  const s64 secSinceSeen = GetSecondsSince(nowTime, lastSeenTime);

  faces.emplace_back(LoadedKnownFace(secSinceEnrolled, secSinceUpdated,
                                     secSinceSeen, secSinceEpoch,
                                     entry.GetFaceID(), entry.GetName()));

  if (debugLabel != nullptr) {
    LOG_INFO(debugLabel,
             "User '%s' with ID=%d. Seconds since: Enrolled=%lld Updated=%lld "
             "Seen=%lld",
             entry.GetName().piiGuardedString(), entry.GetFaceID(),
             secSinceEnrolled, secSinceUpdated, secSinceSeen);
  }
}
...
FaceRecognizer::FaceRecognizer(const Json::Value& config) {
  if (config.isMember(JsonKey::FaceRecognitionGroup)) {
    const Json::Value& recognitionConfig =
        config[JsonKey::FaceRecognitionGroup];

    // TODO: Make this a console var too? (Not sure about switching it _while_
    // running though.
    std::string runModeStr;
    if (JsonTools::GetValueOptional(recognitionConfig, JsonKey::RunMode,
                                    runModeStr)) {
    ...
    LOG_WARNING("FaceRecognizer.Constructor.NoFaceRecParameters",
                "Did not find '%s' group in config",
                JsonKey::FaceRecognitionGroup);
  }

  LOG_INFO("FaceRecognizer.Constructor.RunMode", "Running in %s mode",
  ...
  if (config.isMember(JsonKey::PerformanceLoggingGroup)) {
    const Json::Value& performanceConfig =
        config[JsonKey::PerformanceLoggingGroup];
    if (!JsonTools::GetValueOptional(performanceConfig,
                                     JsonKey::TimeBetweenInfoPrints,
                                     timeBetweenProfilerInfoPrints_sec)) {
      LOG_WARNING("FaceRecognizer.Constructor.MissingJsonField", "%s.%s",
                  JsonKey::PerformanceLoggingGroup,
                  JsonKey::TimeBetweenInfoPrints);
    }

    if (!JsonTools::GetValueOptional(performanceConfig,
                                     JsonKey::TimeBetweenDasEvents,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
FaceRecognizer::~FaceRecognizer() { Shutdown(); }
...
Result FaceRecognizer::Init(HCOMMON okaoCommonHandle) {
  ...
  OkaoResult okaoResult =
      OKAO_FR_GetVersion(&okaoVersionMajor, &okaoVersionMinor);
  if (okaoResult != OKAO_NORMAL) {
    LOG_ERROR("FaceRecognizer.Init.VersionFail", "");
    return RESULT_FAIL;
  }
  LOG_INFO("FaceRecognizer.Init.Version",
           "Initializing with Recognizer version %d.%d", okaoVersionMajor,
           okaoVersionMinor);

  _okaoCommonHandle = okaoCommonHandle;

  _okaoRecognitionFeatureHandle =
      OKAO_FR_CreateFeatureHandle(_okaoCommonHandle);
  ...
  _okaoFaceAlbum = OKAO_FR_CreateAlbumHandle(
      _okaoCommonHandle, kMaxTotalAlbumEntries, kMaxEnrollDataPerAlbumEntry);
...
Result FaceRecognizer::Shutdown() {
  ...
  if (NULL != _okaoRecogMergeFeatureHandle) {
    if (OKAO_NORMAL !=
        OKAO_FR_DeleteFeatureHandle(_okaoRecogMergeFeatureHandle)) {
      LOG_ERROR(
          "FaceRecognizer.Shutdown."
          "FaceLibRecognitionMergeFeatureHandleDeleteFail",
          "");
    }
  ...
  if (NULL != _okaoRecognitionFeatureHandle) {
    if (OKAO_NORMAL !=
        OKAO_FR_DeleteFeatureHandle(_okaoRecognitionFeatureHandle)) {
      LOG_ERROR(
          "FaceRecognizer.Shutdown.FaceLibRecognitionFeatureHandleDeleteFail",
          "");
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result FaceRecognizer::SanityCheckBookkeeping(
    const HALBUM& okaoFaceAlbum, const EnrollmentData& enrollmentData,
    const AlbumEntryToFaceID& albumEntryToFaceID) {
  // Sanity checks to make sure enrollment data and Okao albums are in sync

  // Number of entries stored in OKAO album should match number of entries in
  // albumEntryToFaceID
  INT32 numEntries = 0;
  ...
  if (numEntries != albumEntryToFaceID.size()) {
    LOG_ERROR("FaceRecognizer.SanityCheckBookkeeping.NumAlbumEntriesMismatch",
              "FaceLibNumEntries=%d, AlbumEntryToFaceIDSize=%zu", numEntries,
              albumEntryToFaceID.size());
    return RESULT_FAIL;
  }
  ...
  for (auto& enrollData : enrollmentData) {
    // Every album entry for this FaceID should exist in _albumEntryToFaceID and
    // it should refer back to this FaceID
    for (auto& albumEntryPair : enrollData.second.GetAlbumEntries()) {
      ...
      if (iter == albumEntryToFaceID.end()) {
        LOG_ERROR("FaceRecognizer.SanityCheckBookkeeping.MissingAlbumEntry",
                  "AlbumEntry %d for FaceID %d does not exist in "
                  "albumEntryToFaceID LUT",
...
Result FaceRecognizer::UpdateRecognitionData(const FaceID_t recognizedID,
  ...
  if (UnknownFaceID == recognizedID) {
    ...
    LOG_DEBUG("UpdateRecognitionData.RecognizedNewTrackingID",
              "Tracking ID=%d recognized as FaceID=%d", -_detectionInfo.nID,
              recognizedID);
    ...
    if (faceIDenrollData == _enrollmentData.end()) {
      // The tracker's assigned ID got removed (via merge while doing
      // RecognizeFaces).
      faceID = recognizedID;
    } else {
      ...
      FaceID_t mergeTo = Vision::UnknownFaceID,
               mergeFrom = Vision::UnknownFaceID;
      ...
      if (false == recIDenrollData->second.IsForThisSessionOnly() &&
        ...
        mergeTo = faceID;
                 ...
                 false == faceIDenrollData->second.IsForThisSessionOnly()) {
        DEV_ASSERT(
            !faceIDenrollData->second.GetName().empty() &&
        ...
        // NOTE: Debug version displays names, warning does not (so they don't
        // get logged)
        LOG_WARNING("FaceRecognizer.UpdateRecognitionData.ConfusedTwoNamedIDs",
                    "While tracking face %d with ID=%d (%s), recognized as "
                    "ID=%d (%s). Not merging!",
        ...
        faceID = recognizedID;  // So that we'll udpate the tracking ID to face
                                // ID info below

        // NOTE: Leave mergeTo / mergeFrom set to unknown ID to signal _not_ to
        // merge below
      } else {
        // Both IDs are for this session only. Keep the oldest one.
        DEV_ASSERT(
            faceIDenrollData->second.IsForThisSessionOnly() &&
                recIDenrollData->second.IsForThisSessionOnly(),
            "FaceRecognizer.UpdateRecognitionData.BothIDsNotSessionOnly");

        if (faceIDenrollData->second.GetEnrollmentTime() <=
            recIDenrollData->second.GetEnrollmentTime()) {
          mergeFrom = recognizedID;
          mergeTo = faceID;
        } else {
          mergeFrom = faceID;
        ...
        if (kFaceRecognitionExtraDebug) {
          LOG_INFO(
              "UpdateRecognitionData.MergeBasedOnEnrollmentTime",
              "Merging ID=%d into ID=%d (%d enrolled at %s, %d enrolled at %s)",
              mergeFrom, mergeTo, faceID,
              EnrolledFaceEntry::GetTimeString(
                  faceIDenrollData->second.GetEnrollmentTime())
                  .c_str(),
              recognizedID,
              EnrolledFaceEntry::GetTimeString(
                  recIDenrollData->second.GetEnrollmentTime())
      ...
      if (mergeFrom != Vision::UnknownFaceID &&
          mergeTo != Vision::UnknownFaceID) {
        ...
        if (mergeFrom == recognizedID || isMergingAllowed) {
          ...
          LOG_INFO("UpdateRecognitionData.UpdateEnrollmentID",
                   "Updating enrollment ID %d->%d while to tracking %d (not "
                   "merging)",
          ...
          LOG_INFO("UpdateRecognitionData.NotMergingTrackedFace",
                   "Enrollment disabled: not merging tracked face ID=%d into "
                   "recognized ID=%d",
                   faceID, recognizedID);
        }
    ...
    // We recognized this person as the same ID already assigned to its tracking
    // ID
    DEV_ASSERT(faceID == recognizedID,
               "FaceRecognizer.UpdateRecognitionData.UnexpectedRecognizedID");
  }

  // Update the stored faceID assigned to this trackerID
  // (This creates an entry for the current trackerID if there wasn't one
  // already,
...
bool FaceRecognizer::HasRecognitionData(TrackingID_t forTrackingID) const {
  const bool haveEntry =
      (_trackingToFaceID.find(forTrackingID) != _trackingToFaceID.end());
  return haveEntry;
}
...
bool FaceRecognizer::HasName(TrackingID_t forTrackingID) const {
  ...
  if (!ANKI_VERIFY(faceID != UnknownFaceID,
                   "FaceRecognizer.HasName.TrackedFaceWithUnknownID", "")) {
    return false;
  }
  ...
  if (!ANKI_VERIFY(enrollIter != _enrollmentData.end(),
                   "FaceRecognizer.HasName.TrackedFaceWithNoEnrollData", "")) {
    return false;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
EnrolledFaceEntry FaceRecognizer::GetRecognitionData(
    INT32 forTrackingID, s32& enrollmentCountReached,
  ...
  if (ProcessingState::FeaturesReady == _state) {
    if (_detectionInfo.nID == UnknownFaceID) {
      // If the tracker ID is set to Unknown, it's because a ClearTrackingData
      // call was received while the recognition thread was running on a
      // detection. That detection ID is no longer valid, so drop the result on
      // the floor. This should only be possible when running asynchronously.
      DEV_ASSERT(_isRunningAsync,
                 "FaceRecognizer.GetRecognitionData.InvalidTrackIDinSyncMode");
      ...
      if (RESULT_OK == result) {
        ...
        if (RESULT_OK != result) {
          LOG_ERROR(
              "FaceRecognizer.GetRecognitionData.UpdateRecognitionDataFailed",
              "");
        }
      } else {
      ...
      if (ANKI_DEVELOPER_CODE) {
        const Result sanityResult = SanityCheckBookkeeping(
            _okaoFaceAlbum, _enrollmentData, _albumEntryToFaceID);
        DEV_ASSERT(sanityResult == RESULT_OK,
  ...
  if (iter != _trackingToFaceID.end()) {
    const FaceID_t faceID = iter->second;
    DEV_ASSERT(faceID != UnknownFaceID,
    ...
    // For example the enrollment data got changed (via load?) without updating
    // or clearing the trackingToFaceID LUT.
    DEV_ASSERT(enrollIter != _enrollmentData.end(),
    ...
    if (_enrollmentID != UnknownFaceID &&
      ...
      // ID just changed for this entry and the old ID was the one we were
      // enrolling. Update the enrollmentID to match.
      if (kFaceRecognitionExtraDebug) {
        LOG_INFO("GetRecognitionData.UpdatingEnrollmentID", "Old:%d -> New:%d",
                 _enrollmentID, enrolledEntry.GetFaceID());
      }
      _enrollmentID = enrolledEntry.GetFaceID();
    }

    // Make a copy to return. Use the numEnrollments to tell if we just
    // completed the specified enrollment count and return that count to the
    // caller.
    entryToReturn = enrolledEntry;  // Make a copy to return
    if (_enrollmentCount == 0 && _origEnrollmentCount > 0 &&
        _enrollmentID == entryToReturn.GetFaceID()) {
      if (kFaceRecognitionExtraDebug) {
        LOG_INFO("GetRecognitionData.EnrollmentCountReached", "Count=%d",
                 _origEnrollmentCount);
      }

      // Log the enrollment ID we just completed and how many album entries it
      // now has
      ...
      DASMSG(vision_face_enrollment_count_reached,
             ...
             "This will be shown when the most number of faces we support is "
             "shown");
...
void FaceRecognizer::Run() {
  ...
  while (_isRunningAsync) {
    ...
    if (anythingToDo) {
      // Note: this puts us in FeaturesReady state when done (or Idle if
      // failure)
      ExtractFeatures();
    }

    {
      ...
      std::unique_lock<std::mutex> lock{_mutex};
      _newImageCondition.wait(lock, [this] {
        return (_state == ProcessingState::HasNewImage) || !_isRunningAsync;
      });
    }
  }

  LOG_WARNING(
      "FaceRecognizer.Run.ThreadHasStopped",
...
void FaceRecognizer::ExtractFeatures() {
  DEV_ASSERT(ProcessingState::HasNewImage == _state,
  ...
  OkaoResult okaoResult = OKAO_FR_ExtractPoints_GRAY(
      _okaoRecognitionFeatureHandle, dataPtr, nWidth, nHeight,
      GRAY_ORDER_Y0Y1Y2Y3, PT_POINT_KIND_MAX, _aptPoint, _anConfidence);
  Toc("OkaoFeatureExtraction");

  if (kFaceRecognitionSimulatedDelay_ms > 0) {
    std::this_thread::sleep_for(
        std::chrono::milliseconds(kFaceRecognitionSimulatedDelay_ms));
  }
...
FaceRecognizer::AlbumEntryID_t FaceRecognizer::GetNextAlbumEntryToUse() {
  ...
  OkaoResult okaoResult =
      OKAO_FR_GetRegisteredUserNum(_okaoFaceAlbum, &numEntriesInAlbum);
  if (OKAO_NORMAL != okaoResult) {
    LOG_WARNING(
        "FaceRecognizer.GetNextAlbumEntryToUse.FaceLibGetNumUsersInAlbumFailed",
        "");
    return failureEntry;
  }

  if (numEntriesInAlbum >= kMaxTotalAlbumEntries) {
    // See if there are any session-only faces we can overwrite before giving
    // up. If so, choose the one that it's been the longest since we updated.
    ...
    for (auto& enrollData : _enrollmentData) {
      if (enrollData.second.IsForThisSessionOnly() &&
          enrollData.second.GetLastUpdateTime() < oldestUpdateTime) {
    ...
    if (UnknownFaceID == oldestSessionOnlyID) {
      // No session-only faces (this really should not happen because
      // MaxNamedFaces*MaxAlbumEntriesPerFace should generally be far less than
      // kMaxTotalAlbumEntries -- which is static_asserted in the header)
      LOG_ERROR("FaceRecognizer.GetNextAlbumEntryToUse.TooManyUsers",
                "Already have %zu users, could not add another",
    ...
    LOG_INFO("GetNextAlbumEntryToUse.ReplacingOldestSessionOnlyUser",
             ...
             oldestSessionOnlyID,
             EnrolledFaceEntry::GetTimeString(oldestUpdateTime).c_str());

    auto& albumEntries =
        _enrollmentData.at(oldestSessionOnlyID).GetAlbumEntries();

    DEV_ASSERT(!albumEntries.empty(),
               "FaceRecognizer.GetNextAlbumEntryToUse.OldestFaceHasNoEntries");
    ...
    if (RESULT_OK != removeResult) {
      LOG_WARNING(
          "FaceRecognizer.GetNextAlbumEntryToUse.RemoveOldestUserFailed",
    ...
    okaoResult = OKAO_FR_IsRegistered(_okaoFaceAlbum, _nextAlbumEntry, 0,
                                      &isStillRegistered);
    if (OKAO_NORMAL != okaoResult) {
      LOG_WARNING(
          "FaceRecognizer.GetNextAlbumEntryToUse."
          "CouldNotCheckIfStillRegistered",
          "Using anyway. Entry:%d FaceLib Result:%d", _nextAlbumEntry,
          okaoResult);
    }
    ...
    if (isStillRegistered) {
      LOG_WARNING(
          "FaceRecognizer.GetNextAlbumEntryToUse."
          "FirstFreedEntryStillRegistered",
    ...
    do {
      ...
      if (OKAO_NORMAL != okaoResult) {
        LOG_WARNING(
            "FaceRecognizer.GetNextAlbumEntryToUse.IsRegisteredCheckFailed",
            "Failed to determine if albumEntry %d is already registered. "
            "FaceLib result=%d",
    ...
    if (tries >= kMaxTotalAlbumEntries) {
      LOG_WARNING(
          "FaceRecognizer.GetNextAlbumEntryToUse.NoIDsAvailable",
...
Result FaceRecognizer::RegisterNewUser(HFEATURE& hFeature, FaceID_t& faceID) {
  ...
  OkaoResult okaoResult =
      OKAO_FR_RegisterData(_okaoFaceAlbum, hFeature, albumEntry, 0);
  if (OKAO_NORMAL != okaoResult) {
  ...
  // NOTE: all new registrations will start out "session only" (otherwise they
  // would have matched something)
  const EnrolledFaceEntry::Time enrollmentTime =
      (kGetEnrollmentTimeFromImageTimestamp
           ? EnrolledFaceEntry::Time(
                 std::chrono::milliseconds(_img.GetTimestamp()))
...
void FaceRecognizer::CancelExistingEnrollment() {
  ...
  if (_enrollmentID != UnknownFaceID && _enrollmentCount > 0) {
    ...
    if (enrollDataIter == _enrollmentData.end()) {
      LOG_WARNING(
          "FaceRecognizer.CancelExistingEnrollment.NoEnrollmentDataToErase",
    ...
    if (albumEntries.empty()) {
      // This really should not happen: erase the bad entry if it does (if there
      // are no album entries associated with a faceID, we can never match to
      // it)
      LOG_ERROR("FaceRecognizer.CancelExistingEnrollment.EmptyAlbumEntries",
                "Erasing %s enrollmentID=%d",
                (enrollDataIter->second.IsForThisSessionOnly() ? "session-only"
                                                               : "named"),
                _enrollmentID);
    ...
    // By definition, there can't be "mixing" of enrollments of different people
    // if there's only one album entry for this ID. Just leave it as a
    // session-only entry, since that doesn't really cause any harm.
    if (albumEntries.size() == 1) {
      return;
    }

    // Figure out which album entry we were in the process of adding/updating
    // for the current enrollment ID and erase it. We'll use the most recently
    // seen one.
    ...
    LOG_INFO("FaceRecognizer.CancelExistingEnrollment.RemoveAlbumEntry",
             ...
             _nextAlbumEntry, _enrollmentID,
             enrollDataIter->second.GetNumAlbumEntries());
  }
...
void FaceRecognizer::SetAllowedEnrollments(s32 N, FaceID_t forFaceID,
  ...
  if (_enrollmentID == UnknownFaceID) {
    DEV_ASSERT(
        !forceNewID,
        "FaceRecognizer.SetAllowedEnrollments.CannotForceNewIDwithUnknownID");
...
bool FaceRecognizer::SetNextFaceToRecognize(const Vision::Image& img,
  ...
  if (ProcessingState::Idle == _state && anythingToDo) {
    ...
    OkaoResult result = OKAO_GetRectangleFromInfo(
        const_cast<DETECTION_INFO*>(&detectionInfo), &ptLeftTop, &ptRightTop,
        &ptLeftBottom, &ptRightBottom);

    if (OKAO_NORMAL != result) {
      LOG_ERROR("FaceRecognizer.SetNextFaceToRecognize.GetRectangleFailed",
                "Result:%d", result);
      return false;
    }
    ...
    DEV_ASSERT(_img.IsContinuous(),
               "FaceRecognizer.SetNextFaceToRecognize.NonContinuousImage");
    ...
    //         "Setting next face to recognize: tracked ID %d",
    //         -_detectionInfo.nID);
    ...
    //                "Idle:%d AnythingToDo:%d EnableEnrollment:%d
    //                HoldCount:%d", (ProcessingState::Idle == _state),
    //                anythingToDo, enableEnrollment, detectionInfo.nHoldCount);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result FaceRecognizer::UpdateExistingAlbumEntry(AlbumEntryID_t albumEntry,
                                                HFEATURE& hFeature,
  ...
  const EnrolledFaceEntry::Time updateTime =
      (kGetEnrollmentTimeFromImageTimestamp
           ? EnrolledFaceEntry::Time(
                 std::chrono::milliseconds(_img.GetTimestamp()))
           : std::chrono::system_clock::now());
  ...
  DEV_ASSERT_MSG(
      albumToFaceIter != _albumEntryToFaceID.end(),
  ...
  DEV_ASSERT_MSG(
      enrollDataIter != _enrollmentData.end(),
  ...
  if (_detectionInfo.nID != enrollData.GetTrackingID()) {
    LOG_INFO("UpdateExistingAlbumEntry.UpdateTrackID",
             "Update trackID for face %d: %d -> %d", faceID,
             -enrollData.GetTrackingID(), -_detectionInfo.nID);
    _trackingToFaceID.erase(enrollData.GetTrackingID());
    enrollData.SetTrackingID(_detectionInfo.nID);
  }

  INT32 numDataStored = 0;
  OkaoResult okaoResult = OKAO_FR_GetRegisteredUsrDataNum(
      _okaoFaceAlbum, albumEntry, &numDataStored);
  if (OKAO_NORMAL != okaoResult) {
    LOG_ERROR(
        "FaceRecognizer.UpdateExistingAlbumEntry.GetRegisteredUserDataFailed",
        "albumEntry:%d dataEntry:%d FaceLib result=%d", albumEntry,
        numDataStored, okaoResult);
    return RESULT_FAIL;
  }

  auto const timeSinceLastUpdate =
      std::chrono::duration_cast<std::chrono::seconds>(
          updateTime - enrollData.GetLastUpdateTime());

  auto const timeBetweenEnrollments =
      std::chrono::seconds(Util::numeric_cast<s64>(
          std::round(kTimeBetweenFaceEnrollmentUpdates_sec)));

  const bool isEnrollmentEnabled =
      _enrollmentCount != 0 &&
      (_enrollmentID == UnknownFaceID || _enrollmentID == faceID);

  const bool isNamedFace = !enrollData.GetName().empty();
  const bool hasEnrollSpaceLeft = numDataStored < kMaxEnrollDataPerAlbumEntry;
  const bool haveEnrollmentCountAndID =
      _enrollmentCount > 0 && _enrollmentID != UnknownFaceID;
  const bool enrollEvenIfFull =
      !isNamedFace || kEnableEnrollmentAfterFull || haveEnrollmentCountAndID;
  const bool isTimeToEnroll =
      timeSinceLastUpdate.count() >= timeBetweenEnrollments.count();

  if (isEnrollmentEnabled && isTimeToEnroll &&
      (hasEnrollSpaceLeft || enrollEvenIfFull)) {
    // Num data for user should be > 0 if we are updating!
    DEV_ASSERT(numDataStored > 0,
               "FaceRecognizer.UpdateExistingAlbumEntry.BadNumData");

    AlbumEntryID_t entryToReplace = EnrolledFaceEntry::UnknownAlbumEntryID;
    if (hasEnrollSpaceLeft) {
      // Simple case: just add this data to the album entry. No need to figure
      // which records to keep.
      entryToReplace = numDataStored;
      OkaoResult okaoResult = OKAO_FR_RegisterData(_okaoFaceAlbum, hFeature,
                                                   albumEntry, entryToReplace);
      if (OKAO_NORMAL != okaoResult) {
        LOG_WARNING(
            "FaceRecognizer.UpdateExistingUser.FailedToRegisterNewEntry",
      ...
      if (kFaceRecognitionExtraDebug) {
        LOG_INFO("UpdateExistingUser",
                 ...
                 entryToReplace, albumEntry,
                 enrollData.GetAlbumEntries().size(), faceID);
      }
    } else if (enrollEvenIfFull) {
      RecognitionScore newEntryScore;
      OkaoResult okaoResult =
          OKAO_FR_Verify(hFeature, _okaoFaceAlbum, albumEntry, &newEntryScore);
      if (OKAO_NORMAL != okaoResult) {
        LOG_ERROR(
            "FaceRecognizer.UpdateExistingAlbumEntry.VerifyNewFeatureFailed",
            "albumEntry:%d FaceLib result=%d", albumEntry, okaoResult);
        return RESULT_FAIL;
      }

      // Find the entry with a score lower than the potential new entry (and
      // keep the lowest of those). If we find one, we will replace it with this
      // new better-matching feature.
      RecognitionScore lowestScoreBelowNewEntry = newEntryScore;
      for (s32 iData = 0; iData < kMaxEnrollDataPerAlbumEntry; ++iData) {
        // Temporarily remove each data entry from the album entry and see what
        // it's score is when compared back against the album entry.
        okaoResult = OKAO_FR_GetFeatureFromAlbum(
            _okaoFaceAlbum, albumEntry, iData, _okaoRecogMergeFeatureHandle);
        if (OKAO_NORMAL != okaoResult) {
        ...
        okaoResult =
            OKAO_FR_Verify(_okaoRecogMergeFeatureHandle, _okaoFaceAlbum,
                           albumEntry, &currentEntryScore);
        if (OKAO_NORMAL != okaoResult) {
          LOG_ERROR(
              "FaceRecognizer.UpdateExistingAlbumEntry."
              "VerifyExistingFeatureFailed",
              "albumEntry:%d FaceLib result=%d", albumEntry, okaoResult);
          return RESULT_FAIL;
        }
        ...
        okaoResult = OKAO_FR_RegisterData(
            _okaoFaceAlbum, _okaoRecogMergeFeatureHandle, albumEntry, iData);
        if (OKAO_NORMAL != okaoResult) {
          LOG_ERROR(
              "FaceRecognizer.UpdateExistingAlbumEntry.ReRegisterDataFailed",
              "albumEntry:%d dataEntry:%d FaceLib result=%d", albumEntry, iData,
              okaoResult);
          return RESULT_FAIL;
        }
        ...
        if (kFaceRecognitionExtraDebug) {
          LOG_INFO("UpdateExistingUser.EvaluatingAlbumEntryClusteredness",
                   "Data:%d in AlbumEntry:%d got score:%d (vs. possible "
                   "addition with score:%d)",
                   iData, albumEntry, currentEntryScore, newEntryScore);
        }
      }

      if (EnrolledFaceEntry::UnknownAlbumEntryID != entryToReplace) {
        okaoResult = OKAO_FR_RegisterData(_okaoFaceAlbum, hFeature, albumEntry,
                                          entryToReplace);
        if (OKAO_NORMAL != okaoResult) {
          LOG_ERROR(
              "FaceRecognizer.UpdateExistingAlbumEntry."
              "RegisterReplacementDataFailed",
              "albumEntry:%d dataEntry:%d FaceLib result=%d", albumEntry,
              entryToReplace, okaoResult);
          return RESULT_FAIL;
        }

        if (kFaceRecognitionExtraDebug) {
          LOG_INFO(
              "UpdateExistingUser.ReplacedDataEntry",
      ...
      DEV_ASSERT(false,
                 "FaceRecognizer.UpdateExistingAlbumEntry.IfElseLogicError");
    }

    // Even if we didn't actually update: the following will update the last
    // time we at least considered doing so.
    ...
    if (kGatherDebugEnrollmentImages &&
        entryToReplace != EnrolledFaceEntry::UnknownAlbumEntryID) {
      // Update the enrollment image _after_ we update the lastUpdateTime so it
    ...
    LOG_DEBUG(
        "FaceRecognizer.UpdateExistingAlbumEntry.NotAddingEnrollmentEntry",
        "FaceID:%d isEnabled:%d isNamed:%d hasSpaceLeft:%d haveCountAndID:%d "
        "enrollIfFull:%d isTime:%d",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void FaceRecognizer::SetEnrollmentImage(AlbumEntryID_t albumEntry,
                                        s32 dataEntry) {
  POINT ptLeftTop, ptRightTop, ptLeftBottom, ptRightBottom;
  OkaoResult okaoResult = OKAO_CO_ConvertCenterToSquare(
      _detectionInfo.ptCenter, _detectionInfo.nHeight, 0, &ptLeftTop,
      &ptRightTop, &ptLeftBottom, &ptRightBottom);
  DEV_ASSERT(OKAO_NORMAL == okaoResult,
             "FaceRecognizer.SetEnrollmentImage.GetDetectionSquareFail");
  ...
  if (kDisplayDebugEnrollmentImages) {
    ...
    _enrollmentImages[albumEntry][dataEntry].Allocate(kEnrollmentThumbnailSize,
                                                      kEnrollmentThumbnailSize);
  } else {
    // If we're not displaying images don't resize them so we can use them for
    // testing if we would like.
    _enrollmentImages[albumEntry][dataEntry].Allocate(detectionRect.GetHeight(),
                                                      detectionRect.GetWidth());
  }
  _img.GetROI(detectionRect).Resize(_enrollmentImages[albumEntry][dataEntry]);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Helper to create the title for the
// dislay window. Either "UserN" or "UserN:<name>"
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void FaceRecognizer::DisplayEnrollmentImages(
    DebugImageList<CompressedImage>& debugImages) const {
  // Parameters for creating tiled display of enrollment images
  const s32 numDispRows = kMaxAlbumEntriesPerFace,
            numDispCols = kMaxEnrollDataPerAlbumEntry;
  ...
  for (auto& enrollData : _enrollmentData) {
    if (enrollData.second.FindLastSeenTime() <
    ...
    s32 iAlbumEntry = 0;
    EnrolledFaceEntry::Time lastSeenTime =
        EnrolledFaceEntry::Time(std::chrono::seconds(0));
    s32 lastSeenIndex = -1;

    const AlbumEntryID_t sessionOnlyEntry =
        enrollData.second.GetSessionOnlyAlbumEntry();
    ...
    for (auto& albumEntry : enrollData.second.GetAlbumEntries()) {
      ...
      for (s32 iDataEntry = 0; iDataEntry < kMaxEnrollDataPerAlbumEntry;
           ++iDataEntry) {
        ...
        if (!enrollmentImage.IsEmpty()) {
          ...
          Rectangle<s32> dispRect(dispCols[iDataEntry] * dispWidth,
                                  dispRows[iAlbumEntry] * dispHeight, dispWidth,
                                  dispHeight);

          ImageRGB dispROI = dispImg.GetROI(dispRect);

          enrollmentImage.Resize(dispROI,
                                 Vision::ResizeMethod::NearestNeighbor);

          dispROI.DrawText({2, dispROI.GetNumRows() - 2},
                           std::string("Score: ") +
                               std::to_string(enrollData.second.GetScore()),
                           NamedColors::RED, 0.15f, true);
          dispROI.DrawText({0, 10},
                           EnrolledFaceEntry::GetTimeString(
                               enrollData.second.GetLastUpdateTime()),
                           NamedColors::RED, 0.15f, true);
        }
    ...
    // Draw a thick blue bar to the left of the row that's the session-only
    // entry
    ...
    // Draw a thick yellow bar to the right of the row that was seen most
    // recently
    if (lastSeenIndex >= 0 && lastSeenIndex < dispRows.size()) {
      dispImg.DrawLine(
          {(f32)dispImg.GetNumCols() - 3, dispRows[lastSeenIndex] * dispHeight},
          {(f32)dispImg.GetNumCols() - 3,
           (dispRows[lastSeenIndex] + 1) * dispHeight},
          NamedColors::YELLOW, 4);
    }

    // dispImg.Display(GetDisplayName(enrollData.second).c_str());
    debugImages.emplace_back(GetDisplayName(enrollData.second).c_str(),
                             CompressedImage(std::move(dispImg), 75));
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void FaceRecognizer::DisplayMatchImages(
    const INT32 resultNum,
  ...
  ImageRGB matchImg(numRows * kEnrollmentThumbnailSize,
                    numCols * kEnrollmentThumbnailSize);
  s32 row = 0, col = 0;
  for (s32 iResult = 0; iResult < resultNum; ++iResult) {
    Rectangle<s32> roiRect(col * kEnrollmentThumbnailSize,
                           row * kEnrollmentThumbnailSize,
    ...
    const std::string dispStr =
        std::to_string(faceID) + ":" + std::to_string(scores[iResult]);
    matchROI.DrawText(Point2f{1, matchROI.GetNumRows() - 1}, dispStr,
                      NamedColors::RED, 0.5);
    ++col;
    if (col >= numCols) {
      col = 0;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
FaceRecognizer::EnrollmentData::iterator FaceRecognizer::RemoveUser(
    EnrollmentData::iterator userIter) {
  ...
  for (auto& albumEntryPair : userIter->second.GetAlbumEntries()) {
    ...
    if (OKAO_NORMAL != okaoResult) {
      LOG_WARNING("FaceRecognizer.RemoveUser.ClearUserFailed",
                  "AlbumEntry:%d FaceID:%d FaceLib result:%d", albumEntry,
                  faceID, okaoResult);
    }
  ...
  // TODO: Keep a reference to which trackerIDs are pointing to each faceID to
  // avoid this search
  if (ANKI_DEVELOPER_CODE) {
    // Sanity check: there should be no more references to this faceID
    for (auto iter = _trackingToFaceID.begin();
         iter != _trackingToFaceID.end();) {
      if (iter->second == faceID) {
        LOG_WARNING("FaceRecognizer.RemoveUserHelper.StaleTrackingToFaceID",
                    "TrackID %d still maps to FaceID %d", iter->first,
                    iter->second);
    ...
    for (auto iter = _albumEntryToFaceID.begin();
         iter != _albumEntryToFaceID.end();) {
      if (iter->second == faceID) {
        LOG_WARNING("FaceRecognizer.RemoveUserHelper.StaleAlbumEntryToFaceID",
                    "AlbumEntry %d still maps to FaceID %d", iter->first,
                    iter->second);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
FaceID_t FaceRecognizer::GetFaceIDforAlbumEntry(
    AlbumEntryID_t albumEntry) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void FaceRecognizer::AddDebugInfo(
    FaceID_t matchedID, RecognitionScore score,
  ...
  newDebugInfo.emplace_back(FaceRecognitionMatch{
      .name =
          (iter == _enrollmentData.end() ? std::string("")
                                         : iter->second.GetName().asString()),
      .matchedID = matchedID,
      .score = score,
  });
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result FaceRecognizer::RecognizeFace(
    FaceID_t& faceID, RecognitionScore& recognitionScore,
    DebugImageList<CompressedImage>& debugImages) {
  ...
  OkaoResult okaoResult =
      OKAO_FR_GetRegisteredUserNum(_okaoFaceAlbum, &numUsersInAlbum);
  if (OKAO_NORMAL != okaoResult) {
  ...
  if (!canEnrollAnyFace && _forceNewEnrollment && haveEnrollmentCountsLeft &&
    ...
    if (_enrollmentCount == _origEnrollmentCount) {
      ...
      if (ANKI_VERIFY(
              ...
              "EnrollmentID:%d", _enrollmentID)) {
        const auto& albumEntries = enrollDataIter->second.GetAlbumEntries();
        DEV_ASSERT(albumEntries.size() == 1,
                   "FaceRecognizer.RecognizeFace.ExpectingOneAlbumEntry");

        const AlbumEntryID_t albumEntry = albumEntries.begin()->first;
        Result result = UpdateExistingAlbumEntry(
            albumEntry, _okaoRecognitionFeatureHandle, recognitionScore);
        if (RESULT_OK != result) {
          LOG_WARNING(
              "FaceRecognizer.RecognizeFace."
              "ForceUpdatingExistingAlbumEntryFailed",
  ...
  Tic("OkaoIdentify");
  okaoResult = OKAO_FR_Identify(
      _okaoRecognitionFeatureHandle, _okaoFaceAlbum, kMaxIdentifyResults,
  ...
  if (OKAO_NORMAL != okaoResult) {
    LOG_WARNING(
        "FaceRecognizer.RecognizeFace.FaceLibFaceRecognitionIdentifyFailed",
        "maxResults:%d, hFeature:%p, hAlbum:%p, FaceLib Result Code=%d",
        kMaxIdentifyResults, _okaoRecognitionFeatureHandle, _okaoFaceAlbum,
        okaoResult);
  ...
  // const f32   RelativeRecognitionThreshold = 1.5; // Score of top result must
  // be this times the score of the second best result
  ...
  const bool foundMatchAboveThreshold =
      (resultNum > 0) && (scores[0] > kFaceRecognitionThreshold);
  if (foundMatchAboveThreshold) {
    ...
    if (kFaceRecognitionExtraDebug) {
      // Populate the debug info for the recognized face. Always put on the top
      // match Then add the first (top-scoring) album match for each face ID,
      // until we've filled the debug list Do this before checking for 2nd-best
      // or calling UpdateExistingUser because those could cause us to cull an
      // album entry associated with matchedID, which could result in
      // GetFaceIDforAlbumEntry to fail in this loop.
      std::list<FaceRecognitionMatch> newDebugInfo{FaceRecognitionMatch{
          .name = matchIter->second.GetName(),
          .matchedID = matchingID,
          .score = matchingScore,
      }};
      s32 iResult = matchIndex + 1;
      BOUNDED_WHILE(kMaxIdentifyResults,
                    iResult < resultNum &&
                        newDebugInfo.size() < kFaceRecMaxDebugResults) {
        // It's possible, due to using a non-top match above, that we have
        // performed a merge and entries in matchingAlbumEntries
        auto const matchedID =
            GetFaceIDforAlbumEntry(matchingAlbumEntries[iResult]);
        if (matchedID != newDebugInfo.back().matchedID) {
          auto iter = _enrollmentData.find(matchedID);
          newDebugInfo.emplace_back(FaceRecognitionMatch{
              .name = (iter == _enrollmentData.end()
                           ? std::string("")
                           : iter->second.GetName().asString()),
              .matchedID = matchedID,
    ...
    // not the ID we're specifically enrolling, then consider using a
    // lower-scoring, named match instead
    ...
    if (haveMoreResults && isTopMatchSessionOnly && !isTopMatchBeingEnrolled) {
      // Find the next match with a different FaceID than the top match, and
      // which is also named (non-session-only, unlike the top match)
      FaceID_t nextMatchingID = UnknownFaceID;
      s32 nextIndex = 1;
      do {
        nextMatchingID =
            GetFaceIDforAlbumEntry(matchingAlbumEntries[nextIndex]);

        auto nextMatchIter = _enrollmentData.find(nextMatchingID);

        if (nextMatchIter == _enrollmentData.end()) {
          LOG_WARNING(
              "FaceRecognizer.RecognizeFace.Missing2ndMatchEnrollmentData",
              "ID:%d", nextMatchingID);
        ...
        if (nextMatchIter->second.IsForThisSessionOnly()) {
          ...
          const bool nextScoreHighEnough =
              (scores[nextIndex] >
               kFaceRecognitionThreshold -
                   kFaceRecognitionThresholdMarginForUsing2ndBest);

          if (nextScoreHighEnough) {
            // Yep, score is high enough. Check to make sure there are no other
            // named matches with a different ID that are too close to the 2nd
            // best.

            const s32 lowThreshold = std::min(
                scores[nextIndex] -
                    kFaceRecognitionThresholdMarginForUsing2ndBest,
                kFaceRecognitionThreshold -
                    2 * kFaceRecognitionThresholdMarginForUsing2ndBest);

            FaceID_t nextNextID = Vision::UnknownFaceID;
            bool nextNextScoreLowEnough = true;
            s32 nextNextIndex = nextIndex + 1;
            BOUNDED_WHILE(kMaxIdentifyResults,
                          nextNextIndex < resultNum &&
                              scores[nextNextIndex] > lowThreshold) {
              nextNextID =
                  GetFaceIDforAlbumEntry(matchingAlbumEntries[nextNextIndex]);
              if (nextNextID != nextMatchingID) {
                auto nextNextIter = _enrollmentData.find(nextNextID);
                if (nextNextIter == _enrollmentData.end()) {
                  LOG_WARNING(
                      "FaceRecognizer.RecognizeFace."
                      "Missing3rdMatchEnrollmentData",
                      "ID:%d", nextNextID);

                } else if (!nextMatchIter->second.IsForThisSessionOnly()) {
                  // If _any_ named match with a different ID is above the low
                  // threshold then we are done
            ...
            if (nextNextScoreLowEnough) {
              // We just realized a session-only face is actually someone we
              // know with a high enough score. (And the score after that, if
              // there is one, is low enough.) Merge the two. Note that if this
              // is not the second best match and there are other session-only
              // entries in between, this will likely happen again and they will
              // get merged in one by one. At least that's the theory... This is
              // an unlikely scenario regardless.
              LOG_INFO(
                  "RecognizeFace.UsingLowerRankedMatch",
                  "Top match (AlbumEntry:%d ID:%d Score:%d) is session-only. "
                  "Match at index %d (AlbumEntry:%d ID:%d Score:%d) is named "
                  "('%s'). Using it and merging.",
                  matchingAlbumEntries[matchIndex], matchingID, matchingScore,
                  nextIndex, matchingAlbumEntries[nextIndex], nextMatchingID,
                  scores[nextIndex],
              ...
              DASMSG(vision.face_recognition.using_lower_ranked_match,
                     ...
                     "A session-only face was the top match, but we are using "
                     "a lower-ranked named match instead");
              ...
              DASMSG_SET(i4, scores[nextIndex],
                         "Score of nextIndex (=scores[nextIndex])");
              DASMSG_SET(s1, std::to_string(nextNextID), "Next next ID");
              DASMSG_SET(
                  s2,
                  std::to_string(nextNextID == Vision::UnknownFaceID
                                     ? -1
                                     : scores[nextNextIndex]),
                  "Score of nextNextIndex (or -1 if nextNextID is unknown)");
              DASMSG_SEND();

              if (ANKI_DEV_CHEATS) {
                LOG_DEBUG(
                    "RecognizeFace.BeforeMergeOfLowerRankedMatch",
              ...
              if (RESULT_OK != mergeResult) {
                LOG_WARNING("FaceRecognizer.RecognizeFace.MergeFacesFailed",
                            "Merging %d into %d", matchingID, nextMatchingID);
              }
              ...
              if (ANKI_DEV_CHEATS) {
                // Note: matchIter is no longer valid -- its entry has been
                // merged into nextMatchIter!
                LOG_DEBUG(
                    "RecognizeFace.AfterMergeOfLowerRankedMatch",
              ...
              matchingAlbumEntries[matchIndex] =
                  matchingAlbumEntries[nextIndex];
              ...
              const AlbumEntryID_t recognizedAlbumEntryID =
                  matchingAlbumEntries[matchIndex];
              auto const& matchIterAlbumEntries =
                  matchIter->second.GetAlbumEntries();
              if (matchIterAlbumEntries.find(recognizedAlbumEntryID) ==
                  matchIterAlbumEntries.end()) {
                // The merge above removed the album entry we actually
                // recognized, so we have nothing to update below with the call
                // to UpdateExistingAlbumEntry().
                shouldUpdateAlbumEntry = false;

                // If the recognized entry isn't part of the matched enrollment
                // data anymore (matchIter), then neither should it be in the
                // album entry to face ID lookup table anymore.
                DEV_ASSERT_MSG(
                    _albumEntryToFaceID.find(recognizedAlbumEntryID) ==
                        _albumEntryToFaceID.end(),
    ...
    if (shouldUpdateAlbumEntry) {
      ...
      if (RESULT_OK != result) {
        LOG_WARNING(
            "FaceRecognizer.RecognizeFace.UpdatingExistingAlbumEntryFailed",
    ...
    if (_enrollmentID == faceID && _enrollmentTrackID != _detectionInfo.nID) {
      ...
      LOG_INFO("RecognizeFace.SetEnrollmentTrackID",
               "EnrollmentID:%d EnrollmentTrackID:%d (was %d)", _enrollmentID,
               -_detectionInfo.nID, -_enrollmentTrackID);
    ...
    const bool isTrackingEnrollmentID =
        (_detectionInfo.nID == _enrollmentTrackID);

    if (enrollingSpecificFace && isTrackingEnrollmentID) {
      DEV_ASSERT(_detectionInfo.nID != UnknownFaceID,
                 "FaceRecognizer.RecognizeFace.BadDetectionID");

      // Did not recognize the current enrollmentID in the image, but the
      // trackingID matches the enrollment tracking ID. Update using the match
      // score for _enrollmentID.
      recognitionScore = 0;
      for (INT32 iResult = 0; iResult < resultNum; ++iResult) {
        if (GetFaceIDforAlbumEntry(matchingAlbumEntries[iResult]) ==
            _enrollmentID) {
          recognitionScore = scores[iResult];
          Result result = UpdateExistingAlbumEntry(
              matchingAlbumEntries[iResult], _okaoRecognitionFeatureHandle,
              recognitionScore);
          if (RESULT_OK != result) {
            return result;
          }
          ...
          LOG_INFO("RecognizeFace.UpdatingEnrollmentFace",
                   "Did not recognize enrollmentID:%d, updating with score %d "
                   "based on enrolledTrackID:%d",
      ...
      // If the score with which we matched something in the album isn't "too
      // close" to (i.e. within specified marghin of) an entry already in the
      // album, then add this as a new user. The margin is meant to keep
      // multiple entries from being "too" similar, since that is apparently not
      // great for OKAO.
      if (resultNum == 0 ||
          scores[0] < kFaceRecognitionThreshold -
                          kFaceRecognitionThresholdMarginForAdding) {
        LOG_INFO("RecognizeFace.AddingNewUser",
                 "Observed new person. Adding to album.");
        Result lastResult =
            RegisterNewUser(_okaoRecognitionFeatureHandle, faceID);
        if (RESULT_OK != lastResult) {
          return lastResult;
        }
        recognitionScore = kMaxScore;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void FaceRecognizer::UpdateBestGuessName(
    const std::vector<AlbumEntryID_t>& matchingAlbumEntries,
    const std::vector<RecognitionScore>& scores, const int resultNum) {
  // Update the best guess for the current tracking ID. Note that we remove it
  // first, in case nothing is found because we don't want old "best guesses"
  // sticking around if they don't still match the current appearance of the
  // tracked face.
  _trackingIDtoBestGuessName.erase(_detectionInfo.nID);
  s32 iResult = 0;
  while (iResult < resultNum &&
         scores[iResult] > kFaceRecognitionGuessThreshold) {
    FaceID_t matchingID = GetFaceIDforAlbumEntry(matchingAlbumEntries[iResult]);
    auto matchIter = _enrollmentData.find(matchingID);
    if (matchIter != _enrollmentData.end() &&
        !matchIter->second.IsForThisSessionOnly()) {
      _trackingIDtoBestGuessName[_detectionInfo.nID] =
          matchIter->second.GetName();
      break;
    }
    ++iResult;
  }
...
bool FaceRecognizer::IsMergingAllowed(FaceID_t toFaceID) const {
  ...
  const bool isAllowed = (_isEnrollmentEnabled && haveEnrollmentsLeft &&
...
Result FaceRecognizer::MergeFaces(FaceID_t keepID, FaceID_t mergeID) {
  if (keepID == mergeID) {
    LOG_INFO("FaceRecognizer.MergeFaces.NothingToDo", "keepID=mergeID=%d",
             keepID);
    return RESULT_OK;
  }
  ...
  if (kEnableMergingOfSessionOnlyAlbumEntries &&
    ...
    OKAO_FR_GetRegisteredUsrDataNum(
        _okaoFaceAlbum, mergeIter->second.GetSessionOnlyAlbumEntry(),
        &numSessionOnlyMergeData);
    for (s32 iMergeFeature = 0; iMergeFeature < numSessionOnlyMergeData;
         ++iMergeFeature) {
      OKAO_FR_GetFeatureFromAlbum(_okaoFaceAlbum,
                                  mergeIter->second.GetSessionOnlyAlbumEntry(),
                                  iMergeFeature, _okaoRecogMergeFeatureHandle);

      UpdateExistingAlbumEntry(keepIter->second.GetSessionOnlyAlbumEntry(),
  ...
  Result mergeResult = keepIter->second.MergeWith(
      mergeIter->second, kMaxAlbumEntriesPerFace, entriesRemovedFromKeep);
  if (RESULT_OK != mergeResult) {
    LOG_WARNING("FaceRecognizer.MergeFaces.MergeFailed",
                "Merging faceID:%d with %d", keepIter->first, mergeIter->first);
    return mergeResult;
  }

  // Fix the albumEntryToFaceID bookkeeping now that keepID's albumEntries have
  // changed
  ...
  for (AlbumEntryID_t removedEntry : entriesRemovedFromKeep) {
    ...
    if (OKAO_NORMAL != okaoResult) {
      LOG_WARNING("FaceRecognizer.MergeFaces.ClearUserFailed",
                  "AlbumEntry:%d originally part of keepID:%d", removedEntry,
                  keepID);
      return RESULT_FAIL;
    }
  }

  // "Unregister" all album entries still left associated with mergeID (since
  // they were not taken by keepID above).
  for (auto& albumEntryPair : mergeIter->second.GetAlbumEntries()) {
    // If this entry is not now claimed by keepID, we should clear it because
    // we're about to clear its album entries. Note that we must do this before
    // calling RemoveUser below because it would otherwise undo the reassignment
    // to keepID above
    if (_albumEntryToFaceID.at(albumEntryPair.first) == mergeID) {
      _albumEntryToFaceID.erase(albumEntryPair.first);
      OkaoResult okaoResult =
          OKAO_FR_ClearUser(_okaoFaceAlbum, albumEntryPair.first);
      if (OKAO_NORMAL != okaoResult) {
        LOG_WARNING("FaceRecognizer.MergeFaces.ClearUserFailed",
                    "AlbumEntry:%d from mergeID:%d", albumEntryPair.first,
                    mergeID);
        return RESULT_FAIL;
      }
    }
  ...
  if (kFaceRecognitionExtraDebug) {
    LOG_INFO("MergeFaces.Success",
             "Merged FaceID %d into %d. Kept %zu album entries.", mergeID,
             keepID, keepIter->second.GetAlbumEntries().size());
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result FaceRecognizer::GetSerializedAlbum(
    std::vector<u8>& serializedAlbum) const {
  if (!_isInitialized) {
  ...
  tempAlbumHandle = OKAO_FR_CreateAlbumHandle(
      _okaoCommonHandle, kMaxTotalAlbumEntries, kMaxEnrollDataPerAlbumEntry);
  ...
  for (auto const& enrollData : _enrollmentData) {
    if (permanentIdCount >= kMaxNamedFacesInAlbum) {
      LOG_WARNING("FaceRecognizer.GetSerializedAlbum.MaxNumFacesReached",
                  "Can't save more than %d faces", kMaxNamedFacesInAlbum);
      break;
    }
    ...
    if (enrollData.second.IsForThisSessionOnly() == false) {
      for (auto& albumEntryPair : enrollData.second.GetAlbumEntries()) {
        ...
        for (s32 iData = 0; iData < kMaxEnrollDataPerAlbumEntry; ++iData) {
          BOOL isRegistered = false;
          okaoResult = OKAO_FR_IsRegistered(_okaoFaceAlbum, albumEntry, iData,
                                            &isRegistered);
          if (OKAO_NORMAL != okaoResult) {
            LOG_WARNING("FaceRecognizer.GetSerializedAlbum.IsRegisteredFail",
                        "Could not get registration status for face:%d "
                        "albumEntry:%d data:%d",
          ...
          if (isRegistered) {
            // Copy this user's registered feature data to temp album for
            // serialization
            okaoResult =
                OKAO_FR_GetFeatureFromAlbum(_okaoFaceAlbum, albumEntry, iData,
                                            _okaoRecognitionFeatureHandle);
            if (OKAO_NORMAL != okaoResult) {
              LOG_WARNING(
                  "FaceRecognizer.GetSerializedAlbum.GetFeatureFail",
                  "Could not get feature for face:%d albumEntry:%d data:%d",
                  faceID, albumEntry, iData);
              return RESULT_FAIL;
            }

            okaoResult = OKAO_FR_RegisterData(tempAlbumHandle,
                                              _okaoRecognitionFeatureHandle,
                                              albumEntry, iData);
            if (OKAO_NORMAL != okaoResult) {
              LOG_WARNING("FaceRecognizer.GetSerializedAlbum.IsRegisteredFail",
                          "Could not register temp feature for face:%d "
                          "albumEntry:%d data:%d",
                          faceID, albumEntry, iData);
              return RESULT_FAIL;
            }
  ...
  if (permanentIdCount > 0) {
    ...
    okaoResult = OKAO_FR_SerializeAlbum(tempAlbumHandle, &(serializedAlbum[0]),
                                        albumSize);
    if (OKAO_NORMAL != okaoResult) {
    ...
    LOG_INFO("GetSerializedAlbum.AlbumSize",
             ...
             permanentIdCount, albumSize,
             (f32)albumSize / (f32)permanentIdCount);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result FaceRecognizer::SetSerializedAlbum(
    HCOMMON okaoCommonHandle, const std::vector<u8>& serializedAlbum,
    HALBUM& album) {
  if (NULL == okaoCommonHandle) {
  ...
  FR_ERROR error;
  album = OKAO_FR_RestoreAlbum(okaoCommonHandle,
                               const_cast<UINT8*>(serializedAlbum.data()),
                               (UINT32)serializedAlbum.size(), &error);
  if (NULL == album) {
  ...
  LOG_INFO("SetSerializedAlbum.RestoredAlbum",
           "Restored FaceLib album with %d album entries and %d data entries "
           "from %zu-byte serialized album",
...
Result FaceRecognizer::AssignNameToID(FaceID_t faceID, const std::string& name,
  ...
  if (mergeWithID != UnknownFaceID) {
    ...
    // Merge the two records, keeping mergeWithID, but use the name of the new
    // one, since that's what was just assigned
    Result lastResult = MergeFaces(mergeWithID, faceID);
    if (RESULT_OK != lastResult) {
      LOG_WARNING("FaceRecognizer.AssignNameToID.MergeFailed",
                  "Merging faceID:%d into mergeWithID:%d", faceID, mergeWithID);
      return lastResult;
    }
  } else {
    if (!CanAddNamedFace()) {
  ...
  if (iterToRename != _enrollmentData.end()) {
    if (kDisplayDebugEnrollmentImages) {
      ...
      Vision::Image::CloseDisplayWindow(
          GetDisplayName(iterToRename->second).c_str());
    }

    iterToRename->second.SetName(name);
  } else {
    LOG_WARNING("FaceRecognizer.AssignNameToID.InvalidID",
                "Unknown ID %d, ignoring name %s", faceID,
...
Result FaceRecognizer::EraseFace(FaceID_t faceID) {
  ...
  if (enrollIter != _enrollmentData.end()) {
    ...
    if (RESULT_OK != removeResult) {
      LOG_WARNING("FaceRecognizer.EraseName.RemoveUserFailed", "ID=%d",
                  enrollIter->first);
    }
    return removeResult;
  } else {
...
void FaceRecognizer::EraseAllFaces() {
  ...
  for (auto enrollIter = _enrollmentData.begin();
       enrollIter != _enrollmentData.end();) {
    enrollIter = RemoveUser(enrollIter);
  }

  // These should not be necessary since all the RemoveUser calls should have
  // done it, but...
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result FaceRecognizer::RenameFace(
    FaceID_t faceID, const std::string& oldName, const std::string& newName,
  ...
  if (enrollIter != _enrollmentData.end()) {
    if (enrollIter->second.GetName() == oldName) {
      ...
      LOG_INFO("FaceRecognizer.RenameFace.Success",
               "Renamed ID=%d from '%s' to '%s'", faceID,
               Util::HidePersonallyIdentifiableInfo(oldName.c_str()),
               enrollIter->second.GetName().piiGuardedString());

      // Construct and then swap to make sure we miss fields that get added to
      // LoadedKnownFace later
      ...
      LOG_WARNING("FaceRecognizer.RenameFace.OldNameMismatch",
                  ...
                  Util::HidePersonallyIdentifiableInfo(oldName.c_str()),
                  enrollIter->second.GetName().piiGuardedString(), faceID);

      return RESULT_FAIL;
    }
  } else {
    LOG_WARNING("FaceRecognizer.RenameFace.InvalidID", "No record for ID=%d",
                faceID);
    return RESULT_FAIL;
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result FaceRecognizer::SetSerializedData(
  ...
  // Note that we load into these temporary data structures and swap them into
  // the real ones later
  HALBUM loadedAlbum = NULL;
  EnrollmentData loadedEnrollmentData;

  Result lastResult =
      SetSerializedAlbum(_okaoCommonHandle, albumData, loadedAlbum);
  ...
  if (RESULT_OK != lastResult) {
    ...
    if (RESULT_OK != lastResult) {
      LOG_WARNING(
          "FaceRecognizer.SetSerializedData.SetSerializedEnrollDataFail", "");
    } else {
      lastResult =
          UseLoadedAlbumAndEnrollData(loadedAlbum, loadedEnrollmentData);

      if (RESULT_OK == lastResult) {
        LOG_INFO("SetSerializedData.NewNextFaceID", "Setting next FaceID=%d",
                 loadedNextFaceID);
        ...
        for (auto& entry : _enrollmentData) {
          ...
          LOG_INFO("robot.vision.loaded_face_enrollment_entry",
                   "numAlbumEntries %zu, faceID %d", numAlbumEntries, faceID);
          ...
          EmplaceLoadedKnownFace(entry.second, loadedFaces,
                                 "SetSerializedData.AddedEnrollmentDataEntry");
        }
      }
    }
  }
  ...
  //
  // UseLoadedAlbumAndEnrollData() may decide to keep using an existing album
  // handle or the one we just created. Either way, we need to clean up the
  // handle that is NOT being kept open.
  //
  if (loadedAlbum != NULL) {
    LOG_DEBUG("FaceRecognizer.SetSerializedData.DeleteAlbumHandle",
              "Delete album handle %p", loadedAlbum);
    DEV_ASSERT(loadedAlbum != _okaoFaceAlbum,
               "FaceRecognizer.SetSerializedData.InvalidAlbumHandle");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result FaceRecognizer::UseLoadedAlbumAndEnrollData(
    HALBUM& loadedAlbumData, EnrollmentData& loadedEnrollmentData) {
  ...
  Result lastResult = SanityCheckBookkeeping(
      loadedAlbumData, loadedEnrollmentData, loadedAlbumEntryToFaceID);

  if (lastResult == RESULT_OK) {
    // Only if sanity checks pass do we actually swap in the loaded
    // album/enrollment data
    INT32 currentMaxAlbumEntries = 0, loadedMaxAlbumEntries = 0;
    INT32 currentMaxDataEntries = 0, loadedMaxDataEntries = 0;

    OkaoResult okaoResult = OKAO_FR_GetAlbumMaxNum(
        _okaoFaceAlbum, &currentMaxAlbumEntries, &currentMaxDataEntries);
    if (OKAO_NORMAL != okaoResult) {
      LOG_WARNING(
          "FaceRecognizer.UseLoadedAlbumAndEnrollData.GetCurrentMaxNumFailed",
          "FaceLib Result=%d", okaoResult);
      return RESULT_FAIL;
    }

    okaoResult = OKAO_FR_GetAlbumMaxNum(loadedAlbumData, &loadedMaxAlbumEntries,
                                        &loadedMaxDataEntries);
    if (OKAO_NORMAL != okaoResult) {
      LOG_WARNING(
          "FaceRecognizer.UseLoadedAlbumAndEnrollData.GetLoadedMaxNumFailed",
          "FaceLib Result=%d", okaoResult);
      return RESULT_FAIL;
    }

    if (loadedMaxAlbumEntries > currentMaxAlbumEntries ||
        loadedMaxDataEntries > currentMaxDataEntries) {
      LOG_WARNING(
          "FaceRecognizer.UseLoadedAlbumAndEnrollmentData.LoadedMaxNumTooLarge",
          "Loaded album too large (maxAlbumEntries:%d maxDataEntries:%d) too "
          "large "
          "for current settings (%d and %d)",
          loadedMaxAlbumEntries, loadedMaxDataEntries, currentMaxAlbumEntries,
          currentMaxDataEntries);
    ...
    if (loadedMaxAlbumEntries == currentMaxAlbumEntries &&
      ...
      LOG_DEBUG("FaceRecognizer.UseLoadedAlbumAndEnrollmentData.Swap",
                "Stop using %p, start using %p", _okaoFaceAlbum,
                loadedAlbumData);
      ...
      LOG_DEBUG("FaceRecognizer.UseLoadedAlbumAndEnrollmentData.Update",
                "Keep using %p, copy data from %p", _okaoFaceAlbum,
                loadedAlbumData);

      okaoResult = OKAO_FR_ClearAlbum(_okaoFaceAlbum);
      if (OKAO_NORMAL != okaoResult) {
        LOG_WARNING(
            "FaceRecognizer.UserLoadedAlbumAndEnrollmentData.ClearAlbumFailed",
            "FaceLib Result=%d", okaoResult);
        return RESULT_FAIL;
      }

      LOG_INFO(
          "UseLoadedAlbumAndEnrollmentData.ManualLoadDueToDifferingMaxNums",
          "Loaded album has smaller max album/data entries than current "
          "settings (%d/%d vs. %d/%d) "
          "Loading each manually.",
          loadedMaxAlbumEntries, loadedMaxDataEntries, currentMaxAlbumEntries,
          currentMaxDataEntries);

      for (AlbumEntryID_t iAlbumEntry = 0; iAlbumEntry < loadedMaxAlbumEntries;
           ++iAlbumEntry) {
        for (INT32 iData = 0; iData < loadedMaxDataEntries; ++iData) {
          BOOL isRegistered = false;
          okaoResult = OKAO_FR_IsRegistered(loadedAlbumData, iAlbumEntry, iData,
                                            &isRegistered);
          if (OKAO_NORMAL != okaoResult) {
            LOG_WARNING(
                "FaceRecognizer.UserLoadedAlbumAndEnrollmentData."
                "IsRegisteredFailed",
                "AlbumEntry:%d DataEntry:%d FaceLib Result=%d", iAlbumEntry,
                iData, okaoResult);
            return RESULT_FAIL;
          }
          ...
          if (isRegistered) {
            okaoResult =
                OKAO_FR_GetFeatureFromAlbum(loadedAlbumData, iAlbumEntry, iData,
                                            _okaoRecogMergeFeatureHandle);
            if (OKAO_NORMAL != okaoResult) {
              LOG_WARNING(
                  "FaceRecognizer.UserLoadedAlbumAndEnrollmentData."
            ...
            okaoResult = OKAO_FR_RegisterData(_okaoFaceAlbum,
                                              _okaoRecogMergeFeatureHandle,
                                              iAlbumEntry, iData);
            if (OKAO_NORMAL != okaoResult) {
              LOG_WARNING(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result FaceRecognizer::GetSerializedEnrollData(
    std::vector<u8>& serializedEnrollData) {
  ...
  if (!_enrollmentData.empty()) {
    ...
    std::copy(VersionPrefixU8, VersionPrefixU8 + 4,
              std::back_inserter(serializedEnrollData));

    DEV_ASSERT(
        ((u32*)serializedEnrollData.data())[0] == ((u32*)VersionPrefix)[0],
        "FaceRecognizer.GetSerialzedEnrollData.AddingVersionPrefixFailed");

    LOG_INFO("GetSerializedEnrollData.AddedVersionData",
             "Added to front: %04X%04X", ((u16*)serializedEnrollData.data())[0],
    ...
    std::copy(faceIdU8, faceIdU8 + sizeof(FaceID_t),
              std::back_inserter(serializedEnrollData));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result FaceRecognizer::SetSerializedEnrollData(
    const std::vector<u8>& serializedEnrollData,
    EnrollmentData& newEnrollmentData, FaceID_t& newNextFaceID) {
  ...
  if (serializedEnrollData.size() < kMinSize) {
    LOG_WARNING(
        "FaceRecognizer.SetSerializedEnrollData.TooShortForVersion",
  ...
  if (incomingVersion != correctVersion) {
    LOG_WARNING("FaceRecognizer.SetSerializedEnrollData.VersionPrefixMismatch",
                "Expected: %04X%04X, Incoming: %04X%04X", VersionPrefix[0],
                VersionPrefix[1], ((u16*)serializedEnrollData.data())[0],
                ((u16*)serializedEnrollData.data())[1]);
  ...
  while (startIndex < serializedEnrollData.size() -
                          3)  // "-3" to handle an extra bytes of padding
  {
    ...
    if (RESULT_OK != lastResult) {
      LOG_WARNING("FaceRecognizer.SetSerializedEnrollData.DeserializeFail",
                  "Failed to deserialize EnrolledFaceEntry at start=%zu (from "
                  "%zu-length buffer)",
...
Result FaceRecognizer::SaveAlbum(const std::string& albumName) {
  ...
  if (false == Util::FileUtils::CreateDirectory(albumName, false, true)) {
    ...
    if (!fs.is_open()) {
      LOG_WARNING("FaceRecognizer.SaveAlbum.FileOpenFail", "Filename: %s",
                  dataFilename.c_str());
      result = RESULT_FAIL;
    } else {
      ...
      if ((fs.rdstate() & std::ios::badbit) ||
          (fs.rdstate() & std::ios::failbit)) {
        LOG_WARNING("FaceRecognizer.SaveAlbum.FileWriteFail", "Filename: %s",
                    dataFilename.c_str());
        result = RESULT_FAIL;
      } else {
        Json::Value json;
...
Result FaceRecognizer::LoadAlbum(const std::string& albumName,
  ...
  // Loading a new album, even an "empty" or nonexistent one, clears any
  // existing named or session-only faces.
  EraseAllFaces();
  ...
  const std::string dataFilename(
      Util::FileUtils::FullFilePath({albumName, "data.bin"}));
  std::ifstream fs(dataFilename, std::ios::in | std::ios::binary);
  if (!fs.is_open()) {
    // At this point, we've verified the album directory exists, so we should be
    // able to load the data.bin file
    LOG_WARNING("FaceRecognizer.LoadAlbum.FileOpenFail", "Filename: %s",
                dataFilename.c_str());
    return RESULT_FAIL;
  } else {
    ...
    if (serializedAlbum.size() != fileLength) {
      LOG_WARNING("FaceRecognizer.LoadAlbum.FileReadFail", "Filename: %s",
                  dataFilename.c_str());
      return RESULT_FAIL;
    } else {
      // Temporary data structures to load into, so we can check they are
      // consistent before using them
      HALBUM loadedAlbum;
      EnrollmentData loadedEnrollmentData;

      result =
          SetSerializedAlbum(_okaoCommonHandle, serializedAlbum, loadedAlbum);
        ...
        if (!success) {
          ...
          for (auto& idStr : json.getMemberNames()) {
            ...
            if (!json.isMember(idStr)) {
              ...
              EmplaceLoadedKnownFace(entry, namesAndIDs,
                                     "LoadAlbum.LoadedEnrollmentData");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool FaceRecognizer::GetFaceIDFromTrackingID(const TrackingID_t trackingID,
                                             FaceID_t& faceID) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::string FaceRecognizer::GetBestGuessNameForTrackingID(
    const TrackingID_t trackingID) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result FaceRecognizer::ComputeFeaturesFromFace(const Image& img,
...

  {
    ...
    const auto& leftEyeConf =
        face.GetFeatureConfidence(TrackedFace::FeatureName::LeftEye);
...

  {
    ...
    const auto& rightEyeConf =
        face.GetFeatureConfidence(TrackedFace::FeatureName::RightEye);
...

  {
    ...
    const auto& noseConf =
        face.GetFeatureConfidence(TrackedFace::FeatureName::Nose);
...

  {
    ...
    const auto& mouthConf =
        face.GetFeatureConfidence(TrackedFace::FeatureName::UpperLip);
  ...
  OkaoResult okaoResult = OKAO_FR_ExtractPoints_GRAY(
      featureHandle, dataPtr, nWidth, nHeight, GRAY_ORDER_Y0Y1Y2Y3,
      FR_PTPOINT_KIND_MAX, aptPoint, anConfidence);
  if (OKAO_NORMAL != okaoResult) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result FaceRecognizer::DevAddFaceToAlbum(const Image& img,
                                         const TrackedFace& face,
                                         int albumEntry) {
  const Result result =
      ComputeFeaturesFromFace(img, face, _okaoRecognitionFeatureHandle);
  if (RESULT_OK != result) {
  ...
  OkaoResult okaoResult =
      OKAO_FR_GetAlbumMaxNum(_okaoFaceAlbum, &maxUserNum, &maxUserDataNum);
  if (OKAO_NORMAL != okaoResult) {
    LOG_ERROR("FaceRecognizer.DevAddFaceToAlbum.GetAlbumMaxFailed", "");
    return RESULT_FAIL;
  }
  if (albumEntry >= maxUserNum) {
    LOG_ERROR("FaceRecognizer.DevAddFaceToAlbum.AlbumFull",
              "MaxUsers:%d AlbumEntry:%d", maxUserNum, albumEntry);
    return RESULT_FAIL;
  }

  INT32 numUserData = 0;
  okaoResult =
      OKAO_FR_GetRegisteredUsrDataNum(_okaoFaceAlbum, albumEntry, &numUserData);
  if (OKAO_NORMAL != okaoResult) {
  ...
  if (numUserData >= maxUserDataNum) {
    LOG_ERROR("FaceRecognizer.DevAddFaceToAlbum.UserFull",
              "AlbumEntry:%d MaxData:%d UserData:%d", albumEntry,
              maxUserDataNum, numUserData);
    return RESULT_FAIL;
  }

  okaoResult = OKAO_FR_RegisterData(
      _okaoFaceAlbum, _okaoRecognitionFeatureHandle, albumEntry, numUserData);
  if (OKAO_NORMAL != okaoResult) {
    LOG_ERROR("FaceRecognizer.DevAddFaceToAlbum.RegisterDataFailed",
              "AlbumEntry:%d Data:%d Error Code: %d", albumEntry, numUserData,
              okaoResult);
    return RESULT_FAIL;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result FaceRecognizer::DevFindFaceInAlbum(const Image& img,
                                          const TrackedFace& face,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result FaceRecognizer::DevFindFaceInAlbum(
    const Image& img, const TrackedFace& face, const int maxMatches,
  ...
  INT32 resultNum = 0;
  OkaoResult okaoResult = OKAO_FR_Identify(
      _okaoRecognitionFeatureHandle, _okaoFaceAlbum, maxMatches, userIDs.data(),
      rawScores.data(), &resultNum);
  if (OKAO_NORMAL != okaoResult) {
    LOG_ERROR("FaceRecognizer.FindFaceInAlbum.ComputeFeaturesFailed", "");
    return RESULT_FAIL;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
float FaceRecognizer::DevComputePairwiseMatchScore(int faceID1,
                                                   int faceID2) const {
  OkaoResult okaoResult = OKAO_FR_GetFeatureFromAlbum(
      _okaoFaceAlbum, faceID1, 0, _okaoRecognitionFeatureHandle);
  if (OKAO_NORMAL != okaoResult) {
  ...
  INT32 scoreInt = 0;
  okaoResult = OKAO_FR_Verify(_okaoRecognitionFeatureHandle, _okaoFaceAlbum,
                              faceID2, &scoreInt);
  if (OKAO_NORMAL != okaoResult) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
float FaceRecognizer::DevComputePairwiseMatchScore(
    int faceID1, const Image& img2, const TrackedFace& face2) const {
  HFEATURE features2 = OKAO_FR_CreateFeatureHandle(_okaoCommonHandle);
  if (NULL == features2) {
    LOG_ERROR(
        "FaceRecognizer.DevComputePairwiseMatchScore."
        "FaceLibFeatureHandle1AllocFail",
        "");
    return -1.f;
  }
  ...
  if (result != RESULT_OK) {
    LOG_ERROR(
        "FaceRecognizer.DevComputePairwiseMatchScore.ComputeFeaturesFailed",
        "");
    return -1.f;
  } else {
    const OkaoResult okaoResult =
        OKAO_FR_Verify(features2, _okaoFaceAlbum, faceID1, &score);
    if (OKAO_NORMAL != okaoResult) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
float FaceRecognizer::DevComputePairwiseMatchScore(const Image& img1,
                                                   const TrackedFace& face1,
                                                   const Image& img2,
                                                   const TrackedFace& face2) {
  ...
  if (NULL == features2) {
    LOG_ERROR(
        "FaceRecognizer.DevComputePairwiseMatchScore."
        "FaceLibFeatureHandle2AllocFail",
        "");
  ...
  for (const auto& albumEntry : _enrollmentImages) {
    ...
    for (const auto& enrollmentImage : albumEntry.second) {
      const std::string fullFilename =
          imagePathPrefix + "_" + std::to_string(_albumEntryToFaceID[entryId]) +
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void FaceRecognizer::DeleteAllRecognitionImages() { _enrollmentImages.clear(); }
#endif  // ANKI_DEV_CHEATS

//
// This may prove useful later if we ever want to try to do more complicated /
// smarter merging of records based on cluster similarity / dissimilarity. I ran
// out of time and steam working on this, but rather than it getting totally
// lost in git history, I'm going to deliberately leave it here for now.
//
//  Result FaceRecognizer::SelectiveMergeHelper(EnrollmentData::iterator
//  keepIter,
//                                              EnrollmentData::iterator
//                                              mergeIter)
//  {
//    // - Create one big list of all the album entries from both mergeID and
//    keepID
//    // - Compute pairwise comparison of all the entries
//    // - Start with everything in the "remove" set
//    // - First choose the two entries that are furthest apart and move to the
//    "keep" set
//    // - Incrementally move entries from "remove" to "keep" according to which
//    one
//    //    has the nearest neighbor in the current "keep" set that's furthest
//    away (this
...
//
//    // Start with everything in the "remove" set. We will shift things from
//    this
...
//    {
//      // Since Okao does not offer a way to do albumEntry to albumEntry (i.e.
//      "userID" to "userID")
//      // comparisons (??), I'm using the maximum score of any one of entry_i's
//      features
//      // to entry_j as a proxy. So I have to loop over each data entry in
//      albumEntry i
//      // and compare it to albumEntry j using the "Verify" function to get a
//      score. for(s32 iData = 0; iData < kMaxEnrollDataPerAlbumEntry; ++iData)
//      {
//        BOOL isRegistered = false;
//        okaoResult = OKAO_FR_IsRegistered(_okaoFaceAlbum,
//        combinedEntries[iComp], iData, &isRegistered); if(OKAO_NORMAL !=
//        okaoResult) {
...
//        }
//
//        okaoResult = OKAO_FR_GetFeatureFromAlbum(_okaoFaceAlbum,
//        combinedEntries[iComp], iData, _okaoRecogMergeFeatureHandle);
//        if(OKAO_NORMAL != okaoResult) {
//          LOG_ERROR("FaceRecognizer.SelectiveMergeHelper.GetFeatureFromMergeAlbumFailed",
//                    "AlbumEntry:%d Data:%d FaceLib result=%d",
//                    combinedEntries[iComp], iData, okaoResult);
//          return RESULT_FAIL;
//        }
//
...
//
//        // Note we start jComp at iComp+1 because (a) comparisons are
//        symmetric so we can
//        // update two entries at once, and (b) there's no reason to compare an
//        entry to itself for(s32 jComp = iComp + 1; jComp <
//        combinedEntries.size(); ++jComp)
//        {
//          RecognitionScore currentScore = 0;
//          okaoResult = OKAO_FR_Verify(_okaoRecogMergeFeatureHandle,
//          _okaoFaceAlbum,
//                                      combinedEntries[jComp], &currentScore);
//          if(OKAO_NORMAL != okaoResult) {
//            LOG_ERROR("FaceRecognizer.SelectiveMergeHelper.VerifyFailed",
//                              "Comparing AlbumEntry %d and %d. FaceLib
//                              result=%d", combinedEntries[iComp],
//                              combinedEntries[jComp], okaoResult);
//            return RESULT_FAIL;
//          }
//
...
//    }
//
//    // Find the minimum scoring entry in the matrix and keep both
//    corresponding AlbumEntries double minScore = 0; cv::Point minLoc;
//    cv::minMaxLoc(pairwiseScores.get_CvMat_(), &minScore, nullptr, &minLoc,
//    nullptr);
//
...
//             "Keeping indices %d and %d (albumEntries %d and %d, from FaceIDs
//             %d and %d) " "with pairwise score %d", firstToKeep, secondToKeep,
...
//
//    // Now repeatedly find the entry not yet selected to keep (i.e. still in
//    the "remove"
//    // set) that has the lowest max score to whatever is currently in the set
//    of keepers.
//    // This will be the entry who is furthest away from its closest neighbor
//    in the keeper set. while(indicesToKeep.size() < kMaxAlbumEntriesPerFace &&
//    !indicesToRemove.empty())
//    {
//      RecognitionScore minMaxScore =
//      std::numeric_limits<RecognitionScore>::max(); s32 nextToAdd = -1;
...
//
//      LOG_INFO("FaceRecognizerDebug",
//      "SelectiveMergeHelper.KeepingAlbumEntry",
//               "Index:%d AlbumEntry:%d from FaceID:%d with minimum
//               maxScore:%d", nextToAdd, combinedEntries.at(nextToAdd),
//               _albumEntryToFaceID.at(combinedEntries.at(nextToAdd)),
//               minMaxScore);
//    }
//
...
//      EnrolledFaceEntry::Time origTime =
//      keepIter->second.GetAlbumEntries().at(albumEntry);
//      keepIter->second.AddOrUpdateAlbumEntry( albumEntry, origTime ); // NOTE:
//      Does nothing if already an entry _albumEntryToFaceID[albumEntry] =
//      keepIter->first;
//    }
//
...
//    }
//
//    ASSERT_NAMED_EVENT(keepIter->second.GetAlbumEntries().size() ==
