...
const TrackedFace::ExpressionValues& TrackedFace::GetExpressionValues() const {
  if (ANKI_DEVELOPER_CODE) {
    // Documentation (in trackedFace.h and in Okao expression manual) says this
    // should be a histogram summing to 100. Verify that's true (in debug
    // builds).
    s32 sum = 0;
    ...
    DEV_ASSERT_MSG(noExpressionValues || (sum == 100),
                   "TrackedFace.GetExpressionValues.HistSumNot100", "%d", sum);
  }
...
FacialExpression TrackedFace::GetMaxExpression(
    ExpressionValue* valuePtr) const {
  ...
  // This is used to increment through the expression array, starting from the 0
  // index, and using the underlying type of the FacialExpression enum
...
void TrackedFace::SetExpressionValue(FacialExpression whichExpression,
                                     ExpressionValue newValue) {
  DEV_ASSERT_MSG(
      Util::InRange(newValue, ExpressionValue(0), ExpressionValue(100)),
      "TrackedFace.SetExpressionValue.BadValue", "%s:%d",
      EnumToString(whichExpression), newValue);
