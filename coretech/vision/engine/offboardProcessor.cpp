...
const char* const OffboardProcessor::Filenames::Result =
    "offboardProcResult.json";
...
const char* const OffboardProcessor::JsonKeys::TimeoutDuration =
    "timeoutDuration_sec";
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - NOTE: Not using "default" in the header
// b/c of forward-declared LocalUdpClient
...
Result OffboardProcessor::Init(const std::string& name,
  ...
  // NOTE: This implementation of an INeuralNetRunner::Model does not actual
  // load any model as it is communicating
  //       with a standalone process which is loading and running the model.
  //       Here we'll just set any inititalization parameters for communicating
  //       with that process.

  _params.name = name;

  if (false == JsonTools::GetValueOptional(config, JsonKeys::PollingPeriod,
                                           _params.pollingPeriod_ms)) {
    LOG_ERROR("OffboardModel.LoadModelInternal.MissingPollPeriod", "");
    return RESULT_FAIL;
  }

  if (false == JsonTools::GetValueOptional(config, JsonKeys::TimeoutDuration,
  ...
  if (false == JsonTools::GetValueOptional(config, JsonKeys::CommsType,
                                           commsTypeString)) {
    LOG_ERROR("OffboardModel.LoadModelInternal.MissingCommsType", "");
    return RESULT_FAIL;
  } else {
    const bool success =
        OffboardCommsTypeFromString(commsTypeString, _params.commsType);
    if (!success) {
      LOG_ERROR("OffboardProcessor.LoadModelInternal.BadCommsType", "%s",
                commsTypeString.c_str());
      return RESULT_FAIL;
    }
  }
  ...
  if (jsonProcTypes.isArray()) {
    for (auto const& jsonProcType : jsonProcTypes) {
      _params.procTypes.emplace_back(
          JsonTools::GetValue<std::string>(jsonProcType));
    }
  } else {
    _params.procTypes.emplace_back(
        JsonTools::GetValue<std::string>(jsonProcTypes));
  }
  ...
  if (_params.pollingPeriod_ms <= 0) {
    LOG_ERROR("OffboardProcessor.Init.BadPollPeriod", "%d",
              _params.pollingPeriod_ms);
    return RESULT_FAIL;
  }
  if (Util::IsFltLEZero(_params.timeoutDuration_sec)) {
    LOG_ERROR("OffboardProcessor.Init.BadTimeout", "%f",
              _params.timeoutDuration_sec);
    return RESULT_FAIL;
  }
  ...
  LOG_INFO("OffboardProcessor.Init.Success", "Polling period: %dms, Cache: %s",
           _params.pollingPeriod_ms, _cachePath.c_str());

  // Anything not talking over FileIO is assumed to be talking over UDP
  if ((OffboardCommsType::FileIO != _params.commsType) &&
      _udpClient == nullptr) {
    if (!ANKI_DEV_CHEATS) {
...
#ifdef VICOS
    // Right now we only support vicos for offboard vision per the
    // implementation in vic-cloud, if you're running on mac ... there isn't
    // really a use case for offboard ... yet.
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result OffboardProcessor::Detect(
    Vision::ImageRGB& img, std::list<Vision::SalientPoint>& salientPoints) {
  salientPoints.clear();
  ...
  switch (_params.commsType) {
    ...
    case OffboardCommsType::CLAD: {
      ...
      if (connected) {
        ...
        LOG_WARNING("OffboardProcessor.Detect.StillNotConnected",
                    "Will keep trying");
        return RESULT_FAIL;
      }
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result OffboardProcessor::DetectWithFileIO(
    const ImageRGB& img, std::list<SalientPoint>& salientPoints) {
  const std::string imageFilename =
      Util::FileUtils::FullFilePath({_cachePath, Filenames::Image});
  {
    // Write image to a temporary file
    const std::string tempFilename =
        Util::FileUtils::FullFilePath({_cachePath, "temp.png"});
    img.Save(tempFilename);

    // Write timestamp to file
    const std::string timestampFilename =
        Util::FileUtils::FullFilePath({_cachePath, Filenames::Timestamp});
    ...
    // Rename to what the "offboard processor" expects once the data is fully
    // written (poor man's "lock")
    ...
    LOG_DEBUG("OffboardProcessor.DetectWithFileIO.SavedImageFileForProcessing",
              "%s t:%d", imageFilename.c_str(), img.GetTimestamp());
  }

  // Wait for detection result JSON to appear (blocking until timeout!)
  const std::string resultFilename =
      Util::FileUtils::FullFilePath({_cachePath, Filenames::Result});
...
#pragma unused(resultAvailable)
  ...
  LOG_DEBUG("NeuralNetRunner.DetectWithFileIO.DeletingImageFile",
            "%s, deleting %s",
            (resultAvailable ? "Result found" : "Polling timed out"),
            imageFilename.c_str());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result OffboardProcessor::DetectWithCLAD(
    const ImageRGB& img, std::list<SalientPoint>& salientPoints) {
  // Until we have a way to send image data via CLAD (or protobuf), just write
  // to file
  const std::string imageFilename =
      Util::FileUtils::FullFilePath({_cachePath, Filenames::Image});
  const Result saveResult = img.Save(imageFilename);
  if (RESULT_OK != saveResult) {
    LOG_ERROR("OffboardProcessor.DetectWithCLAD.SaveFailed", "%s",
              imageFilename.c_str());
    return saveResult;
  }

  LOG_DEBUG("OffboardProcessor.DetectWithCLAD.SavedImageFileForProcessing",
            "%s t:%d", imageFilename.c_str(), img.GetTimestamp());

  const bool kIsCompressed = true;  // We chose to use JPG above
  const bool kIsEncrypted = false;
  Vision::OffboardImageReady imageReadyMsg(
      img.GetTimestamp(), img.GetNumRows(), img.GetNumCols(),
      img.GetNumChannels(), kIsCompressed, kIsEncrypted, _params.procTypes,
      imageFilename);
  ...
  if (!ANKI_VERIFY(packedSize == expectedSize,
                   "OffboardProcessor.DetectWithCLAD.MessageSizeMismatch",
                   "PackedSize:%zu ExpectedSize:%zu", packedSize,
                   expectedSize)) {
    return RESULT_FAIL;
  }

  const size_t sentSize =
      _udpClient->Send((const char*)messageData.data(), packedSize);
  if (!ANKI_VERIFY(
          sentSize == packedSize,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool OffboardProcessor::WaitForResultFile(
    const std::string& resultFilename, std::list<SalientPoint>& salientPoints) {
  bool resultAvailable = false;
  const f32 startTime_sec =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  f32 currentTime_sec = startTime_sec;

  while (!resultAvailable &&
         (currentTime_sec - startTime_sec < _params.timeoutDuration_sec)) {
    std::this_thread::sleep_for(
        std::chrono::milliseconds(_params.pollingPeriod_ms));
  ...
  if (resultAvailable) {
    LOG_DEBUG("OffboardProcessor.DetectWithFileIO.FoundDetectionResultsJSON",
              "%s", resultFilename.c_str());
    ...
    if (parseSuccess) {
      resultAvailable =
          ParseSalientPointsFromJson(detectionResult, _imageRows, _imageCols,
                                     _imageTimestamp, salientPoints);
    } else {
      LOG_ERROR("OffboardProcessor.WaitForResultFile.FailedToReadJSON", "%s",
    ...
    LOG_WARNING("OffboardProcessor.WaitForResultFile.PollingForResultTimedOut",
                "Start:%.1fsec Current:%.1f Timeout:%.1fsec", startTime_sec,
                currentTime_sec, _params.timeoutDuration_sec);
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool OffboardProcessor::WaitForResultCLAD(
  ...
  while (!resultAvailable &&
    ...
    while (_udpClient->IsConnected()) {
      LOG_PERIODIC_INFO(
          100, "OffboardProcessor.WaitForResultCLAD.CheckingForMessage", "");
      ...
      if (n < 0) {
        ...
        LOG_PERIODIC_INFO(
            100, "OffboardProcessor.WaitForResultCLAD.NothingToRead", "");
        std::this_thread::sleep_for(
            std::chrono::milliseconds(_params.pollingPeriod_ms));
        break;
      } else {
        LOG_INFO("OffboardProcessor.WaitForResultCLAD.ProcessMessage",
                 "Read %zu/%zu", n, sizeof(buf));
        ...
        if (resultReadyMsg.timestamp != _imageTimestamp) {
          LOG_INFO("OffboardProcessor.WaitForResultCLAD.DumpingOldResult",
                   "Found result with t:%u, instead of t:%u. Dropping on floor "
        ...
        const bool parseSuccess =
            reader.parse(resultReadyMsg.jsonResult, detectionResult);
        if (parseSuccess) {
          std::string keys;
          ...
          LOG_INFO("OffboardProcessor.WaitForResultCLAD.ParsedMessageJson",
                   "Keys:%s", keys.c_str());
          const Result parseResult = ParseSalientPointsFromJson(
              detectionResult, _imageRows, _imageCols, _imageTimestamp,
              salientPoints);
...
bool OffboardProcessor::Connect() {
  ...
  const std::string clientPath =
      std::string(LOCAL_SOCKET_PATH) + "_" + _params.name;

  const bool udpSuccess = _udpClient->Connect(clientPath, serverPath);
  ++numTries;

  LOG_INFO("OffboardProcessor.Connect.Status",
           "Try %d: %s - Server:%s Client:%s", numTries,
           (udpSuccess ? "Success" : "Fail"), serverPath, clientPath.c_str());
...
bool OffboardProcessor::IsConnected() const {
  if (ANKI_VERIFY(nullptr != _udpClient,
                  "OffboardProcessor.IsConnected.NoUdpClient",
                  "UDP client should be instantiated")) {
