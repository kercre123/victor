...
using MarkerMatch =
    std::pair<const Vision::ObservedMarker*, const Vision::KnownMarker*>;
...
class ObservableObject {
 public:
  ...
  // with that code.
  std::vector<KnownMarker*> const& GetMarkersWithCode(
      const Marker::Code& whichCode) const;
  ...
  void GetObservedMarkers(
      std::vector<const KnownMarker*>& observedMarkers) const;

  // Get the pose of the marker closest to the reference pose, optionally using
  // X and Y only (when ignoreZ=true). If no marker is found due to mismatched
  // origins match, then RESULT_FAIL_ORIGIN_MISMATCH is returned.
  ...
  // "Active" objects are those that can be communicated with and have lights,
  // accelerometers, etc.
  ...
  // Set the moving state of the object and when it either started or stopped
  // moving.
  ...
  void SetMarkerAsObserved(
      const ObservedMarker* nearestTo, const TimeStamp_t atTime,
      const f32 areaRatioThreshold =
          0.1f);  // i.e., 1 - abs(obsArea/knownArea) < threshold

  // Return true if any of the object's markers is visible from the given
  // camera. See also KnownMarker::IsVisibleFrom().
  bool IsVisibleFrom(const Camera& camera, const f32 maxFaceNormalAngle,
                     const f32 minMarkerImageSize,
                     const bool requireSomethingBehind,
                     const u16 xBorderPad = 0, const u16 yBorderPad = 0) const;
  ...
  bool IsVisibleFrom(const Camera& camera, const f32 maxFaceNormalAngle,
                     const f32 minMarkerImageSize, const u16 xBorderPad,
                     const u16 yBorderPad, bool& hasNothingBehind) const;

  // Similar to the above, but return the highest NotVisibleReason that was
  // returned for any of the markers (the latest check we got through for any
  // one marker).
  KnownMarker::NotVisibleReason IsVisibleFromWithReason(
      const Camera& camera, const f32 maxFaceNormalAngle,
      const f32 minMarkerImageSize, const bool requireSomethingBehind,
      const u16 xBorderPad = 0, const u16 yBorderPad = 0) const;
  ...
  virtual void SetPose(const Pose3d& newPose, f32 fromDistance = -1.f,
  ...
  // Copy observation times from another object, keeping the max counts / latest
  // times
  ...
  bool IsSameAs(const ObservableObject& otherObject,
                const Point3f& distThreshold, const Radians& angleThreshold,
                Point3f& Tdiff, Radians& angleDiff) const;
  ...
  Point3f GetSizeInParentFrame()
      const;  // use object's current pose w.r.t. origin
  ...
  // - A Z-translation moved from the object's center up along the origin's z
  // axis. The
  //   amount moved up is the given fraction of the object's height in the
  //   origin's z axis. (So 0 = object center, 0.5 = top of object (common
  //   case), -0.5 = btm of object)
  // - x/y rotation of 0 and z rotation equal to _poses's rotation relative to
  // origin
  ...
  Point3f GetRotatedBoundingCube() const { return
  GetRotatedBoundingCube(GetPose()); }
  */
  ...
  constexpr static f32 DEFAULT_SAME_DIST_TOL_FRACTION =
      0.8f;  // fraction of GetSize() to use
  ...
  // orientation around the Z axis (w.r.t. its parent), but no rotation about
  // the X and Y axes.
  bool IsRestingFlat(const Radians& angleTol = DEG_TO_RAD(10)) const;

  // If the pose is within angleTol of being "flat" clamp it to exactly flat.
  ...
  // Check if the bottom of an object is resting at a certain height within a
  // tolerance
  bool IsRestingAtHeight(float height, float tolerance) const;

  // Helper to check whether a pose is too high above an object:
  // True if pose.z is more than heightTol above (object.z +
  // (heightMultiplier*object.height)), where the object.height is its height
  // relative to the pose's parent frame. If offsetFraction != 0, then (pose.z +
  // object.height*offsetFraction) is used. This is useful for comparing the top
  // or bottom of the object (by using offsetFraction=0.5 or -0.5,
  // respectively).
...
inline const ObjectID& ObservableObject::GetID() const { return _ID; }

inline const ColorRGBA& ObservableObject::GetColor() const { return _color; }
...
inline bool ObservableObject::IsSameAs(
    const ObservableObject& otherObject) const {
  return IsSameAs(otherObject, this->GetSameDistanceTolerance(),
                  this->GetSameAngleTolerance());
}
...
inline void ObservableObject::Visualize() const { Visualize(_color); }
...
inline Point3f ObservableObject::GetSameDistanceTolerance() const {
  // TODO: This is only ok because we're only using totally symmetric 1x1x1
  // blocks at the moment.
  //       The proper way to do the IsSameAs check is to have objects return a
  //       scaled down bounding box of themselves and see if the the origin of
  //       the candidate object is within it. COZMO-9440
...
inline bool ObservableObject::IsSameAs(const ObservableObject& otherObject,
  ...
  Point3f Tdiff;
  Radians angleDiff;
  return IsSameAs(otherObject, distThreshold, angleThreshold, Tdiff, angleDiff);
}
...
inline void ObservableObject::GetObservedMarkers(
    std::vector<const KnownMarker*>& observedMarkers) const {
...
inline Point3f ObservableObject::GetSizeInParentFrame(
    const Pose3d& atPose) const {
