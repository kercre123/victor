...
class MarkerDetector {
 public:
  ...
  Result Detect(const Image& inputImage,
                std::list<ObservedMarker>& observedMarkers);

 private:
  const Camera& _camera;

  // Using forward declaration and unique_ptr pattern for Parameters and Memory
  // because those internally rely on old, crazy "Embedded" code and data
  // structures and we don't want to expose that outside of this class.
