...
class FaceTracker::Impl : public Profiler {
 ...
 private:
  ...
  u32 _frameCtr = 0;
  const u32 _checkForNewFacesEveryNthFrame =
      5;  // TODO: Make a settable parameter?
...
FaceTracker::Impl::Impl(const std::string& modelPath,
  ...
  const std::string faceCascadeFilename =
      modelPath + "/haarcascade_frontalface_alt2.xml";
  ...
  const std::string eyeCascadeFilename =
      modelPath + "/haarcascade_eye_tree_eyeglasses.xml";
...
Result FaceTracker::Impl::Update(
    const Vision::Image& frame, std::list<TrackedFace>& returnedFaces,
    std::list<FaceTracker::UpdatedID>& updatedIDs) {
  if (!_isInitialized) {
  ...
  if (_frameCtr++ == _checkForNewFacesEveryNthFrame) {
    ...
    // Keep a list of existing faces to check and remove any that we find
    // matches for in the loop below
    ...
    for (auto& newFaceRect : newFaceRects) {
      ...
      for (auto existingIter = existingFacesToCheck.begin();
        ...
        if (IoU > intersectionOverUnionThreshold) {
          // Update existing face and remove it from additional checking for
          // matches
      ...
      if (updatedRects.empty()) {
        ...
        if (updatedRects.size() > 1) {
          PRINT_NAMED_WARNING("FaceTracker.Impl.Update.MultipleFaceUpdates",
                              "Found more than one face in vicinity of "
                              "existing face. Using first.");
        }
        faceIter->second.SetRect(Rectangle<f32>(
            cvFaceRect.x + updatedRects[0].x, cvFaceRect.y + updatedRects[0].y,
  ...
  for (auto& facePair : _faces) {
    ...
    if (eyeRects.size() == 2) {
      ...
      Point2f rightEyeCen(
          faceRect.x + rightEyeRect.x + rightEyeRect.width / 2,
