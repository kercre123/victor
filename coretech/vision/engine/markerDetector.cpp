 ...
 * Date:   08/22/2017
 *
 * Description: Detector for Vision Markers, which wraps the old Embedded
 *implementation with an engine/basestation-friendly API.
 *
...
struct MarkerDetector::Parameters
    : public Embedded::FiducialDetectionParameters {
  s32 maxMarkers;
...
// This horrible mess is leftover from the original embedded implementation
// where we could not allocate memory. "Off Chip", "On Chip", and "CCM" were
// different blocks of memory with different speeds and max sizes. Someday it
// would be nice to remove all that...
class MarkerDetector::Memory {
 ...
 public:
  ...
  // This still relies on Embedded data structures so is part of "Memory"
  // instead of MarkerDetector class for now
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result MarkerDetector::Memory::ResetBuffers(s32 numRows, s32 numCols,
                                            s32 maxMarkers) {
  ...
  _offchipScratch = Embedded::MemoryStack(
      _offchipBuffer.data(), Util::numeric_cast<s32>(_offchipBuffer.size()));
  _onchipScratch = Embedded::MemoryStack(
      _onchipBuffer.data(), Util::numeric_cast<s32>(_onchipBuffer.size()));
  _ccmScratch = Embedded::MemoryStack(
      _ccmBuffer.data(), Util::numeric_cast<s32>(_ccmBuffer.size()));

  if (!_offchipScratch.IsValid() || !_onchipScratch.IsValid() ||
      !_ccmScratch.IsValid()) {
    PRINT_NAMED_ERROR("MarkerDetector.Memory.ResetBuffers.Failure",
                      "Failed to initialize scratch buffers");
    return RESULT_FAIL_MEMORY;
  }

  _markers = Embedded::FixedLengthList<Embedded::VisionMarker>(maxMarkers,
                                                               _offchipScratch);
...
MarkerDetector::MarkerDetector(const Camera& camera)
    : _camera(camera), _params(new Parameters()), _memory(new Memory()) {
  ...
  PRINT_NAMED_INFO("MarkerDetector.Constructor.LoadNearestNeighborLibrary",
                   "Markers generated on %s",
                   Vision::MarkerDefinitionVersionString);
  Embedded::VisionMarker::GetNearestNeighborLibrary();

#elif RECOGNITION_METHOD == RECOGNITION_METHOD_CNN
  // TODO: Will need to pass in a datapath from somewhere if we ever switch back
  // to using CNNs
...
Result MarkerDetector::Init(s32 numRows, s32 numCols) {
  ...
  Result initMemoryResult =
      _memory->ResetBuffers(numRows, numCols, _params->maxMarkers);
...
static Result GetImageHelper(const Vision::Image& srcImage,
  ...
  if (srcImage.GetNumRows() != captureHeight ||
      srcImage.GetNumCols() != captureWidth) {
    PRINT_NAMED_ERROR(
        "MarkerDetector.GetImageHelper.MismatchedImageSizes",
        "Source Vision::Image and destination Embedded::Array should "
        "be the same size (source is %dx%d and destination is %dx%d)",
        srcImage.GetNumRows(), srcImage.GetNumCols(), captureHeight,
        captureWidth);
    return RESULT_FAIL_INVALID_SIZE;
  }

  // Copy one row at a time (a) to support source ROIs, which have discontinuous
  // memory and (b) to make sure memory is aligned as expected for the
  // Embedded::Array data structure even for continuous data
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result MarkerDetector::Detect(const Image& inputImageGray,
                              std::list<ObservedMarker>& observedMarkers) {
  ANKI_CPU_PROFILE("MarkerDetector_Detect");

  DEV_ASSERT(_params->isInitialized,
             "MarkerDetector.Detect.ParamsNotInitialized");

  const Result memResult =
      _memory->ResetBuffers(inputImageGray.GetNumRows(),
                            inputImageGray.GetNumCols(), _params->maxMarkers);

  if (!ANKI_VERIFY(memResult == RESULT_OK,
                   "MarkerDetector.Detect.MemoryResetFailed", "")) {
    return memResult;
  }

  // Convert to an Embedded::Array<u8> so the old embedded methods can use the
  // image data.
  Embedded::Array<u8> grayscaleImage(
      inputImageGray.GetNumRows(), inputImageGray.GetNumCols(),
      _memory->_onchipScratch, Embedded::Flags::Buffer(false, false, false));
  ...
  DEV_ASSERT(
      Util::IsFltGTZero(_params->fiducialThicknessFraction.x) &&
  ...
  _params->SetComputeComponentMinNumPixels(inputImageGray.GetNumRows(),
                                           inputImageGray.GetNumCols());
  _params->SetComputeComponentMaxNumPixels(inputImageGray.GetNumRows(),
                                           inputImageGray.GetNumCols());
  ...
  const Result result = DetectFiducialMarkers(
      grayscaleImage, markers, *_params, kDarkOnLightMode, _memory->_ccmScratch,
      _memory->_onchipScratch, _memory->_offchipScratch);
  ...
  for (s32 i_marker = 0; i_marker < numMarkers; ++i_marker) {
    ...
    Quad2f quad(
        {crntMarker.corners[Embedded::Quadrilateral<f32>::TopLeft].x,
    ...
    observedMarkers.emplace_back(inputImageGray.GetTimestamp(),
                                 crntMarker.markerType, quad, _camera);
  }  // for(each marker)
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
MarkerDetector::Parameters::Parameters() : isInitialized(false) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void MarkerDetector::Parameters::SetComputeComponentMinNumPixels(s32 numRows,
                                                                 s32 numCols) {
  const f32 minSideLength =
      minSideLengthFraction * static_cast<f32>(MIN(numRows, numCols));
  const f32 interiorLength = component_numPixelsFraction * minSideLength;
  component_minimumNumPixels = std::round(minSideLength * minSideLength -
                                          (interiorLength * interiorLength));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void MarkerDetector::Parameters::SetComputeComponentMaxNumPixels(s32 numRows,
                                                                 s32 numCols) {
  const f32 maxSideLength =
      maxSideLengthFraction * static_cast<f32>(MAX(numRows, numCols));
  const f32 interiorLength = component_numPixelsFraction * maxSideLength;
  component_maximumNumPixels = std::round(maxSideLength * maxSideLength -
                                          (interiorLength * interiorLength));
}
...
void MarkerDetector::Parameters::Initialize() {
  ...
  // NOTE: 0.8 was good for dark-on-light (Cozmo) markers, 1.1 for light-on-dark
  // Victor markers
  ...
  // It seems strange that the number of pyramid levels is only 1, given the
  // desire to compute the "best" neighborhood size for binarization using
  // characteristic scale computation, but there may be a deeper bug since this
  // seemed to work best. Punting for now.
  scaleImage_numPyramidLevels = 1;

  // Smaller window sizes for binarization seem to allow us to break apart
  // nearly-connected regions better, at the possible expense of hollowing out
  // thick fiducial regions. However, unless we were to completely hollow out
  // the rounded fiducial rectangle into two separate "rings" (or break it
  // somewhere), this is likely generally less of a problem than the "leakage"
  // problem when the fiducial is far away and near another bright region.
  ...
  cornerMethod =
      Embedded::CORNER_METHOD_LINE_FITS;  // {CORNER_METHOD_LAPLACIAN_PEAKS,
                                          // CORNER_METHOD_LINE_FITS};
  ...
  quads_quadSymmetryThreshold =
      512;  // ANS: corresponds to 2.0, loosened from 384 (1.5), for large mat
            // markers at extreme perspective distortion
  ...
  // Radius of rounds as a fraction of the height/width of the fiducial
  // rectangle
