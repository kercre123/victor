...
// Literally a data type that represents the name of a user (which is generally
// mapped with a face) Giving it a concrete data type to abstract away the
// comparison of phonetically similar names
...
class UserName {
 public:
  ...
  // simulate a minimal set of std::string functions since that's how it's
  // expected to be used
  ...
  // use this for any debugging or clad messages where we want to obfuscate the
  // names of the users
  const char* piiGuardedString() const {
    return Util::HidePersonallyIdentifiableInfo(_name.c_str());
  }
...
class EnrolledFaceEntry {
 public:
  ...
  void SetTrackingID(TrackingID_t newID) {
    _prevTrackID = _trackID;
    _trackID = newID;
  }

  void ClearTrackingID() {
    _prevTrackID = UnknownFaceID;
    _trackID = UnknownFaceID;
  }
  ...
  const std::map<AlbumEntryID_t, Time>& GetAlbumEntries() const {
    return _albumEntrySeenTimes;
  }

  void AddOrUpdateAlbumEntry(AlbumEntryID_t entry, Time updateTime,
                             bool isSessionOnly);

  void RemoveAlbumEntry(AlbumEntryID_t entry) {
    _albumEntrySeenTimes.erase(entry);
  }

  void ClearAlbumEntries() {
  ...
  AlbumEntryID_t GetSessionOnlyAlbumEntry() const {
    return _sessionOnlyAlbumEntry;
  }

  // Get when this enrolled face was added to our album
  Time GetEnrollmentTime() const { return _enrollmentTime; }

  const std::list<FaceRecognitionMatch>& GetDebugMatchingInfo() const {
    return _debugMatchingInfo;
  }
  void SetDebugMatchingInfo(std::list<FaceRecognitionMatch>&& newDebugInfo);

  // Return a string containing the album entries, with session-only marked with
  // "S"
 ...
 protected:
  Result MergeAlbumEntriesHelper(const EnrolledFaceEntry& other,
  ...
  // Returns 0 and throws an error if there is a session-only entry but no time
  // found for it
  Time GetSessionOnlySeenTime() const;

  FaceID_t _faceID = UnknownFaceID;  // The ID used for recognition
  FaceID_t _prevID = UnknownFaceID;  // The previous ID if the ID just changed
                                     // (e.g. due to merge)

  TrackingID_t _trackID = UnknownFaceID;  // The last associated tracker ID
  TrackingID_t _prevTrackID =
      UnknownFaceID;  // The previous track ID in case it changed
  ...
  Time _lastDataUpdateTime;  // Last time we updated any album entry associated
                             // with this face

  // Stores album entries (effectively as a sorted, unique set) and the last
  // time we saw (i.e. matched to) that specific album entry.
