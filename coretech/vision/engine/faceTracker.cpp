...
FaceTracker::FaceTracker(const Camera& camera, const std::string& modelPath,
...
bool FaceTracker::CanAddNamedFace() const { return _pImpl->CanAddNamedFace(); }
...
void FaceTracker::EraseAllFaces() { return _pImpl->EraseAllFaces(); }

std::vector<Vision::LoadedKnownFace> FaceTracker::GetEnrolledNames() const {
  return _pImpl->GetEnrolledNames();
}

Result FaceTracker::RenameFace(FaceID_t faceID, const std::string& oldName,
...
void FaceTracker::PrintTiming() { _pImpl->PrintAverageTiming(); }

void FaceTracker::SetFaceEnrollmentMode(FaceID_t forFaceID, s32 numEnrollments,
                                        bool forceNewID) {
...
Result FaceTracker::DevAddFaceToAlbum(const Image& img, const TrackedFace& face,
                                      int albumEntry) {
  return _pImpl->DevAddFaceToAlbum(img, face, albumEntry);
}

Result FaceTracker::DevFindFaceInAlbum(const Image& img,
                                       const TrackedFace& face, int& albumEntry,
                                       float& score) const {
  return _pImpl->DevFindFaceInAlbum(img, face, albumEntry, score);
}

Result FaceTracker::DevFindFaceInAlbum(
...
float FaceTracker::DevComputePairwiseMatchScore(FaceID_t faceID1,
                                                FaceID_t faceID2) const {
  return _pImpl->DevComputePairwiseMatchScore(faceID1, faceID2);
}

float FaceTracker::DevComputePairwiseMatchScore(
