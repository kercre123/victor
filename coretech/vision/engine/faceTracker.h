...
class FaceTracker {
 public:
  FaceTracker(const Camera& camera, const std::string& modelPath,
  ...
  // CropFactor is the fraction of original image width to use for
  // detection/tracking
  Result Update(const Vision::Image& frameOrig, const float cropFactor,
  ...
  // Will return false if the private implementation does not support face
  // recognition
  static bool IsRecognitionSupported();

  // Enable/Disable automatic recognition/enrollment of faces that are detected
  // (i.e. if disabled, just detect and track faces)
  void EnableRecognition(bool enable);
  bool IsRecognitionEnabled() const;

  // returns the minimum distance between eyes a face has to have in order to be
  // enrollable
  ...
  // if mergeWithID=UnknownFaceID, will fail if there is not enough room for a
  // new named face
  ...
  Result DevAddFaceToAlbum(const Image& img, const TrackedFace& face,
                           int albumEntry);
  Result DevFindFaceInAlbum(const Image& img, const TrackedFace& face,
  ...
  float DevComputePairwiseMatchScore(int faceID1, const Image& img2,
                                     const TrackedFace& face2) const;
#endif
