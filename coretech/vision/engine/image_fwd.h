...
class ImageBase : public Array2d<T> {
 public:
  ...
  ImageBase(s32 nrows, s32 ncols, const T& pixel)
      : Array2d<T>(nrows, ncols, pixel) {}
  ...
  // Write to a file, format determined by extension (quality is only used for
  // JPEG)
  ...
  void Resize(ImageBase<T>& resizedImage,
              ResizeMethod method = ResizeMethod::Linear) const;

  // Resize in place to a specific size, keeps the aspect ratio
  // If onlyReduceSize=true, does not resize if new size is larger than
  // requested size
  void ResizeKeepAspectRatio(s32 desiredRows, s32 desiredCols,
                             ResizeMethod method = ResizeMethod::Linear,
                             bool onlyReduceSize = false);

  // Resize into the new image (which is already the desired size), keeps the
  // aspect ratio
  void ResizeKeepAspectRatio(ImageBase<T>& resizedImage,
                             ResizeMethod method = ResizeMethod::Linear) const;
  ...
  void DrawLine(const Point2f& start, const Point2f& end,
                const ColorRGBA& color, const s32 thickness = 1);

  // Draw circle at center <X,Y> with given radius and line thickness
  void DrawCircle(const Point2f& center, const ColorRGBA& color,
                  const s32 radius, const s32 thickness = 1);

  // Draw filled circle at center <X,Y> with given radius
  void DrawFilledCircle(const Point2f& center, const ColorRGBA& color,
                        const s32 radius);

  // Draw rectangle from top left <X,Y> to bottom right <X+width,Y+height>
  void DrawRect(const Rectangle<f32>& rect, const ColorRGBA& color,
                const s32 thickness = 1);
  void DrawRect(const Rectangle<s32>& rect, const ColorRGBA& color,
                const s32 thickness = 1);

  // Draw filled rectangle from top left <X,Y> to bottom right
  // <X+width,Y+height>
  ...
  void DrawQuad(const Quad2f& quad, const ColorRGBA& color,
                const s32 thickness = 1);

  // Draw generic, unfilled polygon (note, floating point polylines are rounded)
  void DrawPoly(const Poly2f& poly, const ColorRGBA& color,
                const s32 thickness = 1, const bool closed = true);
  void DrawPoly(const Poly2i& poly, const ColorRGBA& color,
                const s32 thickness = 1, const bool closed = true);

  // Draw a filled convex polygon defined by a list of points
  void DrawFilledConvexPolygon(const std::vector<Point2i> points,
                               const ColorRGBA& color);

  // TODO: Expose font?
  void DrawText(const Point2f& position, const std::string& str,
                const ColorRGBA& color, f32 scale = 1.f,
                bool dropShadow = false, int thickness = 1,
                bool centered = false);
  ...
  // imageWidth
  static void MakeTextFillImageWidth(const std::string& text, int font,
  ...
  // fill the gaps
  void DrawTextCenteredHorizontally(const std::string& text, int font,
                                    float scale, int thickness,
                                    const ColorRGBA& color, int verticalPos,
  ...
  Rectangle<s32> GetBoundingRect() {
    return Rectangle<s32>(0, 0, GetNumCols(), GetNumRows());
  }
  ...
  // Remove radial distortion. If the calibration is for a different resolution
  // image, it will be scaled to match this image's resolution before
  // undistorting. NOTE: Cannot be done in place (i.e. do not do
  // img.Undistort(calib, img)), will trigger assertion. NOTE: If calling
  // repeatedly, consider using an Vision::Undistorter object, which uses
  // caching for better performance.
  void Undistort(const CameraCalibration& calib,
 ...
 protected:
  ...
  // If drawBlankPixels is set to false the draw will require slightly more
  // computation but no pixels with empty value will be copied allowing
  // "transparency" in undrawn areas
  template <typename DerivedType>
...
class Image : public ImageBase<u8> {
 public:
  ...
  // in ImageRGB565
  u8* GetRawDataPointer() { return ImageBase<u8>::GetDataPointer(); }
  ...
  void DrawSubImage(const Image& subImage, const Point2f& topLeftCorner,
                    bool drawBlankPixels = true) {
    return ImageBase<u8>::DrawSubImage<Image>(subImage, topLeftCorner, 0,
                                              drawBlankPixels);
  }
  Image GetROI(Rectangle<s32>& roiRect) {
    return ImageBase<u8>::GetROI<Image>(roiRect);
  }
  const Image GetROI(Rectangle<s32>& roiRect) const {
    return ImageBase<u8>::GetROI<Image>(roiRect);
  }
...
class ImageRGB : public ImageBase<PixelRGB> {
 public:
  ...
  ImageRGB(const ImageBase<PixelRGB>& imageBase)
      : ImageBase<PixelRGB>(imageBase) {}
  ...
  void DrawSubImage(const ImageRGB& subImage, const Point2f& topLeftCorner,
                    bool drawBlankPixels = true) {
    return ImageBase<PixelRGB>::DrawSubImage<ImageRGB>(
        subImage, topLeftCorner, Vision::PixelRGB(), drawBlankPixels);
  }
  ImageRGB GetROI(Rectangle<s32>& roiRect) {
    return ImageBase<PixelRGB>::GetROI<ImageRGB>(roiRect);
  }
  const ImageRGB GetROI(Rectangle<s32>& roiRect) const {
    return ImageBase<PixelRGB>::GetROI<ImageRGB>(roiRect);
  }
  ...
  Image ToGray(const RGBToGrayMethod = RGBToGrayMethod::GreenChannel)
      const;  // Return a new gray image from this RGB one
  void FillGray(Image& grayOut,
                const RGBToGrayMethod = RGBToGrayMethod::GreenChannel)
      const;  // Fill existing gray image from this RGB image

  ImageRGB& SetFromGray(
      const Image& grayIn);  // Set this RGB image from given gray image
  ImageRGB& SetFromRGB565(
      const ImageRGB565& rgb565);  // Set from given RGB565 image

  // Sets all pixels > value to 255 and all values <= value to 0.
  // If anyChannel=true, then any channel being above the value suffices.
  // Otherwise, all channels must be.
  Image Threshold(u8 value, bool anyChannel) const;

  virtual s32 GetNumChannels() const override { return 3; }

  // Divide each channel by the sum across channels. Multiply result by 255 to
  // keep in 8bit range. Optionally, you can provide a working array to avoid
  // allocation (will allocate to [NumRows x NumCols x 1])
  ImageRGB& NormalizeColor(Array2d<s32>* workingArray = nullptr);  // in place
  void GetNormalizedColor(ImageRGB& imgNorm,
                          Array2d<s32>* workingArray = nullptr) const;
 ...
 protected:
  Image ToGrayFromDoubleGreen() const;  // Return a new gray image from the
                                        // green channel of this RBG image
  void FillGrayFromDoubleGreen(
      Image& grayImage) const;  // Fill existing gray image from the green
                                // channel of this RGB image
  Image ToGrayFromGreenChannel() const;  // Return a new gray image from the
                                         // green channel of this RBG image
  void FillGrayFromGreenChannel(
      Image& grayImage) const;  // Fill existing gray image from the green
                                // channel of this RGB image
...
class ImageRGB565 : public ImageBase<PixelRGB565> {
 public:
  ...
  explicit ImageRGB565(const ImageBase<PixelRGB565>& imageBase)
      : ImageBase<PixelRGB565>(imageBase) {}
  ImageRGB565(const Array2d<PixelRGB565>& array2d)
      : ImageBase<PixelRGB565>(array2d) {}
  ...
  void DrawSubImage(const ImageRGB565& subImage, const Point2f& topLeftCorner,
                    bool drawBlankPixels = true) {
    return ImageBase<PixelRGB565>::DrawSubImage<ImageRGB565>(
        subImage, topLeftCorner, Vision::PixelRGB565(), drawBlankPixels);
  }
  ImageRGB565 GetROI(Rectangle<s32>& roiRect) {
    return ImageBase<PixelRGB565>::GetROI<ImageRGB565>(roiRect);
  }
  const ImageRGB565 GetROI(Rectangle<s32>& roiRect) const {
    return ImageBase<PixelRGB565>::GetROI<ImageRGB565>(roiRect);
  }

  const u16* GetRawDataPointer() const {
    DEV_ASSERT(IsContinuous(), "ImageRGB565.GetRawDataPointer.NotContinuous");
    static_assert(sizeof(PixelRGB565) == sizeof(u16),
                  "Unexpected size for PixelRGB565");
...
class ImageRGBA : public ImageBase<PixelRGBA> {
 public:
  ...
  ImageRGBA(const ImageBase<PixelRGBA>& imageBase)
      : ImageBase<PixelRGBA>(imageBase) {}
  ...
  ImageRGBA(const Array2d<PixelRGBA>& array2d)
      : ImageBase<PixelRGBA>(array2d) {}

  ImageRGBA(cv::Mat& cvMat);

  Image ToGray() const;
  void FillGray(Image& grayOut) const;
  ImageRGBA& SetFromGray(
      const Image& imageGray);  // Set this RGBA image from given gray image
  ImageRGBA& SetFromRGB565(
      const ImageRGB565& rgb565,
      const u8 alpha = 255);  // Set from given RGB565 image

  void DrawSubImage(const ImageRGBA& subImage, const Point2f& topLeftCorner,
                    bool drawBlankPixels = true) {
    return ImageBase<PixelRGBA>::DrawSubImage<ImageRGBA>(
        subImage, topLeftCorner, Vision::PixelRGBA(0, 0, 0, 255),
        drawBlankPixels);
  }
...
//
template <typename T>
inline ImageBase<T>::ImageBase(s32 nrows, s32 ncols, const ColorRGBA& color)
    : Array2d<T>(nrows, ncols) {
  // We can't set elements of Array2d<T> directly because OpenCV does not expose
  // an operator to convert from ColorRGBA or cvScalar to arbitrary pixel type
  // <T>.  Workaround is to draw a filled rectangle that covers the entire
  // image.
