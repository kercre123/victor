...
extern "C" {
// These two functions must exist to avoid linker errors but until we need to
// use the timeout functionality of the OMCV Pet Detector library, they can just
// be dummy functions See also page 30 of
// OMCVPetDetectionV2.0SoftwareSpecification_E.pdf
UINT32 OkaoExtraInitTime(void) { return 0; }

UINT32 OkaoExtraGetTime(UINT32 unInitTime) { return 0; }
}
#endif

// If < 0, use value from JSON config. Otherwise use this one (for dev
// adjustment live)
CONSOLE_VAR_RANGED(s32, kRuntimePetDetectionThreshold, "Vision.PetDetection",
                   -1, -1, 1000);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
PetTracker::PetTracker() : _handles(new Handles()) {
...
Result PetTracker::Init(const Json::Value& config) {
  ...
  if (!config.isMember(JsonKey::PetTracker)) {
    PRINT_NAMED_ERROR("PetTracker.Init.MissingConfigGroup", "Expecting '%s'",
                      JsonKey::PetTracker);
  ...
  std::map<const char*, s32> parameters{
      {JsonKey::MaxPets, -1},        {JsonKey::MinFaceSize, -1},
      {JsonKey::MaxFaceSize, -1},    {JsonKey::InitialSearchCycle, -1},
      {JsonKey::NewSearchCycle, -1}, {JsonKey::DetectionThreshold, -1},
      {JsonKey::TrackLostCount, -1}, {JsonKey::TrackSteadiness, -1},
  };
  ...
  _handles->omcvPetDetector =
      OMCV_PD_CreateHandle(DETECTION_MODE_MOVIE, parameters[JsonKey::MaxPets]);
  ...
  const UINT32 searchAngles =
      ROLL_ANGLE_U45;  // (ROLL_ANGLE_U45 | ROLL_ANGLE_2 | ROLL_ANGLE_10);
  omcvResult =
      OMCV_PD_SetAngle(_handles->omcvPetDetector, POSE_YAW_FRONT, searchAngles);
  if (OMCV_NORMAL != omcvResult) {
    PRINT_NAMED_ERROR("PetTracker.Init.OmcvSetAngleFailed", "OMCV Result=%d",
                      omcvResult);
    return RESULT_FAIL_INVALID_PARAMETER;
  }

  omcvResult = OMCV_PD_MV_SetSearchCycle(
      _handles->omcvPetDetector, parameters[JsonKey::InitialSearchCycle],
      parameters[JsonKey::NewSearchCycle]);
  if (OMCV_NORMAL != omcvResult) {
    PRINT_NAMED_ERROR("PetTracker.Init.OmcvSetSearchCycleFailed",
                      "OMCV Result=%d", omcvResult);
    return RESULT_FAIL_INVALID_PARAMETER;
  }
  ...
  if (OMCV_NORMAL != omcvResult) {
    PRINT_NAMED_ERROR("PetTracker.Init.OmcvSetSizeRangeFailed",
                      "OMCV Result=%d", omcvResult);
    return RESULT_FAIL_INVALID_PARAMETER;
  }

  omcvResult = OMCV_PD_SetThreshold(_handles->omcvPetDetector,
                                    parameters[JsonKey::DetectionThreshold]);
  if (OMCV_NORMAL != omcvResult) {
    PRINT_NAMED_ERROR("PetTracker.Init.FaceLibSetThresholdFailed",
                      "OMCV Result=%d", omcvResult);
    return RESULT_FAIL_INVALID_PARAMETER;
  }
  ...
  omcvResult = OMCV_PD_MV_SetLostParam(_handles->omcvPetDetector,
                                       ...
                                       parameters[JsonKey::TrackLostCount]);
  if (OMCV_NORMAL != omcvResult) {
    PRINT_NAMED_ERROR("PetTracker.Init.FaceLibSetLostParamFailed",
                      "OMCV Result=%d", omcvResult);
    return RESULT_FAIL_INVALID_PARAMETER;
  }

  // NOTE: using same steadiness parameter for position and size
  omcvResult = OMCV_PD_MV_SetSteadinessParam(
      _handles->omcvPetDetector, parameters[JsonKey::TrackSteadiness],
      parameters[JsonKey::TrackSteadiness]);
  if (OMCV_NORMAL != omcvResult) {
    PRINT_NAMED_ERROR("PetTracker.Init.FaceLibSetSteadinessFailed",
                      "OMCV Result=%d", omcvResult);
    return RESULT_FAIL_INVALID_PARAMETER;
  }
...
PetTracker::~PetTracker() {
  if (NULL != _handles->omcvDetectionResult) {
    if (OMCV_NORMAL !=
        OMCV_PD_DeleteResultHandle(_handles->omcvDetectionResult)) {
...
Result PetTracker::Update(const Vision::Image& frameOrig,
  ...
  if (kRuntimePetDetectionThreshold >= 0 &&
    ...
    INT32 omcvResult = OMCV_PD_SetThreshold(_handles->omcvPetDetector,
                                            _runtimeDetectionThreshold);
    if (OMCV_NORMAL != omcvResult) {
      PRINT_NAMED_ERROR("PetTracker.Update.FaceLibSetThresholdFailed",
                        "OMCV Result=%d", omcvResult);
      return RESULT_FAIL_INVALID_PARAMETER;
    }
  }
  ...
  omcvResult = OMCV_PD_Detect(_handles->omcvPetDetector, dataPtr, nWidth,
                              nHeight, _handles->omcvDetectionResult);
  if (OMCV_NORMAL != omcvResult) {
    PRINT_NAMED_WARNING(
        "PetTracker.Update.OmcvDetectFail",
        "OMCV Result Code=%d, dataPtr=%p, nWidth=%d, nHeight=%d", omcvResult,
        dataPtr, nWidth, nHeight);
    return RESULT_FAIL;
  }

  INT32 numDetections = 0;
  omcvResult =
      OMCV_PD_GetResultCount(_handles->omcvDetectionResult,
                             OBJ_TYPE_DOG | OBJ_TYPE_CAT, &numDetections);
  if (OMCV_NORMAL != omcvResult) {
  ...
  for (INT32 detectionIndex = 0; detectionIndex < numDetections;
    ...
    omcvResult = OMCV_PD_GetResultInfo(_handles->omcvDetectionResult,
                                       (OBJ_TYPE_DOG | OBJ_TYPE_CAT),
    ...
    switch (detectionInfo.nObjectType) {
      ...
      default:
        PRINT_NAMED_WARNING("PetTracker.Update.InvalidPetType", "ObjectType=%d",
                            detectionInfo.nObjectType);
        continue;
    }  // switch(detectiONInfo.nObjectType
    ...
    pet.SetRect(
        Rectangle<f32>(detectionInfo.ptCenter.x - detectionInfo.nWidth / 2,
    ...
    pet.SetIsBeingTracked(detectionInfo.nDetectionMethod ==
                          DET_METHOD_TRACKED_MIDDLE);
