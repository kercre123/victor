    ...
    for (s32 col = 0; col < cols; col += stepCol) {
      ...
      if (isBright) {
        ...
        cv::Scalar_<double> color =
            cv::mean(inputImage.GetROI(roi).get_CvMat_());
        ...
        poly.emplace_back(Util::Clamp(x, 0.f, 1.f), Util::Clamp(y, 0.f, 1.f));
        ...
        salientPoints.emplace_back(timestamp, x + kWidthStep2, y + kHeightStep2,
                                   score, (kWidthScale * kHeightScale), type,
                                   typeString, poly, rgba);
      }
    }
  }
  return RESULT_OK;
...
float BrightColorDetector::GetScore(const ImageRGB& image) {
  ...
  meanRoot =
      sqrt((meanRG.val[0] * meanRG.val[0]) + (meanYB.val[0] * meanYB.val[0]));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BrightColorDetector::Parameters::Parameters() : isInitialized(false) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BrightColorDetector::Parameters::Initialize() { isInitialized = true; }
