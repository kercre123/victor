...
class ObservedMarker : public Marker {
 public:
  ...
  ObservedMarker(const TimeStamp_t t, const Code& type, const Quad2f& corners,
                 const Camera& seenBy, UserHandle userHandle = UnknownHandle);
...
class KnownMarker : public Marker {
 public:
  ...
  // tolerance.
  bool IsVisibleFrom(const Camera& camera, const f32 maxAngleRad,
                     const f32 minImageSize, const bool requireSomethingBehind,
...
inline Quad3f const& KnownMarker::Get3dCorners() const { return _corners3d; }

inline Pose3d const& KnownMarker::GetPose() const { return _pose; }

inline Marker::Code const& Marker::GetCode() const { return _code; }

inline Point2f const& KnownMarker::GetSize() const { return _size; }

inline Camera const& ObservedMarker::GetSeenBy() const { return _seenByCam; }
...
inline bool KnownMarker::IsVisibleFrom(const Camera& camera,
  ...
  return IsVisibleFrom(camera, maxAngleRad, minImageSize,
                       requireSomethingBehind, xBorderPad, yBorderPad, dummy);
}
