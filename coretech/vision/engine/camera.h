...
class Camera {
 public:
  ...
  const std::shared_ptr<CameraCalibration> GetCalibration()
      const;  // nullptr if not calibrated
  ...
  // Returns true if the calibration actually changed, false if this was the
  // same calibration as before. NOTE: Once a calibration is set, calling
  // SetCalibration(nullptr) will generate an error, return false,
  //       and not "un-set" it.
  ...
  Result ComputeObjectPose(const Quad2f& imgPoints, const Quad3f& objPoints,
  ...
  void Project3dPoints(const Quad3f& objPoints, Quad2f& imgPoints) const;
  ...
  bool IsAnythingBehind(const Point2f& projectedPoint,
                        const f32 atDistance) const;
  bool IsAnythingBehind(const Quad2f& projectedQuad,
                        const f32 atDistance) const;
  ...
  // center of the image.
  void ComputePanAndTiltAngles(const Point2f& imgPoint, Radians& relPanAngle,
                               Radians& relTiltAngle) const;
 ...
 protected:
  ...
  Pose3d ComputeObjectPoseHelper(
      const std::vector<cv::Point2f>& cvImagePoints,
...
// Inline accessors:
inline const CameraID_t Camera::GetID(void) const { return _camID; }

inline const Pose3d& Camera::GetPose(void) const { return _pose; }

inline std::shared_ptr<CameraCalibration> Camera::GetCalibration(void) {
  return _calibration;
}

inline const std::shared_ptr<CameraCalibration> Camera::GetCalibration(
    void) const {
  return _calibration;
}

inline void Camera::SetID(const CameraID_t ID) { _camID = ID; }

inline void Camera::SetPose(const Pose3d& newPose) { _pose = newPose; }

inline bool Camera::IsCalibrated() const { return (_calibration != nullptr); }

inline bool Camera::IsOccluded(const Point2f& projectedPoint,
                               const f32 atDistance) const {
  return _occluderList.IsOccluded(projectedPoint, atDistance);
}

inline bool Camera::IsAnythingBehind(const Point2f& projectedPoint,
                                     const f32 atDistance) const {
  return _occluderList.IsAnythingBehind(projectedPoint, atDistance);
}

inline bool Camera::IsAnythingBehind(const Quad2f& projectedQuad,
                                     const f32 atDistance) const {
...
inline void Camera::GetAllOccluders(
    std::vector<Rectangle<f32> >& occluders) const {
