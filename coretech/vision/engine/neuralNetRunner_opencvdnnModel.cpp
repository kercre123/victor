 ...
 * Date:   6/29/2017
 *
 * Description: Implementation of NeuralNetRunner Model class which wrapps
 *OpenCV's DNN module.
 *
...
// Useful just for printing every frame, since detection is slow, even through
// the profiler already has settings for printing based on time. Set to 0 to
// disable.
CONSOLE_VAR(s32, kNeuralNetRunner_PrintTimingFrequency,
            "Vision.NeuralNetRunner", 1);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result NeuralNetRunner::Model::LoadModel(const std::string& modelPath,
                                         const Json::Value& config) {
#define GetFromConfig(keyName)                                                \
  if (false ==                                                                \
      JsonTools::GetValueOptional(config, QUOTE(keyName), _params.keyName)) { \
  ...
  if (!ANKI_VERIFY(Util::IsFltGEZero(_params.min_score) &&
                       Util::IsFltLE(_params.min_score, 1.f),
  ...
  if (_params.graph.substr(_params.graph.size() - 3, 3) == ".pb") {
    ...
    const std::string graphFileName =
        Util::FileUtils::FullFilePath({modelPath, _params.graph});
    const std::string graphTxtFileName =
        Util::FileUtils::FullFilePath({modelPath, _params.graph + "txt"});
    if (Util::FileUtils::FileExists(graphFileName)) {
      try {
        if (Util::FileUtils::FileExists(graphTxtFileName)) {
          _network =
              cv::dnn::readNetFromTensorflow(graphFileName, graphTxtFileName);
        } else {
          _network = cv::dnn::readNetFromTensorflow(graphFileName);
        }
      } catch (const cv::Exception& e) {
        PRINT_NAMED_ERROR(
            "NeuralNetRunner.Model.LoadModel.ReadTensorflowModelCvException",
            "Model file: %s, Error: %s", graphFileName.c_str(), e.what());
        return RESULT_FAIL;
      } catch (...) {
        PRINT_NAMED_ERROR(
            "NeuralNetRunner.Model.LoadModel."
            "ReadTensorflowModelUnknownException",
      ...
      PRINT_NAMED_ERROR(
          "NeuralNetRunner.Model.LoadModel.TensorflowModelFileNotFound",
          "Model file: %s", graphFileName.c_str());
      return RESULT_FAIL;
    }
  } else {
    ...
    const std::string protoFileName =
        Util::FileUtils::FullFilePath({modelPath, _params.graph + ".prototxt"});
    const std::string modelFileName = Util::FileUtils::FullFilePath(
        {modelPath, _params.graph + ".caffemodel"});
    if (Util::FileUtils::FileExists(protoFileName) &&
        Util::FileUtils::FileExists(modelFileName)) {
      try {
        _network = cv::dnn::readNetFromCaffe(protoFileName, modelFileName);
      } catch (const cv::Exception& e) {
        PRINT_NAMED_ERROR(
            "NeuralNetRunner.Model.LoadModel.ReadCaffeModelCvException",
            "Model file: %s, Error: %s", _params.graph.c_str(), e.what());
        return RESULT_FAIL;
      } catch (...) {
        PRINT_NAMED_ERROR(
            "NeuralNetRunner.Model.LoadModel.ReadCaffeModelUnknownException",
            "Model file: %s", _params.graph.c_str());
        return RESULT_FAIL;
      }
    } else {
      PRINT_NAMED_ERROR("NeuralNetRunner.Model.LoadModel.CaffeFilesNotFound",
                        "Reading %s/%s.{prototxt|caffemodel}",
                        modelPath.c_str(), _params.graph.c_str());
      return RESULT_FAIL;
    }
  }
  ...
  {
    const cv::dnn::MatShape shape = {1, 3, _params.input_height,
                                     _params.input_width};
    ...
    PRINT_CH_INFO(
        kLogChannelName, "NeuralNetRunner.Model.LoadModel.NetworkFLOPS",
        "Input %dx%d: %.3f %sFLOPS", _params.input_width, _params.input_height,
...
Result NeuralNetRunner::Model::ReadLabelsFile(const std::string& fileName) {
  ...
  if (!file) {
    PRINT_NAMED_ERROR("NeuralNetRunner.ReadLabelsFile.LabelsFileNotFound", "%s",
                      fileName.c_str());
    return RESULT_FAIL;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result NeuralNetRunner::Model::Run(const ImageRGB& img,
  ...
  const cv::Scalar mean(_params.input_mean_R, _params.input_mean_G,
                        _params.input_mean_B);
  const bool kSwapRedBlue = false;  // Our image class is already RGB
  cv::Mat dnnBlob = cv::dnn::blobFromImage(img.get_CvMat_(), scale,
                                           processingSize, mean, kSwapRedBlue);
  ...
  if (_isDetectionMode) {
    const Array2d<float> detectionMat(detections.size[2], detections.size[3],
    ...
    for (int iDetection = 0; iDetection < numDetections; ++iDetection) {
      ...
      if (confidence >= _params.min_score) {
        ...
        const float xmin =
            std::max(0.f, detection[3] * (float)img.GetNumCols());
        const float ymin =
            std::max(0.f, detection[4] * (float)img.GetNumRows());
        const float xmax = std::min((float)img.GetNumCols(),
                                    detection[5] * (float)img.GetNumCols());
        const float ymax = std::min((float)img.GetNumRows(),
                                    detection[6] * (float)img.GetNumRows());

        DEV_ASSERT_MSG(Util::IsFltGT(xmax, xmin),
                       "NeuralNetRunner.Model.Run.InvalidDetectionBoxWidth",
                       "xmin=%f xmax=%f", xmin, xmax);
        DEV_ASSERT_MSG(Util::IsFltGT(ymax, ymin),
                       "NeuralNetRunner.Model.Run.InvalidDetectionBoxHeight",
                       "ymin=%f ymax=%f", ymin, ymax);

        const bool labelIndexOOB =
            (labelIndex < 0 || labelIndex > _labels.size());

        salientPoint.timestamp = img.GetTimestamp();
        salientPoint.x_img = (f32)(xmin + xmax) * 0.5f;
        salientPoint.y_img = (f32)(ymin + ymax) * 0.5f;
        salientPoint.area_fraction =
            (xmax - xmin) * (ymax - ymin) / (float)img.GetNumElements();
        salientPoint.score = confidence;
        salientPoint.description =
            (labelIndexOOB ? "UNKNOWN" : _labels.at((size_t)labelIndex));

        // TODO: fill in shape
        // object.shape         = {{xmin,ymin}, {xmax,ymin}, {xmax,ymax},
        // {xmin,ymax}};
  ...
  } else  // classification mode
  {
    ...
    // NOTE: this is more general / safer than using cols. (Some architectures
    // seem to output a 4D tensor
    //       and not set cols)
    const int numDetections = detections.size[1];

    DEV_ASSERT_MSG(numDetections == _labels.size(),
                   "NeuralNetRunner.Model.Run.UnexpectedResultSize",
                   "NumLabels:%zu, DNN returned %d values", _labels.size(),
                   numDetections);
    ...
    if (maxLabelIndex >= 0) {
      ...
      // TODO: object.shape =
      // object.rect        =
      // Rectangle<s32>(0,0,img.GetNumCols(),img.GetNumRows());
  ...
  if (wasSalientPointDetected) {
    PRINT_CH_DEBUG(
        kLogChannelName, "NeuralNetRunner.Model.Run.SalientPointDetected",
        "Name:%s Score:%.3f t:%ums", salientPoint.description.c_str(),
        salientPoint.score, salientPoint.timestamp);
