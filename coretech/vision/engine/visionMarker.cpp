...
Marker::Marker(const Code& withCode) : _code(withCode) {}
...
const char* Marker::GetNameForCode(Marker::Code code) {
  if (code >= 0 && code <= NUM_MARKER_TYPES) {
    ...
    PRINT_NAMED_ERROR("Marker.GetNameForCode",
                      "Could not look up name for code=%d", code);
    return "UNKNOWN";
  }
}

const char* Marker::GetCodeName() const { return GetNameForCode(_code); }

ObservedMarker::ObservedMarker()
    : Marker(MARKER_UNKNOWN), _userHandle(UnknownHandle) {}
...
KnownMarker::KnownMarker(const Code& withCode, const Pose3d& atPose,
                         const Point2f& size_mm)
    : Marker(withCode), _size(size_mm), _lastObservedTime(0) {
  SetPose(atPose);
}
...
bool KnownMarker::IsVisibleFrom(const Camera& camera, const f32 maxAngleRad,
                                const f32 minImageSize,
                                const bool requireSomethingBehind,
                                const u16 xBorderPad, const u16 yBorderPad,
  ...
  if (_pose.GetWithRespectTo(camera.GetPose(), markerPoseWrtCamera) == false) {
    PRINT_NAMED_WARNING("KnownMarker.IsVisibleFrom.NotInCameraPoseTree",
                        "Marker must be in the same pose tree as the camera to "
  ...
  const f32 dotProd = DotProduct(
      faceNormal, Z_AXIS_3D());  // TODO: Optimize to just: faceNormal.z()?
  if (dotProd > 0.f ||
      acos(-dotProd) >
          maxAngleRad) {  // TODO: Optimize to simple dotProd comparison
  ...
  if (not camera.IsWithinFieldOfView(imgCorners[TopLeft], xBorderPad,
                                     yBorderPad) ||
      not camera.IsWithinFieldOfView(imgCorners[TopRight], xBorderPad,
                                     yBorderPad) ||
      not camera.IsWithinFieldOfView(imgCorners[BottomLeft], xBorderPad,
                                     yBorderPad) ||
      not camera.IsWithinFieldOfView(imgCorners[BottomRight], xBorderPad,
                                     yBorderPad)) {
  ...
  // NOTE: occluder list uses closest point as the distance to the quad, so use
  // that here as well
...
Result KnownMarker::EstimateObservedPose(const ObservedMarker& obsMarker,
  ...
  for (s32 i = 0; i < NUM_AVERAGE_POSES; ++i) {
    ...
    Result result =
        camera.ComputeObjectPose(perturbedCorners, _corners3d, pose);
    if (RESULT_OK != result) {
      return result;
    }
...
const char* NotVisibleReasonToString(KnownMarker::NotVisibleReason reason) {
  switch (reason) {
    case KnownMarker::NotVisibleReason::IS_VISIBLE:
      return "IS_VISIBLE";
    case KnownMarker::NotVisibleReason::CAMERA_NOT_CALIBRATED:
      return "CAMERA_NOT_CALIBRATED";
    case KnownMarker::NotVisibleReason::POSE_PROBLEM:
      return "POSE_PROBLEM";
    case KnownMarker::NotVisibleReason::BEHIND_CAMERA:
      return "BEHIND_CAMERA";
    case KnownMarker::NotVisibleReason::NORMAL_NOT_ALIGNED:
      return "NORMAL_NOT_ALIGNED";
    case KnownMarker::NotVisibleReason::TOO_SMALL:
      return "TOO_SMALL";
    case KnownMarker::NotVisibleReason::OUTSIDE_FOV:
      return "OUTSIDE_FOV";
    case KnownMarker::NotVisibleReason::OCCLUDED:
      return "OCCLUDED";
    case KnownMarker::NotVisibleReason::NOTHING_BEHIND:
