 ...
 * Date:   2/18/2016
 *
 * Description: Container for holding bookkeeping data for faces enrolled for
 *face recognition. This class is used heavily by the FaceRecognizer and makes
 *use of the EnrolledFaceStorage CLAD struct to serialize/deserialize for
 *storing in the robot's NVStorage.
 *
 *
...
inline static Json::LargestInt TimeToInt64(EnrolledFaceEntry::Time time) {
  ...
  auto jsonTime = Util::numeric_cast<Json::LargestInt>(
      duration_cast<seconds>(time.time_since_epoch()).count());
  return jsonTime;
}

UserName::UserName(const std::string& name)
    : _name(Util::StringToLower(name)), _displayName(name) {}
...
EnrolledFaceEntry::EnrolledFaceEntry(FaceID_t withID, Json::Value& json)
  ...
  if (!json.isMember(JsonKey::albumEntries)) {
    ...
    if (!jsonAlbumEntries.isArray()) {
      PRINT_NAMED_WARNING(
          "EnrolledFaceEntry.ConstructFromJson.AlbumEntriesNotArray", "");
    } else {
      bool sessionOnlySet = false;
      for (auto const& jsonAlbumEntry : jsonAlbumEntries) {
        if (!jsonAlbumEntry.isMember(JsonKey::albumEntry) ||
            !jsonAlbumEntry.isMember(JsonKey::lastSeenTime)) {
          PRINT_NAMED_WARNING(
              "EnrolledFaceEntry.ConstructFromJson.BadAlbumEntries", "");
          _albumEntrySeenTimes.clear();
          break;
        }

        const AlbumEntryID_t entry =
            jsonAlbumEntry[JsonKey::albumEntry].asInt();
...
EnrolledFaceEntry::EnrolledFaceEntry(const EnrolledFaceStorage& message)
    : _faceID(message.faceID),
      ...
      _lastDataUpdateTime(
          std::chrono::seconds(message.lastDataUpdateTimeCount)) {
  if (message.albumEntries.size() != message.albumEntryUpdateTimes.size()) {
    PRINT_NAMED_WARNING(
        "EnrolledFaceEntry.Constructor.FromEnrolledFaceStorage",
    ...
    for (s32 iEntry = 0; iEntry < message.albumEntries.size(); ++iEntry) {
      ...
      if (albumEntry != UnknownAlbumEntryID) {
        const Time seenTime =
            Time(std::chrono::seconds(message.albumEntryUpdateTimes[iEntry]));
...
void EnrolledFaceEntry::FillJson(Json::Value& entry) const {
  ...
  for (auto& albumEntryPair : _albumEntrySeenTimes) {
    ...
    if (albumEntryPair.first != _sessionOnlyAlbumEntry) {
      ...
      jsonAlbumEntry[JsonKey::lastSeenTime] =
          TimeToInt64(albumEntryPair.second);
      entry[JsonKey::albumEntries].append(jsonAlbumEntry);
    }
  }

  // NOTE: Not storing faceID because we're assuming it's being used as the key
...
EnrolledFaceStorage EnrolledFaceEntry::ConvertToEnrolledFaceStorage() const {
  ...
  message.enrollmentTimeCount =
      duration_cast<seconds>(_enrollmentTime.time_since_epoch()).count();
  message.lastDataUpdateTimeCount =
      duration_cast<seconds>(_lastDataUpdateTime.time_since_epoch()).count();
  ...
  message.albumEntryUpdateTimes.push_back(
      TimeToInt64(GetSessionOnlySeenTime()));
...
void EnrolledFaceEntry::SetDebugMatchingInfo(
...
std::string EnrolledFaceEntry::GetAlbumEntriesString() const {
  ...
  if (_albumEntrySeenTimes.empty()) {
    ...
    for (auto iter = _albumEntrySeenTimes.begin();
         iter != _albumEntrySeenTimes.end(); ++iter) {
...
void EnrolledFaceEntry::Serialize(std::vector<u8>& buffer) const {
  ...
  const size_t numBytes = message.Pack(
      &(buffer[buffer.size() - expectedNumBytes]), expectedNumBytes);
  ...
  PRINT_CH_DEBUG("FaceRecognizer", "EnrolledFaceEntry.Serialize.Success",
                 "Serialized entry for '%s', ID=%d. Added %zu bytes to buffer "
                 "(total length now %zu)",
                 Anki::Util::HidePersonallyIdentifiableInfo(_name.c_str()),
                 _faceID, numBytes, buffer.size());

}  // Serialize()

Result EnrolledFaceEntry::Deserialize(const std::vector<u8>& buffer,
                                      size_t& startIndex) {
  if (buffer.empty()) {
  ...
  if (startIndex + expectedNumBytes > buffer.size()) {
    PRINT_NAMED_WARNING(
        "EnrolledFaceEntry.Deserialize.BufferTooShort",
  ...
  const size_t numBytes =
      message.Unpack(&(buffer[startIndex]), buffer.size() - startIndex);
  if (numBytes <
      expectedNumBytes) {  // Note: numBytes can be > expectedNumBytes b/c of
                           // variable name length
    PRINT_NAMED_WARNING("EnrolledFaceEntry.Deserialize.WrongNumBytesUnPacked",
                        "Unpacked %zu from buffer. Expecting >= %zu.", numBytes,
                        expectedNumBytes);
    return RESULT_FAIL;
  }

  // Now construct this object from the unpacked message and increment the
  // startIndex for the next call
...
void EnrolledFaceEntry::AddOrUpdateAlbumEntry(AlbumEntryID_t entry,
                                              Time updateTime,
                                              bool isSessionOnly) {
  if (isSessionOnly) {
    _sessionOnlyAlbumEntry = entry;
...
void EnrolledFaceEntry::SetAlbumEntryLastSeenTime(AlbumEntryID_t entry,
                                                  Time time) {
...
Result EnrolledFaceEntry::MergeWith(
    const EnrolledFaceEntry& other, s32 maxAlbumEntriesToKeep,
    std::vector<AlbumEntryID_t>& entriesRemoved) {
  Result helperResult =
      MergeAlbumEntriesHelper(other, maxAlbumEntriesToKeep, entriesRemoved);
  ...
  _lastDataUpdateTime =
      std::max(_lastDataUpdateTime, other._lastDataUpdateTime);

  // Keep oldest enrollment time
  _enrollmentTime = std::min(_enrollmentTime, other._enrollmentTime);

  if (ANKI_DEVELOPER_CODE) {
    // Sanity check: neither "this" nor "other" should have times in the future,
    // so merged times also shouldn't
    const bool clippingOccurred = ClipFutureTimesToNow("MergeWith");
    DEV_ASSERT(!clippingOccurred,
               "EnrolledFaceEntry.MergeWith.FutureTimesClipped");
  }
...
EnrolledFaceEntry::Time EnrolledFaceEntry::GetSessionOnlySeenTime() const {
  ...
  if (_sessionOnlyAlbumEntry != UnknownAlbumEntryID) {
    ...
    if (ANKI_VERIFY(iter != _albumEntrySeenTimes.end(),
                    "EnrolledFaceEntry.MergeAlbumEntriesHelper."
                    "MissingSessionOnlySeenTime",
                    "FaceID:%d SessionOnlyEntry:%d", GetFaceID(),
                    _sessionOnlyAlbumEntry)) {
...
Result EnrolledFaceEntry::MergeAlbumEntriesHelper(
    const EnrolledFaceEntry& other, s32 maxAlbumEntriesToKeep,
    std::vector<AlbumEntryID_t>& entriesRemoved) {
  const Time otherSessionOnlyTime = other.GetSessionOnlySeenTime();
  const Time thisSessionOnlyTime = this->GetSessionOnlySeenTime();
  const bool useOtherSessionOnlyEntry =
      (otherSessionOnlyTime > thisSessionOnlyTime);

  PRINT_CH_DEBUG(
      "FaceRecognizer",
      "EnrolledFaceEntry.MergeAlbumEntriesHelper.WhichSessionOnlyEntry",
      ...
      useOtherSessionOnlyEntry ? other._sessionOnlyAlbumEntry
                               : _sessionOnlyAlbumEntry);

  // NOTE: the -1's below are for session-only entries

  // First merge all non-session
  const size_t thisSizeWithoutSessionOnly =
      _albumEntrySeenTimes.size() -
      (_sessionOnlyAlbumEntry != UnknownAlbumEntryID ? 1 : 0);
  const size_t otherSizeWithoutSessionOnly =
      other._albumEntrySeenTimes.size() -
      (other._sessionOnlyAlbumEntry != UnknownAlbumEntryID ? 1 : 0);

  if (thisSizeWithoutSessionOnly + otherSizeWithoutSessionOnly <=
      maxAlbumEntriesToKeep - 1) {
    // Simple case: keep all entries

    PRINT_CH_DEBUG(
        "FaceRecognizer",
        "EnrolledFaceEntry.MergeAlbumEntriesHelper.KeepingBoth",
        "ThisEntry(ID:%d) has %zu entries, OtherEntry(ID:%d) has %zu", _faceID,
        _albumEntrySeenTimes.size(), other._faceID,
        other._albumEntrySeenTimes.size());
    ...
    for (auto& otherAlbumEntryPair : other._albumEntrySeenTimes) {
      if (otherAlbumEntryPair.first != other._sessionOnlyAlbumEntry) {
        _albumEntrySeenTimes[otherAlbumEntryPair.first] =
            otherAlbumEntryPair.second;
      }
    }
    ...
    // Keep the most recently seen entries, sorted from most recent to least
    // recent The bool here represents whether the entry came from "this"
    // EnrolledFaceEntry, so that we know (once sorted) if it needs to be added
    // to the removed list we return.
    struct SortingPair {
      AlbumEntryID_t albumEntry;
      bool isFromThis;
    };
    std::multimap<Time, SortingPair, std::greater<Time>>
        allManualEntriesSortedByTime;
    for (auto& entry : _albumEntrySeenTimes) {
      if (entry.first != _sessionOnlyAlbumEntry) {
        allManualEntriesSortedByTime.emplace(entry.second,
                                             SortingPair{entry.first, true});
      }
    }
    for (auto& entry : other._albumEntrySeenTimes) {
      if (entry.first != other._sessionOnlyAlbumEntry) {
        allManualEntriesSortedByTime.emplace(entry.second,
                                             SortingPair{entry.first, false});
      }
    }
    ...
    while (_albumEntrySeenTimes.size() < (maxAlbumEntriesToKeep - 1) &&
           entriesByTimeIter != allManualEntriesSortedByTime.end()) {
      // Add the entry to this
      PRINT_CH_DEBUG(
          "FaceRecognizer",
          "EnrolledFaceEntry.MergeAlbumEntriesHelper.KeepingFromOne",
      ...
      _albumEntrySeenTimes.emplace(entriesByTimeIter->second.albumEntry,
                                   entriesByTimeIter->first);

      ++entriesByTimeIter;
    }

    // If there's anything left in the list from "this" EnrolledFaceEntry's
    // albumEntries, put those in the removed list
    ...
    if (useOtherSessionOnlyEntry) {
      ...
      // Leave session only entry as it is, but re-insert its time because we
      // cleared above
  ...
  if (ANKI_DEVELOPER_CODE) {
    ...
    for (auto albumEntry : entriesRemoved) {
      DEV_ASSERT_MSG(
          _albumEntrySeenTimes.find(albumEntry) == _albumEntrySeenTimes.end(),
...
bool EnrolledFaceEntry::ClipFutureTimesToNow(const std::string& logString) {
  ...
  if (_enrollmentTime > currentTime) {
    PRINT_NAMED_WARNING(
        ("EnrolledFaceEntry." + logString + ".FutureEnrollmentTime").c_str(),
        "Clipping future enrollment time (%s) to now (%s)",
        GetTimeString(_enrollmentTime).c_str(),
        GetTimeString(currentTime).c_str());
  ...
  if (_lastDataUpdateTime > currentTime) {
    PRINT_NAMED_WARNING(
        ("EnrolledFaceEntry." + logString + ".FutureLastUpdatedTime").c_str(),
