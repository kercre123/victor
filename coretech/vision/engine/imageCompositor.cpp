...
ImageCompositor::ImageCompositor(const Json::Value& config)
    : _kPercentileForMaxIntensity(JsonTools::ParseFloat(
          config, kPercentileForMaxIntensityKey, debugName + ".Ctor")),
      _sumImage(0, 0),
...
void ImageCompositor::ComposeWith(const Vision::Image& img) {
  // We don't know what image dimensions we're compositing at the time of
  // construction. This is controlled by the scale factor applied to input
  // images prior to Marker Detection. Read the dimensions from the input image,
  // and determine the dimensions of the accumulator.
  ...
  if (!ANKI_VERIFY(
          _sumImage.GetNumRows() == img.GetNumRows() &&
              _sumImage.GetNumCols() == img.GetNumCols(),
...
void ImageCompositor::GetCompositeImage(Vision::Image& outImg) const {
  ...
  _sumImage.get_CvMat_().convertTo(outImg.get_CvMat_(), CV_8UC1, averageFactor,
                                   0);
  ...
  const u8 brightIntensityVal =
      hist.ComputePercentile(_kPercentileForMaxIntensity);
  const f32 scalingFactor =
      ((f32)std::numeric_limits<u8>::max()) / brightIntensityVal;
