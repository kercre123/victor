...
CONSOLE_VAR(bool, kVisionBenchmark_DisplayImages, CONSOLE_GROUP,
            false);  // Only works if running synchronously
CONSOLE_VAR(s32, kVisionBenchmark_ScaleMultiplier, CONSOLE_GROUP, 2);

// Toggle corresponding modes at runtime
#if REMOTE_CONSOLE_ENABLED
CONSOLE_VAR(std::underlying_type<Benchmark::Mode>::type,
            kVisionBenchmark_ToggleMode, CONSOLE_GROUP, 0);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Benchmark::Benchmark() : Profiler("Vision.Benchmark") {
  if (ANKI_DEV_CHEATS) {
...
Result Benchmark::Update(ImageCache& imageCache) {
  ...
  if (kVisionBenchmark_ToggleMode != 0) {
    ...
    PRINT_CH_INFO(kLogChannelName, "Benchmark.Update.TogglingMode",
                  "%s mode %hhu", (enable ? "Enabling" : "Disabling"),
                  whichMode);
...
#define RUN_IF_ENABLED(__method__)                        \
  ...
  const ImageCacheSize whichSize =
      ImageCache::GetSize(kVisionBenchmark_ScaleMultiplier);
  const ImageRGB& image = imageCache.GetRGB(whichSize);

  {
    auto ticToc =
        TicToc("TotalBenchmarkUpdate");  // excluding the GetRGB call (which
                                         // could include downsampling)
...
Result Benchmark::ScalarSubtraction(const ImageRGB& image) {
  ...
  if (kDoChecks && diffImage(1, 1) != checkValue) {
    PRINT_NAMED_ERROR("Benchmark.ScalarSubtraction.ForLoopFailed",
                      "Expected:(%d,%d,%d) Got:(%d,%d,%d)", checkValue.r(),
                      checkValue.g(), checkValue.b(), diffImage(1, 1).r(),
                      diffImage(1, 1).g(), diffImage(1, 1).b());
    return RESULT_FAIL;
  }
  ...
  diffImage.FillWith(0);
  {
    ...
    std::function<PixelRGB(const PixelRGB&)> diffFunc =
        [&scalar](const PixelRGB& p) {
          PixelRGB diff(p);
          diff -= scalar;
          return diff;
        };
  ...
  if (kDoChecks && diffImage(1, 1) != checkValue) {
    PRINT_NAMED_ERROR("Benchmark.ScalarSubtraction.ApplyScalarFuncFailed",
                      "Expected:(%d,%d,%d) Got:(%d,%d,%d)", checkValue.r(),
                      checkValue.g(), checkValue.b(), diffImage(1, 1).r(),
                      diffImage(1, 1).g(), diffImage(1, 1).b());
    return RESULT_FAIL;
  ...
  diffImage.FillWith(0);
  {
    ...
    cv::subtract(image.get_CvMat_(),
                 cv::Scalar(scalar.r(), scalar.g(), scalar.b()),
                 diffImage.get_CvMat_());
  }

  if (diffImage.GetNumRows() != image.GetNumRows() ||
      diffImage.GetNumCols() != image.GetNumCols()) {
    PRINT_NAMED_ERROR("Benchmark.ScalarSubtraction.BadResultSize",
                      "OpenCV subtraction yielded %dx%d image instead of %dx%d",
                      diffImage.GetNumCols(), diffImage.GetNumRows(),
                      image.GetNumCols(), image.GetNumRows());
    return RESULT_FAIL_INVALID_SIZE;
  }

  if (kDoChecks && diffImage(1, 1) != checkValue) {
    PRINT_NAMED_ERROR("Benchmark.ScalarSubtraction.OpenCVFailed",
                      "Expected:(%d,%d,%d) Got:(%d,%d,%d)", checkValue.r(),
                      checkValue.g(), checkValue.b(), diffImage(1, 1).r(),
                      diffImage(1, 1).g(), diffImage(1, 1).b());
    return RESULT_FAIL;
  ...
  if (_prevImage.GetNumRows() == image.GetNumRows() &&
      ...
      for (s32 i = 0; i < nrows; ++i) {
        ...
        for (s32 j = 0; j < ncols; ++j) {
          ...
          diffImage_i[j] = std::max(std::abs(pCrnt.r() - pPrev.r()),
                                    std::max(std::abs(pCrnt.g() - pPrev.g()),
                                             std::abs(pCrnt.b() - pPrev.b())));
        }
      }
    }
    ...
    diffImage.FillWith(0);
    {
      ...
      cv::reduce(diffImageRGB.reshape(1, image.GetNumElements()),
                 diffImage.get_CvMat_().reshape(1, image.GetNumElements()), 1,
                 CV_REDUCE_MAX);
    }
...
static inline f32 ratioTestHelper(u8 value1, u8 value2) {
  // NOTE: not checking for divide-by-zero here because
  // kMotionDetection_MinBrightness (DEV_ASSERTed to be > 0 in
...

    {
      ...
      for (s32 i = 0; i < nrows; ++i) {
        ...
        for (s32 j = 0; j < ncols; ++j) {
          ...
          if (p1.IsBrighterThan(kMinBrightness) &&
              p2.IsBrighterThan(kMinBrightness)) {
            ...
            ratioImage_i[j] =
                kDisplayMultiplier *
                (std::max(ratioR, std::max(ratioG, ratioB)) - 1.f);
          } else {
            ratioImage_i[j] = 0;
          }
        }
    ...
    ratioImage.FillWith(0);
    {
      ...
      for (s32 i = 0; i < nrows; ++i) {
        ...
        for (s32 j = 0; j < ncols; ++j) {
          ...
          if (p1.IsBrighterThan(kMinBrightness) &&
              p2.IsBrighterThan(kMinBrightness)) {
            ratioImage_i[j] = kDisplayMultiplier *
                              (std::max(_ratioLUT[p1.r()][p2.r()],
...
Result Benchmark::SquaredDiffFromScalar(const ImageRGB& image) {
  ...
  diffSq.FillWith(0);
  {
    ...
    cv::Mat squared;
    cv::subtract(image.get_CvMat_(),
                 cv::Scalar(scalar.r(), scalar.g(), scalar.b()), squared,
                 cv::noArray(), CV_32F);
...
void Benchmark::CreateNormalizeLUT() {
  ...
  for (u16 numer = 0; numer <= 255;
       ++numer)  // Note: need u16 because ++255 rolls back to zero, making this
                 // "while(true)"!
  {
