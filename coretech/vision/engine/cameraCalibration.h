...
class CameraCalibration {
 public:
  ...
  CameraCalibration(u16 nrows, u16 ncols, f32 fx, f32 fy, f32 center_x,
                    f32 center_y, f32 skew = 0.f);

  // Construct with distortion coeffs provided in some kind of container which
  // supports std::copy
  template <class DistCoeffContainer>
  CameraCalibration(u16 nrows, u16 ncols, f32 fx, f32 fy, f32 center_x,
                    f32 center_y, f32 skew,
  ...
  // Adjust focal length and center for new resolution, leave skew and
  // distortion coefficents alone.
  CameraCalibration& Scale(
      u16 nrows, u16 ncols);  // in place, returning reference to "this"
...
inline u16 CameraCalibration::GetNrows() const { return _nrows; }

inline u16 CameraCalibration::GetNcols() const { return _ncols; }
...
inline f32 CameraCalibration::GetCenter_x() const { return _center.x(); }

inline f32 CameraCalibration::GetCenter_y() const { return _center.y(); }

inline const Point2f& CameraCalibration::GetCenter() const { return _center; }

inline f32 CameraCalibration::GetSkew() const { return _skew; }
...
inline const CameraCalibration::DistortionCoeffs&
CameraCalibration::GetDistortionCoeffs() const {
  return _distortionCoeffs;
}

template <class DistCoeffContainer>
CameraCalibration::CameraCalibration(u16 nrows, u16 ncols, f32 fx, f32 fy,
                                     f32 center_x, f32 center_y, f32 skew,
                                     const DistCoeffContainer& distCoeffs)
    : CameraCalibration(nrows, ncols, fx, fy, center_x, center_y, skew) {
  std::copy(distCoeffs.begin(), distCoeffs.end(),
            std::back_inserter(_distortionCoeffs));
}
