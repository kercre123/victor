...
#define LOG_CHANNEL \
  "NeuralNets"  // TODO: Logging to NeuralNets for historical reasons, should
...
// 2: save full images
CONSOLE_VAR_ENUM(s32, kIAsyncRunner_SaveImages, CONSOLE_GROUP, 0,
                 "Off,Save Resized,Save Original Size");

// 1: Full size, 2: Half size
CONSOLE_VAR_RANGED(s32, kIAsyncRunner_OrigImageSubsample, CONSOLE_GROUP, 1, 1,
                   2);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
IAsyncRunner::IAsyncRunner() : _profiler("IAsyncRunner") {}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result IAsyncRunner::Init(const std::string& cachePath,
  ...
  // Get the input height/width so we can do the resize and only need to
  // share/copy/write as small an image as possible for the standalone CNN
  // process to pick up
  if (false == JsonTools::GetValueOptional(config, JsonKeys::InputHeight,
                                           _processingHeight)) {
    LOG_ERROR("IAsyncRunner.Init.MissingConfig", "%s", JsonKeys::InputHeight);
    return RESULT_FAIL;
  }

  if (false == JsonTools::GetValueOptional(config, JsonKeys::InputWidth,
                                           _processingWidth)) {
    LOG_ERROR("IAsyncRunner.Init.MissingConfig", "%s", JsonKeys::InputWidth);
    return RESULT_FAIL;
  }

  _profiler.SetPrintFrequency(
      config.get("ProfilingPrintFrequency_ms", 10000).asUInt());
  _profiler.SetDasLogFrequency(
      config.get("ProfilingEventLogFrequency_ms", 10000).asUInt());
...
bool IAsyncRunner::StartProcessingHelper() {
  DEV_ASSERT(!_imgOrig.IsEmpty(),
             "IAsyncRunner.StartProcessingHelper.EmptyImage");

  if (kIAsyncRunner_SaveImages == kSaveFullSizedImage) {
    const std::string saveFilename = Util::FileUtils::FullFilePath(
        {_cachePath, "full", std::to_string(_imgOrig.GetTimestamp()) + ".png"});
  ...
  if (IsVerbose()) {
    LOG_INFO("IAsyncRunner.StartProcessingHelper.ProcessingImage",
             ...
             _imgBeingProcessed.GetNumCols(), _imgBeingProcessed.GetNumRows(),
             _imgBeingProcessed.GetTimestamp());
  }

  _future = std::async(std::launch::async,
                       [this]() { return Run(_imgBeingProcessed); });
...
bool IAsyncRunner::StartProcessingIfIdle(ImageCache& imageCache) {
  ...
  const ImageCacheSize kOrigImageSize =
      imageCache.GetSize(kIAsyncRunner_OrigImageSubsample);
...
bool IAsyncRunner::StartProcessingIfIdle(const Vision::ImageRGB& img) {
  if (!_isInitialized) {
    // This will spam the log, but only in the LOG_CHANNEL channel, plus it
    // helps make it more obvious to a developer that something is wrong since
    // it's easy to miss a model load failure (and associated error in the log)
    // at startup.
    //
    // If you do see this error, it is likely one of two things:
    //  1. Your model configuration in vision_config.json is wrong (look for
    //  other errors on load)
    //  2. Git LFS has failed you. See:
    //  https://ankiinc.atlassian.net/browse/VIC-13455
    LOG_INFO("IAsyncRunner.StartProcessingIfIdle.FromImage.NotInitialized",
             "t:%ums", img.GetTimestamp());
    return false;
  }

  // If we're not already processing an image with a "future", create one to
  // process this image asynchronously.
  if (!_future.valid()) {
    img.CopyTo(_imgOrig);
...
bool IAsyncRunner::GetDetections(std::list<SalientPoint>& salientPoints) {
  if (_future.valid()) {
    ...
    const std::future_status futureStatus =
        _future.wait_for(std::chrono::milliseconds{0});
    if (std::future_status::ready == futureStatus) {
      auto newSalientPoints = _future.get();
      std::copy(newSalientPoints.begin(), newSalientPoints.end(),
                std::back_inserter(salientPoints));

      DEV_ASSERT(!_future.valid(),
                 "IAsyncRunner.GetDetections.FutureStillValid");

      if (ANKI_DEV_CHEATS && IsVerbose()) {
        if (salientPoints.empty()) {
          LOG_INFO("IAsyncRunner.GetDetections.NoSalientPoints", "t=%ums",
                   _imgBeingProcessed.GetTimestamp());
        }
