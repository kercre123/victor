 ...
 * Date:   11/30/2015
 *
 * Description: Simple tic/toc style profiler which can track and log averages,
 *as well as report to DAS. See also: ScopedTicToc for even more basic timing.
 *
 *
...
class Profiler {
 public:
  // TODO: Create templated Profiler_ class, with Resolution as template
  // argument. COZMO-3246 Then alias Profiler = Profiler_<milliseconds> and
  // HighResProfiler = Profiler_<microseconds>.
  using Resolution = std::chrono::milliseconds;
  using ClockType = std::chrono::high_resolution_clock;

  static_assert(ClockType::is_steady, "ClockType should be steady");

  // Name is purely used for creating a more specific event name for printing
  // timing messages: "Profiler.<name>".
  ...
  inline void SetPrintChannelName(const char* name) {
    _printChannelName = name;
  }
  ...
  // Set the minimum time between automatic average timing prints in
  // milliseconds
  void SetPrintFrequency(int64_t printFrequency_ms) {
    _timeBetweenPrints_ms = printFrequency_ms;
  }

  // Set the minimum time between logging timing info to DAS, in milliseconds
  void SetDasLogFrequency(int64_t dasFrequency_ms) {
    _timeBetweenDasLogging_ms = dasFrequency_ms;
  }

  // For scoped Tic/Toc pair. Use TicToc() method below. This is just defining
  // the object. This is similar to the ScopedTicToc object but provides the
  // additional stat tracking and logging offered by the Profiler class.
  class TicTocObject {
   public:
   ...
   private:
#if ANKI_VISION_PROFILER
    Profiler& _profiler;
    const char* _timerName = nullptr;
#endif
  };

  // Create a TicToc object to do Tic on instantiation and Toc when going out of
  // scope
  //  For example:
  //   {
