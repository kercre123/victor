...
CONSOLE_VAR(f32, kDistanceFromCameraThresholdSq_mm, "Vision.EyeContact",
            700 * 700.f);
...
EyeContact::EyeContact()
    : _gazeAverage(
          Point2f(GazeData::kleftRightMin_deg, GazeData::kupDownMin_deg)) {
  _gazeHistory.resize(kHistorySize);
}

void EyeContact::Update(const TrackedFace& face, const TimeStamp_t timeStamp) {
  _face = face;
  ...
  _gazeHistory[_currentIndex].Update(
      Point2f(_face.GetGaze().leftRight_deg, _face.GetGaze().upDown_deg));
...
Point2f EyeContact::RecomputeGazeAverage() { return ComputeGazeAverage(true); }
...
bool EyeContact::DetermineMakingEyeContact() {
  ...
  if (_numberOfInliers > kMinNumberOfInliers) {
    ...
    if ((distance < kEyeContactDistanceSq) && _initialized &&
        SecondaryContraints()) {
      eyeContact = true;
    }
  }
...
bool EyeContact::SecondaryContraints() {
  ...
  const bool headRotationInCone =
      ((std::abs(_face.GetHeadPitch().ToFloat()) < kPitchAngleThreshold_rad) &&
  ...
  const bool blinking =
      ((_face.GetBlinkAmount().blinkAmountLeft < kBlinkAmountThreshold) ||
       (_face.GetBlinkAmount().blinkAmountRight < kBlinkAmountThreshold));

  const bool near = (_face.GetHeadPose().GetTranslation().LengthSq() <
