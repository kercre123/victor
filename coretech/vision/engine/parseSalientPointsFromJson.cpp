...
CONSOLE_VAR_RANGED(s32, kNeuralNets_MaxNumSceneDescriptionTags, "NeuralNets", 5,
                   3, 10);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result ParseSceneDescriptionFromJson(
    const Json::Value& jsonSalientPoints, const int imageRows,
    const int imageCols, const TimeStamp_t timestamp,
    std::list<Vision::SalientPoint>& salientPoints) {
  // Assumes Json structure like this is present in detectionResult:
  //      "description":{
  //        "captions":[
  //          {
  //            "text":"a man sitting in front of a laptop",  <-- this is the
  //            scene description "confidence":0.8529333419354393 <-- this is
  //            the "score"
  //          }
  //        ]
  //      }
  ...
  if (jsonSalientPoints.isMember("description")) {
    ...
    if (descriptionJson.isMember("captions")) {
      ...
      Vision::SalientPoint salientPoint(
          timestamp, 0.5f, 0.5f, 0.f, 1.f,
          Vision::SalientPointType::SceneDescription, "<no description>", {},
          0);
      ...
      if (captionsJson.empty()) {
        ...
        if (descriptionJson.isMember("tags")) {
          ...
          if (tagsJson.isArray() && !tagsJson.empty()) {
            salientPoint.description =
                "A scene with: " + tagsJson[0].asString();
            s32 tagCount = 1;
            const s32 maxCount = std::min(
                (s32)tagsJson.size(), kNeuralNets_MaxNumSceneDescriptionTags);
            ...
            if (tagCount < maxCount) {
              ...
              salientPoint.description +=
                  ", and " + tagsJson[tagCount].asString();
            }
        ...
        // Otherwise, create a SalientPoint for each caption, with the "text"
        // field as each one's description, and "confidence" as the score, if
        // available
        for (const Json::Value& captionJson : captionsJson) {
          if (captionJson.isMember("text")) {
            salientPoint.score = 0.f;
            JsonTools::GetValueOptional(captionJson, "confidence",
                                        salientPoint.score);
  ...
  if (!gotSalientPoint) {
    LOG_ERROR(
        "OffboardModel.ParseSalientPointsFromJson."
        "FailedToGetSceneDescriptionSalientPoint",
        "");
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result ParseObjectDetectionsFromJson(
    const Json::Value& jsonSalientPoints, const int imageRows,
    const int imageCols, const TimeStamp_t timestamp,
    std::list<Vision::SalientPoint>& salientPoints) {
  ...
  if (!objectsJson.isArray()) {
    LOG_ERROR(
        "OffboardModel.ParseObjectDetectionsFromJson.ExpectingObjectsArray",
        "");
    return RESULT_FAIL;
  }
  ...
  for (const auto& objectJson : objectsJson) {
    if (!objectJson.isMember("name") || !objectJson.isMember("bounding_poly")) {
      LOG_ERROR(
          "OffboardModel.ParseObjectDetectionsFromJson."
          "MissingNameOrBoundingPoly",
          "");
      continue;
    }
    ...
    Vision::SalientPoint salientPoint(timestamp, 0.5f, 0.5f, 0.f, 1.f,
                                      Vision::SalientPointType::Unknown, name,
                                      {}, 0);
    ...
    for (const auto& pointJson : polyJson) {
      if (ANKI_VERIFY(
              pointJson.isMember("x") && pointJson.isMember("y"),
    ...
    if (!salientPoint.shape.empty()) {
      // center contains a sum at this point: make it an average and store in
      // SalientPoint
      ...
      salientPoint.area_fraction =
          (poly.GetMaxX() - poly.GetMinX()) * (poly.GetMaxY() - poly.GetMinY());
    }
    ...
    LOG_INFO("OffboardModel.ParseObjectDetectionsFromJson.FoundSalientPoint",
             "Type:%s Name:%s", EnumToString(salientPoint.salientType),
...
Result ParseFaceDataFromJson(const Json::Value& resultArray,
  ...
  for (const Json::Value& result : resultArray) {
    ...
    Vision::SalientPoint salientPoint(
        timestamp, faceRect.GetXmid(), faceRect.GetYmid(), 0.f, faceRect.Area(),
      ...
      if (!candidatesJson.isNull()) {
        ...
        for (const auto& candidateJson : candidatesJson) {
          if (!candidateJson.isMember("confidence")) {
            LOG_ERROR(
                "OffboardModel.ParseFaceDataFromJson."
                "MissingCandidateConfidence",
                "");
            continue;
          }
          ...
          if (!candidateJson.isMember("person_name")) {
            LOG_ERROR(
                "OffboardModel.ParseFaceDataFromJson.MissingCandidateName", "");
            continue;
          }
    ...
    // Take all the description data and stuff it into the description string as
    // Json
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result ParseTextDetectionsFromJson(
    const Json::Value& detectionResult, const int imageRows,
    const int imageCols, const TimeStamp_t timestamp,
    std::list<Vision::SalientPoint>& salientPoints) {
  if (detectionResult.isMember("recognitionResult")) {
    ...
    DEV_ASSERT(recResult.isMember("lines"),
               "OffboardModel.ParseTextDetectionsFromJson.MissingLines");
    const Json::Value& linesJson = recResult["lines"];
    DEV_ASSERT(linesJson.isArray(),
               "OffboardModel.ParseTextDetectionsFromJson.LinesNotArray");

    // One SalientPoint per line of text
    for (const auto& lineJson : linesJson) {
      Vision::SalientPoint salientPoint(timestamp, 0.f, 0.f, 0.f, 1.f,
                                        Vision::SalientPointType::Text, "", {},
                                        0);

      DEV_ASSERT(
          lineJson.isMember("boundingBox"),
          "OffboardModel.ParseTextDetectionsFromJson.MissingBoundingBox");
      const Json::Value& bboxJson = lineJson["boundingBox"];
      DEV_ASSERT(
          bboxJson.isArray(),
          "OffboardModel.ParseTextDetectionsFromJson.BoundingBoxNotArray");
      DEV_ASSERT(
          bboxJson.size() % 2 == 0,
          "OffboardModel.ParseTextDetectionsFromJson.BoundingBoxNotEvenSize");
      for (s32 i = 0; i < bboxJson.size(); i += 2) {
        CladPoint2d pt(bboxJson[i].asFloat() / (f32)imageCols,
                       bboxJson[i + 1].asFloat() / (f32)imageRows);
      ...
      DEV_ASSERT(lineJson.isMember("text"),
                 "OffboardModel.ParseTextDetectionsFromJson.MissingText");
    ...
    Vision::SalientPoint salientPoint(
        timestamp, 0.f, 0.f, 0.f, 1.f, Vision::SalientPointType::Text,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result ParseSalientPointsFromJson(
    const Json::Value& detectionResult, const int imageRows,
    const int imageCols, const TimeStamp_t timestamp,
    std::list<Vision::SalientPoint>& salientPoints) {
  using ParseFcn =
      std::function<Result(const Json::Value&, const int imageRows,
                           const int imageCols, const TimeStamp_t timestamp,
                           std::list<Vision::SalientPoint>& salientPoints)>;
  ...
  if (!JsonTools::GetValueOptional(
          detectionResult, OffboardProcessor::JsonKeys::ProcType, procType)) {
  ...
  if (iter == kParserLUT.end()) {
    // No registered parser: Assume the detectionResult just contains an array
    // of SalientPoints in Json format
    if (!detectionResult.isMember(JsonKeys::SalientPoints)) {
      LOG_ERROR(
          "OffboardModel.ParseSalientPointsFromJson.MissingSalientPointsArray",
          "%s", JsonKeys::SalientPoints);
      return RESULT_FAIL;
    }

    const Json::Value& salientPointsJson =
        detectionResult[JsonKeys::SalientPoints];
    ...
    for (auto const& salientPointJson : salientPointsJson) {
      ...
      if (!success) {
        LOG_ERROR(
            "OffboardModel.ParseSalientPointsFromJson.FailedToSetFromJSON", "");
        continue;
      }
    ...
    return iter->second(detectionResult, imageRows, imageCols, timestamp,
                        salientPoints);
  }
