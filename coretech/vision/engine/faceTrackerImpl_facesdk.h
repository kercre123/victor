...
FaceTracker::Impl::Impl(const std::string& modelPath, DetectionMode mode)
  ...
  int result = FSDK_ActivateLibrary(
      "gDULw5eSsCx78rRhbS4hRLR/m0wk+AzQgwrt0mM2PbFg6NJg5ETN+xDKuN0cA/"
      "DQApWO1ySzm+WNTgwoGFoIkbPMRqqEAPF7TkhskEqs4q9uogNDziY1MW7Asj+"
      "fS6IAo9tE7ItR2goUifoNVuAlG8R3brcbum4yA/vfsvrgQ2s=");
  ...
  PRINT_NAMED_INFO("FaceTracker.Impl.LicenseInfo",
                   "Luxand FaceSDK license info: %s", licenseInfo);
  ...
  if (result != FSDKE_OK) {
    PRINT_NAMED_ERROR("FaceTracker.Impl.SetTrackerParametersFail",
                      "Failed setting parameter %d.", errorPosition);
    return;
  }
...
FaceTracker::Impl::~Impl() { FSDK_FreeTracker(_tracker); }

void FaceTracker::Impl::EnableNewFaceEnrollment(bool enable) {
  if (enable != _enrollNewFaces) {
    int result = FSDK_SetTrackerParameter(_tracker, "Learning",
                                          (enable ? "true" : "false"));
    if (result != FSDKE_OK) {
...
static inline TrackedFace::Feature GetFeatureHelper(
    const FSDK_Features& fsdk_features, std::vector<int>&& indices) {
...
Result FaceTracker::Impl::Update(const Vision::Image& frameOrig) {
  if (!_isInitialized) {
    PRINT_NAMED_ERROR(
        "FaceTracker.Impl.Update.NotInitialized",
  ...
  HImage fsdk_img;
  int res = FSDK_LoadImageFromBuffer(
      &fsdk_img, frameOrig.GetDataPointer(), frameOrig.GetNumCols(),
      frameOrig.GetNumRows(), frameOrig.GetNumCols(),
  ...
  if (res != FSDKE_OK) {
    PRINT_NAMED_WARNING("FaceTracker.Impl.Update.TrackerFailed",
                        "With result=%d", res);
    return RESULT_FAIL;
  }
  ...
  for (int iFace = 0; iFace < detectedCount; ++iFace) {
    ...
    if (res != FSDKE_OK) {
      PRINT_NAMED_WARNING("FaceTracker.Impl.Update.FacePositionFailure",
                          "Failed finding position for face %d of %lld", iFace,
                          detectedCount);
    } else {
    ...
    if (res != FSDKE_OK) {
      PRINT_NAMED_WARNING("FaceTracker.Impl.Update.FaceFeatureFailure",
                          "Failed finding features for face %d of %lld", iFace,
                          detectedCount);
    } else {
      face.SetLeftEyeCenter(
          Point2f(features[FSDKP_LEFT_EYE].x, features[FSDKP_LEFT_EYE].y));

      face.SetRightEyeCenter(
          Point2f(features[FSDKP_RIGHT_EYE].x, features[FSDKP_RIGHT_EYE].y));

      // Set the observed head orientation
      // NOTE: FaceSDK doesn't have head pose estimation, so just use angle of
      // the line connecting the eyes as a proxy for roll
      ...
      face.SetFeature(
          TrackedFace::LeftEyebrow,
          GetFeatureHelper(
              features,
              {FSDKP_LEFT_EYEBROW_OUTER_CORNER, FSDKP_LEFT_EYEBROW_MIDDLE_LEFT,
               FSDKP_LEFT_EYEBROW_MIDDLE, FSDKP_LEFT_EYEBROW_MIDDLE_RIGHT,
               FSDKP_LEFT_EYEBROW_INNER_CORNER}));

      face.SetFeature(
          TrackedFace::RightEyebrow,
          GetFeatureHelper(features, {FSDKP_RIGHT_EYEBROW_INNER_CORNER,
      ...
      face.SetFeature(
          ...
          GetFeatureHelper(
              features,
              {FSDKP_LEFT_EYE_OUTER_CORNER, FSDKP_LEFT_EYE_UPPER_LINE1,
               FSDKP_LEFT_EYE_UPPER_LINE2, FSDKP_LEFT_EYE_UPPER_LINE3,
               FSDKP_LEFT_EYE_INNER_CORNER, FSDKP_LEFT_EYE_LOWER_LINE1,
               FSDKP_LEFT_EYE_LOWER_LINE2, FSDKP_LEFT_EYE_LOWER_LINE3,
               FSDKP_LEFT_EYE_OUTER_CORNER}));

      face.SetFeature(
          TrackedFace::RightEye,
          GetFeatureHelper(
              features,
              {FSDKP_RIGHT_EYE_OUTER_CORNER, FSDKP_RIGHT_EYE_UPPER_LINE1,
               FSDKP_RIGHT_EYE_UPPER_LINE2, FSDKP_RIGHT_EYE_UPPER_LINE3,
               FSDKP_RIGHT_EYE_INNER_CORNER, FSDKP_RIGHT_EYE_LOWER_LINE1,
      ...
      face.SetFeature(
          TrackedFace::Nose,
          GetFeatureHelper(
              features, {FSDKP_NOSE_BRIDGE, FSDKP_NOSE_RIGHT_WING,
                         FSDKP_NOSE_RIGHT_WING_OUTER,
                         FSDKP_NOSE_RIGHT_WING_LOWER, FSDKP_NOSE_BOTTOM,
                         FSDKP_NOSE_LEFT_WING_LOWER, FSDKP_NOSE_LEFT_WING_OUTER,
                         FSDKP_NOSE_LEFT_WING, FSDKP_NOSE_BRIDGE}));

      face.SetFeature(
          TrackedFace::UpperLip,
          GetFeatureHelper(
              features,
              {FSDKP_MOUTH_RIGHT_CORNER,  // FaceSDK L/R reversed!
               FSDKP_MOUTH_LEFT_TOP, FSDKP_MOUTH_TOP, FSDKP_MOUTH_RIGHT_TOP,
               FSDKP_MOUTH_LEFT_CORNER,  // FaceSDK L/R reversed!
               FSDKP_MOUTH_RIGHT_TOP_INNER, FSDKP_MOUTH_TOP_INNER,
               FSDKP_MOUTH_LEFT_TOP_INNER,
               FSDKP_MOUTH_RIGHT_CORNER}));  // FaceSDK L/R reversed!

      face.SetFeature(
          TrackedFace::LowerLip,
          GetFeatureHelper(
              features,
              {FSDKP_MOUTH_RIGHT_CORNER,  // FaceSDK L/R reversed!
               FSDKP_MOUTH_LEFT_BOTTOM, FSDKP_MOUTH_BOTTOM,
               FSDKP_MOUTH_RIGHT_BOTTOM,
               FSDKP_MOUTH_LEFT_CORNER,  // FaceSDK L/R reversed!
               FSDKP_MOUTH_RIGHT_BOTTOM_INNER, FSDKP_MOUTH_BOTTOM_INNER,
      ...
      face.SetFeature(
          TrackedFace::Contour,
          GetFeatureHelper(
              features, {FSDKP_FACE_CONTOUR2, FSDKP_FACE_CONTOUR1,
                         FSDKP_CHIN_LEFT, FSDKP_CHIN_BOTTOM, FSDKP_CHIN_RIGHT,
                         FSDKP_FACE_CONTOUR13, FSDKP_FACE_CONTOUR12}));
    }
  }
