...
CONSOLE_VAR(s32, kFaceDetectionDelayDuringEnrollment_ms, "Vision.FaceTracker",
            1000);

// The minimum number of recognition frames needed before an enrollment
// completes
...
// This is the delay for recognition. It isn't all that useful in testing
// because it assumes that the OKAO implementation is running in synchronous
// mode. For the situations we are trying to test here we probably don't care
// about the synchorous use case.
...
Result FaceTracker::Impl::Update(const Vision::Image& frameOrig,
  ...
  face.SetRect(Rectangle<f32>(nWidth / 2 - (detectRectWidth / 2),
                              ...
                              detectRectWidth, detectRectHeight));
  ...
  if (_totalNumberOfCalls > kNumberOfFramesBeforeUpdatedFace) {
    UpdatedFaceID update{.oldID = 1, .newID = 3, .newName = ""};
...
void FaceTracker::Impl::SetFaceEnrollmentMode(Vision::FaceID_t forFaceID,
  ...
  PRINT_NAMED_INFO("FaceTrackerImpl.SetFaceEnrollmentMode.NumFakeEnrollments",
                   "numer of enrollments %d for face %d", forFaceID,
                   numEnrollments);
  if (numEnrollments > 0) {
...
// TODO this is duplicated code but isn't compiled in from the OKAO
// implementation, eventually we should move this some place where it isn't
// duplicated.
...
// TODO same as above this is duplicated code but isn't compiled in from the
// OKAO implementation, eventually we should move to some place where it isn't
// duplicated.
...
bool FaceTracker::Impl::HaveAllowedTrackedFaces() { return false; }

void FaceTracker::Impl::ClearAllowedTrackedFaces() {}

bool FaceTracker::Impl::IsRecognitionSupported() { return true; }

float FaceTracker::Impl::GetMinEyeDistanceForEnrollment() { return 0.f; }

bool FaceTracker::Impl::CanAddNamedFace() const { return true; }
...
Result FaceTracker::Impl::EraseFace(FaceID_t faceID) { return RESULT_OK; }
