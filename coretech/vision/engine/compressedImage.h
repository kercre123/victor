...
class CompressedImage {
 public:
  ...
  template <class PixelType>
  const std::vector<u8>& Compress(const ImageBase<PixelType>& img,
                                  s32 quality = 50);
  ...
  // Returns the raw compressed image data, will be empty if Compress has not
  // been called
  const std::vector<u8>& GetCompressedBuffer() const {
