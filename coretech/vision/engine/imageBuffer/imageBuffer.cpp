...
ImageBuffer::ImageBuffer(u8* data, s32 numRows, s32 numCols,
                         ImageEncoding format, TimeStamp_t timestamp, s32 id)
    : _rawData(data),
      _rawNumRows(numRows),
      _rawNumCols(numCols),
      _format(format),
...
void ImageBuffer::GetNumRowsCols(s32& rows, s32& cols) const {
  switch (_format) {
    ...
    default: {
      ...
      ANKI_VERIFY(false, "ImageBuffer.GetNumRowsCols.UnsupportedFormat", "%s",
...
bool ImageBuffer::GetRGB(ImageRGB& rgb, ImageCacheSize size) const {
  ...
  switch (_format) {
    ...
    default: {
      DEV_ASSERT_MSG(false, "ImageBuffer.GetRGB.UnsupportedFormat", "%s",
                     EnumToString(_format));
      return false;
    } break;
  }
...
bool ImageBuffer::GetGray(Image& gray, ImageCacheSize size) const {
  ...
  switch (_format) {
    ...
    default: {
      DEV_ASSERT_MSG(false, "ImageBuffer.GetGray.UnsupportedFormat", "%s",
                     EnumToString(_format));
      return false;
    } break;
  }
...
bool ImageBuffer::GetRGBFromBAYER(ImageRGB& rgb, ImageCacheSize size) const {
  switch (size) {
    case ImageCacheSize::Full: {
      ImageConversions::DemosaicBGGR10ToRGB(_rawData, _rawNumRows, _rawNumCols,
                                            rgb);
    }
      return true;

    case ImageCacheSize::Half: {
      ImageConversions::HalveBGGR10ToRGB(_rawData, _rawNumRows, _rawNumCols,
                                         rgb);
    }
      return true;
    ...
    case ImageCacheSize::Quarter: {
      ...
      ImageConversions::QuarterBGGR10ToRGB(_rawData, _rawNumRows, _rawNumCols,
                                           rgb);
    }
      return true;
...
bool ImageBuffer::GetRGBFromYUV420sp(ImageRGB& rgb, ImageCacheSize size) const {
  ...
  ImageConversions::ConvertYUV420spToRGB(_rawData, rows, cols, rgb);
...
bool ImageBuffer::GetGrayFromBAYER(Image& gray, ImageCacheSize size) const {
  ...
  ImageConversions::HalveBGGR10ToGray(_rawData, _rawNumRows, _rawNumCols, gray);
...
bool ImageBuffer::GetGrayFromRawRGB(Image& gray, ImageCacheSize size) const {
  ImageRGB rgb;
  // For now there are no optimized conversions from RawRGB to Gray so convert
  // to RGB and then fill gray
...
bool ImageBuffer::GetGrayFromYUV420sp(Image& gray, ImageCacheSize size) const {
  ImageRGB rgb;
  // For now there are no optimized conversions from YUV420sp to Gray so convert
  // to RGB and then fill gray
