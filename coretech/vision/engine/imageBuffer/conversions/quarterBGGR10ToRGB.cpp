...
void QuarterBGGR10ToRGB(const u8* bayer, s32 rows, s32 cols, ImageRGB& rgb) {
  ...
  for (int i = 0; i < rows - 1; i += kNumRowsToProcess) {
    ...
    // Process 20 bytes of bayer data at a time which will result in 4 RGB
    // pixels We only need to look at a quarter of the bayer data so we can skip
    // every other 2x2 chunk of data which also includes the 5th byte of packed
    // LSBs For the first row the data we will load looks like
    // ([r g] r g p [r g] r) g p ([r g] r g p [r g] r) g p
    // We need to process the bytes in [ ] and so will be loading everything in
    // ( ) 2 bytes can be skipped each time, the second g p The goal will be to
    // make all the [r g] blocks sequential in order to unzip them to get all
    // red and all greens in separate registers This will be done with a series
    // of bit shifts and bit selects
    const s32 kNumElementsToProcess = 20;
    s32 j = 0;
    for (; j < (cols - 1) - (kNumElementsToProcess - 1);
         j += kNumElementsToProcess) {
      ...
      // Left shift the entire register 24 bits and bit select it with original
      // row data
      uint8x8_t temp =
          vreinterpret_u8_u64(vshl_n_u64(vreinterpret_u64_u8(row1_2), 24));
    ...
    for (; j < cols; j += 5) {
      ...
      rgbPtr[1] =
          cv::saturate_cast<u8>((((u16)bayer1[1] + (u16)bayer2[0])) << 1);
...
#else
  // No neon available so halve to RGB and then resize to get quarter sized
  // output HalveBGGR10ToRGB has a non-neon implementation
