...
void ConvertYUV420spToRGB(const u8* yuv, s32 rows, s32 cols, ImageRGB& rgb) {
  ...
  for (; r < rows; r += 2) {
    ...
    for (; c < cols - (8 - 1); c += 8) {
      ...
      // be preserved in the conversion to u16. This is neccessary when we
      // reinterpret from u16 to s16 so we end up with the correct value from Y
      // - 16
      ...
      // We only need 4 UV pairs for 8 Y (have to load 8 pairs though) and each
      // UV will operate on 2 consecutive Ys (2x2 subsampling) so we want the U
      // and V data to look like 1 1 2 2 3 3 4 4
    ...
    for (; c < cols; c += 2) {
      ...
      outputPtr1[0] =
          cv::saturate_cast<uchar>((y00 + ruv) >> ITUR_BT_601_SHIFT);
      outputPtr1[1] =
          cv::saturate_cast<uchar>((y00 + guv) >> ITUR_BT_601_SHIFT);
      outputPtr1[2] =
          cv::saturate_cast<uchar>((y00 + buv) >> ITUR_BT_601_SHIFT);

      int y01 = std::max(0, int(yPtr[1]) - 16) * ITUR_BT_601_CY;
      outputPtr1[3] =
          cv::saturate_cast<uchar>((y01 + ruv) >> ITUR_BT_601_SHIFT);
      outputPtr1[4] =
          cv::saturate_cast<uchar>((y01 + guv) >> ITUR_BT_601_SHIFT);
      outputPtr1[5] =
          cv::saturate_cast<uchar>((y01 + buv) >> ITUR_BT_601_SHIFT);

      int y10 = std::max(0, int(y2Ptr[0]) - 16) * ITUR_BT_601_CY;
      outputPtr2[0] =
          cv::saturate_cast<uchar>((y10 + ruv) >> ITUR_BT_601_SHIFT);
      outputPtr2[1] =
          cv::saturate_cast<uchar>((y10 + guv) >> ITUR_BT_601_SHIFT);
      outputPtr2[2] =
          cv::saturate_cast<uchar>((y10 + buv) >> ITUR_BT_601_SHIFT);

      int y11 = std::max(0, int(y2Ptr[1]) - 16) * ITUR_BT_601_CY;
      outputPtr2[3] =
          cv::saturate_cast<uchar>((y11 + ruv) >> ITUR_BT_601_SHIFT);
      outputPtr2[4] =
          cv::saturate_cast<uchar>((y11 + guv) >> ITUR_BT_601_SHIFT);
      outputPtr2[5] =
          cv::saturate_cast<uchar>((y11 + buv) >> ITUR_BT_601_SHIFT);
