...
#define NEON_TIME_START() \
  const auto neon_time_start = std::chrono::steady_clock::now();
...
// Use with NEON_TIME_START to print the average time elapsed between START and
// END
#define NEON_TIME_END_AVG(name)                                               \
  const auto neon_time_end = std::chrono::steady_clock::now();                \
  double neon_time_dif =                                                      \
      std::chrono::duration<double>(neon_time_end - neon_time_start).count(); \
  static double neon_time_dif_avg = 0;                                        \
  neon_time_dif_avg += neon_time_dif;                                         \
  static u64 neon_time_count = 0;                                             \
  neon_time_count++;                                                          \
  PRINT_NAMED_INFO("NEON Time", "%s %f", name,                                \
                   (neon_time_dif_avg / (double)neon_time_count));

// Print the contents of a double word NEON vector `reg` holding data of `type`
// with `size` elements
...
// Print the contents of a quadword NEON vector `reg` holding data of `type`
// with `size` elements
