...
class CameraParamsController {
 public:
  CameraParamsController(u16 minExpTime_ms, u16 maxExpTime_ms, f32 minGain,
                         f32 maxGain, const CameraParams& initialParams);

  // Set parameters used for computing exposure (maxChangeFraction and subSample
  // also used for white balance)
  ...
  // - Image is too bright if the value of the current image at the
  // tooBrightPercentile
  //    is greater than the tooBrightValue.
  // - Image is too dark if the value of the current image at the
  // tooDarkPercentile
  //    is less than the tooDarkValue.
  Result SetImageQualityParameters(const u8 tooDarkValue,
                                   const f32 tooDarkPercentile,
                                   const u8 tooBrightValue,
                                   const f32 tooBrightPercentile);
  ...
  // Give half the weight for exposure to the metering regions, the other half
  // to the rest of the image
  void AddMeteringRegion(
      const Rectangle<s32>& rect);  // TODO: Add ability to specify weight
  void ClearMeteringRegions();

  // Compute "next" exposure and white balance, using color image. (Green
  // channel is used for exposure.)
  ...
  // Actual update the "current" camera parameters, presumably after using the
  // "next" params above to update the real camera's settings
  Result UpdateCurrentCameraParams(const CameraParams& newParams);

  Result SetGammaTable(const std::vector<u8>& inputs,
                       const std::vector<u8>& gamma);

  Result Linearize(const Vision::Image& image,
                   Vision::Image& linearizedImage) const;
  ...
  const CameraParams& GetNextCameraParams() const {
    return _currentCameraParams;
  }
  ...
  bool IsExposureCyclingComplete() const {
    return (_cycleTargetIter == _cyclingTargetValues.end());
  }
  void ResetTargetAutoExposure_Cycling() {
 ...
 private:
  ...
  bool GetMeteringWeightMask(const s32 nrows, const s32 ncols,
                             Image& weightMask);

  // Compute multiplicative amount by which to adjust exposure (whether time or
  // gain)
  Result ComputeExposureAdjustment(const Vision::Image& image,
                                   const bool useCycling,
                                   f32& adjustmentFraction);

  Result ComputeExposureAdjustment(const std::vector<Vision::Image>& imageROIs,
                                   f32& adjustmentFraction);

  // Computes multiplicative amount by which to adjust red, and blue gains for
  // WhiteBalance
  // - Caller can update gains via: newGainX = adjX * oldGainX
  // - Green gain is assumed to be fixed
  // - Uses the gray world assumption
  Result ComputeWhiteBalanceAdjustment(const Vision::ImageRGB& image, f32& adjR,
                                       f32& adjB);

  // Simultaneously compute exposure and white balance adjustment from a color
  // image
  Result ComputeExposureAndWhiteBalance(const Vision::ImageRGB& img,
                                        const bool useCycling, f32& adjExp,
                                        f32& adjR, f32& adjB);
