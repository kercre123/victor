 ...
 * Date:   4/1/2019
 *
 * Description: Helper class for processing images "offboard", generally meaning
 *"not on the robot": in the cloud, on a laptop, etc. The FileIO mode also
 *allows simply processing with a separate process on the robot as well.
 *
 *              Note that this processor is synchronous / blocking. If you
 *additionally want asynchronous behavior use in conjunction with an
 *IAsyncRunner.
 *
...
class OffboardProcessor {
 public:
  ...
  // Use these instead of hard-coded strings to refer to the underlying files
  // and Json keys used by an OffboardProcessor
  struct Filenames {
 ...
 private:
  Result DetectWithFileIO(const ImageRGB& img,
                          std::list<SalientPoint>& salientPoints);

  Result DetectWithCLAD(const ImageRGB& img,
                        std::list<SalientPoint>& salientPoints);
