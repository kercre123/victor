...
class ICNNLayer {
 public:
  ...
  virtual ~ICNNLayer() {}
  virtual Result Load(const std::string& dirname,
...
class ConvLayer : public ICNNLayer {
 public:
  virtual ~ConvLayer() {}
  virtual Result Load(const std::string& dirname,
                      const Json::Value& jsonNode) override;
...
Result ICNNLayer::InitOutput(vl::TensorGeometry& outputGeom) {
  ...
  _output =
      vl::Tensor(&(_outputBuffer[0]), _outputBuffer.size() * sizeof(float),
                 vl::CPU, outputGeom);

  return RESULT_OK;
}

ConvolutionalNeuralNet::ConvolutionalNeuralNet() : _isLoaded(false) {}
...
Result ConvolutionalNeuralNet::Load(std::string dirname) {
  ...
  if (false == reader.parse(jsonFile, jsonRoot)) {
    PRINT_NAMED_ERROR("ConvolutionalNeuralNet.Load",
                      "Failed to parse CNN Json file %s", jsonFilename.c_str());
    return RESULT_FAIL;
  } else {
    PRINT_NAMED_INFO("ConvolutionalNeuralNet.Load", "Loaded CNN Json file %s",
                     jsonFilename.c_str());
  }
  jsonFile.close();
  ...
  PRINT_NAMED_INFO("ConvolutionalNeuralNet.Load",
                   "Loaded %lu CNN layers & %lu class names", _layers.size(),
                   _classNames.size());
...
Result ConvolutionalNeuralNet::Run(const cv::Mat& img, size_t& classLabel,
...
Result ConvolutionalNeuralNet::Run(const cv::Mat& img,
                                   std::vector<float>& output) {
  if (!_isLoaded) {
    PRINT_NAMED_ERROR("ConvolutionalNeuralNet.Run.NotLoaded",
                      "CNN not yet loaded");
    return RESULT_FAIL;
  }

  if (img.channels() > 1) {
    PRINT_NAMED_ERROR("ConvolutionalNeuralNet.Run",
                      "Color image data not yet supported");
    return RESULT_FAIL;
  }
  ...
  vl::Tensor input(&(inputData[0]), inputData.size() * sizeof(float), vl::CPU,
                   inputGeom);
  ...
  for (auto layer : _layers) {
    ...
    if (layerResult != RESULT_OK) {
      PRINT_NAMED_ERROR("ConvolutionalNeuralNet.Run.LayerFail",
                        "Layer returned failure");
      return layerResult;
    }
  ...
  // Using const_cast here because vl library doesn't provide a const
  // getMemory() method. :-/
  const float* outputData = const_cast<vl::Tensor*>(lastOutput)->getMemory();

  // Copy the result out
  std::copy(outputData, outputData + lastOutput->getNumElements(),
...
#define READ(__NAME__)                                              \
  do {                                                              \
    file.read((char*)&__NAME__, sizeof(__NAME__));                  \
    if (file.bad()) {                                               \
      PRINT_NAMED_ERROR("FileREAD.Failure", "%s", QUOTE(__NAME__)); \
      return RESULT_FAIL;                                           \
    }                                                               \
  } while (0)

Result ReadTensor(std::ifstream& file, vl::Tensor& tensor,
                  std::vector<float>& memory, vl::index_t height,
                  vl::index_t width, vl::index_t depth, vl::index_t size) {
  /*
      READ(height);
      READ(width);
      READ(depth);
...
Result ConvLayer::Load(const std::string& dirname,
                       const Json::Value& jsonNode) {
  ...
  if (strideX < 1 || strideY < 1) {
    PRINT_NAMED_ERROR("ConvLayer.Load.BadStride",
                      "At least one element of STRIDE is smaller than one");
    return RESULT_FAIL;
  }
  if (padLeft < 0 || padRight < 0 || padTop < 0 || padBottom < 0) {
    PRINT_NAMED_ERROR("ConvLayer.Load.BadPadding",
                      "An element of PAD is negative");
    return RESULT_FAIL;
  }
  ...
  lastResult = ReadTensor(file, _filters, _filterData, weightHeight,
                          weightWidth, weightDepth, weightSize);
  ...
  lastResult =
      ReadTensor(file, _biases, _biasData, biasHeight, biasWidth, 1, 1);
  if (lastResult != RESULT_OK) {
    return lastResult;
  }
...
Result ConvLayer::Run(const vl::Tensor& inputData) {
  ...
  if (hasFilters) {
    if (filtersGeom.getHeight() == 0 || filtersGeom.getWidth() == 0 ||
        filtersGeom.getDepth() == 0) {
    ...
    filtersGeom =
        vl::TensorGeometry(1, 1, inputData.getDepth(), inputData.getDepth());
    numFilterGroups = 1;
  ...
  const vl::index_t height_out =
      (inputData.getHeight() + (padTop + padBottom) - filtersGeom.getHeight()) /
          strideY +
      1;
  const vl::index_t width_out =
      (inputData.getWidth() + (padLeft + padRight) - filtersGeom.getWidth()) /
          strideX +
      1;
  ...
  if (hasBiases) {
    if (_biases.getNumElements() != filtersGeom.getSize()) {
      PRINT_NAMED_ERROR("vl_cnn_wrapper",
                        "The number of elements of BIASES is not the same as "
  ...
  /*
   ...
   stride of one pixel
   */
  const bool fullyConnectedMode =
      (outputGeom.getHeight() == 1 && outputGeom.getWidth() == 1 &&
       strideY == 1 && strideX == 1 && padTop == 0 && padBottom == 0 &&
       padLeft == 0 && padRight == 0 && numFilterGroups == 1);
  ...
  if (verbosity > 0) {
    PRINT_NAMED_INFO("vl_nnconv", "forward: %s",
                     (inputData.getMemoryType() == vl::GPU) ? "GPU" : "CPU");
    PRINT_NAMED_INFO("vl_nnconv",
                     "stride: [%d %d], pad: [%d %d %d %d], "
                     "num filter groups: %d, has bias: %d, has filters: %d, is "
                     "fully connected: %d",
                     strideY, strideX, padTop, padBottom, padLeft, padRight,
  ...
  if (fullyConnectedMode) {
    error = vl::nnfullyconnected_forward(context, _output, inputData, _filters,
                                         _biases);
  }
  ...
  /* special case: no filters = identity filter bank (subsample + bias) */
    ...
    error =
        vl::nnsubsample_forward(context, _output, inputData, _biases, strideY,
                                strideX, padTop, padBottom, padLeft, padRight);
  }

  /* regular case */
  else {
    error = vl::nnconv_forward(context, _output, 0, inputData, 1, _filters,
                               _biases, strideY, strideX, padTop, padBottom,
                               padLeft, padRight);
  }
...
Result PoolLayer::Load(const std::string& dirname,
                       const Json::Value& jsonNode) {
  ...
  if (method == "max") {
    ...
    PRINT_NAMED_ERROR("PoolLayer.Load.BadMethod",
                      "Unrecognized pooling method '%s'", method.c_str());
    return RESULT_FAIL;
  }

  if (strideX < 1 || strideY < 1) {
    PRINT_NAMED_ERROR("PoolLayer.Load.BadStride",
                      "At least one element of STRIDE is smaller than one");
    return RESULT_FAIL;
  }
  if (poolHeight == 0 || poolWidth == 0) {
    PRINT_NAMED_ERROR("PoolLayer.Load.BadPoolDims",
                      "A dimension of the pooling SIZE is void");
    return RESULT_FAIL;
  }
  if (padLeft < 0 || padRight < 0 || padTop < 0 || padBottom < 0) {
    PRINT_NAMED_ERROR("PoolLayer.Load.BadPadding",
                      "An element of PAD is negative");
    return RESULT_FAIL;
  }
  if (padLeft >= poolWidth || padRight >= poolWidth || padTop >= poolHeight ||
      padBottom >= poolHeight) {
    PRINT_NAMED_ERROR("PoolLayer.Load.IncompatiblePadAndPool",
                      "A padding value is larger or equal than the size of the "
                      "pooling window");
    return RESULT_FAIL;
  }
...
Result PoolLayer::Run(const vl::Tensor& inputData) {
  if (inputData.getHeight() + (padTop + padBottom) < poolHeight ||
    ...
    PRINT_NAMED_ERROR(
        "PoolLayer.Run.InputDimMismatch",
        "The pooling window is larger than the DATA (including padding).");
    return RESULT_FAIL;
  }

  /* Get the output geometry */
  vl::TensorGeometry outputGeom(
      (inputData.getHeight() + (padTop + padBottom) - poolHeight) / strideY + 1,
      (inputData.getWidth() + (padLeft + padRight) - poolWidth) / strideX + 1,
      inputData.getDepth(), inputData.getSize());
  ...
  if (verbosity > 0) {
    PRINT_NAMED_INFO("vl_nnpool:", "forward; %s",
                     (inputData.getMemoryType() == vl::GPU) ? "GPU" : "CPU");
  ...
  error = vl::nnpooling_forward(context, _output, inputData, poolMethod,
                                poolHeight, poolWidth, strideY, strideX, padTop,
...
Result NormLayer::Load(const std::string& dirname,
                       const Json::Value& jsonNode) {
  ...
  if (normDepth < 1) {
    PRINT_NAMED_ERROR("NormLayer.Load.BadDepth",
                      "The normalization depth is smaller than 1.");
    return RESULT_FAIL;
  }
...
Result NormLayer::Run(const vl::Tensor& inputData) {
  ...
  if (verbosity > 0) {
    PRINT_NAMED_INFO("vl_nnnormalize:", "mode %s; %s",
                     ...
                     "forward");
    PRINT_NAMED_INFO(
        "vl_nnnormalize:", "(depth,kappa,alpha,beta): (%lu,%g,%g,%g)",
  ...
  vl::Error error = vl::nnnormalize_forward(
      context, _output, inputData, normDepth, normKappa, normAlpha, normBeta);
...
Result ReLULayer::Load(const std::string& dirname,
                       const Json::Value& jsonNode) {
  return RESULT_OK;
}
...
Result ReLULayer::Run(const vl::Tensor& inputData) {
  ...
  // Using const_cast here because vl::Tensor does not provide a const
  // getMemory() method
