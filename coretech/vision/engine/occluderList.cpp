...
void OccluderList::Clear() { rectDepthPairs_.clear(); }

/* Moved to Camera class
void OccluderList::AddOccluder(const Vision::ObservableObject* object, const
Vision::Camera& camera)
{
  const Pose3d
objectPoseWrtCamera(object->GetPose().GetWithRespectTo(&camera.get_pose()));
...
void OccluderList::AddOccluder(const std::array<Point2f, NumPoints>& points,
...
void OccluderList::AddOccluder(const std::vector<Point2f>& points,
                               const f32 atDistance) {
  AddOccluderHelper(points, atDistance);
}

template <class PointContainer>
void OccluderList::AddOccluderHelper(const PointContainer& points,
                                     const f32 atDistance) {
...
bool OccluderList::IsOccluded(const Point2f& point,
...
bool OccluderList::IsAnythingBehind(const Point2f& point,
                                    const f32 atDistance) const {
  if (!IsEmpty()) {
...
bool OccluderList::IsAnythingBehind(const Quad2f& quad,
                                    const f32 atDistance) const {
  if (!IsEmpty()) {
    // Jump to first occluder farther away than the given point
    auto currentOccluder = rectDepthPairs_.begin();
    ...
    while (currentOccluder != end) {
      ...
      if (boundingBox.Intersect(currentOccluder->second).Area() > 0) {
        // If the quad's bounding box intersects the occluder, then the occluder
        // is behind it
        return true;
      }
...
void OccluderList::GetAllOccluders(
