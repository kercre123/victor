...
// Ignore rotation completely when calling IsSameAs()? If so, just match
// position (translation) and block type.
...
bool ObservableObject::IsVisibleFrom(const Camera& camera,
  ...
  for (auto const& marker : _markers) {
    KnownMarker::NotVisibleReason reason =
        KnownMarker::NotVisibleReason::IS_VISIBLE;
    if (marker.IsVisibleFrom(camera, maxFaceNormalAngle, minMarkerImageSize,
      ...
      if (DEBUG_PRINT_NOT_VISIBLE_REASONS) {
        PRINT_NAMED_DEBUG("ObservableObject.IsVisibleFrom.MarkerVisible",
                          "marker '%s' is visible", marker.GetCodeName());
      }
      return true;
      ...
      PRINT_NAMED_DEBUG("ObservableObject.IsVisibleFrom.MarkerNotVisible",
                        ...
                        marker.GetCodeName(), NotVisibleReasonToString(reason));
    }
  }
...
bool ObservableObject::IsVisibleFrom(const Camera& camera,
  ...
  for (auto const& marker : _markers) {
    if (marker.IsVisibleFrom(camera, maxFaceNormalAngle, minMarkerImageSize,
      ...
      // As soon as any marker is visible from the camera, the object is visible
      // from this camera
      if (DEBUG_PRINT_NOT_VISIBLE_REASONS) {
        PRINT_NAMED_DEBUG("ObservableObject.IsVisibleFrom.MarkerVisible",
                          "marker '%s' is visible", marker.GetCodeName());
...
KnownMarker::NotVisibleReason ObservableObject::IsVisibleFromWithReason(
    const Camera& camera, const f32 maxFaceNormalAngle,
    const f32 minMarkerImageSize, const bool requireSomethingBehind,
    const u16 xBorderPad, const u16 yBorderPad) const {
  KnownMarker::NotVisibleReason latestReason =
      KnownMarker::NotVisibleReason::IS_VISIBLE;
  for (auto const& marker : _markers) {
    KnownMarker::NotVisibleReason reason =
        KnownMarker::NotVisibleReason::IS_VISIBLE;
    if (marker.IsVisibleFrom(camera, maxFaceNormalAngle, minMarkerImageSize,
      ...
      if (Util::EnumToUnderlying(reason) >
          Util::EnumToUnderlying(latestReason)) {
        latestReason = reason;
      }
    }
  }
...
const Pose3d& ObservableObject::GetPose() const {
  // We can't use the pose if the poseState is invalid, since it's potentially
  // garbage. The calling code should instead handle the case of receiving an
  // object whose pose is not valid
  DEV_ASSERT(HasValidPose(), "ObservableObject.GetPose.ObjectPoseIsNotValid");
  return _pose;
}

Vision::KnownMarker const& ObservableObject::AddMarker(
    const Marker::Code& withCode, const Pose3d& atPose,
    const Point2f& size_mm) {
  DEV_ASSERT(!atPose.HasParent(),
             "ObservableObject.AddMarker.MarkerPoseShouldBeRelative");
  ...
  DEV_ASSERT(_markers.back().GetPose().IsChildOf(_pose),
             "ObservableObject.AddMarker.IncorrectParent");
...
std::vector<KnownMarker*> const& ObservableObject::GetMarkersWithCode(
    const Marker::Code& withCode) const {
...
bool ObservableObject::IsSameAs(const ObservableObject& otherObject,
                                ...
                                const Radians& angleThresholdIn, Point3f& Tdiff,
                                Radians& angleDiff) const {
  // If the #define for ignoring rotation is set, use M_PI as the rotation angle
  // threshold, in which case rotation isn't even checked, since it always
  // passes
  const Radians angleThreshold =
      (IGNORE_ROTATION_FOR_IS_SAME_AS ? M_PI : angleThresholdIn);

  const bool isSame = _pose.IsSameAs_WithAmbiguity(
      otherObject.GetPose(), GetRotationAmbiguities(), distThreshold,
...
void ObservableObject::ComputePossiblePoses(
    const ObservedMarker* obsMarker,
  ...
  if (matchingMarkers != _markersWithCode.end()) {
    for (auto matchingMarker : matchingMarkers->second) {
      ...
      Result result =
          matchingMarker->EstimateObservedPose(*obsMarker, markerPoseWrtCamera);
      if (result == RESULT_OK) {
...
void ObservableObject::GetCorners(const Pose3d& atPose,
                                  std::vector<Point3f>& corners) const {
  atPose.ApplyTo(GetCanonicalCorners(), corners);
}

void ObservableObject::GetObservedMarkers(
    std::vector<const KnownMarker*>& observedMarkers,
...
Result ObservableObject::UpdateMarkerObservationTimes(
    const ObservableObject& otherObject) {
  ...
  for (; otherMarkerIter != otherMarkers.end() && markerIter != _markers.end();
    ...
    markerIter->SetLastObservedTime(
        std::max(markerIter->GetLastObservedTime(),
...
void ObservableObject::SetMarkersAsObserved(const Marker::Code& withCode,
  ...
  if (markers != _markersWithCode.end()) {
    ...
    PRINT_NAMED_WARNING("ObservableObject.SetMarkersAsObserved",
                        "No markers found with code %d", withCode);
  }
...
void ObservableObject::SetMarkerAsObserved(const ObservedMarker* nearestTo,
  ...
  if (markers != _markersWithCode.end()) {
    ...
    for (auto marker : markers->second) {
      if (true == marker->GetPose().GetWithRespectTo(camera.GetPose(),
                                                     markerPoseWrtCamera)) {
        Quad2f projPoints;
        camera.Project3dPoints(marker->Get3dCorners(markerPoseWrtCamera),
                               projPoints);

        const bool obsCentroidWithinProjectedQuad =
            projPoints.Contains(obsCentroid);
    ...
    if (whichMarker != nullptr) {
      ...
      PRINT_NAMED_WARNING(
          "ObservableObject.SetMarkerAsObserved.NoMatchingMarkerFound",
          "Marker:%s", nearestTo->GetCodeName());
    }
  } else {
    PRINT_NAMED_WARNING("ObservableObject.SetMarkerAsObserved",
                        "No markers found with code (%d:%s) of given "
                        "'nearestTo' observed marker",
                        nearestTo->GetCode(), nearestTo->GetCodeName());
  }
}  // SetMarkerAsObserved()

Quad2f ObservableObject::GetBoundingQuadXY(const Pose3d& atPose,
                                           const f32 padding_mm) const {
...
static inline bool IsPoseFlat(const Pose3d& pose, const Radians& angleTol,
                              AxisName* outWhichAxis) {
  const RotationMatrix3d Rmat = pose.GetWithRespectToRoot().GetRotationMatrix();
  const bool isFlat =
      (Rmat.GetAngularDeviationFromParentAxis<'Z'>(outWhichAxis) < angleTol);
  return isFlat;
}
...
bool ObservableObject::ClampPoseToFlat(Pose3d& pose, const Radians& angleTol) {
  ...
  // If it is, then keepAxis will be the one around which we'd like to preserve
  // rotation (the one aligned with Z in the parent frame)
  AxisName keepAxis;
  if (IsPoseFlat(pose, angleTol, &keepAxis)) {
    ...
    const bool isNeg = (keepAxis == AxisName::X_NEG) ||
                       (keepAxis == AxisName::Y_NEG) ||
                       (keepAxis == AxisName::Z_NEG);
    const Vec3f& signedZ = (isNeg ? Z_AXIS_3D() * -1.f : Z_AXIS_3D());

    // Compute the angle and axis required to align the top side normal with the
    // signed Z axis Don't bother doing anything if the angle is already near
    // zero
    ...
    if (!alreadyAligned) {
      ...
      DEV_ASSERT(
          !zeroLength,
          "ObservableObject.ClampPoseToFlat.ComputedRotationAxisZeroLength");

      if (!zeroLength) {
        // Update the rotation by applying the rotation required to put the top
        // side normal in line with the (signed) Z axis
...
Pose3d ObservableObject::GetZRotatedPointAboveObjectCenter(
    f32 heightFraction) const {
  // calculate the z translation of the top marker using the current object
  // center and half the size of the object along the current rotated parent
  // axis
  ...
  return Pose3d(
      poseWRTOrigin.GetRotation().GetAngleAroundZaxis(), Z_AXIS_3D(),
      {_pose.GetTranslation().x(), _pose.GetTranslation().y(), zAxisTrans},
      poseWRTOrigin.FindRoot());
}

bool ObservableObject::IsRestingAtHeight(float height, float tolerance) const {
  DEV_ASSERT(HasValidPose(),
             "ObservableObject.IsRestingAtHeight.ObjectPoseIsNotValid");

  const Pose3d& pose = GetPose().GetWithRespectToRoot();
  const f32 blockHeight = GetDimInParentFrame<'Z'>(pose);
  return std::abs(height - (pose.GetTranslation().z() - blockHeight / 2)) <
         tolerance;
}

void ObservableObject::SetObservationTimes(
    const ObservableObject* otherObject) {
  DEV_ASSERT(nullptr != otherObject,
             "ObservableObject.SetObservationTimes.NullOtherObject");

  _lastObservedTime =
      std::max(_lastObservedTime, otherObject->_lastObservedTime);

  UpdateMarkerObservationTimes(*otherObject);
}

Result ObservableObject::GetClosestMarkerPose(
    const Pose3d& referencePose, const bool ignoreZ,
    Pose3d& closestPoseWrtReference) const {
  Marker marker(Marker::ANY_CODE);
  return GetClosestMarkerPose(referencePose, ignoreZ, closestPoseWrtReference,
                              marker);
}

Result ObservableObject::GetClosestMarkerPose(const Pose3d& referencePose,
                                              const bool ignoreZ,
  ...
  for (auto marker : markers) {
    ...
    if (marker.GetPose().GetWithRespectTo(referencePose, markerPoseWrtRef)) {
      const f32 distSq =
          (ignoreZ ? Point2f(markerPoseWrtRef.GetTranslation()).LengthSq()
...
// Helper for the following functions to reason about the object's height (mid
// or top) relative to specified threshold
bool ObservableObject::IsPoseTooHigh(const Pose3d& poseWrtRobot,
                                     float heightMultiplier, float heightTol,
                                     float offsetFraction) const {
  const float rotatedHeight = GetDimInParentFrame<'Z'>(poseWrtRobot);
  const float z =
      poseWrtRobot.GetTranslation().z() + (rotatedHeight * offsetFraction);
  const bool isTooHigh =
      FLT_GT(z, (heightMultiplier * rotatedHeight + heightTol));
  return isTooHigh;
}
