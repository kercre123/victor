 ...
 * Date:   11/8/2018
 *
 * Description: Class for storing a jpg compressed image along with info about
...
template <class PixelType>
const std::vector<u8>& CompressedImage::Compress(
    const ImageBase<PixelType>& img, s32 quality) {
  _compressedBuffer.clear();

  SetMetadata(img);

  _uncompressedBuffer.resize(img.GetNumElements() * img.GetNumChannels());
  cv::Mat_<PixelType> mat(
...
bool CompressedImage::Decompress(ImageBase<PixelRGB>& img) const {
  if (GetNumChannels() != 3) {
    PRINT_NAMED_WARNING(
        "CompressedImage.RGBDecompress.NotRGB",
...
bool CompressedImage::Decompress(ImageBase<u8>& img) const {
  if (GetNumChannels() != 1) {
    PRINT_NAMED_WARNING(
        "CompressedImage.GrayDecompress.NotGray",
