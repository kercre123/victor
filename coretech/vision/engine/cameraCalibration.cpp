...
Result CameraCalibration::Set(const Json::Value& jsonNode) {
  ...
  // Note: unlike parameters above, if distortionCoeffs not present, they are
  // left at 0
...
bool CameraCalibration::operator==(const CameraCalibration& other) const {
  return (_nrows == other._nrows && _ncols == other._ncols &&
          _focalLength_x == other._focalLength_x &&
          _focalLength_y == other._focalLength_y && _center == other._center &&
          _skew == other._skew && _distortionCoeffs == other._distortionCoeffs);
}
...
SmallSquareMatrix<3, PRECISION> CameraCalibration::GetCalibrationMatrix()
    const {
  const PRECISION K_data[9] = {static_cast<PRECISION>(_focalLength_x),
                               static_cast<PRECISION>(_focalLength_x * _skew),
                               static_cast<PRECISION>(_center.x()),
                               PRECISION(0),
                               static_cast<PRECISION>(_focalLength_y),
                               static_cast<PRECISION>(_center.y()),
                               PRECISION(0),
                               PRECISION(0),
                               PRECISION(1)};
...
template <typename PRECISION>
SmallSquareMatrix<3, PRECISION> CameraCalibration::GetInvCalibrationMatrix()
    const {
  const PRECISION invK_data[9] = {
      static_cast<PRECISION>(1.f / _focalLength_x),
      static_cast<PRECISION>(-_skew / _focalLength_y),
      static_cast<PRECISION>(_center.y() * _skew / _focalLength_y -
                             _center.x() / _focalLength_x),
      PRECISION(0),
      static_cast<PRECISION>(1.f / _focalLength_y),
      static_cast<PRECISION>(-_center.y() / _focalLength_y),
      PRECISION(0),
      PRECISION(0),
      PRECISION(1)};
...
template SmallSquareMatrix<3, f64> CameraCalibration::GetCalibrationMatrix()
    const;
template SmallSquareMatrix<3, f32> CameraCalibration::GetCalibrationMatrix()
    const;
template SmallSquareMatrix<3, f64> CameraCalibration::GetInvCalibrationMatrix()
    const;
template SmallSquareMatrix<3, f32> CameraCalibration::GetInvCalibrationMatrix()
    const;
