...
bool PanTiltTracker::IsBusy() { return _isStarted; }

Result PanTiltTracker::StartTracking(TargetSelectionMethod method,
                                     u32 timeout_ms) {
  if (!_isInitialized) {
    PRINT_NAMED_ERROR(
        "PanTiltTracker.StartTracking.NotInitialized",
...
void PanTiltTracker::Target::Set(s32 x, s32 y, s32 h) {
  ...
  // pterry 2017/02/16:  This used to be +=; I've 'fixed' but this code is not
  // used
...
Result PanTiltTracker::Update(std::vector<Rectangle<s32> >& targets,
  ...
  if (!_isInitialized) {
    PRINT_NAMED_ERROR(
        "PanTiltTracker.Update.NotInitialized",
  ...
  if (_isStarted) {
        ...
        switch (_selectionMethod) {
          case LARGEST: {
            ...
            if (diagSizeSq > largestDiagSizeSq) {
              ...
              _currentTarget.Set(target.GetXmid(), target.GetYmid(),
                                 target.GetHeight());
    ...
    if (gotNewTrack) {
      ...
      const f32 yError_pix =
          static_cast<f32>(_yImageCenter - _currentTarget.yCen);
      newTiltAngle = std::atan(yError_pix / _focalLength_pix);

      const f32 xError_pix =
          static_cast<f32>(_xImageCenter - _currentTarget.xCen);
      newPanAngle = std::atan(xError_pix / _focalLength_pix);

      PRINT_NAMED_INFO(
          "PanTiltTracker.Update.NewTrack",
          "yErr=%.1f: tiltAngle=%.1fdeg, xErr=%.1f: panAngle=%.1fdeg",
          yError_pix, newTiltAngle.getDegrees(), xError_pix,
          newPanAngle.getDegrees());

    } else if (_isTracking &&
               BaseStationTimer::getInstance()->GetCurrentTimeStamp() >
                   _currentTarget.timeoutTime_ms) {
      PRINT_NAMED_INFO("PanTiltTracker.Update.TimedOut",
                       "Timed out. Stopping tracking.");
