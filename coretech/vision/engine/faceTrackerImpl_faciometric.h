 ...
 * Date:   11/30/2015
 *
 * Description: Wrapper for FacioMetric face detector / tracker and emotion
 *estimator.
 *
...
class FaceTracker::Impl {
 ...
 private:
  ...
  using SDM = facio::FaceAlignmentSDM<facio::AlignmentFeature,
                                      facio::NO_CONTOUR, facio::LocalManager>;
...
/*!
 ...
 \return std::vector<std::string> Vector containing all the files inside the
 given folder
 */
...
/*!
 ...
 \return std::vector<facio::SubjectRecogData> Vector containing the loaded
 subjects
 */
std::vector<facio::SubjectRecogData> loadEnrolledSubjectsFromFolder(
    std::string folder) {
  std::vector<facio::SubjectRecogData> vectorSubjectRecogData;

  // Init SubjectSerializer
  std::unique_ptr<facio::SubjectSerializer> subjectSerializer(
      new facio::SubjectSerializer());
  ...
  for (int i = 0; i < (int)subjectsPaths.size(); i++) {
    ...
    if (infile.is_open()) {
      ...
      int successDeserialize = subjectSerializer->deserialize(
          bufferMetaData, bufferData, subjectData);
      if (successDeserialize == 0) {
        ...
        PRINT_STREAM_INFO(
            "FaceTracker.loadEnrolledSubjectsFromFolder",
            "There was a problem deserializing the subject with error: "
                << successDeserialize);
      }
    }
  }
...
Result saveSubjectData(facio::SubjectRecogData& subjectDataToEnroll,
  ...
  if (subjectSerializer->serialize(subjectDataToEnroll, subjectMetaData,
                                   subjectMetaDataSize, subjectData,
                                   subjectDataSize) == 1) {
...
/*!
 \param faceRecog  Recognizer object to create the subject and perform the
 recognition. \param imRDList  Vector containing the ImRawData necessary to
 create the subject that will be recognized. \param vectorSubjectRecogData
 Vector containing all the subject to recognize with.
 */
int doRecognition(facio::FaceRecog& faceRecog,
                  std::vector<facio::ImRawData> imRDList,
                  std::vector<facio::SubjectRecogData> vectorSubjectRecogData,
                  std::vector<float>& scores) {
  // Index of the subject recornized (if it is not recognized it will be set to
  // -1)
  int subject = -1;
  ...
  bool subjectCreated = faceRecog.processSubjectRecogData(
      imRDList, label, subjectDataToRecognize);

  if (subjectCreated) {
    // Perform recodnition

    status = faceRecog.predict(subjectDataToRecognize, vectorSubjectRecogData,
                               subject, scores);

    // Debug:
    // saveSubjectData(subjectDataToRecognize,
    // "/Users/andrew/temp/toRecognize.dat");
    // saveSubjectData(vectorSubjectRecogData[0],
    // "/Users/andrew/temp/enrolledFace.dat");

    if (status) { /*
                   if (subject != -1) {
                     PRINT_NAMED_INFO("FaceTracker.doRecognition", "Recognized:
                   %s with score %f",
                                      vectorSubjectRecogData[subject].getLabel().c_str(),
                                      scores[subject]);
                   } else {
                     PRINT_NAMED_INFO("FaceTracker.doRecognition", "Subject not
                   found");
                   }
                   for (int i = 0; i < (int) scores.size(); i++) {
                     PRINT_NAMED_INFO("FaceTracker.doRecognition", "Subject %s
                   with score %f", vectorSubjectRecogData[i].getLabel().c_str(),
                   scores[i]);
                   }
                    */
    } else {
...
/*!
 ...
 \param imRDList  Vector containing the ImRawData necessary to create the
 subject that we will enroll. \param folder Path of the folder we save the
 enrolled objects.
 */
bool enrollSubject(facio::FaceRecog& faceRecog, std::string name,
                   std::vector<facio::ImRawData> imRDList, std::string folder,
  ...
  */
  ...
  bool subjectCreated =
      faceRecog.processSubjectRecogData(imRDList, name, subjectDataToEnroll);
  ...
  if (subjectCreated) {
    ...
    PRINT_NAMED_INFO("FaceTracker.enrollSubject.SubjectAdded",
                     ...
                     subjectDataToEnroll.getLabel().c_str(), name.c_str());
...
//: _displayEnabled(false)
{
  ...
  // Tracker object, we are using a unique
  // pointer(http://en.cppreference.com/w/cpp/memory/unique_ptr) Please notice
  // that the SDM class does not have an explicit constructor. To get an
  // instance of the SDM class, please use the function SDM::getInstance
  if (_sdm == nullptr) {  // Only initialize once to avoid LicenseManager
                          // complaining about multiple instances
...
FaceTracker::Impl::~Impl() {
  // if(_sdm) delete _sdm; // points to a singleton, so don't delete, call
  // ~SDM(); _sdm->~SDM();
...
static inline TrackedFace::Feature GetFeatureHelper(
    const float* x, const float* y, std::vector<int>&& indices) {
...
static inline TrackedFace::Feature GetFeatureHelper(const float* x,
                                                    const float* y,
                                                    const int fromIndex,
                                                    const int toIndex,
                                                    bool closed = false) {
...
static inline void UpdateFaceFeatures(const cv::Mat& landmarks,
  ...
  face.SetFeature(
      ...
      GetFeatureHelper(x, y, {31, 32, 33, 34, 35, 36, 37, 45, 44, 43, 31}));

  face.SetFeature(
      TrackedFace::LowerLip,
      GetFeatureHelper(x, y, {31, 48, 47, 46, 37, 38, 39, 40, 41, 42, 31}));
...
Result FaceTracker::Impl::Update(const Vision::Image& frameOrig) {
  ...
  for (auto faceIter = _faces.begin(); faceIter != _faces.end();
    ...
    // From the landmarks in the previous frame, we predict the landmark in the
    // current iteration
  ...
  _faceCascade.detectMultiScale(frame, newFaceRects, opencvDetectScaleFactor, 2,
                                0, opencvDetectMinFaceSize);

  // TODO: See if we recognize this face

  for (auto& newFaceRect : newFaceRects) {
    // Initializing the tracker, detecting landmarks from the output of the face
    // detector
    cv::Mat landmarks;
    float score = 0.f;
  ...
  for (auto& facePair : _faces) {
    ...
    face.SetHeadOrientation(Rmat.GetAngleAroundZaxis(),
                            Rmat.GetAngleAroundXaxis(),
                            Rmat.GetAngleAroundYaxis());
    ...
    if (!_recognizableFaces.empty()) {
      // Index of the subject recornized (if it is not recognized it will be set
      // to -1)
      subject =
          doRecognition(*_fr, currentRecogData, _recognizableFaces, scores);
    }
    ...
    if (subject != -1 && scores[subject] > scoreThreshold) {
      ...
      if (false == enrollSubject(*_fr, newName, currentRecogData,
                                 _recognizedFacesPath, _recognizableFaces)) {
