...
class FaceTracker::Impl : public Profiler {
 public:
  ...
  bool HaveAllowedTrackedFaces() const {
    return !_allowedTrackedFaceID.empty();
  }
 ...
 private:
  ...
  bool DetectEyeContact(const TrackedFace& face, const TimeStamp_t& frameOrig);

  bool IsEnrollable(const DETECTION_INFO& detectionInfo,
                    const TrackedFace& face, const f32 intraEyeDist);

  void Reset();

  void SetCroppingMask(const INT32 nWidth, const INT32 nHeight,
                       const float cropFactor);

  // Setting the pose of the face uses the camera's pose as its parent
  Result SetFacePoseFromParts(const s32 nrows, const s32 ncols,
