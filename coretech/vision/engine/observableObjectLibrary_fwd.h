/**
 * File: observableObjectLibrary_fwd.h
 *
 * Author: Andrew Stein
 * Date:   8-11-2014
 *
 ...
 *        To get the implementations, you must include
 *observableObjectLibrary.h.
 *
...
class ObservableObjectLibrary {
 public:
  ...
  // Add an object to the list of known objects this library can instantiate
  // from observed markers. Note that this permits addition of new objects at
  // run time. Will return RESULT_FAIL if library already contains an object
  // using a marker present on the given object.
  ...
  // observed objects, returned in objectsSeen. The object poses will be with
  // respect to origin.
  void CreateObjectsFromMarkers(
      const std::list<ObservedMarker>& markers,
      std::vector<std::shared_ptr<ObsObjectType>>& objectsSeen) const;

  // Return a pointer to a known object with at least one of the specified
  // marker or code on it. If there is no object with that marker/code, a NULL
  // pointer is returned.
 ...
 protected:
  ...
  // Only warn once per unknown marker to avoid spamming, e.g. while staring at
  // an undefined object (whose markers haven't been registered)
  ...
  class PoseCluster {
   public:
    ...
    bool TryToAddMatch(const PoseMatchPair& match, const float distThreshold,
