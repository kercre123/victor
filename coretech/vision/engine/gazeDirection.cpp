...
CONSOLE_VAR(u32, kGazeDirectionExpireThreshold_ms, "Vision.GazeDirection",
            1000);
CONSOLE_VAR(f32, kGazeDirectionInlierXThreshold_mm, "Vision.GazeDirection",
            300.f);
CONSOLE_VAR(f32, kGazeDirectionInlierYThreshold_mm, "Vision.GazeDirection",
            100.f);
CONSOLE_VAR(f32, kGazeDirectionInlierZThreshold_mm, "Vision.GazeDirection",
            20.f);
CONSOLE_VAR(f32, kGazeDirectionShiftOutputPointX_mm, "Vision.GazeDirection",
            100.f);
// This value was chosen to be sufficiently large that the difference in the z
// coordinates of the two points used to find the intersection with the ground
// plane weren't too close as to cause numerical instabilities. 500 was too
// small.
CONSOLE_VAR(f32, kGazeDirectionSecondPointTranslationY_mm,
            "Vision.GazeDirection", 1500.f);
}  // namespace
...
bool GazeDirection::GetPointFromHeadPose(const Pose3d& headPose,
                                         Point3f& gazeDirectionPoint) {
  // Get another point in the direction of the rotation of the head pose, to
  // then find the intersection of that line with ground plane, note that this
  // point is in world coordinates "behind" the head if the pose were looking at
  // the ground plane if the y translation is positive.
  Pose3d translatedPose(0.f, Z_AXIS_3D(),
                        {0.f, kGazeDirectionSecondPointTranslationY_mm, 0.f},
                        headPose);
  ...
  if (Util::IsFltNear(translation.z(), point.z())) {
    ...
    // Alpha less than zero means that the ground plane point we found is
    // "behind" the head pose. For some reason the head pose rotation is such
    // that the face normal is pointed "above" the horizon. If alpha > 0 the
    // point is either in the line segment between our two points (0 < alpha <
    // 1) or in the ray starting at the second point and extending in the same
    // direction as the segment. This avoids finding a intersection with the
    // ground plane when face normal is directed above the horizon.
    if (FLT_GT(alpha, 1.f)) {
...
bool GazeDirection::IsStable() const {
  return ((_numberOfInliers > kGazeDirectionMinNumberOfInliers) &&
          _initialized);
}

Point3f GazeDirection::GetGazeDirectionAverage() const {
  return _gazeDirectionAverage +
         Point3f(kGazeDirectionShiftOutputPointX_mm, 0.f, 0.f);
}

Point3f GazeDirection::GetCurrentGazeDirection() const {
  return _gazeDirectionHistory[_currentIndex].point +
         Point3f(kGazeDirectionShiftOutputPointX_mm, 0.f, 0.f);
}
