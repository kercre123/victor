 ...
 * Date:   11/9/2015
 *
 * Description: Defines an RGB & RGBA pixels for color images and sets them up
 *to be understandable/usable by OpenCV. Defines HSV float pixel.
 *
...
class PixelRGB_ : public cv::Vec<Type, 3> {
 public:
  ...
  // Convert to gray (green simply gets double weight and efficient integer math
  // is used)
  Type gray() const;

  // Convert to gray using weights like OpenCV: gray = 0.299*R + 0.587*G +
  // 0.114*B
...
static_assert(sizeof(PixelRGB_<f32>) == 3 * sizeof(f32),
              "PixelRGB<f32> is not 3X sizeof float");
...
class PixelRGB565 : public cv::Vec2b {
 public:
  ...
  PixelRGB565(u8 r, u8 g, u8 b) { SetValue(r, g, b); }
  ...
  inline void SetValue(u8 r, u8 g, u8 b) {
    SetValue((u16(0xF8 & r) << 8) | (u16(0xFC & g) << 3) |
             (u16(0xF8 & b) >> 3));
  };
...
class PixelHSV : public cv::Vec3f {
 public:
  ...
  PixelHSV(const PixelRGB& rgb) : PixelHSV() { FromPixelRGB(rgb); }
  ...
  /**
   * Convert to a PixelRGB. Source:
   * https://www.cs.rit.edu/~ncs/color/t_convert.html
   */
  PixelRGB ToPixelRGB() const;

  /**
   * Convert from a PixelRGB. Source:
   * https://www.cs.rit.edu/~ncs/color/t_convert.html
   * @param rgb
   */
...
template <typename Type>
inline PixelRGB_<Type>& PixelRGB_<Type>::AlphaBlendWith(
    const PixelRGB_<Type>& other, f32 alpha) {
  r() = static_cast<u8>(alpha * static_cast<f32>(r()) +
                        (1.f - alpha) * static_cast<f32>(other.r()));
  g() = static_cast<u8>(alpha * static_cast<f32>(g()) +
                        (1.f - alpha) * static_cast<f32>(other.g()));
  b() = static_cast<u8>(alpha * static_cast<f32>(b()) +
                        (1.f - alpha) * static_cast<f32>(other.b()));
  return *this;
}
...
inline u8 PixelRGBA::gray() const { return SimpleRGB2Gray(r(), g(), b()); }
...
template <>
class DataDepth<Anki::Vision::PixelRGB_<u8> >
    : public DataDepth<DataType<Vec<u8, 3> > > {};
template <>
class DataDepth<Anki::Vision::PixelRGB_<s16> >
    : public DataDepth<DataType<Vec<s16, 3> > > {};
template <>
class DataDepth<Anki::Vision::PixelRGB_<f32> >
    : public DataDepth<DataType<Vec<f32, 3> > > {};
template <>
class DataDepth<Anki::Vision::PixelRGBA>
    : public DataDepth<DataType<Vec<u8, 4> > > {};
template <>
class DataDepth<Anki::Vision::PixelRGB565>
    : public DataDepth<DataType<Vec<u8, 2> > > {};

template <>
class DataType<Anki::Vision::PixelRGB_<u8> > : public DataType<Vec<u8, 3> > {};
template <>
class DataType<Anki::Vision::PixelRGB_<s16> > : public DataType<Vec<s16, 3> > {
};
template <>
class DataType<Anki::Vision::PixelRGB_<f32> > : public DataType<Vec<f32, 3> > {
};
template <>
class DataType<Anki::Vision::PixelRGBA> : public DataType<Vec<u8, 4> > {};
template <>
class DataType<Anki::Vision::PixelRGB565> : public DataType<Vec<u8, 2> > {};
