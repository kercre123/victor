 ...
 *              Computes and caches undistortion maps by image size on first
 *use, then uses those for subsequent calls for matching image sizes.
 *
...
struct Undistorter::MapPair {
  ...
  // CV_16SC2, CV_32FC1,
  //       or CV_32FC2
  // map2: The second map of y values having the type CV_16UC1, CV_32FC1, or
  // none (empty map if map1
...
Result Undistorter::CacheUndistortionMaps(s32 nrows, s32 ncols,
  ...
  if (mapPair == nullptr) {
    auto emplaceResult =
        _mapCache.emplace(resolutionKey, std::make_unique<MapPair>());
    DEV_ASSERT(emplaceResult.second,
               "Undistorter.CacheDistortionMaps.DidNotAddNewMapPairAsExpected");
    mapPair = emplaceResult.first->second.get();

    // This follows the same process as in cv::undistort, which is really just a
    // wrapper for cv::initUndistortRectifyMap() and cv::remap().
    ...
    cv::Mat_<double> K;
    cv::Mat(scaledCalib.GetCalibrationMatrix().get_CvMatx_())
        .convertTo(K, CV_64F);

    // NOTE: has to be double
    const cv::Mat_<double> distCoeffs(
        cv::Mat(scaledCalib.GetDistortionCoeffs()));

    try {
      cv::initUndistortRectifyMap(K, distCoeffs, I, K, cv::Size(ncols, nrows),
                                  mapPair->map1.type(), mapPair->map1,
                                  mapPair->map2);
    } catch (cv::Exception& e) {
      PRINT_NAMED_ERROR(
          "Undistorter.CacheDistortionMaps.OpenCvInitUndistortRectifyMapFailed",
          "%s", e.what());
      return RESULT_FAIL;
    }
  }
...
// - - - - - - - - - - - - - - - - - - - -
template <class T>
Result Undistorter::UndistortImageHelper(const ImageBase<T>& img,
                                         ImageBase<T>& undistortedImage) {
  if (!_calib) {
  ...
  Result cacheResult =
      CacheUndistortionMaps(img.GetNumRows(), img.GetNumCols(), mapPair);
  ...
  try {
    cv::remap(img.get_CvMat_(), undistortedImage.get_CvMat_(), mapPair->map1,
              mapPair->map2, CV_INTER_LINEAR);
  } catch (cv::Exception& e) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result Undistorter::UndistortImage(const ImageRGB& img,
                                   ImageRGB& undistortedImage) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result Undistorter::UndistortPoints(
    const std::shared_ptr<CameraCalibration>& calib, const s32 nrows,
    const s32 ncols, const std::vector<Point2f>& distortedPointsIn,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result Undistorter::UndistortPoint(
    const std::shared_ptr<CameraCalibration>& calib, const s32 nrows,
    const s32 ncols, const Point2f& distortedPointIn,
    Point2f& undistortedPointOut) {
  std::vector<Point2f>
      undistortedPoints;  // dummy vector to hold the single point
  const Result result = UndistortPoints(calib, nrows, ncols, {distortedPointIn},
                                        undistortedPoints);
  if (RESULT_OK == result) {
    if (!ANKI_VERIFY(undistortedPoints.size() == 1,
                     "Undistorter.UndistortPoint.ExpectingSinglePoint",
