...
namespace {
// After observing the charger, we clamp its pose to be 'flat' if it is within
// this tolerance of being flat already
constexpr float kClampChargerPoseAngleTol_rad = DEG_TO_RAD(5.f);
}  // namespace

template <class ObsObjectType>
const std::set<const ObsObjectType*>
    ObservableObjectLibrary<ObsObjectType>::sEmptyObjectVector;

template <class ObsObjectType>
ObservableObjectLibrary<ObsObjectType>::~ObservableObjectLibrary<
    ObsObjectType>() {}

template <class ObsObjectType>
const ObsObjectType* ObservableObjectLibrary<ObsObjectType>::GetObjectWithCode(
    const Marker::Code& code) const {
...
template <class ObsObjectType>
const ObsObjectType*
ObservableObjectLibrary<ObsObjectType>::GetObjectWithMarker(
    const Marker& marker) const {
  return GetObjectWithCode(marker.GetCode());
}

template <class ObsObjectType>
Result ObservableObjectLibrary<ObsObjectType>::AddObject(
    std::unique_ptr<const ObsObjectType>&& object) {
  // Make sure we don't have anything already using any of the markers on this
  // object Note that we don't need to worry if we find an object with the same
  // markers that also has the same type: it will be completely replaced by this
  // new object anyway. We do this first so that we can't accidentally remove an
  // existing type, _then_ discover another object (with different type) that
  // has duplicate markers, return RESULT_FAIL, and then not have actually
  // replaced the type we intended to.
  for (auto& marker : object->GetMarkers()) {
    const ObsObjectType* knownObjWithMarker = GetObjectWithMarker(marker);
    if (nullptr != knownObjWithMarker &&
        (knownObjWithMarker->GetType() != object->GetType())) {
      PRINT_NAMED_WARNING(
          "ObservableObjectsLibrary.AddObject.MarkerAlreadyInUse",
  ...
  // See if we are replacing an existing known type. If so, remove it and
  // unregister its markers.
  s32 numTypesMatched = 0;
  for (auto knownObjectIter = _knownObjects.begin();
       knownObjectIter != _knownObjects.end();) {
    const ObsObjectType* knownObject = knownObjectIter->get();
    if (knownObject->GetType() == object->GetType()) {
      PRINT_NAMED_WARNING(
          "ObservableObjectLibrary.AddObject.RemovingPreviousDefinition",
          "The old definition for a %s object was erased from the library.",
          EnumToString(object->GetType()));

      // Since each Marker can only be bound to one Object, clear out the
      // entries for the matching known object we are about to replace
  ...
  // It should never be possible to have more than one object of the same type
  // in the library, so we can't have matched more than one
  DEV_ASSERT(numTypesMatched <= 1,
             "ObservableObjectLibrary.AddObject.MultipleTypeMatchesFound");

  // Actually add the object to our known objects and register each marker it
  // has on it
  for (auto& marker : object->GetMarkers()) {
...
template <class ObsObjectType>
bool ObservableObjectLibrary<ObsObjectType>::RemoveObjectWithMarker(
  ...
  if (objectWithCodeIter == _objectWithCode.end()) {
    ...
    while (knownObjectIter != _knownObjects.end()) {
      if (knownObjectIter->get() == objectToRemove) {
        ...
        // known object (not just the one passed in that we used to find the
        // object)
    ...
    DEV_ASSERT(objectFound,
               "ObservableObjectLibrary.RemoveObjectWithMarker.ObjectNotFound");
    DEV_ASSERT(_objectWithCode.find(code) == _objectWithCode.end(),
               "ObservableObjectLibrary.RemoveObjectWithMarker."
               "ObjectWithCodeEntryStillFound");
...
template <class ObsObjectType>
void ObservableObjectLibrary<ObsObjectType>::CreateObjectsFromMarkers(
    const std::list<ObservedMarker>& markers,
    std::vector<std::shared_ptr<ObsObjectType>>& objectsSeen) const {
  std::map<const ObsObjectType*, std::vector<const ObservedMarker*>>
      markersByLibObject;
  ...
  for (auto& marker : markers) {
    ...
    if (nullptr != objectWithMarker) {
      ...
      {
        PRINT_NAMED_WARNING(
            "ObservableObjectLibrary.CreateObjectsFromMarkers.UnusedMarker",
  ...
  for (auto& libObjectMarkersPair : markersByLibObject) {
    ...
    // TODO: make the distance/angle thresholds parameters or else
    // object-type-specific
    std::vector<PoseCluster> poseClusters;
    ClusterObjectPoses(possiblePoses, libObject, 5.f, 5.f * M_PI_F / 180.f,
                       poseClusters);
    ...
    for (auto& poseCluster : poseClusters) {
      ...
      // We default the PoseState to `known` right away, since we are assuming
      // that the caller wants to create an object based on a visual
      // observation, and one observation is enough to 'confirm' the object's
      // pose.
      ObsObjectType* newObject = libObject->CloneType();
      Pose3d newPose = poseCluster.GetPose().GetWithRespectToRoot();

      // If this is a charger (and hence could be used for localization), we
      // want to clamp the pose to flat in order to avoid weird localization
      // updates. This implicitly assumes that the charger is indeed sitting
      // flat on the table/surface.
      if (IsChargerType(newObject->GetType(), false)) {
        ObservableObject::ClampPoseToFlat(newPose,
                                          kClampChargerPoseAngleTol_rad);
      }

      const float obsDistance_mm =
          poseCluster.GetPose().GetTranslation().Length();
...
template <class ObsObjectType>
ObservableObjectLibrary<ObsObjectType>::PoseCluster::PoseCluster(
    const PoseMatchPair& match)
    : _pose(match.first) {
...
template <class ObsObjectType>
bool ObservableObjectLibrary<ObsObjectType>::PoseCluster::TryToAddMatch(
    const PoseMatchPair& match, const float distThreshold,
    const Radians angleThreshold, const RotationAmbiguities& R_ambiguities) {
  bool wasAdded = false;
  ...
  if (GetSize() > 1) {
    ...
    for (auto& match : _matches) {
      ...
      if (obsMarker->GetSeenBy().GetID() == camera->GetID()) {
        ...
        PRINT_NAMED_WARNING(
            "ObservableObjectLibrary.PoseCluster.RecomputePose.NotImplemented",
    ...
    if (!_pose.IsChildOf(originalParent)) {
      if (_pose.GetWithRespectTo(originalParent, _pose) == false) {
        PRINT_NAMED_ERROR(
            "ObservableObjectLibrary.PoseCluster.RecomputePose.OriginMisMatch",
...
template <class ObsObjectType>
void ObservableObjectLibrary<ObsObjectType>::ClusterObjectPoses(
    const std::vector<PoseMatchPair>& possiblePoses,
    const ObsObjectType* libObject, const float distThreshold,
  ...
  for (size_t i_pose = 0; i_pose < possiblePoses.size(); ++i_pose) {
    if (not assigned[i_pose]) {
      ...
      for (size_t i_other = (i_pose + 1); i_other < possiblePoses.size();
           ++i_other) {
