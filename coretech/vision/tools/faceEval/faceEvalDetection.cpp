...
FaceEvalDetection::FaceEvalDetection(const FileList& faceFiles,
  ...
  NativeAnkiUtilConsoleSetValueWithString(
      "DetectionMode", (_params.useStillMode ? "Still" : "Movie"));
  NativeAnkiUtilConsoleSetValueWithString(
      "DelayCount", std::to_string(_params.delayCount).c_str());
  ...
  NativeAnkiUtilConsoleSetValueWithString(
      ...
      std::to_string(_params.detectionThreshold * 1000).c_str());

  GetFaceTracker().EnableRecognition(false);
  GetFaceTracker().EnableGazeDetection(
      true);  // to force part detection, since recognition is disabled
}
...
void FaceEvalDetection::Run() {
  ...
  for (const auto& testFile : _testFiles) {
    ...
    if (RESULT_OK != result) {
      LOG_ERROR("FaceEvalDetection.Run.LoadImageFileFailed", "%s",
    ...
    if (_params.verbose) {
      LOG_INFO("FaceEvalDetection.Run.FacesFound", "%s: Found %d, Expected %d",
               ...
               testFile.filename.c_str(), numFacesFound, testFile.numFaces);
    }
    ...
    failureFlags.SetBitFlag(FailureType::MissingParts,
                            partsCount != faces.size());
  ...
  LOG_INFO("FaceEvalDetection.Run.PartDetection",
           "Found parts for %d / %d faces (%.1f%%)", numPartsFound,
           totalFacesFound, 100.f * partDetectionRate);
}
