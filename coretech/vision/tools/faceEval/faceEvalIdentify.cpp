...
void FaceEvalIdentify::Run() {
  if (_params.numRuns > 1 || _params.maxEnrollees > 0) {
    for (int iRun = 0; iRun < _params.numRuns; ++iRun) {
      LOG_INFO("FaceEvalIdentify.Run", "Run %d of %d, %d Enrollees", iRun,
               _params.numRuns, _params.maxEnrollees);
      ShuffleHelper(_dirList);
      GetFaceTracker().EraseAllFaces();

      if (_params.numRuns > 1) {
        _outSuffix = "_" + std::to_string(iRun);
        Util::FileUtils::CreateDirectory(
            Util::FileUtils::FullFilePath({GetOutputDir(), _outSuffix}));
      }
      RunInternal();
    }
  } else {
...
void FaceEvalIdentify::RunInternal() {
  ...
  for (const auto& dir : _dirList) {
    ...
    const bool shouldEnrollDir =
        (_params.maxEnrollees < 0) || (faceID < _params.maxEnrollees);
    if (shouldEnrollDir && ReadEnrollList(dir, filesToEnroll)) {
      // Got enrollment files. Use them to create an album entry for this
      // directory, by finding faces in the specified enrollment files and
      // adding them to the face tracker
      const int numAdded = EnrollFace(filesToEnroll, faceID);
      if (numAdded > 0) {
        // Add all other image files not on the enroll list to the test set
        auto newFiles =
            Util::FileUtils::FilesInDirectory(dir, true, kImageFileExt);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool FaceEvalIdentify::ReadEnrollList(const std::string& dir,
                                      std::set<std::string>& filesToEnroll) {
  filesToEnroll.clear();
  const std::string& enrollListFile =
      Util::FileUtils::FullFilePath({dir, kEnrollListFilename});
  ...
  if (hasEnrollList) {
    ...
    if (!file) {
      LOG_ERROR("EvalFaceIdentity.ReadEnrollList.FileNotFound", "%s",
                enrollListFile.c_str());
      return false;
    }
    ...
    while (std::getline(file, line)) {
      ...
      if (Util::FileUtils::FileExists(filename)) {
        ...
        LOG_WARNING("EvalFaceIdentity.ReadEnrollList.MissingEnrollmentFile",
                    "%s", filename.c_str());
      }
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
int FaceEvalIdentify::EnrollFace(const std::set<std::string>& enrollFiles,
                                 const int faceID) {
  ...
  for (const auto& file : enrollFiles) {
    ...
    if (RESULT_OK != result) {
      LOG_ERROR("FaceEvalIdentify.EnrollFace.LoadImageFileFailed", "%s",
                file.c_str());
      continue;
    }
    ...
    if (faces.size() > 1) {
      LOG_WARNING("FaceEvalIdentify.EnrollFace.UsingFirstFace", "%zu found",
                  faces.size());
    }
    ...
    if (RESULT_OK != result) {
      LOG_WARNING("FaceEvalIdentify.EnrollFace.CouldNotAddToAlbum",
                  "ID:%d File:%s", faceID, file.c_str());
      continue;
    }
#endif
    ...
    if (_params.saveImages) {
      ...
      img.Save(Util::FileUtils::FullFilePath(
          {GetOutputDir(), _outSuffix, filename}));
    }

    ++numAdded;

    if (numAdded >= _params.maxEnrollmentsPerPerson) {
      LOG_INFO("FaceEvalIdentify.EnrollFace.MaxEnrollments",
               "FaceID:%d maxed out at %d enrollments", faceID, numAdded);
      break;
    }
  }
...
void FaceEvalIdentify::TestIdentification() {
  ...
  for (const auto& testFile : _testFileList) {
    ...
    Result result = img.Load(file);
    if (RESULT_OK != result) {
      LOG_ERROR("FaceEvalIdentify.TestIdentification.LoadImageFileFailed", "%s",
                file.c_str());
      continue;
    }
    ...
    for (const auto& face : faces) {
      ...
      const Result findResult =
          faceTracker.DevFindFaceInAlbum(img, face, 3, matches);
      if (RESULT_OK != findResult || matches.empty()) {
        LOG_ERROR("FaceEvalIdentify.TestIdentification.FindFaceInAlbumFailed",
                  "%s", file.c_str());
        continue;
      }
#endif

      // const float verifyScore =
      // faceTracker.ComputePairwiseMatchScore(matchedID, img, face);
      ...
      const bool aboveHighThreshold =
          Util::IsFltGT(score, _params.recognitionThreshold);
      const bool aboveLowThresholdWithMargin =
          (Util::IsFltGT(score, _params.recognitionThreshold2) &&
           Util::IsFltGT(score - score2, _params.recognitionMargin));

      testResults.emplace_back(
          TestResult{score, score2, (matchedID == expectedID)});
      ...
      if (expectedID >= 0) {
        ...
        if (!aboveHighThreshold) {
          ...
          if (_params.saveImages) {
            saveFilename =
                "FalseNegative_" + std::to_string(numFacesChecked) + ".jpg";
          }
        }
      }
      ...
      if (expectedID != matchedID) {
        if (aboveHighThreshold) {
          ...
          if (_params.saveImages) {
            saveFilename =
                "FalsePositive_" + std::to_string(numFacesChecked) + ".jpg";
          }
        }
        if (aboveLowThresholdWithMargin) {
          ++numFalsePos2;
      ...
      const std::string displayStr =
          ("True:" + std::to_string(expectedID) +
           " Found:" + std::to_string(matchedID) + " Score:" + scoreStr);  // +
      //" VerifyScore:" + std::to_string((int)std::round(1000.f*verifyScore)));

      if (!_outSuffix.empty() && !saveFilename.empty()) {
        saveFilename =
            Util::FileUtils::FullFilePath({_outSuffix, saveFilename});
      }
      DrawFaces(img, {face}, failureFlags, displayStr, 0, "Identification",
                saveFilename);
    }
  }
  ...
  LOG_INFO(
      "FaceEvalIdentify.TestIdentification.FalsePositives",
      "%d / %d (%.1f%%) vs. %d / %d (%.1f%%) without 2nd threshold and margin",
      numFalsePos, numFacesChecked, 100.f * falsePosRate, numFalsePos2,
      numFacesChecked, 100.f * ((float)numFalsePos2 / (float)numFacesChecked));
  LOG_INFO(
      "FaceEvalIdentify.TestIdentification.FalseNegatives",
      "%d / %d (%.1f%%) vs. %d / %d (%.1f%%) without 2nd threshold and margin",
      numFalseNeg, numKnownFacesChecked, 100.f * falseNegRate, numFalseNeg2,
      numKnownFacesChecked,
  ...
  // Save results to file
  {
    ...
    const std::string& outFile =
        Util::FileUtils::FullFilePath({GetOutputDir(), _outSuffix, baseName});
    file.open(outFile, std::ios::out | std::ofstream::binary);
    if (file.is_open()) {
      for (const auto& result : testResults) {
        file << result.score << ", " << result.score2 << ", "
             << result.groundTruth << std::endl;
      }
      file.close();
    }
  }
