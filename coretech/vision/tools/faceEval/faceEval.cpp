 ...
 * Date:   12/1/2018
 *
 * Description: The "main" file to run Face Detection/Identification/Confusion
 *tests. Mostly handles common argument parsing and configuration tasks.
 *
...
static void GetImageFilesInDir(const std::string& rootDir,
  ...
  std::list<std::string> subDirs{"."};
  {
    ...
    std::copy(tempSubDirs.begin(), tempSubDirs.end(),
  ...
  for (const auto& subDir : subDirs) {
    if (subDir != ".") {
      ...
      Util::FileUtils::ListAllDirectories(
          Util::FileUtils::FullFilePath({rootDir, subDir}), subSubDirs);
      for (const auto& subSubDir : subSubDirs) {
        subDirs.push_back(Util::FileUtils::FullFilePath({subDir, subSubDir}));
      }
    }

    std::vector<std::string> files = Util::FileUtils::FilesInDirectory(
        Util::FileUtils::FullFilePath({rootDir, subDir}), kUseFullPath,
        imageFileExts);
...
static Result GetImageFiles(const std::string& rootDir,
  ...
  const bool sampleFilesInSubDirs =
      argParser.GetArg("--maxFilesPerSubDir", maxFilesPerSubDir);

  std::string specificFile;
  if (argParser.GetArg("--filename", specificFile)) {
    const std::string& fullPath =
        Util::FileUtils::FullFilePath({rootDir, specificFile});
    if (GetExtension(fullPath) == "txt") {
      ...
      if (file.is_open()) {
        ...
        while (std::getline(file, line)) {
          const std::string& lineFullPath =
              Util::FileUtils::FullFilePath({rootDir, line});
          if (Util::FileUtils::DirectoryExists(lineFullPath)) {
            GetImageFilesInDir(lineFullPath, filesWithFaces,
                               sampleFilesInSubDirs, maxFilesPerSubDir);
          } else {
      ...
      if (Util::FileUtils::DirectoryExists(fullPath)) {
        GetImageFilesInDir(fullPath, filesWithFaces, sampleFilesInSubDirs,
                           maxFilesPerSubDir);
      } else {
    ...
    GetImageFilesInDir(rootDir, filesWithFaces, sampleFilesInSubDirs,
                       maxFilesPerSubDir);

    std::string backgroundDir =
        "resources/test/markerDetectionTests/NoMarkers";  // assumes running
                                                          // from victor base
                                                          // dir
    ...
    if (!backgroundDir.empty()) {
      if (ANKI_VERIFY(Util::FileUtils::DirectoryExists(backgroundDir),
                      "FaceEval.GetImageFiles.BadBackgroundDir", "%s",
                      backgroundDir.c_str())) {
        backgrounds = Util::FileUtils::FilesInDirectory(
            backgroundDir, kUseFullPath, imageFileExts);
      }
    }
    ...
    if (maxTotalFiles > 0) {
      if (filesWithFaces.size() > (size_t)maxTotalFiles) {
        filesWithFaces.erase(filesWithFaces.begin() + maxTotalFiles,
                             filesWithFaces.end());
      }
      if (backgrounds.size() > (size_t)maxTotalFiles) {
        backgrounds.erase(backgrounds.begin() + maxTotalFiles,
                          backgrounds.end());
      }
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  if (doDetection || doConfusion) {
    const Result result =
        GetImageFiles(rootDir, argParser, filesWithFaces, backgrounds);
    if (RESULT_OK != result) {
      return -1;
    }
  ...
  if (doDetection) {
    Vision::FaceEvalDetection evalDetection(filesWithFaces, backgrounds,
                                            argParser);
    evalDetection.Run();
  }
  ...
  if (doIdentification) {
    ...
    for (const auto& tempSubDir : tempSubDirs) {
      const std::string& fullPath =
          Util::FileUtils::FullFilePath({rootDir, tempSubDir});
