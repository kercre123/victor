...
FaceEvalConfusion::FaceEvalConfusion(const FileList& files,
    ...
    : IFaceEval(argParser), _fileList(files) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void FaceEvalConfusion::DisplayHelper(FaceID_t faceID,
                                      const EnrolledFace& enrolledFace,
                                      float score, const char* windowName,
                                      int pauseTime) const {
  ImageRGB img;
  ...
  if (score >= 0.f) {
    dispStr << "[" << std::fixed << std::setprecision(2) << score << " vs. "
            << _params.recognitionThreshold << "]";
  }
#if ANKI_DEVELOPER_CODE
  img.DrawText({1, img.GetNumRows() - 1}, dispStr.str(), NamedColors::RED,
               0.4f);
#endif
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void FaceEvalConfusion::ShowTiledImage(
    const std::map<int, EnrolledFace>& enrolledFaces) const {
#if ANKI_DEVELOPER_CODE
  ...
  for (const auto& enrolledFace : enrolledFaces) {
    identityToIDs[enrolledFace.second.identity].emplace_back(
        enrolledFace.first);
  }
  ...
  for (const auto& identityToID : identityToIDs) {
    ...
    for (const int ID : identityToID.second) {
      ...
      Rectangle<s32> roiRect(col * kTileWidth, row * kTileHeight, kTileWidth,
                             kTileHeight);
      ++col;
      ...
      tiledImg.DrawRect(roiRect,
                        (IsEnrollable(enrolledFace.face) ? NamedColors::GREEN
                                                         : NamedColors::RED),
                        3);
    }
...
void FaceEvalConfusion::Run() {
  ...
  for (const auto& file : _fileList) {
    ...
    if (RESULT_OK != result) {
      LOG_ERROR("FaceEvalConfusion.Run.LoadImageFileFailed", "%s",
                file.c_str());
      continue;
    }
    ...
    for (const auto& face : faces) {
      ...
      if (!face.HasEyes()) {
        LOG_WARNING("FaceEvalConfusion.Run.NoEyesFound",
                    "Identifier:%s File:%s", idString.c_str(), file.c_str());
      } else {
        // TODO this was to quickly fix a build, there must be a better way
#if ANKI_DEVELOPER_CODE
        const Result addResult =
            faceTracker.DevAddFaceToAlbum(img, face, faceID);
        if (RESULT_OK != addResult) {
          LOG_WARNING("FaceEvalConfusion.Run.CouldNotAddToAlbum",
                      "Identifier:%s File:%s", idString.c_str(), file.c_str());
          continue;
        }
#endif

        auto result = faceIDtoEnrolledFace.emplace(
            faceID,
            EnrolledFace{.identity = idString, .file = file, .face = face});

        if (!result.second) {
          LOG_ERROR("FaceEvalConfusion.Run.DuplicateEntry",
                    "ID:%d Identifier:%s", faceID, idString.c_str());
          continue;
        }
  ...
  LOG_INFO("FaceEvalConfusion.Run.AddedFaceIDs", "Unique Faces: %zu",
           faceIDtoEnrolledFace.size());
  ...
  for (auto iter1 = faceIDtoEnrolledFace.begin();
       iter1 != faceIDtoEnrolledFace.end(); ++iter1) {
    ...
    for (auto iter2 = iter1; iter2 != faceIDtoEnrolledFace.end(); ++iter2) {
      ...
      if (isSamePerson) {
        ...
        if (score < _params.recognitionThreshold) {
          ++numFalseNeg;
          if (DisplayMode::All == GetDisplayMode() ||
              DisplayMode::FalseNegatives == GetDisplayMode()) {
        ...
        if (score >= _params.recognitionThreshold) {
          ++numFalsePos;
          if (DisplayMode::All == GetDisplayMode() ||
              DisplayMode::FalsePositives == GetDisplayMode()) {
  ...
  const float falsePosRate =
      (float)numFalsePos / (float)totalInterPersonComparisons;
  const float falseNegRate =
      (float)numFalseNeg / (float)totalIntraPersonComparisons;
  ...
  {
    std::ofstream file;
    const std::string& outFile = Util::FileUtils::FullFilePath(
        {GetOutputDir(), "confusion_results.txt"});
