 ...
 * Date:   12/1/2018
 *
 * Description: Evaluates pairwise face "confusion": The match score between all
 *pairs of faces is evaluated. This is a more direct evaluation of pairwise face
 *discrimination, without the complexities of enrollment and there being
 *multiple faces (and multiple images of each of those faces) in a database.
 *
...
class FaceEvalConfusion : public IFaceEval {
 public:
  FaceEvalConfusion(const FileList& files, const SimpleArgParser& argParser);
 ...
 private:
  ...
  void DisplayHelper(FaceID_t faceID, const EnrolledFace& filename, float score,
                     const char* windowName, int pauseTime = 5) const;
