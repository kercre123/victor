...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Specialization for private type
// DisplayMode
template <>
IFaceEval::DisplayMode SimpleArgParser::GetArgHelper<IFaceEval::DisplayMode>(
  ...
  if (iter == LUT.end()) {
    LOG_ERROR("IFaceEval.GetArgHelper.BadDisplayModeValue", "%s",
              value.c_str());
...
IFaceEval::IFaceEval(const SimpleArgParser& argParser) {
  const std::vector<f32> distortionCoeffs = {
      {-0.07167206757206086, -0.2198782133395603, 0.001435740245449692,
       0.001523365725052927, 0.1341471670512819, 0, 0, 0}};
  auto calib = std::make_shared<CameraCalibration>(
      360, 640, 362.8773099149878, 366.7347434532929, 302.2888225643724,
  ...
  if (!Util::FileUtils::DirectoryExists(_params.outputDir)) {
    ...
    if (!success) {
      LOG_ERROR("IFaceEval.Constructor.CreateOutputDirFailed", "%s",
                _params.outputDir.c_str());
    }
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Needed b/c of use of
// unique_ptr<FaceTracker> w/ forward declaration
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void IFaceEval::DrawFaceOnImage(const TrackedFace& face,
                                Vision::ImageRGB& img) {
...
void IFaceEval::DrawFaces(const Image& img, const std::list<TrackedFace>& faces,
                          ...
                          const std::string& displayStr, int pauseTime_ms,
  ...
  if (doDisplay || doSave) {
    ...
    if (!displayStr.empty()) {
      _dispImg.DrawText({1, _dispImg.GetNumRows() - 1}, displayStr,
    ...
    if (doSave) {
      _dispImg.Save(
          Util::FileUtils::FullFilePath({GetOutputDir(), saveFilename}));
    }
  }
}
...
void IFaceEval::ShuffleHelper(std::vector<std::string>& strList) {
  ...
  static std::mt19937 randomEngine(
      (_params.randomSeed >= 0 ? _params.randomSeed : randomDevice()));
