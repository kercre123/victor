 ...
 * Date:   3/25/2019
 *
 * Description: Program to run debayering, compare to reference images, and
...
int main(int argc, char** argv) {
  ...
  Debayer::InArgs inArgs(inBytes.data(), config.input.height,
                         config.input.width, config.input.layout,
  ...
  for (uint32_t ii = 0; ii < config.output.images.size(); ++ii) {
    ...
    const uint32_t channels =
        OutputFormatToChannels(config.output.images[ii].format);
    const uint32_t size = channels * width * height;
    std::vector<uint8_t> outBytes(size);

    Debayer::OutArgs outArgs(
        outBytes.data(), inArgs.height * scale, inArgs.width * scale,
        config.output.images[ii].scale, config.output.images[ii].format);
    ...
    if (result != Anki::Result::RESULT_OK) {
      std::cout << "Error debayering " << config.output.images[ii].filename
                << std::endl;
    }

    if (config.output.save) {
      std::string path = FileUtils::FullFilePath(
          {config.output.directory, config.output.images[ii].filename});
      std::ofstream ofs(path, std::ios::out | std::ios::binary);
      ofs.write(reinterpret_cast<const char*>(outBytes.data()),
                outBytes.size());
    }

    if (config.compare.enable) {
      std::string path = FileUtils::FullFilePath(
          {config.compare.directory, config.output.images[ii].filename});
      std::ifstream stream(path);
      std::vector<uint8_t> refBytes(std::istreambuf_iterator<char>(stream), {});

      // This is a lazy comparison of bytes. We assume that the output bytes
      // will exactly match the reference bytes. In the future it may be worth
      // allowing for some amount of error between the output bytes and
      // reference bytes.
      if (outBytes != refBytes) {
        std::cout << "Error comparing " << config.output.images[ii].filename
                  << std::endl;
      }
    }
  }
