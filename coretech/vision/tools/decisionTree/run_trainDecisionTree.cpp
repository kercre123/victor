...
FixedLengthList<Type> LoadIntoList_temporaryBuffer(
    const char *filenamePrefix, const char *filenameSuffix,
    MemoryStack scratch1, MemoryStack scratch2, void *allocatedBuffer,
    const s32 allocatedBufferLength);
std::vector<U8Bool> LoadIntoList_grayvalueBool(const char *filenamePrefix,
                                               const char *filenameSuffix,
                                               MemoryStack scratch1,
                                               MemoryStack scratch2);
template <typename Type>
FixedLengthList<Type> LoadIntoList_permanentBuffer(const char *filenamePrefix,
                                                   const char *filenameSuffix,
                                                   MemoryStack scratch,
                                                   MemoryStack &memory);
template <typename Type>
Result SaveList(const FixedLengthList<Type> &in, const char *filenamePrefix,
                const char *filenameSuffix, MemoryStack scratch);
...
FixedLengthList<Type> LoadIntoList_temporaryBuffer(
  ...
  snprintf(filenameBuffer, filenameBufferLength, "%s%s", filenamePrefix,
           filenameSuffix);

  Array<Type> raw = Array<Type>::LoadBinary(filenameBuffer, allocatedBuffer,
                                            allocatedBufferLength);

  AnkiConditionalErrorAndReturnValue(
      raw.get_size(0) == 1, FixedLengthList<Type>(),
      "LoadIntoList_permanentBuffer", "Input is the wrong size");

  const s32 numElements = raw.get_size(0) * raw.get_size(1);

  FixedLengthList<Type> out = FixedLengthList<Type>(
      numElements, raw.Pointer(0, 0), allocatedBufferLength,
      Flags::Buffer(true, false, true));

  AnkiConditionalErrorAndReturnValue(
      AreValid(raw, out), FixedLengthList<Type>(),
      "LoadIntoList_permanentBuffer", "Could not allocate");
...
std::vector<U8Bool> LoadIntoList_grayvalueBool(const char *filenamePrefix,
  ...
  AnkiConditionalErrorAndReturnValue(
      raw.IsValid(), out, "LoadIntoList_grayvalueBool", "Could not allocate");
...
FixedLengthList<Type> LoadIntoList_permanentBuffer(const char *filenamePrefix,
  ...
  FixedLengthList<Type> out = FixedLengthList<Type>(
      numElements, memory, Flags::Buffer(true, false, true));
...
void PrintUsage() {
  CoreTechPrint(
      "Usage: run_trainDecisionTree <inFilenamePrefix> <numFeatures> "
      "<leafNodeFraction> <leafNodeNumItems> <u8MinDistanceForSplits> "
      "<u8MinDistanceFromThreshold> <maxThreads> <outFilenamePrefix>\n"
      "Example: run_trainDecisionTree c:/tmp/treeTraining_ 900 1.0 1 20 40 8 "
      "c:/tmp/treeTrainingOut_");
}
...
int main(int argc, const char *argv[]) {
  ...
  AnkiConditionalErrorAndReturnValue(AreValid(memory), -1,
                                     "run_trainDecisionTree", "Out of memory");

  std::vector<U8Bool> featuresUsed;
  FixedLengthList<const char *> labelNames;
  FixedLengthList<s32> labels;
  FixedLengthList<FixedLengthList<u8> > featureValues(
      numFeatures, memory, Flags::Buffer(true, false, true));
  FixedLengthList<u8> u8ThresholdsToUse;

  AnkiConditionalErrorAndReturnValue(AreValid(featureValues), -5,
                                     "run_trainDecisionTree", "Invalid input");
  ...
  // Load all inputs
  {
    ...
    AnkiConditionalErrorAndReturnValue(AreValid(scratch1, scratch2), -1,
    ...
    s32 numImages;
    {
      ...
      labels = LoadIntoList_temporaryBuffer<s32>(
          inFilenamePrefix, "labels.array", scratch1, scratch2, tmpBigBuffer,
          tmpBigBufferLength);
    ...
    featuresUsed = LoadIntoList_grayvalueBool(
        inFilenamePrefix, "featuresUsed.array", scratch1, scratch2);

    labelNames = LoadIntoList_permanentBuffer<const char *>(
        inFilenamePrefix, "labelNames.array", scratch1, memory);

    const s32 labelsBufferSize =
        numImages * sizeof(s32) + s32(1e4) + 2 * MEMORY_ALIGNMENT;
    bufferPointers.push_back(calloc(labelsBufferSize, 1));
    labels = LoadIntoList_temporaryBuffer<s32>(
        inFilenamePrefix, "labels.array", scratch1, scratch2,
        bufferPointers[bufferPointers.size() - 1], labelsBufferSize);

    const s32 u8ThresholdsToUseBufferSize = s32(1e6);
    bufferPointers.push_back(calloc(u8ThresholdsToUseBufferSize, 1));
    u8ThresholdsToUse = LoadIntoList_temporaryBuffer<u8>(
        inFilenamePrefix, "u8ThresholdsToUse.array", scratch1, scratch2,
        bufferPointers[bufferPointers.size() - 1], u8ThresholdsToUseBufferSize);

    f64 t0 = GetTimeF64();
    const s32 featureValuesBufferSize =
        numImages + s32(1e4) + 2 * MEMORY_ALIGNMENT;
    for (s32 iFeature = 0; iFeature < numFeatures; iFeature++) {
      bufferPointers.push_back(calloc(featureValuesBufferSize, 1));

      const s32 filenameBufferLength = 1024;
      char filenameBuffer[filenameBufferLength];
      snprintf(filenameBuffer, filenameBufferLength, "featureValues%d.array",
               iFeature);
      featureValues[iFeature] = LoadIntoList_temporaryBuffer<u8>(
          inFilenamePrefix, filenameBuffer, scratch1, scratch2,
          bufferPointers[bufferPointers.size() - 1], featureValuesBufferSize);
  ...
  for (s32 i = 0; i < numFeatures; i++) {
    AnkiConditionalErrorAndReturnValue(
        labels.get_size() == featureValues[i].get_size(), -2,
        "run_trainDecisionTree", "Invalid input");
  }

  // Add a const qualifier
  FixedLengthList<const FixedLengthList<u8> > featureValuesConst =
      *reinterpret_cast<FixedLengthList<const FixedLengthList<u8> > *>(
          &featureValues);

  ThreadSafeFixedLengthList<DecisionTreeNode> decisionTree(MAX_TREE_NODES,
                                                           memory);
  std::vector<f32> cpuUsage;

  AnkiConditionalErrorAndReturnValue(AreValid(decisionTree), -1,
                                     "run_trainDecisionTree", "Out of memory");

  const Result result =
      BuildTree(featuresUsed, labelNames, labels, featureValuesConst,
                leafNodeFraction, leafNodeNumItems, u8MinDistanceForSplits,
                u8MinDistanceFromThreshold, u8ThresholdsToUse, maxThreads,
                benchmarkSampleEveryNSeconds, cpuUsage, decisionTree);
  ...
  // Save the output
  {
    const FixedLengthList<DecisionTreeNode> unsafeDecisionTree =
        decisionTree.get_buffer();
    const s32 numNodes = unsafeDecisionTree.get_size();
    const s32 saveBufferSize =
        10000000 + 3 * numNodes * sizeof(DecisionTreeNode);

    MemoryStack scratch(malloc(saveBufferSize), saveBufferSize);

    AnkiConditionalErrorAndReturnValue(scratch.IsValid(), -7,
                                       "run_trainDecisionTree",
                                       "Out of memory for saving");
    ...
    FixedLengthList<s32> leftChildIndexs(numNodes, scratch,
                                         Flags::Buffer(true, false, true));
    FixedLengthList<f32> cpuUsageSamples(cpuUsage.size(), scratch,
                                         Flags::Buffer(true, false, true));

    AnkiConditionalErrorAndReturnValue(
        AreValid(depths, bestEntropys, whichFeatures, u8Thresholds,
                 leftChildIndexs, cpuUsageSamples),
    ...
    SaveList(leftChildIndexs, outFilenamePrefix, "leftChildIndexs.array",
             scratch);
    ...
    SaveList(cpuUsageSamples, outFilenamePrefix, "cpuUsageSamples.array",
             scratch);
  ...
  CoreTechPrint("Tree training took %f seconds. Tree is %d nodes.\n",
                time1 - time0,
                static_cast<s32>(decisionTree.get_buffer().get_size()));
