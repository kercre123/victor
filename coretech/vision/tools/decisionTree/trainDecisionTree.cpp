...
const s32 MIN_IMAGES_PER_THREAD =
    10;  // If the number of images left is below this, one one thread will be
...
// A list of UniqueCounts holds the number of instances of each value, like an
// integer histogram
typedef struct {
  s32 value;
  s32 count;
...
// Hold an example of a training failure, where the tree couldn't split some
// examples with different labels
typedef struct TrainingFailure {
  s32 nodeIndex;
  vector<UniqueCount> labels;

  TrainingFailure() : nodeIndex(-1) {}
} TrainingFailure;

template <typename Type>
RawBuffer<Type>::RawBuffer() : buffer(NULL), size(0) {}

template <typename Type>
RawBuffer<Type>::RawBuffer(s32 size) {
  AnkiAssert(size > 0);
...
template <typename Type>
void RawBuffer<Type>::Clone(const RawBuffer &in) {
...
typedef struct DecisionTreeWorkItem {
  ...
  DecisionTreeWorkItem(s32 treeIndex, const RawBuffer<s32> &remaining,
                       const RawBuffer<U8Bool> &featuresUsed)
      : treeIndex(treeIndex),
        remaining(remaining),
...
typedef struct BuildTreeThreadParameters {
  ...
  const FixedLengthList<const FixedLengthList<u8> >
      &featureValues;  //< For every feature, the value for the feature every
                       //item to train on (outer is small, inner is very large)
  const f32
      leafNodeFraction;  //< What percent of the items in a node have to be the
                         //same for it to be considered a leaf node? From
                         //[0.0, 1.0], where 1.0 is a good value.
  const s32 leafNodeNumItems;  //< If the number of items in a node is equal or
                               //below this, it is a leaf. 1 is a good value.
  const s32 u8MinDistanceForSplits;  //< How close can two grayvalues be to be a
                                     //threshold? 100 is a good value.
  const s32
  ...
  const FixedLengthList<u8>
      &u8ThresholdsToUse;  //< If not empty, this is the list of grayvalue
                           //thresholds to use
  const s32
      maxSecondaryThreads;  //< If we are building the start of the tree, use
                            //one primary thread, and maxSecondaryThreads
                            //threads to compute the information fain
  const std::vector<s32 *>
      &pNumLTs;  //< One allocated buffer for each maxSecondaryThreads. Must be
                 //allocated before calling the thread, and manually freed after
                 //the thread is complete
  const std::vector<s32 *>
      &pNumGEs;  //< One allocated buffer for each maxSecondaryThreads. Must be
                 //allocated before calling the thread, and manually freed after
                 //the thread is complete
  const s32 maxLabel;           //< What is the max label in labels?
  const s32 threadId;           //< What is the id of the thread?
  const s32 maxNodesToProcess;  //< How many nodes should this thread process?
                                //Set to -1 to process until the queue is empty.
  ThreadSafeQueue<DecisionTreeWorkItem>
      &workQueue;  //< What work needs to be done? Starts with just the root
                   //node.
  ThreadSafeVector<TrainingFailure>
      &trainingFailures;  //< Which nodes could not be trained?
  ThreadSafeFixedLengthList<DecisionTreeNode>
      &decisionTree;  //< The output decision tree
  ThreadSafeCounter<s32>
      &numCompleted;  //< How many images have been assigned to a leaf node? If
                      //it is equal to labels.get_size(), then we're finished.
  ...
  BuildTreeThreadParameters(
      ...
      const f32 leafNodeFraction, const s32 leafNodeNumItems,
      const s32 u8MinDistanceForSplits, const s32 u8MinDistanceFromThreshold,
      const FixedLengthList<u8> &u8ThresholdsToUse,
      const s32 maxSecondaryThreads, const std::vector<s32 *> &pNumLTs,
      const std::vector<s32 *> &pNumGEs, const s32 maxLabel, const s32 threadId,
      ...
      : featuresUsed(featuresUsed),
        ...
        leafNodeFraction(leafNodeFraction),
        leafNodeNumItems(leafNodeNumItems),
        ...
        maxSecondaryThreads(maxSecondaryThreads),
        pNumLTs(pNumLTs),
        pNumGEs(pNumGEs),
        maxLabel(maxLabel),
        threadId(threadId),
        maxNodesToProcess(maxNodesToProcess),
        workQueue(workQueue),
        trainingFailures(trainingFailures),
...
typedef struct BenchmarkingParameters {
  ...
  BenchmarkingParameters(volatile bool *isRunning,
                         const f64 sampleEveryNSeconds,
                         std::vector<f32> &cpuUsage,
                         ThreadSafeCounter<s32> &numCompleted,
                         const s32 numTotal,
                         const ThreadSafeQueue<DecisionTreeWorkItem> &workQueue)
      : isRunning(isRunning),
        sampleEveryNSeconds(sampleEveryNSeconds),
        cpuUsage(cpuUsage),
        numCompleted(numCompleted),
        numTotal(numTotal),
        workQueue(workQueue) {}
...
template <typename Type>
void maxAndMin(const Type *restrict pArray, const s32 *restrict pRemaining,
               const s32 numRemaining, Type &minValue, Type &maxValue);

vector<UniqueCount> findUnique(const s32 *restrict pArray,
                               const s32 *restrict pRemaining,
                               const s32 numRemaining, const s32 minValue,
                               const s32 maxValue);
...
ThreadResult BuildTreeThread(void *voidBuildTreeParams) {
  BuildTreeThreadParameters *restrict buildTreeParams =
      reinterpret_cast<BuildTreeThreadParameters *>(voidBuildTreeParams);
  ...
  //
  // Keep popping items from the work queue, until all images are assigned to a
  // leaf node
  //
  ...
  //       buildTreeParams->labels.get_size() images at leaves. This means that
  //       some threads may exit early.
  while (true) {
    ...
    if (buildTreeParams->workQueue.Size_unsafe() == 0) {
      ...
      // If the queue is empty, and all images are at a leaf, exit the loop and
      // the thread. NOTE: numCompleted may be a low estimate
      if (buildTreeParams->numCompleted.Get() >=
          buildTreeParams->labels.get_size())
        break;
    ...
    if ((buildTreeParams->maxNodesToProcess > 0) &&
        (numNodesProcessed > (buildTreeParams->maxNodesToProcess - 1))) {
      buildTreeParams->workQueue.Unlock();  // Unlock workQueue
      break;
    }
    ...
    const s32 numThreadsToUse =
        MAX(1, MIN(buildTreeParams->maxSecondaryThreads,
                   workItem.remaining.size / MIN_IMAGES_PER_THREAD));
    ...
    CoreTechPrint("Thread %d: %d secondary threads for %d items\n",
                  buildTreeParams->threadId, numThreadsToUse,
                  workItem.remaining.size);
#endif

    // WARNING: no other thread should write to this node, but this allows
    // non-thread-safe access
    DecisionTreeNode &curNode =
        buildTreeParams->decisionTree.get_buffer()[workItem.treeIndex];
    ...
    s32 minLabel;
    s32 maxLabel;
    maxAndMin<s32>(buildTreeParams->labels.Pointer(0),
                   workItem.remaining.buffer, workItem.remaining.size, minLabel,
                   maxLabel);

    vector<UniqueCount> uniqueLabelCounts = findUnique(
        buildTreeParams->labels.Pointer(0), workItem.remaining.buffer,
        workItem.remaining.size, minLabel, maxLabel);
    ...
    {
      const s32 numGrayvalueThresholdsToUse =
          buildTreeParams->u8ThresholdsToUse.get_size();
      ...
      for (s32 iFeature = 0; iFeature < numFeatures; iFeature++) {
        ...
        if (numGrayvalueThresholdsToUse > 0) {
          const u8 *restrict pGrayvalueThresholdsToUse =
              buildTreeParams->u8ThresholdsToUse.Pointer(0);

          for (s32 iGray = 0; iGray < numGrayvalueThresholdsToUse; iGray++) {
            if (pFeaturesUsed[iFeature]
                    .values[pGrayvalueThresholdsToUse[iGray]] == 0) {
    ...
    if ((mostCommonLabel_count >=
         saturate_cast<s32>(buildTreeParams->leafNodeFraction *
                            workItem.remaining.size)) ||
        (static_cast<s32>(workItem.remaining.size) <
         buildTreeParams->leafNodeNumItems)) {
...
#ifdef PRINT_INTERMEDIATE
      Anki::CoreTechPrint(
          "Thread %d: LeafNode for label = %d, or \"%s\" at depth %d\n",
          buildTreeParams->threadId, mostCommonLabel,
          buildTreeParams->labelNames[mostCommonLabel], curNode.depth);
#endif
...
#ifdef PRINT_INTERMEDIATE
      Anki::CoreTechPrint(
          "Thread %d: MaxDepth LeafNode for label = %d, or \"%s\" at depth "
          "%d\n",
          buildTreeParams->threadId, mostCommonLabel,
          buildTreeParams->labelNames[mostCommonLabel], curNode.depth);
#endif
    ...
    if (numThreadsToUse == 1) {
      ...
      ComputeInfoGainParameters newParameters(
          buildTreeParams->featureValues, buildTreeParams->labels,
          buildTreeParams->u8ThresholdsToUse,
          buildTreeParams->u8MinDistanceForSplits,
          buildTreeParams->u8MinDistanceFromThreshold, workItem.remaining,
          minFeatureToCheck, maxFeatureToCheck, workItem.featuresUsed,
          buildTreeParams->pNumLTs[0], buildTreeParams->pNumGEs[0]);
      ...
      const s32 numFeaturesPerThread =
          (numFeatures + numThreadsToUse - 1) / numThreadsToUse;
      ...
      for (s32 iThread = 0; iThread < numThreadsToUse; iThread++) {
        computeInfoGainParams[iThread] = new ComputeInfoGainParameters(
            ...
            workItem.featuresUsed, buildTreeParams->pNumLTs[iThread],
            buildTreeParams->pNumGEs[iThread]);

        threadHandles[iThread] =
            CreateSimpleThread(ComputeInfoGain, computeInfoGainParams[iThread]);
      }
      ...
      for (s32 iThread = 0; iThread < numThreadsToUse; iThread++) {
        ...
        // If the entropy is less, or the the entropy is LEQ and the mean
        // distance is more
        if ((computeInfoGainParams[iThread]->bestEntropy <
             curNode.bestEntropy) ||
            (computeInfoGainParams[iThread]->bestEntropy <=
                 curNode.bestEntropy &&
             computeInfoGainParams[iThread]->meanDistanceFromThreshold >
                 curNode.meanDistanceFromThreshold)) {
          curNode.bestEntropy = computeInfoGainParams[iThread]->bestEntropy;
          curNode.whichFeature =
              computeInfoGainParams[iThread]->bestFeatureIndex;
          curNode.numLeft = computeInfoGainParams[iThread]->totalNumLT;
          curNode.numRight = computeInfoGainParams[iThread]->totalNumGE;
          curNode.numLeftAndRight =
              computeInfoGainParams[iThread]->totalNumBoth;
    ...
    if (curNode.bestEntropy > 100000.0 || curNode.whichFeature < 0) {
      // TODO: there are probably very few labels, so this is an inefficent way
      // to compute which are unique
      ...
      Anki::CoreTechPrint(
          "Thread %d: Could not split LeafNode at depth %d for labels = {",
          buildTreeParams->threadId, curNode.depth);

      const s32 numLabels = failure.labels.size();
      for (s32 i = 0; i < numLabels; i++) {
        Anki::CoreTechPrint(
            "(%d, %s), ", failure.labels[i].value,
            buildTreeParams->labelNames[failure.labels[i].value]);
      }
    ...
    buildTreeParams->decisionTree.get_buffer().set_size(
        buildTreeParams->decisionTree.get_buffer().get_size() + 2);

    const s32 leftNodeIndex =
        buildTreeParams->decisionTree.get_buffer().get_size() - 2;
    const s32 rightNodeIndex =
        buildTreeParams->decisionTree.get_buffer().get_size() - 1;

    buildTreeParams->decisionTree.Unlock();  // Unlock decisionTree

    buildTreeParams->decisionTree.get_buffer()[leftNodeIndex] =
        DecisionTreeNode(curNode.depth + 1, FLT_MAX, -1, -1, -1, -1, -1, 0,
                         255);
    buildTreeParams->decisionTree.get_buffer()[rightNodeIndex] =
        DecisionTreeNode(curNode.depth + 1, FLT_MAX, -1, -1, -1, -1, -1, 0,
                         255);
    ...
    AnkiAssert(curNode.numLeft > 0 && curNode.numLeft < numRemaining &&
               curNode.numRight > 0 && curNode.numRight < numRemaining);

    RawBuffer<s32> leftRemaining(curNode.numLeft);
    RawBuffer<s32> rightRemaining(curNode.numRight);

    const u8 *restrict pFeatureValues =
        buildTreeParams->featureValues[curNode.whichFeature].Pointer(0);
    ...
    for (s32 iRemain = 0; iRemain < numRemaining; iRemain++) {
      ...
      if (curFeatureValue < (curGrayvalueThresholdS32 +
                             buildTreeParams->u8MinDistanceFromThreshold)) {
    ...
    const s32 minGray =
        MAX(0, curNode.u8Threshold - buildTreeParams->u8MinDistanceForSplits);
    const s32 maxGray =
        MIN(255, curNode.u8Threshold + buildTreeParams->u8MinDistanceForSplits);
    ...
    DecisionTreeWorkItem leftWorkItem(leftNodeIndex, leftRemaining,
                                      leftFeaturesUsed);
    DecisionTreeWorkItem rightWorkItem(rightNodeIndex, rightRemaining,
                                       rightFeaturesUsed);
    ...
    Anki::CoreTechPrint(
        "Thread %d: Computed split %d and %d, with best entropy of %f, in %f "
        "seconds\n",
        buildTreeParams->threadId, static_cast<s32>(leftRemaining.size),
        static_cast<s32>(rightRemaining.size), curNode.bestEntropy,
        time1 - time0);
#endif
  }  // while(buildTreeParams->numCompleted.Get() <
     // buildTreeParams->labels.get_size())
...
ThreadResult BenchmarkingThread(void *voidBenchmarkingParams) {
  BenchmarkingParameters *restrict benchmarkingParams =
      reinterpret_cast<BenchmarkingParameters *>(voidBenchmarkingParams);
  ...
  while (*(benchmarkingParams->isRunning)) {
    const f64 endTime =
        startTime + benchmarkingParams->sampleEveryNSeconds *
                        static_cast<f64>(benchmarkingParams->cpuUsage.size());
    ...
    // This loop body will run every sampleEveryNSeconds seconds, and once right
    // before quitting
    ...
    CoreTechPrint(
        "CPU %0.1f%% (queue is %d long). %d/%d = %0.2f%% completed in %0.2f "
        "seconds.\n",
        curCpuUsage, benchmarkingParams->workQueue.Size_unsafe(), numComplete,
        benchmarkingParams->numTotal,
        100.0 * static_cast<f64>(numComplete) /
            static_cast<f64>(benchmarkingParams->numTotal),
        lastTime - startTime);
  }  // while(*isRunning)
...
Result BuildTree(
  ...
  AnkiConditionalErrorAndReturnValue(
      AreValid(labelNames, labels, featureValues, u8ThresholdsToUse),
  ...
  AnkiConditionalErrorAndReturnValue(
      leafNodeFraction > 0.01 && leafNodeFraction <= 1.0 &&
          leafNodeNumItems >= 1 && u8MinDistanceForSplits >= 0 &&
          maxThreads <= MAX_THREADS,
      RESULT_FAIL_INVALID_PARAMETER, "BuildTree", "Invalid parameter");

  AnkiConditionalErrorAndReturnValue(featureValues.get_size() == numFeatures,
                                     RESULT_FAIL_INVALID_SIZE, "BuildTree",
                                     "Incorrect input size");

  for (s32 i = 0; i < numFeatures; i++) {
    AnkiConditionalErrorAndReturnValue(featureValues[i].get_size() == numItems,
                                       RESULT_FAIL_INVALID_SIZE, "BuildTree",
                                       "Incorrect input size");
  }
  ...
  for (s32 i = 0; i < maxThreads; i++) {
    ...
    for (s32 j = 0; j < maxThreads; j++) {
      ...
      AnkiConditionalErrorAndReturnValue(pNumLTs[i][j] && pNumGEs[i][j],
                                         RESULT_FAIL_OUT_OF_MEMORY, "BuildTree",
                                         "Out of memory");
    }
  }
  ...
  decisionTree.get_buffer()[0] =
      DecisionTreeNode(0, FLT_MAX, -1, -1, -1, -1, -1, 0, 255);
  ...
  BenchmarkingParameters benchmarkingParams(
      &isBenchmarkingRunning, benchmarkSampleEveryNSeconds, cpuUsage,
      numCompleted, labels.get_size(), workQueue);

  ThreadHandle benchmarkingThreadHandle = CreateSimpleThread(
      BenchmarkingThread, reinterpret_cast<void *>(&benchmarkingParams));

  CoreTechPrint("Starting single thread\n");

  //
  // First, launch one thread, to compute the first N nodes with all threads for
  // computing entropy
  //

  {
    ...
    BuildTreeThreadParameters oneThreadParameters(
        featuresUsed, labelNames, labels, featureValues, leafNodeFraction,
        leafNodeNumItems, u8MinDistanceForSplits, u8MinDistanceFromThreshold,
        u8ThresholdsToUse, maxThreads, pNumLTs[threadId], pNumGEs[threadId],
        maxLabel, threadId, numSingleThreadNodes, workQueue, trainingFailures,
        decisionTree, numCompleted);
  ...
  for (s32 iThread = 0; iThread < maxThreads; iThread++) {
    newParameters[iThread] = new BuildTreeThreadParameters(
        featuresUsed, labelNames, labels, featureValues, leafNodeFraction,
        leafNodeNumItems, u8MinDistanceForSplits, u8MinDistanceFromThreshold,
        u8ThresholdsToUse, 1, pNumLTs[iThread], pNumGEs[iThread], maxLabel,
        iThread, -1, workQueue, trainingFailures, decisionTree, numCompleted);

    AnkiConditionalErrorAndReturnValue(newParameters[iThread],
                                       RESULT_FAIL_OUT_OF_MEMORY, "BuildTree",
                                       "Out of memory");

    threadHandles[iThread] = CreateSimpleThread(
        BuildTreeThread, reinterpret_cast<void *>(newParameters[iThread]));
  }  // for(s32 iThread=0; iThread<maxThreads; iThread++)
