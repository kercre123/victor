...
static void CountValues_u8(const u8 *restrict pFeatureValues,
...
static void ComputeNumAboveThreshold(
    const u8 *restrict pFeatureValues, const s32 *restrict pLabels,
    const s32 maxLabel, const s32 *restrict pRemaining, const s32 numRemaining,
    const u8 curGrayvalueThreshold, const s32 u8MinDistanceFromThreshold,
    s32 *restrict pNumLT, s32 *restrict pNumGE, s32 &totalNumLT,
    s32 &totalNumGE, s32 &totalNumBoth, u8 &meanDistanceFromThreshold) {
  ...
  for (s32 iRemain = 0; iRemain < numRemaining; iRemain++) {
    ...
    totalDifferenceFromThreshold +=
        ABS(curFeatureValue - curGrayvalueThresholdS32);
    ...
    if (curFeatureValue <
        (curGrayvalueThresholdS32 + u8MinDistanceFromThreshold)) {
      totalNumLT++;
      pNumLT[curLabel]++;
      numAssigned++;
    }
  ...
  meanDistanceFromThreshold =
      saturate_cast<u8>(totalDifferenceFromThreshold / numRemaining);
}

// Type should be f32 or f64
template <typename Type>
static Type WeightedAverageEntropy(const s32 *restrict pNumLT,
                                   const s32 *restrict pNumGE,
                                   const s32 totalNumLT, const s32 totalNumGE,
                                   const s32 maxLabel) {
  Type entropyLessThan = 0;
  Type entropyGreaterThan = 0;

  const Type inverseTotalNumLessThan =
      static_cast<Type>(1) / static_cast<Type>(totalNumLT);
  const Type inverseTotalNumGreaterThan =
      static_cast<Type>(1) / static_cast<Type>(totalNumGE);
  ...
  for (s32 iLabel = 0; iLabel <= maxLabel; iLabel++) {
    ...
    if (pNumLT[iLabel] > 0) {
      const Type probability =
          static_cast<Type>(pNumLT[iLabel]) * inverseTotalNumLessThan;
    ...
    if (pNumGE[iLabel] > 0) {
      const Type probability =
          static_cast<Type>(pNumGE[iLabel]) * inverseTotalNumGreaterThan;
      entropyGreaterThan -= probability * log2(probability);
    }
  }  // for(s32 iLabel=0; iLabel<=maxLabel; iLabel++)

  const Type percent_lessThan = static_cast<Type>(totalNumLT) /
                                static_cast<Type>(totalNumLT + totalNumGE);
  const Type percent_greaterThan = static_cast<Type>(totalNumGE) /
                                   static_cast<Type>(totalNumLT + totalNumGE);

  const Type weightedAverageEntropy = percent_lessThan * entropyLessThan +
...
ThreadResult ComputeInfoGain(void *computeInfoGainParameters) {
  ComputeInfoGainParameters *restrict parameters =
      reinterpret_cast<ComputeInfoGainParameters *>(computeInfoGainParameters);
  ...
  for (s32 iFeature = parameters->minFeatureToCheck;
       iFeature <= parameters->maxFeatureToCheck; iFeature++) {
    const u8 *restrict pFeatureValues =
        parameters->featureValues[iFeature].Pointer(0);

    // If the u8ThresholdsToUse haven't been specified, compute them from the
    // data
    if (parameters->u8ThresholdsToUse.get_size() == 0) {
      ...
      // Compute the grayvalue thresholds (just halfway between each pair of
      // detected grayvalues
      ...
      for (s32 i = 0; i < 256; i++) {
        if (counts.counts[i] > 0) {
          if (previousValue > 0) {
            u8Thresholds[numGrayvalueThresholds] =
                (i + previousValue + 1) /
                2;  // The +1 is so it rounds up, like matlab
    ...
    for (s32 iGrayvalueThreshold = 0;
      ...
      ComputeNumAboveThreshold(
          pFeatureValues, pLabels, maxLabel, pRemaining, numRemaining,
          curGrayvalueThreshold, parameters->u8MinDistanceFromThreshold,
          parameters->pNumLT, parameters->pNumGE, totalNumLT, totalNumGE,
          totalNumBoth, meanDistanceFromThreshold);

      AnkiAssert((totalNumLT - totalNumBoth) >= 0 &&
                 (totalNumGE - totalNumBoth) >= 0);

      if ((totalNumLT - totalNumBoth) <= 0 ||
          (totalNumGE - totalNumBoth) <= 0) {
      ...
      // If the entropy is less, or the the entropy is the same and the mean
      // distance is more
      if ((entropy < parameters->bestEntropy) ||
          (entropy <= parameters->bestEntropy &&
           meanDistanceFromThreshold > parameters->meanDistanceFromThreshold)) {
