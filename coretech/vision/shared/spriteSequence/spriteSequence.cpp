 ...
 * Date:   2/6/2018
 *
 * Description: Defines container for sprite sequences that display on the
 *robot's face.
 *
...
SpriteSequence::LoopConfig SpriteSequence::LoopConfigFromString(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
SpriteSequence::SpriteSequence(LoopConfig config) : _loopConfig(config) {}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void SpriteSequence::CacheSequence(
    SpriteCache* cache, ImgTypeCacheSpec cacheSpec,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool SpriteSequence::GetFrame(const u32 index,
                              Vision::SpriteHandle& handle) const {
  if (_frames.empty()) {
    return false;
  }

  if ((_loopConfig == LoopConfig::DoNothing) && (index >= _frames.size())) {
    return false;
  }

  u32 modIndex = 0;
  const bool success = GetModdedIndex(index, modIndex);
  if (ANKI_VERIFY(success, "SpriteSequence.GetFrame.ModIndexFail",
                  "Index %d is invalid for greyscale sprite sequence of length "
                  "%zu and cannot be looped",
                  index, _frames.size())) {
...
bool SpriteSequence::GetModdedIndex(const u32 index, u32& moddedIndex) const {
  switch (_loopConfig) {
    ...
    case LoopConfig::Hold:
      moddedIndex = (index < _frames.size())
                        ? index
                        : static_cast<u32>(_frames.size() - 1);
      break;
    ...
    case LoopConfig::Error:
      if (index >= _frames.size()) {
        PRINT_NAMED_ERROR("SpriteSequence.GetFrame.FrameBeyondIndex",
                          "Attempted to access frame %d", index);
        return false;
      }
  }
  return true;
