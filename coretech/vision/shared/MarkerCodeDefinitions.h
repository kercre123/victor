// Autogenerated by VisionMarkerTrained.GenerateNearestNeighborHeaderFiles() on 03-Apr-2019 14:34:13 with Matlab 9.1.0.441655 (R2016b)

#ifndef ANKI_COZMO_VISIONMARKERTYPES_H
#define ANKI_COZMO_VISIONMARKERTYPES_H

/* ExtractProbeValues Parameters:
'addInversions', 0,
'negativeImageDir', ,
'computeGradMag', 0,
'numPerturbations', 1,
'perturbSigma', 0,
'blurSigmas', 0.005       0.015,
'imageSizes', 64,
'exposures', 1  4,
'computeGradMagWeights', 1,
'blackPoint', 0,
'boxFilterWidth', 0,

*/

#include <map>

namespace Anki {
namespace Vision {

  static const char* MarkerDefinitionVersionString = "03-Apr-2019 14:34:13";

  enum MarkerType {
    MARKER_ARROW = 0,
    MARKER_BULLSEYE2 = 1,
    MARKER_CHARGER_HOME = 2,
    MARKER_INVALID = 3,
    MARKER_LIGHTCUBEI_BACK = 4,
    MARKER_LIGHTCUBEI_BOTTOM = 5,
    MARKER_LIGHTCUBEI_FRONT = 6,
    MARKER_LIGHTCUBEI_LEFT = 7,
    MARKER_LIGHTCUBEI_RIGHT = 8,
    MARKER_LIGHTCUBEI_TOP = 9,
    MARKER_LIGHTCUBEJ_BACK = 10,
    MARKER_LIGHTCUBEJ_BOTTOM = 11,
    MARKER_LIGHTCUBEJ_FRONT = 12,
    MARKER_LIGHTCUBEJ_LEFT = 13,
    MARKER_LIGHTCUBEJ_RIGHT = 14,
    MARKER_LIGHTCUBEJ_TOP = 15,
    MARKER_LIGHTCUBEK_BACK = 16,
    MARKER_LIGHTCUBEK_BOTTOM = 17,
    MARKER_LIGHTCUBEK_FRONT = 18,
    MARKER_LIGHTCUBEK_LEFT = 19,
    MARKER_LIGHTCUBEK_RIGHT = 20,
    MARKER_LIGHTCUBEK_TOP = 21,
    MARKER_SDK_2CIRCLES = 22,
    MARKER_SDK_2DIAMONDS = 23,
    MARKER_SDK_2HEXAGONS = 24,
    MARKER_SDK_2TRIANGLES = 25,
    MARKER_SDK_3CIRCLES = 26,
    MARKER_SDK_3DIAMONDS = 27,
    MARKER_SDK_3HEXAGONS = 28,
    MARKER_SDK_3TRIANGLES = 29,
    MARKER_SDK_4CIRCLES = 30,
    MARKER_SDK_4DIAMONDS = 31,
    MARKER_SDK_4HEXAGONS = 32,
    MARKER_SDK_4TRIANGLES = 33,
    MARKER_SDK_5CIRCLES = 34,
    MARKER_SDK_5DIAMONDS = 35,
    MARKER_SDK_5HEXAGONS = 36,
    MARKER_SDK_5TRIANGLES = 37,
    MARKER_STAR5 = 38,
    NUM_MARKER_TYPES,
    MARKER_UNKNOWN = NUM_MARKER_TYPES
  };

  const char * const MarkerTypeStrings[NUM_MARKER_TYPES+1] = {
    "MARKER_ARROW",
    "MARKER_BULLSEYE2",
    "MARKER_CHARGER_HOME",
    "MARKER_INVALID",
    "MARKER_LIGHTCUBEI_BACK",
    "MARKER_LIGHTCUBEI_BOTTOM",
    "MARKER_LIGHTCUBEI_FRONT",
    "MARKER_LIGHTCUBEI_LEFT",
    "MARKER_LIGHTCUBEI_RIGHT",
    "MARKER_LIGHTCUBEI_TOP",
    "MARKER_LIGHTCUBEJ_BACK",
    "MARKER_LIGHTCUBEJ_BOTTOM",
    "MARKER_LIGHTCUBEJ_FRONT",
    "MARKER_LIGHTCUBEJ_LEFT",
    "MARKER_LIGHTCUBEJ_RIGHT",
    "MARKER_LIGHTCUBEJ_TOP",
    "MARKER_LIGHTCUBEK_BACK",
    "MARKER_LIGHTCUBEK_BOTTOM",
    "MARKER_LIGHTCUBEK_FRONT",
    "MARKER_LIGHTCUBEK_LEFT",
    "MARKER_LIGHTCUBEK_RIGHT",
    "MARKER_LIGHTCUBEK_TOP",
    "MARKER_SDK_2CIRCLES",
    "MARKER_SDK_2DIAMONDS",
    "MARKER_SDK_2HEXAGONS",
    "MARKER_SDK_2TRIANGLES",
    "MARKER_SDK_3CIRCLES",
    "MARKER_SDK_3DIAMONDS",
    "MARKER_SDK_3HEXAGONS",
    "MARKER_SDK_3TRIANGLES",
    "MARKER_SDK_4CIRCLES",
    "MARKER_SDK_4DIAMONDS",
    "MARKER_SDK_4HEXAGONS",
    "MARKER_SDK_4TRIANGLES",
    "MARKER_SDK_5CIRCLES",
    "MARKER_SDK_5DIAMONDS",
    "MARKER_SDK_5HEXAGONS",
    "MARKER_SDK_5TRIANGLES",
    "MARKER_STAR5",
    "MARKER_UNKNOWN"
  };

} // namespace Vision
} // namespace Anki

#endif // ANKI_COZMO_VISIONMARKERTYPES_H

