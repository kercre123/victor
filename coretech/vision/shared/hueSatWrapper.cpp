...
HueSatWrapper::HueSatWrapper(Image* hueImg, Image* saturationImg)
  ...
  const bool imagesAreValid =
      ANKI_VERIFY(!hueImg->IsEmpty() && !saturationImg->IsEmpty(),
  ...
  if (imagesAreValid) {
    ...
    // Create a deleter that does nothing since HueSatWrapper doesn't control
    // image memory
...
HueSatWrapper::HueSatWrapper(uint8_t hue, uint8_t saturation,
    ...
    : _hue(hue), _saturation(saturation), _imagesManagedExternally(false) {
...
uint8_t HueSatWrapper::GetHue() const {
  if (_imagesManagedExternally && (_hueImg != nullptr) &&
      (_hueImg->GetNumRows() > 0) && (_hueImg->GetNumCols() > 0)) {
...
uint8_t HueSatWrapper::GetSaturation() const {
  if (_imagesManagedExternally && (_saturationImg != nullptr) &&
...
bool HueSatWrapper::AreImagesCached(const ImageSize& imageSize) const {
  return (_hueImg != nullptr) && (_hueImg->GetNumRows() == imageSize.numRows) &&
...
void HueSatWrapper::AllocateImages(Image*& hueImg, Image*& saturationImg,
                                   const ImageSize& imageSize) {
  if ((_hueImg != nullptr) && (_hueImg->GetNumRows() == imageSize.numRows) &&
      (_hueImg->GetNumCols() == imageSize.numCols)) {
    PRINT_NAMED_WARNING(
        "HueSatWrapper.AllocateImages.AllocatingMemoryUnnecessarily",
        "Hue and saturation images are cached for hue %d and saturation %d, "
        "but new allocation requested",
        _hue, _saturation);
  }
  AllocateImagesInternal(hueImg, saturationImg, imageSize);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void HueSatWrapper::AllocateImagesInternal(Image*& hueImg,
                                           Image*& saturationImg,
                                           const ImageSize& imageSize) {
...
void HueSatWrapper::GetCachedImages(Image*& hueImg, Image*& saturationImg,
  ...
  if ((_hueImg == nullptr) || (_hueImg->GetNumRows() != imageSize.numRows) ||
    ...
    PRINT_NAMED_ERROR("HueSatWrapper.GetCachedImages.ImagesNotCached",
                      "Called get cached images on a wrapper that doesn't have "
                      "cached images");
    _hueImg =
        std::unique_ptr<Image>(new Image(imageSize.numRows, imageSize.numCols));
    _saturationImg =
        std::unique_ptr<Image>(new Image(imageSize.numRows, imageSize.numCols));
  }
  hueImg = _hueImg.get();
