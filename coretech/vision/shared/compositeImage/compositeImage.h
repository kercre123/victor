...
class CompositeImage {
 public:
  struct Sprite {
    Sprite(const SpriteBox& spriteBox, const SpriteHandle& spriteHandle)
        : spriteBox(spriteBox), spriteHandle(spriteHandle) {}
    SpriteBox spriteBox;
  ...
  // Overlay the SpriteBox contents of the composite image onto the base image
  // in layer order from firstLayer to lastLayer. This allows partial rendering
  // so that content can be deliberately injected at specific layering, e.g.
  // eyes are rendered externally between layers 5 & 6. NOTE: This function
  // assumes there is NO PRE-EXISTING CONTENT which should be preserved in the
 ...
 private:
  void DrawSpriteIntoImage(const Sprite& sprite, ImageRGBA& baseImage,
                           const bool firstImage) const;
  void DrawSpriteImage(const ImageRGBA& spriteImage, ImageRGBA& baseImage,
                       const Point2f& topCorner, bool firstImage) const;
