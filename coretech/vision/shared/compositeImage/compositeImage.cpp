...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CompositeImage::AddImage(const SpriteBox& spriteBox,
  ...
  if (_spriteBoxMap.end() == iter) {
    ...
    const auto& layerEmplacePair =
        _layerMap.emplace(spriteBox.layer, ImageLayer());
    ...
    LOG_WARNING("CompositeImage.AddImage.SpriteBoxOverwritten",
                "SpriteBoxes must be unique within a CompositeImage. "
                "Overwriting prior SpriteBox named: %s",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CompositeImage::DrawIntoImage(ImageRGBA& baseImage,
                                   const LayerName firstLayer,
                                   const LayerName lastLayer) const {
  ...
  if (firstLayer > lastLayer) {
    LOG_ERROR("CompositeImage.DrawIntoImage.InvalidLayers",
              ...
              EnumToString(firstLayer), EnumToString(lastLayer));
    return;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CompositeImage::DrawSpriteIntoImage(const Sprite& sprite,
                                         ImageRGBA& baseImage,
                                         const bool firstImage) const {
  Point2f topCorner = {static_cast<float>(sprite.spriteBox.xPos),
                       static_cast<float>(sprite.spriteBox.yPos)};
  ...
  switch (sprite.spriteBox.renderMethod) {
    case SpriteRenderMethod::RGBA: {
      ...
      if (sprite.spriteHandle->IsContentCached().rgba) {
        const ImageRGBA& spriteImage =
            sprite.spriteHandle->GetCachedSpriteContentsRGBA();
        DrawSpriteImage(spriteImage, baseImage, topCorner, firstImage);
      } else {
        const ImageRGBA& spriteImage =
            sprite.spriteHandle->GetSpriteContentsRGBA();
        DrawSpriteImage(spriteImage, baseImage, topCorner, firstImage);
      }
      break;
    }
    case SpriteRenderMethod::EyeColor: {
      Vision::HueSatWrapper::ImageSize imageSize(
          static_cast<uint32_t>(sprite.spriteBox.height),
      ...
      if (nullptr == _faceHSImageHandle) {
        LOG_ERROR(
            "CompositeImage.DrawSpriteIntoImage.ShouldRenderInEyeHueButCant",
            "HS Image handle missing - image will be renderd with 0,0 hue "
            "saturation");
      }

      // TODO: Kevin K. Copy is happening here due to way we can resize image
      // handles with cached data do something better
      auto hue = _faceHSImageHandle->GetHue();
      auto sat = _faceHSImageHandle->GetSaturation();
      hsImageHandle =
          std::make_shared<Vision::HueSatWrapper>(hue, sat, imageSize);

      // Render the sprite - use the cached RGBA image if possible
      if (sprite.spriteHandle->IsContentCached(hsImageHandle).rgba) {
        const ImageRGBA& spriteImage =
            sprite.spriteHandle->GetCachedSpriteContentsRGBA(hsImageHandle);
        DrawSpriteImage(spriteImage, baseImage, topCorner, firstImage);
      } else {
        const ImageRGBA& spriteImage =
            sprite.spriteHandle->GetSpriteContentsRGBA(hsImageHandle);
        DrawSpriteImage(spriteImage, baseImage, topCorner, firstImage);
      }
      break;
    }
...
void CompositeImage::DrawSpriteImage(const ImageRGBA& spriteImage,
  ...
  // if the first layer rendered is going to be full-screen and draw all pixels,
  // then there's no need to clear the buffer (with FillWith) since all of those
  // blank pixels will be immediately overwritten. This saves about 1/5 ms when
  // those conditions are true.
  if (firstImage && ((spriteImage.GetNumRows() != baseImage.GetNumRows()) ||
