...
class HueSatWrapper {
 public:
  struct ImageSize {
    ImageSize() : numRows(0), numCols(0) {}

    ImageSize(uint32_t rows, uint32_t cols) : numRows(rows), numCols(cols) {}
    uint32_t numRows;
    uint32_t numCols;
  };

  // Pass in pointers to externally managed hue/saturation images
  // Class does not take ownership of the associated memory or have safeguards
  // if the images are deleted while the wrapper persists
  ...
  // Memory will be allocated for new hue/saturation images filled with the
  // appropriate values The pointers passed in will be set to point to these
  // images and the caller is responsible for managing the memory after the call
  void AllocateImages(Image*& hueImg, Image*& saturationImg,
                      const ImageSize& imageSize);

  // Returns hue and saturation images from the internally cached values
  // No allocation cost is incurred if allocateNow was set to true in the
  // constructor Otherwise an error will print and allocation will happen at
  // calltime
  void GetCachedImages(Image*& hueImg, Image*& saturationImg,
                       const ImageSize& imageSize);

 private:
  // When hue/saturation images are passed in as pointers they may be updated
  // externally without notifying the HueSatWrapper - so hue and saturation are
  // stored as mutables so that they can be updated if necessary to stay in sync
  // with _hue/_saturation image values
  mutable uint8_t _hue = 0;
  mutable uint8_t _saturation = 0;
  // Indicates whether the image values may change due to external updates to
  // the image
