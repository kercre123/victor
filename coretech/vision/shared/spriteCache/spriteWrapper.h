...
class SpriteWrapper : public ISpriteWrapper {
 public:
  ...
  virtual ImgTypeCacheSpec IsContentCached(
      const HSImageHandle& hsImage) const override;

  virtual ImageRGBA GetSpriteContentsRGBA(
      const HSImageHandle& hsImage) override;
  virtual Image GetSpriteContentsGrayscale() override;
  virtual const ImageRGBA& GetCachedSpriteContentsRGBA(
      const HSImageHandle& hsImage) override;
  ...
  // cacheGrayscale defines what combination of Grayscale/RGBA to load into
  // memory
  void CacheSprite(const ImgTypeCacheSpec& typesToCache = {false, false},
 ...
 private:
  ...
  void ApplyHS(const Image& grayImg, const HSImageHandle& hsImage,
               ImageRGBA* outImg) const;

  const std::string _fullSpritePath;
  // Keep track of what hue/saturation have been applied to the image if
  // appropriate
  uint16_t _hsID = 0;
