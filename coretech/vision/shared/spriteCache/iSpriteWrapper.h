...
class ISpriteWrapper {
 public:
  ...
  struct ImgTypeCacheSpec {
    ImgTypeCacheSpec() : grayscale(false), rgba(false) {}
    ImgTypeCacheSpec(bool g, bool r) : grayscale(g), rgba(r) {}

    bool grayscale;
    bool rgba;
  };

  // Returns a pair of bools representing whether Grayscale/RGBA are cached
  // respectively If hsImage is passed in the bool for RGBA will represent
  // whether the cached RGBA represents the image with that hue/saturation
  // overlaid This allows functions with no knowledge of the wrapper they're
  // working with and wheather it's cached or not to gracefully use the best
  // available function
  virtual ImgTypeCacheSpec IsContentCached(
      const HSImageHandle& hsImage = {}) const = 0;
