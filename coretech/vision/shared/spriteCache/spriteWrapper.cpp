...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
SpriteWrapper::SpriteWrapper(ImageRGBA* sprite) { _spriteRGBA.reset(sprite); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
SpriteWrapper::SpriteWrapper(Image* sprite) { _spriteGrayscale.reset(sprite); }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
ISpriteWrapper::ImgTypeCacheSpec SpriteWrapper::IsContentCached(
    const HSImageHandle& hsImage) const {
...
ImageRGBA SpriteWrapper::GetSpriteContentsRGBA(const HSImageHandle& hsImage) {
  ...
  if ((_spriteGrayscale != nullptr) && (hsImage != nullptr) &&
      (hsImage->GetHSID() != 0)) {
    ImageRGBA outImage;
...
const ImageRGBA& SpriteWrapper::GetCachedSpriteContentsRGBA(
    const HSImageHandle& hsImage) {
  if ((_spriteRGBA == nullptr) || !ImageMatchesStoredID(hsImage)) {
    PRINT_NAMED_ERROR(
        "SpriteWrapper.GetCachedGetCachedSpriteContents.InvalidContentAccess",
...
bool SpriteWrapper::GetFullSpritePath(std::string& fullSpritePath) {
  ...
  if (ANKI_DEV_CHEATS && _fullSpritePath.empty()) {
    PRINT_NAMED_ERROR("SpriteWrapper.GetFullSpritePath.PathIsEmpty",
                      "The image stored in this wrapper does not reference a "
                      "sprite saved on disk");
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void SpriteWrapper::CacheSprite(const ImgTypeCacheSpec& typesToCache,
                                const HSImageHandle& hsImage) {
  // Check to see if Grayscale is already cached
  if (typesToCache.grayscale && _spriteGrayscale != nullptr) {
    PRINT_NAMED_WARNING(
        "SpriteWrapper.CacheSprite.GrayscaleSpriteAlreadyCached",
        "CacheSprite called on %s which is already cached",
        _fullSpritePath.c_str());
  }

  // Check to see if RGBA is already cached
  const bool isRGBACached =
      (_spriteRGBA != nullptr) && (hsImage->GetHSID() == _hsID);
  if (typesToCache.rgba && isRGBACached) {
...
void SpriteWrapper::ClearCachedSprite() {
  if ((_spriteRGBA == nullptr) && (_spriteGrayscale == nullptr)) {
  ...
  if (_fullSpritePath.empty()) {
    PRINT_NAMED_WARNING("SpriteWrapper.ClearCachedSprite.NoSprite Path",
                        "ClearCachedSprite called on SpriteWrapper which "
                        "stores image directly - sprite cannot be recovered");
  }
...
void SpriteWrapper::LoadSprite(Image* outImage) const {
  ...
  ANKI_VERIFY(RESULT_OK == res,
              ...
              "Failed to load sprite %s", _fullSpritePath.c_str());
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void SpriteWrapper::LoadSprite(ImageRGBA* outImage,
                               const HSImageHandle& hsImage) const {
  if (_fullSpritePath.empty()) {
  ...
  if ((hsImage != nullptr) && hsImage->GetHSID() != 0) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void SpriteWrapper::ApplyHS(const Image& grayImg, const HSImageHandle& hsImage,
                            ImageRGBA* outImg) const {
  if (outImg == nullptr) {
  ...
  if (!dimensionsMatch) {
    PRINT_NAMED_WARNING("SpriteWrapper.ApplyHS.SizeMismatch",
                        "Existing dimensions (%d,%d) did not match grayscale "
                        "dimensions (%d,%d)",
  ...
  Vision::HueSatWrapper::ImageSize imageSize(
      static_cast<uint32_t>(grayImg.GetNumRows()),
  ...
  const std::vector<cv::Mat> channels{hueImage->get_CvMat_(),
