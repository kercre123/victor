...
class SpriteCache {
 public:
  ...
  SpriteHandle GetSpriteHandleForAssetID(
  ...
  SpriteHandle GetSpriteHandleForSpritePath(
      const std::string& fullSpritePath,
      const HSImageHandle& hueAndSaturation = {});

  void Update(BaseStationTime_t currTime_nanosec);

  // Functions for caching/clearing sprites out of cache memory
  void CacheSprite(const SpriteHandle& handle, ImgTypeCacheSpec cacheSpec,
                   BaseStationTime_t cacheFor_ms = 0,
                   const HSImageHandle& hueAndSaturation = {});

  void ClearCachedSprite(SpriteHandle handle, ImgTypeCacheSpec cacheSpec,
                         const HSImageHandle& hueAndSaturation = {});
 ...
 private:
  ...
  SpriteNameToHandleMap& GetHandleMapForHue(
      const HSImageHandle& hueAndSaturation);

  InternalHandle GetSpriteHandleInternal(
      const std::string& fullSpritePath,
      const HSImageHandle& hueAndSaturation = {});

  // In most cases this is the equivelent of a downcast from SpriteHandle to
  // InternalHandle If the SpriteHandle doesn't exist withn the cache, it will
  // be added as part of this function call
  InternalHandle ConvertToInternalHandle(
