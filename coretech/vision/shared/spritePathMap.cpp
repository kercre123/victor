...
std::unordered_map<SpritePathMap::AssetID, SpritePathMap::AssetInfo>
    SpritePathMap::_idToInfoMap;

const SpritePathMap::AssetID SpritePathMap::kClearSpriteBoxID =
    SpritePathMap::HashAssetName("clear_sprite_box");
const SpritePathMap::AssetID SpritePathMap::kEmptySpriteBoxID =
    SpritePathMap::HashAssetName("empty_sprite_box");
const SpritePathMap::AssetID SpritePathMap::kInvalidSpriteID =
    SpritePathMap::HashAssetName("invalid_sprite_box");
...
SpritePathMap::AssetInfo::AssetInfo()
    : isSpriteSequence(false), assetName(""), filePath("") {}
...
SpritePathMap::AssetInfo::AssetInfo(const bool isSpriteSeq,
    ...
    : isSpriteSequence(isSpriteSeq), assetName(assetName), filePath(filePath) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
SpritePathMap::SpritePathMap() {
  // Make sure these ID's don't conflict with any of the hashed AssetNames
  _idToInfoMap.emplace(kClearSpriteBoxID,
                       AssetInfo{false, "ClearSpriteBox", ""});
  _idToInfoMap.emplace(kEmptySpriteBoxID,
                       AssetInfo{false, "EmptySpriteBox", ""});
  _idToInfoMap.emplace(kInvalidSpriteID,
                       AssetInfo{false, "InvalidSpriteBox", ""});
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
SpritePathMap::AssetID SpritePathMap::HashAssetName(
    const std::string& assetName) {
  // Here we use only the bottom 16 bits of the hash. This increases probability
  // of collision, but given the sample size is just our asset names, the
...
SpritePathMap::AssetID SpritePathMap::GetAssetID(const std::string& assetName) {
  if (!_loadingComplete) {
    // We haven't loaded assets yet. Hash the name, return the ID, and make a
    // note so we can verify the asset after loading is finished
  ...
  if (_idToInfoMap.end() == _idToInfoMap.find(assetID)) {
    LOG_ERROR("SpritePathMap.GetAssetID.InvalidAssetName",
              ...
              assetName.c_str(), kPlaceHolderAssetName);
...
SpritePathMap::AssetID SpritePathMap::AddAsset(const std::string& assetName,
  ...
  if (!emplacePair.second) {
    if (emplacePair.first->second.assetName == assetName) {
      LOG_WARNING(
          "SpritePathMap.AddAsset.DuplicateAssetName",
      ...
      LOG_ERROR("SpritePathMap.AddAsset.InvalidHash",
                ...
                assetName.c_str(), emplacePair.first->second.assetName.c_str());
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
const SpritePathMap::AssetInfo* const SpritePathMap::GetInfoForAsset(
  ...
  if (VerifyPlaceholderAsset()) {
    LOG_ERROR("SpritePathMap.GetAssetID.InvalidAssetID",
              ...
              assetID, kPlaceHolderAssetName);
    return GetInfoForAsset(kPlaceHolderAssetID);
  }

  PRINT_NAMED_ERROR("SpritePathMap.DefaultSpriteNotFound",
                    "There must be a sprite named %s among the sprite assets, "
                    "and isn't. CRASH IMMINENT.",
                    kPlaceHolderAssetName);
  return nullptr;
}
...
void SpritePathMap::CheckUnverifiedAssetIDs() const {
  ...
  for (const auto& assetName : *_unverifiedAssets) {
    ANKI_VERIFY(
        _idToInfoMap.end() != _idToInfoMap.find(HashAssetName(assetName)),
        "SpritePathMap.GetAssetID.InvalidAssetName",
        "No asset with name %s is available. Ensure that it is present in "
        "EXTERNALS",
        assetName.c_str());
  }
