...
#define NUM_BYTES_probeLocationsBuffer                            \
  (sizeof(Point<s16>) * MAX_FIDUCIAL_MARKER_BIT_PROBE_LOCATIONS + \
   MEMORY_ALIGNMENT + 32)
#define NUM_BYTES_probeWeightsBuffer                                          \
  (sizeof(s16) * MAX_FIDUCIAL_MARKER_BIT_PROBE_LOCATIONS + MEMORY_ALIGNMENT + \
   32)
#define NUM_BYTES_bitsBuffer                                    \
  (sizeof(FiducialMarkerParserBit) * MAX_FIDUCIAL_MARKER_BITS + \
   MEMORY_ALIGNMENT + 32)
...
// A BlockMarker is a location Quadrilateral, with a given blockType and
// faceType. The blockType and faceType can be computed by a
...
class VisionMarkerImages {
 public:
  ...
  VisionMarkerImages(const FixedLengthList<const char *> &imageFilenames,
                     MemoryStack &memory);

  // Makes a shallow copy, based on the pointers
  VisionMarkerImages(const s32 numDatabaseImages, const s32 databaseImageHeight,
  ...
  Result MatchExhaustive(const Array<u8> &image, const Quadrilateral<f32> &quad,
                         VisionMarker &extractedMarker, f32 &matchQuality,
 ...
 protected:
  ...
  Array<u8> databaseImages;  //< Striped, so for a given pixel, all images data
                             //is consecutive. (e.g. the first N bytes are the
                             //pixel (0,0) for all images)

  FixedLengthList<Anki::Vision::MarkerType>
      databaseLabelIndexes;  // (aka name, aka codeName)
...
class VisionMarker {
 public:
  ...
  Result RefineCorners(const Array<u8> &image, const f32 minContrastRatio,
  ...
  Result ExtractExhaustive(const VisionMarkerImages &allMarkerImages,
                           const Array<u8> &image, MemoryStack fastScratch,
  ...
  Result Deserialize(char *objectName, void **buffer, s32 &bufferLength,
                     MemoryStack scratch);  // Updates the buffer pointer and
                                            // length before returning

  s32 get_serializationSize() const;

  static Vision::MarkerType RemoveOrientation(
 ...
 protected:
  // The constructor isn't always called, so initialize has to be checked in
  // multiple places
  ...
  // Result ComputeThreshold(const Array <u8> &image, const Array<f32>
  // &homography,
  //  const f32 minContrastRatio, bool &isHighContrast, u8
  //  &meanGrayvalueThreshold);

  Result ComputeBrightDarkValues(const Array<u8> &image,
                                 const f32 minContrastRatio,
                                 const bool isDarkOnLight, f32 &backgroundValue,
                                 f32 &foregroundValue, bool &enoughContrast);

#if RECOGNITION_METHOD == RECOGNITION_METHOD_DECISION_TREES
  static bool areTreesInitialized;
  static FiducialMarkerDecisionTree
      multiClassTrees[VisionMarkerDecisionTree::NUM_TREES];
#elif RECOGNITION_METHOD == RECOGNITION_METHOD_CNN
...
// A FiducialMarkerParserBit object samples an input image, to determine if a
// given image area is binary 0 or 1.
class FiducialMarkerParserBit {
 public:
  ...
  FiducialMarkerParserBit(const s16 *const probesX, const s16 *const probesY,
                          const s16 *const probeWeights, const s32 numProbes,
                          const FiducialMarkerParserBit::Type type,
                          const s32 numFractionalBits, MemoryStack &memory);

  // Based on the probe locations that this object was initialized with, compute
  // the mean value in a given image, with an overall bounding Quadrilateral.
  //
  // NOTE:
  // The meanValue is not the mean value of the whole quad, just the part of the
  // quad that is sampled by this Bit.
  Result ExtractMeanValue(const Array<u8> &image,
                          const Quadrilateral<s16> &quad,
                          const Array<f32> &homography, s16 &meanValue) const;
 ...
 protected:
  FixedLengthList<Point<s16> >
      probeLocations;  //< A list of length
...
// A FiducialMarkerParser takes an input image and Quadrilateral, and extracts
// the binary code. It contains a list of FiducialMarkerParserBit objects that
// sample different locations of the code.
class FiducialMarkerParser {
 public:
  ...
  Result ExtractBlockMarker(const Array<u8> &image,
                            const Quadrilateral<s16> &quad,
                            const Array<f32> &homography,
                            const f32 minContrastRatio, BlockMarker &marker,
 ...
 protected:
  ...
  // Once all FiducialMarkerParserBit have been parsed, determine if the code is
  // valid, by checking its checksum
  static bool IsChecksumValid(const FixedLengthList<u8> &checksumBits,
                              const FixedLengthList<u8> &blockBits,
                              const FixedLengthList<u8> &faceBits);

  Result InitializeAsDefaultParser(MemoryStack &memory);

  // Once all FiducialMarkerParserBit have been parsed, determine the
  // orientation of the code
  Result DetermineOrientationAndBinarizeAndReorderCorners(
      const FixedLengthList<s16> &meanValues, const f32 minContrastRatio,
      BlockMarker &marker, FixedLengthList<u8> &binarizedBits,
      MemoryStack scratch) const;

  // Once all FiducialMarkerParserBit have been parsed, determine the ID number
  // of the code
  Result DecodeId(const FixedLengthList<u8> &binarizedBits, s16 &blockType,
                  s16 &faceType, MemoryStack scratch) const;

  // Starting at startIndex, search through this->bits to find the first
  // instance of the given type Returns -1 if the type wasn't found
  s32 FindFirstBitOfType(const FiducialMarkerParserBit::Type type,
                         const s32 startIndex) const;
};  // class FiducialMarkerParser

// Ignores case, and ignore any "MARKER_" prefix
// Also ignores anything before a "/" or "\", and anything after a ".", so you
