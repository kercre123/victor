...
#define DEBUG_QUAD_REFINEMENT \
  0  // Vision system will need to be running synchronously
...
Result RefineQuadrilateral(const Quadrilateral<f32>& initialQuad,
                           const Array<f32>& initialHomography,
                           const Array<u8>& image,
                           const Point<f32>& squareSizeFraction,
                           const Point<f32>& roundedCornerFraction,
                           const s32 maxIterations, const f32 foregroundGray,
                           const f32 backgroundGray, const s32 numSamples,
                           const f32 maxCornerChange, const f32 minCornerChange,
                           Quadrilateral<f32>& refinedQuad,
                           Array<f32>& refinedHomography, MemoryStack scratch) {
  BeginBenchmark("vme_quadrefine_init");

  Result lastResult = RESULT_OK;

  AnkiConditionalErrorAndReturnValue(
      AreEqualSize(3, 3, refinedHomography, initialHomography),
      RESULT_FAIL_INVALID_SIZE, "RefineQuadrilateral",
      "Input and Output refined homography array must be valid and 3x3.");

  AnkiConditionalErrorAndReturnValue(
      NotAliased(initialHomography, refinedHomography),
      RESULT_FAIL_ALIASED_MEMORY, "RefineQuadrilateral",
      "initialHomography and refinedHomography are aliased");
  ...
  // ySquareOuter = [zeros(1,N)      ones(1,N)       linspace(0,1,N)
  // linspace(0,1,N)];
  //
  // xSquareInner = [linspace(this.SquareWidthFraction,
  // 1-this.SquareWidthFraction, N) ...
  ...
  const f32 outerIncX =
      (1.f - 2.f * roundedCornerFraction.x) / static_cast<f32>(N - 1);
  const f32 outerIncY =
      (1.f - 2.f * roundedCornerFraction.y) / static_cast<f32>(N - 1);
  // LinearSequence<f32> OuterOneToN = LinearSequence<f32>(0.f, outerInc, 1.f);

  const f32 innerIncX =
      (1.f - 2.f * std::max(squareSizeFraction.x, roundedCornerFraction.x)) /
      static_cast<f32>(N - 1);
  const f32 innerIncY =
      (1.f - 2.f * std::max(squareSizeFraction.y, roundedCornerFraction.y)) /
      static_cast<f32>(N - 1);
  // LinearSequence<f32> InnerOneToN = LinearSequence<f32>(squareWidthFraction,
  // innerInc, 1.f-squareWidthFraction);
  ...
  */
  ...
  matlab.EvalStringEcho(
      "initialQuad = double(initialQuad); "
...
#else
  auto DrawQuad = [](cv::Mat& img, const Quadrilateral<f32>& quad,
  ...
  };
  {
    ...
    const f32 h02 = initialHomography[0][2];
    const f32 h10 = initialHomography[1][0];
    const f32 h11 = initialHomography[1][1];
    const f32 h12 = initialHomography[1][2];
    const f32 h20 = initialHomography[2][0];
    const f32 h21 = initialHomography[2][1];
    ...
    for (s32 iSample = 0; iSample < actualNumSamples; iSample++) {
      ...
      const f32 normalization =
          1.f / (h20 * pX[iSample] + h21 * pY[iSample] + h22);
  ...
  AnkiConditionalErrorAndReturnValue(
      homographyUpdate.IsValid(), RESULT_FAIL_MEMORY, "RefineQuadrilateral",
      "Failed to allocate homographyUpdate matrix.");

  AnkiConditionalErrorAndReturnValue(
      newHomography.IsValid(), RESULT_FAIL_MEMORY, "RefineQuadrilateral",
  ...
  for (s32 iteration = 0; iteration < maxIterations && !restoreOriginal;
       iteration++) {
    BeginBenchmark("vme_quadrefine_mainLoop_init");

    const f32 h00 = refinedHomography[0][0];
    const f32 h01 = refinedHomography[0][1];
    const f32 h02 = refinedHomography[0][2];
    const f32 h10 = refinedHomography[1][0];
    const f32 h11 = refinedHomography[1][1];
    const f32 h12 = refinedHomography[1][2];
    const f32 h20 = refinedHomography[2][0];
    const f32 h21 = refinedHomography[2][1];
    const f32 h22 = refinedHomography[2][2];
    ...
    for (s32 iSample = 0; iSample < actualNumSamples && !restoreOriginal;
         iSample++) {
    ...
    homographyUpdate[0][0] = 1.0f + pUpdate[0];
    homographyUpdate[0][1] = pUpdate[1];
    homographyUpdate[0][2] = pUpdate[2];
    homographyUpdate[1][0] = pUpdate[3];
    homographyUpdate[1][1] = 1.0f + pUpdate[4];
    homographyUpdate[1][2] = pUpdate[5];
    homographyUpdate[2][0] = pUpdate[6];
    homographyUpdate[2][1] = pUpdate[7];
    homographyUpdate[2][2] = 1.0f;

    // this.tform = this.tform*inv(tformUpdate);
    Invert3x3(
        homographyUpdate[0][0], homographyUpdate[0][1], homographyUpdate[0][2],
    ...
    const f32 currentCornerChange =
        MaxCornerChange(refinedHomography, refinedQuad);
...
#if VISUALIZE_WITH_MATLAB
    {
      ...
      matlab.EvalStringEcho(
          "delete(findobj(gcf, 'Tag', 'refinedQuad')); "
...
#else
    {
      ...
      // cv::imwrite("/Users/andrew/temp/refinedQuad_" +
      // std::to_string(iteration) + ".png", dispImg);
      cv::waitKey(5);
    }
#endif
#endif
  ...
  */
  ...
  if (restoreOriginal) {
    ...
    // Check to make sure the refined quad isn't too different from the intitial
    // one. If it is, restore the original.
    const f32 finalCornerChange =
        MaxCornerChange(refinedHomography, initialQuadF32);
