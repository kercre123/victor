// Example:
// [bestEntropy, bestU8Threshold] = mexComputeInfoGain2_innerLoop(curLabels,
// curFeatureValues, u8Thresholds, maxLabel, 1)
...
typedef struct {
  ...
  s32 *restrict pNumLessThan;  //< Must be allocated before calling the thread,
                               //and freed after the thread is complete
  s32 *restrict
      pNumGreaterThan;  //< Must be allocated before calling the thread, and
                        //freed after the thread is complete
...
static ThreadResult MainLoop(void *threadParameter) {
  MainLoopParameters *restrict parameters =
      (MainLoopParameters *)threadParameter;

  parameters->bestEntropy = FLT_MAX;
  parameters->bestU8Threshold = 0;

  for (s32 iGrayvalueThreshold = parameters->minGrayvalueThresholdIndex;
       iGrayvalueThreshold <= parameters->maxGrayvalueThresholdIndex;
       iGrayvalueThreshold++) {
    const u8 curGrayvalueThreshold =
        parameters->pGrayvalueThresholds[iGrayvalueThreshold];

    memset(parameters->pNumLessThan, 0,
           (parameters->maxLabel + 1) * sizeof(s32));
    memset(parameters->pNumGreaterThan, 0,
           (parameters->maxLabel + 1) * sizeof(s32));

    s32 totalNumLessThan = 0;
    s32 totalNumGreaterThan = 0;

    // labelsLessThan = curLabels(curFeatureValues <
    // u8Thresholds(iGrayvalueThreshold)); labelsGreaterThan =
    // curLabels(curFeatureValues >= u8Thresholds(iGrayvalueThreshold));
    ...
    const PRECISION inverseTotalNumLessThan =
        static_cast<PRECISION>(1) / static_cast<PRECISION>(totalNumLessThan);
    const PRECISION inverseTotalNumGreaterThan =
        static_cast<PRECISION>(1) / static_cast<PRECISION>(totalNumGreaterThan);

    for (s32 iLabel = 0; iLabel <= parameters->maxLabel; iLabel++) {
      // probabilitiesLessThan = allValuesLessThan / sum(allValuesLessThan);
      // entropyLessThan = -sum(probabilitiesLessThan .* log2(max(eps,
      // probabilitiesLessThan)));
      if (parameters->pNumLessThan[iLabel] > 0) {
        const PRECISION probability =
            static_cast<PRECISION>(parameters->pNumLessThan[iLabel]) *
            inverseTotalNumLessThan;
        entropyLessThan -= probability * log2(probability);
      }

      // probabilitiesGreaterThan = allValuesGreaterThan /
      // sum(allValuesGreaterThan); entropyGreaterThan =
      // -sum(probabilitiesGreaterThan .* log2(max(eps,
      // probabilitiesGreaterThan)));
      if (parameters->pNumGreaterThan[iLabel] > 0) {
        const PRECISION probability =
            static_cast<PRECISION>(parameters->pNumGreaterThan[iLabel]) *
            inverseTotalNumGreaterThan;
        entropyGreaterThan -= probability * log2(probability);
      }
    }  // for(s32 iLabel=0; iLabel<=maxLabel; iLabel++)

    const PRECISION percent_lessThan =
        static_cast<PRECISION>(totalNumLessThan) /
        static_cast<PRECISION>(totalNumLessThan + totalNumGreaterThan);
    const PRECISION percent_greaterThan =
        static_cast<PRECISION>(totalNumGreaterThan) /
        static_cast<PRECISION>(totalNumLessThan + totalNumGreaterThan);

    const PRECISION weightedAverageEntropy =
        percent_lessThan * entropyLessThan +
        percent_greaterThan * entropyGreaterThan;
  ...
  }  // for(s32 iGrayvalueThreshold=0;
     // iGrayvalueThreshold<numGrayvalueThresholds; iGrayvalueThreshold++)
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  AnkiConditionalErrorAndReturn(
      nrhs == 5 && nlhs == 2, "mexComputeInfoGain2_innerLoop",
      "Call this function as follows: [bestEntropy, bestU8Threshold] = "
      "mexComputeInfoGain2_innerLoop(curLabels, curFeatureValues, "
      "u8Thresholds, maxLabel, numThreads);");

  const s32 *restrict pCurLabels = reinterpret_cast<s32 *>(mxGetData(prhs[0]));
  const u8 *restrict pCurFeatureValues =
      reinterpret_cast<u8 *>(mxGetData(prhs[1]));
  const u8 *restrict pGrayvalueThresholds =
      reinterpret_cast<u8 *>(mxGetData(prhs[2]));
  ...
  AnkiConditionalErrorAndReturn(
      ...
      "mexComputeInfoGain2_innerLoop", "Incorrect input size");

  AnkiConditionalErrorAndReturn(mxGetM(prhs[0]) == mxGetM(prhs[1]),
                                "mexComputeInfoGain2_innerLoop",
                                "Incorrect input size");

  AnkiConditionalErrorAndReturn(mxGetM(prhs[0]) > 0 && mxGetM(prhs[2]) > 0,
                                "mexComputeInfoGain2_innerLoop",
                                "Incorrect input size");
  ...
  if (numThreads == 1) {
    ...
    parameters.pNumLessThan = reinterpret_cast<s32 *>(
        mxMalloc((parameters.maxLabel + 1) * sizeof(s32)));
    parameters.pNumGreaterThan = reinterpret_cast<s32 *>(
        mxMalloc((parameters.maxLabel + 1) * sizeof(s32)));
    ...
    const s32 numGrayvalueThresholdsPerThread =
        (numGrayvalueThresholds + numThreads - 1) / numThreads;
    ...
    for (s32 iThread = 0; iThread < numThreads; iThread++) {
      ...
      parameters[iThread].minGrayvalueThresholdIndex =
          iThread * numGrayvalueThresholdsPerThread;
      parameters[iThread].maxGrayvalueThresholdIndex =
          MIN(numGrayvalueThresholds - 1,
              (iThread + 1) * numGrayvalueThresholdsPerThread - 1);

      parameters[iThread].pNumLessThan = reinterpret_cast<s32 *>(
          mxMalloc((parameters[iThread].maxLabel + 1) * sizeof(s32)));
      parameters[iThread].pNumGreaterThan = reinterpret_cast<s32 *>(
          mxMalloc((parameters[iThread].maxLabel + 1) * sizeof(s32)));
      ...
      pthread_create(&(threadHandles[iThread]), &connectionAttr, MainLoop,
                     (void *)(&parameters[iThread]));
#endif
    }
