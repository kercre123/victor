...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  AnkiConditionalErrorAndReturn(image.IsValid(), "mexSimpleDetectorSteps1234", "Could not allocate image");

  const u32 numBytes0 = 10000000;
  MemoryStack scratch0(calloc(numBytes0,1), numBytes0);
  AnkiConditionalErrorAndReturn(scratch0.IsValid(), "mexSimpleDetectorSteps1234", "Scratch0 could not be allocated");

  const u32 numBytes1 = 10000000;
  MemoryStack scratch1(calloc(numBytes1,1), numBytes0);
  AnkiConditionalErrorAndReturn(scratch1.IsValid(), "mexSimpleDetectorSteps1234", "Scratch1 could not be allocated");

  const u32 numBytes2 = 10000000;
  MemoryStack scratch2(calloc(numBytes2,1), numBytes2);
  AnkiConditionalErrorAndReturn(scratch2.IsValid(), "mexSimpleDetectorSteps1234", "Scratch2 could not be allocated");
...

  {
    ...
    AnkiConditionalErrorAndReturn(result == RESULT_OK, "mexSimpleDetectorSteps1234", "SimpleDetector_Steps1234 Failed");
  }
