...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (nrhs == 3 && nlhs == 1) {
    ...
    // static const s32 ONCHIP_BUFFER_SIZE = 170000; // The max here is
    // somewhere between 175000 and 180000 bytes static const s32 CCM_BUFFER_SIZE
    ...
    mxAssert(image.IsValid(),
             "Unable to create Array<u8> for image -- out of scratch memory?");

    Array<u8> imageNorm(nrows, ncols, offchipMemory);
    mxAssert(imageNorm.IsValid(),
             "Unable to create Array<u8> for output normalized image -- out of "
             "scratch memory?");
    ...
    Result lastResult = ImageProcessing::BoxFilterNormalize(
        image, boxSize, padValue, imageNorm, offchipMemory);

    mxAssert(lastResult == RESULT_OK,
             "ImageProcessing::BoxFilterNormalize failed!");

    plhs[0] = arrayToMxArray(imageNorm);
  } else {
    mexPrintf(
        "\n\n\timageNorm[UINT8] = mexBoxFilterNormalize(image[UINT8], "
        "boxWidth, padValue);\n\n");
