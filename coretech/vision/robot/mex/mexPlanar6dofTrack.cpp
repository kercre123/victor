...
static const s32 ONCHIP_BUFFER_SIZE =
    170000;  // The max here is somewhere between 175000 and 180000 bytes
static const s32 CCM_BUFFER_SIZE =
    50000;  // The max here is probably 65536 (0x10000) bytes
...
void NormalizeImage(Array<u8>& grayscaleImage,
                    const Quadrilateral<f32>& currentQuad,
                    const f32 filterWidthFraction, MemoryStack scratch) {
  // TODO: Add the ability to only normalize within the vicinity of the quad
  // Note that this requires templateQuad to be sorted!
  const s32 filterWidth = static_cast<s32>(
      filterWidthFraction * ((currentQuad[3] - currentQuad[0]).Length()));
  AnkiAssert(filterWidth > 0.f);

  Array<u8> grayscaleImageNormalized(grayscaleImage.get_size(0),
                                     grayscaleImage.get_size(1), scratch);

  mxAssert(grayscaleImageNormalized.IsValid(),
           "Out of memory allocating grayscaleImageNormalized.\n");

  Anki::Result lastResult = ImageProcessing::BoxFilterNormalize(
      grayscaleImage, filterWidth, static_cast<u8>(128),
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
  ...
  if (nrhs == 14 && (nlhs == 0 || nlhs == 1 || nlhs == 7)) {
    ...
    // Get Initial Image
    mxAssert(mxGetClassID(prhs[0]) == mxUINT8_CLASS, "Image should be UINT8.");
    mxAssert(mxGetNumberOfDimensions(prhs[0]) == 2, "Image must be grayscale.");
    ...
    grayscaleImage = Array<u8>(static_cast<s32>(nrows), static_cast<s32>(ncols),
                               onchipMemory);
    ...
    Quadrilateral<f32> templateQuad(
        Point2f(quadX[0], quadY[0]), Point2f(quadX[1], quadY[1]),
        Point2f(quadX[2], quadY[2]), Point2f(quadX[3], quadY[3]));
    ++argIndex;
    ...
    const f32 scaleTemplateRegionPercent =
        static_cast<f32>(mxGetScalar(prhs[argIndex++]));
    const s32 numPyramidLevels =
        static_cast<s32>(mxGetScalar(prhs[argIndex++]));
    ...
    const f32 fiducialSquareWidthFraction =
        static_cast<f32>(mxGetScalar(prhs[argIndex++]));
    const bool normalizeImage =
        static_cast<bool>(mxGetScalar(prhs[argIndex++]));

    if (normalizeImage) {
      NormalizeImage(grayscaleImage, templateQuad, filterWidthFraction,
                     offchipMemory);
    }

    tracker = TemplateTracker::LucasKanadeTracker_SampledPlanar6dof(
        grayscaleImage, templateQuad, scaleTemplateRegionPercent,
        numPyramidLevels, Transformations::TRANSFORM_PROJECTIVE,
        numFiducialSquareSamples, fiducialSquareWidthFraction,
        maxSamplesAtBaseLevel, numSamplingRegions, focalLength_x, focalLength_y,
        camCenter_x, camCenter_y, templateWidth_mm, ccmMemory, onchipMemory,
        offchipMemory);
    ...
    */
    ...
    if (nlhs > 0) {
      ...
      const FixedLengthList<
          LucasKanadeTracker_SampledPlanar6dof::TemplateSample>& samples =
          tracker.get_templateSamples(0);
      ...
      if (nlhs >= 7) {
        plhs[1] =
            mxCreateDoubleScalar(static_cast<double>(tracker.get_angleX()));
        plhs[2] =
            mxCreateDoubleScalar(static_cast<double>(tracker.get_angleY()));
        plhs[3] =
            mxCreateDoubleScalar(static_cast<double>(tracker.get_angleZ()));

        plhs[4] = mxCreateDoubleScalar(
            static_cast<double>(tracker.GetTranslation().x));
        plhs[5] = mxCreateDoubleScalar(
            static_cast<double>(tracker.GetTranslation().y));
        plhs[6] = mxCreateDoubleScalar(
            static_cast<double>(tracker.GetTranslation().z));
      }
    }
    ...
    const f32 convergenceTol_angle =
        static_cast<f32>(mxGetScalar(prhs[argIndex++]));
    const f32 convergenceTol_dist =
        static_cast<f32>(mxGetScalar(prhs[argIndex++]));
    const f32 verify_maxPixelDiff =
        static_cast<f32>(mxGetScalar(prhs[argIndex++]));
    const bool normalizeImage =
        static_cast<bool>(mxGetScalar(prhs[argIndex++]));
    ...
    Quadrilateral<f32> currentQuad =
        tracker.get_transformation().get_transformedCorners(onchipMemory);

    if (normalizeImage) {
      NormalizeImage(grayscaleImage, currentQuad, filterWidthFraction,
                     offchipMemory);
    }

    // Update tracker
    const Anki::Result trackerResult = tracker.UpdateTrack(
        grayscaleImage, maxIterations, convergenceTol_angle,
        convergenceTol_dist, verify_maxPixelDiff, converged, verify_meanAbsDiff,
        verify_numInBounds, verify_numSimilarPixels, onchipMemory);

    if (trackerResult != Anki::RESULT_OK) {
      char buffer[128];
      snprintf(buffer, 128, "UpdateTrack() failed with result = %x!",
               trackerResult);
    ...
    plhs[argOutIndex++] =
        mxCreateDoubleScalar(static_cast<double>(verify_meanAbsDiff));
    plhs[argOutIndex++] =
        mxCreateDoubleScalar(static_cast<double>(verify_numInBounds));
    plhs[argOutIndex++] =
        mxCreateDoubleScalar(static_cast<double>(verify_numSimilarPixels));
    ...
    Quadrilateral<f32> corners =
        tracker.get_transformation().get_transformedCorners(offchipMemory);
    for (s32 i = 0; i < 4; ++i) {
      xQuad[i] = corners[i].x;
      yQuad[i] = corners[i].y;
    }
    ...
    plhs[argOutIndex++] =
        mxCreateDoubleScalar(static_cast<double>(translation.x));
    plhs[argOutIndex++] =
        mxCreateDoubleScalar(static_cast<double>(translation.y));
    plhs[argOutIndex++] =
        mxCreateDoubleScalar(static_cast<double>(translation.z));
