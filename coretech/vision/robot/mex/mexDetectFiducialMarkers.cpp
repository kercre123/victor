...
// imageMarker =
// rgb2gray(imread('C:\Anki\blockImages\fiducial105_6ContrastReduced.png'));
// image = 255*ones(480,640,'uint8');
// image(101:(100+size(imageMarker,1)-2), 101:(100+size(imageMarker,2)-2)) =
// imageMarker(2:(end-1), 2:(end-1)); imageSize = [480,640];

// image =
// imresize(rgb2gray(imread('C:\Anki\blockImages\testTrainedCodes.png')),
// [240,320]);
...
// component_minimumNumPixels = round(minSideLength*minSideLength -
// (0.8*minSideLength)*(0.8*minSideLength)); component_maximumNumPixels =
// round(maxSideLength*maxSideLength - (0.8*maxSideLength)*(0.8*maxSideLength));
...
// cornerMethod = 0;
// [quads, markerTypes, markerNames, markerValidity] =
// mexDetectFiducialMarkers(image, useIntegralImageFiltering,
...
// component_minHollowRatio, minLaplacianPeakRatio, quads_minQuadArea,
// quads_quadSymmetryThreshold, quads_minDistanceFromImageEdge,
// decode_minContrastRatio, quadRefinementIterations, numRefinementSamples,
// quadRefinementMaxCornerChange, quadRefinementMinCornerChange,
// returnInvalidMarkers, cornerMethod);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  params.maxConnectedComponentSegments =
      0xFFFFF;  // 0xFFFFF is a little over one million

  const s32 bufferSize = 100000000;

  AnkiConditionalErrorAndReturn(
      nrhs >= 21 && nrhs <= 22 && nlhs >= 2 && nlhs <= 4,
      "mexDetectFiducialMarkers",
      "Call this function as following: [quads, markerTypes, <markerNames>, "
      "<markerValidity>] = mexDetectFiducialMarkers(uint8(image), "
      "useIntegralImageFiltering, scaleImage_numPyramidLevels, "
      "scaleImage_thresholdMultiplier, component1d_minComponentWidth, "
      "component1d_maxSkipDistance, component_minimumNumPixels, "
      "component_maximumNumPixels, component_sparseMultiplyThreshold, "
      "component_solidMultiplyThreshold, component_minHollowRatio, "
      "minLaplacianPeakRatio, quads_minQuadArea, quads_quadSymmetryThreshold, "
      "quads_minDistanceFromImageEdge, decode_minContrastRatio, "
      "quadRefinementIterations, numRefinementSamples, "
      "quadRefinementMaxCornerChange, quadRefinementMinCornerChange, "
      "returnInvalidMarkers, <cornerMethod>);");

  MemoryStack memory(mxMalloc(bufferSize), bufferSize);
  AnkiConditionalErrorAndReturn(memory.IsValid(), "mexDetectFiducialMarkers",
                                "Memory could not be allocated");

  Array<u8> image = mxArrayToArray<u8>(prhs[0], memory);
  params.useIntegralImageFiltering =
      static_cast<bool>(Round<s32>(mxGetScalar(prhs[1])));
  params.useIlluminationNormalization = true;
  params.scaleImage_numPyramidLevels = static_cast<s32>(mxGetScalar(prhs[2]));
  params.scaleImage_thresholdMultiplier = Round<s32>(
      pow(2.0, 16) * mxGetScalar(prhs[3]));  // Convert from double to SQ15.16
  ...
  params.component_sparseMultiplyThreshold = Round<s32>(
      pow(2.0, 5) * mxGetScalar(prhs[8]));  // Convert from double to SQ26.5
  params.component_solidMultiplyThreshold = Round<s32>(
      pow(2.0, 5) * mxGetScalar(prhs[9]));  // Convert from double to SQ26.5
  ...
  params.quads_quadSymmetryThreshold = Round<s32>(
      pow(2.0, 8) * mxGetScalar(prhs[13]));  // Convert from double to SQ23.8
  params.quads_minDistanceFromImageEdge =
      static_cast<s32>(mxGetScalar(prhs[14]));
  params.decode_minContrastRatio = static_cast<f32>(mxGetScalar(prhs[15]));
  params.refine_quadRefinementIterations =
      static_cast<s32>(mxGetScalar(prhs[16]));
  params.refine_numRefinementSamples = static_cast<s32>(mxGetScalar(prhs[17]));
  params.refine_quadRefinementMaxCornerChange =
      static_cast<f32>(mxGetScalar(prhs[18]));
  params.refine_quadRefinementMinCornerChange =
      static_cast<f32>(mxGetScalar(prhs[19]));
  params.returnInvalidMarkers =
      static_cast<bool>(Round<s32>(mxGetScalar(prhs[20])));
  ...
  params.cornerMethod = CORNER_METHOD_LINE_FITS;  // {CORNER_METHOD_LAPLACIAN_PEAKS,
                                                  // CORNER_METHOD_LINE_FITS};
  if (nrhs >= 21) {
    params.cornerMethod =
        static_cast<CornerMethod>(Round<s32>(mxGetScalar(prhs[21])));
  }

  AnkiConditionalErrorAndReturn(image.IsValid(), "mexDetectFiducialMarkers",
                                "Could not allocate image");

  MemoryStack scratch0(mxCalloc(bufferSize, 1), bufferSize);
  AnkiConditionalErrorAndReturn(scratch0.IsValid(), "mexDetectFiducialMarkers",
                                "Scratch0 could not be allocated");

  MemoryStack scratch1(mxCalloc(bufferSize, 1), bufferSize);
  AnkiConditionalErrorAndReturn(scratch1.IsValid(), "mexDetectFiducialMarkers",
                                "Scratch1 could not be allocated");

  MemoryStack scratch2(mxCalloc(bufferSize, 1), bufferSize);
  AnkiConditionalErrorAndReturn(scratch2.IsValid(), "mexDetectFiducialMarkers",
                                "Scratch2 could not be allocated");

  MemoryStack scratch3(mxCalloc(bufferSize, 1), bufferSize);
  AnkiConditionalErrorAndReturn(scratch3.IsValid(), "mexDetectFiducialMarkers",
                                "Scratch3 could not be allocated");
...

  {
    const Anki::Result result = DetectFiducialMarkers(
        image, markers, params, scratch1, scratch2, scratch3);

    AnkiConditionalErrorAndReturn(result == Anki::RESULT_OK,
                                  "mexDetectFiducialMarkers",
                                  "mexDetectFiducialMarkers Failed");
  }
  ...
  if (numMarkers != 0) {
    ...
    mxArray *quadsMatlab =
        mxCreateCellArray(markersMatlab_ndim, markersMatlab_dims);
    ...
    if (nlhs >= 3) {
      mxArray *markerNamesMatlab =
          mxCreateCellArray(markersMatlab_ndim, markersMatlab_dims);
      for (s32 i = 0; i < numMarkers; ++i) {
        AnkiAssert(markers[i].markerType >= 0 &&
                   markers[i].markerType <= Anki::Vision::NUM_MARKER_TYPES);
        mxSetCell(markerNamesMatlab, i,
                  mxCreateString(
    ...
    mxArray *quadsMatlab =
        mxCreateCellArray(markersMatlab_ndim, markersMatlab_dims);
    mxArray *markerTypesMatlab =
        mxCreateNumericArray(2, markersMatlab_dims, mxDOUBLE_CLASS, mxREAL);
    ...
    if (nlhs >= 4) {
      plhs[3] =
          mxCreateNumericArray(2, markersMatlab_dims, mxINT32_CLASS, mxREAL);
    }
