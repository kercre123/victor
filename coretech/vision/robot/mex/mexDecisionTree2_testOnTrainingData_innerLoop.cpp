...
static void runTree(
    const u8 *restrict pFeatureValues, const s32 *restrict pWhichFeatures,
    const u8 *restrict pU8Thresholds, const s32 *restrict pLeftChildIndexs,
    const s32 *restrict pSamplePositions,
    const s32 *restrict pGroundTruthLabels, const s32 maxLabelId,
    s32 *restrict pNumCorrect, s32 *restrict pNumTotal, s32 *restrict pLabelIds,
    const s64 numFeatures, const s64 numImages, const s64 treeLength) {
  for (s64 iImage = 0; iImage < numImages; iImage++) {
    const u8 *restrict pFeatureValuesCur =
        pFeatureValues + iImage * numFeatures;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  AnkiConditionalErrorAndReturn(
      nrhs == 7 && nlhs == 3, "mexDecisionTree2_testOnTrainingData_innerLoop",
      "Call this function as follows: [numCorrect, numTotal, labelIds] = "
      "mexDecisionTree2_testOnTrainingData_innerLoop(featureValues, "
      "cTree_whichFeatures, cTree_u8Thresholds, cTree_leftChildIndexs, "
      "samplePositions, groundTruthLabels, numThreads);");
  ...
  AnkiConditionalErrorAndReturn(
      mxGetN(prhs[1]) == 1 && mxGetN(prhs[2]) == 1 && mxGetN(prhs[3]) == 1 &&
          mxGetN(prhs[4]) == 1 && mxGetN(prhs[5]) == 1,
      "mexDecisionTree2_testOnTrainingData_innerLoop", "Incorrect input size");

  AnkiConditionalErrorAndReturn(
      treeLength == mxGetM(prhs[1]) && treeLength == mxGetM(prhs[2]) &&
          treeLength == mxGetM(prhs[3]) && numFeatures == mxGetM(prhs[4]) &&
          numImages == mxGetM(prhs[5]),
      "mexDecisionTree2_testOnTrainingData_innerLoop", "Incorrect input size");

  AnkiConditionalErrorAndReturn(mxGetM(prhs[0]) > 0,
                                "mexDecisionTree2_testOnTrainingData_innerLoop",
                                "Incorrect input size");

  // NOTE: Theoretically, this could be reasonable, and not due to a transpose
  // mistake
  AnkiConditionalErrorAndReturn(numFeatures < numImages,
                                "mexDecisionTree2_testOnTrainingData_innerLoop",
                                "Possible transpose mistake");
  ...
  AnkiConditionalErrorAndReturn(mxGetClassID(prhs[0]) == mxUINT8_CLASS &&
                                    ...
                                    mxGetClassID(prhs[5]) == mxINT32_CLASS,
                                "mexDecisionTree2_testOnTrainingData_innerLoop",
                                "Incorrect input types");

  const u8 *restrict pFeatureValues =
      reinterpret_cast<u8 *>(mxGetData(prhs[0]));
  const s32 *restrict pWhichFeatures =
      reinterpret_cast<s32 *>(mxGetData(prhs[1]));
  const u8 *restrict pU8Thresholds = reinterpret_cast<u8 *>(mxGetData(prhs[2]));
  const s32 *restrict pLeftChildIndexs =
      reinterpret_cast<s32 *>(mxGetData(prhs[3]));
  const s32 *restrict pSamplePositions =
      reinterpret_cast<s32 *>(mxGetData(prhs[4]));
  const s32 *restrict pGroundTruthLabels =
      reinterpret_cast<s32 *>(mxGetData(prhs[5]));
  ...
  runTree(pFeatureValues, pWhichFeatures, pU8Thresholds, pLeftChildIndexs,
          pSamplePositions, pGroundTruthLabels, maxLabelId, pNumCorrect,
          pNumTotal, pLabelIds, numFeatures, numImages, treeLength);
