...
Result DetectFiducialMarkers(const Array<u8> &image,
                             ...
                             const bool isDarkOnLight, MemoryStack scratchCcm,
...
// Warning: fastScratch and slowScratch cannot be the same object pointing to
// the same memory
Result ExtractComponentsViaCharacteristicScale(
    const Array<u8> &image, const FixedLengthList<s32> &filterHalfWidths,
    const s32 scaleImage_thresholdMultiplier,
    const s16 component1d_minComponentWidth,
    const s16 component1d_maxSkipDistance, const bool isDarkOnLight,
    ConnectedComponents &components, MemoryStack fastScratch,
    MemoryStack slowerScratch, MemoryStack slowestScratch);

Result ExtractComponentsViaCharacteristicScale_binomial(
    const Array<u8> &image, const s32 numPyramidLevels,
    const s32 scaleImage_thresholdMultiplier,
    const s16 component1d_minComponentWidth,
    const s16 component1d_maxSkipDistance, ConnectedComponents &components,
    MemoryStack fastScratch, MemoryStack slowerScratch,
    MemoryStack slowestScratch);

// Used by DetectFiducialMarkers
//
// Extracts quadrilaterals from a list of connected component segments
Result ComputeQuadrilateralsFromConnectedComponents(
    const ConnectedComponents &components, const s32 minQuadArea,
    const s32 quadSymmetryThreshold, const s32 minDistanceFromImageEdge,
    const s32 minLaplacianPeakRatio, const s32 imageHeight,
    const s32 imageWidth, const CornerMethod cornerMethod,
    FixedLengthList<Quadrilateral<s16> > &extractedQuads, MemoryStack scratch);

// Does the input quad (with corners in canonical order) have a reasonable
// shape?
//
...
bool IsQuadrilateralReasonable(const Quadrilateral<s16> &quad,
                               const s32 minQuadArea,
                               const s32 quadSymmetryThreshold,
                               const s32 minDistanceFromImageEdge,
...
// Extract the best Laplacian peaks from boundary, up to peaks.get_size() The
// top peaks.get_size() peaks are returned in the order of their original index,
// which preserves their original clockwise or counter-clockwise ordering. The
// ratio of the 4th peak to the 5th peak must exceed minPeakRatio or peaks will
// be empty.
//
...
// Extract the best peaks, using the line fits method. Works with curved corner
// fiducials
...
Result RefineQuadrilateral(const Quadrilateral<f32> &initialQuad,
                           ...
                           const s32 maxIterations, const f32 darkValue,
                           const f32 brightValue, const s32 numSamples,
