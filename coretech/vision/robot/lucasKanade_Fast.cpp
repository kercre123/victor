...
LucasKanadeTracker_Fast::LucasKanadeTracker_Fast(
    ...
    : LucasKanadeTracker_Generic(maxSupportedTransformType, templateImage,
  ...
  templateCoordinates =
      FixedLengthList<Meshgrid<f32> >(numPyramidLevels, memory);
  templateImagePyramid = FixedLengthList<Array<u8> >(numPyramidLevels, memory);
  templateImageXGradientPyramid =
      FixedLengthList<Array<s16> >(numPyramidLevels, memory);
  templateImageYGradientPyramid =
      FixedLengthList<Array<s16> >(numPyramidLevels, memory);
  ...
  for (s32 iScale = 0; iScale < numPyramidLevels; iScale++) {
    ...
    templateImageXGradientPyramid[iScale] =
        Array<s16>(numPointsY, numPointsX, memory);
  ...
  for (s32 iScale = 0; iScale < numPyramidLevels; iScale++) {
    if ((lastResult = Interp2_Affine<u8, u8>(
             ...
             this->templateImagePyramid[iScale], INTERPOLATE_LINEAR)) !=
        RESULT_OK) {
      AnkiError("LucasKanadeTracker_Fast::LucasKanadeTracker_Fast",
  ...
  for (s32 i = 0; i < numPyramidLevels; i++) {
    if ((lastResult = ImageProcessing::ComputeXGradient<u8, s16, s16>(
             templateImagePyramid[i], templateImageXGradientPyramid[i])) !=
        RESULT_OK) {
      AnkiError("LucasKanadeTracker_Fast::LucasKanadeTracker_Fast",
    ...
    if ((lastResult = ImageProcessing::ComputeYGradient<u8, s16, s16>(
             templateImagePyramid[i], templateImageYGradientPyramid[i])) !=
        RESULT_OK) {
      AnkiError("LucasKanadeTracker_Fast::LucasKanadeTracker_Fast",
...
bool LucasKanadeTracker_Fast::IsValid() const {
  ...
  if (!templateImagePyramid.IsValid()) return false;

  if (!templateImageXGradientPyramid.IsValid()) return false;

  if (!templateImageYGradientPyramid.IsValid()) return false;

  for (s32 i = 0; i < numPyramidLevels; i++) {
    if (!templateImagePyramid[i].IsValid()) return false;

    if (!templateImageXGradientPyramid[i].IsValid()) return false;

    if (!templateImageYGradientPyramid[i].IsValid()) return false;
  }
...
Result LucasKanadeTracker_Fast::VerifyTrack_Projective(
  ...
  for (s32 y = 0; y < yIterationMax; y++) {
    ...
    // this->templateImageXGradientPyramid[whichScale].Pointer(y, 0); const s16 *
    // restrict pTemplateImageYGradient =
  ...
  if (verify_numInBounds == 0) {
    ...
    verify_meanAbsoluteDifference =
        totalGrayvalueDifference / verify_numInBounds;
  }
