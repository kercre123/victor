...
Various utilities for transforming coordinates between different reference
frames
...
//
// The first byte is the degrees of freedom of the transformation, so if it
// bit-shifted right by 8, it is equal to the number of parameters
enum TransformType {
...
// Simple helper function to compute the homography from an input quad. The
// input quad point should be ordered in the non-rotated, corner-opposite format
...
// homography transforms the corners from originalQuad into those in
// transformedQuad
...
// 1. Hold the current planar transformation, and optionally the initial extents
// of the quadrilateral
// 2. Update the planar transformation with an update delta
// 3. Transform a set of points, quadrilateral, or image to the new coordinate
// frame
//
...
class PlanarTransformation_f32 {
 public:
  // Initialize with input corners and homography, or if not input, set to zero
  // or identity If the input corners and homography are input, they are copied
  // to object-local copies
  PlanarTransformation_f32(const TransformType transformType,
  ...
  Result TransformPoints(const Array<f32> &xIn, const Array<f32> &yIn,
                         const f32 scale, const bool inputPointsAreZeroCentered,
  ...
  Result Update(const Array<f32> &update, const f32 scale, MemoryStack scratch,
                TransformType updateType = TRANSFORM_UNKNOWN);
  ...
  Quadrilateral<f32> Transform(const Quadrilateral<f32> &in,
                               MemoryStack scratch,
                               const f32 scale = 1.0f) const;

  // Transform an array (like an image)
  Result Transform(const Array<u8> &in, Array<u8> &out, MemoryStack scratch,
                   const f32 scale = 1.0f) const;

  // In and out can point to the same location in memory
  Result Transform(const FixedLengthList<Point<s16> > &in,
                   FixedLengthList<Point<s16> > &out, MemoryStack scratch,
                   const f32 scale = 1.0f) const;
  ...
  Result VerifyTransformation_Projective_NearestNeighbor(
      ...
      const Array<u8> &nextImage, const IntegerCounts &nextImageHistogram,
  ...
  Result VerifyTransformation_Projective_LinearInterpolate(
      const Array<u8> &templateImage,
      const Rectangle<f32> &templateRegionOfInterest,
      const Array<u8> &nextImage, const f32 templateRegionHeight,
      const f32 templateRegionWidth, const s32 templateCoordinateIncrement,
      const u8 maxPixelDifference, s32 &meanAbsoluteDifference,
      s32 &numInBounds, s32 &numSimilarPixels, MemoryStack scratch) const;

  bool IsValid() const;

  // Set this object's transformType, centerOffset, initialCorners, and
  // homography
  ...
  Result SerializeRaw(const char *objectName, void **buffer, s32 &bufferLength)
      const;  // Updates the buffer pointer and length before returning
  Result Deserialize(char *objectName, void **buffer, s32 &bufferLength,
                     MemoryStack &memory);  // Updates the buffer pointer and
  ...
  // Orientation is [0,-pi] for counter-clockwise rotation, and [0,pi] for
  // clockwise rotation
 ...
 protected:
  ...
  // All types of plane transformations are stored in a 3x3 homography matrix,
  // though some values may be zero (or ones for diagonals)
  Array<f32> homography;
  ...
  // The offset applied to an image, so that origin of the coordinate system is
  // at the center of the quadrilateral
  Point<f32> centerOffset;

  static Result TransformPointsStatic(const Array<f32> &xIn,
                                      const Array<f32> &yIn, const f32 scale,
  ...
  Result Init(const TransformType transformType,
              const Quadrilateral<f32> &initialCorners,
              const Array<f32> &initialHomography,
              const Point<f32> &centerOffset, MemoryStack &memory);
};
