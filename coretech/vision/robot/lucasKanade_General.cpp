...
f32 UpdatePreviousCorners(
  ...
  Quadrilateral<f32> in(Point<f32>(-baseImageHalfWidth, -baseImageHalfHeight),
  ...
  f32 minChange = 1e10f;
  for (s32 iPrevious = 0; iPrevious < NUM_PREVIOUS_QUADS_TO_COMPARE;
       iPrevious++) {
    f32 change = 0.0f;
    for (s32 i = 0; i < 4; i++) {
  ...
  for (s32 iPrevious = 0; iPrevious < (NUM_PREVIOUS_QUADS_TO_COMPARE - 1);
       iPrevious++) {
...
LucasKanadeTracker_Generic::LucasKanadeTracker_Generic(
  ...
  AnkiConditionalErrorAndReturn(
      templateImageHeight > 0 && templateImageWidth > 0,
      "LucasKanadeTracker_Generic::LucasKanadeTracker_Generic",
      "template widths and heights must be greater than zero, and multiples of "
      "%d",
      ANKI_VISION_IMAGE_WIDTH_MULTIPLE);

  AnkiConditionalErrorAndReturn(
      numPyramidLevels > 0,
      "LucasKanadeTracker_Generic::LucasKanadeTracker_Generic",
      "numPyramidLevels must be greater than zero");

  AnkiConditionalErrorAndReturn(
      transformType <= maxSupportedTransformType,
      "LucasKanadeTracker_Generic::LucasKanadeTracker_Generic",
      "Transform type %d not supported", transformType);
  ...
  AnkiConditionalErrorAndReturn(
      ((1 << initialImagePowerS32) * templateImage.get_size(1)) ==
          baseImageWidth,
      "LucasKanadeTracker_Generic::LucasKanadeTracker_Generic",
  ...
  templateRegion =
      templateQuad.ComputeBoundingRectangle<f32>().ComputeScaledRectangle<f32>(
  ...
  for (s32 i = 0; i < (numPyramidLevels - 1); i++) {
    ...
    AnkiConditionalErrorAndReturn(
        !IsOdd(curTemplateHeight) && !IsOdd(curTemplateWidth),
        "LucasKanadeTracker_Generic::LucasKanadeTracker_Generic",
        "Template widths and height must divisible by 2^numPyramidLevels");
  }

  this->templateRegionHeight =
      templateRegion.bottom - templateRegion.top + 1.0f;
  this->templateRegionWidth = templateRegion.right - templateRegion.left + 1.0f;

  this->transformation = Transformations::PlanarTransformation_f32(
      transformType, templateQuad, memory);
...
bool LucasKanadeTracker_Generic::IsValid() const {
  ...
  if (!this->transformation.IsValid()) return false;

  return true;
}

Result LucasKanadeTracker_Generic::UpdateTransformation(
    const Array<f32> &update, const f32 scale, MemoryStack scratch,
    Transformations::TransformType updateType) {
...
Result LucasKanadeTracker_Generic::set_transformation(
    const Transformations::PlanarTransformation_f32 &transformation) {
  Result lastResult;

  const Transformations::TransformType originalType =
      this->transformation.get_transformType();

  if ((lastResult = this->transformation.set_transformType(
           transformation.get_transformType())) != RESULT_OK) {
    this->transformation.set_transformType(originalType);
    return lastResult;
  }

  if ((lastResult = this->transformation.set_homography(
           transformation.get_homography())) != RESULT_OK) {
    this->transformation.set_transformType(originalType);
    return lastResult;
  }
...
Transformations::PlanarTransformation_f32
LucasKanadeTracker_Generic::get_transformation() const {
