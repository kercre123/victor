...
LucasKanadeTracker_Affine::LucasKanadeTracker_Affine()
    : LucasKanadeTracker_Fast(Transformations::TRANSFORM_AFFINE) {}
...
LucasKanadeTracker_Affine::LucasKanadeTracker_Affine(
    ...
    const Transformations::TransformType transformType, MemoryStack &memory)
    : LucasKanadeTracker_Fast(Transformations::TRANSFORM_AFFINE, templateImage,
                              templateQuad, scaleTemplateRegionPercent,
                              numPyramidLevels, transformType, memory) {
  this->isValid = true;
}
...
Result LucasKanadeTracker_Affine::UpdateTrack(
    ...
    s32 &verify_meanAbsoluteDifference,  //< For all pixels in the template,
                                         //compute the mean difference between
                                         //the template and the final warped
                                         //template
    s32 &verify_numInBounds,       // How many template pixels are in the image,
                                   // after the template is warped?
    s32 &verify_numSimilarPixels,  //< For all pixels in the template, how many
                                   //are within verifyMaxPixelDifference
                                   //grayvalues? Use in conjunction with
                                   //get_numTemplatePixels() for a percentage.
    MemoryStack scratch) {
  Result lastResult;
  ...
  // lastResult = this->VerifyTrack_Projective(nextImage,
  // verify_maxPixelDifference, verify_meanAbsoluteDifference,
...
Result LucasKanadeTracker_Affine::IterativelyRefineTrack(
  ...
  AnkiConditionalErrorAndReturnValue(
      ...
      "LucasKanadeTracker_Affine::IterativelyRefineTrack",
  ...
  return RESULT_FAIL;
}  // Result LucasKanadeTracker_Affine::IterativelyRefineTrack(const Array<u8>
   // &nextImage, const s32 maxIterations, const s32 whichScale, const f32
   // convergenceTolerance, const TransformType curTransformType, bool
...
Result LucasKanadeTracker_Affine::IterativelyRefineTrack_Translation(
  ...
  // The call would be like: Interp2_Affine<u8,u8>(nextImage,
  ...
  for (s32 iteration = 0; iteration < maxIterations; iteration++) {
    ...
    for (s32 y = 0; y < yIterationMax; y++) {
      const u8 *restrict pTemplateImage =
          this->templateImagePyramid[whichScale].Pointer(y, 0);

      const s16 *restrict pTemplateImageXGradient =
          this->templateImageXGradientPyramid[whichScale].Pointer(y, 0);
      const s16 *restrict pTemplateImageYGradient =
          this->templateImageYGradientPyramid[whichScale].Pointer(y, 0);

      f32 xOriginal = xGridStart;

      // TODO: This could be strength-reduced further, but it wouldn't be much
      // faster
      f32 xTransformed =
          h00 * xOriginal + h01 * yOriginal + h02 + centerOffsetScaled.x;
      f32 yTransformed =
          h10 * xOriginal + h11 * yOriginal + h12 + centerOffsetScaled.y;
      ...
      for (s32 x = 0; x < xIterationMax; x++) {
        ...
        if (x0 < xyReferenceMin || x1 > xReferenceMax || y0 < xyReferenceMin ||
            y1 > yReferenceMax) {
          // strength reduction for the affine transformation along this
          // horizontal line
        ...
        const f32 interpolatedPixelF32 = InterpolateBilinear2d<f32>(
            pixelTL, pixelTR, pixelBL, pixelBR, alphaY, alphaYinverse, alphaX,
            alphaXinverse);
        ...
        // This block is the non-interpolation part of the per-sample algorithm
        {
          ...
          const f32 xGradientValue =
              scaleOverFiveTen * static_cast<f32>(pTemplateImageXGradient[x]);
          const f32 yGradientValue =
              scaleOverFiveTen * static_cast<f32>(pTemplateImageYGradient[x]);
    ...
    if (numInBounds < 16) {
      AnkiWarn("LucasKanadeTracker_Affine::IterativelyRefineTrack_Translation",
...
Result LucasKanadeTracker_Affine::IterativelyRefineTrack_Affine(
  ...
  for (s32 iteration = 0; iteration < maxIterations; iteration++) {
    ...
    if (numInBounds < 16) {
      AnkiWarn("LucasKanadeTracker_Affine::IterativelyRefineTrack_Affine",
               "Template drifted too far out of image.");
      return RESULT_OK;
    }
