...
#define NEAREST_NEIGHBOR_DISTANCE_THRESHOLD \
  50  // TODO: Make this a VisionParameter and pass it in dynamically
#endif

//#define SHOW_DRAWN_COMPONENTS // Requires vision system to be in synchronous
//mode #define SEND_COMPONENTS_TO_MATLAB

namespace Anki {
namespace Embedded {

static Result IlluminationNormalization(
    ...
    cv::Mat_<u8>
        &cvImageROI_orig)  // stores a copy of the original data inside the ROI
{
  ...
  AnkiConditionalErrorAndReturnValue(
      cvImageROI.empty() == false, RESULT_FAIL_INVALID_SIZE,
  ...
  // NOTE: it's a tad slower to use s16, but we seem to get better performance
  // with it
  ...
  const s32 kernelSize = 0.1061f*((corners[Quadrilateral<f32>::TopLeft] -
  corners[Quadrilateral<f32>::BottomRight]).Length() +
                                  (corners[Quadrilateral<f32>::TopRight] -
  corners[Quadrilateral<f32>::BottomLeft]).Length());
  */

  // Kernel size should be twice the thickness of the fiducial. We use the
  // average diagonal/sqrt(2) to estimate the fiducial width, and since the
  // thickness is 10% of the width, we use 0.2/sqrt(2)=0.14 as the multiplier.
  // This kernel size assumes we RE-filter the extracted probe values inside the
  // nearest neighbor code.
  const f32 avgThicknessFraction =
      0.5f * (fiducialThicknessFraction.x + fiducialThicknessFraction.y);
  const f32 kernelSize = std::round(1.4142f * avgThicknessFraction *
                                    ((corners[Quadrilateral<f32>::TopLeft] -
                                      corners[Quadrilateral<f32>::BottomRight])
                                         .Length() +
                                     (corners[Quadrilateral<f32>::TopRight] -
                                      corners[Quadrilateral<f32>::BottomLeft])
                                         .Length()));

  cv::boxFilter(cvImageROI, cvImageROI_filtered, cvImageROI_filtered.depth(),
                cv::Size(kernelSize, kernelSize));

  // Works better to maintain s16 "precision" here and then only to lose it when
  // we normalize below
...
Result DetectFiducialMarkers(const Array<u8> &image,
  ...
  AnkiAssert(params.fiducialThicknessFraction.x > 0 &&
             params.fiducialThicknessFraction.y > 0);
  ...
  AnkiConditionalErrorAndReturnValue(
      image.IsValid() && markers.IsValid() && scratchOffChip.IsValid() &&
          scratchOnchip.IsValid() && scratchCcm.IsValid(),
      RESULT_FAIL_INVALID_OBJECT, "DetectFiducialMarkers",
      "Some input is invalid");

  // On the robot, we don't have enough memory for resolutions over QVGA
  if (scratchOffChip.get_totalBytes() < 1000000 &&
      scratchOnchip.get_totalBytes() < 1000000 &&
      scratchCcm.get_totalBytes() < 1000000) {
    AnkiConditionalErrorAndReturnValue(
        imageHeight <= 240 && imageWidth <= 320, RESULT_FAIL_INVALID_SIZE,
        "DetectFiducialMarkers", "The image is too large to test");

    AnkiConditionalErrorAndReturnValue(
        params.scaleImage_numPyramidLevels <= 3, RESULT_FAIL_INVALID_SIZE,
        "DetectFiducialMarkers", "Only 3 pyramid levels are supported");
  }
  ...
  if (params.maxConnectedComponentSegments <= std::numeric_limits<u16>::max()) {
    extractedComponents = ConnectedComponents(
        params.maxConnectedComponentSegments, imageWidth, true, scratchOffChip);
  } else {
    extractedComponents =
        ConnectedComponents(params.maxConnectedComponentSegments, imageWidth,
                            false, scratchOffChip);
  }

  AnkiConditionalErrorAndReturnValue(
      extractedComponents.IsValid(), RESULT_FAIL_OUT_OF_MEMORY,
      "DetectFiducialMarkers", "extractedComponents could not be allocated");

  if (params.useIntegralImageFiltering) {
    FixedLengthList<s32> filterHalfWidths(
        params.scaleImage_numPyramidLevels + 2, scratchOnchip,
        Flags::Buffer(false, false, true));

    AnkiConditionalErrorAndReturnValue(
        filterHalfWidths.IsValid(), RESULT_FAIL_OUT_OF_MEMORY,
        "DetectFiducialMarkers", "filterHalfWidths could not be allocated");
    ...
    if ((lastResult = ExtractComponentsViaCharacteristicScale(
             image, filterHalfWidths, params.scaleImage_thresholdMultiplier,
             params.component1d_minComponentWidth,
             params.component1d_maxSkipDistance, isDarkOnLight,
             extractedComponents, scratchCcm, scratchOnchip, scratchOffChip)) !=
        RESULT_OK) {
    ...
    if ((lastResult = ExtractComponentsViaCharacteristicScale_binomial(
             image, params.scaleImage_numPyramidLevels,
             params.scaleImage_thresholdMultiplier,
             params.component1d_minComponentWidth,
             params.component1d_maxSkipDistance, extractedComponents,
  ...
  {
    const s32 bigScratchSize =
        1024 +
        image.get_size(0) * RoundUp<s32>(image.get_size(1), MEMORY_ALIGNMENT);
  ...
  {  // 3b. Remove poor components
    ...
    if ((lastResult = extractedComponents.InvalidateSmallOrLargeComponents(
             params.component_minimumNumPixels,
             params.component_maximumNumPixels, scratchOnchip)) != RESULT_OK)
      return lastResult;
    ...
    if ((lastResult = extractedComponents.InvalidateSolidOrSparseComponents(
             params.component_sparseMultiplyThreshold,
             params.component_solidMultiplyThreshold, scratchOnchip)) !=
        RESULT_OK)
      return lastResult;
    ...
    if ((lastResult =
             extractedComponents.InvalidateFilledCenterComponents_hollowRows(
                 params.component_minHollowRatio, scratchOnchip)) != RESULT_OK)
      return lastResult;
...
#ifdef SEND_COMPONENTS_TO_MATLAB
  {
    ...
    if (maxConnectedComponentSegments <= std::numeric_limits<u16>::max()) {
      const ConnectedComponentSegment<u16> *restrict pComponents =
          extractedComponents.get_componentsU16()->Pointer(0);
  ...
  // 4. Compute candidate quadrilaterals from the connected components
  {
    ...
    FixedLengthList<Quadrilateral<s16> > extractedQuads(
        params.maxExtractedQuads, scratchOnchip);

    if ((lastResult = ComputeQuadrilateralsFromConnectedComponents(
             extractedComponents, params.quads_minQuadArea,
             params.quads_quadSymmetryThreshold,
             params.quads_minDistanceFromImageEdge,
             params.minLaplacianPeakRatio, imageHeight, imageWidth,
             params.cornerMethod, extractedQuads, scratchOnchip)) != RESULT_OK)
      return lastResult;
    ...
    for (s32 iQuad = 0; iQuad < extractedQuads.get_size(); iQuad++) {
      ...
      if ((lastResult = Transformations::ComputeHomographyFromQuad(
               extractedQuads[iQuad], currentHomography, numericalFailure,
      ...
      if (numericalFailure) {
        ...
        if (currentHomography[2][0] > maxProjectiveTermValue ||
            currentHomography[2][1] > maxProjectiveTermValue) {
          AnkiWarn("DetectFiducialMarkers",
                   "Homography projective terms are unreasonably large");
          currentMarker.validity = VisionMarker::NUMERICAL_FAILURE;
        }
      }
  ...
  // refinedHomography and meanGrayvalueThreshold are computed by
  // currentMarker.RefineCorners(), then used by currentMarker.Extract()
  ...
  for (s32 iMarker = 0; iMarker < markers.get_size(); iMarker++) {
    ...
    if (params.useIlluminationNormalization) {
      lastResult = IlluminationNormalization(
          currentMarker.corners, params.fiducialThicknessFraction, cvImage,
          cvImageROI, cvImageROI_orig);
      if (lastResult != RESULT_OK) {
        AnkiWarn("DetectFiducialMarkers",
                 "Illumination normalization failed, skipping marker %d",
                 iMarker);
        continue;
      }
    }
    ...
    if (currentMarker.validity == VisionMarker::UNKNOWN) {
      // If refine_quadRefinementIterations > 0, then make this marker's corners
      // more accurate
      lastResult = currentMarker.RefineCorners(
          image, params.decode_minContrastRatio,
          params.refine_quadRefinementIterations,
          params.refine_numRefinementSamples,
          params.refine_quadRefinementMaxCornerChange,
          params.refine_quadRefinementMinCornerChange, params.quads_minQuadArea,
          params.quads_quadSymmetryThreshold,
          params.quads_minDistanceFromImageEdge,
          params.fiducialThicknessFraction, params.roundedCornersFraction,
          isDarkOnLight, meanGrayvalueThreshold, scratchOnchip);

      if (params.useIlluminationNormalization) {
        // Put back the original (non-illumination-normalized) pixel values
        // within the ROI
      ...
      if (lastResult == RESULT_OK) {
        ...
        if (currentMarker.validity == VisionMarker::LOW_CONTRAST) {
          ...
          // the marker
          lastResult = currentMarker.Extract(
              image,
#if RECOGNITION_METHOD == RECOGNITION_METHOD_NEAREST_NEIGHBOR
...
#endif
              params.decode_minContrastRatio, scratchOnchip);
          ...
          if (currentMarker.markerType == Vision::MARKER_INVALID &&
      ...
      // Put back the original (non-illumination-normalized) pixel values within
      // the ROI
      cvImageROI_orig.copyTo(cvImageROI);
    }
