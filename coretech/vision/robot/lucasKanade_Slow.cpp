...
LucasKanadeTracker_Slow::LucasKanadeTracker_Slow(
    ...
    const Transformations::TransformType transformType, const f32 ridgeWeight,
    MemoryStack &memory)
  ...
  AnkiConditionalErrorAndReturn(
      numPyramidLevels > 0, "LucasKanadeTracker_Slow::LucasKanadeTracker_Slow",
      "numPyramidLevels must be greater than zero");

  AnkiConditionalErrorAndReturn(
      transformType == Transformations::TRANSFORM_TRANSLATION ||
          transformType == Transformations::TRANSFORM_AFFINE ||
          transformType == Transformations::TRANSFORM_PROJECTIVE,
      "LucasKanadeTracker_Slow::LucasKanadeTracker_Slow",
      "Only Transformations::TRANSFORM_TRANSLATION, "
      "Transformations::TRANSFORM_AFFINE, and "
      "Transformations::TRANSFORM_PROJECTIVE are supported");

  AnkiConditionalErrorAndReturn(
      ridgeWeight >= 0.0f, "LucasKanadeTracker_Slow::LucasKanadeTracker_Slow",
      "ridgeWeight must be greater or equal to zero");
  ...
  AnkiConditionalErrorAndReturn(
      A_full.IsValid(), "LucasKanadeTracker_Slow::LucasKanadeTracker_Slow",
      "Could not allocate A_full");
  ...
  AnkiConditionalErrorAndReturn(
      ...
      "LucasKanadeTracker_Slow::LucasKanadeTracker_Slow",
      "Could not allocate templateCoordinates");
  ...
  AnkiConditionalErrorAndReturn(
      ...
      "LucasKanadeTracker_Slow::LucasKanadeTracker_Slow",
      "Could not allocate templateImagePyramid");
  ...
  AnkiConditionalErrorAndReturn(
      templateWeights.IsValid(),
      "LucasKanadeTracker_Slow::LucasKanadeTracker_Slow",
      "Could not allocate templateWeights");
  ...
  if (LucasKanadeTracker_Slow::InitializeTemplate(templateImage, memory) !=
      RESULT_OK) {
    this->isValid = false;
  }
...
Result LucasKanadeTracker_Slow::InitializeTemplate(
    const Array<u8> &templateImage, MemoryStack &memory) {
  const bool isOutColumnMajor =
      false;  // TODO: change to false, which will probably be faster

  Result lastResult;

  AnkiConditionalErrorAndReturnValue(
      this->isValid, RESULT_FAIL_INVALID_OBJECT,
      "LucasKanadeTracker_Slow::InitializeTemplate",
      "This object's constructor failed, so it cannot be initialized");

  AnkiConditionalErrorAndReturnValue(
      this->isInitialized == false, RESULT_FAIL,
      "LucasKanadeTracker_Slow::InitializeTemplate",
  ...
  AnkiConditionalErrorAndReturnValue(
      templateImageHeight == templateImage.get_size(0) &&
          templateImageWidth == templateImage.get_size(1),
      RESULT_FAIL_INVALID_SIZE, "LucasKanadeTracker_Slow::InitializeTemplate",
      "template size doesn't match constructor");

  AnkiConditionalErrorAndReturnValue(
      templateRegion.left < templateRegion.right && templateRegion.left >= 0 &&
          templateRegion.right < templateImage.get_size(1) &&
          templateRegion.top < templateRegion.bottom &&
          templateRegion.top >= 0 &&
          templateRegion.bottom < templateImage.get_size(0),
      RESULT_FAIL, "LucasKanadeTracker_Slow::InitializeTemplate",
      "template rectangle is invalid or out of bounds");

  // We do this early, before any memory is allocated This way, an early return
  // won't be able to leak memory with multiple calls to this object
  this->isInitialized = true;
  this->isValid = false;

  const s32 initialImageScaleS32 = baseImageWidth / templateImage.get_size(1);
  const f32 initialImageScaleF32 = static_cast<f32>(
      initialImageScaleS32);  // TODO: check that this is integer

  const s32 numTransformationParameters =
      transformation.get_transformType() >> 8;
  ...
  this->templateWeightsSigma =
      sqrtf(this->templateRegionWidth * this->templateRegionWidth +
            this->templateRegionHeight * this->templateRegionHeight) /
      2.0f;
  ...
  for (s32 iScale = 0; iScale < this->numPyramidLevels; iScale++) {
    ...
    this->A_full[iScale] =
        Array<f32>(numTransformationParameters, numValidPoints, memory);
    AnkiConditionalErrorAndReturnValue(
        this->A_full[iScale].IsValid(), RESULT_FAIL_INVALID_OBJECT,
        "LucasKanadeTracker_Slow::InitializeTemplate",
        "Could not allocate A_full[iScale]");
    ...
    this->templateImagePyramid[iScale] =
        Array<u8>(numPointsY, numPointsX, memory);

    AnkiConditionalErrorAndReturnValue(
        this->templateImagePyramid[iScale].IsValid(),
        RESULT_FAIL_INVALID_OBJECT,
        "LucasKanadeTracker_Slow::InitializeTemplate",
        "Could not allocate templateImagePyramid[i]");

    this->templateWeights[iScale] =
        Array<f32>(1, numPointsY * numPointsX, memory);

    AnkiConditionalErrorAndReturnValue(
        this->templateWeights[iScale].IsValid(), RESULT_FAIL_INVALID_OBJECT,
        "LucasKanadeTracker_Slow::InitializeTemplate",
        "Could not allocate templateWeights[i]");
  ...
  // Everything below here is temporary
  {
    ...
    Array<f32> templateMask =
        Array<f32>(templateImageHeight, templateImageWidth, memory);
    templateMask.SetZero();
    templateMask(
        Round<s32>(templateRegion.top), Round<s32>(templateRegion.bottom),
        Round<s32>(templateRegion.left), Round<s32>(templateRegion.right))
        .Set(1.0f);
    ...
    for (s32 iScale = 0; iScale < this->numPyramidLevels; iScale++) {
      ...
      // if((lastResult = transformation.TransformPoints(xIn, yIn,
      // scale*initialImageScaleF32, xTransformed, yTransformed)) != RESULT_OK)
      if ((lastResult = transformation.TransformPoints(
               xIn, yIn, initialImageScaleF32, true, false, xTransformed,
               yTransformed)) != RESULT_OK)
        return lastResult;
      ...
      if ((lastResult =
               Interp2<u8, u8>(templateImage, xTransformed, yTransformed,
                               this->templateImagePyramid[iScale],
                               INTERPOLATE_LINEAR)) != RESULT_OK)
        return lastResult;
      ...
      Matrix::Subtract<u8, f32, f32>(templateImagePyramid[iScale](1, -2, 2, -1),
                                     templateImagePyramid[iScale](1, -2, 0, -3),
                                     templateDerivativeX(1, -2, 1, -2));
      // Matrix::DotMultiply<f32,f32,f32>(templateDerivativeX, scale / 2.0f,
      // templateDerivativeX);
      Matrix::DotMultiply<f32, f32, f32>(
          templateDerivativeX, scale / (2.0f * 255.0f), templateDerivativeX);

      Matrix::Subtract<u8, f32, f32>(templateImagePyramid[iScale](2, -1, 1, -2),
                                     templateImagePyramid[iScale](0, -3, 1, -2),
                                     templateDerivativeY(1, -2, 1, -2));
      // Matrix::DotMultiply<f32,f32,f32>(templateDerivativeY, scale / 2.0f,
      // templateDerivativeY);
      Matrix::DotMultiply<f32, f32, f32>(
          templateDerivativeY, scale / (2.0f * 255.0f), templateDerivativeY);
      EndBenchmark("InitializeTemplate.ComputeImageGradients");

      // Create the A matrix
      BeginBenchmark("InitializeTemplate.ComputeA");
      if (transformation.get_transformType() ==
      ...
      } else if (transformation.get_transformType() ==
                     Transformations::TRANSFORM_AFFINE ||
                 transformation.get_transformType() ==
        ...
        if (transformation.get_transformType() ==
            Transformations::TRANSFORM_PROJECTIVE) {
          // The seventh and eights terms are for projective only, not for
          // affine

          Array<f32> tmp2(1, numPointsY * numPointsX, memory);

          // -X.^2.*Ix(:) - X.*Y.*Iy(:)
          Matrix::Vectorize(isOutColumnMajor, templateDerivativeX,
                            tmp1);                               // Ix(:)
          ...
          Matrix::Vectorize(isOutColumnMajor, templateDerivativeY,
                            tmp2);                               // Iy(:)
...

      {
        ...
        // GaussianTmp = exp(-((this.xgrid{i_scale}).^2 +
        // (this.ygrid{i_scale}).^2) / (2*(W_sigma)^2));
        {
          ...
          Matrix::DotMultiply<f32, f32, f32>(
              GaussianTmp,
              1.0f / (2.0f * templateWeightsSigma * templateWeightsSigma),
              GaussianTmp);
        ...
        if ((lastResult = Interp2<f32, f32>(templateMask, xTransformed,
                                            yTransformed, templateWeightsTmp,
                                            INTERPOLATE_LINEAR)) != RESULT_OK)
          return lastResult;
        ...
        Matrix::DotMultiply<f32, f32, f32>(templateWeightsTmp, GaussianTmp,
                                           templateWeightsTmp);
        ...
        Matrix::Vectorize(isOutColumnMajor, templateWeightsTmp,
                          templateWeights[iScale]);
...
Result LucasKanadeTracker_Slow::UpdateTrack(const Array<u8> &nextImage,
  ...
  for (s32 iScale = numPyramidLevels - 1; iScale >= 0; iScale--) {
    ...
    if ((lastResult = IterativelyRefineTrack(
             ...
             Transformations::TRANSFORM_TRANSLATION, useWeights,
             verify_converged, scratch)) != RESULT_OK)
      return lastResult;
    ...
    if (this->transformation.get_transformType() !=
      ...
      if ((lastResult = IterativelyRefineTrack(
               nextImage, maxIterations, iScale, convergenceTolerance,
               this->transformation.get_transformType(), useWeights,
               verify_converged, scratch)) != RESULT_OK)
        return lastResult;
...
Result LucasKanadeTracker_Slow::IterativelyRefineTrack(
    ...
    const bool useWeights, bool &verify_converged, MemoryStack scratch) {
  const bool isOutColumnMajor =
      false;  // TODO: change to false, which will probably be faster
  ...
  AnkiConditionalErrorAndReturnValue(
      this->isInitialized == true, RESULT_FAIL,
      "LucasKanadeTracker_Slow::IterativelyRefineTrack",
  ...
  const s32 numPointsY =
      templateCoordinates[whichScale].get_yGridVector().get_size();
  const s32 numPointsX =
      templateCoordinates[whichScale].get_xGridVector().get_size();
  ...
  if (curTransformType == Transformations::TRANSFORM_TRANSLATION) {
    // Translation-only can be performed by grabbing a few rows of the A_full
    // matrix
    if (this->get_transformation().get_transformType() ==
            Transformations::TRANSFORM_AFFINE ||
        this->get_transformation().get_transformType() ==
            Transformations::TRANSFORM_PROJECTIVE) {
      A_part = Array<f32>(2, this->A_full[whichScale].get_size(1), scratch);
      A_part(0, -1, 0, -1)
          .Set(this->A_full[whichScale](2, 3, 5, 0, 1,
                                        -1));  // grab the 2nd and 5th rows
    } else if (this->get_transformation().get_transformType() ==
               Transformations::TRANSFORM_TRANSLATION) {
      ...
      AnkiAssert(false);
             ...
             curTransformType == Transformations::TRANSFORM_PROJECTIVE) {
    A_part = this->A_full[whichScale];
  } else {
    AnkiAssert(false);
  }
  ...
  for (s32 iteration = 0; iteration < maxIterations; iteration++) {
    ...
    // imgi = interp2(img, xi(:), yi(:), 'linear');
    {
      ...
      if ((lastResult = Interp2<u8, f32>(
               nextImage, xTransformed, yTransformed, nextImageTransformed2d,
               INTERPOLATE_LINEAR, -1.0f)) != RESULT_OK)
        return lastResult;

      Matrix::Vectorize<f32, f32>(isOutColumnMajor, nextImageTransformed2d,
                                  nextImageTransformed);
    }  // PUSH_MEMORY_STACK(scratch);
    EndBenchmark("IterativelyRefineTrack.interpTransformedCoords");

    BeginBenchmark("IterativelyRefineTrack.getNumMatches");
    // inBounds = ~isnan(imgi);
    // WARNING: this is also treating real zeros as invalid, but this should not
    // be a big problem
    Find<f32, Comparison::GreaterThanOrEqual<f32, f32>, f32> inBounds(
        nextImageTransformed, 0.0f);
    const s32 numInBounds = inBounds.get_numMatches();

    if (numInBounds < 16) {
      AnkiWarn("LucasKanadeTracker_Slow::IterativelyRefineTrack",
    ...
    Matrix::Vectorize(isOutColumnMajor, this->templateImagePyramid[whichScale],
                      templateImage);
    ...
    {
      PUSH_MEMORY_STACK(scratch);
      Array<f32> templateDerivativeT_allPoints(1, numPointsY * numPointsX,
                                               scratch);
      Matrix::Subtract<f32, f32, f32>(nextImageTransformed, templateImage,
                                      templateDerivativeT_allPoints);
      inBounds.SetArray(templateDerivativeT, templateDerivativeT_allPoints, 1);
      Matrix::DotMultiply<f32, f32, f32>(templateDerivativeT, 1.0f / 255.0f,
                                         templateDerivativeT);
    }
    ...
    if (useWeights) {
      PUSH_MEMORY_STACK(scratch);
      Array<f32> validTemplateWeights =
          inBounds.SetArray(templateWeights[whichScale], 1, scratch);

      for (s32 y = 0; y < numSystemParameters; y++) {
        Matrix::DotMultiply<f32, f32, f32>(
            AW(y, y, 0, -1), validTemplateWeights, AW(y, y, 0, -1));
      }
    }  // if(useWeights)
    ...
    //  matlab.EvalStringEcho("if ~exist('A', 'var') A=cell(0); AW=cell(0);
    //  AWAt=cell(0); end;"); matlab.EvalStringEcho("A{end+1}=A_tmp;
    //  AW{end+1}=AW_tmp; AWAt{end+1}=AWAt_tmp;"); CoreTechPrint("");
    //}

    Array<f32> ridgeWeightMatrix =
        Eye<f32>(numSystemParameters, numSystemParameters, scratch);
    Matrix::DotMultiply<f32, f32, f32>(ridgeWeightMatrix, ridgeWeight,
                                       ridgeWeightMatrix);
    ...
    //  matlab.PutArray(this->transformation.get_homography(),
    //  "homography_tmp");

    //  matlab.EvalStringEcho("if ~exist('update', 'var') update=cell(0);
    //  homography=cell(0); end;");
    //  matlab.EvalStringEcho("update{end+1}=update_tmp;
    //  homography{end+1}=homography_tmp;"); CoreTechPrint("");
    //}
...
bool LucasKanadeTracker_Slow::IsValid() const {
  ...
  if (!A_full.IsValid()) return false;

  if (!templateImagePyramid.IsValid()) return false;

  if (!templateCoordinates.IsValid()) return false;

  if (!templateWeights.IsValid()) return false;

  if (this->isInitialized) {
    for (s32 i = 0; i < numPyramidLevels; i++) {
      if (!A_full[i].IsValid()) return false;

      if (!templateImagePyramid[i].IsValid()) return false;

      if (!templateWeights[i].IsValid()) return false;
    }
  }
...
Result LucasKanadeTracker_Slow::UpdateTransformation(
...
Result LucasKanadeTracker_Slow::set_transformation(
...
LucasKanadeTracker_Slow::get_transformation() const {
