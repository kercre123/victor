...
IntegerCounts::IntegerCounts(const Array<u8> &image,
                             const Rectangle<s32> &imageRegionOfInterest,
                             const s32 yIncrement, const s32 xIncrement,
                             MemoryStack &memory)
    : counts(FixedLengthList<s32>()), numElements(-1) {
  ...
  AnkiConditionalErrorAndReturn(
      this->counts.IsValid(), "IntegerCounts::IntegerCounts", "Out of memory");
  ...
  for (s32 y = imageRegionOfInterestClipped.top;
       y <= (imageRegionOfInterestClipped.bottom - 1); y += yIncrement) {
    const u8 *restrict pImage = image.Pointer(y, 0);

    for (s32 x = imageRegionOfInterestClipped.left;
         x <= (imageRegionOfInterestClipped.right - 1); x += xIncrement) {
  ...
  // TODO: fix for speed
  /*const s32 numY = ((imageRegionOfInterest.bottom - imageRegionOfInterest.top)
  / yIncrement) + 1; const s32 numX = ((imageRegionOfInterest.right -
  imageRegionOfInterest.left) / xIncrement) + 1; this->numElements = numY *
  numX;
...
IntegerCounts::IntegerCounts(const Array<u8> &image,
                             const Quadrilateral<f32> &imageRegionOfInterest,
                             const s32 yIncrement, const s32 xIncrement,
                             MemoryStack &memory)
    : counts(FixedLengthList<s32>()), numElements(-1) {
  ...
  const Rectangle<f32> boundingRect =
      imageRegionOfInterest.ComputeBoundingRectangle<f32>();
  const Quadrilateral<f32> sortedQuad =
      imageRegionOfInterest.ComputeClockwiseCorners<f32>();
  ...
  for (s32 iy = 0; iy < numYs; iy += yIncrement) {
    ...
    for (s32 iCorner = 0; iCorner < 4; iCorner++) {
      if ((corners[iCorner].y < yF32 && corners[iCorner + 1].y >= yF32) ||
          (corners[iCorner + 1].y < yF32 && corners[iCorner].y >= yF32)) {
...
}  // IntegerCounts::IntegerCounts(const Array<u8> &image, const
   // Quadrilateral<f32> &imageRegionOfInterest, const s32 yIncrement, const s32
   // xIncrement, MemoryStack &memory)
...
s32 IntegerCounts::ComputePercentile(const f32 percentile) const {
  ...
  const s32 numBelow =
      Round<s32>(static_cast<f32>(this->numElements) * percentile);
...
IntegerCounts::Statistics IntegerCounts::ComputeStatistics() const {
  ...
  statistics.mean =
      static_cast<f32>(statistics.sum) / static_cast<f32>(this->numElements);
  ...
  statistics.standardDeviation =
      sqrtf(sumOfSquaredDifferences / static_cast<f32>(this->numElements - 1));
...
Result IntegerCounts::Set(const IntegerCounts &in) {
  AnkiConditionalErrorAndReturnValue(
      in.counts.get_size() == this->counts.get_size(), RESULT_FAIL,
      "Histogram::Set", "counts must be allocated");
...
s32 IntegerCounts::get_numBins() const { return this->counts.get_size(); }

s32 IntegerCounts::get_numElements() const { return this->numElements; }
}  // namespace Embedded
}  // namespace Anki
