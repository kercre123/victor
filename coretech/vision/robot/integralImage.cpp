...
namespace Anki {
namespace Embedded {
IntegralImage_u8_s32::IntegralImage_u8_s32() : Array<s32>() {}

IntegralImage_u8_s32::IntegralImage_u8_s32(const Array<u8> &image,
                                           MemoryStack &memory,
...
IntegralImage_u8_u16::IntegralImage_u8_u16() : Array<u16>() {}

IntegralImage_u8_u16::IntegralImage_u8_u16(const Array<u8> &image,
                                           MemoryStack &memory,
                                           const Flags::Buffer flags)
...
ScrollingIntegralImage_u8_s32::ScrollingIntegralImage_u8_s32(
    ...
    MemoryStack &memory, const Flags::Buffer flags)
    : Array<s32>(bufferHeight, imageWidth + 2 * numBorderPixels, memory, flags),
      imageWidth(imageWidth),
      maxRow(-1),
      rowOffset(-numBorderPixels),
      numBorderPixels(numBorderPixels) {
  // AnkiAssert(imageWidth%ANKI_VISION_IMAGE_WIDTH_SHIFT == 0);

  if (numBorderPixels < 0 || numBorderPixels > bufferHeight ||
      numBorderPixels > imageWidth) {
    AnkiError(
        "Anki.ScrollingIntegralImage_u8_s32.ScrollingIntegralImage_u8_s32",
        "numBorderPixels must be greater than or equal to zero, and less than "
        "the size of this ScrollingIntegralImage.");
    InvalidateArray();
    return;
  }
}

Result ScrollingIntegralImage_u8_s32::ScrollDown(const Array<u8> &image,
                                                 s32 numRowsToScroll,
                                                 MemoryStack scratch) {
  Result lastResult;
  ...
  AnkiConditionalErrorAndReturnValue(
      numRowsToScroll > 0 && numRowsToScroll <= integralImageHeight,
      RESULT_FAIL_INVALID_PARAMETER,
      "ScrollingIntegralImage_u8_s32::ScrollDown",
  ...
  // If we are asked to scroll all rows, we won't keep track of any of the
  // previous data, so we'll start by initializing the first rows
  if (numRowsToScroll == integralImageHeight) {
    ...
    ComputeIntegralImageRow(pPaddedRow, this->Pointer(0, 0),
                            integralImageWidth);
    curIntegralImageY++;
    ...
    if (numBorderPixels > 0) {
      ...
      for (s32 y = 0; y < numBorderPixels; y++) {
        ComputeIntegralImageRow(
            pPaddedRow, this->Pointer(curIntegralImageY - 1, 0),
            this->Pointer(curIntegralImageY, 0), integralImageWidth);
        curIntegralImageY++;
      }
    ...
    // integralImage(1:(end-numRowsToUpdate), :) =
    // integralImage((1+numRowsToUpdate):end, :);
    for (s32 y = 0; y < (integralImageHeight - numRowsToScroll); y++) {
      s32 *restrict pIntegralImage_yDst = this->Pointer(y, 0);
      const s32 *restrict pIntegralImage_ySrc =
          this->Pointer(y + numRowsToScroll, 0);
  ...
  if (numRowsToScroll > 0) {
    ...
    while (numRowsToScroll > 0) {
      // integralImage(curIntegralImageY,1) = paddedImageRow(1,1) +
      // integralImage(curIntegralImageY-1,1); for x = 2:size(paddedImageRow, 2)
      //     integralImage(curIntegralImageY,x) = paddedImageRow(1,x) +
      //     integralImage(curIntegralImageY,x-1) +
      //     integralImage(curIntegralImageY-1,x) -
      //     integralImage(curIntegralImageY-1,x-1);
      // end
      ComputeIntegralImageRow(
          pPaddedRow, this->Pointer(curIntegralImageY - 1, 0),
          this->Pointer(curIntegralImageY, 0), integralImageWidth);
...
Result ScrollingIntegralImage_u8_s32::FilterRow(const Rectangle<s16> &filter,
                                                const s32 imageRow,
...
// TODO: implement
// s32 ScrollingIntegralImage_u8_s32::get_minRow(const s32 filterHalfHeight)
// const
//{
...
s32 ScrollingIntegralImage_u8_s32::get_maxRow(
    const s32 filterHalfHeight) const {
  AnkiAssert(filterHalfHeight >= 0);
...
s32 ScrollingIntegralImage_u8_s32::get_imageWidth() const { return imageWidth; }

s32 ScrollingIntegralImage_u8_s32::get_numBorderPixels() const {
  return numBorderPixels;
}

s32 ScrollingIntegralImage_u8_s32::get_rowOffset() const { return rowOffset; }

Result ScrollingIntegralImage_u8_s32::PadImageRow(const Array<u8> &image,
                                                  const s32 whichRow,
                                                  Array<u8> &paddedRow) {
  ...
  const u32 *restrict image_u32rowPointer =
      reinterpret_cast<const u32 *>(image.Pointer(whichRow, 0));
  ...
  u32 *restrict paddedRow_u32rowPointerPxpad =
      reinterpret_cast<u32 *>(paddedRow_u8rowPointer + numBorderPixels);
...
void ScrollingIntegralImage_u8_s32::ComputeIntegralImageRow(
...
void ScrollingIntegralImage_u8_s32::ComputeIntegralImageRow(
  ...
  const u32 *restrict paddedImage_currentRowU32 =
      reinterpret_cast<const u32 *>(paddedImage_currentRow);

  for (; x < integralImageWidth4; x++) {
    const u32 curPixel = paddedImage_currentRowU32[x];

    horizontalSum += curPixel & 0xFF;
    integralImage_currentRow[4 * x] =
        horizontalSum + integralImage_previousRow[4 * x];

    horizontalSum += (curPixel & 0xFF00) >> 8;
    integralImage_currentRow[4 * x + 1] =
        horizontalSum + integralImage_previousRow[4 * x + 1];

    horizontalSum += (curPixel & 0xFF0000) >> 16;
    integralImage_currentRow[4 * x + 2] =
        horizontalSum + integralImage_previousRow[4 * x + 2];

    horizontalSum += (curPixel & 0xFF000000) >> 24;
    integralImage_currentRow[4 * x + 3] =
        horizontalSum + integralImage_previousRow[4 * x + 3];
  }

  x *= 4;

  // Initial attempt at neon optimization is no better than non-accelerated
  // version below #elif ACCELERATION_TYPE == ACCELERATION_ARM_A7
  ...
  //         *integralImage_currentRow = horizontalSum +
  //         *integralImage_previousRow; paddedImage_currentRow++;
...
void ScrollingIntegralImage_u8_s32::FilterRow_innerLoop(
  ...
  if (outputMultiply == 1 && outputRightShift == 0) {
    for (s32 x = minX; x <= maxX; x++) {
      pOutput[x] = static_cast<u8>(pIntegralImage_11[x] - pIntegralImage_10[x] +
                                   pIntegralImage_00[x] - pIntegralImage_01[x]);
    }
  } else {
    for (s32 x = minX; x <= maxX; x++) {
      pOutput[x] =
          static_cast<u8>(((pIntegralImage_11[x] - pIntegralImage_10[x] +
                            pIntegralImage_00[x] - pIntegralImage_01[x]) *
                           outputMultiply) >>
                          outputRightShift);
    }
  ...
  if (outputMultiply == 1 && outputRightShift == 0) {
    ...
    for (s32 x = minX_simd4; x <= maxX_simd4; x++) {
      const u32 out0 =
          static_cast<u8>(pIntegralImage_11[x * 4] - pIntegralImage_10[x * 4] +
                          pIntegralImage_00[x * 4] - pIntegralImage_01[x * 4]);
      const u32 out1 = static_cast<u8>(
          pIntegralImage_11[x * 4 + 1] - pIntegralImage_10[x * 4 + 1] +
          pIntegralImage_00[x * 4 + 1] - pIntegralImage_01[x * 4 + 1]);
      const u32 out2 = static_cast<u8>(
          pIntegralImage_11[x * 4 + 2] - pIntegralImage_10[x * 4 + 2] +
          pIntegralImage_00[x * 4 + 2] - pIntegralImage_01[x * 4 + 2]);
      const u32 out3 = static_cast<u8>(
          pIntegralImage_11[x * 4 + 3] - pIntegralImage_10[x * 4 + 3] +
          pIntegralImage_00[x * 4 + 3] - pIntegralImage_01[x * 4 + 3]);

      pOutputU32[x] = (out0 & 0xFF) | ((out1 & 0xFF) << 8) |
                      ((out2 & 0xFF) << 16) | ((out3 & 0xFF) << 24);
    }
  } else {
    const s32 maxX_firstCycles = RoundUp(minX, 4);
    ...
    const s32 maxX_simd4 = maxX / 4;
    for (s32 x = minX_simd4; x <= maxX_simd4; x++) {
      const u32 out0 = static_cast<u8>(
          ((pIntegralImage_11[x * 4] - pIntegralImage_10[x * 4] +
            pIntegralImage_00[x * 4] - pIntegralImage_01[x * 4]) *
           outputMultiply) >>
          outputRightShift);
      const u32 out1 = static_cast<u8>(
  ...
  int32x4_t outShift =
      vdupq_n_s32(-outputRightShift);  // Negative for right shift

  const u32 kNumElementsProcessedPerLoop = 8;
  const u32 kNumIntegralElementsLoadedAtATime =
      kNumElementsProcessedPerLoop / 2;
  const s32 kMaxXNeon = maxX - (kNumElementsProcessedPerLoop - 1);

  // Process as much as we can with NEON, whatever is left over will need to be
  // done one element at a time
  s32 x = minX;
  ...
  for (; x <= maxX; x++) {
    *pOutput = static_cast<u8>(((*pIntegralImage_11 - *pIntegralImage_10 +
                                 *pIntegralImage_00 - *pIntegralImage_01) *
                                outputMultiply) >>
                               outputRightShift);
