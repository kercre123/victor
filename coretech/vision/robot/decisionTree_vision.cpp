...
FiducialMarkerDecisionTree::FiducialMarkerDecisionTree(
    const void *restrict treeData, const s32 treeDataLength,
    const s32 treeDataNumFractionalBits, const s32 treeMaxDepth,
    const s16 *restrict probeXOffsets, const s16 *restrict probeYOffsets,
    const s32 numProbeOffsets, const u16 *restrict leafLabels,
    const s32 numLeafLabels)
    : DecisionTree(treeData, treeDataLength, treeDataNumFractionalBits,
                   treeMaxDepth, leafLabels, numLeafLabels),
      probeXOffsets(NULL),
      probeYOffsets(NULL),
      numProbeOffsets(-1) {
  AnkiConditionalErrorAndReturn(
      probeXOffsets != NULL && probeYOffsets != NULL,
      "FiducialMarkerDecisionTree::FiducialMarkerDecisionTree",
      "probes are NULL");
...
Result FiducialMarkerDecisionTree::FindLeaf(const Array<u8> &image,
                                            const Array<f32> &homography,
                                            const u8 meanGrayvalueThreshold,
                                            s32 &nodeIndex) const {
  AnkiConditionalErrorAndReturnValue(
      AreValid(*this, image, homography), RESULT_FAIL_INVALID_OBJECT,
      "FiducialMarkerDecisionTree::Classify", "Invalid objects");

  // This function is optimized for 9 or fewer probes, but this is not a big
  // issue
  ...
  fixedPointDivider =
      1.0f / static_cast<f32>(1 << this->treeDataNumFractionalBits);

  f32 probeXOffsetsF32[9];
  f32 probeYOffsetsF32[9];

  for (s32 iProbe = 0; iProbe < numProbeOffsets; iProbe++) {
    probeXOffsetsF32[iProbe] =
        static_cast<f32>(this->probeXOffsets[iProbe]) * fixedPointDivider;
    probeYOffsetsF32[iProbe] =
        static_cast<f32>(this->probeYOffsets[iProbe]) * fixedPointDivider;
  }

  const Node *restrict pTreeData =
      reinterpret_cast<const Node *>(this->treeData);

  nodeIndex = 0;
  for (s32 iDepth = 0; iDepth <= treeMaxDepth; iDepth++) {
    const bool isLeaf =
        static_cast<bool>(pTreeData[nodeIndex].label & (1 << 15));
    ...
    const f32 xCenter =
        static_cast<f32>(pTreeData[nodeIndex].probeXCenter) * fixedPointDivider;
    const f32 yCenter =
        static_cast<f32>(pTreeData[nodeIndex].probeYCenter) * fixedPointDivider;
    ...
    for (s32 iProbe = 0; iProbe < numProbeOffsets; iProbe++) {
      ...
      AnkiAssert(
          warpedY >= 0 && warpedX >= 0 && warpedY < image.get_size(0) &&
          warpedX <
              image.get_size(
                  1));  // Verify y against image height and x against width
  ...
  AnkiError("FiducialMarkerDecisionTree::Classify",
            "Decision tree ran out of bounds");

  // If this is reached, we ran out of tree to query
  return RESULT_FAIL;
}  // FiducialMarkerDecisionTree::FindLeaf()

Result FiducialMarkerDecisionTree::Classify(const Array<u8> &image,
                                            const Array<f32> &homography,
                                            const u8 meanGrayvalueThreshold,
                                            s32 &label) const {
  ...
  s32 leafNodeIndex = -1;
  if ((lastResult = FindLeaf(image, homography, meanGrayvalueThreshold,
                             leafNodeIndex)) != RESULT_OK) {
    return lastResult;
  }
...
Result FiducialMarkerDecisionTree::Verify(const Array<u8> &image,
                                          const Array<f32> &homography,
                                          const u8 meanGrayvalueThreshold,
                                          const s32 label,
  ...
  if (labelIndexStart == labelIndexEnd) {
    ...
    for (s32 labelIndex = labelIndexStart; labelIndex < labelIndexEnd;
         ++labelIndex) {
...
bool FiducialMarkerDecisionTree::IsValid() const {
  if (DecisionTree::IsValid() == false) return false;

  if (!probeXOffsets) return false;

  if (!probeYOffsets) return false;
