...
#define LINE_FIT_METHOD_KMEANS \
  0  // Works, but needs profiling to see if termination parameters (and
     // initialization) are good and fast enough
#define LINE_FIT_METHOD_HIST \
  1  // Has issues with certain edge cases (b/c of fixed bin centers?)
...
namespace Embedded {
#if DRAW_LINE_FITS
static const ColorRGBA colorList[5] = {NamedColors::GREEN, NamedColors::ORANGE,
                                       NamedColors::WHITE, NamedColors::CYAN,
...
Result ExtractLineFitsPeaks_Ransac(const FixedLengthList<Point<s16>> &boundary,
  ...
  // Look for four lines that each account for at least inlierFraction of the
  // boundary
  ...
  for (s32 iStart = 0; numLinesFit < 4 && iStart < boundaryLength;
       iStart += 3) {
    if (used[iStart]) {
      continue;
    }
    ...
    s32 dir = 1;
    for (s32 iSampleDist = boundaryLength / 4;
         numLinesFit < 4 && iSampleDist >= 2;
         iSampleDist -= (dir == 1 ? 0 : 3), dir = -dir) {
      ...
      // CoreTechPrint("RansacLineFit: iStart=%d, iEnd=%d, sampleDist=%d,
      ...
      if (dx == 0) {
        ...
        lineFits[numLinesFit].intercept =
            pBoundary[iStart].x -
            lineFits[numLinesFit].slope * pBoundary[iStart].y;
      } else {
        assert(numLinesFit >= 0 && numLinesFit < 4);
        ...
        lineFits[numLinesFit].intercept =
            pBoundary[iStart].y -
            lineFits[numLinesFit].slope * pBoundary[iStart].x;
      }
      ...
      for (s32 iPt = 0; iPt < boundaryLength; ++iPt) {
        if (!used[iPt]) {
          cv::Point2f pointVec(pBoundary[iPt].x - pBoundary[iStart].x,
                               pBoundary[iPt].y - pBoundary[iStart].y);
      ...
      if (inlierIndex.size() >= numInliersRequired) {
        // CoreTechPrint("RansacLineFit: Found %lu inliers (> %d) for line
        ...
        // Mark all inliers as used and do final line fits using all the inlier
        // data
        for (auto index : inlierIndex) {
          assert(index >= 0 && index < used.size());
          used[index] = (u8) true;
#if DRAW_LINE_FITS
          blankImg.DrawLine(Point2f(pBoundary[index].x, pBoundary[index].y),
                            Point2f(pBoundary[index].x, pBoundary[index].y),
                            colorList[numLinesFit], 2);
#endif
        ...
        //            CoreTechPrint("RansacLineFit: Fit %sline %d from inliers.
        //            Slope=%f, Int=%f\n",
        //                          lineFits[numLinesFit].switched ? "switched "
        //                          : "", numLinesFit,
        //                          lineFits[numLinesFit].slope,
        //                          lineFits[numLinesFit].intercept);
...
#if DRAW_LINE_FITS
        {
          if (lineFits[numLinesFit].switched) {
            blankImg.DrawLine(
                Point2f(lineFits[numLinesFit].intercept, 0),
                Point2f(lineFits[numLinesFit].slope * (imageHeight - 1) +
                            lineFits[numLinesFit].intercept,
                        imageHeight - 1),
                NamedColors::MAGENTA, 1);
          } else {
            blankImg.DrawLine(
                Point2f(0, lineFits[numLinesFit].intercept),
                Point2f(imageWidth - 1,
                        lineFits[numLinesFit].slope * (imageWidth - 1) +
                            lineFits[numLinesFit].intercept),
                NamedColors::RED, 1);
          }
        }
        ...
        // CoreTechPrint("RansacLineFit: incrementing numLinesFit from %d to
        // %d\n",
  ...
  if (numLinesFit == 4) {
    ...
    for (s32 iLine = 0; iLine < 4; iLine++) {
      for (s32 jLine = iLine + 1; jLine < 4; jLine++) {
        ...
        } else if (lineFits[iLine].switched == true &&
                   lineFits[jLine].switched == false) {
          yInt = (lineFits[jLine].slope * lineFits[iLine].intercept +
                  lineFits[jLine].intercept) /
                 (1.f - lineFits[iLine].slope * lineFits[jLine].slope);
          xInt = lineFits[iLine].slope * yInt + lineFits[iLine].intercept;
        } else if (lineFits[iLine].switched == false &&
                   lineFits[jLine].switched == true) {
          yInt = (lineFits[iLine].slope * lineFits[jLine].intercept +
                  lineFits[iLine].intercept) /
                 (1.f - lineFits[iLine].slope * lineFits[jLine].slope);
          xInt = lineFits[jLine].slope * yInt + lineFits[jLine].intercept;
        }

        if (xInt >= 0 && xInt < imageWidth && yInt >= 0 && yInt < imageHeight) {
          // CoreTechPrint("RansacLineFit: Found corner at (%.1f,%.1f) using
          // lines %d and %d\n", xInt, yInt, iLine, jLine);
          corners.push_back(cv::Point_<f32>(xInt, yInt));

#if DRAW_LINE_FITS
          blankImg.DrawLine(Point2f(xInt, yInt), Point2f(xInt, yInt),
                            NamedColors::BLUE, 3);
#endif
        }
  ...
  if (corners.size() == 4) {
    ...
    Quadrilateral<f32> quad(Point<f32>(corners[0].x, corners[0].y),
    ...
    for (s32 i = 0; i < 4; i++) {
      peaks[i] = Point<s16>(saturate_cast<s16>(sortedQuad.corners[i].x),
                            saturate_cast<s16>(sortedQuad.corners[i].y));
    }

    // NOTE: The output is not reordered like the matlab version, because the
    // next step
    //       after ExtractLaplacianPeaks() or ExtractLineFitsPeaks() expects
    //       sorted corners.
  } else {
    corners.clear();
...
Result ExtractLineFitsPeaks(const FixedLengthList<Point<s16>> &boundary,
  ...
  AnkiConditionalErrorAndReturnValue(
      boundary.IsValid(), RESULT_FAIL_INVALID_OBJECT,
      "ComputeQuadrilateralsFromConnectedComponents", "boundary is not valid");

  AnkiConditionalErrorAndReturnValue(
      peaks.IsValid(), RESULT_FAIL_INVALID_OBJECT,
      "ComputeQuadrilateralsFromConnectedComponents", "peaks is not valid");

  AnkiConditionalErrorAndReturnValue(
      scratch.IsValid(), RESULT_FAIL_INVALID_OBJECT,
      "ComputeQuadrilateralsFromConnectedComponents", "scratch is not valid");

  AnkiConditionalErrorAndReturnValue(
      peaks.get_maximumSize() == 4, RESULT_FAIL_INVALID_PARAMETER,
      "ComputeQuadrilateralsFromConnectedComponents",
      "Currently only four peaks supported");
  ...
  // Center point of the boundary (average of all pixel coordinates making up
  // the boundary)
  ...
  s32 ksize = static_cast<s32>(ceilf((((sigma - 0.8f) / 0.3f) + 1.0f) * 2.0f +
                                     1.0f));  // Equation from opencv docs
  ksize = 2 * (ksize / 2) + 1;

  cv::Mat gk = cv::getGaussianKernel(ksize, sigma, CV_32F).t();

  cv::Mat_<f32> dxKernel(1, 3);
  dxKernel.at<f32>(0, 0) = -0.5f;
  dxKernel.at<f32>(0, 1) = 0;
  dxKernel.at<f32>(0, 2) = 0.5f;

  cv::Mat_<f32> dg;

  cv::filter2D(gk, dg, CV_32F, dxKernel, cv::Point(-1, -1), 0,
               cv::BORDER_DEFAULT);

  cv::Mat_<f32> dB(2, boundaryLength);

  // Circular convolution
  // TODO: may be slow? If/when openCv circular convolution is fixed, switch to
  // that cv::filter2D(dB, dB, -1, dg, cv::Point(-1,-1), 0, cv::BORDER_WRAP); //
  // assertion failure: not supported?
  {
    ...
    for (s32 xBoundary = 0; xBoundary < boundaryLength; xBoundary++) {
      ...
      if (pDB0[xBoundary] != 0 || pDB1[xBoundary] != 0) {
        const f32 invLength =
            1.f / std::sqrt(pDB0[xBoundary] * pDB0[xBoundary] +
  ...
  for (s32 iBin = 0; iBin < histSize; ++iBin) {
    binCenters[iBin] =
        (2.f * M_PI_F / (f32)histSize) * ((f32)iBin + 0.5f) - M_PI_F;
  }

#elif LINE_FIT_METHOD == LINE_FIT_METHOD_KMEANS

  const s32 numTries = 1;
  cv::TermCriteria criteria(cv::TermCriteria::COUNT + cv::TermCriteria::EPS, 15,
                            .1);
...
#error Unknown LINE_FIT_METHOD
  ...
  for (s32 iBin = 0; iBin < 4; iBin++) {
    ...
    for (s32 i = 0; i < boundaryLength; i++) {
      if (pLabels[i] == iBin) {
...
#if DRAW_LINE_FITS
        blankImg.DrawLine(Point2f(pBoundary[i].x, pBoundary[i].y),
                          Point2f(pBoundary[i].x, pBoundary[i].y),
                          colorList[iBin], 2);
#endif
      } else if (pLabels[i] == -1) {
#if DRAW_LINE_FITS
        ...
        blankImg.DrawLine(Point2f(pBoundary[i].x, pBoundary[i].y),
                          Point2f(pBoundary[i].x, pBoundary[i].y), colorList[4],
                          2);
    ...
    for (s32 i = 0; i < boundaryLength; i++) {
      const f32 angDist =
          (Radians(pBin[i]) - binCenters[thisBin]).getAbsoluteVal().ToFloat();
      if (angDist < (2 * M_PI_F) / histSize) {
        boundaryIndex.push_back(i);

#if DRAW_LINE_FITS
        blankImg.DrawPoint(Point2f(pBoundary[i].x, pBoundary[i].y),
                           colorList[iBin], 2);
#endif
      }
    }
#if DRAW_LINE_FITS
    blankImg.DrawText(Point2f(0, imageHeight / 2 + iBin * 15),
                      std::to_string(binCenters[thisBin].getDegrees()),
                      colorList[iBin], 0.5);
#endif
    ...
    if (numSide > 1) {
      ...
      s32 minX = std::numeric_limits<s32>::max(),
          maxX = std::numeric_limits<s32>::min(),
          minY = std::numeric_limits<s32>::max(),
          maxY = std::numeric_limits<s32>::min();
...
#if DRAW_LINE_FITS
      {
        if (lineFits[iBin].switched) {
          blankImg.DrawLine(Point2f(lineFits[iBin].intercept, 0),
                            Point2f(lineFits[iBin].slope * (imageHeight - 1) +
                                        lineFits[iBin].intercept,
                                    imageHeight - 1),
                            NamedColors::MAGENTA, 1);
        } else {
          blankImg.DrawLine(
              Point2f(0, lineFits[iBin].intercept),
              Point2f(imageWidth - 1, lineFits[iBin].slope * (imageWidth - 1) +
                                          lineFits[iBin].intercept),
              NamedColors::RED, 1);
        }
      }
...
#if DRAW_LINE_FITS
  blankImg.DrawLine(Point2f(boundaryCenter.x, boundaryCenter.y),
                    Point2f(boundaryCenter.x, boundaryCenter.y),
                    NamedColors::CYAN, 3);
#endif

  // Vector of the corners of the boundary as well as their squared distance
  // from the center of the boundary
  struct Corner {
    cv::Point2f point;
    u32 distToCenterSq;
  };
  ...
  if (didFitFourLines) {
    for (s32 iLine = 0; iLine < 4; iLine++) {
      for (s32 jLine = iLine + 1; jLine < 4; jLine++) {
        ...
        if (xInt >= 0 && xInt < imageWidth && yInt >= 0 && yInt < imageHeight) {
          const u32 distToCenter = std::pow(xInt - boundaryCenter.x, 2) +
                                   std::pow(yInt - boundaryCenter.y, 2);
  ...
  std::partial_sort(
      corners.begin(), corners.begin() + numToSort, corners.end(),
      [](Corner i, Corner j) { return (i.distToCenterSq < j.distToCenterSq); });

  if (corners.size() >= 4) {
    // Use the four closest corners to the boundary center
    Quadrilateral<f32> quad(Point<f32>(corners[0].point.x, corners[0].point.y),
...
Result ExtractLaplacianPeaks(const FixedLengthList<Point<s16>> &boundary,
                             const s32 minPeakRatio,
                             FixedLengthList<Point<s16>> &peaks,
                             MemoryStack scratch) {
  ...
  FixedPointArray<s16> stencil(1, stencilLength, 0, scratch,
                               Flags::Buffer(false, false, false));  // SQ15.0
  stencil.SetZero();
  ...
  *stencil.Pointer(0, stencil.get_size(1) - 1) = 1;
  ...
  FixedPointArray<s16> gaussian = ImageProcessing::Get1dGaussianKernel<s16>(
      sigma, numSigmaFractionalBits, numStandardDeviations, scratch);  // SQ7.8
  FixedPointArray<s16> differenceOfGaussian(
      1, gaussian.get_size(1) + stencil.get_size(1) - 1, numSigmaFractionalBits,
      scratch, Flags::Buffer(false, false, false));  // SQ7.8

  if ((lastResult = ImageProcessing::Correlate1d<s16, s32, s16>(
           stencil, gaussian, differenceOfGaussian)) != RESULT_OK)
    return lastResult;

  FixedPointArray<s16> boundaryXFiltered(
      1, boundary.get_size(), numSigmaFractionalBits, scratch,
      Flags::Buffer(false, false, false));  // SQ23.8
  FixedPointArray<s16> boundaryYFiltered(
      1, boundary.get_size(), numSigmaFractionalBits, scratch,
      Flags::Buffer(false, false, false));  // SQ23.8

  if (!boundaryYFiltered.IsValid()) return RESULT_FAIL_INVALID_OBJECT;
  ...
  // r_smooth = imfilter(boundary, dg2(:), 'circular');
  {
    ...
    FixedPointArray<s16> boundaryX(
        1, boundary.get_size(), 0, scratch,
        Flags::Buffer(false, false, false));  // SQ15.0

    if (!boundaryX.IsValid()) return RESULT_FAIL_INVALID_OBJECT;
    ...
    if ((lastResult = ImageProcessing::Correlate1dCircularAndSameSizeOutput<
             s16, s32, s16>(boundaryX, differenceOfGaussian, boundaryXFiltered,
                            scratch)) != RESULT_OK)
      return lastResult;
  ...
  {
    PUSH_MEMORY_STACK(scratch);
    FixedPointArray<s16> boundaryY(1, boundary.get_size(), 0,
                                   scratch);  // SQ15.0

    if (!boundaryY.IsValid()) return RESULT_FAIL_INVALID_OBJECT;
    ...
    if ((lastResult = ImageProcessing::Correlate1dCircularAndSameSizeOutput<
             s16, s32, s16>(boundaryY, differenceOfGaussian, boundaryYFiltered,
                            scratch)) != RESULT_OK)
      return lastResult;
  ...
  FixedPointArray<s32> boundaryFilteredAndCombined(
      1, boundary.get_size(), 2 * numSigmaFractionalBits, scratch,
      Flags::Buffer(false, false, false));  // SQ15.16
  s32 *restrict pBoundaryFilteredAndCombined =
      boundaryFilteredAndCombined.Pointer(0, 0);
  ...
  const s32 lengthBoundary = boundary.get_size();
  for (s32 i = 0; i < lengthBoundary; i++) {
    // const s32 xSquared = (pConstBoundaryXFiltered[i] *
    // pConstBoundaryXFiltered[i]) >> numSigmaFractionalBits; // SQ23.8 const s32
    // ySquared = (pConstBoundaryYFiltered[i] * pConstBoundaryYFiltered[i]) >>
    // numSigmaFractionalBits; // SQ23.8
    const s32 xSquared =
        (pConstBoundaryXFiltered[i] *
         pConstBoundaryXFiltered[i]);  // SQ31.0 (multiplied by
                                       // 2^numSigmaFractionalBits)
    const s32 ySquared =
        (pConstBoundaryYFiltered[i] *
         pConstBoundaryYFiltered[i]);  // SQ31.0 (multiplied by
                                       // 2^numSigmaFractionalBits)
  ...
  const s32 *restrict pConstBoundaryFilteredAndCombined =
      boundaryFilteredAndCombined.Pointer(0, 0);
  ...
  // combat rare cases where we have two responses next to each other that are
  // exactly equal. localMaxima = find(r_smooth >= r_smooth([end 1:end-1]) &
  // r_smooth > r_smooth([2:end 1]));

  if (pConstBoundaryFilteredAndCombined[0] >
          pConstBoundaryFilteredAndCombined[1] &&
      pConstBoundaryFilteredAndCombined[0] >=
          pConstBoundaryFilteredAndCombined[boundary.get_size() - 1]) {
    localMaxima.PushBack(0);
  }

  for (s32 i = 1; i < (lengthBoundary - 1); i++) {
    if (pConstBoundaryFilteredAndCombined[i] >
            pConstBoundaryFilteredAndCombined[i + 1] &&
        pConstBoundaryFilteredAndCombined[i] >=
            pConstBoundaryFilteredAndCombined[i - 1]) {
      localMaxima.PushBack(i);
    }
  }

  if (pConstBoundaryFilteredAndCombined[boundary.get_size() - 1] >
          pConstBoundaryFilteredAndCombined[0] &&
      pConstBoundaryFilteredAndCombined[boundary.get_size() - 1] >=
          pConstBoundaryFilteredAndCombined[boundary.get_size() - 2]) {
  ...
  for (s32 iMax = 0; iMax < 5; iMax++) {
    for (s32 i = 0; i < numLocalMaxima; i++) {
      ...
      if (pConstBoundaryFilteredAndCombined[localMaximaIndex] >
          maximaValues[iMax]) {
        maximaValues[iMax] =
            pConstBoundaryFilteredAndCombined[localMaximaIndex];
        maximaIndexes[iMax] = localMaximaIndex;
      }
    }
    // CoreTechPrint("Maxima %d/%d is #%d %d\n", iMax, 4, maximaIndexes[iMax],
    // maximaValues[iMax]);
    pBoundaryFilteredAndCombined[maximaIndexes[iMax]] =
        std::numeric_limits<s32>::min();
  }

  peaks.Clear();

  // Ratio of last of the 4 peaks to the next (fifth) peak should be fairly
  // large if there are 4 distinct corners.
  ...
  // Copy the maxima to the output peaks, ordered by their original index order,
  // so they are still in clockwise or counterclockwise order
  ...
  for (s32 iMax = 0; iMax < 4; iMax++) {
    ...
    for (s32 i = 0; i < 4; i++) {
      if (!whichUsed[i] && maximaIndexes[i] >= 0) {
        if (curMinIndex == -1 ||
            maximaIndexes[curMinIndex] > maximaIndexes[i]) {
          curMinIndex = i;
        }
      }
    }
