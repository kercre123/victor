...
#define NUM_BITS \
  MAX_FIDUCIAL_MARKER_BITS  // TODO: Why do we need a separate NUM_BITS?
...
FiducialMarkerDecisionTree
    VisionMarker::multiClassTrees[VisionMarkerDecisionTree::NUM_TREES];

bool VisionMarker::areTreesInitialized = false;
#endif

BlockMarker::BlockMarker() {}  // BlockMarker::BlockMarker()

void BlockMarker::Print() const {
  CoreTechPrint("[%d,%d: (%d,%d) (%d,%d) (%d,%d) (%d,%d)] ", blockType,
                faceType, corners[0].x, corners[0].y, corners[1].x,
...
FiducialMarkerParserBit::FiducialMarkerParserBit(MemoryStack &memory) {
  ...
  this->probeLocations = FixedLengthList<Point<s16>>(
      MAX_FIDUCIAL_MARKER_BIT_PROBE_LOCATIONS, memory);
  this->probeWeights =
      FixedLengthList<s16>(MAX_FIDUCIAL_MARKER_BIT_PROBE_LOCATIONS, memory);
}

FiducialMarkerParserBit::FiducialMarkerParserBit(
    const FiducialMarkerParserBit &bit2) {
...
}  // FiducialMarkerParserBit::FiducialMarkerParserBit(const
   // FiducialMarkerParserBit& bit2)

FiducialMarkerParserBit::FiducialMarkerParserBit(
    const s16 *const probesX, const s16 *const probesY,
    const s16 *const probeWeights, const s32 numProbes,
    const FiducialMarkerParserBit::Type type, const s32 numFractionalBits,
    MemoryStack &memory) {
...
}  // FiducialMarkerParserBit(const s16 * const probesX, const s16 * const
   // probesY, const s16 * const probesWeights, const s32 numProbes)

FiducialMarkerParserBit &FiducialMarkerParserBit::operator=(
    const FiducialMarkerParserBit &bit2) {
  AnkiAssert(bit2.probeLocations.get_size() == bit2.probeWeights.get_size());
  ...
  return *this;
}  // FiducialMarkerParserBit& FiducialMarkerParserBit::operator= (const
   // FiducialMarkerParserBit& bit2)

Result FiducialMarkerParserBit::ExtractMeanValue(const Array<u8> &image,
                                                 const Quadrilateral<s16> &quad,
  ...
  const f32 fixedPointDivider =
      1.0f / static_cast<f32>(1 << this->numFractionalBits);

  //#define SEND_WARPED_LOCATIONS
#ifdef SEND_WARPED_LOCATIONS
  Matlab matlab(false);
  matlab.EvalStringEcho(
      "if ~exist('warpedPoints', 'var') warpedPoints = zeros(2, 0); end;");
  ...
  for (s32 probe = 0; probe < numProbeLocations; probe++) {
    const f32 x =
        static_cast<f32>(pProbeLocations[probe].x) * fixedPointDivider;
    const f32 y =
        static_cast<f32>(pProbeLocations[probe].y) * fixedPointDivider;
...
#ifdef SEND_WARPED_LOCATIONS
    matlab.EvalStringEcho("warpedPoints(:,end+1) = [%f, %f];", warpedXf,
                          warpedYf);
#endif
  ...
  return RESULT_OK;
   ...
   // const Quadrilateral<s16> &quad, const Array<f32> &homography, s16
   // &meanValue)

const FixedLengthList<Point<s16>> &FiducialMarkerParserBit::get_probeLocations()
    const {
...
FiducialMarkerParser::FiducialMarkerParser(MemoryStack &memory) {
  this->bits = FixedLengthList<FiducialMarkerParserBit>(
      MAX_FIDUCIAL_MARKER_BITS, memory);
  InitializeAsDefaultParser(memory);
}

FiducialMarkerParser::FiducialMarkerParser(
    const FiducialMarkerParser &marker2) {
...
//  3. Upper right
//  4. Lower right
Result FiducialMarkerParser::ExtractBlockMarker(const Array<u8> &image,
                                                const Quadrilateral<s16> &quad,
                                                const Array<f32> &homography,
  ...
  for (s32 bit = 0; bit < numBits; bit++) {
    if ((lastResult = bits[bit].ExtractMeanValue(image, quad, homography,
                                                 meanValues[bit])) != RESULT_OK)
      return lastResult;
  }
...
#ifdef SEND_PROBE_LOCATIONS
  {
    ...
    for (s32 i = 0; i < numBits; i++) {
      FixedLengthList<Point<s16>> probeLocations =
          this->bits[i].get_probeLocations();
      matlab.Put(probeLocations.Pointer(0), probeLocations.get_size(),
                 "probeLocationsTmp");
      matlab.EvalStringEcho(
          "probeLocations(:, (end+1):(end+size(probeLocationsTmp,2))) = "
          "probeLocationsTmp;");
    }

    matlab.EvalStringEcho("probeLocations = double(probeLocations) / (2^%d)",
                          this->bits[0].get_numFractionalBits());
  }
  ...
  if ((lastResult = FiducialMarkerParser::
           DetermineOrientationAndBinarizeAndReorderCorners(
               meanValues, minContrastRatio, marker, binarizedBits, scratch)) !=
      RESULT_OK)
    return lastResult;
  ...
  if ((lastResult = DecodeId(binarizedBits, marker.blockType, marker.faceType,
                             scratch)) != RESULT_OK)
    return lastResult;
...
FiducialMarkerParser &FiducialMarkerParser::operator=(
    const FiducialMarkerParser &marker2) {
  this->bits = marker2.bits;
...
Result FiducialMarkerParser::InitializeAsDefaultParser(MemoryStack &memory) {
  if (INITIALIZE_WITH_DEFINITION_TYPE == 0) {
    ...
    AnkiAssert(NUM_PROBES_PER_BIT_TYPE_0 <=
               MAX_FIDUCIAL_MARKER_BIT_PROBE_LOCATIONS);

    this->bits.Clear();

    for (s32 i = 0; i < NUM_BITS_TYPE_0; i++) {
      this->bits.PushBack(FiducialMarkerParserBit(
          probesX_type0[i], probesY_type0[i], probeWeights_type0[i],
          NUM_PROBES_PER_BIT_TYPE_0, bitTypes_type0[i],
          NUM_FRACTIONAL_BITS_TYPE_0, memory));
    }
  }  // if(INITIALIZE_WITH_DEFINITION_TYPE == 0)

  upBitIndex = FindFirstBitOfType(
      FiducialMarkerParserBit::FIDUCIAL_BIT_ORIENTATION_UP, 0);
  downBitIndex = FindFirstBitOfType(
      FiducialMarkerParserBit::FIDUCIAL_BIT_ORIENTATION_DOWN, 0);
  leftBitIndex = FindFirstBitOfType(
      FiducialMarkerParserBit::FIDUCIAL_BIT_ORIENTATION_LEFT, 0);
  rightBitIndex = FindFirstBitOfType(
      FiducialMarkerParserBit::FIDUCIAL_BIT_ORIENTATION_RIGHT, 0);

  // This should only fail if there was an issue with the FiducialMarkerParser
  // creation
  AnkiAssert(upBitIndex >= 0 && downBitIndex >= 0 && leftBitIndex >= 0 &&
             rightBitIndex >= 0);

  return RESULT_OK;
}

Result FiducialMarkerParser::DetermineOrientationAndBinarizeAndReorderCorners(
    const FixedLengthList<s16> &meanValues, const f32 minContrastRatio,
    BlockMarker &marker, FixedLengthList<u8> &binarizedBits,
    MemoryStack scratch) const {
  AnkiConditionalErrorAndReturnValue(
      meanValues.IsValid(), RESULT_FAIL_INVALID_OBJECT,
      "FiducialMarkerParser::DetermineOrientation", "meanValues is not valid");

  AnkiConditionalErrorAndReturnValue(
      binarizedBits.IsValid(), RESULT_FAIL_INVALID_OBJECT,
      "FiducialMarkerParser::DetermineOrientation",
      "binarizedBits is not valid");
  ...
  const s16 maxValue =
      MAX(upBitValue, MAX(downBitValue, MAX(leftBitValue, rightBitValue)));
  ...
  if (upBitValue == maxValue) {
    ...
    bitReadingOrder[8] = 8;
    bitReadingOrder[9] = 9;
    bitReadingOrder[10] = 10;
    bitReadingOrder[11] = 11;
    bitReadingOrder[12] = 12;
    bitReadingOrder[13] = 13;
    bitReadingOrder[14] = 14;
    bitReadingOrder[15] = 15;
    bitReadingOrder[16] = 16;
    bitReadingOrder[17] = 17;
    bitReadingOrder[18] = 18;
    bitReadingOrder[19] = 19;
    bitReadingOrder[20] = 20;
    bitReadingOrder[21] = 21;
    bitReadingOrder[22] = 22;
    bitReadingOrder[23] = 23;
    bitReadingOrder[24] = 24;
  } else if (downBitValue == maxValue) {
    marker.orientation = BlockMarker::ORIENTATION_DOWN;
    marker.corners = Quadrilateral<s16>(marker.corners[3], marker.corners[2],
                                        marker.corners[1], marker.corners[0]);
    darkValue = (upBitValue + leftBitValue + rightBitValue) / 3;

    bitReadingOrder[0] = 24;
    bitReadingOrder[1] = 23;
    bitReadingOrder[2] = 22;
    bitReadingOrder[3] = 21;
    bitReadingOrder[4] = 20;
    bitReadingOrder[5] = 19;
    bitReadingOrder[6] = 18;
    bitReadingOrder[7] = 17;
    bitReadingOrder[8] = 16;
    bitReadingOrder[9] = 15;
    bitReadingOrder[10] = 14;
    bitReadingOrder[11] = 13;
    bitReadingOrder[12] = 12;
    bitReadingOrder[13] = 11;
    bitReadingOrder[14] = 10;
    bitReadingOrder[15] = 9;
    bitReadingOrder[16] = 8;
    bitReadingOrder[17] = 7;
    bitReadingOrder[18] = 6;
    bitReadingOrder[19] = 5;
    bitReadingOrder[20] = 4;
    bitReadingOrder[21] = 3;
    bitReadingOrder[22] = 2;
    bitReadingOrder[23] = 1;
    bitReadingOrder[24] = 0;
    ...
    marker.corners = Quadrilateral<s16>(marker.corners[1], marker.corners[3],
                                        marker.corners[0], marker.corners[2]);
    darkValue = (upBitValue + downBitValue + rightBitValue) / 3;

    bitReadingOrder[0] = 4;
    bitReadingOrder[1] = 9;
    bitReadingOrder[2] = 14;
    bitReadingOrder[3] = 19;
    bitReadingOrder[4] = 24;
    bitReadingOrder[5] = 3;
    bitReadingOrder[6] = 8;
    bitReadingOrder[7] = 13;
    bitReadingOrder[8] = 18;
    bitReadingOrder[9] = 23;
    bitReadingOrder[10] = 2;
    bitReadingOrder[11] = 7;
    bitReadingOrder[12] = 12;
    bitReadingOrder[13] = 17;
    bitReadingOrder[14] = 22;
    bitReadingOrder[15] = 1;
    bitReadingOrder[16] = 6;
    bitReadingOrder[17] = 11;
    bitReadingOrder[18] = 16;
    bitReadingOrder[19] = 21;
    bitReadingOrder[20] = 0;
    bitReadingOrder[21] = 5;
    bitReadingOrder[22] = 10;
    bitReadingOrder[23] = 15;
    bitReadingOrder[24] = 20;
    ...
    marker.corners = Quadrilateral<s16>(marker.corners[2], marker.corners[0],
                                        marker.corners[3], marker.corners[1]);
    darkValue = (upBitValue + downBitValue + leftBitValue) / 3;

    bitReadingOrder[0] = 20;
    bitReadingOrder[1] = 15;
    bitReadingOrder[2] = 10;
    bitReadingOrder[3] = 5;
    bitReadingOrder[4] = 0;
    bitReadingOrder[5] = 21;
    bitReadingOrder[6] = 16;
    bitReadingOrder[7] = 11;
    bitReadingOrder[8] = 6;
    bitReadingOrder[9] = 1;
    bitReadingOrder[10] = 22;
    bitReadingOrder[11] = 17;
    bitReadingOrder[12] = 12;
    bitReadingOrder[13] = 7;
    bitReadingOrder[14] = 2;
    bitReadingOrder[15] = 23;
    bitReadingOrder[16] = 18;
    bitReadingOrder[17] = 13;
    bitReadingOrder[18] = 8;
    bitReadingOrder[19] = 3;
    bitReadingOrder[20] = 24;
    bitReadingOrder[21] = 19;
    bitReadingOrder[22] = 14;
    bitReadingOrder[23] = 9;
    bitReadingOrder[24] = 4;
  ...
  if (static_cast<f32>(brightValue) <
      minContrastRatio * static_cast<f32>(darkValue)) {
    marker.orientation = BlockMarker::ORIENTATION_UNKNOWN;
    return RESULT_OK;  // Low contrast is not really a failure, as it may be due
                       // to an invalid detection
  }
...
// Starting at startIndex, search through this->bits to find the first instance
// of the given type Returns -1 if the type wasn't found
s32 FiducialMarkerParser::FindFirstBitOfType(
    const FiducialMarkerParserBit::Type type, const s32 startIndex) const {
  AnkiConditionalErrorAndReturnValue(startIndex >= 0, -1,
                                     "FiducialMarkerParser::FindFirstBitOfType",
                                     "startIndex < 0");

  const s32 numBits = bits.get_size();
  for (s32 i = startIndex; i < numBits; i++) {
    if (bits[i].get_type() == type) return i;
  }  // for(s32 i=startIndex;i<numBits; i++)

  return -1;
}

Result FiducialMarkerParser::DecodeId(const FixedLengthList<u8> &binarizedBits,
                                      s16 &blockType, s16 &faceType,
                                      MemoryStack scratch) const {
  blockType = -1;
  faceType = -1;
  ...
  for (s32 bit = 0; bit < numBinarizedBits; bit++) {
    ...
    } else if (bits[bit].get_type() ==
               FiducialMarkerParserBit::FIDUCIAL_BIT_FACE) {
      faceBits.PushBack(binarizedBits[bit]);
    } else if (bits[bit].get_type() ==
               FiducialMarkerParserBit::FIDUCIAL_BIT_CHECKSUM) {
...
bool FiducialMarkerParser::IsChecksumValid(
    const FixedLengthList<u8> &checksumBits,
    const FixedLengthList<u8> &blockBits, const FixedLengthList<u8> &faceBits) {
  ...
  for (s32 i_check = 1; i_check <= numChecksumBits; i_check++) {
    ...
    const s32 expectedChecksumBit =
        faceBits[i_face - 1] ^
        (blockBits[i_block1 - 1] ^ blockBits[i_block2 - 1]);

    if (checksumBits[i_check - 1] != expectedChecksumBit) return false;
...
VisionMarker::VisionMarker()
    : corners(Point<f32>(-1.0f, -1.0f), Point<f32>(-1.0f, -1.0f),
              Point<f32>(-1.0f, -1.0f), Point<f32>(-1.0f, -1.0f)),
...
VisionMarker::VisionMarker(const Quadrilateral<s16> &corners,
                           const ValidityCode validity)
    : markerType(Anki::Vision::MARKER_UNKNOWN),
      observedOrientation(0),
      validity(validity) {
...
VisionMarker::VisionMarker(const Quadrilateral<f32> &corners,
                           const ValidityCode validity)
    : corners(corners),
      markerType(Anki::Vision::MARKER_UNKNOWN),
      observedOrientation(0),
...
void VisionMarker::Print() const {
  ...
  if (static_cast<s32>(markerType) >= 0 &&
      static_cast<s32>(markerType) <= Vision::NUM_MARKER_TYPES) {
    typeString = Vision::MarkerTypeStrings[markerType];
  }

  CoreTechPrint(
      "[Type %d-%s]: (%0.2f,%0.2f) (%0.2f,%0.2f) (%0.2f,%0.2f) (%0.2f,%0.2f)] ",
      markerType, typeString, corners[0].x, corners[0].y, corners[1].x,
      corners[1].y, corners[2].x, corners[2].y, corners[3].x, corners[3].y);
}  // VisionMarker::Print()

Result VisionMarker::Serialize(const char *objectName,
...
Result VisionMarker::SerializeRaw(const char *objectName, void **buffer,
                                  s32 &bufferLength) const {
  if (SerializedBuffer::SerializeDescriptionStrings(
          "VisionMarker", objectName, buffer, bufferLength) != RESULT_OK)
    return RESULT_FAIL;

  SerializedBuffer::SerializeRawBasicType<Quadrilateral<f32>>(
      "corners", this->corners, buffer, bufferLength);
  SerializedBuffer::SerializeRawBasicType<s32>("markerType", this->markerType,
                                               buffer, bufferLength);
  SerializedBuffer::SerializeRawBasicType<ValidityCode>(
      "validity", this->validity, buffer, bufferLength);
  SerializedBuffer::SerializeRawBasicType<f32>(
      "observedOrientation", this->observedOrientation, buffer, bufferLength);

  return RESULT_OK;
}

Result VisionMarker::Deserialize(char *objectName, void **buffer,
                                 s32 &bufferLength, MemoryStack scratch) {
  // TODO: check if the name is correct
  if (SerializedBuffer::DeserializeDescriptionStrings(
          NULL, objectName, buffer, bufferLength) != RESULT_OK)
    return RESULT_FAIL;

  this->corners = SerializedBuffer::DeserializeRawBasicType<Quadrilateral<f32>>(
      NULL, buffer, bufferLength);
  this->markerType = static_cast<Vision::MarkerType>(
      SerializedBuffer::DeserializeRawBasicType<s32>(NULL, buffer,
                                                     bufferLength));
  this->validity = SerializedBuffer::DeserializeRawBasicType<ValidityCode>(
      NULL, buffer, bufferLength);
  this->observedOrientation = SerializedBuffer::DeserializeRawBasicType<f32>(
      NULL, buffer, bufferLength);
...
NearestNeighborLibrary &VisionMarker::GetNearestNeighborLibrary() {
  ...
  static NearestNeighborLibrary nearestNeighborLibrary(_dataPlatform,
                                                       ...
                                                       NUM_PROBES,
                                                       ProbeCenters_X,
  ProbeCenters_Y, ProbePoints_X,  ProbePoints_Y, NUM_PROBE_POINTS,
                                                       NUM_FRACTIONAL_BITS);
   */

  static NearestNeighborLibrary nearestNeighborLibrary(
      NearestNeighborData, NearestNeighborWeights, NearestNeighborLabels,
      NUM_MARKERS_IN_LIBRARY, NUM_PROBES, ProbeCenters_X, ProbeCenters_Y,
      ProbePoints_X, ProbePoints_Y, NUM_PROBE_POINTS, NUM_FRACTIONAL_BITS);
...
Vision::ConvolutionalNeuralNet &VisionMarker::GetCNN() {
  ...
  if (!cnn.IsLoaded()) {
    AnkiConditionalErrorAndReturnValue(
        _dataPath.empty() == false, cnn, "VisionMarker.GetCNN.NoDataPath",
...
void VisionMarker::Initialize() {
  ...
  if (VisionMarker::areTreesInitialized == false) {
    ...
    for (u32 iTree = 0; iTree < NUM_TREES; ++iTree) {
      VisionMarker::multiClassTrees[iTree] = FiducialMarkerDecisionTree(
          reinterpret_cast<const u8 *>(MultiClassNodes[iTree]),
          NUM_NODES_MULTICLASS[iTree], TREE_NUM_FRACTIONAL_BITS,
          MAX_DEPTH_MULTICLASS[iTree], ProbePoints_X, ProbePoints_Y,
...
Result VisionMarker::GetProbeValues(const Array<u8> &image,
  ...
  AnkiConditionalErrorAndReturnValue(
      AreValid(image, homography), RESULT_FAIL_INVALID_OBJECT,
      "VisionMarker::GetProbeValues", "Invalid objects");
  ...
  const f32 fixedPointDivider =
      1.0f / static_cast<f32>(1 << VisionMarker::NUM_FRACTIONAL_BITS);

  AnkiConditionalErrorAndReturnValue(
      probeValues.rows * probeValues.cols == VisionMarker::NUM_PROBES,
      RESULT_FAIL_INVALID_SIZE, "VisionMarker.GetProbeValues",
  ...
  for (s32 iOffset = 0; iOffset < numProbeOffsets; iOffset++) {
    probeXOffsetsF32[iOffset] =
        static_cast<f32>(VisionMarker::ProbePoints_X[iOffset]) *
        fixedPointDivider;
    probeYOffsetsF32[iOffset] =
        static_cast<f32>(VisionMarker::ProbePoints_Y[iOffset]) *
        fixedPointDivider;
  }
  ...
  for (s32 iProbe = 0; iProbe < numProbes; ++iProbe) {
    const f32 xCenter = static_cast<f32>(VisionMarker::ProbeCenters_X[iProbe]) *
                        fixedPointDivider;
    const f32 yCenter = static_cast<f32>(VisionMarker::ProbeCenters_Y[iProbe]) *
                        fixedPointDivider;
    ...
    for (s32 iOffset = 0; iOffset < numProbeOffsets; iOffset++) {
      ...
      // This should only fail if there's a bug in the quad extraction
      AnkiAssert(warpedY >= 0 && warpedX >= 0 && warpedY < image.get_size(0) &&
                 warpedX < image.get_size(1));
  ...
  /*
   ...
   for(s32 iProbe=0; iProbe<numProbes; ++iProbe) {
   pProbeData[iProbe] = (255*static_cast<s32>(pProbeData[iProbe] - minValue)) /
   divisor;
   }
   */

  // Illumination normalization code (assuming we are not using the same
  // filtering used by the fiducial detector to improve quad refinment
  // precision)
  ...
  if (doIlluminationNormalization) {
    ...
    cv::Mat_<u8> temp(VisionMarker::GRIDSIZE, VisionMarker::GRIDSIZE,
                      probeValues.data);
    ...
    if (kernel.empty()) {
      ...
      kernel = -1;
      kernel((halfSize - 1) / 2, (halfSize - 1) / 2) =
          kernel.rows * kernel.cols - 1;
    }
...
Vision::MarkerType VisionMarker::RemoveOrientation(
    Vision::MarkerType orientedMarker) {
#if RECOGNITION_METHOD == RECOGNITION_METHOD_NEAREST_NEIGHBOR || \
    RECOGNITION_METHOD == RECOGNITION_METHOD_CNN
...
Result VisionMarker::ComputeBrightDarkValues(const Array<u8> &image,
  ...
  for (s32 i_pt = 0; i_pt < NUM_PROBE_POINTS; i_pt++) {
    probePointsX_F32[i_pt] =
        static_cast<f32>(ProbePoints_X[i_pt]) * fixedPointDivider;
    probePointsY_F32[i_pt] =
        static_cast<f32>(ProbePoints_Y[i_pt]) * fixedPointDivider;
  }
  ...
  {
    static const Quadrilateral<f32> outsideQuad(
        Point<f32>(0, 0), Point<f32>(0, 1), Point<f32>(1, 1), Point<f32>(1, 0));
    static const Quadrilateral<f32> insideQuad(
        Point<f32>(0.2, 0.2), Point<f32>(0.2, .8), Point<f32>(.8, .8),
        Point<f32>(0.8, 0.2));
    static const Quadrilateral<f32> middleQuad(
        Point<f32>(0.1, 0.1), Point<f32>(0.1, .9), Point<f32>(.9, .9),
        Point<f32>(0.9, 0.1));
    static const Quadrilateral<f32> *quads[3] = {&outsideQuad, &insideQuad,
                                                 &middleQuad};
  ...
  u32 totalForegroundAccumulator = 0,
      totalBackgroundAccumulator = 0;  // for all pairs
  for (s32 i_probe = 0; i_probe < NUM_THRESHOLD_PROBES; ++i_probe) {
    const f32 xCenterDark =
        static_cast<f32>(ThresholdDarkProbe_X[i_probe]) * fixedPointDivider;
    const f32 yCenterDark =
        static_cast<f32>(ThresholdDarkProbe_Y[i_probe]) * fixedPointDivider;

    const f32 xCenterBright =
        static_cast<f32>(ThresholdBrightProbe_X[i_probe]) * fixedPointDivider;
    const f32 yCenterBright =
        static_cast<f32>(ThresholdBrightProbe_Y[i_probe]) * fixedPointDivider;
    ...
    for (s32 i_pt = 0; i_pt < NUM_PROBE_POINTS; i_pt++) {
      {  // Dark points
        ...
        // This should only fail if there's a bug in the quad extraction
        AnkiAssert(warpedY >= 0 && warpedX >= 0 && warpedY < imageHeight &&
                   warpedX < imageWidth);
        ...
        if (probePointsX_F32[i_pt] == 0.f && probePointsY_F32[i_pt] == 0.f) {
          cv::circle(brightDarkPairsImage, cv::Point(warpedX, warpedY), 2,
    ...
    enoughContrast =
        (isDarkOnLight ? backgroundValue > minContrastRatio * foregroundValue
  ...
  const f32 totalDivisor =
      1.f / static_cast<f32>(NUM_PROBE_POINTS * NUM_THRESHOLD_PROBES);
...
Result VisionMarker::RefineCorners(
    const Array<u8> &image, const f32 minContrastRatio,
    ...
    const f32 refine_quadRefinementMinCornerChange, const s32 quads_minQuadArea,
    const s32 quads_quadSymmetryThreshold,
    const s32 quads_minDistanceFromImageEdge,
    const Point<f32> &fiducialThicknessFraction,
    const Point<f32> &roundedCornersFraction, const bool isDarkOnLight,
    u8 &meanGrayvalueThreshold, MemoryStack scratch) {
  ...
  if ((lastResult = this->ComputeBrightDarkValues(
           image, minContrastRatio, isDarkOnLight, backgroundValue,
           foregroundValue, enoughContrast)) != RESULT_OK) {
    return lastResult;
  }
  ...
  if (enoughContrast) {
    ...
    meanGrayvalueThreshold =
        static_cast<u8>(0.5f * (backgroundValue + foregroundValue));

    if (refine_quadRefinementIterations > 0) {
      BeginBenchmark("vmrc_quadrefine");

      if ((lastResult = RefineQuadrilateral(
               initQuad, initHomography, image, fiducialThicknessFraction,
               roundedCornersFraction, refine_quadRefinementIterations,
               foregroundValue, backgroundValue, refine_numRefinementSamples,
               refine_quadRefinementMaxCornerChange,
               refine_quadRefinementMinCornerChange, this->corners,
               this->homography, scratch)) != RESULT_OK) {
        // TODO: Don't fail? Just warn and keep original quad?
        AnkiConditionalErrorAndReturnValue(
            lastResult == RESULT_OK, lastResult, "RefineQuadrilateral",
      ...
      const bool isReasonable = IsQuadrilateralReasonable(
          refinedQuadS16, quads_minQuadArea, quads_quadSymmetryThreshold,
          quads_minDistanceFromImageEdge, image.get_size(0), image.get_size(1),
          areCornersDisordered);
      ...
      // If we're not refining, the refined homography is the same as the
      // initial one
...
Result VisionMarker::Extract(
    ...
    const u8 grayvalueThreshold,  // Trees: for thresholding probes, NN: max
                                  // distance threshold
    const f32 minContrastRatio, MemoryStack scratch) {
#if RECOGNITION_METHOD == RECOGNITION_METHOD_DECISION_TREES
  using namespace VisionMarkerDecisionTree;
#endif
  ...
  //  RESULT_FAIL, "VisionMarker::Extract", "Marker is not initialized. Perhaps
  //  RefineCorners() was not called, or Extract() was called twice.");
  ...
  lastResult = VisionMarker::GetNearestNeighborLibrary().GetNearestNeighbor(
      image, homography, grayvalueThreshold, label, distance);
  ...
  const std::chrono::steady_clock::time_point endTime =
      std::chrono::steady_clock::now();
  const std::chrono::milliseconds nnTime =
  ...
  if (label != -1) {
    if (distance < grayvalueThreshold) {
      ...
      AnkiWarn("VisionMarker.Extract",
               "Valid label returned as nearest neighbor, but distance (%d) >= "
               "threshold (%d)",
  ...
  for (s32 iTree = 0; iTree < NUM_TREES; ++iTree) {
    s32 tempLabel;
    if ((lastResult = VisionMarker::multiClassTrees[iTree].Classify(
             image, homography, grayvalueThreshold, tempLabel)) != RESULT_OK) {
      return lastResult;
    }
  ...
  AnkiConditionalErrorAndReturnValue(maxVotes > 0, RESULT_FAIL,
                                     "VisionMarker.Extract.NoVotes",
                                     "No votes given to any marker label")
      AnkiConditionalErrorAndReturnValue(selectedLabel >= 0, RESULT_FAIL,
                                         "VisionMarker.Extract.NoBestLabel",
                                         "No label with max votes selected");

  const f32 numVotesForMajority = static_cast<f32>(NUM_TREES) * 0.5f;
  verified =
      (static_cast<f32>(maxVotes) > numVotesForMajority &&
       selectedLabel != MARKER_INVALID_000 && selectedLabel != MARKER_UNKNOWN);
  ...
  size_t classLabel;
  static cv::Mat_<u8> _probeValues(VisionMarker::GRIDSIZE,
  ...
  if (verified) {
    ...
    for (s32 i_corner = 0; i_corner < 4; ++i_corner) {
      this->corners[i_corner] =
          initQuad[CornerReorderLUT[selectedLabel][i_corner]];
    }
...
#       ifdef OUTPUT_FAILED_MARKER_STEPS
    AnkiWarn("VisionMarker::Extract", "Verify failed or UNKNOWN/INVALID marker
detected"); #       endif
     */
...
Result VisionMarker::ExtractExhaustive(
    const VisionMarkerImages &allMarkerImages, const Array<u8> &image,
    MemoryStack fastScratch, MemoryStack slowScratch) {
  VisionMarker matchedMarker;
  f32 matchQuality;

  allMarkerImages.MatchExhaustive(image, this->corners, matchedMarker,
                                  matchQuality, fastScratch, slowScratch);
...
VisionMarkerImages::VisionMarkerImages(
    const FixedLengthList<const char *> &imageFilenames, MemoryStack &memory)
    : isValid(false) {
#if ANKICORETECH_EMBEDDED_USE_OPENCV
  ...
  {
    const cv::Mat image =
        cv::imread(imageFilenames[0], CV_LOAD_IMAGE_UNCHANGED);
    ...
    AnkiConditionalErrorAndReturn(databaseImageWidth == databaseImageHeight,
                                  "VisionMarkerImages::VisionMarkerImages",
                                  "All images must be equal size and square");
  }

  /*images = FixedLengthList<Array<u8> >(numImages, memory, Flags::Buffer(false,
   * false, true));*/
  databaseImages =
      Array<u8>(databaseImageHeight, databaseImageWidth * numDatabaseImages,
                memory, Flags::Buffer(true, false, true));
  databaseLabelIndexes = FixedLengthList<Anki::Vision::MarkerType>(
      numDatabaseImages, memory, Flags::Buffer(false, false, true));
  ...
  for (s32 iFile = 0; iFile < numDatabaseImages; iFile++) {
    ...
    AnkiConditionalErrorAndReturn(
        image.rows == databaseImageHeight && image.rows == databaseImageWidth,
        "VisionMarkerImages::VisionMarkerImages",
        "All images must be equal size and square");
...
#else
  AnkiError("VisionMarkerImages::VisionMarkerImages",
            "OpenCV is required to load files");
...
VisionMarkerImages::VisionMarkerImages(
    const s32 numDatabaseImages, const s32 databaseImageHeight,
    const s32 databaseImageWidth, u8 *pDatabaseImages,
    Anki::Vision::MarkerType *pDatabaseLabelIndexes)
    : numDatabaseImages(numDatabaseImages),
      databaseImageHeight(databaseImageHeight),
      databaseImageWidth(databaseImageWidth),
      isValid(true) {
  // Note the 2* lie about the size of the buffer
  this->databaseImages = Array<u8>(
      databaseImageHeight, numDatabaseImages * databaseImageWidth,
      pDatabaseImages,
      2 * databaseImageHeight * numDatabaseImages * databaseImageWidth,
      Flags::Buffer(false, false, true));
  this->databaseLabelIndexes = FixedLengthList<Anki::Vision::MarkerType>(
      numDatabaseImages, pDatabaseLabelIndexes,
      2 * numDatabaseImages * sizeof(Anki::Vision::MarkerType),
      Flags::Buffer(false, false, true));
}
...
Result VisionMarkerImages::MatchExhaustive(const Array<u8> &image,
                                           const Quadrilateral<f32> &quad,
                                           VisionMarker &extractedMarker,
                                           f32 &matchQuality,
                                           MemoryStack fastScratch,
                                           MemoryStack slowScratch) const {
  ...
  AnkiConditionalErrorAndReturnValue(
      NotAliased(fastScratch, slowScratch), RESULT_FAIL_ALIASED_MEMORY,
      "VisionMarkerImages::MatchExhaustive",
      "fastScratch and slowScratch must be different");

  // 1. Compute the transformation from the quad to the known marker images
  // allImagesCorners = [0 0 allImageSize(2) allImageSize(2); 0 allImageSize(1)
  // 0 allImageSize(1)]';
  Quadrilateral<f32> databaseImagesCorners(
      ...
      Point<f32>(static_cast<f32>(databaseImageWidth),
                 static_cast<f32>(databaseImageHeight)));

  Array<f32> homography(3, 3, fastScratch);

  AnkiConditionalErrorAndReturnValue(
      homography.IsValid(), RESULT_FAIL_OUT_OF_MEMORY,
      "VisionMarkerImages::MatchExhaustive", "Out of memory");

  bool numericalFailure;
  Transformations::ComputeHomographyFromQuads(
      quad, databaseImagesCorners, homography, numericalFailure, fastScratch);

  // 2. For each pixel in the quad in the input image, compute the mean-absolute
  // difference with each known marker image Based off
  // DrawFilledConvexQuadrilateral()
  ...
  const f32 h12 = homography[1][2];
  const f32 h20 = homography[2][0];
  const f32 h21 = homography[2][1];  // const f32 h22 = 1.0f;

  const IntegerCounts imageCounts(image, quad, 1, 1, fastScratch);
  IntegerCounts::Statistics imageCountsStatistics =
      imageCounts.ComputeStatistics();
  ...
  AnkiConditionalErrorAndReturnValue(
      toShowImages.IsValid(), RESULT_FAIL_OUT_OF_MEMORY,
      "VisionMarkerImages::MatchExhaustive", "Out of memory");

  for (s32 iRotation = 0; iRotation < 4; iRotation++) {
    for (s32 iDatabase = 0; iDatabase < numDatabaseImages; iDatabase++) {
      toShowImages[iRotation][iDatabase] =
          Array<u8>(imageHeight, imageWidth, slowScratch);

      AnkiConditionalErrorAndReturnValue(
          toShowImages[iRotation][iDatabase].IsValid(),
          RESULT_FAIL_OUT_OF_MEMORY, "VisionMarkerImages::MatchExhaustive",
          "Out of memory");
  ...
  s32 *totalDifferences = reinterpret_cast<s32 *>(fastScratch.Allocate(
      4 * sizeof(s32) * numDatabaseImages, true, numBytesAllocated));
  s32 numInBounds = 0;

  AnkiAssert(totalDifferences != NULL);

  // const u8 * restrict * restrict pDatabaseImages = reinterpret_cast<const
  // u8**>(fastScratch.Allocate(sizeof(u8*)*numDatabaseImages,true,numBytesAllocated));
  // u8 ** pDatabaseImages =
  // reinterpret_cast<u8**>(fastScratch.Allocate(sizeof(u8*)*numDatabaseImages,true,numBytesAllocated));
  ...
  for (s32 iy = 0; iy < numYs; iy += yIncrement) {
    ...
    for (s32 x = minXS32; x <= maxXS32; x += xIncrement) {
      // Do nearest-neighbor lookup from the query image to the image in the
      // dataset
      ...
      if (xTransformed0 < 0 || xTransformed0 >= databaseImageWidth ||
          yTransformed0 < 0 || yTransformed0 >= databaseImageWidth) {
        continue;
      }
      ...
      const s32 xTransformed[4] = {xTransformed0, xTransformed90,
                                   xTransformed180, xTransformed270};
      const s32 yTransformed[4] = {yTransformed0, yTransformed90,
                                   yTransformed180, yTransformed270};
      ...
      for (s32 iRotation = 0; iRotation < 4; iRotation++) {
        const u8 *restrict pDatabaseImages =
            pDatabaseImages_start +
            numDatabaseImages * (yTransformed[iRotation] * databaseImageWidth +
                                 xTransformed[iRotation]);

        for (s32 iDatabase = 0; iDatabase < numDatabaseImages; iDatabase++) {
          const s32 databaseImageValue = pDatabaseImages[iDatabase];
          totalDifferences[numDatabaseImages * iRotation + iDatabase] +=
              ABS(curImageValue - databaseImageValue);
#ifdef SHOW_EXHAUSTIVE_STEPS
  ...
  for (s32 iRotation = 0; iRotation < 4; iRotation++) {
    ...
    for (s32 iDatabase = 0; iDatabase < numDatabaseImages; iDatabase++) {
      ...
      const s32 curTotalDifference =
          totalDifferences[numDatabaseImages * iRotation + iDatabase];

      // CoreTechPrint("(%d,%d) curTotalDifference = %f\n", iRotation,
      // iDatabase, curTotalDifference / (255.0f *
      // static_cast<f32>(numInBounds)));
  ...
  matchQuality =
      bestDatabaseDifference / (255.0f * static_cast<f32>(numInBounds));
...
bool VisionMarkerImages::IsValid() const {
  if (!databaseImages.IsValid()) return false;

  if (!databaseLabelIndexes.IsValid()) return false;
...
const FixedLengthList<Anki::Vision::MarkerType>
    &VisionMarkerImages::get_databaseLabelIndexes() {
