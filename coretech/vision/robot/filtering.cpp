...
template <>
Result BinomialFilter<u8, u8, u8>(const Array<u8> &image,
                                  Array<u8> &imageFiltered,
                                  MemoryStack scratch) {
  const u8 kernel0 = 1;
  const u8 kernel1 = 4;
  const u8 kernel2 = 6;
  ...
  AnkiConditionalErrorAndReturnValue(AreValid(image, imageFiltered, scratch),
                                     ...
                                     "BinomialFilter", "Invalid objects");

  AnkiConditionalErrorAndReturnValue(
      imageHeight == imageFiltered.get_size(0) &&
          imageWidth == imageFiltered.get_size(1),
      RESULT_FAIL_INVALID_SIZE, "BinomialFilter",
      "size(image) != size(imageFiltered) (%dx%d != %dx%d)", imageHeight,
      imageWidth, imageHeight, imageWidth);

  AnkiConditionalErrorAndReturnValue(
      NotAliased(image, imageFiltered), RESULT_FAIL_ALIASED_MEMORY,
      "BinomialFilter", "image and imageFiltered must be different");
  ...
  for (s32 y = 0; y < imageHeight; y++) {
    ...
    pImageFilteredTmp[x] = static_cast<u8>(
        (pImage[x] * kernel2 + pImage[x + 1] * kernel3 +
         pImage[x + 2] * kernel4 + pImage[x] * (kernel0 + kernel1)) >>
        kernelShift);
    x++;
    pImageFilteredTmp[x] =
        static_cast<u8>((pImage[x - 1] * kernel1 + pImage[x] * kernel2 +
                         pImage[x + 1] * kernel3 + pImage[x + 2] * kernel4 +
                         pImage[x - 1] * kernel0) >>
                        kernelShift);
    x++;
    ...
    pImageFilteredTmp[x] =
        static_cast<u8>((pImage[x - 2] * kernel0 + pImage[x - 1] * kernel1 +
                         pImage[x] * kernel2 + pImage[x + 1] * kernel3 +
                         pImage[x + 1] * kernel4) >>
                        kernelShift);
    x++;
    pImageFilteredTmp[x] = static_cast<u8>(
        (pImage[x - 2] * kernel0 + pImage[x - 1] * kernel1 +
         pImage[x] * kernel2 + pImage[x] * (kernel3 + kernel4)) >>
        kernelShift);
    x++;
    ...
    if (y > 1) {
      const u8 *restrict pImageFilteredTmpYm2 =
          pImageFilteredTmp - 4 * imageFilteredTmpStep;
      const u8 *restrict pImageFilteredTmpYm1 =
          pImageFilteredTmp - 3 * imageFilteredTmpStep;
      const u8 *restrict pImageFilteredTmpY0 =
          pImageFilteredTmp - 2 * imageFilteredTmpStep;
      const u8 *restrict pImageFilteredTmpYp1 =
          pImageFilteredTmp - imageFilteredTmpStep;
      ...
      if (y == 2) {
        for (s32 x = 0; x < imageWidth; x++) {
          const u16 prod012 =
              pImageFilteredTmpY0[x] * (kernel0 + kernel1 + kernel2);
  ...
  const u8 *restrict pImageFilteredTmpYEndm4 =
      imageFilteredTmp.Pointer(imageFilteredTmp.get_size(0) - 4, 0);
  ;
  const u8 *restrict pImageFilteredTmpYEndm3 =
      pImageFilteredTmpYEndm4 + imageFilteredTmpStep;
  const u8 *restrict pImageFilteredTmpYEndm2 =
      pImageFilteredTmpYEndm4 + 2 * imageFilteredTmpStep;
  const u8 *restrict pImageFilteredTmpYEndm1 =
      pImageFilteredTmpYEndm4 + 3 * imageFilteredTmpStep;

  u8 *restrict pImageFilteredYEndm2 =
      imageFiltered.Pointer(imageFiltered.get_size(0) - 2, 0);
  u8 *restrict pImageFilteredYEndm1 =
      imageFiltered.Pointer(imageFiltered.get_size(0) - 1, 0);

  for (s32 x = 0; x < imageWidth; x++) {
    pImageFilteredYEndm2[x] =
        static_cast<u8>((pImageFilteredTmpYEndm4[x] * kernel0 +
                         pImageFilteredTmpYEndm3[x] * kernel1 +
                         pImageFilteredTmpYEndm2[x] * kernel2 +
                         pImageFilteredTmpYEndm1[x] * (kernel3 + kernel4)) >>
                        kernelShift);
    pImageFilteredYEndm1[x] = static_cast<u8>(
        (pImageFilteredTmpYEndm3[x] * kernel0 +
         pImageFilteredTmpYEndm2[x] * kernel1 +
         pImageFilteredTmpYEndm1[x] * (kernel2 + kernel3 + kernel4)) >>
        kernelShift);
  }
...
void GetBitShiftDirectionAndMagnitude(const s32 in1_numFractionalBits,
                                      const s32 in2_numFractionalBit,
                                      const s32 out_numFractionalBit,
                                      s32 &shiftMagnitude, bool &shiftRight) {
  if ((in1_numFractionalBits + in2_numFractionalBit) > out_numFractionalBit) {
    shiftRight = true;
    shiftMagnitude =
        in1_numFractionalBits + in2_numFractionalBit - out_numFractionalBit;
  } else if ((in1_numFractionalBits + in2_numFractionalBit) <
             out_numFractionalBit) {
    shiftRight = false;
    shiftMagnitude =
        out_numFractionalBit - in1_numFractionalBits - in2_numFractionalBit;
  } else {
    shiftRight = false;
    shiftMagnitude = 0;
  }
...
Result BoxFilterNormalize(const Array<u8> &image, const s32 boxSize,
                          const u8 padValue, Array<u8> &imageNorm,
                          MemoryStack scratch) {
  Result lastResult = RESULT_OK;

  AnkiConditionalErrorAndReturnValue(
      image.IsValid(), RESULT_FAIL_INVALID_OBJECT, "BoxFilterNormalize",
  ...
  AnkiConditionalErrorAndReturnValue(
      imageNorm.IsValid(), RESULT_FAIL_INVALID_OBJECT, "BoxFilterNormalize",
      "Output normalized image is invalid.");

  AnkiConditionalErrorAndReturnValue(
      AreEqualSize(image, imageNorm), RESULT_FAIL_INVALID_SIZE,
      "BoxFilterNormalize",
  ...
  AnkiConditionalErrorAndReturnValue(
      integralImage.IsValid(), RESULT_FAIL_OUT_OF_MEMORY, "BoxFilterNormalize",
  ...
  for (s32 y = 0; y < imageHeight; y++) {
    ...
    const f32 *restrict pIntegralImageRowBehind =
        integralImage.Pointer(rowBehind, 0);
    const f32 *restrict pIntegralImageRowAhead =
        integralImage.Pointer(rowAhead, 0);

    // Left side
    for (s32 x = 0; x <= halfWidth; x++) {
      f32 OutOfBoundsArea =
          static_cast<f32>(boxArea - (x + halfWidth + 1) * inBoundsHeight);

      f32 boxSum =
          (pIntegralImageRowAhead[x + halfWidth] - pIntegralImageRowAhead[0] -
           pIntegralImageRowBehind[x + halfWidth] + pIntegralImageRowBehind[0] +
           OutOfBoundsArea * static_cast<f32>(padValue));

      pImageRowNorm[x] = static_cast<u8>(
          CLIP(outMean * (static_cast<f32>(pImageRow[x]) * boxArea) / boxSum,
               0.f, 255.f));
    }

    // Middle
    const f32 OutOfBoundsArea =
        static_cast<f32>(boxArea - boxWidth * inBoundsHeight);
    ...
    for (s32 x = halfWidth + 1; x < imageWidth - halfWidth; x++) {
      f32 boxSum = (pIntegralImageRowAhead[x + halfWidth] -
                    ...
                    pIntegralImageRowBehind[x - halfWidth - 1] + paddingSum);
    ...
    for (s32 x = imageWidth - halfWidth; x < imageWidth; x++) {
      f32 OutOfBoundsArea = static_cast<f32>(
          boxArea - (imageWidth - x + halfWidth) * inBoundsHeight);
...
template <>
Result BoxFilter<u8, u16, u16>(const Array<u8> &image, const s32 boxHeight,
                               const s32 boxWidth, Array<u16> &filtered,
                               MemoryStack scratch) {
  AnkiConditionalErrorAndReturnValue(image.IsValid() && filtered.IsValid(),
                                     RESULT_FAIL_INVALID_OBJECT, "BoxFilter",
                                     "Image is invalid");
  ...
  AnkiConditionalErrorAndReturnValue(
      AreEqualSize(image, filtered), RESULT_FAIL_INVALID_SIZE, "BoxFilter",
      "Output normalized image must match input image's size.");

  AnkiConditionalErrorAndReturnValue(imageWidth % 8 == 0,
                                     RESULT_FAIL_INVALID_SIZE, "BoxFilter",
                                     "Image width must be divisible by 8");

  AnkiConditionalErrorAndReturnValue(
      boxHeight > 2 && boxWidth > 2 && IsOdd(boxWidth) && IsOdd(boxHeight),
      RESULT_FAIL_INVALID_SIZE, "BoxFilter",
      "Box filter must be greater than two and odd");

  AnkiConditionalWarn(boxHeight * boxWidth <= 256, "BoxFilter",
                      "Filtering may overflow");

  s32 y;

  // Includes extra padding for simd
  u16 *restrict verticalAccumulator =
      reinterpret_cast<u16 *>(scratch.Allocate(imageWidth * sizeof(u16) + 16));
  ...
  for (y = 0; y < boxHeight; y++) {
    ...
    for (s32 x = 0; x < imageWidth; x += 8) {
      ...
      const u32 toAdd32 =
          ((image3210 & 0xFF0000) >> 16) | ((image3210 & 0xFF000000) >> 8);

      const u32 toAdd54 = (image7654 & 0xFF) | ((image7654 & 0xFF00) << 8);
      const u32 toAdd76 =
          ((image7654 & 0xFF0000) >> 16) | ((image7654 & 0xFF000000) >> 8);
  ...
  {
    // Grab the pointer to the horizontally negative-offset location in the
    // filtered image
    ...
    for (; x < imageWidth - 3; x += 4) {
      const u32 toAdd10 =
          *reinterpret_cast<const u32 *>(verticalAccumulator + x);
      const u32 toAdd32 =
          *reinterpret_cast<const u32 *>(verticalAccumulator + x + 2);

      u32 toSub10 =
          *reinterpret_cast<const u32 *>(verticalAccumulator + x - boxWidth);
      u32 toSub32 = *reinterpret_cast<const u32 *>(verticalAccumulator + x -
                                                   boxWidth + 2);
    ...
    for (; x < imageWidth; x++) {
      horizontalAccumulator +=
          verticalAccumulator[x] - verticalAccumulator[x - boxWidth];
  ...
  for (; y < imageHeight; y++) {
    ...
    for (s32 x = 0; x < imageWidth; x += 4) {
      ...
      const u32 toAdd10 =
          (imageNew3210 & 0xFF) | ((imageNew3210 & 0xFF00) << 8);
      const u32 toAdd32 = ((imageNew3210 & 0xFF0000) >> 16) |
                          ((imageNew3210 & 0xFF000000) >> 8);

      const u32 toSub10 =
          (imageOld3210 & 0xFF) | ((imageOld3210 & 0xFF00) << 8);
      const u32 toSub32 = ((imageOld3210 & 0xFF0000) >> 16) |
                          ((imageOld3210 & 0xFF000000) >> 8);

      *reinterpret_cast<u32 *>(verticalAccumulator + x) += toAdd10 - toSub10;
      *reinterpret_cast<u32 *>(verticalAccumulator + x + 2) +=
          toAdd32 - toSub32;
    }
...
Result DownsampleBilinear(const Array<u8> &in, Array<u8> &out,
                          MemoryStack scratch) {
  ...
  AnkiConditionalErrorAndReturnValue(AreValid(in, out, scratch),
                                     RESULT_FAIL_INVALID_OBJECT,
                                     "DownsampleBilinear", "Invalid objects");
  ...
  Point<f32> scale(static_cast<f32>(inWidth) / static_cast<f32>(outWidth),
                   static_cast<f32>(inHeight) / static_cast<f32>(outHeight));

  AnkiConditionalErrorAndReturnValue(
      scale.x > 1.0f && scale.y > 1.0f, RESULT_FAIL_INVALID_SIZE,
      "DownsampleBilinear", "out is larger than in");
  ...
  AnkiConditionalErrorAndReturnValue(AreValid(inX0s_S32, inX1s_S32, alphaXs),
                                     ...
                                     "DownsampleBilinear", "Out of memory");
  ...
  // Compute the x coordinates
  {
    ...
    for (s32 x = 0; x < outWidth; x++) {
      ...
      // Technically, we can't interpolate the borders. But this is a reasonable
      // approximation
      if (inX0_S32 < 0) inX0_S32 = 0;

      if (inX1_S32 < 0) inX1_S32 = 0;

      if (inX0_S32 > (inWidth - 1)) inX0_S32 = inWidth - 1;

      if (inX1_S32 > (inWidth - 1)) inX1_S32 = inWidth - 1;
  ...
  for (s32 y = 0; y < outHeight; y++) {
    ...
    // Technically, we can't interpolate the borders. But this is a reasonable
    // approximation
    if (inY0_S32 < 0) inY0_S32 = 0;

    if (inY1_S32 < 0) inY1_S32 = 0;

    if (inY0_S32 > (inHeight - 1)) inY0_S32 = inHeight - 1;

    if (inY1_S32 > (inHeight - 1)) inY1_S32 = inHeight - 1;
    ...
    for (; x < outWidth; x++) {
      ...
      // const f32 interpolatedPixelValueF32 =
      ...
      const u32 interpolatedPixelValue =
          alphaYinverseU32 * interpolatedTop + alphaYU32 * interpolatedBottom;
      const u32 interpolatedPixelValueScaled =
          interpolatedPixelValue >> (2 * numSubpixelBits);
  ...
  return RESULT_OK;
}  // Result DownsampleBilinear(const Array<u8> &in, Array<u8> &out, MemoryStack
   // scratch)

Result FastGradient(const Array<u8> &in, Array<s8> &dx, Array<s8> &dy,
                    MemoryStack scratch) {
  const s32 imageHeight = in.get_size(0);
  const s32 imageWidth = in.get_size(1);

  AnkiConditionalErrorAndReturnValue(
      in.IsValid() && dy.IsValid() && dx.IsValid() && scratch.IsValid(),
  ...
  AnkiConditionalErrorAndReturnValue(imageWidth % 8 == 0,
                                     RESULT_FAIL_INVALID_SIZE, "FastGradient",
                                     "Image width must be divisible by 8");
  ...
  for (s32 y = 1; y < (imageHeight - 1); y++) {
    ...
    for (x = 1; x < (imageWidth - 1); x++) {
      pDx[x] = static_cast<s8>((static_cast<s32>(pIn_y0[x + 1]) >> 1) -
                               (static_cast<s32>(pIn_y0[x - 1]) >> 1));
    }
    //#else // #if !defined(USE_ARM_ACCELERATION)
    //          // TODO: make work so Keil doesn't merge loads
    //          for(x = 0; x<(imageWidth-7); x+=8) {
    //            const u32 inM3210 = *reinterpret_cast<const u32*>(pIn_y0 + x -
    //            1); const u32 inM7654 = *reinterpret_cast<const u32*>(pIn_y0 +
    //            x + 3);
    //
    //            const u32 inP3210 = *reinterpret_cast<const u32*>(pIn_y0 + x +
    //            1); const u32 inP7654 = *reinterpret_cast<const u32*>(pIn_y0 +
    //            x + 5);
    //
    ...
    ACCELERATION_TYPE == ACCELERATION_ARM_A7
    for (x = 1; x < (imageWidth - 1); x++) {
      pDy[x] = static_cast<s8>((static_cast<s32>(pIn_yp1[x]) >> 1) -
                               (static_cast<s32>(pIn_ym1[x]) >> 1));
    }
#elif ACCELERATION_TYPE == ACCELERATION_ARM_M4
    // pIn_ym1 and pIn_yp1 should always be aligned, so this is okay even with
    // auto-load-merging
    for (x = 0; x < (imageWidth - 7); x += 8) {
