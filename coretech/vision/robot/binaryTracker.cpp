...
BinaryTracker::EdgeDetectionParameters::EdgeDetectionParameters()
    : type(EDGE_TYPE_GRAYVALUE),
      threshold_yIncrement(-1),
      threshold_xIncrement(-1),
      threshold_blackPercentile(-1),
      threshold_whitePercentile(-1),
      threshold_scaleRegionPercent(-1),
      minComponentWidth(-1),
      maxDetectionsPerType(-1),
      combHalfWidth(-1),
      combResponseThreshold(-1),
      everyNLines(-1) {}

BinaryTracker::EdgeDetectionParameters::EdgeDetectionParameters(
    EdgeDetectionType type, s32 threshold_yIncrement, s32 threshold_xIncrement,
    f32 threshold_blackPercentile, f32 threshold_whitePercentile,
    f32 threshold_scaleRegionPercent, s32 minComponentWidth,
    s32 maxDetectionsPerType, s32 combHalfWidth, s32 combResponseThreshold,
    s32 everyNLines)
    : type(type),
      threshold_yIncrement(threshold_yIncrement),
      threshold_xIncrement(threshold_xIncrement),
      threshold_blackPercentile(threshold_blackPercentile),
      threshold_whitePercentile(threshold_whitePercentile),
      threshold_scaleRegionPercent(threshold_scaleRegionPercent),
      minComponentWidth(minComponentWidth),
      maxDetectionsPerType(maxDetectionsPerType),
      combHalfWidth(combHalfWidth),
      combResponseThreshold(combResponseThreshold),
      everyNLines(everyNLines) {}

BinaryTracker::BinaryTracker() : isValid(false) {}

BinaryTracker::BinaryTracker(
    const Array<u8> &templateImage, const Quadrilateral<f32> &templateQuad,
    const f32
        scaleTemplateRegionPercent,  //< Shrinks the region if less-than 1.0,
                                     //expands the region if greater-than 1.0
    const EdgeDetectionParameters &edgeDetectionParams, MemoryStack &fastMemory,
  ...
  AnkiConditionalErrorAndReturn(
      ...
      "BinaryTracker::BinaryTracker",
      "template widths and heights must be greater than zero");
  ...
  Point<f32> centerOffset((templateImageWidth - 1) / 2.0f,
                          (templateImageHeight - 1) / 2.0f);
  this->transformation = Transformations::PlanarTransformation_f32(
      Transformations::TRANSFORM_PROJECTIVE, templateQuad, centerOffset,
      slowMemory);

  this->templateQuad = templateQuad;

  this->templateEdges.xDecreasing = FixedLengthList<Point<s16> >(
      edgeDetectionParams.maxDetectionsPerType, fastMemory);
  this->templateEdges.xIncreasing = FixedLengthList<Point<s16> >(
      edgeDetectionParams.maxDetectionsPerType, fastMemory);
  this->templateEdges.yDecreasing = FixedLengthList<Point<s16> >(
      edgeDetectionParams.maxDetectionsPerType, fastMemory);
  this->templateEdges.yIncreasing = FixedLengthList<Point<s16> >(
      edgeDetectionParams.maxDetectionsPerType, fastMemory);

  this->templateImage =
      Array<u8>(templateImageHeight, templateImageWidth, slowMemory);
  this->templateImage.Set(templateImage);

  this->originalTemplateOrientation = 0.0f;

  AnkiConditionalErrorAndReturn(
      AreValid(this->templateEdges.xDecreasing, this->templateEdges.xIncreasing,
               this->templateEdges.yDecreasing,
               this->templateEdges.yIncreasing),
      "BinaryTracker::BinaryTracker", "Could not allocate local memory");

  const Rectangle<s32> edgeDetection_imageRegionOfInterest =
      templateQuad.ComputeBoundingRectangle<s32>().ComputeScaledRectangle<s32>(
          edgeDetectionParams.threshold_scaleRegionPercent);

  this->templateIntegerCounts =
      IntegerCounts(this->templateImage, edgeDetection_imageRegionOfInterest,
                    edgeDetectionParams.threshold_yIncrement,
                    edgeDetectionParams.threshold_xIncrement, fastMemory);

  AnkiConditionalErrorAndReturn(this->templateIntegerCounts.IsValid(),
                                "BinaryTracker::BinaryTracker",
                                "Could not allocate local memory");

  this->lastGrayvalueThreshold =
      ComputeGrayvalueThreshold(this->templateIntegerCounts,
  ...
  const Rectangle<s32> templateRect =
      templateQuad.ComputeBoundingRectangle<s32>().ComputeScaledRectangle<s32>(
          scaleTemplateRegionPercent);

  Result result = RESULT_FAIL;
  if (edgeDetectionParams.type ==
      TemplateTracker::BinaryTracker::EDGE_TYPE_GRAYVALUE) {
    result = DetectBlurredEdges_GrayvalueThreshold(
        templateImage, templateRect, this->lastGrayvalueThreshold,
        edgeDetectionParams.minComponentWidth, edgeDetectionParams.everyNLines,
        this->templateEdges);
  } else if (edgeDetectionParams.type ==
             TemplateTracker::BinaryTracker::EDGE_TYPE_DERIVATIVE) {
    result = DetectBlurredEdges_DerivativeThreshold(
        templateImage, templateRect, edgeDetectionParams.combHalfWidth,
        edgeDetectionParams.combResponseThreshold,
        edgeDetectionParams.everyNLines, this->templateEdges);
  }
  ...
  AnkiConditionalErrorAndReturn(result == RESULT_OK,
                                "BinaryTracker::BinaryTracker",
                                "DetectBlurredEdge failed");

  this->isValid = true;
}  // BinaryTracker::BinaryTracker()

BinaryTracker::BinaryTracker(const Anki::Vision::MarkerType markerType,
                             ...
                             MemoryStack &fastMemory, MemoryStack &slowMemory)
    : isValid(false) {
  ...
  //  "BinaryTracker::BinaryTracker", "markerType %d is not supported for header
  //  initialization", markerType);
  AnkiError("BinaryTracker::BinaryTracker",
            "BinaryTracker needs to be updated now that Battery marker has "
            "been removed.");
  ...
  AnkiConditionalErrorAndReturn(
      AreValid(this->templateEdges.xDecreasing, this->templateEdges.xIncreasing,
               this->templateEdges.yDecreasing, this->templateEdges.yIncreasing,
               this->templateImage),
      "BinaryTracker::BinaryTracker", "Could not allocate local memory");
  ...
  // Place the header image in the templateImage
  {
    ...
    // TODO: BinaryTracker needs to be updated now that Battery marker has been
    // removed
    if (false) {  // markerType == Anki::Vision::MARKER_BATTERIES) {
      binaryTemplate = DecodeRunLengthBinary<u8>(
          &battery[0], battery_WIDTH, battery_ORIGINAL_HEIGHT,
          battery_ORIGINAL_WIDTH, Flags::Buffer(false, false, false),
          slowMemory);
    }
    ...
    const s32 binaryTemplateWithBorderHeight =
        binaryTemplateHeight + 2 * borderWidth;
    const s32 binaryTemplateWithBorderWidth =
        binaryTemplateWidth + 2 * borderWidth;
    Array<u8> binaryTemplateWithBorder(
        binaryTemplateWithBorderHeight, binaryTemplateWithBorderWidth,
        slowMemory, Flags::Buffer(false, false, false));
    ...
    for (s32 y = 0; y < binaryTemplateHeight; y++) {
      ...
      u8 *restrict pBinaryTemplateWithBorder =
          binaryTemplateWithBorder.Pointer(y + borderWidth, borderWidth);
    ...
    const f32 binaryTemplateHeightF32 =
        static_cast<f32>(binaryTemplate.get_size(0));
    const f32 binaryTemplateWidthF32 =
        static_cast<f32>(binaryTemplate.get_size(1));
    ...
    binaryCorners[1] = Point<f32>(0.5f + borderWidth,
                                  binaryTemplateHeightF32 - 0.5f + borderWidth);
    binaryCorners[2] = Point<f32>(binaryTemplateWidthF32 - 0.5f + borderWidth,
                                  0.5f + borderWidth);
    binaryCorners[3] = Point<f32>(binaryTemplateWidthF32 - 0.5f + borderWidth,
                                  binaryTemplateHeightF32 - 0.5f + borderWidth);
    ...
    Matrix::EstimateHomography(binaryCorners, templateCorners,
                               binaryTemplateHomography, numericalFailure,
                               slowMemory);

    Transformations::PlanarTransformation_f32 binaryTemplateTransform(
        Transformations::TRANSFORM_PROJECTIVE,
        Quadrilateral<f32>(binaryCorners[0], binaryCorners[1], binaryCorners[2],
                           binaryCorners[3]),
        binaryTemplateHomography, Point<f32>(0.0f, 0.0f), slowMemory);

    this->originalTemplateOrientation =
        binaryTemplateTransform.get_transformedOrientation(slowMemory);

    Array<u8> rotatedBinaryTemplateWithBorder(
        binaryTemplateWithBorder.get_size(0),
        binaryTemplateWithBorder.get_size(1), slowMemory);

    s32 rotationOrder[4];
    if (this->originalTemplateOrientation >= (-M_PI_F / 4) &&
        this->originalTemplateOrientation < (M_PI_F / 4)) {  // No rotation
      ...
      rotationOrder[2] = 2;
      rotationOrder[3] = 3;
    } else if (this->originalTemplateOrientation >= (M_PI_F / 4) &&
               this->originalTemplateOrientation <
                   (3 * M_PI_F / 4)) {  // Rotate 90 degrees clockwise
      Matrix::Rotate90<u8, u8>(binaryTemplateWithBorder,
                               rotatedBinaryTemplateWithBorder);

      rotationOrder[0] = 2;
      rotationOrder[1] = 0;
      rotationOrder[2] = 3;
      rotationOrder[3] = 1;
    } else if (this->originalTemplateOrientation >= (3 * M_PI_F / 4) &&
               this->originalTemplateOrientation <
                   (5 * M_PI_F / 4)) {  // Rotate 180 degrees clockwise
      Matrix::Rotate180<u8, u8>(binaryTemplateWithBorder,
                                rotatedBinaryTemplateWithBorder);
      ...
      Matrix::Rotate270<u8, u8>(binaryTemplateWithBorder,
                                rotatedBinaryTemplateWithBorder);
    ...
    Matrix::EstimateHomography(rotatedBinaryCorners, templateCorners,
                               binaryTemplateHomography, numericalFailure,
                               slowMemory);

    binaryTemplateTransform.set_homography(binaryTemplateHomography);

    // Extract the edges, and transform them to the actual fiducial detection
    // location
    // TODO: add subpixel
    const Rectangle<s32> templateRect(0, binaryTemplateWithBorderWidth - 1, 0,
                                      binaryTemplateWithBorderHeight - 1);

    Result result = RESULT_FAIL;
    if (edgeDetectionParams.type ==
        TemplateTracker::BinaryTracker::EDGE_TYPE_GRAYVALUE) {
      result = DetectBlurredEdges_GrayvalueThreshold(
          rotatedBinaryTemplateWithBorder, templateRect, 128,
          edgeDetectionParams.minComponentWidth,
          edgeDetectionParams.everyNLines, this->templateEdges);
    } else if (edgeDetectionParams.type ==
               TemplateTracker::BinaryTracker::EDGE_TYPE_DERIVATIVE) {
      result = DetectBlurredEdges_DerivativeThreshold(
          rotatedBinaryTemplateWithBorder, templateRect,
          edgeDetectionParams.combHalfWidth,
          edgeDetectionParams.combResponseThreshold,
          edgeDetectionParams.everyNLines, this->templateEdges);
    }
    ...
    binaryTemplateTransform.Transform(this->templateEdges.xDecreasing,
                                      this->templateEdges.xDecreasing,
                                      slowMemory);
    binaryTemplateTransform.Transform(this->templateEdges.xIncreasing,
                                      this->templateEdges.xIncreasing,
                                      slowMemory);
    binaryTemplateTransform.Transform(this->templateEdges.yDecreasing,
                                      this->templateEdges.yDecreasing,
                                      slowMemory);
    binaryTemplateTransform.Transform(this->templateEdges.yIncreasing,
                                      this->templateEdges.yIncreasing,
                                      slowMemory);
    ...
    if (!useRealTemplateImage) {
      ...
      bool numericalFailure;
      Matrix::EstimateHomography(templateCorners, binaryCorners,
                                 binaryTemplateHomography, numericalFailure,
                                 slowMemory);
      const Point<f32> centerOffset(0.0f, 0.0f);
      Meshgrid<f32> originalCoordinates(
          LinearSequence<f32>(0.5f, 1.0f, templateImageWidth - 0.5f),
          LinearSequence<f32>(0.5f, 1.0f, templateImageHeight - 0.5f));

      Interp2_Projective<u8, u8>(binaryTemplateWithBorder, originalCoordinates,
                                 binaryTemplateHomography, centerOffset,
                                 this->templateImage, INTERPOLATE_LINEAR, 255);
    }
...
Result BinaryTracker::ShowTemplate(const char *windowName,
                                   const bool waitForKeypress,
                                   const bool fitImageToWindow,
                                   const f32 displayScale) const {
  ...
  if (toShow.cols == 0) return RESULT_FAIL;
...
bool BinaryTracker::IsValid() const {
  ...
  if (!AreValid(templateEdges.xDecreasing, templateEdges.xIncreasing,
                templateEdges.yDecreasing, templateEdges.yIncreasing))
    return false;

  return true;
}  // bool BinaryTracker::IsValid()

Result BinaryTracker::UpdateTransformation(
...
Result BinaryTracker::Serialize(const char *objectName,
  ...
  if (SerializedBuffer::SerializeDescriptionStrings(
          "BinaryTracker", objectName, &segment, totalDataLength) != RESULT_OK)
    return RESULT_FAIL;

  if (SerializedBuffer::SerializeRawBasicType<bool>(
          "templateEdges.isValid", this->isValid, &segment, totalDataLength) !=
      RESULT_OK)
    return RESULT_FAIL;

  // First, serialize the transformation
  if (this->transformation.SerializeRaw("transformation", &segment,
                                        totalDataLength) != RESULT_OK)
    return RESULT_FAIL;

  // Next, serialize the template lists
  if (SerializedBuffer::SerializeRawBasicType<s32>(
          "templateEdges.imageHeight", this->templateEdges.imageHeight,
          &segment, totalDataLength) != RESULT_OK)
    return RESULT_FAIL;

  if (SerializedBuffer::SerializeRawBasicType<s32>(
          "templateEdges.imageWidth", this->templateEdges.imageWidth, &segment,
          totalDataLength) != RESULT_OK)
    return RESULT_FAIL;

  if (SerializedBuffer::SerializeRawFixedLengthList<Point<s16> >(
          "templateEdges.xDecreasing", this->templateEdges.xDecreasing,
          &segment, totalDataLength) != RESULT_OK)
    return RESULT_FAIL;

  if (SerializedBuffer::SerializeRawFixedLengthList<Point<s16> >(
          "templateEdges.xIncreasing", this->templateEdges.xIncreasing,
          &segment, totalDataLength) != RESULT_OK)
    return RESULT_FAIL;

  if (SerializedBuffer::SerializeRawFixedLengthList<Point<s16> >(
          "templateEdges.yDecreasing", this->templateEdges.yDecreasing,
          &segment, totalDataLength) != RESULT_OK)
    return RESULT_FAIL;

  if (SerializedBuffer::SerializeRawFixedLengthList<Point<s16> >(
          "templateEdges.yIncreasing", this->templateEdges.yIncreasing,
          &segment, totalDataLength) != RESULT_OK)
    return RESULT_FAIL;
...
Result BinaryTracker::Deserialize(char *objectName, void **buffer,
  ...
  // this->transformation =
  // Transformations::PlanarTransformation_f32(Transformations::TRANSFORM_PROJECTIVE,
  // memory);
  this->transformation.Deserialize(objectName, buffer, bufferLength, memory);

  // Next, deserialize the template lists
  this->templateEdges.imageHeight =
      SerializedBuffer::DeserializeRawBasicType<s32>(NULL, buffer,
                                                     bufferLength);
  this->templateEdges.imageWidth =
      SerializedBuffer::DeserializeRawBasicType<s32>(NULL, buffer,
                                                     bufferLength);
  this->templateEdges.xDecreasing =
      SerializedBuffer::DeserializeRawFixedLengthList<Point<s16> >(
...
Result BinaryTracker::set_transformation(
    const Transformations::PlanarTransformation_f32 &transformation) {
  return this->transformation.Set(transformation);
}

Transformations::PlanarTransformation_f32 BinaryTracker::get_transformation()
    const {
  return transformation;
}

Result BinaryTracker::ComputeAllIndexLimits(const EdgeLists &imageEdges,
                                            AllIndexLimits &allLimits,
                                            MemoryStack &memory) {
  allLimits.xDecreasing_yStartIndexes =
      Array<s32>(1, imageEdges.imageHeight + 1, memory,
                 Flags::Buffer(false, false, false));
  allLimits.xIncreasing_yStartIndexes =
      Array<s32>(1, imageEdges.imageHeight + 1, memory,
                 Flags::Buffer(false, false, false));
  allLimits.yDecreasing_xStartIndexes = Array<s32>(
      1, imageEdges.imageWidth + 1, memory, Flags::Buffer(false, false, false));
  allLimits.yIncreasing_xStartIndexes = Array<s32>(
      1, imageEdges.imageWidth + 1, memory, Flags::Buffer(false, false, false));

  AnkiConditionalErrorAndReturnValue(
      AreValid(allLimits.xDecreasing_yStartIndexes,
               allLimits.xIncreasing_yStartIndexes,
               allLimits.yDecreasing_xStartIndexes,
               allLimits.yIncreasing_xStartIndexes),
      RESULT_FAIL_OUT_OF_MEMORY, "BinaryTracker::ComputeAllIndexLimits",
      "Could not allocate local memory");

  ComputeIndexLimitsVertical(imageEdges.xDecreasing,
                             allLimits.xDecreasing_yStartIndexes);

  ComputeIndexLimitsVertical(imageEdges.xIncreasing,
                             allLimits.xIncreasing_yStartIndexes);

  ComputeIndexLimitsHorizontal(imageEdges.yDecreasing,
                               allLimits.yDecreasing_xStartIndexes);

  ComputeIndexLimitsHorizontal(imageEdges.yIncreasing,
                               allLimits.yIncreasing_xStartIndexes);
...
Result BinaryTracker::UpdateTrack_Normal(
    ...
    s32 &verify_numSimilarPixels,  //< For all pixels in the template, how many
                                   ...
                                   //get_numTemplatePixels() or numInBounds for
                                   //a percentage.
    MemoryStack fastScratch, MemoryStack slowScratch) {
  return UpdateTrack_Generic(
      UPDATE_VERSION_NORMAL, nextImage, edgeDetectionParams,
      matching_maxTranslationDistance, matching_maxProjectiveDistance,
      verify_maxTranslationDistance, verify_maxPixelDifference,
      verify_coordinateIncrement, 0, 0, 0, numMatches,
      verify_meanAbsoluteDifference, verify_numInBounds,
      verify_numSimilarPixels, fastScratch, slowScratch);
}

// WARNING: using a list is liable to be slower than normal, and not be more
// accurate
Result BinaryTracker::UpdateTrack_List(
    ...
    MemoryStack fastScratch, MemoryStack slowScratch) {
  return UpdateTrack_Generic(
      UPDATE_VERSION_LIST, nextImage, edgeDetectionParams,
      matching_maxTranslationDistance, matching_maxProjectiveDistance,
      verify_maxTranslationDistance, verify_maxPixelDifference,
      verify_coordinateIncrement, 0, 0, 0, numMatches,
      verify_meanAbsoluteDifference, verify_numInBounds,
...
Result BinaryTracker::UpdateTrack_Ransac(
    ...
    const s32 matching_maxProjectiveDistance,
    const s32 verify_maxTranslationDistance, const u8 verify_maxPixelDifference,
    const s32 verify_coordinateIncrement, const s32 ransac_maxIterations,
    const s32 ransac_numSamplesPerType,  //< for four types
    const s32 ransac_inlinerDistance, s32 &numMatches,
    s32 &verify_meanAbsoluteDifference,  //< For all pixels in the template,
                                         //compute the mean difference between
                                         //the template and the final warped
                                         //template
    ...
    MemoryStack fastScratch, MemoryStack slowScratch) {
  return UpdateTrack_Generic(
      UPDATE_VERSION_RANSAC, nextImage, edgeDetectionParams, 0,
      matching_maxProjectiveDistance, verify_maxTranslationDistance,
      verify_maxPixelDifference, verify_coordinateIncrement,
      ransac_maxIterations, ransac_numSamplesPerType, ransac_inlinerDistance,
      numMatches, verify_meanAbsoluteDifference, verify_numInBounds,
      verify_numSimilarPixels, fastScratch, slowScratch);
}

Result BinaryTracker::UpdateTrack_Generic(
    const UpdateVersion version, const Array<u8> &nextImage,
    const EdgeDetectionParameters &edgeDetectionParams,
    const s32 matching_maxTranslationDistance,
    const s32 matching_maxProjectiveDistance,
    const s32 verify_maxTranslationDistance, const u8 verify_maxPixelDifference,
  ...
  nextImageEdges.xDecreasing = FixedLengthList<Point<s16> >(
      edgeDetectionParams.maxDetectionsPerType, fastScratch);
  nextImageEdges.xIncreasing = FixedLengthList<Point<s16> >(
      edgeDetectionParams.maxDetectionsPerType, fastScratch);
  nextImageEdges.yDecreasing = FixedLengthList<Point<s16> >(
      edgeDetectionParams.maxDetectionsPerType, fastScratch);
  nextImageEdges.yIncreasing = FixedLengthList<Point<s16> >(
      edgeDetectionParams.maxDetectionsPerType, fastScratch);

  AnkiConditionalErrorAndReturnValue(
      AreValid(nextImageEdges.xDecreasing, nextImageEdges.xIncreasing,
               nextImageEdges.yDecreasing, nextImageEdges.yIncreasing),
      RESULT_FAIL_OUT_OF_MEMORY, "BinaryTracker::UpdateTrack",
      "Could not allocate local scratch");

  BeginBenchmark("ut_DetectEdges");

  if (edgeDetectionParams.type ==
      TemplateTracker::BinaryTracker::EDGE_TYPE_GRAYVALUE) {
    lastResult = DetectBlurredEdges_GrayvalueThreshold(
        nextImage, this->lastGrayvalueThreshold,
        edgeDetectionParams.minComponentWidth, edgeDetectionParams.everyNLines,
        nextImageEdges);
  } else if (edgeDetectionParams.type ==
             TemplateTracker::BinaryTracker::EDGE_TYPE_DERIVATIVE) {
    lastResult = DetectBlurredEdges_DerivativeThreshold(
        nextImage, edgeDetectionParams.combHalfWidth,
        edgeDetectionParams.combResponseThreshold,
        edgeDetectionParams.everyNLines, nextImageEdges);
  }
  ...
  AnkiConditionalErrorAndReturnValue(lastResult == RESULT_OK, lastResult,
                                     "BinaryTracker::UpdateTrack",
                                     "DetectBlurredEdge failed");

  // First, to speed up the correspondence search, find the min and max of x or
  // y points
  AllIndexLimits allLimits;

  BeginBenchmark("ut_IndexLimits");

  if ((lastResult = BinaryTracker::ComputeAllIndexLimits(
           nextImageEdges, allLimits, fastScratch)) != RESULT_OK)
    return lastResult;
  ...
  // 2. It does a verify step automatically, but the others need to do one after
  // running
  if (version == UPDATE_VERSION_RANSAC) {
    BeginBenchmark("ut_projective_ransac");
    lastResult = IterativelyRefineTrack_Projective_Ransac(
        nextImageEdges, allLimits, matching_maxProjectiveDistance,
        maxMatchesPerType, ransac_maxIterations, ransac_numSamplesPerType,
        ransac_inlinerDistance, numMatches, fastScratch, slowScratch);
    EndBenchmark("ut_projective_ransac");

    AnkiConditionalErrorAndReturnValue(
        lastResult == RESULT_OK, lastResult, "BinaryTracker::UpdateTrack",
        "IterativelyRefineTrack_Projective failed");
  } else {  // if(version == UPDATE_VERSION_RANSAC) {
    BeginBenchmark("ut_translation");

    lastResult = IterativelyRefineTrack_Translation(
        nextImageEdges, allLimits, matching_maxTranslationDistance,
        fastScratch);
    ...
    AnkiConditionalErrorAndReturnValue(
        lastResult == RESULT_OK, lastResult, "BinaryTracker::UpdateTrack",
        "IterativelyRefineTrack_Translation failed");

    if (version == UPDATE_VERSION_NORMAL) {
      BeginBenchmark("ut_projective_normal");
      lastResult = IterativelyRefineTrack_Projective(
          nextImageEdges, allLimits, matching_maxProjectiveDistance,
          fastScratch);
      EndBenchmark("ut_projective_normal");
    } else if (version == UPDATE_VERSION_LIST) {
      BeginBenchmark("ut_projective_list");
      lastResult = IterativelyRefineTrack_Projective_List(
          nextImageEdges, allLimits, matching_maxProjectiveDistance,
          maxMatchesPerType, fastScratch, slowScratch);
    ...
    lastResult = VerifyTrack(nextImageEdges, allLimits,
                             verify_maxTranslationDistance, numMatches);
  ...
  const Quadrilateral<f32> curWarpedCorners =
      this->get_transformation().get_transformedCorners(fastScratch);

  const Rectangle<s32> edgeDetection_imageRegionOfInterest =
      curWarpedCorners.ComputeBoundingRectangle<s32>()
          .ComputeScaledRectangle<s32>(
              edgeDetectionParams.threshold_scaleRegionPercent);

  IntegerCounts lastImageIntegerCounts(
      nextImage, edgeDetection_imageRegionOfInterest,
      edgeDetectionParams.threshold_yIncrement,
      edgeDetectionParams.threshold_xIncrement, fastScratch);

  this->lastGrayvalueThreshold = ComputeGrayvalueThreshold(
      lastImageIntegerCounts, edgeDetectionParams.threshold_blackPercentile,
      edgeDetectionParams.threshold_whitePercentile);
...

  {
    ...
    // lastResult =
    // this->transformation.VerifyTransformation_Projective_LinearInterpolate(
    lastResult =
        this->transformation.VerifyTransformation_Projective_NearestNeighbor(
            templateImage, this->templateIntegerCounts,
            this->templateQuad.ComputeBoundingRectangle<f32>(), nextImage,
            lastImageIntegerCounts, templateRegionHeight, templateRegionWidth,
            verify_coordinateIncrement, verify_maxPixelDifference,
            verify_meanAbsoluteDifference, verify_numInBounds,
            verify_numSimilarPixels, fastScratch);

    // Array<u8> downsampledTemplate(templateImageHeight/8,
    // templateImageWidth/8, fastScratch); Array<u8>
    // downsampledNext(templateImageHeight/8, templateImageWidth/8, fastScratch);

    // Rectangle<f32> boundingRect =
    // this->templateQuad.ComputeBoundingRectangle(); Rectangle<f32>
    // downsampledRect(boundingRect.left/8, boundingRect.right/8,
    // boundingRect.top/8, boundingRect.bottom/8);

    // ImageProcessing::DownsampleByPowerOfTwo<u8,u32,u8>(templateImage, 3,
    // downsampledTemplate, fastScratch);
    // ImageProcessing::DownsampleByPowerOfTwo<u8,u32,u8>(nextImage, 3,
    // downsampledNext, fastScratch);
    ...
    //  verify_numInBounds, verify_numSimilarPixels, fastScratch);
  }
...
Result BinaryTracker::ComputeIndexLimitsVertical(
    const FixedLengthList<Point<s16> > &points, Array<s32> &yStartIndexes) {
...
Result BinaryTracker::ComputeIndexLimitsHorizontal(
    const FixedLengthList<Point<s16> > &points, Array<s32> &xStartIndexes) {
  const Point<s16> *restrict pPoints = points.Pointer(0);
...
Result BinaryTracker::FindVerticalCorrespondences_Translation(
    ...
    const FixedLengthList<Point<s16> > &newPoints, const s32 imageHeight,
    const s32 imageWidth, const Array<s32> &xStartIndexes, s32 &sumY,
  ...
  // TODO: if the homography is just translation, we can do this faster (just
  // slightly, as most of the cost is the search)
  ...
  for (s32 iPoint = 0; iPoint < numTemplatePoints; iPoint++) {
    ...
    const s32 warpedXrounded =
        RoundS32_minusPointFive(warpedX + centerOffset.x);
    const s32 warpedYrounded =
        RoundS32_minusPointFive(warpedY + centerOffset.y);
...
Result BinaryTracker::FindHorizontalCorrespondences_Translation(
    ...
    const FixedLengthList<Point<s16> > &templatePoints,
    const FixedLengthList<Point<s16> > &newPoints, const s32 imageHeight,
    const s32 imageWidth, const Array<s32> &yStartIndexes, s32 &sumX,
    s32 &numCorrespondences) {
  const s32 numTemplatePoints = templatePoints.get_size();
  // const s32 numNewPoints = newPoints.get_size();
...
NO_INLINE Result BinaryTracker::FindVerticalCorrespondences_Projective(
    ...
    const FixedLengthList<Point<s16> > &templatePoints,
    const FixedLengthList<Point<s16> > &newPoints, const s32 imageHeight,
    const s32 imageWidth, const Array<s32> &xStartIndexes, Array<f32> &AtA,
    Array<f32> &Atb_t) {
  const s32 numTemplatePoints = templatePoints.get_size();
  // const s32 numNewPoints = newPoints.get_size();
  ...
  f32 Atb_t_raw3 = 0, Atb_t_raw4 = 0, Atb_t_raw5 = 0, Atb_t_raw6 = 0,
      Atb_t_raw7 = 0;
  ...
  Atb_t[0][3] = Atb_t_raw3;
  Atb_t[0][4] = Atb_t_raw4;
  Atb_t[0][5] = Atb_t_raw5;
  Atb_t[0][6] = Atb_t_raw6;
  Atb_t[0][7] = Atb_t_raw7;
#endif  // #if !defined(USE_ARM_ACCELERATION) ... #else
...
NO_INLINE Result BinaryTracker::FindHorizontalCorrespondences_Projective(
    ...
    const FixedLengthList<Point<s16> > &templatePoints,
    const FixedLengthList<Point<s16> > &newPoints, const s32 imageHeight,
    const s32 imageWidth, const Array<s32> &yStartIndexes, Array<f32> &AtA,
    Array<f32> &Atb_t) {
  const s32 numTemplatePoints = templatePoints.get_size();
  // const s32 numNewPoints = newPoints.get_size();
  ...
  f32 Atb_t_raw0 = 0, Atb_t_raw1 = 0, Atb_t_raw2 = 0, Atb_t_raw6 = 0,
      Atb_t_raw7 = 0;
#endif  // #if !defined(USE_ARM_ACCELERATION) ... #else
  ...
  Atb_t[0][0] = Atb_t_raw0;
  Atb_t[0][1] = Atb_t_raw1;
  Atb_t[0][2] = Atb_t_raw2;
  Atb_t[0][6] = Atb_t_raw6;
  Atb_t[0][7] = Atb_t_raw7;
#endif  // #if !defined(USE_ARM_ACCELERATION) ... #else
...
NO_INLINE Result BinaryTracker::FindVerticalCorrespondences_Verify(
    ...
    const Array<s32>
        &xStartIndexes,  //< Computed by ComputeIndexLimitsHorizontal
...
NO_INLINE Result BinaryTracker::ApplyVerticalCorrespondenceList_Projective(
    ...
    Array<f32> &AtA, Array<f32> &Atb_t) {
  const s32 numMatchingIndexes = matchingIndexes.get_size();
  const IndexCorrespondence *restrict pMatchingIndexes =
      matchingIndexes.Pointer(0);
...
Result BinaryTracker::IterativelyRefineTrack_Translation(
    const EdgeLists &nextImageEdges, const AllIndexLimits &allLimits,
    const s32 matching_maxDistance, MemoryStack scratch) {
  Result lastResult;
  ...
  lastResult = BinaryTracker::FindHorizontalCorrespondences_Translation(
      ...
      allLimits.xDecreasing_yStartIndexes, sumX_xDecreasing, numX_xDecreasing);

  AnkiConditionalErrorAndReturnValue(
      lastResult == RESULT_OK, lastResult,
      "BinaryTracker::IterativelyRefineTrack_Translation",
      "FindHorizontalCorrespondences 1 failed");

  lastResult = BinaryTracker::FindHorizontalCorrespondences_Translation(
      matching_maxDistance, this->transformation,
      this->templateEdges.xIncreasing, nextImageEdges.xIncreasing,
      nextImageEdges.imageHeight, nextImageEdges.imageWidth,
      allLimits.xIncreasing_yStartIndexes, sumX_xIncreasing, numX_xIncreasing);

  AnkiConditionalErrorAndReturnValue(
      lastResult == RESULT_OK, lastResult,
      "BinaryTracker::IterativelyRefineTrack_Translation",
      "FindHorizontalCorrespondences 2 failed");
  ...
  lastResult = BinaryTracker::FindVerticalCorrespondences_Translation(
      ...
      nextImageEdges.imageHeight, nextImageEdges.imageWidth,
      allLimits.yDecreasing_xStartIndexes, sumY_yDecreasing, numY_yDecreasing);

  AnkiConditionalErrorAndReturnValue(
      lastResult == RESULT_OK, lastResult,
      "BinaryTracker::IterativelyRefineTrack_Translation",
      "FindVerticalCorrespondences 1 failed");

  lastResult = BinaryTracker::FindVerticalCorrespondences_Translation(
      matching_maxDistance, this->transformation,
      this->templateEdges.yIncreasing, nextImageEdges.yIncreasing,
      nextImageEdges.imageHeight, nextImageEdges.imageWidth,
      allLimits.yIncreasing_xStartIndexes, sumY_yIncreasing, numY_yIncreasing);

  AnkiConditionalErrorAndReturnValue(
      lastResult == RESULT_OK, lastResult,
      "BinaryTracker::IterativelyRefineTrack_Translation",
      "FindVerticalCorrespondences 2 failed");
  ...
  // Update the transformation
  {
    ...
    if (numX < 1 || numY < 1) return RESULT_OK;
    ...
    this->transformation.Update(update, 1.0f, scratch,
...
Result BinaryTracker::IterativelyRefineTrack_Projective(
  ...
  AnkiConditionalErrorAndReturnValue(
      lastResult == RESULT_OK, lastResult,
      "BinaryTracker::IterativelyRefineTrack_Projective",
      "FindHorizontalCorrespondences 1 failed");
  ...
  // Update the transformation
  {
    ...
    // The total AtA and Atb matrices are just the elementwise sums of their
    // partial versions
    for (s32 y = 0; y < 8; y++) {
      for (s32 x = 0; x < 8; x++) {
        AtA[y][x] = AtA_xDecreasing[y][x] + AtA_xIncreasing[y][x] +
                    AtA_yDecreasing[y][x] + AtA_yIncreasing[y][x];
      }

      Atb_t[0][y] = Atb_t_xDecreasing[0][y] + Atb_t_xIncreasing[0][y] +
                    Atb_t_yDecreasing[0][y] + Atb_t_yIncreasing[0][y];
    }
    ...
    AnkiConditionalErrorAndReturnValue(
        lastResult == RESULT_OK, lastResult,
        "BinaryTracker::IterativelyRefineTrack_Projective",
        "SolveLeastSquaresWithCholesky failed");

    if (numericalFailure) {
      AnkiWarn("BinaryTracker::IterativelyRefineTrack_Projective",
               "numericalFailure");
      return RESULT_OK;
    }
    ...
    newHomography[0][0] = pAtb_t[0];
    newHomography[0][1] = pAtb_t[1];
    newHomography[0][2] = pAtb_t[2];
    newHomography[1][0] = pAtb_t[3];
    newHomography[1][1] = pAtb_t[4];
    newHomography[1][2] = pAtb_t[5];
    newHomography[2][0] = pAtb_t[6];
    newHomography[2][1] = pAtb_t[7];
    newHomography[2][2] = 1.0f;
...
Result BinaryTracker::IterativelyRefineTrack_Projective_List(
    const EdgeLists &nextImageEdges, const AllIndexLimits &allLimits,
    const s32 matching_maxDistance, const s32 maxMatchesPerType,
    MemoryStack fastScratch, MemoryStack slowScratch) {
  Result lastResult;
  ...
  FixedLengthList<IndexCorrespondence> matchingIndexes_xDecreasing(
      maxMatchesPerType, slowScratch);
  FixedLengthList<IndexCorrespondence> matchingIndexes_xIncreasing(
      maxMatchesPerType, slowScratch);
  FixedLengthList<IndexCorrespondence> matchingIndexes_yDecreasing(
      maxMatchesPerType, slowScratch);
  FixedLengthList<IndexCorrespondence> matchingIndexes_yIncreasing(
      maxMatchesPerType, slowScratch);
  ...
  AnkiConditionalErrorAndReturnValue(
      lastResult == RESULT_OK, lastResult,
      "BinaryTracker::IterativelyRefineTrack_List_Projective",
      "FindHorizontalCorrespondences_List 1 failed");

  lastResult = BinaryTracker::ApplyHorizontalCorrespondenceList_Projective(
      matchingIndexes_xDecreasing, AtA_xDecreasing, Atb_t_xDecreasing);

  AnkiConditionalErrorAndReturnValue(
      lastResult == RESULT_OK, lastResult,
      "BinaryTracker::IterativelyRefineTrack_List_Projective",
      "ApplyHorizontalCorrespondenceList_Projective 1 failed");
  ...
  AnkiConditionalErrorAndReturnValue(
      lastResult == RESULT_OK, lastResult,
      "BinaryTracker::IterativelyRefineTrack_List_Projective",
      "FindHorizontalCorrespondences_List 2 failed");

  lastResult = BinaryTracker::ApplyHorizontalCorrespondenceList_Projective(
      matchingIndexes_xIncreasing, AtA_xIncreasing, Atb_t_xIncreasing);

  AnkiConditionalErrorAndReturnValue(
      lastResult == RESULT_OK, lastResult,
      "BinaryTracker::IterativelyRefineTrack_List_Projective",
      "ApplyHorizontalCorrespondenceList_Projective 2 failed");
  ...
  AnkiConditionalErrorAndReturnValue(
      lastResult == RESULT_OK, lastResult,
      "BinaryTracker::IterativelyRefineTrack_List_Projective",
      "FindVerticalCorrespondences_List 1 failed");

  lastResult = BinaryTracker::ApplyVerticalCorrespondenceList_Projective(
      matchingIndexes_yDecreasing, AtA_yDecreasing, Atb_t_yDecreasing);

  AnkiConditionalErrorAndReturnValue(
      lastResult == RESULT_OK, lastResult,
      "BinaryTracker::IterativelyRefineTrack_List_Projective",
      "ApplyVerticalCorrespondenceList_Projective 1 failed");
  ...
  AnkiConditionalErrorAndReturnValue(
      lastResult == RESULT_OK, lastResult,
      "BinaryTracker::IterativelyRefineTrack_List_Projective",
      "FindVerticalCorrespondences_List 2 failed");

  lastResult = BinaryTracker::ApplyVerticalCorrespondenceList_Projective(
      matchingIndexes_yIncreasing, AtA_yIncreasing, Atb_t_yIncreasing);

  AnkiConditionalErrorAndReturnValue(
      lastResult == RESULT_OK, lastResult,
      "BinaryTracker::IterativelyRefineTrack_List_Projective",
      "ApplyVerticalCorrespondenceList_Projective 2 failed");
  ...
  // Update the transformation
  {
    ...
    if (numericalFailure) {
      AnkiWarn("BinaryTracker::IterativelyRefineTrack_List_Projective",
               "numericalFailure");
      return RESULT_OK;
    }
...
Result BinaryTracker::IterativelyRefineTrack_Projective_Ransac(
    const EdgeLists &nextImageEdges, const AllIndexLimits &allLimits,
    const s32 matching_maxDistance, const s32 maxMatchesPerType,
    const s32 ransac_maxIterations, const s32 ransac_numSamplesPerType,
    const s32 ransac_inlinerDistance, s32 &bestNumInliers,
    MemoryStack fastScratch, MemoryStack slowScratch) {
  Result lastResult;
  ...
  AnkiConditionalErrorAndReturnValue(
      lastResult == RESULT_OK, lastResult,
      "BinaryTracker::IterativelyRefineTrack_Projective_Ransac",
      "FindHorizontalCorrespondences_List 1 failed");
  ...
  // TODO: implement this, perhaps with a different search distance. Or perhaps
  ...
  FixedLengthList<IndexCorrespondence> sampledMatchingIndexes_xDecreasing(
      ransac_numSamplesPerType, fastScratch);
  FixedLengthList<IndexCorrespondence> sampledMatchingIndexes_xIncreasing(
      ransac_numSamplesPerType, fastScratch);
  FixedLengthList<IndexCorrespondence> sampledMatchingIndexes_yDecreasing(
      ransac_numSamplesPerType, fastScratch);
  FixedLengthList<IndexCorrespondence> sampledMatchingIndexes_yIncreasing(
      ransac_numSamplesPerType, fastScratch);
  ...
  const IndexCorrespondence *restrict pMatchingIndexes_xDecreasing =
      matchingIndexes_xDecreasing.Pointer(0);
  const IndexCorrespondence *restrict pMatchingIndexes_xIncreasing =
      matchingIndexes_xIncreasing.Pointer(0);
  const IndexCorrespondence *restrict pMatchingIndexes_yDecreasing =
      matchingIndexes_yDecreasing.Pointer(0);
  const IndexCorrespondence *restrict pMatchingIndexes_yIncreasing =
      matchingIndexes_yIncreasing.Pointer(0);

  IndexCorrespondence *restrict pSampledMatchingIndexes_xDecreasing =
      sampledMatchingIndexes_xDecreasing.Pointer(0);
  IndexCorrespondence *restrict pSampledMatchingIndexes_xIncreasing =
      sampledMatchingIndexes_xIncreasing.Pointer(0);
  IndexCorrespondence *restrict pSampledMatchingIndexes_yDecreasing =
      sampledMatchingIndexes_yDecreasing.Pointer(0);
  IndexCorrespondence *restrict pSampledMatchingIndexes_yIncreasing =
      sampledMatchingIndexes_yIncreasing.Pointer(0);
  ...
  for (s32 iteration = 0; iteration < ransac_maxIterations; iteration++) {
    for (s32 iSample = 0; iSample < ransac_numSamplesPerType; iSample++) {
      ...
      // index_xIncreasing, index_yDecreasing, index_yIncreasing);

      pSampledMatchingIndexes_xDecreasing[iSample] =
          pMatchingIndexes_xDecreasing[index_xDecreasing];
      pSampledMatchingIndexes_xIncreasing[iSample] =
          pMatchingIndexes_xIncreasing[index_xIncreasing];
      pSampledMatchingIndexes_yDecreasing[iSample] =
          pMatchingIndexes_yDecreasing[index_yDecreasing];
      pSampledMatchingIndexes_yIncreasing[iSample] =
          pMatchingIndexes_yIncreasing[index_yIncreasing];
    }
    ...
    lastResult = BinaryTracker::ApplyHorizontalCorrespondenceList_Projective(
        sampledMatchingIndexes_xDecreasing, AtA_xDecreasing, Atb_t_xDecreasing);
    ...
    lastResult = BinaryTracker::ApplyHorizontalCorrespondenceList_Projective(
        sampledMatchingIndexes_xIncreasing, AtA_xIncreasing, Atb_t_xIncreasing);
    ...
    lastResult = BinaryTracker::ApplyVerticalCorrespondenceList_Projective(
        sampledMatchingIndexes_yDecreasing, AtA_yDecreasing, Atb_t_yDecreasing);
    ...
    lastResult = BinaryTracker::ApplyVerticalCorrespondenceList_Projective(
        sampledMatchingIndexes_yIncreasing, AtA_yIncreasing, Atb_t_yIncreasing);
    ...
    // Update the transformation, and count the number of inliers
    {
      ...
      if (!numericalFailure) {
        ...
        s32 numInliers;
        lastResult = VerifyTrack(nextImageEdges, allLimits,
                                 ransac_inlinerDistance, numInliers);
...
Result BinaryTracker::VerifyTrack(const EdgeLists &nextImageEdges,
  ...
  numTemplatePixelsMatched = numTemplatePixelsMatched_xDecreasing +
                             numTemplatePixelsMatched_xIncreasing +
                             numTemplatePixelsMatched_yDecreasing +
                             numTemplatePixelsMatched_yIncreasing;
...
s32 BinaryTracker::get_serializationSize() const {
  ...
  const s32 requiredBytes =
      512 + static_cast<s32>(numTemplatePixels) * sizeof(Point<s16>) +
      Transformations::PlanarTransformation_f32::get_serializationSize() +
      16 * SerializedBuffer::DESCRIPTION_STRING_LENGTH;
