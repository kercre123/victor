...
#define DRAW_DEBUG_IMAGES \
  0  // Requires VisionComponent to be in Synchronous mode

namespace Anki {
namespace Embedded {

NearestNeighborLibrary::NearestNeighborLibrary()
    : _probeXOffsets(NULL), _probeYOffsets(NULL) {}

NearestNeighborLibrary::NearestNeighborLibrary(
    const std::string& dataPath, const s32 numDataPoints, const s32 dataDim,
  ...
  AnkiConditionalErrorAndReturn(
      fp, "NearestNeighborLibrary.Constructor.MissingFile",
...
NearestNeighborLibrary::NearestNeighborLibrary(
    const u8* data, const u8* weights, const u16* labels,
    const s32 numDataPoints, const s32 dataDim, const s16* probeCenters_X,
    const s16* probeCenters_Y, const s16* probePoints_X,
    const s16* probePoints_Y, const s32 numProbePoints,
...
Result NearestNeighborLibrary::GetNearestNeighbor(const Array<u8>& image,
  ...
  Result lastResult =
      VisionMarker::GetProbeValues(image, homography,
  ...
  for (s32 iExample = 0; iExample < _numDataPoints; ++iExample) {
    ...
    if (currentDistance < closestDistance) {
      ...
      if (DRAW_DEBUG_IMAGES) {
        diffImage.reshape(0, VisionMarker::GRIDSIZE)
            .copyTo(secondBestDiffImage);
      }
    }
  }
  ...
  if (closestIndex != -1) {
    ...
    const s32 secondLabel =
        (secondClosestIndex >= 0 ? _labels.at<s16>(secondClosestIndex) : -1);
    ...
    if (DRAW_DEBUG_IMAGES) {
      ...
      cv::Mat_<u8> tempClosestExample(
          VisionMarker::GRIDSIZE, VisionMarker::GRIDSIZE, _data[closestIndex]);
      cv::resize(tempClosestExample, closestExampleDisp,
                 cv::Size(dispSize, dispSize));
      ...
      cv::putText(closestExampleDisp, closestStr,
                  cv::Point(0, closestExampleDisp.rows - 2), CV_FONT_NORMAL,
                  0.4, cv::Scalar(0, 0, 255));
      cv::imshow("ClosestExample", closestExampleDisp);

      if (secondClosestIndex != -1) {
        cv::resize(secondBestDiffImage, secondBestDiffImage,
                   cv::Size(dispSize, dispSize));
        cv::imshow("SecondDiff", secondBestDiffImage);

        cv::Mat secondClosestDisp;
        cv::Mat_<u8> tempSecondExample(VisionMarker::GRIDSIZE,
                                       VisionMarker::GRIDSIZE,
                                       _data[secondClosestIndex]);
        cv::resize(tempSecondExample, secondClosestDisp,
                   cv::Size(dispSize, dispSize));
        cv::cvtColor(secondClosestDisp, secondClosestDisp, CV_GRAY2BGR);

        cv::Mat maskDisp;
        cv::absdiff(tempClosestExample, tempSecondExample, maskDisp);
        cv::resize(maskDisp > distThreshold, maskDisp,
                   cv::Size(dispSize, dispSize), CV_INTER_NN);
        ...
        cv::putText(secondClosestDisp, closestStr,
                    cv::Point(0, secondClosestDisp.rows - 2), CV_FONT_NORMAL,
                    0.4, cv::Scalar(0, 0, 255));
