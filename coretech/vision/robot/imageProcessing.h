...
template <u8 upsamplePowerU8>
void UpsampleByPowerOfTwoBilinear_innerLoop(const u8 *restrict pInY0,
...
void UpsampleByPowerOfTwoBilinear_innerLoop(const u8 *restrict pInY0,
  ...
  for (s32 xSmall = 0; xSmall < smallWidth - 1; xSmall++) {
    ...
    u8 *restrict pOut =
        out.Pointer(ySmall * upsampleFactorU8 + upsampleFactorU8 / 2, 0);
    ...
    for (s32 dy = 0; dy < upsampleFactorU8; dy++) {
      ...
      u16 curValue =
          2 * interpolatedPixelL + ((addAmount - subtractAmount) >> 1);
...
Result ComputeXGradient(const Array<InType> &in, Array<OutType> &out) {
  ...
  AnkiConditionalErrorAndReturnValue(AreValid(in, out),
                                     ...
                                     "ComputeXGradient", "Invalid objects");

  AnkiConditionalErrorAndReturnValue(
      AreEqualSize(in, out), RESULT_FAIL_INVALID_SIZE, "ComputeXGradient",
  ...
  for (s32 y = 1; y < imageHeight - 1; y++) {
    ...
    for (s32 x = 1; x < imageWidth - 1; x++) {
      pOut[x] = static_cast<IntermediateType>(pIn[x + 1]) -
                static_cast<IntermediateType>(pIn[x - 1]);
    }
...
Result ComputeYGradient(const Array<InType> &in, Array<OutType> &out) {
  ...
  AnkiConditionalErrorAndReturnValue(AreValid(in, out),
                                     RESULT_FAIL_INVALID_OBJECT,
                                     "ComputeYGradient", "Invalid objects");

  AnkiConditionalErrorAndReturnValue(
      AreEqualSize(in, out), RESULT_FAIL_INVALID_SIZE, "ComputeYGradient",
      "Image sizes don't match");
  ...
  for (s32 y = 1; y < imageHeight - 1; y++) {
    ...
    for (s32 x = 1; x < imageWidth - 1; x++) {
      pOut[x] = static_cast<IntermediateType>(pIn_yp1[x]) -
                static_cast<IntermediateType>(pIn_ym1[x]);
    }
...
Result BinomialFilter(const Array<InType> &image, Array<OutType> &imageFiltered,
  ...
  const s32 imageFilteredTmpStep =
      imageFilteredTmp.get_stride() / sizeof(IntermediateType);
  ...
  for (s32 y = 0; y < imageHeight; y++) {
    ...
    IntermediateType *restrict pImageFilteredTmp =
        imageFilteredTmp.Pointer(y, 0);

    s32 x = 0;

    pImageFilteredTmp[x] = static_cast<IntermediateType>(
        (pImage[x] * kernel2 + pImage[x + 1] * kernel3 +
         pImage[x + 2] * kernel4 + pImage[x] * (kernel0 + kernel1)) >>
        kernelShift);
    x++;
    pImageFilteredTmp[x] = static_cast<IntermediateType>(
        (pImage[x - 1] * kernel1 + pImage[x] * kernel2 +
    ...
    if (y > 1) {
      const IntermediateType *restrict pImageFilteredTmpYm2 =
          pImageFilteredTmp - 4 * imageFilteredTmpStep;
      const IntermediateType *restrict pImageFilteredTmpYm1 =
          pImageFilteredTmp - 3 * imageFilteredTmpStep;
      const IntermediateType *restrict pImageFilteredTmpY0 =
          pImageFilteredTmp - 2 * imageFilteredTmpStep;
      const IntermediateType *restrict pImageFilteredTmpYp1 =
      ...
      if (y == 2) {
        for (s32 x = 0; x < imageWidth; x++) {
          pImageFiltered[x] = static_cast<OutType>(
              (pImageFilteredTmpY0[x] * (kernel0 + kernel1 + kernel2) +
               pImageFilteredTmpYp1[x] * kernel3 +
               pImageFilteredTmpYp2[x] * kernel4) >>
              kernelShift);
        }
      } else if (y == 3) {
        for (s32 x = 0; x < imageWidth; x++) {
          pImageFiltered[x] = static_cast<OutType>(
              (pImageFilteredTmpYm1[x] * (kernel0 + kernel1) +
               pImageFilteredTmpY0[x] * kernel2 +
               pImageFilteredTmpYp1[x] * kernel3 +
               pImageFilteredTmpYp2[x] * kernel4) >>
              kernelShift);
        }
        ...
        for (s32 x = 0; x < imageWidth; x++) {
          pImageFiltered[x] =
              static_cast<OutType>((pImageFilteredTmpYm2[x] * kernel0 +
                                    pImageFilteredTmpYm1[x] * kernel1 +
                                    pImageFilteredTmpY0[x] * kernel2 +
                                    pImageFilteredTmpYp1[x] * kernel3 +
                                    pImageFilteredTmpYp2[x] * kernel4) >>
                                   kernelShift);
  ...
  const IntermediateType *restrict pImageFilteredTmpYEndm4 =
      imageFilteredTmp.Pointer(imageFilteredTmp.get_size(0) - 4, 0);
  const IntermediateType *restrict pImageFilteredTmpYEndm3 =
      pImageFilteredTmpYEndm4 + imageFilteredTmpStep;
  const IntermediateType *restrict pImageFilteredTmpYEndm2 =
      pImageFilteredTmpYEndm4 + 2 * imageFilteredTmpStep;
  const IntermediateType *restrict pImageFilteredTmpYEndm1 =
      pImageFilteredTmpYEndm4 + 3 * imageFilteredTmpStep;

  OutType *restrict pImageFilteredYEndm2 =
      imageFiltered.Pointer(imageFiltered.get_size(0) - 2, 0);
  OutType *restrict pImageFilteredYEndm1 =
      imageFiltered.Pointer(imageFiltered.get_size(0) - 1, 0);

  for (s32 x = 0; x < imageWidth; x++) {
    pImageFilteredYEndm2[x] = static_cast<OutType>(
        (pImageFilteredTmpYEndm4[x] * kernel0 +
         pImageFilteredTmpYEndm3[x] * kernel1 +
         pImageFilteredTmpYEndm2[x] * kernel2 +
         pImageFilteredTmpYEndm1[x] * (kernel3 + kernel4)) >>
        kernelShift);
    pImageFilteredYEndm1[x] = static_cast<OutType>(
...
Result CreateIntegralImage(const Array<InType> &image,
  ...
  AnkiConditionalErrorAndReturnValue(
      AreValid(image, integralImage), RESULT_FAIL_INVALID_OBJECT,
      "ImageProcessing::CreateIntegralImage", "Invalid objects");

  const s32 imageHeight = image.get_size(0);
  const s32 imageWidth = image.get_size(1);

  AnkiConditionalErrorAndReturnValue(
      AreEqualSize(image, integralImage), RESULT_FAIL_INVALID_SIZE,
  ...
  for (s32 x = 1; x < imageWidth; x++) {
    pIntegralImageRow[x] =
        pIntegralImageRow[x - 1] + static_cast<OutType>(pImageRow[x]);
  }
  ...
  for (s32 y = 1; y < imageHeight; y++) {
    ...
    pIntegralImageRow[0] =
        static_cast<f32>(pImageRow[0]) + pIntegralImageRowPrev[0];
    for (s32 x = 1; x < imageWidth; x++) {
      pIntegralImageRow[x] =
          (pIntegralImageRow[x - 1] + pIntegralImageRowPrev[x] +
...
Result BoxFilter(const Array<InType> &image, const s32 boxHeight,
  ...
  AnkiConditionalErrorAndReturnValue(NotAliased(image, filtered),
                                     RESULT_FAIL_ALIASED_MEMORY, "BoxFilter",
                                     "Images are aliased");
  ...
  IntermediateType *restrict verticalAccumulator =
      reinterpret_cast<IntermediateType *>(
          scratch.Allocate(imageWidth * sizeof(IntermediateType) + 16));
  ...
  for (; y < imageHeight; y++) {
    // Grab the pointer to the horizontally negative-offset location in the
    // filtered image
    OutType *restrict pFiltered =
        filtered.Pointer(y - boxHeight2, 0) - boxWidth2;
...
Result Resize(const Array<InType> &in, Array<OutType> &out) {
  AnkiConditionalErrorAndReturnValue(AreValid(in, out),
                                     RESULT_FAIL_INVALID_OBJECT, "Resize",
                                     "Invalid objects");
...
Result DownsampleByTwo(const Array<InType> &in, Array<OutType> &out) {
  ...
  AnkiConditionalErrorAndReturnValue(AreValid(in, out),
                                     RESULT_FAIL_INVALID_OBJECT,
                                     "DownsampleByFactor", "Invalid objects");

  AnkiConditionalErrorAndReturnValue(
      AreEqualSize(inHeight / 2, inWidth / 2, out), RESULT_FAIL_INVALID_SIZE,
      "DownsampleByFactor",
      "size(out) is not equal to size(in) >> downsampleFactor");
  ...
  for (s32 y = 0, ySmall = 0; y < maxY; y += 2, ySmall++) {
    ...
    for (s32 x = 0, xSmall = 0; x < maxX; x += 2, xSmall++) {
      const u32 inSum = static_cast<IntermediateType>(pImageY0[x]) +
...
Result DownsampleByPowerOfTwo(const Array<InType> &in,
  ...
  AnkiConditionalErrorAndReturnValue(
      AreEqualSize(smallHeight, smallWidth, out), RESULT_FAIL_INVALID_SIZE,
      "DownsampleByFactor",
      "size(out) is not equal to size(in) >> downsampleFactor");

  AnkiConditionalErrorAndReturnValue(
      largeWidth % 4 == 0, RESULT_FAIL_INVALID_SIZE, "DownsampleByFactor",
      "The width of the in Array must be a multiple of four");

  Array<InType> inRow(1, largeWidth, scratch);
  Array<IntermediateType> accumulator(1, largeWidth >> downsamplePower,
                                      scratch);

  InType *restrict pInRow = inRow.Pointer(0, 0);
  IntermediateType *restrict pAccumulator = accumulator.Pointer(0, 0);

  const s32 numWordsToCopy = (sizeof(InType) * largeWidth) >>
                             2;  // If the input in stride is not a multiple of
                                 // four, this will be too small
  ...
  for (s32 ySmall = 0; ySmall < smallHeight; ySmall++) {
    ...
    // Accumulate a block of "largeWidth X downsampleFactor" pixels into a
    // "smallWidth X 1" buffer
    for (s32 yp = 0; yp < downsampleFactor; yp++) {
      const s32 yLarge = (ySmall << downsamplePower) +
                         yp;  // The actual row of the input image
      ...
      for (s32 i = 0; i < numWordsToCopy; i++) {
        ...
        reinterpret_cast<u32 *>(pInRow)[i] =
            reinterpret_cast<const u32 *>(pIn)[i];
      }

      // Next, accumulate into the accumulator
      for (s32 xSmall = 0; xSmall < smallWidth; xSmall++) {
        for (s32 xp = 0; xp < downsampleFactor; xp++) {
          const s32 xLarge = (xSmall << downsamplePower) +
                             xp;  // The actual column of the input image
    ...
    for (s32 xSmall = 0; xSmall < smallWidth; xSmall++) {
      pOut[xSmall] =
          static_cast<OutType>(pAccumulator[xSmall] >> normalizationShift);
    }  // for(s32 xSmall=0; xSmall<smallWidth; xSmall++)
  }    // for(s32 ySmall=0; ySmall<smallHeight; ySmall++)
        ...
        &memory)  //< Memory for the output will be allocated by this function
{
  ...
  AnkiConditionalErrorAndReturnValue(image.IsValid() && numPyramidLevels >= 0,
                                     FixedLengthList<Array<OutType> >(),
                                     "BuildPyramid", "Invalid inputs");
  ...
  for (s32 iLevel = 1; iLevel < numPyramidLevels; iLevel++) {
    ...
    AnkiConditionalErrorAndReturnValue(
        scaledHeight % 2 == 0 && scaledWidth % 2 == 0,
        FixedLengthList<Array<OutType> >(), "BuildPyramid",
        "Too many pyramid levels requested");
  }
  ...
  for (s32 iLevel = 1; iLevel <= numPyramidLevels; iLevel++) {
    ...
    /*const Result result = */ ImageProcessing::DownsampleByTwo<
        InType, IntermediateType, OutType>(pyramid[iLevel - 1],
                                           pyramid[iLevel]);
  }  // for(s32 iLevel=1; iLevel<=numPyramidLevels; iLevel++)
...
FixedPointArray<Type> Get1dGaussianKernel(const s32 sigma,
  ...
  const s32 halfWidth =
      1 + ((sigma * numStandardDeviations) >> numSigmaFractionalBits);
  const f32 halfWidthF32 = static_cast<f32>(halfWidth);

  FixedPointArray<Type> gaussianKernel(1, 2 * halfWidth + 1,
                                       numSigmaFractionalBits, scratch,
...

  {
    ...
    const f32 twoTimesSigmaSquared =
        static_cast<f32>(2 * sigma * sigma) /
        powf(2.0f, static_cast<f32>(numSigmaFractionalBits * 2));
    ...
    const f32 twoToNumBits =
        powf(2.0f, static_cast<f32>(numSigmaFractionalBits));
...
template <typename InType, typename IntermediateType, typename OutType>
NO_INLINE Result Correlate1d(const FixedPointArray<InType> &in1,
  ...
  AnkiConditionalErrorAndReturnValue(AreValid(in1, in2, out),
                                     RESULT_FAIL_INVALID_OBJECT, "Correlate1d",
                                     "Invalid objects");

  AnkiConditionalErrorAndReturnValue(
      in1.get_size(0) == 1 && in2.get_size(0) == 1 && out.get_size(0) == 1,
      RESULT_FAIL_INVALID_SIZE, "Correlate1d",
      "Arrays must be 1d and horizontal");

  AnkiConditionalErrorAndReturnValue(out.get_size(1) == outputLength,
                                     RESULT_FAIL_INVALID_SIZE, "Correlate1d",
                                     "Out must be the size of in1 + in2 - 1");

  AnkiConditionalErrorAndReturnValue(
      NotAliased(in1, in2, out), RESULT_FAIL_ALIASED_MEMORY, "Correlate1d",
      "in1, in2, and out must be in different memory locations");
  ...
  s32 shiftMagnitude;
  bool shiftRight;
  GetBitShiftDirectionAndMagnitude(
      in1.get_numFractionalBits(), in2.get_numFractionalBits(),
      out.get_numFractionalBits(), shiftMagnitude, shiftRight);
  ...
  for (s32 x = 0; x < midStartIndex; x++) {
    ...
    for (s32 xx = 0; xx <= x; xx++) {
      const IntermediateType toAdd =
          static_cast<IntermediateType>(pU[xx] * pV[vLength - x + xx - 1]);
      sum += toAdd;
    }
  ...
  for (s32 x = midStartIndex; x <= midEndIndex; x++) {
    ...
    for (s32 xx = 0; xx < vLength; xx++) {
      const IntermediateType toAdd =
          static_cast<IntermediateType>(pU[x + xx - midStartIndex] * pV[xx]);
      sum += toAdd;
    }
  ...
  return RESULT_OK;
}  // Result Correlate1d(const FixedPointArray<s32> &in1, const
   // FixedPointArray<s32> &in2, FixedPointArray<s32> &out)

template <typename InType, typename IntermediateType, typename OutType>
NO_INLINE Result Correlate1dCircularAndSameSizeOutput(
  ...
  AnkiConditionalErrorAndReturnValue(
      AreValid(image, filter, out), RESULT_FAIL_INVALID_OBJECT,
      "Correlate1dCircularAndSameSizeOutput", "Invalid objects");

  AnkiConditionalErrorAndReturnValue(
      imageHeight == 1 && filterHeight == 1 && out.get_size(0) == 1,
      RESULT_FAIL_INVALID_SIZE, "Correlate1dCircularAndSameSizeOutput",
      "Arrays must be 1d and horizontal");

  AnkiConditionalErrorAndReturnValue(
      imageWidth > filterWidth, RESULT_FAIL_INVALID_SIZE,
      "Correlate1dCircularAndSameSizeOutput",
      "The image must be larger than the filter");

  AnkiConditionalErrorAndReturnValue(
      NotAliased(image, filter, out), RESULT_FAIL_ALIASED_MEMORY,
      "Correlate1dCircularAndSameSizeOutput",
      "in1, in2, and out must be in different memory locations");
  ...
  paddedImage(0, 0, filterWidth + imageWidth - 1, -1)
      .Set(image(0, 0, 0, filterWidth - 2));
  ...
  bool shiftRight;
  GetBitShiftDirectionAndMagnitude(
      image.get_numFractionalBits(), filter.get_numFractionalBits(),
      out.get_numFractionalBits(), shiftMagnitude, shiftRight);

  s32 shiftType = !!shiftMagnitude;
  if (shiftRight) shiftType = 2;
  ...
  for (s32 x = 0; x < imageWidth; x++) {
    ...
    for (s32 xFilter = 0; xFilter < filterWidth; xFilter++) {
      ...
      const IntermediateType toAdd = static_cast<IntermediateType>(
          pPaddedImage[xImage] * pFilter[xFilter]);
      sum += toAdd;
    }
...
#else   // #if ACCELERATION_TYPE == ACCELERATION_NONE || ACCELERATION_TYPE ==
        // ACCELERATION_ARM_A7
  for (s32 x = 0; x < imageWidth; x++) {
    IntermediateType sum = 0;
    ...
    if (sizeof(InType) == 4 && Flags::TypeCharacteristics<InType>::isSigned) {
      ...
      for (; xFilter < filterWidthSimdMax; xFilter += 2) {
        ...
        const IntermediateType toAdd0 = static_cast<IntermediateType>(
            pPaddedImage[xImage] * pFilter[xFilter]);
        const IntermediateType toAdd1 = static_cast<IntermediateType>(
            pPaddedImage[xImage + 1] * pFilter[xFilter + 1]);
    ...
    } else if (sizeof(InType) == 2 &&
               Flags::TypeCharacteristics<InType>::isSigned) {
      ...
      for (; xFilter < filterWidthSimdMax; xFilter += 8, xImage += 8) {
        ...
        const IntermediateType toAdd1 = static_cast<IntermediateType>(
            pPaddedImage[xImage + 1] * pFilter[xFilter + 1]);
        const IntermediateType toAdd2 = static_cast<IntermediateType>(
            pPaddedImage[xImage + 2] * pFilter[xFilter + 2]);
        const IntermediateType toAdd3 = static_cast<IntermediateType>(
            pPaddedImage[xImage + 3] * pFilter[xFilter + 3]);
        const IntermediateType toAdd4 = static_cast<IntermediateType>(
            pPaddedImage[xImage + 4] * pFilter[xFilter + 4]);
        const IntermediateType toAdd5 = static_cast<IntermediateType>(
            pPaddedImage[xImage + 5] * pFilter[xFilter + 5]);
        const IntermediateType toAdd6 = static_cast<IntermediateType>(
            pPaddedImage[xImage + 6] * pFilter[xFilter + 6]);
        const IntermediateType toAdd7 = static_cast<IntermediateType>(
            pPaddedImage[xImage + 7] * pFilter[xFilter + 7]);

        sum += toAdd0 + toAdd1 + toAdd2 + toAdd3 + toAdd4 + toAdd5 + toAdd6 +
               toAdd7;
        //#else // #if !defined(USE_ARM_ACCELERATION_IMAGE_PROCESSING)
        //              // TODO: make work for any template type, and so Keil
        //              doesn't merge loads const u32 image01 =
        //              *reinterpret_cast<const u32*>(&pPaddedImage[xImage]);
        //              const u32 image23 = *reinterpret_cast<const
        //              u32*>(&pPaddedImage[xImage + 2]); const u32 image45 =
        //              *reinterpret_cast<const u32*>(&pPaddedImage[xImage +
        //              4]); const u32 image67 = *reinterpret_cast<const
        //              u32*>(&pPaddedImage[xImage + 6]);
        //
        //              const u32 filter01 = *reinterpret_cast<const
        //              u32*>(&pFilter[xFilter]); const u32 filter23 =
        //              *reinterpret_cast<const u32*>(&pFilter[xFilter + 2]);
        //              const u32 filter45 = *reinterpret_cast<const
        //              u32*>(&pFilter[xFilter + 4]); const u32 filter67 =
        //              *reinterpret_cast<const u32*>(&pFilter[xFilter + 6]);
        //
        ...
        //#endif // #if !defined(USE_ARM_ACCELERATION_IMAGE_PROCESSING) ...
        //#else
      }
...
Result LocalMaxima(const Array<Type> &in, const s32 searchHeight,
  ...
  AnkiConditionalErrorAndReturnValue(
      searchHeight == 3 && searchWidth == 3, RESULT_FAIL_INVALID_PARAMETER,
      "LocalMaxima", "Currently only 3x3 supported");
  ...
  for (s32 y = 1; y < (inHeight - 1); y++) {
    ...
    if (values) {
      for (s32 x = 1; x < (inWidth - 1); x++) {
        ...
        if (centerValue > pIn_ym1[x - 1] && centerValue > pIn_ym1[x] &&
            centerValue > pIn_ym1[x + 1] && centerValue > pIn_y0[x - 1] &&
            centerValue > pIn_y0[x + 1] && centerValue > pIn_yp1[x - 1] &&
            centerValue > pIn_yp1[x] && centerValue > pIn_yp1[x + 1]) {
...
NO_INLINE Result UpsampleByPowerOfTwoBilinear(const Array<u8> &in,
                                              Array<u8> &out,
                                              MemoryStack scratch) {
  // The correct weights would be given by d2, though we approximate them in
  // this function dSize=2; ds=zeros(dSize,dSize,4); ds(1,1,1)=1; ds(1,end,2)=1;
  // ds(end,1,3)=1; ds(end,end,4)=1; d2 = imresize(ds, [size(ds,1),
  // size(ds,2)]*upsampleFactor, 'bilinear', 'Antialiasing', false)
  ...
  AnkiConditionalErrorAndReturnValue(
      AreValid(in, out, scratch), RESULT_FAIL_INVALID_OBJECT,
      "UpsampleByPowerOfTwoBilinear", "Invalid objects");

  AnkiConditionalErrorAndReturnValue(
      AreEqualSize(smallHeight, smallWidth, in), RESULT_FAIL_INVALID_SIZE,
      "UpsampleByPowerOfTwoBilinear",
      "size(out) is not equal to size(in) << downsampleFactor");

  AnkiConditionalErrorAndReturnValue(
      largeWidth % 4 == 0, RESULT_FAIL_INVALID_SIZE,
      "UpsampleByPowerOfTwoBilinear",
      "The width of the in Array must be a multiple of four");

  AnkiConditionalErrorAndReturnValue(
      upsamplePower > 0 && upsamplePower < 8, RESULT_FAIL_INVALID_PARAMETER,
      "UpsampleByPowerOfTwoBilinear", "0 < upsamplePower < 8");
  ...
  // this function dSize=2; ds=zeros(dSize,dSize,4); ds(1,1,1)=1; ds(1,end,2)=1;
  // ds(end,1,3)=1; ds(end,end,4)=1; d2 = imresize(ds, [size(ds,1),
  // size(ds,2)]*4, 'bilinear', 'Antialiasing', false)

  // TODO: do the edges

  // Just compute the boxWidth*UL in an accumulator, and as you go right,
  // subtract and add. As you go down, subtract and add top vs bottom
  ...
  // const s32 ySmall = -1;
  {
    ...
    out(0, (upsampleFactorU8 >> 1) - 1, 0, (upsampleFactorU8 >> 1) - 1)
        .Set(pInY0[0]);
    ...
    for (s32 xSmall = 0; xSmall < smallWidth - 1; xSmall++) {
      ...
      for (s32 dy = upsampleFactorU8 >> 1; dy < upsampleFactorU8; dy++) {
        u8 *restrict pOut = out.Pointer(
            ySmall * upsampleFactorU8 + upsampleFactorU8 / 2 + dy, 0);
        ...
        u16 curValue = (u16)((smallL << (upsamplePowerU8 + 2)) +
                             ((addAmount - subtractAmount) >> 1));
    ...
    out(0, (upsampleFactorU8 >> 1) - 1, -(upsampleFactorU8 >> 1), -1)
        .Set(pInY0[smallWidth - 1]);
  }  // const s32 ySmall = -1;
  ...
  for (s32 ySmall = 0; ySmall < smallHeight - 1; ySmall++) {
    ...
    // const s32 xSmall = -1;
    {
      ...
      for (s32 dy = 0; dy < upsampleFactorU8; dy++) {
        ...
        const u16 interpolatedPixelL =
            interpolatedPixelL0 + interpolatedPixelL1;
    ...
    UpsampleByPowerOfTwoBilinear_innerLoop<upsamplePower>(
        pInY0, pInY1, out, ySmall, smallWidth, outStride);
  ...
  // const s32 ySmall = smallHeight - 1;
  {
    ...
    out(-(upsampleFactorU8 >> 1), -1, 0, (upsampleFactorU8 >> 1) - 1)
        .Set(pInY0[0]);
    ...
    out(-(upsampleFactorU8 >> 1), -1, -(upsampleFactorU8 >> 1), -1)
        .Set(pInY0[smallWidth - 1]);
  }  // const s32 ySmall = smallHeight - 1;

  return RESULT_OK;
}  // Result UpsampleBilinear(const Array<u8> &in, Array<u8> &out, MemoryStack
   // scratch)
}  // namespace ImageProcessing
}  // namespace Embedded
}  // namespace Anki
