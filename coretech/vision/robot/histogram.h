...
namespace Embedded {
class IntegerCounts {
  // An IntegerCounts is a simple histogram that has one bin for every possible
  // value. For example, an IntegerCounts of an Array<u8> will have 256 bins.
 ...
 public:
  typedef struct Statistics {
    ...
    f32 standardDeviation;  //< The unbiased standard deviation of the values of
                            //all elements
  } Statistics;
  ...
  IntegerCounts(const Array<u8> &image,
  ...
  // For example, the .1 percentile is the value where 10% of the histogram
  // energy is less than the value If there's an error, it will return -1
  s32 ComputePercentile(const f32 percentile) const;

  // This operation should be quite fast for u8 IntegerCounts, so even if you
  // only need one of the statistics, computing them all probably isn't an
  // issue.
  ...
  // The sum of the values of all bins. For example, for a 320x240 image,
  // numElements = 76800
