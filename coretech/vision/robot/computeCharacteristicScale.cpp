...
// To normalize a sum of 1 / ((2*n+1)^2), we approximate a division as a
// mulitiply and shift. These multiply coefficients were computed in matlab by
// typing: n=65; bestShifts = zeros(n,1); bestMultipliers = zeros(n,1); for
// i=0:(n-1) [bestShifts(i+1), bestMultipliers(i+1)] =
// computeClosestMultiplicationShift((2*i+1)^2, 31, 8); end;
// toArray(bestMultipliers'); toArray(bestShifts');
static const s32 normalizationMultiply[MAX_FILTER_HALF_WIDTH + 1] = {
    1,   57,  41,  167, 101, 135, 97,  73,  227, 91,  149, 31,  105,
    45,  39,  17,  241, 107, 191, 43,  39,  71,  129, 237, 109, 101,
    187, 173, 161, 151, 141, 33,  31,  117, 55,  13,  197, 93,  177,
    21,  5,   19,  145, 139, 33,  253, 121, 29,  223, 107, 103, 99,
    95,  183, 177, 85,  41,  159, 153, 37,  143, 139, 67,  65,  63};
static const s32 normalizationBitShifts[MAX_FILTER_HALF_WIDTH + 1] = {
    0,  9,  10, 13, 13, 14, 14, 14, 16, 15, 16, 14, 16, 15, 15, 14, 18,
    17, 18, 16, 16, 17, 18, 19, 18, 18, 19, 19, 19, 19, 19, 17, 17, 19,
    18, 16, 20, 19, 20, 17, 15, 17, 20, 20, 18, 21, 20, 18, 21, 20, 20,
    20, 20, 21, 21, 20, 19, 21, 21, 19, 21, 21, 20, 20, 20};

namespace Anki {
namespace Embedded {
// These are not inlined, to make it easier to hand-optimize them. Inlining them
// will probably only slightly increase speed.
NO_INLINE void ecvcs_filterRows(
    const ScrollingIntegralImage_u8_s32 &integralImage,
    const FixedLengthList<s32> &filterHalfWidths, const s32 imageY,
    FixedLengthList<Array<u8>> &filteredRows);

// NO_INLINE void ecvcs_computeBinaryImage_numFilters5(const Array<u8> &image,
// const Array<u8> * restrict filteredRows, const s32
// scaleImage_numPyramidLevels, const s32 scaleImage_thresholdMultiplier, const
// s32 imageY, const s32 imageWidth, u8 * restrict pBinaryImageRow); NO_INLINE
// void ecvcs_computeBinaryImage_numPyramids3_thresholdMultiplier1(const
// Array<u8> &image, const Array<u8> * restrict filteredRows, const s32
// scaleImage_numPyramidLevels, const s32 imageY, const s32 imageWidth, u8 *
// restrict pBinaryImageRow);

NO_INLINE void ecvcs_computeBinaryImage(
    const Array<u8> &image, FixedLengthList<Array<u8>> &filteredRows,
    const s32 scaleImage_thresholdMultiplier, const s32 imageY,
    u8 *restrict pBinaryImageRow, const bool isDarkOnLight);

NO_INLINE void ecvcs_computeBinaryImage_numFilters3(
    const Array<u8> &image, FixedLengthList<Array<u8>> &filteredRows,
    const s32 scaleImage_thresholdMultiplier, const s32 imageY,
    u8 *restrict pBinaryImageRow, const bool isDarkOnLight);

NO_INLINE void ecvcs_computeBinaryImage_numFilters5(
    const Array<u8> &image, FixedLengthList<Array<u8>> &filteredRows,
    const s32 scaleImage_thresholdMultiplier, const s32 imageY,
    u8 *restrict pBinaryImageRow, const bool isDarkOnLight);

NO_INLINE void ecvcs_computeBinaryImage_numFilters5_thresholdMultiplier1(
    const Array<u8> &image, FixedLengthList<Array<u8>> &filteredRows,
    const s32 scaleImage_thresholdMultiplier, const s32 imageY,
    u8 *restrict pBinaryImageRow, const bool isDarkOnLight);
...
NO_INLINE void ecvcs_filterRows(
  ...
  // For the given row of the image, compute the blurred version for each level
  // of the pyramid
  for (s32 iHalfWidth = 0; iHalfWidth < numFilterHalfWidths; iHalfWidth++) {
    ...
    const Rectangle<s16> filter(-filterHalfWidth, filterHalfWidth,
                                -filterHalfWidth, filterHalfWidth);
    ...
    integralImage.FilterRow(filter, imageY, multiply, shift,
                            pFilteredRows[iHalfWidth]);
...
NO_INLINE void ecvcs_computeBinaryImage_numFilters3(
  ...
  const int32x4_t kFracBits = vdupq_n_s32(
      -thresholdMultiplier_numFractionalBits);  // Negative for right shift
  ...
  for (; x < kNumIterations; x += kNumElementsProcessedPerLoop) {
    ...
    // Compare image value to threshold, if it is less than a 1 will be written
    // otherwise 0
    uint32x4_t lessThanThresh_1 =
        vcltq_s32((int32x4_t)img32x4_1, thresholdValue_1);
    uint32x4_t lessThanThresh_2 =
        vcltq_s32((int32x4_t)img32x4_2, thresholdValue_2);
    uint32x4_t lessThanThresh_3 =
        vcltq_s32((int32x4_t)img32x4_3, thresholdValue_3);
    uint32x4_t lessThanThresh_4 =
        vcltq_s32((int32x4_t)img32x4_4, thresholdValue_4);
    ...
    // Use lessThanThresh to select either 1 or 0 to be written as output to the
    // binary image
  ...
  for (; x < imageWidth; x++) {
    const s32 dog0 = ABS(static_cast<s32>(*pFilteredRows1) -
                         static_cast<s32>(*pFilteredRows0));
    const s32 dog1 = ABS(static_cast<s32>(*pFilteredRows2) -
                         static_cast<s32>(*pFilteredRows1));
    ...
    const s32 thresholdValue = (scaleValue * scaleImage_thresholdMultiplier) >>
                               thresholdMultiplier_numFractionalBits;
  ...
  for (s32 x = 0; x < imageWidth; x++) {
    ...
    const s32 dog0 = ABS(static_cast<s32>(pFilteredRows1[x]) -
                         static_cast<s32>(pFilteredRows0[x]));
    const s32 dog1 = ABS(static_cast<s32>(pFilteredRows2[x]) -
                         static_cast<s32>(pFilteredRows1[x]));
    ...
    //} // for(s32 pyramidLevel=0; pyramidLevel<scaleImage_numPyramidLevels;
...
NO_INLINE void ecvcs_computeBinaryImage_numFilters5(
  ...
  for (s32 x = 0; x < imageWidth; x++) {
    ...
    const s32 dog1 = ABS(static_cast<s32>(pFilteredRows2[x]) -
                         static_cast<s32>(pFilteredRows1[x]));
    const s32 dog2 = ABS(static_cast<s32>(pFilteredRows3[x]) -
                         static_cast<s32>(pFilteredRows2[x]));
    const s32 dog3 = ABS(static_cast<s32>(pFilteredRows4[x]) -
                         static_cast<s32>(pFilteredRows3[x]));
...
NO_INLINE void ecvcs_computeBinaryImage_numFilters5_thresholdMultiplier1(
  ...
  for (; x < (imageWidth - 15); x += 16) {
    ...
    const uint8x16_t dogMax =
        vmaxq_u8(vmaxq_u8(dog0, dog1), vmaxq_u8(dog2, dog3));
    ...
    const uint8x16_t binaryVector =
        vbslq_u8(scaleValueIsLarger, ones8x16, zeros8x16);
  ...
  for (; x < imageWidth; x++) {
    // for(s32 iHalfWidth=0; iHalfWidth<(numFilteredRows-1); iHalfWidth++) {
    const s16 dog0 = ABS(static_cast<s16>(pFilteredRows1[x]) -
                         static_cast<s16>(pFilteredRows0[x]));
    const s16 dog1 = ABS(static_cast<s16>(pFilteredRows2[x]) -
                         static_cast<s16>(pFilteredRows1[x]));
    const s16 dog2 = ABS(static_cast<s16>(pFilteredRows3[x]) -
                         static_cast<s16>(pFilteredRows2[x]));
    const s16 dog3 = ABS(static_cast<s16>(pFilteredRows4[x]) -
                         static_cast<s16>(pFilteredRows3[x]));
...
NO_INLINE void ecvcs_computeBinaryImage(
  ...
  for (s32 x = 0; x < imageWidth; x++) {
    ...
    for (s32 iHalfWidth = 0; iHalfWidth < (numFilteredRows - 1); iHalfWidth++) {
      const s32 dog = ABS(static_cast<s32>(pFilteredRows[iHalfWidth + 1][x]) -
                          static_cast<s32>(pFilteredRows[iHalfWidth][x]));
...
Result ExtractComponentsViaCharacteristicScale(
  ...
  AnkiConditionalErrorAndReturnValue(
      AreValid(fastScratch, slowerScratch, slowestScratch),
      RESULT_FAIL_INVALID_OBJECT, "ExtractComponentsViaCharacteristicScale",
      "scratch is not valid");

  AnkiConditionalErrorAndReturnValue(
      NotAliased(fastScratch, slowerScratch, slowestScratch),
      RESULT_FAIL_ALIASED_MEMORY, "ExtractComponentsViaCharacteristicScale",
      "fast and slow scratch buffers cannot be the same object");

  AnkiConditionalErrorAndReturnValue(
      AreValid(image, filterHalfWidths, components), RESULT_FAIL_INVALID_OBJECT,
      "ExtractComponentsViaCharacteristicScale", "Invalid objects");

  AnkiConditionalErrorAndReturnValue(
      numFilterHalfWidths > 0 && numFilterHalfWidths <= MAX_FILTER_HALF_WIDTH,
      RESULT_FAIL_INVALID_PARAMETER, "ExtractComponentsViaCharacteristicScale",
      "invalid numFilterHalfWidths");
  ...
  //      if(imageWidth <= 320) {
  //        integralImageHeight = MAX(2*numBorderPixels + 16, 64); //
  //        96*(640+33*2)*4 = 271104, though with padding, it is 96*720*4 =
  //        276480
  //      } else {
  //        // Note: These numbers are liable to be too big to fit on the M4
  //        efficiently const s32 scaleFactor =
  //        static_cast<s32>(ceilf(static_cast<f32>(imageWidth) / 320.0f));
  //        integralImageHeight = MAX(2*numBorderPixels + 16*scaleFactor,
  //        64*scaleFactor);
  //      }
  ...
  ScrollingIntegralImage_u8_s32 integralImage(integralImageHeight, imageWidth,
                                              numBorderPixels, slowerScratch);
  if ((lastResult = integralImage.ScrollDown(image, integralImageHeight,
                                             fastScratch)) != RESULT_OK)
    return lastResult;
  ...
  //      cv::copyMakeBorder(cvCompute(cv::Rect(1,1,imageWidth,integralImageHeight)),
  //      cvIntImg, 0, 0, numBorderPixels, numBorderPixels,
  //      cv::BORDER_REPLICATE);

  // Prepare the memory for the filtered rows for each level of the pyramid
  FixedLengthList<Array<u8>> filteredRows(numFilterHalfWidths, fastScratch,
                                          Flags::Buffer(false, false, true));

  AnkiConditionalErrorAndReturnValue(
      filteredRows.IsValid(), RESULT_FAIL_OUT_OF_MEMORY,
      "ExtractComponentsViaCharacteristicScale", "filteredRows is not valid");

  for (s32 i = 0; i < numFilterHalfWidths; i++) {
    filteredRows[i] = Array<u8>(1, imageWidth, fastScratch);
    AnkiConditionalErrorAndReturnValue(
        filteredRows[i].IsValid(), RESULT_FAIL_OUT_OF_MEMORY,
        "ExtractComponentsViaCharacteristicScale", "filteredRows is not valid");
  }
  ...
  AnkiConditionalErrorAndReturnValue(
      binaryImageRow.IsValid(), RESULT_FAIL_OUT_OF_MEMORY,
      "ExtractComponentsViaCharacteristicScale", "binaryImageRow is not valid");

  u8 *restrict pBinaryImageRow = binaryImageRow[0];

  if ((lastResult = components.Extract2dComponents_PerRow_Initialize(
           fastScratch, slowerScratch, slowestScratch)) != RESULT_OK)
    return lastResult;
  ...
  if (!g_saveBinaryInitialized) {
    ...
    g_binaryImage = Anki::Embedded::Array<u8>(image.get_size(0),
                                              image.get_size(1), tmpMemory);
    g_saveBinaryInitialized = true;
  }
#endif

  std::function<void(const Array<u8> &, FixedLengthList<Array<u8>> &, const s32,
                     const s32, u8 *restrict, const bool)>
      binarizationFcn = nullptr;
  ...
  switch (numFilterHalfWidths) {
    ...
    case 5:
      if (scaleImage_thresholdMultiplier == 65536) {
        binarizationFcn =
            &ecvcs_computeBinaryImage_numFilters5_thresholdMultiplier1;
      } else {
  ...
  while (imageY < imageHeight) {
    ...
    binarizationFcn(image, filteredRows, scaleImage_thresholdMultiplier, imageY,
                    pBinaryImageRow, isDarkOnLight);
    ...
    if ((lastResult = components.Extract2dComponents_PerRow_NextRow(
             pBinaryImageRow, imageWidth, imageY, component1d_minComponentWidth,
             component1d_maxSkipDistance)) != RESULT_OK)
      return lastResult;
    ...
    if (integralImage.get_maxRow(numBorderPixels - 1) < imageY) {
      if ((lastResult = integralImage.ScrollDown(image, numRowsToScroll,
                                                 fastScratch)) != RESULT_OK)
        return lastResult;
    }
  ...
  if ((lastResult = components.Extract2dComponents_PerRow_Finalize()) !=
      RESULT_OK)
    return lastResult;
