...
Result ExtractComponentsViaCharacteristicScale_binomial(
  ...
  AnkiConditionalErrorAndReturnValue(
      ...
      "ExtractComponentsViaCharacteristicScale_binomial",
  ...
  AnkiConditionalErrorAndReturnValue(
      AreValid(image, components), RESULT_FAIL_INVALID_OBJECT,
      "ExtractComponentsViaCharacteristicScale_binomial", "Invalid objects");
  ...
  for (s32 iLevel = 1; iLevel < numPyramidLevels; iLevel++) {
    ...
    AnkiConditionalErrorAndReturnValue(
        scaledHeight % 2 == 0 && scaledWidth % 2 == 0, RESULT_FAIL,
        "ExtractComponentsViaCharacteristicScale_binomial",
  ...
  FixedLengthList<Array<u8> > blurredImagePyramid(numPyramidLevels,
                                                  slowestScratch);
  ...
  AnkiConditionalErrorAndReturnValue(
      AreValid(imagePyramid, blurredImagePyramid, dogPyramid, dogMax,
               scaleImage),
      RESULT_FAIL_OUT_OF_MEMORY,
      "ExtractComponentsViaCharacteristicScale_binomial", "Out of memory");
  ...
  for (s32 iLevel = 0; iLevel < numPyramidLevels; iLevel++) {
    ...
    if (iLevel == 0) {
      ...
      imagePyramid[iLevel] =
          Array<u8>(scaledHeight, scaledWidth, slowestScratch);
    }

    blurredImagePyramid[iLevel] =
        Array<u8>(scaledHeight, scaledWidth, slowestScratch);
    dogPyramid[iLevel] = Array<u8>(scaledHeight, scaledWidth, slowestScratch);

    AnkiConditionalErrorAndReturnValue(
        AreValid(imagePyramid[iLevel], blurredImagePyramid[iLevel],
                 dogPyramid[iLevel]),
        RESULT_FAIL_OUT_OF_MEMORY,
        "ExtractComponentsViaCharacteristicScale_binomial", "Out of memory");
  }
  ...
  AnkiConditionalErrorAndReturnValue(
      AreValid(binaryImageRow), RESULT_FAIL_OUT_OF_MEMORY,
      "ExtractComponentsViaCharacteristicScale_binomial",
      "binaryImageRow is not valid");
  ...
  for (s32 iLevel = 0; iLevel < numPyramidLevels; iLevel++) {
    if (iLevel != 0) {
      ...
      // if((lastResult =
      // ImageProcessing::DownsampleByTwo<u8,u16,u8>(imagePyramid[iLevel-1],
      // imagePyramid[iLevel])) != RESULT_OK)
      //  return lastResult;
      if ((lastResult = ImageProcessing::DownsampleByTwo<u8, u16, u8>(
               blurredImagePyramid[iLevel - 1], imagePyramid[iLevel])) !=
          RESULT_OK)
        return lastResult;
    ...
    if ((lastResult = ImageProcessing::BinomialFilter<u8, u8, u8>(
             imagePyramid[iLevel], blurredImagePyramid[iLevel],
             slowestScratch)) != RESULT_OK)
      return lastResult;
    EndBenchmark("ecvcsB_binomial");

    BeginBenchmark("ecvcsB_SAD");
    if ((lastResult = Matrix::SumOfAbsDiff<u8, s16, u8>(
             imagePyramid[iLevel], blurredImagePyramid[iLevel],
             dogPyramid[iLevel])) != RESULT_OK)
      return lastResult;
    EndBenchmark("ecvcsB_SAD");

    //#if ANKICORETECH_EMBEDDED_USE_OPENCV
    // char name[1024];
    // snprintf(name, 1024, "image"); imagePyramid[iLevel].Show(name, false,
    // false, true); snprintf(name, 1024, "blurred");
    // blurredImagePyramid[iLevel].Show(name, false, false, true); snprintf(name,
    // 1024, "dog"); dogPyramid[iLevel].Show(name, false, false, true);
    // cv::waitKey();
  ...
  if (upsampleToFullSize) {
    ...
    for (s32 iLevel = 0; iLevel < numPyramidLevels; iLevel++) {
      ...
      if (iLevel == 0) {
        ...
        ImageProcessing::UpsampleByPowerOfTwoBilinear<1>(
            imagePyramid[iLevel], upsampledImage, slowestScratch);
        ImageProcessing::UpsampleByPowerOfTwoBilinear<1>(
            dogPyramid[iLevel], upsampledDog, slowestScratch);
        EndBenchmark("ecvcsB_scale_upsample");
      } else if (iLevel == 2) {
        BeginBenchmark("ecvcsB_scale_upsample");
        upsampledImage = upsampledImageTmp;
        upsampledDog = upsampledDogTmp;
        ImageProcessing::UpsampleByPowerOfTwoBilinear<2>(
            imagePyramid[iLevel], upsampledImage, slowestScratch);
        ImageProcessing::UpsampleByPowerOfTwoBilinear<2>(
            dogPyramid[iLevel], upsampledDog, slowestScratch);
        EndBenchmark("ecvcsB_scale_upsample");
        ...
        upsampledImage = upsampledImageTmp;
        upsampledDog = upsampledDogTmp;
        ImageProcessing::UpsampleByPowerOfTwoBilinear<3>(
            imagePyramid[iLevel], upsampledImage, slowestScratch);
        ImageProcessing::UpsampleByPowerOfTwoBilinear<3>(
            dogPyramid[iLevel], upsampledDog, slowestScratch);
        EndBenchmark("ecvcsB_scale_upsample");
        ...
        upsampledImage = upsampledImageTmp;
        upsampledDog = upsampledDogTmp;
        ImageProcessing::UpsampleByPowerOfTwoBilinear<4>(
            imagePyramid[iLevel], upsampledImage, slowestScratch);
        ImageProcessing::UpsampleByPowerOfTwoBilinear<4>(
            dogPyramid[iLevel], upsampledDog, slowestScratch);
        EndBenchmark("ecvcsB_scale_upsample");
        ...
        upsampledImage = upsampledImageTmp;
        upsampledDog = upsampledDogTmp;
        ImageProcessing::UpsampleByPowerOfTwoBilinear<5>(
            imagePyramid[iLevel], upsampledImage, slowestScratch);
        ImageProcessing::UpsampleByPowerOfTwoBilinear<5>(
            dogPyramid[iLevel], upsampledDog, slowestScratch);
        EndBenchmark("ecvcsB_scale_upsample");
      ...
      // snprintf(name, 1024, "scaleImage"); scaleImage.Show(name, false, false,
      // true); cv::waitKey(); #endif // #if ANKICORETECH_EMBEDDED_USE_OPENCV
    }       // for(s32 iLevel=0; iLevel<numPyramidLevels; iLevel++)
    ...
    for (s32 iLevel = 0; iLevel < numPyramidLevels; iLevel++) {
      ...
      for (s32 yBig = 0; yBig < imageHeight; yBig++) {
        const u8 *restrict pImage =
            imagePyramid[iLevel].Pointer(yBig / xyOutputStep, 0);
        const u8 *restrict pDog =
            dogPyramid[iLevel].Pointer(yBig / xyOutputStep, 0);
      ...
      //#if ANKICORETECH_EMBEDDED_USE_OPENCV
      //          char name[1024];
      //          snprintf(name, 1024, "dogMax"); dogMax.Show(name, false,
      //          false, true); snprintf(name, 1024, "scaleImage");
      //          scaleImage.Show(name, false, false, true); cv::waitKey();
      //#endif // #if ANKICORETECH_EMBEDDED_USE_OPENCV
    }  // for(s32 iLevel=0; iLevel<numPyramidLevels; iLevel++)
  ...
  for (s32 y = 0; y < imageHeight; y++) {
    ...
    for (s32 x = 0; x < imageWidth; x++) {
      const s32 thresholdValue =
          (pScaleImage[x] * scaleImage_thresholdMultiplier) >>
    ...
    if ((lastResult = components.Extract2dComponents_PerRow_NextRow(
             pBinaryImageRow, imageWidth, y, component1d_minComponentWidth,
