...
#define BINOMIAL_KERNEL_SIZE 5
    ...
    IN_DDR Result BinomialFilter(const Array<u8> &image, Array<u8> &imageFiltered, MemoryStack scratch)
    {
      ...
      {
        const u32 * restrict imageFilteredTmp_rowPointerYm4 = imageFilteredTmp.Pointer(imageHeight-4, 0);
        const u32 * restrict imageFilteredTmp_rowPointerYm3 = imageFilteredTmp.Pointer(imageHeight-3, 0);
        const u32 * restrict imageFilteredTmp_rowPointerYm2 = imageFilteredTmp.Pointer(imageHeight-2, 0);
        const u32 * restrict imageFilteredTmp_rowPointerYm1 = imageFilteredTmp.Pointer(imageHeight-1, 0);

        u8 * restrict imageFiltered_rowPointerYm2 = imageFiltered.Pointer(imageHeight-2, 0);
        u8 * restrict imageFiltered_rowPointerYm1 = imageFiltered.Pointer(imageHeight-1, 0);
        for(s32 x=0; x<imageWidth; x++) {
