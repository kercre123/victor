...
inline LucasKanadeTracker_SampledPlanar6dof::TemplateSample
ComputeTemplateSample(const u8 grayValue, const f32 xOriginal,
                      const f32 yOriginal, const f32 xGradient,
                      const f32 yGradient, const HomographyStruct &H,
                      const dR_dtheta_Struct &dR, const f32 focalLength_x,
                      const f32 focalLength_y, Array<f32> &AtA) {
  ...
  const f32 r1thetaXterm =
      /*dr11_dthetaX*xOriginal +*/ dR.dr12_dthetaX * yOriginal;
  const f32 r1thetaYterm =
      dR.dr11_dthetaY * xOriginal + dR.dr12_dthetaY * yOriginal;
  const f32 r1thetaZterm =
      dR.dr11_dthetaZ * xOriginal + dR.dr12_dthetaZ * yOriginal;

  const f32 r2thetaXterm =
      /*dr21_dthetaX*xOriginal + */ dR.dr22_dthetaX * yOriginal;
  const f32 r2thetaYterm =
      dR.dr21_dthetaY * xOriginal + dR.dr22_dthetaY * yOriginal;
  const f32 r2thetaZterm =
      dR.dr21_dthetaZ * xOriginal + dR.dr22_dthetaZ * yOriginal;

  const f32 r3thetaXterm =
      /*dr31_dthetaX*xOriginal + */ dR.dr32_dthetaX * yOriginal;
  const f32 r3thetaYterm =
      dR.dr31_dthetaY * xOriginal + dR.dr32_dthetaY * yOriginal;
...
LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_SampledPlanar6dof(
    const Array<u8> &templateImage, const Quadrilateral<f32> &templateQuadIn,
    ...
    const Point<f32> &roundedCornersFraction, const s32 maxSamplesAtBaseLevel,
    const s32 numSamplingRegions, const f32 focalLength_x,
    const f32 focalLength_y, const f32 camCenter_x, const f32 camCenter_y,
    const std::vector<f32> &distortionCoeffs, const Point<f32> &templateSize_mm,
    MemoryStack ccmMemory, MemoryStack &onchipScratch,
    MemoryStack offchipScratch)
    : LucasKanadeTracker_Generic(Transformations::TRANSFORM_PROJECTIVE,
                                 templateImage, templateQuadIn,
  ...
  // TODO: better way to accomplish this?  Feed in un-reordered corners from
  // detected marker?
  Quadrilateral<f32> clockwiseQuad =
      templateQuadIn.ComputeClockwiseCorners<f32>();
  ...
  const f32 templateQuadDiagonal = MAX(diff03.Length(), diff12.Length()) /
  sqrtf(2.f);
  */
  ...
  Result lastResult = RESULT_FAIL;
  {
    ...
    calibMatrix[0][0] = focalLength_x;
    calibMatrix[0][1] = 0.f;
    calibMatrix[0][2] = camCenter_x;
    calibMatrix[1][0] = 0.f;
    calibMatrix[1][1] = focalLength_y;
    calibMatrix[1][2] = camCenter_y;
    calibMatrix[2][0] = 0.f;
    calibMatrix[2][1] = 0.f;
    calibMatrix[2][2] = 1.f;

    cv::Mat_<f32> cvDistortionCoeffs(
        1, (s32)distortionCoeffs.size(),
        const_cast<f32 *>(distortionCoeffs.data()));
    ...
    cv::solvePnP(cvObjPoints, cvImagePoints, calibMatrix_cvMat,
                 cvDistortionCoeffs, cvRvec, cvTranslation, false,
                 CV_ITERATIVE);
  ...
  Result lastResult = P3P::computePose(
      templateQuad, template3d[0], template3d[1], template3d[2], template3d[3],
      this->focalLength_x, this->focalLength_y, this->camCenter_x,
      this->camCenter_y, R, this->params6DoF.translation);

  // NOTE: isValid will still be false in this case.
  AnkiConditionalErrorAndReturn(
      lastResult == RESULT_OK, "LKTracker_SampledPlanar6dof",
      "Failed to compute initial pose constructing tracker");
  ...
  */
  ...
  Array<f32> initialHomography = Array<f32>(
      3, 3, onchipScratch);  // TODO: on-chip b/c this is permanent, right?

  initialHomography[0][0] = this->focalLength_x * R[0][0];
  initialHomography[0][1] = this->focalLength_x * R[0][1];
  initialHomography[0][2] =
      this->focalLength_x *
      this->params6DoF.translation.x;  // / initialImageScaleF32;

  initialHomography[1][0] = this->focalLength_y * R[1][0];
  initialHomography[1][1] = this->focalLength_y * R[1][1];
  initialHomography[1][2] =
      this->focalLength_y *
      this->params6DoF.translation.y;  // / initialImageScaleF32;
  ...
  Quadrilateral<f32> initCorners(
      Point<float>(template3d[0].x, template3d[0].y),
  ...
  this->transformation = Transformations::PlanarTransformation_f32(
      Transformations::TRANSFORM_PROJECTIVE, initCorners, initialHomography,
      centerOffset,
  ...
  dR_dtheta_Struct dR_dtheta(params6DoF.angle_x, params6DoF.angle_y,
                             params6DoF.angle_z);
  ...
  this->templateSamplePyramid =
      FixedLengthList<FixedLengthList<TemplateSample> >(numPyramidLevels,
                                                        onchipScratch);
  this->verificationSamples = FixedLengthList<VerifySample>(
      verifyGridSize * verifyGridSize, onchipScratch);
  this->AtAPyramid =
      FixedLengthList<Array<f32> >(numPyramidLevels, onchipScratch);

  AnkiConditionalErrorAndReturn(
      this->templateSamplePyramid.IsValid(),
      "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
      "SampledPlanar6dof",
      "Unable to allocate templateSamplePyramid FixedLengthList.");

  AnkiConditionalErrorAndReturn(
      this->verificationSamples.IsValid(),
      "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
      "SampledPlanar6dof",
      "Unable to allocate verificationSamples FixedLengthList.");

  AnkiConditionalErrorAndReturn(
      this->AtAPyramid.IsValid(),
      "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
      "SampledPlanar6dof",
  ...
  this->normalizationMean =
      FixedLengthList<f32>(numPyramidLevels, onchipScratch);
  this->normalizationSigmaInv =
      FixedLengthList<f32>(numPyramidLevels, onchipScratch);
  ...
  for (s32 iScale = 0; iScale < numPyramidLevels; iScale++) {
    ...
    const s32 numFiducialSamplesPerEdge =
        CeilS32(static_cast<f32>(numFiducialSamplesAtScale) / 8.f);
    ...
    //
    {
      ...
      this->templateSamplePyramid[iScale] = FixedLengthList<TemplateSample>(
          curMaxSamples + actualNumFiducialSamplesAtScale, onchipScratch);
      AnkiConditionalErrorAndReturn(
          this->templateSamplePyramid[iScale].IsValid(),
          "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
          "SampledPlanar6dof",
          "Unable to allocate templateSamplePyramid[%d] FixedLengthList.",
          iScale);
      ...
      Array<s32> magnitudeIndexes(
          1, numPointsY * numPointsX,
          offchipScratch);  // NOTE: u16 not enough for full 320x240
      s32 numSelected = 0;
      {
        PUSH_MEMORY_STACK(offchipScratch);
        ...
        AnkiConditionalErrorAndReturn(
            templateImageAtScale.IsValid(),
            "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
            "SampledPlanar6dof",
            "Out of memory allocating templateImageAtScale. Could not allocate "
            "%dx%d points.",
            numPointsX, numPointsY);

#if USE_BLURRING
        Array<u8> templateImageBoxFiltered(templateImage.get_size(0),
                                           templateImage.get_size(1),
                                           offchipScratch);
        ...
        if(boxSize > 2) {
        ImageProcessing::BoxFilter<u8, s32, u8>(templateImage, boxSize, boxSize,
        templateImageBoxFiltered, offchipScratch); } else {
          templateImageBoxFiltered = templateImage;
        }
         */
        cv::Mat_<u8> cvTemplateImage(
            templateImage.get_size(0), templateImage.get_size(1),
            const_cast<u8 *>(
                static_cast<const u8 *>(templateImage.get_buffer())));
        cv::GaussianBlur(cvTemplateImage, cvTemplateImage, cv::Size(0, 0),
                         static_cast<double>(scale) / 3.f);
        ...
        if (0) {
          ...
          fwrite(templateImageBoxFiltered.get_buffer(),
                 templateImageBoxFiltered.get_numElements(), sizeof(u8), fp);
          fclose(fp);
        }

        if ((lastResult = Interp2_Projective<u8, u8>(
                 templateImageBoxFiltered, templateCoordinates,
                 transformation.get_homography(),
                 this->transformation.get_centerOffset(initialImageScaleF32),
                 templateImageAtScale, INTERPOLATE_LINEAR)) != RESULT_OK) {
          AnkiError(
              "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
              "SampledPlanar6dof",
              "Interp2_Projective failed with code 0x%x", lastResult);
          return;
        }

        if (0) {
          cv::Mat_<u8> temp(templateImageAtScale.get_size(0),
                            templateImageAtScale.get_size(1));
          for (s32 i = 0; i < temp.rows; ++i) {
            memcpy(temp[i], templateImageAtScale.Pointer(i, 0),
                   temp.cols * sizeof(u8));
          }

          FILE *fp;
          char filename[64];
          snprintf(filename, 64, "templateImageAtScale_%dx%d.dat",
                   templateImageAtScale.get_size(0),
                   templateImageAtScale.get_size(1));
          fp = fopen(filename, "wb");
        ...
        if ((lastResult = Interp2_Projective<u8, u8>(
                 templateImage, templateCoordinates,
                 transformation.get_homography(),
                 this->transformation.get_centerOffset(initialImageScaleF32),
                 templateImageAtScale, INTERPOLATE_LINEAR)) != RESULT_OK) {
          AnkiError(
        ...
        if (numFiducialSamplesAtScale > 0) {
          ConstArraySliceExpression<u8> temp =
              templateImageAtScale(0, -1, 0, -1);
...
#if USE_INTENSITY_NORMALIZATION
        // Compute the mean and standard deviation of the template for
        // normalization
        f32 tempVariance;
        if ((lastResult = Matrix::MeanAndVar<u8, f32>(
                 templateImageAtScale, this->normalizationMean[iScale],
                 tempVariance)) != RESULT_OK) {
          AnkiError(
              "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
              "SampledPlanar6dof",
              "MeanAndVar failed with code 0x%x", lastResult);
          return;
        }
        AnkiConditionalError(
            this->normalizationMean[iScale] < 0.f ||
                this->normalizationMean[iScale] > 255.f,
            "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
            "SampledPlanar6dof",
        ...
        this->normalizationSigmaInv[iScale] =
            1.f / sqrtf(static_cast<f32>(tempVariance));
#endif

        // Compute X gradient image
        Array<f32> templateImageXGradient(numPointsY, numPointsX,
                                          offchipScratch);
        AnkiConditionalErrorAndReturn(
            templateImageXGradient.IsValid(),
            "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
            "SampledPlanar6dof",
            "Out of memory allocating templateImageXGradient");
        if ((lastResult = ImageProcessing::ComputeXGradient<u8, f32, f32>(
                 templateImageAtScale, templateImageXGradient)) != RESULT_OK) {
          AnkiError(
              "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
              "SampledPlanar6dof",
        ...
        Array<f32> templateImageYGradient(numPointsY, numPointsX,
                                          offchipScratch);
        AnkiConditionalErrorAndReturn(
            templateImageYGradient.IsValid(),
            "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
            "SampledPlanar6dof",
            "Out of memory allocating templateImageYGradient");
        if ((lastResult = ImageProcessing::ComputeYGradient<u8, f32, f32>(
                 templateImageAtScale, templateImageYGradient)) != RESULT_OK) {
          AnkiError(
              "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
              "SampledPlanar6dof",
        ...
        // them
        {
          ...
          */

          {
            ...
            AnkiConditionalErrorAndReturn(
                magnitudeImage.IsValid(),
                "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
                "SampledPlanar6dof",
                "Out of memory allocating magnitudeImage. Could not allocate "
                "%dx%d points.",
                numPointsX, numPointsY);
            {
              PUSH_MEMORY_STACK(offchipScratch);
              ...
              AnkiConditionalErrorAndReturn(
                  tmpMagnitude.IsValid(),
                  "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
                  "SampledPlanar6dof",
                  "Out of memory allocating tmpMagnitude. Could not allocate "
                  "%dx%d points.",
                  numPointsX, numPointsY);

              Matrix::DotMultiply<f32, f32, f32>(
                  templateImageXGradient, templateImageXGradient, tmpMagnitude);
              Matrix::DotMultiply<f32, f32, f32>(templateImageYGradient,
                                                 templateImageYGradient,
                                                 magnitudeImage);
              Matrix::Add<f32, f32, f32>(tmpMagnitude, magnitudeImage,
                                         magnitudeImage);
            }  // pop tmpMagnitude
...

            {
              ...
              Array<bool> magnitudeImageNLMS(numPointsY, numPointsX,
                                             offchipScratch);
              AnkiConditionalErrorAndReturn(
                  magnitudeImageNLMS.IsValid(),
                  "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
                  "SampledPlanar6dof",
                  "Out of memory allocating magnitudeImageNLMS. Could not "
                  "allocate %dx%d points.",
                  numPointsX, numPointsY);
...
#endif
                const f32 *restrict mag_iPrev =
                    magnitudeImage.Pointer(i - 1, 0);  // prev
                const f32 *restrict mag_i = magnitudeImage.Pointer(i, 0);
                const f32 *restrict mag_iNext =
                    magnitudeImage.Pointer(i + 1, 0);  // next
            ...
            LucasKanadeTracker_SampledPlanar6dof::ApproximateSelect(
                magnitudeImage, numSelectBins, numSamplingRegions,
                numDesiredSamples, numSelected, magnitudeIndexes);
          }  // pop magnitudeImage
        } else {
        ...
        if ((lastResult = Matrix::Vectorize<f32, f32>(
                 false, templateImageXGradient, xGradientVector)) !=
            RESULT_OK) {
          AnkiError(
              "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
              "SampledPlanar6dof",
        ...
        if ((lastResult = Matrix::Vectorize<f32, f32>(
                 false, templateImageYGradient, yGradientVector)) !=
            RESULT_OK) {
          AnkiError(
              "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
              "SampledPlanar6dof",
        ...
        if ((lastResult = Matrix::Vectorize<u8, u8>(
                 false, templateImageAtScale, grayscaleVector)) != RESULT_OK) {
          AnkiError(
              "LucasKanadeTracker_SampledPlanar6dof::LucasKanadeTracker_"
              "SampledPlanar6dof",
              "Matrix::Vectorize failed with code 0x%x", lastResult);
        }
      }  // pop templateImageAtScale, templateImageXGradient, and
         // templateImageYGradient

      this->templateSamplePyramid[iScale].set_size(
          numSelected + actualNumFiducialSamplesAtScale);
      ...
      if (numSelected == 0) {
        ...
        Array<f32> yCoordinatesVector =
            templateCoordinates.EvaluateY1(false, offchipScratch);
        Array<f32> xCoordinatesVector =
            templateCoordinates.EvaluateX1(false, offchipScratch);
        ...
        // Fill in these interior samples starting after the on-fiducial
        // samples, if any (even though those are actually filled in below)
        TemplateSample *restrict pTemplateSamplePyramid =
            this->templateSamplePyramid[iScale].Pointer(
                actualNumFiducialSamplesAtScale);
        ...
        for (s32 iSample = 0; iSample < numSamples; iSample++) {
          ...
          pTemplateSamplePyramid[iSample] = ComputeTemplateSample(
              pGrayscaleVector[curIndex], pXCoordinates[curIndex],
              pYCoordinates[curIndex],
              scaleOverFiveTen * pXGradientVector[curIndex],
              scaleOverFiveTen * pYGradientVector[curIndex], currentH,
              dR_dtheta, this->focalLength_x, this->focalLength_y, curAtA);
    ...
    if (numFiducialSquareSamples > 0) {
      const u8 fiducialSampleGrayValue =
          (templateBrightValue + templateDarkValue) / 2;

      const f32 contrast =
          static_cast<f32>(templateBrightValue - templateDarkValue);
      // const f32 derivMagnitude = (0.5f / 255.f) * contrast *
      // templateQuadDiagonal; // NOTE: scale in numerator and denominator cancel
      const f32 derivMagnitude = (0.5f / 255.f) * contrast * scale;

      const f32 innerTemplateWidth =
          (1.f - 2.f * fiducialSquareThicknessFraction.x) * templateSize_mm.x;
      const f32 innerTemplateHeight =
          (1.f - 2.f * fiducialSquareThicknessFraction.y) * templateSize_mm.y;
      ...
      const f32 outerInc_x = 0.5f * (1 - 2 * roundedCornersFraction.x) *
                             (templateSize_mm.x) /
                             (numFiducialSamplesPerEdge - 1);
      const f32 innerInc_x =
          0.5f *
          (1 - 2 * std::max(fiducialSquareThicknessFraction.x,
                            roundedCornersFraction.x)) *
          (templateSize_mm.x) / (numFiducialSamplesPerEdge - 1);

      const f32 outerInc_y = 0.5f * (1 - 2 * roundedCornersFraction.y) *
                             (templateSize_mm.y) /
                             (numFiducialSamplesPerEdge - 1);
      const f32 innerInc_y =
          0.5f *
          (1 - 2 * std::max(fiducialSquareThicknessFraction.y,
                            roundedCornersFraction.y)) *
          (templateSize_mm.y) / (numFiducialSamplesPerEdge - 1);
...
#else
      const f32 outerInc_x = (1 - 2 * roundedCornersFraction.x) *
                             (templateSize_mm.x) /
                             (numFiducialSamplesPerEdge - 1);
      const f32 innerInc_x =
          (1 - 2 * std::max(fiducialSquareThicknessFraction.x,
      ...
      const f32 outerInc_y = (1 - 2 * roundedCornersFraction.y) *
                             (templateSize_mm.y) /
                             (numFiducialSamplesPerEdge - 1);
      const f32 innerInc_y =
          (1 - 2 * std::max(fiducialSquareThicknessFraction.y,
      ...
      const f32 signFlip = 1.f;
#endif
      TemplateSample *restrict pOuterTop =
          this->templateSamplePyramid[iScale].Pointer(0);
      TemplateSample *restrict pOuterBtm =
          this->templateSamplePyramid[iScale].Pointer(
              1 * numFiducialSamplesPerEdge);
      TemplateSample *restrict pInnerTop =
          this->templateSamplePyramid[iScale].Pointer(
              2 * numFiducialSamplesPerEdge);
      TemplateSample *restrict pInnerBtm =
          this->templateSamplePyramid[iScale].Pointer(
              3 * numFiducialSamplesPerEdge);

      TemplateSample *restrict pOuterLeft =
          this->templateSamplePyramid[iScale].Pointer(
              4 * numFiducialSamplesPerEdge);
      TemplateSample *restrict pOuterRight =
          this->templateSamplePyramid[iScale].Pointer(
              5 * numFiducialSamplesPerEdge);
      TemplateSample *restrict pInnerLeft =
          this->templateSamplePyramid[iScale].Pointer(
              6 * numFiducialSamplesPerEdge);
      TemplateSample *restrict pInnerRight =
          this->templateSamplePyramid[iScale].Pointer(
              7 * numFiducialSamplesPerEdge);

      f32 outer_x = -(1 - roundedCornersFraction.x) * templateHalfWidth;
      f32 inner_x = -(1 - std::max(roundedCornersFraction.x,
                                   fiducialSquareThicknessFraction.x)) *
                    innerTemplateHalfWidth;

      f32 outer_y = -(1 - roundedCornersFraction.y) * templateHalfHeight;
      f32 inner_y = -(1 - std::max(roundedCornersFraction.y,
                                   fiducialSquareThicknessFraction.y)) *
                    innerTemplateHalfHeight;

      if (roundedCornersFraction.x == 0 && roundedCornersFraction.y == 0) {
        // Top/Bottom Edges' Left Corners:
        pOuterTop[0] = ComputeTemplateSample(
            fiducialSampleGrayValue, outer_x, -templateHalfHeight,
            -derivMagnitude, -derivMagnitude, currentH, dR_dtheta,
            this->focalLength_x, this->focalLength_y, curAtA);

        pInnerTop[0] = ComputeTemplateSample(
            fiducialSampleGrayValue, inner_x, -innerTemplateHalfHeight,
            derivMagnitude, derivMagnitude, currentH, dR_dtheta,
            this->focalLength_x, this->focalLength_y, curAtA);

        pOuterBtm[0] = ComputeTemplateSample(
            fiducialSampleGrayValue, outer_x * signFlip,
            templateHalfHeight * signFlip, -derivMagnitude,
            derivMagnitude * signFlip, currentH, dR_dtheta, this->focalLength_x,
            this->focalLength_y, curAtA);

        pInnerBtm[0] = ComputeTemplateSample(
            fiducialSampleGrayValue, inner_x * signFlip,
            innerTemplateHalfHeight * signFlip, derivMagnitude,
            -derivMagnitude * signFlip, currentH, dR_dtheta,
            this->focalLength_x, this->focalLength_y, curAtA);

        // Left/Right Edges' Top Corners:
        pOuterLeft[0] = ComputeTemplateSample(
            fiducialSampleGrayValue, -templateHalfWidth, outer_x,
            -derivMagnitude, -derivMagnitude, currentH, dR_dtheta,
            this->focalLength_x, this->focalLength_y, curAtA);

        pOuterRight[0] = ComputeTemplateSample(
            fiducialSampleGrayValue, templateHalfWidth, outer_x, derivMagnitude,
            -derivMagnitude, currentH, dR_dtheta, this->focalLength_x,
            this->focalLength_y, curAtA);

        pInnerLeft[0] = ComputeTemplateSample(
            fiducialSampleGrayValue, -innerTemplateHalfWidth, inner_x,
            derivMagnitude, derivMagnitude, currentH, dR_dtheta,
            this->focalLength_x, this->focalLength_y, curAtA);

        pInnerRight[0] = ComputeTemplateSample(
            fiducialSampleGrayValue, innerTemplateHalfWidth, inner_x,
            -derivMagnitude, derivMagnitude, currentH, dR_dtheta,
            this->focalLength_x, this->focalLength_y, curAtA);
      ...
      for (s32 iSample = 1; iSample < numFiducialSamplesPerEdge - 1;
        ...
        // Top / Bottom Edges
        pOuterTop[iSample] = ComputeTemplateSample(
            fiducialSampleGrayValue, outer_x, -templateHalfHeight, 0.f,
            -derivMagnitude, currentH, dR_dtheta, this->focalLength_x,
            this->focalLength_y, curAtA);

        pInnerTop[iSample] = ComputeTemplateSample(
            fiducialSampleGrayValue, inner_x, -innerTemplateHalfHeight, 0.f,
            derivMagnitude, currentH, dR_dtheta, this->focalLength_x,
            this->focalLength_y, curAtA);

        pOuterBtm[iSample] = ComputeTemplateSample(
            fiducialSampleGrayValue, outer_x * signFlip,
            templateHalfHeight * signFlip, 0.f, derivMagnitude * signFlip,
            currentH, dR_dtheta, this->focalLength_x, this->focalLength_y,
            curAtA);

        pInnerBtm[iSample] = ComputeTemplateSample(
            fiducialSampleGrayValue, inner_x * signFlip,
            innerTemplateHalfHeight * signFlip, 0.f, -derivMagnitude * signFlip,
            currentH, dR_dtheta, this->focalLength_x, this->focalLength_y,
            curAtA);
      }
      ...
      for (s32 iSample = 1; iSample < numFiducialSamplesPerEdge - 1;
        ...
        // Left / Right Edges
        pOuterLeft[iSample] = ComputeTemplateSample(
            fiducialSampleGrayValue, -templateHalfWidth, outer_y,
            -derivMagnitude, 0.f, currentH, dR_dtheta, this->focalLength_x,
            this->focalLength_y, curAtA);

        pOuterRight[iSample] = ComputeTemplateSample(
            fiducialSampleGrayValue, templateHalfWidth, outer_y, derivMagnitude,
            0.f, currentH, dR_dtheta, this->focalLength_x, this->focalLength_y,
            curAtA);

        pInnerLeft[iSample] = ComputeTemplateSample(
            fiducialSampleGrayValue, -innerTemplateHalfWidth, inner_y,
            derivMagnitude, 0.f, currentH, dR_dtheta, this->focalLength_x,
            this->focalLength_y, curAtA);

        pInnerRight[iSample] = ComputeTemplateSample(
            fiducialSampleGrayValue, innerTemplateHalfWidth, inner_y,
            -derivMagnitude, 0.f, currentH, dR_dtheta, this->focalLength_x,
            this->focalLength_y, curAtA);
      ...
      if (roundedCornersFraction.x == 0 && roundedCornersFraction.y == 0) {
        ...
        pInnerTop[numFiducialSamplesPerEdge - 1] = ComputeTemplateSample(
            fiducialSampleGrayValue, inner_x, -innerTemplateHalfHeight,
            -derivMagnitude, derivMagnitude, currentH, dR_dtheta,
            this->focalLength_x, this->focalLength_y, curAtA);

        pOuterTop[numFiducialSamplesPerEdge - 1] = ComputeTemplateSample(
            fiducialSampleGrayValue, outer_x, -templateHalfHeight,
            derivMagnitude, -derivMagnitude, currentH, dR_dtheta,
            this->focalLength_x, this->focalLength_y, curAtA);

        pInnerBtm[numFiducialSamplesPerEdge - 1] = ComputeTemplateSample(
            fiducialSampleGrayValue, inner_x * signFlip,
            innerTemplateHalfHeight * signFlip, -derivMagnitude,
            -derivMagnitude * signFlip, currentH, dR_dtheta,
            this->focalLength_x, this->focalLength_y, curAtA);

        pOuterBtm[numFiducialSamplesPerEdge - 1] = ComputeTemplateSample(
            fiducialSampleGrayValue, outer_x * signFlip,
            templateHalfHeight * signFlip, derivMagnitude,
            derivMagnitude * signFlip, currentH, dR_dtheta, this->focalLength_x,
            this->focalLength_y, curAtA);
        ...
        pOuterLeft[numFiducialSamplesPerEdge - 1] = ComputeTemplateSample(
            fiducialSampleGrayValue, -templateHalfWidth, outer_x,
            -derivMagnitude, derivMagnitude, currentH, dR_dtheta,
            this->focalLength_x, this->focalLength_y, curAtA);

        pOuterRight[numFiducialSamplesPerEdge - 1] = ComputeTemplateSample(
            fiducialSampleGrayValue, templateHalfWidth, outer_x, derivMagnitude,
            derivMagnitude, currentH, dR_dtheta, this->focalLength_x,
            this->focalLength_y, curAtA);

        pInnerLeft[numFiducialSamplesPerEdge - 1] = ComputeTemplateSample(
            fiducialSampleGrayValue, -innerTemplateHalfWidth, inner_x,
            derivMagnitude, -derivMagnitude, currentH, dR_dtheta,
            this->focalLength_x, this->focalLength_y, curAtA);

        pInnerRight[numFiducialSamplesPerEdge - 1] = ComputeTemplateSample(
            fiducialSampleGrayValue, innerTemplateHalfWidth, inner_x,
            -derivMagnitude, -derivMagnitude, currentH, dR_dtheta,
            this->focalLength_x, this->focalLength_y, curAtA);
      }
  ...
  //
  {
    ...
    const f32 verifyCoordScalarInv =
        static_cast<f32>(std::numeric_limits<s8>::max()) / templateHalfWidth;
    ...
    if (numFiducialSquareSamples > 0) {
      ...
      interiorHalfWidth = 0.5f * templateSize_mm.x *
                          (1.f - 4.f * fiducialSquareThicknessFraction.x);
      interiorHalfHeight = 0.5f * templateSize_mm.y *
                           (1.f - 4.f * fiducialSquareThicknessFraction.y);

      const Point<f32> fiducialSquareCenter(
          0.5f * (1.f - fiducialSquareThicknessFraction.x) * templateSize_mm.x,
          0.5f * (1.f - fiducialSquareThicknessFraction.y) * templateSize_mm.y);
      const Point<f32> gapCenter(
          0.5f * (1.f - 3.f * fiducialSquareThicknessFraction.x) *
              templateSize_mm.x,
          0.5f * (1.f - 3.f * fiducialSquareThicknessFraction.y) *
              templateSize_mm.y);
      const Point<f32> gapSidesStart(
          0.5f * (1.f - 4.f * fiducialSquareThicknessFraction.x) *
              templateSize_mm.x,
          0.5f * (1.f - 4.f * fiducialSquareThicknessFraction.y) *
              templateSize_mm.y);

      // Top/Bottom Bars of the Square
      interiorStartIndex = 0;
      lastResult = CreateVerificationSamples(
          templateImage,
          Linspace(-fiducialSquareCenter.x, fiducialSquareCenter.x,
                   verifyGridSize),
#if SAMPLE_TOP_HALF_ONLY
          Linspace(-fiducialSquareCenter.y, -fiducialSquareCenter.y,
                   1),  // top line only
#else
          Linspace(-fiducialSquareCenter.y, fiducialSquareCenter.y, 2),
#endif
          verifyCoordScalarInv, interiorStartIndex, onchipScratch);

      AnkiConditionalErrorAndReturn(lastResult == RESULT_OK,
                                    "LucasKanadeTracker_SampledPlanar6dof::"
                                    "LucasKanadeTracker_SampledPlanar6dof",
                                    "CreateVerificationSamples for Top/Bottom "
                                    "Square Bars failed with code 0x%x",
                                    lastResult);
      ...
      AnkiConditionalErrorAndReturn(lastResult == RESULT_OK,
                                    "LucasKanadeTracker_SampledPlanar6dof::"
                                    "LucasKanadeTracker_SampledPlanar6dof",
                                    "CreateVerificationSamples for Left/Right "
                                    "Square Bars failed with code 0x%x",
                                    lastResult);
      ...
      AnkiConditionalErrorAndReturn(lastResult == RESULT_OK,
                                    ...
                                    "LucasKanadeTracker_SampledPlanar6dof",
                                    "CreateVerificationSamples for Top/Bottom "
                                    "Gap Bars failed with code 0x%x",
                                    lastResult);

      // Left/Right Bars of the Gap
      lastResult = CreateVerificationSamples(
          templateImage, Linspace(-gapCenter.x, gapCenter.x, 2),
#if SAMPLE_TOP_HALF_ONLY
...
Result LucasKanadeTracker_SampledPlanar6dof::CreateVerificationSamples(
    const Array<u8> &image, const LinearSequence<f32> &Xlocations,
    const LinearSequence<f32> &Ylocations, const f32 verifyCoordScalarInv,
    s32 &startIndex, MemoryStack scratch) {
  // static Matlab matlab(false);
  // matlab.EvalStringEcho("figure, h_axes = axes; hold on");//, imshow(img),
  // hold on");

  Result lastResult = RESULT_OK;

  AnkiConditionalErrorAndReturnValue(
      startIndex < this->verificationSamples.get_size(),
  ...
  Array<f32> verifyGrayscaleVector(1, numSamplesX * numSamplesY, scratch);
  {
    ...
    lastResult = Interp2_Projective<u8, f32>(
        image, verifyCoordinates, this->transformation.get_homography(),
        this->transformation.get_centerOffset(1.f), verifyImage,
        INTERPOLATE_LINEAR);

    AnkiConditionalErrorAndReturnValue(
        lastResult == RESULT_OK, lastResult,
        "LucasKanadeTracker_SampledPlanar6dof::CreateVerificationSamples",
        "Interp2_Projective for verification image failed with code 0x%x",
        lastResult);

    // Turn verifyImage into a vector
    lastResult =
        Matrix::Vectorize<f32, f32>(false, verifyImage, verifyGrayscaleVector);
  ...
  AnkiConditionalErrorAndReturnValue(
      xVerifyVector.IsValid(), RESULT_FAIL_MEMORY,
      "LucasKanadeTracker_SampledPlanar6dof::CreateVerificationSamples",
      "Failed to allocate xVerifyVector.");

  AnkiConditionalErrorAndReturnValue(
      yVerifyVector.IsValid(), RESULT_FAIL_MEMORY,
      "LucasKanadeTracker_SampledPlanar6dof::CreateVerificationSamples",
  ...
  for (s32 i = 0; i < numSamplesX * numSamplesY; ++i) {
    ...
    curVerifySample.xCoordinate =
        static_cast<s8>(pXCoordinates[i] * verifyCoordScalarInv);
    curVerifySample.yCoordinate =
        static_cast<s8>(pYCoordinates[i] * verifyCoordScalarInv);
    curVerifySample.grayvalue = static_cast<u8>(pGrayvalues[i]);

    // matlab.EvalStringEcho("plot(%d, %d, 'ro');", curVerifySample.xCoordinate,
    // curVerifySample.yCoordinate);
  ...
  // matlab.EvalStringEcho("set(h_axes, 'XLim', [-130 130], 'YLim', [-130 130]);
  // grid on");

  return RESULT_OK;
}  // CreateVerificationSamples()

const FixedLengthList<LucasKanadeTracker_SampledPlanar6dof::TemplateSample>
    &LucasKanadeTracker_SampledPlanar6dof::get_templateSamples(
        const s32 atScale) const {
  AnkiConditionalError(
      atScale >= 0 && atScale < this->templateSamplePyramid.get_size(),
      "LucasKanadeTracker_SampledPlanar6dof::get_templateSamples()",
...
Result LucasKanadeTracker_SampledPlanar6dof::GetRotationMatrix(
    Array<f32> &R, bool skipLastColumn) const {
  AnkiConditionalErrorAndReturnValue(
      AreEqualSize(3, 3, R), RESULT_FAIL_INVALID_SIZE,
...
// rotation matrix
Result LucasKanadeTracker_SampledPlanar6dof::set_rotationAnglesFromMatrix(
    const Array<f32> &R) {
  return Matrix::GetEulerAngles(R, this->params6DoF.angle_x,
...
const Point3<f32> &LucasKanadeTracker_SampledPlanar6dof::GetTranslation()
    const {
...
Result LucasKanadeTracker_SampledPlanar6dof::UpdateTransformation(
    MemoryStack scratch) {
...
Result LucasKanadeTracker_SampledPlanar6dof::UpdateRotationAndTranslation(
    const Array<f32> &R, const Point3<f32> &T, MemoryStack scratch) {
...
Result LucasKanadeTracker_SampledPlanar6dof::ShowTemplate(
...
s32 LucasKanadeTracker_SampledPlanar6dof::get_numTemplatePixels(
...
Result LucasKanadeTracker_SampledPlanar6dof::UpdateTrack(
    ...
    const f32 convergenceTolerance_distance, const u8 verify_maxPixelDifference,
    bool &verify_converged, s32 &verify_meanAbsoluteDifference,
    s32 &verify_numInBounds, s32 &verify_numSimilarPixels,
    MemoryStack scratch) {
  Result lastResult;

  Transformations::PlanarTransformation_f32 previousTransformation(
      this->transformation.get_transformType(), scratch);

#if USE_BLURRING
  Array<u8> nextImageBlurred(nextImage.get_size(0), nextImage.get_size(1),
                             scratch);
  ...
  for (s32 iScale = numPyramidLevels - 1; iScale >= 0; iScale--) {
    ...
    cv::GaussianBlur(cvNextImage, cvNextImage, cv::Size(0, 0),
                     static_cast<double>(1 << iScale) / 3.f);
...
#define __NEXT_IMAGE__ nextImage
    ...
    if ((lastResult = IterativelyRefineTrack(
             __NEXT_IMAGE__, maxIterations, iScale, convergenceTolerance_angle,
    ...
    if (verify_converged) {
      ...
      if (this->transformation.get_transformType() !=
        ...
        if ((lastResult = IterativelyRefineTrack(
                 __NEXT_IMAGE__, maxIterations, iScale,
                 convergenceTolerance_angle, convergenceTolerance_distance,
  ...
  lastResult = this->VerifyTrack_Projective(
      __NEXT_IMAGE__, verify_maxPixelDifference, verify_meanAbsoluteDifference,
...
Result LucasKanadeTracker_SampledPlanar6dof::IterativelyRefineTrack(
  ...
  AnkiConditionalErrorAndReturnValue(
      ...
      "LucasKanadeTracker_SampledPlanar6dof::IterativelyRefineTrack",
  ...
  AnkiConditionalErrorAndReturnValue(
      convergenceTolerance_angle > 0.0f && convergenceTolerance_distance > 0.0f,
      RESULT_FAIL_INVALID_PARAMETER,
      "LucasKanadeTracker_SampledPlanar6dof::IterativelyRefineTrack",
      "convergenceTolerances must be greater than zero");
  ...
  if (curTransformType == Transformations::TRANSFORM_TRANSLATION) {
    return IterativelyRefineTrack_Translation(
        nextImage, maxIterations, whichScale, convergenceTolerance_distance,
        verify_converged, scratch);
  } else if (curTransformType == Transformations::TRANSFORM_PROJECTIVE) {
    return IterativelyRefineTrack_Projective(
        nextImage, maxIterations, whichScale, convergenceTolerance_angle,
        convergenceTolerance_distance, verify_converged, scratch);
  }

  return RESULT_FAIL;
}  // Result LucasKanadeTracker_SampledPlanar6dof::IterativelyRefineTrack(const
...
void LucasKanadeTracker_SampledPlanar6dof::SetGainScheduling(const f32 zMin,
                                                             const f32 zMax,
                                                             const f32 KpMin,
                                                             const f32 KpMax) {
  this->Kp_min = KpMin;
...
f32 LucasKanadeTracker_SampledPlanar6dof::GetCurrentGain() const {
  if (this->useGainScheduling) {
    ...
    if (this->params6DoF.translation.z <= this->tz_min) {
      ...
      Kp = (this->params6DoF.translation.z - this->tz_min) /
               (this->tz_max - this->tz_min) * (this->Kp_max - this->Kp_min) +
           this->Kp_min;
    }
...
Result LucasKanadeTracker_SampledPlanar6dof::IterativelyRefineTrack_Translation(
    const Array<u8> &nextImage, const s32 maxIterations, const s32 whichScale,
    const f32 convergenceTolerance_distance, bool &verify_converged,
    MemoryStack scratch) {
  ...
  for (s32 iteration = 0; iteration < maxIterations; iteration++) {
    ...
    /*
    if(numInBounds < 16) {
    AnkiWarn("LucasKanadeTracker_SampledPlanar6dof::IterativelyRefineTrack_Translation",
    "Template drifted too far out of image."); return RESULT_OK;
    }
    */
    ...
    if (numericalFailure) {
      AnkiWarn(
          "LucasKanadeTracker_SampledPlanar6dof::IterativelyRefineTrack_"
          "Translation",
          "numericalFailure");
      return RESULT_OK;
    }
...
#if COMPUTE_CONVERGENCE_FROM_CORNER_CHANGE
    f32 minChange =
        UpdatePreviousCorners(transformation, previousCorners, scratch);
  ...
  return RESULT_OK;
   ...
   // LucasKanadeTracker_SampledPlanar6dof::IterativelyRefineTrack_Translation()
...
Result LucasKanadeTracker_SampledPlanar6dof::IterativelyRefineTrack_Projective(
  ...
  for (s32 iteration = 0; iteration < maxIterations; iteration++) {
    ...
    /*
    if(numInBounds < 16) {
    AnkiWarn("LucasKanadeTracker_SampledPlanar6dof::IterativelyRefineTrack_Projective",
    "Template drifted too far out of image."); return RESULT_OK;
    }
    */
    ...
    */
    ...
    /*
    CoreTechPrint("Raw angle update = (%f,%f,%f) degrees, Raw translation udpate
    = (%f,%f,%f)\n", RAD_TO_DEG(b[0][0]), RAD_TO_DEG(b[0][1]),
    RAD_TO_DEG(b[0][2]), b[0][3], b[0][4], b[0][5]);
    */
  ...
  return RESULT_OK;
   ...
   // LucasKanadeTracker_SampledPlanar6dof::IterativelyRefineTrack_Projective()
...
Result LucasKanadeTracker_SampledPlanar6dof::VerifyTrack_Projective(
    ...
    s32 &verify_numSimilarPixels, MemoryStack scratch) {
  // This method is heavily based on Interp2_Projective
  // The call would be like: Interp2_Projective<u8,u8>(nextImage,
  // originalCoordinates, interpolationHomography, centerOffset,
  // nextImageTransformed2d, INTERPOLATE_LINEAR, 0);
  ...
  for (s32 iSample = 0; iSample < numVerifySamples; iSample++) {
    ...
    const f32 yOriginal =
        static_cast<f32>(curSample.yCoordinate) * this->verifyCoordScalar;
    const f32 xOriginal =
        static_cast<f32>(curSample.xCoordinate) * this->verifyCoordScalar;
    ...
    const s32 grayvalueDifference =
        ABS(interpolatedPixelValue - verificationPixelValue);
    ...
    this->verificationSamples[iSample].grayvalue =
        saturate_cast<u8>(interpolatedPixelValue);
#endif
  }  // for(s32 iSample=0; iSample<numTemplateSamples; iSample++)
...
Result LucasKanadeTracker_SampledPlanar6dof::ApproximateSelect(
    const Array<f32> &magnitudeVector, const s32 numBins, const s32 numToSelect,
    s32 &numSelected, Array<s32> &magnitudeIndexes) {
...
Result LucasKanadeTracker_SampledPlanar6dof::ApproximateSelect(
    const Array<f32> &magnitudeImage, const s32 numBins, const s32 numRegions,
    const s32 numToSelect, s32 &numSelected, Array<s32> &magnitudeIndexes) {
  ...
  AnkiConditionalErrorAndReturnValue(
      magnitudeIndexes.get_size(0) * magnitudeIndexes.get_size(1) ==
          nrows * ncols,
      RESULT_FAIL_INVALID_SIZE,
      "LucasKanadeTracker_SampledPlanar6dof::ApproximateSelect",
  ...
  for (s32 i_region = 0; i_region < numRegions; ++i_region) {
    ...
    for (s32 j_region = 0; j_region < numRegions; ++j_region) {
      ...
      ConstArraySlice<f32> currentRegion =
          magnitudeImage(i_min, i_max - 1, j_min, j_max - 1);
      const f32 maxMagnitude = Matrix::Max<f32>(currentRegion);
      const f32 minMagnitude =
          0.5f * maxMagnitude;  // + Matrix::Min<f32>(currentRegion));
      ...
      const f32 magnitudeIncrement =
          (maxMagnitude - minMagnitude) / static_cast<f32>(numBins);

      const s32 numToSelectThisRegion =
          (numToSelect - numSelected) / numRegionsLeft;
      // const s32 numToSelectThisRegion = numToSelect /
      // (numRegions*numRegions);

      f32 foundThreshold = -1.0f;
      for (f32 threshold = minMagnitude; threshold < maxMagnitude;
           threshold += magnitudeIncrement) {
        s32 numAbove = 0;
      ...
      */
      ...
      if (foundThreshold > 0.f) {
        ...
        for (s32 i = i_min; i < i_max; i++) {
          ...
          for (s32 j = j_min;
               j < j_max && numSelectedThisRegion < numToSelectThisRegion;
               j++) {
        ...
        // AnkiWarn("ApproximateSelect", "Finished region (%d,%d), have %d
        // selected", i_region, j_region, numSelected);
      }  // if(foundThreshold > 0.f)
