...
#if defined(ANKICORETECH_EMBEDDED_USE_OPENCV) && \
    ANKICORETECH_EMBEDDED_USE_OPENCV
  // Allocates the returned cv::Mat on the heap
  cv::Mat DrawIndexes(const f32 scale = 1.0f) const;

  static cv::Mat DrawIndexes(const s32 imageHeight, const s32 imageWidth,
...
// Calculate local extrema (edges) in an image. Returns four list for the four
...
u8 ComputeGrayvalueThreshold(
    ...
    const f32 blackPercentile,   //< What percentile of histogram energy is
                                 //black? (.1 is a good value)
    const f32 whitePercentile);  //< What percentile of histogram energy is
                                 //white? (.9 is a good value)

u8 ComputeGrayvalueThreshold(
    const Array<u8> &image,  //< Used to compute the IntegerCounts
    const Rectangle<s32>
        &imageRegionOfInterest,  //< Used to compute the IntegerCounts
