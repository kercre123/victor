...
namespace Anki {
namespace Embedded {
// Starting a components.Pointer(startComponentIndex), trace the exterior
// boundary for the component starting at startComponentIndex. extractedBoundary
// must be at at least "3*componentWidth + 3*componentHeight" (If you don't know
// the size of the component, you can just make it "3*imageWidth +
// 3*imageHeight" ). It's possible that a component could be arbitrarily large,
// so if you have the space, use as much as you have.
//
// endComponentIndex is the last index of the component starting at
// startComponentIndex. The next component is therefore startComponentIndex+1 .
//
// Requires sizeof(s16)*(2*componentWidth + 2*componentHeight) bytes of scratch
Result TraceNextExteriorBoundary(
    const ConnectedComponents &components, const s32 startComponentIndex,
    FixedLengthList<Point<s16> > &extractedBoundary, s32 &endComponentIndex,
    MemoryStack scratch) {
  const s32 numComponents = components.get_size();

  AnkiConditionalErrorAndReturnValue(
      components.IsValid(), RESULT_FAIL_INVALID_OBJECT,
      "ComputeQuadrilateralsFromConnectedComponents",
      "components is not valid");

  AnkiConditionalErrorAndReturnValue(
      extractedBoundary.IsValid(), RESULT_FAIL_INVALID_OBJECT,
      "ComputeQuadrilateralsFromConnectedComponents",
      "extractedBoundary is not valid");
  ...
  AnkiConditionalErrorAndReturnValue(
      components.get_isSortedInId(), RESULT_FAIL_INVALID_PARAMETER,
      "ComputeQuadrilateralsFromConnectedComponents",
      "components must be sorted in id");

  AnkiConditionalErrorAndReturnValue(
      startComponentIndex >= 0 && startComponentIndex < numComponents,
      RESULT_FAIL_INVALID_PARAMETER,
      "ComputeQuadrilateralsFromConnectedComponents",
      "startComponentIndex is not in range");

  const bool useU16 = components.get_useU16();
  const ConnectedComponentsTemplate<u16> *componentsU16 =
      components.get_componentsU16();
  const ConnectedComponentsTemplate<s32> *componentsS32 =
      components.get_componentsS32();
  ...
  if (useU16) {
    ...
    AnkiConditionalErrorAndReturnValue(
        componentIdU16 > 0, RESULT_FAIL,
        "ComputeQuadrilateralsFromConnectedComponents",
        "componentId is not valid.");
  } else {
    componentIdS32 = (*componentsS32)[startComponentIndex].id;

    AnkiConditionalErrorAndReturnValue(
        componentIdS32 > 0, RESULT_FAIL,
        "ComputeQuadrilateralsFromConnectedComponents",
        "componentId is not valid.");
  }
  ...
  Rectangle<s16> boundingBox(
      std::numeric_limits<s16>::max(), std::numeric_limits<s16>::min(),
      std::numeric_limits<s16>::max(), std::numeric_limits<s16>::min());
  ...
  if (useU16) {
    for (s32 i = startComponentIndex; i < numComponents; i++) {
      ...
      boundingBox.right = MAX(boundingBox.right,
                              xEnd + 1);  // +1, because the coorindate we want
                                          // is the crack after the right pixel
      boundingBox.top = MIN(boundingBox.top, y);
      boundingBox.bottom = MAX(boundingBox.bottom,
                               y + 1);  // +1, because the coorindate we want is
                                        // the crack after the bottom pixel
    }
  } else {  // if(useU16)
    for (s32 i = startComponentIndex; i < numComponents; i++) {
  ...
  if (boundingBox.left == std::numeric_limits<s16>::max() ||
      boundingBox.right == std::numeric_limits<s16>::min() ||
      boundingBox.top == std::numeric_limits<s16>::max() ||
      boundingBox.bottom == std::numeric_limits<s16>::min()) {
    AnkiWarn("ComputeQuadrilateralsFromConnectedComponents",
             "Something was corrupted with the input component");
    return RESULT_FAIL;
  }
  ...
  s16 *edge_left =
      reinterpret_cast<s16 *>(scratch.Allocate(sizeof(s16) * boxHeight));
  s16 *edge_right =
      reinterpret_cast<s16 *>(scratch.Allocate(sizeof(s16) * boxHeight));
  s16 *edge_top =
      reinterpret_cast<s16 *>(scratch.Allocate(sizeof(s16) * boxWidth));
  s16 *edge_bottom =
      reinterpret_cast<s16 *>(scratch.Allocate(sizeof(s16) * boxWidth));
  ...
  if (useU16) {
    for (s32 iSegment = startComponentIndex; iSegment <= endComponentIndex;
         iSegment++) {
      ...
      ConnectedComponentSegment<u16> currentSegment =
          (*componentsU16)[iSegment];
      ...
      for (s32 x = currentSegment.xStart; x <= currentSegment.xEnd; x++) {
        ...
        edge_left[currentSegment.y] =
            MIN(edge_left[currentSegment.y], currentSegment.xStart);
        edge_right[currentSegment.y] =
            MAX(edge_right[currentSegment.y], currentSegment.xEnd);
      }  // for(s32 x=currentSegment.xStart; x<=currentSegment.xEnd; x++)
    }    // for(s32 iSegment=startComponentIndex; iSegment<=endComponentIndex;
         // iSegment++)
  } else {  // if(useU16)
    for (s32 iSegment = startComponentIndex; iSegment <= endComponentIndex;
      ...
      // xEnd = component(iSubComponent, 3);
      // y = component(iSubComponent, 1);
      ConnectedComponentSegment<s32> currentSegment =
          (*componentsS32)[iSegment];
      currentSegment.xEnd -= boundingBox.left;
      currentSegment.xStart -= boundingBox.left;
      currentSegment.y -= boundingBox.top;
  ...
  // The components are computed with an approximate method. This means that for
  // complex shapes, it is possible to have non-contiguous components. These
  // checks are for such non-contiguous components.
  //
  // It is possible to use a heuristic to compute the boundary of
  // non-continguous components, but I think they will generally not occur with
  // good, non-occluded fiducial markers.
  bool isNonContiguous = false;

  for (s32 y = 0; y < boxHeight; y++) {
    if (edge_left[y] == std::numeric_limits<s16>::max() ||
        edge_right[y] == std::numeric_limits<s16>::min()) {
      // CoreTechPrint("edge_left[%d]=%d edge_right[%d]=%d\n", y,
      // static_cast<s32>(edge_left[y]), y, static_cast<s32>(edge_right[y]));
  ...
  for (s32 x = 0; x < boxWidth; x++) {
    if (edge_top[x] == std::numeric_limits<s16>::max() ||
        edge_bottom[x] == std::numeric_limits<s16>::min()) {
      // CoreTechPrint("edge_top[%d]=%d edge_bottom[%d]=%d\n", x,
      // static_cast<s32>(edge_left[x]), x, static_cast<s32>(edge_right[x]));
      // AnkiWarn("TraceNextExteriorBoundary", "Bad edge");
      isNonContiguous = true;
    }
  }
  ...
  //% definition start at a pixel that is both a bottom and a right (I don't see
  //a way this can't be true). for x = edge_right(end):-1:(edge_left(end)+1)
  for (s16 x = edge_right[boxHeight - 1]; x >= (edge_left[boxHeight - 1] + 1);
       x--) {
