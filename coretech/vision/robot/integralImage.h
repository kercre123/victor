...
NO_INLINE Result ScrollingIntegralImage_u8_s32::FilterRow(
    const Rectangle<s16> &filter, const s32 imageRow, const s32 outputMultiply,
    const s32 outputRightShift, Array<OutType> &output) const {
  ...
  const bool insufficientBorderPixels_left =
      (-filter.left + 1) > this->numBorderPixels;
  const bool insufficientBorderPixels_right =
      (filter.right) > this->numBorderPixels;

  // These min and max coordinates are in the original image's coordinate frame
  const s32 minX = insufficientBorderPixels_left
                       ? -(this->numBorderPixels + filter.left - 1)
                       : 0;
  const s32 maxX =
      insufficientBorderPixels_right
          ? (this->get_size(1) - 1 - filter.right + this->numBorderPixels)
          : (this->get_imageWidth() - 1);
  ...
  const s32 *restrict pIntegralImage_00 =
      this->Pointer(topOffset, 0) + leftOffset;
  const s32 *restrict pIntegralImage_01 =
      this->Pointer(topOffset, 0) + rightOffset;
  const s32 *restrict pIntegralImage_10 =
      this->Pointer(bottomOffset, 0) + leftOffset;
  const s32 *restrict pIntegralImage_11 =
      this->Pointer(bottomOffset, 0) + rightOffset;

  OutType *restrict pOutput = output.Pointer(0, 0);

  if (minX > 0) memset(pOutput, 0, minX * sizeof(OutType));

  ScrollingIntegralImage_u8_s32::FilterRow_innerLoop(
      minX, maxX, outputMultiply, outputRightShift, pIntegralImage_00,
      pIntegralImage_01, pIntegralImage_10, pIntegralImage_11, pOutput);
...
void ScrollingIntegralImage_u8_s32::FilterRow_innerLoop(
    ...
    const s32 *restrict pIntegralImage_01,
    const s32 *restrict pIntegralImage_10,
    const s32 *restrict pIntegralImage_11, OutType *restrict pOutput) {
  if (outputMultiply == 1 && outputRightShift == 0) {
    for (s32 x = minX; x <= maxX; x++) {
      pOutput[x] =
          static_cast<OutType>(pIntegralImage_11[x] - pIntegralImage_10[x] +
                               pIntegralImage_00[x] - pIntegralImage_01[x]);
    }
  } else {
    for (s32 x = minX; x <= maxX; x++) {
      pOutput[x] =
