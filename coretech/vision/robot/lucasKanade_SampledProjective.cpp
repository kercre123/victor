...
LucasKanadeTracker_SampledProjective::LucasKanadeTracker_SampledProjective()
    : LucasKanadeTracker_Generic(
          Transformations::TransformType::TRANSFORM_PROJECTIVE) {}

LucasKanadeTracker_SampledProjective::LucasKanadeTracker_SampledProjective(
    const Array<u8> &templateImage, const Quadrilateral<f32> &templateQuad,
    const f32 scaleTemplateRegionPercent, const s32 numPyramidLevels,
    const Transformations::TransformType transformType,
    const s32 maxSamplesAtBaseLevel, MemoryStack ccmMemory,
    MemoryStack &onchipScratch, MemoryStack offchipScratch)
    : LucasKanadeTracker_Generic(Transformations::TRANSFORM_PROJECTIVE,
                                 templateImage, templateQuad,
                                 scaleTemplateRegionPercent, numPyramidLevels,
                                 transformType, onchipScratch) {
  ...
  this->templateSamplePyramid =
      FixedLengthList<FixedLengthList<TemplateSample> >(numPyramidLevels,
                                                        onchipScratch);
  ...
  for (s32 iScale = 0; iScale < numPyramidLevels; iScale++) {
    ...
    Meshgrid<f32> curTemplateCoordinates = Meshgrid<f32>(
        Linspace(-this->templateRegionWidth / 2.0f,
                 this->templateRegionWidth / 2.0f,
                 static_cast<s32>(floorf(this->templateRegionWidth / scale))),
        Linspace(-this->templateRegionHeight / 2.0f,
                 this->templateRegionHeight / 2.0f,
    ...
    const s32 curMaxSamples =
        MIN(maxPossibleLocations, maxSamplesAtBaseLevel >> iScale);

    this->templateSamplePyramid[iScale] =
        FixedLengthList<TemplateSample>(curMaxSamples, onchipScratch);
  ...
  //
  {
    // This section is based off lucasKanade_Fast, except uses f32 in offchip
    // instead of integer types in onchip

    FixedLengthList<Meshgrid<f32> > templateCoordinates =
        FixedLengthList<Meshgrid<f32> >(numPyramidLevels, onchipScratch);
    FixedLengthList<Array<f32> > templateImagePyramid =
        FixedLengthList<Array<f32> >(numPyramidLevels, offchipScratch);
    FixedLengthList<Array<f32> > templateImageXGradientPyramid =
        FixedLengthList<Array<f32> >(numPyramidLevels, offchipScratch);
    FixedLengthList<Array<f32> > templateImageYGradientPyramid =
        FixedLengthList<Array<f32> >(numPyramidLevels, offchipScratch);
    FixedLengthList<Array<f32> > templateImageSquaredGradientMagnitudePyramid =
        FixedLengthList<Array<f32> >(numPyramidLevels, offchipScratch);
    ...
    AnkiConditionalErrorAndReturn(
        templateImagePyramid.IsValid() &&
            templateImageXGradientPyramid.IsValid() &&
            templateImageYGradientPyramid.IsValid() &&
            templateCoordinates.IsValid() &&
            templateImageSquaredGradientMagnitudePyramid.IsValid(),
        "LucasKanadeTracker_SampledProjective::LucasKanadeTracker_"
        "SampledProjective",
        "Could not allocate pyramid lists");
    ...
    for (s32 iScale = 0; iScale < numPyramidLevels; iScale++) {
      ...
      const s32 numPointsY =
          templateCoordinates[iScale].get_yGridVector().get_size();
      const s32 numPointsX =
          templateCoordinates[iScale].get_xGridVector().get_size();

      templateImagePyramid[iScale] =
          Array<f32>(numPointsY, numPointsX, offchipScratch);
      templateImageXGradientPyramid[iScale] =
          Array<f32>(numPointsY, numPointsX, offchipScratch);
      templateImageYGradientPyramid[iScale] =
          Array<f32>(numPointsY, numPointsX, offchipScratch);
      templateImageSquaredGradientMagnitudePyramid[iScale] =
          Array<f32>(numPointsY, numPointsX, offchipScratch);

      AnkiConditionalErrorAndReturn(
          templateImagePyramid[iScale].IsValid() &&
              templateImageXGradientPyramid[iScale].IsValid() &&
              templateImageYGradientPyramid[iScale].IsValid() &&
              templateImageSquaredGradientMagnitudePyramid[iScale].IsValid(),
          "LucasKanadeTracker_SampledProjective::LucasKanadeTracker_"
          "SampledProjective",
          "Could not allocate pyramid images");
    }

    // Sample all levels of the pyramid images
    for (s32 iScale = 0; iScale < numPyramidLevels; iScale++) {
      if ((lastResult = Interp2_Affine<u8, f32>(
               templateImage, templateCoordinates[iScale],
               transformation.get_homography(),
               this->transformation.get_centerOffset(initialImageScaleF32),
               templateImagePyramid[iScale], INTERPOLATE_LINEAR)) !=
          RESULT_OK) {
        AnkiError(
            "LucasKanadeTracker_SampledProjective::LucasKanadeTracker_"
            "SampledProjective",
            "Interp2_Affine failed with code 0x%x", lastResult);
        return;
      }
    }
    ...
    for (s32 iScale = 0; iScale < numPyramidLevels; iScale++) {
      ...
      if ((lastResult = ImageProcessing::ComputeXGradient<f32, f32, f32>(
               templateImagePyramid[iScale],
               templateImageXGradientPyramid[iScale])) != RESULT_OK) {
        AnkiError(
            "LucasKanadeTracker_SampledProjective::LucasKanadeTracker_"
            "SampledProjective",
            "ComputeXGradient failed with code 0x%x", lastResult);
        return;
      }

      if ((lastResult = ImageProcessing::ComputeYGradient<f32, f32, f32>(
               templateImagePyramid[iScale],
               templateImageYGradientPyramid[iScale])) != RESULT_OK) {
        AnkiError(
            "LucasKanadeTracker_SampledProjective::LucasKanadeTracker_"
            "SampledProjective",
            "ComputeYGradient failed with code 0x%x", lastResult);
        return;
      }

      // Using the computed gradients, find a set of the max values, and store
      // them

      Array<f32> tmpMagnitude(numPointsY, numPointsX, offchipScratch);

      Matrix::DotMultiply<f32, f32, f32>(templateImageXGradientPyramid[iScale],
                                         templateImageXGradientPyramid[iScale],
                                         tmpMagnitude);
      Matrix::DotMultiply<f32, f32, f32>(
          templateImageYGradientPyramid[iScale],
          templateImageYGradientPyramid[iScale],
          templateImageSquaredGradientMagnitudePyramid[iScale]);
      Matrix::Add<f32, f32, f32>(
          tmpMagnitude, templateImageSquaredGradientMagnitudePyramid[iScale],
          templateImageSquaredGradientMagnitudePyramid[iScale]);

      // Matrix::Sqrt<f32,f32,f32>(templateImageSquaredGradientMagnitudePyramid[iScale],
      // templateImageSquaredGradientMagnitudePyramid[iScale]);

      Array<f32> magnitudeVector = Matrix::Vectorize<f32, f32>(
          false, templateImageSquaredGradientMagnitudePyramid[iScale],
          offchipScratch);
      Array<u16> magnitudeIndexes =
          Array<u16>(1, numPointsY * numPointsX, offchipScratch);

      AnkiConditionalErrorAndReturn(
          magnitudeVector.IsValid() && magnitudeIndexes.IsValid(),
          "LucasKanadeTracker_SampledProjective::LucasKanadeTracker_"
          "SampledProjective",
          "Out of memory");
      ...
      s32 numSelected;
      LucasKanadeTracker_SampledProjective::ApproximateSelect(
          magnitudeVector, numSelectBins, numSamples, numSelected,
          magnitudeIndexes);
      ...
      Array<f32> yCoordinatesVector =
          templateCoordinates[iScale].EvaluateY1(false, offchipScratch);
      Array<f32> xCoordinatesVector =
          templateCoordinates[iScale].EvaluateX1(false, offchipScratch);

      Array<f32> yGradientVector = Matrix::Vectorize<f32, f32>(
          false, templateImageYGradientPyramid[iScale], offchipScratch);
      Array<f32> xGradientVector = Matrix::Vectorize<f32, f32>(
          false, templateImageXGradientPyramid[iScale], offchipScratch);
      Array<f32> grayscaleVector = Matrix::Vectorize<f32, f32>(
          false, templateImagePyramid[iScale], offchipScratch);
      ...
      TemplateSample *restrict pTemplateSamplePyramid =
          this->templateSamplePyramid[iScale].Pointer(0);
...
Result LucasKanadeTracker_SampledProjective::ShowTemplate(
    const char *windowName, const bool waitForKeypress,
  ...
  for (s32 iScale = 0; iScale < numPyramidLevels; iScale++) {
    ...
    const TemplateSample *restrict pTemplateSample =
        this->templateSamplePyramid[iScale].Pointer(0);
    ...
    for (s32 iSample = 0; iSample < numSamples; iSample++) {
      ...
      const s32 curY =
          Round<s32>(curTemplateSample.yCoordinate + centerOffset.y);
      const s32 curX =
          Round<s32>(curTemplateSample.xCoordinate + centerOffset.x);

      if (curX >= 0 && curY >= 0 && curX < this->templateImageWidth &&
          curY < this->templateImageHeight) {
        image[curY][curX] = 255;
      }
    }
...
bool LucasKanadeTracker_SampledProjective::IsValid() const {
  if (!LucasKanadeTracker_Generic::IsValid()) return false;
...
s32 LucasKanadeTracker_SampledProjective::get_numTemplatePixels(
    const s32 whichScale) const {
  if (whichScale < 0 || whichScale > this->numPyramidLevels) return 0;

  return this->templateSamplePyramid[whichScale].get_size();
}

Result LucasKanadeTracker_SampledProjective::UpdateTrack(
    const Array<u8> &nextImage, const s32 maxIterations,
    const f32 convergenceTolerance, const u8 verify_maxPixelDifference,
    bool &verify_converged, s32 &verify_meanAbsoluteDifference,
    s32 &verify_numInBounds, s32 &verify_numSimilarPixels,
    MemoryStack scratch) {
  Result lastResult;
  ...
  for (s32 iScale = numPyramidLevels - 1; iScale >= 0; iScale--) {
    ...
    if ((lastResult = IterativelyRefineTrack(
             nextImage, maxIterations, iScale, convergenceTolerance,
             Transformations::TRANSFORM_TRANSLATION, verify_converged,
             scratch)) != RESULT_OK)
      return lastResult;
    EndBenchmark("UpdateTrack.refineTranslation");

    if (this->transformation.get_transformType() !=
        Transformations::TRANSFORM_TRANSLATION) {
      BeginBenchmark("UpdateTrack.refineOther");
      if ((lastResult = IterativelyRefineTrack(
               nextImage, maxIterations, iScale, convergenceTolerance,
               this->transformation.get_transformType(), verify_converged,
               scratch)) != RESULT_OK)
        return lastResult;
  ...
  lastResult = this->VerifyTrack_Projective(
      nextImage, verify_maxPixelDifference, verify_meanAbsoluteDifference,
      verify_numInBounds, verify_numSimilarPixels, scratch);

  return lastResult;
}

Result LucasKanadeTracker_SampledProjective::IterativelyRefineTrack(
    const Array<u8> &nextImage, const s32 maxIterations, const s32 whichScale,
    const f32 convergenceTolerance,
    const Transformations::TransformType curTransformType,
    bool &verify_converged, MemoryStack scratch) {
  const s32 nextImageHeight = nextImage.get_size(0);
  const s32 nextImageWidth = nextImage.get_size(1);

  AnkiConditionalErrorAndReturnValue(
      this->IsValid() == true, RESULT_FAIL,
      "LucasKanadeTracker_SampledProjective::IterativelyRefineTrack",
      "This object is not initialized");

  AnkiConditionalErrorAndReturnValue(
      nextImage.IsValid(), RESULT_FAIL_INVALID_OBJECT,
      "LucasKanadeTracker_SampledProjective::IterativelyRefineTrack",
      "nextImage is not valid");

  AnkiConditionalErrorAndReturnValue(
      maxIterations > 0 && maxIterations < 1000, RESULT_FAIL_INVALID_PARAMETER,
      "LucasKanadeTracker_SampledProjective::IterativelyRefineTrack",
      "maxIterations must be greater than zero and less than 1000");

  AnkiConditionalErrorAndReturnValue(
      whichScale >= 0 && whichScale < this->numPyramidLevels,
      RESULT_FAIL_INVALID_PARAMETER,
      "LucasKanadeTracker_SampledProjective::IterativelyRefineTrack",
      "whichScale is invalid");

  AnkiConditionalErrorAndReturnValue(
      convergenceTolerance > 0.0f, RESULT_FAIL_INVALID_PARAMETER,
      "LucasKanadeTracker_SampledProjective::IterativelyRefineTrack",
      "convergenceTolerance must be greater than zero");

  AnkiConditionalErrorAndReturnValue(
      nextImageHeight == templateImageHeight &&
          nextImageWidth == templateImageWidth,
      RESULT_FAIL_INVALID_SIZE,
      "LucasKanadeTracker_SampledProjective::IterativelyRefineTrack",
      "nextImage must be the same size as the template");

  const s32 initialImageScaleS32 = baseImageWidth / nextImageWidth;
  const s32 initialImagePowerS32 =
      Log2u32(static_cast<u32>(initialImageScaleS32));

  AnkiConditionalErrorAndReturnValue(
      ((1 << initialImagePowerS32) * nextImageWidth) == baseImageWidth,
      RESULT_FAIL_INVALID_SIZE,
      "LucasKanadeTracker_SampledProjective::IterativelyRefineTrack",
      "The templateImage must be a power of two smaller than baseImageWidth "
      "(%d)",
      baseImageWidth);

  if (curTransformType == Transformations::TRANSFORM_TRANSLATION) {
    return IterativelyRefineTrack_Translation(nextImage, maxIterations,
                                              whichScale, convergenceTolerance,
                                              verify_converged, scratch);
  } else if (curTransformType == Transformations::TRANSFORM_AFFINE) {
    return IterativelyRefineTrack_Affine(nextImage, maxIterations, whichScale,
                                         convergenceTolerance, verify_converged,
                                         scratch);
  } else if (curTransformType == Transformations::TRANSFORM_PROJECTIVE) {
    return IterativelyRefineTrack_Projective(nextImage, maxIterations,
                                             whichScale, convergenceTolerance,
                                             verify_converged, scratch);
  }

  return RESULT_FAIL;
}  // Result LucasKanadeTracker_SampledProjective::IterativelyRefineTrack(const
   // Array<u8> &nextImage, const s32 maxIterations, const s32 whichScale, const
   // f32 convergenceTolerance, const TransformType curTransformType, bool
   // &verify_converged, MemoryStack scratch)

Result LucasKanadeTracker_SampledProjective::IterativelyRefineTrack_Translation(
    const Array<u8> &nextImage, const s32 maxIterations, const s32 whichScale,
    const f32 convergenceTolerance, bool &verify_converged,
    MemoryStack scratch) {
  ...
  FixedLengthList<Quadrilateral<f32> > previousCorners(
      NUM_PREVIOUS_QUADS_TO_COMPARE, scratch);

  for (s32 i = 0; i < NUM_PREVIOUS_QUADS_TO_COMPARE; i++) {
    previousCorners[i] = Quadrilateral<f32>(
        Point<f32>(-1e10f, -1e10f), Point<f32>(-1e10f, -1e10f),
        Point<f32>(-1e10f, -1e10f), Point<f32>(-1e10f, -1e10f));
  }
  ...
  const TemplateSample *restrict pTemplateSamplePyramid =
      this->templateSamplePyramid[whichScale].Pointer(0);
  ...
  for (s32 iteration = 0; iteration < maxIterations; iteration++) {
    ...
    const f32 h21 =
        homography[2][1] * initialImageScaleF32;  // const f32 h22 = 1.0f;
    ...
    for (s32 iSample = 0; iSample < numTemplateSamples; iSample++) {
      ...
      // This block is the non-interpolation part of the per-sample algorithm
      {
        ...
        const f32 tGradientValue =
            oneOverTwoFiftyFive * (interpolatedPixelF32 - templatePixelValue);
    ...
    if (numInBounds < 16) {
      AnkiWarn(
          "LucasKanadeTracker_SampledProjective::IterativelyRefineTrack_"
          "Translation",
          "Template drifted too far out of image.");
      return RESULT_OK;
    }
    ...
    if ((lastResult = Matrix::SolveLeastSquaresWithCholesky(
             AWAt, b, false, numericalFailure)) != RESULT_OK)
      return lastResult;
    ...
    this->transformation.Update(b, initialImageScaleF32, scratch,
                                Transformations::TRANSFORM_TRANSLATION);

    // Check if we're done with iterations
    const f32 minChange =
        UpdatePreviousCorners(transformation, baseImageWidth, baseImageHeight,
                              previousCorners, scratch);
  ...
  return RESULT_OK;
   ...
   // LucasKanadeTracker_SampledProjective::IterativelyRefineTrack_Translation()
  ...
  for (s32 iteration = 0; iteration < maxIterations; iteration++) {
    ...
    for (s32 iSample = 0; iSample < numTemplateSamples; iSample++) {
      ...
      // This block is the non-interpolation part of the per-sample algorithm
      {
        ...
        const f32 values[6] = {xOriginal * xGradientValue,
    ...
    if (numInBounds < 16) {
      AnkiWarn(
          "LucasKanadeTracker_SampledProjective::IterativelyRefineTrack_Affine",
          "Template drifted too far out of image.");
      return RESULT_OK;
    }
...
Result LucasKanadeTracker_SampledProjective::IterativelyRefineTrack_Projective(
  ...
  for (s32 iteration = 0; iteration < maxIterations; iteration++) {
    ...
    for (s32 iSample = 0; iSample < numTemplateSamples; iSample++) {
      ...
      // This block is the non-interpolation part of the per-sample algorithm
      {
        ...
        const f32 values[8] = {xOriginal * xGradientValue,
                               yOriginal * xGradientValue,
                               xGradientValue,
                               xOriginal * yGradientValue,
                               yOriginal * yGradientValue,
                               yGradientValue,
                               -xOriginal * xOriginal * xGradientValue -
                                   xOriginal * yOriginal * yGradientValue,
                               -xOriginal * yOriginal * xGradientValue -
                                   yOriginal * yOriginal * yGradientValue};
  ...
  return RESULT_OK;
   ...
   // LucasKanadeTracker_SampledProjective::IterativelyRefineTrack_Projective()

Result LucasKanadeTracker_SampledProjective::VerifyTrack_Projective(
    const Array<u8> &nextImage, const u8 verify_maxPixelDifference,
    s32 &verify_meanAbsoluteDifference, s32 &verify_numInBounds,
    s32 &verify_numSimilarPixels, MemoryStack scratch) const {
  // This method is heavily based on Interp2_Projective
  // The call would be like: Interp2_Projective<u8,u8>(nextImage,
  // originalCoordinates, interpolationHomography, centerOffset,
  // nextImageTransformed2d, INTERPOLATE_LINEAR, 0);
...
Result LucasKanadeTracker_SampledProjective::ApproximateSelect(
    const Array<f32> &magnitudeVector, const s32 numBins, const s32 numToSelect,
    s32 &numSelected, Array<u16> &magnitudeIndexes) {
  ...
  for (f32 threshold = 0; threshold < maxMagnitude;
       threshold += magnitudeIncrement) {
    s32 numAbove = 0;
  ...
  if (foundThreshold < -0.1f) {
    AnkiWarn("LucasKanadeTracker_SampledProjective::ApproximateSelect",
