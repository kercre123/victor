...
class NearestNeighborLibrary {
 public:
  ...
  NearestNeighborLibrary(const std::string& dataPath, const s32 numDataPoints,
                         const s32 dataDim, const s16* probeCenters_X,
  ...
  NearestNeighborLibrary(const u8* data, const u8* weights, const u16* labels,
  ...
  Result GetNearestNeighbor(const Array<u8>& image,
                            ...
                            const s32 distThreshold, s32& label, s32& distance);
