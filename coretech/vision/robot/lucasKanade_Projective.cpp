...
LucasKanadeTracker_Projective::LucasKanadeTracker_Projective(
    ...
    : LucasKanadeTracker_Fast(
          Transformations::TRANSFORM_PROJECTIVE, templateImage, templateQuad,
          scaleTemplateRegionPercent, numPyramidLevels, transformType, memory) {
  this->isValid = true;
}
...
Result LucasKanadeTracker_Projective::IterativelyRefineTrack(
  ...
  AnkiConditionalErrorAndReturnValue(
      ...
      "LucasKanadeTracker_Projective::IterativelyRefineTrack",
  ...
  return RESULT_FAIL;
}  // Result LucasKanadeTracker_Projective::IterativelyRefineTrack(const
...
Result LucasKanadeTracker_Projective::IterativelyRefineTrack_Translation(
  ...
  for (s32 iteration = 0; iteration < maxIterations; iteration++) {
    ...
    if (numInBounds < 16) {
      AnkiWarn(
          "LucasKanadeTracker_Projective::IterativelyRefineTrack_Translation",
...
Result LucasKanadeTracker_Projective::IterativelyRefineTrack_Affine(
  ...
  for (s32 iteration = 0; iteration < maxIterations; iteration++) {
    ...
    if (numInBounds < 16) {
      AnkiWarn("LucasKanadeTracker_Projective::IterativelyRefineTrack_Affine",
               "Template drifted too far out of image.");
      return RESULT_OK;
    }
...
Result LucasKanadeTracker_Projective::IterativelyRefineTrack_Projective(
  ...
  for (s32 iteration = 0; iteration < maxIterations; iteration++) {
    ...
    if (numInBounds < 16) {
      AnkiWarn(
          "LucasKanadeTracker_Projective::IterativelyRefineTrack_Projective",
          "Template drifted too far out of image.");
      return RESULT_OK;
    }
