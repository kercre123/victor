...
NO_INLINE static void DetectBlurredEdges_GrayvalueThreshold_Horizontal(
    const Array<u8> &image, const Rectangle<s32> &imageRegionOfInterest,
    const u8 grayvalueThreshold, const s32 minComponentWidth,
    const s32 everyNLines, EdgeLists &edgeLists);
NO_INLINE static void DetectBlurredEdges_GrayvalueThreshold_Vertical(
    const Array<u8> &image, const Rectangle<s32> &imageRegionOfInterest,
    const u8 grayvalueThreshold, const s32 minComponentWidth,
    const s32 everyNLines, EdgeLists &edgeLists);

NO_INLINE static void DetectBlurredEdges_DerivativeThreshold_Horizontal(
    const Array<u8> &image, const Rectangle<s32> &imageRegionOfInterest,
    const s32 combHalfWidth, const s32 combResponseThreshold,
    const s32 everyNLines, EdgeLists &edgeLists);
NO_INLINE static void DetectBlurredEdges_DerivativeThreshold_Vertical(
    const Array<u8> &image, const Rectangle<s32> &imageRegionOfInterest,
    const s32 combHalfWidth, const s32 combResponseThreshold,
    const s32 everyNLines, EdgeLists &edgeLists);

Result DetectBlurredEdges_GrayvalueThreshold(const Array<u8> &image,
                                             const u8 grayvalueThreshold,
                                             const s32 minComponentWidth,
                                             const s32 everyNLines,
                                             EdgeLists &edgeLists) {
  Rectangle<s32> imageRegionOfInterest(0, image.get_size(1), 0,
                                       image.get_size(0));

  return DetectBlurredEdges_GrayvalueThreshold(
      image, imageRegionOfInterest, grayvalueThreshold, minComponentWidth,
      everyNLines, edgeLists);
}
...
Result DetectBlurredEdges_GrayvalueThreshold(
  ...
  AnkiConditionalErrorAndReturnValue(
      AreValid(image, edgeLists.xDecreasing, edgeLists.xIncreasing,
               edgeLists.yDecreasing, edgeLists.yIncreasing),
      RESULT_FAIL_INVALID_OBJECT, "DetectBlurredEdges", "Invalid objects");

  AnkiConditionalErrorAndReturnValue(
      minComponentWidth > 0, RESULT_FAIL_INVALID_SIZE, "DetectBlurredEdges",
      "minComponentWidth is too small");

  AnkiConditionalErrorAndReturnValue(
      edgeLists.xDecreasing.get_maximumSize() ==
              edgeLists.xIncreasing.get_maximumSize() &&
          edgeLists.xDecreasing.get_maximumSize() ==
              edgeLists.yDecreasing.get_maximumSize() &&
          edgeLists.xDecreasing.get_maximumSize() ==
              edgeLists.yIncreasing.get_maximumSize(),
      RESULT_FAIL_INVALID_SIZE, "DetectBlurredEdges",
      "All edgeLists must have the same maximum size");
  ...
  // TODO: won't detect an edge on the last horizontal (for x search) or
  // vertical (for y search)
...
Result DetectBlurredEdges_DerivativeThreshold(const Array<u8> &image,
                                              const s32 combHalfWidth,
                                              const s32 combResponseThreshold,
                                              const s32 everyNLines,
                                              EdgeLists &edgeLists) {
  Rectangle<s32> imageRegionOfInterest(0, image.get_size(1), 0,
                                       image.get_size(0));

  return DetectBlurredEdges_DerivativeThreshold(
      image, imageRegionOfInterest, combHalfWidth, combResponseThreshold,
      everyNLines, edgeLists);
...
Result DetectBlurredEdges_DerivativeThreshold(
  ...
  AnkiConditionalErrorAndReturnValue(
      combHalfWidth > 0 && combResponseThreshold >= 0, RESULT_FAIL_INVALID_SIZE,
      "DetectBlurredEdges",
      "combHalfWidth or combResponseThreshold are too small");
...
NO_INLINE static void DetectBlurredEdges_GrayvalueThreshold_Horizontal(
  ...
  u32 *restrict pXDecreasing =
      reinterpret_cast<u32 *>(edgeLists.xDecreasing.Pointer(0));
  u32 *restrict pXIncreasing =
      reinterpret_cast<u32 *>(edgeLists.xIncreasing.Pointer(0));

  for (s32 y = imageRegionOfInterest.top; y < imageRegionOfInterest.bottom;
       y += everyNLines) {
    ...
    while (x < imageRegionOfInterest.right) {
      if (onWhite) {
        ...
        while (x < (imageRegionOfInterest.right - 3)) {
          ...
          if ((pixels_u8x4 & 0xFF) <= grayvalueThreshold) break;

          if (((pixels_u8x4 & 0xFF00) >> 8) <= grayvalueThreshold) break;

          if (((pixels_u8x4 & 0xFF0000) >> 16) <= grayvalueThreshold) break;

          if (((pixels_u8x4 & 0xFF000000) >> 24) <= grayvalueThreshold) break;

          x += 4;
        }

        while ((x < imageRegionOfInterest.right) &&
               (pImage[x] > grayvalueThreshold)) {
          x++;
        }
        ...
        if (x < (imageRegionOfInterest.right - 1)) {
          ...
          if (componentWidth >= minComponentWidth) {
            ...
            if (xDecreasingSize < xMaxSizeM1) {
              const u32 newPoint =
                  (y << 16) + x;  // Set x and y in one operation
...
NO_INLINE static void DetectBlurredEdges_GrayvalueThreshold_Vertical(
  ...
  u32 *restrict pYDecreasing =
      reinterpret_cast<u32 *>(edgeLists.yDecreasing.Pointer(0));
  u32 *restrict pYIncreasing =
      reinterpret_cast<u32 *>(edgeLists.yIncreasing.Pointer(0));

  for (s32 x = imageRegionOfInterest.left; x < imageRegionOfInterest.right;
       x += everyNLines) {
    ...
    while (y < imageRegionOfInterest.bottom) {
      if (onWhite) {
        ...
        while (y < (imageRegionOfInterest.bottom - 1)) {
          ...
          if (pixel0 <= grayvalueThreshold) break;

          if (pixel1 <= grayvalueThreshold) break;
...
NO_INLINE static void DetectBlurredEdges_DerivativeThreshold_Horizontal(
      ...
      if (onEdge) {
        if (absDiff < combResponseThreshold) {
          ...
          if (totalDiff > 0) {
            if (xIncreasingSize < xMaxSizeM1) {
              const u32 newPoint =
                  (y << 16) + middle;  // Set x and y in one operation
              pXIncreasing[xIncreasingSize] = newPoint;
  ...
  }  // for(s32 y=imageRegionOfInterest.top; y<(imageRegionOfInterest.bottom-1);
     // y+=everyNLines)
...
NO_INLINE static void DetectBlurredEdges_DerivativeThreshold_Vertical(
  ...
  for (s32 x = xStart; x < xEnd; x += everyNLines) {
    ...
    s32 y = yStart;
    while (y < yEnd) {
      const s32 diff = pImage[imageStride * combHalfWidth] -
                       pImage[-imageStride * combHalfWidth];
      const s32 absDiff = ABS(diff);

      if (onEdge) {
        if (absDiff < combResponseThreshold) {
          const s32 middle = (componentStart + y - 1) >> 1;
          const s32 totalDiff = pImage_y0[imageStride * (y - 1)] -
                                pImage_y0[imageStride * (componentStart)];

          if (totalDiff > 0) {
            if (yIncreasingSize < yMaxSizeM1) {
              const u32 newPoint =
                  (middle << 16) + x;  // Set x and y in one operation
              pYIncreasing[yIncreasingSize] = newPoint;
...
Result EdgeLists::Serialize(const char *objectName,
  ...
  if (SerializedBuffer::SerializeDescriptionStrings(
          "EdgeLists", objectName, &segment, totalDataLength) != RESULT_OK)
    return RESULT_FAIL;

  // Serialize the template lists
  SerializedBuffer::SerializeRawBasicType<s32>("imageHeight", this->imageHeight,
                                               &segment, totalDataLength);
  SerializedBuffer::SerializeRawBasicType<s32>("imageWidth", this->imageWidth,
                                               &segment, totalDataLength);
  SerializedBuffer::SerializeRawFixedLengthList<Point<s16> >(
      "xDecreasing", this->xDecreasing, &segment, totalDataLength);
  SerializedBuffer::SerializeRawFixedLengthList<Point<s16> >(
      "xIncreasing", this->xIncreasing, &segment, totalDataLength);
  SerializedBuffer::SerializeRawFixedLengthList<Point<s16> >(
      "yDecreasing", this->yDecreasing, &segment, totalDataLength);
  SerializedBuffer::SerializeRawFixedLengthList<Point<s16> >(
      "yIncreasing", this->yIncreasing, &segment, totalDataLength);

  return RESULT_OK;
}

Result EdgeLists::Deserialize(char *objectName, void **buffer,
  ...
  this->imageHeight = SerializedBuffer::DeserializeRawBasicType<s32>(
      NULL, buffer, bufferLength);
  if (bufferLength <= 0) return RESULT_FAIL;

  this->imageWidth = SerializedBuffer::DeserializeRawBasicType<s32>(
      NULL, buffer, bufferLength);
  if (bufferLength <= 0) return RESULT_FAIL;
...
s32 EdgeLists::get_serializationSize() const {
  ...
  const s32 requiredBytes =
      ...
      14 * SerializedBuffer::DESCRIPTION_STRING_LENGTH;
...
cv::Mat EdgeLists::DrawIndexes(const f32 scale) const {
  AnkiConditionalErrorAndReturnValue(
      this->imageHeight > 16 && this->imageHeight < 2000 &&
          this->imageWidth > 16 && this->imageWidth < 2000,
      cv::Mat(), "EdgeLists::DrawIndexes", "This object is invalid");

  return DrawIndexes(this->imageHeight, this->imageWidth, this->xDecreasing,
                     this->xIncreasing, this->yDecreasing, this->yIncreasing,
                     scale);
}

cv::Mat EdgeLists::DrawIndexes(const s32 imageHeight, const s32 imageWidth,
  ...
  AnkiConditionalErrorAndReturnValue(
      imageHeight > 0 && imageHeight < 2000 && imageWidth > 0 &&
          imageWidth < 2000 &&
  ...
  if (!FLT_NEAR(scale, 1.0f)) {
    cv::Mat totalImageLarge(Round<s32>(imageHeight * scale),
                            Round<s32>(imageWidth * scale), CV_8UC3);
    cv::resize(totalImage, totalImageLarge, totalImageLarge.size(), 0, 0,
               cv::INTER_NEAREST);
...
u8 ComputeGrayvalueThreshold(const IntegerCounts &integerCounts,
  ...
  const u8 grayvalueThreshold =
      static_cast<u8>((grayvalueBlack + grayvalueWhite) / 2);

  return grayvalueThreshold;
}

u8 ComputeGrayvalueThreshold(const Array<u8> &image,
                             const Rectangle<s32> &imageRegionOfInterest,
                             const s32 yIncrement, const s32 xIncrement,
                             const f32 blackPercentile,
                             const f32 whitePercentile, MemoryStack scratch) {
  IntegerCounts integerCounts(image, imageRegionOfInterest, yIncrement,
                              xIncrement, scratch);

  return ComputeGrayvalueThreshold(integerCounts, blackPercentile,
                                   whitePercentile);
}
}  // namespace Embedded
}  // namespace Anki
