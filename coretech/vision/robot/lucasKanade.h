...
f32 UpdatePreviousCorners(
    ...
    FixedLengthList<Quadrilateral<f32> > &previousCorners, MemoryStack scratch);

class LucasKanadeTracker_Slow {
  // The generic LucasKanadeTracker class can track a template with the
  // Lucas-Kanade method, either with translation-only, affine, or projective
  // updates. The two main steps are initialization and update.
  //
  // NOTE:
 ...
 public:
  ...
  LucasKanadeTracker_Slow(
      const Array<u8> &templateImage, const Quadrilateral<f32> &templateRegion,
      const f32
  ...
  Result UpdateTrack(const Array<u8> &nextImage, const s32 maxIterations,
                     const f32 convergenceTolerance, const bool useWeights,
 ...
 protected:
  ...
  // The templateImage sizes are the sizes of the image that contains the
  // template
  s32 templateImageHeight;
  s32 templateImageWidth;

  // The templateRegion sizes are the sizes of the part of the template image
  // that will actually be tracked, so must be smaller or equal to the
  // templateImage sizes NOTE: These are in the downsampled template coordinates
  ...
  //
  // Allocated some permanant structures using memory, as well as some temporary
  // structures. As a result, it should only be called once.
  Result InitializeTemplate(const Array<u8> &templateImage,
                            MemoryStack &memory);
  ...
  Result IterativelyRefineTrack(
      ...
      const bool useWeights, bool &converged, MemoryStack scratch);
};  // class LucasKanadeTracker_Slow

// LucasKanadeTracker_Generic handles the transformation-only parts of an LK
// tracker
class LucasKanadeTracker_Generic {
 ...
 protected:
  ...
  // TODO: Probably don't need to support differing detection/tracking
  // resolutions anymore
  u16 baseImageWidth;
  u16 baseImageHeight;
...
// LucasKanadeTracker_Fast handles the basic memory of the Fast Affine and
// Projective trackers (but not the sampled)
class LucasKanadeTracker_Fast : public LucasKanadeTracker_Generic {
 public:
  ...
  Result VerifyTrack_Projective(const Array<u8> &nextImage,
...
class LucasKanadeTracker_Affine : public LucasKanadeTracker_Fast {
  // An Translation-only or Affine-plus-translation LucasKanadeTracker. Unlike
  // the general LucasKanadeTracker, this version uses much less memory, and
 ...
 protected:
  Result IterativelyRefineTrack(
      const Array<u8> &nextImage, const s32 maxIterations, const s32 whichScale,
      const f32 convergenceTolerance,
      const Transformations::TransformType curTransformType, bool &converged,
      MemoryStack scratch);

  Result IterativelyRefineTrack_Translation(
      const Array<u8> &nextImage, const s32 maxIterations, const s32 whichScale,
      const f32 convergenceTolerance, bool &converged, MemoryStack scratch);
  Result IterativelyRefineTrack_Affine(const Array<u8> &nextImage,
                                       const s32 maxIterations,
                                       const s32 whichScale,
                                       const f32 convergenceTolerance,
                                       bool &converged, MemoryStack scratch);
};  // class LucasKanadeTracker_Affine

class LucasKanadeTracker_Projective : public LucasKanadeTracker_Fast {
  // A Projective-plus-translation LucasKanadeTracker. Unlike the general
  // LucasKanadeTracker, this version uses much less memory, and could be better
  // optimized.
...
class LucasKanadeTracker_SampledProjective : public LucasKanadeTracker_Generic {
 ...
 public:
  ...
  Result ShowTemplate(const char *windowName = "SampledProjective Template",
 ...
 protected:
  ...
  static Result ApproximateSelect(const Array<f32> &magnitudeVector,
                                  const s32 numBins, const s32 numToSelect,
                                  s32 &numSelected,
...
class LucasKanadeTracker_SampledPlanar6dof : public LucasKanadeTracker_Generic {
 ...
 public:
  ...
  LucasKanadeTracker_SampledPlanar6dof(
      ...
      const f32 focalLength_x,  // TODO: move all this to CTI-Vision-Basestation
                                // and use take in CameraCalibration
      const f32 focalLength_y, const f32 camCenter_x, const f32 camCenter_y,
      const std::vector<f32> &distortionCoeffs,
      const Point<f32>
          &templateSize_mm,  // actual physical size of the template
  ...
  Result UpdateTrack(
      ...
      const u8 verify_maxPixelDifference, bool &verify_converged,
      s32 &verify_meanAbsoluteDifference,  //< For all pixels in the template,
                                           //compute the mean difference between
                                           //the template and the final warped
                                           //template
  ...
  Result UpdateRotationAndTranslation(const Array<f32> &R, const Point3<f32> &T,
                                      MemoryStack scratch);
  ...
  void SetGainScheduling(const f32 zMin, const f32 zMax, const f32 KpMin,
                         const f32 KpMax);

  bool IsValid() const;

  Result ShowTemplate(const char *windowName = "SampledPlanar6dof Template",
                      const bool waitForKeypress = false,
                      const bool fitImageToWindow = false) const;
  ...
  const FixedLengthList<TemplateSample> &get_templateSamples(
 ...
 protected:
  ...
  Result IterativelyRefineTrack_Projective(
      const Array<u8> &nextImage, const s32 maxIterations, const s32 whichScale,
      const f32 convergenceTolerance_angle,
      const f32 convergenceTolerance_distance, bool &converged,
      MemoryStack scratch);
  ...
  static Result ApproximateSelect(const Array<f32> &magnitudeImage,
                                  const s32 numBins, const s32 numRegions,
                                  const s32 numToSelect, s32 &numSelected,
                                  Array<s32> &magnitudeIndexes);
...
Result CalcOpticalFlowPyrLK(
    const FixedLengthList<Array<u8> >
        &prevPyramid,  //< Use ImageProcessing::BuildPyramid() to create
    const FixedLengthList<Array<u8> >
        &nextPyramid,  //< Use ImageProcessing::BuildPyramid() to create
    const FixedLengthList<Point<f32> >
        &prevPoints,  //< What are the point locations in the prev image? Use
                      //Features::GoodFeaturesToTrack() to detect some good
                      //points.
    FixedLengthList<Point<f32> >
        &nextPoints,  //< What are the point locations in the next image? Also
                      //see parameter "usePreviousFlowAsInit".
    FixedLengthList<bool> &status,  //< Was each point tracked?
    FixedLengthList<f32> &err,  //< What is the difference between the template
                                //in the previous image and the next image?
    ...
    const f32 termination_epsilon,   //< Min movement for a point to count as
                                     //convergence
    const f32 minEigThreshold,  //< If the quality of the pont in the previous
                                //image is below this, it is not tracked.
    const bool
        usePreviousFlowAsInit,  //< If usePreviousFlowAsInit == true, these are
                                //the initial guesses. If usePreviousFlowAsInit
                                //== false, prevPoints is used as the initial
                                //guesses.
