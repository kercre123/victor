...
Result DistanceTransform(const Array<u8> image, const u8 backgroundThreshold,
  ...
  // const u32 backgroundThresholdX2 = static_cast<u32>(backgroundThreshold) |
  // (static_cast<u32>(backgroundThreshold) << 16); const u32 aX2 =
  // static_cast<u32>(a) | (static_cast<u32>(a) << 16); const u32 bX2 =
  // static_cast<u32>(b) | (static_cast<u32>(b) << 16);

  // To prevent overflow, maxDistance is a bit less than
  // std::numeric_limits<s16>::max()
  const u16 maxDistance =
      std::numeric_limits<s16>::max() -
      (MAX(a, b) * (imageHeight + imageWidth) * (1 << numFractionalBits));

  AnkiConditionalErrorAndReturnValue(AreValid(image, distance),
                                     RESULT_FAIL_INVALID_OBJECT,
                                     "DistanceTransform", "Invalid objects");

  AnkiConditionalErrorAndReturnValue(
      AreEqualSize(image, distance), RESULT_FAIL_INVALID_SIZE,
      "DistanceTransform", "image and distance must be the same size");

  AnkiConditionalErrorAndReturnValue(
      numFractionalBits >= 0 && numFractionalBits < 15,
      RESULT_FAIL_INVALID_PARAMETER, "DistanceTransform",
      "numFractionalBits is out of range");
  ...
  // Compute the non left-top pixels (forward pass)
  {
    for (s32 y = 1; y < imageHeight; y++) {
      ...
      const u16 *restrict pDistance_ym1 =
          reinterpret_cast<u16 *>(distance.Pointer(y - 1, 0));
      u16 *restrict pDistance_y0 =
          reinterpret_cast<u16 *>(distance.Pointer(y, 0));
      ...
      // TODO: If all of the image pixels are background, set zero without doing
      // the computation

      const u16 backgroundThreshold_0 = static_cast<u16>(backgroundThreshold);
      const u16 backgroundThreshold_right8 =
          static_cast<u16>(backgroundThreshold) << 8;
      ...
      for (s32 x = 2; x < imageWidth - 1; x += 2) {
        ...
        //              const u32 leftUp10  = *reinterpret_cast<const
        //              u32*>(pDistance_ym1 + x - 1) + bX2; const u32 up10 =
        //              *reinterpret_cast<const u32*>(pDistance_ym1 + x    ) +
        //              aX2; const u32 rightUp10 = *reinterpret_cast<const
        //              u32*>(pDistance_ym1 + x + 1) + bX2;
        //
  ...
  {
    u16 *restrict pDistance =
        reinterpret_cast<u16 *>(distance.Pointer(imageHeight - 1, 0));
    ...
    for (s32 y = imageHeight - 2; y >= 0; y--) {
      *distance.Pointer(y, imageWidth - 1) =
          MIN(*distance.Pointer(y, imageWidth - 1),
              *distance.Pointer(y + 1, imageWidth - 1) + a);
    }
  }  // Compute the right and bottom image edges (backward pass)
  ...
  {
    for (s32 y = imageHeight - 2; y >= 0; y--) {
      const u16 *restrict pDistance_yp1 =
          reinterpret_cast<u16 *>(distance.Pointer(y + 1, 0));
      u16 *restrict pDistance_y0 =
          reinterpret_cast<u16 *>(distance.Pointer(y, 0));
      ...
      for (s32 x = imageWidth - 2; x > 0; x--) {
        ...
        if (center != 0) {
          ...
          pDistance_y0[x] =
              MIN(MIN(MIN(MIN(right, rightDown), down), leftDown), center);
        }
      }
