...
void SolveQuarticTestHelper(std::array<PRECISION, 4>& roots_computed,
  ...
  const std::array<PRECISION, 4> roots_groundTruth = {
      {0.334683441970975, 0.006699578943935, -0.136720934135068,
       -0.213857711381642}};
...
GTEST_TEST(PoseEstimation, FromQuads) {
  ...
  const Quad2f projNoise(Point2f(0.1740f, 0.0116f), Point2f(0.0041f, 0.0073f),
                         Point2f(0.0381f, 0.1436f), Point2f(0.2249f, 0.0851f));
  /*
  ...
  */
  ...
  // Create the 3D marker and put it in the specified pose relative to the
  // camera
  Quad3f marker3d(Point3f(-1.f, -1.f, 0.f), Point3f(-1.f, 1.f, 0.f),
  ...
  auto calib = std::make_shared<Vision::CameraCalibration>(
      camNumRows, camNumCols, focalLength_x, focalLength_y, camCenter_x,
      camCenter_y,
      0.f);  // skew
  ...
  Vec3f Tdiff;
  Radians angleDiff;
  EXPECT_TRUE(poseEst.IsSameAs(poseTrue, distThreshold, angleThreshold, Tdiff,
                               angleDiff));

  printf("Angular difference is %f degrees (threshold = %f degrees)\n",
         angleDiff.getDegrees(), angleThreshold.getDegrees());
...
GTEST_TEST(Camera, VisibilityAndOcclusion) {
  ...
  const auto calib = std::make_shared<Vision::CameraCalibration>(
      240, 320, 300.f, 300.f, 160.f, 120.f);
  ...
  EXPECT_TRUE(object1.IsVisibleFrom(camera, DEG_TO_RAD(5), 5.f,
                                    DoNotRequireObjectBehind));
  ...
  EXPECT_FALSE(
      object1.IsVisibleFrom(camera, DEG_TO_RAD(5), 5.f, RequireObjectBehind));
  ...
  EXPECT_FALSE(
      object2.IsVisibleFrom(camera, DEG_TO_RAD(5), 5.f, RequireObjectBehind));
  EXPECT_FALSE(object2.IsVisibleFrom(camera, DEG_TO_RAD(5), 5.f,
                                     DoNotRequireObjectBehind));
  ...
  EXPECT_TRUE(object2.IsVisibleFrom(camera, DEG_TO_RAD(5), 5.f,
                                    DoNotRequireObjectBehind));
  EXPECT_FALSE(
      object2.IsVisibleFrom(camera, DEG_TO_RAD(5), 5.f, RequireObjectBehind));
  ...
  EXPECT_FALSE(object1.IsVisibleFrom(camera, DEG_TO_RAD(5), 5.f,
                                     DoNotRequireObjectBehind));
  ...
  object1.SetPose(
      Pose3d(3 * M_PI_2, X_AXIS_3D(), {0.f, 0.f, 1000.f}, poseOrigin));
  EXPECT_FALSE(object1.IsVisibleFrom(camera, DEG_TO_RAD(5), 5.f,
                                     DoNotRequireObjectBehind));
  ...
  object1.SetPose(
      Pose3d(M_PI_2, X_AXIS_3D(), {100.f, 0.f, -100.f}, poseOrigin));
  EXPECT_FALSE(object1.IsVisibleFrom(camera, DEG_TO_RAD(5), 5.f,
                                     DoNotRequireObjectBehind));
...
GTEST_TEST(ImageRGB, NormalizedColor) {
  ...
  for (s32 i = 0; i < img.GetNumRows(); ++i) {
    ...
    for (s32 j = 0; j < img.GetNumCols(); ++j) {
      ...
      const PixelRGB truth(
          sum == 0 ? 0 : Util::numeric_cast<u8>(((s32)p.r() * 255) / sum),
      ...
      //       p.r(), p.g(), p.b(), pNorm.r(), pNorm.g(), pNorm.b(), truth.r(),
      //       truth.g(), truth.b());

      // Note: rounding error depending on order of operations can yield +/-1
      // variation
...
GTEST_TEST(ResizeImage, CorrectSizes) {
  ...
  // Resize in place with keep aspect ratio and onlySmaller=true should not
  // change origImg
  origImg.ResizeKeepAspectRatio(30, 25, Vision::ResizeMethod::NearestNeighbor,
                                true);
  EXPECT_EQ(10, origImg.GetNumRows());
  EXPECT_EQ(20, origImg.GetNumCols());

  // Resize in place with keep aspect ratio and onlySmaller=false should yield
  // 13x25
  origImg.ResizeKeepAspectRatio(30, 25, Vision::ResizeMethod::NearestNeighbor,
                                false);
