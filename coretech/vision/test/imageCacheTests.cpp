...
GTEST_TEST(ImageCache, CachedGetters)
{
  ...
  const std::shared_ptr<const Image> getResult = cache.GetGray(ImageCacheSize::Full, &getType);
  ...
  const std::shared_ptr<const Image> halfSize = cache.GetGray(ImageCacheSize::Half, &getType);
  ...
  const std::shared_ptr<const Image> halfSize2 = cache.GetGray(ImageCacheSize::Half, &getType);
  ASSERT_EQ(halfSize.get(), halfSize2.get());
  ASSERT_EQ(ImageCache::GetType::FullyCached, getType);

  // Get full-scale color version (should require computation)
  const std::shared_ptr<const ImageRGB> colorVersion = cache.GetRGB(ImageCacheSize::Full, &getType);
  ...
  const std::shared_ptr<const ImageRGB> colorVersion2 = cache.GetRGB(ImageCacheSize::Full, &getType);
  ASSERT_EQ(colorVersion.get(), colorVersion2.get());
  ASSERT_EQ(ImageCache::GetType::FullyCached, getType);

  // Request for quarter size color should create new entry
  const std::shared_ptr<const ImageRGB> qtrSizeColor = cache.GetRGB(ImageCacheSize::Quarter, &getType);
  ASSERT_EQ(ImageCache::GetType::NewEntry, getType);
  ...
  const std::shared_ptr<const Image> qtrSizeGray = cache.GetGray(ImageCacheSize::Quarter, &getType);
  ASSERT_EQ(ImageCache::GetType::ComputeFromExisting, getType);
  ...
  cache.Reset(newImg);
  const std::shared_ptr<const Image> newHalfSize = cache.GetGray(ImageCacheSize::Half, &getType);
  ...
  const std::shared_ptr<const Image> newGray = cache.GetGray(ImageCacheSize::Full, &getType);
  ASSERT_EQ(ImageCache::GetType::ComputeFromExisting, getType);
...
GTEST_TEST(ImageCache, PriorityResizeRGB)
{
  ...
  ImageCache::GetType getType;
  const std::shared_ptr<const Image> imgGray = cache.GetGray(ImageCacheSize::Half, &getType);
  ASSERT_EQ(ImageCache::GetType::NewEntry, getType);

  const std::shared_ptr<const ImageRGB> imgRGB = cache.GetRGB(ImageCacheSize::Half, &getType);
  ASSERT_EQ(ImageCache::GetType::ResizeIntoExisting, getType);

  // the grayscale image should not have been invalidated by the request for RGB
  const std::shared_ptr<const Image> imgGray2 = cache.GetGray(ImageCacheSize::Half, &getType);
  ASSERT_EQ(ImageCache::GetType::FullyCached, getType);
}
...
GTEST_TEST(ImageCache, ImageBuffer)
{
  ...
  ImageCache::GetType getType;
  const std::shared_ptr<const ImageRGB> imgRGB = cache.GetRGB(ImageCacheSize::Full, &getType);
  ...
  const std::shared_ptr<const Image> imgGray = cache.GetGray(ImageCacheSize::Full, &getType);
  ...
  const std::shared_ptr<const ImageRGB> imgRGB2 = cache.GetRGB(ImageCacheSize::Half, &getType);
  ASSERT_EQ(ImageCache::GetType::NewEntry, getType);
  ASSERT_EQ(imgRGB2->GetNumRows(),   nrows/2);
  ASSERT_EQ(imgRGB2->GetNumCols(),   ncols/2);
  ASSERT_EQ(imgRGB2->GetTimestamp(), buffer.GetTimestamp());
  ASSERT_EQ(imgRGB2->GetImageId(),   buffer.GetImageId());

  // Check a couple of pixels for the BAYER -> RGB conversion
  // when downsampling
  ASSERT_EQ(imgRGB2->get_CvMat_()[1][2],  PixelRGB(0, 0, 255));
  ASSERT_EQ(imgRGB2->get_CvMat_()[1][6],  PixelRGB(0, 255, 0));
  ASSERT_EQ(imgRGB2->get_CvMat_()[1][10], PixelRGB(255, 0, 0));
}
