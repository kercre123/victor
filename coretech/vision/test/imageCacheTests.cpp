...
GTEST_TEST(ImageCache, CachedGetters) {
  ...
  const Vision::Image& getResult =
      cache.GetGray(ImageCacheSize::Full, &getType);
  ...
  const Vision::Image& halfSize2 =
      cache.GetGray(ImageCacheSize::Half, &getType);
  ASSERT_EQ(&halfSize, &halfSize2);
  ASSERT_EQ(ImageCache::GetType::FullyCached, getType);

  // Get full-scale color version (should require computation)
  const Vision::ImageRGB& colorVersion =
      cache.GetRGB(ImageCacheSize::Full, &getType);
  ...
  const Vision::ImageRGB& colorVersion2 =
      cache.GetRGB(ImageCacheSize::Full, &getType);
  ASSERT_EQ(&colorVersion, &colorVersion2);
  ASSERT_EQ(ImageCache::GetType::FullyCached, getType);

  // Request for quarter size color should create new entry
  const Vision::ImageRGB& qtrSizeColor =
      cache.GetRGB(ImageCacheSize::Quarter, &getType);
  ASSERT_EQ(ImageCache::GetType::NewEntry, getType);
  ASSERT_EQ(nrows / 4, qtrSizeColor.GetNumRows());
  ASSERT_EQ(ncols / 4, qtrSizeColor.GetNumCols());

  // Asking for quarter size gray should compute it from the color one we just
  // created
  const Vision::Image& qtrSizeGray =
      cache.GetGray(ImageCacheSize::Quarter, &getType);
  ASSERT_EQ(ImageCache::GetType::ComputeFromExisting, getType);
  ASSERT_EQ(nrows / 4, qtrSizeGray.GetNumRows());
  ASSERT_EQ(ncols / 4, qtrSizeGray.GetNumCols());

  // Resetting with a new image and asking for half size should resize into an
  // existing entry and use same data
  Vision::Image newImg(nrows, ncols);
  cache.Reset(newImg);
  const Vision::Image& newHalfSize =
      cache.GetGray(ImageCacheSize::Half, &getType);
  ASSERT_EQ(ImageCache::GetType::ResizeIntoExisting, getType);
  ASSERT_EQ(newHalfSize.GetDataPointer(), halfSize.GetDataPointer());

  // Resetting with a new color image should mean asking for full-sized gray
  // will cause a compute
  ...
  // ReleaseMemory should clear out the cache and thus a request for an image
  // should fail
  cache.ReleaseMemory();
  ASSERT_DEATH(cache.GetGray(), "");

  // Resetting with a color image and requesting a resized version should
  // trigger a new allocation and not a resize into existing memory, since we
  // called ReleaseMemory
  cache.Reset(newColorImg);
  ASSERT_EQ(true, cache.HasColor());
...
GTEST_TEST(ImageCache, PriorityResizeRGB) {
  ...
  // We do not want to compute color data from an existing cached gray entry at
  // the same size if there's color data to resize from instead
  ImageCache::GetType getType;
...
GTEST_TEST(ImageCache, ImageBuffer) {
  ...
  const s32 id = 2;
  ImageBuffer buffer(&(bayerData[0][0]), nrows, ncols, ImageEncoding::BAYER, t,
                     id);
...
GTEST_TEST(ImageCache, ResizeMethod) {
  ...
  ResizeMethod method;
  // Should not have a resize method due to the cache not having been reset with
  // an image yet
