% function [labelName, labelID] = decisionTree2_query(probeTree, img, tform, blackValue, whiteValue)
...
% [labelName, labelID] = decisionTree2_query(probeTree, img, tform 0, 255)

% quad = [1 1; 1 512; 512 1; 512 512]; tform = cp2tform(quad, [0 0; 0 1; 1 0; 1 1], 'projective');
% for i = 1:length(files) img = imread(files{i}); [labelName, labelID] = decisionTree2_query(probeTree, img, tform, 0, 255); disp(sprintf('%s is %s', files{i}, labelName)); end
...
function [labelName, labelID] = decisionTree2_query(probeTree, img, tform, blackValue, whiteValue)
    ...
    if isfield(probeTree, 'labelName')
        ...
        if curPixel < probeTree.grayvalueThreshold
            [labelName, labelID] = decisionTree2_query(probeTree.leftChild, img, tform, blackValue, whiteValue);
        else
            [labelName, labelID] = decisionTree2_query(probeTree.rightChild, img, tform, blackValue, whiteValue);
        end
    end
