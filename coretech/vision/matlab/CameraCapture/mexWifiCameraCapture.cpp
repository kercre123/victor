...
void closeHelper(void) {
  if (parserThread != NULL) {
    ...
    // mexPrintf("NOTE: not actually deleting the parser -- was crashing matlab.
    // Investigate.\n");
    delete parserThread;
    parserThread = NULL;
  }
...
enum Command { OPEN = 0, GRAB = 1, CLOSE = 2 };
...
#define USE_SOCKET
  ...
  Command cmd =
      static_cast<Command>(static_cast<int>(floor(mxGetScalar(prhs[0]) + 0.5)));
  ...
  if (cmd == GRAB) {
    ...
    // if(capture.empty() || capture[device] == NULL || not
    // capture[device]->isOpened()) {
    if (parserThread == NULL) {
    ...
    // printf("Received %s %s\n", newObject.typeName,
    // newObject.newObject.objectName);

    // Simple example to display an image
    if (strcmp(newObject.objectName, "Robot Image") == 0) {
      Array<u8> image =
          *(reinterpret_cast<Array<u8> *>(newObject.startOfPayload));
#if USE_OPENCV_DISPLAY
