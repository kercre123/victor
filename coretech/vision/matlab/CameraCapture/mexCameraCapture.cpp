...
enum Command { OPEN = 0, GRAB = 1, CLOSE = 2, CHANGE_RESOLUTION = 3 };
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  switch (cmd) {
    ...
    case CHANGE_RESOLUTION: {
      if (nrhs < 4) {
        mexErrMsgTxt(
            "You must supply device and resolution arguments "
  ...
  if (cmd == GRAB) {
    ...
    if (capture.empty() || capture[device] == NULL ||
        not capture[device]->isOpened()) {
    ...
    if (not success) {
      ...
      mwSize dims[3] = {static_cast<mwSize>(height), static_cast<mwSize>(width),
                        3};
