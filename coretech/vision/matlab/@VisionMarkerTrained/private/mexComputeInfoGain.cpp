...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
   ...
   * markerProb_on(i_probe,:)  = max(eps, accumarray(labels(:),
  probeIsOn(i_probe,:)', [numLabels 1])');
   * markerProb_off(i_probe,:) = max(eps, accumarray(labels(:),
  probeIsOff(i_probe,:)', [numLabels 1])');
   * end
   *
   * markerProb_on  = markerProb_on  ./
  (sum(markerProb_on,2)*ones(1,numLabels));
   * markerProb_off = markerProb_off ./
  (sum(markerProb_off,2)*ones(1,numLabels));
   *
   * p_on = sum(probeIsOn,2)/size(probeIsOn,2);
   * p_off = 1 - p_on;
   *
   * conditionalEntropy = -
  (p_on.*sum(markerProb_on.*log2(max(eps,markerProb_on)), 2) + ...
  ...
  // const Precision oneOverNumExamples = 1. /
  // static_cast<Precision>(numExamples);
  ...
  if (mxGetNumberOfElements(labelsArray) != numExamples) {
    mexErrMsgTxt(
        "Number of elements in labels should match number of rows in "
        "probeValues.");
  }
  ...
  if (mxGetNumberOfElements(weightsArray) != numExamples) {
    mexErrMsgTxt(
        "Number of weights should match number of rows in probeValues.");
  }

#if USE_PERSISTENT_MEMORY
  if (numProbes != numProbesCached || numExamples != numExamplesCached ||
      numLabels != numLabelsCached) {
    ...
    markerProb_on =
        (Precision*)mxMalloc(numLabels * numProbes * sizeof(Precision));
  ...
  Precision* markerProb_on =
      (Precision*)mxMalloc(numLabels * numProbes * sizeof(Precision));

  Precision* markerProb_off =
      (Precision*)mxMalloc(numLabels * numProbes * sizeof(Precision));
  ...
  for (mwSize iProbe = 0; iProbe < numProbes; ++iProbe) {
    // const Precision * currentProbe  = (const
    // Precision*)mxGetData(probeValues) + iProbe*numExamples;
    const u8* currentProbe =
        static_cast<u8*>(mxGetData(probeValues)) + iProbe * numExamples;
    ...
    for (mwSize iExample = 0; iExample < numExamples; ++iExample) {
      ...
      const double currentProbeVal =
          static_cast<double>(currentProbe[iExample]) * oneOver255;

      // const Precision probeIsOn  = Precision(1) - currentProbe[iExample];
      // const Precision probeIsOff = (Precision)(currentProbe[iExample] >
      // Precision(0.5)); const Precision probeIsOn  =
      // (Precision)(currentProbe[iExample] <= Precision(0.5));
  ...
  for (mwSize iProbe = 0; iProbe < numProbes; ++iProbe) {
    ...
    const Precision* currentMarkerProb_off =
        markerProb_off + iProbe * numLabels;
    ...
    infoGain[iProbe] =
        (Precision(1) - p_off[iProbe]) * onTerm + p_off[iProbe] * offTerm;
  }
