...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  while (!(sendBuf[0] == 'q' && sendBuf[1] == 'u' && sendBuf[2] == 'i' &&
           sendBuf[3] == 't')) {
    std::cout << "Type message to send to server (type 'quit' to exit):\n";
    std::cin.getline(sendBuf, sizeof(sendBuf));

    int bytes_sent = client.Send(
        sendBuf, strlen(sendBuf));  // +1 to include terminating null
    if (bytes_sent < 0) {
      break;
    }
    ...
    if (bytes_received < 0) {
      ...
      cout << "Received: " << recvBuf << " (" << bytes_received << " bytes)\n";
    }
  }
