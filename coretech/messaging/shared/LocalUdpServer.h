 ...
 *
 * Current implementation is limited to ONE CLIENT PER SERVER for improved
 * performance. Profiling shows that connect()+send() to a single peer is much
 * faster than using sendto() to manage multiple peers.
 *
...
class LocalUdpServer {
 public:
  static constexpr const char kConnectionPacket[] = {'A', 'N', 'K', 'I',
                                                     'C', 'O', 'N', 'N'};
