 ...
 *              A thread is launched to handle client connections and read
 *incoming data.
 *
...
class TcpMultiClientServer {
 public:
  ...
  //       This would make Recv() effectively useless for the specified client
  //       ID.
  // int SetRecvBuf(unsigned int client_id, char* data, int *data_len,
  // std::mutex data_lock);

 private:
  // The main server thread that handles incoming connections and reads data
