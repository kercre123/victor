...
#define LOG_ERROR(name, format, ...) \
  CORETECH_LOG_ERROR(name, format, ##__VA_ARGS__)
#define LOG_WARNING(name, format, ...) \
  CORETECH_LOG_WARNING(name, format, ##__VA_ARGS__)
#define LOG_INFO(name, format, ...) \
  CORETECH_LOG_INFO(LOG_CHANNEL, name, format, ##__VA_ARGS__)
#define LOG_DEBUG(name, format, ...) \
  CORETECH_LOG_DEBUG(LOG_CHANNEL, name, format, ##__VA_ARGS__)
#else
#define LOG_ERROR(name, format, ...) \
  {}
#define LOG_WARNING(name, format, ...) \
...
UdpServer::UdpServer(const std::string &name) : _name(name), socketfd(-1) {}
...
bool UdpServer::StartListening(const unsigned short port) {
  ...
  host_info.ai_socktype =
      SOCK_DGRAM;  // Use SOCK_STREAM for TCP or SOCK_DGRAM for UDP.
  ...
  if (status != 0) {
    LOG_ERROR("UdpServer.StartListening.GetAddrInfoFailed", "%s",
              gai_strerror(status));
  ...
  if (status == -1) {
    LOG_ERROR("UdpServer.StartListening.BindError",
              "Port: %d (You might have orphaned processes running)", port);
  } else {
...
ssize_t UdpServer::Send(const char *data, size_t size) {
  ...
  for (client_list_it it = client_list.begin(); it != client_list.end(); it++) {
    ...
    bytes_sent =
        sendto(socketfd, data, size, 0, (struct sockaddr *)&(*it), sizeof(*it));
    if (bytes_sent != size) {
      LOG_ERROR("UdpServer.Send.SendFailed", "%zd of %zu bytes sent",
                bytes_sent, size);
    }
  }
...
ssize_t UdpServer::Recv(char *data, size_t maxSize) {
  ...
  ssize_t bytes_received = recvfrom(socketfd, data, maxSize, 0,
  ...
  if (bytes_received <= 0) {
    ...
    if (bytes_received == sizeof(kConnectionPacket) &&
      ...
      LOG_DEBUG("UdpServer.Recv.ReceivedConnectionPacket", "[%s]",
                _name.c_str());
      return 0;
    }
  }
...
bool UdpServer::AddClient(struct sockaddr_in &c) {
  for (client_list_it it = client_list.begin(); it != client_list.end(); it++) {
    if (memcmp(&(*it), &c, sizeof(c)) == 0) return false;
  }
...
bool UdpServer::HasClient() { return !client_list.empty(); }

void UdpServer::DisconnectClient() { client_list.clear(); }

int UdpServer::GetNumClients() {
  const ssize_t numClients = client_list.size();

  if (numClients > std::numeric_limits<int>::max()) {
    LOG_WARNING("UdpServer.GetNumClients.TooManyClients",
                "More than MAX_INT clients");
  }
