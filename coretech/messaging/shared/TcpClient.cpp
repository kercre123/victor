...
TcpClient::TcpClient() { socketfd = -1; }
...
int TcpClient::Send(const char* data, int size) {
  ...
  if (bytes_sent < 0) {
    if (errno != EWOULDBLOCK) {
      DEBUG_TCP_CLIENT("TcpClient: Send error ("
                       << errno << "), disconnecting. (bytes_sent = "
                       << bytes_sent << ")\n");
      Disconnect();
      return -1;
    }
  ...
  if (bytes_sent > std::numeric_limits<int>::max()) {
    DEBUG_TCP_CLIENT(
        "TcpClient: Send warning, num bytes sent > max integer.\n");
  }
...
int TcpClient::Recv(char* data, int maxSize) {
  ...
  if (bytes_received <= 0) {
    if (errno != EWOULDBLOCK) {
      DEBUG_TCP_CLIENT("TcpClient: Receive error (" << errno
                                                    << "), disconnecting.\n");
      Disconnect();
      return -1;
    } else {
    ...
    DEBUG_TCP_CLIENT("TcpClient: " << bytes_received
                                   << " bytes received : " << data << "\n");
  }

  if (bytes_received > std::numeric_limits<int>::max()) {
    DEBUG_TCP_CLIENT(
        "TcpClient: Receive warning, num bytes received > max integer.\n");
  }

  return static_cast<int>(bytes_received);
}

bool TcpClient::IsConnected() { return socketfd != -1; }
