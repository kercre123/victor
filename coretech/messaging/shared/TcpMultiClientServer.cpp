...
TcpMultiClientServer::~TcpMultiClientServer() { Stop(); }
...
bool TcpMultiClientServer::server_handler() {
  ...
  int status;
  struct addrinfo
      host_info;  // The struct that getaddrinfo() fills up with data.
  struct addrinfo
      *host_info_list;  // Pointer to the to the linked list of host_info's.
  ...
  // structure pointed to by hints must contain either 0 or a null pointer, as
  // appropriate." When a struct is created in c++, it will be given a block of
  // memory. This memory is not nessesary empty. Therefor we use the memset
  // function to make sure all fields are NULL.
  memset(&host_info, 0, sizeof host_info);

  host_info.ai_family = AF_UNSPEC;  // IP version not specified. Can be both.
  host_info.ai_socktype =
      SOCK_STREAM;  // Use SOCK_STREAM for TCP or SOCK_DGRAM for UDP.
  host_info.ai_flags = AI_PASSIVE;  // IP Wildcard

  // Now fill up the linked list of host_info structs with google's address
  // information.
  char portStr[8];
  ...
  DEBUG_TCP_SERVER("TcpMultiClientServer: Creating a socket on port "
                   << portStr);
  ...
  // we use to make the setsockopt() function to make sure the port is not in
  ...
  // status = setsockopt(socketfd, SOL_SOCKET, SO_KEEPALIVE, &yes, sizeof(int));
  // // For auto-detecting disconnected clients.
  set_nonblock(listenfd);
  if (status == -1) {
    DEBUG_TCP_SERVER(
        "TcpMultiClientServer: Failed to set socket options (status=" << status
                                                                      << ")");
  }
  ...
  select_timeout.tv_usec = 100000;  // Max amount of time it takes to kill the
                                    // thread after Stop() is called
  ...
  while (1) {
    ...
    if (FD_ISSET(listenfd, &rset)) {
      ...
      if (connfd == -1) {
        ...
        DEBUG_TCP_SERVER(
            "TcpMultiClientServer: Connection accepted. Using new socketfd : "
            << connfd);
    ...
    client_info_mutex.lock();
    for (client_info_it it = client_info.begin(); it != client_info.end();
         ++it) {
      if (FD_ISSET(it->second.sd, &rset)) {
        it->second.cli_mutex.lock();

        ssize_t bytes_received;
        bytes_received = recv(it->second.sd,
                              it->second.recv_data + it->second.recv_data_size,
                              MAX_RECV_BUF_SIZE - it->second.recv_data_size, 0);
        ...
        if (bytes_received <= 0) {
          ...
          DEBUG_TCP_SERVER("TcpMultiClientServer: " << bytes_received
                                                    << " bytes received");
        }
...
int TcpMultiClientServer::Send(unsigned int client_id, const char *data,
                               int size) {
  if (size <= 0) return 0;
  ...
  if (it == client_info.end()) {
    DEBUG_TCP_SERVER(
        "TcpMultiClientServer: Send() failed because no client connected");
    return -1;
  }
  ...
  if (it == client_info.end()) {
    client_info_mutex.unlock();
    DEBUG_TCP_SERVER(
        "TcpMultiClientServer: Send() failed because client disconnected while "
        "waiting for lock");
    return -1;
  }
...
int TcpMultiClientServer::Recv(unsigned int client_id, char *data,
                               int maxSize) {
  client_info_it it = client_info.find(client_id);
  if (it == client_info.end()) {
    DEBUG_TCP_SERVER(
        "TcpMultiClientServer: Recv() failed because no client connected");
    return -1;
  }
  ...
  if (it == client_info.end()) {
    client_info_mutex.unlock();
    DEBUG_TCP_SERVER(
        "TcpMultiClientServer: Recv() failed because client disconnected while "
        "waiting for lock");
    return -1;
  }
  ...
  memmove(it->second.recv_data, it->second.recv_data + dataSize,
...
void TcpMultiClientServer::DisconnectClient(unsigned int client_id) {
  ...
  if (it != client_info.end()) {
    it->second.cli_mutex.lock();
    DEBUG_TCP_SERVER("TcpMultiClientServer: Disconnecting client with id "
                     << client_id);
    close(it->second.sd);
    ...
    DEBUG_TCP_SERVER(
        "TcpMultiClientServer: Total num clients = " << client_info.size());
  }
  client_info_mutex.unlock();
}
