...
static std::string to_string(const struct sockaddr_un &saddr,
                             socklen_t saddrlen) {
  return std::string(saddr.sun_path,
                     saddrlen - (sizeof(saddr) - sizeof(saddr.sun_path)));
}
...
LocalUdpServer::LocalUdpServer()
    : LocalUdpServer(UDP_SERVER_SNDBUFSZ, UDP_SERVER_RCVBUFSZ) {}
...
bool LocalUdpServer::StartListening(const std::string &sockname) {
  ...
  LOG_DEBUG("LocalUdpServer.StartListening", "Creating a socket at %s",
            _sockname.c_str());
  ...
  if (_socket == -1) {
    LOG_ERROR("LocalUdpServer.StartListening",
              "Unable to create socket at %s (%s)", _sockname.c_str(),
              strerror(errno));
    return false;
  }

  if (!Anki::Messaging::SetNonBlocking(_socket, 1)) {
    LOG_ERROR("LocalUdpServer.StartListening", "Unable to set nonblocking (%s)",
              strerror(errno));
    close(_socket);
    _socket = -1;
    return false;
  ...
  if (!Anki::Messaging::SetReuseAddress(_socket, 1)) {
    LOG_ERROR("LocalUdpServer.StartListening",
              "Unable to set reuseaddress (%s)", strerror(errno));
    close(_socket);
    _socket = -1;
    return false;
  }
  ...
  if (!Anki::Messaging::SetRecvBufferSize(_socket, _rcvbufsz)) {
    LOG_ERROR("LocalUdpServer.StartListening",
              "Unable to set recv buffer size (%s)", strerror(errno));
    close(_socket);
    _socket = -1;
    return false;
  }
  ...
  if (status == -1) {
    LOG_ERROR("LocalUdpServer.StartListening", "Unable to bind at %s (%s)",
              _sockname.c_str(), strerror(errno));
    LOG_ERROR("LocalUdpServer.StartListening",
  ...
  LOG_DEBUG("LocalUdpServer.StartListening", "Socket %d is bound at %s",
            _socket, _sockname.c_str());
...
void LocalUdpServer::StopListening() {
  ...
  LOG_DEBUG("LocalUdpServer.StopListening",
            "Stopping server listening on socket %s (%d)", _sockname.c_str(),
            _socket);
...
ssize_t LocalUdpServer::Send(const char *data, size_t size) {
  ...
  // LOG_DEBUG("LocalUdpServer.Send", "Sending %zu bytes to %s", size,
  // _peername.c_str());
  ...
  if (bytes_sent != size) {
    // If send fails, log it and report it to caller.  It is caller's
...
ssize_t LocalUdpServer::Recv(char *data, size_t maxSize) {
  ...
  const ssize_t bytes_received =
      recvfrom(_socket, data, maxSize, 0, (struct sockaddr *)&saddr, &saddrlen);
  ...
  if (bytes_received <= 0) {
    if (errno == EWOULDBLOCK) {
      ...
      LOG_ERROR("LocalUdpServer.Recv.Fail",
                "Receive error on %s (sock: %d) (%s)", _sockname.c_str(),
                _socket, strerror(errno));
      return -1;
    }
  }

  // LOG_DEBUG("LocalUdpServer.Recv", "Received %zd bytes from %s",
  // bytes_received, to_string(saddr, saddrlen).c_str());
...
bool LocalUdpServer::AddClient(const struct sockaddr_un &saddr,
                               socklen_t saddrlen) {
  ...
  if (_bindClients) {
    ...
    if (connect(_socket, (struct sockaddr *)&saddr, saddrlen) != 0) {
      LOG_ERROR("LocalUdpServer.AddClient", "Unable to connect to %s (%s)",
...
void LocalUdpServer::Disconnect() {
  ...
  LOG_DEBUG("LocalUdpServer.Disconnect", "Disconnect %d from peer %s", _socket,
            _peername.c_str());
  ...
  if (_bindClients) {
    ...
    if (connect(_socket, &saddr, sizeof(saddr)) != 0) {
      ...
      if (errno != ENOENT) {
        LOG_ERROR("LocalUdpServer.Disconnect", "Failed to disconnect (%s)",
                  strerror(errno));
      }
    }
  }
