...
#ifdef VICOS
// todo: restore logging when vicos toolchain is available -PRA (approved by
// BRC)
#undef LOG_CHANNEL
#endif
...
LocalUdpClient::LocalUdpClient(int sndbufsz, int rcvbufsz)
    : _sndbufsz(sndbufsz), _rcvbufsz(rcvbufsz), _socket(-1) {}

LocalUdpClient::LocalUdpClient()
    : LocalUdpClient(UDP_CLIENT_SNDBUFSZ, UDP_CLIENT_RCVBUFSZ) {}

LocalUdpClient::~LocalUdpClient() { Disconnect(); }

bool LocalUdpClient::Connect(const std::string& sockname,
                             const std::string& peername) {
  // LOG_DEBUG("LocalUdpClient.Connect", "Connect from %s to %s",
  // sockname.c_str(), peername.c_str());
  ...
  if (_socket < 0) {
    LOG_ERROR("LocalUdpClient.Connect", "Unable to create socket (%s)",
              strerror(errno));
    return false;
  }

  if (!Anki::Messaging::SetReuseAddress(_socket, 1)) {
    LOG_ERROR("LocalUdpClient.Connect", "Unable to set reuseaddress (%s)",
              strerror(errno));
    close(_socket);
    _socket = -1;
    return false;
  ...
  if (!Anki::Messaging::SetNonBlocking(_socket, 1)) {
    LOG_ERROR("LocalUdpClient.Connect", "Unable to set nonblocking (%s)",
              strerror(errno));
    close(_socket);
    _socket = -1;
    return false;
  ...
  if (!Anki::Messaging::SetSendBufferSize(_socket, _sndbufsz)) {
    LOG_ERROR("LocalUdpClient.Connect",
              "Unable to set send buffer size %d (%s)", _sndbufsz,
              strerror(errno));
    close(_socket);
    _socket = -1;
    return false;
  ...
  if (!Anki::Messaging::SetRecvBufferSize(_socket, _rcvbufsz)) {
    LOG_ERROR("LocalUdpClient.Connect",
              "Unable to set recv buffer size %d (%s)", _rcvbufsz,
              strerror(errno));
    close(_socket);
    _socket = -1;
    return false;
  ...
  if (bind(_socket, (struct sockaddr*)&_sockaddr, _sockaddr_len) != 0) {
    LOG_ERROR("LocalUdpClient.Connect", "Unable to bind socket (%s)",
              strerror(errno));
    close(_socket);
    _socket = -1;
    return false;
  ...
  if (connect(_socket, (struct sockaddr*)&_peeraddr, _peeraddr_len) != 0) {
    LOG_ERROR("LocalUdpClient.Connect", "Unable to connect to %s (%s)",
              peername.c_str(), strerror(errno));
    close(_socket);
    _socket = -1;
    return false;
  }
  ...
  // Send connection packet (i.e. something so that the server adds us to the
  // client list)
  Send(LocalUdpServer::kConnectionPacket,
       sizeof(LocalUdpServer::kConnectionPacket));
  ...
  if (_socket > -1) {
    if (close(_socket) < 0) {
      LOG_ERROR("LocalUdpClient.Disconnect.Fail",
                "Error closing socket %s (sock: %d) (%s)", _sockname.c_str(),
