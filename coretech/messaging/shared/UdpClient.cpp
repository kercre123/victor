...
UdpClient::UdpClient(const std::string& name)
    : _name(name), host_info_list(nullptr), socketfd(-1) {}
...
bool UdpClient::Connect(const char* host_address, const unsigned short port) {
  ...
  if (status != 0) {
    LOG_ERROR("UdpClient.Connect.GetAddrInfoError", "%s (host: %s, port %hu",
              gai_strerror(status), host_address, port);
    return false;
  }

  LOG_INFO("UdpClient.Connect.CreatingSocket", "Port: %hu (%s)", port,
           _name.c_str());
  ...
  socketAddress.sin_addr.s_addr =
      htonl(INADDR_ANY);  // accept input from any address
  ...
  if (bindResult < 0) {
    if (EADDRINUSE == errno) {
      // Unable to bind to in-use socket, continuing as this is OK in case of
      // running multiple instances on one machine.
      LOG_WARNING("UdpClient.Connect.AddrInUse", "");
    } else {
      LOG_ERROR("UdpClient.Connect.BindFailed", "res = %d, errno = %d '%s'",
                bindResult, errno, strerror(errno));
    }
  }
  // Send connection packet (i.e. something so that the server adds us to the
  // client list)
  LOG_INFO("UdpClient.Connect.SendConnectionPacket", "%s (%zu)", _name.c_str(),
           sizeof(UdpServer::kConnectionPacket));
