...
bool TcpServer::StartListening(const unsigned short port) {
  ...
  // memory. This memory is not necessarily empty. Therefore we use the memset
  ...
  // getaddrinfo returns 0 on success, or some other value when an error
  // occurred. (translated into human readable text by the gai_strerror
  // function).
  if (status != 0) {
  ...
  if (status != 0) {
    DEBUG_TCP_SERVER(
        "TcpServer: Unable to set SO_REUSEADDR, errno=" << strerror(errno));
  }

#if defined(LINUX) || defined(ANDROID) || defined(VICOS)
// No SO_NOSIGPIPE available on these platforms - handled with MSG_NOSIGNAL flag
// on send instead
#else
  ...
  if (status != 0) {
    DEBUG_TCP_SERVER(
        "TcpServer: Unable to set SO_NOSIGPIPE, errno=" << strerror(errno));
  }
  ...
  if (status != 0) {
    DEBUG_TCP_SERVER(
        "TcpServer: Unable to set TCP_NODELAY, errno=" << strerror(errno));
  }

  // status = setsockopt(socketfd, SOL_SOCKET, SO_KEEPALIVE, &yes, sizeof(int));
  // // For auto-detecting disconnected clients. if (status != 0) {
  //  DEBUG_TCP_SERVER("TcpServer: Unable to set SO_KEEPALIVE, errno=" <<
  //  strerror(errno));
  //}
  ...
  if (status == -1) {
    std::cerr << "**** ERROR: bind error on port " << port
              << " (You might have orphaned processes running) ****\n";
    return false;
  }
...
void TcpServer::StopListening() {
  if (socketfd >= 0) {
    DEBUG_TCP_SERVER("TcpServer: Stopping server listening on socket "
                     << socketfd);
...
bool TcpServer::Accept() {
  ...
  if (client_sd == -1) {
    ...
    DEBUG_TCP_SERVER(
        "TcpServer: Connection accepted. Using new socketfd : " << client_sd);
...
int TcpServer::Send(const char* data, int size) {
...
#else
  // No MSG_NOSIGNAL available on these platforms - handled with SO_NOSIGPIPE
  // socketopt instead
  ...
  if (bytes_sent > std::numeric_limits<int>::max()) {
    DEBUG_TCP_SERVER(
        "TcpServer: Send warning, num bytes sent > max integer.\n");
  }
...
int TcpServer::Recv(char* data, int maxSize) {
  ...
  if (bytes_received <= 0) {
    ...
    DEBUG_TCP_SERVER_VERBOSE("TcpServer: " << bytes_received
                                           << " bytes received : " << data);
  }

  if (bytes_received > std::numeric_limits<int>::max()) {
    DEBUG_TCP_SERVER(
        "TcpServer: Receive warning, num bytes received > max integer.\n");
  }
...
bool TcpServer::HasClient() const { return (client_sd != -1); }
