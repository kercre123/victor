...
class MsgPacket {
 public:
  MsgPacket(){};
  MsgPacket(const s32 sourceId, const s32 destId, const u16 dataLen,
            const u8* data, double timestamp = 0.0)
      : dataLen(dataLen),
        sourceId(sourceId),
        destId(destId),
  ...
  // dataLen MUST be stored immediately before data so that we can send messages
  // as [len][msg] without copying
  u16 dataLen = 0;
  u8 data[MAX_SIZE];
...
static_assert(offsetof(MsgPacket, data) ==
                  (offsetof(MsgPacket, dataLen) + sizeof(MsgPacket::dataLen)),
              "Error len and data not contiguous!");
