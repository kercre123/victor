...
typedef enum { LSL = 0, LSR, RSL, RSR, NUM_DUBINS_PATHS } DubinsPathType;
...
// TODO: The following structs and class might be better separated into a robot
// version and basestation version.
...
class PathSegment {
 public:
  ...
  // NOTE: this is not identical to AppendArc because AppendArc may append
  // multiple arcs
  void DefineArc(f32 x_center, f32 y_center, f32 radius, f32 startRad,
  ...
  // @param shortestDistanceToPath: The distance to the closest point on the
  // path segment.
  //                                +ve means the closest point is to the right
  //                                of the robot assuming that the robot is
  //                                aligned with the direction of the path.
  //                                (i.e. Ignores `angle` parameter)
  // @param radDiff                 The angular difference between the angle of
  // the test point and that of the
  //                                tangent of the closest point on the path
  //                                segment
  // @param distAlongSegmentFromClosestPointToEnd: Distance to end of segment
  // from closest point on segment.
  //                                -ve means you've gone past the end of the
  //                                segment
  SegmentRangeStatus GetDistToSegment(
      const f32 x, const f32 y, const f32 angle, f32 &shortestDistanceToPath,
      f32 &radDiff, f32 *distAlongSegmentFromClosestPointToEnd = NULL) const;
 ...
 private:
  SegmentRangeStatus GetDistToLineSegment(
      const f32 x, const f32 y, const f32 angle, f32 &shortestDistanceToPath,
      f32 &radDiff, f32 *distAlongSegmentFromClosestPointToEnd = NULL) const;

  SegmentRangeStatus GetDistToArcSegment(
      const f32 x, const f32 y, const f32 angle, f32 &shortestDistanceToPath,
      f32 &radDiff, f32 *distAlongSegmentFromClosestPointToEnd = NULL) const;

  SegmentRangeStatus GetDistToPointTurnSegment(const f32 x, const f32 y,
                                               const f32 angle,
                                               f32 &shortestDistanceToPath,
                                               f32 &radDiff) const;

  PathSegmentType type_;
  PathSegmentDef def_;

  f32 targetSpeed_;  // Desired speed during segment. (In mm/s for lines and
                     // arcs. In rad/s for point turn.)
  f32 accel_;  // Acceleration with which targetSpeed may be approached. (In
               // mm/s^2 for lines and arcs. In rad/s^2 for point turn.)
  f32 decel_;  // Deceleration with which targetSpeed may be approached. (In
               // mm/s^2 for lines and arcs. In rad/s^2 for point turn.)
};
...
class Path {
 public:
  ...
  bool AppendArc(f32 x_center, f32 y_center, f32 radius, f32 startRad,
                 f32 sweepRad, f32 targetSpeed, f32 accel, f32 decel);
  ...
  const PathSegment &GetSegmentConstRef(const u8 idx) const {
    return path_[idx];
  }
  ...
  bool CheckContinuity(f32 tolerance_distance_squared,
                       s8 pathSegmentIdx = -1) const;
 ...
 private:
  ...
  bool CheckSegmentContinuity(f32 tolerance_distance_squared,
                              s8 pathSegmentIdx) const;

  void AddArc(f32 x_center, f32 y_center, f32 radius, f32 startRad,
              f32 sweepRad, f32 targetSpeed, f32 accel, f32 decel);

  // Returns angle between two points on a circle
  // f32 GetArcAngle(f32 start_x, f32 start_y, f32 end_x, f32 end_y, f32
  // center_x, f32 center_y, bool CCW);
  ...
  //                    DubinsPathType pathType, PathSegment path[], f32
  //                    &path_length);
};
