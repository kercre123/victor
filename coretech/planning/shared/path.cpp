...
void PathSegment::DefineLine(f32 x_start, f32 y_start, f32 x_end, f32 y_end,
  ...
  def_.line.endAngle = Radians(ATAN2_ACC(y_end - y_start, x_end - x_start) +
                               (targetSpeed < 0 ? M_PI_F : 0))
                           .ToFloat();

  SetSpeedProfile(targetSpeed, accel, decel);
}

void PathSegment::DefineArc(f32 x_center, f32 y_center, f32 radius,
  ...
  // If going backwards, flip it 180
  def_.arc.endAngle =
      Radians(radiusAngle + (targetSpeed < 0 ? M_PI_F : 0)).ToFloat();

  SetSpeedProfile(targetSpeed, accel, decel);
}

void PathSegment::DefinePointTurn(f32 x, f32 y, f32 startAngle, f32 targetAngle,
                                  f32 targetRotSpeed, f32 rotAccel,
                                  f32 rotDecel, f32 angleTolerance,
                                  bool useShortestDir) {
...
f32 PathSegment::GetLength() const {
  switch (type_) {
    case PST_LINE:
      return sqrtf((def_.line.startPt_x - def_.line.endPt_x) *
                       (def_.line.startPt_x - def_.line.endPt_x) +
                   (def_.line.startPt_y - def_.line.endPt_y) *
                       (def_.line.startPt_y - def_.line.endPt_y));
    case PST_ARC:
...
void PathSegment::GetEndPose(f32 &x, f32 &y, f32 &angle) const {
  switch (type_) {
    case PST_LINE:
      ...
      break;
    case PST_ARC:
      x = def_.arc.centerPt_x +
          def_.arc.radius * cosf(def_.arc.startRad + def_.arc.sweepRad);
      y = def_.arc.centerPt_y +
          def_.arc.radius * sinf(def_.arc.startRad + def_.arc.sweepRad);
  ...
  switch (type_) {
    case PST_LINE: {
      ...
      snprintf(buf, sizeof(buf),
               "line: (%f, %f) to (%f, %f), speed/accel/decel = (%f, %f, %f)",
               seg.startPt_x, seg.startPt_y, seg.endPt_x, seg.endPt_y,
               GetTargetSpeed(), GetAccel(), GetDecel());
      break;
    }
    case PST_ARC: {
      const PathSegmentDef::s_arc &seg = def_.arc;
      snprintf(buf, sizeof(buf),
               "arc: centerPt (%f, %f), radius %f, startAng %f, sweep %f, "
               "speed/accel/decel = (%f, %f, %f)",
               seg.centerPt_x, seg.centerPt_y, seg.radius, seg.startRad,
               seg.sweepRad, GetTargetSpeed(), GetAccel(), GetDecel());
      break;
    }
    case PST_POINT_TURN: {
      const PathSegmentDef::s_turn &seg = def_.turn;
      snprintf(buf, sizeof(buf),
               "ptTurn: x %f, y %f, targetAngle %f, tol %f deg "
               "speed/accel/decel = (%f, %f, %f)",
               seg.x, seg.y, seg.targetAngle, RAD_TO_DEG(seg.angleTolerance),
               GetTargetSpeed(), GetAccel(), GetDecel());
      break;
    }
    default:
      snprintf(buf, sizeof(buf), "unexpected type %d", type_);
      break;
  }
  return buf;
}

void PathSegment::Print() const { CoreTechPrint("%s\n", ToString().c_str()); }

SegmentRangeStatus PathSegment::GetDistToSegment(
    const f32 x, const f32 y, const f32 angle, f32 &shortestDistanceToPath,
    f32 &radDiff, f32 *distAlongSegmentFromClosestPointToEnd) const {
  ...
  switch (type_) {
    ...
    case PST_POINT_TURN:
      ...
      res = GetDistToPointTurnSegment(x, y, angle, shortestDistanceToPath,
                                      radDiff);
      break;
    default:
      assert(false);
...
SegmentRangeStatus PathSegment::GetDistToLineSegment(
    const f32 x, const f32 y, const f32 angle, f32 &shortestDistanceToPath,
    f32 &radDiff, f32 *distAlongSegmentFromClosestPointToEnd) const {
  ...
  f32 line_m_ =
      (seg->endPt_y - seg->startPt_y) / (seg->endPt_x - seg->startPt_x);
  f32 line_b_ = seg->startPt_y - line_m_ * seg->startPt_x;
  f32 line_dy_sign_ = ((seg->endPt_y - seg->startPt_y) >= 0) ? 1.0 : -1.0;
  Radians line_theta_ =
      ATAN2_FAST(seg->endPt_y - seg->startPt_y, seg->endPt_x - seg->startPt_x);

  // Find shortest path to current segment.
  // Shortest path is along a line with inverse negative slope (i.e. -1/m).
  // Point of intersection is solution to mx + b == (-1/m)*x + b_inv where b_inv
  // = y-(-1/m)*x

#if (DEBUG_PATH)
  CoreTechPrint("LINE (%f, %f, %f, %f)\n", seg->startPt_x, seg->startPt_y,
                seg->endPt_x, seg->endPt_y);
  ...
  if (ABS(line_m_) > 10000) {
    ...
    // currPose.GetAngle().ToFloat());
#endif

    // Compute the sign of the error distance
    shortestDistanceToPath *=
        (signbit(line_m_) ? -1 : 1) * (signbit(dy) ? -1 : 1) * line_dy_sign_;
    ...
    // and the robot is closer to the end point than it is to the start point,
    // then we've passed this segment and should go to next one
    if ((signbit(seg->startPt_x - x_intersect) ==
         signbit(seg->endPt_x - x_intersect)) &&
        (signbit(seg->startPt_y - y_intersect) ==
         signbit(seg->endPt_y - y_intersect))) {
      if (sqDistToStartPt > sqDistToEndPt) {
        distToEnd = -distToEnd;
        segStatus = OOR_NEAR_END;
      } else {
...
SegmentRangeStatus PathSegment::GetDistToArcSegment(
    const f32 x, const f32 y, const f32 angle, f32 &shortestDistanceToPath,
    f32 &radDiff, f32 *distAlongSegmentFromClosestPointToEnd) const {
  const PathSegmentDef::s_arc *seg = &(def_.arc);
  ...
  CoreTechPrint("ARC (%f, %f), startRad: %f, sweepRad: %f, radius: %f\n",
                seg->centerPt_x, seg->centerPt_y, seg->startRad, seg->sweepRad,
                seg->radius);
#endif
  ...
  // Find error between current robot heading and the expected heading at the
  // closest point on the arc
  bool movingCCW = seg->sweepRad >= 0;
  Anki::Radians theta_line =
      ATAN2_FAST(dy, dx);  // angle of line from circle center to robot
  Anki::Radians theta_tangent =
      theta_line + Anki::Radians((movingCCW ? 1 : -1) * M_PI_2_F);

  radDiff = (theta_tangent - angle).ToFloat();

  // Compute shortest distance to arc
  shortestDistanceToPath =
      sqrtf((x - x_center) * (x - x_center) + (y - y_center) * (y - y_center)) -
      r;

  // Compute sign of shortest distance
  // TODO: This method assumes that the robot is already mostly aligned with
  // `theta_tangent`.
  ...
  // Compute distance to end of segment
  f32 distToEnd =
      ABS((Radians(seg->startRad + seg->sweepRad) - theta_line).ToFloat() *
          seg->radius);

  // Did we pass the current segment?
  // Check if the angDiff exceeds the sweep angle.
  // Also check for transitions between -PI and +PI by checking if angDiff
  // ever exceeds a conservative half the distance if PI was approached from the
  // opposite direction.
  SegmentRangeStatus segStatus = IN_SEGMENT_RANGE;
  f32 angDiff = (theta_line - startRad).ToFloat();

  if (movingCCW) {
    if (angDiff > seg->sweepRad ||
        angDiff < -0.5f * (2.f * M_PI_F - seg->sweepRad)) {
      distToEnd = -distToEnd;
      segStatus = OOR_NEAR_END;
    } else if (angDiff < 0 &&
               angDiff > -0.5f * (2.f * M_PI_F - seg->sweepRad)) {
  ...
  CoreTechPrint("dy: %f, dx: %f, dist: %f, radDiff: %f\n", dy, dx,
                shortestDistanceToPath, radDiff);
  CoreTechPrint("insideCircle: %d, segmentRangeStatus: %d\n", robotInsideCircle,
                segStatus);
  CoreTechPrint("theta_line: %f, theta_tangent: %f\n", theta_line.ToFloat(),
                theta_tangent.ToFloat());
#endif

  return segStatus;
}

SegmentRangeStatus PathSegment::GetDistToPointTurnSegment(
    const f32 x, const f32 y, const f32 angle, f32 &shortestDistanceToPath,
    f32 &radDiff) const {
  const PathSegmentDef::s_turn *seg = &(def_.turn);
  ...
  CoreTechPrint("TURN (%f, %f), targetAngle: %f, targetRotSpeed: %f\n", seg->x,
                seg->y, seg->targetAngle, GetTargetSpeed());
#endif

  shortestDistanceToPath =
      sqrtf((x - seg->x) * (x - seg->x) + (y - seg->y) * (y - seg->y));
...
f32 GetArcAngle(f32 start_x, f32 start_y, f32 end_x, f32 end_y, f32 center_x,
                f32 center_y, bool CCW) {
  f32 a_start_x, a_start_y;  // Vector from circle center to starting point on
                             // circle when computing arc length
  f32 a_end_x, a_end_y;      // Vector from circle center to end point on circle
                             // when computing arc length
...
u8 GenerateCSCCurve(f32 startPt_x, f32 startPt_y, f32 startPt_theta,
                    f32 endPt_x, f32 endPt_y, f32 endPt_theta, f32 start_radius,
                    f32 end_radius, f32 targetSpeed, f32 accel, f32 decel,
                    DubinsPathType pathType, PathSegment path[],
                    f32 &path_length) {
  ...
  // So n • (V1 - ((r1-r2) • n)) = 0, since n and (V1 - ((r1-r2) • n)) are
  // orthogonal
  //    n • V1 + r2 - r1 = 0
  //    n • V1 = r1 - r2
  //
  ...
  //
  // Let c = (r1-r2)/D
  // c is the cosine of the angle between v1 and n, since A • B == |A| |B|
  // cos(theta).
  //
  ...
  CoreTechPrint(
      ...
      " p_t1 (%f, %f)\n"
      " p_t2 (%f, %f)\n",
      pathType, p_c1_x, p_c1_y, p_c2_x, p_c2_y, V1_x, V1_y, n_x, n_y, p_t1_x,
      p_t1_y, p_t2_x, p_t2_y);
#endif

  // Generate path segment definitions
  path_length = 0;
  ps = &path[num_segments];
  ps->DefineArc(p_c1_x, p_c1_y, start_radius,
                ATAN2_ACC(startPt_y - p_c1_y, startPt_x - p_c1_x),
                GetArcAngle(startPt_x, startPt_y, p_t1_x, p_t1_y, p_c1_x,
                            p_c1_y, sign1 < 0),
  ...
  ps = &path[num_segments];
  ps->DefineLine(p_t1_x, p_t1_y, p_t2_x, p_t2_y, targetSpeed, accel, decel);
  segment_length = ps->GetLength();
  ...
  ps = &path[num_segments];
  ps->DefineArc(
      p_c2_x, p_c2_y, end_radius, ATAN2_ACC(p_t2_y - p_c2_y, p_t2_x - p_c2_x),
...
// Fails automatically
u8 GenerateDubinsPath(Path &path, f32 start_x, f32 start_y, f32 start_theta,
                      f32 end_x, f32 end_y, f32 end_theta, f32 start_radius,
                      f32 end_radius, f32 targetSpeed, f32 accel, f32 decel,
  ...
  f32 preStraightApproach_x =
      end_x - final_straight_approach_length * cosf(end_theta);
  f32 preStraightApproach_y =
      end_y - final_straight_approach_length * sinf(end_theta);

#if (DEBUG_PATH)
  CoreTechPrint(
      "DUBINS: startPt %f %f %f, preEnd %f %f, endPt %f %f %f, start_radius "
      "%f, end_radius %f\n",
      start_x, start_y, start_theta, preStraightApproach_x,
      preStraightApproach_y, end_x, end_y, end_theta, start_radius, end_radius);
#endif

  for (DubinsPathType i = LSL; i != NUM_DUBINS_PATHS;
       i = (DubinsPathType)(i + 1)) {
    numSegments = GenerateCSCCurve(
        start_x, start_y, start_theta, preStraightApproach_x,
        preStraightApproach_y, end_theta, start_radius, end_radius, targetSpeed,
        accel, decel, i, csc_path[i], pathLength);
#if (DEBUG_PATH)
    CoreTechPrint("Dubins path %d: numSegments %d, length %f m\n", i,
                  numSegments, pathLength);
#endif
  ...
  // If a path was found, copy it to the path_ and tack on the final straight
  // segment.
  if (shortestPathType != NUM_DUBINS_PATHS) {
    ...
    for (u32 j = 0; j < shortestNumSegments; ++j) {
      ...
      switch (csc_path[shortestPathType][j].GetType()) {
        case PST_LINE: {
          const PathSegmentDef::s_line *l =
              &(csc_path[shortestPathType][j].GetDef().line);
          path.AppendLine(l->startPt_x, l->startPt_y, l->endPt_x, l->endPt_y,
                          targetSpeed, accel, decel);
          break;
        }
        case PST_ARC: {
          const PathSegmentDef::s_arc *a =
              &(csc_path[shortestPathType][j].GetDef().arc);
          path.AppendArc(a->centerPt_x, a->centerPt_y, a->radius, a->startRad,
                         a->sweepRad, targetSpeed, accel, decel);
          break;
        }
        default:
          CoreTechPrint(
              "ERROR: Invalid path segment type in Dubins path. Should not be "
              "possible!\n");
          assert(0);
          break;
      }
    ...
    if (final_straight_approach_length != 0) {
      path.AppendLine(preStraightApproach_x, preStraightApproach_y, end_x,
                      end_y, targetSpeed, accel, decel);
...
bool Path::PopFront(const u8 numSegments) {
  ...
  if (numSegments <= numPathSegments_) {
    ...
    memcpy(path_, &(path_[numSegments]),
           numPathSegments_ * sizeof(PathSegment));
...
// orientation is not discontinuous and that velocity profile is smooth
// at transition points.
bool Path::CheckSegmentContinuity(f32 tolerance_distance_squared,
                                  s8 pathSegmentIdx) const {
  assert(capacity_ == MAX_NUM_PATH_SEGMENTS);

  // If checking continuity on non-existent piece
  if (pathSegmentIdx >= numPathSegments_) return false;

  // If checking continuity on first piece
  if (pathSegmentIdx == 0) return true;

  // Compute distance between start point of specified segment to end point of
  // previous segment
  ...
  if ((start_x - end_x) * (start_x - end_x) +
      ...
      tolerance_distance_squared) {
    return true;
  }

  CoreTechPrint(
      "Continuity fail: Segment %d start point (%f, %f), Segment %d end point "
      "(%f, %f)\n",
      pathSegmentIdx, start_x, start_y, pathSegmentIdx - 1, end_x, end_y);
  return false;
}

bool Path::CheckContinuity(f32 tolerance_distance_squared,
                           s8 pathSegmentIdx) const {
  assert(capacity_ == MAX_NUM_PATH_SEGMENTS);
...
void Path::AddArc(f32 x_center, f32 y_center, f32 radius, f32 startRad,
                  f32 sweepRad, f32 targetSpeed, f32 accel, f32 decel) {
  assert(capacity_ == MAX_NUM_PATH_SEGMENTS);
  ...
  path_[numPathSegments_].DefineArc(x_center, y_center, radius, startRad,
...
bool Path::AppendArc(f32 x_center, f32 y_center, f32 radius, f32 startRad,
                     f32 sweepRad, f32 targetSpeed, f32 accel, f32 decel) {
  assert(capacity_ == MAX_NUM_PATH_SEGMENTS);
...
#if DEBUG_PATH_APPEND
  CoreTechPrint("INFO (AppendSegment): numPathSegments_ = %u :",
                numPathSegments_);
  path_[numPathSegments_].Print();
#endif
