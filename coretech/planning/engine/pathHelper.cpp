...
void PathRelativeCozmo::AppendSegment(
    std::shared_ptr<const IRelativePathSegment> segment) {
  _pathComponents.push_back(segment);
};

void PathRelativeCozmo::GetPlanningPath(const Pose3d& startingPose,
                                        Planning::Path& path) const {
...
////Line Implementation
//////////
RelativePathLine::RelativePathLine(f32 distanceForward, f32 targetSpeed,
                                   f32 accel, f32 decel)
...
Pose3d RelativePathLine::ExtendPath(const Pose3d& pose,
                                    Planning::Path& path) const {
  ...
  Pose3d endPosition =
      Pose3d(0, Z_AXIS_3D(), {_distanceForward, 0.f, 0.f}, pose);
...
RelativePathArc::RelativePathArc(f32 radius, f32 sweepRad, f32 targetSpeed,
                                 f32 accel, f32 decel, bool clockwise)
    : _radius(radius),
      _sweepRad(sweepRad),
      _targetSpeed(targetSpeed),
      _accel(accel),
...
Pose3d RelativePathArc::ExtendPath(const Pose3d& pose,
                                   Planning::Path& path) const {
  // get append values
  ...
  path.AppendArc(x_center, y_center, fabs(radius), startAngle.ToFloat(), sweep,
                 _targetSpeed, _accel, _decel);

  // Calculate new translation
  Pose3d newRobotRotated =
      Pose3d(Radians(sweep), Z_AXIS_3D(), {0.f, 0.f, 0.f}, arcCenterRobot);
  Pose3d newRobotTranslated =
      Pose3d(0, Z_AXIS_3D(), {0.f, -radius, 0.f}, newRobotRotated);
...
RelativePathTurn::RelativePathTurn(f32 rotRadians, f32 targetRotSpeed,
                                   f32 rotAccel, f32 rotDecel,
                                   f32 angleTolerance, bool clockwise)
...
Pose3d RelativePathTurn::ExtendPath(const Pose3d& pose,
                                    Planning::Path& path) const {
  // Get append values
  ...
  path.AppendPointTurn(x_start, y_start, startAngle, zAxisRotation.ToFloat(),
                       _targetRotSpeed, _rotAccel, _rotDecel, _angleTolerance,
                       useShortestDistance);

  // Create Ending Pose
  Pose3d newRobotRotated =
      Pose3d(rotRadians, Z_AXIS_3D(), {0.f, 0.f, 0.f}, pose);
