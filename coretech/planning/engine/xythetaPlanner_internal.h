...
struct xythetaPlannerImpl {
  ...
  // this one computes based on the distance to the closest goal (including
  // _costOutsideHeurMapPerGoal)
  Cost heur_internal(StateID sid);

  // must be called after start and goal are set. Returns true if everything is
  // OK, false if we should fail. Also removes any goals (_goals_c and
  // _goalStateIDs) that exceed MAX_COST_HEUR_EXPANSIONS
  ...
  // returns true if a single goal is valid, false otherwise. Sets values of
  // arguments. If false is returned, may have set or not set any of the return
  // arguments. The method that takes as input a goalID looks up in the
  // context's goals the goal pair used the by the second method. This second
  // method still uses the context's env, so only use the second if you know the
  // goal pair is equiv to that in the context, or will soon be equiv (i.e.,
  // youre about to insert it)
  bool CheckContextGoal(const GoalID& goalID, StateID& goalStateID,
                        State_c& roundedGoal_c) const;
  bool CheckGoal(const GoalState_cPair& goal, StateID& goalStateID,
                 State_c& roundedGoal_c) const;
  ...
  // Checks all goals, returns true if any goals are valid. Adds a map element
  // to the arguments if the goal is valid. If false is returned, may have set
  // or not set any of the return arguments.
  ...
  // Context holds everything that can change outside of planning, and should
  // represent all of the config, including start, goal, motion primitives, etc.
  // and holds the environment.
  ...
  // These goalid/state_c pairs are different from the context continuous states
  // because they were rounded down to _goalStateIDs, then converted back to
  // continuous states. These will NOT remain sorted by GoalID.
  ...
  // assuming that goal is in soft collision, this function will do a
  // breadth-first expansion until we "escape" from the soft penalty. It will
  // add these penalties to a heuristic map, so the heuristic can take these
  // soft penalties into account. It works backwards, for the case when
  // goalStateID is the goal
  Cost ExpandCollisionStatesFromGoal(const StateID& goalStateID);

  // heuristic pre-computation stuff
  // for each goal, from closest goal id to min cost at level set around that
  // goal These will NOT remain sorted by GoalID, but the order should match
  // that of _goalStateIDs
