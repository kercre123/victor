...
// returns a list of unit vectors that are tangent to the circle at the origin
// with radius r, that also pass through point p. If p is inside the circle, or
// the circle has an invalid radius, return an empty list
inline std::vector<Vec2f> TangentsToCircle(const Point2f& p, float r) {
  float dx = r * r - p.x() * p.x();
  float raticand = p.y() * p.y() - dx;

  if (FLT_LT(raticand, 0.f) || FLT_LE(r, 0.f)) {
    return {};
  }
  float a = -p.x() * p.y();
...
// given sequence of three points {p0, p1, p2} that define the connected line
// segments (L1 := p0->p1) and (L2 := p1->p2) and a raidus r, generate a
// circumscribed arc {t0, p1, t1} such that:
...
//   - the piecewise function defined by L1'->A->L2' is continuously
//   differentialable
//   - returns FALSE if {p0, p1, p2} are colinear, or if no tangent exists with
//   radius r
inline bool GetCircumscribedArc(const Point2f& p0, const Point2f& p1,
                                const Point2f& p2, float r, Arc& A) {
  // first get the center point for a circle on the angle bisector of p0-p1-p2,
  // where p1 is on the circle
  Vec2f v0 = p0 - p1;
  Vec2f v1 = p2 - p1;
  ...
  if (L1vecs.empty() || L2vecs.empty()) {
    return false;
  }

  // (MRW) we might be able to get away with only calculating two of these if we
  // can calculate orientation before getting the tangent points
  ...
  // of possible tangent points, figure choose those that make work with the
  // current direction
...
// given sequence of three points {p0, p1, p2} that define the connected line
// segments (L1 := p0->p1) and (L2 := p1->p2) and a raidus r, generate an
// inscribed arc {t0, t1, t2} such that:
...
inline bool GetInscribedArc(const Point2f& p0, const Point2f& p1,
                            const Point2f& p2, float r, Arc& A) {
  // exit if co-linear
  if (GetOrientation(p1 - p0, p2 - p1) == 0) {
    return false;
  }

  // first get the center point for a circle of on the angle bisector of
  // p0-p1-p2
  Vec2f v0 = p0 - p1;
  Vec2f v1 = p2 - p1;
  v0.MakeUnitLength();
  ...
  //    r = C · perp
  const Point2f C =
      bisector * (r / ABS(DotProduct(Vec2f(v0.y(), -v0.x()), bisector)));
  ...
  if (!LineSegment(p0, p1).Contains(t0) || !LineSegment(p1, p2).Contains(t2)) {
    return false;
  }
...
// convert a circle defined by three points into a circle defined by a center
// point and a radius
inline Ball2f ArcToBall(const Arc& a) {
  ...
  Point2f center;
  GetIntersectionPoint(chord1.GetPerpendicularBisector(),
...
inline Point2f CreatePoint2f(const Planning::PathSegment& p) {
  DEV_ASSERT(p.GetType() == Planning::PST_POINT_TURN,
             "Cannot create Point2f from non-point turn path segment");
  return {p.GetDef().turn.x, p.GetDef().turn.y};
}

// create a PathSegment<PointTurn> at point `a.middle`, coming from `a.start`,
// turning towards `a.end`
inline Planning::PathSegment CreatePointTurnPath(const Arc& a) {
  ...
  seg.DefinePointTurn(a.middle.x(), a.middle.y(), heading1.ToFloat(),
                      heading2.ToFloat(), 1.f, 1.f, 1.f, .1f, true);
  return seg;
}

// create PathSegment<PointTurn> from `start`, turning towards point `to`
inline Planning::PathSegment CreatePointTurnPath(const Pose2d& start,
                                                 const Point2f& to) {
  // calculate heading angles
  ...
  // build segment
  Planning::PathSegment seg;
  seg.DefinePointTurn(p.x(), p.y(), start.GetAngle().ToFloat(),
                      heading.ToFloat(), 1.f, 1.f, 1.f, .1f, true);
  return seg;
}

// create PathSegment<PointTurn> turning from point `from`, ending at pose `end`
inline Planning::PathSegment CreatePointTurnPath(const Point2f& from,
                                                 const Pose2d& end) {
  // calculate heading angles
  ...
  // build segment
  Planning::PathSegment seg;
  seg.DefinePointTurn(p.x(), p.y(), heading.ToFloat(), end.GetAngle().ToFloat(),
                      1.f, 1.f, 1.f, .1f, true);
  return seg;
}
...
inline Planning::PathSegment CreateLinePath(const LineSegment& l) {
  // build segment
  Planning::PathSegment seg;
  seg.DefineLine(l.GetFrom().x(), l.GetFrom().y(), l.GetTo().x(), l.GetTo().y(),
                 1.f, 1.f, 1.f);
  return seg;
}
...
inline LineSegment CreateLineSegment(const Planning::PathSegment& p) {
  DEV_ASSERT(p.GetType() == Planning::PST_LINE,
             "Cannot create LineSegment from non-line path segment");
  const auto& def = p.GetDef().line;
  return LineSegment({def.startPt_x, def.startPt_y},
                     {def.endPt_x, def.endPt_y});
}
...
inline Planning::PathSegment CreateArcPath(const Arc& a) {
  ...
  // build segment
  Planning::PathSegment seg;
  seg.DefineArc(b.GetCentroid().x(), b.GetCentroid().y(), b.GetRadius(),
                startAngle.ToFloat(), sweepAngle.ToFloat(), 1.f, 1.f, 1.f);
  return seg;
}

// PathSegment<Arc> -> Arc conversion
inline Arc CreateArc(const Planning::PathSegment& p) {
  DEV_ASSERT(p.GetType() == Planning::PST_ARC,
             "Cannot create Arc from non-arc path segment");
