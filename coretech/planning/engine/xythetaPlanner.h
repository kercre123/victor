...
class xythetaPlanner {
 public:
  // NOTE: you can't change context while the planner is running or undefined
  ...
  // Check if a goal is valid. Compares against context obstacles, but not using
  // its goals.
  ...
  // Compares against context's env, returns true if segments in path comprise a
  // safe and complete plan Clears and fills in a list of path segments whose
  // cumulative penalty doesnt exceed the max penalty
  bool PathIsSafe(const Path& path, float startAngle, Path& validPath) const;

  // Computes a path from start to goal. Returns true if path found, false
  // otherwise. Note that replanning may or may not actually trigger the
  // planner. E.g. if the environment hasn't changed (much), it may just use the
  // same path. Everything in context is allowed to change between replan calls
  // (but it won't necessarily be efficient if too many things change). If the
  // second argument is not null, then its value will be checked somewhere
  // within the planenr loop, and the planner will return quickly if it turns
  // false
  bool Replan(unsigned int maxExpansions = DEFUALT_MAX_EXPANSIONS,
              volatile bool* runPlan = nullptr);

  // must Replan before getting the plan
  xythetaPlan& GetPlan();
  const xythetaPlan& GetPlan() const;

  // Return one of a set of hardcoded plans, useful for testing other components
  // of the system
