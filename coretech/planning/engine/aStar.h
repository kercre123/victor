...
class IAStarConfig {
 public:
  ...
  float Heuristic(const T& p) const {
    return static_cast<Impl const&>(*this).Heuristic(p);
  }
  bool IsGoal(const T& p) const {
    return static_cast<Impl const&>(*this).IsGoal(p);
  }
...
class AStar {
 public:
  // Construct A* planner
  AStar(ConfigT& config)
      : _config(config), _open(1024), _closed(1024, StateHasher, StateEqual) {}
  ...
  while (!_config.StopPlanning() && !foundGoal && !_open.empty()) {
    ...
    if (closedRecord.second) {
      ...
      for (const Iter& succ : _config.GetSuccessors(current->state)) {
        ...
        _open.emplace({succ.state, current->state, newCost,
                       newCost + _config.Heuristic(succ.state)});
      }
    }
  }

  return (foundGoal && (current != _closed.end())) ? GetPlan(current->state)
                                                   : std::vector<T>();
}
