...
void xythetaPlan::Append(const xythetaPlan& other) {
  ...
  _actionCostPairs.insert(_actionCostPairs.end(),
                          other._actionCostPairs.begin(),
                          other._actionCostPairs.end());
}

bool MotionPrimitive::IntermediatePosition::Import(const Json::Value& config) {
  try {
    if (config.isNull()) {
      PRINT_NAMED_ERROR("IntermediatePosition.Import.Null",
...
void MotionPrimitive::IntermediatePosition::Dump(
    Util::JsonWriter& writer) const {
...
ActionType::ActionType()
    : _extraCostFactor(0.0), _id(-1), _name("<invalid>"), _reverse(false) {}
...
bool ActionType::Import(const Json::Value& config) {
  try {
    ...
    if (!JsonTools::GetValueOptional(config, "extra_cost_factor",
                                     _extraCostFactor) ||
    ...
    PRINT_NAMED_ERROR("ActionType.Import.Exception", "json exception: %s",
                      e.what());
    return false;
  }
  ...
  try {
    ...
    if (config["cost"].isNull()) {
      PRINT_NAMED_ERROR("MotionPrimitive.Import.InvalidConfig2",
                        "missing 'cost' key. Did you mean to call Create() "
                        "instead of Import()?");
      JsonTools::PrintJsonCout(config, 3);
      return false;
    }
    ...
    if (config["intermediate_poses"].size() <= 1) {
      PRINT_NAMED_ERROR("MotionPrimitive.Import.InvalidConfig3",
                        "'intermediate_poses' size %d too small (or not a "
                        "list). Dump follows",
    ...
    PRINT_NAMED_ERROR("MotionPrimitive.Import.Exception", "json exception: %s",
                      e.what());
    return false;
  }
...
u8 MotionPrimitive::AddSegmentsToPath(State_c start, Path& path) const {
  ...
  for (u8 pathIdx = 0; pathIdx < pathSegments_.GetNumSegments(); ++pathIdx) {
    ...
    if (segment.GetType() == PST_POINT_TURN) {
      segment.GetDef().turn.angleTolerance =
          DEG_TO_RAD(kXYTPlanner_PointTurnTolOverride_deg);
    }
#endif
    ...
    if (path.GetNumSegments() > 0 &&
        path[path.GetNumSegments() - 1].GetType() == segment.GetType()) {
      ...
      switch (segment.GetType()) {
        case PST_LINE: {
          ...
          if (oldSign == newSign) {
            const float dx = (segment.GetDef().line.endPt_x -
                              path[endIdx].GetDef().line.startPt_x);
            const float dy = (segment.GetDef().line.endPt_y -
                              path[endIdx].GetDef().line.startPt_y);
            if (dx * dx + dy * dy <= LATTICE_PLANNER_MAX_LINE_LENGTH *
                                         LATTICE_PLANNER_MAX_LINE_LENGTH) {
              path[endIdx].GetDef().line.endPt_x =
                  segment.GetDef().line.endPt_x;
              path[endIdx].GetDef().line.endPt_y =
                  segment.GetDef().line.endPt_y;
              shouldAdd = false;
            }
          }
          break;
        ...
        case PST_ARC:
          ...
          // if(FLT_NEAR(path[endIdx].GetDef().arc.centerPt_x,
          // segment.GetDef().arc.centerPt_x) &&
          //        FLT_NEAR(path[endIdx].GetDef().arc.centerPt_y,
          //        segment.GetDef().arc.centerPt_y) &&
          //        FLT_NEAR(path[endIdx].GetDef().arc.radius,
          //        segment.GetDef().arc.radius)) {

          //   path[endIdx].GetDef().arc.sweepRad +=
          //   segment.GetDef().arc.sweepRad; shouldAdd = false;
          // }
          break;
        ...
        case PST_POINT_TURN:
          ...
          if (FLT_NEAR(path[endIdx].GetDef().turn.x, segment.GetDef().turn.x) &&
              ...
              FLT_NEAR(path[endIdx].GetTargetSpeed(),
                       segment.GetTargetSpeed())) {
            path[endIdx].GetDef().turn.targetAngle =
                segment.GetDef().turn.targetAngle;
            shouldAdd = false;
          }
          break;
  ...
  if (!added && firstSegment > 0) firstSegment--;

  return firstSegment;
}

bool MotionPrimitive::Create(const Json::Value& config,
                             GraphTheta startingAngle, const ActionSpace& env) {
  ...
  for (unsigned int i = 0; i < numIntermediatePoses; ++i) {
    ...
    if (!intermediatePositions.empty()) {
      ...
      cost += std::abs(deltaTheta.ToFloat()) * env.GetHalfWheelBase_mm() *
  ...
  if (config.isMember("extra_cost_factor")) {
    printf(
        "ERROR: individual primitives shouldn't have cost factors. Old file "
        "format?\n");
    return false;
  }
  ...
  if (length > 0.0) {
    ...
    if (std::abs(signedLength) > 0.001) {
      pathSegments_.AppendLine(
          0.0, 0.0, signedLength * cos(env.LookupTheta(startingAngle)),
          signedLength * sin(env.LookupTheta(startingAngle)),
          isReverse ? -linearSpeed : linearSpeed, LATTICE_PLANNER_ACCEL,
  ...
  if (config.isMember("arc")) {
    ...
    // the total time is the arclength of the outer wheel arc divided by the max
    // outer wheel speed
    ...
    pathSegments_.AppendArc(config["arc"]["centerPt_x_mm"].asFloat(),
                            ...
                            LATTICE_PLANNER_ACCEL, LATTICE_PLANNER_DECEL);
    ...
    double deltaTheta = startRads.angularDistance(
        env.LookupTheta(endStateOffset.theta), direction < 0);

    Cost turnTime =
        std::abs(deltaTheta) * env.GetHalfWheelBase_mm() * oneOverLinearSpeed;
    cost += turnTime;

    float rotSpeed = deltaTheta / turnTime;

    pathSegments_.AppendPointTurn(
        0.0, 0.0, startRads.ToFloat(), env.LookupTheta(endStateOffset.theta),
        rotSpeed, LATTICE_PLANNER_ROT_ACCEL, LATTICE_PLANNER_ROT_DECEL,
        LATTICE_PLANNER_POINT_TURN_TOL, true);
  }
  ...
  if (cost < 1e-6) {
    printf("ERROR: final action cost is %f (%f x) for action %d '%s\n", cost,
           env.GetActionType(id).GetExtraCostFactor(), id,
           env.GetActionType(id).GetName().c_str());
    return false;
  }
...
bool ActionSpace::ParseMotionPrims(const Json::Value& config,
                                   bool useDumpFormat) {
  try {
    float configResolution = 0.f;
    unsigned int configAngles = 0;
    if (!JsonTools::GetValueOptional(config, "resolution_mm",
                                     configResolution) ||
        !JsonTools::GetValueOptional(config, "num_angles", configAngles)) {
      printf(
          "error: could not find key 'resolution_mm' or 'num_angles' in motion "
          "primitives\n");
      JsonTools::PrintJsonCout(config, 1);
      return false;
    }
    ...
    if (_angles.size() != configAngles) {
      printf("ERROR: numAngles is %u, but we read %lu angle definitions\n",
             configAngles, (unsigned long)_angles.size());
      return false;
    }
        ...
        for (unsigned int i = 0; i < prims.size(); ++i) {
          ...
          if (useDumpFormat) {
            ...
            if (!p.Create(prims[i], angle, *this)) {
              PRINT_NAMED_ERROR("ParseMotionPrims.CreateFormat.Mprim",
                                "Failed to import motion primitive");
              return false;
            }
          }
    ...
    PRINT_NAMED_ERROR("ParseMotionPrims.Exception", "json exception: %s",
                      e.what());
    return false;
  }
...
void ActionSpace::PopulateReversePrims() {
  // go through each motion primitive, and populate the corresponding reverse
  // primitive
  for (int startAngle = 0; startAngle < GraphState::numAngles_; ++startAngle) {
    for (int actionID = 0; actionID < _forwardPrims[startAngle].size();
         ++actionID) {
...
void ActionSpace::PrintPlan(const xythetaPlan& plan) const {
  ...
  PRINT_STREAM_DEBUG("xythetaEnvironment.PrintPlan",
                     "plan start: " << plan.start_);

  for (size_t i = 0; i < plan.Size(); ++i) {
    PRINT_NAMED_DEBUG(
        "xythetaEnvironment.PrintPlan",
        "%2lu: (%f, %f, %f [%d]) --> %s (penalty = %f)", (unsigned long)i,
        curr_c.x_mm, curr_c.y_mm, curr_c.theta, curr_c.GetGraphTheta(),
        _actionTypes[plan.GetAction(i)].GetName().c_str(), plan.GetPenalty(i));
...
void ActionSpace::ApplyAction(ActionID action, State_c& state) const {
  const MotionPrimitive* prim =
      &GetForwardMotion(state.GetGraphTheta(), action);
...
void ActionSpace::AppendToPath(xythetaPlan& plan, Path& path,
                               int numActionsToSkip) const {
...
void ActionSpace::ConvertToXYPlan(const xythetaPlan& plan,
                                  std::vector<State_c>& continuousPlan) const {
  ...
  for (size_t i = 0; i < plan.Size(); ++i) {
    printf("curr = (%f, %f, %f [%d]) : %s\n", curr_c.x_mm, curr_c.y_mm,
           curr_c.theta, currTheta,
           _actionTypes[plan.GetAction(i)].GetName().c_str());

    if (currTheta >= GraphState::numAngles_ ||
        plan.GetAction(i) >= GetNumActions()) {
      printf("ERROR: can't look up prim for angle %d and action id %d\n",
             currTheta, plan.GetAction(i));
      break;
    } else {
      const MotionPrimitive* prim =
          &GetForwardMotion(currTheta, plan.GetAction(i));
