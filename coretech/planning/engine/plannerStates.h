...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - PlannerState - basic container for
// info needed by the planner
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
...
struct PlannerState {
  ...
  StateID prev;
  ActionID action;
  // MRW - we need to look into `penalty`. Right now, the only time we use this
  // value is for `CheckPathIsSafe`, which
  //       checks if the penalty has increased at all from when the path is
  //       created, but isn't necessarily fatal. the side affect is that the
  //       replanner always plans from the state right before any obstacle, and
  //       I don't think it is handling soft obstacle costs appropriately.
  Cost penalty;
  Cost g;
};

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - ClosedList - hash for quick lookup if
...
struct ClosedState : public PlannerState {
  ...
  inline bool operator==(const ClosedState& o) const {
    return (curr == o.curr);
  }  // for set hashing
};
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
//  OpenList - Priority Queue for getting best candidate for graph search. Note
//  this is a slight adaptation of the
//    STL priority_queue, but allows for Move semantics when calling `pop()` to
//    save excess copying
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
...
class OpenList2 : private std::vector<OpenState> {
 public:
  ...
  inline void emplace(Args&&... args) {
    ...
    std::push_heap(begin(), end(), [](const OpenState& a, const OpenState& b) {
      return a.f > b.f;
    });
  }

  inline PlannerState pop() {
    std::pop_heap(begin(), end(), [](const OpenState& a, const OpenState& b) {
      return a.f > b.f;
    });
