...
// When doing the soft obstacle goal heuristic expansion, don't do more than
// this many (to avoid getting stuck in a loop there)
#define MAX_HEUR_EXPANSIONS 1000

// When doing the soft obstacle goal heuristic expansion, prune goals if they
// exceed this cost
#define MAX_COST_HEUR_EXPANSIONS 1000.0f

// When doing the soft obstacle goal heuristic expansion, keep expanding until
// this many states "escape" the penalty zone
...
// Extra metric tracking if we want to print debugger stats, at the cost of
// extra operations internal to the loop
...
xythetaPlanner::~xythetaPlanner() { Util::SafeDelete(_impl); }

bool xythetaPlanner::GoalIsValid(GoalID goalID) const {
  return _impl->GoalIsValid(goalID);
}

bool xythetaPlanner::GoalIsValid(
    const std::pair<GoalID, State_c>& goal_cPair) const {
  return _impl->GoalIsValid(goal_cPair);
}

bool xythetaPlanner::GoalsAreValid() const { return _impl->GoalsAreValid(); }

bool xythetaPlanner::StartIsValid() const { return _impl->StartIsValid(); }

bool xythetaPlanner::PathIsSafe(const Planning::Path& path, float startAngle,
...
bool xythetaPlanner::Replan(unsigned int maxExpansions,
  ...
  PRINT_NAMED_INFO(
      "xythetaPlanner.ReplanComplete",
      "planning took %f seconds. (%f exps/sec, %f cons/sec, %f checks/sec)",
      time, ((double)_impl->_expansions) / time,
...
const xythetaPlan& xythetaPlanner::GetPlan() const { return _impl->_plan; }

xythetaPlan& xythetaPlanner::GetPlan() { return _impl->_plan; }

Cost xythetaPlanner::GetFinalCost() const { return _impl->_finalCost; }

GoalID xythetaPlanner::GetChosenGoalID() const { return _impl->_chosenGoalID; }

float xythetaPlanner::GetLastPlanTime() const { return _impl->_lastPlanTime; }

int xythetaPlanner::GetLastNumExpansions() const { return _impl->_expansions; }
...
bool xythetaPlannerImpl::CheckContextGoal(const GoalID& goalID,
                                          StateID& goalStateID,
                                          State_c& roundedGoal_c) const {
  ...
  PRINT_NAMED_ERROR("xythetaPlanner.GoalID.Invalid", "invalid goal id %d",
                    (int)goalID);
  return false;
}

bool xythetaPlannerImpl::CheckGoal(const std::pair<GoalID, State_c>& goal,
                                   StateID& goalStateID,
                                   State_c& roundedGoal_c) const {
  ...
  if (_context.env.IsInCollision(goal_c)) {
    PRINT_NAMED_INFO("xythetaPlanner.Goal.Invalid", "goal %d is in collision",
                     (int)goalID);
    return false;
  }
...
bool xythetaPlannerImpl::CheckContextGoals(
    GoalStateIDPairs& goalStateIDs, GoalState_cPairs& roundedGoals_c) const {
  ...
  if (goalStateIDs.empty()) {
    PRINT_NAMED_INFO(
        "xythetaPlanner.Goal.AllInvalid",
...
bool xythetaPlannerImpl::GoalIsValid(
    const std::pair<GoalID, State_c>& goal_cPair) const {
  StateID waste1;
  State_c waste2;
...
bool xythetaPlannerImpl::CheckContextStart(GraphState& start) const {
  ...
  if (_context.env.IsInCollision(start)) {
    if (!_context.env.RoundSafe(_context.start, start)) {
      PRINT_NAMED_INFO(
          "xythetaPlanner.Start.Invalid",
          "all possible discrete states of start are in collision!");
      return false;
    }
...
bool xythetaPlannerImpl::NeedsReplan() const {
  // TODO:(bn) json. This value balances between keeping the old plan (to look
  // smooth) and being dumb by going too far out of the way, in case the new
  // plan differs from old
  const float default_maxDistanceToReUse_mm = 300.0f;
  return !_context.env.PlanIsSafe(_plan, default_maxDistanceToReUse_mm);
}

bool xythetaPlannerImpl::PathIsSafe(const Planning::Path& path,
                                    float startAngle,
                                    Planning::Path& validPath) const {
...
bool xythetaPlannerImpl::InitializeHeuristic() {
  ...
  // this goes backwards because we want to remove some elements without copying
  // the vector
  for (int i = static_cast<int>(_goalStateIDs.size()) - 1; i >= 0; --i) {
    // Goals are expected to have matching ids, although not necessarily be
    // sorted by them
    DEV_ASSERT(_goalStateIDs[i].first == _goals_c[i].first,
               "xythetaPlannerImpl.InitializeHeuristic.IdMismatch");
    ...
    if (_context.env.IsInSoftCollision(goalIDPair.second)) {
      ...
      PRINT_NAMED_INFO(
          "xythetaPlanner.GoalInSoftCollision.ExpandHeur",
          "expanded penalty states near goal %d. Cost outside map = %f",
          (int)goalIDPair.first, costOutsideHeurMap);
    }
    if (costOutsideHeurMap > MAX_COST_HEUR_EXPANSIONS) {
      PRINT_NAMED_WARNING(
          "xythetaPlanner.HeurMap.Fail",
          "very high cost of _costOutsideHeurMap = %f, so removing goal %d",
          costOutsideHeurMap, (int)goalIDPair.first);

      // remove the i-th. we already checked back() (or this is back), so swap
      // with back and pop the new back things will be out of order, but we
      ...
      std::swap(_costOutsideHeurMapPerGoal[i],
                _costOutsideHeurMapPerGoal.back());
  ...
  if (ANKI_DEVELOPER_CODE) {
    // _goals_c _costOutsideHeurPerGoal and _goalStateIDs should have GoalIDs in
    // matching order
...
Cost xythetaPlannerImpl::ExpandCollisionStatesFromGoal(
  ...
  while (!q.empty()) {
    ...
    if (!_context.env.IsInSoftCollision(curr)) {
      ...
      if (numEscapes >= COLLISION_GOAL_HEURISTIC_NUM_ESCAPES) {
        PRINT_NAMED_INFO("xythetaPlanner.ExpandCollisionStatesFromGoal",
                         "expanded %u states for heuristic, reached free space "
                         "%d times with cost of %f and have %lu in heurMap",
                         heurExpansions, numEscapes, minCostOutside,
                         (unsigned long)_heurMap.size());
        return minCostOutside;
      }

      // NOTE: need more testing to prove to myself that allowing any
      // COLLISION_GOAL_HEURISTIC_NUM_ESCAPES > 1 actually works, and is helpful
      // at all.
    } else {
    ...
    // this logic is almost identical to ExpandState, but using reverse
    // successors
    SuccessorIterator it = _context.env.GetSuccessors(curr, c, true);

    if (!it.Done(_context.env)) it.Next(_context.env);
    ...
    while (!it.Done(_context.env)) {
      ...
      // this might repeat work (no closed list), but who cares, this is a short
      // bit of code
    ...
    if (heurExpansions > MAX_HEUR_EXPANSIONS) {
      PRINT_NAMED_WARNING(
          "xythetaPlanner.ExpandCollisionStatesFromGoal.ExceededMaxExpansions",
          "exceeded max allowed expansions of %d", MAX_HEUR_EXPANSIONS);
      // instead of hanging forever, just return the cost calculated so far
      // TODO:(bn) if we implement ARA* or similar, we should return 0 here
      // instead
      return c;
    }
  }

  // We should usually be able to escape form a soft collision, but if we run
  // out of states to expand, it means we couldn't escape, but hopefully we
  // filled up heurMap enough to plan. This could happen if the start and goal
  // were both inside a soft obstacle that had a hard obstacle around it
  PRINT_NAMED_INFO(
      "xythetaPlanner.ExpandCollisionStatesFromGoal.EmptyOpenList",
...
bool xythetaPlannerImpl::ComputePath(unsigned int maxExpansions,
  ...
  if (newGoalIDs != _goalStateIDs) {
    ...
    // for now, replan if any goals change, but this isn't necessary. Instead,
    // we could just re-order the open list and continue as usual
    fromScratch = true;
  }
  ...
  // push starting state
  _table.emplace(startID, _open.insert(startID, 0.0), startID,
  ...
  while (!_open.empty()) {
    ...
    if (it != _goalState2GoalID.end()) {
      ...
      PRINT_NAMED_INFO("xythetaPlanner.ExpandGoal",
                       ...
                       (int)foundGoalID, (u32)foundGoalStateID, _finalCost);
      break;
    }
    ...
    if (_expansions > maxExpansions) {
      ...
      printf("topF =  %8.5f (%8.5f + %8.5f)\n", _open.topF(),
             _table[_open.top()].g_, _open.topF() - _table[_open.top()].g_);
      return false;
    }

    if (PLANNER_DEBUG_PLOT_STATES_CONSIDERED) {
      State_c c = _context.env.State2State_c(GraphState(sid));
      fprintf(_debugExpPlotFile, "%f %f %f %d\n", c.x_mm, c.y_mm, c.theta,
              sid.s.theta);
    }

    // TODO:(bn) make this a dev option / parameter
    if (_expansions % 10000 == 0) {
      PRINT_NAMED_INFO("xythetaPlanner.PLANDEBUG", "%8d %8.5f = %8.5f + %8.5f",
                       _expansions, _open.topF(), _table[_open.top()].g_,
  ...
  PRINT_NAMED_INFO("xythetaPlanner.Complete",
                   "finished after %d expansions. foundGoal = %d", _expansions,
                   foundGoal);
...
void xythetaPlannerImpl::ExpandState(StateID currID) {
  ...
  if (currTableEntry.closedIter_ == _searchNum) {
    PRINT_NAMED_ERROR("xythetaPlanner.ExpandingClosedState",
                      "This is a planner bug! Tell Brad immediately!");
    return;
  }
  ...
  while (!it.Done(_context.env)) {
    ...
    if (_context.allowFreeTurnInPlaceAtGoal) {
      for (const auto& goalPair : _goalStateIDs) {
        if ((currID.s.x == goalPair.second.s.x) &&
            (currID.s.y == goalPair.second.s.y)) {
          newG = currG;
          break;
        }
      }
    ...
    if (oldEntry == _table.end()) {
      ...
      Cost h = heur(nextID);
      Cost f = newG + h;
      _table.emplace(nextID, _open.insert(nextID, f), currID,
                     it.Front().actionID, it.Front().penalty, newG);
    }
    // TODO:(bn) opt: delay computing the cost. If the node is closed, don't
    // need to compute it. As I'm computing it, pass in the oldEntry g value,
    // because as soon as we hit that, we could bail out early
      ...
      if (newG < oldEntry->second.g_) {
        ...
        // if the states are in the table, then they were in the open list at
        // some point. Since they aren't closed now, they must still be in Open,
        // so remove the old entry to insert the new one
...
Cost xythetaPlannerImpl::heur_internal(StateID sid) {
  ...
  for (size_t i = 0; i < len; ++i) {
    ...
    Cost d = State_c::GetDistanceBetween(goal, s) *
                 _context.env.GetActionSpace().GetOneOverMaxVelocity() +
             costOutsideHeurMap;
    if (d < minCost) {
      minCost = d;
    }
...
void xythetaPlannerImpl::BuildPlan() {
  ...
  PRINT_NAMED_INFO("xythetaPlanner.BuildPlan", "Created plan of length %lu",
                   (unsigned long)_plan.Size());
}

bool xythetaPlannerImpl::AStar(unsigned int maxExpansions,
                               volatile bool* runPlan) {
  _runPlan = runPlan;
  ...
  // search loop
  while (!foundGoal && !_open2.empty() && !(_runPlan && !*_runPlan) &&
         remainingExpansions) {
  ...
  if (foundGoal) {
    ...
    PRINT_NAMED_INFO(
        "xythetaPlanner.CompleteA*",
        "finished after %d expansions. foundGoal = %d, open size = %zu",
...
// NOTE: this is technically not an admissable heuristic, could result in
// slightly suboptimal paths if we care
inline Cost xythetaPlannerImpl::heur_octile(GraphState s) {
  ...
  for (size_t i = 0; i < len; ++i) {
    ...
    Cost h = (std::fmax(d.x(), d.y()) + kDiagScale * std::fmin(d.x(), d.y())) *
             _context.env.GetActionSpace().GetOneOverMaxVelocity();
...
inline void xythetaPlannerImpl::ExpandState_New(const PlannerState& currState) {
  SuccessorIterator it =
      SuccessorIterator(&_context.env, currState.curr, currState.g, false);
  it.Next(_context.env);
  ...
  while (!it.Done(_context.env)) {
    ...
    // if successor has been expanded, don't insert. This saves us some extra
    // heap inserts and memory.
    ...
    // (MRW) alternatively, if we are allowing free turn in place at the end, we
    // can have the goal check conditionally
    //       exclude  theta, rather than check all the goals here. We just need
    //       to add a point turn at the end of searching
    if (_context.allowFreeTurnInPlaceAtGoal) {
      for (const auto& goalPair : _goalStateIDs) {
        if ((currState.curr.s.x == goalPair.second.s.x) &&
            (currState.curr.s.y == goalPair.second.s.y)) {
          newG = currState.g;
          break;
        }
      }
    ...
    // (MRW) consider normalizing the cost here from an exact time to traverse,
    // to some linear scale of this. if we consider a scale by Max Velocity, we
    // can remove a multiplacation here, and exact soft obstacle penalties and
    // reverse penelaties will still have the same compute cost. The problem is
    // just that the cost is less intuitively obvious. We could even provide a
    // helper method to convert planner cost to traversal time to help with
    // debugging/human reading

    // convert heuristic which is a distance to a time-to-traverse cost
    _open2.emplace(it.Front().stateID, currState.curr, it.Front().actionID,
                   it.Front().penalty, newG, heur_octile(it.Front().stateID));
    it.Next(_context.env);
  }
  ...
  if (((_expansions >> 13) & 1) == 1) {  // every 8k iterations
    PRINT_NAMED_INFO("xythetaPlanner.PLANDEBUG",
                     "%8d expansions, cost top: %8.5f", _expansions,
                     _open2.top().g);
  }
#endif
}
...
void xythetaPlannerImpl::BuildPlan_New(const PlannerState& goal) {
  ...
  assert(it !=
         _goalState2GoalID
             .end());  // this should only have been called if goal was found.
  ...
  PRINT_NAMED_INFO("xythetaPlanner.BuildPlan", "Created plan of length %zu",
                   _plan.Size());
}
