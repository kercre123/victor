...
RobotActionParams::RobotActionParams() { Reset(); }

void RobotActionParams::Reset() {
  // TODO:(bn) get this passed in somehow during a constructor somehweres. its
  // in common config
...
bool RobotActionParams::Import(const Json::Value& config) {
  ...
  try {
    if (config.isNull()) {
      PRINT_NAMED_ERROR("RobotActionParams.Import.Null",
                        "config value is null");
      return false;
    }

    halfWheelBase_mm =
        config.get("halfWheelBase_mm", halfWheelBase_mm).asFloat();
    maxVelocity_mmps =
        config.get("maxVelocity_mmps", maxVelocity_mmps).asFloat();
    maxReverseVelocity_mmps =
        config.get("maxReverseVelocity_mmps", maxReverseVelocity_mmps)
            .asFloat();

    if (NEAR_ZERO(maxVelocity_mmps)) {
      PRINT_NAMED_ERROR("RobotActionParams.Import.BadVelocity",
                        "maxVelocity_mmps of %f too close to 0",
                        maxVelocity_mmps);
