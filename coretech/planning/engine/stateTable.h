...
// TODO:(bn) pull out basic defined from environment, so we don't need the whole
// thing
...
struct StateEntry {
  StateEntry(OpenList::iterator openIt, StateID backpointer,
             ActionID backpointerAction, Cost penalty, Cost g)
      : openIt_(openIt),
        closedIter_(-1),
  ...
  StateEntry() : closedIter_(-1), penaltyIntoState_(0.0), g_(-1.0) {
    printf(
        "WARNING: default StateEntry constructor called. This is a performance "
        "bug\n");
  }
...
class StateTable {
 public:
  ...
  void emplace(StateID sid, OpenList::iterator openIt, StateID backpointer,
               ActionID backpointerAction, Cost penalty, Cost g);
