 ...
 * Date:   2016-6-17
 *
 * Description: Functions which make path creation easier using a coordinate
...
class RelativePathLine : public IRelativePathSegment {
 public:
  ...
  virtual Pose3d ExtendPath(const Pose3d& pose,
                            Planning::Path& path) const override;
...
class RelativePathArc : public IRelativePathSegment {
 public:
  RelativePathArc(f32 radius, f32 sweepRad, f32 targetSpeed, f32 accel,
                  f32 decel, bool clockwise);
  virtual Pose3d ExtendPath(const Pose3d& pose,
                            Planning::Path& path) const override;
...
class RelativePathTurn : public IRelativePathSegment {
 public:
  RelativePathTurn(f32 rotRadians, f32 targetRotSpeed, f32 rotAccel,
                   f32 rotDecel, f32 angleTolerance, bool clockwise);
  virtual Pose3d ExtendPath(const Pose3d& pose,
                            Planning::Path& path) const override;
