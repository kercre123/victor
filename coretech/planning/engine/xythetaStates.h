...
class GraphState {
 ...
 public:
  ...
  GraphState(GraphXY x, GraphXY y, GraphTheta theta)
      : x(x), y(y), theta(theta){};
  ...
  inline Point2f GetPointXY_mm() const {
    return Point2f(resolution_mm_ * x, resolution_mm_ * y);
  }

  static constexpr u8 numAngles_ = 1 << THETA_BITS;
  static constexpr float resolution_mm_ = 10;
  static constexpr float oneOverResolution_ =
      (float)(1.0 / ((double)resolution_mm_));

  // NOTE: these are approximate. The actual angles are calculated relative to
  // exterior points on a 5x5 grid,
  //       and they are loaded along with motion primitives
  static constexpr float radiansPerAngle_ = 2 * M_PI / numAngles_;
  static constexpr float oneOverRadiansPerAngle_ =
      (float)(1.f / ((double)radiansPerAngle_));
};

// bit field representation that packs into an int useful for checking the open
// and closed sets for expanded states NOTE: this is separated from State in
// case state needs to contain more information than the 32-bit form can hold,
...
static_assert(sizeof(StateID::S) == sizeof(u32),
              "StateID must pack into 32 bits");
...
class State_c {
 ...
 public:
  ...
  inline GraphTheta GetGraphTheta() const {
    return (GraphTheta)round(Util::ClampAngle2PI(theta) *
  ...
  // TODO: store these as Point2f natively to avoid construction? we almost
  // always use both as a point anyway
  float x_mm;
  float y_mm;
  float theta;
};
