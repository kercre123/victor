 ...
 * Created: 2018-07-26
 *
 * Description: Bidirectional AStar implementation for early termination of
 *infeasible paths
 *
...
//    We use a template type for the implentation to allow for compile time
//    polymorphism of an abstract class to avoid the overhead of virtual
//    functions.
...
class BidirectionalAStarConfig {
 public:
  ...
  decltype(auto) GetSuccessors(const T& p) const {
    return static_cast<Impl const&>(*this).GetSuccessors(p);
  }
  float ForwardHeuristic(const T& p) const {
    return static_cast<Impl const&>(*this).ForwardHeuristic(p);
  }
  float ReverseHeuristic(const T& p) const {
    return static_cast<Impl const&>(*this).ReverseHeuristic(p);
  }
  bool StopPlanning() { return static_cast<Impl&>(*this).StopPlanning(); }

  const T& GetStart() const {
    return static_cast<Impl const&>(*this).GetStart();
  }
  const std::vector<T>& GetGoals() const {
    return static_cast<Impl const&>(*this).GetGoals();
  }

 private:
  // use private constructor and friend class to prevent accidentally templating
  // on wrong derived class
...
class BidirectionalAStar {
  using T =
      std::decay_t<std::result_of_t<decltype (&ConfigT::GetStart)(ConfigT)> >;
 ...
 public:
  ...
  // initialization, search loop, and plan construction of classical A*
  // implementation
  auto Search();

 private:
  // given a state in the closed list, follow the backpointers to the start
  ...
  std::function<bool(const Record&, const Record&)> StateEqual =
      [](const Record& s, const Record& t) {
        return std::equal_to<T>{}(s.state, t.state);
      };

  using ClosedList =
      std::unordered_set<Record, decltype(StateHasher), decltype(StateEqual)>;
...
auto BidirectionalAStar<ConfigT>::Search() {
  ...
  _openForward.emplace({_config.GetStart(), _config.GetStart(), 0,
                        _config.ForwardHeuristic(_config.GetStart())});
  ...
  // search loop
  bool isForward = true;
  while (!_config.StopPlanning() && !foundGoal && !_openForward.empty() &&
         !_openReverse.empty()) {
    ...
    if (closedRecord.second) {
      ...
      for (const Iter& succ : _config.GetSuccessors(current->state)) {
        ...
        float f = newCost + (isForward ? _config.ForwardHeuristic(succ.state)
                                       : _config.ReverseHeuristic(succ.state));
  ...
  return (foundGoal && (current != _closedForward.end()))
             ? GetPlan(current->state)
             : std::vector<T>();
}
