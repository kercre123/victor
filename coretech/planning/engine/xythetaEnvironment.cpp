...
SuccessorIterator::SuccessorIterator(const xythetaEnvironment* env,
                                     StateID startID, Cost startG, bool reverse)
...
void SuccessorIterator::Next(const xythetaEnvironment& env) {
  ...
  while (nextAction_ < numActions) {
    const MotionPrimitive* prim = (reverse_)
                                      ? &env.allActions_.GetReverseMotion(
                                            (size_t)start_.theta, nextAction_)
                                      : &env.allActions_.GetForwardMotion(
                                            (size_t)start_.theta, nextAction_);
    ...
    bool reverseMotion =
        env.GetActionSpace().GetActionType(prim->id).IsReverseAction();
    ...
    if (env.obstacleBounds_.empty() && !env.obstaclesPerAngle_[0].empty()) {
      ...
      PRINT_NAMED_WARNING("xythetaEnvironment.Successor.NoBounds",
                          "missing obstacle bounding boxes! Did you call "
                          "env.PrepareForPlanning()???");
      possibleObstacle = true;
    } else {
      for (const auto& bound : env.obstacleBounds_) {
        if (maxPrimX < bound.minX || minPrimX > bound.maxX ||
            maxPrimY < bound.minY || minPrimY > bound.maxY) {
    ...
    if (possibleObstacle) {
      // two collision check cases. If the angle is changing, then we'll need to
      // potentially switch which obstacle angle we check while checking, so
      // that is the more complciated case

      // First, handle the simpler case, for straight lines. In this case, we
      // can do a quick bounding box check first
        ...
        for (long pointIdx = endPoints - 1; pointIdx >= 0; --pointIdx) {
          ...
          for (const auto& obs : env.obstaclesPerAngle_[angle]) {
            if (obs.first.Contains(
                    primtiveOffset.x_mm +
                        prim->intermediatePositions[pointIdx].position.x_mm,
                    primtiveOffset.y_mm +
                        prim->intermediatePositions[pointIdx].position.y_mm)) {
              if (obs.second >= MAX_OBSTACLE_COST) {
                collision = true;
                break;
              } else {
                // apply soft penalty, but allow the action
                penalty += obs.second * prim->intermediatePositions[pointIdx]
                                            .oneOverDistanceFromLastPosition +
                           (reverseMotion ? REVERSE_OVER_OBSTACLE_COST : 0);
...
Cost xythetaEnvironment::CheckAndApplyAction(const ActionID& action,
  ...
  if (action >= allActions_.GetNumActions()) return FATAL_OBSTACLE_COST;

  const MotionPrimitive* prim =
      &allActions_.GetForwardMotion(curr.theta, action);
  ...
  for (size_t point = 0; point < endPoints; ++point) {
    ...
    for (size_t obs = 0; obs < endObs; ++obs) {
      const Point2f intermediatePoint =
          prim->intermediatePositions[point].position.GetPointXY_mm();
      if (obstaclesPerAngle_[angle][obs].first.Contains(start +
                                                        intermediatePoint)) {
        penalty +=
            obstaclesPerAngle_[angle][obs].second *
...
Cost xythetaEnvironment::CheckAndApplyPathSegment(
    const PathSegment& pathSegment, State_c& state_c, float maxPenalty) const {
  Cost penalty = 0.0f;
  ...
  // since mprim.json uses an intermediate step size of 0.25 in positon, and
  // PI/32 in orientation, find a step size that matches the smaller of those
  const float kPrimPositionStepSizeRecip = 4.0f;
  const float kPrimAngleStepSizeRecip = 32 / M_PI_F;
  const size_t numPointsPosition =
      std::ceil(1 + pathSegment.GetLength() * kPrimPositionStepSizeRecip);
  const size_t numPointsAngle = std::ceil(
      Anki::Util::Abs(
          Radians(segmentDef.turn.targetAngle - start_theta).ToFloat()) *
      kPrimAngleStepSizeRecip);
  // number of points, including start and end
  const size_t numIntermediatePoints = Anki::Util::Max(
      (size_t)2, Anki::Util::Max(numPointsAngle, numPointsPosition));
  DEV_ASSERT(
      numIntermediatePoints > 1,
      "Number of intermediate points for ApplyPathSegments must be >= 2");

  float dx = 0.0f;
  float dy = 0.0f;
  float dtheta = 0.0f;
  const float step = 1.0f / (numIntermediatePoints - 1);
  const float oneOverDistFromLastPosition =
      (PST_POINT_TURN == segmentType)
  ...
  switch (segmentType) {
    case PST_POINT_TURN: {
      dtheta =
          step * Radians(segmentDef.turn.targetAngle - start_theta).ToFloat();
      break;
    }
    case PST_LINE: {
    ...
    default:
      PRINT_NAMED_ERROR("xythetaEnvironment.ApplyPathSegment",
                        "Undefined segment %d", segmentType);
      assert(false);
  ...
  for (size_t i = 1; i < numIntermediatePoints; ++i) {
    if (segmentType == PST_POINT_TURN) {
      ...
      intermediate_c.x_mm =
          segmentDef.arc.centerPt_x +
          segmentDef.arc.radius * cosf(segmentDef.arc.startRad + dtheta * i);
      intermediate_c.y_mm =
          segmentDef.arc.centerPt_y +
          segmentDef.arc.radius * sinf(segmentDef.arc.startRad + dtheta * i);
      intermediate_c.theta = start_theta + dtheta * i;
      intermediateTheta = intermediate_c.GetGraphTheta();
    }
    ...
    for (size_t obs = 0; obs < endObs; ++obs) {
      if (obstaclesPerAngle_[intermediateTheta][obs].first.Contains(
              intermediate_c.x_mm, intermediate_c.y_mm)) {
        penalty += obstaclesPerAngle_[intermediateTheta][obs].second *
                   oneOverDistFromLastPosition;
...
bool xythetaEnvironment::PlanIsSafe(const xythetaPlan& plan,
  ...
  State_c waste1;
  xythetaPlan waste2;
  return PlanIsSafe(plan, maxDistancetoFollowOldPlan_mm, currentPathIndex,
                    waste1, waste2);
}
...
bool xythetaEnvironment::PlanIsSafe(const xythetaPlan& plan,
  ...
  for (size_t i = currentPathIndex; i < numActions; ++i) {
    ...
    if (actionPenalty > plan.GetPenalty(i) + REPLAN_PENALTY_BUFFER) {
      printf(
          "Collision along plan action %lu (starting from %d) Penalty "
          "increased from %f to %f\n",
          (unsigned long)i, currentPathIndex, plan.GetPenalty(i),
          actionPenalty);
    ...
    if (useOldPlan) {
      ...
      if (State_c::GetDistanceBetween(lastSafeState, currentRobotState) >
...
bool xythetaEnvironment::PathIsSafe(const Planning::Path& path,
                                    float startAngle,
  ...
  for (size_t i = 0; i < numSegments; ++i) {
    ...
    // check for collisions and possibly update curr
    Cost penalty =
        CheckAndApplyPathSegment(currSegment, currState_c, maxPenalty);
    totalPenalty += penalty;

    if (totalPenalty > maxPenalty) {
      PRINT_NAMED_INFO(
          "xythetaEnvironment.PathIsSafe",
          "Collision along path segment %lu with penalty %f, total penalty %f",
          (unsigned long)i, penalty, totalPenalty);
      // there was a collision trying to follow action i, so we are done
      return false;
    }
...
void xythetaEnvironment::PrepareForPlanning() {
  ...
  for (size_t angle = 0; angle < GraphState::numAngles_; ++angle) {
    for (int obsIdx = 0; obsIdx < obstaclesPerAngle_[angle].size(); ++obsIdx) {
      if (obstaclesPerAngle_[angle][obsIdx].first.GetMinX() <
          obstacleBounds_[obsIdx].minX) {
        obstacleBounds_[obsIdx].minX =
            obstaclesPerAngle_[angle][obsIdx].first.GetMinX();
      }
      if (obstaclesPerAngle_[angle][obsIdx].first.GetMaxX() >
          obstacleBounds_[obsIdx].maxX) {
        obstacleBounds_[obsIdx].maxX =
            obstaclesPerAngle_[angle][obsIdx].first.GetMaxX();
      }

      if (obstaclesPerAngle_[angle][obsIdx].first.GetMinY() <
          obstacleBounds_[obsIdx].minY) {
        obstacleBounds_[obsIdx].minY =
            obstaclesPerAngle_[angle][obsIdx].first.GetMinY();
      }
      if (obstaclesPerAngle_[angle][obsIdx].first.GetMaxY() >
          obstacleBounds_[obsIdx].maxY) {
        obstacleBounds_[obsIdx].maxY =
            obstaclesPerAngle_[angle][obsIdx].first.GetMaxY();
      }
    }
  }
}
...
bool xythetaEnvironment::Init(const char* mprimFilename) {
  if (ReadMotionPrimitives(mprimFilename)) {
    ...
    PRINT_NAMED_ERROR("xythetaEnvironemnt.Init.Fail",
                      "could not parse motion primitives");
    return false;
  }
...
bool xythetaEnvironment::Import(const Json::Value& config) {
  try {
    if (config.isNull()) {
      PRINT_NAMED_ERROR("xythetaEnvironment.Import.Null",
  ...
  try {
    if (GraphState::numAngles_ == 0 || config["angles"].isNull()) {
      PRINT_NAMED_ERROR("xythetaEnvironment.ParseObstacles.InvalidObjectAngles",
                        "numAngles_ = %d", GraphState::numAngles_);
      return false;
    }
    ...
    if (GraphState::numAngles_ != config["angles"].size()) {
      PRINT_NAMED_ERROR("xythetaEnvironment.ParseObstacles.AnglesMismatch",
                        ...
                        GraphState::numAngles_, config.size());
      return false;
    }
    ...
    for (int theta = 0; theta < GraphState::numAngles_; ++theta) {
      ...
      for (const auto& obsConfig : config["angles"][theta]["obstacles"]) {
        ...
        for (const auto& ptConfig : obsConfig["poly"]) {
          p.push_back(
              Point2f{ptConfig["x"].asFloat(), ptConfig["y"].asFloat()});
        }
    ...
    PRINT_NAMED_ERROR("ParseObstacles.Exception", "json exception: %s",
                      e.what());
    return false;
  }
...
void xythetaEnvironment::AddObstacleAllThetas(const RotatedRectangle& rect,
                                              Cost cost) {
  Poly2f poly(rect);
  FastPolygon fastPoly(poly);
...
FastPolygon xythetaEnvironment::ExpandCSpace(const ConvexPolygon& obstacle,
  ...
  // VIC-1702: Minkowski sum implementation is subject to floating point errors,
  // so take the convex hull for now
  return FastPolygon{ConvexPolygon::ConvexHull(std::move(expansion))};
}

const FastPolygon& xythetaEnvironment::AddObstacleWithExpansion(
    const ConvexPolygon& obstacle, const ConvexPolygon& robot, GraphTheta theta,
    Cost cost) {
  if (theta >= obstaclesPerAngle_.size()) {
    CoreTechPrint(
        "ERROR: theta = %d, but only have %zu obstacle angles and %u angles "
        "total\n",
        theta, obstaclesPerAngle_.size(), GraphState::numAngles_);

    theta = 0;
  }

  obstaclesPerAngle_[theta].emplace_back(
      std::make_pair(ExpandCSpace(obstacle, robot), cost));

  return obstaclesPerAngle_[theta].back().first;
}

bool xythetaEnvironment::RoundSafe(const State_c& c,
                                   GraphState& rounded) const {
  ...
  for (GraphXY x = startX; x <= endX; ++x) {
    for (GraphXY y = startY; y <= endY; ++y) {
      ...
      if (!IsInCollision(candidate)) {
        float dist2 =
            (candidate.GetPointXY_mm() - c.GetPointXY_mm()).LengthSq();
        if (dist2 < bestDist2) {
          bestDist2 = dist2;
          rounded.x = x;
          rounded.y = y;
...
SuccessorIterator xythetaEnvironment::GetSuccessors(StateID startID, Cost currG,
                                                    bool reverse) const {
...
size_t xythetaEnvironment::FindClosestPlanSegmentToPose(const xythetaPlan& plan,
                                                        ...
                                                        bool debug) const {
  // Ideally, we have a position which is within the discretization error of the
  // planner. If so, use the lowest index that fits within the error. We use the
  // lowest to avoid chopping off point-turn actions.

  // Otherwise, that means we are further away, so just find the closest (x,y)
  // point, ignoring theta since we will need to drive onto the path anyway.

  using namespace std;
  if (debug) {
    cout << "Searching for position near " << state << " along plan of length "
         << plan.Size() << endl;
  }
  ...
  // First search *just* the action starting points. If there is an exact match
  // here, that's the winner. Also, keep track of closest distances in case
  ...
  for (size_t planIdx = 0; planIdx < planSize; ++planIdx) {
    ...
    // TODO:(bn) no sqrt!
    const float initialXYDist =
        (state.GetPointXY_mm() - currPlanState.GetPointXY_mm()).Length();
  ...
  // if we get here, it means we didn't find a starting point which was an exact
  // match. Check to see if any intermediate points are exact matches. If so,
  // return the starting point before it, otherwise just return the closest xy
  // distance point
  currPlanState = plan.start_;
  for (size_t planIdx = 0; planIdx < planSize; ++planIdx) {
    // the intermediate position (x,y) coordinates are all centered at 0.
    // Instead of shifting them over each time, we just shift the state we are
    // searching for (fewer ops). (theta doesn't matter here)
    const Point2f offset =
        state.GetPointXY_mm() - currPlanState.GetPointXY_mm();
    const State_c offsetToPlanState(offset.x(), offset.y(), state.theta);

    const MotionPrimitive& prim = allActions_.GetForwardMotion(
        currPlanState.theta, plan.GetAction(planIdx));

    // skip the last intermediate position, since it overlaps with the next
    // start
    size_t intermediatePositionsSize = prim.intermediatePositions.size();
    for (size_t intermediateIdx = 0;
         intermediateIdx + 1 < intermediatePositionsSize; ++intermediateIdx) {
      ...
      const float xyDist =
          State_c::GetDistanceBetween(offsetToPlanState, intermediate.position);
