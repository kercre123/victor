...
GTEST_TEST(TestPath, GetDistToArcSegment) {
  ...
  p.AppendArc(x_center_mm, y_center_mm, pathRadius_mm, 0, M_PI_F, targetSpeed,
              accel, decel);
  p.AppendArc(x_center_mm, y_center_mm, pathRadius_mm, M_PI_F, M_PI_F,
  ...
  int currSegment = 0;
  for (float angleWrtCenter_rad = 0.f; angleWrtCenter_rad < 2 * M_PI_F;
       angleWrtCenter_rad += kAngularStep_rad) {
    ...
    auto status = segment.GetDistToSegment(x, y, angleWrtCenter_rad + M_PI_2_F,
                                           shortestDistToPath_mm, angDiff_rad,
                                           &distToEndOfPath_mm);
    ...
    if (status == SegmentRangeStatus::OOR_NEAR_END) {
      ...
      status = segment.GetDistToSegment(x, y, angleWrtCenter_rad + M_PI_2_F,
                                        shortestDistToPath_mm, angDiff_rad,
                                        &distToEndOfPath_mm);
    }

    EXPECT_TRUE(
        NEAR(shortestDistToPath_mm, -kRadialOffset_mm, kShortestDistTol_mm));
