...
bool CheckPlanAsPathIsSafe(const xythetaPlannerContext& context,
                           xythetaPlan& plan, Path* validPath = nullptr) {
  Path path;
  context.env.GetActionSpace().AppendToPath(plan, path, 0);
  Path wasteValidPath;
  const float startAngle =
      context.env.GetActionSpace().LookupTheta(plan.start_.theta);
  return context.env.PathIsSafe(
      path, startAngle, (nullptr != validPath) ? *validPath : wasteValidPath);
}
...
GTEST_TEST(TestPlanner, PlanOnceEmptyEnv) {
  ...
  // TEMP:  // TODO:(bn) read context params from new json, instead of loading a
  // different file manually here

  // TODO:(bn) open something saved in the test dir isntead, so we
  // know not to change or remove it
  EXPECT_TRUE(context.env.ReadMotionPrimitives(
...
GTEST_TEST(TestPlanner, PlanTwiceEmptyEnv) {
  ...
  // NOTE: this is a lazy way to see that the plans differ. If the motion
  // primitives change, this could break
...
GTEST_TEST(TestPlanner, PlanOnceMultipleGoalsEmptyEnv) {
  ...
  goals.emplace_back(
      6, State_c{-50.0f, 1.0f,
                 -1.5f});  // a movement of 50 mm from the start, but with turns
  goals.emplace_back(
      expectedBest,
      State_c{42.1f, 27.99f,
              0.57f});  // a movement of 50 mm straight from the start
...
GTEST_TEST(TestPlanner, PlanThriceMultipleGoalsEmptyEnv) {
  ...
  // now the previous best (2, at index 1) is slightly farther away than GoalID
  // 6, but still has fewer turns than 6
...
GTEST_TEST(TestPlanner, PlanWithMaxExps) {
  ...
  EXPECT_FALSE(planner.Replan(firstPlanExps - 1))
      << "planning should fail with limited expansions";
}
...
GTEST_TEST(TestPlanner, PlanAroundBox) {
  ...
  context.env.AddObstacleAllThetas(
...
GTEST_TEST(TestPlanner, PlanAroundBoxDumpAndImportContext) {
  ...
  EXPECT_TRUE(context2.env.ReadMotionPrimitives(
      (std::string(QUOTE(TEST_DATA_PATH)) + std::string(TEST_PRIM_FILE))
          .c_str()));
  ...
  // now check that the plans match. I'm not doing floating point equality here,
  // so they may be a bit different, but not much
  ...
  for (int a = 0; a < planner.GetPlan().Size(); ++a) {
    ASSERT_EQ(planner.GetPlan().GetAction(a), planner2.GetPlan().GetAction(a))
        << "action # " << a << " mismatches";
    ASSERT_NEAR(planner.GetPlan().GetPenalty(a),
                planner2.GetPlan().GetPenalty(a), tol)
...
GTEST_TEST(TestPlanner, PlanAroundBox_soft) {
  ...
  for (size_t i = 0; i < plannerSize; ++i) {
    if (context.env.GetRawMotion(0, planner.GetPlan().GetAction(i))
            .endStateOffset.theta != 0) {
      hasTurn = true;
      break;
    }
  }
  ...
  context.env.AddObstacleAllThetas(
      Anki::RotatedRectangle(50.0, -10.0, 80.0, -10.0, 20.0), 50.0);
  ...
  EXPECT_FLOAT_EQ(highCost, fatalCost)
      << "cost should be the same with fatal or high cost obstacle";

  context.env.ClearObstacles();
  // now add it with a very low cost
  context.env.AddObstacleAllThetas(
      Anki::RotatedRectangle(50.0, -10.0, 80.0, -10.0, 20.0), 1e-4);
  ...
  plannerSize = planner.GetPlan().Size();
  for (size_t i = 0; i < plannerSize; ++i) {
    ASSERT_EQ(context.env.GetRawMotion(0, planner.GetPlan().GetAction(i))
                  .endStateOffset.theta,
              0)
        << "with low cost, should drive straight through obstacle, but plan "
           "has a turn!";
  }

  Cost lowCost = planner.GetFinalCost();

  EXPECT_LT(lowCost, highCost)
      << "should be cheaper to drive through obstacle than around it";
  ...
  EXPECT_LT(emptyCost, lowCost)
      << "no obstacle should be cheaper than any obstacle";
}
...
GTEST_TEST(TestPlanner, MultipleGoalsWithSoftPenalty) {
  ...
  GoalState_cPairs goals{
      {0, {200, 0, 0}},    // closer, but with varying penalties (see below)
      {1, {1000, 0, 0}}};  // farther, no penalty
  Anki::RotatedRectangle obstacleOnGoal0(150.0, -10.0, 250.0, -10.0,
                                         20.0);  // covers 0th goal
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    ASSERT_TRUE(planner.Replan());
    EXPECT_TRUE(context.env.PlanIsSafe(planner.GetPlan(), 0));
    EXPECT_EQ(CheckPlanAsPathIsSafe(context, planner.GetPlan()),
              !pathClipsObstacle[i]);
  ...
  EXPECT_FLOAT_EQ(planCosts[0], planCosts[1])
      << "First two plans unaffected by goal 0's penalty";
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    if (validGoals[i]) {
      ...
      EXPECT_TRUE(planFinished[i] ==
                  context.env.PlanIsSafe(planner.GetPlan(), 0));
      EXPECT_EQ(CheckPlanAsPathIsSafe(context, planner.GetPlan()),
                !pathClipsObstacle[i]);
...
GTEST_TEST(TestPlanner, ReplanEasy) {
  ...
  context.env.AddObstacleAllThetas(
      Anki::RotatedRectangle(50.0, -100.0, 80.0, -100.0, 20.0));

  EXPECT_TRUE(context.env.PlanIsSafe(planner.GetPlan(), 0))
      << "new obstacle should not interfere with plan";
  EXPECT_TRUE(CheckPlanAsPathIsSafe(context, planner.GetPlan()));
...
GTEST_TEST(TestPlanner, ReplanHard) {
  ...
  EXPECT_FALSE(context.env.PlanIsSafe(planner.GetPlan(), 0))
      << "new obstacle should block plan!";
  EXPECT_FALSE(CheckPlanAsPathIsSafe(context, planner.GetPlan()))
      << "new obstacle should block path too!";

  State_c newRobotPos(137.9, -1.35, 0.0736);
  ASSERT_FALSE(context.env.IsInCollision(newRobotPos))
      << "position " << newRobotPos << " should be safe";
  ...
  int currentPlanIdx =
      static_cast<int>(context.env.FindClosestPlanSegmentToPose(
          planner.GetPlan(), newRobotPos, distFromPlan));
  ASSERT_EQ(currentPlanIdx, 2) << "should be at action #2 in the plan (plan "
                                  "should start with 3 long actions) d="
                               << distFromPlan;

  EXPECT_LT(distFromPlan, 15.0) << "too far away from plan";

  ASSERT_FALSE(context.env.PlanIsSafe(planner.GetPlan(), 1000.0, currentPlanIdx,
                                      lastSafeState, oldPlan));
  ...
  ASSERT_GE(oldPlan.Size(), 1)
      << "should re-use at least one action from the old plan";
  ASSERT_GE(oldPath.GetNumSegments(), 0)
      << "path is just one line but it should not entirely be invalidated";

  StateID endID = context.env.GetActionSpace().GetPlanFinalState(oldPlan);

  ASSERT_EQ(endID, GraphState(lastSafeState).GetStateID())
      << "end of validOldPlan should match lastSafeState!";
...
// NOTE: disabled this test because I changed the code to round differently. I
// replaced it with a similar but worse test ClosestSegmentToPose_straight2
GTEST_TEST(TestPlanner, DISABLED_ClosestSegmentToPose_straight) {
  ...
  ASSERT_EQ(context.env.GetRawMotion(0, 0).endStateOffset.x, 1)
      << "invalid action";
  ASSERT_EQ(context.env.GetRawMotion(0, 0).endStateOffset.y, 0)
      << "invalid action";
  ...
  for (float distAlong = 0.0f; distAlong < 12.0 * GraphState::resolution_mm_;
       distAlong += 0.7356 * GraphState::resolution_mm_) {
    ...
    ASSERT_EQ(context.env.FindClosestPlanSegmentToPose(planner.GetPlan(), pose,
                                                       distFromPlan, true),
              expected)
...
void TestPlanner_ClosestSegmentToPoseHelper(xythetaPlannerContext& context,
                                            xythetaPlanner& planner) {
  printf("manually created plan:\n");
  context.env.GetActionSpace().PrintPlan(planner.GetPlan());

  GraphState start = planner._impl->_plan.start_;

  // go through each intermediate point, perturb it a bit, and make sure it
  // returns correctly
  GraphState curr = start;

  size_t planSize = planner._impl->_plan.Size();
  for (size_t planIdx = 0; planIdx < planSize; ++planIdx) {
    const MotionPrimitive& prim(context.env.GetRawMotion(
        curr.theta, planner._impl->_plan.GetAction(planIdx)));

    float distFromPlan = 9999.0;

    ASSERT_EQ(planIdx, context.env.FindClosestPlanSegmentToPose(
                           planner.GetPlan(), context.env.State2State_c(curr),
                           distFromPlan))
        << "initial state wrong planIdx=" << planIdx
        << " distFromPlan=" << distFromPlan << " curr = " << curr;
    EXPECT_LT(distFromPlan, 15.0) << "too far away from plan";

    ASSERT_FALSE(prim.intermediatePositions.empty());

    // just check the first one, since I broke the tests with the new way I
    // round states
    size_t sizeToCheck = 1;  // prim.intermediatePositions.size() - 1;
    for (size_t intermediateIdx = 0; intermediateIdx < sizeToCheck;
         intermediateIdx++) {
      State_c pose(prim.intermediatePositions[intermediateIdx].position.x_mm +
                       (curr.x * GraphState::resolution_mm_),
                   prim.intermediatePositions[intermediateIdx].position.y_mm +
                       (curr.y * GraphState::resolution_mm_),
                   prim.intermediatePositions[intermediateIdx].position.theta);

      ASSERT_EQ(planIdx, context.env.FindClosestPlanSegmentToPose(
                             planner.GetPlan(), pose, distFromPlan))
    ...
    context.env.GetActionSpace().ApplyAction(
        planner._impl->_plan.GetAction(planIdx), curr);
  }
}
...
GTEST_TEST(TestPlanner, ClosestSegmentToPose_initial) {
  ...
  for (int startAngle = 0; startAngle < 16; ++startAngle) {
    ...
    // now test that various offsets that would round to the start state also
    // give index 0
    ...
    ASSERT_EQ(0, context.env.FindClosestPlanSegmentToPose(planner.GetPlan(), s,
                                                          distFromPlan))
        << "didn't get correct position for initial state. dist="
        << distFromPlan;
    ...
    for (float dx = -GraphState::resolution_mm_;
         dx < GraphState::resolution_mm_; dx += step) {
      for (float dy = -GraphState::resolution_mm_;
           dy < GraphState::resolution_mm_; dy += step) {
        State_c s = context.env.State2State_c(start);
        s.x_mm += dx;
        s.y_mm += dy;
        if (GraphState(s) == start) {
          ASSERT_EQ(0, context.env.FindClosestPlanSegmentToPose(
                           planner.GetPlan(), s, distFromPlan))
              << "didn't get correct position for offset (" << dx << ',' << dy
              << ") s=" << s << " dist=" << distFromPlan << " (" << numTested
              << " already passed)";
          numTested++;
        }
      }
...
GTEST_TEST(TestPlanner, ZeroSizePathFilterBug) {
  ...
  EXPECT_TRUE(context.env.ReadMotionPrimitives(
      (std::string(QUOTE(TEST_DATA_PATH)) + std::string(TEST_PRIM_FILE2))
          .c_str()));
