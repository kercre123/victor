 ...
 * Date:   7/2/2018
 *
 * Description: Defines interface and shared helpers for various NeuralNetModel
 *implementations.
 *
...
int GetCVTypeHelper(const uint8_t* intputType) { return CV_8UC2; }
int GetCVTypeHelper(const float* intputType) { return CV_32FC2; }

// Inline functions will call proper helper based on output type of network
inline static float QuantizedScalingHelper(const float output,
                                           const float scale,
                                           const int zero_point) {
  return output;
}

inline static float QuantizedScalingHelper(const uint8_t output,
                                           const float scale,
                                           const int zero_point) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Note that these must be implemented
// (even as default) here in the cpp file because of the use of a unique_ptr
// with
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result INeuralNetModel::LoadModel(const std::string& modelPath,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result INeuralNetModel::ReadLabelsFile(const std::string& fileName,
                                       std::vector<std::string>& labels_out) {
  std::ifstream file(fileName);
  if (!file) {
    PRINT_NAMED_ERROR("NeuralNetModel.ReadLabelsFile.LabelsFileNotFound", "%s",
                      fileName.c_str());
    return RESULT_FAIL;
  }
  ...
  LOG_DEBUG("NeuralNetModel.ReadLabelsFile.Success", "Read %d labels",
            (int)labels_out.size());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Small helper class used by
// ClassificationOutputHelper to do sliding windows of scores
class INeuralNetModel::SlidingWindow {
 public:
  SlidingWindow(size_t numFrames, float firstEntry)
      : index(0), scores(numFrames, 0.f) {
    scores[0] = firstEntry;
  }
  ...
  void GetAverageAboveThreshold(const float threshold, f32& average,
                                s32& numAboveThresh) const {
    average = 0.f;
    numAboveThresh = 0;
...
// - - - - - - - - - - - - - - - - - - - -
template <typename T>
void INeuralNetModel::ClassificationOutputHelper(
    const T* outputData, TimeStamp_t timestamp,
  ...
  DEV_ASSERT(!_labels.empty(),
             "INeuralNetModel.ClassificationOutputHelper.EmptyLabels");
  static const int minLabel =
      (Util::StringCaseInsensitiveEquals(_labels.front(), "background") ? 1
                                                                        : 0);
  ...
  if (labelIndex >= minLabel) {
    auto result = _slidingScoreWindows.emplace(
        labelIndex, new SlidingWindow(_params.numFrames, maxScore));
    if (!result.second) {
  ...
  for (auto iter = _slidingScoreWindows.begin();
       iter != _slidingScoreWindows.end(); ++iter) {
    if (iter != updated) {
  ...
  if (!_slidingScoreWindows.empty()) {
    // See if any sliding window has enough scores above the threshold to be
    ...
    for (const auto& entry : _slidingScoreWindows) {
      ...
      slidingWindow->GetAverageAboveThreshold(_params.minScore, avgScore,
                                              numAboveThresh);
      if (numAboveThresh >= _params.majority) {
        bestLabel = entry.first;
        break;
      }
      // TODO: consider removing entries for which all scores in the window are
      // zero?
    }
    ...
    if (bestLabel >= 0) {
      ...
      const std::string& label =
          (bestLabel < _labels.size() ? _labels.at((size_t)bestLabel)
                                      : "<UNKNOWN>");

      // Find a matching SalientPointType for the label if there is one.
      ...
      if (!Vision::SalientPointTypeFromString(label, salientType)) {
        LOG_WARNING("INeuralNetModel.ClassificationOutputHelper.UnknownLabel",
                    "Could not convert label '%s' to a SalientPointType. Using "
                    "Unknown.",
                    label.c_str());
      }
      ...
      if (_params.verbose) {
        LOG_INFO("INeuralNetModel.ClassificationOutputHelper.ObjectFound",
                 ...
                 salientPoint.description.c_str(), salientPoint.score,
                 salientPoint.timestamp);
      }

      salientPoints.push_back(std::move(salientPoint));
    } else if (_params.verbose) {
      LOG_INFO("INeuralNetModel.ClassificationOutputHelper.NoObjects",
               "MinScore: %f", _params.minScore);
    }
  }
}

// Explicitly instantiate for float and uint8
template void INeuralNetModel::ClassificationOutputHelper(
    const float*, TimeStamp_t, std::list<Vision::SalientPoint>&);
template void INeuralNetModel::ClassificationOutputHelper(
    const uint8_t*, TimeStamp_t, std::list<Vision::SalientPoint>&);

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
template <typename T>
void INeuralNetModel::LocalizedBinaryOutputHelper(
    const T* outputData, TimeStamp_t timestamp, const float scale,
    const int zero_point, std::list<Vision::SalientPoint>& salientPoints) {
  ...
  for (int i = 0; i < _detectionGrid.GetNumRows(); ++i) {
    ...
    for (int j = 0; j < _detectionGrid.GetNumCols(); ++j) {
      ...
      const float score =
          QuantizedScalingHelper(outputData[outputIndex], scale, zero_point);
  ...
  if (anyDetections) {
    // Because we want to get average score, we'll do our own "stats"
    // computation below instead of using ConnectedComponentsWithStats()
    const s32 count = _detectionGrid.GetConnectedComponents(_labelsGrid);
    DEV_ASSERT(
        (_detectionGrid.GetNumRows() == _labelsGrid.GetNumRows()) &&
            (_detectionGrid.GetNumCols() == _labelsGrid.GetNumCols()),
        "INeuralNetModel.LocalizedBinaryOutputHelper.MismatchedLabelsGridSize");

    if (_params.verbose) {
      LOG_INFO(
          "INeuralNetModel.LocalizedBinaryOutputHelper."
          "FoundConnectedComponents",
          "NumComponents: %d", count);
    }

    // Compute stats for each connected component
    struct Stat {
      int32_t area;  // NOTE: area will be number of grid squares, not area of
                     // bounding box shape
      int32_t scoreSum;
      Point2f centroid;
      int xmin, xmax, ymin, ymax;
    };
    std::vector<Stat> stats(count, {0,
                                    0,
                                    {0.f, 0.f},
                                    _detectionGrid.GetNumCols(),
                                    -1,
                                    _detectionGrid.GetNumRows(),
                                    -1});
    ...
    for (int i = 0; i < _detectionGrid.GetNumRows(); ++i) {
      ...
      for (int j = 0; j < _detectionGrid.GetNumCols(); ++j) {
        ...
        if (label >
            0)  // zero is background (not part of any connected component)
        {
          DEV_ASSERT(label < count,
                     "INeuralNetModel.LocalizedBinaryOutputHelper.BadLabel");
    ...
    // Create a SalientPoint to return for each connected component (skipping
    // background component 0)
    const float widthScale =
        1.f / static_cast<float>(_detectionGrid.GetNumCols());
    const float heightScale =
        1.f / static_cast<float>(_detectionGrid.GetNumRows());
    for (s32 iComp = 1; iComp < count; ++iComp) {
      ...
      // Centroid currently contains a sum. Divide by area to get actual
      // centroid.
      ...
      stat.centroid.y() =
          Util::Clamp(stat.centroid.y() * heightScale, 0.f, 1.f);

      // Use the single label (this is supposed to be a binary classifier after
      // all) and convert it to a SalientPointType
      DEV_ASSERT(_labels.size() == 1,
                 "INeuralNetModel.LocalizedBinaryOutputHelper.NotBinary");
      Vision::SalientPointType type = Vision::SalientPointType::Unknown;
      const bool success = SalientPointTypeFromString(_labels[0], type);
      DEV_ASSERT(success,
                 "INeuralNetModel.LocalizedBinaryOutputHelper."
                 "NoSalientPointTypeForLabel");

      // Use the bounding box as the shape (in normalized coordinates)
      // TODO: Create a more precise polygon that traces the shape of the
      // connected component (cv::findContours?)
      ...
      const Poly2f shape(
          Rectangle<float>(xmin, ymin, xmax - xmin, ymax - ymin));

      Vision::SalientPoint salientPoint(
          timestamp, stat.centroid.x(), stat.centroid.y(), avgScore,
          area * (widthScale * heightScale),  // convert to area fraction
          type, EnumToString(type), shape.ToCladPoint2dVector(), 0);

      if (_params.verbose) {
        LOG_INFO("INeuralNetModel.LocalizedBinaryOutputHelper.SalientPoint",
                 "%d: %s score:%.2f area:%.2f [%s %s %s %s]", iComp,
                 stat.centroid.ToString().c_str(), avgScore, area,
                 shape[0].ToString().c_str(), shape[1].ToString().c_str(),
                 shape[2].ToString().c_str(), shape[3].ToString().c_str());
      }
...
// Explicitly instantiate for float and uint8
template void INeuralNetModel::LocalizedBinaryOutputHelper(
    const float* outputData, TimeStamp_t timestamp, const float scale,
    const int zero_point, std::list<Vision::SalientPoint>& salientPoints);
template void INeuralNetModel::LocalizedBinaryOutputHelper(
    const uint8_t* outputData, TimeStamp_t timestamp, const float scale,
    const int zero_point, std::list<Vision::SalientPoint>& salientPoints);

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
template <typename T>
void INeuralNetModel::ResponseMapOutputHelper(
    const T* outputData, TimeStamp_t timestamp, const int numberOfChannels,
    std::list<Vision::SalientPoint>& salientPoints) {
  ...
  cv::Mat responseMap(_params.inputHeight, _params.inputWidth,
                      GetCVTypeHelper(outputData),
  ...
  Vision::SalientPoint salientPoint(
      timestamp, x, y, max, 1.f * (kWidthScale * kHeightScale), type,
      EnumToString(type), Poly2f{}.ToCladPoint2dVector(), 0);

  salientPoints.push_back(std::move(salientPoint));
}

// Explicitly instantiate for float and uint8
template void INeuralNetModel::ResponseMapOutputHelper(
    const float* outputData, TimeStamp_t timestamp, const int numberOfChannels,
    std::list<Vision::SalientPoint>& salientPoints);
template void INeuralNetModel::ResponseMapOutputHelper(
    const uint8_t* outputData, TimeStamp_t timestamp,
    const int numberOfChannels, std::list<Vision::SalientPoint>& salientPoints);

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void INeuralNetModel::SaveResponseMaps(const std::vector<cv::Mat>& channels,
                                       const int numberOfChannels,
                                       const TimeStamp_t timestamp) {
  for (int channel = 0; channel < numberOfChannels; ++channel) {
    double channelMin(0), channelMax(0);
    cv::Point2i channelMinLoc(0, 0), channelMaxLoc(0, 0);
    cv::minMaxLoc(channels[channel], &channelMin, &channelMax, &channelMinLoc,
                  &channelMaxLoc);

    const std::string saveFilename = Util::FileUtils::FullFilePath(
        {_params.visualizationDirectory,
         std::to_string(timestamp) + "_" + std::to_string(channel) + ".png"});

    cv::Mat imageToSave;
    channels[channel].copyTo(imageToSave);
    imageToSave = 255 * (imageToSave - channelMin) / (channelMax - channelMin);
    imageToSave.convertTo(imageToSave, CV_8UC1, 1.f / (channelMax - channelMin),
                          -channelMin / (channelMax - channelMin));
    ...
    salientPointFile << channelMaxLoc.x << " " << channelMaxLoc.y << " "
                     << channelMax << +" " << channelMinLoc.x << " "
                     << channelMinLoc.y << " " << channelMin;
