 ...
 * Date:   9/14/2018
 *
 * Description: Interface class to create a standalone process to run forward
 *inference using a neural network.
 *
 *              Currently uses the file system as a poor man's IPC to
 *communicate with the "messenger" NeuralNetRunner::Model implementation in
 *vic-engine's Vision System. See also VIC-2686.
 *
...
class INeuralNetMain {
 public:
  ...
  // If imageFileToProcess is empty, the cachePath is polled for
  // "neuralNetImage.png" to process. Otherwise the given image is processed on
  // Run(), and will complete immediately.
  Result Init(const std::string& configFilename, const std::string& modelPath,
              const std::string& cachePath,
              const std::string& imageFileToProcess = "");

  // Wait for and process images until ShouldShutdown() returns true (is
  // blocking) (Unless imageFileToProcess was provided in Init(), in which case
  // it is processed and Run completes immediately)
  Result Run();

  virtual ~INeuralNetMain();

  // Helpers for turning SalientPoints into Json and saving
  static void ConvertSalientPointsToJson(
      const std::list<Vision::SalientPoint>& salientPoints,
      const bool isVerbose, Json::Value& detectionResults);

  static bool WriteResults(const std::string& jsonFilename,
                           const Json::Value& detectionResults);
 ...
 protected:
  ...
  // Define what happens at the end of each loop of Run() (e.g. a wait, check
  // for shutdown, etc)
