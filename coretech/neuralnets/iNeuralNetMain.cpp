...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Must be implemented here (in .cpp)
// due to use of unique_ptr with a forward declaration
...
Result INeuralNetMain::Init(const std::string& configFilename,
  ...
  if (nullptr == Util::gLoggerProvider) {
    // Having no logger shouldn't kill us but probably isn't what we intended,
    // so issue a warning
  ...
  // NOTE: cachePath need not exist yet as it will be created by the
  // NeuralNetRunner
  if (!Util::FileUtils::FileExists(configFilename)) {
    LOG_ERROR("INeuralNetMain.Init.BadConfigFile", "ConfigFile:%s",
              configFilename.c_str());
    result = RESULT_FAIL;
  }
  if (!Util::FileUtils::DirectoryExists(modelPath)) {
    LOG_ERROR("INeuralNetMain.Init.BadModelPath", "ModelPath:%s",
              modelPath.c_str());
    result = RESULT_FAIL;
  }
  if (RESULT_OK != result) {
  ...
  LOG_INFO("INeuralNetMain.Init.Starting",
           "Config:%s ModelPath:%s CachePath:%s", configFilename.c_str(),
           modelPath.c_str(), cachePath.c_str());
  ...
  Json::Value config;
  {
    ...
    if (!config.isMember(JsonKeys::NeuralNets)) {
      LOG_ERROR("INeuralNetMain.Init.MissingConfigKey",
                "Config file missing '%s' field", JsonKeys::NeuralNets);
      CleanupAndExit(RESULT_FAIL);
    }
  ...
  if (!config.isMember(JsonKeys::Models)) {
    LOG_ERROR("INeuralNetMain.Init.MissingModelsField",
              "No 'models' specified in config file");
    CleanupAndExit(RESULT_FAIL);
  }
  ...
  for (auto& modelConfig : modelsConfig) {
    if (!modelConfig.isMember(JsonKeys::NetworkName)) {
      LOG_ERROR("INeuralNetMain.Init.MissingNameField",
                "No 'name' specified in model spec in config");
      CleanupAndExit(RESULT_FAIL);
    }
    ...
    if (!modelConfig.isMember(JsonKeys::ModelType)) {
      LOG_ERROR("INeuralNetModel.CreateFromTypeConfig.MissingConfig", "%s",
                JsonKeys::ModelType);
      CleanupAndExit(RESULT_FAIL);
    }

    const std::string& modelTypeString =
        modelConfig[JsonKeys::ModelType].asString();
    if (NeuralNets::JsonKeys::TFLiteModelType == modelTypeString) {
      ...
      LOG_ERROR("NeuralNetRunner.Init.UnknownModelType", "%s",
                modelTypeString.c_str());
      CleanupAndExit(RESULT_FAIL);
    }

    auto insertionResult = _neuralNets.emplace(name, std::move(model));
    if (!insertionResult.second) {
      LOG_ERROR("INeuralNetMain.Init.DuplicateModelName",
                "More than one model named '%s'", name.c_str());
      CleanupAndExit(RESULT_FAIL);
    }
    ...
    if (RESULT_OK != result) {
      LOG_ERROR("INeuralNetMain.Init.LoadModelFail",
                "Failed to load model '%s' from path: %s", name.c_str(),
                modelPath.c_str());
      CleanupAndExit(result);
    }
    ...
    const std::string outputDir =
        Util::FileUtils::FullFilePath({_cachePath, name});
    const bool success = Util::FileUtils::CreateDirectory(outputDir);
    if (!success) {
      LOG_ERROR("INeuralNetMain.Init.CreateOutputDirFailed", "Tried: %s",
                outputDir.c_str());
      CleanupAndExit(RESULT_FAIL);
    }
  ...
  LOG_INFO("INeuralNetMain.Init.DetectorInitialized",
           "Loaded %zu model(s). Polling for images every %dms",
  ...
  while (!anyFailures && !ShouldShutdown()) {
    for (auto& model : _neuralNets) {
      ...
      const std::string fullImagePath =
          (imageFileProvided ? _imageFilename
                             : Util::FileUtils::FullFilePath(
                                   {_cachePath, networkName,
                                    OffboardModel::Filenames::Image}));

      const bool isImageAvailable = Util::FileUtils::FileExists(fullImagePath);

      if (isImageAvailable) {
        if (neuralNet->IsVerbose()) {
          LOG_INFO("INeuralNetMain.Run.FoundImage", "%s",
                   fullImagePath.c_str());
        }
        ...
        Vision::ImageRGB img;
        {
          ...
          const std::string timestampFilename = Util::FileUtils::FullFilePath(
              {_cachePath, networkName, OffboardModel::Filenames::Timestamp});

          GetImage(fullImagePath, timestampFilename, img);

          if (img.IsEmpty()) {
            LOG_ERROR("INeuralNetMain.Run.ImageReadFailed",
                      "Error while loading image %s", fullImagePath.c_str());
            if (imageFileProvided) {
              // If we loaded in image file specified on the command line, we
              // are done
              anyFailures = true;
              break;
            } else {
              // Remove the corrupted image image to show we are ready for a new
              // one
              if (neuralNet->IsVerbose()) {
                LOG_INFO("INeuralNetMain.Run.DeletingCorruptedImageFile", "%s",
                         fullImagePath.c_str());
              }
              Util::FileUtils::DeleteFile(fullImagePath);
              continue;  // no need to stop the process, it was just a bad
                         // image, won't happen again
            }
          }
        }
        ...
        ConvertSalientPointsToJson(salientPoints, neuralNet->IsVerbose(),
                                   detectionResults);
        ...
        // Write out the Json
        {
          if (!imageFileProvided) {
            ...
            if (neuralNet->IsVerbose()) {
              LOG_INFO("INeuralNetMain.Run.DeletingImageFile", "%s",
                       fullImagePath.c_str());
            }
            Util::FileUtils::DeleteFile(fullImagePath);
          }
          ...
          const std::string tempFilename = Util::FileUtils::FullFilePath(
              {_cachePath, networkName, "tempResult.json"});
          if (neuralNet->IsVerbose()) {
            LOG_INFO("INeuralNetMain.Run.WritingTempResults", "%s",
                     tempFilename.c_str());
          }
          ...
          const std::string jsonFilename = Util::FileUtils::FullFilePath(
              {_cachePath, networkName, OffboardModel::Filenames::Result});
          if (neuralNet->IsVerbose()) {
            LOG_INFO("INeuralNetMain.Run.MovingToFinalResults", "%s",
                     jsonFilename.c_str());
          }
        ...
        LOG_ERROR("INeuralNetMain.Run.ImageFileDoesNotExist", "%s",
        ...
        if (count++ * _pollPeriod_ms >= kVerbosePrintFreq_ms) {
          LOG_INFO("INeuralNetMain.Run.WaitingForImage", "%s",
                   fullImagePath.c_str());
          count = 0;
        }
      }
    ...
    if (imageFileProvided) {
      // No polling needed when a specific image to process is provided, so
      // finish
      break;
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void INeuralNetMain::GetImage(const std::string& imageFilename,
  ...
  if (RESULT_OK != loadResult) {
    PRINT_NAMED_ERROR("INeuralNetMain.GetImage.EmptyImageRead", "%s",
                      imageFilename.c_str());
    return;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void INeuralNetMain::ConvertSalientPointsToJson(
    const std::list<Vision::SalientPoint>& salientPoints, const bool isVerbose,
    Json::Value& detectionResults) {
  ...
  for (auto const& salientPoint : salientPoints) {
    salientPointsStr += salientPoint.description + "[" +
                        std::to_string((int)round(100.f * salientPoint.score)) +
                        "] ";
  ...
  if (isVerbose && !salientPoints.empty()) {
    LOG_INFO("INeuralNetMain.ConvertSalientPointsToJson",
             "Detected %zu objects: %s", salientPoints.size(),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool INeuralNetMain::WriteResults(const std::string& jsonFilename,
                                  const Json::Value& detectionResults) {
  // Write to a temporary file, then move into place once the write is complete
  // (poor man's "lock")
  ...
  if (!fs.is_open()) {
    LOG_ERROR("INeuralNetMain.WriteResults.OutputFileOpenFailed", "%s",
              jsonFilename.c_str());
    return false;
  }
  ...
  if (!success) {
    LOG_ERROR("INeuralNetMain.WriteResults.RenameFail", "%s -> %s",
              tempFilename.c_str(), jsonFilename.c_str());
    return false;
  }
