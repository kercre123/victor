 ...
 * Date:   12/5/2017
 *
 * Description: Implementation of ObjectDetector Model class which wraps
 *TensorFlow Lite.
 *
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
TFLiteModel::~TFLiteModel() { LOG_DEBUG("TFLiteModel.Destructor", ""); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result TFLiteModel::LoadModelInternal(const std::string& modelPath,
                                      const Json::Value& config) {
  DEV_ASSERT(!modelPath.empty(),
             "TFLiteModel.LoadModelInternal.EmptyModelPath");
  ...
  if (!Util::FileUtils::FileExists(graphFileName)) {
    LOG_ERROR("TFLiteModel.LoadModelInternal.GraphFileDoesNotExist", "%s",
              graphFileName.c_str());
    return RESULT_FAIL;
  }
  ...
  if (!_model) {
    LOG_ERROR("TFLiteModel.LoadModelInternal.FailedToBuildFromFile", "%s",
              graphFileName.c_str());
    return RESULT_FAIL;
  }
...
void TFLiteModel::ScaleImage(Vision::ImageRGB& img) {
  ...
  if (_params.useFloatInput) {
    ...
    DEV_ASSERT(scaledInputData != nullptr,
               "TFLiteModel.ScaleImage.NullInputData");

    // Resize uint8 image data, and *then* convert smaller image to float below
    // TODO: Resize and convert directly into the scaledInputData (ideally using
    // NEON?)
    if (img.GetNumRows() != _params.inputHeight ||
      ...
      LOG_INFO("TFLiteModel.ScaleImage.SkipResize",
    ...
    DEV_ASSERT(img.GetNumChannels() == 3,
               "TFLiteModel.ScaleImage.BadNumChannels");
    ...
    // Resize uint8 input image directly into the uint8 tensor data by wrapping
    // image "header" around the tensor's data buffer
    uint8_t* scaledInputData = _interpreter->typed_tensor<uint8_t>(inputIndex);
    Vision::ImageRGB tensorImg(_params.inputHeight, _params.inputWidth,
                               scaledInputData);
...
Result TFLiteModel::Detect(Vision::ImageRGB& img,
  ...
  // Scale image, subtract mean, divide by standard deviation and store in the
  ...
  if (_params.benchmarkRuns == 0) {
    ...
    // TODO: Upgrade to TF r1.10 in order to build profile_summarizer.cc for
    // detailed timing
    ...
    for (auto const& e : profile_events) {
      ...
      LOG_INFO("TFLiteModel.Detect.Profiling",
               "Num Runs: %d, Avg: %f ms, Node: %u, OpCode: %i, %s \n",
               _params.benchmarkRuns,
               (e->end_timestamp_us - e->begin_timestamp_us) /
                   (1000.0 * _params.benchmarkRuns),
               op_index, registration.builtin_code,
               EnumNameBuiltinOperator(static_cast<tflite::BuiltinOperator>(
  ...
  switch (_params.outputType) {
    case NeuralNetParams::OutputType::Classification: {
      if (_params.useFloatInput) {
        ...
        ClassificationOutputHelper(outputData, img.GetTimestamp(),
                                   salientPoints);
      } else {
        const uint8_t* outputData =
            _interpreter->typed_output_tensor<uint8_t>(0);
        ClassificationOutputHelper(outputData, img.GetTimestamp(),
                                   salientPoints);
      }
    ...
    case NeuralNetParams::OutputType::BinaryLocalization: {
      if (_params.useFloatInput) {
        ...
        LocalizedBinaryOutputHelper(outputData, img.GetTimestamp(), scale,
                                    zero_point, salientPoints);
      }
