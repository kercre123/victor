...
// NOTE: this wrapper completely compiles out if we're using a different model
// (e.g. TFLite)
...
NeuralNetModel::~NeuralNetModel() {
  ...
  if (_session) {
    ...
    if (!sessionCloseStatus.ok()) {
      PRINT_NAMED_WARNING("NeuralNetModel.Destructor.CloseSessionFailed",
                          "Status: %s", sessionCloseStatus.ToString().c_str());
    }
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result NeuralNetModel::LoadModel(const std::string& modelPath,
  ...
  const std::string graphFileName =
      Util::FileUtils::FullFilePath({modelPath, _params.graphFile});

  if (!Util::FileUtils::FileExists(graphFileName))

  {
    PRINT_NAMED_ERROR("NeuralNetModel.Model.LoadGraph.GraphFileDoesNotExist",
                      "%s", graphFileName.c_str());
    return RESULT_FAIL;
  }

  if (_params.verbose) {
    LOG_INFO("NeuralNetModel.LoadModel.FoundGraphFile", "%s",
             graphFileName.c_str());
  }
  ...
  if (_params.memoryMapGraph) {
    // Using memory-mapped graphs needs more testing/work, but this is a start.
    // (VIC-3141)

    // See also: https://www.tensorflow.org/mobile/optimizing

    // Note that this is a class member because it needs to persist as long as
    // we use the graph referring to it
    _memmappedEnv.reset(
        new tensorflow::MemmappedEnv(tensorflow::Env::Default()));

    tensorflow::Status mmapStatus =
        _memmappedEnv->InitializeFromFile(graphFileName);

    tensorflow::Status loadGraphStatus = ReadBinaryProto(
        _memmappedEnv.get(),
    ...
    if (!loadGraphStatus.ok()) {
      PRINT_NAMED_ERROR(
          "NeuralNetModel.Model.LoadGraph.MemoryMapBinaryProtoFailed",
          "Status: %s", loadGraphStatus.ToString().c_str());
      return RESULT_FAIL;
    }

    LOG_INFO("NeuralNetModel.LoadModel.MemMappedModelLoadSuccess", "%s",
             graphFileName.c_str());
    ...
    tensorflow::Status session_status =
        tensorflow::NewSession(options, &sessionPtr);
    ...
    tensorflow::Status loadGraphStatus = tensorflow::ReadBinaryProto(
        tensorflow::Env::Default(), graphFileName, &graphDef);
    if (!loadGraphStatus.ok()) {
    ...
    LOG_INFO("NeuralNetModel.LoadModel.ReadBinaryProtoSuccess", "%s",
             graphFileName.c_str());
  ...
  if (_session) {
    tensorflow::Status sessionCloseStatus = _session->Close();
    if (!sessionCloseStatus.ok()) {
      PRINT_NAMED_WARNING("NeuralNetModel.LoadModel.CloseSessionFailed",
                          "Status: %s", sessionCloseStatus.ToString().c_str());
    }
  }
  _session.reset(sessionPtr);
  ...
  if (_params.verbose) {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      LOG_INFO("NeuralNetModel.LoadModel.Summary",
               "Layer %d - Name: %s, Op: %s", i, n.name().c_str(),
               n.op().c_str());
      if (n.op() == "Const") {
        tensorflow::Tensor t;
        if (!t.FromProto(n.attr().at("value").tensor())) {
          LOG_INFO("NeuralNetModel.LoadModel.SummaryFail",
                   "Failed to create Tensor from proto");
          continue;
        }

        LOG_INFO("NeuralNetModel.LoadModel.Summary", "%s",
                 t.DebugString().c_str());
      } else if (n.op() == "Conv2D") {
        const auto& filterNodeName = n.input(1);
        LOG_INFO("NeuralNetModel.LoadModel.Summary",
                 "Filter input from Conv2D node: %s", filterNodeName.c_str());
      }
    }
  }

  const std::string labelsFileName =
      Util::FileUtils::FullFilePath({modelPath, _params.labelsFile});
  Result readLabelsResult = ReadLabelsFile(labelsFileName, _labels);
  if (RESULT_OK == readLabelsResult) {
    LOG_INFO("NeuralNetModel.LoadModel.ReadLabelFileSuccess", "%s",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void NeuralNetModel::GetDetectedObjects(
    const std::vector<tensorflow::Tensor>& outputRensors, TimeStamp_t timestamp,
    std::list<Vision::SalientPoint>& salientPoints) {
  DEV_ASSERT(outputRensors.size() == 4,
             "NeuralNetModel.GetDetectedObjects.WrongNumOutputs");

  const int numDetections = (int)outputRensors[3].tensor<float, 1>().data()[0];

  if (_params.verbose) {
    LOG_INFO("NeuralNetModel.GetDetectedObjects.NumDetections",
             "%d raw detections", numDetections);
  }
  ...
  if (numDetections > 0) {
    ...
    for (int i = 0; i < numDetections; ++i) {
      if (scores[i] >= _params.minScore) {
        ...
        Vision::SalientPoint salientPoint(
            timestamp, (float)(xmin + xmax) * 0.5f, (float)(ymin + ymax) * 0.5f,
            scores[i], bbox.Area(), Vision::SalientPointType::Object,
    ...
    if (_params.verbose) {
      ...
      LOG_INFO("NeuralNetModel.GetDetectedObjects.ReturningObjects",
               ...
               (int)salientPoints.size(), _params.minScore,
               salientPointsStr.c_str());
    }
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result NeuralNetModel::Detect(Vision::ImageRGB& img,
                              std::list<Vision::SalientPoint>& salientPoints) {
  tensorflow::Tensor imageTensor;

  if (_params.useGrayscale) {
    LOG_ERROR("NeuralNetModel.Detect.UseGrayscaleDeprecated",
              "Now assuming RGB data");
    return RESULT_FAIL;
  }
  ...
  if (_params.verbose) {
    LOG_INFO("NeuralNetModel.Detect.Resizing",
             "From [%dx%dx%d] image to [%dx%dx%d] %s tensor", img.GetNumCols(),
             img.GetNumRows(), img.GetNumChannels(), _params.inputWidth,
             _params.inputHeight, (_params.useGrayscale ? 1 : 3), typeStr);
  }
  ...
  if (_params.useFloatInput) {
    ...
    Vision::ImageRGB imgTensor(_params.inputHeight, _params.inputWidth,
                               imageTensor.tensor<uint8_t, 4>().data());
    img.Resize(imgTensor, kResizeMethod);
  }

  if (_params.verbose) {
    LOG_INFO("NeuralNetModel.Detect.RunningSession",
             "Input=[%dx%dx%d], %s, %d output(s)", img.GetNumCols(),
             img.GetNumRows(), img.GetNumChannels(), typeStr,
             (int)_params.outputLayerNames.size());
  }
  ...
  switch (_params.outputType) {
    case NeuralNetParams::OutputType::Classification: {
      DEV_ASSERT(outputTensors.size() == 1,
                 "NeuralNetModel.Detect.Classification.WrongNumOutputTensors");
    ...
    case NeuralNetParams::OutputType::BinaryLocalization: {
      DEV_ASSERT(
          ...
          "NeuralNetModel.Detect.BinaryLocalization.WrongNumOutputTensors");
      auto const& outputTensor = outputTensors[0];

      // This raw (Eigen) tensor data appears to be _column_ major (i.e. "not
      // row major"). Ensure that remains true.
      ...
      LocalizedBinaryOutputHelper(outputData, img.GetTimeStamp(), 1.f, 0,
                                  salientPoints);
      break;
    }
    ...
    case NeuralNetParams::OutputType::Segmentation: {
      ...
      DEV_ASSERT(outputTensors.size() == 1,
                 "NeuralNetModel.Detect.Segmentation.WrongNumOutputTensors");
      const int numberOfChannels = 2;
      tensorflow::Tensor squeezedTensor(
          tensorflow::DT_FLOAT,
          tensorflow::TensorShape(
              {_params.inputWidth, _params.inputHeight, numberOfChannels}));

      // Reshape tensor from [1, inputWidth, inputHeight, 2] to [inputWidth,
      // inputHeight, 2]
      if (!squeezedTensor.CopyFrom(
              outputTensors[0],
              tensorflow::TensorShape({_params.inputWidth, _params.inputHeight,
                                       numberOfChannels}))) {
        PRINT_NAMED_ERROR(
            "NeuralNetModel.GetSalientPointsFromResponseMap.CopyFromFailed",
            "");
        return RESULT_FAIL;
      }
      const float* outputData = squeezedTensor.tensor<float, 3>().data();
      ResponseMapOutputHelper(outputData, img.GetTimeStamp(), numberOfChannels,
                              salientPoints);
      break;
    }
    default:
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result NeuralNetModel::Run(tensorflow::Tensor imageTensor,
                           std::vector<tensorflow::Tensor>& outputTensors) {
  ...
  if (0 == _params.benchmarkRuns) {
    ...
    for (uint32_t i = 0; i < _params.benchmarkRuns; ++i) {
      runStatus = _session->Run(
          runOptions, {{_params.inputLayerName, imageTensor}},
          _params.outputLayerNames, {}, &outputTensors, &runMetadata);
      if (!runStatus.ok()) {
        break;
      }
      if (nullptr != stats) {
        DEV_ASSERT(runMetadata.has_step_stats(),
                   "ObjectDetector.Detect.Run.NullBenchmarkStats");
  ...
  if (!runStatus.ok()) {
    PRINT_NAMED_ERROR("ObjectDetector.Detect.Run.DetectionSessionRunFail", "%s",
                      runStatus.ToString().c_str());
    return RESULT_FAIL;
  } else {
    return RESULT_OK;
  }
