...
// This test is meant to avoid checking in a vision_config.json or code changes
// that break the ability to load the neural net model(s) successfully.
GTEST_TEST(NeuralNets, InitFromConfigAndLoadModel) {
  using namespace Anki;

  const std::string configFilename =
      Util::FileUtils::FullFilePath({TestPaths::TestRoot, "test_config.json"});
  ...
  for (const auto& modelConfig : modelsConfig) {
    const Result loadResult =
        neuralNet.LoadModel(TestPaths::ModelPath, modelConfig);
...
GTEST_TEST(NeuralNets, MobileNet) {
  ...
  const std::vector<Test> tests{
      Test{.imageFile = "grace_hopper.jpg",
  ...
  for (auto const& test : tests) {
    const std::string testImageFile =
        Util::FileUtils::FullFilePath({TestPaths::ImagePath, test.imageFile});
...
GTEST_TEST(NeuralNets, PersonDetection) {
  ...
  // The alternative is to duplicate the config and model within
  // coretech/neuralnets.
  const std::string vectorResourcePath =
      Util::FileUtils::FullFilePath({TestPaths::ModelPath, "..", "..", "..",
                                     "..", "resources", "config", "engine"});
  ASSERT_TRUE(Util::FileUtils::DirectoryExists(vectorResourcePath));

  const std::string configFilename =
      Util::FileUtils::FullFilePath({vectorResourcePath, "vision_config.json"});
  ASSERT_TRUE(Util::FileUtils::FileExists(configFilename));

  const std::string vectorModelPath = Util::FileUtils::FullFilePath(
      {vectorResourcePath, "vision", "dnn_models"});
  ...
  for (const auto& modelConfig : modelsConfig) {
    ...
    if (JsonTools::GetValueOptional(
            modelConfig, NeuralNets::JsonKeys::NetworkName, networkName)) {
  ...
  const std::string peopleFilePath =
      Util::FileUtils::FullFilePath({TestPaths::ImagePath, "people"});
  const auto files =
      Util::FileUtils::FilesInDirectory(peopleFilePath, true, {".jpg", ".png"});
  ...
  for (auto const& file : files) {
    ...
    for (auto const& salientPoint : salientPoints) {
      ...
      // TODO: Compare actual centroid/shape to ground truth (may be annoying to
...
GTEST_TEST(NeuralNets, MultipleModels) {
  ...
  class TestMain : public NeuralNets::INeuralNetMain {
   public:
    ...
    Result ReadResult(const std::string& resultFilename,
      ...
      file.close();
      if (!success) {
        PRINT_NAMED_ERROR("NeuralNetRunner.Model.FailedToReadJSON", "%s",
   ...
   protected:
    virtual bool ShouldShutdown() override {
      // NOTE: ShouldShutdown is superfluous because this test provides an input
      // file, so
      //       Run() will always stop immediately after processing that one
      //       file.
      return false;
    }
    ...
    virtual int GetPollPeriod_ms(const Json::Value& config) const override {
      assert(
          config.isMember(NeuralNets::OffboardModel::JsonKeys::PollingPeriod));
  ...
  const std::string testImageFile =
      Util::FileUtils::FullFilePath({TestPaths::ImagePath, "cat.jpg"});
  const Result initResult = testMain.Init(configFilename, TestPaths::ModelPath,
                                          TestPaths::CachePath, testImageFile);
  ...
  const std::vector<std::string> resultFilenames = {
      Util::FileUtils::FullFilePath(
          {TestPaths::CachePath, "quantized",
           NeuralNets::OffboardModel::Filenames::Result}),
      Util::FileUtils::FullFilePath(
          {TestPaths::CachePath, "non-quantized",
           NeuralNets::OffboardModel::Filenames::Result}),
  };
  ...
  for (const auto& resultFilename : resultFilenames) {
    ...
    const Result readResult =
        testMain.ReadResult(resultFilename, salientPoints);
...
GTEST_TEST(NeuralNets, ClassificationConsensus) {
  ...
  const std::vector<Test> tests{
      Test{
          ...
          .expectedDetections = {false, false, false, false, true, false,
                                 false},
      },
      Test{
          .imageFile = "cat.jpg",
      ...
      Test{
          ...
          .expectedDetections = {false, false, true, true, true, true, true,
                                 false},
      },
  };
