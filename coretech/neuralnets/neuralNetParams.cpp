...
static inline void SetFromConfigHelper(const Json::Value& json,
                                       int32_t& value) {
  value = json.asInt();
}
...
// static inline void SetFromConfigHelper(const Json::Value& json, uint8_t&
// value) {
//   value = json.asUInt();
// }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void NeuralNetParams::Clear() { *this = NeuralNetParams(); }
...
#define GetFromConfig(keyName)                                       \
  if (!config.isMember(QUOTE(keyName))) {                            \
    PRINT_NAMED_ERROR("NeuralNetParams.SetFromConfig.MissingConfig", \
                      QUOTE(keyName));                               \
    return RESULT_FAIL;                                              \
  } else {                                                           \
  ...
  if ("ssd_mobilenet" == architecture) {
    ...
    outputLayerNames = {"detection_scores", "detection_classes",
    ...
    if (config.isMember("outputType")) {
      PRINT_NAMED_WARNING("NeuralNetParams.SetFromConfig.IgnoringOutputType",
                          "Ignoring outputType and using 'AnchorBoxes' because "
                          "architecture='ssd_mobilenet' was specified");
    }
    ...
    if (config.isMember("outputType")) {
      PRINT_NAMED_WARNING("NeuralNetParams.SetFromConfig.IgnoringOutputType",
                          "Ignoring outputType and using 'Classification' "
                          "because architecture='mobilenet' was specified");
    }
    ...
    PRINT_NAMED_ERROR("NeuralNetParams.SetFromConfig.UnrecognizedArchitecture",
                      "%s", architecture.c_str());
    return RESULT_FAIL;
  }
  ...
  if (verbose) {
    ...
    LOG_INFO("NeuralNetParams.SetFromConfig.Summary",
             "Arch: %s, %s Input: %s, Outputs: %s", architecture.c_str(),
             (useGrayscale ? "Grayscale" : "Color"), inputLayerName.c_str(),
             outputNames.c_str());
  }
...
Result NeuralNetParams::SetOutputTypeFromConfig(const Json::Value& config) {
  ...
  if (!config.isMember("outputType")) {
    PRINT_NAMED_ERROR(
        "NeuralNetParams.SetOutputTypeFromConfig.MissingOutputType",
  ...
  if (iter == kOutputTypeMap.end()) {
    ...
    PRINT_NAMED_ERROR("NeuralNetParams.SetOutputTypeFromConfig.BadOutputType",
                      "Valid types: %s", validKeys.c_str());
    return RESULT_FAIL;
  } else {
    if (outputLayerNames.size() != iter->second.numOutputs) {
      PRINT_NAMED_ERROR(
          "NeuralNetParams.SetOutputTypeFromConfig.WrongNumberOfOutputs",
          "OutputType %s requires %d outputs (%d provided)",
          iter->first.c_str(), iter->second.numOutputs,
          (int)outputLayerNames.size());
      return RESULT_FAIL;
    }
