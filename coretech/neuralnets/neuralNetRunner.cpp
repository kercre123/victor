...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result NeuralNetRunner::InitInternal(const std::string& cachePath,
                                     const Json::Value& config) {
  std::string modelTypeString;
  if (JsonTools::GetValueOptional(config, NeuralNets::JsonKeys::ModelType,
                                  modelTypeString)) {
    if (NeuralNets::JsonKeys::OffboardModelType == modelTypeString) {
      ...
      LOG_ERROR("NeuralNetRunner.InitInternal.UnknownModelType", "%s",
                modelTypeString.c_str());
      return RESULT_FAIL;
    }
  } else {
    LOG_ERROR("NeuralNetRunner.InitInternal.MissingConfig", "%s",
              NeuralNets::JsonKeys::ModelType);
    return RESULT_FAIL;
  }
  ...
  if (JsonTools::GetValueOptional(config,
                                  NeuralNets::JsonKeys::VisualizationDir,
                                  _visualizationDirectory)) {
    Util::FileUtils::CreateDirectory(
        Util::FileUtils::FullFilePath({cachePath, _visualizationDirectory}));
  }
  ...
  PRINT_NAMED_INFO("NeuralNetRunner.InitInternal.LoadModelTime",
                   "Loading model from '%s' took %.1fsec", _modelPath.c_str(),
                   Util::MilliSecToSec(GetProfiler().AverageToc("LoadModel")));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool NeuralNetRunner::IsVerbose() const { return _model->IsVerbose(); }
