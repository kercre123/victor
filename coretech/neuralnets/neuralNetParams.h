...
struct NeuralNetParams {
  // Specified in config, used to determine how to interpret the output of the
  // network, and thus which helper above to call (string to use in JSON config
  // shown for each)
  ...
  // When input to graph is float, data is first divided by scale and then
  // shifted I.e.:  float_input = data / scale + shift
  float inputShift = 0.f;
  float inputScale = 255.f;

  float minScore = 0.5f;  // in [0,1]

  // When outputType = Classification, a class must appear in majority of
  // numFrames to be reported. Note that this adds lag.
  int32_t numFrames = 1;
  int32_t majority = 1;
