 ...
 * Date:   3/8/2018
 *
 * Description: Implementation of INeuralNetModel interface class does not
 *actually run forward inference through a neural network model but instead
 *communicates with an "offboard" process using a Vision::OffboardProcessor.
 *(Really just a thin wrapper around OffboardProcessor to match the
 *INeuralNetModel interface, so it can be used by a NeuralNetRunner.)
 *
