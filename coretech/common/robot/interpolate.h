...
Affine<Type>::Affine(const Array<Type> &homography)
    : h00(homography[0][0]),
      h01(homography[0][1]),
      h02(homography[0][2]),
      h10(homography[1][0]),
...
template <typename Type>
inline void Affine<Type>::Apply(const Type inX, const Type inY, Type &outX,
                                Type &outY) {
...
template <typename Type>
inline Type InterpolateBilinear2d(const Type pixelTL, const Type pixelTR,
                                  const Type pixelBL, const Type pixelBR,
                                  const Type alphaY, const Type alphaYinverse,
                                  const Type alphaX, const Type alphaXinverse) {
  ...
  const Type interpolatedPixel =
      alphaYinverse * interpolatedTop + alphaY * interpolatedBottom;

  return interpolatedPixel;
}

template <typename InType, typename OutType>
Result Interp2(const Array<InType> &reference, const Array<f32> &xCoordinates,
               const Array<f32> &yCoordinates, Array<OutType> &out,
               const InterpolationType interpolationType,
               const OutType invalidValue) {
  AnkiConditionalErrorAndReturnValue(interpolationType == INTERPOLATE_LINEAR,
                                     RESULT_FAIL_INVALID_PARAMETER, "Interp2",
                                     "Only INTERPOLATE_LINEAR is supported");

  AnkiConditionalErrorAndReturnValue(
      AreValid(reference, xCoordinates, yCoordinates, out),
  ...
  AnkiConditionalErrorAndReturnValue(
      AreEqualSize(out, xCoordinates, yCoordinates), RESULT_FAIL_INVALID_SIZE,
      "Interp2",
      "xCoordinates, yCoordinates, and out must all be the same sizes");

  if (isOutputOneDimensional) {
    AnkiConditionalErrorAndReturnValue(
        AreEqualSize(1, numOutputElements, out), RESULT_FAIL_INVALID_SIZE,
        "Interp2",
        "If out is a row vector, then out, xCoordinates, and yCoordinates must "
        "all be 1xN");
  }

  AnkiConditionalErrorAndReturnValue(
      NotAliased(out, xCoordinates, yCoordinates, reference),
      RESULT_FAIL_ALIASED_MEMORY, "Interp2",
      "xCoordinates, yCoordinates, and reference cannot be the same as out");
  ...
  const s32 xIterationMax =
      isOutputOneDimensional ? (outHeight * outWidth) : outWidth;
  ...
  for (s32 y = 0; y < yIterationMax; y++) {
    ...
    for (s32 x = 0; x < xIterationMax; x++) {
      ...
      const OutType interpolatedPixel =
          RoundIfInteger<OutType>(interpolatedPixelF32);
...
template <typename InType, typename OutType>
Result Interp2_Affine(const Array<InType> &reference,
                      const Meshgrid<f32> &originalCoordinates,
                      const Array<f32> &homography,
                      const Point<f32> &centerOffset, Array<OutType> &out,
                      const InterpolationType interpolationType,
                      const OutType invalidValue) {
  AnkiConditionalErrorAndReturnValue(
      interpolationType == INTERPOLATE_LINEAR, RESULT_FAIL_INVALID_PARAMETER,
      "Interp2_Affine", "Only INTERPOLATE_LINEAR is supported");

  AnkiConditionalErrorAndReturnValue(AreValid(reference, out),
                                     RESULT_FAIL_INVALID_OBJECT,
                                     "Interp2_Affine", "Invalid objects");

  AnkiConditionalErrorAndReturnValue(
      NotAliased(reference, out), RESULT_FAIL_ALIASED_MEMORY, "Interp2_Affine",
      "reference cannot be the same as out");
  ...
  if (isOutputOneDimensional) {
    const s32 numOutputElements = outHeight * outWidth;
    const s32 numOriginalCoordinates =
        xGridVector.get_size() * yGridVector.get_size();

    AnkiConditionalErrorAndReturnValue(
        outWidth == numOutputElements &&
            numOriginalCoordinates == numOutputElements,
        RESULT_FAIL_INVALID_SIZE, "Interp2_Affine",
        "originalCoordinates is the wrong size");
  } else {
    AnkiConditionalErrorAndReturnValue(yGridVector.get_size() == outHeight &&
  ...
  for (s32 y = 0; y < yIterationMax; y++) {
    if (isOutputOneDimensional) {
      // If the output is one dimensional, then we will do the next set of x
      // iterations later on the same output row
      pOut += xIterationMax;
    } else {
    ...
    for (s32 x = 0; x < xIterationMax; x++) {
      ...
      // strength reduction for the affine transformation along this horizontal
      // line
      xTransformed += xTransformedDelta;
      yTransformed += yTransformedDelta;
    }  // for(s32 x=0; x<xIterationMax; x++)
...
template <typename InType, typename OutType>
Result Interp2_Projective(const Array<InType> &reference,
                          const Meshgrid<f32> &originalCoordinates,
                          const Array<f32> &homography,
                          const Point<f32> &centerOffset, Array<OutType> &out,
                          const InterpolationType interpolationType,
  ...
  AnkiConditionalErrorAndReturnValue(
      interpolationType == INTERPOLATE_LINEAR, RESULT_FAIL_INVALID_PARAMETER,
      "Interp2_Projective", "Only INTERPOLATE_LINEAR is supported");

  AnkiConditionalErrorAndReturnValue(AreValid(reference, out),
                                     RESULT_FAIL_INVALID_OBJECT,
                                     "Interp2_Projective", "Invalid objects");

  AnkiConditionalErrorAndReturnValue(
      NotAliased(reference, out), RESULT_FAIL_ALIASED_MEMORY,
      "Interp2_Projective", "reference cannot be the same as out");

  // AnkiConditionalErrorAndReturnValue(FLT_NEAR(homography[2][2], 1.0f),
  //  RESULT_FAIL_INVALID_PARAMETER, "Interp2_Projective", "homography[2][2]
  //  should be 1.0");
  ...
  if (isOutputOneDimensional) {
    ...
    AnkiConditionalErrorAndReturnValue(
        outWidth == numOutputElements &&
            numOriginalCoordinates == numOutputElements,
        RESULT_FAIL_INVALID_SIZE, "Interp2_Projective",
        "originalCoordinates is the wrong size");
  } else {
    AnkiConditionalErrorAndReturnValue(yGridVector.get_size() == outHeight &&
                                           xGridVector.get_size() == outWidth,
  ...
  for (s32 y = 0; y < yIterationMax; y++) {
    ...
    for (s32 x = 0; x < xIterationMax; x++) {
      ...
      const f32 xTransformed =
          (xTransformedRaw / normalization) + centerOffset.x;
      const f32 yTransformed =
          (yTransformedRaw / normalization) + centerOffset.y;
