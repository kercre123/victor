...
Result DrawRectangle(Array<Type> &image, Point<s16> point1, Point<s16> point2,
  ...
  AnkiConditionalErrorAndReturnValue(image.IsValid(),
                                     ...
                                     "DrawRectangle", "image is not valid");
  ...
  return RESULT_OK;
}  // template<typename Type> Result DrawRectangle(Array<Type> &image, const
   // Point<s16> point1, const Point<s16> point2, const s32 lineWidth, const
   // Type lineColor, const Type backgroundColor)
...
Result DrawFilledConvexQuadrilateral(Array<Type> &image,
  ...
  AnkiConditionalErrorAndReturnValue(
      image.IsValid(), RESULT_FAIL_INVALID_OBJECT,
      "DrawFilledConvexQuadrilateral", "image is not valid");
  ...
  for (s32 iy = 0; iy < numYs; iy++) {
    ...
    for (s32 iCorner = 0; iCorner < 4; iCorner++) {
      if ((corners[iCorner].y < y && corners[iCorner + 1].y >= y) ||
          (corners[iCorner + 1].y < y && corners[iCorner].y >= y)) {
  ...
  return RESULT_OK;
}  // template<typename Type> Result DrawFilledConvexQuadrilateral(Array<Type>
   // &image, Quadrilateral<f32> quad, const Type color)
...
Result DrawExampleSquaresImage(Array<Type> &image) {
  ...
  upperLeft[1] = 19;
  for (s32 i = 1; i <= 5; i++) {
    const Point<s16> point1 =
        Point<s16>(static_cast<s16>(upperLeft[1] + boxWidth * 2 * (i - 1)),
                   static_cast<s16>(upperLeft[0]));
    const Point<s16> point2 = Point<s16>(
        static_cast<s16>(upperLeft[1] + boxWidth + boxWidth * 2 * (i - 1)),
        static_cast<s16>(upperLeft[0] + boxWidth));
    //    image = drawBox(image, upperLeft + [0, boxWidth*2*(i-1)], upperLeft +
    //    boxWidth + [0, boxWidth*2*(i-1)], i, lineColdor, backgroundColor);
  ...
  for (s32 i = 1; i <= 5; i++) {
    ...
    const Point<s16> point2 = Point<s16>(
        ...
        static_cast<s16>(upperLeft[0] + boxWidth));
    //    image = drawBox(image, upperLeft + [0, boxWidth*2*(i-1)], upperLeft +
    //    boxWidth + [0, boxWidth*2*(i-1)], i+5, lineColor, backgroundColor);
    DrawRectangle<Type>(image, point1, point2, i + 5, lineColor,
                        backgroundColor);
  }
  ...
  for (s32 i = 1; i <= 5; i++) {
    ...
    const Point<s16> point2 = Point<s16>(
        ...
        static_cast<s16>(upperLeft[0] + boxWidth));
    //    image = drawBox(image, upperLeft + [0, boxWidth*2*(i-1)], upperLeft +
    //    boxWidth + [0, boxWidth*2*(i-1)], i+10, lineColor, backgroundColor);
    DrawRectangle<Type>(image, point1, point2, i + 10, lineColor,
                        backgroundColor);
  }
  ...
  for (s32 i = 1; i <= maxSpeckWidth; i++) {
    //    image = drawBox(image, upperLeft + [0, 2*sum(speckWidths(1:i))],
    //    upperLeft + [0, i + 2*sum(speckWidths(1:i))], -1, lineColor,
    //    backgroundColor);
    ...
    const Point<s16> point1 =
        Point<s16>(static_cast<s16>(upperLeft[1] + 2 * sumVal),
                   static_cast<s16>(upperLeft[0]));
    const Point<s16> point2 =
        Point<s16>(static_cast<s16>(upperLeft[1] + i + 2 * sumVal),
                   static_cast<s16>(upperLeft[0]));
  ...
  for (s32 i = 1; i <= maxSpeckWidth; i++) {
    //    image = drawBox(image, upperLeft + [0, 2*sum(speckWidths(1:i))],
    //    upperLeft + [1, i + 2*sum(speckWidths(1:i))], -1, lineColor,
    //    backgroundColor);
  ...
  for (s32 i = 1; i <= maxSpeckWidth; i++) {
    //    image = drawBox(image, upperLeft + [0, 2*sum(speckWidths(1:i))],
    //    upperLeft + [2, i + 2*sum(speckWidths(1:i))], -1, lineColor,
    //    backgroundColor);
    s32 sumVal = 0;
    for (s32 iSum = 1; iSum <= i; iSum++) {
      sumVal += iSum;
  ...
  for (s32 i = 1; i <= maxSpeckWidth; i++) {
    //    image = drawBox(image, upperLeft + [0, 2*sum(speckWidths(1:i))],
    //    upperLeft + [4, i + 2*sum(speckWidths(1:i))], -1, lineColor,
    //    backgroundColor);
    s32 sumVal = 0;
    for (s32 iSum = 1; iSum <= i; iSum++) {
      sumVal += iSum;
  ...
  for (s32 i = 1; i <= maxSpeckWidth; i++) {
    //    image = drawBox(image, upperLeft + [0, 2*sum(speckWidths(1:i))],
    //    upperLeft + [8, i + 2*sum(speckWidths(1:i))], -1, lineColor,
    //    backgroundColor);
    s32 sumVal = 0;
    for (s32 iSum = 1; iSum <= i; iSum++) {
      sumVal += iSum;
...
Result DrawPoints(const FixedLengthList<Point<Type> > &points,
  ...
  AnkiConditionalErrorAndReturnValue(AreValid(image, points),
                                     RESULT_FAIL_INVALID_OBJECT, "DrawPoints",
                                     "Invalid objects");
