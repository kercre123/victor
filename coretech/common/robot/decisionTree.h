...
The main prototype for an embedded decision tree. It's essentially an interface,
but without the expicit overhead.
...
class DecisionTree {
 public:
  ...
  // available
  DecisionTree(const void* restrict treeData, const s32 treeDataLength,
               const s32 treeDataNumFractionalBits, const s32 treeMaxDepth,
               const u16* restrict leafLabels, const s32 numLeafLabels);

  // TODO: These could be virtual methods, but I'm avoiding such things on the
  // embedded side
