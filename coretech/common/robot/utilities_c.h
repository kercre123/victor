...
Various simple macros and utilities. Everything in this file should be usable
with C99, and not require any C++.
...
// ct_assert is a compile time assertion, useful for checking sizeof() and other
...
#define SwapEndianU32(value)                                                  \
  ((((u32)((value)&0x000000FF)) << 24) | (((u32)((value)&0x0000FF00)) << 8) | \
   (((u32)((value)&0x00FF0000)) >> 8) | (((u32)((value)&0xFF000000)) >> 24))
...
void explicitPrintf(int (*writeChar)(int), int reverseEachFourCharacters,
                    const char *format, ...);
...
void PrintU32Hex(
    int (*writeChar)(int),
    u32 value);  // Print a single unsigned int in hex format 0xabcd0123
void PrintF64(int (*writeChar)(int), f64 value);  // Print a single float
void PrintF64WithExponent(
    int (*writeChar)(int),
    f64 value);  // Print a single float in format "significand * 2^exponent"
