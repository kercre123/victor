...
void CvPutTextFixedWidth(cv::Mat &img, const char *text, cv::Point org,
  ...
  if (last_fontFace != fontFace || last_fontScale != fontScale ||
      last_thickness != thickness || last_lineType != lineType) {
    ...
    const char characters[numCharacters] = "abcdefghijklmnopqrstuvwxyz
    ...
    for (s32 i = 0; i < (numCharacters - 1); i++) {
      ...
      cv::Size textSize = cv::getTextSize(std::string(tmpBuffer), fontFace,
                                          fontScale, thickness, NULL);
      // CoreTechPrint("%c = %dx%d\n", characters[i], textSize.width,
      // textSize.height);
  ...
  for (s32 iChar = 0; iChar < stringLength; iChar++) {
    ...
    cv::putText(img, std::string(tmpBuffer), curOrg, fontFace, fontScale, color,
...
void CvPutTextWithBackground(cv::Mat &img, const char *text, cv::Point org,
  ...
  cv::putText(img, text, textPoint, fontFace, fontScale, textColor, thickness,
              lineType);
}
#endif  //#if ANKICORETECH_EMBEDDED_USE_OPENCV
...
f32 GetTimeF32() {
  ...
  // Subtract startSeconds, so the floating point number has reasonable
  // precision
  if (startCounter == 0) {
  ...
  const f32 timeInSeconds =
      (f32)(time.tv_sec - startSeconds) + ((f32)time.tv_usec / 1000000.0f);
  ...
  const f32 timeInSeconds =
      (f32)(ts.tv_sec) + (f32)(ts.tv_nsec) / 1000000000.0f;
#endif
...
f64 GetTimeF64() {
  ...
  const f64 timeInSeconds =
      (f64)(time.tv_sec - startSeconds) + ((f64)time.tv_usec / 1000000.0);
#elif defined(__EDG__)  // ARM-MDK
  const f64 timeInSeconds =
      Anki::Vector::HAL::GetMicroCounter() * (1.0 / 1000000.0);
#else                   // Generic Unix
  timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
  const f64 timeInSeconds =
      (f64)(ts.tv_sec) + (f64)(ts.tv_nsec) * (1.0 / 1000000000.0);
#endif
...
u32 GetTimeU32() {
  ...
  // Subtract startCounter, so the floating point number has reasonable
  // precision
  if (startCounter == 0) {
    startCounter = counter.QuadPart;
  }
...
static u64 FiletimeDelta(const FILETIME &secondTime,
                         const FILETIME &firstTime) {
...
f32 GetCpuUsage() {
  ...
  if (firstCall) {
    ...
    // percentUsage = 100.0f * static_cast<f32>(numCpus) *
    // static_cast<f32>(totalSystemDelta - idleDelta) /
    // static_cast<f32>(totalSystemDelta);
    percentUsage = 100.0 * static_cast<f64>(totalSystemDelta - idleDelta) /
                   static_cast<f64>(totalSystemDelta);
  }
  ...
  curCpuTime = static_cast<f64>(usage.ru_utime.tv_sec) +
               static_cast<f64>(usage.ru_utime.tv_usec) / 1000000.0 +
               static_cast<f64>(usage.ru_stime.tv_sec) +
               static_cast<f64>(usage.ru_stime.tv_usec) / 1000000.0;

  if (firstCall) {
    percentUsage = 0;
    firstCall = false;
  } else {
    // TODO: Check that everything works, with the number of cores and
    // everything
    percentUsage =
        100.0 * (curCpuTime - lastCpuTime) / (curTime - lastTime) / numCpus;
  }
