...
Different classes and functions need several variable-argument parameters and
flags. These flag classes help separate interface and implementation.
...
class Buffer {
 public:
  // zeroAllocatedMemory - On initialization, set all values in the buffer to
  // zero useBoundaryFillPatterns - Buffers can have fill patterns at the edges
  // of each piece isFullyAllocated - If true, the input raw buffer is full of
  // data, and the object should have usedSize == totalSize.
  Buffer();
...
template <>
class TypeCharacteristics<bool> {
 public:
...
template <>
class TypeCharacteristics<char*> {
 public:
  const static bool isBasicType = false;
  const static bool isInteger = false;
  const static bool isSigned = false;
...
template <>
class TypeCharacteristics<const char*> {
 public:
  const static bool isBasicType = false;
  const static bool isInteger = false;
  const static bool isSigned = false;
