...
class Point {
 public:
  ...
  void SetCast(const Point<InType>& in);
...
class Rectangle {
 public:
  ...
  // The left, right, top, and bottom coordinates are the coordinates of the
  // cracks between the pixels. The top-left pixel's center is at (.5, .5)
  // WARNING: The right value should not be the index of the right pixel, but
  // one more "rightPixelIndex + 1" (i.e. the value of the edge to the right of
  // the pixel). Likewise for the bottom value.
  Rectangle(const Type left, const Type right, const Type top,
            const Type bottom);
  ...
  Point<OutType> ComputeCenter() const;

  // If scalePercent is less-than 1.0, the rectangle is shrunk around its center
  // If scalePercent is greater-than 1.0, the rectangle is expanded around its
  // center
  template <typename OutType>
  Rectangle<OutType> ComputeScaledRectangle(const f32 scalePercent) const;
...
class Quadrilateral {
 public:
  ...
  Quadrilateral(const Point<Type>& corner1, const Point<Type>& corner2,
                const Point<Type>& corner3, const Point<Type>& corner4);
  ...
  template <typename OutType>
  Rectangle<OutType> ComputeBoundingRectangle() const;

  // Returns a copy of this Quadrilateral with sorted corners, so they are
  // clockwise around the centroid Warning: This may give weird results for
  // non-convex quadrilaterals
  template <typename OutType>
  Quadrilateral<OutType> ComputeClockwiseCorners() const;

  template <typename OutType>
  Quadrilateral<OutType> ComputeRotatedCorners(const f32 radians) const;
  ...
  // different types
  template <typename InType>
  void SetCast(const Quadrilateral<InType>& quad2);
