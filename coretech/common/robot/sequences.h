...
LinearSequence<Type>::LinearSequence()
    : size(-1), start(-1), increment(static_cast<Type>(-1)) {
  this->size = -1;
}

template <typename Type>
LinearSequence<Type>::LinearSequence(const Type start, const Type end)
    : start(start), increment(1) {
  this->size = computeSize(this->start, this->increment, end);
}

template <typename Type>
LinearSequence<Type>::LinearSequence(const Type start, const Type increment,
                                     const Type end)
...
template <typename Type>
LinearSequence<Type>::LinearSequence(const Type start, const Type increment,
                                     const Type /*end*/, const s32 size)
    : size(size), start(start), increment(increment) {}

template <typename Type>
Array<Type> LinearSequence<Type>::Evaluate(MemoryStack &memory,
                                           const Flags::Buffer flags) const {
  const s32 numRows = 1;
...
template <typename Type>
Result LinearSequence<Type>::Evaluate(ArraySlice<Type> out) const {
  const s32 size = this->get_size();
  Array<Type> &outArray = out.get_array();

  AnkiConditionalErrorAndReturnValue(
      outArray.IsValid(), RESULT_FAIL_INVALID_OBJECT,
      "LinearSequence<Type>::Evaluate", "Invalid array");

  AnkiConditionalErrorAndReturnValue(
      out.get_ySlice().get_size() == 1 && out.get_xSlice().get_size() == size,
      RESULT_FAIL_INVALID_OBJECT, "LinearSequence<Type>::Evaluate",
      "Invalid array");
...
template <typename Type>
Type LinearSequence<Type>::get_start() const {
  return start;
}

template <typename Type>
Type LinearSequence<Type>::get_increment() const {
  return increment;
}

template <typename Type>
s32 LinearSequence<Type>::get_size() const {
  return size;
}

template <typename Type>
s32 LinearSequence<Type>::computeSize(const Type start, const Type increment,
                                      const Type end) {
  if (start == end) {
    return 1;
  } else {
  ...
  AnkiConditionalErrorAndReturnValue(size >= 0, 0,
                                     "LinearSequence<Type>::computeSize",
...
template <typename Type>
LinearSequence<Type> IndexSequence(Type start, Type end, s32 arraySize) {
  return IndexSequence(start, 1, end, arraySize);
}

template <typename Type>
LinearSequence<Type> IndexSequence(Type start, Type increment, Type end,
                                   s32 arraySize) {
...
template <typename Type>
LinearSequence<Type> Linspace(const Type start, const Type end,
                              const s32 size) {
  Type increment;
...
template <typename Type>
Meshgrid<Type>::Meshgrid() : xGridVector(), yGridVector() {}

template <typename Type>
Meshgrid<Type>::Meshgrid(const LinearSequence<Type> xGridVector,
                         const LinearSequence<Type> yGridVector)
    : xGridVector(xGridVector), yGridVector(yGridVector) {}

template <typename Type>
Array<Type> Meshgrid<Type>::EvaluateX1(bool isOutColumnMajor,
                                       MemoryStack &memory,
                                       const Flags::Buffer flags) const {
  const s32 numRows = 1;
  const s32 numCols =
      this->xGridVector.get_size() * this->yGridVector.get_size();
...
template <typename Type>
Array<Type> Meshgrid<Type>::EvaluateX2(MemoryStack &memory,
                                       const Flags::Buffer flags) const {
...
template <typename Type>
Array<Type> Meshgrid<Type>::EvaluateY1(bool isOutColumnMajor,
                                       MemoryStack &memory,
                                       const Flags::Buffer flags) const {
  const s32 numRows = 1;
  const s32 numCols =
...
template <typename Type>
Array<Type> Meshgrid<Type>::EvaluateY2(MemoryStack &memory,
                                       const Flags::Buffer flags) const {
  const s32 numRows = this->yGridVector.get_size();
  const s32 numCols = this->xGridVector.get_size();
...
template <typename Type>
Result Meshgrid<Type>::EvaluateX1(bool isOutColumnMajor,
                                  ArraySlice<Type> out) const {
  ...
  AnkiConditionalErrorAndReturnValue(
      outArray.IsValid(), RESULT_FAIL_INVALID_OBJECT,
      "Meshgrid<Type>::Evaluate", "Invalid array");

  AnkiConditionalErrorAndReturnValue(
      out.get_ySlice().get_size() == 1 &&
          out.get_xSlice().get_size() == numElements,
      RESULT_FAIL_INVALID_SIZE, "Meshgrid<Type>::Evaluate",
      "Array out is not the correct size");
...
template <typename Type>
Result Meshgrid<Type>::EvaluateX2(ArraySlice<Type> out) const {
  const s32 xGridSize = this->xGridVector.get_size();
  const s32 yGridSize = this->yGridVector.get_size();
  // const s32 numElements = xGridSize * yGridSize;
  ...
  AnkiConditionalErrorAndReturnValue(
      out.get_ySlice().get_size() == yGridSize &&
          out.get_xSlice().get_size() == xGridSize,
      RESULT_FAIL_INVALID_SIZE, "Meshgrid<Type>::Evaluate",
      "Array out is not the correct size");
...
template <typename Type>
Result Meshgrid<Type>::EvaluateY1(bool isOutColumnMajor,
                                  ArraySlice<Type> out) const {
  const s32 xGridSize = this->xGridVector.get_size();
  const s32 yGridSize = this->yGridVector.get_size();
  const s32 numElements = xGridSize * yGridSize;
...
template <typename Type>
Result Meshgrid<Type>::EvaluateY2(ArraySlice<Type> out) const {
  const s32 xGridSize = this->xGridVector.get_size();
  const s32 yGridSize = this->yGridVector.get_size();
  // const s32 numElements = xGridSize * yGridSize;
...
template <typename Type>
s32 Meshgrid<Type>::get_numElements() const {
  const s32 numElements = xGridVector.get_size() * yGridVector.get_size();
  return numElements;
}

template <typename Type>
inline const LinearSequence<Type> &Meshgrid<Type>::get_xGridVector() const {
  return xGridVector;
}

template <typename Type>
inline const LinearSequence<Type> &Meshgrid<Type>::get_yGridVector() const {
  return yGridVector;
}
...
template <>
s32 LinearSequence<u8>::computeSize(const u8 start, const u8 increment,
                                    const u8 end);
template <>
s32 LinearSequence<f32>::computeSize(const f32 start, const f32 increment,
                                     const f32 end);
template <>
s32 LinearSequence<f64>::computeSize(const f64 start, const f64 increment,
                                     const f64 end);
}  // namespace Embedded
}  // namespace Anki
