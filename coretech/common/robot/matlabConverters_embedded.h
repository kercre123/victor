...
void mxArrayToArray(const mxArray *const array, Array<Type> &mat);

// Convert a Matlab mxArray to an Anki::Array. Allocate and return the
// Anki::Array
template <typename Type>
Array<Type> mxArrayToArray(const mxArray *const matlabArray,
                           MemoryStack &memory);

// This works with cell arrays if every element is the same type
template <typename Type>
Array<Array<Type> > mxCellArrayToArray(const mxArray *const matlabArray,
                                       MemoryStack &memory);

// Copy a string to a null-terminated string, allocated from the MemoryStack
// Overloaded version of Matlab's mxArrayToString() that uses a MemoryStack
// instead of the heap
...
// The strings are allocated from "MemoryStack &memory", so do not need to be
// freed
Array<char *> mxCellArrayToStringArray(const mxArray *const matlabArray,
                                       MemoryStack &memory);

// Convert an Anki::Array to a Matlab mxArray. Allocate and return the mxArray
template <typename Type>
mxArray *arrayToMxArray(const Array<Type> &array);
...
template <typename Type>
void mxArrayToArray(const mxArray *const array, Array<Type> &mat) {
  AnkiConditionalError(mxGetNumberOfDimensions(array) == 2, "mxArrayToArray",
                       "Matlab array should be 2D");

  const int nrows = mat.get_size(0);
  const int ncols = mat.get_size(1);

  const Type *const matlabMatrixStartPointer =
      reinterpret_cast<const Type *>(mxGetData(array));

  if (mxGetM(array) != nrows || mxGetN(array) != ncols) {
    AnkiError("mxArrayToArray",
              "Matlab array size does not match Anki::Embedded::Array size: "
              "(%d x %d) vs (%d x %d)",
  ...
  if (matlabClassId != templateClassId) {
    AnkiError("mxArrayToArray",
              "mxArrayToArray<Type>(array,mat) - Matlab classId does not match "
              "with template %d!=%d",
...
}  // template<typename Type> void mxArrayToArray(const mxArray * const array,
   // Array<Type> &mat)
...
Array<Type> mxArrayToArray(const mxArray *const matlabArray,
                           MemoryStack &memory) {
  const Type *const matlabMatrixStartPointer =
      reinterpret_cast<const Type *>(mxGetData(matlabArray));
  ...
  if (numDimensions != 2) {
    AnkiError("mxArrayToArray",
              "mxArrayToArray<Type> - Matlab array must be 2D");
    return Array<Type>();
  }

  if (mxIsCell(matlabArray)) {
    AnkiError("mxArrayToArray",
              "Input can't be a cell array. Use mxCellArrayToArray() instead.");
    return Array<Type>(static_cast<s32>(dimensions[0]),
                       static_cast<s32>(dimensions[1]), memory);
  }
  ...
  if (matlabClassId != templateClassId) {
    AnkiError("mxArrayToArray",
              "mxArrayToArray<Type> - Matlab classId does not match with "
              "template %d!=%d",
              matlabClassId, templateClassId);
    return Array<Type>();
  }

  Array<Type> array(static_cast<s32>(dimensions[0]),
                    static_cast<s32>(dimensions[1]), memory);

  if (!array.IsValid()) {
    AnkiError("mxArrayToArray",
              "mxArrayToArray<Type> - Could not allocate array");
    return Array<Type>();
  }
  ...
  return array;
}  // template<typename Type> Array<Type> mxArrayToArray(const mxArray * const
   // matlabArray)
...
Array<Array<Type> > mxCellArrayToArray(const mxArray *const matlabArray,
  ...
  if (numDimensions != 2) {
    AnkiError("mxCellArrayToArray",
              "mxCellArrayToArray<Type> - Matlab array must be 2D");
  ...
  if (dimensions[0] == 0 || dimensions[1] == 0) {
    return Array<Array<Type> >(static_cast<s32>(dimensions[0]),
                               static_cast<s32>(dimensions[1]), memory);
  }
  ...
  if (matlabClassId != templateClassId) {
    AnkiError("mxCellArrayToArray",
              "mxCellArrayToArray<Type> - Matlab classId does not match with "
              "template %d!=%d",
              matlabClassId, templateClassId);
    return Array<Array<Type> >();
  }

  Array<Array<Type> > array(static_cast<s32>(dimensions[0]),
                            static_cast<s32>(dimensions[1]), memory);

  if (!array.IsValid()) {
    AnkiError("mxCellArrayToArray",
              "mxCellArrayToArray<Type> - Could not allocate array");
    return Array<Array<Type> >();
  }
  ...
  if (numElements > 0) {
    ...
    for (mwSize y = 0; y < dimensions[0]; ++y) {
      ...
      for (mwSize x = 0; x < dimensions[1]; ++x) {
        ...
        const mwIndex cellIndex =
            mxCalcSingleSubscript(matlabArray, 2, &subs[0]);
...
inline char *mxArrayToString(const mxArray *const matlabArray,
                             MemoryStack &memory) {
  const mxClassID matlabClassId = mxGetClassID(matlabArray);

  AnkiConditionalErrorAndReturnValue(
      matlabClassId == mxCHAR_CLASS, NULL, "mxArrayToString",
      "Matlab classId is not char %d!=%d", matlabClassId, mxCHAR_CLASS);

  char *curMatlabString = mxArrayToString(matlabArray);

  AnkiConditionalErrorAndReturnValue(curMatlabString, NULL, "mxArrayToString",
  ...
  AnkiConditionalErrorAndReturnValue(curCString, NULL, "mxArrayToString",
...
inline Array<char *> mxCellArrayToStringArray(const mxArray *const matlabArray,
                                              MemoryStack &memory) {
  const mwSize numDimensions = mxGetNumberOfDimensions(matlabArray);
  const mwSize *dimensions = mxGetDimensions(matlabArray);

  if (numDimensions != 2) {
    AnkiError("mxCellArrayToStringArray",
              "mxCellArrayToStringArray - Matlab array must be 2D");
    return Array<char *>();
  }
  ...
  if (matlabClassId != mxCHAR_CLASS) {
    AnkiError("mxCellArrayToStringArray",
              "mxCellArrayToStringArray - Matlab classId does not match with "
              "template %d!=%d",
              matlabClassId, mxCHAR_CLASS);
    return Array<char *>();
  }

  Array<char *> array(static_cast<s32>(dimensions[0]),
                      static_cast<s32>(dimensions[1]), memory);

  if (!array.IsValid()) {
    AnkiError("mxCellArrayToStringArray",
              "mxCellArrayToStringArray - Could not allocate array");
    return Array<char *>();
  }
  ...
  if (numElements > 0) {
    for (mwSize y = 0; y < outputDims[0]; ++y) {
      ...
      for (mwSize x = 0; x < outputDims[1]; ++x) {
        ...
        const mwIndex cellIndex =
            mxCalcSingleSubscript(outputArray, 2, &subs[0]);
