...
Result Zeros(Array<Type> &out) {
  AnkiConditionalErrorAndReturnValue(out.IsValid(), RESULT_FAIL_INVALID_OBJECT,
                                     "Zeros", "out is invalid");
...
template <typename Type>
Array<Type> Zeros(const s32 arrayHeight, const s32 arrayWidth,
                  MemoryStack &memory) {
  Array<Type> out(arrayHeight, arrayWidth, memory,
                  Flags::Buffer(true, false, false));

  return out;
}

template <typename Type>
Result Ones(Array<Type> &out) {
  AnkiConditionalErrorAndReturnValue(out.IsValid(), RESULT_FAIL_INVALID_OBJECT,
                                     "Ones", "out is invalid");
...
template <typename Type>
Array<Type> Ones(const s32 arrayHeight, const s32 arrayWidth,
                 MemoryStack &memory) {
  Array<Type> out(arrayHeight, arrayWidth, memory,
...
template <typename Type>
Array<Type> Eye(const s32 arrayHeight, const s32 arrayWidth,
                MemoryStack &memory) {
  Array<Type> out(arrayHeight, arrayWidth, memory,
                  Flags::Buffer(false, false, false));
...
template <typename Type>
Array<Type> Exp(const Array<Type> &in, MemoryStack &memory) {
  Array<Type> out(in.get_size(0), in.get_size(1), memory,
                  Flags::Buffer(false, false, false));
