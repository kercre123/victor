...
Basic configuation file for all common files, as well as all other coretech
libraries. Everything in this file should be compatible with plain C, as well as
C++
...
//
// Compiler-specific configuration, with various defines that make different
// compilers work on the same code
//
...
#pragma warning( \
    disable : 4127)  // Disable warning for conditional expression is constant
//#pragma warning(2: 4100) // Enable warning for unused variable warning
#pragma warning( \
    disable : 4800)  // Disable warning for 'BOOL' : forcing value to bool
...
// Warning: High levels of compiler optimization could cause this test to not
// work
#define isnan(a) ((a) != (a))
...
#define MEMORY_ALIGNMENT_RAW \
  16  // Sometimes the preprocesor can't handle the safer version
      // MEMORY_ALIGNMENT
#define MEMORY_ALIGNMENT ((size_t)(MEMORY_ALIGNMENT_RAW))

// To make processing faster, some kernels require image widths that are a
// multiple of ANKI_VISION_IMAGE_WIDTH_MULTIPLE
...
#define ANKI_DEBUG_MINIMAL \
  0  // Only check and output issues with explicit unit tests
#define ANKI_DEBUG_ERRORS \
  10  // Check and output AnkiErrors and explicit unit tests
#define ANKI_DEBUG_ERRORS_AND_WARNS \
  20  // Check and output AnkiErrors, AnkiWarns, and explicit unit tests
#define ANKI_DEBUG_ERRORS_AND_WARNS_AND_ASSERTS \
  30  // Check and output AnkiErrors, AnkiWarns, AnkiAsserts, and explicit unit
      // tests
#define ANKI_DEBUG_ALL \
  40  // Check and output AnkiErrors, AnkiWarns, and explicit unit tests, plus
      // run any additional extensive tests
