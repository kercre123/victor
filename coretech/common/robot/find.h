...
template <typename Type1, typename Operator, typename Type2>
Find<Type1, Operator, Type2>::Find(const Array<Type1> &array1,
                                   const Array<Type2> &array2)
    : array1(array1),
      compareWithValue(false),
      array2(array2),
      value(static_cast<Type2>(0)),
...
template <typename Type1, typename Operator, typename Type2>
Find<Type1, Operator, Type2>::Find(const Array<Type1> &array,
                                   const Type2 &value)
    : array1(array),
      ...
      value(value),
      numOutputDimensions(0)
// array2 is initialized to array, but this is just because it has to point to
// something, though it should not be accessed
{
  if (!array1.IsValid()) {
    this->isValid = false;
    AnkiError("Find", "Invalid inputs");
...
template <typename Type1, typename Operator, typename Type2>
Result Find<Type1, Operator, Type2>::Evaluate(Array<s32> &indexes,
                                              MemoryStack &memory) const {
  AnkiConditionalErrorAndReturnValue(AreValid(*this, memory),
                                     RESULT_FAIL_INVALID_OBJECT,
                                     "Find.Evaluate", "Invalid objects");

  AnkiConditionalErrorAndReturnValue(
      this->numOutputDimensions == 1, RESULT_FAIL_INVALID_PARAMETER,
      "Find.Evaluate",
      "One-dimensional Evaluate only works with one-dimensional Array input");
  ...
  if (this->get_numMatches() == 0) return RESULT_OK;

  AnkiConditionalErrorAndReturnValue(AreValid(indexes),
                                     RESULT_FAIL_OUT_OF_MEMORY, "Find.Evaluate",
                                     "Invalid objects");
...
template <typename Type1, typename Operator, typename Type2>
Result Find<Type1, Operator, Type2>::Evaluate(Array<s32> &yIndexes,
                                              Array<s32> &xIndexes,
                                              MemoryStack &memory) const {
  AnkiConditionalErrorAndReturnValue(AreValid(*this, memory),
                                     RESULT_FAIL_INVALID_OBJECT,
...
template <typename Type1, typename Operator, typename Type2>
void Find<Type1, Operator, Type2>::Initialize() {
  this->isValid = true;

  // If the input is a row vector, then it can be used for 1-dimensional
  // indexing
...
template <typename Type1, typename Operator, typename Type2>
Result Find<Type1, Operator, Type2>::ComputeNumMatches() const {
  const s32 arrayHeight = array1.get_size(0);
  const s32 arrayWidth = array1.get_size(1);
  ...
  if (this->numMatchesComputed) {
    AnkiAssert(newNumMatches ==
               this->numMatches);  // This should only happen if the data is
                                   // changed, which it shouldn't be
  }
  ...
  return RESULT_OK;
}  // template<typename Type1, typename Operator, typename Type2> s32
   // Find<Type1,Operator,Type2>::ComputeNumMatches() const

template <typename Type1, typename Operator, typename Type2>
Result Find<Type1, Operator, Type2>::ComputeLimits() const {
  const s32 arrayHeight = array1.get_size(0);
  const s32 arrayWidth = array1.get_size(1);
  ...
  return RESULT_OK;
}  // template<typename Type1, typename Operator, typename Type2> Rectangle<s32>
   // Find<Type1,Operator,Type2>::ComputeLimits() const

template <typename Type1, typename Operator, typename Type2>
template <typename ArrayType>
Result Find<Type1, Operator, Type2>::SetArray(Array<ArrayType> &out,
  ...
  AnkiConditionalErrorAndReturnValue(AreValid(*this, out),
                                     RESULT_FAIL_INVALID_OBJECT,
                                     "Find.SetArray", "Invalid objects");

  AnkiConditionalErrorAndReturnValue(
      AreEqualSize(array1, out), RESULT_FAIL_INVALID_SIZE, "Find.SetArray",
      "out is not the same size as the input(s)");
...
template <typename Type1, typename Operator, typename Type2>
template <typename ArrayType>
Result Find<Type1, Operator, Type2>::SetArray(
    Array<ArrayType> &out, const Array<ArrayType> &in,
    const s32 findWhichDimension) const {
  AnkiConditionalErrorAndReturnValue(AreValid(*this, in, out),
                                     RESULT_FAIL_INVALID_OBJECT,
                                     "Find.SetArray", "Invalid objects");

  AnkiConditionalErrorAndReturnValue(
      this->numOutputDimensions == 1, RESULT_FAIL_INVALID_SIZE, "Find.SetArray",
      "One-dimensional SetArray only works with one-dimensional Array input");

  AnkiConditionalErrorAndReturnValue(
      findWhichDimension == 0 || findWhichDimension == 1,
      RESULT_FAIL_INVALID_PARAMETER, "Find.SetArray",
      "findWhichDimension must be zero or one");
  ...
  if (findWhichDimension == 0) {
    AnkiConditionalErrorAndReturnValue(
        outHeight == numMatches && outWidth == inWidth,
        RESULT_FAIL_INVALID_SIZE, "Find.SetArray",
        "out is not the correct size");

    AnkiConditionalErrorAndReturnValue(
        inHeight == MAX(array1Height, array1Width), RESULT_FAIL_INVALID_SIZE,
        "Find.SetArray", "in is not the correct size");
  } else {
    AnkiConditionalErrorAndReturnValue(
        outHeight == inHeight && outWidth == numMatches,
        RESULT_FAIL_INVALID_SIZE, "Find.SetArray",
        "out is not the correct size");

    AnkiConditionalErrorAndReturnValue(
        inWidth == MAX(array1Height, array1Width), RESULT_FAIL_INVALID_SIZE,
        "Find.SetArray", "in is not the correct size");
  }

  // This is a two-deep nested set of binary if-thens. Each of the four "leaves"
  // will iterate through the entire array, and set the appropriate values.
  //
  // Levels:
  ...
  if (this->compareWithValue) {
    ...
    if (findWhichDimension == 0) {
      ...
      // i iterates on the widths of Array array1 and Array array2, and the
      // height of Array in
      for (s32 i = 0; i < array1Width; i++) {
...
template <typename ArrayType>
Result Find<Type1, Operator, Type2>::SetArray(
    Array<ArrayType> &out, const ArrayType value,
    const s32 findWhichDimension) const {
  return RESULT_OK;
}
...
template <typename Type1, typename Operator, typename Type2>
template <typename ArrayType>
Array<ArrayType> Find<Type1, Operator, Type2>::SetArray(
    const Array<ArrayType> &in, const s32 findWhichDimension,
    MemoryStack &memory) const {
  const s32 numMatches = this->get_numMatches();
...
template <typename Type1, typename Operator, typename Type2>
bool Find<Type1, Operator, Type2>::IsValid() const {
  if (!this->isValid) return false;

  // TODO: check other things with the arrays, like to guess if they've been
  // changed
...
template <typename Type1, typename Operator, typename Type2>
s32 Find<Type1, Operator, Type2>::get_numMatches() const {
  if (!numMatchesComputed) ComputeNumMatches();

  return numMatches;
}

template <typename Type1, typename Operator, typename Type2>
const Rectangle<s32> &Find<Type1, Operator, Type2>::get_limits() const {
  if (!limitsComputed) ComputeLimits();
