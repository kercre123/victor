...
// These mex-only functions and #defines output to the Matlab command line, in a
// similar way to the AnkiError #defines
...
#define AnkiWarn(eventName, eventValue_format, ...)                           \
  {                                                                           \
    _AnkiMatlab_Logf(ANKI_LOG_LEVEL_WARN, eventName, (eventValue_format),     \
...
#define AnkiError(eventName, eventValue_format, ...)                     \
  _AnkiMatlab_Logf(ANKI_LOG_LEVEL_ERROR, eventName, (eventValue_format), \
                   __FILE__, __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
...
class Matlab : public SharedMatlabInterface {
 public:
  ...
  Result PutArray(const Array<Type> &matrix, const std::string name);

  template <typename Type>
  Array<Type> GetArray(const std::string name, MemoryStack &memory);

  template <typename Type>
  Result PutQuad(const Quadrilateral<Type> &quad, const std::string name);

  template <typename Type>
  Quadrilateral<Type> GetQuad(const std::string name);

  // Overload the templated Put() coming from base class for Point<s16>
  Result PutPoints(const Point<s16> *values, s32 nValues,
...
Result Matlab::PutArray(const Array<Type> &matrix, const std::string name) {
  ...
  AnkiConditionalErrorAndReturnValue(ep, RESULT_FAIL, "Anki.PutArray<Type>",
                                     "Matlab engine is not started/connected");

  const mxClassID whichClass = GetMatlabClassID<Type>();
  AnkiConditionalErrorAndReturnValue(whichClass != mxUNKNOWN_CLASS, RESULT_FAIL,
                                     "Anki.PutArray<Type>",
                                     "Unknown type to convert to a mxClassID");
  ...
  AnkiConditionalErrorAndReturnValue(
      matrixHeight <= std::numeric_limits<s32>::max(), RESULT_FAIL_INVALID_SIZE,
      "Anki.PutArray<Type>",
      "Matrix too large to use call to matrix.Pointer()");

  for (mwSize y = 0; y < matrixHeight; y++) {
    memcpy(data, matrix.Pointer(static_cast<s32>(y), 0),
           matrixWidth * sizeof(Type));
    data += matrixWidth;
  }
  ...
  /*
   ...
   const std::string matlabTypeName =
   Anki::Embedded::ConvertToMatlabTypeString(typeid(Type).name(), sizeof(Type));

   EvalStringEcho("%s=zeros([%d,%d],'%s');", name.data(), matrixHeight,
   matrixWidth, matlabTypeName.data());
...
template <typename Type>
Result Matlab::PutQuad(const Quadrilateral<Type> &quad,
                       const std::string name) {
  AnkiConditionalErrorAndReturnValue(ep, RESULT_FAIL, "Anki.PutQuad<Type>",
                                     "Matlab engine is not started/connected");

  // Initially just put a 8-element array into Matlab
  Type tempArray[8] = {quad[0].x, quad[1].x, quad[2].x, quad[3].x,
...
Array<Type> Matlab::GetArray(const std::string name, MemoryStack &memory) {
  AnkiConditionalErrorAndReturnValue(ep, Array<Type>(0, 0, NULL, 0),
                                     "Anki.GetArray<Type>",
                                     "Matlab engine is not started/connected");
  ...
  AnkiConditionalErrorAndReturnValue(
      arrayTmp, Array<Type>(0, 0, NULL, 0), "Anki.GetArray<Type>",
      "%s could not be got from Matlab", tmpName.data());

  AnkiConditionalErrorAndReturnValue(
      mxGetClassID(arrayTmp) == GetMatlabClassID<Type>(),
      Array<Type>(0, 0, NULL, 0), "Anki.GetArray<Type>",
      "matlabClassId != ankiVisionClassId");

  const size_t numCols = mxGetM(arrayTmp);
  const size_t numRows = mxGetN(arrayTmp);
  // Unused? const s32 stride =
  // Array<Type>::ComputeRequiredStride(static_cast<s32>(numCols),
  // Flags::Buffer(true,false));

  Array<Type> ankiArray(static_cast<s32>(numRows), static_cast<s32>(numCols),
                        memory);
...
Quadrilateral<Type> Matlab::GetQuad(const std::string name) {
  ...
  AnkiConditionalErrorAndReturnValue(
      mxQuad != NULL, Quadrilateral<Type>(), "Anki.GetQuad",
      "No variable named '%s' found.", name.c_str());
  AnkiConditionalErrorAndReturnValue(
      mxGetM(mxQuad) == 4 && mxGetN(mxQuad) == 2, Quadrilateral<Type>(),
      "Anki.GetQuad", "Variable '%s' is not 4x2 in size.", name.c_str());
  AnkiConditionalErrorAndReturnValue(
      mxGetClassID(mxQuad) == mxDOUBLE_CLASS, Quadrilateral<Type>(),
      "Anki.GetQuad", "Variable '%s' must be DOUBLE to get as quad.",
      name.c_str());
  ...
  return Quadrilateral<Type>(
      Point<Type>(static_cast<Type>(x[0]), static_cast<Type>(y[0])),
