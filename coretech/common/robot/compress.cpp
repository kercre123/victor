...
namespace Anki {
namespace Embedded {
Result EncodeRunLengthBinary(const u8 *restrict in, const s32 inLength,
                             u8 *restrict out, const s32 outMaxLength,
  ...
  for (s32 iByte = 1; iByte < inLength; iByte++) {
    ...
    const bool isNewColor = (curComponentColor == 0 && in[iByte] != 0) ||
                            (curComponentColor == 1 && in[iByte] == 0);

    if (isNewColor || curComponentCount > 126) {
      out[outCompressedLength] =
          static_cast<u8>(curComponentColor * 128 + curComponentCount);
      outCompressedLength++;
...
Result DecodeRunLengthBinary(const u8 *restrict in, const s32 inLength,
                             u8 *restrict out, const s32 outMaxLength,
