...
1. InitBenchmarking()
2. At the beginning of the section you want to benchmark, put
BeginBenchmark("event type");
3. At the end of the section you want to benchmark, put EndBenchmark("event
type");
4. When you're done running the program, call PrintBenchmarkResults() to print
the results
...
typedef struct BenchmarkElement {
  ...
  void Print(const bool verbose = true, const bool microseconds = true,
             const FixedLengthList<s32> *minCharacterToPrint = NULL) const;

  // Like snprintf(). Returns the number of characters printed, not including
  // the final null byte.
  s32 Snprint(char *buffer, const s32 bufferLength, const bool verbose = true,
...
typedef struct ShowBenchmarkParameters {
  ...
  ShowBenchmarkParameters(const char *name, const bool showExclusiveTime,
                          const u8 *color);  //< Color is {R,G,B}
} ShowBenchmarkParameters;

// Call this before doing any benchmarking, to clear the buffer of
// benchmarkEvents. Can be called multiple times.
void InitBenchmarking();

// Use these functions to add a new event to the list. These functions are very
// fast.
//
// WARNING: name must be in globally available memory
//
// WARNING: the character string must be less than BenchmarkElement::NAME_LENGTH
// bytes
//
// WARNING: Using the same name for different benchmark events
//
// WARNING: nesting BeginBenchmark() and EndBenchmark() events that have the
// same name won't work. This is okay: BeginBenchmark("a"); BeginBenchmark("b");
// EndBenchmark("b"); EndBenchmark("a"); This is not okay: BeginBenchmark("a");
// BeginBenchmark("a"); EndBenchmark("a"); EndBenchmark("a"); This is not okay:
// BeginBenchmark("a"); BeginBenchmark("b"); EndBenchmark("a");
// EndBenchmark("b");
...
Result PrintBenchmarkResults(const FixedLengthList<BenchmarkElement> &results,
                             const bool verbose = true,
                             const bool microseconds = true);

// Compile and print out all the benchmark events that were recorded
Result ComputeAndPrintBenchmarkResults(const bool verbose,
                                       const bool microseconds,
                                       MemoryStack scratch);

// "benchmarkElements" is a list of lists of benchmarkElements, where each list
// is a set from one session computed from ComputeBenchmarkResults() This
// function sorts them and prints the elementPercentile" percentile
Result PrintPercentileBenchmark(
    const FixedLengthList<FixedLengthList<BenchmarkElement> >
        &benchmarkElements,
    const s32 numRuns, const f32 elementPercentile, MemoryStack scratch);
...
Result ShowBenchmarkResults(
    ...
    const f32 pixelsPerMillisecond, const s32 imageHeight,
    const s32 imageWidth);

s32 GetNameIndex(const char *name,
                 const FixedLengthList<BenchmarkElement> &outputResults);
#else
