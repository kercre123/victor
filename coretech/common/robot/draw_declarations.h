...
// If lineWidth < 1, then the box will be solid. Otherwise, the inside will be
// filled with backgroundColor
template <typename Type>
Result DrawRectangle(Array<Type> &image, Point<s16> point1, Point<s16> point2,
                     const s32 lineWidth, const Type lineColor,
                     const Type backgroundColor);

// Fill in the pixels inside quad. The center of the top-left pixel is
// (0.5,0.5). If the quad is not convex, this function will fill a set of pixels
// such that: convexHull >= numPixelsFilled >= nonConvexFill
template <typename Type>
Result DrawFilledConvexQuadrilateral(Array<Type> &image,
                                     Quadrilateral<f32> quad, const Type color);
...
template <typename Type>
Result DrawExampleSquaresImage(Array<Type> &image);

// points is a list of point coordinates. DrawPoints draws them on image.
template <typename Type>
Result DrawPoints(const FixedLengthList<Point<Type> > &points,
                  const u8 grayvalue, Array<u8> &image);
}  // namespace Embedded
}  // namespace Anki
