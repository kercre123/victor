...
// Works the same as Matlab's unique.m, but if the input is an integer array
// with a small maximum value, it uses a different (often faster) algorithm

// Example:
// uniqueValues = mexUnique(inputArray)

void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  // If there's too many possible unique elements, this method uses too much
  // memory
  ...
  AnkiConditionalErrorAndReturn(
      nrhs == 1 && nlhs == 1, "mexUnique",
      "Call this function as follows: [uniqueValues = mexUnique(inputArray);");
  ...
  if (mxGetClassID(prhs[0]) == mxINT32_CLASS) {
    ...
    // If there's too many possible unique elements, this method isn't
    // efficient, so just call the normal Matlab unique.m
  ...
  // If the class type is not supported, or the statistics are non-ideal, just
  // call Matlab's unique.m
  if (useMatlabDefault) {
