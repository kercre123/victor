...
Result AllocateAndSave(const mxArray *matlabArray, const char *filename,
                       const s32 compressionLevel, MemoryStack scratch1,
                       MemoryStack scratch2);
template <typename Type>
Result AllocateAndSaveCell(const mxArray *matlabArray, const char *filename,
                           const s32 compressionLevel, MemoryStack scratch1,
                           MemoryStack scratch2);
template <>
Result AllocateAndSaveCell<char *>(const mxArray *matlabArray,
                                   const char *filename,
                                   const s32 compressionLevel,
                                   MemoryStack scratch1, MemoryStack scratch2);
...
Result AllocateAndSave(const mxArray *matlabArray, const char *filename,
  ...
  if (!ankiArray.IsValid()) return RESULT_FAIL;
...
Result Save(const mxArray *matlabArray, const char *filename,
            const s32 compressionLevel, MemoryStack scratch1,
            MemoryStack scratch2) {
  ...
  if (mxIsCell(matlabArray)) {
    ...
    //  return AllocateAndSaveCell<f64>(matlabArray, filename, scratch1,
    //  scratch2);
    //} else if(matlabCellClassId == mxSINGLE_CLASS) {
    //  return AllocateAndSaveCell<f32>(matlabArray, filename, scratch1,
    //  scratch2);
    //} else if(matlabCellClassId == mxINT8_CLASS) {
    //  return AllocateAndSaveCell<s8>(matlabArray, filename, scratch1,
    //  scratch2);
    //} else if(matlabCellClassId == mxUINT8_CLASS) {
    //  return AllocateAndSaveCell<u8>(matlabArray, filename, scratch1,
    //  scratch2);
    //} else if(matlabCellClassId == mxINT16_CLASS) {
    //  return AllocateAndSaveCell<s16>(matlabArray, filename, scratch1,
    //  scratch2);
    //} else if(matlabCellClassId == mxUINT16_CLASS) {
    //  return AllocateAndSaveCell<u16>(matlabArray, filename, scratch1,
    //  scratch2);
    //} else if(matlabCellClassId == mxINT32_CLASS) {
    //  return AllocateAndSaveCell<s32>(matlabArray, filename, scratch1,
    //  scratch2);
    //} else if(matlabCellClassId == mxUINT32_CLASS) {
    //  return AllocateAndSaveCell<u32>(matlabArray, filename, scratch1,
    //  scratch2);
    //} else if(matlabCellClassId == mxINT64_CLASS) {
    //  return AllocateAndSaveCell<s64>(matlabArray, filename, scratch1,
    //  scratch2);
    //} else if(matlabCellClassId == mxUINT64_CLASS) {
    //  return AllocateAndSaveCell<u64>(matlabArray, filename, scratch1,
    //  scratch2);
    //} else
    if (matlabCellClassId == mxCHAR_CLASS) {
      return AllocateAndSaveCell<char *>(matlabArray, filename,
                                         compressionLevel, scratch1, scratch2);
    } else {
      AnkiAssert(false);
    }
  } else {  // if(mxIsCell(matlabArray))
    if (matlabClassId == mxDOUBLE_CLASS) {
      return AllocateAndSave<f64>(matlabArray, filename, compressionLevel,
                                  scratch1, scratch2);
    } else if (matlabClassId == mxSINGLE_CLASS) {
      return AllocateAndSave<f32>(matlabArray, filename, compressionLevel,
                                  scratch1, scratch2);
    } else if (matlabClassId == mxINT8_CLASS) {
      return AllocateAndSave<s8>(matlabArray, filename, compressionLevel,
                                 scratch1, scratch2);
    } else if (matlabClassId == mxUINT8_CLASS) {
      return AllocateAndSave<u8>(matlabArray, filename, compressionLevel,
                                 scratch1, scratch2);
    } else if (matlabClassId == mxINT16_CLASS) {
      return AllocateAndSave<s16>(matlabArray, filename, compressionLevel,
                                  scratch1, scratch2);
    } else if (matlabClassId == mxUINT16_CLASS) {
      return AllocateAndSave<u16>(matlabArray, filename, compressionLevel,
                                  scratch1, scratch2);
    } else if (matlabClassId == mxINT32_CLASS) {
      return AllocateAndSave<s32>(matlabArray, filename, compressionLevel,
                                  scratch1, scratch2);
    } else if (matlabClassId == mxUINT32_CLASS) {
      return AllocateAndSave<u32>(matlabArray, filename, compressionLevel,
                                  scratch1, scratch2);
    } else if (matlabClassId == mxINT64_CLASS) {
      return AllocateAndSave<s64>(matlabArray, filename, compressionLevel,
                                  scratch1, scratch2);
    } else if (matlabClassId == mxUINT64_CLASS) {
      return AllocateAndSave<u64>(matlabArray, filename, compressionLevel,
                                  scratch1, scratch2);
    } else {
      AnkiAssert(false);
    }
...
typedef struct SaveThreadParams {
  ...
  SaveThreadParams(const mxArray *matlabArray, const char *filename,
                   const s32 compressionLevel, MemoryStack scratch1,
                   MemoryStack scratch2)
...
ThreadResult SaveThread(void *voidSaveThreadParams) {
  SaveThreadParams *restrict saveThreadParams =
      reinterpret_cast<SaveThreadParams *>(voidSaveThreadParams);

  Save(saveThreadParams->matlabArray, saveThreadParams->filename,
       saveThreadParams->compressionLevel, saveThreadParams->scratch1,
       saveThreadParams->scratch2);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  AnkiConditionalErrorAndReturn(
      (nrhs >= 2 && nrhs <= 4) && nlhs == 0, "mexSaveEmbeddedArray",
      "Call this function as follows: mexSaveEmbeddedArray(array, filename, "
      "<compressionLeve>, <numThreads>); % array and filename may be cell "
      "arrays.");
  ...
  //
  // Note: This multithreading is a bit wierd, because Matlab doesn't like
  // threads allocating their own memory. It could be improved.
  //
  ...
  if (mxIsCell(prhs[0])) {
    if (mxIsCell(prhs[1])) {
      ...
      AnkiConditionalErrorAndReturn(
          mxIsChar(curMatlabArray), "mexSaveEmbeddedArray",
          "If one input is a cell both must be. The only exception is if the "
          "first input is a cell array of strings.");
    }
  }

  if (areCellArrays) {
    MemoryStack memory(mxMalloc(100000), 100000);
    AnkiConditionalErrorAndReturn(memory.IsValid(), "mexSaveEmbeddedArray",
                                  "Memory could not be allocated");

    Array<char *> filenames = mxCellArrayToStringArray(prhs[1], memory);
    const s32 numFilenames = filenames.get_size(1);

    SaveThreadParams **threadParams = reinterpret_cast<SaveThreadParams **>(
        mxMalloc(numThreads * sizeof(SaveThreadParams *)));
    ThreadHandle *threadHandles = reinterpret_cast<ThreadHandle *>(
        mxMalloc(numThreads * sizeof(ThreadHandle)));
    MemoryStack *scratch1s = reinterpret_cast<MemoryStack *>(
        mxMalloc(numThreads * sizeof(MemoryStack)));
    MemoryStack *scratch2s = reinterpret_cast<MemoryStack *>(
        mxMalloc(numThreads * sizeof(MemoryStack)));
    ...
    for (s32 iThread = 0; iThread < numThreads; iThread++) {
      ...
      AnkiConditionalErrorAndReturn(
          scratch1s[iThread].IsValid() && scratch2s[iThread].IsValid(),
          "mexSaveEmbeddedArray", "Memory could not be allocated");
    }
    ...
    for (s32 iFile = 0; iFile < numFilenames; iFile += numThreads) {
      for (s32 iThread = 0; iThread < numThreads; iThread++) {
        ...
        if ((curWidth == 1 && curHeight > 1) || (curWidth * 10 < curHeight)) {
          mexPrintf(
              "Warning: a Nx1 matrix is much less memory efficient than a 1xN "
              "matrix.\n");
        }

        // Each thread is responsible for deleting its own
        threadParams[iThread] = new SaveThreadParams(
            curMatlabArray, filenames[0][index], compressionLevel,
            scratch1s[iThread], scratch2s[iThread]);

        threadHandles[iThread] =
            CreateSimpleThread(SaveThread, threadParams[iThread]);
      }
    ...
    AnkiConditionalErrorAndReturn(!mxIsCell(prhs[1]), "mexSaveEmbeddedArray",
                                  "If one input is a cell, both must be");

    MemoryStack memory1(mxMalloc(bufferSize), bufferSize);
    MemoryStack memory2(mxMalloc(bufferSize), bufferSize);
    AnkiConditionalErrorAndReturn(memory1.IsValid() && memory2.IsValid(),
