...
mxArray* Load(const char* filename, void* allocatedBuffer,
              const s32 allocatedBufferLength) {
  AnkiConditionalErrorAndReturnValue(filename && allocatedBuffer, NULL, "Load",
                                     "Invalid inputs");
  ...
  Array<u8> arrayTmp = LoadBinaryArray_UnknownType(
      filename, NULL, NULL, allocatedBuffer, allocatedBufferLength,
  ...
  if (basicType_isString) {
    Array<const char*> array =
        *reinterpret_cast<Array<const char*>*>(&arrayTmp);
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
  ...
  AnkiConditionalErrorAndReturn(
      nrhs == 1 && nlhs == 1, "mexLoadEmbeddedArray",
      "Call this function as follows: array = mexLoadEmbeddedArray(filename);");
  ...
  AnkiConditionalErrorAndReturn(allocatedBuffer, "mexLoadEmbeddedArray",
