...
// TODO: For the love of all that is holy, use our normal error/logging macros!
// (VIC-4941)
...
#define AnkiError(eventName, eventValue_format, ...)                      \
  _Anki_Log(::Anki::Embedded::ANKI_LOG_LEVEL_ERROR, eventName,            \
            (eventValue_format), __FILE__, __PRETTY_FUNCTION__, __LINE__, \
            ##__VA_ARGS__);
...
#define AnkiWarn(eventName, eventValue_format, ...)                         \
  {                                                                         \
    _Anki_Log(::Anki::Embedded::ANKI_LOG_LEVEL_WARN, eventName,             \
              (eventValue_format), __FILE__, __PRETTY_FUNCTION__, __LINE__, \
              ##__VA_ARGS__);                                               \
  }
...
#define AnkiAssert(expression)                                               \
  if (!(expression)) {                                                       \
    _Anki_Log(::Anki::Embedded::ANKI_LOG_LEVEL_ASSERT, "Assert failure", "", \
...
void _Anki_Log(int logLevel, const char* eventName, const char* eventValue,
