...
#define ConditionalErrorAndReturn(expression, eventName, eventValue) \
  if (!(expression)) {                                               \
...
// transformedPoints = homography_groundTruth * originalPoints;
// transformedPoints = transformedPoints(1:2,:) ./
// repmat(transformedPoints(3,:), [2,1]); originalPoints =
// originalPoints(1:2,:)'; transformedPoints = transformedPoints'; homography =
// mexEstimateHomography(originalPoints, transformedPoints);

/*
void Print_FixedLengthList_Point<f64>(FixedLengthList_Point<f64> &arr, const
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ConditionalErrorAndReturn(
      nrhs == 2 && nlhs == 1, "mexEstimateHomography",
      "Call this function as following: homography = "
      "mexEstimateHomography(originalPoints, transformedPoints); "
      "originalPoints and transformedPoints should be double-precision and "
      "Nx2");
  ...
  ConditionalErrorAndReturn(
      originalPointsArray.get_size(1) == 2 &&
          transformedPointsArray.get_size(1) == 2 &&
          originalPointsArray.get_size(0) == transformedPointsArray.get_size(0),
  ...
  FixedLengthList<Point<f64> > originalPointsList =
      AllocateFixedLengthListFromHeap<Point<f64> >(
          originalPointsArray.get_size(0));
  FixedLengthList<Point<f64> > transformedPointsList =
      AllocateFixedLengthListFromHeap<Point<f64> >(
          originalPointsArray.get_size(0));
  ...
  // Print_FixedLengthList_Point<f64>(transformedPointsList,
  // "transformedPointsList");

  const Result result = EstimateHomography(
      originalPointsList, transformedPointsList, homography, scratch);

  ConditionalErrorAndReturn(result == RESULT_OK, "mexEstimateHomography",
