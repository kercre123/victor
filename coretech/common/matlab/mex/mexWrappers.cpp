...
template <>
mxArray *image2mxArray<double>(const double *img, const int nrows,
  ...
  for (mwSize i = 0, i_out = 0; i < static_cast<mwSize>(npixels * nbands);
       i += nbands, i_out++) {
...
template <>
unsigned int mxArray2simpleArray<double>(const mxArray *array,
                                         double *&outputArray) {
...
mxArray *cvMat2mxArray(const cv::Mat &mat) {
  ...
  switch (depth) {
    ...
    case CV_16U: {
      ...
      mapCvMat2mxArray<unsigned short>(mat, static_cast<int>(nbands),
                                       outputArray);
      break;
    }
    case CV_16S: {
    ...
    default: {
      // std::cout << "NOT supported depth!!" << depth << ", type: " << type <<
...
void safeCallMATLAB(int nlhs, mxArray *plhs[], int nrhs, mxArray *prhs[],
  ...
  if (mxErr != NULL) {
    ...
    mxArray *plhs[1];
    char buf[1024];
    if (mexCallMATLABWithTrap(1, plhs, 1, &mxErr, "getMExceptionMessage") !=
        NULL) {
      snprintf(buf, 1024, "<<Could not get message from MException object!>>");
    } else if (mxGetString(plhs[0], buf, 1024) == 1) {
      snprintf(buf, 1024,
               "<<Could not create string from exception message!>>");
    }
