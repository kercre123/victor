...
#define DEBUG_MSG(...)
#else
#define DEBUG_MSG(__verbosity__, ...) \
...
template <class T>
mxArray *image2mxArray(const T *img, const int nrows, const int ncols,
                       const int nbands);
...
void assignInMatlab(mxArray *var, const char *name,
...
double getScalarDoubleFromMatlab(const char *name, const char *workspace = NULL,
...
mxArray *image2mxArray(const T *img, const int nrows, const int ncols,
  ...
  const mwSize outputDims[3] = {static_cast<mwSize>(ncols),
                                static_cast<mwSize>(nrows),
                                static_cast<mwSize>(nbands)};

  mxArray *outputArray =
      mxCreateNumericArray(3, outputDims, mxSINGLE_CLASS, mxREAL);
...
mxArray *cvMatVec2mxArray(const cv::Mat_<cv::Vec<T, BANDS> > &Img) {
  ...
  mxArray *outputArray =
      mxCreateNumericArray(3, outputDims, mxDOUBLE_CLASS, mxREAL);
...
unsigned int mxArray2simpleArray(const mxArray *array, T *&outputArray) {
  ...
  const unsigned int N =
      static_cast<unsigned int>(mxGetNumberOfElements(array));
...
void mapCvMat2mxArray(const cv::Mat &mat, const int nbands, mxArray *array) {
  ...
  if (numMatlabElements != numOpenCvElements) {
    Anki::CoreTechPrint(
        "Matlab array has a different number of elements than the OpenCV "
        "Mat(%d != %d)\n",
  ...
  for (int i = 0; i < nrows; ++i, ++array_i) {
    ...
    for (int j = 0; j < ncols; ++j, array_j += nrows) {
      ...
      for (int band = 0; band < nbands;
           ++band, ++mat_i, array_band += npixels) {
        *array_band = *mat_i;
      }
    }
  }
...
template <class T>
std::vector<cv::Point_<T> > mxArray2CvPointVector(const mxArray *array) {
  const T *array_i = (const T *)mxGetData(array);
  ...
  if (numMatlabElements % 2 != 0) {
    Anki::CoreTechPrint(
        "Matlab array number of elements must be divisible by two\n");
    return points;
  }
