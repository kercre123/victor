...
GTEST_TEST(TestLinearClassifier, ClassifyChecks) {
  ...
  EXPECT_TRUE(
      Util::IsNear(classifier.ClassifyOdds(testVec), 148.4131591025766f, eps));
  EXPECT_TRUE(Util::IsNear(classifier.ClassifyProbability(testVec),
                           0.9933071490757152f, eps));
  ...
  testVec[1] = 0.0f;
  testVec[2] = 0.0f;
  EXPECT_EQ(classifier.ClassifyOdds(testVec),
            std::numeric_limits<float>::infinity());
  EXPECT_EQ(classifier.ClassifyLogOdds(testVec),
            std::numeric_limits<float>::infinity());
