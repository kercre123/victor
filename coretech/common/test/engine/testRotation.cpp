...
#define ASSERT_NEAR_EQ(a, b) \
  ASSERT_NEAR(a, b, FLOATING_POINT_COMPARISON_TOLERANCE)
...
GTEST_TEST(TestRotation, PointRotation3d) {
  ...
  p = {44.f, 44.f, 44.f};
  Rotation3d R(1.076822398155559f,
               {0.573788055924467f, 0.808395453061550f, 0.131392763681385f});
  q = R * p;
...
GTEST_TEST(TestRotation, ExtractAnglesFromMatrix) {
  std::vector<f32> testAngles = {0.f,   30.f,  45.f,  60.f,  90.f,  121.f,
                                 147.f, 180.f, 182.f, 233.f, 270.f, 291.f,
                                 333.f, 352.f, 360.f, 378.f};
...
GTEST_TEST(TestRotation, ToFromEulerAngles) {
  ...
  const std::vector<Radians> angles = {1.83f,   .345f, -.521f, 0,      M_PI_2,
                                       -M_PI_2, M_PI,  -M_PI,  M_PI_4, -M_PI_4};
  ...
  for (auto angleX : angles) {
    for (auto angleY : angles) {
      for (auto angleZ : angles) {
        ...
        Radians checkX2 = 0.f, checkY2 = 0.f,
                checkZ2 = 0.f;  // for alt. solution
        bool inGimbalLock = R.GetEulerAngles(checkX, checkY, checkZ, &checkX2,
                                             &checkY2, &checkZ2);
...
GTEST_TEST(TestRotation, Rotation3d) {
  ...
  // Testing rotation multiplication (as well as conversion from quaternion to
  // Rotation Matrix)
  Rotation3d R3(R1 * R2);
...
GTEST_TEST(TestPose, IsSameAs) {
  ...
  // P6 is not axis aligned. distThreshold will be considered in its rotated
  // frame
  ...
  EXPECT_FALSE(
      P6.IsSameAs(P7, distThreshold,
                  DEG_TO_RAD(30)));  // Should fail: angle thresh too tight
  EXPECT_TRUE(
      P6.IsSameAs(P7, distThreshold,
                  DEG_TO_RAD(90)));  // Should pass with looser angle threshold

  P7.SetRotation(P6.GetRotation());  // Now rotation matches perfectly
  P7.SetTranslation(Point3f{
      105.f, 95.f,
      0.f});  // But translation is just outside the (rotated) box around P6
  EXPECT_FALSE(P6.IsSameAs(P7, distThreshold, DEG_TO_RAD(90)));

  P7.SetTranslation(Point3f{
      120.f, 120.f,
      1.f});  // Rotation still matches, translation along lenient dimension
  EXPECT_TRUE(P6.IsSameAs(
      P7, distThreshold,
      DEG_TO_RAD(5)));  // Should match even with tight angle threshold
...
GTEST_TEST(TestPose, IsSameWithAmbiguity) {
  ...
  Pose3d Tform(M_PI / 3.f, {0.5f * sqrtf(2.f), 0.5f * sqrtf(2.f), 0.f},
               {0, 0, 0});
  ...
  RotationAmbiguities ambiguities(
  ...
  Pose3d P1_mod(P1);
  P1_mod *=
      P_ref.GetInverse();  // Use in-place multiplication to preserve origin
  Pose3d P2_mod(P2);
  P2_mod *= P_ref.GetInverse();  //  "

  EXPECT_TRUE(P1_mod.IsSameAs_WithAmbiguity(P2_mod, ambiguities, 5.f,
                                            5 * M_PI / 180.f));

  // The symmetric test should also pass
  EXPECT_TRUE(P2_mod.IsSameAs_WithAmbiguity(P1_mod, ambiguities, 5.f,
                                            5 * M_PI / 180.f));
}
