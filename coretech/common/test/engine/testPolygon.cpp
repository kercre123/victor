...
GTEST_TEST(TestPolygon, MinAndMax) {
  Poly3f poly3d{{1.0f, 1.1f, 7.5f}, {0.0f, 1.2f, -7.5f}, {-1.0f, -0.8f, 30.3f}};
...
GTEST_TEST(TestPolygon, average) {
  ...
  Poly3f poly3d{{1.0f, 1.0f, 5.0f}, {0.0f, 2.0f, -2.0f}, {-1.0f, 0.0f, 3.0f}};
...
GTEST_TEST(TestPolygon, FastPolygonContains) {
  ...
  Poly2f poly{{-184.256309, 108.494849}, {-201.111228, 150.067496},
              {-201.111228, 204.267496}, {-160.19712, 220.855432},
              {-82.19712, 220.855432},   {-65.342201, 179.282785},
              {-65.342201, 125.082785},  {-106.256309, 108.494849}};

  std::vector<Point2f> pointsInside{
      {-200.0, 202.0}, {-180.0, 210.0}, {-132.0, 220.0}, {-81.0, 215.0},
      {-68.0, 180.0},  {-67.0, 126.0},  {-180.0, 109},   {-188.0, 121.0},
      {-198.0, 153.0}, {-146.0, 174.0}, {-130.0, 164.0}};

  std::vector<Point2f> pointsOutside{
      {0.0, 0.0},     {-0.0, -0.0},    {-187.0, 214.0}, {-61.0, 215.0},
      {-65.0, 186.0}, {-135.0, 107.0}, {-200.0, 105.0}};
  ...
  printf("did %d dot products for %d checks\n", FastPolygon::_numDotProducts,
         FastPolygon::_numChecks);
}
...
GTEST_TEST(TestPolygon, FastPolygonSortedEdges) {
  ...
  ASSERT_GT(numInside, testPoints.size() / 8)
      << "not enough points inside polygon";
  ASSERT_LT(numInside, testPoints.size() * (7.0 / 8.0))
      << "too many points inside polygon";

  ASSERT_GT(FastPolygon::_numDotProducts, 0)
      << "not tracking dot products! must compile in debug!";
  ...
  for (size_t i = 0; i < testPoints.size(); ++i) {
    ASSERT_EQ(pointInside[i], fastPoly.Contains(testPoints[i]))
        << "point[" << i << "]: " << testPoints[i]
        << " inconsistent! Before sort, value was " << pointInside[i];
  }

  unsigned int numDotsSorted = FastPolygon::_numDotProducts;
  ASSERT_GT(numDotsSorted, 0) << "not correctly tracking dot products!";

  EXPECT_LT(numDotsSorted, numDotsUnsorted)
      << "should take less work once sorted";
...
GTEST_TEST(TestPoly, CladConversion) {
  ...
  {
    const Poly2f poly2d{
        {1.2f, 2.3f}, {3.4f, 4.5f}, {5.6f, 6.7f}, {7.8f, 8.9f}, {9.0f, 0.1f},
    };
  ...
  {
    const Poly3f poly3d{
        {1.2f, 2.3f, -3.2f}, {3.4f, 4.5f, -5.4f}, {5.6f, 6.7f, -7.6f},
        {7.8f, 8.9f, -9.8f}, {9.0f, 0.1f, -1.0f}, {-0.1f, -1.2f, -2.3f},
    };
