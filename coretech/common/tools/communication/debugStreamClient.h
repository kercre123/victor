...
Simple class to handle tcp connections to process messages made of
SerializedBuffer objects
...
class DebugStreamClient {
 public:
  class Object {
   public:
    ...
    char
        objectName[Anki::Embedded::SerializedBuffer::DESCRIPTION_STRING_LENGTH];

    void *buffer;  //< This buffer is allocated by DebugStreamClient, and the
                   //user must free it
    s32 bufferLength;  //< The number of bytes allocated for buffer

    void *startOfPayload;  //< This points after any header in buffer. Just cast
                           //this pointer to the appropriate type.

    f64 timeReceived;  //< All objects from the same message will have the same
                       //timeReceived
  ...
  DebugStreamClient(const s32 comPort, const s32 baudRate,
                    const char parity = 'N', const s32 dataBits = 8,
                    const s32 stopBits = 1);

  // Warning: calling the destructor from a different thread than the
  // constructor is a race condition
  ~DebugStreamClient();

  // Close the socket and stop the parsing threads
  // Warning: calling Close from a different thread than the constructor is a
  // race condition
  Result Close();

  // Blocks until an Object is available, the returns it. To use an object, just
  // cast its buffer based on its typeName and objectName.
  //
  // If the object bufferLength is zero, it's not a big issue, just ignore the
  // object If the object bufferLength is less than zero, something failed
 ...
 protected:
  typedef struct {
    ...
    s32 curDataLength;
    s32 maxDataLength;
    f64 timeReceived;  // The time (in seconds) when the receiving of this
                       // buffer was complete
  } RawBuffer;
  ...
  char saveFilenamePattern
      [DebugStreamClient::ObjectToSave::SAVE_FILENAME_PATTERN_LENGTH];
  ...
  volatile bool
      isRunning;  //< If true, keep working. If false, close everything down
  ...
  static DebugStreamClient::RawBuffer AllocateNewRawBuffer(
      const s32 bufferRawSize);

  // Process the buffer, and adds any parsed objects to parsedObjectQueue. Frees
  // the buffer on completion.
  static void ProcessRawBuffer(
      DebugStreamClient::RawBuffer &buffer,
      ThreadSafeQueue<DebugStreamClient::Object> &parsedObjectQueue,
      const bool requireMatchingSegmentLengths);

  // Grab data from the socket and put in a big temporary buffer, as fast as
  // possible
