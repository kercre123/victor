...
Serial::Serial() : isOpen(false) {}  // Serial::Serial()

Result Serial::Open(s32 comPort, s32 baudRate, char parity, s32 dataBits,
                    s32 stopBits) {
#ifdef _MSC_VER
  AnkiConditionalErrorAndReturnValue(isOpen == false, RESULT_FAIL,
                                     "Serial::Open", "Port is already open");
  ...
  comPortHandle = CreateFile(lpPortName, GENERIC_READ | GENERIC_WRITE, 0, NULL,
                             OPEN_EXISTING, FILE_FLAG_OVERLAPPED, 0);

  // std::cout << GetLastError() << "\n";

  AnkiConditionalErrorAndReturnValue(
      comPortHandle != NULL && comPortHandle != INVALID_HANDLE_VALUE,
  ...
  snprintf(dcbInitString, STRING_LENGTH, "baud=%d parity=%c data=%d stop=%d",
           baudRate, parity, dataBits, stopBits);

  const bool areCommTimeoutsSet = SetCommTimeouts(comPortHandle, &commTimeOuts);
  AnkiConditionalErrorAndReturnValue(areCommTimeoutsSet, RESULT_FAIL,
                                     "Serial::Open",
                                     "Could not set comm timeouts");

  const bool isCommMaskSet = SetCommMask(comPortHandle, dwEvtMask);
  AnkiConditionalErrorAndReturnValue(isCommMaskSet, RESULT_FAIL, "Serial::Open",
                                     "Could not set comm mask");

  const bool gotCommState = GetCommState(comPortHandle, &dcb);
  AnkiConditionalErrorAndReturnValue(gotCommState, RESULT_FAIL, "Serial::Open",
                                     "Could not get comm state");

  dcb.fRtsControl = RTS_CONTROL_ENABLE;

  const bool builtCommDcb = BuildCommDCB(dcbInitString, &dcb);
  AnkiConditionalErrorAndReturnValue(builtCommDcb, RESULT_FAIL, "Serial::Open",
                                     "Could not build comm dcb");

  const bool setCommState = SetCommState(comPortHandle, &dcb);
  AnkiConditionalErrorAndReturnValue(setCommState, RESULT_FAIL, "Serial::Open",
                                     "Could not set comm state");

  const bool isPurged =
      PurgeComm(comPortHandle,
                PURGE_RXABORT | PURGE_TXABORT | PURGE_RXCLEAR | PURGE_TXCLEAR);
  AnkiConditionalErrorAndReturnValue(isPurged, RESULT_FAIL, "Serial::Open",
                                     "Could not purge comm");
...
Result Serial::Close() {
#ifdef _MSC_VER
  if (readEventHandle.hEvent != NULL) CloseHandle(readEventHandle.hEvent);

  if (writeEventHandle.hEvent != NULL) CloseHandle(writeEventHandle.hEvent);
...
Result Serial::Read(void *buffer, s32 bufferLength, s32 &bytesRead) {
  ...
  DWORD bytesReadWord;
  const bool fileRead = ReadFile(comPortHandle, buffer, bytesRead,
                                 &bytesReadWord, &readEventHandle);

  bytesRead = bytesReadWord;

  AnkiConditionalErrorAndReturnValue(fileRead, RESULT_FAIL, "Serial::Open",
                                     "Could not read from port");
...
Socket::Socket() : isOpen(false), socketHandle(0) {}  // Socket::Socket()

Result Socket::Open(const char *ipAddress, const s32 port) {
#ifdef _MSC_VER
  ...
  if (error) return RESULT_FAIL_IO;
  ...
  setsockopt(socketHandle, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv,
             sizeof(struct timeval));

  // connect

  const s32 connectResult =
      connect(socketHandle, (SOCKADDR *)&target, sizeof(target));
  if (connectResult < 0) {
  ...
  // connect
  const s32 connectResult =
      connect(socketHandle, (struct sockaddr *)&target, sizeof(target));
  if (connectResult < 0) {
    socketHandle = 0;
    return RESULT_FAIL_IO;  // Couldn't connect
  }
...
#else
  if (socketHandle) shutdown(socketHandle, 2);
#endif
...
Result Socket::Read(void *buffer, s32 bufferLength, s32 &bytesRead) {
  bytesRead = static_cast<s32>(
      recv(socketHandle, reinterpret_cast<char *>(buffer), bufferLength, 0));
  ...
  if (bytesRead < 0) {
    ...
    /*s32 errorCode;
    getsockopt(socketHandle, SOL_SOCKET, SO_ERROR, &errorCode,
    sizeof(errorCode))*/
