...
A simple mutex-guarded counter that can be incremented, decremented, or set.
Works in Windows or Posix.
...
class ThreadSafeCounter {
 public:
  ...
  // If this->value + delta is greater than maxValue, Increment will set
  // this->value to this->maxValue Returns the change between this->value,
  // before and after the call. If this->value was not capped at this->maxValue,
  // the return value is equal to delta.
...
template <typename Type>
ThreadSafeCounter<Type>::ThreadSafeCounter(const Type initialValue,
                                           const Type maxValue)
...
template <typename Type>
Type ThreadSafeCounter<Type>::Increment(const Type delta) {
...
template <typename Type>
Type ThreadSafeCounter<Type>::Set(const Type value) {
  LockSimpleMutex(mutex);
...
template <typename Type>
Type ThreadSafeCounter<Type>::Get() const {
  LockSimpleMutex(mutex);
