...
inline void usleep(const unsigned int microseconds) {
  Sleep(MAX(1, microseconds / 1000));
}
...
namespace Anki {
// WARNING: Linux and OSX will deadlock if the same thread locks the same mutex
// multiple times. Windows won't.
...
#ifdef _MSC_VER
inline ThreadHandle CreateSimpleThread(
    _In_ LPTHREAD_START_ROUTINE lpStartAddress, void *parameters) {
  DWORD threadId = -1;
  ThreadHandle handle =
  ...
  return handle;
}  // ThreadHandle CreateSimpleThread(_In_ LPTHREAD_START_ROUTINE
   // lpStartAddress, void * parameters)
#else
inline ThreadHandle CreateSimpleThread(void *(*start_routine)(void *),
                                       void *parameters) {
  ...
  return handle;
}  // ThreadHandle CreateSimpleThread( void *(*start_routine) (void *), void *
   // parameters)
#endif
