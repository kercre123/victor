...
#if (defined(ANKICORETECH_EMBEDDED_USE_MATLAB) && \
     ANKICORETECH_EMBEDDED_USE_MATLAB) ||         \
...
}  // std::string convertToMatlabTypeString(const char *typeName, size_t
   // byteDepth)
...
SharedMatlabInterface::SharedMatlabInterface(bool clearWorkspace) {
  ...
  if (!ep) {
    LOG_ERROR("SharedMatlabInterface Constructor",
...
std::string SharedMatlabInterface::EvalString(const char *const format, ...) {
  if (!ep) {
    // AnkiConditionalErrorAndReturnValue(this->ep, "", "Anki.", "Matlab engine
    // is not started/connected");
...
std::string SharedMatlabInterface::EvalStringEcho(const char *const format,
  ...
  EvalString(
      "if length(lastAnkiCommandBuffer)==%d "
      "lastAnkiCommandBuffer=lastAnkiCommandBuffer(2:end); end; "
      "lastAnkiCommandBuffer{end+1}=lastAnkiCommand;",
      SharedMatlabInterface::COMMAND_BUFFER_SIZE);
...
mxArray *SharedMatlabInterface::GetArray(const std::string name) {
  if (!ep) {
    // AnkiConditionalErrorAndReturnValue(this->ep, NULL, "Anki.", "Matlab
    // engine is not started/connected");
    LOG_ERROR("Anki.", "Matlab engine is not started/connected");
    return NULL;
  }
...
MatlabVariableType SharedMatlabInterface::GetType(const std::string name) {
  if (!ep) {
    // AnkiConditionalErrorAndReturnValue(this->ep, TYPE_UNKNOWN, "Anki.",
  ...
  EvalStringEcho(
      "%s=int32([isa(%s, 'int8'), isa(%s, 'u8'), isa(%s, 'int16'), isa(%s, "
      "'u16'), isa(%s, 'int32'), isa(%s, 'u32'), isa(%s, 'int64'), isa(%s, "
      "'u64'), isa(%s, 'single'), isa(%s, 'double')]);",
      typeName, name.data(), name.data(), name.data(), name.data(), name.data(),
      name.data(), name.data(), name.data(), name.data(), name.data());
...
Result SharedMatlabInterface::GetCvMat(const CvMat *matrix, std::string name) {
  if (!ep) {
    // AnkiConditionalErrorAndReturnValue(this->ep, RESULT_FAIL, "Anki.",
    // "Matlab engine is not started/connected");
    LOG_ERROR("Anki.", "Matlab engine is not started/connected");
    return RESULT_FAIL;
  }
  ...
  if (matrix == NULL) {
    // AnkiConditionalErrorAndReturnValue(matrix != NULL, RESULT_FAIL, "Error:
    // CvMat is not initialized for %s\n", name.data());
...
Result SharedMatlabInterface::PutCvMat(const CvMat *matrix, std::string name) {
  AnkiConditionalErrorAndReturnValue(this->ep, RESULT_FAIL,
                                     "Anki.SharedMatlabInterface.PutCvMat",
                                     "Matlab engine is not started/connected");

  AnkiConditionalErrorAndReturnValue(
      matrix != NULL, RESULT_FAIL, "Anki.SharedMatlabInterface.PutCvMat",
  ...
  if ((matrix->type & CV_MAT_DEPTH_MASK) == CV_64F) {
    Put<f64>(matrix->data.db, matrix->rows * matrix->step / sizeof(f64),
             tmpName);
  } else if ((matrix->type & CV_MAT_DEPTH_MASK) == CV_32F) {
    Put<f32>(matrix->data.fl, matrix->rows * matrix->step / sizeof(f32),
             tmpName);
  } else if ((matrix->type & CV_MAT_DEPTH_MASK) == CV_16S) {
    Put<s16>(matrix->data.s, matrix->rows * matrix->step / sizeof(s16),
             tmpName);
  } else if ((matrix->type & CV_MAT_DEPTH_MASK) == CV_16U) {
    Put<u16>((const unsigned short *)matrix->data.s,
             matrix->rows * matrix->step / sizeof(u16), tmpName);
  } else if ((matrix->type & CV_MAT_DEPTH_MASK) == CV_32S) {
    Put<s32>(matrix->data.i, matrix->rows * matrix->step / sizeof(s32),
             tmpName);
  ...
  if (numChannels == 1) {
    EvalString("%s=reshape(%s, [%d, %d])';", name.data(), tmpName, matrix->cols,
               matrix->rows);
  } else {
    EvalString("%s=permute(reshape(%s, [%d, %d, %d]), [3,2,1]);", name.data(),
               tmpName, numChannels, matrix->cols, matrix->rows);
...
Result SharedMatlabInterface::PutString(const char *characters, s32 nValues,
...
s32 SharedMatlabInterface::SetVisible(bool isVisible) {
  if (!ep) {
    // AnkiConditionalErrorAndReturnValue(this->ep, -1, "Anki.", "Matlab engine
    // is not started/connected");
    LOG_ERROR("Anki.", "Matlab engine is not started/connected");
    return -1;
  }
...
#endif  // #if (defined(ANKICORETECH_EMBEDDED_USE_MATLAB) &&
        // ANKICORETECH_EMBEDDED_USE_MATLAB) ||
        // (defined(ANKICORETECH_USE_MATLAB) && ANKICORETECH_USE_MATLAB)
