...
// Convert a raw image data pointer with size to a mxAarray.  Allocate and
// return the mxArray
template <typename Type>
mxArray *imageArrayToMxArray(const Type *data, const s32 nrows, const s32 ncols,
                             const mwSize nbands);
...
template <typename Type>
mxClassID GetMatlabClassID(void) {
  return mxUNKNOWN_CLASS;
}

// Forward declare the defined specializations for known types:
template <>
mxClassID GetMatlabClassID<f32>(void);
template <>
mxClassID GetMatlabClassID<f64>(void);

template <>
mxClassID GetMatlabClassID<s16>(void);
template <>
mxClassID GetMatlabClassID<u16>(void);

template <>
mxClassID GetMatlabClassID<s8>(void);
template <>
mxClassID GetMatlabClassID<u8>(void);

template <>
mxClassID GetMatlabClassID<s32>(void);
template <>
mxClassID GetMatlabClassID<u32>(void);

template <>
mxClassID GetMatlabClassID<s64>(void);
template <>
mxClassID GetMatlabClassID<u64>(void);
...
template <typename Type>
mxArray *imageArrayToMxArray(const Type *img, const int nrows, const int ncols,
                             const mwSize nbands) {
  ...
  const mwSize outputDims[3] = {static_cast<mwSize>(nrows),
                                static_cast<mwSize>(ncols),
                                static_cast<mwSize>(nbands)};

  mxArray *outputArray =
      mxCreateNumericArray(3, outputDims, GetMatlabClassID<Type>(), mxREAL);
