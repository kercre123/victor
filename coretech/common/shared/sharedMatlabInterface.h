...
A simple wrapper around the Matlab interface between C and matlab. These
routines can be used to send and receive data from the Matlab process, and to
execute commands in the Matlab window.
...
// (It's kinda gross for common-shared to reach into basestation- or
// robot-specific code.)
...
#define LOG_ERROR(name, format, ...) \
  PRINT_NAMED_ERROR(name, format, ##__VA_ARGS__)
#define LOG_WARNING(name, format, ...) \
  PRINT_NAMED_WARNING(name, format, ##__VA_ARGS__)
...
class SharedMatlabInterface {
 public:
  ...
  // Same as EvalString, but in addition, Matlab will keep a std::list of the
  // last few EvalStringEcho calls, stored in "lastC2Mat2CommandBuffer"
  ...
  // Character strings need to be converted slightly to appear correctly in
  // Matlab
  Result PutString(const char *characters, s32 nValues, const std::string name);

  template <typename Type>
  Result PutOpencvMat(const cv::Mat_<Type> &matrix, const std::string name);

  template <typename Type>
  Result Put(const Type *values, s32 nValues, const std::string name);
...
template <typename Type>
Result SharedMatlabInterface::PutOpencvMat(const cv::Mat_<Type> &matrix,
  ...
  if (!ep) {
    ...
    // AnkiConditionalErrorAndReturnValue(ep, RESULT_FAIL,
    // "Anki.PutOpencvMat<Type>", "Matlab engine is not started/connected");
    LOG_ERROR("Anki.PutOpencvMat<Type>",
  ...
  if (whichClass == mxUNKNOWN_CLASS) {
    // TODO: How to print a message the right way for basestation vs. embedded??
    // AnkiConditionalErrorAndReturnValue(whichClass != mxUNKNOWN_CLASS,
    // RESULT_FAIL, "Anki.PutOpencvMat<Type>", "Unknown type to convert to a
    // mxClassID");
    return RESULT_FAIL;
  }
...
template <typename Type>
Result SharedMatlabInterface::Put(const Type *values, s32 nValues,
                                  const std::string name) {
  if (!ep) {
    // TODO: How to print a message the right way for basestation vs. embedded??
  ...
  return RESULT_OK;
}  // template<typename Type> Result Matlab::Put(const Type * values, s32
   // nValues, const std::string name)

template <typename Type>
Type *SharedMatlabInterface::Get(const std::string name) {
  if (!ep) {
    // TODO: How to print a message the right way for basestation vs. embedded??
    // AnkiConditionalErrorAndReturnValue(this->ep, NULL, "Anki.Get", "Matlab
