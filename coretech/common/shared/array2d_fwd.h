 ...
 *
 *
 * Description: Defines a generic, templated Array2d storage class.
 *
 ...
 *   2. This header "forward declares"
...
static_assert(ANKICORETECH_USE_OPENCV,
              "Array2d implementation requires OpenCV");
...
class Array2d : private cv::Mat_<T>  // Note the private inheritance!
{
 public:
  ...
  //    Array2d(const Embedded::Array2d<T> &other); // *copies* data from
  //    unmanaged array
  ...
  // Otherwise just changes the size of the array and reuses memory. (Like
  // cv::Mat.)
  void Allocate(s32 nrows, s32 ncols);

  // "Empties" an image to make it size zero, but does not actually release
  // memory.
  ...
  void ApplyScalarFunction(std::function<Tresult(const T&)> fcn,
