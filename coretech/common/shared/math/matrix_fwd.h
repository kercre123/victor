 ...
 *        To get the implementations, you must include matrix.h.
 *
...
static_assert(ANKICORETECH_USE_OPENCV,
              "Matrix class implementations require OpenCV");
    ...
    : private cv::Matx<T, NROWS, NCOLS>  // private inheritance from cv::Matx
{
 ...
 public:
  ...
  SmallMatrix<NROWS, KCOLS, T_work> operator*(
      const SmallMatrix<NCOLS, KCOLS, T_other>& other) const;

  SmallMatrix<NROWS, NCOLS, T> operator*(T value) const;
  SmallMatrix<NROWS, NCOLS, T>& operator*=(T value);
  SmallMatrix<NROWS, NCOLS, T> operator+(T value) const;
  SmallMatrix<NROWS, NCOLS, T>& operator+=(T value);
  SmallMatrix<NROWS, NCOLS, T> operator+(
      const SmallMatrix<NROWS, NCOLS, T>& other) const;
  SmallMatrix<NROWS, NCOLS, T>& operator+=(
      const SmallMatrix<NROWS, NCOLS, T>& other);
  SmallMatrix<NROWS, NCOLS, T> operator-(
      const SmallMatrix<NROWS, NCOLS, T>& other) const;
  SmallMatrix<NROWS, NCOLS, T>& operator-=(
      const SmallMatrix<NROWS, NCOLS, T>& other);
...
template <MatDimType NROWS, MatDimType NCOLS, typename Tmat, typename Tpt,
          typename Twork = Tpt>
...
class SmallSquareMatrix : public SmallMatrix<DIM, DIM, T> {
 public:
  ...
  explicit SmallSquareMatrix(
      std::initializer_list<T> valsList);  // DIMxDIM list of values
  explicit SmallSquareMatrix(
      std::initializer_list<Point<DIM, T> > colsList);  // list of columns
  ...
  // ... this = this * other;
  template <typename T_other>
  SmallSquareMatrix<DIM, T>& operator*=(
      const SmallSquareMatrix<DIM, T_other>& other);
  // ... this = other * this;
  template <typename T_other>
  SmallSquareMatrix<DIM, T>& PreMultiplyBy(
      const SmallSquareMatrix<DIM, T_other>& other);
