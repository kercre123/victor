...
inline constexpr bool AllHelper(const Point<N, T>& p1, const Point<N, T>& p2,
                                Compare&& compareFcn) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - STL specializations for common
...
struct std::hash<Anki::Point2<T>> {
  std::enable_if_t<sizeof(T) <= 4, s64> constexpr operator()(
      const Anki::Point2<T>& p) const {
...
struct std::equal_to<Anki::Point<N, T>> {
  ...
  constexpr bool operator()(const Anki::Point<N, T1>& p,
                            const Anki::Point<N, T1>& q) const {
    return AllHelper(p, q, [](T1 a, T1 b) { return FLT_NEAR(a, b); });
  }

  constexpr bool operator()(const Anki::Point<N, T>& p,
                            const Anki::Point<N, T>& q) const {
...
namespace {
// NOTE: since `std::fill` is not constexpr until C++20, use an index_sequence
// to create
...
constexpr Point<N, T>::Point(Ts... ts) : std::array<T, N>{{T(ts)...}} {}
...
// - - - - - - - - - - - - - - - - - - - -
template <PointDimType N, typename T>
Point<N, T>::Point(const cv::Point_<T>& pt) : Point(pt.x, pt.y) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
template <PointDimType N, typename T>
Point<N, T>::Point(const cv::Point3_<T>& pt) : Point(pt.x, pt.y, pt.z) {}
...
// - - - - - - - - - - - - - - - - - - - -
template <PointDimType N, typename T>
constexpr bool operator==(const Point<N, T>& point1,
                          const Point<N, T>& point2) {
...
constexpr bool IsNearlyEqual(const Point<N, T>& point1,
                             const Point<N, T>& point2, const T eps) {
...
constexpr bool AreUnitVectorsAligned(const Point<N, f32>& unitVec1,
                                     ...
                                     const Radians& angleThreshold) {
  assert(
      NEAR(unitVec1.Length(), 1.f, 10.f * std::numeric_limits<f32>::epsilon()));
  assert(
      NEAR(unitVec2.Length(), 1.f, 10.f * std::numeric_limits<f32>::epsilon()));
