...
class UnitQuaternion_ : public Point<4, T> {
 public:
  ...
  UnitQuaternion_(const T w, const T x, const T y,
                  const T z);  // will normalize the inputs
...
// The default UnitQuaternion is double to minimize effects of floating-point
// error
...
class RotationMatrix3d : public RotationMatrixBase<3> {
 public:
  ...
  RotationMatrix3d(const Radians& angleX, const Radians& angleY,
  ...
  //   pointers for the second set of angles. If any is requested, all should
  //   be.
  bool GetEulerAngles(Radians& angle_x, Radians& angle_y, Radians& angle_z,
                      Radians* angle_x2 = nullptr, Radians* angle_y2 = nullptr,
  ...
  // Get the angular difference between the given parent axis and its rotated
  // version. AXIS can by 'X', 'Y', or 'Z' If outWhichAxis is not null, it will
  // be set to GetRotatedParentAxis<AXIS>() (see below)
  template <char AXIS>
  Radians GetAngularDeviationFromParentAxis(
      AxisName* outWhichAxis = nullptr) const;
  ...
  // Returns the axis that is most aligned with the specified axis of the parent
  // pose.
  template <char AXIS>
...
// useAbsoluteValue=true is a shortcut to reduce the number of rotations stored
// by ignoring sign when comparing.
class RotationAmbiguities {
 public:
  // Default constructor: no ambiguities
  RotationAmbiguities();

  RotationAmbiguities(bool useAbsoluteValue,
                      std::vector<RotationMatrix3d>&& rotations);

  // Returns true if the given rotation is within threshold of any ambiguous
  // rotation stored here
...
inline Radians RotationVector3d::GetAngle(void) const { return _angle; }
...
template <char parentAxis>
inline Radians RotationMatrix3d::GetAngularDeviationFromParentAxis(
    AxisName* outWhichAxis) const {
  f32 maxVal;
