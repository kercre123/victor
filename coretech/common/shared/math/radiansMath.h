...
// meanAngle: Mean of all the angles in angles. (Same as what ComputeMean()
// returns) meanToMinAngleDist_rad: Angular distance to furthest angle (up to
// -PI) from the mean in the CW direction meanToMaxAngleDist_rad: Angular
// distance to furthest angle (up to PI) from the mean in the CCW direction
bool ComputeMinMaxMean(const std::vector<Radians>& angles, Radians& meanAngle,
