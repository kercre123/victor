...
Point2<Twork> operator*(const SmallSquareMatrix<2, Tmat>& M,
  ...
  Point2<Twork> result(
      static_cast<Twork>(M(0, 0)) * px + static_cast<Twork>(M(0, 1)) * py,
...
Point3<Twork> operator*(const SmallSquareMatrix<3, Tmat>& M,
  ...
  Point3<Twork> result(
      static_cast<Twork>(M(0, 0)) * px + static_cast<Twork>(M(0, 1)) * py +
          static_cast<Twork>(M(0, 2)) * pz,
      static_cast<Twork>(M(1, 0)) * px + static_cast<Twork>(M(1, 1)) * py +
          static_cast<Twork>(M(1, 2)) * pz,
      static_cast<Twork>(M(2, 0)) * px + static_cast<Twork>(M(2, 1)) * py +
          static_cast<Twork>(M(2, 2)) * pz);
...
template <typename T>
Matrix<T>::Matrix() : Array2d<T>() {}

template <typename T>
Matrix<T>::Matrix(s32 nrows, s32 ncols) : Array2d<T>(nrows, ncols) {
  DEV_ASSERT(nrows > 0 && ncols > 0,
             "Matrix.Constructor.InitializedRequiresPositiveDimensions");
}
...
Matrix<T>::Matrix(s32 nrows, s32 ncols, T* data)
  ...
  DEV_ASSERT(nrows > 0 && ncols > 0 && data != NULL,
             "Matrix.Constructor.InitializedRequiresPositiveDimensions");
}
...
Matrix<T>::Matrix(const cv::Mat_<T>& cvMatrix)
    : Array2d<T>(cvMatrix) {}  // Constructor: Matrix<T>(cv::Mat_<T>)

template <typename T>
Matrix<T> Matrix<T>::operator*(const Matrix<T>& other) const {
  // Make sure the matrices have compatible sizes for multiplication
  DEV_ASSERT(this->GetNumCols() == other.GetNumRows(),
             "Matrix.Multiply.DimensionMismatch");
...
Matrix<T>& Matrix<T>::operator*=(const Matrix<T>& other) {
  // Make sure the matrices have compatible sizes for multiplication
  DEV_ASSERT(this->GetNumCols() == other.GetNumRows(),
             "Matrix.MultiplyInPlace.DimensionMismatch");
...
Matrix<T>& Matrix<T>::Invert(void) {
  DEV_ASSERT(this->GetNumRows() == this->GetNumCols(),
             "Matrix.Invert.NonSquareMatrix");
...
void Matrix<T>::GetInverse(Matrix<T>& outInverse) const {
  DEV_ASSERT(this->GetNumRows() == this->GetNumCols(),
             "Matrix.GetInverse.NonSquareMatrix");
...
SmallMatrix<NROWS, NCOLS, T>::SmallMatrix() : cv::Matx<T, NROWS, NCOLS>() {}
...
SmallMatrix<NROWS, NCOLS, T>::SmallMatrix(std::initializer_list<T> valsList) {
  DEV_ASSERT(valsList.size() == NROWS * NCOLS,
             "SmallMatrix.Constructor.InitializerListDimensionMismatch");
...
SmallMatrix<NROWS, NCOLS, T>::SmallMatrix(
    std::initializer_list<Point<NROWS, T> > colsList) {
  DEV_ASSERT(colsList.size() == NCOLS,
             "SmallMatrix.Constructor.InitializerListDimensionMismatch");
...
SmallMatrix<NROWS,NCOLS,float>::SmallMatrix(const SmallMatrix<NROWS,NCOLS,float>
&M)
{
  // TODO: Is there a better way to do this than looping and casting each
element? for(size_t i=0; i<NROWS; ++i) { for(size_t j=0; j<NCOLS; ++j) {
...
template <MatDimType NROWS, MatDimType NCOLS, typename T>
SmallMatrix<NROWS, NCOLS, T>::SmallMatrix(
    const cv::Matx<T, NROWS, NCOLS>& cvMatrix)
    : cv::Matx<T, NROWS, NCOLS>(cvMatrix) {}

// Matrix element access:
template <MatDimType NROWS, MatDimType NCOLS, typename T>
T& SmallMatrix<NROWS, NCOLS, T>::operator()(const MatDimType i,
                                            const MatDimType j) {
...
template <MatDimType NROWS, MatDimType NCOLS, typename T>
const T& SmallMatrix<NROWS, NCOLS, T>::operator()(const MatDimType i,
                                                  const MatDimType j) const {
  DEV_ASSERT(i < NROWS && j < NCOLS, "SmallMatrix.ElementAccess.OutOfBounds");
  return cv::Matx<T, NROWS, NCOLS>::operator()((int)i, (int)j);
}
...
template <MatDimType NROWS, MatDimType NCOLS, typename T>
Point<NROWS, T> SmallMatrix<NROWS, NCOLS, T>::GetColumn(
    const MatDimType j) const {
...
template <MatDimType NROWS, MatDimType NCOLS, typename T>
void SmallMatrix<NROWS, NCOLS, T>::SetRow(const MatDimType i,
                                          const Point<NCOLS, T>& row) {
...
template <MatDimType NROWS, MatDimType NCOLS, typename T>
void SmallMatrix<NROWS, NCOLS, T>::SetColumn(const MatDimType j,
                                             const Point<NROWS, T>& column) {
...
SmallMatrix<NROWS,NCOLS,T>& SmallMatrix<NROWS,NCOLS,T>::operator=(const
SmallMatrix& other)
{
  if(this != &other) {
...
SmallMatrix<NROWS, KCOLS, T_work> SmallMatrix<NROWS, NCOLS, T>::operator*(
...
template <MatDimType NROWS, MatDimType NCOLS, typename T>
SmallMatrix<NROWS, NCOLS, T> SmallMatrix<NROWS, NCOLS, T>::operator*(
    T value) const {
...
// Matrix transpose:
template <MatDimType NROWS, MatDimType NCOLS, typename T>
SmallMatrix<NCOLS, NROWS, T> SmallMatrix<NROWS, NCOLS, T>::GetTranspose()
    const {
...
bool IsNearlyEqual(const SmallMatrix<NROWS, NCOLS, T>& M1,
                   const SmallMatrix<NROWS, NCOLS, T>& M2, const T eps) {
  for (unsigned int i = 0; i < NROWS; ++i) {
    for (unsigned int j = 0; j < NCOLS; ++j) {
...
template <MatDimType NROWS, MatDimType NCOLS, typename T>
const cv::Matx<T, NROWS, NCOLS>& SmallMatrix<NROWS, NCOLS, T>::get_CvMatx_()
    const {
  return (*this);
}
...
std::ostream& operator<<(std::ostream& out,
                         const SmallMatrix<NROWS, NCOLS, T>& m) {
...
template <MatDimType NROWS, MatDimType NCOLS, typename T>
void SmallMatrix<NROWS, NCOLS, T>::GetInverse(
    SmallMatrix<NROWS, NCOLS, T>& outInverse) const {
  outInverse = *this;
  outInverse.Invert();
}
...
SmallSquareMatrix<DIM, T>::SmallSquareMatrix(
    std::initializer_list<Point<DIM, T> > colsList)
    : SmallMatrix<DIM, DIM, T>(colsList) {}

template <MatDimType DIM, typename T>
template <typename T_other>
SmallSquareMatrix<DIM, T>& SmallSquareMatrix<DIM, T>::operator*=(
...
template <MatDimType DIM, typename T>
template <typename T_other>
SmallSquareMatrix<DIM, T>& SmallSquareMatrix<DIM, T>::PreMultiplyBy(
    const SmallSquareMatrix<DIM, T_other>& other) {
