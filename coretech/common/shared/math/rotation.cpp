...
RotationMatrixBase<2>::RotationMatrixBase(void)
    : RotationMatrixBase{1.f, 0.f, 0.f, 1.f} {}

template <>
RotationMatrixBase<3>::RotationMatrixBase(void)
    : RotationMatrixBase{1.f, 0.f, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f, 1.f} {}
...
void RotationMatrixBase<DIM>::Renormalize() {
  ...
  for (s32 i = 0; i < DIM; ++i) {
    ...
    if (!NEAR(rowNorm, 1.f,
              RotationMatrixBase<DIM>::OrthogonalityToleranceHigh)) {
      std::string rowString;
      for (s32 j = 0; j < DIM; ++j) {
        rowString += std::to_string(this->operator()(i, j)) + " ";
      }
      PRINT_NAMED_WARNING(
          "RotationMatrixBase.Renormalize.BadRowNorm",
          "Norm of row %d = %f! (Expecting near 1.0) Row = [%s]", i, rowNorm,
          rowString.c_str());
    ...
    if (rowNorm > 1.f ||
        rowNorm < 1 - RotationMatrixBase<DIM>::OrthogonalityToleranceLow) {
      needsRenormalization = true;
      break;
    }
  }
...
template <MatDimType DIM>
RotationMatrixBase<DIM> RotationMatrixBase<DIM>::operator*(
    const RotationMatrixBase<DIM>& R_other) const {
  RotationMatrixBase<DIM> Rnew(
...
template <MatDimType DIM>
RotationMatrixBase<DIM>& RotationMatrixBase<DIM>::operator*=(
    const RotationMatrixBase<DIM>& R_other) {
  SmallSquareMatrix<DIM, float>::operator*=(R_other);
  this->Renormalize();
  return *this;
}
...
template <MatDimType DIM>
RotationMatrixBase<DIM>& RotationMatrixBase<DIM>::PreMultiplyBy(
    const RotationMatrixBase<DIM>& R_other) {
...
template <MatDimType DIM>
RotationMatrixBase<DIM>::RotationMatrixBase(
    const SmallSquareMatrix<DIM, float>& matrix)
...
template <MatDimType DIM>
RotationMatrixBase<DIM>::RotationMatrixBase(
    std::initializer_list<float> initValues)
...
template <MatDimType DIM>
void RotationMatrixBase<DIM>::GetInverse(
    RotationMatrixBase<DIM>& outInverted) const {
  outInverted = *this;
  outInverted.Invert();
}
...
RotationMatrix2d::RotationMatrix2d() : RotationMatrixBase<2>() {}
...
RotationVector3d::RotationVector3d(void) : _angle(0.f), _axis(X_AXIS_3D()) {}

RotationVector3d::RotationVector3d(const Radians& angleIn, const Vec3f& axisIn)
    : _angle(angleIn), _axis(axisIn) {
...
RotationVector3d::RotationVector3d(const Vec3f& rvec) : _axis(rvec) {
...
Rotation3d::Rotation3d(const Radians& angle, const Vec3f& axis) {
  ...
  DEV_ASSERT_MSG(FLT_NEAR(axis.LengthSq(), 1.f),
                 "Rotation3d.Constructor.NonUnitAxisLength", "%s, Length=%f",
                 axis.ToString().c_str(), axis.LengthSq());
...
Rotation3d::Rotation3d(const UnitQuaternion& q) : _q(q) {}

const Radians Rotation3d::GetAngle() const { return 2.f * std::acos(_q[0]); }
...
Radians Rotation3d::GetAngleDiffFrom(const Rotation3d& otherRotation) const {
  ...
  // the innerProd below may have magnitude less than 1 resulting in non-zero
  // angleDiff.
  if (q_this == q_other) {
    return 0;
  }

  const double innerProd = std::min(
      1.0,
      std::max(-1.0, (q_this.w() * q_other.w() + q_this.x() * q_other.x() +
                      q_this.y() * q_other.y() + q_this.z() * q_other.z())));
...
// First row of Rmat
template <>
f32 Rotation3d::GetRmatEntry<0, 0>() const {
  return 1.f - 2.f * (_q.y() * _q.y() + _q.z() * _q.z());
}

template <>
f32 Rotation3d::GetRmatEntry<0, 1>() const {
  return 2.f * (_q.x() * _q.y() - _q.z() * _q.w());
}

template <>
f32 Rotation3d::GetRmatEntry<0, 2>() const {
  return 2.f * (_q.x() * _q.z() + _q.y() * _q.w());
}

// Second row of Rmat
template <>
f32 Rotation3d::GetRmatEntry<1, 0>() const {
  return 2.f * (_q.x() * _q.y() + _q.z() * _q.w());
}

template <>
f32 Rotation3d::GetRmatEntry<1, 1>() const {
  return 1.f - 2.f * (_q.x() * _q.x() + _q.z() * _q.z());
}

template <>
f32 Rotation3d::GetRmatEntry<1, 2>() const {
  return 2.f * (_q.y() * _q.z() - _q.x() * _q.w());
}

// Third row of Rmat
template <>
f32 Rotation3d::GetRmatEntry<2, 0>() const {
  return 2.f * (_q.x() * _q.z() - _q.y() * _q.w());
}

template <>
f32 Rotation3d::GetRmatEntry<2, 1>() const {
  return 2.f * (_q.y() * _q.z() + _q.x() * _q.w());
}

template <>
f32 Rotation3d::GetRmatEntry<2, 2>() const {
...
const Vec3f Rotation3d::GetAxis() const {
  ...
  if (NEAR_ZERO(length)) {
    // Not a valid axis (since rotation angle is zero) so just return any valid
    // one
    return Z_AXIS_3D();
  }
  return axis;
...
bool IsNearlyEqual(const Rotation3d& R1, const Rotation3d& R2,
                   const f32 tolerance) {
  return IsNearlyEqual(R1.GetQuaternion(), R2.GetQuaternion(),
                       (double)tolerance);
}
...
template <typename T>
UnitQuaternion_<T>::UnitQuaternion_() : Point<4, T>(1, 0, 0, 0) {}
...
template <typename T>
UnitQuaternion_<T> UnitQuaternion_<T>::operator*(
    const UnitQuaternion_<T>& other) const {
...
UnitQuaternion_<T>& UnitQuaternion_<T>::SetCast(
    const UnitQuaternion_<T_other>& other) {
  for (int i = 0; i < 4; ++i) {
...
template <typename T>
UnitQuaternion_<T>& UnitQuaternion_<T>::operator*=(
    const UnitQuaternion_<T>& other) {
  const T wNew = (this->w() * other.w()) - (this->x() * other.x()) -
                 (this->y() * other.y()) - (this->z() * other.z());
  const T xNew = (this->w() * other.x()) + (this->x() * other.w()) +
                 (this->y() * other.z()) - (this->z() * other.y());
  const T yNew = (this->w() * other.y()) - (this->x() * other.z()) +
                 (this->y() * other.w()) + (this->z() * other.x());
  const T zNew = (this->w() * other.z()) + (this->x() * other.y()) -
...
Point3<T> UnitQuaternion_<T>::operator*(const Point3<T>& p) const {
  ...
  Point3<T> p_out((w2 + x2 - y2 - z2) * p.x() + (2 * xy - 2 * wz) * p.y() +
                      (2 * xz + 2 * wy) * p.z(),
                  (2 * xy + 2 * wz) * p.x() + (w2 - x2 + y2 - z2) * p.y() +
                      (2 * yz - 2 * wx) * p.z(),
                  (2 * xz - 2 * wy) * p.x() + (2 * yz + 2 * wx) * p.y() +
                      (w2 - x2 - y2 + z2) * p.z());
...
RotationMatrix3d::RotationMatrix3d() : RotationMatrixBase<3>() {}

RotationMatrix3d::RotationMatrix3d(const Matrix_3x3f& matrix3x3)
    : RotationMatrixBase<3>(matrix3x3) {}

RotationMatrix3d::RotationMatrix3d(const Radians& angleX, const Radians& angleY,
                                   const Radians& angleZ) {
...
Radians RotationMatrix3d::GetAngleDiffFrom(
    const RotationMatrix3d& other) const {
  // See also:
...
bool RotationMatrix3d::GetEulerAngles(Radians& angle_x, Radians& angle_y,
                                      Radians& angle_z, Radians* angle_x2,
                                      Radians* angle_y2,
...
template <>
Radians RotationMatrix3d::GetAngleAroundAxis<'X'>() const {
  return GetAngleAroundXaxis();
}

template <>
Radians RotationMatrix3d::GetAngleAroundAxis<'Y'>() const {
  return GetAngleAroundYaxis();
}

template <>
Radians RotationMatrix3d::GetAngleAroundAxis<'Z'>() const {
  return GetAngleAroundZaxis();
}
...
void Rodrigues(const RotationVector3d& Rvec_in, RotationMatrix3d& Rmat_out) {
#if ANKICORETECH_USE_OPENCV
...
void Rodrigues(const RotationMatrix3d& Rmat_in, RotationVector3d& Rvec_out) {
#if ANKICORETECH_USE_OPENCV
  cv::Vec3f cvRvec;
...
RotationAmbiguities::RotationAmbiguities() : _useAbsoluteValue(false) {}

RotationAmbiguities::RotationAmbiguities(
    bool useAbsoluteValue, std::vector<RotationMatrix3d>&& rotations)
    : _rotations(std::move(rotations)), _useAbsoluteValue(useAbsoluteValue) {}

bool RotationAmbiguities::IsRotationSame(const Rotation3d& R,
                                         const Radians& angleThreshold) const {
  if (!_rotations.empty()) {
    // TODO: Do this directly with quaternions instead of converting to
    // RotationMatrix
