 ...
 * Created: 5/10/2014
 *
 * Description: Implements a templated class for storing 2D rectangles according
 *to their upper left corner and width + height.
 *
...
template <typename T>
Rectangle<T>::Rectangle() : cv::Rect_<T>() {}
...
Rectangle<T>::Rectangle(T x, T y, T width, T height)
  ...
  DEV_ASSERT(height >= T(0),
             "Rectangle.XYWidthHeightConstructor.NegativeHeight");
}
...
template <typename T>
Rectangle<T>::Rectangle(const cv::Rect_<T>& cvRect) : cv::Rect_<T>(cvRect) {}
...
Rectangle<T>::Rectangle(
    const std::array<Point<2, T_other>, NumPoints>& points) {
  InitFromPointContainer(points);
}
...
Rectangle<T> Rectangle<T>::Scale(const f32 scaleFactor) const {
  return Rectangle<T>(
      x + static_cast<T>(static_cast<f32>(width) * .5f * (1.f - scaleFactor)),
...
f32 Rectangle<T>::ComputeOverlapScore(const Rectangle<T>& other) const {
  ...
  if (denom <= 0.f) {
    PRINT_NAMED_ERROR(
        "Rectangle.ComputeOverlapScore.InvalidDenom",
