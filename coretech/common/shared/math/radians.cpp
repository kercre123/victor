...
Radians::Radians(const Radians& initRad) { (*this) = initRad; }
...
Radians operator+(const Radians& a, float b) { return a + Radians(b); }
Radians operator+(float a, const Radians& b) { return Radians(a) + b; }
void Radians::operator+=(float b) { (*this) = (*this) + b; }
void Radians::operator+=(const Radians& b) { (*this) = (*this) + b; }
...
Radians operator*(float a, const Radians& b) { return b * a; }
void Radians::operator*=(float b) { (*this) = (*this) * b; }
...
bool operator==(const Radians& a, const Radians& b) { return a.IsNear(b); }

// Inequality operator: Returns true if the magnitude of difference between
// the two radian values is outside tolerance.
bool operator!=(const Radians& a, const Radians& b) { return !(a == b); }
...
// Returns true if a < b
bool operator<(const Radians& a, const Radians& b) { return (b > a); }
...
Radians Radians::getAbsoluteVal() const { return Radians(std::abs(radians_)); }

// Returns the object's radians value in degrees
float Radians::getDegrees() const { return RAD_TO_DEG(radians_); }
...
void Radians::performRescaling(bool doRescaling) { doRescaling_ = doRescaling; }
...
void Radians::rescale() {
  ...
  if (std::abs(radians_) < 10.0f) {
    ...
    // If not, compute amount to shift by.  Divide by 2*M_PI_F but subtract .5
    // since we're going to be in the (-PI, PI] range.
