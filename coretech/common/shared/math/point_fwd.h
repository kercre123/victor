...
// Forward declaration
class Radians;
template <MatDimType, MatDimType, typename>
class SmallMatrix;
...
class Point : private std::array<T, N> {
 ...
 public:
  ...
  // extract elements in inclusive range [A,B] from the current point into a
  // smaller point e.g.:
  //    Point<5,int> a{1,2,3,4,5};
  //    Point<3,int> b = a.Slice<1,3>();  // b == {2,3,4}
  template <PointDimType A, PointDimType B,
            typename = std::enable_if_t<B <= N && A <= N &&
                                        A<B>> constexpr Point<B - A + 1, T>
                Slice() const;
  ...
  // Comparison
  // NOTE: Comparison of *all* elements must be true for the operator to return
  // true!!!
  //       See Any/All methods below to make this more explicit.
  constexpr bool operator<(
      const Point<N, T>& other) const;  // all elements less than
  constexpr bool operator>(
      const Point<N, T>& other) const;  // all elements greater than
  constexpr bool operator==(
      const Point<N, T>& other) const;  // all elements equal
  constexpr bool operator!=(
      const Point<N, T>& other) const;  // all elements equal
  constexpr bool operator<=(
      const Point<N, T>& other) const;  // all elements less than or equal
  constexpr bool operator>=(
      const Point<N, T>& other) const;  // all elements greater than or equal

  // Explicit Any vs. All comparison for choosing whether all elements or any
  // element has to satisfy the comparison. GT = Greater Than, GTE = Greater
  // Than or Equal, LT = Less Than, etc.
...
template <>
inline constexpr s32 AxisToIndex<'X'>() {
  return 0;
}
template <>
inline constexpr s32 AxisToIndex<'Y'>() {
  return 1;
}
template <>
inline constexpr s32 AxisToIndex<'Z'>() {
  return 2;
}
...
class UnitVector : public Point<N,T>
{
  ...
  UnitVector(typename std::enable_if<sizeof...(Tail)+1 == N, T>::type head,
Tail... tail) : Point<N,T>(head, T(tail)...)
  {
    this->makeUnitLength();
  }
...
template <PointDimType N, typename T>
constexpr bool IsNearlyEqual(const Point<N, T>& point1,
                             const Point<N, T>& point2,
                             const T eps = T(10) *
                                           std::numeric_limits<T>::epsilon());

// TODO: make arithemetic operations constexpr once constexpr lambdas are
// enabled in C++17
template <PointDimType N, typename T>
...
// append point2 onto the end of point1, making a new point of size Dim(p1) +
// Dim(p2)
template <PointDimType M, PointDimType N, typename T>
constexpr Point<M + N, T> Join(const Point<M, T>& point1,
                               const Point<N, T>& point2);
...
template <PointDimType N, typename T>
constexpr T ComputeDistanceBetween(const Point<N, T>& point1,
                                   const Point<N, T>& point2);

// Return true if the two points (vectors) are aligned within the given angle
// threshold
template <PointDimType N, typename T>
constexpr bool AreVectorsAligned(const Point<N, T>& point1,
                                 const Point<N, T>& point2,
                                 const Radians& angleThreshold);

// Return true if the two unit vectors are aligned within the given angle
// threshold NOTE: In DEBUG, the unit vectors' lengths are confirmed (i.e.
// asserted) to be 1.0, but not in RELEASE.
template <PointDimType N>
constexpr bool AreUnitVectorsAligned(const Point<N, f32>& unitVec1,
                                     const Point<N, f32>& unitVec2,
                                     const Radians& angleThreshold);
