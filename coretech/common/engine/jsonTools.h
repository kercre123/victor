...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - Set of functions that asserts the
// expected key with the expected type to be present
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -

// config: input json
// key:  key to look for
// debugName: name of place calling the function, to append to the assert event
// (for logging/debugging) ParseX: find the given key in the given json and
// assert that it's present and matches the given type
float ParseFloat(const Json::Value& config, const char* key,
                 const std::string& debugName);
uint8_t ParseUint8(const Json::Value& config, const char* key,
                   const std::string& debugName);
int8_t ParseInt8(const Json::Value& config, const char* key,
                 const std::string& debugName);
int32_t ParseInt32(const Json::Value& config, const char* key,
                   const std::string& debugName);
uint32_t ParseUInt32(const Json::Value& config, const char* key,
                     const std::string& debugName);
...
// Gets a value by name.  Returns true if value set successfully, false
// otherwise.
template <typename T>
bool GetValueOptional(const Json::Value& config, const std::string& key,
                      T& value);

// Gets (fixed-length) array of values by name.  Returns true if values is set
// successfully, false otherwise.
template <typename T, size_t N>
bool GetArrayOptional(const Json::Value& config, const std::string& key,
                      std::array<T, N>& values);

// Gets (variable-length) vector of values by name.  Returns true if values is
// set successfully, false otherwise.
template <typename T>
bool GetVectorOptional(const Json::Value& config, const std::string& key,
                       std::vector<T>& values);

// Get an Anki::Point by name.  Returns true if the point is set successfully,
// or false otherwise.
template <typename T, size_t N>
bool GetPointOptional(const Json::Value& node, const std::string& key,
                      Anki::Point<N, T>& pt);
...
bool GetPoseOptional(const Json::Value& node, const std::string& key,
                     Anki::Pose3d& pose);
...
bool GetColorOptional(const Json::Value& node, const std::string& key,
                      Anki::ColorRGBA& color);

// Get an angle back in Radians, possibly stored in degrees in the Json
bool GetAngleOptional(const Json::Value& jsonRoot, const std::string& key,
                      Radians& angle, bool storedInDegrees);
...
// Dump the json to the selected output (pretty-printed). The depth argument
// limits the depth of the tree that is printed. It is 0 by default, which means
// to print the whole tree
void PrintJsonCout(const Json::Value& config, int maxDepth = 0);  // std::cout
void PrintJsonDebug(const Json::Value& config, const std::string& eventName,
                    int maxDepth = 0);  // print_named_debug
void PrintJsonInfo(const Json::Value& config, const std::string& eventName,
                   int maxDepth = 0);  // print_named_info
void PrintJsonError(const Json::Value& config, const std::string& eventName,
                    int maxDepth = 0);  // print_named_error

// Returns true if a key other than something in expectedKeys is found at the
// root level. sets the badKey if an unexpected key is found
bool HasUnexpectedKeys(const Json::Value& config,
...
bool GetArrayOptional(const Json::Value& config, const std::string& key,
                      std::array<T, N>& values) {
  const Json::Value& child(config[key]);
  if (child.isNull() || not child.isArray() || child.size() != N) return false;
...
bool GetVectorOptional(const Json::Value& config, const std::string& key,
                       std::vector<T>& values) {
  const Json::Value& child(config[key]);
  if (child.isNull() || not child.isArray()) return false;
...
bool GetColorValuesToArrayOptional(const Json::Value& jsonRoot,
                                   const std::string& key,
                                   std::array<u32, N>& arr, bool parseAsFloat) {
  if (jsonRoot.isMember(key)) {
    ...
    if (arr.size() != values.size()) {
      PRINT_NAMED_ERROR("JsonTools.GetColorValuesToArrayOptional.DiffSizes",
                        "The json array and destination array are different "
                        "sizes. Expected %zu, got %u. Key: %s",
    ...
    for (u8 i = 0; i < (int)arr.size(); ++i) {
      if (parseAsFloat) {
        ColorRGBA color(values[i][0].asFloat(), values[i][1].asFloat(),
                        values[i][2].asFloat(), values[i][3].asFloat());
        arr[i] = color.AsRGBA();
      } else {
