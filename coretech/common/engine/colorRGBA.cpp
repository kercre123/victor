 ...
 * Date:   8/8/2014
 *
 * Description:  Implements an object for defining a RGBA (Red, Green, Blue,
 *Alpha) color, which will pack itself into a 32-bit integer for sending in
 *messages.
 *
...
ColorRGBA::ColorRGBA() : ColorRGBA(NamedColors::DEFAULT) {}
...
ColorRGBA::ColorRGBA(f32 red, f32 green, f32 blue, f32 alpha)
  ...
  if (red < 0.f || red > 1.f || green < 0.f || green > 1.f || blue < 0.f ||
      blue > 1.f || alpha < 0.f || alpha > 1.f) {
    PRINT_NAMED_WARNING(
        "ColorRGBA.OutOfRangeValues",
...
const ColorRGBA& GetByString(const std::string& name) {
  static const std::map<std::string, const ColorRGBA&> LUT = {
      ...
      {"YELLOW", YELLOW},     {"CYAN", CYAN},         {"ORANGE", ORANGE},
      {"MAGENTA", MAGENTA},   {"WHITE", WHITE},       {"BLACK", BLACK},
      {"DEFAULT", DEFAULT},   {"DARKGRAY", DARKGRAY}, {"DARKGREEN", DARKGREEN},
      {"OFFWHITE", OFFWHITE},
  };
...
ColorRGBA ColorRGBA::CreateFromColorIndex(uint32_t colorIndex) {
  ...
  // Generates 1 of the 7 possible non-black binary RGB combos
  // (R,G,B,RG,RB,GB,RGB) at decreasing intensities for higher indices

  const uint32_t kNumIntensityLevels = 5;
  const uint32_t kMaxIntensity = 255;
  const uint32_t kMinIntesity =
      64;  // used for "off", min "on" intensity is 1 step above this
  const uint32_t kIntensityStep =
      (kMaxIntensity - kMinIntesity) / kNumIntensityLevels;
  ...
  const uint32_t intensity =
      kMaxIntensity - (kIntensityStep * (colorIndex / 7));
