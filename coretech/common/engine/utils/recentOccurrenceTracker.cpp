 ...
 * Created: 2018-06-16
 *
 * Description: Simple tool to track arbitrary occurrences to see if they happen
 *X times within Y seconds
 *
...
RecentOccurrenceHandle::~RecentOccurrenceHandle() {
  // since this tracker handle is gone, the tracker may not need to carry as
  // many events anymore, so let it recompute it's limits
  if (_tracker != nullptr) {
...
void RecentOccurrenceHandle::TrackerDestructing() { _tracker = nullptr; }
...
RecentOccurrenceTracker::~RecentOccurrenceTracker() {
  // if there are any handles out there, let them know that the tracker is going
  // away
  int numHandles = 0;
  ...
  if (numHandles > 0) {
    PRINT_NAMED_WARNING(
        "RecentOccurrenceTracker.Destructor.WithHandles",
        "Tracker '%s' destructing but still has %d active handles",
        _debugName.c_str(), numHandles);
  }
}
...
void RecentOccurrenceTracker::AddOccurrence() {
  ...
  if (_occurrences.empty()) {
    ...
    PRINT_NAMED_WARNING("RecentOccurrenceTracker.AddOccurrence.NotTracked",
                        "Tracker '%s' added an occurrence, but pruned it away. "
                        "Has %zu handles (some may be null)",
                        _debugName.c_str(), _handles.size());
  }
}

void RecentOccurrenceTracker::Reset() { _occurrences.clear(); }

RecentOccurrenceTracker::Handle RecentOccurrenceTracker::GetHandle(
    int numberOfTimes, float amountOfSeconds) {
  std::shared_ptr<RecentOccurrenceHandle> handle(
      new RecentOccurrenceHandle(this, numberOfTimes, amountOfSeconds));
...
bool RecentOccurrenceTracker::ParseConfig(const Json::Value& config,
                                          int& numberOfTimes,
                                          float& amountOfSeconds) {
  if (config[kRecentOccurrenceKey].isObject()) {
    // default if nothing is specified is to return true if this has happened at
    // all (ever) if only a number or only a time is specified, that should work
    // as well
    numberOfTimes =
        config[kRecentOccurrenceKey].get(kNumberOfTimesKey, 1).asInt();
    amountOfSeconds =
        config[kRecentOccurrenceKey]
            .get(kAmountOfSecondsKey, std::numeric_limits<float>::max())
            .asFloat();
...
void RecentOccurrenceTracker::GetConfigJsonKeys(
    std::set<const char*>& expectedKeys) {
  expectedKeys.insert(kRecentOccurrenceKey);
}

bool RecentOccurrenceTracker::Check(int numberOfTimes,
                                    float amountOfSeconds) const {
  const float currTime_s = _timeProvider();
  ...
  // note: could use a binary search here, or cut off the search early and
  // compare to size, but this is much simpler
  int num = 0;
...
void RecentOccurrenceTracker::Prune(float currTime_s) {
  ...
  // now remove anything we're storing that's older than the max time we care
  // about
