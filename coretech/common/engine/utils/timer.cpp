 ...
 * Removes instance
 */
void BaseStationTimer::removeInstance() { Util::SafeDelete(_instance); }
...
void BaseStationTimer::UpdateTime(const BaseStationTime_t currTime_NS) {
  ...
  currTimeSecondsDouble_ =
      Util::numeric_cast<double>(Util::NanoSecToSec(currTime_NS));
  currTimeSecondsFloat_ = Util::numeric_cast<float>(currTimeSecondsDouble_);
  currTimeStamp_ = Util::numeric_cast<TimeStamp_t>(
      Util::SecToMilliSec(currTimeSecondsDouble_));
...
const size_t BaseStationTimer::GetTickCount() const { return tickCount_; }

/*
void SchedulingTimer::Reset(double newPeriodTime, bool repeating, bool
checkForPeriodSkips)
{
if(newPeriodTime > 0) {
...
// Not initialized
if(periodTime_ < 0) {
  //PRINT_NAMED_WARNING("SchedulingTimer", "Attempting to use uninitialized
timer."); return false;
}

// Skip logic if this is a non-repeating timer that already fired
if(!repeating_ && firedFirstTime_) {
  //PRINT_NAMED_ERROR("SchedulingTimer", "Non-repeating timer already fired.
Won't fire again..."); return false;
}

// Check for sufficiently elapsed time
bool ready = false;
bool printedError = false;
while(lastFireTime_ + periodTime_ <=
BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() +
FLOATING_POINT_COMPARISON_TOLERANCE) {
  ...
  if(repeating_) {
    ...
    if(checkForPeriodSkips_ && ready && !printedError && firedFirstTime_) {
      printedError = true;
      PRINT_NAMED_WARNING("SchedulingTimer", "Skipped over a period of timer.
Elapsed time: %f, Period = %f",
          BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() -
lastFireTime_, periodTime_);
    }
