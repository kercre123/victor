...
float ParseFloat(const Json::Value& config, const char* key,
  ...
  DEV_ASSERT_MSG(val.isNumeric(),
                 (debugName + ".ParseFloat.NotValidFloat").c_str(), "%s", key);
  return val.asFloat();
}
...
uint8_t ParseUint8(const Json::Value& config, const char* key,
  ...
  const auto& val = config[key];
  DEV_ASSERT_MSG(val.isNumeric(),
                 (debugName + ".ParseUint8.NotValidUint8").c_str(), "%s", key);
...
int8_t ParseInt8(const Json::Value& config, const char* key,
  ...
  const auto& val = config[key];
  DEV_ASSERT_MSG(val.isNumeric(),
                 (debugName + ".ParseUint8.NotValidInt8").c_str(), "%s", key);
...
int32_t ParseInt32(const Json::Value& config, const char* key,
  ...
  const auto& val = config[key];
  DEV_ASSERT_MSG(val.isNumeric(),
                 (debugName + ".ParseInt.NotValidInt32").c_str(), "%s", key);
  return val.asInt();
}
...
uint32_t ParseUInt32(const Json::Value& config, const char* key,
  ...
  const auto& val = config[key];
  DEV_ASSERT_MSG(val.isNumeric(),
                 (debugName + ".ParseUint.NotValidUInt32").c_str(), "%s", key);
  return val.asUInt();
}
...
bool ParseBool(const Json::Value& config, const char* key,
               const std::string& debugName) {
  const auto& val = config[key];
  DEV_ASSERT_MSG(val.isBool(), (debugName + ".ParseBool.NotValidBool").c_str(),
                 "%s", key);
  return val.asBool();
};
...
std::string ParseString(const Json::Value& config, const char* key,
                        const std::string& debugName) {
  const auto& val = config[key];
  DEV_ASSERT_MSG(val.isString(),
                 (debugName + ".ParseString.NotValidString").c_str(), "%s",
                 key);
  return val.asString();
};
...
bool GetPoseOptional(const Json::Value& config, const std::string& key,
                     Anki::Pose3d& pose) {
  bool retVal = false;
  ...
  if (not child.isNull()) {
    ...
    if (child.isMember("Angle") && GetPointOptional(child, "Axis", axis) &&
...
bool GetColorOptional(const Json::Value& jsonRoot, const std::string& key,
  ...
  if (jsonRoot.isMember(key)) {
    ...
    if (node.isString()) {
      ...
      PRINT_NAMED_WARNING(
          "JsonTools.GetColorOptional",
          "Expecting color in Json to be a string or 3 or 4 element array");
    }
...
__attribute__((used)) void PrintJsonInternal(
    const Json::Value& config, int maxDepth,
    const std::function<void(std::string)>& outputFunction) {
  if (maxDepth == 0) {
    ...
    for (unsigned int currDepth = 1; currDepth < maxDepth; ++currDepth) {
      ...
      for (std::vector<Json::Value::iterator>::iterator qIt = Q1.begin();
           qIt != Q1.end(); ++qIt) {
        if ((**qIt).size() > 1) {
          for (Json::Value::iterator it = (**qIt).begin(); it != (**qIt).end();
               ++it) {
            Q2.push_back(it);
          }
        }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
__attribute__((used)) void PrintJsonCout(const Json::Value& config,
                                         int maxDepth) {
  auto function = [](const std::string& str) { std::cout << str << std::endl; };
  PrintJsonInternal(config, maxDepth, function);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
__attribute__((used)) void PrintJsonDebug(const Json::Value& config,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
__attribute__((used)) void PrintJsonInfo(const Json::Value& config,
                                         const std::string& eventName,
                                         int maxDepth) {
  auto function = [&eventName](const std::string& str) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
__attribute__((used)) void PrintJsonError(const Json::Value& config,
                                          const std::string& eventName,
                                          int maxDepth) {
  auto function = [&eventName](const std::string& str) {
...
bool HasUnexpectedKeys(const Json::Value& config,
  ...
  for (const auto& key : config.getMemberNames()) {
    const auto it = std::find_if(expectedKeys.begin(), expectedKeys.end(),
                                 [&key](const char* c) { return (key == c); });
