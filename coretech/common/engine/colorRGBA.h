...
inline u8 ColorRGBA::GetU8(const f32 value) {
  if (value < 0.f || value > 1.f) {
    PRINT_NAMED_WARNING(
        "ColorRGBA.GetU8.OutOfRangeValue",
...
inline u32 ColorRGBA::AsRGBA() const {
  const u32 packedValue =
      ((static_cast<u32>(r()) << 24) | (static_cast<u32>(g()) << 16) |
       (static_cast<u32>(b()) << 8) | (static_cast<u32>(alpha())));
...
inline u32 ColorRGBA::As0RGB() const {
  const u32 packedValue =
      ((static_cast<u32>(r()) << 16) | (static_cast<u32>(g()) << 8) |
       (static_cast<u32>(b())));
