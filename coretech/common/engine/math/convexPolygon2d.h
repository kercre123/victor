 ...
 * Created: 2017-10-3
 *
 * Description: A class wrapper for a 2d polygon with convexity helpers and
 *constraints.
 *
...
class ConvexPolygon : public Poly2f {
 public:
  ...
  virtual Point2f& operator[](size_t i) override {
    return _points[GetInternalIdx(i)];
  }
  virtual const Point2f& operator[](size_t i) const override {
    return _points[GetInternalIdx(i)];
  }
 ...
 private:
  ...
  size_t GetInternalIdx(size_t i) const {
    return (_currDirection == CW) ? i : size() - i;
  }
};
