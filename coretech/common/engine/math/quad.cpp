...
CornerName& operator++(CornerName& cname) {
  cname = (cname < NumCorners)
              ? static_cast<CornerName>(static_cast<int>(cname) + 1)
              : NumCorners;
  return cname;
}
...
CornerName operator++(CornerName& cname, int) {
  CornerName cnameToReturn =
      cname;  // make a copy to return before we increment
  ++cname;
...
void test(void) {
  ...
  // Point2f n;
  // q.SortCornersClockwise(n); // Should fail: triggers static assert because
  // N==2

  Quad3f q3;
  Point3f n3;
  // q3.SortCornersClockwise(); // Should fail: triggers static assert because
  // N!=2
