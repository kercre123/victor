 ...
 * Created: 8/3/2017
 *
 * Description: Defines classes for storing a 2D/3D Transform, meaning a
...
class Transform2d {
 public:
  ...
  // Check *exact* numerical equality (generally only useful for unit tests,
  // since floating point is involved)
...
class Transform3d {
 public:
  Transform3d(const Rotation3d& R = Rotation3d(0.f, Z_AXIS_3D()),
              const Vec3f& T = {0.f, 0.f, 0.f})
      : _rotation(R), _translation(T) {}
  ...
  // translate along its current angle, along the +x axis (negative means
  // backwards)
  void TranslateForward(float dist);
