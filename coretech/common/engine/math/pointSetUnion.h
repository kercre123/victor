...
// TODO: probably should add some typetrait checks to make sure GetAABB and
// intersects are all
//       valid for arbitrary instances, but this should be fine for now

template <DimType N, typename T, class S1, class S2,
          typename = typename std::enable_if<(
              std::is_base_of<PointSet<N, T>, S1>::value &&
              std::is_base_of<PointSet<N, T>, S2>::value)>::type>
class PointSetUnion : public PointSet<N, T> {
 ...
 protected:
  ...
  template <class Set, typename = typename std::enable_if<std::is_base_of<
                           ConvexPointSet<N, T>, Set>::value>::type>
  inline static bool SetContains(const Set& S,
                                 const AxisAlignedHyperCube<N, T>& C) {
    return S.ContainsAll(C.GetVertices());
  }

  // fall through for Union types
  template <class M1, class M2>
  inline static bool SetContains(const PointSetUnion<N, T, M1, M2>& S,
                                 const AxisAlignedHyperCube<N, T>& C) {
...
// helper for making unions of multiple sets via recursion: (S₁ ∪ (S₂ ∪ (S₃ ∪
// (... ∪ Sₓ))))
