...
bool FastPolygon::Contains(float x, float y) const {
  ...
  for (const auto& edgeVec : _perpendicularEdgeVectors) {
    ...
    float dot = edgeVec.first.x() * (x - _points[pointIdx].x()) +
                edgeVec.first.y() * (y - _points[pointIdx].y());
...
bool FastPolygon::InHalfPlane(const Halfplane2f& H) const {
  return std::all_of(begin(), end(),
                     [&H](const Point2f& p) { return H.Contains(p); });
}
...
bool FastPolygon::Intersects(const AxisAlignedQuad& quad) const {
  ...
  if (FLT_LT(GetMaxX(), quad.GetMinVertex().x())) {
    return false;
  }
  if (FLT_LT(GetMaxY(), quad.GetMinVertex().y())) {
    return false;
  }
  if (FLT_GT(GetMinX(), quad.GetMaxVertex().x())) {
    return false;
  }
  if (FLT_GT(GetMinY(), quad.GetMaxVertex().y())) {
    return false;
  }

  // fastPolygon line segments define the halfplane boundary of points inside
  // the polygon, so check negative halfplane instead
  return std::none_of(
      _edgeSegments.begin(), _edgeSegments.end(),
      [&](const LineSegment& l) { return quad.InNegativeHalfPlane(l); });
}
...
void FastPolygon::ComputeCircles() {
    ...
    for (size_t i = 0; i < numPts; ++i) {
      ...
      if (!IsNearlyEqual(_circleCenter - _points[pointIdx], {0.f, 0.f}) &&
        ...
        distanceToEdgeSquared =
            std::pow(Anki::DotProduct(_perpendicularEdgeVectors[i].first,
                                      _circleCenter - _points[pointIdx]),
                     2);
      }
  ...
  if (_inscribedRadiusSquared > _circumscribedRadiusSquared) {
    CoreTechPrint(
        "ERROR: inscribed radius of %f is >= circumscribed radius of %f\n",
        _inscribedRadiusSquared, _circumscribedRadiusSquared);
  }
}
...
void FastPolygon::CreateEdgeVectors() {
  ...
  if (numPts > 1)  // proper polygon
  {
    for (size_t i = 0; i < numPts; ++i) {
      ...
      float oneOverNorm = IsNearlyEqual(edgeVector, {0.f, 0.f})
                              ? 0.f
                              : 1.0 / edgeVector.Length();
...
void FastPolygon::SortEdgeVectors() {
  ...
  for (float theta = 0; theta <= 2 * M_PI + 1e-5; theta += stepSize) {
    testPoints.emplace_back(std::make_pair(
        false, Point2f{outerRadius * cosf(theta) + _circleCenter.x(),
                       outerRadius * sinf(theta) + _circleCenter.y()}));
    testPoints.emplace_back(std::make_pair(
        false, Point2f{innerRadius * cosf(theta) + _circleCenter.x(),
...
unsigned int FastPolygon::CheckTestPoints(
    std::vector<std::pair<bool, Point2f> >& testPoints, size_t edgeIdx,
    bool dryRun) {
  ...
  for (auto& testEntry : testPoints) {
    ...
    const float dot = Anki::DotProduct(_perpendicularEdgeVectors[edgeIdx].first,
                                       testEntry.second - _points[pointIdx]);

    if (dot > 0.0f) {
      // is outside of edge, so this point is a hit! (we want to throw out as
      // many test points as we can)
      newHits++;
      if (!dryRun) {
...
bool FastPolygon::Contains_Assumptionless(const Point2f& p) const {
  ...
  if (distSquared < _inscribedRadiusSquared)
    return true;  // definitely is inside
  ...
  // TODO: (mrw) cache sqrt call or dont use sqrt at all. Try improved winding#
  // algo here:
  //             http://geomalgorithms.com/a03-_inclusion.html
  int nCollisions = 0;
  const LineSegment testRay(
      p, p + Point2f(2 * sqrt(_circumscribedRadiusSquared), 0));
...
bool FastPolygon::Intersects(const LineSegment& l) const {
  // TODO: we can add a AABB check on the line segment to see if it is even
  ...
  // need to explicitly check all edges since the end points of the test segment
  // are both outside the polygon
  for (const auto& edge : _edgeSegments) {
