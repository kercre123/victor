...
ConvexPolygon::ConvexPolygon(const Poly2f& basePolygon)
    : Poly2f(basePolygon), _currDirection(CW) {
  // check convexity only if in dev code, otherwise assume it is good
  DEV_ASSERT(IsConvex(basePolygon),
             "Tried to create a convex polygon from non-convex polygon");

  // only check internal angle orientation if basePolygon is not a point or line
  if (basePolygon.size() > 2) {
    // check first internal angle and force to [0, 2π) to get current clock
    // direction
    f32 internalAngle =
        std::fmod(GetEdgeAngle(1) - GetEdgeAngle(0) + 2 * M_PI, 2 * M_PI);
    if (internalAngle < M_PI)  // poly inserted is oriented counter clockwise,
                               // so reverse it to CW
    {
...
bool ConvexPolygon::IsConvex(const Poly2f& poly) {
  ...
  for (int i = 2; i < poly.size(); ++i) {
    ...
    // we may end up with straight line segments, so make sure floating point
    // precision doesnt break anything
    if (!d_angle1.IsNear(0) && !d_angle2.IsNear(0)) {
      // make sure the direction of both angles is the same
      if (std::signbit(d_angle1.ToFloat()) !=
          std::signbit(d_angle2.ToFloat())) {
        return false;
      }
    }
  }
...
void ConvexPolygon::RadialExpand(f32 d) {
  if (d >= 0) {
    ...
    for (Point2f& p : _points) {
      ...
      // (point / length) is a unit vector, therefor offset is a push scaled by
      // `d` on this vector
      p += centeredPoint * (d / length);
    }
  } else {
    PRINT_NAMED_WARNING("ConvexPolygon.RadialExpand",
                        "called expand with a negative distance.");
  }
}
...
ConvexPolygon ConvexPolygon::ConvexHull(std::vector<Point2f>&& points) {
  ...
  std::nth_element(points.begin(), points.begin(), points.end(),
                   [](const Point2f& p, const Point2f& q) {
                     return FLT_NEAR(p.y(), q.y()) ? FLT_LT(p.x(), q.x())
                                                   : FLT_LT(p.y(), q.y());
                   });
  ...
  const auto crossProd = [](const Point2f& p0, const Point2f& p1,
                            const Point2f& p2) -> float {
    return (p1.x() - p0.x()) * (p2.y() - p0.y()) -
           (p2.x() - p0.x()) * (p1.y() - p0.y());
  };

  // sort function for sorting all points relative to their angle with the start
  // point
  const auto compareCounterClockwise =
      [&p0, &crossProd](const Point2f& p1, const Point2f& p2) -> bool {
  ...
  for (size_t i = 2; i < numPoints; ++i) {
    // calculate orientation we would obtain adding the point to the current
    // convex hull
    size_t curPolyTop = outPoly.size() - 1;
    float newOrientation =
        crossProd(outPoly[curPolyTop - 1], outPoly[curPolyTop], points[i]);
