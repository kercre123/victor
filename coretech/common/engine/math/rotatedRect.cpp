...
bool CheckBoundingRect(const std::vector<Point2f>& polygon, size_t startIndex,
                       float& minArea, Rectangle<float>& bestRect,
                       float& bestTheta);
...
RotatedRectangle::RotatedRectangle(float x0, float y0, float x1, float y1,
                                   float otherSideLength) {
  InitFromPoints(x0, y0, x1, y1, otherSideLength);
}

RotatedRectangle::RotatedRectangle(const Quad2f& quad) { ImportQuad(quad); }

void RotatedRectangle::InitFromPoints(float x0, float y0, float x1, float y1,
                                      float otherSideLength) {
...
bool RotatedRectangle::Contains(float x, float y) const {
  ...
  if (proj0 < 0.0 || proj0 > length0) return false;

  float proj1 = vec1X * testVecX + vec1Y * testVecY;
  if (proj1 < 0.0 || proj1 > length1) return false;
...
void RotatedRectangle::ImportQuad(const Quad2f& quadUnsorted) {
  ...
  std::vector<Point2f> polygon = {quad[Quad::TopLeft], quad[Quad::TopRight],
  ...
  if (size > 1) {
    for (size_t i = 0; i < size; ++i) {
      if (RotatedRectHelper::CheckBoundingRect(polygon, i, minArea, bestRect,
                                               bestTheta)) {
        found = true;
        bestIdx = i;
      }
  ...
  if (found) {
    ...
    InitFromPoints(point1.x(), point1.y(), point0.x(), point0.y(),
                   bestRect.GetHeight());
  }
}

bool RotatedRectHelper::CheckBoundingRect(const std::vector<Point2f>& polygon,
                                          size_t startIndex, float& minArea,
                                          Rectangle<float>& bestRect,
                                          float& bestTheta) {
