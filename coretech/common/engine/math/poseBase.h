...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
PoseBase<PoseNd, TransformNd>::PoseBase(const TransformNd& transform,
                                        const PoseNd& parentPose,
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
PoseBase<PoseNd, TransformNd>::PoseBase(const PoseNd& parentPose,
                                        const std::string& name)
...
PoseBase<PoseNd, TransformNd>::~PoseBase() {
  if (!IsNull()) {
    ...
    if (!_node->IsOwned() && !AreUnownedParentsAllowed()) {
      // Trying to diagnose COZMO-10891: if this was the last owner, the node
      // should have use_count==1
      ANKI_VERIFY(_node.use_count() == 1, "PoseBase.Destructor.NotLastOwner",
                  "Removing pose %d(%s)'s node's last known owner, but it "
                  "still has use_count=%u",
...
PoseBase<PoseNd, TransformNd>::PoseBase(const PoseBase& other)
    : _node(new PoseTreeNode(
          *other._node))  // don't share Nodes with other! copy the contents!
{
  _node->AddOwner();
}
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
PoseBase<PoseNd, TransformNd>& PoseBase<PoseNd, TransformNd>::operator=(
    const PoseBase& other) {
  if (this != &other) {
    if (other.IsNull()) {
      _node->RemoveOwner();
      if (!_node->IsOwned() && !AreUnownedParentsAllowed()) {
      ...
      // don't share Nodes with other! assign the contents! note that there's no
      // new owner of _node here!
      *_node = *other._node;
    }
  }
  return *this;
...
//  // rvalue assignment
//  template<class PoseNd, class TransformNd>
//  PoseBase<PoseNd,TransformNd>&
//  PoseBase<PoseNd,TransformNd>::operator=(PoseBase&& other)
//  {
//    if(this != &other)
//    {
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
inline bool PoseBase<PoseNd, TransformNd>::IsChildOf(
    const PoseNd& otherPose) const {
  return (IsNull() || otherPose.IsNull() ? false
                                         : _node->IsChildOf(*otherPose._node));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
inline bool PoseBase<PoseNd, TransformNd>::IsParentOf(
    const PoseNd& otherPose) const {
  return (IsNull() || otherPose.IsNull() ? false
                                         : _node->IsParentOf(*otherPose._node));
}
...
void PoseBase<PoseNd, TransformNd>::SetParent(const PoseNd& otherPose) {
  ...
  if (!AreUnownedParentsAllowed()) {
    ANKI_VERIFY(otherPose.IsOwned(), "PoseBase.SetParent.UnownedParent",
                "Setting parent of %d(%s) to unowned pose %d(%s)", GetID(),
                GetName().c_str(), otherPose.GetID(),
                otherPose.GetName().c_str());
  }
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
void PoseBase<PoseNd, TransformNd>::PrintNamedPathToRoot(
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
std::string PoseBase<PoseNd, TransformNd>::GetNamedPathToRoot(
    bool showTranslations) const {
  if (IsNull()) {
    return "NullNode";
  }
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
bool PoseBase<PoseNd, TransformNd>::HasSameParentAs(
    const PoseNd& otherPose) const {
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
bool PoseBase<PoseNd, TransformNd>::HasSameRootAs(
    const PoseNd& otherPose) const {
  if (IsNull() || otherPose.IsNull()) {
    return false;
  }
...
std::string PoseBase<PoseNd, TransformNd>::GetParentString() const {
  ...
  if (IsNull() || !HasParent()) {
    ...
    ss << _node->GetRawParentPtr()->GetName() << "(0x"
       << _node->GetRawParentPtr() << ")";
  }
...
PoseNd PoseBase<PoseNd, TransformNd>::GetParent() const {
  ...
  return parent;  // Likely uses RVO so no copy. TODO: Use move semantics
                  // instead?
}
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
bool PoseBase<PoseNd, TransformNd>::GetWithRespectTo(const PoseNd& fromPose,
                                                     const PoseNd& toPose,
                                                     PoseNd& P_wrt_other) {
  if (!ANKI_VERIFY(
          !fromPose.IsNull() && !toPose.IsNull() && !P_wrt_other.IsNull(),
          "PoseBase.GetWithRespectTo.NullInputPose",
          "FromNull:%s ToNull:%s WrtOtherNull:%s",
          fromPose.IsNull() ? "Y" : "N", toPose.IsNull() ? "Y" : "N",
          P_wrt_other.IsNull() ? "Y" : "N")) {
    return false;
  }

  if (&fromPose == &toPose) {
    // Asked for pose w.r.t. itself. Just return a pose with a zero transform,
    // parented to toPose.
  ...
  DEV_ASSERT(to->GetTreeDepth() == from->GetTreeDepth(),
