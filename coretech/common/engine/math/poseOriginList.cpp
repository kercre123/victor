...
PoseOriginID_t PoseOriginList::AddNewOrigin() {
  ...
  ANKI_VERIFY(success, "PoseOriginList.AddNewOrigin.AddWithIDFailed", "ID:%d",
              ID);
...
bool PoseOriginList::AddOriginWithID(PoseOriginID_t ID) {
  ...
  auto result =
      _origins.emplace(ID, std::unique_ptr<PoseOrigin>(new PoseOrigin(name)));
  if (!ANKI_VERIFY(result.second, "PoseOriginList.AddOriginWithID.DuplicateID",
                   "%d", ID)) {
    if (ANKI_DEV_CHEATS) {
      Util::sAbort();
    }
...
const PoseOrigin& PoseOriginList::GetOriginByID(PoseOriginID_t ID) const {
  auto iter = _origins.find(ID);
  if (!ANKI_VERIFY(iter != _origins.end(), "PoseOriginList.GetOriginByID.BadID",
                   "%d", ID)) {
    static const PoseOrigin DefaultOrigin("OriginListDefaultOrigin");
    return DefaultOrigin;
  }

  DEV_ASSERT_MSG(nullptr != iter->second,
                 "PoseOriginList.GetOriginByID.NullOrigin", "%d", ID);

  return *iter->second;
}

Result PoseOriginList::Rejigger(const PoseOrigin& newOrigin,
                                const Transform3d& transform) {
  const PoseOriginID_t newOriginID = newOrigin.GetID();
  if (!ANKI_VERIFY(ContainsOriginID(newOriginID),
                   "PoseOriginList.Rejigger.UnknownOrigin",
                   "Pose ID:%d (%s) not known to PoseOriginList", newOriginID,
                   newOrigin.GetName().c_str())) {
    return RESULT_FAIL;
  }

  if (!ANKI_VERIFY(_current.originID != newOriginID,
                   "PoseOriginList.Rejigger.NewOriginIsCurrent", "ID:%d",
                   newOriginID)) {
    return RESULT_FAIL;
  }
  ...
  DEV_ASSERT_MSG(origIter != _origins.end(),
                 "PoseOriginList.Rejigger.InvalidCurrentOriginID", "ID:%d",
                 _current.originID);
  DEV_ASSERT_MSG(newIter != _origins.end(),
                 "PoseOriginList.Rejigger.InvalidNewOriginID", "ID:%d",
                 newOriginID);
  DEV_ASSERT_MSG(
      IsNearlyEqual(origIter->second->GetTranslation(), {0.f, 0.f, 0.f}),
  ...
  DEV_ASSERT_MSG(_current.originID == newOriginID,
                 "PoseOriginList.Rejigger.BadFinalCurrentOriginID",
...
void PoseOriginList::Flatten(PoseOriginID_t worldOriginID) {
  DEV_ASSERT_MSG(ContainsOriginID(worldOriginID),
                 "PoseOriginList.Flatten.BadID", "ID:%d", worldOriginID);
  ...
  for (auto& originAndIdPair : _origins) {
    ...
    DEV_ASSERT(
        origin != nullptr,
        "PoseOriginList.Flatten.NullOrigin");  // Should REALLY never happen

    // if this origin has a parent and it's not the world origin, we want to
    // update this origin to be a direct child of the world origin
    if (origin->HasParent() && !origin->IsChildOf(worldOrigin)) {
      // get WRT current origin, and store directly in place (we need to avoid
      // creating a new Pose3d because we don't want poses stored in the
      // PoseOriginList to ever be deleted (which affects their ownership of
      // internal PoseTreeNodes)
...
bool PoseOriginList::SanityCheckOwnership() const {
  ...
  for (auto const& originAndIdPair : _origins) {
    ...
    allPosesOwned &= ANKI_VERIFY(
        origin->IsOwned(), "PoseOriginList.SanityCheckOwnership.UnownedOrigin",
    ...
    if (origin->HasParent()) {
      ...
      allPosesOwned &= ANKI_VERIFY(
          ...
          "Pose %d(%s)'s parent %d(%s) is unowned", origin->GetID(),
          origin->GetName().c_str(), parent.GetID(), parent.GetName().c_str());
    }
  }
