...
#include "coretech/common/engine/math/triangle_fwd.h"

namespace Anki {

template <typename T>
Triangle<T>::Triangle(const Point2<T>& point1, const Point2<T>& point2,
...
template <typename T>
bool IsPointWithinTriangleHelper(const Point<2, T>& point, const f32 x1,
                                 const f32 y1, const f32 x2, const f32 y2,
  ...
  const f32 divisor = 1.f / ((y2 - y3) * (x1 - x3) +
                             (x3 - x2) * (y1 - y3));  // == 1 / determinant
...
bool IsPointWithinTriangle(const Point<2, T>& point,
  ...
  const bool isContained =
      IsPointWithinTriangleHelper(point, x1, y1, x2, y2, x3, y3);
...
Point<2, T> Triangle<T>::GetCentroid() const {
  ...
  Point<2, T> centroid =
      (*this)[2] + oppositePointTowardsMidPoint *
                       0.666f;  // it's 2/3 away from the point along the median
  return centroid;
}
