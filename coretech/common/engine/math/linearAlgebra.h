...
static inline int GetOpenCvSolveMethod(LeastSquaresMethod method) {
  switch (method) {
    case LeastSquaresMethod::LU:
      return cv::DECOMP_LU;
    case LeastSquaresMethod::Cholesky:
      return cv::DECOMP_CHOLESKY;
    case LeastSquaresMethod::Eigenvalue:
      return cv::DECOMP_EIG;
    case LeastSquaresMethod::SVD:
      return cv::DECOMP_SVD;
...
Result LeastSquares(const SmallMatrix<M, N, T>& A,
  ...
  const bool success =
      cv::solve(A.get_CvMatx_(), b.get_CvMatx_(), x.get_CvMatx_(),
...
template <MatDimType M, MatDimType N, typename T>
Result LeastSquares(const SmallMatrix<M, N, T>& A, const Point<M, T>& bIn,
                    Point<N, T>& xOut, LeastSquaresMethod method) {
