...
class PoseBase {
 public:
  ...
  // Note that HasSameRootAs will be true if this is other's root or vice versa
  ...
  void PreComposeWith(const PoseNd& other) {
    GetTransform().PreComposeWith(other.GetTransform());
  }
  void operator*=(const PoseNd& other) {
  ...
  // This is "cheap", but not "free". Consider using one of the above helpers
  // for comparing relationships b/w poses. NOTE: These return by value, not
  // reference. Do not store references to members in line! I.e., never do:
  ...
  // Check for exact numerical equality of Transform and matching parents/names.
  // Generally only useful for unit tests.
  bool operator==(const PoseNd& other) const {
    return ((GetTransform() == other.GetTransform()) &&
            HasSameParentAs(other) && GetName() == other.GetName());
  }

  // Useful for debugging and logging messages: print/return a string indicating
  // the path up to the root node
  std::string GetNamedPathToRoot(bool showTranslations) const;
  void PrintNamedPathToRoot(bool showTranslations) const;

  // An "unowned" parent can result when, for example, Pose A uses a temporary
  // Pose B as its parent and then Pose B goes out of scope. No Pose actually
  // "owns" the node inside Pose B, but Pose A still refers to it. If unowned
  // parents are not allowed, then a parent will be ANKI_VERIFY'd to have an
  // owner each time its node's GetParent() method is called. Normally, unowned
  // parents are *not* allowed but this can be globally adjusted if desired
  // (e.g. for unit tests).
 ...
 protected:
  ...
  PoseBase();
  PoseBase(const TransformNd& transform, const PoseNd& parentPose,
           const std::string& name = "");
  PoseBase(const TransformNd& transform, const std::string& name = "");
  PoseBase(const PoseNd& parentPose, const std::string& name);

  // Copy/assignment. NOTE: IDs are *NOT* copied and name is appended with
  // "_COPY".
  PoseBase(const PoseBase& other);
  PoseBase& operator=(const PoseBase& other);

  // COZMO-10891: Removing RValue construction/assignment for trying to debug
  // this ticket
  // // Rvalue construction/assignment. NOTE: IDS *are* moved and name is
  // preserved.
  PoseBase(PoseBase&& other) = delete;
  PoseBase& operator=(PoseBase&& other) = delete;

  static bool GetWithRespectTo(const PoseNd& from, const PoseNd& to,
                               PoseNd& newPose);

  // Create a new Pose object which "wraps" an existing node
  class PoseTreeNode;
  void WrapExistingNode(const std::shared_ptr<PoseTreeNode>& node) {
    _node = node;
    _node->AddOwner();
  }
