 ...
 *
 *
 * Description: Defines a PoseOriginList for maintaining multiple origins
 *(a.k.a. coordinate frames).
 *
...
class PoseOriginList {
 public:
  ...
  // Add a new origin to the list and return its unique ID. This is the
  // preferred way to add origins.
  PoseOriginID_t AddNewOrigin();

  // Allows you to add an origin with a specific ID, e.g. if you are mirroring
  // another origin lists via messages. Will refuse to add duplicate ID (and
  // return false to indicate failure). Use with care!
  ...
  bool IsPoseInCurrentOrigin(const Pose3d& pose) const {
    return pose.HasSameRootAs(GetCurrentOrigin());
  }
 ...
 private:
  ...
  static_assert(
      std::is_same<PoseID_t, PoseOriginID_t>::value,
...
inline const PoseOrigin& PoseOriginList::GetCurrentOrigin() const {
  DEV_ASSERT(_current.originID != UnknownOriginID,
             "PoseOriginList.GetCurrentOrigin.NoCurrentOriginID");
  DEV_ASSERT(_current.originPtr != nullptr,
             "PoseOriginList.GetCurrentOrigin.NullCurrentOrigin");
...
inline PoseOriginID_t PoseOriginList::GetCurrentOriginID() const {
  DEV_ASSERT_MSG(_current.originPtr == nullptr ||
                 ...
                 "Origin.GetID:%d OriginID:%d", _current.originPtr->GetID(),
                 _current.originID);
