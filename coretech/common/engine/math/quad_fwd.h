...
class Quadrilateral : public std::array<Point<N, T>, 4> {
 public:
  ...
  // WARNING:
  // The width and height of a floating point Rectangle is different than that
  // of an integer rectangle.
  // template<typename OutType> Rectangle<OutType> ComputeBoundingRectangle()
  // const;
  ...
  Quadrilateral<N, T> SortCornersClockwise(
      const Point<N, T>& aroundNormal) const;  // in place
  ...
  // Returns true if this quad contains all the corners from the other quad
  // (fully contains the other convex quad)
  ...
  // any corners of one quad are inside the other one, as well as whether any
  // segments collide
  bool Intersects(const Quadrilateral<2, T>& other) const;
  bool Intersects(const Point2<T>& rayFrom, const Point2<T>& rayTo,
                  Point2<T>* outIntersectionPoint = nullptr) const;
...
template <QuadDimType N, typename T>
bool IsNearlyEqual(const Quadrilateral<N, T>& quad1,
                   const Quadrilateral<N, T>& quad2, const T eps);
