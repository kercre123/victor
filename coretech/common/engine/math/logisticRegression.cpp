...
// Ptr<cv::ml::LogisticRegression> cv::ml::LogisticRegression::load(const
// String& filepath, const String& nodeName)
//{
...
float calculateError(const cv::Mat& _p_labels, const cv::Mat& _o_labels) {
  ...
  accuracy = (float)cv::countNonZero(_p_labels_temp == _o_labels_temp) /
             _p_labels_temp.rows;
  error = 1 - accuracy;
  return error;
}
...
bool WeightedLogisticRegression::train(const Ptr<TrainData>& trainData, int) {
  ...
  if (!_sample_weights.empty() && (_labels_i.rows != _sample_weights.rows)) {
    CV_Error(CV_StsBadArg,
             "number of rows in sample weights and labels should be equal");
  }
  ...
  if (num_classes == 2) {
    ...
    if (this->params.train_method == cv::ml::LogisticRegression::BATCH)
      new_theta =
          batch_gradient_descent(data_t, labels, init_theta, _sample_weights);
    else
    ...
    Mat labels_binary;
    int ii = 0;
    for (std::map<int, int>::iterator it = this->forward_mapper.begin();
...
float WeightedLogisticRegression::predict(InputArray samples,
                                          OutputArray results,
                                          int flags) const {
...
double WeightedLogisticRegression::compute_cost(const Mat& _data,
                                                const Mat& _labels,
...
struct LogisticRegression_ComputeDradient_ : ParallelLoopBody {
  ...
  LogisticRegression_ComputeDradient_(const Mat& _data, const Mat& _theta,
                                      const Mat& _pcal_a, const Mat& _weights,
  ...
  void operator()(const cv::Range& r) const override {
    ...
    for (int ii = r.start; ii < r.end; ii++) {
      ...
      _gradient.row(ii) =
          (1.0 / m) * sum(pcal_wab)[0] + (lambda / m) * _theta.row(ii);
    }
  }
};

void WeightedLogisticRegression::compute_gradient(
    const Mat& _data, const Mat& _labels, const Mat& _weights,
  ...
  LogisticRegression_ComputeDradient_ invoker(_data, _theta, pcal_a, _weights,
                                              _lambda, _gradient);
  cv::parallel_for_(cv::Range(1, _gradient.rows), invoker);
}

Mat WeightedLogisticRegression::batch_gradient_descent(const Mat& _data,
                                                       const Mat& _labels,
                                                       const Mat& _init_theta,
                                                       const Mat& _weights) {
...
Mat WeightedLogisticRegression::mini_batch_gradient_descent(
...
Mat WeightedLogisticRegression::remap_labels(
    const Mat& _labels_i, const std::map<int, int>& lmap) const {
  Mat labels;
  _labels_i.convertTo(labels, CV_32S);
