...
class Pose2d : public PoseBase<Pose2d, Transform2d> {
 public:
  ...
  //       IDs are *not* copied, but *are* moved. Copied names will have "_COPY"
  //       appended.
  ...
  RotationMatrix2d GetRotationMatrix() const {
    return GetTransform().GetRotationMatrix();
  }
  ...
  void ApplyTo(const Quadrilateral<2, T>& quadIn,
    ...
    GetTransform().ApplyTo(quadIn, quadOut);
  }
  ...
  // NOTE: This returns by value, not reference. Do not store references to its
  // members in line! I.e., never do:
  //   const Foo& = somePose.GetWithRespectToRoot().GetFoo(); // Foo will
  //   immediately be invalidated
  // Either store "somePoseWrtRoot" and then get Foo& from it, or make Foo a
  // copy.
  Pose2d GetWithRespectToRoot() const;

  void Print(const std::string& channel = "Pose",
             const std::string& eventName = "Pose3d.Print") const;
...
class Pose3d : public PoseBase<Pose3d, Transform3d> {
 public:
  ...
  // In that case, you MUST intialize the transformation (Rotation/Translation)
  ...
  Pose3d(const Transform3d& transform, const Pose3d& parentPose,
         const std::string& name = "");

  // Construct from generic rotation and translation vector
  Pose3d(const Rotation3d& R, const Vec3f& T, const std::string& name = "");
  Pose3d(const Rotation3d& R, const Vec3f& T, const Pose3d& parentPose,
         const std::string& name = "");

  // Construct from rotation vector and translation vector
  Pose3d(const RotationVector3d& Rvec, const Vec3f& T,
         const std::string& name = "");
  Pose3d(const RotationVector3d& Rvec, const Vec3f& T, const Pose3d& parentPose,
         const std::string& name = "");

  // Construct from rotation matrix and translation vector
  Pose3d(const RotationMatrix3d& Rmat, const Vec3f& T,
         const std::string& name = "");
  Pose3d(const RotationMatrix3d& Rmat, const Vec3f& T, const Pose3d& parentPose,
         const std::string& name = "");

  // Construct from an angle, axis, and translation vector
  Pose3d(const Radians& angle, const Vec3f& axis, const Vec3f& translation,
         const std::string& name = "");
  Pose3d(const Radians& angle, const Vec3f& axis, const Vec3f& translation,
  ...
  explicit Pose3d(const PoseStruct3d& poseStruct,
                  const PoseOriginList& originList);
  PoseStruct3d ToPoseStruct3d(const PoseOriginList& originList) const;

  // Accessors:
  //   Note: we return by value for Rvalues to avoid keeping references to
  //   temporary poses' members
  ...
  RotationVector3d GetRotationVector() const {
    return GetRotation().GetRotationVector();
  }
  ...
  Radians GetRotationAngle() const {
    return GetTransform().GetRotationAngle<AXIS>();
  }
  ...
  void SetRotation(const RotationMatrix3d& Rmat) {
    GetTransform().SetRotation(Rotation3d(Rmat));
  }
  void SetRotation(const RotationVector3d& Rvec) {
    GetTransform().SetRotation(Rotation3d(Rvec));
  }
  void SetRotation(const Radians angle, const Vec3f& axis) {
    GetTransform().SetRotation(Rotation3d(angle, axis));
  }
  ...
  void TranslateForward(T dist) {
    GetTransform().TranslateForward(dist);
  }
  ...
  void ApplyTo(const std::vector<Point<3, T> >& pointsIn,
    ...
    GetTransform().ApplyTo(pointsIn, pointsOut);
  }
  ...
  // * distThreshold is aligned to "this" pose's orientation, not the parent
  // frame!
  // * If distThreshold is same in all dimensions (e.g. if set from scalar
  // float),
  //   simple Euclidean distance is used.
  // * If requested, Tdiff and angleDiff will contain how different the two
  //   transformations are.
  bool IsSameAs(const Pose3d& P_other, const Point3f& distThreshold,
                const Radians& angleThreshold) const;

  bool IsSameAs(const Pose3d& P_other, const Point3f& distThreshold,
                const Radians& angleThreshold, Vec3f& Tdiff) const;
  ...
  // TODO: Provide IsSameAs_WithAmbiguity() wrappers that don't require
  // Tdiff/angleDiff outputs
...
// Calculate vector from pose2's translation to pose1's translation (rotations
// of the input poses are ignored), with the resulting vector expressed in the
// outputFrame (which can be any pose in the same tree as pose1 and pose2).
// returns true/false depending on whether poses (and output frame) are
// comparable (share root) stores result in outVector if the return value is
// true, untouched if false
...
// returns true/false depending on whether poses are comparable (share root)
// stores result in outDistanceSQ if the return value is true, untouched if
// false
...
inline bool Pose2d::GetWithRespectTo(const Pose2d& otherPose,
                                     Pose2d& newPoseWrtOtherPose) const {
  return PoseBase<Pose2d, Transform2d>::GetWithRespectTo(*this, otherPose,
                                                         newPoseWrtOtherPose);
}
...
inline Pose2d Pose2d::GetWithRespectToRoot() const {
  ...
  if (this->IsRoot()) {
    poseWrtOrigin = *this;
  } else if (PoseBase<Pose2d, Transform2d>::GetWithRespectTo(
                 *this, this->FindRoot(), poseWrtOrigin) == false) {
    PRINT_NAMED_ERROR(
        "Pose2d.GetWithRespectToRoot.Failed",
...
inline bool Pose3d::GetWithRespectTo(const Pose3d& otherPose,
                                     Pose3d& newPoseWrtOtherPose) const {
  return PoseBase<Pose3d, Transform3d>::GetWithRespectTo(*this, otherPose,
                                                         newPoseWrtOtherPose);
}
...
inline Pose3d Pose3d::GetWithRespectToRoot() const {
  ...
  if (this->IsRoot()) {
    poseWrtOrigin = *this;
  } else if (PoseBase<Pose3d, Transform3d>::GetWithRespectTo(
                 *this, this->FindRoot(), poseWrtOrigin) == false) {
    PRINT_NAMED_ERROR(
        "Pose3d.GetWithRespectToRoot.Failed",
        "Could not get pose w.r.t. its own root. This should never happen.");
...
inline bool Pose3d::IsSameAs_WithAmbiguity(
    const Pose3d& P_other, const RotationAmbiguities& R_ambiguities,
    const Point3f& distThreshold, const Radians& angleThreshold) const {
  Vec3f Tdiff;
  Radians angleDiff;
