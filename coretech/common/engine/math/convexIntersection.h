 ...
 * Created: 2018-06-15
 *
 * Description: Defines an the intersection of an arbitrary number of bounded
 *convex sets S₁ ⋂ S₂ ⋂ ... ⋂ Sₓ  = { p ∈ ℝⁿ | p ∈ S₁, p ∈ S₂, ..., p ∈ Sₓ }
 *
...
template <DimType N, typename T, class Set1, class Set2,
          typename = typename std::enable_if<(
              std::is_base_of<PointSet<N, T>, Set1>::value &&
              std::is_base_of<PointSet<N, T>, Set2>::value)>::type>
 ...
 public:
  ...
  virtual AxisAlignedHyperCube<N, T> GetAxisAlignedBoundingBox()
    ...
    for (DimType dim = 0; dim < N; ++dim) {
      if (s2BoundingBox.GetMinVertex()[dim] > min[dim]) {
        min[dim] = s2BoundingBox.GetMinVertex()[dim];
      }
      if (s2BoundingBox.GetMaxVertex()[dim] < max[dim]) {
        max[dim] = s2BoundingBox.GetMaxVertex()[dim];
      }
    }
...
// helper for making intersections of multiple sets via recursion: (S₁ ⋂ (S₂ ⋂
// (S₃ ⋂ (... ⋂ Sₓ))))
template <DimType N, typename T, class Set>
Set MakeIntersection(Set v) {
  return v;
};
