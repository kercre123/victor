 ...
 *
 *
 * Description: Implements the internal tree node for poses, which contains a 2D
 *or 3D transform, a parent pointer, a name, and an ID.
 *
...
// Make a separate define so we can easily dissociate "Dev" pose checks from
// debug/release/shipping decisions (i.e., we may want to switch back to doing
// them only in Debug)
...
class PoseBase<PoseNd, TransformNd>::PoseTreeNode {
 public:
  PoseTreeNode(const TransformNd& tform,
  ...
  const TransformNd& GetParentTransform() const {
    return _parentPtr->GetTransform();
  }
  ...
  // Register/unregister as an "owner" of this node: i.e. that this node is
  // "wrapped" by one or more PoseBase objects (as opposed to simply having a
  // shared_ptr hanging on to it somewhere)
 ...
 private:
  ...
  // asserts that the parent is valid and that the child is a current valid
  // child of the parent
  static void Dev_AssertIsValidParentPointer(
      const PoseTreeNode* parent, const PoseTreeNode* childBasePointer);

  // updates children bookkeeping variables for old and new parents
  static void Dev_SwitchParent(const PoseTreeNode* oldParent,
                               const PoseTreeNode* newParent,
                               const PoseTreeNode* childBasePointer);
  ...
  // funny note: because plugins create PoseNds on load/creation, we can't rely
  // on static variables being initialized, so we need all static variables here
  // to be created on demand, which should be guaranteed by using static local
  // variables returned from a method. I say funny because now it's funny, but
  // believe it was not funny before.
  ...
  // bookkeeping variables to ensure parent pointer validity. They should be
  // empty in release. During debug, however, parents keep track of their
  // children so that we can validate proper parenthood
...
PoseBase<PoseNd, TransformNd>::PoseTreeNode::PoseTreeNode(
    const TransformNd& tform, const std::shared_ptr<PoseTreeNode>& parent,
    const std::string& name)
    : _transform(tform), _name(name) {
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
typename PoseBase<PoseNd, TransformNd>::PoseTreeNode&
PoseBase<PoseNd, TransformNd>::PoseTreeNode::operator=(
    const PoseTreeNode& other) {
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
bool PoseBase<PoseNd, TransformNd>::PoseTreeNode::IsChildOf(
    const PoseTreeNode& otherNode) const {
  if (IsRoot()) {
    return false;
  }
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
bool PoseBase<PoseNd, TransformNd>::PoseTreeNode::IsParentOf(
    const PoseTreeNode& otherNode) const {
  if (otherNode.IsRoot()) {
    return false;
  }
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
bool PoseBase<PoseNd, TransformNd>::PoseTreeNode::HasSameParentAs(
    const PoseTreeNode& other) const {
  // A node has the same parent as itself, even if it is a root
  if (this == &other) {
    return true;
  }

  // Otherwise, a root cannot have the same parent as anything else, by
  // definition
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
bool PoseBase<PoseNd, TransformNd>::PoseTreeNode::HasSameRootAs(
    const PoseTreeNode& other) const {
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
void PoseBase<PoseNd, TransformNd>::PoseTreeNode::SetParent(
    const std::shared_ptr<PoseTreeNode>& newParent) {
  DEV_ASSERT(newParent.get() != this,
             "PoseBase.PoseTreeNode.SetParent.ParentCannotBeSelf");
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
const std::shared_ptr<typename PoseBase<PoseNd, TransformNd>::PoseTreeNode>&
PoseBase<PoseNd, TransformNd>::PoseTreeNode::GetParent() const {
  Dev_AssertIsValidParentPointer(_parentPtr.get(), this);
  return _parentPtr;
}
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
const typename PoseBase<PoseNd, TransformNd>::PoseTreeNode*
PoseBase<PoseNd, TransformNd>::PoseTreeNode::GetRawParentPtr() const {
  Dev_AssertIsValidParentPointer(_parentPtr.get(), this);
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
std::shared_ptr<typename PoseBase<PoseNd, TransformNd>::PoseTreeNode>
PoseBase<PoseNd, TransformNd>::PoseTreeNode::FindRoot() const {
  ...
  BOUNDED_WHILE(1000, !root->IsRoot()) {
    ...
    DEV_ASSERT(root->_parentPtr != nullptr,
               "PoseBase.PoseTreeNode.FindRoot.RootHasNullParent");
    root = root->GetParent();
  }
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
std::string PoseBase<PoseNd, TransformNd>::PoseTreeNode::GetNamedPathToRoot(
    bool showTranslations) const {
  std::string str;

  const PoseCPtrSet* validPoses =
      (ANKI_DEV_CHEATS ? &Dev_GetValidPoses() : nullptr);
  ...
  BOUNDED_WHILE(1000, true) {
    if (DO_DEV_POSE_CHECKS) {
      ...
      if (validPoses->find(current) == validPoses->end()) {
        // Stop as soon as we reach an invalid pose in the chain b/c it may be a
        // garbage pointer
        str += "[INVALID]";
        return str;
      }
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
void PoseBase<PoseNd, TransformNd>::PoseTreeNode::Dev_SwitchParent(
    const PoseTreeNode* oldParent, const PoseTreeNode* newParent,
  ...
  if (DO_DEV_POSE_CHECKS) {
    ...
    if (oldParent) {
      // if the parent is not a valid pose anymore it's not an error, we had a
      // bad pointer but we are not using it, we are actually switching to a
      // different one
    ...
    if (newParent) {
      ...
      if (match != validPoses.end()) {
        ...
        ANKI_VERIFY(
            newParent->_devChildrenPtrs.find(child) ==
                newParent->_devChildrenPtrs.end(),
            ...
            oldParent,
            oldParent == nullptr ? "(none)"
                                 : oldParent->GetNamedPathToRoot(true).c_str(),
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
void PoseBase<PoseNd, TransformNd>::PoseTreeNode::
    Dev_AssertIsValidParentPointer(const PoseTreeNode* parent,
                                   const PoseTreeNode* child) {
  // Note: (for now?) *always* doing this cheap check, even in shipping, but
  // only aborting in Debug/Release
  if (parent != nullptr &&
      !PoseBase<PoseNd, TransformNd>::AreUnownedParentsAllowed()) {
    if (!ANKI_VERIFY(parent->_ownerCount > 0,
                     "PoseBase.Dev_AssertIsValidParentPointer.UnownedParent",
                     "Pose %d(%s) has parent %d(%s) which is not owned by any "
                     "PoseBase wrapper",
                     child->_id, child->_name.c_str(), parent->_id,
                     parent->_name.c_str())) {
      if (DO_DEV_POSE_CHECKS) {
        Anki::Util::sLogFlush();
        Anki::Util::sAbort();
  ...
  if (DO_DEV_POSE_CHECKS) {
    ...
    PoseCPtrSet& validPoses = Dev_GetValidPoses();
    if (!ANKI_VERIFY(
            validPoses.find(child) != validPoses.end(),
            "PoseBase.Dev_AssertIsValidParentPointer.ChildNotAValidPose",
            "This pose is bad. Not printing info because it's garbage, so I "
            "don't want to access it")) {
      Anki::Util::sLogFlush();
      Anki::Util::sAbort();
    }
    ...
    if (nullptr != parent) {
      ...
      if (!ANKI_VERIFY(
              validPoses.find(parent) != validPoses.end(),
              "PoseBase.Dev_AssertIsValidParentPointer.ParentNotAValidPose",
              "Path of parent '%s'(%p) to root: %s. Path of child '%s'(%p) to "
              "root: %s",
              parent->GetName().c_str(), parent,
              parent->GetNamedPathToRoot(true).c_str(),
              child == nullptr ? "(none)" : child->GetName().c_str(), child,
              child == nullptr ? "(none)"
                               : child->GetNamedPathToRoot(true).c_str())) {
        Anki::Util::sLogFlush();
        Anki::Util::sAbort();
      }
      ...
      if (!ANKI_VERIFY(
              parent->_devChildrenPtrs.find(child) !=
                  parent->_devChildrenPtrs.end(),
      ...
      // Note on c): If you crash on c), it means that the child is pointing at
      // this parent, and this parent is indeed a valid pose, but it is not a
      // valid parent of this child. This can happen by this series of steps:
      // Create pose A
      // Create pose B
      // Set A as B's parent
      ...
      // -> In that case B's pointer says that it's a valid pose, however is not
      // a valid child of C, B did not intend to point to C, and using B's
      // parent pointer is an error
    }
  }
}
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
void PoseBase<PoseNd, TransformNd>::PoseTreeNode::Dev_PoseDestroyed(
    const PoseTreeNode* nodePtr) {
  if (DO_DEV_POSE_CHECKS) {
    std::lock_guard<std::mutex> lock(Dev_GetMutex());
    ...
    ANKI_VERIFY(removedCount == 1,
                "PoseBase.Dev_PoseDestroyed.DestroyingInvalidPose",
                "Path from '%s'(%p) to root: %s", nodePtr->GetName().c_str(),
                nodePtr, nodePtr->GetNamedPathToRoot(true).c_str());
  }
}
...
// - - - - - - - - - - - - - - - - - - -
template <class PoseNd, class TransformNd>
void PoseBase<PoseNd, TransformNd>::PoseTreeNode::Dev_PoseCreated(
    const PoseTreeNode* nodePtr) {
  if (DO_DEV_POSE_CHECKS) {
    std::lock_guard<std::mutex> lock(Dev_GetMutex());
    ...
    ANKI_VERIFY(insertRetInfo.second,
                "PoseBase.Dev_PoseCreated.CreatingDuplicatedPointer",
                "Path from '%s'(%p) to root: %s", nodePtr->GetName().c_str(),
                nodePtr, nodePtr->GetNamedPathToRoot(true).c_str());
  }
}
...
inline void PoseBase<PoseNd, TransformNd>::PoseTreeNode::RemoveOwner() {
  if (ANKI_VERIFY(_ownerCount > 0,
                  "PoseBase.PoseTreeNode.RemoveOwner.ZeroOwners", "")) {
    --_ownerCount;

    // COZMO-10891: We can probably remove this once this ticket is figured out.
    // In our usage, only PoseOrigins in the PoseOriginList have IDs and are
    // named OriginNN. Those origins should never get deleted, so they should
    // always be owned. Flag if this is not the case.
    if (_ownerCount == 0 && !AreUnownedParentsAllowed()) {
      ANKI_VERIFY(
          GetID() == 0, "PoseBase.PoseTreeNode.RemoveOwner.ZeroOwnersWithID",
