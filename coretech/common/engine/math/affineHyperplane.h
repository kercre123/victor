...
class Halfplane : public AffineHyperplane<N, T> {
 public:
  ...
  // return a'x + b > 0
  virtual bool Contains(const Point<N, T>& x) const override {
    return (FLT_GT(this->Evaluate(x), 0.f));
  }
};
...
inline bool GetIntersectionPoint(const Line2f& l1, const Line2f& l2,
                                 Point2f& p) {
