...
class LineSegment : public Line2f {
 public:
  ...
  // check if the point is Co-linear with line segment and constrained by start
  // and end points
  ...
  // check if two line segments intersect and return the intersection in
  // location. returns true is there's a single point of intersection
  bool IntersectsAt(const LineSegment& l, Point2f& location) const;

  // Create a Line perpendicular to the line segment, intersecting at the
  // midpoint
