...
class Ball : public BoundedConvexSet<N, T> {
 public:
  ...
  Ball(const Point<N, T>& p = Point<N, T>(T(0)), const T& r = T(1))
      : _p(p), _r(ABS(r)), _rSq(r * r) {}
  ...
  virtual bool Intersects(
      const AxisAlignedHyperCube<N, T>& cube) const override {
    if (Contains(cube.GetCentroid())) {
      return true;
    }

    // get the closest point in the ball to the hypercube center
    Point<N, T> vec = cube.GetCentroid() - GetCentroid();
    vec.MakeUnitLength();  // this needs to be on its own line, since it returns
                           // the original length, not a point.
