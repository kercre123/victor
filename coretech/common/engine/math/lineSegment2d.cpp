 ...
 * Created: 2017-10-12
 *
 * Description: A class that holds a 2d line segment for fast intersection
 *checks in point-point form: Δy(x - x₁) - Δx(y - y₁) = 0
 *
...
LineSegment::LineSegment(const Point2f& from, const Point2f& to)
    : AffineHyperplane(
          Point2f(from.y() - to.y(), to.x() - from.x()),
          (from.x() - to.x()) * to.y() + (to.y() - from.y()) * to.x()),
      from(from),
      to(to),
...
bool LineSegment::IntersectsWith(const LineSegment& l) const {
  ...
  if (o1 ==
      o2)  // both points of test segment are on the same side of this line
  {
    return false;
  }
  ...
  if (o3 ==
      o4)  // both points of this segment are on the same side of test line
  {
    return false;
  }
...
LineSegment::EOrientation LineSegment::Orientation(const Point2f& p) const {
  ...
  if (NEAR_ZERO(val)) {
    return EOrientation::COLINEAR;  // point is on the line generated by the
                                    // segment
...
bool LineSegment::IntersectsAt(const LineSegment& l, Point2f& location) const {
  // shortcutting will stop GetInterSectionPoint from running if the
  // line-segments do not intersect, even if the infinite lines defined by the
  // two points do. If you want the intersection of the infinite lines, call
  // `GeGetIntersectionPoint` directly.
