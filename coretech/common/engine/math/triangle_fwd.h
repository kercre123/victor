...
//        To get the implementations, you must include triangle.h.
//
...
class Triangle : public std::array<Point<2, T>, 3> {
 public:
  ...
  Triangle(const Point2<T>& point1, const Point2<T>& point2,
  ...
  // Calculates centroid of the triangle (barycenter), which always divides each
  // median in a 2:1 ratio, or in other words, it's always 2/3 away from each
  // vertex
