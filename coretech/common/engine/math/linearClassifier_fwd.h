 ...
 *
 * Computes the log ratio of positive to negative classes as a linear (dot)
 * product with a bias term. Uses single precision floating point and provides
 * methods for interpreting output in various ways.
 *
 * Parameters are loaded through JSON, typically as a subfield from the owning
 * module or class's config.
 *
 * Classify methods are templatized to operate on any []-iterable object, and
 * return NaN when the input size is incorrect.
 * TODO Extend this to other generic types, possibly switch to an actual linear
 * algebra library
 *
 * Note that there are template implementations in linearClassifier.h which must
 * be included in your source files for compilation.
 */
