...
Pose2d::Pose2d() : Pose2d(0, {0.f, 0.f}) {}
...
Pose2d::Pose2d(const Pose3d& pose3d)
    : PoseBase<Pose2d, Transform2d>(Transform2d(
          pose3d.GetRotationAngle<'Z'>(),
          {pose3d.GetTranslation().x(), pose3d.GetTranslation().y()})),
      _planeNormal(Z_AXIS_3D()) {}

void Pose2d::Print(const std::string& channel,
                   const std::string& eventName) const {
  PRINT_CH_INFO(channel.c_str(), eventName.c_str(),
                "Pose2d%s%s: Translation=%s, RotAng=%frad (%.1fdeg), %s",
                GetName().empty() ? "" : " ", GetName().c_str(),
                GetTranslation().ToString().c_str(), GetAngle().ToFloat(),
                GetAngle().getDegrees(), GetParentString().c_str());
}
...
Pose3d::Pose3d(const std::string& name)
    : Pose3d(0, Z_AXIS_3D(), {0.f, 0.f, 0.f}, name) {}  // Constructor: Pose3d()

Pose3d::Pose3d(const Transform3d& transform, const std::string& name)
    : PoseBase<Pose3d, Transform3d>(transform, name) {}

Pose3d::Pose3d(const Transform3d& transform, const Pose3d& parentPose,
               const std::string& name)
    : PoseBase<Pose3d, Transform3d>(transform, parentPose, name) {}

Pose3d::Pose3d(const Rotation3d& R, const Vec3f& T, const std::string& name)
    : PoseBase<Pose3d, Transform3d>(Transform3d(R, T), name) {}

Pose3d::Pose3d(const Rotation3d& R, const Vec3f& T, const Pose3d& parentPose,
               const std::string& name)
    : PoseBase<Pose3d, Transform3d>(Transform3d(R, T), parentPose, name) {}

Pose3d::Pose3d(const RotationVector3d& Rvec, const Vec3f& T,
               const std::string& name)
    : Pose3d(Rotation3d(Rvec), T, name) {}

Pose3d::Pose3d(const RotationVector3d& Rvec, const Vec3f& T,
               const Pose3d& parentPose, const std::string& name)
    : Pose3d(Rotation3d(Rvec), T, parentPose, name) {}

Pose3d::Pose3d(const RotationMatrix3d& Rmat, const Vec3f& T,
               const std::string& name)
...
Pose3d::Pose3d(const Radians& angle, const Vec3f& axis, const Vec3f& T,
               const std::string& name)
...
Pose3d::Pose3d(const PoseStruct3d& poseStruct, const PoseOriginList& originList)
    : Pose3d(Rotation3d(UnitQuaternion(poseStruct.q0, poseStruct.q1,
                                       poseStruct.q2, poseStruct.q3)),
...
bool Pose3d::IsSameAs(const Pose3d& P_other, const Point3f& distThreshold,
                      const Radians& angleThreshold, Vec3f& T_diff,
                      Radians& angleDiff) const {
  static const RotationAmbiguities kNoAmbiguities;

  return IsSameAs_WithAmbiguity(P_other, kNoAmbiguities, distThreshold,
                                angleThreshold, T_diff, angleDiff);
...
bool Pose3d::IsSameAs_WithAmbiguity(const Pose3d& P_other_in,
                                    ...
                                    const Radians& angleThreshold, Vec3f& Tdiff,
                                    Radians& angleDiff) const {
  DEV_ASSERT(distThreshold.AllGTE(0.f),
             "Pose3d.IsSameAs_WithAmbiguity.NegativeDistThreshold");

  DEV_ASSERT(angleThreshold.ToFloat() >= 0.f,
  ...
  if (this->IsChildOf(P_other_in)) {
    ...
    if (P_other_in.GetWithRespectTo(*this, P_other) == false) {
      PRINT_NAMED_WARNING(
          "Pose3d.IsSameAs.PosesHaveDifferentOrigins",
  ...
  // are comparable
    ...
    //                      "Could not get other object w.r.t. this object's
    //                      parent. Returning isSame == false.");
  ...
  */
  ...
  // Problem with the above, seemingly-simple check: rotation affects the
  // translation comparison. So if it is large, it will make the translations
  // appear different _even if we have a large angle threshold_. Instead, we
  // choose to just directly compare the translations, followed by comparing the
  // rotations.
  ...
  if (Util::IsFltNear(distThreshold.x(), distThreshold.y()) &&
    ...
    // NOTE: using > instead of >= so threshold of zero will still return true
    // for identical poses
  ...
  // NOTE: using <= instead of < so threshold of zero still will still return
  // true for identical poses
  ...
  if (R_ambiguities.HasAmbiguities()) {
    // Need to consider ambiguities. Compute the rotation difference between the
    // two poses' rotations. See if that difference is one of the ambiguities we
    // should ignore.
...
void Pose3d::Print(const std::string& channel,
                   const std::string& eventName) const {
  PRINT_CH_INFO(channel.c_str(), eventName.c_str(),
                "Pose%s%s: Translation=%s, RotVec=(%f, %f, %f), RotAng=%frad "
                "(%.1fdeg), %s",
                GetName().empty() ? "" : " ", GetName().c_str(),
                GetTranslation().ToString().c_str(), GetRotationAxis().x(),
                GetRotationAxis().y(), GetRotationAxis().z(),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool ComputeVectorBetween(const Pose3d& pose1, const Pose3d& pose2,
                          const Pose3d& outputFrame, Vec3f& outVector) {
  ...
  outVector = (pose1wrtOutputFrame.GetTranslation() -
               pose2wrtOutputFrame.GetTranslation());
  return true;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool ComputeDistanceBetween(const Pose3d& pose1, const Pose3d& pose2,
                            f32& outDistance) {
  Pose3d pose1Wrt2;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool ComputeDistanceSQBetween(const Pose3d& pose1, const Pose3d& pose2,
                              f32& outDistanceSQ) {
  Pose3d pose1Wrt2;
  if (!pose1.GetWithRespectTo(pose2, pose1Wrt2)) {
    return false;
