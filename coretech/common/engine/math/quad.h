 ...
 *
 *
 * Description: Implements templated methods for a general N-dimensional
 *Quadrilateral class for holding four points in arbitrary positions.
 *
...
template <QuadDimType N, typename T>
Quadrilateral<N, T>& Quadrilateral<N, T>::operator+=(
    const Quadrilateral<N, T>& quad2) {
  using namespace Quad;
  for (CornerName i = FirstCorner; i < NumCorners; ++i) {
    (*this)[i] += quad2[i];
  }
...
Quadrilateral<N, T>& Quadrilateral<N, T>::Scale(const T scaleFactor) {
  ...
  for (CornerName i_corner = FirstCorner; i_corner < NumCorners; ++i_corner) {
...
template <QuadDimType N, typename T>
inline const Point<N, T>& Quadrilateral<N, T>::operator[](
    const Quad::CornerName whichCorner) const {
  // return this->corners[whichCorner];
  return std::array<Point<N, T>, 4>::operator[](static_cast<int>(whichCorner));
}

template <QuadDimType N, typename T>
inline Point<N, T>& Quadrilateral<N, T>::operator[](
    const Quad::CornerName whichCorner) {
  // return this->corners[whichCorner];
  return std::array<Point<N, T>, 4>::operator[](static_cast<int>(whichCorner));
}
...
Quadrilateral<2, T> Quadrilateral<N, T>::SortCornersClockwise() const {
  ...
  static_assert(N == 2,
                "No normal should be supplied when sorting a 2D "
  ...
  for (CornerName i = FirstCorner; i < NumCorners; ++i) {
    ...
    angleIndexPairs[i].first =
        atan2f(static_cast<f32>(corner.y()), static_cast<f32>(corner.x()));
  ...
  std::sort(angleIndexPairs.begin(), angleIndexPairs.end(),
            CompareFirst<f32, Quad::CornerName>());

  // Re-order the corners using the sorted indices
  const Quadrilateral<2, T> sortedQuad(
      (*this)[angleIndexPairs[0].second],  // TopLeft    (min angle)
...
template <QuadDimType N, typename T>
Quadrilateral<N, T> Quadrilateral<N, T>::SortCornersClockwise(
    const Point<N, T>& aroundNormal) const {
  // Catch invalid usage of this method with a 2D quad at compile time.
  static_assert(
      N != 2,
      "A normal must be supplied when sorting an ND Quadrilateral's corners.");
  ...
  const Quadrilateral<N, T> sortedQuad(
      (*this)[Quad::FirstCorner], (*this)[angleIndexPairs[0].second],
      (*this)[angleIndexPairs[1].second], (*this)[angleIndexPairs[2].second]);
...
Quadrilateral<2, T> GetBoundingQuad(std::vector<Point<2, T> >& points) {
  ...
  try {
    ...
    PRINT_NAMED_ERROR("GetBoundingQuad.CvMinAreaRectFailed.COZMO-1916",
                      "cvPoints.size=%zu%s", cvPoints.size(),
                      pointsStr.c_str());
  ...
  Quadrilateral<2, T> boundingQuad(
      Point<2, T>(static_cast<T>(cvQuad[0].x), static_cast<T>(cvQuad[0].y)),
...
bool Quadrilateral<N, T>::Contains(const Point<2, T>& point) const {
  ...
  std::array<const Point<N, T>*, 3> triangle = {
      {&this->operator[](TopLeft), &this->operator[](BottomLeft),
       &this->operator[](BottomRight)}};
...
template <QuadDimType N, typename T>
bool Quadrilateral<N, T>::Intersects(const Point2<T>& rayFrom,
                                     const Point2<T>& rayTo,
                                     Point2<T>* outIntersectionPoint) const {
  using namespace Quad;
  ...
  for (int i = 0; i < 4; ++i) {
    kmVec2 interPoint;
    const kmBool segmentIntersects = kmSegment2WithSegmentIntersection(
        &thisSegments[i], &testRay, &interPoint);
    if (segmentIntersects) {
      if (outIntersectionPoint) {
        (*outIntersectionPoint) = Point2<T>(interPoint.x, interPoint.y);
      }
      return true;
    }
  }
...
bool Quadrilateral<N, T>::Intersects(const Quadrilateral<2, T>& other) const {
  ...
  // check if segments intersect to fix this case, where no corner is contained
  // within the other quad
  //              __
  //             |  |
  //           __|__|__
  //          |  |  |  |
  ...
  //             |__|
  //
  // rsam: I think this should be enough in addition to the corner checks. There
  // are some optimizations that can be made (for example checking 3 sides
  // should be enough, only corners from the smaller quad could be checked,
  // etc), but I don't foresee this to be a bottleneck atm, and I want to err on
  // the safe side Also, I'm relying on kazMath to not reinvent the wheel at
  // this moment
  {
    using namespace Quad;
    ...
    for (int i = 0; i < 4; ++i) {
      for (int j = 0; j < 4; ++j) {
        kmVec2 interPoint;
        const kmBool segmentIntersects = kmSegment2WithSegmentIntersection(
            &thisSegments[i], &otherSegments[j], &interPoint);
        if (segmentIntersects) {
          return true;
        }
      }
