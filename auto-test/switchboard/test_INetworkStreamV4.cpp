...
Test_INetworkStreamV4::Test_INetworkStreamV4()
  ...
  _cladHandler->OnReceiveRtsMessage().SubscribeForever(std::bind(
      &Test_INetworkStreamV4::ReceiveMessage, this, std::placeholders::_1));
}
...
void Test_INetworkStreamV4::ReceiveMessage(
...
void Test_INetworkStreamV4::Update() {
    ...
    switch (msg.GetTag()) {
      ...
      case Anki::Vector::ExternalComms::RtsConnection_4Tag::RtsNonceMessage: {
        ...
        SendRtsMessage<Vector::ExternalComms::RtsAck>(
            (uint8_t)Anki::Vector::ExternalComms::RtsConnection_4Tag::
                RtsNonceMessage);
  ...
  if (_connected && !_reconnect) {
    ...
    _securePairing = new RtsComms(
        ...
        nullptr,
        nullptr,  // tokenClient
        nullptr, wifiWatcher, taskExecutor,
        false,   // is pairing
        false,   // is ota-ing
...
//
//
int Test_INetworkStreamV4::ClientEncrypt(uint8_t* buffer, int length,
                                         uint8_t* output,
...
int Test_INetworkStreamV4::ClientDecrypt(uint8_t* buffer, int length,
                                         uint8_t* output,
                                         uint64_t* outputLength) {
  // decrypt message
