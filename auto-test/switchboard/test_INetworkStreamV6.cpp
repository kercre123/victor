...
Test_INetworkStreamV6::Test_INetworkStreamV6() :
...
_reconnect(false) {
  _cladHandler = new CladHandlerV6();
  _cladHandler->OnReceiveRtsMessage().SubscribeForever(std::bind(&Test_INetworkStreamV6::ReceiveMessage, this, std::placeholders::_1));
}
...
void Test_INetworkStreamV6::ReceiveMessage(const Anki::Vector::ExternalComms::RtsConnection_6& msg) {
  _queue.push(msg);
}

int Test_INetworkStreamV6::SendPlainText(uint8_t* bytes, int length) {
  // Received a message
...
int Test_INetworkStreamV6::SendEncrypted(uint8_t* bytes, int length) {
...
void Test_INetworkStreamV6::Update() {
  // instance tick
  ...
  while(_queue.size() > 0) {
    Anki::Vector::ExternalComms::RtsConnection_6 msg = _queue.front();
    _queue.pop();

    switch(msg.GetTag()) {
      case Anki::Vector::ExternalComms::RtsConnection_6Tag::RtsConnRequest_6: {
        Anki::Vector::ExternalComms::RtsConnRequest_6 m = msg.Get_RtsConnRequest_6();
      ...
      case Anki::Vector::ExternalComms::RtsConnection_6Tag::RtsNonceMessage: {
        ...
        SendRtsMessage<Vector::ExternalComms::RtsAck>
          ((uint8_t)Anki::Vector::ExternalComms::RtsConnection_6Tag::RtsNonceMessage);
...
//
//
int Test_INetworkStreamV6::ClientEncrypt(uint8_t* buffer, int length, uint8_t* output, uint64_t* outputLength) {  
  // encrypt message
...
int Test_INetworkStreamV6::ClientDecrypt(uint8_t* buffer, int length, uint8_t* output, uint64_t* outputLength) {
  // decrypt message
...
void TickV6(struct ev_loop* loop, struct ev_timer* w, int revents) {  
  Test_INetworkStreamV6* stream = (Test_INetworkStreamV6*)w->data;
...
void TimeoutV6(struct ev_loop* loop, struct ev_timer* w, int revents) {  
  Test_INetworkStreamV6* stream = (Test_INetworkStreamV6*)w->data;
...
void Test_INetworkStreamV6::Test(RtsComms* securePairing) {
  ev_timer t;
  t.data = this;
  ev_timer_init(&t, TickV6, 0.1f, 0.1f);
  ...
  ev_timer timeout;
  timeout.data = this;
  ev_timer_init(&timeout, TimeoutV6, 3, 0);
...
void Test_INetworkStreamV6::SendRtsMessage(Args&&... args) {
  Anki::Vector::ExternalComms::ExternalComms msg = Anki::Vector::ExternalComms::ExternalComms(
    Anki::Vector::ExternalComms::RtsConnection(Anki::Vector::ExternalComms::RtsConnection_6(T(std::forward<Args>(args)...))));
