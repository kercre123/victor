...
Test_INetworkStream::Test_INetworkStream()
  ...
  _cladHandler->OnReceiveRtsMessage().SubscribeForever(std::bind(
      &Test_INetworkStream::ReceiveMessage, this, std::placeholders::_1));
}
...
void Test_INetworkStream::ReceiveMessage(
...
int Test_INetworkStream::SendPlainText(uint8_t* bytes, int length) {
  ...
  if (_state == MessageState::TestRaw) {
    if ((SetupMessage)bytes[0] == SetupMessage::MSG_HANDSHAKE) {
      ...
      ASSERT((hostVersion == SB_PAIRING_PROTOCOL_VERSION),
             "Host version wrong.");
...
int Test_INetworkStream::SendEncrypted(uint8_t* bytes, int length) {
  ...
  int status =
      ClientDecrypt(outBytes, (int)outLength, decryptedBytes, &decryptedLength);

  ASSERT(status == 0, "Client could not decrypt message.");

  // Received a secure message
  _cladHandler->ReceiveExternalCommsMsg(decryptedBytes,
...
void Test_INetworkStream::Update() {
  ...
  while (_queue.size() > 0) {
    Anki::Vector::ExternalComms::RtsConnection_2 msg = _queue.front();
    _queue.pop();

    switch (msg.GetTag()) {
      case Anki::Vector::ExternalComms::RtsConnection_2Tag::RtsConnRequest: {
        Anki::Vector::ExternalComms::RtsConnRequest m =
            msg.Get_RtsConnRequest();
        ...
        std::copy(publicKeyPtr, publicKeyPtr + crypto_kx_PUBLICKEYBYTES,
                  publicKey.begin());
      ...
      case Anki::Vector::ExternalComms::RtsConnection_2Tag::RtsNonceMessage: {
        Anki::Vector::ExternalComms::RtsNonceMessage m =
            msg.Get_RtsNonceMessage();

        _keyExchange->CalculateSharedKeysClient(
            (const uint8_t*)_securePairing->GetPin().c_str());

        std::copy(m.toRobotNonce.begin(), m.toRobotNonce.end(), _EncryptNonce);

        std::copy(m.toDeviceNonce.begin(), m.toDeviceNonce.end(),
                  _DecryptNonce);
        ...
        SendRtsMessage<Vector::ExternalComms::RtsAck>(
            (uint8_t)Anki::Vector::ExternalComms::RtsConnection_2Tag::
                RtsNonceMessage);
      ...
      case Anki::Vector::ExternalComms::RtsConnection_2Tag::
          RtsChallengeMessage: {
        Anki::Vector::ExternalComms::RtsChallengeMessage m =
...
//
//
int Test_INetworkStream::ClientEncrypt(uint8_t* buffer, int length,
                                       uint8_t* output,
                                       uint64_t* outputLength) {
  // encrypt message
  int result = crypto_aead_xchacha20poly1305_ietf_encrypt(
      output, outputLength, buffer, length, nullptr, 0, nullptr, _EncryptNonce,
      _EncryptKey);

  if (result == 0) {
    // increment nonce if successful (don't let attacker derail our comms)
    sodium_increment(_EncryptNonce,
...
int Test_INetworkStream::ClientDecrypt(uint8_t* buffer, int length,
                                       uint8_t* output,
                                       uint64_t* outputLength) {
  // decrypt message
  int result = crypto_aead_xchacha20poly1305_ietf_decrypt(
      output, outputLength, nullptr, buffer, length, nullptr, 0, _DecryptNonce,
      _DecryptKey);

  if (result == 0) {
    // increment nonce if successful (don't let attacker derail our comms)
    sodium_increment(_DecryptNonce,
                     crypto_aead_xchacha20poly1305_ietf_NPUBBYTES);
  }
...
void Test_INetworkStream::SendRtsMessage(Args&&... args) {
  Anki::Vector::ExternalComms::ExternalComms msg =
      Anki::Vector::ExternalComms::ExternalComms(
          Anki::Vector::ExternalComms::RtsConnection(
              Anki::Vector::ExternalComms::RtsConnection_2(
  ...
  if (_state == MessageState::TestClad) {
    ...
    ClientEncrypt(messageData.data(), packedSize, encryptedBuffer,
                  &encryptedSize);
