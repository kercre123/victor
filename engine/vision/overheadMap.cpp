 ...
 * Created: 10/9/17
 *
 * Description: This class maintains an overhead map of the robot's surrounding.
 *The map has images taken from the ground plane in front of the robot.
 *
...
#define DEBUG_SAVE_OVERHEAD \
  0  // how many frames before saving the files, 0 means no saving

namespace {

const char* kLogChannelName = "VisionSystem";

// In some circumstances the bounding box is slightly larger than the one
// returned by stock openCV. Need to test this function, I'm not sure it's
// working 100%
__attribute__((used)) cv::Rect OptimizedBoundingBox(
...
Result OverheadMap::Update(
  ...
  if (!poseData.groundPlaneVisible) {
    PRINT_CH_DEBUG(kLogChannelName, "OverheadMap.Update.Groundplane",
  ...
  try {
    ...
    PRINT_NAMED_ERROR("OverheadMap.Update.ExceptionGetImageQuad",
                      "Error while getting the image quad: %s", e.what());
    return RESULT_FAIL;
  }

  // TODO a map could have a resolution lower than 1mm per pixel. In that case
  // we need a different way to index elements here

  // For each point on the ground plane, project it back into the image to get
  // the color Then add it to the overhead image
  s32 pointsInMap = 0;
  for (s32 i = 0; i < roi.GetWidthFar(); ++i) {
    const u8* mask_i = roi.GetOverheadMask().GetRow(i);
    // i is an index, but given that in the overhead mask 1 pixel = 1mm, i is
    // also a distance
    const f32 ground_y = static_cast<f32>(i) -
                         0.5f * roi.GetWidthFar();  // Zero is at the center
    for (s32 j = 0; j < roi.GetLength(); ++j) {
      ...
      if (x_img >= 0 && y_img >= 0 && x_img < image.GetNumCols() &&
          y_img < image.GetNumRows()) {
        const Vision::PixelRGB value = image(y_img, x_img);

        // Get corresponding map point in world coords
        Point3f mapPoint =
            poseData.histState.GetPose() * Point3f(ground_x, ground_y, 0.f);
        // World map is assumed to have the origin of the world in the center,
        // hence (rows/2, cols/2)
        const s32 x_map = (s32)std::round(
            mapPoint.x() + static_cast<f32>(_overheadMap.GetNumCols()) * 0.5f);
        const s32 y_map = (s32)std::round(
            -mapPoint.y() + static_cast<f32>(_overheadMap.GetNumRows()) * 0.5f);
        if (x_map >= 0 && y_map >= 0 && x_map < _overheadMap.GetNumCols() &&
            y_map < _overheadMap.GetNumRows()) {
          pointsInMap++;

          // Replace the old value here rather than blending,
          // want to make sure the map is up-to-date and it doesn't have
          // spurious values
  ...
  PRINT_CH_DEBUG(kLogChannelName, "OverheadMap.Update.UpdatedPixels",
                 "Number of pixels updated in overhead map: %d, "
  ...
  if (DEBUG_VISUALIZE) {
    const Vision::ImageRGB robotFootPrint =
        GetImageCenteredOnRobot(poseData.histState.GetPose(), debugImages);
  ...
  if (DEBUG_SAVE_OVERHEAD > 0) {
    ...
    if (frame_no == 0) {
      SaveMaskedOverheadPixels("positivePixels.txt", "negativePixels.txt",
                               "overheadMap.jpg");
...
Vision::ImageRGB OverheadMap::GetImageCenteredOnRobot(
    const Pose3d& robotPose,
    Vision::DebugImageList<Vision::CompressedImage>& debugImages) const {
  // To extract the pixels underneath the robot, the (cropped) overhead map is
  // rotated by the the current robot angle and then the region underneath the
  // footprint is extracted
  ...
  cv::Mat rotatationMatrix = cv::getRotationMatrix2D(
      {float(croppedOverhead.cols) / 2.0f, float(croppedOverhead.rows) / 2.0f},
      angle, 1.0);

  cv::Mat rotatedOverhead;
  // rotate the image
  try {
    cv::warpAffine(croppedOverhead, rotatedOverhead, rotatationMatrix,
                   croppedOverhead.size(), cv::INTER_LINEAR);
  } catch (cv::Exception& e) {
    PRINT_NAMED_ERROR("OverheadMap.GetImageCenteredOnRobot.ErrorOnWarp",
                      " Error while warping image: %s", e.what());
  ...
  try {
    cv::getRectSubPix(rotatedOverhead, footprintSize,
                      {float(rotatedOverhead.cols) * 0.5f,
                       float(rotatedOverhead.rows) *
                           0.5f},  // since we cropped the image take the center
                      toRet.get_CvMat_());
  } catch (const cv::Exception& e) {
    PRINT_NAMED_ERROR("OverheadMap.GetImageCenteredOnRobot.CVError",
                      "Error while cv::getRectSubPix: %s", e.what());
    return Vision::ImageRGB();
  }

  if (toRet.GetNumCols() == 0 || toRet.GetNumRows() == 0) {
    PRINT_NAMED_ERROR("OverheadMap.GetImageCenteredOnRobot.EmptyImage",
                      "Error: result image has %d rows and %d cols",
  ...
  // Note: see https://stackoverflow.com/a/13152420/1047543 to calculate the
  // points in the rotated rectangle without having to do the image. This
  // approach allows for visualization though
}

cv::RotatedRect OverheadMap::GetFootprintRotatedRect(
    const Pose3d& robotPose) const {
  // there's a bit of magic in this function, mostly the result of trial and
  // error.
  ...
  // TODO this number has been found in simulation. It's probably a duplicate of
  // Robot::GetDriveCenterOffset().
  // TODO This needs further investigation with the real robot
  const f32 CENTROID_AXIS_OFFSET_MM =
      16.9f;  // distance between the robot's centroid and the front axis middle
              // point

  // The robot footprint is not centered on the robot centroid, but the center
  // of the front axis :(
  robotFootprint += Point2f(CENTROID_AXIS_OFFSET_MM * std::cos(R.ToFloat()),
                            CENTROID_AXIS_OFFSET_MM * std::sin(R.ToFloat()));

  // Move the footprint to the image coordinate system, with the center in top
  // left and the y axis pointing down
  ...
  try {
    ...
    cv::RotatedRect footprintRect = cv::RotatedRect(
...
void OverheadMap::GetDrivableNonDrivablePixels(
    OverheadMap::PixelSet& drivablePixels,
  ...
  for (uint i = 0; i < _overheadMap.GetNumRows(); i++) {
    ...
    for (uint j = 0; j < _overheadMap.GetNumCols(); j++) {
      ...
      if (pixelValue !=
          Vision::PixelRGB(0, 0, 0)) {  // this pixel has been mapped
  ...
  // There's some debate as to how to insert elements in an array without
  // duplicates. See https://stackoverflow.com/q/12200486/1047543 See
  // https://stackoverflow.com/a/24477023/1047543 for a comparison Using an
  // unordered map seems to yield the best results when the number of duplicates
  // is > 1%
}
...
void OverheadMap::SaveMaskedOverheadPixels(
    const std::string& positiveExamplesFilename,
  ...
  if (!Util::FileUtils::CreateDirectory(path, false, true)) {
    PRINT_NAMED_ERROR("Overheadmap.SaveMaskedOverheadPixels.DirectoryError",
                      "Error while creating folder %s", path.c_str());
    return;
  }

  PRINT_CH_INFO(kLogChannelName,
                "OverheadMap.SaveMaskedOverheadPixels.PathInfo",
  ...
  {
    std::string fullPath =
        Util::FileUtils::FullFilePath({path, positiveExamplesFilename});
    std::ofstream outputFile(fullPath);
    if (!outputFile.is_open()) {
      PRINT_NAMED_ERROR("Overheadmap.SaveMaskedOverheadPixels.FileNotOpen",
    ...
    for (const auto& pixel : drivablePixels) {
      outputFile << int(pixel.r()) << " " << int(pixel.g()) << " "
  ...
  {
    std::string fullPath =
        Util::FileUtils::FullFilePath({path, negativeExamplesFileName});
    std::ofstream outputFile(fullPath);
    if (!outputFile.is_open()) {
      PRINT_NAMED_ERROR("Overheadmap.SaveMaskedOverheadPixels.FileNotOpen",
                        "Error while opening file %s for writing",
  ...
  {
    std::string fullPath =
        Util::FileUtils::FullFilePath({path, overheadMapFileName});
    _overheadMap.Save(fullPath, 100);
  }
}

void OverheadMap::UpdateFootprintMask(
    const Pose3d& robotPose,
    Vision::DebugImageList<Vision::CompressedImage>& debugImages) {
  cv::RotatedRect footprintRect = GetFootprintRotatedRect(robotPose);
  if ((footprintRect.size.width == 0) || (footprintRect.size.height == 0)) {
    PRINT_NAMED_WARNING("OverheadMap.UpdateFootprintMask.EmptyFootprintRect",
                        "Empty Footprint Rect!");
    return;
  }
  cv::Point2f vertices[4];
  footprintRect.points(vertices);
  ...
  std::vector<Point2i> points = {
      Point2i(s32(std::round(vertices[0].x)), s32(std::round(vertices[0].y))),
