...
/*
 * This class maintains an overhead map of the robot's surrounding. The map has
 * images taken from the ground plane in front of the robot. Every time the
 * robot covers a distance the map is updated/overwritten. The area underneath
 * the robot can be extracted using GetImageCenteredOnRobot()
 */
class OverheadMap {
 public:
  ...
  // Save the generated overhead map plus a list of drivable and non drivable
  // pixels. Used for offline training and testing
  ...
  // Returns a pair of:
  //  * all the pixels in the overhead map that are non-black in the footprint
  //  mask, i.e. areas that the robot traversed
  //  * all the pixels in the overhead map that the robot mapped but didn't
  //  traverse, making them potential obstacles
  ...
  // It's less efficient since data is copied. Useful for pre-allocated vectors
  // and array. Duplicate pixels are still removed
  ...
  void GetDrivableNonDrivablePixels(InputArray& drivablePixels,
    ...
    std::copy(drivableSet.begin(), drivableSet.end(),
              std::begin(drivablePixels));
    std::copy(nonDrivableSet.begin(), nonDrivableSet.end(),
              std::begin(nonDrivablePixels));
  }

 private:
  // Set the overhead map to be all black
  void ResetMaps();
  // Paint all the _footprintMask pixels underneath the robot footprint with
  // white
  void UpdateFootprintMask(
      const Pose3d& robotPose,
      Vision::DebugImageList<Vision::CompressedImage>& debugImages);

  // TODO in the future these could be sparse matrices for optimized iteration
  // over non-black pixels
  Vision::ImageRGB _overheadMap;
  Vision::Image _footprintMask;  // this image has 0 (black) if the robot has
                                 // never been to that position, 1 otherwise

  const CozmoContext* _context;

  // Calculate the footprint as a cv::RotatedRect. To get the correct size the
  // footprint is aligned with the axis
  cv::RotatedRect GetFootprintRotatedRect(const Pose3d& robotPose) const;

  Vision::ImageRGB GetImageCenteredOnRobot(
      const Pose3d& robotPose,
