...
struct ImageSaverParams {
  ...
  enum class SaveConditionType : uint8_t {
    ModeProcessed =
        0,        // Save when mode was run, whether or not it found anything
  ...
  float thumbnailScale =
      0.f;                // in range [0,1], as fraction of size, 0 to disable
  ...
  explicit ImageSaverParams(
      const std::string& path, Mode saveMode, int8_t quality,
      const std::string& basename = "",
      Vision::ImageCacheSize size = Vision::ImageCacheSize::Half,
      float thumbnailScale = 0.f, float saveScale = 1.f,
      bool removeDistortion = false, uint8_t medianFilterSize = 0,
      float sharpeningAmount = 0.f);

  static bool SaveConditionTypeFromString(const std::string& str,
                                          SaveConditionType& saveCondType);
};
...
class ImageSaver {
 public:
  ...
  // This must be called before calling SetParams with removeDistortion=true,
  // and before CacheUndistortionMaps
  void SetCalibration(
      const std::shared_ptr<Vision::CameraCalibration>& camCalib);

  // Pre-cache maps for undistortion, for a given image size. Will fail if
  // SetCalibration not called yet.
  ...
  // Returns true if the current mode is set such that the saver wants to save
  // an image (SingleShot* or Stream)
  bool WantsToSave() const;

  // Uses specified save conditions (if any) and the given processing result to
  // determine whether to save. Will check whether detections within result
  // match atTimestamp.
  bool WantsToSave(const VisionProcessingResult& result,
                   const TimeStamp_t atTimestamp) const;

  // Returns true if the current mode is SingleShotWithSensorData or Stream
  bool ShouldSaveSensorData() const;

  // Return the full filename to use for saving, using the given path and
  // frameNumber/basename, and appending the given extension. If no basename has
  // been provided in params, will use frameNumber. Otherwise frameNumber is
  // ignored.
  std::string GetFullFilename(const s32 frameNumber,
                              const char* extension) const;

  // Save the specified size image from the cache and a corresponding thumbnail
  // if requested.
