 ...
 * Date:   11/20/15
 *
 * Description: Defines a class for visual reasoning about a region of interest
 *(ROI) on the ground plane immediately in front of the robot.
 *
 *
...
const Vision::Image& GroundPlaneROI::GetOverheadMask() const {
  if (_overheadMask.IsEmpty()) {
    ...
    cv::fillConvexPoly(
        _overheadMask.get_CvMat_(),
        std::vector<cv::Point>{cv::Point(0, w), cv::Point(_length - 1, 0),
...
Quad3f GroundPlaneROI::GetGroundQuad(f32 zHeight) {
  return Quad3f{{_dist + _length, 0.5f * _widthFar, zHeight},
                {_dist, 0.5f * _widthClose, zHeight},
                {_dist + _length, -0.5f * _widthFar, zHeight},
                {_dist, -0.5f * _widthClose, zHeight}};
}

bool GroundPlaneROI::GetVisibleGroundQuad(const Matrix_3x3f& H, s32 imgWidth,
                                          s32 imgHeight, Quad3f& groundQuad,
                                          f32 z) const {
  Quad2f imgQuad;
  ...
  if (intersectsBorder) {
    ...
    //  Technically, we are only checking for "near" intersection in
    //  GetImageQuad above, so the far points can't move, so there's no reason
    //  to warp them.
    Matrix_3x3f invH = H.GetInverse();
    for (Quad::CornerName iCorner : {Quad::BottomLeft, Quad::BottomRight}) {
      Point3f temp =
          invH * Point3f(imgQuad[iCorner].x(), imgQuad[iCorner].y(), 1.f);
      DEV_ASSERT(temp.z() > 0,
                 "GroundPlaneROI.GetVisibleGroundQuad.BadProjectedZ");
...
const Vision::Image GroundPlaneROI::GetVisibleOverheadMask(
    const Matrix_3x3f& H, s32 imgWidth, s32 imgHeight) const {
...
void GroundPlaneROI::GetVisibleX(const Matrix_3x3f& H, s32 imageWidth,
                                 s32 imageHeight, f32& near, f32& far) const {
  Matrix_3x3f invH = H.GetInverse();
...
bool GroundPlaneROI::GetImageQuad(const Matrix_3x3f& H, s32 imgWidth,
                                  s32 imgHeight, Quad2f& imgQuad) const {
  ...
  for (Quad::CornerName iCorner = Quad::CornerName::FirstCorner;
    ...
    DEV_ASSERT(!Util::IsNearZero(temp.z()),
               "GroundPlaneROI.GetImageQuad.ProjectedGroundQuadPointAtZero");
    const f32 divisor = 1.f / temp.z();
  ...
  const bool intersectsImageBorder =
      ClampQuad(imgQuad, imgBotLeft, imgBotRight);

  return intersectsImageBorder;
}  // GetImageQuad()

template <class PixelType>
void GroundPlaneROI::GetOverheadImageHelper(
    const Vision::ImageBase<PixelType>& image, const Matrix_3x3f& H,
    Vision::ImageBase<PixelType>& overheadImg, bool useMask) const {
  ...
  Matrix_3x3f InvShift{1.f, 0.f,  _dist,  // Negated b/c we're using inv(Shift)
  ...
  cv::warpPerspective(image.get_CvMat_(), overheadImg.get_CvMat_(),
                      (H * InvShift).get_CvMatx_(),
                      cv::Size(_length, _widthFar),
                      cv::INTER_LINEAR | cv::WARP_INVERSE_MAP);

  if (useMask) {
    const Vision::Image& mask = GetOverheadMask();

    DEV_ASSERT(overheadImg.IsContinuous() && mask.IsContinuous(),
               "Overhead image and mask should be continuous");
...
Vision::ImageRGB GroundPlaneROI::GetOverheadImage(const Vision::ImageRGB& image,
                                                  const Matrix_3x3f& H,
                                                  bool useMask) const {
  Vision::ImageRGB overheadImg(_overheadMask.GetNumRows(),
                               _overheadMask.GetNumCols());
  GetOverheadImageHelper(image, H, overheadImg, useMask);
  return overheadImg;
}

Vision::Image GroundPlaneROI::GetOverheadImage(const Vision::Image& image,
                                               const Matrix_3x3f& H,
                                               bool useMask) const {
  Vision::Image overheadImg(_overheadMask.GetNumRows(),
                            _overheadMask.GetNumCols());
  GetOverheadImageHelper(image, H, overheadImg, useMask);
  return overheadImg;
}
...
bool GroundPlaneROI::ClampQuad(Quad2f& quad, const Point2f& groundLeft,
                               const Point2f& groundRight) {
  // this is a trick to prevent precision errors around the borders. We are just
  // trying to find intersection with a line, not a segment, so we artificially
  // extend the segment given to provide a safer line
  ...
  kmRay2 segmentLeftLine =
      Point2fToRay(quad[Quad::BottomLeft], quad[Quad::TopLeft]);
  kmRay2 segmentRightLine =
      Point2fToRay(quad[Quad::BottomRight], quad[Quad::TopRight]);

  // find intersections of segment lines (it should always happen unless there's
  // a precision error in the border, which can happen)
  kmVec2 interBL, interBR;
  const kmBool leftBotInter = kmSegment2WithSegmentIntersection(
      &groundBotLine, &segmentLeftLine, &interBL);
  const kmBool rightBotInter = kmSegment2WithSegmentIntersection(
      &groundBotLine, &segmentRightLine, &interBR);
  if (leftBotInter && rightBotInter) {
    ...
    //                  "Could not find intersection of fake vision quad with
    //                  ground plane. Ignoring segment");
    return false;
  }
}
