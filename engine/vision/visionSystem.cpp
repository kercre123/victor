...
const char* kImageCompositorReadyCycleResetKey =
    "numImageReadyCyclesBeforeReset";
}  // namespace

CONSOLE_VAR_RANGED(u8, kUseCLAHE_u8, "Vision.PreProcessing", 0, 0,
                   4);  // One of MarkerDetectionCLAHE enum
CONSOLE_VAR(s32, kClaheClipLimit, "Vision.PreProcessing", 32);
CONSOLE_VAR(s32, kClaheTileSize, "Vision.PreProcessing", 4);
CONSOLE_VAR(u8, kClaheWhenDarkThreshold, "Vision.PreProcessing",
            80);  // In MarkerDetectionCLAHE::WhenDark mode, only use CLAHE when
                  // img avg < this
CONSOLE_VAR(s32, kPostClaheSmooth, "Vision.PreProcessing",
            -3);  // 0: off, +ve: Gaussian sigma, -ve (& odd): Box filter size
CONSOLE_VAR(s32, kMarkerDetector_ScaleMultiplier, "Vision.MarkerDetection", 2);
CONSOLE_VAR(f32, kHeadTurnSpeedThreshBlock_degs, "Vision.MarkerDetection",
            10.f);
CONSOLE_VAR(f32, kBodyTurnSpeedThreshBlock_degs, "Vision.MarkerDetection",
            30.f);

// This is fraction of full width we use with the CropScheduler to crop the
// image for marker detection.
CONSOLE_VAR_RANGED(f32, kMarkerDetector_CropWidthFraction,
                   "Vision.MarkerDetection", 0.65f, 0.5f, 1.f);

// Show the crops being used for MarkerDetection. Need to increase Viz debug
// windows if enabled.
CONSOLE_VAR(bool, kMarkerDetector_VizCropScheduler, "Vision.MarkerDetection",
            false);
...
// Loose constraints on how fast Cozmo can move and still trust tracker (which
// has no knowledge of or access to camera movement). Rough means of deciding
// these angles: look at angle created by distance between two faces seen close
// together at the max distance we care about seeing them from. If robot turns
// by that angle between two consecutve frames, it is possible the tracker will
// be confused and jump from one to the other.
CONSOLE_VAR(f32, kFaceTrackingMaxHeadAngleChange_deg, "Vision.FaceDetection",
            8.f);
CONSOLE_VAR(f32, kFaceTrackingMaxBodyAngleChange_deg, "Vision.FaceDetection",
            8.f);
...
CONSOLE_VAR_RANGED(s32, kImageMeanSampleInc, "VisionSystem.Statistics", 10, 1,
                   32);

// For testing artificial slowdowns of the vision thread
CONSOLE_VAR(u32, kVisionSystemSimulatedDelay_ms, "Vision.General", 0);

CONSOLE_VAR(u32, kCalibTargetType, "Vision.Calibration",
            (u32)CameraCalibrator::CalibTargetType::CHECKERBOARD);

// The percentage of the width of the image that will remain after cropping
CONSOLE_VAR_RANGED(f32, kFaceTrackingCropWidthFraction, "Vision.FaceDetection",
                   2.f / 3.f, 0.f, 1.f);

// Fake hand and pet detections for testing behaviors while we don't have
// reliable neural net models
CONSOLE_VAR_RANGED(f32, kFakeHandDetectionProbability, "Vision.NeuralNets", 0.f,
                   0.f, 1.f);
CONSOLE_VAR_RANGED(f32, kFakeCatDetectionProbability, "Vision.NeuralNets", 0.f,
                   0.f, 1.f);
CONSOLE_VAR_RANGED(f32, kFakeDogDetectionProbability, "Vision.NeuralNets", 0.f,
                   0.f, 1.f);
...
VisionSystem::VisionSystem(const CozmoContext* context)
    : _rollingShutterCorrector(),
      ...
      _cameraParamsController(new Vision::CameraParamsController(
          MIN_CAMERA_EXPOSURE_TIME_MS, MAX_CAMERA_EXPOSURE_TIME_MS,
          MIN_CAMERA_GAIN, MAX_CAMERA_GAIN, _currentCameraParams)),
      ...
      _overheadEdgeDetector(
          new OverheadEdgesDetector(_camera, _vizManager, *this)),
...
Result VisionSystem::Init(const Json::Value& config) {
  ...
  if (_context->GetDataPlatform() != nullptr) {
    dataPath = _context->GetDataPlatform()->pathToResource(
        Util::Data::Scope::Resources,
        Util::FileUtils::FullFilePath({"config", "engine", "vision"}));
    cachePath = _context->GetDataPlatform()->pathToResource(
        Util::Data::Scope::Cache, "vision");
  } else {
  ...
  // Helper macro to try to get the specified field and store it in the given
  // variable and return RESULT_FAIL if that doesn't work
#define GET_JSON_PARAMETER(__json__, __fieldName__, __variable__)              \
  do {                                                                         \
    if (!JsonTools::GetValueOptional(__json__, __fieldName__, __variable__)) { \
      PRINT_NAMED_ERROR("VisionSystem.Init.MissingJsonParameter", "%s",        \
                        __fieldName__);                                        \
      return RESULT_FAIL;                                                      \
    }                                                                          \
  } while (0)

  {
    ...
    GET_JSON_PARAMETER(imageQualityConfig, "MeterFromDetections",
                       kMeterFromDetections);
    ...
    GET_JSON_PARAMETER(imageQualityConfig, "TargetPercentile",
                       kTargetPercentile);
    GET_JSON_PARAMETER(imageQualityConfig, "TargetValue", targetValue);
    GET_JSON_PARAMETER(imageQualityConfig, "MaxChangeFraction",
                       maxChangeFraction);
    GET_JSON_PARAMETER(imageQualityConfig, "SubSample", subSample);

    std::vector<u8> cyclingTargetValues;
    if (!JsonTools::GetVectorOptional(imageQualityConfig, "CyclingTargetValues",
                                      cyclingTargetValues)) {
      PRINT_NAMED_ERROR("VisionSystem.Init.MissingJsonParameter", "%s",
                        "CyclingTargetValues");
      return RESULT_FAIL;
    ...
    Result result = _cameraParamsController->SetExposureParameters(
        targetValue, cyclingTargetValues, kTargetPercentile, maxChangeFraction,
        subSample);
    if (RESULT_OK == result) {
      PRINT_CH_INFO(kLogChannelName, "VisionSystem.Init.SetAutoExposureParams",
                    "subSample:%d tarVal:%d tarPerc:%.3f changeFrac:%.3f",
                    subSample, targetValue, kTargetPercentile,
                    maxChangeFraction);
    } else {
    ...
    result = _cameraParamsController->SetImageQualityParameters(
        kTooDarkValue, kHighPercentile, kTooBrightValue, kLowPercentile);
    if (RESULT_OK != result) {
...

  {
    ...
    GET_JSON_PARAMETER(performanceConfig, "TimeBetweenProfilerInfoPrints_sec",
                       timeBetweenProfilerInfoPrints_sec);
    GET_JSON_PARAMETER(performanceConfig, "TimeBetweenProfilerDasLogs_sec",
    ...
    Profiler::SetPrintFrequency(
        Util::SecToMilliSec(timeBetweenProfilerInfoPrints_sec));
    Profiler::SetDasLogFrequency(
        Util::SecToMilliSec(timeBetweenProfilerDasLogs_sec));
  }
  ...
  PRINT_CH_INFO(kLogChannelName,
                "VisionSystem.Init.DoneInstantiatingFaceTracker", "");
  ...
  {
    _imageCompositorReadyPeriod = JsonTools::ParseUInt32(
        imageCompositeCfg, kImageCompositorReadyPeriodKey, "VisionSystem.Ctor");

    // The Reset Period is an integer multiple of the Ready Period
    _imageCompositorResetPeriod =
        _imageCompositorReadyPeriod *
  ...
  _groundPlaneClassifier.reset(
      new GroundPlaneClassifier(config["GroundPlaneClassifier"], _context));
  ...
  const std::string modelPath =
      Util::FileUtils::FullFilePath({dataPath, "dnn_models"});
  if (Util::FileUtils::DirectoryExists(
          modelPath))  // TODO: Remove once DNN models are checked in somewhere
                       // (VIC-1071)
  {
    const Json::Value& neuralNetConfig =
        config[NeuralNets::JsonKeys::NeuralNets];
    ...
    const Json::Value& modelsConfig =
        neuralNetConfig[NeuralNets::JsonKeys::Models];
...
#ifdef VICOS
    // Use faster tmpfs partition for the cache, to make I/O less of a
    // bottleneck
    const std::string dnnCachePath = "/tmp/vision/neural_nets";
#else
    const std::string dnnCachePath =
        Util::FileUtils::FullFilePath({cachePath, "neural_nets"});
#endif
    ...
    for (const auto& modelConfig : modelsConfig) {
      ...
      const std::string& name =
          modelConfig[NeuralNets::JsonKeys::NetworkName].asString();
      auto addModelResult = _neuralNetRunners.emplace(
          name, new NeuralNets::NeuralNetRunner(modelPath));
      if (!addModelResult.second) {
        PRINT_NAMED_ERROR("VisionSystem.Init.DuplicateNeuralNetModelName", "%s",
                          name.c_str());
        continue;
      }
      std::unique_ptr<NeuralNets::NeuralNetRunner>& neuralNetRunner =
          addModelResult.first->second;
      Result neuralNetResult = neuralNetRunner->Init(dnnCachePath, modelConfig);
      if (RESULT_OK != neuralNetResult) {
        PRINT_NAMED_ERROR("VisionSystem.Init.NeuralNetInitFailed", "Name: %s",
                          name.c_str());
        continue;
      }
    }
  ...
  if (!config.isMember("IlluminationDetector")) {
    PRINT_NAMED_ERROR(
        "VisionSystem.Init.MissingIlluminationDetectorConfigField", "");
    return RESULT_FAIL;
  }
  Result illuminationResult =
      _illuminationDetector->Init(config["IlluminationDetector"], _context);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result VisionSystem::UpdateCameraCalibration(
  ...
  // Provide the ImageSaver with the camera calibration so that it can remove
  // distortion if needed Also pre-cache distortion maps for Sensor resolution,
  // since we use that for photos
  _imageSaver->SetCalibration(camCalib);
  _imageSaver->CacheUndistortionMaps(CAMERA_SENSOR_RESOLUTION_HEIGHT,
...
Result VisionSystem::SetNextCameraParams(const Vision::CameraParams& params) {
  if (!_cameraParamsController->AreCameraParamsValid(params)) {
    PRINT_PERIODIC_CH_INFO(
        100, kLogChannelName, "VisionSystem.SetNextCameraParams.InvalidParams",
  ...
  if (nextParamsSet) {
    PRINT_NAMED_WARNING(
        "VisionSystem.SetNextCameraParams.OverwritingPreviousParams",
        ...
        _nextCameraParams.second.whiteBalanceGainB, params.exposureTime_ms,
        params.gain, params.whiteBalanceGainR, params.whiteBalanceGainB);
  }
...
Result VisionSystem::UpdatePoseData(const VisionPoseData& poseData) {
  ...
  // Update cameraPose and historical state's pose to use the vision system's
  // pose origin
  {
    // We expect the passed-in historical pose to be w.r.t. to an origin and
    // have its parent removed so that we can set its parent to be our
    // poseOrigin on this thread. The cameraPose should use the histState's pose
    // as its parent.
    DEV_ASSERT(!poseData.histState.GetPose().HasParent(),
               "VisionSystem.UpdatePoseData.HistStatePoseHasParent");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool VisionSystem::IsInitialized() const { return _isInitialized; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
u8 VisionSystem::ComputeMean(Vision::ImageCache& imageCache,
                             const s32 sampleInc) {
  ...
  // Consider that in the loop above, we always start at row 0, and we always
  // start at column 0
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void VisionSystem::UpdateMeteringRegions(
    TimeStamp_t currentTime_ms, DetectionRectsByMode&& detectionsByMode) {
  const bool meterFromChargerOnly =
      IsModeEnabled(VisionMode::Markers_ChargerOnly);

  // Before we do image quality / auto exposure, swap in the detections for any
  // mode that actually ran, in case we need them for metering
  // - if a mode ran but detected nothing, then an empty vector of rects will be
  // swapped in
  // - if a mode did not run, the previous detections for that mode will persist
  // until it runs again
  ...
  // Clear out any stale "previous" detections for modes that are completely
  // disabled by the current schedule Also remove empty vectors of rectangles
  // that got swapped in above
...
#define DEBUG_IMAGE_HISTOGRAM 0
  ...
  Vision::CameraParamsController::AutoExpMode aeMode =
      Vision::CameraParamsController::AutoExpMode::Off;
  ...
  Vision::CameraParamsController::WhiteBalanceMode wbMode =
      Vision::CameraParamsController::WhiteBalanceMode::Off;
  ...
  if (!kMeterFromDetections || _meteringRegions.empty()) {
    if ((_lastMeteringTimestamp_ms > 0) &&  // if we've ever metered
        imageCache.GetTimeStamp() <=
            (_lastMeteringTimestamp_ms + kMeteringHoldTime_ms)) {
      // Don't update auto exposure for a little while after we lose metered
      // regions
      PRINT_CH_INFO("VisionSystem",
                    "VisionSystem.UpdateCameraParams."
                    "HoldingExposureAfterRecentMeteredRegions",
                    "");
      return RESULT_OK;
    }
    ...
    // When we have detections to weight, don't use cycling exposures. Just let
    // the detections drive the exposure.
    useCycling = false;
  ...
  if (imageCache.HasColor()) {
    ...
    expResult = _cameraParamsController->ComputeNextCameraParams(
        inputImage, aeMode, wbMode, useCycling, nextParams);
  } else {
    const Vision::Image& inputImage = imageCache.GetGray();
    expResult = _cameraParamsController->ComputeNextCameraParams(
        inputImage, aeMode, useCycling, nextParams);
  }

  if (RESULT_OK != expResult) {
    PRINT_NAMED_WARNING(
        "VisionSystem.UpdateCameraParams.ComputeNewExposureFailed", "");
    return expResult;
  }

  if (DEBUG_IMAGE_HISTOGRAM) {
    const Vision::ImageBrightnessHistogram& hist =
        _cameraParamsController->GetHistogram();
    std::vector<u8> values = hist.ComputePercentiles(
        {kLowPercentile, kTargetPercentile, kHighPercentile});
  ...
  const bool completedExposureCycling =
      _cameraParamsController->IsExposureCyclingComplete();
  if (IsModeEnabled(VisionMode::AutoExp_Cycling) &&
      (_isMeteringForDetection || completedExposureCycling)) {
  ...
  _currentResult.modesProcessed.Enable(
      VisionMode::AutoExp_MinGain,
      (aeMode == Vision::CameraParamsController::AutoExpMode::MinGain));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result VisionSystem::AssignNameToFace(Vision::FaceID_t faceID,
                                      const std::string& name,
                                      Vision::FaceID_t mergeWithID) {
  if (!_isInitialized) {
    PRINT_NAMED_WARNING(
        "VisionSystem.AssignNameToFace.NotInitialized",
        "Cannot assign name '%s' to face ID %d before being initialized",
        name.c_str(), faceID);
    return RESULT_FAIL;
  }

  DEV_ASSERT(_faceTracker != nullptr,
             "VisionSystem.AssignNameToFace.NullFaceTracker");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void VisionSystem::SetFaceEnrollmentMode(Vision::FaceID_t forFaceID,
                                         s32 numEnrollments, bool forceNewID) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void VisionSystem::EraseAllFaces() { _faceTracker->EraseAllFaces(); }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result VisionSystem::RenameFace(Vision::FaceID_t faceID,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Vision::Image BlackOutRects(const Vision::Image& img,
                            const std::vector<Anki::Rectangle<s32>>& rects) {
  // Black out detected markers so we don't find faces in them
  Vision::Image maskedImage;
  img.CopyTo(maskedImage);

  DEV_ASSERT(maskedImage.GetTimestamp() == img.GetTimestamp(),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result VisionSystem::DetectFaces(
    Vision::ImageCache& imageCache,
    std::vector<Anki::Rectangle<s32>>& detectionRects, const bool useCropping) {
  DEV_ASSERT(_faceTracker != nullptr,
             "VisionSystem.DetectFaces.NullFaceTracker");
  ...
  const bool hasHeadMoved = !_poseData.IsHeadAngleSame(
      _prevPoseData, DEG_TO_RAD(kFaceTrackingMaxHeadAngleChange_deg));
  const bool hasBodyMoved = !_poseData.IsBodyPoseSame(
      _prevPoseData, DEG_TO_RAD(kFaceTrackingMaxBodyAngleChange_deg),
  ...
  if (!detectionRects.empty()) {
    ...
    _faceTracker->Update(maskedImage, cropFactor, _currentResult.faces,
                         _currentResult.updatedFaceIDs,
                         _currentResult.debugImages);
  } else {
    // Nothing already detected, so nothing to black out before looking for
    // faces
    _faceTracker->Update(grayImage, cropFactor, _currentResult.faces,
                         _currentResult.updatedFaceIDs,
                         _currentResult.debugImages);
  }

  for (auto faceIter = _currentResult.faces.begin();
       faceIter != _currentResult.faces.end(); ++faceIter) {
    auto& currentFace = *faceIter;

    DEV_ASSERT(currentFace.GetTimeStamp() == grayImage.GetTimestamp(),
               "VisionSystem.DetectFaces.BadFaceTimestamp");

    detectionRects.emplace_back(
        (s32)std::round(faceIter->GetRect().GetX()),
    ...
    DEV_ASSERT(headPose.IsChildOf(_poseOrigin),
               "VisionSystem.DetectFaces.BadHeadPoseParent");
    DEV_ASSERT(eyePose.IsChildOf(_poseOrigin),
               "VisionSystem.DetectFaces.BadEyePoseParent");

    // Leave faces in the output result with no parent pose (b/c we will assume
    // they are w.r.t. the origin)
...
Result VisionSystem::DetectBrightColors(Vision::ImageCache& imageCache) {
  ...
  Result result =
      _brightColorDetector->Detect(image, _currentResult.salientPoints);
  return result;
}  // DetectBrightColors()
...
Result VisionSystem::UpdateOverheadMap(Vision::ImageCache& imageCache) {
  ...
  const Vision::ImageRGB& image = imageCache.GetRGB();
  Result result =
      _overheadMap->Update(image, _poseData, _currentResult.debugImages);
  return result;
}

Result VisionSystem::UpdateGroundPlaneClassifier(
    Vision::ImageCache& imageCache) {
  DEV_ASSERT(imageCache.HasColor(),
             "VisionSystem.UpdateGroundPlaneClassifier.NoColor");
  const Vision::ImageRGB& image = imageCache.GetRGB();
  Result result = _groundPlaneClassifier->Update(
      image, _poseData, _currentResult.debugImages,
...
Result VisionSystem::DetectLaserPoints(Vision::ImageCache& imageCache) {
  const bool isDarkExposure =
      (Util::IsNear(_currentCameraParams.exposureTime_ms,
                    GetMinCameraExposureTime_ms()) &&
       Util::IsNear(_currentCameraParams.gain, GetMinCameraGain()));

  Result result = _laserPointDetector->Detect(
      imageCache, _poseData, isDarkExposure, _currentResult.laserPoints,
      _currentResult.debugImages);
...
Result VisionSystem::DetectIllumination(Vision::ImageCache& imageCache) {
  Result result = _illuminationDetector->Detect(imageCache, _poseData,
...
Result VisionSystem::ApplyCLAHE(Vision::ImageCache& imageCache,
  ...
  const Vision::ImageCacheSize whichSize =
      imageCache.GetSize(kMarkerDetector_ScaleMultiplier);
  ...
  switch (useCLAHE) {
    ...
    case MarkerDetectionCLAHE::WhenDark: {
      ...
      const s32 numRows = inputImageGray.GetNumRows();
      const s32 numCols = inputImageGray.GetNumCols();
      // Consider that in the loop below, we always start at row 0, and we
  ...
  claheImage.SetTimestamp(
      inputImageGray.GetTimestamp());  // make sure to preserve timestamp!
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result VisionSystem::DetectMarkers(
    Vision::ImageCache& imageCache, const Vision::Image& claheImage,
    std::vector<Anki::Rectangle<s32>>& detectionRects,
    MarkerDetectionCLAHE useCLAHE, const VisionPoseData& poseData) {
  // Currently assuming we detect markers first, so we won't make use of
  // anything already detected
  DEV_ASSERT(
      detectionRects.empty(),
      "VisionSystem.DetectMarkersWithCLAHE.ExpectingEmptyDetectionRects");
  ...
  switch (useCLAHE) {
    ...
    case MarkerDetectionCLAHE::On: {
      DEV_ASSERT(!claheImage.IsEmpty(),
                 "VisionSystem.DetectMarkersWithCLAHE.useOn.ImageIsEmpty");
    ...
    case MarkerDetectionCLAHE::Both: {
      DEV_ASSERT(!claheImage.IsEmpty(),
                 "VisionSystem.DetectMarkersWithCLAHE.useBoth.ImageIsEmpty");
    ...
    case MarkerDetectionCLAHE::Alternating: {
      if (_currentUseCLAHE) {
        DEV_ASSERT(
            !claheImage.IsEmpty(),
            "VisionSystem.DetectMarkersWithCLAHE.useAlternating.ImageIsEmpty");
        imagePtrs.push_back(&claheImage);
      } else {
        imagePtrs.push_back(&imageCache.GetGray(whichSize));
      }
    ...
    case MarkerDetectionCLAHE::WhenDark: {
      // NOTE: _currentUseCLAHE should have been set based on image brightness
      // already

      if (_currentUseCLAHE) {
        DEV_ASSERT(
            !claheImage.IsEmpty(),
            "VisionSystem.DetectMarkersWithCLAHE.useWhenDark.ImageIsEmpty");
        imagePtrs.push_back(&claheImage);
      } else {
        imagePtrs.push_back(&imageCache.GetGray(whichSize));
      }
...
#define DEBUG_IMAGE_COMPOSITING 0
  ...
  if (IsModeEnabled(VisionMode::Markers_Composite)) {
    ...
    const size_t numImagesComposited =
        _imageCompositor->GetNumImagesComposited();

    const bool shouldRunOnComposite =
        (numImagesComposited % _imageCompositorReadyPeriod) == 0;
    if (shouldRunOnComposite) {
      _imageCompositor->GetCompositeImage(compositeImage);
      imagePtrs.push_back(&compositeImage);
#if (DEBUG_IMAGE_COMPOSITING)
      dispCompositeImg.Allocate(compositeImage.GetNumRows(),
                                compositeImage.GetNumCols());
      compositeImage.CopyTo(dispCompositeImg);
#endif
    }

    const bool shouldReset =
        (numImagesComposited == _imageCompositorResetPeriod);
    if (shouldReset) {
      ...
      DEV_ASSERT_MSG(
          shouldRunOnComposite,
          "VisionSystem.DetectMarkers.InvalidResetCallBeforeImageUsed", "");
  ...
  if (!dispCompositeImg.IsEmpty()) {
    _currentResult.debugImages.emplace_back("ImageCompositing",
                                            dispCompositeImg);
  }
#endif

  // Set up cropping rectangles to cycle through each time DetectMarkers is
  // called
  DEV_ASSERT(!imagePtrs.empty(),
             "VisionSystem.DetectMarkersWithCLAHE.NoImagePointers");
  static CropScheduler cropScheduler(_camera);
  if (!Util::IsNear(cropScheduler.GetCropWidthFraction(),
                    kMarkerDetector_CropWidthFraction)) {
    cropScheduler.Reset(kMarkerDetector_CropWidthFraction,
                        CropScheduler::CyclingMode::Middle_Left_Middle_Right);
  }

  Rectangle<s32> cropRect;
  if (IsModeEnabled(VisionMode::Markers_FullFrame)) {
    cropRect = Rectangle<s32>(0, 0, imagePtrs.front()->GetNumCols(),
                              imagePtrs.front()->GetNumRows());
    _currentResult.modesProcessed.Insert(VisionMode::Markers_FullFrame);
  } else {
    const bool useHorizontalCycling =
        !IsModeEnabled(VisionMode::Markers_FullWidth);
    const bool useVariableHeight =
        !IsModeEnabled(VisionMode::Markers_FullHeight);
    const bool cropInBounds = cropScheduler.GetCropRect(
        imagePtrs.front()->GetNumRows(), imagePtrs.front()->GetNumCols(),
        useHorizontalCycling, useVariableHeight, poseData, cropRect);

    if (!cropInBounds) {
      PRINT_CH_DEBUG(kLogChannelName,
                     "VisionSystem.DetectMarkersWithCLAHE.CropRectOOB", "");
      return RESULT_OK;
    }

    DEV_ASSERT(cropRect.Area() > 0,
               "VisionSystem.DetectMarkersWithCLAHE.EmptyCrop");

    _currentResult.modesProcessed.Enable(VisionMode::Markers_FullWidth,
                                         !useHorizontalCycling);
    _currentResult.modesProcessed.Enable(VisionMode::Markers_FullHeight,
                                         !useVariableHeight);
  }
  ...
  for (auto imgPtr : imagePtrs) {
    ...
    lastResult =
        _markerDetector->Detect(imgROI, _currentResult.observedMarkers);
    ...
    if (kMarkerDetector_VizCropScheduler) {
      ...
      dispImg.DrawRect(
          Rectangle<s32>{0, 0, cropRect.GetWidth(), cropRect.GetHeight()},
          NamedColors::RED);
  ...
  const bool meterFromChargerOnly =
      IsModeEnabled(VisionMode::Markers_ChargerOnly);
  _currentResult.modesProcessed.Enable(VisionMode::Markers_ChargerOnly,
                                       meterFromChargerOnly);
  ...
  while (markerIter != _currentResult.observedMarkers.end()) {
    ...
    if (meterFromChargerOnly &&
        (marker.GetCode() != Vision::MARKER_CHARGER_HOME)) {
      markerIter = _currentResult.observedMarkers.erase(markerIter);
      continue;
    }

    // Adjust the marker for the crop rectangle / processing resolution, to put
    // it back in original image coordinates
    Quad2f scaledCorners(marker.GetImageCorners());
    if (cropRect.GetX() > 0 || cropRect.GetY() > 0 ||
        kMarkerDetector_ScaleMultiplier != 1) {
      for (auto& corner : scaledCorners) {
        corner.x() += cropRect.GetX();
        corner.y() += cropRect.GetY();

        // By default we display images at the default image cache size so we
        // need to scale the marker corners to that size
        const f32 scaleMultiplier = ImageCacheSizeToScaleFactor(
            Vision::ImageCache::GetSize(kMarkerDetector_ScaleMultiplier));
        const f32 defaultScaleMultiplier = ImageCacheSizeToScaleFactor(
            Vision::ImageCache::GetDefaultImageCacheSize());
    ...
    // Add the bounding rect of the (unwarped) marker to the detection
    // rectangles Note that the scaled corners might get changed slightly by
    // rolling shutter warping below, making the detection rect slightly
    // inaccurate, but these rectangles don't need to be super accurate. And
    // we'd rather still report something being detected here even if the
    // warping pushes a corner OOB, thereby removing the marker from the actual
    // reported list.
    detectionRects.emplace_back(scaledCorners);

    // Instead of correcting the entire image only correct the quads
    // Apply the appropriate shift to each of the corners of the quad to get a
    // shifted quad
    if (_doRollingShutterCorrection) {
      bool allCornersInBounds = true;
      for (auto& corner : scaledCorners) {
        const s32 fullNumRows =
            imageCache.GetNumRows(Vision::ImageCacheSize::Half);
        const s32 fullNumCols =
            imageCache.GetNumCols(Vision::ImageCacheSize::Half);
        const int warpIndex = std::floor(
            corner.y() /
            (fullNumRows / _rollingShutterCorrector.GetNumDivisions()));
        DEV_ASSERT_MSG(
            warpIndex >= 0 &&
                warpIndex < _rollingShutterCorrector.GetPixelShifts().size(),
            "VisionSystem.DetectMarkersWithCLAHE.WarpIndexOOB",
            "Index:%d Corner y:%f", warpIndex, corner.y());

        const Vec2f& pixelShift =
            _rollingShutterCorrector.GetPixelShifts().at(warpIndex);
        corner -= pixelShift;

        if (Util::IsFltLTZero(corner.x()) || Util::IsFltLTZero(corner.y()) ||
            Util::IsFltGE(corner.x(), (f32)fullNumCols) ||
            Util::IsFltGE(corner.y(), (f32)fullNumRows)) {
          // Warped corner is outside image bounds. Just drop this entire
          // marker. Technically, we could still probably estimate its pose just
          // fine, but other things later may expect all corners to be in bounds
      ...
      if (!allCornersInBounds) {
        ...
        PRINT_CH_DEBUG(kLogChannelName,
                       "VisionSystem.DetectMarkersWithCLAHE.RemovingMarkerOOB",
                       "%s", Vision::MarkerTypeStrings[marker.GetCode()]);
        markerIter = _currentResult.observedMarkers.erase(markerIter);
        continue;
      }

      marker.SetImageCorners(
          scaledCorners);  // "scaled" corners are now the warped corners
    }
...
void VisionSystem::CheckForNeuralNetResults() {
  for (const auto& neuralNetRunnerEntry : _neuralNetRunners) {
    ...
    const bool resultReady =
        neuralNetRunner->GetDetections(_currentResult.salientPoints);
    if (resultReady) {
      PRINT_CH_DEBUG(kLogChannelName,
                     "VisionSystem.CheckForNeuralNetResults.GotDetections",
                     "Network:%s NumSalientPoints:%zu", networkName.c_str(),
                     _currentResult.salientPoints.size());

      std::set<VisionMode> modes;
      const bool success = GetVisionModesForNeuralNet(networkName, modes);
      if (ANKI_VERIFY(
              success,
              "VisionSystem.CheckForNeuralNetResults.NoModeForNetworkName",
              "Name: %s", networkName.c_str())) {
        for (auto& mode : modes) {
          _currentResult.modesProcessed.Insert(mode);
        }

        if (IsModeEnabled(VisionMode::SaveImages)) {
          const Vision::ImageRGB& neuralNetRunnerImage =
              neuralNetRunner->GetOrigImg();

          if (!neuralNetRunnerImage.IsEmpty() &&
              _imageSaver->WantsToSave(_currentResult,
                                       neuralNetRunnerImage.GetTimestamp())) {
            const Result saveResult =
                _imageSaver->Save(neuralNetRunnerImage, _frameNumber);
            if (RESULT_OK == saveResult) {
              _currentResult.modesProcessed.Insert(VisionMode::SaveImages);
            }

            const std::string jsonFilename =
                _imageSaver->GetFullFilename(_frameNumber, "json");
            Json::Value jsonSalientPoints;
            NeuralNets::INeuralNetMain::ConvertSalientPointsToJson(
                _currentResult.salientPoints, false, jsonSalientPoints);
            const bool success = NeuralNets::INeuralNetMain::WriteResults(
                jsonFilename, jsonSalientPoints);
            if (!success) {
              LOG_WARNING(
                  "VisionSystem.CheckForNeuralNets."
                  "WriteJsonSalientPointsFailed",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void VisionSystem::AddFakeDetections(const TimeStamp_t atTimestamp,
                                     const std::set<VisionMode>& modes) {
  // DEBUG: Randomly fake detections of hands and pets if this network was
  // registered to those modes
  if (Util::IsFltGTZero(kFakeHandDetectionProbability) ||
    ...
    for (auto& mode : modes) {
      ...
      if ((VisionMode::Hands == mode) &&
          (rng.RandDbl() < kFakeHandDetectionProbability)) {
        fakeDetectionsToAdd.emplace_back(Vision::SalientPointType::Hand);
      }
      if ((VisionMode::Pets == mode) &&
          (rng.RandDbl() < kFakeCatDetectionProbability)) {
        fakeDetectionsToAdd.emplace_back(Vision::SalientPointType::Cat);
      }
      if ((VisionMode::Pets == mode) &&
          (rng.RandDbl() < kFakeDogDetectionProbability)) {
    ...
    for (const auto& type : fakeDetectionsToAdd) {
      ...
      Vision::SalientPoint salientPoint(
          atTimestamp, 0.5f, 0.5f, 1.f, 1.f, type, EnumToString(type),
          {CladPoint2d{0.f, 0.f}, CladPoint2d{0.f, 1.f}, CladPoint2d{1.f, 1.f},
           CladPoint2d{1.f, 0.f}},
          0);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void VisionSystem::UpdateRollingShutter(const VisionPoseData& poseData,
                                        const Vision::ImageCache& imageCache) {
  // If we've already updated the corrector at this timestamp, don't have to do
  // it again
  if (!_doRollingShutterCorrection ||
      (imageCache.GetTimeStamp() <= _lastRollingShutterCorrectionTime)) {
    return;
  }
...
Result VisionSystem::Update(const VisionPoseData& poseData,
  ...
  if (kVisionSystemSimulatedDelay_ms > 0) {
    std::this_thread::sleep_for(
        std::chrono::milliseconds(kVisionSystemSimulatedDelay_ms));
  }
  ...
  MarkerDetectionCLAHE useCLAHE =
      static_cast<MarkerDetectionCLAHE>(kUseCLAHE_u8);
  Vision::Image claheImage;
  ...
  ANKI_VERIFY(RESULT_OK == lastResult, "VisionSystem.Update.FailedCLAHE",
              "ApplyCLAHE supposedly has no failure mode");
  ...
  if (IsModeEnabled(VisionMode::Markers)) {
    if (IsModeEnabled(VisionMode::Markers_Off)) {
      ...
      const bool allowWhileRotatingFast =
          IsModeEnabled(VisionMode::Markers_FastRotation);
      const bool wasRotatingTooFast =
          (allowWhileRotatingFast
               ? false
               : poseData.imuDataHistory.WasRotatingTooFast(
                     imageCache.GetTimeStamp(),
      ...
      if (!wasRotatingTooFast) {
        ...
        Tic("TotalMarkers");
        lastResult = DetectMarkers(imageCache, claheImage,
                                   detectionsByMode[VisionMode::Markers],
                                   useCLAHE, poseData);
        ...
        if (RESULT_OK != lastResult) {
          ...
          visionModesProcessed.Enable(VisionMode::Markers_FastRotation,
                                      allowWhileRotatingFast);
        }
        Toc("TotalMarkers");
      }
  ...
  if (IsModeEnabled(VisionMode::Faces)) {
    const bool estimatingFacialExpression =
        IsModeEnabled(VisionMode::Faces_Expression);
    ...
    Tic("TotalFaces");
    // NOTE: To use rolling shutter in DetectFaces, call
    // UpdateRollingShutterHere See: VIC-1417 UpdateRollingShutter(poseData,
    // imageCache);
    const bool useCropping = IsModeEnabled(VisionMode::Faces_Crop);
    if ((lastResult = DetectFaces(
             imageCache, detectionsByMode[VisionMode::Faces], useCropping)) !=
        RESULT_OK) {
      ...
      visionModesProcessed.Enable(VisionMode::Faces_Expression,
  ...
  if (IsModeEnabled(VisionMode::Pets)) {
    Tic("TotalPets");
    if ((lastResult = DetectPets(
             imageCache, detectionsByMode[VisionMode::Pets])) != RESULT_OK) {
  ...
  if (IsModeEnabled(VisionMode::BrightColors)) {
    if (imageCache.HasColor()) {
      ...
      PRINT_NAMED_WARNING("VisionSystem.Update.NoColorImage",
                          "Could not process bright colors. No color image!");
    }
  }
  ...
  if (IsModeEnabled(VisionMode::OverheadMap)) {
    if (imageCache.HasColor()) {
      ...
      PRINT_NAMED_WARNING("VisionSystem.Update.NoColorImage",
                          "Could not process overhead map. No color image!");
    }
  }
  ...
  if (IsModeEnabled(VisionMode::Obstacles)) {
    if (imageCache.HasColor()) {
      ...
      PRINT_NAMED_WARNING(
          "VisionSystem.Update.NoColorImage",
          "Could not process visual obstacles. No color image!");
    }
  }
  ...
  if (IsModeEnabled(VisionMode::OverheadEdges)) {
    ...
    lastResult =
        _overheadEdgeDetector->Detect(imageCache, _poseData, _currentResult);
  ...
  if (IsModeEnabled(VisionMode::Calibration)) {
    switch (kCalibTargetType) {
      case CameraCalibrator::CalibTargetType::CHECKERBOARD: {
        lastResult = _cameraCalibrator->ComputeCalibrationFromCheckerboard(
            _currentResult.cameraCalibration, _currentResult.debugImages);
        break;
      }
      case CameraCalibrator::CalibTargetType::QBERT:
      case CameraCalibrator::CalibTargetType::INVERTED_BOX: {
        // Marker detection needs to have run before trying to do single target
        // calibration
        DEV_ASSERT(visionModesProcessed.Contains(VisionMode::Markers),
                   "VisionSystem.Update.Calibration.MarkersNotDetected");

        CameraCalibrator::CalibTargetType targetType =
            static_cast<CameraCalibrator::CalibTargetType>(kCalibTargetType);
        lastResult = _cameraCalibrator->ComputeCalibrationFromSingleTarget(
            targetType, _currentResult.observedMarkers,
            _currentResult.cameraCalibration, _currentResult.debugImages);
        break;
      }
    }
  ...
  // Check for any results from any neural nets thave have been running
  // (asynchronously). Note that any SalientPoints found will be from a
  // different image than the one in the cache and will have their own timestamp
  // which does not match the current VisionProcessingResult.
  CheckForNeuralNetResults();

  // Figure out if any modes requiring neural nets are enabled and keep up with
  // the set of networks needed to satisfy those modes (multiple modes could use
  // the same network!)
  ...
  for (const auto& mode : GetVisionModesUsingNeuralNets()) {
    if (IsModeEnabled(mode)) {
      ...
      if (ANKI_VERIFY(success, "VisionSystem.Update.NoNetworkForMode", "%s",
                      EnumToString(mode))) {
  ...
  for (const auto& networkName : networksToRun) {
    ...
    if (iter == _neuralNetRunners.end()) {
      // If the network does not exist, something has been configured / set up
      // wrong in vision_config.json or perhaps in registering network names and
      // VisionModes in visionModeHelpers.cpp. Die immediately. Don't waste time
      // sifting through logs trying to figure out why the associated features
      // isn't working.
    ...
    if (started) {
      PRINT_CH_DEBUG("NeuralNets", "VisionSystem.Update.StartedNeuralNet",
                     "Running %s on image at time t:%u", networkName.c_str(),
                     imageCache.GetTimeStamp());
    }
  }
  ...
  if (IsModeEnabled(VisionMode::Illumination) &&
      !IsModeEnabled(
          VisionMode::AutoExp_Cycling))  // don't check for illumination if
                                         // cycling exposure
  {
    Tic("Illumination");
  ...
  // NOTE: This should come after any detectors that add things to
  // "detectionRects"
  ...
  if (IsModeEnabled(VisionMode::SaveImages) &&
      _imageSaver->WantsToSave(_currentResult, imageCache.GetTimeStamp())) {
    Tic("SaveImages");

    // Check this before calling Save(), since that can modify imageSaver's
    // state
    ...
    const Result saveSensorResult =
        (shouldSaveSensorData ? SaveSensorData() : RESULT_OK);

    Toc("SaveImages");

    if ((RESULT_OK != saveResult) ||
        (RESULT_OK != saveSensorResult))  // || (RESULT_OK != thumbnailResult))
    {
      PRINT_NAMED_ERROR("VisionSystem.Update.SaveImagesFailed",
                        "Image:%s SensorData:%s",
  ...
  if (IsModeEnabled(VisionMode::Viz)) {
    ...
    _currentResult.compressedDisplayImg.Compress(
        imageCache.GetRGB(_vizImageBroadcastSize), _imageCompressQuality);
  ...
  // NOTE: This should come at the end because it relies on elements of the
  // current VisionProcessingResult
  //       (i.e. _currentResult) to be populated for the purposes of drawing
  //       them. Note that any asynchronous results (e.g. from neural nets) get
  //       drawn on whatever image is current when they complete, for better or
  //       worse, so they are not actually in sync.
  if (IsModeEnabled(VisionMode::MirrorMode)) {
    // TODO: Add an ImageCache::Size for MirrorMode directly
    const Result result = _mirrorModeManager->CreateMirrorModeImage(
        imageCache.GetRGB(), _currentResult);
    if (RESULT_OK != result) {
  ...
  // Push the result, along with any neural net results, onto the queue of
...
Result VisionSystem::SaveSensorData() const {
  ...
  PRINT_CH_DEBUG(kLogChannelName, "VisionSystem.SaveSensorData.Filename",
                 "Saving to %s", fullFilename.c_str());

  std::ofstream outFile(fullFilename);
  if (!outFile.is_open()) {
    PRINT_NAMED_ERROR("VisionSystem.SaveSensorData.CantOpenFile",
                      "Can't open file %s for writing", fullFilename.c_str());
    return RESULT_FAIL;
  }
  ...
  Json::Value config;
  {
    ...
    config["requestedCamWhiteBalanceRed"] =
        _currentCameraParams.whiteBalanceGainR;
    config["requestedCamWhiteBalanceGreen"] =
        _currentCameraParams.whiteBalanceGainG;
    config["requestedCamWhiteBalanceBlue"] =
...
Vision::CameraParams VisionSystem::GetCurrentCameraParams() const {
  ...
  return (_nextCameraParams.first ? _nextCameraParams.second
...
Result VisionSystem::SetCameraExposureParams(const s32 currentExposureTime_ms,
  ...
  // TODO: Expose these x values ("knee locations") somewhere. These are
  // specific to the camera. (So I'm keeping them out of Vision::ImagingPipeline
  // and defined in Cozmo namespace)
  ...
  Result result =
      _cameraParamsController->SetGammaTable(kKneeLocations, gammaVector);
  if (RESULT_OK != result) {
    PRINT_NAMED_WARNING("VisionSystem.SetCameraExposureParams.BadGammaCurve",
                        "");
  }

  const Vision::CameraParams cameraParams(
  ...
  PRINT_CH_INFO(kLogChannelName, "VisionSystem.SetCameraExposureParams.Success",
                "Current Exposure Time:%dms, Gain:%.3f", currentExposureTime_ms,
                currentGain);
...
Result VisionSystem::GetSerializedFaceData(std::vector<u8>& albumData,
  ...
  DEV_ASSERT(nullptr != _faceTracker,
             "VisionSystem.GetSerializedFaceData.NullFaceTracker");
  return _faceTracker->GetSerializedData(albumData, enrollData);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result VisionSystem::SetSerializedFaceData(
    const std::vector<u8>& albumData, const std::vector<u8>& enrollData,
    std::list<Vision::LoadedKnownFace>& loadedFaces) {
  DEV_ASSERT(nullptr != _faceTracker,
             "VisionSystem.SetSerializedFaceData.NullFaceTracker");
  return _faceTracker->SetSerializedData(albumData, enrollData, loadedFaces);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result VisionSystem::LoadFaceAlbum(
    const std::string& albumName,
    std::list<Vision::LoadedKnownFace>& loadedFaces) {
  DEV_ASSERT(nullptr != _faceTracker,
             "VisionSystem.LoadFaceAlbum.NullFaceTracker");
...
Result VisionSystem::SaveFaceAlbum(const std::string& albumName) {
  DEV_ASSERT(nullptr != _faceTracker,
             "VisionSystem.SaveFaceAlbum.NullFaceTracker");
...
void VisionSystem::SetFaceRecognitionIsSynchronous(bool isSynchronous) {
  DEV_ASSERT(nullptr != _faceTracker,
             "VisionSystem.SetFaceRecognitionRunMode.NullFaceTracker");
  _faceTracker->SetRecognitionIsSynchronous(isSynchronous);
}

void VisionSystem::ClearImageCache() { _imageCache->ReleaseMemory(); }
