 ...
 * Date:   2017-04-25
 *
 * Description: Vision system component for detecting motion in images and/or on
 *the ground plane.
 *
 *
...
// For speed, compute motion detection at lower resolution (1 for full
// resolution, 2 for half, etc)
CONSOLE_VAR_RANGED(s32, kMotionDetection_ScaleMultiplier, CONSOLE_GROUP_NAME, 4,
                   1, 8);

// How long we have to wait between motion detections. This may be reduce-able,
// but can't get too small or we'll hallucinate image change (i.e. "motion") due
// to the robot moving.
CONSOLE_VAR(u32, kMotionDetection_LastMotionDelay_ms, CONSOLE_GROUP_NAME, 500);

// Affects sensitivity (darker pixels are inherently noisier and should be
// ignored for change detection). Range is [0,255]
WRAP_EXTERN_CONSOLE_VAR(u8, kMotionDetection_MinBrightness, CONSOLE_GROUP_NAME);

// This is the main sensitivity parameter: higher means more image difference is
// required to register a change and thus report motion.
WRAP_EXTERN_CONSOLE_VAR(f32, kMotionDetection_RatioThreshold,
                        CONSOLE_GROUP_NAME);
CONSOLE_VAR(f32, kMotionDetection_MinAreaFraction, CONSOLE_GROUP_NAME,
            1.f / 225.f);  // 1/15 of each image dimension

// For computing robust "centroid" of motion
CONSOLE_VAR(f32, kMotionDetection_CentroidPercentileX, CONSOLE_GROUP_NAME,
            0.5f);  // In image coordinates
CONSOLE_VAR(f32, kMotionDetection_CentroidPercentileY, CONSOLE_GROUP_NAME,
            0.5f);  // In image coordinates
CONSOLE_VAR(f32, kMotionDetection_GroundCentroidPercentileX, CONSOLE_GROUP_NAME,
            0.05f);  // In robot coordinates (Most important for pounce:
                     // distance from robot)
CONSOLE_VAR(f32, kMotionDetection_GroundCentroidPercentileY, CONSOLE_GROUP_NAME,
            0.50f);  // In robot coordinates

// Tight constraints on max movement allowed to attempt frame differencing for
// "motion detection"
CONSOLE_VAR(f32, kMotionDetection_MaxHeadAngleChange_deg, CONSOLE_GROUP_NAME,
            0.1f);
CONSOLE_VAR(f32, kMotionDetection_MaxBodyAngleChange_deg, CONSOLE_GROUP_NAME,
            0.1f);
CONSOLE_VAR(f32, kMotionDetection_MaxPoseChange_mm, CONSOLE_GROUP_NAME, 0.5f);

CONSOLE_VAR(bool, kMotionDetection_DrawGroundDetectionsInCameraView,
            CONSOLE_GROUP_NAME, false);

// The smaller this value the more broken up will be the motion areas, leading
// to fragmented ones. If too big artificially big motion areas can be created.
CONSOLE_VAR(u32, kMotionDetection_MorphologicalSize_pix, CONSOLE_GROUP_NAME,
            20);

// The higher this value the less susceptible to noise motion detection will be.
// A too high value will lead to discarding some motion areas.
CONSOLE_VAR(u32, kMotionDetection_MinAreaForMotion_pix, CONSOLE_GROUP_NAME,
            500);
...
// This class is used to accumulate data for peripheral motion detection. The
// image area is divided in three sections: top, right and left. If the centroid
// of a motion patch falls inside one these areas, it's increased, otherwise
// it's decreased. This follows a very simple impulse/decay model. The
// parameters *horizontalSize* and *verticalSize* control how much of the image
// is for the left/right sectors and the top/bottom sector. The parameters
// *increaseFactor* and *decreaseFactor* control the impulse response.
//
// The centroid of the motion in the different sectors are also stored here.
// Every time one of the areas is activated, the centroid "moves" towards the
// new activation following an exponential moving average method.
class MotionDetector::ImageRegionSelector {
 public:
  ImageRegionSelector(int imageWidth, int imageHeight, float horizontalSize,
                      float verticalSize, float increaseFactor,
                      float decreaseFactor, float maxValue, float alpha)
      : _alpha(alpha),
        _maxValue(maxValue),
    ...
    DEV_ASSERT(horizontalSize <= 0.5,
               "MotionDetector::ImageRegionSelector: horizontal size has to be "
               "less then half"
               "of the image");
    DEV_ASSERT(verticalSize <= 0.5,
               "MotionDetector::ImageRegionSelector: vertical size has to be "
               "less then half"
               "of the image");
  }

  float GetTopResponse() const { return _topID.Value(); }
  float GetBottomResponse() const { return _bottomID.Value(); }
  float GetLeftResponse() const { return _leftID.Value(); }
  float GetRightResponse() const { return _rightID.Value(); }

  float GetTopActivationArea() const { return _topActivatedArea; }
  float GetBottomActivationArea() const { return _bottomActivatedArea; }
  float GetLeftActivationArea() const { return _leftActivatedArea; }
  float GetRightActivationArea() const { return _rightActivatedArea; }

  bool IsTopActivated() const { return GetTopResponse() >= _maxValue; }
  bool IsBottomActivated() const { return GetBottomResponse() >= _maxValue; }
  bool IsRightActivated() const { return GetRightResponse() >= _maxValue; }
  bool IsLeftActivated() const { return GetLeftResponse() >= _maxValue; }

  const Point2f& GetTopResponseCentroid() const { return _topCentroid; }
  const Point2f& GetBottomResponseCentroid() const { return _bottomCentroid; }
  const Point2f& GetLeftResponseCentroid() const { return _leftCentroid; }
  const Point2f& GetRightResponseCentroid() const { return _rightCentroid; }
  ...
  void Update(const Point2f& point, float value) {
 ...
 private:
  ...
  const float
      _bottomArea;        // area of bottom of the image (down below _topMargin)
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
MotionDetector::MotionDetector(const Vision::Camera& camera,
                               VizManager* vizManager,
                               const Json::Value& config)
    : _regionSelector(
          nullptr)  // need image size information before we can build this
      ,
      _camera(camera),
      _vizManager(vizManager),
      _config(config) {
  DEV_ASSERT(kMotionDetection_MinBrightness > 0,
             "MotionDetector.Constructor.MinBrightnessIsZero");
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
s32 MotionDetector::RatioTest(const Vision::ImageRGB& image,
                              Vision::Image& ratioImg) {
  DEV_ASSERT(ratioImg.GetNumRows() == image.GetNumRows() &&
                 ratioImg.GetNumCols() == image.GetNumCols(),
  ...
  // somehow auto doens't work here... the right type cannot be deduced. Bug in
  // clang?
  std::function<u8(const Vision::PixelRGB& thisElem,
                   const Vision::PixelRGB& otherElem)>
      ratioTest = [&numAboveThresh](const Vision::PixelRGB& p1,
                                    const Vision::PixelRGB& p2) {
        u8 retVal = 0;
...
s32 MotionDetector::RatioTest(const Vision::Image& image,
                              Vision::Image& ratioImg) {
  DEV_ASSERT(ratioImg.GetNumRows() == image.GetNumRows() &&
                 ratioImg.GetNumCols() == image.GetNumCols(),
  ...
  std::function<u8(const u8& thisElem, const u8& otherElem)> ratioTest =
      [&numAboveThresh](const u8& p1, const u8& p2) {
        u8 retVal = 0;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result MotionDetector::Detect(
    Vision::ImageCache& imageCache, const VisionPoseData& crntPoseData,
  ...
  const Vision::ImageCacheSize imageSize =
      Vision::ImageCache::GetSize(kMotionDetection_ScaleMultiplier);
  ...
  if (imageCache.HasColor()) {
    ...
    return DetectHelper(imageColor,
                        ...
                        kMotionDetection_ScaleMultiplier, crntPoseData,
                        prevPoseData, observedMotions, debugImages);
  } else {
...
Result MotionDetector::DetectHelper(
    const ImageType& image, s32 origNumRows, s32 origNumCols,
    f32 scaleMultiplier, const VisionPoseData& crntPoseData,
    const VisionPoseData& prevPoseData,
    std::list<ExternalInterface::RobotObservedMotion>& observedMotions,
    Vision::DebugImageList<Vision::CompressedImage>& debugImages) {
  // Create the ImageRegionSelector. It has to be done here since the image size
  // is not known before
...
#define GET_JSON_PARAMETER(__json__, __fieldName__, __variable__)              \
  do {                                                                         \
    if (!JsonTools::GetValueOptional(__json__, __fieldName__, __variable__)) { \
      PRINT_NAMED_ERROR("MotionDetection.DetectHelper.MissingJsonParameter",   \
                        "%s", __fieldName__);                                  \
      return RESULT_FAIL;                                                      \
    }                                                                          \
  } while (0)
    ...
    GET_JSON_PARAMETER(detectionConfig, "CentroidStability",
                       kCentroidStability);

    _regionSelector.reset(new ImageRegionSelector(
        image.GetNumCols(), image.GetNumRows(), kHorizontalSize, kVerticalSize,
        kIncreaseFactor, kDecreaseFactor, kMaxValue, kCentroidStability));
  }

  const bool headSame = crntPoseData.IsHeadAngleSame(
      prevPoseData, DEG_TO_RAD(kMotionDetection_MaxHeadAngleChange_deg));

  const bool poseSame = crntPoseData.IsBodyPoseSame(
      prevPoseData, DEG_TO_RAD(kMotionDetection_MaxBodyAngleChange_deg),
  ...
  const bool longEnoughSinceLastMotion =
      ((image.GetTimestamp() - _lastMotionTime) >
       kMotionDetection_LastMotionDelay_ms);

  if (headSame && poseSame && HavePrevImage<ImageType>() &&
      !crntPoseData.histState.WasCameraMoving() &&
      !crntPoseData.histState.WasPickedUp() && longEnoughSinceLastMotion) {
    ...
    Vision::Image foregroundMotion(blurredImage.GetNumRows(),
                                   blurredImage.GetNumCols());
    s32 numAboveThresh = RatioTest(blurredImage, foregroundMotion);

    // Run the peripheral motion detection
    const bool peripheralMotionDetected = DetectPeripheralMotionHelper(
        foregroundMotion, debugImages, msg, scaleMultiplier);

    const bool groundMotionDetected = DetectGroundAndImageHelper(
        foregroundMotion, numAboveThresh, origNumRows, origNumCols,
        scaleMultiplier, crntPoseData, prevPoseData, observedMotions,
        debugImages, msg);

    if (peripheralMotionDetected || groundMotionDetected) {
      if (kMotionDetectionDebug) {
        PRINT_CH_INFO(kLogChannelName,
                      "MotionDetector.DetectMotion.DetectHelper",
                      "Motion found, sending message");
      }
      observedMotions.emplace_back(std::move(msg));
    }

    // Store a blurred copy of the current image for next time (at correct
    // resolution!)
...
bool MotionDetector::DetectGroundAndImageHelper(
    Vision::Image& foregroundMotion, int numAboveThresh, s32 origNumRows,
    s32 origNumCols, f32 scaleMultiplier, const VisionPoseData& crntPoseData,
    const VisionPoseData& prevPoseData,
    std::list<ExternalInterface::RobotObservedMotion>& observedMotions,
    Vision::DebugImageList<Vision::CompressedImage>& debugImages,
    ExternalInterface::RobotObservedMotion& msg) {
  ...
  const size_t minArea = std::round((f32)foregroundMotion.GetNumElements() *
                                    kMotionDetection_MinAreaFraction);
  f32 imgRegionArea = 0.f;
  f32 groundRegionArea = 0.f;
  if (numAboveThresh > minArea) {
    imgRegionArea = GetCentroid(foregroundMotion, centroid,
  ...
  if (crntPoseData.groundPlaneVisible && prevPoseData.groundPlaneVisible) {
    ExtractGroundPlaneMotion(origNumRows, origNumCols, scaleMultiplier,
                             crntPoseData, foregroundMotion, centroid,
                             groundPlaneCentroid, groundRegionArea);
  }
  ...
  if (imgRegionArea > 0 || groundRegionArea > 0.f) {
    ...
    if (kMotionDetectionDebug) {
      PRINT_CH_INFO(
          kLogChannelName,
          "MotionDetector.DetectGroundAndImageHelper.FoundCentroid",
          "Found motion centroid for %.1f-pixel area region at (%.1f,%.1f) "
          "-- %.1f%% of ground area at (%.1f,%.1f)",
          imgRegionArea, centroid.x(), centroid.y(), groundRegionArea * 100.f,
          groundPlaneCentroid.x(), groundPlaneCentroid.y());
    }

    if (kMotionDetection_DrawGroundDetectionsInCameraView &&
        (nullptr != _vizManager)) {
      const f32 radius =
          std::max(1.f, sqrtf(groundRegionArea *
                              (f32)foregroundMotion.GetNumElements() / M_PI_F));
      _vizManager->DrawCameraOval(centroid * scaleMultiplier, radius, radius,
                                  NamedColors::YELLOW);
    }

    if (imgRegionArea > 0) {
      DEV_ASSERT(centroid.x() >= 0.f &&
                     centroid.x() <= foregroundMotion.GetNumCols() &&
                     centroid.y() >= 0.f &&
                     centroid.y() <= foregroundMotion.GetNumRows(),
                 "MotionDetector.DetectGroundAndImageHelper.CentroidOOB");

      // Using scale multiplier to return the coordinates in original image
      // coordinates
      msg.img_x = int16_t(std::round(centroid.x() * scaleMultiplier));
      msg.img_y = int16_t(std::round(centroid.y() * scaleMultiplier));
      msg.img_area =
          imgRegionArea / static_cast<f32>(foregroundMotion.GetNumElements());
    } else {
      msg.img_area = 0;
      msg.img_x = 0;
      msg.img_y = 0;
    ...
    if (kMotionDetectionDebug) {
      ...
      ratioImgDisp.DrawCircle(
          centroid +
              (_camera.GetCalibration()->GetCenter() * (1.f / scaleMultiplier)),
          NamedColors::RED, 4);
      snprintf(tempText, 127, "Area:%.2f X:%d Y:%d", imgRegionArea, msg.img_x,
               msg.img_y);
      putText(ratioImgDisp.get_CvMat_(), std::string(tempText),
              cv::Point(0, ratioImgDisp.GetNumRows()), CV_FONT_NORMAL, .4f,
              CV_RGB(0, 255, 0));
      debugImages.emplace_back("RatioImg", ratioImgDisp);

      //_currentResult.debugImages.push_back({"PrevRatioImg", _prevRatioImg});
      //_currentResult.debugImages.push_back({"ForegroundMotion",
      //foregroundMotion}); _currentResult.debugImages.push_back({"AND",
      //cvAND});

      Vision::Image foregroundMotionFullSize(origNumRows, origNumCols);
      foregroundMotion.Resize(foregroundMotionFullSize,
                              Vision::ResizeMethod::NearestNeighbor);
      Vision::ImageRGB ratioImgDispGround(
          crntPoseData.groundPlaneROI.GetOverheadImage(
              foregroundMotionFullSize, crntPoseData.groundPlaneHomography));
      if (groundRegionArea > 0.f) {
        Point2f dispCentroid(groundPlaneCentroid.x(),
                             -groundPlaneCentroid.y());  // Negate Y for display
        ratioImgDispGround.DrawCircle(
            dispCentroid - crntPoseData.groundPlaneROI.GetOverheadImageOrigin(),
            NamedColors::RED, 2);
        snprintf(tempText, 127, "Area:%.2f X:%d Y:%d", groundRegionArea,
                 msg.ground_x, msg.ground_y);
        putText(ratioImgDispGround.get_CvMat_(), std::string(tempText),
                cv::Point(0, crntPoseData.groundPlaneROI.GetWidthFar()),
                CV_FONT_NORMAL, .4f, CV_RGB(0, 255, 0));
      }
...
void MotionDetector::ExtractGroundPlaneMotion(
    s32 origNumRows, s32 origNumCols, f32 scaleMultiplier,
    const VisionPoseData& crntPoseData, const Vision::Image& foregroundMotion,
    const Point2f& centroid, Point2f& groundPlaneCentroid,
    f32& groundRegionArea) const {
  Quad2f imgQuad;
  crntPoseData.groundPlaneROI.GetImageQuad(crntPoseData.groundPlaneHomography,
                                           origNumCols, origNumRows, imgQuad);
  ...
  // Zero out everything in the ratio image that's not inside the ground plane
  // quad
  ...
  mask.FillWith(0);
  fillConvexPoly(mask.get_CvMat_(),
                 std::vector<cv::Point>{
  ...
  groundRegionArea =
      GetCentroid(groundPlaneForegroundMotion, groundPlaneCentroid,
  ...
  groundPlaneCentroid +=
      boundingRect.GetTopLeft().CastTo<float>();  // casting is explicit

  /* Experimental: Try computing moments in an overhead warped view of the ratio
   image groundPlaneRatioImg =
   _poseData.groundPlaneROI.GetOverheadImage(ratioImg,
   _poseData.groundPlaneHomography);

   cv::Moments moments = cv::moments(groundPlaneRatioImg.get_CvMat_(), true);
   if(moments.m00 > 0) {
   groundMotionAreaFraction = moments.m00 /
   static_cast<f32>(groundPlaneRatioImg.GetNumElements());
  ...
  if (groundRegionArea > 0.f) {
    ...
    DEV_ASSERT(Util::IsFltGTZero(imgQuadArea),
               "MotionDetector.Detect.QuadWithZeroArea");
    ...
    if (RESULT_OK != solveResult) {
      PRINT_NAMED_WARNING(
          "MotionDetector.DetectMotion.LeastSquaresFailed",
      ...
      PRINT_NAMED_WARNING(
          "MotionDetector.DetectMotion.BadProjectedZ",
          "z<=0 (%f) when projecting motion centroid to ground. Bad homography "
          "at head angle %.3fdeg?",
          homographyMappedPoint.z(),
          RAD_TO_DEG(crntPoseData.histState.GetHeadAngle_rad()));
      // Don't report this centroid
      groundRegionArea = 0.f;
      groundPlaneCentroid = 0.f;
    } else {
      ...
      if (ANKI_DEVELOPER_CODE) {
        ...
        if (!testQuad.Contains(groundPlaneCentroid)) {
          PRINT_NAMED_WARNING(
              "MotionDetector.DetectMotion.BadGroundPlaneCentroid",
...
bool MotionDetector::DetectPeripheralMotionHelper(
    Vision::Image& ratioImage,
    Vision::DebugImageList<Vision::CompressedImage>& debugImages,
  ...
  // The image has several disjoint components, try to join them
  {
    const int kernelSize =
        int(kMotionDetection_MorphologicalSize_pix / scaleMultiplier);
    ...
    cv::morphologyEx(cvRatioImage, cvRatioImage, cv::MORPH_CLOSE,
                     structuringElement);
  }
  ...
  // The update is done per connected component, which means that several areas
  // might activate at once
  bool updated = false;
  for (uint i = 1; i < stats.size();
       ++i) {  // skip stats[0] since that's background
    ...
    PRINT_CH_DEBUG(kLogChannelName,
                   "MotionDetector.DetectPeripheralMotionHelper.MotionDetected",
                   "Motion detected with an area of %d (scaled %f)",
                   int(stat.area), scaledArea);
  }
  ...
  if (!updated) {
    ...
    PRINT_CH_DEBUG(
        kLogChannelName,
        "MotionDetector.DetectPeripheralMotionHelper.NoMotionDetected", "");
  }
  ...
  // Filling the message
  {
    ...
    if (_regionSelector->IsTopActivated()) {
      ...
      DEV_ASSERT_MSG(
          value > 0,
          "MotionDetector::DetectPeripheralMotionHelper.WrongTopValue",
    ...
    if (_regionSelector->IsBottomActivated()) {
      ...
      DEV_ASSERT_MSG(
          value > 0,
          "MotionDetector::DetectPeripheralMotionHelper.WrongBottomValue",
    ...
    if (_regionSelector->IsLeftActivated()) {
      ...
      DEV_ASSERT_MSG(
          value > 0,
          "MotionDetector::DetectPeripheralMotionHelper.WrongLeftValue",
    ...
    if (_regionSelector->IsRightActivated()) {
      ...
      DEV_ASSERT_MSG(
          value > 0,
          "MotionDetector::DetectPeripheralMotionHelper.WrongRightValue",
  ...
  if (kMotionDetectionDebug) {
    ...
    // Draw the text
    {
      ...
      if (_regionSelector->IsBottomActivated()) {
        ...
        const std::string& text = to_string_with_precision(value, 3);
        const Point2f origin(imageToDisplay.GetNumCols() / 2 - 10,
                             imageToDisplay.GetNumRows() - 30);
      ...
      if (_regionSelector->IsRightActivated()) {
        ...
        const std::string& text = to_string_with_precision(value, 3);
        const Point2f origin(imageToDisplay.GetNumCols() - 50,
                             imageToDisplay.GetNumRows() / 2);
    ...
    {  // Top line
      ...
      const Point2f topRight(imageToDisplay.GetNumCols(),
                             _regionSelector->GetTopMargin());
      imageToDisplay.DrawLine(topLeft, topRight, Anki::NamedColors::RED,
                              thickness);
    }
    {  // Bottom line
      const int thickness = 1;
      ...
      const Point2f topRight(imageToDisplay.GetNumCols(),
                             _regionSelector->GetBottomMargin());
      imageToDisplay.DrawLine(topLeft, topRight, Anki::NamedColors::RED,
                              thickness);
    }
    {  // Left line
      ...
      const Point2f bottomLeft(_regionSelector->GetLeftMargin(),
                               imageToDisplay.GetNumRows());
      imageToDisplay.DrawLine(topLeft, bottomLeft, Anki::NamedColors::RED,
                              thickness);
    }
    {  // Right line
      const int thickness = 1;
      const Point2f topRight(_regionSelector->GetRightMargin(), 0);
      const Point2f BottomRight(_regionSelector->GetRightMargin(),
                                imageToDisplay.GetNumCols());
      imageToDisplay.DrawLine(topRight, BottomRight, Anki::NamedColors::RED,
                              thickness);
    }
    ...
    {
      if (_regionSelector->IsTopActivated()) {
        const Point2f centroid(msg.top_img_x / scaleMultiplier,
                               msg.top_img_y / scaleMultiplier);
        imageToDisplay.DrawFilledCircle(centroid, Anki::NamedColors::RED, 10);
      }
      if (_regionSelector->IsBottomActivated()) {
        const Point2f centroid(msg.bottom_img_x / scaleMultiplier,
                               msg.bottom_img_y / scaleMultiplier);
        imageToDisplay.DrawFilledCircle(centroid, Anki::NamedColors::BLUE, 10);
      }
      if (_regionSelector->IsLeftActivated()) {
        const Point2f centroid(msg.left_img_x / scaleMultiplier,
                               msg.left_img_y / scaleMultiplier);
        imageToDisplay.DrawFilledCircle(centroid, Anki::NamedColors::YELLOW,
                                        10);
      }
      if (_regionSelector->IsRightActivated()) {
        const Point2f centroid(msg.right_img_x / scaleMultiplier,
                               msg.right_img_y / scaleMultiplier);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - Explicit instantiation of Detect()
// method for Gray and RGB images
template Result MotionDetector::DetectHelper(
    const Vision::Image&, s32, s32, f32, const VisionPoseData&,
    const VisionPoseData&, std::list<ExternalInterface::RobotObservedMotion>&,
    Vision::DebugImageList<Vision::CompressedImage>&);

template Result MotionDetector::DetectHelper(
    const Vision::ImageRGB&, s32, s32, f32, const VisionPoseData&,
    const VisionPoseData&, std::list<ExternalInterface::RobotObservedMotion>&,
    Vision::DebugImageList<Vision::CompressedImage>&);

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - Computes "centroid" at specified
// percentiles in X and Y
size_t MotionDetector::GetCentroid(const Vision::Image& motionImg,
                                   Point2f& centroid, f32 xPercentile,
                                   f32 yPercentile) {
  ...
  DEV_ASSERT(xValues.size() == yValues.size(),
             "MotionDetector.GetCentroid.xyValuesSizeMismatch");

  if (xValues.empty()) {
    centroid = 0.f;
    return 0;
  } else {
    DEV_ASSERT(xPercentile >= 0.f && xPercentile <= 1.f,
               "MotionDetector.GetCentroid.xPercentileOOR");
    DEV_ASSERT(yPercentile >= 0.f && yPercentile <= 1.f,
               "MotionDetector.GetCentroid.yPercentileOOR");
    ...
    DEV_ASSERT_MSG(centroid.x() >= 0.f && centroid.x() < motionImg.GetNumCols(),
                   ...
                   "xcen=%f, not in [0,%d)", centroid.x(),
                   motionImg.GetNumCols());
    DEV_ASSERT_MSG(centroid.y() >= 0.f && centroid.y() < motionImg.GetNumRows(),
                   "MotionDetector.GetCentroid.yCenOOR",
                   "ycen=%f, not in [0,%d)", centroid.y(),
                   motionImg.GetNumRows());
    return area;
  }
}
// GetCentroid()
