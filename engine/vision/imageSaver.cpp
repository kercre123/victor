 ...
 * Date:   06/07/2018
 *
 * Description: Class for saving image data according to a variety of
 *parameters. Can optionally create thumbnails and also undistort images.
 *
...
ImageSaverParams::ImageSaverParams(
    const std::string& pathIn, Mode saveModeIn, int8_t qualityIn,
    const std::string& basenameIn, Vision::ImageCacheSize sizeIn,
    float thumbnailScaleIn, float saveScaleIn, bool removeDistortionIn,
    uint8_t medianFilterSizeIn, float sharpeningAmountIn)
    : path(pathIn),
      basename(basenameIn),
      mode(saveModeIn),
      quality(qualityIn),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ImageSaverParams::SaveConditionTypeFromString(
    const std::string& str, SaveConditionType& saveCondType) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Required here in the .cpp b/c of use
// of unique_ptr and forward declaration of Undistorter
ImageSaver::ImageSaver() = default;
ImageSaver::~ImageSaver() = default;

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ImageSaver::SetCalibration(
    const std::shared_ptr<Vision::CameraCalibration>& camCalib) {
...
Result ImageSaver::SetParams(const ImageSaverParams& params) {
  ...
  if (params.quality != -1 &&
      !Util::InRange(params.quality, int8_t(0), int8_t(100))) {
    PRINT_NAMED_ERROR("ImageSaver.SetParams.BadQuality",
                      "Should be -1 or [0,100], not %d", params.quality);
    return RESULT_FAIL;
  }

  if (!Util::InRange(params.thumbnailScale, 0.f, 1.f)) {
    PRINT_NAMED_ERROR("ImageSaver.SetParams.BadThumbnailScale",
                      "Should be [0.0, 1.0], not %.3f", params.thumbnailScale);
    return RESULT_FAIL;
  }

  if (params.removeDistortion && !_undistorter) {
    PRINT_NAMED_ERROR(
        "ImageSaver.SetParams.NeedUndistorter",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ImageSaver::WantsToSave() const { return (_params.mode != Mode::Off); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ImageSaver::WantsToSave(const VisionProcessingResult& result,
...
bool ImageSaver::ShouldSaveSensorData() const {
  return (Mode::SingleShotWithSensorData == _params.mode) ||
         (Mode::Stream == _params.mode);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result ImageSaver::Save(const Vision::ImageRGB& inputImg,
                        const s32 frameNumber) {
  const std::string fullFilename =
      GetFullFilename(frameNumber, GetExtension(_params.quality));

  PRINT_CH_INFO(kLogChannelName, "ImageSaver.Save.SavingImage",
                "Saving image with timestamp %u to %s", inputImg.GetTimestamp(),
                fullFilename.c_str());
  ...
  if (_params.removeDistortion) {
    ...
    const Result undistortResult =
        _undistorter->UndistortImage(sizedImage, undistortedImage);
  ...
  if (_params.medianFilterSize > 0) {
    ...
    try {
      cv::medianBlur(sizedImage.get_CvMat_(), smoothedImage.get_CvMat_(),
                     _params.medianFilterSize);
    } catch (cv::Exception& e) {
      PRINT_NAMED_ERROR("ImageSaver.Save.OpenCvMedianBlurFailed",
                        "%s (ksize=%d)", e.what(),
                        (int)_params.medianFilterSize);
  ...
  if (Util::IsFltGTZero(_params.sharpeningAmount)) {
    ...
    try {
      cv::GaussianBlur(sizedImage.get_CvMat_(), imgBlur.get_CvMat_(),
                       cv::Size(3, 3), 1.0);
      ...
      PRINT_NAMED_ERROR("ImageSaver.Save.SharpenFailed", "%s", e.what());
    }
  }
  ...
  if ((RESULT_OK == saveResult) && Util::IsFltGTZero(_params.thumbnailScale)) {
    const std::string fullFilename =
        GetFullFilename(frameNumber, GetThumbnailExtension(_params.quality));
    Vision::ImageRGB thumbnail;
    sizedImage.Resize(_params.thumbnailScale);
    thumbnailResult = sizedImage.Save(fullFilename);
  }

  if ((ImageSendMode::SingleShot == _params.mode) ||
      (ImageSendMode::SingleShotWithSensorData == _params.mode)) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::string ImageSaver::GetFullFilename(const s32 frameNumber,
                                        const char* extension) const {
  std::string fullFilename;
  if (_params.basename.empty()) {
    // No base name provided: Use zero-padded frame number as filename
    const std::string filename(GetZeroPaddedFrameNumber(frameNumber) + "." +
                               extension);
    fullFilename = Util::FileUtils::FullFilePath({_params.path, filename});
  } else {
    // Add the specified extension to the specified base name. Include frame
    // number iff streaming.
    std::string basename(_params.basename);
    if (ImageSendMode::Stream == _params.mode) {
      basename += "_";
      basename += GetZeroPaddedFrameNumber(frameNumber);
    }
    fullFilename = Util::FileUtils::FullFilePath(
        {_params.path, basename + "." + extension});
  }
