...
#define PER_CHANNEL_RATIO(channel1, channel2)                                  \
  {                                                                            \
    ...
    valueRf32x4_1_2 = vmulq_f32(valueRf32x4_1_2, valueRf32x4_2_2);             \
                                                                               \
    /* Figure out which ratios are greater than kMotionThresh and OR them into \
     * the WhichAboveThresh vec */                                             \
...
template <>
inline s32 MotionDetector::RatioTestNeonHelper<Vision::ImageRGB>(
    const u8*& imagePtr, const u8*& prevImagePtr, u8*& ratioImgPtr,
  ...
  const float32x4_t kMotionThresh =
      vdupq_n_f32(kMotionDetection_RatioThreshold);

  const u32 kNumElementsProcessedPerLoop = 8;
  const u32 kSizeOfRGBElement = 3;
  const s32 kNumIterations =
      numElementsToProcess - (kNumElementsProcessedPerLoop - 1);
  ...
  for (i = 0; i < kNumIterations; i += kNumElementsProcessedPerLoop) {
    ...
    // Compare all channels to figure out which elements are greater than min
    // brightness in p1 (current image)
    ...
    // AND the results of the min brightness comparison for each element of each
    // channel to figure out which pixels have all r, g, and b greater than min
    // brightness
    ...
    // we will still be performing the ratio test on them and need the result to
    // be 0
    ...
    // Setup value1 to be the numerator of the ratio and value2 to be the
    // denominator based on which channel of each pixel is larger in order to
    // have a ratio that is always >= 1
    ...
    // Element will be all 1s if the ratio in any channel is above
    // RatioThreshold
    ...
    // Each element will either be all 1s (255) if the ratio of any channel of a
    // given pair of pixels is greater than the ratio threshold or all 0s
    ...
    // pixelVal which will be the number of pixels that are above the ratio
    // threshold
    pixelVal = vpadd_u8(pixelVal, pixelVal);
    pixelVal = vpadd_u8(pixelVal, pixelVal);
    pixelVal = vpadd_u8(pixelVal, pixelVal);

    // All lanes of pixelVal will be the same so pull out one of them add it to
    // numAboveThresh
    numAboveThresh += vget_lane_u8(pixelVal, 0);
  }

  const Vision::PixelRGB* imagePtr2 =
      reinterpret_cast<const Vision::PixelRGB*>(imagePtr);
  const Vision::PixelRGB* prevImagePtr2 =
      reinterpret_cast<const Vision::PixelRGB*>(prevImagePtr);
...
template <>
inline s32 MotionDetector::RatioTestNeonHelper<Vision::Image>(
    const u8*& imagePtr, const u8*& prevImagePtr, u8*& ratioImgPtr,
    u32 numElementsToProcess) {
  s32 numAboveThresh = 0;
...
s32 MotionDetector::RatioTestNeon(const ImageType& image,
                                  Vision::Image& ratioImg) {
  s32 numAboveThresh = 0;
  ...
  for (int i = 0; i < numRows; i++) {
    ...
    const u8* prevImagePtr =
        reinterpret_cast<const u8*>(_prevImageRGB.GetRow(i));
    u8* ratioImgPtr = reinterpret_cast<u8*>(ratioImg.GetRow(i));

    numAboveThresh += RatioTestNeonHelper<ImageType>(
        imagePtr, prevImagePtr, ratioImgPtr, numElementsToProcessAtATime);
  }
