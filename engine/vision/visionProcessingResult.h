 ...
 * Date:   12/10/2018
 *
 * Description: Everything that can be generated from one image in one big
 *package.
 *
...
struct VisionProcessingResult {
  RobotTimeStamp_t timestamp;  // Always set, even if all the lists below are
                               // empty (e.g. nothing is found)
  ...
  // Returns true if there is a detection for the corresponding mode present in
  // this result. The detection(s) must match the given timestamp as well (since
  // in some cases, such as salientPoints,
  //  a detections' timestamp may not match the VisionProcessingResult's
  //  timestamp).
  // Note that this method may not be true even if the mode was processed, as
  // indicated by 'modesProcessed'.
  bool ContainsDetectionsForMode(const VisionMode mode,
