...
class VisionModeSet {
 public:
  ...
  // Insertion / Removal
  // TODO: switch to const& or && if/when VisionMode becomes a full-blown class
  // (VIC-9525)
  ...
  template <class Container>  // Statically asserts this is
                              // Container<VisionMode>
  ...
  // Returns string in the form "mode1+mode2+...+modeN", for all N modes in the
  // set
...
void VisionModeSet::Insert(const Container& modes) {
  ...
  std::for_each(modes.begin(), modes.end(),
                [this](const VisionMode mode) { Insert(mode); });
}
...
void VisionModeSet::Remove(const Container& modes) {
  ...
  std::for_each(modes.begin(), modes.end(),
                [this](const VisionMode mode) { Remove(mode); });
}
...
void VisionModeSet::Enable(Container modes, bool enable) {
  ...
  std::for_each(
      ...
      [this, enable](const VisionMode mode) { Enable(mode, enable); });
}
