 ...
 * Date:   2017-04-25
 *
 * Description: Vision system component for detecting laser points on the
 *ground.
 *
 *
...
CONSOLE_VAR_RANGED(f32, kLaser_darkThresholdFraction_darkExposure,
                   CONSOLE_GROUP_NAME, 0.7f, 0.f, 1.f);
CONSOLE_VAR_RANGED(f32, kLaser_darkThresholdFraction_normalExposure,
                   CONSOLE_GROUP_NAME, 0.9f, 0.f, 1.f);
...
// For determining when a laser point is saturated enough in either red or
// green, when color data is available. Bounding box fraction should be >= 1.0
...
CONSOLE_VAR(f32, kLaser_saturationBoundingBoxFraction, CONSOLE_GROUP_NAME,
            1.25f);
...
// Set to 2 to also draw separate debug images showing laser saliency (in image
// and on ground)
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
inline static u8 GetValue(const u8 p) { return p; }

inline static u8 GetValue(const Vision::PixelRGB& p) { return p.max(); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
template <class ImageType>
static void ConnCompValidityHelper(
    const Array2d<s32>& labelImage,
    const std::vector<Vision::Image::ConnectedComponentStats>& ccStats,
    const ImageType& img, const u8 highThreshold,
    std::vector<bool>& isConnCompValid) {
  DEV_ASSERT(!img.IsEmpty(),
             "LaserPointDetector.ConnCompValidityHelper.EmptyImage");

  DEV_ASSERT(
      labelImage.GetNumRows() == img.GetNumRows() &&
          labelImage.GetNumCols() == img.GetNumCols(),
      "LaserPointDetector.ConnCompValidityHelper.LabelImageSizeMismatch");

  DEV_ASSERT(!isConnCompValid.empty(),
             "LaserPointDetector.ConnCompValidityHelper.EmptyValidityVector");
  ...
  for (s32 iStat = 1; iStat < ccStats.size(); ++iStat) {
    ...
    // Get ROI around this connected component in the label image and the
    // color/gray image
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result LaserPointDetector::FindConnectedComponents(
    const Vision::ImageRGB& imgColor, const Vision::Image& imgGray,
    const u8 lowThreshold, const u8 highThreshold) {
  DEV_ASSERT(!imgGray.IsEmpty(),
             "LaserPointDetector.FindConnectedComponents.EmptyGrayImage");
  ...
  DEV_ASSERT(
      aboveLowThreshImg.GetNumRows() == imgGray.GetNumRows() &&
  ...
  size_t numRegions =
      aboveLowThreshImg.GetConnectedComponents(labelImage, allConnCompStats);

  // Make the min/max area threshold resolution-independent
  const f32 tuningArea =
      Params::kRadiusAtResolution.x() * Params::kRadiusAtResolution.y();
  const f32 minAreaFraction = (f32)(Params::kLaser_minRadius_pix *
                                    Params::kLaser_minRadius_pix * M_PI_F) /
                              tuningArea;
  const f32 maxAreaFraction = (f32)(Params::kLaser_maxRadius_pix *
                                    Params::kLaser_maxRadius_pix * M_PI_F) /
                              tuningArea;
  ...
  if (isColorAvailable) {
    ConnCompValidityHelper(labelImage, allConnCompStats, imgColor,
                           highThreshold, isConnCompValid);
  } else {
    ConnCompValidityHelper(labelImage, allConnCompStats, imgGray, highThreshold,
                           isConnCompValid);
  }

  // Keep only connected components we selected above that are also within area
  // limits Note: start at iStat=1 because we don't care about the 0th connected
  // component, which is "background"
  ...
  if (Params::kLaserDetectionDebug > 1) {
    _debugImage.Allocate(aboveLowThreshImg.GetNumRows(),
                         aboveLowThreshImg.GetNumCols());

    // Record only those connected components that we're keeping in the debug
    // iamge
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result LaserPointDetector::Detect(
    Vision::ImageCache& imageCache, const VisionPoseData& poseData,
  ...
  const Vision::ImageCacheSize scaleSize =
      Vision::ImageCache::GetSize(Params::kLaser_scaleMultiplier);
  ...
  const u8 lowThreshold =
      (isDarkExposure ? Params::kLaser_lowThreshold_darkExposure
                      : Params::kLaser_lowThreshold_normalExposure);
  const u8 highThreshold =
      (isDarkExposure ? Params::kLaser_highThreshold_darkExposure
                      : Params::kLaser_highThreshold_normalExposure);

  Result result = FindConnectedComponents(imageColor, imageGray, lowThreshold,
                                          highThreshold);

  if (RESULT_OK != result) {
    PRINT_NAMED_WARNING(
        "LaserPointDetector.Detect.FindConnectedComponentsFailed", "");
    return result;
  }

  // Get centroid of all the motion within the ground plane, if we have one to
  // reason about
  Quad2f imgQuad;
  poseData.groundPlaneROI.GetImageQuad(
      poseData.groundPlaneHomography,
      imageCache.GetNumCols(Vision::ImageCacheSize::Half),
      imageCache.GetNumRows(Vision::ImageCacheSize::Half), imgQuad);
  ...
  f32 groundRegionArea = FindLargestRegionCentroid(
      imageColor, imageGray, imgQuad, isDarkExposure, groundCentroidInImage);
  ...
  Point3f temp;
  Result solveResult = LeastSquares(
      poseData.groundPlaneHomography,
      Point3f{groundCentroidInImage.x(), groundCentroidInImage.y(), 1.f}, temp);
  if (RESULT_OK != solveResult) {
    PRINT_NAMED_WARNING(
        "LaserPointDetector.Detect.LeastSquaresFailed",
    ...
    PRINT_NAMED_WARNING("LaserPointDetector.Detect.BadProjectedZ",
                        "z<=0 (%f) when projecting laser centroid to ground. "
                        "Bad homography at head angle %.3f deg?",
                        temp.z(),
                        RAD_TO_DEG(poseData.histState.GetHeadAngle_rad()));
  ...
  if (Params::kLaserDetectionDebug) {
    PRINT_CH_INFO(kLogChannelName, "LaserPointDetector.Detect.FoundCentroid",
                  ...
                  groundRegionArea, groundPlaneCentroid.x(),
                  groundPlaneCentroid.y());
  }

  {
    // Note that we convert area to fraction of image area (to be
    // resolution-independent)
    ExternalInterface::RobotObservedLaserPoint laserPoint(
        imageGray.GetTimestamp(), groundRegionArea / imgQuadArea,
  ...
  if (Params::kLaser_DrawDetectionsInCameraView && (nullptr != _vizManager)) {
    const f32 groundOvalSize =
        std::max(0.5f, Params::kLaser_scaleMultiplier *
                           std::sqrtf(groundRegionArea / M_PI_F));
    _vizManager->DrawCameraOval(groundCentroidInImage, groundOvalSize,
                                groundOvalSize, NamedColors::GREEN);
  }

  if (Params::kLaserDetectionDebug > 1) {
    Vision::ImageRGB saliencyImageFullSize;
    if (Params::kLaser_scaleMultiplier > 1) {
      saliencyImageFullSize.Allocate(
          imageCache.GetNumRows(Vision::ImageCacheSize::Half),
          imageCache.GetNumCols(Vision::ImageCacheSize::Half));
      _debugImage.Resize(saliencyImageFullSize,
                         Vision::ResizeMethod::NearestNeighbor);
    }

    _debugImage.DrawCircle(
        groundCentroidInImage * (1.f / (f32)Params::kLaser_scaleMultiplier),
        NamedColors::RED, 4);
    char tempText[128];
    // snprintf(tempText, 127, "Area:%.2f X:%d Y:%d", imgRegionArea,
    // salientPoint.img_x, salientPoint.img_y);
    // cv::putText(saliencyImgDisp.get_CvMat_(), std::string(tempText),
    //            cv::Point(0,saliencyImgDisp.GetNumRows()), CV_FONT_NORMAL,
    //            .4f, CV_RGB(0,255,0));
    debugImages.emplace_back("LaserSaliencyImage", _debugImage);

    Vision::ImageRGB saliencyImageDispGround(
        poseData.groundPlaneROI.GetOverheadImage(
            saliencyImageFullSize, poseData.groundPlaneHomography));
    if (groundRegionArea > 0.f) {
      Point2f dispCentroid(groundPlaneCentroid.x(),
                           -groundPlaneCentroid.y());  // Negate Y for display
      saliencyImageDispGround.DrawCircle(
          dispCentroid - poseData.groundPlaneROI.GetOverheadImageOrigin(),
          NamedColors::RED, 3);
      ...
      snprintf(tempText, 127, "Area:%.2f%% X:%d Y:%d", groundRegionArea * 100.f,
               (s32)std::round(groundPlaneCentroid.x()),
               (s32)std::round(groundPlaneCentroid.y()));
      saliencyImageDispGround.DrawText(
          {0.f, poseData.groundPlaneROI.GetWidthFar()}, tempText,
          NamedColors::GREEN, .4f);
    }
    debugImages.emplace_back("LaserSaliencyImageGround",
                             saliencyImageDispGround);
  }
...
Result LaserPointDetector::Detect(
    Vision::ImageCache& imageCache, const bool isDarkExposure,
    std::list<ExternalInterface::RobotObservedLaserPoint>& points,
    Vision::DebugImageList<Vision::CompressedImage>& debugImages) {
  ...
  const Quad2f wholeImageQuad(
      Point2f(0, 0), Point2f(0, imageGray.GetNumRows()),
      Point2f(imageGray.GetNumCols(), 0),
      Point2f(imageGray.GetNumCols(), imageGray.GetNumRows()));

  f32 regionArea = FindLargestRegionCentroid(
      imageColor, imageGray, wholeImageQuad, isDarkExposure, centroidInImage);
...

  {
    ...
    // resolution-independent)
    ExternalInterface::RobotObservedLaserPoint laserPoint(
        imageGray.GetTimestamp(), regionArea / imageGray.GetNumElements(),
        std::round(centroidInImage.x()), std::round(centroidInImage.y()));

    points.emplace_back(std::move(laserPoint));
  }

  if (Params::kLaser_DrawDetectionsInCameraView && (nullptr != _vizManager)) {
    const f32 groundOvalSize = std::max(
        0.5f, Params::kLaser_scaleMultiplier * std::sqrtf(regionArea / M_PI_F));
    _vizManager->DrawCameraOval(centroidInImage, groundOvalSize, groundOvalSize,
                                NamedColors::GREEN);
  }
  ...
  if (Params::kLaserDetectionDebug > 1) {
    ...
    _debugImage.DrawCircle(
        centroidInImage * (1.f / (f32)Params::kLaser_scaleMultiplier),
        NamedColors::RED, 4);
    debugImages.emplace_back("LaserSaliencyImage", _debugImage);
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
size_t LaserPointDetector::FindLargestRegionCentroid(
    const Vision::ImageRGB& imgColor, const Vision::Image& imgGray,
    const Quad2f& groundQuadInImage, const bool isDarkExposure,
    Point2f& centroid) {
  const bool isColorAvailable = !imgColor.IsEmpty();

  const f32 darkThresholdFraction =
      (isDarkExposure ? Params::kLaser_darkThresholdFraction_darkExposure
  ...
  for (auto const& stat : _connCompStats) {
    if (stat.area > largestArea && IsOnGroundPlane(groundQuadInImage, stat) &&
        IsSurroundedByDark(imgGray, stat, darkThresholdFraction) &&
        (!isColorAvailable ||
         IsSaturated(imgColor, stat, Params::kLaser_saturationThreshold_red,
                     Params::kLaser_saturationThreshold_green))) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
inline bool LaserPointDetector::IsOnGroundPlane(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool LaserPointDetector::IsSurroundedByDark(
    const Vision::Image& image,
    const Vision::Image::ConnectedComponentStats& stat,
    const f32 darkThresholdFraction) {
  const u8 centerPixel = std::round(
      darkThresholdFraction *
      (f32)image(std::round(stat.centroid.y()), std::round(stat.centroid.x())));

  const f32 radius = Params::kLaser_darkSurroundRadiusFraction *
                     std::sqrtf((f32)stat.area / M_PI_F);

  // sin/cos of [0 45 90 135 180 225 270 315] degrees. (cos is first, sin is
  // second)
  ...
  for (auto& sinCosPair : sinCosPairs) {
    ...
    if (x >= 0 && x < image.GetNumCols() && y >= 0 && y < image.GetNumRows()) {
      if (Params::kLaser_DrawDetectionsInCameraView &&
          (nullptr != _vizManager)) {
        _vizManager->DrawCameraOval(
            {(f32)x * (f32)Params::kLaser_scaleMultiplier,
             (f32)y * Params::kLaser_scaleMultiplier},
            0.5f, 0.5f, NamedColors::RED);
      }

      // If any surrounding point in the saliency image is _on_ ignore this
      // region (not dot shaped!)
      const u8 pixVal = image(y, x);
      if (pixVal > centerPixel) {
        if (Params::kLaserDetectionDebug > 1) {
          PRINT_NAMED_WARNING("LaserPointDetector.IsSurroundedByDark",
                              "Not surrounded by dark ring: %d > %d", pixVal,
                              centerPixel);
        }
  ...
  const s32 surroundVar =
      (surroundSumSq / kNumSurroundPoints) - (surroundMean * surroundMean);
  if (surroundVar >
      (Params::kLaser_MaxSurroundStdDev * Params::kLaser_MaxSurroundStdDev)) {
    if (Params::kLaserDetectionDebug > 1) {
      PRINT_NAMED_WARNING(
          "LaserPointDetector.IsSurroundedByDark.VarianceTooHigh",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool LaserPointDetector::IsSaturated(
    const Vision::ImageRGB& image,
    const Vision::Image::ConnectedComponentStats& stat, const f32 redThreshold,
    const f32 greenThreshold) {
  DEV_ASSERT(!image.IsEmpty(),
             "LaserPointDetector.IsSaturated.EmptyColorImage");

  // check if the region is somewhat saturated (i.e. mostly red, green, or
  // blue), to help reduce false positives for bright spots which are uncolored
  ...
  for (s32 i = 0; i < roi.GetNumRows(); ++i) {
    ...
    for (s32 j = 0; j < roi.GetNumCols(); ++j) {
      ...
      sumSaturation_red += std::max(
          0, (s32)pixel.r() - std::max((s32)pixel.g(), (s32)pixel.b()));
      sumSaturation_green += std::max(
          0, (s32)pixel.g() - std::max((s32)pixel.r(), (s32)pixel.b()));
    }
  }

  const f32 avgSaturation_red =
      (f32)sumSaturation_red / (f32)roi.GetNumElements();
  const f32 avgSaturation_green =
      (f32)sumSaturation_green / (f32)roi.GetNumElements();

  // Debug display
  if (Params::kLaserDetectionDebug && _vizManager) {
    _vizManager->DrawCameraText(
        stat.centroid * Params::kLaser_scaleMultiplier,
        std::to_string((s32)std::round(avgSaturation_red)) + ":" +
            std::to_string((s32)std::round(avgSaturation_green)),
        NamedColors::RED);
  }

  if (avgSaturation_red > redThreshold ||
      avgSaturation_green > greenThreshold) {
    return true;
  } else {
    // Not saturated enough
    if (Params::kLaserDetectionDebug > 1) {
      PRINT_NAMED_WARNING("LaserPointDetector.IsSaturated",
                          "Not saturated: R=%1.f G=%.1f", avgSaturation_red,
                          avgSaturation_green);
    }
    return false;
  }
