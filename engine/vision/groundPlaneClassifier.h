...
/**
 * Generic interface class to extract features from an image. It uses the type
 * FeatureType defined in RawPixelsClassifier
 */
class IFeaturesExtractor {
 public:
   ...
   * @return the features
   */
  virtual std::vector<RawPixelsClassifier::FeatureType> Extract(
      const Vision::ImageRGB& image, int row, int col) const = 0;

  /**
   * Calculates the features over the whole image and returns them in an array.
   */
  virtual Array2d<RawPixelsClassifier::FeatureType> Extract(
      const Vision::ImageRGB& image) const = 0;
};

/**
 * For each pixel, the features are the mean (per channel) of the neighbour
 * pixels.
 */
class MeanFeaturesExtractor : public IFeaturesExtractor {
 public:
  /**
   * @param padding specifies half the size of the square around the pixel to
   * use to compute the mean. The square will have an edge of 2*padding+1
   * length. For example with padding=1 the square will be 3x3.
   */
  explicit MeanFeaturesExtractor(int padding)
      : _padding(padding), _profiler("MeanFeaturesExtractor") {}

  Array2d<RawPixelsClassifier::FeatureType> Extract(
      const Vision::ImageRGB& image) const override;

  std::vector<RawPixelsClassifier::FeatureType> Extract(
      const Vision::ImageRGB& image, int row, int col) const override;

 private:
  int _padding;
  // These parameters are mutable since MeanFeaturesExtractor calculates the
  // mean image once at the beginning, caches it and accesses it for subsequent
  // calls. Better to use Extract that takes the whole image.
  mutable uchar* _prevImageData =
      nullptr;  // used to check if a new image is being used
...
void CVMatToVector(const cv::Mat& mat, std::vector<std::vector<T2>>& vec) {
  DEV_ASSERT(mat.type() == cv::DataType<T1>::type,
             "CVMatToVector.WrongMatrixType");
...
/**
 * Class to interface the VisionSystem with the RawPixelsClassifier. At the
 * moment it uses a DTRawPixelsClassifier but it can accept any
 * RawPixelsClassifier. The classifier can either be serialized and later loaded
 * or trained on the fly.
 */
class GroundPlaneClassifier {
 public:
  ...
  const RawPixelsClassifier& GetClassifier() const {
    DEV_ASSERT(_classifier.get() != nullptr,
               "GroundPlaneClassifier.GetClassifier.NULLClassifier");
