...
// Find the neural network names registered to a given vision mode (or vice
// versa). Returns true if there are any and populates networkNames. Returns
// false (and does not modify networkNames) otherwise.
bool GetNeuralNetsForVisionMode(const VisionMode mode,
                                std::set<std::string>& networkNames);
bool GetVisionModesForNeuralNet(const std::string& networkName,
