...
class CameraCalibrator {
 public:
  ...
  // Outputs calibrations and debugImages via reference and returns whether or
  // not calibration succeeded
  Result ComputeCalibrationFromCheckerboard(
      std::list<Vision::CameraCalibration>& calibration_out,
      Vision::DebugImageList<Vision::CompressedImage>& debugImages_out);

  // Computes camera calibration using observed markers on either the
  // INVERTED_BOX or QBERT target Outputs calibrations and debugImages via
  // reference and returns whether or not calibration succeeded
  Result ComputeCalibrationFromSingleTarget(
  ...
  struct CalibImage {
    ...
    // Output
    // Whether of not dots were found in the image (dot checkerboard
    // calibration)
    bool dotsFound;
  };

  // Returns a vector of all stored calibration images (may or may not have
  // already been used for calibration)
  const std::vector<CalibImage>& GetCalibrationImages() const {
    return _calibImages;
  }

  // Returns a vector of Camera poses based on where the camera was when taking
  // each CalibImage Each index matches the corresponding images in _calibImages
  const std::vector<Pose3d>& GetCalibrationPoses() const { return _calibPoses; }

 private:
  // Calculates expected corner positions of the CHECKERBOARD target with the
  // given board and square sizes
  void CalcBoardCornerPositions(cv::Size boardSize, float squareSize,
                                std::vector<cv::Point3f>& corners);

  // Populates markersTo3dCoords with the 3d world coordinates of each corner of
  // each marker on the respective target
  void GetCalibTargetMarkersTo3dCoords_Qbert(
      std::map<Vision::MarkerType, Quad3f>& markersTo3dCoords,
      std::set<Vision::MarkerType>& markersNeededToBeSeen);
  void GetCalibTargetMarkersTo3dCoords_InvertedBox(
      std::map<Vision::MarkerType, Quad3f>& markersTo3dCoords,
      std::set<Vision::MarkerType>& markersNeededToBeSeen);
