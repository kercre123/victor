 ...
 * Date:   (various)
 *
 * Description: Container for passing around pose/state information from a
 *certain timestamp, useful for vision processing.
 *
...
void VisionPoseData::Set(const RobotTimeStamp_t histTimeStamp_in,
  ...
  DEV_ASSERT(
      histState_in.GetPose().GetRootID() != PoseOriginList::UnknownOriginID,
      "VisionPoseData.Set.UnknownOriginID");

  // The camera pose's parent is expected to be the histState's pose
  DEV_ASSERT(cameraPose_in.IsChildOf(histState_in.GetPose()),
             "VisionPoseData.Set.BadCameraPoseParent");
  ...
  DEV_ASSERT(histState.GetPose().GetParent().IsRoot(),
             "VisionPoseData.Set.HistoricalPoseParentNotRoot");
  histState.ClearPoseParent();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool VisionPoseData::IsBodyPoseSame(const VisionPoseData& other,
                                    const Radians& bodyAngleThresh,
                                    const f32 bodyPoseThresh_mm) const {
  const bool isXPositionSame =
      NEAR(histState.GetPose().GetTranslation().x(),
           other.histState.GetPose().GetTranslation().x(), bodyPoseThresh_mm);

  const bool isYPositionSame =
      NEAR(histState.GetPose().GetTranslation().y(),
           other.histState.GetPose().GetTranslation().y(), bodyPoseThresh_mm);

  const bool isAngleSame = NEAR(
      histState.GetPose().GetRotation().GetAngleAroundZaxis().ToFloat(),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool VisionPoseData::IsHeadAngleSame(const VisionPoseData& other,
                                     const Radians& headAngleThresh) const {
  const bool headSame =
      NEAR(histState.GetHeadAngle_rad(), other.histState.GetHeadAngle_rad(),
