...
struct VisionPoseData {
  ...
  // TODO: Add getters for these and make them private, prefixed with underscore
  // (COZMO-14998)
  ...
  void Set(const RobotTimeStamp_t histTimeStamp_in,
           const HistRobotState& histState_in, const Pose3d& cameraPose_in,
           const bool groundPlaneVisible_in,
           const Matrix_3x3f& groundPlaneHomography_in,
           const ImuDataHistory& imuHistory_in);

  // Helpers to check whether the body/head have changed more than a given
  // amount as compared to another VisionPoseData
  bool IsBodyPoseSame(const VisionPoseData& other,
                      const Radians& bodyAngleThresh,
                      const f32 bodyPoseThresh_mm) const;
  bool IsHeadAngleSame(const VisionPoseData& other,
...
void swap(T1&& first, T2&& second) {
  ...
  // Because the cameraPose is wrt the pose contained in poseStamp, set it
  // explicitly
