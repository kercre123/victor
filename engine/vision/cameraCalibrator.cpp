...
// TODO Figure out min number of markers (what if top row is cut off thats like
// 12 markers)
...
Result CameraCalibrator::ComputeCalibrationFromCheckerboard(
  ...
  Util::CleanupHelper disableCalibration([this, &calibration_out,
                                          &calibration]() {
    if (calibration == nullptr) {
      PRINT_NAMED_WARNING(
          "CameraCalibrator.ComputeCalibrationFromCheckerboard.NullCalibration",
          "");
    } else {
  ...
  if (_calibImages.size() < kMinNumCalibImages) {
    PRINT_CH_INFO(
        kLogChannelName,
        "CameraCalibrator.ComputeCalibrationFromCheckerboard.NotEnoughImages",
  ...
  PRINT_CH_INFO(kLogChannelName,
                "CameraCalibrator.ComputeCalibrationFromCheckerboard.NumImages",
  ...
  for (auto& calibImage : _calibImages) {
    ...
    calibImage.dotsFound = cv::findCirclesGrid(
        img.get_CvMat_(), boardSize, pointBuf, findCirclesFlags, blobDetector);

    if (calibImage.dotsFound) {
      PRINT_CH_INFO(
          kLogChannelName,
          "CameraCalibrator.ComputeCalibrationFromCheckerboard.FoundPoints",
          "");
      imagePoints.push_back(pointBuf);
    } else {
      PRINT_CH_INFO(
          kLogChannelName,
          "CameraCalibrator.ComputeCalibrationFromCheckerboard.NoPointsFound",
          "");
    }
    ...
    if (kDrawCalibImages) {
      ...
      if (calibImage.dotsFound) {
        cv::drawChessboardCorners(dispImg.get_CvMat_(), boardSize,
                                  cv::Mat(pointBuf), calibImage.dotsFound);
      }

      debugImages_out.emplace_back(
          std::string("CalibImage") + std::to_string(imgCnt), dispImg);
    }
  ...
  if (imagePoints.size() < kMinNumCalibImages) {
    PRINT_CH_INFO(kLogChannelName,
                  "CameraCalibrator.ComputeCalibrationFromCheckerboard."
                  "InsufficientImagesWithPoints",
  ...
  // Get object points
  CalcBoardCornerPositions(boardSize, kCheckerboardSquareSize_mm,
                           objectPoints[0]);
  ...
  cv::Mat_<f64> distCoeffs =
      cv::Mat_<f64>::zeros(1, NUM_RADIAL_DISTORTION_COEFFS);

  const f64 rms = cv::calibrateCamera(objectPoints, imagePoints, imageSize,
  ...
  std::copy(distCoeffs_data, distCoeffs_data + NUM_RADIAL_DISTORTION_COEFFS,
            distCoeffsVec.begin());

  calibration.reset(new Vision::CameraCalibration(
      imageSize.height, imageSize.width, cameraMatrix(0, 0), cameraMatrix(1, 1),
  ...
  DEV_ASSERT_MSG(
      ...
      "Got %zu rotations and %zu translations", rvecs.size(), tvecs.size());
...
Result CameraCalibrator::ComputeCalibrationFromSingleTarget(
    CalibTargetType targetType,
  ...
  Util::CleanupHelper disableCalibration([this, &calibration_out,
    ...
    if (calibration == nullptr) {
      PRINT_NAMED_WARNING(
          "CameraCalibrator.ComputeCalibrationFromSingleTarget.NullCalibration",
          "");
    } else {
      calibration_out.push_back(*calibration);
    }
  ...
  if (observedMarkers.size() < kNumMarkersNeededForCalibration) {
    PRINT_NAMED_WARNING(
        "CameraCalibrator.ComputeCalibrationFromSingleTarget.NotEnoughMarkers",
        "Seeing only %zu markers, need to be seeing at least %d",
        observedMarkers.size(), kNumMarkersNeededForCalibration);
    return RESULT_FAIL;
  }
  ...
  switch (targetType) {
    ...
    case CHECKERBOARD: {
      PRINT_NAMED_WARNING(
          "CameraCalibrator.ComputeCalibrationFromSingleTarget.InvalidTarget",
          "");
      return RESULT_FAIL;
    }
  }
  ...
  for (const auto& marker : observedMarkers) {
    if (codes.count(marker.GetCode()) != 0) {
      PRINT_NAMED_WARNING(
          "CameraCalibrator.ComputeCalibrationFromSingleTarget."
          "MultipleMarkersWithSameCode",
          "Observed multiple markers with code %s", marker.GetCodeName());
      return RESULT_FAIL;
    }

    codes.insert(marker.GetCode());
    const auto& iter = markersTo3dCoords.find(
        static_cast<Vision::MarkerType>(marker.GetCode()));

    if (iter != markersTo3dCoords.end()) {
      const auto& corners = marker.GetImageCorners();

      imgPts.emplace_back(corners.GetTopLeft().x(), corners.GetTopLeft().y());
      worldPts.emplace_back(iter->second.GetTopLeft().x(),
                            iter->second.GetTopLeft().y(),
                            iter->second.GetTopLeft().z());

      imgPts.emplace_back(corners.GetTopRight().x(), corners.GetTopRight().y());
  ...
  if (!s.empty()) {
    PRINT_CH_INFO(
        kLogChannelName,
        "CameraCalibrator.ComputeCalibrationFromSingleTarget.MarkersNotSeen",
        "Expected to see the following markers but didnt %s", s.c_str());
  }
  ...
  if (!s.empty()) {
    PRINT_NAMED_ERROR(
        "CameraCalibrator.ComputeCalibrationFromSingleTarget.MissingMarkers",
        "Needed to see the following markers but didnt %s", s.c_str());
    return RESULT_FAIL;
  }
  ...
  // Depending on what type of robot we are running, provide a different initial
  // guess for calibration
#ifdef SIMULATOR
  cv::Mat_<f64> cameraMatrix =
      (cv::Mat_<f64>(3, 3) << 507, 0, 639, 0, 507, 359, 0, 0, 1);

  cv::Mat_<f64> distCoeffs =
      (cv::Mat_<double>(1, NUM_RADIAL_DISTORTION_COEFFS) << -0.07f, -0.2f,
       0.001f, 0.001f, 0.1f, 0.f, 0.f, 0.f);
#else
  cv::Mat_<f64> cameraMatrix =
      (cv::Mat_<double>(3, 3) << 362, 0, 303, 0, 364, 196, 0, 0, 1);

  cv::Mat_<f64> distCoeffs =
      (cv::Mat_<double>(1, NUM_RADIAL_DISTORTION_COEFFS) << -0.1, -0.1, 0.00005,
       -0.0001, 0.05, 0, 0, 0);
#endif
  ...
  try {
    rms = cv::calibrateCamera(
        vecOfWorldPts, vecOfImgPts, cv::Size(numCols, numRows), cameraMatrix,
        distCoeffs, rvecs, tvecs, cv::CALIB_USE_INTRINSIC_GUESS);
  } catch (cv::Exception& e) {
    PRINT_NAMED_ERROR(
        "CameraCalibrator.ComputeCalibrationFromSingleTarget.OpenCVError", "%s",
        e.what());
    return RESULT_FAIL;
  }
  ...
  PRINT_CH_INFO(kLogChannelName,
                "CameraCalibrator.ComputeCalibrationFromSingleImage.K", "%s",
                ss.str().c_str());

  ss.str(std::string());
  ss << distCoeffs << std::endl;
  PRINT_CH_INFO(kLogChannelName,
                "CameraCalibrator.ComputeCalibrationFromSingleImage.D", "%s",
                ss.str().c_str());

  const f64* distCoeffs_data = distCoeffs[0];
  std::array<f32, NUM_RADIAL_DISTORTION_COEFFS> distCoeffsVec;
  distCoeffsVec.fill(0.f);
  std::copy(distCoeffs_data, distCoeffs_data + distCoeffs.cols,
            distCoeffsVec.begin());

  calibration.reset(new Vision::CameraCalibration(
      numRows, numCols, cameraMatrix(0, 0), cameraMatrix(1, 1),
      cameraMatrix(0, 2), cameraMatrix(1, 2),
      0.f,  // skew
      distCoeffsVec));
  ...
  PRINT_CH_INFO(
      kLogChannelName,
      "CameraCalibrator.ComputeCalibrationFromSingleTarget.CalibValues",
      "fx: %f, fy: %f, cx: %f, cy: %f (rms %f)",
      calibration->GetFocalLength_x(), calibration->GetFocalLength_y(),
      calibration->GetCenter_x(), calibration->GetCenter_y(), rms);
...
Result CameraCalibrator::AddCalibrationImage(const Vision::Image& calibImg,
  ...
  if (_isCalibrating) {
    PRINT_CH_INFO(kLogChannelName,
                  "CameraCalibrator.AddCalibrationImage.AlreadyCalibrating",
                  "Cannot add calibration image while already in the middle of "
                  "doing calibration.");

    return RESULT_FAIL;
  }

  if (targetROI.GetX() < 0 && targetROI.GetY() < 0 &&
      targetROI.GetWidth() == 0 && targetROI.GetHeight() == 0) {
    // Use entire image if negative ROI specified
    const Anki::Rectangle<s32> entireImgROI(0, 0, calibImg.GetNumCols(),
                                            calibImg.GetNumRows());
    _calibImages.push_back(
        {.img = calibImg, .roiRect = entireImgROI, .dotsFound = false});
  } else {
    _calibImages.push_back(
        {.img = calibImg, .roiRect = targetROI, .dotsFound = false});
  }
...
Result CameraCalibrator::ClearCalibrationImages() {
  if (_isCalibrating) {
    PRINT_CH_INFO(kLogChannelName,
                  "CameraCalibrator.ClearCalibrationImages.AlreadyCalibrating",
                  "Cannot clear calibration images while already in the middle "
                  "of doing calibration.");
...
void CameraCalibrator::GetCalibTargetMarkersTo3dCoords_Qbert(
    std::map<Vision::MarkerType, Quad3f>& markersTo3dCoords,
    std::set<Vision::MarkerType>& markersNeededToBeSeen) {
  ...
  | |
   ...
   Marker corners are defined relative to center of bottom left cube of the
  target in this orientation (before rotations are applied to get cubes to their
  actual positions) FrontFace is the marker that is facing the robot in this
  orientation. !FrontMarker is the left marker, that will be visible when
  rotation are applied (rotate 45 degree on Z and then -30 degree in Y in this
  origin)
   */
  ...
  auto GetCoordsForFace = [&originsLeftFace, &originsFrontFace](
                              bool isFrontFace, int numCubesRightOfOrigin,
    ...
    Pose3d p;
    p.SetTranslation(
        {BLEACHER_CALIB_TARGET_FACE_SIZE_MM * numCubesRightOfOrigin,
  ...
  // Bottom row of cubes
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEK_RIGHT] =
      GetCoordsForFace(true, 0, 0, 0);

  markersTo3dCoords[Vision::MARKER_LIGHTCUBEK_LEFT] =
      GetCoordsForFace(false, 1, -1, 0);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEK_FRONT] =
      GetCoordsForFace(true, 1, -1, 0);

  markersTo3dCoords[Vision::MARKER_LIGHTCUBEK_TOP] =
      GetCoordsForFace(false, 2, -2, 0);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEK_BACK] =
      GetCoordsForFace(true, 2, -2, 0);

  markersTo3dCoords[Vision::MARKER_LIGHTCUBEJ_TOP] =
      GetCoordsForFace(false, 3, -3, 0);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEJ_RIGHT] =
      GetCoordsForFace(true, 3, -3, 0);

  markersTo3dCoords[Vision::MARKER_LIGHTCUBEJ_LEFT] =
      GetCoordsForFace(false, 4, -4, 0);

  // Second row of cubes
  markersTo3dCoords[Vision::MARKER_ARROW] = GetCoordsForFace(true, 0, 1, 1);
  markersNeededToBeSeen.insert(Vision::MARKER_ARROW);

  markersTo3dCoords[Vision::MARKER_SDK_2HEXAGONS] =
      GetCoordsForFace(true, 1, 0, 1);
  markersNeededToBeSeen.insert(Vision::MARKER_SDK_2HEXAGONS);

  markersTo3dCoords[Vision::MARKER_SDK_5DIAMONDS] =
      GetCoordsForFace(false, 2, -1, 1);
  markersTo3dCoords[Vision::MARKER_SDK_4DIAMONDS] =
      GetCoordsForFace(true, 2, -1, 1);
  markersNeededToBeSeen.insert(Vision::MARKER_SDK_4DIAMONDS);

  markersTo3dCoords[Vision::MARKER_SDK_3DIAMONDS] =
      GetCoordsForFace(false, 3, -2, 1);
  markersNeededToBeSeen.insert(Vision::MARKER_SDK_3DIAMONDS);
  markersTo3dCoords[Vision::MARKER_SDK_2DIAMONDS] =
      GetCoordsForFace(true, 3, -2, 1);

  markersTo3dCoords[Vision::MARKER_SDK_5CIRCLES] =
      GetCoordsForFace(false, 4, -3, 1);
  markersNeededToBeSeen.insert(Vision::MARKER_SDK_5CIRCLES);

  markersTo3dCoords[Vision::MARKER_SDK_3CIRCLES] =
      GetCoordsForFace(false, 5, -4, 1);
  markersNeededToBeSeen.insert(Vision::MARKER_SDK_3CIRCLES);

  // Third row of cubes
  markersTo3dCoords[Vision::MARKER_SDK_4HEXAGONS] =
      GetCoordsForFace(true, 0, 2, 2);
  markersNeededToBeSeen.insert(Vision::MARKER_SDK_4HEXAGONS);

  markersTo3dCoords[Vision::MARKER_SDK_2CIRCLES] =
      GetCoordsForFace(true, 1, 1, 2);
  markersNeededToBeSeen.insert(Vision::MARKER_SDK_2CIRCLES);

  markersTo3dCoords[Vision::MARKER_LIGHTCUBEJ_FRONT] =
      GetCoordsForFace(false, 2, 0, 2);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEK_TOP] =
      GetCoordsForFace(true, 2, 0, 2);
  ...
  markersTo3dCoords[Vision::MARKER_BULLSEYE2] =
      GetCoordsForFace(true, 3, -1, 2);

  markersTo3dCoords[Vision::MARKER_SDK_5TRIANGLES] =
      GetCoordsForFace(false, 4, -2, 2);
  markersNeededToBeSeen.insert(Vision::MARKER_SDK_5TRIANGLES);
  markersTo3dCoords[Vision::MARKER_SDK_4TRIANGLES] =
      GetCoordsForFace(true, 4, -2, 2);

  markersTo3dCoords[Vision::MARKER_SDK_3TRIANGLES] =
      GetCoordsForFace(false, 5, -3, 2);
  markersNeededToBeSeen.insert(Vision::MARKER_SDK_3TRIANGLES);

  markersTo3dCoords[Vision::MARKER_SDK_5HEXAGONS] =
      GetCoordsForFace(false, 6, -4, 2);
  markersNeededToBeSeen.insert(Vision::MARKER_SDK_5HEXAGONS);

  // Fourth row of cubes (top row)
  markersTo3dCoords[Vision::MARKER_SDK_4CIRCLES] =
      GetCoordsForFace(true, 0, 3, 3);

  markersTo3dCoords[Vision::MARKER_LIGHTCUBEJ_BACK] =
      GetCoordsForFace(true, 1, 2, 3);

  markersTo3dCoords[Vision::MARKER_LIGHTCUBEI_RIGHT] =
      GetCoordsForFace(true, 2, 1, 3);

  markersTo3dCoords[Vision::MARKER_LIGHTCUBEI_LEFT] =
      GetCoordsForFace(false, 3, 0, 3);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEI_FRONT] =
      GetCoordsForFace(true, 3, 0, 3);

  markersTo3dCoords[Vision::MARKER_LIGHTCUBEI_BOTTOM] =
      GetCoordsForFace(false, 4, -1, 3);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEI_BACK] =
      GetCoordsForFace(true, 4, -1, 3);

  markersTo3dCoords[Vision::MARKER_LIGHTCUBEI_TOP] =
      GetCoordsForFace(false, 5, -2, 3);

  markersTo3dCoords[Vision::MARKER_LIGHTCUBEJ_BOTTOM] =
      GetCoordsForFace(false, 6, -3, 3);

  markersTo3dCoords[Vision::MARKER_SDK_2TRIANGLES] =
      GetCoordsForFace(false, 7, -4, 3);
#else
  PRINT_NAMED_ERROR(
      "CameraCalibrator.GetCalibTargetMarkersTo3dCoords_Qbert.NotInFactoryTest",
...
void CameraCalibrator::GetCalibTargetMarkersTo3dCoords_InvertedBox(
    std::map<Vision::MarkerType, Quad3f>& markersTo3dCoords,
    std::set<Vision::MarkerType>& markersNeededToBeSeen) {
  markersTo3dCoords.clear();
  ...
  |  |* |  |  |
   ...
   Marker corners are defined relative to center of the * cube of the target in
  this orientation (before rotations are applied to get cubes to their actual
  positions) isFrontFace are the markers that is facing the robot in this
  orientation. !isFrontFace are the left markers, that will be visible when
  rotation are applied isBottomFace are the markers on the top face
   */
#if FACTORY_TEST
  ...
  auto GetCoordsForFace = [&originsLeftFace, &originsFrontFace,
                           &originsBottomFace](bool isFrontFace,
    ...
    Pose3d p;
    p.SetTranslation(
        {INVERTEDBOX_CALIB_TARGET_FACE_SIZE_MM * numCubesRightOfOrigin,
  ...
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEK_LEFT] =
      GetCoordsForFace(true, 0, 0, 0);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEK_RIGHT] =
      GetCoordsForFace(true, 1, 0, 0);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEK_TOP] =
      GetCoordsForFace(true, 2, 0, 0);

  // Middle row
  markersTo3dCoords[Vision::MARKER_SDK_3CIRCLES] =
      GetCoordsForFace(true, -1, 0, 1);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEJ_TOP] =
      GetCoordsForFace(true, 0, 0, 1);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEK_BACK] =
      GetCoordsForFace(true, 1, 0, 1);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEK_BOTTOM] =
      GetCoordsForFace(true, 2, 0, 1);

  // Top row
  markersTo3dCoords[Vision::MARKER_SDK_2CIRCLES] =
      GetCoordsForFace(true, -1, 0, 2);
  markersTo3dCoords[Vision::MARKER_SDK_2DIAMONDS] =
      GetCoordsForFace(true, 0, 0, 2);
  markersTo3dCoords[Vision::MARKER_SDK_2HEXAGONS] =
      GetCoordsForFace(true, 1, 0, 2);
  markersTo3dCoords[Vision::MARKER_SDK_2TRIANGLES] =
      GetCoordsForFace(true, 2, 0, 2);
  ...
  // Bottom row
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEJ_BOTTOM] =
      GetCoordsForFace(false, 3, -1, 0);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEJ_FRONT] =
      GetCoordsForFace(false, 3, -2, 0);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEJ_LEFT] =
      GetCoordsForFace(false, 3, -3, 0);

  // Middle row
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEI_LEFT] =
      GetCoordsForFace(false, 3, -1, 1);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEI_RIGHT] =
      GetCoordsForFace(false, 3, -2, 1);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEI_TOP] =
      GetCoordsForFace(false, 3, -3, 1);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEJ_BACK] =
      GetCoordsForFace(false, 3, -4, 1);

  // Top row
  markersTo3dCoords[Vision::MARKER_ARROW] = GetCoordsForFace(false, 3, -1, 2);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEI_BACK] =
      GetCoordsForFace(false, 3, -2, 2);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEI_BOTTOM] =
      GetCoordsForFace(false, 3, -3, 2);
  markersTo3dCoords[Vision::MARKER_LIGHTCUBEI_FRONT] =
      GetCoordsForFace(false, 3, -4, 2);

  // Top face
  markersTo3dCoords[Vision::MARKER_BULLSEYE2] =
      GetCoordsForFace(false, -1, -1, 3, true);
  markersTo3dCoords[Vision::MARKER_SDK_5TRIANGLES] =
      GetCoordsForFace(false, 0, -1, 3, true);
  markersTo3dCoords[Vision::MARKER_SDK_4TRIANGLES] =
      GetCoordsForFace(false, 1, -1, 3, true);
  markersTo3dCoords[Vision::MARKER_SDK_5HEXAGONS] =
      GetCoordsForFace(false, 2, -1, 3, true);

  markersTo3dCoords[Vision::MARKER_SDK_4DIAMONDS] =
      GetCoordsForFace(false, 0, -2, 3, true);
  markersTo3dCoords[Vision::MARKER_SDK_4CIRCLES] =
      GetCoordsForFace(false, 1, -2, 3, true);
  markersTo3dCoords[Vision::MARKER_SDK_4HEXAGONS] =
      GetCoordsForFace(false, 2, -2, 3, true);

  markersTo3dCoords[Vision::MARKER_SDK_3HEXAGONS] =
      GetCoordsForFace(false, 1, -3, 3, true);
  markersTo3dCoords[Vision::MARKER_SDK_3TRIANGLES] =
      GetCoordsForFace(false, 2, -3, 3, true);

  markersTo3dCoords[Vision::MARKER_SDK_3DIAMONDS] =
      GetCoordsForFace(false, 2, -4, 3, true);
#else
  PRINT_NAMED_ERROR(
      "CameraCalibrator.GetCalibTargetMarkersTo3dCoords_InvertedBox."
      "NotInFactoryTest",
      "Markers have diverged from factory test build");
#endif
}
