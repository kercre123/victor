...
// There's two main components: one that detects motion on the ground plane, and
// one that detects motion in the peripheral areas (top, left and right).
class MotionDetector {
 ...
 private:
  ...
  Result DetectHelper(
      const ImageType &resizedImage, s32 origNumRows, s32 origNumCols,
      f32 scaleMultiplier, const VisionPoseData &crntPoseData,
      const VisionPoseData &prevPoseData,
      std::list<ExternalInterface::RobotObservedMotion> &observedMotions,
      Vision::DebugImageList<Vision::CompressedImage> &debugImages);

  // To detect peripheral motion, a simple impulse-decay model is used. The
  // longer motion is detected in a specific area, the higher its activation
  // will be. When it reaches a max value motion is activated in that specific
  // area.
  bool DetectPeripheralMotionHelper(
      Vision::Image &ratioImage,
      Vision::DebugImageList<Vision::CompressedImage> &debugImages,
      ExternalInterface::RobotObservedMotion &msg, f32 scaleMultiplier);

  bool DetectGroundAndImageHelper(
  ...
  void ExtractGroundPlaneMotion(s32 origNumRows, s32 origNumCols,
                                f32 scaleMultiplier,
  ...
  // Returns the number of times the ratio between the pixels in image and the
  // pixels in the previous image is above a threshold. The corresponding pixels
  // in ratio12 will be set to 255
  ...
  static size_t GetCentroid(const Vision::Image &motionImg,
                            Anki::Point2f &centroid, f32 xPercentile,
                            f32 yPercentile);

  template <class ImageType>
  inline s32 RatioTestNeonHelper(const u8 *&imagePtr, const u8 *&prevImagePtr,
                                 u8 *&ratioImgPtr, u32 numElementsToProcess);
