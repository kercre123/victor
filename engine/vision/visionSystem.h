...
class VisionSystem : public Vision::Profiler {
 public:
  ...
  Result UpdateCameraCalibration(
      std::shared_ptr<Vision::CameraCalibration> camCalib);

  const VisionModeSet& GetEnabledModes() const { return _modes; }
  bool IsModeEnabled(VisionMode whichMode) const {
    return _modes.Contains(whichMode);
  }
  ...
  Result AddCalibrationImage(const Vision::Image& calibImg,
    ...
    return _cameraCalibrator->AddCalibrationImage(calibImg, targetROI);
  }
  Result ClearCalibrationImages() {
    return _cameraCalibrator->ClearCalibrationImages();
  }
  size_t GetNumStoredCalibrationImages() const {
    return _cameraCalibrator->GetNumStoredCalibrationImages();
  }
  const std::vector<CameraCalibrator::CalibImage>& GetCalibrationImages()
      const {
    return _cameraCalibrator->GetCalibrationImages();
  }
  const std::vector<Pose3d>& GetCalibrationPoses() const {
    return _cameraCalibrator->GetCalibrationPoses();
  }
  ...
  void SetFaceEnrollmentMode(Vision::FaceID_t forFaceID = Vision::UnknownFaceID,
                             s32 numEnrollments = -1, bool forceNewID = false);
  ...
  Result LoadFaceAlbum(const std::string& albumName,
  ...
  Result RenameFace(Vision::FaceID_t faceID, const std::string& oldName,
  ...
  const RollingShutterCorrector& GetRollingShutterCorrector() {
    return _rollingShutterCorrector;
  }
  void ShouldDoRollingShutterCorrection(bool b) {
    _doRollingShutterCorrection = b;
  }
  bool IsDoingRollingShutterCorrection() const {
    return _doRollingShutterCorrection;
  }

  static f32 GetBodyTurnSpeedThresh_degPerSec();

  s32 GetMinCameraExposureTime_ms() const {
    return MIN_CAMERA_EXPOSURE_TIME_MS;
  }
  s32 GetMaxCameraExposureTime_ms() const {
    return MAX_CAMERA_EXPOSURE_TIME_MS;
  }
 ...
 protected:
  ...
  std::pair<bool, Vision::CameraParams>
      _nextCameraParams;  // bool represents if set but not yet sent
  ...
  std::map<std::string, std::unique_ptr<NeuralNets::NeuralNetRunner>>
      _neuralNetRunners;
  ...
  enum class MarkerDetectionCLAHE : u8 {
    ...
    Alternating =
        3,  // Alternate using CLAHE vs. original in each successive frame
    WhenDark = 4,  // Only if mean of image is below kClaheWhenDarkThreshold
    Count
  };

  // Updates the rolling shutter corrector
  // Will only recompute compensation once per timestamp, so can be called
  // multiple times
  void UpdateRollingShutter(const VisionPoseData& poseData,
                            const Vision::ImageCache& imageCache);

  // Uses grayscale
  Result ApplyCLAHE(Vision::ImageCache& imageCache,
  ...
  // Used for UpdateCameraParams below to keep up with regions to use for
  // metering, based on detected markers/faces The TimeStamp is used to keep
  // metering from recent detections briefly, even after we lose them
  using DetectionRectsByMode =
      std::map<VisionMode, std::vector<Rectangle<s32>>>;
  ...
  void AddFakeDetections(const TimeStamp_t atTimestamp,
                         const std::set<VisionMode>& modes);  // For debugging
