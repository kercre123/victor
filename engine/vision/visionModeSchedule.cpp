...
VisionModeSchedule::VisionModeSchedule() : VisionModeSchedule(true) {}
...
VisionModeSchedule::VisionModeSchedule(int onFrequency, int frameOffset)
  ...
  DEV_ASSERT((frameOffset < onFrequency),
             "VisionModeSchedule.ReceivedOutOfBoundsFrameOffset");
...
Result VisionModeSchedule::SetFromJSON(const Json::Value& jsonSchedule) {
  if (jsonSchedule.isArray()) {
    ...
    for (auto jsonIter = jsonSchedule.begin(); jsonIter != jsonSchedule.end();
         ++jsonIter) {
    ...
    *this = VisionModeSchedule(jsonSchedule.asInt());
    ...
    PRINT_NAMED_ERROR(
        "VisionModeSchedule.SetFromJSON.UnrecognizedModeScheduleValue",
...
AllVisionModesSchedule::ScheduleArray
    AllVisionModesSchedule::sDefaultSchedules =
        AllVisionModesSchedule::InitDefaultSchedules();
...
AllVisionModesSchedule::AllVisionModesSchedule(
    const std::list<std::pair<VisionMode, VisionModeSchedule>>& schedules,
    bool useDefaultsForUnspecified)
    : AllVisionModesSchedule(useDefaultsForUnspecified) {
  for (auto schedIter = schedules.begin(); schedIter != schedules.end();
       ++schedIter) {
...
AllVisionModesSchedule::InitDefaultSchedules() {
  ...
  std::fill(defaultInitialArray.begin(), defaultInitialArray.end(),
            VisionModeSchedule(false));
  return defaultInitialArray;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
VisionModeSchedule& AllVisionModesSchedule::GetScheduleForMode(
    VisionMode mode) {
  return _schedules[(size_t)mode];
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
const VisionModeSchedule& AllVisionModesSchedule::GetScheduleForMode(
    VisionMode mode) const {
  return _schedules[(size_t)mode];
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void AllVisionModesSchedule::SetDefaultSchedule(VisionMode mode,
                                                VisionModeSchedule&& schedule) {
  sDefaultSchedules[(size_t)mode] = std::move(schedule);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Result AllVisionModesSchedule::SetDefaultSchedulesFromJSON(
