 ...
 * Date:   2018-05-25
 *
 * Description: Vision system component for detecting scene illumination
 *state/changes.
 *
...
CONSOLE_VAR(bool, kEnableExtraIlluminationDetectorDebug, "Vision.Illumination",
            false);
}  // namespace
...
IlluminationDetector::IlluminationDetector()
    : _featureGate(nullptr), _classifier(new LinearClassifier()) {}

Result IlluminationDetector::Init(const Json::Value& config,
...
#define PARSE_PARAM(conf, key, var, vtype, func)                    \
  if (!JsonTools::func<vtype>(conf, key, var)) {                    \
    PRINT_NAMED_ERROR("IlluminationDetector.Init.MissingParameter", \
                      "Could not parse parameter: %s", key);        \
    return RESULT_FAIL;                                             \
  }

  // Read classifier parameters from separate file
  std::string classifierConfigPath;
  PARSE_PARAM(config, "ClassifierConfigPath", classifierConfigPath, std::string,
              GetValueOptional);
  const std::string fullPath = context->GetDataPlatform()->pathToResource(
      Anki::Util::Data::Scope::Resources, classifierConfigPath);
  ...
  Result classifierResult =
      _classifier->Init(classifierConfig["LinearClassifier"]);
  ...
  PARSE_PARAM(classifierConfig, "FeatureWindowLength", _featWindowLength, u32,
              GetValueOptional);
  PARSE_PARAM(classifierConfig, "FeaturePercentiles", percs, f32,
              GetVectorOptional);
  for (unsigned int i = 0; i < percs.size(); ++i) {
    ...
    if (i > 0 && Util::IsFltLE(percs[i], percs[i - 1])) {
      PRINT_NAMED_ERROR("IlluminationDetector.Init.InvalidPercentile",
                        "Percentile %f not greater than previous %f", percs[i],
                        percs[i - 1]);
      return RESULT_FAIL;
    }
  ...
  PARSE_PARAM(config, "FeaturePercentileSubsample", _featPercSubsample, s32,
              GetValueOptional);
  PARSE_PARAM(config, "IlluminatedMinProbability", _illumMinProb, f32,
              GetValueOptional);
  PARSE_PARAM(config, "DarkenedMaxProbability", _darkMaxProb, f32,
              GetValueOptional);
...
Result IlluminationDetector::Detect(
    Vision::ImageCache& cache, const VisionPoseData& poseData,
...
bool IlluminationDetector::CanRunDetection(
    const VisionPoseData& poseData) const {
  ...
  return !state.WasCarryingObject() && !state.WasPickedUp() &&
         (_allowMovement || notMoving);
}
