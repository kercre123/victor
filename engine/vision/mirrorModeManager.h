...
class MirrorModeManager {
 public:
  ...
  // Populates the mirrorModeImg field of the VisionProcessingResult with the
  // given image and any detections it can from the same result.
  Result CreateMirrorModeImage(const Vision::ImageRGB& cameraImg,
                               VisionProcessingResult& procResult);

 private:
  Vision::ImageRGB _screenImg;
  std::list<std::pair<EngineTimeStamp_t, Vision::SalientPoint>>
      _salientPointsToDraw;
  std::array<u8, 256> _gammaLUT;
  f32 _currentGamma = 0.f;

  void DrawVisionMarkers(
