...
class LaserPointDetector {
 public:
  ...
  // If imageInColor is not empty, extra checks are done to verify red/green
  // color saturation. Otherwise, imageInGray is used for detecting potential
  // laser dots. isDarkExposure specifies whether the passed-in images were
  // captured under low-gain, fast-exposure settings.
  Result Detect(Vision::ImageCache& imageCache, const VisionPoseData& poseData,
  ...
  // Same as above, but without the poseData. Searches in the whole image. Used
  // for testing and debug
  Result Detect(Vision::ImageCache& imageCache, const bool isDarkExposure,
                std::list<ExternalInterface::RobotObservedLaserPoint>& points,
                Vision::DebugImageList<Vision::CompressedImage>& debugImages);
 ...
 private:
  ...
  bool IsOnGroundPlane(const Quad2f& groundQuadInImage,
                       const Vision::Image::ConnectedComponentStats& stat);
  bool IsSurroundedByDark(const Vision::Image& image,
                          const Vision::Image::ConnectedComponentStats& stat,
                          const f32 darkThresholdFraction);
  bool IsSaturated(const Vision::ImageRGB& image,
                   const Vision::Image::ConnectedComponentStats& stat,
