 ...
 */
float DiagonalMahalanobisDistance(const float *input, const double *means, const cv::Mat& cov) {
...
DrivingSurfaceClassifier::DrivingSurfaceClassifier(const Json::Value& config, const CozmoContext* context)
  : _context(context)
{
...
#define GET_JSON_PARAMETER(__config, __paramname, __variable) \
  ...
  if (! regularization.empty()) {
    if (regularization == "L1") {
      ...
      PRINT_NAMED_WARNING("DrivingSurfaceClassifier.WrongJsonParameter", "Regularization value is unknown: %s. Valid"
                                     ...
                                     const OverheadMap::PixelSet& nonDrivablePixels)
{
  ...
  // iterators here make sense to modify two matrices in sequence
  {
    ...
    auto outputsIterator = allClasses.begin<cv::Vec3b>();
...
bool DrivingSurfaceClassifier::Train(const cv::Mat& allInputs, const cv::Mat& allClasses, uint numberOfPositives)
{
  ...
  DEV_ASSERT(allClasses.cols == 1, "Classes matrix must have 3 columns");
  ...
  // Train the Gaussian Mixture Model
  {
    ...
    try {
      result = this->_gmm->trainEM(gmmInput);
    }
    catch (cv::Exception& e) {
      PRINT_NAMED_ERROR("DrivingSurfaceClassifier.Train.ErrorWhileTrainingEM", "%s", e.what());
      return false;
    }
    if (! result) {
      PRINT_NAMED_ERROR("DrivingSurfaceClassifier.Train.EMTRainingFail","");
      return false;
    }
  }
  ...
  // Train Logistic Regression
  {
    ...
    const std::vector<float> minDistances = this->MinMahalanobisDistanceFromGMM(allInputs);
    ...
    try {
      ...
      PRINT_NAMED_ERROR("DrivingSurfaceClassifier.Train.ErrorWhileTrainingLogistic", "%s", e.what());
      return false;
    }
    if (! result) {
      PRINT_NAMED_ERROR("DrivingSurfaceClassifier.Train.LogisticTrainingFail","Result from training is false!");
      return false;
    }
  }
...
std::vector<float> DrivingSurfaceClassifier::MinMahalanobisDistanceFromGMM(const cv::Mat& input, bool useWeight) const
{

  DEV_ASSERT(input.cols == 3, "Input matrix must have 3 columns");
  ...
  for (int i = 0; i < nRows; ++i) {
    ...
    for (int k = 0; k < _gmm->getClustersNumber(); ++k) {
      ...
      DEV_ASSERT(meansMat.type() == CV_64FC1, "DrivingSurfaceClassifier.MinMahalanobisDistanceFromGMM.WrongMatrixType");
      const double* means = _gmm->getMeans().ptr<double>(k);
      const cv::Mat& covariance = covs[k];
      DEV_ASSERT(covariance.type() == CV_64FC1, "DrivingSurfaceClassifier.MinMahalanobisDistanceFromGMM.WrongMatrixType");
      float dist = DiagonalMahalanobisDistance(inputRow, means, covariance);
      if (useWeight) {
        dist = dist / w;
      }
      if ( dist < minDistance) {
...
bool DrivingSurfaceClassifier::TrainFromFiles(const char *positiveDataFileName, const char *negativeDataFileName)
{
  ...
  {
    std::ifstream file(positiveDataFileName);
    if (!file.is_open()) {
      PRINT_NAMED_ERROR("DrivingSurfaceClassifier.TrainFromFiles.ErrorOpeningFile", "Error while opening file %s",
                        positiveDataFileName);
      return false;
    }
    ...
    while (! file.eof()) {
      int r, g, b;
      file >> r >> g >> b;
      inputElements.push_back(SingleRow(float(r), float(g), float(b)));
      numberOfPositives++;
    }
    numberOfPositives--; // remove last addition at eof
  }


  // negative elements
  {
    std::ifstream file(negativeDataFileName);
    if (!file.is_open()) {
      PRINT_NAMED_ERROR("DrivingSurfaceClassifier.TrainFromFiles.ErrorOpeningFile", "Error while opening file %s",
                        negativeDataFileName);
      return false;
    }
    using SingleRow = cv::Matx<float,1, 3>;
    while (! file.eof()) {
...
uchar DrivingSurfaceClassifier::PredictClass(const Vision::PixelRGB& pixel) const
{
  // Step 1: get the GMM response
  const cv::Mat input = (cv::Mat_<float>(1,3) << pixel.r(), pixel.g(), pixel.b());

  // Step 2: calculate the Mahalanobis distance
  const std::vector<float> minDistances = this->MinMahalanobisDistanceFromGMM(input);
  const cv::Mat minDistancesMat = cv::Mat(minDistances);
  ...
  DEV_ASSERT(result.size() == 1, "DrivingSurfaceClassifier.PredictClass.EmptyResultVector");
  return uchar(result[0]);
}

std::vector<uchar> DrivingSurfaceClassifier::PredictClass(const std::vector<Vision::PixelRGB>& pixels) const
{
...
{
  DEV_ASSERT(mat.channels() == 1, "DrivingSurfaceClassifier.WriteMat.WrongNumberOfChannels");
  DEV_ASSERT((mat.type() == CV_32F) ||
             (mat.type() == CV_64F) ||
             (mat.type() == CV_8U), "DrivingSurfaceClassifier.WriteMat.InvalidMatType"
  );
  ...
  if (!Util::FileUtils::CreateDirectory(path, false, true)) {
    PRINT_NAMED_ERROR("DrivingSurfaceClassifier.WriteMat.DirectoryError", "Error while creating folder %s",
                      path.c_str());
    return;
  }

  PRINT_CH_INFO("DrivingSurfaceClassifier", "DrivingSurfaceClassifier.WriteMat.PathInfo",
                "Saving the files to %s", path.c_str());

  std::string fullPath = Util::FileUtils::FullFilePath({path, filename});
  std::ofstream outputFile(fullPath);
  if (! outputFile.is_open()) {
    PRINT_NAMED_ERROR("DrivingSurfaceClassifier.WriteMat.FileNotOpen", "Error while opening file %s for writing",
                      fullPath.c_str());
    return;
  }
  switch (mat.type()) {
    ...
    default: // this will never happen
      DEV_ASSERT(false, "DrivingSurfaceClassifier.WriteMat.WrongType");
      break;
  }
