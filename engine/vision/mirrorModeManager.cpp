 ...
 * Date:   09/28/2018
 *
 * Description: Handles creating a "MirrorMode" image for displaying the camera
 *feed on the robot's face, along with various detections in a
...
// scale) and and in mirror mode (at specified scale)
CONSOLE_VAR_RANGED(f32, kDisplayMarkerNamesScale, "Vision.MirrorMode", 0.f, 0.f,
                   1.f);
...
CONSOLE_VAR_RANGED(f32, kMirrorModeFaceDebugFontScale, "Vision.MirrorMode",
                   0.5f, 0.1f, 1.f);

// TODO: Figure out the original image resolution? This just assumes "Default"
// for marker/face detection
constexpr f32 kXmax = (f32)DEFAULT_CAMERA_RESOLUTION_WIDTH;
constexpr f32 kHeightScale =
    (f32)FACE_DISPLAY_HEIGHT / (f32)DEFAULT_CAMERA_RESOLUTION_HEIGHT;
constexpr f32 kWidthScale =
    (f32)FACE_DISPLAY_WIDTH / (f32)DEFAULT_CAMERA_RESOLUTION_WIDTH;
}  // namespace

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
MirrorModeManager::MirrorModeManager()
    : _screenImg(FACE_DISPLAY_HEIGHT, FACE_DISPLAY_WIDTH), _gammaLUT{} {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
static Rectangle<f32> DisplayMirroredRectHelper(f32 x_topLeft, f32 y_topLeft,
                                                f32 width, f32 height) {
  const f32 x_topRight =
      x_topLeft + width;  // will become upper left after mirroring
  const Rectangle<f32> rect(
      (f32)FACE_DISPLAY_WIDTH -
          kWidthScale * x_topRight,  // mirror rectangle for display
      y_topLeft * kHeightScale, width * kWidthScale, height * kHeightScale);
...
static inline Point<2, T> MirrorPointHelper(const Point<2, T>& pt) {
  const Point<2, T> pt_mirror(kWidthScale * (kXmax - pt.x()),
                              pt.y() * kHeightScale);
  return pt_mirror;
}
...
static Quad2f DisplayMirroredQuadHelper(const Quad2f& quad) {
  // Mirror x coordinates, swap left/right points, and scale for each point in
  // the quad:
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void MirrorModeManager::DrawVisionMarkers(
    const std::list<Vision::ObservedMarker>& visionMarkers) {
  for (auto const& visionMarker : visionMarkers) {
    ...
    if (Util::IsFltGTZero(kDisplayMarkerNamesScale)) {
      _screenImg.DrawText({1., _screenImg.GetNumRows() - 1},
                          name.substr(strlen("MARKER_"), std::string::npos),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void MirrorModeManager::DrawFaces(
    const std::list<Vision::TrackedFace>& faceDetections) {
  for (auto const& faceDetection : faceDetections) {
    ...
    _screenImg.DrawRect(
        DisplayMirroredRectHelper(rect.GetX(), rect.GetY(), rect.GetWidth(),
                                  rect.GetHeight()),
        color, 3);

    const auto& debugInfo = faceDetection.GetRecognitionDebugInfo();
    if (!debugInfo.empty()) {
      const Vec2f fontSize =
          _screenImg.GetTextSize("Test", kMirrorModeFaceDebugFontScale, 1);
      s32 line = 1;
      for (const auto& info : debugInfo) {
        const std::string& name = info.name;
        std::string dispName(name.empty() ? "<unknown>" : name);
        dispName += "[" + std::to_string(info.matchedID) +
                    "]: " + std::to_string(info.score);
        const Point2f position{
            1.f, _screenImg.GetNumRows() - 1 -
                     (debugInfo.size() - line) * (fontSize.y() + 1)};
        _screenImg.DrawText(position, dispName, NamedColors::YELLOW,
                            kMirrorModeFaceDebugFontScale, true);
        ++line;
      }
    } else {
      ...
      _screenImg.DrawText({1.f, _screenImg.GetNumRows() - 1}, dispName,
                          NamedColors::YELLOW, kFontScale, true);
    }
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void MirrorModeManager::DrawAutoExposure(
    const VisionProcessingResult& procResult) {
  // Draw exposure and gain in the lower left of the screen
  // Use a static to keep the last params displayed even when AE wasn't run
  // (since it doesn't run every frame)
  ...
  if (procResult.modesProcessed.Contains(VisionMode::AutoExp)) {
    ...
    std::stringstream ss;
    ss << params.exposureTime_ms << " " << std::fixed << std::setprecision(2)
       << params.gain;
    exposureStr = ss.str();
  }
  ...
  _screenImg.DrawText(
      {_screenImg.GetNumCols() - textSize.x() - 1, _screenImg.GetNumRows() - 1},
      exposureStr, NamedColors::RED, kFontScale, kUseDropShadow);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void MirrorModeManager::DrawSalientPoints(
    const VisionProcessingResult& procResult) {
  ...
  if (procResult.modesProcessed.Contains(VisionMode::BrightColors) ||
      procResult.modesProcessed.ContainsAnyOf(
          GetVisionModesUsingNeuralNets())) {
    if (!usingFixedDrawTime) {
    ...
    const std::list<Vision::SalientPoint>& salientPoints =
        procResult.salientPoints;
    for (auto const& salientPoint_norm : salientPoints) {
      // Salient points are in normalized coordinates, so scale directly to
      // screen image size
  ...
  for (auto const& salientPointToDraw : _salientPointsToDraw) {
    ...
    const ColorRGBA color =
        (salientPoint.description.empty()
             ? NamedColors::RED
             : ColorRGBA::CreateFromColorIndex(colorIndex++));

    const std::string caption(
        salientPoint.description + "[" +
        std::to_string((s32)std::round(100.f * salientPoint.score)) +
    ...
    const Point2f mirroredCentroid(_screenImg.GetNumCols() - salientPoint.x_img,
                                   salientPoint.y_img);
    if (!str.empty()) {
      const bool kDropShadow = true;
      const bool kCentered = true;
      _screenImg.DrawText(mirroredCentroid, str, NamedColors::YELLOW, 0.6f,
                          kDropShadow, 1, kCentered);
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result MirrorModeManager::CreateMirrorModeImage(
    const Vision::ImageRGB& cameraImg,
  ...
  if (!Util::IsFltNear(_currentGamma, kMirrorModeGamma)) {
    ...
    for (s32 value = 0; value < 256; ++value) {
      _gammaLUT[value] =
          std::round(255.f * std::powf((f32)value * divisor, invGamma));
    }
  }
