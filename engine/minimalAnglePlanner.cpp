 ...
 * Created: 2015-11-09
 *
 * Description: A simple "planner" which tries to minimize the amount it turns
 *away from the angle it is currently facing. It will back straight up some
 *distance, then turn in place to face the goal, drive to the goal, then turn in
 *place again. Very similar to FaceAndApproachPlanner, but will look better in
 *some cases, e.g. when docking
 *
...
#define MINIMAL_ANGLE_PLANNER_LENGTH_THRESHOLD \
  (0.25f * DEFAULT_POSE_EQUAL_DIST_THRESOLD_MM)
...
EComputePathStatus MinimalAnglePlanner::ComputeNewPathIfNeeded(
  ...
  // backupDistance is automatically computed so that turn0 turns at most
  // maxTurnAngle away from the robots starting angle. The are caps on how long
  // that backup distance can be.

  // Here's some math on how to calculate the backup distance. feel free to
  // ignore The goal is to minimize backupDistance such that we turn at most
  // maxTurnAngle

  // This can equivalently be though of as turning exactly maxTurnAngle, and
  // solving for backupDistance. If we get a negative backupDistance, that means
  // we don't need to back up at all

  // let the robot start at (x_r, y_r, theta_r), and the goal is (x_g, y_g,
  // theta_g)
  ...
  // We first back up to an intermediate point (x_i, y_i, theta_r), which
  // matches theta_r since it is a straight back up then we turn exactly
  // maxTurnAngle (in the correct direction). now we have the following:

  // maxTurnAngle = atan2( y_i - y_g, x_i - x_g)
  // y_i = y_r - backupDistance * sin( theta_r )
  // x_i = x_r - backupDistance * cos( theta_r )

  // to make life easier, lets solve this problem assuming theta_r = 0. We are
  // solving for backupDistance, and backupDistance won't depend on theta_r, so
  // we can ignore it. Imagine this as rotating the entire problem by -theta_r
  ...
  DEV_ASSERT(maxTurnAngle > 0.0f && maxTurnAngle < M_PI / 2.0f,
             "MinimalAnglePlanner.InvalidMaxAngle");
  ...
  if (backupDistance > MINIMAL_ANGLE_PLANNER_LENGTH_THRESHOLD) {
    ...
    _path.AppendLine(curr.GetX(), curr.GetY(), backupIntermediatePose.GetX(),
                     backupIntermediatePose.GetY(),
                     -MINIMAL_ANGLE_PLANNER_TARGET_SPEED,
                     MINIMAL_ANGLE_PLANNER_ACCEL, MINIMAL_ANGLE_PLANNER_DECEL);
  ...
  Radians turn0Angle =
      atan2(_targetVec.y() - curr.GetY(), _targetVec.x() - curr.GetX());
  ...
  if (straightDist > MINIMAL_ANGLE_PLANNER_LENGTH_THRESHOLD) {
    // if we need to drive straight, then apply the previous turn (if there was
    // one) first
    if (deltaTheta.getAbsoluteVal().ToFloat() >
        MINIMAL_ANGLE_PLANNER_THETA_THRESHOLD) {
      _path.AppendPointTurn(
          curr.GetX(), curr.GetY(), curr.GetAngle().ToFloat(),
          turn0Angle.ToFloat(),
          deltaTheta < 0 ? -MINIMAL_ANGLE_PLANNER_TARGET_ROT_SPEED
                         : MINIMAL_ANGLE_PLANNER_TARGET_ROT_SPEED,
          MINIMAL_ANGLE_PLANNER_ROT_ACCEL, MINIMAL_ANGLE_PLANNER_ROT_DECEL,
          POINT_TURN_ANGLE_TOL, true);
    ...
    _path.AppendLine(curr.GetX(), curr.GetY(), nextPose.GetX(), nextPose.GetY(),
                     MINIMAL_ANGLE_PLANNER_TARGET_SPEED,
                     MINIMAL_ANGLE_PLANNER_ACCEL, MINIMAL_ANGLE_PLANNER_DECEL);
  ...
  LOG_INFO("MinimalAnglePlanner.FinalPosition", "(%f, %f, %fdeg)", curr.GetX(),
           curr.GetY(), curr.GetAngle().getDegrees());
