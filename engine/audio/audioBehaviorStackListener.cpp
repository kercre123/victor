 ...
 * Created: 5/30/2018
 *
 * Description: Audio Behavior Stack listeners is used to post audio scene event
 * for behavior stack updates. It loads a JSON metadata file that is maintained
 * by the Audio Designers, formatted by the audio build server scripts and is
 * delivered with the other audio assets. The file defines a behavior path and
 * stack state audio events. The Path string is considered to be the tail of the
 * path allowing designers to wildcard the beginning of the path. When the
 * behavior stack has an update it sends a message containing the current
 * behavior path and stack state. This class listens for those messages, finds
 * the most relevant behavior node and plays the event corresponding with the
 * stack state.
 *
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
AudioBehaviorStackListener::AudioBehaviorStackListener(
    EngineRobotAudioClient& audioClient, const CozmoContext* context)
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AudioBehaviorStackListener::HandleAudioBehaviorMessage(
    const ExternalInterface::AudioBehaviorStackUpdate& message) {
  using namespace ExternalInterface;
  if (message.branchPath.empty()) {
    PRINT_NAMED_WARNING("AudioBehaviorStackListener.HandleAudioBehaviorMessage",
                        "message.branchPath.IsEmpty");
    return;
  }
  ...
  if (nullptr == node->audioEvents.get()) {
    PRINT_NAMED_WARNING("AudioBehaviorStackListener.HandleAudioBehaviorMessage",
                        "BehaviorStackNode.AudioEvents.IsNull");
    // Do nothing
    return;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AudioBehaviorStackListener::LoadMetaData(
    Util::Data::DataPlatform* dataPlatform) {
  ASSERT_NAMED(dataPlatform != nullptr,
               "AudioBehaviorStackListener.LoadMetaData.dataPlatform.IsNull");
  if (dataPlatform == nullptr) {
    return;
  }

  // Load file from disk
  const std::string fileName = Util::FileUtils::FullFilePath(
      {"sound", std::string(kAudioBehaviorMetadataFile)});
  Json::Value behaviorNodeData;
  const bool readJson = dataPlatform->readAsJson(
      Anki::Util::Data::Scope::Resources, fileName, behaviorNodeData);
  if (!readJson || !behaviorNodeData.isArray()) {
    PRINT_NAMED_WARNING("AudioBehaviorStackListener.LoadMetaData",
                        "ErrorReadingJsonFile: '%s'", fileName.c_str());
    return;
  }
  ...
  for (const auto& aNode : behaviorNodeData) {
    if (!JsonTools::GetValueOptional(aNode, kPathKey, pathStr)) {
      // Must have path
      PRINT_NAMED_WARNING("AudioBehaviorStackListener.LoadMetaData",
                          "InvalidData.MissingPathKey");
      continue;
    }
    if (pathStr.empty()) {
      PRINT_NAMED_WARNING("AudioBehaviorStackListener.LoadMetaData",
                          "InvalidData.Path.EmptyString: '%s'",
                          pathStr.c_str());
      continue;
    }
    ...
    if (JsonTools::GetValueOptional(aNode, kActivateKey, activateEventStr)) {
      if (AudioMetaData::GameEvent::EnumFromString(activateEventStr,
                                                   activateEvent)) {
        audioNode->onActivate = activateEvent;
      } else {
        PRINT_NAMED_WARNING("AudioBehaviorStackListener.LoadMetaData",
                            "Path: '%s' has invalid '%s' audio event: '%s'",
                            pathStr.c_str(), kActivateKey.c_str(),
                            activateEventStr.c_str());
      }
    }
    // Set Deactivate audio event
    if (JsonTools::GetValueOptional(aNode, kDeactivateKey,
                                    deactivateEventStr)) {
      if (AudioMetaData::GameEvent::EnumFromString(deactivateEventStr,
                                                   deactivateEvent)) {
        audioNode->onDeactivate = deactivateEvent;
      } else {
        PRINT_NAMED_WARNING("AudioBehaviorStackListener.LoadMetaData",
                            "Path: '%s' has invalid '%s' audio event: '%s'",
                            pathStr.c_str(), kDeactivateKey.c_str(),
                            deactivateEventStr.c_str());
      }
    }

    // Store data into behavior tree
    // Behavior tree is built by storing the path in reverse order. First tail
    // and traverse to the head of path

    // Parse path
    std::istringstream stream(pathStr);
    std::vector<std::string> results(
        (std::istream_iterator<PathDelimiter<'/'>>(stream)),
    ...
    if (!EnumFromString(*revPathIt, behaviorId)) {
      PRINT_NAMED_WARNING("AudioBehaviorStackListener.LoadMetaData",
                          "Invalid BehaviorId from string: '%s'",
                          revPathIt->c_str());
      continue;
    }

    const auto reversedPathBehaviorTreeNodeIt =
        _reversedPathBehaviorTree.find(behaviorId);
    if (reversedPathBehaviorTreeNodeIt != _reversedPathBehaviorTree.end()) {
      // Found Leaf
      node = &reversedPathBehaviorTreeNodeIt->second;
    } else {
      // Add Leaf
      const auto emplaceIt =
          _reversedPathBehaviorTree.emplace(behaviorId, behaviorId);
    ...
    while (++revPathIt != results.rend()) {
      ...
      if (!EnumFromString(*revPathIt, behaviorId)) {
        ...
        if (childIt != node->childrenMap.end()) {
          ...
          // Add Child
          const auto emplaceIt =
              node->childrenMap.emplace(behaviorId, behaviorId);
          node = &emplaceIt.first->second;
        }
      }
    }  // while ( ++revPathIt != results.rend() )
