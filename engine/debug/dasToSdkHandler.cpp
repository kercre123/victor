...
void DasToSdkHandler::Init(IExternalInterface* externalInterface) {
  ...
  auto dasToSdkEventCallback =
      std::bind(&DasToSdkHandler::HandleEvent, this, std::placeholders::_1);
  ...
  for (auto tag : tagList) {
    _signalHandles.push_back(
        _externalInterface->Subscribe(tag, dasToSdkEventCallback));
  }
}

void SendJsonDasLogMessage(std::string jsonData, std::string fileName,
...
void SendAllDasSentMessage(uint8_t filesSent,
                           IExternalInterface* externalInterface) {
...
void DasToSdkHandler::SendJsonDasLogsToSdk() {
  PRINT_NAMED_INFO("DasToSdkHandler.SendJsonDasLogs",
                   "Sending das logs from %s", DASGetLogDir());

  std::vector<std::string> logFiles =
      Util::FileUtils::FilesInDirectory(DASGetLogDir(), true);
  uint8_t filesSent = 0;
  for (const auto& logFile : logFiles) {
    PRINT_NAMED_INFO("DasToSdkHandler.SendJsonDasLogs",
                     "Sending DAS json file: %s", logFile.c_str());
    ...
    for (size_t i = 0; i < postBody.size(); i += kMaxFlushSize) {
      ...
      SendJsonDasLogMessage(postBody.substr(i, kMaxFlushSize),
                            logFileName.c_str(), _externalInterface);
    }
    filesSent++;
  }

  PRINT_NAMED_INFO("DasToSdkHandler.SendJsonDasLogs",
                   "Done sending DAS json files");
  SendAllDasSentMessage(filesSent, _externalInterface);
}

void DasToSdkHandler::HandleEvent(
  ...
  switch (eventData.GetTag()) {
    ...
    default:
      PRINT_NAMED_ERROR(
          "DasToSdkManager.HandleEvent.UnhandledMessageGameToEngineTag",
