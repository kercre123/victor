...
DevLogProcessor::DevLogProcessor(const std::string& directory)
  ...
  if (!Util::FileUtils::DirectoryExists(_directoryName)) {
    PRINT_NAMED_ERROR("DevLogProcessor.Constructor.InvalidDirectory",
                      "Directory %s not found", _directoryName.c_str());
    _directoryName = "";
    return;
  }

  _vizMessageReader.reset(new DevLogReaderRaw(Util::FileUtils::FullFilePath(
      {_directoryName, DevLoggingSystem::kEngineToVizName})));
  _printReader.reset(new DevLogReaderPrint(Util::FileUtils::FullFilePath(
      {_directoryName, DevLoggingSystem::kPrintName})));
...
void DevLogProcessor::SetVizMessageCallback(
    DevLogReader::DataCallback callback) {
  if (_vizMessageReader) {
