...
bool DevLogReaderPrint::FillLogData(std::ifstream& fileHandle,
  ...
  std::copy(nextLineBuffer, nextLineBuffer + lineLength,
            reinterpret_cast<char*>(logData_out._data.data()));

  // Put in a null character at the end of the line we just copied with the
  // extra space we created above
  logData_out._data[lineLength] = '\0';

  return true;
}

uint32_t DevLogReaderPrint::GetFinalTimestamp_ms(
    std::ifstream& fileHandle) const {
  if (!fileHandle.good()) {
  ...
  // go to the end of the file and work backwards until we find a line which
  // starts with at a n-digit (or longer) number
  ...
  while (fileHandle.good()) {
    ...
    if (fileHandle.get() == '\n') {
      ...
      // if we didn't return, that means we didn't get a digit, so go back
      // further. Clear any error flags first (in case we hit an EOF along the
      // way)
      fileHandle.clear();
      fileHandle.seekg(pos);
    ...
    // not a newline, move backwards one character (-1 to undo the get, and -1
    // to move back)
