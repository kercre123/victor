...
class DevLogReader : Util::noncopyable {
 public:
  ...
  // Move forward in time by number of milliseconds specified. Can trigger
  // callbacks if they have been set Returns whether there is more data in the
  // logs to process
  ...
  // return the last known timestamp in this log. Note that some loggers may
  // return 0 if they can't easily calculate the total time
  uint32_t GetFinalTime() const { return _finalTime_ms; }

  // return the delta between current time and the next message, aka how much
  // time we should advance to (hopefully) see another message. Note that this
  // may return less than the given time (e.g. if it needs to read a new log
  // file)
  uint32_t GetNextMessageTimeDelta_ms() const;

 protected:
  virtual bool FillLogData(std::ifstream& fileHandle,
                           LogData& logData_out) const = 0;

  // called on init with a stream to the last file in the log. Function can do
  // what it wants with the file, but must leave the stream in the same state it
  // found it. Should return the final timestamp contained in the log, or 0 if
  // it can't process.
  virtual uint32_t GetFinalTimestamp_ms(std::ifstream& fileHandle) const {
    return 0;
  }
