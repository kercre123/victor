 ...
 * Created: 11/17/15
 *
 * Description: A lightweight wrapper around the console so that it can use CLAD
 *at the game level instead of util
 *
...
void DebugConsoleManager::Init(IExternalInterface* externalInterface,
  ...
  DEV_ASSERT(nullptr != externalInterface,
             "DebugConsoleManager.Init.NullExternalInterface");
  DEV_ASSERT(nullptr != robotInterface,
             "DebugConsoleManager.Init.NullRobotInterface");

  _externalInterface = externalInterface;
  _robotInterface = robotInterface;

  auto debugConsoleEventCallback =
      std::bind(&DebugConsoleManager::HandleEvent, this, std::placeholders::_1);
  ...
  for (auto tag : tagList) {
    _signalHandles.push_back(
        _externalInterface->Subscribe(tag, debugConsoleEventCallback));
  }
}

void FlushBuffer(std::vector<ExternalInterface::DebugConsoleVar>&& dataVals,
  ...
  if (dataVals.size() > 0) {
    ...
    externalInterface->Broadcast(
        ExternalInterface::MessageEngineToGame(std::move(message)));
  }
}

static void SetCladVarUnionFromConsoleVar(
    Anki::Vector::ConsoleVarUnion& outVarValue,
...
static void SetCladVarFromConsoleVar(
    ExternalInterface::DebugConsoleVar& outDebugConsoleVar,
    const Anki::Util::IConsoleVariable* consoleVar) {
...
static void SetCladVarFromConsoleFunc(
    ExternalInterface::DebugConsoleVar& outDebugConsoleVar,
    const Anki::Util::IConsoleFunction* consoleFunc) {
...
void DebugConsoleManager::SendAllDebugConsoleVars() {
  const Anki::Util::ConsoleSystem& consoleSystem =
      Anki::Util::ConsoleSystem::Instance();

  const Anki::Util::ConsoleSystem::VariableDatabase& varDatabase =
      consoleSystem.GetVariableDatabase();
  ...
  // Note: There is an MTU limit too to avoid the message being split up into
  // multiple packets - staying <1400 is good
  ...
  for (const auto& entry : varDatabase) {
    ...
    if (nullptr == consoleVar) {
      PRINT_NAMED_ERROR(
          "DebugConsoleManager.SendAllDebugConsoleVars.InvalidConsoleVariable",
    ...
    if (messageSize >= kMaxFlushSize) {
      DEV_ASSERT(messageSize < Anki::Comms::MsgPacket::MAX_SIZE,
                 "DebugConsoleManager.VarDatabaseOverMaxSize");
  ...
  const Anki::Util::ConsoleSystem::FunctionDatabase& funcDatabase =
      consoleSystem.GetFunctionDatabase();
  for (const auto& entry : funcDatabase) {
    const Anki::Util::IConsoleFunction* consoleFunc = entry.second;
    if (nullptr == consoleFunc) {
      PRINT_NAMED_ERROR(
          "DebugConsoleManager.SendAllDebugConsoleVars.InvalidConsoleFunction",
    ...
    if (messageSize >= kMaxFlushSize) {
      DEV_ASSERT(messageSize < Anki::Comms::MsgPacket::MAX_SIZE,
                 "DebugConsoleManager.FuncDatabaseOverMaxSize");
      FlushBuffer(std::move(dataVals), _externalInterface);
      dataVals.clear();
      messageSize = 0;
    }
...
void SendVerifyDebugConsoleVarMessage(
    IExternalInterface* externalInterface, const char* varName,
    const char* statusMessageText,
    const Anki::Util::IConsoleVariable* consoleVar, bool success) {
...
inline static void CopyStringHelper(const std::string& inputString,
                                    std::array<T, N_in>& outputString) {
  // Ensure zero-termination
  outputString.fill(0);

  // Make sure we don't look outside input or output's bounds, and leave
  // outputString zero-terminated
...
void DebugConsoleManager::HandleEvent(
  ...
  switch (eventData.GetTag()) {
    case ExternalInterface::MessageGameToEngineTag::GetDebugConsoleVarMessage: {
      const Anki::Vector::ExternalInterface::GetDebugConsoleVarMessage& msg =
          eventData.Get_GetDebugConsoleVarMessage();
      Anki::Util::IConsoleVariable* consoleVar =
          Anki::Util::ConsoleSystem::Instance().FindVariable(
              msg.varName.c_str());
      if (consoleVar) {
        SendVerifyDebugConsoleVarMessage(
            _externalInterface, msg.varName.c_str(),
            consoleVar->ToString().c_str(), consoleVar, true);
      } else {
        PRINT_NAMED_WARNING("DebugConsoleManager.HandleEvent.NoConsoleVar",
                            "No Console Var '%s'", msg.varName.c_str());
        SendVerifyDebugConsoleVarMessage(
            _externalInterface, msg.varName.c_str(), "Error: No such variable",
            consoleVar, false);
      }
    } break;
    case ExternalInterface::MessageGameToEngineTag::
        GetAllDebugConsoleVarMessage: {
      // Shoot back all the init messages
      SendAllDebugConsoleVars();
    } break;
    case ExternalInterface::MessageGameToEngineTag::
        RunDebugConsoleFuncMessage: {
      const Anki::Vector::ExternalInterface::RunDebugConsoleFuncMessage& msg =
          eventData.Get_RunDebugConsoleFuncMessage();
      Anki::Util::IConsoleFunction* consoleFunc =
          Anki::Util::ConsoleSystem::Instance().FindFunction(
              msg.funcName.c_str());
      if (consoleFunc) {
        enum { kBufferSize = 512 };
        char buffer[kBufferSize];
        const uint32_t res = NativeAnkiUtilConsoleCallFunction(
            msg.funcName.c_str(), msg.funcArgs.c_str(), kBufferSize, buffer);
        SendVerifyDebugConsoleFuncMessage(
            _externalInterface, msg.funcName.c_str(), buffer, (res != 0));
      } else {
        PRINT_NAMED_WARNING("DebugConsoleManager.HandleEvent.NoConsoleFunc",
                            "No Func named '%s'", msg.funcName.c_str());
        SendVerifyDebugConsoleFuncMessage(_externalInterface,
                                          msg.funcName.c_str(),
                                          "Error: No such function", false);
      }
    } break;
    case ExternalInterface::MessageGameToEngineTag::SetDebugConsoleVarMessage: {
      const Anki::Vector::ExternalInterface::SetDebugConsoleVarMessage& msg =
          eventData.Get_SetDebugConsoleVarMessage();
      ...
      if (consoleVar && consoleVar->ParseText(msg.tryValue.c_str())) {
        ...
        PRINT_NAMED_WARNING(
            "DebugConsoleManager.HandleEvent.SetDebugConsoleVarMessage",
            "Error setting %svar '%s' to '%s'", consoleVar ? "" : "UNKNOWN ",
    ...
    case ExternalInterface::MessageGameToEngineTag::
      ...
      const Anki::Vector::ExternalInterface::SetAnimDebugConsoleVarMessage&
          msg = eventData.Get_SetAnimDebugConsoleVarMessage();
      ...
      if (msg.varName.size() > robotInterfaceMsg.varName.size()) {
        ...
        Result sendResult = _robotInterface->SendMessage(
    ...
    case ExternalInterface::MessageGameToEngineTag::
      ...
      if (msg.funcName.size() > robotInterfaceMsg.funcName.size()) {
        PRINT_NAMED_WARNING(
            "DebugConsoleManager.HandleEvent.RunAnimDebugConsoleFuncMessage."
            "FuncNameTooLong",
            "Variable name '%s' exceeds maximum length of %zu",
        ...
        PRINT_NAMED_WARNING(
            "DebugConsoleManager.HandleEvent.RunAnimDebugConsoleFuncMessage."
            "FuncArgsTooLong",
            "Args '%s' exceeds maximum length of %zu", msg.funcArgs.c_str(),
            robotInterfaceMsg.funcArgs.size());
      } else {
    ...
    default:
      PRINT_NAMED_ERROR(
          "DebugConsoleManager.HandleEvent.UnhandledMessageGameToEngineTag",
          "Unexpected tag %u", (uint32_t)eventData.GetTag());
      assert(0);
      break;
  }
}
