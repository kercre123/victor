...
bool DevLogReaderRaw::FillLogData(std::ifstream& fileHandle,
                                  LogData& logData_out) const {
  ...
  static constexpr uint32_t metaDataSize =
      sizeof(sizeInBytes) + sizeof(logData_out._timestamp_ms);
  // There is no exact limit on the largest size a message could be, so this is
  // a really a rough sanity check
  static constexpr uint32_t kLargestReasonableDataSize = 4 * 1024;

  // Verify the size makes sense
  bool sizeMakesSense = (sizeInBytes > metaDataSize) &&
                        (sizeInBytes <= kLargestReasonableDataSize);
  ...
  fileHandle.read(reinterpret_cast<char*>(&logData_out._timestamp_ms),
                  sizeof(logData_out._timestamp_ms));
  if (!fileHandle.good()) {
    return false;
  }

  // Update our size to remove the metadata so we can copy the right number of
  // bytes remaining
  sizeInBytes -= metaDataSize;

  logData_out._data.resize(sizeInBytes);
  fileHandle.read(reinterpret_cast<char*>(logData_out._data.data()),
                  sizeInBytes);
  if (!fileHandle.good()) {
    return false;
  }
