...
void DevLogReader::Init() {
  if (!_files.empty()) {
    ...
    // if there is only one file, we can leave this one open. Otherwise, we'll
    // need to close it so UpdateForCurrentTime will open the proper file
    if (_files.size() > 1) {
...
bool DevLogReader::UpdateForCurrentTime(uint32_t time_ms) {
  ...
  while (_currentLogFileHandle.good()) {
    ...
    // If the log file is still good we decided to stop reading for a different
    // reason (aka next message is for later)
    if (_currentLogFileHandle.good()) {
      break;
    }
    ...
    if (!_currentLogFileHandle.good()) {
      PRINT_NAMED_ERROR("DevLogReader.UpdateForCurrentTime.FailBitSet",
                        "Fail bit set for file %s", _files.front().c_str());
    }
  }
...
// Returns true if a data message was extracted and callback called (because
// timestamp was earlier than passed in time)
bool DevLogReader::ExtractAndCallback(uint32_t time_ms) {
  // If it's now time to deal with the data we've already been holding on to, do
  // so. (if CheckTimeAndCallback returns false we know it isn't time yet for
  // the data, so return early)
  if (_currentLogData.IsValid() && !CheckTimeAndCallback(time_ms)) {
    return false;
  }

  // We expect the current log data to be empty at this point
  DEV_ASSERT(!_currentLogData.IsValid(),
             "DevLogReader.ExtractAndCallback.StaleExtractedData");
