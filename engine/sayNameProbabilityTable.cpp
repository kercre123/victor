 ...
 *              Probability of saying reduces each time the table returns that
 *the name should be said, down to a minimum. A minimum time between saying a
 *each name must have passed as well. Statistics/decay are tracked on a per-name
 *basis.
 *
...
// Controls how fast the probability of saying each name goes down after each
// time ShouldSayName returns true. Higher is _slower_ decay.
CONSOLE_VAR_RANGED(float, kSayNameProbDecayFactor, "SayNameProbability", 0.75f,
                   0.f, 1.f);

// Sets the minimum probability of saying a name, despite the decay factor above
CONSOLE_VAR_RANGED(float, kSayNameMinProb, "SayNameProbability", 0.1f, 0.f,
                   1.f);
...
bool SayNameProbabilityTable::UpdateShouldSayName(const std::string& name) {
  ...
  LOG_INFO("SayNameProbabilityTable.UpdateShouldSayName",
           ...
           (result.second ? "[NEW]" : ""), prob, timePassed, shouldSay);
