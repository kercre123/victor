...
SpeedChooser::SpeedChooser(Robot& robot) : _robot(robot) {
  if (_robot.HasExternalInterface()) {
    auto helper = MakeAnkiEventUtil(*_robot.GetExternalInterface(), *this,
                                    _signalHandles);
    using namespace ExternalInterface;
    helper
        .SubscribeGameToEngine<MessageGameToEngineTag::SetEnableSpeedChooser>();
  }
}
...
PathMotionProfile SpeedChooser::GetPathMotionProfile(const Pose3d& goal) {
  ...
  // Random acceleration
  motionProfile.accel_mmps2 = Util::numeric_cast<float>(
      _robot.GetRNG().RandDblInRange(minAccel_mmps2, maxAccel_mmps2));

  // Deceleration is opposite of acceleration
  motionProfile.decel_mmps2 =
      maxAccel_mmps2 - motionProfile.accel_mmps2 + minAccel_mmps2;

  // Speed based on distance to goal
  Pose3d pose;
  goal.GetWithRespectTo(_robot.GetPose(), pose);
  f32 distToObject = pose.GetTranslation().Length();
  f32 speed = (distToObject) * (maxSpeed_mmps - minSpeed_mmps) /
                  (distToObjectForMaxSpeed_mm) +
              minSpeed_mmps;
  ...
  LOG_INFO("SpeedChooser.GetPathMotionProfile",
           "distToGoal:%f using speed:%f revSpeed:%f accel:%f", distToObject,
           motionProfile.speed_mmps, motionProfile.reverseSpeed_mmps,
           motionProfile.accel_mmps2);

  return motionProfile;
}

PathMotionProfile SpeedChooser::GetPathMotionProfile(
    const std::vector<Pose3d>& goals) {
  if (goals.empty()) {
...
template <>
void SpeedChooser::HandleMessage(
    const ExternalInterface::SetEnableSpeedChooser& msg) {
