...
CONSOLE_VAR_RANGED(f32, kDefaultMaxObservationDistance_mm, "PoseConfirmation",
                   500.f, 50.f, 1000.f);
...
void ObservableObject::InitPose(const Pose3d& pose, PoseState poseState,
                                const float fromDistance_mm) {
  // This indicates programmer error: InitPose should only be called once on
  // an object and never once SetPose has been called
  DEV_ASSERT_MSG(!_poseHasBeenSet, "ObservableObject.InitPose.PoseAlreadySet",
                 "%s Object %d", EnumToString(GetType()), GetID().GetValue());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ObservableObject::SetPose(const Pose3d& newPose, f32 fromDistance,
  ...
  // Every object's pose should always be able to find a path to a valid origin
  // without crashing
  if (ANKI_DEV_CHEATS) {
    ANKI_VERIFY(GetPose().FindRoot().IsRoot(),
                "ObservableObject.SetPose.PoseRootIsNotRoot",
                "%s ID:%d at %s with parent '%s'", EnumToString(GetType()),
                GetID().GetValue(),
