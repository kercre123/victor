...
{
  _headingLine1 =
      "                     Engine   Engine    Sleep    Sleep     Over      "
      "RtE   EtR   GtE   EtG  GWtE  EtGW   Viz  Battery    CPU";
  _headingLine2 =
      "                   Duration     Freq Intended   Actual    Sleep    "
      "Count Count Count Count Count Count Count  Voltage   Freq";
  _headingLine2Extra = "  Active Feature/Behavior";
  _headingLine1CSV =
      ",,Engine,Engine,Sleep,Sleep,Over,RtE,EtR,GtE,EtG,GWtE,EtGW,Viz,Battery,"
      "CPU";
  _headingLine2CSV =
      ",,Duration,Freq,Intended,Actual,Sleep,Count,Count,Count,Count,Count,"
      "Count,Count,Voltage,Freq";
...
void PerfMetricEngine::Init(Util::Data::DataPlatform* dataPlatform,
...
void PerfMetricEngine::Update(const float tickDuration_ms,
  ...
  if (_isRecording) {
    ...
    if (robot != nullptr) {
      const auto& bc =
          robot->GetAIComponent().GetComponent<BehaviorComponent>();
      ...
      strncpy(frame._behavior, bsm.GetTopBehaviorDebugLabel().c_str(),
              sizeof(frame._behavior));
      frame._behavior[FrameMetricEngine::kBehaviorStringMaxSize - 1] =
...
const PerfMetric::FrameMetric& PerfMetricEngine::UpdateDumpAccumulators(
...
#define ENGINE_LINE_DATA_VARS_GRAPHABLE_ONLY                                  \
  frame._messageCountRobotToEngine, frame._messageCountEngineToRobot,         \
      ...
      frame._messageCountViz, frame._batteryVoltage, frame._cpuFreq_kHz

    static const char* kFormatLine =
        "    %5i %5i %5i %5i %5i %5i %5i %8.3f %6i\n";
    static const char* kFormatLineCSV = ",%i,%i,%i,%i,%i,%i,%i,%.3f,%i\n";

    return snprintf(&_dumpBuffer[dumpBufferOffset],
                    kSizeDumpBuffer - dumpBufferOffset,
...
#define ENGINE_LINE_DATA_VARS                                                 \
  frame._messageCountRobotToEngine, frame._messageCountEngineToRobot,         \
      ...
      frame._messageCountGatewayToEngine, frame._messageCountEngineToGateway, \
      frame._messageCountViz, frame._batteryVoltage, frame._cpuFreq_kHz,      \
      EnumToString(frame._activeFeature), frame._behavior

    static const char* kFormatLine =
        "    %5i %5i %5i %5i %5i %5i %5i %8.3f %6i  %s  %s\n";
...
int PerfMetricEngine::AppendSummaryData(const DumpType dumpType,
  ...
  DEV_ASSERT_MSG(lineIndex < kNumLinesInSummary,
                 "PerfMetricEngine.AppendSummaryData",
...
#define ENGINE_SUMMARY_LINE_VARS(StatCall)                                   \
  _accMessageCountRtE.StatCall(), _accMessageCountEtR.StatCall(),            \
      ...
      _accMessageCountGatewayToE.StatCall(),                                 \
      _accMessageCountEToGateway.StatCall(), _accMessageCountViz.StatCall(), \
      _accBatteryVoltage.StatCall(), _accCPUFreq.StatCall()

  static const char* kFormatLine =
      "    %5.1f %5.1f %5.1f %5.1f %5.1f %5.1f %5.1f %8.3f %6.0f\n";
  static const char* kFormatLineCSV =
      ",%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.3f,%.0f\n";

#define APPEND_SUMMARY_LINE(StatCall)                                       \
  lenOut = snprintf(&_dumpBuffer[dumpBufferOffset],                         \
                    kSizeDumpBuffer - dumpBufferOffset,                     \
                    dumpType == DT_FILE_CSV ? kFormatLineCSV : kFormatLine, \
