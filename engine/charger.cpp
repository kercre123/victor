 ...
 * Date:   6/5/2015
 *
 * Description: Defines a charger base object, which is a type of
 *ActionableObject
 *
 *
...
Charger::Charger()
  ...
  _marker = &AddMarker(Vision::MARKER_CHARGER_HOME, frontPose,
                       Point2f(kMarkerWidth, kMarkerHeight));

}  // Charger() Constructor

Charger::~Charger() { EraseVisualization(); }

void Charger::GeneratePreActionPoses(
  ...
  switch (type) {
    ...
    case PreActionPose::ActionType::PLACE_RELATIVE: {
      ...
      Pose3d poseWrtMarker(
          M_PI_2_F + kChargerPreDockPoseOffset.GetAngle().ToFloat(),
          Z_AXIS_3D(),
          {kChargerPreDockPoseOffset.GetX(), -kChargerPreDockPoseOffset.GetY(),
           -halfHeight},
          _marker->GetPose());

      poseWrtMarker.SetName("Charger" + std::to_string(GetID().GetValue()) +
                            "PreActionPose");

      preActionPoses.emplace_back(type, _marker, poseWrtMarker, 0);
      break;
    }
...
Pose3d Charger::GetRobotDockedPose() const {
  Pose3d pose(M_PI, Z_AXIS_3D(), Point3f{kRobotToChargerDistWhenDocked, 0, 0},
              GetPose());
...
Pose3d Charger::GetRobotPostRollOffPose() const {
  Pose3d pose(M_PI_F, Z_AXIS_3D(),
              Point3f{-kRobotToChargerDistPostRollOff, 0, 0}, GetPose());

  pose.SetName("Charger" + std::to_string(GetID().GetValue()) +
               "PostRollOffPose");
...
Pose3d Charger::GetDockPoseRelativeToRobot(const Robot& robot) {
  return Pose3d(M_PI_F, Z_AXIS_3D(),
                Point3f{kRobotToChargerDistWhenDocked, 0, 0}, robot.GetPose(),
                "ChargerDockPose");
}
...
Quad2f Charger::GetDockingAreaQuad() const {
  ...
  // Re-center
  Point2f center(chargerPose.GetTranslation().x(),
                 chargerPose.GetTranslation().y());
  boundingQuad += center;

  return boundingQuad;
}

std::vector<Pose3d> Charger::GenerateObservationPoses(
    Util::RandomGenerator& rng, const size_t nPoses,
    const float& span_rad) const {
  // Generate a uniformly distributed set of random poses in a semi-circle
  // (really a semi-annulus) in front of the charger. The poses should point at
  // the charger, and they should not be too far off from the marker normal, so
  ...
  // The charger's origin is at the front of the lip of the charger, and its x
  // axis points inward toward the marker. Therefore we want poses centered
  // around the angle pi (w.r.t. the charger), and pointing toward the charger
  // origin.
  const auto& chargerPose = GetPose();
  std::vector<Pose3d> outPoses;
  outPoses.reserve(nPoses);
  for (int i = 0; i < nPoses; i++) {
    const auto pt = RobotPointSamplerHelper::SamplePointInAnnulus(
        rng, kInnerAnnulusRadiusForObservation_mm,
        kOuterAnnulusRadiusForObservation_mm, minTheta, maxTheta);
    const f32 th = std::atan2(pt.y(), pt.x());
    outPoses.emplace_back(
        th + M_PI_F, Z_AXIS_3D(),
        Vec3f{pt.x(), pt.y(), chargerPose.GetTranslation().z()}, chargerPose);
  }
...
Charger* Charger::CloneType() const { return new Charger(); }

void Charger::Visualize(const ColorRGBA& color) const {
  Pose3d vizPose = GetPose().GetWithRespectToRoot();
  _vizHandle = _vizManager->DrawCharger(
      GetID().GetValue(), Charger::kPlatformLength + Charger::kWallWidth,
      Charger::kSlopeLength, Charger::kWidth, Charger::kHeight, vizPose, color);
}  // Visualize()
