...
class SmartFaceIDImpl {
 public:
  SmartFaceIDImpl(IExternalInterface& externalInterface)
      : _id(Vision::UnknownFaceID), _externalInterface(externalInterface) {
    ...
    helper.SubscribeEngineToGame<
        MessageEngineToGameTag::RobotChangedObservedFaceID>();
  }

  SmartFaceIDImpl(const SmartFaceIDImpl& other)
      : SmartFaceIDImpl(other._externalInterface) {
    // this will create new subscriptions for the messages so we call the
    // correct member functions
    _id = other._id;
  }

  SmartFaceIDImpl& operator=(const SmartFaceIDImpl& other) {
    // we are already subscribed to the messages, so no need to re-subscribe
    // here
    _id = other._id;
    return *this;
  }
  ...
  // we need to store this so that copy constructors can work (we need our own
  // subscription to the events, so that our member functions get called instead
  // of functions on the (possibly deleted) _other_
...
template <>
void SmartFaceIDImpl::HandleMessage(
    const ExternalInterface::RobotChangedObservedFaceID& msg) {
  if (_id == msg.oldID) {
    _id = msg.newID;
  }
}
...
template <>
void SmartFaceIDImpl::HandleMessage(
    const ExternalInterface::RobotDeletedFace& msg) {
  if (_id == msg.faceID) {
...
SmartFaceID::SmartFaceID(const Robot& robot, Vision::FaceID_t faceID)
    : _impl(
          robot.HasExternalInterface()
              ? new SmartFaceIDImpl(*robot.GetContext()->GetExternalInterface())
              : nullptr) {
  if (_impl) {
    _impl->_id = faceID;
  }
...
SmartFaceID::SmartFaceID(SmartFaceID&& other) : _impl(std::move(other._impl)) {
  // Move constructor transfers ownership of the impl, no need to do anything
  // else
}
...
bool SmartFaceID::IsValid() const {
  const bool hasImplAndHasValidID =
      _impl && (_impl->_id != Vision::UnknownFaceID);
  return hasImplAndHasValidID;
}

Vision::FaceID_t SmartFaceID::GetID() const {
  const Vision::FaceID_t idOrInvalid =
      _impl ? _impl->_id : Vision::UnknownFaceID;
  return idOrInvalid;
}
