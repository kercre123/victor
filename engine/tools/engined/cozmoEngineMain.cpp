...
static void configure_engine_advertising(Json::Value& config) {
  ...
  if (!config.isMember(AnkiUtil::kP_UI_ADVERTISING_PORT)) {
    config[AnkiUtil::kP_UI_ADVERTISING_PORT] =
        Anki::Vector::UI_ADVERTISING_PORT;
  }
}

static Anki::Util::Data::DataPlatform* createPlatform(
...
static bool cozmo_start(const Json::Value& configuration) {
  ...
  // When collecting developer logs, ownership of singleton VictorLogger is
  // transferred to singleton MultiLogger.
  //
  ...
  // - console filter for logs
  {
    ...
    if (!gDataPlatform->readAsJson(Anki::Util::Data::Scope::Resources,
                                   consoleFilterConfigPath,
                                   consoleFilterConfig)) {
      LOG_ERROR("cozmo_start", "Failed to parse Json file '%s'",
  ...
  if (!FACTORY_TEST ||
      (FACTORY_TEST &&
       !Anki::Vector::Factory::GetEMR()->fields.PACKED_OUT_FLAG)) {
    // Initialize Developer Logging System
    using DevLoggingSystem = Anki::Vector::DevLoggingSystem;
    const std::string& devlogPath =
        gDataPlatform->GetCurrentGameLogPath(LOG_PROCNAME);
    DevLoggingSystem::CreateInstance(devlogPath, appRunId);

    //
    // Replace singleton victor logger with a MultiLogger that manages both
    // victor logger and dev logger. Ownership of victor logger is transferred
    // to MultiLogger. Ownership of MultiLogger is managed by singleton
    // unique_ptr.
    //
    std::vector<Anki::Util::ILoggerProvider*> loggers = {
        gVictorLogger.release(), DevLoggingSystem::GetInstancePrintProvider()};
  ...
  LOG_INFO("cozmo_start",
           "Creating engine; Initialized data platform with persistentPath = "
           "%s, cachePath = %s, resourcesPath = %s",
  ...
  ANKI_CONSOLE_SYSTEM_INIT(
      gDataPlatform->GetCachePath("consoleVarsEngine.ini").c_str());
...
int main(int argc, char* argv[]) {
  ...
  auto targetEndFrameTime =
      runStart + (microseconds)(Anki::Vector::BS_TIME_STEP_MICROSECONDS);

  while (!gShutdown) {
    const duration<double> curTimeSeconds = tickStart - runStart;
    const double curTimeNanoseconds =
        Anki::Util::SecToNanoSec(curTimeSeconds.count());

    const bool tickSuccess = gEngineAPI->Update(
        Anki::Util::numeric_cast<BaseStationTime_t>(curTimeNanoseconds));

    const auto tickAfterEngineExecution = TimeClock::now();
    const auto remaining_us = duration_cast<microseconds>(
        targetEndFrameTime - tickAfterEngineExecution);
    const auto tickDuration_us =
        duration_cast<microseconds>(tickAfterEngineExecution - tickStart);
    ...
    if (remaining_us < microseconds(-10000)) {
      LOG_WARNING("CozmoEngineMain.main.overtime",
                  "Update() (%dms max) is behind by %.3fms",
                  Anki::Vector::BS_TIME_STEP_MS,
                  (float)(-remaining_us).count() * 0.001f);
    }
#endif
    ...
    targetEndFrameTime +=
        (microseconds)(Anki::Vector::BS_TIME_STEP_MICROSECONDS);

    // See if we've fallen quite far behind; if so, compensate by catching the
    // target frame end time up somewhat. This is so that we don't spend SEVERAL
    // frames trying to catch up (by depriving sleep time).
    const auto timeBehind_us = -remaining_us;
    static const auto kusPerFrame =
        ((microseconds)(Anki::Vector::BS_TIME_STEP_MICROSECONDS)).count();
    static const int kTooFarBehindFramesThreshold = 2;
    static const auto kTooFarBehindThreshold =
        (microseconds)(kTooFarBehindFramesThreshold * kusPerFrame);
    if (timeBehind_us >= kTooFarBehindThreshold) {
      ...
      LOG_WARNING("CozmoEngineMain.main.catchup",
                  "Update was too far behind so moving target end frame time "
                  "forward by an additional %.3fms",
    ...
    const auto timeSinceLastTick_us =
        duration_cast<microseconds>(tickStart - prevTickStart);
    prevTickStart = tickStart;

    const auto sleepTimeActual_us =
        duration_cast<microseconds>(tickStart - tickAfterEngineExecution);
    gEngineAPI->RegisterEngineTickPerformance(
        tickDuration_us.count() * 0.001f, timeSinceLastTick_us.count() * 0.001f,
        sleepTime_us.count() * 0.001f, sleepTimeActual_us.count() * 0.001f);

    if (!tickSuccess) {
      // If we fail to update properly, stop running (but after we've recorded
      // the above stuff)
