...
bool ArchiveUtil::CreateArchiveFromFiles(
    const std::string& outputPath, const std::string& filenameBase,
    const std::vector<std::string>& filenames) {
#if ANKI_HAS_LIBARCHIVE
  struct archive* newArchive = archive_write_new();
  if (nullptr == newArchive) {
    PRINT_NAMED_ERROR("ArchiveUtil.CreateArchiveFromFiles",
                      "Could not alloc new archive");
  }
  ...
  if (ARCHIVE_OK != errorCode) {
    PRINT_NAMED_ERROR("ArchiveUtil.CreateArchiveFromFiles",
                      "Error %s opening file for archive",
                      GetArchiveErrorString(errorCode));
...
std::string ArchiveUtil::RemoveFilenameBase(const std::string& filenameBase,
...
bool ArchiveUtil::CreateFilesFromArchive(const std::string& archivePath,
  ...
  if (nullptr == read_archive) {
    PRINT_NAMED_ERROR("ArchiveUtil.CreateFilesFromArchive",
                      "Could not alloc read_archive");
    return false;
  }

  // Set up the finishing task list (to close things properly after error or at
  // end)
  auto finishingTasks = std::vector<std::function<void()>>();
  auto doFinishingTasks = [&finishingTasks]() {
    for (const auto& task : finishingTasks) {
      task();
    }
  };
  ...
  if (nullptr == extract_archive) {
    PRINT_NAMED_ERROR("ArchiveUtil.CreateFilesFromArchive",
                      "Could not alloc extract_archive");
    doFinishingTasks();
    return false;
  }
  ...
  errorCode =
      archive_read_open_filename(read_archive, archivePath.c_str(), 10240);
  if (ARCHIVE_OK != errorCode) {
    PRINT_NAMED_ERROR("ArchiveUtil.CreateFilesFromArchive",
                      "Could not open filename %s: %s", archivePath.c_str(),
  ...
  for (;;) {
    archive_entry_clear(entry);
    // Read the next entry header. Use the '2' version to pull info into our
    // allocated entry object
    ...
    std::string destPathname =
        Util::FileUtils::FullFilePath({outputPath, curPathname});
...
static int copy_data(struct archive* ar, struct archive* aw) {
  ...
  for (;;) {
    ...
    errorCode = Util::numeric_cast<int>(
