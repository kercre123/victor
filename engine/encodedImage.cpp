...
EncodedImage::EncodedImage(const Vision::Image& imgGray, const u32 imageID)
  ...
  std::copy(imgGray.GetDataPointer(),
            imgGray.GetDataPointer() + imgGray.GetNumElements(),
...
bool EncodedImage::AddChunk(const ImageChunk& chunk) {
  ...
  if (chunk.imageId != _imgID) {
    ...
    // If the encoding is MiniGray then first byte of the image is whether or
    // not the image is in color (The encoding is hard coded deep within
    // firmware code and can't easily be changed so this is a work around) If it
    // is in color then update the encoding
    if (chunk.data[0] != 0 &&
        _encoding == Vision::ImageEncoding::JPEGMinimizedGray) {
  ...
  if (chunk.chunkId != _expectedChunkId) {
    PRINT_NAMED_WARNING("EncodedImage.AddChunk.ChunkOutOfOrder",
                        "Expected chunk %d, got chunk %d", _expectedChunkId,
                        chunk.chunkId);
    _isImgValid = false;
  }
  ...
  if (isLastChunk) {
    ...
    if (_numChunksReceived != chunk.imageChunkCount) {
      PRINT_NAMED_WARNING(
          "EncodedImage.AddChunk.UnexpectedNumberOfChunks",
          "Got last chunk, expected %d chunks but received %d chunks",
          chunk.imageChunkCount, _numChunksReceived);
      _isImgValid = false;
    } else {
      ...
      if (_prevTimestamp > _timestamp) {
        PRINT_NAMED_WARNING("EncodedImage.AddChunk.TimestampNotIncreasing",
                            "Got last chunk but current timestamp %u is less "
                            "than previous timestamp %u",
...
bool EncodedImage::IsColor() const {
  switch (_encoding) {
    case Vision::ImageEncoding::NoneImageEncoding: {
      ANKI_VERIFY(false, "EncodedImage.IsColor.UnsupportedImageEncoding", "%s",
...
static inline void DecodeHelper(const std::vector<u8>& buffer,
                                Vision::ImageRGB& decodedImg) {
  cv::imdecode(buffer, cv::IMREAD_COLOR, &decodedImg.get_CvMat_());
  cvtColor(decodedImg.get_CvMat_(), decodedImg.get_CvMat_(),
           CV_BGR2RGB);  // opencv will decode as BGR
}

static inline void DecodeHelper(const std::vector<u8>& buffer,
                                Vision::Image& decodedImg) {
  cv::imdecode(buffer, cv::IMREAD_GRAYSCALE, &decodedImg.get_CvMat_());
}

// General template for when Gray or RGB data is both passed in and also
// requested out.
...
template <>
inline void RawHelper(const Vision::Image& grayImgIn,
                      Vision::ImageRGB& rgbImgOut) {
  rgbImgOut = Vision::ImageRGB(grayImgIn);
}

// Specialization for converting rgb data to gray data
template <>
inline void RawHelper(const Vision::ImageRGB& rgbImageIn,
                      Vision::Image& grayImgOut) {
...
Result EncodedImage::DecodeImageHelper(ImageType& decodedImg) const {
  switch (_encoding) {
    ...
    case Vision::ImageEncoding::RawGray: {
      ...
      Vision::Image grayImg(_imgHeight, _imgWidth,
                            const_cast<u8*>(&(_buffer[0])));
    ...
    case Vision::ImageEncoding::RawRGB: {
      // Already decompressed.
      Vision::ImageRGB rgbImg(_imgHeight, _imgWidth,
                              const_cast<u8*>(&(_buffer[0])));
    ...
    case Vision::ImageEncoding::JPEGColorHalfWidth: {
      DecodeHelper(_buffer, decodedImg);
      cv::copyMakeBorder(decodedImg.get_CvMat_(), decodedImg.get_CvMat_(), 0, 0,
                         160, 160, cv::BORDER_CONSTANT, 0);
      break;
    }
    ...
    default:
      PRINT_NAMED_ERROR(
          "EncodedImage.DecodeImageRGB.UnsupportedEncoding",
  ...
  if (decodedImg.GetNumRows() != _imgHeight ||
      decodedImg.GetNumCols() != _imgWidth) {
    PRINT_NAMED_WARNING("EncodedImage.DecodeImageRGB.BadDecode",
                        "Failed to decode %dx%d image from buffer. Got %dx%d",
                        _imgWidth, _imgHeight, decodedImg.GetNumCols(),
                        decodedImg.GetNumRows());
...
void EncodedImage::MiniGrayToJpeg(const std::vector<u8>& bufferIn,
                                  const u16 height, const u16 width,
  ...
  static const u8 header50[] = {
      ...
      0x00, 0x10, 0x0B, 0x0C, 0x0E, 0x0C, 0x0A, 0x10,  // 0x19 = QTable
      ...
      // 0x0B, 0x08, 0x00, 0xF0, // 0x5E = Height x Width
      0x71, 0x79, 0x70, 0x64, 0x78, 0x5C, 0x65, 0x67, 0x63, 0xFF, 0xC0, 0x00,
      0x0B, 0x08, 0x01, 0x28,  // 0x5E = Height x Width

      // 0x01, 0x40, 0x01, 0x01, 0x11, 0x00, 0xFF, 0xC4, 0x00, 0xD2, 0x00, 0x00,
      // 0x01, 0x05, 0x01, 0x01,
...
void EncodedImage::MiniColorToJpeg(const std::vector<u8>& bufferIn,
                                   const u16 height, const u16 width,
                                   std::vector<u8>& bufferOut) {
  ...
  static const u8 header[] = {
      ...
      0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8, 0xF9, 0xFA, 0xFF, 0xDA, 0x00, 12,
      0x03,        // 3 components
      0x01, 0x00,  // Y
