...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Consts for mapping
// ObjectInteractionIntentions to their filters
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
using DependentIntentionArray =
    Util::FullEnumToValueArrayChecker::FullEnumToValueArray<
        ObjectInteractionIntention, const std::set<ObjectInteractionIntention>*,
...
constexpr DependentIntentionArray kDependentIntentionMap = {
    ...
    {ObjectInteractionIntention::StackBottomObjectAxisCheck,
     &stackBottomDependentAxis},
    {ObjectInteractionIntention::StackBottomObjectNoAxisCheck,
     &stackBottomDependentNoAxis},
...
ObjectInteractionInfoCache::ObjectInteractionInfoCache(const Robot& robot)
    : IDependencyManagedComponent<AIComponentID>(
          this, AIComponentID::ObjectInteractionInfoCache),
      _robot(robot) {
  // Pickup no axis check
  BlockWorldFilter* pickupAnyFilter = new BlockWorldFilter;
  pickupAnyFilter->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanBeInteractedWith, this,
                std::placeholders::_1));
  pickupAnyFilter->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanPickupNoAxisCheck, this,
                std::placeholders::_1));

  // Pickup with axis check
  BlockWorldFilter* pickupWithAxisFilter = new BlockWorldFilter;
  pickupWithAxisFilter->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanBeInteractedWith, this,
                std::placeholders::_1));
  pickupWithAxisFilter->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanPickupAxisCheck, this,
                std::placeholders::_1));

  // Stack top no axis check
  BlockWorldFilter* stackTopFilter = new BlockWorldFilter;
  stackTopFilter->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanBeInteractedWith, this,
                std::placeholders::_1));
  stackTopFilter->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanUseAsStackTopNoAxisCheck, this,
                std::placeholders::_1));

  // Stack top with axis check
  BlockWorldFilter* stackTopWithAxisFilter = new BlockWorldFilter;
  stackTopWithAxisFilter->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanBeInteractedWith, this,
                std::placeholders::_1));
  stackTopWithAxisFilter->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanUseAsStackTopAxisCheck, this,
                std::placeholders::_1));

  // stack bottom no axis check
  BlockWorldFilter* stackBottomFilter = new BlockWorldFilter;
  stackBottomFilter->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanBeInteractedWith, this,
                std::placeholders::_1));
  stackBottomFilter->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanUseAsStackBottomNoAxisCheck,
                this, std::placeholders::_1));

  // stack bottom axis check
  BlockWorldFilter* stackBottomWithAxis = new BlockWorldFilter;
  stackBottomWithAxis->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanBeInteractedWith, this,
                std::placeholders::_1));
  stackBottomWithAxis->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanUseAsStackBottomAxisCheck, this,
                std::placeholders::_1));

  // Roll object with delegate - no axis check
  BlockWorldFilter* rollNoAxisFilter = new BlockWorldFilter;
  rollNoAxisFilter->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanBeInteractedWith, this,
                std::placeholders::_1));
  rollNoAxisFilter->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanRollObjectDelegateNoAxisCheck,
                this, std::placeholders::_1));

  // Roll object with delegate - axis check
  BlockWorldFilter* rollWithAxisFilter = new BlockWorldFilter;
  rollWithAxisFilter->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanBeInteractedWith, this,
                std::placeholders::_1));
  rollWithAxisFilter->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanRollObjectDelegateAxisCheck,
                this, std::placeholders::_1));

  // Function for selecting the best object for rolling
  auto rollObjectBestObjectFunc =
      std::bind(&ObjectInteractionInfoCache::RollBlockBestObjectFunction, this,
                std::placeholders::_1);

  // Pop A Wheelie check
  BlockWorldFilter* popFilter = new BlockWorldFilter;
  popFilter->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanBeInteractedWith, this,
                std::placeholders::_1));
  popFilter->AddFilterFcn(
      std::bind(&ObjectInteractionInfoCache::CanUseForPopAWheelie, this,
                std::placeholders::_1));
  ...
  // Function for selecting the best object given distance/position in
  // configurations
  auto defaultBestObjectFunc =
      std::bind(&ObjectInteractionInfoCache::DefaultBestObjectFunction, this,
                std::placeholders::_1);

  FullBestInteractionArray findBestObjectFunctions = {
      {ObjectInteractionIntention::PickUpObjectNoAxisCheck,
       defaultBestObjectFunc},
      ...
      {ObjectInteractionIntention::StackTopObjectNoAxisCheck,
       defaultBestObjectFunc},
      {ObjectInteractionIntention::RollObjectWithDelegateNoAxisCheck,
       rollObjectBestObjectFunc},
      {ObjectInteractionIntention::RollObjectWithDelegateAxisCheck,
       rollObjectBestObjectFunc},
      {ObjectInteractionIntention::PopAWheelieOnObject, defaultBestObjectFunc}};

  ConfigureObjectInteractionFilters(robot, validInteractionFilters,
                                    findBestObjectFunctions);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ObjectInteractionInfoCache::ConfigureObjectInteractionFilters(
  ...
  while (static_cast<ObjectInteractionIntention>(interactionEnum) !=
         ObjectInteractionIntention::Count) {
    auto validFilter = validInteractions[interactionEnum];
    auto bestFilter = bestInteractions[interactionEnum];
    _trackers.emplace(
        validFilter.EnumValue(),
        ObjectInteractionCacheEntry(
            robot, ObjectUseIntentionToString(validFilter.EnumValue()),
            validFilter.Value(), bestFilter.Value()));
    interactionEnum++;
  }
}
...
ObjectInteractionInfoCache::GetDefaultFilterForIntention(
    ObjectInteractionIntention intention) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ObjectInteractionInfoCache::IsObjectValidForInteraction(
    ObjectInteractionIntention intention, const ObjectID& object) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ObjectInteractionInfoCache::EnsureInformationValid(
    ObjectInteractionIntention intention) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ObjectInteractionInfoCache::ObjectTapInteractionOccurred(
  ...
  if (!filterCanUseObject) {
    PRINT_CH_INFO("ObjectInteractionInfoCache",
                  "SetObjectTapInteration.NoFilter",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
const char* ObjectInteractionInfoCache::ObjectUseIntentionToString(
    ObjectInteractionIntention intention) {
  switch (intention) {
    case ObjectInteractionIntention::PickUpObjectNoAxisCheck: {
      return "PickUpObjectNoAxisCheck";
    }
    case ObjectInteractionIntention::PickUpObjectAxisCheck: {
      return "PickUpObjectAxisCheck";
    }
    case ObjectInteractionIntention::StackBottomObjectAxisCheck: {
      return "StackBottomObjectAxisCheck";
    }
    case ObjectInteractionIntention::StackBottomObjectNoAxisCheck: {
      return "StackBottomObjectNoAxisCheck";
    }
    case ObjectInteractionIntention::StackTopObjectAxisCheck: {
      return "StackTopObjectAxisCheck";
    }
    case ObjectInteractionIntention::StackTopObjectNoAxisCheck: {
      return "StackTopObjectNoAxisCheck";
    }
    case ObjectInteractionIntention::RollObjectWithDelegateAxisCheck: {
      return "RollObjectWithDelegateAxisCheck";
    }
    case ObjectInteractionIntention::RollObjectWithDelegateNoAxisCheck: {
      return "RollObjectWithDelegateNoAxisCheck";
    }
    case ObjectInteractionIntention::PopAWheelieOnObject: {
      return "PopAWheelieOnObject";
    }
    case ObjectInteractionIntention::Count: {
      DEV_ASSERT(false, "ObjectInteractionInfoCache.InvalidIntention");
      return "";
    }
  };

  // should never get here, assert if it does (programmer error specifying
  // intention enum class)
  DEV_ASSERT(
      false,
      "ObjectInteractionInfoCache.ObjectUseIntentionToString.InvalidIntention");
  return "UNDEFINED_ERROR";
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ObjectInteractionInfoCache::CanBeInteractedWith(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ObjectInteractionInfoCache::CanPickupNoAxisCheck(
    const ObservableObject* object) const {
  if (nullptr == object) {
    PRINT_NAMED_ERROR(
        "ObjectInteractionInfoCache.CanPickupNoAxisCheck.NullObject",
        "object was null");
    return false;
  }
  ...
  const bool recentlyFailed = whiteboard.DidFailToUse(
      object->GetID(),
      {{ObjectActionFailure::PickUpObject,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ObjectInteractionInfoCache::CanPickupAxisCheck(
    const ObservableObject* object) const {
  ...
  // check the up axis
  const bool upAxisOk = object->GetPose()
                            .GetWithRespectToRoot()
                            .GetRotationMatrix()
                            .GetRotatedParentAxis<'Z'>() == AxisName::Z_POS;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ObjectInteractionInfoCache::CanUseAsStackTopNoAxisCheck(
    const ObservableObject* object) const {
  if (_robot.GetCarryingComponent().IsCarryingObject()) {
    return object == _robot.GetBlockWorld().GetLocatedObjectByID(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ObjectInteractionInfoCache::CanUseAsStackTopAxisCheck(
    const ObservableObject* object) const {
  if (_robot.GetCarryingComponent().IsCarryingObject()) {
    const bool isCarriedObj =
        (object == _robot.GetBlockWorld().GetLocatedObjectByID(
                       _robot.GetCarryingComponent().GetCarryingObjectID()));
    const bool isCarriedUpright =
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ObjectInteractionInfoCache::CanUseAsStackBottomHelper(
    const ObservableObject* object, ObjectInteractionIntention intention) {
  // already in use as the top object
  if (object->GetID() == GetBestObjectForIntention(intention)) {
    return false;
  }

  const bool hasFailedRecently =
      _robot.GetAIComponent().GetComponent<AIWhiteboard>().DidFailToUse(
          object->GetID(), ObjectActionFailure::StackOnObject,
          kTimeObjectInvalidAfterStackFailure_sec, object->GetPose(),
          DefaultFailToUseParams::kObjectInvalidAfterFailureRadius_mm,
          kAngleToleranceAfterFailure_radians);

  bool ret =
      (!hasFailedRecently && IsValidLightCube(object->GetType(), false) &&
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ObjectInteractionInfoCache::CanUseAsStackBottomNoAxisCheck(
    const ObservableObject* object) {
  return CanUseAsStackBottomHelper(
      object, ObjectInteractionIntention::StackTopObjectNoAxisCheck);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ObjectInteractionInfoCache::CanUseAsStackBottomAxisCheck(
    const ObservableObject* object) {
  if (CanUseAsStackBottomHelper(
          object, ObjectInteractionIntention::StackTopObjectAxisCheck)) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ObjectInteractionInfoCache::CanUseForPopAWheelie(
    const ObservableObject* object) const {
  auto& whiteboard = _robot.GetAIComponent().GetComponent<AIWhiteboard>();

  const bool hasFailedToPopAWheelie = whiteboard.DidFailToUse(
      object->GetID(), ObjectActionFailure::RollOrPopAWheelie,
      DefaultFailToUseParams::kTimeObjectInvalidAfterFailure_sec,
      object->GetPose(),
      DefaultFailToUseParams::kObjectInvalidAfterFailureRadius_mm,
      kAngleToleranceAfterFailure_radians);

  return (!hasFailedToPopAWheelie &&
          _robot.GetDockingComponent().CanPickUpObjectFromGround(*object));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ObjectInteractionInfoCache::CanRollObjectDelegateNoAxisCheck(
    const ObservableObject* object) const {
  auto& whiteboard = _robot.GetAIComponent().GetComponent<AIWhiteboard>();

  const bool hasFailedToRoll = whiteboard.DidFailToUse(
      object->GetID(), ObjectActionFailure::RollOrPopAWheelie,
      DefaultFailToUseParams::kTimeObjectInvalidAfterFailure_sec,
      object->GetPose(),
      DefaultFailToUseParams::kObjectInvalidAfterFailureRadius_mm,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ObjectInteractionInfoCache::CanRollObjectDelegateAxisCheck(
    const ObservableObject* object) const {
  const Pose3d p = object->GetPose().GetWithRespectToRoot();
  return (
      CanRollObjectDelegateNoAxisCheck(object) &&
      (p.GetRotationMatrix().GetRotatedParentAxis<'Z'>() != AxisName::Z_POS));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
ObjectID ObjectInteractionInfoCache::DefaultBestObjectFunction(
    const std::set<ObjectID>& validObjects) {
  ...
  for (const auto& objID : validObjects) {
    ...
    if (ComputeDistanceSQBetween(_robot.GetPose(), obj->GetPose(),
                                 currentDistSQ) &&
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
ObjectID ObjectInteractionInfoCache::RollBlockBestObjectFunction(
    const std::set<ObjectID>& validObjects) {
  std::set<ObjectID> objsOnSideNothingOnTop;
  for (const auto& objID : validObjects) {
    const ObservableObject* validObj =
        _robot.GetBlockWorld().GetLocatedObjectByID(objID);
    if (validObj == nullptr) {
      continue;
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
ObjectInteractionCacheEntry::ObjectInteractionCacheEntry(
    const Robot& robot, const std::string& debugName,
    BlockWorldFilter* validFilter, BestObjectFunction bestObjFunc)
    : _robot(robot),
...
void ObjectInteractionCacheEntry::EnsureInformationValid() {
  const f32 currentTimeInSeconds =
  ...
  if (!FLT_NEAR(_timeUpdated_s, currentTimeInSeconds)) {
    ...
    blockWorld.FindLocatedMatchingObjects(*_blockWorldFilterValidBlocks,
                                          objects);
    ...
    const bool bestIsStillValid =
        _validObjects.find(_bestObject) != _validObjects.end();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ObjectInteractionCacheEntry::ObjectTapInteractionOccurred(
    const ObjectID& objectID) {
  ...
  // consider located instance only, if we don't know where it is we can't use
  // it
  if ((nullptr != locatedObject) &&
    ...
    if (oldBest != objectID) {
      PRINT_CH_INFO("ObjectInteractionInfoCache", "SetBestObjectForTap",
                    ...
                    objectID.GetValue(), _debugName.c_str());
    }
