...
class FaceSelectionComponent
 ...
 public:
  using FaceSelectionFactorMap =
      std::unordered_map<FaceSelectionPenaltyMultiplier, float>;

  static const FaceSelectionFactorMap kDefaultSelectionCriteria;

  explicit FaceSelectionComponent(
      const Robot& robot, const FaceWorld& faceWorld,
      const MicDirectionHistory& micDirectionHistory,
      const VisionComponent& visionComp)
      : IDependencyManagedComponent<AIComponentID>(
            this, AIComponentID::FaceSelection),
        _robot(robot),
  ...
  // try to read a list of factor penalties from the given json config. Return
  // true and modify the passed in criteria on success, false otherwise
  static bool ParseFaceSelectionFactorMap(const Json::Value& config,
                                          FaceSelectionFactorMap& output);

  SmartFaceID GetBestFaceToUse(
  ...
  // Checks to see whether any faces should be in front of the robot based on
  // its current orientation NOTE: This is NOT a check of whether faces are
  // currently in frame/at the robot's current head angle it's a check of last
  // known position within a range of where the robot's facing
 ...
 private:
  ...
  float CalculateTimeSinceSeenCost(
      const Vision::TrackedFace* currentFace) const;
  float CalculateNotMakingEyeContact(
      const Vision::TrackedFace* currentFace) const;
