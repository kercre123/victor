 ...
 * Created: 06/04/18
 *
 * Description: A component for salient points. Listens to messages and
 *categorizes them based on their type
 *
...
SalientPointsComponent::SalientPointsComponent()
    : IDependencyManagedComponent<AIComponentID>(
          this, AIComponentID::SalientPointsDetectorComponent),
      _robot(nullptr) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void SalientPointsComponent::InitDependent(Robot* robot,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void SalientPointsComponent::AddSalientPoints(
    const std::list<Vision::SalientPoint>& c) {
  PRINT_CH_DEBUG("Behaviors",
                 "SalientPointsComponent.AddSalientPoints.PointsReceived",
                 "Number of salient point received: %zu, previous size: %zu",
                 c.size(), _salientPoints.size());

  std::set<Vision::SalientPointType>
      erased;  // which lists (associated with a type) have been erased already
  ...
  PRINT_CH_DEBUG(
      "Behaviors", "SalientPointsComponent.AddSalientPoints.NewPointsSize",
  ...
  for (auto& element : _salientPoints) {
    for (auto& salientPoint : element.second) {
      PRINT_CH_DEBUG("Behaviors",
                     "SalientPointsComponent.AddSalientPoints.PointInfo",
...
void SalientPointsComponent::GetSalientPointSinceTime(
  ...
  if (it != _salientPoints.end()) {
    ...
    std::copy_if(oldPoints.begin(), oldPoints.end(),
  ...
  PRINT_CH_DEBUG(
      "Behaviors",
      "SalientPointsComponent.GetSalientPointSinceTime.CopiedElements",
...
bool SalientPointsComponent::SalientPointDetected(
    const Vision::SalientPointType& type,
    const RobotTimeStamp_t timestamp) const {
  if (kRandomPersonDetection) {
    // TODO Here I'm cheating: test if x seconds have passed and if so create a
    // few fake salient points
    PRINT_NAMED_WARNING("SalientPointsComponent.SalientPointDetected.Cheating",
                        "");
    DEV_ASSERT(_robot != nullptr,
               "SalientPointsComponent.PersonDetected.RobotCantBeNull");
    ...
    if ((currentTickCount - _timeSinceLastObservation) > 8) {  // 8 seconds
      ...
      auto addPoint = [this,
                       &pointsToAdd](const Vision::SalientPointType& type) {
        ...
        {
          const float minX =
              std::max(float(salientPoint.x_img - rng->RandDbl()), 0.0f);
          const float maxX =
              std::min(float(salientPoint.x_img + rng->RandDbl()), 1.0f);
          const float minY =
              std::max(float(salientPoint.y_img - rng->RandDbl()), 0.0f);
          const float maxY =
              std::min(float(salientPoint.y_img + rng->RandDbl()), 1.0f);

          salientPoint.shape = {
              CladPoint2d(minX, minY), CladPoint2d(maxX, minY),
              CladPoint2d(minX, maxY), CladPoint2d(maxX, maxY)};
        }
