...
namespace {
static const int kObsSampleWindow_ms =
    300;  // sample all measurements over this period
static const f32 kObsSampleWindow_s =
    Anki::Util::MilliSecToSec((f32)kObsSampleWindow_ms);
static const int kNumRequiredSamples =
    static_cast<int>(kObsSampleWindow_s / 0.03f) -
    1;  // Require almost all samples to be valid
static const f32 kObsTriggerSensitivity =
    1.2;  // scaling factor for robot speed to object speed
static const f32 kObsMinObjectSpeed_mmps =
    50;  // prevents trigger if robot is stationary
static const f32 kObsMaxRotation_rad = DEG_TO_RAD(
    3);  // max rotation amount across all samples before considered turning
static const int kObsMaxObjectDistance_mm =
    100;  // don't respond if sensor reading is too far
}  // namespace
...
AIComponent::AIComponent()
    : UnreliableComponent<BCComponentID>(this, BCComponentID::AIComponent),
      IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::AIComponent),
      _suddenObstacleDetected(false) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
AIComponent::~AIComponent() { _aiComponents.reset(); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AIComponent::InitDependent(Vector::Robot* robot,
...

  {
    _aiComponents = std::make_unique<EntityType>();
    {
      const MicDirectionHistory& micDirectionHistory =
          dependentComps.GetComponent<MicComponent>().GetMicDirectionHistory();
      auto* faceSelectionComp = new FaceSelectionComponent(
          *robot, dependentComps.GetComponent<FaceWorld>(), micDirectionHistory,
          dependentComps.GetComponent<VisionComponent>());

      _aiComponents->AddDependentComponent(AIComponentID::AlexaComponent,
                                           new AlexaComponent(*robot));
      _aiComponents->AddDependentComponent(AIComponentID::BehaviorComponent,
                                           new BehaviorComponent());
      _aiComponents->AddDependentComponent(AIComponentID::ContinuityComponent,
                                           new ContinuityComponent(*robot));
      _aiComponents->AddDependentComponent(AIComponentID::FaceSelection,
                                           faceSelectionComp);
      _aiComponents->AddDependentComponent(
          AIComponentID::ObjectInteractionInfoCache,
          new ObjectInteractionInfoCache(*robot));
      _aiComponents->AddDependentComponent(AIComponentID::Puzzle,
                                           new PuzzleComponent(*robot));
      _aiComponents->AddDependentComponent(
          AIComponentID::SalientPointsDetectorComponent,
          new SalientPointsComponent());
      _aiComponents->AddDependentComponent(AIComponentID::TimerUtility,
                                           new TimerUtility());
      _aiComponents->AddDependentComponent(AIComponentID::Whiteboard,
...
void AIComponent::CheckForSuddenObstacle(Robot& robot) {
  ...
  const RobotTimeStamp_t endTime =
      robot.GetLastMsgTimestamp() - kObsSampleWindow_ms;

  int n = 0;
  for (auto st = states.rbegin(); st != states.rend() && st->first > endTime;
       ++st) {
    ...
    if (proxData.foundObject) {
      ...
      avgRobotSpeed_mmps +=
          state.GetLeftWheelSpeed_mmps() + state.GetRightWheelSpeed_mmps();
      n++;
    }
  }
  ...
  RadiansMath::ComputeMinMaxMean(angleVec, meanAngle, distToMinAngle_rad,
                                 distToMaxAngle_rad);
  const f32 angleRange_rad = distToMaxAngle_rad - distToMinAngle_rad;

  // Get latest distance reading and assess validity
  const auto& latestProxData =
      robot.GetProxSensorComponent().GetLatestProxData();
  ...
  f32 avgObjectSpeed_mmps =
      (avgProxValue_mm - latestDistance_mm) / kObsSampleWindow_s;
  ...
  _suddenObstacleDetected =
      foundObject && (avgRobotSpeed_mmps >= 0.f) &&
  ...
  if (!wasObstacleDetected && _suddenObstacleDetected) {
    DASMSG(robot_obstacle_detected, "robot.obstacle_detected",
           "The robot has detected (with his prox sensor) that an obstacle has "
           "suddenly appeared in front of him");
    DASMSG_SET(i1, static_cast<int64_t>(avgProxValue_mm),
               "Average prox sensor value in the recent past (mm)");
    DASMSG_SET(i2, static_cast<int64_t>(avgObjectSpeed_mmps),
               "Average object speed in the recent past (mm/sec)");
    DASMSG_SET(i3, static_cast<int64_t>(avgRobotSpeed_mmps),
               "Average robot speed in the recent past (mm/sec)");
    DASMSG_SEND();
  }
