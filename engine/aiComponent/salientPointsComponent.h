...
class SalientPointsComponent
 ...
 public:
  ...
  void GetSalientPointSinceTime(std::list<Vision::SalientPoint>& salientPoints,
                                ...
                                const RobotTimeStamp_t timestamp = 0) const;

  // Returns true wheter a SalientPoint of a spefic type has been seen since a
  // specific timestamp
  bool SalientPointDetected(const Vision::SalientPointType& type,
                            const RobotTimeStamp_t timestamp = 0) const;

  // Adds a list of SalientPoint. All the points are stored according to their
  // type. If a specific list of points is already stored for a type, it will be
  // replaced.
 ...
 private:
  ...
  // a list of the latest salient points organized per type. The points are
  // replaced every time new ones of the same type arrive
  std::map<Vision::SalientPointType, std::list<Vision::SalientPoint>>
      _salientPoints;

  mutable float _timeSinceLastObservation =
      0;  // TODO this is temporary for testing
