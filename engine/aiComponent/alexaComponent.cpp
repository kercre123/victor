 ...
 * Created: Oct 16 2018
 *
 * Description: Component that manages Alexa interactions among anim, engine,
 *and app
 *
...
// This component also handles the VC for sign in and sign out to hide from any
// behaviors whether it was app or voice command that requested sign in/out,
// since app intents got cut.
const UserIntentTag kSignInIntent = USER_INTENT(amazon_signin);
const UserIntentTag kSignOutIntent = USER_INTENT(amazon_signout);

// If a request to sign in/out is not claimed in this many ticks, this component
// will perform the sign in/out. Note that voice command-initiated requests wait
// until the intent is cleared in the uic before signing in/out, so that
// BehaviorReactToVoiceCommand has time to finish
...
// this class assumes that Listening, Thinking, Speaking, and Error are 0,1,2,3
// when sending indices to animProcess and when getting a tag array from
// AnimationComponent, so it doesn't need to know about AlexaUXState. IMPORTANT:
// If you hit this, please also change AnimationComponent's _tagForAlexa*, which
// only use indices and not AlexaUXState's.
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaComponent::InitDependent(Robot* robot,
                                   const AICompMap& dependentComps) {
  // setup event handlers

  if (robot->HasGatewayInterface()) {
    auto* gi = robot->GetGatewayInterface();
    auto callback = std::bind(&AlexaComponent::HandleAppEvents, this,
                              std::placeholders::_1);
    _signalHandles.push_back(gi->Subscribe(
        external_interface::GatewayWrapperTag::kAlexaAuthStateRequest,
        callback));
    _signalHandles.push_back(gi->Subscribe(
        external_interface::GatewayWrapperTag::kAlexaOptInRequest, callback));
    _signalHandles.push_back(gi->Subscribe(
        external_interface::GatewayWrapperTag::kAppDisconnected, callback));
    _signalHandles.push_back(gi->Subscribe(
        external_interface::GatewayWrapperTag::kOnboardingStateRequest,
        callback));
  }
  auto* ri = robot->GetRobotMessageHandler();
  if (ri != nullptr) {
    auto callback = std::bind(&AlexaComponent::HandleAnimEvents, this,
                              std::placeholders::_1);
    _signalHandles.push_back(ri->Subscribe(
        RobotInterface::RobotToEngineTag::alexaAuthChanged, callback));
    _signalHandles.push_back(ri->Subscribe(
        RobotInterface::RobotToEngineTag::alexaUXChanged, callback));
  }
  ...
  _consoleFuncs.emplace_front("ForceAlexaOptOut", std::move(forceOptOut),
                              "Alexa", "");
  ...
  if (ctx != nullptr) {
    ...
    // if !_featureFlagEnabled, now would be a good time to tell anim that alexa
    // is disabled, just in case the feature flag was disabled, but animProcess
    // still has a file indicating that we used to be authenticated. However,
    // messaging to anim might not be init'd yet. Instead, SendAuthStateToApp
    // will check the feature flag and disable alexa if it some how ends up
    // initialized without the feature flag.
  }
#else
  ...
  _animTags = animComponent.SetAlexaUXResponseCallback(
      [this](unsigned int idx, bool playing) {
        const auto uxState = static_cast<AlexaUXState>(idx);
        if (playing) {
          // only set to wait if this animation was passed as a get-in. the same
          // anim may be used elsewhere
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaComponent::AdditionalUpdateAccessibleComponents(
    AICompIDSet& components) const {
...
void AlexaComponent::UpdateDependent(const AICompMap& dependentComps) {
  // no behavior handles the intents kSignInIntent or kSignOutIntent. We just
  // message the anim process of the request to sign in/out, and depending on
  // its state, it will either not do anything, or send a message back to the
  // engine switch stacks into a face screen-style Wait behavior for alexa
  // pairing
  if (_uic == nullptr) {
  ...
  for (auto& response : _uxResponseInfo) {
    if ((response.second.timeout_s > 0.0f) &&
        (currTime_s > response.second.timeout_s)) {
      response.second.SetWaiting(false);
    }
  }

  if ((_requestTimeout != 0) &&
      (_requestTimeout < BaseStationTimer::getInstance()->GetTickCount())) {
    // Clear app requests when the timer expires.
    // Clear VC requests when the intent is gone from the uic ==> this implies
    // it expired in the uic, or someone else handled the intent. We have to
    // wait at least as long as the uic timeout, because that timeout is
    // dynamically adjusted by BehaviorReactToVoiceCommand
    if (_request == Request::SignInApp) {
      SignIn();
    } else if (_request == Request::SignOutApp) {
      SignOut();
    } else if ((_request == Request::SignInVC) &&
               !_uic->IsUserIntentPending(kSignInIntent)) {
      SignIn();
    } else if ((_request == Request::SignOutVC) &&
               !_uic->IsUserIntentPending(kSignOutIntent)) {
      SignOut();
    }
  }
}
...
bool AlexaComponent::IsSignInPending() const {
  const bool pending =
      (_request == Request::SignInApp) || (_request == Request::SignInVC);
...
bool AlexaComponent::IsSignOutPending() const {
  const bool pending =
      (_request == Request::SignOutApp) || (_request == Request::SignOutVC);
  return pending && (_requestTimeout != 0);
}
...
void AlexaComponent::SetAlexaOption(bool optedIn) {
  ...
  _robot.SendMessage(
      RobotInterface::EngineToRobot(RobotInterface::SetAlexaUsage(optedIn)));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaComponent::HandleAppEvents(
  ...
  switch (msg.GetTag()) {
    case external_interface::GatewayWrapperTag::kAlexaAuthStateRequest: {
      // respond to the app with the latest and greatest auth state received
      // from anim
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaComponent::HandleAnimEvents(
    const AnkiEvent<RobotInterface::RobotToEngine>& event) {
  const auto& msg = event.GetData();
  if (msg.GetTag() == RobotInterface::RobotToEngineTag::alexaAuthChanged) {
    ...
    if ((newState == AlexaAuthState::Authorized) && _pendingAuthIsFromOptIn) {
      // the user opted in, and now we're authorized. Design calls for switching
      // the backpack button functionality to Alexa, so save that setting now.
      // Note that the app may be displaying a successful login screen that
      // contains a toggle for this setting. In case things occur out of order,
      // the app assumes that a successful authentication implies that the
      // setting is now Alexa. If the below code is removed, the app will need
      // to change as well
...
void AlexaComponent::ToggleButtonWakewordSetting(bool isAlexa) const {
  ...
  const auto newSetting = isAlexa
                              ? external_interface::BUTTON_WAKEWORD_ALEXA
                              : external_interface::BUTTON_WAKEWORD_HEY_VECTOR;
  const bool success = settings->SetRobotSetting(
      external_interface::RobotSetting::button_wakeword, newSetting,
      updateSettingsJdoc, ignoredDueToNoChange);
  ANKI_VERIFY(
      success || ignoredDueToNoChange,
...
void AlexaComponent::SendAuthStateToApp(bool isResponse) {
  ...
  if (_robot.HasGatewayInterface()) {
    ...
    if (isResponse) {
      ...
      const external_interface::AlexaAuthState authState =
          CladProtoTypeTranslator::ToProtoEnum(_authState);
      ...
      if (authState == external_interface::ALEXA_AUTH_WAITING_FOR_CODE) {
        ...
        ANKI_VERIFY(_authStateExtra.empty(),
                    ...
                    "Unexpected extra data '%s'", _authStateExtra.c_str());
      }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaComponent::SetAlexaUXResponses(
    const std::unordered_map<AlexaUXState, AlexaUXResponse>& responses) {
  ...
  msg.postAudioEvents = {{AECH::CreatePostAudioEvent(
  ...
  static_assert(AlexaUXStateNumEntries == 5,
                "New states need changing the size below (decremented by one)");
  ...
  for (const auto& response : responses) {
    ...
    if (!ANKI_VERIFY((idx < 4), "AlexaComponent.SetAlexaUXResponses.Invalid",
                     "Invalid state %hhu. Allowed values are Listening, "
    ...
    const bool hasAudio = (response.second.audioEvent !=
                           AudioMetaData::GameEvent::GenericEvent::Invalid);

    if (!hasAudio && !hasAnim) {
      LOG_WARNING(
          "AlexaComponent.SetAlexaUXResponses.Unnecessary",
          "You should just supply those states you want to have a response");
      continue;
    }

    msg.postAudioEvents[idx] = AECH::CreatePostAudioEvent(
        response.second.audioEvent, AudioMetaData::GameObjectType::Behavior, 0);

    _uxResponseInfo[response.first].hasAnim = hasAnim;
    // always send this tag when there is any response, even when there is no
    // anim, so animProc knows if there is a valid response
  ...
  PRINT_NAMED_WARNING("AlexaComponent.SetUXResponses",
                      "new response get-ins: %s",
                      msg.csvGetInAnimNames.c_str());
...
void AlexaComponent::HandleNewUXState(AlexaUXState state) {
  // TODO (VIC-11517): downgrade. for now this is useful in webots
  PRINT_NAMED_WARNING("AlexaComponent.HandleNewUXState", "State=%s",
  ...
  if (_uxState != state) {
    ...
    DASMSG(ux_state_msg, "alexa.ux_state_transition",
           "The alexa agent changed UX states");
    DASMSG_SET(s1, AlexaUXStateToString(state),
               "new UX state (AlexaUXState in alexaTypes.clad)");
    DASMSG_SET(s2, AlexaUXStateToString(_uxState),
               "old UX state (AlexaUXState in alexaTypes.clad)");
    DASMSG_SET(i1, ((int)(lastStateTime_s * 1000.0f)),
               "Time (in milliseconds) the last ux state was active");
    DASMSG_SEND();
  ...
  if ((_uxState == AlexaUXState::Idle) && (_uxState != state)) {
    // alexa exited from idle. the anim process is likely playing some getin
    // right now if we pushed one.
...
void AlexaComponent::AlexaUXResponseInfo::SetWaiting(bool waiting) {
  if (waiting) {
    ...
    // if the ux state change happens before anim has received our new anim
    // response, anim and engine will be out of sync. Just in case, make sure
    // this times out in a few seconds.
    timeout_s = kAnimTimeout_s +
                BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  } else {
...
void AlexaComponent::SetRequest(Request request) {
  if ((_request == Request::None) && (request != Request::None)) {
    _request = request;
    _requestTimeout =
        kRequestDelayTicks + BaseStationTimer::getInstance()->GetTickCount();
    ...
    LOG_WARNING("AlexaComponent.AddRequest.NotHandled",
                "Request of type %d dropped because %d is already pending or "
                "request is none",
...
void AlexaComponent::SendSignInDAS(UserIntentSource source) const {
  DASMSG(alexa_opt_in, "alexa.user_sign_in_attempt",
         "User attempted to sign in to alexa");
  DASMSG_SET(s1, UserIntentSourceToString(source),
             "source of the request (Voice or App)");
  DASMSG_SEND();
}
...
void AlexaComponent::SendSignOutDAS(UserIntentSource source) const {
  DASMSG(alexa_opt_in, "alexa.user_sign_out_attempt",
         "User attempted to sign out of alexa");
  DASMSG_SET(s1, UserIntentSourceToString(source),
             "source of the request (Voice or App)");
  DASMSG_SEND();
}
