...
CONSOLE_VAR(u32, kAdvanceTimerAndAnticSeconds,
            "TimerUtility.AdvanceTimerAndAnticSeconds", 60);
...
TimerUtility::TimerUtility()
    : IDependencyManagedComponent<AIComponentID>(this,
                                                 AIComponentID::TimerUtility) {
  if (sTimerUtility != nullptr) {
    PRINT_NAMED_WARNING("TimerUtility.Constructor.MultipleInstances",
                        "TimerUtility instance exists already");
  }
  sTimerUtility = this;
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
TimerUtility::~TimerUtility() { sTimerUtility = nullptr; }
...
TimerUtility::SharedHandle TimerUtility::StartTimer(int timerLength_s) {
  ANKI_VERIFY(_activeTimer == nullptr,
              ...
              "Current design says we don't overwrite timers - remove this "
              "verify if that changes");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TimerUtility::ClearTimer() { _activeTimer.reset(); }
