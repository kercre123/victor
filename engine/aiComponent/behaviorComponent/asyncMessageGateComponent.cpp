...
AsyncMessageGateComponent::AsyncMessageGateComponent(
    IExternalInterface* externalInterface, IGatewayInterface* gatewayInterface,
...
void AsyncMessageGateComponent::PrepareCache() {
  ...
  for (int idx = 0; idx < _cachedTracker->_robotToEngineEvents.size(); idx++) {
    auto eventTag =
        _cachedTracker->_robotToEngineEvents[idx].GetData().GetTag();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AsyncMessageGateComponent::GetEventsForBehavior(
    IBehavior* subscriber, std::vector<const GameToEngineEvent>& events) {
  if (!ANKI_VERIFY(
          events.empty(),
...
// - - - - - - - - - - - - - - - - - - - -
void AsyncMessageGateComponent::GetEventsForBehavior(
    IBehavior* subscriber, std::vector<const EngineToGameEvent>& events) {
  if (!ANKI_VERIFY(
          events.empty(),
...
// - - - - - - - - - - - - - - - - - - - -
void AsyncMessageGateComponent::GetEventsForBehavior(
    IBehavior* subscriber, std::vector<const RobotToEngineEvent>& events) {
  if (!ANKI_VERIFY(
          events.empty(),
...
// - - - - - - - - - - - - - - - - - - - -
void AsyncMessageGateComponent::GetEventsForBehavior(
    IBehavior* subscriber, std::vector<const AppToEngineEvent>& events) {
  if (!ANKI_VERIFY(
          events.empty(),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AsyncMessageGateComponent::SubscribeToTags(
    IBehavior* subscriber,
  ...
  for (auto& tag : tags) {
    ...
    if (tagIter == _gameToEngineSubscribers.end()) {
      ...
      _eventHandles.push_back(_externalInterface->Subscribe(
          tag, [this](const GameToEngineEvent& event) {
            std::lock_guard<std::mutex> lock(
                _activeTracker->_gameToEngineMutex);
...
void AsyncMessageGateComponent::SubscribeToTags(
  ...
  for (auto& tag : tags) {
    ...
    if (tagIter == _engineToGameSubscribers.end()) {
      ...
      _eventHandles.push_back(_externalInterface->Subscribe(
          tag, [this](const EngineToGameEvent& event) {
            std::lock_guard<std::mutex> lock(
                _activeTracker->_engineToGameMutex);
...
// - - - - - - - - - - - - - - - - - - - -
void AsyncMessageGateComponent::SubscribeToTags(
    IBehavior* subscriber, std::set<RobotInterface::RobotToEngineTag>&& tags) {
  ...
  for (auto& tag : tags) {
    ...
    if (tagIter == _robotToEngineSubscribers.end()) {
      ...
      _eventHandles.push_back(_robotInterface->Subscribe(
          tag, [this](const RobotToEngineEvent& event) {
            std::lock_guard<std::mutex> lock(
                _activeTracker->_robotToEngineMutex);
...
// - - - - - - - - - - - - - - - - - - - -
void AsyncMessageGateComponent::SubscribeToTags(
    IBehavior* subscriber, std::set<AppToEngineTag>&& tags) {
  ...
  for (auto& tag : tags) {
    ...
    if (tagIter == _appToEngineSubscribers.end()) {
      ...
      _eventHandles.push_back(_gatewayInterface->Subscribe(
          tag, [this](const AppToEngineEvent& event) {
            std::lock_guard<std::mutex> lock(_activeTracker->_appToEngineMutex);
