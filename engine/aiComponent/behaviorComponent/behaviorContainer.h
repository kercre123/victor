...
class BehaviorContainer : public IDependencyManagedComponent<BCComponentID>,
 ...
 public:
  ...
  virtual void GetInitDependencies(BCCompIDSet& dependencies) const override {
    ...
    // Init the onboardingMessageHandler before BC so the OnboardingCoordinator
  ...
  std::set<ICozmoBehaviorPtr> FindBehaviorsByClass(
      BehaviorClass behaviorClass) const;

  // Sometimes it's necessary to downcast to a behavior to a specific behavior
  // pointer, e.g. so an Activity can access its member functions. This function
  // will help with that and provide a few assert checks along the way. It sets
  // outPtr in arguments, and returns true if the cast is successful
  template <typename T>
  ...
  // Create a behavior from the given config. Config must specify a behavior ID
  // and class. If the behavior is successfully created and added to the
  // container, True is returned. Errors include malformed JSON or duplicate
  // behavior IDs
...
#if ANKI_DEV_CHEATS
  const BehaviorIDToBehaviorMap& GetBehaviorMap() const {
    return _idToBehaviorMap;
  }
#endif
...
template <typename T>
bool BehaviorContainer::FindBehaviorByIDAndDowncast(
    BehaviorID behaviorID, BehaviorClass requiredClass,
    std::shared_ptr<T>& outPtr) const {
  ...
  if (ANKI_VERIFY(behavior != nullptr,
      ...
      ANKI_VERIFY(
          behavior != nullptr && GetBehaviorClass(behavior) == requiredClass,
    ...
    if (ANKI_VERIFY(outPtr != nullptr,
                    "BehaviorContainer.FindBehaviorByIDAndDowncast.CastFailed",
                    "BehaviorID: %s requiredClass: %s",
                    BehaviorTypesWrapper::BehaviorIDToString(behaviorID),
                    GetClassString(requiredClass).c_str())) {
      return true;
