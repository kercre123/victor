...
class IBehaviorMessageSubscriber {
 public:
  virtual void SubscribeToTags(IBehavior* subscriber,
                               std::set<GameToEngineTag>&& tags) const = 0;
  virtual void SubscribeToTags(IBehavior* subscriber,
                               std::set<EngineToGameTag>&& tags) const = 0;
  virtual void SubscribeToTags(
      IBehavior* subscriber,
      std::set<RobotInterface::RobotToEngineTag>&& tags) const = 0;
  virtual void SubscribeToTags(IBehavior* subscriber,
                               std::set<AppToEngineTag>&& tags) const = 0;
};
