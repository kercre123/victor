...
class Delegator : private Util::noncopyable {
 public:
  Delegator(BehaviorSystemManager& bsm, ContinuityComponent& continuityComp);
  virtual ~Delegator(){};
...
class DelegationComponent : public IDependencyManagedComponent<BCComponentID>,
 ...
 public:
  ...
  // called when an action completes (with any status). Argument is the action
  // id
  void HandleActionComplete(u32 actionTag);
  ...
  // return nullptr (including if control was delegated to an action)
