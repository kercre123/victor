...
class BehaviorEventComponent
    : public IBehaviorMessageSubscriber,
 ...
 public:
  ...
  virtual void SubscribeToTags(IBehavior* subscriber,
                               std::set<GameToEngineTag>&& tags) const override;
  virtual void SubscribeToTags(IBehavior* subscriber,
                               std::set<EngineToGameTag>&& tags) const override;
  virtual void SubscribeToTags(
      IBehavior* subscriber,
      std::set<RobotInterface::RobotToEngineTag>&& tags) const override;
  virtual void SubscribeToTags(IBehavior* subscriber,
                               std::set<AppToEngineTag>&& tags) const override;

  const std::vector<const GameToEngineEvent>& GetGameToEngineEvents() const {
    return _gameToEngineEvents;
  }
  const std::vector<const EngineToGameEvent>& GetEngineToGameEvents() const {
    return _engineToGameEvents;
  }
  const std::vector<const RobotToEngineEvent>& GetRobotToEngineEvents() const {
    return _robotToEngineEvents;
  }
  const std::vector<const AppToEngineEvent>& GetAppToEngineEvents() const {
    return _appToEngineEvents;
  }

  const std::vector<ExternalInterface::RobotCompletedAction>&
  GetActionsCompletedThisTick() const {
    return _actionsCompletedThisTick;
  }
 ...
 private:
  struct SubscriberWrapper {
    SubscriberWrapper(IBehaviorMessageSubscriber& ref) : _ref(ref) {}
