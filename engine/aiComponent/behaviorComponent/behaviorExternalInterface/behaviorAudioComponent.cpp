 ...
 * Created: 11/02/2016
 *
 * Description: This Client provides behavior audio needs for updating Sparked
 *Behavior music state and round. When using first call ActivateSparkedMusic()
 *to activate audio client and when behavior is completed call
...
using Util::FullEnumToValueArrayChecker::
    IsSequentialArray;  // import IsSequentialArray to this namespace

namespace {

// const AudioMetaData::GameObjectType kMusicGameObject =
// AudioMetaData::GameObjectType::Default;

// Mapping from ActivityId to FreeplayMood for music states
// 'buildPyramid'    - when Cozmo is building a pyramid (manages its own music
// states) 'feeding'         - when Cozmo is being fed cubes by the user to
// refil his energy need 'hiking'          - when Cozmo is exploring its
// surroundings on his own 'needsSevereLowEnergy' - when Cozmo's needstate is
// severely low energy 'needsSevereLowRepair' - when Cozmo's needstate is
// severely low repair 'needsSevereLowPlayGetIn' - when Cozmo is transitioning
// into severely low play state 'playWithHumans'  - when Cozmo requests games to
// play with the player 'playAlone'       - when Cozmo does stuff in place on
// his own, showing off, playing with cubes, .. 'putDownDispatch' - when Cozmo
// is looking for stuff after returning to on treads 'singing'         - when
// Cozmo is singing (manages its own music states) 'socialize'       - when
// Cozmo wants to interact with faces and players, but without playing games
// 'nothingToDo'     - fallback when Cozmo can't do anything else
/**{const std::unordered_map<BehaviorID, Freeplay_Mood> freeplayStateMap
{
   BehaviorID::Activity_BuildPyramid,
AudioMetaData::SwitchState::Freeplay_Mood::Invalid }, {
BehaviorID::Activity_Feeding,
AudioMetaData::SwitchState::Freeplay_Mood::Nurture_Feeding }, {
BehaviorID::Activity_Hiking, AudioMetaData::SwitchState::Freeplay_Mood::Hiking
}, { BehaviorID::Activity_NeedsSevereLowEnergy,
AudioMetaData::SwitchState::Freeplay_Mood::Invalid }, {
BehaviorID::Activity_NeedsSevereLowRepair,
AudioMetaData::SwitchState::Freeplay_Mood::Invalid }, {
BehaviorID::Activity_NeedsSevereLowPlayGetIn,
AudioMetaData::SwitchState::Freeplay_Mood::Invalid }, {
BehaviorID::Activity_PlayWithHumans,
AudioMetaData::SwitchState::Freeplay_Mood::Neutral }, {
BehaviorID::Activity_PlayAlone,
AudioMetaData::SwitchState::Freeplay_Mood::Neutral }, {
BehaviorID::Activity_PutDownDispatch,
AudioMetaData::SwitchState::Freeplay_Mood::Neutral }, {
BehaviorID::Activity_Singing, AudioMetaData::SwitchState::Freeplay_Mood::Invalid
}, { BehaviorID::Activity_Socialize,
AudioMetaData::SwitchState::Freeplay_Mood::Neutral }, {
BehaviorID::Activity_NothingToDo,
AudioMetaData::SwitchState::Freeplay_Mood::Bored }
};**/
...
BehaviorID::Activity_SparksBuildPyramid}
};**/
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorAudioComponent::BehaviorAudioComponent(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAudioComponent::InitDependent(Vector::Robot* robot,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAudioComponent::Init(
    BehaviorExternalInterface& behaviorExternalInterface) {
  // Get the appropriate spark music state from unity
  /**auto externalInterface =
  behaviorExternalInterface.GetRobotExternalInterface().lock();
  if(externalInterface != nullptr){
    auto handleSetSparkMusicState = [this, &behaviorExternalInterface](const
  AnkiEvent<ExternalInterface::MessageGameToEngine>& musicState){
      _sparkedMusicState =
  musicState.GetData().Get_SetSparkedMusicState().sparkedMusicState;
      HandleSparkUpdates(behaviorExternalInterface, _lastUnlockIDReceived,
  _round);
    };
  ...
  auto handleStateChangeWrapper =
      [this, &behaviorExternalInterface](
          const AnkiEvent<RobotPublicState>& stateEvent) {
        HandleRobotPublicStateChange(behaviorExternalInterface,
                                     stateEvent.GetData());
      };

  if (behaviorExternalInterface.HasPublicStateBroadcaster()) {
    auto& publicStateBroadcaster =
        behaviorExternalInterface.GetRobotPublicStateBroadcaster();
    _eventHandles.push_back(
        publicStateBroadcaster.Subscribe(handleStateChangeWrapper));
  }

  if (ANKI_DEV_CHEATS) {
    /**
    const auto nonSparkFreeplayActivities =
    robot.GetBehaviorManager().GetNonSparkFreeplayActivities(); for(const auto&
    activity : nonSparkFreeplayActivities)
    {
      const BehaviorID activityID = activity->GetID();

      DEV_ASSERT_MSG(freeplayStateMap.find(activityID) !=
    freeplayStateMap.end(),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAudioComponent::UpdateActivityMusicState(
  ...
  //                  "No freeplay music for ActivityName '%s'",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAudioComponent::HandleRobotPublicStateChange(
    BehaviorExternalInterface& behaviorExternalInterface,
  ...
  if (currActivity != _prevActivity) {
    ...
    const BehaviorStageStruct& currPublicStateStruct =
  ...
  BehaviorStageTag currentStageTag =
  stateEvent.userFacingBehaviorStageStruct.behaviorStageTag; if(currentStageTag
  != GetActiveBehaviorStage()){ SetActiveBehaviorStage(currentStageTag);
  }

  _lastUnlockIDReceived = stateEvent.currentSpark;
  // Ensure behavior stages have been cleared/re-set appropriately if the
  activity
  // has changed
  if(ANKI_DEV_CHEATS){
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAudioComponent::HandleWorldEventUpdates(
    const RobotPublicState& stateEvent) {
  ...
  //
  //    if(_isCubeInLift){
  //      _robot.GetRobotAudioClient()->PostEvent(AE::Play__Cue_World_Event__Lift_Cube,
  //      kMusicGameObject);
  //    }else{
  //      _robot.GetRobotAudioClient()->PostEvent(AE::Stop__Cue_World_Event__Lift_Cube,
  //      kMusicGameObject);
  //    }
  //  }
  //
  ...
  //      _robot.GetRobotAudioClient()->PostEvent(AE::Play__Cue_World_Event__Request_Game,
  //      kMusicGameObject);
  //    }else{
  //      _robot.GetRobotAudioClient()->PostEvent(AE::Stop__Cue_World_Event__Request_Game,
  //      kMusicGameObject);
  //    }
  //  }
  //
  ...
  //
  //    if(_stackExists){
  //      _robot.GetRobotAudioClient()->PostEvent(AE::Play__Cue_World_Event__Cubes_Stacked,
  //      kMusicGameObject);
  //    }else{
  //      _robot.GetRobotAudioClient()->PostEvent(AE::Stop__Cue_World_Event__Cubes_Stacked,
  //      kMusicGameObject);
  //    }
  //  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAudioComponent::HandleGuardDogUpdates(
    const BehaviorStageStruct& currPublicStateStruct) {
  ...
  //      _robot.GetRobotAudioClient()->PostSwitchState(SwitchGroupType::Freeplay_Mood,
  //                                                    static_cast<const
  //                                                    GenericSwitch>(Freeplay_Mood::Guard_Dog),
  //                                                    AudioMetaData::GameObjectType::Default);
  //    }
  //
  ...
  //      const auto round =
  //      Util::EnumToUnderlying(currPublicStateStruct.currentGuardDogStage);
  ...
  //      _robot.GetRobotAudioClient()->PostSwitchState(SwitchGroupType::Gameplay_Round,
  //                                                    static_cast<const
  //                                                    GenericSwitch>(kGameplayRoundMap[round]),
  //                                                    AudioMetaData::GameObjectType::Default);
  //
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAudioComponent::HandleDancingUpdates(
    const BehaviorStageStruct& currPublicStateStruct) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAudioComponent::HandleFeedingUpdates(
    const BehaviorStageStruct& currPublicStateStruct) {
  //  const bool stageOrRoundChanged = (GetActiveBehaviorStage() !=
  //  BehaviorStageTag::Feeding) ||
  //                                     (_prevFeedingStage !=
  //                                     currPublicStateStruct.currentFeedingStage);
  //
  ...
  //    _robot.GetRobotAudioClient()->PostSwitchState(SwitchGroupType::Gameplay_Round,
  //                                                  static_cast<const
  //                                                  GenericSwitch>(kGameplayRoundMap[_round]),
  //                                                  AudioMetaData::GameObjectType::Default);
  //  }
}
...
void BehaviorAudioComponent::SetActiveBehaviorStage(BehaviorStageTag stageTag) {
  DEV_ASSERT_MSG(
      (stageTag == BehaviorStageTag::Count) ||
          (_activeBehaviorStage == BehaviorStageTag::Count),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAudioComponent::HandleDimMusicForActivity(
    const RobotPublicState& stateEvent) {
  //  using AE = AudioMetaData::GameEvent::GenericEvent;
