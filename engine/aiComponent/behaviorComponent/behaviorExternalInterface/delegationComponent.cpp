...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool Delegator::Delegate(IBehavior* delegatingBehavior, IActionRunner* action) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void DelegationComponent::InitDependent(Robot* robot,
                                        const BCCompMap& dependentComps) {
  auto* bsm = dependentComps.GetComponentPtr<BehaviorSystemManager>();
  Init(bsm, dependentComps.GetComponent<AIComponent>()
                .GetComponentPtr<ContinuityComponent>());
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void DelegationComponent::Init(BehaviorSystemManager* bsm,
                               ContinuityComponent* continuityComp) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool DelegationComponent::IsControlDelegated(
    const IBehavior* delegatingBehavior) {
...
void DelegationComponent::CancelActionIfRunning(IBehavior* delegatingBehavior) {
  if (_delegator->_behaviorThatDelegatedAction != nullptr &&
    ...
    if (_delegator->_behaviorThatDelegatedAction == delegatingBehavior ||
        delegatingBehavior == nullptr) {
      _delegator->_behaviorThatDelegatedAction =
          nullptr;  // TEMP:  // TODO:(bn) redundant checks now
      _continuityComp->GetOutOfAction(tagToCancel);
    }
    // note that the callback, if there was one (and it was allowed to run),
    // should have already been called at this point, so it's safe to clear the
    // tag. Also, if the cancel itself failed, that is probably a bug, but
    // somehow the action is gone, so no sense keeping the tag around (and it
    // clearly isn't running). If the callback called DelegateIfInControl, we
    // may have a new action tag, so only clear this if the cancel didn't change
    // it
...
bool DelegationComponent::HasDelegator(IBehavior* delegatingBehavior) {
  return _bsm->CanDelegate(delegatingBehavior) &&
         !IsControlDelegated(delegatingBehavior);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
const IBehavior* DelegationComponent::GetBehaviorDelegatedTo(
