...
class BehaviorStack : private Util::noncopyable {
 public:
  ...
  // Currently only used for development output (e.g. audio). Don't use this
  // internally for anything important
  ...
  inline IBehavior* GetTopOfStack() {
    return _behaviorStack.empty() ? nullptr : _behaviorStack.back();
  }
  inline const IBehavior* GetTopOfStack() const {
    return _behaviorStack.empty() ? nullptr : _behaviorStack.back();
  }
  inline IBehavior* GetBottomOfStack() const {
    return _behaviorStack.empty() ? nullptr : _behaviorStack.front();
  }
  inline bool IsInStack(const IBehavior* behavior) {
    return _stackMetadataMap.find(behavior) != _stackMetadataMap.end();
  }

  // if the passed in behavior is in the stack, return a pointer to the behavior
  // which is above it in the stack, or null if it is at the top or not in the
  // stack
  ...
  Json::Value BuildDebugBehaviorTree(
 ...
 private:
  struct StackMetadataEntry {
    ...
    static void RecursivelyGatherLinkedBehaviors(
  ...
  // calls all appropriate functions to prep the delegates of something about to
  // be added to the stack
  void PrepareDelegatesToEnterScope(IBehavior* delegated);

  // calls all appropriate functions to prepare a delegate to be removed from
  // the stack
