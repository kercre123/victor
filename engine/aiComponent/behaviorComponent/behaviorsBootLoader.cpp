 ...
 * Created: 2018 jun 2
 *
 * Description: Component that decides what behavior stack to initialize upon
 *engine loading. This is based on factory state and dev options, but primarily
 *is based on onboarding state. Since onboarding only occurs once in a robot's
 *lifetime, this component "reboots" the behavior stack when onboarding ends,
 *instead of having one giant stack containing both onboarding and regular
 *behaviors.
 *
...
BehaviorsBootLoader::BehaviorsBootLoader(const Json::Value& config)
  ...
  if (ANKI_VERIFY(!config.empty(), "BehaviorsBootLoader.Ctor.InvalidConfig",
                  "Empty config")) {
    auto setFromJSON = [&config](const char* key) {
      const std::string behaviorIDStr =
          JsonTools::ParseString(config, key, "BehaviorsBootLoader");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorsBootLoader::BehaviorsBootLoader(IBehavior* overrideBehavior,
                                         const UnitTestKey& key)
    : IDependencyManagedComponent(this, BCComponentID::BehaviorsBootLoader) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorsBootLoader::InitDependent(Robot* robot,
                                        const BCCompMap& dependentComps) {
  _robot = robot;

  using namespace Util;
  const Data::DataPlatform* platform = robot->GetContextDataPlatform();
  _saveFolder = platform->pathToResource(
      Data::Scope::Persistent,
  ...
  const bool inPRDemo = robot->GetContext()->GetFeatureGate()->IsFeatureEnabled(
      FeatureType::PRDemo);

  // If this is the factory test forcibly set _bootBehavior as playpen as long
  // as the robot has not been through packout
  bool startInPlaypen = false;
#if FACTORY_TEST
  {
    startInPlaypen = !Factory::GetEMR()->fields.PACKED_OUT_FLAG &&
                     !OSState::getInstance()->IsInRecoveryMode();
  }
#endif
  if (startInPlaypen) {
  ...
  if (msgHandler != nullptr) {
    auto startSelfTestLambda =
        [this, robot](const AnkiEvent<RobotInterface::RobotToEngine>& event) {
          // When the self test starts, save the current boot behavior as well
          // as the bottom of the behavior stack in order to restore them when
          // the self test ends Note: The bottom of the stack should be the wait
          // behavior due to having to go through the customer care screen to
          // start the self test
          _prevBootBehavior = GetBootBehavior();

          auto& bsm = robot->GetAIComponent()
                          .GetComponent<BehaviorComponent>()
                          .GetComponent<BehaviorSystemManager>();
          _prevBottomOfStackBehavior =
              const_cast<IBehavior*>(bsm.GetBaseBehavior());

          // Switch to the self test behavior
          SetNewBehavior(_behaviors.selfTestBehavior);
        };

    _eventHandles.push_back(msgHandler->Subscribe(
        RobotInterface::RobotToEngineTag::startSelfTest, startSelfTestLambda));
  }

  auto* ei = _externalInterface;
  if (ei != nullptr) {
    // listen for messages from the robot that changed onboarding state. don't
    // worry about anything from the app, since it can't tell the robot to
    // change its onboarding state
    auto onOnboardingStage =
        [this](const AnkiEvent<ExternalInterface::MessageEngineToGame>&
                   onboardingStageEvt) {
          const auto& msg = onboardingStageEvt.GetData().Get_OnboardingState();
          const auto newState = msg.stage;
          // save the stage so the app knows it, but only change the stack if
          // requested, so that, if desired, onboarding can remain in app
          // onboarding (Complete). Even if not desired, set the _bootBehavior
          // without changing the behavior stack so that entering and leaving CC
          // screen then pulls the new behavior
          _stage = newState;
          const bool requestStackReset = !msg.forceSkipStackReset;
          if (static_cast<u8>(newState) <
              static_cast<u8>(OnboardingStages::Complete)) {
            RestartOnboarding();
    ...
    _eventHandles.push_back(ei->Subscribe(
        ExternalInterface::MessageEngineToGameTag::OnboardingState,
        onOnboardingStage));

    auto selfTestEndLambda =
        [this](const AnkiEvent<ExternalInterface::MessageEngineToGame>&
                   selfTestEnd) { _selfTestEnded = true; };

    _eventHandles.push_back(
        ei->Subscribe(ExternalInterface::MessageEngineToGameTag::SelfTestEnd,
                      selfTestEndLambda));
    ...
    auto setDevDoNothingFunc = [this](ConsoleFunctionContextRef context) {
      ...
      SetNewBehavior(_behaviors.devBaseBehavior, requestStackReset);
      Json::Value toSave;
      toSave[BehaviorOnboardingCoordinator::kOnboardingStageKey] =
          OnboardingStagesToString(_stage);
      const std::string filename =
          _saveFolder + BehaviorOnboardingCoordinator::kOnboardingFilename;
...
#if ALEXA_ACOUSTIC_TEST
    const bool enterAcousticTestMode =
        (_bootBehaviorID != _behaviors.onboardingBehavior);
#else
    const bool enterAcousticTestMode = kAcousticTestMode;
    if (kAcousticTestMode &&
        (_bootBehaviorID != _behaviors.acousticTestBehavior)) {
...
void BehaviorsBootLoader::InitOnboarding() {
  ...
  {
    // always start in DevDoNothing stage on mac if no persistent file exists,
    // so that webots work can continue with a simple console var
    // DevDispatchAfterShake. We can't use Complete since that would interfere
    // with webots tests
    _stage = OnboardingStages::DevDoNothing;
  }
#endif
  ...
  if (!fileContents.empty() &&
      reader.parse(fileContents, onboardingStateJSON)) {
    if (ANKI_VERIFY(onboardingStateJSON
                        [BehaviorOnboardingCoordinator::kOnboardingStageKey]
                            .isString(),
                    "BehaviorsBootLoader.InitOnboarding.InvalidKey", "")) {
      const auto& stageStr =
          onboardingStateJSON
              [BehaviorOnboardingCoordinator::kOnboardingStageKey]
                  .asString();
  ...
  PRINT_CH_INFO("Behaviors",
                "BehaviorsBootLoader.InitOnboarding.OnboardingStage",
                "Robot booted with onboarding state %s",
                OnboardingStagesToString(_stage));
...
IBehavior* BehaviorsBootLoader::GetBootBehavior() {
  ANKI_VERIFY(_bootBehavior != nullptr,
              "BehaviorsBootLoader.GetBootBehavior.BootNotInitialized",
              "Requested behavior before initialization");
...
void BehaviorsBootLoader::SetNewBehavior(BehaviorID behaviorID,
                                         bool requestStackReset) {
  _bootBehaviorID = behaviorID;
  DEV_ASSERT(_behaviorContainer != nullptr,
             "BehaviorsBootLoader.SetNewBehavior.NoBC");

  IBehavior* behavior = _behaviorContainer->FindBehaviorByID(behaviorID).get();
  ANKI_VERIFY(behavior != nullptr, "BehaviorsBootLoader.SetNewBehavior.Invalid",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorsBootLoader::SetNewBehavior(IBehavior* behavior,
                                         bool requestStackReset) {
  if (behavior != nullptr) {
    if ((behavior != _bootBehavior) || _pendingBehavior) {
      ...
      // If not resetting the stack, flag so that it's possible to call
      // SetNewBehavior() again with the same behaviorID when it's time to
      // actually switch behaviors. Otherwise this if block won't run
...
void BehaviorsBootLoader::RestartOnboarding() {
  // hacky way of de-activating the current onboarding and then re-activating
  // it. Flag to start the Wait behavior, when it starts change the stage within
  // BehaviorOnboarding, pause a few ticks, then flag to start onboarding again.
  SetNewBehavior(BEHAVIOR_ID(Wait));
  _countUntilResetOnboarding = 20;

  DASMSG(onboarding_restart, "onboarding.restart",
         "User requested to start onboarding from the beginning");
  DASMSG_SEND();
}
