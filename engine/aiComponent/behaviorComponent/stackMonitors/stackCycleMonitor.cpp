 ...
 * Created: 2018-06 26
 *
 * Description: Monitors the stack for cycles, which usually indicate that the
 *robot is looking stupid. For now, this is only looking for _rapid_ cycles,
 *which usually indicates a bug, but eventually we may consider longer loops.
 *
...
void StackCycleMonitor::NotifyOfChange(BehaviorExternalInterface& bei,
  ...
  // We could be clever here in detecting cycles or common subsequences. For
  // now, this is simple. Clear the history whenever a behavior last for a few
  // ticks, and if not, search for AAAAA, ABABA, or variations of these in the
  // history. The pattern AAAAA implies that behavior A delegates to B, but
  // behavior B cancels self or doesn't delegate (and behaviorAlwaysDelegates).
  // If kMaxTicks>1, patterns like AABAABA match as well
  if (currTick - _lastTick <= kMaxTicks) {
    if (CheckForCycle()) {
      std::string behaviorA = (_recentBehaviors[0] != nullptr)
                                  ? _recentBehaviors[0]->GetDebugLabel()
                                  : "null";
      std::string behaviorB = (_recentBehaviors[1] != nullptr)
                                  ? _recentBehaviors[1]->GetDebugLabel()
                                  : "null";
      // intentional spam. deal with it. this will also become a das event in
      // dev
      if (behaviorA == behaviorB) {
        LOG_ERROR("StackCycleMonitor.NotifyOfChange.CycleDetected",
                  "A cycle was detected. %s is repeatedly delegating to "
                  "something that ends immediately",
                  behaviorA.c_str());
      } else {
        LOG_ERROR("StackCycleMonitor.NotifyOfChange.CycleDetected",
                  "A cycle was detected between %s and %s", behaviorA.c_str(),
                  behaviorB.c_str());
      }
      // in prod, send a das event only once per robot boot
      SendDASEvent(behaviorA, behaviorB);

      auto& bbl = bei.GetAIComponent()
                      .GetComponent<BehaviorComponent>()
                      .GetComponent<BehaviorsBootLoader>();
...
bool StackCycleMonitor::CheckForCycle() const {
  ...
  if (_recentBehaviors.size() == _recentBehaviors.capacity()) {
    ...
    patternMatches = true;
    for (int i = 2; patternMatches && (i < _recentBehaviors.size() - 1);
         i += 2) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void StackCycleMonitor::SwitchToSafeStack(BehaviorExternalInterface& bei,
                                          IBehavior* newBaseBehavior) const {
  auto& BC = bei.GetBehaviorContainer();

  std::shared_ptr<BehaviorResetState> resetBehavior;
  if (BC.FindBehaviorByIDAndDowncast(
          kBehaviorIDForReset, BEHAVIOR_CLASS(ResetState), resetBehavior)) {
    auto* castPtr = dynamic_cast<ICozmoBehavior*>(newBaseBehavior);
    if (ANKI_VERIFY(castPtr != nullptr,
                    "StackCycleMonitor.SwitchToSafeStack.Invalid",
                    "Could not cast")) {
      const BehaviorID id = castPtr->GetID();
      resetBehavior->SetFollowupBehaviorID(id);

      auto& bsm = bei.GetAIComponent()
                      .GetComponent<BehaviorComponent>()
                      .GetComponent<BehaviorSystemManager>();
      const bool waitUntilNextTick = true;
      bsm.ResetBehaviorStack(resetBehavior.get(), waitUntilNextTick);
      // note that even if resetBehavior is already the base of the stack, it
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void StackCycleMonitor::SendDASEvent(const std::string& behaviorA,
                                     const std::string& behaviorB) {
  ...
  if (it == _dasMsgsSent.end()) {
    ...
    DASMSG(behavior_cycle_detected, "behavior.cycle_detected",
           "A cycle was detected. Only one msg is sent per behavior pair per "
           "boot");
