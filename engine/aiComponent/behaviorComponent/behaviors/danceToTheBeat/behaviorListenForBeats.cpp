 ...
 * Created: 2018-08-20
 *
 * Description: Listens for beats from the BeatDetectorComponent, playing
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorListenForBeats::InstanceConfig::InstanceConfig(
  ...
  JsonTools::GetCladEnumFromJSON(config, kPreListeningAnim_key,
                                 preListeningAnim, debugName);
  JsonTools::GetCladEnumFromJSON(config, kListeningAnim_key, listeningAnim,
                                 debugName);
  JsonTools::GetCladEnumFromJSON(config, kPostListeningAnim_key,
                                 postListeningAnim, debugName);
  JsonTools::GetCladEnumFromJSON(config, kNoBeatAnim_key, noBeatAnim,
                                 debugName);

  minListeningTime_sec =
      JsonTools::ParseFloat(config, kMinListeningTime_key, debugName);
  maxListeningTime_sec =
      JsonTools::ParseFloat(config, kMaxListeningTime_key, debugName);

  cancelSelfIfBeatLost =
      JsonTools::ParseBool(config, kCancelSelfIfBeatLost_key, debugName);

  DEV_ASSERT(minListeningTime_sec < maxListeningTime_sec,
             "BehaviorListenForBeats.InstanceConfig.InvalidMinMaxTimes");
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorListenForBeats::WantsToBeActivatedBehavior() const { return true; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorListenForBeats::GetBehaviorOperationModifiers(
    BehaviorOperationModifiers& modifiers) const {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorListenForBeats::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorListenForBeats::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kPreListeningAnim_key,    kListeningAnim_key,
                        kPostListeningAnim_key,   kNoBeatAnim_key,
                        kMinListeningTime_key,    kMaxListeningTime_key,
...
void BehaviorListenForBeats::BehaviorUpdate() {
  ...
  // Since we are now listening for beats, reset the listen for beats cooldown
  // timer
  GetBEI()
      .GetBehaviorTimerManager()
      .GetTimer(BehaviorTimerTypes::ListenForBeatsCooldown)
      .Reset();
  ...
  if (listeningTime_sec > _iConfig.maxListeningTime_sec) {
    PRINT_NAMED_WARNING(
        "BehaviorListenForBeats.BehaviorUpdate.Timeout",
  ...
  } else if (!beatDetector.IsPossibleBeatDetected() &&
             _iConfig.cancelSelfIfBeatLost) {
    PRINT_NAMED_WARNING("BehaviorListenForBeats.BehaviorUpdate.NoMoreBeat",
                        "Cancelling since beat detector says there is no "
                        "possible beat detected");
    exitWithAnim = _iConfig.noBeatAnim;
  }
...
void BehaviorListenForBeats::TransitionToListening() {
  _dVars.listeningStartTime_sec =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  DelegateIfInControl(
      new ReselectingLoopAnimationAction(_iConfig.listeningAnim));
}
