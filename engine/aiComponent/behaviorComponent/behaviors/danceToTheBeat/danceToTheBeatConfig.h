...
// DanceAnimMetadata stores information about dance animations. Each dance
// animation is short, and meant to last only a single beat. The beatDelay_sec
// indicates when in the animation the musical beat should land, and is
// determined by an event keyframe in the animation itself.
class DanceAnimMetadata {
 public:
  ...
  // Is it safe to listen for new tempos/beat detection while this animation is
  // playing?
  bool CanListenForBeats() const { return _canListenForBeats; }
  void SetCanListenForBeats(const bool b) { _canListenForBeats = b; }

  // Should a getout be played after this animation if dancing is interrupted
  ...
  void SetPlayGetoutIfInterrupted(const bool b) {
    _playGetoutIfInterrupted = b;
  }

  float GetBeatDelay_sec() const { return _beatDelay_sec; }

  // Populates beatDelay_sec with the time into the given animation where the
  // beat should land. For example, a value of 0.100 would mean that the musical
  // beat should fall 100 ms into the animation. Returns true if we successfully
...
// A DancePhrase is made up of one or more possible dance animations that can be
// strung together and played on sequential musical beats. DancePhraseConfig
// specifies the rules by which dance phrases are generated when the behavior is
// run.
//
// When the behavior begins, animations are randomly drawn from the list in
// accordance with the min/max beats. The number of animations that make up the
// phrase is random, but is always between 'minBeats' and 'maxBeats', and is
...
// A DanceSession is made up of one or more DancePhrases. It corresponds to a
// full, choreographed dance 'event'.
class DanceSession {
 public:
  // Construct from json
