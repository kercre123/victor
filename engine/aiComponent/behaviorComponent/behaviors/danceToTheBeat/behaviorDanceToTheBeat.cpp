 ...
 * Created: 2018-05-07
 *
 * Description: Dance to a beat detected by the microphones/beat detection
 *algorithm
 *
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorDanceToTheBeat::InstanceConfig::InstanceConfig(
    const Json::Value& config, const std::string& debugName)
    : useBackpackLights(
          JsonTools::ParseBool(config, kUseBackpackLights_key, debugName)) {
  eyeHoldAnim = AnimationTrigger::Count;
  getOutAnim = AnimationTrigger::Count;
  JsonTools::GetCladEnumFromJSON(config, kBackpackAnim_key, backpackAnim,
                                 debugName);
  JsonTools::GetCladEnumFromJSON(config, kEyeHoldAnim_key, eyeHoldAnim,
                                 debugName);
  JsonTools::GetCladEnumFromJSON(config, kGetOutAnim_key, getOutAnim,
                                 debugName);

  const auto& danceSessionConfig = config[kDanceSessions_key];
  DEV_ASSERT(
      !danceSessionConfig.isNull() && danceSessionConfig.isArray() &&
          !danceSessionConfig.empty(),
  ...
  for (const auto& json : danceSessionConfig) {
    ...
    DEV_ASSERT(
        danceSessionConfigs.back().IsValid(),
        "BehaviorDanceToTheBeat.InstanceConfig.InvalidDanceSessionConfig");
  }
}
...
bool BehaviorDanceToTheBeat::InstanceConfig::CheckValid() {
  ...
  if (danceSessionConfigs.empty()) {
    isValid = false;
    PRINT_NAMED_WARNING(
        "BehaviorDanceToTheBeat.InstanceConfig.CheckValid.NoDanceSessions",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDanceToTheBeat::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {
      kUseBackpackLights_key, kBackpackAnim_key,  kEyeHoldAnim_key,
      kGetOutAnim_key,        kDanceSessions_key,
  };
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorDanceToTheBeat::WantsToBeActivatedBehavior() const { return true; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDanceToTheBeat::InitBehavior() {
  // Initialize "dance sessions"
  auto& dataAccessorComp =
      GetBEI()
          .GetComponentWrapper(BEIComponentID::DataAccessor)
  ...
  for (auto& session : _iConfig.danceSessionConfigs) {
    ...
    DEV_ASSERT(
        success,
        "BehaviorDanceToTheBeat.InitBehavior.FailedInitializingDanceSession");
  }

  // Double check that the config is still valid
  ANKI_VERIFY(_iConfig.CheckValid(),
              "BehaviorDanceToTheBeat.InitBehavior.InvalidConfig", "");
}
...
void BehaviorDanceToTheBeat::OnBehaviorActivated() {
  if (!_iConfig.isValid) {
    PRINT_NAMED_WARNING(
        "BehaviorDanceToTheBeat.OnBehaviorActivated.InvalidConfig",
  ...
  // Pre-generate the queue of all animations to play by asking each dance
  // session to generate a list of animations
  for (const auto& sessionConfig : _iConfig.danceSessionConfigs) {
    const auto& animSequence = sessionConfig.GenerateAnimSequence();
    _dVars.danceAnims.insert(_dVars.danceAnims.end(), animSequence.begin(),
                             animSequence.end());
  }
  ...
  DASMSG_SET(i1, beatDetector.GetCurrTempo_bpm(),
             "Current tempo estimate (beats per minute)");
  DASMSG_SET(i2, 1000.f * beatDetector.GetCurrConfidence(),
             "Current detection confidence (unitless). This is the raw "
             "confidence value multiplied by 1000");
  DASMSG_SET(i3, (int)_dVars.danceAnims.size(),
             "Number of dance animations that have been queued to play");
  DASMSG_SEND();
...
void BehaviorDanceToTheBeat::BehaviorUpdate() {
  ...
  // If we have no animation queued and we're not currently doing anything, it's
  // time to quit.
...
void BehaviorDanceToTheBeat::OnBehaviorDeactivated() {
  ...
  DASMSG_SET(i2, 1000.f * beatDetector.GetCurrConfidence(),
             ...
             "confidence value multiplied by 1000");
  DASMSG_SET(i3, (int)_dVars.danceAnims.size(),
             "Number of unplayed dance animations remaining in the queue");
  DASMSG_SEND();
...
void BehaviorDanceToTheBeat::TransitionToDancing() {
  ...
  // Double check that a beat is still detected. If no beat is detected, do not
  // queue a dance animation. This will cause the behavior to cancel itself in
  // Update(). Note: We cannot cancel the behavior here since unit tests
  // disallow cancelling a behavior on the same tick it was activated.
  if (!beatDetector.IsBeatDetected()) {
    PRINT_NAMED_WARNING(
        "BehaviorDanceToTheBeat.TransitionToDancing.NoBeat",
  ...
  // grab the next beat time here and wait to play the anim until the
  // appropriate time
...
void BehaviorDanceToTheBeat::WhileDancing() {
  ...
  const auto& beatDetector = GetBEI().GetBeatDetectorComponent();
  if (_dVars.listeningForBeats && !beatDetector.IsBeatDetected()) {
    PRINT_NAMED_WARNING(
        "BehaviorDanceToTheBeat.WhileDancing.BeatNoLongerDetected",
        "Cancelling behavior since we are currently listening for beats "
        "and BeatDetectorComponent says beat is no longer detected (play "
        "getout anim: %d)",
        _dVars.playGetoutIfBeatLost);
    DASMSG(dttb_cancel_beat_lost, "dttb.cancel_beat_lost",
           "DanceToTheBeat cancelling because beat is lost");
    DASMSG_SET(i1, beatDetector.GetCurrTempo_bpm(),
               "Current tempo estimate (beats per minute)");
    DASMSG_SET(i2, 1000.f * beatDetector.GetCurrConfidence(),
               "Current detection confidence (unitless). This is the raw "
    ...
    if (_dVars.playGetoutIfBeatLost) {
      DelegateNow(new TriggerAnimationAction(_iConfig.getOutAnim),
                  [this]() { CancelSelf(); });
    } else {
  ...
  const auto now_sec =
      static_cast<float>(Util::Time::UniversalTime::GetCurrentTimeInSeconds());
  // If we're not listening for beats, we need to call OnBeat() 'manually' when
  // the next 'extrapolated' beat occurs.
...
void BehaviorDanceToTheBeat::PlayNextDanceAnim() {
  ...
  DEV_ASSERT(!_dVars.danceAnims.empty(),
             "BehaviorDanceToTheBeat.PlayNextDanceAnim.NoDanceSessions");

  SetListeningForBeats(_dVars.danceAnims.front().CanListenForBeats());
  _dVars.playGetoutIfBeatLost =
      _dVars.danceAnims.front().PlayGetoutIfInterrupted();
  auto* animAction =
      new PlayAnimationAction(_dVars.danceAnims.front().GetAnimName());
  ...
  if (_dVars.danceAnims.empty()) {
    ...
    DelegateNow(animAction, [this]() {
      DelegateIfInControl(
          new ReselectingLoopAnimationAction(_iConfig.eyeHoldAnim));
    });
    SetNextAnimTriggerTime();
  }

  // Since we've played a dancing animation, reset the dancing cooldown behavior
  // timer
  GetBEI()
      .GetBehaviorTimerManager()
      .GetTimer(BehaviorTimerTypes::DancingCooldown)
      .Reset();
}
...
void BehaviorDanceToTheBeat::SetNextAnimTriggerTime() {
  if (_dVars.danceAnims.empty()) {
    _dVars.nextAnimTriggerTime_sec = -1.f;
    DEV_ASSERT(false,
               "BehaviorDanceToTheBeat.SetNextAnimTriggerTime.NoAnimsInQueue");
    return;
  }
  ...
  _dVars.nextAnimTriggerTime_sec = _dVars.nextBeatTime_sec -
                                   nextAnim.GetBeatDelay_sec() -
                                   kLatencyCorrectionTime_sec;
...
void BehaviorDanceToTheBeat::OnBeat() {
  ...
  PRINT_CH_INFO("Behaviors", "BehaviorDanceToTheBeat.OnBeat",
                "OnBeat() called. Currently%s listening for new beats. Current "
                "universal time (sec): %.3f",
  ...
  // If we're currently listening for beats, then this means we have new
  // information from the BeatDetectorComponent.
  if (_dVars.listeningForBeats) {
    // Update some stuff now that we have new beat information from
    // BeatDetectorComponent. Only update if there is still a beat currently
    // detected (to avoid invalid values for tempo and nextBeatTime). If the
    // beat has stopped, WhileDancing() will take care of cancelling the
    // behavior.
    const auto& beatDetector = GetBEI().GetBeatDetectorComponent();
    if (beatDetector.IsBeatDetected()) {
      _dVars.nextBeatTime_sec = beatDetector.GetNextBeatTime_sec();
    ...
    // Update the next animation trigger time if we still have animations in the
    // queue.
    if (!_dVars.danceAnims.empty()) {
      SetNextAnimTriggerTime();
    }
  } else {
    // We are not currently listening for beats, so just manually extrapolate
    // the next beat time.
...
void BehaviorDanceToTheBeat::SetListeningForBeats(const bool listenForBeats) {
  ...
  PRINT_CH_INFO("Behaviors", "BehaviorDanceToTheBeat.SetListeningForBeats.Set",
                "%d", listenForBeats);
  ...
  if (listenForBeats) {
    if (_dVars.onBeatCallbackId < 0) {
      _dVars.onBeatCallbackId = beatDetector.RegisterOnBeatCallback(
          std::bind(&BehaviorDanceToTheBeat::OnBeat, this));
    }
...
void BehaviorDanceToTheBeat::UnregisterOnBeatCallback() {
  if (_dVars.onBeatCallbackId > 0) {
    ...
    if (!beatDetector.UnregisterOnBeatCallback(_dVars.onBeatCallbackId)) {
      PRINT_NAMED_ERROR(
          "BehaviorDanceToTheBeat.UnregisterOnBeatCallback."
          "FailedToUnregisterCallback",
          "Failed to unregister OnBeat callback (id %d)",
          _dVars.onBeatCallbackId);
    }
  }
