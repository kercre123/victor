...
// #include
// "engine/aiComponent/behaviorComponent/behaviors/appBehaviors/behaviorEyeColor.h"
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorEyeColor::GetBehaviorJsonKeys(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorEyeColor::GetBehaviorOperationModifiers(
...
bool BehaviorEyeColor::WantsToBeActivatedBehavior() const {
  return GetBEI().GetSettingsManager().IsSettingsUpdateRequestPending(
...
void BehaviorEyeColor::OnBehaviorActivated() {
  ...
  GetBEI().GetSettingsManager().ClaimPendingSettingsUpdate(
      external_interface::RobotSetting::eye_color);

  CompoundActionSequential* animationSequence = new CompoundActionSequential();
  animationSequence->AddAction(
      new TriggerLiftSafeAnimationAction(AnimationTrigger::EyeColorGetIn),
      true);
  animationSequence->AddAction(
      new TriggerLiftSafeAnimationAction(AnimationTrigger::EyeColorSwitch),
      true);
  animationSequence->AddAction(
      new TriggerLiftSafeAnimationAction(AnimationTrigger::EyeColorGetOut),
      true);
...
void BehaviorEyeColor::OnBehaviorDeactivated() {
  ...
  if (settings.IsSettingsUpdateRequestPending(
          external_interface::RobotSetting::eye_color)) {
    settings.ApplyPendingSettingsUpdate(
        external_interface::RobotSetting::eye_color, true);
  }
}
