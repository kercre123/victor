...
bool BehaviorEyeColorVoiceCommand::WantsToBeActivatedBehavior() const {
  ...
  const bool eyecolorPending =
      uic.IsUserIntentPending(USER_INTENT(imperative_eyecolor));
  const bool eyecolorSpecificPending =
      uic.IsUserIntentPending(USER_INTENT(imperative_eyecolor_specific));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorEyeColorVoiceCommand::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorEyeColorVoiceCommand::GetAllDelegates(
    std::set<IBehavior*>& delegates) const {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorEyeColorVoiceCommand::GetBehaviorJsonKeys(
...
void BehaviorEyeColorVoiceCommand::OnBehaviorActivated() {
  ...
  if (uic.IsUserIntentPending(USER_INTENT(imperative_eyecolor_specific))) {
    ...
    intentData =
        SmartActivateUserIntent(USER_INTENT(imperative_eyecolor_specific));
    ...
    // in this case the user has non-specifically requested a change--cycle or
    // random
    ...
    LOG_WARNING(
        "BehaviorEyeColorVoiceCommand.OnBehaviorActivated."
        "NoRecognizedPendingIntent",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
external_interface::EyeColor
BehaviorEyeColorVoiceCommand::GetDesiredColorFromIntent(
    UserIntentPtr intentData) {
  const std::string colorRequest =
      intentData->intent.Get_imperative_eyecolor_specific().eye_color;
  ...
  if (it != kEyeColorMap.end()) {
    ...
    LOG_DEBUG("BehaviorEyeColorVoiceCommand.GetDesiredColorFromIntent",
              "mapped eye color request %s to desiredEyeColor %u",
              colorRequest.c_str(), desiredEyeColor);
  } else {
    ...
    desiredEyeColor = static_cast<external_interface::EyeColor>(
        settings.GetRobotSettingAsUInt(
...
external_interface::EyeColor BehaviorEyeColorVoiceCommand::ChooseColorCyclic() {
  ...
  SettingsManager& settings = GetBEI().GetSettingsManager();
  external_interface::EyeColor newEyeColor =
      static_cast<external_interface::EyeColor>(
          settings.GetRobotSettingAsUInt(
              external_interface::RobotSetting::eye_color) +
          1);
  // roll over if needed
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorEyeColorVoiceCommand::SetEyeColor(
    external_interface::EyeColor desiredEyeColor) {
  SettingsManager& settings = GetBEI().GetSettingsManager();
  bool ignoredDueToNoChange;
  const bool success =
      settings.SetRobotSetting(external_interface::RobotSetting::eye_color,
                               desiredEyeColor, true, ignoredDueToNoChange);
  if (success) {
    LOG_INFO("BehaviorEyeColorVoiceCommand.SetEyeColor.Success",
             "Successfully changed eye color to %u", desiredEyeColor);
  } else {
    LOG_INFO("BehaviorEyeColorVoiceCommand.SetEyeColor.Failure",
             "Failed to change eye color.");
  }
  if (ignoredDueToNoChange) {
    ...
    // (usually it's triggered in the eyeColor reaction behavior, but in this
    // case it won't be)
    CompoundActionSequential* animationSequence =
