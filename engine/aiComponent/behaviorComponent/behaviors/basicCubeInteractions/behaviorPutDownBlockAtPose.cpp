...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPutDownBlockAtPose::SetPutDownAnimTrigger(
...
BehaviorPutDownBlockAtPose::DynamicVariables::DynamicVariables()
    : state(PutDownCubeState::DriveToDestinationPose), persistent() {}
...
bool BehaviorPutDownBlockAtPose::WantsToBeActivatedBehavior() const {
  return GetBEI().GetRobotInfo().GetCarryingComponent().IsCarryingObject() ||
         IsControlDelegated();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPutDownBlockAtPose::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPutDownBlockAtPose::GetBehaviorJsonKeys(
...
void BehaviorPutDownBlockAtPose::OnBehaviorDeactivated() {
  // Clear out the persistent settings, they should be reset before activation
  // if needed
...
void BehaviorPutDownBlockAtPose::TransitionToDriveToDestinationPose() {
  ...
  if (_dVars.persistent.customDestPending) {
    DriveToPoseAction* driveToPoseAction =
        new DriveToPoseAction(_dVars.persistent.destinationPose);
    DelegateIfInControl(driveToPoseAction,
                        &BehaviorPutDownBlockAtPose::TransitionToPutDownBlock);
  } else {
...
void BehaviorPutDownBlockAtPose::TransitionToPutDownBlock() {
  ...
  DelegateIfInControl(
      new TriggerAnimationAction(_dVars.persistent.putDownAnimTrigger),
      [this](ActionResult result) {
        const auto& robotInfo = GetBEI().GetRobotInfo();
        if (robotInfo.GetCarryingComponent().IsCarryingObject()) {
          LOG_INFO("BehaviorPlaceCubeByCharger.ObjectStillHeld",
                   "Robot still thinks its carrying an object after the anim "
                   "finished");
...
IActionRunner* BehaviorPutDownBlockAtPose::CreateLookAfterPlaceAction() {
  ...
  CompoundActionParallel* parallel = new CompoundActionParallel(
      {new MoveHeadToAngleAction(DEG_TO_RAD(kBackupVerifyHeadAngle_deg)),
       new DriveStraightAction(kBackupVerifyDist_mm)});

  action->AddAction(parallel);
  action->AddAction(
      new WaitForImagesAction(kBackupVerifyNumFrames, VisionMode::Markers));
