...
void BehaviorPutDownBlock::OnBehaviorActivated() {
  ...
  const float backupDistance = GetRNG().RandDblInRange(
      kBPDB_kBackupDistanceMin_mm, kBPDB_kBackupDistanceMax_mm);
...
void BehaviorPutDownBlock::LookDownAtBlock() {
  DelegateIfInControl(
      CreateLookAfterPlaceAction(GetBEI().GetRobotInfo().GetCarryingComponent(),
                                 true),
      [this]() {
        ...
        if (robotInfo.GetCarryingComponent().IsCarryingObject()) {
          ...
          // TODO: We should really be using some kind of PlaceOnGroundAction
          // instead of raw animation (see COZMO-2192)
          PRINT_NAMED_WARNING(
              "BehaviorPutDownBlock.LookDownAtBlock.DidNotSeeBlock",
              "Forcibly setting carried objects as unattached (See "
              "COZMO-2192)");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
IActionRunner* BehaviorPutDownBlock::CreateLookAfterPlaceAction(
    CarryingComponent& carryingComponent, bool doLookAtFaceAfter) {
  ...
  if (carryingComponent.IsCarryingObject()) {
    ...
    CompoundActionParallel* parallel = new CompoundActionParallel(
        {new MoveHeadToAngleAction(DEG_TO_RAD(kBPDB_finalHeadAngle_deg)),
  ...
  if (doLookAtFaceAfter) {
    // in any case, look back at the last face after this is done (to give them
    // a chance to show another cube)
    const bool sayName = false;
    action->AddAction(new TurnTowardsFaceWrapperAction(
        new TriggerAnimationAction(AnimationTrigger::PutDownBlockKeepAlive),
