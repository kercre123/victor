 ...
 * Created: 2016-04-05
 *
 * Description: This behavior rolls blocks when it see's them facing a direction
 *specified in config
 *
...
BehaviorRollBlock::BehaviorRollBlock(const Json::Value& config)
  ...
  _iConfig.isBlockRotationImportant =
      config.get(kIsBlockRotationImportant, true).asBool();
  if (config.isMember(kRetryCountKey)) {
    const std::string debugStr =
        "BehaviorRollBlock.Constructor.RetryCountParseIssue";
    _iConfig.rollRetryCount =
        JsonTools::ParseUint8(config, kRetryCountKey, debugStr);
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorRollBlock::GetBehaviorJsonKeys(
...
void BehaviorRollBlock::OnBehaviorActivated() {
  ...
  const ObservableObject* object =
      GetBEI().GetBlockWorld().GetLocatedObjectByID(_dVars.targetID);
  if (object != nullptr) {
    UpdateTargetsUpAxis();
...
void BehaviorRollBlock::BehaviorUpdate() {
  ...
  if (object != nullptr && _dVars.behaviorState == State::RollingBlock) {
    const AxisName currentUpAxis =
        object->GetPose().GetRotationMatrix().GetRotatedParentAxis<'Z'>();
    if (currentUpAxis != _dVars.upAxisOnBehaviorStart) {
      ...
      if (ComputeDistanceSQBetween(GetBEI().GetRobotInfo().GetPose(),
          ...
          (distBetween >
           (kMaxDistCozmoIsRollingCube_mm * kMaxDistCozmoIsRollingCube_mm))) {
        UpdateTargetsUpAxis();
...
void BehaviorRollBlock::TransitionToPerformingAction() {
  ...
  if (!_dVars.targetID.IsSet()) {
    PRINT_NAMED_WARNING(
        "BehaviorRollBlock.NoBlockID",
        "%s: Transitioning to action state, but we don't have a valid block ID",
        GetDebugLabel().c_str());
    return;
  }

  DriveToRollObjectAction* rollAction =
      new DriveToRollObjectAction(_dVars.targetID);
  rollAction->SetPreDockCallback(
      [this](Robot& robot) { _dVars.didAttemptDock = true; });

  DelegateIfInControl(rollAction, [this](ActionResult result) {
    if (result == ActionResult::SUCCESS) {
      TransitionToRollSuccess();
    } else if ((IActionRunner::GetActionResultCategory(result) ==
      ...
      const ObservableObject* pickupObj =
          blockWorld.GetLocatedObjectByID(_dVars.targetID);
      if (pickupObj != nullptr) {
        auto& whiteboard = GetAIComp<AIWhiteboard>();
        whiteboard.SetFailedToUse(
            *pickupObj, AIWhiteboard::ObjectActionFailure::RollOrPopAWheelie);
      }
    }
  });
}
...
void BehaviorRollBlock::TransitionToRollSuccess() {
  ...
  // The mood manager listens for actions to succeed to modify mood, but we may
  // have just canceled the action, so manually send the mood event here
  ...
  if (!ShouldStreamline()) {
    DelegateIfInControl(
        new TriggerAnimationAction(AnimationTrigger::RollBlockSuccess));
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorRollBlock::SetState_internal(State state,
