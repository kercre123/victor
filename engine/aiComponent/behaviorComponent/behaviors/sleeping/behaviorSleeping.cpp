...
CONSOLE_VAR_RANGED(f32, kSleepingStirSpacing_min_s, CONSOLE_GROUP, 2700.0f,
                   0.0f, 7200.0f);
CONSOLE_VAR_RANGED(f32, kSleepingStirSpacing_max_s, CONSOLE_GROUP, 5400.0f,
                   0.0f, 7200.0f);

CONSOLE_VAR_RANGED(f32, kSleepingBoutSpacing_min_s, CONSOLE_GROUP, 1.5f, 0.0f,
                   30.0f);
CONSOLE_VAR_RANGED(f32, kSleepingBoutSpacing_max_s, CONSOLE_GROUP, 5.0f, 0.0f,
                   7200.0f);

// note: the anim group includes a very subtle animation and a more noticeable
// one, so these numbers should be a bit higher than they might otherwise be
...
BehaviorSleeping::BehaviorSleeping(const Json::Value& config)
  ...
  _iConfig.shouldEnterPowerSave =
      config.get(kEnablePowerSaveKey, true).asBool();
  _iConfig.shouldPlayEmergencyGetOut =
      config.get(kPlayEmergencyGetOut, true).asBool();
  _iConfig.canActivateOffTreads =
      config.get(kCanActivateOffTreads, false).asBool();
}

void BehaviorSleeping::GetBehaviorOperationModifiers(
    BehaviorOperationModifiers& modifiers) const {
  modifiers.wantsToBeActivatedWhenOffTreads = _iConfig.canActivateOffTreads;
}

void BehaviorSleeping::GetBehaviorJsonKeys(
...
void BehaviorSleeping::OnBehaviorActivated() {
  ...
  // Each time we go to sleep, re-load the album of named faces. This will clear
  // out any session-only faces which the robot "forgets" between awake
  // "sessions", and keeps the number of faces he knows about in check (and not
  // growing without bound) In addition, clear all faces in FaceWorld.
  ...
  // always start with one round of sleeping to make sure the face is in a good
  // state
  DelegateIfInControl(
      new TriggerAnimationAction(AnimationTrigger::GoToSleepSleeping),
...
void BehaviorSleeping::TransitionToSleeping() {
  ...
  _dVars.numRemainingInBout = GetRNG().RandIntInRange(
      kSleepingBoutNumStirs_min, kSleepingBoutNumStirs_max);

  const float waitTime_s = GetRNG().RandDblInRange(kSleepingStirSpacing_min_s,
...
void BehaviorSleeping::TransitionToBoutOfStirring() {
  ...
  if (_dVars.numRemainingInBout-- >= 0) {
    ...
    const float waitTime_s = GetRNG().RandDblInRange(
        kSleepingBoutSpacing_min_s, kSleepingBoutSpacing_max_s);
...
void BehaviorSleeping::HoldFaceForTime(const float waitTime_s,
