...
CONSOLE_VAR(f32, kSleepCycle_DeepSleep_PersonCheckInterval_s, CONSOLE_GROUP,
            4 * 60.0f * 60.0f);
CONSOLE_VAR(f32, kSleepCycle_LightSleep_PersonCheckInterval_s, CONSOLE_GROUP,
            1 * 60.0f * 60.0f);
...
// The amount of time that the robot must be on the charger but not actually
// charging because of overheating battery before he is forced to go to sleep.
CONSOLE_VAR(f32, kSleepCycle_TooLongOnChargerNotChargingDuration_sec,
            CONSOLE_GROUP, 5 * 60.f);
...
BehaviorSleepCycle::BehaviorSleepCycle(const Json::Value& config)
  ...
  _iConfig.awakeDelegateName =
      JsonTools::ParseString(config, kBehaviorIDConfigKey, debugStr);
  _iConfig.findChargerBehaviorName =
      JsonTools::ParseString(config, kFindChargerBehaviorKey, debugStr);
  ...
  _iConfig.emergencyCondition = BEIConditionFactory::CreateBEICondition(
      config[kEmergencyConditionKey], GetDebugLabel());
}
...
void BehaviorSleepCycle::ParseWakeReasonConditions(const Json::Value& config) {
  for (const auto& group : config[kWakeReasonConditionsKey]) {
    ...
    JsonTools::GetCladEnumFromJSON(
        group, "reason", reason,
        "BehaviorSleepCycle.ParseWakeReasonConditions");
    auto condition = BEIConditionFactory::CreateBEICondition(group["condition"],
                                                             GetDebugLabel());
    if (ANKI_VERIFY(condition != nullptr,
                    "BehaviorSleepCycle.ParseWakeReasonConditions."
                    "FailedToCreateCondition",
...
void BehaviorSleepCycle::CreateCustomWakeReasonConditions() {
  _iConfig.wakeConditions.emplace(
      WakeReason::SDK,
      new ConditionLambda(
          [this](BehaviorExternalInterface& bei) {
            ...
            const bool notEmergency =
                (_iConfig.emergencyCondition == nullptr) ||
                !_iConfig.emergencyCondition->AreConditionsMet(bei);
            const bool offCharger = !robotInfo.IsOnChargerPlatform();
            // don't let SDK take control if the robot is on the charger during
            // an emergency
  ...
  _iConfig.wakeConditions.emplace(
      WakeReason::VoiceCommand,
      new ConditionLambda(
          [this](BehaviorExternalInterface& bei) {
            const bool isIntentPending =
                GetBehaviorComp<UserIntentComponent>().IsAnyUserIntentPending();
  ...
  auto addSleepReactionWakeCondition = [this](WakeReason reason,
                                              SleepReactionType type) {
    _iConfig.wakeConditions.emplace(
        reason,
        new ConditionLambda(
            [this, type](BehaviorExternalInterface& bei) {
              const bool isDoneReacting =
                  (_dVars.reactionState == type && !IsControlDelegated());
  ...
  addSleepReactionWakeCondition(WakeReason::PickedUpFromPalm,
                                SleepReactionType::PickedUpFromPalm);
}
...
void BehaviorSleepCycle::CheckWakeReasonConfig() {
  ...
  for (uint8_t reasonIdx = 1; reasonIdx < WakeReasonNumEntries; ++reasonIdx) {
    ...
    if (reason != WakeReason::SawPerson && reason != WakeReason::NotSleepy &&
        reason != WakeReason::Interrupted) {
      // skip special cases, everything else must exist

      ANKI_VERIFY(
          _iConfig.wakeConditions.find(reason) != _iConfig.wakeConditions.end(),
          "BehaviorSleepCycle.WakeReasonCondition.Missing",
          "Missing wake condition for reason '%s'", WakeReasonToString(reason));
    }
  }
}
...
void BehaviorSleepCycle::ParseWakeReasons(const Json::Value& config) {
  for (const auto& reasonJson : config[kAlwaysWakeReasonsKey]) {
    WakeReason reason = WakeReason::Invalid;
    if (JsonTools::ParseCladEnumFromJSON(
            reasonJson, reason, "BehaviorSleepCycle.ParseWakeReasons")) {
  ...
  for (const auto& stateGroup : config[kWakeFromStatesKey]) {
    ...
    JsonTools::GetCladEnumFromJSON(
        stateGroup, "fromState", fromState,
        "BehaviorSleepCycle.ParseWakeStates.FromState");
    ...
    LOG_DEBUG("BehaviorSleepCycle.ParseWakeReasons.WakeReasonsFromState",
              ...
              wakeReasons.size(), SleepStateIDToString(fromState));
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorSleepCycle::WantsToBeActivatedBehavior() const { return true; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorSleepCycle::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorSleepCycle::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorSleepCycle::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kBehaviorIDConfigKey,    kWakeReasonConditionsKey,
                        kAlwaysWakeReasonsKey,   kWakeFromStatesKey,
                        kFindChargerBehaviorKey, kEmergencyConditionKey};
...
void BehaviorSleepCycle::InitBehavior() {
...

  {
    BehaviorID delegateID =
        BehaviorTypesWrapper::BehaviorIDFromString(_iConfig.awakeDelegateName);
    _iConfig.awakeDelegateName.clear();
    _iConfig.awakeDelegate = BC.FindBehaviorByID(delegateID);
  }

  {
    BehaviorID delegateID = BehaviorTypesWrapper::BehaviorIDFromString(
        _iConfig.findChargerBehaviorName);
  ...
  _iConfig.sleepingSoundReactionBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(ReactToSoundAsleep));
  _iConfig.sleepingWakeWordBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(SleepingTriggerWord));
  _iConfig.wiggleBackOntoChargerBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(WiggleBackOntoChargerFromPlatform));
  _iConfig.emergencyModeAnimBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(EmergencyModeAnimDispatcher));
  _iConfig.personCheckBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(SleepingPersonCheck));
  _iConfig.joltInPalmBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(ReactToJoltInPalm));
  _iConfig.pickupFromPalmReaction =
      BC.FindBehaviorByID(BEHAVIOR_ID(ReactToPickupFromPalm));
  ...
  if (context != nullptr) {
    ...
    if (webService != nullptr) {
      auto onWebVizSubscribed =
          [this](const std::function<void(const Json::Value&)>& sendToClient) {
      ...
      _eventHandles.emplace_back(
          webService->OnWebVizSubscribed(kWebVizModuleNameSleeping)
              .ScopedSubscribe(onWebVizSubscribed));
    }
  }
}
...
void BehaviorSleepCycle::OnBehaviorActivated() {
  ...
  LOG_INFO("BehaviorSleepCycle.Activated", "Starting out %s",
...
void BehaviorSleepCycle::OnBehaviorDeactivated() {
  ...
  // if we were asleep, and being canceled by something above, play emergency
  // get out (it is disabled in the sleeping behavior itself)
    ...
    case SleepStateID::LightSleep: {
      ...
      if (context) {
        if (auto webSender = WebService::WebVizSender::CreateWebVizSender(
                kWebVizModuleNameSleeping, context->GetWebService())) {
...
void BehaviorSleepCycle::BehaviorUpdate() {
  ...
  if (sForcePersonCheck) {
    LOG_WARNING("BehaviorSleepCycle.ConsoleFunc.ForcingPersonCheck",
                "forcing a person check from state '%s' (if we are in light or "
                "deep sleep)",
  ...
  if (_dVars.currState == SleepStateID::Invalid) {
    LOG_ERROR("BehaviorSleepCycle.Update.InvalidState", "sleep state invalid");
    return;
  }
  ...
  if (_dVars.reactionState == SleepReactionType::TriggerWord) {
    ...
    if (IsControlDelegated()) {
      ...
      if (_dVars.currState == SleepStateID::EmergencySleep) {
        // show "charger face" or "too hot face" animation to explain to the
        // user why the voice command failed
        if (_iConfig.emergencyModeAnimBehavior->WantsToBeActivated()) {
          if (DelegateIfInControl(_iConfig.emergencyModeAnimBehavior.get(),
                                  clearReactionState)) {
            return;
          }
        }
        LOG_WARNING(
            "BehaviorSleepCycle.BehaviorUpdate.CantPlayEmergencyAnim",
        ...
        // not an emergency, so we're done with the reaction now, wake reason
        // will wake us if needed (e.g. an intent is set)
        clearReactionState();
      }
    }
  }
  ...
  if (_dVars.currState == SleepStateID::Awake) {
    ...
    // not awake from any of the global conditions, check any conditions based
    // on the current sleep state
    ...
    if (_dVars.currState == SleepStateID::EmergencySleep) {
      if (!_iConfig.emergencyCondition->AreConditionsMet(GetBEI())) {
        // no longer in emergency sleep. If a voice command was activated for
        // sleeping (before we went into emergency mode) go to deep sleep,
        // otherwise select the right sleep automatically
    ...
    if (_dVars.currState == SleepStateID::Comatose) {
      const bool timeToTransition =
          currTime_s >
          _dVars.comatoseStartTime_s + kSleepCycle_ComatoseLength_s;
      if (timeToTransition) {
        // We only get into Comatose from the sleeping voice command, so always
    ...
    if (_dVars.currState == SleepStateID::LightSleep ||
      ...
      if (_dVars.nextPersonCheckTime_s > 0.0f &&
        ...
        LOG_INFO(
            "BehaviorSleepCycle.TimeForPersonCheck",
            "In state '%s', reached t=%f (>= %f), time to do a person check",
            SleepStateIDToString(_dVars.currState), currTime_s,
    ...
    if (ShouldWiggleOntoChargerFromSleep()) {
      ...
      DelegateIfInControl(_iConfig.wiggleBackOntoChargerBehavior.get(),
                          [this]() {
                            // even if this fails, clear the variable so we
                            // don't keep trying
    ...
    if (!IsControlDelegated()) {
      // in a sleeping state but not sleeping, re-start the behavior here (this
      // may happen, e.g. if the wake word behavior completed without a valid
      // intent)
      SleepIfInControl();
    }
  }
...
bool BehaviorSleepCycle::ShouldWiggleOntoChargerFromSleep() {
  ...
  if (_dVars.currState == SleepStateID::GoingToCharger) {
    // never do a wiggle while we're going to the charger (the go home behavior
    // handles that internally)
    return false;
  }

  const bool notReacting =
      (_dVars.reactionState == SleepReactionType::NotReacting);
  const bool onContacts = GetBEI().GetRobotInfo().IsOnChargerContacts();

  return notReacting && _dVars.wasOnChargerContacts && !onContacts &&
...
bool BehaviorSleepCycle::GoToSleepIfNeeded() {
  ...
  const bool isSleepIntentPending =
      uic.IsUserIntentPending(USER_INTENT(system_sleep));
  ...
  const bool isGoodnightIntentPending =
      uic.IsUserIntentPending(USER_INTENT(greeting_goodnight));
  ...
  const float currTime_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  const bool wokeRecently =
      _dVars.lastWakeUpTime_s > 0.0f &&
      (currTime_s - _dVars.lastWakeUpTime_s) <= kSleepCycle_RecentSleepLength_s;

  // if "react to dark" has finished (or some other behavior that makes the
  // robot want to sleep), it will offer a post behavior suggestion. Check that
  // here
  ...
  if (!wokeRecently &&
      checkSuggestion(PostBehaviorSuggestions::SleepOnCharger)) {
    TransitionToCharger();
  ...
  const bool lowStim =
      GetBEI().GetMoodManager().GetSimpleMood() == SimpleMoodType::LowStim;
  const ActiveFeature currActiveFeature =
      GetBehaviorComp<ActiveFeatureComponent>().GetActiveFeature();
  const bool isObserving =
      (currActiveFeature == ActiveFeature::Observing) ||
      (currActiveFeature == ActiveFeature::ObservingOnCharger);

  if (isSleepy && lowStim && isObserving && !wokeRecently) {
    TransitionToCharger();
    SendGoToSleepDasEvent(SleepReason::Sleepy);
    return true;
  }

  // Go to sleep if on charger for a certain amount of time while the battery is
  // disconnected since this means the battery is overheated and needs to go to
  // sleep in order to cooldown. But only do this during ObservingOnCharger.
  const auto& battComp = GetBEI().GetRobotInfo().GetBatteryComponent();
  const bool isCharging = battComp.IsCharging();
  const float durationDisconnected_sec =
      battComp.GetBatteryDisconnectedDurationSec();
  if (isCharging &&
      durationDisconnected_sec >
...
bool BehaviorSleepCycle::WakeIfNeeded(const WakeReason& forReason) {
  ...
  if (ANKI_VERIFY(conditionIt != _iConfig.wakeConditions.end() &&
    ...
    if (conditionIt->second->AreConditionsMet(GetBEI())) {
      ...
      const bool playAnim =
          ShouldPlayWakeUpAnimForReason(forReason, defaultVal);
...
void BehaviorSleepCycle::SendGoToSleepDasEvent(const SleepReason& reason) {
  ...
  DASMSG(goto_sleep, "behavior.sleeping.falling_asleep",
         "The robot is preparing to fall asleep");
  DASMSG_SET(s1, SleepReasonToString(reason),
             "The reason the robot is falling asleep");
  DASMSG_SET(s2, SleepStateIDToString(_dVars.currState),
             "The state of sleeping we are going to");
  DASMSG_SEND();
...
void BehaviorSleepCycle::WakeUp(const WakeReason& reason, bool playWakeUp) {
  LOG_INFO("BehaviorSleepCycle.WakeUp",
           ...
           SleepStateIDToString(_dVars.currState), WakeReasonToString(reason));

  DASMSG(wakeup, "behavior.sleeping.wake_up", "Robot is waking up from sleep");
  DASMSG_SET(s1, WakeReasonToString(reason),
             "The reason for waking (see SleepingTypes.clad)");
  DASMSG_SET(s2, SleepStateIDToString(_dVars.currState),
             "The state of sleeping we were in before waking");
  DASMSG_SEND();

  // never play wake up if we were in the middle of a reaction (we're already
  // visibly "awake" in that case)
  const bool wasPlayingReaction =
      _dVars.reactionState != SleepReactionType::NotReacting;
  ...
  if (!wasPlayingReaction && playWakeUp &&
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorSleepCycle::SetConditionsActiveForState(SleepStateID state,
                                                     bool active) {
...
void BehaviorSleepCycle::TransitionToCheckingForPerson() {
  ...
  // sleep states will set the next time appropriately, for now disable the next
  // check
  _dVars.nextPersonCheckTime_s = -1.0f;

  // note: for now this will cause a wake up animation.... might be nice to do a
  // different animation or not wake up here
  CancelDelegates(false);

  _dVars.personCheckStartTimestamp =
      GetBEI().GetRobotInfo().GetLastImageTimeStamp();

  // Mute audio and lock the lift so that the person check is as quiet as
  // possible
  MuteForPersonCheck(true);

  DelegateIfInControl(
      new TriggerAnimationAction(AnimationTrigger::WakeupGetout), [this]() {
        if (_iConfig.personCheckBehavior->WantsToBeActivated()) {
          DelegateIfInControl(_iConfig.personCheckBehavior.get(),
                              &BehaviorSleepCycle::RespondToPersonCheck);
        } else {
...
void BehaviorSleepCycle::RespondToPersonCheck() {
  ...
  if (GetBEI().GetFaceWorld().HasAnyFaces(_dVars.personCheckStartTimestamp,
                                          onlyRecognizable)) {
  ...
  if (!sleepTracker.IsSleepy(fromSleep) && !sleepTracker.IsNightTime()) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorSleepCycle::SleepTransitionHelper(
    const SleepStateID& newState, const bool playSleepGetIn,
...
void BehaviorSleepCycle::TransitionToEmergencySleep() {
  SleepTransitionHelper(SleepStateID::EmergencySleep, true,
                        _iConfig.emergencyModeAnimBehavior);
}
...
void BehaviorSleepCycle::TransitionToDeepSleep() {
  ...
  const float currTime_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  _dVars.nextPersonCheckTime_s =
      currTime_s + kSleepCycle_DeepSleep_PersonCheckInterval_s;
}
...
void BehaviorSleepCycle::TransitionToLightSleep() {
  ...
  const float currTime_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  _dVars.nextPersonCheckTime_s =
      currTime_s + kSleepCycle_LightSleep_PersonCheckInterval_s;
}
...
void BehaviorSleepCycle::SleepIfInControl(bool playGetIn) {
  if (!IsControlDelegated()) {
    // if we get here but are in a state like GoingToCharger then something
    // broke and we are about to fall asleep in an invalid state
    if (IsPreSleepState(_dVars.currState)) {
      LOG_ERROR(
          "BehaviorSleepCycle.SleepStateMistmatch",
      ...
      if (ANKI_VERIFY(
              ...
              "Called the TransitionToLightOrDeepSleep but it didn't delegate "
              "control, this is a bug")) {
    ...
    auto playAsleepBehavior = [this]() {
      // now that we're asleep, let's remove the "active intent state" in case
      // we were in it
    ...
    // even if requested, skip the get-in if this is a recent reboot (go
    // straight to sleep in this case)
    const bool skipGetIn = WasNightlyReboot();

    if (playGetIn && !skipGetIn) {
      if (_iConfig.goToSleepBehavior->WantsToBeActivated()) {
        DelegateIfInControl(_iConfig.goToSleepBehavior.get(),
                            playAsleepBehavior);
      } else {
...
void BehaviorSleepCycle::TransitionToSayingGoodnight() {
  ...
  DelegateIfInControl(
      new TriggerLiftSafeAnimationAction(AnimationTrigger::ReactToGoodNight),
  ...
  if (GetBEI().GetRobotInfo().IsOnChargerContacts()) {
    ...
    if (_iConfig.findChargerBehavior->WantsToBeActivated()) {
      ...
      LOG_WARNING("BehaviorSleepCycle.TransitionToCharger.WontRun",
                  "Not on charger contacts, but find charger behavior '%s' "
                  "doesn't want to activate",
...
void BehaviorSleepCycle::SetState(SleepStateID state) {
  ...
  const bool wasEmergencyMode =
      _dVars.currState == SleepStateID::EmergencySleep;

  _dVars.currState = state;
  LOG_INFO("BehaviorSleepCycle.SetState", "%s", SleepStateIDToString(state));
  ...
  if (wasEmergencyMode && !isEmergencyMode) {
    ...
    SmartAlterStreamStateForCurrentResponse(
        StreamAndLightEffect::StreamingDisabledButWithLight);
  }

  SetConditionsActiveForState(_dVars.currState, true);

  // if the state changes, our sleeping reaction will not be active (or be
  // canceled) so clear here
  _dVars.reactionState = SleepReactionType::NotReacting;
...
void BehaviorSleepCycle::SetReactionState(SleepReactionType reaction) {
  ...
  LOG_INFO("BehaviorSleepCycle.Reaction", "sleeping reaction: %s",
...
bool BehaviorSleepCycle::ShouldReactToSoundInState(const SleepStateID& state) {
  switch (state) {
    ...
    case SleepStateID::Awake:
      // this is specifically for the "react from asleep", so don't do it if
      // we're awake
      return false;
...
void BehaviorSleepCycle::MuteForPersonCheck(bool mute) {
  if (_dVars.isMuted != mute) {
    if (mute) {
      // lock the lift track because the robot likes to slam it around in a
      // noisy way in some animations
      SmartLockTracks(static_cast<u8>(AnimTrackFlag::LIFT_TRACK),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - NOTE:(bn) this code is copied from
// BehaviorQuietModeCoordinator. Decided against sharing it somewhere common for
// now because this is not a pattern I want to encourage
void BehaviorSleepCycle::SetAudioActive(bool active) {
  ...
  const auto event = active ? GE::Play__Robot_Vic_Scene__Quiet_Off
                            : GE::Play__Robot_Vic_Scene__Quiet_On;
