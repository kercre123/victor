 ...
 * Created: 2018-06-25
 *
 * Description: This behavior causes victor to, upon seeing a face, react
 *"loudly" if he hasn't seen the person for a certain amount of time.
 *
...
void ForceNextBigGreeting(ConsoleFunctionContextRef context) {
  if (sMapPtr) {
    ...
    for (auto& namePair : *sMapPtr) {
      namePair.second =
          time_t(0);  // should set to epoch (in any case, a long time ago)
    }
...
BehaviorGreetAfterLongTime::DynamicVariables::DynamicVariables()
    : lastSeenTimesPtr(
          std::make_shared<std::unordered_map<std::string, time_t>>()),
...
void BehaviorGreetAfterLongTime::InitBehavior() {
  ...
  _iConfig.driveOffChargerBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(DriveOffChargerFace));

  auto& variableSnapshotComp = GetBEI().GetVariableSnapshotComponent();
  variableSnapshotComp.InitVariable<std::unordered_map<std::string, time_t>>(
      VariableSnapshotId::BehaviorGreetAfterLongTime_LastSeenTimes,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorGreetAfterLongTime::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorGreetAfterLongTime::DebugPrintState(const char* debugLabel,
                                                 const LastSeenMapPtr mapPtr) {
  PRINT_CH_INFO("Behaviors", "BehaviorGreetAfterLongTime.DebugState", "%s",
                debugLabel);

  if (!mapPtr) {
    PRINT_NAMED_WARNING("BehaviorGreetAfterLongTime.DebugState.NoPointer",
                        "pointer is null");
    return;
  }
...
bool BehaviorGreetAfterLongTime::WantsToBeActivatedBehavior() const {
  ...
  const bool shouldActivate =
      currTime_s <= _dVars.shouldActivateBehaviorUntil_s;
  return shouldActivate;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorGreetAfterLongTime::GetBehaviorJsonKeys(
...
void BehaviorGreetAfterLongTime::OnBehaviorActivated() {
  ...
  const float currTime_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  const int timeSince_ms =
      std::round((_dVars.shouldActivateBehaviorUntil_s - currTime_s) * 1000.0f);

  DASMSG(
      big_greeting_played, "behavior.big_greeting.played",
      "We saw a face that we haven't seen in a while, playing a big greeting");
  DASMSG_SET(i1, _dVars.timeSinceSeenThisFace_s,
             "seconds since we last saw this face");
  DASMSG_SET(i2, _dVars.thisCooldown_s,
             "what the cooldown was for this instance of the big greeting");
  DASMSG_SET(i3, timeSince_ms,
             "milliseconds since we saw the face in BaseStationTime");
  DASMSG_SEND();

  // TODO:(bn) use normal pattern here of resetting all non-persistent variables
  _dVars.shouldActivateBehaviorUntil_s =
      -1.0f;  // don't activate again (right away)
  ...
  if (GetBEI().GetRobotInfo().IsOnChargerPlatform() &&
    ...
    DelegateIfInControl(_iConfig.driveOffChargerBehavior.get(),
                        &BehaviorGreetAfterLongTime::TurnTowardsFace);
  } else {
    TurnTowardsFace();
  }
}
...
void BehaviorGreetAfterLongTime::BehaviorUpdate() {
  ...
  if (!IsActivated() && _dVars.shouldActivateBehaviorUntil_s > 0.0f &&
    ...
    DASMSG(big_greeting_missed, "behavior.big_greeting.missed",
           "We saw a face that we haven't seen in a while, but failed to play "
           "the big greeting in time because "
  ...
  auto faceIds =
      faceWorld.GetFaceIDs(_dVars.lastFaceCheckTime_ms, kRecognizableFacesOnly,
                           kDontCheckRelativeAngles, kDontCheckRangeOfAngles);
  ...
  for (const auto& faceId : faceIds) {
    ...
    if (!name.empty()) {
      ...
      // if the face is newly recognizable, add it to the map with a greeting
      // cooldown starting now
      const bool isFaceNewlySeen =
          faceLastSeenTimeIter == _dVars.lastSeenTimesPtr->end();
      if (isFaceNewlySeen) {
        _dVars.lastSeenTimesPtr->emplace(name, unixUtcTime_s);
      } else {
        time_t faceLastSeenTime_s = faceLastSeenTimeIter->second;

        // flag behavior for activation if we see the face after long enough. It
        // can run immediately or in a few seconds, but not after too long
        const auto cooldownPeriod_s = GetCooldownPeriod_s();
        const bool shouldActivate =
            (faceLastSeenTime_s + cooldownPeriod_s <= unixUtcTime_s);
        if (shouldActivate) {
          _dVars.shouldActivateBehaviorUntil_s =
              currTime_s + kMaxTimerAfterSeenToReact_s;
          _dVars.targetFace = faceWorld.GetSmartFaceID(faceId);

          // save some variables now for DAS later
          _dVars.timeSinceSeenThisFace_s =
              (int)((long int)unixUtcTime_s - (long int)faceLastSeenTime_s);
      ...
      // keeps track of the last time a face was seen to avoid rechecking old
      // faces necessary because vision may add faces into the past after
      // completing processing note: times stored in lastFaceCheckTime_ms use
      // robot time and are not compatible with WallTime
      _dVars.lastFaceCheckTime_ms = std::max(
          _dVars.lastFaceCheckTime_ms, (RobotTimeStamp_t)face->GetTimeStamp());
    }
  }
}
...
bool BehaviorGreetAfterLongTime::ShouldBehaviorUpdate(time_t& unixUtcTime_s) {
  ...
  // TODO:(bn) switch back to non-approximate time once we trust that it is
  // working...
...
void BehaviorGreetAfterLongTime::TurnTowardsFace() {
  if (_dVars.targetFace.IsValid()) {
    ...
    PRINT_NAMED_WARNING(
        "BehaviorGreetAfterLongTime.TurnTowardsFace.NoFace",
...
void BehaviorGreetAfterLongTime::PlayReactionAnimation() {
  ...
  TriggerAnimationAction* action =
      new TriggerAnimationAction(AnimationTrigger::GreetAfterLongTime,
                                 kPlayAnimOnce, kCanAnimationInterrupt);
  DelegateNow(action);
}
