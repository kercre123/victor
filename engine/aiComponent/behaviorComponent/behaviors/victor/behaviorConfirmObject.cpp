 ...
 * Created: 2018-04-18
 *
 * Description: Runs only if an object position is known for a given object
 *type(s), drives to where it should be, and ends when it (or any other object
 *of the given type[s]) is seen. If it is not seen and there are multiple
 *objects of the given type(s) in blockworld, it will sequentially try them all
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorConfirmObject::InstanceConfig::InstanceConfig() { maxAttempts = -1; }
...
BehaviorConfirmObject::BehaviorConfirmObject(const Json::Value& config)
  ...
  DEV_ASSERT(_iConfig.maxAttempts == -1 || _iConfig.maxAttempts > 0,
             "Invalid param");

  // create the 3 conditions (or just two if min age isnt specified)
  Json::Value conditionConfig =
      IBEICondition::GenerateBaseConditionConfig(BEIConditionType::ObjectKnown);
  conditionConfig[kObjectTypesKey] = config[kObjectTypesKey];
  _iConfig.objectKnownCondition.reset(
      new ConditionObjectKnown(conditionConfig));
  ...
  if (config[kMinAgeForActivationKey].isDouble()) {
    ...
    _iConfig.objectKnownRecentlyCondition.reset(
        new ConditionObjectKnown(conditionConfig));
  ...
  //   If types A and B are known, and type A hasnt been seen recently, but type
  //   B has, within maxAge_ms, this behavior won't activate
  // Similarly, if you provide one object type, but there are multiple objects
  // of that type:
  //   If objects A and B are known, and object A hasnt been seen recently, but
  //   object B has, within maxAge_ms, this behavior won't activate
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorConfirmObject::GetBehaviorOperationModifiers(
    BehaviorOperationModifiers& modifiers) const {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorConfirmObject::GetBehaviorJsonKeys(
...
void BehaviorConfirmObject::OnBehaviorActivated() {
  ...
  std::vector<const ObservableObject*> knownObjects =
      _iConfig.objectKnownCondition->GetObjects(GetBEI());
  if (_iConfig.objectKnownRecentlyCondition != nullptr) {
    std::vector<const ObservableObject*> recentObjects =
        _iConfig.objectKnownRecentlyCondition->GetObjects(GetBEI());
  ...
  // since DriveToObjectAction only takes one object, find the closest one, but
  // keep all the rest in the same vector, sorted, with the closest at the end
  const auto& robotPose = GetBEI().GetRobotInfo().GetPose();
  _dVars.sortedObjects = knownObjects;
  std::sort(_dVars.sortedObjects.begin(), _dVars.sortedObjects.end(),
            [&robotPose](const auto* a, const auto* b) {
              ...
              if (!ComputeDistanceBetween(poseA, robotPose, distA) ||
                ...
                LOG_ERROR(
                    "BehaviorConfirmObject.OnBehaviorActivated.Sort."
                    "ComputeDistanceFailure",
                    "Failed computing distance between input pose(s) and robot "
                    "pose - returning false");
                return false;
              }
...
void BehaviorConfirmObject::DriveToClosestKnownObject() {
  ...
  // docking is what's used for cubes and charger, so we'll use it for all
  // object types
  auto* driveToAction = new DriveToObjectAction(
      closestObject, PreActionPose::ActionType::DOCKING);
  DelegateIfInControl(driveToAction, [this](ActionResult result) {
    ...
    // arrived at the destination and didnt see an object, or driving to the
    // pose failed. in either case, pop that object from the list and try the
    // next one
...
void BehaviorConfirmObject::BehaviorUpdate() {
  if (!IsActivated()) {
    const bool known =
        _iConfig.objectKnownCondition->AreConditionsMet(GetBEI());
    bool seenRecently = false;
    if (_iConfig.objectKnownRecentlyCondition != nullptr) {
      seenRecently =
          _iConfig.objectKnownRecentlyCondition->AreConditionsMet(GetBEI());
    }
