...
const int kPlayAnimOnce =
    1;  // value for number of loops argument for playing anim once
const bool kCanAnimationInterrupt =
    true;  // value for whether starting the animation can interrupt
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToTouchPetting::BehaviorReactToTouchPetting(
  ...
  _timeTilTouchCheck =
      JsonTools::ParseFloat(config, kTimeTilTouchCheckKey, kDebugStr);

  std::vector<std::string> tmp;
  ANKI_VERIFY(JsonTools::GetVectorOptional(config, kAnimGroupNamesKey, tmp),
              "BehaviorReactToTouchPetting.Ctor.MissingVectorOfAnimTriggers",
              "");
  ...
  tmp.clear();
  ANKI_VERIFY(
      JsonTools::GetVectorOptional(config, kAnimGroupNamesGetoutKey, tmp),
      "BehaviorReactToTouchPetting.Ctor.MissingVectorOfAnimTriggersGetout", "");
  for (const auto& t : tmp) {
    _animPettingGetout.push_back(AnimationTriggerFromString(t));
  }

  _animPettingGetin = AnimationTriggerFromString(
      JsonTools::ParseString(config, kAnimGroupGetinKey,
                             "BehaviorReactToTouchPetting.Ctor.MissingGetin"));

  _blissTimeout =
      JsonTools::ParseFloat(config, kTimeTilBlissGetoutKey, kDebugStr);
  _nonBlissTimeout =
      JsonTools::ParseFloat(config, kTimeTilNonBlissGetoutKey, kDebugStr);
  _minNumPetsToAdvanceBliss =
      JsonTools::ParseInt8(config, kNumPetsToAdvanceBlissKey, kDebugStr);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToTouchPetting::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {
      kTimeTilTouchCheckKey,     kAnimGroupNamesKey,
      kAnimGroupNamesGetoutKey,  kTimeTilBlissGetoutKey,
      kTimeTilNonBlissGetoutKey, kNumPetsToAdvanceBlissKey,
      kAnimGroupGetinKey,
  };
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToTouchPetting::AlwaysHandleInScope(
...
void BehaviorReactToTouchPetting::OnBehaviorActivated() {
  ...
  GetBEI()
      ...
      .GetTimer(BehaviorTimerTypes::ReactToTouchPetting)
      .Reset();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToTouchPetting::CancelAndPlayAnimation(
    AnimationTrigger anim) {
  TriggerAnimationAction* action =
      new TriggerAnimationAction(anim, kPlayAnimOnce, kCanAnimationInterrupt);
  CancelDelegates();
...
void BehaviorReactToTouchPetting::PlayBlissLoopAnimation() {
  TriggerAnimationAction* action = new TriggerAnimationAction(
      _animPettingResponse.back(), kPlayAnimOnce, kCanAnimationInterrupt);
...
void BehaviorReactToTouchPetting::BehaviorUpdate() {
  ...
  switch (_currResponseState) {
    case PlayTransitionToLevel: {
      const bool willBlissTimeout =
          reachedMaxBliss && (now > _checkForTimeoutTimeBliss);
      const bool willNonblissTimeout =
          !reachedMaxBliss && (now > _checkForTimeoutTimeNonbliss);
      const bool willTransition =
          (_isPressed && (now > _checkForTransitionTime)) ||
      ...
      if (!_isPressed && (willBlissTimeout || willNonblissTimeout)) {
        ...
        const bool willTransitionToMaxBliss =
            _currBlissLevel == (_animPettingResponse.size() - 1);
        ...
        if (_currBlissLevel == 0) {
          GetBEI().GetRobotAudioClient().PostEvent(
              AMD_GE_GE::Play__Robot_Vic_Sfx__Purr_Loop_Play,
              AMD_GOT::Behavior);
        } else {
          GetBEI().GetRobotAudioClient().PostEvent(
              AMD_GE_GE::Play__Robot_Vic_Sfx__Purr_Increase_Level,
              AMD_GOT::Behavior);
        }

        // important: increase the bliss level AFTER getting the animation index
        // reasoning:
        // - value of 0 for _currBlissLevel implies only the getin has been
        // played
        // - values 1-N for _currBlissLevel are for indexing the correct
        // animation for the level thus the animation's index is
        // _currBlissLevel-1 this keeps it consistent with how we pick the
        // getout animation index
        _currBlissLevel =
            MIN(_currBlissLevel + 1, (int)_animPettingResponse.size());

        const bool nowAtMaxBliss =
            (_currBlissLevel == _animPettingResponse.size());
        ...
        if (nowAtMaxBliss) {
          ...
          GetBehaviorComp<RobotStatsTracker>().IncrementBehaviorStat(
              BehaviorStat::PettingReachedMaxBliss);
        } else {
          ...
          GetBehaviorComp<RobotStatsTracker>().IncrementBehaviorStat(
              BehaviorStat::PettingBlissIncrease);
        }
      }
      break;
    }
    case PlayGetoutFromLevel: {
      ...
      if (wasPlayingGetin) {
        ...
        // - previously was in PlayTransitionToLevel, now play appropriate
        // getout
        // - expected flow:
        ...
        //                    |
        //      PlayBlissLoop | bliss animation cannot requeue because getout
        //      requested
        //                    |
        //      Update()      | Update() no animation playing, getout plays
        const auto animIdx =
            _currBlissLevel - 1;  // bliss levels are +1 wrt animation indexes
        CancelAndPlayAnimation(_animPettingGetout[animIdx]);

        GetBEI().GetRobotAudioClient().PostEvent(
            AMD_GE_GE::Stop__Robot_Vic_Sfx__Purr_Loop_Stop, AMD_GOT::Behavior);
...
void BehaviorReactToTouchPetting::OnBehaviorDeactivated() {
  if (_currBlissLevel >= 2) {
    // some decent amount of petting happened, so after this behavior finishes
    // the robot should chill for a bit (rather than, e.g. going back into
    // exploring)
    GetAIComp<AIWhiteboard>().OfferPostBehaviorSuggestion(
        PostBehaviorSuggestions::Nothing);
  }
