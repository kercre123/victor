...
class BehaviorReactToTouchPetting : public ICozmoBehavior {
 ...
 private:
  ...
  // future time after which we can end petting session (in the non max-bliss
  // state)
  ...
  // value 1-(N-1) maps to the petting transition animations, and their
  // respective getout value N maps to the maxbliss animation cycle, and its
  // getout
  u32 _currBlissLevel;

  // tracking variable -- counting individual press/release pairs per bliss
  // level
