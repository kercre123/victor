...
static constexpr float kMaxNormalAngle =
    DEG_TO_RAD(60.0f);  // how steep of an angle we can see
...
BehaviorTrackCube::BehaviorTrackCube(const Json::Value& config)
  ...
  _iConfig.maxTimeSinceObserved_ms =
      config.get(kMaxTimeSinceObsKey, _iConfig.maxTimeSinceObserved_ms).asInt();
  _iConfig.maxDistance_mm =
      config.get(kMaxDistanceKey, _iConfig.maxDistance_mm).asInt();
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorTrackCube::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorTrackCube::GetBehaviorJsonKeys(
...
ObjectID BehaviorTrackCube::GetVisibleCube() const {
  ...
  // the closest moving one is "most interesting" and is always selected, even
  // if it's farther than a stationary one
  ObjectID movingObject;
  float movingObjectDist_mm = std::numeric_limits<float>::max();

  const RobotTimeStamp_t currentTime_ms =
      GetBEI().GetRobotInfo().GetLastImageTimeStamp();

  for (const auto* object : objects) {
    if (object != nullptr) {
      using NotVisibleReason = Vision::KnownMarker::NotVisibleReason;
      NotVisibleReason reason = object->IsVisibleFromWithReason(
          visComp.GetCamera(), kMaxNormalAngle, kMinImageSizePix, false);

      const bool isVisible = (reason == NotVisibleReason::IS_VISIBLE);
      // currently, cubes arent removed if they are not seen in their original
      // location, so check the last time they were seen
      const bool recent =
          (_iConfig.maxTimeSinceObserved_ms < 0) ||
          (object->GetLastObservedTime() + _iConfig.maxTimeSinceObserved_ms >=
           currentTime_ms);
      if (isVisible && recent) {
        float dist = 0.f;
