...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorPoweringRobotOff::InstanceConfig::InstanceConfig(
    const Json::Value& config) {
  const std::string debugName =
      "BehaviorPoweringRobotOff.InstanceConfig.MissingKey. ";
  powerOnAnimName = JsonTools::ParseString(config, kPowerOnAnimName,
                                           debugName + kPowerOnAnimName);
  powerOffAnimName = JsonTools::ParseString(config, kPowerOffAnimName,
                                            debugName + kPowerOffAnimName);

  const auto timeActivateBehavior = JsonTools::ParseUInt32(
      config, kPowerButtonActivationKey, debugName + kPowerButtonActivationKey);
  activateBehaviorCondition =
      std::shared_ptr<IBEICondition>(new ConditionTimePowerButtonPressed(
          timeActivateBehavior, "BehaviorPoweringRobotOff"));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPoweringRobotOff::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPoweringRobotOff::GetAllDelegates(
    std::set<IBehavior*>& delegates) const {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPoweringRobotOff::GetBehaviorJsonKeys(
...
void BehaviorPoweringRobotOff::OnBehaviorActivated() {
  ...
  const bool prevShouldStartPowerOffAnimaiton =
      _dVars.shouldStartPowerOffAnimaiton;
  _dVars = DynamicVariables();

  // make shouldStartPowerOffAnimaiton persist if _iConfig.waitForAnimMsg, since
  // this behavior WantToBeActivated iff shouldStartPowerOffAnimaiton, whenever
  // _iConfig.waitForAnimMsg
  if (_iConfig.waitForAnimMsg) {
...
void BehaviorPoweringRobotOff::BehaviorUpdate() {
  ...
  const bool wasPowerOffLastAnimPlayed =
      _dVars.lastAnimPlayedName == _iConfig.powerOffAnimName;
  const bool wasPowerOnLastAnimPlayed =
      _dVars.lastAnimPlayedName == _iConfig.powerOnAnimName;

  // Don't do anything until the power button has been held down long enough to
  // start powering down
  ...
  } else if (!_dVars.shouldStartPowerOffAnimaiton &&
             wasPowerOffLastAnimPlayed) {
    if (IsControlDelegated()) {
      CancelDelegates(false);
      _dVars.waitingForAnimationCallback = true;
    } else {
  ...
  // If user released power button see if eyes are restored and normal behavior
  // should continue
  if (!IsControlDelegated() && !_dVars.shouldStartPowerOffAnimaiton &&
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPoweringRobotOff::AlwaysHandleInScope(
    const RobotToEngineEvent& event) {
  if (event.GetData().GetTag() ==
      RobotInterface::RobotToEngineTag::startShutdownAnim) {
...
void BehaviorPoweringRobotOff::TransitionToPoweringOff() {
  ...
  const auto startTime_ms =
      havePlayedAnyAnim ? GetLengthOfAnimation_ms(_iConfig.powerOffAnimName) -
                              _dVars.timeLastPowerAnimStopped_ms
                        : 0;
...
void BehaviorPoweringRobotOff::TransitionToPoweringOn() {
  const bool havePlayedAnyAnim = _dVars.timeLastPowerAnimStopped_ms > 0;
  const auto startTime_ms =
      havePlayedAnyAnim ? GetLengthOfAnimation_ms(_iConfig.powerOnAnimName) -
                              _dVars.timeLastPowerAnimStopped_ms
                        : 0;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPoweringRobotOff::StartAnimation(const std::string& animName,
  ...
  DelegateIfInControl(new PlayAnimationAction(
      animName, numLoops, interruptRunning, tracksToLock, timeout_sec,
      startTime_ms, callback));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
TimeStamp_t BehaviorPoweringRobotOff::GetLengthOfAnimation_ms(
  ...
  if ((animContainer != nullptr) && !_iConfig.powerOffAnimName.empty()) {
    ...
    if (animPtr != nullptr) {
      ...
      PRINT_NAMED_ERROR(
          "BehaviorPoweringRobotOff.GetLengthOfAnimation_ms.MissingAnimation",
