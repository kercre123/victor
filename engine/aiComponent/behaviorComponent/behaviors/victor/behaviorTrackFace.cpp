...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorTrackFace::WantsToBeActivatedBehavior() const { return true; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorTrackFace::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorTrackFace::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kFaceSelectionPenaltiesKey};
...
void BehaviorTrackFace::OnBehaviorActivated() {
  ...
  if (_dVars->faceToTrack.IsValid()) {
    PRINT_CH_INFO("Behaviors", "BehaviorTrackFace.Start",
                  "%s: starting by tracking face %s", GetDebugLabel().c_str(),
...
void BehaviorTrackFace::BeginTracking() {
  ...
  if (onCharger) {
    // TODO:(bn) something is "fighting" the eye move and re-centering the eyes
    // each time
...
void BehaviorTrackFace::BehaviorUpdate() {
  ...
  if (IsActivated()) {
    ...
    const bool newFace =
        bestFace.IsValid() && (bestFace != _dVars->faceToTrack);
