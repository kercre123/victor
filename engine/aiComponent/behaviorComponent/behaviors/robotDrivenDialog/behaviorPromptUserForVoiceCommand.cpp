...
// TODO:(str) Currently not in use. Rework this to use a smarter TurnToFace
// structure, perhaps LookAtFaceInFront
...
constexpr float kMaxRecordTime_s =
    10.0f;  // matches timeouts for TriggerWord and KnowledgeGraph

static_assert(kMaxRecordTime_s >=
                  ((MicData::kStreamingTimeout_ms + 2000) / 1000.f),
              "kMaxRecordTime_s should be >= kStreamingTimeout_ms by about 2 "
              "seconds to give chipper time to respond");

// when we heard something but don't have a matching intent, do we want to stop
// immediately or wait for animation timeout?
...
#define SET_STATE(s)                                                      \
  do {                                                                    \
    ...
    PRINT_CH_INFO("Behaviors", "BehaviorPromptUserForVoiceCommand.State", \
                  "State = %s", #s);                                      \
  } while (0);
...
BehaviorPromptUserForVoiceCommand::BehaviorPromptUserForVoiceCommand(
  ...
  // we must have a stream type supplied, else the cloud doesn't know what to do
  // with it
  // * defaults don't make much sense at this point either
  const std::string streamTypeString = JsonTools::ParseString(
      config, kStreamType,
  ...
  JsonTools::GetValueOptional(config, kShouldTurnToFaceKey,
                              _iConfig.shouldTurnToFace);

  // Set up the TextToSpeech Behavior
  JsonTools::GetValueOptional(config, kTextToSpeechBehaviorKey,
                              _iConfig.ttsBehaviorID);

  // Configurable localization keys
  JsonTools::GetValueOptional(config, kVocalPromptKey, _iConfig.vocalPromptKey);
  JsonTools::GetValueOptional(config, kVocalResponseToIntentKey,
                              _iConfig.vocalResponseToIntentKey);
  JsonTools::GetValueOptional(config, kVocalResponseToBadIntentKey,
                              _iConfig.vocalResponseToBadIntentKey);
  JsonTools::GetValueOptional(config, kVocalRepromptKey,
                              _iConfig.vocalRepromptKey);

  // Should we exit the behavior as soon as an intent is pending, or finish what
  // we're doing first?
  JsonTools::GetValueOptional(config, kStopListeningOnIntentsKey,
                              _iConfig.stopListeningOnIntents);
  // play the getIn animation for the listening anim?
  JsonTools::GetValueOptional(config, kPlayListeningGetInKey,
                              _iConfig.playListeningGetIn);
  // play the getOut animation for the listening anim?
  JsonTools::GetValueOptional(config, kPlayListeningGetOutKey,
                              _iConfig.playListeningGetOut);
  // Should we repeat the prompt if it fails? If so, how many times
  JsonTools::GetValueOptional(config, kMaxRepromptKey,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPromptUserForVoiceCommand::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPromptUserForVoiceCommand::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPromptUserForVoiceCommand::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kEarConSuccess,
                        kEarConFail,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPromptUserForVoiceCommand::SetPromptString(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPromptUserForVoiceCommand::SetRepromptString(
    const std::string& text) {
...
void BehaviorPromptUserForVoiceCommand::InitBehavior() {
  BehaviorID ttsID =
      BehaviorTypesWrapper::BehaviorIDFromString(_iConfig.ttsBehaviorID);
  GetBEI().GetBehaviorContainer().FindBehaviorByIDAndDowncast(
      ttsID, BEHAVIOR_CLASS(TextToSpeechLoop), _iConfig.ttsBehavior);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPromptUserForVoiceCommand::OnBehaviorActivated() {
  // _dVars are reset on deactivation so that the effects of
  // SetPrompt/SetReprompt persist

  // Configure streaming params with defaults in case they're not set due to
  // behaviorStack state
  namespace AECH = AudioEngine::Multiplexer::CladMessageHelper;
  const auto postAudioEvent = AECH::CreatePostAudioEvent(
...
void BehaviorPromptUserForVoiceCommand::OnBehaviorDeactivated() {
  // Any resultant intents should be handled by external behaviors or
  // transitions, let 'em roll
  ...
  if (EState::Listening == _dVars.state) {
    if (!IsControlDelegated()) {
      bool waitingOnGetIn = _iConfig.playListeningGetIn &&
                            GetBehaviorComp<UserIntentComponent>()
                                .WaitingForTriggerWordGetInToFinish();
      if (!waitingOnGetIn) {
        DelegateIfInControl(
            new ReselectingLoopAnimationAction(
                AnimationTrigger::VC_ListeningLoop, 0, true,
                (uint8_t)AnimTrackFlag::NO_TRACKS,
    ...
    if (_iConfig.stopListeningOnIntents) {
      const bool intentHeard =
          (EIntentStatus::IntentHeard == _dVars.intentStatus);
      const bool intentUnknown =
          (EIntentStatus::IntentUnknown == _dVars.intentStatus) &&
          kStopListeningOnUnknownIntent;
      const bool intentSilence =
          (EIntentStatus::IntentSilence == _dVars.intentStatus);
...
void BehaviorPromptUserForVoiceCommand::CheckForPendingIntents() {
  ...
  if ((EIntentStatus::NoIntentHeard == _dVars.intentStatus) &&
      uic.IsAnyUserIntentPending()) {
    // Don't dismiss unclaimed intents until this behavior exits, or other
    // behaviors may miss their chance to claim the pending intents
    uic.SetUserIntentTimeoutEnabled(false);

    _dVars.intentStatus = EIntentStatus::IntentHeard;

    // If robot heard an unmatched intent, note it so we can respond
    // appropriately, then clear it.
    ...
    // If robot heard silence, record the outcome for proper handling, then
    // clear it.
...
void BehaviorPromptUserForVoiceCommand::TransitionToTurnToFace() {
  ...
  DelegateIfInControl(
      ...
      &BehaviorPromptUserForVoiceCommand::TransitionToPrompting);
}
...
void BehaviorPromptUserForVoiceCommand::TransitionToPrompting() {
  ...
  if (_iConfig.ttsBehavior->WantsToBeActivated()) {
    DelegateIfInControl(
        _iConfig.ttsBehavior.get(),
        &BehaviorPromptUserForVoiceCommand::TransitionToListening);
  }
}
...
void BehaviorPromptUserForVoiceCommand::TransitionToListening() {
  ...
  GetBehaviorComp<UserIntentComponent>().StartWakeWordlessStreaming(
      _iConfig.streamType, _iConfig.playListeningGetIn);
}
...
void BehaviorPromptUserForVoiceCommand::TransitionToThinking() {
  ...
  // Play the Listening getOut, then close out the streaming stuff in case an
  // intent was just a little late
  auto callback = [this]() {
    // Play "earCon end"
    Audio::EngineRobotAudioClient& rac = GetBEI().GetRobotAudioClient();

    if (EIntentStatus::IntentHeard == _dVars.intentStatus) {
      if (AudioMetaData::GameEvent::GenericEvent::Invalid !=
          _iConfig.earConSuccess) {
        rac.PostEvent(_iConfig.earConSuccess,
                      AudioMetaData::GameObjectType::Behavior);
      }
    } else {
      if (AudioMetaData::GameEvent::GenericEvent::Invalid !=
          _iConfig.earConFail) {
        rac.PostEvent(_iConfig.earConFail,
                      AudioMetaData::GameObjectType::Behavior);
      }
    }
  ...
  if (_iConfig.playListeningGetOut) {
    DelegateIfInControl(
        new TriggerAnimationAction(AnimationTrigger::VC_ListeningGetOut),
        callback);
  } else {
    callback();
  }
...
void BehaviorPromptUserForVoiceCommand::TransitionToIntentReceived() {
  ...
  //  1. Any resultant intent is handled by a non-interrupting behavior &&
  //  !ExitOnIntents
  ...
  if (EIntentStatus::IntentHeard == _dVars.intentStatus) {
    ...
    const auto& vocalResponseToBadIntentString =
        GetVocalResponseToBadIntentString();
    if (vocalResponseToBadIntentString.empty()) {
      ...
      if (_iConfig.ttsBehavior->WantsToBeActivated()) {
        DelegateIfInControl(
            _iConfig.ttsBehavior.get(),
            &BehaviorPromptUserForVoiceCommand::TransitionToReprompt);
      }
    }
  }
}
...
void BehaviorPromptUserForVoiceCommand::TransitionToReprompt() {
  if (_dVars.repromptCount < _iConfig.maxNumReprompts) {
    ...
    if (vocalRepromptString.empty()) {
      // If we don't have any Reprompt anims or vocalizations, just reuse the
      // prompting state
      PRINT_CH_INFO(
          "Behaviors", "BehaviorPromptUserForVoiceCommand.RepromptGeneric",
          "Reprompting user %d of %d times with original prompt action",
          _dVars.repromptCount, _iConfig.maxNumReprompts);
      TransitionToPrompting();
      return;
    } else {
      PRINT_CH_INFO(
          "Behaviors", "BehaviorPromptUserForVoiceCommand.RepromptSpecialized",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::string BehaviorPromptUserForVoiceCommand::GetVocalResponseToIntentString()
    const {
...
// - - - - - - - - - - - - - - - - - - - -
std::string
BehaviorPromptUserForVoiceCommand::GetVocalResponseToBadIntentString() const {
  const auto& vocalResponseToBadIntentKey =
      _iConfig.vocalResponseToBadIntentKey;
