...
// speed up high level AI with this. Be careful -- some conditions that check
// for time within a short interval [a,b] may not be met if you choose too fast
// a speedup factor
...
const char* kPlayWithCubeOnChargerCooldownKey =
    "playWithCubeOnChargerCooldown_s";
const char* kMaxFaceDistanceToSocializeKey = "maxFaceDistanceToSocialize_mm";
const char* kPostBehaviorSuggestionResumeOverrides =
    "postBehaviorSuggestionResumeOverrides";  // whew
...
BehaviorHighLevelAI::BehaviorHighLevelAI(const Json::Value& config)
  ...
  _params.socializeKnownFaceCooldown_s =
      JsonTools::ParseFloat(config, kSocializeKnownFaceCooldownKey, kDebugName);
  _params.playWithCubeCooldown_s =
      JsonTools::ParseFloat(config, kPlayWithCubeCooldownKey, kDebugName);
  _params.playWithCubeOnChargerCooldown_s = JsonTools::ParseFloat(
      config, kPlayWithCubeOnChargerCooldownKey, kDebugName);
  _params.maxFaceDistanceToSocialize_mm =
      JsonTools::ParseFloat(config, kMaxFaceDistanceToSocializeKey, kDebugName);

  if (config[kPostBehaviorSuggestionResumeOverrides].isObject()) {
    const auto& resumeOverrides =
        config[kPostBehaviorSuggestionResumeOverrides];
    for (const auto& suggestionName : resumeOverrides.getMemberNames()) {
      ...
      if (ANKI_VERIFY(
              PostBehaviorSuggestionsFromString(suggestionName, suggestion),
        ...
        std::string stateName = JsonTools::ParseString(
            resumeOverrides, suggestionName.c_str(), GetDebugLabel());
        StateID state = GetStateID(stateName);
        if (ANKI_VERIFY(state != InvalidStateID,
                        "BehaviorHighLevelAI.Ctor.PBSInvalidState",
                        "State '%s' is no longer a state", stateName.c_str())) {
  ...
  MakeMemberTunable(_params.socializeKnownFaceCooldown_s,
                    kSocializeKnownFaceCooldownKey, kDebugName);
  MakeMemberTunable(_params.playWithCubeOnChargerCooldown_s,
                    kPlayWithCubeOnChargerCooldownKey, kDebugName);
...
void BehaviorHighLevelAI::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  InternalStatesBehavior::GetBehaviorJsonKeys(expectedKeys);
  const char* list[] = {
      kSocializeKnownFaceCooldownKey,         kPlayWithCubeCooldownKey,
      kPlayWithCubeOnChargerCooldownKey,      kMaxFaceDistanceToSocializeKey,
...
CustomBEIConditionHandleList BehaviorHighLevelAI::CreateCustomConditions() {
  ...
  const std::string emptyOwnerLabel = "";  // when these conditions are claimed
                                           // from the factory, a label is added
  ...
  handles.emplace_back(BEIConditionFactory::InjectCustomBEICondition(
      ...
      std::make_shared<ConditionLambda>(
          [this](BehaviorExternalInterface& bei) {
            ...
            const auto& timer = GetBEI().GetBehaviorTimerManager().GetTimer(
                BehaviorTimerTypes::Socializing);
            const bool cooldownExpired = timer.HasCooldownExpired(
                _params.socializeKnownFaceCooldown_s / kTimeMultiplier,
                valueIfNeverRun);
            if (!cooldownExpired) {
              // still on cooldown
              return false;
            ...
            for (const auto& faceID : faces) {
              ...
              if (face != nullptr) {
                ...
                if (ComputeDistanceSQBetween(bei.GetRobotInfo().GetPose(),
                                             facePose, distanceToFace) &&
                    distanceToFace <
                        Util::Square(_params.maxFaceDistanceToSocialize_mm)) {
                  return true;
                }
              }
            }
  ...
  handles.emplace_back(BEIConditionFactory::InjectCustomBEICondition(
      ...
      std::make_shared<ConditionLambda>(
          [this](BehaviorExternalInterface& bei) {
            // this keeps two separate timers, one for driving off the charger
            // into play, and one for playing. the latter is useful when seeing
            // cubes when in the observing state, and is used by the
            // CloseCubeForPlaying strategy. The former decides whether the
            // robot should leave the charger to play. In case the robot finds
            // no cube after leaving the charger, placing it back on the charger
            // won't cause it to immediately drive off in search of play.
            // Similarly, if a long time has elapsed since the robot last
            // successfully drove off the charger for playing, but it recently
            // played with a cube, then it shouldn't try to drive off the
            // charger again.
            ...
            const auto& timer = GetBEI().GetBehaviorTimerManager().GetTimer(
                BehaviorTimerTypes::PlayingWithCube);
            const bool hasntPlayed = timer.HasCooldownExpired(
                _params.playWithCubeCooldown_s / kTimeMultiplier,
                valueIfNeverRun);
  ...
  handles.emplace_back(BEIConditionFactory::InjectCustomBEICondition(
      ...
      std::make_shared<ConditionLambda>(
          [this](BehaviorExternalInterface& bei) {
            ...
            BlockWorldFilter filter;
            filter.SetFilterFcn([](const ObservableObject* obj) {
              return IsValidLightCube(obj->GetType(), false) &&
                     obj->IsPoseStateKnown();
            });
            const auto& blockWorld = bei.GetBlockWorld();
            const auto* block = blockWorld.FindLocatedMatchingObject(filter);
  ...
  handles.emplace_back(BEIConditionFactory::InjectCustomBEICondition(
      ...
      std::make_shared<ConditionLambda>(
          [this](BehaviorExternalInterface& bei) {
            const float stateStartTime_s =
                GetLastTimeStarted(GetCurrentStateID());
            const float currTime_s =
                BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();

            if (currTime_s - stateStartTime_s <
...
void BehaviorHighLevelAI::OverrideResumeState(StateID& resumeState) {
  ...
  for (const auto& configSuggestion : _params.pbsResumeOverrides) {
    size_t tick = 0;
    if (GetAIComp<AIWhiteboard>().GetPostBehaviorSuggestion(
            configSuggestion.first, tick) &&
        (tick >= maxTick)) {
      maxTick = tick;
...
void BehaviorHighLevelAI::OnStateNameChange(
    const std::string& oldStateName, const std::string& newStateName) const {
  // send a DAS msg. A lot of this is captured by the feature start/ending, but
  // if a state holds more than one feature, or if some states are transient,
  // this helps to understand the whole behavior
  DASMSG(behavior_hlai_change, "behavior.hlai.change",
         "HLAI changed its state");
