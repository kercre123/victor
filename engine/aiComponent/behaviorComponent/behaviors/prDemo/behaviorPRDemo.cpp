...
void BehaviorPRDemo::SetOverideState(StateID state) {
  if (_resumeOverride != InternalStatesBehavior::InvalidStateID &&
    ...
    PRINT_NAMED_WARNING(
        "BehaviorPRDemo.SetOverrideState.Repalcing",
        "Wiping out old override state of '%s' with new state '%s'",
        GetStateName(_resumeOverride).c_str(), GetStateName(state).c_str());
  }

  PRINT_CH_INFO("Demo", "BehaviorPRDemo.SetOverrideState", "%s",
