...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPRDemoBase::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPRDemoBase::GetBehaviorOperationModifiers(
...
void BehaviorPRDemoBase::OnBehaviorActivated() {
  ...
  GetBEI().GetAnimationComponent().AddKeepFaceAliveDisableLock(
      kPRDemoDisableLockName);
  ...
  if (_iConfig.sleepingBehavior->WantsToBeActivated()) {
    DelegateIfInControl(_iConfig.sleepingBehavior.get(), [this]() {
      ...
      GetBEI().GetAnimationComponent().RemoveKeepFaceAliveDisableLock(
          kPRDemoDisableLockName);
...
void BehaviorPRDemoBase::BehaviorUpdate() {
  ...
  if (uic.IsAnyUserIntentPending()) {
    ...
    if (userIntentDataPtr != nullptr) {
      _iConfig.demoBehavior->InformOfVoiceIntent(
          userIntentDataPtr->intent.GetTag());
    }
  }
