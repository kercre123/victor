...
#define SET_ANIM_STATE(s)                                                   \
  do {                                                                      \
    _dVars.animState = AnimState::s;                                        \
    PRINT_CH_INFO("Behaviors", "BehaviorAlexa.AnimState", "AnimState = %s", \
                  #s);                                                      \
  } while (0);
}  // namespace
...
BehaviorAlexa::BehaviorAlexa(const Json::Value& config)
  ...
  if (ANKI_VERIFY(!responseData.isNull(),
                  "BehaviorAlexa.Ctor.MissingResponseData",
                  "Could not find key 'responseData'")) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAlexa::GetBehaviorJsonKeys(
...
void BehaviorAlexa::LoadConfig(const Json::Value& data) {
  ...
  for (const auto& uxStateStr : data.getMemberNames()) {
    ...
    if (ANKI_VERIFY(AlexaUXStateFromString(uxStateStr, uxState),
                    ...
                    "'%s' is not a valid AlexaUXState", uxStateStr.c_str())) {
      ...
      if (!fromIdle.isNull()) {
        ...
        if (!animTrigger.empty()) {
          _iConfig.uxResponses[uxState].animTrigger =
              AnimationTriggerFromString(animTrigger);
        }
        if (!audioEvent.empty()) {
          _iConfig.uxResponses[uxState].audioEvent =
              AudioMetaData::GameEvent::GenericEventFromString(audioEvent);
        }
      }
        ...
        for (const auto& nextUXStateStr : transitions.getMemberNames()) {
          ...
          if (ANKI_VERIFY(AlexaUXStateFromString(nextUXStateStr, nextUXState),
            ...
            if (ANKI_VERIFY(transitions[nextUXStateStr].isString(),
                            ...
                            nextUXStateStr.c_str())) {
              const std::string& transAnim =
                  transitions[nextUXStateStr].asString();
              AnimationTrigger transTrigger;
              if (ANKI_VERIFY(
                      AnimationTriggerFromString(transAnim, transTrigger),
                      "BehaviorAlexa.LoadConfig.InvalidTransTrigger",
                      "Could not read transition anim trigger from '%s'",
                      transAnim.c_str())) {
                // insert into map of what animation to play when transitioning
                // from uxState to nextUXState
                _iConfig.animInfoMap[uxState].transitions.emplace_back(
                    nextUXState, transTrigger);
              }
            }
          }
        }
      ...
      if (ANKI_VERIFY(stateInfo["loop"].isString(),
                      ...
                      "'loop' not found for ux state %s", uxStateStr.c_str())) {
        ...
        if (ANKI_VERIFY(AnimationTriggerFromString(loopAnim, loopTrigger),
                        ...
                        loopAnim.c_str())) {
          // insert into map of what animation to play when transitioning from
          // uxState to Idle
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAlexa::GetBehaviorOperationModifiers(
...
void BehaviorAlexa::OnBehaviorActivated() {
  ...
  if (!alexaComp.IsUXStateGetInPlaying(currUxState)) {
    // there was no get-in from idle for this state, or it somehow finished
    // really quickly
...
void BehaviorAlexa::BehaviorUpdate() {
  ...
  // TODO: to deal with eye color changes, we will need to replace the existing
  // get-in anims with anims for the current eye color whenever the setting
  // changes
  ...
  // don't actually change _dVars.uxState. That only happens when starting an
  // animation
  const bool animsComplete =
      !IsControlDelegated() && !alexaComp.IsAnyUXStateGetInPlaying();
  ...
  if (uxChanged) {
    switch (_dVars.animState) {
      ...
      case AnimState::Transition:
        ...
        break;
      case AnimState::Idle: {
        DEV_ASSERT(false && "Unexpected state",
                   "BehaviorAlexa.BehaviorUpdate.UnexpectedAnimState");
      } break;
    }
  } else if (animsComplete && !alexaComp.IsIdle()) {
    // ux state didnt change, and any animations we have started have finished,
    // which means we should now start the state loop
    TransitionTo_StateLoop(newState);
  }

  if (alexaComp.IsIdle() && (_dVars.uxState == AlexaUXState::Idle) &&
      animsComplete && !IsControlDelegated()) {
    // we might consider a timeout here if the ux state switches very briefly to
    // idle
    CancelSelf();
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
AnimationTrigger BehaviorAlexa::GetTransitionAnimation(
    AlexaUXState fromState, AlexaUXState toState) const {
  ...
  if (it != _iConfig.animInfoMap.end()) {
    auto itTrigger = std::find_if(
        it->second.transitions.begin(), it->second.transitions.end(),
        [&](const auto& p) { return p.first == toState; });
  ...
  if (result == AnimationTrigger::Count) {
    PRINT_NAMED_WARNING("BehaviorAlexa.GetTransitionAnimation.NotFound",
                        ...
                        AlexaUXStateToString(fromState),
                        AlexaUXStateToString(toState));
  }
  return result;
}
...
void BehaviorAlexa::TransitionTo_IdleToNonIdle(AlexaUXState newState) {
  ...
  // nothing to do here other than wait. the anim process is playing any getins
  // we provided in OnBehaviorEnteredActivatableScope
}
...
void BehaviorAlexa::TransitionTo_StateLoop(AlexaUXState newState) {
  ...
  if (loopAnim != AnimationTrigger::Count) {
    ...
    DelegateIfInControl(action);  // no callback, Update() will transition out
                                  // when the ux state changes
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAlexa::TransitionTo_Transition(AlexaUXState oldState,
                                            AlexaUXState newState) {
  ...
  if (transAnim != AnimationTrigger::Count) {
    ...
    action->SetRenderInEyeHue(false);
    // no callback, Update() will either transition to the state loop, or, if
    // the ux state changes between now and when this action finishes, then
    // another transition will run
