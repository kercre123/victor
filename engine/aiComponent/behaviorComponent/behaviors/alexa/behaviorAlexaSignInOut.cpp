...
void BehaviorAlexaSignInOut::InitBehavior() {
  ...
  _iConfig.wakeWordBehaviors.AddBehavior(GetBEI().GetBehaviorContainer(),
...
bool BehaviorAlexaSignInOut::WantsToBeActivatedBehavior() const {
  ...
  // don't run if the wake word behavior is active, since BehaviorAlexaSignInOut
  // could be higher priority, but we want to give the wake word behavior time
  // to finish
  const bool wakeWordComplete =
      !_iConfig.wakeWordBehaviors.AreBehaviorsActivated();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAlexaSignInOut::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAlexaSignInOut::GetAllDelegates(
...
void BehaviorAlexaSignInOut::OnBehaviorActivated() {
  ...
  const bool signInPending = alexaComp.IsSignInPending();
  const bool signOutPending = alexaComp.IsSignOutPending();
  DEV_ASSERT(signInPending != signOutPending,
             "BehaviorAlexaSignInOut.OnBehaviorActivated.TwoCommands");
...
void BehaviorAlexaSignInOut::OnBehaviorDeactivated() {
  // make sure sign in/out is handled if this behavior gets interrupted. won't
  // do anything if already handled
  SignInOrOut();
}
...
void BehaviorAlexaSignInOut::BehaviorUpdate() {
  ...
  if (_dVars.signingIn && (GetActivatedDuration() >= kSignInDelay_s)) {
    // it takes a second or so for the server to respond with a code, so start
    // that process now. does nothing if already handled
    SignInOrOut();
  }
}
...
void BehaviorAlexaSignInOut::PlaySignOutAnimAndExit() {
  // hack: until there's a new stub re-use blackjack animation but with audio
  // track locked
  auto* action = new TriggerLiftSafeAnimationAction{
      AnimationTrigger::AlexaSignOut, 1, true, (u8)AnimTrackFlag::AUDIO_TRACK};
  DelegateIfInControl(action,
                      [this](const ActionResult& res) { CancelSelf(); });
}
