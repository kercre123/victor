...
namespace {
constexpr const float kMonthsThresh_days =
    30.436875;  // days at which we start counting in months instead
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorHowOldAreYou::WantsToBeActivatedBehavior() const { return true; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorHowOldAreYou::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorHowOldAreYou::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorHowOldAreYou::GetBehaviorJsonKeys(
...
void BehaviorHowOldAreYou::InitBehavior() {
  ...
  container.FindBehaviorByIDAndDowncast<BehaviorCountingAnimation>(
      BEHAVIOR_ID(HowOldAreYouCounting), BEHAVIOR_CLASS(CountingAnimation),
...
void BehaviorHowOldAreYou::OnBehaviorActivated() {
  ...
  LOG_INFO("BehaviorHowOldAreYou.OnBehaviorActivated.PresentableAge",
           "Robot presentable age: %d %s", presentable.first,
           presentable.second.c_str());

  // delegate to counting animation
  _iConfig.countingAnimationBehavior->SetCountTarget(presentable.first,
                                                     presentable.second);
...
std::chrono::hours BehaviorHowOldAreYou::GetRobotAge() {
  ...
  auto saveFolder = platform->pathToResource(
      Util::Data::Scope::Persistent,
      BehaviorOnboardingCoordinator::kOnboardingFolder);
  saveFolder = Util::FileUtils::AddTrailingFileSeparator(saveFolder);
  const std::string& onboardingDataFilePath =
      saveFolder + BehaviorOnboardingCoordinator::kOnboardingFilename;
  if (Util::FileUtils::DirectoryExists(saveFolder) &&
      Util::FileUtils::FileExists(onboardingDataFilePath)) {
    // file exists
    const std::string& file_s =
        Util::FileUtils::ReadFile(onboardingDataFilePath);
    ...
    if (parsed) {
      ...
      LOG_WARNING("BehaviorHowOldAreYou.GetRobotAge.JsonParsingFailed",
                  "%s exists but failed to parse as Json",
                  onboardingDataFilePath.c_str());
    }
    ...
    if (parsed) {
      ...
      containsBoD =
          file_j.isMember(BehaviorOnboardingCoordinator::kOnboardingTimeKey);
      if (containsBoD) {
        // if so, use that
        onboardingTime_sse =
            file_j[BehaviorOnboardingCoordinator::kOnboardingTimeKey].asInt64();
        LOG_INFO("BehaviorHowOldAreYou.GetRobotAge.ReadOnboardingTime",
                 "Read onboarding time (seconds since epoch): %lld",
                 onboardingTime_sse);
      } else {
        // INFO because plenty of robots have onboarded before we introduce the
        // change that writes this value to file.
    ...
    if (!parsed || !containsBoD) {
      // if not we couldn't get born on date from file, use modification time of
      // the file
      onboardingTime_sse = Util::FileUtils::GetFileLastModificationTime(
          onboardingDataFilePath);  // seconds since the epoch
      LOG_INFO("BehaviorHowOldAreYou.GetRobotAge.ModificationTimeFallback",
               "Using file modification time as fallback (seconds since "
               "epoch): %lld",
               onboardingTime_sse);
    }

    // convert seconds since the epoch to age in hours
    // make a duration, in seconds; make a system_clock timepoint from that
    // duration--i.e., that many seconds since the epoch
    const auto onboarding_tp = std::chrono::system_clock::time_point(
        std::chrono::seconds(onboardingTime_sse));
    // subtract onboarding_tp from now to get a duration--the time from
    // onboarding time until now
    const auto onboarding_dsc =
        std::chrono::system_clock::now() - onboarding_tp;
    // convert that duration to the units we want--hours, in this case. We're
    // totally cool with losing precision.
    const std::chrono::hours robotAge_dh =
        std::chrono::duration_cast<std::chrono::hours>(onboarding_dsc);
    ...
    // onboarding save file does not exist; fall back on RobotStatsTracker as
    // done above WARNING because it's really expected that this file exists by
    // the time we get here
    LOG_WARNING(
        "BehaviorHowOldAreYou.GetRobotAge.NoOnboardingFallback",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorHowOldAreYou::PresentableAge
BehaviorHowOldAreYou::PresentableAgeFromHours(std::chrono::hours age_hours) {
  ...
  const duration_days age_days =
      std::chrono::duration_cast<duration_days>(age_hours);
  const int count_days = age_days.count();

  // translate days to months
  // loss of precision is basically the point
  const duration_months age_months =
      std::chrono::duration_cast<duration_months>(age_days);
  const int count_months = age_months.count();

  //
  // Use locale component to get localized version of announcement string.
  // This is basically a four-way switch between "one day", "N days", "one
  // month", and "N months". If we're less than kMonthsThresh_days days, use
  // days, else translate to months.
  //
  // Note: current implementation (at Design's request) is to return the floor:
  // i.e., round everything down until we get to a whole day: 47 hours -> 1 day,
  // 49 hours -> 2 days
  //
  ...
  if (count_days == 1) {
    ...
    return PresentableAge(
        count_days,
        localeComponent.GetString(kSomeDaysKey, std::to_string(count_days)));
  } else if (count_months == 1) {
    return PresentableAge(count_months,
                          localeComponent.GetString(kOneMonthKey));
  } else {
    return PresentableAge(
        count_months, localeComponent.GetString(kSomeMonthsKey,
                                                std::to_string(count_months)));
  }
}
