...
class BehaviorAnimSequence : public ICozmoBehavior {
 ...
 protected:
  virtual void GetBehaviorOperationModifiers(
    ...
    // always runs on the charger, but the animation itself might lock the body
    // tracks if it's not in the whitelist
  ...
  // Returns an action that will play all animations in the class the
  // appropriate number of times for one loop
  IActionRunner* GetAnimationAction();

  // Returns true if multiple animations will be played as a loop _numLoops
  // times Returns false if a single animation will play _numLoops times
  bool IsSequenceLoop();
 ...
 private:
  struct InstanceConfig {
    ...
    u8 tracksToLock;
    bool renderInEyeHue;
    // Class supports playing a series of animation triggers OR a series of
    // animations by name BUT NOT BOTH AT THE SAME TIME!!!!
  ...
  struct DynamicVariables {
    DynamicVariables();
    // for sequences it's not per animation, but per sequence, so we have to
