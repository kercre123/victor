...
class BehaviorTextToSpeechLoop : public ICozmoBehavior {
 public:
  ...
  // callback passes in true if generation was completed successfully, false if
  ...
  // immediate false means it will finish the next loop animation and then exit
  // with the normal get out anim
 ...
 private:
  enum class State { IdleLoop, GetIn, SpeakingLoop, GetOut, EmergencyGetOut };
  ...
  // NOTE: Because state set on this behavior before it is run must persist into
  // running,
  //       Dynamic variables are reset in OnBehaviorDeactivated instead of
  //       OnBehaviorActivated
