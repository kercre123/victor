 ...
 * Created: 2018-11-19
 *
 * Description: A hopefully-reusable behavior that coordinates a counting
 *animation.
 *
...
BehaviorCountingAnimation::BehaviorCountingAnimation(const Json::Value& config)
  ...
  if (ANKI_VERIFY(
          config.isMember(JsonKeys::SlowLoopBeginSizeKey),
          "BehaviorCountingAnimation.Constructor.MissingSlowLoopBeginSize",
          "")) {
    if (ANKI_VERIFY(config[JsonKeys::SlowLoopBeginSizeKey].isUInt(),
                    "BehaviorCountingAnimation.Constructor.TypeError",
                    "SlowLoopBeginSize is not a uint")) {
      _iConfig.slowLoopBeginSize_loops =
          config[JsonKeys::SlowLoopBeginSizeKey].asUInt();
    }
  }
  if (ANKI_VERIFY(
          config.isMember(JsonKeys::SlowLoopEndSizeKey),
          "BehaviorCountingAnimation.Constructor.MissingSlowLoopEndSize", "")) {
    if (ANKI_VERIFY(config[JsonKeys::SlowLoopEndSizeKey].isUInt(),
                    "BehaviorCountingAnimation.Constructor.TypeError",
                    "SlowLoopEndSize is not a uint")) {
      _iConfig.slowLoopEndSize_loops =
          config[JsonKeys::SlowLoopEndSizeKey].asUInt();
    }
  }
}
...
bool BehaviorCountingAnimation::WantsToBeActivatedBehavior() const {
  ...
  if (!_dVars.setTargetCalled) {
    LOG_WARNING(
        "BehaviorCountingAnimation.WantsToBeActivated.SetCountTargetNotCalled",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorCountingAnimation::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorCountingAnimation::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {JsonKeys::SlowLoopBeginSizeKey,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorCountingAnimation::SetCountTarget(
    uint32_t target, const std::string& announcement) {
  ...
  LOG_INFO("BehaviorCountingAnimation.SetCountTarget.SetCountTarget",
           "target set to %d with announcement %s", _dVars.target,
           _dVars.announcement.c_str());
}
...
void BehaviorCountingAnimation::SetCountParameters() {
  ...
  if (!_dVars.setTargetCalled) {
    LOG_WARNING(
        "BehaviorCountingAnimation.SetCountParameters.SetCountTargetNotCalled",
        "SetCountTarget must be called before activating. Target defaults to "
        "0. Probably a mistake.");
  }
  // check console vars
  ...
  // set these up for the minimum case: zero. (these are also the defaults, but
  // just to be explicit...
  _dVars.even = false;  // yes, I know 0 is actually even. But in this case we
                        // want to count as little as possible.
  ...
  if (remaining == 0) {
    ...
    LOG_INFO("BehaviorCountingAnimation.SetCountParameters.settings",
             ...
             _dVars.target, _dVars.even, _dVars.slowLoopBeginNumLoops,
             _dVars.fastLoopNumLoops, _dVars.slowLoopEndNumLoops);
    return;
  }
  ...
  DEV_ASSERT(
      remaining % 2 == 0,
      "BehaviorCountingAnimation.SetCountParameters.RemainingShouldBeEven");
  // beginning slow loop
  if (remaining < _iConfig.slowLoopBeginSize_loops * 2) {
    _dVars.slowLoopBeginNumLoops =
        remaining / 2;  // divide by 2 because the loop has two counts
    remaining = 0;
  ...
  remaining -= _iConfig.slowLoopBeginSize_loops *
               2;  // multiply by 2 because the loop has two counts
  // assert that remaining is even (sanity check)
  DEV_ASSERT(
      remaining % 2 == 0,
      "BehaviorCountingAnimation.SetCountParameters.RemainingShouldBeEven");
  ...
  remaining -= _iConfig.slowLoopEndSize_loops *
               2;  // multiply by 2 because the loop has two counts
  // assert that remaining is even (sanity check)
  DEV_ASSERT(
      remaining % 2 == 0,
...
void BehaviorCountingAnimation::OnBehaviorActivated() {
  ...
  // get in
  // right now we're doing a get-in for zero, even though the smallest get-in
  // counts to 1. I'd like to fix that at some point, but it requires changes to
  // the animations
  IAction* getIn = new TriggerLiftSafeAnimationAction(
      _dVars.even ? AnimationTrigger::CountingGetInEven
                  : AnimationTrigger::CountingGetInOdd);
  animationSequence->AddAction(getIn);

  // slow loop begin
  if (_dVars.slowLoopBeginNumLoops > 0) {
    IAction* slowLoopBegin = new TriggerLiftSafeAnimationAction(
        AnimationTrigger::CountingSlowLoop, _dVars.slowLoopBeginNumLoops);
    animationSequence->AddAction(slowLoopBegin);
  }

  // fast loop
  if (_dVars.fastLoopNumLoops > 0) {
    // IAction* fastLoop = new
    // ReselectingLoopAnimationAction(AnimationTrigger::CountingFastLoop,
    //    _dVars.fastLoopNumLoops);
    IAction* fastLoop = new TriggerLiftSafeAnimationAction(
        AnimationTrigger::CountingFastLoop, _dVars.fastLoopNumLoops);
    animationSequence->AddAction(fastLoop);
  }

  // slow loop end
  if (_dVars.slowLoopEndNumLoops > 0) {
    IAction* slowLoopEnd = new TriggerLiftSafeAnimationAction(
        AnimationTrigger::CountingSlowLoop, _dVars.slowLoopEndNumLoops);
  ...
  if (_dVars.announcement == "") {
    ...
    getOut =
        new TriggerLiftSafeAnimationAction(AnimationTrigger::CountingGetOut);
  } else {
    auto* getOutSTA = new SayTextAction(_dVars.announcement);
    getOutSTA->SetAnimationTrigger(AnimationTrigger::CountingGetOut);
    getOut = getOutSTA;
    // note that doing it like this puts the potential for a slight delay (while
    // the TTS audio is prepared) before the get-out. The other option is to
    // wrap the whole sequence in the SayTextAction, which would put the delay
    // at the beginning of the whole thing Or we could manage all the TTS state
    // ourselves (using TextToSpeechCoordinator) directly. This seems like a
    // fine approach for the time being.
  }
...
void BehaviorCountingAnimation::OnBehaviorDeactivated() {
  ...
  // We do this here rather than on activation because we need to set the target
  // before activation.
