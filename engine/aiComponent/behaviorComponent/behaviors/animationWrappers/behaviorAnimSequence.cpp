...
BehaviorAnimSequence::BehaviorAnimSequence(const Json::Value& config)
  ...
  for (const auto& trigger : config[kAnimTriggerKey]) {
    _iConfig.animTriggers.push_back(
        AnimationTriggerFromString(trigger.asString()));
  }
  ...
  DEV_ASSERT_MSG(
      _iConfig.animTriggers.empty() != _iConfig.animationNames.empty(),
      "BehaviorAnimSequence.Ctor.InvalidConfig",
      "Behavior '%s' invalid config. Must specify either animations or "
  ...
  if (!config[kTracksToLock].isNull()) {
    for (const auto& trackStr : config[kTracksToLock]) {
      if (ANKI_VERIFY(trackStr.isString(),
                      "BehaviorAnimSequence.Ctor.TrackNotString",
                      "Must be array of track strings")) {
        ...
        if (ANKI_VERIFY(AnimTrackFlagFromString(trackStr.asString(), flag),
                        ...
                        "Track '%s' is not a track",
                        trackStr.asString().c_str())) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAnimSequence::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {
      kAnimTriggerKey, kAnimNamesKey, kLoopsKey, kTracksToLock, kRenderInEyeHue,
  };
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAnimSequence::OnBehaviorActivated() { StartPlayingAnimations(); }
...
void BehaviorAnimSequence::StartSequenceLoop() {
  ...
  if (_iConfig.numLoops == 0 || _dVars.sequenceLoopsDone < _iConfig.numLoops) {
    IActionRunner* action = GetAnimationAction();
...
IActionRunner* BehaviorAnimSequence::GetAnimationAction() {
  ...
  if (IsSequenceLoop()) {
    numLoops = 1;  // just one loop per animation, so we can loop the entire
                   // sequence together
  }
  ...
  for (AnimationTrigger trigger : _iConfig.animTriggers) {
    ReselectingLoopAnimationAction* playAnim =
        new ReselectingLoopAnimationAction(trigger, numLoops, interruptRunning,
                                           tracksToLock);
  ...
  for (const auto& name : _iConfig.animationNames) {
    PlayAnimationAction* playAnim =
        new PlayAnimationAction(name, numLoops, interruptRunning, tracksToLock);
    playAnim->SetRenderInEyeHue(_iConfig.renderInEyeHue);
    sequenceAction->AddAction(playAnim);
  }
