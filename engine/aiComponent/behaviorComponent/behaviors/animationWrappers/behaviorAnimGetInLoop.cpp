...
const char* kLoopIntervalKey =
    "loopInterval_s";  // If less than length of animation, will not interrupt
...
BehaviorAnimGetInLoop::BehaviorAnimGetInLoop(const Json::Value& config)
  ...
  auto loadTrigger = [&config](AnimationTrigger& trigger, const char* key) {
    ...
    trigger = AnimationTriggerFromString(
        JsonTools::ParseString(config, key, debugStr + key));
    ANKI_VERIFY(trigger != AnimationTrigger::Count,
                "BehaviorAnimGetInLoop.Constructor.InvalidAnimTrigger",
                "%s cannot be count", key);
  };
  ...
  if (config.isMember(kLoopEndConditionKey)) {
    _iConfig.endLoopCondition = BEIConditionFactory::CreateBEICondition(
        config[kLoopEndConditionKey], GetDebugLabel());
  ...
  if (config.isMember(kCheckEndCondKey)) {
    std::string debugStr =
        "BehaviorAnimGetInLoop.Constructor.CheckEndCondIssue";
    _iConfig.checkEndConditionDuringAnim =
        JsonTools::ParseBool(config, kCheckEndCondKey, debugStr);
  }

  if (config.isMember(kLoopIntervalKey)) {
    std::string debugStr = "BehaviorAnimGetInLoop.Constructor.LoopInterval";
    _iConfig.loopInterval_s =
        JsonTools::ParseFloat(config, kLoopIntervalKey, debugStr);
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAnimGetInLoop::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {
      kGetInAnimationKey,   kLoopAnimationKey,
      kGetOutAnimationKey,  kEmergencyGetOutAnimationKey,
      kLoopEndConditionKey, kCheckEndCondKey,
      kLoopIntervalKey,     kForceLoopGetOutAnimKey,
      kLockTreadsKey,
  };
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorAnimGetInLoop::WantsToBeActivatedBehavior() const { return true; }
...
void BehaviorAnimGetInLoop::BehaviorUpdate() {
  ...
  if (_iConfig.checkEndConditionDuringAnim &&
    ...
    _dVars.shouldLoopEnd |=
        _iConfig.endLoopCondition->AreConditionsMet(GetBEI());
  }
...
void BehaviorAnimGetInLoop::TransitionToGetIn() {
  _dVars.stage = BehaviorStage::GetIn;
  DelegateIfInControl(new TriggerAnimationAction(_iConfig.getInTrigger, 1, true,
...
void BehaviorAnimGetInLoop::TransitionToLoop() {
  ...
  DelegateIfInControl(new TriggerAnimationAction(_iConfig.loopTrigger, 1, true,
                                                 _iConfig.tracksToLock));
}
...
void BehaviorAnimGetInLoop::TransitionToGetOut() {
  ...
  DelegateIfInControl(new TriggerAnimationAction(_iConfig.getOutTrigger, 1,
