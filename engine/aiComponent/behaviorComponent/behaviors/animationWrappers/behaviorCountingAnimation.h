...
class BehaviorCountingAnimation : public ICozmoBehavior {
 public:
  ...
  // announcement is what Vector should say when he finishes counting. (Keep it
  // short.)
 ...
 private:
  struct InstanceConfig {
    ...
    uint32_t slowLoopBeginSize_loops;  // how many we want to count slow at the
                                       // beginning - expressed in loops (1 loop
                                       // = 2 counts)
    uint32_t
        slowLoopEndSize_loops;  // how many we want to count slow at the end -
                                // expressed in loops (1 loop = 2 counts)
