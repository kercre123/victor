...
// update: this interface is a moving target. now changed to VOLUME_1 through
// VOLUME_5... (once it's stabilized we could remove the older ones...)
const std::map<std::string, EVolumeLevel> kVolumeLevelMap{
    //{"mute", EVolumeLevel::MUTE}, // don't ever actually use "mute"
    {"minimum", EVolumeLevel::MIN},      {"min", EVolumeLevel::MIN},
    {"VOLUME_1", EVolumeLevel::MIN},     {"low", EVolumeLevel::MEDLOW},
    {"VOLUME_2", EVolumeLevel::MEDLOW},  {"medium", EVolumeLevel::MED},
    {"VOLUME_3", EVolumeLevel::MED},     {"high", EVolumeLevel::MEDHIGH},
    {"VOLUME_4", EVolumeLevel::MEDHIGH}, {"maximum", EVolumeLevel::MAX},
    {"max", EVolumeLevel::MAX},          {"VOLUME_5", EVolumeLevel::MAX}};
const std::map<EVolumeLevel, AnimationTrigger> kVolumeLevelAnimMap{
...
BehaviorVolume::DynamicVariables::DynamicVariables()
    : lastVolumeChangeNotification_s(0.0), volumeChangeReactorId(0) {}
...
bool BehaviorVolume::WantsToBeActivatedBehavior() const {
  ...
  const bool volumeLevelPending =
      uic.IsUserIntentPending(USER_INTENT(imperative_volumelevel));
  const bool volumeUpPending =
      uic.IsUserIntentPending(USER_INTENT(imperative_volumeup));
  const bool volumeDownPending =
      uic.IsUserIntentPending(USER_INTENT(imperative_volumedown));
  // we don't want to respond to an external notification if there's another
  // user intent pending
  const bool externalNotificationPending =
      (ExternalNotificationPending() && !uic.IsAnyUserIntentPending());

  return volumeLevelPending || volumeUpPending || volumeDownPending ||
         externalNotificationPending;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorVolume::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorVolume::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  /*
  const char* list[] = {
    // TODO: insert any possible root-level json keys that this class is
  expecting.
    // TODO: replace this method with a simple {} in the header if this class
  doesn't use the ctor's "config" argument.
  };
...
void BehaviorVolume::OnBehaviorActivated() {
  // to get the identity of the pending intent I basically have to do this check
  // again
  ...
  if (ExternalNotificationPending()) {
    // a little quirk: we don't use the volume from the volume change
    // notification. instead, we read it again from settings, to get the most up
    // to date volume (one less piece of state to keep around in a potentially
    // invalid state...)
    const uint32_t newVol = settings.GetRobotSettingAsUInt(
    ...
    if (uic.IsUserIntentPending(USER_INTENT(imperative_volumelevel))) {
      ...
      LOG_WARNING(
          "BehaviorVolume.OnBehaviorActivated.NoRecognizedPendingIntent",
          "No recognized pending intent for volume. (Or maybe an external "
          "notification expired.)");
      return;
    }

    const uint32_t oldVol = settings.GetRobotSettingAsUInt(
        external_interface::RobotSetting::master_volume);
    ...
    if (newVolumeLevel != desiredVolume) {
      LOG_WARNING("BehaviorVolume.OnBehavrioActivated.setFailed",
                  "New volume level %u does not match desired volume level %u",
                  newVolumeLevel, desiredVolume);
    }
    // issue DAS event
    ...
    // NOTE: we only do this for voice-sourced intents. DAS for app-sourced
    // happens in SettingsCommManager and we only need to do the animation
    // (below)
  ...
  if (it == kVolumeLevelAnimMap.end()) {
    // we don't have an anim designated for this volume level (probably because
    // it's MUTE)
  ...
  TriggerLiftSafeAnimationAction* animation =
      new TriggerLiftSafeAnimationAction(animTrigger);
...
void BehaviorVolume::OnBehaviorDeactivated() {
  // clear lastVolumeChangeNotification_s here! that will clear any pending
  // notifications
...
void BehaviorVolume::OnBehaviorEnteredActivatableScope() {
  ...
  _dVars.volumeChangeReactorId =
      settingsCommManager.RegisterVolumeChangeReactor(
...
void BehaviorVolume::OnBehaviorLeftActivatableScope() {
  ...
  SettingsCommManager& settingsCommManager = GetBEI().GetSettingsCommManager();
  settingsCommManager.UnRegisterVolumeChangeReactor(
      _dVars.volumeChangeReactorId);
}
...
bool BehaviorVolume::SetVolume(EVolumeLevel desiredVolumeEnum) {
  ...
  // should really be unnecessary now that we're using the enum, but let's be
  // safe
  if ((desiredVolumeEnum < EVolumeLevel::MIN) ||
      (desiredVolumeEnum > EVolumeLevel::MAX)) {
    LOG_WARNING("BehaviorVolume.SetVolume.OutsidePermittedRange",
                "Requested volume %u outside permitted range of [%u,%u].",
                desiredVolumeEnum, EVolumeLevel::MIN, EVolumeLevel::MAX);
    return false;
  }
  ...
  // cast the enum to uint32_t. Bonus! It's already a uint32_t, this is just to
  // be really explicit
  ...
  const bool success =
      settings.SetRobotSetting(external_interface::RobotSetting::master_volume,
                               desiredVolume, true, ignoredDueToNoChange);
  if (success) {
    LOG_INFO("BehaviorVolume.SetVolume.Success",
             "Successfully changed volume to %u.", desiredVolume);
  } else {
    LOG_INFO("BehaviorVolume.SetVolume.Failure", "Failed to change volume.");
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
EVolumeLevel BehaviorVolume::ComputeDesiredVolumeFromLevelIntent(
    UserIntentPtr intentData, bool& valid) {
  const std::string levelRequest =
      intentData->intent.Get_imperative_volumelevel().volume_level;
  ...
  if (it != kVolumeLevelMap.end()) {
    ...
    LOG_DEBUG("BehaviorVolume.ComputeDesiredVolumeFromLevelIntent.desiredVol",
              "mapped level request %s to desiredVol %u", levelRequest.c_str(),
              desiredVol);
  } else {
    // invalid level request
    valid = false;
    LOG_WARNING("BehaviorVolume.ComputeDesiredVolumeFromLevelIntent.invalid",
                "unexpected user intent volume_level: %s",
                levelRequest.c_str());
    // what should we return in this case? (since we don't want to throw an
    // exception) let's not return invalid data, just in case it gets used.
    // Rather, get the current volume and return that. A bit roundabout, but oh
    // well...
    SettingsManager& settings = GetBEI().GetSettingsManager();
    uint32_t vol = settings.GetRobotSettingAsUInt(
        external_interface::RobotSetting::master_volume);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
EVolumeLevel BehaviorVolume::ComputeDesiredVolumeFromIncrement(
    bool positiveIncrement) {
  // read volume from settings
  SettingsManager& settings = GetBEI().GetSettingsManager();
  uint32_t vol = settings.GetRobotSettingAsUInt(
      external_interface::RobotSetting::master_volume);
  if ((vol < static_cast<uint32_t>(EVolumeLevel::MIN)) ||
      (vol > static_cast<uint32_t>(EVolumeLevel::MAX))) {
    LOG_WARNING(
        "BehaviorVolume.ComputeDesiredVolumeFromIncrement."
        "unexpectedCurrentVolume",
        "Volume read from settings was outside expected range: %u", vol);
    if (vol < 1 && !positiveIncrement) {
      // we're about to try to decrement an unsigned zero, which would
      // underflow, which would be bad so let's not do that. Instead, since
      // we're already (unexpectedly) at MUTE, return MUTE
  ...
  // cast to EVolumeLevel. The enum is already a uint32_t, this is just to be
  // really explicit
...
void BehaviorVolume::OnVolumeChange() {
  _dVars.lastVolumeChangeNotification_s =
...
bool BehaviorVolume::ExternalNotificationPending() const {
  const float currentTime_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  return (_dVars.lastVolumeChangeNotification_s +
          kVolumeChangeNotificationAgeLimit_s) > currentTime_s;
}
