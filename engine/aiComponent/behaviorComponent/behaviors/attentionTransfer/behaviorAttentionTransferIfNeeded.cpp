...
BehaviorAttentionTransferIfNeeded::BehaviorAttentionTransferIfNeeded(
  ...
  JsonTools::GetCladEnumFromJSON(config, kReasonKey, _iConfig.reason,
                                 GetDebugLabel());
  ANKI_VERIFY(RecentOccurrenceTracker::ParseConfig(
                  config, _iConfig.numberOfTimes, _iConfig.amountOfSeconds),
  ...
  for (const auto& triggerJson : config[kAnimsIfNotRecentKey]) {
    ...
    if (AnimationTriggerFromString(triggerJson.asString(), anim)) {
      ...
      PRINT_NAMED_ERROR(
          "BehaviorAttentionTransferIfNeeded.AnimTriggers.Invalid",
          "Behavior '%s' config specified an invalid animation trigger: '%s'",
          GetDebugLabel().c_str(), triggerJson.asCString());
    }
  }
}
...
void BehaviorAttentionTransferIfNeeded::InitBehavior() {
  if (ANKI_VERIFY(
          _iConfig.reason != AttentionTransferReason::Invalid,
          "BehaviorAttentionTransferIfNeeded.Init.NoValidReason",
          "Behavior '%s' does not have a valid attention transfer reason",
          GetDebugLabel().c_str())) {
    _iConfig.recentOccurrenceHandle =
        GetBehaviorComp<AttentionTransferComponent>().GetRecentOccurrenceHandle(
            _iConfig.reason, _iConfig.numberOfTimes, _iConfig.amountOfSeconds);
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAttentionTransferIfNeeded::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kReasonKey, kAnimsIfNotRecentKey};
...
bool BehaviorAttentionTransferIfNeeded::WantsToBeActivatedBehavior() const {
  ...
  const bool featureEnabled = featureGate->IsFeatureEnabled(
      Anki::Vector::FeatureType::AttentionTransfer);
...
void BehaviorAttentionTransferIfNeeded::OnBehaviorActivated() {
  GetBehaviorComp<AttentionTransferComponent>().PossibleAttentionTransferNeeded(
      _iConfig.reason);
...
void BehaviorAttentionTransferIfNeeded::TransitionToAttentionTransfer() {
  DelegateIfInControl(
      new TriggerLiftSafeAnimationAction(AnimationTrigger::LookAtDeviceGetIn),
      [this]() {
        GetBehaviorComp<AttentionTransferComponent>().OnAttentionTransferred(
            _iConfig.reason);

        CompoundActionSequential* action = new CompoundActionSequential(
            {new TriggerLiftSafeAnimationAction(AnimationTrigger::LookAtDevice),
             new TriggerLiftSafeAnimationAction(
                 AnimationTrigger::LookAtDeviceGetOut)});
