 ...
 * Created: 2017-10-24
 *
 * Description: Behavior to search for and interact with faces in a "quiet" way
 *during the observing state
 *
...
static const float kDefaultSearchTime_s =
    -1.f;  // Negative values disable the timeout.
...
BehaviorObservingLookAtFaces::BehaviorObservingLookAtFaces(
    const Json::Value& config)
    : ICozmoBehavior(config) {
  const std::string& debugName = "Behavior" + GetDebugLabel() + ".LoadConfig";
  _iConfig.searchBehaviorStr =
      JsonTools::ParseString(config, kSearchBehaviorKey, debugName);

  // Parse the maximum allowed face-search time from the config if possible,
  // otherwise assign to the default value.
  if (JsonTools::GetValueOptional(config, kSearchTimeoutKey,
                                  _iConfig.searchTimeout_sec)) {
    const std::string& errorStr = debugName + ".InvalidSearchTimeout";
    DEV_ASSERT(Util::IsFltGTZero(_iConfig.searchTimeout_sec), errorStr.c_str());
    LOG_DEBUG(debugName.c_str(), "Search timeout set to %f seconds",
              _iConfig.searchTimeout_sec);
  } else {
    _iConfig.searchTimeout_sec = kDefaultSearchTime_s;
  }

  // Parse the staring time from the config if possible, otherwise assign to the
  // default value
  if (JsonTools::GetValueOptional(config, kStaringTimeKey,
                                  _iConfig.staringTime_sec)) {
    const std::string& errorStr = debugName + ".InvalidStaringTime";
    DEV_ASSERT(Util::IsFltGTZero(_iConfig.staringTime_sec), errorStr.c_str());
    LOG_DEBUG(debugName.c_str(), "Staring time set to %f seconds",
              _iConfig.staringTime_sec);
  } else {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorObservingLookAtFaces::GetBehaviorJsonKeys(
...
void BehaviorObservingLookAtFaces::GetAllDelegates(
...
void BehaviorObservingLookAtFaces::TransitionToFindFaces() {
  ...
  if (_iConfig.searchBehavior->WantsToBeActivated()) {
    ...
    LOG_WARNING(
        "BehaviorObservingLookAtFaces.FindFaces.DoesntWantToActivate",
...
void BehaviorObservingLookAtFaces::TransitionToStareAtFace(SmartFaceID face) {
  ...
  if (face.IsValid() && ShouldStareAtFace(face)) {
    // if the face is still valid and one we want to look at, go ahead and stare
    // for a bit

    LOG_INFO("BehaviorObservingLookAtFaces.StareAtFace",
             "Adding %s as a 'stared at' face", face.GetDebugStr().c_str());
    ...
    // Some variations of this behavior run in the user's palm, so we need to
    // allow the tracking action to run in the InAir tread state as well.
    trackAction->SetValidOffTreadsStates(
        {OffTreadsState::OnTreads, OffTreadsState::InAir});

    // If the track action component fails, the entire action should end.
    // Otherwise the robot will just sit there staring at a point in space that
    // may not correspond to the face's location any more until the wait action
    // expires.
    CompoundActionParallel* waitAndTrackAction =
        new CompoundActionParallel({waitAction, trackAction});
...
SmartFaceID BehaviorObservingLookAtFaces::GetFaceToStareAt() {
  // NOTE: because face ids can start out different and then become equal,
  // faceIdsLookedAt may have multiple entries that are now equal
  ...
  for (const auto& rawFaceID : faces) {
    ...
    if (ShouldStareAtFace(faceID)) {
      LOG_INFO("BehaviorObservingLookAtFaces.GetFaceToStareAt",
               "Face %s is a valid one (didn't match any of the %zu we've "
               "already seen)",
               faceID.GetDebugStr().c_str(), _dVars.faceIdsLookedAt.size());

      // TODO:(bn) track the nearest face? Or the furthest? For now make it
      // arbitrary
      return faceID;
    }
...
bool BehaviorObservingLookAtFaces::ShouldStareAtFace(
...
void BehaviorObservingLookAtFaces::SetState_internal(
    BehaviorObservingLookAtFaces::State state, const std::string& stateName) {
