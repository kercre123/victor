...
// add behavior _classes_ here if we should disable the prox-based "react to
// sudden obstacle" behavior while _any_ behavior of that class is running below
// us on the stack
static const std::set<BehaviorClass> kBehaviorClassesToSuppressProx = {
    {BEHAVIOR_CLASS(BlackJack), BEHAVIOR_CLASS(FistBump),
     BEHAVIOR_CLASS(FindCube), BEHAVIOR_CLASS(Keepaway),
     BEHAVIOR_CLASS(InspectCube), BEHAVIOR_CLASS(PickUpCube),
     BEHAVIOR_CLASS(PopAWheelie), BEHAVIOR_CLASS(PounceWithProx),
     BEHAVIOR_CLASS(RollBlock)}};

static const std::set<BehaviorClass> kBehaviorClassesToSuppressReactToSound = {
    {BEHAVIOR_CLASS(BlackJack), BEHAVIOR_CLASS(DanceToTheBeat),
     BEHAVIOR_CLASS(FetchCube), BEHAVIOR_CLASS(FistBump),
     BEHAVIOR_CLASS(Keepaway), BEHAVIOR_CLASS(ListenForBeats),
     BEHAVIOR_CLASS(InspectCube), BEHAVIOR_CLASS(PickUpCube),
     BEHAVIOR_CLASS(PopAWheelie), BEHAVIOR_CLASS(PounceWithProx),
     BEHAVIOR_CLASS(RollBlock), BEHAVIOR_CLASS(FindCubeAndThen)}};

static const std::set<BehaviorClass> kBehaviorClassesToSuppressTouch = {
    BEHAVIOR_CLASS(BlackJack)};

static const std::set<BehaviorClass> kBehaviorClassesToSuppressCliff = {
    BEHAVIOR_CLASS(BlackJack), BEHAVIOR_CLASS(FetchCube)};

static const std::set<BehaviorClass> kBehaviorClassesToSuppressTimerAntics = {
    {BEHAVIOR_CLASS(BlackJack), BEHAVIOR_CLASS(CoordinateWeather)}};

static const std::set<BehaviorID> kBehaviorIDsToSuppressWhenMeetVictor = {{
    BEHAVIOR_ID(ReactToTouchPetting),  // the user will often turn the robot to
                                       // face them and in the process touch it
    BEHAVIOR_ID(ReactToUnexpectedMovement),  // the user will often turn the
                                             // robot to face them
    BEHAVIOR_ID(ReactToSoundAwake),  // fully concentrate on what's in front
    BEHAVIOR_ID(ReactToDarkness)  // user hand near camera may trigger darkened
                                  // condition
}};
...
void BehaviorCoordinateGlobalInterrupts::InitPassThrough() {
  ...
  _iConfig.wakeWordBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(TriggerWordDetected));
  ...
  _iConfig.triggerWordPendingCond = BEIConditionFactory::CreateBEICondition(
      BEIConditionType::TriggerWordPending, GetDebugLabel());
  _iConfig.triggerWordPendingCond->Init(GetBEI());

  _iConfig.reactToObstacleBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(ReactToObstacle));
  _iConfig.meetVictorBehavior = BC.FindBehaviorByID(BEHAVIOR_ID(MeetVictor));
  _iConfig.danceToTheBeatBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(DanceToTheBeat));

  _iConfig.behaviorsThatShouldntReactToUnexpectedMovement.AddBehavior(
      BC, BEHAVIOR_CLASS(BumpObject));
  _iConfig.behaviorsThatShouldntReactToUnexpectedMovement.AddBehavior(
      BC, BEHAVIOR_CLASS(ClearChargerArea));
  _iConfig.behaviorsThatShouldntReactToUnexpectedMovement.AddBehavior(
      BC, BEHAVIOR_CLASS(ReactToHand));
  _iConfig.reactToUnexpectedMovementBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(ReactToUnexpectedMovement));

  _iConfig.reactToSoundAwakeBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(ReactToSoundAwake));
  for (const auto& behaviorClass : kBehaviorClassesToSuppressReactToSound) {
    _iConfig.behaviorsThatShouldntReactToSoundAwake.AddBehavior(BC,
                                                                behaviorClass);
  }

  _iConfig.reactToTouchPettingBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(ReactToTouchPetting));
  ...
  for (const auto& behaviorClass : kBehaviorClassesToSuppressTimerAntics) {
    _iConfig.behaviorsThatShouldSuppressTimerAntics.AddBehavior(BC,
                                                                behaviorClass);
  }

  _iConfig.reactToCliffBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(ReactToCliff));
  ...
  std::set<ICozmoBehaviorPtr> driveToFaceBehaviors =
      BC.FindBehaviorsByClass(BEHAVIOR_CLASS(DriveToFace));
...
void BehaviorCoordinateGlobalInterrupts::PassThroughUpdate() {
...

  {
    ...
    // certain intents do not want to turn vector after the wakeword was heard
    // so that they can go directly into their behavior facing the same
    // direction he was when the wakeword was heard.
    for (const UserIntentTag& tag : kUserIntentTagsToSuppressWakeWordTurn) {
      shouldSuppressTurn |= uic.IsUserIntentPending(tag);
    }

    // If we are responding to "take a photo", and the user is not requesting a
    // selfie Disable the react to voice command turn so that Victor takes the
    // photo in his current direction Exception: If storage is full we want to
    // turn towards the user to let them know
    UserIntent photoIntent;
    const bool isPhotoPending =
        uic.IsUserIntentPending(USER_INTENT(take_a_photo), photoIntent);
    if (isPhotoPending) {
      const auto& takeAPhoto = photoIntent.Get_take_a_photo();
      const bool isNotASelfie = takeAPhoto.empty_or_selfie.empty();
      const bool isStorageFull =
          GetBEI().GetPhotographyManager().IsPhotoStorageFull();
      shouldSuppressTurn |= (isNotASelfie && !isStorageFull);
    }

    if (shouldSuppressTurn) {
      const EngineTimeStamp_t ts =
          BaseStationTimer::getInstance()->GetCurrentTimeStamp();
      _iConfig.reactToVoiceCommandBehavior->DisableTurnForTimestamp(ts);
    }

    const bool isGoHomeActive =
        uic.IsUserIntentActive(USER_INTENT(system_charger));
    if (isGoHomeActive) {
  ...
  {
    if (_iConfig.behaviorsThatShouldntReactToUnexpectedMovement
            .AreBehaviorsActivated()) {
      _iConfig.reactToUnexpectedMovementBehavior->SetDontActivateThisTick(
          GetDebugLabel());
    }
  }
  ...
  {
    if (_iConfig.behaviorsThatShouldntReactToSoundAwake
            .AreBehaviorsActivated()) {
      _iConfig.reactToSoundAwakeBehavior->SetDontActivateThisTick(
          GetDebugLabel());
    }
  }
  ...
  // Suppress ReactToTouchPetting if needed
  {
    if (_iConfig.behaviorsThatShouldntReactToTouch.AreBehaviorsActivated()) {
      _iConfig.reactToTouchPettingBehavior->SetDontActivateThisTick(
          GetDebugLabel());
    }
  }
  ...
  // tell BehaviorDriveToFace whenever a cliff interruption behavior is active,
  // so that it knows when it is reasonable to
  // resume-i-mean-wants-to-be-activated-sorry-kevin
  {
    if (_iConfig.reactToCliffBehavior->IsActivated()) {
      for (const auto& driveToFaceBehavior : _iConfig.driveToFaceBehaviors) {
        driveToFaceBehavior->SetInterruptionEndTick(
...
bool BehaviorCoordinateGlobalInterrupts::ShouldSuppressProxReaction() {
  // scan through the stack below this behavior and return true if any behavior
  // is active which is listed in kBehaviorClassesToSuppressProx

  const auto& behaviorIterator = GetBehaviorComp<ActiveBehaviorIterator>();

  // If the behavior stack has changed this tick or last tick, then update,
  ...
  if (behaviorIterator.GetLastTickBehaviorStackChanged() + 1 >= currTick) {
    ...
    auto callback = [this](const ICozmoBehavior& behavior) {
      if (kBehaviorClassesToSuppressProx.find(behavior.GetClass()) !=
          kBehaviorClassesToSuppressProx.end()) {
...
void BehaviorCoordinateGlobalInterrupts::CreateConsoleVars() {
  // deque can contain non-copyable objects. its kept here to keep the header
  // cleaner
  ...
  if (!passThroughList.empty()) {
    ...
    for (const auto* delegate : globalInterruptions) {
      ...
      if (cozmoDelegate != nullptr) {
        ...
        auto pairIt = _iConfig.devActivatableOverrides.emplace(
            BC.FindBehaviorByID(id), true);
        std::string name = std::string{"Toggle_"} +
                           BehaviorTypesWrapper::BehaviorIDToString(id);
        vars.emplace_back(pairIt.first->second, name.c_str(),
                          "BehaviorCoordinateGlobalInterrupts", true);
      }
    }
  }
}
