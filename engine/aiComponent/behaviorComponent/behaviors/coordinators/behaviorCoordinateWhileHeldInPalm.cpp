...
CONSOLE_VAR_RANGED(u32, kMaxTimeForInitialHeldInPalmReaction_ms,
                   "HeldInPalm.Coordinator", 1000, 0, 5000);

namespace {
const BehaviorID kHeldInPalmDispatcher = BEHAVIOR_ID(HeldInPalmDispatcher);
const BehaviorID kInitialHeldInPalmReaction =
    BEHAVIOR_ID(InitialHeldInPalmReaction);
const char* const kBehaviorStatesToSuppressHeldInPalmReactionsKey =
    "suppressingBehaviors";
}  // namespace

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorCoordinateWhileHeldInPalm::InstanceConfig::InstanceConfig(
    const Json::Value& config, const std::string& debugName) {
  std::vector<std::string> tmpNames;
  ANKI_VERIFY(
      JsonTools::GetVectorOptional(
          config, kBehaviorStatesToSuppressHeldInPalmReactionsKey, tmpNames),
      (debugName + ".MissingVectorOfSuppressingBehaviorNames").c_str(), "");
  for (const auto& name : tmpNames) {
    behaviorStatesToSuppressHeldInPalmReactions.insert(
        BehaviorTypesWrapper::BehaviorIDFromString(name));
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorCoordinateWhileHeldInPalm::GetPassThroughJsonKeys(
...
void BehaviorCoordinateWhileHeldInPalm::InitPassThrough() {
  ...
  _iConfig.initialHeldInPalmReaction =
      BC.FindBehaviorByID(kInitialHeldInPalmReaction);
  {
    _iConfig.suppressHeldInPalmBehaviorSet =
        std::make_unique<AreBehaviorsActivatedHelper>(
            BC, _iConfig.behaviorStatesToSuppressHeldInPalmReactions);
  }
}
...
void BehaviorCoordinateWhileHeldInPalm::PassThroughUpdate() {
  ...
  const bool isHeldInPalmReactionPlaying =
      _iConfig.heldInPalmDispatcher->IsActivated();
  _dVars.persistent.hasInitialHIPReactionPlayed |=
      _iConfig.initialHeldInPalmReaction->IsActivated();

  // Block the activation of all "held-in-palm" behaviors if a supressing
  // behavior is already activated, or block only the initial get-in animation
  // reaction if the robot never left the palm of the user's hand while it was
  // being supressed.
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorCoordinateWhileHeldInPalm::
    SuppressHeldInPalmReactionsIfAppropriate() {
  const bool shouldSuppress =
      _iConfig.suppressHeldInPalmBehaviorSet->AreBehaviorsActivated();
...
void BehaviorCoordinateWhileHeldInPalm::
    SuppressInitialHeldInPalmReactionIfAppropriate() {
  const u32 currHeldInPalmDuration_ms =
      GetBEI().GetHeldInPalmTracker().GetHeldInPalmDuration_ms();
  const bool shouldSuppressInitialReaction =
      currHeldInPalmDuration_ms > kMaxTimeForInitialHeldInPalmReaction_ms;

  // Only play the initial "get-in palm" animation if the robot has recently
  // been put into a user's palm
  if (shouldSuppressInitialReaction ||
      _dVars.persistent.hasInitialHIPReactionPlayed) {
    _iConfig.initialHeldInPalmReaction->SetDontActivateThisTick(
        GetDebugLabel());
