...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorQuietModeCoordinator::BehaviorQuietModeCoordinator(
    const Json::Value& config)
    : ICozmoBehavior(config) {
  _iConfig.activeTime_s =
      JsonTools::ParseFloat(config, kActiveTimeKey, GetDebugLabel());
  _iConfig.timeToPowerSave_s =
      JsonTools::ParseFloat(config, kTimeToPowerSaveKey, GetDebugLabel());

  const auto& behaviors = config[kBehaviorsKey];
  if (behaviors.isArray()) {
    for (const auto& entry : behaviors) {
      const bool allowsAudio =
          JsonTools::ParseBool(entry, kAudioAllowedKey, GetDebugLabel());
      const std::string behaviorIDStr =
          JsonTools::ParseString(entry, kBehaviorKey, GetDebugLabel());
      BehaviorID behaviorID;
      const bool behaviorMatches =
          BehaviorTypesWrapper::BehaviorIDFromString(behaviorIDStr, behaviorID);
...
bool BehaviorQuietModeCoordinator::WantsToBeActivatedBehavior() const {
  // dont activate if the trigger word behavior is active, so that it can exit
  // before we respond to the user intent
  return !_iConfig.wakeWordBehavior->IsActivated();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorQuietModeCoordinator::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorQuietModeCoordinator::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorQuietModeCoordinator::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kBehaviorsKey, kBehaviorKey, kAudioAllowedKey,
                        kActiveTimeKey, kTimeToPowerSaveKey};
...
void BehaviorQuietModeCoordinator::OnBehaviorActivated() {
  ...
  bool carrying =
      GetBEI().GetRobotInfo().GetCarryingComponent().IsCarryingObject();
  if (carrying) {
...
void BehaviorQuietModeCoordinator::BehaviorUpdate() {
  ...
  if (uic.IsAnyUserIntentPending() &&
      !uic.IsUserIntentPending(USER_INTENT(unmatched_intent))) {
    // note that this means the trigger word was used (to break us out of the
    // quiet behavior) but that the stimulation was still fixed at 0. we might
    // consider issuing a fake "ReactToTriggerWord" emotion event here to
    // compensate note that this also doesn't send any wake word triggered DAS
    // events or help accumulate wake word stats!
    CancelSelf();
    return;
  }

  // wake word should disable timer
  if ((_iConfig.timerBehavior != nullptr) &&
      (_iConfig.wakeWordBehavior != nullptr) &&
      _iConfig.timerBehavior->IsTimerRinging()) {
  ...
  if (GetBEI().GetRobotInfo().GetHeadAccelMagnitudeFiltered() >
      kAccelMagnitudeShakingStartedThreshold) {
    CancelSelf();
    return;
  }

  if (timeActivated_s > _iConfig.timeToPowerSave_s &&
      !_dVars.requestedPowerSave) {
    ICozmoBehavior::SmartRequestPowerSaveMode();
    _dVars.requestedPowerSave = true;
  }

  // if we're here, quiet mode is stll active. go through the behavior list like
  // a DispatcherStrictPriority would
  for (const auto& entry : _iConfig.behaviors) {
    if (!entry.behavior->IsActivated() &&
        entry.behavior->WantsToBeActivated()) {
...
void BehaviorQuietModeCoordinator::SimmerDownNow() {
  ...
  for (const auto* obj : connectedCubes) {
    GetBEI().GetCubeLightComponent().PlayLightAnimByTrigger(
        obj->GetID(), CubeAnimationTrigger::SleepNoFade);
  }
