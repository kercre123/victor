...
namespace {
// The set of behaviors which maintain control of the robot, even when it's in
// the air
static const std::set<BehaviorID> kBehaviorStatesToSuppressInAirReaction = {
    {BEHAVIOR_ID(BlackJack), BEHAVIOR_ID(HeldInPalmDispatcher),
     BEHAVIOR_ID(ReactToTouchPetting), BEHAVIOR_ID(TakeAPhotoCoordinator),
     BEHAVIOR_ID(WeatherResponses), BEHAVIOR_ID(ShowWallTime),
     BEHAVIOR_ID(SingletonCancelTimer), BEHAVIOR_ID(SingletonTimerAlreadySet),
     BEHAVIOR_ID(SingletonAnticShowClock), BEHAVIOR_ID(SingletonTimerCheckTime),
     BEHAVIOR_ID(SingletonTimerSet)}};

static const std::set<BehaviorID> kBehaviorStatesToNotLockTracks = {
    {BEHAVIOR_ID(WhatsMyNameVoiceCommand), BEHAVIOR_ID(MeetVictor)}};
...
void BehaviorCoordinateWhileInAir::InitPassThrough() {
  ...
  _initialPickupReaction =
      BC.FindBehaviorByID(BEHAVIOR_ID(InitialPickupAnimation));
  ...
  _heldInPalmDispatcher =
      BC.FindBehaviorByID(BEHAVIOR_ID(HeldInPalmDispatcher));
  {
    _suppressInAirBehaviorSet = std::make_unique<AreBehaviorsActivatedHelper>(
        BC, kBehaviorStatesToSuppressInAirReaction);
    _dontLockTracksBehaviorSet = std::make_unique<AreBehaviorsActivatedHelper>(
        BC, kBehaviorStatesToNotLockTracks);
  }
}
...
void BehaviorCoordinateWhileInAir::PassThroughUpdate() {
  ...
  const OffTreadsState offTreadsState =
      GetBEI().GetRobotInfo().GetOffTreadsState();

  if (offTreadsState == OffTreadsState::OnTreads) {
    _lastTimeWasOnTreads_ms =
        BaseStationTimer::getInstance()->GetCurrentTimeStamp();
  }

  const bool isInAirReactionPlaying =
      _whileInAirDispatcher->IsActivated() || _onBackReaction->IsActivated() ||
      _onFaceReaction->IsActivated() || _heldInPalmDispatcher->IsActivated();

  // Only lock tracks if the "picked up reaction" behavior is _not_ running
  // (since the "react to picked up" animations make use of the treads).
  //
  // Same applies to onBack and onFace reactions, and the HeldInPalmDispatcher
  // as well. Since they can transition through the InAir state while their
  // reaction animations are playing or in between different behaviors of the
  // held-in-palm dispatcher, we don't want to lock tracks in the middle since
  // that can cause the wheels to keep moving based on the last
  // BodyMotionKeyFrame that went through before the lock.
...
void BehaviorCoordinateWhileInAir::LockTracksIfAppropriate() {
  const bool shouldLockTracks =
      !_dontLockTracksBehaviorSet->AreBehaviorsActivated() &&
      !GetBEI().GetHeldInPalmTracker().IsHeldInPalm();
  if (!_areTreadsLocked && shouldLockTracks) {
    SmartLockTracks(static_cast<u8>(AnimTrackFlag::BODY_TRACK), GetDebugLabel(),
                    GetDebugLabel());
...
void BehaviorCoordinateWhileInAir::SuppressInAirReactionIfAppropriate() {
  const bool shouldSuppress =
      _suppressInAirBehaviorSet->AreBehaviorsActivated();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorCoordinateWhileInAir::
    SuppressInitialPickupReactionIfAppropriate() {
  const EngineTimeStamp_t currTime_ms =
      BaseStationTimer::getInstance()->GetCurrentTimeStamp();
  _hasPickupReactionPlayed |=
      (currTime_ms >
       (_lastTimeWasOnTreads_ms + kMaxTimeForInitialPickupReaction_ms));
...
void BehaviorCoordinateWhileInAir::OnFirstPassThroughUpdate() {
  // Safety check - behaviors that are  in the list need to internally list that
  // they can be activated while in the air or response will not work as
  // expected.
  const auto& BC = GetBEI().GetBehaviorContainer();
  for (const auto& id : kBehaviorStatesToSuppressInAirReaction) {
    const auto behaviorPtr = BC.FindBehaviorByID(id);
    const BehaviorOperationModifiers& modifiers =
        behaviorPtr->GetBehaviorOperationModifiersPostInit();
    ANKI_VERIFY(
        modifiers.wantsToBeActivatedWhenOffTreads,
        ...
        "Behavior %s is listed as a behavior that suppresses the in air reaction, \
                but modifier says it can't run while in the air",
