...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorRespondToRenameFace::BehaviorRespondToRenameFace(
    const Json::Value& config)
    : ICozmoBehavior(config), _name(""), _faceID(Vision::UnknownFaceID) {
  SubscribeToTags({EngineToGameTag::RobotRenamedEnrolledFace});

  //  const std::string& animTriggerString =
  //  config.get(JsonKeys::AnimationTriggerKey,
  //  "MeetCozmoRenameFaceSayName").asString(); _animTrigger =
  //  AnimationTriggerFromString(animTriggerString.c_str());
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorRespondToRenameFace::GetBehaviorJsonKeys(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorRespondToRenameFace::HandleWhileInScopeButNotActivated(
...
bool BehaviorRespondToRenameFace::WantsToBeActivatedBehavior() const {
  // todo: this behavior shouldn't respond if the vision system didn't actually
  // handle it, which can happen if the wrong original name is used (maybe we
  // just get rid of that param?)
...
void BehaviorRespondToRenameFace::OnBehaviorActivated() {
  ...
  //  For some reason the following didn't work (action immediately completed)
  //  and I ran out of time figuring out why. I also tried simply turning
  //  towards last face pose with no luck.
  //
  //  const bool kSayName = true;
  //  TurnTowardsFaceAction* turnTowardsFace = new
  //  TurnTowardsLastFacePoseAction(_faceID, M_PI_F, kSayName);
  //
