...
class BehaviorLookAroundInPlace : public ICozmoBehavior {
 ...
 public:
  ...
  // todo: document. Is this behavior alway activatable, or we won't look around
  // in an area we already know everything?
  ...
  // a number which increases each time the state machine reaches the end. It is
  // never reset for the behavior
  unsigned int GetNumIterationsCompleted() const {
    return _numIterationsCompleted;
  }

 protected:
  virtual void GetBehaviorOperationModifiers(
      BehaviorOperationModifiers& modifiers) const override {
    modifiers.wantsToBeActivatedWhenCarryingObject =
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // - - - - - - - - - - - - - - - - - - - - State helpers accessible to derived
  // classes
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // - - - - - - - - - - - - - - - - - - - -

  // request the proper action given the parameters so that the robot moves its
  // head. Animation team suggested to also do a small body move
  IAction* CreateHeadTurnAction(
      float bodyRelativeMin_deg,
      float bodyRelativeMax_deg,  // to provide mini-turns with head moves
                                  // (animation team suggests)
      float bodyReference_deg,  // angle the relative ones use as referance (to
                                // turn absolute)
      float headAbsoluteMin_deg,
      float headAbsoluteMax_deg,  // head min/max range absolute
  ...
  void SetInitialBodyDirection(const Radians& rads) {
    _initialBodyDirection = rads;
  }
  ...
  struct Configuration {
    ...
    bool behavior_ShouldResetTurnDirection;  // if true, a new clock direction
                                             // is picked every run, otherwise
                                             // picked once and locked
    bool behavior_ResetBodyFacingOnStart;  // whether or not to reset the "cone"
                                           // when the robot is put down
    bool behavior_ShouldLowerLift;
    float behavior_AngleOfFocus_deg;
    uint8_t behavior_NumberOfScansBeforeStop;  // if 0 it will loop as long as
                                               // it's not kicked out
    ...
    // Locks treads during animations to prevent unwanted forward/backward
    // motion during pauses
    bool behavior_LockTreadsDuringWaitAnims;

    // instead of turn speeds, the user can optionally specify a motion profile
    // (must specify this XOR turn speeds above)
    ...
    AnimationTrigger
        s2_WaitAnimTrigger;  // animation that will play instead of the wait
    ...
    float s4_BodyAngleRelativeRangeMin_deg;  // this is relative with respect to
                                             // s4 start
    float s4_BodyAngleRelativeRangeMax_deg;  // this is relative with respect to
                                             // s4 start
    ...
    AnimationTrigger
        s4_WaitAnimTrigger;  // animation that will play instead of the wait
 ...
 private:
  ...
  static inline float GetTurnSign(const EClockDirection direction) {
    return (direction == EClockDirection::CW) ? -1.0f : 1.0f;
  }
  ...
  static inline EClockDirection GetOpposite(const EClockDirection direction) {
    return (direction == EClockDirection::CW) ? EClockDirection::CCW
                                              : EClockDirection::CW;
  }
  ...
  IAction* CreatePauseAction(const AnimationTrigger& trigger,
                             const float waitMin_sec, const float waitMax_sec);

  // request the proper action given the parameters so that the robot turns and
  // moves head
  IAction* CreateBodyAndHeadTurnAction(
      EClockDirection clockDirection,  // body turn clock direction
      float bodyStartRelativeMin_deg,
      float bodyStartRelativeMax_deg,  // body min/max range relative to
                                       // starting angle (from original state)
      float headAbsoluteMin_deg,
      float headAbsoluteMax_deg,  // head min/max range absolute
      float bodyTurnSpeed_degPerSec,
      float headTurnSpeed_degPerSec);  // turn speeds
  ...
  // number of times we have bounced off a cone side (requires cone of focus). 2
  // means an entire cone was done, 4 twice, ...
  uint8_t _coneSidesReached;

  // count the total number of iterations through the state machine. This number
  // will only increase (it never gets reset)
  unsigned int _numIterationsCompleted = 0;

  // the robot completes a turn in this direction, but in between angles, it
  // moves back and forth and it moves the head up and down
  EClockDirection _mainTurnDirection;

  // s4 specific vars
  Radians
      _s4_s5StartingBodyFacing_rad;  // starting facing for s4 (because angle
                                     // changes are relative to this angle)
