...
BehaviorPounceWithProx::BehaviorPounceWithProx(const Json::Value& config)
  ...
  _iConfig.inRangeCondition = BEIConditionFactory::CreateBEICondition(
      config["wantsToBeActivatedCondition"], GetDebugLabel());
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorPounceWithProx::WantsToBeActivatedBehavior() const { return true; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPounceWithProx::GetAllDelegates(
      ...
      case PounceState::WaitForMotion: {
        ...
        if (_dVars.motionObserved ||
          ...
          DelegateIfInControl(new CompoundActionParallel(
              {new MoveHeadToAngleAction(
                   MoveHeadToAngleAction::Preset::GROUND_PLANE_VISIBLE),
...
void BehaviorPounceWithProx::TransitionToResultAnim() {
  ...
  if (caught) {
    newAction =
        new TriggerLiftSafeAnimationAction(AnimationTrigger::PounceSuccess);
    PRINT_CH_INFO("Behaviors", "BehaviorPounceOnMotion.CheckResult.Caught",
                  "got it!");
  } else {
    newAction =
        new TriggerLiftSafeAnimationAction(AnimationTrigger::PounceFail);
    PRINT_CH_INFO("Behaviors", "BehaviorPounceOnMotion.CheckResult.Miss",
                  "missed...");
  }

  DelegateIfInControl(
      newAction, [this]() { _dVars.pounceState = PounceState::WaitForMotion; });
}
...
bool BehaviorPounceWithProx::IsFingerCaught() {
  ...
  float robotBodyAngleDelta =
      robotInfo.GetPitchAngle().ToFloat() - _dVars.prePouncePitch;

  // check the lift angle, after some time, transition state
  PRINT_CH_INFO("Behaviors", "BehaviorPounceOnMotion.CheckResult",
                "lift: %f body: %fdeg (%frad) (%f -> %f)",
                robotInfo.GetLiftHeight(), RAD_TO_DEG(robotBodyAngleDelta),
                robotBodyAngleDelta, RAD_TO_DEG(_dVars.prePouncePitch),
                robotInfo.GetPitchAngle().getDegrees());
  return robotInfo.GetLiftHeight() > liftHeightThresh ||
         robotBodyAngleDelta > bodyAngleThresh;
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPounceWithProx::HandleWhileActivated(
  ...
  switch (event.GetData().GetTag()) {
    case MessageEngineToGameTag::RobotObservedMotion: {
      ...
      const bool inGroundPlane =
          motionObserved.ground_area > _iConfig.minGroundAreaForPounce;
      ...
      if ((_dVars.pounceState == PounceState::WaitForMotion) && inGroundPlane) {
        float dist =
            std::sqrt(std::pow(robotOffsetX, 2) + std::pow(robotOffsetY, 2));
