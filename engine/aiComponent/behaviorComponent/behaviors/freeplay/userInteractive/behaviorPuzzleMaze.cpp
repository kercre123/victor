...
CONSOLE_VAR_RANGED(float, kPuzzleTimeout_sec, CONSOLE_GROUP, 24000.f, 0.f,
                   24000.f);
...
BehaviorPuzzleMaze::InstanceConfig::InstanceConfig(const Json::Value& config) {
  ...
  JsonTools::GetValueOptional(config, kTimeBetweenTilesKey,
                              timeBetweenMazeSteps_Sec);
  JsonTools::GetValueOptional(config, kTimePauseAtIntersectionMinKey,
                              timePauseAtIntersectionMin_Sec);
  JsonTools::GetValueOptional(config, kTimePauseAtIntersectionMaxKey,
                              timePauseAtIntersectionMax_Sec);
  JsonTools::GetValueOptional(config, kChanceRandomWrongChoiceKey,
                              chanceWrongTurn);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPuzzleMaze::GetBehaviorJsonKeys(
...
void BehaviorPuzzleMaze::TransitionToState(const MazeState& state) {
  if (_dVars.state != state) {
    LOG_TRACE("BehaviorPuzzleMaze.TransitionToState", "%s to %s",
              EnumToString(_dVars.state), EnumToString(state));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPuzzleMaze::StartAnimation(
    const AnimationTrigger& animationTrigger, const MazeState& nextState) {
  LOG_TRACE(
      "BehaviorPuzzleMaze.StartAnimation", "Start animation %s, nextState %s",
  ...
  DEV_ASSERT(!IsControlDelegated(),
             "BehaviorPuzzleMaze.StartAnimation.ShouldBeInControl");

  auto callback = [this, nextState, animationTrigger] {
    LOG_TRACE("BehaviorPuzzleMaze.StartAnimation.Callback",
              "Finish animation %s, nextState %s",
              AnimationTriggerToString(animationTrigger),
              EnumToString(nextState));
    TransitionToState(nextState);
  };
  IActionRunner* action = new TriggerAnimationAction(animationTrigger);
  ANKI_VERIFY(DelegateIfInControl(action, callback),
              "BehaviorPuzzleMaze.StartAnimation.ShouldNotBeInControl", "");
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
MazeWalls BehaviorPuzzleMaze::GetNextDir(const MazeWalls& currCell,
                                         const MazeWalls& currDir,
                                         Point2i& outMoveDir,
                                         bool rotateOrder) {
  // priorities order
  std::vector<MazeWalls> orderCheck;
  if (currDir == MazeWalls::North) {
    orderCheck.insert(orderCheck.begin(), {MazeWalls::East, MazeWalls::North,
                                           MazeWalls::West, MazeWalls::South});
  } else if (currDir == MazeWalls::West) {
    orderCheck.insert(orderCheck.begin(), {MazeWalls::North, MazeWalls::West,
                                           MazeWalls::South, MazeWalls::East});
  } else if (currDir == MazeWalls::South) {
    orderCheck.insert(orderCheck.begin(), {MazeWalls::West, MazeWalls::South,
                                           MazeWalls::East, MazeWalls::North});
  } else if (currDir == MazeWalls::East) {
    orderCheck.insert(orderCheck.begin(), {MazeWalls::South, MazeWalls::East,
                                           MazeWalls::North, MazeWalls::West});
  }

  // Shuffle the order randomly, so the pattern isn't so obvious, basically go
  // left first instead of right.
  if (rotateOrder) {
  ...
  PRINT_NAMED_WARNING("BehaviorPuzzleMaze.GetNextDir.BlockedCell",
                      "No cell exit found, Puzzle is malformed.");
  return currDir;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Check if behavior can run at this
// time
bool BehaviorPuzzleMaze::WantsToBeActivatedBehavior() const { return true; }
...
void BehaviorPuzzleMaze::SingleStepMaze() {
  const auto& currMaze = GetAIComp<PuzzleComponent>().GetCurrentMaze();
  // This function is called once we've reached the end of an animation, so
  ...
  if (!(_dVars.currPos == currMaze._end)) {
    ...
    MazeWalls currcell =
        (MazeWalls)currMaze._maze[_dVars.currPos.y()][_dVars.currPos.x()];
    ...
    if (_dVars.avatarState == AvatarState::MovingBetweenTiles && numExits > 2) {
      ...
      float thinkingTime = GetBEI().GetRNG().RandDblInRange(
          _iConfig.timePauseAtIntersectionMin_Sec,
          _iConfig.timePauseAtIntersectionMax_Sec);
      _dVars.nextStep_Sec =
          BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() +
          thinkingTime;
      // Even when doing our balancing and we skip animating we want this to
      // simulate as if we had animated.
      _dVars.totalTimeInLastPuzzle_Sec += thinkingTime;

      LOG_TRACE("BehaviorPuzzleMaze.SingleStepMaze.ToThinking",
                "NumExits:%d x: %d y: %d, wait: %.2f", numExits,
                _dVars.currPos.x(), _dVars.currPos.y(), thinkingTime);
    } else {
      _dVars.avatarState = AvatarState::MovingBetweenTiles;
      _dVars.nextStep_Sec =
          BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() +
          _iConfig.timeBetweenMazeSteps_Sec;
      // Even when doing our balancing and we skip animating we want this to
      // simulate as if we had animated.
      ...
      Point2i moveDir;
      _dVars.currFacing =
          GetNextDir(currcell, _dVars.currFacing, moveDir, makeWrongTurn);
      _dVars.nextPos = _dVars.currPos + moveDir;
      _dVars.path.emplace_back(_dVars.currPos);
      LOG_TRACE("BehaviorPuzzleMaze.SingleStepMaze.ToMoving",
                "at %d, %d, facing %d", _dVars.currPos.x(), _dVars.currPos.y(),
                (int)_dVars.currFacing);
    }
  } else {
...
void BehaviorPuzzleMaze::UpdateMaze() {
  ...
  if ((_dVars.nextStep_Sec <
           BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() ||
       !_iConfig.animateBetweenTiles) &&
      !_dVars.isMazeSolved) {
    SingleStepMaze();
  }
}
...
void BehaviorPuzzleMaze::DrawMaze(Vision::Image& image) {
  ...
  float t = 1.f - ((_dVars.nextStep_Sec - currTime) /
                   _iConfig.timeBetweenMazeSteps_Sec);

  float lerpedPosX =
      _dVars.currPos.x() + t * (_dVars.nextPos.x() - _dVars.currPos.x());
  float lerpedPosY =
      _dVars.currPos.y() + t * (_dVars.nextPos.y() - _dVars.currPos.y());
  ...
  for (int y = 0; y < h; ++y) {
    for (int x = 0; x < w; ++x) {
      ...
      if ((wall & MazeWalls::South) != 0) {
        Rectangle<f32> rect(
            (_iConfig.tileSize_px * x) + offsetX,
      ...
      if ((wall & MazeWalls::East) != 0) {
        Rectangle<f32> rect(
            (_iConfig.tileSize_px * (x + 1)) - _iConfig.wallSize_px + offsetX,
            (_iConfig.tileSize_px * y) + offsetY, _iConfig.wallSize_px,
            _iConfig.tileSize_px);
        image.DrawFilledRect(rect, NamedColors::WHITE);
      }
    }
  }
...
void BehaviorPuzzleMaze::DrawCozmo(Vision::Image& image) {
  // Cozmo is basically just always center screen dot. forcing world around him
  // to scroll.
  // TODO: Loop an animation when in _dVars.avatarState =
  // AvatarState::ThinkingAnim
  image.DrawFilledCircle(
      Point2f(FACE_DISPLAY_WIDTH / 2 + _iConfig.tileSize_px / 2,
              FACE_DISPLAY_HEIGHT / 2 + _iConfig.tileSize_px / 2),
...
void BehaviorPuzzleMaze::UpdateDisplay() {
  ...
  if (!IsControlDelegated() && _iConfig.animateBetweenTiles) {
    ...
    GetBEI().GetAnimationComponent().DisplayFaceImage(
        image, AnimationComponent::DEFAULT_STREAMING_FACE_DURATION_MS, true);
  }
}
...
void BehaviorPuzzleMaze::BehaviorUpdate() {
  ...
  if (GetActivatedDuration() > kPuzzleTimeout_sec) {
    LOG_WARNING("BehaviorPuzzleMaze.UpdateIntern_Legacy",
                "Behavior has timed out");
    CancelSelf();
    return;
  }
  ...
  switch (_dVars.state) {
    case MazeState::Init: {
      ...
      DEV_ASSERT(_dVars.state != MazeState::Init,
                 "BehaviorPuzzleMaze.Update.InvalidState");
      break;
    }
    case MazeState::GetIn: {
    ...
    case MazeState::GetOut: {
      if (!IsControlDelegated()) {
        StartAnimation(AnimationTrigger::CubePounceGetOutBored,
                       MazeState::Complete);
      }
      break;
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorPuzzleMaze::IsPuzzleCompleted() { return _dVars.isMazeSolved; }
