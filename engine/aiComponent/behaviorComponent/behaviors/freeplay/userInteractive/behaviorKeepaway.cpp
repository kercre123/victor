 ...
 * Date:   02/26/2018
 *
 * Description: Victor attempts to pounce on an object while the user tries to
 *dodge the pounce
 *
...
namespace {
static const DrivingAnimationHandler::DrivingAnimations kKeepawayDrivingAnims{
    AnimationTrigger::CubePounceDriveGetIn,
...
// Distance past nominalPounceDist that we allow victor to creep. Helps ensure
// he eventually pounces.
...
// Helper macro to grab the key with the same name as the param struct member
// from the Json and set it as a float
#define SET_FLOAT_HELPER(__name__)                                       \
  do {                                                                   \
    __name__ = JsonTools::ParseFloat(config, QUOTE(__name__),            \
                                     "BehaviorKeepaway.InstanceConfig"); \
    if (ANKI_DEVELOPER_CODE) {                                           \
      floatNames.emplace_back(QUOTE(__name__));                          \
    }                                                                    \
  } while (0)

  minPouncesForSoloPlay =
      JsonTools::ParseInt8(config, kMinPouncesForSoloPlayKey, kDebugName);
  maxPouncesForSoloPlay =
      JsonTools::ParseInt8(config, kMaxPouncesForSoloPlayKey, kDebugName);
  ...
  useProxForDistance =
      JsonTools::ParseBool(config, kUseProxForDistance, kDebugName);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorKeepaway::DynamicVariables::DynamicVariables(
    const InstanceConfig& iConfig)
...
BehaviorKeepaway::BehaviorKeepaway(const Json::Value& config)
    : ICozmoBehavior(config), _iConfig(config), _dVars(_iConfig) {
  // Add configurable params as ConsoleVars
  MakeMemberTunable(_iConfig.minPouncesForSoloPlay, "minPouncesForSoloPlay",
                    kDebugName);
  MakeMemberTunable(_iConfig.maxPouncesForSoloPlay, "maxPouncesForSoloPlay",
                    kDebugName);
  MakeMemberTunable(_iConfig.targetUnmovedGameEndTimeout_s,
                    "targetUnmovedGameEndTimeout_s", kDebugName);
  MakeMemberTunable(_iConfig.noVisibleTargetGameEndTimeout_s,
                    "noVisibleTargetGameEndTimeout_s", kDebugName);
  MakeMemberTunable(_iConfig.outOfPlayGameEndTimeout_s,
                    "outOfPlayGameEndTimeout_s", kDebugName);
  MakeMemberTunable(_iConfig.targetVisibleTimeout_s, "targetVisibleTimeout_s",
                    kDebugName);
  MakeMemberTunable(_iConfig.globalOffsetDist_mm, "globalOffsetDist_mm",
                    kDebugName);
  ...
  MakeMemberTunable(_iConfig.outOfPlayDistance_mm, "outOfPlayDistance_mm",
                    kDebugName);
  MakeMemberTunable(_iConfig.allowablePointingError_deg,
                    "allowablePointingError_deg", kDebugName);
  MakeMemberTunable(_iConfig.targetUnmovedCreepTimeout_s,
                    "targetUnmovedCreepTimeout_s", kDebugName);
  ...
  MakeMemberTunable(_iConfig.pounceChanceIncrement, "pounceChanceIncrement",
                    kDebugName);
  MakeMemberTunable(_iConfig.nominalPounceDistance_mm,
                    "nominalPounceDistance_mm", kDebugName);
  MakeMemberTunable(_iConfig.mousetrapPounceDistance_mm,
                    "mousetrapPounceDistance_mm", kDebugName);
  MakeMemberTunable(_iConfig.probBackupInsteadOfMousetrap,
                    "probBackupInsteadOfMousetrap", kDebugName);
  MakeMemberTunable(_iConfig.pounceSuccessPitchDiff_deg,
                    "pounceSuccessPitchDiff_deg", kDebugName);
  MakeMemberTunable(_iConfig.excitementIncPerHit, "excitementIncPerHit",
                    kDebugName);
  MakeMemberTunable(_iConfig.maxProbExitExcited, "maxProbExitExcited",
                    kDebugName);
  MakeMemberTunable(_iConfig.frustrationIncPerMiss, "frustrationIncPerMiss",
                    kDebugName);
  MakeMemberTunable(_iConfig.maxProbExitFrustrated, "maxProbExitFrustrated",
                    kDebugName);
  ...
  MakeMemberTunable(_iConfig.probReactIncrement, "probReactIncrement",
                    kDebugName);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorKeepaway::GetBehaviorJsonKeys(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorKeepaway::WantsToBeActivatedBehavior() const { return true; }
...
void BehaviorKeepaway::OnBehaviorActivated() {
  ...
  _dVars.pounceCountToExit = GetRNG().RandIntInRange(
      _iConfig.minPouncesForSoloPlay, _iConfig.maxPouncesForSoloPlay);
  ...
  robotInfo.GetDrivingAnimationHandler().PushDrivingAnimations(
      kKeepawayDrivingAnims, GetDebugLabel());
...
void BehaviorKeepaway::OnBehaviorDeactivated() {
  auto& robotInfo = GetBEI().GetRobotInfo();
  robotInfo.GetDrivingAnimationHandler().RemoveDrivingAnimations(
      GetDebugLabel());
}
...
void BehaviorKeepaway::UpdateTargetStatus() {
  ...
  _dVars.target.isVisible =
      (GetCurrentTimeInSeconds() - status.lastObservedTime_s) <
      _iConfig.targetVisibleTimeout_s;
  _dVars.target.isInPlay = status.distance_mm < (_iConfig.globalOffsetDist_mm +
                                                 _iConfig.outOfPlayDistance_mm);
  _dVars.target.isOffCenter =
      ABS(status.angleFromRobotFwd_deg) > _iConfig.allowablePointingError_deg;
  _dVars.target.isInMousetrapRange =
      status.distance_mm <
      (_iConfig.globalOffsetDist_mm + _iConfig.mousetrapPounceDistance_mm);

  // Handle pertinent state changes
  bool isNotMoving = (GetCurrentTimeInSeconds() - status.lastMovedTime_s) >
                     _iConfig.targetUnmovedCreepTimeout_s;
  bool isInPounceRange =
      status.distance_mm <
      (_iConfig.globalOffsetDist_mm + _iConfig.nominalPounceDistance_mm);

  // If the target just stopped moving
  if (!_dVars.target.isNotMoving && isNotMoving) {
    float creepDelay = GetRNG().RandDblInRange(_iConfig.creepDelayTimeMin_s,
                                               _iConfig.creepDelayTimeMax_s);
  ...
  if (!_dVars.target.isInPounceRange && isInPounceRange) {
    float pounceDelay = GetRNG().RandDblInRange(_iConfig.pounceDelayTimeMin_s,
                                                _iConfig.pounceDelayTimeMax_s);
...
void BehaviorKeepaway::CheckGetOutTimeOuts() {
  ...
  if (status.visibleRecently) {
    ...
    float timeSinceGameStart =
        GetCurrentTimeInSeconds() - _dVars.gameStartTime_s;

    // Don't do a movement timeout if the cube hasn't moved since starting. This
    // typically indicates that he is pouncing on a cube without an opponent
    if (timeSinceMoved > timeSinceGameStart) {
      // just keep playing until we exit from too much success, or someone
      // engages
      ...
      victorIsBored = true;
      PRINT_CH_INFO(kLogChannelName, "BehaviorKeepaway.GameEndTimeout",
                    "Victor got bored and quit because the cube didn't move "
                    "for %f seconds",
    ...
    if ((_iConfig.noVisibleTargetGameEndTimeout_s > 0.0f) &&
      ...
      victorIsBored = true;
      PRINT_CH_INFO(kLogChannelName, "BehaviorKeepaway.GameEndTimeout",
                    "Victor got bored and quit because his cube was hidden for "
                    "%f seconds",
                    _iConfig.noVisibleTargetGameEndTimeout_s);
    }
  }
  ...
  if (KeepawayState::Stalking == _dVars.state &&
    ...
    PRINT_CH_INFO(kLogChannelName, "BehaviorKeepaway.OutOfPlayTimeout",
                  "Victor got bored and quit because his cube was out of play "
                  "for %f seconds",
...
void BehaviorKeepaway::TransitionToGetIn() {
  ...
  DelegateIfInControl(
      new TriggerAnimationAction(AnimationTrigger::CubePounceGetIn),
...
void BehaviorKeepaway::TransitionToStalking() {
  ...
  // Recompute the next creep and pounce times here as the SOONEST we will
  // execute these actions creepTime will get overwritten from
  // UpdateTargetStatus() if the target moves
  float creepDelay = GetRNG().RandDblInRange(_iConfig.creepDelayTimeMin_s,
                                             _iConfig.creepDelayTimeMax_s);
  _dVars.creepTime = GetCurrentTimeInSeconds() + creepDelay;
  // pounceTime will get overwritten from UpdateTargetStatus() if the target
  // enters pounce range
  float pounceDelay = GetRNG().RandDblInRange(_iConfig.pounceDelayTimeMin_s,
                                              _iConfig.pounceDelayTimeMax_s);
  _dVars.pounceTime = GetCurrentTimeInSeconds() + pounceDelay;

  _dVars.outOfPlayExitTime_s =
      GetCurrentTimeInSeconds() + _iConfig.outOfPlayGameEndTimeout_s;
    ...
    switch (_dVars.pounceReadyState) {
      case PounceReadyState::Ready: {
        if (!_dVars.target.isInPlay) {
          // If the target goes outOfPlay but not outOfSight,
          // TransitionToUnready
          StopIdleAnimation();
          DelegateIfInControl(
              new TriggerAnimationAction(
                  AnimationTrigger::CubePounceGetUnready),
              [this]() {
                _dVars.pounceReadyState = PounceReadyState::Unready;
                _dVars.outOfPlayExitTime_s = GetCurrentTimeInSeconds() +
                                             _iConfig.outOfPlayGameEndTimeout_s;
                StartIdleAnimation();
              });
          ...
          if (GetRNG().RandDbl() < _iConfig.probBackupInsteadOfMousetrap) {
            ...
            DelegateIfInControl(new TriggerLiftSafeAnimationAction(
                                    AnimationTrigger::CubePounceBackup),
            ...
            if (GetRNG().RandDbl() < _dVars.pounceChance) {
              ...
              TransitionToFakeOut();
                   ...
                   (GetCurrentTimeInSeconds() > _dVars.pounceTime)) {
          if (GetRNG().RandDbl() < _dVars.pounceChance) {
            TransitionToPouncing();
          } else {
            TransitionToFakeOut();
                   ...
                   (GetCurrentTimeInSeconds() > _dVars.creepTime)) {
      ...
      case PounceReadyState::Unready: {
        if (_dVars.target.isInPlay) {
          ...
          _dVars.outOfPlayExitTime_s = 0.0f;
          DelegateIfInControl(
              new TriggerAnimationAction(AnimationTrigger::CubePounceGetReady),
              [this]() {
...
void BehaviorKeepaway::TransitionToCreeping() {
  ...
  float maxCreepDistance = status.distance_mm -
                           _iConfig.nominalPounceDistance_mm +
                           kCreepOverlapDist_mm;
  ...
  if (maxCreepDistance <= 0.0f) {
    ...
    creepDistance =
        GetRNG().RandDblInRange(_iConfig.creepDistanceMin_mm, maxCreepDistance);
  }

  DelegateIfInControl(new DriveStraightAction(creepDistance),
...
void BehaviorKeepaway::TransitionToPouncing() {
  ...
  GetBehaviorComp<RobotStatsTracker>().IncrementBehaviorStat(
      BehaviorStat::AttemptedPounceOnCube);

  _dVars.victorGotLastPoint = false;
  float startingPitch = GetBEI().GetRobotInfo().GetPitchAngle().getDegrees();
  _dVars.pounceSuccessPitch_deg =
      startingPitch + _iConfig.pounceSuccessPitchDiff_deg;
  _dVars.pounceChance = _iConfig.basePounceChance;
  AnimationTrigger pounceTrigger =
      _dVars.target.isInMousetrapRange
          ? AnimationTrigger::CubePouncePounceClose
          : AnimationTrigger::CubePouncePounceNormal;
  ++_dVars.pounceCount;
  DelegateIfInControl(new TriggerAnimationAction(pounceTrigger),
                      &BehaviorKeepaway::TransitionToReacting);
}
...
void BehaviorKeepaway::TransitionToFakeOut() {
  ...
  _dVars.pounceChance = Util::Clamp(
      (_dVars.pounceChance + _iConfig.pounceChanceIncrement), 0.0f, 1.0f);
  DelegateIfInControl(
      new TriggerAnimationAction(AnimationTrigger::CubePounceFake),
      &BehaviorKeepaway::TransitionToStalking);
}
...
void BehaviorKeepaway::TransitionToReacting() {
  ...
  if (_dVars.victorGotLastPoint) {
    GetBehaviorComp<RobotStatsTracker>().IncrementBehaviorStat(
        BehaviorStat::PounceOnCubeSuccess);

    float max = 1.0f + _iConfig.maxProbExitExcited;
    float newVal =
        _dVars.frustrationExcitementScale + _iConfig.excitementIncPerHit;
    ...
    PRINT_CH_INFO("Behaviors", "BehaviorKeepaway.Hit",
                  ...
                  _dVars.frustrationExcitementScale, probExit);
  } else {
    float min = 1.0f - _iConfig.maxProbExitFrustrated;
    float newVal =
        _dVars.frustrationExcitementScale - _iConfig.frustrationIncPerMiss;
  ...
  if (exitRoll < _iConfig.minProbToExit) {
    ...
    PRINT_CH_INFO("Behaviors", "BehaviorKeepaway.ClampedExitProbability",
                  "Exit roll of: %f clamped to: %f", exitRoll,
                  _iConfig.minProbToExit);
  }

  if (exitRoll < probExit) {
    PRINT_CH_INFO("Behaviors", "BehaviorKeepaway.EmotionalExit",
                  "Exiting keepaway due to excessive excitement/frustration. "
                  "prob: %f. rand: %f.",
                  probExit, exitRoll);

    AnimationTrigger winResponseAnim =
        _dVars.victorGotLastPoint ? AnimationTrigger::CubePounceWinSession
    ...
    if (_dVars.victorGotLastPoint) {
      if (GetRNG().RandDblInRange(_iConfig.minProbToReact, 1.0f) <
          _dVars.probReactToHit) {
        _dVars.probReactToHit = _iConfig.baseProbReact;
        DelegateIfInControl(
            new TriggerAnimationAction(AnimationTrigger::CubePounceWinHand),
            &BehaviorKeepaway::TransitionToStalking);
      } else {
        _dVars.probReactToHit =
            Anki::Util::Min(_dVars.probReactToHit + _iConfig.probReactIncrement,
      ...
      if (GetRNG().RandDblInRange(_iConfig.minProbToReact, 1.0f) <
          _dVars.probReactToMiss) {
        _dVars.probReactToMiss = _iConfig.baseProbReact;
        DelegateIfInControl(
            new TriggerAnimationAction(AnimationTrigger::CubePounceLoseHand),
            &BehaviorKeepaway::TransitionToStalking);
      } else {
        _dVars.probReactToMiss = Anki::Util::Min(
            _dVars.probReactToMiss + _iConfig.probReactIncrement,
            _iConfig.probReactMax);
        DelegateIfInControl(
            new TriggerAnimationAction(AnimationTrigger::CubePounceGetReady),
            &BehaviorKeepaway::TransitionToStalking);
...
void BehaviorKeepaway::TransitionToGetOutBored() {
  ...
  DelegateIfInControl(
      new TriggerAnimationAction(AnimationTrigger::CubePounceGetOutBored));
}
...
void BehaviorKeepaway::TransitionToSoloGetOut() {
  ...
  DelegateIfInControl(
      new TriggerAnimationAction(AnimationTrigger::CubePounceWinSession));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorKeepaway::PitchIndicatesPounceSuccess() const {
  if (GetBEI().GetRobotInfo().GetPitchAngle().getDegrees() >
      _dVars.pounceSuccessPitch_deg) {
    return true;
  }
  return false;
...
void BehaviorKeepaway::StartIdleAnimation() {
  AnimationTrigger idleAnimationTrigger =
      (PounceReadyState::Ready == _dVars.pounceReadyState)
  ...
  if (IsControlDelegated()) {
    PRINT_NAMED_ERROR("BehaviorKeepaway.InvalidStartIdleAnimationCall",
                      "Attempted to loop an idle animation while the behavior "
                      "had already delegated control");
