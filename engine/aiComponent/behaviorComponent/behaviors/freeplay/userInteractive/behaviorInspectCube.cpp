 ...
 * Created: 2018-04-03
 *
 * Description: This behavior serves as a stage for implementing user
 *interactive, cube driven behavior transitions.
 *
...
BehaviorInspectCube::InstanceConfig::InstanceConfig(const Json::Value& config) {
  JsonTools::GetValueOptional(config, kPlayWithCubeDelegateKey,
                              playWithCubeBehaviorIDString);
}
...
void BehaviorInspectCube::InitBehavior() {
  ...
  _iConfig.driveOffChargerBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(DriveOffChargerCube));

  if (!_iConfig.playWithCubeBehaviorIDString.empty()) {
    BehaviorID behaviorID;
    if (BehaviorTypesWrapper::BehaviorIDFromString(
            _iConfig.playWithCubeBehaviorIDString, behaviorID)) {
      _iConfig.playWithCubeBehavior = BC.FindBehaviorByID(behaviorID);
    } else {
      PRINT_NAMED_ERROR("BehaviorInspectCube.InvalidBehaviorID",
                        "%s is not a valid behaviorID, InspectCube will "
                        "self-cancel when it would have delegated",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorInspectCube::WantsToBeActivatedBehavior() const { return true; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorInspectCube::GetBehaviorOperationModifiers(
  ...
  // Interaction tracking is MUCH better when connected. Request a background
  // connection once we've decided the user is trying to engage with a cube
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorInspectCube::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorInspectCube::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kPlayWithCubeDelegateKey};
...
void BehaviorInspectCube::OnBehaviorActivated() {
  ...
  float lastSeenTime_s =
      GetBEI().GetCubeInteractionTracker().GetTargetStatus().lastObservedTime_s;
  float timeSinceSeen_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() -
      lastSeenTime_s;
...
void BehaviorInspectCube::BehaviorUpdate() {
  ...
  _currentTimeThisTick_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  TargetStatus targetStatus =
      GetBEI().GetCubeInteractionTracker().GetTargetStatus();
...
void BehaviorInspectCube::UpdateBoredomTimeouts(
  ...
  if ((_currentTimeThisTick_s - _dVars.behaviorStartTime_s) >
      kBoredGetOutTimeout_s) {
    PRINT_CH_INFO(kLogChannelName, "BehaviorInspectCube.BoredTimeout",
                  "We've been diddling about inspecing the cube too long with "
                  "no result. Victor got bored.");
...
void BehaviorInspectCube::TransitionToGetIn() {
  ...
  DelegateIfInControl(new TriggerLiftSafeAnimationAction(
                          AnimationTrigger::InvestigateHeldCubeGetIn),
                      [this]() {
                        const bool currOnCharger =
                            GetBEI().GetRobotInfo().IsOnChargerContacts();
                        const bool currOnChargerPlatform =
...
void BehaviorInspectCube::TransitionToDriveOffCharger() {
  ...
  if (_iConfig.driveOffChargerBehavior->WantsToBeActivated()) {
    ...
    PRINT_NAMED_ERROR(
        "BehaviorInspectCube.DriveOffChargerBehaviorNotActivatable", "");
  }
}
...
void BehaviorInspectCube::TransitionToSearching() {
  ...
  if (targetStatus.visibleRecently) {
    ...
    _dVars.searchEndTime_s =
        BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() +
        kSearchGetOutTimeout_s;
...
void BehaviorInspectCube::SearchLoopHelper() {
  ...
  if (nullptr != targetStatus.observableObject) {
    DelegateIfInControl(
        new SearchForNearbyObjectAction(targetStatus.observableObject->GetID()),
...
void BehaviorInspectCube::UpdateSearching(const TargetStatus& targetStatus) {
  if (targetStatus.visibleThisFrame) {
    TransitionToTracking();
  } else if (BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() >
             _dVars.searchEndTime_s) {
...
void BehaviorInspectCube::TransitionToTracking() {
  ...
  TrackObjectAction* trackAction = new TrackObjectAction(
      targetStatus.observableObject->GetID(), trackByType);
  ...
  trackAction->SetClampSmallAnglesPeriod(kMinTrackingClampPeriod_s,
                                         kMaxTrackingClampPeriod_s);

  auto* trackAnimAction = new TriggerLiftSafeAnimationAction(
      AnimationTrigger::InvestigateHeldCubeTrackingLoop);

  auto compoundAction =
      new CompoundActionParallel({trackAction, trackAnimAction});
...
void BehaviorInspectCube::UpdateTracking(const TargetStatus& targetStatus) {
  if (targetStatus.distance_mm <= kTriggerDistance_mm) {
    PRINT_CH_INFO(kLogChannelName, "BehaviorInspectCube.DistanceTriggerTripped",
                  "Cube came within %f mm of victor, while (we think) held by "
                  "the user. Triggering keepaway",
                  kTriggerDistance_mm);
  ...
  if (timeSinceSeen_s > kTrackingUnseenSearchTimeout_s) {
    TransitionToSearching();
    PRINT_CH_INFO(
        kLogChannelName, "BehaviorInspectCube.TrackingUnseenTimeout",
  ...
  if (!targetStatus.isHeld && (timeSinceHeld_s > kTrackingNotHeldPlayTime_s)) {
    ...
    PRINT_CH_INFO(kLogChannelName, "BehaviorInspectCube.GoPlayWithcube",
                  "CubeInteractionTracker Indicated that the cube had been set "
                  "down. Going to interact.");
  }
}
...
void BehaviorInspectCube::TransitionToPlayingWithCube() {
  ...
  CancelDelegates(false);
  if (nullptr == _iConfig.playWithCubeBehavior) {
    PRINT_CH_INFO(kLogChannelName, "BehaviorInspectCube.NoSpecifiedDelegate",
                  "Conditions to play with cube were met, but there was no "
                  "delegate specified for this state");
  ...
  if (_iConfig.playWithCubeBehavior->WantsToBeActivated()) {
    DelegateIfInControl(
        new TriggerLiftSafeAnimationAction(
            AnimationTrigger::InvestigateHeldCubeOnSetDown),
        [this]() {
          if (_iConfig.playWithCubeBehavior->WantsToBeActivated()) {
    ...
    PRINT_NAMED_ERROR("BehaviorInspectCube.DelegateDidNotWantToBeActivated",
                      "PlayWithCube delegate behavior implementation error. "
                      "InspectCube will self-cancel");
    // behavior should exit due to non-delegation
  }
}
...
void BehaviorInspectCube::TransitionToGetOutBored() {
  ...
  CancelDelegates(false);
  DelegateIfInControl(new TriggerLiftSafeAnimationAction(
      AnimationTrigger::InvestigateHeldCubeGetOutBored));
}
