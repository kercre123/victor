...
static const std::vector<f32> kLookForFaceAngleChanges_rad = {DEG_TO_RAD(-15),
                                                              DEG_TO_RAD(30)};
...
BehaviorFistBump::InstanceConfig::InstanceConfig(const Json::Value& config) {
  ...
  JsonTools::GetValueOptional(config, kMaxTimeToLookForFaceKey,
                              maxTimeToLookForFace_s);
  JsonTools::GetValueOptional(config, kAbortIfNoFaceFoundKey,
                              abortIfNoFaceFound);
  JsonTools::GetValueOptional(config, kUpdateLastCompletionTimeKey,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorFistBump::GetBehaviorJsonKeys(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorFistBump::WantsToBeActivatedBehavior() const { return true; }
...
void BehaviorFistBump::OnBehaviorActivated() {
  ...
  if (robotInfo.GetCarryingComponent().IsCarryingObject()) {
    ...
    DEV_ASSERT(!_iConfig.abortIfNoFaceFound,
               "BehaviorFistBump.InvalidConfig.NoTimeToLookButNeedsFace");
...
void BehaviorFistBump::BehaviorUpdate() {
  ...
  if (GetBEI().GetOffTreadsState() != OffTreadsState::OnTreads) {
    if (_dVars.lastTimeOffTreads_s == 0) {
      _dVars.lastTimeOffTreads_s = now;
               ...
               _dVars.lastTimeOffTreads_s + kMaxPickedupDurationBeforeExit_s) {
      CancelSelf();
      return;
    }
  } else {
  ...
  // WaitingForMotorsToSettle or WaitingForBump in which case we loop the idle
  // animation.
  switch (_dVars.state) {
  ...
  switch (_dVars.state) {
    ...
    case State::LookForFace: {
      ...
      TurnTowardsLastFacePoseAction* turnToFace =
          new TurnTowardsLastFacePoseAction();
      ...
      DelegateIfInControl(turnToFace, [this](ActionResult result) {
        if (result == ActionResult::NO_FACE) {
          _dVars.startLookingForFaceTime_s =
    ...
    case State::LookingForFace: {
      ...
      if (now >
          _dVars.startLookingForFaceTime_s + _iConfig.maxTimeToLookForFace_s) {
      ...
      Pose3d facePose;
      RobotTimeStamp_t lastObservedFaceTime =
          GetBEI().GetFaceWorld().GetLastObservedFace(facePose);
      if (lastObservedFaceTime > 0 &&
          (robotInfo.GetLastMsgTimestamp() - lastObservedFaceTime <
           kMaxTimeInPastToHaveObservedFace_ms)) {
      ...
      if (now > _dVars.nextGazeChangeTime_s) {
        PanAndTiltAction* ptAction = new PanAndTiltAction(
            kLookForFaceAngleChanges_rad[_dVars.nextGazeChangeIndex],
            kLookForFaceHeadAngle, false, true);
        DelegateIfInControl(ptAction);

        // Set next gaze change time
        _dVars.nextGazeChangeTime_s =
            now + Util::numeric_cast<float>(GetRNG().RandDblInRange(
                      kMinTimeBeforeGazeChange_s, kMaxTimeBeforeGazeChange_s));
        if (++_dVars.nextGazeChangeIndex >=
            kLookForFaceAngleChanges_rad.size()) {
    ...
    case State::RequestInitialFistBump: {
      DelegateIfInControl(
          new TriggerAnimationAction(AnimationTrigger::FistBumpRequestOnce),
          [this]() {
            GetBehaviorComp<RobotStatsTracker>().IncrementBehaviorStat(
                BehaviorStat::AttemptedFistBump);
          });
    ...
    case State::RequestingFistBump: {
      ...
      DelegateIfInControl(new TriggerAnimationAction(
          AnimationTrigger::FistBumpIdle, 1, true,
    ...
    case State::WaitingForMotorsToSettle: {
      ...
      if (!robotInfo.GetMoveComponent().IsLiftMoving() &&
        ...
        if (now - _dVars.waitStartTime_s > kMaxTimeForMotorSettling_s) {
          PRINT_NAMED_WARNING("BehaviorFistBump.Update.MotorSettleTimeTooLong",
                              "%f", now - _dVars.waitStartTime_s);
        }
      }
      break;
    }
    case State::WaitingForBump: {
      ...
      if (CheckForBump(robotInfo)) {
        ...
        DelegateIfInControl(
            new TriggerAnimationAction(AnimationTrigger::FistBumpSuccess));
      ...
      if (!IsControlDelegated()) {
        ...
        if (++_dVars.fistBumpRequestCnt < kMaxNumAttempts) {
          DelegateIfInControl(new TriggerAnimationAction(
              AnimationTrigger::FistBumpRequestRetry));
          _dVars.state = State::RequestingFistBump;
        } else {
          DelegateIfInControl(new TriggerAnimationAction(
              AnimationTrigger::FistBumpLeftHanging));
...
bool BehaviorFistBump::CheckForBump(const BEIRobotInfo& robotInfo) {
  bool liftBumped =
      std::fabsf(robotInfo.GetLiftAngle() - _dVars.liftWaitingAngle_rad) >
      kLiftAngleBumpThresh_radps;
  bool gyroBumped =
      std::fabsf(robotInfo.GetHeadGyroData().y) > kGyroYBumpThresh_radps;
  bool accelBumped =
      std::fabsf(robotInfo.GetHeadAccelData().x - _dVars.waitingAccelX_mmps2) >
      kAccelXBumpThresh_mmps2;
...
void BehaviorFistBump::ResetFistBumpTimer() const {
  GetBEI()
      ...
      .GetTimer(BehaviorTimerTypes::FistBump)
      .Reset();
}
