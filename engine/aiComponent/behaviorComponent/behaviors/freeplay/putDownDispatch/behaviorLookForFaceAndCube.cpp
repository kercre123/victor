...
const char* const kFace_headAngleAbsRangeMinKey =
    "face_headAngleAbsRangeMin_deg";
const char* const kFace_headAngleAbsRangeMaxKey =
    "face_headAngleAbsRangeMax_deg";
const char* const kFace_bodyAngleRelRangeMinKey =
    "face_bodyAngleRelRangeMin_deg";
const char* const kFace_bodyAngleRelRangeMaxKey =
    "face_bodyAngleRelRangeMax_deg";
const char* const kFace_sidePicksKey = "face_sidePicks";
const char* const kCube_headAngleAbsRangeMinKey =
    "cube_headAngleAbsRangeMin_rad";
const char* const kCube_headAngleAbsRangeMaxKey =
    "cube_headAngleAbsRangeMax_rad";
const char* const kCube_bodyAngleRelRangeMinKey =
    "cube_bodyAngleRelRangeMin_rad";
const char* const kCube_bodyAngleRelRangeMaxKey =
    "cube_bodyAngleRelRangeMax_rad";
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorLookForFaceAndCube::InstanceConfig::InstanceConfig(
  ...
  // shared
  bodyTurnSpeed_radPerSec =
      DEG_TO_RAD(ParseFloat(config, kBodyTurnSpeedKey, debugName));
  headTurnSpeed_radPerSec =
      DEG_TO_RAD(ParseFloat(config, kHeadTurnSpeedKey, debugName));

  // face states
  face_headAngleAbsRangeMin_rad =
      DEG_TO_RAD(ParseFloat(config, kFace_headAngleAbsRangeMinKey, debugName));
  face_headAngleAbsRangeMax_rad =
      DEG_TO_RAD(ParseFloat(config, kFace_headAngleAbsRangeMaxKey, debugName));
  face_bodyAngleRelRangeMin_rad =
      DEG_TO_RAD(ParseFloat(config, kFace_bodyAngleRelRangeMinKey, debugName));
  face_bodyAngleRelRangeMax_rad =
      DEG_TO_RAD(ParseFloat(config, kFace_bodyAngleRelRangeMaxKey, debugName));
  face_sidePicks = ParseUint8(config, kFace_sidePicksKey, debugName);
  // cube states
  cube_headAngleAbsRangeMin_rad =
      DEG_TO_RAD(ParseFloat(config, kCube_headAngleAbsRangeMinKey, debugName));
  cube_headAngleAbsRangeMax_rad =
      DEG_TO_RAD(ParseFloat(config, kCube_headAngleAbsRangeMaxKey, debugName));
  cube_bodyAngleRelRangeMin_rad =
      DEG_TO_RAD(ParseFloat(config, kCube_bodyAngleRelRangeMinKey, debugName));
  cube_bodyAngleRelRangeMax_rad =
      DEG_TO_RAD(ParseFloat(config, kCube_bodyAngleRelRangeMaxKey, debugName));
  ...
  stopBehaviorOnAnyFace =
      ParseBool(config, kStopBehaviorOnAnyFaceKey, debugName);
  stopBehaviorOnNamedFace =
      ParseBool(config, kStopBehaviorOnNamedFaceKey, debugName);

  // anim triggers
  std::string lookInPlaceAnimTriggerStr =
      ParseString(config, kLookInPlaceAnimTriggerKey, debugName);
  lookInPlaceAnimTrigger =
      lookInPlaceAnimTriggerStr.empty()
          ? AnimationTrigger::Count
          : AnimationTriggerFromString(lookInPlaceAnimTriggerStr.c_str());
  if (lookInPlaceAnimTrigger == AnimationTrigger::Count) {
    PRINT_NAMED_ERROR(
        "BehaviorLookForFaceAndCube.LoadConfig.Invalid.lookInPlaceAnimTrigger",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorLookForFaceAndCube::DynamicVariables::DynamicVariables(
    Radians&& startingRads) {
...
BehaviorLookForFaceAndCube::BehaviorLookForFaceAndCube(
    const Json::Value& config)
    : ICozmoBehavior(config), _iConfig(config), _dVars(Radians()) {
  if (_iConfig.verifySeenFaces || _iConfig.stopBehaviorOnAnyFace ||
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorLookForFaceAndCube::GetBehaviorJsonKeys(
...
bool BehaviorLookForFaceAndCube::WantsToBeActivatedBehavior() const {
  ...
  // otherwise, can run as long as it's not carrying a cube (potentially it
  // could, but may look weird), which is handled in base
  return true;
}
...
void BehaviorLookForFaceAndCube::OnBehaviorActivated() {
  PRINT_CH_INFO("Behaviors", (GetDebugLabel() + ".InitInternal").c_str(),
                "Starting to look for face at center");

  auto currRads = GetBEI()
                      .GetRobotInfo()
                      .GetPose()
                      .GetWithRespectToRoot()
                      .GetRotationAngle<'Z'>();
  ...
  // create head move action
  {
    IAction* centerLookForFace = CreateBodyAndHeadTurnAction(
        ...
        _dVars.startingBodyFacing_rad, _iConfig.face_headAngleAbsRangeMin_rad,
        _iConfig.face_headAngleAbsRangeMax_rad,
        _iConfig.bodyTurnSpeed_radPerSec, _iConfig.bodyTurnSpeed_radPerSec);
    initialActions->AddAction(centerLookForFace);
  }

  // look here
  DelegateIfInControl(initialActions,
                      &BehaviorLookForFaceAndCube::TransitionToS1_FaceOnLeft);
}
...
/**void BehaviorLookForFaceAndCube::ResumeInternal()
{
  ...
  if( _dVars.currentState == State::Done ) {
    PRINT_NAMED_ERROR("BehaviorLookForFaceAndCube.ResumeInternal.AlreadyDone",
"Behavior was done but it's trying to resume."); return RESULT_FAIL;
  }
...
void BehaviorLookForFaceAndCube::ResumeCurrentState() {
  switch (_dVars.currentState) {
    ...
    case State::S5Center: {
      // do not transition, we will resume, but we won't do anything, since
      // first tick will finish due to no actions
      break;
    }
    case State::Done: {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorLookForFaceAndCube::HandleWhileActivated(
  ...
  if (event.GetData().GetTag() == EngineToGameTag::RobotObservedFace) {
    if (!_iConfig.verifySeenFaces && !_iConfig.stopBehaviorOnAnyFace &&
      ...
      PRINT_NAMED_ERROR("BehaviorLookForFaceAndCube.HandleFace.InvalidConfig",
                        "We are handling a face event, but we shouldn't have "
                        "subscribed because we don't care");
      return;
    }
    ...
    if (_iConfig.verifySeenFaces) {
      if (!_dVars.isVerifyingFace) {
        const bool haveAlreadyVerifiedFace =
            (_dVars.verifiedFaces.find(msg.faceID) !=
             _dVars.verifiedFaces.end());
        const bool trackingOnlyFace = (msg.faceID < 0);

        if (trackingOnlyFace || !haveAlreadyVerifiedFace) {
          // if we have a tracking only face, we always want to turn to it so we
          // can verify it and get a real id out of it (hopefully). Otherwise,
          // turn towards it if we haven't already. After verifying, we'll stop
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorLookForFaceAndCube::StopBehaviorOnFaceIfNeeded(
    FaceID_t observedID) {
  const bool trackingOnlyFace = (observedID < 0);

  if (!trackingOnlyFace) {
    // here we want to stop on some faces (named or any recognizable). If we are
    // verifying faces _and_ want to stop, the CancelActionAndVerifyFace
    // function will handle that for us _after_ the verify, so don't need to
    // handle that here. We never kill the behavior on a tracking only face
    ...
    if (_iConfig.stopBehaviorOnAnyFace) {
      ...
      if (ANKI_VERIFY(
              facePtr != nullptr, "BehaviorLookForFaceAndCube.NullObservedFace",
              "Face '%d' observed but faceworld returns null", observedID)) {
        if (facePtr->HasName()) {
          PRINT_CH_INFO("Behaviors", (GetDebugLabel() + ".SawFace.End").c_str(),
                        "Stopping behavior because we saw (any) face id %d",
                        observedID);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorLookForFaceAndCube::CancelActionAndVerifyFace(
    FaceID_t observedFace) {
  PRINT_CH_INFO("Behaviors", (GetDebugLabel() + ".VerifyFace").c_str(),
                "Stopping current action to verify face %d", observedFace);
  ...
  if (isTrackingOnly) {
    action->AddAction(
        new WaitForImagesAction(kNumFramesToWaitForTrackingOnlyFace));
    // note that if this turns into a real face, it may trigger another "turn
    // towards" for that face ID
  } else {
    // don't bother waiting for any frames if we already have a name. Otherwise,
    // give it a few frames to give it a chance to realize that this may be a
    // named person. Note: it would be better to do this logic _after_ the turn
    // to action completes, because we may collect data while turning, but I'm
    // lazy
    auto* facePtr = GetBEI().GetFaceWorld().GetFace(observedFace);
    if (facePtr && !facePtr->HasName()) {
      action->AddAction(
          new WaitForImagesAction(kNumFramesToWaitForUnNamedFace));
    }
  }
...
void BehaviorLookForFaceAndCube::TransitionToS1_FaceOnLeft() {
  ...
  PRINT_CH_INFO("Behaviors", (GetDebugLabel() + ".S1FaceOnLeft").c_str(),
                "Looking for face to my left (%u out of %u)",
                _dVars.currentSidePicksDone, _iConfig.face_sidePicks);

  // create head move action
  IAction* leftLookForFace = CreateBodyAndHeadTurnAction(
      _iConfig.face_bodyAngleRelRangeMin_rad,
      _iConfig.face_bodyAngleRelRangeMax_rad, _dVars.startingBodyFacing_rad,
      _iConfig.face_headAngleAbsRangeMin_rad,
      _iConfig.face_headAngleAbsRangeMax_rad, _iConfig.bodyTurnSpeed_radPerSec,
  ...
  if (_dVars.currentSidePicksDone < _iConfig.face_sidePicks) {
    DelegateIfInControl(
        leftLookForFace,
        &BehaviorLookForFaceAndCube::TransitionToS1_FaceOnLeft);  // left again
  } else {
    _dVars.currentSidePicksDone = 0;  // reset for next state
    DelegateIfInControl(
        leftLookForFace,
        &BehaviorLookForFaceAndCube::TransitionToS2_FaceOnRight);  // go to
                                                                   // right
  }
}
...
void BehaviorLookForFaceAndCube::TransitionToS2_FaceOnRight() {
  ...
  PRINT_CH_INFO("Behaviors", (GetDebugLabel() + ".S2FaceOnRight").c_str(),
                "Looking for face to my right (%u out of %u)",
                _dVars.currentSidePicksDone, _iConfig.face_sidePicks);
  ...
  if (_dVars.currentSidePicksDone < _iConfig.face_sidePicks) {
    DelegateIfInControl(
        rightLookForFace,
        &BehaviorLookForFaceAndCube::TransitionToS2_FaceOnRight);  // right
                                                                   // again
  } else {
    DelegateIfInControl(
        rightLookForFace,
        &BehaviorLookForFaceAndCube::TransitionToS3_CubeOnRight);  // go on to
                                                                   // cubes
  }
}
...
void BehaviorLookForFaceAndCube::TransitionToS3_CubeOnRight() {
  ...
  PRINT_CH_INFO("Behaviors", (GetDebugLabel() + ".S3CubeOnRight").c_str(),
                "Looking for cube to my right (%u out of %u)",
                _dVars.currentSidePicksDone, _iConfig.cube_sidePicks);

  // create head move action
  IAction* rightLookForCube = CreateBodyAndHeadTurnAction(
      _iConfig.cube_bodyAngleRelRangeMin_rad,
      _iConfig.cube_bodyAngleRelRangeMax_rad, _dVars.startingBodyFacing_rad,
      _iConfig.cube_headAngleAbsRangeMin_rad,
      _iConfig.cube_headAngleAbsRangeMax_rad, _iConfig.bodyTurnSpeed_radPerSec,
      _iConfig.bodyTurnSpeed_radPerSec);
  ...
  if (_dVars.currentSidePicksDone < _iConfig.cube_sidePicks) {
    DelegateIfInControl(
        rightLookForCube,
        &BehaviorLookForFaceAndCube::TransitionToS3_CubeOnRight);  // right
                                                                   // again
  } else {
    _dVars.currentSidePicksDone = 0;  // reset for next state
    DelegateIfInControl(
        rightLookForCube,
        &BehaviorLookForFaceAndCube::TransitionToS4_CubeOnLeft);  // go on
  }
}
...
void BehaviorLookForFaceAndCube::TransitionToS4_CubeOnLeft() {
  ...
  PRINT_CH_INFO("Behaviors", (GetDebugLabel() + ".S4CubeOnLeft").c_str(),
                "Looking for cube to my left (%u out of %u)",
                _dVars.currentSidePicksDone, _iConfig.cube_sidePicks);
  ...
  IAction* leftLookForCube = CreateBodyAndHeadTurnAction(
      ...
      _dVars.startingBodyFacing_rad, _iConfig.cube_headAngleAbsRangeMin_rad,
      _iConfig.cube_headAngleAbsRangeMax_rad, _iConfig.bodyTurnSpeed_radPerSec,
      _iConfig.bodyTurnSpeed_radPerSec);
  ...
  if (_dVars.currentSidePicksDone < _iConfig.cube_sidePicks) {
    DelegateIfInControl(
        leftLookForCube,
        &BehaviorLookForFaceAndCube::TransitionToS4_CubeOnLeft);  // left again
  } else {
    _dVars.currentSidePicksDone = 0;  // reset for next state
    DelegateIfInControl(
        leftLookForCube,
        &BehaviorLookForFaceAndCube::TransitionToS5_Center);  // go on
  }
}
...
void BehaviorLookForFaceAndCube::TransitionToS5_Center() {
  ...
  IAction* centerLookForCube = CreateBodyAndHeadTurnAction(
      0, 0, _dVars.startingBodyFacing_rad,
      _iConfig.cube_headAngleAbsRangeMin_rad,
      _iConfig.cube_headAngleAbsRangeMax_rad, _iConfig.bodyTurnSpeed_radPerSec,
      _iConfig.bodyTurnSpeed_radPerSec);
  ...
  DelegateIfInControl(
      centerLookForCube,
      &BehaviorLookForFaceAndCube::TransitionToS6_Done);  // done after
}
...
void BehaviorLookForFaceAndCube::TransitionToS6_Done() {
  ...
  // Note that this is specific to the PutDownDispatch activity. If this
  // behavior needs to be generic for other activities, it should store the
  // start time on InitInternal and pass it here to compare timestamps. The
  // reason why I don't implement it that way is that I want to also to track
  // objects seen during putdown reactions, which would not be included in that
  // case. We could also json config it, but I don't have a reason for it atm
}
...
IAction* BehaviorLookForFaceAndCube::CreateBodyAndHeadTurnAction(
  ...
  const double bodyRelativeRandomAngle_rad = GetRNG().RandDblInRange(
      bodyRelativeMin_rad.ToDouble(), bodyRelativeMax_rad.ToDouble());
  const Radians bodyTargetAngleAbs_rad =
      bodyAbsoluteTargetAngle_rad + bodyRelativeRandomAngle_rad;

  // [min,max] range for random head angle turn
  const Radians headTargetAngleAbs_rad = GetRNG().RandDblInRange(
      headAbsoluteMin_rad.ToDouble(), headAbsoluteMax_rad.ToDouble());

  // create proper action for body & head turn
  PanAndTiltAction* turnAction = new PanAndTiltAction(
      bodyTargetAngleAbs_rad, headTargetAngleAbs_rad, true, true);
