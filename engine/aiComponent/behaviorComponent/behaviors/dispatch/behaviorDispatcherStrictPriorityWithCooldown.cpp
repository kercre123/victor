...
const char* kResetBehaviorTimerOnDispatcherActivationKey =
    "resetLinkedTimerOnDispatcherActivation";
}  // namespace
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorDispatcherStrictPriorityWithCooldown::DynamicVariables::
    DynamicVariables() {
...
BehaviorDispatcherStrictPriorityWithCooldown::
  ...
  _iConfig.resetCooldownOnDeactivation =
      config.get(kResetCooldownOnDeactivationKey, false).asBool();

  const Json::Value& behaviorArray = config[kBehaviorsKey];
  DEV_ASSERT_MSG(
      !behaviorArray.isNull(),
  ...
  if (!behaviorArray.isNull()) {
    for (const auto& behaviorDefinitionGroup : behaviorArray) {
      const std::string& behaviorIDStr =
          JsonTools::ParseString(behaviorDefinitionGroup, "behavior",
                                 "BehaviorDispatcherStrictPriorityWithCooldown."
                                 "BehaviorGroup.NoBehaviorID");

      IBehaviorDispatcher::AddPossibleDispatch(behaviorIDStr);

      _iConfig.cooldownInfo.emplace_back(
          BehaviorCooldownInfo{behaviorDefinitionGroup});

      BehaviorTimerTypes timerType = BehaviorTimerTypes::Invalid;
      bool resetTimerOnDispatchActivation = false;
      {
        std::string timerName =
            BehaviorTimerTypesToString(BehaviorTimerTypes::Invalid);
        if (JsonTools::GetValueOptional(behaviorDefinitionGroup,
                                        kLinkedBehaviorTimerKey, timerName)) {
          ANKI_VERIFY(BehaviorTimerTypesFromString(timerName, timerType),
                      "BehaviorDispatcherStrictPriorityWithCooldown."
                      "InvalidBehaviorTimer",
                      "%s is not recognized as a valid BehaviorTimer type!",
                      timerName.c_str());
          JsonTools::GetValueOptional(
              behaviorDefinitionGroup,
              kResetBehaviorTimerOnDispatcherActivationKey,
              resetTimerOnDispatchActivation);
        }
      }
      _iConfig.linkedBehaviorTimerInfo.emplace_back(
          timerType, resetTimerOnDispatchActivation);
    }
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDispatcherStrictPriorityWithCooldown::GetBehaviorJsonKeys(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDispatcherStrictPriorityWithCooldown::
    GetLinkedActivatableScopeBehaviors(std::set<IBehavior*>& delegates) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorDispatcherStrictPriorityWithCooldown::WantsToBeActivatedBehavior()
    const {
  if (!_iConfig.linkScope) {
  ...
  for (size_t idx = 0; idx < numBehaviors; ++idx) {
    ...
    const auto& behavior =
        IBehaviorDispatcher::GetAllPossibleDispatches().at(idx);

    if (!cooldownInfo.OnCooldown() && behavior->WantsToBeActivated()) {
      return true;
    }
  }
...
// - - - - - - - - - - - - - - - - - - - -
ICozmoBehaviorPtr
BehaviorDispatcherStrictPriorityWithCooldown::GetDesiredBehavior() {
  DEV_ASSERT_MSG(_iConfig.cooldownInfo.size() ==
  ...
  for (size_t idx = 0; idx < numBehaviors; ++idx) {
    ...
    if (!cooldownInfo.OnCooldown() &&
        (behavior->IsActivated() || behavior->WantsToBeActivated())) {
...
void BehaviorDispatcherStrictPriorityWithCooldown::DispatcherUpdate() {
  if (IsActivated() &&
    ...
    LOG_INFO(
        "BehaviorDispatcherStrictPriorityWithCooldown.LastBehaviorStopped."
        "StartCooldown",
        ...
        IBehaviorDispatcher::GetAllPossibleDispatches()
            [_dVars.lastDesiredBehaviorIdx]
                ->GetDebugLabel()
                .c_str());
    _iConfig.cooldownInfo[_dVars.lastDesiredBehaviorIdx].StartCooldown(
        GetRNG());

    // Reset the appropriate behavior timer if one was specified in the
    // configuration
    const auto& timerToReset =
        _iConfig.linkedBehaviorTimerInfo[_dVars.lastDesiredBehaviorIdx].first;
    if (timerToReset != BehaviorTimerTypes::Invalid) {
      LOG_INFO(
          "BehaviorDispatcherStrictPriorityWithCooldown.LastBehaviorStopped."
          "ResetBehaviorTimer",
          "Behavior idx %zu '%s' seems to have stopped, resetting behavior "
          "timer %s",
          _dVars.lastDesiredBehaviorIdx,
          IBehaviorDispatcher::GetAllPossibleDispatches()
              [_dVars.lastDesiredBehaviorIdx]
...
void BehaviorDispatcherStrictPriorityWithCooldown::
  ...
  // Some behaviors may have a linked behavior timer that they request to be
  // reset upon activation of the entire dispatcher. Iterate through the linked
  // timers and reset the ones specified by the configuration.
  ...
  for (const auto& linkedTimerInfo : _iConfig.linkedBehaviorTimerInfo) {
    ...
    if (timerToReset != BehaviorTimerTypes::Invalid) {
      const bool shouldResetTimerOnDispatcherActivation =
          linkedTimerInfo.second;
      if (shouldResetTimerOnDispatcherActivation) {
        LOG_INFO(
            "BehaviorDispatcherStrictPriorityWithCooldown.OnActivated."
            "ResetBehaviorTimer",
            "Behavior idx %zu '%s' requested a reset of behavior timer %s upon "
            "activation of this dispatcher",
            behaviorIdx,
            IBehaviorDispatcher::GetAllPossibleDispatches()[behaviorIdx]
                ->GetDebugLabel()
                .c_str(),
            BehaviorTimerTypesToString(timerToReset));
        GetBEI().GetBehaviorTimerManager().GetTimer(timerToReset).Reset();
...
void BehaviorDispatcherStrictPriorityWithCooldown::
  ...
  // If we are stopped while a behavior was active, put it on cooldown, and
  // reset its associated behavior timer (if one was specified).
  if (_dVars.lastDesiredBehaviorIdx < _iConfig.cooldownInfo.size()) {
    ...
    if (timerToReset != BehaviorTimerTypes::Invalid) {
      LOG_INFO(
          "BehaviorDispatcherStrictPriorityWithCooldown.OnDeactivated."
          "ResetBehaviorTimer",
