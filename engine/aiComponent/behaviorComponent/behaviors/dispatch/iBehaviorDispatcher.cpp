...
IBehaviorDispatcher::IBehaviorDispatcher(const Json::Value& config)
  ...
  _iConfig.shouldInterruptActiveBehavior =
      JsonTools::ParseBool(config, kInterruptBehaviorKey,
...
IBehaviorDispatcher::IBehaviorDispatcher(const Json::Value& config,
  ...
  // the config should either _not_ set the value at all, or it should be
  // consistent with what the base class passes in
  ANKI_VERIFY(
      config.get(kInterruptBehaviorKey, shouldInterruptActiveBehavior)
              .asBool() == shouldInterruptActiveBehavior,
      "IBehaviorDispatcher.ConfigMismatch",
      "Behavior '%s' specified a '%s' value of %s, but behaviors of this type "
      "will ignore this flag",
      GetDebugLabel().c_str(), kInterruptBehaviorKey,
      config.get(kInterruptBehaviorKey, false).asBool() ? "true" : "false");
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void IBehaviorDispatcher::GetBehaviorJsonKeys(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void IBehaviorDispatcher::GetBehaviorOperationModifiers(
  ...
  for (const auto& behaviorPtr : _iConfig.behaviors) {
    ...
    subBehaviorCarryingObject |=
        subModifiers.wantsToBeActivatedWhenCarryingObject;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void IBehaviorDispatcher::GetAllDelegates(
...
bool IBehaviorDispatcher::CanBeGentlyInterruptedNow() const {
  // it's a good time to interrupt if we aren't running a behavior, or if the
  // behavior we're running says it's OK
  if (!IsControlDelegated()) {
    return true;
  } else if (GetBEI().HasDelegationComponent()) {
    auto& delegationComponent = GetBEI().GetDelegationComponent();

    const IBehavior* behaviorDelegatedTo =
        delegationComponent.GetBehaviorDelegatedTo(this);

    // TODO:(bn) work around this ugly cast by caching a version in
    // ICozmoBehavior during the Delegate calls?
    DEV_ASSERT(dynamic_cast<const ICozmoBehavior*>(behaviorDelegatedTo),
               "IBehaviorDispatcher.DelegatedBehaviorNotACozmoBehavior");

    const ICozmoBehavior* delegate =
        static_cast<const ICozmoBehavior*>(behaviorDelegatedTo);
...
void IBehaviorDispatcher::BehaviorUpdate() {
  ...
  // only choose a new behavior if we should interrupt the active behavior, or
  // if no behavior is active
  if (!IsControlDelegated() || _iConfig.shouldInterruptActiveBehavior) {
    auto& delegationComponent = GetBEI().GetDelegationComponent();

    const IBehavior* currBehavior =
        delegationComponent.GetBehaviorDelegatedTo(this);
    ...
    if (desiredBehavior != nullptr &&
      ...
      DEV_ASSERT_MSG(delegated,
                     "IBehaviorDispatcher.BehaviorUpdate.DelegateFailed",
