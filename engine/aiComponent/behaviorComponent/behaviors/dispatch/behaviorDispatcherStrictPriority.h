 ...
 *behaviors and dispatches to the first on in that list that wants to be
 *activated
 *
...
class BehaviorDispatcherStrictPriority : public IBehaviorDispatcher {
 ...
 private:
  struct InstanceConfig {
    ...
    // if true, will CancelSelf after the FIRST behavior that WantsToBeActivated
    // returns control
    bool actAsSelector;
  };
  ...
  struct DynamicVariables {
    ...
    // Track whether this behavior has yet delegated, in case it is acting as a
    // selector.
    bool hasDelegated;
  };
