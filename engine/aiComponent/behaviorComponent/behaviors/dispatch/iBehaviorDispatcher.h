 ...
 * Created: 2017-10-24
 *
 * Description: common interface for dispatch-type behaviors. These are
 *behaviors that only dispatch to other behaviors
 *
...
class IBehaviorDispatcher : public ICozmoBehavior {
 protected:
  ...
  // second version of the constructor that allows child classes to force the
  // value of "interruptActiveBehavior". If not specified, the first constructor
  // will read it from config
  IBehaviorDispatcher(const Json::Value& config,
                      bool shouldInterruptActiveBehavior);

  // called during (child) constructors to add a behavior string to dispatch to.
  // First, anonymous behaviors will be searched, if there are no matches there,
  // behavior ID's will be checked. If those also don't match, this will print
  // an error (during Init)
  void AddPossibleDispatch(const std::string& behavior) {
    _iConfig.behaviorStrs.push_back(behavior);
  }

 public:
  virtual bool WantsToBeActivatedBehavior() const override;
  virtual void GetAllDelegates(
      std::set<IBehavior*>& delegates) const final override;
 ...
 protected:
  ...
  // This function should be overridden to return the behavior that should run.
  // This will only be called at "appropriate" times by the base class (the base
  // class handles whether or not to interrupt running behaviors)
  ...
  const std::vector<ICozmoBehaviorPtr>& GetAllPossibleDispatches() const {
    return _iConfig.behaviors;
  }
