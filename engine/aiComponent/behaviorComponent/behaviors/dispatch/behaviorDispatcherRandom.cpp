 ...
 * Created: 2017-10-31
 *
 * Description: Dispatcher which runs behaviors randomly based on weights and
 *cooldowns. Once the delegated-to behavior ends, this behavior will cancel
 *itself (this should probably be configurable - see VIC-4836)
 *
  ...
  if (!behaviorArray.isNull()) {
    for (const auto& behaviorDefinitionGroup : behaviorArray) {
      const std::string& behaviorIDStr = JsonTools::ParseString(
          behaviorDefinitionGroup, "behavior",
      ...
      const float weight = JsonTools::ParseFloat(
          behaviorDefinitionGroup, "weight",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDispatcherRandom::GetBehaviorJsonKeys(
...
ICozmoBehaviorPtr BehaviorDispatcherRandom::GetDesiredBehavior() {
  DEV_ASSERT_MSG(_iConfig.cooldownInfo.size() ==
                 ...
                 IBehaviorDispatcher::GetAllPossibleDispatches().size());
  DEV_ASSERT_MSG(_iConfig.weights.size() ==
                     IBehaviorDispatcher::GetAllPossibleDispatches().size(),
  ...
  if (!availableBehaviorIdxs.Empty()) {
    ...
    // no behaviors were available, maybe it was because of cooldown? Go through
    // again and find the one closest to being off cooldown and return that
    ...
    if (bestBehavior != nullptr) {
      PRINT_CH_INFO("Behaviors", "BehaviorDispatcherRandom.AllOnCooldown",
                    "%s: all behaviors on cooldown, so selecting '%s' which is "
                    "closest to being off cooldown (%f secs)",
                    GetDebugLabel().c_str(),
...
void BehaviorDispatcherRandom::DispatcherUpdate() {
  if (_dVars.shouldEndAfterBehavior && !IsControlDelegated() && IsActivated()) {
    CancelSelf();
  }
