...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorCooldownInfo::BehaviorCooldownInfo(float cooldownLength_s,
                                           float randomFactor)
    : _cooldown_s(cooldownLength_s), _randomCooldownFactor(randomFactor) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorCooldownInfo::BehaviorCooldownInfo(const Json::Value& config) {
  const float cooldown = JsonTools::ParseFloat(config, "cooldown_s",
                                               "BehaviorCooldownInfo.Cooldown");
  const float randomFactor =
      config.get("cooldown_random_factor", 0.0f).asFloat();

  _ignoreFastForward = config.get("ignoreFastForward", false).asBool();

  if (Anki::Util::IsFltLTZero(
          cooldown)) {  // a clearly negative cooldown ==> forever
...
void BehaviorCooldownInfo::StartCooldown(Util::RandomGenerator& rng) {
  if (_cooldown_s > 0.0f) {
    ...
    if (_randomCooldownFactor > 0.0f) {
      cooldown_s *= rng.RandDblInRange(
          Anki::Util::Max(0.0f, 1.0f - _randomCooldownFactor),
