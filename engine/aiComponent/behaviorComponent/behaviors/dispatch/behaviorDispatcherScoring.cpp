...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDispatcherScoring::GetBehaviorJsonKeys(
...
ICozmoBehaviorPtr BehaviorDispatcherScoring::GetDesiredBehavior() {
  ...
  if (ANKI_VERIFY(
          GetAllPossibleDispatches().size() == _iConfig.scoringTracker.size(),
    ...
    // Iterate through available behaviors, find the highest scored behavior
    // that wants to be activated, and activate it
    ...
    for (int i = 0; i < allDispatches.size(); i++) {
      ...
      if (entry->IsActivated() || entry->WantsToBeActivated()) {
        const float newScore =
            _iConfig.scoringTracker[i].EvaluateScore(GetBEI());
    ...
    if (highestScoreIdx != invalidIdx) {
      ...
      if (desiredDispatch != _dVars.currentDispatch) {
        ...
        _dVars.currentScoringTracker =
            &_iConfig.scoringTracker[highestScoreIdx];
