...
class BehaviorDispatcherPassThrough : public ICozmoBehavior {
 ...
 private:
  ...
  // NOTE(GB): This is a hack for pass-through dispatchers to safely check
  // modifiers of external behaviors that they are dependent on for some reason.
  // Doing this in the InitPassThrough() function is not robust due to the
  // fragileness of init order dependencies causing some behaviors to not have
  // their operation modifiers set up yet when the dispatcher is initialized.
