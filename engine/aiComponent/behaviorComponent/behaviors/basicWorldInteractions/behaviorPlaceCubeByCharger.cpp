...
BehaviorPlaceCubeByCharger::BehaviorPlaceCubeByCharger(
    ...
    : ICozmoBehavior(config), _iConfig() {}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPlaceCubeByCharger::GetBehaviorJsonKeys(
...
void BehaviorPlaceCubeByCharger::InitBehavior() {
  ...
  BC.FindBehaviorByIDAndDowncast<BehaviorPutDownBlockAtPose>(
      BEHAVIOR_ID(PutDownBlockAtPose), BEHAVIOR_CLASS(PutDownBlockAtPose),
      _iConfig.putCubeSomewhereBehavior);
  DEV_ASSERT(
      _iConfig.putCubeSomewhereBehavior != nullptr,
      "BehaviorPlaceCubeByCharger.InitBehavior.NullPutCubeSomewhereBehavior");

  _iConfig.turnToLastFaceBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(TurnToLastFace));
  DEV_ASSERT(
      nullptr != _iConfig.turnToLastFaceBehavior,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPlaceCubeByCharger::GetAllDelegates(
...
bool BehaviorPlaceCubeByCharger::WantsToBeActivatedBehavior() const {
  ...
  return (
      nullptr !=
      GetBEI().GetCubeInteractionTracker().GetTargetStatus().observableObject);
}
...
void BehaviorPlaceCubeByCharger::TransitionToConfirmCharger() {
  ...
  TurnTowardsObjectAction* visualCheckAction = new TurnTowardsObjectAction(
      _dVars.chargerID, Radians{M_PI_F}, visuallyVerify);
  ...
  if (RobotKnowsWhereChargerIs()) {
    DelegateIfInControl(
        new TriggerLiftSafeAnimationAction(
            AnimationTrigger::PlaceCubeByChargerReactToCharger),
...
void BehaviorPlaceCubeByCharger::TransitionToTakeCubeToCharger() {
  ...
  if (charger == nullptr) {
    LOG_ERROR(
        "BehaviorPlaceCubeByCharger.TransitionToTakeCubeToCharger.NullCharger",
  ...
  if (RobotKnowsWhereChargerIs()) {
    _iConfig.putCubeSomewhereBehavior->SetPutDownAnimTrigger(
        AnimationTrigger::PlaceCubeByChargerSuccess);
  } else {
    _iConfig.putCubeSomewhereBehavior->SetPutDownAnimTrigger(
        AnimationTrigger::PlaceCubeByChargerFail);
  }

  if (_iConfig.putCubeSomewhereBehavior->WantsToBeActivated()) {
    DelegateIfInControl(_iConfig.putCubeSomewhereBehavior.get(),
                        &BehaviorPlaceCubeByCharger::TransitionToGetOut);
  }
}
...
void BehaviorPlaceCubeByCharger::StartNextSearchForChargerTurn() {
  ...
  if (sweptEnough || exceededMaxTurns) {
    ...
    // stop the search and move on
    TransitionToReactToCharger();
    LOG_INFO(
        "BehaviorPlaceCubeByCharger.TransitionToSearchTurn."
        "CompletedQuickSearch",
