...
class BehaviorDriveOffCharger : public ICozmoBehavior {
 ...
 private:
  ...
  struct DynamicVariables {
    DynamicVariables();
    State state;
    unsigned int directionIndex;  // gets incremented either if a direction is
                                  // unknown or the drive attempt fails
  };

  InstanceConfig _iConfig;
  DynamicVariables _dVars;

  DriveDirection ParseDriveDirection(
  ...
  // Each of these is tried in the order supplied to _dVars.driveDirections. If
  // one does not delegate, or if it delegates but afterwards the robot is still
  // on the charger, the next one is tried. After all entries in
  // _dVars.driveDirections are attempted, we attempt
  // TransitionToDrivingStraightProcedural until it's off the charger
