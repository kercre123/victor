...
const float kSecondBumpBuffer_mm =
    60.0f;  // let the treads spin a bit longer. No worries, unexpected movement
            // is disabled
const float kSecondBumpEvilBuffer_mm = 150.0f;

const float kPauseDuration_s = 0.6f;

const float kPaddingPushedObject_mm =
    30.0f;  // distance extending from drive center where an object would fall
            // off a cliff
const float kCliffWidth_mm = 100.0f;

CONSOLE_VAR_RANGED(float, kExploringPostBumpReferenceProb, "BehaviorExploring",
                   1.0f, 0.0f, 1.0f);

static const DrivingAnimationHandler::DrivingAnimations kSlowDrivingAnimations{
    AnimationTrigger::Count, AnimationTrigger::PokeObjectDriveLoop,
    AnimationTrigger::Count};
static const DrivingAnimationHandler::DrivingAnimations kFastDrivingAnimations{
    AnimationTrigger::Count, AnimationTrigger::BumpObjectFastLoop,
    AnimationTrigger::Count};
...
BehaviorBumpObject::BehaviorBumpObject(const Json::Value& config)
  ...
  _iConfig.minDist_mm =
      JsonTools::ParseInt32(config, kMinDistKey, GetDebugLabel());
  _iConfig.maxDist_mm =
      std::numeric_limits<decltype(_iConfig.maxDist_mm)>::max();
  ...
  _iConfig.pBumpWhenNotEvil =
      config.get(kProbabilityBumpWhenNotEvilKey, 0.0f).asFloat();
  _iConfig.pBumpWhenEvil =
      config.get(kProbabilityBumpWhenEvilKey, 0.0f).asFloat();
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorBumpObject::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorBumpObject::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorBumpObject::GetBehaviorJsonKeys(
...
void BehaviorBumpObject::OnBehaviorActivated() {
  ...
  GetBEI().GetRobotInfo().GetDrivingAnimationHandler().PushDrivingAnimations(
      kSlowDrivingAnimations, GetDebugLabel());
...
void BehaviorBumpObject::DoFirstBump() {
  ...
  if (!proxData.foundObject) {
    PRINT_NAMED_WARNING(
        "BehaviorBumpObject.OnBehaviorActivated.InvalidProxReading",
  ...
  // animation
  action->AddAction(
      new TriggerLiftSafeAnimationAction{AnimationTrigger::PokeObjectGetIn});

  // bump
  const float distForward_mm =
      kFirstBumpBuffer_mm + static_cast<float>(proxData.distance_mm);
  const float speedForward_mmps = kFirstBumpSpeed_mmps;
  action->AddAction(
      new DriveStraightAction(distForward_mm, speedForward_mmps, true));

  // retreat animation
  action->AddAction(
      new TriggerLiftSafeAnimationAction{AnimationTrigger::PokeObjectGetOut});

  DelegateIfInControl(action, [this](const ActionResult& res) {
    GetBEI()
        .GetRobotInfo()
        .GetDrivingAnimationHandler()
        .RemoveDrivingAnimations(GetDebugLabel());
    GetBEI().GetRobotInfo().GetDrivingAnimationHandler().PushDrivingAnimations(
        kFastDrivingAnimations, GetDebugLabel());
...
void BehaviorBumpObject::DoSecondBumpIfDesired() {
  ...
  const auto& proxData = proxSensor.GetLatestProxData();
  const bool closeEnough = (proxData.distance_mm <= _iConfig.maxDist_mm);
  // don't bother checking if it's too close here, like in
  ...
  const float niceDistForward_mm =
      static_cast<float>(proxData.distance_mm) + kSecondBumpBuffer_mm;
  const float evilDistForward_mm =
      static_cast<float>(proxData.distance_mm) + kSecondBumpEvilBuffer_mm;

  // todo: inform this choice based on MoodManager::GetEmotionValue(
  // EmotionType::Evil );
  const bool evil = (GetRNG().RandDbl() <= _iConfig.pEvil);

  const bool shouldBump =
      (evil && (GetRNG().RandDbl() <= _iConfig.pBumpWhenEvil)) ||
  ...
  // if a regular bump would push something off a cliff and we're not evil, dont
  // do it at all. else if a LONG bump would push something off a cliff and we
  // ARE evil, use the longer distance. otherwise don't do it at all
  if (!evil) {
  ...
  // pause a bit
  // todo (VIC-4230) would be a good time to reference a human pose, with a
  ...
  DelegateIfInControl(action, [this](const ActionResult& res) {
    // retreat animation
    auto* retreat = new TriggerLiftSafeAnimationAction{
        AnimationTrigger::BumpObjectFastGetOut};
    DelegateIfInControl(retreat,
                        &BehaviorBumpObject::DoReferenceHumanIfDesired);
  });
}
...
void BehaviorBumpObject::DoReferenceHumanIfDesired() {
  ...
  const bool shouldActivate =
      (GetRNG().RandDbl() < kExploringPostBumpReferenceProb);
  if (refBehaviorWantsToBeActivated && shouldActivate) {
    _dVars.state = State::ReferenceHumanAfterBump;
    DelegateIfInControl(
        _iConfig.referenceHumanBehavior
            .get());  // end this behavior once the delegate finishes
  }
...
void BehaviorBumpObject::BehaviorUpdate() {
  if (IsActivated()) {
    ...
    _dVars.unexpectedMovement |=
        GetBEI().GetMovementComponent().IsUnexpectedMovementDetected();
...
void BehaviorBumpObject::OnBehaviorDeactivated() {
  GetBEI().GetRobotInfo().GetDrivingAnimationHandler().RemoveDrivingAnimations(
      GetDebugLabel());

  DASMSG(behavior_exploring_poked_object, "behavior.exploring.poke",
         "The robot intentionally poked something");
  DASMSG_SET(i1, 1 + (int)(_dVars.bumpedAgain >= 0), "Number of pokes");
  DASMSG_SET(i2, _dVars.bumpedAgain,
             "If there was a second poke, whether it was evil");
  DASMSG_SEND();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorBumpObject::WouldBumpPushSomethingOffCliff(
    float driveDist_mm) const {
  const auto& robotPose = GetBEI().GetRobotInfo().GetPose();
  const auto memoryMap = GetBEI().GetMapComponent().GetCurrentMemoryMap();
  RobotPointSamplerHelper::RejectIfWouldCrossCliff cliffDetector{
      kCliffWidth_mm};
  ...
  const Point2f pt =
      (robotPose.GetTransform() * Transform3d(rot, offset)).GetTranslation();
