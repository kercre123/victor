 ...
 * Created: 2018-08-20
 *
 * Description: Run BehaviorFindCube. Upon locating a cube, go to the pre-dock
 *pose if found/known, connect to the cube, (visiblly if this behavior was user
 *driven) then delegate to the followUp behavior, if specified.
 *
...
BehaviorFindCubeAndThen::InstanceConfig::InstanceConfig()
    : skipConnectToCubeBehavior(
          true)  // default to true and override for user driven instances
{}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorFindCubeAndThen::DynamicVariables::DynamicVariables()
    : state(FindCubeState::FindCube), cubeID() {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorFindCubeAndThen::BehaviorFindCubeAndThen(const Json::Value& config)
    : ICozmoBehavior(config) {
  JsonTools::GetValueOptional(config, kFollowUpBehaviorKey,
                              _iConfig.followUpBehaviorID);
  JsonTools::GetValueOptional(config, kSkipConnectToCubeBehaviorKey,
                              _iConfig.skipConnectToCubeBehavior);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorFindCubeAndThen::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorFindCubeAndThen::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorFindCubeAndThen::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kFollowUpBehaviorKey, kSkipConnectToCubeBehaviorKey};
...
void BehaviorFindCubeAndThen::InitBehavior() {
  ...
  _iConfig.connectToCubeBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(ConnectToCube));
...
void BehaviorFindCubeAndThen::TransitionToFindCube() {
  ...
  if (_iConfig.findCubeBehavior->WantsToBeActivated()) {
    DelegateIfInControl(_iConfig.findCubeBehavior.get(), [this]() {
      ...
      }  // else just exit, the get-out will have been handled by the
         // findCubeBehavior since it didn't find anything
    });
  }
}
...
void BehaviorFindCubeAndThen::TransitionToDriveToPredockPose() {
  ...
  DelegateIfInControl(new DriveToObjectAction(
                          _dVars.cubeID, PreActionPose::ActionType::DOCKING),
...
void BehaviorFindCubeAndThen::TransitionToAttemptConnection() {
  ...
  if (_iConfig.skipConnectToCubeBehavior) {
    ...
    if (_iConfig.connectToCubeBehavior->WantsToBeActivated()) {
      DelegateIfInControl(_iConfig.connectToCubeBehavior.get(), [this]() {
        auto& ccc = GetBEI().GetCubeConnectionCoordinator();
...
void BehaviorFindCubeAndThen::TransitionToFollowUpBehavior() {
  ...
  if (nullptr != _iConfig.followUpBehavior &&
      _iConfig.followUpBehavior->WantsToBeActivated()) {
...
void BehaviorFindCubeAndThen::TransitionToGetOutFailure() {
  ...
  DelegateIfInControl(
      new TriggerLiftSafeAnimationAction(AnimationTrigger::FetchCubeFailure));
}
