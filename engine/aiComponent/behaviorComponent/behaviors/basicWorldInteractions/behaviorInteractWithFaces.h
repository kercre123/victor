 ...
 * Created: 2016-08-22
 *
 * Description: Turn towards a face and "interact" with it, which for now just
 *means drive towards it and track it for a while.
 *
 * NOTE: This behavior shares a name with an older "interact with faces"
 *behavior, but has been re-written since then
 *
...
class BehaviorInteractWithFaces : public ICozmoBehavior {
 ...
 private:
  ...
  struct DynamicVariables {
    ...
    // We only want to run for faces we've seen since the last time we ran, so
    // keep track of the final timestamp when the behavior finishes
    RobotTimeStamp_t lastImageTimestampWhileRunning;
    // In the face tracking stage the action will hang, so store a time at which
    // we want to stop it (from within Update)
