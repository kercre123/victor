 ...
 * Created: 2016-05-19
 *
 * Description: Behavior to drive to off a charger and deal with being on/off
 *treads while on the charger. Supply a prioritized list of options to choose
 *the direction based on the most recent face, cube, or mic direction, or simply
 *drive randomly or straight. If a supplied direction is not applicable (e.g.,
 *no faces) or is unsuccessful, the next one is attempted.
 *
...
namespace {
// array of directions in {face, cube, straight, left, right, randomly,
// micDirection, proceduralStraight}
...
const float kExtraProceduralTimeout_s =
    2.5f;  // buffer to account for accel/decel and bumps

const AnimationTrigger kAnimFarLeft = AnimationTrigger::DriveOffChargerFarLeft;
const AnimationTrigger kAnimFarRight =
    AnimationTrigger::DriveOffChargerFarRight;
...
const AnimationTrigger kAnimStraight =
    AnimationTrigger::DriveOffChargerStraight;
const BehaviorID kRandomDriveBehavior =
    BEHAVIOR_ID(DriveOffChargerRandomlyAnim);
...
#define SET_STATE(s)                                                          \
  do {                                                                        \
    ...
    PRINT_CH_INFO("Behaviors", "BehaviorDriveOffCharger.State", "State = %s", \
                  #s);                                                        \
  } while (0);
...
BehaviorDriveOffCharger::BehaviorDriveOffCharger(const Json::Value& config)
  ...
  float extraDist_mm =
      config.get(kExtraDriveDistKey, kDefaultExtraDist_mm).asFloat();
  _iConfig.proceduralDistToDrive_mm = Charger::GetLength() + extraDist_mm;

  if (ANKI_VERIFY(config[kDriveDirectionCuesKey].isArray() &&
                      (config[kDriveDirectionCuesKey].size() > 0),
    ...
    for (const auto& directionStr : config[kDriveDirectionCuesKey]) {
      if (ANKI_VERIFY(directionStr.isString(),
                      "BehaviorDriveOffCharger.Ctor.InvalidDirStr", "")) {
        _iConfig.driveDirections.push_back(
            ParseDriveDirection(directionStr.asString()));
      }
    }
  }
  ...
  PRINT_CH_DEBUG("Behaviors", "BehaviorDriveOffCharger.DriveDist",
                 ...
                 _iConfig.proceduralDistToDrive_mm, Charger::GetLength(),
                 extraDist_mm);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDriveOffCharger::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDriveOffCharger::GetBehaviorJsonKeys(
...
void BehaviorDriveOffCharger::InitBehavior() {
  ...
  const bool mayDriveRandomly =
      std::find(_iConfig.driveDirections.begin(),
                _iConfig.driveDirections.end(),
                DriveDirection::Randomly) != _iConfig.driveDirections.end();
  if (mayDriveRandomly) {
...
bool BehaviorDriveOffCharger::WantsToBeActivatedBehavior() const {
  ...
  DEV_ASSERT(
      robotInfo.IsOnChargerPlatform() || !robotInfo.IsOnChargerContacts(),
      "BehaviorDriveOffCharger.WantsToBeActivatedBehavior."
      "InconsistentChargerFlags");

  // can run any time we are on the charger platform

  // bn: this used to be "on charger platform", but that caused weird issues
  // when the robot thought it drove over the platform later due to
  // mis-localization. Then this changed to "on charger" to avoid those issues,
  // but that caused other issues (if the robot was bumped during wakeup, it
  // wouldn't drive off the charger). Now, we've gone back to OnChargerPlatform
  // but fixed it to work better
...
void BehaviorDriveOffCharger::OnBehaviorActivated() {
  ...
  robotInfo.GetDrivingAnimationHandler().PushDrivingAnimations(
      {AnimationTrigger::DriveStartLaunch, AnimationTrigger::DriveLoopLaunch,
  ...
  if (onTreads) {
    ...
    // otherwise we'll just wait in Update until we are on the treads (or
...
void BehaviorDriveOffCharger::BehaviorUpdate() {
  ...
  if (robotInfo.IsOnChargerPlatform()) {
    const bool onTreads =
        GetBEI().GetOffTreadsState() == OffTreadsState::OnTreads;
    if (!onTreads) {
      // if we aren't on the treads anymore, but we are on the charger, then the
      // user must be holding us inside the charger.... so just sit in this
      // behavior doing nothing until we get put back down
      CancelDelegates(false);
      SET_STATE(WaitForOnTreads);
      // TODO:(bn) play an idle here?
    } else if (!IsControlDelegated()) {
      // if we finished the last action, but are still on the charger, queue
      // another one
      SelectAndDrive();
    }
...
void BehaviorDriveOffCharger::SelectAndDrive() {
  ...
  for (; _dVars.directionIndex < kMaxAttempts; ++_dVars.directionIndex) {
    if (IsControlDelegated()) {
      // the (_dVars.directionIndex - 1)-th direction just started successfully,
      // and then we incremented the index and got here
      break;
    }

    // if we're here, the (_dVars.directionIndex - 1)-th direction either
    // finished or was not applicable (e.g., no faces)
  ...
  if ((_dVars.directionIndex >= kMaxAttempts) && !IsControlDelegated()) {
    PRINT_NAMED_WARNING(
        "BehaviorDriveOffCharger.SelectAndDrive.CouldntLeave",
        "Behavior '%s' couldn't get off charger in %d attempts. onCharger=%d",
        GetDebugLabel().c_str(), kMaxAttempts,
...
void BehaviorDriveOffCharger::TransitionToDrivingStraightProcedural() {
  PRINT_CH_INFO("Behaviors",
                "BehaviorDriveOffCharger.TransitionToDrivingStraightProcedural",
                "Driving straight");
  DriveStraightAction* action =
      new DriveStraightAction(_iConfig.proceduralDistToDrive_mm);
  action->SetCanMoveOnCharger(true);
  const float timeout_s =
      kExtraProceduralTimeout_s +
      (static_cast<float>(_iConfig.proceduralDistToDrive_mm) /
       DEFAULT_PATH_MOTION_PROFILE.speed_mmps);
  action->SetTimeoutInSeconds(timeout_s);
  DelegateIfInControl(action);
  // the Update function will transition back to this or another direction, or
  // end the behavior, as appropriate
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDriveOffCharger::TransitionToDrivingAnim(
...
void BehaviorDriveOffCharger::TransitionToDrivingRandomly() {
  if (ANKI_VERIFY(_iConfig.driveRandomlyBehavior->WantsToBeActivated(),
    ...
    PRINT_CH_INFO("Behaviors",
                  "BehaviorDriveOffCharger.TransitionToDrivingRandomly",
                  "Driving randomly");
...
void BehaviorDriveOffCharger::TransitionToDrivingFace() {
  Pose3d facePose;
  const RobotTimeStamp_t imageTimeStamp_ms =
      GetBEI().GetFaceWorld().GetLastObservedFace(facePose);
  const RobotTimeStamp_t lastImageTimeStamp_ms =
      GetBEI().GetRobotInfo().GetLastImageTimeStamp();
  if (imageTimeStamp_ms > 0 &&
      (imageTimeStamp_ms + 1000 * _iConfig.maxFaceAge_s >=
       lastImageTimeStamp_ms)) {
    Pose3d faceWrtRobot;
    const bool poseSuccess = facePose.GetWithRespectTo(
        GetBEI().GetRobotInfo().GetPose(), faceWrtRobot);
    if (poseSuccess) {
      PRINT_CH_INFO("Behaviors",
                    "BehaviorDriveOffCharger.TransitionToDrivingFace",
                    "Driving toward face");
...
void BehaviorDriveOffCharger::TransitionToDrivingCube() {
  ...
  const ObservableObject* obj =
      GetBEI().GetBlockWorld().FindMostRecentlyObservedObject(filter);
  if (obj != nullptr) {
    const RobotTimeStamp_t cubeTimeStamp_ms = obj->GetLastObservedTime();
    const RobotTimeStamp_t lastImageTimeStamp_ms =
        GetBEI().GetRobotInfo().GetLastImageTimeStamp();
    if (cubeTimeStamp_ms + 1000 * _iConfig.maxCubeAge_s >=
        lastImageTimeStamp_ms) {
      Pose3d cubeWrtRobot;
      const bool poseSuccess = obj->GetPose().GetWithRespectTo(
          GetBEI().GetRobotInfo().GetPose(), cubeWrtRobot);
      if (poseSuccess) {
        PRINT_CH_INFO("Behaviors",
                      "BehaviorDriveOffCharger.TransitionToDrivingCube",
                      "Driving toward cube");
...
void BehaviorDriveOffCharger::TransitionToDrivingMicDirection() {
  ...
  // direction ranges from 0 (12 oclock) to 11 (11 oclock), and then
  // invalid/unknown
  const bool directionKnown = (direction <= 11);
  PRINT_CH_INFO(
      "Behaviors",
      "BehaviorDriveOffCharger.TransitionToDrivingMicDirection.Direction",
      "Direction = [%d] (%s)", (int)direction,
      directionKnown ? "known" : "unknown");
  if (directionKnown) {
    AnimationTrigger directionAnim;
    if (direction == 2) {  // 2
      directionAnim = kAnimRight;
    } else if (direction > 2 &&
               direction <= 6) {  // 3-6 (tie breaker: 6 oclock drives right)
...
MicDirectionIndex BehaviorDriveOffCharger::GetDirectionFromMicHistory() const {
  const MicDirectionHistory& history =
...
void BehaviorDriveOffCharger::DriveToPose(const Pose3d& pose) {
  ...
  const float angleToFace_rad =
      atan2f(groundTranslation.y(), groundTranslation.x());
  AnimationTrigger directionAnim;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorDriveOffCharger::DriveDirection
BehaviorDriveOffCharger::ParseDriveDirection(
    const std::string& driveDirectionStr) const {
  ...
  ANKI_VERIFY(false, "BehaviorDriveOffCharger.ParseDriveDirection.Unknown",
              "Unknown direction %s", driveDirectionStr.c_str());
  return DriveDirection::Straight;
}
