...
#define LOG_FUNCTION_NAME()                              \
  PRINT_CH_INFO("Behaviors", "BehaviorClearChargerArea", \
                "BehaviorClearChargerArea.%s", __func__);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorClearChargerArea::InstanceConfig::InstanceConfig(
    const Json::Value& config, const std::string& debugName)
    : maxNumAttempts(
          JsonTools::ParseInt32(config, kMaxNumAttemptsKey, debugName)),
      tryToPickUpCube(
          JsonTools::ParseBool(config, kTryToPickUpCubeKey, debugName)),
      chargerFilter(std::make_unique<BlockWorldFilter>()),
      cubesFilter(std::make_unique<BlockWorldFilter>()) {
  DEV_ASSERT(maxNumAttempts > 0,
             "BehaviorClearChargerArea.InstanceConfig.InvalidMaxNumAttempts");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorClearChargerArea::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kTryToPickUpCubeKey, kMaxNumAttemptsKey};
  expectedKeys.insert(std::begin(list), std::end(list));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorClearChargerArea::GetAllDelegates(
...
void BehaviorClearChargerArea::OnBehaviorActivated() {
  ...
  if (cube == nullptr) {
    PRINT_NAMED_ERROR(
        "BehaviorClearChargerArea.OnBehaviorActivated.NoCubeInChargerArea",
        "There is no cube intersecting the charger area!");
    return;
  }

  const auto& robotPose = GetBEI().GetRobotInfo().GetPose();
  const auto* charger = GetBEI().GetBlockWorld().FindLocatedObjectClosestTo(
      robotPose, *_iConfig.chargerFilter);

  if (charger == nullptr) {
    PRINT_NAMED_ERROR("BehaviorClearChargerArea.OnBehaviorActivated",
                      "No charger found!");
    return;
  }
  ...
  // Check here if we're carrying an object and put it down next to the charger
  // if so
...
void BehaviorClearChargerArea::TransitionToCheckDockingArea() {
  ...
  if (++_dVars.numAttempts > _iConfig.maxNumAttempts) {
    PRINT_NAMED_WARNING(
        "BehaviorClearChargerArea.TransitionToCheckDockingArea.OutOfRetries",
        "We've attempted to clear the charger docking area the maximum number "
        "of times (%d) - falling back to charger request behavior",
        _iConfig.maxNumAttempts);
    const bool requestWantsToRun =
        _iConfig.requestHomeBehavior->WantsToBeActivated();
  ...
  if (_iConfig.tryToPickUpCube && pickupWantsToRun) {
    DelegateIfInControl(_iConfig.pickupBehavior.get(), [this]() {
...
void BehaviorClearChargerArea::TransitionToPositionForRamming() {
  ...
  // From where the robot currently is, see if we are able to shove the block
  // out of the way (i.e. if we shove it from our current position, it should
  // not hit the charger). Do this by creating a stretched rectangle roughly the
  // width of the robot and extending it along the path from the robot to the
  // cube (and past the cube), then check if this rectangle intersects the
  // charger.
  const auto& robotPose = GetBEI().GetRobotInfo().GetPose();
  ...
  if (charger == nullptr) {
    PRINT_NAMED_ERROR(
        "BehaviorClearChargerArea.TransitionToPositionForRamming.NullCharger",
        "Null charger!");
    return;
  }
  ...
  // First move the lift down, so that the robot can effectively 'shove' it to
  // the side
  action->AddAction(
      new MoveLiftToHeightAction(MoveLiftToHeightAction::Preset::LOW_DOCK));

  const bool shovingPathIntersectsCharger =
      charger->GetBoundingQuadXY().Intersects(rect.GetQuad());
  if (shovingPathIntersectsCharger) {
    // Cannot ram the block, since we would ram it into the charger. Position
    // ourselves to the side of the charger first before shoving it. Choose the
    // side such that we will shove the cube _away_ from the charger.
    float cubeToRobotDist_mm = 0.f;
    if (!ComputeDistanceBetween(robotPose, cubePose, cubeToRobotDist_mm)) {
      LOG_ERROR(
          "BehaviorClearChargerArea.TransitionToPositionForRamming."
          "ComputeDistanceFailure",
          "poses not comparable");
      return;
    }
    const auto& chargerPose = charger->GetPose();
    const float chargerToRobotAngle =
        atan2f(chargerPose.GetTranslation().y() - robotY,
               chargerPose.GetTranslation().x() - robotX);
    const float direction =
        Radians(cubeToRobotAngle - chargerToRobotAngle) < 0.f ? 1.f : -1.f;
    const float lateralDist_mm =
        65.f;  // This puts the robot roughly right next to the cube
    const float turnAngle_rad =
        cubeToRobotAngle +
        direction * std::atan(lateralDist_mm / cubeToRobotDist_mm);
    const float driveDist_mm =
        std::abs(DRIVE_CENTER_OFFSET) +
        std::sqrt(cubeToRobotDist_mm * cubeToRobotDist_mm -
                  lateralDist_mm * lateralDist_mm);
  ...
  DelegateIfInControl(action, &BehaviorClearChargerArea::TransitionToRamCube);
}
...
void BehaviorClearChargerArea::TransitionToRamCube() {
  ...
  if (charger == nullptr) {
    PRINT_NAMED_ERROR(
        "BehaviorClearChargerArea.TransitionToRamCube.NullCharger",
        "Null charger!");
    return;
  }
  ...
  if (!ComputeDistanceBetween(robotPose, cubePose, cubeToRobotDist_mm)) {
    LOG_ERROR(
        "BehaviorClearChargerArea.TransitionToRamCube.ComputeDistanceFailure",
        "poses not comparable");
    return;
  }
  ...
  action->AddAction(new VisuallyVerifyObjectAction(cube->GetID()),
                    true);  // ignore failure
  action->AddAction(new TurnTowardsObjectAction(charger->GetID()));
  DelegateIfInControl(action, [this]() {
    // If we still think the block is in front of the charger, then remove it
    // from blockworld. Ramming it will have caused its position estimate to get
    // messed up, and we do not want the robot to continue to think that the
    // cube is in front of the charger if it's really not.
    const auto* cube = GetCubeInChargerArea();
    if (cube != nullptr) {
...
void BehaviorClearChargerArea::TransitionToPlacingCubeOnGround() {
  ...
  if (charger == nullptr) {
    PRINT_NAMED_ERROR(
        "BehaviorClearChargerArea.TransitionToPlacingCubeOnGround.NullCharger",
        "Null charger!");
    return;
  }

  // Place the cube on the ground next to the charger.
  // Generate some optional placement poses, and let DriveToPose decide which
  // one is best.
  const auto& chargerPose = charger->GetPose();
  std::vector<Pose3d> cubePlacementPoses;
  cubePlacementPoses.push_back(
      Pose3d{DEG_TO_RAD(40.f), Z_AXIS_3D(), {-35.f, 60.f, 0.f}, chargerPose});
  cubePlacementPoses.push_back(
      Pose3d{DEG_TO_RAD(50.f), Z_AXIS_3D(), {-50.f, 65.f, 0.f}, chargerPose});
  cubePlacementPoses.push_back(
      Pose3d{DEG_TO_RAD(-40.f), Z_AXIS_3D(), {-35.f, -60.f, 0.f}, chargerPose});
  cubePlacementPoses.push_back(
      Pose3d{DEG_TO_RAD(-50.f), Z_AXIS_3D(), {-50.f, -65.f, 0.f}, chargerPose});
...
const ObservableObject* BehaviorClearChargerArea::GetCubeInChargerArea() const {
  ...
  const auto& robotPose = GetBEI().GetRobotInfo().GetPose();
  const auto* charger =
      dynamic_cast<const Charger*>(blockWorld.FindLocatedObjectClosestTo(
          robotPose, *_iConfig.chargerFilter));
  if (charger == nullptr) {
    return nullptr;
  }
