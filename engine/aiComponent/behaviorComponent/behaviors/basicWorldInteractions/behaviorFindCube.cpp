 ...
 * Created: 2018-10-15
 *
 * Description: Quickly check the last known location of a cube and search
 *nearby for one if none are known/found in the initial check
 *
...
BehaviorFindCube::BehaviorFindCube(const Json::Value& config)
  ...
  JsonTools::GetValueOptional(config, kSkipReactToCubeKey,
                              _iConfig.skipReactToCubeAnim);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorFindCube::WantsToBeActivatedBehavior() const { return true; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorFindCube::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorFindCube::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kSkipReactToCubeKey};
...
void BehaviorFindCube::BehaviorUpdate() {
  ...
  if (FindCubeState::CheckForCubeInFront == _dVars.state ||
    ...
    if (proxData.foundObject &&
        (proxData.distance_mm < kProxBackupThreshold_mm)) {
      TransitionToBackUpAndCheck();
    }

    // If we don't have a target yet, check if a cube has been seen in the last
    // frame
    ...
    if (nullptr == targetCube) {
      ...
      // Otherwise, we already had a target but it wasn't found where expected
      // during VisuallyVerify. Watch for new observations and pose updates
      if (targetCube->GetLastObservedTime() > _dVars.lastPoseCheckTimestamp) {
        _dVars.lastPoseCheckTimestamp =
            GetBEI().GetRobotInfo().GetLastMsgTimestamp();
        Pose3d cubePose = targetCube->GetPose();
        if (!cubePose.IsSameAs(_dVars.cubePoseAtSearchStart, kDistThreshold_mm,
                               DEG_TO_RAD(kAngleThreshold_deg))) {
          CancelDelegates(false);
          TransitionToReactToCube();
        }
      }
...
void BehaviorFindCube::TransitionToDriveOffCharger() {
  ...
  if (_iConfig.driveOffChargerBehavior->WantsToBeActivated()) {
    ...
    LOG_WARNING("BehaviorFindCube.TransitionError",
...
void BehaviorFindCube::TransitionToVisuallyCheckLastKnown() {
  ...
  if (UpdateTargetCube()) {
    ...
    TurnTowardsObjectAction* visualCheckAction = new TurnTowardsObjectAction(
        _dVars.cubeID, Radians{M_PI_F}, visuallyVerify);
...
void BehaviorFindCube::TransitionToCheckForCubeInFront() {
  ...
  auto searchInFrontAction = new CompoundActionSequential({
      ...
      new WaitForImagesAction(kNumImagesToWaitDuringSearch,
                              VisionMode::Markers),
  ...
  auto* targetCube = GetTargetCube();
  if (nullptr != targetCube) {
    // If we made it here then the visual verification failed, so the cube pose
    // is antequated and wrong Make a note of it, and don't trust the cube pose
    // until it is updated, indicating new observations.
    _dVars.cubePoseAtSearchStart = targetCube->GetPose();
    _dVars.lastPoseCheckTimestamp =
        GetBEI().GetRobotInfo().GetLastMsgTimestamp();
  }

  DelegateIfInControl(searchInFrontAction,
                      &BehaviorFindCube::TransitionToQuickSearchForCube);
}
...
void BehaviorFindCube::TransitionToBackUpAndCheck() {
  ...
  CompoundActionSequential* backupAndCheckAction =
      new CompoundActionSequential();
  backupAndCheckAction->AddAction(
      new TriggerLiftSafeAnimationAction(AnimationTrigger::ReactToObstacle));

  auto* loopAndWaitAction = new CompoundActionParallel();
  loopAndWaitAction->AddAction(new TriggerLiftSafeAnimationAction(
      AnimationTrigger::ChargerDockingSearchWaitForImages));
  loopAndWaitAction->AddAction(new WaitForImagesAction(
      kNumImagesToWaitDuringSearch, VisionMode::Markers));
  backupAndCheckAction->AddAction(loopAndWaitAction);

  DelegateIfInControl(backupAndCheckAction,
                      &BehaviorFindCube::TransitionToQuickSearchForCube);
}
...
void BehaviorFindCube::TransitionToReactToCube() {
  if (_iConfig.skipReactToCubeAnim) {
    ...
    DelegateIfInControl(new TriggerLiftSafeAnimationAction(
        AnimationTrigger::FindCubeReactToCube));
  }
}
...
void BehaviorFindCube::StartNextTurn() {
  ...
  const bool exceededMaxTurns =
      (_dVars.numTurnsCompleted >= kMaxNumSearchTurns);

  if (sweptEnough || exceededMaxTurns) {
    // If we made it all the way through the search without finding anything,
    // call it a failure
    TransitionToGetOutFailure();
    LOG_INFO(
        "BehaviorFindCube.TransitionToSearchTurn.QuickSearchFoundNothing",
        "We have completed a full search. Played %d turn animations (max is "
        "%d), "
        "and swept an angle of %.2f deg (min required sweep angle %.2f deg)",
        _dVars.numTurnsCompleted, kMaxNumSearchTurns, _dVars.angleSwept_deg,
        kMinSearchAngle);
  } else {
    ...
    action->AddAction(
        new TriggerLiftSafeAnimationAction(AnimationTrigger::FindCubeTurns));

    auto* loopAndWaitAction = new CompoundActionParallel();
    loopAndWaitAction->AddAction(
        new TriggerLiftSafeAnimationAction(AnimationTrigger::FindCubeWaitLoop));
    loopAndWaitAction->AddAction(new WaitForImagesAction(
        kNumImagesToWaitDuringSearch, VisionMode::Markers));
    ...
    DelegateIfInControl(action, [this, startHeading]() {
      ...
      DEV_ASSERT(angleSwept_deg < 0.f,
                 "BehaviorQuickSearchForCube.TransitionToSearchTurn."
                 "ShouldTurnClockwise");
      if (std::abs(angleSwept_deg) > 75.f) {
        LOG_WARNING(
            "BehaviorQuickSearchForCube.TransitionToSearchTurn.TurnedTooMuch",
            "The last turn swept an angle of %.2f deg - may have missed seeing "
            "the charger due to the camera's limited FOV",
...
bool BehaviorFindCube::UpdateTargetCube() {
  const auto& robotPose = GetBEI().GetRobotInfo().GetPose();
  auto* targetCube = GetBEI().GetBlockWorld().FindLocatedObjectClosestTo(
      robotPose, *_iConfig.cubesFilter);
  if (nullptr == targetCube) {
    return false;
  }
