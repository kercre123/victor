...
BehaviorFetchCube::InstanceConfig::InstanceConfig()
    : cubesFilter(std::make_unique<BlockWorldFilter>()),
      ...
      skipConnectToCubeBehavior(
          true)  // skip by default so we run it only when user driven
{
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorFetchCube::WantsToBeActivatedBehavior() const { return true; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorFetchCube::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorFetchCube::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kSkipConnectToCubeBehaviorKey};
...
void BehaviorFetchCube::InitBehavior() {
  ...
  BC.FindBehaviorByIDAndDowncast<BehaviorFindFaceAndThen>(
      BEHAVIOR_ID(FindFacesFetchCube), BEHAVIOR_CLASS(FindFaceAndThen),
  ...
  BC.FindBehaviorByIDAndDowncast<BehaviorPickUpCube>(
      BEHAVIOR_ID(PickupCubeNoInitialReaction), BEHAVIOR_CLASS(PickUpCube),
  ...
  _iConfig.reactToCliffBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(ReactToCliffDuringFetch));
  ...
  _iConfig.putCubeByChargerBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(PlaceCubeByCharger));
...
void BehaviorFetchCube::BehaviorUpdate() {
  ...
  if (!_iConfig.reactToCliffBehavior->IsActivated() &&
    ...
    LOG_INFO("BehaviorFetchCube.CliffDetected",
             "Delegating to reactToCliffBehavior");
    CancelDelegates(false);
    DelegateIfInControl(_iConfig.reactToCliffBehavior.get(), [this]() {
...
void BehaviorFetchCube::TransitionToLookForUser() {
  ...
  if (_iConfig.lookForUserBehavior->WantsToBeActivated()) {
    DelegateIfInControl(_iConfig.lookForUserBehavior.get(), [this]() {
...
void BehaviorFetchCube::TransitionToPickUpCube() {
  ...
  if (nullptr == GetTargetCube()) {
    ...
    LOG_ERROR("BehaviorFetchCube.PickUpCube.NoCubeFound",
              "No cube was available, should not have made it to this state of "
              "the behavior");
    return;
  }
  ...
  if (_iConfig.pickUpCubeBehavior->WantsToBeActivated()) {
    DelegateIfInControl(_iConfig.pickUpCubeBehavior.get(), [this]() {
...
void BehaviorFetchCube::TransitionToGetOutSuccess() {
  ...
  DelegateIfInControl(
      new TriggerLiftSafeAnimationAction(AnimationTrigger::FetchCubeSuccess));
}
...
bool BehaviorFetchCube::ComputeFaceBasedTargetPose() {
  ...
  // Create a target pose pointing at the face, on the ground between the robot
  // and the face
  ...
  if (!ComputeDistanceBetween(projectedFacePose, robotPose,
                              distanceToProjectedPose)) {
    return false;
  }
  float fwdTranslation = distanceToProjectedPose - kTargetDistFromFace_mm;
  Radians angleRobotFwdToFace(atan2f(projectedFacePose.GetTranslation().y(),
                                     projectedFacePose.GetTranslation().x()));
