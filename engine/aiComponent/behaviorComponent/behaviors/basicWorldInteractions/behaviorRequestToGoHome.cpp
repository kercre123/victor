...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorRequestToGoHome::DynamicVariables::DynamicVariables(
...
BehaviorRequestToGoHome::BehaviorRequestToGoHome(const Json::Value& config)
    : ICozmoBehavior(config), _dVars(_iConfig) {
  LoadConfig(config);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorRequestToGoHome::GetBehaviorJsonKeys(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorRequestToGoHome::GetAllDelegates(
...
void BehaviorRequestToGoHome::InitBehavior() {
  ...
  _iConfig.findFacesBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(ObservingFindFaces));
  DEV_ASSERT(_iConfig.findFacesBehavior != nullptr,
             "BehaviorRequestToGoHome.InitBehavior.NullFindFacesBehavior");
}
...
void BehaviorRequestToGoHome::BehaviorUpdate() {
  ...
  if (_dVars.state == EState::FindingFaces) {
    if (IsControlDelegated()) {
      ...
      const bool hasFace = GetBEI().GetFaceWorld().HasAnyFaces(
          _dVars.imageTimestampWhenActivated);
      if (hasFace) {
        DEV_ASSERT(_iConfig.findFacesBehavior->IsActivated(),
                   "BehaviorRequestToGoHome.BehaviorUpdate.FindFacesNotActive");
        CancelDelegates();
  ...
  } else if ((_dVars.state == EState::Requesting) && !IsControlDelegated()) {
...
void BehaviorRequestToGoHome::TransitionToCheckForFaces() {
  ...
  RobotTimeStamp_t maxFaceAge_ms =
      Util::numeric_cast<TimeStamp_t>(1000.f * _iConfig.maxFaceAge_sec);
  maxFaceAge_ms = std::min(maxFaceAge_ms, _dVars.imageTimestampWhenActivated);
  RobotTimeStamp_t oldestFaceTimestamp =
      _dVars.imageTimestampWhenActivated - maxFaceAge_ms;

  const bool hasFace = GetBEI().GetFaceWorld().HasAnyFaces(oldestFaceTimestamp);
  if (hasFace) {
    // Turn to last known face. Even if it fails, still jump to playing the
    // animation
...
void BehaviorRequestToGoHome::TransitionToRequestAnim() {
  ...
  action->AddAction(new TriggerAnimationAction(
      _dVars.currRequestParamsPtr->requestAnimTrigger));

  DelegateIfInControl(
      action, &BehaviorRequestToGoHome::TransitionToRequestWaitLoopAnim);
}
...
void BehaviorRequestToGoHome::TransitionToRequestWaitLoopAnim() {
  ...
  auto* action = new ReselectingLoopAnimationAction(
      animTrigger,
      0,     // numLoops
      true,  // interrupt running
      (u8)AnimTrackFlag::NO_TRACKS, animTimeout);

  DelegateIfInControl(action,
                      &BehaviorRequestToGoHome::TransitionToRequestGetoutAnim);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorRequestToGoHome::TransitionToRequestGetoutAnim() {
  auto* action = new TriggerAnimationAction(
      _dVars.currRequestParamsPtr->getoutAnimTrigger);

  DelegateIfInControl(action, [this]() {
    // Increment the appropriate request counters now that a request has
    // completed
...
void BehaviorRequestToGoHome::TransitionToLowPowerMode() {
  ...
  PRINT_NAMED_WARNING(
      "BehaviorRequestToGoHome.TransitionToLowPowerMode.LowPowerModeUndefined",
...
void BehaviorRequestToGoHome::LoadConfig(const Json::Value& config) {
  ...
  for (auto& pair : configEntryMap) {
    ...
    params->numRequests =
        JsonTools::ParseUint8(json, kNumRequestsKey, debugName);

    JsonTools::GetCladEnumFromJSON(json, kRequestAnimTriggerKey,
                                   params->requestAnimTrigger, debugName);
    JsonTools::GetCladEnumFromJSON(json, kRequestGetoutAnimTriggerKey,
                                   params->getoutAnimTrigger, debugName);
    JsonTools::GetCladEnumFromJSON(json, kRequestWaitLoopAnimTriggerKey,
                                   params->waitLoopAnimTrigger, debugName);

    params->idleWaitTime_sec =
        JsonTools::ParseFloat(json, kRequestIdleWaitTimeKey, debugName);
  }

  JsonTools::GetCladEnumFromJSON(config, kPickupAnimTriggerKey,
                                 _iConfig.pickupAnimTrigger, debugName);
  _iConfig.maxFaceAge_sec =
      JsonTools::ParseFloat(config, kMaxFaceAgeKey, debugName);
}
