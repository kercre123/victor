...
CONSOLE_VAR_RANGED(f32, kWiggle_ForwardDist_mm, CONSOLE_GROUP, 6.0f, 0.0f,
                   20.0f);
CONSOLE_VAR_RANGED(f32, kWiggle_ForwardSpeed_mmps, CONSOLE_GROUP, 120.0, 0.0f,
                   200.0f);
CONSOLE_VAR_RANGED(f32, kWiggle_BackupDist_mm, CONSOLE_GROUP, 15.0f, 0.0f,
                   20.0f);
CONSOLE_VAR_RANGED(f32, kWiggle_BackupSpeed_mmps, CONSOLE_GROUP, 100.0f, 0.0f,
                   200.0f);
CONSOLE_VAR_RANGED(f32, kWiggle_VerifyWaitTime_s, CONSOLE_GROUP, 0.25f, 0.0f,
                   2.0f);
CONSOLE_VAR_RANGED(f32, kWiggle_BackupSettleTime_s, CONSOLE_GROUP, 0.4f, 0.0f,
                   2.0f);
}  // namespace
...
BehaviorWiggleOntoChargerContacts::DynamicVariables::DynamicVariables()
    : state(State::BackingUp), attempts(0), contactTime_s(-1.0f) {}
...
bool BehaviorWiggleOntoChargerContacts::WantsToBeActivatedBehavior() const {
  ...
  if (GetBEI().GetOffTreadsState() == OffTreadsState::OnTreads) {
    ...
    // only want to attempt this if we are backed up a bit onto the charger, so
    // only if the pitch is reasonable (if the robot is rolled too much, it
    // should get counted as on-side
    const Radians& pitch = GetBEI().GetRobotInfo().GetPitchAngle();
    if (pitch <= DEG_TO_RAD(kMaxPitchToRun_degs) &&
        pitch >= DEG_TO_RAD(kMinPitchToRun_degs)) {
      return true;
    }
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorWiggleOntoChargerContacts::GetBehaviorOperationModifiers(
    BehaviorOperationModifiers& modifiers) const {
  modifiers.wantsToBeActivatedWhenOnCharger = true;

  // allow off treads in case the robot drives up onto the back of the charger
  // and thinks it's in the air
  modifiers.wantsToBeActivatedWhenOffTreads = true;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorWiggleOntoChargerContacts::GetBehaviorJsonKeys(
...
void BehaviorWiggleOntoChargerContacts::TransitionToBackingUp() {
  ...
  auto* action = new DriveStraightAction(
      -kWiggle_BackupDist_mm, kWiggle_BackupSpeed_mmps, playAnimation);
  action->SetCanMoveOnCharger(true);

  // if we hit the contacts during the option, the update will stop us for
  // verification
  DelegateIfInControl(
      action, &BehaviorWiggleOntoChargerContacts::TransitionToMovingForward);
}
...
void BehaviorWiggleOntoChargerContacts::TransitionToMovingForward() {
  ...
  const bool playAnimation = false;
  auto* action = new DriveStraightAction(
      kWiggle_ForwardDist_mm, kWiggle_ForwardSpeed_mmps, playAnimation);
  action->SetCanMoveOnCharger(true);

  // after moving forward, if we don't get interrupted because of the contacts,
  // try looping the state machine again
  DelegateIfInControl(action,
                      &BehaviorWiggleOntoChargerContacts::StartStateMachine);
}
  ...
  if (IsActivated()) {
    ...
    switch (_dVars.state) {
      case State::BackingUp: {
        // let the keep driving a bit to make sure we are "fully" on the
        // contacts
        if (onContacts &&
            currTime_s - _dVars.contactTime_s > kWiggle_BackupSettleTime_s) {
      ...
      case State::MovingForward: {
        // stop immediately since this means we were up on the back wall and now
        // "dropped" into place
        CancelDelegates(false);
        TransitionToVerifyingContacts();
        break;
