...
#define LOG_FUNCTION_NAME() \
  PRINT_CH_INFO("Behaviors", "BehaviorGoHome", "BehaviorGoHome.%s", __func__);

namespace Anki {
namespace Vector {

CONSOLE_VAR_EXTERN(bool, kRobustChargerObservation_SaveImages);
CONSOLE_VAR(bool, kGoHome_VisualVerification_SaveImages,
            "Behaviors.BehaviorGoHome", false);
...
// If we are too far from the charger at the beginning of the behavior, or we
// have not observed the charger in a while, then first try to drive to a pose
// from which to verify the charger's location before trying to dock to it.
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorGoHome::InstanceConfig::InstanceConfig(const Json::Value& config,
  ...
  useCliffSensorCorrection =
      JsonTools::ParseBool(config, kUseCliffSensorsKey, debugName);

  JsonTools::GetCladEnumFromJSON(config, kLeftTurnAnimKey, leftTurnAnimTrigger,
                                 debugName);
  JsonTools::GetCladEnumFromJSON(config, kRightTurnAnimKey,
                                 rightTurnAnimTrigger, debugName);
  JsonTools::GetCladEnumFromJSON(config, kDrivingStartAnimKey,
                                 drivingStartAnimTrigger, debugName);
  JsonTools::GetCladEnumFromJSON(config, kDrivingEndAnimKey,
                                 drivingEndAnimTrigger, debugName);
  JsonTools::GetCladEnumFromJSON(config, kDrivingLoopAnimKey,
                                 drivingLoopAnimTrigger, debugName);
  JsonTools::GetCladEnumFromJSON(config, kRaiseLiftAnimKey,
                                 raiseLiftAnimTrigger, debugName);
  JsonTools::GetCladEnumFromJSON(config, kNuzzleAnimKey, nuzzleAnimTrigger,
                                 debugName);
  homeFilter = std::make_unique<BlockWorldFilter>();
  driveToRetryCount =
      JsonTools::ParseInt32(config, kDriveToRetryCountKey, debugName);
  turnToDockRetryCount =
      JsonTools::ParseInt32(config, kTurnToDockRetryCountKey, debugName);
  mountChargerRetryCount =
      JsonTools::ParseInt32(config, kMountChargerRetryCountKey, debugName);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorGoHome::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {
      kUseCliffSensorsKey,        kLeftTurnAnimKey,
      kRightTurnAnimKey,          kDrivingStartAnimKey,
      kDrivingEndAnimKey,         kDrivingLoopAnimKey,
      kRaiseLiftAnimKey,          kNuzzleAnimKey,
      kDriveToRetryCountKey,      kTurnToDockRetryCountKey,
...
void BehaviorGoHome::InitBehavior() {
  ...
  BC.FindBehaviorByIDAndDowncast<BehaviorClearChargerArea>(
      BEHAVIOR_ID(ClearChargerArea), BEHAVIOR_CLASS(ClearChargerArea),
  ...
  BC.FindBehaviorByIDAndDowncast<BehaviorRequestToGoHome>(
      BEHAVIOR_ID(RequestHomeBecauseStuck), BEHAVIOR_CLASS(RequestToGoHome),
  ...
  BC.FindBehaviorByIDAndDowncast<BehaviorWiggleOntoChargerContacts>(
  ...
  DEV_ASSERT(_iConfig.observeChargerBehavior != nullptr,
             "BehaviorGoHome.InitBehavior.NullObserveChargerBehavior");
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorGoHome::WantsToBeActivatedBehavior() const {
  const bool isOnChargerContacts =
  ...
  GetBEI().GetBlockWorld().FindLocatedMatchingObjects(*_iConfig.homeFilter,
                                                      locatedHomes);
...
void BehaviorGoHome::OnBehaviorActivated() {
  ...
  if (times.size() > kNumRepeatedActivationsAllowed) {
    LOG_WARNING(
        "BehaviorGoHome.OnBehaviorActivated.RepeatedlyActivated",
        "We have been activated %zu times in the past %.1f seconds, so instead "
        "of continuing "
        "with this behavior, we are playing the failure anim and exiting.",
        times.size(), kRepeatedActivationCheckWindow_sec);
    // Clear the list of activated times (so that we don't get stuck in a loop
    // here) and play the 'failure' anim
    _dVars.persistent.activatedTimes.clear();
    DelegateIfInControl(
        new TriggerAnimationAction(AnimationTrigger::ChargerDockingFailure));
    return;
  }

  const auto& robotPose = GetBEI().GetRobotInfo().GetPose();
  const auto* object = GetBEI().GetBlockWorld().FindLocatedObjectClosestTo(
...
void BehaviorGoHome::OnBehaviorDeactivated() {
  ...
  if (_dVars.HasResult()) {
    ...
    DASMSG_SET(i1, _dVars.HasSucceeded(),
               "Success or failure to get onto the charger (1 for success, 0 "
               "for failure)");
    DASMSG_SEND();
  }
}
...
void BehaviorGoHome::TransitionToObserveCharger() {
  ...
  const bool observeChargerWantsToRun =
      _iConfig.observeChargerBehavior->WantsToBeActivated();
  if (observeChargerWantsToRun) {
    DelegateIfInControl(_iConfig.observeChargerBehavior.get(),
                        &BehaviorGoHome::TransitionToPostObservation);
  } else {
    LOG_ERROR(
        "BehaviorGoHome.TransitionToObserveCharger.ObserveChargerNotActivating",
        "Visual verify behavior does not want to be activated.");
  }
}
...
void BehaviorGoHome::TransitionToDriveToObservationPose(const bool canRetry) {
  ...
  // If we have not observed the charger recently or we are too far away, then
  // drive to a pose from which to try and observe the charger and confirm its
  // pose.
  const auto* charger = dynamic_cast<const Charger*>(
      GetBEI().GetBlockWorld().GetLocatedObjectByID(_dVars.chargerID));
  if (charger == nullptr) {
  ...
  if (!ComputeDistanceBetween(charger->GetPose(), robotInfo.GetPose(),
                              robotToChargerDist_mm)) {
    LOG_ERROR(
        "BehaviorGoHome.TransitionToObserveCharger.ComputeDistanceFailure",
        "poses not comparable");
    return;
  }
  const bool farFromCharger =
      (robotToChargerDist_mm > kFarFromChargerThreshold_mm);
  const bool observedRecently =
      ((robotInfo.GetLastMsgTimestamp() - charger->GetLastObservedTime()) <
       kRecentlyObservedChargerThreshold_ms);

  if (farFromCharger || !observedRecently) {
    const float incidenceAngle_rad =
        (canRetry) ? (kInitialChargerIncidenceForObservation_rad)
                   : (kFinalChargerIncidenceForObservation_rad);
    auto poses = charger->GenerateObservationPoses(
        GetRNG(), kNumRandomPosesForObservation, incidenceAngle_rad);
    ...
    DelegateIfInControl(driveAction, [this, canRetry](const ActionResult& res) {
      const bool isPlanningFailure =
          (res == ActionResult::PATH_PLANNING_FAILED_ABORT) ||
          (res == ActionResult::PATH_PLANNING_FAILED_RETRY);
...
void BehaviorGoHome::TransitionToCheckDockingArea() {
  ...
  const bool clearChargerWantsToRun =
      _iConfig.clearChargerAreaBehavior->WantsToBeActivated();
...
void BehaviorGoHome::TransitionToPlacingCubeOnGround() {
  ...
  const auto* charger =
      GetBEI().GetBlockWorld().GetLocatedObjectByID(_dVars.chargerID);
  if (charger == nullptr) {
    LOG_ERROR("BehaviorGoHome.TransitionToPlacingCubeOnGround",
              "Null charger!");
    return;
  }
  ...
  DelegateIfInControl(
      new PlaceObjectOnGroundAtPoseAction(cubePlacementPose),
      [this](ActionResult result) {
        ...
        if (robotInfo.GetCarryingComponent().IsCarryingObject()) {
          ...
          if (charger == nullptr) {
            LOG_ERROR(
                "BehaviorGoHome.TransitionToPlacingCubeOnGroundCallback."
                "NullCharger",
                "Null charger!");
            return;
          }
          ...
          const float turnAngle =
              atan2f(robotInfo.GetPose().GetTranslation().y() -
                         charger->GetPose().GetTranslation().y(),
                     robotInfo.GetPose().GetTranslation().x() -
                         charger->GetPose().GetTranslation().x());
...
void BehaviorGoHome::TransitionToDriveToCharger() {
  ...
  const auto* charger = dynamic_cast<const Charger*>(
      GetBEI().GetBlockWorld().GetLocatedObjectByID(_dVars.chargerID));
  if (charger == nullptr) {
    LOG_ERROR("BehaviorGoHome.TransitionToDriveToCharger.NullCharger",
              "Null charger!");
    return;
  }

  // We always just clear the area in front of the charger of obstacles and
  // cliffs. If there is still an obstacle or cliff there, we will encounter it
  // when we drive up to it.
  ...
  GetBEI().GetMapComponent().InsertData(
      chargerDockingPoly,
      MemoryMapData(MemoryMapTypes::EContentType::ClearOfCliff, lastTimestamp));

  auto* driveToAction = new DriveToObjectAction(
      _dVars.chargerID, PreActionPose::ActionType::DOCKING);
  driveToAction->SetPreActionPoseAngleTolerance(
      kDriveToChargerPreActionPoseAngleTol_rad);
  driveToAction->SetVisuallyVerifyWhenDone(
      false);  // We will visually verify position in
  ...
  DelegateIfInControl(driveToAction, [this](ActionResult result) {
    ...
    if (resultCategory == ActionResultCategory::SUCCESS) {
      ...
      if (result == ActionResult::PATH_PLANNING_FAILED_ABORT) {
        LOG_WARNING(
            "BehaviorGoHome.TransitionToDriveToCharger.PathPlanningTimedOut",
            "Path planning timed out possibly due to prox obstacles or there "
            "is no "
            "valid path from our location. Clearing an area of the nav map, "
            "turning in "
            "place a bit, then trying again.");
        ClearNavMapUpToCharger();
        // Do a small point turn to give the planner a new starting point, so
        // that hopefully it does not time out again.
        const float direction = GetRNG().RandBool() ? 1.f : -1.f;
        const float angle =
            direction * GetRNG().RandDblInRange(M_PI_4_F, M_PI_2_F);
        const bool isAbsolute = false;
        DelegateIfInControl(
            new TurnInPlaceAction(angle, isAbsolute),
            [this](ActionResult res) { TransitionToDriveToCharger(); });
      } else {
        TransitionToDriveToCharger();
      }
    } else {
...
void BehaviorGoHome::TransitionToCheckPreTurnPosition() {
  ...
  if (!observeChargerWantsToRun) {
    LOG_ERROR(
        "BehaviorGoHome.TransitionToCheckPreTurnPosition."
        "ObserveChargerBehaviorDWTA",
        "");
    return;
  }
  ...
  RobotTimeStamp_t verifyStartTime =
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorGoHome::TransitionToPostVisualVerification(
  ...
  if (charger != nullptr) {
    ...
    if (robotPose.GetWithRespectTo(charger->GetPose(), robotPoseWrtCharger)) {
      ...
      const auto& angleWrtCharger =
          robotPoseWrtCharger.GetRotation().GetAngleAroundZaxis();
      ...
      poseOk = Util::IsNear(xWrtCharger, kIdealXWrtCharger_mm, kMaxXError_mm) &&
               ...
               angleWrtCharger.IsNear(kIdealAngleWrtCharger_rad,
                                      kMaxAngularError_rad);

      if (!poseOk) {
        LOG_WARNING(
            "BehaviorGoHome.TransitionToCheckPreTurnPosition.NotInPosition",
            "Ended up not in a good position to commence the turn to begin "
            "docking. RobotPoseWrtCharger {%.1f, %.1f, %.1f deg}",
            xWrtCharger, yWrtCharger, angleWrtCharger.getDegrees());
      }
    } else {
      LOG_WARNING(
          "BehaviorGoHome.TransitionToCheckPreTurnPosition."
          "GetPoseWithRespectToFailed",
  ...
  const bool chargerSeen = (charger != nullptr) &&
                           (charger->GetLastObservedTime() >= verifyStartTime);

  const bool canRetryTurnToDock =
  ...
  DEV_ASSERT_MSG((!chargerExists && !poseOk) || (chargerExists),
                 "BlockWorld.TransitionToPostVisualVerification."
                 "NullChargerImpliesNotPoseOk",
                 "If the charger does not exist, then the pose cannot be ok");
  DEV_ASSERT_MSG(
      (!chargerExists && !chargerSeen) || (chargerExists),
      "BlockWorld.TransitionToPostVisualVerification."
      "NullChargerImpliesNotSeenCharger",
  ...
  if (chargerExists) {
    if (poseOk && chargerSeen) {
      ...
      if (canRetryTurnToDock) {
        ...
        if (!chargerSeen) {
          // If visual observation failed, then we've successfully gotten to the
          // charger
          //  pre-action pose, but it is no longer there
          LOG_WARNING(
              "BehaviorGoHome.TransitionToCheckPreTurnPosition.NoChargerSeen",
              "Charger id=%d failed visual verification (start=%u end=%u)",
              _dVars.chargerID.GetValue(), (TimeStamp_t)verifyStartTime,
              (TimeStamp_t)GetBEI().GetRobotInfo().GetLastMsgTimestamp());
          DASMSG(go_home_charger_not_visible, "go_home.charger_not_visible",
                 "GoHome behavior failure because the charger is not seen when "
                 "should be.");
          DASMSG_SEND();
          // Marking the object as dirty will delegate the responsibility of
          // clearing it
...
void BehaviorGoHome::TransitionToMountCharger() {
  ...
  auto* mountAction = new MountChargerAction(_dVars.chargerID,
                                             _iConfig.useCliffSensorCorrection);
  ...
  DelegateIfInControl(action, [this](ActionResult result) {
    ...
    if (resultCategory == ActionResultCategory::SUCCESS) {
      GetBehaviorComp<RobotStatsTracker>().IncrementBehaviorStat(
          BehaviorStat::MountedCharger);
      _dVars.SetSucceeded(true);
      TransitionToPlayingNuzzleAnim();
    } else if ((resultCategory == ActionResultCategory::RETRY) &&
               (_dVars.mountChargerRetryCount++ <
                _iConfig.mountChargerRetryCount)) {
      // Simply go back to the starting pose, which will allow visual
      // verification to happen again, etc.
      TransitionToDriveToCharger();
    } else {
      // Either out of retries or we got another failure type. If the charger is
      // unplugged, drive forward off of it before ending the behavior.
      if (result == ActionResult::CHARGER_UNPLUGGED_ABORT) {
        ...
        DelegateIfInControl(
            new DriveStraightAction(
                100.f, DEFAULT_PATH_MOTION_PROFILE.speed_mmps, false),
            [this]() { TransitionToFailureReaction(); });
      } else {
        TransitionToFailureReaction();
      }
    }
...
void BehaviorGoHome::TransitionToOnChargerCheck() {
  ...
  const bool wiggleWantsToRun =
      _iConfig.wiggleOntoChargerBehavior->WantsToBeActivated();
  if (!GetBEI().GetRobotInfo().IsOnChargerContacts() && wiggleWantsToRun) {
...
void BehaviorGoHome::TransitionToFailureReaction() {
  LOG_WARNING("BehaviorGoHome.TransitionToFailureReaction",
              "BehaviorGoHome had an action failure.");
  // Play the "charger face" animation indicating that we have failed, then
  // allow the behavior to exit
  DelegateIfInControl(
      new TriggerAnimationAction(AnimationTrigger::ChargerDockingFailure));
...
void BehaviorGoHome::PushDrivingAnims() {
  DEV_ASSERT(!_dVars.drivingAnimsPushed,
             "BehaviorGoHome.PushDrivingAnims.AnimsAlreadyPushed");

  if (!_dVars.drivingAnimsPushed) {
    auto& drivingAnimHandler =
        GetBEI().GetRobotInfo().GetDrivingAnimationHandler();
    drivingAnimHandler.PushDrivingAnimations(
        {_iConfig.drivingStartAnimTrigger, _iConfig.drivingLoopAnimTrigger,
         _iConfig.drivingEndAnimTrigger},
        GetDebugLabel());
...
void BehaviorGoHome::ClearNavMapUpToCharger() {
  ...
  const auto* charger =
      GetBEI().GetBlockWorld().GetLocatedObjectByID(_dVars.chargerID);
  if (charger == nullptr) {
    LOG_ERROR("BehaviorGoHome.ClearNavMapUpToCharger.NullCharger",
              "Null charger!");
    return;
  }

  const auto& robotPoseWrtOrigin =
      GetBEI().GetRobotInfo().GetPose().GetWithRespectToRoot();
  const auto& chargerPoseWrtOrigin = charger->GetPose().GetWithRespectToRoot();

  float distanceBetween_mm = 0.f;
  if (!ComputeDistanceBetween(robotPoseWrtOrigin, chargerPoseWrtOrigin,
                              distanceBetween_mm)) {
    DEV_ASSERT(
        false,
        "BehaviorGoHome.ClearNavMapUpToCharger.FailedComputeDistanceBetween");
  }

  const Point2f centerPoint((robotPoseWrtOrigin.GetTranslation().x() +
                             chargerPoseWrtOrigin.GetTranslation().x()) /
                                2.f,
                            (robotPoseWrtOrigin.GetTranslation().y() +
                             chargerPoseWrtOrigin.GetTranslation().y()) /
                                2.f);
  ...
  const auto lastTimestamp = GetBEI().GetRobotInfo().GetLastMsgTimestamp();
  GetBEI().GetMapComponent().InsertData(
      pointsPoly, MemoryMapData(MemoryMapTypes::EContentType::ClearOfObstacle,
                                lastTimestamp));
}
