 ...
 * Created: 2016-06-15
 *
 * Description: Behavior that allows cozmo to use a block to throw himself on
 *his back
 *
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPopAWheelie::GetBehaviorJsonKeys(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPopAWheelie::OnBehaviorDeactivated() { ResetBehavior(); }
...
void BehaviorPopAWheelie::UpdateTargetBlock() const {
  if (!_dVars.idSetExternally) {
    ...
    _dVars.targetBlock = cache.GetBestObjectForIntention(
        ObjectInteractionIntention::PopAWheelieOnObject);
  }
}
...
void BehaviorPopAWheelie::TransitionToReactingToBlock() {
  ...
  // Turn towards the object and then react to it before performing the pop a
  // wheelie action
  ...
  if (_iConfig.playCubeReaction) {
    action->AddAction(new TriggerLiftSafeAnimationAction(
        AnimationTrigger::PopAWheelieInitial));
  }
...
void BehaviorPopAWheelie::TransitionToPerformingAction(bool isRetry) {
  ...
  if (isRetry) {
    ...
    PRINT_CH_INFO("Behaviors",
                  "BehaviorPopAWheelie.TransitionToPerformingAction.Retrying",
                  "Retry %d of %d", _dVars.numPopAWheelieActionRetries,
                  _iConfig.numRetries);
  } else {
  ...
  const Radians maxTurnToFaceAngle(
      ((isRetry || ShouldStreamline() || !_iConfig.sayName) ? 0
                                                            : DEG_TO_RAD(90)));

  DriveToPopAWheelieAction* goPopAWheelie = new DriveToPopAWheelieAction(
      _dVars.targetBlock, false, 0, maxTurnToFaceAngle, _iConfig.sayName);
  if (_iConfig.sayName) {
    goPopAWheelie->SetSayNameAnimationTrigger(
        AnimationTrigger::PopAWheeliePreActionNamedFace);
    goPopAWheelie->SetNoNameAnimationTrigger(
        AnimationTrigger::PopAWheeliePreActionUnnamedFace);
  }

  // once we get to the predock pose, before docking, disable the cliff sensor
  // and associated reactions so that we play the correct animation instead of
  // getting interrupted)
  auto disableCliff = [this](Robot& robot) {
    // tell the robot not to stop the current action / animation if the cliff
    // sensor fires
    _dVars.hasDisabledcliff = true;
    _dVars.isPoppingWheelie = true;
    robot.SendMessage(RobotInterface::EngineToRobot(
        RobotInterface::EnableStopOnCliff(false)));
  };
        ...
        switch (IActionRunner::GetActionResultCategory(msg.result)) {
          ...
          case ActionResultCategory::SUCCESS: {
            ...
            DelegateIfInControl(new TriggerAnimationAction(
                AnimationTrigger::SuccessfulWheelie));
          ...
          case ActionResultCategory::ABORT: {
            // assume that failure is because we didn't visually verify
            // (although it could be due to an error)
            ...
            const ObservableObject* failedObject =
                GetBEI().GetBlockWorld().GetLocatedObjectByID(
                    _dVars.targetBlock);
            if (failedObject) {
              GetAIComp<AIWhiteboard>().SetFailedToUse(
                  *failedObject,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPopAWheelie::SetupRetryAction(
    const ExternalInterface::RobotCompletedAction& msg) {
  // Ensure that the closest block is selected
  UpdateTargetBlock();

  // Pick which retry animation, if any, to play. Then try performing the action
  // again, with "isRetry" set to true.

  IActionRunner* animAction = nullptr;
  switch (msg.result) {
    case ActionResult::DID_NOT_REACH_PREACTION_POSE: {
      animAction = new TriggerLiftSafeAnimationAction(
          AnimationTrigger::PopAWheelieRealign);
      break;
    }

    default: {
      animAction = new TriggerLiftSafeAnimationAction(
          AnimationTrigger::PopAWheelieRetry);
      break;
    }
  }
  ...
  if (nullptr != animAction) {
    DelegateIfInControl(animAction,
                        [this]() { this->TransitionToPerformingAction(true); });
  } else {
...
void BehaviorPopAWheelie::ResetBehavior() {
  ...
  if (_dVars.hasDisabledcliff) {
    ...
    // NOTE: assumes that we want the cliff to be re-enabled when we leave this
    // behavior. If it was disabled before this behavior started, it will be
    // enabled anyway. If this becomes a problem, then we need to count / track
    // the requests to enable and disable like we do with track locking or
