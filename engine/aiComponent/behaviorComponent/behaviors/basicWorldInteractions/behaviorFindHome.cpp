 ...
 * Created: 1/31/18
 *
 * Description: Drive to a random nearby position, and play a 'searching'
 *animation
 *
...
const char* kNumSearchesBeforePlayingPostSearchAnim =
    "numSearchesBeforePlayingPostSearchAnim";
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorFindHome::InstanceConfig::InstanceConfig(const Json::Value& config,
                                                 const std::string& debugName) {
  searchTurnAnimTrigger = AnimationTrigger::Count;
  postSearchAnimTrigger = AnimationTrigger::Count;

  JsonTools::GetCladEnumFromJSON(config, kSearchTurnAnimKey,
                                 searchTurnAnimTrigger, debugName);
  JsonTools::GetCladEnumFromJSON(config, kPostSearchAnimTrigger,
                                 postSearchAnimTrigger, debugName);
  minSearchAngleSweep_deg =
      JsonTools::ParseFloat(config, kMinSearchAngleSweepKey, debugName);
  maxSearchTurns = JsonTools::ParseUint8(config, kMaxSearchTurnsKey, debugName);
  recentSearchWindow_sec =
      JsonTools::ParseFloat(config, kRecentSearchWindowKey, debugName);
  numSearchesBeforePlayingPostSearchAnim = JsonTools::ParseInt32(
      config, kNumSearchesBeforePlayingPostSearchAnim, debugName);
  maxNumRecentSearches =
      JsonTools::ParseUint8(config, kMaxNumRecentSearchesKey, debugName);
  minDrivingDist_mm =
      JsonTools::ParseFloat(config, kMinDrivingDistKey, debugName);
  maxDrivingDist_mm =
      JsonTools::ParseFloat(config, kMaxDrivingDistKey, debugName);
  homeFilter = std::make_unique<BlockWorldFilter>();
  searchSpacePointEvaluator =
      std::make_unique<Util::RejectionSamplerHelper<Point2f>>();
  searchSpacePolyEvaluator =
      std::make_unique<Util::RejectionSamplerHelper<Poly2f>>();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorFindHome::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorFindHome::GetBehaviorJsonKeys(
...
void BehaviorFindHome::InitBehavior() {
  ...
  _iConfig.condHandleNearPrevSearch =
      _iConfig.searchSpacePointEvaluator->AddCondition(
          std::make_shared<RejectIfInRange>(0.f,
                                            kMinDistFromPreviousSearch_mm));
  ...
  _iConfig.condHandleCollisions =
      _iConfig.searchSpacePolyEvaluator->AddCondition(
          std::make_shared<RejectIfCollidesWithMemoryMap>(
              kTypesToBlockSampling));

  _iConfig.observeChargerBehavior =
      GetBEI().GetBehaviorContainer().FindBehaviorByID(
          BEHAVIOR_ID(RobustChargerObservation));
  DEV_ASSERT(_iConfig.observeChargerBehavior != nullptr,
             "BehaviorFindHome.InitBehavior.NullObserveChargerBehavior");
}
  ...
  switch (event.GetData().GetTag()) {
    ...
    case EngineToGameTag::RobotProcessedImage: {
      ...
      if (IsActivated() && GetBEI().HasVisionComponent()) {
        ...
        if (GetBEI().GetVisionComponent().GetLastImageQuality() ==
            Vision::ImageQuality::TooDark) {
...
void BehaviorFindHome::OnBehaviorDeactivated() {
  ...
  const ObservableObject* charger =
      GetBEI().GetBlockWorld().FindLocatedObjectClosestTo(robotPose,
                                                          *_iConfig.homeFilter);

  // NOTE: The charger is considered not seen, if it was deleted from Blockworld
  // (i.e. the charger object is null). This can happen under certain situations
  // when getting a "negative-observation" (not seeing it when it was expected).
  ...
  DASMSG(find_home_result, "find_home.result",
         "Whether the FindHome behavior succeeded in locating the object");
  DASMSG_SET(i1, chargerSeen,
             "Success/failure on locating the charger. 1=success 0=failuire");
  DASMSG_SET(
      i2, _dVars.numFramesOfMarkers,
      "Count of total number of processed image frames searching for Markers");
  DASMSG_SET(i3, _dVars.numFramesOfImageTooDark,
             "Count of number of processed image frames (searching for "
             "Markers) that are TooDark");
  DASMSG_SEND();
}
...
void BehaviorFindHome::TransitionToStartSearch() {
  ...
  _iConfig.condHandleNearPrevSearch->SetOtherPositions(
      GetRecentlySearchedLocations());
  const bool nearPreviousSearch =
      !_iConfig.condHandleNearPrevSearch->operator()(robotPosition);
  if (nearPreviousSearch) {
    LOG_INFO(
        "BehaviorFindHome.TransitionToStartSearch.TooCloseToPreviousSearch",
        "We are too close to a previously-searched location, so driving to a "
        "new search pose");
...
void BehaviorFindHome::TransitionToLookInPlace() {
  if (ANKI_VERIFY(_iConfig.observeChargerBehavior->WantsToBeActivated(),
                  "BehaviorFindHome.TransitionToLookInPlace."
                  "ObserveChargerBehavior.NotActivatable",
                  "")) {
    DelegateIfInControl(_iConfig.observeChargerBehavior.get(),
...
void BehaviorFindHome::TransitionToSearchTurn() {
  ...
  const bool sweptEnough =
      (_dVars.angleSwept_deg > _iConfig.minSearchAngleSweep_deg);
  const bool exceededMaxTurns =
      (_dVars.numTurnsCompleted >= _iConfig.maxSearchTurns);

  if (sweptEnough || exceededMaxTurns) {
    LOG_INFO("BehaviorFindHome.TransitionToSearchTurn.CompletedSearch",
             "We have completed a full search. Played %d turn animations (max "
             "is %d), and swept an angle of %.2f deg (min required sweep angle "
             "%.2f deg)",
             _dVars.numTurnsCompleted, _iConfig.maxSearchTurns,
             _dVars.angleSwept_deg, _iConfig.minSearchAngleSweep_deg);
    ...
    const auto& robotPose = GetBEI().GetRobotInfo().GetPose();
    _dVars.persistent.searchedLocations[now_sec] =
        Point2f{robotPose.GetTranslation()};
    // If appropriate, play the "post search" animation before transferring to
    // random drive
    const bool shouldPlayPostSearchAnim =
        (_iConfig.numSearchesBeforePlayingPostSearchAnim >= 0) &&
        (_dVars.numSearchesCompleted >
         _iConfig.numSearchesBeforePlayingPostSearchAnim);
    if (shouldPlayPostSearchAnim) {
      DelegateIfInControl(
          new TriggerAnimationAction(_iConfig.postSearchAnimTrigger),
          [this]() { TransitionToRandomDrive(); });
    } else {
      TransitionToRandomDrive();
    }
  } else {
    ...
    DelegateIfInControl(action, [this, startHeading]() {
      ...
      if (!Util::InRange(std::abs(angleSwept_deg), 30.f, 75.f)) {
        // We turned an unexpected amount - may have missed seeing the charger
        // due to the camera's limited FOV
        DASMSG(find_home_invalid_turn, "behavior.find_home.invalid_turn_angle",
               "Invalid turn angle during a search-for-charger turn");
        DASMSG_SET(i1, (int)std::round(angleSwept_deg),
                   "The actual angle swept during this turn (degrees)");
        DASMSG_SET(i2, _dVars.numTurnsCompleted,
                   "Number of search turns completed at this location");
        DASMSG_SEND_WARNING();
      }
...
void BehaviorFindHome::TransitionToRandomDrive() {
  ...
  // Have we searched too many times recently? If so, we should exit (which will
  // allow the next behavior to run, which is likely "RequestHome"). It's
  // possible that this behavior is continually being interrupted for some
  // reason, and if that's the case we don't want to just keep searching
  // endlessly, so we need a persistent stopping condition.
  ...
  if (recentlySearchedLocations.size() >= _iConfig.maxNumRecentSearches) {
    LOG_WARNING(
        "BehaviorFindHome.TransitionToRandomDrive.TooManyRecentSearches",
        "We have performed too many (%zu) searches in the past %.1f seconds - "
        "ending behavior",
  ...
  const Radians angleTol =
      M_PI_F;  // basically don't care about the angle of arrival
  auto driveToAction = new DriveToPoseAction(potentialSearchPoses);
  driveToAction->SetGoalThresholds(DEFAULT_POSE_EQUAL_DIST_THRESOLD_MM,
                                   angleTol);
...
void BehaviorFindHome::GenerateSearchPoses(std::vector<Pose3d>& outPoses) {
  ...
  // If we have a charger in the map, first try to drive to a position in front
  // of it to begin a search. Only do this if we haven't recently visited the
  // charger's location.
  const float now_sec =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  const auto* charger = dynamic_cast<const Charger*>(
      GetBEI().GetBlockWorld().FindLocatedObjectClosestTo(
          robotPose, *_iConfig.homeFilter));
  if ((charger != nullptr) &&
      (now_sec > _dVars.persistent.lastVisitedOldChargerTime +
                     _iConfig.recentSearchWindow_sec)) {
    outPoses = charger->GenerateObservationPoses(
        GetRNG(), kNumRandomPosesForObservation, kIncidenceForObservation_rad);
    _dVars.persistent.lastVisitedOldChargerTime = now_sec;
    return;
  }

  const int kNumPositionsForSearch =
      4;  // How many search positions to generate
  const int kNumAnglesAtPose =
      6;  // How many poses at each position to generate
  ...
  DEV_ASSERT(nullptr != memoryMap,
             "BehaviorFindHome.GenerateSearchPoses.NeedMemoryMap");
  ...
  for (unsigned int cnt = 0; cnt < kMaxNumSampleSteps; ++cnt) {
    ...
    const bool acceptedPoint =
        _iConfig.searchSpacePointEvaluator->Evaluate(GetRNG(), sampledPos);
    if (!acceptedPoint) {
      continue;
    }
    ...
    const bool acceptedFootprint =
        _iConfig.searchSpacePolyEvaluator->Evaluate(GetRNG(), footprint);
    ...
    // Generate poses with the same position but different orientations. This
    // should be pretty cheap compared to a pose at a different location.
  ...
  if (outPoses.empty()) {
    LOG_WARNING("BehaviorFindHome.GenerateSearchPoses.NoPosesFound",
                "No poses that satisfy the sampling requirements were found - "
                "using naive random sampling method.");
...
void BehaviorFindHome::GetRandomDrivingPose(Pose3d& outPose) {
  ...
  const float distance_mm = GetRNG().RandDblInRange(_iConfig.minDrivingDist_mm,
                                                    _iConfig.maxDrivingDist_mm);
  ...
  outPose.SetRotation(outPose.GetRotation() *
                      Rotation3d{newAngle, Z_AXIS_3D()});
...
std::vector<Point2f> BehaviorFindHome::GetRecentlySearchedLocations() {
  // Update previous search positions. Cull to recent window and transform into
  // a vector.
  const float now_sec =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  ...
  std::transform(prevSearchMap.begin(), prevSearchMap.end(),
                 std::back_inserter(prevSearchPositions),
                 [](const std::pair<float, Point2f>& mapEntry) {
