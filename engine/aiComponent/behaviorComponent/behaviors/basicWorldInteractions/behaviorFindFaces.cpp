 ...
 * Created: 2016-08-31
 *
 * Description: Delegates to the configured search behavior, terminating based
 *on configurable stopping conditions.
 *
...
BehaviorFindFaces::BehaviorFindFaces(const Json::Value& config)
  ...
  JsonTools::GetValueOptional(config, kMaxFaceAgeToLookKey,
                              _iConfig.maxFaceAgeToLook_ms);

  const std::string& debugName = "Behavior" + GetDebugLabel() + ".LoadConfig";

  const auto& conditionStr =
      JsonTools::ParseString(config, kStoppingConditionKey, debugName);
  ...
  if (_iConfig.stoppingCondition == StoppingCondition::Timeout) {
    DEV_ASSERT(Util::IsFltGTZero(_iConfig.timeout_sec),
               "BehaviorFindFaces.InvalidTimeout");
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorFindFaces::GetBehaviorJsonKeys(
...
void BehaviorFindFaces::InitBehavior() {
  ...
  _iConfig.searchBehavior = BC.FindBehaviorByID(
      BehaviorTypesWrapper::BehaviorIDFromString(_iConfig.searchBehaviorStr));
...
void BehaviorFindFaces::OnBehaviorActivated() {
  ...
  // Get known faces
  const RobotTimeStamp_t latestImageTimeStamp =
      robotInfo.GetLastImageTimeStamp();
  _dVars.imageTimestampWhenActivated = latestImageTimeStamp;

  _dVars.startingFaces = GetBEI().GetFaceWorld().GetFaceIDs(
...
void BehaviorFindFaces::BehaviorUpdate() {
  ...
  if (_dVars.searchingForFaces) {
    ...
    if (shouldStop) {
      PRINT_CH_INFO("Behaviors", "FindFaces.CancellingSearch",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorFindFaces::StoppingCondition
BehaviorFindFaces::StoppingConditionFromString(const std::string& str) const {
  if (str == "AnyFace") {
    return StoppingCondition::AnyFace;
  } else if (str == "NewFace") {
    return StoppingCondition::NewFace;
  } else if (str == "Timeout") {
    return StoppingCondition::Timeout;
  } else if (str == "None") {
    return StoppingCondition::None;
  } else {
    DEV_ASSERT_MSG(
        false, "BehaviorFindFaces.StoppingConditionFromString.InvalidString",
        "String %s is not a valid StoppingCondition", str.c_str());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
const char* BehaviorFindFaces::StoppingConditionToString(
    BehaviorFindFaces::StoppingCondition condition) const {
  switch (condition) {
    case StoppingCondition::Invalid:
      return "Invalid";
    case StoppingCondition::None:
      return "None";
    case StoppingCondition::AnyFace:
      return "AnyFace";
    case StoppingCondition::NewFace:
      return "NewFace";
