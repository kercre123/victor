 ...
 * Created: 2018-07-17
 *
 * Description: A behavior that enables a user to select a reaction to a
 *condition that becomes the new default behavior to respond to that trigger.
 *
...
bool BehaviorUserDefinedBehaviorSelector::WantsToBeActivatedBehavior() const {
  ...
  auto& userBehaviorTreeComp =
      GetBehaviorComp<UserDefinedBehaviorTreeComponent>();
  return userBehaviorTreeComp.IsEnabled();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorUserDefinedBehaviorSelector::GetAllDelegates(
...
void BehaviorUserDefinedBehaviorSelector::InitBehavior() {
  ...
  behaviorContainer.FindBehaviorByIDAndDowncast(
      BEHAVIOR_ID(UserDefinedBehaviorTreeTextToSpeech),
      BEHAVIOR_CLASS(TextToSpeechLoop), _iConfig.ttsBehavior);
  behaviorContainer.FindBehaviorByIDAndDowncast(
      BEHAVIOR_ID(UserDefinedBehaviorTreeConfirmNewBehavior),
      BEHAVIOR_CLASS(PromptUserForVoiceCommand), _iConfig.confirmationBehavior);
}
...
void BehaviorUserDefinedBehaviorSelector::OnBehaviorActivated() {
  ...
  std::set<BehaviorID> behaviorIds =
      userBehaviorTreeComp.GetConditionOptions(condType);

  // get preset behavior (is anonymous if no preset)
  ICozmoBehaviorPtr delegationBehaviorPtr =
      userBehaviorTreeComp.GetDelegationBehavior(condType);
  _dVars.currentBehaviorId = (nullptr != delegationBehaviorPtr)
                                 ? delegationBehaviorPtr->GetID()
                                 : BehaviorID::Anonymous;
  ...
  if (_dVars.demoQueue.empty()) {
    PRINT_NAMED_ERROR(
        "BehaviorUserDefinedBehaviorTreeSelector.OnBehaviorActivated."
        "NoBehaviorsAvailable",
...
void BehaviorUserDefinedBehaviorSelector::DemoBehaviorRouter() {
  ...
  // if the queue is now empty, then after demoing the final behavior, the
  // selector should cancel itself
  ...
  // delegate as desired, to either the cancellation or the selection
...
void BehaviorUserDefinedBehaviorSelector::DemoBehaviorCallback() {
  ...
  if (_iConfig.confirmationBehavior->WantsToBeActivated()) {
    DelegateNow(_iConfig.confirmationBehavior.get(),
                &BehaviorUserDefinedBehaviorSelector::PromptYesOrNoCallback);
  }
}
...
void BehaviorUserDefinedBehaviorSelector::PromptYesOrNoCallback() {
  ...
  // otherwise, break out of the loop and reset the original behavior to be
  // delegated. if no original behavior, use the last behavior demoed.
  if (uic.IsUserIntentPending(affirmativeIntent)) {
    ...
    const std::string noSelectionString =
        "Looks like we've run out of options. Let's try again.";
    ...
    const std::string sameThingString =
        "I'll just do the same thing next time.";
    ...
    DelegateNow(SetUpSpeakingBehavior(responseString),
                &BehaviorUserDefinedBehaviorSelector::CancelDelegatesAndSelf);
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
IBehavior* BehaviorUserDefinedBehaviorSelector::SetUpSpeakingBehavior(
