...
bool BehaviorUserDefinedBehaviorTreeRouter::WantsToBeActivatedBehavior() const {
  ...
  for (auto& condition : _iConfig.customizableConditions) {
    if (condition->AreConditionsMet(GetBEI())) {
      // do not activate unless user-defined behavior tree is activated in
      // console
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorUserDefinedBehaviorTreeRouter::GetAllDelegates(
  ...
  ICozmoBehaviorPtr selectorBehaviorPtr = behaviorContainer.FindBehaviorByID(
      BehaviorID::UserDefinedBehaviorSelector);
...
void BehaviorUserDefinedBehaviorTreeRouter::OnBehaviorActivated() {
  ...
  for (auto& condition : _iConfig.customizableConditions) {
    ...
    // if a custom behavior hasn't yet been set, use the selector behavior to
    // set one
  ...
  if (!isAtLeastOneConditionMet) {
    PRINT_NAMED_ERROR(
        "BehaviorUserDefinedBehaviorTreeRouter.OnBehaviorActivated."
        "NoConditionWantsToBeActivated",
...
void BehaviorUserDefinedBehaviorTreeRouter::OnReturnFromSelectorBehavior() {
  ...
  BEIConditionType currentBeiCondType =
      userBehaviorTreeComp.GetCurrentCondition();
  auto behaviorPtr =
      userBehaviorTreeComp.GetDelegationBehavior(currentBeiCondType);
  if (behaviorPtr->WantsToBeActivated()) {
    DelegateNow(behaviorPtr.get(),
                &BehaviorUserDefinedBehaviorTreeRouter::EraseCondition);
  }
}
...
void BehaviorUserDefinedBehaviorTreeRouter::SetNewCustomBehaviorWithSelector() {
  ...
  ICozmoBehaviorPtr selectorBehaviorPtr =
      behaviorContainer.FindBehaviorByID(selectorBehaviorId);
  if (selectorBehaviorPtr->WantsToBeActivated()) {
    DelegateNow(
        selectorBehaviorPtr.get(),
        &BehaviorUserDefinedBehaviorTreeRouter::OnReturnFromSelectorBehavior);
  } else {
    PRINT_NAMED_WARNING(
        "BehaviorUserDefinedBehaviorTreeRouter.OnBehaviorActivated."
        "SelectorDoesNotWantToBeActivated",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorUserDefinedBehaviorTreeRouter::UsePreviouslySetCustomBehavior(
    BEIConditionType beiCondType) {
  auto& userBehaviorTreeComp =
      GetBehaviorComp<UserDefinedBehaviorTreeComponent>();

  auto behaviorPtr = userBehaviorTreeComp.GetDelegationBehavior(beiCondType);
  const bool behaviorDoesNotWantToBeActivated =
      !behaviorPtr->WantsToBeActivated();

  if (behaviorDoesNotWantToBeActivated) {
    PRINT_NAMED_ERROR(
        "BehaviorUserDefinedBehaviorTreeRouter.OnBehaviorActivated."
        "BehaviorActivationReqsTooStrict",
        "Behavior %s did not want to be activated. Check the conditions under "
...
void BehaviorUserDefinedBehaviorTreeRouter::BehaviorUpdate() {
  // we can avoid a circular dependency of the user-defined behavior component
  // on behavior container by setting the customizable conditions on the first
  // tick
