 ...
 * Created: 2017-10-16
 *
 * Description: Behavior that manages an internal state machine for behavior
 *delegation
 *
 ...
 * light 1: green = on charger, red = cliff detected (purple = both) (middle
 * light) light 2: blinking while update is running (status light)
 */
...
class InternalStatesBehavior::State {
 public:
  struct Transition {
    Transition(StateID state, IBEIConditionPtr cond,
               const std::string& emotionEvent = "")
        : toState(state), condition(cond), emotionEvent(emotionEvent) {}
  ...
  // Get the time this state has been active. This timer starts when the state
  // is activated. If there's an interruption, the timer pauses rather than
  // resetting (or continuing to track) and resumes if the state is resumed.
  float GetTimeActive();

  std::string _name;

  // note that these also count a state as "starting" and "ending" when this
  // behavior itself is interrupted (even if we later "resume" the state)
  ...
  // Transitions are evaluated in order, and if the function returns true, we
  // will transition to the given state id.
...
InternalStatesBehavior::InternalStatesBehavior(
  ...
  _ignoreMissingTransitions =
      config.get(kIgnoreMissingTransitionsKey, false).asBool();

  // create custom BEI conditions for timers (if any are specified)
  CustomBEIConditionHandleList customTimerHandles =
      CreateCustomTimerConditions(config);

  // First, parse the state definitions to create all of the "state names" as a
  // first pass
  for (const auto& stateConfig : config[kStateConfigKey]) {
    AddStateName(JsonTools::ParseString(stateConfig, kStateNameConfigKey,
  ...
  DEV_ASSERT(
      _states->size() == _stateNameToID.size(),
  ...
  // keep set of which states we've used so we can warn later if they aren't all
  // used
  ...
  for (const auto& transitionDefConfig : config[kTransitionDefinitionsKey]) {
    ...
    for (const StateID fromStateID : fromStates) {
      ...
      auto parseTransitions =
          [this, &transitionDefConfig, &allToStates, &fromState](
              const std::string& key, const TransitionType transitionType) {
            for (const auto& transitionConfig : transitionDefConfig[key]) {
              const StateID toState =
                  ParseStateFromJson(transitionConfig, "to");
              IBEIConditionPtr condition =
                  ParseTransitionStrategy(transitionConfig);
              const std::string& emotionEvent =
                  transitionConfig.get(kEmotionEventKey, "").asString();

              fromState.AddTransition(
                  transitionType,
                  State::Transition{toState, condition, emotionEvent});
      ...
      parseTransitions("nonInterruptingTransitions",
                       TransitionType::NonInterrupting);
  ...
  if (allFromStates.size() != _states->size()) {
    PRINT_NAMED_WARNING(
        "InternalStatesBehavior.TransitionDefinitions.DeadEndStates",
        "Some states don't have any outgoing transition strategies! %zu from "
        "states, but %zu states",
        allFromStates.size(), _states->size());
  }

  if ((allToStates.size() != _states->size()) && !_ignoreMissingTransitions) {
    PRINT_NAMED_WARNING(
        "InternalStatesBehavior.TransitionDefinitions.UnusedStates",
        "Some states don't have any incoming transition strategies! %zu to "
        "states, but %zu states",
        allToStates.size(), _states->size());
  }

  PRINT_CH_DEBUG("Behaviors", "InternalStatesBehavior.StatesCreated",
                 "Created %zu states", _states->size());

  const std::string& initialStateStr = JsonTools::ParseString(
      config, kInitialStateKey, "InternalStatesBehavior.StateConfig");
  ...
  if (ANKI_VERIFY(stateIt != _stateNameToID.end(),
                  ...
                  "State named '%s' does not exist", initialStateStr.c_str())) {
    _currState = stateIt->second;
    _defaultState = stateIt->second;
  }

  // fill out _resumeReplacements with any state replacements to be made when
  // re-activating the behavior
  ...
  BEIConditionFactory::CheckConditionsAreUsed(customTimerHandles,
                                              GetDebugLabel());
}
...
CustomBEIConditionHandleList
InternalStatesBehavior::CreateCustomTimerConditions(const Json::Value& config) {
  CustomBEIConditionHandleList handles;

  static const char* kDebugName =
      "InternalStatesBehavior.CreateCustomTimerConditions";

  for (const auto& timerConfig : config[kStateTimerConditionsKey]) {
    const std::string& name =
        JsonTools::ParseString(timerConfig, kStateNameConfigKey, kDebugName);
    const float beginTime_s =
        JsonTools::ParseFloat(timerConfig, kBeginTimeKey, kDebugName);

    handles.emplace_back(BEIConditionFactory::InjectCustomBEICondition(
        name, std::make_shared<ConditionLambda>(
...
void InternalStatesBehavior::GetBehaviorJsonKeys(
...
void InternalStatesBehavior::InitBehavior() {
  ...
  _putDownBlockBehavior = GetBEI().GetBehaviorContainer().FindBehaviorByID(
      BEHAVIOR_ID(PutDownBlock));

  // initialize all transitions (from the set so they each only get initialized
  // once)
  ...
  PRINT_CH_DEBUG("Behaviors", "InternalStatesBehavior.Init",
                 "initialized %zu states", _states->size());
}
...
void InternalStatesBehavior::AddState(State&& state) {
  if (ANKI_VERIFY(GetStateID(state._name) != InvalidStateID,
                  ...
                  "State has invalid name '%s'", state._name.c_str()) &&
...
void InternalStatesBehavior::GetAllDelegates(
...
void InternalStatesBehavior::OnBehaviorActivated() {
  ...
  if (!_firstRun) {
    // resuming this behavior -- see if we should be resuming in a different
    // state than when we were previously de-activated
    StateID resumeState = _currState;
    const auto it =
        std::find_if(_resumeReplacements.begin(), _resumeReplacements.end(),
                     [this](const auto& p) { return p.first == _currState; });
  ...
  if (ANKI_VERIFY(_currState != InvalidStateID,
                  ...
                  "_currState set to invalid state id, falling back to initial "
                  "state")) {
...
void InternalStatesBehavior::OnBehaviorDeactivated() {
  // hold the state in case we get re-activated, but still "transition" out of
  // it (e.g. to disable vision modes)
  // TODO:(bn) not this.
...
void InternalStatesBehavior::BehaviorUpdate() {
  ...
  if (_useDebugLights) {
    ...
    if (_debugLightsDirty) {
      GetBEI().GetBackpackLightComponent().SetBackpackAnimation(
          _currDebugLights);
  ...
  for (const auto& transition :
       state._transitions[TransitionType::Interrupting]) {
    if (tryTransition(transition)) {
      return;
    }
  }

  // if we get here, then there must be no interrupting conditions that
  // activated

  if (_isRunningGetIn) {
    // else, the get in is still running, so don't evaluate non-interrupting
    // conditions (or exit conditions)
    return;
  }

  // it's ok to dispatch now if either we aren't dispatched to anything, or the
  // behavior we are dispatched to is ok with a "gentle" interruption
  bool okToDispatch = !IsControlDelegated();

  if (!okToDispatch) {
    if (GetBEI().HasDelegationComponent()) {
      // TODO:(bn) rather than calling this directly, we can introduce a "stop
      // yourself in X seconds" event that ICozmoBehavior implements (and hears
      // about from StateChangeComponent)
      auto& delegationComponent = GetBEI().GetDelegationComponent();
      const IBehavior* delegate =
          delegationComponent.GetBehaviorDelegatedTo(this);
      const ICozmoBehavior* delegateBehavior =
          dynamic_cast<const ICozmoBehavior*>(delegate);
      if (delegateBehavior) {
  ...
  if (okToDispatch) {
    for (const auto& transition :
         state._transitions[TransitionType::NonInterrupting]) {
      if (tryTransition(transition)) {
        return;
      }
    }
    ...
    if (!IsControlDelegated()) {
      // Now there have been no interrupting or non-interrupting transitions, so
    ...
    // if we get here, then there is no state we want to switch to, so re-start
    // the current one if it wants to run and isn't already running

    // TODO:(bn) can behaviors be null?
    if (!IsControlDelegated()) {
      if (nullptr == state._behavior) {
        PRINT_CH_INFO(
            "Behaviors", "InternalStatesBehavior.BehaviorUpdate.CancelSelf",
            "%s: in state '%s', behavior not specified, canceling self",
            GetDebugLabel().c_str(), state._name.c_str());
        CancelSelf();
      } else if (state._behavior->WantsToBeActivated()) {
        DelegateIfInControl(state._behavior.get());
      } else if (_lastTransitionTick ==
                 BaseStationTimer::getInstance()->GetTickCount() - 1) {
        PRINT_NAMED_WARNING("InternalStateBehavior.BehaviorUpdate.NoTransition",
                            "There were no transitions out of state %s and the "
                            "behavior %s still didn't activate!",
                            state._name.c_str(), state._behaviorName.c_str());
      }
    }
    // else we'll just sit here doing nothing evaluating the conditions each
    // tick
  }
...
void InternalStatesBehavior::TransitionToState(const StateID targetState) {
  // TODO:(bn) don't de- and re-activate behaviors if switching states doesn't
  // change the behavior
  ...
  if (_currState != InvalidStateID) {
    ...
    DEV_ASSERT(
        !IsControlDelegated() || targetState == InvalidStateID,
        "InternalStatesBehavior.TransitionToState.WasInCountButHadDelegate");
  }

  // TODO:(bn) channel for high level ai?
  const std::string& oldStateStr =
      (_currState != InvalidStateID) ? _states->at(_currState)._name : "<NONE>";
  const std::string& newStateStr = (targetState != InvalidStateID)
                                       ? _states->at(targetState)._name
                                       : "<NONE>";
  PRINT_CH_INFO("Unfiltered", "InternalStatesBehavior.TransitionToState",
                "Transition from state '%s' -> '%s'", oldStateStr.c_str(),
                newStateStr.c_str());
  ...
  // Although state transitions shouldn't be internally driven while putting
  // down a cube, external interrupts could occur which would leave this state
  // hung if we don't clear it deliberately
...
bool InternalStatesBehavior::PutDownBlockIfNecessary(State& state) {
  if (_putDownBlockBehavior->WantsToBeActivated()) {
    ...
    if (!modifiers.wantsToBeActivatedWhenCarryingObject) {
      ...
      DelegateIfInControl(_putDownBlockBehavior.get(), [this, &state]() {
        PRINT_CH_INFO("Behaviors",
                      "InternalStatesBehavior.PutDownBlockBehavior.Complete",
...
bool InternalStatesBehavior::RunStateGetInIfAble(State& state) {
  if (state._getInBehavior != nullptr) {
    if (state._getInBehavior->WantsToBeActivated()) {
      ...
      PRINT_CH_INFO(
          "Behaviors", "InternalStatesBehavior.GetInBehavior.Delegate",
          "%s: transitioning into state '%s', so will delegate to get in '%s'",
          GetDebugLabel().c_str(), state._name.c_str(),
          state._getInBehavior->GetDebugLabel().c_str());
      DelegateIfInControl(state._getInBehavior.get(), [this, &state]() {
        PRINT_CH_INFO("Behaviors",
                      "InternalStatesBehavior.GetInBehavior.Complete",
      ...
      PRINT_CH_INFO("Behaviors", "InternalStatesBehavior.GetInBehavior.Skip",
                    "%s: transitioning into state '%s', but get in behavior "
...
void InternalStatesBehavior::RunMainStateBehavior(State& state) {
  if (nullptr == state._behavior) {
    PRINT_CH_INFO("Behaviors",
                  "InternalStatesBehavior.TransitionToState.CancelSelf",
    ...
    PRINT_NAMED_WARNING(
        "InternalStatesBehavior.TransitionToState.NoActivation",
...
float InternalStatesBehavior::GetCurrentStateActiveTime_s() const {
  if (_currState == InvalidStateID) {
    PRINT_NAMED_ERROR(
        "InternalStatesBehavior.GetCurrentStateActiveTime.InvalidState",
...
InternalStatesBehavior::State::State(const Json::Value& config) {
  _name = JsonTools::ParseString(config, kStateNameConfigKey,
                                 "InternalStatesBehavior.StateConfig");
  _getInBehaviorName = config.get(kGetInBehaviorKey, "").asString();
  _ensureMinStim =
      config.get(kEnsureMinStimKey, -1.0f).asFloat();  // -1 means disabled
  ...
  if (config[kResetBehaviorTimerKey].isString()) {
    _behaviorTimer = BehaviorTimerManager::BehaviorTimerFromString(
        config[kResetBehaviorTimerKey].asString());
  }

  const std::string& debugColorStr =
      config.get("debugColor", "BLACK").asString();
  _debugColor = NamedColors::GetByString(debugColorStr);

  const std::string& activateIntent =
      config.get("activateIntent", "").asString();
  if (!activateIntent.empty()) {
    ANKI_VERIFY(UserIntentTagFromString(activateIntent, _activateIntent),
                ...
                "Could not get user intent from '%s'", activateIntent.c_str());
  }
}
...
void InternalStatesBehavior::State::Init(BehaviorExternalInterface& bei) {
  ...
  if (_behaviorName.empty()) {
    ...
    _behavior = BC.FindBehaviorByID(
        BehaviorTypesWrapper::BehaviorIDFromString(_behaviorName));
    DEV_ASSERT_MSG(_behavior != nullptr,
                   "InternalStatesBehavior.State.NoBehavior",
                   "State '%s' cannot find behavior '%s'", _name.c_str(),
                   _behaviorName.c_str());
  }

  if (!_getInBehaviorName.empty()) {
    _getInBehavior = BC.FindBehaviorByID(
        BehaviorTypesWrapper::BehaviorIDFromString(_getInBehaviorName));
    DEV_ASSERT_MSG(_getInBehavior != nullptr,
                   "InternalStatesBehavior.State.NoGetInBehavior",
                   "State '%s' cannot find getInBehavior '%s'", _name.c_str(),
    ...
    PRINT_CH_DEBUG("Behaviors", "InternalStatesBehavior.GetInBehavior.Defined",
                   ...
                   _name.c_str(), _getInBehavior.get(),
                   _getInBehaviorName.c_str());
  }
}

void InternalStatesBehavior::State::GetAllTransitions(
    std::set<IBEIConditionPtr>& allTransitions) {
...
void InternalStatesBehavior::State::AddTransition(
    TransitionType transType, const Transition& transition) {
...
void InternalStatesBehavior::State::OnActivated(BehaviorExternalInterface& bei,
  ...
  if (_activateIntent != USER_INTENT(INVALID)) {
    auto& uic = bei.GetAIComponent()
                    .GetComponent<BehaviorComponent>()
                    .GetComponent<UserIntentComponent>();
    if (uic.IsUserIntentPending(_activateIntent)) {
      // if we have a behavior associated with this state, activate the intent
      // through it so that it can control how it wants to display the active
      // intent feedback
      if (nullptr != _behavior) {
        _behavior->ActivateUserIntentHelper(_activateIntent,
                                            ("InternalState:" + _name));
      } else {
  ...
  if (!isResuming || adjustedTimeNeverSet) {
    ...
    // this is a "resume" of the state, so subtract the time that this was
    // "paused" (due to this behavior itself being interrupted)
  ...
  if (!isResuming && _ensureMinStim > 0.0f) {
    ...
    if (currStim < _ensureMinStim) {
      LOG_DEBUG("InternalStatesBehavior.State.OnActivated.EnsureMinStim",
                ...
                _name.c_str(), currStim, _ensureMinStim);
      moodManager.SetEmotion(EmotionType::Stimulated, _ensureMinStim,
                             "MinStimForInternalState");
    }
  }
}
...
float InternalStatesBehavior::State::GetTimeActive() {
  const bool adjustedTimeNeverSet =
      (_adjustedStartTime_s == -std::numeric_limits<float>::min());
  ...
  DEV_ASSERT(Util::IsFltGE(returnTime, 0.0f),
             "InternalStatesBehavior.State.AdjustedTime.NegativeTimeBug");
  return returnTime;
}

bool InternalStatesBehavior::StateExitCooldownExpired(
    StateID state, float timeout,
    InternalStatesBehavior::StateCooldownDefault neverRunDefault) const {
  const float currTime_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();

  const auto it = _states->find(state);
  if (it != _states->end() && it->second._lastTimeEnded_s >= 0.0f) {
    ...
    switch (neverRunDefault) {
      ...
      case StateCooldownDefault::UseFirstBehaviorActivationTime: {
        const bool cooldownExpired =
            (_firstTimeActivated_s + timeout <= currTime_s);
        return cooldownExpired;
      }
    }
  }
...
InternalStatesBehavior::StateID InternalStatesBehavior::AddStateName(
    const std::string& stateName) {
  if (ANKI_VERIFY(_stateNameToID.find(stateName) == _stateNameToID.end(),
                  "InternalStatesBehavior.AddStateName.DuplicateName",
                  "Adding '%s' more than once", stateName.c_str())) {
    const size_t newID =
        _stateNameToID.size() + 2;  // +1 for the state itself, +1 to skip 0
    _stateNameToID[stateName] = newID;
    DEV_ASSERT(GetStateID(stateName) == newID,
               "InternalStatesBehavior.StateNameBug");
    return newID;
  }
  return InvalidStateID;
}

void InternalStatesBehavior::AddConsoleVarTransitions(const char* uniqueVarName,
                                                      const char* category) {
  if (ANKI_DEV_CHEATS) {
    auto func = [this](ConsoleFunctionContextRef context) {
      ...
      std::transform(requestLowerCase.begin(), requestLowerCase.end(),
                     requestLowerCase.begin(), tolower);
      // and make a lower-cased version of _stateNameToID
      NameToIdMapType lcStateNameToID;
      for (const auto& pair : _stateNameToID) {
        std::string lcName(pair.first.size(), 0);
        std::transform(pair.first.begin(), pair.first.end(), lcName.begin(),
                       tolower);
      ...
      if (it != lcStateNameToID.end()) {
        ...
        context->channel->WriteLog("No state '%s', available states: %s",
                                   stateName, states.str().c_str());

        PRINT_NAMED_WARNING("InternalStatesBehavior.InvalidState",
                            "No state '%s', available states: %s", stateName,
                            states.str().c_str());
      }
    };
    auto* cfunc = new Anki::Util::IConsoleFunction(
        uniqueVarName, std::move(func), category, "const char* stateName");
    _consoleFunc.reset(cfunc);
  }
}

InternalStatesBehavior::StateID InternalStatesBehavior::GetStateID(
  ...
  if (ANKI_VERIFY(it != _stateNameToID.end(),
                  ...
                  "State named '%s' does not exist", stateName.c_str())) {
    return it->second;
  }
  return InvalidStateID;
}
...
InternalStatesBehavior::StateID InternalStatesBehavior::ParseStateFromJson(
    const Json::Value& config, const std::string& key) {
  if (ANKI_VERIFY(config[key].isString(),
                  "InternalStatesBehavior.ParseStateFromJson.InvalidJson",
                  "key '%s' not present in json", key.c_str())) {
    return GetStateID(config[key].asString());
  } else {
    JsonTools::PrintJsonError(
        config, "InternalStatesBehavior.ParseStateFromJson.InvalidJson", 2);
  }
  return InvalidStateID;
}

IBEIConditionPtr InternalStatesBehavior::ParseTransitionStrategy(
    const Json::Value& transitionConfig) {
  const Json::Value& strategyConfig = transitionConfig["condition"];
  if (ANKI_VERIFY(strategyConfig.isObject(),
                  "InternalStatesBehavior.ParseTransitionStrategy.NoCondition",
                  "Behavior '%s' missing condition config in a transition",
                  GetDebugLabel().c_str())) {
    // create state concept strategy from config
    return BEIConditionFactory::CreateBEICondition(
        transitionConfig["condition"], GetDebugLabel());
  } else {
    return nullptr;
  }
}

InternalStatesBehavior::TransitionType
InternalStatesBehavior::TransitionTypeFromString(const std::string& str) {
  if (str == "NonInterrupting")
    return TransitionType::NonInterrupting;
  else if (str == "Interrupting")
    return TransitionType::Interrupting;
  else if (str == "Exit")
    return TransitionType::Exit;
  else {
    PRINT_NAMED_ERROR(
        "InternalStatesBehavior.TransitionTypeFromString.InvalidString",
        "String '%s' does not match a transition type", str.c_str());
...
bool InternalStatesBehavior::TESTONLY_IsStateRunning(
    UnitTestKey key, const std::string& name) const {
