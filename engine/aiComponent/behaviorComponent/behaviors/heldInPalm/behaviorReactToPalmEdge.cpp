 ...
 * Created: 03/25/2019
 *
 * Description: Behavior for immediately responding to a robot driving both
...
// If the behavior encounters this many failures with the initial
// animation/action while activated, then just give up and go to
// ForceStuckOnPalmEdge.
CONSOLE_VAR(uint32_t, kMaxNumInitialReactAttemptsBeforeGivingUp, CONSOLE_GROUP,
            2);
}  // namespace

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToPalmEdge::GetBehaviorJsonKeys(
...
BehaviorReactToPalmEdge::InstanceConfig::InstanceConfig(
  ...
  if (JsonTools::GetValueOptional(config, kAnimOnActionFailureKey,
                                  animOnTurnFailureName)) {
    ...
    ANKI_VERIFY(animOnActionFailure != AnimationTrigger::Count,
                ...
                "%s is not a valid AnimationTrigger value",
                animOnTurnFailureName.c_str());
  }
}
...
void BehaviorReactToPalmEdge::InitBehavior() {
  ...
  _iConfig.askForHelpBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(ForceStuckOnPalmEdge));
  _iConfig.joltInPalmReaction =
...
bool BehaviorReactToPalmEdge::WantsToBeActivatedBehavior() const {
  ...
  // Robot should react iff it is on a user's palm, and two cliff sensors on the
  // same side (front or back) have detected a cliff/edge.
...
void BehaviorReactToPalmEdge::OnBehaviorActivated() {
  ...
  auto waitForStopLambda = [this, &robotInfo](Robot& robot) {
    ...
    LOG_INFO("BehaviorReactToPalmEdge.OnBehaviorActivated.CliffValsAfterStop",
             "%u %u %u %u (%x)", cliffData[0], cliffData[1], cliffData[2],
             cliffData[3], cliffFlags);
    ...
    // Send DAS event each time the robot is asked to react to a new palm edge.
    {
      ...
      DASMSG_SET(i1, _dVars.persistent.cliffFlagsAtStart,
  ...
  DelegateIfInControl(
      waitForStopAction,
      &BehaviorReactToPalmEdge::TransitionToPlayingInitialReaction);
}
...
void BehaviorReactToPalmEdge::TransitionToStuckOnPalmEdge() {
  ...
  if (ANKI_VERIFY(_iConfig.askForHelpBehavior->WantsToBeActivated(),
                  "BehaviorReactToPalmEdge.TransitionToStuckOnPalmEdge."
...
void BehaviorReactToPalmEdge::TransitionToPlayingInitialReaction() {
  ...
  if (!WantsToBeActivated()) {
    const auto& cliffComponent =
        GetBEI().GetRobotInfo().GetCliffSensorComponent();
    LOG_INFO("BehaviorReactToPalmEdge.TransitionToPlayingInitialReaction",
             "%s does NOT want to be activated prior to initial reaction. "
             "Cliff flags: %x",
             GetDebugLabel().c_str(), cliffComponent.GetCliffDetectedFlags());
    return;
  }
  ...
  // somehow stuck in some orientation that StuckOnPalmEdge is not detecting.
  // Just go to StuckOnPalmEdge to be safe.
  if (_dVars.persistent.numInitialReactAttempts >
  ...
  if (_dVars.persistent.cliffFlagsAtStart == 0) {
    ...
    LOG_INFO(
        "BehaviorReactToPalmEdge.TransitionToPlayingInitialReaction.NoCliffs",
        "");
    return;
  }
  ...
  if (action == nullptr) {
    ...
    DelegateIfInControl(action, [this](const ActionResult& res) {
      ...
      if (!WantsToBeActivated()) {
        // The action succeeded in moving the robot enough to get it away from
        // the edge of the palm, such that at most only part of the edge is
        // still detected by the cliff sensors.
        TransitionToFaceAndBackAwayFromEdge();
      } else {
        // Cliff reaction failed for some reason, something might be preventing
        // the robot from moving away from the edge, possibly because the treads
        // are slipping on the user's palm and therefore the robot is stuck on
        // the edge of the palm.
        if (_iConfig.animOnActionFailure != AnimationTrigger::Count) {
          // If the user provided an animation to play when this happens, play
          // the animation and then retry the initial reaction.
          auto animOnFail =
              new TriggerLiftSafeAnimationAction(_iConfig.animOnActionFailure);

          // Lock the tracks, in case the selected animation causes some tread
          // movement that might cause the robot to fall off the edge of the
          // user's palm.
          const u8 tracksToLock =
              Util::EnumToUnderlying(AnimTrackFlag::BODY_TRACK);
          ...
          // If no animation was specified by the user, simply retry the initial
          // reaction.
...
void BehaviorReactToPalmEdge::TransitionToRecoveryBackup() {
  ...
  if (cliffComp.IsCliffDetected()) {
    ...
    LOG_INFO(
        "BehaviorReactToPalmEdge.TransitionToRecoveryBackup."
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToPalmEdge::GetAllDelegates(
...
void BehaviorReactToPalmEdge::BehaviorUpdate() {
  ...
  if (_iConfig.joltInPalmReaction->WantsToBeActivated()) {
    if (_dVars.canBeInterruptedByJolt) {
      ...
      return;
    } else if (!_iConfig.joltInPalmReaction->IsActivated() &&
               !_dVars.hasTriggeredMoodEventForJolt) {
  ...
  const auto latestCliffFlags =
      GetBEI().GetCliffSensorComponent().GetCliffDetectedFlags();
  if (WantsToBeActivated() && _dVars.persistent.state != State::Activating &&
      latestCliffFlags != _dVars.persistent.cliffFlagsAtStart) {
    // One of the animations or actions we delegated to must have moved us over
    // a different edge edge of the palm when trying to back away from the
    // current edge, we should re-start the reaction.
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
IActionRunner* BehaviorReactToPalmEdge::GetInitialEdgeReactAction(
    const uint8_t cliffDetectedFlags) {
  IActionRunner* action = nullptr;

  LOG_INFO("ReactToPalmEdge.GetInitialEdgeReactAction.CliffsDetected", "0x%x",
           cliffDetectedFlags);
...
void BehaviorReactToPalmEdge::TransitionToFaceAndBackAwayFromEdge() {
  ...
  if (_dVars.persistent.cliffFlagsAtStart == kBackCliffSensors) {
    ...
    // Turn 180 degrees and animate simultaneously in order to face the cliff
    // behind us. The direction of the turn (left or right) is chosen at random.
    const float bodyTurnAngle =
        GetBEI().GetRNG().RandBool() ? M_PI : -1.0f * M_PI;
    AnimationTrigger anim = (bodyTurnAngle < 0.f)
                                ? AnimationTrigger::ReactToCliffTurnRight180
                                : AnimationTrigger::ReactToCliffTurnLeft180;
    ...
    compoundAction->AddAction(
        new WaitForLambdaAction(flagAsNonInterruptibleLambda));
    compoundAction->AddAction(new TriggerLiftSafeAnimationAction(
        AnimationTrigger::HeldOnPalmLookingNervous));
    action = compoundAction;
  } else {
    _dVars.canBeInterruptedByJolt = false;
    action = new TriggerLiftSafeAnimationAction(
        AnimationTrigger::HeldOnPalmLookingNervous);
  }
  ...
  DelegateIfInControl(action, [this](const ActionResult& res) {
    ...
    if (res != ActionResult::SUCCESS &&
        _iConfig.animOnActionFailure != AnimationTrigger::Count) {
      // If the action failed, play the animation selected/provided by the
      // configuration
      auto animOnFail =
          new TriggerLiftSafeAnimationAction(_iConfig.animOnActionFailure);

      // Lock the tracks, in case the selected animation causes some tread
      // movement that might cause the robot to fall off the edge of the user's
      // palm.
      const u8 tracksToLock = Util::EnumToUnderlying(AnimTrackFlag::BODY_TRACK);
      animOnFail->SetTracksToLock(tracksToLock);

      // Then execute a recovery backup action if the robot is still detecting
      // cliffs at this point.
      DelegateIfInControl(animOnFail,
                          &BehaviorReactToPalmEdge::TransitionToRecoveryBackup);
    } else {
      // Otherwise, simply proceed to check if we're still detecting cliffs, and
      // activate a recovery backup action if this is the case.
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToPalmEdge::SetState_internal(
    BehaviorReactToPalmEdge::State state, const std::string& stateName) {
