...
const char* const kAnimSelectorBehaviorName =
    "InitialHeldInPalmReactionInternal";
const char* const kBehaviorsThatInterruptInitialReaction =
    "interruptingBehaviors";
...
BehaviorInitialHeldInPalmReaction::InstanceConfig::InstanceConfig(
  ...
  if (JsonTools::GetVectorOptional(
          config, kBehaviorsThatInterruptInitialReaction, tmpNames)) {
...
void BehaviorInitialHeldInPalmReaction::InitBehavior() {
  const std::string debugStr =
      "BehaviorInitialHeldInPalmReaction.Constructor.BehaviorNotFound";

  // Find and cache the jolt-in-palm reaction behavior
  _iConfig.joltInPalmReaction =
      GetBEI().GetBehaviorContainer().FindBehaviorByID(kJoltBehaviorID);
  ...
  for (const auto& name : _iConfig.interruptingBehaviorNames) {
    ...
    if (ANKI_VERIFY(behavior != nullptr, debugStr.c_str(),
                    "Interrupting behavior %s could not be found",
                    name.c_str())) {
      _iConfig.interruptingBehaviors.insert(behavior);
    }
  }

  // Cache the animation selector behavior that is always delegated to upon
  // activation
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorInitialHeldInPalmReaction::GetBehaviorJsonKeys(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorInitialHeldInPalmReaction::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorInitialHeldInPalmReaction::GetAllDelegates(
...
void BehaviorInitialHeldInPalmReaction::OnBehaviorActivated() {
  ...
  auto callback = [this]() {
    if (!_dVars.animWasInterrupted && GetBEI().HasMoodManager()) {
      GetBEI().GetMoodManager().TriggerEmotionEvent(
          kEmotionEventOnSuccessfulAnim);
    }
    ...
    DASMSG_SET(i1, _dVars.animWasInterrupted,
               "Was initial get-in animation interrupted?");
    DASMSG_SEND();
  };
...
void BehaviorInitialHeldInPalmReaction::BehaviorUpdate() {
  if (IsActivated()) {
    // Interrupt the initial animation if either the jolt reaction or palm-tilt
    // reaction want to be activated. Record the fact that the animation was
    // interrupted so that the callback assigned to it in OnBehaviorActivated
    // publishes a DAS message that records a interrupted playthrough and does
    // NOT trigger the EmotionEvent associated with this behavior.
    if (!_dVars.animWasInterrupted) {
      if (!_iConfig.joltInPalmReaction->IsActivated() &&
        ...
        // The jolt reaction activation conditions may not hold true on the next
        // engine tick, so cancel the current delegate here now and switch to
        // the jolt reaction right away.
        CancelDelegates();
        DelegateIfInControl(_iConfig.joltInPalmReaction.get());
      } else if (BehaviorShouldBeInterrupted()) {
        _dVars.animWasInterrupted = true;
        // Just let the HeldInPalm dispatcher handle the disruption, it isn't
        // super-likely that the behavior that wants to interrupt and deactivate
        // this initial reaction will not want to activate on the next engine
        // tick.
        CancelSelf();
      }
    }
