 ...
 *  + Streams a users voice (assumed to be a question for Victor) to the cloud
 *which returns a response/answer string
 *
 **********************************************************************************************************************/
...
class BehaviorKnowledgeGraphQuestion : public ICozmoBehavior {
 ...
 protected:
  ...
  // translate that response intent into and answer string and store it for
  // later
  ...
  // streaming cues are backpack lights and audio cues to let the user know when
  // to speak
  void PlayEarconEnd();
 ...
 private:
  ...
  struct InstanceConfig {
    ...
    double
        streamingDuration;  // how long before we give up on streaming and bail
    std::string readyStringID;  // audio tts to let the user know they can begin
                                // speaking
  ...
  struct DynamicVariables {
    ...
    EState state;
    double streamingBeginTime;   // the time we begun actually streaming the mic
                                 // data
    std::string responseString;  // the text we got back from knowledge graph
    EGenerationStatus
        ttsGenerationStatus;  // track the status of the response tts
