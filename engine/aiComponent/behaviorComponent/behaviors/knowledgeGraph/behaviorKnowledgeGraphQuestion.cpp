 ...
 *
 *  Description
 *  + Parent behavior for allowing the user to record a message and save it to
 *Victor's local storage.
 *
 **********************************************************************************************************************/

// #include
// "engine/aiComponent/behaviorComponent/behaviors/knowledgeGraph/behaviorKnowledgeGraphQuestion.h"
...
#define PRINT_DEBUG(format, ...)                                             \
  PRINT_CH_DEBUG("KnowledgeGraph", "BehaviorKnowledgeGraphQuestion", format, \
                 ##__VA_ARGS__)

#define PRINT_INFO(format, ...)                                             \
  PRINT_CH_INFO("KnowledgeGraph", "BehaviorKnowledgeGraphQuestion", format, \
                ##__VA_ARGS__)
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorKnowledgeGraphQuestion::BehaviorKnowledgeGraphQuestion(
  ...
  _iVars.readyStringID =
      JsonTools::ParseString(config, kKey_ReadyStringID, kDefaultReadyStringID);

  std::string earConString;
  if (JsonTools::GetValueOptional(config, kKey_EarConEnd, earConString)) {
    _iVars.earConEnd =
        AudioMetaData::GameEvent::GenericEventFromString(earConString);
  }

  SubscribeToTags({{
      ExternalInterface::MessageEngineToGameTag::TouchButtonEvent,
      ExternalInterface::MessageEngineToGameTag::
          RobotFallingEvent,  // do we need this?
  }});
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorKnowledgeGraphQuestion::GetBehaviorJsonKeys(
...
void BehaviorKnowledgeGraphQuestion::InitBehavior() {
  ...
  container.FindBehaviorByIDAndDowncast<BehaviorTextToSpeechLoop>(
      BEHAVIOR_ID(KnowledgeGraphTTS), BEHAVIOR_CLASS(TextToSpeechLoop),
      _iVars.ttsBehavior);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorKnowledgeGraphQuestion::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorKnowledgeGraphQuestion::GetBehaviorOperationModifiers(
...
void BehaviorKnowledgeGraphQuestion::OnBehaviorActivated() {
  ...
  const std::string& readyText =
      localeComponent.GetString(_iVars.readyStringID);

  // start generating our ready text; if we fail, then we'll simply exit the
  // behavior, and cry :(
  if (_readyTTSWrapper.SetUtteranceText(readyText, {})) {
    auto callback = [this]() {
      ...
      // Need to loop this forever and we'll just cancel it on our own after a
      // timeout
      DelegateIfInControl(new ReselectingLoopAnimationAction(
          AnimationTrigger::KnowledgeGraphListening));
    };

    // open up streaming after we play our get-in to avoid motor noise
    DelegateIfInControl(new TriggerLiftSafeAnimationAction(
                            AnimationTrigger::KnowledgeGraphGetIn),
                        callback);
  } else {
    PRINT_NAMED_WARNING("BehaviorKnowledgeGraphQuestion",
                        "Failed to generate Ready TTS (%s)", readyText.c_str());
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorKnowledgeGraphQuestion::HandleWhileActivated(
...
void BehaviorKnowledgeGraphQuestion::BehaviorUpdate() {
  if (IsActivated()) {
    ...
    // at this point our get in animation is complete, so as soon as the audio
    // is finished playing we can transition in
    if (EState::WaitingToStream == _dVars.state) {
      // once we've finished speaking our ready text, we can start streaming
      // hopefully the ready text is already finished by the time we even get
      // into this state
      if (_readyTTSWrapper.IsFinished()) {
        BeginStreamingQuestion();
      } else if (!_readyTTSWrapper.IsValid()) {
        PRINT_NAMED_WARNING("BehaviorKnowledgeGraphQuestion",
                            "Ready prompt TTS failed to play, not cool man");

        // we need to bail, luckily TransitionToNoResponse() handles this exact
        // transition for us
        CancelDelegates(false);
        TransitionToNoResponse();
      }
    }
    // if we're recording the user's question, we need to be listening for a
    // response
    else if (EState::Listening == _dVars.state) {
      // if we've gotten a response from knowledge graph, transition into the
      // response state
      const double currentTime =
          BaseStationTimer::getInstance()->GetCurrentTimeInSecondsDouble();
      if (FLT_NEAR(_dVars.streamingBeginTime, 0.f) && uic.IsCloudStreamOpen()) {
        _dVars.streamingBeginTime = currentTime;
      }

      const bool timeIsUp = (!FLT_NEAR(_dVars.streamingBeginTime, 0.f)) &&
                            (currentTime >= (_dVars.streamingBeginTime +
      ...
      // require victor to be on the ground prior to being picked up, else
      // ignore it if he's already in the air
...
void BehaviorKnowledgeGraphQuestion::BeginStreamingQuestion() {
  ...
  GetBehaviorComp<UserIntentComponent>().StartWakeWordlessStreaming(
      CloudMic::StreamType::KnowledgeGraph);
}
...
void BehaviorKnowledgeGraphQuestion::OnStreamingComplete() {
  ...
  if (!_dVars.responseString.empty()) {
    ...
    // since we always want to loop at least once, play the get in + loop anim
    // before we do any logic for the tts
    CompoundActionSequential* messageAnimation = new CompoundActionSequential();
    messageAnimation->AddAction(
        new TriggerLiftSafeAnimationAction(
            AnimationTrigger::KnowledgeGraphSearchingGetIn),
        true);
    messageAnimation->AddAction(new TriggerLiftSafeAnimationAction(
                                    AnimationTrigger::KnowledgeGraphSearching),
                                true);
...
void BehaviorKnowledgeGraphQuestion::ConsumeResponse() {
  ...
  // if we have a valid knowledge graph response intent, grab the response
  // string from it if it's not the knowledge graph response, we simply return
  // the empty string which will be handled appropriately
  UserIntentComponent& uic = GetBehaviorComp<UserIntentComponent>();
  ...
  if (uic.IsUserIntentPending(USER_INTENT(knowledge_response), intent)) {
    ...
    const UserIntent_KnowledgeResponse& intentResponse =
        intent.Get_knowledge_response();
    _dVars.responseString = intentResponse.answer;

    PRINT_DEBUG("Knowledge Graph Question: %s",
                Util::HidePersonallyIdentifiableInfo(
                    intentResponse.query_text.c_str()));
    PRINT_DEBUG(
        "Knowledge Graph Response: %s",
        Util::HidePersonallyIdentifiableInfo(intentResponse.answer.c_str()));
    ...
    PRINT_NAMED_WARNING(
        ...
        "unmatched_intent returned as response from knowledge graph");
  } else {
#if ALLOW_DEBUG_LOGGING
    {
      ...
      const std::string intentString = UserIntentTagToString(
          intentData ? intentData->intent.GetTag() : UserIntentTag::INVALID);
      PRINT_NAMED_ERROR("BehaviorKnowledgeGraphQuestion",
                        "Invalid intent returned from Knowledge Graph: %s",
                        intentString.c_str());
    }
#endif
  }
...
void BehaviorKnowledgeGraphQuestion::TransitionToSearchingLoop() {
  ...
  if (EGenerationStatus::None != _dVars.ttsGenerationStatus) {
    if (EGenerationStatus::Success == _dVars.ttsGenerationStatus) {
      // TTS generation is done, so let's transition out of the searching
      // animation and into the hearts all over the world!!!
      DelegateIfInControl(
          new TriggerLiftSafeAnimationAction(
              AnimationTrigger::KnowledgeGraphSearchingGetOutSuccess),
          &BehaviorKnowledgeGraphQuestion::TransitionToBeginResponse);
    } else {
      // we failed to generate the tts, but since we've already looped our
      // searching anim, we just need to get out now
      CompoundActionSequential* messageAnimation =
          new CompoundActionSequential();
      messageAnimation->AddAction(
          new TriggerLiftSafeAnimationAction(
              AnimationTrigger::KnowledgeGraphSearchingFail),
          true);
      messageAnimation->AddAction(
          new TriggerLiftSafeAnimationAction(
...
void BehaviorKnowledgeGraphQuestion::TransitionToBeginResponse() {
  ...
  DEV_ASSERT(
      !_dVars.responseString.empty(),
      "Responding to knowledge graph request but no response string exists");
...
void BehaviorKnowledgeGraphQuestion::OnResponseInterrupted() {
  DEV_ASSERT(EState::Responding == _dVars.state,
             "Should only allow interruptions during response state");
...
void BehaviorKnowledgeGraphQuestion::PlayEarconEnd() {
  ...
  if (GenericEvent::Invalid != _iVars.earConEnd) {
    ...
    bei.GetRobotAudioClient().PostEvent(
        _iVars.earConEnd, AudioMetaData::GameObjectType::Behavior);
  }
}
