...
// + user metadata eg. VisionComponent::IsNameTaken(...) should be moved outside
// of the vision system
// + record/playback can be interrupted by "petting him", picking him up, or
// wakeword?
...
BehaviorLeaveAMessage::InstanceConfig::InstanceConfig()
    : recordDuration(kDefaultDuration), requireKnownUser(true) {}
...
BehaviorLeaveAMessage::BehaviorLeaveAMessage(const Json::Value& config)
  ...
  _iVars.ttsNoRecipientKey = JsonTools::ParseString(
      config, kKey_ttsNoRecipientKey, "BehaviorLeaveAMessage.NoRecipient");
  _iVars.ttsUnknownRecipientKey =
      JsonTools::ParseString(config, kKey_ttsUnknownRecipientKey,
                             "BehaviorLeaveAMessage.UnknownRecipient");
  _iVars.ttsMailboxFullKey = JsonTools::ParseString(
      config, kKey_ttsMailboxFullKey, "BehaviorLeaveAMessage.MailboxFull");

  JsonTools::GetValueOptional(config, kKey_Duration, _iVars.recordDuration);
  JsonTools::GetValueOptional(config, kKey_RequireKnownUser,
                              _iVars.requireKnownUser);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorLeaveAMessage::GetBehaviorJsonKeys(
...
void BehaviorLeaveAMessage::InitBehavior() {
  ...
  container.FindBehaviorByIDAndDowncast<BehaviorTextToSpeechLoop>(
      BEHAVIOR_ID(MessagingRecordTTS), BEHAVIOR_CLASS(TextToSpeechLoop),
      _iVars.ttsBehavior);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorLeaveAMessage::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorLeaveAMessage::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorLeaveAMessage::WantsToBeActivatedBehavior() const { return true; }
...
void BehaviorLeaveAMessage::OnBehaviorActivated() {
  ...
  UserIntentPtr activeIntentPtr =
      uic.GetUserIntentIfActive(USER_INTENT(message_record));
  ...
  const UserIntent_RecordMessage& messageIntent =
      activeIntentPtr->intent.Get_message_record();
  _dVars.messageRecipient = messageIntent.given_name;

  RecordMessageResult result = {.id = kInvalidVoiceMessageId,
                                .error = EVoiceMessageError::InvalidMessage};

  // we require a specified user to deliver the message to; this user is
  // expected to be a known user that has been enrolled to victor's knowledge
  // base (via meet victor)
  if (!_dVars.messageRecipient.empty()) {
    ...
    if (!DoesRequireKnownUser() ||
        vision.IsNameTaken(_dVars.messageRecipient)) {
      ...
      result = voicemail.StartRecordingMessage(_dVars.messageRecipient,
                                               duration, onRecordingFinished);

      if (EVoiceMessageError::Success == result.error) {
        PRINT_DEBUG("Leaving a message for (%s)",
                    Util::HidePersonallyIdentifiableInfo(
                        _dVars.messageRecipient.c_str()));
  ...
  switch (result.error) {
    ...
    case EVoiceMessageError::MessageAlreadyActive:
      DEV_ASSERT_MSG(false, "BehaviorLeaveAMessage.OnBehaviorActivated",
                     "VoiceMessageSystem::StartRecordingMessage(...) return "
                     "error that we did not handle [%d]",
                     (int)result.error);
      CancelSelf();
      break;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorLeaveAMessage::PlayTextToSpeech(const std::string& ttsString,
...
void BehaviorLeaveAMessage::TransitionToRecordingMessage() {
  ...
  messageAnimation->AddAction(
      ...
      true);
  messageAnimation->AddAction(new ReselectingLoopAnimationAction(
...
void BehaviorLeaveAMessage::OnMessagedRecordingComplete() {
  ...
  CompoundActionSequential* messageAnimation = new CompoundActionSequential();
  messageAnimation->AddAction(
      new TriggerAnimationAction(AnimationTrigger::MessagingMessageGetOut),
      true);
  messageAnimation->AddAction(new TriggerAnimationAction(
      AnimationTrigger::MessagingMessageRecordReaction));
...
void BehaviorLeaveAMessage::TransitionToInvalidRecipient() {
  ...
  if (_dVars.messageRecipient.empty()) {
    ...
    const std::string& text =
        localeComponent.GetString(_iVars.ttsNoRecipientKey);
    PlayTextToSpeech(text);
  } else {
    // we don't know the recipient
    const std::string& text = localeComponent.GetString(
        _iVars.ttsUnknownRecipientKey, _dVars.messageRecipient);
    PlayTextToSpeech(text);
  }
}
