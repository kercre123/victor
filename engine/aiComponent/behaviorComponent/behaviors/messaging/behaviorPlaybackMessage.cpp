...
// todo notes:
// + move the part where we say what messages we have (PlayNextRecipientTTS())
// into it's own behavior as this will be used elsewhere
// + there are many common helper functions and things used between the
// messaging behavior, create a helpers class

#define PRINT_DEBUG(format, ...)                                    \
  PRINT_CH_DEBUG("VoiceMessage", "BehaviorPlaybackMessage", format, \
                 ##__VA_ARGS__)

#define PRINT_INFO(format, ...)                                    \
  PRINT_CH_INFO("VoiceMessage", "BehaviorPlaybackMessage", format, \
                ##__VA_ARGS__)
...
BehaviorPlaybackMessage::BehaviorPlaybackMessage(const Json::Value& config)
  ...
  _iVars.ttsAnnounceSingleKey = JsonTools::ParseString(
      config, kTTSAnnounceSingleKey, "BehaviorPlaybackMessage.AnnounceSingle");
  _iVars.ttsAnnouncePluralKey = JsonTools::ParseString(
      config, kTTSAnnouncePluralKey, "BehaviorPlaybackMessage.AnnouncePlural");
  _iVars.ttsNoRecipientKey = JsonTools::ParseString(
      config, kTTSNoRecipientKey, "BehaviorPlaybackMessage.NoRecipient");
  _iVars.ttsNoMessagesKey = JsonTools::ParseString(
      config, kTTSNoMessagesKey, "BehaviorPlaybackMessage.NoMessages");
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPlaybackMessage::GetBehaviorJsonKeys(
...
void BehaviorPlaybackMessage::InitBehavior() {
  ...
  container.FindBehaviorByIDAndDowncast<BehaviorTextToSpeechLoop>(
      BEHAVIOR_ID(MessagingPlaybackTTS), BEHAVIOR_CLASS(TextToSpeechLoop),
      _iVars.ttsBehavior);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPlaybackMessage::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPlaybackMessage::GetBehaviorOperationModifiers(
...
void BehaviorPlaybackMessage::OnBehaviorActivated() {
  ...
  UserIntentPtr activeIntentPtr =
      uic.GetUserIntentIfActive(USER_INTENT(message_playback));

  DEV_ASSERT_MSG(
      activeIntentPtr != nullptr &&
      ...
      activeIntentPtr ? UserIntentTagToString(activeIntentPtr->intent.GetTag())
                      : "NULL");

  // store the intended recipient to use either for playback or error response
  const UserIntent_PlaybackMessage& messageIntent =
      activeIntentPtr->intent.Get_message_playback();
  ...
  if (!_dVars.messageRecipient.empty()) {
    ...
    VoiceMessageSystem& voicemail =
        GetBEI().GetMicComponent().GetVoiceMessageSystem();
    _dVars.messages = voicemail.GetUnreadMessages(
        _dVars.messageRecipient, VoiceMessageSystem::SortType::NewestToOldest);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPlaybackMessage::PlayTextToSpeech(
    const std::string& text, BehaviorSimpleCallback callback) {
...
void BehaviorPlaybackMessage::PlaybackNextMessage() {
  ...
  if (!_dVars.messages.empty()) {
    ...
    // we assume any get-in animation has already been played, so we simply jump
    // right into the looping anim
    DelegateIfInControl(new ReselectingLoopAnimationAction(
        AnimationTrigger::MessagingMessageLoop));
  }
}
...
void BehaviorPlaybackMessage::OnMessagePlaybackComplete(VoiceMessageID id) {
  ...
  DelegateIfInControl(new TriggerAnimationAction(
                          AnimationTrigger::MessagingMessageDeletedShort),
                      [this]() {
...
void BehaviorPlaybackMessage::TransitionToPlayingFirstMessage() {
  ...
  const std::string& announceKey =
      (isPlural ? _iVars.ttsAnnouncePluralKey : _iVars.ttsAnnounceSingleKey);
  ...
  std::string textToSay =
      localeComponent.GetString(announceKey, recipient, count);

  PlayTextToSpeech(textToSay, [this]() {
    // after we've made our announcement, play some anims then get into the
    // message delivery
    CompoundActionSequential* messageAnimation = new CompoundActionSequential();
    messageAnimation->AddAction(
        new TriggerAnimationAction(AnimationTrigger::MessagingMessageGetIn),
        true);
    messageAnimation->AddAction(
        new TriggerAnimationAction(AnimationTrigger::MessagingMessageRewind));
...
void BehaviorPlaybackMessage::TransitionToPlayingNextMessage() {
  ...
  TriggerAnimationAction* animationAction =
      new TriggerAnimationAction(AnimationTrigger::MessagingMessageGetIn);
...
void BehaviorPlaybackMessage::TransitionToFailureResponse() {
  ...
  if (_dVars.allMessages.empty()) {
    ...
    // if we have messages for people, but not the user they asked for, simply
    // rhyme off the names that we have
...
void BehaviorPlaybackMessage::TransitionToNoMessagesResponse() {
  DEV_ASSERT_MSG(_dVars.messages.empty(),
                 ...
                 "Transitioning to the no messages state but we actually do "
                 "have messages!");
  const auto& bei = GetBEI();
  const auto& robotInfo = bei.GetRobotInfo();
  const auto& localeComponent = robotInfo.GetLocaleComponent();
  const auto& text = localeComponent.GetString(_iVars.ttsNoMessagesKey,
                                               _dVars.messageRecipient);
  PlayTextToSpeech(text);
}
...
void BehaviorPlaybackMessage::PlayNextRecipientTTS() {
  if (!_dVars.allMessages.empty()) {
    // this allows us to reuse the identifier replacement in
    // PlayTextToSpeech(...)
    ...
    const auto& localeComponent = robotInfo.GetLocaleComponent();
    const bool isPlural = (_dVars.messages.size() > 1);
    const auto& ttsAnnounceKey =
        (isPlural ? _iVars.ttsAnnouncePluralKey : _iVars.ttsAnnounceSingleKey);
    const auto& text =
        localeComponent.GetString(ttsAnnounceKey, _dVars.messageRecipient,
                                  std::to_string(_dVars.messages.size()));
