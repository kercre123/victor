...
bool BehaviorSimpleVoiceResponse::WantsToBeActivatedBehavior() const {
  ...
  const bool pendingIntent =
      uic.IsUserIntentPending(USER_INTENT(simple_voice_response));
  return pendingIntent;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorSimpleVoiceResponse::GetBehaviorOperationModifiers(
...
void BehaviorSimpleVoiceResponse::OnBehaviorActivated() {
  const UserIntentPtr intentData =
      SmartActivateUserIntent(USER_INTENT(simple_voice_response));
  const MetaUserIntent_SimpleVoiceResponse& response =
      intentData->intent.Get_simple_voice_response();
