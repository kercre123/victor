...
BehaviorCoordinateWeather::BehaviorCoordinateWeather(const Json::Value& config)
  ...
  if (ANKI_VERIFY(isMember,
    ...
    for (const auto& entry : config[kResponseMapKey]) {
      const auto behaviorIDStr = BehaviorTypesWrapper::BehaviorIDFromString(
          entry[kBehaviorIDKey].asString());
      _iConfig.behaviorIDs.push_back(behaviorIDStr);
      _iConfig.conditions.push_back(
          WeatherConditionTypeFromString(entry[kCladTypeKey].asString()));
    }
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorCoordinateWeather::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorCoordinateWeather::GetLinkedActivatableScopeBehaviors(
    std::set<IBehavior*>& delegates) const {
  for (const auto& pair : _iConfig.weatherBehaviorMap) {
    delegates.insert(pair.second.get());
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorCoordinateWeather::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorCoordinateWeather::GetBehaviorJsonKeys(
...
void BehaviorCoordinateWeather::InitBehavior() {
  ...
  for (auto i = 0; i < _iConfig.behaviorIDs.size(); i++) {
    const auto& behaviorPtr =
        behaviorContainer.FindBehaviorByID(_iConfig.behaviorIDs[i]);

    if (behaviorPtr != nullptr) {
      _iConfig.weatherBehaviorMap.emplace(_iConfig.conditions[i], behaviorPtr);
    } else {
      PRINT_NAMED_ERROR(
          "BehaviorCoordinateWeather.InitBehavior.MissingBehavior",
          "BehaviorID %s not found in container - no behavior will be added "
          "for condition %s",
          EnumToString(_iConfig.behaviorIDs[i]),
          EnumToString(_iConfig.conditions[i]));
    }
  }
  _iConfig.behaviorIDs.clear();

  _iConfig.iCantDoThatBehavior =
      behaviorContainer.FindBehaviorByID(BEHAVIOR_ID(SingletonICantDoThat));
...
void BehaviorCoordinateWeather::OnBehaviorActivated() {
  ...
  const bool isPending =
      uic.IsUserIntentPending(USER_INTENT(weather_response), intent);
  if (!ANKI_VERIFY(
          isPending,
  ...
  if (_iConfig.intentParser->IsForecast(weatherResponse)) {
    ...
    if ((iter != _iConfig.weatherBehaviorMap.end()) &&
      ...
      cantDoThat = true;
      PRINT_NAMED_WARNING(
          "BehaviorCoordinateWeather.OnBehaviorActivated."
          "NoResponseForCondition",
