...
const Vision::SpriteBoxName kDigitSpriteBoxes[]{
    Vision::SpriteBoxName::SpriteBox_1, Vision::SpriteBoxName::SpriteBox_2,
    Vision::SpriteBoxName::SpriteBox_3, Vision::SpriteBoxName::SpriteBox_4,
...
BehaviorDisplayWeather::BehaviorDisplayWeather(const Json::Value& config)
  ...
  _iConfig.animationName = JsonTools::ParseString(
      config, kAnimationNameKey,
      "BehaviorDisplayWeather.Constructor.MissingAnimName");
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorDisplayWeather::WantsToBeActivatedBehavior() const { return true; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDisplayWeather::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDisplayWeather::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kAnimationNameKey};
  expectedKeys.insert(std::begin(list), std::end(list));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDisplayWeather::GetAllDelegates(
...
void BehaviorDisplayWeather::InitBehavior() {
  ...
  _iConfig.intentParser = std::make_unique<WeatherIntentParser>(
      dac.GetWeatherResponseMap(), dac.GetWeatherRemaps());

  auto& behaviorContainer = GetBEI().GetBehaviorContainer();
  _iConfig.lookAtFaceInFront = behaviorContainer.FindBehaviorByID(
      BEHAVIOR_ID(SingletonFindFaceInFrontWallTime));
}
...
void BehaviorDisplayWeather::OnBehaviorActivated() {
  ...
  _dVars.currentIntent =
      uic.GetUserIntentIfActive(USER_INTENT(weather_response));
  DEV_ASSERT(_dVars.currentIntent != nullptr,
             "BehaviorDisplayWeather.InvalidTriggeringIntent");

  const auto& weatherResponse =
      _dVars.currentIntent->intent.Get_weather_response();
...
void BehaviorDisplayWeather::TransitionToFindFaceInFront() {
  ANKI_VERIFY(_iConfig.lookAtFaceInFront->WantsToBeActivated(),
              "BehaviorDisplayWeather.TransitionToFindFaceInFront."
              "BehaviorDoesNotWantToBeActivated",
              "");
  // We should see a face during this behavior if there's one in front of us to
  // center on
  DelegateIfInControl(_iConfig.lookAtFaceInFront.get(), [this]() {
    DelegateIfInControl(new TriggerLiftSafeAnimationAction(
        AnimationTrigger::LookAtUserEndearingly));
  });
}
...
void BehaviorDisplayWeather::TransitionToDisplayWeatherResponse() {
  ...
  auto animationCallback = [this](const AnimationComponent::AnimResult res,
                                  u32 streamTimeAnimEnded) { CancelSelf(); };

  int temperature = 0;
  auto success =
      _iConfig.intentParser->GetRawTemperature(weatherResponse, temperature);
  const bool isFahrenheit =
      _iConfig.intentParser->IsFahrenheit(weatherResponse);
  AnimationComponent::RemapMap spriteBoxRemaps;
  success &=
      GenerateTemperatureRemaps(temperature, isFahrenheit, spriteBoxRemaps);
  if (!success) {
    return;
  }
  ...
  const Result result =
      GetBEI().GetAnimationComponent().PlayAnimWithSpriteBoxRemaps(
          _iConfig.animationName, spriteBoxRemaps, interruptRunning,
          animationCallback);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorDisplayWeather::GenerateTemperatureRemaps(
  ...
  const DigitMap& digitMap = (ABS(temp) < 10)
                                 ? kSingleDigitMap
                                 : ((ABS(temp) >= 10) && (ABS(temp) < 100))
                                       ? kDoubleDigitMap
                                       : kTripleDigitMap;

  if (temp < 0) {
    spriteBoxRemaps[digitMap.at(DigitType::NegativeSymbol)] =
        kNegativeTempIndicatorSpriteID;
  }

  spriteBoxRemaps[digitMap.at(DigitType::Ones)] =
      kTemperatureAssets[ABS(temp) % 10];

  if ((kDoubleDigitMap == digitMap) || (kTripleDigitMap == digitMap)) {
    spriteBoxRemaps[digitMap.at(DigitType::Tens)] =
        kTemperatureAssets[(ABS(temp) / 10) % 10];
  }

  if (kTripleDigitMap == digitMap) {
    spriteBoxRemaps[digitMap.at(DigitType::Hundreds)] =
        kTemperatureAssets[(ABS(temp) / 100) % 10];
  }

  spriteBoxRemaps[digitMap.at(DigitType::DegreeSymbol)] =
      isFahrenheit ? kFahrenheitIndicatorSpriteID : kCelsiusIndicatorSpriteID;
...
void BehaviorDisplayWeather::StartTTSGeneration() {
  ...
  const auto& ttsMap =
      bei.GetDataAccessorComponent().GetWeatherConditionTTSMap();
  ...
  if (success && ttsIter != ttsMap->end()) {
    ...
    const auto& ttsString =
        localeComponent.GetString(ttsIter->second, std::to_string(temperature));

    // Generate TTS utterance for localized string using KeyFrame type trigger
    // so that it plays at a time determined from within the animation
    ...
    _dVars.utteranceID =
        ttsCoordinator.CreateUtterance(ttsString, triggerType, style, callback);
  }
