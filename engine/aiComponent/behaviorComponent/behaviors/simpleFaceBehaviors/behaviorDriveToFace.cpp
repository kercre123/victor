...
const char* const kFindFaceBehaviorKey =
    "findFaceBehavior";  // must be a BehaviorFindFaceAndThen
...
BehaviorDriveToFace::BehaviorDriveToFace(const Json::Value& config)
  ...
  _iConfig.minDriveToFaceDistance_mm =
      JsonTools::ParseFloat(config, kMinDriveToFaceDistanceKey, kDebugName);

  _iConfig.trackFaceOnceKnown =
      config.get(kTrackFaceOnceKnownKey, true).asBool();
  if (_iConfig.trackFaceOnceKnown) {
    _iConfig.timeUntilCancelTracking_s =
        JsonTools::ParseFloat(config, kTimeUntilCancelFaceTrackKey, kDebugName);
  } else if (config.isMember(kTimeUntilCancelFaceTrackKey)) {
    PRINT_NAMED_WARNING("BehaviorDriveToFace.InvalidConfig.UselessKey",
                        "%s: Key '%s' specified, but no tracking ('%s' not "
                        "true). Will be ignored",
                        GetDebugLabel().c_str(), kTimeUntilCancelFaceTrackKey,
                        kTrackFaceOnceKnownKey);
  }

  JsonTools::GetCladEnumFromJSON(config, kAnimTooCloseKey,
                                 _iConfig.animTooClose, kDebugName);
  _iConfig.animDriveOverride = AnimationTrigger::Count;
  JsonTools::GetCladEnumFromJSON(config, kAnimDriveOverrideKey,
                                 _iConfig.animDriveOverride, kDebugName, false);
  _iConfig.initialAnim = AnimationTrigger::Count;
  JsonTools::GetCladEnumFromJSON(config, kInitialAnimKey, _iConfig.initialAnim,
                                 kDebugName, false);
  _iConfig.successAnim = AnimationTrigger::Count;
  JsonTools::GetCladEnumFromJSON(config, kSuccessAnimKey, _iConfig.successAnim,
                                 kDebugName, false);
  _iConfig.resumeAnim = AnimationTrigger::Count;
  JsonTools::GetCladEnumFromJSON(config, kResumeAnimKey, _iConfig.resumeAnim,
                                 kDebugName, false);

  _iConfig.maxNumResumes = config.get(kMaxNumResumesKey, 0).asInt();
  ANKI_VERIFY((_iConfig.maxNumResumes == 0) ==
                  (_iConfig.resumeAnim == AnimationTrigger::Count),
              "BehaviorDriveToFace.Ctor.ResumeAnimWithoutCount",
              "maxNumResumes OR resumeAnimation was provided, but expecting "
              "both or neither");

  _iConfig.maxNumFailures = config.get(kMaxNumFailuresKey, 2).asInt();

  _iConfig.findFaceBehaviorStr =
      JsonTools::ParseString(config, kFindFaceBehaviorKey, kDebugName);

  if (_iConfig.animDriveOverride != AnimationTrigger::Count &&
      (_iConfig.initialAnim != AnimationTrigger::Count ||
       _iConfig.successAnim != AnimationTrigger::Count)) {
    PRINT_NAMED_WARNING("BehaviorDriveToFace.InvalidConfig.AnimConflict",
                        "Behavior '%s' specified json key '%s' and at least "
                        "one of '%s' or '%s', which won't have effect",
                        GetDebugLabel().c_str(), kAnimDriveOverrideKey,
                        kInitialAnimKey, kSuccessAnimKey);
  }
  ...
  MakeMemberTunable(_iConfig.timeUntilCancelTracking_s,
                    kTimeUntilCancelFaceTrackKey, kDebugName);
  MakeMemberTunable(_iConfig.minDriveToFaceDistance_mm,
                    kMinDriveToFaceDistanceKey, kDebugName);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDriveToFace::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDriveToFace::GetBehaviorJsonKeys(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDriveToFace::GetAllDelegates(
...
void BehaviorDriveToFace::InitBehavior() {
  ...
  _iConfig.findFaceBehavior =
      std::dynamic_pointer_cast<BehaviorFindFaceAndThen>(behavior);
...
bool BehaviorDriveToFace::WantsToBeActivatedBehavior() const {
  ...
  const bool recentTick =
      (_dVars.interruptionEndTick != 0) &&
      (currTick < _dVars.interruptionEndTick + kMaxTicksAllowResume);
  const bool shouldResume = recentTick && (_dVars.numResumesRemaining > 0) &&
                            (_iConfig.resumeAnim != AnimationTrigger::Count);
  return (_iConfig.findFaceBehavior != nullptr) &&
         (isVoiceCommand || shouldResume);
}
...
void BehaviorDriveToFace::TransitionToDrivingToFace() {
  ...
  if (facePtr != nullptr &&
    ...
    if (_iConfig.maxNumResumes > 0) {
      // if the behavior gets interrupted during this action, flag that we want
      // to restart the behavior with a resume animation
      _dVars.numResumesRemaining = _iConfig.maxNumResumes;
    }

    GetBEI().GetMoodManager().TriggerEmotionEvent("DrivingToFace");

    // Set pose now (rather than when anim finishes) in case the anim turns the
    // robot. Do not pick a pose that would cross a cliff.
    const auto& robotPose = GetBEI().GetRobotInfo().GetPose();

    auto crossesCliff = [this, &robotPose](const auto& candidatePose) {
      auto isCliffType = [](const auto& data) {
        return (data->type == MemoryMapTypes::EContentType::Cliff);
      };
      const auto& mapComp = GetBEI().GetMapComponent();
      return mapComp.CheckForCollisions(
          FastPolygon(
              {robotPose.GetTranslation(), candidatePose.GetTranslation()}),
          isCliffType);
    };

    float distanceToTranslate =
        std::max(0.f, distToHead - _iConfig.minDriveToFaceDistance_mm);
    auto targetPose = robotPose;
    targetPose.TranslateForward(distanceToTranslate);

    if (crossesCliff(targetPose)) {
      // Refine the target pose so that it is close to the cliff but does not
      // cross it (binary search)
      size_t nIterations = 6;
      for (int i = 0; i < nIterations; i++) {
        distanceToTranslate /= 2;
        targetPose.TranslateForward(crossesCliff(targetPose)
                                        ? -distanceToTranslate
                                        : distanceToTranslate);
      }
    ...
    DelegateIfInControl(action, [this](ActionResult res) {
      if (res == ActionResult::SUCCESS) {
        ...
        TransitionToFindingFace();  // note this may fail to find the face now,
                                    // at which point the behavior ends
      }
...
void BehaviorDriveToFace::TransitionToPlayingFinalAnim() {
  ...
  if (_iConfig.successAnim != AnimationTrigger::Count) {
    DelegateIfInControl(new TriggerAnimationAction(_iConfig.successAnim),
...
void BehaviorDriveToFace::TransitionToResumeAnimation() {
  ...
  DelegateIfInControl(new TriggerAnimationAction{
      _iConfig.resumeAnim});  // and then the behavior ends
}

//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
//- - - - - - - - - - - - - - - - - - - -
void BehaviorDriveToFace::SetState_internal(State state,
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
//- - - - - - - - - - - - - - - - - - - -
bool BehaviorDriveToFace::IsRobotAlreadyCloseEnoughToFace(
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
//- - - - - - - - - - - - - - - - - - - -
bool BehaviorDriveToFace::CalculateDistanceToFace(Vision::FaceID_t faceID,
                                                  float& distance) {
  ...
  if (facePtr != nullptr) {
    ...
    return ComputeDistanceBetween(headPoseModified, robotInfo.GetPose(),
                                  distance);
  }
