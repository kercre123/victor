...
class BehaviorFindFaceAndThen : public ICozmoBehavior {
 public:
  // Returns the face found while running this behavior. It is reset upon
  // activation, and persists after deactivation.
 ...
 private:
  ...
  struct InstanceConfig {
    ...
    // when looking in a specific direction for a face, _extend_
    // timeUntilCancelFaceLooking_s by this much time if a body is seen, and run
    // a find-face-from-body behavior for the remainder
    float additionalLookTimeIfSawBody_s;
    // when searching all over for a face, _extend_
    // timeUntilCancelFaceSearching_s by this much time, and run the
    // find-face-from-body behavior for only this long. Afterwards, if the total
    // time searching for a face is still less than the updated
    // timeUntilCancelSearching_s, continue the original search behavior
    float additionalSearchTimeIfSawBody_s;

    // if it starts on the charger, if can either leave the charger before
    // looking for a face, or stay on the charger looking for a face. If it
    // leaves the charger, it will subsequently turn to the last seen face
    bool shouldLeaveChargerFirst;

    // if true, this behavior assumes it started facing in the dominant mic
    // direction, unless it was on the charger.
    bool startedWithMicDirection;

    std::string searchBehaviorID;
    ICozmoBehaviorPtr searchFaceBehavior;  // if set, this behavior will search
                                           // for a face if one is not found
  ...
  // If there is a face, and it is the most recent, and it shares the same
  // origin, this returns true and sets the params
  bool GetRecentFaceSince(RobotTimeStamp_t sinceTime_ms, SmartFaceID& faceID,
                          RobotTimeStamp_t& timeStamp_ms);
  bool GetRecentFace(SmartFaceID& faceID, RobotTimeStamp_t& timeStamp_ms);

  bool GetRecentBodySince(RobotTimeStamp_t sinceTime_ms,
                          Vision::SalientPoint& person) const;
