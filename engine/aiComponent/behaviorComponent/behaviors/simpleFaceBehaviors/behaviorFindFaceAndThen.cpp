 ...
 * Created: 2018-06-22
 *
 * Description: Finds a face either in the activation direction, or wherever one
 *was last seen, and if it finds one, either delegates to a followup behavior or
 *exits
 *
...
const char* const kTimeUntilCancelFaceLookingKey =
    "timeUntilCancelFaceLooking_s";
...
const char* const kAddtlSearchTimeIfSawBodyKey =
...
const TimeStamp_t kTimeToWaitForImage_ms =
    2 * STATE_MESSAGE_FREQUENCY *
    ROBOT_TIME_STEP_MS;  // = 60ms (*2 in case slowdown)

CONSOLE_VAR_RANGED(float, kMinTimeLookInMicDirection_s,
                   "Behaviors.FindFaceAndThen", 0.5f, 0.0f, 2.0f);
}  // namespace
...
BehaviorFindFaceAndThen::BehaviorFindFaceAndThen(const Json::Value& config)
  ...
  _iConfig.timeUntilCancelFaceLooking_s =
      JsonTools::ParseFloat(config, kTimeUntilCancelFaceLookingKey, kDebugName);
  _iConfig.timeUntilCancelFollowup_s =
      config.get(kTimeUntilCancelFollowupKey, -1.0f).asFloat();

  _iConfig.shouldLeaveChargerFirst =
      JsonTools::ParseBool(config, kShouldLeaveChargerFirstKey, kDebugName);
  if (_iConfig.shouldLeaveChargerFirst) {
    _iConfig.driveOffChargerBehaviorID =
        JsonTools::ParseString(config, kDriveOffChargerBehaviorKey, kDebugName);
  }

  _iConfig.startedWithMicDirection =
      config.get(kStartsWithMicDirectionKey, true).asBool();

  _iConfig.alwaysDetectFaces =
      config.get(kAlwaysDetectFacesKey, false).asBool();

  _iConfig.searchBehaviorID =
      config.get(kSearchForFaceBehaviorKey, "").asString();
  if (!_iConfig.searchBehaviorID.empty()) {
    _iConfig.timeUntilCancelSearching_s = JsonTools::ParseFloat(
        config, kTimeUntilCancelFaceSearchKey, kDebugName);
  }

  _iConfig.behaviorOnceFoundID =
      config.get(kBehaviorOnceFoundKey, "").asString();

  _iConfig.behaviorOnceFoundIsSimpleFace =
      config.get(kBehaviorIsSimpleFaceKey, true).asBool();
  ...
  if (_iConfig.useBodyDetector) {
    _iConfig.additionalLookTimeIfSawBody_s = JsonTools::ParseFloat(
        config, kAddtlLookTimeIfSawBodyKey, GetDebugLabel());
    _iConfig.additionalSearchTimeIfSawBody_s = JsonTools::ParseFloat(
        config, kAddtlSearchTimeIfSawBodyKey, GetDebugLabel());
    _iConfig.upperPortionLookUpPercent =
        config.get(kUpperPortionLookUpPercent, 0.3f).asFloat();
    _iConfig.upperPortionLookUpPercent =
        Util::Clamp(_iConfig.upperPortionLookUpPercent, 0.0f, 1.0f);
  } else {
  ...
  JsonTools::GetCladEnumFromJSON(config, kAnimWhenSeesFaceKey,
                                 _iConfig.animWhenSeesFace, GetDebugLabel(),
                                 false);

  _iConfig.animWhileSearching = AnimationTrigger::Count;
  JsonTools::GetCladEnumFromJSON(config, kAnimWhileSearching,
                                 _iConfig.animWhileSearching, GetDebugLabel(),
                                 false);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorFindFaceAndThen::GetBehaviorOperationModifiers(
  ...
  if (_iConfig.alwaysDetectFaces) {
    // always look for faces, so that when the robot turns in the direction of a
    // voice command, hopefully a face pose is already known
    modifiers.visionModesForActivatableScope->insert(
        {VisionMode::Faces, EVisionUpdateFrequency::Low});
  }
  if (_iConfig.useBodyDetector) {
    modifiers.visionModesForActiveScope->insert(
        {VisionMode::People, EVisionUpdateFrequency::Med});
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorFindFaceAndThen::GetBehaviorJsonKeys(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorFindFaceAndThen::GetAllDelegates(
...
void BehaviorFindFaceAndThen::InitBehavior() {
  ...
  if (!_iConfig.driveOffChargerBehaviorID.empty()) {
    _iConfig.driveOffChargerBehavior =
        FindBehavior(_iConfig.driveOffChargerBehaviorID);
  }
...
void BehaviorFindFaceAndThen::OnBehaviorActivated() {
  ...
  if (_iConfig.animWhileSearching != AnimationTrigger::Count) {
    ...
    auto* animAction =
        new ReselectingLoopAnimationAction{_iConfig.animWhileSearching};
    _dVars.animWhileSearchingTag = animAction->GetTag();
    GetBEI().GetRobotInfo().GetActionList().QueueAction(
        QueueActionPosition::IN_PARALLEL, animAction);
  }

  const bool hasRecentFace =
      GetRecentFace(_dVars.targetFace, _dVars.lastFaceTimeStamp_ms);
...
void BehaviorFindFaceAndThen::BehaviorUpdate() {
  ...
  if (_iConfig.useBodyDetector && !_dVars.sawBodyDuringState &&
    ...
    // add a bit more time to when we consider bodies in this direction to make
    // sure the body is actually is in this direction
    const RobotTimeStamp_t time_ms =
        _dVars.timeAtStateChange + kTimeToWaitForImage_ms;
    sawBodyWhenLookingForFace =
        GetRecentBodySince(time_ms, _dVars.lastPersonDetected);
    if (sawBodyWhenLookingForFace) {
      PRINT_CH_INFO("Behaviors",
                    "BehaviorFindFaceAndThen.BehaviorUpdate.PersonDetected",
                    "Detected a person");
      _dVars.sawBody = true;
    }
  }
  ...
  switch (_dVars.currentState) {
    case State::LookForFaceInMicDirection: {
      if (currentTime_s < _dVars.stateEndTime_s) {
        ...
        const bool hasFace = GetRecentFaceSince(_dVars.activationTimeStamp_ms,
                                                newFace, timeStamp_ms);
        const bool differentOrNew =
            (newFace != _dVars.targetFace) ||
            (timeStamp_ms > _dVars.lastFaceTimeStamp_ms);
        if (hasFace && differentOrNew &&
            (currentTime_s >= _dVars.stateMinTime_s)) {
          // a new face was spotted in the direction it's facing, or the same
          // face was spotted in that direction again
          ...
          // a body was seen. extend this state's duration and switch to a
          // behavior that looks for a face attached to a body
        ...
        // timeout in the mic direction, but the original face is still valid,
        // turn back towards it. we could potentially only do this if the
        // timestamp is recent, but it still might look cool if it looks in the
        // direction it last saw a face
        TransitionToTurningTowardsFace();
      } else if (_iConfig.searchFaceBehavior != nullptr) {
        // no face is known at this point, and we already spent time looking in
    ...
    case State::SearchForFace: {
      if (currentTime_s < _dVars.stateEndTime_s) {
        ...
        if (hasFace && differentOrNew) {
          ...
          // a body was seen. extend this state's duration and switch to a
          // behavior that looks for a face attached to the body
          _dVars.sawBodyDuringState = true;
          if (_dVars.currentState == State::SearchForFace) {
            _dVars.stateEndTime_s += _iConfig.additionalSearchTimeIfSawBody_s;
            _dVars.timeEndFaceFromBodyBehavior_s =
                currentTime_s + _iConfig.additionalSearchTimeIfSawBody_s;
          } else if (_dVars.currentState == State::FindFaceInCurrentDirection) {
            _dVars.stateEndTime_s += _iConfig.additionalLookTimeIfSawBody_s;
          }
          RunFindFaceFromBodyAction();
        } else if ((_dVars.timeEndFaceFromBodyBehavior_s > 0.0f) &&
                   (currentTime_s >= _dVars.timeEndFaceFromBodyBehavior_s)) {
          // the behavior to look for faces attached to bodies should end, so
          // switch back to the search action. in other states, dont do anything
          ...
          if (_dVars.currentState == State::SearchForFace) {
            RunSearchFaceBehavior();
                 ...
                 (_iConfig.searchFaceBehavior != nullptr)) {
        // no face is known at this point. look for one if config added a search
        // behavior
        TransitionToSearchingForFace();
      } else {
    ...
    case State::FollowupBehavior: {
      if ((_dVars.stateEndTime_s >= 0.0f) &&
          (_dVars.stateEndTime_s < currentTime_s)) {
        // timeout
        CancelSelf();
      }
    } break;
...
void BehaviorFindFaceAndThen::TransitionToTurningTowardsFace() {
  ...
  _dVars.timeAtStateChange = GetBEI().GetRobotInfo().GetLastImageTimeStamp();
  _dVars.timeEndFaceFromBodyBehavior_s = 0.0f;
  const Vision::TrackedFace* facePtr =
      GetBEI().GetFaceWorld().GetFace(_dVars.targetFace);
  if (facePtr != nullptr) {
    auto* action = new TurnTowardsFaceAction(_dVars.targetFace);
    action->SetLockOnClosestFaceAfterTurn(
        true);  // accept any face once turning in the direction of targetFace
    ...
    if (_iConfig.animWhenSeesFace != AnimationTrigger::Count) {
      ...
      const u8 tracksToLock = ((u8)AnimTrackFlag::HEAD_TRACK) |
    ...
    auto callback = [this](ActionResult result) {
      if (result == ActionResult::SUCCESS) {
        ...
        if (IActionRunner::GetActionResultCategory(result) ==
            ActionResultCategory::ABORT) {
          // mark this as a "bad" face (reset only when the behavior activates
          // again)
          if (_dVars.targetFace.IsValid()) {
            PRINT_CH_INFO(
                "Behaviors",
                "BehaviorFindFaceAndThen.TurnTowardsFace.Failure.AddFailedFace",
                "%s: failed to turn towards face '%s' with result %s, skipping "
                "this face",
                GetDebugLabel().c_str(),
...
void BehaviorFindFaceAndThen::TransitionToSearchingForFace() {
  ...
  if (_iConfig.searchFaceBehavior != nullptr) {
    _dVars.stateEndTime_s =
        BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() +
        _iConfig.timeUntilCancelSearching_s;
    _dVars.stateMinTime_s = 0.0f;
    _dVars.timeAtStateChange = GetBEI().GetRobotInfo().GetLastImageTimeStamp();
    _dVars.sawBodyDuringState = false;
...
void BehaviorFindFaceAndThen::TransitionToFollowupBehavior() {
  ...
  _dVars.sawFace = true;
  // send the event now instead of deactivation so it occurs before the followup
  // behavior, if there exists one
  SendCompletionDASEvent();
  ...
  if (_iConfig.behaviorOnceFoundIsSimpleFace) {
    std::shared_ptr<ISimpleFaceBehavior> simpleFaceBehavior =
        std::dynamic_pointer_cast<ISimpleFaceBehavior>(
            _iConfig.behaviorOnceFound);

    if (ANKI_VERIFY(
            simpleFaceBehavior != nullptr,
  ...
  if (_iConfig.behaviorOnceFound->WantsToBeActivated()) {
    DelegateIfInControl(_iConfig.behaviorOnceFound.get(),
                        [this]() { CancelSelf(); });
  } else {
    CancelSelf();
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorFindFaceAndThen::GetRecentFace(SmartFaceID& faceID,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorFindFaceAndThen::GetRecentFaceSince(
    RobotTimeStamp_t sinceTime_ms, SmartFaceID& faceID,
    RobotTimeStamp_t& timeStamp_ms) {
  SmartFaceID retFace;
  retFace.Reset();
  ...
  Pose3d facePose;
  RobotTimeStamp_t timeLastFaceObserved =
      GetBEI().GetFaceWorld().GetLastObservedFace(facePose, true);
  ...
  if (lastFaceInCurrentOrigin) {
    ...
    const auto facesObserved =
        GetBEI().GetFaceWorld().GetFaceIDs(timeLastFaceObserved);
...
bool BehaviorFindFaceAndThen::GetRecentBodySince(
    RobotTimeStamp_t sinceTime_ms, Vision::SalientPoint& person) const {
  ...
  salientPtsComponent.GetSalientPointSinceTime(
      bodiesFound, Vision::SalientPointType::Person, sinceTime_ms);
  ...
  auto maxElementIter = std::max_element(
      bodiesFound.begin(), bodiesFound.end(),
...
void BehaviorFindFaceAndThen::RunSearchFaceBehavior() {
  ...
  if (_iConfig.searchFaceBehavior->WantsToBeActivated()) {
    PRINT_CH_INFO("Behaviors",
                  "BehaviorFindFaceAndThen.RunSearchFaceBehavior.Run",
                  "Running search face behavior");
    DelegateIfInControl(_iConfig.searchFaceBehavior.get());
  } else {
    PRINT_NAMED_WARNING(
        "BehaviorFindFaceAndThen.RunSearchFaceBehavior."
        "SearchDoesntWantToActivate",
        "Search behavior %s doesn't want to activate",
        _iConfig.searchBehaviorID.c_str());
    CancelSelf();
  }
}
...
void BehaviorFindFaceAndThen::RunFindFaceFromBodyAction() {
  ...
  if (_iConfig.behaviorFindFaceInBB->WantsToBeActivated()) {
    ...
    CancelDelegates(
        false);  // it's possible the regular find faces behavior is running
    DelegateIfInControl(_iConfig.behaviorFindFaceInBB.get(), [this]() {
      // not expected to end, but just in case it does, resume with the other
      // search behavior
      PRINT_CH_INFO(
          "Behaviors",
          "BehaviorFindFaceAndThen.RunFindFaceFromBodyAction.UnexpectedEnd",
          "Behavior SearchWithinBoundingBox ended unexpectedly. Recovering");
      if (_dVars.currentState == State::SearchForFace) {
        RunSearchFaceBehavior();
      } else {
        // We're coming from the LookForFaceInMicDirection state, lets go on to
    ...
    PRINT_NAMED_WARNING(
        "BehaviorFindFaceAndThen.RunFindFaceFromBodyAction."
        "FindFromBodyDoesntWantToActivate",
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
//- - - - - - - - - - - - - - - - - - - -
void BehaviorFindFaceAndThen::SetState_internal(State state,
...
void BehaviorFindFaceAndThen::SendCompletionDASEvent() {
  ...
  DASMSG(behavior_findfaceduration, "behavior.findfaceduration",
         "Info about how long it took a voice command to find a face");
  DASMSG_SET(i1, (int)Util::SecToMilliSec(GetActivatedDuration()),
             "Time spent looking (ms)");
  DASMSG_SET(i2, (int)_dVars.sawFace,
             "Whether a face was seen / voice command successful [may not be "
             "accurate for takeaphoto]");
  DASMSG_SET(i3, (int)_dVars.sawBody,
             "Whether a body was detected when looking for a face");
  DASMSG_SEND();
...
void BehaviorFindFaceAndThen::StopSearchAnimation() {
  if (_dVars.animWhileSearchingTag != ActionConstants::INVALID_TAG) {
    GetBEI().GetRobotInfo().GetActionList().Cancel(
        _dVars.animWhileSearchingTag);
