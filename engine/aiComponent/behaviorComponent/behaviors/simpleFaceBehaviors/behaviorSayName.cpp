...
static const char* const kWaitForRecognitionMaxTime =
    "waitForRecognitionMaxTime_sec";
...
BehaviorSayName::InstanceConfig::InstanceConfig(const Json::Value& config) {
  ...
  JsonTools::GetCladEnumFromJSON(config, JsonKeys::kDontKnowNameAnimation,
                                 dontKnowNameAnimation,
                                 "BehaviorSayName.InstanceConfig");

  JsonTools::GetCladEnumFromJSON(config, JsonKeys::kKnowNameAnimation,
                                 knowNameAnimation,
                                 "BehaviorSayName.InstanceConfig");

  JsonTools::GetValueOptional(config, JsonKeys::kDontKnowTextKey,
                              dontKnowTextKey);

  JsonTools::GetValueOptional(config, JsonKeys::kWaitForRecognitionMaxTime,
                              waitForRecognitionMaxTime_sec);
}
...
BehaviorSayName::BehaviorSayName(const Json::Value& config)
    : ISimpleFaceBehavior(config), _iConfig(config) {}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - -
void BehaviorSayName::GetBehaviorJsonKeys(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorSayName::GetBehaviorOperationModifiers(
  ...
  // Assumption is that we're already looking at the face, so use cropping for
  // better efficiency
  modifiers.visionModesForActiveScope->insert(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorSayName::OnBehaviorActivated() { _dVars = DynamicVariables(); }
...
void BehaviorSayName::BehaviorUpdate() {
  ...
  const Vision::TrackedFace* facePtr =
      GetBEI().GetFaceWorld().GetFace(GetTargetFace());
  if (facePtr == nullptr) {
    LOG_ERROR("BehaviorSayName.OnBehaviorActivated.InvalidFace",
              "TargetFaceID:%s", GetTargetFace().GetDebugStr().c_str());
    Finish();
    return;
  }
  ...
  if (_dVars.waitingForRecognition) {
    ...
    if (!nameToSay.empty()) {
      ...
      LOG_INFO("BehaviorSayName.Update.NameKnown",
               "CurrentTick:%zu Name:%s IsGuess:%d",
               BaseStationTimer::getInstance()->GetTickCount(),
               Util::HidePersonallyIdentifiableInfo(nameToSay.c_str()),
               isGuess);
      ...
      if (_dVars.waitingForRecognition) {
        ...
        if ((currentTime - GetTimeActivated_s()) >
            _iConfig.waitForRecognitionMaxTime_sec) {
          DASMSG(behavior_sayname_recognition_timed_out,
                 "behavior.sayname.recognition_timeout",
                 "Behavior timed out waiting for tracked face to complete "
                 "recognition");
          DASMSG_SEND();
          SayDontKnow();
        } else {
          LOG_DEBUG("BehaviorSayName.Update.WaitingForRecognition",
                    "Start:%.1f Current:%.1f", GetTimeActivated_s(),
                    currentTime);
        }
      } else {
        // Recognition complete, but we apparently don't know the name
        // (otherwise nameKnown would be true above)
        SayDontKnow();
      }
    }
  }
...
void BehaviorSayName::SayName(const std::string& name, const bool isGuess) {
  DEV_ASSERT(!_dVars.waitingForRecognition,
             "BehaviorSayName.SayName.StillWaitingForRecognition");
  ...
  // TODO: Use animation + TTS behavior instead of SayTextAction with animation
  // containing special TTS keyframe?
...
void BehaviorSayName::SayDontKnow() {
  ...
  auto* animAction =
      new TriggerLiftSafeAnimationAction(_iConfig.dontKnowNameAnimation);
  ...
  if (haveDontKnowText) {
    ...
    const auto& dontKnowText =
        localeComponent.GetString(_iConfig.dontKnowTextKey);
  ...
  DelegateIfInControl(animAction, [this]() {
    const bool haveDontKnowText = !_iConfig.dontKnowTextKey.empty();
    if (_dVars.waitingForRecognition) {
      // Now that animation has played, eating a little more time, check one
      // last time to see if the face has a name yet. Say it if so.
      const Vision::TrackedFace* facePtr =
          GetBEI().GetFaceWorld().GetFace(GetTargetFace());
      if (facePtr != nullptr) {
        ...
        if (!nameToSay.empty()) {
          DASMSG(
              behavior_sayname_switch_to_do_know,
              "behavior.sayname.switch_to_do_know",
    ...
    // TODO: add ability to tell FaceWorld to log DAS for unnamed->named ID
    // update that occurs for this face later
