 ...
 * Created: 2018-07-06
 *
 * Description: Behavior to move the head/body whitin a confined space. The
 *limits of the space are defined by a bounding box in normalized image
 *coordinates.
 *
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorSearchWithinBoundingBox::BehaviorSearchWithinBoundingBox(
    const Json::Value& config)
    : ICozmoBehavior(config) {
  const std::string& debugName = "Behavior" + GetDebugLabel() + ".LoadConfig";

  _iConfig.howManySearches =
      JsonTools::ParseInt8(config, kHowManySearches, debugName);
  _iConfig.waitBeforeTurning =
      JsonTools::ParseFloat(config, kWaitBeforeTurning, debugName);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorSearchWithinBoundingBox::SetNewBoundingBox(float minX, float maxX,
                                                        float minY,
                                                        float maxY) {
  _iConfig.minX = minX;
  _iConfig.maxX = maxX;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorSearchWithinBoundingBox::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorSearchWithinBoundingBox::GetAllDelegates(
    std::set<IBehavior*>& delegates) const {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorSearchWithinBoundingBox::GetBehaviorJsonKeys(
...
void BehaviorSearchWithinBoundingBox::OnBehaviorActivated() {
  LOG_DEBUG("BehaviorSearchWithinBoundingBox.OnBehaviorActivated.Activated",
            "I am active");
...
void BehaviorSearchWithinBoundingBox::TransitionToGenerateNewPose() {
  ...
  if (_dVars.numberOfRemainingSearches <= 0) {
    LOG_DEBUG(
        "BehaviorSearchWithinBoundingBox::TransitionToGenerateNewPose.Finished",
        "Number of searches reached 0");
    // end of the behavior
    return;
  } else {
    LOG_DEBUG(
        "BehaviorSearchWithinBoundingBox::TransitionToGenerateNewPose."
        "RemainingSearches",
  ...
  _dVars.nextX =
      float(GetBEI().GetRNG().RandDblInRange(_iConfig.minX, _iConfig.maxX));
  _dVars.nextY =
      float(GetBEI().GetRNG().RandDblInRange(_iConfig.minY, _iConfig.maxY));
...
void BehaviorSearchWithinBoundingBox::TransitionToMoveToLookAt() {
  ...
  const TimeStamp_t timestamp =
      static_cast<TimeStamp_t>(GetBEI().GetRobotInfo().GetLastImageTimeStamp());
  // will use the SalientPoint to use the Action that takes image coordinates
  // between 0 and 1
  const Vision::SalientPoint point(timestamp, _dVars.nextX, _dVars.nextY, 0, 0,
                                   Vision::SalientPointType::Unknown, "", {},
                                   0);
  auto* turnAction = new TurnTowardsImagePointAction(point);
  CancelDelegates(false);
  // TODO After moving, should an animation be played? Look, blink, something
  // else?
  DelegateIfInControl(turnAction,
                      &BehaviorSearchWithinBoundingBox::TransitionToWait);
}
...
void BehaviorSearchWithinBoundingBox::TransitionToWait() {
  ...
  DelegateIfInControl(
      waitAction,
      &BehaviorSearchWithinBoundingBox::TransitionToMoveToOriginalPose);
}
...
void BehaviorSearchWithinBoundingBox::TransitionToMoveToOriginalPose() {
  ...
  auto* turnAction =
      new PanAndTiltAction(_dVars.initialTheta, _dVars.initialTilt, true, true);
