...
BehaviorLookAtMe::BehaviorLookAtMe(const Json::Value& config)
  ...
  JsonTools::GetCladEnumFromJSON(config, kAnimGetInKey, _iConfig.animGetIn,
                                 GetDebugLabel(), false);
  JsonTools::GetCladEnumFromJSON(config, kAnimLoopKey, _iConfig.animLoop,
                                 GetDebugLabel(), false);
  JsonTools::GetCladEnumFromJSON(config, kAnimGetOutKey, _iConfig.animGetOut,
                                 GetDebugLabel(), false);
  ...
  if (_iConfig.animGetOut != AnimationTrigger::Count) {
    _iConfig.playEmergencyGetOut =
        config.get(kPlayEmergencyGetOutKey, false).asBool();
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorLookAtMe::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorLookAtMe::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {
      kPanToleranceKey, kAnimGetInKey, kAnimLoopKey,
      kAnimGetOutKey,   kNumLoopsKey,  kPlayEmergencyGetOutKey,
  };
...
void BehaviorLookAtMe::OnBehaviorActivated() {
  ...
  if (facePtr != nullptr) {
    ...
    // If getin/loop/getout animation actions are provided, then they will be
    // cut short when the track action ends. For example, if the track action
    // ends in the middle of the looping animation, then at the end of the
    // current loop, it will play the getout. However, if the getin/loop/getout
    // end before the track action, the behavior continues until tracking is
    // complete
    ...
    if (_iConfig.animGetIn != AnimationTrigger::Count) {
      _dVars.getInAnimAction = animActions->AddAction(
          new TriggerLiftSafeAnimationAction{_iConfig.animGetIn});
    }
    if (_iConfig.animLoop != AnimationTrigger::Count) {
      _dVars.loopingAnimAction =
          animActions->AddAction(new ReselectingLoopAnimationAction{
              _iConfig.animLoop, _iConfig.numLoops});
    }
    if (_iConfig.animGetOut != AnimationTrigger::Count) {
      animActions->AddAction(
          new TriggerLiftSafeAnimationAction{_iConfig.animGetOut});
    }
...
void BehaviorLookAtMe::BehaviorUpdate() {
  ...
  // if the track face actions ends, tell the looping animation to end if it is
  // still running
  if (_dVars.trackFaceAction.expired()) {
    // track action ended
    if (IsControlDelegated() && _dVars.getInAnimAction.expired()) {
      // animation action is still running and the getin either didn't exist or
      // finished
      auto loopingAnimation = _dVars.loopingAnimAction.lock();
      if (loopingAnimation) {
        // get-in ended (or didnt exist) and looping animation hasn't started or
        // is currently looping. make it end after the current loop
        auto castAnim =
            std::dynamic_pointer_cast<ReselectingLoopAnimationAction>(
                loopingAnimation);
        if (castAnim != nullptr) {
        ...
        // looping animation ended or didn't exist, so the getout must have
        // started, if it exists. this will mean the emergency getout is
        // skipped.
...
void BehaviorLookAtMe::OnBehaviorDeactivated() {
  if ((_iConfig.animGetOut != AnimationTrigger::Count) &&
      !_dVars.startedGetOut && _iConfig.playEmergencyGetOut) {
