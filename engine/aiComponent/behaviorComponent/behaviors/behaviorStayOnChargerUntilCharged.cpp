...
CONSOLE_VAR(float, kSafeguardTimeout_s, CONSOLE_GROUP,
            30 * 60.0f);  // TODO: get better intuition for reasonable defaults
CONSOLE_VAR(float, kCooldown_s, CONSOLE_GROUP, 20 * 60.0f);
CONSOLE_VAR(
    float, kMinTimeAtNominal_s, CONSOLE_GROUP,
    4.0f);  // >= time for any drive-off-charger anim to clear charger platform
...
BehaviorStayOnChargerUntilCharged::BehaviorStayOnChargerUntilCharged(
  ...
  auto debugStr =
      "BehaviorStayOnChargerUntilCharged.Constructor.MissingDelegateID";
  _iConfig.delegateName =
      JsonTools::ParseString(config, kDelegateKey, debugStr);
}
...
bool BehaviorStayOnChargerUntilCharged::WantsToBeActivatedBehavior() const {
  ...
  const bool onCooldown =
      (currTime_s < (_dVars.lastTimeCancelled_s + kCooldown_s));
  ...
  // the battery level can drop from Full to Nominal the moment the robot leaves
  // the contacts, so enfore a minimum time
  const bool droppedToNominal = (batteryLevel == BatteryLevel::Nominal) &&
                                (prevBatteryLevel == BatteryLevel::Full);
  const float levelDuration_s =
      robotInfo.GetTimeAtBatteryLevelSec(batteryLevel);
  const bool briefDropToNominal =
      droppedToNominal && (levelDuration_s < kMinTimeAtNominal_s);

  // if a voice command is pending or active, it may be handled at a lower
  // priority level than this behavior, so don't activate this one (e.g.
  // exploring)
  const auto& uic = GetBehaviorComp<UserIntentComponent>();
  const bool hasIntent =
      uic.IsAnyUserIntentPending() || uic.IsAnyUserIntentActive();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorStayOnChargerUntilCharged::GetBehaviorOperationModifiers(
    BehaviorOperationModifiers& modifiers) const {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorStayOnChargerUntilCharged::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorStayOnChargerUntilCharged::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kDelegateKey};
...
void BehaviorStayOnChargerUntilCharged::OnBehaviorActivated() {
  ...
  LOG_INFO("StayOnChargerUntilCharged.OnBehaviorActivated.Activated",
           "Vector is charging, staying on charger.");

  // delegate
  ANKI_VERIFY(_iConfig.delegate->WantsToBeActivated(),
              "StayOnChargerUntilCharged.DelegateDoesNotWantToActivate", "");
...
void BehaviorStayOnChargerUntilCharged::BehaviorUpdate() {
  if (IsActivated()) {
    // if nay condition wants to cancel, set this bool instead of directly
    // cancelling so that all checks will be performed (and the cooldown dVar
    // can be set, if needed)
    bool cancel = false;

    // if we have a pending or active voice intent, voice commands should take
    // priority, so cancel this NOTE: as of this writing, the usage of this
    // behavior is such that most voice commands are higher priority, but
    // "exploring" is handled lower down in HLAI, hence the need for this
    const auto& uic = GetBehaviorComp<UserIntentComponent>();
    const bool hasIntent =
        uic.IsAnyUserIntentPending() || uic.IsAnyUserIntentActive();
    if (hasIntent) {
    ...
    if (isBatteryFull) {
      LOG_INFO("StayOnChargerUntilCharged.BehaviorUpdate.BatteryFull",
               "Battery is full, canceling self.");
      cancel = true;
    ...
    if (onChargerDuration_s > kSafeguardTimeout_s) {
      LOG_INFO("StayOnChargerUntilCharged.BehaviorUpdate.SafeguardTimeout",
               "Timeout of %f seconds reached, cancelling self",
               kSafeguardTimeout_s);
      // set the cooldown
      const float currTime_s =
          BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
