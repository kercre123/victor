...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorResetState::WantsToBeActivatedBehavior() const { return true; }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorResetState::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorResetState::GetBehaviorOperationModifiers(
...
void BehaviorResetState::OnBehaviorActivated() {
  ...
  // delocalize robot. this also resets blockworld, unexpected movement, and the
  // navmap
  ExternalInterface::ForceDelocalizeRobot delocMsg;
  GetBEI().GetRobotInfo().GetExternalInterface()->Broadcast(
      ExternalInterface::MessageGameToEngine{std::move(delocMsg)});

  // don't do anything for a short while. If the robot thinks it's off treads,
  // wait even longer, in case it thrashing was causing it to not realize it's
  // on treads
  const bool offTreads =
      (GetBEI().GetRobotInfo().GetOffTreadsState() != OffTreadsState::OnTreads);
  float timeToWait_s =
      offTreads ? kTimeWaitWhenOffTreads_s : kTimeWaitWhenOnTreads_s;
  ...
  if (carrying) {
    ...
    DelegateIfInControl(putDownCube, [this](const ActionResult& res) {
      if (res == ActionResult::SUCCESS) {
        ...
        auto* moveLiftDown = new MoveLiftToHeightAction{
            MoveLiftToHeightAction::Preset::LOW_DOCK};
        DelegateIfInControl(moveLiftDown,
                            &BehaviorResetState::ResetComponentsAndDelegate);
      }
    });
  } else {
    ResetComponentsAndDelegate();
...
void BehaviorResetState::ResetComponentsAndDelegate() {
  // ForceDelocalizeRobot already handled blockworld, unexpected movement, and
  // the navmap because of resulting calls to OnRobotDelocalized(). Now take
  // care of other components:

  // In case the last known face was causing the robot to turn towards
  // something...? Might not be necessary
  GetBEI().GetFaceWorldMutable().ClearAllFaces();

  if (_currBehavior != nullptr) {
    ANKI_VERIFY(
        _currBehavior->WantsToBeActivated(),
...
void BehaviorResetState::BehaviorUpdate() {
  ...
  if (IsActivated() && !IsControlDelegated() && (_currBehavior != nullptr)) {
    LOG_WARNING("BehaviorResetState.BehaviorUpdate.Ended",
                "Delegate should not end");
