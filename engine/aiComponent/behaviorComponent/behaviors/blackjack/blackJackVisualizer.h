 ...
 * Created: 2018-05-09
 *
 * Description: Class to encapsulate the View functionality of the BlackJack
 *behavior. Displays appropriate card art and animations to Victor's display
 *during BlackJack.
 *
...
class BlackJackVisualizer {
 public:
  ...
  // This should only be called by the BlackJack behavior from inside the
  // BehaviorUpdate funtion to allow actions in the visualizer that are synced
  // with the behavior system update tick
  void Update(BehaviorExternalInterface& bei);

  void DealToPlayer(BehaviorExternalInterface& bei,
                    std::function<void()> callback = nullptr);
  void DealToDealer(BehaviorExternalInterface& bei,
                    std::function<void()> callback = nullptr);

  // TODO:(str) implement this later depending on whether we get dynamic layouts
  // in time
  void SpreadPlayerCards(BehaviorExternalInterface& bei);

  void Flop(BehaviorExternalInterface& bei,
            std::function<void()> callback = nullptr);
  void DisplayCharlieFrame(BehaviorExternalInterface& bei,
                           std::function<void()> callback = nullptr);
  void SwipeToClearFace(BehaviorExternalInterface& bei,
                        std::function<void()> callback = nullptr);
  void ResetHands();

  // If the visualizer has control of parts of the robot (face track etc) this
  // function will release that control (probably becasue the behavior is
  // ending) and clear out any lasting state like callbacks and bools
