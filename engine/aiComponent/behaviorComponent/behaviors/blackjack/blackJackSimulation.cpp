...
static const char* ranks[] = {"Ace",  "Two",   "Three", "Four", "Five",
...
// For use in testing the BlackJack behavior. Due to the "5-card charlie rule",
// a game will consume a maximum of 10 cards from the deck, so specifying 10
// cards specifies the maximal length game. Calling this function will ensure
// that the deck is initialized with the specified card order until
// "StopStackingDeck" is called, or a different sequence is specified. NOTE:
// This function is for test purposes only and DOES NOT ENFORCE RATIONAL
// COLLECTIONS OF CARDS. Duplicates are perfectly permissible.
void StackDeck(ConsoleFunctionContextRef context) {
  std::vector<int> stackedDeckOrder;
  stackedDeckOrder.push_back(
      ConsoleArg_GetOptional_Int(context, "card1", kInvalidCardID));
  stackedDeckOrder.push_back(
      ConsoleArg_GetOptional_Int(context, "card2", kInvalidCardID));
  stackedDeckOrder.push_back(
      ConsoleArg_GetOptional_Int(context, "card3", kInvalidCardID));
  stackedDeckOrder.push_back(
      ConsoleArg_GetOptional_Int(context, "card4", kInvalidCardID));
  stackedDeckOrder.push_back(
      ConsoleArg_GetOptional_Int(context, "card5", kInvalidCardID));
  stackedDeckOrder.push_back(
      ConsoleArg_GetOptional_Int(context, "card6", kInvalidCardID));
  stackedDeckOrder.push_back(
      ConsoleArg_GetOptional_Int(context, "card7", kInvalidCardID));
  stackedDeckOrder.push_back(
      ConsoleArg_GetOptional_Int(context, "card8", kInvalidCardID));
  stackedDeckOrder.push_back(
      ConsoleArg_GetOptional_Int(context, "card9", kInvalidCardID));
  stackedDeckOrder.push_back(
      ConsoleArg_GetOptional_Int(context, "card10", kInvalidCardID));

  BlackJackGame::StackDeck(stackedDeckOrder);
}

CONSOLE_FUNC(StackDeck,
             "TestBlackJackBehavior.StackDeck(Up To Ten Comma Separated "
             "Integer Card ID's)",
             optional int card1, optional int card2, optional int card3,
             optional int card4, optional int card5, optional int card6,
             optional int card7, optional int card8, optional int card9,
             optional int card10);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Card::Card(int cardID, bool faceUp) : _cardID(cardID), _faceUp(faceUp) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
int Card::GetRank() const { return (_cardID % kNumCardsPerSuit); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::string Card::GetString() const {
  std::string nameString(ranks[GetRank()] + std::string(" of ") +
                         suits[_cardID / kNumCardsPerSuit]);
  return nameString;
}
...
void Deck::Shuffle(Anki::Util::RandomGenerator* rng) {
  _deck = Anki::Util::RandomIndexSampler::CreateFullShuffledVector(
      *rng, kNumCardsInDeck);
  _index = 0;
}
...
int Deck::PopTopCard() {
  if (_index >= _deck.size()) {
    PRINT_NAMED_WARNING("CardSimulation.EmptyDeck",
                        "Simulated deck of cards has been played out, reset "
                        "and shuffle required");
...
void BlackJackGame::Init(Anki::Util::RandomGenerator* rng) {
  if (nullptr != _stackedDeck) {
    PRINT_NAMED_WARNING("BlackJackSimulation.Init.StackingDeck",
                        "Deck order was set from a console func, stacking deck "
                        "per saved setting");
    ...
    if (nullptr != rng) {
      ...
      PRINT_NAMED_WARNING("BlackJackSimulation.Init.NullRNG",
                          "Game was initialized without an RNG reference. Deck "
...
bool BlackJackGame::DealToPlayer(const bool faceUp) {
  ...
  if (kInvalidCardID != topCardID) {
    ...
    PRINT_CH_INFO("Behaviors", "BlackJackGame.DealToPlayer",
                  ...
                  _playerHand.back().GetString().c_str(), topCardID);
...
bool BlackJackGame::DealToDealer(const bool faceUp) {
  ...
  if (kInvalidCardID != topCardID) {
    ...
    PRINT_CH_INFO("Behaviors", "BlackJackGame.DealToDealer",
                  ...
                  _dealerHand.back().GetString().c_str(), topCardID,
                  faceUpString.c_str());
...
const Card& BlackJackGame::LastCard() {
  if (nullptr == _lastCard) {
    PRINT_NAMED_ERROR(
        "BlackJackSimulation.NullLastCard",
...
void BlackJackGame::StackDeck(const std::vector<int>& stackedDeckOrder) {
  ...
  if (tempVec.size() == 0) {
    PRINT_NAMED_WARNING(
        "BlackJackSimulation.StackDeck.InvalidInput",
        "StackDeck input should be a series of one to ten positive, comma "
        "separated integers in the range [0,51]");
    return;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BlackJackGame::StopStackingDeck() { _stackedDeck = nullptr; }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BlackJackGame::ScoreHandAndReportAces(const std::vector<Card>& hand,
                                           int& outScore) {
  int score = 0;
  ...
  if (handHasAce && (score <= kAceHighValue)) {
    ...
    // Doesn't matter if there's more than one ace, only one can ever be
    // converted without busting.
...
void BlackJackGame::ScorePlayerHand() {
  ...
  PRINT_CH_INFO("Behaviors", "BlackJackSimulation.ScorePlayerHand",
                "Player has score of %d", _playerScore);
}
...
void BlackJackGame::ScoreDealerHand() {
  ...
  if (ScoreHandAndReportAces(_dealerHand, _dealerScore)) {
    // Check for "soft 17"
    if ((_dealerScore == 17) &&
        // Don't play the soft 17 if vector is at his card limit, he should just
        // max out his score
