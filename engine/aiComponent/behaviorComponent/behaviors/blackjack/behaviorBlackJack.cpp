...
const char* kPlayerWinsNaturalBlackJack =
    "BlackJack.PlayerWinsNaturalBlackJack";
...
static const UserIntentTag affirmativeIntent =
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorBlackJack::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorBlackJack::WantsToBeActivatedBehavior() const { return true; }
...
void BehaviorBlackJack::OnBehaviorActivated() {
  ...
  _dVars.gameStartTime_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();

  // --- Log DAS events ---
  // Session DAS
  if (_newSession) {
    DASMSG(behavior_blackjack_session_start, "behavior.blackjack_session_start",
  ...
  // Game DAS
  DASMSG(behavior_blackjack_game_start, "behavior.blackjack_game_start",
...
void BehaviorBlackJack::OnBehaviorDeactivated() {
  ...
  std::string sessionWinLoseString(std::to_string(_humanWinsInSession) + "," +
                                   std::to_string(_robotWinsInSession));
  float timeInSession_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() -
      _sessionStartTime_s;
  DASMSG(behavior_blackjack_session_end, "behavior.blackjack_session_end",
         "User has ended blackjack session");
  DASMSG_SET(s1, sessionWinLoseString,
             "win/lose status: 'numHumanWins,numRobotWins'");
  DASMSG_SET(s2, _gamesInSession, "Games in session (played back to back)");
  DASMSG_SET(i1, std::round(timeInSession_s),
...
void BehaviorBlackJack::TransitionToTurnToFace() {
  ...
  if (_iConfig.lookAtFaceInFrontBehavior->WantsToBeActivated()) {
    DelegateIfInControl(_iConfig.lookAtFaceInFrontBehavior.get(),
                        &BehaviorBlackJack::TransitionToGetIn);
  } else {
    TransitionToGetIn();
  }
}
...
void BehaviorBlackJack::TransitionToGetIn() {
  SET_STATE(GetIn);
  DelegateIfInControl(
      new TriggerAnimationAction(AnimationTrigger::BlackJack_GetIn),
  ...
  switch (_dVars.dealingState) {
      ...
      _visualizer.DealToPlayer(GetBEI(), [this]() {
        ...
        if (_game.LastCard().IsAnAce()) {
          ...
          if (!ANKI_VERIFY(_iConfig.goodLuckTTSBehavior->WantsToBeActivated(),
                           ...
                           "The Good Luck TTS behavior did not want to be "
                           "activated, this indicates a usage error")) {
            CancelSelf();
          } else {
    ...
    case EDealingState::Finished: {
      PRINT_NAMED_ERROR("BehaviorBlackJack.InvalidDealingState",
                        "Should never enter TransitionToDealing() when "
                        "DealingState is: Finished");
      break;
    }
  }
}
...
void BehaviorBlackJack::TransitionToReactToPlayerCard() {
  ...
  if (_game.PlayerBusted()) {
    ...
    const auto& tts =
        GetLocalizedString(kPlayerScoreBusted, _game.GetPlayerScore());
...
void BehaviorBlackJack::TransitionToHitOrStandPrompt() {
  ...
  if (_iConfig.hitOrStandPromptBehavior->WantsToBeActivated()) {
    DelegateIfInControl(_iConfig.hitOrStandPromptBehavior.get(),
                        &BehaviorBlackJack::TransitionToHitOrStand);
  }
}
...
void BehaviorBlackJack::TransitionToHitOrStand() {
  ...
  if (uic.IsUserIntentPending(playerHitIntent) ||
    ...
    _game.DealToPlayer();
    _visualizer.DealToPlayer(
        GetBEI(),
        std::bind(&BehaviorBlackJack::TransitionToReactToPlayerCard, this));
    ...
    DelegateIfInControl(
        new TriggerAnimationAction(AnimationTrigger::BlackJack_Response),
        [this]() { TransitionToVictorsTurn(); });
  }
}
...
void BehaviorBlackJack::TransitionToVictorsTurn() {
  ...
  // TODO:(str) work out whether a "spread" is happening or not, dependent on
  // dynamic layouts _visualizer.VisualizeSpread();
  const auto& tts = GetLocalizedString(kDealerTurn);
  DelegateIfInControl(SetUpSpeakingBehavior(tts), [this]() {
    _game.Flop();
    _visualizer.Flop(
        GetBEI(),
        std::bind(&BehaviorBlackJack::TransitionToReactToDealerCard, this));
  });
}
...
void BehaviorBlackJack::TransitionToReactToDealerCard() {
  ...
  if (_game.DealerBusted()) {
    ...
    const auto& tts =
        GetLocalizedString(kDealerScoreBusted, _game.GetDealerScore());
    DelegateIfInControl(SetUpSpeakingBehavior(tts),
                        &BehaviorBlackJack::TransitionToEndGame);
  } else if (_game.DealerTied()) {
    _dVars.outcome = EOutcome::Tie;
    const auto& tts =
        GetLocalizedString(kDealerScorePush, _game.GetDealerScore());
    DelegateIfInControl(SetUpSpeakingBehavior(tts),
                        &BehaviorBlackJack::TransitionToEndGame);
    ...
    _dVars.outcome = EOutcome::VictorWins;
    const auto& tts =
        GetLocalizedString(kDealerScoreDealerWins, _game.GetDealerScore());
    DelegateIfInControl(SetUpSpeakingBehavior(tts),
                        &BehaviorBlackJack::TransitionToEndGame);
  } else if (_game.DealerHasLost()) {
    _dVars.outcome = EOutcome::VictorLoses;
    const auto& tts =
        GetLocalizedString(kDealerScorePlayerWins, _game.GetDealerScore());
    DelegateIfInControl(SetUpSpeakingBehavior(tts),
                        &BehaviorBlackJack::TransitionToEndGame);
  } else {
    ...
    DelegateIfInControl(SetUpSpeakingBehavior(tts),
                        &BehaviorBlackJack::TransitionToDealToVictor);
  }
}
...
void BehaviorBlackJack::TransitionToEndGame() {
  ...
  GetBehaviorComp<RobotStatsTracker>().IncrementBehaviorStat(
      BehaviorStat::BlackjackGameComplete);
  ...
  switch (_dVars.outcome) {
    case EOutcome::Tie: {
      dasOutcome = "tie";
      endGameAction =
          new TriggerAnimationAction(AnimationTrigger::BlackJack_VictorPush);
      break;
    }
    case EOutcome::VictorWinsBlackJack: {
      GetBehaviorComp<RobotStatsTracker>().IncrementBehaviorStat(
          BehaviorStat::BlackjackDealerWon);
      dasOutcome = "loss";
      endGameAction = new TriggerAnimationAction(
          AnimationTrigger::BlackJack_VictorBlackJackWin);
      break;
    }
    case EOutcome::VictorWins: {
      ...
      dasOutcome = "loss";
      endGameAction =
          new TriggerAnimationAction(AnimationTrigger::BlackJack_VictorWin);
      break;
    }
    case EOutcome::VictorLosesBlackJack: {
      dasOutcome = "win";
      endGameAction = new TriggerAnimationAction(
          AnimationTrigger::BlackJack_VictorBlackJackLose);
      break;
    }
    case EOutcome::VictorBusts: {
      dasOutcome = "win";
      endGameAction =
          new TriggerAnimationAction(AnimationTrigger::BlackJack_VictorBust);
      break;
    }
    case EOutcome::VictorLoses: {
      dasOutcome = "win";
      endGameAction =
          new TriggerAnimationAction(AnimationTrigger::BlackJack_VictorLose);
      break;
    }
  }

  _visualizer.SwipeToClearFace(GetBEI(), [this, endGameAction]() {
    DelegateIfInControl(endGameAction,
                        &BehaviorBlackJack::TransitionToPlayAgainPrompt);
  });
  ...
  DASMSG(behavior_blackjack_game_end, "behavior.blackjack_game_end",
         "BlackJack game finished, reporting outcome");
  DASMSG_SET(s1, dasOutcome,
             "Outcome of the game for the user (win, loss, tie)");
  DASMSG_SET(
      s2, dasWinningScoreString,
      "Winning score i.e. user score if user won, robot score if robot won");
  DASMSG_SET(i1, std::round(timeInGame_s),
             "time spent in this round of the game (seconds)");
  DASMSG_SEND();
}
...
void BehaviorBlackJack::TransitionToPlayAgainPrompt() {
  ...
  if (_iConfig.playAgainPromptBehavior->WantsToBeActivated()) {
    DelegateIfInControl(_iConfig.playAgainPromptBehavior.get(),
                        &BehaviorBlackJack::TransitionToPlayAgain);
  }
}
...
void BehaviorBlackJack::TransitionToGetOut() {
  ...
  DelegateIfInControl(
      new TriggerAnimationAction(AnimationTrigger::BlackJack_Quit),
      [this]() { CancelSelf(); });
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
IBehavior* BehaviorBlackJack::SetUpSpeakingBehavior(
    const std::string& vocalizationString) {
  _iConfig.ttsBehavior->SetTextToSay(vocalizationString);
  if (!ANKI_VERIFY(_iConfig.ttsBehavior->WantsToBeActivated(),
                   "BehaviorBlackjack.TTSError",
                   "The TTSLoop behavior did not want to be activated, this "
