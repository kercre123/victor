...
// These vectors provide index-based conversion from CardID(0-51) to string
// AssetName
...
// Hand(engineer) built placeholder animations made by adding SpriteBoxKeyFrames
// to the original animations are providing BlackJack functionality for now.
// This way the animation team can convert the existing animations (if desired)
// and they can be substituted when ready without an interruption to BlackJack's
// functionality in Master, at which time the placeholders can be deleted.
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BlackJackVisualizer::Init(BehaviorExternalInterface& bei) { ResetHands(); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BlackJackVisualizer::Update(BehaviorExternalInterface& bei) {
  // Hack: if we've received an Update call from the parent behavior, it's
  // active. Don't clear locks on anim callbacks See VIC-5926
  _shouldClearLocksOnCallback = false;

  // Callbacks provided by the parent behavior should only be called during the
  // behavior update tick, in case they impact the state of the parent behavior.
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BlackJackVisualizer::DealToPlayer(BehaviorExternalInterface& bei,
  ...
  const Vision::SpritePathMap::AssetID cardSpriteID =
      kPlayerCardAssets[playerHand.back().GetID()];
  Vision::SpriteBoxName dealSeqSBName =
      kPlayerDealingSpriteBoxes[cardPositionIndex];
  Vision::SpriteBoxName revealSBName =
      kPlayerRevealSpriteBoxes[cardPositionIndex];
  Vision::SpriteBoxName finalSBName =
      kPlayerFinalSpriteBoxes[cardPositionIndex];

  _animCompletedCallback = callback;
  DealCard(bei, dealSeqSBName, kDealPlayerSpriteSeqID, revealSBName,
           finalSBName, cardSpriteID);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BlackJackVisualizer::DealToDealer(BehaviorExternalInterface& bei,
                                       std::function<void()> callback) {
  ...
  Vision::SpriteBoxName dealSeqSBName =
      kDealerDealingSpriteBoxes[cardPositionIndex];
  Vision::SpriteBoxName revealSBName =
      kDealerRevealSpriteBoxes[cardPositionIndex];
  Vision::SpriteBoxName finalSBName =
      kDealerFinalSpriteBoxes[cardPositionIndex];

  _animCompletedCallback = callback;
  DealCard(bei, dealSeqSBName, kDealDealerSpriteSeqID, revealSBName,
           finalSBName, cardSpriteID);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BlackJackVisualizer::Flop(BehaviorExternalInterface& bei,
                               std::function<void()> callback) {
  _animCompletedCallback = callback;
  ...
  // Clear off the card back image holding the dealer's first card position
  // before starting the animation,
  _cardAssetMap[finalSBName] = Vision::SpritePathMap::kEmptySpriteBoxID;
  DealCard(bei, dealSeqSBName, kDealerFlopSpriteSeqID, revealSBName,
           finalSBName, cardSpriteID);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BlackJackVisualizer::DealCard(
    const BehaviorExternalInterface& bei,
  ...
  bei.GetMovementComponent().UnlockTracks((u8)AnimTrackFlag::FACE_TRACK,
                                          kTrackLockingKey);
  ...
  auto animationCallback = [this, &bei](
  ...
  bei.GetAnimationComponent().PlayAnimWithSpriteBoxRemaps(
      kDealAnimationName, _cardAssetMap, interruptRunning, animationCallback,
      kTrackLockingKey);
  ...
  // ...and store the dealt card in the final spritebox so its there in future
  // animations
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BlackJackVisualizer::DisplayCharlieFrame(BehaviorExternalInterface& bei,
                                              std::function<void()> callback) {
  bei.GetMovementComponent().UnlockTracks((u8)AnimTrackFlag::FACE_TRACK,
                                          kTrackLockingKey);
  _animCompletedCallback = callback;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BlackJackVisualizer::SwipeToClearFace(BehaviorExternalInterface& bei,
                                           std::function<void()> callback) {
  bei.GetMovementComponent().UnlockTracks((u8)AnimTrackFlag::FACE_TRACK,
                                          kTrackLockingKey);
  _animCompletedCallback = callback;
  ...
  // Just grab the final card positions since the swipe anim doesn't have SB's
  // for deal/reveal
  ...
  const bool interruptRunning = true;
  bei.GetAnimationComponent().PlayAnimWithSpriteBoxRemaps(
      kSwipeAnimationName, swipeRemaps, interruptRunning, animationCallback);
...
void BlackJackVisualizer::ResetHands() {
  ...
  _cardAssetMap.insert(
      {kCharlieFrameSpriteBox, Vision::SpritePathMap::kEmptySpriteBoxID});
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BlackJackVisualizer::ReleaseControlAndClearState(
    BehaviorExternalInterface& bei) {
  ResetHands();
  bei.GetMovementComponent().UnlockTracks((u8)AnimTrackFlag::FACE_TRACK,
                                          kTrackLockingKey);

  // Hack: Assume the parent behavior is being interrupted. If that's the case
  // we will want to unlock the face from the animCompletedCallback for any
  // potentially orphaned CompositeAnimations that might currently be running.
  // This will have no effect if no anim is currently in progress. See VIC-5926
