...
class Card {
 ...
 private:
  Card();  // Don't allow default construction. Cards should always have an
           // identity
...
class BlackJackGame {
 public:
  ...
  bool PlayerHasCharlie() const {
    return (_playerHand.size() >= kHandSizeLimit) && (!PlayerBusted());
  }
  ...
  bool DealerHasTooManyCards() const {
    return _dealerHand.size() >= kHandSizeLimit;
  }
  bool DealerShouldStandPerVegasRules() const {
    return _dealerScore >= kVegasRulesLimit;
  }
  bool DealerTied() const {
    return (DealerHasTooManyCards() || DealerShouldStandPerVegasRules()) &&
