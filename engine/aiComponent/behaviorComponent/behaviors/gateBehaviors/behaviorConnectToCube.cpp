...
#define SET_STATE(s)                                                        \
  do {                                                                      \
    ...
    PRINT_CH_INFO("Behaviors", "BehaviorConnectToCube.State", "State = %s", \
                  #s);                                                      \
  } while (0);
...
// Note: this should only be used when delegating to Queue's or the like which
// have at least one delegation option which is does not have RequiredLazy or
// RequiredManaged cubeConnectionRequirements
...
BehaviorConnectToCube::InstanceConfig::InstanceConfig()
    : delegateBehavior(nullptr), delegateOnFailedConnection(false) {}
...
BehaviorConnectToCube::BehaviorConnectToCube(const Json::Value& config)
  ...
  JsonTools::GetValueOptional(config, kDelegateBehaviorIDKey,
                              _iConfig.delegateBehaviorIDString);
  JsonTools::GetValueOptional(config, kDelegateOnFailedConnectionKey,
                              _iConfig.delegateOnFailedConnection);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorConnectToCube::WantsToBeActivatedBehavior() const { return true; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorConnectToCube::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorConnectToCube::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kDelegateBehaviorIDKey, kDelegateOnFailedConnectionKey};
...
void BehaviorConnectToCube::OnBehaviorActivated() {
  ...
  if (GetBEI().GetCubeConnectionCoordinator().IsConnectedInteractable()) {
    // Go ahead and delegate to our child without making any noise about
    // connecting
    TransitionToConnected();
  } else {
    DelegateIfInControl(new TriggerLiftSafeAnimationAction(
                            AnimationTrigger::ConnectToCubeGetIn),
...
void BehaviorConnectToCube::BehaviorUpdate() {
  ...
  if (EState::Connecting == _dVars.state) {
    // Only once we've started the connection process, check if we've achieved a
    // background connection
    if (_dVars.connectedInBackground && !_dVars.subscribedInteractable) {
      // Once we've got a connection, switch the subscription to foreground to
      // enable lights
      GetBEI().GetCubeConnectionCoordinator().SubscribeToCubeConnection(this);
    }

    // Check in between each loop of the connecting animation so that we play at
    // least one connection loop
    if (!IsControlDelegated()) {
      if (_dVars.connectedOnLastUpdate) {
        ...
        DelegateIfInControl(new TriggerLiftSafeAnimationAction(
            AnimationTrigger::ConnectToCubeLoop));
      }
    }
  }
}
...
void BehaviorConnectToCube::TransitionToConnecting() {
  ...
  DelegateIfInControl(
      new TriggerLiftSafeAnimationAction(AnimationTrigger::ConnectToCubeLoop));
...
void BehaviorConnectToCube::TransitionToConnectionSucceeded() {
  ...
  DelegateIfInControl(new TriggerLiftSafeAnimationAction(
                          AnimationTrigger::ConnectToCubeSuccess),
...
void BehaviorConnectToCube::TransitionToConnectionFailed() {
  ...
  DelegateIfInControl(new TriggerLiftSafeAnimationAction(
                          AnimationTrigger::ConnectToCubeFailure),
                      [this]() {
...
void BehaviorConnectToCube::TransitionToConnected() {
  ...
  if (nullptr != _iConfig.delegateBehavior) {
    if (_iConfig.delegateBehavior->WantsToBeActivated()) {
      // When our delegate behavior finishes, don't delegate, just move on
      // silently
      DelegateIfInControl(_iConfig.delegateBehavior.get());
    } else {
      PRINT_NAMED_ERROR(
          "BehaviorConnectToCube.DelegateDidNotWantToBeActivated",
...
void BehaviorConnectToCube::TransitionToDelegatedWithoutConnection() {
  ...
  PRINT_NAMED_WARNING("BehaviorConnectToCube.DelegatedWithoutConnection",
                      "Connection attempt failed, but config indicated child "
                      "behaviors with optional connections");
...
void BehaviorConnectToCube::TransitionToConnectionLost() {
  ...
  DelegateIfInControl(new TriggerLiftSafeAnimationAction(
      AnimationTrigger::ConnectToCubeLostConnection));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorConnectToCube::ConnectedCallback(
...
void BehaviorConnectToCube::ConnectionLostCallback() {
  // In the event of a lost connection, always kill delegates and handle it
  // here. There may be use-cases for handling the connection loss lower in the
  // tree eventually, in which case this behavior could be linked to a json
  // config value.
