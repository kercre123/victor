...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToRobotOnFace::BehaviorReactToRobotOnFace(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToRobotOnFace::HandleWhileActivated(
  ...
  if (eventData.GetTag() == RobotInterface::RobotToEngineTag::animEvent) {
    if (eventData.Get_animEvent().event_id == AnimEvent::FLIP_DOWN_BEGIN) {
      // We are about to begin actually flipping down from OnFace. At this
      // point, we expect the OffTreadsState to transition away from OnFace, so
...
void BehaviorReactToRobotOnFace::FlipOverIfNeeded() {
  ...
  if (GetBEI().GetOffTreadsState() == OffTreadsState::OnFace) {
    ...
    if (robotInfo.GetCliffSensorComponent().IsCliffDetected()) {
      ...
      PRINT_CH_INFO(
          "Behaviors",
          "BehaviorReactToRobotOnFace.FlipOverIfNeeded.CalibratingHead",
          "%d %d %d %d", cliffs[0], cliffs[1], cliffs[2], cliffs[3]);
      DelegateIfInControl(
          new CalibrateMotorAction(
              true, false, MotorCalibrationReason::BehaviorReactToOnFace),
          &BehaviorReactToRobotOnFace::DelayThenCheckState);
    }
  }
}
...
void BehaviorReactToRobotOnFace::CheckFlipSuccess() {
  if (GetBEI().GetOffTreadsState() == OffTreadsState::OnFace) {
    DelegateIfInControl(
        new TriggerAnimationAction(AnimationTrigger::FailedToRightFromFace),
