...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorReactToPutDown::WantsToBeActivatedBehavior() const { return true; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToPutDown::InitBehavior() {
  // Waiting animation that cancels itself when the robot has been on its treads
  // for at least 3 seconds, or the animation has been played for at least 3
  // seconds.
  _iConfig.waitInternalBehavior =
      FindAnonymousBehaviorByName("PutDownWaitInternal");
  DEV_ASSERT(_iConfig.waitInternalBehavior != nullptr,
             "BehaviorReactToPutDown.InitBehavior.AnonymousBehaviorNotFound");
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToPutDown::GetAllDelegates(
...
void BehaviorReactToPutDown::TransitionToPlayingPutDownAnimation() {
  ...
  {
    DASMSG(behavior_putdownreaction, "Behavior.PutDownReaction",
           "The robot reacted to being put down.");
    DASMSG_SEND();
  }

  // Get the appropriate action/animation to play
  auto action =
      new TriggerLiftSafeAnimationAction(AnimationTrigger::ReactToPutDown);
  DelegateIfInControl(action,
                      &BehaviorReactToPutDown::TransitionToHeadCalibration);
}
...
void BehaviorReactToPutDown::TransitionToHeadCalibration() {
  ...
  DelegateIfInControl(
      new CalibrateMotorAction(true, true,
                               MotorCalibrationReason::BehaviorReactToPutDown),
...
void BehaviorReactToPutDown::TransitionToPlayingWaitAnimation() {
  ...
  DelegateIfInControl(
      new TriggerAnimationAction(AnimationTrigger::ObservingLookUp), [this]() {
        // Now the waiting animation behavior.
        if (ANKI_VERIFY(
                _iConfig.waitInternalBehavior->WantsToBeActivated(),
                "BehaviorReactToPutDown.TransitionToPlayingWaitAnimation."
                "WaitDoesNotWantToBeActivated",
                "")) {
