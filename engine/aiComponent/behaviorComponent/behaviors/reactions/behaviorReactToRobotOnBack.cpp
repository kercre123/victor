...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToRobotOnBack::BehaviorReactToRobotOnBack(
    const Json::Value& config)
    : ICozmoBehavior(config) {
  _iConfig.exitIfHeld = config.get(kExitIfHeldKey, true).asBool();

  SubscribeToTags({RobotInterface::RobotToEngineTag::animEvent});
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToRobotOnBack::GetBehaviorJsonKeys(
...
void BehaviorReactToRobotOnBack::BehaviorUpdate() {
  ...
  if (!onBack && _dVars.cancelIfNotOnBack) {
    LOG_WARNING("BehaviorReactToRobotOnBack.BehaviorUpdate.Canceling",
                "Canceling the reaction since the robot is unexpectedly no "
                "longer OnBack");
    CancelSelf();
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToRobotOnBack::HandleWhileActivated(
  ...
  if (eventData.GetTag() == RobotInterface::RobotToEngineTag::animEvent) {
    if (eventData.Get_animEvent().event_id == AnimEvent::FLIP_DOWN_BEGIN) {
      // We are about to begin actually flipping down from OnBack. At this
      // point, we expect the OffTreadsState to transition away from OnBack, so
      // we should not cancel the behavior if the OffTreadState changes.
...
void BehaviorReactToRobotOnBack::FlipDownIfNeeded() {
  ...
  if (GetBEI().GetOffTreadsState() == OffTreadsState::OnBack) {
    ...
    // If not, then calibrate head because we're not likely to be on back if no
    // cliff detected.
    if (robotInfo.GetCliffSensorComponent().IsCliffDetected()) {
      ...
      PRINT_CH_INFO(
          "Behaviors",
          "BehaviorReactToRobotOnBack.FlipDownIfNeeded.CalibratingHead",
          "%d %d %d %d", cliffs[0], cliffs[1], cliffs[2], cliffs[3]);
      DelegateIfInControl(
          new CalibrateMotorAction(
              true, false, MotorCalibrationReason::BehaviorReactToOnBack),
          &BehaviorReactToRobotOnBack::DelayThenFlipDown);
    }
  }
}
