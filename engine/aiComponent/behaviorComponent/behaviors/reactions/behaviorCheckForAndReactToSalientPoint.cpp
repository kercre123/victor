 ...
 * Created: 2018-11-14
 *
 * Description: Run a single frame of a VisionMode specified in Json and react
 *if one of the Json-configured SalientPointTypes is seen. Does nothing
 *otherwise. The reaction can either be an animation or another behavior.
 *
...
namespace {
// When there is a non-zero cooldown defined in iConfig, override with this if
// set to a value > 0 Note that this will affect *all* instantiations of of this
// behavior class with non-zero cooldown!
CONSOLE_VAR(float, kCFARTSP_CooldownOverride_sec,
            "Behaviors.CheckForAndReactToSalientPoint", 0.f);
}  // namespace
...
struct BehaviorCheckForAndReactToSalientPoint::InstanceConfig {
  ...
  struct ReactionEntry {
    ...
    ICozmoBehaviorPtr
        behavior;  // NOTE: Should have EITHER a behavior or an AnimTrigger
  ...
  inline float GetCooldown() const {
    ...
    if (Util::IsFltGTZero(cooldown_sec) &&
        Util::IsFltGTZero(kCFARTSP_CooldownOverride_sec)) {
      return kCFARTSP_CooldownOverride_sec;
    }
    return cooldown_sec;
  }

  inline bool HasCooldownPassed(const float currentTime_sec,
                                const ReactionEntry& reaction) const {
...
BehaviorCheckForAndReactToSalientPoint::BehaviorCheckForAndReactToSalientPoint(
  ...
  if (ANKI_VERIFY(config.isMember(JsonKeys::ReactionsKey),
                  "BehaviorCheckForAndReactToSalientPoint.Constructor."
                  "MissingAnimTriggers",
                  "")) {
    const Json::Value& animTriggers = config[JsonKeys::ReactionsKey];
    if (ANKI_VERIFY(animTriggers.isArray(),
                    "BehaviorCheckForAndReactToSalientPoint.Constructor."
                    "AnimTriggersNotArray",
                    "")) {
      for (const auto& triggerDefinition : animTriggers) {
        const bool hasAnimTrigger =
            triggerDefinition.isMember(JsonKeys::AnimTriggerKey);
        const bool hasBehaviorID =
            triggerDefinition.isMember(JsonKeys::BehaviorKey);
        if (ANKI_VERIFY(
                triggerDefinition.isMember(JsonKeys::SalientPointTypeKey) &&
                    (hasAnimTrigger ^ hasBehaviorID),
                "BehaviorCheckForAndReactToSalientPoint.Constructor."
                "BadTriggerDefinition",
                "")) {
          const std::string& salientPointTypeString =
              triggerDefinition[JsonKeys::SalientPointTypeKey].asString();
          Vision::SalientPointType salientPointType =
              Vision::SalientPointType::Unknown;
          if (ANKI_VERIFY(Vision::SalientPointTypeFromString(
                              salientPointTypeString, salientPointType),
                          "BehaviorCheckForAndReactToSalientPoint.Constructor."
                          "BadSalientPointType",
                          "%s", salientPointTypeString.c_str())) {
            InstanceConfig::ReactionEntry newEntry{
                "", nullptr, AnimationTrigger::Count, 0.f};

            if (hasAnimTrigger) {
              DEV_ASSERT(!hasBehaviorID,
                         "BehaviorCheckForAndReactToSalientPoint.Constructor."
                         "DefinesAnimTriggerAndBehavior");
              const std::string& animString =
                  triggerDefinition[JsonKeys::AnimTriggerKey].asString();
              ANKI_VERIFY(
                  AnimationTriggerFromString(animString, newEntry.animTrigger),
                  "BehaviorCheckForAndReactToSalientPoint.Constructor."
                  "BadAnimTrigger",
                  "%s", animString.c_str());
            } else if (hasBehaviorID) {
              // NOTE: we store the string here and defer finding the actual
              // behavior to InitBehavior
              //       because we cannot safely use the BEI or FindBehabvior
              //       from within the constructor since the factory is still
              //       constructing other behaviors.
              newEntry.behaviorString =
                  triggerDefinition[JsonKeys::BehaviorKey].asString();
            } else {
              // Should not get here by virtue of earlier checks
              DEV_ASSERT(false,
                         "BehaviorCheckForAndReactToSalientPoint.Constructor."
                         "HasNeitherBehaviorNorAnimDefined");
            }
  ...
  if (ANKI_VERIFY(config.isMember(JsonKeys::VisionModeKey),
                  ...
                  "MissingVisionMode",
                  "")) {
    const std::string& visionModeString =
        config[JsonKeys::VisionModeKey].asString();
    ANKI_VERIFY(
        VisionModeFromString(visionModeString, _iConfig->visionMode),
        "BehaviorCheckForAndReactToSalientPoint.Constructor.BadVisionMode",
        "%s", visionModeString.c_str());
  }

  JsonTools::GetValueOptional(config, JsonKeys::NumFramesKey,
                              _iConfig->numFrames);

  if (config.isMember(JsonKeys::WaitAnimTriggerKey)) {
    const std::string& animString =
        config[JsonKeys::WaitAnimTriggerKey].asString();
  ...
  JsonTools::GetValueOptional(config, JsonKeys::CooldownKey,
                              _iConfig->cooldown_sec);

  float headAngle_deg = 0.f;
  if (JsonTools::GetValueOptional(config, JsonKeys::HeadAngleKey,
                                  headAngle_deg)) {
...
void BehaviorCheckForAndReactToSalientPoint::InitBehavior() {
  // Set up the behaviors for any reactions that have behavior strings defined
  // from config Can't use BEI/FindBehavior in the constructor, so have to do it
  // here :-(
  for (auto& entryPair : _iConfig->reactions) {
    ...
    if (!entry.behaviorString.empty()) {
      DEV_ASSERT(entry.animTrigger == AnimationTrigger::Count,
                 "BehaviorCheckForAndReactToSalientPoint.InitBehavior."
                 "BehaviorStringAndAnimTriggerSet");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorCheckForAndReactToSalientPoint::GetAllDelegates(
  ...
  for (const auto& entryPair : _iConfig->reactions) {
    ...
    if (entry.behavior != nullptr) {
      DEV_ASSERT(entry.animTrigger == AnimationTrigger::Count,
                 "BehaviorCheckForAndReactToSalientPoint.GetAllDelegates."
                 "BehaviorAndAnimTriggerSet");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorCheckForAndReactToSalientPoint::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {
      JsonKeys::AnimTriggerKey,      JsonKeys::BehaviorKey,
      JsonKeys::CooldownKey,         JsonKeys::HeadAngleKey,
      JsonKeys::NumFramesKey,        JsonKeys::ReactionsKey,
      JsonKeys::SalientPointTypeKey, JsonKeys::VisionModeKey,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorCheckForAndReactToSalientPoint::WantsToBeActivatedBehavior()
    const {
  // Only want to be activated if we haven't reacted to at least one of the
  // salient point types too recently
...
void BehaviorCheckForAndReactToSalientPoint::OnBehaviorActivated() {
  _dVars->_lastImageTime_ms =
      GetBEI().GetVisionComponent().GetLastProcessedImageTimeStamp();

  WaitForImagesAction* waitAction =
      new WaitForImagesAction(_iConfig->numFrames, _iConfig->visionMode);
  ...
  DelegateIfInControl(
      action, &BehaviorCheckForAndReactToSalientPoint::CheckForStimulus);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorCheckForAndReactToSalientPoint::CheckForStimulus() {
  const SalientPointsComponent& salientPointsComp =
      GetAIComp<SalientPointsComponent>();

  std::vector<
      std::pair<Vision::SalientPointType, InstanceConfig::ReactionEntry*>>
      possibleReactions;
  const float currentTime_sec =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  for (auto& reaction : _iConfig->reactions) {
    if (_iConfig->HasCooldownPassed(currentTime_sec, reaction.second)) {
      ...
      salientPointsComp.GetSalientPointSinceTime(salientPoints, reaction.first,
                                                 _dVars->_lastImageTime_ms);
  ...
  LOG_DEBUG(
      "BehaviorCheckForAndReactToSalientPoint.CheckForStimulus."
      "NumPossibleReactions",
  ...
  if (!possibleReactions.empty()) {
    ...
    if (chosenReaction->animTrigger != AnimationTrigger::Count) {
      ...
      DelegateIfInControl(action, [chosenReaction](ActionResult result) {
        // Reset the cooldown timer for the thing we reacted to, iff the
        // animation succeeds
      ...
      DEV_ASSERT(false,
                 "BehaviorCheckForAndReactToSalientPoint.CheckForStimulus."
                 "HasNeitherAnimNorBehaviorDefined");
    }

    DASMSG(behavior_check_for_and_react_to_did_react,
           "behavior.check_for_and_react_to.did_react",
