 ...
 * Created: 10/16/15
 *
 * Description: Behavior for immediately responding to a detected cliff. This
 *behavior actually handles both the stop and cliff events
 *
...
// has passed with the robot on its treads in order to give
// `ReactToPlacedOnSlope` time to activate and run.
const f32 kMinPitchToCheckForIncline_rad = DEG_TO_RAD(10.f);

// When the robot is at a steep pitch, there is an additional requirement that
// the robot should consider itself "OnTreads" for at least this number of ms to
...
// while activated, then just give up and go to StuckOnEdge/AskForHelp.
CONSOLE_VAR(uint32_t, kMaxNumCliffReactAttemptsBeforeGivingUp, CONSOLE_GROUP,
            2);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToCliff::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kCliffBackupDistKey, kCliffBackupSpeedKey,
...
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToCliff::InstanceConfig::InstanceConfig(
    const Json::Value& config, const std::string& debugName) {
  cliffBackupDist_mm =
      JsonTools::ParseFloat(config, kCliffBackupDistKey, debugName);
  // Verify that backup distance is valid.
  ANKI_VERIFY(Util::IsFltGTZero(cliffBackupDist_mm),
              (debugName + ".InvalidCliffBackupDistance").c_str(),
              "Value should be greater than 0.0 (not %.2f).",
              cliffBackupDist_mm);

  cliffBackupSpeed_mmps =
      JsonTools::ParseFloat(config, kCliffBackupSpeedKey, debugName);
  // Verify that backup speed is valid.
  ANKI_VERIFY(Util::IsFltGTZero(cliffBackupSpeed_mmps),
              (debugName + ".InvalidCliffBackupSpeed").c_str(),
              "Value should be greater than 0.0 (not %.2f).",
              cliffBackupSpeed_mmps);

  eventFlagTimeout_ms =
      JsonTools::ParseUInt32(config, kEventFlagTimeoutKey, debugName);
}
...
BehaviorReactToCliff::DynamicVariables::DynamicVariables() {
  ...
  std::fill(persistent.cliffValsAtStart.begin(),
            persistent.cliffValsAtStart.end(), kInitVal);
...
void BehaviorReactToCliff::InitBehavior() {
  ...
  _iConfig.askForHelpBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(ForceStuckOnEdge));
}
  ...
  if (_dVars.persistent.gotStop || _dVars.persistent.putDownOnCliff) {
    if (GetBEI().GetOffTreadsState() == OffTreadsState::OnTreads) {
      ...
      if (pitch >= DEG_TO_RAD(kMinPitchToCheckForIncline_rad)) {
        ...
        const EngineTimeStamp_t lastChangedTime_ms =
            GetBEI().GetRobotInfo().GetOffTreadsStateLastChangedTime_ms();
...
void BehaviorReactToCliff::OnBehaviorActivated() {
  ...
  auto waitForStopLambda = [this, &robotInfo](Robot& robot) {
    ...
    LOG_INFO("BehaviorReactToCliff.CliffValsAtEnd", "%u %u %u %u (%x)",
             cliffData[0], cliffData[1], cliffData[2], cliffData[3],
             cliffComp.GetCliffDetectedFlags());

    for (int i = 0; i < CliffSensorComponent::kNumCliffSensors; ++i) {
      if (cliffComp.IsCliffDetected((CliffSensor)(i))) {
        if (_dVars.persistent.cliffValsAtStart[i] + kSuspiciousCliffValDiff <
            cliffData[i]) {
          // There was a sufficiently large increase in cliff value since cliff
          // was
          /// first detected so assuming it was a false cliff and aborting
          /// reaction
          LOG_INFO("BehaviorReactToCliff.QuittingDueToSuspiciousCliff",
                   "index: %d, startVal: %u, currVal: %u", i,
                   _dVars.persistent.cliffValsAtStart[i], cliffData[i]);
    ...
    if (cliffComp.GetCliffPoseRelativeToRobot(cliffComp.GetCliffDetectedFlags(),
                                              _dVars.cliffPose)) {
      _dVars.cliffPose.SetParent(GetBEI().GetRobotInfo().GetPose());
      if (_dVars.cliffPose.GetWithRespectTo(
              GetBEI().GetRobotInfo().GetWorldOrigin(), _dVars.cliffPose)) {
      ...
      LOG_WARNING(
          "BehaviorReactToCliff.OnBehaviorActivated.NoPoseForCliffFlags",
  ...
  WaitForLambdaAction* waitForStopAction =
      new WaitForLambdaAction(waitForStopLambda);
  DelegateIfInControl(waitForStopAction,
...
void BehaviorReactToCliff::TransitionToStuckOnEdge() {
  ...
  if (_iConfig.stuckOnEdgeBehavior->WantsToBeActivated()) {
    ...
    DASMSG(behavior_cliff_stuck_on_edge, "behavior.cliff_stuck_on_edge",
           "The robot is stuck on the edge of a surface");
    DASMSG_SET(i1, robotInfo.GetCliffSensorComponent().GetCliffDetectedFlags(),
               "Cliff detected flags");
    DASMSG_SEND();
    ...
    if (ANKI_VERIFY(_iConfig.askForHelpBehavior->WantsToBeActivated(),
                    "BehaviorReactToCliff.TransitionToStuckOnEdge."
                    "DoesNotWantToBeActivated",
                    "Behavior %s does not want to be activated!!",
...
void BehaviorReactToCliff::TransitionToPlayingCliffReaction() {
  ...
  // send DAS event for activation. don't send it if quitReaction is false,
  // because the user likely didn't notice
  {
    DASMSG(behavior_cliffreaction, "behavior.cliffreaction",
           "The robot reacted to a cliff");
    DASMSG_SET(i1, _dVars.persistent.cliffValsAtStart[0],
               "Sensor value 1 (front left)");
    DASMSG_SET(i2, _dVars.persistent.cliffValsAtStart[1],
               "Sensor value 2 (front right)");
    DASMSG_SET(i3, _dVars.persistent.cliffValsAtStart[2],
               "Sensor value 3 (back left)");
    DASMSG_SET(i4, _dVars.persistent.cliffValsAtStart[3],
               "Sensor value 4 (back right)");
    DASMSG_SEND();
  }

  GetBehaviorComp<RobotStatsTracker>().IncrementBehaviorStat(
      BehaviorStat::ReactedToCliff);

  // Have we tried executing too many cliff reaction animations/actions for this
  // activation of the behavior? We must either be in a "cliffy" area, or
  // somehow stuck in some orientation that StuckOnEdge is not detecting. Just
  // go to StuckOnEdge to be safe.
  if (_dVars.persistent.numCliffReactAttempts >
      kMaxNumCliffReactAttemptsBeforeGivingUp) {
    LOG_INFO(
        "BehaviorReactToCliff.TransitionToPlayingCliffReaction."
        "TooManyCliffReactionAttempts",
        "");
  ...
  if (ShouldStreamline()) {
    // If we skip over the animation, we still count the recovery backup as the
    // cliff reaction attempt.
    ++_dVars.persistent.numCliffReactAttempts;
    TransitionToRecoveryBackup();
  } else {
    Anki::Util::sInfo("robot.cliff_detected", {}, "");

    auto cliffDetectedFlags = GetBEI()
                                  .GetRobotInfo()
                                  .GetCliffSensorComponent()
                                  .GetCliffDetectedFlags();
    ...
    if (action == nullptr) {
      ...
      LOG_INFO(
          "BehaviorReactToCliff.TransitionToPlayingCliffReaction.StuckOnEdge",
          "%x", cliffDetectedFlags);
      TransitionToStuckOnEdge();
    } else {
      ++_dVars.persistent.numCliffReactAttempts;
      DelegateIfInControl(action, [this](const ActionResult& res) {
        if (!GetBEI()
                 ...
                 .GetCliffSensorComponent()
                 .IsCliffDetected()) {
          // The action reported success or partially succeeded moving the robot
          // enough to get it away from the cliff.
          TransitionToFaceAndBackAwayCliff();
        } else {
          // Cliff reaction failed for some reason, something might be
          // preventing the robot from moving away from the cliff (e.g. the
          // robot was teetering and unsure of whether it's been picked up, or
          // the treads slipped and got the robot stuck on the edge of a cliff).
...
void BehaviorReactToCliff::TransitionToWaitForNoMotion() {
  ...
  // Wait function to allow the robot to stop teetering on an edge if it is
  // stuck.
  auto waitForStationaryLambda = [this](Robot& robot) {
    ...
    const auto timeSinceLastTreadStateChange_ms =
        BaseStationTimer::getInstance()->GetCurrentTimeStamp() -
        robotInfo.GetOffTreadsStateLastChangedTime_ms();
    if (areWheelsMoving || robotInfo.IsPickedUp() ||
      ...
      LOG_PERIODIC_INFO(5, "BehaviorReactToCliff.WaitForNoMotion",
                        "Wheels moving? %d, Picked up? %d, Last time tread's "
                        "state changed? %u ms",
                        areWheelsMoving, robotInfo.IsPickedUp(),
                        static_cast<u32>(timeSinceLastTreadStateChange_ms));
      return false;
    }
    // Once we've stabilized, double check that we're still detecting cliffs
  ...
  WaitForLambdaAction* waitForStationaryAction =
      new WaitForLambdaAction(waitForStationaryLambda);
  // NOTE: If, after waiting for all motion to stop, the robot is actually stuck
  // on an edge, TransitionToPlayingCliffReaction will catch this and call
  // TransitionToStuckOnEdge (see above).
  DelegateIfInControl(waitForStationaryAction,
                      &BehaviorReactToCliff::TransitionToPlayingCliffReaction);
}
...
void BehaviorReactToCliff::TransitionToRecoveryBackup() {
  ...
  if (robotInfo.GetCliffSensorComponent().IsCliffDetected()) {
    ...
    f32 direction = 1.f;
    if (robotInfo.GetCliffSensorComponent().IsCliffDetected(
            CliffSensor::CLIFF_FL) ||
        robotInfo.GetCliffSensorComponent().IsCliffDetected(
            CliffSensor::CLIFF_FR)) {
      direction = -1.f;
    }

    LOG_INFO(
        "BehaviorReactToCliff.TransitionToRecoveryBackup."
        "DoingExtraRecoveryMotion",
        "");
    IActionRunner* backupAction =
        new DriveStraightAction(direction * _iConfig.cliffBackupDist_mm,
                                _iConfig.cliffBackupSpeed_mmps);
    BehaviorSimpleCallback callback = [this](void) -> void {
      LOG_INFO(
          "BehaviorReactToCliff.TransitionToRecoveryBackup."
          "ExtraRecoveryMotionComplete",
          "");
      auto& cliffComponent = GetBEI().GetRobotInfo().GetCliffSensorComponent();
      if (cliffComponent.IsCliffDetected()) {
        LOG_INFO(
            "BehaviorReactToCliff.TransitionToRecoveryBackup.StillStuckOnEdge",
            "%x", cliffComponent.GetCliffDetectedFlags());
        TransitionToStuckOnEdge();
...
void BehaviorReactToCliff::TransitionToHeadCalibration() {
  ...
  // To avoid causing a loop, reset it here, since we're about to calibrate the
  // head motor.
  _dVars.persistent.putDownOnCliff = false;
  // Force all motors to recalibrate since it's possible that Vector may have
  // been put down too aggressively, resulting in gear slippage for a motor, or
  // the user might have forced one of the motors into a different position
  // while in the air or while sensors were disabled.
  DelegateIfInControl(
      new CalibrateMotorAction(true, true,
                               MotorCalibrationReason::BehaviorReactToCliff),
...
Pose3d BehaviorReactToCliff::GetCliffPoseToLookAt() const {
  ...
  if (_dVars.hasTargetCliff) {
    bool result = _dVars.cliffPose.GetWithRespectTo(
        GetBEI().GetRobotInfo().GetWorldOrigin(), cliffLookAtPose);
    if (result) {
      LOG_INFO("BehaviorReactToCliff.GetCliffLookAtPose.CliffAt",
               "x=%4.2f y=%4.2f", _dVars.cliffPose.GetTranslation().x(),
               _dVars.cliffPose.GetTranslation().y());
    } else {
      LOG_WARNING(
          "BehaviorReactToCliff.GetCliffLookAtPose."
          "CliffPoseNotInSameTreeAsCurrentWorldOrigin",
          "");
    }
  } else {
    ...
    LOG_WARNING(
        "BehaviorReactToCliff.GetCliffLookAtPose.CliffDefaultPoseAssumed", "");
    cliffLookAtPose = Pose3d(0.f, Z_AXIS_3D(), {60.f, 0.f, 0.f});
    cliffLookAtPose.SetParent(GetBEI().GetRobotInfo().GetPose());
    cliffLookAtPose.GetWithRespectTo(GetBEI().GetRobotInfo().GetWorldOrigin(),
                                     cliffLookAtPose);
  }
...
void BehaviorReactToCliff::TransitionToVisualExtendCliffs() {
  ...
  compoundAction->AddAction(new MoveLiftToHeightAction(
      MoveLiftToHeightAction::Preset::LOW_DOCK));  // move lift to be out of the
                                                   // FOV

  // sometimes the animation will have us slightly off from the pose
  auto turnAction = new TurnTowardsPoseAction(cliffLookAtPose);
  turnAction->SetMaxPanSpeed(
      DEG_TO_RAD(kBodyTurnSpeedForCliffSearch_degPerSec));
  compoundAction->AddAction(turnAction);

  // if we're too close to the cliff, we need to backup to view it better
  float dist = ComputeDistanceBetween(
      cliffLookAtPose.GetTranslation(),
      GetBEI().GetRobotInfo().GetPose().GetTranslation());
  if (dist < kMinViewingDistanceToCliff_mm) {
    LOG_INFO(
        "BehaviorReactToCliff.TransitionToVisualCliffExtend."
        "FurtherBackupNeeded",
        "%6.6fmm", dist);
    // note: we will always be facing the cliff, so the backup direction is set
    // at this point
    compoundAction->AddAction(
        new DriveStraightAction(-(kMinViewingDistanceToCliff_mm - dist),
                                _iConfig.cliffBackupSpeed_mmps));
    ...
    turnAction2->SetMaxPanSpeed(
        DEG_TO_RAD(kBodyTurnSpeedForCliffSearch_degPerSec));
    compoundAction->AddAction(turnAction2);
  }
  compoundAction->AddAction(new WaitForImagesAction(kNumImagesToWaitForEdges,
...
void BehaviorReactToCliff::OnBehaviorDeactivated() {
  ...
  // reset dvars
  _dVars = DynamicVariables();
  _dVars.persistent.lastActiveTime_ms =
      BaseStationTimer::getInstance()->GetCurrentTimeStamp();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToCliff::GetAllDelegates(
...
void BehaviorReactToCliff::BehaviorUpdate() {
  ...
  if (!IsActivated()) {
    ...
    const auto& currentTime_ms =
        BaseStationTimer::getInstance()->GetCurrentTimeStamp();
    const auto& latestCliffStopTime_ms =
        cliffComp.GetLatestStopDueToCliffTime_ms();
    const auto& lastPutDownOnCliffTime_ms =
        cliffComp.GetLatestPutDownOnCliffTime_ms();

    const auto& timeSinceLastStop_ms = currentTime_ms - latestCliffStopTime_ms;
    const auto& timeSinceLastPutDownOnCliff_ms =
        currentTime_ms - lastPutDownOnCliffTime_ms;
    ...
    if (isPickedUp) {
      ...
      if (latestCliffStopTime_ms > _dVars.persistent.lastActiveTime_ms) {
        ...
        std::copy(cliffData.begin(), cliffData.end(),
                  _dVars.persistent.cliffValsAtStart.begin());
      }
  ...
  // const bool unexpectedMovement =
  // GetBEI().GetMovementComponent().IsUnexpectedMovementDetected(); const bool
  // cliffDetected =
  // GetBEI().GetRobotInfo().GetCliffSensorComponent().IsCliffDetected(); if
  // (unexpectedMovement && cliffDetected) {
  ...
  // Cancel if picked up
  // Often, when the robot gets too close to a curved edge, the robot can teeter
  // and trigger a false positive for pick-up detection. To counter this we wait
  // for more than half of the cliff sensors to also report that they are
  // detecting "cliffs", due to the robot getting picked up. Otherwise, we wait
  // until the next engine tick to check all conditions
  ...
  if (isPickedUp) {
    ...
    const bool tooLongInPickup = (now_ms - _dVars.lastPickupStartTime_ms) >
                                 kMaxPickedupTimeBeforeCancel_ms;
    ...
    if (cliffsDetected > 2 || tooLongInPickup) {
      LOG_INFO("BehaviorReactToCliff.Update.CancelDueToPickup",
               "numCliffs: %d, tooLongInPickup: %d", cliffsDetected,
               tooLongInPickup);
      CancelSelf();
    } else {
      LOG_PERIODIC_INFO(
          5, "BehaviorReactToCliff.Update.PossibleFalsePickUpDetected",
...
void BehaviorReactToCliff::AlwaysHandleInScope(const EngineToGameEvent& event) {
  ...
  switch (event.GetData().GetTag()) {
    case EngineToGameTag::RobotStopped: {
      ...
      // Record triggered cliff sensor value(s) and compare to what they are
      // when wheels stop moving. If the values are higher, the cliff is
      // suspicious and we should quit. NOTE(GB): Due to the delay between the
      // latest cliff sensor data and the RobotStopped event, the raw data
      // values at the time of retrieval may not register a cliff being
      // detected, since the latest cliff data can arrive up to ~30 ms after the
      // message arrives.
      const auto& cliffComp = robotInfo.GetCliffSensorComponent();
      const auto& cliffData = cliffComp.GetCliffDataRaw();
      std::copy(cliffData.begin(), cliffData.end(),
      ...
      LOG_INFO("BehaviorReactToCliff.CliffValsAtStart",
               ...
               _dVars.persistent.cliffValsAtStart[3], alreadyActivated);
    ...
    case EngineToGameTag::RobotOffTreadsStateChanged: {
      const auto treadsState =
          event.GetData().Get_RobotOffTreadsStateChanged().treadsState;
      const bool cliffsDetected =
          GetBEI().GetRobotInfo().GetCliffSensorComponent().IsCliffDetected();

      if (treadsState == OffTreadsState::OnTreads && cliffsDetected) {
        LOG_INFO("BehaviorReactToCliff.AlwaysHandleInScope",
                 "Possibly put down on cliff");
        _dVars.persistent.putDownOnCliff = true;
      } else {
        _dVars.persistent.putDownOnCliff = false;
      }
    ...
    default:
      LOG_ERROR("BehaviorReactToCliff.ShouldRunForEvent.BadEventType",
                "Calling ShouldRunForEvent with an event we don't care about, "
                "this is a bug");
      break;
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
IActionRunner* BehaviorReactToCliff::GetCliffReactAction(
  ...
  LOG_INFO("ReactToCliff.GetCliffReactAction.CliffsDetected", "0x%x",
           cliffDetectedFlags);
...
void BehaviorReactToCliff::TransitionToFaceAndBackAwayCliff() {
  ...
  cliffPose.GetWithRespectTo(GetBEI().GetRobotInfo().GetPose(),
                             cliffPoseWrtRobot);
  ...
  if (angularDistanceToCliff > M_PI_4_F) {
    ...
    cliffPose.GetWithRespectTo(GetBEI().GetRobotInfo().GetPose(),
                               cliffPoseWrtRobot);
    float bodyTurnAngle = std::atan2(cliffPoseWrtRobot.GetTranslation().y(),
                                     cliffPoseWrtRobot.GetTranslation().x());
    ...
    auto waitAction =
        new WaitAction(0.25f);  // constant time-delay prior to turn: reads
    ...
    compoundAction->AddAction(
        new TriggerLiftSafeAnimationAction(AnimationTrigger::ReactToCliff));
