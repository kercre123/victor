...
BehaviorReactToDarkness::BehaviorReactToDarkness(const Json::Value& config)
  ...
  _iConfig.lookHeadAngleMin_deg =
      JsonTools::ParseFloat(config, kLookHeadAngleMinKey, debugStr);
  _iConfig.lookHeadAngleMax_deg =
      JsonTools::ParseFloat(config, kLookHeadAngleMaxKey, debugStr);
  _iConfig.lookWaitTime_s =
      JsonTools::ParseFloat(config, kLookWaitTimeKey, debugStr);
  _iConfig.lookTurnAngleMax_deg =
      JsonTools::ParseFloat(config, kLookTurnAngleMaxKey, debugStr);
  _iConfig.lookTurnAngleMin_deg =
      JsonTools::ParseFloat(config, kLookTurnAngleMinKey, debugStr);
  _iConfig.numOnChargerLooks =
      JsonTools::ParseUInt32(config, kNumOnChargerLooksKey, debugStr);
  _iConfig.numOffChargerLooks =
      JsonTools::ParseUInt32(config, kNumOffChargerLooksKey, debugStr);
  _iConfig.sleepInPlace =
      JsonTools::ParseBool(config, kSleepInPlaceKey, debugStr);
  _iConfig.reactionEmotion =
      JsonTools::ParseString(config, kEmotionKey, debugStr);

  const u32 lookMinImages =
      JsonTools::ParseUInt32(config, kLookMinImages, debugStr);

  // NOTE These are hard-coded so they don't have to be in the config JSON
  // Any state is valid for positive pre-transition, so we don't specify any pre
  // parameters
  _iConfig.positiveConfig = IBEICondition::GenerateBaseConditionConfig(
      BEIConditionType::IlluminationDetected);
  _iConfig.positiveConfig["postTriggerStates"].append(
      EnumToString(IlluminationState::Darkened));
  ...
  // Any state is valid for negative pre-transition, so we don't specify any pre
  // parameters
  _iConfig.negativeConfig = IBEICondition::GenerateBaseConditionConfig(
      BEIConditionType::IlluminationDetected);
  _iConfig.negativeConfig["postTriggerStates"].append(
      EnumToString(IlluminationState::Illuminated));
...
void BehaviorReactToDarkness::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {
      kLookHeadAngleMinKey,   kLookHeadAngleMaxKey,  kLookTurnAngleMinKey,
      kLookTurnAngleMaxKey,   kLookWaitTimeKey,      kLookMinImages,
      kNumOffChargerLooksKey, kNumOnChargerLooksKey, kEmotionKey,
      kSleepInPlaceKey};
  expectedKeys.insert(std::begin(list), std::end(list));
}

void BehaviorReactToDarkness::GetAllDelegates(
...
void BehaviorReactToDarkness::InitBehavior() {
  _iConfig.positiveCondition.reset(
      new ConditionIlluminationDetected(_iConfig.positiveConfig));
  ...
  _iConfig.negativeCondition.reset(
      new ConditionIlluminationDetected(_iConfig.negativeConfig));
...
void BehaviorReactToDarkness::BehaviorUpdate() {
  ...
  switch (_dVars.state) {
    ...
    case State::TurnedAndWaiting: {
      ...
      if (Util::IsFltGT(currTime,
                        _dVars.waitStartTime + _iConfig.lookWaitTime_s)) {
        PRINT_CH_INFO("Behaviors",
                      "BehaviorReactToDarkness.BehaviorUpdate.Timeout", "");
        CancelSelf();
        break;
      }
      ...
      if (_iConfig.positiveCondition->AreConditionsMet(GetBEI())) {
        PRINT_CH_INFO("Behaviors",
                      "BehaviorReactToDarkness.BehaviorUpdate.PositiveMet", "");
      ...
      // Illumination check failed
        ...
        PRINT_CH_INFO("Behaviors",
                      "BehaviorReactToDarkness.BehaviorUpdate.NegativeMet", "");
        CancelSelf();
        break;
      }
      ...
      if (LookedEnoughTimes()) {
        PRINT_CH_INFO("Behaviors",
                      "BehaviorReactToDarkness.BehaviorUpdate.RunningTriggers",
                      "");
...
void BehaviorReactToDarkness::TransitionToTurning() {
  ...
  const double headAngle_deg = rng.RandDblInRange(
      _iConfig.lookHeadAngleMin_deg, _iConfig.lookHeadAngleMax_deg);
  ...
  // NOTE If we're on platform, don't turn in place, but still go home if
  // triggering
  if (!GetBEI().GetRobotInfo().IsOnChargerPlatform()) {
    ...
    const double turnMagnitude = rng.RandDblInRange(
        _iConfig.lookTurnAngleMin_deg, _iConfig.lookTurnAngleMax_deg);
    const double turnAngle_deg = turnSign * turnMagnitude;
    PRINT_CH_INFO("Behaviors",
                  "BehaviorReactToDarkness.TransitionToTurning.SetHeadAndTurn",
                  "Setting head to %.1fdeg and turning %f", headAngle_deg,
                  turnAngle_deg);

    action->AddAction(new TurnInPlaceAction(DEG_TO_RAD(turnAngle_deg), false));
  } else {
    PRINT_CH_INFO("Behaviors",
                  "BehaviorReactToDarkness.TransitionToTurning.SetHead",
                  "Setting head to %.1fdeg and not turning because on charger",
                  headAngle_deg);
  }

  DelegateIfInControl(action,
                      &BehaviorReactToDarkness::TransitionToTurnedAndWaiting);
}

void BehaviorReactToDarkness::TransitionToTurnedAndWaiting() {
  _dVars.state = State::TurnedAndWaiting;
  _dVars.waitStartTime =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
}

bool BehaviorReactToDarkness::LookedEnoughTimes() const {
  const u32 numLooks = GetBEI().GetRobotInfo().IsOnChargerContacts()
                           ? _iConfig.numOnChargerLooks
                           : _iConfig.numOffChargerLooks;
...
void BehaviorReactToDarkness::TransitionToResponding() {
  ...
  const f32 currTime =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  GetBEI().GetMoodManager().TriggerEmotionEvent(_iConfig.reactionEmotion,
                                                currTime);

  // Reset cooldown
  GetBEI()
      .GetBehaviorTimerManager()
      .GetTimer(BehaviorTimerTypes::ReactToIllumination)
      .Reset();

  // Trigger animation reaction
  auto animAction =
      new TriggerAnimationAction(AnimationTrigger::ReactToDarkness);
  DelegateIfInControl(animAction,
                      &BehaviorReactToDarkness::TransitionToGoingHome);
}
...
void BehaviorReactToDarkness::TransitionToGoingHome() {
  ...
  // If we're on the charger, don't know where home is, or are configured to
  // sleep in place Suggest napping to high-level AI through the whiteboard NOTE
  // "Suggesting" actually is more of "telling" the high-level AI to nap
  if (GetBEI().GetRobotInfo().IsOnChargerContacts()) {
    PRINT_CH_INFO(
        "Behaviors",
        "BehaviorReactToDarkness.TransitionToGoingHome.SuggestSleepOnCharger",
        "Already on charger, suggesting SleepOnCharger");
    GetAIComp<AIWhiteboard>().OfferPostBehaviorSuggestion(
        PostBehaviorSuggestions::SleepOnCharger);
    CancelSelf();
  } else if (!hasAHome || _iConfig.sleepInPlace) {
    PRINT_CH_INFO(
        "Behaviors",
        "BehaviorReactToDarkness.TransitionToGoingHome.SuggestSleep",
        "Sleeping in place (may not know where home is), suggesting Sleep");
    GetAIComp<AIWhiteboard>().OfferPostBehaviorSuggestion(
        PostBehaviorSuggestions::Sleep);
    CancelSelf();
  }
  // Else delegate to the FindAndGoHome behavior and wait for it to return
  else {
    PRINT_CH_INFO("Behaviors",
                  "BehaviorReactToDarkness.TransitionToGoingHome.FindAndGoHome",
