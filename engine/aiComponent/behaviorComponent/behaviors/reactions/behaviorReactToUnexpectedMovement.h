 ...
 * Created: 7/11/2016
 *
 * Description: Behavior for reacting to unexpected movement like being spun
 *while moving
 *
...
class BehaviorReactToUnexpectedMovement : public ICozmoBehavior {
 private:
  ...
  // Updates the internal tracking history of how frequently this behavior is
  // being activated while the robot is in a similar pose.
  void UpdateActivationHistory();

  // Keeps track of how frequently the behavior has detected being activated too
  // frequently
  ...
  struct DynamicVariables {
    DynamicVariables(){};
    struct Persistent {
      std::vector<float> activatedTimes_sec;  // set of basestation times at
                                              // which we've been activated
      std::vector<Radians> yawAnglesAtActivation_rad;  // set of yaw angles when
                                                       // we've been activated

      // Set of basestation times at which the behavior detected it had been
