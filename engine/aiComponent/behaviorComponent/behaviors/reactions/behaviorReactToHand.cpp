...
namespace {
CONSOLE_VAR_RANGED(float, kHandReaction_DriveForwardSpeed_mmps,
                   "Behavior.ReactToHand", 100.0f, 0.f,
                   MAX_SAFE_WHEEL_SPEED_MMPS);
CONSOLE_VAR_RANGED(float, kReactToHand_DriveDistanceFraction,
                   "Behavior.ReactToHand", 1.f, 0.f, 1.f);
CONSOLE_VAR_RANGED(float, kReactToHand_PitchAngleThresh_deg,
                   "Behavior.ReactToHand", 2.f, 0.f, 10.f);
}  // namespace
...
bool BehaviorReactToHand::IsRobotLifted() const {
  ...
  const float pitchDelta_rad =
      (pitchAngle_rad - _pitchAngleStart_rad).ToFloat();
  const bool isLifted = Util::IsFltGT(
      pitchDelta_rad, DEG_TO_RAD(kReactToHand_PitchAngleThresh_deg));
...
void BehaviorReactToHand::OnBehaviorActivated() {
  ...
  if (!proxData.foundObject) {
    PRINT_NAMED_WARNING(
        "BehaviorReactToHand.OnBehaviorActivated.InvalidProxReadingToHand", "");

    // TODO: should probably handle this failure differently (and have it impact
    // which hand reaction we choose
    //       for now (for review/test) just fake a distance
    driveDist_mm = 10.f;
  } else {
    driveDist_mm =
        kReactToHand_DriveDistanceFraction * (float)proxData.distance_mm;
  }

  CompoundActionParallel* driveAction = new CompoundActionParallel({
      new DriveStraightAction(driveDist_mm,
                              kHandReaction_DriveForwardSpeed_mmps, false),
      new ReselectingLoopAnimationAction{
          AnimationTrigger::ExploringReactToHandDrive},
      new WaitForLambdaAction([](Robot& robot) {
        const bool detectedUnexpectedMovement =
            (robot.GetMoveComponent().IsUnexpectedMovementDetected());
        if (detectedUnexpectedMovement) {
          LOG_DEBUG(
              "BehaviorReactToHand.OnBehaviorActivated."
              "UnexpectedMovementInterruptingDrive",
              "");
        }
  ...
  // The compound drive action will complete either when the DriveStraight
  // finishes or unexpected movement is detected (the looping anim will never
  // finish).
  ...
  action->AddAction(new TriggerLiftSafeAnimationAction{
      AnimationTrigger::ExploringReactToHandGetIn});
...
void BehaviorReactToHand::TransitionToLifting() {
  ...
  DelegateIfInControl(
      new TriggerLiftSafeAnimationAction{
          AnimationTrigger::ExploringReactToHandLift},
      [this]() {
        // If lifting didn't raise the robot, go straight to get out. If it did,
        // react first.
        if (IsRobotLifted()) {
          TransitionToReaction();
        } else {
...
void BehaviorReactToHand::TransitionToReaction() {
  ...
  DelegateIfInControl(
      new TriggerLiftSafeAnimationAction{
          AnimationTrigger::ExploringReactToHandReaction},
...
void BehaviorReactToHand::TransitionToGetOut() {
  ...
  DelegateIfInControl(new TriggerLiftSafeAnimationAction{
      AnimationTrigger::ExploringReactToHandGetOut});
}
