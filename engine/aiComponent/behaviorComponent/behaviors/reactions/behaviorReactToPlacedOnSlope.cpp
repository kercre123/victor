...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToPlacedOnSlope::BehaviorReactToPlacedOnSlope(
...
bool BehaviorReactToPlacedOnSlope::WantsToBeActivatedBehavior() const {
  ...
  if (cliffComp.IsCliffDetected()) {
    // When placed on a slope, only the front-facing cliff sensors should be
    // detecting cliffs, since the animation that plays from this behavior
  ...
  // When the robot is simply placed on a ramp, no cliffs are detected, or if
  // the robot is perched too close to the ground, the cliff sensors may not
  // detect any cliffs at all.
  return true;
}
...
void BehaviorReactToPlacedOnSlope::OnBehaviorActivated() {
  ...
  if (hasBehaviorRunRecently && _endedOnInclineLastTime) {
    ...
    // Don't run the animation. Instead, run a motor cal since his head may be
    // out of calibration.
    PRINT_CH_INFO("Behaviors", "BehaviorReactToPlacedOnSlope.CalibratingHead",
                  "%f", robotInfo.GetPitchAngle().getDegrees());
    DelegateIfInControl(new CalibrateMotorAction(
        true, false, MotorCalibrationReason::BehaviorReactToPlacedOnSlope));
    _endedOnInclineLastTime = false;
  } else {
    // Play the animation then check if we're still on a slope or if we were
...
void BehaviorReactToPlacedOnSlope::BehaviorUpdate() {
  ...
  const auto& cliffComp = GetBEI().GetRobotInfo().GetCliffSensorComponent();
  const auto cliffFlags = cliffComp.GetCliffDetectedFlags();
  // The animation to drive backwards to "un-perch" the robot might cause the
  // robot to drive onto a cliff behind it. Cancel the behavior if a cliff is
  // suddenly detected by either of the rear cliff sensors.
  if (cliffComp.IsCliffDetected() && !AreCliffDetectedFlagsValid(cliffFlags)) {
    PRINT_CH_INFO("Behaviors", "BehaviorReactToPlacedOnSlope.CancelSelf",
                  "Invalid cliffs detected: 0x%X", cliffFlags);
    CancelSelf();
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorReactToPlacedOnSlope::AreCliffDetectedFlagsValid(
    const u8 cliffDetectedFlags) const {
...
void BehaviorReactToPlacedOnSlope::CheckPitch() {
  const auto& robotInfo = GetBEI().GetRobotInfo();
  // Was the robot on an inclined surface or was the lift simply perched on
  // something?
