...
#define DEBUG_TRIGGER_WORD_VERBOSE \
  0  // add some verbose debugging if trying to track down issues

#define LOG_CHANNEL "Microphones"
#define CONSOLE_GROUP "TriggerWord"

#if DEBUG_TRIGGER_WORD_VERBOSE
// only print these debug if we're tracking down something so we don't spam the
// logs
...
const char* kExitAfterListeningIfNotStreamingKey =
    "exitAfterListeningIfNotStreaming";
...
// If supplied, any received intent not whitelisted will be consumed and the
// "unknown intent" outcome execized
const char* kIntentWhitelistKey = "whiteListedIntents";

CONSOLE_VAR(bool, kRespondsToTriggerWord, CONSOLE_GROUP, true);

// the behavior will always "listen" for at least this long once it hears the
// wakeword, even if we receive an error sooner than this. Note that the
// behavior will also consider the intent to be an error if the stream doesn't
// open within this amount of time, so don't lower this number too much
CONSOLE_VAR_RANGED(float, kMinListeningTimeout_s, CONSOLE_GROUP, 5.0f, 0.0f,
                   30.0f);
// this is the maximum duration we'll wait from streaming begin
CONSOLE_VAR_RANGED(float, kMaxStreamingDuration_s, CONSOLE_GROUP, 10.0f, 0.0f,
                   20.0f);

// when our streaming begins/ends there is a high chance that we will record
// some non-intent sound, these values allow us to chop off the front and back
// of the streaming window when determining the intent direction
CONSOLE_VAR_RANGED(double, kDirStreamingTimeToIgnoreBegin, CONSOLE_GROUP, 0.5,
                   0.0, 2.0);
CONSOLE_VAR_RANGED(double, kDirStreamingTimeToIgnoreEnd, CONSOLE_GROUP, 1.25,
                   0.0, 2.0);
// ignore mic direction with confidence below this when trying to determine
// streaming direction
CONSOLE_VAR_RANGED(MicDirectionConfidence, kDirStreamingConfToIgnore,
                   CONSOLE_GROUP, 500, 0, 10000);
// if we cannot determine the mic direction, we fall back to the most recent
// direction this allows you to specify how far back we sample for the most
// recent direction
CONSOLE_VAR_RANGED(double, kRecentDirFallbackTime, CONSOLE_GROUP, 1.0, 0.0,
                   10.0);

// pretend all responses are errors: NOTE intents may still get processed with
// this set true, recommendation is to use silence or a known mismatch intent
// (my favorite happens to be "potatoes").
...
BehaviorReactToVoiceCommand::BehaviorReactToVoiceCommand(
    ...
    : ICozmoBehavior(config), _triggerDirection(kMicDirectionUnknown) {
  ...
  // do we play the backpack lights from the behavior, else assume anims will
  // handle it
  JsonTools::GetValueOptional(config, kProceduralBackpackLights,
                              _iVars.backpackLights);

  // get the behavior to play after an intent comes in
  JsonTools::GetValueOptional(config, kIntentBehaviorKey,
                              _iVars.reactionBehaviorString);

  std::string animGetIn;
  if (JsonTools::GetValueOptional(config, kAnimListeningGetIn, animGetIn) &&
      !animGetIn.empty()) {
    ANKI_VERIFY(
        AnimationTriggerFromString(animGetIn, _iVars.animListeningGetIn),
        "BehaviorReactToVoiceCommand.Ctor.InvalidGetIn",
        "Get-in %s is not a valid animation trigger", animGetIn.c_str());
  }

  std::string animLoop;
  if (JsonTools::GetValueOptional(config, kAnimListeningLoop, animLoop) &&
      !animLoop.empty()) {
    ANKI_VERIFY(AnimationTriggerFromString(animLoop, _iVars.animListeningLoop),
                "BehaviorReactToVoiceCommand.Ctor.InvalidLoop",
                "Get-in %s is not a valid animation trigger", animLoop.c_str());
  }

  std::string animGetOut;
  if (JsonTools::GetValueOptional(config, kAnimListeningGetOut, animGetOut) &&
      !animGetOut.empty()) {
    ANKI_VERIFY(
        AnimationTriggerFromString(animGetOut, _iVars.animListeningGetOut),
        "BehaviorReactToVoiceCommand.Ctor.InvalidGetOut",
        "Get-in %s is not a valid animation trigger", animGetOut.c_str());
  }

  JsonTools::GetValueOptional(config, kExitAfterGetInKey,
                              _iVars.exitAfterGetIn);

  JsonTools::GetValueOptional(config, kExitAfterListeningIfNotStreamingKey,
                              _iVars.exitAfterListeningIfNotStreaming);
  ...
  if (!config[kNotifyOnWifiErrorsKey].isNull()) {
    ...
    ANKI_VERIFY(
        RecentOccurrenceTracker::ParseConfig(config[kNotifyOnWifiErrorsKey],
        ...
        GetDebugLabel().c_str());
    _iVars.wifiErrorHandle = _iVars.wifiErrorTracker.GetHandle(
        numErrorsToTriggerAnim, errorTrackingWindow_s);
  } else {
    PRINT_NAMED_WARNING(
        "BehaviorReactToVoiceCommand.NoErrorNotification.WifiErrors",
  ...
  if (!config[kNotifyOnCloudErrorsKey].isNull()) {
    ...
    ANKI_VERIFY(
        RecentOccurrenceTracker::ParseConfig(config[kNotifyOnCloudErrorsKey],
                                             numErrorsToTriggerAnim,
                                             errorTrackingWindow_s),
        "BehaviorReactToVoiceCommand.Constructor.InvalidConfig",
        "Behavior '%s' specified invalid recent occurrence config for cloud",
        GetDebugLabel().c_str());
    _iVars.cloudErrorHandle = _iVars.cloudErrorTracker.GetHandle(
        numErrorsToTriggerAnim, errorTrackingWindow_s);
  } else {
    PRINT_NAMED_WARNING(
        "BehaviorReactToVoiceCommand.NoErrorNotification.CloudErrors",
        "Behavior '%s' is not configured to show cloud errors at all",
        GetDebugLabel().c_str());
  }

  if (!config[kIntentWhitelistKey].isNull()) {
    const Json::Value& whiteListedIntents =
        ConditionUserIntentPending::GenerateConfig(config[kIntentWhitelistKey]);
    _iVars.intentWhitelistCondition =
        std::make_unique<ConditionUserIntentPending>(whiteListedIntents);
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Public destructor must be explicitly
// defined to allow std::unique_ptr<FwdDeclaredType>
// (ConditionUserIntentPending)
BehaviorReactToVoiceCommand::~BehaviorReactToVoiceCommand() {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToVoiceCommand::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kEarConBegin,
                        kEarConSuccess,
                        kEarConFail,
...
void BehaviorReactToVoiceCommand::InitBehavior() {
  ...
  if (!_iVars.reactionBehaviorString.empty()) {
    ICozmoBehaviorPtr reactionBehavior =
        FindBehavior(_iVars.reactionBehaviorString);
    // downcast to a BehaviorReactToMicDirection since we're forcing all
    // reactions to be of this behavior
    DEV_ASSERT_MSG(
        reactionBehavior->GetClass() == BehaviorClass::ReactToMicDirection,
        "BehaviorReactToVoiceCommand.Init.IncorrectMicDirectionBehavior",
        "Reaction behavior specified is not of valid class "
        "BehaviorClass::ReactToMicDirection");
    _iVars.reactionBehavior =
        std::static_pointer_cast<BehaviorReactToMicDirection>(reactionBehavior);
  }

  _iVars.unmatchedIntentBehavior =
      GetBEI().GetBehaviorContainer().FindBehaviorByID(
          BEHAVIOR_ID(IntentUnmatched));
  DEV_ASSERT(_iVars.unmatchedIntentBehavior != nullptr,
             "BehaviorReactToVoiceCommand.Init.UnmatchedIntentBehaviorMissing");

  _iVars.silenceIntentBehavior =
      GetBEI().GetBehaviorContainer().FindBehaviorByID(
          BEHAVIOR_ID(TriggerWordWithoutIntent));
  DEV_ASSERT(_iVars.silenceIntentBehavior != nullptr,
             "BehaviorReactToVoiceCommand.Init.silenceIntentBehavior");

  _iVars.noCloudBehavior =
      GetBEI().GetBehaviorContainer().FindBehaviorByID(BEHAVIOR_ID(NoCloud));
  DEV_ASSERT(_iVars.noCloudBehavior != nullptr,
             "BehaviorReactToVoiceCommand.Init.NoCloudBehaviorMissing");

  _iVars.noWifiBehavior =
      GetBEI().GetBehaviorContainer().FindBehaviorByID(BEHAVIOR_ID(NoWifi));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToVoiceCommand::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToVoiceCommand::GetBehaviorOperationModifiers(
  ...
  // Since so many voice commands need faces, this helps improve the changes
  // that a behavior following this one will know about faces when the behavior
  // starts
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToVoiceCommand::AlwaysHandleInScope(
  ...
  if (event.GetData().GetTag() ==
...
#if DEBUG_TRIGGER_WORD_VERBOSE
    {
      LOG_DEBUG("BehaviorReactToVoiceCommand.Debug",
                "Received TriggerWordDetected event with direction [%d]",
...
void BehaviorReactToVoiceCommand::OnBehaviorEnteredActivatableScope() {
  // don't push a custom response if disabled via config, in case, e.g., a
  // parent wants control of the response
  if (_iVars.pushResponse) {
    ...
    auto postAudioEvent = AECH::CreatePostAudioEvent(
        _iVars.earConBegin, AudioMetaData::GameObjectType::Behavior, 0);
    GetBehaviorComp<UserIntentComponent>().PushResponseToTriggerWord(
        GetDebugLabel(), _iVars.animListeningGetIn, postAudioEvent,
...
void BehaviorReactToVoiceCommand::OnBehaviorActivated() {
  ...
  if (GetBEI().HasMoodManager()) {
    ...
    moodManager.TriggerEmotionEvent("ReactToTriggerWord",
...
void BehaviorReactToVoiceCommand::OnBehaviorDeactivated() {
  // in case we were interrupted before we had a chance to turn off backpack
  // lights, do so now ...
  ...
  if (gi != nullptr) {
    ...
    if (intentHeard) {
      UserIntentComponent& uic = GetBehaviorComp<UserIntentComponent>();
      // we use this dirty dirty method here instead of sending this message
      // directly from the uic since we know whether the intent was heard here,
      // and it's nice that the same behavior on activation/deactivation sends
      // the two messages. if the uic sent the end message, it might be sent
      // without an initial message.
      const UserIntentData* intentData = uic.GetPendingUserIntent();
      if (intentData != nullptr) {
        // ideally we'd send a proto message structured the same as the intent,
        // but this would mean duplicating the entire userIntent.clad file for
        // proto, or converting the engine handling of intents to clad, neither
        // of which I've got time for.
        std::stringstream ss;
  ...
  // we've done all we can, now it's up to the next behavior to consume the user
  // intent
...
void BehaviorReactToVoiceCommand::BehaviorUpdate() {
  ...
  DEV_ASSERT(
      (GetStreamingDuration() >= (MicData::kStreamingTimeout_ms / 1000.0)),
      "BehaviorReactToVoiceCommand: Behavior streaming timeout is less than "
      "mic streaming timeout");

  const bool wasStreaming = (_dVars.streamingBeginTime > 0.0);
  const bool isStreaming =
      GetBehaviorComp<UserIntentComponent>().IsCloudStreamOpen();

  // track when our stream opens and closes; technically this is not synced with
  // our states which is why we track it independently.
  if (!wasStreaming) {
    if (isStreaming) {
      OnStreamingBegin();
    }
  ...
  if (_dVars.state == EState::ListeningGetIn) {
    // Once the animation process's GetIn animation has finished, queue the
    // listening loop animation
    if (!GetBehaviorComp<UserIntentComponent>()
             .WaitingForTriggerWordGetInToFinish()) {
      // we don't want to enter EState::Listening until we're in our loop or
      // else we could end up exiting too soon and looking like garbage
      if (_iVars.exitAfterGetIn) {
        OnVictorListeningEnd();
        CancelSelf();
      }

      // we now loop indefinitely and wait for the timeout in the update
      // function this is because we don't know when the streaming will begin
      // (if it hasn't already) so we can't time it accurately
      if (_dVars.persistent.forcedAnimListeningLoop !=
          AnimationTrigger::Count) {
        DelegateIfInControl(new ReselectingLoopAnimationAction(
            _dVars.persistent.forcedAnimListeningLoop));
      } else {
        DelegateIfInControl(
            new ReselectingLoopAnimationAction(_iVars.animListeningLoop));
      }
    ...
    if (isIntentPending) {
      ...
      // note: passing true to CancelDelegates doesn't call the callback if we
      // also delegate
      ...
      if (currTime_s >= listeningTimeout) {
        LOG_INFO("BehaviorReactToVoiceCommand.StopListening.Error",
                 ...
                 GetBehaviorComp<UserIntentComponent>().WasUserIntentError()
                     ? "error"
                     : "timeout");
        CancelDelegates(false);
        StopListening();
...
void BehaviorReactToVoiceCommand::ComputeReactionDirectionFromStream() {
  // what we are trying to do is figure out the direction that the user is
  // located with respect to the robot. we know the robot will not be moving
  // while streaming, so we can assume all directions recorded during streaming
  // are in the same "coordinate space".
  // -> We will take the most common direction recorded during streaming as the
  // most probable direction the user is located we can expect a few spikes in
  // noise causing the speaking direction to be false, but hopefully the user's
  // actual speaking direction will be the most consistent
  const MicDirectionHistory& micHistory =
      GetBEI().GetMicComponent().GetMicDirectionHistory();
  if (_dVars.streamingBeginTime > 0.0f) {
    // take into account the "uknown" direction
    // we are going to ignore the unknown direction in our calculations since it
    // is not helpful to us and very misleading as any time the robot is moving
    // or making noise will record the direction as unknown, skewing us to that
    // direction
    ...
    const TimeStamp_t streamBeginTime =
        (_dVars.streamingBeginTime + kDirStreamingTimeToIgnoreBegin) * 1000.0;
    const TimeStamp_t streamEndTime =
        (_dVars.streamingEndTime - kDirStreamingTimeToIgnoreEnd) * 1000.0;

    if (streamEndTime >= streamBeginTime) {
      const TimeStamp_t elapsed = (streamEndTime - streamBeginTime);
      MicDirectionNodeList list =
          micHistory.GetHistoryAtTime(streamEndTime, elapsed);

      // in the case where we have more than 1 sample, we want ignore partial
      // samples outside of our time range, so we'll just lop off the front and
      // back portions of the list nodes
      if (list.size() > 1) {
        ...
        if (front.timestampBegin < streamBeginTime) {
          DEV_ASSERT((front.timestampEnd >= streamBeginTime),
                     "Including node that is outside of streaming window");
          const double nodeDuration =
              TimeStamp_t(front.timestampEnd - front.timestampBegin);
          const double timeInNode =
              TimeStamp_t(front.timestampEnd - streamBeginTime);
        ...
        if (back.timestampEnd > streamEndTime) {
          DEV_ASSERT((back.timestampBegin <= streamEndTime),
                     "Including node that is outside of streaming window");
          const double nodeDuration =
              TimeStamp_t(back.timestampEnd - back.timestampBegin);
          const double timeInNode =
              TimeStamp_t(streamEndTime - back.timestampBegin);
          back.count *= (timeInNode / nodeDuration);
        }
      }

      // walk our list of heard directions and add up all of their counts
      // we're assuming a constant sample rate; we could always add up times as
      // well
      for (const auto& node : list) {
        // ignore directions that have too low of a confidence
        if (node.confidenceAvg > kDirStreamingConfToIgnore) {
          PRINT_TRIGGER_DEBUG(
              "Heard valid direction [%d], with confidence [%d]",
              node.directionIndex, node.confidenceAvg);
          micDirectionCounts[node.directionIndex] += node.count;
        }
      }

      // now go through all of our valid directions and find the one with the
      // highset count
      uint32_t highestCount = 0;
      for (MicDirectionIndex i = 0; i < kNumHistoryIndices; ++i) {
        PRINT_TRIGGER_DEBUG("Direction [%d], Count [%d]", i,
                            micDirectionCounts[i]);
        if ((i != kMicDirectionUnknown) &&
            (micDirectionCounts[i] > highestCount)) {
          _dVars.reactionDirection = i;
          highestCount = micDirectionCounts[i];
        }
      }

      PRINT_TRIGGER_INFO("Computed trigger reaction direction of %d",
                         (int)_dVars.reactionDirection);
    } else {
      PRINT_TRIGGER_INFO(
          "Streaming duration was too short, falling back to most recent "
          "direction");
...
MicDirectionIndex BehaviorReactToVoiceCommand::GetReactionDirection() const {
  ...
  if (kMicDirectionUnknown == direction) {
    PRINT_TRIGGER_INFO(
        "Didn't have a reaction or trigger direction, so falling back to "
        "trigger direction");

    // fallback to our trigger direction
    // accuracy is generally off by the amount the robot has turned
    // there's been some observed inaccuracy with this direction reported from
    // trigger word event
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
MicDirectionIndex BehaviorReactToVoiceCommand::GetDirectionFromMicHistory()
    const {
...
void BehaviorReactToVoiceCommand::OnStreamingEnd() {
  ...
  if (hasBegunStreaming && notAlreadyRecordedEnd) {
    ...
    _dVars.streamingEndTime =
        BaseStationTimer::getInstance()->GetCurrentTimeInSecondsDouble();

    // let's attempt to compute the reaction direction as soon as we know the
    // stream is closed note: this can be called outside of IsActivated(), but
...
void BehaviorReactToVoiceCommand::StartListening() {
  ...
  // to get into our listening state, we need to play our get-in anim followed
  // by our looping animation.
...
void BehaviorReactToVoiceCommand::StopListening() {
  ASSERT_NAMED_EVENT(
      ...
      "Transitioning to EState::IntentReceived from invalid state [%hhu]",
      _dVars.state);

  // force our model of the streaming to close, in the case that we timed out
  // (etc) before the actual stream closed
  OnStreamingEnd();
...
void BehaviorReactToVoiceCommand::OnVictorListeningEnd() {
  ...
  // play our "earcon end" audio, which depends on if our intent was
  // successfully heard or not
  ...
  if (EIntentStatus::IntentHeard == _dVars.intentStatus) {
    if (GenericEvent::Invalid != _iVars.earConSuccess) {
      ...
      bei.GetRobotAudioClient().PostEvent(
          _iVars.earConSuccess, AudioMetaData::GameObjectType::Behavior);
    }
  } else {
    if (GenericEvent::Invalid != _iVars.earConFail) {
      // Play earcon end audio
      bei.GetRobotAudioClient().PostEvent(
          _iVars.earConFail, AudioMetaData::GameObjectType::Behavior);
    }
  }
}
...
void BehaviorReactToVoiceCommand::UpdateUserIntentStatus() {
  UserIntentComponent& uic = GetBehaviorComp<UserIntentComponent>();
  if ((_dVars.intentStatus == EIntentStatus::NoIntentHeard) &&
      uic.IsAnyUserIntentPending()) {
    // next behavior is going to deal with the intent, but we still have more
    // shit to do
    ...
    if (nullptr != _iVars.intentWhitelistCondition &&
        !_iVars.intentWhitelistCondition->AreConditionsMet(GetBEI())) {
      LOG_DEBUG(
          "BehaviorReactToVoiceCommand.UpdateUserIntentStatus.NotOnWhitelist",
    ...
    // so we're assuming that a behavior is going to come along and consume this
    // pending intent, so we want to transition our streaming lights into the
    // intent feedback lights
...
void BehaviorReactToVoiceCommand::TransitionToThinking() {
  ...
  LOG_DEBUG("BehaviorReactToVoiceCommand.TransitionToThinking",
            "Thinking state starting");

  auto callback = [this]() {
    const bool streamingToCloud = _dVars.expectingStream;
    if (!streamingToCloud && _iVars.exitAfterListeningIfNotStreaming) {
      PRINT_CH_INFO("Behaviors",
                    "BehaviorReactToVoiceCommand.TransitionToThinkingCallback."
                    "NotStreaming",
                    "We are not streaming to the cloud currently, so no point "
                    "in continuing with the behavior (since "
                    "we do not want to increment the error count, etc.). "
                    "Playing the \"unheard\" anim then exiting");
      DelegateIfInControl(new TriggerLiftSafeAnimationAction(
          AnimationTrigger::VC_IntentNeutral));
      return;
    }

    // Play a reaction behavior if we were told to ...
    // ** only in the case that we've heard a valid intent **
    const bool heardValidIntent =
        (_dVars.intentStatus == EIntentStatus::IntentHeard);
    if (heardValidIntent && _iVars.reactionBehavior) {
      if (IsTurnEnabled()) {
        const MicDirectionIndex triggerDirection = GetReactionDirection();
        PRINT_CH_INFO("Behaviors",
                      "BehaviorReactToVoiceCommand.TransitionToThinking."
                      "ReactionDirection",
                      "%d", triggerDirection);
        ...
        if (_iVars.reactionBehavior->WantsToBeActivated()) {
          DelegateIfInControl(
              _iVars.reactionBehavior.get(),
              &BehaviorReactToVoiceCommand::TransitionToIntentReceived);
        } else {
          LOG_DEBUG(
              "BehaviorReactToVoiceCommand.Thinking."
              "ReactionDoesntWantToActivate",
        ...
        LOG_DEBUG("BehaviorReactToVoiceCommand.Thinking.TurnDisabled",
                  "Turn after intent is received has been disabled this tick, "
                  "skipping");
      }
    }
  ...
  if (_dVars.persistent.forcedAnimListeningGetOut != AnimationTrigger::Count) {
    DelegateIfInControl(new TriggerLiftSafeAnimationAction(
                            _dVars.persistent.forcedAnimListeningGetOut),
                        callback);
  } else {
    DelegateIfInControl(
        new TriggerLiftSafeAnimationAction(_iVars.animListeningGetOut),
        callback);
  }
}
...
void BehaviorReactToVoiceCommand::TransitionToIntentReceived() {
  ...
  const auto status =
      kTriggerWord_FakeError ? EIntentStatus::Error : _dVars.intentStatus;

  switch (status) {
    case EIntentStatus::IntentHeard: {
      // no animation for valid intent, go straight into the intent behavior
      LOG_DEBUG("BehaviorReactToVoiceCommand.Intent.Heard",
                "Heard valid user intent, woot!");

      // also reset the attention transfer counters since we got a valid intent
      GetBehaviorComp<AttentionTransferComponent>().ResetAttentionTransfer(
          AttentionTransferReason::UnmatchedIntent);
      GetBehaviorComp<AttentionTransferComponent>().ResetAttentionTransfer(
          AttentionTransferReason::NoWifi);
      GetBehaviorComp<AttentionTransferComponent>().ResetAttentionTransfer(
          AttentionTransferReason::NoCloudConnection);
    ...
    case EIntentStatus::IntentUnknown: {
      ...
      if (_iVars.unmatchedIntentBehavior->WantsToBeActivated()) {
        // this behavior will (should) interact directly with the attention
        // transfer component
        DelegateIfInControl(_iVars.unmatchedIntentBehavior.get());
      }

      // even an unknown intent means we got something back from the cloud, so
      // reset those transfer counters
      GetBehaviorComp<AttentionTransferComponent>().ResetAttentionTransfer(
          AttentionTransferReason::NoWifi);
      GetBehaviorComp<AttentionTransferComponent>().ResetAttentionTransfer(
          AttentionTransferReason::NoCloudConnection);
    ...
    case EIntentStatus::SilenceTimeout: {
      ...
      // even a silent intent means we got something back from the cloud, so
      // reset those transfer counters. This does not reset unmatched intent
      // because the user may have difficulty speaking
      GetBehaviorComp<AttentionTransferComponent>().ResetAttentionTransfer(
          AttentionTransferReason::NoWifi);
      GetBehaviorComp<AttentionTransferComponent>().ResetAttentionTransfer(
          AttentionTransferReason::NoCloudConnection);
...
void BehaviorReactToVoiceCommand::HandleStreamFailure() {
  ...
  // if console var faking is enabled, then check the console var to determine
  // wifi rather than the true os state
  const bool hasSSID =
      kTriggerWord_FakeError ? kTriggerWord_FakeError_HasWifi : osHasSSID;
  ...
  if (hasSSID) {
    ...
    if (_iVars.cloudErrorHandle &&
      ...
      if (ANKI_VERIFY(
              _iVars.noCloudBehavior != nullptr,
  ...
  if (errorBehavior) {
    ...
    auto errorBehaviorCallback = [this, errorBehavior]() {
      if (ANKI_VERIFY(
              errorBehavior->WantsToBeActivated(),
    ...
    DelegateIfInControl(
        new TriggerLiftSafeAnimationAction(AnimationTrigger::VC_IntentNeutral));
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToVoiceCommand::SetListeningAnims(
    const AnimationTrigger& listeningLoop,
    const AnimationTrigger& listeningGetOut) {
  if (IsActivated()) {
    LOG_ERROR(
        "BehaviorReactToVoiceCommand.CannotSetListeningAnimsWhileActivated",
        "Behavior %s is activated, cannot currently set new listening "
        "animations!",
        GetDebugLabel().c_str());
    return;
  }
  ...
  if (listeningLoop == AnimationTrigger::Count &&
      listeningGetOut == AnimationTrigger::Count) {
    ...
    if (listeningLoop != AnimationTrigger::Count) {
      LOG_DEBUG("BehaviorReactToVoiceCommand.SetListeningLoopAnim",
                ...
                GetDebugLabel().c_str(),
                AnimationTriggerToString(listeningLoop));
    }
    if (listeningGetOut != AnimationTrigger::Count) {
      LOG_DEBUG("BehaviorReactToVoiceCommand.SetListeningGetOutAnim",
                ...
                GetDebugLabel().c_str(),
                AnimationTriggerToString(listeningGetOut));
    }
  }
}
...
void BehaviorReactToVoiceCommand::ResetListeningAnimsToConfig() {
  if (IsActivated()) {
    ...
    LOG_WARNING(
        "BehaviorReactToVoiceCommand.CannotResetListeningAnimsWhileActivated",
        "Behavior %s is activated, cannot currently reset listening "
        "animations, but a"
...
bool BehaviorReactToVoiceCommand::IsTurnEnabled() const {
  ...
  if (uic.IsUserIntentPending(USER_INTENT(simple_voice_response),
                              pendingIntent)) {
    const MetaUserIntent_SimpleVoiceResponse& response =
        pendingIntent.Get_simple_voice_response();
...
double BehaviorReactToVoiceCommand::GetStreamingDuration() const {
  // our streaming duration is how long after streaming begins do we wait for an
  // intent
