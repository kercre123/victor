...
class BehaviorReactToRobotOnFace : public ICozmoBehavior {
 ...
 private:
  struct DynamicVariables {
    // If true, we cancel the behavior if we detect that the OffTreadsState is
    // no longer OnFace. Note that we set this to false when the animation is
    // about to flip the robot over, since at that point the OffTreadsState is
    // expected to change.
