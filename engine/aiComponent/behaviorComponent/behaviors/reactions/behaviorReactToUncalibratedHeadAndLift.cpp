...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorReactToUncalibratedHeadAndLift::WantsToBeActivatedBehavior()
    const {
  // This shouldn't interrupt pending intents. Its a rare coincidence, except
  // apparently when transitioning from onboarding to normal operation with a
  // pending intent, when it happens constantly... could be my robot is kinda
  ...
  const bool inPowerSaveMode =
      GetBEI().GetPowerStateManager().InPowerSaveMode();
  const bool isAnimating =
      GetBEI().GetAnimationComponent().IsPlayingAnimation();
  bool shouldActivate = !inPowerSaveMode && !isAnimating &&
                        !userIntentPending && !userIntentActive &&
                        !triggerPending && !isListening &&
  ...
  if (shouldActivate) {
    // If a calibration seems necessary, first verify that we are not in a
    // behavior which we know can cause an encoder to become invalid since
    // manipulation by user is expected.
    const auto checkInterruptCallback =
        [&shouldActivate](const ICozmoBehavior& behavior) -> bool {
      const bool classActive =
          (kDoNotInterruptBehaviorClasses.find(behavior.GetClass()) !=
           kDoNotInterruptBehaviorClasses.end());
      if (classActive) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToUncalibratedHeadAndLift::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToUncalibratedHeadAndLift::GetAllDelegates(
    std::set<IBehavior*>& delegates) const {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToUncalibratedHeadAndLift::GetBehaviorJsonKeys(
...
void BehaviorReactToUncalibratedHeadAndLift::OnBehaviorActivated() {
  DEV_ASSERT_MSG(!GetBEI().GetPowerStateManager().InPowerSaveMode(),
                 "BehaviorReactToUncalibratedHeadAndLift.OnBehaviorActivated."
                 "InPowerSaveModeDuringCalibration",
                 "");
  const auto& robot = GetBEI().GetRobotInfo();
  bool calibrateHead =
      robot.IsHeadMotorOutOfBounds() || robot.IsHeadEncoderInvalid();
  bool calibrateLift =
      robot.IsLiftMotorOutOfBounds() || robot.IsLiftEncoderInvalid();
  if (calibrateHead || calibrateLift) {
    LOG_INFO("BehaviorReactToUncalibratedHeadAndLift.Activated",
             "IsHeadMotorOutOfBounds %d, IsLiftMotorOutOfBounds %d, "
             "IsHeadEncoderInvalid %d, IsLiftEncoderInvalid %d",
             GetBEI().GetRobotInfo().IsHeadMotorOutOfBounds(),
             GetBEI().GetRobotInfo().IsLiftMotorOutOfBounds(),
             GetBEI().GetRobotInfo().IsHeadEncoderInvalid(),
             GetBEI().GetRobotInfo().IsLiftEncoderInvalid());

    DelegateNow(new CalibrateMotorAction(
        calibrateHead, calibrateLift,
