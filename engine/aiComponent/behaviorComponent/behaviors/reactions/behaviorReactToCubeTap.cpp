...
// #include
// "engine/aiComponent/behaviorComponent/behaviors/reactions/behaviorReactToCubeTap.h"
...
constexpr BehaviorReactToCubeTap::CubeIntensityAnimations
    BehaviorReactToCubeTap::kReactToCubeAnimations = {
        {BehaviorReactToCubeTap::EIntensityLevel::Level1,
         AnimationTrigger::ReactToCubeTapCubeTappedLvl1},
        {BehaviorReactToCubeTap::EIntensityLevel::Level2,
         AnimationTrigger::ReactToCubeTapCubeTappedLvl2},
        {BehaviorReactToCubeTap::EIntensityLevel::Level3,
         AnimationTrigger::ReactToCubeTapCubeTappedLvl3},
};

constexpr BehaviorReactToCubeTap::CubeIntensityAnimations
    BehaviorReactToCubeTap::kSearchForCubeAnimations = {
        {BehaviorReactToCubeTap::EIntensityLevel::Level1,
         AnimationTrigger::ReactToCubeSearchForCubeLvl1},
        {BehaviorReactToCubeTap::EIntensityLevel::Level2,
         AnimationTrigger::ReactToCubeSearchForCubeLvl2},
        {BehaviorReactToCubeTap::EIntensityLevel::Level3,
         AnimationTrigger::ReactToCubeSearchForCubeLvl3},
};

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToCubeTap::InstanceConfig::InstanceConfig()
    : cubeInteractionDuration(0.0f), targetCube({ObjectID(), 0.0f}) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToCubeTap::DynamicVariables::DynamicVariables()
    : state(EState::Invalid), intensity(EIntensityLevel::FirstLevel) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToCubeTap::BehaviorReactToCubeTap(const Json::Value& config)
    : ICozmoBehavior(config) {
  JsonTools::GetValueOptional(config, kCubeInteractionDurationKey,
                              _iVars.cubeInteractionDuration);
  JsonTools::GetValueOptional(config, kChargerBehaviorKey,
                              _iVars.chargerBehaviorString);
}
...
void BehaviorReactToCubeTap::InitBehavior() {
  ...
  SubscribeToTags({ExternalInterface::MessageEngineToGame::Tag::ObjectTapped});
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToCubeTap::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToCubeTap::GetBehaviorOperationModifiers(
    BehaviorOperationModifiers& modifiers) const {
  modifiers.wantsToBeActivatedWhenCarryingObject = false;
  modifiers.wantsToBeActivatedWhenOnCharger =
      (nullptr != _iVars.chargerBehavior);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToCubeTap::GetBehaviorJsonKeys(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToCubeTap::AlwaysHandleInScope(
  ...
  if (event.GetData().GetTag() ==
    ...
    const ExternalInterface::ObjectTapped& payload =
        event.GetData().Get_ObjectTapped();

    _iVars.targetCube.id = payload.objectID;
    _iVars.targetCube.lastTappedTime =
        BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();

    PRINT_CH_DEBUG("Behaviors", "BehaviorReactToCubeTap",
                   "Received Cube Tapped event [%u]", payload.objectID);

    if (IsActivated()) {
      // if we're currently active, we want to react to the tap depending on our
      // current state.
...
void BehaviorReactToCubeTap::BehaviorUpdate() {
  if (IsActivated()) {
    ...
    // this is so that if we cannot respond to the cube tap immediately (eg.
    // Victor is tied up doing something else) we don't want to react to said
    // cube down the line
    if (_iVars.targetCube.id.IsSet()) {
      const float currentTime =
          BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
      if (currentTime >=
          (_iVars.targetCube.lastTappedTime + kCubeTapMaxResponseTime)) {
        _iVars.targetCube.id.UnSet();
      }
    }
  }
...
void BehaviorReactToCubeTap::ReactToCubeTapped() {
  // begin playing our response to cube tap anim, then transition into figuring
  // out if we need to drive off the charger or not

  const AnimationTrigger animation = GetCubeReactionAnimation();
  ASSERT_NAMED_EVENT(AnimationTrigger::Count != animation,
                     "BehaviorReactToCubeTap.OnBehaviorActivated",
                     "No react to cube animation was found");

  DelegateIfInControl(new TriggerAnimationAction(animation),
...
void BehaviorReactToCubeTap::OnCubeTappedWhileActive() {
  DEV_ASSERT_MSG(IsActivated(), "BehaviorReactToCubeTap.State",
                 "Calling OnCubeTappedWhileActive() while not Activated");
  if (EState::FindCube == _dVars.state) {
    if (_dVars.intensity < EIntensityLevel::LastLevel) {
      // note: for now we don't care if it's the same cube or a different one;
      ...
      PRINT_CH_DEBUG("Behaviors", "BehaviorReactToCubeTap",
                     "Reacting to multiple cube taps ... spaz level [%d]!",
                     _dVars.intensity);
      ReactToCubeTapped();
    }
  }
...
void BehaviorReactToCubeTap::TransitionToGetOffCharger() {
  PRINT_CH_INFO("Behaviors", "BehaviorReactToCubeTap.State",
                "Transitioning to GetOffCharger state");
  _dVars.state = EState::GetOffCharger;

  // if we have a 'get off charger' behavior, attempt to delegate
  // if we're on the charger, BehaviorDriveOffCharger::WantsToBeActivated() will
  // return true
  if ((nullptr != _iVars.chargerBehavior) &&
      _iVars.chargerBehavior->WantsToBeActivated()) {
    DelegateIfInControl(_iVars.chargerBehavior.get(),
                        &BehaviorReactToCubeTap::TransitionToFindCube);
  } else {
...
void BehaviorReactToCubeTap::TransitionToFindCube() {
  PRINT_CH_INFO("Behaviors", "BehaviorReactToCubeTap.State",
                "Transitioning to FindCube state");
  _dVars.state = EState::FindCube;

  if (!IsCubeLocated()) {
    // if we find a cube before the animation is complete, then we will cancel
    // the delegate and proceed to going to said cube.  If we make it to the end
    // of the animation, we assume no cube was found and bail.
...
bool BehaviorReactToCubeTap::IsCubeLocated() const {
  const ObservableObject* locatedCube =
      GetBEI().GetBlockWorld().GetLocatedObjectByID(_iVars.targetCube.id);
...
void BehaviorReactToCubeTap::OnCubeNotFound() {
  ...
  DelegateIfInControl(
      new TriggerAnimationAction(AnimationTrigger::ReactToCubeTapCubeNotFound));
}
...
void BehaviorReactToCubeTap::TransitionToGoToCube(bool playFoundCubeAnimation) {
  PRINT_CH_INFO("Behaviors", "BehaviorReactToCubeTap.State",
                "Transitioning to GoToCube state");
  _dVars.state = EState::GoToCube;

  if (playFoundCubeAnimation) {
    // can't supply DriveToTargetCube() directly as the callback since it has
    // arguments
    auto driveToCube = [this]() { DriveToTargetCube(); };

    // play our "holy shit we found a cube" animation, then go to that cube
    DelegateIfInControl(
        new TriggerAnimationAction(AnimationTrigger::ReactToCubeTapCubeFound),
        driveToCube);
  } else {
    DriveToTargetCube();
  }
...
void BehaviorReactToCubeTap::DriveToTargetCube(unsigned int attempt) {
  ...
  auto onCompleteCallback = [this, attempt](ActionResult result) mutable {
    ...
    switch (result) {
      ...
      case ActionResult::RETRY:
        // attempt to drive to the cube at most kDriveToCubeAttempts times, then
        // give up
        if (attempt < kDriveToCubeAttempts) {
          PRINT_CH_INFO("Behaviors", "BehaviorReactToCubeTap.DriveToTargetCube",
                        "DriveToTargetCube failed, retrying attempt #%u",
                        attempt + 1);
          DriveToTargetCube(attempt + 1);
        } else {
          PRINT_CH_INFO(
              "Behaviors", "BehaviorReactToCubeTap.DriveToTargetCube",
              "DriveToTargetCube failed, too many attempts, exiting behavior");
          OnCubeNotFound();
        }
        break;
      ...
      default:
        ...
        PRINT_CH_INFO("Behaviors", "BehaviorReactToCubeTap.DriveToTargetCube",
                      "DriveToTargetCube failed, exiting behavior");
        OnCubeNotFound();
        break;
    }
  };
  ...
  DelegateIfInControl(
      new DriveToObjectAction(_iVars.targetCube.id,
                              PreActionPose::ActionType::DOCKING, 20.0f),
      onCompleteCallback);
}
...
void BehaviorReactToCubeTap::TransitionToInteractWithCube() {
  PRINT_CH_INFO("Behaviors", "BehaviorReactToCubeTap.State",
                "Transitioning to InteractWithCube state");
  ...
  if (_iVars.cubeInteractionDuration > 0.0f) {
    ...
    cubeInterAction->AddAction(
        new ReselectingLoopAnimationAction(
            AnimationTrigger::ReactToCubeTapInteractionLoop, 0, true,
            (u8)AnimTrackFlag::NO_TRACKS, _iVars.cubeInteractionDuration),
        true);

    cubeInterAction->AddAction(new TriggerAnimationAction(
        AnimationTrigger::ReactToCubeTapInteractionGetOut));
...
AnimationTrigger BehaviorReactToCubeTap::GetCubeReactionAnimation() const {
  static_assert(IsSequentialArray(kReactToCubeAnimations),
                "The array does not define each entry once and only once!");
  DEV_ASSERT_MSG(
      (_dVars.intensity >= 0) && (_dVars.intensity < EIntensityLevel::Count),
...
AnimationTrigger BehaviorReactToCubeTap::GetSearchForCubeAnimation() const {
  static_assert(IsSequentialArray(kSearchForCubeAnimations),
                "The array does not define each entry once and only once!");
  DEV_ASSERT_MSG(
      (_dVars.intensity >= 0) && (_dVars.intensity < EIntensityLevel::Count),
      "BehaviorReactToCubeTap", "Invalid Intensity level");
