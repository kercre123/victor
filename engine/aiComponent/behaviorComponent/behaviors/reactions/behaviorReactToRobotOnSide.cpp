...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToRobotOnSide::BehaviorReactToRobotOnSide(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToRobotOnSide::InstanceConfig::InstanceConfig(
    const Json::Value& config, const std::string& debugName) {
  const bool hasAskForHelpTime = JsonTools::GetValueOptional(
      config, kAskForHelpAfter_key, askForHelpAfter_sec);
  const bool hasAskForHelpBehavior = JsonTools::GetValueOptional(
      config, kAskForHelpBehavior_key, askForHelpBehaviorStr);

  DEV_ASSERT(hasAskForHelpTime == hasAskForHelpBehavior,
             "BehaviorReactToRobotOnSide.InstanceConfig.InvalidConfig");
}
  ...
  if (!_iConfig.askForHelpBehaviorStr.empty()) {
    _iConfig.askForHelpBehavior =
        BC.FindBehaviorByID(BehaviorTypesWrapper::BehaviorIDFromString(
            _iConfig.askForHelpBehaviorStr));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToRobotOnSide::GetBehaviorJsonKeys(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToRobotOnSide::GetAllDelegates(
...
void BehaviorReactToRobotOnSide::BehaviorUpdate() {
  ...
  // If we are no longer on our side (but have not been cancelled externally
  // yet), then play the get-out and exit
  const bool onSide =
  ...
  if (!onSide && !_dVars.getOutPlayed) {
    DelegateNow(
        new TriggerAnimationAction(AnimationTrigger::ReactToOnSideGetOut));
...
void BehaviorReactToRobotOnSide::ReactToBeingOnSide() {
  if (GetBEI().GetOffTreadsState() == OffTreadsState::OnLeftSide) {
    DelegateIfInControl(
        new TriggerAnimationAction(AnimationTrigger::ReactToOnLeftSideGetIn),
        &BehaviorReactToRobotOnSide::HoldingLoop);
  } else if (GetBEI().GetOffTreadsState() == OffTreadsState::OnRightSide) {
    DelegateIfInControl(
        new TriggerAnimationAction(AnimationTrigger::ReactToOnRightSideGetIn),
        &BehaviorReactToRobotOnSide::HoldingLoop);
  }
}
...
void BehaviorReactToRobotOnSide::HoldingLoop() {
  // Check if we have been activated for longer than the timeout. If so, play
  // the getout then transition to the "ask for help" behavior
  ...
  if (loopAnim != AnimationTrigger::Count) {
    ...
    DelegateIfInControl(action, &BehaviorReactToRobotOnSide::HoldingLoop);
  }
}
