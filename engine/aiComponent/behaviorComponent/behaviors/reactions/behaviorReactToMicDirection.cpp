...
// #include
// "engine/aiComponent/behaviorComponent/behaviors/reactions/behaviorReactToMicDirection.h"
...
#define DEBUG_MIC_REACTION_VERBOSE \
...
const BehaviorReactToMicDirection::DynamicStateReaction
    BehaviorReactToMicDirection::kFallbackReaction{{
        {kInvalidAnimationTrigger,
         -((float)EClockDirection::TwelveOClock* kDegreesPerDirection)},
        {kInvalidAnimationTrigger,
         -((float)EClockDirection::OneOClock* kDegreesPerDirection)},
        {kInvalidAnimationTrigger,
         -((float)EClockDirection::TwoOClock* kDegreesPerDirection)},
        {kInvalidAnimationTrigger,
         -((float)EClockDirection::ThreeOClock* kDegreesPerDirection)},
        {kInvalidAnimationTrigger,
         -((float)EClockDirection::FourOClock* kDegreesPerDirection)},
        {kInvalidAnimationTrigger,
         -((float)EClockDirection::FiveOClock* kDegreesPerDirection)},
        {kInvalidAnimationTrigger,
         -((float)EClockDirection::SixOClock* kDegreesPerDirection)},
        {kInvalidAnimationTrigger,
         -((float)EClockDirection::SevenOClock* kDegreesPerDirection)},
        {kInvalidAnimationTrigger,
         -((float)EClockDirection::EightOClock* kDegreesPerDirection)},
        {kInvalidAnimationTrigger,
         -((float)EClockDirection::NineOClock* kDegreesPerDirection)},
        {kInvalidAnimationTrigger,
         -((float)EClockDirection::TenOClock* kDegreesPerDirection)},
        {kInvalidAnimationTrigger,
         -((float)EClockDirection::ElevenOClock* kDegreesPerDirection)},
...
BehaviorReactToMicDirection::BehaviorReactToMicDirection(
  ...
  ASSERT_NAMED_EVENT(
      (!allResponses.isNull()), "BehaviorReactToMicDirection.Init",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToMicDirection::GetBehaviorJsonKeys(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToMicDirection::LoadDynamicReactionStateData(
    const Json::Value& config) {
  ...
  {
    for (uint state = 0; state < EDynamicReactionState::Num; ++state) {
      // default everything to null, which tells the behavior to fallback to the
      // default values
      _iVars.reactionEnabled[state] = true;
      _iVars.reactions[state] = nullptr;

      const std::string stateName = ConvertReactionStateToString(
          static_cast<EDynamicReactionState>(state));
      const Json::Value& reactionElement(config[stateName]);
      if (!reactionElement.isNull()) {
        JsonTools::GetValueOptional(reactionElement, kKeyEnabled,
                                    _iVars.reactionEnabled[state]);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToMicDirection::LoadDynamicStateReactions(
    const Json::Value& config, DynamicStateReaction& reactionState) {
  for (uint dir = 0; dir < EClockDirection::NumDirections; ++dir) {
    const std::string directionString =
        ConvertClockDirectionToString(static_cast<EClockDirection>(dir));
    const Json::Value& responseConfig(config[directionString]);
    ASSERT_NAMED_EVENT(!responseConfig.isNull(),
                       "BehaviorReactToMicDirection.Init",
                       "Direction missing from list of reactions [%s]",
                       directionString.c_str());

    // default our values
    reactionState.directionalResponseList[dir] =
        _iVars.defaultReaction.directionalResponseList[dir];
    LoadDirectionResponse(responseConfig,
                          reactionState.directionalResponseList[dir]);
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToMicDirection::LoadDirectionResponse(
    const Json::Value& config, DirectionResponse& outResponse) {
  // all json values are optional ...
  {
    JsonTools::GetValueOptional(config, kKeyReactionDirection,
                                outResponse.facing);

    std::string animString;
    if (JsonTools::GetValueOptional(config, kKeyReactionAnimation,
                                    animString)) {
      if (!EnumFromString(animString, outResponse.animation)) {
        PRINT_NAMED_ERROR(
            "BehaviorReactToMicDirection.LoadDirectionResponse",
  ...
  // note: if the direction is purposely left blank, the "default" value will be
  // used
  bool enabled = true;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToMicDirection::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToMicDirection::EClockDirection
BehaviorReactToMicDirection::ConvertMicDirectionToClockDirection(
    MicDirectionIndex dir) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::string BehaviorReactToMicDirection::ConvertReactionStateToString(
    EDynamicReactionState state) const {
  switch (state) {
    ...
    case EDynamicReactionState::Num:
      PRINT_NAMED_ERROR(
          "BehaviorReactToMicDirection.ConvertReactionStateToString",
          "Invalid conversion of EDynamicReactionState::Num to string");
      break;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::string BehaviorReactToMicDirection::ConvertClockDirectionToString(
    EClockDirection dir) const {
  switch (dir) {
    ...
    case EClockDirection::Invalid:
      PRINT_NAMED_ERROR(
          "BehaviorReactToMicDirection.ConvertClockDirectionToString",
          "Invalid conversion of EClockDirection::Invalid to string");
      break;
  }
...
bool BehaviorReactToMicDirection::WantsToBeActivatedBehavior() const {
  // we heard a sound that we want to focus on, so let's activate our behavior
  // so that we can respond to this setting reactionDirection will cause us to
  // activate
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToMicDirection::OnBehaviorActivated() { RespondToSound(); }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
const BehaviorReactToMicDirection::DirectionResponse&
BehaviorReactToMicDirection::GetResponseData(EClockDirection dir) const {
  // we need to update what set of "sound response data" to use depending on
  // Victor's status. victor can resond differently depeding on his current
  // state (on/off charger, picked up, etc)
  const EDynamicReactionState currentState = GetCurrentReactionState();

  // if we have a reaction for this speciic state, use it, else fall back to the
  // default response
  const DynamicStateReaction* result = _iVars.reactions[currentState];
  return (nullptr != result)
             ? result->directionalResponseList[dir]
             : _iVars.defaultReaction.directionalResponseList[dir];
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToMicDirection::EDynamicReactionState
BehaviorReactToMicDirection::GetCurrentReactionState() const {
...
void BehaviorReactToMicDirection::RespondToSound() {
  // normally we would play an animation, but currently we don't have animation
  // ready, so for now we'll just turn to sound
  if (EClockDirection::Invalid != _dVars.reactionDirection) {
    LOG_DEBUG("BehaviorReactToMicDirection.Reacting",
              "Responding to sound from direction [%u]",
              _dVars.reactionDirection);

    // if an animation was specified, use it, else procedurally turn to the
    // facing direction
    const DirectionResponse& response =
        GetResponseData(_dVars.reactionDirection);
    if (kInvalidAnimationTrigger != response.animation) {
      DelegateIfInControl(
          new TriggerLiftSafeAnimationAction(response.animation));

#if DEBUG_MIC_REACTION_VERBOSE
      {
        LOG_DEBUG("BehaviorReactToMicDirection.Debug",
                  "Playing reaction anim [%s]",
      ...
      if (std::abs(response.facing) >= kMinDegreesToTurn) {
        ...
        {
          LOG_DEBUG("BehaviorReactToMicDirection.Debug",
                    "Playing reaction turn to [%d] degrees",
