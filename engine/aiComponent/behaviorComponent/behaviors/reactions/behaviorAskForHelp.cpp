...
namespace {
const u8 kTracksToLock =
    (u8)AnimTrackFlag::BODY_TRACK | (u8)AnimTrackFlag::LIFT_TRACK;
}
...
void BehaviorAskForHelp::SetAnimTriggers() {
  ...
  // Check if a cliff is detected on either side of ther robot in the location
  // where we're stuck.
  const auto& cliffComp = GetBEI().GetRobotInfo().GetCliffSensorComponent();
  bool leftCliffs = cliffComp.IsCliffDetected(CliffSensor::CLIFF_FL) &&
                    cliffComp.IsCliffDetected(CliffSensor::CLIFF_BL);
  bool rightCliffs = cliffComp.IsCliffDetected(CliffSensor::CLIFF_FR) &&
                     cliffComp.IsCliffDetected(CliffSensor::CLIFF_BR);
...
void BehaviorAskForHelp::TriggerGetInAnim() {
  PRINT_CH_INFO("Behaviors", "BehaviorAskForHelp.TriggerGetInAnim", "%s",
                EnumToString(_dVars.getInTrigger));
  auto action =
      new TriggerAnimationAction(_dVars.getInTrigger, 1, true, kTracksToLock);
...
void BehaviorAskForHelp::TriggerIdleAnim() {
  PRINT_CH_INFO("Behaviors", "BehaviorAskForHelp.TriggerIdleAnim", "%s",
                EnumToString(_dVars.idleTrigger));
  auto action =
      new TriggerAnimationAction(_dVars.idleTrigger, 1, true, kTracksToLock);
  DelegateIfInControl(action, &BehaviorAskForHelp::TriggerIdleAnim);
}
