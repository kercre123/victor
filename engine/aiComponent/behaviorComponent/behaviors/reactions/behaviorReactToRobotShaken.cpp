...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToRobotShaken::BehaviorReactToRobotShaken(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToRobotShaken::GetBehaviorJsonKeys(
...
void BehaviorReactToRobotShaken::OnBehaviorActivated() {
  ...
  _dVars.shakeStartTime =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  _dVars.shakeMaxMagnitude =
      GetBEI().GetRobotInfo().GetHeadAccelMagnitudeFiltered();

  TriggerAnimationAction* animationAction =
      new TriggerAnimationAction(_iVars.getInAnimation);
...
void BehaviorReactToRobotShaken::PlayNextShakeReactionLoop() {
  switch (_dVars.state) {
    case EState::ShakeGetIn: {
      ...
      // the first shaking loop always begins with the EReactionLevel::Soft, so
    ...
    case EState::Shaking: {
      // update our current reaction level so that we grab the appropriate
      // animation
    ...
    default:
      DEV_ASSERT(
          false,
          "BehaviorReactToRobotShaken.PlayNextShakeReactionLoop.InvalidState");
      return;
  }

  // we retrigger this looping animation each time through so that we can adjust
  // the "level" in case the shaking magnitude increases
  const AnimationTrigger reactionAnim =
      GetReactionAnimation(EReactionAnimation::Loop);
  TriggerAnimationAction* animationAction =
      new TriggerAnimationAction(reactionAnim);
  animationAction->SetRenderInEyeHue(_iVars.renderInEyeHue);
...
void BehaviorReactToRobotShaken::TransitionToDoneShaking() {
  ...
  auto getOutCallback = [this]() {
    // now that we've had time to transition out of the shake anim, check if
    // we're on the ground or still in hand and play the appropriate animation
    const bool isOnTreads =
        (GetBEI().GetOffTreadsState() == OffTreadsState::OnTreads);
    const EReactionAnimation reactionType =
        (isOnTreads ? EReactionAnimation::OnGround
                    : EReactionAnimation::InHand);
  ...
  const AnimationTrigger reactionAnim =
      GetReactionAnimation(EReactionAnimation::ShakeTransition);
  TriggerAnimationAction* animationAction =
      new TriggerAnimationAction(reactionAnim);
  animationAction->SetRenderInEyeHue(_iVars.renderInEyeHue);
...
void BehaviorReactToRobotShaken::OnBehaviorDeactivated() {
  ...
  DASMSG(robot_dizzy_reaction, "robot.dizzy_reaction",
         "Robot is dizzy after being shaken");
  DASMSG_SET(i1, maxShakenAccelMag,
             "max shake magnitude while robot was shaken");
...
void BehaviorReactToRobotShaken::UpdateCurrentReactionLevel() {
  ...
  LOG_DEBUG("BehaviorReactToRobotShaken.UpdateCurrentReactionLevel",
            "Current ReactionLevel now %d", (int)_dVars.currentLevel);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToRobotShaken::ReactionLevel
BehaviorReactToRobotShaken::GetReactionLevelFromMagnitude() const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToRobotShaken::ShakeReaction::ShakeReaction(
    const Json::Value& config) {
  static const std::string debugString =
      "BehaviorReactToRobotShaken.ShakeReaction.Load";

  const auto loadAnimation =
      [&config](const char* animKey) -> AnimationTrigger {
    ASSERT_NAMED_EVENT(config.isMember(animKey), debugString.c_str(),
                       "must specify shake animation (%s)", animKey);

    const std::string& trigger =
        JsonTools::ParseString(config, animKey, debugString);
    return AnimationTriggerFromString(trigger);
  };

  threshold = JsonTools::ParseFloat(config, kKey_TriggerThreshold, debugString);

  animations[(size_t)EReactionAnimation::Loop] =
      loadAnimation(kKey_AnimLooping);
  animations[(size_t)EReactionAnimation::ShakeTransition] =
      loadAnimation(kKey_AnimWaiting);
  animations[(size_t)EReactionAnimation::InHand] =
      loadAnimation(kKey_AnimInHand);
  animations[(size_t)EReactionAnimation::OnGround] =
      loadAnimation(kKey_AnimOnGround);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToRobotShaken::LoadReactionAnimations(
    const Json::Value& config) {
  const std::string& debugString =
      "BehaviorReactToRobotShaken.LoadReactionAnimations";

  // Do we want to render these animations in the eye hue, or use the rgb from
  // the animations themselves
  _iVars.renderInEyeHue = config.get(kKey_RenderInEyeHue, true).asBool();

  // load our get-in animation
  ASSERT_NAMED_EVENT(config.isMember(kKey_AnimGetIn), debugString.c_str(),
                     "must specify get-in animation (%s)", kKey_AnimGetIn);
  const std::string& getInAnimString =
      JsonTools::ParseString(config, kKey_AnimGetIn, debugString);
  _iVars.getInAnimation = AnimationTriggerFromString(getInAnimString);

  // load all of our reaction animations
  ASSERT_NAMED_EVENT(config.isMember(kKey_Animations), debugString.c_str(),
                     "missing required field (%s)", kKey_Animations);
  for (const auto& reactionConfig : config[kKey_Animations]) {
    _iVars.shakeReactions.emplace_back(reactionConfig);
  }

  ASSERT_NAMED_EVENT(!_iVars.shakeReactions.empty(), debugString.c_str(),
                     "must specify at least one set of reactions");

  // order our animations by ascending threshold so that we can easily jump
  // between them as we shake
  std::sort(_iVars.shakeReactions.begin(), _iVars.shakeReactions.end(),
            [](const ShakeReaction& lhs, const ShakeReaction& rhs) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
AnimationTrigger BehaviorReactToRobotShaken::GetReactionAnimation(
...
// - - - - - - - - - - - - - - - - - - - -
AnimationTrigger BehaviorReactToRobotShaken::GetReactionAnimation(
    ReactionLevel level, EReactionAnimation type) const {
