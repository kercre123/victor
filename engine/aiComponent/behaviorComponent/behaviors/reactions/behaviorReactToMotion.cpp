...
// wait this long after turning before looking for motion. this both helps make
// sure the camera is steady before looking for motion, and elongates the time
// spent in this state
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToMotion::InstanceConfig::InstanceConfig(
    const Json::Value& config) {
  eyeTimeout_s =
      JsonTools::ParseFloat(config, kEyeMotionTimeoutKey, kDebugName);
  eyeShiftTimeout_s =
      JsonTools::ParseFloat(config, kEyeShiftTimeoutKey, kDebugName);
  DEV_ASSERT(eyeTimeout_s >= eyeShiftTimeout_s,
             "eyeMotionTimeout must be >= eyeMotionShiftTimeout");

  ticksInIntervalForTurn =
      JsonTools::ParseUInt32(config, kTicksInIntervalForTurnKey, kDebugName);
  turnIntervalSize =
      JsonTools::ParseUInt32(config, kTurnIntervalSizeKey, kDebugName);

  procedural = false;
  JsonTools::GetValueOptional(config, kProceduralKey, procedural);
  if (procedural) {
    procHeadAngle_deg =
        JsonTools::ParseFloat(config, kProceduralHeadAngleKey, kDebugName);
    procTurnAngle_deg =
        JsonTools::ParseFloat(config, kProceduralTurnAngleKey, kDebugName);
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToMotion::AreaCondition::AreaCondition(
    MotionArea a, unsigned int intervalSize, Json::Value& config,
    const std::string& ownerDebugLabel)
    : area(a), condition(new ConditionMotionDetected(config)) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToMotion::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToMotion::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {
      kProceduralKey,          kEyeMotionTimeoutKey,
      kEyeShiftTimeoutKey,     kTicksInIntervalForTurnKey,
      kTurnIntervalSizeKey,    kProceduralTurnAngleKey,
...
void BehaviorReactToMotion::InitBehavior() {
  ...
  _dVars.motionConditions.emplace_back(
      MotionArea::Left, _iConfig.turnIntervalSize, config, GetDebugLabel());
  _dVars.motionConditions.back().condition->Init(GetBEI());
  config["motionArea"] = "Right";
  _dVars.motionConditions.emplace_back(
      MotionArea::Right, _iConfig.turnIntervalSize, config, GetDebugLabel());
  _dVars.motionConditions.back().condition->Init(GetBEI());
  config["motionArea"] = "Top";
  _dVars.motionConditions.emplace_back(
      MotionArea::Top, _iConfig.turnIntervalSize, config, GetDebugLabel());
  _dVars.motionConditions.back().condition->Init(GetBEI());
}
...
void BehaviorReactToMotion::OnBehaviorActivated() {
  ...
  if (mostMotionArea != MotionArea::None) {
    // todo: once we decide whether eyes should precede turns, and whether to
    // use animations vs procedural, simplify all this logic
    ...
    PRINT_NAMED_WARNING(
        "BehaviorReactToMotion.OnBehaviorActivated.NoMotionArea",
...
void BehaviorReactToMotion::BehaviorUpdate() {
  ...
  const bool motorsMoving =
      GetBEI().GetRobotInfo().GetMoveComponent().IsMoving();
  const bool pickedUp = GetBEI().GetRobotInfo().IsPickedUp();
  if (motorsMoving || pickedUp) {
    _dVars.inMotion = true;

    // if running and the robot is picked up, stop. don't worry about if running
    // and motorsMoving, since the behavior handles that by tracking
    // _dVars.state.
  ...
  if (_dVars.state == State::EyeTracking) {
    auto itCurrent = std::find_if(
        _dVars.motionConditions.begin(), _dVars.motionConditions.end(),
        [this](const auto& x) { return x.area == _dVars.lookingDirection; });
    if (!ANKI_VERIFY((itCurrent != _dVars.motionConditions.end()) &&
                         (itCurrent->lastMotionTime_s >= 0.0f),
    ...
    // if motion has happened in a different location, and the previous location
    // hasnt seen motion in some amount of time, switch eye tracking to a
    // different spot
    ...
    if (!GetBEI().GetRobotInfo().IsOnChargerPlatform()) {
      // if motion has repeatedly happened in the same spot for a while, turn
      // towards it
    ...
    // wait a small while after entering this state so we're completely sure
    // we've stopped turning
    const bool lookingForMotion = (_dVars.timeFinishedTurning_s < 0.0f);
    const bool timeExpired =
        !lookingForMotion &&
        (currTime - _dVars.timeFinishedTurning_s > kTurnTimeBuffer);
    if (timeExpired) {
    ...
    if (lookingForMotion) {
      ...
      if (mostMotionArea != MotionArea::None) {
        ...
        // if enough time has passed without motion, time out and exit
        if (currTime - _dVars.timeFinishedTurning_s >
            kTurnTimeBuffer + _iConfig.eyeTimeout_s) {
          TransitionToCompleted();
          return;
        }
      }
...
void BehaviorReactToMotion::TransitionToEyeAnimation(MotionArea area) {
  ...
  if (_dVars.animationEyeShiftActive) {
    ...
    if (_dVars.lookingDirection == MotionArea::Left) {
      action->AddAction(new TriggerLiftSafeAnimationAction(
          AnimationTrigger::ReactToMotionLeftGetout));
    } else if (_dVars.lookingDirection == MotionArea::Right) {
      action->AddAction(new TriggerLiftSafeAnimationAction(
          AnimationTrigger::ReactToMotionRightGetout));
    } else if (_dVars.lookingDirection == MotionArea::Top) {
      action->AddAction(new TriggerLiftSafeAnimationAction(
          AnimationTrigger::ReactToMotionUpGetout));
    }
  }
  ...
  u8 tracksToLock = GetBEI().GetRobotInfo().IsOnChargerPlatform()
                        ? (static_cast<u8>(AnimTrackFlag::BODY_TRACK) |
                           static_cast<u8>(AnimTrackFlag::HEAD_TRACK))
                        : static_cast<u8>(AnimTrackFlag::HEAD_TRACK);

  action->AddAction(
      new TriggerLiftSafeAnimationAction(trigger, 1, true, tracksToLock));
...
void BehaviorReactToMotion::TransitionToProceduralEyes(MotionArea area) {
  ...
  const TimeStamp_t duration =
      static_cast<TimeStamp_t>(1.0f + _iConfig.eyeTimeout_s);
  GetBEI().GetAnimationComponent().AddOrUpdateEyeShift(kEyeMotionLayer, pixX,
                                                       pixY, duration);
}
...
void BehaviorReactToMotion::TransitionToTurnAnimation() {
  ...
  CancelDelegates(false);
  DelegateIfInControl(
      new TriggerLiftSafeAnimationAction(trigger),
      [this](const ActionResult& result) { TransitionToTurnedAndWaiting(); });
}
...
void BehaviorReactToMotion::TransitionToProceduralTurn() {
  ...
  CancelDelegates(false);
  DelegateIfInControl(
      action, [this](ActionResult result) { TransitionToTurnedAndWaiting(); });
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToMotion::TransitionToTurnedAndWaiting() {
  _dVars.timeFinishedTurning_s =
  ...
  for (auto& motionCondition : _dVars.motionConditions) {
    if (ANKI_VERIFY(motionCondition.condition != nullptr,
      ...
      if (motionCondition.condition->AreConditionsMet(GetBEI())) {
        ...
        motionCondition.motionLevel =
            motionCondition.condition->GetLastObservedMotionLevel();
        ...
        if (motionCondition.historyBuff.front()) {
          motionCondition.intervalCount -= 1;
          DEV_ASSERT(motionCondition.intervalCount >= 0,
                     "Shouldn't end up negative");
        }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToMotion::MotionArea BehaviorReactToMotion::GetAreaWithMostMotion()
    const {
...
void BehaviorReactToMotion::DevAddFakeMotion() {
  if ((GetAreaWithMostMotion() == MotionArea::None) ||
...
void BehaviorReactToMotion::TransitionToCompleted(bool revertHead) {
  ...
  if (action != nullptr) {
    CancelDelegates(false);
    DelegateIfInControl(action, [this](ActionResult result) { CancelSelf(); });
  } else {
    CancelSelf();
  }
}
