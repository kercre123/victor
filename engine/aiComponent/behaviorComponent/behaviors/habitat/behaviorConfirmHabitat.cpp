...
BehaviorConfirmHabitat::BehaviorConfirmHabitat(const Json::Value& config)
  ...
  SubscribeToTags({{EngineToGameTag::RobotStopped, EngineToGameTag::CliffEvent,
  ...
  DEV_ASSERT(config.isMember(kOnTreadsTimeCondition),
             "ConfirmHabitat.Ctor.OnTreadsTimeConditionUndefined");
  _iConfig.onTreadsTimeCondition = BEIConditionFactory::CreateBEICondition(
      config[kOnTreadsTimeCondition], GetDebugLabel());
  ...
  _iConfig.searchForChargerBehaviorIDStr = JsonTools::ParseString(
      config, kSearchForChargerBehaviorKey, GetDebugLabel());
}
...
void BehaviorConfirmHabitat::InitBehavior() {
  ...
  auto behaviorID = BehaviorTypesWrapper::BehaviorIDFromString(
      _iConfig.searchForChargerBehaviorIDStr);
  _iConfig.searchForChargerBehavior =
      GetBEI().GetBehaviorContainer().FindBehaviorByID(behaviorID);
  DEV_ASSERT(_iConfig.searchForChargerBehavior != nullptr,
             "ConfimHabitat.InitBehavior.NullSearchForChargerBehavior");
}

void BehaviorConfirmHabitat::GetAllDelegates(
...
bool BehaviorConfirmHabitat::WantsToBeActivatedBehavior() const {
  ...
  if (_iConfig.onTreadsTimeCondition != nullptr) {
    onTreadsTimeValid =
        _iConfig.onTreadsTimeCondition->AreConditionsMet(GetBEI());
  }

  if (!onChargerPlatform && !intentsPendingOrActive &&
      hbelief == HabitatBeliefState::Unknown && onTreadsTimeValid) {
    ...
    const auto checkInterruptCallback =
        [&canInterrupt](const ICozmoBehavior& behavior) -> bool {
...
void BehaviorConfirmHabitat::GetBehaviorJsonKeys(
...
bool BehaviorConfirmHabitat::CheckIfFrontCliffsAreInLogoRegion() {
  const auto getCliffSensorPoseWRTLogo =
      [this](float x, float y) -> std::pair<bool, Pose3d> {
  ...
  auto gotFL = getCliffSensorPoseWRTLogo(kCliffSensorXOffsetFront_mm,
                                         kCliffSensorYOffset_mm);
  if (!gotFL.first) {
    return false;
  }

  auto gotFR = getCliffSensorPoseWRTLogo(kCliffSensorXOffsetFront_mm,
                                         -kCliffSensorYOffset_mm);
  if (!gotFR.first) {
    return false;
  }
  ...
  // we pad these measurements to 50mm x 90mm to account for slop in charger
  // pose-estimate these measurements are taken using CAD drawings note: the
  // LogoCenter is slightly offset inside the rectangle
...
void BehaviorConfirmHabitat::BehaviorUpdate() {
  ...
  if (habitatDetector.GetHabitatBeliefState() ==
    ...
    return;
  } else if (habitatDetector.GetHabitatBeliefState() ==
             HabitatBeliefState::NotInHabitat) {
    CancelSelf();
    return;
  }
  ...
  switch (_dVars._phase) {
    case ConfirmHabitatPhase::InitialCheckObstacle: {
      ...
      if (UpdateProxSensorFilter()) {
        if (CheckIsCloseToObstacle()) {
          ...
          TransitionToBackupTurnForward(-20, RandomSign() * DEG_TO_RAD(30.0f),
                                        0);
        }
    ...
    case ConfirmHabitatPhase::LocalizeCharger: {
      ...
      if (chargerSeen) {
        PRINT_CH_INFO(
            "Behaviors",
            "ConfirmHabitat.BehaviorUpdate.LocalizeCharger.FoundCharger", "");
        _dVars._phase = ConfirmHabitatPhase::SeekLineFromCharger;
        TransitionToSeekLineFromCharger();
      } else {
        TransitionToLocalizeCharger();
    ...
    case ConfirmHabitatPhase::SeekLineFromCharger: {
      ...
      if (habitatDetector.GetHabitatLineRelPose() !=
              HabitatLineRelPose::Invalid &&
          habitatDetector.GetHabitatLineRelPose() !=
              HabitatLineRelPose::AllGrey) {
        _dVars._phase = ConfirmHabitatPhase::LineDocking;
      } else {
        PRINT_CH_INFO(
            "Behaviors",
            "ConfirmHabitat.BehaviorUpdate.SeekLine.NotStoppedOnWhite", "");
    ...
    case ConfirmHabitatPhase::RandomWalk: {
      ...
      } else if (habitatDetector.GetHabitatLineRelPose() ==
                     HabitatLineRelPose::Invalid ||
                 habitatDetector.GetHabitatLineRelPose() ==
                     HabitatLineRelPose::AllGrey) {
        TransitionToRandomWalk();
      ...
      switch (habitatDetector.GetHabitatLineRelPose()) {
        case HabitatLineRelPose::AllGrey: {
          PRINT_CH_INFO("Behaviors",
                        "ConfirmHabitat.BehaviorUpdate.LineDocking.LineLost",
                        "");
          _dVars._phase = ConfirmHabitatPhase::RandomWalk;
          TransitionToRandomWalk();
          break;
        ...
        case HabitatLineRelPose::WhiteBR: {
          ...
          f32 angle = habitatDetector.GetHabitatLineRelPose() ==
                              HabitatLineRelPose::WhiteBL
                          ? (M_PI_4_F)
                          : (-M_PI_4_F);
...
void BehaviorConfirmHabitat::DelegateActionHelper(
  ...
  if (action != nullptr) {
    ...
    GetBEI().GetRobotInfo().GetPathComponent().SetCustomMotionProfileForAction(
        slowProfile, action);

    // add an action callback to print the result if we didn't assign one
    if (callback == nullptr) {
      callback =
          [](const ExternalInterface::RobotCompletedAction& msg) -> void {
        switch (msg.result) {
          ...
          default: {
            PRINT_CH_INFO(
                "Behaviors",
                "ConfirmHabitat.DelegateActionHelper.UnhandledActionResult",
...
void BehaviorConfirmHabitat::TransitionToReactToHabitat() {
  ...
  action->AddAction(new WaitForLambdaAction(
      [](Robot& robot) -> bool {
  ...
  action->AddAction(
      new TriggerAnimationAction(AnimationTrigger::ReactToHabitat, 1, true));
  ...
  RobotCompletedActionCallback callback =
      [this](const ExternalInterface::RobotCompletedAction& msg) -> void {
    this->CancelSelf();
  };
...
void BehaviorConfirmHabitat::TransitionToLocalizeCharger() {
  ...
  BehaviorSimpleCallback callback = [this](void) -> void {
    ...
    if (GetChargerIfObserved() != nullptr) {
      PRINT_CH_INFO("Behaviors",
                    "ConfirmHabitat.TransitionToLocalizeCharger.ChargerFound",
                    "");
      _dVars._phase = ConfirmHabitatPhase::SeekLineFromCharger;
      TransitionToSeekLineFromCharger();
    } else {
      PRINT_CH_INFO(
          "Behaviors",
          "ConfirmHabitat.TransitionToLocalizeCharger.ChargerNotFound", "");
      _dVars._phase = ConfirmHabitatPhase::RandomWalk;
    }
  };
...
void BehaviorConfirmHabitat::TransitionToCliffAlignWhite() {
  PRINT_CH_INFO("Behaviors", "ConfirmHabitat.TransitionToCliffAlignWhite", "%s",
                _dVars._cliffAlignRetry ? "Retry" : "");

  IActionRunner* action = new CliffAlignToWhiteAction();
  DEV_ASSERT_MSG(action != nullptr,
                 "ConfirmHabitat.TransitionToCliffAlignWhite.NullActionPtr",
                 "");
    ...
    switch (msg.result) {
      ...
      case ActionResult::CLIFF_ALIGN_FAILED_TIMEOUT: {
        if (!_dVars._cliffAlignRetry) {
          ...
          _dVars._phase = ConfirmHabitatPhase::RandomWalk;
          TransitionToBackupTurnForward(-30, RandomSign() * DEG_TO_RAD(30.0f),
                                        40);
          break;
        }
      ...
      case ActionResult::CLIFF_ALIGN_FAILED_OVER_TURNING:  // deliberate fall
                                                           // through
      {
        ...
        GetBEI().GetHabitatDetectorComponent().ForceSetHabitatBeliefState(
            HabitatBeliefState::NotInHabitat, ss.str());
        break;
      }
      case ActionResult::CLIFF_ALIGN_FAILED_STOPPED:
      case ActionResult::CLIFF_ALIGN_FAILED_NO_WHITE:  // deliberate fall
                                                       // through
      {
        PRINT_NAMED_WARNING("ConfirmHabitat.TransitionToCliffAlignWhite.Failed",
                            "%s", EnumToString(msg.result));
        break;
      }
      case ActionResult::CLIFF_ALIGN_FAILED_NO_TURNING: {
        PRINT_CH_INFO("Behaviors",
                      "ConfirmHabitat.TransitionToCliffAlignWhite.Failed", "");
        ...
        switch (linePose) {
          ...
          case HabitatLineRelPose::WhiteBR: {
            TransitionToBackupTurnForward(-20, 0.0f,
                                          0);  // no turn requested here
            break;
          }
          default: {
            PRINT_NAMED_WARNING(
                "ConfirmHabitat.TransitionToCliffAlignWhite.UnhandledLinePose",
      ...
      default: {
        PRINT_CH_INFO(
            "Behaviors",
            "ConfirmHabitat.TransitionToCliffAlignWhite.UnhandledActionResult",
            "%s", EnumToString(msg.result));
      }
    }
  };
...
void BehaviorConfirmHabitat::TransitionToReactToWhite(
    HabitatLineRelPose lineRelPose) {
  u8 bitFlags = 0;
  switch (lineRelPose) {
    case HabitatLineRelPose::WhiteFL: {
      bitFlags = kFL;
...
void BehaviorConfirmHabitat::TransitionToReactToWhite(
  ...
  PRINT_CH_INFO("Behaviors", "ConfirmHabitat.TransitionToWhiteReactionAnim",
                "%d", (int)whiteDetectedFlags);

  const float currTime_sec =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  const bool doWhiteReactionAnim =
      currTime_sec > _dVars._nextWhiteReactionAnimTime_sec;
  ...
  if (doWhiteReactionAnim) {
    ...
    switch (whiteDetectedFlags) {
      case (kFL | kFR): {
        animToPlay = AnimationTrigger::ExploringHuhClose;
        break;
      }
      case kFL: {
        animToPlay = AnimationTrigger::ExploringHuhClose;
      ...
      case (kBL | kBR): {
        ...
        PRINT_CH_INFO("Behaviors",
                      "ConfirmHabitat.TransitionToReactToWhite."
                      "BackCliffDetectingWhiteNoReact",
                      "");
        delete action;
        return;
      }
      default: {
        // This is some scary configuration that we probably shouldn't move
        // from.
        delete (action);
        PRINT_NAMED_WARNING(
            "ConfirmHabitat.TransitionToReactToWhite.InvalidCliffSensorConfig",
            "");
        return;
      }
    }
    action->AddAction(new TriggerLiftSafeAnimationAction(
        animToPlay, 1, true, (u8)AnimTrackFlag::BODY_TRACK));

    // sometimes after playing an animation, the lift is blocking the prox
    action->AddAction(
        new MoveLiftToHeightAction(MoveLiftToHeightAction::Preset::LOW_DOCK));

    _dVars._nextWhiteReactionAnimTime_sec =
        currTime_sec + kReactToWhiteAnimCooldown_sec;
  }

  // insert a short wait to observe the line
  // when we receive a StopOnWhite message, there is a 1-2 tick lag for engine
  // to register that it is seeing a white line beneath it. This is used as a
  // buffer for engine to catch up and update its internal tracking of white
  action->AddAction(new WaitForLambdaAction(
      [](Robot& robot) -> bool {
        return robot.GetHabitatDetectorComponent().GetHabitatLineRelPose() !=
                   HabitatLineRelPose::Invalid &&
               robot.GetHabitatDetectorComponent().GetHabitatLineRelPose() !=
                   HabitatLineRelPose::AllGrey;
      },
      0.1f));
...
void BehaviorConfirmHabitat::TransitionToBackupTurnForward(int backDist_mm,
                                                           f32 angle,
                                                           int forwardDist_mm) {
  PRINT_CH_INFO("Behaviors", "ConfirmHabitat.TransitionToBackupTurnForward",
                "%d %4.2f %d", backDist_mm, angle, forwardDist_mm);
  CompoundActionSequential* action =
      new CompoundActionSequential(std::list<IActionRunner*>{
...
void BehaviorConfirmHabitat::TransitionToTurnBackupForward(f32 angle,
                                                           int backDist_mm,
                                                           int forwardDist_mm) {
  ...
  CompoundActionSequential* action =
      new CompoundActionSequential(std::list<IActionRunner*>{
          ...
          new WaitForLambdaAction([](Robot& robot) -> bool {
            robot.GetComponentPtr<CliffSensorComponent>()->EnableStopOnWhite(
                false);
            return true;
          })});
...
void BehaviorConfirmHabitat::TransitionToRandomWalk() {
  ...
  if (index == 0) {
    ...
    desiredOffsetPose =
        Pose3d(Rotation3d(-M_PI_4, Z_AXIS_3D()), Vec3f(75, -50, 0));
  } else if (index == 2) {
    desiredOffsetPose =
        Pose3d(Rotation3d(M_PI_4, Z_AXIS_3D()), Vec3f(75, 50, 0));
  }
  ...
  DelegateActionHelper(action, std::move(callback));
  PRINT_CH_INFO("Behaviors", "ConfirmHabitat.TransitionToRandomWalk",
                "ActionNum=%d", index);
}
...
void BehaviorConfirmHabitat::TransitionToSeekLineFromCharger() {
  ...
  if (!ANKI_VERIFY(
          charger != nullptr,
          "ConfirmHabitat.TransitionToSeekLineFromCharger.ChargerIsNull", "")) {
    _dVars._phase = ConfirmHabitatPhase::RandomWalk;
    return;
  }
...
const ObservableObject* BehaviorConfirmHabitat::GetChargerIfObserved() const {
  ...
  return GetBEI().GetBlockWorld().FindLocatedObjectClosestTo(robotPose,
                                                             *_chargerFilter);
}

void BehaviorConfirmHabitat::HandleWhileActivated(
  ...
  switch (event.GetData().GetTag()) {
    ...
    case EngineToGameTag::UnexpectedMovement: {
      PRINT_CH_INFO("Behaviors",
                    "ConfirmHabitat.HandleWhiteActivated.UnexpectedMovement",
                    "");
      if (_dVars._phase == ConfirmHabitatPhase::RandomWalk &&
          IsControlDelegated()) {
        CancelDelegates();
        ...
        const auto& proxData = GetBEI()
                                   .GetRobotInfo()
                                   .GetProxSensorComponent()
                                   .GetLatestProxData();
        if (proxData.foundObject &&
            proxData.distance_mm <=
                HabitatDetectorComponent::kConfirmationConfigProxMaxReading) {
...
void BehaviorConfirmHabitat::AlwaysHandleInScope(
    const EngineToGameEvent& event) {
  using namespace ExternalInterface;
  switch (event.GetData().GetTag()) {
    case EngineToGameTag::RobotStopped: {
      ...
      const auto belief =
          GetBEI().GetHabitatDetectorComponent().GetHabitatBeliefState();
      if (reason == StopReason::WHITE &&
          belief == HabitatBeliefState::Unknown) {
        if (IsActivated()) {
          if (!CheckIfFrontCliffsAreInLogoRegion()) {
            PRINT_CH_INFO("Behaviors",
                          "ConfirmHabitat.AlwaysHandleInScope.StoppedOnWhite",
                          "");
            TransitionToReactToWhite(
                event.GetData().Get_RobotStopped().whiteDetectedFlags);
          } else {
