...
CONSOLE_VAR_RANGED(float, kProbReferenceOnResume, "BehaviorExploring", 1.0f,
                   0.0f, 1.0f);
CONSOLE_VAR_RANGED(float, kResumeReferenceCooldown_s, "BehaviorExploring",
                   20.0f, 0.0f, 60.0f);

// if no face is known (meaning we can't run the referencing behavior) then run
// a short face search (at most) this often (instead of driving to a new pose)
const float kPeriodToCheckForFaces_s = 30.0f;

// If we have driven this far since the last time we looked at the charger, then
// we should turn and look at the charger before driving again. This helps keep
// the nav map accurate.
const float kReferenceChargerDistanceThreshold_mm = 200.f;

static_assert(kMinCliffPenaltyDist_mm < kMaxCliffPenaltyDist_mm,
              "Max must be > min");

// if driving to a goal this far away, it's ok if the robot stops to examine a
// nearby obstacle
const float kMinGoalDistForPitStop_mm = 300.0f;
const float kMinPathCompleteForPitStop =
    0.3f;  // >= this % of the distance followed
const float kMaxPathCompleteForPitStop =
    0.7f;  // <= this % of the distance followed

const float kChargerRadius_mm =
    68.89f;  // radius of circle that circumscribes the charger

const char* const kDebugName = "BehaviorExploring";

// number of sample positions for new poses. multiple goals are provided so that
// the planner can choose one, which will probably cause the robot to tend in
// its current direction when possible
const unsigned int kNumPositionsForSearch = 2;
// It's not terribly expensive (todo: check this) to search for multiple pose
// angles on a given point, so increase this if needed
const unsigned int kNumAnglesAtPose = 5;
// number of samples before we give up trying to find a non-colliding pose.
// Note: the behavior will end if no pose was found
const unsigned int kNumSampleSteps = 50;

// number of poses to select that are offset from a prox obstacle by some nearby
// amount
const unsigned int kNumProxPoses = 1;
// distance prox obstacles should be from one another to be included in the
// sample list (set to 0 for an efficiency boost!)
...
BehaviorExploring::DynamicVariables::DynamicVariables() {
  state = State::Invalid;
  sampledPoses.clear();
  sampledPoses.reserve(kNumProxPoses +
                       kNumPositionsForSearch * kNumAnglesAtPose);
...
BehaviorExploring::BehaviorExploring(const Json::Value& config)
  ...
  _iConfig.minSearchRadius_m =
      JsonTools::ParseFloat(config, kMinSearchRadiusKey, kDebugName);
  _iConfig.maxSearchRadius_m =
      JsonTools::ParseFloat(config, kMaxSearchRadiusKey, kDebugName);
  _iConfig.maxChargerDistance_m =
      JsonTools::ParseFloat(config, kMaxChargerDistanceKey, kDebugName);

  _iConfig.pAcceptKnownAreas =
      JsonTools::ParseFloat(config, kPAcceptKnownAreasKey, kDebugName);
  ...
  MakeMemberTunable(_iConfig.minSearchRadius_m, kMinSearchRadiusKey,
                    kDebugName);
  MakeMemberTunable(_iConfig.maxSearchRadius_m, kMaxSearchRadiusKey,
                    kDebugName);
  MakeMemberTunable(_iConfig.maxChargerDistance_m, kMaxChargerDistanceKey,
                    kDebugName);
  MakeMemberTunable(_iConfig.pAcceptKnownAreas, kPAcceptKnownAreasKey,
                    kDebugName);
}
...
bool BehaviorExploring::WantsToBeActivatedBehavior() const {
  const bool chargerKnown =
      _iConfig.allowNoCharger ||
      IsChargerPositionKnown();  // this should also probably be an
                                 // IsObjectKnown BEICondition
...
void BehaviorExploring::InitBehavior() {
  ...
  _iConfig.referenceHumanBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(ExploringReferenceHuman));
  _iConfig.searchForHumanBehavior =
      BC.FindBehaviorByID(BEHAVIOR_ID(ShortLookAroundForFaceAndCube));

  using namespace RobotPointSamplerHelper;
  _iConfig.openSpacePointEvaluator.reset(
      new Util::RejectionSamplerHelper<Point2f>());
  _iConfig.openSpacePolyEvaluator.reset(
      new Util::RejectionSamplerHelper<Poly2f>());

  // below is a list of factors for rejection sampling. other ideas:
  // how much of a ray covers unknown ground? (totally possible with current
  // accumulators,
  //    but # quads in a line segment unknown, so hard to normalize into a
  //    probability)
  // sample only from unknown points... maybe this needs the iNavMap method
  // FindContentIf, and
  //    then sampled from that? depends on how big the return set is and how
  //    many quads share a data obj
  // sample only in angles near the current angle? hopefully the planner does
  // this job for us

  _iConfig.condHandleNearCharger =
      _iConfig.openSpacePointEvaluator->AddCondition(
          std::make_shared<RejectIfNotInRange>(
              0.0f, M_TO_MM(_iConfig.maxChargerDistance_m)));
  ...
  _iConfig.condHandleCliffs->SetAcceptanceInterpolant(kMaxCliffPenaltyDist_mm,
                                                      GetRNG());

  _iConfig.condHandleChargerOutOfView =
      _iConfig.openSpacePointEvaluator->AddCondition(
  ...
  _iConfig.condHandleUnknowns->SetAcceptanceProbability(
      _iConfig.pAcceptKnownAreas, GetRNG());
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorExploring::GetBehaviorOperationModifiers(
  ...
  modifiers.visionModesForActiveScope->insert({
      {VisionMode::Markers, EVisionUpdateFrequency::Low},
      {VisionMode::Faces,
       EVisionUpdateFrequency::Med}  // so it is able to occasionally look back
                                     // at faces
  });
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorExploring::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {
      kMinSearchRadiusKey,   kMaxSearchRadiusKey, kMaxChargerDistanceKey,
      kPAcceptKnownAreasKey, kAllowNoChargerKey,  kMotionProfileKey,
  };
...
void BehaviorExploring::OnBehaviorActivated() {
  // if there is a valid end reason, then the last time this behavior ran it
  // stopped on purpose. Otherwise, it's either the first time this behavior has
  // ever run, or we were interrupted last time
  ...
  const bool isResume =
      everRan && !hasEndReason &&
      (currTime_s - _dVars.timeDeactivated_s <= kMaxTimeToCountAsResume_s);

  // reset dynamic variables
  _dVars = DynamicVariables();
  _dVars.lastSearchForFaceTime_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  ...
  if (isResume) {
    if (_iConfig.referenceHumanBehavior->WantsToBeActivated() &&
        (currTime_s - _iConfig.referenceHumanBehavior->GetTimeActivated_s() >
      ...
      LOG_INFO("BehaviorExploring.OnBehaviorActivated.ResumeReference",
               "do resume reference");
      DelegateIfInControl(_iConfig.referenceHumanBehavior.get(),
                          &BehaviorExploring::SampleAndDrive);
      return;
    }
  }
...
void BehaviorExploring::OnBehaviorDeactivated() {
  ...
  LOG_INFO("BehaviorExploring.Ended", "reason: %s",
           _dVars.endReason.empty() ? "<NONE>" : _dVars.endReason.c_str());

  _dVars.timeDeactivated_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
}
...
void BehaviorExploring::BehaviorUpdate() {
  ...
  if (currTick > _dVars.devWarnIfNotInterruptedByTick) {
    PRINT_NAMED_WARNING("BehaviorExploring.BehaviorUpdate.WasNonInterrupted",
                        "Behavior assumed an interruption that didn't occur. "
                        "This could mean the robot stops driving");
    _dVars.devWarnIfNotInterruptedByTick = std::numeric_limits<size_t>::max();
    if (_dVars.numDriveAttempts <= 2) {
      SampleAndDrive();
    } else {
      // numDriveAttempts is reset every activation, so this should only happen
      // if something internal is cancelling the driving
      PRINT_NAMED_WARNING(
          "BehaviorExploring.BehaviorUpdate.InterruptedMultiple",
  ...
  if (_dVars.state == State::SearchForHuman) {
    // if we're searching, but could reference instead, then do that by going to
    // the "Arrived" state similarly, if the search ended without finding
    // someone, do the same
  ...
  // tell our examine delegate that it's ok to stop mid path if an unexplored
  // obstacle is passed. this is allowed when the robot is driving a long path
  // and is about halfway to the goal (as the bird flies). otherwise, it looks a
  // bit silly that a robot plans a path around something it hasn't examined up
  // close yet
  // ("how does it know it's there if it hasn't examined it?")
  // It might make sense to do the memory map checks here and delegate
  // accordingly, instead of depending on this behavior's WantsToBeActivated,
  // but this way all memory map checks (aside from sampling) are in one
  // behavior
  bool cutDistanceByHalf = false;
  if ((_dVars.distToGoal_mm > kMinGoalDistForPitStop_mm) &&
      !_dVars.sampledPoses.empty()) {
    const float currDist_mm = CalcDistToCachedGoal(0);
    const float perc = currDist_mm / _dVars.distToGoal_mm;
    cutDistanceByHalf = (perc >= kMinPathCompleteForPitStop) &&
                        (perc <= kMaxPathCompleteForPitStop);
  }
  _iConfig.examineBehavior->SetCanSeeAndTurnToSideObstacles(
      cutDistanceByHalf && !_dVars.hasTakenPitStop);
  ...
  if (!_iConfig.examineBehavior->IsActivated() &&
    ...
    if (cutDistanceByHalf) {
      ...
      // don't clear cached poses. by keeping them if the robot hasnt reached
      // its goal yet, it means it will resume to drive to the original goal
      // pose.
    } else {
      // clear cached poses
      _dVars.sampledPoses.clear();
    }
    // todo: perhaps this should only start if we've driven far enough from the
    // last spot we examined? this would help with noisy prox data
    DelegateNow(_iConfig.examineBehavior.get(),
                &BehaviorExploring::RegainedControl);
  }
}
...
void BehaviorExploring::SetGentleInterruptionOKForNow() {
  ...
  // add one to let it work next tick too (in most cases, it won't be checked
  // until next tick anyway because our parent already ran)
...
const ObservableObject* BehaviorExploring::GetCharger() const {
  ...
  GetBEI().GetBlockWorld().FindLocatedMatchingObjects(chargerFilter,
                                                      locatedChargers);
...
bool BehaviorExploring::IsCubeNearCharger() const {
  ...
  if (charger != nullptr) {
    const ObservableObject* cube =
        GetBEI().GetBlockWorld().FindLocatedObjectClosestTo(charger->GetPose(),
                                                            cubeFilter);
...
void BehaviorExploring::SampleAndDrive() {
  // if we don't have a face (can't run the referencing behavior) then we should
  // occasionally search for faces. Check that here
  if (!_iConfig.referenceHumanBehavior->WantsToBeActivated()) {
    const float currTime_s =
        BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
    if (currTime_s - _dVars.lastSearchForFaceTime_s >=
        kPeriodToCheckForFaces_s) {
  ...
  const auto* charger = GetCharger();
  if (charger != nullptr) {
    // See if we've traveled far enough away from the last pose at which we
    // looked at the charger
    const auto& robotPose = GetBEI().GetRobotInfo().GetPose();
    f32 distance = 0.f;
    const bool distanceValid = ComputeDistanceBetween(
        _dVars.lastReferenceChargerPose, robotPose, distance);
    const bool shouldReferenceCharger =
        !distanceValid || (distance > kReferenceChargerDistanceThreshold_mm);
  ...
  if (_dVars.sampledPoses.empty()) {
    // flag to CancelSelf, making sure CancelSelf doesn't happen on the same
    // tick as activation
    SET_STATE(Complete);
...
void BehaviorExploring::TransitionToDriving() {
  ...
  if (kMoveLiftAboveProx) {
    action->AddAction(new MoveLiftToHeightAction(
        MoveLiftToHeightAction::Preset::JUST_ABOVE_PROX));
  }
  ...
  DelegateIfInControl(action, [this](ActionResult res) {
    if (res == ActionResult::CANCELLED_WHILE_RUNNING) {
      // something interrupted this action (ReactToCliff/OnBack/etc.). don't
      // delegate again, since hopefully that means something higher up is
      // interrupting. This case is here to prevent the sampling computation if
      // this behavior is just going to get interrupted. Just to be sure,
      _dVars.devWarnIfNotInterruptedByTick =
          2 + BaseStationTimer::getInstance()->GetTickCount();
    } else if (res != ActionResult::SUCCESS) {
      // this can happen if we cleared all but one of the goal poses, then the
      // robot stopped to examine something midway, then when trying to start
      // again, there is no path to the selected goal. try a couple more times
      // (maybe this needs more precise ActionResult types?)
      if (_dVars.numDriveAttempts <= 5) {
        if ((res == ActionResult::PATH_PLANNING_FAILED_ABORT) ||
            (res == ActionResult::FAILED_TRAVERSING_PATH)) {
          // it's possible noise from the prox sensor is causing a legitimate
          // planner failure (timeout), so do a quick point turn to hopefully
          // find an escape before continuing
          const float angle =
              (GetRNG().RandDbl() > 0.5f) ? M_PI_2_F : -M_PI_2_F;
          const bool isAbsolute = false;
          auto* action = new TurnInPlaceAction{angle, isAbsolute};
          DelegateIfInControl(action,
                              [this](ActionResult res) { SampleAndDrive(); });
        } else {
          SampleAndDrive();
        }
      } else {
  ...
  auto callback = [this]() {
    ...
    if (animFirst) {
      action->AddAction(new TriggerLiftSafeAnimationAction(
          AnimationTrigger::ExploringLookAround));
    }

    const float angleChange =
        GetRNG().RandDblInRange(kMinScanAngle, kMaxScanAngle);
...
std::vector<Pose3d> BehaviorExploring::SampleVisitLocations() const {
  ...
  DEV_ASSERT(nullptr != memoryMap,
             "BehaviorExploring.SampleVisitLocations.NeedMemoryMap");
  ...
  // all the logic below works just fine if the charger position is centered
  // directly on the robot. we use that if the charger positions is not known,
  // which can happen if this behavior is started with a voice command
  const bool chargerEqualsRobot =
      (charger == nullptr && _iConfig.allowNoCharger);
  const auto& chargerPose =
      chargerEqualsRobot ? currRobotPose : charger->GetPose();

  // check how far the robot is from the charger.. if it's far enough, only
  // sample points to get it closer
  Point2f chargerPos{chargerPose.GetTranslation()};
  Point2f currRobotPos{currRobotPose.GetTranslation()};
  const float distFromCharger_mm =
      ComputeDistanceBetween(chargerPos, currRobotPos);

  // don't attempt rejection sampling if the area of intersection of the two
  // circles is small todo: this could be more smartly derived from the eq for
  // area of intersection of two circles, and then pick a minimum area. instead,
  // here's a magic number
  const bool tooFarFromCharger =
      distFromCharger_mm >= 0.75f * M_TO_MM(_iConfig.maxSearchRadius_m +
                                            _iConfig.maxChargerDistance_m);

  if ((kNumPositionsForSearch > 0) || tooFarFromCharger) {
    SampleVisitLocationsOpenSpace(memoryMap, tooFarFromCharger,
                                  chargerEqualsRobot, chargerPose, currRobotPos,
                                  retPoses);
  }

  // also sample poses pointing at known but unexplored prox obstacle
  if ((kNumProxPoses > 0) && !tooFarFromCharger) {
    SampleVisitLocationsFacingObstacle(memoryMap, charger, currRobotPos,
                                       retPoses);
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorExploring::SampleVisitLocationsOpenSpace(
    std::shared_ptr<const INavMap> memoryMap, bool tooFarFromCharger,
    bool chargerEqualsRobot, const Pose3d& chargerPose, const Point2f& robotPos,
  ...
  // If the charger pose is actually just the robot pose, then we don't want
  // this check. So set the acceptance probability to 1.
  if (chargerEqualsRobot) {
    _iConfig.condHandleChargerOutOfView->SetAcceptanceProbability(1.f,
                                                                  GetRNG());
  } else {
  ...
  for (unsigned int cnt = 0; cnt < kNumSampleSteps; ++cnt) {
    ...
    {
      const Point2f pt =
          RobotPointSamplerHelper::SamplePointInAnnulus(GetRNG(), r1, r2);
      sampledPos.x() += pt.x();
      sampledPos.y() += pt.y();
    }

    const bool acceptedPoint =
        _iConfig.openSpacePointEvaluator->Evaluate(GetRNG(), sampledPos);
    ...
    const bool acceptedFootprint =
        _iConfig.openSpacePolyEvaluator->Evaluate(GetRNG(), footprint);
    ...
    // if desired, add additional poses with the same position but different
    // orientations. this should be pretty cheap compared to a pose at a
    // different location
    for (size_t idxAddtl = 0; idxAddtl < kNumAnglesAtPose; ++idxAddtl) {
      const float otherAngle =
          M_TWO_PI * static_cast<float>(GetRNG().RandDbl());
    ...
    if (numAcceptedPoses >= kNumPositionsForSearch) {
      LOG_INFO("BehaviorExploring.SampleVisitLocationsOpenSpace.Completed",
               "Met required sampling of %d points, cnt=%d", numAcceptedPoses,
               cnt);
      break;
    }
  }
  ...
  if (numAcceptedPoses < kNumPositionsForSearch) {
    PRINT_NAMED_WARNING(
        "BehaviorExploring.SampleVisitLocationOpenSpace.Unfinished",
        "Sampled only %d accepted positions(%zu poses) out of expected %d with "
        "%d attempts",
        numAcceptedPoses, retPoses.size(), kNumPositionsForSearch,
        kNumSampleSteps);
  }
}

void BehaviorExploring::SampleVisitLocationsFacingObstacle(
  ...
  MemoryMapTypes::NodePredicate findFunc =
      [](MemoryMapTypes::MemoryMapDataConstPtr d) {
        if (d->type == MemoryMapTypes::EContentType::ObstacleProx) {
          auto castPtr =
              MemoryMapData::MemoryMapDataCast<MemoryMapData_ProxObstacle>(d);
  ...
  if (charger != nullptr) {
    ...
    const float chargerScaleFactor =
        M_TO_MM(_iConfig.maxChargerDistance_m) / kChargerRadius_mm;
    ...
    memoryMap->FindContentIf(findFunc, unexploredProxObstacles,
                             FastPolygon(chargerPoly));
  } else {
    memoryMap->FindContentIf(findFunc, unexploredProxObstacles);
  }

  // returns a pose that is kProxPoseOffset_mm away from ptr and antiparallel to
  // its normal
  auto getOffsetPose = [this](const MemoryMapDataConstPtr& ptr) {
    auto castPtr =
        MemoryMapData::MemoryMapDataCast<const MemoryMapData_ProxObstacle>(ptr);
    Pose3d pose{castPtr->GetObservationPose().GetAngle(),
                Z_AXIS_3D(),
                {castPtr->GetObservationPose().GetTranslation().x(),
                 castPtr->GetObservationPose().GetTranslation().y(), 0.0f},
                GetBEI().GetRobotInfo().GetWorldOrigin()};
    pose.TranslateForward(-kProxPoseOffset_mm);
    return pose;
  };

  const auto currListBegin =
      retPoses
          .begin();  // to mark the position of the beginning before adding more
  // not too close to existing sampled poses, and close to the robot
  auto isPositionedWell =
      [&currListBegin, &robotPos](
          const MemoryMapDataWrapper<const MemoryMapData_ProxObstacle>& data,
        ...
        for (auto itOthers = currListBegin; itOthers != existing.end();
             ++itOthers) {
          Point2f otherPoint{itOthers->GetTranslation()};
          const float distSq = (otherPoint - dataPoint).LengthSq();
          if (distSq <
              kMinProxObstacleSeparation_mm * kMinProxObstacleSeparation_mm) {
            return false;
          }
        }
        return true;
  ...
  if (unexploredProxObstacles.size() <= kNumProxPoses) {
    ...
    for (auto it = unexploredProxObstacles.begin();
         it != unexploredProxObstacles.end(); ++it) {
      bool add = true;
    ...
    // randomly sample, but do it sample by sample so that we can reject two
    // samples that are close together. unfortunately the starting container is
    // an unordered_set, which makes sampling less efficient, so first transfer
    // all the ptrs to a random access list
    std::vector<MemoryMapDataConstPtr> obstacleVec(
        unexploredProxObstacles.begin(), unexploredProxObstacles.end());
    std::vector<size_t> acceptedList;

    int obstacleCount = static_cast<int>(obstacleVec.size());
    Anki::Util::RandomIndexSampler sampler(
...
void BehaviorExploring::PrepRobotForProx() {
  const auto& proxSensor = GetBEI()
                               .GetComponentWrapper(BEIComponentID::ProxSensor)
                               .GetComponent<ProxSensorComponent>();
  const auto& proxData = proxSensor.GetLatestProxData();
  if (!proxData.foundObject) {
    if (!IsControlDelegated() && proxData.isLiftInFOV) {
      auto preset = kMoveLiftAboveProx
                        ? MoveLiftToHeightAction::Preset::JUST_ABOVE_PROX
