...
// For bumping an object. The robot is usually around 5-8cm from the object at
// this point, but may not be facing it perfectly, so only bump if the object
// seems to have an appropriate width. The delegated behavior decides if it is
// close enough
CONSOLE_VAR_RANGED(float, kMinObjectWidthToBump_rad, "BehaviorExploring",
                   DEG_TO_RAD(10.0f), 0.0f, M_PI_F);
CONSOLE_VAR_RANGED(float, kMaxObjectWidthToBump_rad, "BehaviorExploring",
                   DEG_TO_RAD(80.0f), 0.0f, M_TWO_PI_F);
const float kProbBumpNominalObject = 0.8f;

// BN: disabled because it looks much nicer when he bumps right after the scan,
// so instead I've got it set to reference _after_ the bump from within the bump
// behavior
CONSOLE_VAR_RANGED(float, kProbReferenceBeforeBump, "BehaviorExploring", 0.0f,
                   0.0f, 1.0f);
...
// if discovered an obstacle within this long, and an unrelated path brings you
// nearby turn toward it
...
bool BehaviorExploringExamineObstacle::WantsToBeActivatedBehavior() const {
  ...
  return (_dVars.persistent.seesFrontObstacle ||
          _dVars.persistent.seesSideObstacle) &&
         hasMap;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorExploringExamineObstacle::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorExploringExamineObstacle::GetAllDelegates(
...
void BehaviorExploringExamineObstacle::OnBehaviorActivated() {
  ...
  _dVars.initialPoseAngle_rad =
      GetBEI().GetRobotInfo().GetPose().GetRotationAngle<'Z'>().ToFloat();

  if (_dVars.persistent.seesSideObstacle) {
    DEV_ASSERT(!_dVars.persistent.seesFrontObstacle,
               "Should not be facing an obstacle now");
    ...
    auto* huhAction =
        new TriggerLiftSafeAnimationAction{AnimationTrigger::ExploringHuhFar};
    auto* turnAndHuh = new CompoundActionSequential({turnAction, huhAction});
    DelegateIfInControl(turnAndHuh,
                        [this](ActionResult res) { TransitionToNextAction(); });

  } else {
    if (!_dVars.persistent.seesFrontObstacle) {
      // can happen in tests
      PRINT_NAMED_WARNING(
          "BehaviorExploringExamineObstacle.OnBehaviorActivated.NoObst",
          "Should be facing an obstacle now");
    }

    const auto& proxSensor =
        GetBEI()
    ...
    // ignore if we actually found an object. this is only used to select an
    // animation so isnt vital
    const bool obstacleIsFar =
        (proxData.distance_mm >= kMinDistForFarReaction_mm);
    const AnimationTrigger huhAnim = obstacleIsFar
                                         ? AnimationTrigger::ExploringHuhFar
                                         : AnimationTrigger::ExploringHuhClose;
    auto* huhAction = new TriggerLiftSafeAnimationAction{huhAnim};
    DelegateIfInControl(huhAction,
                        [this](ActionResult res) { TransitionToNextAction(); });
  }
...
void BehaviorExploringExamineObstacle::OnBehaviorDeactivated() {
  if (_dVars.playingScanSound) {
    ...
    const auto event = AudioMetaData::GameEvent::GenericEvent::
        Stop__Robot_Vic_Sfx__Planning_Loop_Stop;
    GetBEI().GetRobotAudioClient().PostEvent(
        event, AudioMetaData::GameObjectType::Behavior);
  }
}
...
void BehaviorExploringExamineObstacle::TransitionToNextAction() {
  ...
  const auto* featureGate =
      GetBEI().GetRobotInfo().GetContext()->GetFeatureGate();
  const bool handDetectionEnabled =
      featureGate->IsFeatureEnabled(FeatureType::HandDetection);

  if (handDetectionEnabled && (_dVars.state == State::Initial)) {
    // Decide if this is a hand by running a single image through the hand
    // detection vision mode.
    // TODO: Add some kind of parallel face/sound animation while we wait for
    // hand result to come back?
    SET_STATE(CheckForHand);

    _dVars.lastImageTime =
        GetBEI().GetVisionComponent().GetLastProcessedImageTimeStamp();
    WaitForImagesAction* action =
        new WaitForImagesAction(1, VisionMode::Hands, _dVars.lastImageTime);
    DelegateNow(action, [this]() {
      ...
      salientPtsComponent.GetSalientPointSinceTime(
          handsFound, Vision::SalientPointType::Hand, _dVars.lastImageTime);
  ...
  if ((handDetectionEnabled && (_dVars.state == State::CheckForHand)) ||
    ...
    if (RobotPathFreeOfObstacle(kDistanceForStartApproach_mm, useRobotWidth)) {
      ...
      DriveStraightAction* action =
          new DriveStraightAction(kDistanceForStartApproach_mm);
      DelegateNow(action, [this](ActionResult res) {
        // if we got here, it wasn't canceled because of the absence of an
        // obstacle.
        TransitionToNextAction();
      });
      return;
  ...
  // if we're here, we either skipped or have finished the initial approach
  // maneuver
  if ((_dVars.state == State::QuickAnim) ||
      (_dVars.state == State::ReactToHand)) {
    CancelSelf();
    return;
  }
  ...
  // - in Initial state if hand detection was not enabled and the path was not
  // free of an obstacle
  // - in CheckForHand state because hand detection was enabled, completed, but
  // path was not free of an obstacle
  // - in DriveToObstacle state because we drove to the obstacle whether or not
  // hand detection ran
  const bool readyToReact =
      (_dVars.state == State::Initial || _dVars.state == State::CheckForHand ||
       _dVars.state == State::DriveToObstacle);

  if (readyToReact) {
    // Either react to a hand if we saw one, or do a generic scan-and-bump of an
    // object
    if (_dVars.handSeen) {
      SET_STATE(ReactToHand);
      if (_iConfig.handReactionBehavior->WantsToBeActivated()) {
        DelegateNow(_iConfig.handReactionBehavior.get(),
                    &BehaviorExploringExamineObstacle::TransitionToNextAction);
      }
      return;

    } else {
      // No hand (or handDetection disabled): Scan and bump whatever is in front
      // of us
      if (GetRNG().RandDbl() < kProbScan) {
        SET_STATE(FirstTurn);

        AnimationTrigger turnAnim =
            _dVars.firstTurnDirectionIsLeft
        ...
        // we manually trigger the audio since the looping scan animation
        // doesn't always precede a followup animation that could be used to
        // stop the audio
        ...
        action->AddAction(new TriggerLiftSafeAnimationAction{
            AnimationTrigger::ExploringQuickScan});
      }
    }
    ...
    // todo: clean this up. too many cases here, not enough time to separate
    // them logic is: after the first turn it should return to center. after the
    // second turn it should either exit, or return to center and bump the
    // object, or reference a human then return to center then bump the object
    ...
    if (_dVars.state == State::SecondTurn) {
      ...
      if (!shouldBump) {
        // object is too thin or wide, exit the behavior instead of
        // re-centering. If it re-centered, it would immediately have to turn
        // again to start driving, so it looks better to leave from this pose
        CancelSelf();
        return;
      }

      const Radians dAngle =
          GetBEI().GetRobotInfo().GetPose().GetRotationAngle<'Z'>() -
          _dVars.initialPoseAngle_rad;
      _dVars.totalObjectAngle_rad += fabsf(dAngle.ToFloat());

      const bool refBehaviorWantsToBeActivated =
          _iConfig.referenceHumanBehavior->WantsToBeActivated();
      if (refBehaviorWantsToBeActivated &&
          (GetRNG().RandDbl() < kProbReferenceBeforeBump)) {
    ...
    if (_dVars.state != State::ReferenceHuman) {
      ...
      if (_dVars.firstTurnDirectionIsLeft) {
        turnAnim = (_dVars.state == State::ReturnToCenter)
                       ? AnimationTrigger::ExploringScanCenterFromLeft
                       : AnimationTrigger::ExploringScanCenterFromRight;
      } else {
        turnAnim = (_dVars.state == State::ReturnToCenter)
                       ? AnimationTrigger::ExploringScanCenterFromRight
                       : AnimationTrigger::ExploringScanCenterFromLeft;
      ...
      // keep a weakptr to the turn action, so that the whole parallel action
      // can be aborted the moment this one ends
    ...
    if (_iConfig.bumpBehavior->WantsToBeActivated()) {
      DelegateIfInControl(_iConfig.bumpBehavior.get(),
                          [this]() { CancelSelf(); });
      return;
    } else {
      CancelSelf();
...
void BehaviorExploringExamineObstacle::BehaviorUpdate() {
  if (!IsActivated()) {
    ...
    // while this one could be called from elsewhere, currently we only care
    // about the result when this behavior is in scope, so it's called here
    GetBEI().GetMapComponent().FlagProxObstaclesTouchingExplored();

    const static bool forActivation = true;
    _dVars.persistent.seesFrontObstacle =
        RobotSeesObstacleInFront(kDistanceForActivation_mm, forActivation);
    // don't try evaluating the cone if there's already one in front
    if (!_dVars.persistent.seesFrontObstacle &&
        _dVars.persistent.canSeeSideObstacle) {
      _dVars.persistent.seesSideObstacle = RobotSeesNewObstacleInCone(
          kDistanceForConeActivation_mm, kHalfAngleForConeActivation_rad,
    ...
    if (GetBEI().HasMapComponent()) {
      // mark prox obstscles that are within a short beam from the robot as
      // explored. this is done before the flood fill step ( see above comment )
      // so that obstacles flagged as explored this tick count as seens for
      // filling
  ...
  if ((_dVars.state == State::FirstTurn) ||
      (_dVars.state == State::SecondTurn)) {
    // while in the middle of a turn, if no prox obstacle is seen, we can stop
    // turning
    const static bool forActivation = false;
    const bool seenObstacle =
        RobotSeesObstacleInFront(kDistanceForStopTurn_mm, forActivation);
    if (!seenObstacle) {
    ...
    if (!_dVars.scanCenterAction.lock() && IsControlDelegated()) {
      // finished returning to center but the parallel action hasnt finished.
      // cancel it and begin the next action
      CancelDelegates(false);
      TransitionToNextAction();
    }
  }
  const bool useRobotWidth = false;  // only care how close we are, not if
                                     // driving would hit an obstacle
...
bool BehaviorExploringExamineObstacle::RobotPathFreeOfObstacle(
    float dist_mm, bool useRobotWidth) const {
  ...
  DEV_ASSERT(
      nullptr != memoryMap,
      "BehaviorExploringExamineObstacle.RobotPathFreeOfObstacle.NeedMemoryMap");
  ...
  if (!useRobotWidth) {
    ...
    const Point2f p2 = (currRobotPose.GetTransform() * Transform3d(rot, offset))
                           .GetTranslation();
    hasCollision = GetBEI().GetMapComponent().CheckForCollisions(
        FastPolygon{{Point2f{currRobotPose.GetTranslation()}, p2}});
  } else {
    ...
    const Point2f p1 =
        (currRobotPose.GetTransform() * Transform3d(rot, offset1))
            .GetTranslation();
    const Point2f p2 =
        (currRobotPose.GetTransform() * Transform3d(rot, offset2))
            .GetTranslation();
    const Point2f p3 =
        (currRobotPose.GetTransform() * Transform3d(rot, offset2 + offset3))
            .GetTranslation();
    const Point2f p4 =
        (currRobotPose.GetTransform() * Transform3d(rot, offset1 + offset3))
            .GetTranslation();

    hasCollision = GetBEI().GetMapComponent().CheckForCollisions(
        FastPolygon{{p1, p2, p3, p4}});
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorExploringExamineObstacle::RobotSeesObstacleInFront(
    float dist_mm, bool forActivation) const {
  const auto& robotInfo = GetBEI().GetRobotInfo();

  const auto memoryMap = GetBEI().GetMapComponent().GetCurrentMemoryMap();
  DEV_ASSERT(nullptr != memoryMap,
             "BehaviorExploringExamineObstacle.WTBAB.NeedMemoryMap");
  ...
  // this behavior activates if a new unexplored ObstacleProx is seen, and ends
  // when no explored or unexplored ObstacleProx is seen.
  ...
  const bool foundNewObstacle = memoryMap->AnyOf(
      FastPolygon{{Point2f{currRobotPose.GetTranslation()}, p2}}, evalFunc);
  return foundNewObstacle;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorExploringExamineObstacle::RobotSeesNewObstacleInCone(
    float dist_mm, float halfAngle_rad, Point2f& position) const {
  const auto& robotInfo = GetBEI().GetRobotInfo();
  ...
  const Point2f t1 = (poseOffset.GetTransform() * Transform3d(rot, rayOffset1))
                         .GetTranslation();
  const Point2f t2 = (poseOffset.GetTransform() * Transform3d(rot, rayOffset2))
                         .GetTranslation();

  const Point2f t3 = currRobotPose.GetTranslation();

  // only react to unexplored prox obstacles
  const EContentTypePackedType nodeTypeFlags =
      EContentTypeToFlag(MemoryMapTypes::EContentType::ObstacleProx);
  // and only if theyre recent
  RobotTimeStamp_t earliestTime =
      GetBEI().GetRobotInfo().GetLastMsgTimestamp() -
      kTimeForTurnToPassingObstacles_ms;
  ...
  const bool foundNewObstacle =
      memoryMap->AnyOf(FastPolygon{{t1, t2, t3}}, evalFunc);
  return foundNewObstacle;
}
...
void BehaviorExploringExamineObstacle::DevTakePhoto() const {
  // shutter sound
  {
    ...
    using GO = AudioMetaData::GameObjectType;
    GetBEI().GetRobotAudioClient().PostEvent(
        GE::Play__Robot_Vic_Sfx__Camera_Flash, GO::Behavior);
  }

  // save
  const std::string path = Util::FileUtils::FullFilePath(
      {GetBEI().GetRobotInfo().GetContext()->GetDataPlatform()->GetCachePath(
           "camera"),
       "images", "exploringObstacles"});
  auto& visionComponent = GetBEI()
                              .GetComponentWrapper(BEIComponentID::Vision)
