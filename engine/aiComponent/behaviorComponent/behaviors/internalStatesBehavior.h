...
class InternalStatesBehavior : public ICozmoBehavior {
 protected:
  ...
  // subclasses of InternalStatesBehavior can pass handles to custom BEI
  // conditions that can be used in various BEIConditions in this class
 ...
 protected:
  ...
  // for debug/DAS
  virtual void OnStateNameChange(const std::string& oldStateName,
                                 const std::string& newStateName) const {}
  ...
  enum class StateCooldownDefault {
    ...
    UseBehaviorActivationTime,       // if the state has never run, use this
                                     // behaviors start time instead
    UseFirstBehaviorActivationTime,  // use the first time this behavior ever
                                     // activated instead
  };

  bool StateExitCooldownExpired(
      StateID state, float timeout,
      StateCooldownDefault neverRunDefault = StateCooldownDefault::True) const;

  // Set up a console var under uniqueVarName that transitions to each state
  // type by name
  void AddConsoleVarTransitions(const char* uniqueVarName,
                                const char* category);
 ...
 private:
  ...
  enum class TransitionType {
    // transitions that can happen when the currently delegated-to behavior
    // thinks it's a decent time for a
    // gentle interruption (or if there is no currently delegated behavior)
    NonInterrupting,

    // transitions that can happen while the state is active (and in the middle
    // of doing something)
    Interrupting,

    // exit transitions only run if the currently-delegated-to behavior stop
    // itself. Note that these are
  ...
  // Checks if the robot is carrying an object, then if the behavior for the
  // state WantsToBeActivatedWhenCarryingObject, if the cube needs to be put
  // down, delegates to PutDownBlock behavior and returns true. The completion
  // callback will resolve the state and delegate to the GetIn or state behavior
  // as necessary. Cannot be interrupted.
  bool PutDownBlockIfNecessary(State& state);

  // returns true if delegating to a valid GetIn behavior as defined for the
  // state. The completion callback will resolve the state and delegate to the
  // state behavior as necessary.
  bool RunStateGetInIfAble(State& state);

  void RunMainStateBehavior(State& state);

  // return the "adjusted" time of the current state (time it's been active -
  // time it's been paused because this behavior was interrupted)
  float GetCurrentStateActiveTime_s() const;

  CustomBEIConditionHandleList CreateCustomTimerConditions(
      const Json::Value& config);
 ...
 public:
  ...
  TESTONLY_GetAllTransitions(UnitTestKey key) const;
