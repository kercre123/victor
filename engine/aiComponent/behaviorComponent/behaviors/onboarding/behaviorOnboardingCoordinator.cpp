...
const std::string BehaviorOnboardingCoordinator::kOnboardingFolder =
    "onboarding";
const std::string BehaviorOnboardingCoordinator::kOnboardingFilename =
    "onboardingState.json";
const std::string BehaviorOnboardingCoordinator::kOnboardingStageKey =
    "onboardingStage";
const std::string BehaviorOnboardingCoordinator::kOnboardingTimeKey =
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorOnboardingCoordinator::InstanceConfig::InstanceConfig(
    const Json::Value& config) {
  const Json::Value& phases = config[kPhasesKey];
  for (const Json::Value& phase : phases) {
    std::string phaseName =
        JsonTools::ParseString(phase, kPhaseNameKey, kDebugLabel);
    OnboardingPhase onboardingPhase;
    if (EnumFromString(phaseName, onboardingPhase)) {
      OnboardingPhaseData phaseData;
      phaseData.behaviorIDStr =
          JsonTools::ParseString(phase, kPhaseBehaviorIDKey, kDebugLabel);
      phaseData.timeout_s =
          JsonTools::ParseFloat(phase, kPhaseTimeoutKey, kDebugLabel);
      phaseData.allowPowerOff = false;
      JsonTools::GetValueOptional(phase, kAllowPowerOffKey,
                                  phaseData.allowPowerOff);

      OnboardingPhaseMap.insert({onboardingPhase, phaseData});
    } else {
      LOG_ERROR(
          "BehaviorOnboardingCoordinator.InstanceConfigCtor."
          "InvalidOnboardingPhase",
  ...
  completionTimeout_s =
      JsonTools::ParseFloat(config, kFinishTimeoutKey, kDebugLabel);
  appDisconnectTimeout_s =
      JsonTools::ParseFloat(config, kAppDisconnectKey, kDebugLabel);
  idleTimeout_s = JsonTools::ParseFloat(config, kIdleTimeoutKey, kDebugLabel);
  allowPowerOffFromIdle =
      JsonTools::ParseBool(config, kPowerOffFromIdleKey, kDebugLabel);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorOnboardingCoordinator::BehaviorOnboardingCoordinator(
    const Json::Value& config)
    : ICozmoBehavior(config), _iConfig(config) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingCoordinator::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kPhasesKey,          kPhaseNameKey,
                        kPhaseBehaviorIDKey, kPhaseTimeoutKey,
                        kAllowPowerOffKey,   kStartTimeoutKey,
                        kFinishTimeoutKey,   kAppDisconnectKey,
                        kIdleTimeoutKey,     kPowerOffFromIdleKey};
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingCoordinator::GetBehaviorOperationModifiers(
    BehaviorOperationModifiers& modifiers) const {
  modifiers.wantsToBeActivatedWhenOffTreads = true;
  modifiers.wantsToBeActivatedWhenOnCharger = true;
  modifiers.behaviorAlwaysDelegates = false;
  modifiers.visionModesForActiveScope->insert(
      {VisionMode::Faces, EVisionUpdateFrequency::Med});
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingCoordinator::GetAllDelegates(
...
void BehaviorOnboardingCoordinator::InitBehavior() {
  ...
  GetBehaviorComp<OnboardingMessageHandler>().SubscribeToAppTags(
      ...
      std::bind(&BehaviorOnboardingCoordinator::HandleOnboardingMessageFromApp,
                this, std::placeholders::_1));

  const auto& BC = GetBEI().GetBehaviorContainer();
  _iConfig.behaviorPowerOff =
      BC.FindBehaviorByID(BEHAVIOR_ID(OnboardingPowerOff));

  for (auto& pair : _iConfig.OnboardingPhaseMap) {
    pair.second.behavior = BC.FindBehaviorByID(
        BehaviorTypesWrapper::BehaviorIDFromString(pair.second.behaviorIDStr));
...

  {
    ...
    const auto* platform =
        GetBEI().GetRobotInfo().GetContext()->GetDataPlatform();
    _iConfig.saveFolder =
        platform->pathToResource(Data::Scope::Persistent, kOnboardingFolder);
    _iConfig.saveFolder =
        FileUtils::AddTrailingFileSeparator(_iConfig.saveFolder);
  ...
  if (ANKI_DEV_CHEATS) {
    auto clearOnboardingDataFunc = [this](ConsoleFunctionContextRef context) {
      ...
      if (GetBEI().GetRobotInfo().HasExternalInterface()) {
        ...
        ei->Broadcast(ExternalInterface::MessageGameToEngine{
            ExternalInterface::EraseAllEnrolledFaces{}});
      }
    };
    ...
    for (auto& phaseDataPair : _iConfig.OnboardingPhaseMap) {
      ...
      _iConfig.consoleFuncs.emplace_front(funcNameStr, std::move(setPhaseFunc),
                                          "OnboardingCoordinator", "");
    }
  }  // Dev console utilities
}
...
void BehaviorOnboardingCoordinator::OnBehaviorActivated() {
  // If the behavior exited naturally, reset dynamic variables. Otherwise,
  // assume we were interrupted by something and pick up where we left off to
  // maintain App Authoritative design if at all possible
  if (_dVars.terminatedNaturally) {
    _dVars = DynamicVariables();
    // If this isn't set to true before exiting, onboarding was interrupted,
    // probably by an infoFace feature like mute or customer care
    _dVars.terminatedNaturally = false;

    float currTime_s =
        BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
    _dVars.globalExitTime_s = (_iConfig.startTimeout_s > 0.0f)
                                  ? (currTime_s + _iConfig.startTimeout_s)
                                  : 0.0f;
    ...
    if (OnboardingPhase::InvalidPhase != _dVars.currentPhase) {
      ...
      TransitionToPhase(_dVars.currentPhase, appCommandedTransition,
                        resumingPhaseAfterInterruption);
    }
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingCoordinator::HandleOnboardingMessageFromApp(
    const AppToEngineEvent& event) {
  switch (event.GetData().GetTag()) {
    case AppToEngineTag::kOnboardingRestart: {
      RestartOnboarding();
      break;
    }
    // kOnboardingWakeUpRequest and kOnboardingWakeUpStartedRequest are
    // maintained for backward compatiblity with older app versions which use
    // "hail mary" onboarding sequence
    case AppToEngineTag::kOnboardingWakeUpRequest: {
      _dVars.emulate1p0Onboarding = true;
      const float chargerTime_s = GetBEI()
                                      .GetRobotInfo()
                                      .GetBatteryComponent()
    ...
    case AppToEngineTag::kOnboardingWakeUpStartedRequest: {
      ...
      auto* gi = GetBEI().GetRobotInfo().GetGatewayInterface();
      if (nullptr != gi) {
        // tell app wakeup started if we already received the wakeup command
        // from the app
        auto* msg = new external_interface::OnboardingWakeUpStartedResponse{
            _dVars.started1p0WakeUp};
        gi->Broadcast(ExternalMessageRouter::WrapResponse(msg));
      }
      break;
    }
    case AppToEngineTag::kOnboardingSetPhaseRequest: {
      _dVars.pendingPhase = CladProtoTypeTranslator::ToCladEnum(
          event.GetData().onboarding_set_phase_request().phase());
      _dVars.lastSetPhase = _dVars.pendingPhase;
      auto iter = _iConfig.OnboardingPhaseMap.find(_dVars.lastSetPhase);
      _dVars.lastSetPhaseBehavior = (_iConfig.OnboardingPhaseMap.end() != iter)
    ...
    case AppToEngineTag::kAppDisconnected: {
      // If we were running 1p0 onboarding emulation and we've already started
      // waking up, don't react to the lost connection, just let the emulation
      // behavior finish up onboarding naturally on a 60 sec timeout or the
      // first voice command
      if (!_dVars.emulate1p0Onboarding || !_dVars.started1p0WakeUp) {
        // Don't timeout based on AppDisconnected messages if we haven't yet
        // started onboarding
        if (_dVars.onboardingStarted) {
          const float currTime_s =
              BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
          const bool appDisconnectTimeOutSet =
              _iConfig.appDisconnectTimeout_s > 0.0f;
          _dVars.appDisconnectExitTime_s =
              appDisconnectTimeOutSet
                  ? (currTime_s + _iConfig.appDisconnectTimeout_s)
                  : 0.0f;
        }
      }
      // This particular message should NOT reset appDisconnect params; return
      // early
      return;
    }
    default: {
      LOG_WARNING(
          "BehaviorOnboardingCoordinator.HandleOnboardingMessageFromApp."
          "UnhandledAppMessageReceived",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingCoordinator::OnBehaviorDeactivated() { UnFixStim(); }
...
void BehaviorOnboardingCoordinator::BehaviorUpdate() {
  ...
  if (_dVars.markCompleteAndExitOnNextUpdate) {
    ...
    // Skipping is treated just like it never started, and should overwrite
    // previous timeouts
    TerminateOnboarding();
  ...
  } else if (_dVars.appDisconnectExitTime_s != 0.0f &&
             !_dVars.waitingForAppReconnect) {
  ...
  UpdateBatteryInfo();
  // If the battery is low, tell the App, but don't do anything special other
  // than that
  if ((_dVars.batteryInfo.needsToCharge && !_dVars.batteryInfo.onCharger) &&
      ((_dVars.nextTimeSendBattInfo_s > 0.0f) &&
       (_dVars.nextTimeSendBattInfo_s < currTime_s))) {
    SendBatteryInfoToApp();
  }

  if (ShouldExitDueToTimeout()) {
    TerminateOnboarding();
    // For now we don't distinguish between OnboardingStages::Complete and
    // OnboardingStages::TimedOut for any functional purpose, so just mark
    // complete. In the future we could use TimedOut to deliberately offer users
  ...
  if (_dVars.shouldCheckPowerOff) {
    if (!_iConfig.behaviorPowerOff->IsActivated() &&
        _iConfig.behaviorPowerOff->WantsToBeActivated()) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingCoordinator::TransitionToPhase(
    const OnboardingPhase& phase, bool appCommandedTransition,
    bool resumingPhaseAfterInterruption) {
  // App Authoritative. Abandon anything we're currently doing without callbacks
  CancelDelegates(false);

  const auto currTime_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();

  if (!_dVars.onboardingStarted && appCommandedTransition) {
    _dVars.globalExitTime_s = (_iConfig.completionTimeout_s > 0.0f)
                                  ? (currTime_s + _iConfig.completionTimeout_s)
                                  : 0.0f;
  ...
  if (_iConfig.OnboardingPhaseMap.end() != iter) {
    ...
    if (behavior->WantsToBeActivated()) {
      if (resumingPhaseAfterInterruption) {
        IOnboardingPhaseWithProgress* phaseWithProgress =
            dynamic_cast<IOnboardingPhaseWithProgress*>(behavior.get());
        if (phaseWithProgress) {
          phaseWithProgress->ResumeUponNextActivation();
        }
      } else {
        // Only reset the phase timeout if this transition is NOT resuming after
        // an interruption
        const bool phaseHasTimeout = iter->second.timeout_s > 0.0f;
        _dVars.phaseExitTime_s =
            phaseHasTimeout ? (currTime_s + iter->second.timeout_s) : 0.0f;
      }
      ...
      DelegateIfInControl(behavior.get(),
                          [this, phase]() { OnPhaseComplete(phase); });
    } else {
      LOG_ERROR(
          "BehaviorOnboardingCoordinator.TransitionToPhase."
          "PhaseBehaviorFailure",
          "Phase %s behavior did not want to be activated. Setting Default "
          "phase if available",
          EnumToString(phase));
      if (OnboardingPhase::Default != phase) {
        TransitionToPhase(OnboardingPhase::Default);
      } else {
        // If we can't get to the default phase, exit onboarding and don't
        // re-enter it... its broken.
        TerminateOnboarding();
        SaveToDisk(OnboardingStages::Complete);
        return;
      }
    ...
    LOG_ERROR(
        "BehaviorOnboardingCoordinator.TransitionToPhase.InvalidPhase",
        "No behavior specified for phase %s in the onboarding coordinator.",
        EnumToString(phase));
    if (OnboardingPhase::Default != phase) {
      TransitionToPhase(OnboardingPhase::Default);
    } else {
      // If the "default" phase is invalid, exit onboarding and don't re-enter
      // it... its broken.
      TerminateOnboarding();
      SaveToDisk(OnboardingStages::Complete);
      return;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingCoordinator::OnPhaseComplete(
    const OnboardingPhase& phase) {
  LOG_INFO("BehaviorOnboardingCoordinator.OnPhaseComplete",
           "Completed onboarding phase %s. Idling.", EnumToString(phase));

  if (_dVars.emulate1p0Onboarding) {
    if (OnboardingPhase::WakeUp == phase) {
      auto* gi = GetBEI().GetRobotInfo().GetGatewayInterface();
      if (gi != nullptr) {
        auto* onboardingWakeUpFinished =
            new external_interface::OnboardingWakeUpFinished;
      ...
      // If we come back from the Emulation phase, always exit. We either got a
      // voice command or something broke
      TerminateOnboarding();
      SaveToDisk(OnboardingStages::Complete);
    }
    return;
  ...
  _dVars.phaseExitTime_s =
      hasIdleTimeout ? (currTime_s + _iConfig.idleTimeout_s) : 0.0f;
...
void BehaviorOnboardingCoordinator::UpdateBatteryInfo(bool sendEvent) {
  ...
  if ((needsToCharge != _dVars.batteryInfo.needsToCharge) ||
      (onCharger != _dVars.batteryInfo.onCharger)) {
    ...
    PRINT_CH_INFO(
        "Behaviors",
        "BehaviorOnboardingCoordinator.OnboardingStatus.UpdateBatteryInfo",
        "Charging status changed: onCharger=%d, needsToCharge=%d, "
        "chargerTime=%f",
        _dVars.batteryInfo.onCharger, _dVars.batteryInfo.needsToCharge,
        chargerTime_s);
...
void BehaviorOnboardingCoordinator::SendBatteryInfoToApp() {
  ...
  if (nullptr != gi) {
    ...
    const float currTime_s =
        BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
    _dVars.nextTimeSendBattInfo_s = (kBattInfoUpdateTime_s > 0.0f)
                                        ? (currTime_s + kBattInfoUpdateTime_s)
                                        : 0.0f;
  }
}
...
float BehaviorOnboardingCoordinator::GetChargerTime() const {
  const float chargerTime_s =
      GetBEI().GetRobotInfo().GetBatteryComponent().GetSuggestedChargerTime();
...
void BehaviorOnboardingCoordinator::SendSetPhaseResponseToApp() {
  ...
  if (nullptr != gi) {
    ...
    if (!IsActivated()) {
      // Indicate to the App that this message was handled atypically incase the
      // App wants to disregard results
      msg->set_allocated_status(new external_interface::ResponseStatus(
          external_interface::ResponseStatus::StatusCode::
              ResponseStatus_StatusCode_UNKNOWN));
    } else {
      msg->set_allocated_status(new external_interface::ResponseStatus(
          external_interface::ResponseStatus::StatusCode::
    ...
    msg->set_phase_state(
        CladProtoTypeTranslator::ToProtoEnum(_dVars.lastSetPhaseState));
    gi->Broadcast(ExternalMessageRouter::WrapResponse(msg));
  }
}
...
void BehaviorOnboardingCoordinator::Send1p0WakeUpResponseToApp(bool canWakeUp) {
  ...
  if (nullptr != gi) {
    ...
    LOG_INFO("BehaviorOnboarding.OnboardingStatus.Received1p0Wakeup",
             "Received wakeup. Responding with ok=%d, onCharger=%d, "
             "needsToCharger=%d, chargerTime=%f",
             canWakeUp, _dVars.batteryInfo.onCharger,
             _dVars.batteryInfo.needsToCharge, GetChargerTime());

    auto* onboardingWakeUpResponse =
        new external_interface::OnboardingWakeUpResponse;
    onboardingWakeUpResponse->set_waking_up(canWakeUp);
    onboardingWakeUpResponse->set_allocated_charging_info(chargingInfo);
    gi->Broadcast(
        ExternalMessageRouter::WrapResponse(onboardingWakeUpResponse));
  }
}
...
void BehaviorOnboardingCoordinator::SendPhaseProgressResponseToApp() {
  ...
  if (nullptr != gi) {
    ...
    msg->set_last_set_phase(
        CladProtoTypeTranslator::ToProtoEnum(_dVars.lastSetPhase));
    msg->set_last_set_phase_state(
        CladProtoTypeTranslator::ToProtoEnum(_dVars.lastSetPhaseState));

    IOnboardingPhaseWithProgress* phaseWithProgress =
        dynamic_cast<IOnboardingPhaseWithProgress*>(
            _dVars.lastSetPhaseBehavior.get());
    int progress = (nullptr != phaseWithProgress)
                       ? phaseWithProgress->GetPhaseProgressInPercent()
                       : 0.0f;
...
void BehaviorOnboardingCoordinator::RestartOnboarding() {
  ...
  // we shouldn't CancelSelf() because this is the base behavior in the stack.
  // But we can tell the BehaviorsBootLoader to reboot with a different stack
  if (GetBEI().GetRobotInfo().HasExternalInterface()) {
    auto* ei = GetBEI().GetRobotInfo().GetExternalInterface();
...
void BehaviorOnboardingCoordinator::SaveToDisk(OnboardingStages stage) const {
  ...
  if (stage == OnboardingStages::Complete) {
    ...
    toSave[kOnboardingTimeKey] =
        std::chrono::duration_cast<std::chrono::seconds>(tse)
            .count();  // write out as seconds since the epoch
  }
  const std::string filename = _iConfig.saveFolder + kOnboardingFilename;
...
void BehaviorOnboardingCoordinator::UnFixStim() {
  if (GetBEI().HasMoodManager() && _dVars.isStimMaxed) {
    auto& moodManager = GetBEI().GetMoodManager();
    // since this behavior is always at the base of the stack (todo: unit test
    // this), just set it fixed==false
...
bool BehaviorOnboardingCoordinator::ShouldExitDueToTimeout() {
  ...
  if ((_dVars.globalExitTime_s > 0.0f) &&
      (_dVars.globalExitTime_s < currTime_s)) {
    auto timeoutType =
        _dVars.onboardingStarted ? "DELAYED_COMPLETION" : "DELAYED_START";
    LOG_INFO("BehaviorOnboardingCoordinator.TimeoutExpired",
             "Onboarding aborted due to %s timeout.", timeoutType);
    reason = timeoutType;
    shouldTimeOut = true;
  } else if ((_dVars.phaseExitTime_s > 0.0f) &&
             (_dVars.phaseExitTime_s < currTime_s)) {
    std::string timeoutType(EnumToString(_dVars.currentPhase));
    std::transform(timeoutType.begin(), timeoutType.end(), timeoutType.begin(),
                   ::toupper);
    timeoutType += "_PHASE_TIMEOUT";
    LOG_INFO("BehaviorOnboardingCoordinator.TimeoutExpired",
             "Onboarding aborted due to expired %s.", timeoutType.c_str());
    reason = timeoutType;
    shouldTimeOut = true;
  } else if ((_dVars.appDisconnectExitTime_s > 0.0f) &&
             (_dVars.appDisconnectExitTime_s < currTime_s)) {
    LOG_INFO("BehaviorOnboardingCoordinator.TimeoutExpired",
             "Onboarding aborted due to expired AppDisconnected timeout while "
  ...
  if (shouldTimeOut) {
    DASMSG(onboarding_timeout, "onboarding.timeout",
           "Onboarding timed out and why");
