...
BehaviorOnboardingTeachWakeWord::InstanceConfig::InstanceConfig(
    ...
    : lookAtUserBehavior(nullptr),
      ...
      simulatedStreamingDuration_ms(
          -1)  // -1 will use the default setting from the anim process
      ,
      numWakeWordsToCelebrate(3) {
  JsonTools::GetValueOptional(config, kSimulatedStreamingDurationKey,
                              simulatedStreamingDuration_ms);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorOnboardingTeachWakeWord::DynamicVariables::PersistentVars::
    PersistentVars()
...
int BehaviorOnboardingTeachWakeWord::GetPhaseProgressInPercent() const {
  return (int)(100.0f * ((float)_dVars.persistent.numWakeWordDetections /
                         (float)_iConfig.numWakeWordsToCelebrate));
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingTeachWakeWord::GetBehaviorOperationModifiers(
    BehaviorOperationModifiers& modifiers) const {
  // Must allow periods of non-delegation to handle the
  // WaitingForWakeWordGetInToFinish state
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingTeachWakeWord::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingTeachWakeWord::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kSimulatedStreamingDurationKey};
...
void BehaviorOnboardingTeachWakeWord::TransitionToListenForWakeWord() {
  ...
  if (_iConfig.lookAtUserBehavior->WantsToBeActivated()) {
    ...
    DelegateIfInControl(
        _iConfig.lookAtUserBehavior.get(),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingTeachWakeWord::
    TransitionToWaitForWakeWordGetInToFinish() {
  CancelDelegates(false);
...
void BehaviorOnboardingTeachWakeWord::TransitionToReactToWakeWord() {
  ...
  if (++_dVars.persistent.numWakeWordDetections >=
      _iConfig.numWakeWordsToCelebrate) {
    TransitionToCelebrateSuccess();
  } else {
    DelegateIfInControl(
        new TriggerLiftSafeAnimationAction(_iConfig.listenGetOutAnimTrigger),
        &BehaviorOnboardingTeachWakeWord::TransitionToListenForWakeWord);
  }
}
...
void BehaviorOnboardingTeachWakeWord::TransitionToCelebrateSuccess() {
  ...
  DelegateIfInControl(
      new TriggerLiftSafeAnimationAction(_iConfig.celebrationAnimTrigger),
      [this]() { CancelSelf(); });
}
...
void BehaviorOnboardingTeachWakeWord::EnableWakeWordDetection() {
  ...
  auto earConBegin = AudioMetaData::GameEvent::GenericEventFromString(
  ...
  SmartPushResponseToTriggerWord(
      _iConfig.listenGetInAnimTrigger, postAudioEvent,
