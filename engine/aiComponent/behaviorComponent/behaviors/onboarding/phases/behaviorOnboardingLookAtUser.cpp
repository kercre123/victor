 ...
 * Created: 2018-11-05
 *
 * Description: Onboarding "Death Stare" implementation that handles On/Off
 *charger state. Behavior does not return on its own, and expects to be
 *interrupted by a timeout or new onboarding state from the App
 *
...
void BehaviorOnboardingLookAtUser::InitBehavior() {
  ...
  _iConfig.behaviorLookAtUser =
      BC.FindBehaviorByID(BEHAVIOR_ID(OnboardingLookAtUserOffCharger));
  _iConfig.behaviorLookAtUserOnCharger =
      BC.FindBehaviorByID(BEHAVIOR_ID(OnboardingLookAtUserOnCharger));
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingLookAtUser::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingLookAtUser::GetAllDelegates(
...
void BehaviorOnboardingLookAtUser::BehaviorUpdate() {
  ...
  const bool inOnChargerState =
      (_dVars.state == LookAtUserState::LookAtUserOnCharger);
  const bool onTreads =
      (robotInfo.GetOffTreadsState() == OffTreadsState::OnTreads);
  const bool inOffTreadsState =
      (_dVars.state == LookAtUserState::WaitForPutDown);
...
void BehaviorOnboardingLookAtUser::TransitionToLookAtUser() {
  ...
  if (_dVars.lastDelegate->WantsToBeActivated()) {
    ...
    DelegateIfInControl(_dVars.lastDelegate.get(), [this]() {
...
void BehaviorOnboardingLookAtUser::TransitionToWaitingForPutDown() {
  ...
  auto* action = new WaitForLambdaAction{waitForOnTreads,
                                         std::numeric_limits<float>::max()};
  DelegateIfInControl(action, [this]() {
    if (GetBEI().GetRobotInfo().GetOffTreadsState() !=
        OffTreadsState::OnTreads) {
      // still not on treads... was likely falling again, causing this action to
      // abort
      TransitionToWaitingForPutDown();
    } else {
      TransitionToLookAtUser();
