...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorOnboardingLookAtPhone::BehaviorOnboardingLookAtPhone(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingLookAtPhone::AlwaysHandleInScope(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingLookAtPhone::HandleWhileInScopeButNotActivated(
    const GameToEngineEvent& event) {
  HandleGameToEngineEvent(event);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingLookAtPhone::HandleGameToEngineEvent(
    const GameToEngineEvent& event) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorOnboardingLookAtPhone::GetBehaviorOperationModifiers(
...
void BehaviorOnboardingLookAtPhone::OnBehaviorActivated() {
  ...
  // since this behavior runs on and off charger without any user facing battery
  // alerts, make sure it's in low power mode to avoid overheating the battery.
  // We should be able to still receive app messages during this time to wake up
  // (which ends low power mode).
  SmartRequestPowerSaveMode();

  GetBehaviorComp<OnboardingMessageHandler>().RequestBleSessions();

  // if the app requests we restart onboarding in the middle of something else,
  ...
  DelegateIfInControl(moveLiftAction, [this, hasRun]() {
    if (hasRun) {
      // start with the loop action, which has a delayed head keyframe in the UP
      // position, instead of MoveHeadUp, which has an initial keyframe in the
      // DOWN position, to avoid a head snap
      if (_hasBleKeys) {
        RunLoopAction();
      }
    } else {
...
void BehaviorOnboardingLookAtPhone::MoveHeadUp() {
  if (_hasBleKeys) {
    ...
    auto* action = new TriggerLiftSafeAnimationAction{
        AnimationTrigger::OnboardingLookAtPhoneUp};
    action->SetRenderInEyeHue(false);
    DelegateIfInControl(action,
                        [this](const ActionResult& res) { RunLoopAction(); });
  } else {
...
void BehaviorOnboardingLookAtPhone::CheckIfTooHotToCharge() {
  ...
  if (battComp.IsChargingStalledBecauseTooHot()) {
    if (!_dVars.displayingTooHotToCharge) {
      ...
      LOG_INFO(
          "BehaviorOnboardingLookAtPhone.CheckIfTooHotToCharge.BatteryIsTooHot",
          "");
      const auto& dataPlatform = robotInfo.GetContext()->GetDataPlatform();
      static const std::string kTooHotToChargeImg =
          "config/sprites/independentSprites/battery_overheated.png";
      const std::string imgPath = dataPlatform->pathToResource(
          Anki::Util::Data::Scope::Resources, kTooHotToChargeImg);
      Vision::ImageRGB img;
      img.Load(imgPath);
...
void BehaviorOnboardingLookAtPhone::RunLoopAction() {
  ...
  if (!_dVars.displayingTooHotToCharge) {
    auto* loopAction = new ReselectingLoopAnimationAction{
        AnimationTrigger::OnboardingLookAtPhoneLoop};
