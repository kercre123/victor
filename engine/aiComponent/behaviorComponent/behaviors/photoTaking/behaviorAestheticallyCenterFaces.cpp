...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAestheticallyCenterFaces::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAestheticallyCenterFaces::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAestheticallyCenterFaces::GetBehaviorJsonKeys(
...
void BehaviorAestheticallyCenterFaces::InitBehavior() {
  ...
  _iConfig.criteriaMap.insert(std::make_pair(
      FaceSelectionPenaltyMultiplier::RelativeBodyAngleRadians, 1000));
}
...
void BehaviorAestheticallyCenterFaces::TransitionToSearchForFaces() {
  ...
  ANKI_VERIFY(_iConfig.findFacesBehavior->WantsToBeActivated(),
              "BehaviorAestheticallyCenterFaces.TransitionToSearchForFaces."
              "DoesNotWantToBeActivated",
              "");
  // Delegate to the find faces behavior. It will exit if a face is seen, so in
  // that case skip directly to TransitionToCenterFace. It will also dynamically
  // adjust its timeout based on whether a body is seen
  DelegateIfInControl(
      _iConfig.findFacesBehavior.get(),
...
void BehaviorAestheticallyCenterFaces::TransitionToCenterFace() {
  ...
  if (face != nullptr) {
    const auto& smartFaceId =
        GetBEI().GetFaceWorld().GetSmartFaceID(face->GetID());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
const Vision::TrackedFace*
BehaviorAestheticallyCenterFaces::GetBestFaceToCenter() {
  ...
  auto haveNotSeenSinceStarted = [this](const SmartFaceID& smartId) {
    ...
    return (face == nullptr) ||
           (face->GetTimeStamp() < _dVars.imageTimestampWhenActivated);
  };
  smartFaces.erase(std::remove_if(smartFaces.begin(), smartFaces.end(),
                                  haveNotSeenSinceStarted),
                   smartFaces.end());

  const auto& faceSelection = GetAIComp<FaceSelectionComponent>();
  auto smartFaceID =
      faceSelection.GetBestFaceToUse(_iConfig.criteriaMap, smartFaces);
  const Vision::TrackedFace* trackedFace =
      GetBEI().GetFaceWorld().GetFace(smartFaceID);
  return trackedFace;
}
