...
bool BehaviorTakeAPhotoCoordinator::WantsToBeActivatedBehavior() const {
  ...
  const bool isPhotoPending =
      uic.IsUserIntentPending(USER_INTENT(take_a_photo));
  return isPhotoPending;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorTakeAPhotoCoordinator::GetBehaviorOperationModifiers(
  ...
  modifiers.visionModesForActiveScope->insert(
      {VisionMode::AutoExp_MinGain, EVisionUpdateFrequency::High});
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorTakeAPhotoCoordinator::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorTakeAPhotoCoordinator::GetBehaviorJsonKeys(
...
void BehaviorTakeAPhotoCoordinator::InitBehavior() {
  ...
  _iConfig.storageIsFullBehavior =
...
void BehaviorTakeAPhotoCoordinator::OnBehaviorActivated() {
  ...
  if (isStorageFull) {
    ...
    return;  // nothing to do here, especially don't go down and enable High Res
             // photo mode
  } else if (intentData != nullptr) {
    const bool robotPickedUp =
        GetBEI().GetRobotInfo().GetOffTreadsState() != OffTreadsState::OnTreads;
    _dVars.isASelfie =
        !(intentData->intent.Get_take_a_photo().empty_or_selfie.empty());
    // If we're taking a selfie we need to center the faces first - otherwise
    // just take a photo
    if (_dVars.isASelfie && !robotPickedUp) {
      TransitionToDriveOffCharger();
    } else {
      DelegateIfInControl(new MoveHeadToAngleAction(DEG_TO_RAD(kHeadAngleDeg)),
                          [this]() { TransitionToFocusingAnimation(); });
    }
  } else {
    PRINT_NAMED_ERROR(
        "BehaviorTakeAPhotoCoordinator.OnBehaviorActivated.NullIntentData", "");
  }

  if (RESULT_OK != GetBEI().GetPhotographyManager().EnablePhotoMode(true)) {
    PRINT_NAMED_ERROR(
        "BehaviorTakeAPhotoCoordinator.OnBehaviorActivated."
        "EnablePhotoModeFailed",
        "");
  }
}
...
void BehaviorTakeAPhotoCoordinator::OnBehaviorDeactivated() {
  if (RESULT_OK != GetBEI().GetPhotographyManager().EnablePhotoMode(false)) {
    PRINT_NAMED_ERROR(
        "BehaviorTakeAPhotoCoordinator.OnBehaviorDectivated."
        "DisablePhotoModeFailed",
        "");
  }
}
...
void BehaviorTakeAPhotoCoordinator::TransitionToStorageIsFull() {
  ANKI_VERIFY(_iConfig.storageIsFullBehavior->WantsToBeActivated(),
              "BehaviorTakeAPhotoCoordinator.TransitionToStorageIsFull."
...
void BehaviorTakeAPhotoCoordinator::TransitionToDriveOffCharger() {
  ...
  if (isOnCharger) {
    ANKI_VERIFY(_iConfig.driveOffChargerBehavior->WantsToBeActivated(),
                "BehaviorTakeAPhotoCoordinator.TransitionToDriveOffCharger."
                "DoesNotWantToBeActivated",
                "");
...
void BehaviorTakeAPhotoCoordinator::TransitionToFrameFaces() {
  ANKI_VERIFY(_iConfig.frameFacesBehavior->WantsToBeActivated(),
              ...
              "DoesNotWantToBeActivated",
              "");
  const auto imageTimestampWhenStarted =
      GetBEI().GetRobotInfo().GetLastImageTimeStamp();
  DelegateIfInControl(
      _iConfig.frameFacesBehavior.get(), [this, imageTimestampWhenStarted]() {
        const bool sawAnyFaces =
            GetBEI().GetFaceWorld().HasAnyFaces(imageTimestampWhenStarted);
        if (sawAnyFaces) {
          ...
          DelegateIfInControl(
              new TriggerAnimationAction(AnimationTrigger::ICantDoThat));
          static const bool kSucceeded = false;
          GetBEI().GetPhotographyManager().SendDASEvent(kSucceeded,
                                                        "UnableToFindFace");
        }
      });
}
...
void BehaviorTakeAPhotoCoordinator::TransitionToFocusingAnimation() {
  DelegateIfInControl(
      new TriggerAnimationAction(AnimationTrigger::TakeAPictureFocusing),
...
void BehaviorTakeAPhotoCoordinator::TransitionToTakePhoto() {
  if (GetBEI().GetPhotographyManager().IsReadyToTakePhoto()) {
    // We should normally be ready immediately because the animations should
    // have taken long enough to switch camera formats
    CaptureCurrentImage();
  } else {
    // We generally should not need to wait any more than for the animations to
    // play, so issue an error and wait for the photo manager to be ready to
    // take a photo and *then* transition
    PRINT_NAMED_ERROR(
        "BehaviorTakeAPhotoCoordinator.TransitionToTakePhoto."
        "NotReadyAfterAnimating",
        "");

    WaitForLambdaAction* waitAction = new WaitForLambdaAction(
        [](Robot& robot) {
...
void BehaviorTakeAPhotoCoordinator::CaptureCurrentImage() {
  ...
  DelegateIfInControl(action, [this, photoHandle](ActionResult result) {
    if (ActionResult::SUCCESS == result) {
      PRINT_CH_INFO(
          "Behaviors",
          "BehaviorTakeAPhotoCoordinator.CaptureCurrentImage.PhotoWasTaken",
          "Handle: %zu", photoHandle);
    } else {
      GetBEI().GetPhotographyManager().CancelTakePhoto();
      PRINT_NAMED_ERROR(
          "BehaviorTakeAPhotoCoordinator.CaptureCurrentImage.TakePhotoTimedOut",
          "Handle: %zu Timeout: %.2fsec", photoHandle, kTakingPhotoTimeout_sec);
    }
  });
}
