...
class BehaviorAdvanceClock : public BehaviorProceduralClock {
 public:
  ...
  void SetAdvanceClockParams(int startTime_sec, int endTime_sec,
                             int displayTime_sec);
 ...
 protected:
  ...
  virtual void GetBehaviorJsonKeysInternal(
 ...
 private:
  ...
  int GetTotalSecToAdvance() const {
    return std::abs(_endTime_sec - _startTime_sec);
  }
