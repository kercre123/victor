 ...
 * Created: 2018-05-30
 *
 * Description: Advance the clock display between two times over the specified
 *interval
 *
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAdvanceClock::GetBehaviorJsonKeysInternal(
    std::set<const char*>& expectedKeys) const {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorAdvanceClock::SetAdvanceClockParams(int startTime_sec,
                                                 int endTime_sec,
                                                 int displayTime_sec) {
...
void BehaviorAdvanceClock::TransitionToShowClockInternal() {
...

  {
    ...
    audioMessage.audioEvent = AudioMetaData::GameEvent::GenericEvent::
        Play__Robot_Vic_Sfx__Timer_Run_Down_Loop_Play;

    RobotInterface::EngineToRobot wrapper(std::move(audioMessage));
    GetBEI().GetAnimationComponent().AlterStreamingAnimationAtTime(
        std::move(wrapper), 0);
  }
  {
    AudioEngine::Multiplexer::PostAudioEvent audioMessage;
    ...
    RobotInterface::EngineToRobot wrapper(std::move(audioMessage));
    GetBEI().GetAnimationComponent().AlterStreamingAnimationAtTime(
        std::move(wrapper), Util::SecToMilliSec(GetTimeDisplayClock_sec()));
  }
}
...
BehaviorAdvanceClock::BuildTimerFunction() const {
  return [this](const int offset) {
    ...
    const int nextSecToDisplay = TimeGoesUp()
                                     ? _startTime_sec + currentSecondsOffset
                                     : _startTime_sec - currentSecondsOffset;

    std::map<Vision::SpriteBoxName, int> outMap;

    const int hoursRemaining =
        TimerHandle::SecondsToDisplayHours(nextSecToDisplay);
    const int minsRemaining =
        TimerHandle::SecondsToDisplayMinutes(nextSecToDisplay);
    const int secsRemaining =
        TimerHandle::SecondsToDisplaySeconds(nextSecToDisplay);
    ...
    // Tens Digit (left of colon)
    {
      ...
      outMap.emplace(
          std::make_pair(Vision::SpriteBoxName::SpriteBox_1, tensDigit));
    }
    ...
    // Ones Digit (left of colon)
    {
      ...
      outMap.emplace(
          std::make_pair(Vision::SpriteBoxName::SpriteBox_2, onesDigit));
    }
    ...
    // Tens Digit (right of colon)
    {
      ...
      outMap.emplace(
          std::make_pair(Vision::SpriteBoxName::SpriteBox_4, tensDigit));
    }
    ...
    // Ones Digit (right of colon)
    {
      ...
      outMap.emplace(
          std::make_pair(Vision::SpriteBoxName::SpriteBox_5, onesDigit));
    }
