 ...
 * Created: 2018-05-30
 *
 * Description: If the robot has a valid wall time, display it on the robot's
 *face
 *
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDisplayWallTime::GetBehaviorOperationModifiersProceduralClock(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDisplayWallTime::GetBehaviorJsonKeysInternal(
...
void BehaviorDisplayWallTime::SetOverrideDisplayTime(struct tm& time) {
  if (_hasTimeOverride) {
    PRINT_NAMED_WARNING(
        "BehaviorDisplayWallTime.TimeAlreadyOverriden",
...
bool BehaviorDisplayWallTime::WantsToBeActivatedBehavior() const {
  ...
  struct tm unused;
  return _hasTimeOverride ||
         WallTime::getInstance()->GetApproximateLocalTime(unused);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorDisplayWallTime::ShouldDimLeadingZeros() const {
  // in 24 hour clocks, show leading zeros as full brightness (e.g. 00:07),
  // otherwise let them be dim (e.g. 01:30)
...
BehaviorDisplayWallTime::BuildTimerFunction() const {
    ...
    if (_hasTimeOverride) {
      ...
      if (!WallTime::getInstance()->GetApproximateLocalTime(localTime)) {
        PRINT_NAMED_ERROR("BehaviorProceduralClock.BuildTimerFunction.NoTime",
                          "Behavior '%s' activated when it had a valid wall "
                          "time, but now it does not",
                          GetDebugLabel().c_str());
        return outMap;
      }
    }
...
bool BehaviorDisplayWallTime::ShouldDisplayAsMilitaryTime() const {
  ...
  const bool clockIs24Hour = settingsManager.GetRobotSettingAsBool(
