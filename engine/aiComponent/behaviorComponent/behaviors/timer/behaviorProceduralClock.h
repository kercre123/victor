...
class BehaviorProceduralClock : public ICozmoBehavior {
 public:
  ...
  using GetDigitsFunction =
      std::function<std::map<Vision::SpriteBoxName, int>(const int offset)>;

  // Specify how each digit on the face should be calculated - must set all 4
  // values in map
  void SetGetDigitFunction(GetDigitsFunction&& function);
  // Specify a callback which should be called when the clock is shown on the
  // robot's face
  void SetShowClockCallback(std::function<void()>&& callback) {
    _instanceParams.showClockCallback = callback;
  }

  float GetTimeDisplayClock_sec() const {
    return _instanceParams.totalTimeDisplayClock_sec;
  }
 ...
 protected:
  ...
  virtual void GetBehaviorOperationModifiersProceduralClock(
  ...
  // Default behavior is to update the clock once per second for the length of
  // display clock
  virtual void TransitionToShowClockInternal();

  void SetTimeDisplayClock_sec(float displayTime_sec) {
    _instanceParams.totalTimeDisplayClock_sec = displayTime_sec;
  }

  // Function which builds and displays the proceduralClock - adds the 4 core
  // digits on top of any quadrant images passed into the function
  void BuildAndDisplayProceduralClock(const int clockOffset_s = 0,
                                      const int displayOffset_ms = 0);

  void AddKeyFramesForOffset(const int clockOffset_s = 0,
                             const int displayTime_ms = 0);
 ...
 private:
  ...
  // Updates the target face within lifetime params - returns the member face
  // for checking success inline
