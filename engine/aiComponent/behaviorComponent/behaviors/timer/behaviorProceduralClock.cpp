...
const Vision::SpriteBoxKeyFrame kTensLeftOfColonKeyFrame(
    Vision::SpriteBox(100.0f, 27, 26, 29, 43,
                      Vision::SpriteBoxName::SpriteBox_1,
                      Anki::Vision::LayerName::Layer_6,
                      Anki::Vision::SpriteRenderMethod::EyeColor, 0),
    0.0f, kClockEmptyGridSpriteID, Anki::Vision::SpriteSeqEndType::Clear, 0);

const Vision::SpriteBoxKeyFrame kOnesLeftOfColonKeyFrame(
    Vision::SpriteBox(100.0f, 57, 26, 29, 43,
...
const Vision::SpriteBoxKeyFrame kColonKeyFrame(
    Vision::SpriteBox(100.0f, 87, 27, 10, 43,
                      Vision::SpriteBoxName::SpriteBox_3,
                      Anki::Vision::LayerName::Layer_6,
                      Anki::Vision::SpriteRenderMethod::EyeColor, 0),
    0.0f, Vision::SpritePathMap::GetAssetID("clock_colon"),
    Anki::Vision::SpriteSeqEndType::Clear, 0);

const Vision::SpriteBoxKeyFrame kTensRightOfColonKeyFrame(
    Vision::SpriteBox(100.0f, 98, 26, 29, 43,
...
const Vision::SpriteBoxKeyFrame kOnesRightOfColonKeyFrame(
    Vision::SpriteBox(100.0f, 128, 26, 29, 43,
...
BehaviorProceduralClock::BehaviorProceduralClock(const Json::Value& config)
  ...
  _instanceParams.getInAnim = AnimationTriggerFromString(
      JsonTools::ParseString(config, kGetInTriggerKey, kDebugStr));
  _instanceParams.getOutAnim = AnimationTriggerFromString(
      JsonTools::ParseString(config, kGetOutTriggerKey, kDebugStr));
  _instanceParams.totalTimeDisplayClock_sec = static_cast<float>(
      JsonTools::ParseUint8(config, kDisplayClockSKey, kDebugStr));
  JsonTools::GetValueOptional(config, kShouldTurnToFaceKey,
                              _instanceParams.shouldTurnToFace);
  JsonTools::GetValueOptional(config, kShouldPlayAudioKey,
                              _instanceParams.shouldPlayAudioOnClockUpdates);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorProceduralClock::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kGetInTriggerKey, kGetOutTriggerKey, kDisplayClockSKey,
                        kShouldTurnToFaceKey, kShouldPlayAudioKey};
...
void BehaviorProceduralClock::OnBehaviorActivated() {
  ...
  Vision::SpriteBoxKeyFrame colonStartKeyFrame =
      kKeyFrameMap.at(Vision::SpriteBoxName::SpriteBox_3);
  Vision::SpriteBoxKeyFrame colonEndKeyFrame =
      kKeyFrameMap.at(Vision::SpriteBoxName::SpriteBox_3);

  int timeToDisplay_ms =
      Util::SecToMilliSec(_instanceParams.totalTimeDisplayClock_sec);
  colonEndKeyFrame.triggerTime_ms =
      timeToDisplay_ms - (timeToDisplay_ms % ANIM_TIME_STEP_MS);
...
void BehaviorProceduralClock::TransitionToTurnToFace() {
  if (ANKI_VERIFY(_lifetimeParams.targetFaceID.IsValid(),
                  ...
                  "")) {
    DelegateIfInControl(
        new TurnTowardsFaceAction(_lifetimeParams.targetFaceID, M_PI_F, true),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorProceduralClock::AddKeyFramesForOffset(const int clockOffset_s,
                                                    const int displayTime_ms) {
  // TODO(str): Could optimize by enforcing "animation chronological" order when
  // adding keyframes, then check for change against the last keyframe for each
  // given spritebox to prevent sending redundant keyframes if e.g. only one of
  // the four digits changes. As it is, the SpriteBoxKeyFrame technique requires
  // many fewer messages than the CompositeImage approach, so we can probably
  // forgo the complexity

  // Truncate to the nearest valid frame time for the triggerTime
  const int triggerTime_ms =
      displayTime_ms - (displayTime_ms % ANIM_TIME_STEP_MS);

  // set digits
  bool isLeadingZero = ShouldDimLeadingZeros();

  const std::map<Vision::SpriteBoxName, int> digitMap =
      _instanceParams.getDigitFunction(clockOffset_s);
  ...
  for (auto& pair : digitMap) {
    ...
    if (digit < 0) {
      digit = 0;
    };  // The timer is shorted than the initial diplay time
...
void BehaviorProceduralClock::DisplayClock() {
  ...
  if (_lifetimeParams.keyFrames.size() > maxFramesForPlayAnimMsg) {
    auto firstKeyFrameIter =
        _lifetimeParams.keyFrames.begin() + maxFramesForPlayAnimMsg;
    auto lastKeyFrameIter = _lifetimeParams.keyFrames.end();
    std::move(firstKeyFrameIter, lastKeyFrameIter,
              std::back_inserter(residualKeyFrames));
  ...
  GetBEI().GetAnimationComponent().PlayAnimWithSpriteBoxKeyFrames(
      "", _lifetimeParams.keyFrames, true, animationCallback);

  // If there were excess keyframes, send them now
  if (!residualKeyFrames.empty()) {
    RobotInterface::AddSpriteBoxKeyFrames dummyAddMsg;
    const size_t maxFramesForAddKeyFramesMsg =
        dummyAddMsg.spriteBoxKeyFrames.size();
    while (!residualKeyFrames.empty()) {
      const size_t numToSend =
          std::min(residualKeyFrames.size(), maxFramesForAddKeyFramesMsg);
      ...
      std::move(firstKeyFrameIter, lastKeyFrameIter,
                std::back_inserter(keyFramesToSend));
      residualKeyFrames.erase(firstKeyFrameIter, lastKeyFrameIter);
      GetBEI().GetAnimationComponent().AddSpriteBoxKeyFramesToRunningAnim(
          keyFramesToSend);
    }
  }

  // TODO(str): This will still send one message for every update, and ends up
  // being handled by the StreamingAnimationModifier. We could/should provide
  // keyframe based handling for these audio events, and any other keyframe
  // types we need, if we really want to "build animations" from the engine like
  // we're doing here. The StreamingAnimationModifier obfuscates logical flow in
  // the AnimationStreamer.
  if (_instanceParams.shouldPlayAudioOnClockUpdates) {
    ...
    for (const auto& triggerTime_ms : _lifetimeParams.audioTickTimes) {
      ...
      GetBEI().GetAnimationComponent().AlterStreamingAnimationAtTime(
          std::move(wrapper), triggerTime_ms);
    }
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorProceduralClock::SetGetDigitFunction(
    GetDigitsFunction&& function) {
...
SmartFaceID BehaviorProceduralClock::UpdateTargetFace() {
  ...
  _lifetimeParams.targetFaceID =
      faceSelection.GetBestFaceToUse(criteriaMap, smartFaces);
