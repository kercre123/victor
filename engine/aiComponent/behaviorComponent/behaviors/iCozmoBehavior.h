...
struct BehaviorOperationModifiers {
  BehaviorOperationModifiers() {
    visionModesForActivatableScope =
        std::make_unique<std::set<VisionModeRequest>>();
    visionModesForActiveScope = std::make_unique<std::set<VisionModeRequest>>();
  }

  // Alters the default value of the behavior operation modifiers via the
  // behavior's JSON configuration map. Returns a set of tags of which defaults
  // were set.
  std::set<std::string> SetDefaultBehaviorOperationModifiers(
      const Json::Value& config, const std::string& debugLabel);
  ...
  const std::unordered_map<std::string, bool*> stringToModifiersFlagMap = {
      {"wantsToBeActivatedWhenCarryingObject",
       &wantsToBeActivatedWhenCarryingObject},
      ...
      {"ensuresCubeConnectionAtDelegation",
       &ensuresCubeConnectionAtDelegation}};
  ...
  // If true iCozmoBehavior will automatically cancel the behavior if it hasn't
  // delegated control by the end of its tick Default is True for two reasons:
  //   1) Most behaviors don't want the robot to sit still not doing anything
  //      so they are always delegating or have hit an unexpected bug, and it's
  //      better to cancel so the robot doesn't freeze
  //   2) It supports open-ended final Delegation calls. After the final
  //   delegation the behavior doesn't have to
  //      monitor its status - it will be automatically canceled when the final
  ...
  // Behaviors which require cube connections with varying degrees of
  // flexibility should specify their type of requirement here. Subscription to
  // cube connections will then be managed directly by iCozmoBehavior on
  // Activation/Deactivation, and WantsToBeActivatedBase will confirm that
  // connection requirements are met.
  enum class CubeConnectionRequirements {
    None,
    OptionalLazy,    // Always wantsToBeActivated; Subscribe only if already
                     // connected
    OptionalActive,  // Always wantsToBeActivated; Always subscribe
    RequiredLazy,    // Run only if already connected. Always subscribe if
                     // activated.
    RequiredManaged  // Run only if already connected. Always subscribe if
                     // activated. Requires Ancestor to manage connection.
  } cubeConnectionRequirements = CubeConnectionRequirements::None;

  bool CubeConnectionRequirementFromString(
      const std::string& str, CubeConnectionRequirements& enumOutput) const;

  // Background connections will open and hold a cube connection open, but will
  // not trigger connection/status lights. If a non-background subscription is
  // made, the connection will convert to foreground until all foreground
  // subscriptions are gone, whereupon we will indicate disconnection to the
  // user and convert back to a background connection until ALL subscriptions
  // are released which will trigger disconnection. NOTE: THIS ONLY SUPPRESSES
  // STATUS LIGHTS! Light anims may still be invoked via the CubeLightComponent
  // while in a background connection
  bool connectToCubeInBackground = false;

  // If set to true, this behavior will satisfy the Ancestry requirements for
  // behaviors with CubeConnectionRequirements::RequiredManaged.
  bool ensuresCubeConnectionAtDelegation = false;

  // Behaviors which require vision processing can add requests to these vectors
  // to have the base class manage subscriptions to those VisionModes. Default
  // is none.
...
class ICozmoBehavior : public IBehavior,
 ...
 protected:
  ...
  // Constructs in the same way, but holds a container which keeps custom
  // injected BEI conditions in scope. This allows a derived class to declare
  // custom BEI conditions and have them be used here (e.g. for a
  // WantsToBeActivated json condition)
 ...
 public:
  static Json::Value CreateDefaultBehaviorConfig(BehaviorClass behaviorClass,
                                                 BehaviorID behaviorID);
  static void InjectBehaviorClassAndIDIntoConfig(BehaviorClass behaviorClass,
                                                 BehaviorID behaviorID,
                                                 Json::Value& config);
  static BehaviorID ExtractBehaviorIDFromConfig(
      const Json::Value& config, const std::string& fileName = "");
  static BehaviorClass ExtractBehaviorClassFromConfig(
      const Json::Value& config);

  // check if the given json is a valid config for this behavior. Note that the
  // behavior must already be constructed in order for this to work.
  void CheckJson(const Json::Value& config);

  // After Init is called, this function should be called on every behavior to
  // set the operation modifiers. It will internally call
  // GetBehaviorOperationModifiers() as needed
  ...
  // returns the number of times this behavior has been started (number of times
  // Init was called and returned OK, not counting calls to Resume)
  int GetNumTimesBehaviorStarted() const { return _startCount; }
  void ResetStartCount() { _startCount = 0; }

  // Time this behavior was activated, in seconds, as measured by basestation
  // time
  ...
  // Calls protected virtual OnBehaviorActivated() method, which each derived
  // class should implement.
  void OnActivatedInternal() override final;

  // This behavior was active, but is now stopping (to make way for a new
  // current behavior). Any behaviors from DelegateIfInControl will be canceled.
  void OnDeactivatedInternal() final override;

  // Returns true if the state of the world/robot is sufficient for this
  ...
  // Return true if this is a good time to interrupt this behavior. This allows
  // more gentle interruptions for things which aren't immediately urgent.
  // Eventually this may become a mandatory override, but for now, the default
  // is that we can never interrupt gently
  virtual bool CanBeGentlyInterruptedNow() const { return false; }

  // Helper function for having DriveToObjectActions use the second closest
  // preAction pose useful when the action is being retried or the action failed
  // due to visualVerification
  ActionResult UseSecondClosestPreActionPose(DriveToObjectAction* action,
                                             ActionableObject* object,
                                             std::vector<Pose3d>& possiblePoses,
                                             bool& alreadyInPosition);

  // Add Listeners to a behavior which will notify them of milestones/events in
  // the behavior's lifecycle
  // TODO:(bn) this is likely unused and should be deleted (we tend to use
  // direct casts to behavior types now)
  ...
  // Give derived behaviors the opportunity to override default behavior
  // operations
  virtual void GetBehaviorOperationModifiers(
      BehaviorOperationModifiers& modifiers) const = 0;

  // Give other behaviors access to a constant reference to the finalized
  // operation modifiers, including the defaults set by the JSON configuration
  // during the construction of the behavior. In the case of a dispatcher
  // behavior, this also factors in the operation modifiers of all its possible
  // dispatches. Should only be called after Init.
  const BehaviorOperationModifiers& GetBehaviorOperationModifiersPostInit()
      const;

  // gets list of ICozmoBehavior's expected keys and the derived class's,
  // through GetBehaviorJsonKeys
  std::vector<const char*> GetAllJsonKeys() const;

  // Allow external behaviors to ask that this behavior return false for
  // WantsToBeActivated for this tick. This should only be used by "coordinator"
  // behaviors to allow an "event" to bypass one of the behaviors
  // InActivatableScope so that it can be handled by something further down the
  // stack
  void SetDontActivateThisTick(const std::string& coordinatorName);

  // if an active feature is associated with this behavior, return true and set
  // it in arguments
  bool GetAssociatedActiveFeature(ActiveFeature& feature) const;

  UserIntentPtr ActivateUserIntentHelper(UserIntentTag tag,
                                         const std::string& owner = "");
  void DeactivateUserIntentHelper(UserIntentTag tag);

  std::map<std::string, ICozmoBehaviorPtr> TESTONLY_GetAnonBehaviors(
      UnitTestKey key) const;
 ...
 protected:
  ...
  inline void SetDebugStateName(const std::string& inName) {
    PRINT_CH_INFO("Behaviors", "Behavior.TransitionToState",
                  "Behavior:%s, FromState:%s ToState:%s",
                  GetDebugLabel().c_str(), _debugStateName.c_str(),
                  inName.c_str());
  ...
  using EvalUserIntentFunc =
      std::function<bool(const UserIntent&)>;  // should be true if data matches

  // You can add any number of user intents that you wish to wait for. If there
  // is a pending intent and it matches, based on any of the conditions you
  // supply, then this ICozmoBehavior will WantsToBeActivated(), in the absence
  // of any other blocking conditions. When the behavior activates, it will
  // clear the pending intent that matched one of your conditions. The
  // functionality of all of these methods is also available in your behavior
  // instance json with the parameter "respondToUserIntents," but sometimes it's
  // more appropriate to use code. All of these methods must be called before
  // Init() is called. You may: Wait for a match with the specfic tag
  ...
  // Wait for a match with tag AND func(intent). The func will only be evaluated
  // if the tag matches, so your lambda only needs to worry about the params,
  // not the tag.
  ...
  // Add to a list of BehaviorTimerManager timers that this behavior should
  // reset upon activation. You don't want to put a reset in too high (/ deep in
  // the stack) a behavior in case one of its delegates fails, which might reset
  // the timer without the action being performed. You also don't want to put it
  // in too low a delegate for the same reason -- if it doesn't start, the
  // higher level behavior might try again, and you end up in a loop. Choose
  // wisely my friend.
  void AddResetTimer(const std::string& timerName) {
    _resetTimers.push_back(timerName);
  }

  // Derived behaviors can specify all json keys they expect at the root level,
  // so that behavior json loading fails (in dev) if unexpected keys are found.
  // You should insert into this list instead of assigning it.
  virtual void GetBehaviorJsonKeys(
      std::set<const char*>& expectedKeys) const = 0;
  ...
  // This function can be implemented by behaviors. It should return Running
  // while it is running, and Complete or Failure as needed. If it returns
  // Complete, Stop will be called. Default implementation is to return Running
  // while ControlIsDelegated, and Complete otherwise
  ...
  void SubscribeToAppTags(
      std::set<AppToEngineTag>&&
          tags);  // can't be an overload since it is not a scoped enum
  ...
  // is not running. If the behavior is subscribed to multiple tags, the
  // presumption is that this will handle switching based on tag internally.
  // NOTE: AlwaysHandle is called before HandleWhileRunning and
  ...
  // Many behaviors use a pattern of delegating control to a behavior or action,
  // then waiting for it to finish before selecting what to do next. Behaviors
  // can use the functions below to pass lambdas or member functions as
  // callbacks when delegation finishes instead of monitoring for changes in
  // IsControlDelegated() directly. Standard delegation restrictions apply:
  ...
  // Delegation can still fail if the behavior is not in charge of the behavior
  // stack
  bool DelegateNow(IActionRunner* action,
  ...
  // DelegateIfInControl functions will only successfully delegate if the
  // behavior is currently in control of the behavior stack and has not
  // delegated to an action
  bool DelegateIfInControl(IActionRunner* action,
                           RobotCompletedActionCallback callback = {});
  bool DelegateIfInControl(IActionRunner* action,
  ...
  // DelegateNow templated functions - allow member functions to be used for
  // callbacks
  template <typename T>
  bool DelegateNow(IActionRunner* action,
                   void (T::*callback)(RobotCompletedActionCallback));
  template <typename T>
  ...
  // DelegateIfInControl templated functions - allow member functions to be used
  // for callbacks
  template <typename T>
  bool DelegateIfInControl(IActionRunner* action,
                           void (T::*callback)(RobotCompletedActionCallback));
  ...
  // Searches both the behavior container and anonymous behaviors. Returns the
  // behavior or nullptr if not found (and performs an ANKI_VERIFY internally)
  ICozmoBehaviorPtr FindBehavior(const std::string& behaviorIDStr) const;

  // Behaviors can easily create delegates using "anonymous" behaviors in their
  // config file (see _anonymousBehaviorMap comments below).  This function
  // enables access to those anonymous behaviors.
  ICozmoBehaviorPtr FindAnonymousBehaviorByName(
      const std::string& behaviorName) const;

  // Sometimes it's necessary to downcast to a behavior to a specific behavior
  // pointer, e.g. so an Activity can access it's member functions. This
  // function will help with that and provide a few assert checks along the way.
  // It sets outPtr in arguemnts, and returns true if the cast is successful
  template <typename T>
  ...
  // This function cancels the action started by DelegateIfInControl (if there
  // is one). Returns true if an action was canceled, false otherwise. Note that
  // if you are activated, this will trigger a callback for the cancellation
  // unless you set allowCallback to false.
  bool CancelDelegates(bool allowCallback = true);

  // This function cancels this behavior. It never allows callbacks to continue.
  // This is just a convenience wrapper for calling CancelSelf on the delegation
  // component. It returns true if the behavior was successfully canceled, false
  // otherwise (e.g. the behavior wasn't active to begin with)
  bool CancelSelf();

  /////////////
  /// "Smart" helpers - Behaviors can call these functions to set properties
  /// that need to be cleared when the behavior stops.  ICozmoBehavior will hold
  /// the reference and clear it appropriately.  Functions also exist to clear
  /// these properties before the behavior stops.
  ////////////////

  // For the duration of this behavior, or until SmartClearMotionProfile() is
  // called (whichever is sooner), use the specified motion profile for all
  // motions. Note that this will result in an error if the behavior tries to
  // manually set a speed or acceleration on an action. This may be called
  // automatically based on behavior json data here at the ICozmoBehavior level
  void SmartSetMotionProfile(const PathMotionProfile& motionProfile);
  void SmartClearMotionProfile();

  // Allows the behavior to lock and unlock tracks without worrying about the
  // possibility of the behavior being interrupted and leaving the track locked
  bool SmartLockTracks(u8 animationTracks, const std::string& who,
                       const std::string& debugName);
  bool SmartUnLockTracks(const std::string& who);

  // Allows the behavior to set a custom light pattern which will be
  // automatically canceled if the behavior ends
  bool SmartSetCustomLightPattern(
      const ObjectID& objectID, const CubeAnimationTrigger& anim,
      const CubeLightAnimation::ObjectLights& modifier = {});
  bool SmartRemoveCustomLightPattern(
      const ObjectID& objectID, const std::vector<CubeAnimationTrigger>& anims);

  // Allows the behavior to activate a user intent and have it automatically
  // deactived when this behavior is deactivated. For convenience (in the case
  // where there is extra intent data), a pointer the the intent is returned.
  // This pointer will be null if the intent couldn't be activated (i.e. it
  // wasn't pending)
  ...
  void SmartPushResponseToTriggerWord(
      const AnimationTrigger& getInAnimTrigger,
  ...
  void SmartAlterStreamStateForCurrentResponse(
  ...
  // From code, a behavior can use this function to override the active feature
  // for a _single_ activation of this behavior. This function must be called
  // before the end of OnBehaviorActivated for it to work, and can not be
  // combined with a json-set active feature. NOTE: in most cases, you probably
  // don't want this and instead should set a fixed active feature in the json
  // instance of the behavior (most behavior instances correspond to a single
  // feature) NOTE: if you get too fancy here you might break the
  // CheckActiveFeatures unit test in testBehaviorSystemDelegationTree.cpp
  void SmartSetActiveFeatureOnActivated(const ActiveFeature& feature);

  // Helper function to play an emergency get out through the continuity
  // component
  ...
  // NOTE: this is old functionality from Cozmo sparks, but may become useful
  // again if we want a way to "streamline" certain behaviors (i.e. skip some of
  // the animations / reactions that can slow things down). BN: as of 1.0.1 this
  // is only ever used in a single behavior (pickupCube) and maybe not
  // intentionally so...
  bool ShouldStreamline() const { return (_alwaysStreamline); }

// make a member variable a console var that is only around as long as its class
// instance is
#if ANKI_DEV_CHEATS
  template <typename T>
 ...
 private:
  ...
  // Helper function for WantsToBeActivatedBase, returns true if the
  // cubeConnectionRequirements for this behavior are satisfied
  ...
  // 1) Return false from WantToBeActivated if the condition evaluates to false
  // (and true in
  ...
  // The tag of the intent that should be deactivated when this behavior
  // deactivates
  UserIntentTag _intentToDeactivate;

  // A behavior can specify an associated ActiveFeature. If it does, the
  // ActiveFeatureComponent can check this while the behavior is active on the
  // stack
  ...
  // a list of named timers in the BehaviorTimerManager that should be reset
  ...
  // Loaded in from data - these tracks will be locked/unlocked automatically
  ...
  // a NON-BINDING suggestion of what this behavior thinks should occur after
  // de-activation. this value will be written to the whiteboard upon
  // de-activation, and other behaviors can check it
  PostBehaviorSuggestions _postBehaviorSuggestion =
      PostBehaviorSuggestions::Invalid;

  // list of member vars in this behavior that have been marked as tunable. upon
  // class desctruction, each console var will be unregistered.
...
// - - - - - - - - - - - - - - - - - - - -
template <typename T>
bool ICozmoBehavior::DelegateNow(
    IActionRunner* action, void (T::*callback)(RobotCompletedActionCallback)) {
  return DelegateNow(action, std::bind(callback, static_cast<T*>(this),
                                       std::placeholders::_1));
}
...
// - - - - - - - - - - - - - - - - - - - -
template <typename T>
bool ICozmoBehavior::DelegateNow(IActionRunner* action,
                                 void (T::*callback)(ActionResult)) {
  return DelegateNow(action, std::bind(callback, static_cast<T*>(this),
                                       std::placeholders::_1));
}
...
bool ICozmoBehavior::DelegateNow(IBehavior* delegate, void (T::*callback)()) {
  return DelegateNow(delegate, std::bind(callback, static_cast<T*>(this)));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
template <typename T>
bool ICozmoBehavior::DelegateIfInControl(
    IActionRunner* action, void (T::*callback)(RobotCompletedActionCallback)) {
  return DelegateIfInControl(action, std::bind(callback, static_cast<T*>(this),
                                               std::placeholders::_1));
}
...
// - - - - - - - - - - - - - - - - - - - -
template <typename T>
bool ICozmoBehavior::DelegateIfInControl(IActionRunner* action,
                                         void (T::*callback)(ActionResult)) {
  return DelegateIfInControl(action, std::bind(callback, static_cast<T*>(this),
                                               std::placeholders::_1));
}
...
// - - - - - - - - - - - - - - - - - - - -
template <typename T>
bool ICozmoBehavior::FindAnonymousBehaviorByNameAndDowncast(
    const std::string& behaviorName, BehaviorClass requiredClass,
    std::shared_ptr<T>& outPtr) const {
  ...
  if (ANKI_VERIFY(
          ...
          "BehaviorName: %s requiredClass: %s", behaviorName.c_str(),
          GetClassString(requiredClass).c_str()) &&

      ANKI_VERIFY(
          behavior->GetClass() == requiredClass,
    ...
    if (ANKI_VERIFY(
            outPtr != nullptr,
            "ICozmoBehavior.FindAnonymousBehaviorByNameAndDowncast.CastFailed",
            "BehaviorName: %s requiredClass: %s", behaviorName.c_str(),
            GetClassString(requiredClass).c_str())) {
      return true;
    }
...
void ICozmoBehavior::MakeMemberTunable(T& param, const std::string& name,
  ...
  _tunableParams.emplace_back(new Util::ConsoleVar<T>(
      param, uniqueName.c_str(), category, unregisterOnDestruction));
}
#endif
