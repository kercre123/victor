...
void ApplyForceDelegationRequirements(ICozmoBehavior* behavior,
                                      BehaviorExternalInterface& bei) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Put your DEV delegation requirements
// here. Feel free to make a separate file that is included by this one. If this
// should only be done in unit tests, use testDelegationRequirements.h

template <>
void ApplyForceDelegationRequirements<BehaviorClass::PromptUserForVoiceCommand>(
    ICozmoBehavior* behavior, BehaviorExternalInterface& bei) {
  BehaviorPromptUserForVoiceCommand* castPtr =
      static_cast<BehaviorPromptUserForVoiceCommand*>(behavior);
  castPtr->SetPromptString("hello");
}

template <>
void ApplyForceDelegationRequirements<BehaviorClass::EyeColor>(
    ICozmoBehavior* behavior, BehaviorExternalInterface& bei) {
...
template <>
void ApplyForceDelegationRequirements<BehaviorClass::EnrollFace>(
    ICozmoBehavior* behavior, BehaviorExternalInterface& bei) {
...
template <>
void ApplyForceDelegationRequirements<BehaviorClass::RespondToRenameFace>(
    ICozmoBehavior* behavior, BehaviorExternalInterface& bei) {
  BehaviorRespondToRenameFace* castPtr =
      static_cast<BehaviorRespondToRenameFace*>(behavior);
  castPtr->SetName("Bobert");
}

template <>
void ApplyForceDelegationRequirements<BehaviorClass::LeaveAMessage>(
    ICozmoBehavior* behavior, BehaviorExternalInterface& bei) {
  auto& uic = bei.GetAIComponent()
                  .GetComponent<BehaviorComponent>()
                  .GetComponent<UserIntentComponent>();
...
template <>
void ApplyForceDelegationRequirements<BehaviorClass::PlaybackMessage>(
    ICozmoBehavior* behavior, BehaviorExternalInterface& bei) {
  auto& uic = bei.GetAIComponent()
                  .GetComponent<BehaviorComponent>()
                  .GetComponent<UserIntentComponent>();
...
template <>
void ApplyForceDelegationRequirements<BehaviorClass::TakeAPhotoCoordinator>(
    ICozmoBehavior* behavior, BehaviorExternalInterface& bei) {
  auto& uic = bei.GetAIComponent()
                  .GetComponent<BehaviorComponent>()
                  .GetComponent<UserIntentComponent>();
...
template <>
void ApplyForceDelegationRequirements<BehaviorClass::CoordinateWeather>(
    ICozmoBehavior* behavior, BehaviorExternalInterface& bei) {
  auto& uic = bei.GetAIComponent()
                  .GetComponent<BehaviorComponent>()
                  .GetComponent<UserIntentComponent>();
  ...
  UserIntent intent =
      UserIntent::Createweather_response(std::move(weatherResponse));
  uic.DevSetUserIntentPending(std::move(intent));
}

template <>
void ApplyForceDelegationRequirements<BehaviorClass::DisplayWeather>(
    ICozmoBehavior* behavior, BehaviorExternalInterface& bei) {
  auto& uic = bei.GetAIComponent()
                  .GetComponent<BehaviorComponent>()
                  .GetComponent<UserIntentComponent>();
...
template <>
void ApplyForceDelegationRequirements<BehaviorClass::ReactToMotion>(
    ICozmoBehavior* behavior, BehaviorExternalInterface& bei) {
  BehaviorReactToMotion* castPtr =
      static_cast<BehaviorReactToMotion*>(behavior);
  castPtr->DevAddFakeMotion();
}

template <>
void ApplyForceDelegationRequirements<BehaviorClass::SimpleVoiceResponse>(
    ICozmoBehavior* behavior, BehaviorExternalInterface& bei) {
  auto& uic = bei.GetAIComponent()
                  .GetComponent<BehaviorComponent>()
                  .GetComponent<UserIntentComponent>();
  ...
  UserIntent intent =
      UserIntent::Createsimple_voice_response(std::move(simpleResponse));
  uic.DevSetUserIntentPending(std::move(intent));
}
...
#define ADD_CASE_FOR_BEHAVIOR(x)                                       \
  case BehaviorClass::x: {                                             \
    ApplyForceDelegationRequirements<BehaviorClass::x>(behavior, bei); \
  } break
