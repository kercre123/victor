...
class BehaviorDockingTestSimple : public ICozmoBehavior {
 ...
 private:
  // returns true if the callback handled the action, false if we should
  // continue to handle it in HandleActionCompleted
  using ActionResultCallback = std::function<bool(
      const ActionResult& result, const ActionCompletedUnion& completionInfo)>;
  ...
  void HandleObservedObject(Robot& robot,
                            const ExternalInterface::RobotObservedObject& msg);
  void HandleRobotStopped(Robot& robot,
  ...
  void HandleDockingStatus(
      const AnkiEvent<RobotInterface::RobotToEngine>& message);

  void DelegateIfInControl(Robot& robot, IActionRunner* action,
  ...
  void EndAttempt(Robot& robot, ActionResult result, std::string name,
                  bool endingFromFailedAction = false);
