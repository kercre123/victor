 ...
 * Created: 2017-12-12
 *
 * Description: Dev behavior to use the touch sensor or backpack button to
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
static inline void SetRangeHelper(const Json::Value& config,
  ...
  LOG_ERROR("BehaviorDevImageCapture.SetRangeHelper.Failure",
            "Failed to set %s range", key);
}
...
BehaviorDevImageCapture::BehaviorDevImageCapture(const Json::Value& config)
  ...
  _iConfig.imageSavePath =
      JsonTools::ParseString(config, kSavePathKey, "BehaviorDevImageCapture");
  _iConfig.imageSaveQuality = JsonTools::ParseInt8(config, kImageSaveQualityKey,
                                                   "BehaviorDevImageCapture");
  JsonTools::GetValueOptional(config, kUseSavePrefixKey,
                              _iConfig.useSavePrefix);
  _iConfig.useCapTouch = JsonTools::ParseBool(config, kUseCapacitiveTouchKey,
                                              "BehaviorDevImageCapture");
  _iConfig.useShutterSound = JsonTools::ParseBool(config, kUseShutterSoundKey,
                                                  "BehaviorDevImageCapture");
  _iConfig.allowStreaming = JsonTools::ParseBool(config, kAllowStreamingKey,
                                                 "BehaviorDevImageCapture");
  std::string scaleStr =
      JsonTools::ParseString(config, kImageScaleKey, "BehaviorDevImageCapture");
  ...
  if (config.isMember(kMultiImageModeKey)) {
    ...
    _iConfig.numImagesPerCapture = JsonTools::ParseInt32(
        multiImageConfig, kNumImagesPerCaptureKey, "BehaviorDevImageCapture");
    SetRangeHelper(multiImageConfig, kDistanceRangeKey,
                   _iConfig.distanceRange_mm, false);
    SetRangeHelper(multiImageConfig, kHeadAngleRangeKey,
                   _iConfig.headAngleRange_rad, true);
    SetRangeHelper(multiImageConfig, kBodyAngleRangeKey,
                   _iConfig.bodyAngleRange_rad, true);
  }
  ...
  if (config.isMember(kVisionModesKey)) {
    ...
    auto setVisionModeHelper = [this](const Json::Value& jsonVisionMode) {
      ...
      if (success) {
        ...
        LOG_WARNING("BehaviorDevImageCapture.Constructor.InvalidVisionMode",
                    "%s", visionModeStr.c_str());
      }
    };

    if (visionModes.isArray()) {
      std::for_each(visionModes.begin(), visionModes.end(),
                    setVisionModeHelper);
...
void BehaviorDevImageCapture::InitBehavior() {
  if (_iConfig.useSavePrefix) {
    ...
    // thus be non-unique if the robot eventually talks to a time server
    // somewhere and updates itself after having already saved images.
    const auto context = GetBEI().GetRobotInfo().GetContext();
    const std::string prefixFileName =
        context->GetDataPlatform()->GetPersistentPath(
            "devImageCapturePrefix.txt");
    if (Util::FileUtils::FileExists(prefixFileName)) {
      ...
      LOG_INFO("BehaviorDevImageCapture.Constructor.UsingPrefix.", "%s",
               _iConfig.imageSavePrefix.c_str());
    }
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDevImageCapture::GetBehaviorOperationModifiers(
  ...
  for (auto const& mode : _iConfig.visionModesBesidesSaving) {
    ...
    modifiers.visionModesForActiveScope->insert(
        {mode, EVisionUpdateFrequency::Standard});
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDevImageCapture::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {
      kSavePathKey,           kImageSaveQualityKey, kImageScaleKey,
      kUseCapacitiveTouchKey, kSaveSensorDataKey,   kUseShutterSoundKey,
      kAllowStreamingKey,     kClassNamesKey,       kVisionModesKey,
      kMultiImageModeKey,     kUseSavePrefixKey,
  };
...
std::string BehaviorDevImageCapture::GetSavePath() const {
  ...
  if (_dVars.currentClassIter == _iConfig.classNames.end()) {
    return Util::FileUtils::FullFilePath(
        {cachePath, "images", _iConfig.imageSavePath});
  }

  return Util::FileUtils::FullFilePath(
      {cachePath, "images", _iConfig.imageSavePath, *_dVars.currentClassIter});
}
...
void BehaviorDevImageCapture::BehaviorUpdate() {
  ...
  const bool haveCompletedCapture =
      (_dVars.imagesSaved >= _iConfig.numImagesPerCapture);
  if (!_dVars.isStreaming && doingMultiImageCapture && haveStartedCapture &&
      !haveCompletedCapture) {
    // Ignore additional button presses / touches while we're in the process of
    // taking multiple images at different poses
    return;
  }
  ...
  if (_dVars.currentClassIter != _iConfig.classNames.end()) {
    ...
    const size_t numFiles =
        FileUtils::FilesInDirectory(
            GetSavePath(), false,
            ImageSaver::GetExtension(_iConfig.imageSaveQuality))
            .size();
  ...
  if (wasTouched && !isTouched) {
    // just "released", see if it's been long enough to count as a "hold" for
    // toggling streaming
    ImageSendMode sendMode = ImageSendMode::Off;
    if (_iConfig.allowStreaming &&
        (currTime_s >= _dVars.touchStartedTime_s + kHoldTimeForStreaming_s)) {
      LOG_DEBUG("BehaviorDevImageCapture.touch.longPress",
                "long press release");
      ...
      sendMode =
          (_dVars.isStreaming ? ImageSendMode::Stream : ImageSendMode::Off);

      if (_dVars.isStreaming) {
        BlinkLight();
      }
    } else {
      LOG_DEBUG("BehaviorDevImageCapture.touch.shortPress",
                "short press release");
      // take single photo
      _dVars.imagesSaved = 0;
      sendMode =
          (_iConfig.saveSensorData ? ImageSendMode::SingleShotWithSensorData
    ...
    if (_dVars.isStreaming) {
      ...
      ImageSaverParams params(GetSavePath(), ImageSendMode::Off,
...
void BehaviorDevImageCapture::BlinkLight() {
  ...
  GetBEI().GetBackpackLightComponent().SetBackpackAnimation(
      _dVars.blinkOn ? kLightsOn : kLightsOff);

  // always blink if we are streaming, and set up another blink for single photo
  // if we need to turn off the light
...
void BehaviorDevImageCapture::MoveToNewPose() {
  ...
  // For each valid range (where the max > min), compute a random angle/distance
  // and create a compound action to perform the movements. Note that head and
  // body turns can be done in parallel. We then do the body movement
  // _afterward_. This could be streamlined a bit to handle special cases that
  // we have, for example just a head and drive action (but no body turn
  // action), since we could also run those in parallel. But I'm not gonna
  ...
  if (Util::IsFltLT(_iConfig.headAngleRange_rad.first,
                    _iConfig.headAngleRange_rad.second)) {
    const f32 headAngleDelta_rad = GetRNG().RandDblInRange(
        _iConfig.headAngleRange_rad.first, _iConfig.headAngleRange_rad.second);
    const f32 headAngle_rad = Util::Clamp(
        (_dVars.startingHeadAngle_rad + headAngleDelta_rad).ToFloat(),
        MIN_HEAD_ANGLE, MAX_HEAD_ANGLE);
    turnAction->AddAction(new MoveHeadToAngleAction(headAngle_rad));
    LOG_DEBUG("BehaviorDevImageCapture.MoveToNewPose.HeadAngle", "%.1fdeg",
              RAD_TO_DEG(headAngle_rad));
  }

  if (Util::IsFltLT(_iConfig.bodyAngleRange_rad.first,
                    _iConfig.bodyAngleRange_rad.second)) {
    const f32 bodyAngleDelta_rad = GetRNG().RandDblInRange(
        _iConfig.bodyAngleRange_rad.first, _iConfig.bodyAngleRange_rad.second);
    const f32 bodyAngle_rad =
        (_dVars.startingBodyAngle_rad + bodyAngleDelta_rad).ToFloat();
    const bool isAbsolute = true;  // using absolute because we want to be
                                   // relative to starting angle, not current
    turnAction->AddAction(new TurnInPlaceAction(bodyAngle_rad, isAbsolute));
    LOG_DEBUG(
        "BehaviorDevImageCapture.MoveToNewPose.BodyAngle",
  ...
  if (Util::IsFltLT(_iConfig.distanceRange_mm.first,
                    _iConfig.distanceRange_mm.second)) {
    const f32 dist_mm = GetRNG().RandDblInRange(
        _iConfig.distanceRange_mm.first, _iConfig.distanceRange_mm.second);
    const bool kPlayAnim = false;
    action->AddAction(new DriveStraightAction(
        dist_mm, DEFAULT_PATH_MOTION_PROFILE.speed_mmps, kPlayAnim));
    LOG_DEBUG("BehaviorDevImageCapture.MoveToNewPose.Distance", "%.1fmm",
              dist_mm);
  }

  DelegateIfInControl(action, [this]() {
    const ImageSendMode sendMode =
        (_iConfig.saveSensorData ? ImageSendMode::SingleShotWithSensorData
                                 : ImageSendMode::SingleShot);
    SaveImages(sendMode);
  });
...
void BehaviorDevImageCapture::SaveImages(const ImageSendMode sendMode) {
  // To help avoid duplicate images names when combining images from multiple
  // robots on multiple runs of this behavior, use a basename built from the
  // robot's serial number and the seconds since epoch. If multiple images are
  // being captured, a counter is appended as well. Note that for streaming, a
  // frame number will also be appended by the ImageSaver because all saved
  // images will share the same basename (since they comes from the same button
  // press).
  using namespace std::chrono;
  const auto time_sec =
      duration_cast<seconds>(system_clock::now().time_since_epoch()).count();
  std::string basename =
      (_iConfig.imageSavePrefix + _iConfig.serialNumber + "_" +
       _iConfig.buildSha + "_" + std::to_string(time_sec));
  if (_iConfig.numImagesPerCapture > 1) {
    basename += "_" + std::to_string(_dVars.imagesSaved);
  }

  // Tell VisionComponent to save an image
  ImageSaverParams params(GetSavePath(), sendMode, _iConfig.imageSaveQuality,
                          basename, _iConfig.imageSaveSize);

  // Special case: if class name is FalsePositive or FalseNegative, set up save
  // conditions to
  //  only save image at the right time
  if (!_iConfig.visionModesBesidesSaving.empty() &&
      _dVars.currentClassIter != _iConfig.classNames.end()) {
    ...
    if (kFalsePositiveStr == currentClass) {
      for (const auto visionMode : _iConfig.visionModesBesidesSaving) {
        params.saveConditions[visionMode] =
            ImageSaverParams::SaveConditionType::OnDetection;
      }
    } else if (kFalseNegativeStr == currentClass) {
      for (const auto visionMode : _iConfig.visionModesBesidesSaving) {
        params.saveConditions[visionMode] =
            ImageSaverParams::SaveConditionType::NoDetection;
      }
    }
  }
  ...
  if (!_dVars.isStreaming) {
    WaitForImagesAction* waitAction =
        new WaitForImagesAction(1, VisionMode::SaveImages);
    ...
    DelegateIfInControl(waitAction, [this]() {
      ...
      if (_dVars.imagesSaved == 1) {
        // After the first image is taken, store starting angles for each button
        // press so we can move to new positions relative to that
        _dVars.startingBodyAngle_rad =
