 ...
 * Created: 2018-01-18
 *
 * Description: Simple behavior to wait for the robot to be shaken and placed
 *down before delegating to another data-defined behavior
 *
...
// set to > 0 from console to fake the repeated "shakes" (shakes are hard to do
// in webots sim)
...
// if the robot is put down and no behavior wants to be active after this many
// ticks, the shake count is reset
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorDispatchAfterShake::BehaviorDispatchAfterShake(
    const Json::Value& config)
    : ICozmoBehavior(config) {
  for (const auto& behavior : config[kBehaviorsKey]) {
    _iConfig.delegateIDs.push_back(
        BehaviorTypesWrapper::BehaviorIDFromString(behavior.asString()));
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDispatchAfterShake::GetBehaviorJsonKeys(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDispatchAfterShake::GetAllDelegates(
    std::set<IBehavior*>& delegates) const {
  // copy _iConfig.delegates into delegates, taking the raw ptrs
  std::transform(_iConfig.delegates.begin(), _iConfig.delegates.end(),
...
void BehaviorDispatchAfterShake::BehaviorUpdate() {
...
#endif  // REMOTE_CONSOLE_ENABLED
  {
    ...
    const bool isBeingShaken = (robotInfo.GetHeadAccelMagnitudeFiltered() >
                                kAccelMagnitudeShakingStartedThreshold);
    ...
    const bool timeElapsed =
        (currentTime - _dVars.lastChangeTime_s >= kShakeTime);
    ...
    // shaking is one of those words where if you write it enough times it
    // starts to look wrong.
  }

  const bool isOnTreads =
      (robotInfo.GetOffTreadsState() == OffTreadsState::OnTreads);
  if ((_dVars.countShaken > 0) && (!_dVars.shakingSession) && isOnTreads) {
    EngineTimeStamp_t currTimeStamp =
    ...
    if (_dVars.countShaken > _iConfig.delegates.size()) {
      PRINT_NAMED_WARNING(
          "BehaviorDispatchAfterShake.BehaviorUpdate.TooManyShakes",
          "You shook the robot (%zu) times but there were only (%zu) behaviors",
          _dVars.countShaken, _iConfig.delegates.size());
      idx = std::min(idx, _iConfig.delegates.size() - 1);
    }

    DEV_ASSERT(idx < _iConfig.delegates.size(),
               "BehaviorDispatchAfterShake.Update.OutOfRange");
    auto& delegate = _iConfig.delegates[idx];
    if (ANKI_VERIFY(delegate != nullptr,
                    "BehaviorDispatchAfterShake.Update.NullDelegate",
                    "Behavior idx (%zu) is null", idx)) {
      if (delegate->WantsToBeActivated()) {
        DelegateIfInControl(delegate.get());
        // clear shaken count so you have to shake again to run the behavior
        // again if it completes
        _dVars.countShaken = 0;
        _dVars.tickPlacedDown = 0;
      } else if (currTimeStamp >
                 _dVars.tickPlacedDown + kFailTicksBeforeReset) {
        // start over
        _dVars.countShaken = 0;
        _dVars.tickPlacedDown = 0;
        PRINT_NAMED_WARNING(
            "BehaviorDispatchAfterShake.BehaviorUpdate.NoWant",
