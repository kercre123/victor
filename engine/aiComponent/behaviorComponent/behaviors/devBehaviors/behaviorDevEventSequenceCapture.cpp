 ...
 *
 * Usage:       After pressing the touch sensor or button, the robot will play a
 *sound and wait for a specified setup time, during which the user should
 *prepare to generate the event. After the setup time, the robot will play a
 *sound and save images for the pre-event capture time. After this time, the
 *robot will play a sound to notify the user to generate the event, and will
 *continue saving images for the post-event capture time. Finally the robot will
 *play a sound to signal the end of the sequence.
 *
 *              The screen will mirror the image and display the number of
 *sequences stored for the current event class. Sequences carry over across
 *restarts and will not be overwritten. The class can be changed by moving the
 *robot lift up and down.
 *
 * Storage:     Sequences are stored as numbered folders in
 *<cache>/images/<class>/. The numbers begin with 0 and are unpadded. Sequence
 *images are saved in their respective folders along with their JSON sidecars
 *and a sequenceInfo.json that contains the sequence start, event, and end
 *timestamps.
 *
 *
...
BehaviorDevEventSequenceCapture::BehaviorDevEventSequenceCapture(
  ...
  _iConfig.imageSavePath = JsonTools::ParseString(
      config, kSavePathKey, "BehaviorDevEventSequenceCapture");
  _iConfig.imageSaveQuality = JsonTools::ParseInt8(
      config, kImageSaveQualityKey, "BehaviorDevEventSequenceCapture");
  _iConfig.useCapTouch = JsonTools::ParseBool(
      config, kUseCapacitiveTouchKey, "BehaviorDevEventSequenceCapture");

  std::string scaleStr = JsonTools::ParseString(
      config, kImageScaleKey, "BehaviorDevEventSequenceCapture");
  _iConfig.imageSaveSize = Vision::ImageCache::StringToSize(scaleStr);

  _iConfig.sequenceSetupTime = JsonTools::ParseFloat(
      config, kSequenceSetupTimeKey, "BehaviorDevEventSequenceCapture");
  _iConfig.preEventCaptureTime = JsonTools::ParseFloat(
      config, kPreEventCaptureTimeKey, "BehaviorDevEventSequenceCapture");
  _iConfig.postEventCaptureTime = JsonTools::ParseFloat(
      config, kPostEventCaptureTimeKey, "BehaviorDevEventSequenceCapture");

  _iConfig.enableRandomHeadTilt = JsonTools::ParseBool(
      config, kEnableRandomHeadTiltKey, "BehaviorDevEventSequenceCapture");
  if (_iConfig.enableRandomHeadTilt) {
    _iConfig.minHeadTilt = MIN_HEAD_ANGLE;
    _iConfig.maxHeadTilt = MAX_HEAD_ANGLE;
    if (JsonTools::GetValueOptional(config, kMinHeadTiltKey,
                                    _iConfig.minHeadTilt)) {
      _iConfig.minHeadTilt = DEG_TO_RAD(_iConfig.minHeadTilt);
    }
    if (JsonTools::GetValueOptional(config, kMaxHeadTiltKey,
                                    _iConfig.maxHeadTilt)) {
  ...
  if (config.isMember(kClassNamesKey)) {
    ...
    if (classNames.isArray()) {
      ...
      PRINT_NAMED_WARNING(
          "BehaviorDevEventSequenceCapture.Constructor.InvalidClassNames", "");
    }
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDevEventSequenceCapture::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kSavePathKey,
...
std::string BehaviorDevEventSequenceCapture::GetRelClassSavePath() const {
  ...
  return Util::FileUtils::FullFilePath(
      {_iConfig.imageSavePath, *_dVars.currentClassIter});
}
...
std::string BehaviorDevEventSequenceCapture::GetRelSequenceSavePath() const {
  return Util::FileUtils::FullFilePath(
      {GetRelClassSavePath(), std::to_string(_dVars.currentSeqNumber)});
}
...
std::string BehaviorDevEventSequenceCapture::GetAbsSequenceSavePath() const {
  return Util::FileUtils::FullFilePath(
      {GetAbsBaseSavePath(), GetRelSequenceSavePath()});
}
...
std::string BehaviorDevEventSequenceCapture::GetAbsInfoSavePath() const {
  return Util::FileUtils::FullFilePath(
      {GetAbsSequenceSavePath(), "sequenceInfo.json"});
}
...
std::string BehaviorDevEventSequenceCapture::GetAbsBaseSavePath() const {
  const std::string cachePath =
      GetBEI().GetRobotInfo().GetContext()->GetDataPlatform()->pathToResource(
...
int32_t BehaviorDevEventSequenceCapture::GetNumCurrentSequences() const {
  ...
  std::string absClassSavePath = Util::FileUtils::FullFilePath(
      {GetAbsBaseSavePath(), GetRelClassSavePath()});
...
RobotTimeStamp_t BehaviorDevEventSequenceCapture::GetTimestamp() const {
  // NOTE We're clocking ourselves using the image timestamps to make sure
  // sequence info is on the same clock This should be fixed when a proper wall
  // time implementation is added
...
void BehaviorDevEventSequenceCapture::BehaviorUpdate() {
  ...
  const bool isTouched =
      (_iConfig.useCapTouch ? GetBEI().GetTouchSensorComponent().GetIsPressed()
                            : GetBEI().GetRobotInfo().IsPowerButtonPressed());
  const bool isLiftUp =
      (GetBEI().GetRobotInfo().GetLiftHeight() > LIFT_HEIGHT_HIGHDOCK);
  ...
  const std::string str(*_dVars.currentClassIter + ":" +
                        std::to_string(numCurrentSeqs));
  ...
  switch (_dVars.seqState) {
    case SequenceState::Waiting: {
      ...
      if (_dVars.wasTouched && !isTouched) {
        ...
        if (_iConfig.enableRandomHeadTilt) {
          double headAngle = GetBEI().GetRobotInfo().GetRNG().RandDblInRange(
              _iConfig.minHeadTilt, _iConfig.maxHeadTilt);
        ...
        GetBEI().GetRobotAudioClient().PostEvent(
            GE::Play__Robot_Vic_Sfx__Lift_High_Down_Long_Effort, GO::Behavior);
        GetBEI().GetBackpackLightComponent().SetBackpackAnimation(kLightsSetup);
        PRINT_CH_DEBUG("Behaviors",
                       "BehaviorDevEventSequenceCapture.startSequence",
    ...
    case SequenceState::Setup: {
      ...
      if (waitTime_s > _iConfig.sequenceSetupTime) {
        ...
        ImageSaverParams params(GetAbsSequenceSavePath(), ImageSendMode::Stream,
                                _iConfig.imageSaveQuality, "",
                                _iConfig.imageSaveSize);
        visionComponent.SetSaveImageParameters(params);
        GetBEI().GetRobotAudioClient().PostEvent(
            GE::Play__Robot_Vic_Sfx__Timer_Countdown, GO::Behavior);
        GetBEI().GetBackpackLightComponent().SetBackpackAnimation(
            kLightsPreCap);
        PRINT_CH_DEBUG("Behaviors",
                       "BehaviorDevEventSequenceCapture.setupSequence",
    ...
    case SequenceState::PreEventCapture: {
      ...
      if (waitTime_s > _iConfig.preEventCaptureTime) {
        ...
        GetBEI().GetRobotAudioClient().PostEvent(
            GE::Play__Robot_Vic_Sfx__Timer_Beep, GO::Behavior);
        GetBEI().GetBackpackLightComponent().SetBackpackAnimation(
            kLightsPostCap);
        PRINT_CH_DEBUG("Behaviors",
                       "BehaviorDevEventSequenceCapture.preCapSequence",
    ...
    case SequenceState::PostEventCapture: {
      ...
      if (waitTime_s > _iConfig.postEventCaptureTime) {
        ...
        ImageSaverParams params(GetAbsSequenceSavePath(), ImageSendMode::Off,
                                _iConfig.imageSaveQuality,
                                "",  // No basename: use auto-numbering
                                _iConfig.imageSaveSize);
        visionComponent.SetSaveImageParameters(params);
        GetBEI().GetRobotAudioClient().PostEvent(
            GE::Play__Robot_Vic_Sfx__Timer_Cancel, GO::Behavior);
        GetBEI().GetBackpackLightComponent().SetBackpackAnimation(
            kLightsWaiting);
        PRINT_CH_DEBUG("Behaviors",
                       "BehaviorDevEventSequenceCapture.endSequence",
    ...
    default: {
      PRINT_NAMED_ERROR("DevEventSequenceCapture.SequenceStateError",
                        "Unrecognized sequence state!");
      _dVars.seqState = SequenceState::Waiting;
    }
  }
