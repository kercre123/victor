...
CONSOLE_VAR_RANGED(float, kLiftMovementDuration_s, CONSOLE_NAME, 0.5f, 0.1f,
                   2.5f);
CONSOLE_VAR_RANGED(float, kHeadMovementDuration_s, CONSOLE_NAME, 0.5f, 0.1f,
                   2.5f);
CONSOLE_VAR_RANGED(float, kTreadMovementSpeed_mmps, CONSOLE_NAME, 200.0f, 20.0f,
                   MAX_WHEEL_SPEED_MMPS);
CONSOLE_VAR_ENUM(uint8_t, kLoopingAnimationState, CONSOLE_NAME, 0,
                 "NONE, Move Head, Move Lift");
CONSOLE_VAR(bool, kUseRestrictedMotionAnim, CONSOLE_NAME, true);

#define MOTOR_ACTION_ASSERT \
  ASSERT_NAMED(_robot != nullptr, "BehaviorDevSquawkBoxTest._robot.IsNull")
...
void BehaviorDevSquawkBoxTest::SetupConsoleFuncs() {
  if (ANKI_DEV_CHEATS) {
    auto toggleLiftOnOff = [this](ConsoleFunctionContextRef context) {
      ...
      MoveLift();
      const auto* msg =
          _dVars._isLiftMoving ? "Start Lift Movement" : "Stop Lift Movment";
    ...
    auto toggleHeadOnOff = [this](ConsoleFunctionContextRef context) {
      ...
      MoveHead();
      const auto* msg =
          _dVars._isHeadMoving ? "Start Head Movement" : "Stop Head Movment";
      context->channel->WriteLog(" %s", msg);
    };
    auto toggleTreadOnOff = [this](ConsoleFunctionContextRef context) {
      ...
      MoveTreads();
      const auto* msg = _dVars._isMovingTreads ? "Start Tread Movement"
                                               : "Stop Tread Movment";
      context->channel->WriteLog(" %s", msg);
    };
    auto setHeadAngle = [this](ConsoleFunctionContextRef context) {
      ...
      const float angleRad =
          Util::Clamp(DEG_TO_RAD(angleDeg), MIN_HEAD_ANGLE, MAX_HEAD_ANGLE);
    ...
    _consoleFuncs.emplace_front("ToggleLiftOnOff", std::move(toggleLiftOnOff),
                                CONSOLE_NAME, "");
    _consoleFuncs.emplace_front("ToggleHeadOnOff", std::move(toggleHeadOnOff),
                                CONSOLE_NAME, "");
    _consoleFuncs.emplace_front("ToggleTreadsOnOff",
                                std::move(toggleTreadOnOff), CONSOLE_NAME, "");
    _consoleFuncs.emplace_front("SetHeadAngle", std::move(setHeadAngle),
                                CONSOLE_NAME, "float angleDeg");
    _consoleFuncs.emplace_front("SetLoopingAnimationState",
                                std::move(setAnimationState), CONSOLE_NAME, "");
  }
}
...
void BehaviorDevSquawkBoxTest::InitBehavior() {
  if (ANKI_DEV_CHEATS) {
    ...
    // Robot -> Engine
    SubscribeToTags({RobotInterface::RobotToEngineTag::triggerWordDetected});
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDevSquawkBoxTest::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDevSquawkBoxTest::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  // const char* list[] = {
  //   // NOTE: insert any possible root-level json keys that this class is
  //   expecting.
  //   // NOTE: replace this method with a simple {} in the header if this class
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDevSquawkBoxTest::HandleWhileActivated(
...
void BehaviorDevSquawkBoxTest::HandleWhileActivated(
  ...
  if (event.GetData().GetTag() ==
    ...
    PRINT_CH_INFO(
        "Behaviors",
        "nameBehaviorDevSquawkBoxTest.HandleWhileActivated.RobotToEngineEvent",
...
void BehaviorDevSquawkBoxTest::OnBehaviorActivated() {
  ...
  SmartPushResponseToTriggerWord(AnimationTrigger::Count, postAudioEvent,
...
void BehaviorDevSquawkBoxTest::BehaviorUpdate() {
  if (IsActivated()) {
    ...
    if (_dVars._isMovingTreads &&
        (kTreadMovementSpeed_mmps != _dVars._currentTreadSpeed)) {
      MoveTreads();
    }
  }
}
...
void BehaviorDevSquawkBoxTest::MoveLift() {
  ...
  auto* compoundSeq =
      new CompoundActionSequential({liftUpAction, liftDownAction});
...
void BehaviorDevSquawkBoxTest::MoveHead() {
  ...
  auto* headUpAction =
      new MoveHeadToAngleAction(DEG_TO_RAD(42), DEG_TO_RAD(10.0f));
  headUpAction->SetDuration(kHeadMovementDuration_s);

  auto* headDownAction =
      new MoveHeadToAngleAction(DEG_TO_RAD(-20), DEG_TO_RAD(10.0f));
  headDownAction->SetDuration(kHeadMovementDuration_s);

  auto* compoundSeq =
      new CompoundActionSequential({headUpAction, headDownAction});
...
void BehaviorDevSquawkBoxTest::MoveTreads() {
  ...
  _dVars._currentTreadSpeed =
      _dVars._isMovingTreads ? kTreadMovementSpeed_mmps : 0;
  auto& moveComp = _robot->GetMoveComponent();
  moveComp.DriveWheels(_dVars._currentTreadSpeed, _dVars._currentTreadSpeed,
                       0.f, 0.f);
}
...
std::string BehaviorDevSquawkBoxTest::SetLoopingAnimationState() {
  ...
  switch (kLoopingAnimationState) {
    case 1:
      animName = kUseRestrictedMotionAnim ? "anim_qa_head_updown_alt"
                                          : "anim_qa_head_updown";
      break;

    case 2:
      animName = kUseRestrictedMotionAnim ? "anim_qa_lift_updown_alt"
                                          : "anim_qa_lift_updown";
      break;
