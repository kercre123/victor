...
const char* const kShouldDriveStraightIfNotOnCharger =
    "shouldDriveStraightIfNotOnCharger";
...
BehaviorReactToBody::InstanceConfig::InstanceConfig(const Json::Value& config) {
  const std::string& debugName =
      "BehaviorReactToBody.InstanceConfig.LoadConfig";
  moveOffChargerBehaviorStr =
      JsonTools::ParseString(config, kDriveOffChargerBehaviorKey, debugName);
  shouldDriveStraightWhenBodyDetected = JsonTools::ParseBool(
      config, kShouldDriveStraightIfNotOnCharger, debugName);
  drivingForwardDistance =
      JsonTools::ParseFloat(config, kDrivingForwardDistance, debugName);
  upperPortionLookUpPercent =
      JsonTools::ParseFloat(config, kUpperPortionLookUpPercent, debugName);
  upperPortionLookUpPercent =
      Util::Clamp(upperPortionLookUpPercent, 0.0f, 1.0f);
  trackingTimeout = JsonTools::ParseFloat(config, kTrackingTimeout, debugName);
  const bool parsedOk = FaceSelectionComponent::ParseFaceSelectionFactorMap(
      config[kFaceSelectionPenaltiesKey], faceSelectionCriteria);
  DEV_ASSERT(parsedOk,
             "BehaviorReactToBody.InstanceConfig.InstanceConfig."
             "MissingFaceSelectionPenalties");
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorReactToBody::WantsToBeActivatedBehavior() const { return true; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToBody::InitBehavior() {
  const auto& BC = GetBEI().GetBehaviorContainer();
  _iConfig.moveOffChargerBehavior =
      BC.FindBehaviorByID(BehaviorTypesWrapper::BehaviorIDFromString(
          _iConfig.moveOffChargerBehaviorStr));
  ...
  const bool result = BC.FindBehaviorByIDAndDowncast(
      BEHAVIOR_ID(SearchWithinBoundingBox),
      BEHAVIOR_CLASS(SearchWithinBoundingBox), _iConfig.searchBehavior);
  DEV_ASSERT(result, "BehaviorSearchWithinBoundingBoxNotFound");
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToBody::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToBody::GetAllDelegates(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToBody::GetBehaviorJsonKeys(
...
void BehaviorReactToBody::OnBehaviorActivated() {
  ...
  LOG_DEBUG("BehaviorReactToPersonDetected.OnBehaviorActivated",
            "I am active!");

  // don't look for faces in the past
  _dVars.imageTimestampWhenActivated =
      GetBEI().GetRobotInfo().GetLastImageTimeStamp();

  // no need to start a ruckus if we already see a face
  const bool faceFound =
      GetBEI().GetFaceWorld().HasAnyFaces(_dVars.imageTimestampWhenActivated);
  if (faceFound) {
    LOG_DEBUG("BehaviorReactToPersonDetected.OnBehaviorActivated.FaceFound",
              "Face found, no need to search");
  ...
  component.GetSalientPointSinceTime(latestPersons,
                                     Vision::SalientPointType::Person,
                                     _dVars.persistent.lastSeenTimeStamp);

  if (latestPersons.empty()) {
    LOG_DEBUG(
        "BehaviorReactToPersonDetected.OnBehaviorActivated.NoPersonDetected",
        "Activated but no person with a timestamp > %u",
        (TimeStamp_t)_dVars.persistent.lastSeenTimeStamp);
  ...
  auto maxElementIter = std::max_element(
      latestPersons.begin(), latestPersons.end(),
  ...
  DEV_ASSERT(
      _dVars.lastPersonDetected.salientType == Vision::SalientPointType::Person,
      "BehaviorReactToPersonDetected.OnBehaviorActivated."
      "LastSalientPointMustBePerson");
...
void BehaviorReactToBody::BehaviorUpdate() {
  ...
  if (_dVars.searchingForFaces) {
    const bool faceFound =
        GetBEI().GetFaceWorld().HasAnyFaces(_dVars.imageTimestampWhenActivated);
    if (faceFound) {
      LOG_DEBUG("BehaviorReactToPersonDetected.BehaviorUpdate.FaceFound",
                "Face found, cancelling search");
    ...
    if (!_iConfig.searchBehavior->IsActivated()) {
      LOG_DEBUG(
          "BehaviorReactToPersonDetected.BehaviorUpdate."
          "SearchBehaviorNotActive",
...
void BehaviorReactToBody::TransitionToStart() {
  ...
  if (GetBEI().GetRobotInfo().IsOnChargerContacts()) {
    DelegateIfInControl(_iConfig.moveOffChargerBehavior.get(), [this]() {
      if (GetBEI().GetRobotInfo().IsOnChargerContacts()) {
        LOG_WARNING(
            "BehaviorReactToBody.TransitionToStart.CouldntMoveOffCharger",
...
void BehaviorReactToBody::TransitionToMaybeGoStraightAndLookUp() {
    ...
    if (!_dVars.droveOffCharger) {  // no need to drive straight twice
      ...
      if (proxData.foundObject) {
        if (proxData.distance_mm < _iConfig.drivingForwardDistance) {
          LOG_DEBUG(
              "BehaviorReactToBody.TransitionToMaybeGoStraightAndLookUp."
              "ObstacleClose",
              "Can't go straight, an obstacle is at %d while the driving "
              "distance would be %f",
          ...
          LOG_DEBUG(
              "BehaviorReactToBody.TransitionToMaybeGoStraightAndLookUp."
              "ProxSensorData",
              "Distance is %u", proxData.distance_mm);
          shouldGoStraight = true;
        }
      } else {
        LOG_DEBUG(
            "BehaviorReactToBody.TransitionToMaybeGoStraightAndLookUp."
            "NoValidProxSensorData",
            "");
        shouldGoStraight = false;  // redundant, but it makes things clear
      }
    }
  ...
  if (shouldGoStraight) {
    action = new CompoundActionSequential(
        {new DriveStraightAction(_iConfig.drivingForwardDistance),
...
void BehaviorReactToBody::TransitionToLookForFace() {
  ...
  LOG_DEBUG("BehaviorReactToBody.TransitionToLookForFace.BoundingBox",
            "The search bounding box is: x: (%f, %f), y: (%f, %f)", minX, maxX,
            minY, maxY);
  ...
  if (_iConfig.searchBehavior->WantsToBeActivated()) {
    ...
    DelegateIfInControl(_iConfig.searchBehavior.get(),
                        &BehaviorReactToBody::TransitionToCompleted);
  } else {
    PRINT_NAMED_WARNING(
        "BehaviorReactToBody.TransitionToLookForFace.SearchBehaviorNotReady",
...
void BehaviorReactToBody::TransitionToFaceFound() {
  ...
  if (!ANKI_VERIFY(!_dVars.actingOnFaceFound,  // using double not for clarity
                                               // of intention
                   "BehaviorReactToBody.TransitionToFaceFound."
                   "ActingOnFaceFoundShouldBeFalse",
                   "")) {
    TransitionToCompleted();
    return;
  }
  ...
  if (bestFace.IsValid()) {
    ...
    LOG_WARNING("BehaviorReactToBody.TransitionToFaceFound.NoValidFace",
                "No valid face found for tracking");
    TransitionToCompleted();
  }
}
