...
std::pair<std::string, std::string> BehaviorDevTouchDataCollection::ToString(
    DataAnnotation a) const {
  ...
  auto got = mm.find(a);
  DEV_ASSERT(got != mm.end(),
             "DevTouchDataCollection.DataAnnotation.MissingMapping");
  return got->second;
}

std::pair<std::string, std::string> BehaviorDevTouchDataCollection::ToString(
    CollectionMode cm) const {
  ...
  auto got = mm.find(cm);
  DEV_ASSERT(got != mm.end(),
             "DevTouchDataCollection.CollectionMode.MissingMapping");
  return got->second;
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
BehaviorDevTouchDataCollection::BehaviorDevTouchDataCollection(
    const Json::Value& config)
    : ICozmoBehavior(config) {
  // listening for raw touch input data
  SubscribeToTags({RobotInterface::RobotToEngineTag::state});
}

void BehaviorDevTouchDataCollection::HandleWhileActivated(
  ...
  switch (event.GetData().GetTag()) {
    ...
    default: {
      PRINT_NAMED_WARNING(
          "BehaviorDevTouchDataCollection.HandleWhileRunning."
          "InvalidRobotToEngineTag",
...
void BehaviorDevTouchDataCollection::EnqueueMotorActions() {
  ...
  ActionResultCallback arc =
      std::bind(&BehaviorDevTouchDataCollection::LoopMotorsActionCallback, this,
                std::placeholders::_1);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool BehaviorDevTouchDataCollection::RobotConfigMatchesExpected(
    BehaviorExternalInterface& bexi) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void BehaviorDevTouchDataCollection::LoopMotorsActionCallback(
    ActionResult res) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void BehaviorDevTouchDataCollection::OnNextDataset(
    BehaviorExternalInterface& bexi, CollectionMode cm,
  ...
  std::string outputPath =
      bexi.GetRobotInfo().GetDataPlatform()->pathToResource(
          Util::Data::Scope::Cache, "touchDataCollection");
  outputPath += "/";
  outputPath += serialno;
  ...
  PRINT_CH_INFO("Behaviors", "WriteToFile", "%zd values to %s",
                _dVars.touchValues.size(), filePath.c_str());

  if (Util::FileUtils::FileExists(filePath)) {
    PRINT_NAMED_WARNING("BehaviorDevTouchDataCollection.FileAlreadyExists",
                        "Overwriting %s", filePath.c_str());
  }
  ...
  if (!Util::FileUtils::FileExists(filePath)) {
    PRINT_NAMED_WARNING("BehaviorDevTouchDataCollection.FailedToWrite", "%s",
                        filePath.c_str());
  }
}
