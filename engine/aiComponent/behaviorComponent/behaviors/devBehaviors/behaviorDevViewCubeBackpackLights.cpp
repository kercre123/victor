 ...
 * Created: 2018-06-24
 *
 * Description: A behavior which reads in a list of cube/backpack lights and
 *exposes console vars to move through them
 *
...
BehaviorDevViewCubeBackpackLights::BehaviorDevViewCubeBackpackLights(
  ...
  if (ANKI_VERIFY(
          config[kCubeTriggersKey].isArray(),
          "BehaviorDevViewCubeBackpackLights.Constructor.CubeTriggersNotArray",
          "")) {
    for (const auto& trigger : config[kCubeTriggersKey]) {
      _iConfig.cubeTriggers.push_back(
          CubeAnimationTriggerFromString(trigger.asString()));
    }
  }

  if (ANKI_VERIFY(config[kBackpackTriggersKey].isArray(),
                  "BehaviorDevViewCubeBackpackLights.Constructor."
                  "BackpackTriggersNotArray",
                  "")) {
    for (const auto& trigger : config[kBackpackTriggersKey]) {
      _iConfig.backpackTriggers.push_back(
          BackpackAnimationTriggerFromString(trigger.asString()));
    }
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDevViewCubeBackpackLights::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDevViewCubeBackpackLights::GetAllDelegates(
    std::set<IBehavior*>& delegates) const {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDevViewCubeBackpackLights::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {kCubeTriggersKey, kBackpackTriggersKey};
...
void BehaviorDevViewCubeBackpackLights::BehaviorUpdate() {
  ...
  if (kBackpackTriggerIdx != _dVars.lastBackpackTriggerIdx) {
    const auto validIdx =
        std::min(kBackpackTriggerIdx,
                 static_cast<u32>(_iConfig.backpackTriggers.size() - 1));
    GetBEI().GetBackpackLightComponent().SetBackpackAnimation(
        _iConfig.backpackTriggers[validIdx]);
  ...
  if (!_dVars.objectID.IsSet()) {
    ...
    const auto* obj =
        GetBEI().GetBlockWorld().FindConnectedMatchingBlock(filter);
  ...
  if (kCubeTriggerIdx != _dVars.lastCubeTriggerIdx) {
    const auto validIdx = std::min(
        kCubeTriggerIdx, static_cast<u32>(_iConfig.cubeTriggers.size() - 1));
    ...
    if (_dVars.lastCubeTrigger == CubeAnimationTrigger::Count) {
      GetBEI().GetCubeLightComponent().PlayLightAnimByTrigger(
          _dVars.objectID, cubeTrigger, animCompleteCallback);
    } else {
      GetBEI().GetCubeLightComponent().StopAndPlayLightAnim(
          _dVars.objectID, _dVars.lastCubeTrigger, cubeTrigger,
