 ...
 * Created: 2018-05-03
 *
 * Description: Upon spotting cube(s), the robot drives to one of them, then to
 *a point far from it, then back again, forever. Point the cube lights in some
 *direction for the robot to follow that direction. Otherwise, the planner
 *choses the side of the cube (and the cube, if there are multiple cubes),
 *although it will stick to that selected pose near the cube for the remainder
 *of the behavior. To reset, pick up the robot. The robot will use TTS to convey
 *success/fail for each step
 *
...
CONSOLE_VAR_RANGED(float, kCubeDistance_mm, "BehaviorPlannerTest", 25.0f, 0.0f,
                   100.0f);
CONSOLE_VAR_RANGED(float, kDistance_mm, "BehaviorPlannerTest", 1000.0f, 0.0f,
                   2000.0f);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPlannerTest::GetBehaviorOperationModifiers(
...
void BehaviorPlannerTest::OnBehaviorActivated() {
  // DON'T reset dynamic variables when resuming, in case mandatory physical
  // reactions interrupt
}
...
bool BehaviorPlannerTest::FindPoseFromCube() {
  ...
  for (const auto* object : objects) {
    ...
    if (verticalMarker.GetCode() == markerTop.GetCode() ||
        verticalMarker.GetCode() == markerBottom.GetCode()) {
    ...
    for (const auto* pose : poses) {
      ...
      _dVars.cubePoses.emplace_back(
          M_PI_2, Z_AXIS_3D(), Point3f{0.f, -kCubeDistance_mm, -halfHeight},
          *pose);
    }
  }
...
void BehaviorPlannerTest::GoToCubePose() {
  ...
  DelegateIfInControl(action, [this](const ActionResult& res) {
    // if we havent done so already, remove all but the selected goal pose by
    // the cube
    if (_dVars.cubePoses.size() > 1) {
      auto indexPtr =
          GetBEI().GetRobotInfo().GetPathComponent().GetLastSelectedIndex();
      if (indexPtr) {
    ...
    if (IActionRunner::GetActionResultCategory(res) ==
        ActionResultCategory::RETRY) {
      DelegateIfInControl(
          new SayTextAction(
              "CUBE RETRY",
      ...
      DelegateIfInControl(new SayTextAction(
          "CUBE FAIL", SayTextAction::AudioTtsProcessingStyle::Unprocessed));
    } else {
      DelegateIfInControl(new SayTextAction(
          "CUBE SUCCESS", SayTextAction::AudioTtsProcessingStyle::Unprocessed));
    }
  });
}
...
void BehaviorPlannerTest::GoToFarAwayPose() {
  ...
  CalcFarAwayPose();
  DelegateIfInControl(
      new DriveToPoseAction(_dVars.drivePose), [this](const ActionResult& res) {
        if (IActionRunner::GetActionResultCategory(res) ==
            ActionResultCategory::RETRY) {
          DelegateIfInControl(
              new SayTextAction(
