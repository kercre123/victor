...
BehaviorSelfTestDockWithCharger::BehaviorSelfTestDockWithCharger(
...
Result BehaviorSelfTestDockWithCharger::OnBehaviorActivatedInternal() {
  ...
  if (objects.empty()) {
    PRINT_NAMED_WARNING(
        "BehaviorSelfTestDockWithCharger.GetExpectedObjectMarkerPoseWrtRobot."
        "NullObject",
    ...
    DriveToAndMountChargerAction* action = new DriveToAndMountChargerAction(
        object->GetID(), useCliffSensorCorrection, enableDockingAnims,
...
void BehaviorSelfTestDockWithCharger::TransitionToOnChargerChecks() {
  ...
  const bool disconnected =
      robot.GetBatteryComponent().IsBatteryDisconnectedFromCharger();
  const bool batteryVoltageGood =
      batteryVolts >= SelfTestConfig::kMinBatteryVoltage;
  ...
  if (!disconnected && !batteryVoltageGood) {
    PRINT_NAMED_WARNING(
        "BehaviorSelfTestDockWithCharger.OnActivated.BatteryTooLow", "%fv",
        batteryVolts);
  ...
  if (!chargerVoltageGood) {
    PRINT_NAMED_WARNING(
        "BehaviorSelfTestDockWithCharger.OnActivated.ChargerTooLow", "%fv",
        chargerVolts);
