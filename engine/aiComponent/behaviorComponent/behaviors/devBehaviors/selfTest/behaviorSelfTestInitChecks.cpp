...
BehaviorSelfTestInitChecks::BehaviorSelfTestInitChecks(
...
Result BehaviorSelfTestInitChecks::OnBehaviorActivatedInternal() {
  ...
  DriveStraightAction* drive =
      new DriveStraightAction(-SelfTestConfig::kDriveBackwardsDist_mm,
  ...
  // the drive action will run forever. Add in some wait actions to cancel the
  // drive after some amount of time
  CompoundActionParallel* action = new CompoundActionParallel();
  const bool ignoreFailure = true;
  std::weak_ptr<IActionRunner> drivePtr =
      action->AddAction(drive, ignoreFailure);

  WaitAction* wait = new WaitAction(SelfTestConfig::kDriveBackwardsTime_sec);
  WaitForLambdaAction* cancel =
      new WaitForLambdaAction([drivePtr](Robot& robot) {
...
void BehaviorSelfTestInitChecks::TransitionToChecks() {
  ...
  // Check that raw touch values are in expected range (the range assumes no
  // touch)
  const u16 rawTouchValue =
      robot.GetTouchSensorComponent().GetLatestRawTouchValue();
  if (!Util::InRange(rawTouchValue, SelfTestConfig::kMinExpectedTouchValue,
                     SelfTestConfig::kMaxExpectedTouchValue)) {
    PRINT_NAMED_WARNING("BehaviorSelfTestInitChecks.OnActivated.TouchOOR",
                        "Min %u < Val %u < Max %u",
                        SelfTestConfig::kMinExpectedTouchValue, rawTouchValue,
                        SelfTestConfig::kMaxExpectedTouchValue);
  ...
  // Make sure we are considered on the charger and charging (or not charging
  // because battery is disconnected)
  if (onCharger) {
    if (isCharging && isDisconnected) {
      ...
      SELFTEST_SET_RESULT(
          SelfTestResultCode::ON_CHARGER_NOT_CHARGING_OR_DISCONNECTED);
    }

    const bool battVoltGood =
        (battComp.GetBatteryVoltsRaw() > SelfTestConfig::kMinBatteryVoltage);
  ...
  const bool chargerVoltGood =
      (battComp.GetChargerVoltsRaw() > SelfTestConfig::kMinChargerVoltage);
  if (!chargerVoltGood) {
