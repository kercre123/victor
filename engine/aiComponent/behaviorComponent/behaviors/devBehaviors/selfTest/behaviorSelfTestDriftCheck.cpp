...
BehaviorSelfTestDriftCheck::BehaviorSelfTestDriftCheck(
...
Result BehaviorSelfTestDriftCheck::OnBehaviorActivatedInternal() {
  ...
  MoveHeadToAngleAction* moveHeadToAngle =
      new MoveHeadToAngleAction(SelfTestConfig::kHeadAngleForDriftCheck);
...
void BehaviorSelfTestDriftCheck::TransitionToStartDriftCheck() {
  ...
  AddTimer(SelfTestConfig::kIMUDriftDetectPeriod_ms,
           [this]() { CheckDrift(); });
}
...
void BehaviorSelfTestDriftCheck::CheckDrift() {
  ...
  if (angleChange > SelfTestConfig::kIMUDriftAngleThreshDeg) {
    PRINT_NAMED_WARNING(
        "BehaviorSelfTestDriftCheck.CheckDrift.DriftDetected",
        "Angle change of %f deg detected in %f seconds", angleChange,
        Util::MilliSecToSec((float)SelfTestConfig::kIMUDriftDetectPeriod_ms));
...
BehaviorSelfTestDriftCheck::SelfTestUpdateInternal() {
