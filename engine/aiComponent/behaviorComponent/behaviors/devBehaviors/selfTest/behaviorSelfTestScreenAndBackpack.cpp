 ...
 * Created: 11/16/2018
 *
 * Description: Displays the same color on the backpack and screen to test
 *lights
 *
...
BehaviorSelfTestScreenAndBackpack::BehaviorSelfTestScreenAndBackpack(
...
Result BehaviorSelfTestScreenAndBackpack::OnBehaviorActivatedInternal() {
  if (!DidReceiveFFTResult()) {
    SELFTEST_SET_RESULT_WITH_RETURN_VAL(SelfTestResultCode::NO_FFT_RESULT,
                                        RESULT_FAIL);
  }
  ...
  DriveStraightAction* action = new DriveStraightAction(
      SelfTestConfig::kDistToDriveOffCharger_mm,
...
void BehaviorSelfTestScreenAndBackpack::TransitionToButtonCheck() {
  ...
  if (onCharger) {
    PRINT_NAMED_WARNING(
        "BehaviorSelfTestScreenAndBackpack.TransitionToButtonCheck."
        "StillOnCharger",
        "");
  ...
  DrawTextOnScreen(robot,
                   {"Press button if screen", "and all four", "lights match"},
                   SelfTestConfig::kTextColor, SelfTestConfig::kColorCheck);

  static const BackpackLightAnimation::BackpackAnimation lights = {
      .onColors = {{SelfTestConfig::kColorCheck, SelfTestConfig::kColorCheck,
                    SelfTestConfig::kColorCheck}},
      .offColors = {{SelfTestConfig::kColorCheck, SelfTestConfig::kColorCheck,
                     SelfTestConfig::kColorCheck}},
  ...
  robot.SendRobotMessage<RobotInterface::SetSystemLight>(
      std::move(systemLight));
...
BehaviorSelfTestScreenAndBackpack::SelfTestUpdateInternal() {
  // DEPRECATED - Grabbing robot to support current cozmo code, but this should
  // be removed
  Robot& robot = GetBEI().GetRobotInfo()._robot;
  ...
  if (buttonReleased && !_buttonStartedPressed) {
    // Button was pressed indicating the screen and backpack lights look good,
    // so drive back onto charger
    DriveStraightAction* drive =
        new DriveStraightAction(SelfTestConfig::kDistToDriveOnCharger_mm,
                                SelfTestConfig::kDriveBackwardsSpeed_mmps);
    drive->SetCanMoveOnCharger(true);

    // Driving backwards on the charger does not update the robot's position so
    // the drive action will run forever. Add in a wait action to cancel the
    // drive once we are one the charger
    CompoundActionParallel* action = new CompoundActionParallel();

    // Once the below wait action completes, it will also cause the drive action
    // to complete
...
void BehaviorSelfTestScreenAndBackpack::OnBehaviorDeactivated() {
  ...
  robot.GetBackpackLightComponent().SetBackpackAnimation(
      BackpackLightAnimation::BackpackAnimation());
