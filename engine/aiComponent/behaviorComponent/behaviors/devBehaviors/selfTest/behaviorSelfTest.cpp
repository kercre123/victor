...
void BehaviorSelfTest::InitBehavior() {
  ...
  ICozmoBehaviorPtr putOnChargerBehavior =
      BC.FindBehaviorByID(BehaviorID::SelfTestPutOnCharger);
  DEV_ASSERT(
      putOnChargerBehavior != nullptr &&
          putOnChargerBehavior->GetClass() ==
              BehaviorClass::SelfTestPutOnCharger,
      "BehaviorSelfTest.ImproperClassRetrievedForName.SelfTestPutOnCharger");
  _selfTestBehaviors.push_back(
      std::static_pointer_cast<IBehaviorSelfTest>(putOnChargerBehavior));

  ICozmoBehaviorPtr buttonBehavior =
      BC.FindBehaviorByID(BehaviorID::SelfTestButton);
  ...
  _selfTestBehaviors.push_back(
      std::static_pointer_cast<IBehaviorSelfTest>(buttonBehavior));

  ICozmoBehaviorPtr initChecksBehavior =
      BC.FindBehaviorByID(BehaviorID::SelfTestInitChecks);
  DEV_ASSERT(
      initChecksBehavior != nullptr &&
  ...
  _selfTestBehaviors.push_back(
      std::static_pointer_cast<IBehaviorSelfTest>(initChecksBehavior));

  ICozmoBehaviorPtr motorCalibrationBehavior =
      BC.FindBehaviorByID(BehaviorID::SelfTestMotorCalibration);
  DEV_ASSERT(motorCalibrationBehavior != nullptr &&
                 motorCalibrationBehavior->GetClass() ==
                     BehaviorClass::SelfTestMotorCalibration,
             "BehaviorSelfTest.ImproperClassRetrievedForName."
             "SelfTestMotorCalibration");
  _selfTestBehaviors.push_back(
      std::static_pointer_cast<IBehaviorSelfTest>(motorCalibrationBehavior));

  ICozmoBehaviorPtr driftCheckBehavior =
      BC.FindBehaviorByID(BehaviorID::SelfTestDriftCheck);
  DEV_ASSERT(
      driftCheckBehavior != nullptr &&
  ...
  _selfTestBehaviors.push_back(
      std::static_pointer_cast<IBehaviorSelfTest>(driftCheckBehavior));

  ICozmoBehaviorPtr soundCheckBehavior =
      BC.FindBehaviorByID(BehaviorID::SelfTestSoundCheck);
  DEV_ASSERT(
      soundCheckBehavior != nullptr &&
  ...
  _selfTestBehaviors.push_back(
      std::static_pointer_cast<IBehaviorSelfTest>(soundCheckBehavior));

  ICozmoBehaviorPtr driveFowardsBehavior =
      BC.FindBehaviorByID(BehaviorID::SelfTestDriveForwards);
  DEV_ASSERT(
      driveFowardsBehavior != nullptr &&
          driveFowardsBehavior->GetClass() ==
              BehaviorClass::SelfTestDriveForwards,
      "BehaviorSelfTest.ImproperClassRetrievedForName.SelfTestDriveForwards");
  _selfTestBehaviors.push_back(
      std::static_pointer_cast<IBehaviorSelfTest>(driveFowardsBehavior));

  ICozmoBehaviorPtr lookAtCharger =
      BC.FindBehaviorByID(BehaviorID::SelfTestLookAtCharger);
  DEV_ASSERT(
      lookAtCharger != nullptr &&
  ...
  _selfTestBehaviors.push_back(
      std::static_pointer_cast<IBehaviorSelfTest>(lookAtCharger));

  ICozmoBehaviorPtr dockWithCharger =
      BC.FindBehaviorByID(BehaviorID::SelfTestDockWithCharger);
  DEV_ASSERT(
      dockWithCharger != nullptr &&
  ...
  _selfTestBehaviors.push_back(
      std::static_pointer_cast<IBehaviorSelfTest>(dockWithCharger));

  ICozmoBehaviorPtr pickupBehavior =
      BC.FindBehaviorByID(BehaviorID::SelfTestPickup);
  ...
  _selfTestBehaviors.push_back(
      std::static_pointer_cast<IBehaviorSelfTest>(pickupBehavior));

  ICozmoBehaviorPtr putOnChargerBehavior2 =
      BC.FindBehaviorByID(BehaviorID::SelfTestPutOnCharger2);
  DEV_ASSERT(
      putOnChargerBehavior2 != nullptr &&
          putOnChargerBehavior2->GetClass() ==
              BehaviorClass::SelfTestPutOnCharger,
      "BehaviorSelfTest.ImproperClassRetrievedForName.SelfTestPutOnCharger2");
  _selfTestBehaviors.push_back(
      std::static_pointer_cast<IBehaviorSelfTest>(putOnChargerBehavior2));

  ICozmoBehaviorPtr touchBehavior =
      BC.FindBehaviorByID(BehaviorID::SelfTestTouch);
  ...
  _selfTestBehaviors.push_back(
      std::static_pointer_cast<IBehaviorSelfTest>(touchBehavior));

  ICozmoBehaviorPtr screenAndBackpackBehavior =
      BC.FindBehaviorByID(BehaviorID::SelfTestScreenAndBackpack);
  DEV_ASSERT(screenAndBackpackBehavior != nullptr &&
                 screenAndBackpackBehavior->GetClass() ==
                     BehaviorClass::SelfTestScreenAndBackpack,
             "BehaviorSelfTest.ImproperClassRetrievedForName."
             "SelfTestScreenAndBackpack");
  _selfTestBehaviors.push_back(
      std::static_pointer_cast<IBehaviorSelfTest>(screenAndBackpackBehavior));
  ...
  if (robot.HasExternalInterface()) {
    auto helper =
        MakeAnkiEventUtil(*robot.GetExternalInterface(), *this, _signalHandles);
    helper.SubscribeEngineToGame<
        ExternalInterface::MessageEngineToGameTag::SelfTestBehaviorFailed>();
  }
}
...
void BehaviorSelfTest::OnBehaviorActivated() {
  ...
  robot.GetDrivingAnimationHandler().PushDrivingAnimations(anims,
                                                           GetDebugLabel());
  ...
  // Copy from string into fixed sized array, taking care to only copy what we
  // need or what we have space for msg.ssid.size()-1 to account for null
  // terminator in fixed sized array
  memcpy(&msg.ssid, SelfTestConfig::kWifiSSID.data(),
         std::min(msg.ssid.size() - 1, SelfTestConfig::kWifiSSID.size()));
  memcpy(&msg.pwd, SelfTestConfig::kWifiPwd.data(),
         std::min(msg.pwd.size() - 1, SelfTestConfig::kWifiPwd.size()));
  ...
  DASMSG(behavior_self_test_start, "behavior.self_test_start",
         "Self test has started");
  DASMSG_SEND();
}
...
void BehaviorSelfTest::OnBehaviorDeactivated() {
  ...
  // Reapply current settings, this is to put robot volume back to user set
  // level
...
void BehaviorSelfTest::BehaviorUpdate() {
  ...
  if (_currentBehavior != nullptr &&
    ...
    if (_currentSelfTestBehaviorIter != _selfTestBehaviors.end()) {
      ...
      _currentBehavior = *_currentSelfTestBehaviorIter;
      DEV_ASSERT(_currentBehavior != nullptr,
                 "BehaviorSelfTest.BehaviorUpdate.NullCurrentBehavior");

      if (!_currentBehavior->WantsToBeActivated()) {
        PRINT_NAMED_ERROR(
            "BehaviorSelfTest.ChooseNextBehaviorInternal.BehaviorNotRunnable",
    ...
    // All self test behaviors have run so success!
      ...
      PRINT_NAMED_INFO("BehaviorSelfTest.Complete",
                       "All behaviors have been run");
...
void BehaviorSelfTest::HandleResult(SelfTestResultCode result) {
  PRINT_NAMED_INFO("BehaviorSelfTest.HandleResult.OrigResult", "%s",
                   EnumToString(result));

  const auto& allResults = IBehaviorSelfTest::GetAllSelfTestResults();

  // If this is a success but we are ignoring failures a behavior may have
  // actually failed so check all results
  if (result == SelfTestResultCode::SUCCESS &&
      SelfTestConfig::kIgnoreFailures) {
    for (const auto& resultPair : allResults) {
      for (const auto& res : resultPair.second) {
        if (res != SelfTestResultCode::SUCCESS) {
          result = res;
          PRINT_NAMED_INFO(
              "BehaviorSelfTest.HandleResultInternal.ChangingResult",
  ...
  PRINT_NAMED_INFO("BehaviorSelfTest.HandleResultInternal.Result",
                   "Playpen completed with %s", EnumToString(result));
  ...
  DASMSG(behavior_self_test_end, "behavior.self_test_end",
         "Self test has completed");
  DASMSG_SET(s1, testPassed ? "success" : "failure", "Test passed or failed");
  DASMSG_SET(s2, testPassed ? "" : EnumToString(result),
...
void BehaviorSelfTest::DisplayResult(SelfTestResultCode result) {
  ...
  if (result == SelfTestResultCode::SUCCESS) {
    static const BackpackLightAnimation::BackpackAnimation passLights = {
        .onColors = {{NamedColors::GREEN, NamedColors::GREEN,
                      NamedColors::GREEN}},
        .offColors = {{NamedColors::BLACK, NamedColors::BLACK,
                       NamedColors::BLACK}},
    ...
    IBehaviorSelfTest::DrawTextOnScreen(
        robot, {"OK", "Press button", "to finish test"}, NamedColors::BLACK,
        NamedColors::GREEN);
  } else {
    // I would like to play this animation here but it will cause the face to go
    // blank so the user would not be able to see the result text
    ...
    IBehaviorSelfTest::DrawTextOnScreen(
        robot, {std::to_string((u32)result), "", "Press button", "to end test"},
        NamedColors::BLACK, NamedColors::RED);
  }
}

template <>
void BehaviorSelfTest::HandleMessage(
    const ExternalInterface::SelfTestBehaviorFailed& msg) {
...
void BehaviorSelfTest::HandleWhileActivated(const GameToEngineEvent& event) {
  ...
  // for normal users this is expected as the wifi checks for a hardcoded
  // network
  if (payload.status_code != 0) {
    PRINT_NAMED_INFO(
        "BehaviorSelfTest.HandleWifiConnectResponse",
