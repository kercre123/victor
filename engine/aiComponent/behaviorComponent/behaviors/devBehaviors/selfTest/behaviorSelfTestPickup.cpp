 ...
 * Created: 11/16/2018
 *
 * Description: Prompts user to pickup robot to check cliff sensor while upside
 *down
 *
...
Result BehaviorSelfTestPickup::OnBehaviorActivatedInternal() {
  ...
  DrawTextOnScreen(robot, {"Pickup Robot", "Turn Upside Down"});

  return RESULT_OK;
}

IBehaviorSelfTest::SelfTestStatus
BehaviorSelfTestPickup::SelfTestUpdateInternal() {
  // DEPRECATED - Grabbing robot to support current cozmo code, but this should
  // be removed
  Robot& robot = GetBEI().GetRobotInfo()._robot;
  ...
  if (isBeingHeld) {
    ...
    if (accel.z <= SelfTestConfig::kUpsideDownZAccel) {
      if (!_upsideDownTimerAdded) {
        ...
        AddTimer(
            SelfTestConfig::kUpsideDownTimeout_ms,
            [this]() {
      ...
      if (_upsideDownStartTime_ms == 0) {
        ...
        DrawTextOnScreen(robot, {"Please Wait"}, NamedColors::WHITE,
                         NamedColors::BLACK, 180);
      }
      // Have been upside down for long enough, check cliff sensors
      else if (now - _upsideDownStartTime_ms >
               SelfTestConfig::kTimeToBeUpsideDown_ms) {
        ...
        if (cliffSenseComp.IsCliffDetected(CliffSensor::CLIFF_FL) &&
          ...
          // All cliff sensors should be seeing cliffs and have values below a
          // threshold
          const auto& cliffData = cliffSenseComp.GetCliffDataRaw();
          bool allCliffs = true;
          for (int c = 0; c < CliffSensorComponent::kNumCliffSensors; c++) {
            allCliffs |=
                (cliffData[c] < SelfTestConfig::kUpsideDownCliffValThresh);
          }
      ...
      if (_upsideDownStartTime_ms != 0) {
        DrawTextOnScreen(robot, {"Pickup Robot", "Turn Upside Down"},
                         NamedColors::WHITE, NamedColors::BLACK, 0);
      }
