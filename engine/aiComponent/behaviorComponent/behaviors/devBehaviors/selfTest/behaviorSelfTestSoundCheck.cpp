...
BehaviorSelfTestSoundCheck::BehaviorSelfTestSoundCheck(
...
Result BehaviorSelfTestSoundCheck::OnBehaviorActivatedInternal() {
  ...
  MoveHeadToAngleAction* head =
      new MoveHeadToAngleAction(SelfTestConfig::kHeadAngleToPlaySound);
  MoveLiftToHeightAction* lift =
      new MoveLiftToHeightAction(LIFT_HEIGHT_LOWDOCK);

  CompoundActionParallel* liftAndHead =
      new CompoundActionParallel({head, lift});
...
void BehaviorSelfTestSoundCheck::TransitionToPlayingSound() {
  ...
  robot.GetExternalInterface()
      ->BroadcastToEngine<ExternalInterface::SetRobotVolume>(
          SelfTestConfig::kSoundVolume);

  // Start recording mic audio of the sound and run an FFT on the audio to check
  // that we actually heard the sound we played
  const bool runFFT = true;
  robot.SendMessage(
      RobotInterface::EngineToRobot(RobotInterface::StartRecordingMicsRaw(
          SelfTestConfig::kDurationOfAudioToRecord_ms, runFFT, "/data/beep")));

  CompoundActionSequential* action = new CompoundActionSequential(
      {new PlayAnimationAction("soundTestAnim"),
       new WaitAction(Util::MilliSecToSec(
           (float)SelfTestConfig::kDurationOfAudioToRecord_ms))});

  DelegateIfInControl(action, [this, &robot]() {
    DrawTextOnScreen(robot, {"Test Running"});
...
void BehaviorSelfTestSoundCheck::AlwaysHandleInScope(
  ...
  if (tag == RobotInterface::RobotToEngineTag::audioFFTResult) {
    ...
    for (u8 i = 0; i < payload.result.size(); ++i) {
      ...
      PRINT_NAMED_INFO("BehaviorSelfTestDriftCheck.HandleAudioFFTResult.Result",
                       "FFT result for channel %u : %u", i, fftResult);

      // Check that the most prominent frequency heard by this mic is
      // near the expected frequency
      if (!Util::IsNear((float)fftResult, SelfTestConfig::kFFTExpectedFreq_hz,
                        SelfTestConfig::kFFTFreqTolerance_hz)) {
        ++count;
        res = channelToMic[i];
        PRINT_NAMED_WARNING(
            "BehaviorSelfTestDriftCheck.HandleAudioFFTResult.FFTFailed",
            "%s picked up freq %u which is outside %u +/- %u",
            EnumToString(res), fftResult, SelfTestConfig::kFFTExpectedFreq_hz,
            SelfTestConfig::kFFTFreqTolerance_hz);
      }
    }
    ...
    if (count == payload.result.size()) {
      ...
      PRINT_NAMED_WARNING(
          "BehaviorSelfTestDriftCheck.HandleAudioFFTResult.Speaker",
          "No mics picked up expected frequency %u, assuming speaker is not "
          "working",
    ...
    if (!robot.IsPhysical()) {
      PRINT_NAMED_DEBUG(
          "BehaviorSelfTestDriftCheck.HandleAudioFFTResult.SimulatedRobot",
    ...
    if (res != SelfTestResultCode::UNKNOWN) {
      ...
      const_cast<Robot&>(robot).Broadcast(
          MessageEngineToGame(SelfTestBehaviorFailed(res)));
    }
  }
}
