...
BehaviorSelfTestPutOnCharger::BehaviorSelfTestPutOnCharger(
...
Result BehaviorSelfTestPutOnCharger::OnBehaviorActivatedInternal() {
  ...
  // as this behavior runs after BehaviorSelfTestPickup so the robot is likely
  // upside down
  float textAngle = 0;
  ...
  DrawTextOnScreen(robot, {"Put on charger"}, NamedColors::WHITE,
                   NamedColors::BLACK, textAngle);

  return RESULT_OK;
}

IBehaviorSelfTest::SelfTestStatus
BehaviorSelfTestPutOnCharger::SelfTestUpdateInternal() {
  // DEPRECATED - Grabbing robot to support current cozmo code, but this should
  // be removed
  Robot& robot = GetBEI().GetRobotInfo()._robot;
