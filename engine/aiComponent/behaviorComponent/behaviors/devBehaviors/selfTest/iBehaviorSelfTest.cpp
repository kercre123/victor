 ...
 *              All SelfTest behaviors should be written to be able to continue
 *even after receiving unexpected things (basically conditional branches should
 *only contain code that calls SET_RESULT)
 *
...
// Hacky way of giving any selftest behavior easy access to check something that
...
// behaviorSelfTestSoundCheck should be setting this bool
static bool receivedFFTResult = false;
}  // namespace

IBehaviorSelfTest::IBehaviorSelfTest(const Json::Value& config,
                                     SelfTestResultCode timeoutCode)
    : ICozmoBehavior(config), _timeoutCode(timeoutCode) {
...
bool IBehaviorSelfTest::WantsToBeActivatedBehavior() const { return true; }
...
bool IBehaviorSelfTest::DelegateIfInControl(IActionRunner* action,
...
bool IBehaviorSelfTest::DelegateIfInControl(IActionRunner* action,
  ...
  auto callbackWrapper = [this, callback](ActionResult result) {
    if (result != ActionResult::SUCCESS) {
      PRINT_NAMED_WARNING("IBehaviorSelfTest.ActionFailed",
...
bool IBehaviorSelfTest::DelegateIfInControl(
...
const std::map<std::string, std::vector<SelfTestResultCode>>&
IBehaviorSelfTest::GetAllSelfTestResults() {
  return results;
}

void IBehaviorSelfTest::ResetAllSelfTestResults() { results.clear(); }

const std::set<ExternalInterface::MessageEngineToGameTag>&
IBehaviorSelfTest::GetFailureTags() {
  return kFailureTags;
}

void IBehaviorSelfTest::ReceivedFFTResult() { receivedFFTResult = true; }
...
void IBehaviorSelfTest::DrawTextOnScreen(Robot& robot,
                                         ...
                                         ColorRGBA textColor, ColorRGBA bg,
                                         float rotate_deg) {
  ...
  // Figure out the minimum text size we need such that all lines of text will
  // fit on the display
  for (const auto& t : text) {
    cv::Size size;
    float scale;

    Vision::Image::MakeTextFillImageWidth(t, font, thickness, img.GetNumCols(),
                                          size, scale);
  ...
  if (text.size() == 1) {
    ...
    offset = img.GetNumRows() / 2 -
             (std::ceil(text.size() / 2.f) * minSize.height / 2);
  }
  ...
  for (auto iter = text.begin(); iter != text.end(); ++iter) {
    ...
    img.DrawTextCenteredHorizontally(*iter, font, scale, textThickness,
                                     textColor, offset, false);
  ...
  if (!Util::IsNearZero(rotate_deg)) {
    const auto M = cv::getRotationMatrix2D(
        {(float)img.GetNumCols() / 2, (float)img.GetNumRows() / 2}, rotate_deg,
        1);
    cv::warpAffine(img.get_CvMat_(), img.get_CvMat_(), M,
