...
BehaviorSelfTestButton::BehaviorSelfTestButton(const Json::Value& config)
  ...
  // We sometimes detect pickup/InAir when the button is pressed while on the
  // charger due to bouncy charge contacts
...
BehaviorSelfTestButton::SelfTestUpdateInternal() {
  // DEPRECATED - Grabbing robot to support current cozmo code, but this should
  // be removed
  Robot& robot = GetBEI().GetRobotInfo()._robot;
  ...
  if (buttonReleased && !_buttonStartedPressed) {
    ...
    if (onCharger) {
      ...
      // and think we are back on treads as the charger contacts can be quite
      // bouncy
      DrawTextOnScreen(robot, {"Please Wait"});

      WaitToBeOnTreads();
    } else {
      SELFTEST_SET_RESULT_WITH_RETURN_VAL(
          SelfTestResultCode::CHARGER_UNDETECTED, SelfTestStatus::Failure);
    }
  }
...
void BehaviorSelfTestButton::WaitToBeOnTreads() {
  ...
  if (robot.GetOffTreadsState() == OffTreadsState::OnTreads) {
    ...
    AddTimer(500, [this]() { WaitToBeOnTreads(); });
  }
}
