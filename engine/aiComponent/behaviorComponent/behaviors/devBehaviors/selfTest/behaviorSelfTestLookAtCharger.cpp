...
BehaviorSelfTestLookAtCharger::BehaviorSelfTestLookAtCharger(
  ...
  if (!res) {
    PRINT_NAMED_ERROR(
        "BehaviorSelfTestLookAtCharger.Constructor.MissingConfigKey",
...
void BehaviorSelfTestLookAtCharger::GetBehaviorJsonKeys(
...
Result BehaviorSelfTestLookAtCharger::OnBehaviorActivatedInternal() {
  ...
  if (_expectedObjectType == ObjectType::UnknownObject) {
    PRINT_NAMED_WARNING(
        "BehaviorSelfTestLookAtCharger.InternalInitInternal.NoObject",
        "No object type specified to look for, failing");
    SELFTEST_SET_RESULT_WITH_RETURN_VAL(SelfTestResultCode::CHARGER_NOT_FOUND,
                                        RESULT_FAIL);
  }
  ...
  DriveStraightAction* drive =
      new DriveStraightAction(_distToDrive_mm, 60, false);
...
BehaviorSelfTestLookAtCharger::SelfTestUpdateInternal() {
  // DEPRECATED - Grabbing robot to support current cozmo code, but this should
  // be removed
  Robot& robot = GetBEI().GetRobotInfo()._robot;
  ...
  // We have distance readings left to record
    ...
    if (!Util::IsNear(data.proxDistanceToTarget_mm - bias,
      ...
      SELFTEST_SET_RESULT_WITH_RETURN_VAL(
          SelfTestResultCode::DISTANCE_SENSOR_OOR, SelfTestStatus::Running);
    }
...
void BehaviorSelfTestLookAtCharger::TransitionToRefineTurn() {
  ...
  if (res) {
    ...
    if (!Util::IsNear(
      ...
      PRINT_NAMED_WARNING(
          "BehaviorSelfTestLookAtCharger.TransitionToRefineTurn.MarkerOOR",
    ...
    PRINT_NAMED_INFO(
        "BehaviorSelfTestLookAtCharger.TransitionToRefineTurn.TurnAngle",
...
bool BehaviorSelfTestLookAtCharger::GetExpectedObjectMarkerPoseWrtRobot(
    Pose3d& markerPoseWrtRobot, ObjectID& objectID) {
  // DEPRECATED - Grabbing robot to support current cozmo code, but this should
  // be removed
  Robot& robot = GetBEI().GetRobotInfo()._robot;
  ...
  if (objects.empty()) {
    PRINT_NAMED_WARNING(
        "BehaviorSelfTestLookAtCharger.GetExpectedObjectMarkerPoseWrtRobot."
        "NullObject",
        "Expected object of type %s does not exist or am not seeing it",
        ObjectTypeToString(_expectedObjectType));
    SELFTEST_SET_RESULT_WITH_RETURN_VAL(
        SelfTestResultCode::DISTANCE_MARKER_NOT_FOUND, false);
    return false;
  } else {
    ...
    if ((u32)robot.GetLastImageTimeStamp() - lastObservedTime >
        SelfTestConfig::kChargerMarkerLastObservedTimeThresh_ms) {
      PRINT_NAMED_INFO(
          "BehaviorSelfTestLookAtCharger.GetExpectedObjectMarkerPoseWrtRobot."
          "MarkerTooOld",
          "%u - %u > %u", (u32)robot.GetLastImageTimeStamp(), lastObservedTime,
          SelfTestConfig::kChargerMarkerLastObservedTimeThresh_ms);
      return false;
    }
