 ...
 *              All self test behaviors should be written to be able to continue
  ...
  {                                                                         \
    if (!ShouldIgnoreFailures() || result == SelfTestResultCode::SUCCESS) { \
      ...
      PRINT_NAMED_WARNING("IBehaviorSelfTest.IgnoringFailure",              \
                          ...
                          EnumToString(result), GetDebugLabel().c_str());   \
...
#define SELFTEST_TRY(expr, result) \
  {                                \
    if (!(expr)) {                 \
      SELFTEST_SET_RESULT(result); \
    }                              \
  }
...
class IBehaviorSelfTest : public ICozmoBehavior {
 ...
 public:
  ...
  // Will be UNKNOWN while running and either SUCCESS or something else when
  // complete
  ...
  static const std::map<std::string, std::vector<SelfTestResultCode>>&
  GetAllSelfTestResults();
  static void ResetAllSelfTestResults();

  static const std::set<ExternalInterface::MessageEngineToGameTag>&
  GetFailureTags();
 ...
 protected:
  ...
  enum SelfTestStatus { Failure, Running, Complete };
  ...
  // Final override of OnBehaviorActivated so we can do things before the
  // subclass inits OnBehaviorActivatedInternal() is provided for the subclass
  // to override
  virtual void OnBehaviorActivated() override final;

  // Final override of UpdateInternal so we can do things before the subclass
  // updates SelfTestUpdateInternal() is provided for the subclass to override
  // if they wish
  virtual void BehaviorUpdate() override final;

  // Final override of HandleWhileActivated so we can do things before the
  // subclass handles the event HandleWhileActivatedInternal() is provided for
  // the subclass to override
  virtual void HandleWhileActivated(
      const EngineToGameEvent& event) override final;
  virtual void HandleWhileActivated(
      const RobotToEngineEvent& event) override final;

  // Hides IBehavior's SubscribeToTags function so we can do some bookkeeping on
  // what the subclass is subscribing to
  void SubscribeToTags(
      std::set<EngineToGameTag>&& tags);  // Hide base class function
  void SubscribeToTags(std::set<GameToEngineTag>&& tags);

  // Hides some of ICozmoBehavior's various DelegateIfInControl functions so we
  // can inject callbacks that automatically print warnings/failures if actions
  // fail
  bool DelegateIfInControl(
      IActionRunner* action,
      SimpleCallback callback);  // Hide base class function
  bool DelegateIfInControl(
      IActionRunner* action,
      ActionResultCallback callback);  // Hide base class function
  bool DelegateIfInControl(
      IActionRunner* action,
      RobotCompletedActionCallback callback);  // Hide base class function
  ...
  virtual SelfTestStatus SelfTestUpdateInternal() {
  ...
  // Use the macro SELFTEST_SET_RESULT if you can instead of directly calling
  // this function
  ...
  // Will make all lines the same size such that the longest can fit on the
  // screen Rotates the image by rotate_deg before displaying Note: An empty
  // string in the text vector has special behavior
  //       It will cause the element immediately before it to be drawn across
  //       multiple lines Ex: {"36", "", "Test Failed"}, Text will be drawn on
  //       "3" lines. "36" will be drawn across the first two and "Test Failed"
  //       will be drawn on the third
  static void DrawTextOnScreen(Robot& robot,
  ...
  class Timer {
   public:
    Timer(TimeStamp_t time_ms, std::function<void(void)> callback,
          const std::string& name = "")
        : _time_ms(BaseStationTimer::getInstance()->GetCurrentTimeStamp() +
                   time_ms),
          _callback(callback),
          _name(name) {}
    ...
    void Tick() {
      if (BaseStationTimer::getInstance()->GetCurrentTimeStamp() > _time_ms &&
        ...
        // Nullify callback so it will only be called once since timers are
        // still alive/exist even after _time_ms has been reached
        _callback = nullptr;
      }
    }
