...
CONSOLE_VAR_RANGED(u32, kLED1_s1_transition_s2_ms, CONSOLE_GROUP, 0, 0,
                   MAX_SEG_LEN);
CONSOLE_VAR_RANGED(u32, kLED1_s2_transition_s1_ms, CONSOLE_GROUP, 0, 0,
                   MAX_SEG_LEN);
CONSOLE_VAR_RANGED(u32, kLED1_s1_hold_offset_ms, CONSOLE_GROUP, 0, 0,
                   MAX_SEG_LEN * 4);
...
CONSOLE_VAR_RANGED(u32, kLED2_s1_transition_s2_ms, CONSOLE_GROUP, 0, 0,
                   MAX_SEG_LEN);
CONSOLE_VAR_RANGED(u32, kLED2_s2_transition_s1_ms, CONSOLE_GROUP, 0, 0,
                   MAX_SEG_LEN);
CONSOLE_VAR_RANGED(u32, kLED2_s1_hold_offset_ms, CONSOLE_GROUP, 0, 0,
                   MAX_SEG_LEN * 4);
...
CONSOLE_VAR_RANGED(u32, kLED3_s1_transition_s2_ms, CONSOLE_GROUP, 0, 0,
                   MAX_SEG_LEN);
CONSOLE_VAR_RANGED(u32, kLED3_s2_transition_s1_ms, CONSOLE_GROUP, 0, 0,
                   MAX_SEG_LEN);
CONSOLE_VAR_RANGED(u32, kLED3_s1_hold_offset_ms, CONSOLE_GROUP, 0, 0,
                   MAX_SEG_LEN * 4);
...
CONSOLE_VAR_RANGED(u32, kLED4_s1_transition_s2_ms, CONSOLE_GROUP, 0, 0,
                   MAX_SEG_LEN);
CONSOLE_VAR_RANGED(u32, kLED4_s2_transition_s1_ms, CONSOLE_GROUP, 0, 0,
                   MAX_SEG_LEN);
CONSOLE_VAR_RANGED(u32, kLED4_s1_hold_offset_ms, CONSOLE_GROUP, 0, 0,
                   MAX_SEG_LEN * 4);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDevDesignCubeLights::GetBehaviorOperationModifiers(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDevDesignCubeLights::GetAllDelegates(
    std::set<IBehavior*>& delegates) const {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDevDesignCubeLights::GetBehaviorJsonKeys(
...
void BehaviorDevDesignCubeLights::BehaviorUpdate() {
  ...
  if (s_shouldSaveNextTick) {
    ...
    const std::string filename = "DevLights_" + std::to_string(time.tm_hour) +
  ...
  if ((newLights != _dVars.currentLights) && (obj != nullptr)) {
    auto debugStr = "DevDesignCubeLights";
    if (_dVars.handle == kInvalidHandle) {
      GetBEI().GetCubeLightComponent().StopAllAnims();
      GetBEI().GetCubeLightComponent().PlayLightAnim(obj->GetID(), anim, {},
                                                     debugStr, _dVars.handle);
    } else {
      GetBEI().GetCubeLightComponent().StopAndPlayLightAnim(
          obj->GetID(), _dVars.handle, anim, debugStr);
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
CubeLightAnimation::ObjectLights
BehaviorDevDesignCubeLights::BuildLightsFromConsoleVars() const {
