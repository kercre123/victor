 ...
 * Date:   05/22/2017
 *
 * Description: Simple test for TurnInPlace action at various speeds/accels,
 *etc. Test configuration defined in the behavior's json file.
 *
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorDevTurnInPlaceTest::BehaviorDevTurnInPlaceTest(
  ...
  _iConfig.gapBetweenTests_s =
      ParseFloat(config, kGapBetweenTestsKey, debugName);
  ...
  PRINT_CH_INFO(kChannelName, "BehaviorDevTurnInPlaceTest.LoadFromJson",
                "Loaded %zu tests from config file.", _iConfig.tests.size());
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDevTurnInPlaceTest::GetBehaviorJsonKeys(
...
void BehaviorDevTurnInPlaceTest::OnBehaviorActivated() {
  ...
  DEV_ASSERT(!_iConfig.tests.empty(),
             "BehaviorDevTurnInPlaceTest.NoTestsLoaded");
  DEV_ASSERT(_dVars.testInd == 0,
             "BehaviorDevTurnInPlaceTest.TestIndexNonzero");
  DEV_ASSERT(_iConfig.nRunsPerTest > 0,
             "BehaviorDevTurnInPlaceTest.InvalidNRunsPerTest");
...
void BehaviorDevTurnInPlaceTest::ActionCallback() {
  ...
  PRINT_CH_INFO(
      kChannelName, "BehaviorDevTurnInPlaceTest.TestComplete",
      "index %i, run %d, finalHeading_deg %.3f", _dVars.testInd,
      1 + currTest.nTimesRun,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
CompoundActionSequential* BehaviorDevTurnInPlaceTest::GenerateTestAction(
    const Pose3d& robotPose, const int testInd) const {
  DEV_ASSERT(testInd < _iConfig.tests.size(),
             "BehaviorDevTurnInPlaceTest.GenerateTestAction.TestIndexOOB");

  const auto test = _iConfig.tests[testInd];

  PRINT_CH_INFO(
      kChannelName, "BehaviorDevTurnInPlaceTest.TestStart",
      "index %i, run %d, angle_deg %.3f, speed_deg_per_sec %.3f, "
      "accel_deg_per_sec2 %.3f, tol_deg %.3f, startingHeading_deg %.3f",
      testInd, 1 + test.nTimesRun, test.angle_deg, test.speed_deg_per_sec,
      test.accel_deg_per_sec2, test.tol_deg,
