...
// !!! These should match the values in PickAndPlaceController for accurate
// testing
...
BehaviorLiftLoadTest::BehaviorLiftLoadTest(const Json::Value& config)
  ...
  SubscribeToTags({GameToEngineTag::SetLiftLoadTestAsActivatable});

  SubscribeToTags({RobotInterface::RobotToEngineTag::liftLoad});
}
...
void BehaviorLiftLoadTest::InitBehavior() {
  _iConfig.logger = std::make_unique<Util::RollingFileLogger>(
      ...
      GetBEI().GetRobotInfo()._robot.GetContextDataPlatform()->pathToResource(
          Util::Data::Scope::Cache, "liftLoadTest"));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorLiftLoadTest::WantsToBeActivatedBehavior() const {
  return _dVars.canRun && (_dVars.currentState == State::Init ||
                           _dVars.currentState == State::TestComplete);
}
...
void BehaviorLiftLoadTest::BehaviorUpdate() {
  ...
  if (_dVars.numLiftRaises == kNumLiftRaises || _dVars.abortTest) {
    if (_dVars.numLiftRaises == kNumLiftRaises) {
      PRINT_CH_INFO("Behaviors",
                    "BehaviorLiftLoadTest.UpdateInternal_Legacy.TestComplete",
                    "%d", _dVars.numLiftRaises);
      Write("\nTest Completed Successfully");
    } else {
      PRINT_CH_INFO("Behaviors",
                    "BehaviorLiftLoadTest.UpdateInternal_Legacy.TestAborted",
                    "%d", _dVars.numLiftRaises);
      Write("\nTest Aborted");
    }
  ...
  switch (_dVars.currentState) {
    case State::Init: {
      ...
      CompoundActionSequential* compoundAction =
          new CompoundActionSequential({lowerLiftAction, raiseLiftAction});

      DelegateIfInControl(compoundAction, [this, &robot](ActionResult result) {
        if (ActionResult::SUCCESS == result) {
          // Tell robot to check for load and wait for response
          robot.SendRobotMessage<RobotInterface::CheckLiftLoad>();

          PRINT_CH_INFO("Behaviors",
                        "BehaviorLiftLoadTest.WaitingForLiftLoadMsg", "");
    ...
    default: {
      PRINT_NAMED_ERROR("BehaviorLiftLoadTest.Update.UnknownState",
...
void BehaviorLiftLoadTest::SetCurrState(State s) {
  ...
  BEHAVIOR_VERBOSE_PRINT(DEBUG_LIFT_LOAD_TEST_BEHAVIOR,
                         "BehaviorLiftLoadTest.SetState", "set state to '%s'",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorLiftLoadTest::HandleWhileActivated(
  ...
  switch (event.GetData().GetTag()) {
    ...
    default:
      PRINT_CH_INFO(
          "Behaviors", "BehaviorLiftLoadTest.HandleWhileRunning.InvalidTag",
  ...
  switch (event.GetData().GetTag()) {
    ...
    default: {
      PRINT_CH_INFO("Behaviors", "BehaviorLiftLoadTest.AlwaysHandle.InvalidTag",
                    "Received unexpected event with tag %hhu.",
...
void BehaviorLiftLoadTest::HandleWhileActivated(
  ...
  switch (event.GetData().GetTag()) {
    case RobotInterface::RobotToEngineTag::liftLoad: {
      ...
      PRINT_CH_DEBUG("Behaviors", "BehaviorLiftLoadTest.HandleLiftLoad.HasLoad",
                     "%d / %d", _dVars.numHadLoad, _dVars.numLiftRaises);
      break;
    }
    default: {
      PRINT_CH_INFO(
          "Behaviors",
          "BehaviorLiftLoadTest.HandleWhileRunning.InvalidRobotToEngineTag",
          "Received unexpected event with tag %hhu.", event.GetData().GetTag());
      break;
    }
  }
...
void BehaviorLiftLoadTest::PrintStats() {
  ...
  PRINT_CH_DEBUG("Behaviors", "BehaviorLiftLoadTest.PrintStats.LoadRate", "%d",
                 loadPercent);
