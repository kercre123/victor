 ...
 * Created: 2/13/18
 *
 * Description: Dev behavior which makes it easy to write text to Victor's face
 *so that design and engineering can quickly get a sense of realtime values the
 *robot is using for decision making
 *
...
BehaviorDevDisplayReadingsOnFace::DynamicVariables::DynamicVariables() {
  image = Vision::Image(FACE_DISPLAY_HEIGHT, FACE_DISPLAY_WIDTH,
                        NamedColors::BLACK);
...
void BehaviorDevDisplayReadingsOnFace::BehaviorUpdate() {
  ...
  _dVars.image = Vision::Image(FACE_DISPLAY_HEIGHT, FACE_DISPLAY_WIDTH,
                               NamedColors::BLACK);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDevDisplayReadingsOnFace::DrawTextWithAutoLayout(
    const std::string& text) {
  auto layoutPoint = _dVars.autoLayoutPoint;
  _dVars.image.DrawText(layoutPoint, text, NamedColors::WHITE,
                        kMagicNumberTextScale);
...
void BehaviorDevDisplayReadingsOnFace::UpdateDisplayPeripheralMotion() {
  if (_dVars.motionIsValid) {
    std::string leftText =
        "Left Periph:" + std::to_string(_dVars.motionMessage.left_img_area);
    std::string rightText =
        "Right Periph:" + std::to_string(_dVars.motionMessage.right_img_area);
    std::string topText =
        "Top Periph:" + std::to_string(_dVars.motionMessage.top_img_area);
    std::string bottomText =
        "Bottom Periph:" + std::to_string(_dVars.motionMessage.bottom_img_area);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDevDisplayReadingsOnFace::AlwaysHandleInScope(
