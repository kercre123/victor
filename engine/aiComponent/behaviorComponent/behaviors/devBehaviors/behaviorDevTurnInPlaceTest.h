...
class BehaviorDevTurnInPlaceTest : public ICozmoBehavior {
 ...
 private:
  struct sTest {
    sTest(float angle_deg, float speed_deg_per_sec, float accel_deg_per_sec2,
          float tol_deg)
        : angle_deg(angle_deg),
  ...
  CompoundActionSequential* GenerateTestAction(const Pose3d& robotPose,
                                               const int testInd) const;
