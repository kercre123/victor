 ...
 *                - Repeat until consecutively fails kMaxConsecFails times or
 *does kMaxNumAttempts attempts
 *
 *
...
#define END_TEST_IN_HANDLER(RESULT, NAME) \
  EndAttempt(robot, RESULT, NAME);        \
  return;
...
namespace {
// This macro uses PRINT_NAMED_INFO if the supplied define (first arg) evaluates
// to true, and PRINT_NAMED_DEBUG otherwise All args following the first are
// passed directly to the chosen print macro
#define BEHAVIOR_VERBOSE_PRINT(_BEHAVIORDEF, ...) \
  do {                                            \
    if ((_BEHAVIORDEF)) {                         \
      PRINT_CH_INFO("Behaviors", __VA_ARGS__);    \
    } else {                                      \
      PRINT_CH_DEBUG("Behaviors", __VA_ARGS__);   \
    }                                             \
  } while (0)

CONSOLE_VAR(u32, kMaxNumAttempts, "DockingTest", 30);
CONSOLE_VAR(u32, kMaxConsecFails, "DockingTest", 3);
CONSOLE_VAR(u32, kTestDockingMethod, "DockingTest",
            (u8)DockingMethod::HYBRID_DOCKING);
...
BehaviorDockingTestSimple::BehaviorDockingTestSimple(const Json::Value& config)
  ...
  SubscribeToTags({{EngineToGameTag::RobotCompletedAction,
                    ...
                    EngineToGameTag::ObjectMoved, EngineToGameTag::RobotStopped,
...
void BehaviorDockingTestSimple::InitBehavior() {
  _iConfig.logger = std::make_unique<Util::RollingFileLogger>(
      nullptr,
      GetBEI().GetRobotInfo()._robot.GetContextDataPlatform()->pathToResource(
          Util::Data::Scope::Cache, "dockingTest"));
  ...
  if (nullptr != robot.GetContext()->GetRobotManager() &&
    ...
    _iConfig.signalHandle =
        (robot.GetContext()->GetRobotManager()->GetMsgHandler()->Subscribe(
            RobotInterface::RobotToEngineTag::dockingStatus,
            std::bind(&BehaviorDockingTestSimple::HandleDockingStatus, this,
                      std::placeholders::_1)));
  }
}
...
void BehaviorDockingTestSimple::OnBehaviorActivated() {
  ...
  _dVars.cubePlacementPose = Pose3d(Radians(DEG_TO_RAD(0)), Z_AXIS_3D(),
                                    {176, 0, 22}, robot.GetWorldOrigin());
  ...
  ss.clear();
  ss << "@" << kMaxXAwayFromPreDock_mm << " " << kMaxYAwayFromPreDock_mm << " "
  ...
  if (robot.GetContext()->GetVizManager() != nullptr) {
    std::stringstream ss;
    ss << "images_" << now->tm_mon + 1 << "-" << now->tm_mday << "-"
...
void BehaviorDockingTestSimple::BehaviorUpdate() {
  Robot& robot = GetBEI().GetRobotInfo()._robot;
  if (_dVars.numAttempts == kMaxNumAttempts &&
      _dVars.currentState != State::ManualReset) {
    Write("\nTest Completed Successfully");

    PrintStats();

    if (robot.GetContext()->GetVizManager() != nullptr) {
      robot.GetContext()->GetVizManager()->SendSaveImages(
  ...
  DEV_ASSERT(
      robot.IsPoseInWorldOrigin(robot.GetPose()),
  ...
  switch (_dVars.currentState) {
    case State::Init: {
      ...
      if (robot.GetContext()->GetVizManager() != nullptr) {
        robot.GetContext()->GetVizManager()->SendSaveImages(
            ImageSendMode::Stream, _iConfig.imageFolder);
      }
      ...
      CompoundActionSequential* action = new CompoundActionSequential(
          {new MoveHeadToAngleAction(0, DEG_TO_RAD(1), 0), new WaitAction(2)});
      DelegateIfInControl(
          robot, action,
          [this, &robot](const ActionResult& result,
                         const ActionCompletedUnion& completionInfo) {
    ...
    case State::Roll: {
      if (_dVars.blockObjectIDPickup.IsSet()) {
        ObservableObject* object = robot.GetBlockWorld().GetLocatedObjectByID(
        ...
        Pose3d junk;
        _dVars.initialVisionMarker =
            const_cast<Vision::KnownMarker&>(block->GetTopMarker(junk));

        DriveToObjectAction* driveAction = new DriveToObjectAction(
            _dVars.blockObjectIDPickup, PreActionPose::ROLLING);
        DelegateIfInControl(
            robot, driveAction,
            [this, &robot](const ActionResult& result,
                           const ActionCompletedUnion& completionUnion) {
              if (result == ActionResult::SUCCESS) {
                _dVars.initialRobotPose = robot.GetPose();

                RollObjectAction* action =
                    new RollObjectAction(_dVars.blockObjectIDPickup);
                ...
                DelegateIfInControl(
                    robot, action,
                    [this, &robot](const ActionResult& result,
                                   const ActionCompletedUnion& completedUnion) {
                      if (result != ActionResult::SUCCESS) {
                        if (_dVars.numSawObject < 5) {
                          EndAttempt(robot, result, "RollNotSeeingObject",
                                     true);
                        } else {
    ...
    case State::PickupLow: {
      if (_dVars.blockObjectIDPickup.IsSet()) {
        const ObservableObject* blockToPickup =
            robot.GetBlockWorld().GetLocatedObjectByID(
                _dVars.blockObjectIDPickup);
        ...
        const ActionableObject* aObject =
            dynamic_cast<const ActionableObject*>(blockToPickup);

        if (aObject == nullptr) {
          PRINT_NAMED_ERROR("BehaviorDockingTest.PickupLow.NullObject",
                            "ActionableObject is null");
          CancelSelf();
          return;
        }
        ...
        aObject->GetCurrentPreActionPoses(
            preActionPoses, robot.GetPose(),
            {(kRollInsteadOfPickup ? PreActionPose::ROLLING
                                   : PreActionPose::DOCKING)},
            {_dVars.initialVisionMarker.GetCode()}, obstacles);

        if (preActionPoses.empty()) {
          break;
        }

        const Pose3d preActionPose =
            (kUseClosePreActionPose ? preActionPoses.back().GetPose()
                                    : preActionPoses.front().GetPose());

        // Store the angle of the preAction pose so we can use it as the
        // approach angle for the driveTo/dock actions so that we will always
        // approach the object from the same angle
        if (_dVars.initialPreActionPoseAngle_rad == kInvalidAngle) {
          _dVars.initialPreActionPoseAngle_rad =
              preActionPose.GetRotation().GetAngleAroundZaxis().ToFloat();
        }
        ...
        if (kNumRandomObstacles > 0) {
          ...
          // Add new obstacles at random poses around the preDock pose
          // corresponding with _dVars.initialVisionMarker
          for (u32 i = 0; i < kNumRandomObstacles; ++i) {
            ...
            f32 randX = GetRNG().RandDblInRange(kObstacleRangeXmin_mm,
                                                kObstacleRangeXmax_mm);
            f32 randY = GetRNG().RandDblInRange(-kObstacleRangeY_mm,
                                                kObstacleRangeY_mm);
            ...
            while (randY < kNoObstaclesWithinXmmOfPreDockPose &&
              ...
              randY = GetRNG().RandDblInRange(-kObstacleRangeY_mm,
                                              kObstacleRangeY_mm);
              count++;
            ...
            Pose3d p(angle, Z_AXIS_3D(),
                     {x + randX, y + randY, kObstacleSize_mm.z() * 0.5f},
                     robot.GetWorldOrigin());

            robot.GetBlockWorld().CreateFixedCustomObject(
                p, kObstacleSize_mm.x(), kObstacleSize_mm.y(),
                kObstacleSize_mm.z());
          }
        }
        ...
        // If we are just doing a straight pickup without driving to the preDock
        // pose
        if (kJustPickup) {
          PickupObjectAction* action =
              new PickupObjectAction(_dVars.blockObjectIDPickup);
          action->SetDockingMethod((DockingMethod)kTestDockingMethod);
        ...
        // Otherwise if we are doing a DriveToPickupObject action in one state
        // (instead of the driveTo and pickup in separate states
        else if (kDriveToAndPickupBlockOneAction) {
          IActionRunner* action = nullptr;
          if (kAlignInsteadOfPickup) {
            // This compound action will essentially function as a
            // DriveToPickupObjectAction By aligning to the LIFT_PLATE and then
            // moving the lift up we should pickup the object
            action = new CompoundActionSequential(
                {new DriveToAlignWithObjectAction(_dVars.blockObjectIDPickup, 0,
                                                  false, 0,
            ...
            action = new DriveToPickupObjectAction(
                _dVars.blockObjectIDPickup, true,
                _dVars.initialPreActionPoseAngle_rad);
            static_cast<DriveToPickupObjectAction*>(action)->SetDockingMethod(
                (DockingMethod)kTestDockingMethod);
          }
          ...
          DelegateIfInControl(
              ...
              [this, &robot](const ActionResult& result,
                ...
                if (result != ActionResult::SUCCESS) {
                  if (_dVars.numSawObject < 5) {
                    EndAttempt(robot, result, "DriveToAndPickupNotSeeingObject",
                               true);
                  } else {
                ...
                if (kAlignInsteadOfPickup) {
                  robot.GetCarryingComponent().SetCarryingObject(
                      _dVars.blockObjectIDPickup,
          ...
          DriveToObjectAction* driveAction = new DriveToObjectAction(
              _dVars.blockObjectIDPickup, PreActionPose::DOCKING, 0, true,
              _dVars.initialPreActionPoseAngle_rad);
          DelegateIfInControl(
              robot, driveAction,
              [this, &robot](const ActionResult& result,
                ...
                if (result == ActionResult::SUCCESS) {
                  ...
                  DelegateIfInControl(
                      ...
                      [this, &robot](
                        ...
                        if (result != ActionResult::SUCCESS) {
                          if (_dVars.numSawObject < 5) {
                            EndAttempt(robot, result, "PickupNotSeeingObject",
                                       true);
                          } else {
                            EndAttempt(robot, result, "Pickup", true);
                          }
    ...
    case State::PlaceLow: {
      PlaceObjectOnGroundAtPoseAction* action =
          new PlaceObjectOnGroundAtPoseAction(_dVars.cubePlacementPose, true);
      DelegateIfInControl(
          robot, action,
          [this, &robot](const ActionResult& result,
                         const ActionCompletedUnion& completionInfo) {
    ...
    case State::Reset: {
      f32 randX = GetRNG().RandDblInRange(-kMaxXAwayFromPreDock_mm,
                                          kMaxXAwayFromPreDock_mm);
      f32 randY = GetRNG().RandDblInRange(-kMaxYAwayFromPreDock_mm,
                                          kMaxYAwayFromPreDock_mm);
      f32 randA =
          GetRNG().RandDblInRange(-DEG_TO_RAD(kMaxAngleAwayFromPreDock_deg),
      ...
      if (kNumRandomObstacles > 0) {
        if (kMaxYAwayFromPreDock_mm <= kNoObstaclesWithinXmmOfPreDockPose) {
          PRINT_NAMED_ERROR("BehaviorDockingTest.Reset.YAwayTooSmall",
                            "MaxYAwayFromPreDock_mm is too small %f <= %f need "
                            "to be able to drive into obstacles",
                            kMaxYAwayFromPreDock_mm,
        ...
        // Make sure y falls outside the no obstacle zone so we will always have
        // to drive to a point where obstacles could be
        int count = 0;
        while (randY < kNoObstaclesWithinXmmOfPreDockPose &&
               randY > -kNoObstaclesWithinXmmOfPreDockPose) {
          randY = GetRNG().RandDblInRange(-kMaxYAwayFromPreDock_mm,
                                          kMaxYAwayFromPreDock_mm);
          count++;
      ...
      ActionableObject* aObject = dynamic_cast<ActionableObject*>(
          robot.GetBlockWorld().GetLocatedObjectByID(
              _dVars.blockObjectIDPickup));

      if (aObject == nullptr) {
        PRINT_NAMED_ERROR("BehaviorDockingTest.Reset.NullObject",
                          "ActionableObject is null");
        CancelSelf();
        return;
      }
      ...
      if (preActionPoses.size() != kExpectedNumPreDockPoses) {
        // If we are rolling and we can't find any preAction poses it means the
        // block was not rolled so get preActionPoses for the marker we are
        // currently seeing
        if (kRollInsteadOfPickup) {
          aObject->GetCurrentPreActionPoses(
              preActionPoses, robot.GetPose(),
              {(kRollInsteadOfPickup ? PreActionPose::ROLLING
                                     : PreActionPose::DOCKING)},
              {_dVars.markerBeingSeen.GetCode()}, obstacles);

          if (preActionPoses.size() != kExpectedNumPreDockPoses) {
            PRINT_NAMED_ERROR(
                "BehaviorDockingTest.Reset.RollingBadNumPreActionPoses",
      ...
      // If we are reseting due to an action failing then we shouldn't go to a
      // random offset and we should not count this reset pickup as an attempt
      ...
      Pose3d p(Radians(angle + randA), Z_AXIS_3D(), {x + randX, y + randY, 0},
               robot.GetWorldOrigin());

      ICompoundAction* action = new CompoundActionSequential();

      // If we are carrying the object when we reset then make sure to put it
      // down before driving to the pose
      if (robot.GetCarryingComponent().IsCarryingObject()) {
        PlaceObjectOnGroundAtPoseAction* placeAction =
            new PlaceObjectOnGroundAtPoseAction(_dVars.cubePlacementPose, true);
    ...
    case State::ManualReset: {
      ...
      if (_dVars.yellForCompletion) {
        ...
        IActionRunner* action = new CompoundActionSequential(
            {new SayTextAction(
                 "Help", SayTextAction::AudioTtsProcessingStyle::Unprocessed),
             new WaitAction(3)});
        DelegateIfInControl(robot, action,
                            [this](const ActionResult& result,
                                   const ActionCompletedUnion& completionInfo) {
    ...
    default: {
      PRINT_NAMED_ERROR("BehaviorDockingTest.Update.UnknownState",
                        "Reached unknown state %d", (u32)_dVars.currentState);
      CancelSelf();
      return;
    }
  }
...
void BehaviorDockingTestSimple::SetCurrState(State s) {
  ...
  BEHAVIOR_VERBOSE_PRINT(DEBUG_DOCKING_TEST_BEHAVIOR,
                         "BehaviorDockingTest.SetState", "set state to '%s'",
                         GetDebugStateName().c_str());
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDockingTestSimple::SetCurrStateAndFlashLights(State s,
                                                           Robot& robot) {
...
void BehaviorDockingTestSimple::UpdateStateName() {
  ...
  name +=
      std::to_string(_dVars.numAttempts + _dVars.numExtraAttemptsDueToFailure);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDockingTestSimple::HandleWhileActivated(
  ...
  switch (event.GetData().GetTag()) {
    ...
    case EngineToGameTag::RobotOffTreadsStateChanged: {
      if (event.GetData().Get_RobotOffTreadsStateChanged().treadsState ==
          OffTreadsState::OnTreads) {
        _dVars.currentState = State::Init;
    ...
    default:
      PRINT_CH_INFO(
          "Behaviors", "BehaviorDockingTest.HandleWhileRunning.InvalidTag",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDockingTestSimple::HandleActionCompleted(
    Robot& robot, const ExternalInterface::RobotCompletedAction& msg) {
  if (!IsActivated()) {
  ...
  if (callback == _dVars.actionCallbackMap.end()) {
    BEHAVIOR_VERBOSE_PRINT(
        DEBUG_DOCKING_TEST_BEHAVIOR,
        "BehaviorDockingTest.HandleActionCompleted.OtherAction",
        "finished action id=%d type=%s but don't care", msg.idTag,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDockingTestSimple::EndAttempt(Robot& robot, ActionResult result,
                                           std::string name,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDockingTestSimple::HandleObservedObject(
    Robot& robot, const ExternalInterface::RobotObservedObject& msg) {
  ObjectID objectID = msg.objectID;
  const ObservableObject* oObject =
      robot.GetBlockWorld().GetLocatedObjectByID(objectID);

  if (nullptr == oObject) {
    PRINT_NAMED_WARNING(
        "BehaviorDockingTestSimple.HandleObservedObject.NullObject",
  ...
  if (IsValidLightCube(oObject->GetType(), false)) {
    if (_dVars.blockObjectIDPickup.IsSet() &&
      ...
      if (observedMarkers.size() == 1) {
        ...
        PRINT_CH_INFO("Behaviors", "BehaviorDockingTest.HandleObservedObject",
                      "Saw more than one marker");
        _dVars.yellForHelp = true;
      ...
      PRINT_NAMED_WARNING(
          "BehaviorDockingTest.HandleObservedObject.UnexpectedBlock",
          "ID: %d, Type: %d", objectID.GetValue(), oObject->GetType());
      _dVars.yellForHelp = true;
      END_TEST_IN_HANDLER(ActionResult::ABORT, "UnexpectedBlock");
    }
  } else {
    PRINT_NAMED_WARNING(
        "BehaviorDockingTest.HandleObservedObject.UnexpectedObservedObject",
        "ID: %d, Type: %d", objectID.GetValue(), oObject->GetType());
    _dVars.yellForHelp = true;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDockingTestSimple::HandleRobotStopped(
    Robot& robot, const ExternalInterface::RobotStopped& msg) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDockingTestSimple::HandleDockingStatus(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDockingTestSimple::DelegateIfInControl(
    Robot& robot, IActionRunner* action, ActionResultCallback callback) {
  assert(_dVars.actionCallbackMap.count(action->GetTag()) == 0);

  if (robot.GetActionList().QueueAction(QueueActionPosition::NOW, action) ==
      RESULT_OK) {
    _dVars.actionCallbackMap[action->GetTag()] = callback;
  } else {
    PRINT_NAMED_WARNING(
        "BehaviorDockingTest.DelegateIfInControl.QueueActionFailed",
        "Action type %s", EnumToString(action->GetType()));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorDockingTestSimple::RecordAttempt(ActionResult result,
                                              std::string name) {
  std::stringstream ss;
  ss << "Attempt: " << _dVars.numAttempts + _dVars.numExtraAttemptsDueToFailure
     << " TimeStarted: " << _dVars.attemptStartTime
     << " Result: " << EnumToString(result);
  ss << " " << name << " DockingMethod: " << (int)kTestDockingMethod;
  ss << " HM: " << _dVars.didHM
     << " NumDockingRetries: " << _dVars.numDockingRetries << "\n\t\t";

  char buf[300];
  sprintf(
      buf,
      "Translation=(%f, %f %f), RotVec=(%f, %f, %f), RotAng=%frad (%.1fdeg)",
  ...
  sprintf(
      buf,
      "Translation=(%f, %f %f), RotVec=(%f, %f, %f), RotAng=%frad (%.1fdeg)",
      p.GetTranslation().x(), p.GetTranslation().y(), p.GetTranslation().z(),
      p.GetRotationAxis().x(), p.GetRotationAxis().y(), p.GetRotationAxis().z(),
      p.GetRotationAngle().ToFloat(), p.GetRotationAngle().getDegrees());

  ss << "CubeRelativeToRobot " << buf << "\n";

  // For log parsing
  // result, rel block pose x, rel block pose y, rel block angle, dock method,
  // didHM, numDockingRetries numSawObject
  sprintf(buf, "!!!%i %f %f %f %i %i %i %i", (int)result,
          p.GetTranslation().x(), p.GetTranslation().y(),
          p.GetRotationAngle().ToFloat(), (int)kTestDockingMethod, _dVars.didHM,
          _dVars.numDockingRetries, _dVars.numSawObject);
  ss << buf << "\n";
...
void BehaviorDockingTestSimple::PrintStats() {
  ...
  ss << "*****************\n";
  ss << "Successful Runs: "
     << _dVars.numAttempts - _dVars.numFails +
            _dVars.numExtraAttemptsDueToFailure
     << " (";
  ss << ((_dVars.numAttempts - _dVars.numFails +
          _dVars.numExtraAttemptsDueToFailure) /
         (1.0 * _dVars.numAttempts + _dVars.numExtraAttemptsDueToFailure)) *
            100
     << "%)\n";
  ss << "Failed Runs: " << _dVars.numFails << " ("
     << (_dVars.numFails /
         (1.0 * _dVars.numAttempts + _dVars.numExtraAttemptsDueToFailure)) *
            100
     << "%)\n";
  ss << "Total Runs: "
     << _dVars.numAttempts + _dVars.numExtraAttemptsDueToFailure;
  Write(ss.str());
