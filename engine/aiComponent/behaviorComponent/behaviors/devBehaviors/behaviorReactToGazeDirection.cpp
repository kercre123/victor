...
CONSOLE_VAR(f32, kFaceDirectedAtRobotMinXThres_mm, "Vision.GazeDirection",
            -25.f);
CONSOLE_VAR(f32, kFaceDirectedAtRobotMaxXThres_mm, "Vision.GazeDirection",
            20.f);
CONSOLE_VAR(f32, kFaceDirectedAtRobotMinYThres_mm, "Vision.GazeDirection",
            -100.f);
CONSOLE_VAR(f32, kFaceDirectedAtRobotMaxYThres_mm, "Vision.GazeDirection",
            100.f);
...
CONSOLE_VAR(f32, kMaxPanSpeed_radPerSec, "Vision.GazeDirection",
...
CONSOLE_VAR(f32, kConeFor180TurnForFaceSearch_deg, "Vision.GazeDirection",
            40.f);
CONSOLE_VAR(f32, kSearchForFaceTurnSideAngle_deg, "Vision.GazeDirection",
            -90.f);
CONSOLE_VAR(f32, kSearchForFaceTurnAroundAngle_deg, "Vision.GazeDirection",
            180.f);
CONSOLE_VAR(s32, kSearchForFaceNumberOfImagesToWait, "Vision.GazeDirection", 5);
CONSOLE_VAR(bool, kUseExistingFacesWhenSearchingForFaces,
            "Vision.GazeDirection", false);
CONSOLE_VAR(s32, kNumberOfTurnsForSurfacePoint, "Vision.GazeDirection", 1);
CONSOLE_VAR(u32, kMaxTimeSinceTrackedFaceUpdated_ms, "Vision.GazeDirection",
            500);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToGazeDirection::BehaviorReactToGazeDirection(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorReactToGazeDirection::InstanceConfig::InstanceConfig(
    const Json::Value& config) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToGazeDirection::GetBehaviorJsonKeys(
...
bool BehaviorReactToGazeDirection::WantsToBeActivatedBehavior() const {
  return GetBEI().GetFaceWorld().AnyStableGazeDirection(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToGazeDirection::TransitionToCheckForFace(
    const Radians& turnAngle) {
  // Turn to the angle, pick the correct (left or right) animation, and then
  // search for a face.
  CompoundActionSequential* turnAction = new CompoundActionSequential();
  if (turnAngle > 0) {
    turnAction->AddAction(new TriggerAnimationAction(
        AnimationTrigger::GazingLookAtFacesGetInLeft));
  } else {
    turnAction->AddAction(new TriggerAnimationAction(
        AnimationTrigger::GazingLookAtFacesGetInRight));
  }

  CompoundActionParallel* turnAndAnimate = new CompoundActionParallel();
  if (turnAngle > 0) {
    turnAndAnimate->AddAction(
        new TurnInPlaceAction(turnAngle.ToFloat(), false));
    turnAndAnimate->AddAction(new ReselectingLoopAnimationAction{
        AnimationTrigger::GazingLookAtFacesTurnLeft});
  } else {
    turnAndAnimate->AddAction(
        new TurnInPlaceAction(turnAngle.ToFloat(), false));
    turnAndAnimate->AddAction(new ReselectingLoopAnimationAction{
        AnimationTrigger::GazingLookAtFacesTurnRight});
  }

  turnAndAnimate->SetShouldEndWhenFirstActionCompletes(true);
  turnAction->AddAction(turnAndAnimate);
  turnAction->AddAction(new WaitForImagesAction(
      kSearchForFaceNumberOfImagesToWait, VisionMode::Faces));
  turnAction->AddAction(new MoveHeadToAngleAction(Radians(MAX_HEAD_ANGLE)));
  TurnTowardsFaceAction* turnTowardsFace =
      new TurnTowardsFaceAction(SmartFaceID(), M_PI_2);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToGazeDirection::TransitionToLookAtFace(
    const SmartFaceID& faceToTurnTowards, const Radians& turnAngle) {
  ...
  TurnTowardsFaceAction* turnTowardsFace =
      new TurnTowardsFaceAction(faceToTurnTowards, M_PI);
  turnTowardsFace->SetRequireFaceConfirmation(true);

  CompoundActionParallel* turnAndAnimate = new CompoundActionParallel();
  turnAndAnimate->AddAction(turnTowardsFace);
  if (turnAngle > 0) {
    turnAndAnimate->AddAction(new TriggerLiftSafeAnimationAction(
        AnimationTrigger::GazingLookAtFacesTurnLeft));
  } else {
    turnAndAnimate->AddAction(new TriggerLiftSafeAnimationAction(
        AnimationTrigger::GazingLookAtFacesGetInRight));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToGazeDirection::TransitionToCheckForPointOnSurface(
    const Pose3d& gazePose) {
  const auto& translation = gazePose.GetTranslation();
  auto makingEyeContact = GetBEI().GetFaceWorld().IsMakingEyeContact(
      kMaxTimeSinceTrackedFaceUpdated_ms);

  // Check to see if our gaze points is within constraints to be considered
  // "looking" at vector.
  const bool isWithinXConstraints =
      (Util::InRange(translation.x(), kFaceDirectedAtRobotMinXThres_mm,
                     kFaceDirectedAtRobotMaxXThres_mm));
  const bool isWithinYConstarints =
      (Util::InRange(translation.y(), kFaceDirectedAtRobotMinYThres_mm,
                     kFaceDirectedAtRobotMaxYThres_mm));
  if ((isWithinXConstraints && isWithinYConstarints) ||
      (makingEyeContact && kUseEyeContact)) {
    // If we're making eye contact with the robot or looking at the robot, react
    // to that.
    CompoundActionSequential* turnAction = new CompoundActionSequential();
    turnAction->AddAction(new TriggerAnimationAction(
        AnimationTrigger::GazingLookAtVectorReaction));
    ...
    for (int i = 0; i < kNumberOfTurnsForSurfacePoint; ++i) {
      if (Util::IsFltLT(translation.y(), 0.f)) {
        turnAction->AddAction(new TriggerAnimationAction(
            AnimationTrigger::GazingLookAtSurfacesGetInRight));
      } else {
        turnAction->AddAction(new TriggerAnimationAction(
            AnimationTrigger::GazingLookAtSurfacesGetInLeft));
      }

      TurnTowardsPoseAction* turnTowardsPose =
          new TurnTowardsPoseAction(gazePose);
      ...
      if (Util::IsFltLT(translation.y(), 0.f)) {
        turnAndAnimate->AddAction(new ReselectingLoopAnimationAction{
            AnimationTrigger::GazingLookAtSurfacesTurnRight});
      } else {
        turnAndAnimate->AddAction(new ReselectingLoopAnimationAction{
            AnimationTrigger::GazingLookAtSurfaceTurnLeft});
      }
      turnAndAnimate->SetShouldEndWhenFirstActionCompletes(true);
      turnAction->AddAction(turnAndAnimate);
      turnAction->AddAction(new TriggerAnimationAction(
          AnimationTrigger::GazingLookAtSurfaceReaction));
      turnAction->AddAction(new WaitAction(kTurnWaitAfterFinalTurn_s));
      turnAction->AddAction(
          new TurnTowardsFaceAction(_dVars.faceIDToTurnBackTo));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Radians BehaviorReactToGazeDirection::ComputeTurnAngleFromGazePose(
    const Pose3d& gazePose) {
  // Compute angle to turn and look for faces. There are three buckets each with
  // a corresponding turn: left, right, turn-around. Put the pose into one on
  // these buckets and return the turn angle associated with that bucket.
  Radians turnAngle;
  const auto& translation = gazePose.GetTranslation();
  Radians gazeAngle = atan2f(translation.y(), translation.x());
  const Radians angleDifference = Radians(DEG_TO_RAD(180)) - gazeAngle;
  if ((angleDifference <=
       Radians(DEG_TO_RAD(kConeFor180TurnForFaceSearch_deg / 2.f))) &&
      (angleDifference >=
       -Radians(DEG_TO_RAD(kConeFor180TurnForFaceSearch_deg / 2.f)))) {
    // Turn around
...
void BehaviorReactToGazeDirection::TransitionToCheckGazeDirection() {
  if (GetBEI().GetFaceWorld().GetGazeDirectionPose(
          kMaxTimeSinceTrackedFaceUpdated_ms, _dVars.gazeDirectionPose,
          _dVars.faceIDToTurnBackTo)) {
    const auto& robotPose = GetBEI().GetRobotInfo().GetPose();
    Pose3d gazeDirectionPoseWRTRobot;
    if (_dVars.gazeDirectionPose.GetWithRespectTo(robotPose,
                                                  gazeDirectionPoseWRTRobot)) {
      SendDASEventForPoseToFollow(gazeDirectionPoseWRTRobot);

      // Now that we know we are going to turn clear the history
      GetBEI().GetFaceWorldMutable().ClearGazeDirectionHistory(
          _dVars.faceIDToTurnBackTo);

      if (_iConfig.searchForFaces) {
        // Bucket the angle to turn to look for faces
        const Radians turnAngle =
            ComputeTurnAngleFromGazePose(gazeDirectionPoseWRTRobot);

        // If we're configured to look for existing faces in face world to turn
        // towards check if there are any existing faces visible from the angle
        // we want to turn to. faces visible from that angle, or we're not
        // configured to do use faces existing in face world then turn to the
        // specificied angle and search for a face.
        SmartFaceID faceToTurnTowards;
        if (GetBEI().GetFaceWorld().FaceInTurnAngle(
                Radians(turnAngle), _dVars.faceIDToTurnBackTo, robotPose,
                faceToTurnTowards) &&
          ...
          // If there aren't any faces visible from that angle, or we're not
          // configured to use faces existing in face world then turn to the
          // specificied angle and search for a face.
      ...
      LOG_ERROR(
          "BehaviorReactToGazeDirection.TransitionToCheckFaceDirection."
          "GetWithRespectToFailed",
          "");
    }
  }
}
...
void BehaviorReactToGazeDirection::FoundNewFace(ActionResult result) {
  switch (result) {
    case ActionResult::NO_FACE: {
      DASMSG(
          behavior_react_to_gaze_direction_found_no_new_face,
    ...
    case ActionResult::SUCCESS: {
      DASMSG(
          behavior_react_to_gaze_direction_found_new_face,
    ...
    default: {
      LOG_ERROR(
          "BehaviorReactToGazeDirection.FoundNewFace."
          "UnexpectedResultFromTurnTowardsFace",
          "Result: %d", result);
      break;
    }
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToGazeDirection::GetBehaviorOperationModifiers(
  ...
  // This will result in running facial part detection whenever is behavior is
  // an activatable scope
  modifiers.visionModesForActiveScope->insert(
      {VisionMode::Faces, EVisionUpdateFrequency::High});
  modifiers.visionModesForActiveScope->insert(
      {VisionMode::Faces_Gaze, EVisionUpdateFrequency::High});
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorReactToGazeDirection::SendDASEventForPoseToFollow(
    const Pose3d& gazePose) const {
  const auto& translation = gazePose.GetTranslation();
  std::string type;
  if (_iConfig.searchForFaces) {
    type = "Surface";
  ...
  DASMSG_SET(i1, static_cast<int>(translation.x()),
             "X coordinate in mm of point to turn towards");
  DASMSG_SET(i2, static_cast<int>(translation.y()),
             "Y coordinate in mm of point to turn towards");
  DASMSG_SET(i3, static_cast<int>(translation.z()),
             "Z coordinate in mm of point to turn towards");
  DASMSG_SEND();
}
