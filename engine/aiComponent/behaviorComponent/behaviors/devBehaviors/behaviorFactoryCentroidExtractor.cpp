...
// This define comments out code that depends on messages that don't exist
// outside of factory branch. This skips a variety of version checks so it
// should never be used on the line!
...
static const BackpackLightAnimation::BackpackLEDArray fail_onColorRed{
    {NamedColors::RED, NamedColors::RED, NamedColors::RED}};
static const BackpackLightAnimation::BackpackLEDArray fail_onColorOrange{
    {NamedColors::ORANGE, NamedColors::ORANGE, NamedColors::ORANGE}};
static const BackpackLightAnimation::BackpackLEDArray fail_onColorMagenta{
    {NamedColors::MAGENTA, NamedColors::MAGENTA, NamedColors::MAGENTA}};
static const BackpackLightAnimation::BackpackLEDArray fail_onColorBlue{
    {NamedColors::BLUE, NamedColors::BLUE, NamedColors::BLUE}};
static const BackpackLightAnimation::BackpackLEDArray fail_offColor{
    {NamedColors::BLACK, NamedColors::BLACK, NamedColors::BLACK}};
}  // namespace
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorFactoryCentroidExtractor::BehaviorFactoryCentroidExtractor(
...
void BehaviorFactoryCentroidExtractor::OnBehaviorActivated() {
  ...
  _iConfig.factoryTestLogger.StartLog(serialNumString.str() + "_centroids",
                                      true, robot.GetContextDataPlatform());
...
void BehaviorFactoryCentroidExtractor::BehaviorUpdate() {
  ...
  if (_dVars.waitingForDots || !_dVars.liftCalibrated ||
      !_dVars.headCalibrated) {
    return;
  }
...
void BehaviorFactoryCentroidExtractor::TransitionToMovingHead(Robot& robot) {
  ...
  DelegateIfInControl(action, [this, &robot](ActionResult res) {
    if (res == ActionResult::SUCCESS) {
      ...
      PRINT_NAMED_WARNING("BehaviorFactoryCentroidExtractor.MoveHead",
                          "Moving head to 0 degrees failed");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorFactoryCentroidExtractor::HandleWhileActivated(
  ...
  if (event.GetData().GetTag() ==
    ...
    if (!msg.success) {
      ...
      if (!msg.didComputePose) {
        PRINT_NAMED_WARNING(
            "BehaviorFactoryCentroidExtractor.DidNotComputeCameraPose",
        ...
        if (!NEAR(msg.camPoseRoll_rad, 0, rollThresh_rad)) {
          PRINT_NAMED_WARNING("BehaviorFactoryCentroidExtractor.CamPose",
                              "Roll exceeds threshold");
          exceedsThresh = true;
        }
        if (!NEAR(msg.camPosePitch_rad - msg.headAngle, DEG_TO_RAD(-4.f),
                  pitchThresh_rad)) {
          PRINT_NAMED_WARNING("BehaviorFactoryCentroidExtractor.CamPose",
        ...
        if (!NEAR(msg.camPoseX_mm, 0, xThresh_mm)) {
          PRINT_NAMED_WARNING("BehaviorFactoryCentroidExtractor.CamPose",
                              "xTrans exceeds threshold");
          exceedsThresh = true;
        }
        if (!NEAR(msg.camPoseY_mm, 0, yThresh_mm)) {
          PRINT_NAMED_WARNING("BehaviorFactoryCentroidExtractor.CamPose",
                              "yTrans exceeds threshold");
          exceedsThresh = true;
        }
        if (!NEAR(msg.camPoseZ_mm, 0, zThresh_mm)) {
          PRINT_NAMED_WARNING("BehaviorFactoryCentroidExtractor.CamPose",
                              "zTrans exceeds threshold");
          exceedsThresh = true;
        }
    ...
    PRINT_NAMED_WARNING("BehaviorFactoryCentroidExtractor.HandleWhileRunning",
                        "Received event with tag %hhu not handling",
