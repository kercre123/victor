...
BehaviorPlaypenDriveForwards::BehaviorPlaypenDriveForwards(
    const Json::Value& config)
    : IBehaviorPlaypen(config) {
  SubscribeToTags(
      {{EngineToGameTag::RobotStopped, EngineToGameTag::CliffEvent}});
}
...
Result BehaviorPlaypenDriveForwards::OnBehaviorActivatedInternal() {
  ...
  // Clear and pause cliff sensor component so it doesn't try to update the
  // cliff thresholds
  robot.GetCliffSensorComponent().SetPause(true);
  CliffSensorComponent::CliffSensorDataArray thresholds = {
  ...
  // Record mics while driving forwards for at least how long it should take to
  // complete the 3 drives
  const u32 recordTime_s = ((PlaypenConfig::kDistanceToTriggerFrontCliffs_mm /
                             PlaypenConfig::kCliffSpeed_mmps) +
                            (PlaypenConfig::kDistanceToTriggerBackCliffs_mm /
                             PlaypenConfig::kCliffSpeed_mmps) +
                            (PlaypenConfig::kDistanceToDriveOverCliff_mm /
                             DEFAULT_PATH_MOTION_PROFILE.speed_mmps));

  robot.SendMessage(
      RobotInterface::EngineToRobot(RobotInterface::StartRecordingMicsRaw(
          Util::SecToMilliSec((float)recordTime_s), false,
          GetLogger().GetLogName() + "wheels")));

  // Drive fowards some amount until front cliffs trigger, which will cause the
  // action to fail with CANCELLED_WHILE_RUNNING
  MoveHeadToAngleAction* headToZero = new MoveHeadToAngleAction(0);
  // Move lift up a little so it doesn't rub against the ground as we drive off
  // the charger
  MoveLiftToHeightAction* liftDown =
      new MoveLiftToHeightAction(LIFT_HEIGHT_LOWDOCK + 10);
  DriveStraightAction* driveForwards =
      new DriveStraightAction(PlaypenConfig::kDistanceToTriggerFrontCliffs_mm,
                              PlaypenConfig::kCliffSpeed_mmps);
  driveForwards->SetShouldPlayAnimation(false);

  CompoundActionParallel* action =
      new CompoundActionParallel({headToZero, liftDown, driveForwards});
  ...
  DelegateIfInControl(action, [this](ActionResult result) {
    ...
    // Action should only complete with CANCELLED_WHILE_RUNNING, if it completes
    // in any other manner then fail
    if (result != ActionResult::CANCELLED_WHILE_RUNNING) {
      PRINT_NAMED_WARNING(
          "BehaviorPlaypenDriveForwards.InternalInitInternal.ActionCompleted",
          "Expected DriveStraightAction to fail with %s but instead completed "
          "with %s",
    ...
    // We have a delay between the action being cancelled from the RobotStopped
    // event and when the cliff event will be confirmed and sent from the robot
    // so we need to wait a little bit to receive it
    AddTimer(PlaypenConfig::kTimeToWaitForCliffEvent_ms, [this]() {
      // If front cliff was successfully detected then move onto waiting for the
      // back cliffs to be detected otherwise fail
...
BehaviorPlaypenDriveForwards::PlaypenUpdateInternal() {
  // There are times during this behavior that we are not acting (waiting for a
  // message to come from the robot) so we need to keep Running using this empty
  // PlaypenUpdateInternal
...
void BehaviorPlaypenDriveForwards::TransitionToWaitingForBackCliffs() {
  // We have seen the front cliffs fire so we need to drive forwards until the
  // back cliffs fire
  DriveStraightAction* action =
      new DriveStraightAction(PlaypenConfig::kDistanceToTriggerBackCliffs_mm,
                              PlaypenConfig::kCliffSpeed_mmps);
  ...
  DelegateIfInControl(action, [this](ActionResult result) {
    ...
    if (result != ActionResult::CANCELLED_WHILE_RUNNING) {
      PRINT_NAMED_WARNING(
          "BehaviorPlaypenDriveForwards.TransitionToWaitingForBackCliffs."
          "ActionCompleted",
          "Expected DriveStraightAction to fail with %s but instead completed "
          "with %s",
          EnumToString(ActionResult::CANCELLED_WHILE_RUNNING),
    ...
    // so we need to wait a little bit to receive it
    AddTimer(PlaypenConfig::kTimeToWaitForCliffEvent_ms, [this]() {
      // If back cliffs were successfully detected then transition to waiting
      // for them to be undetected inidicating we are no longer on the cliff
      // section of playpen
      if (!_backCliffsDetected) {
        PRINT_NAMED_WARNING(
            "BehaviorPlaypenDriveForwards.CliffEventTimeout."
            "BackCliffsNotDetected",
            "");
        PLAYPEN_SET_RESULT(FactoryTestResultCode::BACK_CLIFFS_NOT_DETECTED);
      }

      // Front cliffs should have already been detected so fail if for some
      // reason they haven't
      if (!_frontCliffsDetected) {
        PRINT_NAMED_ERROR(
            "BehaviorPlaypenDriveForwards.CliffEventTimeout."
            "FrontCliffsNotDetected",
            "Back cliffs are detected but front cliffs are not, this shouldn't "
            "be possible");
        PLAYPEN_SET_RESULT(FactoryTestResultCode::FRONT_CLIFFS_NOT_DETECTED);
      }
...
void BehaviorPlaypenDriveForwards::TransitionToWaitingForBackCliffUndetected() {
  ...
  DriveStraightAction* driveAction =
      new DriveStraightAction(PlaypenConfig::kDistanceToDriveOverCliff_mm);
  driveAction->SetShouldPlayAnimation(false);
  driveAction->SetMotionProfile(DEFAULT_PATH_MOTION_PROFILE);

  MoveLiftToHeightAction* liftAction =
      new MoveLiftToHeightAction(LIFT_HEIGHT_LOWDOCK);

  CompoundActionParallel* action =
      new CompoundActionParallel({driveAction, liftAction});

  DelegateIfInControl(action, [this, &robot]() {
    AddTimer(PlaypenConfig::kTimeToWaitForCliffEvent_ms, [this, &robot]() {
      // We should end up in the NONE state after successfully undetecting the
      // back cliffs We also expect the above DriveStraighAction to complete
      // successfully in order to get here
      if (!(_waitingForCliffsState == NONE && _frontCliffsDetected &&
            _backCliffsDetected)) {
        PRINT_NAMED_WARNING(
            "BehaviorPlaypenDriveForwards."
            "TransitionToWaitingForBackCliffUndetected.WrongState",
      ...
      PLAYPEN_TRY(GetLogger().AppendCliffValuesOnGround(cliffVals),
...
void BehaviorPlaypenDriveForwards::HandleWhileActivatedInternal(
  ...
  if (tag == EngineToGameTag::CliffEvent) {
    ...
    PRINT_CH_INFO("Behaviors", "BehaviorPlaypenDriveForwards.CliffEvent",
                  ...
                  cliffDetected, cliffData[(u16)CliffSensor::CLIFF_FR],
                  cliffData[(u16)CliffSensor::CLIFF_FL],
                  cliffData[(u16)CliffSensor::CLIFF_BR],
                  cliffData[(u16)CliffSensor::CLIFF_BL]);
    ...
    const bool frontCliffsDetected =
        (payload.detectedFlags & FR) && (payload.detectedFlags & FL);
    const bool backCliffsDetected =
        (payload.detectedFlags & BR) && (payload.detectedFlags & BL);
    ...
    switch (_waitingForCliffsState) {
      ...
      case WAITING_FOR_FRONT_CLIFFS: {
        // While waiting for front cliffs we expect to only get a cliff event
        // that has seen a front cliff otherwise we will fail
        if (!cliffDetected) {
          PRINT_NAMED_WARNING(
              "BehaviorPlaypenDriveForwards.WaitingForFrontCliffs."
              "CliffUndetected",
        ...
        const CliffSensorValues cliffVals(
        ...
        PRINT_CH_INFO(
            "Behaviors",
            "BehaviorPlaypenDriveForwards.WaitingForFrontCliffs.CliffDetected",
            "");
      ...
      case WAITING_FOR_FRONT_CLIFFS_UNDETECTED: {
        // If a cliff has been detected while waiting for cliffs to be
        // undetected then fail
        if (cliffDetected) {
          PRINT_NAMED_WARNING(
              "BehaviorPlaypenDriveForwards.WaitingForFrontCliffsUndetected."
              "CliffDetected",
        ...
        PRINT_CH_INFO("Behaviors",
                      "BehaviorPlaypenDriveForwards."
                      "WaitingForFrontCliffsUndetected.CliffUndetected",
      ...
      case WAITING_FOR_BACK_CLIFFS: {
        if (!cliffDetected) {
          PRINT_NAMED_WARNING(
              "BehaviorPlaypenDriveForwards.WaitingForBackCliffs."
              "CliffUndetected",
      ...
      case WAITING_FOR_BACK_CLIFFS_UNDETECTED: {
        if (cliffDetected) {
          PRINT_NAMED_WARNING(
              "BehaviorPlaypenDriveForwards.WaitingForBackCliffsUndetected."
              "CliffDetected",
        ...
        PRINT_CH_INFO("Behaviors",
                      "BehaviorPlaypenDriveForwards."
                      "WaitingForBackCliffsUndetected.CliffUndetected",
