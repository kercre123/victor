...
// The playpen behaviors should be written in such a way so that they can
// continue running even after something has gone wrong (basically no branching)
...
// Threshold on calculated distance to distance sensor target (using detected
// marker)
// +/- this from the expected distance to the object/marker defined in the
// distance sensor behavior json file
static f32 kVisualDistanceToDistanceSensorObjectThresh_mm = 30;

// Threshold on the raw distance sensor reading from the expected distance
// defined in the distance sensor behavior json file
static f32 kDistanceSensorReadingThresh_mm = 20;

// Bias adjustment for raw distance sensor reading when comparing to visual
// distance
static f32 kDistanceSensorBiasAdjustment_mm = 30;

// Angle to move the head to when doing the distance sensor check
// Whiskey ToF is angled 11 degrees down so move head to 11 degrees so ToF is
// looking forwards
...
// Which calibration target we are using (see
// CameraCalibrator::CalibTargetTypes)
static u32 kPlaypenCalibTarget = 2;  // 1 = INVERTED_BOX 2 = QBERT (I don't feel
                                     // like including cameraCalibrator.h here)
...
// Which marker we should wait to be seeing before automatically starting camera
// calibration
static CustomObjectMarker kMarkerToTriggerCalibration =
    CustomObjectMarker::Triangles2;

// ----------Drive Forwards----------
// Distance to drive forwards to trigger the front cliff sensors
static f32 kDistanceToTriggerFrontCliffs_mm = 140;

//  Distance to drive forwards to trigger the back cliff sensors after trigger
//  the front ones
static f32 kDistanceToTriggerBackCliffs_mm = 80;

// Distance to drive forwards to have all cliff sensors on the ground after
// triggering the back cliff sensors
...
// Set threshold for cliff detection while the Drive Forwards behavior is
// running
