...
Result BehaviorPlaypenDriftCheck::OnBehaviorActivatedInternal() {
  ...
  MoveHeadToAngleAction* moveHeadToAngle =
      new MoveHeadToAngleAction(PlaypenConfig::kHeadAngleForDriftCheck);
  MoveLiftToHeightAction* moveLiftUp =
      new MoveLiftToHeightAction(LIFT_HEIGHT_CARRY);

  CompoundActionSequential* headUpDown =
      new CompoundActionSequential({moveHeadUp, moveHeadToAngle});
  CompoundActionParallel* liftAndHead =
      new CompoundActionParallel({headUpDown, moveLiftUp});

  // After moving head and lift, wait to ensure that audio recording has stopped
  // before transitioning to playing the sound and starting more recording
  CompoundActionSequential* action = new CompoundActionSequential(
      {liftAndHead, new WaitAction(Util::MilliSecToSec(
                        (float)PlaypenConfig::kDurationOfAudioToRecord_ms))});
...
void BehaviorPlaypenDriftCheck::TransitionToStartDriftCheck() {
  ...
  // Record intial starting orientation and after kIMUDriftDetectPeriod_ms check
  // for drift
  _startingRobotOrientation =
...
void BehaviorPlaypenDriftCheck::CheckDrift() {
  ...
  f32 angleChange = std::fabsf(
      (robot.GetPose().GetRotationMatrix().GetAngleAroundAxis<'Z'>() -
       _startingRobotOrientation)
          .getDegrees());

  // Write drift rate and imu temperature change during drift detection period
  // to robot
  IMUInfo imuInfo;
  imuInfo.driftRate_degPerSec =
      angleChange /
      Util::MilliSecToSec((float)PlaypenConfig::kIMUDriftDetectPeriod_ms);
  _imuTemp.tempEnd_c = robot.GetImuTemperature();
  _imuTemp.duration_ms =
      BaseStationTimer::getInstance()->GetCurrentTimeStamp() -
      _imuTemp.duration_ms;
  imuInfo.tempDuration = std::move(_imuTemp);

  WriteToStorage(robot, NVStorage::NVEntryTag::NVEntry_IMUInfo, (u8*)&imuInfo,
                 sizeof(imuInfo), FactoryTestResultCode::IMU_INFO_WRITE_FAILED);

  // Write drift rate to log
  PLAYPEN_TRY(GetLogger().Append(imuInfo),
              FactoryTestResultCode::WRITE_TO_LOG_FAILED);

  if (angleChange > PlaypenConfig::kIMUDriftAngleThreshDeg) {
    PRINT_NAMED_WARNING(
        "BehaviorPlaypenDriftCheck.CheckDrift.DriftDetected",
        "Angle change of %f deg detected in %f seconds", angleChange,
        Util::MilliSecToSec((float)PlaypenConfig::kIMUDriftDetectPeriod_ms));
...
BehaviorPlaypenDriftCheck::PlaypenUpdateInternal() {
