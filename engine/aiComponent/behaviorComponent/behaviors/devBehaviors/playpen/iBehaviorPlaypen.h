...
class IBehaviorPlaypen : public ICozmoBehavior {
 ...
 public:
  ...
  // complete
  FactoryTestResultCode GetResult() {
    if (_recordingTouch) {
      return FactoryTestResultCode::UNKNOWN;
    }
    return _result;
  }
  ...
  static const std::map<std::string, std::vector<FactoryTestResultCode>>&
  GetAllPlaypenResults();
 ...
 protected:
  ...
  enum PlaypenStatus { Failure, Running, Complete };
  ...
  // updates PlaypenUpdateInternal() is provided for the subclass to override if
  // they wish
  ...
  virtual PlaypenStatus PlaypenUpdateInternal() {
  ...
  // Attempts to write to robot storage, if enabled, and will fail with
  // failureCode if writing fails
  void WriteToStorage(Robot& robot, NVStorage::NVEntryTag tag, const u8* data,
                      size_t size, FactoryTestResultCode failureCode);

  // Use the macro PLAYPEN_SET_RESULT if you can instead of directly calling
  // this function
  ...
  // Starts recording touch sensor data and will automatically stop once
  // 'kDurationOfTouchToRecord_ms' time has passed
