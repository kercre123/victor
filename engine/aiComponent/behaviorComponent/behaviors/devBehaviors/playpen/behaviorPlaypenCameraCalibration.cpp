...
static const std::shared_ptr<Vision::CameraCalibration> kApproxCalib(
    new Vision::CameraCalibration(360, 640, 296, 296, 319, 179, 0.f,
                                  std::vector<f32>({0.f, 0.f, 0.f, 0.f, 0.f,
                                                    0.f, 0.f, 0.f})));
#else
// V2 Physical camera calibration
static const std::shared_ptr<Vision::CameraCalibration> kApproxCalib(
    new Vision::CameraCalibration(360, 640, 362, 364, 303, 196, 0.f,
                                  std::vector<f32>({-0.1, -0.15, 0.00005,
                                                    -0.0001, 0.05, 0.f, 0.f,
                                                    0.f})));
#endif
...
BehaviorPlaypenCameraCalibration::BehaviorPlaypenCameraCalibration(
...
Result BehaviorPlaypenCameraCalibration::OnBehaviorActivatedInternal() {
  ...
  // Set CameraCalibrator's CalibTargetType console var to what the playpen
  // config says
  NativeAnkiUtilConsoleSetValueWithString(
      "CalibTargetType",
      std::to_string(PlaypenConfig::kPlaypenCalibTarget).c_str());

  // Define a custom object with marker kMarkerToTriggerCalibration so we can
  // know when we are seeing the calibration target via a RobotObservedObject
  // message
  CustomObject* customCube = CustomObject::CreateCube(
      ObjectType::CustomType00, PlaypenConfig::kMarkerToTriggerCalibration,
      PlaypenConfig::kCalibMarkerCubeSize_mm,
      PlaypenConfig::kCalibMarkerSize_mm, PlaypenConfig::kCalibMarkerSize_mm,
      true);
  ...
  if (!robot.GetVisionComponent().IsCameraCalibrationSet()) {
    PRINT_CH_INFO("Behaviors",
                  "BehaviorPlaypenCameraCalibration.SettingFakeCalib", "");
  ...
  CompoundActionParallel* action = new CompoundActionParallel(
      {new MoveHeadToAngleAction(PlaypenConfig::kHeadAngleToSeeTarget_rad),
       new MoveLiftToHeightAction(LIFT_HEIGHT_LOWDOCK)});

  DelegateIfInControl(action, [this]() {
    AddTimer(
        PlaypenConfig::kTimeoutWaitingForTarget_ms,
        [this]() {
          PLAYPEN_SET_RESULT(
              FactoryTestResultCode::NOT_SEEING_CALIB_TARGET_TIMEOUT);
        },
        kWaitingForTargetTimer);
  });
...
BehaviorPlaypenCameraCalibration::PlaypenUpdateInternal() {
  // DEPRECATED - Grabbing robot to support current cozmo code, but this should
  // be removed
  Robot& robot = GetBEI().GetRobotInfo()._robot;

  if (!_computingCalibration) {
    // If we are seeing the target and our head is not moving and we haven't yet
    // taken a calibration image then store the next image for camera
    // calibration Note: Technically don't need to store a calibration image as
    // single image calibration just uses markers detected from the current
    // image, however, we need to store an image so we can grab it later to
    // write to log
    if (!_waitingToStoreImage && !IsControlDelegated() &&
    ...
    // Otherwise if we have a calibration image, start computing calibration
      ...
      AddTimer(
          PlaypenConfig::kTimeoutForCalibration_ms,
          [this, &robot]() {
            // We have at least seen the calibration target so we should at
            // least save the image we were trying to use for calibration before
            // failing with calibration timeout
            std::list<std::vector<u8> > rawJpegData =
                robot.GetVisionComponent().GetCalibrationImageJpegData(nullptr);

            // Verify number of images
            // 2 images, the second is the undistorted version of the first
            const u32 NUM_CAMERA_CALIB_IMAGES = 2;
            bool invalidNumCalibImages =
                rawJpegData.size() != NUM_CAMERA_CALIB_IMAGES;
            if (invalidNumCalibImages) {
              PRINT_NAMED_WARNING(
                  "BehaviorPlaypenCameraCalibration.HandleCameraCalibration."
                  "InvalidNumCalibImagesFound",
            ...
            PLAYPEN_TRY(
                GetLogger().AddFile("calibImage.jpg", *rawJpegData.begin()),
...
void BehaviorPlaypenCameraCalibration::OnBehaviorDeactivated() {
  ...
  // Make sure to disable computing calibration mode in case it was left on for
  // some reason
  ...
  // Clear all objects from blockworld since calib target contains markers it
  // will cause objects to get added to the world
...
void BehaviorPlaypenCameraCalibration::HandleWhileActivatedInternal(
...
void BehaviorPlaypenCameraCalibration::HandleCameraCalibration(
  ...
  std::shared_ptr<Vision::CameraCalibration> camCalib(
      new Vision::CameraCalibration(
          calibMsg.nrows, calibMsg.ncols, calibMsg.focalLength_x,
          calibMsg.focalLength_y, calibMsg.center_x, calibMsg.center_y,
          calibMsg.skew, calibMsg.distCoeffs));

  robot.GetVisionComponent().SetCameraCalibration(camCalib);

  // Write camera calibration to log
  PLAYPEN_TRY(GetLogger().Append(calibMsg),
              FactoryTestResultCode::WRITE_TO_LOG_FAILED);

  // Write camera calibration to robot
  u8 buf[calibMsg.Size()];
  size_t numBytes = calibMsg.Pack(buf, sizeof(buf));
  WriteToStorage(robot, NVStorage::NVEntryTag::NVEntry_CameraCalib, buf,
                 numBytes, FactoryTestResultCode::CAMERA_CALIB_WRITE_FAILED);
  ...
  WriteToStorage(robot, NVStorage::NVEntryTag::NVEntry_CalibImage1,
                 rawJpegData.begin()->data(), rawJpegData.begin()->size(),
                 FactoryTestResultCode::CALIB_IMAGES_WRITE_FAILED);

  // Write calibration image to log
  PLAYPEN_TRY(
      GetLogger().AddFile("calibImageUndistort.jpg", *(++rawJpegData.begin())),
      FactoryTestResultCode::WRITE_TO_LOG_FAILED);

  // Check if calibration values are sane
  if (!Util::InRange(calibMsg.focalLength_x,
                     kApproxCalib->GetFocalLength_x() -
                         PlaypenConfig::kFocalLengthTolerance,
                     kApproxCalib->GetFocalLength_x() +
                         PlaypenConfig::kFocalLengthTolerance) ||
      !Util::InRange(calibMsg.focalLength_y,
                     kApproxCalib->GetFocalLength_y() -
                         PlaypenConfig::kFocalLengthTolerance,
                     kApproxCalib->GetFocalLength_y() +
                         PlaypenConfig::kFocalLengthTolerance) ||
      !Util::InRange(
    ...
    PRINT_NAMED_WARNING(
        "BehaviorPlaypenCameraCalibration.HandleCameraCalibration.Intrinsics."
        "OOR",
        "focalLength (%f, %f), center (%f, %f)", calibMsg.focalLength_x,
        calibMsg.focalLength_y, calibMsg.center_x, calibMsg.center_y);
  ...
  if (!Util::InRange(calibMsg.distCoeffs[0],
                     kApproxCalib->GetDistortionCoeffs()[0] -
                         PlaypenConfig::kRadialDistortionTolerance,
                     kApproxCalib->GetDistortionCoeffs()[0] +
                         PlaypenConfig::kRadialDistortionTolerance) ||
      !Util::InRange(calibMsg.distCoeffs[1],
                     kApproxCalib->GetDistortionCoeffs()[1] -
                         PlaypenConfig::kRadialDistortionTolerance,
                     kApproxCalib->GetDistortionCoeffs()[1] +
                         PlaypenConfig::kRadialDistortionTolerance) ||
      !Util::InRange(calibMsg.distCoeffs[2],
                     kApproxCalib->GetDistortionCoeffs()[2] -
                         PlaypenConfig::kTangentialDistortionTolerance,
                     kApproxCalib->GetDistortionCoeffs()[2] +
                         PlaypenConfig::kTangentialDistortionTolerance) ||
      !Util::InRange(calibMsg.distCoeffs[3],
                     kApproxCalib->GetDistortionCoeffs()[3] -
                         PlaypenConfig::kTangentialDistortionTolerance,
                     kApproxCalib->GetDistortionCoeffs()[3] +
                         PlaypenConfig::kTangentialDistortionTolerance) ||
      !Util::InRange(calibMsg.distCoeffs[4],
                     kApproxCalib->GetDistortionCoeffs()[4] -
                         PlaypenConfig::kRadialDistortionTolerance,
                     kApproxCalib->GetDistortionCoeffs()[4] +
                         PlaypenConfig::kRadialDistortionTolerance) ||
      calibMsg.distCoeffs[5] != 0.f || calibMsg.distCoeffs[6] != 0.f ||
    ...
    PRINT_NAMED_WARNING(
        "BehaviorPlaypenCameraCalibration.HandleCameraCalibration.Distortions."
        "OOR",
        "%s", ss.str().c_str());
  ...
  DelegateIfInControl(new MoveHeadToAngleAction(0), [this]() {
    PLAYPEN_SET_RESULT(FactoryTestResultCode::SUCCESS);
  });
}

void BehaviorPlaypenCameraCalibration::HandleRobotObservedObject(
