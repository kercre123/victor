...
template <>
ConsoleVar<Vector::CustomObjectMarker>::ConsoleVar(
    Vector::CustomObjectMarker& value, const char* id, const char* category,
...
template <>
bool ConsoleVar<Vector::CustomObjectMarker>::ParseText(const char* text) {
  _value = Vector::CustomObjectMarkerFromString(text);
  return true;
}

template <>
std::string ConsoleVar<Vector::CustomObjectMarker>::ToString() const {
  return Vector::EnumToString(_value);
}

template <>
std::string ConsoleVar<Vector::CustomObjectMarker>::GetDefaultAsString() const {
  return Vector::EnumToString(_defaultValue);
}

template <>
int64_t ConsoleVar<Vector::CustomObjectMarker>::GetMinAsInt64() const {
  return numeric_cast_clamped<int64_t>((int)_minValue);
}

template <>
int64_t ConsoleVar<Vector::CustomObjectMarker>::GetMaxAsInt64() const {
  return numeric_cast_clamped<int64_t>((int)_maxValue);
}

template <>
uint64_t ConsoleVar<Vector::CustomObjectMarker>::GetMinAsUInt64() const {
  return numeric_cast_clamped<uint64_t>((int)_minValue);
}

template <>
uint64_t ConsoleVar<Vector::CustomObjectMarker>::GetMaxAsUInt64() const {
  return numeric_cast_clamped<uint64_t>((int)_maxValue);
}

template <>
void ConsoleVar<Vector::CustomObjectMarker>::ToggleValue() {
  _value = (Vector::CustomObjectMarker)(!(((bool)(_value))));
}
...
WRAP_EXTERN_CONSOLE_VAR(CustomObjectMarker, kMarkerToTriggerCalibration,
                        "Playpen");
