...
BehaviorPlaypenDistanceSensor::BehaviorPlaypenDistanceSensor(
  ...
  if (!res) {
    PRINT_NAMED_ERROR(
        "BehaviorPlaypenDistanceSensor.Constructor.MissingConfigKey",
  ...
  res = JsonTools::GetValueOptional(config, kExpectedDistKey,
  ...
  res = JsonTools::GetValueOptional(config, kPerformCalibrationKey,
                                    _performCalibration);
  if (!res) {
    PRINT_NAMED_ERROR(
        "BehaviorPlaypenDistanceSensor.Constructor.MissingConfigKey",
        "Missing %s key from PlaypenDistanceSensor Config",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorPlaypenDistanceSensor::GetBehaviorJsonKeys(
    std::set<const char*>& expectedKeys) const {
  const char* list[] = {
      kAngleToTurnKey,  kDistToDriveKey,        kExpectedObjectKey,
      kExpectedDistKey, kPerformCalibrationKey,
  };
...
Result BehaviorPlaypenDistanceSensor::OnBehaviorActivatedInternal() {
  ...
  if (_expectedObjectType == ObjectType::UnknownObject) {
    PRINT_NAMED_WARNING(
        "BehaviorPlaypenDistanceSensor.InternalInitInternal.NoObject",
        "No object type specified to look for, failing");
    PLAYPEN_SET_RESULT_WITH_RETURN_VAL(FactoryTestResultCode::CUBE_NOT_FOUND,
                                       RESULT_FAIL);
  }
  ...
  // Add an extra 30 seconds to timeout time if we are calibrating since it is
  // slow
  IncreaseTimeoutTimer(30000);

  // Move head and lift to be able to see target marker and turn towards the
  // target
  ...
  DriveStraightAction* drive =
      new DriveStraightAction((_distToDrive_mm > 0 ? _distToDrive_mm : 0));
  CompoundActionParallel* liftHeadDrive =
      new CompoundActionParallel({lift, head, drive});

  TurnInPlaceAction* turn =
      new TurnInPlaceAction(_angleToTurn.ToFloat(), false);
  ...
  CompoundActionSequential* action =
      new CompoundActionSequential({liftHeadDrive, turn, wait});
...
BehaviorPlaypenDistanceSensor::PlaypenUpdateInternal() {
  // DEPRECATED - Grabbing robot to support current cozmo code, but this should
  // be removed
  Robot& robot = GetBEI().GetRobotInfo()._robot;
  ...
  // We have distance readings left to record
    ...
    const auto& rangeData =
        robot.GetRangeSensorComponent().GetLatestRawRangeData(isNewData);
    if (isNewData) {
      --_numRecordedReadingsLeft;
      PRINT_NAMED_INFO(
          "BehaviorPlaypenDistanceSensor.PlaypenUpdateInternal.Recording",
          "Recording %u more", _numRecordedReadingsLeft);
      ...
      if (!GetLogger().Append(GetDebugLabel(), data)) {
        PLAYPEN_SET_RESULT_WITH_RETURN_VAL(
            FactoryTestResultCode::WRITE_TO_LOG_FAILED, PlaypenStatus::Running);
      }
    }

    // TODO NEED TO REWRITE TO CHECK ALL ROIs
    // if(robot.IsPhysical() &&
    //    !Util::IsNear(data.proxSensorData.distance_mm -
    ...
    //                       "Sensor reading %u - %f not near visual reading %f
    //                       with threshold %f",
    ...
    //   PLAYPEN_SET_RESULT_WITH_RETURN_VAL(FactoryTestResultCode::DISTANCE_SENSOR_OOR,
    //   PlaypenStatus::Running);
    // }
  ...
  // We've recorded all distance readings we need to
    ...
    // If we aren't acting then turn back to where we were facing when the
    // behavior started
...
void BehaviorPlaypenDistanceSensor::TransitionToRefineTurn() {
  ...
  if (res) {
    ...
    if (!Util::IsNear(
            distToMarker_mm, _expectedDistanceToObject_mm,
            PlaypenConfig::kVisualDistanceToDistanceSensorObjectThresh_mm)) {
      PRINT_NAMED_WARNING(
          "BehaviorPlaypenDistanceSensor.TransitionToRefineTurn.MarkerOOR",
          "Observing distance sensor marker at %fmm, expected distance is %fmm "
          "+/- %fmm",
          distToMarker_mm, _expectedDistanceToObject_mm,
          PlaypenConfig::kVisualDistanceToDistanceSensorObjectThresh_mm);
    ...
    // We are within expected distance to update refined turn angle to put us
    // perpendicular with the marker
    else {
      markerPose = markerPose.GetWithRespectToRoot();
      // Marker pose rotation is kind of wonky, compared to the robot's rotation
      // they are rotated 90 degrees. So when the robot is looking at a marker,
      // you have to add 90 degrees to get its rotation to match that of the
      // robot Taking the difference of these two angles tells us how much the
      // robot needs to turn to be perpendicular with the marker
      angle =
          ((markerPose.GetRotation().GetAngleAroundZaxis() + DEG_TO_RAD(90)) -
           robot.GetPose().GetRotation().GetAngleAroundZaxis());
    }

    PRINT_CH_INFO(
        "Behaviors",
        "BehaviorPlaypenDistanceSensor.TransitionToRefineTurn.TurnAngle",
        "Turning %f degrees to be perpendicular to marker", angle.getDegrees());
  ...
  // Once we are perpendicular to the marker, start recording distance sensor
  // readings
  DelegateIfInControl(action.release(),
                      [this]() { TransitionToRecordSensor(); });
}
...
void BehaviorPlaypenDistanceSensor::TransitionToRecordSensor() {
  ...
  if (res) {
    ...
    if (visualDistanceToTarget_mm <= 0) {
      PRINT_NAMED_WARNING(
          "BehaviorPlaypenDistanceSensor.TransitionToRecordSensor.ZeroDist",
    ...
    // Marker pose rotation is kind of wonky, compared to the robot's rotation
    // they are rotated 90 degrees. So when the robot is looking at a marker,
    // you have to add 90 degrees to get its rotation to match that of the robot
    // Taking the difference of these two angles tells us how much the robot
    // needs to turn to be perpendicular with the marker
    const auto angle =
        ((markerPose.GetRotation().GetAngleAroundZaxis() + DEG_TO_RAD(90)) -
         robot.GetPose().GetRotation().GetAngleAroundZaxis());
  ...
  if (_performCalibration) {
    ...
    MoveHeadToAngleAction* head =
        new MoveHeadToAngleAction(PlaypenConfig::kDistanceSensorHeadAngle_rad);
    action->AddAction(head);

    action->AddAction(new WaitAction(3.f));

    WaitForLambdaAction* wait = new WaitForLambdaAction(
        [this, visualDistanceToTarget_mm](Robot& robot) {
          ...
          if (!isCalibrating && !_calibrationComplete) {
            ToFSensor::getInstance()->PerformCalibration(
                visualDistanceToTarget_mm,
                PlaypenConfig::kDistanceSensorTargetReflectance,
                [this](ToFSensor::CommandResult res) {
                  if (res != ToFSensor::CommandResult::Success) {
                    PLAYPEN_SET_RESULT(
                        FactoryTestResultCode::CALIBRATE_TOF_FAILED);
                  }
                });
          }
    ...
    DelegateIfInControl(action, [this]() {
      _numRecordedReadingsLeft =
          PlaypenConfig::kNumDistanceSensorReadingsToRecord;
    });
  } else {
    ...
    MoveHeadToAngleAction* head =
        new MoveHeadToAngleAction(PlaypenConfig::kDistanceSensorHeadAngle_rad);
    CompoundActionSequential* action =
        new CompoundActionSequential({turn, head});
...
void BehaviorPlaypenDistanceSensor::TransitionToTurnBack() {
  ...
  TurnInPlaceAction* turn =
      new TurnInPlaceAction(_startingAngle.ToFloat(), true);
  DriveStraightAction* drive =
      new DriveStraightAction((_distToDrive_mm < 0 ? _distToDrive_mm : 0));
  CompoundActionSequential* action = new CompoundActionSequential();
  ...
  DelegateIfInControl(
      action, [this]() { PLAYPEN_SET_RESULT(FactoryTestResultCode::SUCCESS); });
}

bool BehaviorPlaypenDistanceSensor::GetExpectedObjectMarkerPoseWrtRobot(
  ...
  if (objects.empty()) {
    PRINT_NAMED_WARNING(
        "BehaviorPlaypenDistanceSensor.GetExpectedObjectMarkerPoseWrtRobot."
        "NullObject",
        "Expected object of type %s does not exist or am not seeing it",
        ObjectTypeToString(_expectedObjectType));
    PLAYPEN_SET_RESULT_WITH_RETURN_VAL(
        FactoryTestResultCode::DISTANCE_MARKER_NOT_FOUND, false);
    return false;
  } else {
    ...
    if (object == nullptr) {
      PRINT_CH_INFO("Behaviors",
                    ...
                    "GetExpectedObjectMarkerPoseWrtRobot.NullObject",
                    "");
      return false;
    }
    ...
    if (!Util::IsNear((float)lastObservedTime,
                      (float)robot.GetLastImageTimeStamp(), 500.f)) {
      PRINT_CH_INFO("Behaviors",
                    "BehaviorPlaypenDistanceSensor."
                    "GetExpectedObjectMarkerPoseWrtRobot.MarkerTooOld",
                    "%u %u", (u32)lastObservedTime,
                    (u32)robot.GetLastImageTimeStamp());
      return false;
    }

    const bool res =
        markerPose.GetWithRespectTo(robot.GetPose(), markerPoseWrtRobot);
    if (!res) {
      PRINT_NAMED_WARNING(
          "BehaviorPlaypenDistanceSensor.GetExpectedObjectMarkerPoseWrtRobot."
          "NoClosestMarker",
