...
namespace {
static const Pose3d kExpectedCubePose =
    Pose3d(0, Z_AXIS_3D(),
           {PlaypenConfig::kExpectedCubePoseX_mm,
            PlaypenConfig::kExpectedCubePoseY_mm, 22},
           "");

// Rotation ambiguities for observed blocks.
// We only care that the block is upright.
static const RotationAmbiguities kBlockRotationAmbiguities(
    true, {RotationMatrix3d({1, 0, 0, 0, 1, 0, 0, 0, 1}),
...
Result BehaviorPlaypenPickupCube::OnBehaviorActivatedInternal() {
  ...
  // TurnInPlaceAction* turn = new TurnInPlaceAction(robot, DEG_TO_RAD(-90),
  // false); CompoundActionParallel* action = new CompoundActionParallel(robot,
  // {head, turn});
...
void BehaviorPlaypenPickupCube::TransitionToWaitForCube() {
  ...
  // Clear all objects from blockworld since the marker for the distance sensor
  // check creates an object that is in the way of the pickup object's predock
  // pose
...
void BehaviorPlaypenPickupCube::TransitionToPickupCube() {
  ...
  if (object == nullptr) {
    ...
    // Should we be ignoring playpen failures we will need a valid object to do
    // stuff with so make a ghost object
  ...
  // Store to robot
  WriteToStorage(robot, NVStorage::NVEntryTag::NVEntry_ObservedCubePose,
  ...
  Vec3f Tdiff;
  Radians angleDiff;
  if (!object->GetPose().IsSameAs_WithAmbiguity(
          _expectedCubePose, kBlockRotationAmbiguities,
          PlaypenConfig::kExpectedCubePoseDistThresh_mm,
          PlaypenConfig::kExpectedCubePoseAngleThresh_rad, Tdiff, angleDiff)) {
    PRINT_NAMED_WARNING(
        "BehaviorPlaypenPickupCube.InternalInitInternal.CubeNotWhereExpected",
        "actual: (x,y,deg) = %f, %f, %f; expected: %f %f %f; tdiff: %f %f %f; "
        "angleDiff (deg): %f",
        object->GetPose().GetTranslation().x(),
        object->GetPose().GetTranslation().y(),
        object->GetPose()
            .GetRotationMatrix()
            .GetAngleAroundAxis<'Z'>()
            .getDegrees(),
        kExpectedCubePose.GetTranslation().x(),
        kExpectedCubePose.GetTranslation().y(),
        kExpectedCubePose.GetRotationMatrix()
            .GetAngleAroundAxis<'Z'>()
            .getDegrees(),
  ...
  const f32 kObservedCubePoseHeightDiff_mm =
      object->GetPose().GetTranslation().z() - (0.5f * object->GetSize().z());
  if (std::fabsf(kObservedCubePoseHeightDiff_mm) >
      PlaypenConfig::kExpectedCubePoseHeightThresh_mm) {
    const f32 kObservedCubePoseHeight_mm =
        object->GetPose().GetTranslation().z();

    PRINT_NAMED_WARNING(
        "BehaviorPlaypenPickupCube.InternalInitInternal.CubeNotWhereExpectedZ",
        "%f %f", kObservedCubePoseHeight_mm, (0.5f * object->GetSize().z()));
  ...
  DelegateIfInControl(
      action, [this, objectID = object->GetID()](ActionResult result) {
        // DEPRECATED - Grabbing robot to support current cozmo code, but this
        // should be removed
        Robot& robot = GetBEI().GetRobotInfo()._robot;
...
void BehaviorPlaypenPickupCube::TransitionToPlaceCube() {
  ...
  Radians robotAngleAfterBackup =
      robot.GetPose().GetRotationMatrix().GetAngleAroundAxis<'Z'>();
  f32 angleChange_rad =
      std::fabsf((robotAngleAfterBackup - _robotAngleAtPickup).ToFloat());

  PRINT_CH_INFO(
      "Behaviors",
      "BehaviorPlaypenPickupCube.TransitionToPlaceCube.AngleChangeDuringBackup",
...
void BehaviorPlaypenPickupCube::TransitionToBackup() {
  ...
  DelegateIfInControl(action, [this]() {
    ...
    // Check that robot orientation didn't change much since it lifted up the
    // block
    Radians robotAngleAfterBackAndForth =
        robot.GetPose().GetRotation().GetAngleAroundZaxis();
    f32 angleChange_rad = std::fabsf(
        (robotAngleAfterBackAndForth - _robotAngleAtPickup).ToFloat());

    PRINT_CH_INFO("Behaviors",
                  "BehaviorPlaypenPickupCube.TransitionToBackup."
                  "AngleChangeDuringBackAndForth",
                  "%f deg", RAD_TO_DEG(angleChange_rad));
...
void BehaviorPlaypenPickupCube::HandleWhileActivatedInternal(
  ...
  if (tag == RobotInterface::RobotToEngineTag::pickAndPlaceResult) {
    ...
    if (payload.didSucceed &&
        payload.blockStatus == BlockStatus::BLOCK_PICKED_UP) {
      // DEPRECATED - Grabbing robot to support current cozmo code, but this
      // should be removed
      Robot& robot = GetBEI().GetRobotInfo()._robot;
