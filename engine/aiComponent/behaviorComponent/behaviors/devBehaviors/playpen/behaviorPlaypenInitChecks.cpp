...
Result BehaviorPlaypenInitChecks::OnBehaviorActivatedInternal() {
  ...
  // if(res != RESULT_OK)
  // {
  //   PLAYPEN_SET_RESULT_WITH_RETURN_VAL(FactoryTestResultCode::RAMPOST_ERROR,
  //   RESULT_FAIL);
  // }

  // Should not be seeing any cliffs
  if (robot.GetCliffSensorComponent().IsCliffDetected()) {
    PLAYPEN_SET_RESULT_WITH_RETURN_VAL(FactoryTestResultCode::CLIFF_UNEXPECTED,
                                       RESULT_FAIL);
  }
  ...
  if (!Util::InRange(rawTouchValue, PlaypenConfig::kMinExpectedTouchValue,
                     PlaypenConfig::kMaxExpectedTouchValue)) {
    PRINT_NAMED_WARNING("BehaviorPlaypenWaitToStart.OnActivated.TouchOOR",
                        "Min %u < Val %u < Max %u",
                        PlaypenConfig::kMinExpectedTouchValue, rawTouchValue,
                        PlaypenConfig::kMaxExpectedTouchValue);
    PLAYPEN_SET_RESULT_WITH_RETURN_VAL(FactoryTestResultCode::TOUCH_VALUES_OOR,
                                       RESULT_FAIL);
  }
  ...
  if (batteryVolts < PlaypenConfig::kMinBatteryVoltage) {
    PRINT_NAMED_WARNING("BehaviorPlaypenInitChecks.OnActivated.BatteryTooLow",
                        "%fv", batteryVolts);
