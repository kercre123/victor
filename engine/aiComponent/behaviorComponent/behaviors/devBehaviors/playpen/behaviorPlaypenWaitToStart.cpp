...
BehaviorPlaypenWaitToStart::BehaviorPlaypenWaitToStart(
    const Json::Value& config)
    : IBehaviorPlaypen(config) {
  // Subscribe to all of the base class's failure tags to prevent us from
  // automatically failing if we get put on the charger, motor calibration on
  // startup, etc...
...
IBehaviorPlaypen::PlaypenStatus
BehaviorPlaypenWaitToStart::PlaypenUpdateInternal() {
  // DEPRECATED - Grabbing robot to support current cozmo code, but this should
  // be removed
  Robot& robot = GetBEI().GetRobotInfo()._robot;
  ...
  if (robot.GetTouchSensorComponent().GetIsPressed() &&
  ...
  } else if (!robot.GetTouchSensorComponent().GetIsPressed() &&
             _touchStartTime_ms != 0) {
    _touchStartTime_ms = 0;
  ...
  // If we aren't using touchToStart or we are using it and we have been touched
  // long enough
  const bool touchGood = !PlaypenConfig::kUseTouchToStart ||
                         (PlaypenConfig::kUseTouchToStart &&
                          (_touchStartTime_ms != 0 &&
                           curTime - _touchStartTime_ms >
                               PlaypenConfig::kTouchDurationToStart_ms));

  // If we aren't using buttonToStart or we are using it and the button has been
  // pressed or this is sim
  const bool buttonGood = !PlaypenConfig::kUseButtonToStart ||
                          (PlaypenConfig::kUseButtonToStart && _buttonPressed);

  if (touchGood && buttonGood &&
      (robot.GetBatteryComponent().IsOnChargerContacts() ||
       robot.GetBatteryComponent().IsCharging())) {
    // Draw nothing on the screen to clear it
    robot.SendMessage(
        RobotInterface::EngineToRobot(RobotInterface::DrawTextOnScreen(
            true, RobotInterface::ColorRGB(0, 0, 0),
            RobotInterface::ColorRGB(0, 0, 0), "")));

    PLAYPEN_SET_RESULT_WITH_RETURN_VAL(FactoryTestResultCode::SUCCESS,
                                       PlaypenStatus::Complete);
  }
...
void BehaviorPlaypenWaitToStart::OnBehaviorDeactivated() {
  ...
  robot.GetBackpackLightComponent().SetBackpackAnimation(
      BackpackAnimationTrigger::Off);
  ...
  // Tell robot process that playpen is starting so that it can reset touch
  // sensor valid state and update range values in case they were updated in emr
  robot.SendMessage(
      RobotInterface::EngineToRobot(RobotInterface::PlaypenStart()));
}
