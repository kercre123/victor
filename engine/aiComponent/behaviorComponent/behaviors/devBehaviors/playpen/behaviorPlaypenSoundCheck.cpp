...
Result BehaviorPlaypenSoundCheck::OnBehaviorActivatedInternal() {
  ...
  MoveHeadToAngleAction* head =
      new MoveHeadToAngleAction(PlaypenConfig::kHeadAngleToPlaySound);
...
void BehaviorPlaypenSoundCheck::AlwaysHandleInScope(
  ...
  if (tag == RobotInterface::RobotToEngineTag::audioFFTResult) {
    ...
    for (u8 i = 0; i < payload.result.size(); ++i) {
      ...
      PRINT_CH_INFO("Behaviors",
                    "BehaviorPlaypenDriftCheck.HandleAudioFFTResult.Result",
      ...
      if (!Util::IsNear((float)fftResult, PlaypenConfig::kFFTExpectedFreq_hz,
                        PlaypenConfig::kFFTFreqTolerance_hz)) {
        ++count;
        res = channelToMic[i];
        PRINT_NAMED_WARNING(
            "BehaviorPlaypenDriftCheck.HandleAudioFFTResult.FFTFailed",
            "%s picked up freq %u which is outside %u +/- %u",
            EnumToString(res), fftResult, PlaypenConfig::kFFTExpectedFreq_hz,
            PlaypenConfig::kFFTFreqTolerance_hz);
      }
    }
    ...
    if (count == payload.result.size()) {
      ...
      PRINT_NAMED_WARNING(
          "BehaviorPlaypenDriftCheck.HandleAudioFFTResult.Speaker",
    ...
    if (!robot.IsPhysical()) {
      PRINT_CH_DEBUG(
          "Behaviors",
          "BehaviorPlaypenDriftCheck.HandleAudioFFTResult.SimulatedRobot",
    ...
    if (res != FactoryTestResultCode::UNKNOWN) {
      ...
      const_cast<Robot&>(robot).Broadcast(
          MessageEngineToGame(PlaypenBehaviorFailed(res)));
    }
  }
}
