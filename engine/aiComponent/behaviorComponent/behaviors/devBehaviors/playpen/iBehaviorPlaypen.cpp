 ...
 *              All Playpen behaviors should be written to be able to continue
 *even after receiving unexpected things (basically conditional branches should
 *only contain code that calls SET_RESULT) E.g. Even if camera calibration is
 *outside our threshold we should still be able to continue running through the
 *rest of playpen.
 *
...
// Unless they have explicitly subscribed to the message themselves and are
// expecting to see it
...
// Maps a behavior name/idStr to a vector of results it has failed/completed
// with
static std::map<std::string, std::vector<FactoryTestResultCode>> results;

// Hacky way of giving any playpen behavior easy access to check something that
// an individual behavior sets. This way the behavior that is checking doesn't
// have to dynamic cast to the individual behavior. Only
// behaviorPlaypenSoundCheck should be setting this bool
static bool receivedFFTResult = false;
}  // namespace

IBehaviorPlaypen::IBehaviorPlaypen(const Json::Value& config)
    : ICozmoBehavior(config) {
  // Don't want to invailidate kFailureTags by std::move-ing it so make a copy
  // and move that
...
void IBehaviorPlaypen::OnBehaviorActivated() {
  ...
  AddTimer(PlaypenConfig::kDefaultTimeout_ms, [this]() {
    PRINT_NAMED_WARNING(
        "IBehaviorPlaypen.Timeout",
        "Behavior %s has timed out and we are %signoring failures",
        GetDebugLabel().c_str(), (ShouldIgnoreFailures() ? "" : "NOT "));
...
void IBehaviorPlaypen::HandleWhileActivated(const EngineToGameEvent& event) {
  ...
  // If a subclass subscribed to the tag then let them handle it otherwise it is
  // one we (the baseclass) subscribed to, one of the kFailureTags
    ...
    switch (tag) {
      ...
      case EngineToGameTag::MotorCalibration: {
        ...
        if (payload.motorID == MotorID::MOTOR_HEAD) {
          PLAYPEN_SET_RESULT(
              FactoryTestResultCode::HEAD_MOTOR_CALIB_UNEXPECTED);
        } else if (payload.motorID == MotorID::MOTOR_LIFT) {
          PLAYPEN_SET_RESULT(
              FactoryTestResultCode::LIFT_MOTOR_CALIB_UNEXPECTED);
        } else {
...
void IBehaviorPlaypen::SubscribeToTags(std::set<EngineToGameTag>&& tags) {
  ...
  // If the caller wants to subscribe to one of the failure type tags then don't
  // let them subscribe since we already are subscribed to it and it will
  // already call HandleWhileActivatedInternal()
...
bool IBehaviorPlaypen::DelegateIfInControl(IActionRunner* action,
...
bool IBehaviorPlaypen::DelegateIfInControl(IActionRunner* action,
  ...
  // Caller is checking the action result so just print a warning on failure and
  // let the caller deal with the action result
  auto callbackWrapper = [this, callback](ActionResult result) {
    if (result != ActionResult::SUCCESS) {
      PRINT_NAMED_WARNING("IBehaviorPlaypen.ActionFailed",
                          "Behavior %s had an action fail with result %s",
                          GetDebugLabel().c_str(), EnumToString(result));
    }
...
bool IBehaviorPlaypen::DelegateIfInControl(
    IActionRunner* action, RobotCompletedActionCallback callback) {
  // Caller is checking the rca so just print a warning on failure and let the
  // caller deal with the rca
  auto callbackWrapper =
      [this, callback](const ExternalInterface::RobotCompletedAction& rca) {
...
void IBehaviorPlaypen::WriteToStorage(Robot& robot, NVStorage::NVEntryTag tag,
  ...
  if (PlaypenConfig::kWriteToStorage) {
    ...
    PRINT_CH_INFO("Behaviors",
                  "IBehaviorPlaypen.WriteToStorage.WritingNotEnabled",
                  "Not writing to %s", EnumToString(tag));
  }
}
...
const std::map<std::string, std::vector<FactoryTestResultCode>>&
IBehaviorPlaypen::GetAllPlaypenResults() {
  return results;
}

void IBehaviorPlaypen::ResetAllPlaypenResults() { results.clear(); }

const std::set<ExternalInterface::MessageEngineToGameTag>&
IBehaviorPlaypen::GetFailureTags() {
  return kFailureTags;
}

void IBehaviorPlaypen::ReceivedFFTResult() { receivedFFTResult = true; }
...
void IBehaviorPlaypen::RecordTouchSensorData(Robot& robot,
  ...
  if (PlaypenConfig::kDurationOfTouchToRecord_ms > 0) {
    ...
    // This timer will not be removed from the timer vector so it must complete
    // before the behavior is allowed to complete
    AddTimer(
        PlaypenConfig::kDurationOfTouchToRecord_ms,
        [this, &robot, nameOfData]() {
          robot.GetTouchSensorComponent().StopRecordingData();
          PLAYPEN_TRY(GetLogger().Append("TouchSensor_" + nameOfData,
                                         _touchSensorValues),
