...
BehaviorPlaypenEndChecks::BehaviorPlaypenEndChecks(const Json::Value& config)
  ...
  SubscribeToTags({EngineToGameTag::ObjectAvailable});

  SubscribeToTags({{GameToEngineTag::WifiScanResponse,
...
Result BehaviorPlaypenEndChecks::OnBehaviorActivatedInternal() {
  ...
  if (robot.GetBatteryComponent().GetBatteryVolts() <
      PlaypenConfig::kMinBatteryVoltage) {
    PRINT_NAMED_WARNING("BehaviorPlaypenEndChecks.OnActivated.BatteryTooLow",
                        "%fv", robot.GetBatteryComponent().GetBatteryVolts());
    PLAYPEN_SET_RESULT_WITH_RETURN_VAL(FactoryTestResultCode::BATTERY_TOO_LOW,
                                       RESULT_FAIL);
  }

  // Check if we heard from a cube and that this check is even enabled
  if (!(Factory::GetEMR()->fields.playpenTestDisableMask &
        PlaypenTestMask::CubeRadioError)) {
    if (!PlaypenConfig::kSkipActiveObjectCheck && !_heardFromLightCube) {
      PLAYPEN_SET_RESULT_WITH_RETURN_VAL(
          FactoryTestResultCode::NO_ACTIVE_OBJECTS_DISCOVERED, RESULT_FAIL);
    }
  }
  // Else test is disabled via emr print message
    ...
    PRINT_CH_INFO(
        "Behaviors",
        "BehaviorPlaypenEndChecks.OnActivated.SkippingActiveObjectCheck", "");
  }

  // Check if we detected any wifi APs and that this check is even enabled
  if (!(Factory::GetEMR()->fields.playpenTestDisableMask &
        PlaypenTestMask::WifiScanError)) {
    if (!_wifiScanPassed) {
      PLAYPEN_SET_RESULT_WITH_RETURN_VAL(
          FactoryTestResultCode::NO_WIFI_APS_DISCOVERED, RESULT_FAIL);
    }
  }
  // Else test is disabled via emr print message
  else {
    PRINT_CH_INFO("Behaviors",
                  "BehaviorPlaypenEndChecks.OnActivated.SkippingWifiScanCheck",
                  "");
  }

  if (!DidReceiveFFTResult()) {
    PLAYPEN_SET_RESULT_WITH_RETURN_VAL(FactoryTestResultCode::NO_FFT_RESULT,
                                       RESULT_FAIL);
  }

  if (PlaypenConfig::kCheckForCert &&
      Util::FileUtils::GetFileSize(PlaypenConfig::kCertPath) <
          PlaypenConfig::kMinCertSize_bytes) {
    PLAYPEN_SET_RESULT_WITH_RETURN_VAL(FactoryTestResultCode::CERT_CHECK_FAILED,
                                       RESULT_FAIL);
  }
...
void BehaviorPlaypenEndChecks::AlwaysHandleInScope(
...
void BehaviorPlaypenEndChecks::AlwaysHandleInScope(
  ...
  if (eventData.GetTag() == GameToEngineTag::WifiScanResponse) {
    ...
    PRINT_CH_INFO(
        "Behaviors",
        "BehaviorPlaypenEndChecks.AlwaysHandleInScope.WifiScanResponse",
        "ssid_count: %u, status: %hhu", scanResponse.ssid_count,
    ...
    PRINT_NAMED_INFO(
        "BehaviorPlaypenEndChecks.AlwaysHandleInScope.WifiConnectResponse",
