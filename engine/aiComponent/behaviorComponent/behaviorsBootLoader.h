...
class BehaviorsBootLoader : public IDependencyManagedComponent<BCComponentID>,
 ...
 public:
  ...
  // Returns the behavior that should be at the base of the stack. Note that
  // this may not be the current behavior at the base of the stack if some other
  // component has reset the stack recently, or if there is a "pending" behavior
  // that should be the next base behavior upon reset
 ...
 private:
  ...
  // The return value of GetBootBehavior() wasn't switched to, and won't be
  // until SetNewBehavior is called again
  ...
  struct Behaviors {
    ...
    BehaviorID postOnboardingBehavior;  // normal use behavior tree, but doesn't
                                        // wake up
    BehaviorID devBaseBehavior;  // for people who want it to turn on, eyes too,
                                 // but not go anywhere
