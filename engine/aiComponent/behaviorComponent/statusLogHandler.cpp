...
StatusLogHandler::StatusLogHandler(const CozmoContext* context)
  ...
  if (gi != nullptr) {
    auto handleEvent =
        [this](const AnkiEvent<external_interface::GatewayWrapper>& msg) {
          if (msg.GetData().GetTag() ==
              external_interface::GatewayWrapperTag::kEvent) {
            const auto& event = msg.GetData().event();
            if (event.GetTag() ==
                external_interface::EventTag::kTimeStampedStatus) {
              SaveStatusHistory(event.time_stamped_status());
            }
          }
        };
    // this is a good example of where it would be nice to subscribe to a leaf
    // "status" instead of a branch "event"
    _signalHandles.push_back(gi->Subscribe(
        external_interface::GatewayWrapperTag::kEvent, handleEvent));

    // handle request
    auto handleRequest =
        [this](const AnkiEvent<external_interface::GatewayWrapper>& msg) {
          if (msg.GetData().GetTag() ==
              external_interface::GatewayWrapperTag::kRobotHistoryRequest) {
            SendStatusHistory();
          }
        };
    _signalHandles.push_back(gi->Subscribe(
...
void StatusLogHandler::SetFeature(const std::string& featureName,
...
void StatusLogHandler::SaveStatusHistory(
    const external_interface::TimeStampedStatus& status) {
