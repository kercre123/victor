...
AttentionTransferComponent::AttentionTransferComponent()
    : IDependencyManagedComponent(this,
                                  BCComponentID::AttentionTransferComponent) {}

void AttentionTransferComponent::InitDependent(
  ...
  _eventHandles.push_back(_gi->Subscribe(
      AppToEngineTag::kLatestAttentionTransferRequest,
      [this](const AppToEngineEvent& event) { HandleAppRequest(event); }));
}

void AttentionTransferComponent::PossibleAttentionTransferNeeded(
    AttentionTransferReason reason) {
  auto& tracker = GetTracker(reason);
  tracker.AddOccurrence();
}

RecentOccurrenceTracker::Handle
AttentionTransferComponent::GetRecentOccurrenceHandle(
...
void AttentionTransferComponent::OnAttentionTransferred(
    AttentionTransferReason reason) {
  PRINT_CH_INFO("Behaviors",
                "AttentionTransferComponent.OnAttentionTransferred",
  ...
  // special case: don't store "no wifi" since there's no way the app will
  // connect to the robot to see it
  if (reason != AttentionTransferReason::NoWifi) {
    _lastAttentionTransferReason = reason;
    _lastAttentionTransferTime_s =
        BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  }
}

void AttentionTransferComponent::ResetAttentionTransfer(
    AttentionTransferReason reason) {
...
void AttentionTransferComponent::HandleAppRequest(
    const AppToEngineEvent& event) {
  DEV_ASSERT(event.GetData().GetTag() == external_interface::GatewayWrapperTag::
                                             kLatestAttentionTransferRequest,
  ...
  if (_gi != nullptr) {
    ...
    if (_lastAttentionTransferReason != AttentionTransferReason::Invalid) {
      auto* attentionTransfer = new external_interface::AttentionTransfer;
      attentionTransfer->set_reason(
          CladProtoTypeTranslator::ToProtoEnum(_lastAttentionTransferReason));
...
RecentOccurrenceTracker& AttentionTransferComponent::GetTracker(
    AttentionTransferReason reason) {
  auto it = _transferTrackers.find(reason);
  if (it == _transferTrackers.end()) {
    std::string debugName =
        "AttentionTransferReason::" +
        std::string(AttentionTransferReasonToString(reason));
    it = _transferTrackers
             .emplace(std::piecewise_construct, std::forward_as_tuple(reason),
                      std::forward_as_tuple(debugName))
             .first;
  }
