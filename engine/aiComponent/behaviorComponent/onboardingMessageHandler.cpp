...
OnboardingMessageHandler::OnboardingMessageHandler()
    : IDependencyManagedComponent(this,
                                  BCComponentID::OnboardingMessageHandler) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void OnboardingMessageHandler::AdditionalInitAccessibleComponents(
    BCCompIDSet& components) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void OnboardingMessageHandler::InitDependent(Robot* robot,
  ...
  if (nullptr != _gatewayInterface) {
    ...
    _eventHandles.push_back(_gatewayInterface->Subscribe(
        AppToEngineTag::kOnboardingStateRequest,
        std::bind(&OnboardingMessageHandler::HandleOnboardingStateRequest, this,
                  std::placeholders::_1)));

    _eventHandles.push_back(_gatewayInterface->Subscribe(
        AppToEngineTag::kOnboardingCompleteRequest,
        std::bind(&OnboardingMessageHandler::HandleOnboardingCompleteRequest,
                  this, std::placeholders::_1)));

    _eventHandles.push_back(_gatewayInterface->Subscribe(
        AppToEngineTag::kOnboardingWakeUpRequest,
        std::bind(&OnboardingMessageHandler::HandleOnboardingWakeUpRequest,
                  this, std::placeholders::_1)));

    _eventHandles.push_back(_gatewayInterface->Subscribe(
        AppToEngineTag::kOnboardingSetPhaseRequest,
        std::bind(&OnboardingMessageHandler::HandleOnboardingSetPhaseRequest,
                  this, std::placeholders::_1)));
  }
  ...
  if (ANKI_DEV_CHEATS) {
    auto restartOnboarding = [this](ConsoleFunctionContextRef context) {
      ...
      wrapper.set_allocated_onboarding_restart(
          new external_interface::OnboardingRestart);
      if (nullptr != _gatewayInterface) {
    ...
    auto simulateDisconnectFunc = [this](ConsoleFunctionContextRef context) {
      external_interface::GatewayWrapper wrapper;
      wrapper.set_allocated_app_disconnected(
          new external_interface::AppDisconnected);
      if (nullptr != _gatewayInterface) {
        _gatewayInterface->Broadcast(wrapper);
      }
    };
    ...
    auto terminateIncompleteFunc = [this](ConsoleFunctionContextRef context) {
      external_interface::GatewayWrapper wrapper;
      wrapper.set_allocated_onboarding_skip_onboarding(
          new external_interface::OnboardingSkipOnboarding);
      if (nullptr != _gatewayInterface) {
        _gatewayInterface->Broadcast(wrapper);
      }
    };
    ...
    auto terminateCompleteFunc = [this](ConsoleFunctionContextRef context) {
      external_interface::GatewayWrapper wrapper;
      wrapper.set_allocated_onboarding_mark_complete_and_exit(
          new external_interface::OnboardingMarkCompleteAndExit);
      if (nullptr != _gatewayInterface) {
        _gatewayInterface->Broadcast(wrapper);
      }
    };
    ...
    auto hailMaryWakeUpFunc = [this](ConsoleFunctionContextRef context) {
      external_interface::GatewayWrapper wrapper;
      wrapper.set_allocated_onboarding_wake_up_request(
          new external_interface::OnboardingWakeUpRequest);
      if (nullptr != _gatewayInterface) {
        _gatewayInterface->Broadcast(wrapper);
      }
    };
    ...
    auto hailMaryWakeUpStartedFunc = [this](ConsoleFunctionContextRef context) {
      external_interface::GatewayWrapper wrapper;
      wrapper.set_allocated_onboarding_wake_up_started_request(
          new external_interface::OnboardingWakeUpStartedRequest);
      if (nullptr != _gatewayInterface) {
        _gatewayInterface->Broadcast(wrapper);
      }
    };
...
void OnboardingMessageHandler::RequestBleSessions() {
  _robot->Broadcast(ExternalInterface::MessageEngineToGame(
      SwitchboardInterface::HasBleKeysRequest()));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void OnboardingMessageHandler::UpdateDependent(
    const BCCompMap& dependentComps) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void OnboardingMessageHandler::SubscribeToAppTags(
    std::set<AppToEngineTag>&& tags,
    std::function<void(const AppToEngineEvent&)> messageHandler) {
  if (nullptr != _gatewayInterface) {
    for (auto& tag : tags) {
      _eventHandles.push_back(
          _gatewayInterface->Subscribe(tag, messageHandler));
    }
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Messages handled only locally by
// OnboardingMessageHandler
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void OnboardingMessageHandler::HandleOnboardingStateRequest(
  ...
  if (nullptr != _gatewayInterface) {
    if (nullptr != _behaviorsBootLoader) {
      ...
      auto* onboardingState = new external_interface::OnboardingState{
          CladProtoTypeTranslator::ToProtoEnum(stage)};
      _gatewayInterface->Broadcast(
          ExternalMessageRouter::WrapResponse(onboardingState));
    }
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void OnboardingMessageHandler::HandleOnboardingCompleteRequest(
    const AppToEngineEvent& event) {
  if (nullptr != _gatewayInterface) {
    if (nullptr != _behaviorsBootLoader) {
      OnboardingStages stage = _behaviorsBootLoader->GetOnboardingStage();
      const bool completed = (stage == OnboardingStages::Complete) ||
                             (stage == OnboardingStages::DevDoNothing);
      auto* onboardingComplete =
          new external_interface::OnboardingCompleteResponse{completed};
      _gatewayInterface->Broadcast(
          ExternalMessageRouter::WrapResponse(onboardingComplete));
    }
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void OnboardingMessageHandler::HandleOnboardingWakeUpRequest(
    const AppToEngineEvent& event) {
  // This event is primarily handled by BehaviorOnboardingCoordinator, but from
  // OnboardingMessageHandler we want to make sure that the robot isn't stuck in
  // some customer care screen
  HandleWakeUpRequest();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void OnboardingMessageHandler::HandleOnboardingSetPhaseRequest(
    const AppToEngineEvent& event) {
  // This event is primarily handled by BehaviorOnboardingCoordinator, but from
  // OnboardingMessageHandler we want to make sure that the robot isn't stuck in
  // some customer care screen
  if (event.GetData().onboarding_set_phase_request().phase() ==
      external_interface::OnboardingPhase::WakeUp) {
    HandleWakeUpRequest();
  }
}
