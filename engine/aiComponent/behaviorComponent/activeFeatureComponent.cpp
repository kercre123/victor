...
void ActiveFeatureComponent::InitDependent(Robot* robot,
...
void ActiveFeatureComponent::UpdateDependent(const BCCompMap& dependentComps) {
  const auto& behaviorIterator =
      dependentComps.GetComponent<ActiveBehaviorIterator>();
  ...
  if (behaviorIterator.GetLastTickBehaviorStackChanged() == currTick) {
    ...
    if (_activeFeature != newFeature) {
      ...
      // TODO:(bn) delete this soon. This shouldn't happen but is possible
      // theoretically. If this is a real use case, I need to know about it
      if (activeIntent == nullptr && uic.IsAnyUserIntentPending() &&
          newFeature != ActiveFeature::NoFeature) {
        PRINT_NAMED_ERROR("ActiveFeatureComponent.PossibleBug",
                          "TELL BRAD: Feature '%s' is activating (old feature "
                          "is %s). No intent active, but one is pending.",
      ...
      if (activeIntent != nullptr) {
        ...
        if (newFeature != ActiveFeature::NoFeature &&
          ...
          // set intent source from this feature, and mark it's ID so we don't
          // use it again
      ...
      const SimpleMoodType simpleMood =
          dependentComps.GetComponent<MoodManager>().GetSimpleMood();
      ...
      if (_activeFeature == ActiveFeature::Petting &&
        ...
        dependentComps.GetComponent<RobotStatsTracker>()
            .IncrementPettingDuration(timeActive);
      }

      if (newFeature != ActiveFeature::NoFeature) {
        dependentComps.GetComponent<RobotStatsTracker>().IncrementActiveFeature(
            newFeature, intentSource);
      }
...
void ActiveFeatureComponent::SendActiveFeatureToWebViz(
    const std::string& intentSource) const {
  if (_activeFeature != ActiveFeature::NoFeature) {
    if (_context != nullptr) {
      if (auto webSender = WebService::WebVizSender::CreateWebVizSender(
              "behaviors", _context->GetWebService())) {
        webSender->Data()["activeFeature"] =
            std::string(ActiveFeatureToString(_activeFeature)) + " (" +
            intentSource + ")";
      }
    }
  }
...
void ActiveFeatureComponent::OnFeatureChanged(
    const ActiveFeature& newFeature, const ActiveFeature& oldFeature,
    const std::string& source, const SimpleMoodType& simpleMood) {
  const float currTime_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  ...
  if (oldFeature != ActiveFeature::NoFeature) {
    ...
    DASMSG(behavior_feature_end, "behavior.feature.end",
           "This feature is no longer active, but there may not be a new "
           "feature yet");
    DASMSG_SET(s1, ActiveFeatureToString(_activeFeature), "The feature");
    DASMSG_SET(i1, std::round(timeActive),
               "Time the feature was active in seconds");
    DASMSG_SEND();
  }

  if (newFeature != ActiveFeature::NoFeature) {
    const std::string& uuid = Util::GetUUIDString();
    // override the active feature's ActiveFeatureType if the feature is part of
    // onboarding
    const ActiveFeatureType& featureType =
        _isTutorial
            ? ActiveFeatureType::Onboarding
            : GetActiveFeatureType(newFeature, ActiveFeatureType::System);

    // before sending the official feature start event, send a "pre_start" event
    // which will still be tracked under the previous feature run ID
    DASMSG(behavior_feature_pre_start, "behavior.feature.pre_start",
           "A new feature will activate (next message)");
    DASMSG_SET(s1, ActiveFeatureToString(newFeature),
               "The feature that will start");
    DASMSG_SET(s2, SimpleMoodTypeToString(simpleMood),
               "The current simple mood");
    DASMSG_SET(s3, uuid,
               "The UUID the new feature will use for its feature run ID");
    DASMSG_SET(s4, ActiveFeatureTypeToString(featureType),
               "The type of that new feature");
    DASMSG_SEND();

    // NOTE: this event is inspected by the DAS Manager to determine the feature
    // id and type columns, so be especially careful if changing it
    DASMSG(behavior_feature_start, DASMSG_FEATURE_START,
           "A new feature is active");
    DASMSG_SET(s1, ActiveFeatureToString(newFeature), "The feature");
    DASMSG_SET(s2, source,
               "The source of the intent (possible values are AI, App, Voice, "
               "or Unknown)");
    DASMSG_SET(s3, uuid, "A unique ID associated with this feature run");
    DASMSG_SET(s4, ActiveFeatureTypeToString(featureType),
               "The feature type (category)");
    DASMSG_SEND();
