...
BehaviorComponentCloudServer::BehaviorComponentCloudServer(
    const CozmoContext* context, CallbackFunc callback, const std::string& name,
    const int sleepMs)
...
#if SEND_CLOUD_DEV_RESULTS
  AddSignalHandle(_context->GetWebService()
                      ->OnWebVizSubscribed(kWebVizTab)
                      .ScopedSubscribe([this](const auto& sendFunc) {
                        OnClientInit(sendFunc);
                      }));
#endif
}
...
bool BehaviorComponentCloudServer::AddDebugResult(
    const CloudMic::Message& msg) {
#if SEND_CLOUD_DEV_RESULTS
  Json::Value value = msg.GetJSON();

  auto epochTime = std::chrono::system_clock::now().time_since_epoch();
  value["time"] =
      std::chrono::duration_cast<std::chrono::seconds>(epochTime).count();
...
#if SEND_CLOUD_DEV_RESULTS
void BehaviorComponentCloudServer::OnClientInit(
    const WebService::SendToClientFunc& sendFunc) {
