...
void BehaviorComponentCloudServer::RunThread(std::string sockName)
{
  ...
  while (!_shutdown)
  {
    ...
    while (!ready)
    {
      ...
      // this thread from hanging on shutdown of engine.  The timeout is 5 seconds because
      // it's reasonably long but within the approximately 12 seconds it takes for the
      // thread to crash after engine starts shutting down.
      fd_set fdset;
      FD_ZERO(&fdset);
      FD_SET(fd, &fdset);
      timeval timeout;
