...
namespace {
static const BehaviorID kBehaviorIDForDevMessage =
    BEHAVIOR_ID(DevExecuteBehaviorRerun);
...
const std::string kDisableTriggerWordName =
    "BehaviorComponentMessageHandlerTriggerLock";

// string used as an identifier for 'locking' certain things
static const char* kLockName = "BehaviorMessageHandler";

// This value is enough for things to settle down. This is many ticks, but it's
// a only pause after exiting a screen, so doesn't look excessive. Testing shows
...
BehaviorComponentMessageHandler::BehaviorComponentMessageHandler(Robot& robot)
    : IDependencyManagedComponent<BCComponentID>(
          this, BCComponentID::BehaviorComponentMessageHandler),
      _robot(robot),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorComponentMessageHandler::GetInitDependencies(
    BCCompIDSet& dependencies) const {
  ...
  // These are not needed during init, but may be needed in an arbitrarily short
  // time after it if the user opens the dev screen
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorComponentMessageHandler::InitDependent(
    Robot* robot, const BCCompMap& dependentComps) {
  ...
  if (_robot.HasExternalInterface()) {
    ...
    auto handlerCallback = [this, &bContainer, &bsm,
                            &bei](const GameToEngineEvent& event) {
      const auto& msg = event.GetData().Get_ExecuteBehaviorByID();

      BehaviorID behaviorID =
          BehaviorTypesWrapper::BehaviorIDFromString(msg.behaviorID);
    ...
    auto setConnectionStatusCallback = [this, &bContainer,
                                        &bsm](const GameToEngineEvent& event) {
    ...
    auto debugScreenModeHandler = [this, &bContainer, &bsm,
                                   &behaviorsBootLoader,
                                   &uic](const RobotToEngineEvent& event) {
      const auto& msg = event.GetData().Get_debugScreenMode();
      LOG_DEBUG("BehaviorComponentMessageHandler.DebugScreenModeChange",
                "isDebug=%d needsWait=%d, fromMute=%d", msg.isDebug,
                msg.needsWait, msg.fromMute);
      if (!msg.isDebug) {
        // We only care if the debug screen is disabled.
        // We don't use this same message (with enabled == true) for going into
        // the wait behavior because of a race condition which could result in
        // an animation being played from engine _after_ the anim process has
        // already played a face animation for the pairing screen. Don't clear
        // the trigger if leaving mute since exiting mute allows a trigger word
        // (TODO: VIC-11795 not this)
    ...
    _eventHandles.push_back(_robot.GetRobotMessageHandler()->Subscribe(
        RobotInterface::RobotToEngineTag::debugScreenMode,
        debugScreenModeHandler));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorComponentMessageHandler::UpdateDependent(
  ...
  if (_tickInfoScreenEnded != 0) {
    ...
    if (currTick - _tickInfoScreenEnded >= kTicksBeforeEnableFaceKeepalive) {
      // remove the keepalive lock after the stack has a chance to send its
      // first animation
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorComponentMessageHandler::OnEnterInfoFace(
    BehaviorContainer& bContainer, BehaviorSystemManager& bsm) {
  ...
  ANKI_VERIFY(waitBehavior != nullptr,
              "BehaviorComponentMessageHandler.OnEnterInfoFace.NoWait",
              "Could not find wait behavior");
  ...
  // Disable neutral eyes while in the dev screens, because symmetry with
  // another call to enable it in this class
  _robot.GetAnimationComponent().AddKeepFaceAliveDisableLock(kLockName);

  // Prevent the Update loop from sending an EnableKeepFaceAlive(true) in case
  // the user is entering and exiting the pairing screen quickly. There's a
  // potential race condition here if the update loop has just sent a re-enable
  // message but the backpack was also double clicked, but since the anim
  // process is also calling EnableKeepFaceAlive(false), it should be ok
  _tickInfoScreenEnded = 0;

  auto& uic = _robot.GetAIComponent()
                  .GetComponent<BehaviorComponent>()
                  .GetComponent<UserIntentComponent>();
  // note that disabling the engine response for the mic mute screen will be
  // ignored if the user presses the backpack button because of a field fromMute
  // in the triggerWordDetected message
  uic.DisableEngineResponseToTriggerWord(kDisableTriggerWordName, true);

  uic.AlterStreamStateForCurrentResponse(
      kLockName, StreamAndLightEffect::StreamingDisabled);
}
...
void BehaviorComponentMessageHandler::OnExitInfoFace(BehaviorSystemManager& bsm,
  ...
  // ignore any unclaimed trigger word or intent warnings, if a user said the
  // wake word or a command just prior to accessing a dev screen, unless
  // clearTrigger
  ...
  // Mark the time the info screen ended so the Update loop can handle
  // re-enabling face keepalive
...
void BehaviorComponentMessageHandler::SetupUserIntentEvents() {
  ...
  auto fakeTriggerWordCallback = [this](const GameToEngineEvent& event) {
    // Since wakeword handling has moved to the anim process, this now forwards
    // the event there so that the full streaming stack is exercised. The event
    // will propogate back down to the engine when appropriate
    RobotInterface::FakeWakeWordFromExternalInterface msg;
    _robot.SendMessage(RobotInterface::EngineToRobot(std::move(msg)));
  };
  _eventHandles.push_back(EI->Subscribe(
      GameToEngineTag::FakeTriggerWordDetected, fakeTriggerWordCallback));
  ...
  auto fakeUserIntentCallback = [this](const GameToEngineEvent& event) {
    ...
    if (UserIntentTagFromString(intent, tag)) {
      ...
      LOG_DEBUG(
          ...
          "Invalid intent '%s'", intent.c_str());
    }
  };
  _eventHandles.push_back(
      EI->Subscribe(GameToEngineTag::FakeUserIntent, fakeUserIntentCallback));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
ICozmoBehaviorPtr
BehaviorComponentMessageHandler::WrapRequestedBehaviorInDispatcherRerun(
    BehaviorContainer& bContainer, BehaviorID requestedBehaviorID,
    const int numRuns, bool presetConditions) {
  // See if behaviorID has already been created
  ICozmoBehaviorPtr rerunDispatcher =
      bContainer.FindBehaviorByID(kBehaviorIDForDevMessage);
  if (rerunDispatcher != nullptr) {
    bContainer.RemoveBehaviorFromMap(rerunDispatcher);
  }

  Json::Value config = BehaviorDispatcherRerun::CreateConfig(
      kBehaviorIDForDevMessage, requestedBehaviorID, numRuns, presetConditions);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorComponentMessageHandler::SubscribeToWebViz(
    BehaviorExternalInterface& bei, const BehaviorSystemManager& bsm) {
  ...
  if (context != nullptr) {
    ...
    if (webService != nullptr) {
      auto onSubscribedBehaviors =
          [&bei,
           &bsm](const std::function<void(const Json::Value&)>& sendToClient) {
            ...
            for (uint16_t i = 0;
                 i < BehaviorTypesWrapper::GetBehaviorIDNumEntries(); ++i) {
      ...
      auto onDataBehaviors =
          [this](const Json::Value& data,
            ...
            if (name.isString()) {
              ...
              auto* ei = _robot.GetExternalInterface();
              const int numRuns = 1;
              const bool presetConditions =
                  data.get("presetConditions", false).asBool();
              using namespace ExternalInterface;
              ei->Broadcast(MessageGameToEngine(ExecuteBehaviorByID(
                  name.asString(), numRuns, presetConditions)));
            }
          };

      _eventHandles.emplace_back(
          webService->OnWebVizSubscribed(kWebVizModuleNameBehaviors)
              .ScopedSubscribe(onSubscribedBehaviors));
      _eventHandles.emplace_back(
          webService->OnWebVizData(kWebVizModuleNameBehaviors)
              .ScopedSubscribe(onDataBehaviors));

      // now for user/cloud intents

      auto onSubscribedIntents =
          [this](const std::function<void(const Json::Value&)>& sendToClient) {
            // client just connected, send all {user/cloud} intents and current
            // pending {user/cloud} intent
            ...
            std::vector<std::string> allCloudIntentsStr =
                uic.DevGetCloudIntentsList();
            ...
            for (Tag i{0}; i < static_cast<Tag>(USER_INTENT(test_SEPARATOR));
                 ++i) {
          ...
          [this](const Json::Value& data,
            ...
            if (type == "user") {
              ...
              if (UserIntentTagFromString(request, tag)) {
                ...
                LOG_WARNING(
                    "BehaviorComponentMessageHandler.WebVizUserIntentReceived."
                    "Invalid",
                    "Invalid intent '%s'", request.c_str());
              }
            } else if (type == "cloud") {
              if ((request.find("{") !=
                   std::string::npos)  // super awesome json detection
      ...
      _eventHandles.emplace_back(
          webService->OnWebVizSubscribed(kWebVizModuleNameIntents)
              .ScopedSubscribe(onSubscribedIntents));
      _eventHandles.emplace_back(
          webService->OnWebVizData(kWebVizModuleNameIntents)
              .ScopedSubscribe(onDataIntents));
    }
  }
}
