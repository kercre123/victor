...
class IBehavior : private Util::noncopyable {
 public:
  ...
  void SetDebugLabel(const std::string& debugLabel) {
  ...
  // to start any processes which need to be running for the Behavior to be
  // activated
  void OnEnteredActivatableScope();

  // Guaranteed to be ticked every tick that the behavior is within activatable
  // scope
  void Update();
  ...
  // Function which informs the Behavior that it has fallen out of scope to be
  // activated the behavior should stop any processes it started on entering
  // selectable scope
  void OnLeftActivatableScope();

  virtual void GetAllDelegates(std::set<IBehavior*>& delegates) const = 0;

  // Sometimes behaviors (like dispatchers) want to maintain the same scope as
  // one of their delegates even if they are in Activatable scope (i.e. their
  // delegates are still out of scope ) This allows them to pass calls to
  // WantsToBeActivated or maintain proper vision mode settings Behaviors
  // returned from this function will be moved into activatable scope in lock
  // step with the behavior that overrides them
 ...
 protected:
  ...
  // Called when this behavior has entered activatable scope (it could be
  // delegated to)
  virtual void OnEnteredActivatableScopeInternal() {}

  // Called when this behavior is no longer in activatable scope (no longer
  // valid to be delegated to)
  ...
  BehaviorExternalInterface& GetBEI() const {
    assert(_beiWrapper);
    return _beiWrapper->_bei;
  }

 private:
  // string for identifying Behaviors (different than an ID since the latter
  // might just be "Anonymous")
  ...
  struct BEIWrapper {
    BEIWrapper(BehaviorExternalInterface& bei) : _bei(bei) {}
    BehaviorExternalInterface& _bei;
  };
  std::unique_ptr<BEIWrapper> _beiWrapper;

  enum class ActivationState { NotInitialized, OutOfScope, InScope, Activated };
  ...
  void AssertActivationState_DevOnly(ActivationState state,
                                     const std::string& debugStr) const;
  void AssertNotActivationState_DevOnly(ActivationState state,
                                        const std::string& debugStr) const;
