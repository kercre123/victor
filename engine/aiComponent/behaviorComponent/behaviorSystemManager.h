...
class BehaviorSystemManager : public IDependencyManagedComponent<BCComponentID>,
 ...
 public:
  ...
  // cancel delegates, and return true if any were canceled (note that false
  // here isn't an error, it's OK for a behavior with no delegates to call this
  // function and have false returned)
  bool CancelDelegates(IBehavior* delegator) override;

  void CancelSelf(IBehavior* delegator) override;

  // If control of the passed in behavior is delegated (to another behavior),
  // return the pointer of the behavior that it was delegated to. Otherwise,
  // return nullptr (including if control was delegated to an action or helper)
  const IBehavior* GetBehaviorDelegatedTo(
      const IBehavior* delegatingBehavior) const;

  // Return the pointer of the behavior which delegated to the passed in
  // behavior. If at the base behavior, return nullptr
  ...
  // calls upon the behavior stack to build and return the behavior tree as a
  // flat array in json
  ...
  size_t GetLastTickBehaviorStackChanged() const {
    return _lastBehaviorStackUpdateTick;
  }
