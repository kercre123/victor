...
void IBehavior::OnEnteredActivatableScope() {
  AssertNotActivationState_DevOnly(ActivationState::NotInitialized,
                                   "IBehavior.OnEnteredActivatableScope");

  _currentInScopeCount++;
  // If this isn't the first EnteredActivatableScope don't call internal
  // functions
  ...
  // so set the last tick count as being one tickInterval before the current
  // tickCount
  _lastTickOfUpdate =
      (BaseStationTimer::getInstance()->GetTickCount() - kBSTickInterval);
...
void IBehavior::Update() {
  ...
  DEV_ASSERT_MSG(_lastTickOfUpdate == (tickCount - kBSTickInterval),
                 ...
                 "Behavior '%s' is receiving tick on %zu, but hasn't been "
                 "ticked since %zu",
                 _debugLabel.c_str(), tickCount, _lastTickOfUpdate);
...
bool IBehavior::WantsToBeActivated() const {
#if ANKI_DEV_CHEATS
  // It's possible that this behavior appears in multiple places in the tree, so
  // it may be active when checking WantsToBeActivated.
  DEV_ASSERT_MSG((_currentActivationState == ActivationState::Activated) ||
                 ...
                 "Behavior '%s' in activation state '%s' should not get "
                 "WantsToBeActivated call",
                 _debugLabel.c_str(),
  ...
  auto accessGuard =
      GetBEI().GetComponentWrapper(BEIComponentID::Delegation).StripComponent();
...
void IBehavior::OnLeftActivatableScope() {
  AssertActivationState_DevOnly(ActivationState::InScope,
                                "IBehavior.OnLeftActivatableScope");
  ...
  if (_currentInScopeCount != 0) {
    if (kDebugActivationState) {
      LOG_DEBUG("IBehavior.OnLeftActivatableScope.StillInScope",
                ...
                _currentInScopeCount, _debugLabel.c_str());
    }
    return;
  }
...
void IBehavior::SetActivationState_DevOnly(ActivationState state,
  ...
  if (kDebugActivationState) {
    PRINT_CH_DEBUG("Behaviors", "IBehavior.SetActivationState",
                   "%s: Behavior '%s' Activation state set to %s",
                   debugStr.c_str(), _debugLabel.c_str(),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void IBehavior::AssertActivationState_DevOnly(
    ActivationState state, const std::string& debugStr) const {
#if ANKI_DEV_CHEATS
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void IBehavior::AssertNotActivationState_DevOnly(
    ActivationState state, const std::string& debugStr) const {
#if ANKI_DEV_CHEATS
  DEV_ASSERT_MSG(
      _currentActivationState != state,
      "IBehavior.AssertNotActivationState_DevOnly.WrongActivationState",
      "%s: Behavior '%s' is state %s, but should not be", debugStr.c_str(),
      _debugLabel.c_str(), ActivationStateToString(_currentActivationState));
#endif
}
...
const char* IBehavior::ActivationStateToString(ActivationState state) const {
  switch (state) {
    case ActivationState::NotInitialized:
      return "NotInitialized";
    case ActivationState::OutOfScope:
      return "OutOfScope";
    case ActivationState::Activated:
      return "Activated";
    case ActivationState::InScope:
      return "InScope";
  }
  return "Invalid";
}
