...
class AsyncMessageGateComponent
 ...
 public:
  ...
  void SubscribeToTags(IBehavior* subscriber,
  ...
  void GetEventsForBehavior(IBehavior* subscriber,
 ...
 private:
  ...
  struct EventTracker {
    ...
    // Only used by the cache instance of the tracker - allows faster access to
    // events as behavior indexed
  ...
  std::unordered_map<GameToEngineTag, std::set<IBehavior*>>
      _gameToEngineSubscribers;
  std::unordered_map<EngineToGameTag, std::set<IBehavior*>>
      _engineToGameSubscribers;
  std::unordered_map<RobotInterface::RobotToEngineTag, std::set<IBehavior*>>
      _robotToEngineSubscribers;
  std::unordered_map<AppToEngineTag, std::set<IBehavior*>>
      _appToEngineSubscribers;
