...
CONSOLE_VAR(float, kSleepTracker_debtToConsiderSleepy_awake, CONSOLE_GROUP,
            60.0f * 60.0f);
CONSOLE_VAR(float, kSleepTracker_debtToConsiderSleepy_fromSleep, CONSOLE_GROUP,
            30.0f * 60.0f);
...
void ForceAddSleepDebtHours(ConsoleFunctionContextRef context) {
  sForceSleepDebtDelta_s =
      ConsoleArg_Get_Float(context, "deltaHours") * 60.0f * 60.0f;
}
#endif
...
void SleepTracker::SetIsSleeping(bool sleeping) {
  LOG_INFO("SleepTracker.SetSleeping", "%s", sleeping ? "asleep" : "awake");

  _asleep = sleeping;
}

void SleepTracker::InitDependent(Robot* robot,
                                 const BCCompMap& dependentComps) {
  const auto* context =
      dependentComps.GetComponent<BEIRobotInfo>().GetContext();
  if (context != nullptr) {
...
void SleepTracker::UpdateDependent(const BCCompMap& dependentComps) {
  ...
  if (consoleFuncUsed || currBSTime_s > _nextUpdateTime_s) {
    ...
    if (_asleep) {
      ...
      _sleepDebt_s +=
          kSleepTracker_updatePeriod_s * kSleepTracker_awakeSleepDebtRate;
    }
    ...
    _sleepDebt_s = Util::Clamp(_sleepDebt_s, 0.0f,
                               kSleepTracker_maxSleepDebt_hours * 60 * 60);
...
bool SleepTracker::IsSleepy(const bool fromSleep) const {
  const float debtToConsiderSleepy =
      fromSleep ? kSleepTracker_debtToConsiderSleepy_fromSleep
...
bool SleepTracker::IsNightTime() const {
  ...
  if (!WallTime::getInstance()->GetApproximateLocalTime(localTime)) {
    // arbitrarily default to "night", if timezone isn't set or system reports
    // some other time failure (doesn't need to be synced accurately though)
    return true;
  }
  ...
  const float morningDecimalHours =
      kSleepTracker_moning_hour + kSleepTracker_moning_minute / 60.0f;
  const float nightDecimalHours =
      kSleepTracker_night_hour + kSleepTracker_night_minute / 60.0f;

  DEV_ASSERT(morningDecimalHours < nightDecimalHours,
             "SleepTracker.MorningMustBeBeforeNight");
...
void SleepTracker::EnsureSleepDebtAtLeast(const float minDebt_s) {
  if (minDebt_s < kSleepTracker_debtToConsiderSleepy_fromSleep) {
    PRINT_NAMED_WARNING(
        "SleepTracker.EnsureSleepDebtAtLeast.MinDebtProbablyTooLow",
        "You specified a min sleep debt of %f, but the min to wake up from "
        "sleep is higher at %f",
        minDebt_s, kSleepTracker_debtToConsiderSleepy_fromSleep);
  }

  if (_sleepDebt_s < minDebt_s) {
    // set to min but also make sure it's in range
    _sleepDebt_s = Util::Clamp(minDebt_s, 0.0f,
                               kSleepTracker_maxSleepDebt_hours * 60 * 60);
  }
}
