 ...
 * Created: 2017-10-18
 *
 * Description: This file provides a set of wrappers around the CLAD types
 *defined in BehaviorTypes.clad. In almost all cases, these wrappers should be
 *used instead of the raw clad files to avoid very slow incremental builds. If
 *you directly use the CLAD enums, even in a .cpp file, that file will need to
 *be re-built every time a behavior ID is added. By using these wrappers, the
 *              incremental build time after touching BehaviorTypes.clad will be
...
// if set to 1, use strings to avoid triggering massive rebuilds when clad
// changes otherwise, use the enums to guarantee compile-time errors for invalid
// ids
...
#if BEHAVIOR_ID_DEV_MODE
#define BEHAVIOR_ID(name) \
  Anki::Vector::BehaviorTypesWrapper::BehaviorIDFromString(#name)
#define BEHAVIOR_CLASS(name) \
  Anki::Vector::BehaviorTypesWrapper::BehaviorClassFromString(#name)
#else
