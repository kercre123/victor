...
bool UserIntentComponent::SetCloudIntentPendingFromJSONValue(Json::Value json)
{
  ...
  if( userIntentTag == USER_INTENT(unmatched_intent) ) {
    if( TryRewriteDevIntent( json ) ) {
      if( !JsonTools::GetValueOptional(json, kCloudIntentJsonKey, cloudIntent) ) {
...
bool ParseDevIntent( const std::string& input, Json::Value& json )
{
  static std::vector<DevIntentInfo> devMap = {
    {"intent_play_any_music", "play(?: me)? some music", {}},
    {"intent_play_music", "play(?: me)? some ([a-zA-Z ]+)", {"request"}},
    {"intent_eat_potatoes", "eat ([0-9]+) potatoes", {"number"}},
    {"intent_imperative_come", "go to your master", {}},
  };
...
bool UserIntentComponent::TryRewriteDevIntent( Json::Value& json ) const
{
    ...
    for( const auto& entry : entries ) {
      ...
      if( idx != std::string::npos ) {
        ...
        if( idx + searchStr.length() < entry.size() ) {
          const std::string text = entry.substr( idx + searchStr.length() );
          if( ParseDevIntent( text, json ) ) {
            return true;
          }
        }
      }
