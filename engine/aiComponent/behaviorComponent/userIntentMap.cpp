...
UserIntentMap::UserIntentMap(const Json::Value& config, const CozmoContext* ctx)
  ...
  ANKI_VERIFY(config[kUserIntentMapKey].size() > 0,
              "UserIntentMap.InvalidConfig", "expected to find group '%s'",
              kUserIntentMapKey);

  // return true if this feature passes the gate (or no gate is enabled)
  auto featureGateOK = [ctx](const Json::Value& jsonGroup,
                             const std::string& intentStr) {
    const std::string& featureName =
        jsonGroup.get(kFeatureGateKey, "").asString();
    if (!featureName.empty() && (ctx != nullptr)) {
      ...
      ANKI_VERIFY(FeatureTypeFromString(featureName, feature),
                  ...
                  featureName.c_str(), intentStr.c_str());
      const bool featureEnabled =
          ctx->GetFeatureGate()->IsFeatureEnabled(feature);
      if (!featureEnabled) {
  ...
  for (const auto& mapping : config[kUserIntentMapKey]) {
    const std::string& userIntentStr =
        JsonTools::ParseString(mapping, kUserIntentKey, kDebugName);

    UserIntentTag intentTag;
    ANKI_VERIFY(UserIntentTagFromString(userIntentStr, intentTag),
                "UserIntentMap.Ctor.InvalidIntent",
                "supplied %s '%s' is invalid", kUserIntentKey,
                userIntentStr.c_str());
    ...
    auto processMapping = [&intentTag](
                              const Json::Value& input, MapType& container,
                              const char* intentKey, const char* subsKey,
                              const char* numericsKey, const char* testKey,
      ...
      if (!intentName.empty()) {
        ...
        if (!subs.isNull()) {
          for (const auto& fromStr : subs.getMemberNames()) {
            ...
            if (ANKI_VERIFY(
                    !to.isNull(),
                    "UserIntentMap.Ctor.InvalidConfig.MissingSubstitution",
                    "Missing value")) {
              const auto& toStr = to.asString();
              varSubstitutions.emplace_back(
                  SanitationActions{fromStr, toStr, false});
            }
          }
        }
        ...
        if (!numerics.isNull()) {
          for (const auto& from : numerics) {
            ...
            auto it = std::find_if(
                varSubstitutions.begin(), varSubstitutions.end(),
                [&fromStr](const auto& s) { return (s.from == fromStr); });
            if (it != varSubstitutions.end()) {
              it->isNumeric = true;
            } else {
              varSubstitutions.emplace_back(
                  SanitationActions{fromStr, "", true});
            }
          }
        }

        // a cloud/app intent should only appear once, since it only has one
        // user intent (although multiple cloud/app intents can map to the same
        // user intent
        ANKI_VERIFY(container.find(intentName) == container.end(),
                    ...
                    debugName, intentName.c_str(),
                    UserIntentTagToString(
                        container.find(intentName)->second.userIntent));

        container.emplace(intentName,
                          IntentInfo{intentTag, varSubstitutions, doTest});
      }
    };

    processMapping(mapping, _cloudToUserMap, kCloudIntentKey,
                   kCloudVariableSubstitutionsKey, kCloudVariableNumericsKey,
                   kTestUserIntentParsingKey, "cloud");
    processMapping(mapping, _appToUserMap, kAppIntentKey,
                   kAppVariableSubstitutionsKey, kAppVariableNumericsKey,
                   kTestUserIntentParsingKey, "app");

    if (ANKI_DEVELOPER_CODE) {
      // prevent typos
      std::vector<const char*> validKeys = {kCloudIntentKey,
                                            kCloudVariableSubstitutionsKey,
                                            kCloudVariableNumericsKey,
                                            kAppIntentKey,
                                            kAppVariableSubstitutionsKey,
                                            kAppVariableNumericsKey,
                                            kUserIntentKey,
                                            kFeatureGateKey,
                                            kTestUserIntentParsingKey,
                                            kSimpleVoiceResponsesKey,
                                            kResponseKey};
      std::vector<std::string> badKeys;
      const bool hasBadKeys =
          JsonTools::HasUnexpectedKeys(mapping, validKeys, badKeys);
      if (hasBadKeys) {
        std::string keys;
        ...
        DEV_ASSERT_MSG(false, "UserIntentMap.Ctor.UnexpectedKey",
                       "Keys '%s' unexpected for intent '%s'", keys.c_str(),
  ...
  for (const auto& simpleResponseGroup : config[kSimpleVoiceResponsesKey]) {
    const std::string& cloudIntent = JsonTools::ParseString(
        simpleResponseGroup, kCloudIntentKey, kDebugName);
    ...
    if (_cloudToUserMap.find(cloudIntent) != _cloudToUserMap.end()) {
      LOG_ERROR("UserIntentMap.Ctor.InvalidConfig.DuplicateCloudIntent.Normal",
                "Cloud intent '%s' specified in %s is already defined as a "
                "normal response",
                cloudIntent.c_str(), kSimpleVoiceResponsesKey);
      continue;
    }

    if (_simpleCloudResponseMap->map.find(cloudIntent) !=
        _simpleCloudResponseMap->map.end()) {
      LOG_ERROR("UserIntentMap.Ctor.InvalidConfig.DuplicateCloudIntent.Simple",
                "Cloud intent '%s' specified in %s is already defined as "
    ...
    if (!jsonOK) {
      LOG_ERROR(
          "UserIntentMap.Ctor.InvalidConfig.SimpleResponseParseFail.Response",
          "Could not parse simple voice response for cloud intent '%s'",
          cloudIntent.c_str());
      continue;
    }

    // NOTE: in order to check the validity of anim groups and emotion events,
    // we need access to more components than we have here. This is the job of
    // the owner (i.e. UserIntentComponent)

    _simpleCloudResponseMap->map.emplace(cloudIntent, std::move(response));
  }

  std::string unmatchedString =
      JsonTools::ParseString(config, kUnmatchedKey, kDebugName);
  ANKI_VERIFY(UserIntentTagFromString(unmatchedString, _unmatchedUserIntent),
              "UserIntentMap.Ctor.InvalidUnmatchedIntent",
              "supplied %s '%s' is invalid", kUnmatchedKey,
              unmatchedString.c_str());

  // now verify that all user intents were listed. This isnt necessary for the
  // c++ code base, but we want to keep the json accurate so that if anyone else
  // cares about our user intents, they can parse json instead of clad.
  if (ANKI_DEVELOPER_CODE) {
    // ignore this only for those tests that specify a new mapping. Most tests
    // will run with the real deal data.
    if (config["is_test"].isNull() || !config["is_test"].asBool()) {
      ...
      for (Tag i{0}; i < static_cast<Tag>(USER_INTENT(test_SEPARATOR)); ++i) {
        if (i != static_cast<Tag>(_unmatchedUserIntent)) {
          DEV_ASSERT_MSG(foundUserIntent.find(static_cast<UserIntentTag>(i)) !=
                                 foundUserIntent.end() ||
                             gatedUserIntent.find(static_cast<UserIntentTag>(
                                 i)) != gatedUserIntent.end(),
                         "UserIntentMap.Ctor.MissingUserIntents",
                         "Every user intent found in clad must appear in json. "
                         "You're missing '%s'",
...
void UserIntentMap::VerifySimpleVoiceResponses(
    UserIntentMap::SimpleVoiceResponseVerification verify,
    const std::string& debugKey) {
  for (auto mapIt = _simpleCloudResponseMap->map.begin();
       mapIt != _simpleCloudResponseMap->map.end();) {
    const auto& response = mapIt->second.Get_simple_voice_response();
    if (verify(response)) {
      LOG_DEBUG(
          "UserIntentMap.VerifySimpleVoiceResponses.Pass",
          "%s: response to cloud intent '%s' passed (verify returned true)",
          debugKey.c_str(), mapIt->first.c_str());

      ++mapIt;
    } else {
      LOG_ERROR("UserIntentMap.VerifySimpleVoiceResponses.Fail",
                "%s: response to cloud intent '%s' failed verification, "
                "removing from map",
                debugKey.c_str(), mapIt->first.c_str());
...
UserIntentTag UserIntentMap::GetUserIntentFromCloudIntent(
    const std::string& cloudIntent) const {
  using Tag = std::underlying_type<UserIntentTag>::type;
  auto it = _cloudToUserMap.find(cloudIntent);
  if (it != _cloudToUserMap.end()) {
    DEV_ASSERT(static_cast<Tag>(it->second.userIntent) <
                   static_cast<Tag>(USER_INTENT(INVALID)),
    ...
    if (simpleResponseIter != _simpleCloudResponseMap->map.end()) {
      ...
      DEV_ASSERT(static_cast<Tag>(_unmatchedUserIntent) <
                     static_cast<Tag>(USER_INTENT(INVALID)),
                 "Invalid intent value");
      LOG_WARNING(
          "UserIntentMap.NoCloudIntentMatch",
          "No match for cloud intent '%s', returning default user intent '%s'",
          cloudIntent.c_str(), UserIntentTagToString(_unmatchedUserIntent));
...
bool UserIntentMap::IsValidCloudIntent(const std::string& cloudIntent) const {
  const bool found =
      (_cloudToUserMap.find(cloudIntent) != _cloudToUserMap.end());
  return found;
}

bool UserIntentMap::GetTestParsingBoolFromCloudIntent(
    const std::string& cloudIntent) const {
  auto it = _cloudToUserMap.find(cloudIntent);
  if (it != _cloudToUserMap.end()) {
    ...
    LOG_WARNING("UserIntentMap.NoCloudIntentMatch",
                "No match for cloud intent '%s', returning default value of "
                "true to test user intent parsing",
                cloudIntent.c_str());
    return true;
  }
}

UserIntentTag UserIntentMap::GetUserIntentFromAppIntent(
    const std::string& appIntent) const {
  using Tag = std::underlying_type<UserIntentTag>::type;
  ...
  if (it != _appToUserMap.end()) {
    ...
    DEV_ASSERT(static_cast<Tag>(_unmatchedUserIntent) <
               ...
               "Invalid intent value");
    LOG_WARNING(
        "UserIntentMap.NoAppIntentMatch",
        "No match for app intent '%s', returning default user intent '%s'",
        appIntent.c_str(), UserIntentTagToString(_unmatchedUserIntent));
    return _unmatchedUserIntent;
  }
}
...
void UserIntentMap::SanitizeCloudIntentVariables(
    const std::string& cloudIntent, Json::Value& paramsList) const {
  SanitizeVariables(cloudIntent, _cloudToUserMap, "cloud", paramsList);
}

void UserIntentMap::SanitizeAppIntentVariables(const std::string& appIntent,
    ...
    for (const auto& varName : paramsList.getMemberNames()) {
      ...
      const auto it =
          std::find_if(subs.begin(), subs.end(),
                       [&varName](const auto& p) { return p.from == varName; });
      if (it != subs.end()) {
        if (!it->to.empty()) {
          ...
          if (!ANKI_VERIFY(paramsList[it->to].isNull(),
                           ...
                           debugName, intent.c_str())) {
            continue;
          }

          if (it->to == it->from) {
            LOG_WARNING("UserIntentMap.SanitizeVariables.NoOp",
                        "The provided '%s' substitution '%s' resulted in no "
                        "op. Substitution is optional",
                        debugName, it->to.c_str());
          }

          // replace
          paramsList[it->to] = paramsList[varName];
          paramsList.removeMember(it->from);
          varNameP = &it->to;  // since varname changed, update the pointer for
        ...
        if (it->isNumeric) {
          ...
          auto str =
              JsonTools::ParseString(paramsList, varNameP->c_str(), debugName);
          try {
            ...
            LOG_ERROR("UserIntentMap.SanitizeVariables.Invalid",
                      "Tried to convert %s intent '%s' and failed. No "
...
const UserIntent& UserIntentMap::GetSimpleVoiceResponse(
    const std::string& cloudIntent) const {
  ...
  if (it == _simpleCloudResponseMap->map.end()) {
    LOG_ERROR("UserIntentMap.GetSimpleVoiceResponse.NoSimpleResponse",
              "Cloud intent '%s' not found among the %zu intents in the simple "
              "response map",
              cloudIntent.c_str(), _simpleCloudResponseMap->map.size());
