 ...
 * Created: 9/22/17
 *
 * Description: Component responsible for maintaining all aspects of the AI
 *system relating to behaviors
 *
...
BehaviorComponent::BehaviorComponent()
    : IDependencyManagedComponent<AIComponentID>(
          this, AIComponentID::BehaviorComponent) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorComponent::~BehaviorComponent() {
  // This needs to happen before ActionList is destroyed, because otherwise
  // behaviors will try to respond to actions shutting down
  _comps->ClearEntity();
}
...
void BehaviorComponent::GenerateManagedComponents(Robot& robot,
  ...
  // AIComponent
  {
    DEV_ASSERT(
        ...
        "BehaviorComponent.GenerateManagedComponents.NoAIComponentProvided");
  }
  // Face World
  ...
  if (!entity->HasComponent<RobotStatsTracker>()) {
    entity->AddDependentComponent(BCComponentID::RobotStatsTracker,
                                  robot.GetComponentPtr<RobotStatsTracker>(),
                                  false);
  }
  ...
  if (!entity->HasComponent<AsyncMessageGateComponent>()) {
    auto messagePtr = new AsyncMessageGateComponent(
    ...
    entity->AddDependentComponent(BCComponentID::AsyncMessageComponent,
                                  std::move(messagePtr));
  }
  ...
  if (!entity->HasComponent<UserIntentComponent>()) {
    entity->AddDependentComponent(
        BCComponentID::UserIntentComponent,
        new UserIntentComponent(
            robot, robot.GetContext()->GetDataLoader()->GetUserIntentConfig()));
  }

  // Behavior Container
  if (!entity->HasComponent<BehaviorContainer>()) {
    if (robot.GetContext() != nullptr) {
      auto bcPtr = new BehaviorContainer(
  ...
  if (!entity->HasComponent<BehaviorComponentMessageHandler>()) {
    entity->AddDependentComponent(
        BCComponentID::BehaviorComponentMessageHandler,
  ...
  if (!entity->HasComponent<UserDefinedBehaviorTreeComponent>()) {
    entity->AddDependentComponent(
        BCComponentID::UserDefinedBehaviorTreeComponent,
        new UserDefinedBehaviorTreeComponent());
  }

  if (!entity->HasComponent<ActiveFeatureComponent>()) {
    entity->AddDependentComponent(BCComponentID::ActiveFeature,
                                  new ActiveFeatureComponent);
  }

  if (!entity->HasComponent<ActiveBehaviorIterator>()) {
    entity->AddDependentComponent(BCComponentID::ActiveBehaviorIterator,
                                  new ActiveBehaviorIterator);
  }

  if (!entity->HasComponent<AttentionTransferComponent>()) {
    entity->AddDependentComponent(BCComponentID::AttentionTransferComponent,
                                  new AttentionTransferComponent);
  }

  if (!entity->HasComponent<SleepTracker>()) {
    entity->AddDependentComponent(BCComponentID::SleepTracker,
                                  new SleepTracker);
  }

  if (!entity->HasComponent<OnboardingMessageHandler>()) {
    entity->AddDependentComponent(BCComponentID::OnboardingMessageHandler,
                                  new OnboardingMessageHandler);
  }

  if (!entity->HasComponent<HeldInPalmTracker>()) {
    entity->AddDependentComponent(BCComponentID::HeldInPalmTracker,
                                  new HeldInPalmTracker);
  }
  ...
  if (!entity->HasComponent<BehaviorsBootLoader>()) {
    ...
    if (context == nullptr) {
      PRINT_NAMED_WARNING(
          "BehaviorComponent.GenerateManagedComponents.NoContext",
          "wont be able to load behaviors from context componenets. May be OK "
          "in unit tests");
    } else {
    ...
    entity->AddDependentComponent(
        BCComponentID::BehaviorsBootLoader,
        new BehaviorsBootLoader{behaviorSystemConfig});
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorComponent::InitDependent(Robot* robot,
                                      const AICompMap& dependentComps) {
  if (_comps == nullptr) {
    _comps = std::make_unique<EntityType>();
    _comps->AddDependentComponent(BCComponentID::AIComponent,
                                  robot->GetComponentPtr<AIComponent>(), false);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorComponent::SubscribeToTags(
    IBehavior* subscriber,
    std::set<ExternalInterface::MessageGameToEngineTag>&& tags) const {
  AsyncMessageGateComponent& gateComp =
      GetComponent<AsyncMessageGateComponent>();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorComponent::SubscribeToTags(IBehavior* subscriber,
