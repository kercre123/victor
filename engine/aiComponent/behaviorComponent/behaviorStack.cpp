...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::string BehaviorStack::StackToBehaviorString(
...
void BehaviorStack::InitBehaviorStack(IBehavior* baseOfStack,
  ...
  ANKI_VERIFY(_behaviorStack.empty(),
              "BehaviorSystemManager.BehaviorStack.InitBehaviorStack."
              "StackNotEmptyOnInit",
              "");
  ...
  rootMetaData.RecursivelyGatherLinkedBehaviors(
      baseOfStack, rootMetaData.linkedActivationScope);
  ...
  ANKI_VERIFY(baseOfStack->WantsToBeActivated(),
              "BehaviorSystemManager.BehaviorStack.InitConfig."
              "BasebehaviorDoesNotWantToBeActivated",
              "%s", baseOfStack->GetDebugLabel().c_str());
...
void BehaviorStack::ClearStack() {
  ...
  DEV_ASSERT(_behaviorStack.empty(),
             "BehaviorStack.Destructor.NotAllBehaviorsPoppedFromStack");
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorStack::UpdateBehaviorStack(
    BehaviorExternalInterface& behaviorExternalInterface,
    std::vector<ExternalInterface::RobotCompletedAction>&
        actionsCompletedThisTick,
    AsyncMessageGateComponent& asyncMessageGateComp,
    std::set<IBehavior*>& tickedInStack) {
  if (_behaviorStack.size() == 0) {
    LOG_WARNING(
        "BehaviorSystemManager.BehaviorStack.UpdateBehaviorStack."
        "NoStackInitialized",
        "");
    return;
  }
  ...
  // functions - so track the index in the stack rather than the iterator
  // directly One side effect of this is that if a Behavior ends this tick
  // without queueing an action we potentially lose a tick before the next time
  // a BSBehavior queues an action - to save on complexity we're accepting this
  // tradeoff for the time being but may decide to address it directly here or
  // within the BSBehavior/one of its subclasses in the future
  ...
  for (int idx = 0; idx < _behaviorStack.size(); idx++) {
    ...
    asyncMessageGateComp.GetEventsForBehavior(
        _behaviorStack.at(idx), eventComponent._gameToEngineEvents);
    asyncMessageGateComp.GetEventsForBehavior(
        _behaviorStack.at(idx), eventComponent._engineToGameEvents);
    asyncMessageGateComp.GetEventsForBehavior(
        _behaviorStack.at(idx), eventComponent._robotToEngineEvents);
    asyncMessageGateComp.GetEventsForBehavior(
        _behaviorStack.at(idx), eventComponent._appToEngineEvents);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
const IBehavior* BehaviorStack::GetBehaviorInStackAbove(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
const IBehavior* BehaviorStack::GetBehaviorInStackBelow(
    const IBehavior* behavior) const {
  const auto it = _stackMetadataMap.find(behavior);
  if (it != _stackMetadataMap.end()) {
...
void BehaviorStack::PushOntoStack(IBehavior* behavior) {
  StackMetadataEntry metadata(behavior,
                              static_cast<int>(_behaviorStack.size()));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorStack::IsValidDelegation(IBehavior* delegator,
                                      IBehavior* delegated) {
  auto iter = _stackMetadataMap.find(delegator);
  if (iter != _stackMetadataMap.end()) {
    return (iter->second.delegates.find(delegated) !=
            iter->second.delegates.end());
  }
  return false;
}
...
void BehaviorStack::BroadcastAudioBranch(bool activated) {
  if (ANKI_VERIFY(_externalInterface != nullptr,
    ...
    auto state =
        activated ? BehaviorStackState::Active : BehaviorStackState::NotActive;
    _externalInterface->Broadcast(
        MessageEngineToGame(AudioBehaviorStackUpdate(state, std::move(path))));
  }
}
...
void BehaviorStack::PrepareDelegatesToEnterScope(IBehavior* delegated) {
  ...
  for (auto& entry :
       _stackMetadataMap.find(delegated)->second.linkedActivationScope) {
    entry->OnEnteredActivatableScope();
  }
}
...
void BehaviorStack::DebugPrintStack(const std::string& debugStr) const {
  ...
  for (size_t i = 0; i < _behaviorStack.size(); ++i) {
    LOG_DEBUG("BehaviorSystem", debugName.c_str(), "%zu: %s", i,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Json::Value BehaviorStack::BuildDebugBehaviorTree(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorStack::StackMetadataEntry::StackMetadataEntry(IBehavior* behavior,
                                                      int indexInStack)
    : behavior(behavior), indexInStack(indexInStack) {
  if (behavior != nullptr) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorStack::StackMetadataEntry::RecursivelyGatherLinkedBehaviors(
    IBehavior* baseBehavior, std::set<IBehavior*>& linkedBehaviors) {
  ...
  BOUNDED_WHILE(1000, !rawBehaviors.empty()) {
    ...
    // If insertion was successful this is a new behavior that needs link
    // checking
    if (res.second) {
      // Anki dev cheats - make sure no one erases behavior set passed in
      // existing behaviors
      if (ANKI_DEV_CHEATS) {
        ...
        for (auto& rBehavior : rawBehaviors) {
          ANKI_VERIFY(dupSet.find(rBehavior) != dupSet.end(),
                      "BehaviorStack.RecursivelyGatherLinkedBehaviors."
                      "BehaviorErasedLinkedSet",
                      "Behavior %s erased the behavior set when asked for "
                      "linked behaviors",
