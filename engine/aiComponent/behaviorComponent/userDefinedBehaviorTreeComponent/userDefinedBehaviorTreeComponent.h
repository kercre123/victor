 ...
 * Created: 7/11/2018
 *
 * Description: This is a component meant to provide functionality that
 * maintains a user-defined behavior tree. It keeps track of the conditions that
 * can be given user-defined response behaviors and provides an interface to
 * add/modify them.
 *
...
class UserDefinedBehaviorTreeComponent
 ...
 public:
  ...
  virtual void InitDependent(
      Vector::Robot* robot,
      const BCCompMap& dependentComponents) override final;
  ...
  // GetDelegationBehavior returns a pointer to the behavior that should be
  // delegated to given the condition. If there is no such behavior, it returns
  // a null ICozmoBehaviorPtr.
  ICozmoBehaviorPtr GetDelegationBehavior(BEIConditionType) const;

  // AddCondition adds a mapping from a confition to a behavior to the
  // user-defined behavior tree. It returns true if it overwrites a previous
  // mapping. It overwrites values by default.
  // TODO: Change this function to accept a map class with a serialize function
  // that can define more
  //       types of dispatching
  bool AddCondition(BEIConditionType, BehaviorID);

  // returns true if the component is enabled (if the feature is enabled from
  // the console)
  ...
  // get all possible delegates - fills the passed in set with the possible
  // delegates
  void GetAllDelegates(std::set<IBehavior*>&) const;

  // fills the vector with all of the conditions that have options for
  // customization
  void GetCustomizableConditions(std::vector<BEIConditionType>&) const;

  RobotDataLoader::ConditionToBehaviorsMap* GetConditionToBehaviorsMap() const {
    return _possibleMappingsPtr;
  }

  // set and get the currently activated condition in the router - if an invalid
  // condition is set, an error witll be thrown NOTE: these functions assume
  // that only the router/selector will be setting/getting this info
  void SetCurrentCondition(BEIConditionType);
  BEIConditionType GetCurrentCondition() const {
    assert(BEIConditionType::Invalid != _currentCondition);
  ...
  void EraseCurrentCondition() {
    _currentCondition = BEIConditionType::Invalid;
  };

 private:
  // NOTE: Mapping to BehaviorIDs disallows the use of anonymous behaviors since
  // there are no
  //       serialization functions for them.
  std::shared_ptr<std::unordered_map<BEIConditionType, BehaviorID>>
      _conditionMappingsPtr;
  RobotDataLoader::ConditionToBehaviorsMap* _possibleMappingsPtr;

  // edit mode condition - when triggered within some set of conditions, used to
  // enter an "edit" mode for the user-defined behavior tree feature
