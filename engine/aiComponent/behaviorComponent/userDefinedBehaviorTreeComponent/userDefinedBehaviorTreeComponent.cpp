...
CONSOLE_VAR(bool, kEnableUserDefinedBehaviorTree, "UserDefinedBehaviorTree",
            false);
...
UserDefinedBehaviorTreeComponent::UserDefinedBehaviorTreeComponent()
    : IDependencyManagedComponent<BCComponentID>(
          this, BCComponentID::UserDefinedBehaviorTreeComponent),
      _conditionMappingsPtr(
          std::make_shared<std::unordered_map<BEIConditionType, BehaviorID>>()),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void UserDefinedBehaviorTreeComponent::InitDependent(
    Vector::Robot* robot, const BCCompMap& dependentComponents) {
  ...
  _possibleMappingsPtr =
      dataAccessorComp.GetUserDefinedConditionToBehaviorsMap();
  ...
  variableSnapshotComp.InitVariable<
      std::unordered_map<BEIConditionType, BehaviorID>>(
      VariableSnapshotId::UserDefinedBehaviorTreeComp_CondToBehaviorMappings,
      _conditionMappingsPtr);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
ICozmoBehaviorPtr UserDefinedBehaviorTreeComponent::GetDelegationBehavior(
    BEIConditionType beiCondType) const {
  const auto mappingIter = _conditionMappingsPtr->find(beiCondType);
  const bool userDefinedBehaviorExists =
      (mappingIter != _conditionMappingsPtr->end());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool UserDefinedBehaviorTreeComponent::AddCondition(
    BEIConditionType beiCondType, BehaviorID behaviorId) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void UserDefinedBehaviorTreeComponent::GetAllDelegates(
  ...
  for (const auto& condMapping : *_possibleMappingsPtr) {
    for (const auto& behaviorId : condMapping.second) {
      ICozmoBehaviorPtr behaviorPtr =
          _behaviorContainer->FindBehaviorByID(behaviorId);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void UserDefinedBehaviorTreeComponent::GetCustomizableConditions(
    std::vector<BEIConditionType>& conditions) const {
  for (const auto& condMapping : *_possibleMappingsPtr) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void UserDefinedBehaviorTreeComponent::SetCurrentCondition(
    BEIConditionType beiCond) {
  // ensure that set current condition only works if current condition is erased
  // before being set
  if (!ANKI_VERIFY(
          _currentCondition == BEIConditionType::Invalid,
          "UserDefinedBehaviorTreeComponent.SetCurrentCondition."
          "ConditionHasAlreadyBeenSet",
          "Condition must be erased before using a SetCurrentCondition.")) {
    return;
  }

  const bool isCondValid =
      _possibleMappingsPtr->find(beiCond) != _possibleMappingsPtr->end();
  if (isCondValid) {
    _currentCondition = beiCond;
  } else {
    PRINT_NAMED_ERROR(
        "UserDefinedBehaviorTreeComponent.SetCurrentCondition."
        "BEIConditionTypeNotCustomizable",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::set<BehaviorID> UserDefinedBehaviorTreeComponent::GetConditionOptions(
    BEIConditionType beiCond) const {
  const auto& mapIter = _possibleMappingsPtr->find(beiCond);
  const bool isCondValid = mapIter != _possibleMappingsPtr->end();
  if (!isCondValid) {
    PRINT_NAMED_ERROR(
        "UserDefinedBehaviorTreeComponent.GetConditionOptions."
        "BEIConditionTypeNotCustomizable",
        "BEIConditionType %s not a customizable condition",
        BEIConditionTypeToString(beiCond));
  }
