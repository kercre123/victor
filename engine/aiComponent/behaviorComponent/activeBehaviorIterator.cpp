...
void ActiveBehaviorIterator::InitDependent(Robot* robot,
                                           const BCCompMap& dependentComps) {
  _bsm = dependentComps.GetComponentPtr<BehaviorSystemManager>();
}

void ActiveBehaviorIterator::IterateActiveCozmoBehaviorsForward(
    CozmoBehaviorCallback operand, const IBehavior* startingBehavior) const {
  ...
  BOUNDED_WHILE(1000, (curr != nullptr) && (continueIterating)) {
    const ICozmoBehavior* cozmoBehaviorPtr =
        dynamic_cast<const ICozmoBehavior*>(curr);
    if (ANKI_VERIFY(cozmoBehaviorPtr != nullptr,
                    "ActiveBehaviorIterator.NonCozmoBehaviorOnStack",
...
void ActiveBehaviorIterator::IterateActiveCozmoBehaviorsBackward(
    CozmoBehaviorCallback operand, const IBehavior* startingBehavior) const {
  const IBehavior* curr = startingBehavior;
