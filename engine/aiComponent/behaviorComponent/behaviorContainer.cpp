...
BehaviorContainer::BehaviorContainer(const BehaviorIDJsonMap& behaviorData)
    : IDependencyManagedComponent<BCComponentID>(
          this, BCComponentID::BehaviorContainer) {
  for (const auto& behaviorIDJsonPair : behaviorData) {
    ...
    if (!behaviorJson.empty()) {
      // PRINT_NAMED_DEBUG("BehaviorContainer.Constructor", "Loading '%s'",
      // fullFileName.c_str());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BehaviorContainer::InitDependent(Robot* robot,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
ICozmoBehaviorPtr BehaviorContainer::FindBehaviorByID(
    BehaviorID behaviorID) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::set<ICozmoBehaviorPtr> BehaviorContainer::FindBehaviorsByClass(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BehaviorContainer::CreateAndStoreBehavior(
    const Json::Value& behaviorConfig) {
  ICozmoBehaviorPtr newBehavior =
      BehaviorFactory::CreateBehavior(behaviorConfig);
  if (newBehavior) {
    ...
    if (addedNewEntry) {
      ...
      DEV_ASSERT_MSG(false, "BehaviorContainer.AddToContainer.DuplicateID",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BehaviorClass BehaviorContainer::GetBehaviorClass(
    ICozmoBehaviorPtr behavior) const {
  return behavior->GetClass();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::string BehaviorContainer::GetClassString(
    BehaviorClass behaviorClass) const {
