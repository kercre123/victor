...
class BehaviorComponentMessageHandler
 ...
 private:
  ...
  void OnEnterInfoFace(BehaviorContainer& bContainer,
                       BehaviorSystemManager& bsm);

  void OnExitInfoFace(BehaviorSystemManager& bsm, BehaviorsBootLoader& bbl,
                      UserIntentComponent& uic, bool clearTrigger);

  void SetupUserIntentEvents();

  ICozmoBehaviorPtr WrapRequestedBehaviorInDispatcherRerun(
  ...
  void SubscribeToWebViz(BehaviorExternalInterface& bei,
                         const BehaviorSystemManager& bsm);
};
