...
class BehaviorTimer {
 public:
  ...
  // difference between current time and the last time this was reset. If it was
  // never reset, the elapsed time is wrt the time of creation
  float GetElapsedTimeInSeconds() const;

  // shortcut for using the above functions for the purpose of cooldowns.
  // true if GetElapsedTimeInSeconds is > cooldown. If !HasBeenReset, the return
  // value is valueIfNoReset
