...
class AlexaComponent : public IDependencyManagedComponent<AIComponentID>,
 ...
 public:
  ...
  virtual void AdditionalUpdateAccessibleComponents(
      AICompIDSet& components) const override;
  virtual void UpdateDependent(const AICompMap& dependentComps) override;

  // Set and reset get in anims and audio events for transitions from Idle to:
  // Listening, Thinking, Speaking. If the a given state is not provided as a
  // key, it will have no response (anim nor audio). You can also play audio
  // without an anim by passing InvalidAnimTrigger. Note this doesn't match the
  // stack-style responses used in UserIntentComponent, because we only expect
  // one alexa behavior for now, and hence one response
  ...
  // check if sign in/out is pending. If it is pending, AlexaComponent will
  // SignIn/SignOut for you after some number of ticks. To avoid this, Claim it,
  // but then you must call SignIn/SignOut. Note that only one request is
  // allowed until SignIn() or SignOut() is called (or a pending request times
  // out).
 ...
 private:
  ...
  struct AlexaUXResponseInfo {
    void SetWaiting(bool waiting);  // sets waitingForGetInCompletion and
                                    // timeout_s, or clears both
