...
// intention of what to do with an object. Add things here to centralize logic
// for the best object to use
enum class ObjectInteractionIntention {
  // any object which can be picked up
  PickUpObjectNoAxisCheck = 0,
  // only pick up upright objects, unless rolling is locked (in which case, pick
  // up any object)
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - ObjectInteractionCache - Maintains a
// map of ObjectInteractionIntentions to the Cache entrys that wrap information
// about the best objects to use
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
class ObjectInteractionInfoCache
 ...
 public:
  ...
  using BestObjectFunction =
      std::function<ObjectID(const std::set<ObjectID>& validObjects)>;

  ObjectID GetBestObjectForIntention(ObjectInteractionIntention intention);
  std::set<ObjectID> GetValidObjectsForIntention(
      ObjectInteractionIntention intention);
  const BlockWorldFilter& GetDefaultFilterForIntention(
      ObjectInteractionIntention intention);

  bool IsObjectValidForInteraction(ObjectInteractionIntention intention,
                                   const ObjectID& object);
  // Ensure that all information in the cache that is necessary to get
  // valid/best objects has been updated this tick before accessing it
  void EnsureInformationValid(ObjectInteractionIntention intention);

 protected:
  using FullValidInteractionArray =
      Util::FullEnumToValueArrayChecker::FullEnumToValueArray<
          ObjectInteractionIntention, BlockWorldFilter*,
          ObjectInteractionIntention::Count>;
  using FullBestInteractionArray =
      Util::FullEnumToValueArrayChecker::FullEnumToValueArray<
          ObjectInteractionIntention, BestObjectFunction,
          ObjectInteractionIntention::Count>;

  void ConfigureObjectInteractionFilters(
      const Robot& robot, FullValidInteractionArray validInteractions,
 ...
 private:
  const Robot& _robot;
  std::unordered_map<ObjectInteractionIntention, ObjectInteractionCacheEntry>
      _trackers;

  static const char* ObjectUseIntentionToString(
      ObjectInteractionIntention intention);

  // Common logic for checking validity of blocks for any Pickup, PopAWheelie,
  // or Roll Interactions
  ...
  // Functions which return the "best" object to use given a set of valid
  // objects
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - ObjectInteractionCacheEntry - wraps
// information about the which blocks are valid and best to use for certain
// intentions, and insure this information is up to date before returning it to
// the requester
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
class ObjectInteractionCacheEntry {
 public:
  ...
  ObjectInteractionCacheEntry(const Robot& robot, const std::string& debugName,
  ...
  const BlockWorldFilter& GetValidObjectsFilter() const {
    assert(_blockWorldFilterValidBlocks);
    return *_blockWorldFilterValidBlocks;
  }

  // Updates the bestObject and validObjects if they haven't been updated this
  // tick
  void EnsureInformationValid();
  // Notify the cache that a tap has been detected on an object
  // Function returns true if the filter can use the object that was tapped,
  // false otherwise
