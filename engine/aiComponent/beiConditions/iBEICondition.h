...
class IBEICondition : public IVisionModeSubscriber {
 public:
  ...
  // Signal to the strategy that it is entering(or exiting) a period of time
  // during which it may be evaluated. Must be called on a strategy before the
  // strategy is evaluated, and should be called with setActive == false before
  // any period of time during which the strategy will CERTAINLY NOT be
  // evaluated. Manages VisionMode subscription via the VisionScheduleMediator,
  // and calls SetActiveInternal on implementers to allow for specialized active
  // functionality.
  void SetActive(BehaviorExternalInterface& behaviorExternalInterface,
                 bool setActive);
  ...
  // If a BEICondition has VisionMode Requirements, override this function to
  // specify them. Modes set here will be automatically managed by the SetActive
  // infrastructure.
  virtual void GetRequiredVisionModes(
      std::set<VisionModeRequest>& requests) const {};

  void SetOwnerDebugLabel(const std::string& ownerLabel) {
 ...
 protected:
  ...
  virtual bool AreConditionsMetInternal(
      BehaviorExternalInterface& behaviorExternalInterface) const = 0;

  // Derived classes which have functionality that should only be carried out
  // during an active part of their lifecycle should override this function.
  virtual void SetActiveInternal(
      BehaviorExternalInterface& behaviorExternalInterface, bool isActive) {}

  // Report here whatever factors influence the AreConditionsMetInternal by
  // calling AddFactor and AddChild on factors. You don't need to add a factor
  // for the value of AreConditionsMetInternal since that is already handled for
  // you. Important: Always call AddFactor on variables in the same order!
  // Otherwise the factors will be marked as if they have changed.
