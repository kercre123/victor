...
class BEIConditionDebugFactors {
 public:
  ...
  // Add a key-value pair. supported types are int, float, string, char*, or
  // bool. Always call this on your variables in the same order, or it will look
  // like things "HaveChanged()"
  template <typename T>
 ...
 private:
  // Helper class with way to much code to accomplish: "union { string, float,
  // int, bool };"
  struct Data {
...
template <typename T>
void BEIConditionDebugFactors::AddFactor(const std::string& name,
                                         const T& value) {
