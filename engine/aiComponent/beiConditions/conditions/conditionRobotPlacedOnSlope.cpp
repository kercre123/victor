...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
ConditionRobotPlacedOnSlope::ConditionRobotPlacedOnSlope(
    const Json::Value& config)
    : IBEICondition(config) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionRobotPlacedOnSlope::AreConditionsMetInternal(
  ...
  const bool isMovingNow =
      std::max({std::abs(g.x), std::abs(g.y), std::abs(g.z)}) >
      _kIsMovingNowGyroThreshold_degPerSec;
  ...
  const bool isStationary =
      (now - _lastMovingTime > _kIsStationaryTimeThreshold_sec);

  // Check pitch angle to see if we're inclined (but not too far as to interrupt
  // 'OnBack' behavior):
  const float pitch_deg = robotInfo.GetPitchAngle().getDegrees();
  const bool isOnIncline = (pitch_deg > _kOnInclinePitchThresholdLow_deg) &&
                           (pitch_deg < _kOnInclinePitchThresholdHigh_deg);
  ...
  const bool recentlyTransitionedToNotPickedUp =
      !isPickedUp &&
      (now - _lastPickedUpTime < _kTransitionToNotPickedUpTimeThreshold_sec);

  // Trigger the behavior?
  bool shouldTrigger = isOnIncline && isStationary &&
                       (isPickedUp || recentlyTransitionedToNotPickedUp);
