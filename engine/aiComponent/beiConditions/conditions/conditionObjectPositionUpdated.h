...
class ConditionObjectPositionUpdated : public IBEICondition,
 ...
 private:
  ...
  struct StrategyParams {
    ...
    TimeStamp_t coolDownDuration_ms =
        600000;  // time since last seen (in same pose) before re-reaction
  ...
  struct ReactionData {
    ...
    // helper function to fake a reaction (useful for cases where we want to
    // ignore reactions)
    void FakeReaction();
  };
  ...
  // handles observing a new ID. Updates internal reaction data. If
  // reactionEnabled is omitted, it checks IsReactionEnabled(). If
  // reactionEnabled is false, it will add the observation but ignore it for
  // reactions (by faking a reaction)
  void HandleNewObservation(s32 id, const Pose3d& pose,
                            RobotTimeStamp_t timestamp,
                            bool reactionEnabled = true);

  // For the next three functions, the bool `matchAnyPose` defaults to false. If
  // true, then it checks poses against any other pose, regardless of ID. If
  // false, it only checks against it's own ID. Cooldown is still checked
  // against the passed in ID

  // returns true if there are any valid targets, false otherwise
  bool HasDesiredReactionTargets(
      BehaviorExternalInterface& behaviorExternalInterface,
      bool matchAnyPose = false) const;

  void HandleObjectObserved(
      BehaviorExternalInterface& behaviorExternalInterface,
      const ExternalInterface::RobotObservedObject& msg);

  using ReactionDataMap = std::map<s32, ReactionData>;

  bool ShouldReactToTarget(BehaviorExternalInterface& behaviorExternalInterface,
