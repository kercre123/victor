 ...
 *
 * Description: Strategy that indicates when an object Cozmo cant see starts
 *moving
 *
...
class ReactionObjectData {
 public:
  ...
  bool ObjectHasMovedLongEnough(
      BehaviorExternalInterface& behaviorExternalInterface);
  bool ObjectUpAxisHasChanged(
      BehaviorExternalInterface& behaviorExternalInterface);
  bool ObjectOutsideIgnoreArea(
      BehaviorExternalInterface& behaviorExternalInterface);

  // update values for reactionObject
  void ObjectStartedMoving(BehaviorExternalInterface& behaviorExternalInterface,
                           const ExternalInterface::ObjectMoved& msg);
  void ObjectStoppedMoving(
      BehaviorExternalInterface& behaviorExternalInterface);
  void ObjectUpAxisChanged(BehaviorExternalInterface& behaviorExternalInterface,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionObjectMoved::AreConditionsMetInternal(
  ...
  while (objIter != _reactionObjects.end()) {
    const ObservableObject* cube =
        behaviorExternalInterface.GetBlockWorld().GetLocatedObjectByID(
            objIter->GetObjectID());
    if (cube == nullptr) {
    ...
    static constexpr float kMaxNormalAngle =
        DEG_TO_RAD(45);  // how steep of an angle we can see
    static constexpr float kMinImageSizePix =
        0.0f;  // just check if we are looking at it

    if (objIter->ObjectOutsideIgnoreArea(behaviorExternalInterface) &&
        ((objIter->ObjectHasMovedLongEnough(behaviorExternalInterface)) ||
         objIter->ObjectUpAxisHasChanged(behaviorExternalInterface))) {
      VisionComponent& visComp = behaviorExternalInterface.GetVisionComponent();
      const bool isVisible = cube->IsVisibleFrom(
          visComp.GetCamera(), kMaxNormalAngle, kMinImageSizePix, false);
      if (!isVisible) {
        objIter->ResetObject();
        return true;
      }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionObjectMoved::HandleEvent(
    const EngineToGameEvent& event,
    BehaviorExternalInterface& behaviorExternalInterface) {
  switch (event.GetData().GetTag()) {
    case EngineToGameTag::ObjectMoved:
      HandleObjectMoved(behaviorExternalInterface,
                        event.GetData().Get_ObjectMoved());
      break;
    case EngineToGameTag::ObjectStoppedMoving:
      HandleObjectStopped(behaviorExternalInterface,
                          event.GetData().Get_ObjectStoppedMoving());
      break;
    case EngineToGameTag::RobotObservedObject:
      HandleObservedObject(behaviorExternalInterface,
                           event.GetData().Get_RobotObservedObject());
      break;
    case EngineToGameTag::ObjectUpAxisChanged:
      HandleObjectUpAxisChanged(behaviorExternalInterface,
                                event.GetData().Get_ObjectUpAxisChanged());
      break;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionObjectMoved::HandleObjectMoved(
    BehaviorExternalInterface& behaviorExternalInterface,
    const ExternalInterface::ObjectMoved& msg) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionObjectMoved::HandleObjectStopped(
    BehaviorExternalInterface& behaviorExternalInterface,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionObjectMoved::HandleObjectUpAxisChanged(
    BehaviorExternalInterface& behaviorExternalInterface,
    const ExternalInterface::ObjectUpAxisChanged& msg) {
  auto iter = GetReactionaryIterator(msg.objectID);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionObjectMoved::HandleObservedObject(
    BehaviorExternalInterface& behaviorExternalInterface,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
ConditionObjectMoved::Reaction_iter
ConditionObjectMoved::GetReactionaryIterator(ObjectID objectID) {
  Reaction_iter iter =
      std::find(_reactionObjects.begin(), _reactionObjects.end(), objectID);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
ObjectID ReactionObjectData::GetObjectID() const { return _objectID; }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ReactionObjectData::ObjectHasMovedLongEnough(
    BehaviorExternalInterface& behaviorExternalInterface) {
  const ObservableObject* object =
      behaviorExternalInterface.GetBlockWorld().GetLocatedObjectByID(_objectID);
  if (object == nullptr) {
    return false;
  }

  if (_isObjectMoving && _observedSinceLastReaction) {
    RobotTimeStamp_t time_ms =
        behaviorExternalInterface.GetRobotInfo().GetLastMsgTimestamp();
    if (_timeStartedMoving != 0 &&
        time_ms - _timeStartedMoving > kMinTimeMoving_ms) {
      return true;
    }
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ReactionObjectData::ObjectUpAxisHasChanged(
    BehaviorExternalInterface& behaviorExternalInterface) {
  const ObservableObject* object =
      behaviorExternalInterface.GetBlockWorld().GetLocatedObjectByID(_objectID);
  if (object == nullptr) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ReactionObjectData::ObjectOutsideIgnoreArea(
    BehaviorExternalInterface& behaviorExternalInterface) {
  const ObservableObject* object =
      behaviorExternalInterface.GetBlockWorld().GetLocatedObjectByID(_objectID);
  if (object == nullptr) {
  ...
  f32 distance = 0;
  bool distanceComputed =
      ComputeDistanceBetween(blockPose, robotPose, distance);
  if (!distanceComputed) {
    return false;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ReactionObjectData::ObjectStartedMoving(
    BehaviorExternalInterface& behaviorExternalInterface,
    const ExternalInterface::ObjectMoved& msg) {
  const ObservableObject* object =
      behaviorExternalInterface.GetBlockWorld().GetLocatedObjectByID(_objectID);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ReactionObjectData::ObjectStoppedMoving(
    BehaviorExternalInterface& behaviorExternalInterface) {
  _isObjectMoving = false;
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ReactionObjectData::ObjectUpAxisChanged(
    BehaviorExternalInterface& behaviorExternalInterface,
    const ExternalInterface::ObjectUpAxisChanged& msg) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ReactionObjectData::ObjectObserved(
    BehaviorExternalInterface& behaviorExternalInterface) {
  const ObservableObject* object =
      behaviorExternalInterface.GetBlockWorld().GetLocatedObjectByID(_objectID);
  if (object != nullptr) {
