...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionFaceKnown::AreConditionsMetInternal(
  ...
  if (_maxFaceAge_s >= 0) {
    const RobotTimeStamp_t latestImageTimestamp =
        behaviorExternalInterface.GetRobotInfo().GetLastImageTimeStamp();
    const auto maxFaceAge_ms = 1000 * _maxFaceAge_s;
    RobotTimeStamp_t minAge = (latestImageTimestamp > maxFaceAge_ms)
                                  ? (latestImageTimestamp - maxFaceAge_ms)
                                  : 0;
  ...
  for (const auto& faceID : faces) {
    ...
    if ((face != nullptr) && (!_mustBeNamed || face->HasName())) {
      if (_maxFaceDist_mm < 0.0f) {
        ...
        if (ComputeDistanceSQBetween(
                behaviorExternalInterface.GetRobotInfo().GetPose(), facePose,
                distanceToFace) &&
