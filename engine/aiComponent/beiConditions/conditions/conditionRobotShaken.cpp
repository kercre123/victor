...
ConditionRobotShaken::ConditionRobotShaken(const Json::Value& config)
    : IBEICondition(config), _minTriggerMagnitude(kDefaultAccelThreshold) {
  JsonTools::GetValueOptional(config, kKeyAccelMagnitude, _minTriggerMagnitude);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionRobotShaken::AreConditionsMetInternal(
    BehaviorExternalInterface& behaviorExternalInterface) const {
  // trigger this behavior when the filtered total accelerometer magnitude data
  // exceeds a threshold
  const auto& robotInfo = behaviorExternalInterface.GetRobotInfo();
  const bool isShaken =
      (robotInfo.GetHeadAccelMagnitudeFiltered() >= _minTriggerMagnitude);
