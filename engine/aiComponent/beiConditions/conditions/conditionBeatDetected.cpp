...
ConditionBeatDetected::ConditionBeatDetected(const Json::Value& config)
  ...
  JsonTools::GetValueOptional(config, "allowPotentialBeat",
                              _allowPotentialBeat);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionBeatDetected::AreConditionsMetInternal(
    BehaviorExternalInterface& bei) const {
  const auto& beatDetector = bei.GetBeatDetectorComponent();
  return beatDetector.IsBeatDetected() ||
         (_allowPotentialBeat && beatDetector.IsPossibleBeatDetected());
}
