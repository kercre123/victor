...
// XXX: these lambdas will only be called if the paired tag you provide (in json
// or in ctor) matches the pending intent, so there's no real need to check the
// tag

std::unordered_map<std::string, IConditionUserIntent::EvalUserIntentFunc>
    IConditionUserIntent::sMap = {
        // PLEASE KEEP ALPHABETIZED

        {// test_lambda. Only used in unit tests. keep this at the end
         "test_lambda", [](const UserIntent& intent) {
           DEV_ASSERT(intent.GetTag() == UserIntentTag::test_name,
                      "Tag should have already been verified");
...
IConditionUserIntent::IConditionUserIntent(BEIConditionType type,
    ...
    : IBEICondition(config), _type(type) {
  ANKI_VERIFY((_type == BEIConditionType::UserIntentPending) ||
                  (_type == BEIConditionType::UserIntentActive),
              "IConditionUserIntent.Ctor.InvalidBEICondType",
              "Condition type '%s' not supported",
              BEIConditionTypeToString(type));

  // provide a list of ways to compare. if any entry is true, this condition
  // will be true. you may: (1) provide _only_ a UserIntentTag, like { "type":
  // "mytag" }, and _only_ the tag will be compared (2) provide the full
  // definition, like { "type": "mytag", "parameter": "" }, and _all_ fields
  // will be compared (3) provide a lambda, like { "type": "mytag", "_lambda":
  // "my_lambda" } which names a lambda above
  ...
  ANKI_VERIFY(!_evalList.empty(), "IConditionUserIntent.Ctor.NoElems",
...
void IConditionUserIntent::AddUserIntent(const Json::Value& json) {
  ...
  if (ANKI_VERIFY(intent.SetFromJSON(json),
    ...
    if (!ANKI_VERIFY(intent.GetTag() != USER_INTENT(INVALID),
                     "IConditionUserIntent.Ctor.InvalidTag", "Invalid tag")) {
      return;
    }
    ...
    if (!evalFuncStr.empty()) {
      ...
      if (ANKI_VERIFY(tmp != nullptr, "IConditionUserIntent.Ctor.MissingLambda",
                      "Lambda '%s' is invalid", evalFuncStr.c_str())) {
        evals.func = &tmp;
      }

      // if you provide a lambda, we don't check any other fields. just tag +
      // lambda
      DEV_ASSERT(json.getMemberNames().size() == 2,
                 "Provide only '_lambda' and the 'type'.");
    } else if (json.size() > 1) {
      // do an inefficient check that the number of supplied params matches the
      // number of struct fields. Otherwise, we don't know if the absence of a
      // param means it should match the default struct field value or
      // comparison shouldn't be performed
    ...
    if (ANKI_DEVELOPER_CODE) {
      // validate that for any one tag, there's only one full UserIntent
      // specified, and only one entry identified only by the tag (no func and
      // no intent).
      for (const auto& existing : _evalList) {
        if (existing.tag == evals.tag) {
          DEV_ASSERT(
              !((existing.intent != nullptr) && (evals.intent != nullptr)),
              "two intents per tag");
          DEV_ASSERT(
              !((existing.func == nullptr) && (evals.func == nullptr) &&
                (existing.intent == nullptr) && (evals.intent == nullptr)),
              "two tags found");
        }
      }
...
void IConditionUserIntent::AddUserIntent(UserIntentTag tag,
  ...
  const bool validFunc = ANKI_VERIFY(
      func != nullptr, "IConditionUserIntent.AddUserIntent.NullFunc",
      "The function provided for '%s' was null. Use an overloaded method",
      UserIntentTagToString(tag));
  const bool notInvalid = ANKI_VERIFY(
      tag != USER_INTENT(INVALID),
      "IConditionUserIntent.AddUserIntent.InvalidTag", "The tag is INVALID");
...
void IConditionUserIntent::AddUserIntent(UserIntent&& intent) {
  ...
  const bool notInvalid = ANKI_VERIFY(
      tag != USER_INTENT(INVALID),
      "IConditionUserIntent.AddUserIntent.InvalidTag2", "The tag is INVALID");
  if (notInvalid) {
    _evalList.emplace_back(tag);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool IConditionUserIntent::AreConditionsMetInternal(
  ...
  const auto& uic = behaviorExternalInterface.GetAIComponent()
  ...
  for (const auto& elem : _evalList) {
    ...
    if (gotIntent) {
      ...
      if (elem.func != nullptr) {
        ...
        // todo: enforce that the type of argument to the lambda matches the tag
        // in the pair, i.e., lambda takes typename T as "const T& intent" (like
        // T==UserIntent_MyType) with a clad TagToType check against the param.
        // Should be possible in the constructor versions, not json
        if ((*elem.func)(intent)) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void IConditionUserIntent::SetActiveInternal(
...
UserIntentTag IConditionUserIntent::GetUserIntentTagSelected() const {
  if (_selectedTag == USER_INTENT(INVALID)) {
    PRINT_NAMED_WARNING(
        "IConditionUserIntent.GetUserIntentTagSelected.Invalid",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
const IConditionUserIntent::EvalUserIntentFunc&
IConditionUserIntent::GetFromName(const std::string& name) const {
  const auto it = sMap.find(name);
  if (ANKI_VERIFY(it != sMap.end(),
                  "IConditionUserIntent.GetFromName.MissingLambda",
                  "Could not find lambda '%s'", name.c_str())) {
    return it->second;
  }
...
void IConditionUserIntent::BuildDebugFactorsInternal(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
IConditionUserIntent::EvalStruct::EvalStruct(UserIntentTag it) : tag(it) {}
