...
// this is something that can use the protected ConditionCompound ctor in a
// std::make_shared
...
ConditionCompound::ConditionCompound(const Json::Value& config)
  ...
  // If you hit this, try as hard as you can to refactor your conditions. Maybe
  // some of them can go in the behavior as opposed to the json? Maybe some new
  // condition types are needed? The constant can be changed without detrimental
  // effects, but if this becomes large, we will all regret it. The current
  // constant (2) allows for, e.g., AND(NOT(condition1),condition2) and nothing
  // more
  ANKI_VERIFY(maxDepth <= 2, "ConditionCompound.Ctor.MaxDepthExceeded",
              "Depth=%d", maxDepth);
}
...
ConditionCompound::ConditionCompound(const std::string& ownerDebugLabel)
    : IBEICondition(IBEICondition::GenerateBaseConditionConfig(
          BEIConditionType::Compound)) {
  SetOwnerDebugLabel(ownerDebugLabel);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
IBEIConditionPtr ConditionCompound::CreateAndCondition(
    const std::vector<IBEIConditionPtr>& subConditions,
    const std::string& ownerDebugLabel) {
  DEV_ASSERT(!subConditions.empty(),
             "Cannot create an AND condition with empty subConditions");
  IBEIConditionPtr ret =
      CreateSingleLevelCondition(NodeType::And, subConditions, ownerDebugLabel);
  return ret;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
IBEIConditionPtr ConditionCompound::CreateOrCondition(
    const std::vector<IBEIConditionPtr>& subConditions,
    const std::string& ownerDebugLabel) {
  DEV_ASSERT(!subConditions.empty(),
             "Cannot create an OR condition with empty subConditions");
  IBEIConditionPtr ret =
      CreateSingleLevelCondition(NodeType::Or, subConditions, ownerDebugLabel);
  return ret;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
IBEIConditionPtr ConditionCompound::CreateNotCondition(
    IBEIConditionPtr subCondition, const std::string& ownerDebugLabel) {
  IBEIConditionPtr ret = CreateSingleLevelCondition(
      NodeType::Not, {subCondition}, ownerDebugLabel);
  return ret;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
IBEIConditionPtr ConditionCompound::CreateSingleLevelCondition(
  ...
  for (size_t idx = 0; idx < subConditions.size(); ++idx) {
    DEV_ASSERT(subConditions[idx] != nullptr,
               "Constructing a ConditionCompound with a null condition!");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionCompound::AreConditionsMetInternal(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionCompound::SetActiveInternal(BehaviorExternalInterface& bei,
                                          bool setActive) {
  for (const auto& operand : _operands) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionCompound::GetRequiredVisionModes(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
int ConditionCompound::CreateNode(std::unique_ptr<Node>& node,
                                  const Json::Value& config,
                                  const bool isRoot = false) {
  int childrenDepth = 0;
  node.reset(new Node);
  if (!isRoot &&
      BEIConditionFactory::IsValidCondition(config)) {  // this is a leaf
    node->type = NodeType::Condition;
    IBEIConditionPtr condition =
        BEIConditionFactory::CreateBEICondition(config, GetDebugLabel());
    ...
    ANKI_VERIFY(!childConfig.isArray(), "ConditionCompound.CreateNode.TooMany",
                "Too many children for a NOT condition");
    ...
    if (!config[kAndKey].isNull()) {
      ...
      JsonTools::PrintJsonError(
          config, "ConditionCompound.CreateNode.Unknown.Config", maxPrintDepth);
      ASSERT_NAMED(false && "unknown or missing type name",
                   "ConditionCompound.CreateNode.Unknown");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::vector<bool> ConditionCompound::EvaluateConditions(
    BehaviorExternalInterface& bei) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionCompound::EvaluateNode(
  ...
  switch (node->type) {
    ...
    case NodeType::Invalid: {
      ASSERT_NAMED(false && "unknown type",
                   "ConditionCompound.EvaluateNode.Unknown");
    } break;
  }
  return ret;
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionCompound::BuildDebugFactorsInternal(
    BEIConditionDebugFactors& factors) const {
  DEV_ASSERT(_conditionNames.size() == _operands.size(),
             "ConditionCompound.BuildDebugFactorsInternal.Mismatch");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::string ConditionCompound::GetDebugString(
...
std::vector<IBEIConditionPtr> ConditionCompound::TESTONLY_GetOperands(
