...
ConditionTimedDedup::ConditionTimedDedup(const Json::Value& config)
  ...
  _instanceParams.subCondition = BEIConditionFactory::CreateBEICondition(
      config[kSubConditionKey], GetDebugLabel());
  _instanceParams.dedupInterval_ms =
      JsonTools::ParseFloat(config, kDedupIntervalKey,
                            "ConditionTimedDedup.ConfigError.DedupInterval");
  ANKI_VERIFY(_instanceParams.subCondition,
              "ConditionTimedDedup.Constructor.Config.NullSubCondition", "");
}
...
ConditionTimedDedup::ConditionTimedDedup(IBEIConditionPtr subCondition,
  ...
  ANKI_VERIFY(_instanceParams.subCondition,
              "ConditionTimedDedup.Constructor.Direct.NullSubCondition", "");
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionTimedDedup::AreConditionsMetInternal(
  ...
  if (_instanceParams.subCondition &&
    ...
    if (subResult) {
      _lifetimeParams.nextTimeValid_ms =
          currentTime_ms + _instanceParams.dedupInterval_ms;
    }
    return subResult;
  }
  return false;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionTimedDedup::SetActiveInternal(BehaviorExternalInterface& bei,
                                            bool setActive) {
  if (_instanceParams.subCondition) {
