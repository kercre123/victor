...
ConditionRobotTouched::ConditionRobotTouched(const Json::Value& config)
    : IBEICondition(config), _kMinTouchTime(0.0f) {
  _kMinTouchTime = JsonTools::ParseFloat(
      config, kMinTouchTimeKey,
      "ConditionRobotTouched.ConfigError.NeedsMinTouchTime");
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionRobotTouched::AreConditionsMetInternal(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionRobotTouched::IsReceivingTouch(
    BehaviorExternalInterface& behaviorExternalInterface) const {
  const auto now = BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  bool ret = false;
  const bool currPressed =
      behaviorExternalInterface.GetTouchSensorComponent().GetIsPressed();
  const auto touchPressTime =
      behaviorExternalInterface.GetTouchSensorComponent().GetTouchPressTime();
