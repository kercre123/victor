...
ConditionBehaviorTimer::ConditionBehaviorTimer(const Json::Value& config)
  ...
  ANKI_VERIFY(BehaviorTimerManager::IsValidName(_timerName),
              ...
              "Behavior timer '%s' is not valid", _timerName.c_str());
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionBehaviorTimer::AreConditionsMetInternal(
    BehaviorExternalInterface& behaviorExternalInterface) const {
  const auto timerType =
      BehaviorTimerManager::BehaviorTimerFromString(_timerName);
  const auto& timer =
      behaviorExternalInterface.GetBehaviorTimerManager().GetTimer(timerType);
  const bool valueIfNoReset = true;
  const bool expired =
      timer.HasCooldownExpired(_cooldown_s / kTimeMultiplier, valueIfNoReset);
  return expired;
}
