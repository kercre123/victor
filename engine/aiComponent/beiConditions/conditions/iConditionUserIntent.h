 ...
 * Created: 2018 Feb 13
 *
 * Description: Base condition to check if one or more user intent(s) are
 *pending or active
 *
...
class IConditionUserIntent : public IBEICondition {
 public:
  ...
  // the condition will be true if the lambda evaluates to true. Note that the
  // lambda will only be called if the tag matches, so your func need not check
  // the tag. i.e., only worry about the params
  using EvalUserIntentFunc = std::function<bool(const UserIntent&)>;
  void AddUserIntent(UserIntentTag tag, EvalUserIntentFunc&& func);

  // the condition will be true if the pending intent fully matches the provided
  // intent
  void AddUserIntent(UserIntent&& intent);

  // this works the same as how the constructor handles elements in its config's
  // "list"
  void AddUserIntent(const Json::Value& json);

  // if this condition is true, then this condition should be able to return the
  // tag that made it true out of the list passed during construction
 ...
 protected:
  ...
  // special constructor for which you MUST immediately use one of the Add*
  // methods below, or risk an assert
  explicit IConditionUserIntent(BEIConditionType type);
 ...
 private:
  ...
  struct EvalStruct {
    ...
    const EvalUserIntentFunc* func =
        nullptr;  // if set, will be evaluated with the pending intent
    std::unique_ptr<UserIntent> intent;  // if set, tests for equality in all
                                         // fields with the pending intent
  };

  // list of comparisons to be performed when checking a pending intent
  std::vector<EvalStruct> _evalList;

  // this holds eval funcs that were arguments in the AddUserIntent that accepts
  // lambdas
  std::list<EvalUserIntentFunc> _ownedFuncs;

  // if any in _evalList matches, the tag is assigned here, so we know which out
  // of _evalList was true
