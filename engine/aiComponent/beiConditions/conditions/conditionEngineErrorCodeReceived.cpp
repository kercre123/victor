...
ConditionEngineErrorCodeReceived::ConditionEngineErrorCodeReceived(
  ...
  _iConfig.engineErrorCodeToCheck = EngineErrorCodeFromString(
      JsonTools::ParseString(config, kEngineErrorCodeKey,
                             "ConditionalEngineErrorCodeReceived.Constructor"));
}
...
void ConditionEngineErrorCodeReceived::InitInternal(
...
bool ConditionEngineErrorCodeReceived::AreConditionsMetInternal(
    BehaviorExternalInterface& bei) const {
  return _dVars.engineErrorCodeMatches;
}

void ConditionEngineErrorCodeReceived::HandleEvent(
    const EngineToGameEvent& event, BehaviorExternalInterface& bei) {
  switch (event.GetData().GetTag()) {
    case ExternalInterface::MessageEngineToGameTag::EngineErrorCodeMessage: {
      ...
      _dVars.engineErrorCodeMatches =
          msg.errorCode == _iConfig.engineErrorCodeToCheck;
      break;
    }
    default: {
      PRINT_NAMED_ERROR(
          "ConditionEngineErrorCodeReceived.HandleEvent.InvalidEvent", "");
    }
  }
}
