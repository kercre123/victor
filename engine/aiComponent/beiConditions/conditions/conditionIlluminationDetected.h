 ...
 *
 *              The condition will be true for a specified number of seconds
 *after pre and post-transition conditions have been met.
 *
 *              Specify "preTriggerStates" and/or "postTriggerStates" in JSON,
 *as well as accompanying latch parameters (see
 *conditionIlluminationDetect.cpp). If no states are given for a set of
 *triggers, any illumination state will satisfy that requirement.
 *
 *              To fire on entering a state, specify only "preTriggerStates"
 *              To fire on exiting a state, specify only "postTriggerStates"
 *              To fire on transitioning, specify both "preTriggerStates and
 *"postTriggerStates"
 *
...
class ConditionIlluminationDetected : public IBEICondition,
 ...
 public:
  ...
  virtual void GetRequiredVisionModes(
      std::set<VisionModeRequest>& request) const override;
 ...
 private:
  struct ConfigParams {
    std::vector<IlluminationState>
        preStates;              // Transition starts by entering these states
    ...
    std::vector<IlluminationState>
        postStates;              // Transition finishes by entering these states
    ...
    f32 matchHoldTime_s;  // Number of seconds to hold the condition true on
                          // transition
  };
  ...
  struct DynamicVariables {
    ...
    RobotTimeStamp_t matchStartTime;  // Time at which first match in series
                                      // received (if matched)
  ...
  void TickStateMachine(const RobotTimeStamp_t& currTime,
                        const IlluminationState& obsState);
