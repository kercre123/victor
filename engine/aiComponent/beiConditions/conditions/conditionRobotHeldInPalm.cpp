...
ConditionRobotHeldInPalm::ConditionRobotHeldInPalm(const Json::Value& config)
  ...
  _minDuration_ms =
      Util::SecToMilliSec(config.get(kMinDurationKey, 0.0).asFloat());
  _maxDuration_ms =
      Util::SecToMilliSec(config.get(kMaxDurationKey, FLT_MAX).asFloat());
}

ConditionRobotHeldInPalm::ConditionRobotHeldInPalm(
    const bool shouldBeHeldInPalm, const std::string& ownerDebugLabel,
    const int minDuration_ms, const int maxDuration_ms)
    : IBEICondition(IBEICondition::GenerateBaseConditionConfig(
          BEIConditionType::RobotHeldInPalm)),
...
bool ConditionRobotHeldInPalm::AreConditionsMetInternal(
  ...
  if (heldInPalmTracker.IsHeldInPalm() == _shouldBeHeldInPalm) {
    if (_shouldBeHeldInPalm) {
      const u32 heldInPalmDuration_ms =
          heldInPalmTracker.GetHeldInPalmDuration_ms();
      return heldInPalmDuration_ms >= _minDuration_ms &&
             heldInPalmDuration_ms <= _maxDuration_ms;
    } else {
      const u32 notHeldInPalmDuration_ms =
          heldInPalmTracker.GetTimeSinceLastHeldInPalm_ms();
      return notHeldInPalmDuration_ms >= _minDuration_ms &&
             notHeldInPalmDuration_ms <= _maxDuration_ms;
    }
  }
  return false;
}
