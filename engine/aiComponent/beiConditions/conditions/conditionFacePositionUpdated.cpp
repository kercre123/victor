...
CONSOLE_VAR_RANGED(f32, kDistanceToConsiderClose_mm, "AcknowledgementBehaviors",
                   300.0f, 0.0f, 1000.0f);
CONSOLE_VAR_RANGED(f32, kDistanceToConsiderClose_gap_mm,
                   "AcknowledgementBehaviors", 100.0f, 0.0f, 1000.0f);
CONSOLE_VAR_RANGED(f32, kFaceReactCooldown_s, "AcknowledgementBehaviors", 4.0f,
                   0.0f, 60.0f);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionFacePositionUpdated::InitInternal(
    BehaviorExternalInterface& behaviorExternalInterface) {
  _messageHelper.reset(
      new BEIConditionMessageHelper(this, behaviorExternalInterface));
  _messageHelper->SubscribeToTags({EngineToGameTag::RobotObservedFace});
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionFacePositionUpdated::AreConditionsMetInternal(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionFacePositionUpdated::HandleEvent(
    const EngineToGameEvent& event,
    BehaviorExternalInterface& behaviorExternalInterface) {
  switch (event.GetData().GetTag()) {
    case EngineToGameTag::RobotObservedFace: {
      HandleFaceObserved(behaviorExternalInterface,
                         event.GetData().Get_RobotObservedFace());
      break;
    }

    default: {
      PRINT_NAMED_ERROR(
          "ReactionStrategyFacePositionUpdate.HandleMessages.InvalidTag",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionFacePositionUpdated::HandleFaceObserved(
    BehaviorExternalInterface& behaviorExternalInterface,
  ...
  const Vision::TrackedFace* face =
      behaviorExternalInterface.GetFaceWorld().GetFace(msg.faceID);
  if (nullptr == face) {
    return;
  }

  // We always want to react the first time we see a named face
  const bool newNamedFace =
      face->HasName() &&
      _hasReactedToFace.find(msg.faceID) == _hasReactedToFace.end();

  if (newNamedFace) {
    bool added = AddDesiredFace(behaviorExternalInterface, msg.faceID);
    if (added) {
      PRINT_NAMED_DEBUG(
          "BehaviorAcknowledgeFace.InitialFaceReaction",
  ...
  // We also want to react if the face was previously seen far away, but is now
  // seen closer up
  if (!ComputeDistanceBetween(robotInfo.GetPose(), face->GetHeadPose(),
                              currDistance_mm)) {
    PRINT_NAMED_ERROR("BehaviorAcknowledgeFace.PoseInWrongFrame",
                      "We couldnt get the distance from the robot to the face "
                      "pose that we just saw...");
  ...
  if (kDebugFaceDist) {
    PRINT_CH_INFO("ReactionTriggers", "BehaviorAcknowledgeFace.Debug.FaceDist",
                  "%d: %fmm", msg.faceID, currDistance_mm);
  }
  ...
  const bool notOnCooldown =
      _lastReactionTime_s < 0.0f ||
      _lastReactionTime_s + kFaceReactCooldown_s <= currTime_s;
  if (faceBecameClose && notOnCooldown && !newNamedFace) {
    bool added = AddDesiredFace(behaviorExternalInterface, msg.faceID);
    if (added) {
      PRINT_NAMED_DEBUG("BehaviorAcknowledgeFace.FaceBecomeClose",
                        "face ID %d became close (currDist = %fmm)", msg.faceID,
                        currDistance_mm);
    }
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionFacePositionUpdated::AddDesiredFace(
    BehaviorExternalInterface& behaviorExternalInterface,
