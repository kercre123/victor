...
ConditionOffTreadsState::ConditionOffTreadsState(const Json::Value& config)
  ...
  const std::string& targetStateStr = JsonTools::ParseString(
      config, "targetState", "ConditionOffTreadsState.Config");
  ANKI_VERIFY(OffTreadsStateFromString(targetStateStr, _targetState),
              "ConditionOffTreadsState.Config.IncorrectString",
              "%s is not a valid OffTreadsState", targetStateStr.c_str());
...
ConditionOffTreadsState::ConditionOffTreadsState(
    const OffTreadsState& targetState, const std::string& ownerDebugLabel)
    : IBEICondition(IBEICondition::GenerateBaseConditionConfig(
          BEIConditionType::OffTreadsState)),
      _minTimeSinceChange_ms(0),
...
bool ConditionOffTreadsState::AreConditionsMetInternal(
  ...
  if (currState == _targetState) {
    ...
    const EngineTimeStamp_t lastChangedTime_ms =
        bei.GetRobotInfo().GetOffTreadsStateLastChangedTime_ms();
    ret = (currTime_ms - lastChangedTime_ms >= _minTimeSinceChange_ms) &&
          ((_maxTimeSinceChange_ms < 0) ||
