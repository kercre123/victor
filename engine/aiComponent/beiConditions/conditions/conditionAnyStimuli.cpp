 ...
 * Created: May 15 2018
 *
 * Description: Condition that standardizes the params of stimuli conditions, so
 *you only specify the stimuli.
 *
...
const static std::set<BEIConditionType> kSupportedTypes = {
    BEIConditionType::MotionDetected,      BEIConditionType::FaceKnown,
    BEIConditionType::FacePositionUpdated, BEIConditionType::RobotTouched,
    BEIConditionType::RobotShaken,         BEIConditionType::RobotPoked,
};
}
...
ConditionAnyStimuli::ConditionAnyStimuli(const Json::Value& config)
  ...
  for (const auto& type : kSupportedTypes) {
    ...
    bool typeUsed = JsonTools::ParseBool(
        config, typeStr, "ConditionAnyStimuli.Ctor.MissingRequired");
    if (typeUsed) {
      AddCondition(type);
    }
  }
  ANKI_VERIFY(!_conditions.empty(), "ConditionAnyStimuli.Ctor.NoStimuli",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
ConditionAnyStimuli::ConditionAnyStimuli(
    const std::set<BEIConditionType>& types)
    : IBEICondition(IBEICondition::GenerateBaseConditionConfig(
          BEIConditionType::AnyStimuli)) {
  for (const auto& type : types) {
    if (ANKI_VERIFY(std::find(kSupportedTypes.begin(), kSupportedTypes.end(),
                              type) != kSupportedTypes.end(),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionAnyStimuli::InitInternal(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionAnyStimuli::GetRequiredVisionModes(
    std::set<VisionModeRequest>& requiredVisionModes) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionAnyStimuli::SetActiveInternal(
    BehaviorExternalInterface& behaviorExternalInterface, bool active) {
  for (auto& cond : _conditions) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionAnyStimuli::AreConditionsMetInternal(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionAnyStimuli::BuildDebugFactorsInternal(
