 ...
 *
 * Description: Condition that checks the observed scene illumination entering
 *and/or leaving from specified states
 *
...
ConditionIlluminationDetected::ConditionIlluminationDetected(
  ...
  if (ParseTriggerStates(config, kPreTriggerStatesKey, _params.preStates)) {
    _params.preConfirmationTime_s =
        JsonTools::ParseFloat(config, kPreConfirmationTimeKey,
                              "ConditionIlluminationDetected.Constructor");
    _params.preConfirmationMinNum =
        JsonTools::ParseUInt32(config, kPreConfirmationMinNumKey,
                               "ConditionIlluminationDetected.Constructor");
  }
  ...
  if (ParseTriggerStates(config, kPostTriggerStatesKey, _params.postStates)) {
    _params.postConfirmationTime_s =
        JsonTools::ParseFloat(config, kPostConfirmationTimeKey,
                              "ConditionIlluminationDetected.Constructor");
    _params.postConfirmationMinNum =
        JsonTools::ParseUInt32(config, kPostConfirmationMinNumKey,
                               "ConditionIlluminationDetected.Constructor");
  }

  _params.matchHoldTime_s = JsonTools::ParseFloat(
      config, "matchHoldTime", "ConditionIlluminationDetect.Constructor");
}

ConditionIlluminationDetected::~ConditionIlluminationDetected() {}

void ConditionIlluminationDetected::GetRequiredVisionModes(
    std::set<VisionModeRequest>& request) const {
  request.insert({VisionMode::Illumination, EVisionUpdateFrequency::High});
}

void ConditionIlluminationDetected::InitInternal(
...
bool ConditionIlluminationDetected::AreConditionsMetInternal(
    BehaviorExternalInterface& bei) const {
  return MatchState::MatchConfirmed == _variables.matchState;
}

void ConditionIlluminationDetected::HandleEvent(
  ...
  switch (event.GetData().GetTag()) {
    ...
    default: {
      PRINT_NAMED_ERROR(
          "ConditionIlluminationDetected.HandleEvent.InvalidEvent", "");
    }
  }
}

void ConditionIlluminationDetected::TickStateMachine(
    const RobotTimeStamp_t& currTime, const IlluminationState& obsState) {
  switch (_variables.matchState) {
    ...
    default: {
      PRINT_NAMED_ERROR(
          "ConditionIlluminationDetected.HandleIllumination.InvalidState", "");
      Reset();
    }
  }
}

bool ConditionIlluminationDetected::IsTimePassed(const RobotTimeStamp_t& t,
                                                 const f32& dur) const {
  if (t < _variables.matchStartTime) {
    PRINT_NAMED_ERROR(
        "ConditionIlluminationDetected.HandleIllumination.TimeError",
        "Time has gone backwards!");
    return false;
  }

  return Util::IsFltGT(Util::MilliSecToSec(f32(t - _variables.matchStartTime)),
                       dur);
}

bool ConditionIlluminationDetected::IsTriggerState(
    const std::vector<IlluminationState>& triggers,
...
bool ConditionIlluminationDetected::ParseTriggerStates(
    const Json::Value& config, const char* key,
  ...
  for (auto iter = stateStrs.begin(); iter != stateStrs.end(); ++iter) {
    if (!EnumFromString(*iter, state)) {
      PRINT_NAMED_ERROR(
          "ConditionIlluminationDetected.ParseTriggerStates.InvalidState",
