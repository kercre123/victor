...
class ConditionFacePositionUpdated : public IBEICondition,
 ...
 private:
  void HandleFaceObserved(BehaviorExternalInterface& behaviorExternalInterface,
                          const ExternalInterface::RobotObservedFace& msg);

  // helper to add a face as something we want to react to (or not, if we are
  // disabled). Returns true if it added anything
  bool AddDesiredFace(BehaviorExternalInterface& behaviorExternalInterface,
                      Vision::FaceID_t faceID);

  float _lastReactionTime_s = -1.0f;

  // everything we want to react to before we stop (to handle multiple faces in
  // the same frame)
  ...
  // true if we saw the face close last time, false otherwise (applies
  // hysteresis internally)
