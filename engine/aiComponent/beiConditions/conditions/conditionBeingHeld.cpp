...
ConditionBeingHeld::ConditionBeingHeld(const Json::Value& config)
  ...
  _shouldBeHeld =
      JsonTools::ParseBool(config, "shouldBeHeld", "ConditionBeingHeld.Config");
...
bool ConditionBeingHeld::AreConditionsMetInternal(
  ...
  if (bei.GetRobotInfo().IsBeingHeld() == _shouldBeHeld) {
    ...
    const EngineTimeStamp_t lastChangedTime_ms =
        bei.GetRobotInfo().GetBeingHeldLastChangedTime_ms();
