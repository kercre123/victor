...
bool ConditionHighTemperature::AreConditionsMetInternal(
    BehaviorExternalInterface& bei) const {
  const bool hotCPU = bei.GetRobotInfo().GetCpuTemperature_degC() >=
                      kCPUOverheatingThreshold_degC;
  const bool hotBattery =
      bei.GetRobotInfo().GetBatteryComponent().GetBatteryTemperature_C() >=
      kBatteryOverheatingThreshold_degC;
