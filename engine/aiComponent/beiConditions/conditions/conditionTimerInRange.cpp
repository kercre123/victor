...
ConditionTimerInRange::ConditionTimerInRange(const Json::Value& config)
  ...
  const bool gotBegin = JsonTools::GetValueOptional(config, kBeginRangeKey,
                                                    _params._rangeBegin_s);
  const bool gotEnd =
      JsonTools::GetValueOptional(config, kEndRangeKey, _params._rangeEnd_s);
  JsonTools::GetValueOptional(config, kManualResetKey,
                              _params._manualResetOnly);

  ANKI_VERIFY(gotBegin || gotEnd, "ConditionTimerInRange.BadConfig",
              "Need to specify at least one of '%s' or '%s'", kBeginRangeKey,
              kEndRangeKey);
}

void ConditionTimerInRange::SetActiveInternal(BehaviorExternalInterface& bei,
                                              bool setActive) {
...
bool ConditionTimerInRange::AreConditionsMetInternal(
...
void ConditionTimerInRange::BuildDebugFactorsInternal(
