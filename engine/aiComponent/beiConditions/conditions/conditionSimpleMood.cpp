 ...
 * Created: apr 11 2018
 *
 * Description: True if the SimpleMood takes on a given value, or if it
 *transitions from one value to another this tick, based on config
 *
...
ConditionSimpleMood::ConditionSimpleMood(const Json::Value& config)
  ...
  if (!config[kFromKey].isNull() || !config[kToKey].isNull()) {
    ...
    DEV_ASSERT(config[kMoodKey].isNull(),
               "Can't specify both mood and from/to");
    ...
    if (!fromStr.empty()) {
      ANKI_VERIFY(SimpleMoodTypeFromString(fromStr, _from),
                  ...
                  "Unknown mood type %s", fromStr.c_str());
    }
    if (!toStr.empty()) {
      ANKI_VERIFY(SimpleMoodTypeFromString(toStr, _to),
                  "ConditionSimpleMood.Ctor.InvalidTo", "Unknown mood type %s",
                  toStr.c_str());
    }
  } else {
    ...
    ANKI_VERIFY(SimpleMoodTypeFromString(moodStr, _mood),
                "ConditionSimpleMood.Ctor.InvalidMood", "Unknown mood type %s",
                moodStr.c_str());
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionSimpleMood::AreConditionsMetInternal(
  ...
  if (behaviorExternalInterface.HasMoodManager()) {
    ...
    if (!_useTransition) {
      ...
      if ((_from == SimpleMoodType::Count) && (_to != SimpleMoodType::Count)) {
        ...
        ret =
            (currMood == _to) && moodManager.DidSimpleMoodTransitionThisTick();
      } else if ((_from != SimpleMoodType::Count) &&
                 (_to == SimpleMoodType::Count)) {
        ...
        ret = (currMood == _to) &&
              moodManager.DidSimpleMoodTransitionThisTick(_from, _to);
      } else {
        DEV_ASSERT(false && "from and to both null",
                   "Should not be possible given json loading");
      }
    }
  }
  return ret;
