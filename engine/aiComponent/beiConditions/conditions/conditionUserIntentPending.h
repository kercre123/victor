...
class ConditionUserIntentPending : public IConditionUserIntent {
 public:
  // ctor for use with json and factory. provide a list of ways to compare to an
  // intent. if any are true, this condition will be true. See method for
  // details
  explicit ConditionUserIntentPending(const Json::Value& config);

  // special constructor for which you MUST immediately use an Add* method from
  // the base class
