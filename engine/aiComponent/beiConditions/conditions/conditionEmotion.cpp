...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionEmotion::AreConditionsMetInternal(
...
void ConditionEmotion::LoadJson(const Json::Value& config) {
  if (!config[kMaxKey].isNull() || !config[kMinKey].isNull()) {
    DEV_ASSERT(config[kValueKey].isNull(),
               "Can't specify both value and min/max");
    _minScore =
        config.get(kMinKey, std::numeric_limits<float>::lowest()).asFloat();
    _maxScore =
        config.get(kMaxKey, std::numeric_limits<float>::max()).asFloat();
    _useRange = true;
  } else {
  ...
  std::string emotionStr =
      JsonTools::ParseString(config, kEmotionKey, kDebugKey);
  ANKI_VERIFY(EmotionTypeFromString(emotionStr, _emotion),
              "ConditionEmotion.Ctor.Invalid", "Unknown emotion type %s",
              emotionStr.c_str());
}
