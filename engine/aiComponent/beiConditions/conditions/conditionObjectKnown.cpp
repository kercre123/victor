...
ConditionObjectKnown::ConditionObjectKnown(const Json::Value& config)
  ...
  if (ANKI_VERIFY(typeArray.isArray(),
    ...
    ANKI_VERIFY(!typeArray.empty(), "ConditionObjectKnown.Ctor.EmptyTypeArray",
                "Empty type array");
    for (const auto& type : typeArray) {
      ...
      ANKI_VERIFY(ObjectTypeFromString(objectTypeStr, targetType),
                  ...
                  "Object type '%s' is not valid", objectTypeStr.c_str());
...
// - - - - - - - - - - - - - - - - - - - -
ConditionObjectKnown::ConditionObjectKnown(const Json::Value& config,
                                           TimeStamp_t maxAge_ms)
    : ConditionObjectKnown(config) {
  ANKI_VERIFY(!_setMaxAge, "ConditionObjectKnown.Ctor.MaxAgeSet",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ConditionObjectKnown::SetActiveInternal(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ConditionObjectKnown::AreConditionsMetInternal(
  ...
  behaviorExternalInterface.GetBlockWorld().FindLocatedMatchingObjects(filter,
                                                                       matches);
  ...
  for (const auto& match : matches) {
    ...
    if (thisTickOnly) {
      // this is different than filter.OnlyConsiderLatestUpdate because that
      // checks against the last marker sighting time. If we were to add our own
      // filter lambda to compare the object timestamp against the current
      // timestamp, that still won't tell us if it was sighted this tick. so we
      // rely on _lastObjects. the object should either be new (not in
      // _lastObjects) or the timestamp should be newer than in _lastObjects.
      // (_lastObjects can't be a map because of ObjectID)
      auto it = std::find_if(
          _lastObjects.begin(), _lastObjects.end(),
          [&matchID](const auto& obj) { return obj.objectID == matchID; });
      // always add this one so there's a timestamp to compare against, but only
      // mark it as matchedThisTickOnly if the object is new or newer
      newInfo.emplace_back(matchTime, matchID);
      const bool newCube =
          (it == _lastObjects.end()) && (matchTime == currTimeStamp);
      const bool updatedCube =
          (it != _lastObjects.end()) && (it->observedTime < matchTime);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
const std::vector<const ObservableObject*> ConditionObjectKnown::GetObjects(
  ...
  for (const auto& observed : _lastObjects) {
    ...
    const auto* object =
        bei.GetBlockWorld().GetLocatedObjectByID(observed.objectID);
    if (object != nullptr) {
