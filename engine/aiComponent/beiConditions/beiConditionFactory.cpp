...
std::map<std::string, IBEIConditionPtr>
    BEIConditionFactory::_customConditionMap;
...
CustomBEIConditionHandleInternal::CustomBEIConditionHandleInternal(
  ...
  DEV_ASSERT(!_conditionName.empty(),
             "CustomBEIConditionHandle.NoConditionName");
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
CustomBEIConditionHandle BEIConditionFactory::InjectCustomBEICondition(
    const std::string& name, IBEIConditionPtr condition) {
  DEV_ASSERT_MSG(_customConditionMap.find(name) == _customConditionMap.end(),
                 ...
                 "already have a condition with name '%s'", name.c_str());
  ...
  if (kDebugConditionFactory) {
    PRINT_CH_DEBUG("Behaviors", "BEIConditionFactory.InjectCustomBEICondition",
                   "Added custom condition '%s'", name.c_str());
  }
  {
...
bool BEIConditionFactory::IsValidCondition(const Json::Value& config) {
  ...
  if (!config[IBEICondition::kConditionTypeKey].isNull()) {
    const std::string& typeStr =
        config[IBEICondition::kConditionTypeKey].asString();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
IBEIConditionPtr BEIConditionFactory::GetCustomCondition(
    const Json::Value& config, const std::string& ownerDebugLabel) {
  DEV_ASSERT(config[IBEICondition::kConditionTypeKey].isNull(),
             "BEIConditionFactory.SpecifiedCustomConditionAndType");

  auto it = _customConditionMap.find(config[kCustomConditionKey].asString());
  if (ANKI_VERIFY(it != _customConditionMap.end(),
                  "BEIConditionFactory.GetCustomCondition.NotFound",
                  "No custom condition with name '%s' found. Have %zu custom "
                  "conditions",
                  config[kCustomConditionKey].asString().c_str(),
                  _customConditionMap.size())) {
    // replace the owner debug label, even if it exists, since it was likely
    // created before knowing what behavior or condition would end up grabbing
    // it. Obviously multiple behaviors or conditions could grab it, but we can
    // deal with that if the use of the owner debug label depends on it
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
IBEIConditionPtr BEIConditionFactory::CreateBEICondition(
    const Json::Value& config, const std::string& ownerDebugLabel) {
  if (!config[kCustomConditionKey].isNull()) {
  ...
  switch (conditionType) {
    ...
    case BEIConditionType::Lambda: {
      DEV_ASSERT(
          false,
          "BEIConditionFactory.CreateBeiCondition.CantCreateLambdaFromConfig");
      break;
    }
...
// - - - - - - - - - - - - - - - - - - - -
IBEIConditionPtr BEIConditionFactory::CreateBEICondition(
    BEIConditionType type, const std::string& ownerDebugLabel) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool BEIConditionFactory::CheckConditionsAreUsed(
    const CustomBEIConditionHandleList& handles, const std::string& debugStr) {
  bool ret = true;
  ...
  for (const auto& handle : handles) {
    ...
    if (it == _customConditionMap.end()) {
      ret = false;
      PRINT_NAMED_ERROR(
          "BEIConditionFactory.AreConditionsUsed.HandleNotContained",
    ...
    if (numUses <= 1) {
      PRINT_NAMED_WARNING(
          "BEIConditionFactory.AreConditionsUsed.NotUsed",
          "%s: BEI condition '%s' only has a use count of %lu, may not have "
          "been used",
          debugStr.c_str(),
