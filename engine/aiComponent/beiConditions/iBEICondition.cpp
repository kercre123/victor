...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
BEIConditionType IBEICondition::ExtractConditionType(
    const Json::Value& config) {
  std::string strategyType = JsonTools::ParseString(
      config, kConditionTypeKey,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void IBEICondition::SetActive(BehaviorExternalInterface& bei,
                              bool setToActive) {
  ...
  if (!visionModeRequests.empty()) {
    if (setToActive) {
      bei.GetVisionScheduleMediator().SetVisionModeSubscriptions(
          this, visionModeRequests);
    } else {
  ...
  if (ANKI_DEV_CHEATS && !_isActive) {
    // set this _conditionType instance to inactive so it gets removed from
    // debug output
...
void IBEICondition::Init(BehaviorExternalInterface& bei) {
  if (_isInitialized) {
    // TODO:(bn) this will currently get called multiple times for custom
    // conditions if they are used as operands in multiple compound conditions.
    // I don't think there's a great way around this at the moment...
    if (_conditionType != BEIConditionType::Lambda) {
      PRINT_NAMED_WARNING("IBEICondition.Init.AlreadyInitialized",
  ...
  DEV_ASSERT(!GetOwnerDebugLabel().empty(),
             "IBEICondition.Init.MissingOwnerLabel");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool IBEICondition::AreConditionsMet(
  ...
  // If a condition has a child condition, then setting this static flag means
  // it won't check, send, or reset debug factors. that way, the parent can have
  // full control over when debug info is sent. this is a bit hacky, but it's
  // effect is in ANKI_DEV_CHEATS only. setting the flag to false doesn't really
  // need to be wrapped in a macro, but it makes everything a no-op when shipped
  ...
  if (ANKI_DEV_CHEATS) {
    if (checkDebugFactors) {
      ...
      const auto* webService = behaviorExternalInterface.GetRobotInfo()
                                   .GetContext()
                                   ->GetWebService();
      if (webService != nullptr &&
        ...
        // check valueChanged too in case the sole debug factor is _lastMetValue
        // == false.
        ...
        factors.Reset();
        _checkDebugFactors =
            true;  // other root-level conditions will be evaluated again
      }
    }
  }
  _lastMetValue = met;
...
BEIConditionDebugFactors& IBEICondition::GetDebugFactors() {
  return const_cast<BEIConditionDebugFactors&>(
      static_cast<const IBEICondition*>(this)->GetDebugFactors());
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void IBEICondition::SendConditionsToWebViz(
