...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
IBehavior::Status BehaviorDriveOffCharger::UpdateInternal(BehaviorExternalInterface& behaviorExternalInterface)
{
  // DEPRECATED - Grabbing robot to support current cozmo code, but this should
  // be removed
  const Robot& robot = behaviorExternalInterface.GetRobot();
...
void BehaviorDriveOffCharger::TransitionToDrivingForward(BehaviorExternalInterface& behaviorExternalInterface)
{
  ...
  if( robot.IsOnChargerPlatform() )
  {
    ...
    StartActing(action,[this, &behaviorExternalInterface](ActionResult res){
