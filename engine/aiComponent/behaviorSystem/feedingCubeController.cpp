...
#include "engine/aiComponent/behaviorSystem/feedingCubeController.h"
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
FeedingCubeController::FeedingCubeController(BehaviorExternalInterface& behaviorExternalInterface, const ObjectID& objectControlling)
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void FeedingCubeController::SetControllerState(BehaviorExternalInterface& behaviorExternalInterface,
                                               ControllerState newState, float eatingDuration_s)
{
  ...
  switch(newState){
    ...
    {
      StartListeningForShake(behaviorExternalInterface);
      InitializeController(behaviorExternalInterface);
      break;
    }
    case ControllerState::Deactivated:
    {
      ClearController(behaviorExternalInterface);
      break;
    }
    case ControllerState::DrainCube:
    {
      StartCubeDrain(behaviorExternalInterface, eatingDuration_s);
      break;
    }
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void FeedingCubeController::InitializeController(BehaviorExternalInterface& behaviorExternalInterface)
{
  _cubeStateTracker->_currentAnimationTrigger = CubeAnimationTrigger::Count;
  ReInitializeController(behaviorExternalInterface);
}

  
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void FeedingCubeController::ReInitializeController(BehaviorExternalInterface& behaviorExternalInterface)
{
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void FeedingCubeController::ClearController(BehaviorExternalInterface& behaviorExternalInterface)
{
  // Re-set lights
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void FeedingCubeController::Update(BehaviorExternalInterface& behaviorExternalInterface)
{
  switch(_currentState){
    case ControllerState::Activated:
    {
      CheckForChargeStateChanges(behaviorExternalInterface);
      break;
    }
    case ControllerState::DrainCube:
    {
      if(FLT_GT(_cubeStateTracker->_timeStartedDraining_s, 0)){
        UpdateCubeDrain(behaviorExternalInterface);
      }
      break;
    }
    case ControllerState::Deactivated:
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void FeedingCubeController::CheckForChargeStateChanges(BehaviorExternalInterface& behaviorExternalInterface)
{
  if(_cubeStateTracker->GetChargeState() != ChargeState::FullyCharged){
    ...
    if(shouldLoseCharge){
      ...
      if(_cubeStateTracker->_currentChargeLevel == 0){
        UpdateChargeAudioRound(behaviorExternalInterface, ChargeStateChange::Charge_Stop);
      }else{
        // Currently audio has 30 steps while lights only have 16 - so increments
        // and decrements come in pairs
        UpdateChargeAudioRound(behaviorExternalInterface, ChargeStateChange::Charge_Down);
        UpdateChargeAudioRound(behaviorExternalInterface, ChargeStateChange::Charge_Down);
      }
    }
    ...
    if(didLoseAllCharge){
      ...
      if(_cubeStateTracker->_currentChargeLevel == 4*kChargeLevelToFillSide){
        UpdateChargeAudioRound(behaviorExternalInterface, ChargeStateChange::Charge_Stop);
        UpdateChargeAudioRound(behaviorExternalInterface, ChargeStateChange::Charge_Complete);
      }
      
      UpdateChargeLights(behaviorExternalInterface);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void FeedingCubeController::StartCubeDrain(BehaviorExternalInterface& behaviorExternalInterface, float eatingDuration_s)
{
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void FeedingCubeController::UpdateCubeDrain(BehaviorExternalInterface& behaviorExternalInterface)
{
  const float currentTime_s = BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void FeedingCubeController::UpdateChargeLights(BehaviorExternalInterface& behaviorExternalInterface)
{
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void FeedingCubeController::StartListeningForShake(BehaviorExternalInterface& behaviorExternalInterface)
{
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void FeedingCubeController::ShakeDetected(BehaviorExternalInterface& behaviorExternalInterface, const float shakeScore)
{
  const float currentTime_s = BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  ...
  if(cubeCanCharge &&
    ...
    if(_cubeStateTracker->_currentChargeLevel > 1){
      ...
      // be a false positive
      UpdateChargeAudioRound(behaviorExternalInterface, ChargeStateChange::Charge_Up);
      UpdateChargeAudioRound(behaviorExternalInterface, ChargeStateChange::Charge_Up);
      ...
      UpdateChargeAudioRound(behaviorExternalInterface, ChargeStateChange::Charge_Start);
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void FeedingCubeController::UpdateChargeAudioRound(BehaviorExternalInterface& behaviorExternalInterface, ChargeStateChange changeEnumVal)
{
  PRINT_CH_DEBUG("Feeding",
