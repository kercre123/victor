...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
BehaviorAudioClient::BehaviorAudioClient(BehaviorExternalInterface& behaviorExternalInterface,
                                         Audio::RobotAudioClient* robotAudioClient)
...
bool BehaviorAudioClient::UpdateBehaviorRound(const UnlockId behaviorUnlockId, const int round)
{
  ...
  if (_isActive && roundState != Gameplay_Round::Invalid) {
    _robotAudioClient->PostSwitchState(SwitchGroupType::Gameplay_Round,
...
void BehaviorAudioClient::UpdateActivityMusicState(ActivityID activityID)
{
  ...
  if ( it != freeplayStateMap.end() ) {
    ...
    _robotAudioClient->PostSwitchState(SwitchGroupType::Freeplay_Mood,
                                               ...
                                               const int round)
{
  ...
  if (sparkedState != Sparked::Invalid) {
    _robotAudioClient->PostSwitchState(SwitchGroupType::Sparked,
  ...
  if (musicState != AudioMetaData::GameState::Music::Invalid) {
    _robotAudioClient->PostMusicState(static_cast<AudioMetaData::GameState::GenericState>(musicState));
  }
  return true;
}
...
void BehaviorAudioClient::HandleWorldEventUpdates(const RobotPublicState& stateEvent)
{
  ...
  if(_isCubeInLift != stateEvent.isCubeInLift){
    ...
    if(_isCubeInLift){
      _robotAudioClient->PostEvent(AE::Play__Cue_World_Event__Lift_Cube, kMusicGameObject);
    }else{
      _robotAudioClient->PostEvent(AE::Stop__Cue_World_Event__Lift_Cube, kMusicGameObject);
    }
  }
  ...
  if(_isRequestingGame != stateEvent.isRequestingGame){
    ...
    if(_isRequestingGame){
      _robotAudioClient->PostEvent(AE::Play__Cue_World_Event__Request_Game, kMusicGameObject);
    }else{
      _robotAudioClient->PostEvent(AE::Stop__Cue_World_Event__Request_Game, kMusicGameObject);
    }
  }
  ...
  if(_stackExists != (stateEvent.tallestStackHeight > 1)){
    ...
    if(_stackExists){
      _robotAudioClient->PostEvent(AE::Play__Cue_World_Event__Cubes_Stacked, kMusicGameObject);
    }else{
...
void BehaviorAudioClient::HandleNeedsUpdates(const NeedsLevels& needsLevel)
{
  ...
  if(!FLT_NEAR(needsLevel.energy, _needsLevel.energy)){
    ...
    _robotAudioClient->PostParameter(AudioMetaData::GameParameter::ParameterType::Nurture_Energy,
                                     _needsLevel.energy,
                                     AGO::Invalid);
  }
  ...
  if(!FLT_NEAR(needsLevel.repair, _needsLevel.repair)){
    ...
    _robotAudioClient->PostParameter(AudioMetaData::GameParameter::ParameterType::Nurture_Repair,
                                     _needsLevel.repair,
                                     AGO::Invalid);
  }
  ...
  if(!FLT_NEAR(needsLevel.play, _needsLevel.play)){
    ...
    _robotAudioClient->PostParameter(AudioMetaData::GameParameter::ParameterType::Nurture_Play,
                                     _needsLevel.play,
                                     AGO::Invalid);
  }
