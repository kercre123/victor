...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool IActivityStrategy::WantsToStart(BehaviorExternalInterface& behaviorExternalInterface,
                                     ...
                                     float lastTimeActivityStartedSec) const
{
  ...
  if ( FLT_GT(_cooldownSecs, 0.0f) && (FLT_GT(lastTimeActivityRanSec, 0.0f) || _startInCooldown) )
  {
    ...
    RandomizeCooldown(behaviorExternalInterface);
  }
  ...
  if ( FLT_GT(_requiredRecentOnTreadsEvent_secs, 0.0f) )
  {
    ...
    const float lastEventSecs = behaviorExternalInterface.GetAIComponent().GetWhiteboard().GetTimeAtWhichRobotReturnedToTreadsSecs();
  ...
  const bool ret = WantsToStartInternal(behaviorExternalInterface, lastTimeActivityRanSec);
  return ret;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool IActivityStrategy::WantsToEnd(BehaviorExternalInterface& behaviorExternalInterface, float lastTimeActivityStartedSec) const
{
  ...
  const bool ret = WantsToEndInternal(behaviorExternalInterface, lastTimeActivityStartedSec);
  return ret;
}

void IActivityStrategy::RandomizeCooldown(BehaviorExternalInterface& behaviorExternalInterface) const
{
  _cooldownSecs = _baseCooldownSecs + behaviorExternalInterface.GetRNG().RandDbl(_cooldownRandomnessSecs);
}
