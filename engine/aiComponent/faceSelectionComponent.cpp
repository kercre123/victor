...
const FaceSelectionComponent::FaceSelectionFactorMap
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool FaceSelectionComponent::ParseFaceSelectionFactorMap(
    const Json::Value& config, FaceSelectionFactorMap& output) {
  // use a local map to avoid corrupting the output if we fail to parse later on
  FaceSelectionFactorMap map;

  for (const auto& entry : config) {
    if (!entry[kPenaltyFactorKey].isString() ||
        !entry[kPenaltyMultiplierKey].isNumeric()) {
      return false;
    }

    map.emplace(FaceSelectionPenaltyMultiplierFromString(
                    entry[kPenaltyFactorKey].asString()),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
SmartFaceID FaceSelectionComponent::GetBestFaceToUse(
    const FaceSelectionFactorMap& criteriaMap) const {
...
// - - - - - - - - - - - - - - - - - - - -
SmartFaceID FaceSelectionComponent::GetBestFaceToUse(
    const FaceSelectionFactorMap& criteriaMap,
  ...
  using FullCalcScoreArray =
      Util::FullEnumToValueArrayChecker::FullEnumToValueArray<
          FaceSelectionPenaltyMultiplier, calcScoreFunction,
  ...
  static const FullCalcScoreArray kFullScoreCalcMap = {
      {FaceSelectionPenaltyMultiplier::RelativeBodyAngleRadians,
       std::bind(&FaceSelectionComponent::CalculateBodyAngleCost, this,
                 std::placeholders::_1)},
      {FaceSelectionPenaltyMultiplier::RelativeHeadAngleRadians,
       std::bind(&FaceSelectionComponent::CalculateHeadAngleCost, this,
                 std::placeholders::_1)},
      {FaceSelectionPenaltyMultiplier::RelativeMicDirectionRadians,
       std::bind(&FaceSelectionComponent::CalculateMicDirectionCost, this,
                 std::placeholders::_1)},
      {FaceSelectionPenaltyMultiplier::UnnamedFace,
       std::bind(&FaceSelectionComponent::CalculateUnnamedCost, this,
                 std::placeholders::_1)},
      {FaceSelectionPenaltyMultiplier::TrackingOnly,
       std::bind(&FaceSelectionComponent::CalculateTrackingOnlyCost, this,
                 std::placeholders::_1)},
      {FaceSelectionPenaltyMultiplier::TimeSinceSeen_s,
       std::bind(&FaceSelectionComponent::CalculateTimeSinceSeenCost, this,
                 std::placeholders::_1)},
      {FaceSelectionPenaltyMultiplier::NotMakingEyeContact,
       std::bind(&FaceSelectionComponent::CalculateNotMakingEyeContact, this,
                 std::placeholders::_1)},
      {FaceSelectionPenaltyMultiplier::Distance_m,
       std::bind(&FaceSelectionComponent::CalculateDistanceCost, this,
                 std::placeholders::_1)}

  };
  DEV_ASSERT(
      IsSequentialArray(kFullScoreCalcMap),
      "FaceSelectionComponent.GetBestFaceToUse.ScoreFuncsNotSequenttial");

  // add up score for all criteria

  // NOTE: it is possible that multiple entries in this vector are actually the
  // same face (even if they weren't when inserted). This is OK
  ...
  for (auto& currentID : possibleFaces) {
    ...
    for (const auto& criteriaPair : criteriaMap) {
      const auto& scoringFunc =
          kFullScoreCalcMap[Util::EnumToUnderlying(criteriaPair.first)].Value();
      ...
      if (kFaceSelectionDebugging) {
        PRINT_CH_INFO(
            "Behaviors", "FaceSelectionComponent.Scoring",
            "%s: %s = %f * factor %f", currentID.GetDebugStr().c_str(),
            FaceSelectionPenaltyMultiplierToString(criteriaPair.first),
            criteriaScore, criteriaFactor);
      }
    }

    if (kFaceSelectionDebugging) {
      PRINT_CH_INFO("Behaviors", "FaceSelectionComponent.Scoring",
                    "%s: total = %f", currentID.GetDebugStr().c_str(),
                    currentScore);
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool FaceSelectionComponent::AreFacesInFrontOfRobot(
    std::vector<SmartFaceID>& faceIDs, RobotTimeStamp_t seenSinceTime_ms,
    bool includeRecognizableOnly) const {
  // To check if faces are within Robot's range of view, pass in half the
  // camera's FOV as the acceptabel face range
  auto calibration = _visionComp.GetCamera().GetCalibration();
  if (calibration != nullptr) {
    const Radians fov = calibration->ComputeHorizontalFOV();
    faceIDs = _faceWorld.GetSmartFaceIDs(
        seenSinceTime_ms, includeRecognizableOnly, fov.ToFloat() / 2);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
float FaceSelectionComponent::CalculateHeadAngleCost(
    const Vision::TrackedFace* currentFace) const {
  Pose3d poseWrtRobot;
  if (!currentFace->GetHeadPose().GetWithRespectTo(_robot.GetPose(),
                                                   poseWrtRobot)) {
    // no transform, probably a different origin
    return 0.0f;
  }

  Radians absHeadTurnAngle =
      TurnTowardsPoseAction::GetAbsoluteHeadAngleToLookAtPose(
          poseWrtRobot.GetTranslation());
  Radians relHeadTurnAngle =
      absHeadTurnAngle - _robot.GetComponent<FullRobotPose>().GetHeadAngle();
  return relHeadTurnAngle.getAbsoluteVal().ToFloat();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
float FaceSelectionComponent::CalculateBodyAngleCost(
    const Vision::TrackedFace* currentFace) const {
  Pose3d poseWrtRobot;
  if (!currentFace->GetHeadPose().GetWithRespectTo(_robot.GetPose(),
                                                   poseWrtRobot)) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
float FaceSelectionComponent::CalculateMicDirectionCost(
    const Vision::TrackedFace* currentFace) const {
  // Recent direction is provided in terms of an index 0 - 11 coresponding to
  // the face of a clock these calculations assume that we've done the work to
  // normalize recent direction to already be relative to robot
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
float FaceSelectionComponent::CalculateUnnamedCost(
    const Vision::TrackedFace* currentFace) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
float FaceSelectionComponent::CalculateTrackingOnlyCost(
    const Vision::TrackedFace* currentFace) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
float FaceSelectionComponent::CalculateTimeSinceSeenCost(
    const Vision::TrackedFace* currentFace) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
float FaceSelectionComponent::CalculateNotMakingEyeContact(
    const Vision::TrackedFace* currentFace) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
float FaceSelectionComponent::CalculateDistanceCost(
    const Vision::TrackedFace* currentFace) const {
  float distanceBetween_mm = 0.0f;

  if (!ComputeDistanceBetween(currentFace->GetHeadPose(), _robot.GetPose(),
                              distanceBetween_mm)) {
    // no transform, probably a different origin
    return 0.0f;
  }
