...
ContinuityComponent::ContinuityComponent(Robot& robot)
    : IDependencyManagedComponent<AIComponentID>(
          this, AIComponentID::ContinuityComponent),
      _robot(robot),
...
void ContinuityComponent::InitDependent(Robot* robot,
                                        const AICompMap& dependentComps) {
  if (robot->HasExternalInterface()) {
    auto onCompletedAction =
...
void ContinuityComponent::UpdateDependent(const AICompMap& dependentComps) {
  if (!_playingGetOut && (_nextActionToQueue != nullptr)) {
...
bool ContinuityComponent::GetIntoAction(IActionRunner* action) {
  if (_playingGetOut) {
    if (_nextActionToQueue != nullptr) {
      PRINT_NAMED_WARNING("ContinuityComponent.GetIntoAction.ReplacingAction",
                          "Replacing delegated action");
...
void ContinuityComponent::PlayEmergencyGetOut(AnimationTrigger anim) {
  if (_playingGetOut) {
    PRINT_NAMED_WARNING(
        "ContinuityComponent.PlayEmergencyGetOut.MultipleGetOuts",
        "Continuity component is trying to play multiple emergency getouts "
        "(%s)",
        AnimationTriggerToString(anim));
  }

  // Prevent emergency getouts from playing when we are displaying info screens
  // such as pairing or CC screens as the getout animations can draw over the
  // info screens
  if (_displayingInfoFace) {
    PRINT_NAMED_INFO(
        "ContinuityComponent.PlayEmergencyGetOut.DisplayingInfoFace",
        "Not playing emergency get out %s due to info face being displayed",
        EnumToString(anim));
    return;
  }

  BehaviorComponent& bComp =
      _robot.GetAIComponent().GetComponent<BehaviorComponent>();
  if (bComp.GetComponent<UserIntentComponent>()
          .WaitingForTriggerWordGetInToFinish()) {
    PRINT_NAMED_INFO(
        "ContinuityComponent.PlayEmergencyGetOut."
        "WaitingForTriggerWordGetInToFinish",
        "Not playing emergency get out %s due to trigger word get in anim "
        "playing",
        EnumToString(anim));
    return;
  }
...
bool ContinuityComponent::QueueAction(IActionRunner* action) {
  ...
  if (!success) {
    PRINT_NAMED_WARNING(
        "ContinuityComponent.GetIntoAction.Failure.ActionNotSet",
        "Unable to queue action '%s' (error %d)", action->GetName().c_str(),
        result);
    delete action;
  }
  return success;
