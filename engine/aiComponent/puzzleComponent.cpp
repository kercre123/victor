...
template <typename T>
bool GetVectorofVectorsOptional(const Json::Value& config,
                                const std::string& key,
                                std::vector<std::vector<T>>& values) {
  const Json::Value& child(config[key]);
  if (child.isNull() || !child.isArray()) return false;
  values.reserve(child.size());
  for (auto const& element : child) {
    if (element.isNull() || !element.isArray()) return false;
...
Result MazeConfig::InitConfiguration(const Json::Value& config) {
  ...
  DEV_ASSERT(std::all_of(_maze.begin(), _maze.end(),
                         [mazeWidth](const auto& vec) {
                           return vec.size() == mazeWidth;
                         }),
...
size_t MazeConfig::GetHeight() const { return _maze.size(); }
...
Result PuzzleComponent::InitConfigs() {
  ...
  const std::string mazeFolder =
      _robot.GetContextDataPlatform()->pathToResource(
          Util::Data::Scope::Resources, puzzlePath + "/maze");
  // TODO: refactor loading to robotDataLoader once we figure out how we're
  // actually getting puzzles. Pre-existings vs stored on cards. And if we're
  // storing based on state.
  auto filePaths =
      Util::FileUtils::FilesInDirectory(mazeFolder, true, fileExts, true);
  for (const auto& path : filePaths) {
    Json::Value jsonRoot;
    const bool success =
        _robot.GetContextDataPlatform()->readAsJson(path, jsonRoot);
    if (success) {
      std::unique_ptr<MazeConfig> mazeConfig =
          std::make_unique<MazeConfig>(path);
...
const MazeConfig& PuzzleComponent::GetCurrentMaze() const {
  ...
  DEV_ASSERT(_currMaze != _mazes.end(),
             "PuzzleComponent.GetCurrentMaze.NoMaze");
...
size_t PuzzleComponent::GetNumMazes() const { return _mazes.size(); }
