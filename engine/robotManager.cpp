  ...
  if (ANKI_DEBUG_LEVEL >= ANKI_DEBUG_ERRORS_AND_WARNS) {
    ...
    if (timeSpent_millis > maxInitTime_millis) {
      LOG_WARNING("RobotManager.Init.TimeSpent",
                  ...
                  timeSpent_millis, maxInitTime_millis);
    }
  }
...
void RobotManager::Shutdown(ShutdownReason reason) {
  // Order of destruction matters! Robot actions call back into robot manager,
  // so they must be released before robot manager itself.
  LOG_INFO("RobotManager.Shutdown", "Shutting down");

  if (_robot != nullptr) {
    const auto battFilt_mV = static_cast<int>(
        1000 * _robot->GetBatteryComponent().GetBatteryVolts());
    const auto battRaw_mV = static_cast<int>(
        1000 * _robot->GetBatteryComponent().GetBatteryVoltsRaw());
    ...
    if (reason != ShutdownReason::SHUTDOWN_UNKNOWN) {
      ...
      DASMSG(robot_power_off, "robot.power_off",
             "Reason why robot powered off during the previous run");
...
#define SHUTDOWN_CASE(x)         \
  case ShutdownReason::x:        \
    ...
    break;
      ...
      switch (reason) {
        ...
        default:
          LOG_ERROR("Robot.Shutdown.UnknownFaultCode", "reason: %s",
                    EnumToString(reason));
          break;
      }
#undef SHUTDOWN_CASE
...
void RobotManager::AddRobot(const RobotID_t withID) {
  if (_robot == nullptr) {
    ...
    LOG_WARNING("RobotManager.AddRobot.AlreadyAdded",
                "Robot already exists. Must remove first.");
  }
}

void RobotManager::RemoveRobot(bool robotRejectedConnection) {
  if (_robot != nullptr) {
    LOG_INFO("RobotManager.RemoveRobot.Removing", "Removing robot with ID=%d",
             _robot->GetID());

    if (_initialConnection) {
      const auto result = robotRejectedConnection
                              ? RobotConnectionResult::ConnectionRejected
                              : RobotConnectionResult::ConnectionFailure;
...
Robot* RobotManager::GetRobot() { return _robot.get(); }
...
Result RobotManager::UpdateRobot() {
  ...
  if (_robot) {
    ...
    if (_robot->HasReceivedRobotState()) {
      _context->GetExternalInterface()->Broadcast(
          ExternalInterface::MessageEngineToGame(_robot->GetRobotState()));
      _context->GetGatewayInterface()->Broadcast(
          ExternalMessageRouter::Wrap(_robot->GenerateRobotStateProto()));
    } else {
...
bool RobotManager::ShouldFilterMessage(
    const RobotInterface::RobotToEngineTag msgType) const {
  if (_initialConnection) {
...
bool RobotManager::ShouldFilterMessage(
    const RobotInterface::EngineToRobotTag msgType) const {
  if (_initialConnection) {
    return _initialConnection->ShouldFilterMessage(msgType);
  }
  return false;
