...
class IExternalInterface {
 public:
  ...
  virtual void Broadcast(
      const ExternalInterface::MessageGameToEngine& message) = 0;
  virtual void Broadcast(ExternalInterface::MessageGameToEngine&& message) = 0;
  virtual void BroadcastDeferred(
      const ExternalInterface::MessageGameToEngine& message) = 0;
  virtual void BroadcastDeferred(
      ExternalInterface::MessageGameToEngine&& message) = 0;

  virtual void Broadcast(
      const ExternalInterface::MessageEngineToGame& message) = 0;
  virtual void Broadcast(ExternalInterface::MessageEngineToGame&& message) = 0;
  virtual void BroadcastDeferred(
      const ExternalInterface::MessageEngineToGame& message) = 0;
  virtual void BroadcastDeferred(
      ExternalInterface::MessageEngineToGame&& message) = 0;

  template <typename T, typename... Args>
  void BroadcastToGame(Args&&... args) {
    Broadcast(
        ExternalInterface::MessageEngineToGame(T(std::forward<Args>(args)...)));
  }

  template <typename T, typename... Args>
  void BroadcastToEngine(Args&&... args) {
    Broadcast(
        ExternalInterface::MessageGameToEngine(T(std::forward<Args>(args)...)));
  }
...
class SimpleExternalInterface : public IExternalInterface {
 protected:
  void DeliverToGame(const ExternalInterface::MessageEngineToGame& message,
                     DestinationId desinationId) override;
};
