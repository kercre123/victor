 ...
 * Created: may 31 2018
 *
 * Description: Templates to automatically wrap messages included in the
 * MessageEngineToGame union and the GatewayWrapper protobuf oneof (union) based
 * on external requirements and event organization (the hierarchy in clad and
 * proto files)
...
class ExternalMessageRouter {
 public:
  ...
  // If your message is a response to a request, call WrapResponse() on your
  // allocated message pointer. If your message is NOT a response to a request,
  // call Wrap() on your allocated message pointer Eventually, gateway changes
  // should render this distinction obsolete
  // -------------------------------------------------------------------------------------------------
  ...
  template <typename T>
  inline static external_interface::GatewayWrapper WrapResponse(
      T* message, uint64_t connId = 0) {
  ...
  template <typename T>
  inline static ReturnIf<CanConstruct<Status, T>::value, GatewayWrapper> Wrap(
      T* message) {
...
// macro to generate Wrap() methods based on Event types listed in the proto
// file
#define MAKE_MESSAGE_WRAPPER(Type)                            \
  template <typename T>                                       \
  inline static ReturnIf<CanConstruct<Type, T>::value,        \
  ...
  template <typename T>
  using CanBeEngineToGame =
      Anki::Util::is_explicitly_constructible<MessageEngineToGame, T>;
  ...
  template <typename T>
  inline static ReturnIf<!CanBeCLADEvent<T>::value &&
                             CanBeEngineToGame<T>::value,
                         MessageEngineToGame>
