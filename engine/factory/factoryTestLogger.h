...
class FactoryTestLogger {
 public:
  ...
  // Optionally, specify if you want to append the current date and time to the
  // log name
  bool StartLog(const std::string& logName, bool appendDateTime = false,
                Util::Data::DataPlatform* dataPlatform = nullptr);
  void CloseLog();
  ...
  // DistanceSensorData is added to an json array of data called "name".
  // Separate data entries are labelled as "seq_*". Call with an existing name
  // to add to that array
  bool Append(const std::string& name, const DistanceSensorData& data);

  // RangeSensorData is added to an json array of data called "name". Separate
  // data entries are labelled as "seq_*". Call with an existing name to add to
  // that array
 ...
 private:
  std::string ChooseNextFileName(const std::string& dir,
                                 const std::string& name);

  bool AppendCliffSensorValue(const std::string& readingName,
                              const CliffSensorValue& data);
  bool AppendCliffSensorValues(const std::string& readingName,
  ...
  bool ArchiveAndDelete(const std::string& archiveName,
                        const std::string& logBaseDir);

  void ParseIMUTempDuration(const IMUTempDuration& data, Json::Value* json,
                            std::stringstream& ss);
