 ...
 * Created: 6/14/2016
 *
 * Description: Exports structs to factory test (i.e. Playpen test) formatted
 * log file
 *
...
FactoryTestLogger::FactoryTestLogger(bool exportJson)
    : _logDir(""), _logFileName(""), _exportJson(exportJson) {}

FactoryTestLogger::~FactoryTestLogger() { CloseLog(); }

std::string FactoryTestLogger::ChooseNextFileName(const std::string& dir,
  ...
  if (dirNames.empty()) {
    ...
    newNameStream << name << "_-_" << std::setfill('0')
                  << std::setw(kNumberDigitsLength) << 0;
  ...
  auto iterationNum =
      std::stoi(entryToReplace.substr(iterStrBegin, kNumberDigitsLength));

  if (iterationNum == kMaxIterationNum) {
    PRINT_NAMED_ERROR(
        "FactoryTestLogger.ChooseNextFileName",
  ...
  std::ostringstream newNameStream;
  newNameStream << name << "_-_" << std::setfill('0')
                << std::setw(kNumberDigitsLength) << ++iterationNum;
  return newNameStream.str();
}

bool FactoryTestLogger::StartLog(const std::string& logName,
                                 bool appendDateTime,
                                 Util::Data::DataPlatform* dataPlatform) {
  // Generate new log dir name
  std::string newLogDir = "";
  if (dataPlatform) {
    newLogDir = Util::FileUtils::FullFilePath(
        {dataPlatform->pathToResource(_kLogScope, _kLogRootDirName)});
    newLogDir += "/" +
                 ChooseNextFileName(
                     dataPlatform->pathToResource(_kLogScope, _kLogRootDirName),
                     logName) +
  ...
  if (Util::FileUtils::DirectoryExists(newLogDir)) {
    if (_logDir == newLogDir) {
      PRINT_NAMED_WARNING("FactoryTestLogger.StartLog.DirIsCurrentLog",
                          "Aborting current log %s because why are you trying "
                          "to start it again?",
                          newLogDir.c_str());
      CloseLog();
    } else {
      PRINT_NAMED_WARNING("FactoryTestLogger.StartLog.DirExists",
                          "Ignoring log %s because it already exists",
                          newLogDir.c_str());
    }
    return false;
  } else {
  ...
  _logDir = newLogDir;
  PRINT_NAMED_INFO("FactoryTestLogger.StartLog.CreatingLogDir", "%s",
                   _logDir.c_str());
  Util::FileUtils::CreateDirectory(_logDir);
  _logFileName = Util::FileUtils::FullFilePath(
      {_logDir, _kLogTextFileName + (_exportJson ? ".json" : ".txt")});
  ...
  PRINT_NAMED_INFO("FactoryTestLogger.StartLog.CreatingLogFile", "%s",
                   _logFileName.c_str());
...
void FactoryTestLogger::CloseLog() {
  if (_logFileHandle.is_open()) {
    PRINT_NAMED_INFO("FactoryTestLogger.CloseLog.Closing", "%s",
                     _logFileName.c_str());

    // If exporting json, write it to file here
    if (_exportJson) {
      // Use FastWriter to "compress" the json string (removes newlines, tabs,
      // etc)
    ...
    // Copy log to factory partition. This should overwrite any log already
    // there so it will always contain the most recent log
    PRINT_NAMED_INFO("FactoryTestLogger.CloseLog.Copying",
                     "Copying log from %s to %s", _logFileName.c_str(),
                     _kPathToCopyLogTo.c_str());
    Util::FileUtils::CopyFile(_kPathToCopyLogTo, _logFileName);

    // The log file has been copied to the correct directory but now needs to be
    // renamed
    const std::string oldFileName =
        _kPathToCopyLogTo + "/" +
        (_kLogTextFileName + (_exportJson ? ".json" : ".txt"));
    const std::string newFileName = _kPathToCopyLogTo + "/log0";
    int rc = std::rename(oldFileName.c_str(), newFileName.c_str());
...
bool FactoryTestLogger::IsOpen() { return _logFileHandle.is_open(); }
...
bool FactoryTestLogger::Append(const FactoryTestResultEntry& data) {
  ...
  PRINT_NAMED_INFO("FactoryTestLogger.Append.FactoryTestResultEntry", "%s",
                   ss.str().c_str());
...
bool FactoryTestLogger::Append(const CameraCalibration& data) {
  ...
  PRINT_NAMED_INFO("FactoryTestLogger.Append.CameraCalibration", "%s",
                   ss.str().c_str());
  return AppendToFile(ss.str());
}
...
bool FactoryTestLogger::Append(const BirthCertificate& data) {
  ...
  PRINT_NAMED_INFO("FactoryTestLogger.Append.BirthCertificate", "%s",
                   ss.str().c_str());
  return AppendToFile(ss.str());
}
...
bool FactoryTestLogger::Append(const CalibMetaInfo& data) {
  ...
  PRINT_NAMED_INFO("FactoryTestLogger.Append.CalibMetaInfo", "%s",
                   ss.str().c_str());
  return AppendToFile(ss.str());
}
...
bool FactoryTestLogger::Append(const IMUTempDuration& data) {
  ...
  PRINT_NAMED_INFO("FactoryTestLogger.Append.IMUTempDuration", "%s",
                   ss.str().c_str());
  return AppendToFile(ss.str());
}
...
bool FactoryTestLogger::AppendCliffValuesOnFrontDrop(
    const CliffSensorValues& data) {
  return AppendCliffSensorValues("CliffsOnFrontDrop", data);
}

bool FactoryTestLogger::AppendCliffValuesOnBackDrop(
    const CliffSensorValues& data) {
  return AppendCliffSensorValues("CliffsOnBackDrop", data);
}

bool FactoryTestLogger::AppendCliffValuesOnGround(
    const CliffSensorValues& data) {
  return AppendCliffSensorValues("CliffsOnGround", data);
}

bool FactoryTestLogger::AppendCliffSensorValue(const std::string& readingName,
                                               const CliffSensorValue& data) {
  std::stringstream ss;
  if (_exportJson) {
  ...
  PRINT_NAMED_INFO("FactoryTestLogger.Append.CliffSensorValue", "%s",
                   ss.str().c_str());
  return AppendToFile(ss.str());
}

bool FactoryTestLogger::AppendCliffSensorValues(const std::string& readingName,
                                                const CliffSensorValues& data) {
  std::stringstream ss;
  if (_exportJson) {
    Json::Value& node = _json[readingName];
  ...
  PRINT_NAMED_INFO("FactoryTestLogger.Append.CliffSensorValues", "%s",
                   ss.str().c_str());
  return AppendToFile(ss.str());
}
...
bool FactoryTestLogger::AppendPoseData(const std::string& poseName,
                                       const PoseData& data) {
  std::stringstream ss;
  if (_exportJson) {
    ...
    ss << "\n[" << poseName << "]" << std::fixed
       << "\nRot_deg: " << RAD_TO_DEG(data.angleX_rad) << " "
       << RAD_TO_DEG(data.angleY_rad) << " " << RAD_TO_DEG(data.angleZ_rad)
       << "\nTrans_mm: " << data.transX_mm << " " << data.transY_mm << " "
       << data.transZ_mm;
  }
...
bool FactoryTestLogger::Append(
    const ExternalInterface::RobotCompletedFactoryDotTest& msg) {
  std::stringstream ss;
  if (_exportJson) {
    if (msg.success) {
      ...
      if (msg.didComputePose) {
        PoseData pd(msg.camPoseRoll_rad, msg.camPosePitch_rad,
                    msg.camPoseYaw_rad, msg.camPoseX_mm, msg.camPoseY_mm,
                    msg.camPoseZ_mm);
  ...
  PRINT_NAMED_INFO("FactoryTestLogger.Append.CentroidInfo", "%s",
                   ss.str().c_str());
  return AppendToFile(ss.str());
}

bool FactoryTestLogger::Append(const std::string& name,
                               const DistanceSensorData& data) {
  std::stringstream ss;
  if (_exportJson) {
  ...
  PRINT_NAMED_INFO("FactoryTestLogger.Append.DistanceSensorData", "%s",
                   ss.str().c_str());
  return AppendToFile(ss.str());
}

bool FactoryTestLogger::Append(const std::string& name,
                               const RangeSensorData& data) {
  std::stringstream ss;
  if (_exportJson) {
    Json::Value& node = _json[name];
    Json::Value newNode;
...
bool FactoryTestLogger::Append(
    const std::map<std::string, std::vector<FactoryTestResultCode>>& results) {
  std::stringstream ss;
  ...
  PRINT_NAMED_INFO("FactoryTestLogger.Append.AllPlaypenResults", "%s",
                   ss.str().c_str());
  return AppendToFile(ss.str());
}

bool FactoryTestLogger::Append(const std::string& dataTypeName,
                               const TouchSensorValues& data) {
  std::stringstream ss;
  ...
  PRINT_NAMED_INFO("FactoryTestLogger.Append.TouchSensorValues", "%s",
                   ss.str().c_str());
  return AppendToFile(ss.str());
}

bool FactoryTestLogger::Append(const std::string& name,
                               const TouchSensorFilt& data) {
  std::stringstream ss;
...
bool FactoryTestLogger::AppendToFile(const std::string& data) {
  ...
  if (!_logFileHandle.is_open()) {
    PRINT_NAMED_WARNING("FactoryTestLogger.Append.LogNotStarted",
                        "Ignoring because log not started");
    _json.clear();
    return false;
  }
  if (!_exportJson) {
...
bool FactoryTestLogger::AddFile(const std::string& filename,
                                const std::vector<uint8_t>& data) {
  if (_logDir.empty()) {
    PRINT_NAMED_WARNING("FactoryTestLogger.AddFile.LogNotStarted",
                        "Ignoring because log not started");
    return false;
  }
  ...
  PRINT_NAMED_INFO("FactoryTestLogger.AddFile", "File: %s, size: %zu bytes",
...
bool FactoryTestLogger::CopyEngineLog(Util::Data::DataPlatform* dataPlatform) {
  if (_logDir.empty()) {
    PRINT_NAMED_WARNING("FactoryTestLogger.CopyEngineLog.LogNotStarted",
                        "Ignoring because log not started");
    return false;
  }
  ...
  // TODO (Al): Get LOGNAME (log folder) from cozmoeEngineMain.cpp instead of
  // duplicating it
  std::string srcDir = dataPlatform->pathToResource(
      Util::Data::Scope::CurrentGameLog, "vic-engine");
  ...
  if (dirs.size() > 1) {
    PRINT_NAMED_WARNING(
        "FactoryTestLogger.CopyEngineLog.MoreLogDirsThanExpected", "%zu",
        dirs.size());
  }

  srcDir = Util::FileUtils::FullFilePath({srcDir, dirs.front(), "print"});
  std::vector<std::string> engineLogFiles =
      Util::FileUtils::FilesInDirectory(srcDir, true, ".log", true);
  ...
  for (auto f : engineLogFiles) {
    if (!Util::FileUtils::CopyFile(_logDir, f, _kMaxEngineLogSizeBytes)) {
      PRINT_NAMED_WARNING("FactoryTestLogger.CopyEngineLog.Failed", "%s",
                          f.c_str());
      res = false;
    }
  }
...
uint32_t FactoryTestLogger::GetNumArchives(
    Util::Data::DataPlatform* dataPlatform) {
  // Get base directory of log directories
  std::string baseDirectory = _kArchiveRootDirName;
  if (dataPlatform) {
    baseDirectory =
        dataPlatform->pathToResource(_kLogScope, _kArchiveRootDirName);
  }
...
bool FactoryTestLogger::ArchiveAndDelete(const std::string& archiveName,
                                         const std::string& logBaseDir) {
  auto filePaths =
      Util::FileUtils::FilesInDirectory(logBaseDir, true, nullptr, true);
  ...
  PRINT_NAMED_WARNING("FactoryTestLogger.ArchiveAndDelete.Failed",
                      "ArchiveName: %s, LogBaseDir: %s", archiveName.c_str(),
                      logBaseDir.c_str());
  return false;
}
...
bool FactoryTestLogger::ArchiveLogs(Util::Data::DataPlatform* dataPlatform) {
  ...
  // Create archive
  if (!ArchiveAndDelete(archiveBaseDirectory + "/" + archiveName,
                        logBaseDirectory)) {
    return false;
  }
  ...
  logBaseDirectory =
      dataPlatform->pathToResource(Util::Data::Scope::Cache, _kLogRootDirName);
  if (!ArchiveAndDelete(archiveBaseDirectory + "/" + archiveName,
                        logBaseDirectory)) {
    return false;
  }
