...
class Block : public ActionableObject {
 public:
  Block(const ObjectType& type, const ActiveID& activeID = -1,
        const FactoryID& factoryID = "");

  virtual ~Block();

  virtual Block* CloneType() const override { return new Block(this->_type); }
...
#include "engine/BlockDefinitions.h"
  ...
  virtual void GetCorners(const Pose3d& atPose,
                          std::vector<Point3f>& corners) const override;
  ...
  virtual Quad2f GetBoundingQuadXY(const Pose3d& atPose,
                                   const f32 padding_mm = 0.f) const override;
  ...
  // NOTE: This prevents us from having multiple active objects in the world at
  // the same time: this means we
  //  match to existing objects based solely on type. If we ever do anything
  //  like COZMO-23 to get around that, then this would need to be changed.
  ...
  void SetLEDs(const WhichCubeLEDs whichLEDs, const ColorRGBA& onColor,
               const ColorRGBA& offColor, const u32 onPeriod_ms,
               const u32 offPeriod_ms, const u32 transitionOnPeriod_ms,
               const u32 transitionOffPeriod_ms, const s32 offset,
               const bool turnOffUnspecifiedLEDs);

  // Specify individual colors and flash frequencies for all the LEDS of the
  // block The index of the arrays matches the diagram above. NOTE: Alpha is
  // ignored
  ...
  // Get the LED specification for the top (and bottom) LEDs on the corner
  // closest to the specified (x,y) position, using the ActiveCube's current
  // pose.
  ...
  static WhichCubeLEDs RotateWhichLEDsAroundTopFace(WhichCubeLEDs whichLEDs,
                                                    bool clockwise);
  ...
  virtual bool IsMoving(RobotTimeStamp_t* t = nullptr) const override {
    if (t) *t = _movingTime;
    return _isMoving;
  }
  ...
  // Current tapCount, which is just an incrementing counter in the raw cube
  // messaging
  int GetTapCount() const { return _tapCount; }
  void SetTapCount(const int cnt) { _tapCount = cnt; }

 protected:
  virtual void GeneratePreActionPoses(
      const PreActionPose::ActionType type,
