...
#define TRACK_INCOMING_PACKET_LATENCY_TIMESTAMP_MS() \
  ::Anki::Util::GetCurrentNetTimeStamp()
#else
#define TRACK_INCOMING_PACKET_LATENCY_TIMESTAMP_MS() \
  ::Anki::Util::kNetTimeStampZero
#endif
...
class RobotConnectionMessageData {
 public:
  ...
  RobotConnectionMessageData(const uint8_t* data, uint32_t numBytes,
                             const Util::TransportAddress& address,
                             Util::NetTimeStamp timeReceived_ms)
  ...
  RobotConnectionMessageData(RobotConnectionMessageType newType,
                             const Util::TransportAddress& address,
                             Util::NetTimeStamp timeReceived_ms)
  ...
  uint32_t GetMemorySize() const {
    return Util::numeric_cast<uint32_t>(
        _rawMessageData.capacity() *
            sizeof(decltype(_rawMessageData)::value_type) +
        sizeof(*this));
  }
