...
RobotConnectionManager::~RobotConnectionManager() { DisconnectCurrent(); }
...
Result RobotConnectionManager::Update() {
  ...
  // Update queue stats before processing messages so we get stats about how big
  // the queue was prior to it being cleared
...
void RobotConnectionManager::SendAndResetQueueStats() {
  // Note: This used to be the DAS message
  // "robot.msg_queue.recent_incoming_size" but was demoted to an INFO since it
  // was spamming DAS
  LOG_INFO("RobotConnectionManager.SendAndResetQueueStats.Stats",
           "num %d, min %.2f, mean %.2f, max %.2f",
           _queueSizeAccumulator.GetNum(), _queueSizeAccumulator.GetMin(),
           _queueSizeAccumulator.GetMean(), _queueSizeAccumulator.GetMax());
...
Result RobotConnectionManager::Connect(RobotID_t robotID) {
  // LOG_DEBUG("RobotConnectionManager.Connect", "Connect to robot %d",
  // robotID);
  ...
  const std::string& client_path =
      ENGINE_ANIM_CLIENT_PATH + std::to_string(robotID);
  const std::string& server_path =
      ENGINE_ANIM_SERVER_PATH + std::to_string(robotID);

  const bool ok = _udpClient.Connect(client_path, server_path);
  if (!ok) {
    LOG_WARNING("RobotConnectionManager.Connect",
                "Unable to connect from %s to %s", client_path.c_str(),
                server_path.c_str());
...
bool RobotConnectionManager::SendData(const uint8_t* buffer,
                                      unsigned int size) {
  ...
  if (sent != size) {
    LOG_ERROR("RobotConnectionManager.SendData.Error",
              "Sent %zd/%d bytes to robot", sent, size);
    DisconnectCurrent();
    return false;
  }
...
void RobotConnectionManager::ProcessArrivedMessages() {
  ...
  while (_udpClient.IsConnected()) {
    ...
    if (n < 0) {
      LOG_ERROR("RobotConnectionManager.ProcessArrivedMessages",
                "Read error from robot");
      break;
    } else if (n == 0) {
      // LOG_DEBUG("RobotConnectionManager.ProcessArrivedMessages", "Nothing to
      // read");
      break;
    } else {
      // LOG_DEBUG("RobotConnectionManager.ProcessArrivedMessages", "Read
      // %zd/%lu from robot", n, sizeof(buf));
      _currentConnectionData->PushArrivedMessage((const uint8_t*)buf,
                                                 (uint32_t)n, addr);
    }
  }

  while (_currentConnectionData->HasMessages()) {
    RobotConnectionMessageData nextMessage =
        _currentConnectionData->PopNextMessage();
    ...
    if (timeReceived != Util::kNetTimeStampZero) {
      const double timeQueued_ms =
          Util::GetCurrentNetTimeStamp() - timeReceived;
    ...
    } else if (RobotConnectionMessageType::ConnectionResponse ==
               nextMessage.GetType()) {
      HandleConnectionResponseMessage(nextMessage);
    } else if (RobotConnectionMessageType::Disconnect ==
               nextMessage.GetType()) {
      HandleDisconnectMessage(nextMessage);
    } else if (RobotConnectionMessageType::ConnectionRequest ==
               nextMessage.GetType()) {
      HandleConnectionRequestMessage(nextMessage);
    } else {
      LOG_ERROR(
          "RobotConnectionManager.ProcessArrivedMessages.UnhandledMessageType",
          "Unhandled message type %d. Ignoring", nextMessage.GetType());
    }
  }
}

void RobotConnectionManager::HandleDataMessage(
    RobotConnectionMessageData& nextMessage) {
  const bool isConnected = IsValidConnection();
  if (!isConnected) {
    LOG_INFO("RobotConnectionManager.HandleDataMessage.NotValidState",
             "Connection not yet valid, dropping message");
    return;
  }

  const bool correctAddress =
      _currentConnectionData->GetAddress() == nextMessage.GetAddress();
  if (!correctAddress) {
    LOG_ERROR(
        "RobotConnectionManager.HandleDataMessage.IncorrectAddress",
...
void RobotConnectionManager::HandleConnectionResponseMessage(
    RobotConnectionMessageData& nextMessage) {
  LOG_DEBUG("RobotConnectionManager.HandleConnectionResponseMessage",
            "Handle connection response");

  const bool isWaitingState =
      _currentConnectionData->GetState() == RobotConnectionData::State::Waiting;
  if (!isWaitingState) {
    LOG_ERROR(
        "RobotConnectionManager.HandleConnectionResponseMessage."
        "NotWaitingForConnection",
...
void RobotConnectionManager::HandleDisconnectMessage(
    RobotConnectionMessageData& nextMessage) {
  LOG_DEBUG("RobotConnectionManager.HandleDisconnectMessage",
            "Handle disconnect");

  const bool connectionWasInWaitingState =
      (RobotConnectionData::State::Waiting ==
       _currentConnectionData->GetState());

  // This connection is no longer valid.
  // Note not calling DisconnectCurrent because this message means
  // reliableTransport is already deleting this connection data
  _currentConnectionData->Clear();

  // This robot is gone.
  Robot* robot = _robotManager->GetRobot();
  if (nullptr != robot) {
    // If the connection is waiting when we handle this disconnect message,
    // report it as a robot rejection
    _robotManager->RemoveRobot(connectionWasInWaitingState);
  }
}

void RobotConnectionManager::HandleConnectionRequestMessage(
    RobotConnectionMessageData& nextMessage) {
...
bool RobotConnectionManager::IsValidConnection() const {
  return _currentConnectionData->GetState() ==
         RobotConnectionData::State::Connected;
}
...
void RobotConnectionManager::ClearData() { _readyData.clear(); }

const Anki::Util::Stats::StatsAccumulator&
RobotConnectionManager::GetQueuedTimes_ms() const {
#if TRACK_INCOMING_PACKET_LATENCY
...
void RobotConnectionManager::InitSocketBufferStats() {
  ...
  _incomingStats =
      std::make_unique<Histogram>(lowest, highest, significant_figures);
  _outgoingStats =
      std::make_unique<Histogram>(lowest, highest, significant_figures);
  ...
  DEV_ASSERT(
      _incomingStats,
      "RobotConnectionManager.InitSocketBufferStats.InvalidIncomingStats");
  DEV_ASSERT(
      _outgoingStats,
      "RobotConnectionManager.InitSocketBufferStats.InvalidOutgoingStats");
}
...
void RobotConnectionManager::UpdateSocketBufferStats() {
  ...
  DEV_ASSERT(
      _incomingStats,
      "RobotConnectionManager.UpdateSocketBufferStats.InvalidIncomingStats");
  DEV_ASSERT(
      _outgoingStats,
      "RobotConnectionManager.UpdateSocketBufferStats.InvalidOutgoingStats");
...
void RobotConnectionManager::ReportSocketBufferStats(
    const std::string& name, const HistogramPtr& histogram) {
  // Preconditions
  DEV_ASSERT(histogram,
             "RobotConnectionManager.ReportSocketBufferStats.InvalidHistogram");

  const int64_t min = histogram->GetMin();
  const int64_t mean = histogram->GetMean();
  const int64_t max = histogram->GetMax();

  LOG_INFO("RobotConnectionManager.ReportSocketBufferStats",
           "%s: %lld/%lld/%lld", name.c_str(), min, mean, max);
}
...
void RobotConnectionManager::ReportSocketBufferStats() {
  ...
  DEV_ASSERT(
      _incomingStats,
      "RobotConnectionManager.ReportSocketBufferStats.InvalidIncomingStats");
  DEV_ASSERT(
      _outgoingStats,
      "RobotConnectionManager.ReportSocketBufferStats.InvalidOutgoingStats");
