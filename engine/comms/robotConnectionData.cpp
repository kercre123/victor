...
RobotConnectionMessageData RobotConnectionData::PopNextMessage() {
  ...
  DEV_ASSERT(!_arrivedMessages.empty(),
             "RobotConnectionData.PopNextMessage.NoMessages");
  ...
  ANKI_VERIFY(queueSizeOK,
              "RobotConnectionMessageData.PopNextMessage.NegativeSize",
...
void RobotConnectionData::PushArrivedMessage(
    const uint8_t* buffer, uint32_t numBytes,
    const Util::TransportAddress& address) {
...

  {
    ...
    if (messageType == RobotConnectionMessageType::Data) {
      // Note we don't bother passing the MessageType::Data into this
      // constructor because that's the default
      _arrivedMessages.emplace_back(
          buffer, numBytes, address,
          TRACK_INCOMING_PACKET_LATENCY_TIMESTAMP_MS());
    } else {
      _arrivedMessages.emplace_back(
          messageType, address, TRACK_INCOMING_PACKET_LATENCY_TIMESTAMP_MS());
    }
...
void RobotConnectionData::ReceiveData(
    const uint8_t* buffer, unsigned int size,
    const Util::TransportAddress& sourceAddress) {
  const bool isConnectionRequest =
      INetTransportDataReceiver::OnConnectRequest == buffer;
  DEV_ASSERT(!isConnectionRequest,
             "RobotConnectionManager.ReceiveData.ConnectionRequest.NotHandled");
...
void RobotConnectionData::QueueConnectionDisconnect() {
  PushArrivedMessage(Util::INetTransportDataReceiver::OnDisconnected, 0,
                     _address);
}
...
void RobotConnectionData::UpdateQueueSizeStatistics() {
  ...
  if (shouldSetWarning) {
    PRINT_NAMED_WARNING(
        "RobotConnectionManager.ArrivedMessageQueue.QueueTooLarge",
        "Queue size is %u bytes", queueSize);

  } else if (shouldClearWarning) {
    // we're out of the warning zone now. Send up another warning to signify
    // this
    PRINT_NAMED_WARNING(
        "RobotConnectionManager.ArrivedMessageQueue.QueueNoLongerTooLarge",
        "Queue size is down to %u bytes. Max this run is %u", queueSize,
        maxQueueSize);
  }
}
