...
class ObservableObject : public Vision::ObservableObject,
 ...
 public:
  ...
  ObservableObject(ObjectType type) : _type(type) {}

  virtual ObservableObject* CloneType() const override = 0;

  // Can only be called once and only before SetPose is called. Will assert
  // otherwise, since this indicates programmer error. The parameter
  // fromDistance_mm is the distance from which the object was visually
  // observed, if applicable. A value of -1 indicates that the pose is not being
  // initiailized from a visual observation.
  void InitPose(const Pose3d& pose, PoseState poseState,
                const float fromDistance_mm = -1.f);

  // Override base class SetID to use unique ID for each type (base class has no
  // concept of ObjectType)
  ...
  // Defines the maximum distance from which we can observe the object and
  // update its pose. In other words, if the camera observes the object from a
  // distance greater than this, we will discard the observation as
  // untrustworthy.
  virtual f32 GetMaxObservationDistance_mm() const;

  // CTI SetIsMoving/IsMoving methods with TimeStamp_t are forwarded to cozmo
  // methods and marked as final to force child classes to use RobotTimeStamp_t
  virtual bool IsMoving(
      TimeStamp_t* t = (TimeStamp_t*)nullptr) const override final;
  virtual void SetIsMoving(bool isMoving, TimeStamp_t t) override final {
    SetIsMoving(isMoving, RobotTimeStamp_t{t});
  }
 ...
 protected:
  ...
  // Note: we make SetPose protected and friend BlockWorld, so that only
  // BlockWorld can update objects' poses
  virtual void SetPose(const Pose3d& newPose, f32 fromDistance,
                       PoseState newPoseState) override;
...
inline bool ObservableObject::IsSameAs(const ObservableObject& otherObject,
  ...
  if (isSame) {
    isSame = Vision::ObservableObject::IsSameAs(
        otherObject, distThreshold, angleThreshold, Tdiff, angleDiff);
  }
...
inline void ObservableObject::SetActiveID(ActiveID activeID) {
  if (!IsActive()) {
    PRINT_NAMED_WARNING("ObservableObject.SetActiveID.NotActive", "ID: %d",
                        GetID().GetValue());
    return;
  }
...
inline void ObservableObject::SetFactoryID(FactoryID factoryID) {
  if (!IsActive()) {
    PRINT_NAMED_WARNING("ObservableObject.SetFactoryID.NotActive", "ID: %d",
                        GetID().GetValue());
    return;
  }
