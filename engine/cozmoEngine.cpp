...
static int GetEngineStatsWebServerImpl(
    WebService::WebService::Request* request) {
  auto* cozmoEngine = static_cast<CozmoEngine*>(request->_cbdata);
  if (cozmoEngine->GetEngineState() != EngineState::Running) {
    LOG_INFO("CozmoEngine.GetEngineStatsWebServerImpl.NotReady",
             "GetEngineStatsWebServerImpl called but engine not running");
    return 0;
  }
  ...
  ss << std::fixed << std::setprecision(3)
     << batteryComponent.GetBatteryVoltsRaw() << '\n';
  ss << std::fixed << std::setprecision(3)
     << batteryComponent.GetChargerVoltsRaw() << '\n';
  ss << EnumToString(batteryComponent.GetBatteryLevel()) << '\n';
  ss << std::to_string(
            static_cast<int>(batteryComponent.GetBatteryTemperature_C()))
     << '\n';
  ...
  ss << std::to_string(static_cast<int>(
            batteryComponent.GetTimeAtLevelSec(BatteryLevel::Full)))
     << '\n';
  ss << std::to_string(static_cast<int>(
            batteryComponent.GetTimeAtLevelSec(BatteryLevel::Low)))
     << '\n';
  ...
  ss << std::fixed << std::setprecision(1)
     << RAD_TO_DEG(robotState.poseAngle_rad) << '\n';
  ss << std::fixed << std::setprecision(1)
     << RAD_TO_DEG(robotState.posePitch_rad) << '\n';
  ss << std::fixed << std::setprecision(1)
     << RAD_TO_DEG(robotState.headAngle_rad) << '\n';
  ...
  const auto& micDirectionHistory =
      robot->GetMicComponent().GetMicDirectionHistory();
  ...
  ss << std::fixed << std::setprecision(3)
     << 1.f / Util::MilliSecToSec((f32)framePeriod_ms) << '\n';
  ss << std::fixed << std::setprecision(3)
     << 1.f / Util::MilliSecToSec((f32)procPeriod_ms) << '\n';
...
static int GetEngineStatsWebServerHandler(struct mg_connection* conn,
                                          void* cbdata) {
  // We ignore the query string because overhead is minimal

  auto* cozmoEngine = static_cast<CozmoEngine*>(cbdata);
  if (cozmoEngine->GetEngineState() != EngineState::Running) {
    LOG_INFO("CozmoEngine.GetEngineStatsWebServerHandler.NotReady",
             "GetEngineStatsWebServerHandler called but engine not running");
    return 0;
  }

  auto ws = cozmoEngine->GetRobot()->GetContext()->GetWebService();
  const int returnCode =
      ws->ProcessRequestExternal(conn, cbdata, GetEngineStatsWebServerImpl);
...
CozmoEngine::CozmoEngine(Util::Data::DataPlatform* dataPlatform)
    : _uiMsgHandler(new UiMessageHandler(1)),
      ...
      _context(new CozmoContext(dataPlatform, _uiMsgHandler.get(),
                                _protoMsgHandler.get())),
      _animationTransferHandler(
          new AnimationTransfer(_uiMsgHandler.get(), dataPlatform)) {
#if ANKI_CPU_PROFILER_ENABLED
  // Initialize CPU profiler early and put tracing file at known location with
  // no dependencies on other systems
  Anki::Util::CpuProfiler::GetInstance();
  Anki::Util::CpuThreadProfiler::SetChromeTracingFile(
      dataPlatform
          ->pathToResource(Util::Data::Scope::Cache, "vic-engine-tracing.json")
          .c_str());
  Anki::Util::CpuThreadProfiler::SendToWebVizCallback(
      [&](const Json::Value& json) {
        _context->GetWebService()->SendToWebViz("cpuprofile", json);
      });
#endif

  DEV_ASSERT(_context->GetExternalInterface() != nullptr,
             "Cozmo.Engine.ExternalInterface.nullptr");
  if (Anki::Util::gTickTimeProvider == nullptr) {
    Anki::Util::gTickTimeProvider = BaseStationTimer::getInstance();
  }

  // Designate this thread as the one from which the engine can broadcast
  // messages
  _context->SetEngineThread();

  DASMSG(engine_language_locale, "engine.language_locale",
         "Prints out the language locale of the robot");
  DASMSG_SET(s1, _context->GetLocale()->GetLocaleString().c_str(),
             "Locale on start up");
  DASMSG_SEND();
  ...
  auto handler =
      [this](const std::vector<Util::AnkiLab::AssignmentDef>& assignments) {
        _context->GetExperiments()->UpdateLabAssignments(assignments);
      };
  _signalHandles.emplace_back(_context->GetExperiments()
                                  ->GetAnkiLab()
                                  .ActiveAssignmentsUpdatedSignal()
                                  .ScopedSubscribe(handler));

  _debugConsoleManager.Init(_context->GetExternalInterface(),
                            _context->GetRobotManager()->GetMsgHandler());
...
Result CozmoEngine::Init(const Json::Value& config) {
  if (_isInitialized) {
    LOG_INFO(
        "CozmoEngine.Init.ReInit",
        "Reinitializing already-initialized CozmoEngineImpl with new config.");
  }
  ...
  osState->SendToWebVizCallback([&](const Json::Value& json) {
    _context->GetWebService()->SendToWebViz("cpu", json);
  });

  _config = config;

  if (!_config.isMember(AnkiUtil::kP_ADVERTISING_HOST_IP)) {
    PRINT_NAMED_ERROR("CozmoEngine.Init",
                      "No AdvertisingHostIP defined in Json config.");
    return RESULT_FAIL;
  }

  if (!_config.isMember(AnkiUtil::kP_UI_ADVERTISING_PORT)) {
    PRINT_NAMED_ERROR("CozmoEngine.Init",
                      "No UiAdvertisingPort defined in Json config.");
    return RESULT_FAIL;
  }
  ...
  if (RESULT_OK != lastResult) {
    PRINT_NAMED_ERROR("CozmoEngine.Init",
                      "Error initializing UiMessageHandler");
    return lastResult;
  }
  ...
  if (RESULT_OK != lastResult) {
    PRINT_NAMED_ERROR("CozmoEngine.Init",
                      "Error initializing ProtoMessageHandler");
    return lastResult;
  }
  ...
  if (ANKI_DEV_CHEATS) {
    if (nullptr != _context->GetExternalInterface()) {
      ...
      _context->GetVizManager()->SubscribeToEngineEvents(
  ...
  webService->RegisterRequestHandler("/getenginestats",
                                     GetEngineStatsWebServerHandler, this);
...
Result CozmoEngine::Update(const BaseStationTime_t currTime_nanosec) {
  ...
  if (!_isInitialized) {
    PRINT_NAMED_ERROR("CozmoEngine.Update",
                      "Cannot update CozmoEngine before it is initialized.");
    return RESULT_FAIL;
  }
  ...
  if (!_hasRunFirstUpdate) {
    ...
    // Controls OpenCV's built-in multithreading for the calling thread, so we
    // have to do this on the first call to update due to the threading quirk
    Result cvResult =
        SetNumOpencvThreads(NUM_OPENCV_THREADS, "CozmoEngine.Init");
  ...
  // Enable/disable external motor commands depending on whether we have an
  // external UI connection (i.e. Webots). If we are connected via webots, then
  // we want to allow external motion commands. Else do not allow motor
  // commands. Note: this cannot be done in the state change logic above, since
  // robot is sometimes null when the UI connection is established.
  Robot* robot = GetRobot();
  if (robot != nullptr && _updateMoveComponent) {
    const bool hasUiConnection = _uiMsgHandler->HasDesiredNumUiDevices();
    robot->GetMoveComponent().AllowExternalMovementCommands(hasUiConnection,
                                                            "ui");
  ...
  if (RESULT_OK != lastResult) {
    PRINT_NAMED_ERROR("CozmoEngine.Update",
                      "Error updating ProtoMessageHandler");
    return lastResult;
  }
  ...
  switch (_engineState) {
    ...
    case EngineState::LoadingData: {
      ...
      if (_context->GetDataLoader()->DoNonConfigDataLoading(
              currentLoadingDone)) {
    ...
    case EngineState::ConnectingToRobot: {
      ...
      if (RESULT_OK != result) {
        // LOG_WARNING("CozmoEngine.Update.ConnectingToRobot", "Unable to
        // connect to robot (result %d)", result);
        break;
      }

      // Now connected
      LOG_INFO("CozmoEngine.Update.ConnectingToRobot",
               "Now connected to robot");
    ...
    case EngineState::Running: {
      ...
      if (RESULT_OK != result) {
        LOG_ERROR("CozmoEngine.Update.Running",
                  "Unable to update robot connection (result %d)", result);
        return result;
      }
      ...
      if (result != RESULT_OK) {
        LOG_WARNING("CozmoEngine.Update.UpdateRobotFailed",
                    "Update robot failed with %d", result);
        return result;
      }
    ...
    default:
      PRINT_NAMED_ERROR("CozmoEngine.Update.UnexpectedState",
                        "Running Update in an unexpected state!");
      break;
  }
...
void PrintTimingInfoStats(const ExternalInterface::TimingInfo& timingInfo,
                          const char* name) {
  PRINT_CH_INFO("UiComms", "CozmoEngine.LatencyStats", "%s: = %f (%f..%f)",
                name, timingInfo.avgTime_ms, timingInfo.minTime_ms,
                timingInfo.maxTime_ms);
}

void PrintTimingInfoStats(
    const ExternalInterface::CurrentTimingInfo& timingInfo, const char* name) {
  PRINT_CH_INFO("UiComms", "CozmoEngine.LatencyStats",
                "%s: = %f (%f..%f) (curr: %f)", name, timingInfo.avgTime_ms,
                timingInfo.minTime_ms, timingInfo.maxTime_ms,
                timingInfo.currentTime_ms);
}
...
void CozmoEngine::UpdateLatencyInfo() {
  ...
  if (Util::kNetConnStatsUpdate) {
    ...
    if (kLogMessageLatencyOnce) {
      ExternalInterface::TimingInfo wifiLatency(Util::gNetStat2LatencyAvg,
                                                Util::gNetStat4LatencyMin,
                                                Util::gNetStat5LatencyMax);
      ExternalInterface::TimingInfo extSendQueueTime(
          Util::gNetStat7ExtQueuedAvg_ms, Util::gNetStat8ExtQueuedMin_ms,
          Util::gNetStat9ExtQueuedMax_ms);
      ExternalInterface::TimingInfo sendQueueTime(Util::gNetStatAQueuedAvg_ms,
                                                  Util::gNetStatBQueuedMin_ms,
                                                  Util::gNetStatCQueuedMax_ms);
      const Util::Stats::StatsAccumulator& queuedTimes_ms =
          _context->GetRobotManager()->GetMsgHandler()->GetQueuedTimes_ms();
      ExternalInterface::TimingInfo recvQueueTime(queuedTimes_ms.GetMean(),
                                                  queuedTimes_ms.GetMin(),
                                                  queuedTimes_ms.GetMax());

      const Util::Stats::StatsAccumulator& unityLatency =
          _uiMsgHandler->GetLatencyStats(UiConnectionType::UI);
      ExternalInterface::TimingInfo unityEngineLatency(
          unityLatency.GetMean(), unityLatency.GetMin(), unityLatency.GetMax());
...
Result CozmoEngine::InitInternal() {
  ...
  if (numLogs >= MIN_NUM_FACTORY_TEST_LOGS_FOR_ARCHIVING) {
    if (factoryTestLogger.ArchiveLogs(_context->GetDataPlatform())) {
      LOG_INFO("CozmoEngine.InitInternal.ArchivedFactoryLogs",
               "%d logs archived", numLogs);
    } else {
...
Result CozmoEngine::ConnectToRobotProcess() {
  ...
  if (!msgHandler->IsConnected(robotID)) {
    ...
    if (RESULT_OK != result) {
      // LOG_WARNING("CozmoEngine.ConnectToRobotProcess", "Unable to connect to
      // robot %d (result %d)", robotID, result);
      return result;
    }
  }
...
void CozmoEngine::HandleMessage(const ExternalInterface::ImageRequest& msg) {
  Robot* robot = GetRobot();
  if (robot != nullptr) {
    return robot->GetVisionComponent().EnableImageSending(
        msg.mode == ImageSendMode::Stream);
  }
}
...
void CozmoEngine::HandleMessage(const ExternalInterface::StartTestMode& msg) {
  Robot* robot = GetRobot();
  if (robot != nullptr) {
    robot->SendRobotMessage<StartControllerTestMode>(msg.p1, msg.p2, msg.p3,
                                                     msg.mode);
  }
}
...
void CozmoEngine::InitUnityLogger() {
  ...
  if (Anki::Util::gLoggerProvider != nullptr) {
    Anki::Util::MultiLoggerProvider* multiLoggerProvider =
        dynamic_cast<Anki::Util::MultiLoggerProvider*>(
            Anki::Util::gLoggerProvider);
    if (multiLoggerProvider != nullptr) {
      const std::vector<Anki::Util::ILoggerProvider*>& loggers =
          multiLoggerProvider->GetProviders();
      for (int i = 0; i < loggers.size(); ++i) {
        CLADLoggerProvider* unityLoggerProvider =
            dynamic_cast<CLADLoggerProvider*>(loggers[i]);
        if (unityLoggerProvider != nullptr) {
          unityLoggerProvider->SetExternalInterface(
              _context->GetExternalInterface());
          break;
        }
      }
...
void CozmoEngine::HandleMessage(const ExternalInterface::RedirectViz& msg) {
  ...
  if (ANKI_DEV_CHEATS) {
    ...
    ss << (int)ipBytes[0] << "." << (int)ipBytes[1] << "." << (int)ipBytes[2]
       << "." << (int)ipBytes[3];
    ...
    // Erase anything that's still being visualized in case there were leftovers
    // from a previous run?? (We should really be cleaning up after ourselves
    // when we tear down, but it seems like Webots restarts aren't always
    // allowing the cleanup to happen)
...
Util::AnkiLab::AssignmentStatus CozmoEngine::ActivateExperiment(
  ...
  return _context->GetExperiments()->ActivateExperiment(request,
                                                        outVariationKey);
}

void CozmoEngine::RegisterEngineTickPerformance(
    const float tickDuration_ms, const float tickFrequency_ms,
