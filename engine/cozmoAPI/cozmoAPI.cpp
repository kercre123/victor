...
CONSOLE_VAR_ENUM(u8, kCozmoEngine_Logging, ANKI_CPU_CONSOLEVARGROUP, 0,
                 Util::CpuProfiler::CpuProfilerLogging());
#endif

bool CozmoAPI::Start(Util::Data::DataPlatform* dataPlatform,
                     const Json::Value& config) {
  // Game init happens in EngineInstanceRunner construction, so we get the
  // result If we already had an instance, kill it and start again
  bool gameInitResult = false;
  _engineRunner.reset();
  _engineRunner.reset(
      new EngineInstanceRunner(dataPlatform, config, gameInitResult));
...
ANKI_CPU_PROFILER_ENABLED_ONLY(
    const float kMaxDesiredEngineDuration = 60.0f);  // Above this warn etc.
...
bool CozmoAPI::Update(const BaseStationTime_t currentTime_nanosec) {
  ...
  // Replace Util::CpuThreadProfiler::kLogFrequencyNever with a small value to
  // output logging, can be used with Chrome Tracing format
  ANKI_CPU_TICK(
      "CozmoEngine", kMaxDesiredEngineDuration,
      Util::CpuProfiler::CpuProfilerLoggingTime(kCozmoEngine_Logging));

  return _engineRunner->Update(currentTime_nanosec);
}

uint32_t CozmoAPI::ActivateExperiment(const uint8_t* requestBuffer,
                                      size_t requestLen,
                                      uint8_t* responseBuffer,
                                      size_t responseLen) {
  using namespace Anki::Util::AnkiLab;

  // Response buffer will be filled in by ActivateExperiment. Set default values
  // here.
  ...
  ASSERT_NAMED((nullptr != requestBuffer) && (requestLen > 0),
               "Must provide a valid requestBuffer/requestBufferLen to "
               "activate experiment");
  ASSERT_NAMED(
      (nullptr != responseBuffer) && (responseLen >= minResponseBufferLen),
      "Must provide a valid responseBuffer/responseBufferLen to activate "
      "experiment");
...
void CozmoAPI::RegisterEngineTickPerformance(
  ...
  _engineRunner->GetEngine()->RegisterEngineTickPerformance(
      tickDuration_ms, tickFrequency_ms, sleepDurationIntended_ms,
      sleepDurationActual_ms);
}

CozmoAPI::~CozmoAPI() {
  if (_engineRunner) {
    // We are now "owning thread" for engine message sending; this is here in
    // case messages are sent during destruction
...
CozmoAPI::EngineInstanceRunner::EngineInstanceRunner(
    Util::Data::DataPlatform* dataPlatform, const Json::Value& config,
    bool& initResult)
  ...
  if (initResultReturn != RESULT_OK) {
    LOG_ERROR("CozmoAPI.EngineInstanceRunner",
              "cozmo init failed with error %d", initResultReturn);
  }
  initResult = initResultReturn == RESULT_OK;
}

// Destructor must exist in cpp (even though it's empty) in order for CozmoGame
// unique_ptr to be defined and deletable
CozmoAPI::EngineInstanceRunner::~EngineInstanceRunner() {}

bool CozmoAPI::EngineInstanceRunner::Update(
    const BaseStationTime_t currentTime_nanosec) {
  Result updateResult;
  {
  ...
  if (updateResult != RESULT_OK) {
    LOG_ERROR("CozmoAPI.EngineInstanceRunner.Update",
              "Cozmo update failed with error %d", updateResult);
  }
  return updateResult == RESULT_OK;
}

void CozmoAPI::EngineInstanceRunner::SyncWithEngineUpdate(
...
void CozmoAPI::EngineInstanceRunner::SetEngineThread() {
  ...
  DEV_ASSERT(_engineInstance,
             "CozmoAPI.EngineInstanceRunner.InvalidEngineInstance");
  std::lock_guard<std::mutex> lock{_updateMutex};
