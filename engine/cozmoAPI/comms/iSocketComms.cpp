...
void ISocketComms::Update() {
  // If a client is connected, artificially set the lastPingTime so that it
  // still times out if no ping is ever received.
  const uint32_t curTime_ms = static_cast<uint32_t>(
  ...
  if (_pingTimeoutForDisconnect_ms > 0 && _lastPingTime_ms > 0) {
    if (curTime_ms - _lastPingTime_ms > _pingTimeoutForDisconnect_ms) {
      _lastPingTime_ms = 0;
      PRINT_CH_INFO("UiComms", "Update.DisconnectByPingTimeout",
                    "Timeout: %d ms", _pingTimeoutForDisconnect_ms);
...
void ISocketComms::HandlePingResponse(const ExternalInterface::Ping& pingMsg) {
  ...
  if (kPrintUiMessageLatency) {
    ...
    if (numSamples && ((numSamples % kReportFrequency) == 0)) {
      ...
      PRINT_CH_INFO("UiComms", "UiMessageLatency",
                    "%.2f ms, [%.2f..%.2f], SD= %.2f, %u samples",
                    averageLatency, minLatency, maxLatency, stdDevLatency,
                    numSamples);
    }
  }
...
void ISocketComms::SetPingTimeoutForDisconnect(uint32_t ms,
