...
class ProtoMessageHandler : public IGatewayInterface {
 public:
  ...
  virtual void Broadcast(
      const external_interface::GatewayWrapper& message) override;
  ...
  AnkiEventMgr<external_interface::GatewayWrapper>& GetEventMgr() {
    return _eventMgr;
  }

  const Util::Stats::StatsAccumulator& GetLatencyStats() const;

  virtual uint32_t GetMessageCountOutgoing() const override {
    return _messageCountOutgoing;
  }
  virtual uint32_t GetMessageCountIncoming() const override {
    return _messageCountIncoming;
  }
 ...
 private:
  ...
  virtual void DeliverToExternal(
      const external_interface::GatewayWrapper& message) override;
