...
class IGameMessageHandler {
 public:
  ...
  virtual Result SendMessage(
      const UserDeviceID_t devID,
      const ExternalInterface::MessageGameToEngine& msg) = 0;
...
class GameMessageHandler : public IGameMessageHandler {
 public:
  ...
  // Returns RESULT_FAIL if no handler callback was registered for one or more
  ...
  Result SendMessage(
      const UserDeviceID_t devID,
      const ExternalInterface::MessageGameToEngine& msg) override;

  inline void RegisterCallbackForMessage(
      const std::function<void(const ExternalInterface::MessageEngineToGame&)>&
          messageCallback) {
 ...
 protected:
  ...
  std::function<void(const ExternalInterface::MessageEngineToGame&)>
      messageCallback;
...
class GameMessageHandlerStub : public IGameMessageHandler {
  ...
  virtual Result Init(Comms::IComms* comms) override { return RESULT_OK; }
