...
GameComms::GameComms(int deviceID, int serverListenPort,
                     const char* advertisementRegIP, int advertisementRegPort)
    : server_("gameComms"),
  ...
  if (false == server_.StartListening(serverListenPort_)) {
    PRINT_NAMED_ERROR("GameComms.Constructor",
                      "Failed to start listening on port %d",
                      serverListenPort_);
  }
}

GameComms::~GameComms() { DisconnectClient(); }

// Returns true if we are ready to use TCP
bool GameComms::IsInitialized() { return isInitialized_; }
...
u32 GameComms::GetNumMsgPacketsInSendQueue(int devID) {
  // TODO: This function isn't used on the game side, sent messages aren't
  // queued anyway, so just returning 0.
  return 0;
}
  ...
  if (!IsInitialized()) {
    ...
    if (regClient_.Connect(advertisementRegIP_, advertisementRegPort_)) {
...

      {
        ...
        char tempBuff[32];
        snprintf(tempBuff, sizeof(tempBuff), "%d.%d.%d.%d", (int)ipBytes[0],
                 (int)ipBytes[1], (int)ipBytes[2], (int)ipBytes[3]);
        regMsg_.ip = tempBuff;
      }
      ...
      PRINT_NAMED_INFO("GameComms.Update",
                       "Waiting to connect to advertisement service");
      return;
    }
  }
...
void GameComms::ReadAllMsgPackets() {
  ...
  while ((recvDataSize = server_.Recv((char*)(_recvBuf), MAX_RECV_BUF_SIZE)) >
         0) {
    recvdMsgPackets_.emplace_back(
        (s32)(0),  // Source device ID. Not used for anything now so just 0.
        (s32)-1, recvDataSize, (u8*)(_recvBuf));
  }
...
void GameComms::ClearMsgPackets() { recvdMsgPackets_.clear(); };

bool GameComms::HasClient() { return server_.HasClient(); }

void GameComms::DisconnectClient() { server_.DisconnectClient(); }
...
void GameComms::AdvertiseToService() {
  ...
  PRINT_NAMED_INFO(
      "GameComms.AdvertiseToService",
      "Sending registration for UI device %d at address %s on port %d/%d",
      regMsg_.id, regMsg_.ip.c_str(), (int)regMsg_.toEnginePort,
      (int)regMsg_.fromEnginePort);
  ...
  const size_t bytesPacked =
      outMessage.Pack(messageBuffer, sizeof(messageBuffer));
