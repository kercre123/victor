 ...
 *              RobotMessageHandler handles messages between basestation and
 *robot.
 *
...
ProtoMessageHandler::ProtoMessageHandler()
    : _socketComms(
          new LocalUdpSocketComms(true, ENGINE_GATEWAY_PROTO_SERVER_PATH)),
...
Result ProtoMessageHandler::Init(CozmoContext* context,
                                 const Json::Value& config) {
  if (_socketComms) {
    if (!_socketComms->Init(UiConnectionType::Gateway,
                            config))  // These params don't really do anything
    {
      return RESULT_FAIL;
    }
  }
  ...
  auto versionStateRequestCallback =
      std::bind(&RobotExternalRequestComponent::GetVersionState,
                externalRequestComponent, std::placeholders::_1);
  auto batteryStateRequestCallback =
      std::bind(&RobotExternalRequestComponent::GetBatteryState,
                externalRequestComponent, std::placeholders::_1);

  // Subscribe to desired simple events
  _signalHandles.push_back(
      Subscribe(external_interface::GatewayWrapperTag::kBatteryStateRequest,
                batteryStateRequestCallback));
  _signalHandles.push_back(
      Subscribe(external_interface::GatewayWrapperTag::kVersionStateRequest,
                versionStateRequestCallback));

  return RESULT_OK;
}

void ProtoMessageHandler::DeliverToExternal(
    const external_interface::GatewayWrapper& message) {
...
Result ProtoMessageHandler::ProcessMessageBytes(
    const uint8_t* const packetBytes, const size_t packetSize,
  ...
  if (packetSize > 0) {
    const bool success =
        message.ParseFromArray(packetBytes, static_cast<int>(packetSize));
    ...
    // Is there a potential, in adding the redirect and not returning (on
    // success), for these messages to arrive at their destination twice?
...
void ProtoMessageHandler::Broadcast(
    const external_interface::GatewayWrapper& message) {
...
void ProtoMessageHandler::Broadcast(
  ...
  _eventMgr.Broadcast(AnkiEvent<external_interface::GatewayWrapper>(
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds(),
      static_cast<u32>(message.GetTag()), std::move(message)));
}  // Broadcast(MessageGameToEngine &&)

// Provides a way to subscribe to message types using the AnkiEventMgrs
Signal::SmartHandle ProtoMessageHandler::Subscribe(
    const external_interface::GatewayWrapperTag& tagType,
    std::function<void(const AnkiEvent<external_interface::GatewayWrapper>&)>
        messageHandler) {
    ...
    if (_socketComms) {
      ...
      while (keepReadingMessages) {
        ...
        if (keepReadingMessages) {
          Result res = ProcessMessageBytes(buffer.data(), buffer.size(),
                                           isSingleMessage);
          if (res != RESULT_OK) {
            retVal = RESULT_FAIL;
          }
...
const Util::Stats::StatsAccumulator& ProtoMessageHandler::GetLatencyStats()
    const {
  if (_socketComms) {
