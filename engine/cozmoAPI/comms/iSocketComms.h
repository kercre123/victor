 ...
 * Created: 05/14/16
 *
 * Description: Interface for any socket-based communications from e.g. Game/SDK
 *to Engine
 *
...
class ISocketComms {
 public:
  ...
  bool SendMessage(const Comms::MsgPacket& msgPacket) {
    return SendMessageInternal(msgPacket);
  }
  bool RecvMessage(std::vector<uint8_t>& outBuffer) {
    return RecvMessageInternal(outBuffer);
  }
  ...
  virtual void GetAdvertisingDeviceIDs(
      std::vector<ISocketComms::DeviceId>& outDeviceIds) = 0;
  ...
  bool HasDesiredDevices() const {
    ...
    return (numConnectedDevices >= _desiredNumDevices) &&
  ...
  const Util::Stats::StatsAccumulator& GetLatencyStats() const {
    return _latencyStats.GetPrimaryAccumulator();
  }
  ...
  uint32_t GetPingTimeoutForDisconnect() const {
    return _pingTimeoutForDisconnect_ms;
  }
