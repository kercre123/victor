...
bool IsExternalSdkConnection(UiConnectionType type) {
  switch (type) {
    ...
    case UiConnectionType::Gateway:
      return false;
    default: {
      PRINT_NAMED_ERROR("IsExternalSdkConnection.BadType", "type = %d",
                        (int)type);
      assert(0);
      return false;
    }
...
ISocketComms* CreateSocketComms(UiConnectionType type,
  ...
  // Note: Some SocketComms are deliberately null depending on the build
  // platform, type etc.
#if FACTORY_TEST
  ...
  switch (type) {
    ...
    case UiConnectionType::Switchboard: {
      ISocketComms* comms =
          new LocalUdpSocketComms(true, ENGINE_SWITCH_SERVER_PATH);
      return comms;
    }
    case UiConnectionType::Gateway: {
      ISocketComms* comms =
          new LocalUdpSocketComms(true, ENGINE_GATEWAY_SERVER_PATH);
      return comms;
    }
    default: {
      assert(0);
...
// than the default ID
  ...
  if (robotID != DEFAULT_ROBOT_ID) {
    PRINT_NAMED_WARNING(
        "UiMessageHandler.Ctor.SkippingUIConnections",
        "RobotID: %d - Only DEFAULT_ROBOT_ID may accept UI connections",
        robotID);

    for (UiConnectionType i = UiConnectionType(0); i < UiConnectionType::Count;
         ++i) {
  ...
  for (UiConnectionType i = UiConnectionType(0); i < UiConnectionType::Count;
    ...
    if ((i == UiConnectionType::UI) && (socket != nullptr)) {
      ...
      socket->SetPingTimeoutForDisconnect(kPingTimeoutForDisconnect_ms,
                                          disconnectCallback);
    }
  }
}
...
Result UiMessageHandler::Init(CozmoContext* context,
  ...
  auto commonCallback =
      std::bind(&UiMessageHandler::HandleEvents, this, std::placeholders::_1);

  // Subscribe to desired simple events
  _signalHandles.push_back(
      Subscribe(ExternalInterface::MessageGameToEngineTag::ConnectToUiDevice,
                commonCallback));
  _signalHandles.push_back(Subscribe(
      ExternalInterface::MessageGameToEngineTag::DisconnectFromUiDevice,
      commonCallback));
  _signalHandles.push_back(Subscribe(
      ExternalInterface::MessageGameToEngineTag::UiDeviceConnectionWrongVersion,
      commonCallback));
  _signalHandles.push_back(Subscribe(
      ExternalInterface::MessageGameToEngineTag::TransferFile, commonCallback));

  return RESULT_OK;
}

bool UiMessageHandler::ShouldHandleMessagesFromConnection(
    UiConnectionType type) const {
  switch (type) {
    case UiConnectionType::UI:
      return kAcceptMessagesFromUI;
    case UiConnectionType::SdkOverUdp:
      return false;
    case UiConnectionType::SdkOverTcp:
      return false;
...
bool UiMessageHandler::AreAnyConnectedDevicesOnAnySocket() const {
  for (UiConnectionType i = UiConnectionType(0); i < UiConnectionType::Count;
    ...
    if (socketComms) {
      if (socketComms->GetNumConnectedDevices() > 0) return true;
    }
  }
  return false;
}

void UiMessageHandler::DeliverToGame(
    const ExternalInterface::MessageEngineToGame& message,
    DestinationId destinationId) {
  // There is almost always a connected device, so better to just always pack
  // the message even if it won't be sent pterry 09/26/2018: Verified this is
  // still true; I think because we use messages as engine-to-engine
  // if (AreAnyConnectedDevicesOnAnySocket())
  {
    ...
    UiConnectionType connectionType =
        sendToEveryone ? UiConnectionType(0) : (UiConnectionType)destinationId;
    if (connectionType >= UiConnectionType::Count) {
      PRINT_NAMED_WARNING("UiMessageHandler.DeliverToGame.BadDestinationId",
                          "Invalid destinationId %u = UiConnectionType '%s'",
                          destinationId, EnumToString(connectionType));
    }
...
Result UiMessageHandler::ProcessMessageBytes(
  ...
  while (bytesRemaining > 0) {
    ...
    if (isSingleMessage && (bytesUnpacked != packetSize)) {
      PRINT_STREAM_ERROR(
          "UiMessageHandler.MessageBufferWrongSize",
          "Buffer's size does not match expected size for this message ID. "
          "(Msg "
              << ExternalInterface::MessageGameToEngineTagToString(
                     message.GetTag())
              << ", expected " << message.Size() << ", recvd " << packetSize
              << ")");
      return RESULT_FAIL;
    } else if (!isSingleMessage &&
               (bytesUnpacked > bytesRemaining || bytesUnpacked == 0)) {
      PRINT_STREAM_ERROR(
          "UiMessageHandler.MessageBufferWrongSize",
    ...
    HandleProcessedMessage(message, connectionType, bytesUnpacked,
                           handleMessagesFromConnection);
  }

  return RESULT_OK;
}

bool AlwaysHandleMessageTypeForConnection(
    ExternalInterface::MessageGameToEngine::Tag messageTag) {
  // Return true for small subset of message types that we handle even if we're
  // not listening to that connection We still want to accept certain message
  // types (e.g. console vars to allow a connection to enable itself)

  using GameToEngineTag = ExternalInterface::MessageGameToEngineTag;
  switch (messageTag) {
    case GameToEngineTag::SetDebugConsoleVarMessage:
      return true;
    case GameToEngineTag::GetDebugConsoleVarMessage:
      return true;
    case GameToEngineTag::GetAllDebugConsoleVarMessage:
      return true;
    default:
      return false;
  }
...
void UiMessageHandler::HandleProcessedMessage(
  ...
  const ExternalInterface::MessageGameToEngine::Tag messageTag =
      message.GetTag();
  if (!handleMessagesFromConnection) {
    // We still want to accept certain message types (e.g. console vars to allow
    // a connection to enable itself)
  ...
  // We must handle pings at this level because they are a connection type
  // specific message and must be dealt with at the transport level rather than
  // at the app level
  if (messageTag == ExternalInterface::MessageGameToEngineTag::Ping) {
    ...
    if (socketComms) {
      ...
      if (pingMsg.isResponse) {
        ...
        ExternalInterface::Ping outPing(pingMsg.counter, pingMsg.timeSent_ms,
                                        true);
...
void UiMessageHandler::Broadcast(
    const ExternalInterface::MessageGameToEngine& message) {
  ANKI_CPU_PROFILE(
      "UiMH::Broadcast_GToE");  // Some expensive and untracked - TODO: Capture
                                // message type for profile
...
void UiMessageHandler::BroadcastDeferred(
    const ExternalInterface::MessageGameToEngine& message) {
...
// Broadcasting MessageEngineToGame messages also delivers them out of the
// engine
void UiMessageHandler::Broadcast(
    const ExternalInterface::MessageEngineToGame& message) {
...
Signal::SmartHandle UiMessageHandler::Subscribe(
  ...
  if (_isInitialized) {
    ...
    for (UiConnectionType i = UiConnectionType(0); i < UiConnectionType::Count;
      ...
      if (socketComms) {
        ...
        const bool handleMessagesFromConnection =
            ShouldHandleMessagesFromConnection(i);
        while (keepReadingMessages) {
          std::vector<uint8_t> buffer;
          keepReadingMessages = socketComms->RecvMessage(buffer);

          if (keepReadingMessages) {
            Result res = ProcessMessageBytes(buffer.data(), buffer.size(), i,
                                             isSingleMessage,
                                             handleMessagesFromConnection);
            if (res != RESULT_OK) {
              retVal = RESULT_FAIL;
...
Result UiMessageHandler::Update() {
  ...
  const double currTime_ms =
      Util::Time::UniversalTime::GetCurrentTimeInMilliseconds();
  const bool sendPingThisTick =
      (kPingSendFreq_ms > 0.0) &&
      (currTime_ms - _lastPingTime_ms > kPingSendFreq_ms);
        ...
        if (deviceId == GetHostUiDeviceID()) {
          ...
          if (ConnectToUiDevice(deviceId, i)) {
            PRINT_CH_INFO("UiComms", "UiMessageHandler.Update.Connected",
                          "Automatically connected to local %s device %d!",
                          EnumToString(i), deviceId);
          } else {
            PRINT_NAMED_WARNING("UiMessageHandler.Update.FailedToConnect",
                                "Failed to connected to local %s device %d!",
                                EnumToString(i), deviceId);
          }
        } else {
          Broadcast(ExternalInterface::MessageEngineToGame(
              ExternalInterface::UiDeviceAvailable(i, deviceId)));
        }
      }
    }
  }
...
bool UiMessageHandler::ConnectToUiDevice(ISocketComms::DeviceId deviceId,
                                         UiConnectionType connectionType) {
  ISocketComms* socketComms = GetSocketComms(connectionType);

  const bool success =
      socketComms ? socketComms->ConnectToDeviceByID(deviceId) : false;

  std::array<uint8_t, 16> toGameCLADHash;
  std::copy(std::begin(messageEngineToGameHash),
            std::end(messageEngineToGameHash), std::begin(toGameCLADHash));

  std::array<uint8_t, 16> toEngineCLADHash;
  std::copy(std::begin(messageGameToEngineHash),
            std::end(messageGameToEngineHash), std::begin(toEngineCLADHash));

  // kReservedForTag is for future proofing - if we need to increase tag size to
  // 16 bits, the
  const uint8_t kReservedForTag = 0;
  ExternalInterface::UiDeviceConnected deviceConnected(
      kReservedForTag, connectionType, deviceId, success, toGameCLADHash,
      toEngineCLADHash, kBuildVersion);
  ...
  if (success) {
    ...
    Broadcast(ExternalInterface::MessageGameToEngine(
        ExternalInterface::RequestRobotSettings()));
  }

  return success;
}

void UiMessageHandler::HandleEvents(
  ...
  switch (event.GetData().GetTag()) {
    case ExternalInterface::MessageGameToEngineTag::
      ...
      const ExternalInterface::UiDeviceConnectionWrongVersion& msg =
          event.GetData().Get_UiDeviceConnectionWrongVersion();
    ...
    case ExternalInterface::MessageGameToEngineTag::ConnectToUiDevice: {
      const ExternalInterface::ConnectToUiDevice& msg =
          event.GetData().Get_ConnectToUiDevice();
      const ISocketComms::DeviceId deviceID = msg.deviceID;

      const bool success = ConnectToUiDevice(deviceID, msg.connectionType);
      if (success) {
        PRINT_CH_INFO("UiComms", "UiMessageHandler.HandleEvents",
                      "Connected to %s device %d!",
                      EnumToString(msg.connectionType), deviceID);
      } else {
        PRINT_NAMED_ERROR("UiMessageHandler.HandleEvents",
                          "Failed to connect to %s device %d!",
                          EnumToString(msg.connectionType), deviceID);
      }
    ...
    case ExternalInterface::MessageGameToEngineTag::DisconnectFromUiDevice: {
      const ExternalInterface::DisconnectFromUiDevice& msg =
          event.GetData().Get_DisconnectFromUiDevice();

      ISocketComms* socketComms = GetSocketComms(msg.connectionType);
      const ISocketComms::DeviceId deviceId = msg.deviceID;

      if (socketComms && socketComms->DisconnectDeviceByID(deviceId)) {
        PRINT_CH_INFO("UiComms", "UiMessageHandler.ProcessMessage",
                      "Disconnected from %s device %d!",
                      EnumToString(msg.connectionType), deviceId);
      }
...
const Util::Stats::StatsAccumulator& UiMessageHandler::GetLatencyStats(
    UiConnectionType type) const {
