 ...
 * Created: 05/14/16
 *
 * Description: UDP implementation for socket-based communications.  Used by
 *webots for Vector.
 *
...
UdpSocketComms::UdpSocketComms(UiConnectionType connectionType)
    : _comms(new MultiClientComms()), _advertisementService(nullptr) {
  std::string serviceName =
      std::string(EnumToString(connectionType)) + "AdvertisementService";
...
void UdpSocketComms::StartAdvertising(UiConnectionType connectionType) {
  assert((connectionType == UiConnectionType::UI) ||
         (connectionType == UiConnectionType::SdkOverUdp));
  ...
  PRINT_CH_INFO("UiComms", "UdpSocketComms::StartAdvertising",
                ...
                EnumToString(connectionType), registrationPort,
                advertisingPort);

  _advertisementService->StartService(registrationPort, advertisingPort);
}

bool UdpSocketComms::Init(UiConnectionType connectionType,
  ...
  if (hostIPValue.isString() && advertPortValue.isNumeric()) {
    Result retVal =
        _comms->Init(hostIPValue.asCString(), advertPortValue.asInt());

    if (retVal != RESULT_OK) {
      PRINT_NAMED_ERROR("UdpSocketComms.Init.InitComms",
                        "Failed to initialize %s Comms.",
                        EnumToString(connectionType));
      return false;
    }
  } else {
    if (isUI) {
      PRINT_NAMED_ERROR("UdpSocketComms.Init.MissingSettings",
                        "Missing advertising host IP / UI advertising port in "
                        "Json config file.");
      return false;
    }
  }
  ...
  if (!config.isMember(numDevicesKey)) {
    PRINT_NAMED_WARNING("UdpSocketComms.Init.NoNumDevices",
                        "No %s defined in Json config, defaulting to %d",
                        numDevicesKey, GetNumDesiredDevices());
  } else {
...
void UdpSocketComms::UpdateInternal() {
  ...
  // if this becomes an issue we could check if there are no recently active
  // connections
...
void UdpSocketComms::GetAdvertisingDeviceIDs(
