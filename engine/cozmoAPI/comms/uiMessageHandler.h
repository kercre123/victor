...
class UiMessageHandler : public IExternalInterface {
 public:
  UiMessageHandler(
      u32 hostUiDeviceID);  // Force construction with stuff in Init()?
  ...
  virtual void Broadcast(
      const ExternalInterface::MessageGameToEngine& message) override;
  virtual void Broadcast(
      ExternalInterface::MessageGameToEngine&& message) override;
  virtual void BroadcastDeferred(
      const ExternalInterface::MessageGameToEngine& message) override;
  virtual void BroadcastDeferred(
      ExternalInterface::MessageGameToEngine&& message) override;

  virtual void Broadcast(
      const ExternalInterface::MessageEngineToGame& message) override;
  virtual void Broadcast(
      ExternalInterface::MessageEngineToGame&& message) override;
  virtual void BroadcastDeferred(
      const ExternalInterface::MessageEngineToGame& message) override;
  virtual void BroadcastDeferred(
      ExternalInterface::MessageEngineToGame&& message) override;

  virtual Signal::SmartHandle Subscribe(
      const ExternalInterface::MessageEngineToGameTag& tagType,
      std::function<
          void(const AnkiEvent<ExternalInterface::MessageEngineToGame>&)>
          messageHandler) override;

  virtual Signal::SmartHandle Subscribe(
      const ExternalInterface::MessageGameToEngineTag& tagType,
      std::function<
          void(const AnkiEvent<ExternalInterface::MessageGameToEngine>&)>
          messageHandler) override;

  inline u32 GetHostUiDeviceID() const { return _hostUiDeviceID; }

  AnkiEventMgr<ExternalInterface::MessageEngineToGame>& GetEventMgrToGame() {
    return _eventMgrToGame;
  }
  AnkiEventMgr<ExternalInterface::MessageGameToEngine>& GetEventMgrToEngine() {
    return _eventMgrToEngine;
  }

  const Util::Stats::StatsAccumulator& GetLatencyStats(
      UiConnectionType type) const;

  bool HasDesiredNumUiDevices() const;

  virtual void SetSdkStatus(SdkStatusType statusType,
                            std::string&& statusText) override {
    _sdkStatus.SetStatus(statusType, std::move(statusText));
  }

  virtual uint32_t GetMessageCountGtE() const override {
    return _messageCountGameToEngine;
  }
  virtual uint32_t GetMessageCountEtG() const override {
    return _messageCountEngineToGame;
  }
  virtual void ResetMessageCounts() override {
 ...
 private:
  ...
  ISocketComms* GetSocketComms(UiConnectionType type) {
    return const_cast<ISocketComms*>(
        const_cast<const UiMessageHandler*>(this)->GetSocketComms(type));
  }
  ...
  Result ProcessMessageBytes(const uint8_t* packetBytes, size_t packetSize,
                             ...
                             bool handleMessagesFromConnection);
  void HandleProcessedMessage(
      const ExternalInterface::MessageGameToEngine& message,
      UiConnectionType connectionType, size_t messageSize,
      bool handleMessagesFromConnection);

  // Send a message to a specified ID
  virtual void DeliverToGame(
      const ExternalInterface::MessageEngineToGame& message,
      DestinationId = kDestinationIdEveryone) override;

  bool ConnectToUiDevice(ISocketComms::DeviceId deviceId,
                         UiConnectionType connectionType);
  void HandleEvents(
  ...
  UiConnectionType _connectionSource =
      UiConnectionType::Count;  // Connection source currently processing
                                // messages
