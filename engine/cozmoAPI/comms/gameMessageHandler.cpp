...
Result GameMessageHandler::Init(Comms::IComms* comms) {
  ...
  if (comms_) {
    ...
    if (isInitialized_ == false) {
      // TODO: PRINT_NAMED_ERROR("MessageHandler", "Unable to initialize
      // comms!");
      retVal = RESULT_OK;
    }
  }
...
bool GameMessageHandler::IsInitialized() const { return isInitialized_; }

Result GameMessageHandler::SendMessage(
    const UserDeviceID_t devID,
...
Result GameMessageHandler::ProcessPacket(const std::vector<uint8_t>& buffer) {
  ...
  if (message.Unpack(buffer.data(), buffer.size()) != buffer.size()) {
    PRINT_STREAM_ERROR(
        "GameMessageHandler.MessageBufferWrongSize",
        "Buffer's size does not match expected size for this message ID. (Msg "
            << ExternalInterface::MessageEngineToGameTagToString(
                   message.GetTag())
            << ", expected " << message.Size() << ", recvd " << buffer.size()
            << ")");
  }
