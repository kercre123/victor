 ...
 *              Whatever tracks are locked by the action will stay locked while
 *the start and loop animations but the tracks will be unlocked while the end
 *animation plays The end animation will always play and will cancel the
 *start/loop animations if needed
 *
 *
...
class DrivingAnimationHandler
 ...
 public:
  ...
  // Returns true if any driving animation is playing or just finished
  // (start/loop/end/finished)
  bool InDrivingAnimsState() const;
  // Returns true if any planning animation is playing or just finished
  // (start/loop/end/finished)
  ...
  bool IsPlayingDrivingEndAnim() const {
    return _state == AnimState::DrivingEnd;
  }

  // Returns true if the drivingEnd animation has finished.
  // Once this is true, action's CheckIfDone can return return a non-running
  // ActionResult.
  bool HasFinishedDrivingEndAnim() const {
    return (_state == AnimState::FinishedDriving);
  }

  bool HasFinishedPlanningEndAnim() const {
    return (_state == AnimState::FinishedPlanning);
  }

  // Takes in the tag of the action that is calling this and whether or not it
  // is suppressing track locking If keepLoopingWithoutPath is false, endAnim is
  // played automatically once no path is being followed. If true, then calling
  // action must call EndDrivingAnim.
  void Init(const u8 tracksToUnlock, const u32 tag,
  ...
  // Cancels drivingStart and drivingLoop animations and starts playing
  // drivingEnd animation
  bool EndDrivingAnim();
 ...
 private:
  // Listens for driving animations to complete and handles what animation to
  // play next
  ...
  enum class AnimState {
    ...
    FinishedPlanning,  // End planning anim has finished but driving anims
                       // haven't started yet
    ...
    FinishedDriving,   // End anim has finished but the action hasn't been
                       // destroyed yet
    ActionDestroyed,   // The action has been destroyed so we are waiting for
                       // Init() to be called
  };
