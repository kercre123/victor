 ...
 * Date:
 * Description:   A service with which devices can register at the registration
 * port if they want to advertise to presence to others. Listener devices can
 * connect to the advertisementPort if they want to see advertising devices.
 *
 * Author:        Kevin Yoon
 */
...
AdvertisementService::AdvertisementService(const char* serviceName,
                                           RegMsgTag regMsgTag)
    : regServer_("regServer"), advertisingServer_("advertisingServer") {
  if (regMsgTag == kInvalidRegMsgTag) {
    using EMessageTag = Vector::ExternalInterface::MessageGameToEngineTag;
    static_assert(sizeof(EMessageTag) == sizeof(RegMsgTag),
                  "Robot and Game tag size must match");
    _regMsgTag = Util::numeric_cast<RegMsgTag>(
        EMessageTag::AdvertisementRegistrationMsg);
  } else {
...
void AdvertisementService::StartService(int registrationPort,
...
void AdvertisementService::Update() {
  ...
  const size_t kMinAdRegMsgSize =
      sizeof(RegMsgTag) +
      regMsg.Size();  // Size of message with an empty ip string
    ...
    if (bytes_recvd >= kMinAdRegMsgSize) {
      ...
      if (messageTag == _regMsgTag) {
        ...
        if (bytesUnpacked == innerMessageSize) {
          ...
          PRINT_NAMED_WARNING("AdvertisementService.Recv.ErrorUnpacking",
        ...
        PRINT_NAMED_WARNING(
            "AdvertisementService.Recv.BadTag",
            "%s: Received %d byte message with tag %u('%s') when expected "
            "%u('%s')\n%s",
            serviceName_, bytes_recvd, (int)messageTag,
            CoEx::MessageGameToEngineTagToString((GToETag)messageTag),
            (int)_regMsgTag,
            CoEx::MessageGameToEngineTagToString((GToETag)_regMsgTag),
            Util::ConvertMessageBufferToString(messageData, bytes_recvd,
                                               Util::eBTTT_Ascii)
                .c_str());
...
#if DEBUG_AD_SERVICE
      PRINT_NAMED_WARNING(
          "AdvertisementService.Recv.AdRegTooSmall",
  ...
  do {
    ...
    uint8_t messageData[64];
    bytes_recvd =
        advertisingServer_.Recv((char*)messageData, sizeof(messageData));

    // if (bytes_recvd > 0) {
    //  std::cout << serviceName_ << ": " << "Received ping from advertisement
    //  listener\n";
    //}
  } while (bytes_recvd > 0);

  // Notify all clients of advertising devices
  if (advertisingServer_.GetNumClients() > 0 &&
      (!connectionInfoMap_.empty() ||
       !oneShotAdvertiseConnectionInfoMap_.empty())) {
    ...
    for (int mapType = 0; mapType < 2; ++mapType) {
      const ConnectionInfoMap& connectionMap =
          (mapType == 0) ? connectionInfoMap_
                         : oneShotAdvertiseConnectionInfoMap_;

      for (const auto& kv : connectionMap) {
        const Vector::AdvertisementMsg& adMsg = kv.second;
        Anki::Vector::ExternalInterface::MessageGameToEngine
            message;  // We pretend that this came directly from Game
        message.Set_AdvertisementMsg(adMsg);

        PRINT_NAMED_INFO("AdvertisementService.NotifyClients",
                         "%s: Sending %s Advertisement: Device %d on host %s "
                         "at ports ToEngine: %d FromEngine: %d",
                         serviceName_,
                         (mapType == 0) ? "Connected" : "One-shot",
                         (int)adMsg.id, adMsg.ip.c_str(),
                         (int)adMsg.toEnginePort, (int)adMsg.fromEnginePort);
...
void AdvertisementService::ProcessRegistrationMsg(
    const Vector::AdvertisementRegistrationMsg& regMsg) {
  if (regMsg.enableAdvertisement) {
    PRINT_NAMED_INFO(regMsg.oneShot ? "ProcessRegistrationMsg.ReceivedOneShot"
                                    : "ProcessRegistrationMsg.ReceivedRegReq",
                     "%s: Received from device %d on host %s at ports "
                     "ToEngine: %d FromEngine: %d with advertisement service",
                     serviceName_, (int)regMsg.id, regMsg.ip.c_str(),
                     (int)regMsg.toEnginePort, (int)regMsg.fromEnginePort);

    Vector::AdvertisementMsg& destMsg =
        regMsg.oneShot ? oneShotAdvertiseConnectionInfoMap_[regMsg.id]
                       : connectionInfoMap_[regMsg.id];
