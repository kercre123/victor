...
// Receives AdvertisementRegistrationMsg CLAD messages from devices at port(s)
// that wants to advertise. If enableAdvertisement == 1 and oneShot == 0, the
// device is registered to the service which will then advertise for the device
// on subsequent calls to Update(). If enableAdvertisement == 1 and oneShot ==
// 1, the service will advertise one time on the next call to Update(). This
// mode is helpful in that an advertising device need not know whether an
// advertisement service is running before it sends a registration message. It
// just keeps sending them! If enableAdvertisement == 0, the device is
// deregistered if it isn't already.
//
// It then tracks and sends on AdvertisementMsg CLAD messages based on the
// RegistrationMessages to all clients interested in knowing about advertising
// devices on given port(s).
...
class AdvertisementService {
 public:
  ...
  explicit AdvertisementService(const char* serviceName,
                                RegMsgTag regMsgTag = kInvalidRegMsgTag);
  ...
  // Stops listening for clients and clears all registered advertisers and
  // advertisement listeners.
  void StopService();

  // This needs to be called at the frequency you want to accept registrations
  // and advertise.
  // TODO: Perhaps StartService() should launch a thread to just do this
  // internally.
  void Update();
