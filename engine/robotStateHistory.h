...
class HistRobotState {
 public:
  ...
  // Only update pose-related information: includes pose frame ID, body pose in
  // the world, and head/lift angles
  void SetPose(const PoseFrameID_t frameID, const Pose3d& pose,
  ...
  // TODO: remove this once `_pose` actually contains full 3d orientation
  // (currently it only includes yaw)
  ...
  //            its own history buffer and keep HistRobotState as a container
  //            for raw unprocessed states (i.e. RobotState) only.
  void SetProxSensorData(const ProxSensorData& data) { _proxData = data; }

  bool WasCarryingObject() const {
    return (_state.status &
            Util::EnumToUnderlying(RobotStatusFlag::IS_CARRYING_BLOCK));
  }
  bool WasMoving() const {
    return (_state.status & Util::EnumToUnderlying(RobotStatusFlag::IS_MOVING));
  }
  bool WasHeadMoving() const {
    return !(_state.status &
             Util::EnumToUnderlying(RobotStatusFlag::HEAD_IN_POS));
  }
  bool WasLiftMoving() const {
    return !(_state.status &
             Util::EnumToUnderlying(RobotStatusFlag::LIFT_IN_POS));
  }
  bool WereWheelsMoving() const {
    return (_state.status &
            Util::EnumToUnderlying(RobotStatusFlag::ARE_WHEELS_MOVING));
  }
  bool WasPickedUp() const {
    return (_state.status &
            Util::EnumToUnderlying(RobotStatusFlag::IS_PICKED_UP));
  }
  bool WasCameraMoving() const {
    return (WasHeadMoving() || WereWheelsMoving());
  }
  bool WasCliffDetected(CliffSensor sensor) const;

  // Returns a new HistRobotState the given fraction between 1 and 2, where
  // fraction is [0,1]. Note: always uses histState1's PoseFrameID
  static HistRobotState Interpolate(const HistRobotState& histState1,
 ...
 private:
  // Note that the _state.pose is not guaranteed to match what's in _pose
  // (COZMO-10225)
 ...
 * raw odometry based poses from the robot and vision-based poses computed by
 * Blockworld. Can also be used to check robot state (e.g. whether carrying an
 * object) at a historical time.
 *
 */
class RobotStateHistory : public IDependencyManagedComponent<RobotComponentID> {
 public:
  ...
  // Returns the number of states that were added via AddRawOdomState() that
  // still remain in history
  size_t GetNumRawStates() const { return _states.size(); }

  // Returns the number of states that were added via AddVisionOnlyState() that
  // still remain in history
  ...
  Result AddVisionOnlyState(const RobotTimeStamp_t t,
                            const HistRobotState& state);

  // Sets p to the raw odometry state nearest the given timestamp t in the
  // history. Interpolates state if withInterpolation == true. Returns OK if t
  // is between the oldest and most recent timestamps stored.
  Result GetRawStateAt(const RobotTimeStamp_t t_request, RobotTimeStamp_t& t,
  ...
  //            its own history buffer and keep HistRobotState as a container
  //            for raw unprocessed states (i.e. RobotState) only.
  Result UpdateProxSensorData(const RobotTimeStamp_t t,
                              const ProxSensorData& data);

  // Returns OK and points statePtr to a vision-based state at the specified
  // time if such a state exists. Note: The state that statePtr points to may be
  // invalidated by subsequent calls to
  //       the history like Clear() or Add...(). Use carefully!
  Result GetVisionOnlyStateAt(const RobotTimeStamp_t t_request,
  ...
  Result ComputeStateAt(const RobotTimeStamp_t t_request, RobotTimeStamp_t& t,
                        HistRobotState& state,
                        bool withInterpolation = false) const;
  ...
  // Points *statePtr to a computed state in the history that was insert via
  // ComputeAndInsertStateAt
  Result GetComputedStateAt(const RobotTimeStamp_t t_request,
                            HistRobotState** statePtr,
                            HistStateKey* key = nullptr);

  // NOTE: p is not const, but *statePtr is. So you can assign to *statePtr, or
  // statePtr, but not **statePtr
  Result GetComputedStateAt(const RobotTimeStamp_t t_request,
                            const HistRobotState** statePtr,
                            HistStateKey* key = nullptr) const;

  // If at least one vision only state exists, the most recent one is returned
  // in p and the time it occured at in t.
  Result GetLatestVisionOnlyState(RobotTimeStamp_t& t,
                                  HistRobotState& state) const;

  // Get the last state in history with the given pose frame ID.
  Result GetLastStateWithFrameID(const PoseFrameID_t frameID,
                                 HistRobotState& state) const;
  ...
  // Checks whether or not the given key is associated with a valid computed
  // state
