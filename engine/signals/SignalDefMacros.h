...
// Explicitly undefine so we can redefine without warnings from previous
// includes
#undef DEF_SIGNAL
...
//
//  static Signal::Signal<void (RobotID_t robotID, bool successful)>&
//  GetRobotConnectSignal();
//
...
//      CozmoEngineSignals::GetRobotConnectSignal().ScopedSubscribe(cbRobotConnectSignal));
//
//      where _signalHandles is a container for the SmartHandle that
//      ScopedSubscribe() returns. In this case, _signalHandles should be a
//      member of the same class as HandleRobotConnectSignal(). See
//      simpleSignal.hpp for more details.
//
...
//
//  static Signal::Signal<void (RobotID_t robotID, bool successful)>
//  _RobotConnectSignal; Signal::Signal<void (RobotID_t robotID, bool
//  successful)>& CozmoEngineSignals::GetRobotConnectSignal() { return
//  _RobotConnectSignal; }
//
...
#define DEF_SIGNAL(__SIGNAME__, ...)                               \
  ...
  Signal::Signal<void(__VA_ARGS__)>&                               \
      SIGNAL_CLASS_NAME::__SIGNAME__##Signal() {                   \
    return _##__SIGNAME__##Signal;                                 \
  }
