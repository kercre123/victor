...
class RobotTest {
 public:
  ...
  void Init(Util::Data::DataPlatform* dataPlatform,
  ...
  void ExecuteWebCommandRun(const std::string& scriptName,
  ...
  void ExecuteWebCommandGetScript(const std::string& scriptName,
                                  std::string* resultStr);
 ...
 private:
  ...
  bool StringToScriptCommand(const std::string& commandStr,
  ...
  struct RobotTestScript {
    std::string _name;
    bool _wasUploaded = false;  // Was this script uploaded to persistent
                                // folder? (Otherwise found in resources)
