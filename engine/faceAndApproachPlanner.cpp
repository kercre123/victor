...
#define FACE_AND_APPROACH_LENGTH_SQUARED_THRESHOLD \
  FACE_AND_APPROACH_LENGTH_THRESHOLD* FACE_AND_APPROACH_LENGTH_THRESHOLD
...
EComputePathStatus FaceAndApproachPlanner::ComputePath(
    const Pose3d& startPose, const Pose3d& targetPose) {
...
EComputePathStatus FaceAndApproachPlanner::ComputeNewPathIfNeeded(
  ...
  // If a straight segment ends up being unnecessary then this is just the start
  // angle.
  ...
  float distSquared =
      pow(target2d.x() - start2d.x(), 2) + pow(target2d.y() - start2d.y(), 2);
  if (distSquared > FACE_AND_APPROACH_LENGTH_SQUARED_THRESHOLD) {
    LOG_INFO("FaceAndApproachPlanner.Straight",
             "doing straight because distance^2 of %f > %f", distSquared,
             FACE_AND_APPROACH_LENGTH_SQUARED_THRESHOLD);
    doStraight = true;

    // If doing a straight then, the target angle is the approach angle to the
    // target point.
    intermediateTargetAngle =
        atan2(_targetVec.y() - startVec.y(), _targetVec.x() - startVec.x());
  }
  ...
  if (std::abs(deltaTheta1) > FACE_AND_APPROACH_THETA_THRESHOLD) {
    LOG_INFO("FaceAndApproachPlanner.FinalTurn",
             "doing final turn because delta theta of %f > %f", deltaTheta1,
  ...
  if (doStraight && std::abs(deltaTheta) > FACE_AND_APPROACH_THETA_THRESHOLD) {
    LOG_INFO("FaceAndApproachPlanner.InitialTurn",
             "doing initial turn because delta theta of %f > %f", deltaTheta,
             FACE_AND_APPROACH_THETA_THRESHOLD);
    doTurn0 = true;
  }
  ...
  if (doTurn0) {
    if (std::abs(deltaTheta) > FACE_AND_APPRACH_DELTA_THETA_FOR_BACKUP) {
      printf(
          "FaceAndApproachPlanner: deltaTheta of %f above threshold, doing "
          "backup!\n",
          deltaTheta);
    ...
    _path.AppendPointTurn(startVec.x(), startVec.y(), currAngle.ToFloat(),
                          intermediateTargetAngle.ToFloat(),
                          deltaTheta < 0 ? -FACE_AND_APPROACH_TARGET_ROT_SPEED
                                         : FACE_AND_APPROACH_TARGET_ROT_SPEED,
  ...
  if (doStraight) {
    _path.AppendLine(startVec.x(), startVec.y(), _targetVec.x(), _targetVec.y(),
                     backup ? -FACE_AND_APPROACH_TARGET_SPEED
  ...
  if (doTurn1) {
    _path.AppendPointTurn(_targetVec.x(), _targetVec.y(),
                          intermediateTargetAngle.ToFloat(), _finalTargetAngle,
                          deltaTheta1 < 0 ? -FACE_AND_APPROACH_TARGET_ROT_SPEED
                                          : FACE_AND_APPROACH_TARGET_ROT_SPEED,
                          FACE_AND_APPROACH_PLANNER_ROT_ACCEL,
                          FACE_AND_APPROACH_PLANNER_ROT_DECEL,
                          POINT_TURN_ANGLE_TOL, true);
