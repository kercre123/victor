...
class EncodedImage {
 public:
  EncodedImage();
  // Create an "encoded" image from an existing image. Copies data from the
  // image into the EncodedImage's buffer. ImageEncoding will be RawGray or
  // RawRGB.
  explicit EncodedImage(const Vision::Image& imgGray,
                        const u32 imageID = std::numeric_limits<u32>::max());
  ...
  void SetPrevTimestamp(const RobotTimeStamp_t timestamp) {
 ...
 private:
  ...
  static void MiniGrayToJpeg(const std::vector<u8>& bufferIn, const u16 height,
                             const u16 width, std::vector<u8>& bufferOut);

  static void MiniColorToJpeg(const std::vector<u8>& bufferIn, const u16 height,
                              const u16 width, std::vector<u8>& bufferOut);

  static void MiniToJpegHelper(const std::vector<u8>& bufferIn,
                               const u16 height, const u16 width,
                               std::vector<u8>& bufferOut, const u8* header,
