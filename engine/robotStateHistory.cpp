...
HistRobotState::HistRobotState() : _state(Robot::GetDefaultRobotState()) {}
...
void HistRobotState::ClearPoseParent() { _pose.ClearParent(); }
...
const u16 HistRobotState::GetCliffData(CliffSensor sensor) const {
  DEV_ASSERT(sensor < CliffSensor::CLIFF_COUNT,
             "HistRobotState.GetCliffData.InvalidIndex");
  return _state.cliffDataRaw[Util::EnumToUnderlying(sensor)];
}

void HistRobotState::Print() const {
  printf(
      "Frame %d, headAng %f, cliff %d %d %d %d, carrying %s, moving %s, "
      "whichMoving [%s%s%s]",
      GetFrameId(), GetHeadAngle_rad(), GetCliffData(CliffSensor::CLIFF_FL),
      GetCliffData(CliffSensor::CLIFF_FR), GetCliffData(CliffSensor::CLIFF_BL),
      GetCliffData(CliffSensor::CLIFF_BR), WasCarryingObject() ? "Y" : "N",
      WasMoving() ? "Y" : "N", WasHeadMoving() ? "H" : "",
      WasLiftMoving() ? "L" : "", WereWheelsMoving() ? "B" : "");
  _pose.Print();
}

bool HistRobotState::WasCliffDetected(CliffSensor sensor) const {
  DEV_ASSERT(sensor < CliffSensor::CLIFF_COUNT,
             "HistRobotState.WasCliffDetected.InvalidIndex");
  return _cliffDetectedFlags.IsBitFlagSet(sensor);
}

HistRobotState HistRobotState::Interpolate(const HistRobotState& histState1,
                                           const HistRobotState& histState2,
                                           const Pose3d& pose2wrtPose1,
                                           f32 fraction) {
  ...
  const HistRobotState* closestHistRobotState =
      isCloserToFirst ? &histState1 : &histState2;
  ...
  // Interp head angle
  interpState.headAngle = histState1.GetHeadAngle_rad() +
                          fraction * (histState2.GetHeadAngle_rad() -
                                      histState1.GetHeadAngle_rad());

  // Interp lift angle
  interpState.liftAngle = histState1.GetLiftAngle_rad() +
                          fraction * (histState2.GetLiftAngle_rad() -
                                      histState1.GetLiftAngle_rad());

  // Interp pitch
  interpState.pose.pitch_angle =
      histState1.GetPitch_rad() +
      fraction * (histState2.GetPitch_rad() - histState1.GetPitch_rad());

  // Interp cliff data
  for (int i = 0; i < interpState.cliffDataRaw.size(); i++) {
    const auto s = static_cast<CliffSensor>(i);
    interpState.cliffDataRaw[i] =
        std::round(f32(histState1.GetCliffData(s)) +
                   fraction * f32(histState2.GetCliffData(s) -
                                  histState1.GetCliffData(s)));
  }

  // Interp wheel speeds
  interpState.lwheel_speed_mmps =
      histState1.GetLeftWheelSpeed_mmps() +
      fraction * (histState2.GetLeftWheelSpeed_mmps() -
                  histState1.GetLeftWheelSpeed_mmps());
  interpState.rwheel_speed_mmps =
      histState1.GetRightWheelSpeed_mmps() +
      fraction * (histState2.GetRightWheelSpeed_mmps() -
                  histState1.GetRightWheelSpeed_mmps());

  // Interp prox data
  // Only interpolating the ProxSensorData struct instead of the
  // ProxSensorDataRaw struct ProxSensorDataRaw struct in RobotState. If there's
  // a use case for exposing the raw data we should interpolate here. Yes,
  // there's some data duplication for convenience of having all the useful
  // stuff in ProxSensorData.
  ProxSensorData interpProxData =
      closestHistRobotState
          ->GetProxSensorData();  // Full copy to take care of the bools
  interpProxData.distance_mm =
      std::round(f32(histState1.GetProxSensorData().distance_mm) +
                 fraction * f32(histState2.GetProxSensorData().distance_mm -
                                histState1.GetProxSensorData().distance_mm));
  interpProxData.signalQuality =
      histState1.GetProxSensorData().signalQuality +
      fraction * (histState2.GetProxSensorData().signalQuality -
                  histState1.GetProxSensorData().signalQuality);
  ...
  const Radians interpRotation =
      histState1.GetPose().GetRotationAngle<'Z'>() +
      Radians(pose2wrtPose1.GetRotationAngle<'Z'>() * fraction);
  const Pose3d interpPose(interpRotation, Z_AXIS_3D(), interpTrans,
                          histState1.GetPose().GetParent());

  //
  // Interpolate booleans
  //
  interpState.cliffDetectedFlags =
      closestHistRobotState->_cliffDetectedFlags.GetFlags();

  // Construct interpolated HistRobotState to return
  HistRobotState interpHistState(interpPose, interpState, interpProxData);
...
Result RobotStateHistory::AddRawOdomState(const RobotTimeStamp_t t,
  ...
  if (!_states.empty()) {
    ...
    if (newestTime > _windowSize_ms && t < newestTime - _windowSize_ms) {
      LOG_WARNING("RobotStateHistory.AddRawOdomState.TimeTooOld",
                  "newestTime %u, oldestAllowedTime %u, t %u",
                  (TimeStamp_t)newestTime,
                  (TimeStamp_t)(newestTime - _windowSize_ms), (TimeStamp_t)t);
      return RESULT_FAIL;
    }
  }
  ...
  if (!res.second) {
    LOG_WARNING("RobotStateHistory.AddRawOdomState.AddFailed", "Time: %u",
                (TimeStamp_t)t);
    return RESULT_FAIL;
  }
...
Result RobotStateHistory::AddVisionOnlyState(const RobotTimeStamp_t t,
  ...
  if (it != _visStates.end()) {
    ...
    if (ANKI_DEV_CHEATS) {
      ...
      LOG_INFO("RobotStateHistory.AddVisionOnlyState.Overwriting", "%s",
               ss.str().c_str());
    ...
    if (!res.second) {
      LOG_ERROR("RobotStateHistory.AddVisionOnlyState.EmplaceFailed",
                "Emplace of pose with t=%d, frameID=%d failed", (TimeStamp_t)t,
                state.GetFrameId());
      return RESULT_FAIL;
    }
...
Result RobotStateHistory::GetRawStateBeforeAndAfter(
    const RobotTimeStamp_t t, RobotTimeStamp_t& t_before,
    HistRobotState& state_before, RobotTimeStamp_t& t_after,
    HistRobotState& state_after) {
  // Get the iterator for time t
  const_StateMapIter_t it = _states.lower_bound(t);

  if (it == _states.begin() || it == _states.end() ||
      t < _states.begin()->first) {
    return RESULT_FAIL;
  }
  ...
  if (t_request == it->first) {
    ...
    if (it->second.GetFrameId() != prev_it->second.GetFrameId()) {
      LOG_INFO("RobotStateHistory.GetRawStateAt.MisMatchedFrameIds",
               "Cannot interpolate at t=%u as requested because the two frame "
               "IDs don't match: prev=%d vs next=%d",
               (TimeStamp_t)t_request, prev_it->second.GetFrameId(),
               it->second.GetFrameId());

      // they asked us for a t_request that is between two frame IDs, which for
      // all intents and purposes is just as bad as trying to choose between two
      // poses with mismatched origins (like above).
    ...
    if (withInterpolation) {
      ...
      // We don't need to check return value (bool) here because we've
      // effectively checked it already in the call to HasSameRootAs above
      Pose3d pTransform;
      inSameOrigin = it->second.GetPose().GetWithRespectTo(
          prev_it->second.GetPose(), pTransform);

      if (inSameOrigin) {
        // Compute scale factor between time to previous pose and time between
        // previous pose and next pose.
        const f32 timeScale = (f32)(t_request - prev_it->first) /
                              TimeStamp_t(it->first - prev_it->first);

        state = HistRobotState::Interpolate(prev_it->second, it->second,
                                            pTransform, timeScale);

        t = t_request;
      }
    } else {
      inSameOrigin =
          it->second.GetPose().HasSameRootAs(prev_it->second.GetPose());
    ...
    if (!inSameOrigin) {
      LOG_INFO("RobotStateHistory.GetRawStateAt.MisMatchedOrigins",
               "Cannot interpolate at t=%u as requested because the two poses "
               "don't share the same origin: prev=%s vs next=%s",
               (TimeStamp_t)t_request,
               prev_it->second.GetPose().FindRoot().GetName().c_str(),
               it->second.GetPose().FindRoot().GetName().c_str());

      // they asked us for a t_request that is between two origins. We can't
      // interpolate or decide which origin is "right" for you, so, we are going
      // to fail
      return RESULT_FAIL_ORIGIN_MISMATCH;
    }
  }
...
Result RobotStateHistory::UpdateProxSensorData(const RobotTimeStamp_t t,
                                               const ProxSensorData& data) {
...
Result RobotStateHistory::GetVisionOnlyStateAt(const RobotTimeStamp_t t_request,
                                               HistRobotState** state) {
...
Result RobotStateHistory::ComputeStateAt(const RobotTimeStamp_t t_request,
  ...
  const Result getRawResult =
      GetRawStateAt(t_request, t, state1, withInterpolation);
  ...
  // If there are no vision-based poses then return the raw pose that we just
  // got
  if (git == _visStates.end()) {
    ...
    // If this is the first vision-based pose then return the raw pose that we
    // got
  ...
  // Now get the pose in _states that immediately follows the vision-based
  // pose's time.
  ...
  // Compute the total transformation taking us from the vision-only pose at
  // time corresponding to p0, forward to p1. We will be applying this
  // transformation to whatever is stored in the vision-only pose below.
  Pose3d pTransform;
  if (p0_it->second.GetFrameId() == state1.GetFrameId()) {
    // Special case: no intermediate frames to chain through. The total
    // transformation is just going from p0 to p1.
    Pose3d p1_wrt_p0_parent;
    const bool inSameOrigin = state1.GetPose().GetWithRespectTo(
        p0_it->second.GetPose().GetParent(), pTransform);
    ...
    for (pMid1 = p0_it; pMid1 != _states.end(); ++pMid1) {
      ...
      if (pMid1->second.GetFrameId() > pMid0->second.GetFrameId()) {
        // pMid1 is now pointing to the first pose in the next frame after
        // pMid0. Point pMid1 to the last pose of the same frame as pMid0.
        // Compute the transform for this frame (from pMid0 to pMid1) and fold
        // it into the running total stored in pTransform.

        // We expect the beginning (pMid0) and end (pMid1) of this part of
        // history to have the same frame ID and origin.
        --pMid1;  // (temporarily) move back to last pose in same frame as pMid0
        DEV_ASSERT(
            pMid0->second.GetFrameId() == pMid1->second.GetFrameId(),
            "RobotStateHistory.ComputeStateAt.MismatchedIntermediateFrameIDs");
        DEV_ASSERT(
            pMid0->second.GetPose().HasSameRootAs(pMid1->second.GetPose()),
            "RobotStateHistory.ComputeStateAt.MismatchedIntermediateOrigins");

        // Get pMid0 and pMid1 w.r.t. the same parent and store in the
        // intermediate transformation pMidTransform, which is going to hold the
        // transformation from pMid0 to pMid1
        Pose3d pMidTransform;
        const bool inSameOrigin = pMid1->second.GetPose().GetWithRespectTo(
            pMid0->second.GetPose().GetParent(), pMidTransform);
        DEV_ASSERT(inSameOrigin,
                   "RobotStateHistory.ComputeStateAt.FailedGetWRT2");

        // pMidTransform = pMid1 * pMid0^(-1)
        pMidTransform *= pMid0->second.GetPose().GetInverse();

        // Fold the transformation from pMid0 to pMid1 into the total
  ...
  // NOTE: We are about to return p, which is a transformed version of the
  // vision-only pose in "git", so it should still be relative to whatever "git"
  // was relative to.
  pTransform *=
      git->second.GetPose();  // Apply pTransform to git and store in pTransform
  pTransform.SetParent(git->second.GetPose().GetParent());  // Keep git's parent
  state.SetPose(state1.GetFrameId(), pTransform, state1.GetHeadAngle_rad(),
                state1.GetLiftAngle_rad());

  return RESULT_OK;
}

Result RobotStateHistory::ComputeAndInsertStateAt(
    const RobotTimeStamp_t t_request, RobotTimeStamp_t& t,
    HistRobotState** state, HistStateKey* key, bool withInterpolation) {
  HistRobotState state_computed;
  // printf("COMPUTE+INSERT\n");
  const Result computeResult =
      ComputeStateAt(t_request, t, state_computed, withInterpolation);
  ...
  if (it != _computedStates.end()) {
    ...
    *state = &(res.first->second);
    ...
    _keyByTsMap.emplace(std::piecewise_construct, std::forward_as_tuple(t),
                        std::forward_as_tuple(currHistStateKey_));
...
// TODO:(bn) is there a way to avoid this duplicated code here? Not eager to use
// templates...
Result RobotStateHistory::GetComputedStateAt(const RobotTimeStamp_t t_request,
                                             HistRobotState** state,
                                             HistStateKey* key) {
  return GetComputedStateAt(t_request,
                            const_cast<const HistRobotState**>(state), key);
}

Result RobotStateHistory::GetLatestVisionOnlyState(
    RobotTimeStamp_t& t, HistRobotState& state) const {
  if (!_visStates.empty()) {
...
Result RobotStateHistory::GetLastStateWithFrameID(const PoseFrameID_t frameID,
                                                  HistRobotState& state) const {
  ...
  if (_states.empty()) {
    LOG_INFO(
        "RobotStateHistory.GetLastStateWithFrameID.EmptyHistory",
  ...
  for (poseIter = _states.rbegin();
       poseIter != _states.rend() && poseIter->second.GetFrameId() >= frameID;
       ++poseIter) {
  ...
  // NOTE: this second loop over vision poses will only occur if found is still
  // false, meaning we didn't find a pose already in the first loop.
  if (!found) {
    for (poseIter = _visStates.rbegin();
         poseIter != _visStates.rend() &&
         poseIter->second.GetFrameId() >= frameID;
         ++poseIter) {
      if (poseIter->second.GetFrameId() == frameID) {
        found = true;
        break;
  ...
  if (found) {
    ...
    LOG_INFO(
        "RobotStateHistory.GetLastStateWithFrameID.FrameIdNotFound",
        ...
        frameID, _states.begin()->second.GetFrameId(),
...
u32 RobotStateHistory::GetNumRawStatesWithFrameID(
    const PoseFrameID_t frameID) const {
  // First look through "raw" poses for the frame ID. We don't need to look
  // any further once the frameID drops below the one we are looking for,
  // because they are ordered
  u32 cnt = 0;
  for (auto poseIter = _states.rbegin(); poseIter != _states.rend();
       ++poseIter) {
...
void RobotStateHistory::Print() const {
  ...
  std::multimap<TimeStamp_t, std::pair<std::string, const_StateMapIter_t> >
      mergedPoses;
  std::multimap<TimeStamp_t,
                std::pair<std::string, const_StateMapIter_t> >::iterator
      mergedIt;
  ...
  printf("\nRobotStateHistory\n");
  printf("================\n");
  for (mergedIt = mergedPoses.begin(); mergedIt != mergedPoses.end();
       ++mergedIt) {
