 ...
 * Created: 10/30/15
 *
 * Description: Helper for ExternalInterface::MessageGameToEngine handling in
 your class. Add a public

 template<typename T>
 void HandleMessage(const T& msg);

 to your header. Then, in class, add specializations for each message type to
 handle like so:
...
class AnkiEventUtil {
 public:
  ...
  void SubscribeGameToEngine() {
    T& temp = _object;
    _eventHandlers.push_back(_interface.Subscribe(
        Tag,
        [&temp](
...
AnkiEventUtil<TNew, HNew> MakeAnkiEventUtil(
    IExternalInterface& externalInterface, TNew& object, HNew& handlersIn) {
