 ...
 * Created: 09/22/16
 *
 * Description: Container for chunked uploads for the SDK uploading animation
 *files at runtime
 *
...
const std::string AnimationTransfer::kCacheFaceAnimsDir(
    Anki::Util::FileUtils::FullFilePath({"assets", "faceAnimations"}));

AnimationTransfer::AnimationTransfer(
    Anki::Vector::IExternalInterface *externalInterface,
    Anki::Util::Data::DataPlatform *dataPlatform) {
  _externalInterface = externalInterface;
  _dataPlatform = dataPlatform;

  auto callback = std::bind(&AnimationTransfer::HandleGameEvents, this,
                            std::placeholders::_1);
  _signalHandle = _externalInterface->Subscribe(
      ExternalInterface::MessageGameToEngineTag::TransferFile, callback);
  CleanUp();
}

// Clean up the temp resources
AnimationTransfer::~AnimationTransfer() { CleanUp(); }

void AnimationTransfer::CleanUp(bool removeFaceImgDir) {
  std::string full_path = _dataPlatform->pathToResource(
      Anki::Util::Data::Scope::Cache, kCacheAnimFileName);
  ...
  if (removeFaceImgDir && !_lastFaceAnimDir.empty()) {
    std::string face_anim_dir = _dataPlatform->pathToResource(
        Anki::Util::Data::Scope::Cache, kCacheFaceAnimsDir);
    std::string full_path_face_imgs =
        Anki::Util::FileUtils::FullFilePath({face_anim_dir, _lastFaceAnimDir});
...
void AnimationTransfer::HandleGameEvents(
    const AnkiEvent<ExternalInterface::MessageGameToEngine> &event) {
  if (event.GetData().GetTag() ==
      ExternalInterface::MessageGameToEngineTag::TransferFile) {
    const Anki::Vector::ExternalInterface::TransferFile &msg =
        event.GetData().Get_TransferFile();
    ...
    if (_expectedNextChunk == msg.filePart) {
      ...
      // processing FaceImg below does another cleanup if the subdirectory has
      // changed, but we don't want to remove the whole directory if this is
      // just a new image in the same set.
      CleanUp(msg.fileType != ExternalInterface::FileType::FaceImg);
      ++_expectedNextChunk;
    } else {
      PRINT_NAMED_ERROR("FileTransfer.Upload",
                        "File Part unexpected got: %d expected: %d",
                        msg.filePart, _expectedNextChunk);
      CleanUp();
      return;
    }
    ...
    if (msg.fileType == ExternalInterface::FileType::Animation) {
      ...
      std::string full_path = _dataPlatform->pathToResource(
          Util::Data::Scope::Cache, kCacheAnimFileName);
      ...
      if (found_char != std::string::npos) {
        // first part of the filename without the frame number is the directory
        // name
        ...
        if (_lastFaceAnimDir != currDir) {
          ...
          std::string face_anim_dir = _dataPlatform->pathToResource(
              Util::Data::Scope::Cache, kCacheFaceAnimsDir);
          std::string full_path_face_imgs = Anki::Util::FileUtils::FullFilePath(
              {face_anim_dir, _lastFaceAnimDir});
      ...
      if (!_lastFaceAnimDir.empty()) {
        std::string face_anim_dir = _dataPlatform->pathToResource(
            Util::Data::Scope::Cache, kCacheFaceAnimsDir);
        std::string full_path_face_imgs = Anki::Util::FileUtils::FullFilePath(
            {face_anim_dir, _lastFaceAnimDir, msg.filename});
