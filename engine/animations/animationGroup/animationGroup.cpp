...
AnimationGroup::AnimationGroup(Util::RandomGenerator& rng,
                               const std::string& name)
    : _rng(rng), _name(name) {}

Result AnimationGroup::DefineFromJson(const std::string& name,
  ...
  for (s32 iEntry = 0; iEntry < numEntries; ++iEntry) {
    ...
    if (RESULT_OK != addResult) {
      LOG_ERROR("AnimationGroup.DefineFromJson.AddEntryFailure",
                "Adding animation %d failed.", iEntry);
      anyFailures = true;
    } else {
...
bool AnimationGroup::IsEmpty() const { return _animations.empty(); }

const std::string& AnimationGroup::GetAnimationName(
    const MoodManager& moodManager,
    AnimationGroupContainer& animationGroupContainer, float headAngleRad,
    bool strictCooldown) const {
  return GetAnimationName(
      moodManager.GetSimpleMood(), moodManager.GetLastUpdateTime(),
      animationGroupContainer, headAngleRad, strictCooldown);
}
...
const std::string& AnimationGroup::GetAnimationName(
    ...
    AnimationGroupContainer& animationGroupContainer, float headAngleRad,
    bool strictCooldown) const {
  LOG_DEBUG("AnimationGroup.GetAnimation",
            "getting animation from group '%s', simple mood = '%s'",
            _name.c_str(), SimpleMoodTypeToString(mood));
  ...
  for (auto entry = _animations.begin(); entry != _animations.end(); entry++) {
    if (entry->GetMood() == mood) {
      ...
      if (entry->GetUseHeadAngle()) {
        if (!(headAngleRad >= entry->GetHeadAngleMin() &&
              headAngleRad <= entry->GetHeadAngleMax())) {
          validHeadAngle = false;
        }
      }
      if (validHeadAngle) {
        if (!animationGroupContainer.IsAnimationOnCooldown(entry->GetName(),
                                                           currentTime_s)) {
        ...
        LOG_INFO("AnimationGroup.GetAnimation.RejectAnimation.HeadAngle",
                 "%s: rejecting animation %s with head angle (%f) out of range "
                 "(%f,%f)",
                 _name.c_str(), entry->GetName().c_str(),
                 RAD_TO_DEG(headAngleRad), entry->GetHeadAngleMin(),
      ...
      LOG_INFO("AnimationGroup.GetAnimation.RejectAnimation.WrongMood",
               ...
               _name.c_str(), entry->GetName().c_str(),
               SimpleMoodTypeToString(entry->GetMood()),
               animationGroupContainer.IsAnimationOnCooldown(entry->GetName(),
                                                             currentTime_s)
                   ? ""
                   : "not ");
    }
  ...
  float weightedSelection =
      Util::numeric_cast<float>(_rng.RandDbl(totalWeight));
  ...
  if (lastEntry != nullptr) {
    animationGroupContainer.SetAnimationCooldown(
        lastEntry->GetName(), currentTime_s + lastEntry->GetCooldown());

    LOG_DEBUG("AnimationGroup.GetAnimation.Found",
              "Group '%s' returning animation name '%s'", _name.c_str(),
              lastEntry->GetName().c_str());

    return lastEntry->GetName();
  }

  // we couldn't find an animation. If we were in a non-default mood, try again
  // with the default mood
  if (mood != SimpleMoodType::Default) {
    LOG_DEBUG("AnimationGroup.GetAnimation.NoMoodMatch",
              "No animations from group '%s' selected matching mood '%s', "
              "trying with default mood",
              _name.c_str(), SimpleMoodTypeToString(mood));

    return GetAnimationName(SimpleMoodType::Default, currentTime_s,
                            animationGroupContainer, headAngleRad,
                            strictCooldown);
  }

  static const std::string empty = "";
  // Since this is the backup emergency case, also ignore head angle and just
  // play something
  if (anyAnimationsMatchingMood && !strictCooldown) {
    ...
    LOG_INFO("AnimationGroup.GetAnimation.AllOnCooldown",
             "All animations are on cooldown. Selecting the one closest to "
             "being finished");

    for (auto entry = _animations.begin(); entry != _animations.end();
         entry++) {
      if (entry->GetMood() == mood) {
        float timeLeft = animationGroupContainer.TimeUntilCooldownOver(
            entry->GetName(), currentTime_s);

        if (DEBUG_ANIMATION_GROUP_SELECTION) {
          LOG_DEBUG("AnimationGroup.GetAnimation.ConsiderIgnoringCooldown",
                    "%s: animation %s has %f left on it's cooldown",
                    _name.c_str(), entry->GetName().c_str(), timeLeft);
        }
    ...
    if (bestEntry != nullptr) {
      LOG_INFO("AnimationGroup.GetAnimation.BackupAnimationFound",
               "All animations in group '%s' were on cooldown / invalid, so "
               "selected '%s'",
               _name.c_str(), bestEntry->GetName().c_str());

      return bestEntry->GetName();
    } else {
      LOG_INFO("AnimationGroup.GetAnimation.NoBackup",
               "All animations in group '%s' were on cooldown / invalid "
  ...
  LOG_ERROR("AnimationGroup.GetAnimation.NoAnimation",
            "Could not find a single animation from group '%s' to run. "
            "Returning empty",
            _name.c_str());
  return empty;
}
