...
Result AnimationGroupEntry::DefineFromJson(const Json::Value& jsonRoot) {
  ...
  // TODO: Verify each animation actually exists in the CannedAnimationContainer
  // (which is now in the animation process) (VIC-370)
  /*
  if(nullptr != cannedAnimations && nullptr ==
  cannedAnimations->GetAnimation(_name)) {
  ...
  */
  ...
  if (_mood == SimpleMoodType::Count) {
    LOG_WARNING("SimpleMoodScorer.ReadFromJson.BadType", "Bad '%s' = '%s'",
                kMoodKey, moodTypeString);
    return RESULT_FAIL;
  }
  ...
  _useHeadAngle = false;
  if (JsonTools::GetValueOptional(jsonRoot, kUseHeadAngleKey, _useHeadAngle) &&
      _useHeadAngle) {
    ...
    if (!minHeadAngle.isDouble() || !maxHeadAngle.isDouble()) {
      LOG_ERROR(
          "AnimationGroupEntry.DefineFromJson.NoHeadAngleWhenUsingHeadAngles",
          "Missing '%s' or '%s' field for animation.", kHeadAngleMinKey,
          kHeadAngleMaxKey);
