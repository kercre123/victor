...
AnimationGroup* AnimationGroupContainer::GetAnimationGroup(
    const std::string& name) {
  const AnimationGroup* animGroupPtr =
      const_cast<const AnimationGroupContainer*>(this)->GetAnimationGroup(name);
  return const_cast<AnimationGroup*>(animGroupPtr);
}

const AnimationGroup* AnimationGroupContainer::GetAnimationGroup(
  ...
  if (retVal == _animationGroups.end()) {
    PRINT_NAMED_ERROR(
        "AnimationGroupContainer.GetAnimationGroup_Const.InvalidName",
...
std::vector<std::string> AnimationGroupContainer::GetAnimationGroupNames() {
  ...
  for (std::unordered_map<std::string, AnimationGroup>::iterator i =
           _animationGroups.begin();
       i != _animationGroups.end(); ++i) {
    v.push_back(i->first);
  }
  return v;
}

Result AnimationGroupContainer::DefineFromJson(
    const Json::Value& jsonRoot, const std::string& animationGroupName) {
  if (RESULT_OK != AddAnimationGroup(animationGroupName)) {
    PRINT_CH_INFO(
        "Animations",
        "AnimationGroupContainer.DefineAnimationGroupFromJson.ReplaceName",
...
void AnimationGroupContainer::Clear() { _animationGroups.clear(); }  // Clear()

bool AnimationGroupContainer::IsAnimationOnCooldown(
    const std::string& name, double currentTime_s) const {
...
float AnimationGroupContainer::TimeUntilCooldownOver(
    const std::string& name, double currentTime_s) const {
  auto retVal = _animationCooldowns.find(name);
  if (retVal == _animationCooldowns.end()) {
    return 0.0f;
...
void AnimationGroupContainer::SetAnimationCooldown(
    const std::string& name, double cooldownExpiration_s) {
