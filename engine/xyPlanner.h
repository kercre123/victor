...
class XYPlanner : public IPathPlanner, private Util::noncopyable {
 public:
  ...
  virtual EComputePathStatus ComputePath(
      const Pose3d& startPose, const std::vector<Pose3d>& targetPoses) override;
  ...
  virtual bool CheckIsPathSafe(const Planning::Path& path,
                               float startAngle) const override;
  virtual bool CheckIsPathSafe(const Planning::Path& path, float startAngle,
                               Planning::Path& validPath) const override;

  // return a test path
  virtual void GetTestPath(
      const Pose3d& startPose, Planning::Path& path,
 ...
 protected:
  virtual EComputePathStatus ComputePath(const Pose3d& startPose,
    ...
    return ComputePath(startPose, std::vector<Pose3d>({targetPose}));
  }
 ...
 private:
  ...
  EComputePathStatus InitializePlanner(const Pose2d& start,
                                       const std::vector<Pose2d>& targets,
                                       bool forceReplan, bool allowGoalChange);
  ...
  std::vector<Point2f> GenerateWayPoints(
      const std::vector<Point2f>& plan) const;

  // given a set of points, generate the largest safe circumscibed arc for each
  // turn
  std::vector<Planning::PathSegment> SmoothCorners(
      const std::vector<Point2f>& pts) const;

  // if p corresponds to a pose in _targets, return the correspondance index. if
  // it is not, returns _targets.size()
  ...
  // returns true if the provided region has no collisions with supported types
  // in the map
  ...
  const bool _isSynchronous =
      false;  // if TRUE, do not start a thread on construction
  volatile bool _stopThread = false;  // clean up and stop the thread entirely
  volatile bool _startPlanner =
      false;  // start planning now if it isn't running
  volatile bool _stopPlanner =
      false;  // if the planner is currently running, force it to stop
};
