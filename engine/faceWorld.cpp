 ...
 * Created: 2014
 *
 * Description: Implements a container for mirroring on the main thread the
 *faces from the vision system (which generally runs on another thread).
 *
...
// This used to be smaller, but we're starting it on the order of minutes. This
// might need to be settable by whatever behaviors are running
CONSOLE_VAR(u32, kDeletionTimeout_ms, "Vision.FaceWorld", 10 * 60 * 1000);

// The distance threshold inside of which to head positions are considered to be
// the same face
CONSOLE_VAR(float, kHeadCenterPointThreshold_mm, "Vision.FaceWorld", 220.f);

// We don't log session-only (unnamed) faces to DAS until we consider them
// "stable"
CONSOLE_VAR(u32, kNumTimesToSeeFrontalToBeStable, "Vision.FaceWorld", 30);

// Log recognition to DAS if we haven't seen a face for this long and then
// re-see it
CONSOLE_VAR(u32, kTimeUnobservedBeforeReLoggingToDAS_ms, "Vision.FaceWorld",
            10000);

// Ignore faces detected below the robot (except when picked up), to help reduce
// false positives
CONSOLE_VAR(bool, kIgnoreFacesBelowRobot, "Vision.FaceWorld", true);

// Ignore new faces detected while rotating too fast
CONSOLE_VAR(f32, kHeadTurnSpeedThreshFace_degs,
            "WasRotatingTooFast.Face.Head_deg/s", 10.f);
CONSOLE_VAR(f32, kBodyTurnSpeedThreshFace_degs,
            "WasRotatingTooFast.Face.Body_deg/s", 30.f);
CONSOLE_VAR(u8, kNumImuDataToLookBackFace,
            "WasRotatingTooFast.Face.NumToLookBack", 5);
...
FaceWorld::FaceEntry::FaceEntry(const Vision::TrackedFace& faceIn)
    : face(faceIn), vizHandle(VizManager::INVALID_HANDLE) {}
...
inline bool FaceWorld::FaceEntry::HasStableID() const {
  ...
  DEV_ASSERT(!IsNamed() || face.GetID() > 0,
             "FaceWorld.FaceEntry.HasStableID.NamedFaceWithNonPositiveID");
  return face.GetID() > 0 && (IsNamed() || numTimesObservedFacingCamera >=
...
FaceWorld::FaceWorld()
    : UnreliableComponent<BCComponentID>(this, BCComponentID::FaceWorld),
      IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::FaceWorld) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void FaceWorld::InitDependent(Vector::Robot* robot,
...
void FaceWorld::SetupEventHandlers(IExternalInterface& externalInterface) {
  ...
  //    auto helper = MakeAnkiEventUtil(externalInterface, *this,
  //    _eventHandles);
...
void FaceWorld::ClearAllFaces() {
  for (auto faceEntryIter = _faceEntries.begin();
       faceEntryIter != _faceEntries.end();
       /* inc in loop */) {
...
Result FaceWorld::ChangeFaceID(const Vision::UpdatedFaceID& update) {
  ...
  if (faceEntryIter != _faceEntries.end()) {
    ...
    PRINT_CH_INFO(kLoggingChannelName, "FaceWorld.ChangeFaceID.Success",
                  "Updating old face %d (%s) to new ID %d (%s)", oldID,
                  face.HasName() ? Util::HidePersonallyIdentifiableInfo(
                                       face.GetName().c_str())
                                 : "<NoName>",
                  newID,
                  newName.empty()
    ...
    const bool existingFaceHasDifferentName =
        face.HasName() && (newName != face.GetName());
    if (existingFaceHasDifferentName) {
      PRINT_NAMED_WARNING(
          "FaceWorld.ChangeFaceID.ChangingName",
          "OldID:%d OldName:%s, NewID:%d NewName:%s", oldID,
          Util::HidePersonallyIdentifiableInfo(face.GetName().c_str()), newID,
          Util::HidePersonallyIdentifiableInfo(newName.c_str()));
    }
    ...
    if (oldID > 0 && newID > 0 && newFaceEntry.HasStableID()) {
      DASMSG(robot.vision.update_face_id, "robot.vision.update_face_id",
             "Face ID updated");
    ...
    PRINT_CH_INFO(kLoggingChannelName, "FaceWorld.ChangeFaceID.UnknownOldID",
                  "ID %d does not exist, cannot update to %d", oldID, newID);
  } else {
  ...
  // Always notify game: let it decide whether or not it cares or knows about
  // oldID
  using namespace ExternalInterface;
  _robot->Broadcast(
...
Result FaceWorld::AddOrUpdateFace(const Vision::TrackedFace& face) {
  ...
  DEV_ASSERT(!face.GetHeadPose().HasParent(),
             "FaceWorld.AddOrUpdateFace.HeadPoseHasParent");
  RobotTimeStamp_t t = 0;
  HistRobotState* histStatePtr = nullptr;
  HistStateKey histStateKey;
  const Result histStateResult =
      _robot->GetStateHistory()->ComputeAndInsertStateAt(
          face.GetTimeStamp(), t, &histStatePtr, &histStateKey, true);

  if (RESULT_OK != histStateResult || histStatePtr == nullptr) {
    PRINT_NAMED_WARNING("FaceWorld.AddOrUpdateFace.GetComputedStateAtFailed",
                        "face timestamp=%u", face.GetTimeStamp());
    return histStateResult;
  }

  const auto& origin = _robot->GetPoseOriginList().GetOriginByID(
      histStatePtr->GetPose().GetRootID());
  ...
  if (kIgnoreFacesBelowRobot && robotOnTreads && headBelowRobot) {
    // Don't report faces that are below the origin (which we are assuming is on
    // the ground plane)
    PRINT_CH_DEBUG(kLoggingChannelName,
                   "FaceWorld.AddOrUpdateFace.IgnoringFaceBelowRobot", "z=%f",
                   headPoseWrtWorldOrigin.GetTranslation().z());
    return RESULT_OK;
  }
  ...
  T=(%.1f,%.1f,%.1f).", face.GetRect().GetX(), face.GetRect().GetY(),
  ...
  */
  ...
  if (false == Vision::FaceTracker::IsRecognitionSupported()) {
    ...
    for (auto faceEntryIter = _faceEntries.begin();
         faceEntryIter != _faceEntries.end(); ++faceEntryIter) {
      ...
      if (currentIOU > IOU_threshold) {
        ...
        auto posDiffVec =
            (faceEntryIter->second.face.GetHeadPose().GetTranslation() -
             headPoseWrtWorldOrigin.GetTranslation());
        float posDiffSq = posDiffVec.LengthSq();
        if (posDiffSq <=
            kHeadCenterPointThreshold_mm * kHeadCenterPointThreshold_mm) {
          if (foundMatch) {
            // If we had already found a match, delete the last one, because
            // this new face matches multiple existing faces
            assert(nullptr != faceEntry);
            RemoveFaceByID(faceEntry->face.GetID());
          }
    ...
    if (foundMatch) {
      ...
      // PRINT_CH_DEBUG(kLoggingChannelName,
      // "FaceWorld.UpdateFace.UpdatingFaceEntryByPose",
      //               "Updating face with ID=%lld from t=%d to %d, observed %d
      //               times", matchedID, faceEntry->face.GetTimeStamp(),
      //               face.GetTimeStamp(), face.GetNumTimesObserved());
    ...
    // Didn't find a match based on pose, so add a new face with a new ID:
      ...
      PRINT_CH_INFO(kLoggingChannelName, "FaceWorld.UpdateFace.NewFace",
                    "Added new face with ID=%d at t=%d.", _idCtr,
                    face.GetTimeStamp());

      auto insertResult = _faceEntries.insert({_idCtr, face});
      if (insertResult.second == false) {
        PRINT_NAMED_ERROR(
            "FaceWorld.UpdateFace.ExistingID",
            "Did not find a match by pose, but ID %d already in use.", _idCtr);
        return RESULT_FAIL;
      }
    ...
    if (isNewFace) {
      // Make sure we aren't rotating too fast to add a new face (this helps
      // safeguard against false positives)
      const bool fastRotationAllowed =
          (_robot->GetVisionComponent().IsVisionWhileRotatingFastEnabled() &&
      ...
      const bool wasRotatingTooFast =
          (!fastRotationAllowed &&
           imuHistory.WasRotatingTooFast(
               face.GetTimeStamp(), DEG_TO_RAD(kBodyTurnSpeedThreshFace_degs),
      ...
      if (wasRotatingTooFast) {
        ...
        PRINT_CH_INFO(kLoggingChannelName, "FaceWorld.UpdateFace.NewFace",
                      "Added new face with ID=%d at t=%d.", face.GetID(),
                      face.GetTimeStamp());
      ...
      if (face.GetTimeStamp() > faceEntry->face.GetTimeStamp()) {
        timeSinceLastSeen_ms =
            face.GetTimeStamp() - faceEntry->face.GetTimeStamp();
      } else {
        PRINT_NAMED_WARNING(
            "FaceWorld.UpdateFace.BadTimeStamp",
    ...
    // update the observation time if this is a named face. Note that this is
    // using current wall time, which is slightly different from the actual
    // image timestamp when the face was observed, but should be close enough.
    // Only store if time is accurate.
    if (face.HasName()) {
      ...
      if (WallTime::getInstance()->GetTime(wallTime)) {
        ...
        if (it != _wallTimesObserved.end()) {
          ...
          // if the new sighting is in a different day than the last one, we
          // need to update robot stats
          const auto lastSeen = it->second.front();
          if (!WallTime::AreTimePointsInSameDay(lastSeen, wallTime)) {
            PRINT_CH_INFO(kLoggingChannelName,
                          "FaceWorld.UpdateFace.FaceSeenOnNewDay",
                          "face %d seen on new day", face.GetID());
            _robot->GetComponent<RobotStatsTracker>()
                .IncrementNamedFacesPerDay();
          }
        } else {
          // new entry
          _wallTimesObserved.emplace(face.GetID(),
                                     ObservationTimeHistory{{wallTime}});

          PRINT_CH_INFO(
              kLoggingChannelName, "FaceWorld.UpdateFace.NamedFaceFirstDaySeen",
              "face %d has been seen for the first time", face.GetID());
  ...
  if (faceEntry->numTimesObserved == 1 && isNamed) {
    ...
    DASMSG_SEND();
  } else if (!isNamed && faceEntry->face.GetID() > 0 &&
             faceEntry->numTimesObservedFacingCamera ==
                 kNumTimesToSeeFrontalToBeStable) {
    DASMSG(robot.vision.face_recognition.persistent_session_only,
           "robot.vision.face_recognition.persistent_session_only",
           "We have seen a session-only face for awhile and not recognized it "
           "as someone else (so this is a stable "
           "session-only face) NOTE: we do this just once, when we cross the "
           "num times observed threshold");
    DASMSG_SET(i1, faceEntry->face.GetID(), "Face ID");
    DASMSG_SEND();

    // HACK: increment the counter again so we don't send this multiple times if
    // not seeing frontal anymore
    faceEntry->numTimesObservedFacingCamera++;
  } else if (timeSinceLastSeen_ms > kTimeUnobservedBeforeReLoggingToDAS_ms &&
             faceEntry->HasStableID()) {
    DASMSG(robot.vision.face_recognition.persistent_session_only,
           "robot.vision.face_recognition.persistent_session_only",
           "We are re-seeing a face after not having seen it for a bit (and "
           "recognizing it as an existing named "
  ...
  // Wait to report this face until we've seen it enough times to be convinced
  // it's not a false positive (random detection), or if it has been recognized
  // already.
  if (faceEntry->numTimesObserved >= MinTimesToSeeFace ||
      !faceEntry->face.GetName().empty()) {
    // Update the last observed face pose.
    // If more than one was observed in the same timestamp then take the closest
    // one.
    const bool newerThanLastObservation =
        (faceEntry->face.GetTimeStamp() > _lastObservedFaceTimeStamp);
    bool closerThanLastObservation =
        false;  // note: only computed if there were multiple observations in
                // one tick
    if (!newerThanLastObservation) {
      // More than one face was observed in the same timestamp, so see if this
      // one is closest
      float lastObservedFaceDist_mm = 0.f;
      float thisFaceDist_mm = 0.f;
      if (!ComputeDistanceBetween(_robot->GetPose(), _lastObservedFacePose,
                                  lastObservedFaceDist_mm) ||
    ...
    if (newerThanLastObservation || closerThanLastObservation) {
      ...
      faceEntry->vizHandle =
          _robot->GetContext()->GetVizManager()->DrawHumanHead(
              0, kHumanHeadSize, faceEntry->face.GetHeadPose(),
    ...
    PRINT_CH_INFO(kLoggingChannelName, "FaceWorld.AddOrUpdateFace",
                     ...
                     "at t=%d Pose: roll=%.1f, pitch=%.1f yaw=%.1f,
    T=(%.1f,%.1f,%.1f).", faceEntry->face.GetRect().GetX(),
    faceEntry->face.GetRect().GetY(), faceEntry->face.GetRect().GetWidth(),
    faceEntry->face.GetRect().GetHeight(), faceEntry->face.GetTimeStamp(),
    ...
    */
    ...
    const std::vector<
        std::pair<std::vector<CladPoint2d>*, Vision::TrackedFace::FeatureName>>
        features{
    ...
    RobotObservedFace msg(
        faceEntry->face.GetID(), faceEntry->face.GetTimeStamp(),
        faceEntry->face.GetHeadPose().ToPoseStruct3d(
            _robot->GetPoseOriginList()),
        ...
        faceEntry->face.GetName(), faceEntry->face.GetMaxExpression(),
        faceEntry->face.GetSmileAmount(), faceEntry->face.GetGaze(),
        faceEntry->face.GetBlinkAmount(), faceEntry->face.GetExpressionValues(),
    ...
    if(!faceThumbnail.IsEmpty()) {
      ...
      cv::imencode(".jpg",  faceThumbnail.get_CvMat_(), msg.jpgImage,
    compressionParams);
  ...
  if (face.HasEyes()) {
    ...
    if (entry.GetExpired(face.GetTimeStamp())) {
      ...
      if (isGazeStable) {
        ...
        Pose3d gazeDirectionPose(0.f, Z_AXIS_3D(), faceDirectionAverage,
...
Result FaceWorld::Update(const std::list<Vision::TrackedFace>& observedFaces) {
  ...
  const RobotTimeStamp_t lastProcImageTime =
      _robot->GetVisionComponent().GetLastProcessedImageTimeStamp();
  ...
  for (auto faceIter = _faceEntries.begin(); faceIter != _faceEntries.end();) {
    ...
    if (face.GetName().empty() &&
        (lastProcImageTime > kDeletionTimeout_ms + face.GetTimeStamp())) {
      PRINT_CH_INFO(kLoggingChannelName, "FaceWorld.Update.DeletingOldFace",
                    "Removing unnamed face %d at t=%d, because it hasn't been "
                    "seen since t=%d.",
                    faceIter->first, (TimeStamp_t)lastProcImageTime,
                    face.GetTimeStamp());
  ...
  if (_previousEyeContact != currentEyeContact) {
    if (currentEyeContact) {
      _robot->GetAnimationComponent().AddKeepFaceAliveFocus(
          kKeepFaceAliveEyeContactName);
    } else {
      _robot->GetAnimationComponent().RemoveKeepFaceAliveFocus(
          kKeepFaceAliveEyeContactName);
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool FaceWorld::ShouldReturnFace(const FaceEntry& faceEntry,
  ...
  if (faceEntry.face.GetTimeStamp() >= seenSinceTime_ms) {
    if (!includeRecognizableOnly || faceEntry.face.GetID() > 0) {
      bool isFaceValid =
          _robot->IsPoseInWorldOrigin(faceEntry.face.GetHeadPose());
      if (isFaceValid &&
        ...
        if (faceEntry.face.GetHeadPose().GetWithRespectTo(robotPose, relPose)) {
          Radians angle{atan2f(relPose.GetTranslation().y(),
                               relPose.GetTranslation().x())};
          if (!angle.IsNear(angleRelativeRobot_rad,
...
void FaceWorld::OnRobotDelocalized(PoseOriginID_t worldOriginID) {
  ...
  // Note that we deliberately do not clear the last observed face pose!
  // Sometimes we use it (despite it's incorrect origin) as a best guess for
  // where to look to find a face.
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
int FaceWorld::UpdateFaceOrigins(PoseOriginID_t oldOriginID,
                                 PoseOriginID_t newOriginID) {
  DEV_ASSERT_MSG(_robot->GetPoseOriginList().ContainsOriginID(oldOriginID),
                 "FaceWorld.UpdateFaceOrigins.InvalidOldOrigin", "ID:%d",
                 oldOriginID);
  DEV_ASSERT_MSG(_robot->GetPoseOriginList().ContainsOriginID(newOriginID),
                 "FaceWorld.UpdateFaceOrigins.InvalidNewOrigin", "ID:%d",
                 newOriginID);
  ...
  for (auto& pair : _faceEntries) {
    ...
    // If this entry's face is directly w.r.t. the old origin, flatten it to the
    // new origin
    if (oldOrigin.IsParentOf(face.GetHeadPose())) {
      Pose3d poseWrtNewOrigin;
      const bool success =
          face.GetHeadPose().GetWithRespectTo(newOrigin, poseWrtNewOrigin);
      if (success) {
        PRINT_CH_DEBUG(kLoggingChannelName,
                       "FaceWorld.UpdateFaceOrigins.FlatteningFace",
                       "Flattened FaceID:%d w.r.t. %s", pair.first,
        ...
        PRINT_NAMED_WARNING(
            "FaceWorld.UpdateFaceOrigins.HeadPoseUpdateFailed",
            "Head pose of FaceID:%d is w.r.t. to old origin %s but "
            "failed to flatten to be w.r.t new origin %s",
            pair.first, oldOrigin.GetName().c_str(),
            newOrigin.GetName().c_str());
      }
    }

    if (newOrigin.IsParentOf(face.GetHeadPose())) {
      // Draw everything in the new origin (but don't draw in the image since
      // we're not actually observing it)
  ...
  if (_lastObservedFaceTimeStamp > 0) {
    if (oldOrigin.IsParentOf(_lastObservedFacePose)) {
      const bool success = _lastObservedFacePose.GetWithRespectTo(
          newOrigin, _lastObservedFacePose);
      if (!success) {
        PRINT_NAMED_WARNING(
            "FaceWorld.UpdateFaceOrigins.UpdateLastObservedPoseFailed",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void FaceWorld::UpdateSmartFaceToID(const Vision::FaceID_t faceID,
...
const Vision::TrackedFace* FaceWorld::GetFace(Vision::FaceID_t faceID) const {
  const bool kIncludeRecognizableOnly =
      false;  // FaceID directly specified, search everything
  auto faceIter = _faceEntries.find(faceID);
  if (faceIter != _faceEntries.end() &&
      ShouldReturnFace(faceIter->second, 0, kIncludeRecognizableOnly)) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
std::set<Vision::FaceID_t> FaceWorld::GetFaceIDs(
    RobotTimeStamp_t seenSinceTime_ms, bool includeRecognizableOnly,
    float relativeRobotAngleTolerance_rad,
    const Radians& angleRelativeRobot_rad) const {
  ...
  while (faceEntryIter != _faceEntries.end()) {
    ...
    if (ShouldReturnFace(
            faceEntryIter->second, seenSinceTime_ms, includeRecognizableOnly,
            relativeRobotAngleTolerance_rad, angleRelativeRobot_rad)) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
std::vector<SmartFaceID> FaceWorld::GetSmartFaceIDs(
    RobotTimeStamp_t seenSinceTime_ms, bool includeRecognizableOnly,
    float relativeRobotAngleTolerance_rad,
    const Radians& angleRelativeRobot_rad) const {
  std::set<Vision::FaceID_t> faces =
      GetFaceIDs(seenSinceTime_ms, includeRecognizableOnly,
                 relativeRobotAngleTolerance_rad, angleRelativeRobot_rad);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool FaceWorld::HasAnyFaces(RobotTimeStamp_t seenSinceTime_ms,
                            bool includeRecognizableOnly) const {
  auto faceEntryIter = _faceEntries.begin();
  while (faceEntryIter != _faceEntries.end()) {
    if (ShouldReturnFace(faceEntryIter->second, seenSinceTime_ms,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
RobotTimeStamp_t FaceWorld::GetLastObservedFace(Pose3d& poseWrtRobotOrigin,
                                                bool inRobotOriginOnly) const {
  RobotTimeStamp_t returnTime = 0;

  if (_lastObservedFaceTimeStamp > 0) {
    const bool lastPoseIsWrtRobotOrigin =
        _robot->IsPoseInWorldOrigin(_lastObservedFacePose);
    ...
    if (lastPoseIsWrtRobotOrigin) {
      ...
      poseWrtRobotOrigin.SetParent(
          _robot->GetWorldOrigin());  // Totally not true, but we're faking it!
      returnTime = _lastObservedFaceTimeStamp;
    }
  }
...
bool FaceWorld::HasTurnedTowardsFace(Vision::FaceID_t faceID) const {
  ...
  if (it == _faceEntries.end()) {
    // either this is a bad ID, or the face was deleted, so assume we haven't
    // animated at it. Note that (as of this comment writing...) named faces are
    // not deleted
    return false;
  }
...
void FaceWorld::SetTurnedTowardsFace(Vision::FaceID_t faceID, bool val) {
  ...
  if (it == _faceEntries.end()) {
    PRINT_NAMED_WARNING(
        "FaceWorld.SetTurnedTowardsFaceAndAnimation.InvalidFace",
        "Claiming that we animated at face %d, but that face doesn't exist in "
        "FaceWorld",
        faceID);
    return;
  }
...
void FaceWorld::DrawFace(FaceEntry& faceEntry, bool drawInImage) const {
  ...
  ColorRGBA drawFaceColor =
      ColorRGBA::CreateFromColorIndex((u32)trackedFace.GetID());

  const s32 vizID =
      (s32)trackedFace.GetID() + (trackedFace.GetID() >= 0 ? 1 : 0);
  faceEntry.vizHandle = _robot->GetContext()->GetVizManager()->DrawHumanHead(
      vizID, kHumanHeadSize, trackedFace.GetHeadPose(), drawFaceColor);

  const auto* featureGate = _robot->GetContext()->GetFeatureGate();
  if (kRenderGazeDirectionPoints &&
      featureGate->IsFeatureEnabled(FeatureType::GazeDirection)) {
    ...
    if (entry != _gazeDirection.end()) {
      ...
      Pose3d currentGazePose(
          Transform3d(Rotation3d(0.f, Z_AXIS_3D()), currentGazeDirection));
      faceEntry.vizHandle = _robot->GetContext()->GetVizManager()->DrawCuboid(
          startingObjectId, kGazeGroundPointSize, currentGazePose,
          ::Anki::NamedColors::ORANGE);

      if (gazeDirection.IsStable()) {
        const auto averageGazeDirection =
            gazeDirection.GetGazeDirectionAverage();
        Pose3d averageGazePose(
            Transform3d(Rotation3d(0.f, Z_AXIS_3D()), averageGazeDirection));
        faceEntry.vizHandle = _robot->GetContext()->GetVizManager()->DrawCuboid(
            startingObjectId + 1, kGazeGroundPointSize, averageGazePose,
  ...
  if (drawInImage) {
    // Draw box around recognized face (with ID) now that we have the real ID
    // set
    _robot->GetContext()->GetVizManager()->DrawCameraFace(trackedFace,
                                                          drawFaceColor);
  }
}
...
void FaceWorld::Enroll(Vision::FaceID_t faceID, bool forceNewID) {
  ...
  // If starting session enrollment, then set the num enrollments to -1 to get
  // "ongoing" enrollment. Otherwise, use the max we can store.
  const bool sessionOnly = (Vision::UnknownFaceID == faceID);
  const s32 numEnrollmentsRequired =
      (sessionOnly ? -1
                   : (s32)Vision::FaceRecognitionConstants::
                         MaxNumEnrollDataPerAlbumEntry);

  _robot->GetVisionComponent().SetFaceEnrollmentMode(
      faceID, numEnrollmentsRequired, forceNewID);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void FaceWorld::SendObjectUpdateToWebViz(
    const ExternalInterface::RobotDeletedFace& msg) const {
  if (msg.faceID <= 0) {
...
// - - - - - - - - - - - - - - - - - - -
void FaceWorld::SendObjectUpdateToWebViz(
    const ExternalInterface::RobotObservedFace& msg) const {
  if (msg.faceID <= 0) {
    return;  // ignore half-recognized or invalid faces
  }
  ...
  const auto* webService = _robot->GetContext()->GetWebService();
  if (webService != nullptr) {
    const bool isSubscribedObservedObjects =
        webService->IsWebVizClientSubscribed(kWebVizObservedObjectsName);
    const bool isSubscribedNavMap =
        webService->IsWebVizClientSubscribed(kWebVizNavMapName);
...
bool FaceWorld::IsMakingEyeContact(const u32 withinLast_ms) const {
  ...
  const RobotTimeStamp_t recentTime =
      lastImgTime > withinLast_ms ? (lastImgTime - withinLast_ms) : 0;
  // Loop over all the faces and see if any of them are making eye contact
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool FaceWorld::GetGazeDirectionPose(const u32 withinLast_ms,
...
bool FaceWorld::ClearGazeDirectionHistory(const SmartFaceID& faceID) {
  ...
  for (const auto& entry : _faceEntries) {
    if (faceID.MatchesFaceID(entry.second.face.GetID())) {
      const auto& gazeDirectionEntry =
          _gazeDirection.find(entry.second.face.GetID());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool FaceWorld::FaceInTurnAngle(const Radians& turnAngle,
                                const SmartFaceID& smartFaceIDToIgnore,
                                const Pose3d& robotPose,
                                SmartFaceID& faceIDToTurnTowards) const {
  for (const auto& entry : _faceEntries) {
    ...
    if (headPose.GetWithRespectTo(robotPose, headPoseWRTRobot)) {
      const Radians& horizontalFOV = _robot->GetVisionComponent()
                                         .GetCamera()
                                         .GetCalibration()
                                         ->ComputeHorizontalFOV();
      const Radians faceTurnAngle =
          TurnTowardsPoseAction::GetRelativeBodyAngleToLookAtPose(
              headPoseWRTRobot.GetTranslation());
      if (Util::InRange(turnAngle - faceTurnAngle, -horizontalFOV / 2.f,
                        horizontalFOV / 2.f) &&
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void FaceWorld::InitLoadedKnownFaces(
    const std::list<Vision::LoadedKnownFace>& loadedFaces) {
  for (const auto& loadedFace : loadedFaces) {
    const auto epoch = WallTime::getInstance()->GetEpochTime();
    const auto sinceEpoch =
        std::chrono::seconds(loadedFace.lastSeenSecondsSinceEpoch);
    ...
    if (ANKI_VERIFY(!loadedFace.name.empty(),
                    ...
                    loadedFace.faceID)) {
      _wallTimesObserved.emplace(loadedFace.faceID,
                                 ObservationTimeHistory{{wallTime}});

      PRINT_CH_INFO(kLoggingChannelName,
                    "FaceWorld.InitLoadedKnownFaces.InitFace",
                    "Loaded face %d, last observed at time (since epoch): %llu",
                    loadedFace.faceID, wallTime.time_since_epoch().count());
    }
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
const FaceWorld::ObservationTimeHistory& FaceWorld::GetWallTimesObserved(
