...
struct OverheadEdgePointChain {
  ...
  bool isBorder;  // isBorder: true = detected border; false = reached end of
                  // ground plane without detecting a border
};
...
class OverheadEdgeChainVector {
 ...
 public:
  const std::vector<OverheadEdgePointChain>& GetVector() const {
    return _chains;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - information processed for a frame at
// the given timestamp
