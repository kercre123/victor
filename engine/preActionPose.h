...
class PreActionPose {
 public:
  enum ActionType {
    ...
    PLACE_RELATIVE,  // for placing a carried object on top of / in front of the
                     // specified object
  ...
  PreActionPose(ActionType type, const Vision::KnownMarker* marker,
                const f32 distance, const f32 length_mm);
  ...
  PreActionPose(const PreActionPose& canonicalPose,
                const Pose3d& markerParentPose, const f32 length_mm,
                const f32 offset_mm);
