...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void OverheadEdgeChainVector::RemoveChainsShorterThan(
    const u32 minChainLength) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void OverheadEdgeChainVector::AddEdgePoint(const OverheadEdgePoint& pointInfo,
                                           bool isBorder) {
  static const f32 kMaxDistBetweenEdges_mm =
      5.f;  // start new chain after this distance seen
  ...
  if (!_chains.empty()) {
    ...
    if (currentChain.points.empty()) {
      // current chain does not have points yet, we can add this one as the
      // first one
      addToCurrentChain = true;
    } else {
      // there are points, does the chain and this point match border vs
      // no_border flag?
      if (isBorder == currentChain.isBorder) {
        // they do, is the new point close enough to the last point in the
        // current chain?
        const f32 distToPrevPoint = ComputeDistanceBetween(
            pointInfo.position, _chains.back().points.back().position);
  ...
  // if we don't want to add the point to the current chain, then we need to
  // start a new chain
  ...
  if (newCurrentChain.points.empty()) {
    ...
    DEV_ASSERT(newCurrentChain.isBorder == isBorder,
               "VisionSystem.AddEdgePoint.BadBorderFlag");
  }
