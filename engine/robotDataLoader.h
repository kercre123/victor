...
class RobotDataLoader : private Util::noncopyable {
 public:
  ...
  // Starts a thread to handle loading non-config data if it hasn't been done
  // yet. Can be repeatedly called to get an updated loading complete ratio.
  // Returns false if loading is ongoing, otherwise returns true
  ...
  using CubeAnimationTriggerMap =
      Util::CladEnumToStringMap<CubeAnimationTrigger>;
  ...
  const FileJsonMap& GetCubeLightAnimations() const {
    return _cubeLightAnimations;
  }

  CannedAnimationContainer* GetCannedAnimationContainer() const {
    return _cannedAnimations.get();
  }
  AnimationGroupContainer* GetAnimationGroups() const {
    return _animationGroups.get();
  }

  AnimationTriggerMap* GetAnimationTriggerMap() const {
    return _animationTriggerMap.get();
  }
  CubeAnimationTriggerMap* GetCubeAnimationTriggerMap() const {
    return _cubeAnimationTriggerMap.get();
  }
  ...
  const std::set<AnimationTrigger>& GetDasBlacklistedAnimationTriggers() const {
    return _dasBlacklistedAnimationTriggers;
  }
  const std::set<std::string>& GetDasBlacklistedAnimationNames() const {
    return _dasBlacklistedAnimationNames;
  }

  // Returns true if the given animation is allowed to move the body while on
  // the charger
  ...
  const std::vector<std::string>& GetAllWhitelistedChargerAnimationPrefixes()
      const {
  ...
  const Json::Value& GetVictorFreeplayBehaviorConfig() const {
    return _victorFreeplayBehaviorConfig;
  }
  const Json::Value& GetRobotVisionConfig() const { return _robotVisionConfig; }
  const Json::Value& GetVisionScheduleMediatorConfig() const {
    return _visionScheduleMediatorConfig;
  }
  const Json::Value& GetWebServerEngineConfig() const {
    return _webServerEngineConfig;
  }
  ...
  const Json::Value& GetAccountSettingsConfig() const {
    return _accountSettingsConfig;
  }
  const Json::Value& GetUserEntitlementsConfig() const {
    return _userEntitlementsConfig;
  }
  ...
  using ConditionToBehaviorsMap =
      std::unordered_map<BEIConditionType, std::set<BehaviorID>>;
  ConditionToBehaviorsMap* GetUserDefinedConditionToBehaviorsMap() const {
    assert(nullptr != _conditionToBehaviorsMap);
    return _conditionToBehaviorsMap.get();
  }
  const BEIConditionType GetUserDefinedEditCondition() const {
    assert(BEIConditionType::Invalid != _userDefinedEditCondition);
    return _userDefinedEditCondition;
  }

  // images are stored as a map of stripped file name (no file extension) to
  // full path
  const Vision::SpritePathMap* GetSpritePaths() const {
    assert(_spritePaths != nullptr);
    return _spritePaths.get();
  }
  Vision::SpriteSequenceContainer* GetSpriteSequenceContainer() {
    return _spriteSequenceContainer.get();
  }
  Vision::SpriteCache* GetSpriteCache() const {
    assert(_spriteCache != nullptr);
    return _spriteCache.get();
  }

  // weather response map
  using WeatherResponseMap =
      std::unordered_map<std::string, WeatherConditionType>;
  using WeatherConditionTTSMap =
      std::unordered_map<WeatherConditionType, std::string>;

  // variable snapshot json map
  using VariableSnapshotJsonMap =
      std::unordered_map<VariableSnapshotId, Json::Value>;

  const WeatherResponseMap* GetWeatherResponseMap() const {
    assert(_weatherResponseMap);
    return _weatherResponseMap.get();
  }
  const WeatherConditionTTSMap* GetWeatherConditionTTSMap() const {
    assert(_weatherConditionTTSMap);
    return _weatherConditionTTSMap.get();
  }
  ...
  VariableSnapshotJsonMap* GetVariableSnapshotJsonMap() const {
    assert(_variableSnapshotJsonMap);
    return _variableSnapshotJsonMap.get();
  }
 ...
 private:
  ...
  void WalkAnimationDir(
      const std::string& animationDir, TimestampMap& timestamps,
  ...
  std::map<std::string, std::string> CreateFileNameToFullPathMap(
      const std::vector<const char*>& srcDirs,
      const std::string& fileExtensions) const;
