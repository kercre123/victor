...
static EToGLookupMap InitLookupMap() {
  ...
  for (counter = 0; counter < std::numeric_limits<EToGUnderlyingType>::max();
       ++counter) {
    ...
    // This needs to be fixed by updating CLAD to generate code for going from
    // string to tagtype rather than relying on
...
MessageEngineToGameTag GetEToGMessageTypeFromString(const char* inString) {
  ...
  PRINT_NAMED_ERROR("MessageHelpers.GetEToGMessageTypeFromString.NotFound",
                    "No match found for '%s'", lowerCaseString.c_str());

  // We want to return SOMETHING, so return the max value possible from the
