 ...
 * Created: 2018 Jun 29
 *
 * Description: Helper class for rejection sampling 2D positions that abide by
 *some constraints related to the robot
 *
...
Point2f SamplePointInCircle(Util::RandomGenerator& rng, f32 radius,
  ...
  DEV_ASSERT((radius > 0.f) && (minTheta < maxTheta) &&
                 (minTheta + M_TWO_PI_F >= maxTheta),
             "RobotPointSamplerHelper.SamplePointInCircle.InvalidArgs");
  Point2f ret;
  const float theta =
      static_cast<float>(rng.RandDblInRange(minTheta, maxTheta));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Point2f SamplePointInAnnulus(Util::RandomGenerator& rng, f32 minRadius,
                             f32 maxRadius, f32 minTheta, f32 maxTheta) {
  DEV_ASSERT((minRadius >= 0.f) && (minRadius < maxRadius) &&
                 (minTheta < maxTheta) && (minTheta + M_TWO_PI_F >= maxTheta),
  ...
  const float u = static_cast<float>(rng.RandDbl());
  const float r =
      std::sqrtf(minRadiusSq + (maxRadius * maxRadius - minRadiusSq) * u);
  ret.x() = r * cosf(theta);
  ret.y() = r * sinf(theta);
  return ret;
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void RejectIfWouldCrossCliff::SetAcceptanceInterpolant(float maxCliffDist_mm,
                                                       RandomGenerator& rng) {
  _rng = &rng;
  _maxCliffDistSq = maxCliffDist_mm * maxCliffDist_mm;
  DEV_ASSERT(_maxCliffDistSq > _minCliffDistSq,
             "RejectIfWouldCrossCliff.SetAcceptanceInterpolant.DistanceError");
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void RejectIfWouldCrossCliff::UpdateCliffs(
    std::shared_ptr<const INavMap> memoryMap) {
  _cliffs.clear();
  if (memoryMap == nullptr) {
    return;
  }
  MemoryMapTypes::MemoryMapDataConstList wasteList;
  memoryMap->FindContentIf(
      [this](MemoryMapTypes::MemoryMapDataConstPtr data) {
        if (data->type == MemoryMapTypes::EContentType::Cliff) {
          const auto& cliffData =
              MemoryMapData::MemoryMapDataCast<MemoryMapData_Cliff>(data);
...
bool RejectIfWouldCrossCliff::operator()(const Point2f& sampledPos) {
  DEV_ASSERT(_setRobotPos,
             "RejectIfWouldCrossCliff.CallOperator.UninitializedRobotPos");
  ...
  for (const auto* cliffPose : _cliffs) {
    ...
    // do this in 2d
    const Vec2f cliffEdgeDirection =
        CrossProduct(Z_AXIS_3D(), cliffDirection);  // sign doesn't matter
    const Point2f cliffPos = cliffPose->GetTranslation();
    // find intersection of lineRobotToSample with cliffEdgeDirection
    LineSegment cliffLine{
        cliffPos + cliffEdgeDirection * kMaxCliffIntersectionDist_mm,
        cliffPos - cliffEdgeDirection * kMaxCliffIntersectionDist_mm};
    Point2f intersectionPoint;
    const bool intersects =
        lineRobotToSample.IntersectsAt(cliffLine, intersectionPoint);
    if (intersects) {
      // confirm intersection point lies on cliff edge
      if (!AreVectorsAligned((intersectionPoint - cliffPos), cliffEdgeDirection,
                             0.001f)) {
        LOG_WARNING("RejectIfWouldCrossCliff.CallOperator.BadIntersection",
                    "vectors not aligned");
      }
      // if the intersection pos is close to the cliff pos, reject. If it's far,
      // accept. interpolate in between.
      ...
      if (_rng != nullptr) {
        const float p = (distFromCliffSq > _maxCliffDistSq)
                            ? 0.0f
                            : 1.0f - (distFromCliffSq - _minCliffDistSq) /
                                         (_maxCliffDistSq - _minCliffDistSq);
...
RejectIfInRange::RejectIfInRange(float minDist_mm, float maxDist_mm)
    : _minDistSq(minDist_mm * minDist_mm), _maxDistSq(maxDist_mm * maxDist_mm) {
  DEV_ASSERT(!std::signbit(minDist_mm) && !std::signbit(maxDist_mm) &&
...
bool RejectIfInRange::operator()(const Point2f& sampledPos) {
  DEV_ASSERT(_setOtherPos,
             "RejectIfInRange.CallOperator.OtherPosUninitialized");
  ...
  const bool reject =
      std::any_of(_otherPos.begin(), _otherPos.end(), inRangePred);
  return !reject;
}
...
bool RejectIfNotInRange::operator()(const Point2f& sampledPos) {
  DEV_ASSERT(_setOtherPos,
             "RejectIfNotInRange.CallOperator.OtherPosUninitialized");
...
bool RejectIfChargerOutOfView::operator()(const Point2f& sampledPos) {
  DEV_ASSERT(_setChargerPose,
             "RejectIfChargerOutOfView.CallOperator.ChargerPoseUninitialized");
  if (_pAccept == 1.f) {
    return true;
  }
  ...
  samplePose.SetTranslation(
      {sampledPos.x(), sampledPos.y(), _chargerPose.GetTranslation().z()});
  ...
  // The charger's origin is at the front of the 'lip' of the charger, and the x
  // axis points inward toward the marker. Therefore if the relative x position
  // of the sample point is negative, we should be able to see the marker.
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void RejectIfChargerOutOfView::SetAcceptanceProbability(float p,
                                                        RandomGenerator& rng) {
  _rng = &rng;
  _pAccept = p;
  DEV_ASSERT(_pAccept >= 0.0f && _pAccept <= 1.0f,
             "RejectIfChargerOutOfView.SetAcceptanceProbability.InvalidP");
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
RejectIfCollidesWithMemoryMap::RejectIfCollidesWithMemoryMap(
    const MemoryMapTypes::FullContentArray& collisionTypes)
    : _collisionTypes(ConvertContentArrayToFlags(collisionTypes)) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void RejectIfCollidesWithMemoryMap::SetAcceptanceProbability(
    float p, RandomGenerator& rng) {
  _rng = &rng;
  _pAccept = p;
  DEV_ASSERT(_pAccept >= 0.0f && _pAccept <= 1.0f,
             "RejectIfCollidesWithMemoryMap.SetAcceptanceProbability.InvalidP");
}
...
bool RejectIfCollidesWithMemoryMap::operator()(const Poly2f& sampledPoly) {
  ...
  const bool collides =
      _memoryMap->AnyOf(FastPolygon(sampledPoly), [&](const auto& data) {
        return IsInEContentTypePackedType(data->type, _collisionTypes);
      });
