 ...
 * Created: 2018 Jun 29
 *
 * Description: Helper class for rejection sampling 2D positions and polygons
 *that abide by some constraints related to the robot
 *
...
// uniformly sample a point on circle of radius (0, radius). Optionally supply
// minTheta and maxTheta to only sample points in a semi-circle where theta is
// in [minTheta, maxTheta)
...
// Optionally supply minTheta and maxTheta to only sample points in a
// semi-annulus where theta is in [minTheta, maxTheta)
Point2f SamplePointInAnnulus(Util::RandomGenerator& rng, f32 minRadius,
                             f32 maxRadius, f32 minTheta = -M_PI_F,
                             f32 maxTheta = M_PI_F);

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
class RejectIfWouldCrossCliff
    : public Anki::Util::RejectionSamplingCondition<Point2f> {
 public:
  explicit RejectIfWouldCrossCliff(float minCliffDist_mm);

  void SetRobotPosition(const Point2f& pos);

  // If not set, any sample that is within minCliffDist_mm is accepted and any
  // outside is rejected. If set, then additionally, any sample between
  // minCliffDist_mm and maxCliffDist_mm is accepted with probability linearly
  // increasing from 0 to 1 over that range
  void SetAcceptanceInterpolant(float maxCliffDist_mm,
                                Util::RandomGenerator& rng);

  // This method caches cliff pointers, so must be called every time you want to
  // use this condition with the latest memory map data
...
class RejectIfInRange : public Anki::Util::RejectionSamplingCondition<Point2f> {
 public:
  ...
  // Call SetOtherPositions() with a vector of all the known cube positions.
  // Then call operator() with your sample position, and it will return false if
  // it is too close to any cube.
  //
...
class RejectIfChargerOutOfView
 ...
 public:
  ...
  // If not set, any sample from which the charger is out of view is rejected.
  // If set, it is accepted with probability p
  void SetAcceptanceProbability(float p, Util::RandomGenerator& rng);
  void ClearAcceptanceProbability();

  // Will reject any position from which the charger would not be visible. Note
  // that this does not check distance to charger, just that the marker would be
...
class RejectIfCollidesWithMemoryMap
    : public Anki::Util::RejectionSamplingCondition<Poly2f> {
 public:
  explicit RejectIfCollidesWithMemoryMap(
      const MemoryMapTypes::FullContentArray& collisionTypes);

  void SetMemoryMap(std::shared_ptr<const INavMap> memoryMap) {
    _memoryMap = memoryMap;
  }

  // If not set, any sample that collides is rejected. If set, it is accepted
  // with probability p
  void SetAcceptanceProbability(float p, Util::RandomGenerator& rng);
 ...
 private:
  std::shared_ptr<const INavMap> _memoryMap =
      nullptr;  // BAD! Nothing guarantees that this is current
