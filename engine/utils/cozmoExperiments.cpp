...
#if USE_DAS
  DEV_ASSERT(DASGetPlatform() != nullptr,
             "CozmoExperiments.GetDeviceId.MissingDASPlatform");
...
void CozmoExperiments::InitExperiments() {
  ...
  auto userIdAccessor = [this] {
    ...
    return robot != nullptr ? std::to_string(robot->GetHeadSerialNumber())
                            : GetDeviceId();
  };
...
Util::AnkiLab::AssignmentStatus CozmoExperiments::ActivateExperiment(
  ...
  return _lab.ActivateExperiment(request.experiment_key, *userIdString, tags,
                                 outVariationKey);
}
...
void CozmoExperiments::WriteLabAssignmentsToRobot(
    const std::vector<Util::AnkiLab::AssignmentDef>& assignments) {
  Robot* robot = _context->GetRobotManager()->GetRobot();
  if (robot != nullptr) {
    LabAssignments labAssignments;
    for (const auto& assignment : assignments) {
      LabAssignment labAssignment(assignment.GetExperiment_key(),
                                  assignment.GetVariation_key());
    ...
    if (!robot->GetNVStorageComponent().Write(
            NVStorage::NVEntryTag::NVEntry_LabAssignments,
            assignmentsVec.data(), assignmentsVec.size())) {
      PRINT_NAMED_ERROR("CozmoExperiments.WriteLabAssignmentsToRobot.Failed",
                        "Write failed");
    }
  }
}
...
void CozmoExperiments::ReadLabAssignmentsFromRobot(const u32 serialNumber) {
  ...
  if (robot != nullptr) {
    ...
    if (!robot->GetNVStorageComponent().Read(
            NVStorage::NVEntryTag::NVEntry_LabAssignments,
            [this, serialNumber](u8* data, size_t size,
                                 NVStorage::NVResult res) {
              (void)RestoreLoadedActiveExperiments(data, size, res,
                                                   serialNumber);
            })) {
      PRINT_NAMED_ERROR("CozmoExperiments.ReadLabAssignmentsFromRobot.Failed",
                        "Read failed");
    }
  }
}

bool CozmoExperiments::RestoreLoadedActiveExperiments(
    const u8* data, const size_t size, const NVStorage::NVResult res,
    u32 serialNumber) {
  if (res < NVStorage::NVResult::NV_OKAY) {
    // The tag doesn't exist on the robot indicating the robot is new or has
    // been wiped
    if (res == NVStorage::NVResult::NV_NOT_FOUND) {
      ...
      PRINT_NAMED_ERROR(
          "CozmoExperiments.RestoreLoadedActiveExperiments.ReadFailedFinish",
  ...
  for (const auto& assignment : _loadedLabAssignments.labAssignments) {
    (void)lab.RestoreActiveExperiment(assignment.experiment_key, userId,
                                      assignment.variation_key);
  }
...
void CozmoExperiments::PossiblyWriteLabAssignmentsToRobot() {
  ...
  if (!needToWriteToRobot) {
    for (int i = 0; i < oldNumAssignments; i++) {
      if ((_loadedLabAssignments.labAssignments[i].experiment_key !=
           _assignments[i].experiment_key) ||
          (_loadedLabAssignments.labAssignments[i].variation_key !=
           _assignments[i].variation_key)) {
  ...
  if (needToWriteToRobot) {
    ...
    // Now copy the new set of assignments over the 'loaded' set, so that when
    // we call this function again, we can tell if we need to write to robot
    // again
    _loadedLabAssignments.labAssignments.clear();
...
void CozmoExperiments::UpdateLabAssignments(
    const std::vector<Util::AnkiLab::AssignmentDef>& assignments) {
