...
void PrintFeatureOverrides() {
  for (uint8_t i = 0; i < FeatureTypeNumEntries; ++i) {
    ...
    // printing as a warning so it stands out in the console to remind people
    // they have things overridden
...
void LoadFeatureOverrides() {
  ...
  const std::string fileContents =
      Util::FileUtils::ReadFile(sOverrideSaveFilename);
  if (!fileContents.empty() && reader.parse(fileContents, data)) {
    for (uint8_t i = 0; i < FeatureTypeNumEntries; ++i) {
      ...
      if (!overrideTypeJson.isNull()) {
        sFeatureTypeOverrides[i] =
            static_cast<FeatureTypeOverride>(overrideTypeJson.asUInt());
      }
    }

    // if we've loaded something, it means we have overrides, so print them so
    // people are aware they have something set
...
void SaveFeatureOverrides() {
  ...
  for (uint8_t i = 0; i < FeatureTypeNumEntries; ++i) {
    ...
    data[featureString] =
        static_cast<Json::Value::UInt>(sFeatureTypeOverrides[i]);
  ...
  if (isOverrideSet) {
    ...
    // if we have nothing set, remove the file (since this will be the default
    // state mainly)
...
void InitFeatureOverrides(Util::Data::DataPlatform* platform) {
  ...
  sOverrideSaveFilename =
      platform->pathToResource(Anki::Util::Data::Scope::Cache, fileName);
...
const char* InitFeatureEnumString() {
  ...
  static const std::string& kFeatureEnumString =
      Util::StringJoin(featureNames, ',');
  return kFeatureEnumString.c_str();
}
CONSOLE_VAR_ENUM(uint8_t, kFeatureToEdit, kConsoleFeatureGroup, 0,
                 InitFeatureEnumString());
...
void DefaultAllFeatures(ConsoleFunctionContextRef context) {
  for (uint8_t i = 0; i < FeatureTypeNumEntries; ++i) {
    if (FeatureTypeOverride::Default != sFeatureTypeOverrides[i]) {
      sFeatureTypeOverrides[i] = FeatureTypeOverride::Default;
      PRINT_NAMED_DEBUG("FeatureGate.Override",
                        "Removing override for feature %s",
...
bool CozmoFeatureGate::IsFeatureEnabled(FeatureType feature) const {
...
#if FEATURE_OVERRIDES_ENABLED
  {
    ...
    if (FeatureTypeOverride::Default != sFeatureTypeOverrides[featureIndex]) {
      return (FeatureTypeOverride::Enabled ==
              sFeatureTypeOverrides[featureIndex]);
    }
  }
#endif

  std::string featureEnumName{FeatureTypeToString(feature)};
  std::transform(featureEnumName.begin(), featureEnumName.end(),
                 featureEnumName.begin(), ::tolower);
...
void CozmoFeatureGate::Init(const CozmoContext* context,
  ...
  if (context != nullptr) {
    ...
    if (gi != nullptr) {
      auto handleFeatureFlagRequest = [this,
                                       gi](const AnkiEvent<
                                           external_interface::GatewayWrapper>&
                                               msg) {
        if (msg.GetData().GetTag() ==
            external_interface::GatewayWrapperTag::kFeatureFlagRequest) {
          const std::string& featureName =
              msg.GetData().feature_flag_request().feature_name();
          FeatureType featureType = FeatureType::Invalid;
          const bool valid = FeatureTypeFromString(featureName, featureType) &&
                             (featureType != FeatureType::Invalid);

          const bool enabled = valid ? IsFeatureEnabled(featureType) : false;
          auto* featureFlagResponse =
              new external_interface::FeatureFlagResponse;
          featureFlagResponse->set_valid_feature(valid);
          featureFlagResponse->set_feature_enabled(enabled);
          gi->Broadcast(
              ExternalMessageRouter::WrapResponse(featureFlagResponse));
        } else if (msg.GetData().GetTag() ==
                   external_interface::GatewayWrapperTag::
                       kFeatureFlagListRequest) {
          const bool returnAll =
              msg.GetData().feature_flag_list_request().request_list().empty();
          // list only those features that are enabled so that an sdk user can't
          // find SuperSecretFeature without brute forcing it
          auto* response = new external_interface::FeatureFlagListResponse;
          response->mutable_list()->Reserve(FeatureTypeNumEntries);
          const auto begin =
              msg.GetData().feature_flag_list_request().request_list().begin();
          const auto end =
              msg.GetData().feature_flag_list_request().request_list().end();
          for (uint8_t i = 0; i < FeatureTypeNumEntries; ++i) {
            const auto featureType = static_cast<FeatureType>(i);
            if (IsFeatureEnabled(featureType)) {
              const bool featureMatches =
                  returnAll || (std::find_if(begin, end, [&](const auto& s) {
                                  return Util::StringCaseInsensitiveEquals(
                                      EnumToString(featureType), s);
                                }) != end);
              if (featureMatches) {
      ...
      _signalHandles.push_back(gi->Subscribe(
          external_interface::GatewayWrapperTag::kFeatureFlagListRequest,
          handleFeatureFlagRequest));
    }
    ...
    if (webService != nullptr) {
      auto onDataFeatures = [](const Json::Value& data,
                               const std::function<void(const Json::Value&)>&
                                   sendToClient) {
        bool success = false;
#if FEATURE_OVERRIDES_ENABLED
        {
      ...
      _signalHandles.emplace_back(webService->OnWebVizData(kWebVizModuleName)
                                      .ScopedSubscribe(onDataFeatures));
      _signalHandles.emplace_back(
          webService->OnWebVizSubscribed(kWebVizModuleName)
              .ScopedSubscribe(
                  std::bind(&CozmoFeatureGate::SendFeaturesToWebViz, this,
                            std::placeholders::_1)));
    }
  }
}

void CozmoFeatureGate::SendFeaturesToWebViz(
  ...
  for (uint8_t i = 0; i < FeatureTypeNumEntries; ++i) {
    ...
    const std::string featureName =
        Util::StringToLower(FeatureTypeToString(feature));
    entry["default"] =
        FeatureGate::IsFeatureEnabled(featureName) ? "enabled" : "disabled";
#if FEATURE_OVERRIDES_ENABLED
    {
