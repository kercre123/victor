...
const float kAccelMovingThresh_mmps2 =
    50.f;  // highpass-filtered accelerometer magnitude must be below this to be
           // considered stationary
const uint32_t kBiasCheckDuration_ms =
    5000;  // duration that robot must be stationary before checking for gyro
           // bias
const int kBiasCheckMinReadings =
    50;  // minimum required number of readings for bias detection
const float kBiasDetectionThresh_rad_per_sec = DEG_TO_RAD(
    0.1f);  // near-constant gyro readings above this value is considered 'bias'
const float kBiasMaxRange_rad_per_sec =
    DEG_TO_RAD(0.05f);  // maximum allowed difference between min and max gyro
                        // readings during bias check period.
}  // anonymous namespace

RobotGyroDriftDetector::RobotGyroDriftDetector()
    : IDependencyManagedComponent(this, RobotComponentID::GyroDriftDetector) {}

void RobotGyroDriftDetector::InitDependent(Vector::Robot* robot,
...
void RobotGyroDriftDetector::DetectGyroDrift(const RobotState& msg) {
  ...
  if (!_gyroDriftReported) {
    ...
    // 5) Drift detector started but the raw gyro reading deviated too much from
    // starting values, indicating motion.
    if (_robot->GetMoveComponent().IsMoving() ||
        ...
        ((_startTime_ms != 0) &&
         ((std::fabsf(_startGyroZ_rad_per_sec - gyroZ) >
           kDriftCheckGyroZMotionThresh_rad_per_sec) ||
    ...
    // If gyro readings have been accumulating for long enough...
      ...
      // ...check if there was a sufficient change in heading angle or pitch.
      // Otherwise, reset detector.
      const f32 headingAngleChange =
          std::fabsf((_startAngle_rad -
                      _robot->GetPose().GetRotation().GetAngleAroundZaxis())
                         .ToFloat());
      const f32 angleChangeThresh = kDriftCheckMaxAngleChangeRate_rad_per_sec *
                                    Util::MilliSecToSec(kDriftCheckPeriod_ms);

      if (headingAngleChange > angleChangeThresh) {
        // Report drift detected just one time during a session
        const int min_mdeg_per_sec =
            std::round(RAD_TO_DEG(1000.f * _minGyroZ_rad_per_sec));
        const int max_mdeg_per_sec =
            std::round(RAD_TO_DEG(1000.f * _maxGyroZ_rad_per_sec));
        const int mean_mdeg_per_sec = std::round(
            RAD_TO_DEG(1000.f * _cumSumGyroZ_rad_per_sec) / _numReadings);
        const int headingAngleChange_mdeg_per_sec =
            std::round(RAD_TO_DEG(1000.f * headingAngleChange));

        DASMSG(gyro_bias_detected, "gyro.drift_detected",
               "We have detected gyro bias drift ('legacy' detection method)");
        DASMSG_SET(i1, min_mdeg_per_sec,
                   "min gyro z value (millidegrees per sec)");
        DASMSG_SET(i2, max_mdeg_per_sec,
                   "max gyro z value (millidegrees per sec)");
        DASMSG_SET(i3, mean_mdeg_per_sec,
                   "mean gyro z value (millidegrees per sec)");
        DASMSG_SET(i4, headingAngleChange_mdeg_per_sec,
                   "heading angle change (millidegrees per sec)");
        DASMSG_SEND();
    ...
    // Record min and max observed gyro readings and cumulative sum for later
    // mean computation
    else {
...
void RobotGyroDriftDetector::DetectBias(const RobotState& msg) {
  ...
  bool isMoving = static_cast<bool>(
      msg.status & Util::EnumToUnderlying(RobotStatusFlag::IS_MOVING));
  ...
  _hpFiltAccelMag =
      kFiltAccel * (currAccelMag - _accelMagPrev + _hpFiltAccelMag);
      ...
      if (_nReadings >= kBiasCheckMinReadings) {
        ...
        for (int i = 0; i < 3; i++) {
          ...
          if (sameSign &&
            ...
            DASMSG(gyro_bias_detected, "gyro.bias_detected",
                   "We have detected gyro bias drift");
            DASMSG_SET(i1, std::round(1000.f * RAD_TO_DEG(minGyro)),
                       "min gyro value (millidegrees per sec)");
            DASMSG_SET(i2, std::round(1000.f * RAD_TO_DEG(maxGyro)),
                       "max gyro value (millidegrees per sec)");
            DASMSG_SET(s1, axisStr, "axis of bias");
            DASMSG_SEND();
            PRINT_NAMED_WARNING("RobotGyroDriftDetector.BiasDetected",
                                "Gyro bias detected on %s axis (min=%.2f "
                                "deg/sec, max=%.2f deg/sec)",
                                axisStr.c_str(), RAD_TO_DEG(minGyro),
                                RAD_TO_DEG(maxGyro));
