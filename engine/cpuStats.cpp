...
namespace {
const float kSamplePeriod_sec =
    60.f;  // sample every minute.
           // If you change this, you have to change how
           // _numSamplesAboveReportingThresh is reported
const float kDasSendPeriod_sec =
    60.f * 60.f * 24.f;  // send to DAS every 24 hours or shutdown
const float kReportingTempThresh_degC =
    60.f;  // count minutes above this threshold
}  // namespace

CPUStats::CPUStats()
    : _temperatureStats_degC(
          std::make_unique<Util::Stats::StatsAccumulator>()) {
...
void CPUStats::LogToDas() {
  DASMSG(cpu_temperature_stats, "cpu.temperature_stats",
         "CPU temperature statistics");
  DASMSG_SET(i1, _temperatureStats_degC->GetIntMin(),
             "Minimum CPU temperature experienced (degC)");
  DASMSG_SET(i2, _temperatureStats_degC->GetIntMax(),
             "Maximum CPU temperature experienced (degC)");
  DASMSG_SET(i3, _numSamplesAboveReportingThresh,
             "Time spent above 60C (min)");  // depends on kSamplePeriod_sec==60
