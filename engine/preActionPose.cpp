 ...
 * Date:   7/9/2014
 *
 * Description: Implements a "Pre-Action" pose, which is used by
 *ActionableObjects to define a position to be in before acting on the object
 *with a given type of ActionType.
 *
 *
...
const ColorRGBA& PreActionPose::GetVisualizeColor(ActionType type) {
  ...
  if (iter == ColorLUT.end()) {
    PRINT_NAMED_WARNING(
        "PreActionPose.GetVisualizationColor.ColorNotDefined",
...
PreActionPose::PreActionPose(ActionType type, const Vision::KnownMarker* marker,
...
PreActionPose::PreActionPose(ActionType type, const Vision::KnownMarker* marker,
    ...
    : _type(type),
      _marker(marker),
      _poseWrtMarkerParent(M_PI_2, Z_AXIS_3D(), offset,
                           marker->GetPose())  // init w.r.t. marker
      ,
      _preActionPoseLineLength_mm(length_mm) {
  // Now make pose w.r.t. marker parent
  if (_poseWrtMarkerParent.GetWithRespectTo(_marker->GetPose().GetParent(),
                                            _poseWrtMarkerParent) == false) {
    PRINT_NAMED_ERROR(
        "PreActionPose.GetPoseWrtMarkerParentFailed",
...
PreActionPose::PreActionPose(ActionType type, const Vision::KnownMarker* marker,
    ...
    : _type(type), _marker(marker), _preActionPoseLineLength_mm(length_mm) {
  if (!poseWrtMarker.IsChildOf(marker->GetPose())) {
    PRINT_NAMED_ERROR(
        "PreActionPose.PoseWrtMarkerParentInvalid",
        "Given pose w.r.t. marker should have the marker as its parent pose.");
  }
  if (poseWrtMarker.GetWithRespectTo(_marker->GetPose().GetParent(),
                                     _poseWrtMarkerParent) == false) {
    PRINT_NAMED_ERROR(
        "PreActionPose.GetPoseWrtMarkerParentFailed",
        "Could not get the pre-action pose w.r.t. the marker's parent.");
...
PreActionPose::PreActionPose(const PreActionPose& canonicalPose,
                             ...
                             const f32 length_mm, const f32 offset_mm)
...
//, _poseWrtMarkerParent(markerParentPose*canonicalPose._poseWrtMarkerParent)
{
  ...
  Pose3d canonicalPoseWithOffset(
      canonicalPose._poseWrtMarkerParent.GetRotationMatrix(), trans);
