...
const VizManager::Handle_t VizManager::INVALID_HANDLE =
    std::numeric_limits<u32>::max();

Result VizManager::Connect(const char* udp_host_address,
  ...
  if (!_vizClient.Connect(udp_host_address, port)) {
    PRINT_NAMED_WARNING("VizManager.Connect.Failed",
                        "Failed to init VizManager client (%s:%d)",
...
VizManager::VizManager()
    : _isConnected(false), _messageCountViz(0), _sendImages(false) {
...
void VizManager::SendMessage(const VizInterface::MessageViz& message) {
...

  {
    ...
    if (_vizClient.Send((const char*)buffer, numPacked) <= 0) {
      PRINT_NAMED_WARNING(
          "VizManager.SendMessage.Fail", "Send vizMsgID %s of size %zu failed",
          VizInterface::MessageVizTagToString(message.GetTag()), numPacked);
    }
  }
...
void VizManager::DrawRobot(const Pose3d& pose, const f32 headAngle,
                           const f32 liftAngle) {
  ANKI_CPU_PROFILE("VizManager::DrawRobot");
  SendMessage(VizInterface::MessageViz(VizInterface::SetRobot(
      (float)MM_TO_M(pose.GetTranslation().x()),
      (float)MM_TO_M(pose.GetTranslation().y()),
      (float)MM_TO_M(pose.GetTranslation().z()),
      pose.GetRotationAngle().ToFloat(), pose.GetRotationAxis().x(),
      pose.GetRotationAxis().y(), pose.GetRotationAxis().z(), headAngle,
      liftAngle)));
}
...
VizManager::Handle_t VizManager::DrawRobot(const u32 robotID,
  ...
  if (robotID >= _VizObjectMaxID[(int)VizObjectType::VIZ_OBJECT_ROBOT]) {
    PRINT_NAMED_WARNING("VizManager.DrawRobot.IDtooLarge",
                        "Specified robot ID=%d larger than maxID=%d", robotID,
                        _VizObjectMaxID[(int)VizObjectType::VIZ_OBJECT_ROBOT]);
    return INVALID_HANDLE;
  }

  const u32 vizID =
      VizObjectBaseID[(int)VizObjectType::VIZ_OBJECT_ROBOT] + robotID;
...
VizManager::Handle_t VizManager::DrawCuboid(const u32 blockID,
  ...
  if (blockID >= _VizObjectMaxID[(int)VizObjectType::VIZ_OBJECT_CUBOID]) {
    PRINT_NAMED_WARNING("VizManager.DrawCuboid.IDtooLarge",
                        "Specified block ID=%d larger than maxID=%d", blockID,
                        _VizObjectMaxID[(int)VizObjectType::VIZ_OBJECT_CUBOID]);
    return INVALID_HANDLE;
  }

  const u32 vizID =
      VizObjectBaseID[(int)VizObjectType::VIZ_OBJECT_CUBOID] + blockID;
  DrawObject(vizID, VizObjectType::VIZ_OBJECT_CUBOID, size, pose, color);
  return vizID;
}

VizManager::Handle_t VizManager::DrawTextAtPose(const u32 textObjectID,
  ...
  if (textObjectID >= _VizObjectMaxID[(int)VizObjectType::VIZ_OBJECT_TEXT]) {
    PRINT_NAMED_WARNING("VizManager.DrawTextAtPose.IDtooLarge",
                        ...
                        textObjectID,
                        _VizObjectMaxID[(int)VizObjectType::VIZ_OBJECT_TEXT]);
    return INVALID_HANDLE;
  }

  const u32 vizID =
      VizObjectBaseID[(int)VizObjectType::VIZ_OBJECT_TEXT] + textObjectID;
  DrawObject(vizID, VizObjectType::VIZ_OBJECT_TEXT, Point3f(0.f, 0.f, 0.f),
             pose, color, nullptr, text);
...
VizManager::Handle_t VizManager::DrawPreDockPose(const u32 preDockPoseID,
                                                 ...
                                                 const ColorRGBA& color) {
  if (preDockPoseID >=
      _VizObjectMaxID[(int)VizObjectType::VIZ_OBJECT_PREDOCKPOSE]) {
    PRINT_NAMED_WARNING(
        "VizManager.DrawPreDockPose.IDtooLarge",
        "Specified PreDockPose ID=%d larger than maxID=%d", preDockPoseID,
        _VizObjectMaxID[(int)VizObjectType::VIZ_OBJECT_PREDOCKPOSE]);
    return INVALID_HANDLE;
  }

  const u32 vizID =
      VizObjectBaseID[(int)VizObjectType::VIZ_OBJECT_PREDOCKPOSE] +
      preDockPoseID;
  Anki::Point3f dims;  // junk
...
VizManager::Handle_t VizManager::DrawCharger(const u32 chargerID,
  ...
  if (chargerID >= _VizObjectMaxID[(int)VizObjectType::VIZ_OBJECT_CHARGER]) {
    PRINT_NAMED_WARNING(
        "VizManager.DrawCharger.IDtooLarge",
        "Specified charger ID=%d larger than maxID=%d", chargerID,
        _VizObjectMaxID[(int)VizObjectType::VIZ_OBJECT_CHARGER]);
    return INVALID_HANDLE;
  }
  ...
  const u32 vizID =
      VizObjectBaseID[(int)VizObjectType::VIZ_OBJECT_CHARGER] + chargerID;
...
VizManager::Handle_t VizManager::DrawHumanHead(const s32 headID,
                                               const Point3f& size,
                                               const Pose3d& pose,
                                               const ColorRGBA& color) {
  if (std::abs(headID) >=
      _VizObjectMaxID[(int)VizObjectType::VIZ_OBJECT_HUMAN_HEAD]) {
    PRINT_NAMED_WARNING(
        "VizManager.DrawHumanHead.IDtooLarge",
        "Specified head ID=%d larger than maxID=%d", headID,
        _VizObjectMaxID[(int)VizObjectType::VIZ_OBJECT_HUMAN_HEAD]);
    return INVALID_HANDLE;
  }
...
void VizManager::DrawFrameAxes(const std::string identifier, const Pose3d& pose,
                               const f32 scale_mm) {
  ...
  DrawSegment(identifier, pose.GetTranslation(), yHead, NamedColors::GREEN,
              false);
  DrawSegment(identifier, pose.GetTranslation(), zHead, NamedColors::BLUE,
              false);
  // TODO: add some arrow head decorations?
}

void VizManager::DrawCameraOval(const Point2f& center, float xRadius,
                                float yRadius, const Anki::ColorRGBA& color) {
  ANKI_CPU_PROFILE("VizManager::DrawCameraOval");

  SendMessage(VizInterface::MessageViz(VizInterface::CameraOval(
      (uint32_t)color, center.x(), center.y(), xRadius, yRadius)));
}

void VizManager::DrawCameraLine(const Point2f& start, const Point2f& end,
                                const ColorRGBA& color) {
  ANKI_CPU_PROFILE("VizManager::DrawCameraLine");

  SendMessage(VizInterface::MessageViz(VizInterface::CameraLine(
      (uint32_t)color, start.x(), start.y(), end.x(), end.y())));
}

void VizManager::DrawCameraPoly(const Poly2f& poly, const ColorRGBA& color,
                                const bool isClosed) {
  ANKI_CPU_PROFILE("VizManager::DrawCameraPoly");
  if (poly.size() < 2) {
    PRINT_NAMED_WARNING("VizManager.DrawCameraPoly.NotEnoughPoints",
                        "Polygon provided needs two or more points but has %zd",
                        poly.size());
    return;
  }
...
void VizManager::DrawCameraText(const Point2f& position,
  ...
  SendMessage(VizInterface::MessageViz(
      VizInterface::CameraText((uint32_t)color, std::round(position.x()),
                               std::round(position.y()), {text})));
}
...
void VizManager::DrawCameraFace(const Vision::TrackedFace& face,
  ...
  // Draw features
  for (s32 iFeature = 0; iFeature < (s32)Vision::TrackedFace::NumFeatures;
       ++iFeature) {
    Vision::TrackedFace::FeatureName featureName =
        (Vision::TrackedFace::FeatureName)iFeature;
    const Vision::TrackedFace::Feature& feature = face.GetFeature(featureName);

    for (size_t crntPoint = 0, nextPoint = 1; nextPoint < feature.size();
         ++crntPoint, ++nextPoint) {
  ...
  if (Vision::FacialExpression::Unknown != expression) {
    ...
    // Draw expression score histogram (NOTE: sum of all OKAO expression scores
    // is 100)
    ...
    const s32 barWidth = std::round((1.f - (2.f * kBarFraction)) *
                                    (f32)face.GetRect().GetWidth() /
                                    (f32)expressionValues.size());
    f32 xLeft = face.GetRect().GetBottomLeft().x() +
                kBarFraction * face.GetRect().GetWidth();
    for (s32 iExp = 0; iExp < expressionValues.size(); ++iExp) {
      const auto value = expressionValues[iExp];
      const s32 barHeight =
          std::round((f32)value / kTotalScoreSum * face.GetRect().GetHeight() *
                     (0.5f - kBarFraction));
      if (barHeight > 0) {
        const f32 yTop = face.GetRect().GetBottomLeft().y() - barHeight -
                         kBarFraction * face.GetRect().GetHeight();
        ...
        DrawCameraText(Point2f{xLeft, yTop}, expStr.substr(0, 3),
  ...
  for (auto& info : debugScoreInfo) {
    ...
    name += "[" + std::to_string(info.matchedID) +
            "]=" + std::to_string(info.score);
  }

  DrawCameraText(Point2f(face.GetRect().GetX(), face.GetRect().GetYmax()), name,
                 color);
  ...
  // Draw smile amount bar along bottom of bounding quad for face. Thickness
  // (height) of bar corresponds to confidence.
  const Vision::SmileAmount& smile = face.GetSmileAmount();
  if (smile.wasChecked) {
    const f32 barHeight = std::max(1.f, smile.confidence * kBarFraction *
                                            (quad.GetMaxY() - quad.GetMinY()));
    const f32 barWidth =
        std::max(1.f, smile.amount * (quad.GetMaxX() - quad.GetMinX()));

    Rectangle<s32> smileBar(quad.GetBottomLeft().x(),
                            quad.GetBottomLeft().y() - barHeight, barWidth,
                            barHeight);
  ...
  if (blink.wasChecked) {
    ...
    {
      const f32 barHeight =
          (1.f - blink.blinkAmountLeft) * (quad.GetMaxY() - quad.GetMinY());
      Rectangle<s32> blinkBar(quad.GetTopLeft().x(), quad.GetTopLeft().y(),
                              barWidth, barHeight);
    ...
    {
      const f32 barHeight =
          (1.f - blink.blinkAmountRight) * (quad.GetMaxY() - quad.GetMinY());
      Rectangle<s32> blinkBar(quad.GetTopRight().x() - barWidth,
                              quad.GetTopRight().y(), barWidth, barHeight);
  ...
  if (gaze.wasChecked) {
    ...
    lineEnd.x() += face.GetRect().GetWidth() * 0.5f *
                   std::sinf(DEG_TO_RAD(gaze.leftRight_deg));

    // Note that we subtract for y because positive y is down in the image
    lineEnd.y() -= face.GetRect().GetHeight() * 0.5f *
                   std::sinf(DEG_TO_RAD(gaze.upDown_deg));
...
void VizManager::EraseRobot(const u32 robotID) {
  ...
  DEV_ASSERT(robotID < _VizObjectMaxID[viztype],
             "VizManager.EraseRobot.InvalidID");
...
void VizManager::EraseCuboid(const u32 blockID) {
  ...
  DEV_ASSERT(blockID < _VizObjectMaxID[viztype],
             "VizManager.EraseCuboid.InvalidID");
...
void VizManager::ErasePreDockPose(const u32 preDockPoseID) {
  ...
  DEV_ASSERT(preDockPoseID < _VizObjectMaxID[viztype],
             "VizManager.ErasePreDockPose.InvalidID");
...
void VizManager::ErasePoly(u32 __polyID) {
  ...
  // TODO: For now polys are drawn using the path drawing logic, but when it
  // gets implmeneted properly this should be updated
  ErasePath(pathId);
}

// ================== Object drawing methods ====================

void VizManager::DrawObject(const u32 objectID, VizObjectType objectTypeID,
                            const Anki::Point3f& size_mm,
                            const Anki::Pose3d& pose, const ColorRGBA& color,
                            const f32* params, const std::string& text) {
...
void VizManager::EraseVizObject(const Handle_t objectID) {
  ...
  SendMessage(
      VizInterface::MessageViz(VizInterface::EraseObject(objectID, 0, 0)));
}
...
void VizManager::EraseAllVizObjects() {
  ...
  SendMessage(VizInterface::MessageViz(
      VizInterface::EraseObject((uint32_t)VizConstants::ALL_OBJECT_IDs, 0, 0)));
}

void VizManager::EraseVizObjectType(const VizObjectType type) {
  ANKI_CPU_PROFILE("VizManager::EraseVizObjectType");

  SendMessage(VizInterface::MessageViz(VizInterface::EraseObject(
      (uint32_t)VizConstants::OBJECT_ID_RANGE, VizObjectBaseID[(int)type],
      VizObjectBaseID[(int)type + 1] - 1)));
}
// ================== Path drawing methods ====================

void VizManager::DrawPath(const u32 pathID, const Planning::Path& p,
                          const ColorRGBA& color) {
  ErasePath(pathID);
  ...
  for (int s = 0; s < p.GetNumSegments(); ++s) {
    ...
    switch (p.GetSegmentConstRef(s).GetType()) {
      case Planning::PST_LINE:
        AppendPathSegmentLine(pathID, seg.line.startPt_x, seg.line.startPt_y,
                              seg.line.endPt_x, seg.line.endPt_y);
        break;
      case Planning::PST_ARC:
        AppendPathSegmentArc(pathID, seg.arc.centerPt_x, seg.arc.centerPt_y,
                             seg.arc.radius, seg.arc.startRad,
                             seg.arc.sweepRad);
        break;
      default:
        break;
...
void VizManager::AppendPathSegmentLine(const u32 pathID, const f32 x_start_mm,
                                       const f32 y_start_mm, const f32 x_end_mm,
                                       const f32 y_end_mm) {
...
void VizManager::AppendPathSegmentArc(const u32 pathID, const f32 x_center_mm,
                                      const f32 y_center_mm,
                                      const f32 radius_mm, const f32 startRad,
                                      const f32 sweepRad) {
...
void VizManager::EraseQuad(const uint32_t quadType, const u32 quadID) {
  ...
  SendMessage(
      VizInterface::MessageViz(VizInterface::EraseQuad(quadType, quadID)));
}
...
void VizManager::EraseAllQuads() {
  EraseQuad((uint32_t)VizConstants::ALL_QUAD_TYPEs,
            (uint32_t)VizConstants::ALL_QUAD_IDs);
}
...
void VizManager::EraseSegments(const std::string& identifier) {
  ...
  SendMessage(
      VizInterface::MessageViz(VizInterface::EraseLineSegments{identifier}));
}

// =============== Circle methods ==================

void VizManager::EraseCircle(u32 polyID) { ErasePoly(polyID); }

// =============== Text methods ==================

void VizManager::SetText(const TextLabelType& labelType, const ColorRGBA& color,
                         const char* format, ...) {
  ...
  va_end(argptr);
  SendMessage(VizInterface::MessageViz(VizInterface::SetLabel(
      (uint32_t)labelType, (uint32_t)color, {std::string(buffer)})));
}

// ============== Misc. Debug methods =================
void VizManager::SetDockingError(const f32 x_dist, const f32 y_dist,
                                 const f32 z_dist, const f32 angle) {
  ANKI_CPU_PROFILE("VizManager::SetDockingError");
  SendMessage(VizInterface::MessageViz(
      VizInterface::DockingErrorSignal(x_dist, y_dist, z_dist, angle)));
}

void VizManager::SendCameraParams(const Vision::CameraParams& params) {
  ANKI_CPU_PROFILE("VizManager::SendCameraParams");
  SendMessage(
      VizInterface::MessageViz(VizInterface::CameraParams(std::move(params))));
}
...
void VizManager::SendCurrentAnimation(const std::string& animName, u8 animTag) {
  ...
  SendMessage(VizInterface::MessageViz(
      VizInterface::CurrentAnimation(animTag, animName)));
}

void VizManager::SendBehaviorStackDebug(
    VizInterface::BehaviorStackDebug&& behaviorStackDebug) {
  ANKI_CPU_PROFILE("VizManager::SendBehaviorStackDebug");
  SendMessage(VizInterface::MessageViz(std::move(behaviorStackDebug)));
}

void VizManager::SendVisionModeDebug(
    VizInterface::VisionModeDebug&& visionModeDebug) {
  ANKI_CPU_PROFILE("VizManager::SendVisionModeDebug");
  SendMessage(VizInterface::MessageViz(std::move(visionModeDebug)));
}

void VizManager::SendEnabledVisionModes(
    VizInterface::EnabledVisionModes&& modes) {
...
void VizManager::SendTrackerQuad(const u16 topLeft_x, const u16 topLeft_y,
                                 ...
                                 const u16 bottomRight_x,
                                 const u16 bottomRight_y,
                                 const u16 bottomLeft_x,
                                 const u16 bottomLeft_y) {
...
void VizManager::SubscribeToEngineEvents(
    IExternalInterface& externalInterface) {
  auto helper = AnkiEventUtil<VizManager, decltype(_eventHandlers)>(
      externalInterface, *this, _eventHandlers);
