 ...
 *              world needs to invoke the cozmo_physics plugin in order for this
 *to work.
 *
...
class VizManager {
 public:
  ...
  void DrawRobot(const Pose3d& pose, const f32 headAngle, const f32 liftAngle);
  ...
  Handle_t DrawRobot(const u32 robotID, const Pose3d& pose,
                     const ColorRGBA& color = ::Anki::NamedColors::DEFAULT);

  Handle_t DrawCuboid(const u32 blockID, const Point3f& size,
                      const Pose3d& pose,
                      const ColorRGBA& color = ::Anki::NamedColors::DEFAULT);

  Handle_t DrawPreDockPose(
      const u32 preDockPoseID, const Pose3d& pose,
      const ColorRGBA& color = ::Anki::NamedColors::DEFAULT);

  Handle_t DrawCharger(const u32 chargerID, const f32 platformLength,
                       const f32 slopeLength, const f32 width, const f32 height,
                       const Pose3d& pose,
                       const ColorRGBA& color = ::Anki::NamedColors::DEFAULT);

  Handle_t DrawHumanHead(const s32 headID, const Point3f& size,
                         const Pose3d& pose,
                         const ColorRGBA& color = ::Anki::NamedColors::DEFAULT);

  // Draws XYZ axes as corresponding RGB lines
  void DrawFrameAxes(const std::string identifier, const Pose3d& pose,
                     const f32 scale_mm = 100.f);

  void DrawCameraFace(const Vision::TrackedFace& face, const ColorRGBA& color);
  ...
  void DrawObject(const u32 objectID, VizObjectType objectTypeID,
                  const Point3f& size, const Pose3d& pose,
                  const ColorRGBA& color = ::Anki::NamedColors::DEFAULT,
                  const f32* params = nullptr, const std::string& text = "");
  ...
  void DrawPath(const u32 pathID, const Planning::Path& p,
                const ColorRGBA& color = ::Anki::NamedColors::DEFAULT);

  // Appends the specified line segment to the path with id pathID
  void AppendPathSegmentLine(const u32 pathID, const f32 x_start_mm,
                             const f32 y_start_mm, const f32 x_end_mm,
                             const f32 y_end_m);

  // Appends the specified arc segment to the path with id pathID
  void AppendPathSegmentArc(const u32 pathID, const f32 x_center_mm,
                            const f32 y_center_mm, const f32 radius_mm,
                            const f32 startRad, const f32 sweepRad);
  ...
  template <typename T>
  void DrawGenericQuad(const u32 quadID, const Quadrilateral<3, T>& quad,
                       const ColorRGBA& color);

  // Draws a generic 2D quadrilateral in the XY plane at the specified Z height
  template <typename T>
  void DrawGenericQuad(const u32 quadID, const Quadrilateral<2, T>& quad,
                       const T zHeight, const ColorRGBA& color);

  // Draw a generic 2D quad in the camera display
  // TopColor is the color of the line connecting the upper left and upper right
  // corners.
  template <typename T>
  void DrawCameraQuad(const Quadrilateral<2, T>& quad, const ColorRGBA& color);
  template <typename T>
  void DrawCameraQuad(const Quadrilateral<2, T>& quad, const ColorRGBA& color,
  ...
  template <typename T>
  void DrawCameraRect(const Rectangle<T>& rect, const ColorRGBA& color,
                      bool filled = false);

  // Draw a line segment in the camera display
  void DrawCameraLine(const Point2f& start, const Point2f& end,
                      const ColorRGBA& color);

  // Draw a polygon (as a set of line segments) in the camera display
  void DrawCameraPoly(const Poly2f& poly, const ColorRGBA& color,
                      const bool isClosed = true);

  // Draw an oval in the camera display
  void DrawCameraOval(const Point2f& center, float xRadius, float yRadius,
                      const ColorRGBA& color);

  // Draw text in the camera display
  void DrawCameraText(const Point2f& position, const std::string& text,
                      const ColorRGBA& color);

  template <typename T>
  void DrawMatMarker(const u32 quadID, const Quadrilateral<3, T>& quad,
                     const ColorRGBA& color);

  template <typename T>
  void DrawRobotBoundingBox(const u32 quadID, const Quadrilateral<3, T>& quad,
                            const ColorRGBA& color);

  template <typename T>
  void DrawPoseMarker(const u32 quadID, const Quadrilateral<2, T>& quad,
                      const ColorRGBA& color);

  // Draw quads of a specified type (usually called as a helper by the
  // above methods for specific types)
  template <typename T>
  void DrawQuad(const VizQuadType quadType, const u32 quadID,
                const Quadrilateral<2, T>& quad, const T zHeight_mm,
                const ColorRGBA& color);

  template <typename T>
  void DrawQuad(const VizQuadType quadType, const u32 quadID,
                const Quadrilateral<3, T>& quad, const ColorRGBA& color);

  template <typename T>
  void DrawPoly(const u32 polyID, const Polygon<2, T>& poly,
                const ColorRGBA& color);
  ...
  // This supports sending requests to draw segments without requiring to assign
  // a single ID to every one of them, but a group. Used for debugging purposes
  // where the underlaying geometry is not directly related to a given object

  template <typename T>
  void DrawSegment(const std::string& identifier, const Point<3, T>& from,
                   const Point<3, T>& to, const ColorRGBA& color,
                   bool clearPrevious, float zOffset = 0.0f);
  void EraseSegments(const std::string& identifier);

  // circle as segments
  template <typename T>
  void DrawXYCircleAsSegments(const std::string& identifier,
                              const Point<3, T>& center, const T radius,
                              const ColorRGBA& color, bool clearPrevious,
                              u32 numSegments = 8, const T startAngle = T());

  // non-axis aligned quads as 4 segments
  template <typename T>
  void DrawQuadAsSegments(const std::string& identifier,
                          const Quadrilateral<2, T>& quad, T z,
                          const ColorRGBA& color, bool clearPrevious);
  template <typename T>
  void DrawQuadAsSegments(const std::string& identifier,
  ...
  // ==== Circle functions =====
  template <typename T>
  void DrawXYCircle(u32 polyID, const ColorRGBA& color,
                    const Point<2, T>& center, const T radius,
                    u32 numSegments = 20);

  void EraseCircle(u32 polyID);

  // ==== Text functions =====
  void SetText(const TextLabelType& labelType, const ColorRGBA& color,
               const char* format, ...);

  Handle_t DrawTextAtPose(const u32 textObjectID, const std::string& text,
                          const ColorRGBA& color, const Pose3d& pose);

  // ==== Misc. Debug functions =====
  void SetDockingError(const f32 x_dist, const f32 y_dist, const f32 z_dist,
                       const f32 angle);
 ...
 protected:
  ...
  // TODO: Won't need this offest once Polygon is implmeneted correctly (not
  // drawing with path)
...
template <typename T>
void VizManager::DrawQuad(const VizQuadType quadType, const u32 quadID,
                          const Quadrilateral<2, T>& quad, const T zHeight_mm,
                          const ColorRGBA& color) {
...
template <typename T>
void VizManager::DrawPoly(const u32 __polyID, const Polygon<2, T>& poly,
                          const ColorRGBA& color) {
  ...
  for (size_t i = 0; i < numPts; ++i) {
    size_t j = (i + 1) % numPts;
    polyPath.AppendLine(poly[i].x(), poly[i].y(), poly[j].x(), poly[j].y(), 1.0,
                        1.0, 1.0);
  }
...
template <typename T>
void VizManager::DrawCameraRect(const Rectangle<T>& rect,
                                const ColorRGBA& color, bool filled) {
...
void VizManager::DrawSegment(const std::string& identifier,
                             const Point<3, T>& from, const Point<3, T>& to,
                             const ColorRGBA& color, bool clearPrevious,
                             float zOffset) {
...
// - - - - - - - - - - - - - - - - - -
template <typename T>
void VizManager::DrawXYCircleAsSegments(const std::string& identifier,
                                        const Point<3, T>& center,
                                        const T radius, const ColorRGBA& color,
                                        bool clearPrevious, u32 numSegments,
                                        const T startAngle) {
  // Note we create the polygon clockwise intentionally
  T anglePerSegment =
      static_cast<T>(-2) * static_cast<T>(M_PI) / static_cast<T>(numSegments);

  // Use the tangential and radial factors to draw the segments without
  // recalculating every time. Algorithm found here:
  ...
  for (u32 i = 0; i < numSegments; ++i) {
    ...
    DrawSegment(identifier, prevPoint, newPoint, color,
                (i == 0) && (clearPrevious));
  }
}
...
// - - - - - - - - - - - - - - - - - -
template <typename T>
void VizManager::DrawQuadAsSegments(const std::string& identifier,
                                    const Quadrilateral<2, T>& quad, T z,
                                    const ColorRGBA& color,
                                    bool clearPrevious) {
  const Point<3, T> topLeft = {quad[Quad::CornerName::TopLeft].x(),
                               quad[Quad::CornerName::TopLeft].y(), z};
  const Point<3, T> topRight = {quad[Quad::CornerName::TopRight].x(),
                                quad[Quad::CornerName::TopRight].y(), z};
  const Point<3, T> bottomLeft = {quad[Quad::CornerName::BottomLeft].x(),
                                  quad[Quad::CornerName::BottomLeft].y(), z};
  const Point<3, T> bottomRight = {quad[Quad::CornerName::BottomRight].x(),
                                   quad[Quad::CornerName::BottomRight].y(), z};
...
template <typename T>
void VizManager::DrawXYCircle(u32 polyID, const ColorRGBA& color,
                              const Point<2, T>& center, const T radius,
                              u32 numSegments) {
  // Note we create the polygon clockwise intentionally
  T anglePerSegment =
