 ...
 * Created: 10-24-2016
 *
 * Description: Implements a container for mirroring on the main thread any pet
 *faces detected on the vision system thread.
 *
...
CONSOLE_VAR(f32, kHeadTurnSpeedThreshPet_degs,
            "WasRotatingTooFast.Pet.Head_deg/s", 10.f);
CONSOLE_VAR(f32, kBodyTurnSpeedThreshPet_degs,
            "WasRotatingTooFast.Pet.Body_deg/s", 30.f);
CONSOLE_VAR(u8, kNumImuDataToLookBackPet,
            "WasRotatingTooFast.Pet.NumToLookBack", 5);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - -
void PetWorld::InitDependent(Vector::Robot* robot,
...
Result PetWorld::Update(const std::list<Vision::TrackedPet>& pets) {
  ...
  // while maintaining numTimesObserved
  {
    ...
    for (auto& petDetection : pets) {
      ...
      // (a) it's a new pet and we weren't rotating fast (try to avoid adding
      // hallucinated pets), OR (b) it's an existing pet and we want to keep it
      // (even if we were moving too fast)
      ...
      if (existingIter != _knownPets.end()) {
        ...
        const bool rotatingTooFastCheckEnabled =
            (Util::IsFltGT(kBodyTurnSpeedThreshPet_degs, 0.f) ||
  ...
  for (auto const& knownPetEntry : _knownPets) {
    ...
    if (!knownPet.IsBeingTracked())  // The very first time we see a pet, it is
                                     // not being "tracked" yet
    {
      ANKI_VERIFY(knownPet.GetNumTimesObserved() == 1,
                  "PetWorld.Update.NewPetDetectionShouldBeObservedOnce",
                  "ID:%d NumTimesObserved:%d", knownPet.GetID(),
                  knownPet.GetNumTimesObserved());

      if (ANKI_DEVELOPER_CODE) {
        // DEV code to make sure we don't re-log an event for the same ID twice
        // (unless our IDs roll over)
        static std::set<s32> DEBUG_broadcastIDs;
        static const Vision::FaceID_t kMaxPetID =
            4095;  // max output by OMCV detector
        auto insert = DEBUG_broadcastIDs.insert(knownPet.GetID());

        if (insert.second == false) {
          PRINT_NAMED_WARNING("PetWorld.Update.DuplicateEvent",
                              "Already logged event for Pet ID:%d",
                              knownPet.GetID());
        }

        if (knownPet.GetID() == kMaxPetID) {
          // Not likely to see kMaxID pets, but if we reset the ID a bunch in a
          // single session, we could conceivably overrun this and the IDs would
          // start over with ID=1. We shouldn't fail in this case.
          DEBUG_broadcastIDs.clear();
        }
      }

      DASMSG(robot.vision.detected_pet, "robot.vision.detected_pet",
             "Detected a pet");
    ...
    // Broadcast the detection for Game/SDK
    {
      ...
      RobotObservedPet observedPet(
          knownPet.GetID(), knownPet.GetTimeStamp(),
          knownPet.GetNumTimesObserved(), knownPet.GetScore(),
          CladRect(knownPet.GetRect().GetX(), knownPet.GetRect().GetY(),
    ...
    if (ANKI_DEV_CHEATS) {
      const ColorRGBA& vizColor =
          ColorRGBA::CreateFromColorIndex(knownPet.GetID());
      _robot->GetContext()->GetVizManager()->DrawCameraOval(
          Point2f(knownPet.GetRect().GetXmid(), knownPet.GetRect().GetYmid()),
          knownPet.GetRect().GetWidth() * 0.5f,
          knownPet.GetRect().GetHeight() * 0.5f, vizColor);

      const s32 kStrLen = 16;
      char strbuffer[kStrLen];
      snprintf(strbuffer, kStrLen, "%s%d[%d]",
               knownPet.GetType() == Vision::PetType::Cat ? "CAT" : "DOG",
               knownPet.GetID(), knownPet.GetNumTimesObserved());

      _robot->GetContext()->GetVizManager()->DrawCameraText(
          Point2f(knownPet.GetRect().GetX(), knownPet.GetRect().GetY()),
          strbuffer, vizColor);
    }
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - -
std::set<Vision::FaceID_t> PetWorld::GetKnownPetsWithType(
    Vision::PetType type) const {
