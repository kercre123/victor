 ...
 *              Succeeds if the robot can see the given object or face in its
 *current pose.
 *
 *
...
class IVisuallyVerifyAction : public IAction {
 public:
  ...
  IVisuallyVerifyAction(const std::string name, const RobotActionType type,
                        VisionMode imageTypeToWaitFor, LiftPreset liftPosition);
...
class VisuallyVerifyObjectAction : public IVisuallyVerifyAction {
 public:
  VisuallyVerifyObjectAction(ObjectID objectID, Vision::Marker::Code whichCode =
                                                    Vision::Marker::ANY_CODE);

  virtual ~VisuallyVerifyObjectAction();

  // When called, this will cause the action to use the "cycling exposure"
  // vision mode when looking for the object. This is useful for more robustly
  // verifying an object under adverse lighting conditions, for example. Note
  // that this will also set NumImagesToWaitFor to a value appropriate for
  // cycling exposure mode. Note: This must be called before the action is
  // started.
...
class VisuallyVerifyNoObjectAtPoseAction : public IAction {
 public:
  VisuallyVerifyNoObjectAtPoseAction(const Pose3d& pose,
                                     const Point3f& thresholds_mm = {10, 10,
                                                                     10});
