...
IVisuallyVerifyAction::IVisuallyVerifyAction(const std::string name,
    ...
    : IAction(name, type, (u8)AnimTrackFlag::HEAD_TRACK),
...
void IVisuallyVerifyAction::SetupEventHandler(EngineToGameTag tag,
                                              EventCallback callback) {
  _observationHandle =
      GetRobot().GetExternalInterface()->Subscribe(tag, callback);
}
...
VisuallyVerifyObjectAction::VisuallyVerifyObjectAction(
    ...
    : IVisuallyVerifyAction(
          ...
          RobotActionType::VISUALLY_VERIFY_OBJECT, VisionMode::Markers,
...
void VisuallyVerifyObjectAction::SetUseCyclingExposure() {
  ...
  // The CyclingExposure mode cycles exposures every 5 frames, with a cycle
  // length of 3. Therefore, wait for 15 images. Note: This should be computed
  // directly from the vision config instead (VIC-12803)
  const int kNumImagesToWaitFor = 15;
  SetNumImagesToWaitFor(kNumImagesToWaitFor);
}

void VisuallyVerifyObjectAction::GetRequiredVisionModes(
  ...
  if (_useCyclingExposure) {
    requests.insert(
        {VisionMode::AutoExp_Cycling, EVisionUpdateFrequency::High});
  }
}
...
bool VisuallyVerifyObjectAction::HaveSeenObject() {
  if (_objectSeen) {
    if (!_markerSeen) {
      // We've seen the object, check if we've seen the correct marker if one
      // was specified and we haven't seen it yet
      ObservableObject* object =
          GetRobot().GetBlockWorld().GetLocatedObjectByID(_objectID);
      if (object == nullptr) {
        PRINT_NAMED_WARNING(
            "VisuallyVerifyObjectAction.HaveSeenObject.ObjectNotFound",
            "[%d] Object with ID=%d no longer exists in the world.", GetTag(),
            _objectID.GetValue());
        return false;
      }

      // Look for which markers were seen since (and including) last observation
      // time
      ...
      if (!_markerSeen) {
        ...
        LOG_INFO(
            "VisuallyVerifyObjectAction.HaveSeenObject.WrongMarker",
            "[%d] Have seen object %d, but not marker code %d. Have seen: %s",
            GetTag(), _objectID.GetValue(), _whichCode,
            observedMarkerNames.c_str());
      }
    }  // if(!_markerSeen)
...
VisuallyVerifyFaceAction::VisuallyVerifyFaceAction(Vision::FaceID_t faceID)
    : IVisuallyVerifyAction("VisuallyVerifyFace" + std::to_string(faceID),
                            ...
                            VisionMode::Faces, LiftPreset::LOW_DOCK),
      _faceID(faceID) {}

VisuallyVerifyFaceAction::~VisuallyVerifyFaceAction() {}

void VisuallyVerifyFaceAction::GetRequiredVisionModes(
    std::set<VisionModeRequest>& requests) const {
...
void VisuallyVerifyNoObjectAtPoseAction::GetRequiredVisionModes(
    std::set<VisionModeRequest>& requests) const {
  requests.insert({VisionMode::Markers, EVisionUpdateFrequency::High});
}
  ...
  _turnTowardsPoseAction.reset(new CompoundActionParallel(
      {new TurnTowardsPoseAction(_pose),
       new MoveLiftToHeightAction(
           MoveLiftToHeightAction::Preset::OUT_OF_FOV)}));
  _turnTowardsPoseAction->SetRobot(&GetRobot());
  if (_waitForImagesAction != nullptr) {
    _waitForImagesAction->PrepForCompletion();
  }
  _waitForImagesAction.reset(
      new WaitForImagesAction(_numImagesToWaitFor, VisionMode::Markers));
...
void VisuallyVerifyNoObjectAtPoseAction::AddIgnoreID(const ObjectID& objID) {
  if (HasStarted()) {
    ...
    PRINT_NAMED_WARNING(
        "VisuallyVerifyNoObjectAtPoseAciton.AddIgnoreID.ActionAlreadyStarted",
        "");
  } else {
...
ActionResult VisuallyVerifyNoObjectAtPoseAction::CheckIfDone() {
  ...
  // If the turnTowardsPoseAction is null then it must have completed so tick
  // the waitForImagesAction if it succeeds then that means we went
  // _numImagesToWaitFor without seeing an object close to _pose so this action
  // will succeed
    ...
    // Only do this check once we have turned towards the pose and have started
    // waiting for images in case there isn't actually an object at the pose but
    // blockworld thinks there is
    if (GetRobot().GetBlockWorld().FindLocatedObjectClosestTo(
            _pose, _thresholds_mm, _filter) != nullptr) {
      LOG_DEBUG("VisuallyVerifyNoObjectAtPose.FoundObject",
                "Seeing object near pose (%f %f %f)",
                _pose.GetTranslation().x(), _pose.GetTranslation().y(),
