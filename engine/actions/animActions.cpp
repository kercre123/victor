 ...
 * Date:   8/29/2014
 *
 * Description: Implements animation and audio cozmo-specific actions, derived
 *from the IAction interface.
 *
 *
...
namespace {
static constexpr const char* kManualBodyTrackLockName =
    "PlayAnimationOnChargerSpecialLock";
...
PlayAnimationAction::PlayAnimationAction(
    const std::string& animName, u32 numLoops, bool interruptRunning,
    u8 tracksToLock, float timeout_sec, TimeStamp_t startAtTime_ms,
    AnimationComponent::AnimationCompleteCallback callback)
    : IAction("PlayAnimation" + animName, RobotActionType::PLAY_ANIMATION,
              tracksToLock),
      _animName(animName),
  ...
  if ((numLoops == 0) && (timeout_sec == _kDefaultTimeout_sec)) {
...
PlayAnimationAction::~PlayAnimationAction() {
  if (HasStarted() && !_stoppedPlaying) {
    LOG_INFO("PlayAnimationAction.Destructor.StillStreaming",
             "Action destructing, but AnimationComponent is still playing: %s. "
             "Telling it to stop.",
             _animName.c_str());
    if (HasRobot()) {
  ...
  if (HasStarted() && _bodyTrackManuallyLocked) {
    GetRobot().GetMoveComponent().UnlockTracks((u8)AnimTrackFlag::BODY_TRACK,
                                               kManualBodyTrackLockName);
    _bodyTrackManuallyLocked = false;
  }
}

void PlayAnimationAction::OnRobotSet() { OnRobotSetInternalAnim(); }
...
void PlayAnimationAction::InitTrackLockingForCharger() {
  ...
  if (GetRobot().GetBatteryComponent().IsOnChargerPlatform() &&
    ...
    const bool onWhitelist =
        dataLoader->IsAnimationAllowedToMoveBodyOnCharger(_animName);
    if (!onWhitelist) {
      // time to lock the body track. Unfortunately, the action has already been
      // Init'd, so it's tracks are already locked. Therefore we have to
      // manually lock the body to make this work
      GetRobot().GetMoveComponent().LockTracks(
          (u8)AnimTrackFlag::BODY_TRACK, kManualBodyTrackLockName,
...
ActionResult PlayAnimationAction::Init() {
  ...
  Result res = GetRobot().GetAnimationComponent().PlayAnimByName(
      _animName, _numLoopsRemaining, _interruptRunning, callback, GetTag(),
  ...
  if (res != RESULT_OK) {
    ...
    GetRobot().GetAnimationComponent().AddAdditionalAnimationCallback(
        _animName, _passedInCallback, callEvenIfAnimCanceled);
  }

  GetRobot().GetComponent<RobotStatsTracker>().IncrementBehaviorStat(
      BehaviorStat::AnimationPlayed);
...
void PlayAnimationAction::SendStatsToDasAndWeb(
    const std::string& animClipName, const std::string& animGroupName,
    const AnimationTrigger& animTrigger) {
  const auto simpleMood = GetRobot().GetMoodManager().GetSimpleMood();
  const float headAngle_deg =
      Util::RadToDeg(GetRobot().GetComponent<FullRobotPose>().GetHeadAngle());
  ...
  if (animTrigger != AnimationTrigger::Count) {
    const std::set<AnimationTrigger>& dasBlacklistedTriggers =
        dataLoader->GetDasBlacklistedAnimationTriggers();
    isBlacklisted = dasBlacklistedTriggers.find(animTrigger) !=
                    dasBlacklistedTriggers.end();
    animTriggerStr = AnimationTriggerToString(animTrigger);
  } else {
    const std::set<std::string>& dasBlacklistedNames =
        dataLoader->GetDasBlacklistedAnimationNames();
    isBlacklisted =
        dasBlacklistedNames.find(animClipName) != dasBlacklistedNames.end();
  }

  if (!isBlacklisted) {
    // NOTE: you can add events to the blacklist in das_event_config.json to
    // block them from sending here.

    DASMSG(action_play_animation, "action.play_animation",
           "An animation action has been started on the robot (that wasn't "
           "blacklisted for DAS)");
    ...
    DASMSG_SET(i1, std::round(headAngle_deg),
               "The current head angle (in degrees)");
    DASMSG_SEND();
  }

  if (auto webSender = WebService::WebVizSender::CreateWebVizSender(
          "animationengine", GetRobot().GetContext()->GetWebService())) {
...
void PlayAnimationAction::GetCompletionUnion(
...
TriggerAnimationAction::TriggerAnimationAction(
    AnimationTrigger animEvent, u32 numLoops, bool interruptRunning,
    u8 tracksToLock, float timeout_sec, bool strictCooldown)
    : PlayAnimationAction("", numLoops, interruptRunning, tracksToLock,
                          timeout_sec),
...
void TriggerAnimationAction::SetAnimGroupFromTrigger(
...
ActionResult TriggerAnimationAction::Init() {
  if (_animGroupName.empty()) {
    LOG_WARNING("TriggerAnimationAction.NoAnimationForTrigger", "Event: %s",
                EnumToString(_animTrigger));

    return ActionResult::NO_ANIM_NAME;
  }

  _animName = GetRobot().GetAnimationComponent().GetAnimationNameFromGroup(
      _animGroupName, _strictCooldown);
...
PlayAnimationGroupAction::PlayAnimationGroupAction(
    const std::string& animGroupName)
    : PlayAnimationAction(""), _animGroupName(animGroupName) {
  SetName("PlayAnimationGroup");
}

ActionResult PlayAnimationGroupAction::Init() {
  if (_animGroupName.empty()) {
    LOG_ERROR("TriggerAnimationAction.NoAnimationGroupSet",
              "PlayAnimationGroup created with empty group name");
    return ActionResult::NO_ANIM_NAME;
  }

  const bool strictCooldown = false;
  _animName = GetRobot().GetAnimationComponent().GetAnimationNameFromGroup(
      _animGroupName, strictCooldown);
...
TriggerLiftSafeAnimationAction::TriggerLiftSafeAnimationAction(
    AnimationTrigger animEvent, u32 numLoops, bool interruptRunning,
    u8 tracksToLock, float timeout_sec, bool strictCooldown)
    : TriggerAnimationAction(animEvent, numLoops, interruptRunning,
                             tracksToLock, timeout_sec, strictCooldown) {}

u8 TriggerLiftSafeAnimationAction::TracksToLock(Robot& robot,
                                                u8 tracksCurrentlyLocked) {
  // Ensure animation doesn't throw cube down, but still can play get down
  // animations
  if (robot.GetCarryingComponent().IsCarryingObject() &&
      robot.GetOffTreadsState() == OffTreadsState::OnTreads) {
    tracksCurrentlyLocked =
        tracksCurrentlyLocked | (u8)AnimTrackFlag::LIFT_TRACK;
  }
...
ReselectingLoopAnimationAction::ReselectingLoopAnimationAction(
    ...
    : IAction(GetDebugString(animEvent),
              RobotActionType::RESELECTING_LOOP_ANIMATION, tracksToLock),
      _numLoops(numLoops),
  ...
  constexpr f32 defaultTimeout_s =
      PlayAnimationAction::GetDefaultTimeoutInSeconds();
  if ((numLoops == 0) && (_animParams.timeout_sec == defaultTimeout_s)) {
    _animParams.timeout_sec =
        PlayAnimationAction::GetInfiniteTimeoutInSeconds();
  }
}

std::string ReselectingLoopAnimationAction::GetDebugString(
    const AnimationTrigger& trigger) {
  return std::string{"ReselectingLoopAnimationAction"} +
...
void ReselectingLoopAnimationAction::GetCompletionUnion(
    ActionCompletedUnion& completionUnion) const {
  if (_subAction != nullptr) {
...
void ReselectingLoopAnimationAction::ResetSubAction() {
  ...
  _subAction.reset(new TriggerLiftSafeAnimationAction{
      _animParams.animEvent,
      1,  // only one loop here!
      _animParams.interruptRunning,
      // track locking is done by this action, don't double-lock
      (u8)AnimTrackFlag::NO_TRACKS, _animParams.timeout_sec,
...
ActionResult ReselectingLoopAnimationAction::CheckIfDone() {
  ...
  const ActionResultCategory category =
      IActionRunner::GetActionResultCategory(subActionResult);
...
void ReselectingLoopAnimationAction::StopAfterNextLoop() {
  if (!HasStarted()) {
    // StopAfterNextLoop() was called before Init(). Set a flag to stop on the
    // first call to CheckIfDone(), since the other flags (_numLoopsRemaining,
    // etc) get set during Init().
    _completeImmediately = true;
    LOG_INFO("ReselectingLoopAnimationAction.StopAfterNextLoop.NotStarted",
             "Action was told to StopAfterNextLoop, but hasn't started, so "
             "will end before the first loop");
  }
...
LoopAnimWhileAction::LoopAnimWhileAction(IActionRunner* primaryAction,
    ...
    : CompoundActionParallel(), _maxWaitTime_sec(maxWaitTime_sec) {
...
Result LoopAnimWhileAction::UpdateDerived() {
  ...
  if (primaryActionComplete && _timePrimaryActionCompleted < 0.f) {
    ...
    if (auto ptr = _animAction.lock()) {
      auto animActionPtr =
          std::static_pointer_cast<ReselectingLoopAnimationAction>(ptr);
  ...
  if (primaryActionComplete && hasMaxWaitTime &&
      (now_sec - _timePrimaryActionCompleted) > _maxWaitTime_sec) {
    LOG_WARNING("LoopAnimWhileAction.UpdateDerived.MaxWaitTimeExceeded",
                "The primary action has completed, and we have been waiting "
                "for the animation to complete for too "
