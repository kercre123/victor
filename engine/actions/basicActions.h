...
// Turn in place by a given angle, wherever the robot is when the action is
// executed.
//
// If isAbsolute==true, then angle_rad specifies the absolute body angle to turn
// to, and the robot will take the shortest path to the desired angle.
//
// If isAbsolute==false, then the robot will turn by the amount specified by
// angle_rad (which can be any arbitrarily large angular displacement, possibly
// greater than 180 degrees or possibly multiple turns)
class TurnInPlaceAction : public IAction {
 public:
  explicit TurnInPlaceAction(
      const float
          angle_rad,  // float instead of Radians to allow angles > 180 deg.
  ...
  void SetVariability(const Radians& angleVar_rad) {
    _variability = angleVar_rad;
  }
  void SetValidOffTreadsStates(const std::set<OffTreadsState>& states) {
    _validTreadStates = states;
  }

  virtual bool SetMotionProfile(
      const PathMotionProfile& motionProfile) override;
  virtual f32 GetTimeoutInSeconds() const override { return _timeout_s; }

  // Note: PROCEDURAL_EYE_LEADING is a compile-time option to enable/disable eye
  // leading
  void SetMoveEyes(bool enable) {
    _moveEyes = (enable && PROCEDURAL_EYE_LEADING);
  }
 ...
 private:
  ...
  const f32 _kMaxRelativeTurnRevs =
      25.f;  // Maximum number of revolutions allowed for a relative turn.
...
// A simple compound action which is useful for identifying blocks that are
// close to Cozmo's current frame of view.  Cozmo drives backwards slightly,
// looks left and right and up and down slightly to identify blocks that may be
// slightly outside the camera optionally an objectID can be passed in for the
// action to complete immediately on finding the object
class SearchForNearbyObjectAction : public IAction {
 public:
  ...
  SearchForNearbyObjectAction(
      ...
      f32 backupDistance_mm = Util::numeric_cast<f32>(
          Util::EnumToUnderlying(SFNOD::BackupDistance_mm)),
      f32 backupSpeed_mms = Util::numeric_cast<f32>(
          Util::EnumToUnderlying(SFNOD::BackupSpeed_mms)),
      f32 headAngle_rad = Util::numeric_cast<f32>(
...
class DriveStraightAction : public IAction {
 public:
  ...
  void SetShouldPlayAnimation(bool shouldPlay) {
    _shouldPlayDrivingAnimation = shouldPlay;
  }

  // By default, this action cannot move while on the charger (platform). This
  // function can be used to override this setting, and must be called before
...
class PanAndTiltAction : public IAction {
 public:
  ...
  PanAndTiltAction(Radians bodyPan, Radians headTilt, bool isPanAbsolute,
                   bool isTiltAbsolute);
...
class CalibrateMotorAction : public IAction {
 public:
  CalibrateMotorAction(bool calibrateHead, bool calibrateLift,
...
class MoveHeadToAngleAction : public IAction {
 public:
  enum class Preset : u8 {
    GROUND_PLANE_VISIBLE,  // at this head angle, the whole ground plane (or the
                           // max amount) will be visible
  ...
  // remain in their final position until the next time something moves the
  // head. Note: PROCEDURAL_EYE_LEADING is a compile-time option to
  // enable/disable eye leading
  void SetMoveEyes(bool enable, bool hold = false) {
    _moveEyes = (enable && PROCEDURAL_EYE_LEADING);
    _holdEyes = hold;
  }
...
class MoveLiftToAngleAction : public IAction {
 public:
  ...
  void SetMaxLiftSpeed(float speedRadPerSec) {
    _maxLiftSpeedRadPerSec = speedRadPerSec;
  }
  void SetLiftAccel(float accelRadPerSec2) {
    _liftAccelRacPerSec2 = accelRadPerSec2;
  }
...
class MoveLiftToHeightAction : public IAction {
 public:
  ...
  enum class Preset : u8 {
    ...
    HIGH_DOCK,
    CARRY,
    OUT_OF_FOV,       // Moves to low or carry, depending on which is closer to
                      // current height
    JUST_ABOVE_PROX,  // High enough to avoid the prox sensor, and improves
                      // driving over cluttered spaces
  };
...
class TurnTowardsPoseAction : public PanAndTiltAction {
 public:
  ...
  TurnTowardsPoseAction(const Pose3d& pose, Radians maxTurnAngle = M_PI_F);

  void SetMaxTurnAngle(Radians angle) { _maxTurnAngle = angle; }

  // compute the turn angles. Can be useful to check what will happen before
  // this action executes. Arguments are translations with respect to the robot
  static Radians GetAbsoluteHeadAngleToLookAtPose(
      const Point3f& translationWrtRobot);
  static Radians GetRelativeBodyAngleToLookAtPose(
      const Point3f& translationWrtRobot);
...
class TurnTowardsImagePointAction : public PanAndTiltAction {
 public:
  TurnTowardsImagePointAction(const Point2f& imgPoint,
                              const RobotTimeStamp_t imgTimeStamp);

  // Constructor for turning towards a SalientPoint, whose (x,y) location is in
  // normalized coordinates (and which has its own timestamp)
...
class WaitForImagesAction : public IAction {
 public:
  ...
  WaitForImagesAction(u32 numFrames, VisionMode visionMode = VisionMode::Count,
                      RobotTimeStamp_t afterTimeStamp = 0);

  struct UseDefaultNumImages_t {};
  static constexpr UseDefaultNumImages_t UseDefaultNumImages =
      UseDefaultNumImages_t{};

  // use a default number of images to give the robot a good chance to see
  // something with the given vision modes
  ...
  // If Mode is Stream, will save all images (irrespective of visionMode above)
  // while the action
  //   is running. In this case, the mode will be set back to off when the
  //   action ends, so anything that was previously saving will be disabled.
 ...
 protected:
  // This action will automatically subscribe to whatever VisionMode it is asked
  // to wait on
  virtual void GetRequiredVisionModes(
      std::set<VisionModeRequest>& requests) const override;
...
class TurnTowardsObjectAction : public TurnTowardsPoseAction {
 public:
  ...
  // will first turn to face the object, then tilt its head. To disallow
  // turning, set maxTurnAngle to zero.

  TurnTowardsObjectAction(ObjectID objectID, Radians maxTurnAngle = M_PI_F,
                          bool visuallyVerifyWhenDone = false,
                          bool headTrackWhenDone = false);

  TurnTowardsObjectAction(ObjectID objectID, Vision::Marker::Code whichCode,
  ...
  // usually, an object ID should be passed in in the constructor, but this
  // function can be called to specify an object pointer which may live outside
  // of blockworld
...
// Turn towards the last known face pose. Note that this action "succeeds"
// without doing anything if there is no face (unless requireFace is set to
// true) If a face is seen after we stop turning, "fine tune" the turn a bit and
// say the face's name if we recognize it (and sayName=true).
class TurnTowardsFaceAction : public TurnTowardsPoseAction {
 public:
  TurnTowardsFaceAction(const SmartFaceID& faceID,
                        Radians maxTurnAngle = M_PI_F, bool sayName = false);
  ...
  // was true. Cannot be used with SetAnyFaceAnimationTrigger
  ...
  // want to use this animation instead). Cannot be used with
  // SetAnyFaceAnimationTrigger.
  void SetNoNameAnimationTrigger(AnimationTrigger trigger);

  // Play an animation after turning to the face
  // (Mutually exclusive with
  // SetSayNameAnimationTrigger/SetNoNameAnimationTrigger)
  void SetAnyFaceAnimationTrigger(AnimationTrigger trigger);

  // instead of manually specifying a trigger, this function allows a lambda to
  // be called when the face is turned to. It is called right before the
  // animation should be played, only if the face is named. Input is the face we
  // are reacting to, and the return value should be the animation to play. If
  // AnimationTrigger::Count is returned, no animation will play. Cannot be used
  // with SetAnyFaceTriggerCallback
  using AnimTriggerForFaceCallback = std::function<AnimationTrigger(
      const Robot& robot, const SmartFaceID& faceID)>;
  ...
  // Cannot be used with SetAnyFaceTriggerCallback
  void SetNoNameTriggerCallback(AnimTriggerForFaceCallback&& callback);

  // same as above, but runs for any face.
  // (Mutually exclusive with SetSayNameTriggerCallback and
  // SetNoNameTriggerCallback)
  ...
  void SetAnimTracksToLock(u8 tracksToLock) {
    _animTracksToLock = tracksToLock;
  }

  // Sets whether or not we require a face. Default is false (it will play
  // animations and return success even if no face is found). If set to true and
  // no face is found, the action will fail with NO_FACE and no animations will
  // be played
  void SetRequireFaceConfirmation(bool isRequired) {
    _requireFaceConfirmation = isRequired;
  }

  // After turning toward the supplied faceID, this sets whether the action
  // locks onto the closest face in that direction (true) or if it should only
  // lock onto the supplied faceID (false). Defaults to false.
  void SetLockOnClosestFaceAfterTurn(bool shouldLock) {
    _lockOnClosestFace = shouldLock;
  }
 ...
 private:
  enum class State : u8 {
    ...
    PlayingAnimation,  // playing an recognition animation, possibly including
                       // TTS for the name
  };
...
class TurnTowardsLastFacePoseAction : public TurnTowardsFaceAction {
 public:
  TurnTowardsLastFacePoseAction(Radians maxTurnAngle = M_PI_F,
                                bool sayName = false)
...
class TurnTowardsFaceWrapperAction : public CompoundActionSequential {
 public:
  // Create a wrapper around the given action which looks towards a face before
  // and/or after (default before) the action. This takes ownership of action,
...
// Dummy action that just never finishes, can be useful for testing or holding
// the queue
class HangAction : public IAction {
 public:
  HangAction()
      : IAction("Hang", RobotActionType::HANG, (u8)AnimTrackFlag::NO_TRACKS) {}
...
class WaitForLambdaAction : public IAction {
 public:
  WaitForLambdaAction(std::function<bool(Robot&)> lambda,
      ...
      : IAction("WaitForLambda", RobotActionType::WAIT_FOR_LAMBDA,
