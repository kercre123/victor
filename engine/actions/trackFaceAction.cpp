 ...
 * Date:   12/11/2015
 *
 * Description: Defines an action for tracking (human) faces, derived from
 *ITrackAction
 *
 *
...
TrackFaceAction::TrackFaceAction(FaceID rawFaceID)
    : ITrackAction("TrackFace", RobotActionType::TRACK_FACE),
      _tmpFaceID(rawFaceID) {
  SetName("TrackFace" + std::to_string(rawFaceID));
}

TrackFaceAction::TrackFaceAction(SmartFaceID faceID)
    : ITrackAction("TrackFace", RobotActionType::TRACK_FACE), _faceID(faceID) {
...
void TrackFaceAction::GetRequiredVisionModes(
...
void TrackFaceAction::GetCompletionUnion(
...
ITrackAction::UpdateResult TrackFaceAction::UpdateTracking(
    Radians& absPanAngle, Radians& absTiltAngle, f32& distance_mm) {
  ...
  if (nullptr == face) {
    ...
    PRINT_CH_INFO(kLogChannelName, "TrackFaceAction.UpdateTracking.BadFaceID",
                  "No face %s in FaceWorld", _faceID.GetDebugStr().c_str());
    return UpdateResult::NoNewInfo;
  }

  // Only update pose if we've actually observed the face again since last
  // update
  ...
  if (false == face->GetHeadPose().GetWithRespectTo(GetRobot().GetPose(),
                                                    headPoseWrtRobot)) {
  ...
  const f32 zDist =
      headPoseWrtRobot.GetTranslation().z() - NECK_JOINT_POSITION[2];

  if (DEBUG_TRACKING_ACTIONS) {
    LOG_INFO("TrackFaceAction.UpdateTracking.HeadPose",
             "Translation w.r.t. robot = (%.1f, %.1f, %.1f) [t=%d]", xDist,
             yDist, zDist, face->GetTimeStamp());
  }
  ...
  absPanAngle = std::atan2(yDist, xDist) +
...
bool TrackFaceAction::AreContinueCriteriaMet(const f32 currentTime_sec) {
  if (!Util::IsFltNear(_eyeContactCriteria.earliestStoppingTime_sec, -1.f) &&
    ...
    const bool eyeContact = GetRobot().GetFaceWorld().IsMakingEyeContact(
        _eyeContactCriteria.eyeContactWithinLast_ms);
    if (eyeContact) {
    ...
    if (currentTime_sec < _eyeContactCriteria.earliestStoppingTime_sec) {
      ...
      if (eyeContact) {
        return true;
      } else if ((currentTime_sec -
                  _eyeContactCriteria.timeOfLastEyeContact_sec) <=
                 _eyeContactCriteria.noEyeContactTimeout_sec) {
        return true;
      }
    }
  } else {
    ...
    PRINT_NAMED_ERROR(
        "TrackFaceAction.AreContinueCriteriaMet.MissingContinueCriteria",
        "Both earliest stopping time and no eye contact timeout must be "
        "provided");
  }
  return false;
}

void TrackFaceAction::SetEyeContactContinueCriteria(
    const f32 minTimeToTrack_sec, const f32 noEyeContactTimeout_sec,
    const TimeStamp_t eyeContactWithinLast_ms) {
  DEV_ASSERT(!HasStarted(),
             "ITrackAction.SetEyeContactContinueCriteria.ActionAlreadyStarted");
  ...
  const auto currentTime_sec =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  _eyeContactCriteria.earliestStoppingTime_sec =
      currentTime_sec + minTimeToTrack_sec;
