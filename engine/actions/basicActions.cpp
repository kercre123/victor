 ...
 * Date:   8/29/2014
 *
 * Description: Implements basic cozmo-specific actions, derived from the
 *IAction interface.
 *
 *
...
// Whether or not to insert WaitActions before and after TurnTowardsObject's
// VisuallyVerifyAction
CONSOLE_VAR(bool, kInsertWaitsInTurnTowardsObjectVerify,
            "BasicActions.TurnTowardsObject", false);

CONSOLE_VAR(u32, kDefaultNumFramesToWait, "BasicActions.WaitForImages", 3);

CONSOLE_VAR(f32, kMaxTimeToWaitForRecognition_sec,
            "BasicActions.TurnTowardsFace", 3.f);

// The value of this console var should always be set to a value less than the
// value of `kMaxUnexpectedMovementCountWhileHeldInPalm` from the
// MovementComponent/UnexpectedMovement implementation, in order for
// `IsActionMakingProgress` to detect/trigger correctly.
CONSOLE_VAR_RANGED(u8, kMaxUnexpectedMoveCountHeldInPalm,
                   "BasicActions.TurnInPlace", 11, 1, 200);

TurnInPlaceAction::TurnInPlaceAction(const float angle_rad,
                                     const bool isAbsolute)
    : IAction("TurnInPlace", RobotActionType::TURN_IN_PLACE,
...
void TurnInPlaceAction::SetAccel(f32 accel_radPerSec2) {
  DEV_ASSERT(!_isInitialized,
             "TurnInPlaceAction.SetAccel.ActionAlreadyInitialized");
...
bool TurnInPlaceAction::SetMotionProfile(
    const PathMotionProfile& motionProfile) {
  DEV_ASSERT(!_isInitialized,
             "TurnInPlaceAction.SetMotionProfile.ActionAlreadyInitialized");
  if (_motionProfileManuallySet) {
    // don't want to use the custom profile since someone manually specified
    // speeds
    return false;
  } else {
...
void TurnInPlaceAction::SetTolerance(const Radians& angleTol_rad) {
  DEV_ASSERT(!_isInitialized,
             "TurnInPlaceAction.SetTolerance.ActionAlreadyInitialized");
...
inline Result TurnInPlaceAction::SendSetBodyAngle() {
  return GetRobot().GetMoveComponent().TurnInPlace(
      _currentTargetAngle.ToFloat(), _maxSpeed_radPerSec, _accel_radPerSec2,
      _angleTolerance.ToFloat(),

      // For relative turns, the total angle to turn can be greater than 180
      // degrees.
      //  So we need to tell the robot how 'far' it should turn. For absolute
      //  angles, the robot should always just take the shortest path to the
      //  desired angle.
      _isAbsoluteAngle
          ? 0
          : (uint16_t)std::floor(std::abs(_angularDistExpected_rad / M_PI_F)),
...
float TurnInPlaceAction::RecalculateTimeout() {
  // If the pan acceleration is too slow, the robot will never reach
  // _maxPanSpeed_radPerSec in the allowed _bodyPanAngle. The check to verify
  // this is: d_total/2 >= v_max^2 / (2 * a_max) This is rewritten below to
  // avoid float division, as: d_total * a_max >= v_max^2
  if (fabs(_angularDistExpected_rad * _accel_radPerSec2) >=
      _maxSpeed_radPerSec * _maxSpeed_radPerSec) {
    // The acceleration is sufficiently fast, we can calculate time of travel as
    // follows: t_total = t_accel + t_decel + (d_total - d_accel - d_decel) /
    // v_max Which simplifies (assuming t_accel == t_decel and d_accel ==
    // d_decel) to: t_total = v_max / a_max + d_total / v_max
    _expectedTotalAccelTime_s =
        2.0 * fabs(_maxSpeed_radPerSec / _accel_radPerSec2);
    const float totalTime_s =
        fabs(_maxSpeed_radPerSec / _accel_radPerSec2) +
        fabs(_angularDistExpected_rad / _maxSpeed_radPerSec);
    _expectedMaxSpeedTime_s = totalTime_s - _expectedTotalAccelTime_s;
    return totalTime_s;
  } else {
    // Otherwise, we can just assume we're accelerating and decelerating the
    // entire time, and therefore the following is true: d_total / 2  = (a_max /
    // 2) * (t_total / 2)^2 Or alternatively: (4 * d_total / a_max)^0.5 =
    // t_total
    _expectedMaxSpeedTime_s = 0.f;
    _expectedTotalAccelTime_s =
        sqrt(4.0 * fabs(_angularDistExpected_rad / _accel_radPerSec2));
...
ActionResult TurnInPlaceAction::Init() {
  ...
  DEV_ASSERT(GetRobot().GetPose().IsChildOf(GetRobot().GetWorldOrigin()),
  ...
  if (_variability != 0.f) {
    variabilityToAdd_rad = (float)GetRNG().RandDblInRange(
        -_variability.ToDouble(), _variability.ToDouble());
  }
  ...
  if (_isAbsoluteAngle) {
    ...
    if (std::abs(_requestedAngle_rad) > 2.f * M_PI_F * _kMaxRelativeTurnRevs) {
      PRINT_NAMED_WARNING(
          "TurnInPlaceAction.Init.AngleTooLarge",
    ...
    // Add the requested relative angle to the current heading to get the
    // absolute target angle.
    _currentTargetAngle =
        _currentAngle + _requestedAngle_rad + variabilityToAdd_rad;

    // The angular distance is simply the requested relative angle and any
    // variability
    //  (note: abs() of this can be greater than 2*PI rads).
    _angularDistExpected_rad = _requestedAngle_rad + variabilityToAdd_rad;

    // Also, for relative turns, the sign of the requested angle should dictate
    // the direction of
    //   the turn. (the robot uses the sign of _maxSpeed_radPerSec to decide
    //   which direction to turn)
    _maxSpeed_radPerSec =
        std::copysign(_maxSpeed_radPerSec, _requestedAngle_rad);
  }

  // Recalculate the timeout limit allowed for this turn, if the robot is held
  // on a palm since the treads tend to slip often, so we decrease the timeout
  // according to the expected runtime of the action.
  if (GetRobot().GetMoveComponent().IsHeldInPalmModeEnabled()) {
    // Increase the tolerance for reaching the target angle, since the robot
    // often experiences tread-slippage and the user is always rotating the
    // robot at least slightly when holding it in the palm, this decreases rate
    // of point-turn failures.
    SetTolerance(_kHeldInPalmAngleTolerance);

    // The movement component automatically clamps the robot's rotational speed
    // when it is being held in a palm, so in order to not mess up the
    // calculations in RecalculateTimeout(), clamp the max speed here too.
    const f32 speedCapWhileHeldInPalm =
        GetRobot().GetMoveComponent().GetMaxTurnSpeedWhileHeldInPalm_radps();
    if (fabs(_maxSpeed_radPerSec) > speedCapWhileHeldInPalm) {
      LOG_INFO("TurnInPlaceAction.Init.CappedMaxSpeed",
               "Movement component has HeldInPalmMode enabled, but max speed "
               "commanded was "
               "%.2f [rad/s], clamping to %.2f [rad/s]",
               fabs(_maxSpeed_radPerSec), speedCapWhileHeldInPalm);
  ...
  if (!_inPosition) {
    ...
    if (_moveEyes) {
      // Store the angular distance at which to remove eye shift (halfway
      // through the turn)
      _absAngularDistToRemoveEyeDart_rad =
          0.5f * std::abs(_angularDistExpected_rad);

      // Move the eyes (only if not in position)
      // Note: assuming screen is about the same x distance from the neck joint
      // as the head cam
      float angleDiff_rad = _angularDistExpected_rad;

      // Clip angleDiff to 89 degrees to prevent unintended behavior due to
      // tangent
      angleDiff_rad =
          Anki::Util::Clamp(angleDiff_rad, DEG_TO_RAD(-89.f), DEG_TO_RAD(89.f));

      const f32 x_mm = std::tan(angleDiff_rad) * HEAD_CAM_POSITION[0];
      const f32 xPixShift =
          x_mm * (static_cast<f32>(GetRobot().GetDisplayWidthInPixels()) /
                  (4 * SCREEN_SIZE[0]));
      GetRobot().GetAnimationComponent().AddOrUpdateEyeShift(
          _kEyeShiftLayerName, xPixShift, 0, 4 * ANIM_TIME_STEP_MS);
    }
  }

  // Subscribe to motor command ack
  auto actionStartedLambda =
      [this](const AnkiEvent<RobotInterface::RobotToEngine>& event) {
        if (_motionCommanded &&
            _actionID == event.GetData().Get_motorActionAck().actionID) {
          LOG_INFO("TurnInPlaceAction.MotorActionAcked", "[%d] ActionID: %d",
                   GetTag(), _actionID);
          _motionCommandAcked = true;
        }
      };

  _signalHandle = GetRobot().GetRobotMessageHandler()->Subscribe(
      RobotInterface::RobotToEngineTag::motorActionAck, actionStartedLambda);
...
bool TurnInPlaceAction::IsBodyInPosition(Radians& currentAngle) const {
  ...
  if (_isAbsoluteAngle) {
    // For absolute turns, we only care if we are near the target angle (we
    // don't care about the angle traversed)
    inPosition =
        currentAngle.IsNear(_currentTargetAngle, _angleTolerance.ToFloat());
  } else {
    const float absAngularDistToTarget_rad =
        std::abs(_angularDistExpected_rad - _angularDistTraversed_rad);

    // Only check if body is in position if we're within Pi radians of
    // completing
    //  the turn (to allow for multiple-rotation turns, e.g. 360 degrees).
    if (absAngularDistToTarget_rad < M_PI_F) {
      inPosition = currentAngle.IsNear(
          _currentTargetAngle,
          _angleTolerance.ToFloat() +
              Util::FLOATING_POINT_COMPARISON_TOLERANCE_FLT);

      // If we've relocalized during the turn, also consider the turn complete
      //  if we've turned through the entire expected angular distance (since
      //  the pose jump may cause the target vs. actual angle comparison to
      //  fail)
...
ActionResult TurnInPlaceAction::CheckIfDone() {
  ...
  if (_motionCommanded && !_motionCommandAcked) {
    PRINT_PERIODIC_CH_DEBUG(10, "Actions",
                            "TurnInPlaceAction.CheckIfDone.WaitingForAck",
                            "[%d] ActionID: %d", GetTag(), _actionID);
    return result;
  }
  ...
  if (_prevPoseFrameId != GetRobot().GetPoseFrameID()) {
    ...
    LOG_INFO("TurnInPlaceAction.CheckIfDone.PfidChanged",
             ...
             "No longer comparing angles to check if done - using angular "
             "distance traversed instead. "
  ...
  if (GetRobot().GetAnimationComponent().IsEyeShifting(_kEyeShiftLayerName)) {
    if (_inPosition || (std::abs(_angularDistTraversed_rad) >
                        _absAngularDistToRemoveEyeDart_rad)) {
      LOG_DEBUG(
          "TurnInPlaceAction.CheckIfDone.RemovingEyeShift",
          "Currently at %.1fdeg, on the way to %.1fdeg (traversed %.1fdeg)",
          _currentAngle.getDegrees(), _currentTargetAngle.getDegrees(),
  ...
  if (_inPosition) {
    ...
    LOG_INFO("TurnInPlaceAction.CheckIfDone.InPosition",
             "[%d] In Position: %.1fdeg vs. %.1fdeg(+/-%.1f), "
             "angDistTravd=%+.1fdeg, angDistExpc=%+.1fdeg (tol: %f) (pfid: %d)",
             GetTag(), _currentAngle.getDegrees(),
             _currentTargetAngle.getDegrees(), _variability.getDegrees(),
             RAD_TO_DEG(_angularDistTraversed_rad),
             RAD_TO_DEG(_angularDistExpected_rad), _angleTolerance.getDegrees(),
             GetRobot().GetPoseFrameID());
  } else {
    // Don't spam "AngleNotReached" messages
    PRINT_PERIODIC_CH_DEBUG(
        10, "Actions", "TurnInPlaceAction.CheckIfDone.AngleNotReached",
        "[%d] Waiting for body to reach angle: %.1fdeg vs. %.1fdeg(+/-%.1f), "
        "angDistTravd=%+.1fdeg, angDistExpc=%+.1fdeg (tol: %f) (pfid: %d)",
        GetTag(), _currentAngle.getDegrees(), _currentTargetAngle.getDegrees(),
        _variability.getDegrees(), RAD_TO_DEG(_angularDistTraversed_rad),
        RAD_TO_DEG(_angularDistExpected_rad), _angleTolerance.getDegrees(),
        GetRobot().GetPoseFrameID());
    ...
    if (_turnStarted) {
      if (!areWheelsMoving) {
        PRINT_NAMED_WARNING(
            "TurnInPlaceAction.CheckIfDone.WheelsStoppedMoving",
            "[%d] giving up since we stopped moving. currentAngle=%.1fdeg, "
            "target=%.1fdeg, angDistExp=%.1fdeg, angDistTrav=%.1fdeg (pfid: "
            "%d)",
            GetTag(), _currentAngle.getDegrees(),
            _currentTargetAngle.getDegrees(),
            RAD_TO_DEG(_angularDistExpected_rad),
            RAD_TO_DEG(_angularDistTraversed_rad), GetRobot().GetPoseFrameID());
        result = ActionResult::MOTOR_STOPPED_MAKING_PROGRESS;
      } else if (GetRobot().GetMoveComponent().IsHeldInPalmModeEnabled() &&
                 !IsActionMakingProgress()) {
        LOG_INFO("TurnInPlaceAction.CheckIfDone.StoppedMakingProgress",
                 "[%d] giving up, robot not turning at expected speed, "
                 "currentAngle=%.1f [deg], target=%.1f [deg], angDistExp=%.1f "
                 "[deg], angDistTrav=%.1f [deg]",
                 GetTag(), _currentAngle.getDegrees(),
                 _currentTargetAngle.getDegrees(),
                 RAD_TO_DEG(_angularDistExpected_rad),
                 RAD_TO_DEG(_angularDistTraversed_rad));
...
bool TurnInPlaceAction::IsActionMakingProgress() const {
  // This function is a custom implementation of how to handle unexpected
  // movement for point- turns when the robot is held in a palm, and essentially
  // triggers a "silent" failure of the action so as to not interrupt the flow
  // of the behavior that delegated to the action. This is because the normal
  // `ReactToUnexpectedMovementInAir` behavior is too jarring.
  const u8 unexpectedMovementCount =
      GetRobot().GetMoveComponent().GetUnexpectedMovementCount();
  const bool isActionMakingProgress =
      unexpectedMovementCount < kMaxUnexpectedMoveCountHeldInPalm;
  if (!isActionMakingProgress) {
    LOG_INFO(
        "TurnInPlaceAction.IsActionMakingProgress.UnexpectedMovementDetected",
...
void TurnInPlaceAction::GetCompletionUnion(
    ActionCompletedUnion& completionUnion) const {
...
SearchForNearbyObjectAction::SearchForNearbyObjectAction(
    const ObjectID& desiredObjectID, f32 backupDistance_mm, f32 backupSpeed_mms,
    f32 headAngle_rad)
  ...
  if (GetRobot().HasExternalInterface()) {
    ...
    auto observedObjectCallback =
        [this](const AnkiEvent<ExternalInterface::MessageEngineToGame>& event) {
          if (event.GetData().Get_RobotObservedObject().objectID ==
              _desiredObjectID) {
...
void SearchForNearbyObjectAction::SetSearchAngle(f32 minSearchAngle_rads,
                                                 f32 maxSearchAngle_rads) {
  _minSearchAngle_rads = minSearchAngle_rads;
  _maxSearchAngle_rads = maxSearchAngle_rads;
}

void SearchForNearbyObjectAction::SetSearchWaitTime(f32 minWaitTime_s,
                                                    f32 maxWaitTime_s) {
  _minWaitTime_s = minWaitTime_s;
  _maxWaitTime_s = maxWaitTime_s;
}

void SearchForNearbyObjectAction::GetRequiredVisionModes(
    std::set<VisionModeRequest>& requests) const {
...
ActionResult SearchForNearbyObjectAction::Init() {
  ...
  float firstAngle_rads =
      firstTurnDir *
      GetRNG().RandDblInRange(_minSearchAngle_rads, _maxSearchAngle_rads);
  float afterFirstTurnWait_s =
      GetRNG().RandDblInRange(_minWaitTime_s, _maxWaitTime_s);

  float secondAngle_rads =
      -firstAngle_rads -
      firstTurnDir *
          GetRNG().RandDblInRange(_minSearchAngle_rads, _maxSearchAngle_rads);
  float afterSecondTurnWait_s =
      GetRNG().RandDblInRange(_minWaitTime_s, _maxWaitTime_s);

  LOG_DEBUG("SearchForNearbyObjectAction.Init",
            "Action will wait %f, turn %fdeg, wait %f, turn %fdeg, wait %f",
            initialWait_s, RAD_TO_DEG(firstAngle_rads), afterFirstTurnWait_s,
            RAD_TO_DEG(secondAngle_rads), afterSecondTurnWait_s);

  AddToCompoundAction(new WaitAction(initialWait_s));

  DriveStraightAction* driveBackAction = nullptr;

  const float defaultBackupSpeed =
      Util::numeric_cast<f32>(Util::EnumToUnderlying(SFNOD::BackupSpeed_mms));
  if (Util::IsFltNear(defaultBackupSpeed, _backupSpeed_mms)) {
    // if using the default backup speed, don't specify it to the action (So the
    // motion profile can take over if it's set)
    ...
    driveBackAction =
        new DriveStraightAction(_backupDistance_mm, _backupSpeed_mms, false);
  }

  IActionRunner* driveAndLook = new CompoundActionParallel(
      {driveBackAction, new MoveHeadToAngleAction(_headAngle_rad)});
...
DriveStraightAction::DriveStraightAction(f32 dist_mm)
    : IAction("DriveStraight", RobotActionType::DRIVE_STRAIGHT,
              (u8)AnimTrackFlag::BODY_TRACK),
      _dist_mm(dist_mm),
      _timeout_s(IAction::GetTimeoutInSeconds()) {
...
DriveStraightAction::DriveStraightAction(f32 dist_mm, f32 speed_mmps,
  ...
  if (Util::IsFltLTZero(_speed_mmps)) {
    PRINT_NAMED_WARNING(
        "DriveStraightAction.Constructor.NegativeSpeed",
  ...
  if (Util::IsFltLTZero(dist_mm)) {
    ...
    // internally. Yes, we could have just double-negated if the caller passed
    // in a negative speed already, but this avoids confusion on caller's side
    // about which signs to use and the documentation says speed should always
    // be positive.
    DEV_ASSERT(_speed_mmps >= 0.f,
               "DriveStraightAction.Constructor.NegativeSpeed");
    _speed_mmps = -_speed_mmps;
  }
...
void DriveStraightAction::GetRequiredVisionModes(
    std::set<VisionModeRequest>& requests) const {
...
void DriveStraightAction::SetCanMoveOnCharger(bool canMove) {
  ANKI_VERIFY(!HasStarted(),
              "DriveStraightAction.SetCanMoveOnCharger.ActionAlreadyStarted",
              "[%d]", GetTag());
...
bool DriveStraightAction::SetMotionProfile(const PathMotionProfile& profile) {
  if (_motionProfileManuallySet) {
    ...
    _speed_mmps =
        (_dist_mm < 0.0f) ? -profile.reverseSpeed_mmps : profile.speed_mmps;
...
ActionResult DriveStraightAction::Init() {
  GetRobot().GetDrivingAnimationHandler().Init(GetTracksToLock(), GetTag(),
                                               IsSuppressingTrackLocking());
  ...
  if (!_canMoveOnCharger &&
      GetRobot().GetBatteryComponent().IsOnChargerPlatform()) {
    return ActionResult::SHOULDNT_DRIVE_ON_CHARGER;
  }

  const Radians heading =
      GetRobot().GetPose().GetRotation().GetAngleAroundZaxis();
  ...
  f32 maxSpeed = isCarryingObject ? MAX_SAFE_WHILE_CARRYING_WHEEL_SPEED_MMPS
                                  : MAX_SAFE_WHEEL_SPEED_MMPS;
  _speed_mmps = CLIP(_speed_mmps, -maxSpeed, maxSpeed);

  Planning::Path path;
  if (false == path.AppendLine(x_start, y_start, x_end, y_end, _speed_mmps,
                               _accel_mmps2, _decel_mmps2)) {
...
CalibrateMotorAction::CalibrateMotorAction(bool calibrateHead,
    ...
    : IAction("CalibrateMotor-" + std::string(calibrateHead ? "Head" : "") +
                  std::string(calibrateLift ? "Lift" : ""),
              RobotActionType::CALIBRATE_MOTORS,
              (calibrateHead ? (u8)AnimTrackFlag::HEAD_TRACK : 0) |
                  (calibrateLift ? (u8)AnimTrackFlag::LIFT_TRACK : 0)),
...
ActionResult CalibrateMotorAction::Init() {
  DASMSG(engine_calibrate_motor_action, "calibrate_motors",
         "Engine is sending a motor calibration request to robot "
         "(CalibrateMotorAction)");
  DASMSG_SET(s1, EnumToString(_calibReason),
  ...
  _headCalibStarted = false;
  _liftCalibStarted = false;
  if (RESULT_OK != GetRobot().GetMoveComponent().CalibrateMotors(
                       _calibHead, _calibLift, _calibReason)) {
    return ActionResult::SEND_MESSAGE_TO_ROBOT_FAILED;
  }

  if (GetRobot().HasExternalInterface()) {
    using namespace ExternalInterface;
    auto helper = MakeAnkiEventUtil(*(GetRobot().GetExternalInterface()), *this,
                                    _signalHandles);
...
MoveHeadToAngleAction::MoveHeadToAngleAction(const Radians& headAngle,
                                             const Radians& tolerance,
                                             const Radians& variability)
  ...
  if (_headAngle < MIN_HEAD_ANGLE) {
    PRINT_NAMED_WARNING("MoveHeadToAngleAction.Constructor.AngleTooLow",
                        "Requested head angle (%.1fdeg) less than min head "
                        "angle (%.1fdeg). Clipping.",
    ...
    PRINT_NAMED_WARNING("MoveHeadToAngleAction.Constructor.AngleTooHigh",
                        "Requested head angle (%.1fdeg) more than max head "
                        "angle (%.1fdeg). Clipping.",
...
MoveHeadToAngleAction::MoveHeadToAngleAction(const Preset preset,
                                             const Radians& tolerance,
                                             const Radians& variability)
    : MoveHeadToAngleAction(GetPresetHeadAngle(preset), tolerance,
                            variability) {
...
f32 MoveHeadToAngleAction::GetPresetHeadAngle(Preset preset) {
  switch (preset) {
    case Preset::GROUND_PLANE_VISIBLE: {
      return DEG_TO_RAD(-15.0f);
    }
    case Preset::IDEAL_BLOCK_VIEW: {
      return kIdealViewBlockHeadAngle;
    }
  }
...
MoveHeadToAngleAction::~MoveHeadToAngleAction() {
  if (HasRobot()) {
    // Make sure eye shift gets removed, by this action, or by the MoveComponent
    // if "hold" is enabled
    if (_holdEyes) {
      GetRobot().GetMoveComponent().RemoveEyeShiftWhenHeadMoves(
          _kEyeShiftLayerName, 3 * ANIM_TIME_STEP_MS);
    } else {
      GetRobot().GetAnimationComponent().RemoveEyeShift(_kEyeShiftLayerName);
    }
  }
...
bool MoveHeadToAngleAction::IsHeadInPosition() const {
  const bool inPosition =
      _headAngle.IsNear(GetRobot().GetComponent<FullRobotPose>().GetHeadAngle(),
                        _angleTolerance.ToFloat() +
                            Util::FLOATING_POINT_COMPARISON_TOLERANCE_FLT);
  return inPosition;
}
...
ActionResult MoveHeadToAngleAction::Init() {
  ...
  if (!_inPosition) {
    if (RESULT_OK != GetRobot().GetMoveComponent().MoveHeadToAngle(
                         _headAngle.ToFloat(), _maxSpeed_radPerSec,
                         _accel_radPerSec2, _duration_sec, &_actionID)) {
    ...
    if (_moveEyes) {
      ...
      // as the head cam
      Radians angleDiff =
          GetRobot().GetComponent<FullRobotPose>().GetHeadAngle() - _headAngle;
      const f32 y_mm = std::tan(angleDiff.ToFloat()) * HEAD_CAM_POSITION[0];
      const f32 yPixShift =
          y_mm * (static_cast<f32>(GetRobot().GetDisplayHeightInPixels() / 4) /
                  SCREEN_SIZE[1]);
      GetRobot().GetAnimationComponent().AddOrUpdateEyeShift(
          _kEyeShiftLayerName, 0, yPixShift, 4 * ANIM_TIME_STEP_MS);

      if (!_holdEyes) {
        // Store the half the angle differene so we know when to remove eye
        // shift
        _halfAngle =
            0.5f * (_headAngle -
                    GetRobot().GetComponent<FullRobotPose>().GetHeadAngle())
                       .getAbsoluteVal();
      }
    }
  }
...
ActionResult MoveHeadToAngleAction::CheckIfDone() {
  ...
  if (_motionCommanded && !_motionCommandAcked) {
    PRINT_PERIODIC_CH_DEBUG(10, "Actions",
                            "MoveHeadToAngleAction.CheckIfDone.WaitingForAck",
                            "[%d] ActionID: %d", GetTag(), _actionID);
    return result;
  }
  ...
  if (GetRobot().GetAnimationComponent().IsEyeShifting(_kEyeShiftLayerName) &&
    ...
    if (_inPosition ||
        _headAngle.IsNear(
            ...
            _halfAngle)) {
      LOG_DEBUG(
          "MoveHeadToAngleAction.CheckIfDone.RemovingEyeShift",
  ...
  if (_inPosition) {
    if (isHeadMoving) {
      LOG_INFO(
          ...
          "[%d] Head considered in position at %.1fdeg but still moving at "
          "%.1fdeg",
          GetTag(), _headAngle.getDegrees(),
          RAD_TO_DEG(GetRobot().GetComponent<FullRobotPose>().GetHeadAngle()));
    }
    ...
    PRINT_PERIODIC_CH_DEBUG(
        10, "Actions", "MoveHeadToAngleAction.CheckIfDone.NotInPosition",
        "[%d] Waiting for head to get in position: %.1fdeg vs. "
        "%.1fdeg(+/-%.1f) tol:%.1fdeg",
        GetTag(),
        RAD_TO_DEG(GetRobot().GetComponent<FullRobotPose>().GetHeadAngle()),
        _headAngle.getDegrees(), _variability.getDegrees(),
        _angleTolerance.getDegrees());

    if (_motionStarted && !isHeadMoving) {
      PRINT_NAMED_WARNING(
          "MoveHeadToAngleAction.CheckIfDone.StoppedMakingProgress",
          "[%d] giving up since we stopped moving", GetTag());
      result = ActionResult::MOTOR_STOPPED_MAKING_PROGRESS;
    }
  }
...
MoveLiftToAngleAction::MoveLiftToAngleAction(const f32 angle_rad,
                                             const f32 tolerance_rad,
                                             const f32 variability)
...
bool MoveLiftToAngleAction::IsLiftInPosition() const {
  const bool inPosition =
      (NEAR(_angleWithVariation,
            GetRobot().GetComponent<FullRobotPose>().GetLiftAngle(),
            _angleTolerance_rad) &&
...
ActionResult MoveLiftToAngleAction::Init() {
  ...
  if (_angle_rad < MIN_LIFT_ANGLE || _angle_rad > MAX_LIFT_ANGLE) {
    PRINT_NAMED_WARNING("MoveLiftToAngleAction.Init.InvalidAngle",
                        "%f deg. Clipping to be in range.",
                        RAD_TO_DEG(_angle_rad));
  ...
  _angleWithVariation =
      CLIP(_angleWithVariation, MIN_LIFT_ANGLE, MAX_LIFT_ANGLE);

  if (_angleTolerance_rad < LIFT_ANGLE_TOL) {
    PRINT_NAMED_WARNING("MoveLiftToAngleAction.Init.TolTooSmall",
                        "Angle tolerance (%f rad) must be >= LIFT_ANGLE_TOL. "
                        "Clipping tolerance",
  ...
  if (!_inPosition) {
    if (GetRobot().GetMoveComponent().MoveLiftToAngle(
            _angleWithVariation, _maxLiftSpeedRadPerSec, _liftAccelRacPerSec2,
            _duration, &_actionID) != RESULT_OK) {
      result = ActionResult::SEND_MESSAGE_TO_ROBOT_FAILED;
    } else {
      _motionCommanded = true;
    }
...
ActionResult MoveLiftToAngleAction::CheckIfDone() {
  ...
  if (_motionCommanded && !_motionCommandAcked) {
    PRINT_PERIODIC_CH_DEBUG(10, "Actions",
                            "MoveLiftToAngleAction.CheckIfDone.WaitingForAck",
                            "[%d] ActionID: %d", GetTag(), _actionID);
    return result;
  }
  ...
  if (_inPosition) {
    ...
    PRINT_PERIODIC_CH_DEBUG(
        10, "Actions", "MoveLiftToAngleAction.CheckIfDone.NotInPosition",
        "[%d] Waiting for lift to get in position: %.1fdeg vs. %.1fdeg (tol: "
        "%f)",
        GetTag(),
        RAD_TO_DEG(GetRobot().GetComponent<FullRobotPose>().GetLiftAngle()),
        RAD_TO_DEG(_angleWithVariation), RAD_TO_DEG(_angleTolerance_rad));

    if (_motionStarted && !isLiftMoving) {
      PRINT_NAMED_WARNING(
          "MoveLiftToAngleAction.CheckIfDone.StoppedMakingProgress",
          "[%d] giving up since we stopped moving", GetTag());
      result = ActionResult::MOTOR_STOPPED_MAKING_PROGRESS;
    }
  }
...
MoveLiftToHeightAction::MoveLiftToHeightAction(const f32 height_mm,
                                               const f32 tolerance_mm,
                                               const f32 variability)
...
MoveLiftToHeightAction::MoveLiftToHeightAction(const Preset preset,
                                               const f32 tolerance_mm)
...
bool MoveLiftToHeightAction::IsLiftInPosition() const {
  const bool inPosition = (NEAR(_heightWithVariation,
                                GetRobot().GetLiftHeight(), _heightTolerance) &&
                           !GetRobot().GetMoveComponent().IsLiftMoving());
...
ActionResult MoveLiftToHeightAction::Init() {
  ...
  if (_height_mm >= 0 &&
      (_height_mm < LIFT_HEIGHT_LOWDOCK || _height_mm > LIFT_HEIGHT_CARRY)) {
  ...
  if (_height_mm < 0.f) {
    ...
    if (_variability > 0.f) {
      _heightWithVariation +=
          GetRNG().RandDblInRange(-_variability, _variability);
    }
    _heightWithVariation =
        CLIP(_heightWithVariation, LIFT_HEIGHT_LOWDOCK, LIFT_HEIGHT_CARRY);
  }
  ...
  if (minAngleDiff < LIFT_ANGLE_TOL) {
    ...
    f32 desiredHeightLower =
        ConvertLiftAngleToLiftHeightMM(targetAngle - LIFT_ANGLE_TOL);
    f32 desiredHeightUpper =
        ConvertLiftAngleToLiftHeightMM(targetAngle + LIFT_ANGLE_TOL);
    f32 newHeightTolerance = std::max(_height_mm - desiredHeightLower,
                                      desiredHeightUpper - _height_mm);

    PRINT_NAMED_WARNING("MoveLiftToHeightAction.Init.TolTooSmall",
                        "HeightTol %f mm == AngleTol %f rad near height of %f "
                        "mm. Clipping tol to %f mm",
                        _heightTolerance, minAngleDiff, _heightWithVariation,
                        newHeightTolerance);
  ...
  if (!_inPosition) {
    if (GetRobot().GetMoveComponent().MoveLiftToHeight(
            _heightWithVariation, _maxLiftSpeedRadPerSec, _liftAccelRacPerSec2,
            _duration, &_actionID) != RESULT_OK) {
      result = ActionResult::SEND_MESSAGE_TO_ROBOT_FAILED;
    } else {
      _motionCommanded = true;
...
ActionResult MoveLiftToHeightAction::CheckIfDone() {
  ...
  if (_motionCommanded && !_motionCommandAcked) {
    PRINT_PERIODIC_CH_DEBUG(10, "Actions",
                            "MoveLiftToHeightAction.CheckIfDone.WaitingForAck",
                            "[%d] ActionID: %d", GetTag(), _actionID);
    return result;
  }
  ...
  if (_inPosition) {
    ...
    PRINT_PERIODIC_CH_DEBUG(
        10, "Actions", "MoveLiftToHeightAction.CheckIfDone.NotInPosition",
        "[%d] Waiting for lift to get in position: %.1fmm vs. %.1fmm (tol: %f)",
        GetTag(), GetRobot().GetLiftHeight(), _heightWithVariation,
        _heightTolerance);

    if (_motionStarted && !isLiftMoving) {
      PRINT_NAMED_WARNING(
          "MoveLiftToHeightAction.CheckIfDone.StoppedMakingProgress",
          "[%d] giving up since we stopped moving", GetTag());
      result = ActionResult::MOTOR_STOPPED_MAKING_PROGRESS;
    }
  }
...
PanAndTiltAction::PanAndTiltAction(Radians bodyPan, Radians headTilt,
    ...
    : IAction("PanAndTilt", RobotActionType::PAN_AND_TILT,
...
PanAndTiltAction::~PanAndTiltAction() { _compoundAction.PrepForCompletion(); }
...
void PanAndTiltAction::SetMaxPanSpeed(f32 maxSpeed_radPerSec) {
  ...
  } else if (std::fabsf(maxSpeed_radPerSec) >
             MAX_BODY_ROTATION_SPEED_RAD_PER_SEC) {
    PRINT_NAMED_WARNING(
        "PanAndTiltAction.SetMaxSpeed.PanSpeedExceedsLimit",
        "Speed of %f deg/s exceeds limit of %f deg/s. Clamping.",
        RAD_TO_DEG(maxSpeed_radPerSec), MAX_BODY_ROTATION_SPEED_DEG_PER_SEC);
...
void PanAndTiltAction::SetTiltTolerance(const Radians& angleTol_rad) {
  ...
  if (_tiltAngleTol.ToFloat() < HEAD_ANGLE_TOL) {
    PRINT_NAMED_WARNING("PanAndTiltAction.SetTiltTolerance.InvalidTolerance",
                        "Tried to set tolerance of %fdeg, min is %f",
                        _tiltAngleTol.getDegrees(), RAD_TO_DEG(HEAD_ANGLE_TOL));
...
ActionResult PanAndTiltAction::Init() {
  ...
  TurnInPlaceAction* action =
      new TurnInPlaceAction(_bodyPanAngle.ToFloat(), _isPanAbsolute);
  ...
  ICompoundAction::ShouldIgnoreFailureFcn ignoreFailure =
      [](ActionResult result, const IActionRunner* action) {
        // ignore failures if they failed because we are on the charger. In that
        // case, the head should still move
        const bool failureOK =
            (result == ActionResult::SHOULDNT_DRIVE_ON_CHARGER);
        return failureOK;
      };
  _compoundAction.AddAction(action, ignoreFailure);

  const Radians newHeadAngle =
      _isTiltAbsolute
          ? _headTiltAngle
          : GetRobot().GetComponent<FullRobotPose>().GetHeadAngle() +
                _headTiltAngle;
  MoveHeadToAngleAction* headAction =
      new MoveHeadToAngleAction(newHeadAngle, _tiltAngleTol);
  ...
  // Prevent the compound action from locking tracks (the PanAndTiltAction
  // handles it itself)
...
TurnTowardsObjectAction::TurnTowardsObjectAction(ObjectID objectID,
    ...
    : TurnTowardsObjectAction(objectID, Vision::Marker::ANY_CODE, maxTurnAngle,
                              visuallyVerifyWhenDone, headTrackWhenDone) {}
...
void TurnTowardsObjectAction::GetRequiredVisionModes(
    std::set<VisionModeRequest>& requests) const {
  requests.insert({VisionMode::Markers, EVisionUpdateFrequency::Low});
}
...
void TurnTowardsObjectAction::UseCustomObject(ObservableObject* objectPtr) {
  if (_objectID.IsSet()) {
    PRINT_NAMED_WARNING(
        "TurnTowardsObjectAction.UseCustomObject.CustomObjectOverwriteId",
        "object id was already set to %d, but now setting it to use a custom "
        "object ptr",
        _objectID.GetValue());
    _objectID.UnSet();
  }

  // Note: when using a custom object, caller must guarantee that the object
  // will persist past the lifetime of the action
  _objectPtr = objectPtr;

  SetName("TurnTowardsCustomObject" +
          std::to_string(_objectPtr->GetID().GetValue()));
...
ActionResult TurnTowardsObjectAction::Init() {
  ...
  if (usingCustomObject) {
    ...
    if (!GetRobot().IsPoseInWorldOrigin(_objectPtr->GetPose())) {
      PRINT_NAMED_WARNING(
          "TurnTowardsObjectAction.Init.CustomObjectNotInRobotFrame",
          "Custom %s object %d in origin:%s vs. robot in origin:%s",
          EnumToString(_objectPtr->GetType()), _objectPtr->GetID().GetValue(),
    ...
    if (_visuallyVerifyWhenDone) {
      PRINT_NAMED_WARNING(
          "TurnTowardsObjectAction.Init.CannotVisuallyVerifyCustomObject",
  ...
  if (_whichCode == Vision::Marker::ANY_CODE) {
    // if ANY_CODE is specified, find the closest marker face to the robot and
    // use that pose. We don't want to consider the "top" or "bottom" faces
    // (based on current rotation)

    // Solution: project all points into 2D and pick the closest. The top and
    // bottom faces will never be closer than the closest side face (unless we
    // are inside the cube)

    const Result poseResult = _objectPtr->GetClosestMarkerPose(
        GetRobot().GetPose(), true, objectPoseWrtRobot);

    if (RESULT_OK != poseResult) {
      // This should not occur because we checked above that the object is in
      // the same coordinate frame as the robot
      PRINT_NAMED_ERROR(
          "TurnTowardsObjectAction.Init.NoValidPose",
          "Could not get a valid closest marker pose of %sobject %d",
          usingCustomObject ? "custom " : "", _objectPtr->GetID().GetValue());
      return ActionResult::BAD_MARKER;
    }
  } else {
    // Use the closest marker with the specified code:
    std::vector<Vision::KnownMarker*> const& markers =
        _objectPtr->GetMarkersWithCode(_whichCode);
    ...
    for (auto marker : markers) {
      if (false == marker->GetPose().GetWithRespectTo(GetRobot().GetPose(),
                                                      markerPoseWrtRobot)) {
        PRINT_NAMED_ERROR(
            "TurnTowardsObjectAction.Init.MarkerOriginProblem",
            "Could not get pose of marker with code %d of %sobject %d "
            "w.r.t. robot pose.",
            _whichCode, usingCustomObject ? "custom " : "",
...
ActionResult TurnTowardsObjectAction::CheckIfDone() {
  ...
  if (!_facePoseCompoundActionDone) {
    ...
    if (compoundResult != ActionResult::SUCCESS) {
      ...
      if (_doRefinedTurn) {
        // If we need to refine the turn just reset this action, set appropriate
        // variables
        Reset(false);
  ...
  if (_headTrackWhenDone) {
    if (!_objectID.IsSet()) {
      PRINT_NAMED_WARNING(
          "TurnTowardsObjectAction.CustomObject.TrackingNotsupported",
          "No valid object id (you probably specified a custom action), so "
          "can't track");
...
void TurnTowardsObjectAction::GetCompletionUnion(
    ActionCompletedUnion& completionUnion) const {
...
TurnTowardsPoseAction::TurnTowardsPoseAction(const Pose3d& pose,
                                             Radians maxTurnAngle)
...
Radians TurnTowardsPoseAction::GetAbsoluteHeadAngleToLookAtPose(
    const Point3f& translationWrtRobot) {
  const f32 heightDiff = translationWrtRobot.z() - NECK_JOINT_POSITION[2];
  const f32 distanceXY =
      Point2f(translationWrtRobot).Length() - NECK_JOINT_POSITION[0];

  // Adding bias to account for the fact that the camera tends to look lower
  // than desired on account of it being lower wrt neck joint. Ramp bias down to
  // 0 for distanceXY values from 150mm to 300mm.
  const f32 kFullBiasDist_mm = 150;
  const f32 kNoBiasDist_mm = 300;
  const f32 biasScaleFactorDist =
      CLIP((kNoBiasDist_mm - distanceXY) / (kNoBiasDist_mm - kFullBiasDist_mm),
           0, 1);

  // Adding bias to account for the fact that we don't look high enough when
  // turning towards objects off the ground Apply full bias for object 10mm
  // above neck joint and 0 for objects below neck joint
  const f32 kFullBiasHeight_mm = 10;
  const f32 kNoBiasHeight_mm = 0;
  const f32 biasScaleFactorHeight = CLIP(
      (kNoBiasHeight_mm - heightDiff) / (kNoBiasHeight_mm - kFullBiasHeight_mm),
      0, 1);
...
Radians TurnTowardsPoseAction::GetRelativeBodyAngleToLookAtPose(
    const Point3f& translationWrtRobot) {
  return std::atan2(translationWrtRobot.y(), translationWrtRobot.x());
}
...
ActionResult TurnTowardsPoseAction::Init() {
  ...
  } else if (false == _poseWrtRobot.GetWithRespectTo(GetRobot().GetPose(),
                                                     _poseWrtRobot)) {
    // TODO: It's possible this is just "normal" when dealing with
    // delocalization, so possibly downgradable to Info later
    PRINT_NAMED_WARNING(
        "TurnTowardsPoseAction.Init.PoseOriginFailure",
  ...
  if (_maxTurnAngle > 0) {
    ...
    const Radians turnAngle =
        GetRelativeBodyAngleToLookAtPose(_poseWrtRobot.GetTranslation());
    ...
    if (turnAngle.getAbsoluteVal() <= _maxTurnAngle) {
      ...
      LOG_INFO(
          "TurnTowardsPoseAction.Init.RequiredTurnTooLarge",
  ...
  Result result =
      GetRobot().ComputeHeadAngleToSeePose(_poseWrtRobot, headAngle, kYTolFrac);
  if (RESULT_OK != result) {
    ...
    headAngle =
        GetAbsoluteHeadAngleToLookAtPose(_poseWrtRobot.GetTranslation());
  }
...
TurnTowardsImagePointAction::TurnTowardsImagePointAction(
    const Point2f& imgPoint, const RobotTimeStamp_t t)
...
TurnTowardsImagePointAction::TurnTowardsImagePointAction(
    const Vision::SalientPoint& salientPoint)
    : TurnTowardsImagePointAction(
          Point2f(salientPoint.x_img, salientPoint.y_img),
...
ActionResult TurnTowardsImagePointAction::Init() {
  ...
  Result result = GetRobot().ComputeTurnTowardsImagePointAngles(
      _imgPoint, _timestamp, panAngle, tiltAngle, _isPointNormalized);
  if (RESULT_OK != result) {
    PRINT_NAMED_WARNING(
        "TurnTowardsImagePointAction.Init."
        "ComputeTurnTowardsImagePointAnglesFailed",
        "(%f,%f) at t=%u", _imgPoint.x(), _imgPoint.y(),
...
TurnTowardsFaceAction::TurnTowardsFaceAction(const SmartFaceID& faceID,
                                             Radians maxTurnAngle, bool sayName)
    : TurnTowardsPoseAction(maxTurnAngle), _faceID(faceID), _sayName(sayName) {
...
TurnTowardsFaceAction::TurnTowardsFaceAction(
    const SmartFaceID& faceID, Radians maxTurnAngle,
...
TurnTowardsFaceAction::~TurnTowardsFaceAction() {
  ...
  if (HasRobot() && _tracksLocked) {
    GetRobot().GetMoveComponent().UnlockTracks(
...
void TurnTowardsFaceAction::SetSayNameAnimationTrigger(
    AnimationTrigger trigger) {
  if (!MightSayName()) {
    LOG_DEBUG("TurnTowardsFaceAction.SetSayNameTriggerWithoutSayingName",
              "setting say name trigger, but we aren't going to say the name. "
              "This is useless");
  }
...
void TurnTowardsFaceAction::SetNoNameAnimationTrigger(
    AnimationTrigger trigger) {
  if (!MightSayName()) {
    LOG_DEBUG("TurnTowardsFaceAction.SetNoNameTriggerWithoutSayingName",
              "setting anim trigger for unnamed faces, but we aren't going to "
              "say the name.");
  }
  auto callback = [trigger](const Robot& robot, const SmartFaceID& faceID) {
    return trigger;
...
void TurnTowardsFaceAction::SetAnyFaceAnimationTrigger(
    AnimationTrigger trigger) {
  auto callback = [trigger](const Robot& robot, const SmartFaceID& faceID) {
    return trigger;
  };
...
void TurnTowardsFaceAction::SetSayNameTriggerCallback(
    AnimTriggerForFaceCallback&& callback) {
  DEV_ASSERT(_anyFaceTriggerCallback == nullptr,
             "SetNoNameTriggerCallback is mutually exclusive with "
             "SetAnyFaceTriggerCallback");
  if (!MightSayName()) {
    LOG_DEBUG(
        "TurnTowardsFaceAction.SetSayNameTriggerCallbackWithoutSayingName",
        "setting say name trigger callback, but we aren't going to say the "
        "name. This is useless");
  }
  _sayNameTriggerCallback = std::move(callback);
}

void TurnTowardsFaceAction::SetNoNameTriggerCallback(
    AnimTriggerForFaceCallback&& callback) {
  DEV_ASSERT(_anyFaceTriggerCallback == nullptr,
             "SetNoNameTriggerCallback is mutually exclusive with "
             "SetAnyFaceTriggerCallback");
  if (!MightSayName()) {
    LOG_DEBUG("TurnTowardsFaceAction.SetNoNameTriggerCallbackWithoutSayingName",
              "setting say name trigger callback, but we aren't going to say "
              "the name. This is useless");
  }
  _noNameTriggerCallback = std::move(callback);
}

void TurnTowardsFaceAction::SetAnyFaceTriggerCallback(
    AnimTriggerForFaceCallback&& callback) {
  DEV_ASSERT((_noNameTriggerCallback == nullptr) &&
                 (_sayNameTriggerCallback == nullptr),
             "SetAnyFaceTriggerCallback is mutually exclusive with other anim "
             "trigger callbacks");
  DEV_ASSERT(
      !MightSayName(),
      "SetAnyFaceTriggerCallback is mutually exclusive sayname animations");
  _anyFaceTriggerCallback = std::move(callback);
}

void TurnTowardsFaceAction::GetRequiredVisionModes(
    std::set<VisionModeRequest>& requests) const {
  requests.insert({VisionMode::Faces, EVisionUpdateFrequency::High});
}

ActionResult TurnTowardsFaceAction::Init() {
  // If we have a last observed face set, use its pose. Otherwise pose will not
  // be set so fail if we require a face, skip ahead if we don't
  Pose3d pose;
  bool gotPose = false;
  ...
  if (_faceID.IsValid()) {
    ...
    if (nullptr != face &&
        face->GetHeadPose().GetWithRespectTo(GetRobot().GetPose(), pose)) {
      gotPose = true;
    }
  } else if (GetRobot().GetFaceWorld().GetLastObservedFace(
                 pose, kLastObservedFaceMustBeInRobotOrigin) != 0) {
    ...
    if (success) {
      ...
      PRINT_NAMED_WARNING(
          "TurnTowardsFaceAction.Init.BadLastObservedFacePose",
  ...
  if (gotPose) {
    ...
    if (GetRobot().HasExternalInterface()) {
      using namespace ExternalInterface;
      auto helper = MakeAnkiEventUtil(*(GetRobot().GetExternalInterface()),
                                      *this, _signalHandles);
      helper.SubscribeEngineToGame<MessageEngineToGameTag::RobotObservedFace>();
    }

    _state = State::Turning;
    GetRobot().GetMoveComponent().LockTracks(
        (u8)AnimTrackFlag::HEAD_TRACK | (u8)AnimTrackFlag::BODY_TRACK, GetTag(),
        GetName());
    _tracksLocked = true;
    ...
    if (_requireFaceConfirmation) {
      ...
      _state = State::PlayingAnimation;  // jump to end and play animation (if
                                         // present)
      return ActionResult::SUCCESS;
    }
  }
...
template <>
void TurnTowardsFaceAction::HandleMessage(
  ...
  if (_state == State::Turning || _state == State::WaitingForFace) {
    ...
    const bool allowFaceSwitch = _lockOnClosestFace &&
    ...
    if (!_faceID.IsValid() || allowFaceSwitch) {
      ...
      const Vision::TrackedFace* face =
          GetRobot().GetFaceWorld().GetFace(faceID);
      if (nullptr != face) {
        Pose3d faceWrtRobot;
        if (true == face->GetHeadPose().GetWithRespectTo(GetRobot().GetPose(),
                                                         faceWrtRobot)) {
      ...
      // We know what face we're looking for. If this is it, set the observed
      // face ID to it.
...
void TurnTowardsFaceAction::CreateFineTuneAction() {
  LOG_DEBUG("TurnTowardsFaceAction.CreateFinalAction.SawFace",
            "Observed ID=%s. Will fine tune.",
            _obsFaceID.GetDebugStr().c_str());

  if (_obsFaceID.IsValid()) {
    const Vision::TrackedFace* face =
        GetRobot().GetFaceWorld().GetFace(_obsFaceID);
    if (ANKI_VERIFY(nullptr != face, "TurnTowardsFaceAction.FindTune.NullFace",
                    "id %s returned null", _obsFaceID.GetDebugStr().c_str())) {
      // Valid face...
      Pose3d pose;
      if (true ==
          face->GetHeadPose().GetWithRespectTo(GetRobot().GetPose(), pose)) {
        GetRobot().GetMoodManager().TriggerEmotionEvent(
            "LookAtFaceVerified", MoodManager::GetCurrentTimeInSeconds());

        // ... with valid pose w.r.t. robot. Turn towards that face -- iff it
        // doesn't require too large of an adjustment.
        const Radians maxFineTuneAngle(
            std::min(GetMaxTurnAngle().ToFloat(), DEG_TO_RAD(45.f)));
        TurnTowardsPoseAction* ptr =
            new TurnTowardsPoseAction(pose, maxFineTuneAngle);
...
bool TurnTowardsFaceAction::CreateNameAnimationAction(
    const Vision::TrackedFace* face) {
  // return value
  ...
  if (haveName) {
    if (ShouldSayName(face->GetName())) {
      ...
      if (_sayNameTriggerCallback) {
        AnimationTrigger sayNameAnim =
            _sayNameTriggerCallback(GetRobot(), _obsFaceID);
    ...
    // TODO: If should not say name, provide anim trigger/callback for "name
    // known but not saying"
  } else if (_noNameTriggerCallback) {
    AnimationTrigger noNameAnim =
        _noNameTriggerCallback(GetRobot(), _obsFaceID);
    if (noNameAnim != AnimationTrigger::Count) {
      SetAction(new TriggerLiftSafeAnimationAction(noNameAnim, 1, true,
                                                   _animTracksToLock));
      createdActions = true;
    }
  }
  ...
  switch (_state) {
      ...
      if (ActionResult::SUCCESS == result) {
        ...
        if (!_obsFaceID.IsValid()) {
          ...
          LOG_DEBUG("TurnTowardsFaceAction.CheckIfDone.NoFaceObservedYet",
                    "Will wait no more than %d frames", _maxFramesToWait);
          DEV_ASSERT(nullptr == _action,
                     "TurnTowardsFaceAction.CheckIfDone."
                     "ActionPointerShouldStillBeNull");
          SetAction(
              new WaitForImagesAction(_maxFramesToWait, VisionMode::Faces));
          // TODO:(bn) parallel action with an animation here? This will let us
          // span the gap a bit better and buy us more time. Skipping for now
          _state = State::WaitingForFace;
        } else {
    ...
    case State::WaitingForFace: {
      ...
      if (_obsFaceID.IsValid()) {
        ...
        // the wait action isn't running anymore, we didn't get a face, and we
        // require a face. This is a failure
    ...
    case State::FineTuning: {
      if (nullptr == _action) {
        ...
        result = _action->Update();
        // play an animation, possibly a TTS animation, based on what callbacks
        // have been provided
    ...
    case State::WaitingForRecognition: {
      const Vision::TrackedFace* face =
          GetRobot().GetFaceWorld().GetFace(_obsFaceID);
      const f32 currentTime_sec =
          BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
      const bool timedOut = (currentTime_sec - _startedWaitingForRecognition) >
      ...
      if (face->GetID() > 0 || timedOut) {
        ...
        if (timedOut) {
          DASMSG(turn_towards_face_recognition_timeout,
                 "turn_towards_face.recognition_timeout",
                 "TurnTowardsFaceAction timed out waiting for recognition to "
                 "complete");
          DASMSG_SEND();
        }
      }
...
TurnTowardsFaceWrapperAction::TurnTowardsFaceWrapperAction(
    IActionRunner* action, bool turnBeforeAction, bool turnAfterAction,
    Radians maxTurnAngle, bool sayName)
...
WaitAction::WaitAction(f32 waitTimeInSeconds)
    : IAction("WaitSeconds", RobotActionType::WAIT,
              (u8)AnimTrackFlag::NO_TRACKS),
...
ActionResult WaitAction::CheckIfDone() {
  ...
  if (BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() >
      _doneTimeInSeconds) {
    return ActionResult::SUCCESS;
  } else {
    return ActionResult::RUNNING;
...
WaitForImagesAction::WaitForImagesAction(u32 numFrames, VisionMode visionMode,
                                         RobotTimeStamp_t afterTimeStamp)
    : IAction("WaitFor" + std::to_string(numFrames) + "Images",
              RobotActionType::WAIT_FOR_IMAGES, (u8)AnimTrackFlag::NO_TRACKS),
      _numFramesToWaitFor(numFrames),
      _afterTimeStamp(afterTimeStamp),
      _visionMode(visionMode) {
  // If the caller requested to wait one frame and the specified VisionMode also
  // completes
  //  in a single frame, then we can use the special SingleShot update
  //  frequency. This forcibly disables the mode after a single camera frame.
  // If the VisionMode needs multiple frames to complete a "cycle" (as is the
  // case for
  //  Markers_Composite or AutoExp_Cycling), or multiple frames are requested,
  //  then we simply use High frequency. In this case, there may be one extra
  //  frame actually processed with the specified mode, because the VisionSystem
  //  runs asynchronously and may have already started on the next frame before
  //  this action unsubscribes from the mode.
...
WaitForImagesAction::~WaitForImagesAction() {
  ...
  if (ANKI_DEV_CHEATS && nullptr != _saveParams) {
    LOG_INFO("WaitForImagesAction.Destructor.DisablingSave",
             "Saved %d images to %s", _numFramesToWaitFor,
...
void WaitForImagesAction::GetRequiredVisionModes(
    std::set<VisionModeRequest>& requests) const {
  // If the user has subscribed to VisionMode::Count, they are asking to be
  // notified after N vision processing frames, regardless of mode. This does
  // not require any subscription to be made to the VSM since the
  // RobotProcessImage message will be sent even if no modes are currently
  // enabled.
...
ActionResult WaitForImagesAction::Init() {
  ...
  auto imageProcLambda =
      [this](const AnkiEvent<ExternalInterface::MessageEngineToGame>& msg) {
        DEV_ASSERT(
            ExternalInterface::MessageEngineToGameTag::RobotProcessedImage ==
                msg.GetData().GetTag(),
            "WaitForImagesAction.MessageTypeNotHandled");
        const ExternalInterface::RobotProcessedImage& imageMsg =
            msg.GetData().Get_RobotProcessedImage();
        if (imageMsg.timestamp > _afterTimeStamp) {
          if (VisionMode::Count == _visionMode) {
            ++_numModeFramesSeen;
            LOG_DEBUG("WaitForImagesAction.Callback",
                      "Frame %d of %d for any mode", _numModeFramesSeen,
                      _numFramesToWaitFor);
          } else {
            for (const auto& mode : imageMsg.visionModes) {
              if (mode == _visionMode) {
                ++_numModeFramesSeen;
                LOG_DEBUG("WaitForImagesAction.Callback",
                          "Frame %d of %d for mode %s", _numModeFramesSeen,
                          _numFramesToWaitFor, EnumToString(mode));
                break;
              }
            }
          }
          ...
          if (_numModeFramesSeen >= _numFramesToWaitFor) {
            // Release subscriptions immediately in the callback to avoid
            // possibly waiting an extra tick to call to CheckIfDone() and
            // having the requested VisionMode(s) run any more than absolutely
            // necessary.
  ...
  _imageProcSignalHandle = GetRobot().GetExternalInterface()->Subscribe(
      ExternalInterface::MessageEngineToGameTag::RobotProcessedImage,
      imageProcLambda);

  if (ANKI_DEV_CHEATS && nullptr != _saveParams) {
    LOG_DEBUG("WaitForImagesAction.Init.SetSaveParams",
              "Mode:%s Path:%s Quality:%d", EnumToString(_saveParams->mode),
              _saveParams->path.c_str(), _saveParams->quality);
...
ActionResult WaitForImagesAction::CheckIfDone() {
  ...
  // Reset the signalHandler to unsubscribe from the ProcessedImage message in
  // case this action is not immediatly destroyed after completion
...
CliffAlignToWhiteAction::CliffAlignToWhiteAction()
    : IAction("CliffAlignToWhite", RobotActionType::CLIFF_ALIGN_TO_WHITE,
              (u8)AnimTrackFlag::BODY_TRACK) {}

ActionResult CliffAlignToWhiteAction::Init() {
  // Store stop-on-white state and disable it if it's currently enabled
  _resumeStopOnWhite =
      GetRobot().GetCliffSensorComponent().IsStopOnWhiteEnabled();
  ...
  auto actionStartedLambda =
      [this](const AnkiEvent<RobotInterface::RobotToEngine>& event) {
        ...
        LOG_INFO("CliffAlignToWhiteAction.Init.CliffAlignComplete",
                 "[%d] Success: %s", GetTag(), EnumToString(payload.result));
        switch (payload.result) {
          case CliffAlignResult::CLIFF_ALIGN_SUCCESS: {
            _state = State::Success;
            break;
          }
          case CliffAlignResult::CLIFF_ALIGN_FAILURE_TIMEOUT: {
            _state = State::FailedTimeout;
            break;
          }
          case CliffAlignResult::CLIFF_ALIGN_FAILURE_NO_TURNING: {
            _state = State::FailedNoTurning;
            break;
          }
          case CliffAlignResult::CLIFF_ALIGN_FAILURE_OVER_TURNING: {
            _state = State::FailedOverturning;
            break;
          }
          case CliffAlignResult::CLIFF_ALIGN_FAILURE_NO_WHITE: {
            _state = State::FailedNoWhite;
            break;
          }
          case CliffAlignResult::CLIFF_ALIGN_FAILURE_STOPPED: {
            _state = State::FailedStopped;
            break;
          }
        }
      };
  ...
  _signalHandle = GetRobot().GetRobotMessageHandler()->Subscribe(
      RobotInterface::RobotToEngineTag::cliffAlignComplete,
      actionStartedLambda);
