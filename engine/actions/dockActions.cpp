 ...
 * Date:   8/29/2014
 *
 * Description: Implements cozmo-specific actions, derived from the IAction
 *interface.
 *
 *
...
// AlignWithObject which defines the closest that a block can be approached
// (with lift raised). Doesn't make much sense for PlaceRelObject but it doesn't
// really hurt except that the robot would bump into the block it was docking
// to.
static const float kMaxNegativeXPlacementOffset = 16.f;

// use a fairly large distance offset and tighter angle to try to rule out
// current pose
...
CONSOLE_VAR(u32, kDefaultDockingMethod, "DockingMethod(B:0 T:1 H:2)",
            (u8)DockingMethod::BLIND_DOCKING);
CONSOLE_VAR(u32, kPickupDockingMethod, "DockingMethod(B:0 T:1 H:2)",
            (u8)DockingMethod::HYBRID_DOCKING_BEELINE);
CONSOLE_VAR(u32, kRollDockingMethod, "DockingMethod(B:0 T:1 H:2)",
            (u8)DockingMethod::BLIND_DOCKING);
CONSOLE_VAR(u32, kStackDockingMethod, "DockingMethod(B:0 T:1 H:2)",
            (u8)DockingMethod::BLIND_DOCKING);

// Whether or not to calculate the max preDock pose offset for
...
Point2f ComputePreActionPoseDistThreshold(
    const Pose3d& preActionPose, const Pose3d& actionObjectPose,
  ...
  if (preActionPoseAngleTolerance > 0.f) {
    ...
    if (false == actionObjectPose.GetWithRespectTo(preActionPose,
                                                   objectWrtPreActionPose)) {
      PRINT_NAMED_WARNING(
          "ComputePreActionPoseDistThreshold.ObjectPoseOriginProblem",
    ...
    const f32 thresh =
        objectDistance * std::sin(preActionPoseAngleTolerance.ToFloat());

    // We don't care so much about the distance to the object (x threshold) so
    // scale it
    const Point2f preActionPoseDistThresh(
        thresh * PREACTION_POSE_X_THRESHOLD_SCALAR, thresh);

    PRINT_CH_INFO("Actions", "ComputePreActionPoseDistThreshold.DistThresh",
                  "At a distance of %.1fmm, will use pre-dock pose distance "
                  "threshold of (%.1fmm, %.1fmm)",
                  objectDistance, preActionPoseDistThresh.x(),
...
IDockAction::IDockAction(ObjectID objectID, const std::string name,
...
IDockAction::~IDockAction() {
  ...
  if (_lightsSet) {
    LOG_INFO("IDockAction.UnsetInteracting",
             "%s[%d] Unsetting interacting object to %d", GetName().c_str(),
             GetTag(), _dockObjectID.GetValue());
    GetRobot().GetCubeLightComponent().StopLightAnimAndResumePrevious(
        CubeAnimationTrigger::Interacting, _dockObjectID);
  }
...
void IDockAction::SetPlacementOffset(f32 offsetX_mm, f32 offsetY_mm,
                                     f32 offsetAngle_rad) {
  if (FLT_LT(offsetX_mm, -kMaxNegativeXPlacementOffset)) {
    DEV_ASSERT_MSG(false, "IDockAction.SetPlacementOffset.InvalidOffset",
                   "x offset %f cannot be negative (through block)",
                   offsetX_mm);
    // for release set offset to 0 so that Cozmo doesn't look stupid plowing
    // through a block
    offsetX_mm = 0;
  }
...
void IDockAction::SetPostDockLiftMovingAudioEvent(
    AudioMetaData::GameEvent::GenericEvent event) {
  _liftMovingAudioEvent = event;
}

ActionResult IDockAction::ComputePlacementApproachAngle(
    const Robot& robot, const Pose3d& placementPose, f32& approachAngle_rad) {
  ...
  // Get carried object
  const ObservableObject* object = robot.GetBlockWorld().GetLocatedObjectByID(
      carryingComponentRef.GetCarryingObjectID());
  if (nullptr == object) {
    PRINT_NAMED_WARNING(
        "DriveToActions.ComputePlacementApproachAngle.NullObject",
        "ObjectID=%d", carryingComponentRef.GetCarryingObjectID().GetValue());
    return ActionResult::BAD_OBJECT;
  }

  // Check that up axis of carried object and the desired placementPose are the
  // same. Otherwise, it's impossible for the robot to place it there!
  const AxisName targetUpAxis =
      placementPose.GetRotationMatrix().GetRotatedParentAxis<'Z'>();
  ...
  if (!object->GetPose().GetWithRespectTo(robot.GetPose(),
                                          poseObjectWrtRobot)) {
    PRINT_NAMED_WARNING(
        "ComputePlacementApproachAngle.FailedToComputeObjectWrtRobotPose", "");
    return ActionResult::BAD_POSE;
  }

  // Get pose of robot if the carried object were aligned with the placementPose
  // and the robot was still carrying it
  Pose3d poseRobotIfPlacingObject = poseObjectWrtRobot.GetInverse();
  poseRobotIfPlacingObject.PreComposeWith(placementPose);

  approachAngle_rad = poseRobotIfPlacingObject.GetRotationMatrix()
                          .GetAngleAroundParentAxis<'Z'>()
                          .ToFloat();
...
void IDockAction::GetPreActionPoses(const Pose3d& robotPose,
  ...
  LOG_DEBUG("IsCloseEnoughToPreActionPose.GetCurrentPreActionPoses",
            "Using preDockPoseOffset_mm %f and %s", preDockPoseDistOffsetX_mm,
            (doNearPredockPoseCheck ? "checking if near pose"
                                    : "NOT checking if near pose"));
  dockObject->GetCurrentPreActionPoses(
      preActionPoses, robotPose, {preActionPoseType},
      std::set<Vision::Marker::Code>(), obstacles, preDockPoseDistOffsetX_mm);

  const Pose3d& robotPoseParent = robotPose.GetParent();

  // If using approach angle remove any preAction poses that aren't close to the
  ...
  if (input.useApproachAngle) {
    for (auto iter = preActionPoses.begin(); iter != preActionPoses.end();) {
      ...
      if (iter->GetPose().GetWithRespectTo(robotPoseParent, preActionPose) ==
          false) {
        PRINT_NAMED_WARNING(
            "IsCloseEnoughToPreActionPose.PreActionPoseOriginProblem",
      ...
      Radians headingDiff =
          preActionPose.GetRotationAngle<'Z'>() - input.approachAngle_rad;
      // If the heading difference between our desired approach angle and the
      // preAction pose's heading is greater than 45 degrees this preAction pose
      // will not be the one of the poses closest to approach angle
  ...
  if (preActionPoses.empty()) {
    PRINT_NAMED_WARNING("IsCloseEnoughToPreActionPose.NoPreActionPoses",
                        "Action object with ID=%d returned no pre-action poses "
                        "of the given type.",
                        dockObject->GetID().GetValue());
  ...
  for (size_t index = 0; index < preActionPoses.size(); ++index) {
    Pose3d preActionPose;
    if (preActionPoses[index].GetPose().GetWithRespectTo(
            robotPoseParent, preActionPose) == false) {
      PRINT_NAMED_WARNING(
          "IsCloseEnoughToPreActionPose.PreActionPoseOriginProblem",
          "Could not get pre-action pose w.r.t. world origin.");
      continue;
    ...
    LOG_DEBUG("IsCloseEnoughToPreActionPose.CheckPoint",
              "considering point (%f, %f) dist = %f", dist.x(), dist.y(),
              dist.Length());
  ...
  LOG_INFO("IsCloseEnoughToPreActionPose.ClosestPoint",
           ...
           currentXY.x(), currentXY.y(), closestPoint.Length());

  output.distThresholdUsed = ComputePreActionPoseDistThreshold(
      preActionPoses[closestIndex].GetPose(), dockObject->GetPose(),
  ...
  if (output.distThresholdUsed > 0) {
    if (closestPoint.AnyGT(output.distThresholdUsed)) {
      // If we are checking that we are close enough to the preDock pose and our
      // closestPoint is outside the distThreshold then fail saying we are too
      // far away Otherwise we will succeed but robotAtClosestPreActionPose will
      // stay false
    ...
    // Else closestPoint is within the distThreshold and if the angle of the
    // closest preAction pose is within preActionPoseAngleTolerance to the
    // current angle of the robot then set robotAtClosestPreActionPose to true
    else {
      Pose3d p;
      preActionPoses[closestIndex].GetPose().GetWithRespectTo(robotPose, p);

      if (FLT_LT(std::abs(p.GetRotation().GetAngleAroundZaxis().ToFloat()),
                 preActionPoseAngleTolerance.ToFloat())) {
        PRINT_CH_INFO(
            "Actions", "IsCloseEnoughToPreActionPose.AtClosestPreActionPose",
            "Robot is close enough to closest preAction pose (%.1fmm, %.1fmm) "
            "with threshold (%.1fmm, %.1fmm)",
            closestPoint.x(), closestPoint.y(), output.distThresholdUsed.x(),
...
bool IDockAction::RemoveMatchingPredockPose(
    const Pose3d& pose, std::vector<Pose3d>& possiblePoses) {
  bool removed = false;
  for (auto iter = possiblePoses.begin(); iter != possiblePoses.end();) {
    if (iter->IsSameAs(pose, kSamePreactionPoseDistThresh_mm,
...
void IDockAction::GetRequiredVisionModes(
...
ActionResult IDockAction::Init() {
  ...
  ActionableObject* dockObject = dynamic_cast<ActionableObject*>(
      GetRobot().GetBlockWorld().GetLocatedObjectByID(_dockObjectID));
  ...
  if (_doNearPredockPoseCheck) {
    PreActionPoseInput preActionPoseInput(
        dockObject, GetPreActionType(), _doNearPredockPoseCheck,
        _preDockPoseDistOffsetX_mm, _preActionPoseAngleTolerance.ToFloat(),
        false, 0);

    GetPreActionPoses(GetRobot().GetPose(), GetRobot().GetCarryingComponent(),
                      GetRobot().GetBlockWorld(), preActionPoseInput,
                      preActionPoseOutput);
  ...
  auto liftSoundLambda = [this](const AnkiEvent<RobotToEngine>& event) {
    if (_curDockTrigger != _getOutDockTrigger) {
      ...
      if (ShouldPlayDockingAnimations() &&
          _getOutDockTrigger != AnimationTrigger::Count) {
    ...
    if (_liftMovingAudioEvent != GE::Invalid) {
      ...
      if (_dockAction != recvdAction) {
        PRINT_NAMED_WARNING(
            "IDockAction.MovingLiftPostDockHandler.ActionMismatch",
            "Expected %u, got %u. Ignoring.", (u32)_dockAction,
            (u32)recvdAction);
        return;
      }
  ...
  auto liftLoadLambda = [this](const AnkiEvent<RobotToEngine>& event) {
    _liftLoadState = event.GetData().Get_liftLoad().hasLoad
                         ? LiftLoadState::HAS_LOAD
                         : LiftLoadState::HAS_NO_LOAD;
  };

  _liftMovingSignalHandle = GetRobot().GetRobotMessageHandler()->Subscribe(
      RobotToEngineTag::movingLiftPostDock, liftSoundLambda);
  _liftLoadSignalHandle = GetRobot().GetRobotMessageHandler()->Subscribe(
      RobotToEngineTag::liftLoad, liftLoadLambda);
  ...
  if (GetRobot().HasExternalInterface()) {
    ...
    helper.SubscribeEngineToGame<
        MessageEngineToGameTag::RobotDeletedLocatedObject>();
  }
  ...
  if (_doNearPredockPoseCheck) {
    LOG_INFO("IDockAction.Init.BeginDockingFromPreActionPose",
             ...
             preActionPoseOutput.closestPoint.x(),
             preActionPoseOutput.closestPoint.y());

    // Set dock markers
    dockMarkerPtr =
        preActionPoseOutput.preActionPoses[preActionPoseOutput.closestIndex]
            .GetMarker();
    dockMarkerPtr2 = GetDockMarker2(preActionPoseOutput.preActionPoses,
                                    preActionPoseOutput.closestIndex);
    ...
    if (markers.empty()) {
      PRINT_NAMED_ERROR("IDockAction.Init.NoMarkers",
                        "Using currently observed markers instead of preDock "
                        "pose but no currently visible marker");
      ...
      for (const Vision::KnownMarker* marker : markers) {
        ...
        if (p.GetTranslation().LengthSq() <
            distToClosestMarker * distToClosestMarker) {
    ...
    LOG_INFO("IDockAction.Init.BeginDockingToMarker",
             "Proceeding with docking to marker %s",
             dockMarkerPtr->GetCodeName());
  }

  if (dockMarkerPtr == nullptr) {
    PRINT_NAMED_WARNING("IDockAction.Init.NullDockMarker",
                        "Dock marker is null returning failure");
    return ActionResult::BAD_MARKER;
  }

  // cache marker codes (required before SetupTurnAndVerifyAction)
  _dockMarkerCode = (nullptr != dockMarkerPtr) ? dockMarkerPtr->GetCode()
                                               : Vision::MARKER_INVALID;
  _dockMarkerCode2 = (nullptr != dockMarkerPtr2) ? dockMarkerPtr2->GetCode()
                                                 : Vision::MARKER_INVALID;

  SetupTurnAndVerifyAction(dockObject);

  if (_shouldSetCubeLights && !_lightsSet) {
    LOG_INFO("IDockAction.SetInteracting",
             "%s[%d] Setting interacting object to %d", GetName().c_str(),
             GetTag(), _dockObjectID.GetValue());
    GetRobot().GetCubeLightComponent().PlayLightAnimByTrigger(
        _dockObjectID, CubeAnimationTrigger::Interacting);
    _lightsSet = true;
  }

  // Allow actions the opportunity to check or set any properties they need to
  // this allows actions that are part of driveTo or wrappers a chance to check
  // data when they know they're at the pre-dock pose
...
ActionResult IDockAction::CheckIfDone() {
    ...
    if (actionResult == ActionResult::RUNNING) {
      ...
      if (actionResult == ActionResult::SUCCESS) {
        ...
        LOG_INFO("IDockAction.DockWithObjectHelper.BeginDocking",
                 ...
                 _dockMarkerCode,
                 Vision::Marker::GetNameForCode(_dockMarkerCode),
                 DockActionToString(_dockAction));
        if (VerifyDockingComponentValid() &&
            _dockingComponentPtr->DockWithObject(
                _dockObjectID, _dockSpeed_mmps, _dockAccel_mmps2,
                _dockDecel_mmps2, _dockMarkerCode, _dockMarkerCode2,
                _dockAction, _placementOffsetX_mm, _placementOffsetY_mm,
                _placementOffsetAngle_rad, _numDockingRetries, _dockingMethod,
                _doLiftLoadCheck, _backUpWhileLiftingCube) == RESULT_OK) {
          // NOTE: Any completion (success or failure) after this point should
          // tell
        ...
        PRINT_NAMED_WARNING(
            "IDockAction.CheckIfDone.VisualVerifyFailed",
  ...
  if (!_wasPickingOrPlacing && VerifyDockingComponentValid()) {
    ...
    // While head is moving to verification angle, this shouldn't count towards
    // the waitToVerifyTime
    ...
    if (currentTime >= _waitToVerifyTimeSecs) {
      ...
      //              "Robot has stopped moving and picking/placing. Will
      //              attempt to verify success.");

      actionResult = Verify();
    }
  } else {
    // If dock anim is null then it means the get in finished so time to start
    // the loop
...
void IDockAction::SetupTurnAndVerifyAction(const ObservableObject* dockObject) {
  ...
  if (_firstTurnTowardsObject) {
    // Set up a visual verification action to make sure we can still see the
    // correct marker of the selected object before proceeding NOTE: This also
    // disables tracking head to object if there was any
    IAction* turnTowardsDockObjectAction = new TurnTowardsObjectAction(
        _dockObjectID,
        (_visuallyVerifyObjectOnly ? Vision::Marker::ANY_CODE
                                   : _dockMarkerCode),
        0, true, false);
...
void IDockAction::UpdateDockingAnim() {
  if (_dockAnim != nullptr) {
    ...
    const ActionResultCategory resCat =
        IActionRunner::GetActionResultCategory(res);
    ...
    if (resCat != ActionResultCategory::RUNNING) {
      ...
      if (resCat != ActionResultCategory::SUCCESS) {
        PRINT_NAMED_WARNING("IDockAction.UpdateDockingAnim.AnimFailed",
                            ...
                            _dockAnim->GetName().c_str(), _dockAnim->GetTag(),
                            EnumToString(res));
      }
...
PopAWheelieAction::PopAWheelieAction(ObjectID objectID)
    : IDockAction(objectID, "PopAWheelie", RobotActionType::POP_A_WHEELIE) {}

void PopAWheelieAction::GetCompletionUnion(
  ...
  switch (_dockAction) {
    case DockAction::DA_POP_A_WHEELIE: {
      if (VerifyCarryingComponentValid() &&
        ...
        PRINT_NAMED_WARNING(
            "PopAWheelieAction.EmitCompletionSignal.ExpectedNotCarryingObject",
            "");
      } else {
    ...
    default: {
      if (GetState() != ActionResult::NOT_STARTED) {
        PRINT_NAMED_WARNING(
            "PopAWheelieAction.EmitCompletionSignal.DockActionNotSet",
...
ActionResult PopAWheelieAction::SelectDockAction(ActionableObject* object) {
  Pose3d objectPose;
  if (object->GetPose().GetWithRespectTo(GetRobot().GetPose().GetParent(),
                                         objectPose) == false) {
    PRINT_NAMED_WARNING(
        "PopAWheelieAction.SelectDockAction.PoseWrtFailed",
  ...
  // carrying a block
  const f32 dockObjectHeightWrtRobot =
      objectPose.GetTranslation().z() -
  ...
  // TODO: There might be ways to roll high blocks when not carrying object and
  // low blocks when carrying an object.
  //       Do them later.
  if (dockObjectHeightWrtRobot >
      0.5f * ROBOT_BOUNDING_Z) {  //  dockObject->GetSize().z()) {
    LOG_INFO("PopAWheelieAction.SelectDockAction.ObjectTooHigh",
             "Object is too high to pop-a-wheelie. Aborting.");
    return ActionResult::BAD_OBJECT;
  ...
  switch (_dockAction) {
    case DockAction::DA_POP_A_WHEELIE: {
      if (VerifyDockingComponentValid() &&
        ...
        LOG_INFO(
            "PopAWheelieAction.Verify.DockingFailed",
    ...
    default:
      PRINT_NAMED_WARNING("PopAWheelieAction.Verify.ReachedDefaultCase",
                          "Don't know how to verify unexpected dockAction %s.",
                          DockActionToString(_dockAction));
...
void FacePlantAction::GetCompletionUnion(
    ActionCompletedUnion& completionUnion) const {
  ObjectInteractionCompleted info;
  switch (_dockAction) {
    case DockAction::DA_FACE_PLANT: {
      if (VerifyCarryingComponentValid() &&
          _carryingComponentPtr->IsCarryingObject()) {
        PRINT_NAMED_WARNING(
            "FacePlantAction.EmitCompletionSignal.ExpectedNotCarryingObject",
            "");
      } else {
        info.objectID = _dockObjectID;
      }
    ...
    default:
      PRINT_NAMED_WARNING(
          "FacePlantAction.EmitCompletionSignal.DockActionNotSet",
          "Dock action not set before filling completion signal.");
  }
  completionUnion.Set_objectInteractionCompleted(std::move(info));
  IDockAction::GetCompletionUnion(completionUnion);
...
ActionResult FacePlantAction::SelectDockAction(ActionableObject* object) {
  ...
  if (object->GetPose().GetWithRespectTo(GetRobot().GetPose().GetParent(),
                                         objectPose) == false) {
    PRINT_NAMED_WARNING(
        "FacePlantAction.SelectDockAction.PoseWrtFailed",
        "Could not get pose of dock object w.r.t. robot's parent.");
    return ActionResult::BAD_OBJECT;
  }
...
PreActionPose::ActionType
AlignWithObjectAction::GetPreActionTypeFromAlignmentType(
  ...
  switch (alignmentType) {
    ...
    case AlignmentType::CUSTOM:
      // Normally this action uses the DOCKING preAction poses but if we are
      // aligning to a custom distance then the DOCKING poses could be too close
      // so use the PLACE_RELATIVE preAction poses. Plus, we want to be able to
      // align with non-pickupable sides.
      return PreActionPose::ActionType::PLACE_RELATIVE;
    default:
      PRINT_NAMED_ERROR(
          "AlignWithObjectAction.GetPreActionTypeByAlignmentType."
          "InvalidAlignmentType",
          "%s", EnumToString(alignmentType));
      return PreActionPose::ActionType::PLACE_RELATIVE;
  }
}
...
AlignWithObjectAction::AlignWithObjectAction(ObjectID objectID,
    ...
    : IDockAction(objectID, "AlignWithObject",
  ...
  switch (alignmentType) {
    ...
    case (AlignmentType::LIFT_PLATE): {
      ...
      // If we are aligning to the LIFT_PLATE then assume that we want the lift
      // fingers in the object grooves (as if to pickup the object) so use the
      // same docking method as pickup
...
void AlignWithObjectAction::GetCompletionUnion(
    ActionCompletedUnion& completionUnion) const {
  ObjectInteractionCompleted info;
  info.objectID = _dockObjectID;
  completionUnion.Set_objectInteractionCompleted(std::move(info));
...
ActionResult AlignWithObjectAction::SelectDockAction(ActionableObject* object) {
  ...
  // If we are aligning to the LIFT_PLATE then assume that we want the lift
  // fingers in the object grooves (as if to pickup the object) so use a special
  // align dock action which basically functions the same as pickup (does the
  // Hanns Manuever) except doesn't move the lift
  ...
  switch (_dockAction) {
    ...
    case DockAction::DA_ALIGN_SPECIAL: {
      if (VerifyDockingComponentValid() &&
        ...
        LOG_INFO("AlignWithObjectAction.Verify",
                 "Align with object SUCCEEDED!");
        result = ActionResult::SUCCESS;
      }
      break;
    }  // ALIGN
...
PickupObjectAction::PickupObjectAction(ObjectID objectID)
    : IDockAction(objectID, "PickupObject",
                  RobotActionType::PICK_AND_PLACE_INCOMPLETE) {
  _dockingMethod = (DockingMethod)kPickupDockingMethod;
  using GE = AudioMetaData::GameEvent::GenericEvent;
  SetPostDockLiftMovingAudioEvent(
      GE::Play__Robot_Vic_Sfx__Lift_High_Up_Short_Excited);
...
void PickupObjectAction::GetCompletionUnion(
    ActionCompletedUnion& completionUnion) const {
  ObjectInteractionCompleted info;
  ...
  switch (_dockAction) {
    ...
    default: {
      ...
      if (GetState() != ActionResult::NOT_STARTED) {
        PRINT_NAMED_WARNING(
            "PickupObjectAction.EmitCompletionSignal.DockActionNotSet",
            "Dock action not set before filling completion signal");
      }
    }
  }
...
ActionResult PickupObjectAction::SelectDockAction(ActionableObject* object) {
  ...
  if (object->GetPose().GetWithRespectTo(GetRobot().GetPose().GetParent(),
                                         _dockObjectOrigPose) == false) {
    PRINT_NAMED_WARNING(
        "PickupObjectAction.SelectDockAction.PoseWrtFailed",
  ...
  // carrying a block
  const f32 dockObjectHeightWrtRobot =
      _dockObjectOrigPose.GetTranslation().z() -
      GetRobot().GetPose().GetTranslation().z();
  ...
  if (VerifyCarryingComponentValid() &&
      _carryingComponentPtr->IsCarryingObject()) {
    LOG_INFO("PickupObjectAction.SelectDockAction.CarryingObject",
             "Already carrying object. Can't pickup object. Aborting.");
    return ActionResult::STILL_CARRYING_OBJECT;
  } else if (dockObjectHeightWrtRobot >
             0.5f * ROBOT_BOUNDING_Z) {  // TODO: Stop using constant
                                         // ROBOT_BOUNDING_Z for this
    _dockAction = DockAction::DA_PICKUP_HIGH;
    SetType(RobotActionType::PICKUP_OBJECT_HIGH);
  }

  // If we are either in the habitat or unsure, we should do the version of cube
  // pickup where instead of driving forward at the same time as raising the
  // lift, we drive backward. This improves the cube pickup success rate in case
  // the cube is pressed against the wall of the habitat.
  const auto habitatBeliefState = GetRobot()
                                      .GetComponent<HabitatDetectorComponent>()
                                      .GetHabitatBeliefState();
  const bool possiblyInHabitat =
      (habitatBeliefState == HabitatBeliefState::InHabitat) ||
...
ActionResult PickupObjectAction::Verify() {
  ...
  if (VerifyDockingComponentValid() &&
    ...
    // Determine whether or not we should do a SearchForNearbyObject instead of
    // TurnTowardsPose depending on if the liftLoad test resulted in HAS_NO_LOAD
    // since this could be due to sticky lift.
    if (_doLiftLoadCheck) {
      if (_liftLoadState == LiftLoadState::UNKNOWN) {
        ...
        if (_liftLoadWaitTime_ms == 0) {
          ...
          // If LiftLoadCheck times out for some reason -- lift probably just
          // couldn't get into position fast enough -- then just proceed to
          // motion check.
    ...
    // Assuming that the robot stopping coincides closely with the first call to
    // Verify(). If the cube is moving too long after the first call to Verify()
    // the cube is probably in someone's hand. If it hasn't moved at all for
    // some period before the first call to Verify() the cube probably wasn't in
    // the lift during pickup.
    if (checkObjectMotion) {
      ...
      if (nullptr == obj) {
        PRINT_NAMED_WARNING("PickupObjectAction.Verify.nullObject",
                            "ObjectID %d", _dockObjectID.GetValue());
        return ActionResult::BAD_OBJECT;
      }
      ...
      if (obj->GetActiveID() >= 0) {
        ...
        // Check that object is not moving for longer than expected following
        // the first call to Verify(). If it's moving for too long it's probably
        // being handled by someone.
        if (obj->IsMoving(&lastMovingTime)) {
          if (VerifyCarryingComponentValid() &&
              (currentTime > _firstVerifyCallTime +
                                 kMaxObjectStillMovingAfterRobotStopTime_ms)) {
        ...
        // Check that the object has moved at all in certain time window before
        // we started calling Verify(). If it hasn't moved at all we probably
        // missed, note the outcome and retry.
        else if (
            VerifyCarryingComponentValid() &&
            (_firstVerifyCallTime >
             lastMovingTime +
                 (_dockAction == DockAction::DA_PICKUP_LOW
                      ? kMaxObjectHasntMovedBeforeRobotStopTime_ms
                      : kMaxObjectHasntMovedBeforeRobotStopTimeForHighPickup_ms))) {
          _carryingComponentPtr->SetCarriedObjectAsUnattached(true);
          LOG_INFO("PickupObjectAction.Verify.ObjectDidntMoveAsExpected",
                   "lastMovedTime %d, firstTime: %d",
                   (TimeStamp_t)lastMovingTime,
                   (TimeStamp_t)_firstVerifyCallTime);
  ...
  if (_verifyAction == nullptr) {
    _verifyAction.reset(
        new VisuallyVerifyNoObjectAtPoseAction(_dockObjectOrigPose));
  ...
  switch (_dockAction) {
    ...
    case DockAction::DA_PICKUP_HIGH: {
      if (VerifyCarryingComponentValid() &&
        ...
        PRINT_NAMED_WARNING(
            "PickupObjectAction.Verify.ExpectedCarryingObject",
      ...
      if (VerifyCarryingComponentValid()) {
        carryObject = blockWorld.GetLocatedObjectByID(
            _carryingComponentPtr->GetCarryingObjectID());
      }

      if (carryObject == nullptr) {
        PRINT_NAMED_WARNING(
            "PickupObjectAction.Verify.CarryObjectNoLongerExists",
            "Object %d we were carrying no longer exists in the world.",
            VerifyCarryingComponentValid()
                ? _carryingComponentPtr->GetCarryingObjectID().GetValue()
                : -1);
      ...
      for (const auto& object : objectsWithType) {
        ...
        if (object->GetPose().IsSameAs_WithAmbiguity(
                _dockObjectOrigPose,  // dock obj orig pose is w.r.t. robot
                carryObject->GetRotationAmbiguities(),
                carryObject->GetSameDistanceTolerance() * 0.5f,
                carryObject->GetSameAngleTolerance(), Tdiff, angleDiff)) {
          LOG_INFO(
              "PickupObjectAction.Verify.ObjectInOrigPose",
              "Seeing object %d in original pose. (Tdiff = (%.1f,%.1f,%.1f), "
              "AngleDiff=%.1fdeg), carrying object %d",
              object->GetID().GetValue(), Tdiff.x(), Tdiff.y(), Tdiff.z(),
              angleDiff.getDegrees(), carryObject->GetID().GetValue());
          break;
        }
      }

      LOG_INFO("PickupObjectAction.Verify.Success",
               "Object pick-up SUCCEEDED!");
      result = ActionResult::SUCCESS;
      break;
    }  // PICKUP
  ...
  if (result == ActionResult::SUCCESS) {
    GetRobot().GetComponent<RobotStatsTracker>().IncrementBehaviorStat(
        BehaviorStat::PickedUpCube);
  }
...
PlaceObjectOnGroundAction::PlaceObjectOnGroundAction()
    : IAction("PlaceObjectOnGround", RobotActionType::PLACE_OBJECT_LOW,
...
ActionResult PlaceObjectOnGroundAction::Init() {
  ...
  if (carryingComponentRef.IsCarryingObject() == false) {
    PRINT_NAMED_WARNING(
        "PlaceObjectOnGroundAction.CheckPreconditions.NotCarryingObject",
    ...
    if (carryingComponentRef.PlaceObjectOnGround() == RESULT_OK) {
      ...
      PRINT_NAMED_WARNING(
          "PlaceObjectOnGroundAction.CheckPreconditions."
          "SendPlaceObjectOnGroundFailed",
          "Robot's SendPlaceObjectOnGround method reported failure.");
      result = ActionResult::SEND_MESSAGE_TO_ROBOT_FAILED;
    }

    const Vision::KnownMarker::Code carryObjectMarkerCode =
        carryingComponentRef.GetCarryingMarkerCode();
    _faceAndVerifyAction.reset(new TurnTowardsObjectAction(
        _carryingObjectID, carryObjectMarkerCode, 0, true, false));
    _faceAndVerifyAction->SetRobot(&GetRobot());
    _faceAndVerifyAction->ShouldSuppressTrackLocking(true);
...
ActionResult PlaceObjectOnGroundAction::CheckIfDone() {
  ...
  const bool isPickingAndPlacing =
      GetRobot().GetDockingComponent().IsPickingOrPlacing();

  if (isPickingAndPlacing) {
    _startedPlacing = true;
  }

  if (_startedPlacing && !isPickingAndPlacing &&
      !GetRobot().GetMoveComponent().IsMoving()) {
    ...
    if (actionResult != ActionResult::RUNNING &&
      ...
      PRINT_NAMED_WARNING(
          "PlaceObjectOnGroundAction.CheckIfDone.FaceAndVerifyFailed",
          "FaceAndVerify action reported failure, just clearing object %d.",
          _carryingObjectID.GetValue());
      // rsam: it's arguably whether the action should do this.
      // _carryingObjectID may no longer be equal robot.GetCarryingObjectID(),
      // and be the reason why the actionResult is != Success, which would make
      // this operation useless if the object doesn't exist anymore. I'm not
      // sure this should be clearing or totally deleting (from PoseState
      // refactor)
      GetRobot().GetBlockWorld().ClearLocatedObjectByIDInCurOrigin(
          _carryingObjectID);
    }
...
void PlaceObjectOnGroundAction::GetCompletionUnion(
    ActionCompletedUnion& completionUnion) const {
  ObjectInteractionCompleted info;
...
PlaceObjectOnGroundAtPoseAction::PlaceObjectOnGroundAtPoseAction(
    const Pose3d& placementPose, const bool useExactRotation,
    const bool checkFreeDestination, const float destinationObjectPadding_mm)
    : CompoundActionSequential() {
  auto* driveAction = new DriveToPlaceCarriedObjectAction(
      placementPose, true, useExactRotation, checkFreeDestination,
...
PlaceRelObjectAction::PlaceRelObjectAction(ObjectID objectID,
    ...
    : IDockAction(objectID, "PlaceRelObject",
                  RobotActionType::PICK_AND_PLACE_INCOMPLETE),
  ...
  using GE = AudioMetaData::GameEvent::GenericEvent;
  SetPostDockLiftMovingAudioEvent(
      GE::Play__Robot_Vic_Sfx__Lift_High_Down_Short_Excited);
  ...
  if (!(FLT_NEAR(placementOffsetX_mm, 0.f) &&
    ...
    PRINT_CH_INFO(
        "Actions", "PlaceRelObjectAction.Constructor.WillNotCheckPreDockPoses",
...
ActionResult PlaceRelObjectAction::InitInternal() {
  ...
  // If attempting to place the block off to the side of the target, do it even
  // blinder so that Cozmo doesn't fail when he inevitably looses sight of the
  // tracker
...
void PlaceRelObjectAction::GetCompletionUnion(
    ActionCompletedUnion& completionUnion) const {
  ObjectInteractionCompleted info;
  ...
  switch (_dockAction) {
    ...
    case DockAction::DA_PLACE_LOW: {
      ...
      if (object == nullptr) {
        PRINT_NAMED_WARNING(
            "PlaceRelObjectAction.EmitCompletionSignal.NullObject",
    ...
    default: {
      ...
      if (GetState() != ActionResult::NOT_STARTED) {
        PRINT_NAMED_WARNING(
            "PlaceRelObjectAction.EmitCompletionSignal.DockActionNotSet",
            "Dock action not set before filling completion signal.");
      }
    }
  }
...
ActionResult PlaceRelObjectAction::SelectDockAction(ActionableObject* object) {
  if (VerifyCarryingComponentValid() &&
      !_carryingComponentPtr->IsCarryingObject()) {
    LOG_INFO("PlaceRelObjectAction.SelectDockAction.NotCarryingObject",
             "Can't place if not carrying an object. Aborting.");
    return ActionResult::NOT_CARRYING_OBJECT_ABORT;
  }

  if (!_placeObjectOnGroundIfCarrying && VerifyDockingComponentValid() &&
      !_dockingComponentPtr->CanStackOnTopOfObject(*object)) {
    PRINT_NAMED_WARNING(
        "PlaceRelObjectAction.SelectDockAction.CantStackOnObject", "");
    return ActionResult::BAD_OBJECT;
  }

  _dockAction = _placeObjectOnGroundIfCarrying ? DockAction::DA_PLACE_LOW
    ...
    case DockAction::DA_PLACE_HIGH: {
      if (GetRobot().GetDockingComponent().GetLastPickOrPlaceSucceeded()) {
        if (VerifyCarryingComponentValid() &&
          ...
          PRINT_NAMED_WARNING(
              "PlaceRelObjectAction.Verify.ExpectedNotCarryingObject",
              "Expecting robot to think it's NOT carrying an object at this "
              "point.");
          return ActionResult::STILL_CARRYING_OBJECT;
        }
        ...
        if (_placementVerifyAction == nullptr) {
          _placementVerifyAction.reset(new TurnTowardsObjectAction(
              _carryObjectID, Radians(0), true, false));
        ...
        if (result != ActionResult::RUNNING) {
          ...
          if (result != ActionResult::SUCCESS) {
            PRINT_NAMED_WARNING(
                "PlaceRelObjectAction.Verify.VerifyFailed",
                "Robot thinks it placed the object %s, but verification of "
                "placement "
            ...
            GetRobot().GetBlockWorld().ClearLocatedObjectByIDInCurOrigin(
                _carryObjectID);
          } else if (_dockAction == DockAction::DA_PLACE_HIGH &&
                     !_verifyComplete) {
            ...
            if (result == ActionResult::SUCCESS) {
              ...
              // leave it up, since we are assuming we are still carrying the
              // object)
              _placementVerifyAction.reset(new MoveLiftToHeightAction(
        ...
        PRINT_NAMED_WARNING(
            "PlaceRelObjectAction.Verify.DockingFailed",
...
ActionResult PlaceRelObjectAction::TransformPlacementOffsetsRelativeObject() {
  ...
  const bool success =
      topPose.GetWithRespectTo(GetRobot().GetPose(), dockObjectWRTRobot);
  ...
  if (Util::IsNear(robotObjRelRotation_rad, kInAlignment_rad,
                   kRotationTolerance_rad)) {
    xAbsolutePlacementOffset_mm = -_relOffsetX_mm;
    yAbsolutePlacementOffset_mm = _relOffsetY_mm;
  } else if (Util::IsNear(robotObjRelRotation_rad, kCounterClockwise_rad,
                          kRotationTolerance_rad)) {
    xAbsolutePlacementOffset_mm = _relOffsetY_mm;
    yAbsolutePlacementOffset_mm = _relOffsetX_mm;
  } else if (Util::IsNear(robotObjRelRotation_rad, kClockwise_rad,
                          kRotationTolerance_rad)) {
    xAbsolutePlacementOffset_mm = -_relOffsetY_mm;
    yAbsolutePlacementOffset_mm = -_relOffsetX_mm;
  } else if (Util::IsNear(robotObjRelRotation_rad, kOppposite_rad,
                          kRotationTolerance_rad) ||
             Util::IsNear(robotObjRelRotation_rad, kOppposite_rad_neg,
                          kRotationTolerance_rad)) {
    xAbsolutePlacementOffset_mm = _relOffsetX_mm;
    yAbsolutePlacementOffset_mm = -_relOffsetY_mm;
  } else {
    PRINT_NAMED_WARNING(
        "PlaceRelObjectAction.CalculatePlacementOffset.InvalidOrientation",
        "Robot and block are not within alignment threshold - rotation:%f "
        "threshold:%f",
  ...
  if (FLT_LT(xAbsolutePlacementOffset_mm, -kMaxNegativeXPlacementOffset)) {
    PRINT_NAMED_ERROR(
        "PlaceRelObjectAction.TransformPlacementOffsetsRelativeObject."
        "InvalidNegativeOffset",
        "Attempted to set negative xOffset. xOffset:%f, yOffset:%f",
        xAbsolutePlacementOffset_mm, yAbsolutePlacementOffset_mm);
...
// The max lateral offset an object at a given distance from a pose can be such
// that it is still visible by the camera with x and y offsets applied maxOffset
// is a lateral distance from pose relative to object
static Result GetMaxOffsetObjectStillVisible(
    const Vision::Camera& camera, const ObservableObject* object,
    const f32 distanceToObject, const f32 desiredOffsetX_mm,
    const f32 desiredOffsetY_mm, const Pose3d& pose, f32& maxOffset) {
  ...
  const Result res = object->GetClosestMarkerPose(
      pose, true, objectPoseWrtRobot_unused, closestMarker);
  if (res != RESULT_OK) {
  ...
  if (markers.size() != 1) {
    // This should not happen we just found this marker by calling
    // GetClosestMarkerPose
  ...
  if (distance < minDistance) {
    PRINT_NAMED_WARNING("GetMaxOffsetObjectStillVisible.InvalidDistance",
                        "Total distance to object %f < min possible distance "
                        "%f to see the object",
                        distance, minDistance);
    return RESULT_FAIL;
  }

  // Find the distance between the center of the camera's fov and the edge of it
  // at the given distance + x_offset
...
ActionResult PlaceRelObjectAction::ComputePlaceRelObjectOffsetPoses(
    const ActionableObject* object, const f32 placementOffsetX_mm,
    const f32 placementOffsetY_mm, const Pose3d& robotPose,
    const Pose3d& worldOrigin, const CarryingComponent& carryingComp,
    BlockWorld& blockWorld, const VisionComponent& visionComp,
  ...
  const IDockAction::PreActionPoseInput preActionPoseInput(
      object, PreActionPose::ActionType::PLACE_RELATIVE, false, 0,
      DEFAULT_PREDOCK_POSE_ANGLE_TOLERANCE, false, 0);
  ...
  if (preActionPoseOutput.actionResult == ActionResult::SUCCESS) {
    ...
    for (PoseIter fullIter = possiblePoses.begin();
         fullIter != possiblePoses.end();) {
      const Pose3d& idealCenterPose =
          object->GetZRotatedPointAboveObjectCenter(0.f);
      Pose3d preDocWRTUnrotatedBlock;
      const bool posesOk =
          fullIter->GetWithRespectTo(idealCenterPose, preDocWRTUnrotatedBlock);
      if (!posesOk) {
        // this should not be possible at all, since the predock poses and the
        // object have to be in same origin
        PRINT_NAMED_ERROR(
            "DriveToPlaceRelObjectAction.GetPossiblePosesFunc.InvalidPoses",
      ...
      const bool isPoseInvalid =
          (xOffsetRelevant &&
           (FLT_GT(poseX, 0) != FLT_GT(placementOffsetX_mm, 0))) ||
          (yOffsetRelevant &&
           (FLT_GT(poseY, 0) != FLT_GT(placementOffsetY_mm, 0)));

      if (isPoseInvalid) {
        fullIter = possiblePoses.erase(fullIter);

        LOG_INFO(
            "DriveToPlaceRelObjectAction.PossiblePosesFunc.RemovingInvalidPose",
            "Removing pose x:%f y:%f because Cozmo can't place at offset x:%f, "
            "y:%f, xRelevant:%d, yRelevant:%d",
            poseX, poseY, placementOffsetX_mm, placementOffsetY_mm,
            xOffsetRelevant, yOffsetRelevant);
      } else {
        // We need to visually verify placement since there are high odds
        // that we will bump objects while placing relative to them, so if
        // possible place using a y offset
        const bool onlyOnePlacementDirection =
            xOffsetRelevant != yOffsetRelevant;
        const bool currentXPoseIdeal =
            xOffsetRelevant &&
            IN_RANGE(poseY, -minIllegalOffset, minIllegalOffset);
        const bool currentYPoseIdeal =
            yOffsetRelevant &&
            IN_RANGE(poseX, -minIllegalOffset, minIllegalOffset);

        if (onlyOnePlacementDirection && possiblePoses.size() > 2 &&
          ...
          const Radians angle =
              preDocWRTUnrotatedBlock.GetRotation().GetAngleAroundZaxis();
          f32 preDockOffsetX = placementOffsetX_mm;
          f32 preDockOffsetY = placementOffsetY_mm;
          f32 distanceToObject = trans.x();

          // we expect the Z angle to be a quarter (0,90,180,270). Check below
          ...
          DEV_ASSERT(angle.IsNear(DEG_TO_RAD(0.0f), kAngleEpsilonRad) ||
                     ...
                     "PreDockPoseAngleNotNearQuarter");

          const bool isAlignedWithYAxis =
              angle.IsNear(DEG_TO_RAD(90.0f), kAngleEpsilonRad) ||
              angle.IsNear(DEG_TO_RAD(270.0f), kAngleEpsilonRad);

          // Flip the x and y offset and use the y translation should this
          // preDock pose be at 90 or 270 degrees relative to the object
          ...
          // Find the max lateral offset from the preDock pose that the object
          // will still be visible This is to ensure we will be seeing the
          // object when we are at the preDock pose
          f32 maxOffset_mm = 0;
          const Result res = GetMaxOffsetObjectStillVisible(
              visionComp.GetCamera(), object, distanceToObject, preDockOffsetX,
              preDockOffsetY, *fullIter, maxOffset_mm);
          if (res != RESULT_OK) {
            PRINT_NAMED_WARNING(
                "DriveToPlaceRelObjectAction.GetPossiblePosesFunc."
                "GetMaxYOffset",
                "Failed to get max offset where %s is still visible from "
                "distance %f with placement offsets (%f, %f)",
                EnumToString(object->GetType()), trans.x(), placementOffsetX_mm,
                placementOffsetY_mm);
          ...
          // Subtract a bit of padding from maxOffset to account for errors in
          // path following should we actually decide to drive to this predock
          // pose Still doesn't guarantee that we will be seeing the object once
          // we get to the preDock pose but greatly increases our chances
          const static f32 padding_mm = 20;
          if (maxOffset_mm > padding_mm) {
            maxOffset_mm -= padding_mm;
          }

          // Depending on which preDock pose this is, either the x or y
          // placementOffset (whichever corresponds to horizontal distance
          // relative to the preDock pose) will need to be clipped to the
          // maxOffset
          ...
          preDocWRTUnrotatedBlock.SetTranslation(
              {trans.x() + clipX_mm, trans.y() + clipY_mm, trans.z()});

          const bool poseOriginOk =
              preDocWRTUnrotatedBlock.GetWithRespectTo(worldOrigin, *fullIter);
          if (!poseOriginOk) {
            // this should not be possible at all, since the predock poses are
            // in robot origin
            PRINT_NAMED_ERROR(
                "DriveToPlaceRelObjectAction.GetPossiblePosesFunc."
                "UnrotatedBlockPoseBadOrigin",
                "Could not obtain predock pose from unrotated wrt origin.");
            continue;
          }

          Point2f distThreshold = ComputePreActionPoseDistThreshold(
              *fullIter, object->GetPose(),
              DEFAULT_PREDOCK_POSE_ANGLE_TOLERANCE);

          // If the new preAction pose is close enough to the robot's current
          // pose mark as alreadyInPosition Don't really care about z
          static const f32 kDontCareZThreshold = 100;
          if (robotPose.IsSameAs(
                  *fullIter,
    ...
    PRINT_CH_INFO("Actions",
                  "DriveToPlaceRelObjectAction.PossiblePosesFunc."
                  "PossiblePosesResultNotSuccess",
                  "Received possible poses result:%u",
                  preActionPoseOutput.actionResult);
  }
  ...
  if (possiblePoses.size() > 0) {
    ...
    PRINT_CH_INFO("Actions",
                  ...
                  "After filtering invalid pre-doc poses none remained for "
                  "placement offset x:%f, y%f",
...
RollObjectAction::RollObjectAction(ObjectID objectID)
    : IDockAction(objectID, "RollObject", RobotActionType::ROLL_OBJECT_LOW) {
  _dockingMethod = (DockingMethod)kRollDockingMethod;
  _dockAction = DockAction::DA_ROLL_LOW;
  using GE = AudioMetaData::GameEvent::GenericEvent;
  SetPostDockLiftMovingAudioEvent(
      GE::Play__Robot_Vic_Sfx__Lift_High_Down_Long_Excited);
}
...
void RollObjectAction::EnableRollWithoutDock(bool enable) {
  _dockAction =
      enable ? DockAction::DA_POST_DOCK_ROLL : DockAction::DA_ROLL_LOW;
  SetName(enable ? "RollWithoutDock" : "RollObject");

  // Don't check if we are near a predock pose because we won't actually be
  // docking
  SetDoNearPredockPoseCheck(!enable);

  // We are likely right next to the object to roll so don't bother turning
  // towards it/trying to verify it is in front of us
  SetShouldFirstTurnTowardsObject(!enable);
}

bool RollObjectAction::CanActionRollObject(
    const DockingComponent& dockingComponent, const ObservableObject* object) {
  return dockingComponent.CanPickUpObjectFromGround(*object);
}

void RollObjectAction::GetCompletionUnion(
    ActionCompletedUnion& completionUnion) const {
  ObjectInteractionCompleted info;
  switch (_dockAction) {
    ...
    case DockAction::DA_POST_DOCK_ROLL: {
      if (VerifyCarryingComponentValid() &&
          _carryingComponentPtr->IsCarryingObject()) {
        PRINT_NAMED_WARNING(
            "RollObjectAction.EmitCompletionSignal.ExpectedNotCarryingObject",
            "");
      } else {
        info.objectID = _dockObjectID;
      }
    ...
    default: {
      ...
      if (GetState() != ActionResult::NOT_STARTED) {
        PRINT_NAMED_WARNING(
            "RollObjectAction.EmitCompletionSignal.DockActionNotSet",
            "Dock action not set before filling completion signal.");
      }
    }
  }
...
ActionResult RollObjectAction::SelectDockAction(ActionableObject* object) {
  // Record the object's original pose (before picking it up) so we can
  // verify later whether we succeeded.
  // Make it w.r.t. robot's parent so we don't have to worry about differing
  // origins later.
  if (object->GetPose().GetWithRespectTo(GetRobot().GetPose().GetParent(),
                                         _dockObjectOrigPose) == false) {
    PRINT_NAMED_WARNING(
        "RollObjectAction.SelectDockAction.PoseWrtFailed",
        "Could not get pose of dock object w.r.t. robot's parent.");
    return ActionResult::BAD_OBJECT;
  }
  ...
  if (block == nullptr) {
    PRINT_NAMED_WARNING("RollObjectAction.SelectDockAction.NonBlock",
                        "Only blocks can be rolled");
    return ActionResult::BAD_OBJECT;
  }
  Pose3d junk;
  ...
  if (dockObjectHeightWrtRobot >
      0.5f * ROBOT_BOUNDING_Z) {  //  dockObject->GetSize().z()) {
    LOG_INFO("RollObjectAction.SelectDockAction.ObjectTooHigh",
             "Object is too high to roll. Aborting.");
    return ActionResult::BAD_OBJECT;
  } else if (VerifyCarryingComponentValid() &&
             _carryingComponentPtr->IsCarryingObject()) {
    ...
    case DockAction::DA_POST_DOCK_ROLL: {
      if (VerifyDockingComponentValid() &&
        ...
        if (VerifyCarryingComponentValid() &&
            (_carryingComponentPtr->IsCarryingObject() == true)) {
          PRINT_NAMED_WARNING(
              "RollObjectAction.Verify.ExpectedNotCarryingObject", "");
          result = ActionResult::STILL_CARRYING_OBJECT;
          break;
        }

        // If the physical robot thinks it succeeded, verify that the expected
        // marker is being seen
        if (_rollVerifyAction == nullptr) {
          // Since rolling is the only action that moves the block and then
          // immediately needs to visually verify The head needs to look down
          // more to account for the fact the block pose moved towards us and
          // then we can do the verification
          _rollVerifyAction.reset(new CompoundActionSequential(
              {new MoveHeadToAngleAction(kAngleToLookDown),
               new VisuallyVerifyObjectAction(
                   _dockObjectID, _expectedMarkerPostRoll->GetCode())}));

          _rollVerifyAction->ShouldSuppressTrackLocking(true);
          _rollVerifyAction->SetRobot(&GetRobot());

          // Do one update step immediately after creating the action to get
          // Init done
        ...
        if (result != ActionResult::RUNNING) {
          ...
          if (result != ActionResult::SUCCESS) {
            LOG_INFO(
                "RollObjectAction.Verify.VisualVerifyFailed",
        ...
        // failed to dock/track.
        PRINT_NAMED_WARNING(
            "RollObjectAction.Verify.DockingFailed",
