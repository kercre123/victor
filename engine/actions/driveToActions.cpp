...
DriveToObjectAction::DriveToObjectAction(
    const ObjectID& objectID, const PreActionPose::ActionType& actionType,
    const f32 predockOffsetDistX_mm, const bool useApproachAngle,
    const f32 approachAngle_rad)
    : IAction("DriveToObject", RobotActionType::DRIVE_TO_OBJECT,
...
void DriveToObjectAction::SetApproachAngle(const f32 angle_rad) {
  if (GetState() != ActionResult::NOT_STARTED) {
    PRINT_NAMED_WARNING(
        "DriveToObjectAction.SetApproachAngle.Invalid",
        "Tried to set the approach angle, but action has already started");
    return;
  }

  LOG_INFO("DriveToObjectAction.SetApproachingAngle", "[%d] %f rad", GetTag(),
           angle_rad);
...
const bool DriveToObjectAction::GetClosestPreDockPose(
    ActionableObject* object, Pose3d& closestPose) const {
  const IDockAction::PreActionPoseInput preActionPoseInput(
      object, _actionType, false, _predockOffsetDistX_mm,
      _preActionPoseAngleTolerance_rad, _useApproachAngle,
  ...
  if (preActionPoseOutput.actionResult == ActionResult::SUCCESS) {
    if (preActionPoseOutput.preActionPoses.size() > 0) {
      const bool closestIndexValid = preActionPoseOutput.closestIndex <
                                     preActionPoseOutput.preActionPoses.size();
      DEV_ASSERT_MSG(
          closestIndexValid,
          "DriveToObjectAction.GetClosestPreDockPose.ClosestIndexOutOfRange",
          "Attempted to access closest index %zu when preactionPoses has a "
          "size %zu",
...
ActionResult DriveToObjectAction::GetPossiblePoses(
  ...
  if (preActionPoseOutput.preActionPoses.empty()) {
    PRINT_NAMED_WARNING(
        "DriveToObjectAction.CheckPreconditions.NoPreActionPoses",
        "ActionableObject %d did not return any pre-action poses with action "
        "type %d.",
  ...
  if (alreadyInPosition) {
    Pose3d p =
        preActionPoseOutput.preActionPoses[preActionPoseOutput.closestIndex]
            .GetPose();
    LOG_INFO(
        "DriveToObjectAction.GetPossiblePoses.UseRobotPose",
        "Robot's current pose (x:%f y:%f a:%f) is close enough to preAction "
        "pose (x:%f y:%f a:%f)"
...
ActionResult DriveToObjectAction::InitHelper(ActionableObject* object) {
  ...
  if (PreActionPose::ActionType::NONE == _actionType) {
    if (_distance_mm < 0.f) {
      ...
      if (false ==
          object->GetPose().GetWithRespectTo(GetRobot().GetPose().GetParent(),
                                             objectWrtRobotParent)) {
        PRINT_NAMED_ERROR(
            "DriveToObjectAction.InitHelper.PoseProblem",
        ...
        if (currentDistance < _distance_mm) {
          ...
          possiblePoses.push_back(Pose3d(std::atan2f(-vec.y(), -vec.x()),
                                         Z_AXIS_3D(), T,
                                         objectWrtRobotParent.GetParent()));
        }
  ...
  // In case we are re-running this action, make sure compound actions are
  // cleared. These will do nothing if compoundAction has nothing in it yet
  // (i.e., on first Init)
  ...
  if (result == ActionResult::SUCCESS) {
    ...
    if (!GetRobot().GetCarryingComponent().IsCarryingObject(object->GetID())) {
      ...
      auto* turnTowardsObjectAction = new TurnTowardsObjectAction(
          _objectID, Radians(0), _visuallyVerifyWhenDone, headTrackWhenDone);
      LOG_DEBUG("IActionRunner.CreatedSubAction",
                "Parent action [%d] %s created a sub action [%d] %s", GetTag(),
                GetName().c_str(), turnTowardsObjectAction->GetTag(),
...
ActionResult DriveToObjectAction::Init() {
  ...
  if (object == nullptr) {
    PRINT_NAMED_WARNING(
        "DriveToObjectAction.CheckPreconditions.NoObjectWithID",
        "Block world does not have an ActionableObject with ID=%d.",
        _objectID.GetValue());
    return ActionResult::BAD_OBJECT;
  }

  // Use a helper here so that it can be shared with
  ...
  // Need to check if we have set the cube lights already in case the action was
  // reset
  if (_shouldSetCubeLights && !_lightsSet) {
    LOG_INFO("DriveToObjectAction.SetInteracting",
             "%s[%d] Setting interacting object to %d", GetName().c_str(),
             GetTag(), _objectID.GetValue());
    GetRobot().GetCubeLightComponent().PlayLightAnimByTrigger(
        _objectID, CubeAnimationTrigger::DrivingTo);
    _lightsSet = true;
  }
...
ActionResult DriveToObjectAction::CheckIfDone() {
  ...
  if (result == ActionResult::SUCCESS) {
    if (!_doPositionCheckOnPathCompletion) {
      LOG_INFO("DriveToObjectAction.CheckIfDone.SkippingPositionCheck",
               "Action complete");
      return result;
    }
    ...
    if (object == nullptr) {
      PRINT_NAMED_WARNING(
          "DriveToObjectAction.CheckIfDone.NoObjectWithID",
          "Block world does not have an ActionableObject with ID=%d.",
          _objectID.GetValue());
      ...
      if (false ==
        ...
        const f32 distanceSq =
            (Point2f(objectPoseWrtRobotParent.GetTranslation()) -
             Point2f(GetRobot().GetPose().GetTranslation()))
                .LengthSq();
...
void DriveToObjectAction::GetCompletionUnion(
...
DriveToPlaceCarriedObjectAction::DriveToPlaceCarriedObjectAction(
    const Pose3d& placementPose, const bool placeOnGround,
    const bool useExactRotation, const bool checkDestinationFree,
    const float destinationObjectPadding_mm)
    : DriveToObjectAction(0,
                          placeOnGround ? PreActionPose::PLACE_ON_GROUND
...
ActionResult DriveToPlaceCarriedObjectAction::Init() {
  ...
  if (GetRobot().GetCarryingComponent().IsCarryingObject() == false) {
    PRINT_NAMED_WARNING(
        "DriveToPlaceCarriedObjectAction.CheckPreconditions.NotCarryingObject",
    ...
    if (object == nullptr) {
      PRINT_NAMED_ERROR(
          "DriveToPlaceCarriedObjectAction.CheckPreconditions.NoObjectWithID",
          "Robot %d's block world does not have an ActionableObject with "
          "ID=%d.",
          GetRobot().GetID(), _objectID.GetValue());

      result = ActionResult::BAD_OBJECT;
    } else {
      // Compute the approach angle given the desired placement pose of the
      // carried block
      if (_useExactRotation) {
        f32 approachAngle_rad;
        ActionResult res = IDockAction::ComputePlacementApproachAngle(
            GetRobot(), _placementPose, approachAngle_rad);
        if (res != ActionResult::SUCCESS) {
          PRINT_NAMED_WARNING(
              "DriveToPlaceCarriedObjectAction.Init."
              "FailedToComputeApproachAngle",
              "");
          return res;
        }
      ...
      ActionableObject* tempObject =
          dynamic_cast<ActionableObject*>(object->CloneType());
      DEV_ASSERT(tempObject != nullptr,
                 "DriveToPlaceCarriedObjectAction.Init.DynamicCastFail");
...
ActionResult DriveToPlaceCarriedObjectAction::CheckIfDone() {
  ...
  if (_checkDestinationFree) {
    ...
    if (!isFree) {
      LOG_INFO(
          "DriveToPlaceCarriedObjectAction.PlacementGoalNotFree",
...
bool DriveToPlaceCarriedObjectAction::IsPlacementGoalFree() const {
  ObservableObject* object = GetRobot().GetBlockWorld().GetLocatedObjectByID(
      GetRobot().GetCarryingComponent().GetCarryingObjectID());
  if (nullptr != object) {
    ...
    // TODO rsam: this only checks for other cubes, but not for unknown
    // obstacles since we don't have collision sensor
    std::vector<ObservableObject*> intersectingObjects;
    GetRobot().GetBlockWorld().FindLocatedIntersectingObjects(
        candidateQuad, intersectingObjects, _destinationObjectPadding_mm,
        ignoreSelfFilter);
...
DriveToPoseAction::DriveToPoseAction()
    : IAction("DriveToPose", RobotActionType::DRIVE_TO_POSE,
              (u8)AnimTrackFlag::BODY_TRACK),
      _selectedGoalIndex(std::make_shared<Planning::GoalID>(0)) {}

DriveToPoseAction::DriveToPoseAction(const Pose3d& pose) : DriveToPoseAction() {
  SetGoals({pose});
}
...
Result DriveToPoseAction::SetGoals(const std::vector<Pose3d>& poses) {
  ...
  if (_goalPoses.size() == 1) {
    LOG_INFO("DriveToPoseAction.SetGoals",
             "[%d] Setting pose goal to (%.1f,%.1f,%.1f) @ %.1fdeg", GetTag(),
    ...
    LOG_INFO("DriveToPoseAction.SetGoals",
             "[%d] Setting %lu possible goal options.", GetTag(),
...
ActionResult DriveToPoseAction::Init() {
  ...
  // Just in case, ask the ProxSensor to check if the lift might need
  // calibration
  // TODO: if we later follow up and decide we should calibrate the motors, we
  // should delegate
  ...
  // todo: we might consider dynamically turning off _precompute if
  // GetCollisionArea() of the map is negligible
  ...
  if (!_isGoalSet) {
    ...
    for (auto& pose : _goalPoses) {
      if (pose.GetWithRespectTo(GetRobot().GetWorldOrigin(), pose) == false) {
        // this means someone passed in a goal in a different origin than the
        // robot.
    ...
    *_selectedGoalIndex = 0;
    ...
    if (_precompute) {
      planningResult =
          pathComponent.PrecomputePath(_goalPoses, _selectedGoalIndex);
    }
    ...
    if (pathComponent.IsPlanReady() || !_precompute) {
      ...
      planningResult =
          pathComponent.StartDrivingToPose(_goalPoses, _selectedGoalIndex);
    }

    if (planningResult != RESULT_OK) {
      LOG_INFO("DriveToPoseAction.Init.FailedToFindPath",
               "[%d] Failed to get path to goal pose.", GetTag());
...
ActionResult DriveToPoseAction::CheckIfDone() {
  ...
  switch (GetRobot().GetPathComponent().GetDriveToPoseStatus()) {
    case ERobotDriveToPoseStatus::Failed:
      LOG_INFO("DriveToPoseAction.CheckIfDone.Failure",
               "Robot driving to pose failed");
      _timeToAbortPlanning = -1.0f;
      result = ActionResult::PATH_PLANNING_FAILED_ABORT;
      break;
    ...
    case ERobotDriveToPoseStatus::Ready: {
      ...
      // No longer traversing the path, so check to see if we ended up in the
      // right place
      Vec3f Tdiff;
      ...
      // If the goals were generated from an object then compute the distance
      // threshold using the pose of the goal that was actually selected
      if (_useObjectPose) {
        const Point2f thresh = ComputePreActionPoseDistThreshold(
            _goalPoses[*_selectedGoalIndex], _objectPoseGoalsGeneratedFrom,
            _goalAngleThreshold);
      ...
      if (GetRobot().GetPose().IsSameAs(_goalPoses[*_selectedGoalIndex],
                                        distanceThreshold, _goalAngleThreshold,
                                        Tdiff)) {
        LOG_INFO("DriveToPoseAction.CheckIfDone.Success",
                 "[%d] Robot successfully finished following path "
                 "(Tdiff=%.1fmm) robotPose (%.1f, %.1f) goalPose (%.1f %.1f) "
                 "threshold (%.1f %.1f).",
                 GetTag(), Tdiff.Length(),
                 GetRobot().GetPose().GetTranslation().x(),
                 GetRobot().GetPose().GetTranslation().y(),
                 ...
                 distanceThreshold.x(), distanceThreshold.y());
      ...
      // and it is no longer executing it, but we appear to not be in position
      else if (GetRobot().GetPathComponent().GetLastSentPathID() ==
               GetRobot().GetPathComponent().GetLastRecvdPathID()) {
        LOG_INFO("DriveToPoseAction.CheckIfDone.DoneNotInPlace",
                 "[%d] Robot is done traversing path, but is not in position "
                 "(dist=%.1fmm). lastReceivedPathID=%d lastSentPathID=%d"
        ...
        PRINT_NAMED_ERROR(
            "DriveToPoseAction.CheckIfDone.Failure",
    ...
    case ERobotDriveToPoseStatus::WaitingToCancelPathAndSetFailure:
      // nothing to do, just waiting for the robot (path component will timeout
      // on it's own here, if needed)
      break;
  }

  // If we are no longer running and have at least started moving (path planning
  // succeeded) then start the drivingEnd animation and keep this action running
  // VIC-6077 VIC-5039: the line with FAILED_TRAVERSING_PATH shouldnt be
  // necessary according to the above comment, but there's some loop where it
  // gets stuck in FAILED_TRAVERSING_PATH and improperly calls EndDrivingAnim
  // without it
...
ActionResult DriveToPoseAction::HandleComputingPath() {
  ...
  const ERobotDriveToPoseStatus status =
      GetRobot().GetPathComponent().GetDriveToPoseStatus();
  ...
  if (_timeToAbortPlanning < 0.0f) {
    ...
    LOG_INFO(
        "DriveToPoseAction.HandleComputingAndFollowingPath."
        "ComputingPathTimeout",
  ...
  if (_precompute) {
    if (pathComponent.IsReplanning()) {
      ...
      if (finishedDriving && animHandler.InDrivingAnimsState() &&
          !animHandler.HasFinishedDrivingEndAnim()) {
    ...
    if (pathComponent.IsPlanReady()) {
      ...
      if (animHandler.InPlanningAnimsState() &&
          !animHandler.HasFinishedPlanningEndAnim()) {
        // Has no effect if it already called
        animHandler.EndPlanningAnim();
      } else {
        // Start following the plan. If the drive center pose doesn't match
        // where the plan originates from, it will start a new plan from scratch
        auto planningResult =
            pathComponent.StartDrivingToPose(_goalPoses, _selectedGoalIndex);
        if (planningResult != RESULT_OK) {
    ...
    } else if (animHandler.InDrivingAnimsState() &&
               !animHandler.HasFinishedDrivingEndAnim()) {
      animHandler.EndDrivingAnim();
    } else {
      ...
      // todo: maybe only play this if there are known obstacles between the
      // robot and closest goal?
...
ActionResult DriveToPoseAction::HandleFollowingPath() {
  ...
  if (_precompute && pathComponent.IsReplanning()) {
    ...
    if (finishedDriving && animHandler.InDrivingAnimsState() &&
      ...
      if (!planReady) {
        // Plan still isnt ready so start animations, which are then terminated
        // in the next two elseif blocks
        animHandler.StartPlanningAnim();
      } else {
        // Plan is ready now so start driving it
        pathComponent.SetStartPath(true);
      }
    } else if (animHandler.InPlanningAnimsState() &&
               !animHandler.HasFinishedPlanningEndAnim()) {
      ...
      if (planReady) {
        animHandler.EndPlanningAnim();
               ...
               animHandler.HasFinishedPlanningEndAnim()) {
      // The planning animation was started but finished, meaning the plan is
      // ready to start driving it
      if (planReady) {
        pathComponent.SetStartPath(true);
      }
    }
  } else {
    // Following path while not precomputing, or the precomputing finished
    // (along with its planning animations)
    ...
    PRINT_PERIODIC_CH_INFO(
        ...
        GetTag(), GetRobot().GetPathComponent().GetCurrentPathSegment(),
        GetRobot().GetPathComponent().GetLastSentPathID(),
        GetRobot().GetPathComponent().GetLastRecvdPathID());
  }
...
IDriveToInteractWithObject::IDriveToInteractWithObject(
    const ObjectID& objectID, const PreActionPose::ActionType& actionType,
    const f32 predockOffsetDistX_mm, const bool useApproachAngle,
    const f32 approachAngle_rad, const Radians& maxTurnTowardsFaceAngle_rad,
    const bool sayName)
    : CompoundActionSequential(),
      _objectID(objectID),
      _preDockPoseDistOffsetX_mm(predockOffsetDistX_mm) {
  IActionRunner* driveToObjectAction =
      new DriveToObjectAction(_objectID, actionType, predockOffsetDistX_mm,
                              useApproachAngle, approachAngle_rad);

  // TODO: Use the function-based ShouldIgnoreFailure option for AddAction to
  // catch some failures of DriveToObject earlier
  //  (Started to do this but it started to feel messy/dangerous right before
  //  ship)
  /*
  ...
  ICompoundAction::ShouldIgnoreFailureFcn shouldIgnoreFailure = [](ActionResult
  result, const IActionRunner* action)
  {
  if(nullptr == action)
  {
  ...
  "Not expecting ShouldIgnoreFailure to be called for successful action
  result"); return false;
  }
  ...
  const ObjectInteractionCompleted& objInteractionCompleted =
  completionUnion.Get_objectInteractionCompleted();
  ...
  */
  ...
  if (kEnablePredockDistanceCheckFix) {
    // This is a workaround for the bug in COZMO-5880. The problem is that the
    // DriveTo action has a predock pose check which is different from the Dock
    // action. This causes DriveTo to succeed, but Dock to fail. Then, if this
    // whole action is retried, the same thing happens, and the robot fails to
    // dock without ever moving

    // The fix is to _not_ do the check within the dock action, but _only_ if
    // the entire driveTo action succeeds. We need to ignore failures from the
    // drive action because of the way the proxy action works, so we work around
    // this by adding an inner action. The inner action cannot fail, but within
    // the inner action is the drive to action, which _can_ fail. If that action
    // fails, it will _not_ call the Wait action, otherwise it will. This way,
    // we have a lambda that only gets called when the drive to succeeds
    ...
    // within innerAction, we do want to consider failures of driving (to
    // prevent the lambda from running if the drive fails)
    ...
    auto waitLambda = [this](Robot& robot) {
      if (_shouldSetCubeLights) {
        ...
        robot.GetCubeLightComponent().PlayLightAnimByTrigger(
            _objectID, CubeAnimationTrigger::DrivingTo);
      }

      // if this lambda gets called, that means the drive to must have
      // succeeded.
      if (!_dockAction.expired()) {
        LOG_INFO("IDriveToInteractWithObject.DriveToSuccess",
                 "DriveTo action succeeded, telling dock action not to check "
                 "predock pose distance");

        // For debug builds do a dynamic cast for validity checks
        DEV_ASSERT(
            dynamic_cast<IDockAction*>(_dockAction.lock().get()) != nullptr,
            "IDriveToInteractWithObjectAction.Constructor.DynamicCastFailed");

        IDockAction* rawDockAction =
            static_cast<IDockAction*>(_dockAction.lock().get());
        rawDockAction->SetDoNearPredockPoseCheck(false);
      } else {
        PRINT_NAMED_ERROR(
            "IDriveToInteractWithObject.InnerAction.WaitLambda.NoDockAction",
    ...
    // Add the entire inner action, but ignore failures here so that we will
    // always run the dock action even if driving fails (so that the dock action
    // will be the one to fail)
  ...
  if (maxTurnTowardsFaceAngle_rad > 0.f) {
    _turnTowardsLastFacePoseAction = AddAction(
        new TurnTowardsLastFacePoseAction(maxTurnTowardsFaceAngle_rad, sayName),
        true);

    _turnTowardsObjectAction = AddAction(
        new TurnTowardsObjectAction(_objectID, maxTurnTowardsFaceAngle_rad),
        true);
  }
}
...
IDriveToInteractWithObject::IDriveToInteractWithObject(const ObjectID& objectID,
                                                       const f32 distance)
    : CompoundActionSequential(), _objectID(objectID) {
  _driveToObjectAction =
      AddAction(new DriveToObjectAction(_objectID, distance), true);
}
...
std::weak_ptr<IActionRunner> IDriveToInteractWithObject::AddDockAction(
    IDockAction* dockAction, bool ignoreFailure) {
  if (HasRobot()) {
    dockAction->SetRobot(&GetRobot());
  }

  // right before the dock action, we want to call the PreDock callback (if one
  // was specified). TO achieve this, we use a WaitForLambda action which always
  // completes immediately.
  // TODO:(bn) this could be done much more elegantly as part of
  // CompoundActionSequential
  {
...
void IDriveToInteractWithObject::SetSayNameAnimationTrigger(
    AnimationTrigger trigger) {
  if (HasStarted()) {
    PRINT_NAMED_ERROR(
        "IDriveToInteractWithObject.SetSayNameAnimationTrigger.AfterRunning",
        "Tried to update the animations after the action started, this isn't "
        "supported");
    return;
  }
  if (!_turnTowardsLastFacePoseAction.expired()) {
    static_cast<TurnTowardsLastFacePoseAction*>(
        _turnTowardsLastFacePoseAction.lock().get())
        ->SetSayNameAnimationTrigger(trigger);
  }
}

void IDriveToInteractWithObject::SetNoNameAnimationTrigger(
    AnimationTrigger trigger) {
  if (HasStarted()) {
    PRINT_NAMED_ERROR(
        "IDriveToInteractWithObject.SetNoNameAnimationTrigger.AfterRunning",
        "Tried to update the animations after the action started, this isn't "
        "supported");
    return;
  }
  if (!_turnTowardsLastFacePoseAction.expired()) {
    static_cast<TurnTowardsLastFacePoseAction*>(
        _turnTowardsLastFacePoseAction.lock().get())
        ->SetNoNameAnimationTrigger(trigger);
  }
}
...
void IDriveToInteractWithObject::SetMaxTurnTowardsFaceAngle(
    const Radians angle) {
  if (_turnTowardsObjectAction.expired() ||
      _turnTowardsLastFacePoseAction.expired()) {
    PRINT_NAMED_WARNING("IDriveToInteractWithObject.SetMaxTurnTowardsFaceAngle",
                        "Can not set angle of null actions (the action were "
                        "originally constructed with an angle of zero)");
    return;
  }
  LOG_DEBUG("IDriveToInteractWithObject.SetMaxTurnTowardsFaceAngle",
            "Setting maxTurnTowardsFaceAngle to %f degrees",
            angle.getDegrees());
  static_cast<TurnTowardsLastFacePoseAction*>(
      _turnTowardsLastFacePoseAction.lock().get())
      ->SetMaxTurnAngle(angle);
  static_cast<TurnTowardsObjectAction*>(_turnTowardsObjectAction.lock().get())
      ->SetMaxTurnAngle(angle);
}
...
void IDriveToInteractWithObject::SetPreActionPoseAngleTolerance(f32 angle_rad) {
  if (GetState() != ActionResult::NOT_STARTED) {
    PRINT_NAMED_WARNING(
        "IDriveToInteractWithObject.SetPreActionPoseAngleTolerance.Invalid",
        "Tried to set the preaction pose angle tolerance, but action has "
        "already started");
    return;
  }
  ...
  if (!_driveToObjectAction.expired()) {
    LOG_INFO("IDriveToInteractWithObject.SetPreActionPoseAngleTolerance",
             "[%d] %f rad", GetTag(), angle_rad);

    static_cast<DriveToObjectAction*>(_driveToObjectAction.lock().get())
        ->SetPreActionPoseAngleTolerance(angle_rad);
  } else {
    PRINT_NAMED_WARNING(
        "IDriveToInteractWithObject.SetApproachAngle.NullDriveToAction", "");
  }
}
...
void IDriveToInteractWithObject::SetApproachAngle(const f32 angle_rad) {
  if (GetState() != ActionResult::NOT_STARTED) {
    PRINT_NAMED_WARNING(
        "IDriveToInteractWithObject.SetApproachAngle.Invalid",
        "Tried to set the approach angle, but action has already started");
    return;
  }

  if (!_driveToObjectAction.expired()) {
    LOG_INFO("IDriveToInteractWithObject.SetApproachingAngle", "[%d] %f rad",
             GetTag(), angle_rad);

    static_cast<DriveToObjectAction*>(_driveToObjectAction.lock().get())
        ->SetApproachAngle(angle_rad);
  } else {
    PRINT_NAMED_WARNING(
        "IDriveToInteractWithObject.SetApproachAngle.NullDriveToAction", "");
  }
...
Result IDriveToInteractWithObject::UpdateDerived() {
  if (_shouldSetCubeLights && !_lightsSet) {
    LOG_INFO("IDriveToInteractWithObject.SetInteracting",
             "%s[%d] Setting interacting object to %d", GetName().c_str(),
             GetTag(), _objectID.GetValue());
    GetRobot().GetCubeLightComponent().PlayLightAnimByTrigger(
        _objectID, CubeAnimationTrigger::DrivingTo);
...
IDriveToInteractWithObject::~IDriveToInteractWithObject() {
  if (_lightsSet) {
    LOG_INFO("IDriveToInteractWithObject.UnsetInteracting",
             "%s[%d] Unsetting interacting object to %d", GetName().c_str(),
             GetTag(), _objectID.GetValue());
    if (HasRobot()) {
      GetRobot().GetCubeLightComponent().StopLightAnimAndResumePrevious(
...
DriveToAlignWithObjectAction::DriveToAlignWithObjectAction(
    const ObjectID& objectID, const f32 distanceFromMarker_mm,
    const bool useApproachAngle, const f32 approachAngle_rad,
    const AlignmentType alignmentType, Radians maxTurnTowardsFaceAngle_rad,
    const bool sayName)
    : IDriveToInteractWithObject(
          objectID,
          AlignWithObjectAction::GetPreActionTypeFromAlignmentType(
              alignmentType),
          0, useApproachAngle, approachAngle_rad, maxTurnTowardsFaceAngle_rad,
          sayName) {
  AlignWithObjectAction* action =
      new AlignWithObjectAction(objectID, distanceFromMarker_mm, alignmentType);
...
DriveToPickupObjectAction::DriveToPickupObjectAction(
    const ObjectID& objectID, const bool useApproachAngle,
    const f32 approachAngle_rad, Radians maxTurnTowardsFaceAngle_rad,
    const bool sayName, AnimationTrigger animBeforeDock)
    : IDriveToInteractWithObject(objectID, PreActionPose::DOCKING, 0,
                                 useApproachAngle, approachAngle_rad,
...
void DriveToPickupObjectAction::SetDockingMethod(DockingMethod dockingMethod) {
  if (!_pickupAction.expired()) {
    static_cast<IDockAction*>(_pickupAction.lock().get())
        ->SetDockingMethod(dockingMethod);
  } else {
    PRINT_NAMED_WARNING(
        "DriveToPickupObjectAction.SetDockingMethod.NullPickupAction", "");
  }
}

void DriveToPickupObjectAction::SetPostDockLiftMovingAudioEvent(
    AudioMetaData::GameEvent::GenericEvent event) {
  if (!_pickupAction.expired()) {
    static_cast<IDockAction*>(_pickupAction.lock().get())
        ->SetPostDockLiftMovingAudioEvent(event);
  } else {
    PRINT_NAMED_WARNING(
        "DriveToPickupObjectAction.SetPostDockLiftMovingAudioEvent."
        "NullPickupAction",
        "");
  }
}
...
DriveToPlaceOnObjectAction::DriveToPlaceOnObjectAction(
    const ObjectID& objectID, const bool useApproachAngle,
    const f32 approachAngle_rad, Radians maxTurnTowardsFaceAngle_rad,
    const bool sayName)
    : IDriveToInteractWithObject(objectID, PreActionPose::PLACE_RELATIVE, 0,
                                 useApproachAngle, approachAngle_rad,
                                 maxTurnTowardsFaceAngle_rad, sayName) {
  PlaceRelObjectAction* action =
      new PlaceRelObjectAction(objectID, false, 0, 0);
  AddDockAction(action);
  SetProxyTag(action->GetTag());
}
...
DriveToPlaceRelObjectAction::DriveToPlaceRelObjectAction(
    const ObjectID& objectID, const bool placingOnGround,
    const f32 placementOffsetX_mm, const f32 placementOffsetY_mm,
    const bool useApproachAngle, const f32 approachAngle_rad,
    Radians maxTurnTowardsFaceAngle_rad, const bool sayName,
    const bool relativeCurrentMarker)
    : IDriveToInteractWithObject(objectID, PreActionPose::PLACE_RELATIVE, 0,
                                 useApproachAngle, approachAngle_rad,
                                 maxTurnTowardsFaceAngle_rad, sayName) {
  PlaceRelObjectAction* action =
      new PlaceRelObjectAction(objectID, placingOnGround, placementOffsetX_mm,
                               placementOffsetY_mm, relativeCurrentMarker);
  AddDockAction(action);
  SetProxyTag(action->GetTag());
  ...
  if (!relativeCurrentMarker) {
    ...
    if (driveToAction != nullptr) {
      driveToAction->SetGetPossiblePosesFunc(
          [this, placementOffsetX_mm, placementOffsetY_mm](
              ActionableObject* object, std::vector<Pose3d>& possiblePoses,
              bool& alreadyInPosition) {
            return PlaceRelObjectAction::ComputePlaceRelObjectOffsetPoses(
                object, placementOffsetX_mm, placementOffsetY_mm,
                GetRobot().GetPose(), GetRobot().GetWorldOrigin(),
                GetRobot().GetCarryingComponent(), GetRobot().GetBlockWorld(),
                GetRobot().GetVisionComponent(), possiblePoses,
                alreadyInPosition);
          });
    } else {
      LOG_INFO(
...
DriveToRollObjectAction::DriveToRollObjectAction(
    const ObjectID& objectID, const bool useApproachAngle,
    const f32 approachAngle_rad, Radians maxTurnTowardsFaceAngle_rad,
    const bool sayName)
    : IDriveToInteractWithObject(objectID, PreActionPose::ROLLING, 0,
                                 useApproachAngle, approachAngle_rad,
                                 maxTurnTowardsFaceAngle_rad, sayName),
      _objectID(objectID) {
...
void DriveToRollObjectAction::RollToUpright(const BlockWorld& blockWorld,
                                            const Pose3d& robotPose) {
  if (GetState() != ActionResult::NOT_STARTED) {
    PRINT_NAMED_WARNING(
        "DriveToRollObjectAction.RollToUpright.AlreadyRunning",
  ...
  if (!_objectID.IsSet()) {
    PRINT_NAMED_WARNING("DriveToRollObjectAction.RollToUpright.NoObject",
                        "[%d] invalid object id", GetTag());
    return;
  }

  f32 approachAngle_rad;
  if (DriveToRollObjectAction::GetRollToUprightApproachAngle(
          blockWorld, robotPose, _objectID, approachAngle_rad)) {
    SetApproachAngle(approachAngle_rad);
  }
}

bool DriveToRollObjectAction::GetRollToUprightApproachAngle(
    const BlockWorld& blockWorld, const Pose3d& robotPose,
    const ObjectID& objID, f32& approachAngle_rad) {
  if (!objID.IsSet()) {
  ...
  const ObservableObject* observableObject =
      blockWorld.GetLocatedObjectByID(objID);
  if (nullptr == observableObject) {
    PRINT_NAMED_WARNING("DriveToRollObjectAction.RollToUpright.NullObject",
                        "invalid object id %d", objID.GetValue());
    return false;
  }

  if (!IsBlockType(observableObject->GetType(), false)) {
    LOG_INFO(
        "DriveToRollObjectAction.RollToUpright.WrongType",
        "Can only use this function on blocks or light cubes, ignoring call");
    return false;
  }

  // unfortunately this needs to be a dynamic cast because Block inherits from
  // observable object virtually
  const Block* block = dynamic_cast<const Block*>(observableObject);
  if (block == nullptr) {
    PRINT_NAMED_ERROR(
        "DriveToRollObjectAction.RollToUpright.NotABlock",
  ...
  block->GetCurrentPreActionPoses(preActionPoses, robotPose,
  ...
  // if we have any valid predock poses which approach the bottom face, use
  // those

  const Vision::KnownMarker& bottomMarker =
      block->GetMarker(Block::FaceName::BOTTOM_FACE);
  ...
  for (const auto& preActionPose : preActionPoses) {
    ...
    if (nullptr != marker && marker->GetCode() == bottomMarker.GetCode()) {
      // found at least one valid pre-action pose using the bottom marker, so
      // limit the approach angle so we will roll the block to upright Compute
      // approachVec in the frame of the preActionPose itself
      Pose3d blockPoseWrtPreactionPose;
      if (!block->GetPose().GetWithRespectTo(preActionPose.GetPose(),
                                             blockPoseWrtPreactionPose)) {
        LOG_WARNING(
            "DriveToRollObjectAction.RollToUpright.GetWithRespectToFailed",
  ...
  // if we got here, that means none of the predock poses (if there are any)
  // will roll from the bottom. In this case, don't limit the predock poses at
  // all. This will make it so we *might* get lucky and roll the cube into a
  // state where we can roll it again to upright it, although there is no
  // guarantee. A real solution would need a high-level planner to solve this.
  // By doing nothing here, we don't limit the approach angle at all
...
Result DriveToRollObjectAction::EnableDeepRoll(bool enable) {
  if (GetState() != ActionResult::NOT_STARTED) {
    PRINT_NAMED_WARNING(
        "DriveToRollObjectAction.EnableDeepRoll.Invalid",
        "[%d] Tried to set deep roll mode, but action has started", GetTag());
    return RESULT_FAIL;
  }

  DEV_ASSERT(!_rollAction.expired(), "DriveToRollObjectAction.actionIsNull");
  static_cast<RollObjectAction*>(_rollAction.lock().get())
      ->EnableDeepRoll(enable);
  return RESULT_OK;
}

#pragma mark---- DriveToPopAWheelieAction ----

DriveToPopAWheelieAction::DriveToPopAWheelieAction(
    const ObjectID& objectID, const bool useApproachAngle,
    const f32 approachAngle_rad, Radians maxTurnTowardsFaceAngle_rad,
    const bool sayName)
    : IDriveToInteractWithObject(objectID, PreActionPose::ROLLING, 0,
...
DriveToFacePlantAction::DriveToFacePlantAction(
    const ObjectID& objectID, const bool useApproachAngle,
    const f32 approachAngle_rad, Radians maxTurnTowardsFaceAngle_rad,
    const bool sayName)
    : IDriveToInteractWithObject(objectID, PreActionPose::DOCKING, 0,
...
DriveToRealignWithObjectAction::DriveToRealignWithObjectAction(
    ObjectID objectID, float dist_mm)
    : CompoundActionSequential(), _objectID(objectID), _dist_mm(dist_mm) {}
...
void DriveToRealignWithObjectAction::OnRobotSetInternalCompound() {
  ...
  if (nullptr == observableObject) {
    PRINT_NAMED_WARNING(
        "DriveToRealignWithObjectAction.Constructor.NullObservableObject",
    ...
    if (!(observableObject->IsPoseStateKnown()) ||
        (p.GetTranslation().y() < minTrans)) {
      MoveHeadToAngleAction* moveHeadToAngleAction =
          new MoveHeadToAngleAction(kIdealViewBlockHeadAngle);
  ...
  DriveToAlignWithObjectAction* driveToAlignWithObjectAction =
      new DriveToAlignWithObjectAction(_objectID, _dist_mm);
