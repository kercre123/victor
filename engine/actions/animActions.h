...
class PlayAnimationAction : public IAction {
 public:
  ...
  PlayAnimationAction(
      const std::string& animName, u32 numLoops = 1,
      bool interruptRunning = true,
      u8 tracksToLock = (u8)AnimTrackFlag::NO_TRACKS,
      float timeout_sec = _kDefaultTimeout_sec, TimeStamp_t startAtTime_ms = 0,
  ...
  virtual void SetRenderInEyeHue(bool renderInEyeHue) {
    _renderInEyeHue = renderInEyeHue;
  }

  static constexpr f32 GetDefaultTimeoutInSeconds() {
    return _kDefaultTimeout_sec;
  }
  static constexpr f32 GetInfiniteTimeoutInSeconds() {
    return _kDefaultTimeoutForInfiniteLoops_sec;
  }
 ...
 protected:
  ...
  // called at the end of Init, can (and should) be overridden to log additional
  // stats to DAS / webviz
  virtual void InitSendStats();

  // helper that can be called from InitSendStats to send stats with the
  // specified information. May use GetRobot() to add robot info. Note that this
  // will _only_ send to DAS if a trigger name is specified
  ...
  static constexpr float _kDefaultTimeoutForInfiniteLoops_sec =
...
class TriggerAnimationAction : public PlayAnimationAction {
 public:
  ...
  // Numloops 0 causes the action to loop forever
  explicit TriggerAnimationAction(
      AnimationTrigger animEvent, u32 numLoops = 1,
      bool interruptRunning = true,
      u8 tracksToLock = (u8)AnimTrackFlag::NO_TRACKS,
      float timeout_sec = _kDefaultTimeout_sec, bool strictCooldown = false);
...
// Directly call an animation group. Note that this should almost never be used
// from code, instead you should use the animation trigger so that it can be
// mapped properly to a group. However, this may be useful for dev tools or
// data-driven use cases where the animation group is provided in data
...
class TriggerLiftSafeAnimationAction : public TriggerAnimationAction {
 public:
  // Preferred constructor, used by the factory CreatePlayAnimationAction
  // Numloops 0 causes the action to loop forever
  explicit TriggerLiftSafeAnimationAction(
      AnimationTrigger animEvent, u32 numLoops = 1,
      bool interruptRunning = true,
      u8 tracksToLock = (u8)AnimTrackFlag::NO_TRACKS,
      float timeout_sec = _kDefaultTimeout_sec, bool strictCooldown = false);
...
// Repeatedly creates and plays TriggerLiftSafeAnimationAction numLoops times.
// This is different than using a TriggerLiftSafeAnimationAction with the param
// numLoops, since that will select one animation from the anim group at Init
// and loop it, whereas this reselects the animation each loop.
class ReselectingLoopAnimationAction : public IAction {
 public:
  ...
  // once called, the action will end as soon as the current loop finishes, and
  // Init() must be called to reset
  void StopAfterNextLoop();

  void SetRenderInEyeHue(bool renderInEyeHue) {
    _renderInEyeHue = renderInEyeHue;
  }
 ...
 protected:
  ...
  virtual f32 GetTimeoutInSeconds() const override {
    return _animParams.timeout_sec;
  }
...
// Loops the given animation while doing the 'primary' action in parallel. Once
// the primary action completes, we wait for either the current animation loop
// to finish or maxWaitTime_sec (whichever comes first).
class LoopAnimWhileAction : public CompoundActionParallel {
 public:
  ...
  // Once the primary action completes, this is the maximum amount of time we
  // will wait for the current animation loop to finish before bailing. A
