 ...
 * Date:   12/11/2015
 *
 * Description: Defines an action for tracking objects (from BlockWorld),
 *derived from ITrackAction
 *
 *
...
TrackObjectAction::TrackObjectAction(const ObjectID& objectID, bool trackByType)
    : ITrackAction("TrackObject", RobotActionType::TRACK_OBJECT),
      _objectID(objectID),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TrackObjectAction::GetRequiredVisionModes(
...
ActionResult TrackObjectAction::InitInternal() {
  ...
  if (nullptr == object) {
    PRINT_NAMED_ERROR("TrackObjectAction.Init.InvalidObject",
                      "Object %d does not exist in BlockWorld",
                      _objectID.GetValue());
    return ActionResult::BAD_OBJECT;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
ITrackAction::UpdateResult TrackObjectAction::UpdateTracking(
  ...
  if (_trackByType) {
    ...
    matchingObject =
        GetRobot().GetBlockWorld().FindLocatedClosestMatchingObject(
            _objectType, _lastTrackToPose, 1000.f, DEG_TO_RAD(180), filter);
    ...
    if (nullptr == matchingObject) {
      ...
      if (DEBUG_TRACKING_ACTIONS) {
        PRINT_CH_INFO(kLogChannelName,
                      "TrackObjectAction.UpdateTracking.NoMatchingTypeFound",
                      "Could not find matching %s object.",
                      EnumToString(_objectType));
      }
      return UpdateResult::NoNewInfo;
    } else {
      // We've possibly switched IDs that we're tracking. Keep
      // MovementComponent's ID in sync.
    ...
    if (nullptr == matchingObject) {
      PRINT_NAMED_WARNING(
          "TrackObjectAction.UpdateTracking.ObjectNoLongerExists",
          "Object %d no longer exists in BlockWorld", _objectID.GetValue());
      return UpdateResult::NoNewInfo;
    }
  }

  DEV_ASSERT(nullptr != matchingObject,
             "TrackObjectAction.UpdateTracking.NullMatchingObject");
  ...
  matchingObject->GetObservedMarkers(observedMarkers,
                                     matchingObject->GetLastObservedTime());

  if (observedMarkers.empty()) {
    PRINT_NAMED_ERROR(
        "TrackObjectAction.UpdateTracking.NoObservedMarkers",
  ...
  for (auto marker : observedMarkers) {
    ...
    if (currentDistSq < minDistSq) {
      ...
      // Keep track of best zDist too, so we don't have to redo the
      // GetWithRespectTo call outside this loop NOTE: This isn't perfectly
      // accurate since it doesn't take into account the the head angle and is
      // simply using the neck joint (which should also probably be queried from
      // the robot instead of using the constant here)
