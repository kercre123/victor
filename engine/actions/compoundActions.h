 ...
 *              is added to a compound action, the compound action completely
 *manages the action including deleting it (see IDriveToInteractWithObject for
 *examples)
 *
...
class ICompoundAction : public IActionRunner {
 public:
  ...
  // Adds an action to this compound action. Completely hands ownership and
  // memory management of the action over to this compoundAction Internally
  // creates a shared_ptr and will return a weak_ptr to it should the caller
  // want to do something with the action at a later time
  using ShouldIgnoreFailureFcn =
      std::function<bool(ActionResult, const IActionRunner*)>;
  virtual std::weak_ptr<IActionRunner> AddAction(
      IActionRunner* action, ShouldIgnoreFailureFcn fcn,
      bool emitCompletionSignal = false);
  virtual std::weak_ptr<IActionRunner> AddAction(
      IActionRunner* action, bool ignoreFailure = false,
      bool emitCompletionSignal = false);
  ...
  const std::list<std::shared_ptr<IActionRunner>>& GetActionList() const {
    return _actions;
  }
  ...
  // The proxy action, if set, is the one whose type and completion info are
  // used. Specify it by the constituent action's tag.
  void SetProxyTag(u32 tag);

  // Sets whether or not to delete actions from the compound action when they
  // complete By default actions will be destroyed on completion
 ...
 protected:
  ...
  bool ShouldIgnoreFailure(ActionResult result,
                           const std::shared_ptr<IActionRunner>& action) const;

  // Stack of pairs of actionCompletionUnions and actionTypes of the already
  // completed actions
  ...
  void StoreUnionAndDelete(
      std::list<std::shared_ptr<IActionRunner>>::iterator& currentAction);
...
class CompoundActionParallel : public ICompoundAction {
 public:
  ...
  // By default, CompoundActionParallel continues as long as its longest
  // sub-action. Setting this to false will end the CompoundActionParallel the
  // moment any of its sub-actions end
  void SetShouldEndWhenFirstActionCompletes(bool shouldEnd) {
    _endWhenFirstActionCompletes = shouldEnd;
  }
