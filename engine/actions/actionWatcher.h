 ...
 * Date:   11/10/2016
 *
 * Description: Monitors actions as they run to track what sub-actions get
 *created and what their results are. This is a passive system and does not
 *modify the robot or any of the actions
 *
...
class ActionWatcher {
 ...
 public:
  ...
  // Register a callback. This callback will be called when _any_ action ends
  // (including sub-actions or actions inside compound actions). It will be
  // called after the action completed message is broadcast, and after the
  // action itself has been fully deleted. Returns a unique integer id for the
  // callback (so it can later be removed). Eventually this will move to the
  // action container (see COZMO-11465 )
  ActionEndedCallbackID RegisterActionEndedCallbackForAllActions(
      ActionEndedCallback callback);

  // Remove a registered callback. Returns true if the callback was found, false
  // otherwise
  bool UnregisterCallback(ActionEndedCallbackID callbackID);

  // Called every tick of basestation after the action list has updated its
  // actions
  void Update();
  ...
  // Called when an action is destroyed, will delete the action from the
  // _actionTree map but its node will still exist as a child of its parent
  // action. If a parent action is being destroyed then the full action tree
  // will be destroyed so it is invalid to access the tree for this action
  void ActionEnding(const IActionRunner* action);

  // Returns all unique ActionResults of all actions that are children of action
  // with tag
  void GetSubActionResults(const ActionTag tag,
                           std::vector<ActionResult>& results);

  // Prints the action tree for this tag
  void Print(const ActionTag tag);

  // Deletes all nodes in this action's tree and removes all tag->node mappings
  // for each of the actions in the this action's tree from the _actionTrees map
 ...
 private:
  struct Node {
    ...
    Node(const ActionTag actionTag)
        : actionTag(actionTag), name(""), neverUpdated(false) {
      children.clear();
  ...
  // Maps a parent action's tag to a stack of subActions that are in the process
  // of updating
