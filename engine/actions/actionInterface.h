...
class IActionRunner {
 public:
  IActionRunner(const std::string& name, const RobotActionType type,
  ...
  // Returns true if the tag has been set, false if it is invalid or already
  // exists
  bool SetTag(u32 tag);
  // If a custom tag has been set return will be that otherwise it is the same
  // as the auto-generated tag
  u32 GetTag() const { return _customTag; }

  // returns true if the action tag is currently "in use". Tags are in use from
  // the moment the action is created (in the constructor), until the action is
  // deleted
  static bool IsTagInUse(u32 tag) {
    return sInUseTagSet.find(tag) != sInUseTagSet.end();
  }
  ...
  // If a subclass wants to update their name or type after construction they
  // can call these setters
  ...
  // with the action. I.e., by default actions will lockout all control of the
  // robot, and extra movement commands are ignored. Note: uses the bits defined
  // by AnimTrackFlag.
  void SetTracksToLock(const u8 tracks);
  const u8 GetTracksToLock() const { return _tracks; }

  // If this method returns true, then it means the derived class is
  // interruptible, can safely be re-queued using "NOW_AND_RESUME", and will
  // pick back up safely after the newly-queued action completes. Otherwise,
  // this action will just be cancelled when NOW_AND_RESUME is used. Note that
  // this relies on subclasses implementing InterruptInternal() and Reset().
  bool Interrupt();

  // Override this to take care of anything that needs to be done on
  // Retry/Interrupt.
  ...
  // By default, the completion of any action could cause a mood event (the
  // robot's mood manager defines this). If this is set to false, this action
  // won't trigger any mood events
  ...
  virtual void GetCompletionUnion(ActionCompletedUnion& completionUnion) const {
    completionUnion = _completionUnion;
  }

  void GetRobotCompletedActionMessage(
  ...
  // Called when the action stops running and sets variables needed for
  // completion. This calls the overload-able GetCompletionUnion() method above.
  ...
  // Forces the actions state to SUCCESS so in the next update call the action
  // will immediately complete Use caution when calling this because it could
  // result in an incomplete completionUnion
  void ForceComplete();

  static ActionResultCategory GetActionResultCategory(const ActionResult& res) {
    return static_cast<ActionResultCategory>(static_cast<u32>(res) >>
                                             ARCBitShift::NUM_BITS);
  }

  // This should only be used from the PathComponent. If set, this action will
  // clear the custom profile when it finishes. This allows actions to be
  // created with a custom motion profile (e.g. from Unity or SDK)
 ...
 protected:
  ...
  // Override to handle setting of a motion profile. Returns true if the profile
  // was used correctly (or if it was irrelevant, e.g. for an animation action).
  // Returns false if the action is unable to use the profile, e.g. because it
  // is already using manually set speeds. Note that this action only needs to
  // worry about itself, any other actions created by this action (either as
  // direct sub-actions or added to a compound action), will have this function
  // automatically called when appropriate
  virtual bool SetMotionProfile(const PathMotionProfile& motionProfile) {
    return true;
  }
...
class IAction : public IActionRunner, public IVisionModeSubscriber {
 public:
  IAction(const std::string& name, const RobotActionType type,
          const u8 tracksToLock);
 ...
 protected:
  // This UpdateInternal() is what gets called by IActionRunner's Update().  It
  // in turn handles timing delays and runs (protected) Init() and CheckIfDone()
  ...
  // If the derived action has specific vision mode requirements, they should be
  // noted by implementing this function, subscriptions to the
  // VisionScheduleMediator will then be handled by IAction::UpdateInternal().
  ...
  // Normally, actions unsubscribe from vision modes automatically when they
  // destruct. Call this from a derived class to unsubscribe early.
  void UnsubscribeFromVisionModes();

  // If the derived action needs to fail if the robot's tread state transitions
  // from OnTreads to any other state at runtime, then the action returns an
  // action result of INVALID_OFF_TREADS_STATE. The check to verify this is
  // handled by IAction::UpdateInternal, which calls
  // IAction::IsCurrentTreadStateValid. By default, we assume that the actions
  // can run even if the tread state transitions from OnTreads to any other
  // state.
  ...
  }  // Optional: default is no preconditions to meet
  ...
  // Before first CheckIfDone() call, after preconditions are met. Optional:
  // default is no delay
  ...
  // Verifies that the robot's tread state has NOT recently transitioned from
  // OnTreads to any other state.
