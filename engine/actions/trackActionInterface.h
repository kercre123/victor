 ...
 * Date:   12/11/2015
 *
 * Description: Vision system component for benchmarking operations. Add new
 *methods as needed to compare performance of various ways of implementing
 *operations on images (e.g. using OpenCV, directly looping over the pixels,
 *using a lookup table, etc. Each method is tied to a "Mode" which can be
 *enabled/disabled via console vars as well.
 *
 *
...
class ITrackAction : public IAction {
 public:
  enum class Mode { HeadAndBody, HeadOnly, BodyOnly };
  ...
  // Stop this action after maintaining the target within tolerances for the
  // given amount of time. If interruptDrivingAnim=true (and driving animations
  // are enabled), then if stop criteria are met, SUCCESS is returned
  // immediately and the end driving animation is not played. This presumes the
  // caller wants to more quickly play their own final animation. Set time to 0
  // to disable (default).
  void SetStopCriteria(const Radians& panTol, const Radians& tiltTol,
  ...
  // track. Set to 0 to disable timeout (default). If there is no StopCriteria,
  // a timeout is a "successful" completion of this action. If StopCriteria are
  // provided, a timeout will result in this action completing with a timeout
  // failure.
  void SetUpdateTimeout(float timeout_sec);

  // Tells this action to keep running until another action (being run
  // separately) stops. As soon as this other action completes, this action will
  // complete as well
  void StopTrackingWhenOtherActionCompleted(u32 otherActionTag);

  // Instead of setting pan _speed_, set the desired duration of the pan to turn
  // towards the target and compute the speed internally. So small turns will
  // move more slowly and large turns will be quicker. If this duration is set
  // (non-zero), it will take precedence over any pan speeds specified above.
  void SetPanDuration(f32 panDuration_sec);
  void SetTiltDuration(f32 tiltDuration_sec);

  // Set desired time to reach target if forward motion is supported by the
  // derived class The shorter this is, the faster the robot will drive to try
  // to reach the target distance returned by UpdateTracking().
  ...
  // minimum angle required to play sound. Use AnimationTrigger::Count for sound
  // to disable (default).
  ...
  // If enabled, angles returned by GetAngles() below tolerance (which would
  // generally be ignored) are clamped to the tolerances so that the robot
  // _always_ moves by at least the tolerance amounts (in the correct
  // direction). This creates extra (technically, unncessary) movement of the
  // robot, but keeps him looking more alive while tracking. Disabled by
  // default.
  void SetClampSmallAnglesToTolerances(bool tf);

  // If we are clamping small angles, this setting can be set to minimize how
  // "often" we do the clamp. If max is > 0, a random number will be rolled
  // between min and max and the "clamping" will next happen after that interval
  // has elapsed
 ...
 protected:
  ...
  // Anything which inherits from track action needs to have appropriate
  // VisionModes enabled.
  virtual void GetRequiredVisionModes(
      std::set<VisionModeRequest>& requests) const override = 0;

  // Note that derived classes should override InitInternal, which is called by
  // Init
  virtual ActionResult Init() override final;
  virtual ActionResult InitInternal() = 0;

  // Derived classes must implement InitInternal(), but cannot implement Init()
  // or CheckIfDone().
  virtual ActionResult CheckIfDone() override final;

  enum UpdateResult : u8 { NoNewInfo, NewInfo, PredictedInfo, ShouldStop };

  // These are the methods that children classes should call to use the
  // stop/continue criteria of their choice. However, as it is implemented right
  // now you can have some unintended consquences. As an example if
  // SetStopCriteria is called followed by UseContinueCriteria, stop criteria
  // will not be used and instead continue criteria will be. VIC-5821 further
  // describes these potential issues.
  void UseContinueCriteria(bool useContinueCriteria) {
    _useStopCriteria = !useContinueCriteria;
  }
  ...
  virtual UpdateResult UpdateTracking(Radians& absPanAngle,
  ...
  // see TrackFaceAction.
  virtual bool AreContinueCriteriaMet(const f32 currentTime_sec) {
    return false;
  };
 ...
 private:
  // sets internal values to track clamping small angles. Returns true if we
  // should clamp, false otherwise
  ...
  // When driving animations are used, we have to wait until the End animation
  // is complete before returning whatever actual final result for the action we
  // wanted. In the mean time we have to return RUNNING. So we use this member
  // variable to store the return value we want to use once driving animations
  // finish (if _shouldPlayDrivingAnimation=true).
  ...
  // Helper for storing the return result if we are using driving animations and
  // just returning result immediately if not
