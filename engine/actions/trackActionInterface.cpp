 ...
 * Date:   12/11/2015
 *
 * Description: Defines an interface tracking, derived from the general IAction
 *interface.
 *
 *
...
CONSOLE_VAR_RANGED(f32, kOverride_PanDuration_s, CONSOLE_GROUP, -1.0f, 0.0f,
                   1.0f);
CONSOLE_VAR_RANGED(f32, kOverride_TiltDuration_s, CONSOLE_GROUP, -1.0f, 0.0f,
                   1.0f);

CONSOLE_VAR(bool, kOverride_ClampSmallAngles, CONSOLE_GROUP, false);
CONSOLE_VAR_RANGED(f32, kOverride_ClampSmallAnglesMinPeriod_s, CONSOLE_GROUP,
                   -1.0f, 0.0f, 5.0f);
CONSOLE_VAR_RANGED(f32, kOverride_ClampSmallAnglesMaxPeriod_s, CONSOLE_GROUP,
                   -1.0f, 0.0f, 5.0f);

CONSOLE_VAR_RANGED(f32, kOverride_PanTolerance_deg, CONSOLE_GROUP, -1.0f, 0.0f,
                   20.0f);
CONSOLE_VAR_RANGED(f32, kOverride_TiltTolerance_deg, CONSOLE_GROUP, -1.0f, 0.0f,
                   20.0f);
...
ITrackAction::~ITrackAction() {
  if (HasRobot()) {
    ...
    GetRobot().GetAnimationComponent().RemoveKeepFaceAliveFocus(
        _kKeepFaceAliveITrackActionName);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
// TODO:(bn) if we implemented a parallel compound action function like "Stop on
// first action complete" instead of the current behavior of "stop when all
// actions are complete", I don't think we'd need this anymore
  ...
  if (HasStarted()) {
    if (otherActionTag != ActionConstants::INVALID_TAG &&
      ...
      PRINT_NAMED_WARNING("ITrackAction.SetOtherAction.InvalidOtherActionTag",
                          "[%d] trying to set tag %d, but it is not in use. "
                          "Keeping tag as old value of %d",
                          GetTag(), otherActionTag, _stopOnOtherActionTag);
    } else {
      // This means we are changing the tag while we are running, which is a bit
      // weird but should work as long as the action is valid (or INVALID_TAG)

      if (otherActionTag == ActionConstants::INVALID_TAG) {
        PRINT_CH_INFO(kLogChannelName,
                      "ITrackAction.StopTrackingOnOtherAction.Clear",
                      "[%d] Was waiting on action %d to stop, now will hang",
                      GetTag(), _stopOnOtherActionTag);
      } else {
        PRINT_CH_INFO(kLogChannelName,
                      "ITrackAction.StopTrackingOnOtherAction.SetWhileRunning",
                      "[%d] Will stop this action when %d completes", GetTag(),
                      otherActionTag);
      }
    ...
    // this action will be checked in Init to see if it is in use (it is done
    // there so it can cause the action to fail), so don't do anything with it
    // now
...
void ITrackAction::SetTiltDuration(f32 tiltDuration_sec) {
  DEV_ASSERT(!HasStarted(),
             "ITrackAction.SetTiltDuration.ActionAlreadyStarted");
...
void ITrackAction::SetUpdateTimeout(float timeout_sec) {
  DEV_ASSERT(!HasStarted(),
             "ITrackAction.SetUpdateTimeout.ActionAlreadyStarted");
...
void ITrackAction::SetDesiredTimeToReachTarget(f32 time_sec) {
  DEV_ASSERT(!HasStarted(),
             "ITrackAction.SetDesiredTimeToReachTarget.ActionAlreadyStarted");
...
void ITrackAction::EnableDrivingAnimation(bool enable) {
  DEV_ASSERT(!HasStarted(),
             "ITrackAction.EnableDrivingAnimation.ActionAlreadyStarted");
...
void ITrackAction::SetMinPanAngleForSound(const Radians& angle) {
  DEV_ASSERT(!HasStarted(),
             "ITrackAction.SetMinPanAngleForSound.ActionAlreadyStarted");
...
void ITrackAction::SetMinTiltAngleForSound(const Radians& angle) {
  DEV_ASSERT(!HasStarted(),
             "ITrackAction.SetMinTiltAngleForSound.ActionAlreadyStarted");
...
void ITrackAction::SetClampSmallAnglesToTolerances(bool tf) {
  DEV_ASSERT(
      !HasStarted(),
      "ITrackAction.SetClampSmallAnglesToTolerances.ActionAlreadyStarted");
...
void ITrackAction::SetClampSmallAnglesPeriod(float min_sec, float max_sec) {
  DEV_ASSERT(!HasStarted(),
             "ITrackAction.SetClampSmallAnglesPeriod.ActionAlreadyStarted");
...
void ITrackAction::SetMaxHeadAngle(const Radians& maxHeadAngle_rads) {
  DEV_ASSERT(!HasStarted(),
             "ITrackAction.SetMaxHeadAngle.ActionAlreadyStarted");
...
void ITrackAction::SetSoundSpacing(f32 spacingMin_sec, f32 spacingMax_sec) {
  DEV_ASSERT(!HasStarted(),
             "ITrackAction.SetSoundSpacing.ActionAlreadyStarted");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void ITrackAction::SetStopCriteria(const Radians& panTol,
                                   const Radians& tiltTol, f32 minDist_mm,
                                   f32 maxDist_mm, f32 time_sec,
                                   bool interruptDrivingAnim) {
  DEV_ASSERT(!HasStarted(),
             "ITrackAction.SetStopCriteria.ActionAlreadyStarted");
...
ActionResult ITrackAction::Init() {
  ...
  if (HaveStopCriteria() && _stopCriteria.interruptDrivingAnim &&
      !_shouldPlayDrivingAnimation) {
    PRINT_NAMED_WARNING("ITrackAction.Init.NoDrivingAnimToInterrupt",
                        "Stop criteria set with interruptDrivingAnim=true, but "
                        "driving animation not enabled");
  }

  // Reduce eye darts so we better appear to be tracking and not look around
  // NOTE: When action destructs, this parameter will be changed back to
  // default.
  //       So if the default value is not what it used to be, and we care, we
  //       would need some way of getting the current parameter value from
  //       animation process but for now it seems unnecessary since nobody else
  //       changes this parameter.
  GetRobot().GetAnimationComponent().AddKeepFaceAliveFocus(
      _kKeepFaceAliveITrackActionName);
  ...
  if (_stopOnOtherActionTag != ActionConstants::INVALID_TAG &&
    ...
    PRINT_NAMED_WARNING("ITrackAction.Init.InvalidOtherActionTag",
                        "[%d] Waiting on tag %d to stop this action, but that "
                        "tag is no longer in use. Stopping now",
                        GetTag(), _stopOnOtherActionTag);
...
ActionResult ITrackAction::CheckIfDoneReturnHelper(ActionResult result,
  ...
  if (_shouldPlayDrivingAnimation) {
    // Special case: stop criteria were met and it was requested to interrupt
    // driving animations in that case. Immediately return result and don't play
    // out the driving end animation.
    ...
    _finalActionResult =
        result;  // This will get returned once the end anim completes
...
ActionResult ITrackAction::CheckIfDone() {
  if (_shouldPlayDrivingAnimation) {
    if (GetRobot().GetDrivingAnimationHandler().IsPlayingDrivingEndAnim()) {
      return ActionResult::RUNNING;
                   ...
                   .GetDrivingAnimationHandler()
                   .HasFinishedDrivingEndAnim()) {
      DEV_ASSERT(_finalActionResult != ActionResult::NOT_STARTED,
                 "ITrackAction.CheckIfDone.FinalActionResultNotSet");
  ...
  if (_stopOnOtherActionTag != ActionConstants::INVALID_TAG &&
    ...
    PRINT_CH_INFO(kLogChannelName, "ITrackAction.FinishedByOtherAction",
                  "[%d] action %s stopping because we were told to stop when "
                  "another action stops (and it did)",
                  GetTag(), GetName().c_str());
  ...
  // if console vars are set, update the tolerance. In release, this should
  ...
  const UpdateResult updateResult =
      UpdateTracking(absPanAngle, absTiltAngle, distance_mm);
  switch (updateResult) {
    ...
    case UpdateResult::PredictedInfo: {
      ...
      if (DEBUG_TRACKING_ACTIONS) {
        PRINT_CH_INFO(
            kLogChannelName, "ITrackAction.CheckIfDone.NewInfo",
            "[%d] Commanding %sabs angles: pan=%.1fdeg, tilt=%.1fdeg, "
            "dist=%1.fmm",
            GetTag(),
      ...
      if (shouldClampSmallAngles &&
          FLT_LE(std::abs(relTiltAngle), _tiltTolerance.ToFloat())) {
        relTiltAngle = std::copysign(_tiltTolerance.ToFloat(), relTiltAngle);
        absTiltAngle = GetRobot().GetComponent<FullRobotPose>().GetHeadAngle() +
                       relTiltAngle;
      }

      if ((Mode::HeadAndBody == _mode || Mode::HeadOnly == _mode) &&
          FLT_GE(std::abs(relTiltAngle), _tiltTolerance.ToFloat())) {
        const float tiltDuraction_s = kOverride_TiltDuration_s > 0.0f
        ...
        if (RESULT_OK != GetRobot().GetMoveComponent().MoveHeadToAngle(
                             absTiltAngle.ToFloat(), speed, accel)) {
          return CheckIfDoneReturnHelper(
              ActionResult::SEND_MESSAGE_TO_ROBOT_FAILED, false);
        }
        ...
        if (_moveEyes) {
          ...
          eyeShiftY =
              y_mm *
              (static_cast<f32>(GetRobot().GetDisplayHeightInPixels() / 2) /
               SCREEN_SIZE[1]);
        }
      }
      ...
      const bool isPanWithinTol =
          Util::IsFltLE(std::abs(relPanAngle), _panTolerance.ToFloat());
      // If enabled, always move at least the tolerance amount
      if (shouldClampSmallAngles && isPanWithinTol) {
        relPanAngle = std::copysign(_panTolerance.ToFloat(), relPanAngle);
        absPanAngle =
            GetRobot().GetPose().GetRotation().GetAngleAroundZaxis().ToFloat() +
            relPanAngle;
      }

      // If distance is non-zero and the body is allowed to move based on mode,
      // then we need to move forward (fwd) or backward (bwd)
      const bool needToMoveFwdBwd =
          (_mode != Mode::HeadOnly) && !Util::IsNearZero(distance_mm);

      // If the relative pan angle is greater than the tolerance, we need to pan
      const bool needToPan =
          Util::IsFltGE(std::abs(relPanAngle), _panTolerance.ToFloat());

      if ((Mode::HeadAndBody == _mode || Mode::BodyOnly == _mode) &&
          (needToMoveFwdBwd || needToPan)) {
        // If the robot is not on its treads, it may exhibit erratic turning
        // behavior, but in some cases this is expected (e.g. driving on the
        // palm of a user's hand) In those cases, the caller will have to
        // specify that the action is allowed to run tread states other than
        // OnTreads (the only state allowed by default).
        const auto& otState = GetRobot().GetOffTreadsState();
        if (_validTreadStates.find(otState) == _validTreadStates.end()) {
          PRINT_NAMED_WARNING(
              "ITrackAction.CheckIfDone.OffTreadsStateInvalid",
              "[%d] Off tread state %s is invalid for turning in place",
              GetTag(), EnumToString(otState));
          return CheckIfDoneReturnHelper(ActionResult::INVALID_OFF_TREADS_STATE,
                                         false);
        }
        ...
        if (needToMoveFwdBwd) {
          s16 radius =
              std::numeric_limits<s16>::max();  // default: drive straight
          if (!isPanWithinTol) {
            // Set wheel speeds to drive an arc to the salient point. Note: use
            // the *relative* angle for this!
            const f32 denomAngle =
                std::min(std::abs(relPanAngle), DEG_TO_RAD(kMaxPanAngle_deg));
            const f32 d = distance_mm / std::cosf(denomAngle);
            const f32 d2 = d * d;
            const f32 radiusDenom =
                2.f * std::sqrtf(d2 - distance_mm * distance_mm);
            radius = std::round(std::copysign((d2 / radiusDenom), relPanAngle));
          }

          // Specify a fixed duration to reach the goal and compute speed from
          // it
          const f32 wheelspeed_mmps = std::min(
              MAX_SAFE_WHEEL_SPEED_MMPS, distance_mm / _timeToReachTarget_sec);
          const f32 accel = MAX_WHEEL_ACCEL_MMPS2;  // Expose?

          if (DEBUG_TRACKING_ACTIONS) {
            PRINT_CH_INFO(kLogChannelName,
                          "ITrackAction.CheckIfDone.DriveWheelsCurvature",
                          "[%d] d=%f r=%hd relPan=%.1fdeg speed=%f accel=%f",
                          GetTag(), distance_mm, radius,
                          RAD_TO_DEG(relPanAngle), wheelspeed_mmps, accel);
          }

          Result result =
              GetRobot().SendRobotMessage<RobotInterface::DriveWheelsCurvature>(
                  wheelspeed_mmps, accel, radius);
          ...
          const Radians& turnAngle =
              rotatedPose.GetRotation().GetAngleAroundZaxis();

          const float panDuration_s = kOverride_PanDuration_s > 0.0f
                                          ? kOverride_PanDuration_s
                                          : _panDuration_sec;

          // Just turn in place
          const f32 rotSpeed_radPerSec =
              std::min(MAX_BODY_ROTATION_SPEED_RAD_PER_SEC,
                       std::abs(relPanAngle) / panDuration_s);
          const f32 accel = MAX_BODY_ROTATION_ACCEL_RAD_PER_SEC2;

          if (DEBUG_TRACKING_ACTIONS) {
            PRINT_CH_INFO(kLogChannelName,
                          "ITrackAction.CheckIfDone.SetBodyAngle",
                          "[%d] d=%f relPan=%.1fdeg speed=%f accel=%f",
                          GetTag(), distance_mm, RAD_TO_DEG(relPanAngle),
                          rotSpeed_radPerSec, accel);
          }

          if (RESULT_OK != GetRobot().GetMoveComponent().TurnInPlace(
                               turnAngle.ToFloat(),  // angle_rad
        ...
        PRINT_CH_INFO(kLogChannelName, "ITrackAction.CheckIfDone.NoMotion",
                      "[%d] %sneed to pan (relPanAngle=%f, tol=%f). %sneed to "
                      "move fwd/bwd",
                      GetTag(), needToPan ? "" : "don't", relPanAngle,
                      _panTolerance.ToFloat(), needToMoveFwdBwd ? "" : "don't");
      }

      if (_moveEyes) {
        // Compute horizontal eye movement
        // Note: assuming screen is about the same x distance from the neck
        // joint as the head cam
        const f32 x_mm = std::tan(relPanAngle) * HEAD_CAM_POSITION[0];
        eyeShiftX =
            x_mm * (static_cast<f32>(GetRobot().GetDisplayWidthInPixels() / 2) /
                    SCREEN_SIZE[0]);
      }

      // Play sound if it's time and either angle was big enough
      const bool haveTurningSoundAnim =
          (AnimationTrigger::Count != _turningSoundAnimTrigger);
      if (haveTurningSoundAnim && (currentTime > _nextSoundTime) &&
          angleLargeEnoughForSound) {
        // Queue sound to only play if nothing else is playing
        PlayAnimationAction* soundAction = new TriggerLiftSafeAnimationAction(
            _turningSoundAnimTrigger, 1, false);
        _soundAnimTag = soundAction->GetTag();
        GetRobot().GetActionList().QueueAction(QueueActionPosition::IN_PARALLEL,
                                               soundAction);

        _nextSoundTime =
            currentTime + Util::numeric_cast<float>(GetRNG().RandDblInRange(
                              _soundSpacingMin_sec, _soundSpacingMax_sec));
      }
      ...
      if (_moveEyes && (eyeShiftX != 0.f || eyeShiftY != 0.f)) {
        ...
        if (DEBUG_TRACKING_ACTIONS) {
          PRINT_CH_INFO(kLogChannelName, "ITrackAction.CheckIfDone.EyeShift",
                        "[%d] Adjusting eye shift to (%.1f,%.1f)", GetTag(),
                        eyeShiftX, eyeShiftY);
        }
        ...
        const f32 kXMax =
            static_cast<f32>(GetRobot().GetDisplayWidthInPixels() / 4);
        const f32 kYMax =
            static_cast<f32>(GetRobot().GetDisplayHeightInPixels() / 4);
        GetRobot().GetAnimationComponent().AddOrUpdateEyeShift(
            _kEyeShiftLayerName, eyeShiftX, eyeShiftY, BS_TIME_STEP_MS, kXMax,
            kYMax, kMaxLookUpScale, kMinLookDownScale, kOuterEyeScaleIncrease);
      }  // if(_moveEyes)

      // Can't meet stop criteria based on predicted updates (as opposed to
      // actual observations)
      if (updateResult != UpdateResult::PredictedInfo) {
        const bool shouldStop =
            IsTimeToStop(relPanAngle, relTiltAngle, distance_mm, currentTime);
        if (shouldStop) {
    ...
    case UpdateResult::ShouldStop: {
      // Stop immediately. Yes, the destructor will also do this, but if we have
      // driving animations, we may return RUNNING for a bit while those finish
      // and we want to make sure to stop _now_ if the UpdateTracking function
      // said we should.
      switch (_mode) {
        case Mode::HeadAndBody:
          GetRobot().GetMoveComponent().StopHead();
      ...
      // NOTE: Intentional fall-through to NoNewInfo case below so we check for
      // timeout
    }
      ...
      if (_updateTimeout_sec > 0.0f && _lastUpdateTime > 0.0f) {
        if (currentTime - _lastUpdateTime > _updateTimeout_sec) {
          PRINT_CH_INFO(kLogChannelName, "ITrackAction.CheckIfDone.Timeout",
                        "No tracking angle update received in %f seconds, "
                        "returning done.",
                        _updateTimeout_sec);
          // Remove eye shift
          GetRobot().GetAnimationComponent().RemoveEyeShift(_kEyeShiftLayerName,
          ...
          PRINT_CH_INFO(kLogChannelName, "ITrackAction.CheckIfDone.NotTimedOut",
                        ...
                        GetTag(), currentTime, _lastUpdateTime,
                        _updateTimeout_sec);
        }
      } else {
...
bool ITrackAction::UpdateSmallAngleClamping() {
  ...
  const bool clampSmallAngles =
      kOverride_ClampSmallAngles ? clampOverride : _clampSmallAngles;

  if (clampSmallAngles) {
    const float clampSmallAnglesMaxPeriod_s =
        clampOverride ? kOverride_ClampSmallAnglesMaxPeriod_s
                      : _clampSmallAnglesMaxPeriod_s;

    const float clampSmallAnglesMinPeriod_s =
        clampOverride ? kOverride_ClampSmallAnglesMinPeriod_s
    ...
    if (hasClampPeriod) {
      const float currTime_s =
          BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
      const bool shouldClamp = _nextTimeToClampSmallAngles_s < 0.0f ||
                               (currTime_s >= _nextTimeToClampSmallAngles_s);
      if (shouldClamp) {
        // re-roll the next period
        const float randPeriod_s = GetRNG().RandDblInRange(
            clampSmallAnglesMinPeriod_s, clampSmallAnglesMaxPeriod_s);
...
bool ITrackAction::HaveStopCriteria() const {
  const bool atLeastOneTolerance =
      (!Util::IsFltNear(_stopCriteria.panTol.ToFloat(), -1.f) ||
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool ITrackAction::IsTimeToStop(const f32 relPanAngle_rad,
                                const f32 relTiltAngle_rad,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool ITrackAction::IsWithinTolerances(const f32 relPanAngle_rad,
                                      const f32 relTiltAngle_rad,
                                      const f32 distance_mm,
                                      const f32 currentTime_sec) const {
  bool isWithinPanTol = true;
  if (!Util::IsFltNear(_stopCriteria.panTol.ToFloat(), -1.f)) {
    isWithinPanTol = Util::IsFltLE(std::abs(relPanAngle_rad),
                                   _stopCriteria.panTol.ToFloat());
  }
  bool isWithinTiltTol = true;
  if (!Util::IsFltNear(_stopCriteria.tiltTol.ToFloat(), -1.f)) {
    isWithinTiltTol = Util::IsFltLE(std::abs(relTiltAngle_rad),
                                    _stopCriteria.tiltTol.ToFloat());
  }
  bool isWithinDistTol = true;
  if (!Util::IsFltNear(_stopCriteria.minDist_mm, -1.f) &&
      !Util::IsFltNear(_stopCriteria.maxDist_mm, -1.f)) {
    isWithinDistTol = Util::InRange(distance_mm, _stopCriteria.minDist_mm,
                                    _stopCriteria.maxDist_mm);
  }

  if (DEBUG_TRACKING_ACTIONS) {
    PRINT_CH_INFO(
        kLogChannelName, "ITrackAction.CheckIfDone.CheckingStopCriteria",
        "[%d] Pan:%.1fdeg vs %.1f (%c), Tilt:%.1fdeg vs %.1f (%c), Dist:%.1fmm "
        "vs (%.1f,%.1f) (%c)",
        GetTag(), std::abs(RAD_TO_DEG(relPanAngle_rad)),
        _stopCriteria.panTol.getDegrees(), isWithinPanTol ? 'Y' : 'N',
        std::abs(RAD_TO_DEG(relTiltAngle_rad)),
        _stopCriteria.tiltTol.getDegrees(), isWithinTiltTol ? 'Y' : 'N',
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool ITrackAction::AreStopCriteriaMet(const f32 relPanAngle_rad,
                                      const f32 relTiltAngle_rad,
                                      const f32 distance_mm,
                                      const f32 currentTime_sec) {
  const bool haveStopCriteria = HaveStopCriteria();
  if (haveStopCriteria) {
    const bool isWithinTol = IsWithinTolerances(
        relPanAngle_rad, relTiltAngle_rad, distance_mm, currentTime_sec);
    if (isWithinTol) {
      const bool wasWithinTol = (_stopCriteria.withinTolSince_sec >= 0.f);
      if (wasWithinTol) {
        // Been within tolerance for long enough to stop yet?
        if (currentTime_sec - _stopCriteria.withinTolSince_sec >
            _stopCriteria.duration_sec) {
          if (DEBUG_TRACKING_ACTIONS) {
            PRINT_CH_INFO(
                kLogChannelName, "ITrackAction.AreStopCriteriaMet.MetCriteria",
                "Within tolerances for > %.1fsec (panTol=%.1fdeg "
                "tiltTol=%.1fdeg distTol=[%.1f,%.1f]",
                _stopCriteria.duration_sec, _stopCriteria.panTol.getDegrees(),
                _stopCriteria.tiltTol.getDegrees(), _stopCriteria.minDist_mm,
                _stopCriteria.maxDist_mm);
          }
          return true;
        }
        ...
        if (DEBUG_TRACKING_ACTIONS) {
          PRINT_CH_INFO(
              kLogChannelName,
              "ITrackAction.AreStopCriteriaMet.FailedToMeetCriteria",
              "[%d] Setting start of stop criteria being met to t=%.1fsec",
              GetTag(), currentTime_sec);
        }
