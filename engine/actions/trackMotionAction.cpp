 ...
 * Date:   12/11/2015
 *
 * Description: Defines an action for tracking motion (on the ground), derived
 *from ITrackAction.
 *
 *
...
void TrackMotionAction::GetRequiredVisionModes(
...
ActionResult TrackMotionAction::InitInternal() {
  ...
  _signalHandle = GetRobot().GetExternalInterface()->Subscribe(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
ITrackAction::UpdateResult TrackMotionAction::UpdateTracking(
  ...
  if (_gotNewMotionObservation && _motionObservation.img_area > 0) {
    ...
    const Point2f motionCentroid(_motionObservation.img_x,
                                 _motionObservation.img_y);

    // Note: we start with relative angles here, but make them absolute below.
    GetRobot().GetVisionComponent().GetCamera().ComputePanAndTiltAngles(
        motionCentroid, absPanAngle, absTiltAngle);

    // Find pose of robot at time motion was observed
    HistRobotState* histStatePtr = nullptr;
    RobotTimeStamp_t junkTime;
    if (RESULT_OK !=
        GetRobot().GetStateHistory()->ComputeAndInsertStateAt(
            _motionObservation.timestamp, junkTime, &histStatePtr)) {
      PRINT_NAMED_ERROR("TrackMotionAction.UpdateTracking.PoseHistoryError",
                        "Could not get historical pose for motion observed at "
                        "t=%d (lastRobotMsgTime = %d)",
                        _motionObservation.timestamp,
                        (TimeStamp_t)GetRobot().GetLastMsgTimestamp());
      return UpdateResult::NoNewInfo;
    }

    DEV_ASSERT(nullptr != histStatePtr,
               "TrackMotionAction.UpdateTracking.NullHistStatePtr");
    ...
    if (DEBUG_TRACKING_ACTIONS) {
      PRINT_CH_INFO(kLogChannelName, "TrackMotionAction.UpdateTracking.Motion",
                    ...
                    _motionObservation.img_area * 100.f, motionCentroid.x(),
                    motionCentroid.y());
    }
