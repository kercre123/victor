...
class ActionQueue {
 public:
  ...
  // Queue action to run right after the current action, before anything else in
  // the queue
  ...
  // Cancel the current action and immediately run the new action, preserving
  // rest of queue
  ...
  // current action in the queue to run fresh next (after this newly-inserted
  // action)
  ...
  const IActionRunner* GetCurrentRunningAction() const {
    return _currentAction;
  }

  // Deletes the action only if it isn't in the process of being deleted
  // Safeguards against the action being deleted multiple times due to handling
  // action completion signals Returns true if this call actually deleted the
  // action, false if the action is already in the process of being deleted
 ...
 private:
  // Deletes the action only if it isn't in the process of being deleted
  // If iter is not the end of the queue, the iter will be removed from the
  // queue (iter should always point to the action)
  bool DeleteActionIter(std::list<IActionRunner*>::iterator& iter);

  bool DeleteActionAndIter(IActionRunner*& action,
                           std::list<IActionRunner*>::iterator& iter);

  // Reference to robot so that actions queues receive a robot to inject into
  // actions
  Robot& _robot;
  ...
  // A set of tags that are in the process of being deleted. This is used to
  // protect actions from being deleted multiple times/recursively
...
class ActionList : public IDependencyManagedComponent<RobotComponentID> {
 public:
  ...
  Result QueueAction(QueueActionPosition inPosition, IActionRunner* action,
                     u8 numRetries = 0);

  bool IsEmpty() const;

  const ActionQueue& GetQueue(SlotHandle atSlot) const {
    return _queues.at(atSlot);
  }
  ...
  // Find and cancel the action with the specified ID Tag. All slots are
  // searched. Returns true if the action was found and cancelled.
  bool Cancel(u32 idTag);

  void Print() const;

  // Returns true if actionName is the name of one of the actions that are
  // currently being executed.
  bool IsCurrAction(const std::string& actionName) const;

  // Returns true if the passed in action tag matches the action currently
  // playing in the given slot
  bool IsCurrAction(u32 idTag, SlotHandle fromSlot = 0) const;

  // If we are currently clearing the action will be destroyed and will return
  // true Otherwise will return if the action is a duplicate
  ...
  // NOTE: Currently these callback functions (below) are wrappers for the
  // ActionWatcher functions, but eventually they may be implemented here (see
  // COZMO-11465), so prefer using these
  ...
  // callback (so it can later be removed).
...
inline size_t ActionList::GetQueueLength(SlotHandle atSlot) const {
  ...
  if (iter != _queues.end()) {
    length = iter->second.Length() +
             (nullptr == iter->second.GetCurrentRunningAction() ? 0 : 1);
  }
  return length;
}

inline size_t ActionList::GetNumQueues() const { return _queues.size(); }
