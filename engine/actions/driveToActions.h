 ...
 * Date:   8/29/2014
 *
 * Description: Implements drive-to cozmo-specific actions, derived from the
 *IAction interface.
 *
 *
...
class DriveToPoseAction : public IAction {
 public:
  ...
  // Call this to indicate that the goal options were generated from an object's
  // pose (predock poses). The object's pose should be given as the argument.
  void SetObjectPoseGoalsGeneratedFrom(
      const Pose3d& objectPoseGoalsGeneratedFrom);

  // If true and if multiple goals were provided, only the originally-selected
  // goal will be used
  void SetMustContinueToOriginalGoal(bool mustUse) {
    _mustUseOriginalGoal = mustUse;
  }

  // If shouldPlay, the robot will play planning animations while it computes a
  // plan or replans, for any planner that doesn't return a path immediately. If
  // !shouldPlay, the robot will plan and start driving in one fell swoop,
  // without any logic for planning animations. Default is true.
...
class DriveToObjectAction : public IAction {
 public:
  ...
  DriveToObjectAction(const ObjectID& objectID, const f32 distance_mm);
  virtual ~DriveToObjectAction();

  // TODO: Add version where marker code is specified instead of action?
  // DriveToObjectAction(Robot& robot, const ObjectID& objectID,
  // Vision::Marker::Code code);

  // If set, instead of driving to the nearest preActionPose, only the
  // preActionPose that is most closely aligned with the approach angle is
  // considered.
  ...
  const bool GetClosestPreDockPose(ActionableObject* object,
                                   Pose3d& closestPose) const;
  ...
  void DoPositionCheckOnPathCompletion(bool doCheck) {
    _doPositionCheckOnPathCompletion = doCheck;
  }

  // Set the angle tolerance to use for the pre action pose checks done by this
  // action. Defaults to using the default value specified in cozmo config
  void SetPreActionPoseAngleTolerance(f32 angle_rad) {
    _preActionPoseAngleTolerance_rad = angle_rad;
  }

  using GetPossiblePosesFunc = std::function<ActionResult(
      ActionableObject* object, std::vector<Pose3d>& possiblePoses,
      bool& alreadyInPosition)>;

  void SetGetPossiblePosesFunc(GetPossiblePosesFunc func) {
    if (IsRunning()) {
      PRINT_NAMED_ERROR(
          "DriveToActions.SetGetPossiblePosesFunc.TriedToSetWhileRunning",
          "PossiblePosesFunc is not allowed to change while the driveToAction is running. \
                             ActionName: %s ActionTag:%i",
  ...
  virtual void GetCompletionUnion(
      ActionCompletedUnion& completionUnion) const override;
...
class DriveToPlaceCarriedObjectAction : public DriveToObjectAction {
 public:
  // destinationObjectPadding_mm: padding around the object size at destination
  // used if checkDestinationFree is true
  DriveToPlaceCarriedObjectAction(
      const Pose3d& placementPose, const bool placeOnGround,
 ...
 protected:
  virtual ActionResult Init() override;
  virtual ActionResult CheckIfDone()
      override;  // Simplified version from DriveToObjectAction

  // checks if the placement destination is free (alternatively we could provide
  ...
  bool _checkDestinationFree;  // if true the action will often check that the
                               // destination is still free to place the object
  float _destinationObjectPadding_mm;  // padding around the object size at
                                       // destination if _checkDestinationFree
                                       // is true
...
// If maxTurnTowardsFaceAngle > 0, robot will turn a maximum of that angle
// towards last face after driving to the object (and say name if that is
// specified).
class IDriveToInteractWithObject : public CompoundActionSequential {
 protected:
  ...
  IDriveToInteractWithObject(const ObjectID& objectID, const f32 distance);

 public:
  virtual ~IDriveToInteractWithObject();

  // Forces both of the turnTowards subActions to force complete (basically not
  // run)
  ...
  DriveToObjectAction* GetDriveToObjectAction() {
    ...
    DEV_ASSERT(dynamic_cast<DriveToObjectAction*>(
                   _driveToObjectAction.lock().get()) != nullptr,
  ...
  // Subclasses that are a drive-to action followed by a dock action should be
  // calling this function instead of the base classes AddAction() in order to
  // set the approriate preDock pose offset for the dock action
  std::weak_ptr<IActionRunner> AddDockAction(IDockAction* dockAction,
                                             bool ignoreFailure = false);

  // Sets the animation trigger to use to say the name. Only valid if sayName
  // was true
  void SetSayNameAnimationTrigger(AnimationTrigger trigger);

  // Sets the backup animation to play if the name is not known, but there is a
  // confirmed face. Only valid if sayName is true (this is because we are
  // trying to use an animation to say the name, but if we don't have a name, we
  // want to use this animation instead)
  void SetNoNameAnimationTrigger(AnimationTrigger trigger);

  // Pass in a callback which will get called when the robot switches from
  // driving to it's predock pose to the actual docking action
 ...
 private:
  // Keep weak_ptrs to each of the actions inside this compound action so they
  // can be easily modified Unfortunately the weak_ptrs need to be cast to the
  // appropriate types to use them. Static casts are used for this. These are
  // safe in this case as they are only ever casting to the original type of the
  // action
...
// Compound action for driving to an object, visually verifying it can still be
// seen, and then driving to it until it is at the specified distance (i.e.
// distanceFromMarker_mm) from the marker.
// @param distanceFromMarker_mm - The distance from the marker along it's normal
// axis that the robot should stop at.
// @param useApproachAngle  - If true, then only the preAction pose that results
// in a robot
//                            approach angle closest to approachAngle_rad is
//                            considered.
// @param approachAngle_rad - The desired docking approach angle of the robot in
// world coordinates.
class DriveToAlignWithObjectAction : public IDriveToInteractWithObject {
 public:
  DriveToAlignWithObjectAction(
      const ObjectID& objectID, const f32 distanceFromMarker_mm,
      const bool useApproachAngle = false, const f32 approachAngle_rad = 0,
      const AlignmentType alignmentType = AlignmentType::CUSTOM,
      Radians maxTurnTowardsFaceAngle_rad = 0.f, const bool sayName = false);
...
class DriveToPickupObjectAction : public IDriveToInteractWithObject {
 public:
  DriveToPickupObjectAction(
      const ObjectID& objectID, const bool useApproachAngle = false,
      const f32 approachAngle_rad = 0,
      Radians maxTurnTowardsFaceAngle_rad = 0.f, const bool sayName = false,
  ...
  void SetPostDockLiftMovingAudioEvent(
      AudioMetaData::GameEvent::GenericEvent event);
...
// @param placementOffsetX_mm - The desired distance between the center of the
// docking marker
//                              and the center of the object that is being
//                              placed, along the direction of the docking
//                              marker's normal.
// @param useApproachAngle  - If true, then only the preAction pose that results
// in a robot
//                            approach angle closest to approachAngle_rad is
...
class DriveToPlaceRelObjectAction : public IDriveToInteractWithObject {
 public:
  ...
  // chooses preAction pose closest to approachAngle_rad if useApproachAngle ==
  // true.
  DriveToPlaceRelObjectAction(
      const ObjectID& objectID, const bool placingOnGround = true,
      const f32 placementOffsetX_mm = 0, const f32 placementOffsetY_mm = 0,
      const bool useApproachAngle = false, const f32 approachAngle_rad = 0,
      Radians maxTurnTowardsFaceAngle_rad = 0.f, const bool sayName = false,
...
class DriveToRollObjectAction : public IDriveToInteractWithObject {
 public:
  ...
  // Sets the approach angle so that, if possible, the roll action will roll the
  // block to land upright. If the block is upside down or already upright, and
  // roll action will be allowed
...
// Common compound action for driving to an object and popping a wheelie off of
// it
// @param useApproachAngle  - If true, then only the preAction pose that results
// in a robot
//                            approach angle closest to approachAngle_rad is
...
// Common compound action for driving to an object (stack) and face planting off
// of it by knocking the stack over
// @param useApproachAngle  - If true, then only the preAction pose that results
// in a robot
//                            approach angle closest to approachAngle_rad is
//                            considered.
