 ...
 * Date:   12/11/2015
 *
 * Description: Defines an action for tracking pet faces, derived from
 *ITrackAction.
 *
 *
...
TrackPetFaceAction::TrackPetFaceAction(Vision::PetType petType)
    : ITrackAction("TrackPetFace", RobotActionType::TRACK_PET_FACE),
      _petType(petType) {
  switch (_petType) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TrackPetFaceAction::GetRequiredVisionModes(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TrackPetFaceAction::GetCompletionUnion(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
ITrackAction::UpdateResult TrackPetFaceAction::UpdateTracking(
  ...
  if (_faceID != Vision::UnknownFaceID) {
    ...
    if (nullptr == petFace) {
      ...
      PRINT_CH_INFO(kLogChannelName,
                    "TrackPetFaceAction.UpdateTracking.BadFaceID",
    ...
    if (!petIDs.empty()) {
      ...
      PRINT_CH_INFO(kLogChannelName,
                    "TrackPetFaceAction.UpdateTracking.NoPetsWithType",
                    "Type=%s", EnumToString(_petType));
      return UpdateResult::NoNewInfo;
    }
  }
  ...
  Result result = GetRobot().ComputeTurnTowardsImagePointAngles(
      petFace->GetRect().GetMidPoint(), petFace->GetTimeStamp(), absPanAngle,
      absTiltAngle);
  if (RESULT_OK != result) {
    PRINT_NAMED_WARNING(
        "TrackpetFaceAction.UpdateTracking."
        "ComputeTurnTowardsImagePointAnglesFailed",
        "t=%u", petFace->GetTimeStamp());
    return UpdateResult::NoNewInfo;
  }
