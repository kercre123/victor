...
ActionEndedCallbackID ActionWatcher::RegisterActionEndedCallbackForAllActions(
...
void ActionWatcher::ParentActionUpdating(const IActionRunner* action) {
  DEV_ASSERT(action != nullptr,
             "ActionWatcher.ParentActionUpdating.NullAction");
...
// All of the updating action stacks should be empty since no actions besides
// the parent action are currently updating
#if ANKI_DEV_ASSERT_ENABLED
  for (const auto& i : _parentToUpdatingActions) {
    DEV_ASSERT(
        i.second.empty(),
        "ActionWatcher.ParentActionUpdating.ParentToUpdatingActionsNotEmpty");
  }
#endif
}
...
void ActionWatcher::ActionStartUpdating(const IActionRunner* action) {
  ...
  if (root == _actionTrees.end()) {
    ...
    if (_lastActionTag != ActionConstants::INVALID_TAG) {
      ...
      DEV_ASSERT(iter != _actionTrees.end(),
                 "ActionWatcher.ActionStartUpdating.LastActionNotInTree");
...
void ActionWatcher::ActionEndUpdating(const IActionRunner* action) {
  ...
  // Update _currentActionTag and _lastActionTag to be the top of the stack and
  // the second to top of the stack, respectively
...
void ActionWatcher::ActionEnding(const IActionRunner* action) {
  ...
  // If the action that is ending is a parent action then print information
  // about it and delete its action tree
  if (iter != _parentToUpdatingActions.end()) {
    ...
    // If the action is not a parent action and the action itself does not have
    // a parent then delete it from the tree
...
void ActionWatcher::GetSubActionResults(const ActionTag tag,
  ...
  // Helper function to recursively add each child's completion result to the
  // set of results
...
void ActionWatcher::DeleteActionTree(const ActionTag tag) {
  ...
  // Helper function to recursively delete nodes and actionTags from
  // _actionTrees
...
void ActionWatcher::Print(const ActionTag tag) {
  ...
  ss << "Parent: " << _actionTrees[tag]->name << "["
     << _actionTrees[tag]->actionTag << "]"
     << " created ";
...
void ActionWatcher::PrintHelper(const Node* node, int level, int child,
                                int& numLeaves) {
  std::stringstream ss;
  ss << (node->neverUpdated ? "**" : "") << node->name << "[" << node->actionTag
     << "]"
     << " created ";
