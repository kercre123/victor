...
SayTextAction::SayTextAction(const std::string& text,
    ...
    : IAction("SayText", RobotActionType::SAY_TEXT,
...
void SayTextAction::OnRobotSet() {
  LOG_INFO("SayTextAction.RobotSet", "Text '%s' Style '%s' DurScalar %f",
           Util::HidePersonallyIdentifiableInfo(_text.c_str()),
           EnumToString(_style), _durationScalar);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void SayTextAction::SetAnimationTrigger(AnimationTrigger trigger,
                                        u8 ignoreTracks) {
...
ActionResult SayTextAction::Init() {
  ...
  //
  const auto triggerType = ((_animTrigger == AnimationTrigger::Count)
                                ? UtteranceTriggerType::Manual
                                : UtteranceTriggerType::KeyFrame);

  _ttsCoordinator = &GetRobot().GetTextToSpeechCoordinator();
  _callbackPtr = std::make_shared<CallbackType>(
      std::bind(&SayTextAction::TtsCoordinatorStateCallback, this,
                std::placeholders::_1));
  std::weak_ptr<CallbackType> weakCallback = _callbackPtr;
  auto ttsCallback = [weakCallback](const UtteranceState& state) {
    // SayTextAction may have been aborted and destroyed before the callback
    // fires
    if (auto callback = weakCallback.lock()) {
      (*callback)(state);
    }
  };
  _ttsID = _ttsCoordinator->CreateUtterance(
      _text, triggerType, _style, _durationScalar, _pitchScalar, ttsCallback);

  _actionState = SayTextActionState::Waiting;

  // When does this action expire?
  _expiration_sec =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() + _timeout_sec;

  LOG_INFO("SayTextAction.Init", "ttsID %d text %s", _ttsID,
           Util::HidePersonallyIdentifiableInfo(_text.c_str()));
...
ActionResult SayTextAction::TransitionToRunning() {
  DEV_ASSERT(_ttsCoordinator != nullptr,
             "SayTextAction.TransitionToRunning.InvalidCoordinator");

  const bool ok = _ttsCoordinator->PlayUtterance(_ttsID);
  if (!ok) {
    LOG_ERROR("SayTextAction.TransitionToRunning.FailedToPlay",
              "Unable to play ttsID %d", _ttsID);
    _actionState = SayTextActionState::Invalid;
    return ActionResult::ABORT;
  }

  if (_animTrigger != AnimationTrigger::Count) {
    LOG_DEBUG("SayTextAction.TransitionToRunning",
              "ttsID %d now running with animation", _ttsID);
    _animAction = std::make_unique<TriggerAnimationAction>(
        _animTrigger, 1, true, _ignoreAnimTracks);
    _animAction->SetRobot(&GetRobot());
    _actionState = SayTextActionState::Running_Anim;
    return ActionResult::RUNNING;
  }

  LOG_DEBUG("SayTextAction.TransitionToRunning", "ttsID %d now running",
            _ttsID);
...
ActionResult SayTextAction::CheckIfDone() {
  ...
  switch (_actionState) {
    ...
    case SayTextActionState::Running_Anim: {
      // Tick animation while running, will return success when animation is
      // completed
...
void SayTextAction::TtsCoordinatorStateCallback(const UtteranceState& state) {
  LOG_DEBUG("SayTextAction.TtsCoordinatorStateCallback",
            "ttsID %d now state %d (%s)", _ttsID, state, EnumToString(state));
  _ttsState = state;
