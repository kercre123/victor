 ...
 *
 *              Note about subActions (manually ticking actions inside another
 *action) Store subActions as unique_ptrs since the subAction is unique to the
 *              parent and the parent is responsible for managing everything
 *about the subAction. (see PickupObjectAction for examples)
 *
...
// Ensure that nobody sets bad tag ranges (we want them all to be mutually
// exclusive
static_assert(ActionConstants::FIRST_GAME_TAG > ActionConstants::INVALID_TAG,
              "Game Tag Overlap");
static_assert(ActionConstants::FIRST_SDK_TAG > ActionConstants::LAST_GAME_TAG,
              "Sdk Tag Overlap");
static_assert(ActionConstants::FIRST_ENGINE_TAG > ActionConstants::LAST_SDK_TAG,
              "Engine Tag Overlap");
static_assert(ActionConstants::LAST_GAME_TAG > ActionConstants::FIRST_GAME_TAG,
              "Bad Game Tag Range");
static_assert(ActionConstants::LAST_SDK_TAG > ActionConstants::FIRST_SDK_TAG,
              "Bad Sdk Tag Range");
static_assert(ActionConstants::LAST_ENGINE_TAG >
                  ActionConstants::FIRST_ENGINE_TAG,
              "Bad Engine Tag Range");
...
u32 IActionRunner::NextIdTag() {
  // Post increment IActionRunner::sTagCounter (and loop within the ENGINE_TAG
  // range)
  ...
  assert((nextIdTag >= ActionConstants::FIRST_ENGINE_TAG) &&
         (nextIdTag <= ActionConstants::LAST_ENGINE_TAG));
...
IActionRunner::IActionRunner(const std::string& name,
                             const RobotActionType type, const u8 tracksToLock)
  ...
  while (!IActionRunner::sInUseTagSet.insert(_idTag).second) {
    PRINT_NAMED_ERROR("IActionRunner.TagCounterClash",
                      "TagCounters shouldn't overlap");
    _idTag = NextIdTag();
  }

  _customTag = _idTag;

  // This giant switch is necessary to set the appropriate completion union tags
  // in order to avoid emitting a completion union with an invalid tag There is
  // no default case in order to prevent people from forgetting to add it here
  switch (type) {
    ...
    case RobotActionType::TURN_TOWARDS_OBJECT: {
      _completionUnion.Set_objectInteractionCompleted(
...
IActionRunner::~IActionRunner() {
  ...
  if (!HasRobot()) {
    if (HasStarted()) {
      PRINT_NAMED_ERROR("IActionRunner.Destructor.RobotNotSet", "[%d]",
                        GetTag());
    } else {
      LOG_INFO(
          "IActionRunner.Destructor.RobotNotSetAndNotStarted",
          "[%d] robot not set, but action [%s] also not started so this is OK",
          GetTag(), RobotActionTypeToString(_type));
    }
    return;
  }
  ...
  if (!debugStr.empty()) {
    LOG_INFO("IActionRunner.Destroy.StopMovement",
             "Stopping movement on the following tracks since they were "
             "locked: %s[%s][%d]",
  ...
  if (!_suppressTrackLocking && _state != ActionResult::NOT_STARTED) {
    if (DEBUG_ANIM_TRACK_LOCKING) {
      LOG_INFO("IActionRunner.Destroy.UnlockTracks",
               "unlocked: (0x%x) %s by %s [%d]", _tracks,
  ...
  // We should not be locking _any_ tracks at this point. If we are, then just
  // unlock them and report this.
  const auto lockedTracks = mc.GetTracksLockedBy(lockStr);
  if (lockedTracks != 0) {
    LOG_ERROR("IActionRunner.Destroy.TracksStillLocked",
              "%s [%d]: Somehow we are still locking tracks 0x%02X. Unlocking "
              "them. Current state %s, _suppressTrackLocking %d",
              _name.c_str(), GetTag(), lockedTracks,
              ActionResultToString(_state), _suppressTrackLocking);
...
Robot& IActionRunner::GetRobot() {
  DEV_ASSERT_MSG(_robot != nullptr, "IActionRunner.GetRobot.RobotIsNull",
                 "Robot not set for action %s with tag %d", GetName().c_str(),
                 GetTag());
  return *_robot;
}
...
bool IActionRunner::SetTag(u32 tag) {
  // Probably a bad idea to be able to change the tag while the action is
  // running
  if (_state == ActionResult::RUNNING) {
    PRINT_NAMED_WARNING("IActionRunner.SetTag",
                        "Action %s [%d] is running unable to set tag to %d",
                        GetName().c_str(), GetTag(), tag);
    _state = ActionResult::BAD_TAG;
    return false;
  }

  // If the tag has already been set and the action is not running then erase
  // the current tag in order to set the new one
  ...
  if (tag == static_cast<u32>(ActionConstants::INVALID_TAG) ||
    ...
    PRINT_NAMED_ERROR("IActionRunner.SetTag.InvalidTag", "Tag [%d] is invalid",
                      tag);
    _state = ActionResult::BAD_TAG;
    return false;
  }
...
bool IActionRunner::Interrupt() {
  if (InterruptInternal()) {
    // Only need to unlock tracks if this is running because Update() locked
    // tracks
...
void IActionRunner::ShouldSuppressTrackLocking(bool tf) {
  if (_state != ActionResult::NOT_STARTED) {
    PRINT_NAMED_WARNING(
        "IActionRunner.ShouldSuppressTrackLocking.AlreadyStarted",
        "Action %s [%d] not suppressing track locking since we have already "
        "started (current state %s)",
        GetName().c_str(), GetTag(), ActionResultToString(_state));
    return;
  }
...
void IActionRunner::ForceComplete() {
  LOG_INFO("IActionRunner.ForceComplete",
           "Forcing %s[%d] in state %s to complete", GetName().c_str(),
           GetTag(), EnumToString(_state));
...
ActionResult IActionRunner::Update() {
  ...
  switch (_state) {
    ...
    case ActionResult::INTERRUPTED: {
      // Before we set the action to running, apply the motion profile to this
      // action if there is one. This will apply the profile automatically to
      // every action that runs, including nested or compound actions. It's up
      // to each individual action to implement the SetMotionProfile function to
      // update their motion profile appropriately
      if (GetRobot().GetPathComponent().HasCustomMotionProfile()) {
        const bool profileApplied = SetMotionProfile(
            GetRobot().GetPathComponent().GetCustomMotionProfile());
        if (!profileApplied) {
          LOG_INFO("IActionRunner.SetMotionProfile.Unused",
                   "Action %s [%d] unable to set motion profile. Perhaps "
                   "speeds already set manually?",
      ...
      if (!_suppressTrackLocking) {
        ...
        if (moveComponent.AreAnyTracksLocked(tracksToLock)) {
          ...
          PRINT_NAMED_WARNING(
              "IActionRunner.Update.TracksLocked",
              "Action %s [%d] not running because required tracks are locked",
              GetName().c_str(), GetTag());

          PRINT_NAMED_WARNING(
              "IActionRunner.Update.TracksLockedBecause",
        ...
        if (DEBUG_ANIM_TRACK_LOCKING) {
          LOG_INFO(
              "IActionRunner.Update.LockTracks", "locked: (0x%x) %s by %s [%d]",
              tracksToLock,
              AnimTrackHelpers::AnimTrackFlagsToString(tracksToLock).c_str(),
              GetName().c_str(), GetTag());
        }
      ...
      if (DEBUG_ACTION_RUNNING && _displayMessages) {
        LOG_DEBUG("IActionRunner.Update.IsRunning", "Action [%d] %s running",
    ...
    case ActionResult::RUNNING: {
      ...
      // UpdateInternal() returned something other than running so fall through
      // to handle action completion
    }
    // Every other case is a completion case (ie the action is no longer running
    // due to success, failure, or cancel)
    default: {
      if (_displayMessages) {
        LOG_INFO("IActionRunner.Update.ActionCompleted",
                 "%s [%d] %s with state %s.", GetName().c_str(), GetTag(),
                 (_state == ActionResult::SUCCESS
                      ? "succeeded"
      ...
      if (DEBUG_ACTION_RUNNING && _displayMessages) {
        LOG_DEBUG("IActionRunner.Update.IsRunning",
                  "Action [%d] %s NOT running", GetTag(), GetName().c_str());
      }
    }
  }
  actionWatcher.ActionEndUpdating(this);
...
void IActionRunner::PrepForCompletion() {
  if (!_preppedForCompletion) {
    ...
    LOG_DEBUG("IActionRunner.PrepForCompletion.AlreadyPrepped", "%s [%d]",
              _name.c_str(), GetTag());
  }
}
...
void IActionRunner::UnlockTracks() {
  // Tracks aren't locked until the action starts so don't unlock them until
  // then
  if (!_suppressTrackLocking && (_state != ActionResult::NOT_STARTED)) {
    u8 tracks = GetTracksToLock();
    if (DEBUG_ANIM_TRACK_LOCKING) {
      LOG_INFO("IActionRunner.UnlockTracks", "unlocked: (0x%x) %s by %s [%d]",
               tracks, AnimTrackHelpers::AnimTrackFlagsToString(tracks).c_str(),
               _name.c_str(), _idTag);
    }
    GetRobot().GetMoveComponent().UnlockTracks(tracks, GetTag());
  }
...
void IActionRunner::SetTracksToLock(const u8 tracks) {
  if (_state == ActionResult::NOT_STARTED) {
    ...
    PRINT_NAMED_WARNING("IActionRunner.SetTracksToLock",
                        "Trying to set tracks to lock while running");
  }
}
...
void IActionRunner::GetRobotCompletedActionMessage(
  ...
  GetRobot().GetActionList().GetActionWatcher().GetSubActionResults(
      GetTag(), subActionResults);

  ActionCompletedUnion acu;
  GetCompletionUnion(acu);

  msg = ExternalInterface::RobotCompletedAction(GetTag(), GetType(), GetState(),
                                                subActionResults, acu);
}

#pragma mark---- IAction ----

IAction::IAction(const std::string& name, const RobotActionType type,
                 const u8 tracksToLock)
    : IActionRunner(name, type, tracksToLock) {
  Reset();
}
...
IAction::~IAction() {
  ...
  if (HasRobot() && !_requiredVisionModes.empty()) {
    LOG_DEBUG("IAction.Destructor.UnSettingVisionModes",
              "Action %s [%d] Releasing VisionModes", GetName().c_str(),
              GetTag());
    GetRobot().GetVisionScheduleMediator().ReleaseAllVisionModeSubscriptions(
        this);
  }
}
...
void IAction::Reset(bool shouldUnlockTracks) {
  LOG_DEBUG("IAction.Reset", "Resetting action,%s unlocking tracks",
            (shouldUnlockTracks ? "" : " NOT"));
...
Util::RandomGenerator& IAction::GetRNG() const { return GetRobot().GetRNG(); }

bool IAction::DidTreadStateChangeFromOnTreads() const {
  const auto& currTreadState = GetRobot().GetOffTreadsState();
  return _prevTreadsState == OffTreadsState::OnTreads &&
         currTreadState != OffTreadsState::OnTreads;
}
...
ActionResult IAction::UpdateInternal() {
  ...
  // (Include CheckIfDoneDelay in wait time if we have already met
  // pre-conditions
  const f32 waitUntilTime =
      (_startTime_sec + GetStartDelayInSeconds() +
       (_actionSpecificPreconditionsMet ? GetCheckIfDoneDelayInSeconds()
                                        : 0.f));
  ...
  if (currentTimeInSeconds >= timeoutTime) {
    if (IsMessageDisplayEnabled()) {
      PRINT_NAMED_WARNING("IAction.Update.TimedOut",
                          "%s timed out after %.1f seconds.", GetName().c_str(),
                          GetTimeoutInSeconds());
    }
  ...
  // Don't do anything until we have reached the waitUntilTime
    ...
    if (!_actionSpecificPreconditionsMet) {
      // LOG_INFO("IAction.Update", "Updating %s: checking preconditions.",
      // GetName().c_str());
      ...
      // pre-conditions are not met: if they return RUNNING, then the action
      // will effectively wait for the preconditions to be met. Otherwise, a
      // failure will get propagated out as the return value of the Update
      // method.
      result = Init();
      ...
      if (result == ActionResult::SUCCESS) {
        ...
        if (!_requiredVisionModes.empty()) {
          LOG_DEBUG("IAction.Update.SettingVisionModes",
                    "Action %s [%d] Requesting VisionModes", GetName().c_str(),
                    GetTag());
          GetRobot().GetVisionScheduleMediator().SetVisionModeSubscriptions(
              this, _requiredVisionModes);
        }
      ...
      // When attempting to initialize, cache the current treads state for
      // comparison at runtime.
      _prevTreadsState = GetRobot().GetOffTreadsState();
    }

    // Re-check if ALL preconditions are met, since they could have _just_ been
    // met
    if (_actionSpecificPreconditionsMet &&
        currentTimeInSeconds >= waitUntilTime) {
      // LOG_INFO("IAction.Update", "Updating %s: checking if done.",
      // GetName().c_str());
      ...
      if (ShouldFailOnTransitionOffTreads() &&
          DidTreadStateChangeFromOnTreads()) {
        result = ActionResult::INVALID_OFF_TREADS_STATE;
      } else {
        // Pre-conditions already met, just run until done
        result = CheckIfDone();
      }
      // Cache the current treads state for comparison at the next time that the
      // action is updated.
      _prevTreadsState = GetRobot().GetOffTreadsState();
    }
  }  // if(currentTimeInSeconds > _waitUntilTime)

  const bool shouldRetry = (IActionRunner::GetActionResultCategory(result) ==
                            ActionResultCategory::RETRY);
  if (shouldRetry && RetriesRemain()) {
    if (IsMessageDisplayEnabled()) {
      LOG_INFO("IAction.Update.CurrentActionFailedRetrying",
               "Failed running action %s. Retrying.", GetName().c_str());
    }
