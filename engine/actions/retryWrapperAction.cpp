 ...
 * Date:   7/21/16
 *
 * Description: A wrapper action for handling retrying an action and playing
 *retry animations
 *
 *
...
RetryWrapperAction::RetryWrapperAction(IAction* action,
                                       ...
                                       u8 numRetries)
    : IAction("RetryWrapper", RobotActionType::UNKNOWN,
...
RetryWrapperAction::RetryWrapperAction(ICompoundAction* action,
  ...
  // Don't delete actions from the compound action on completion so that they
  // can be retried
...
RetryWrapperAction::RetryWrapperAction(IAction* action,
  ...
  _retryCallback =
      [retryTrigger](const ExternalInterface::RobotCompletedAction&,
                     const u8 retryCount, AnimationTrigger& retryAnimTrigger) {
...
ActionResult RetryWrapperAction::CheckIfDone() {
  ...
  if (_animationAction == nullptr) {
    ...
    // Update the retryWrapperAction's type to match the subAction's type in
    // case it is changing at runtime
    SetType(_subAction->GetType());

    // Only attempt to retry on failure results
    // TODO Could be updated to use ActionResultCategory
    if (res != ActionResult::RUNNING && res != ActionResult::SUCCESS &&
      ...
      GetRobot().GetActionList().GetActionWatcher().GetSubActionResults(
          _subAction->GetTag(), subActionResults);

      using RCA = ExternalInterface::RobotCompletedAction;
      RCA robotCompletedAction =
          RCA(_subAction->GetTag(), _subAction->GetType(),
              _subAction->GetState(), subActionResults, completionUnion);

      // Retry callback should NOT modify things that would be reset by reset
      // (ie action's state)
      LOG_DEBUG("RetryWrapperAction.CheckIfDone.CallingRetryCallback", "");
      AnimationTrigger animTrigger = AnimationTrigger::Count;
      bool shouldRetry =
          _retryCallback(robotCompletedAction, _retryCount, animTrigger);
      ...
      // If the animationTrigger to play is Count (indicates None/No animation)
      // check retry count and don't new the animation action
      if (animTrigger == AnimationTrigger::Count) {
        LOG_DEBUG("RetryWrapperAction.CheckIfDone.NoAnimation",
                  "RetryCallback returned AnimationTrigger::Count so not "
                  "playing animation");
        ...
        // Reset the subaction again, and unlock the tracks locked by the
        // subaction.
        _subAction->Reset(true);
  ...
  // Note: Doing an if here instead of an else so we can "fallthrough" to here
  // if we just newed the _animationAction
  if (_animationAction != nullptr) {
    ...
    if (res != ActionResult::RUNNING) {
      ...
      // Check retry count here so that if we end up reaching our max number of
      // retries this action ends when the animation does
      if (_retryCount++ >= _numRetries) {
...
void RetryWrapperAction::GetCompletionUnion(
