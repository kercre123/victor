 ...
 * Date:   8/10/2017
 *
 * Description: Implements charger-related actions, e.g. docking with the
 *charger.
 *
 *
...
MountChargerAction::MountChargerAction(ObjectID chargerID,
    ...
    : IAction("MountCharger", RobotActionType::MOUNT_CHARGER,
...
ActionResult MountChargerAction::Init() {
  ...
  const auto* charger =
      GetRobot().GetBlockWorld().GetLocatedObjectByID(_chargerID);
  if ((charger == nullptr) ||
  ...
  if (_mountAction != nullptr) {
    ...
    if (IActionRunner::GetActionResultCategory(result) ==
      ...
      const auto* charger =
          GetRobot().GetBlockWorld().GetLocatedObjectByID(_chargerID);
      if (charger != nullptr) {
        const auto& chargerAngle =
            charger->GetPose().GetRotation().GetAngleAroundZaxis();
        const auto& robotAngle =
            GetRobot().GetPose().GetRotation().GetAngleAroundZaxis();
        isFacingAwayFromCharger =
            (chargerAngle - robotAngle).getAbsoluteVal().ToFloat() > M_PI_2_F;
      }

      if (isFacingAwayFromCharger) {
        PRINT_NAMED_WARNING(
            "MountChargerAction.CheckIfDone.PositionForRetry",
            "Turning and mounting the charger failed (action result = %s). "
            "Driving forward to position for a retry.",
            EnumToString(result));
...
ActionResult MountChargerAction::ConfigureMountAction() {
  DEV_ASSERT(_mountAction == nullptr,
             "MountChargerAction.ConfigureMountAction.AlreadyConfigured");
  ...
  auto* backupAction =
      new BackupOntoChargerAction(_chargerID, _useCliffSensorCorrection);
  if (_dockingAnimTriggersSet) {
    backupAction->SetDockAnimations(_dockingStartTrigger, _dockingLoopTrigger,
                                    _dockingEndTrigger);
  }
...
ActionResult MountChargerAction::ConfigureDriveForRetryAction() {
  DEV_ASSERT(
      _driveForRetryAction == nullptr,
      "MountChargerAction.ConfigureDriveForRetryAction.AlreadyConfigured");
...
TurnToAlignWithChargerAction::TurnToAlignWithChargerAction(
    ObjectID chargerID, AnimationTrigger leftTurnAnimTrigger,
...
void TurnToAlignWithChargerAction::GetRequiredVisionModes(
...
ActionResult TurnToAlignWithChargerAction::Init() {
  ...
  //
  // The charger's origin is at the 'front' edge of the ramp (furthest from the
  // marker). This value is the distance from the origin into the charger of the
  // point that the robot should angle towards. Setting this distance to 0 means
  // the robot will angle itself toward the charger origin.
  ...
  // Get the vector from the target pose to the drive center pose, expressed in
  // the world origin frame
  Vec3f targetToRobotVec;
  if (!ComputeVectorBetween(GetRobot().GetDriveCenterPose(), poseToAngleToward,
                            GetRobot().GetWorldOrigin(), targetToRobotVec)) {
    PRINT_NAMED_WARNING(
        "TurnToAlignWithChargerAction.Init.CouldNotComputeVector",
        "Failed to compute vector from target pose to robot pose");
    return ActionResult::BAD_POSE;
  }
  const float angleToTurnTo =
      atan2f(targetToRobotVec.y(), targetToRobotVec.x());
  ...
  // Play the left/right turn animation depending on the anticipated direction
  // of the turn
  const auto& robotAngle =
      GetRobot().GetPose().GetRotation().GetAngleAroundZaxis();
  const bool clockwise = (angleToTurnTo - robotAngle).ToFloat() < 0.f;

  const auto animationTrigger =
      clockwise ? _rightTurnAnimTrigger : _leftTurnAnimTrigger;
...
ActionResult TurnToAlignWithChargerAction::CheckIfDone() {
  DEV_ASSERT(_compoundAction != nullptr,
             "TurnToAlignWithChargerAction.CheckIfDone.NullCompoundAction");
...
BackupOntoChargerAction::BackupOntoChargerAction(ObjectID chargerID,
    ...
    : IDockAction(chargerID, "BackupOntoCharger",
...
ActionResult BackupOntoChargerAction::SelectDockAction(
    ActionableObject* object) {
  auto objType = object->GetType();
  if (objType != ObjectType::Charger_Basic) {
    PRINT_NAMED_ERROR(
        "BackupOntoChargerAction.SelectDockAction.NotChargerObject",
...
ActionResult BackupOntoChargerAction::Verify() {
  ...
  const bool pitchSuggestsOnCharger =
      pitchAngleChange.IsNear(-kChargerSlopeAngle_rad, DEG_TO_RAD(2.f));
  const bool pitchSuggestsStillOnGround =
      pitchAngleChange.IsNear(0.f, DEG_TO_RAD(2.f));
  ...
  if (pitchSuggestsOnCharger) {
    ...
    PRINT_NAMED_WARNING("BackupOntoChargerAction.Verify.ChargerUnplugged",
                        "Pitch angle says we're on the charger platform, but "
                        "not sensing contacts. Charger may be unplugged."
  ...
  if (pitchSuggestsStillOnGround) {
    ...
    PRINT_NAMED_WARNING("BackupOntoChargerAction.Verify.StillOnGround",
                        "Pitch angles says we are still on the ground and not "
                        "on the charger platform. "
                        "(starting pitch %.2f deg, current pitch %.2f deg)",
                        _initialPitchAngle.getDegrees(),
                        currPitchAngle.getDegrees());
  ...
  PRINT_NAMED_WARNING(
      "BackupOntoChargerAction.Verify.Failed",
      ...
      "(starting pitch %.2f deg, current pitch %.2f deg)",
      _initialPitchAngle.getDegrees(), currPitchAngle.getDegrees());
  return ActionResult::NOT_ON_CHARGER_RETRY;
}

#pragma mark---- DriveToAndMountChargerAction ----

DriveToAndMountChargerAction::DriveToAndMountChargerAction(
    const ObjectID& objectID, const bool useCliffSensorCorrection,
    const bool enableDockingAnims, const bool doPositionCheckOnPathCompletion)
    : CompoundActionSequential() {
  // Get DriveToObjectAction
  auto driveToAction = new DriveToObjectAction(
      objectID, PreActionPose::ActionType::DOCKING, 0, false, 0);
  driveToAction->SetPreActionPoseAngleTolerance(DEG_TO_RAD(15.f));
  driveToAction->DoPositionCheckOnPathCompletion(
