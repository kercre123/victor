...
ICompoundAction::ICompoundAction(const std::list<IActionRunner*>& actions)
    : IActionRunner("ICompoundAction", RobotActionType::COMPOUND,
...
ICompoundAction::~ICompoundAction() { DeleteActions(); }
...
std::weak_ptr<IActionRunner> ICompoundAction::AddAction(
    IActionRunner* action, bool ignoreFailure, bool emitCompletionSignal) {
...
void ICompoundAction::DeleteActions() {
  for (auto iter = _actions.begin(); iter != _actions.end();) {
    ...
    DEV_ASSERT(iter->unique(),
               "ICompoundAction.DeleteActions.ActionPtrHasMultipleOwners");

    std::shared_ptr<IActionRunner> action = *iter;
    assert(action != nullptr);

    // Because we need to unlock tracks when we would have normally deleted the
    // action (which unlocks the tracks) we now need to relock the tracks so
    // that they can be unlocked normally by the action destructor Also, only
    // lock tracks if they aren't already locked as we will get only one unlock
    // from the action destructor
    if (HasRobot() && !_deleteActionOnCompletion) {
      if (action->GetState() != ActionResult::NOT_STARTED &&
          !action->IsSuppressingTrackLocking() &&

          !GetRobot().GetMoveComponent().AreAllTracksLockedBy(
              action->GetTracksToLock(), std::to_string((*iter)->GetTag()))) {
        GetRobot().GetMoveComponent().LockTracks(
            action->GetTracksToLock(), action->GetTag(), action->GetName());
      }
    }

    // TODO: issue a warning when a group is deleted without all its actions
    // completed?
...
void ICompoundAction::SetDeleteActionOnCompletion(bool deleteOnCompletion) {
  ...
  // Need to go through all of our subactions and update _deleteOnCompletion for
  // any compound actions
  for (auto& action : _actions) {
...
void ICompoundAction::StoreUnionAndDelete(
    std::list<std::shared_ptr<IActionRunner>>::iterator& currentAction) {
  // This will assert if someone is storing a shared_ptr to this action
  // (locked the weak_ptr returned from AddAction) and has not yet released it
  DEV_ASSERT(currentAction->unique(),
             "ICompoundAction.StoreUnionAndDelete.ActionPtrHasMultipleOwners");
  ...
  // If the proxy action's type changes while it is running then we need to
  // update our (CompoundAction) type to match (eg PickupObjectAction)
  if (_proxySet && (*currentAction)->GetTag() == _proxyTag) {
  ...
  if (_deleteActionOnCompletion) {
    ...
    // If we aren't deleting actions when they complete we need to unlock their
    // tracks so subsequent actions can run
    const auto& action = *currentAction;
    if (!action->IsSuppressingTrackLocking() &&
        action->GetState() != ActionResult::NOT_STARTED) {
      GetRobot().GetMoveComponent().UnlockTracks(action->GetTracksToLock(),
                                                 action->GetTag());
    }
    ++currentAction;
  }
...
bool ICompoundAction::ShouldIgnoreFailure(
    ActionResult result, const std::shared_ptr<IActionRunner>& action) const {
...
void ICompoundAction::GetCompletionUnion(
  ...
  if (_proxySet) {
    for (auto action : _actions) {
      if (action->GetTag() == _proxyTag) {
        LOG_DEBUG("ICompoundAction.GetCompletionUnion.UsingProxy",
                  ...
                  GetName().c_str(), GetTag(), action->GetName().c_str(),
                  action->GetTag());
    ...
    if (iter != _completedActionInfoStack.end()) {
      LOG_DEBUG("ICompoundAction.GetCompletionUnion.UsingProxy",
                ...
                GetName().c_str(), GetTag(), iter->first);
    ...
    PRINT_NAMED_WARNING("ICompoundAction.GetCompletionUnion.InvalidProxyTag",
                        "CompletionData with proxy tag=%d not found",
                        _proxyTag);
  }
...
CompoundActionSequential::CompoundActionSequential()
    : ICompoundAction({}), _delayBetweenActionsInSeconds(0) {
  Reset();
}

CompoundActionSequential::CompoundActionSequential(
    const std::list<IActionRunner*>& actions)
    : ICompoundAction(actions), _delayBetweenActionsInSeconds(0) {
  Reset();
}
...
ActionResult CompoundActionSequential::MoveToNextAction(
    float currentTime_secs) {
  ...
  if (_currentAction == _actions.end()) {
    ...
    LOG_INFO("CompoundActionSequential.Update.NextAction",
             "Moving to action %s [%d]", (*_currentAction)->GetName().c_str(),
             (*_currentAction)->GetTag());

    // If the compound action is suppressing track locking then the constituent
    // actions should too
    if ((*_currentAction)->IsSuppressingTrackLocking() !=
        IsSuppressingTrackLocking()) {
      (*_currentAction)
          ->ShouldSuppressTrackLocking(IsSuppressingTrackLocking());
    }
...
// no more actions, safe to return success for the compound action
    ...
    // this sub-action finished, but we still have others that we are waiting to
    // run, probably due to delay between actions, so return running
...
ActionResult CompoundActionSequential::UpdateInternal() {
  ...
  if (_currentAction != _actions.end()) {
    assert((*_currentAction) !=
           nullptr);  // should not have been allowed in by constructor
      ...
      switch (IActionRunner::GetActionResultCategory(subResult)) {
        ...
        case ActionResultCategory::CANCELLED: {
          ...
          if (ShouldIgnoreFailure(subResult, *_currentAction)) {
            // We are ignoring this action's failures, so just move to next
            // action
            ...
            LOG_DEBUG("CompoundActionSequential.UpdateInternal",
                      ...
                      (*_currentAction)->GetName().c_str(),
                      (*_currentAction)->GetTag(), EnumToString(subResult));
            StoreUnionAndDelete(_currentAction);
            return subResult;
          }
        }
...
CompoundActionParallel::CompoundActionParallel() : ICompoundAction({}) {}
...
ActionResult CompoundActionParallel::UpdateInternal() {
  // Return success unless we encounter anything still running or failed in loop
  // below. Note that we will return SUCCESS on the call following the one where
  // the last action actually finishes.
  ...
  if (RESULT_OK != derivedUpdateResult) {
    PRINT_CH_INFO("Actions",
                  "CompoundActionParallel.UpdateInternal.UpdateDerivedFailed",
                  "");
    return ActionResult::UPDATE_DERIVED_FAILED;
  }
  ...
  for (auto currentAction = _actions.begin();
       currentAction != _actions.end();) {
    ...
    // If the compound action is suppressing track locking then the constituent
    // actions should too
    if ((*currentAction)->IsSuppressingTrackLocking() !=
        IsSuppressingTrackLocking()) {
...
// Return failure, aborting updating remaining actions the group
        ...
        if (ShouldIgnoreFailure(subResult, *currentAction)) {
          ...
          (*currentAction)
              ->PrepForCompletion();  // Just in case we were cancelled
          StoreUnionAndDelete(currentAction);
        } else {
          return subResult;
        }
