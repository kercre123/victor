...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - -
void TrackGroundPointAction::GetRequiredVisionModes(
  ...
  switch (_salientTag) {
    case ExternalInterface::MessageEngineToGameTag::RobotObservedLaserPoint: {
      ...
      // If other messages yield valid points for tracking, add support for them
      // here (to enable the vision mode which produces them)

    default:
      ANKI_VERIFY(
          false, "TrackGroundPointAction.Constructor.NoVisionModeForTag",
          "Unsupported Tag: %s",
          ExternalInterface::MessageEngineToGameTagToString(_salientTag));
  }
}
...
ActionResult TrackGroundPointAction::InitInternal() {
  ...
  // Subscribe to the right message based on the saliency tag specified at
  // construction
  {
    ...
    switch (_salientTag) {
      ...
      default:
        PRINT_NAMED_ERROR(
            "TrackGroundPointAction.InitInternal.UnsupportedMessageTag", "%s",
            MessageEngineToGameTagToString(_salientTag));
        return ActionResult::BAD_MESSAGE_TAG;
    }

    DEV_ASSERT(
        _getObservedPointFromEvent != nullptr,
    ...
    auto handler =
        [this](const AnkiEvent<ExternalInterface::MessageEngineToGame>& event) {
          auto const& newObservation =
              _getObservedPointFromEvent(event.GetData());
          ...
          if (newObservation.groundArea > 0.f) {
            ...
            if (_isXPredictionEnabled || _isYPredictionEnabled) {
              ...
              _canPredict = (_pointObservation.timestamp -
                                 _prevPointObservation.timestamp <
                             kMaxPredictionTimeDelta_ms);
            }
          }
        };

    _signalHandle =
        GetRobot().GetExternalInterface()->Subscribe(_salientTag, handler);
  }

  // TODO: Use an action or animation for this? (Not super simple b/c base class
  // prevents overloading CheckIfDone)
  GetRobot().GetMoveComponent().MoveHeadToAngle(
      MIN_HEAD_ANGLE, MAX_HEAD_SPEED_RAD_PER_S, MAX_HEAD_ACCEL_RAD_PER_S2);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - -
ITrackAction::UpdateResult TrackGroundPointAction::UpdateTrackingHelper(
    Radians& absPanAngle_out, Radians& absTiltAngle_out, f32& distance_mm_out) {
  // Find pose of robot at time point was observed
  HistRobotState* histStatePtr = nullptr;
  RobotTimeStamp_t junkTime;
  if (RESULT_OK != GetRobot().GetStateHistory()->ComputeAndInsertStateAt(
                       _pointObservation.timestamp, junkTime, &histStatePtr)) {
    PRINT_NAMED_ERROR(
        "TrackGroundPointAction.UpdateTrackingHelper.PoseHistoryError",
        "Could not get historical pose for point observed at t=%d "
        "(lastRobotMsgTime = %d)",
  ...
  DEV_ASSERT(nullptr != histStatePtr,
             "TrackGroundPointAction.UpdateTrackingHelper.NullHistStatePtr");

  const Pose3d& histPose = histStatePtr->GetPose();

  const Point2f& groundPoint = ComputeGroundPointWrtCurrentRobot(
      histPose, GetRobot().GetPose(), _pointObservation.groundPoint);

  ComputeAbsAngles(GetRobot(), histPose, groundPoint, absPanAngle_out,
                   absTiltAngle_out);
  ...
  if (DEBUG_TRACKING_ACTIONS) {
    LOG_DEBUG("TrackGroundPointAction.UpdateTrackingHelper.GotObservation",
              ...
              _pointObservation.groundPoint.x(),
              _pointObservation.groundPoint.y());
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - -
ITrackAction::UpdateResult TrackGroundPointAction::PredictTrackingHelper(
    Radians& absPanAngle_out, Radians& absTiltAngle_out, f32& distance_mm_out) {
  DEV_ASSERT(
      _isXPredictionEnabled || _isYPredictionEnabled,
      "TrackGroundPointAction.PredictTrackingHelper.PredictionNotEnabled");

  // Convert observations to absolute coordinates so we can compare them
  // relative to a common origin
  HistRobotState* histStatePtr1 = nullptr;
  RobotTimeStamp_t t1;
  if (RESULT_OK != GetRobot().GetStateHistory()->ComputeAndInsertStateAt(
                       _prevPointObservation.timestamp, t1, &histStatePtr1)) {
    PRINT_NAMED_ERROR(
        "TrackGroundPointAction.PredictTrackingHelper.PoseHistoryError",
        "Could not get historical pose for point observed at t=%d "
        "(lastRobotMsgTime = %d)",
  ...
  const Point2f& groundPoint1 = ComputeGroundPointWrtCurrentRobot(
      histStatePtr1->GetPose(), GetRobot().GetPose(),
  ...
  if (DEBUG_TRACKING_ACTIONS) {
    LOG_DEBUG("TrackGroundPointAction.PredictTrackingHelper.GroundPoint1",
              ...
              _prevPointObservation.groundPoint.x(),
              _prevPointObservation.groundPoint.y(),
  ...
  RobotTimeStamp_t t2;
  if (RESULT_OK != GetRobot().GetStateHistory()->ComputeAndInsertStateAt(
                       _pointObservation.timestamp, t2, &histStatePtr2)) {
    PRINT_NAMED_ERROR(
        "TrackGroundPointAction.PredictTrackingHelper.PoseHistoryError",
        "Could not get historical pose for point observed at t=%d "
        "(lastRobotMsgTime = %d)",
  ...
  const Point2f& groundPoint2 = ComputeGroundPointWrtCurrentRobot(
      histStatePtr2->GetPose(), GetRobot().GetPose(),
  ...
  // Estimate the current position of the ground point assuming it continued
  // traveling the same velocity since it was last seen up until "now" (the last
  // message timestamp)
  const RobotTimeStamp_t now = GetRobot().GetLastMsgTimestamp();
  DEV_ASSERT(now >= t2,
             "TrackGroundPointAction.PredictTrackingHelper.BadTimestamp");
  ...
  ComputeAbsAngles(GetRobot(), histStatePtr2->GetPose(), predictedGroundPoint,
                   absPanAngle_out, absTiltAngle_out);
  ...
  if (DEBUG_TRACKING_ACTIONS) {
    LOG_DEBUG("TrackGroundPointAction.PredictTrackingHelper.Prediction",
              ...
              (TimeStamp_t)_prevPointObservation.timestamp,
              (TimeStamp_t)_pointObservation.timestamp, (TimeStamp_t)now,
              _prevPointObservation.groundPoint.x(),
              _pointObservation.groundPoint.x(), predictedGroundPoint.x(),
              _prevPointObservation.groundPoint.y(),
              _pointObservation.groundPoint.y(), predictedGroundPoint.y(),
              absPanAngle_out.getDegrees(), absTiltAngle_out.getDegrees(),
              distance_mm_out);
  }
...
void TrackGroundPointAction::ComputeAbsAngles(const Robot& robot,
  ...
  // Tilt angle
  const f32 kComputeHeadAngleTol =
      2.f * HEAD_ANGLE_TOL;  // How accurately we want to compute the head angle
  const Pose3d groundPoseWrtRobot(
      0, Z_AXIS_3D(), {groundPoint.x(), groundPoint.y(), 0.f}, histRobotPose);
  Result result = robot.ComputeHeadAngleToSeePose(
      groundPoseWrtRobot, absTiltAngle, kComputeHeadAngleTol);
  if (RESULT_OK != result) {
    // Approximation:
    PRINT_NAMED_WARNING(
        "TrackGroundPointAction.ComputeAbsAngles."
        "ComputeHeadAngleToSeePoseFailed",
        "");
  ...
  absTiltAngle =
      std::min(DEG_TO_RAD(kMaxHeadAngle_deg), absTiltAngle.ToFloat());

  // Pan Angle:
  absPanAngle =
      std::atan2f(groundPoint.y(), groundPoint.x());  // starts relative
  absPanAngle += histRobotPose.GetRotation().GetAngleAroundZaxis();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - -
Point2f TrackGroundPointAction::ComputeGroundPointWrtCurrentRobot(
    const Pose3d& histRobotPose, const Pose3d& currentRobotPose,
    const Point2f& observedGroundPt) {
  // Compute the ground point relative to the _current_ robot pose, based on its
  // position relative to the _historical_ pose when it was observed.

  Pose3d groundPose(0, Z_AXIS_3D(),
                    {observedGroundPt.x(), observedGroundPt.y(), 0.f},
                    histRobotPose);

  const bool success =
      groundPose.GetWithRespectTo(currentRobotPose, groundPose);
  ANKI_VERIFY(success,
              "TrackGroundPointAction.GetGroundWrtCurrentRobot.GetWrtFailed",
              "");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - -
ITrackAction::UpdateResult TrackGroundPointAction::UpdateTracking(
    Radians& absPanAngle_out, Radians& absTiltAngle_out, f32& distance_mm_out) {
  ...
  if (_gotNewPointObservation) {
    ...
    if (_pointObservation.groundPoint.x() < _maxDistance_mm) {
      ...
      updateResult = UpdateTrackingHelper(absPanAngle_out, absTiltAngle_out,
                                          distance_mm_out);
    } else {
    ...
    DEV_ASSERT_MSG(
        ...
        (TimeStamp_t)GetRobot().GetLastImageTimeStamp(),
        (TimeStamp_t)_pointObservation.timestamp);

    const RobotTimeStamp_t timeSinceLastPoint_ms =
        GetRobot().GetLastImageTimeStamp() - _pointObservation.timestamp;

    // Didn't see the point in the last image
    if (_canPredict && (timeSinceLastPoint_ms < _maxPredictionWindow_ms)) {
      updateResult = PredictTrackingHelper(absPanAngle_out, absTiltAngle_out,
                                           distance_mm_out);
