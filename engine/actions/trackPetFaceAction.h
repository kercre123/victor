...
class TrackPetFaceAction : public ITrackAction {
 public:
  ...
  // Track first pet with the right type (or any pet at all if PetType set to
  // Unknown). Note the pet being tracked could change during tracking as it is
  // the first one found in PetWorld on each update tick.
  TrackPetFaceAction(Vision::PetType petType);

  virtual void GetCompletionUnion(
      ActionCompletedUnion& completionInfo) const override;
