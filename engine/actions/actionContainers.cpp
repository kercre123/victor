...
void ActionList::InitDependent(Vector::Robot* robot,
...
Result ActionList::QueueAction(QueueActionPosition inPosition,
  ...
  if (action == nullptr) {
    PRINT_NAMED_ERROR("ActionList.QueueAction.NullAction",
                      "Can't queue null action");
    return RESULT_FAIL;
  }
...
bool ActionList::Cancel(RobotActionType withType) {
  // Don't bother cancelling actions if we are in the process of clearing the
  // queues
...
bool ActionList::Cancel(u32 idTag) {
  ...
  for (auto& q : _queues) {
    if (q.second.Cancel(idTag) == true) {
      if (found) {
        PRINT_NAMED_WARNING(
            "ActionList.Cancel.DuplicateTags",
...
bool ActionList::IsEmpty() const { return _queues.empty(); }

void ActionList::Print() const {
  if (IsEmpty()) {
    PRINT_STREAM_INFO("ActionList.Print", "ActionList is empty.\n");
  } else {
    PRINT_STREAM_INFO("ActionList.Print",
                      "ActionList contains " << _queues.size() << " queues:\n");
...
ActionList::SlotHandle ActionList::AddConcurrentAction(IActionRunner* action,
                                                       u8 numRetries) {
  if (action == nullptr) {
    PRINT_NAMED_WARNING("ActionList.AddAction.NullActionPointer",
                        "Refusing to add null action");
    return -1;
  }
  ...
  // Find an empty slot starting at 1 since all other queue positions will queue
  // into slot 0
  ...
  if (queue.QueueAtEnd(action, numRetries) != RESULT_OK) {
    PRINT_NAMED_ERROR("ActionList.AddAction.FailedToAdd",
                      "Failed to add action to new queue");
    return -1;
  }
...
ActionEndedCallbackID ActionList::RegisterActionEndedCallbackForAllActions(
    ActionEndedCallback callback) {
...
ActionQueue& ActionList::GetActionQueueForSlot(SlotHandle handle) {
  ...
  if (iter == _queues.end()) {
    ...
    const auto resultPair =
        _queues.insert(std::make_pair(handle, std::move(newQueue)));
...
ActionQueue::ActionQueue(Robot& robot) : _robot(robot) {}
...
bool ActionQueue::Cancel(RobotActionType withType) {
  ...
  if (_currentAction != nullptr) {
    if (withType == RobotActionType::UNKNOWN ||
        _currentAction->GetType() == withType) {
      _currentAction->Cancel();
      DeleteAction(_currentAction);
      found = true;
    }
  ...
  for (auto iter = _queue.begin(); iter != _queue.end();) {
    ...
    if (withType == RobotActionType::UNKNOWN ||
        (*iter)->GetType() == withType) {
      // If this doesn't actually delete the action then it must have already
      // been deleted so our iter will be invalid since the call that actually
      // deleted the action will erase the iter
...
Result ActionQueue::QueueNow(IActionRunner* action, u8 numRetries) {
  ...
  if (_queue.empty()) {
    ...
    if (currentAction != nullptr) {
      LOG_DEBUG("ActionQueue.QueueNow.CancelingPrevious",
                "Canceling %s [%d] in favor of action %s [%d]",
                currentAction->GetName().c_str(), currentAction->GetTag(),
                action->GetName().c_str(), action->GetTag());
    }
    DeleteAction(_currentAction);
  ...
  if (IsEmpty()) {
    ...
    if (_currentAction != nullptr && _currentAction->Interrupt()) {
      ...
      LOG_INFO("ActionQueue.QueueAtFront.Interrupt",
               ...
               _currentAction->GetName().c_str(), action->GetName().c_str());
      action->SetNumRetries(numRetries);
...
Result ActionQueue::Update() {
  ...
  if (!_queue.empty() || _currentAction != nullptr) {
    ...
    _currentAction->GetRobot()
        .GetActionList()
        .GetActionWatcher()
        .ParentActionUpdating(_currentAction);
    ...
    if (isRunning) {
      ...
      cozmoContext->SetSdkStatus(SdkStatusType::Action,
                                 std::string(_currentAction->GetName()));
    } else {
      vizManager->SetText(TextLabelType::ACTION, NamedColors::GREEN, "");
      cozmoContext->SetSdkStatus(SdkStatusType::Action, "");
    }
    ...
    if (!isRunning) {
      ...
      if (actionResult != ActionResult::SUCCESS &&
          actionResult != ActionResult::CANCELLED_WHILE_RUNNING) {
...
const IActionRunner* ActionQueue::GetCurrentAction() const {
  // If don't have a current action (aren't running anything) but have things
  // that will be run then the current action is the first one in the queue
...
bool ActionQueue::DeleteActionAndIter(
    IActionRunner*& action, std::list<IActionRunner*>::iterator& iter) {
  if (action != nullptr) {
    if (iter != _queue.end()) {
      DEV_ASSERT((*iter) == action,
                 "ActionQueue.DeleteAction.IterAndActionNotTheSame");
    }

    // If the action isn't currently being deleted meaning it was successfully
    // inserted into the set so the second element of the pair will be true
    ...
    if (pair.second) {
      ...
      // Remove the action from the queue before emitting a RobotActionCompleted
      // signal This is to prevent there being a null action in the queue while
      // systems are handling the action completion
      if (iter != _queue.end()) {
      ...
      if (externalInterface != nullptr) {
        externalInterface->Broadcast(
            ExternalInterface::MessageEngineToGame(std::move(rca)));
      }
