 ...
 * Date:   8/29/2014
 *
 * Description: Implements docking related cozmo-specific actions, derived from
...
Point2f ComputePreActionPoseDistThreshold(
    const Pose3d& preActionPose, const Pose3d& actionObject,
...
class IDockAction : public IAction {
 public:
  IDockAction(ObjectID objectID, const std::string name,
  ...
  // If true robot will check that it is close enough to the closest preaction
  // pose before docking If false robot will dock with closest visible marker on
  // dockObject from current position
  void SetDoNearPredockPoseCheck(bool doCheck) {
    _doNearPredockPoseCheck = doCheck;
  }

  // Use a value <= 0 to ignore how far away the robot is from the closest
  // PreActionPose and proceed to dock with marker corresponding to closest
  // preaction pose.
  ...
  void SetDockingMethod(DockingMethod dockingMethod) {
    _dockingMethod = dockingMethod;
  }

  void SetDoLiftLoadCheck(bool enable) { _doLiftLoadCheck = enable; }

  void SetBackUpWhileLiftingCube(bool enable) {
    _backUpWhileLiftingCube = enable;
  }

  void SetNumDockingRetries(u8 numRetries) { _numDockingRetries = numRetries; }

  // The offset for the preDock pose as opposed to the offset for the actual
  // docking manuever Is used when checking if we are close enough to the
  // preDock pose
  void SetPreDockPoseDistOffset(f32 offset) {
    _preDockPoseDistOffsetX_mm = offset;
  }

  // Whether or not the action will check that we are currently seeing a
  // specific marker (the one corresponding to the closest preDock pose) on the
  // object before docking or that we are seeing any marker on the object By
  // default this is false (the action is looking for a specific marker)
  void SetShouldVisuallyVerifyObjectOnly(const bool b) {
    _visuallyVerifyObjectOnly = b;
  }

  // Whether or not we should first turn towards and visually verify the
  // dockObject
  void SetShouldFirstTurnTowardsObject(const bool b) {
    _firstTurnTowardsObject = b;
  }
  ...
  struct PreActionPoseInput {
    ...
    PreActionPoseInput(const ActionableObject* object,
                       ...
                       bool doNearPreDockPoseCheck,
                       f32 preDockPoseDistOffsetX_mm,
                       f32 preActionPoseAngleTolerance, bool useApproachAngle,
                       f32 approachAngle_rad)
        : object(object),
  ...
  // Computes that angle (wrt world) at which the robot would have to approach
  // the given pose such that it places the carried object at the given pose
  ...
  // Common function for filtering a list of possible predock poses by removing
  // any "matching" the given pose. Returns true if one or more elements were
  // removed from the vector, false otherwise
  static bool RemoveMatchingPredockPose(const Pose3d& pose,
                                        std::vector<Pose3d>& possiblePoses);

  // Whether or not the lift is believed to be carrying something based on
  // liftLoadCheck at the end of a pickup action.
 ...
 protected:
  // IDockAction derived classes nearly universally require the same
  // VisionModes. Special cases should override this function.
  ...
  virtual const Vision::KnownMarker* GetDockMarker2(
  ...
  virtual void GetCompletionUnion(
    ...
    // TODO: Annoying we have to copy this out, bet the Get_() method is
    // const...
  ...
  // Identify cases where the robot should play start, loop, and end animations
  // while docking. Override this to false to prevent playing docking anims.
 ...
 private:
  // Sets up the turnTowardsObject action and the "glance up to see if there is
  // a block on top of the block we are docking with" action
  ...
  // Handler for when lift begins to move so that we can play an accompanying
  // sound
  ...
  // These default docking animation triggers can be overridden with
  // SetDockAnimations()
...
class PopAWheelieAction : public IDockAction {
 ...
 protected:
  virtual bool ShouldFailOnTransitionOffTreads() const override {
    return false;
  }

  virtual PreActionPose::ActionType GetPreActionType() override {
    return PreActionPose::ROLLING;
  }
...
// If not carrying anything, does a face plant by knocking over a stack of
// blocks
...
class AlignWithObjectAction : public IDockAction {
 public:
  AlignWithObjectAction(
      ObjectID objectID, const f32 distanceFromMarker_mm,
  ...
  static PreActionPose::ActionType GetPreActionTypeFromAlignmentType(
 ...
 protected:
  ...
  virtual PreActionPose::ActionType GetPreActionType() override {
    return _preActionPoseActionType;
  }
 ...
 private:
  ...
  PreActionPose::ActionType _preActionPoseActionType =
...
class PickupObjectAction : public IDockAction {
 ...
 private:
  ...
  // The max amount of time that cube motion is allowed to be moving after robot
  // completes backup. This is to check that the cube is not in the user's
  // hands.
  const u32 kMaxObjectStillMovingAfterRobotStopTime_ms = 500;

  // The max amount of time that a cube is allowed to have not been moving
  // before the point that the robot completes backup. This is to check to make
  // sure the cube was moving at all during the pickup action.
  ...
  // which often results in only brief motion when cube is first lifted compared
  // to low pickup. This means the high pickup action is easier to fool if you
  // move the block out of the way at the last second and set it down somewhere,
  // but... you know... just stop being a dick.
...
// @param placementPose    - The pose in which the carried object should be
// placed.
// @param useExactRotation - If true, then the carried object is placed in the
// exact
//                           6D pose represented by placement pose. Otherwise,
//                           x,y and general axis alignment with placementPose
//                           rotation are the only constraints.
class PlaceObjectOnGroundAtPoseAction : public CompoundActionSequential {
 public:
  PlaceObjectOnGroundAtPoseAction(
      const Pose3d& placementPose, const bool useExactRotation = false,
...
class PlaceRelObjectAction : public IDockAction {
 public:
  PlaceRelObjectAction(ObjectID objectID, const bool placeOnGround = false,
  ...
  // placement offsets are relative to the object's coordinate system, not
  // marker
  static ActionResult ComputePlaceRelObjectOffsetPoses(
 ...
 private:
  ...
  // Uses the robot's angle in its pre-dock pose and the docking object's
  // rotation to calculate how to reflect/negate the placement offsets so they
  // are relative to the docking object's world coordinates instead of the
...
class RollObjectAction : public IDockAction {
 ...
 private:
  // How much we should look down to be able to see the marker of the object we
  // just rolled
