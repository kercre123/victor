...
void MessageHandler::Init(const Json::Value& config, RobotManager* robotMgr,
                          const CozmoContext* context) {
  _robotManager = robotMgr;
  _robotConnectionManager =
      std::make_unique<RobotConnectionManager>(_robotManager);
...
Result MessageHandler::ProcessMessages() {
  ...
  if (_isInitialized) {
    DEV_ASSERT(_robotConnectionManager,
               "MessageHander.ProcessMessages.InvalidRobotConnectionManager");
    ...
    if (RESULT_OK != result) {
      LOG_ERROR("MessageHandler.ProcessMessages",
    ...
    while (_robotConnectionManager->PopData(nextData)) {
      ...
      if (dataSize <= 0) {
        PRINT_NAMED_ERROR("MessageHandler.ProcessMessages",
                          "Tried to process message of invalid size");
        continue;
      }

      // see if message type should be filtered out based on potential firmware
      // mismatch
      const RobotInterface::RobotToEngineTag msgType =
          static_cast<RobotInterface::RobotToEngineTag>(nextData.data()[0]);
      ...
      if (unpackSize != nextData.size()) {
        PRINT_NAMED_ERROR(
            "RobotMessageHandler.MessageUnpack",
            "Message unpack error, tag %s expecting %zu but have %zu",
...
Result MessageHandler::SendMessage(const RobotInterface::EngineToRobot& msg,
                                   bool reliable, bool hot) {
  ...
  DEV_ASSERT(packedSize == expectedSize,
             "MessageHandler.SendMessage.MessageSizeMismatch");
  if (packedSize != expectedSize) {
    return RESULT_FAIL;
  }

  const bool success = _robotConnectionManager->SendData(
      messageData.data(), Util::numeric_cast<unsigned int>(packedSize));
  if (!success) {
    return RESULT_FAIL;
  }
...
void MessageHandler::Broadcast(const RobotInterface::RobotToEngine& message) {
  ...
  _eventMgr.Broadcast(AnkiEvent<RobotInterface::RobotToEngine>(
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds(), type,
      message));
}
