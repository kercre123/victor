...
CONSOLE_VAR(bool, kStressTestThreadedPrintsDuringLoad, "RobotDataLoader",
            false);
...
void RobotDataLoader::LoadNonConfigData() {
...

  {
    ...
    auto* sContainer = seqLoader.LoadSpriteSequences(
        _platform, _spritePaths.get(), _spriteCache.get(), spriteSequenceDirs);
...
void RobotDataLoader::CollectAnimFiles() {
  // animations
  {
    ...
    for (const auto& path : paths) {
      WalkAnimationDir(path, _animFileTimestamps,
  ...
  {
    WalkAnimationDir(
        "config/engine/lights/cubeLights", _cubeLightAnimFileTimestamps,
        [this](const std::string& filename) {
  ...
  if (!FACTORY_TEST) {
    ...
    {
      WalkAnimationDir(
          "assets/animationGroups/", _groupAnimFileTimestamps,
          [this](const std::string& filename) {
  ...
  // print results
  {
    for (const auto& fileListPair : _jsonFiles) {
      PRINT_CH_INFO("Animations", "RobotDataLoader.CollectAnimFiles.Results",
                    "Found %zu animation files of type %d",
...
void RobotDataLoader::LoadCubeLightAnimations() {
  ...
  MyDispatchWorker::FunctionType loadFileFunc =
      std::bind(&RobotDataLoader::LoadCubeLightAnimationFile, this,
                std::placeholders::_1);
  ...
  PRINT_CH_INFO("Animations",
                "RobotDataLoader.LoadCubeLightAnimations.LoadTime",
...
void RobotDataLoader::LoadAnimationGroups() {
  using MyDispatchWorker = Util::DispatchWorker<3, const std::string&>;
  MyDispatchWorker::FunctionType loadFileFunc = std::bind(
      &RobotDataLoader::LoadAnimationGroupFile, this, std::placeholders::_1);
  MyDispatchWorker myWorker(loadFileFunc);
  const auto& fileList = _jsonFiles[FileType::AnimationGroup];
  const auto size = fileList.size();
  for (int i = 0; i < size; i++) {
    myWorker.PushJob(fileList[i]);
    // LOG_DEBUG("RobotDataLoader.LoadAnimationGroups", "loaded anim group %d of
    // %zu", i, size);
  }
  myWorker.Process();
}

void RobotDataLoader::WalkAnimationDir(
  ...
  const std::string animationFolder =
      _platform->pathToResource(Util::Data::Scope::Resources, animationDir);
  ...
  for (const auto& path : filePaths) {
    ...
    if (result == -1) {
      LOG_WARNING("RobotDataLoader.WalkAnimationDir",
...
void RobotDataLoader::LoadAnimationGroupFile(const std::string& path) {
  ...
  if (success && !animGroupDef.empty()) {
    std::string animationGroupName =
        Util::FileUtils::GetFileName(path, true, true);

    // PRINT_CH_DEBUG("Animations",
    // "RobotDataLoader.LoadAnimationGroupFile.LoadingSpecificAnimGroupFromJson",
    //               "Loading '%s' from %s", animationGroupName.c_str(),
    //               path.c_str());
...
void RobotDataLoader::LoadEmotionEvents() {
  const std::string emotionEventFolder = _platform->pathToResource(
      Util::Data::Scope::Resources, "config/engine/emotionevents/");
  auto eventFiles = Util::FileUtils::FilesInDirectory(emotionEventFolder, true,
                                                      ".json", false);
  for (const std::string& filename : eventFiles) {
    ...
    if (success && !eventJson.empty()) {
      _emotionEvents.emplace(std::piecewise_construct,
                             std::forward_as_tuple(filename),
                             std::forward_as_tuple(std::move(eventJson)));
      LOG_DEBUG("RobotDataLoader.EmotionEvents", "Loaded '%s'",
                filename.c_str());
    } else {
      LOG_WARNING("RobotDataLoader.EmotionEvents", "Failed to read '%s'",
                  filename.c_str());
    }
  }
}
...
void RobotDataLoader::LoadBehaviors() {
  ...
  const std::string behaviorFolder =
      _platform->pathToResource(Util::Data::Scope::Resources, path);
  auto behaviorJsonFiles =
      Util::FileUtils::FilesInDirectory(behaviorFolder, true, ".json", true);
  for (const auto& filename : behaviorJsonFiles) {
    ...
    if (success && !behaviorJson.empty()) {
      ...
      auto result = _behaviors.emplace(
      ...
      DEV_ASSERT_MSG(result.second,
                     ...
                     "Failed to insert BehaviorID %s - make sure all behaviors "
                     "have unique IDs",
                     BehaviorTypesWrapper::BehaviorIDToString(behaviorID));

    } else if (!success) {
      LOG_WARNING("RobotDataLoader.Behavior", "Failed to read '%s'",
                  filename.c_str());
    }
  }
}
...
void RobotDataLoader::LoadAnimationWhitelist() {
  static const std::string jsonFilename =
      "config/engine/animation_whitelist.json";
  Json::Value whitelistConfig;
  const bool success = _platform->readAsJson(Util::Data::Scope::Resources,
                                             jsonFilename, whitelistConfig);
  if (!success) {
    LOG_ERROR(
        "RobotDataLoader.AnimationWhitelistConfig",
        "Animation whitelist json config file %s not found or failed to parse",
        jsonFilename.c_str());
  } else {
    static constexpr const char* kDriveOffChargerAnimsKey =
    ...
    PRINT_CH_INFO("Animations",
                  "RobotDataLoader.AnimationWhitelist.LoadedConfig",
...
void RobotDataLoader::LoadWeatherResponseMaps() {
  ...
  const std::string path =
      "config/engine/behaviorComponent/weather/weatherResponseMaps/";
  const char* kAPIValueKey = "APIValue";
  const char* kCladTypeKey = "CladType";

  const std::string responseFolder =
      _platform->pathToResource(Util::Data::Scope::Resources, path);
  auto responseJSONFiles = Util::FileUtils::FilesInDirectory(
      responseFolder, useFullPath, extensions, recurse);
  for (const auto& filename : responseJSONFiles) {
    Json::Value responseJSON;
    const bool success = _platform->readAsJson(filename, responseJSON);
    if (success && !responseJSON.empty() && responseJSON.isArray()) {
      for (const auto& pair : responseJSON) {
        if (ANKI_VERIFY(
                pair.isMember(kAPIValueKey) && pair.isMember(kCladTypeKey),
                "RobotDataLoader.LoadWeatherResponseMaps.PairMissingKey",
                "File %s has an invalid pair", filename.c_str())) {
          WeatherConditionType cond =
              WeatherConditionTypeFromString(pair[kCladTypeKey].asString());
          ...
          if (!str.empty()) {
            auto resPair =
                _weatherResponseMap->emplace(std::make_pair(str, cond));
            ANKI_VERIFY(
                resPair.second,
            ...
            PRINT_NAMED_ERROR(
                "RobotDataLoader.LoadWeatherResponseMaps.MissingAPIValue",
      ...
      LOG_WARNING("RobotDataLoader.LoadWeatherResponseMap",
...
void RobotDataLoader::LoadWeatherRemaps() {
  static const std::string jsonFilename =
      "config/engine/behaviorComponent/weather/condition_remaps.json";
  const bool success = _platform->readAsJson(Util::Data::Scope::Resources,
                                             jsonFilename, _weatherRemaps);
  if (!success) {
...
void RobotDataLoader::LoadWeatherConditionTTSMap() {
  ...
  static const std::string jsonFilename =
      "config/engine/behaviorComponent/weather/condition_to_tts.json";

  Json::Value conditionList;
  const bool success = _platform->readAsJson(Util::Data::Scope::Resources,
                                             jsonFilename, conditionList);
  if (!success || !conditionList.isArray()) {
    PRINT_NAMED_WARNING(
        "RobotDataLoader.LoadWeatherConditionTTSMap.ErrorReadingFile", "");
    return;
  }
  ...
  for (const auto& entry : conditionList) {
    if (!entry.isMember(kConditionKey)) {
      PRINT_NAMED_WARNING(
          "RobotDataLoader.LoadWeatherConditionTTSMap."
          "EntryDoesNotContainCondition",
          "");
      continue;
    }
    if (!entry.isMember(kWhatToSayKey)) {
      PRINT_NAMED_WARNING(
          "RobotDataLoader.LoadWeatherConditionTTSMap."
          "EntryDoesNotContainSayKey",
          "");
      continue;
    }
    const bool conditionExists = WeatherConditionTypeFromString(
        entry[kConditionKey].asString(), condition);
    if (!conditionExists) {
      PRINT_NAMED_WARNING(
          "RobotDataLoader.LoadWeatherConditionTTSMap.InvalidWeatherCondition",
          "Condition %s not found in weather condition enum",
          entry[kConditionKey].asString().c_str());
      continue;
    }
    _weatherConditionTTSMap->emplace(std::move(condition),
                                     entry[kWhatToSayKey].asString());
  }

  if (_weatherConditionTTSMap->size() !=
      static_cast<int>(WeatherConditionType::Count)) {
    PRINT_NAMED_WARNING(
        "RobotDataLoader.LoadWeatherConditionTTSMap.MissingConditions",
...
void RobotDataLoader::LoadVariableSnapshotJsonMap() {
  ...
  std::string path = VariableSnapshotComponent::GetSavePath(
      _platform, VariableSnapshotComponent::kVariableSnapshotFolder,
      VariableSnapshotComponent::kVariableSnapshotFilename);
  Json::Value outLoadedJson;
  const bool success = _platform->readAsJson(path, outLoadedJson);
  // check whether the look up was successful and we got back a nonempty JSON
  // array
  if (success && !outLoadedJson.empty() && outLoadedJson.isArray()) {
    for (const auto& loadedInfo : outLoadedJson) {
      ...
      const auto key =
          loadedInfo[VariableSnapshotEncoder::kVariableSnapshotIdKey]
              .asString();
      ...
      if (VariableSnapshotIdFromString(key, variableSnapshotId)) {
        ...
        PRINT_NAMED_WARNING(
            "RobotDataLoader.LoadVariableSnapshotJsonMap.UnknownStringinJson",
            "Key %s was not recognized as a valid snapshot value, will be "
            "dropped",
            key.c_str());
      }
    }
...
void RobotDataLoader::LoadCubeSpinnerConfig() {
  static const std::string jsonFilename =
      "config/engine/behaviorComponent/cubeSpinnerLightMaps.json";
  const bool success = _platform->readAsJson(Util::Data::Scope::Resources,
                                             jsonFilename, _cubeSpinnerConfig);
  if (!success) {
    LOG_ERROR("RobotDataLoader.LoadCubeSpinnerConfig",
              "LoadCubeSpinnerConfig Json config file %s not found or failed "
              "to parse",
              jsonFilename.c_str());
  }
}
...
void RobotDataLoader::LoadUserDefinedBehaviorTreeConfig() {
  ...
  static const std::string jsonFilename =
      "config/engine/userDefinedBehaviorTree/conditionToBehaviorMap.json";
  const bool jsonSuccess =
      _platform->readAsJson(Util::Data::Scope::Resources, jsonFilename,
                            _userDefinedBehaviorTreeConfig);

  // if json is read properly, load the config data
  if (!jsonSuccess) {
    LOG_ERROR("RobotDataLoader.LoadUserDefinedBehaviorTreeConfig",
              "LoadUserDefinedBehaviorTreeConfig Json config file %s not found "
              "or failed to parse",
              jsonFilename.c_str());
  }
  ...
  const std::string editBehaviorIdString = JsonTools::ParseString(
      _userDefinedBehaviorTreeConfig, kEditModeTriggerIDKey,
      "RobotDataLoader.LoadUserDefinedBehaviorTreeConfig."
      "ParseEditConditionStringFailed");
  const bool editBehaviorIdSuccess = BEIConditionTypeFromString(
      editBehaviorIdString, _userDefinedEditCondition);

  if (!editBehaviorIdSuccess) {
    LOG_ERROR("RobotDataLoader.LoadUserDefinedBehaviorTreeConfig",
              "LoadUserDefinedBehaviorTreeConfig: Edit behavior %s not a valid "
              "BehaviorID.",
  ...
  for (const Json::Value& mapOptionsJson :
       _userDefinedBehaviorTreeConfig[kMappingOptionsListKey]) {
    BEIConditionType beiCondType = BEIConditionType::Invalid;
    const std::string beiCondTypeString = JsonTools::ParseString(
        mapOptionsJson, kConditionTypeKey,
        "RobotDataLoader.LoadUserDefinedBehaviorTreeConfig."
        "ParseConditionStringFailed");
    const bool beiCondTypeParseSuccess =
        BEIConditionTypeFromString(beiCondTypeString, beiCondType);
    const bool editConditionMappedToBehaviors =
        _userDefinedEditCondition == beiCondType;

    // edit condition should not be customizable
    if (editConditionMappedToBehaviors) {
      LOG_ERROR("RobotDataLoader.LoadUserDefinedBehaviorTreeConfig",
                "LoadUserDefinedBehaviorTreeConfig: edit condition should not "
                "be customizable.");
      return;
    }
    ...
    // if the BehaviorID strings are parsed to strings successfully, convert
    // them to BehaviorIDs
    std::vector<std::string> behaviorIdStrings;
    const bool behaviorIdStringsParseSuccess =
        JsonTools::GetVectorOptional<std::string>(
            mapOptionsJson, kBehaviorOptionsKey, behaviorIdStrings);

    if (!behaviorIdStringsParseSuccess) {
      LOG_ERROR(
          "RobotDataLoader.LoadUserDefinedBehaviorTreeConfig."
          "ParseBehaviorStringsFailed",
          "LoadUserDefinedBehaviorTreeConfig: Could not parse list of Json "
          "BehaviorID Strings.");
      return;
    }

    // if the string->BehaviorID conversion happens successfully, add them into
    // a set
    std::set<BehaviorID> behaviors;
    for (const auto& behaviorIdString : behaviorIdStrings) {
      BehaviorID behaviorId = BehaviorID::Anonymous;
      const bool behaviorIdSuccess =
          BehaviorIDFromString(behaviorIdString, behaviorId);
...
std::map<std::string, std::string> RobotDataLoader::CreateFileNameToFullPathMap(
  ...
  // Get all independent sprites
  {
    ...
    for (const auto& dir : srcDirs) {
      const std::string fullPathFolder =
          _platform->pathToResource(Util::Data::Scope::Resources, dir);

      auto fullImagePaths = Util::FileUtils::FilesInDirectory(
          fullPathFolder, useFullPath, fileExtensions.c_str(), recurse);
...
void RobotDataLoader::LoadAnimationTriggerMap() {
  _animationTriggerMap->Load(
      _platform, "assets/cladToFileMaps/AnimationTriggerMap.json", "AnimName");
}

void RobotDataLoader::LoadCubeAnimationTriggerMap() {
  _cubeAnimationTriggerMap->Load(
      _platform, "assets/cladToFileMaps/CubeAnimationTriggerMap.json",
      "AnimName");
}

void RobotDataLoader::LoadDasBlacklistedAnimations() {
  static const std::string kBlacklistedAnimationTriggersConfigKey =
      "blacklisted_animation_triggers";
  const Json::Value& blacklistedTriggers =
      _dasEventConfig[kBlacklistedAnimationTriggersConfigKey];
  for (int i = 0; i < blacklistedTriggers.size(); i++) {
    const std::string& trigger = blacklistedTriggers[i].asString();
    _dasBlacklistedAnimationTriggers.insert(
        AnimationTriggerFromString(trigger));
  }
  static const std::string kBlacklistedAnimationNamesConfigKey =
      "blacklisted_animation_names";
  const Json::Value& blacklistedAnims =
      _dasEventConfig[kBlacklistedAnimationNamesConfigKey];
...
void RobotDataLoader::LoadRobotConfigs() {
  ...
  {
    static const std::string jsonFilename =
        "config/engine/behaviorComponent/victor_behavior_config.json";
    const bool success =
        _platform->readAsJson(Util::Data::Scope::Resources, jsonFilename,
                              _victorFreeplayBehaviorConfig);
    if (!success) {
  ...
  // vision config
  {
    ...
    const bool success = _platform->readAsJson(
        Util::Data::Scope::Resources, jsonFilename, _robotVisionConfig);
    if (!success) {
  ...
  {
    static const std::string jsonFilename =
        "config/engine/visionScheduleMediator_config.json";
    const bool success =
        _platform->readAsJson(Util::Data::Scope::Resources, jsonFilename,
                              _visionScheduleMediatorConfig);
    if (!success) {
      LOG_ERROR("RobotDataLoader.VisionScheduleMediatorConfigNotFound",
                "VisionScheduleMediator Json config file %s not found or "
                "failed to parse",
                jsonFilename.c_str());
    }
  }
  ...
  {
    static const std::string jsonFilename =
        "config/engine/behaviorComponent/user_intent_map.json";
    const bool success = _platform->readAsJson(
        Util::Data::Scope::Resources, jsonFilename, _userIntentsConfig);
    if (!success) {
  ...
  {
    static const std::string jsonFilename =
        "config/engine/das_event_config.json";
    const bool success = _platform->readAsJson(Util::Data::Scope::Resources,
                                               jsonFilename, _dasEventConfig);
    if (!success) {
  ...
  // feature gate
  {
    const std::string filename{_platform->pathToResource(
        Util::Data::Scope::Resources, "config/features.json")};
  ...
  {
    const std::string filename{_platform->pathToResource(
        Util::Data::Scope::Resources, "config/experiments.json")};
    const std::string fileContents{Util::FileUtils::ReadFile(filename)};
  ...
  {
    static const std::string jsonFilename =
        "webserver/webServerConfig_engine.json";
    const bool success = _platform->readAsJson(
        Util::Data::Scope::Resources, jsonFilename, _webServerEngineConfig);
    if (!success) {
  ...
  {
    static const std::string jsonFilename =
        "config/engine/photography_config.json";
    const bool success = _platform->readAsJson(
        Util::Data::Scope::Resources, jsonFilename, _photographyConfig);
    if (!success) {
  ...
  {
    static const std::string jsonFilename =
        "config/engine/settings_config.json";
    const bool success = _platform->readAsJson(Util::Data::Scope::Resources,
                                               jsonFilename, _settingsConfig);
    if (!success) {
  ...
  {
    static const std::string jsonFilename =
        "config/engine/eye_color_config.json";
    const bool success = _platform->readAsJson(Util::Data::Scope::Resources,
                                               jsonFilename, _eyeColorConfig);
    if (!success) {
  ...
  {
    static const std::string jsonFilename =
        "config/engine/accountSettings_config.json";
    const bool success = _platform->readAsJson(
        Util::Data::Scope::Resources, jsonFilename, _accountSettingsConfig);
    if (!success) {
  ...
  {
    static const std::string jsonFilename =
        "config/engine/userEntitlements_config.json";
    const bool success = _platform->readAsJson(
        Util::Data::Scope::Resources, jsonFilename, _userEntitlementsConfig);
    if (!success) {
...
std::string RobotDataLoader::GetCubeAnimationForTrigger(
    CubeAnimationTrigger ev) const {
  return _cubeAnimationTriggerMap->GetValue(ev);
}

bool RobotDataLoader::IsAnimationAllowedToMoveBodyOnCharger(
