...
class RobotToEngineImplMessaging
 ...
 public:
  ...
  void HandleImuRawData(const AnkiEvent<RobotInterface::RobotToEngine>& message,
                        Robot* const robot);
  void HandleSyncRobotAck(
      const AnkiEvent<RobotInterface::RobotToEngine>& message,
      Robot* const robot);
  void HandleMotorCalibration(
      const AnkiEvent<RobotInterface::RobotToEngine>& message,
      Robot* const robot);
  void HandleMotorAutoEnabled(
      const AnkiEvent<RobotInterface::RobotToEngine>& message,
      Robot* const robot);
  void HandleAudioInput(const AnkiEvent<RobotInterface::RobotToEngine>& message,
                        Robot* const robot);
  void HandleMicDirection(
      const AnkiEvent<RobotInterface::RobotToEngine>& message,
      Robot* const robot);
  void HandleMicDataState(
      const AnkiEvent<RobotInterface::RobotToEngine>& message,
      Robot* const robot);
  void HandleStreamCameraImages(
      const AnkiEvent<RobotInterface::RobotToEngine>& message,
      Robot* const robot);
  void HandleDisplayedFaceImage(
      const AnkiEvent<RobotInterface::RobotToEngine>& message,
      Robot* const robot);
 ...
 private:
  ...
  const u32 kAllFaceImageRGBChunksReceivedMask =
      0x3fffffff;  // 30 bits for 30 expected chunks
};
