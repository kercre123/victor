...
bool ActionableObject::IsPreActionPoseValid(
    const PreActionPose& preActionPose,
  ...
  const f32 vertAlignThresh =
      1.f - std::cos(PreActionPose::ANGLE_TOLERANCE);  // TODO: tighten?
  bool isValid =
      NEAR(checkPose.GetRotationMatrix()(2, 2), 1.f, vertAlignThresh);
  ...
  if (isValid && !obstacles.empty()) {
    ...
    Point2f xyStart(
        preActionPose.GetPose().GetWithRespectToRoot().GetTranslation());
    const Point2f xyEnd(preActionPose.GetMarker()
                            ->GetPose()
    ...
    // Pull back xyStart to the rear of the robot's bounding box when the robot
    // is at the preaction pose.
    xyStart -= (stepVec * (ROBOT_BOUNDING_X - ROBOT_BOUNDING_X_FRONT));
    lineLength += (ROBOT_BOUNDING_X - ROBOT_BOUNDING_X_FRONT);

    const s32 numSteps =
        Util::numeric_cast<s32>(std::floor(lineLength / stepSize));
    ...
    for (s32 i = 0; i < numSteps && pathClear; ++i) {
      ...
      // DEBUG VIZ
      //          _vizManager->DrawGenericQuad(i+1000+(0xffff &
      //          (long)preActionPose.GetMarker()),
      //                                                     Quad2f(currentPoint+Point2f(-1.f,-1.f),
      //                                                            currentPoint+Point2f(-1.f,
      //                                                            1.f),
      //                                                            currentPoint+Point2f(
      //                                                            1.f,-1.f),
      //                                                            currentPoint+Point2f(
      //                                                            1.f, 1.f)),
      //                                                     1.f,
      //                                                     NamedColors::BLUE);
      //          _vizManager->DrawGenericQuad(i+2000+(0xffff &
      //          (long)preActionPose.GetMarker()),
      //                                                     Quad2f(currentPointL+Point2f(-1.f,-1.f),
      //                                                            currentPointL+Point2f(-1.f,
      //                                                            1.f),
      //                                                            currentPointL+Point2f(
      //                                                            1.f,-1.f),
      //                                                            currentPointL+Point2f(
      //                                                            1.f, 1.f)),
      //                                                     1.f,
      //                                                     NamedColors::RED);
      //          _vizManager->DrawGenericQuad(i+3000+(0xffff &
      //          (long)preActionPose.GetMarker()),
      //                                                     Quad2f(currentPointR+Point2f(-1.f,-1.f),
      //                                                            currentPointR+Point2f(-1.f,
      //                                                            1.f),
      //                                                            currentPointR+Point2f(
      //                                                            1.f,-1.f),
      //                                                            currentPointR+Point2f(
      //                                                            1.f, 1.f)),
      //                                                     1.f,
      ...
      for (auto& obstacle : obstacles) {
        // DEBUG VIZ
        //            _vizManager->DrawGenericQuad(obstacle.second.GetValue(),
        //            obstacle.first, 1.f, NamedColors::ORANGE);

        // Make sure this obstacle is not from this object (the one we are
        ...
        if (obstacle.second != this->GetID()) {
          ...
          // bounding quad
          if (boundingQuad.Contains(obstacle.first.ComputeCentroid()) ==
              false) {
      ...
      assert(((currentPoint + stepVec) - xyEnd).Length() <
             (currentPoint - xyEnd).Length());
      assert(((currentPointL + stepVec) - xyEnd).Length() <
             (currentPointL - xyEnd).Length());
      assert(((currentPointR + stepVec) - xyEnd).Length() <
             (currentPointR - xyEnd).Length());
...
bool ActionableObject::GetCurrentPreActionPoses(
    std::vector<PreActionPose>& preActionPoses, const Pose3d& robotPose,
    ...
    const f32 offset_mm, bool visualize) const {
  bool res = false;
  ...
  for (const PreActionPose::ActionType type : withAction) {
    ...
    genPreActionPoses.insert(genPreActionPoses.end(), cachedPoses.begin(),
                             cachedPoses.end());
  }

  for (const auto& preActionPose : genPreActionPoses) {
    if ((withCode.empty() ||
         withCode.count(preActionPose.GetMarker()->GetCode()) > 0) &&
        (withAction.empty() ||
         withAction.count(preActionPose.GetActionType()) > 0)) {
      // offset_mm is scaled by some amount because otherwise it might too far
      // to see the marker it's docking to.
      PreActionPose currentPose(preActionPose, relToObjectPose,
                                preActionPose.GetLineLength(),
                                PREACTION_POSE_OFFSET_SCALAR * offset_mm);

      // If our preActionPoses aren't using an offset then use the point on the
      // preActionLine closest to the robot
      if (offset_mm == 0) {
        // Find the end point of the preActionLine
        const f32 angle =
            currentPose.GetPose().GetRotation().GetAngleAroundZaxis().ToFloat();
        const Point3f endPoint = {
            currentPose.GetPose().GetTranslation().x() +
                cosf(angle) * preActionPose.GetLineLength(),
            currentPose.GetPose().GetTranslation().y() +
                sinf(angle) * preActionPose.GetLineLength(),
        ...
        // x and y difference between the intersection point and the start of
        // the preActionLine
        f32 x = 0;
        f32 y = 0;
        ...
        // Horizontal preActionLine
          ...
          // Find the point on the preActionLine that is closest to the robot's
          // pose
          const f32 m = (yDiff) / (xDiff);
          const f32 b = p.GetTranslation().y() - m * p.GetTranslation().x();

          const f32 b_inv =
              robot.GetTranslation().y() + robot.GetTranslation().x() / m;

          const f32 x_intersect = (b_inv - b) / (m + (1.f / m));
          const f32 y_intersect = -(x_intersect / m) + b_inv;

          // Find the offset the closest point on the line to the robot is from
          // the start of the preActionLine (pose of the preActionPose)
          x = x_intersect - p.GetTranslation().x();
          y = y_intersect - p.GetTranslation().y();
        }

        // Clip the offset so it will stay on the preActionLine
        // Offset will always be positive which is what causes the slightly odd
        // (but desirable) behavior of the preDock pose moving away from the
        // robot when we are infront of the end of the preActionLine closest to
        // the object
        const f32 offset =
            CLIP(sqrtf(x * x + y * y), 0.f, preActionPose.GetLineLength());

        PreActionPose newPose(preActionPose, relToObjectPose,
                              preActionPose.GetLineLength(), offset);
        currentPose = newPose;
      }
      ...
      if (IsPreActionPoseValid(currentPose, obstacles)) {
        ...
        if (visualize) {
          PreActionPose basePose(preActionPose, relToObjectPose,
                                 preActionPose.GetLineLength(), 0);
          Pose3d end = basePose.GetPose();
          const f32 endAngle =
              end.GetRotation().GetAngleAroundZaxis().ToFloat();
          end.SetTranslation(
              {end.GetTranslation().x() -
                   cosf(endAngle) * preActionPose.GetLineLength(),
               end.GetTranslation().y() -
                   sinf(endAngle) * preActionPose.GetLineLength(),
               end.GetTranslation().z()});

          // Arbitrarily add 100 to the pathID so it doesn't conflict with other
          // pathIDs like path planner paths
          u32 id = GetID() + 100 + (count++);
          _vizPreActionLineIDs.insert(id);
          _vizManager->ErasePath(id);
          _vizManager->AppendPathSegmentLine(
              id, basePose.GetPose().GetTranslation().x(),
              basePose.GetPose().GetTranslation().y(), end.GetTranslation().x(),
              end.GetTranslation().y());
...
void ActionableObject::VisualizePreActionPoses(
  ...
  for (PreActionPose::ActionType actionType :
       {PreActionPose::DOCKING, PreActionPose::ENTRY}) {
    GetCurrentPreActionPoses(poses, robotPose, {actionType},
                             std::set<Vision::Marker::Code>(), obstacles, 0,
                             true);
    for (auto& pose : poses) {
      // TODO: In computing poseID to pass to DrawPreDockPose, multiply object
      // ID by the max number of
      //       preaction poses we expect to visualize per object. Currently,
      //       hardcoded to 48 (4 dock and 4 roll per side). We probably won't
      //       have more than this.
      _vizPreActionPoseHandles.emplace(_vizManager->DrawPreDockPose(
          poseID + GetID().GetValue() * 48,
...
void ActionableObject::SetPose(const Pose3d& newPose, f32 fromDistance,
