...
class ActionableObject : public Vector::ObservableObject {
 public:
  ...
  // Returns true if we had to generate preActionPoses, false if cached poses
  // were used return value is currently only used for unit tests
  bool GetCurrentPreActionPoses(
      std::vector<PreActionPose>& preActionPoses, const Pose3d& robotPose,
      const std::set<PreActionPose::ActionType>& withAction =
          std::set<PreActionPose::ActionType>(),
      const std::set<Vision::Marker::Code>& withCode =
          std::set<Vision::Marker::Code>(),
      const std::vector<std::pair<Quad2f, ObjectID> >& obstacles =
          std::vector<std::pair<Quad2f, ObjectID> >(),
      const f32 offset_mm = 0, bool visualize = false) const;
 ...
 protected:
  ...
  virtual bool IsPreActionPoseValid(
 ...
 private:
  ...
  mutable std::array<std::vector<PreActionPose>,
                     PreActionPose::ActionType::NONE>
      _cachedPreActionPoses;
