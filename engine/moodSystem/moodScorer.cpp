...
MoodScorer::MoodScorer(const Json::Value& inJson) : _emotionScorers() {
  ReadFromJson(inJson);
}
...
bool MoodScorer::ReadFromJson(const Json::Value& emotionScorersJson) {
  ...
  for (uint32_t i = 0; i < numEmotionScorers; ++i) {
    const Json::Value& emotionScorerJson =
        emotionScorersJson.get(i, kNullScorerValue);
...
float MoodScorer::EvaluateEmotionScore(const MoodManager& moodManager) const {
  ...
  const uint32_t kTickRangeForDelta =
      60;  // 2 would give any chances between the last 2 updates, but we might
           // want to track more if it takes longer for a running behavior to
           // interrupt and allow this to run

  for (const EmotionScorer& emotionScorer : _emotionScorers) {
    const EmotionType emotionType = emotionScorer.GetEmotionType();
    const float emotionValue = emotionScorer.TrackDeltaScore()
                                   ? moodManager.GetEmotionDeltaRecentTicks(
                                         emotionType, kTickRangeForDelta)
    ...
    if (FLT_NEAR(score, 0.0f)) {
      // any component scoring 0 forces an overal 0 score (e.g. prevent behavior
      // ever being picked)
      return 0.0f;
    } else {
      totalScore += score;
  ...
  const float averageScore =
      (numEmotionsScored > 0) ? (totalScore / float(numEmotionsScored)) : 0.0f;
  return averageScore;
}
