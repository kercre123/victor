...
class MoodManager : public IDependencyManagedComponent<RobotComponentID>,
 ...
 public:
  ...
  using UnreliableComponent<
      BCComponentID>::AdditionalUpdateAccessibleComponents;
  //////
  ...
  // trigger an event at the given time. If no time specified, use
  // GetCurrentTimeInSeconds
  void TriggerEmotionEvent(const std::string& eventName,
                           float currentTimeInSeconds);
  void TriggerEmotionEvent(const std::string& eventName);

  void AddToEmotion(EmotionType emotionType, float baseValue,
  ...
  void SetEmotion(EmotionType emotionType, float value,
                  const char* debugLabel = "SetEmotion");

  // This manager internally listens for ActionCompleted events from the robot,
  // and can use those to trigger emotion events. By default, it listens for any
  // actions that complete, but this function can be used to enable or disable
  // listening for specific actions
  void SetEnableMoodEventOnCompletion(u32 actionTag, bool enable);

  void SetEmotionFixed(EmotionType emotionType, bool fixed) {
    _fixedEmotions[(size_t)emotionType] = fixed;
  }
  bool IsEmotionFixed(EmotionType emotionType) const {
    return _fixedEmotions[(size_t)emotionType];
  }

  // ==================== GetEmotion... ====================

  float GetEmotionValue(EmotionType emotionType) const {
    return GetEmotion(emotionType).GetValue();
  }

  float GetEmotionDeltaRecentTicks(EmotionType emotionType,
                                   uint32_t numTicksBackwards) const {
    return GetEmotion(emotionType).GetDeltaRecentTicks(numTicksBackwards);
  }

  float GetEmotionDeltaRecentSeconds(EmotionType emotionType,
                                     float secondsBackwards) const {
  ...
  bool DidSimpleMoodTransitionThisTick(SimpleMoodType from,
  ...
  // Helper for anything calling functions that require currentTimeInSeconds,
  // where they don't already have access to it
 ...
 private:
  ...
  float UpdateLatestEventTimeAndGetTimeElapsedInSeconds(
      const std::string& eventName, float currentTimeInSeconds);

  float UpdateEventTimeAndCalculateRepetitionPenalty(
      const std::string& eventName, float currentTimeInSeconds);

  void FadeEmotionsToDefault(float delta);

  const Emotion& GetEmotion(EmotionType emotionType) const {
    return GetEmotionByIndex((size_t)emotionType);
  }
  Emotion& GetEmotion(EmotionType emotionType) {
    return GetEmotionByIndex((size_t)emotionType);
  }
  ...
  void SendMoodToWebViz(const CozmoContext* context,
  ...
  using ActionCompletedEventMap =
      std::map<std::pair<RobotActionType, ActionResultCategory>, std::string>;
  ActionCompletedEventMap _actionCompletedEventMap;

  using AudioParameterType = AudioMetaData::GameParameter::ParameterType;
  // map from emotion to audio parameter type to inform audio system of mood
  // parameters
