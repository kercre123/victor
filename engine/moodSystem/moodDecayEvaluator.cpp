...
bool MoodDecayEvaulator::ReadFromJson(const Json::Value& inJson) {
  const std::string& graphTypeStr = JsonTools::ParseString(
      inJson, kGraphTypeKey, "MoodDecayEvaluator.ReadFromJson");
...
bool MoodDecayEvaulator::VerifyDecayGraph(
    const Util::GraphEvaluator2d& newGraph, DecayGraphType graphType,
    bool warnOnErrors) {
  const size_t numNodes = newGraph.GetNumNodes();
  if (numNodes == 0) {
    if (warnOnErrors) {
      PRINT_NAMED_WARNING("VerifyDecayGraph.NoNodes",
                          "Invalid graph has 0 nodes");
    }
    return false;
  }
  ...
  for (size_t i = 0; i < numNodes; ++i) {
    ...
    if (xMustBePositive && node._x < 0.0f) {
      if (warnOnErrors) {
        PRINT_NAMED_WARNING("MoodDecayEvaulator.VerifyDecayGraph.NegativeXNode",
                            "Node[%zu] = (%f, %f) is Negative!", i, node._x,
                            node._y);
      }
      isValid = false;
    }
    if (node._y < 0.0f) {
      if (warnOnErrors) {
        PRINT_NAMED_WARNING("VerifyDecayGraph.NegativeYNode",
                            "Node[%zu] = (%f, %f) is Negative!", i, node._x,
                            node._y);
      }
      isValid = false;
    ...
    if (mustBeDecreasing && (i != 0) && (node._y > lastY)) {
      if (warnOnErrors) {
        PRINT_NAMED_WARNING(
            "VerifyDecayGraph.IncreasingYNode",
            "Node[%zu] = (%f, %f) is has y > than previous (%f)", i, node._x,
            node._y, lastY);
      }
      isValid = false;
    }
...
void MoodDecayEvaulator::SetDecayGraph(const Util::GraphEvaluator2d& newGraph,
...
bool MoodDecayEvaulator::Empty() const {
  const bool emptyGraph =
      (nullptr == _decayGraph) || (_decayGraph->GetNumNodes() == 0);
  return emptyGraph;
}

float MoodDecayEvaulator::EvaluateDecay(float currentValue,
                                        float currentTimeSinceEvent_s,
                                        float deltaTime_s, float& velocity,
                                        float& accel) const {
  if (Empty()) {
    PRINT_NAMED_ERROR("MoodDecayEvaulator.EvaluateDecay.Empty",
                      "trying to evaluate decay, but graph is empty! Returning "
                      "the same value");
    return currentValue;
  }
  ...
  switch (_graphType) {
    case DecayGraphType::TimeRatio: {
      const float prevDecayScalar =
          _decayGraph->EvaluateY(currentTimeSinceEvent_s);
      ...
      const float deltaScalar = Util::IsFltGT(prevDecayScalar, 0.0f)
                                    ? (newDecayScalar / prevDecayScalar)
                                    : newDecayScalar;
      const float newValue = currentValue * deltaScalar;

      const float originalValue = Util::IsFltGT(prevDecayScalar, 0.0f)
                                      ? (currentValue / prevDecayScalar)
                                      : currentValue;
