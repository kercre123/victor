...
StimulationFaceDisplay::StimulationFaceDisplay()
    : IDependencyManagedComponent(this,
                                  RobotComponentID::StimulationFaceDisplay) {}

StimulationFaceDisplay::~StimulationFaceDisplay() {}

void StimulationFaceDisplay::InitDependent(Vector::Robot* robot,
...
void StimulationFaceDisplay::UpdateDependent(
    const RobotCompMap& dependentComps) {
  ...
  const bool inPRDemo =
      context->GetFeatureGate()->IsFeatureEnabled(FeatureType::PRDemo);
  if (inPRDemo) {
    return;
  }

  auto& moodManager = dependentComps.GetComponent<MoodManager>();
  const float stim = Anki::Util::Clamp(
      moodManager.GetEmotionValue(EmotionType::Stimulated), 0.0f, 1.0f);
