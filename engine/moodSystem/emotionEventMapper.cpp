...
EmotionEventMapper::~EmotionEventMapper() { Clear(); }

void EmotionEventMapper::Clear() { ClearEvents(); }
...
const EmotionEvent* EmotionEventMapper::FindEvent(
...
void EmotionEventMapper::AddEvent(EmotionEvent* emotionEvent) {
  ...
  if (it != _events.end()) {
    ...
    PRINT_NAMED_WARNING("EmotionEventMapper.AddEvent.DuplicateKey",
                        "Event %p already exists with key '%s' replacing with "
                        "new event %p '%s'",
                        oldEvent, oldEvent->GetName().c_str(), emotionEvent,
                        eventName.c_str());
    delete oldEvent;
...
bool EmotionEventMapper::LoadEmotionEvents(const Json::Value& inJson) {
  ...
  if (emotionEvents.isNull()) {
    ...
    if (inJson.isNull() || !LoadEmotionEvent(inJson)) {
      PRINT_NAMED_WARNING("EmotionEventMapper.LoadEmotionEvents.MissingValue",
                          "Missing '%s' entry", kEmotionEventsKey);
      return false;
    } else {
  ...
  for (uint32_t i = 0; i < numEvents; ++i) {
    ...
    if (eventJson.isNull() || !LoadEmotionEvent(eventJson)) {
      PRINT_NAMED_WARNING("EmotionEventMapper.LoadEmotionEvents.BadEvent",
                          "Event %u failed to read", i);
      return false;
    }
  }
