 ...
 * Created: 11/16/15
 *
 * Description: Storage for static data (e.g. loaded from files) for the mood
 *system, can be shared amongst multiple Robots / MoodManagers etc.
 *
...
StaticMoodData::StaticMoodData() { InitDecayEvaluators(); }
...
void StaticMoodData::InitDecayEvaluators() {
  for (size_t i = 0; i < (size_t)EmotionType::Count; ++i) {
    ...
    if (decayEvaluator.Empty()) {
      ...
      // NOTE: unfortunately, unit tests depend on this. It shouldn't make a
      // difference in production at all
      ...
      const auto defaultDecayType =
          MoodDecayEvaulator::DecayGraphType::TimeRatio;
      if (ANKI_VERIFY(MoodDecayEvaulator::VerifyDecayGraph(
                          decayGraph, defaultDecayType, true),
                      "StaticMoodData.Init.InvalidDefaultDecayGraph",
                      "default static-constructed mood graph is invalid! This "
                      "is a bug")) {
...
bool StaticMoodData::SetDecayEvaluator(
    EmotionType emotionType, const Util::GraphEvaluator2d& newGraph,
...
const MoodDecayEvaulator& StaticMoodData::GetDecayEvaluator(
    EmotionType emotionType) const {
  const size_t index = (size_t)emotionType;
  DEV_ASSERT((index >= 0) && (index < (size_t)EmotionType::Count),
             "StaticMoodData.GetDecayEvaluator.InvalidIndex");
...
bool StaticMoodData::ReadFromJson(const Json::Value& inJson) {
  ...
  if (decayGraphsJson.isNull()) {
    PRINT_NAMED_WARNING("StaticMoodData.ReadFromJson.MissingValue",
                        "Missing '%s' entry", kDecayGraphsKey);
    return false;
  }
  ...
  for (uint32_t i = 0; i < decayGraphsJson.size(); ++i) {
    ...
    const char* emotionTypeString =
        graphEmotionTypeName.isString() ? graphEmotionTypeName.asCString() : "";
    ...
    if (strcmp(emotionTypeString, "default") == 0) {
      ...
      if (emotionType < EmotionType::Count) {
        if (_emotionDecayEvaluators[(size_t)emotionType].ReadFromJson(
                decayGraphJson)) {
          decayGraphsSet[(size_t)emotionType] = true;
        } else {
          PRINT_NAMED_WARNING("StaticMoodData.ReadFromJson.InvalidDecayGraph",
                              "DecayGraph %u '%s' failed to set", i,
                              emotionTypeString);
          return false;
        }
      } else {
        PRINT_NAMED_WARNING(
            "StaticMoodData.ReadFromJson.BadDecayGraphEmotionType",
            "DecayGraph %u failed to read - unknown type name '%s'", i,
            emotionTypeString);
        return false;
      }
    }
  ...
  if (emotionEventsJson.isNull() ||
      !_emotionEventMapper.ReadFromJson(emotionEventsJson)) {
    PRINT_NAMED_WARNING("StaticMoodData.ReadFromJson.BadEventMapper",
                        "EventMapper '%s' failed to read", kEventMapperKey);
    return false;
  }

  const Json::Value& defaultRepetitionPenaltyJson =
      inJson[kDefaultRepetitionPenaltyKey];
  if (!defaultRepetitionPenaltyJson.isNull()) {
    if (!_defaultRepetitionPenalty.ReadFromJson(defaultRepetitionPenaltyJson)) {
      PRINT_NAMED_WARNING(
          "StaticMoodData.ReadFromJson.BadDefaultRepetitionPenalty",
          "'%s' failed to read", kDefaultRepetitionPenaltyKey);
    }
  }
  ...
  if (!valueRangesJson.isNull()) {
    for (const auto& entry : valueRangesJson) {
      const std::string& emotionStr =
          JsonTools::ParseString(entry, kEmotionTypeKey, kJsonDebugName);
      const EmotionType emotion = EmotionTypeFromString(emotionStr);

      const float minVal =
          JsonTools::ParseFloat(entry, kMinKey, kJsonDebugName);
      const float maxVal =
          JsonTools::ParseFloat(entry, kMaxKey, kJsonDebugName);
      if (ANKI_VERIFY(minVal <= maxVal,
                      "StaticMoodData.ReadFromJson.BadValueRange",
                      "Value range for emotion '%s' in invalid: %f - %f",
                      emotionStr.c_str(), minVal, maxVal)) {
  ...
  if (_defaultRepetitionPenalty.GetNumNodes() == 0) {
    PRINT_NAMED_WARNING(
        "StaticMoodData.ReadFromJson.EmptyDefaultRepetitionPenalty",
        "'%s' missing or bad - defaulting to no penalty",
        kDefaultRepetitionPenaltyKey);
