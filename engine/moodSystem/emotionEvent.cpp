...
EmotionEvent::EmotionEvent() : _affectors(), _name() {}
...
void EmotionEvent::AddEmotionAffector(const EmotionAffector& inAffector) {
  ...
  for (const EmotionAffector& affector : _affectors) {
    if (affector.GetType() == inAffector.GetType()) {
      PRINT_NAMED_WARNING(
          "EmotionEvent.AddEmotionAffector.DuplicateType",
          "Type '%s' already present (%f vs %f) - adding anyway!",
          EnumToString(inAffector.GetType()), inAffector.GetValue(),
          affector.GetValue());
    }
  }
...
float EmotionEvent::CalculateRepetitionPenalty(
    float timeSinceLastOccurrence,
    const Util::GraphEvaluator2d& defaultPenalty) const {
  const float repetitionPenalty =
      _repetitionPenalty.Empty()
...
bool EmotionEvent::ReadFromJson(const Json::Value& inJson) {
  ...
  if (!eventNameField.isString()) {
    PRINT_NAMED_WARNING("EmotionEvent.ReadFromJson.MissingName",
                        "Missing '%s' string entry", kEventName);
    return false;
  }
  ...
  if (emotionAffectors.isNull()) {
    PRINT_NAMED_WARNING("EmotionEvent.ReadFromJson.MissingValue",
                        "Missing '%s' entry", kEmotionAffectorsKey);
    return false;
  }
  ...
  for (uint32_t i = 0; i < numAffectors; ++i) {
    const Json::Value& affectorJson =
        emotionAffectors.get(i, kNullAffectorValue);

    EmotionAffector newAffector;
    if (affectorJson.isNull() || !newAffector.ReadFromJson(affectorJson)) {
      PRINT_NAMED_WARNING("EmotionEvent.ReadFromJson.BadAffector",
                          "Affector %u failed to read", i);
      return false;
    }
  ...
  if (!repetitionPenaltyJson.isNull()) {
    if (!_repetitionPenalty.ReadFromJson(repetitionPenaltyJson)) {
      PRINT_NAMED_WARNING("EmotionEvent.ReadFromJson.BadRepetitionPenalty",
                          "Behavior '%s': %s failed to read", _name.c_str(),
