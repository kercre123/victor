...
class Emotion {
 public:
  ...
  float GetDeltaRecentTicks(uint32_t numTicksBackwards) const {
    return _value - GetHistoryValueTicksAgo(numTicksBackwards);
  }
  float GetDeltaRecentSeconds(float secondsBackwards) const {
    return _value - GetHistoryValueSecondsAgo(secondsBackwards);
  }
  ...
  struct HistorySample {
    explicit HistorySample(float value = 0.0f, float timeDelta = 0.0f)
        : _value(value), _timeDelta(timeDelta) {}
    float _value;
    float _timeDelta;
  };
