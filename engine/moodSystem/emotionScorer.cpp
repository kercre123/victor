...
EmotionScorer::EmotionScorer(const Json::Value& inJson)
    : _emotionType(EmotionType::Count), _scoreGraph(), _trackDeltaScore(false) {
  ReadFromJson(inJson);
}
...
bool EmotionScorer::ReadFromJson(const Json::Value& inJson) {
  ...
  const char* emotionTypeString =
      emotionType.isString() ? emotionType.asCString() : "";
  _emotionType = EmotionTypeFromString(emotionTypeString);

  if (_emotionType == EmotionType::Count) {
    PRINT_NAMED_WARNING("EmotionScorer.ReadFromJson.BadType", "Bad '%s' = '%s'",
                        kEmotionTypeKey, emotionTypeString);
    return false;
  }

  if (!_scoreGraph.ReadFromJson(scoreGraph)) {
    PRINT_NAMED_WARNING("EmotionScorer.ReadFromJson.BadScoreGraph",
                        "Bad '%s' failed to parse", kScoreGraphKey);
    return false;
  }

  if (trackDelta.isNull()) {
    PRINT_NAMED_WARNING("EmotionScorer.ReadFromJson.MissingTrackDelta",
                        "Missing '%s' entry", kTrackDeltaKey);
    return false;
  }
