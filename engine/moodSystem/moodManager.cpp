...
// For now StaticMoodData is basically a singleton, but hidden behind an
// interface in mood manager incase we ever need it to be different per robot /
// moodManager
...
StaticMoodData& MoodManager::GetStaticMoodData() { return sStaticMoodData; }
...
MoodManager::MoodManager()
    : IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::MoodManager),
      UnreliableComponent<BCComponentID>(this, BCComponentID::MoodManager),
      _lastUpdateTime(0.0f),
      _fixedEmotions{},
      _simpleMoodAudioParameter(AudioParameterType::Invalid) {
  _pendingAppEvents.reserve(
      3);  // ~max expected events in a single Update() tick
}

MoodManager::~MoodManager() {
  // if the robot is destructing, it might not have an action list, so check
  // that here
  if (_actionCallbackID != 0 && _robot != nullptr &&
      _robot->HasComponent<ActionList>()) {
    _robot->GetActionList().UnregisterCallback(_actionCallbackID);
    _actionCallbackID = 0;
  }
}

void MoodManager::InitDependent(Vector::Robot* robot,
...
void MoodManager::ReadMoodConfig(const Json::Value& inJson) {
  ...
  for (const auto& valueRangeEntry :
       GetStaticMoodData().GetEmotionValueRangeMap()) {
  ...
  if (nullptr != _robot) {
    ...
    _actionCallbackID =
        _robot->GetActionList().RegisterActionEndedCallbackForAllActions(
            std::bind(&MoodManager::HandleActionEnded, this,
                      std::placeholders::_1));
  }
}
...
void MoodManager::VerifyAudioEvents() const {
  ...
  for (const auto& mapPair : _audioParameterMap) {
    ...
    ANKI_VERIFY(result.second,
                ...
                kAudioParametersMapKey, EnumToString(mapPair.second));
  }
  ...
  if (_simpleMoodAudioParameter != AudioParameterType::Invalid) {
    ...
    ANKI_VERIFY(result.second,
                ...
                "config '%s' specifies the same event '%s' as is used in the "
                "emotion map",
                kSimpleMoodAudioKey, EnumToString(_simpleMoodAudioParameter));
  }
}

void MoodManager::LoadEmotionEvents(
    const RobotDataLoader::FileJsonMap& emotionEventData) {
  for (const auto& fileJsonPair : emotionEventData) {
    ...
    if (!eventJson.empty() && LoadEmotionEvents(eventJson)) {
      // PRINT_CH_DEBUG("Mood", "MoodManager.LoadEmotionEvents", "Loaded '%s'",
      // filename.c_str());
    } else {
      PRINT_NAMED_WARNING("MoodManager.LoadEmotionEvents",
...
void MoodManager::LoadAudioParameterMap(const Json::Value& inJson) {
  ...
  if (ANKI_VERIFY(!inJson.isArray(),
                  "MoodManager.LoadAudioParameterMap.MissingKey",
    ...
    for (auto mapIt = inJson.begin(); mapIt != inJson.end(); ++mapIt) {
      ...
      if (ANKI_VERIFY(EmotionTypeFromString(mapIt.key().asCString(), emo),
        ...
        if (ANKI_VERIFY(
                AudioMetaData::GameParameter::ParameterTypeFromString(
                    mapIt->asCString(), audioParam),
...
void MoodManager::LoadAudioSimpleMoodMap(const Json::Value& inJson) {
  ...
  const std::string& audioParamStr = JsonTools::ParseString(
      inJson, "event", "MoodManager.AudioSimpleMoodMap.ConfigError");

  ANKI_VERIFY(AudioMetaData::GameParameter::ParameterTypeFromString(
                  audioParamStr, _simpleMoodAudioParameter),
  ...
  for (auto mapIt = mapJson.begin(); mapIt != mapJson.end(); ++mapIt) {
    ...
    if (ANKI_VERIFY(
            SimpleMoodTypeFromString(mapIt.key().asCString(), simpleMood),
...
void MoodManager::LoadActionCompletedEventMap(const Json::Value& inJson) {
    ...
    for (auto actionIt = inJson.begin(); actionIt != inJson.end(); ++actionIt) {
      if (!actionIt->isNull()) {
        RobotActionType actionType =
            RobotActionTypeFromString(actionIt.key().asCString());

        for (auto resultIt = actionIt->begin(); resultIt != actionIt->end();
             ++resultIt) {
          if (!resultIt->isNull()) {
            ActionResultCategory resultCategory =
                ActionResultCategoryFromString(resultIt.key().asCString());
            _actionCompletedEventMap.insert(
                {{actionType, resultCategory}, resultIt->asString()});
          }
        }
      }
    }
  ...
  PRINT_CH_INFO(
      "Mood", "MoodManager.LoadedEventMap",
...
void MoodManager::PrintActionCompletedEventMap() const {
  PRINT_CH_INFO("Mood", "MoodManager.PrintActionCompletedEventMap",
                "action result event map follows:");

  for (const auto& it : _actionCompletedEventMap) {
    PRINT_CH_INFO("Mood", "MoodManager.PrintActionCompletedEventMap",
                  "%20s %15s %s", RobotActionTypeToString(it.first.first),
...
void MoodManager::UpdateDependent(const RobotCompMap& dependentComps) {
  ...
  const float kMinTimeStep =
      0.0001f;  // minimal sensible timestep, should be at least > epsilon
  float timeDelta = (_lastUpdateTime != 0.0f) ? (currentTime - _lastUpdateTime)
                                              : kMinTimeStep;
  if (timeDelta < kMinTimeStep) {
    PRINT_NAMED_WARNING("MoodManager.BadTimeStep",
                        "TimeStep %f (%f-%f) is < %f - clamping!", timeDelta,
                        currentTime, _lastUpdateTime, kMinTimeStep);
  ...
  for (size_t i = 0; i < (size_t)EmotionType::Count; ++i) {
    ...
    if (!IsEmotionFixed(emotionType)) {
      emotion.Update(GetStaticMoodData().GetDecayEvaluator(emotionType),
                     timeDelta, rate, accel);
    }
  ...
  const bool hasAudioComp =
      dependentComps.HasComponent<Audio::EngineRobotAudioClient>();
  if (hasAudioComp &&
      ((currentTime - _lastAudioSendTime_s) > kMoodManager_AudioSendPeriod_s)) {
    SendEmotionsToAudio(
        dependentComps.GetComponent<Audio::EngineRobotAudioClient>());
  }
  ...
  if (dependentComps.HasComponent<RobotStatsTracker>()) {
    ...
    if (delta > 0.0f) {
      dependentComps.GetComponent<RobotStatsTracker>()
          .IncreaseStimulationSeconds(delta);
    }

    if (_cumlPosStimDeltaToAdd > 0.0) {
      dependentComps.GetComponent<RobotStatsTracker>()
          .IncreaseStimulationCumulativePositiveDelta(_cumlPosStimDeltaToAdd);
  ...
  if (!_pendingAppEvents.empty() ||
      (currentTime - _lastAppSentStimTime_s >= kMoodManager_AppPeriod_s)) {
    // this won't send a new stim rate/accel the moment it changes, but since
    // this is currently the only user-facing viz of stim, that's ok. It _will_
    // send a new stim/rate accel when an event affects it, and periodically
  ...
  if (simpleMood != _lastSimpleMood) {
    ...
    if (_lastSimpleMood != SimpleMoodType::Count) {
      ...
      DASMSG(
          simple_mood_transition, "mood.simple_mood_transition",
      ...
      DASMSG_SET(s2, SimpleMoodTypeToString(_lastSimpleMood),
                 "Old simple mood");
      DASMSG_SET(i1, (int)delta_s,
                 "time spent in previous simple mood (in seconds)");
      DASMSG_SEND();
    }
...
void MoodManager::SendMoodToWebViz(const CozmoContext* context,
...
void MoodManager::HandleActionEnded(
    const ExternalInterface::RobotCompletedAction& completion) {
  ...
  ActionResultCategory resultCategory =
      IActionRunner::GetActionResultCategory(completion.result);

  auto it =
      _actionCompletedEventMap.find({completion.actionType, resultCategory});
  if (it != _actionCompletedEventMap.end()) {
    PRINT_CH_DEBUG("Mood", "MoodManager.ActionCompleted.Reaction",
                   "Reacting to action of type '%s' completion with category "
                   "'%s' by triggering event '%s'",
...
void MoodManager::HandleMessage(const ExternalInterface::MoodMessage& msg) {
  const Anki::Vector::ExternalInterface::MoodMessageUnion& moodMessage =
      msg.MoodMessageUnion;
  switch (moodMessage.GetTag()) {
    ...
    case ExternalInterface::MoodMessageUnionTag::SetEmotion: {
      const Anki::Vector::ExternalInterface::SetEmotion& msg =
          moodMessage.Get_SetEmotion();
      SetEmotion(msg.emotionType, msg.newVal);
      break;
    }
    case ExternalInterface::MoodMessageUnionTag::AddToEmotion: {
      const Anki::Vector::ExternalInterface::AddToEmotion& msg =
          moodMessage.Get_AddToEmotion();
    ...
    case ExternalInterface::MoodMessageUnionTag::TriggerEmotionEvent: {
      const Anki::Vector::ExternalInterface::TriggerEmotionEvent& msg =
          moodMessage.Get_TriggerEmotionEvent();
      TriggerEmotionEvent(msg.emotionEventName, GetCurrentTimeInSeconds());
      break;
    }
    default:
      PRINT_NAMED_ERROR("MoodManager.HandleMoodEvent.UnhandledMessageUnionTag",
                        "Unexpected tag %u", (uint32_t)moodMessage.GetTag());
      assert(0);
  }
}
...
void MoodManager::SendEmotionsToGame() {
  ANKI_CPU_PROFILE(
      "MoodManager::SendEmotionsToGame");  // ~1ms per tick (iPhone 5S in
                                           // Release) - could send every N
                                           // ticks?
  ...
  if (_robot) {
    ...
    _robot->Broadcast(
        ExternalInterface::MessageEngineToGame(std::move(message)));
  }
}

void MoodManager::SendEmotionsToAudio(
    Audio::EngineRobotAudioClient& audioClient) {
  _lastAudioSendTime_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
...
void MoodManager::SendStimToApp(float velocity, float accel) {
  ...
  if ((_robot != nullptr) && _robot->HasGatewayInterface()) {
    ...
    external_interface::StimulationInfo* msg =
        new external_interface::StimulationInfo;
    // todo: extend proto plugin to take repeated types so this can be done in
    // the ctor, to ensure we fill out all params
    *msg->mutable_emotion_events() = {_pendingAppEvents.begin(),
                                      _pendingAppEvents.end()};
    msg->set_value(value);
    msg->set_velocity(velocity);
    msg->set_accel(accel);
    msg->set_value_before_event(_pendingAppEvents.empty() ? value
                                                          : _lastStimValue);
...
// updates the most recent time this event was triggered, and returns how long
// it's been since the event was last seen returns FLT_MAX if this is the first
// time the event has been seen
float MoodManager::UpdateLatestEventTimeAndGetTimeElapsedInSeconds(
    const std::string& eventName, float currentTimeInSeconds) {
  auto newEntry = _moodEventTimes.insert(
      MoodEventTimes::value_type(eventName, currentTimeInSeconds));

  if (newEntry.second) {
    // first time event has occurred, map insert has successfully updated the
    // time seen
    return FLT_MAX;
  } else {
    // event has happened before - calculate time since it last occurred and the
    // matching penalty, then update the time

    float& timeEventLastOccurred = newEntry.first->second;
    const float timeSinceLastOccurrence =
        Util::numeric_cast<float>(currentTimeInSeconds - timeEventLastOccurred);
...
float MoodManager::UpdateEventTimeAndCalculateRepetitionPenalty(
    const std::string& eventName, float currentTimeInSeconds) {
  const float timeSinceLastOccurrence =
      UpdateLatestEventTimeAndGetTimeElapsedInSeconds(eventName,
                                                      currentTimeInSeconds);

  const EmotionEvent* emotionEvent =
      GetStaticMoodData().GetEmotionEventMapper().FindEvent(eventName);

  if (emotionEvent) {
    // Use the emotionEvent with the matching name for calculating the
    // repetition penalty
    const auto& defaultPenalty =
        GetStaticMoodData().GetDefaultRepetitionPenalty();
    const float repetitionPenalty = emotionEvent->CalculateRepetitionPenalty(
        timeSinceLastOccurrence, defaultPenalty);
    return repetitionPenalty;
  } else {
    PRINT_NAMED_WARNING(
        "MoodManager.UpdateEventTimeAndCalculateRepetitionPenalty.NoEvent",
    ...
    const float repetitionPenalty =
        GetStaticMoodData().GetDefaultRepetitionPenalty().EvaluateY(
...
void MoodManager::TriggerEmotionEvent(const std::string& eventName,
  ...
  if (emotionEvent) {
    ...
    if (ANKI_DEV_CHEATS && kMoodManager_WebVizPeriod_s >= 0.0f &&
        nullptr != _robot) {
    ...
    const std::vector<EmotionAffector>& emotionAffectors =
        emotionEvent->GetAffectors();
    for (const EmotionAffector& emotionAffector : emotionAffectors) {
      const float penalizedDeltaValue =
          emotionAffector.GetValue() * repetitionPenalty;
      if (!IsEmotionFixed(emotionAffector.GetType())) {
        ...
        eventEmotionDeltas[emotionAffector.GetType()] =
            std::round((after - before) * 1000);
        ...
        PRINT_CH_INFO(
            "Mood", "MoodManager.TriggerFixedEmotion",
    ...
    if (modified) {
      if (_robot) {
        ...
        SendEmotionsToAudio(
            _robot->GetComponent<Audio::EngineRobotAudioClient>());
      }

      DASMSG(mood_event, "mood.event", "An emotion event triggered");
      DASMSG_SET(s1, eventName, "name of the emotion event (json defined)");
      if (eventEmotionDeltas.find(EmotionType::Stimulated) !=
          eventEmotionDeltas.end()) {
        DASMSG_SET(i1, eventEmotionDeltas[EmotionType::Stimulated],
                   "Stimulated delta * 1000");
      }
      if (eventEmotionDeltas.find(EmotionType::Confident) !=
          eventEmotionDeltas.end()) {
        DASMSG_SET(i2, eventEmotionDeltas[EmotionType::Confident],
                   "Confident delta * 1000");
      }
      if (eventEmotionDeltas.find(EmotionType::Social) !=
          eventEmotionDeltas.end()) {
        DASMSG_SET(i3, eventEmotionDeltas[EmotionType::Social],
                   "Social delta * 1000");
      }
      if (eventEmotionDeltas.find(EmotionType::Happy) !=
          eventEmotionDeltas.end()) {
        DASMSG_SET(i4, eventEmotionDeltas[EmotionType::Happy],
                   "Happy delta * 1000");
      }
      DASMSG_SEND();
    ...
    PRINT_NAMED_ERROR("MoodManager.TriggerEmotionEvent.EventNotFound",
                      "Failed to find event '%s'", eventName.c_str());
  }
}

void MoodManager::AddToEmotion(EmotionType emotionType, float baseValue,
                               const char* uniqueIdString,
                               float currentTimeInSeconds) {
  if (!IsEmotionFixed(emotionType)) {
    const float repetitionPenalty =
        UpdateEventTimeAndCalculateRepetitionPenalty(uniqueIdString,
                                                     currentTimeInSeconds);
...
void MoodManager::AddToEmotions(EmotionType emotionType1, float baseValue1,
                                ...
                                const char* uniqueIdString,
                                float currentTimeInSeconds) {
  const float repetitionPenalty = UpdateEventTimeAndCalculateRepetitionPenalty(
      uniqueIdString, currentTimeInSeconds);
  bool modified = false;
  ...
  if (!modified) {
    PRINT_CH_INFO(
        "Mood", "MoodManager.AddToFixedEmotions2",
...
void MoodManager::AddToEmotions(EmotionType emotionType1, float baseValue1,
  ...
  if (!modified) {
    PRINT_CH_INFO(
        "Mood", "MoodManager.AddToFixedEmotions3",
        "AddToEmotions use with emotions '%s','%s','%s' that are fixed = "
        "%d,%d,%d",
        EmotionTypeToString(emotionType1), EmotionTypeToString(emotionType2),
        EmotionTypeToString(emotionType3), IsEmotionFixed(emotionType1),
        IsEmotionFixed(emotionType2), IsEmotionFixed(emotionType3));
  }
}
...
SimpleMoodType MoodManager::GetSimpleMood(float stimulated, float confident) {
  ...
  // Note: never returns Default (default is used by the animation group
  // selector if more specific anims aren't provided)
}

bool MoodManager::DidSimpleMoodTransitionThisTick(SimpleMoodType from,
                                                  SimpleMoodType to) const {
  bool ret = false;
  SimpleMoodType currMood = GetSimpleMood();
  if (currMood == to) {
    const float prevStim =
        GetEmotion(EmotionType::Stimulated).GetHistoryValueTicksAgo(1);
    const float prevConf =
        GetEmotion(EmotionType::Confident).GetHistoryValueTicksAgo(1);
...
bool MoodManager::DidSimpleMoodTransitionThisTickFrom(
    SimpleMoodType from) const {
  const float prevStim =
      GetEmotion(EmotionType::Stimulated).GetHistoryValueTicksAgo(1);
  const float prevConf =
      GetEmotion(EmotionType::Confident).GetHistoryValueTicksAgo(1);
...
void MoodManager::SubscribeToWebViz() {
  ...
  auto onSubscribedBehaviors = [](const std::function<void(const Json::Value&)>&
                                      sendToClient) {
    // a client subscribed. send them min/max values for each emotion, and a
    // list of SimpleMoods and arbitrary emotion values for each SimpleMood
    ...
    // some example values that, if received, would transition to the given
    // simple mood. only some emotions are provided, so that only those will be
    // sent back, leaving others unchanged
    ...
    {
      static_assert(
          SimpleMoodTypeNumEntries == 6,
          "Please update the values above with the new simple moods.");
      const auto moodList = {SimpleMoodType::LowStim, SimpleMoodType::MedStim,
                             SimpleMoodType::HighStim,
                             SimpleMoodType::Frustrated};
      for (const auto& simpleMood : moodList) {
        const float stim =
            simpleMoods[SimpleMoodTypeToString(simpleMood)]["Stimulated"]
                    .isDouble()
                ? simpleMoods[SimpleMoodTypeToString(simpleMood)]["Stimulated"]
                      .asDouble()
                : 0.0f;
        const float conf =
            simpleMoods[SimpleMoodTypeToString(simpleMood)]["Confident"]
                    .isDouble()
                ? simpleMoods[SimpleMoodTypeToString(simpleMood)]["Confident"]
                      .asDouble()
                : 0.0f;
        ANKI_VERIFY(
            (GetSimpleMood(stim, conf) == simpleMood) &&
                (stim >= GetStaticMoodData()
                             .GetEmotionValueRangeMap()
                             .at(EmotionType::Stimulated)
                             .first) &&
                (stim <= GetStaticMoodData()
                             .GetEmotionValueRangeMap()
                             .at(EmotionType::Stimulated)
  ...
  _signalHandles.emplace_back(webService->OnWebVizSubscribed(kWebVizModuleName)
                                  .ScopedSubscribe(onSubscribedBehaviors));
  _signalHandles.emplace_back(webService->OnWebVizData(kWebVizModuleName)
                                  .ScopedSubscribe(onDataBehaviors));
}
