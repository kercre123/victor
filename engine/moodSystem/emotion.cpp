 ...
 * Created: 10/14/15
 *
 * Description: Tracks a single emotion (which together make up the mood for a
 *Cozmo Robot)
 *
...
float Emotion::GetHistoryValueTicksAgo(uint32_t numTicksBackwards) const {
  ...
  if ((numEntries > 0) && (numTicksBackwards > 0)) {
    ++numTicksBackwards;  // so that this method name is accurate, skip the last
                          // entry in _history, which is the current _value
    const uint32_t sampleIndex =
        (numEntries > numTicksBackwards) ? (numEntries - numTicksBackwards) : 0;
...
float Emotion::GetHistoryValueSecondsAgo(float secondsBackwards) const {
  ...
  if ((numEntries > 0) && (secondsBackwards > 0.0f)) {
    float timeRewound =
        0.0f;  // ideally this would be "time since last sample added" but we
               // don't know that, so assume 0 (it must be < 1 tick)
    ...
    for (uint32_t i = numEntries; i > 0; --i) {
      ...
      // Note: This sample says how long it was since the previous one, so we
      // only increment here (not before the test above)
...
void Emotion::Update(const MoodDecayEvaulator& evaluator, float timeDelta_s,
  ...
  _value = evaluator.EvaluateDecay(_value, _timeDecaying, timeDelta_s, velocity,
                                   accel);
...
void Emotion::Add(float penalizedDeltaValue) {
  ...
  const float newValue =
      Anki::Util::Clamp(_value + penalizedDeltaValue, _minValue, _maxValue);
  _value = newValue;

  const bool isSufficentChangeToResetDecay =
      (Util::Abs(penalizedDeltaValue) >
       0.05f);  // penalty can prevent reset (if it scales value too far), but
                // clamping at min/max cannot prevent it
  const bool isChangeInOppositeDirectionToDecay =
      ((oldValue >= 0.0f) ==
       (penalizedDeltaValue >=
        0.0f));  // only true if penalizedValue is driving value away from zero

  if (isSufficentChangeToResetDecay && isChangeInOppositeDirectionToDecay) {
    // Reset the decay (e.g. if +ve value was dropping back to 0 but we've just
    // awarded a +ve score)
    _timeDecaying = 0.0f;
...
void Emotion::SetEmotionValueRange(float min, float max) {
  ANKI_VERIFY(min <= max, "Emotion.SetEmotionValueRange.Invalid",
              "Invalid range %f - %f", min, max);
  _minValue = min;
  _maxValue = max;
}
