...
// the Basestation frequency since that's what defines how often Update() is
// called.
...
class ConnectedDeviceInfo {
 ...
 private:
  // Note: in/out Client ptrs can be identical (if in and out are on the same
  // port)
  UdpClient* _inClient;
...
class MultiClientComms : public Comms::IComms {
 public:
  ...
  Result Init(const char* advertisingHostIP, int advertisingPort,
              unsigned int maxSentBytesPerTic = 0);
  ...
  u32 GetNumAdvertisingDevices() const {
    return (u32)advertisingDevices_.size();
  }
 ...
 private:
  ...
  using advertisingDevicesIt_t =
      std::map<int, DeviceConnectionInfo_t>::iterator;
  ...
  // 'Queue' of received messages from all connected devices with their received
  // times.
