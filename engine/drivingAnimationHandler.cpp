...
DrivingAnimationHandler::DrivingAnimationHandler()
    : IDependencyManagedComponent(this,
      ...
      _moodBasedDrivingAnims(
          {{SimpleMoodType::Default,
            {AnimationTrigger::DriveStartDefault,
             AnimationTrigger::DriveLoopDefault,
             AnimationTrigger::DriveEndDefault, AnimationTrigger::PlanningGetIn,
             AnimationTrigger::PlanningLoop, AnimationTrigger::PlanningGetOut}},
           {SimpleMoodType::HighStim,
            {AnimationTrigger::DriveStartHappy,
             AnimationTrigger::DriveLoopHappy, AnimationTrigger::DriveEndHappy,
             AnimationTrigger::PlanningGetIn, AnimationTrigger::PlanningLoop,
             AnimationTrigger::PlanningGetOut}},
           {SimpleMoodType::Frustrated,
            {AnimationTrigger::DriveStartAngry,
             AnimationTrigger::DriveLoopAngry, AnimationTrigger::DriveEndAngry,
             AnimationTrigger::PlanningGetIn, AnimationTrigger::PlanningLoop,
             AnimationTrigger::PlanningGetOut}}}),
...
void DrivingAnimationHandler::InitDependent(
  ...
  if (_robot->HasExternalInterface()) {
    ...
    _signalHandles.push_back(_robot->GetExternalInterface()->Subscribe(
        ...
        [this](const AnkiEvent<ExternalInterface::MessageGameToEngine>& event) {
          ...
          PushDrivingAnimations(
              {payload.drivingStartAnim, payload.drivingLoopAnim,
               payload.drivingEndAnim},
              payload.lockName);
        }));
    ...
    _signalHandles.push_back(_robot->GetExternalInterface()->Subscribe(
        ...
        [this](const AnkiEvent<ExternalInterface::MessageGameToEngine>& event) {
          RemoveDrivingAnimations(
              event.GetData().Get_RemoveDrivingAnimations().lockName);
        }));
  }
}

void DrivingAnimationHandler::PushDrivingAnimations(
    const DrivingAnimations& drivingAnimations, const std::string& lockName) {
...
void DrivingAnimationHandler::RemoveDrivingAnimations(
  ...
  if (_drivingAnimationStack.empty()) {
    ...
    // find the driving animation with the matching lock name in the driving
    // animation stack (top down)
    auto drivingAnimReverseIter = std::find_if(
        _drivingAnimationStack.rbegin(), _drivingAnimationStack.rend(),
        [&lockName](
            const std::pair<DrivingAnimations, std::string>& stackEntry) {
    ...
    if (drivingAnimReverseIter != _drivingAnimationStack.rend()) {
      ...
      PRINT_NAMED_WARNING(
          "DrivingAnimationHandler.RemoveDrivingAnimations.NotFound",
          "Could not find driving animation with name '%s'", lockName.c_str());
    }
  }
}
...
void DrivingAnimationHandler::UpdateCurrDrivingAnimations() {
  if (_drivingAnimationStack.empty()) {
    ...
    auto it =
        _moodBasedDrivingAnims.find(_robot->GetMoodManager().GetSimpleMood());
    ...
    if (ANKI_VERIFY(it != _moodBasedDrivingAnims.end(),
                    "DrivingAnimationHandler.UpdateCurrDrivingAnimations."
                    "MoodBased.Missing",
...
void DrivingAnimationHandler::HandleActionCompleted(
  ...
  if (msg.idTag == _drivingStartAnimTag &&
    ...
    const bool stillDrivingPath = pathComponent.HasPathToFollow() &&
                                  !pathComponent.HasStoppedBeforeExecuting();
    ...
    if (_isActionLockingTracks) {
      _robot->GetMoveComponent().LockTracks(_tracksToUnlock, _actionTag,
                                            "DrivingAnimations");
    }
  } else if (msg.idTag == _planningStartAnimTag &&
             msg.result == ActionResult::SUCCESS) {
    const bool playLooping = !pathComponent.IsPlanReady();
    if (playLooping &&
        _currDrivingAnimations.planningLoopAnim != AnimationTrigger::Count) {
...
void DrivingAnimationHandler::Init(const u8 tracksToUnlock, const u32 tag,
...
void DrivingAnimationHandler::StartPlanningAnim() {
  ...
  // Don't do anything until Init is called, or until the previous driving
  // animation has stopped (this can happen during replanning)
  ...
  } else if (_currDrivingAnimations.planningLoopAnim !=
...
bool DrivingAnimationHandler::EndPlanningAnim() {
  ...
  // If we are currently playing the end anim or have already completed it don't
  // play it again
  if (_state == AnimState::PlanningEnd ||
      _state == AnimState::FinishedPlanning) {
    return false;
  }
...
void DrivingAnimationHandler::StartDrivingAnim() {
  ...
  // Don't do anything until Init is called, or it finished the last driving
  // animation, or the planning animation ends
  ...
  } else if (_currDrivingAnimations.drivingLoopAnim !=
             AnimationTrigger::Count) {
    PlayDrivingLoopAnim();
  }
}
...
bool DrivingAnimationHandler::EndDrivingAnim() {
  ...
  // If we are currently playing the end anim or have already completed it don't
  // play it again
  if (_state == AnimState::DrivingEnd || _state == AnimState::FinishedDriving ||
      _state == AnimState::ActionDestroyed) {
    return false;
  }
...
void DrivingAnimationHandler::PlayDrivingStartAnim() {
  ...
  IActionRunner* animAction = new TriggerLiftSafeAnimationAction(
      _currDrivingAnimations.drivingStartAnim, 1, true);
  _drivingStartAnimTag = animAction->GetTag();
  _robot->GetActionList().QueueAction(QueueActionPosition::IN_PARALLEL,
                                      animAction);
}
...
void DrivingAnimationHandler::PlayDrivingLoopAnim() {
  ...
  IActionRunner* animAction = new TriggerLiftSafeAnimationAction(
      _currDrivingAnimations.drivingLoopAnim, 1, true);
...
void DrivingAnimationHandler::PlayDrivingEndAnim() {
  ...
  IActionRunner* animAction = new TriggerLiftSafeAnimationAction(
      _currDrivingAnimations.drivingEndAnim, 1, true);
...
void DrivingAnimationHandler::PlayPlanningStartAnim() {
  ...
  IActionRunner* animAction = new TriggerLiftSafeAnimationAction(
      _currDrivingAnimations.planningStartAnim, 1, true);
...
void DrivingAnimationHandler::PlayPlanningLoopAnim() {
  ...
  IActionRunner* animAction = new TriggerLiftSafeAnimationAction(
      _currDrivingAnimations.planningLoopAnim, 1, true);
...
void DrivingAnimationHandler::PlayPlanningEndAnim() {
  ...
  IActionRunner* animAction = new TriggerLiftSafeAnimationAction(
      _currDrivingAnimations.planningEndAnim, 1, true);
  _planningEndAnimTag = animAction->GetTag();
  _robot->GetActionList().QueueAction(QueueActionPosition::IN_PARALLEL,
                                      animAction);

  // TODO: We may consider combining the planning getout animation and the drive
  // getin animation if the planner search was successful to reduce the overall
