...
void RollingShutterCorrector::ComputePixelShifts(
    const VisionPoseData& poseData, const VisionPoseData& prevPoseData,
    const u32 numRows) {
  _pixelShifts.clear();
  ...
  // Whether or not a call to computePixelShiftsWithImageIMU returned false
  // meaning it was unable to compute the pixelShifts from imageIMU data
  bool didComputePixelShiftsFail = false;

  // Compounded pixelShifts across the image
  Vec2f shiftOffset = 0;

  // The fraction each subdivided row in the image will contribute to the total
  // shifts for this image
  const f32 frac = 1.f / _rsNumDivisions;

  for (int i = 1; i <= _rsNumDivisions; i++) {
    Vec2f pixelShifts;
    const RobotTimeStamp_t time =
        poseData.timeStamp -
        Anki::Util::numeric_cast<TimeStamp_t>(std::round(i * timeDif));
    didComputePixelShiftsFail |= !ComputePixelShiftsWithImageIMU(
        time, pixelShifts, poseData, prevPoseData, frac);

    _pixelShifts.insert(_pixelShifts.end(),
                        Vec2f(pixelShifts.x() + shiftOffset.x(),
                              pixelShifts.y() + shiftOffset.y()));
  ...
  if (didComputePixelShiftsFail) {
    if (!poseData.imuDataHistory.empty()) {
      PRINT_NAMED_WARNING(
          "RollingShutterCorrector.ComputePixelShifts.NoImageIMUData",
      ...
      PRINT_NAMED_WARNING(
          "RollingShutterCorrector.ComputePixelShifts.EmptyHistory",
...
bool RollingShutterCorrector::ComputePixelShiftsWithImageIMU(
    RobotTimeStamp_t t, Vec2f& shift, const VisionPoseData& poseData,
    const VisionPoseData& prevPoseData, const f32 frac) {
  if (poseData.imuDataHistory.empty()) {
    shift = Vec2f(0, 0);
    return false;
  }
  ...
  for (auto iter = poseData.imuDataHistory.begin();
       iter != poseData.imuDataHistory.end(); ++iter) {
    if (iter->timestamp >= t) {
      ...
      const TimeStamp_t tMinusBeforeTime =
          (TimeStamp_t)(t - ImuBeforeT->timestamp);
      const TimeStamp_t afterMinusBeforeTime =
          (TimeStamp_t)(ImuAfterT->timestamp - ImuBeforeT->timestamp);

      // Linearly interpolate the imu data using the timestamps before and after
      // imu data was captured
      rateY = (((tMinusBeforeTime) *
                (ImuAfterT->gyroRobotFrame.y - ImuBeforeT->gyroRobotFrame.y)) /
               (afterMinusBeforeTime)) +
              ImuBeforeT->gyroRobotFrame.y;
      rateZ = (((tMinusBeforeTime) *
                (ImuAfterT->gyroRobotFrame.z - ImuBeforeT->gyroRobotFrame.z)) /
               (afterMinusBeforeTime)) +
              ImuBeforeT->gyroRobotFrame.z;

      break;
    }
  }

  // If we don't have imu data for the timestamps before and after the timestamp
  // we are looking for just use the latest data
  if (!beforeAfterSet && !poseData.imuDataHistory.empty()) {
    // if the IMU data is recent enough, just assume gyro data hasn't changed
    // too much
  ...
  // If we aren't doing vertical correction then setting rateY to zero will
  // ensure no Y shift
  if (!doVerticalCorrection) {
    rateY = 0;
  }

  // The rates are in world coordinate frame but we want them in camera frame
  // which is why Z and X are switched
