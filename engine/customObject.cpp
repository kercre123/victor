...
CustomObject::CustomObject(
    ObjectType objectType, CustomObjectMarker markerFront,
    CustomObjectMarker markerBack, CustomObjectMarker markerTop,
    CustomObjectMarker markerBottom, CustomObjectMarker markerLeft,
    CustomObjectMarker markerRight, f32 xSize_mm, f32 ySize_mm, f32 zSize_mm,
    f32 markerWidth_mm, f32 markerHeight_mm, bool isUnique, CustomShape shape)
  ...
  switch (_customShape) {
    ...
    case CubeShape:
      // A cube with the same marker on all faces has complete rotation
      // ambiguity
      _rotationAmbiguities = RotationAmbiguities(
    ...
    case WallShape:
      // A wall with the same marker on both sides has 180deg rotation ambiguity
      // around the Z axis
      _rotationAmbiguities = RotationAmbiguities(
          false, {
...
bool CustomObject::IsValidCustomType(ObjectType objectType) {
  static_assert(
      ((ObjectType)(Util::EnumToUnderlying(ObjectType::CustomType19) + 1)) ==
          ObjectType::CustomFixedObstacle,
      "CustomFixedObstacle should immediately follow the max CustomType");

  if (objectType < ObjectType::CustomType00 ||
      objectType >= ObjectType::CustomFixedObstacle) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - -- - - - - - -
CustomObject* CustomObject::CreateBox(
    ObjectType objectType, CustomObjectMarker markerFront,
    CustomObjectMarker markerBack, CustomObjectMarker markerTop,
    CustomObjectMarker markerBottom, CustomObjectMarker markerLeft,
    CustomObjectMarker markerRight, f32 xSize_mm, f32 ySize_mm, f32 zSize_mm,
    f32 markerWidth_mm, f32 markerHeight_mm, bool isUnique) {
  if (!IsValidCustomType(objectType)) {
    return nullptr;
  }

  // Validate that all markers are different, since that is required for
  // BoxShape custom objects
  const std::set<CustomObjectMarker> uniqueMarkers{markerFront, markerBack,
                                                   markerTop,   markerBottom,
                                                   markerLeft,  markerRight};
  if (uniqueMarkers.size() != NumFaces) {
    PRINT_NAMED_WARNING(
        "CustomObject.CreateCustomBox.DuplicateMarkers",
        "Expecting custom box object to have 6 different markers");
    return nullptr;
  }

  CustomObject* object = new CustomObject(
      objectType, markerFront, markerBack, markerTop, markerBottom, markerLeft,
      markerRight, xSize_mm, ySize_mm, zSize_mm, markerWidth_mm,
      markerHeight_mm, isUnique, BoxShape);
...
CustomObject* CustomObject::CreateWall(ObjectType objectType,
                                       CustomObjectMarker marker, f32 width_mm,
                                       f32 height_mm, f32 markerWidth_mm,
                                       f32 markerHeight_mm, bool isUnique) {
  if (!IsValidCustomType(objectType)) {
    return nullptr;
  }

  return new CustomObject(objectType, marker,
                          marker,  // Only define front/back markers
                          CustomObjectMarker::Count, CustomObjectMarker::Count,
                          CustomObjectMarker::Count, CustomObjectMarker::Count,
                          kWallThickness_mm, width_mm, height_mm,
                          markerWidth_mm, markerHeight_mm, isUnique, WallShape);
}
...
CustomObject* CustomObject::CreateCube(ObjectType objectType,
  ...
  return new CustomObject(objectType, marker, marker, marker, marker, marker,
                          marker,  // same marker on all faces
                          size_mm, size_mm, size_mm, markerWidth_mm,
                          markerHeight_mm, isUnique, CubeShape);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - -- - - - - - -
CustomObject* CustomObject::CreateFixedObstacle(f32 xSize_mm, f32 ySize_mm,
                                                f32 zSize_mm) {
  return new CustomObject(ObjectType::CustomFixedObstacle,
                          CustomObjectMarker::Count, CustomObjectMarker::Count,
                          CustomObjectMarker::Count, CustomObjectMarker::Count,
                          CustomObjectMarker::Count, CustomObjectMarker::Count,
                          xSize_mm, ySize_mm, zSize_mm, 0.f, 0.f, false,
                          UnmarkedBoxShape);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - -- - - - - - -
Vision::MarkerType CustomObject::GetVisionMarkerType(
    const CustomObjectMarker customMarker) {
  switch (customMarker) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - -- - - - - - -
void CustomObject::AddFace(const FaceName whichFace,
                           const CustomObjectMarker customMarker) {
  ...
  switch (whichFace) {
    ...
    case NumFaces:
      PRINT_NAMED_ERROR("CustomObject.AddFace.NumFaces",
                        "Attempting to add NumFaces as a custom object face.");
      return;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - -- - - - - - -
CustomObject::~CustomObject() { EraseVisualization(); }
