 ...
 *known faces from the vision system (which generally runs on another thread).
 *
...
// dependencies on FaceWorld since when it's Init/Update relative to
...
class FaceWorld : public UnreliableComponent<BCComponentID>,
 ...
 public:
  ...
  // NOTE: many functions in this API have two versions, one which takes a
  // Vision::FaceID_t and one which takes a SmartFaceID. The use of SmartFaceID
  // is preferred because it automatically handles face id changes and deleted
  // faces. The raw face id API is maintained only for backwards compatibility.
  // COZMO-10839 is the task that will eventually remove this old interface
  ...
  // create a smart face ID or update an existing ID from a raw ID (useful, for
  // example for IDs from CLAD messages)
  SmartFaceID GetSmartFaceID(Vision::FaceID_t faceID) const;
  void UpdateSmartFaceToID(const Vision::FaceID_t faceID,
  ...
  // Set includeRecognizableOnly=true to only return faces that have been (or
  // can be) recognized. NOTE: This does not necessarily mean they have been
  // recognized as a _named_ person introduced via
  //       MeetCozmo. They could simply be recognized as a session-only person
  //       already seen in this session.
  // If relativeRobotAngleTolerence_rad is set to something other than 0, only
  // faces within +/- the relative robot angle will be returned
  std::set<Vision::FaceID_t> GetFaceIDs(
  ...
  std::vector<SmartFaceID> GetSmartFaceIDs(
      RobotTimeStamp_t seenSinceTime_ms = 0,
      bool includeRecognizableOnly = false,
      float relativeRobotAngleTolerence_rad = kDontCheckRelativeAngle,
      const Radians& angleRelativeRobot_rad = 0) const;
  ...
  bool HasAnyFaces(RobotTimeStamp_t seenSinceTime_ms = 0,
                   bool includeRecognizableOnly = false) const;

  // If the robot has observed a face, sets poseWrtRobotOrigin to the pose of
  // the last observed face and returns the timestamp when that face was last
  // seen. Otherwise, returns 0. Normally, inRobotOrigin=true, so that the last
  // observed pose is required to be w.r.t. the current origin.
  //
  // If inRobotOriginOnly=false, the returned pose is allowed to be that of a
  // face observed w.r.t. a different coordinate frame, modified such that its
  // parent is the robot's current origin. This could be a completely inaccurate
  // guess for the last observed face pose, but may be "good enough" for some
  // uses.
  RobotTimeStamp_t GetLastObservedFace(Pose3d& poseWrtRobotOrigin,
                                       bool inRobotOriginOnly = true) const;
  ...
  // Tell FaceWorld that the robot has turned towards this face (or not, if
  // val=false)
  ...
  // Specify a faceID to start an enrollment of a specific ID, i.e. with the
  // intention of naming that person. Use UnknownFaceID to enable (or return to)
  // ongoing "enrollment" of session-only / unnamed faces.
  ...
  void SetFaceEnrollmentComplete(bool complete) {
    _lastEnrollmentCompleted = complete;
  }
  ...
  // This method checks whether there will be a different face than the one
  // provided in the FOV if the robot were to turn a specific angle, and
  // populates that face and returns true if it finds one. Otherwise if it does
  // not find a face it returns false.
  bool FaceInTurnAngle(const Radians& turnAngle,
                       const SmartFaceID& smartFaceIDToIgnore,
                       const Pose3d& robotPose,
                       SmartFaceID& faceIDToTurnTowards) const;

  // Get the wall times that the given face ID has been observed for named
  // faces. This implementation returns at most 2 entries with front() being the
  // wall time that was recorded first. On loading time, this will populate with
  // wall times from enrolled face entries (even if those faces haven't been
  // seen since boot). It will be updated whenever the face is observed. If it
  // returns 2 entries, then the difference between them can be used as the
  // delta between when we most recently saw the face and the time before that,
  // e.g. to determine when we see someone how long it's been since the last
  // time we saw them.  If the face is unknown, an empty queue will be returned.
  // The queue may contain a single element in the case that it's an enrolled
  // face loaded from storage, or in the case that the face has only been seen
  // once. Tracking only (negative) face IDs are not returned here.
  //
  // Note: times are only updated here if wall time is accurate (synced with
  // NTP). Inaccurate times (e.g. if we're off wifi) won't get added here at all
  // (although times loaded from disk will)
 ...
 private:
  ...
  // Helper used by public Get() methods to determine if an entry should be
  // returned
  bool ShouldReturnFace(
      const FaceEntry& faceEntry, RobotTimeStamp_t seenSinceTime_ms,
  ...
  // For each enrolled face, keep track of the last wall time where we observed
  // it as well as the time before that in a deque of max size 2. On engine
  // startup, this timestamp will be read from the known faces saved album data
  // for the initial entry so it can work across boots
  using ObservationHistoryMap =
      std::map<Vision::FaceID_t, ObservationTimeHistory>;
