...
const size_t kMaxSubsampleDepth = 2;  // this currently corresponds to a minimum
                                      // step size of 8mm == navMap resolution
...
constexpr std::array<Point2<T>, 4> FourConnectedGrid(const T& res) {
  return {{{res, T(0)}, {-res, T(0)}, {T(0), -res}, {T(0), res}}};
}

// NOTE: the escape grid resolution needs to be the same as the planner
// Resolution, otherwise it will
...
const std::array<Point2f, 8> escapeGrid =
    EightConnectedGrid(kPlanningResolution_mm);
...
static_assert(plannerFullGrid.size() == plannerHalfGrid.size(),
              "PlannerPoint Half Steps and Full Steps must have the same "
...
class PlannerPoint : public Point2f {
 public:
  ...
  PlannerPoint(const Point2f& p, size_t depth = 0)
      : Point2f(p), _depth(depth), _stepSize(kPlanningResolution_mm >> depth) {}
  ...
  PlannerPoint FullStep(size_t dir) const {
    // since we are only do half and full steps, if we aren't at depth 0, a
    // "full step" will put us one level higher, decreasing depth.
    return {(*this) + plannerFullGrid[dir] * _stepSize,
            (_depth > 0) ? _depth - 1 : 0};
  }

  PlannerPoint HalfStep(size_t dir) const {
    return (_depth <= kMaxSubsampleDepth)
               ? PlannerPoint((*this) + (plannerHalfGrid[dir] * _stepSize),
                              _depth + 1)
               : FullStep(dir);
  }
...
class PlannerConfig
 ...
 public:
  ...
  class SuccessorIter
   ...
   private:
    inline void UpdateState() {
      ...
      if (_map.CheckForCollisions(
              Ball2f(_state, kRobotRadius_mm + kPlanningPadding_mm))) {
  ...
  PlannerConfig(const Point2f& start, const std::vector<Point2f>& goals,
                const MapComponent& map, const volatile bool& stopPlanning)
      : _start(start),
        _goals(goals.begin(), goals.end()),
        _map(map),
        _abort(stopPlanning) {}

  inline bool StopPlanning() {
    return _abort || (++_numExpansions > kPlanPathMaxExpansions);
  }
  inline size_t GetNumExpansions() const { return _numExpansions; }
  inline SuccessorIter GetSuccessors(const PlannerPoint& p) const {
    return SuccessorIter(p, _map);
  };

  inline float ReverseHeuristic(const Point2f& p) const {
    return ManhattanDistance(p, _start);
  };
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
//  Dijkstra Configuration that finds the nearest collision free state with
//  uniform action cost
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
class EscapeObstaclePlanner
    : public IAStarConfig<Point2f, EscapeObstaclePlanner> {
 public:
  EscapeObstaclePlanner(const MapComponent& map,
                        const volatile bool& stopPlanning)
      : _map(map), _abort(stopPlanning) {}

  inline float Heuristic(const Point2f& p) const { return 0.f; };
  inline bool StopPlanning() {
    return _abort || (++_numExpansions > kEscapeObstacleMaxExpansions);
  }
  inline bool IsGoal(const Point2f& p) const {
    return !_map.CheckForCollisions(
        Ball2f(p, kRobotRadius_mm + kPlanningPadding_mm));
  }
  ...
  inline std::array<Successor, 8> GetSuccessors(const Point2f& p) const {
    ...
    std::transform(escapeGrid.begin(), escapeGrid.end(), retv.begin(),
                   [&gridP](const auto& dir) {
                     return Successor{gridP + dir, dir.Length()};
                   });
    return retv;
  };
