...
XYPlanner::~XYPlanner() {
  ...
  if (_plannerThread != nullptr) {
    ...
    try {
      ...
      LOG_ERROR("XYPlanner.Destroy.Exception",
                "locking the context mutex threw: %s", err.what());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
EComputePathStatus XYPlanner::ComputePath(
    const Pose3d& startPose, const std::vector<Pose3d>& targetPoses) {
  std::vector<Pose2d> goalCopy;
  for (const auto& t : targetPoses) {
    goalCopy.push_back(t);
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
EComputePathStatus XYPlanner::ComputeNewPathIfNeeded(
    const Pose3d& startPose, bool forceReplanFromScratch,
    bool allowGoalChange) {
  std::vector<Pose2d> goalCopy = _targets;
  return InitializePlanner(startPose, goalCopy, forceReplanFromScratch,
                           allowGoalChange);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
EComputePathStatus XYPlanner::InitializePlanner(
    const Pose2d& start, const std::vector<Pose2d>& targets, bool forceReplan,
    bool allowGoalChange) {
  // planner will start on next thread cycle
  if (!forceReplan && _startPlanner) {
    return EComputePathStatus::Running;
  }

  // if the planner is running, flag an abort on the current instance so we can
  // restart ASAP
  ...
  if (!forceReplan) {
    ...
    LOG_INFO("XYPlanner.InitializePlanner.CollisionCostIncreased",
             "Replanning. Old=%6.6f New=%6.6f", _collisionPenalty,
             currentPenalty);
  }
...
void XYPlanner::StartPlanner() {
  ...
  if (!_contextMutex.try_lock()) {
    LOG_ERROR("XYPlanner.StartPlanner.InternalThreadingError",
              "Somehow failed to get mutex inside StartPlanning, but we should "
              "already have it at this point");
    return;
  }
  ...
  std::map<Point2i, Point2f>
      goalLookup;  // we need to map grid-aligned planner goals to true targets
  if (_allowGoalChange || (_path.GetNumSegments() == 0)) {
    for (const auto& g : _targets) {
      Point2f grid_g =
          GetNearestGridPoint(g.GetTranslation(), kPlanningResolution_mm);
      plannerGoals.push_back(grid_g);
      // grid_g should be a whole number, so cast to int here to prevent weird
  ...
  // NOTE:  if no safe point exists, the A* search will timeout, but we probably
  // have bigger problems to deal with.
  //        Why can we not find a single safe point anywhere within the
  //        searchable range of EscapeObstaclePlanner?
  //
  // NOTE2: there seems to be a bug in the planner where using Point::IsNear is
  // not a sufficient check for determining
  //        that the goal is safe, even if we use a known safe point for the
  //        goal. The work around, for now, is to find the nearest safe -grid-
  //        point, and then insert the true goal state after a plan has been
  //        made.
  const Point2f plannerStart = FindNearestSafePoint(
      GetNearestGridPoint(_start.GetTranslation(), kPlanningResolution_mm));

#if !defined(NDEBUG)
  for (const auto& s : plannerGoals) {
    LOG_DEBUG("XYPlanner.StartPlanner", "Plan from %s to %s (%.1f mm)",
              plannerStart.ToString().c_str(), s.ToString().c_str(),
              (plannerStart - s).Length());
  }
#endif
  ...
  if (!plan.empty()) {
    // planner will only go to the nearest safe grid point, so add the real
    ...
    if (goalLookup.find(plannerGoal) != goalLookup.end()) {
      ...
      LOG_WARNING("XYPlanner.StartPlanner",
                  "Could not match planner goal point to requested goal point, "
                  "planning to nearest Planner Point");
    }
    ...
    ANKI_VERIFY(_selectedTargetIdx < _targets.size(),
                ...
                "totalGoals=%zd nearestIdx=%u", _targets.size(),
                _selectedTargetIdx);
  ...
  auto planTime_ms = duration_cast<std::chrono::milliseconds>(
      high_resolution_clock::now() - startTime);
  LOG_INFO("XYPlanner.StartPlanner",
           "planning took %s ms (%zu expansions at %.2f exp/sec)",
...
inline Planning::GoalID XYPlanner::FindGoalIndex(const Point2f& p) const {
  const auto isPose = [&p](const Pose2d& t) {
    return IsNearlyEqual(p, t.GetTranslation());
  };
  // Will be _targets.size() if no match was found
  return std::distance(_targets.begin(),
                       std::find_if(_targets.begin(), _targets.end(), isPose));
}
...
// straight lines are rectangles the length of the line segment, with robot
// width
inline FastPolygon GetLineCollisionSet(const LineSegment& l, float padding) {
  Point2f normal(l.GetFrom().y() - l.GetTo().y(),
                 l.GetTo().x() - l.GetFrom().x());
  normal.MakeUnitLength();
  float width = kRobotRadius_mm + padding;

  return FastPolygon({l.GetFrom() + normal * width, l.GetTo() + normal * width,
...
inline std::vector<Ball2f> GetArcCollisionSet(const Arc& a, float padding) {
  ...
  const int nChecks =
      std::ceil(ABS(sweepAngle.ToFloat() * b.GetRadius()) / kRobotRadius_mm);
  const float checkLen = sweepAngle.ToFloat() / nChecks;

  std::vector<Ball2f> retv;
  for (int i = 0; i <= nChecks; ++i) {
    f32 rad = startAngle.ToFloat() + i * checkLen;
    retv.emplace_back(b.GetCentroid() + Point2f(std::cos(rad), std::sin(rad)) *
                                            (kRobotRadius_mm + padding),
                      kRobotRadius_mm + padding);
  }
...
inline float XYPlanner::GetArcPenalty(const Arc& arc, float padding) const {
  ...
  return std::accumulate(disks.begin(), disks.end(), 0.f,
                         [this](float cost, const auto& disk) {
                           return cost + _map.GetCollisionArea(disk);
                         });
}
...
inline bool XYPlanner::IsArcSafe(const Arc& arc, float padding) const {
  const auto disks = GetArcCollisionSet(arc, padding);
  return std::none_of(disks.begin(), disks.end(), [this](const auto& disk) {
    return _map.CheckForCollisions(disk);
  });
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
inline float XYPlanner::GetLinePenalty(const LineSegment& seg,
                                       float padding) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool XYPlanner::CheckIsPathSafe(const Planning::Path& path,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool XYPlanner::CheckIsPathSafe(const Planning::Path& path, float startAngle,
                                Planning::Path& validPath) const {
  const auto isSafe = [this](const Planning::PathSegment& seg) -> bool {
    switch (seg.GetType()) {
      case Planning::PST_POINT_TURN: {
        return IsPointSafe(CreatePoint2f(seg), 0.);
      }
      case Planning::PST_LINE: {
        return IsLineSafe(CreateLineSegment(seg), 0.);
      }
      case Planning::PST_ARC: {
        return IsArcSafe(CreateArc(seg), 0.f);
      }
      default: {
        return true;
      }
  ...
  for (int i = 0; i < path.GetNumSegments(); ++i) {
    // TODO VIC-4315 return the actual safe subpath. It might need splitting
    // into smaller components if the current segment is long and only part of
    // it is unsafe.
    if (!isSafe(path.GetSegmentConstRef(i))) {
      return false;
    }
  }
  return true;
...
float XYPlanner::GetPathCollisionPenalty(const Planning::Path& path) const {
  const auto getCost = [this](const Planning::PathSegment& seg) -> float {
    switch (seg.GetType()) {
      case Planning::PST_POINT_TURN: {
        return GetPointPenalty(CreatePoint2f(seg), 0.);
      }
      case Planning::PST_LINE: {
        return GetLinePenalty(CreateLineSegment(seg), 0.);
      }
      case Planning::PST_ARC: {
        return GetArcPenalty(CreateArc(seg), 0.f);
      }
      default: {
        return 0.f;
      }
...
Planning::Path XYPlanner::BuildPath(const std::vector<Point2f>& plan) const {
  ...
  if (!NEAR(turns.front().GetDef().turn.targetAngle,
            _start.GetAngle().ToFloat(), kPathPrecisionTolerance)) {
  ...
  for (int i = 1; i < turns.size(); ++i) {
    ...
    if ((Point2f(x0, y0) - Point2f(x1, y1)).Length() >
        kPathPrecisionTolerance) {
  ...
  if (!path.CheckContinuity(.0001)) {
    LOG_WARNING("XYPlanner.BuildPath",
                "Path smoother gnereated a non-continuous plan");
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::vector<Point2f> XYPlanner::GenerateWayPoints(
    const std::vector<Point2f>& plan) const {
  ...
  for (int i = 0; i < plan.size(); ++i) {
    const FastPolygon p =
        GetLineCollisionSet({*iter1, plan[i]}, kPlanningPadding_mm);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::vector<Planning::PathSegment> XYPlanner::SmoothCorners(
  ...
  // for now, always start and end with a point turn the correct heading.
  // Generating the first/last arc uses different logic since heading angles are
  // constrained, while all intermediate headings are not.
  ...
  for (int i = 1; i < pts.size() - 1; ++i) {
    ...
    // add the first safe arc that can be constructed for the current waypoints,
    // prioritizing inscribed arcs over circumscribed arcs
    for (const float r : arcRadii) {
      // try inscribed arc first since it is faster, otherwise try circumscibed
      // arc
      if (GetInscribedArc(pts[i - 1], pts[i], pts[i + 1], r, corner)) {
        safeArc = IsArcSafe(corner, kPlanningPadding_mm);
      }
      if (!safeArc &&
          GetCircumscribedArc(pts[i - 1], pts[i], pts[i + 1], r, corner)) {
        safeArc = IsArcSafe(corner, kPlanningPadding_mm) &&
                  IsLineSafe({Point2f(x_tail, y_tail), corner.start},
                             kPlanningPadding_mm) &&
    ...
    turns.emplace_back(
        (safeArc) ? CreateArcPath(corner)
                  : CreatePointTurnPath({pts[i - 1], pts[i], pts[i + 1]}));
  }
