 ...
 * Created: 8/22/2018
 *
 * Description: Stores account settings on robot; accepts new account settings
 *from app or cloud; provides access to these settings for other components
 *
...
class AccountSettingsManager
 ...
 public:
  ...
  bool SetAccountSetting(
      const external_interface::AccountSetting accountSetting,
      const Json::Value& valueJson, const bool updateSettingsJdoc,
      bool& ignoredDueToNoChange);

  // Return the account setting value (currently strings, bools, uints
  // supported)
  std::string GetAccountSettingAsString(
      const external_interface::AccountSetting key) const;
  bool GetAccountSettingAsBool(
      const external_interface::AccountSetting key) const;
  uint32_t GetAccountSettingAsUInt(
      const external_interface::AccountSetting key) const;
 ...
 private:
  ...
  using SettingSetters =
      std::map<external_interface::AccountSetting, SettingSetter>;
