...
class VisionScheduleMediator
 ...
 public:
  ...
  // Perform most of the Init from here, called by InitDependent, to allow for
  // Unit Testing
  void Init(const Json::Value& config);

  // Set up baseline subscriptions to manage VisionMode defaults via the VSM
  void GetInternalSubscriptions(
      std::set<VisionModeRequest>& baselineSubscriptions) const {}

  // Subscribe at "standard" update frequency to a set of VisionModes. This call
  // REPLACES existing subscriptions for the pertinent subscriber
  void SetVisionModeSubscriptions(IVisionModeSubscriber* subscriber,
                                  const std::set<VisionMode>& desiredModes);

  // Subscribe at defined update frequencies to a vector of VisionModes. This
  // call REPLACES existing subscriptions for the pertinent subscriber
  void SetVisionModeSubscriptions(IVisionModeSubscriber* subscriber,
                                  const std::set<VisionModeRequest>& requests);

  // Subscribe at defined update frequencies to a vector of VisionModes. This
  // call only updates the requested VisionModes subscriptions for the pertinent
  // subscriber
  void AddAndUpdateVisionModeSubscriptions(
  ...
  bool RemoveVisionModeSubscriptions(IVisionModeSubscriber* subscriber,
  ...
  // If andReset=true, also counts the single shot modes as "processed" so they
  // won't be returned anymore after this VisionComponent (the actual user of
  // the schedule) is expected to be the only caller to use andReset=true
 ...
 private:
  struct VisionModeData {
    ...
    static bool CompareRecords(Record i, Record j) {
      return i.second < j.second;
    }
    int GetMinUpdatePeriod() const {
      auto minRecord = min_element(requestMap.begin(), requestMap.end(),
                                   &VisionModeData::CompareRecords);
  ...
  // Makes a pass over the VisionModeSchedule to spread out processing
  // requirements for various modes. The generated schedule is returned for
  // evaluation in Unit Tests, but is not normally used.
  const AllVisionModesSchedule::ModeScheduleList GenerateBalancedSchedule();

  // Returns true if the update period for this mode changed as a result of
  // subscription changes
  bool UpdateModePeriodIfNecessary(VisionModeData& mode) const;

  // Helper method to convert between enums and settings in (frames between
  // updates)
  int GetUpdatePeriodFromEnum(
      const VisionMode& mode,
      const EVisionUpdateFrequency& frequencySetting) const;

  void UpdateModeDataMapWithRequests(
      IVisionModeSubscriber* subscriber,
      const std::set<VisionModeRequest>& requests);

  using RequestRecord =
      std::pair<IVisionModeSubscriber*, EVisionUpdateFrequency>;
