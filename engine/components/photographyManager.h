...
class PhotographyManager : public IDependencyManagedComponent<RobotComponentID>,
 ...
 public:
  ...
  // Updates camera settings when taking a photo so that the image is
  // aesthetically pleasing
  Result EnablePhotoMode(bool enable);

  // Returns true once switch to photo mode is complete after a call to
  // EnablePhotoMode(true)
  bool IsReadyToTakePhoto() const;

  // Tells Vision component to take a photograph and saves it to the user's
  // photo storage Returns a handle to use to check if the photo has been
  // taken/saved yet If called when not ready to take photo (see above check),
  // returns 0
 ...
 private:
  ...
  int PhotoIndexFromID(
      const int id) const;  // Returns photo info index, or -1 if not found

  void HandleEvents(const AnkiEvent<external_interface::GatewayWrapper>& event);
  void OnRequestPhotosInfo(
  ...
  struct PhotoInfo {
    ...
    PhotoInfo(int id, TimeStamp_t dt, bool photoCopied, bool thumbCopied)
        : _id(id),
          _dateTimeTaken(dt),
          _photoCopiedToApp(photoCopied),
          _thumbCopiedToApp(thumbCopied) {}
