 ...
 *              Less useful on Vector than it was on Cozmo, but still used in
 *playpen so we're still hanging onto it.
 *
...
NVStorageComponent::NVStorageComponent()
    : IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::NVStorage) {}

NVStorageComponent::~NVStorageComponent() {}

void NVStorageComponent::InitDependent(Vector::Robot* robot,
                                       const RobotCompMap& dependentComps) {
  _robot = robot;
  _kStoragePath = (_robot->GetContextDataPlatform() != nullptr
                       ? _robot->GetContextDataPlatform()->pathToResource(
                             Util::Data::Scope::Persistent, "nvStorage/")
                       : "");
#ifdef SIMULATOR
  LoadSimData();
#endif
...
bool NVStorageComponent::IsFactoryEntryTag(NVEntryTag tag) const {
  return IsValidEntryTag(tag) &&
         (static_cast<u32>(tag) &
          static_cast<u32>(NVConst::NVConst_FACTORY_DATA_BIT));
}

bool NVStorageComponent::Write(NVEntryTag tag, const std::vector<u8>* data,
                               NVStorageWriteEraseCallback callback) {
  // Check for null data
  if (data == nullptr) {
    PRINT_CH_INFO("NVStorage", "NVStorageComponent.Write.NullData", "%s",
                  EnumToString(tag));
    return false;
  }
...
bool NVStorageComponent::Write(NVEntryTag tag, const u8* data, size_t size,
                               NVStorageWriteEraseCallback callback) {
  bool validArgs = true;
  ...
  // Check for null data
  if (data == nullptr) {
    PRINT_NAMED_WARNING("NVStorageComponent.Write.NullData", "%s",
                        EnumToString(tag));
    validArgs = false;
  }
  ...
  if (!_writingFactory) {
    ...
    if (size <= 0) {
      PRINT_NAMED_WARNING("NVStorageComponent.Write.InvalidSize",
                          "Tag: %s, %zu bytes", EnumToString(tag), size);
      validArgs = false;
    }
  }
  ...
  PRINT_CH_INFO("NVStorage", "NVStorageComponent.Write.WritingData", "%s",
                EnumToString(tag));

  _tagDataMap[tag] = std::vector<u8>(data, data + size);

  if (callback) {
    PRINT_CH_DEBUG("NVStorage", "NVStorageComponent.Write.ExecutingCallback",
                   "%s", EnumToString(tag));
...
bool NVStorageComponent::Erase(NVEntryTag tag,
  ...
  PRINT_CH_INFO("NVStorage", "NVStorageComponent.Erase.ErasingData", "%s",
                EnumToString(tag));

  _tagDataMap.erase(tag);

  if (callback) {
    PRINT_CH_DEBUG("NVStorage", "NVStorageComponent.Erase.ExecutingCallback",
                   "%s", EnumToString(tag));
    callback(NVResult::NV_OKAY);
  }
...
bool NVStorageComponent::Read(NVEntryTag tag, NVStorageReadCallback callback,
                              std::vector<u8>* data) {
  PRINT_CH_INFO("NVStorage", "NVStorageComponent.Read.ReadingData", "%s",
                EnumToString(tag));
  ...
  if (callback) {
    PRINT_CH_DEBUG("NVStorage", "NVStorageComponent.Read.ExecutingCallback",
                   "%s", EnumToString(tag));
...
// If you bypass this it's your own responsibility to delete
// webotsCtrlGameEngine2/files/output/nvStorage to get a clean system again.
  ...
  // TODO(Al): This probably needs to write somewhere else for actual factory
  // nvStorage
  ...
  if (iter == _tagDataMap.end()) {
    PRINT_CH_DEBUG("NVStorage", "NVStorageComponent.WriteEntryToFile.Erasing",
  ...
  // Write data to file
  PRINT_CH_DEBUG("NVStorage", "NVStorageComponent.WriteEntryToFile.Writing",
                 "Tag %s, size %zu", EnumToString(tag), iter->second.size());
  Util::FileUtils::CreateDirectory(path);
  if (!Util::FileUtils::WriteFile(path + ss.str(), iter->second)) {
    PRINT_NAMED_ERROR("NVStorageComponent.WriteEntryToFile.Failed", "%s",
                      ss.str().c_str());
    return false;
  }
...
void NVStorageComponent::LoadDataFromFiles() {
  auto fileList = Util::FileUtils::FilesInDirectory(_kStoragePath, false,
                                                    _kNVDataFileExtension);
  ...
  for (auto& fileName : fileList) {
    ...
    if (!IsValidEntryTag(tag)) {
      PRINT_NAMED_ERROR("NVStorageComponent.LoadDataFromFiles.InvalidTagValues",
                        "0x%x", tagNum);
      continue;
    }
    ...
    // Read data from file
    std::vector<u8> file =
        Util::FileUtils::ReadFileAsBinary(_kStoragePath + fileName);
    if (file.empty()) {
      PRINT_NAMED_ERROR("NVStorageComponent.LoadDataFromFiles.ReadFileFailed",
                        "Unable to read nvStorage entry file %s",
                        fileName.c_str());
      continue;
    }

    PRINT_CH_DEBUG(
        "NVStorage", "NVStorageComponent.LoadDataFromFiles.LoadingData",
        "Tag %s, data size = %zu bytes", EnumToString(tag), file.size());
  ...
  // TODO: For now load factory related nvstorage files from the "factory"
  // subdirectory. Will probably be moved to some read only place
  fileList = Util::FileUtils::FilesInDirectory(_kFactoryPath, false,
                                               _kNVDataFileExtension);
  ...
  for (auto& fileName : fileList) {
    ...
    if (!IsFactoryEntryTag(tag)) {
      PRINT_NAMED_ERROR(
          "NVStorageComponent.LoadFactoryDataFromFiles.InvalidTagValues",
          "0x%x", tagNum);
      continue;
    }

    // Camera calibration is special and is stored in a block device so we
    // should not be trying to load it from the normal nvstorage files
    if (tag == NVEntryTag::NVEntry_CameraCalib) {
      PRINT_NAMED_ERROR(
          "NVStorageComponent.LoadFactoryDataFromFiles.CameraCalibFile",
          "Trying to load camera calibration from nvStorage file when it "
          "should be in the block device");
      continue;
    }

    // Read data from file
    std::vector<u8> file =
        Util::FileUtils::ReadFileAsBinary(_kFactoryPath + fileName);
    if (file.empty()) {
      PRINT_NAMED_ERROR(
          "NVStorageComponent.LoadFactoryDataFromFiles.ReadFileFailed",
          "Unable to read nvStorage entry file %s", fileName.c_str());
      continue;
    }

    PRINT_CH_DEBUG("NVStorage",
                   "NVStorageComponent.LoadFactoryDataFromFiles.LoadingData",
                   "Tag 0x%x, data size = %zu bytes", tagNum, file.size());
    _tagDataMap[tag].assign(file.data(), file.data() + file.size());
  }
}
...
void NVStorageComponent::LoadSimData() {
  ...
  const CameraCalibration* camCalib =
      CameraService::getInstance()->GetHeadCamInfo();

  _tagDataMap[NVEntryTag::NVEntry_CameraCalib].assign(
      reinterpret_cast<const u8*>(camCalib),
      reinterpret_cast<const u8*>(camCalib) + sizeof(*camCalib));
}
#endif  // ifdef SIMULATOR
...
int NVStorageComponent::OpenCameraCalibFile(int openFlags) {
  ...
  if (fd == -1) {
    PRINT_NAMED_ERROR(
        "NVStorageComponent.OpenCameraCalibFile.FailedToOpenBlockDevice", "%d",
        errno);
    return -1;
  }
...
void NVStorageComponent::ReadCameraCalibFile() {
  ...
  if (fd == -1) {
    PRINT_NAMED_ERROR(
        "NVStorageComponent.LoadDataFromFiles.FailedToOpenCameraCalib", "");
    return;
  }
  ...
  const u32 kNumBytesToRead =
      4096;  // Read a whole page due to this being a block device
  static_assert(kNumBytesToRead >= kNumBytesForCalib,
                "Bytes to read smaller than size of camera calibration");

  u8 buf[kNumBytesToRead];
  ssize_t numBytesRead = 0;
  do {
    numBytesRead +=
        read(fd, buf + numBytesRead, kNumBytesToRead - numBytesRead);
    if (numBytesRead == -1) {
      PRINT_NAMED_ERROR(
          "NVStorageComponent.LoadDataFromFiles.FailedToReadCameraCalib", "%d",
          errno);
      return;
    }
  ...
  if (dataMagic != _kCameraCalibMagic) {
    PRINT_NAMED_ERROR(
        "NVStorageComponent.LoadDataFromFiles.CameraCalibMagicMismatch",
        "Read magic 0x%x and expected magic 0x%x do not match", dataMagic,
        _kCameraCalibMagic);
    return;
  }
  ...
  std::vector<u8> data;
  std::copy(&buf[kSizeOfMagic], &buf[kNumBytesForCalib],
  ...
  if (rc == -1) {
    PRINT_NAMED_ERROR(
        "NVStorageComponent.LoadDataFromFiles.FailedToCloseCameraCalib", "%d",
        errno);
  }
}

bool NVStorageComponent::WriteCameraCalibFile(
    const TagDataMap::iterator& iter) {
  ...
  if (fd == -1) {
    PRINT_NAMED_ERROR(
        "NVStorageComponent.WriteEntryToFile.FailedToOpenCameraCalib", "");
    return false;
  }

  const u32 kNumBytesForCalib =
      sizeof(_kCameraCalibMagic) + sizeof(CameraCalibration);
  u8 buf[kNumBytesForCalib];
  *((u32*)buf) = _kCameraCalibMagic;
  std::copy(iter->second.begin(), iter->second.end(),
            buf + sizeof(_kCameraCalibMagic));

  bool ret = true;
  ssize_t numBytesWritten = write(fd, buf, kNumBytesForCalib);
  if (numBytesWritten == -1) {
    PRINT_NAMED_ERROR(
        "NVStorageComponent.WriteEntryToFile.FailedToWriteBlockDevice", "%d",
        errno);

    ret = false;
  } else if (numBytesWritten != kNumBytesForCalib) {
    PRINT_NAMED_ERROR(
        "NVStorageComponent.WriteEntryToFile.FailedToWriteBlockDevice",
        "Only wrote %zd bytes instead of %d", numBytesWritten,
        kNumBytesForCalib);
    ret = false;
  }
  ...
  if (rc == -1) {
    PRINT_NAMED_ERROR(
        "NVStorageComponent.WriteEntryToFile.FailedToCloseBlockDevice", "%d",
        errno);
    ret = false;
  }
