...
class PowerStateManager : public IDependencyManagedComponent<RobotComponentID>,
 ...
 public:
  ...
  virtual void AdditionalUpdateAccessibleComponents(
      RobotCompIDSet& comps) const override {
  ...
  // Request the robot enter power save mode. If any requests are active, this
  // component will attempt to enter power save. The requester string should be
  // unique and is useful for debugging
  void RequestPowerSaveMode(const std::string& requester);

  // Remove a specific power save request, and return true if one was found.
  // Once no requests remain, this component will attempt to leave power save
  // mode
  bool RemovePowerSaveModeRequest(const std::string& requester);

  bool IsPowerSaveRequestPending() const {
    return _powerSaveRequests.empty() == _inPowerSaveMode;
  }
  ...
  // NOTE: In an ideal system, we'd work the opposite way, where specific
  // behaviors or pieces of code could request a higher power mode, and the
  // _default_ would be power save. This would potentially allow better power
  // saving, but also be harder to debug and have systems understand what to do
  // and not do in power save mode, so for now it's "opt-in" instead of
  // "opt-out"
 ...
 private:
  ...
  void TogglePowerSaveSetting(const RobotCompMap& components,
                              PowerSaveSetting setting, bool enabled);

  const CozmoContext* _context = nullptr;

  std::set<PowerSaveSetting> _enabledSettings;

  enum class CameraState { ShouldInit, Running, ShouldDelete, Deleted };
