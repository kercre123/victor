...
class JdocsManager : public IDependencyManagedComponent<RobotComponentID>,
 ...
 public:
  ...
  const std::string& GetJdocName(
      const external_interface::JdocType jdocTypeKey) const;
  const uint64_t GetJdocDocVersion(
      const external_interface::JdocType jdocTypeKey) const;
  const uint64_t GetJdocFmtVersion(
      const external_interface::JdocType jdocTypeKey) const;
  const uint64_t GetCurFmtVersion(
      const external_interface::JdocType jdocTypeKey) const;
  void SetJdocFmtVersionToCurrent(
      const external_interface::JdocType jdocTypeKey);
  const Json::Value& GetJdocBody(
      const external_interface::JdocType jdocTypeKey) const;
  Json::Value* GetJdocBodyPointer(
      const external_interface::JdocType jdocTypeKey);
  ...
  bool UpdateJdoc(const external_interface::JdocType jdocTypeKey,
                  const Json::Value* jdocBody, const bool saveToDiskImmediately,
 ...
 private:
  ...
  void HandleWriteResponse(const JDocs::WriteRequest& writeRequest,
                           const JDocs::WriteResponse& writeResponse);
  void HandleReadResponse(const JDocs::ReadRequest& readRequest,
                          const JDocs::ReadResponse& readResponse);
  void HandleDeleteResponse(const JDocs::DeleteRequest& deleteRequest,
                            const Void& voidResponse);
  void HandleErrResponse(const JDocs::DocRequest& request,
  ...
  void SubmitJdocToCloud(const external_interface::JdocType jdocTypeKey,
                         const bool isJdocNewInCloud);
  bool CopyJdocFromCloud(const external_interface::JdocType jdocTypeKey,
                         const JDocs::Doc& doc);
  void SendJdocsChangedMessage(
      const std::vector<external_interface::JdocType>& jdocTypes);

  external_interface::JdocType JdocTypeFromDocName(
  ...
  // We save currTime_s here each tick, because we need it in the destructor,
  // and by then BasetationTimer is gone
  float _currTime_s;
  uint32_t _minCloudGetPeriod_s;  // Minimum time between 'get latest jdocs from
                                  // cloud'

  struct JdocInfo {
    // Start of jdoc contents
    uint64_t _jdocVersion;  // Major version of jdoc; ONLY the cloud server can
                            // change this
    uint64_t _jdocFormatVersion;  // Format version of jdoc; cloud server CANNOT
                                  // change this
    std::string _jdocClientMetadata;  // (I think this may contain minor version
                                      // at some point)
    ...
    bool _needsCreation;    // True if this jdoc needs to be created (by another
                            // subsystem)
    bool _needsMigration;  // True if this jdoc needs a format version migration
                           // at startup
    uint64_t _curFormatVersion;  // Current/latest format version this code
                                 // knows about

    std::string _jdocFullPath;  // Full path of file on disk if applicable
    bool _diskFileDirty;
    int _diskSavePeriod_s;  // Disk save period, or 0 for always save
                            // immediately
    float
        _nextDiskSaveTime;  // Time of next disk save ("at this time or after")

    bool _bodyOwnedByJM;  // True when JdocsManager owns the jdoc body
    ...
    int _cloudSavePeriod_s;      // Cloud save period, or 0 for always save
                                 // immediately
    int _origCloudSavePeriod_s;  // Original cloud save period (so we can
                                 // restore if changed)
    float _nextCloudSaveTime;    // Time of next cloud save ("at this time or
                                 // after")
    bool _pendingCloudSave;  // True when cloud save is awaiting response from
                             // prior cloud save
    external_interface::JdocResolveMethod
        _resolveMethod;  // Resolve method to use when cloud has newer version
                         // of this jdoc
    uint32_t _lastCloudGetTime;  // Timestamp of last request to cloud to get
                                 // latest version
    ...
    struct CloudAbuseDetectionConfig {
      ...
      RecentOccurrenceTracker
          _cloudWriteTracker;  // Used to detect cloud spam abuse
      int _abuseLevel;
    } _abuseConfig;

    enum class CloudDisabled {
      NotDisabled,
      FormatVersion,  // Disabled because cloud has higher format version of
                      // this jdoc than code can handle
      CloudError,     // Disabled because we received a cloud error on write or
                      // read of jdoc to/from cloud
    };
    CloudDisabled _cloudDisabled = CloudDisabled::NotDisabled;
    bool _hasAccessedCloud;  // Have we accessed the cloud for this jdoc since
                             // clearing user data?

    OverwriteNotificationCallback
        _overwrittenCB;  // Called when this jdoc is overwritten from the cloud
    FormatMigrationCallback
        _formatMigrationCB;  // Called when this jdoc needs a format migration
  ...
  UnsentDocRequestQueue _unsentDocRequestQueue;  // Unsent requests that are
