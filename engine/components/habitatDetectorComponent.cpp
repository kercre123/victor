...
HabitatDetectorComponent::HabitatDetectorComponent()
    : IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::HabitatDetector),
...
void HabitatDetectorComponent::InitDependent(
    Robot* robot, const RobotCompMap& dependentComponents) {
  _robot = robot;

  if (robot->HasExternalInterface()) {
    using namespace ExternalInterface;
    auto helper = MakeAnkiEventUtil(*(robot->GetExternalInterface()), *this,
                                    _signalHandles);
        ...
        if (webService != nullptr) {
          auto onData =
              [this](
                ...
                ForceSetHabitatBeliefState(
                    HabitatBeliefStateFromString(
                        data["forceHabitatState"].asCString()),
                    "WebViz");
              };
          _signalHandles.emplace_back(
              webService->OnWebVizData(kWebVizHabitatModuleName)
    ...
    PRINT_NAMED_WARNING(
        "HabitatDetectorComponent.InitDependent.MissingExternalIterface", "");
  }
...
template <>
void HabitatDetectorComponent::HandleMessage(
...
void HabitatDetectorComponent::HandleMessage(
  ...
  // that way we are able to always make sure we're computing odometry with
  // respect to the last known putdown position (rather than some arbitrary one
  // from localization)

  // we save the pose-origin ID after any delocalization because if this pose
  // gets rejiggered by localizing to an object, then it counts as far as
  // measuring displacement for checking we are in a non-habitat location (in
  // the absence of a putdown-origin pose)
  _poseOriginIdOnDelocalize = _robot->GetWorldOriginID();
}

void HabitatDetectorComponent::SetBelief(HabitatBeliefState state,
                                         std::string devReasonStr) {
  PRINT_NAMED_INFO("HabitatDetectorComponent.SetBelief", "%s <- %s",
                   EnumToString(state), devReasonStr.c_str());
...
bool HabitatDetectorComponent::UpdateProxObservations() {
  ...
  if (proxData.foundObject &&
      _proxReadingBuffer.size() < kMinProxReadingsRequired) {
...
void HabitatDetectorComponent::ForceSetHabitatBeliefState(
    HabitatBeliefState belief, const std::string& sourceStr) {
  // prepend ForceSet to calls to this method so we can track the caller
  SetBelief(belief, "ForceSet(" + sourceStr + ")");
}

void HabitatDetectorComponent::UpdateDependent(
    const DependencyManagedEntity<RobotComponentID>& dependentComps) {
  ...
  if (cliffSensor.IsCliffDetected() && isPickedUp &&
    ...
    // reset the Cliff-Grey measurement variables when the robot is moved and
    // needs to re-determine whether it is inside the habitat or not
    _numGreyCliffReadingsSincePutdown = 0;
    _minGreyCliffFL = std::numeric_limits<u16>::max();
    _minGreyCliffFR = std::numeric_limits<u16>::max();

    // initialized to zero to force a message to sent down with nominal
    // thresholds upon the next call to UpdateWhiteDetectThreshold() which
    // subsequently calls SendWhiteDetectThresholdsIfNeeded() which will only
    // send if the last sent values differ by a minimum amount, and if enough
    // time has passed since the last message message
    _lastSentWhiteThreshFL = 0;
    _lastSentWhiteThreshFR = 0;
    _timeForWhiteThresholdUpdate_s = 0.0;
  }

  // habitat confirmation (or disconfirmation) can occur if we are in the
  // unknown state
  if (!_robot->GetBatteryComponent().IsOnChargerPlatform() &&
      _habitatBelief == HabitatBeliefState::Unknown && !isPickedUp) {
    ...
    if (_robot->GetPoseOriginList().ContainsOriginID(_poseOriginIdOnPutdown) &&
      ...
      if (_robot->GetPoseOriginList().ContainsOriginID(
              _poseOriginIdOnDelocalize) &&
          !ComputeDistanceBetween(_robot->GetPose(),
                                  _robot->GetPoseOriginList().GetOriginByID(
                                      _poseOriginIdOnDelocalize),
                                  distanceDriven)) {
        // had a valid delocalized origin, but could not compute the
        // displacement
        if (!ComputeDistanceBetween(_robot->GetPose(), _robot->GetWorldOrigin(),
                                    distanceDriven)) {
          ...
          distanceDriven = 0.f;
          PRINT_NAMED_ERROR(
              "HabitatDetectorComponent.UpdateDependent."
              "ComputeDistanceBetweenFailure",
              "Unable to compute distance between robot pose (%s) and the "
              "current world origin (id = %u)",
    ...
    if (!_detectedWhiteFromCliffs &&
        distanceDriven > kMinTravelDistanceWithoutSeeingWhite_mm) {
    ...
    if (numWhite >= 3 || numNone >= 3) {
      ...
      if (numGrey >= 2 && numWhite == 0) {
        ...
        if (cliffCats[0] == White && cliffCats[1] == White) {
          _habitatLineRelPose = HabitatLineRelPose::WhiteFLFR;
        } else if (cliffCats[1] == White && cliffCats[3] == White) {
          _habitatLineRelPose = HabitatLineRelPose::WhiteFRBR;
        } else if (cliffCats[0] == White && cliffCats[2] == White) {
          _habitatLineRelPose = HabitatLineRelPose::WhiteFLBL;
        } else if (cliffCats[2] == White && cliffCats[3] == White) {
          _habitatLineRelPose = HabitatLineRelPose::WhiteBLBR;
        }
        ...
        if (cliffCats[0] == White) {
          _habitatLineRelPose = HabitatLineRelPose::WhiteFL;
        } else if (cliffCats[1] == White) {
          _habitatLineRelPose = HabitatLineRelPose::WhiteFR;
        } else if (cliffCats[2] == White) {
          _habitatLineRelPose = HabitatLineRelPose::WhiteBL;
        } else if (cliffCats[3] == White) {
          _habitatLineRelPose = HabitatLineRelPose::WhiteBR;
        }
      } else {
        _habitatLineRelPose = HabitatLineRelPose::Invalid;
      }
      ...
      if (_habitatLineRelPose == HabitatLineRelPose::WhiteFLFR) {
        ...
        if (proxDataCollectionEnded) {
          if (_proxReadingBuffer.size() >= kMinProxReadingsRequired) {
            ...
            SetBelief(HabitatBeliefState::NotInHabitat,
                      "NotEnoughProxReadings");
          }
        }
      } else {
        // not positioned on the white line => clear the buffer for the next
        // time we are aligned
        _proxReadingBuffer.clear();
      }
    }
...
void HabitatDetectorComponent::UpdateWhiteDetectThreshold(
    const CliffSensorComponent::CliffSensorDataArray& cliffValues) {
  // while the robot is:
  // - moving
  // - on treads
  ...
  if (_robot->GetOffTreadsState() == OffTreadsState::OnTreads &&
    ...
    if (cliffFL >= kCliffGreyMinHabitat && cliffFL <= kCliffGreyMaxHabitat &&
        cliffFL < _minGreyCliffFL) {
      _minGreyCliffFL = cliffFL;
    }
    auto cliffFR = cliffValues[(int)CliffSensor::CLIFF_FR];
    if (cliffFR >= kCliffGreyMinHabitat && cliffFR <= kCliffGreyMaxHabitat &&
        cliffFR < _minGreyCliffFR) {
...
void HabitatDetectorComponent::SendWhiteDetectThresholdsIfNeeded() {
  ...
  const bool enoughReadingsForMin =
      _numGreyCliffReadingsSincePutdown > kMinNumGreyCliffReadings;
  const u16 whiteThreshFL =
      (enoughReadingsForMin ? _minGreyCliffFL : kNominalGreyValue) +
      kWhiteThresholdOffsetFromGrey;
  const u16 whiteThreshFR =
      (enoughReadingsForMin ? _minGreyCliffFR : kNominalGreyValue) +
      kWhiteThresholdOffsetFromGrey;
  const bool whiteThreshChanged =
      (ABS((int)whiteThreshFL - (int)_lastSentWhiteThreshFL) >=
       kMinWhiteThreshChangeToMessage) ||
      (ABS((int)whiteThreshFR - (int)_lastSentWhiteThreshFR) >=
       kMinWhiteThreshChangeToMessage);
  ...
  if (whiteThreshChanged && now >= _timeForWhiteThresholdUpdate_s) {
    ...
    _robot->SendRobotMessage<SetWhiteDetectThresholds>(
        CliffSensorComponent::CliffSensorDataArray{
            {u16(whiteThreshFL), u16(whiteThreshFR),
...
void HabitatDetectorComponent::SendDataToWebViz() const {
  ...
  if (context != nullptr) {
    auto* webService = context->GetWebService();
    if (webService != nullptr &&
        webService->IsWebVizClientSubscribed(kWebVizHabitatModuleName)) {
      _toSendJson["habitatState"] = EnumToString(_habitatBelief);
      _toSendJson["stopOnWhiteEnabled"] =
          _robot->GetCliffSensorComponent().IsStopOnWhiteEnabled() ? "yes"
                                                                   : "no";
