...
CONSOLE_VAR_ENUM(u8, kVisionComponent_Logging, ANKI_CPU_CONSOLEVARGROUP, 0,
                 Util::CpuProfiler::CpuProfilerLogging());
#endif
...
CONSOLE_VAR(bool, kRollingShutterCorrectionEnabled, "Vision.PreProcessing",
            true);
CONSOLE_VAR(f32, kMinCameraGain, "Vision.PreProcessing", 0.1f);

// Set to a value greater than 0 to randomly drop that fraction of frames, for
// testing
CONSOLE_VAR_RANGED(f32, kSimulateDroppedFrameFraction, "Vision.General", 0.f,
                   0.f, 1.f);  // DO NOT COMMIT > 0!

CONSOLE_VAR(bool, kVisualizeObservedMarkersIn3D, "Vision.General", false);
// If > 0, displays detected marker names in Viz Camera Display (still at fixed
// scale) and
CONSOLE_VAR(bool, kDisplayMarkerNames, "Vision.General", false);

// TODO: Is there any way to move this into VisionSystem? (Unlikely, since it
// needs FaceWorld)
...
CONSOLE_VAR(u32, kMaxExpectedTimeBetweenCapturedFrames_ms, "Vision.General",
            500);

void DebugEraseAllEnrolledFaces(ConsoleFunctionContextRef context) {
  LOG_INFO("VisionComponent.ConsoleFunc",
           "DebugEraseAllEnrolledFaces function called");
...
void DebugToggleCameraEnabled(ConsoleFunctionContextRef context) {
  ...
  LOG_INFO("VisionComponent.ConsoleFunc", "Camera %s",
...
VisionComponent::VisionComponent()
    : IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::Vision) {}  // VisionSystem()

void VisionComponent::InitDependent(Vector::Robot* robot,
  ...
  if (nullptr != _context && nullptr != _context->GetExternalInterface()) {
    ...
    auto helper = MakeAnkiEventUtil(*_context->GetExternalInterface(), *this,
                                    _signalHandles);
    ...
    helper
        .SubscribeGameToEngine<MessageGameToEngineTag::EraseAllEnrolledFaces>();
    helper
        .SubscribeGameToEngine<MessageGameToEngineTag::EraseEnrolledFaceByID>();
    helper
        .SubscribeGameToEngine<MessageGameToEngineTag::RequestEnrolledNames>();
    helper
        .SubscribeGameToEngine<MessageGameToEngineTag::LoadFaceAlbumFromFile>();
    helper.SubscribeGameToEngine<MessageGameToEngineTag::SaveFaceAlbumToFile>();
    helper.SubscribeGameToEngine<
        MessageGameToEngineTag::UpdateEnrolledFaceByID>();
    ...
    helper.SubscribeGameToEngine<
        MessageGameToEngineTag::SetCameraCaptureFormat>();

    // Separate list for engine messages to listen to:
    helper.SubscribeEngineToGame<
        MessageEngineToGameTag::RobotConnectionResponse>();
  }
  ...
  if (nullptr != context->GetDataPlatform()) {
    const Json::Value& config =
        context->GetDataLoader()->GetRobotVisionConfig();
    ReadVisionConfig(config);

    if (!config.isMember("InitialModeSchedules")) {
      LOG_ERROR(
          "VisionComponent.InitDependent."
          "MissingInitialModeSchedulesConfigField",
          "");
    }

    const Json::Value& modeSchedulesConfig = config["InitialModeSchedules"];
    const Result result = AllVisionModesSchedule::SetDefaultSchedulesFromJSON(
        modeSchedulesConfig);
    if (RESULT_OK != result) {
      LOG_ERROR(
          "VisionComponent.InitDependent."
          "FailedToInitializeDefaultModeSchedules",
          "");
    }
  }
...
#define GET_JSON_PARAMETER(__json__, __fieldName__, __variable__)              \
  ...
  GET_JSON_PARAMETER(imageQualityConfig, JsonKey::ImageQualityAlertDuration,
                     kImageQualityAlertDuration_ms);
  GET_JSON_PARAMETER(imageQualityConfig, JsonKey::ImageQualityAlertSpacing,
                     kImageQualityAlertSpacing_ms);
  GET_JSON_PARAMETER(imageQualityConfig, JsonKey::InitialExposureTime,
  ...
  GET_JSON_PARAMETER(performanceConfig, JsonKey::DropStatsWindowLength,
                     kDropStatsWindowLength_sec);
...
void VisionComponent::SetCameraCalibration(
    std::shared_ptr<Vision::CameraCalibration> camCalib) {
...
void VisionComponent::Start() {
  ...
  if (_running) {
    LOG_INFO("VisionComponent.Start.Restarting",
             "Thread already started, calling Stop() and then restarting "
             "(paused:%d).",
             _paused);
    Stop();
  } else {
    LOG_INFO("VisionComponent.Start",
             "Starting vision processing thread (paused:%d)", _paused);
  }
...
void VisionComponent::Lock() { _lock.lock(); }

void VisionComponent::Unlock() { _lock.unlock(); }
...
Result VisionComponent::EnableMode(VisionMode mode, bool enable) {
  if (enable) {
    ...
    _robot->GetVisionScheduleMediator().AddAndUpdateVisionModeSubscriptions(
        this, {request});
  } else {
    _robot->GetVisionScheduleMediator().RemoveVisionModeSubscriptions(this,
                                                                      {mode});
  }
  return RESULT_OK;
}
...
static Result GetImageHistState(const Robot& robot,
  ...
  // Handle the (rare, Webots-test-only?) possibility that the image timstamp is
  // _newer_ than the latest thing in history. In that case, we'll just use the
  // last pose information we have, since we can't really interpolate.
  const RobotTimeStamp_t requestedTimeStamp =
      std::min(imageTimeStamp, robot.GetStateHistory()->GetNewestTimeStamp());

  Result lastResult = robot.GetStateHistory()->ComputeStateAt(
      requestedTimeStamp, imageHistTimeStamp, imageHistState, true);
...
void VisionComponent::UpdateDependent(const RobotCompMap& dependentComps) {
  ...
  if (!_enabled) {
    LOG_PERIODIC_INFO(200, "VisionComponent.Update.NotEnabled",
                      "If persistent, camera calibration is probably missing");
    return;
  }

  if (!IsCameraCalibrationSet()) {
    LOG_WARNING(
        "VisionComponent.Update.NoCameraCalibration",
  ...
  if (!_visionSystemInput.locked) {
    ...
    if (_lastReceivedImageTimeStamp_ms > 0) {
      ...
      const bool timeWentBackwards =
          buffer.GetTimestamp() < _lastReceivedImageTimeStamp_ms;
      if (timeWentBackwards) {
        LOG_WARNING("VisionComponent.SetNextImage.UnexpectedTimeStamp",
                    "Current:%u Last:%u", buffer.GetTimestamp(),
                    (TimeStamp_t)_lastReceivedImageTimeStamp_ms);

        // This should be recoverable (it could happen if we receive a bunch of
        // garbage image data) so reset the lastReceived and lastProcessed
        // timestamps so we can set them fresh next time we get an image
      ...
      _framePeriod_ms =
          (TimeStamp_t)(buffer.GetTimestamp() - _lastReceivedImageTimeStamp_ms);
    }
    _lastReceivedImageTimeStamp_ms = buffer.GetTimestamp();

    // Try to get the corresponding historical state
    const bool imageOlderThanOldestState =
        (buffer.GetTimestamp() <
         _robot->GetStateHistory()->GetOldestTimeStamp());
    if (imageOlderThanOldestState) {
      // Special case: we're trying to process an image with a timestamp older
      // than the oldest thing in state history. This can happen at startup, or
      // possibly when we delocalize and clear state history. Just drop this
      // image.
      LOG_INFO("VisionComponent.Update.DroppingImageOlderThanStateHistory",
               "ImageTime=%d OldestState=%d", buffer.GetTimestamp(),
    ...
    const bool haveHistStateAtLeastAsNewAsImage =
...
Result VisionComponent::SetNextImage(Vision::ImageBuffer& buffer) {
  ...
  DEV_ASSERT(!_visionSystemInput.locked,
             "VisionComponent.SetNextImage.AlreadyProcessingImage");

  // Fill in the pose data for the given image, by querying robot history
  HistRobotState imageHistState;
  RobotTimeStamp_t imageHistTimeStamp;

  Result lastResult = GetImageHistState(*_robot, buffer.GetTimestamp(),
                                        imageHistState, imageHistTimeStamp);

  if (lastResult == RESULT_FAIL_ORIGIN_MISMATCH) {
    // Don't print a warning for this case: we expect not to get pose history
    // data successfully
    LOG_INFO(
        "VisionComponent.SetNextImage.OriginMismatch",
  ...
  const Pose3d& cameraPose =
      _robot->GetHistoricalCameraPose(imageHistState, imageHistTimeStamp);
  Matrix_3x3f groundPlaneHomography;
  const bool groundPlaneVisible = LookupGroundPlaneHomography(
      imageHistState.GetHeadAngle_rad(), groundPlaneHomography);

  _visionSystemInput.poseData.Set(
      imageHistTimeStamp, imageHistState, cameraPose, groundPlaneVisible,
      groundPlaneHomography, _robot->GetImuComponent().GetImuHistory());
  ...
  const AllVisionModesSchedule& schedule =
      _robot->GetVisionScheduleMediator().GetSchedule();
  for (VisionMode mode = VisionMode(0); mode < VisionMode::Count; mode++) {
    _visionSystemInput.modesToProcess.Enable(
        mode, schedule.IsTimeToProcess(mode, scheduleCount));
    _visionSystemInput.futureModesToProcess.Enable(
        mode, schedule.GetScheduleForMode(mode).WillEverRun());
  }
  const bool kResetSingleShotModes = true;
  _robot->GetVisionScheduleMediator().AddSingleShotModesToSet(
      _visionSystemInput.modesToProcess, kResetSingleShotModes);
  scheduleCount++;

  // We are all set to process this image so lock input so VisionSystem can use
  // it; then notify the processing thread that the image is ready to be
  // processed
  {
    std::unique_lock<std::mutex> lock{_imageReadyMutex};
...
void VisionComponent::PopulateGroundPlaneHomographyLUT(
    f32 angleResolution_rad) {
  const Pose3d& robotPose = _robot->GetPose();

  DEV_ASSERT(
      _camera->IsCalibrated(),
      "VisionComponent.PopulateGroundPlaneHomographyLUT.CameraNotCalibrated");
  ...
  for (f32 headAngle_rad =
           MIN_HEAD_ANGLE - headAngleSlop_rad;  // Start a little below min
                                                // angle to account for slop
       headAngle_rad <=
       MAX_HEAD_ANGLE + headAngleSlop_rad;  // End a little above max angle to
                                            // account for slop
       headAngle_rad += angleResolution_rad) {
    // Get the robot origin w.r.t. the camera position with the camera at
    // the current head angle
    const Pose3d& cameraPose = _robot->GetCameraPose(
        headAngle_rad);  // need to store the camera pose so it can be parent
    Pose3d robotPoseWrtCamera;
    const bool result =
        robotPose.GetWithRespectTo(cameraPose, robotPoseWrtCamera);
    // this really shouldn't fail! camera has to be in the robot's pose tree
    DEV_ASSERT(result == true,
               "VisionComponent.PopulateGroundPlaneHomographyLUT.GetWrtFailed");
#pragma unused( \
    result)  // Avoid errors in release/shipping when assert compiles out
    ...
    Quad2f imgQuad;
    groundPlaneROI.GetImageQuad(H, calibration->GetNcols(),
                                calibration->GetNrows(), imgQuad);

    const bool isRoiVisible =
        (_camera->IsWithinFieldOfView(imgQuad[Quad::CornerName::TopLeft]) ||
...
bool VisionComponent::LookupGroundPlaneHomography(f32 atHeadAngle,
                                                  Matrix_3x3f& H) const {
  if (!ANKI_VERIFY(!_groundPlaneHomographyLUT.empty(),
                   "VisionComponent.LookupGroundPlaneHomography.EmptyLUT",
                   "")) {
    return false;
  }
  ...
  if (iter == _groundPlaneHomographyLUT.end()) {
    LOG_WARNING("VisionComponent.LookupGroundPlaneHomography.KeyNotFound",
                "Failed to find homography using headangle of %.2frad "
                "(%.1fdeg) as lower bound",
    ...
    if (nextIter != _groundPlaneHomographyLUT.end()) {
      if (std::abs(atHeadAngle - iter->first) >
          std::abs(atHeadAngle - nextIter->first)) {
        iter = nextIter;
      }
    }
  }
...
void VisionComponent::Processor() {
  ...
  Result cvResult =
      SetNumOpencvThreads(_openCvNumThreads, "VisionComponent.Processor");
  ...
  while (_running) {
    ANKI_CPU_TICK(
        "VisionComponent", 100.0,
        Util::CpuProfiler::CpuProfilerLoggingTime(kVisionComponent_Logging));
...

    {
      ...
      std::unique_lock<std::mutex> lock{_imageReadyMutex};
      _imageReadyCondition.wait(
          lock, [this] { return _visionSystemInput.locked || !_running; });
    }
  }  // while(_running)
...
void VisionComponent::VisualizeObservedMarkerIn3D(
    const Vision::ObservedMarker& marker) const {
  ...
  filter.AddFilterFcn([&marker, &quadID, this](const ObservableObject* object) {
    ...
    for (auto& blockMarker : markersWithCode) {
      Pose3d markerPose;
      Result poseResult = marker.GetSeenBy().ComputeObjectPose(
          marker.GetImageCorners(), blockMarker->Get3dCorners(canonicalPose),
          markerPose);
      if (poseResult != RESULT_OK) {
        ...
        if (markerPose.GetWithRespectTo(marker.GetSeenBy().GetPose().FindRoot(),
                                        markerPose) == true) {
          _robot->GetContext()->GetVizManager()->DrawGenericQuad(
              quadID++, blockMarker->Get3dCorners(markerPose),
              NamedColors::OBSERVED_QUAD);
        } else {
          LOG_WARNING(
              "VisionComponent.VisualizeObservedMarkerIn3D."
              "MarkerOriginNotCameraOrigin",
              "Cannot visualize a marker whose pose origin is not the camera's "
              "origin that saw it.");
        }
      }
    }
...
Result VisionComponent::UpdateAllResults() {
  ...
  if (_visionSystem != nullptr) {
    ...
    while (true == _visionSystem->CheckMailbox(result)) {
      if ((_vizManager != nullptr) && _vizManager->IsConnected()) {
      ...
      using LocalHandlerType =
          Result (VisionComponent::*)(const VisionProcessingResult&);
      auto tryAndReport = [this, &result, &anyFailures](
                              LocalHandlerType handler,
                              const VisionModeSet& modes) {
        if (!modes.IsEmpty()) {
      ...
      //  object observations and should be done before sending
      //  RobotProcessedImage below!)
      tryAndReport(&VisionComponent::UpdateVisionMarkers,
                   {VisionMode::Markers});

      // NOTE: UpdateFaces will also update FaceWorld (which broadcasts face
      // observations
      //  and should be done before sending RobotProcessedImage below!)
      tryAndReport(&VisionComponent::UpdateFaces, {VisionMode::Faces});

      // NOTE: UpdatePets will also update PetWorld (which broadcasts pet face
      // observations
      //  and should be done before sending RobotProcessedImage below!)
      tryAndReport(&VisionComponent::UpdatePets, {VisionMode::Pets});

      tryAndReport(&VisionComponent::UpdateMotionCentroid,
                   {VisionMode::Motion});
      tryAndReport(&VisionComponent::UpdateOverheadEdges,
                   {VisionMode::OverheadEdges});
      tryAndReport(&VisionComponent::UpdateComputedCalibration,
                   {VisionMode::Calibration});

      // NOTE: Same handler for two modes
      tryAndReport(&VisionComponent::UpdateCameraParams,
                   {VisionMode::AutoExp, VisionMode::WhiteBalance});

      tryAndReport(&VisionComponent::UpdateLaserPoints, {VisionMode::Lasers});
      tryAndReport(
          &VisionComponent::UpdateSalientPoints,
          {});  // Use empty set here to always call UpdateSalientPoints
      tryAndReport(&VisionComponent::UpdateVisualObstacles,
                   {VisionMode::Obstacles});
      tryAndReport(&VisionComponent::UpdatePhotoManager,
                   {VisionMode::SaveImages});
      tryAndReport(&VisionComponent::UpdateDetectedIllumination,
                   {VisionMode::Illumination});

      // Note: we always run this because it handles switching to the mirror
      // mode debug screen It internally checks whether the MirrorMode flag is
      // set in modesProcessed
      tryAndReport(&VisionComponent::UpdateMirrorMode,
                   {});  // Use empty set to always run

#undef ToVisionModeMask

      // Store frame rate and last image processed time. Time should only move
      // forward.
      DEV_ASSERT(result.timestamp >= _lastProcessedImageTimeStamp_ms,
                 "VisionComponent.UpdateAllResults.BadTimeStamp");
      if (_lastProcessedImageTimeStamp_ms != 0) {
        _processingPeriod_ms =
            (TimeStamp_t)(result.timestamp - _lastProcessedImageTimeStamp_ms);
      }
      ...
      // Send the processed image message last
      {
        ...
        _robot->Broadcast(MessageEngineToGame(
            RobotProcessedImage((TimeStamp_t)result.timestamp,
                                std::move(visionModesList), imageMean)));
      }
    }
  }
...
Result VisionComponent::UpdateVisionMarkers(
    const VisionProcessingResult& procResult) {
  ...
  if (!procResult.observedMarkers.empty()) {
    ...
    lastResult = _robot->GetStateHistory()->ComputeAndInsertStateAt(
        procResult.timestamp, t, &histStatePtr, &histStateKey, true);

    if (RESULT_FAIL_ORIGIN_MISMATCH == lastResult) {
      // Not finding pose information due to an origin mismatch is a normal
      // thing if we just delocalized, so just report everything's cool
      return RESULT_OK;
      ...
      LOG_INFO("VisionComponent.UpdateVisionMarkers.HistoricalPoseNotFound",
               "Time: %u, hist: %u to %u", (TimeStamp_t)procResult.timestamp,
               (TimeStamp_t)_robot->GetStateHistory()->GetOldestTimeStamp(),
               (TimeStamp_t)_robot->GetStateHistory()->GetNewestTimeStamp());
      return RESULT_OK;
    }
    ...
    Vision::Camera histCamera =
        _robot->GetHistoricalCamera(*histStatePtr, procResult.timestamp);

    // Note: we deliberately make a copy of the vision markers in
    // observedMarkers as we loop over them here, because procResult is const
    // but we want to modify each marker to hook up its camera to pose history
    for (auto visionMarker : procResult.observedMarkers) {
      if (visionMarker.GetTimeStamp() != procResult.timestamp) {
        LOG_ERROR("VisionComponent.UpdateVisionMarkers.MismatchedTimestamps",
                  ...
                  visionMarker.GetTimeStamp(),
                  (TimeStamp_t)procResult.timestamp);
        continue;
      }

      // Remove observed markers whose historical poses have become invalid.
      // This shouldn't happen! If it does, robotStateMsgs may be buffering up
      // somewhere. Increasing history time window would fix this, but it's not
      // really a solution.
      if ((visionMarker.GetSeenBy().GetID() == GetCamera().GetID()) &&
          !_robot->GetStateHistory()->IsValidKey(histStateKey)) {
        LOG_WARNING("VisionComponent.Update.InvalidHistStateKey", "key=%d",
                    histStateKey);
        continue;
      }
      ...
      const ColorRGBA& drawColor =
          (visionMarker.GetCode() == Vision::MARKER_UNKNOWN ? NamedColors::BLUE
                                                            : NamedColors::RED);
      ...
      if (kDisplayMarkerNames) {
        ...
        _vizManager->DrawCameraText(
            boundingRect.GetTopLeft(),
            markerName.substr(strlen("MARKER_"), std::string::npos), drawColor);
      }
  ...
  if (!observedMarkers.empty()) {
    _robot->GetDockingComponent().UpdateDockingErrorSignal(
  ...
  for (auto& faceDetection : procResult.faces) {
    /*
     LOG_INFO("VisionComponent.Update",
                      ...
                      "at t=%d Pose: roll=%.1f, pitch=%.1f yaw=%.1f,
     T=(%.1f,%.1f,%.1f).", faceDetection.GetRect().GetX(),
     faceDetection.GetRect().GetY(), faceDetection.GetRect().GetWidth(),
     faceDetection.GetRect().GetHeight(), faceDetection.GetTimestamp(),
    ...
    // Check this before potentially ignoring the face detection for faceWorld's
    // purposes below
...
Result VisionComponent::UpdateMotionCentroid(
  ...
  {
    _robot->Broadcast(
        ExternalInterface::MessageEngineToGame(std::move(motionCentroid)));
  }

  return RESULT_OK;
}  // UpdateMotionCentroid()

Result VisionComponent::UpdateVisualObstacles(
    const VisionProcessingResult& procResult) {
...
Result VisionComponent::UpdateLaserPoints(
    const VisionProcessingResult& procResult) {
  for (auto laserPoint :
       procResult.laserPoints)  // deliberate copy: we move this to the message
  {
    _robot->Broadcast(
        ExternalInterface::MessageEngineToGame(std::move(laserPoint)));
  }

  return RESULT_OK;
}  // UpdateMotionCentroid()

Result VisionComponent::UpdateSalientPoints(
    const VisionProcessingResult& procResult) {
  EngineTimeStamp_t currentTime_ms =
  ...
  if (procResult.modesProcessed.ContainsAnyOf(visionModesUsingNeuralNets) ||
    ...
    if (procResult.salientPoints.size() > 0) {
      _robot->GetAIComponent()
          .GetComponent<SalientPointsComponent>()
          .AddSalientPoints(procResult.salientPoints);
    }
  ...
  for (auto const& unscaledSalientPointToDraw : _salientPointsToDraw) {
    ...
    switch (object.salientType) {
      case Vision::SalientPointType::BrightColors: {
        color = (object.color_rgba == 0) ? NamedColors::BLACK
                                         : ColorRGBA(object.color_rgba);
        caption = object.description + "[" +
                  std::to_string((s32)std::round(object.score)) +
                  "] t:" + std::to_string(object.timestamp);
        break;
      }
      default: {
        color = (object.description.empty())
                    ? NamedColors::RED
                    : ColorRGBA::CreateFromColorIndex(colorIndex++);
        caption = object.description + "[" +
                  std::to_string((s32)std::round(100.f * object.score)) +
                  "] t:" + std::to_string(object.timestamp);
        break;
      }
    }
    ...
    _vizManager->DrawCameraText(Point2f(object.x_img, object.y_img), caption,
                                color);
  }
...
Result VisionComponent::UpdateComputedCalibration(
    const VisionProcessingResult& procResult) {
  DEV_ASSERT(
      (procResult.cameraCalibration.empty() ||
       procResult.cameraCalibration.size() == 1),
  ...
  for (auto& calib : procResult.cameraCalibration) {
    ...
    DEV_ASSERT_MSG(
        msg.distCoeffs.size() == calib.GetDistortionCoeffs().size(),
        "VisionComponent.UpdateComputedCalibration.WrongNumDistCoeffs",
        "Message expects %zu, got %zu", msg.distCoeffs.size(),
        calib.GetDistortionCoeffs().size());

    std::copy(calib.GetDistortionCoeffs().begin(),
              calib.GetDistortionCoeffs().end(), msg.distCoeffs.begin());
...
Result VisionComponent::UpdateCameraParams(
    const VisionProcessingResult& procResult) {
  if (!_robot->IsPhysical() ||
      procResult.imageQuality == Vision::ImageQuality::Unchecked) {
    // Nothing to do
    return RESULT_OK;
  }
  ...
  // Note that we set all parameters together. If WB or AE isn't enabled
  // according to current VisionModes, their corresponding values should not
  // actually be different in the params.
  ...
  if (RESULT_OK == result) {
    LOG_DEBUG("VisionComponent.UpdateImageQuality",
              ...
              params.exposureTime_ms, params.gain, params.whiteBalanceGainR,
              params.whiteBalanceGainG, params.whiteBalanceGainB);

    auto cameraService = CameraService::getInstance();

    const bool isWhiteBalanceEnabled =
        procResult.modesProcessed.Contains(VisionMode::WhiteBalance);
    ...
    const bool isAutoExposureEnabled =
        procResult.modesProcessed.Contains(VisionMode::AutoExp);
    if (isAutoExposureEnabled) {
      cameraService->CameraSetParameters(
          procResult.cameraParams.exposureTime_ms,
...

    {
      ...
      using namespace ExternalInterface;
      const u16 exposure_ms_u16 =
          Util::numeric_cast<u16>(params.exposureTime_ms);
      _robot->Broadcast(MessageEngineToGame(CurrentCameraParams(
          params.gain, exposure_ms_u16, isAutoExposureEnabled)));
    }
  }

  if (procResult.imageQuality != _lastImageQuality ||
      _currentQualityBeginTime_ms == 0) {
    // Just switched image qualities
    _currentQualityBeginTime_ms = procResult.timestamp;
    _waitForNextAlert_ms =
        kImageQualityAlertDuration_ms;  // for first alert, use "duration" time
    _lastBroadcastImageQuality =
        Vision::ImageQuality::Unchecked;  // i.e. reset so we definitely
                                          // broadcast again
  } else if (_lastBroadcastImageQuality !=
             Vision::ImageQuality::Good)  // Don't keep broadcasting once in
                                          // Good state
  {
    const RobotTimeStamp_t timeWithThisQuality_ms =
        procResult.timestamp - _currentQualityBeginTime_ms;

    if (timeWithThisQuality_ms > _waitForNextAlert_ms) {
      // If we get here, we've been in a new image quality for long enough to
      // trust it and broadcast.
      EngineErrorCode errorCode = EngineErrorCode::Count;

      switch (_lastImageQuality) {
        case Vision::ImageQuality::Unchecked:
          // can't get here due to IF above (but need case to prevent compiler
      ...
      LOG_DEBUG(
          ...
          EnumToString(procResult.imageQuality),
          (TimeStamp_t)timeWithThisQuality_ms,
      ...
      // Reset start time just to avoid spamming if this is a "bad" quality,
      // which we will keep sending until it's better. (Good quality is only
      // broadcast one time, once we transition out of Bad quality)
      _currentQualityBeginTime_ms = procResult.timestamp;
      _waitForNextAlert_ms =
          kImageQualityAlertSpacing_ms;  // after first alert use "spacing" time
    }
  }
...
Result VisionComponent::UpdatePhotoManager(
    const VisionProcessingResult& procResult) {
...
Result VisionComponent::UpdateDetectedIllumination(
    const VisionProcessingResult& procResult) {
...
Result VisionComponent::UpdateMirrorMode(
    const VisionProcessingResult& procResult) {
  // Handle switching the debug screen on/off when mirror mode changes
  static bool wasMirrorModeEnabled = false;
  const bool isMirrorModeEnabled =
      procResult.modesProcessed.Contains(VisionMode::MirrorMode);
  if (wasMirrorModeEnabled != isMirrorModeEnabled) {
    ...
    _robot->SendRobotMessage<RobotInterface::EnableMirrorModeScreen>(
        isMirrorModeEnabled);
    wasMirrorModeEnabled = isMirrorModeEnabled;
  }

  // If Mirror is no longer scheduled then don't bother trying to draw anything
  // Normally you would only need to check if MirrorMode was in the processed
  // vision modes, however, due to a timing issue between anim and engine when
  // entering pairing from the mirror mode screen we need to check the schedule
  // instead of processed vision modes. Ex: MirrorMode enabled, double press
  // backpack button, switchboard sends down enter pairing message, engine gets
  // it first and unsubscribes from MirrorMode, anim then gets the message and
  // draws the pairing screen, then on the next tick VisionSystem comes back
  // with a processed image from when MirrorMode was still enabled and we send
  // that image to be drawn on the face overriding the pairing screen. The bug
  // technically lies in anim process for still drawing the face image even
  // though it is on the pairing screens but it feels safer to "fix" it here
  // instead of messing with animStreamer, faceInfoScreenManager, and
  // connectionFlow.
  const AllVisionModesSchedule& schedule =
      _robot->GetVisionScheduleMediator().GetSchedule();
  ...
  if (isMirrorModeEnabled) {
    // NOTE: This creates a non-const image "header" around the same data as is
    // in procResult.mirrorModeImg. Due to a bug / design flaw in OpenCV, this
    // actually allows us to draw on that image, even though it's technically
    // const. Since this is a debug mode, we're using this to avoid a copy in
    // the case that we have eye contact or a display string, since performance
    // is the higher priority here.
    Vision::ImageRGB565 mirrorModeImg = procResult.mirrorModeImg;

    if (!_mirrorModeDisplayString.empty()) {
      mirrorModeImg.DrawText({1, 14}, _mirrorModeDisplayString,
                             _mirrorModeStringColor, 0.6f, true);
    }

    if (kDisplayEyeContactInMirrorMode) {
      const u32 maxTimeSinceSeenFaceToLook_ms =
          ConditionEyeContact::GetMaxTimeSinceTrackedFaceUpdated_ms();
      const bool making_eye_contact = _robot->GetFaceWorld().IsMakingEyeContact(
          maxTimeSinceSeenFaceToLook_ms);
      if (making_eye_contact) {
        ...
        mirrorModeImg.DrawFilledRect(Rectangle<f32>(x, y, width, height),
                                     NamedColors::YELLOW);
      }
    }
    ...
    animComponent.DisplayFaceImage(
        mirrorModeImg, AnimationComponent::DEFAULT_STREAMING_FACE_DURATION_MS,
        kInterruptRunning);
  }
  return RESULT_OK;
}
...
void VisionComponent::AddLiftOccluder(const RobotTimeStamp_t t_request) {
  ...
  RobotTimeStamp_t t;
  Result result =
      _robot->GetStateHistory()->GetRawStateAt(t_request, t, histState, false);

  if (RESULT_FAIL_ORIGIN_MISMATCH == result) {
    // Not a warning, this can legitimately happen
    LOG_INFO(
        "VisionComponent.VisionComponent.AddLiftOccluder."
        "StateHistoryOriginMismatch",
        "Could not get pose at t=%u due to origin change. Skipping.",
  ...
  const Transform3d& liftPoseWrtCamera = _robot->GetLiftTransformWrtCamera(
      histState.GetLiftAngle_rad(), histState.GetHeadAngle_rad());
  ...
  _camera->AddOccluder(liftCrossBarProj,
                       liftPoseWrtCamera.GetTranslation().Length());
}

Vision::ImageRGB GetUndistorted(const Vision::ImageRGB& img,
...
Vision::Image GetUndistorted(const Vision::Image& img,
                             const Vision::Camera& camera) {
...
Result VisionComponent::SendCompressedImage(const Vision::CompressedImage& img,
  ...
  ImageChunk m;
  const bool vizConnected =
      _robot->GetContext()->GetVizManager()->IsConnected();
  ...
  if (displayIndexIter == _vizDisplayIndexMap.end()) {
    ...
    _vizDisplayIndexMap.emplace(
        identifier,
        displayIndex);  // NOTE: this will increase size() for next time
  } else {
  ...
  if (_sendProtoImageChunks) {
    ...
    if (img.GetNumChannels() == 1) {
      imageChunk->set_image_encoding(
          external_interface::ImageChunk_ImageEncoding_JPEG_GRAY);
    } else {
      imageChunk->set_image_encoding(
          external_interface::ImageChunk_ImageEncoding_JPEG_COLOR);
    }
  }
  ...
  while (bytesRemainingToSend > 0) {
    ...
    auto startIt = compressedBuffer.begin() +
                   (compressedBuffer.size() - bytesRemainingToSend);
...
Result VisionComponent::GetCalibrationPoseToRobot(size_t whichPose,
                                                  Pose3d& p) const {
  ...
  if (whichPose >= calibPoses.size()) {
    LOG_WARNING("VisionComponent.WriteCalibrationPoseToRobot.InvalidPoseIndex",
                "Requested %zu, only %zu available", whichPose,
                calibPoses.size());
  } else {
    ...
    if (!calibImages[whichPose].dotsFound) {
      LOG_INFO("VisionComponent.WriteCalibrationPoseToRobot.PoseNotComputed",
               "Dots not found in image %zu, no pose available", whichPose);
    } else {
...
std::list<std::vector<u8> > VisionComponent::GetCalibrationImageJpegData(
    u8* dotsFoundMask) const {
  ...
  for (auto const& calibImage : calibImages) {
    ...
    cv::imencode(".jpg", img.get_CvMat_(), imgVec,
                 std::vector<int>({CV_IMWRITE_JPEG_QUALITY, 50}));

    Vision::Image imgUndistorted(img.GetNumRows(), img.GetNumCols());
    DEV_ASSERT(_camera->IsCalibrated(),
               "VisionComponent.GetCalibrationImageJpegData.NoCalibration");
    img.Undistort(*_camera->GetCalibration(), imgUndistorted);

    std::vector<u8> imgVecUndistort;
    cv::imencode(".jpg", imgUndistorted.get_CvMat_(), imgVecUndistort,
                 std::vector<int>({CV_IMWRITE_JPEG_QUALITY, 50}));
...
Result VisionComponent::ComputeCameraPoseVsIdeal(const Quad2f& obsQuad,
                                                 Pose3d& pose) const {
  ...
  static const Quad3f targetQuad(
      Point3f(-0.5f * kDotSpacingX_mm, -0.5f * kDotSpacingY_mm, 0.f),
      Point3f(-0.5f * kDotSpacingX_mm, 0.5f * kDotSpacingY_mm, 0.f),
      Point3f(0.5f * kDotSpacingX_mm, -0.5f * kDotSpacingY_mm, 0.f),
      Point3f(0.5f * kDotSpacingX_mm, 0.5f * kDotSpacingY_mm, 0.f));

  // Distance between target center and intersection of camera's optical axis
  // with the target, due to 4-degree look-down.
  static const f32 kTargetCenterVertDist_mm = 4.2f;

  // Compute pose of target's center w.r.t. the camera
  Pose3d targetWrtCamera;
  Result result =
      _camera->ComputeObjectPose(obsQuad, targetQuad, targetWrtCamera);
  if (RESULT_OK != result) {
  ...
  // NOTE: 4.2mm is
  //       The center of the target lies along the ideal (unrotated) camera's
  //       optical axis and the target dots are defined in that coordinate frame
  //       (x,y) at upper left.
...
Result VisionComponent::FindFactoryTestDotCentroids(
  ...
  static const f32 kDotArea_pix =
      (f32)(kDotDiameter_pix * kDotDiameter_pix) * 0.25f * M_PI_F;
  ...
  for (s32 iDot = 0; iDot < 4; ++iDot) {
    ...
    Rectangle<s32> roiRect(expectedCenter.x() - kSearchSize_pix / 2,
                           expectedCenter.y() - kSearchSize_pix / 2,
                           kSearchSize_pix, kSearchSize_pix);

    // Use Otsu inverted thresholding to get the dark stuff as "on" with an
    // automatically-computed threshold
    Vision::Image roi;
    image.GetROI(roiRect).CopyTo(roi);
    cv::threshold(roi.get_CvMat_(), roi.get_CvMat_(), 128, 255,
                  cv::THRESH_BINARY_INV | cv::THRESH_OTSU);

    // Find the connected component closest to center (that is of reasonable
    // area) NOTE: The squares of the checkerboard pattern will hopefully be too
    // small
    ...
    const Point2f center(0.5f * (f32)roiRect.GetWidth(),
                         0.5f * (f32)roiRect.GetHeight());
    f32 bestDistFromCenter = std::numeric_limits<f32>::max();
    size_t largestArea = 0;
    s32 bestComp = 0;
    for (s32 iComp = 1; iComp < N;
         ++iComp)  // Note: skipping "background" component 0
    {
      ...
      if (bbox.Area() < kMaxAreaFrac * kDotDiameter_pix * kDotDiameter_pix) {
        ...
        if (currentArea > kDotArea_pix * kMinAreaFrac &&
    ...
    if (bestComp == 0) {
      LOG_WARNING(
          "VisionComponent.FindFactoryTestDotCentroids.NotComponentLargeEnough",
  ...
  if (_camera->IsCalibrated()) {
    // Go ahead and compute the camera pose using the dots if the camera is
    // already calibrated
    ...
    if (RESULT_OK != poseResult) {
      LOG_WARNING(
          "VisionComponent.FindFactoryTestDotCentroids.ComputePoseFailed", "");
    } else {
  ...
  if (kDrawDebugDisplay) {
    ...
    for (s32 iDot = 0; iDot < 4; ++iDot) {
      debugDisp.DrawCircle(
          Point2f(msg.dotCenX_pix[iDot], msg.dotCenY_pix[iDot]),
          NamedColors::RED, 3);

      Rectangle<f32> roiRect(
          kExpectedDotCenters_pix[iDot].x() - kSearchSize_pix / 2,
...
inline static std::string GetFullFaceAlbumPath(const CozmoContext* context,
                                               const std::string& pathIn) {
  const std::string fullPath = context->GetDataPlatform()->pathToResource(
      Util::Data::Scope::Persistent,
      Util::FileUtils::FullFilePath({"faceAlbums", pathIn}));
  return fullPath;
}

Result VisionComponent::SaveFaceAlbum() {
  const std::string fullFaceAlbumPath =
      GetFullFaceAlbumPath(_context, _faceAlbumName);
...
Result VisionComponent::LoadFaceAlbumFromFile(
    const std::string& path, std::list<Vision::LoadedKnownFace>& loadedFaces) {
  Lock();
...
void VisionComponent::AssignNameToFace(Vision::FaceID_t faceID,
                                       const std::string& name,
                                       Vision::FaceID_t mergeWithID) {
  // These are "failsafe" checks to avoid getting duplicate names in the face
  // records. This is a pretty gross, heavy-handed way to achieve this, but is
  // meant to try to catch bugs in the complex EnrollFace behavior in the wild.
  const auto& idsWithName = GetFaceIDsWithName(name);
  if (mergeWithID == Vision::UnknownFaceID) {
    if (!idsWithName.empty()) {
      // This should not happen (caller error?), but for now wallpaper over it
      // and avoid duplicate names polluting the system
      mergeWithID =
          *idsWithName.begin();  // Just use first. Existence of multiple will
                                 // be checked in next 'if'
      std::string idStr;
      ...
      LOG_ERROR("VisionComponent.AssignNameToFace.DuplicateNameWithoutMerge",
                "Name '%s' already in use (IDs:%s) with no mergeID specified. "
                "Forcing merge with ID:%d",
                Util::HidePersonallyIdentifiableInfo(name.c_str()),
                idStr.c_str(), mergeWithID);
    }
  }
  if (mergeWithID !=
      Vision::UnknownFaceID)  // deliberate recheck of mergeWithID, not "else"
  {
    if (idsWithName.size() != 1) {
      // THIS IS VERY BAD! WE HAVE SOMEHOW ENROLLED TWO RECORDS WITH THE SAME
      // NAME!
      std::string idStr;
      for (auto idWithName : idsWithName) {
        idStr += std::to_string(idWithName);
      ...
      LOG_ERROR("VisionComponent.AssignNameToFace.MultipleIDsWithSameName",
                "Found %zu IDs with name '%s': %s", idsWithName.size(),
                Util::HidePersonallyIdentifiableInfo(name.c_str()),
                idStr.c_str());
    }
  }
  ...
  Unlock();
  {
    DASMSG(vision_enrolled_names_new, "vision.enrolled_names.new",
           "A face was assigned a name");
...
void VisionComponent::SetFaceEnrollmentMode(Vision::FaceID_t forFaceID,
                                            s32 numEnrollments,
                                            bool forceNewID) {
...
Result VisionComponent::EraseFace(Vision::FaceID_t faceID) {
  ...
  if (RESULT_OK == result) {
    ...
    _robot->Broadcast(ExternalInterface::MessageEngineToGame(std::move(msg)));
    {
      DASMSG(vision_enrolled_names_erase, "vision.enrolled_names.erase",
             "An enrolled face/name was erased");
      DASMSG_SET(i1, faceID, "The face ID (int)");
...
void VisionComponent::EraseAllFaces() {
  ...
  _robot->Broadcast(ExternalInterface::MessageEngineToGame(
      ExternalInterface::RobotErasedAllEnrolledFaces()));
}
...
void VisionComponent::RequestEnrolledNames() {
  ...
  _robot->Broadcast(
      ExternalInterface::MessageEngineToGame(std::move(response)));
}

Result VisionComponent::RenameFace(Vision::FaceID_t faceID,
                                   const std::string& oldName,
                                   const std::string& newName) {
  if (oldName == newName) {
    LOG_INFO("VisionComponent.RenameFace.SameOldAndNewNames",
             "Ignoring request to rename face %d from %s to %s", faceID,
  ...
  Lock();
  Result result =
      _visionSystem->RenameFace(faceID, oldName, newName, renamedFace);
  int numTotal = static_cast<int>(_visionSystem->GetEnrolledNames().size());
  Unlock();
  ...
  if (RESULT_OK == result) {
    SaveFaceAlbum();
    _robot->Broadcast(
        ExternalInterface::MessageEngineToGame(std::move(renamedFace)));
    {
      DASMSG(vision_enrolled_names_modify, "vision.enrolled_names.modify",
             "An enrolled face/name was modified");
      DASMSG_SET(i1, faceID, "The face ID (int)");
      DASMSG_SET(i2, numTotal, "total number of enrolled faces");
      DASMSG_SEND();
    }
...
std::set<Vision::FaceID_t> VisionComponent::GetFaceIDsWithName(
    const std::string& name) {
  Lock();
...
void VisionComponent::BroadcastLoadedNamesAndIDs(
    const std::list<Vision::LoadedKnownFace>& loadedFaces) const {
  ...
  for (auto& loadedFace : loadedFaces) {
    LOG_INFO("VisionComponent.BroadcastLoadedNamesAndIDs",
             "broadcasting loaded face id: %d", loadedFace.faceID);
...
void VisionComponent::SetAndDisableCameraControl(
    const Vision::CameraParams& params) {
  const Result result = _visionSystem->SetNextCameraParams(params);
  if (RESULT_OK != result) {
    LOG_WARNING(
        "VisionComponent.SetAndDisableCameraControl.SetNextCameraParamsFailed",
        "");
    return;
  }
  ...
  // Directly set the specified camera values, since they won't be coming from
  // the VisionSystem in a VisionProcessingResult anymore. Also manually update
  // Viz
  auto cameraService = CameraService::getInstance();
  if (cameraService) {
    cameraService->CameraSetParameters(params.exposureTime_ms, params.gain);
    cameraService->CameraSetWhiteBalanceParameters(params.whiteBalanceGainR,
                                                   params.whiteBalanceGainG,
                                                   params.whiteBalanceGainB);
...
bool VisionComponent::CaptureImage(Vision::ImageBuffer& buffer) {
  ...
  if (IsCameraCalibrationSet()) {
    ...
    DEV_ASSERT_MSG(numRows == GetCameraCalibration()->GetNrows() &&
                   ...
                   GetCameraCalibration()->GetNcols(), numCols, numRows);
  }

  // Get image buffer. Only request an image from a timestamp for which we have
  // both robot state history _and_ IMU history.
  u32 newestStateHistoryTimeStamp =
      (u32)_robot->GetStateHistory()->GetNewestTimeStamp();
  const auto& imuHistory = _robot->GetImuComponent().GetImuHistory();
  if (!imuHistory.empty()) {
    newestStateHistoryTimeStamp =
        std::min(newestStateHistoryTimeStamp, (u32)imuHistory.back().timestamp);
  }
  const bool gotImage =
      cameraService->CameraGetFrame(newestStateHistoryTimeStamp, buffer);
  ...
  if (gotImage) {
    ...
    // No image captured, and we're not currently waiting for a format change,
    // nor are we in power save mode
    auto const& powerStateMgr = _robot->GetComponent<PowerStateManager>();
    if (!IsWaitingForCaptureFormatChange() &&
        !powerStateMgr.InPowerSaveMode() && (_lastImageCaptureTime_ms > 0) &&
        (currTime_ms >
         _lastImageCaptureTime_ms + kMaxExpectedTimeBetweenCapturedFrames_ms)) {
      LOG_WARNING("VisionComponent.CaptureImage.TooLongSinceFrameWasCaptured",
                  "last: %dms, now: %dms",
                  (TimeStamp_t)_lastImageCaptureTime_ms,
...
void VisionComponent::SetLiftCrossBar() {
  ...
  const std::vector<Point3f> liftCrossBar{
      // NOTE: adding points for front and back because which will be outermost
      // in projection
      //       depends on lift angle, so let Occluder, which uses bounding box
      //       of all the
      //       points, take care of that for us

      // Top:
      Point3f(LIFT_FRONT_WRT_WRIST_JOINT, -LIFT_XBAR_WIDTH * 0.5f,
              LIFT_XBAR_HEIGHT_WRT_WRIST_JOINT),
      Point3f(LIFT_FRONT_WRT_WRIST_JOINT, LIFT_XBAR_WIDTH * 0.5f,
              LIFT_XBAR_HEIGHT_WRT_WRIST_JOINT),
      Point3f(LIFT_BACK_WRT_WRIST_JOINT, -LIFT_XBAR_WIDTH * 0.5f,
              LIFT_XBAR_HEIGHT_WRT_WRIST_JOINT),
      Point3f(LIFT_BACK_WRT_WRIST_JOINT, LIFT_XBAR_WIDTH * 0.5f,
              LIFT_XBAR_HEIGHT_WRT_WRIST_JOINT),
      ...
      // Bottom
      Point3f(LIFT_FRONT_WRT_WRIST_JOINT, -LIFT_XBAR_WIDTH * 0.5f,
              LIFT_XBAR_BOTTOM_WRT_WRIST_JOINT - padding),
      Point3f(LIFT_FRONT_WRT_WRIST_JOINT, LIFT_XBAR_WIDTH * 0.5f,
              LIFT_XBAR_BOTTOM_WRT_WRIST_JOINT - padding),
      Point3f(LIFT_BACK_WRT_WRIST_JOINT, -LIFT_XBAR_WIDTH * 0.5f,
              LIFT_XBAR_BOTTOM_WRT_WRIST_JOINT - padding),
...
bool VisionComponent::SetCameraCaptureFormat(Vision::ImageEncoding format) {
  ...
  LOG_INFO("VisionComponent.SetCameraCaptureFormat.RequestingSwitch",
           "From %s to %s", ImageEncodingToString(currentFormat),
  ...
  switch (_captureFormatState) {
    ...
    case CaptureFormatState::WaitingForProcessingToStop: {
      ...
      if (!_visionSystemInput.locked) {
        ...
        LOG_INFO(
            "VisionComponent.UpdateCaptureFormatChange.SwitchToWaitForFrame",
    ...
    case CaptureFormatState::WaitingForFrame: {
      LOG_INFO(
          "VisionComponent.UpdateCaptureFormatChange."
          "WaitingForFrameWithNewFormat",
          "");
      ...
      switch (_desiredImageFormat) {
        ...
        default:
          LOG_ERROR(
              "VisionComponent.UpdateCaptureFormatChange.BadDesiredFormat",
              "%s", ImageEncodingToString(_desiredImageFormat));
          return;
      }
      ...
      if (gotNumRows == expectedNumRows) {
        DEV_ASSERT(_paused,
                   "VisionComponent.UpdateCaptureFormatChange."
                   "ExpectingVisionComponentToBePaused");

        LOG_INFO(
            "VisionComponent.UpdateCaptureFormatChange.FormatChangeComplete",
            "New format: %s, NumRows=%d",
            ImageEncodingToString(_desiredImageFormat), gotNumRows);

        _captureFormatState = CaptureFormatState::None;
        _desiredImageFormat = Vision::ImageEncoding::NoneImageEncoding;
        Pause(false);  // now that state/format are updated, un-pause the vision
                       // system
      }
...
template <>
void VisionComponent::HandleMessage(
    const ExternalInterface::EnableVisionMode& payload) {
  EnableMode(payload.mode, payload.enable);
}

template <>
void VisionComponent::HandleMessage(
    const ExternalInterface::VisionWhileMoving& msg) {
  EnableVisionWhileRotatingFast(msg.enable);
}

template <>
void VisionComponent::HandleMessage(
    const ExternalInterface::VisionRunMode& msg) {
  SetIsSynchronous(msg.isSync);
}

template <>
void VisionComponent::HandleMessage(
    const ExternalInterface::EraseEnrolledFaceByID& msg) {
  EraseFace(msg.faceID);
}

template <>
void VisionComponent::HandleMessage(
    const ExternalInterface::EraseAllEnrolledFaces& msg) {
  EraseAllFaces();
}

template <>
void VisionComponent::HandleMessage(
    const ExternalInterface::RequestEnrolledNames& msg) {
  RequestEnrolledNames();
}

template <>
void VisionComponent::HandleMessage(
    const ExternalInterface::SaveFaceAlbumToFile& msg) {
  // NOTE: Renames/deletes will propagate to the _faceAlbumName loaded from
  // vision_config! (not msg.path)
...
template <>
void VisionComponent::HandleMessage(
    const ExternalInterface::LoadFaceAlbumFromFile& msg) {
  // NOTE: Renames/deletes will propagate to the _faceAlbumName loaded from
  // vision_config! (not msg.path)
  if (msg.isRelativePath) {
...
template <>
void VisionComponent::HandleMessage(
    const ExternalInterface::UpdateEnrolledFaceByID& msg) {
  RenameFace(msg.faceID, msg.oldName, msg.newName);
}

template <>
void VisionComponent::HandleMessage(
    const ExternalInterface::EnableColorImages& msg) {
...
template <>
void VisionComponent::HandleMessage(
    const ExternalInterface::SetCameraSettings& payload) {
  if (payload.enableAutoExposure) {
    ...
    Vision::CameraParams params(
        payload.exposure_ms, payload.gain, currentParams.whiteBalanceGainR,
        currentParams.whiteBalanceGainG, currentParams.whiteBalanceGainB);
  ...
  if (nullptr != _visionSystem) {
    ...
    if (fullPath.empty()) {
      ...
      const std::string cachePath =
          _robot->GetContext()->GetDataPlatform()->pathToResource(
              Util::Data::Scope::Cache, "camera");
...
template <>
void VisionComponent::HandleMessage(
    const ExternalInterface::SaveImages& payload) {
  ImageSaverParams params(payload.path, payload.mode, payload.qualityOnRobot,
                          "", Vision::ImageCacheSize::Half, 0.f, 1.f,
...
template <>
void VisionComponent::HandleMessage(
    const ExternalInterface::SetCameraCaptureFormat& msg) {
  SetCameraCaptureFormat(msg.format);
}

template <>
void VisionComponent::HandleMessage(
    const ExternalInterface::RobotConnectionResponse& msg) {
  if (msg.result == RobotConnectionResult::Success) {
    NVStorageComponent::NVStorageReadCallback readCamCalibCallback =
          ...
          if (res == NVStorage::NVResult::NV_OKAY) {
            ...
            if (size != NVStorageComponent::MakeWordAligned(payload.Size())) {
              LOG_WARNING("VisionComponent.ReadCameraCalibration.SizeMismatch",
                          "Expected %zu, got %zu",
                          NVStorageComponent::MakeWordAligned(payload.Size()),
                          size);
              ...
              LOG_INFO("VisionComponent.ReadCameraCalibration.Recvd",
                       "Received new %dx%d camera calibration from robot. (fx: "
                       "%f, fy: %f, cx: %f, cy: %f, distCoeffs: %s)",
                       payload.ncols, payload.nrows, payload.focalLength_x,
                       payload.focalLength_y, payload.center_x,
                       payload.center_y, ss.str().c_str());

              // Convert calibration data in payload to a shared
              // CameraCalibration object
              auto calib = std::make_shared<Vision::CameraCalibration>(
                  payload.nrows, payload.ncols, payload.focalLength_x,
                  payload.focalLength_y, payload.center_x, payload.center_y,
                  payload.skew, payload.distCoeffs);

              SetCameraCalibration(calib);

              // Compute FOV from focal length and send
              CameraFOVInfo msg(calib->ComputeHorizontalFOV().ToFloat(),
                                calib->ComputeVerticalFOV().ToFloat());
              if (_robot->SendMessage(RobotInterface::EngineToRobot(
                      std::move(msg))) != RESULT_OK) {
                LOG_WARNING(
                    "VisionComponent.ReadCameraCalibration.SendCameraFOVFailed",
                    "");
              }
            }
          }
          // If this is the factory test and we failed to read calibration then
          // use a dummy one since we should be getting a real one during
          // playpen
          else if (FACTORY_TEST) {
            LOG_WARNING("VisionComponent.ReadCameraCalibration.Failed", "");

            // TEMP HACK: Use dummy calibration for now since final camera not
            // available yet
            LOG_WARNING(
                "VisionComponent.ReadCameraCalibration.UsingDummyV2Calibration",
                "");

            // Calibration computed from Inverted Box target using one of the
            // proto robots Should be close enough for other robots without
            // calibration to use
            const std::array<f32, 8> distortionCoeffs = {
                {-0.03822904514363595, -0.2964213946476391,
                 -0.00181089972406104, 0.001866070303033584, 0.1803429725181202,
                 0, 0, 0}};

            auto calib = std::make_shared<Vision::CameraCalibration>(
                360, 640, 364.7223064012286, 366.1693698832141,
                310.6264440545544, 196.6729350209868, 0, distortionCoeffs);
    ...
    _robot->GetNVStorageComponent().Read(
        NVStorage::NVEntryTag::NVEntry_CameraCalib, readCamCalibCallback);
  }
}
...
void VisionComponent::SendImages(VisionProcessingResult& result) {
  if (ANKI_DEV_CHEATS) {
    ...
    LOG_ERROR("VisionComponent.UpdateAllResults.DebugImagesPresent", "%s",
              str.c_str());
  }
...
void VisionComponent::UpdateForCalibration() {
  ...
  // Store image for calibration or factory test (*before* we swap image with
  // _nextImg below!) NOTE: This means we do decoding on main thread, but this
  // is just for the factory
  //       test, so I'm not going to the trouble to store encoded images for
  //       calibration
  // if (_storeNextImageForCalibration || _doFactoryDotTest)
  // {
  //   // If we were moving too fast at the timestamp the image was taken then
  //   don't use it for
  ...
  //       } else {
  //         Lock();
  //         Result result = _visionSystem->AddCalibrationImage(imageGray,
  //         _calibTargetROI); Unlock();
...
void VisionComponent::EnableImageCapture(bool enable) {
  LOG_INFO("VisionComponent.EnableImageCapture", "%s image capture",
...
void VisionComponent::SetupVisionModeConsoleVars() {
  ...
  for (auto m = VisionMode(0); m < VisionMode::Count; m++) {
    ...
    pair.first = new Util::ConsoleVar<bool>(pair.second, EnumToString(m),
                                            "Vision.General.VisionModes", true);
  }
#endif
}
...
void VisionComponent::UpdateVisionModeConsoleVars() {
  ...
  for (int i = 0; i < _visionModeConsoleVars.size(); i++) {
    ...
    if (pair.second != prevConsoleVars[i]) {
      // Enable/disable the vision mode as well as subscribing/unsubscribing to
      // the vision mode
      EnableMode(static_cast<VisionMode>(i), pair.second);

      // Need to subscribe/unsubscribe as when you are enabling/disabling
      // VisionModes with the console var it is very likely not scheduled. The
      // regular subscription mechanism will remove all subscriptions to all
      // modes for the subscriber as it accepts a full list of all desired
      // subscriptions. The DevOnly calls allow you to update individual mode
      // subscriptions for the subscriber.
      if (prevConsoleVars[i]) {
        _robot->GetVisionScheduleMediator().DevOnly_SelfUnsubscribeVisionMode(
            {static_cast<VisionMode>(i)});
      } else {
        _robot->GetVisionScheduleMediator().DevOnly_SelfSubscribeVisionMode(
            {static_cast<VisionMode>(i)});
      }
...
void VisionComponent::UpdateImageReceivedChecks(bool gotImage) {
  // Display a fault code in case we have not received an image from the camera
  // for some amount of time and we aren't in power save mode in which case we
  // expect to not be receiving images and image capture is enabled This is a
  // catch-all for any issues with the camera server/daemon should something go
  // wrong and we stop receiving images
  static const EngineTimeStamp_t kNoImageFaultCodeTimeout_ms = 60000;

  // If we haven't got an image in this amount of time, try deleting and
  // restarting the camera service just in case that knocks something loose
  static const EngineTimeStamp_t kNoImageRestartCamera_ms =
      kNoImageFaultCodeTimeout_ms / 4;
  static const EngineTimeStamp_t kRestartCameraDelay_ms = 1000;
  static EngineTimeStamp_t sTimeSinceValidImg_ms = 0;

  const bool inPowerSave =
      _robot->GetComponent<PowerStateManager>().InPowerSaveMode();
  if (!gotImage && !inPowerSave && _enableImageCapture) {
    const EngineTimeStamp_t curTime_ms =
        BaseStationTimer::getInstance()->GetCurrentTimeStamp();
    if (sTimeSinceValidImg_ms == 0) {
      sTimeSinceValidImg_ms = curTime_ms;
    }
    // If it has been way too long since we last received an image, show a fault
    // code
    ...
    // If we haven't received an image for some amount of time, try restarting
    // camera
      ...
      // Stopping/Starting are asynchronous so we need to wait a bit between the
      // calls
      else if (_restartingCameraTime_ms != 0 &&
               curTime_ms - _restartingCameraTime_ms > kRestartCameraDelay_ms) {
        // Prevent the camera restart checks from triggering again until we
        // either start getting images again or the CAMERA_STOPPED fault code
        // triggers
...
void VisionComponent::EnableSendingSDKImageChunks(
    bool enableImageStreaming, bool enableHighResolutionImages) {
