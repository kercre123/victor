...
class HabitatDetectorComponent
 ...
 public:
  static constexpr int kNumCliffSensors =
  ...
  virtual void UpdateDependent(
      const DependencyManagedEntity<RobotComponentID>& dependentComps) override;
  ...
  // callers of this code may read the sub-state and take special actions to
  // make the state converge to the best state available to for grabbing prox
  // sensor data when in the habitat
  HabitatLineRelPose GetHabitatLineRelPose() const {
    return _habitatLineRelPose;
  }
 ...
 protected:
  ...
  void UpdateWhiteDetectThreshold(
 ...
 private:
  enum HabitatCliffReadingType { None = 0x00, Grey = 0x01, White = 0x02 };
  ...
  // current possible configurations of the robot with respect to habitat
  // white-line feature
  ...
  // readings aggregated from the prox sensor while the front two cliffs are
  // positioned on top of the white line of the habitat, used to compute an
  // average distance
  std::vector<u16> _proxReadingBuffer = {};

  // counter for the number of ticks the robot has been collecting readings from
  // prox
  ...
  // number of valid grey cliff readings accumulated so far (for computing the
  // min)
