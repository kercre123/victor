 ...
 * Created: 05/03/2018
 *
 * Description: Component for managing beat/tempo detection (which is actually
 *done in the anim process)
 *
...
CONSOLE_VAR_RANGED(float, kConfidenceThreshold, CONSOLE_GROUP, 0.18f, 0.01f,
                   1.f);

// If confidence is above this value, it's very likely that a beat is _actually_
// being detected
CONSOLE_VAR_RANGED(float, kHighConfidenceThreshold, CONSOLE_GROUP, 0.75f, 0.01f,
                   20.f);

// Length of time to keep a history of detected beats
CONSOLE_VAR_RANGED(float, kBeatHistoryWindowSize_sec, CONSOLE_GROUP, 10.f, 1.f,
                   60.f);
...
CONSOLE_VAR_RANGED(float, kPossibleBeatWindow_sec, CONSOLE_GROUP, 9.f, 1.f,
                   10.f);

// Used to fake a detected beat. If greater than 0, then a series of confident
// beats is created at the given bpm and confidence.
CONSOLE_VAR_RANGED(float, kFakeBeatConfidence, CONSOLE_GROUP, 0.50f, 0.f,
                   100.f);
...
BeatDetectorComponent::BeatDetectorComponent()
    : IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::BeatDetector) {}

BeatDetectorComponent::~BeatDetectorComponent() {}

void BeatDetectorComponent::GetInitDependencies(
    RobotCompIDSet& dependencies) const {
  dependencies.insert(RobotComponentID::CozmoContextWrapper);
}

void BeatDetectorComponent::InitDependent(Vector::Robot* robot,
                                          const RobotCompMap& dependentComps) {
  _robot = robot;

  auto* messageHandler = robot->GetRobotMessageHandler();
  _signalHandle = messageHandler->Subscribe(
      RobotInterface::RobotToEngineTag::beatDetectorState,
...
void BeatDetectorComponent::UpdateDependent(
    const RobotCompMap& dependentComps) {
  const float now_sec =
  ...
  if (kFakeBeat_bpm > 0.f) {
    ...
    const float lastFakeBeatTime_sec =
        _recentBeats.empty() ? 0.f : _recentBeats.back().time_sec;
    ...
    if (now_sec > lastFakeBeatTime_sec + fakeBeatPeriod_sec) {
      ...
      beat.time_sec = _recentBeats.empty()
                          ? now_sec
                          : lastFakeBeatTime_sec + fakeBeatPeriod_sec;
      OnBeat(beat);
    }
  }
...
bool BeatDetectorComponent::IsPossibleBeatDetected() const {
  ...
  // In order for a possible beat to be considered detected, the following
  // conditions must be true. For the past kPossibleBeatWindow_sec seconds:
  //   - We have a minimum number of beats
  //   - Tempo has been steady for kPossibleBeatWindow_sec
  //   - The most recent beat in the history has a confidence above a minimum
  //   threshold
  //   - All beats in the past kPossibleBeatWindow_sec have a confidence of at
  ...
  auto earliestBeatIt =
      std::lower_bound(_recentBeats.begin(), _recentBeats.end(),
  ...
  // Do all the beats in the past kPossibleBeatWindow_sec have a confidence of
  // at least half the threshold?
  const bool allConfidenceHighEnough =
      std::all_of(earliestBeatIt, _recentBeats.end(), [](const BeatInfo& b) {
        return b.confidence > 0.5f * kConfidenceThreshold;
      });

  const bool latestConfidenceHighEnough =
      _recentBeats.back().confidence > kConfidenceThreshold;

  const auto minMaxTempoItPair =
      std::minmax_element(earliestBeatIt, _recentBeats.end(),
                          [](const BeatInfo& b1, const BeatInfo& b2) {
...
bool BeatDetectorComponent::IsBeatDetected() const {
  // In order for a _definite_ beat to be considered detected, the following
  // conditions must be true, for the entire beat history:
  //   - We have a minimum number of beats
  //   - Tempo has been steady the entire time
  //   - All beats in the history have a confidence above a minimum threshold OR
  //   the latest beat has a very high
  //     confidence.
  ...
  const bool confidenceHighEnough =
      (_recentBeats.back().confidence > kHighConfidenceThreshold) ||
      std::all_of(_recentBeats.begin(), _recentBeats.end(),
                  [](const BeatInfo& b) {
                    return b.confidence > kConfidenceThreshold;
                  });

  const auto minMaxTempoItPair =
      std::minmax_element(_recentBeats.begin(), _recentBeats.end(),
                          [](const BeatInfo& b1, const BeatInfo& b2) {
                            return b1.tempo_bpm < b2.tempo_bpm;
                          });
  const bool tempoSteady = Util::IsNear(minMaxTempoItPair.second->tempo_bpm,
...
float BeatDetectorComponent::GetCurrConfidence() const {
  if (!IsPossibleBeatDetected() || _recentBeats.empty()) {
    PRINT_NAMED_WARNING(
        "BeatDetectorComponent.GetCurrConfidence.NoBeatDetected",
        "No current beat is detected");
    return 0.f;
  }
...
void BeatDetectorComponent::Reset() {
  ...
  _robot->SendMessage(
      RobotInterface::EngineToRobot(RobotInterface::ResetBeatDetector()));
}

int BeatDetectorComponent::RegisterOnBeatCallback(
    const OnBeatCallback& callback) {
...
bool BeatDetectorComponent::UnregisterOnBeatCallback(const int callbackId) {
  ...
  const bool success =
      ANKI_VERIFY(numErased == 1,
                  "BeatDetectorComponent.UnregisterOnBeatCallback."
                  "FailedToUnregisterCallback",
                  "Failed to erase callback with ID %d (num erased %zu)",
                  callbackId, numErased);
  return success;
}

void BeatDetectorComponent::SendToWebViz() {
  if (auto webSender = WebService::WebVizSender::CreateWebVizSender(
          "beatdetector", _robot->GetContext()->GetWebService())) {
    const float now_sec = static_cast<float>(
        Util::Time::UniversalTime::GetCurrentTimeInSeconds());
    auto& jsonData = webSender->Data();
    Json::Value detectorJson;
    detectorJson["possibleBeatDetected"] =
        IsPossibleBeatDetected() ? "yes" : "no";
    detectorJson["beatDetected"] = IsBeatDetected() ? "yes" : "no";
    detectorJson["latestTempo_bpm"] =
        _recentBeats.empty() ? -1.f : _recentBeats.back().tempo_bpm;
    detectorJson["latestConf"] =
        _recentBeats.empty() ? -1.f : _recentBeats.back().confidence;
...
BeatInfo BeatDetectorComponent::TEST_fakeLowConfidenceBeat(
    const float tempo_bpm, const float time_sec) {
...
BeatInfo BeatDetectorComponent::TEST_fakeHighConfidenceBeat(
    const float tempo_bpm, const float time_sec) {
...
BeatInfo BeatDetectorComponent::TEST_fakeVeryHighConfidenceBeat(
    const float tempo_bpm, const float time_sec) {
