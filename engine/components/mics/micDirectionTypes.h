...
enum {
  kInvalidMicDirectionIndex = std::numeric_limits<MicDirectionIndex>::max()
};
...
// a Sound Reactor needs to register this callback function which is called each
// time we record a valid mic power sample arguments = ( micPowerLevel,
// micConfidenceValue, micDirection )
using OnMicPowerSampledCallback =
    std::function<bool(double, MicDirectionConfidence, MicDirectionIndex)>;
