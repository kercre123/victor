...
MicComponent::MicComponent()
    : IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::MicComponent),
...
void MicComponent::GetInitDependencies(RobotCompIDSet& dependencies) const {
  // we could allow our sub-systems to add to this, but it's simple enough at
  // this point
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void MicComponent::InitDependent(Vector::Robot* robot,
...
void MicComponent::SetBufferFullness(float val) {
  if (!Util::InRange(val, 0.0f, 1.0f)) {
    PRINT_NAMED_WARNING("MicComponent.SetBufferFullness.InvalidValue",
                        "Fullness value %f invalid, must be [0, 1]", val);
    _fullness = 0.0f;
  } else {
    _fullness = val;
  }
