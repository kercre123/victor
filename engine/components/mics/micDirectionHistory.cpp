...
namespace Vector {
namespace {
const EngineTimeStamp_t kVadSessionLength_ms =
    (30 * 60 * 1000);  // 30 mins == 1,800,000 ms
}

CONSOLE_VAR_RANGED(uint32_t, kRTS_PowerAvgNumSamples, "SoundReaction", 100, 1,
                   250);
CONSOLE_VAR_RANGED(double, kRTS_WebVizUpdateInterval, "SoundReaction", 0.20,
                   0.0, 1.0);

// NOTE: these are only for display purposes now (actual values live in instance
// jsons)
...
static_assert(std::is_same<decltype(RobotInterface::MicDirection::timestamp),
                           TimeStamp_t>::value,
              "Expecting type of MicDirection::timestamp to match TimeStamp_t");
static_assert(
    std::is_same<decltype(RobotInterface::MicDirection::direction),
...
static_assert(std::is_same<decltype(RobotInterface::MicDirection::confidence),
              ...
              "Expecting type of MicDirection::confidence to match "
              "MicDirectionConfidence");
...
void MicDirectionHistory::PrintNodeData(uint32_t index) const {
  const MicDirectionNode& node =
      _micDirectionBuffer[GetPrevNodeIdx(GetNextNodeIdx(index))];
  PRINT_NAMED_INFO("MicDirectionHistory::PrintNodeData",
                   "idx: %d ts: %d dir: %d confAvg: %d count: %d", index,
                   (TimeStamp_t)node.timestampEnd, node.directionIndex,
                   node.confidenceAvg, node.count);
}

void MicDirectionHistory::AddMicSample(
    const RobotInterface::MicDirection& message) {
  ...
  AddDirectionSample(currentTime_ms, (message.activeState == 1),
                     message.direction, message.confidence,
                     message.selectedDirection);
  AddMicPowerSample(currentTime_ms, message.latestPowerValue,
                    message.latestNoiseFloor);
  AddVadLogSample(currentTime_ms, message.latestNoiseFloor,
                  message.activeState);
...
MicDirectionIndex MicDirectionHistory::GetDirectionAtTime(
    EngineTimeStamp_t timestampEnd, TimeStamp_t timeLength_ms) const {
  // Special case where we're asked for time after or at the end of history
  {
    ...
    if (timestampEnd >= currentEntry.timestampEnd) {
      const auto timeDiff =
          TimeStamp_t(timestampEnd - currentEntry.timestampEnd);
      // Update our timelength to remove time not yet recorded in direction
      // history
  ...
  while (prevToEndingIndex != _micDirectionBufferIndex &&
         _micDirectionBuffer[prevToEndingIndex].timestampEnd > timestampEnd) {
  ...
  const auto timeSpentInEndNode = TimeStamp_t(
      timestampEnd - _micDirectionBuffer[prevToEndingIndex].timestampEnd);
  ...
  DirectionHistoryCount directionCount =
      GetDirectionCountAtIndex(prevToEndingIndex, timePrevToNode);

  // Add in the partial count based on leftover time
  const auto fullNodeLength =
      endingNode.timestampEnd -
      _micDirectionBuffer[prevToEndingIndex].timestampEnd;
  const auto proportionCount =
      (timeSpentInEndNode * endingNode.count) / (TimeStamp_t)fullNodeLength;
  directionCount[endingNode.directionIndex] += proportionCount;

  return GetBestDirection(directionCount);
}

MicDirectionIndex MicDirectionHistory::GetRecentDirection(
  ...
  DirectionHistoryCount directionCount =
      GetDirectionCountAtIndex(startIndex, timeLength_ms);
  return GetBestDirection(directionCount);
}

MicDirectionHistory::DirectionHistoryCount
MicDirectionHistory::GetDirectionCountAtIndex(uint32_t startIndex,
                                              TimeStamp_t timeLength_ms) const {
  DirectionHistoryCount directionCount{};
  ...
  while (timeLength_ms > 0) {
    ...
    const auto nodeTimeDiff =
        TimeStamp_t(currentNode.timestampEnd - prevNode.timestampEnd);

    // If we're using up the last of the time we're counting over, do a
    // proportional count and break
    if (nodeTimeDiff >= timeLength_ms) {
      const TimeStamp_t divisor = nodeTimeDiff > 0 ? nodeTimeDiff : 1;
      const auto proportionCount =
          (timeLength_ms * currentNode.count) / divisor;
...
MicDirectionIndex MicDirectionHistory::GetBestDirection(
    const DirectionHistoryCount& directionCount) {
  MicDirectionIndex bestIndex = kMicDirectionUnknown;
  uint32_t bestCount = 0;
  for (MicDirectionIndex i = kFirstMicDirectionIndex; i < kNumMicDirections;
       ++i)  // Don't consider unknown direction
  {
...
MicDirectionNodeList MicDirectionHistory::GetRecentHistory(
    TimeStamp_t timeLength_ms) const {
  const auto startIndex = _micDirectionBufferIndex;
...
MicDirectionNodeList MicDirectionHistory::GetHistoryAtTime(
    EngineTimeStamp_t timestampEnd, TimeStamp_t timeLength_ms) const {
  // Special case where we're asked for time after or at the end of history
  {
    auto& currentEntry = _micDirectionBuffer[_micDirectionBufferIndex];
    if (timestampEnd >= currentEntry.timestampEnd) {
      ...
      // history
      timeLength_ms =
          timeDiff > timeLength_ms ? 0 : TimeStamp_t(timeLength_ms - timeDiff);
  ...
  MicDirectionNodeList results =
      GetHistoryAtIndex(prevToEndingIndex, timePrevToNode);
  if (endingNode.IsValid()) {
...
MicDirectionNodeList MicDirectionHistory::GetHistoryAtIndex(
    uint32_t startIndex, TimeStamp_t timeLength_ms) const {
  MicDirectionNodeList results;
  uint32_t curIndex = startIndex;
  while (timeLength_ms > 0) {
    const auto& currentNode = _micDirectionBuffer[curIndex];
...
SoundReactorId MicDirectionHistory::RegisterSoundReactor(
...
void MicDirectionHistory::UnRegisterSoundReactor(SoundReactorId id) {
  ...
  PRINT_NAMED_WARNING("MicDirectionHistory.UnRegisterSoundReactor",
                      "No SoundReactor found with id %d", (int)id);
}

void MicDirectionHistory::AddDirectionSample(
    EngineTimeStamp_t timestamp, bool isVadActive,
    MicDirectionIndex newDirection, MicDirectionConfidence newConf,
  ...
  if (_micDirectionBuffer[_micDirectionBufferIndex].directionIndex ==
      newDirection) {
    auto& currentEntry = _micDirectionBuffer[_micDirectionBufferIndex];
    ...
    const auto weightedConf = Util::numeric_cast<uint32_t>(
        (curAvgConf * currentEntry.count) + newConf);
    const auto newAvgConf = Util::numeric_cast<MicDirectionConfidence>(
        weightedConf / (currentEntry.count + 1));
...
void MicDirectionHistory::AddMicPowerSample(EngineTimeStamp_t timestamp,
                                            float powerLevel,
                                            float noiseFloor) {
  ...
  if (previousPowerLevel != 0.0) {
    // we want to only track the "average peak magnitude" of the mic power
    // level, as it fluctuates wildly each sample. we are trying to determine
    // when the power level spikes high outside of the "average peak magnitude"
    // to find the "peak magnitude", we need to essentially find the local
    // maximum of the power samples
    const double powerLevelDelta =
        (_soundTrackingData.latestPowerLevel - previousPowerLevel);
    if (!Util::IsNearZero(powerLevelDelta)) {
      // if we were currently increasing in value, and now we're decreasing,
      // consider this our local maximum
      const bool isIncreasing = (powerLevelDelta > 0.0f);
      if (!isIncreasing &&
          _soundTrackingData.increasing) {  // we've gone from increasing ->
                                            // decreasing ... PEAK
        // we know we're at a peak right now, but we only care about peaks that
...
void MicDirectionHistory::AddVadLogSample(EngineTimeStamp_t timeStamp,
  ...
  // Active -> NOT Active
    ...
    _vadTrackingData.totalTimeActive_ms +=
        (timeStamp - _vadTrackingData.timeActive_ms);
  }
  // Always track noise floor
  _vadTrackingData.AddNoiseFloorAverage(noiseFloorLevel);

  // Check if Session time expired
  if ((timeStamp - _vadTrackingData.sessionStartTime_ms) >
      kVadSessionLength_ms) {
    // Time to create event, wait until the VAD is NOT Active
    if (!_vadTrackingData.isActive) {
      // Create Event
      const EngineTimeStamp_t totalSessionLength_ms =
          timeStamp - _vadTrackingData.sessionStartTime_ms;

      DASMSG(
          wakeword_vad, "wakeword.vad",
      ...
      DASMSG_SET(
          i2,
          static_cast<int>((TimeStamp_t)_vadTrackingData.totalTimeActive_ms),
          "Total time actived in milliseconds");
      DASMSG_SET(i3, static_cast<int>(_vadTrackingData.noiseFloorAverage),
                 "Average noise floor level, this is the microphone’s sound "
                 "pressure value");
      ...
      _vadTrackingData.noiseFloorAverage =
          noiseFloorLevel;  // Start the next session with the current level
      _vadTrackingData.sessionStartTime_ms = timeStamp;  // Start next session
    }
  }
}

double MicDirectionHistory::AccumulatePeakPowerLevel(
    double latestPeakPowerLevel) {
  ...
  _soundTrackingData.averagePeakLevel =
      (alpha * latestPeakPowerLevel) +
      (1.0f - alpha) * _soundTrackingData.averagePeakLevel;

#if SEND_MICENGINE_DATA
  {
    // we want to visualize the spikes in mic power (even if we're a frame late
    // at this point)
...
void MicDirectionHistory::ProcessSoundReactors() {
  ...
  if (0.0 != _soundTrackingData.averagePeakLevel) {
    ...
    const double powerDelta = (_soundTrackingData.latestPowerLevel -
                               _soundTrackingData.latestNoiseFloor);
    const double powerScore =
        (powerDelta - _soundTrackingData.averagePeakLevel);

    if (Util::IsFltGTZero(powerScore)) {
      for (auto reactor : _soundReactors) {
        isTriggered |=
            reactor.callback(powerScore, _soundTrackingData.latestMicConfidence,
                             _soundTrackingData.latestMicDirection);
      }
    }
    ...
    if (nullptr != _webService) {
      ...
      if (_webService->IsWebVizClientSubscribed(kWebVizModuleName)) {
        ...
        if ((currentTime >= sNextUpdateTime) || _webServerData.forceUpdate) {
          ...
          webData["selectedDirection"] =
              _soundTrackingData.selectedMicDirection;
          ...
          webData["isTriggered"] =
              (currentTime <=
               (_webServerData.timeOfReaction + kWebserverTriggerDisplayTime));
          ...
          sNextUpdateTime =
              currentTime + kRTS_WebVizUpdateInterval;  // update every Xms
...
void MicDirectionHistory::VadTrackingData::AddNoiseFloorAverage(
    float noiseFloorLevel) {
