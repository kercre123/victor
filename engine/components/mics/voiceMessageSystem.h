...
class VoiceMessageSystem : public Util::SignalHolder {
 public:
  ...
  VoiceMessageList GetUnreadMessages(const std::string& name,
                                     SortType sortType = SortType::None) const;
  VoiceMessageUserList GetUnreadMessages(
      SortType sortType = SortType::None) const;
  ...
  RecordMessageResult StartRecordingMessage(const std::string& name,
                                            uint32_t maxDuration_ms = 0,
                                            OnCompleteCallback = {});

  // playback the specified voice message; returns the either Success or the
  // error code
  EVoiceMessageError PlaybackRecordedMessage(VoiceMessageID messageId,
                                             OnCompleteCallback = {});
 ...
 private:
  ...
  struct MessageRecord {
    ...
    bool operator==(const MessageRecord& other) const {
      return (id == other.id);
    }
  ...
  void CreateNewMessageInternal(MessageIndex index,
  ...
  void ReadMessageRecordFromJson(const Json::Value& value,
                                 MessageRecord& message) const;
  void WriteMessageRecordToJson(const MessageRecord& message,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
VoiceMessageSystem::MessageRecord& VoiceMessageSystem::GetMessageRecordAtIndex(
    MessageIndex index) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
const VoiceMessageSystem::MessageRecord&
VoiceMessageSystem::GetMessageRecordAtIndex(MessageIndex index) const {
  DEV_ASSERT(((index >= 0) && (index < kNumMessageSlots)),
             "VoiceMessageSystem");
  return _messages[index];
}
