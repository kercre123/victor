 ...
 *
 *  Description
 *  + System to handle the access / recording / playback of voice messages left
 *by the user
 *
 **********************************************************************************************************************/
...
void PlaybackNextMessageFor(ConsoleFunctionContextRef context) {
  const char* name = ConsoleArg_GetOptional_String(context, "name", nullptr);
  if (nullptr != name) {
    VoiceMessageList messages = sVMSystem->GetUnreadMessages(
        name, VoiceMessageSystem::SortType::OldestToNewest);
    if (!messages.empty()) {
...
VoiceMessageSystem::MessageRecord::MessageRecord()
    : id(kInvalidVoiceMessageId), status(EMessageStatus::Invalid) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
VoiceMessageSystem::VoiceMessageSystem() : _robot(nullptr) {}
...
void VoiceMessageSystem::Initialize(Robot* robot) {
  ...
  _saveFolder =
      platform->pathToResource(Data::Scope::Persistent, kMessageFolder);
  ...
  if (FileUtils::DirectoryDoesNotExist(_saveFolder)) {
    ...
    PRINT_CH_DEBUG("VoiceMessage", "VoiceMessageSystem",
                   "Creating message directory: %s", _saveFolder.c_str());
  }
...

  {
    ...
    AddSignalHandle(messageHandler->Subscribe(
        RobotToEngineTag::audioPlaybackEnd,
        [this](const AnkiEvent<RobotToEngine>& event) {
          const AudioPlaybackEnd& audioEvent =
              event.GetData().Get_audioPlaybackEnd();
          OnPlaybackComplete(audioEvent.path);
        }));

    AddSignalHandle(messageHandler->Subscribe(
        RobotToEngineTag::micRecordingComplete,
        [this](const AnkiEvent<RobotToEngine>& event) {
          const MicRecordingComplete& audioEvent =
              event.GetData().Get_micRecordingComplete();
...
void VoiceMessageSystem::LoadMessageSystem() {
  ...
  if (!fileContents.empty() && reader.parse(fileContents, metadata)) {
    ...
    for (const Json::Value& jsonMessage : messageArray) {
      ...
      DEV_ASSERT(((index >= 0) && (index < kNumMessageSlots)),
                 "VoiceMessageSystem");
      ...
      // make sure our id is at least as great as the highest stored id so make
      // sure we don't duplicate ids
    ...
    PRINT_CH_DEBUG("VoiceMessage", "VoiceMessageSystem",
                   "Failed to load voice message metadata file: %s",
                   filename.c_str());
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void VoiceMessageSystem::WriteMessageRecordToJson(const MessageRecord& message,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void VoiceMessageSystem::ReadMessageRecordFromJson(
    const Json::Value& value, MessageRecord& message) const {
...
bool VoiceMessageSystem::VerifyAndSyncMessageRecords() {
  ...
  for (MessageRecord& message : _messages) {
    ...
    messageIsValid &= (message.fileHandle.empty() ||
                       Util::FileUtils::FileExists(message.fileHandle));
    // I can't see how this is possible, but doesn't hurt to check
    messageIsValid &=
        (!message.HasMessage() || (message.id != kInvalidVoiceMessageId));

    if (!messageIsValid) {
      PRINT_NAMED_ERROR("VoiceMessageSystem",
                        "Message records don't match files on disk (%s)",
                        message.fileHandle.c_str());
      DeleteMessageInternal(message);

      dataIsValid = false;
    }

    // note: we could also check for the case when we have a wav file with no
    // metadata, but we'd have to default the metadata
...
std::string VoiceMessageSystem::GetDefaultFilename(MessageIndex index) const {
  const std::string filename =
      kMessageFilenamePrefix + "_" + std::to_string(index);
  return (_saveFolder + filename);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
VoiceMessageSystem::FileHandle VoiceMessageSystem::GetDefaultFileHandle(
    MessageIndex index) const {
  const std::string filename =
      (kMessageFilenamePrefix + "_" + std::to_string(index));
  const std::string intermediateFolder =
      Util::FileUtils::AddTrailingFileSeparator(_saveFolder + filename);
  return (intermediateFolder + filename + kMessageFileType);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
VoiceMessageSystem::MessageIndex VoiceMessageSystem::GetMessageIndex(
    VoiceMessageID id) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
VoiceMessageSystem::MessageIndex VoiceMessageSystem::GetFreeMessageIndex()
    const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void VoiceMessageSystem::CreateNewMessageInternal(
    MessageIndex index, const std::string& recipient) {
  ...
  if (message.HasMessage()) {
    PRINT_NAMED_WARNING(
        "VoiceMessageSystem",
        "New voice message will overwrite existing unread message");
    DeleteMessageInternal(message);
  }
...

  {
    ...
    message.recipientName =
        Util::StringToLower(recipient);  // note: change this when we have the
                                         // UserName class pulled out
...
void VoiceMessageSystem::DeleteMessageInternal(MessageRecord& message) {
  PRINT_CH_DEBUG("VoiceMessage", "VoiceMessageSystem",
                 "[Slot %d] Deleting message with id [%u]",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
RecordMessageResult VoiceMessageSystem::StartRecordingMessage(
    const std::string& name, uint32_t duration_ms,
  ...
  if (kInvalidSlot != index) {
    if (nullptr == _currentRecording.messageRecord) {
      ...
      // generate a filename (not the actual filename on disk since the anim
      // process changes it)
      ...
      const uint32_t duration =
          ((duration_ms != 0) ? duration_ms
                              : GetDefaultRecordingMessageDurationMS());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
EVoiceMessageError VoiceMessageSystem::PlaybackRecordedMessage(
    VoiceMessageID messageId, OnCompleteCallback callback) {
  ...
  if (kInvalidSlot != index) {
    if (nullptr == _currentPlayback.messageRecord) {
      PRINT_CH_INFO("VoiceMessage", "VoiceMessageSystem.Playback",
                    "[Slot %d] Playing message with id [%u]", (int)index,
                    messageId);
      ...
      _robot->SendMessage(
          RobotInterface::EngineToRobot(std::move(playbackMessage)));
    ...
    PRINT_CH_INFO("VoiceMessage", "VoiceMessageSystem.Playback",
                  "Couldn't find message with id [%u]", messageId);
  }
...
void VoiceMessageSystem::OnRecordingComplete(const std::string& path) {
  if ((_currentRecording.messageRecord != nullptr) &&
      (_currentRecording.messageRecord->fileHandle == path)) {
    OnCompleteCallback callback = std::move(_currentRecording.callback);

    // delete this before we call the callback so that the user can fire off
    // another recording from the callback
...
void VoiceMessageSystem::OnPlaybackComplete(const std::string& path) {
  if ((_currentPlayback.messageRecord != nullptr) &&
      (_currentPlayback.messageRecord->fileHandle == path)) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool VoiceMessageSystem::IsSameRecipient(const std::string& lhs,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
VoiceMessageList VoiceMessageSystem::GetUnreadMessages(
    const std::string& name, SortType sortType) const {
  ...
  // luckily our message id's are guaranteed to be in chronological order, so
  // simply sort by the id
  switch (sortType) {
    case SortType::OldestToNewest:
      std::sort(
          messages.begin(), messages.end(),
          [](VoiceMessageID lhs, VoiceMessageID rhs) { return (lhs < rhs); });
      break;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
VoiceMessageUserList VoiceMessageSystem::GetUnreadMessages(
  ...
  // note: we can easily speed this up by storing our messages by recipient, but
  // with only 10 max messages
...
void VoiceMessageSystem::Debug_PrintMessageRecords() const {
#if DEBUG_VOICESYSTEM
  {
    auto printMessage = [](int index, const MessageRecord& message) {
      if (!message.HasMessage()) {
        PRINT_CH_DEBUG("VoiceMessage", "VoiceMessageSystem", "[Slot %d] Empty",
                       index);
      } else {
        PRINT_CH_DEBUG(
            "VoiceMessage", "VoiceMessageSystem", "[Slot %d] %s", index,
            Util::FileUtils::GetFileName(message.fileHandle).c_str());
        PRINT_CH_DEBUG(
            "VoiceMessage", "VoiceMessageSystem", "         Name: %s",
            !message.recipientName.empty() ? message.recipientName.c_str()
                                           : "All");
        PRINT_CH_DEBUG("VoiceMessage", "VoiceMessageSystem", "         ID: %u",
                       message.id);
        PRINT_CH_DEBUG(
            "VoiceMessage", "VoiceMessageSystem", "         Status: %s",
            (message.status == EMessageStatus::Invalid)
                ? "Invalid"
                : ((message.status == EMessageStatus::Unread) ? "Unread"
                                                              : "Read"));
      }
    };
