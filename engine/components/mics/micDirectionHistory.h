...
class MicDirectionHistory {
 public:
  ...
  static constexpr uint32_t kMicDirectionHistoryLen =
      kMicDirectionHistory_ms / kTimePerDirectionUpdate_ms;

  void AddMicSample(const RobotInterface::MicDirection& message);
  void AddDirectionSample(EngineTimeStamp_t timestamp, bool isVadActive,
                          MicDirectionIndex newIndex,
                          MicDirectionConfidence newConf,
                          MicDirectionIndex selectedDirection);
  void AddMicPowerSample(EngineTimeStamp_t timestamp, float powerLevel,
                         float noiseFloor);
  void AddVadLogSample(EngineTimeStamp_t timeStamp, float noiseFloorLevel,
                       int activeState);

  // Interface for requesting the "best" direction
  static constexpr uint32_t kDefaultDirectionRecentTime_ms = 1000;
  MicDirectionIndex GetRecentDirection(
      TimeStamp_t timeLength_ms = kDefaultDirectionRecentTime_ms) const;
  MicDirectionIndex GetDirectionAtTime(EngineTimeStamp_t timestampEnd,
                                       TimeStamp_t timeLength_ms) const;

  // Selected direction is only valid when microphone beamforming is enabled,
  // the direction will either be the what the mic processing has determined
  // same as "RecentDirection" or if the beamforming direction has been
  // specifically set.
  MicDirectionIndex GetSelectedDirection() const {
    return _mostRecentSelectedDirection;
  }

  MicDirectionNodeList GetRecentHistory(TimeStamp_t timeLength_ms) const;
  MicDirectionNodeList GetHistoryAtTime(EngineTimeStamp_t timestampEnd,
                                        TimeStamp_t timeLength_ms) const;

  // currently we only allow the listerner to supply an OnSound "reaction", but
  // could easily have them supply the "testing" function to determine
  // themselves what is considered a valid sound reaction
 ...
 private:
  ...
  // data we want to send to the webserver for debugging, but don't otherwise
  // need to store
  ...
  using DirectionHistoryCount =
      std::array<uint32_t,
                 (kLastMicDirectionIndex - kFirstMicDirectionIndex + 1)>;
  ...
  DirectionHistoryCount GetDirectionCountAtIndex(
      uint32_t startIndex, TimeStamp_t timeLength_ms) const;
  MicDirectionNodeList GetHistoryAtIndex(uint32_t startIndex,
                                         TimeStamp_t timeLength_ms) const;
  static MicDirectionIndex GetBestDirection(
