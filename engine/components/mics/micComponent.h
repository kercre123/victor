...
class MicComponent : public IDependencyManagedComponent<RobotComponentID> {
 public:
  ...
  const MicDirectionHistory& GetMicDirectionHistory() const {
    return *_micHistory;
  }

  VoiceMessageSystem& GetVoiceMessageSystem() { return *_messageSystem; }
  const VoiceMessageSystem& GetVoiceMessageSystem() const {
    return *_messageSystem;
  }

  // set / get the fullness of the audio processing buffer on the robot (float
  // from 0 to 1)
