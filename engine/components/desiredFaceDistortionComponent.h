...
class DesiredFaceDistortionComponent : private Anki::Util::noncopyable {
 public:
  ...
  // If we should start a new face distortion _now_, this returns the degree to
  // which the face should be distorted (see scanlineDistorter for more info),
  // otherwise it returns a negative number. Once it returns a positive number,
  // it will return negative until it's time to start a _new_ face distortion
 ...
 private:
  struct Params : private Anki::Util::noncopyable {
    ...
    // how "random" the cooldown should be. Current cooldown value from the
    // evaluator will be multiplied by this factor to get the maximum range of
    // the uniform random cooldown time
    ...
    // As with cooldown, this is the factor determining the amount of randomness
    // on the degree
