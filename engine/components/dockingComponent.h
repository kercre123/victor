...
class DockingComponent : public IDependencyManagedComponent<RobotComponentID>,
 ...
 public:
  ...
  // - whether or not to begin raising the lift at the same time as beginning to
  // back up
  Result DockWithObject(
      ...
      const f32 placementOffsetAngle_rad = 0, const u8 numRetries = 2,
  ...
  // Sends an updated docking error signal to the robot if we are currently
  // docking
  ...
  const Vision::KnownMarker::Code& GetDockMarkerCode() const {
    return _dockMarkerCode;
  }
  void UnsetDockObjectID() {
    _dockObjectID.UnSet();
    _dockMarkerCode = Vision::MARKER_INVALID;
  }

  // lets the robot decide if we should try to stack on top of the given object,
  // so that we have a central place to make the appropriate checks. returns
  // true if we should try to stack on top of the given object, false if
  // something would prevent it, for example if we think that block has
  // something on top or it's too high to reach
  bool CanStackOnTopOfObject(const ObservableObject& object) const;

  // lets the robot decide if we should try to pick up the given object
  // (assuming it is flat, not picking up out of someone's hand). Checks that
  // object is flat, not moving, no unknown pose, etc.
  bool CanPickUpObject(const ObservableObject& object) const;

  // same as above, but check that the block is on the ground (as opposed to
  // stacked, on top of a notebook or something, or in someone's hand
 ...
 private:
  ...
  bool CanInteractWithObjectHelper(const ObservableObject& object,
                                   Pose3d& relPose) const;
  ...
  // We can't store pointers to makers because the object may be unobserved and
  // reoverserved, which could cause a located instance to be destroyed and
  // recreated
