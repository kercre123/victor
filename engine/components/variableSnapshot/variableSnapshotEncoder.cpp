 ...
 * VariableSnapshotObjects. A separate pair of (de)serializer must be
...
template <>
void Deserialize<int>(std::shared_ptr<int> dataPtr,
...
template <>
void Deserialize<bool>(std::shared_ptr<bool> dataPtr,
                       const Json::Value& loadedJson) {
...
// strings
template <>
bool Serialize<std::string>(std::shared_ptr<std::string> dataPtr,
                            Json::Value& outJson) {
  outJson[kVariableSnapshotKey] = *dataPtr;
  return true;
}
...
template <>
void Deserialize<std::string>(std::shared_ptr<std::string> dataPtr,
                              const Json::Value& loadedJson) {
...
template <>
bool Serialize<std::unordered_map<std::string, time_t>>(
    std::shared_ptr<std::unordered_map<std::string, time_t>> dataPtr,
    Json::Value& outJson) {
  Json::Value dataList;
...
template <>
void Deserialize<std::unordered_map<std::string, time_t>>(
    std::shared_ptr<std::unordered_map<std::string, time_t>> dataPtr,
    const Json::Value& loadedJson) {
  for (const auto& jsonData : loadedJson[kVariableSnapshotKey]) {
    std::string key = JsonTools::ParseString(
        jsonData, TypeConstants::kMapKey,
        "failed to deserialize key of umap: string -> time_t");
    time_t value_s = JsonTools::ParseUInt32(
        jsonData, TypeConstants::kMapValueKey,
        "failed to deserialize value of umap: string -> time_t");
...
template <>
bool Serialize<std::unordered_map<BEIConditionType, BehaviorID>>(
    std::shared_ptr<std::unordered_map<BEIConditionType, BehaviorID>> dataPtr,
    Json::Value& outJson) {
  Json::Value dataList;
  for (const auto& dataIter : *dataPtr) {
    Json::Value datum;
...
template <>
void Deserialize<std::unordered_map<BEIConditionType, BehaviorID>>(
    std::shared_ptr<std::unordered_map<BEIConditionType, BehaviorID>> dataPtr,
    const Json::Value& loadedJson) {
  for (const auto& jsonData : loadedJson[kVariableSnapshotKey]) {
    BEIConditionType key = BEIConditionType::Invalid;
    const std::string loadedBEICondString = JsonTools::ParseString(
        jsonData, TypeConstants::kMapKey,
        "failed to deserialize key of umap: BEIConditionType -> BehaviorID");
    const bool beiCondLoaded =
        BEIConditionTypeFromString(loadedBEICondString, key);
    if (!beiCondLoaded) {
      PRINT_NAMED_WARNING(
          "VariableSnapshotEncoder.DeserializeUMapBEICondToBehaviorID."
          "UnknownStringinJson",
          "Key %s was not recognized as a valid BEIConditionType value, will "
          "be dropped",
          loadedBEICondString.c_str());
    }

    BehaviorID value = BehaviorID::Anonymous;
    const std::string loadedBehaviorIdString = JsonTools::ParseString(
        jsonData, TypeConstants::kMapValueKey,
        "failed to deserialize value of umap: BEIConditionType -> BehaviorID");
    const bool behaviorIdLoaded =
        BehaviorIDFromString(loadedBehaviorIdString, value);
    if (!behaviorIdLoaded) {
      PRINT_NAMED_WARNING(
          "VariableSnapshotEncoder.DeserializeUMapBEICondToBehaviorID."
          "UnknownStringinJson",
          "Key %s was not recognized as a valid BehaviorID value, will be "
          "dropped",
          loadedBehaviorIdString.c_str());
    }
