...
class BackpackLightComponent
 ...
 public:
  ...
  // Allows the caller to start a looping light pattern on Vector's backpack and
  // get a handle to cancel it later.
  void StartLoopingBackpackAnimation(
      const BackpackLightAnimation::BackpackAnimation& lights,
      BackpackLightDataLocator& lightLocator_out);

  // Related to the above start looping call, allows the caller to cancel a
  // currently looping light pattern
  bool StopLoopingBackpackAnimation(
 ...
 private:
  Result SetBackpackAnimationInternal(
      const BackpackLightAnimation::BackpackAnimation& lights);

  void StartLoopingBackpackAnimationInternal(
      const BackpackLightAnimation::BackpackAnimation& lights,
      BackpackLightDataLocator& lightLocator_out);
  ...
  // Locator handle for the shared light configuration associated with
  // SetBackpackLightAnimation above
