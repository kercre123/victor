...
BackpackLightComponent::BackpackLightComponent()
  ...
  static_assert((int)LEDId::NUM_BACKPACK_LEDS == 3,
                "BackpackLightComponent.WrongNumBackpackLights");
}

void BackpackLightComponent::InitDependent(Vector::Robot* robot,
...
void BackpackLightComponent::UpdateDependent(
    const RobotCompMap& dependentComps) {
...
void BackpackLightComponent::SetBackpackAnimation(
    const BackpackLightAnimation::BackpackAnimation& lights) {
  StartLoopingBackpackAnimationInternal(lights, _sharedLightConfig);
}

void BackpackLightComponent::SetBackpackAnimation(
    const BackpackAnimationTrigger& trigger) {
  StopLoopingBackpackAnimation(_sharedLightConfig);

  _backpackLightList.emplace_front(
      new BackpackLightData{._isTrigger = true, ._trigger = trigger});
...
bool BackpackLightComponent::StopLoopingBackpackAnimation(
  ...
  if (!_backpackLightList.empty()) {
    ...
    PRINT_NAMED_WARNING(
        "BackpackLightComponent.StopLoopingBackpackAnimation.NoLocators",
...
template <>
void BackpackLightComponent::HandleMessage(
    const ExternalInterface::SetBackpackLEDs& msg) {
  const BackpackLightAnimation::BackpackAnimation lights = {
...
Result BackpackLightComponent::SetBackpackAnimationInternal(
    const BackpackLightAnimation::BackpackAnimation& lights) {
  return _robot->SendMessage(RobotInterface::EngineToRobot(lights.ToMsg()));
}

void BackpackLightComponent::StartLoopingBackpackAnimationInternal(
    const BackpackLightAnimation::BackpackAnimation& lights,
    BackpackLightDataLocator& lightLocator_out) {
  StopLoopingBackpackAnimation(lightLocator_out);

  _backpackLightList.emplace_front(new BackpackLightData{
      ._isTrigger = false, ._lightConfiguration = std::move(lights)});
