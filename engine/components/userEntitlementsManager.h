...
class UserEntitlementsManager
 ...
 public:
  ...
  // Return the user entitlement value (currently strings, bools, uints
  // supported)
  std::string GetUserEntitlementAsString(
      const external_interface::UserEntitlement key) const;
  bool GetUserEntitlementAsBool(
      const external_interface::UserEntitlement key) const;
  uint32_t GetUserEntitlementAsUInt(
      const external_interface::UserEntitlement key) const;

  bool DoesUserEntitlementUpdateCloudImmediately(
      const external_interface::UserEntitlement key) const;
 ...
 private:
  ...
  using SettingSetters =
      std::map<external_interface::UserEntitlement, SettingSetter>;
