...
PublicStateBroadcaster::PublicStateBroadcaster()
    : IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::PublicStateBroadcaster) {
  ...
  _currentState.reset(new RobotPublicState(
      isCubeInLift, isRequestingGame, tallestStackHeight, needsLevels, empty));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void PublicStateBroadcaster::UpdateDependent(
  ...
  // After all update checks, if any changed a state property, send the updated
  // state
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void PublicStateBroadcaster::UpdateBroadcastBehaviorStage(
    BehaviorStageTag stageType, uint8_t stage) {
  ...
  if (stageType ==
      _currentState->userFacingBehaviorStageStruct.behaviorStageTag) {
    int oldStage = GetStageForBehaviorStageType(
        stageType, _currentState->userFacingBehaviorStageStruct);
    if (stage != oldStage) {
      PRINT_CH_INFO(
          kChannelName,
          "PublicStateBroadcaster.UpdateUserFacingBehaviorStage.UpdatingStage",
          "Updating stage type %s to stage %d", EnumToString(stageType), stage);
    } else {
      PRINT_NAMED_WARNING(
          "PublicStateBroadcaster.UpdateUserFacingBehaviorStage",
  ...
  switch (stageType) {
    ...
    case BehaviorStageTag::PyramidConstruction: {
      newStruct.currentPyramidConstructionStage =
          static_cast<PyramidConstructionStage>(stage);
      break;
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
int PublicStateBroadcaster::GetBehaviorRoundFromMessage(
    const RobotPublicState& stateEvent) {
  return GetStageForBehaviorStageType(
      stateEvent.userFacingBehaviorStageStruct.behaviorStageTag,
      stateEvent.userFacingBehaviorStageStruct);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
int PublicStateBroadcaster::GetStageForBehaviorStageType(
  ...
  switch (stageType) {
    ...
    case BehaviorStageTag::PyramidConstruction: {
      return Util::EnumToUnderlying(
          stageStruct.currentPyramidConstructionStage);
    }
    case BehaviorStageTag::Count: {
      return 0;
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Signal::SmartHandle PublicStateBroadcaster::Subscribe(
...
void PublicStateBroadcaster::SendUpdatedState() {
  _eventMgr.Broadcast(
      AnkiEvent<RobotPublicState>(static_cast<uint32_t>(0), *_currentState));
}
