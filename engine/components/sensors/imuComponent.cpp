...
ImuComponent::ImuComponent()
    : ISensorComponent(kLogDirName),
      IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::ImuSensor) {}
...
void ImuHistory::AddData(IMUDataFrame&& frame) {
  ...
  DEV_ASSERT_MSG(
      _history.empty() || (frame.timestamp >= _history.back().timestamp),
...
bool ImuHistory::IsImuDataBeforeTimeGreaterThan(const RobotTimeStamp_t t,
                                                ...
                                                const f32 rateX,
                                                const f32 rateY,
                                                const f32 rateZ) const {
  if (_history.empty()) {
    return false;
  }

  auto iter = _history.begin();
  // Separate check for the first element due to there not being anything before
  // it
  if (iter->timestamp > t) {
    if (ABS(iter->gyroRobotFrame.x) > rateX &&
        ABS(iter->gyroRobotFrame.y) > rateY &&
        ABS(iter->gyroRobotFrame.z) > rateZ) {
      return true;
    }
  }
  // If the first element has a timestamp of zero then nothing else will have
  // valid timestamps due to how ImuData comes in during an image so the data
  // that we get for an image that we are in the process of receiving will not
  // have timestamps
  ...
  for (iter = _history.begin() + 1; iter != _history.end(); ++iter) {
    // If we get to the imu data after or equal to the timestamp
    if (iter->timestamp >= t) {
      // Once we get to the imu data after the timestamp look at the
      // numToLookBack imu data before it
...
bool ImuHistory::WasHeadRotatingTooFast(RobotTimeStamp_t t,
  ...
  if (numImuDataToLookBack > 0) {
    return (IsImuDataBeforeTimeGreaterThan(t, numImuDataToLookBack, 0,
                                           headTurnSpeedLimit_radPerSec, 0));
  }

  IMUDataFrame prev, next;
  if (!GetImuDataBeforeAndAfter(t, prev, next)) {
    const u32 newestTimestamp =
        _history.empty() ? 0 : (u32)_history.back().timestamp;
    LOG_INFO("ImuComponent.WasHeadRotatingTooFast.NoIMUData",
             "Could not get next/previous imu data for timestamp %u (newest "
             "timestamp in history %u - diff %u)",
...
bool ImuHistory::WasBodyRotatingTooFast(RobotTimeStamp_t t,
                                        ...
                                        const int numImuDataToLookBack) const {
  if (numImuDataToLookBack > 0) {
    return (IsImuDataBeforeTimeGreaterThan(t, numImuDataToLookBack, 0, 0,
                                           bodyTurnSpeedLimit_radPerSec));
  }

  IMUDataFrame prev, next;
  if (!GetImuDataBeforeAndAfter(t, prev, next)) {
    LOG_INFO("ImuComponent..WasBodyRotatingTooFast",
             "Could not get next/previous imu data for timestamp %u",
             (TimeStamp_t)t);
    return true;
  }
