...
// Between these min/max lift heights, the lift interferes with the prox
// sensor's beam.
...
inline float GetSignalQuality(const ProxSensorDataRaw& proxData) {
  ...
  if (badSpadCount) {
    PRINT_NAMED_WARNING("ProxSensorComponent.GetSignalQuality.BadSpadCount",
                        "Invalid sensor reading, SpadCount <= 0");
  }
...
ProxSensorComponent::ProxSensorComponent()
    : ISensorComponent(kLogDirName),
      IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::ProxSensor) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::string ProxSensorComponent::GetLogHeader() {
  return std::string(
      "timestamp_ms, distance_mm, signalIntensity, ambientIntensity, "
      "spadCount, rangeStatus");
}
...
void ProxSensorComponent::NotifyOfRobotStateInternal(const RobotState& msg) {
  ...
  if (msg.proxData.timestamp_ms > msg.timestamp) {
    LOG_WARNING("ProxSensorComponent.NotifyOfRobotStateInternal.BadSensorTime",
                "prox sensor recorded measurement at time %d, but last robot "
                "state message was at time %d",
                msg.proxData.timestamp_ms, msg.timestamp);
  }

  // NOTE: We are assuming here that the latest data is already in robot state
  // history
  ...
  const auto& res = _robot->GetStateHistory()->ComputeStateAt(
      measurmentTime, histTimestamp, histState, kUseInterp);
  if (res != RESULT_OK) {
    // If robot is localized and there are at least 3 RobotState messages in
    // history which span 60ms (i.e. the slowest rate expected of the prox
    // sensor) then report warning since a historical state should've been
    // found.
    const u32 numRawStatesInSameFrame =
        _robot->GetStateHistory()->GetNumRawStatesWithFrameID(
            msg.pose_frame_id);
    if (numRawStatesInSameFrame >= 3 && _robot->IsLocalized()) {
      LOG_WARNING(
          "ProxSensorComponent.ProcessRawSensorData.NoHistoricalPose",
  ...
  _latestData.distance_mm = Util::Clamp(
      msg.proxData.distance_mm, kMinObsThreshold_mm, kMaxObsThreshold_mm);
  _latestData.signalQuality = GetSignalQuality(msg.proxData);
  _latestData.unobstructed =
      (_latestData.signalQuality <=
       kMinQualityThreshold) ||  // low signal quality means we aren't hitting
                                 // anything
      (msg.proxData.distance_mm >=
       kMaxObsThreshold_mm);  // object is further away than we care about
  _latestData.foundObject =
      !_latestData.isLiftInFOV &&  // we are not blocked by the lift
      (_latestData.signalQuality >
       kMinQualityThreshold) &&  // signal is strong enough
      (msg.proxData.distance_mm <
       kMaxObsThreshold_mm) &&  // object is close enough
      (msg.proxData.rangeStatus ==
       RangeStatus::RANGE_VALID) &&  // the sensor is working
      (histState.GetPitch_rad() >=
       kMinPitch) &&  // we are not looking at the ground
      (histState.GetPitch_rad() <=
       kMaxPitch);  // we are not looking at the ceiling

  // check if the robot has moved or the sensor reading has changed
  // significantly
  const bool measurementChanged =
      FLT_GT(fabs(_latestData.distance_mm - prevMeasurement) / prevMeasurement,
             kMeasurementTolerance);
  const bool resetMeasurementCounter =
      !_currentRobotPose.IsSameAs(prevRobotPose, kRobotTranslationTolerance_mm,
                                  kRobotRotationTolerance_rad) ||
  ...
  const bool stillUpdating =
      (_measurementsAtPose <
       kNumMeasurementsAtPose);  // if the robot hasn't moved but we still need
                                 // to update the belief state
  const bool measurementInFrame =
      (histState.GetFrameId() !=
       _robot->GetPoseFrameID());  // if measurement frame is not the current
                                   // robot frame
...
bool ProxSensorComponent::CheckLiftOcclusion() {
  ...
  if (_robot->IsLiftCalibrated()) {
    ...
    isInFOV = Util::InRange(
        _robot->GetLiftHeight(),
        isCarryingObject ? kLiftHeightOccludingProxSensorWithCubeMin
                         : kLiftHeightOccludingProxSensorMin,
        isCarryingObject ? kLiftHeightOccludingProxSensorWithCubeMax
                         : kLiftHeightOccludingProxSensorMax);
...
bool ProxSensorComponent::VerifyLiftCalibration() const {
  ...
  const auto firstMsgTime =
      _robot->GetLastMsgTimestamp() - kLiftCalibrationWindow_ms;
  ...
  for (const auto& state : _robot->GetStateHistory()->GetRawStates()) {
    if (IN_RANGE(state.first, firstMsgTime, lastMsgTime)) {
      ...
      if (histProxData.foundObject != prevFoundObject) {
        ...
        if (abs(prevDistance_mm - (int)histProxData.distance_mm) >
            kNoiseTolerance_mm) {
          ++nSuddenTransitions;
        }
      ...
      allReadingsTooClose &=
          (histProxData.foundObject &&
           histProxData.distance_mm < kLiftSensorDistance_mm);
    }
  }

  const float samplePeriod_s = kLiftCalibrationWindow_ms / 1000.f;
  const bool readingsDithering =
      FLT_GT(nSuddenTransitions / samplePeriod_s, kMaxSensorFlicker_Hz);
  ...
  if (needsToCalibrate && !_robot->GetCarryingComponent().IsCarryingObject()) {
    // fire DAS event
    DASMSG(robot.bad_prox_data, "robot.bad_prox_data",
           "Calibration found noisy data although signal quality is good");
    DASMSG_SET(s1, allReadingsTooClose ? "true" : "false",
               "readings too close");
...
// - - - - - - - - - - - - - - - - - - - -
namespace {
// we would like to clear the whole sensor cone as defined by the aperture, but
// it creates really large obstacles if it detects something far away. To
// prevent planning failures, we would like to clamp the max obstacle width to
// one half robot width, but that can result in stretching of the clear region
// cone, which causes some map artifacts. To get around this, define the clear
// region as the intersection of the sensor cone and a straight beam with
// obstacle width (which is clamped to robot width).
...
std::unique_ptr<BoundedConvexSet2f> GetClearRegion(const Pose2d& robotPose,
                                                   const Pose2d& objectPose,
                                                   float measurement) {
  // sensor points
  ...
  // use a slightly larger padding for clear than for obstacle to clean up
  // floating point rounding errors
  const Vec2f sensorOffset1(-kObsPadding_x_mm,
                            -sensorBeamHalfWidth_mm - kObsPadding_y_mm - .5);
  const Vec2f sensorOffset2(-kObsPadding_x_mm,
                            sensorBeamHalfWidth_mm + kObsPadding_y_mm + .5);
  ...
  // clear sensor beam
  // NOTE: making an intersection of a quad and a triangle here results in a
  // 50%-100% speedup over using a
  //       five sided polygon, so use that if we can't insert a triangle FP.
  FastPolygon sensorCone{
      {sensorCorner1, sensorCorner2, robotPose.GetTranslation()}};
  if (FLT_LT(sensorBeamHalfWidth_mm, kMaxObstacleWidth_mm * .5)) {
    // the sensor beam width is smaller than the max obstacle size, we don't
    // need to clamp the sides
    return std::make_unique<FastPolygon>(sensorCone);
  } else {
    const Point2f clampOffset1(-measurement,
                               -obstacleHalfWidth_mm - kObsPadding_y_mm);
    const Point2f clampOffset2(-measurement,
                               obstacleHalfWidth_mm + kObsPadding_y_mm);
    ...
    // obstacle points
    const Point2f obstacleOffset1(-kObsPadding_x_mm,
                                  -obstacleHalfWidth_mm - kObsPadding_y_mm);
    const Point2f obstacleOffset2(-kObsPadding_x_mm,
                                  obstacleHalfWidth_mm + kObsPadding_y_mm);
    ...
    auto retv = MakeIntersection2f(
        sensorCone,
        FastPolygon{{clampCorner1, clampCorner2, obstacleP2, obstacleP1}});
...
FastPolygon GetObstacleRegion(const Pose2d& objectPose, u16 measurement) {
  ...
  const Point2f obstacleP3 =
      objectPose.GetTransform() * (obstacleRight + obstacleDepth);
  const Point2f obstacleP4 =
      objectPose.GetTransform() * (obstacleLeft + obstacleDepth);
...
void ProxSensorComponent::UpdateNavMap(uint32_t timestamp) {
  ...
  if (_latestData.unobstructed || _latestData.foundObject) {
    ...
    const Pose2d objectPose =
        static_cast<Pose2d>(_currentRobotPose) * Pose2d(0, {measurement, 0});
    const auto clearRegion =
        GetClearRegion(_currentRobotPose, objectPose, measurement);

    _robot->GetMapComponent().ClearRegion(*clearRegion, timestamp);

    if (_latestData.foundObject) {
      MemoryMapData_ProxObstacle proxData(
          MemoryMapData_ProxObstacle::NOT_EXPLORED, objectPose, timestamp);
      _robot->GetMapComponent().AddProxData(
          GetObstacleRegion(objectPose, measurement), proxData);
    }
  }
}
