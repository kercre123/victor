...
void ISensorComponent::StartLogging(const uint32_t duration_ms) {
  if (!_loggingRawData) {
    ...
    PRINT_NAMED_INFO(
        "ISensorComponent.StartLogging.Start",
        "Starting raw data logging, duration %d ms%s. Log will appear in '%s'",
        duration_ms, _logRawDataUntil_s == 0.f ? " (indefinitely)" : "",
        _robot->GetContextDataPlatform()
            ->pathToResource(Util::Data::Scope::Cache, _logDirectory)
            .c_str());
  } else {
    PRINT_NAMED_WARNING("ISensorComponent.StartLogging.AlreadyLogging",
                        "Already logging raw data!");
  }
}
...
void ISensorComponent::StopLogging() {
  if (_loggingRawData) {
    ...
    PRINT_NAMED_WARNING("ISensorComponent.StopLogging.NotLogging",
...
void ISensorComponent::Log() {
  ...
  const auto now = BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  if (_logRawDataUntil_s != 0.f && now >= _logRawDataUntil_s &&
      _rawDataLogger != nullptr) {
  ...
  if (_rawDataLogger == nullptr) {
    _rawDataLogger = std::make_unique<Util::RollingFileLogger>(
        nullptr, _robot->GetContextDataPlatform()->pathToResource(
                     Util::Data::Scope::Cache, _logDirectory));
    auto header = GetLogHeader();
    DEV_ASSERT((header.find('\n') == std::string::npos) &&
                   (header.find('\r') == std::string::npos),
               "ISensorComponent.Log.LinebreakInHeaderNotAllowed");
    header += "\n";
  ...
  auto row = GetLogRow();
  DEV_ASSERT((row.find('\n') == std::string::npos) &&
                 (row.find('\r') == std::string::npos),
             "ISensorComponent.Log.LinebreakInRowNotAllowed");
  row += "\n";
