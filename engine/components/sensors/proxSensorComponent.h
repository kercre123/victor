...
struct ProxSensorData {
  ...
  bool
      unobstructed;  // The sensor has not detected anything up to its max range
  bool foundObject;  // The sensor detected an object in the valid operating
                     // range
...
class ProxSensorComponent
 ...
 private:
  ...
  ProxSensorDataRaw
      _latestDataRaw;  // raw data sent from robot process - should only be used
                       // for debugging purposes
  ProxSensorData
      _latestData;  // processed data that has additional cleanup and state
  Pose3d _currentRobotPose;  // robot pose at current sensor reading
  uint32_t _measurementsAtPose =
      0;  // counter to limit number of map updates while not moving
  uint32_t _numTicsLiftOutOfFOV =
      0;  // counter for lift FOV checks to account for sensor delay
  bool _mapEnabled =
      true;  // disable map updates entirely (currently for low-power mode)
};
