 ...
 * Created: 6/27/2017
 *
 * Description: Component for managing cliff sensors and cliff sensor-related
 *functions such as carpet detection (and corresponding cliff threshold
 *adjustment), 'suspicious' cliff detection, and raw data access.
 *
...
// The maximum rate at which cliff sensor threshold updates will be sent to the
// robot.
const int kCliffThresholdMaxUpdateRate_ms = 1000;

// The default allowed delta above the minimum-ever-seen cliff sensor value
// before cliff detection is triggered.
const u16 kCliffDetectAllowedDeltaDefault = 35;

// This value is used whenever it seems like we may be driving on a dark
// surface, to reduce the chance of false cliff detections.
const u16 kCliffDetectAllowedDeltaLow = 20;

// If all cliff sensors are reporting below this value, then we reduce the
// AllowedDelta to prevent cliff detection false positives. Likewise if all
// cliff sensors are _above_ this value, the AllowedDelta is restored to the
// default.
const u16 kCliffValDarkSurface = 250;

// Also reduce the AllowedDelta if the robot could be driving over something
// (i.e. its pitch angle is sufficiently far from zero)
...
CliffSensorComponent::CliffSensorComponent()
    : ISensorComponent(kLogDirectory),
      IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::CliffSensor),
...
void CliffSensorComponent::InitDependent(Vector::Robot* robot,
...
void CliffSensorComponent::HandleMessage(
    const ExternalInterface::RobotStopped& msg) {
  if (msg.reason == StopReason::CLIFF) {
    _lastStopDueToCliffTime_ms =
    ...
    // Reset the recorded data for the last event where the robot was put down
    // on a cliff.
...
void CliffSensorComponent::HandleMessage(
  ...
  if (msg.treadsState == OffTreadsState::OnTreads && IsCliffDetected()) {
    _lastPutDownOnCliffTime_ms =
        BaseStationTimer::getInstance()->GetCurrentTimeStamp();
    LOG_INFO("CliffSensorComponent.RobotPutDownOnCliff", "");

    // Reset the recorded data for the last event where the robot stopped for a
    // cliff.
...
void CliffSensorComponent::NotifyOfRobotStateInternal(const RobotState& msg) {
  ...
  for (int i = 0; i < kNumCliffSensors; i++) {
    ...
    _cliffDataFilt[i] = Util::IsNearZero(_cliffDataFilt[i])
                            ? _cliffDataRaw[i]
                            : (kCliffFiltCoef * _cliffDataFilt[i]) +
                                  ((1.f - kCliffFiltCoef) *
                                   static_cast<float>(_cliffDataRaw[i]));
  }
  ...
  const bool isPickedUp =
      (msg.status & (uint32_t)RobotStatusFlag::IS_PICKED_UP) != 0;
  _maxNumCliffsDetectedWhilePickedUp =
      isPickedUp ? std::max(_maxNumCliffsDetectedWhilePickedUp,
                            _latestNumCliffsDetected)
                 : 0;
  ...
  if (_stopOnWhiteEnabled && _robot->IsPickedUp()) {
    LOG_INFO(
        "CliffSensorComponent.NotifyOfRobotStateInternal."
        "AutoDisableStopOnWhiteDueToPickup",
        "");
...
void CliffSensorComponent::UpdateCliffDetectThresholds() {
  // Estimate whether or not we are on a dark surface and adjust the cliff
  // allowedDelta to reduce false positive cliff detections. Note that this will
  // also reduce the allowedDelta while the robot is in the air, but that is
  // fine since we don't know what kind of surface he'll be on when he's placed
  // down on his treads again. If it's a bright surface, the allowedDelta will
  // automatically increase again.
  const auto minMaxCliffItPair =
      std::minmax_element(std::begin(_cliffDataRaw), std::end(_cliffDataRaw));
  const auto minCliffVal = *minMaxCliffItPair.first;
  const auto maxCliffVal = *minMaxCliffItPair.second;
  const bool IsRobotSittingFlat = _robot->GetPitchAngle().IsNear(
      0.f, kRobotPitchThresholdPossibleCliff_rad);
  if (maxCliffVal < kCliffValDarkSurface || !IsRobotSittingFlat) {
    // All cliff values are below the dark threshold (or IMU is saying we might
    // be driving over something), so use a reduced allowedDelta to reduce false
    // cliff detections.
    _cliffDetectAllowedDelta = kCliffDetectAllowedDeltaLow;
  } else if (minCliffVal >= kCliffValDarkSurface) {
    // All cliff values are above the dark threshold and we are sitting flat, so
    // reset the allowedDelta back to the default.
    _cliffDetectAllowedDelta = kCliffDetectAllowedDeltaDefault;
  }

  // Update minimum observed cliff sensor values and cliff thresholds (using
  // filtered data):
  for (int i = 0; i < kNumCliffSensors; i++) {
    _cliffMinObserved[i] = std::min(_cliffMinObserved[i],
                                    static_cast<uint16_t>(_cliffDataFilt[i]));
    uint16_t newThresh = _cliffMinObserved[i] + _cliffDetectAllowedDelta;
    newThresh = Util::Clamp(newThresh, CLIFF_SENSOR_THRESHOLD_MIN,
                            CLIFF_SENSOR_THRESHOLD_MAX);
...
void CliffSensorComponent::QueueCliffThresholdUpdate() {
  if (_nextCliffThresholdUpdateToRobot_ms == 0) {
    _nextCliffThresholdUpdateToRobot_ms =
        _latestMsgTimestamp + kCliffThresholdMaxUpdateRate_ms;
  }
}
...
void CliffSensorComponent::SendCliffDetectThresholdsToRobot() {
  ...
  LOG_DEBUG(
      "CliffSensorComponent.SendCliffDetectThresholdsToRobot.SendThresholds",
      "New cliff thresholds being sent to robot: %d %d %d %d",
      _cliffDetectThresholds[0], _cliffDetectThresholds[1],
      _cliffDetectThresholds[2], _cliffDetectThresholds[3]);
  ...
  _robot->Broadcast(ExternalInterface::MessageEngineToGame(
      SetCliffDetectThresholds(_cliffDetectThresholds)));
#endif
}

u16 CliffSensorComponent::GetCliffDetectThreshold(unsigned int ind) const {
  DEV_ASSERT(ind < kNumCliffSensors,
             "CliffSensorComponent.GetCliffDetectThreshold.InvalidIndex");
  return _cliffDetectThresholds[ind];
}

void CliffSensorComponent::SetCliffDetectThreshold(unsigned int ind,
                                                   uint16_t newThresh) {
  DEV_ASSERT(ind < kNumCliffSensors,
             "CliffSensorComponent.SetCliffDetectThreshold.InvalidIndex");

  DEV_ASSERT(newThresh >= CLIFF_SENSOR_THRESHOLD_MIN ||
                 newThresh <= CLIFF_SENSOR_THRESHOLD_MAX,
             "CliffSensorComponent.SetCliffDetectThreshold.OutOfRange");
  ...
  if (curThresh != newThresh) {
    LOG_DEBUG("CliffSensorComponent.SetCliffDetectThreshold.NewThreshold",
              "New cliff threshold for %s (old: %d, new %d). Message to robot "
              "queued.",
              EnumToString(static_cast<CliffSensor>(ind)), curThresh,
              newThresh);
    curThresh = newThresh;
...
void CliffSensorComponent::UpdateLatestCliffDetectionDuration() {
  // Cache the latest number of cliffs detected to compare later whether it
  // changes after the recount
  ...
  if (_latestNumCliffsDetected != prevNumCliffsDetected) {
    // When the number of cliffs detected increases, update all start times for
    // entries for cliff detections greater than the previous number seen, up to
    // the current level. Otherwise, this function call does nothing when the
    // number of cliffs decreases.
    std::fill_n(_cliffDetectionTimes_ms.begin() + prevNumCliffsDetected + 1,
                _latestNumCliffsDetected - prevNumCliffsDetected, currTime);

    // When we stop detecting any cliffs, the above call won't work to populate
    // the zeroth entry in the array of start times, so we must update it
    // manually.
    if (_latestNumCliffsDetected == 0) {
      _cliffDetectionTimes_ms[_latestNumCliffsDetected] = currTime;
    }

    // When the number of cliffs detected decreases, reset all start times for
    // entries that are tracking cliff detections greater than the current
    // level. Otherwise, this function call does nothing when the number of
    // cliffs increases.
    std::fill_n(_cliffDetectionTimes_ms.begin() + _latestNumCliffsDetected + 1,
                prevNumCliffsDetected - _latestNumCliffsDetected, 0);

    // When we start detecting any cliffs at all, we don't want the start time
    // of the zero-cliff-detection slot to remain at zero, since it's not true
    // that zero cliffs are still being detected.
...
u32 CliffSensorComponent::GetDurationForAtLeastNCliffDetections_ms(
    const int minNumCliffs) const {
  DEV_ASSERT(minNumCliffs >= 0 && minNumCliffs <= kNumCliffSensors,
             "CliffSensorComponent.GetDurationForAtLeastNCliffDetections."
             "InvalidNumCliffs");
  ...
  return (cliffDetectionStartTime > 0) && (cliffDetectionStartTime < currTime)
             ? (currTime - cliffDetectionStartTime)
             : 0u;
}

u32 CliffSensorComponent::GetDurationForAtMostNCliffDetections_ms(
    const int numCliffs) const {
  DEV_ASSERT(numCliffs >= 0 && numCliffs <= kNumCliffSensors,
             "CliffSensorComponent.GetDurationForAtMostNCliffDetections_ms."
             "InvalidNumCliffs");
...
bool CliffSensorComponent::GetCliffPoseRelativeToRobot(
    const uint8_t cliffDetectedFlags, Pose3d& relativePose) const {
  // The cliff pose depends on which cliff sensors were tripped
  // Bit flags for each of the cliff sensors:
  static const uint8_t FL =
      (1 << Util::EnumToUnderlying(CliffSensor::CLIFF_FL));
  static const uint8_t FR =
      (1 << Util::EnumToUnderlying(CliffSensor::CLIFF_FR));
  static const uint8_t BL =
      (1 << Util::EnumToUnderlying(CliffSensor::CLIFF_BL));
  static const uint8_t BR =
      (1 << Util::EnumToUnderlying(CliffSensor::CLIFF_BR));
  ...
  switch (cliffDetectedFlags) {
    case (FL | FR):
      ...
      break;
    case FL:
      relativePose.SetTranslation(
          {kCliffSensorXOffsetFront_mm, kCliffSensorYOffset_mm, 0.f});
    ...
    case BL:
      relativePose.SetTranslation(
          {kCliffSensorXOffsetRear_mm, kCliffSensorYOffset_mm, 0.f});
    ...
    case (FL | BL):
      relativePose.SetTranslation(
          {(kCliffSensorXOffsetFront_mm + kCliffSensorXOffsetRear_mm) / 2.f,
           kCliffSensorYOffset_mm, 0.f});
    ...
    case (BL | BR):
      ...
      break;
    default:
      // Improbable combination of cliff sensors - just assume robot hit cliff
      // straight-on
      relativePose.SetTranslation({kCliffSensorXOffsetFront_mm, 0.f, 0.f});
      relativePose.SetRotation(0.f, Z_AXIS_3D());
      return false;
...
bool CliffSensorComponent::ComputeCliffPose(uint32_t timestampOfCliff,
                                            uint8_t cliffDetectedFlags,
                                            Pose3d& cliffPose) const {
  if (cliffDetectedFlags == 0) {
    LOG_WARNING("CliffSensorComponent.ComputeCliffPose.NoCliff",
                "CliffEvent::detectedFlags == 0! Can't compute a cliff pose if "
                "there was no cliff detected.");
    return false;
  }
  ...
  const auto& res = _robot->GetStateHistory()->ComputeStateAt(
      timestampOfCliff, histTimestamp, histState, kUseInterp);
  if (res != RESULT_OK) {
  ...
  const bool isValidPose =
      GetCliffPoseRelativeToRobot(cliffDetectedFlags, cliffWrtRobot);
  if (!isValidPose) {
    LOG_INFO("CliffSensorComponent.ComputeCliffPose.NoPoseForCliffFlags",
  ...
  if (!cliffWrtRobot.GetWithRespectTo(_robot->GetWorldOrigin(), cliffPose)) {
    LOG_ERROR("CliffSensorComponent.ComputeCliffPose.OriginMismatch",
              "cliffWrtRobot and robot.GetWorldOrigin() do not share the same "
              "origin!");
    return false;
  }
...
void CliffSensorComponent::UpdateNavMapWithCliffAt(
    const Pose3d& pose, const uint32_t timestamp) const {
  Point2f cliffSize{10.f, ROBOT_BOUNDING_Y};
  Quad2f cliffquad{{+cliffSize.x(), +cliffSize.y() * .5f},  // up L
