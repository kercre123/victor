...
void TouchBaselineCalibrator::UpdateBaseline(float reading, bool isPressed) {
  ...
  if (!IsCalibrated() && isBaselineStable) {
    ...
    if (_numStableBaselineReadings > kBaselineMinReadingsCountForStability) {
      LOG_INFO("TouchSensorComponent.UpdateBaseline.Calibrated", "%4.2f",
               _baseline);
      _isCalibrated = true;
    }
  ...
  if (!IsCalibrated()) {
    ...
    if (_baseline < 0.0f) {
      ...
      _baseline = kBaselineFilterCoeffFast * reading +
                  (1.0f - kBaselineFilterCoeffFast) * _baseline;
    }
    ...
    if (_numCalibrationReadings > kBaselineMaxCalibReadings) {
      PRINT_NAMED_WARNING(
          "TouchSensorComponent.UpdateBaseline.CalibratedWithoutStability",
          "%4.2f", _baseline);
      _isCalibrated = true;
    }
  } else {
    ...
    const bool maybeTouch =
        (reading >= (_baseline + _maxAllowedOffsetFromBaseline));
    ...
    if (!isPressed && !maybeTouch) {
      ...
      if (_numConsecNoTouch >= kBaselineMinCountNoTouch) {
        DEV_ASSERT_MSG(_baseline > 0.0f,
                       "TouchBaselineCalibrator.BaselineNotFastCalibrated", "");
        if (_durationForFastCalibration > 0) {
          ...
          if (_durationForFastCalibration == 0) {
            ...
            DASMSG_SET(i1, std::round(_baseline),
                       "baseline value after fast calibration");
            DASMSG_SEND();
          }
        } else {
          _baseline = kBaselineFilterCoeffSlow * reading +
                      (1.0f - kBaselineFilterCoeffSlow) * _baseline;
        }
  ...
  // vice versa.
  // In this mode, we:
  // + initialize and accumulate proposal baseline, separate from the current
  // baseline
  ...
  if (_chargerModeChanged) {
    if (_numPostChargerModeReadings < kChargerModeCheckDelayAccumulation) {
    ...
    } else if (_numPostChargerModeReadings ==
               kChargerModeCheckDelayAccumulation) {
      ...
      _postChargerModeChangeBaseline =
          kBaselineFilterCoeffFast * reading +
          (1.0f - kBaselineFilterCoeffFast) * _postChargerModeChangeBaseline;
      _numPostChargerModeReadings++;
      if (_numPostChargerModeReadings > kChargerModeCheckTimeout) {
        // did not receive enough evidence that we should overwrite the current
        // baseline
        LOG_INFO(
            "TouchBaselineCalibrator.UpdateBaseline."
            "NoBaselineChangeDueToChargerModeSwitch",
            "%d", _numTicksLargeDifferenceInBaselines);
        ...
        DASMSG_SET(i1, (int)_numTicksLargeDifferenceInBaselines,
                   "number of ticks of large deviation between "
                   "proposal/current baseline");
        DASMSG_SEND();
        _chargerModeChanged = false;
        ResetBaselineMonitoring();
      } else if (_numPostChargerModeReadings >
                 kChargerModeCheckMinBaselineReadings) {
        _numTicksLargeDifferenceInBaselines +=
            fabsf(_baseline - _postChargerModeChangeBaseline) >
            kChargerModeCheckAllowedDifference;
        if (_numTicksLargeDifferenceInBaselines >
            kChargerModeCheckMinLargeDiffCount) {
          // received enough evidence since initializing the proposal baseline
          // to overwrite
          LOG_INFO(
              "TouchBaselineCalibrator.UpdateBaseline."
              "BaselineChangedDueToChargerModeSwitch",
              "%4.2f, %4.2f", _baseline, _postChargerModeChangeBaseline);
          ...
          DASMSG_SET(i2, std::round(_postChargerModeChangeBaseline),
                     "new baseline value");
          DASMSG_SEND();
    ...
    const bool readingTooLow =
        (_baseline - reading) > kAllowedDifferenceFromBaseline;
    if (readingTooLow && _durationForFastCalibration == 0) {
      _numTicksReadingsTooLowFromBaseline++;
      if (_numTicksReadingsTooLowFromBaseline >
          kMinTooLowReadingsForFastCalib) {
        LOG_INFO(
            "TouchBaselineCalibrator.UpdateBaseline."
            "FastCalibrationDueToLowSignal",
            "%4.2f", _baseline);
        DASMSG(touch_baseline_reading_difference_too_low,
               "touch_sensor.baseline_reading_difference_monitor_too_low",
               "send when robot detects the current signal input is deviating "
               "below the baseline too much for too long");
...
// values chosen manually after inspecting some robots --- parameter sweep not
// necessary
const TouchSensorComponent::FilterParameters
    TouchSensorComponent::_kFilterParamsForProd = {
        .boxFilterSize = 5,
        .dynamicThreshGap = 20,
...
TouchSensorComponent::TouchSensorComponent()
    : ISensorComponent(kLogDirectory),
      IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::TouchSensor),
  ...
  static const std::set<u32> exceptionESNWithPVThardware = {
      0x00e10017, 0x00e10074, 0x00e10034, 0x00e10057, 0x00e1006d,
      0x00e10008, 0x00e1005c,
      0x00e10052,  // data set robot
      0x00e10062, 0x00e1001f, 0x00e10032, 0x00e10064, 0x00e1006c,
      0x00e10069, 0x00e10014, 0x00e10009, 0x00e1006e, 0x00e1001d,
  };
  ...
  const bool isExceptionESN = exceptionESNWithPVThardware.find(esn) !=
                              exceptionESNWithPVThardware.end();
  _enabled = isValidESN || isExceptionESN;

  // send up the OS version (useful to determine if corresponding required
  // syscon changes are present)
...
TouchSensorComponent::~TouchSensorComponent() { DevLogTouchSensorData(); }

void TouchSensorComponent::InitDependent(Robot* robot,
        ...
        auto onData =
            [this](
              ...
              if (data.isMember("enabled")) {
                const std::string& stateStr = JsonTools::ParseString(
                    data, "enabled", "TouchSensorComponent.WebVizJsonCallback");
              ...
              if (context != nullptr) {
                ...
                if (webService != nullptr) {
                  _toSendJson["enabled"] = _enabled ? "true" : "false";
                  _toSendJson["count"] = (int)_touchCountForWebviz;
                  webService->SendToWebViz(kWebVizTouchSensorModuleName,
                                           _toSendJson);
                }
              }
            };
        _signalHandles.emplace_back(
            webService->OnWebVizData(kWebVizTouchSensorModuleName)
...
void TouchSensorComponent::DevLogTouchSensorData() const {
  if (ANKI_DEV_CHEATS) {
    std::string path = _robot->GetContextDataPlatform()->pathToResource(
        Util::Data::Scope::Cache, kLogDirectory);
    ...
    if (!Util::FileUtils::WriteFile(path, output.str())) {
      PRINT_NAMED_WARNING("TouchSensorComponent.FailedToWrite", "%s",
                          path.c_str());
    }
  }
}
...
void TouchSensorComponent::NotifyOfRobotStateInternal(const RobotState& msg) {
  ...
  if (context != nullptr) {
    auto* webService = context->GetWebService();
    if (webService != nullptr &&
        webService->IsWebVizClientSubscribed(kWebVizTouchSensorModuleName)) {
      const float now_sec =
          BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
      if (now_sec > _nextSendWebVizDataTime_sec) {
        _toSendJson["enabled"] = _enabled ? "true" : "false";
        _toSendJson["count"] = (int)_touchCountForWebviz;
        _toSendJson["calibrated"] =
            _baselineCalibrator.IsCalibrated() ? "yes" : "no";
  ...
  // in the process of going from DVT to PVT, the touch sensor hardware changed
  // enough to require a new scheme of sampling data from the sensor itself. As
  // a result, the DVT sensors are no longer supported. To guard against
  // undefined behavior, we disable the sensor in the robotics layer, if we
  // detect here that the robot is not PVT hardware.
  //
  ...
  if (wasOnCharger != _isOnCharger &&
    ...
    LOG_INFO(
        "TouchSensorComponent.NotifyOfRobotStateInternal."
        "BaselineChargerModeCheckActivated",
        "");
    DASMSG(touch_charger_mode_check_activate,
           "touch_sensor.activate_charger_mode_check",
           "send when robot is checking the baseline because of a charger "
           "state change");
    DASMSG_SET(i1, (int)_isOnCharger,
               "whether the robot is on the charger or not");
    DASMSG_SEND();
  ...
  if (!_baselineCalibrator.IsCalibrated()) {
    // note: treat isPressed as false, because we cannot detect touch while
    // uncalibrated
    ...
    if (!_baselineCalibrator.IsChargerModeCheckRunning()) {
      _isPressed = ProcessWithDynamicThreshold(
          std::floor(_baselineCalibrator.GetBaseline()),
    ...
    if (kTestOnlyLoggingEnabled) {
      ...
      fprintf(fp, "%d,%d,%d,%d,%d\n", _lastRawTouchValue, _isPressed,
              (int)_baselineCalibrator.GetBaseline(), _confirmedPressState,
              _baselineCalibrator.IsChargerModeCheckRunning());
    }
#endif

    DEV_ASSERT(_baselineCalibrator.IsCalibrated(),
               "TouchSensorComponent.ClassifyingBeforeCalibration");
    // note: the baseline calibrator uses the raw touch detection, instead of
    // the confirmed
    //  one instead, because we prefer it to be consevative in the values that
    //  it will accumulate
  ...
  if (ANKI_DEV_CHEATS) {
    DevLogTouchRow entry{_lastRawTouchValue,
                         boxFiltVal,
    ...
    if (_devLogMode == PostTouch) {
      ...
      if (_devLogSampleCount > kMinPostTouchSamples ||
          _devLogBuffer.size() > kMaxDevLogBufferSize) {
...
bool TouchSensorComponent::ProcessWithDynamicThreshold(const int baseline,
                                                       const int input) {
  ...
  if (input > detectLevel) {
    ...
    if ((input - detectLevel) > _filterParams.dynamicThreshDetectFollowOffset) {
      ...
      _countBelowUndetectLevel = 0;
      if (_countAboveDetectLevel >
          _filterParams.minConsecCountToUpdateThresholds) {
        const auto candidateDetectOffset =
            input - _filterParams.dynamicThreshDetectFollowOffset - baseline;
        _detectOffsetFromBase =
            MIN(_filterParams.dynamicThreshMaximumDetectOffset,
                candidateDetectOffset);
        // detect and undetect levels move in lockstep
        _undetectOffsetFromBase =
            _detectOffsetFromBase - _filterParams.dynamicThreshGap;
        _countAboveDetectLevel = 0;
      }
    } else {
    ...
    if ((undetectLevel - input) >
        _filterParams.dynamicThreshUndetectFollowOffset) {
      _countBelowUndetectLevel++;
      _countAboveDetectLevel = 0;
      if (_countBelowUndetectLevel >
          _filterParams.minConsecCountToUpdateThresholds) {
        auto candidateUndetectOffset =
            input + _filterParams.dynamicThreshUndetectFollowOffset - baseline;
        _undetectOffsetFromBase =
            MAX(_filterParams.dynamicThreshMininumUndetectOffset,
                candidateUndetectOffset);
        // detect and undetect levels move in lockstep
        _detectOffsetFromBase =
            _undetectOffsetFromBase + _filterParams.dynamicThreshGap;
        _countBelowUndetectLevel = 0;
      }
    } else {
      // tracks count of *consecutive* readings which are
...
bool TouchSensorComponent::GetIsPressed() const { return _confirmedPressState; }
