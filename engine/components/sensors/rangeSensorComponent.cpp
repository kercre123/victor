...
RangeSensorComponent::RangeSensorComponent()
    : IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::RangeSensor) {}

RangeSensorComponent::~RangeSensorComponent() { ToFSensor::removeInstance(); }

void RangeSensorComponent::InitDependent(Robot* robot,
                                         const RobotCompMap& dependentComps) {
  _robot = robot;

  // Subscribe to the SendRangeData request
  auto sendRangeDataLambda =
  ...
  _signalHandle = _robot->GetRobotMessageHandler()->Subscribe(
      RobotInterface::RobotToEngineTag::sendRangeData, sendRangeDataLambda);
}
...
void RangeSensorComponent::UpdateDependent(const RobotCompMap& dependentComps) {
  ...
  // If we have been requested to send range data, then populate a
  // RangeDataToDisplay message
  if (_sendRangeData) {
    ...
    for (const auto& e : _latestRawRangeData.data) {
      ...
      for (const auto& reading : e.readings) {
        // Use the status and signalRate from the reading that corresponds to
        // the processedRange_mm
  ...
  Pose3d co = _robot->GetCameraPose(
      _robot->GetComponent<FullRobotPose>().GetHeadAngle());
  // Parent a pose to the camera so we can rotate our current camera axis (Z out
  // of camera) to match world axis (Z up) also account for angle tof sensor is
  // relative to camera
  ...
  const auto axis = Z_AXIS_3D();
    ...
    TOF_CONFIGURATION == TOF_CENTER_OF_FACE
  ...
  Pose3d lp(leftAngle, axis,
            {TOF_LEFT_TRANS_REL_CAMERA_MM[0], TOF_LEFT_TRANS_REL_CAMERA_MM[1],
             TOF_LEFT_TRANS_REL_CAMERA_MM[2]},
            c, "leftProx");
  Pose3d rp(rightAngle, axis,
            {TOF_RIGHT_TRANS_REL_CAMERA_MM[0], TOF_RIGHT_TRANS_REL_CAMERA_MM[1],
...
// lp.RotateBy(Rotation3d(DEG_TO_RAD(180), X_AXIS_3D()));
  ...
  //     const f32 leftDist_mm = _latestRawRangeData.data[c +
  //     (r*8)].processedRange_mm;
