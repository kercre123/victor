...
class ISensorComponent : private Util::noncopyable {
 ...
 protected:
  ...
  // GetLogHeader() should return a comma-separated header line that describes
  // the data being logged
  virtual std::string GetLogHeader() = 0;

  // GetLogRow() should return a comma-separated row of data for logging, with
  // columns corresponding to the header
