...
class CliffSensorComponent
    : public ISensorComponent,
      public IDependencyManagedComponent<RobotComponentID> {
 public:
  static const int kNumCliffSensors =
      static_cast<int>(CliffSensor::CLIFF_COUNT);

  using CliffSensorDataArray = std::array<uint16_t, kNumCliffSensors>;
  static_assert(std::is_same<CliffSensorDataArray,
                             decltype(RobotState::cliffDataRaw)>::value,
                "CliffSensorDataArray must be same as type used in RobotState");
 ...
 public:
  ...
  bool IsCliffDetected(const CliffSensor sensor) const {
    return _cliffDetectedFlags.IsBitFlagSet(sensor);
  }
  uint8_t GetCliffDetectedFlags() const {
    return _cliffDetectedFlags.GetFlags();
  }

  // White detection based on RobotState
  bool IsWhiteDetected() const { return _whiteDetectedFlags.AreAnyFlagsSet(); }
  bool IsWhiteDetected(const CliffSensor sensor) const {
    return _whiteDetectedFlags.IsBitFlagSet(sensor);
  }
  uint8_t GetWhiteDetectedFlags() const {
    return _whiteDetectedFlags.GetFlags();
  }
  ...
  const CliffSensorDataArray& GetCliffDetectThresholds() const {
    return _cliffDetectThresholds;
  }

  // Sends message to the robot process containing all of the current cliff
  ...
  // - timestamp represents the time at which the cliff was detected (used to
  // calculate
  //   the robot's historical pose)
  // - cliff detected flags used to estimate the cliff's pose relative to the
  // robot
  bool ComputeCliffPose(uint32_t timestampOfCliff, uint8_t cliffDetectedFlags,
                        Pose3d& cliffPose) const;

  // given a set of cliff detected flags, gets the pose of the cliff relative to
  // the robot returns true if the output pose is valid
  bool GetCliffPoseRelativeToRobot(const uint8_t cliffDetectedFlags,
                                   Pose3d& relativePose) const;

  // mark a quad of fixed dimensions as cliff-type obstacles in the navmap
  void UpdateNavMapWithCliffAt(const Pose3d& pose,
  ...
  // Returns latest timestamp in seconds of when the robot was put down on a
  // cliff.
  TimeStamp_t GetLatestPutDownOnCliffTime_ms() const {
    return _lastPutDownOnCliffTime_ms;
  }

  // Returns latest timestamp in seconds of a stop due to a cliff detection
  TimeStamp_t GetLatestStopDueToCliffTime_ms() const {
    return _lastStopDueToCliffTime_ms;
  }

  const CliffSensorDataArray& GetCliffDataRawAtLastStop() const {
    return _cliffDataRawAtLastStop;
  }

  const CliffSensorFiltDataArray& GetCliffDataFiltered() const {
    return _cliffDataFilt;
  }

  // Retrieves the number of milliseconds that the cliff sensors have reported
  // @param numCliffs being detected.
  //
  // If @param numCliffs is a positive value, this assumes that reports of N
  // cliff detections from the sensors, where N is greater than @param
  // numCliffs, also counts as valid time to include in the reported duration.
  //
  // If @param numCliffs is 0, then this assumes that the user wants only the
  // duration of time that the cliff sensors have currently been reporting
  // _exactly_ zero cliffs detections.
  u32 GetDurationForAtLeastNCliffDetections_ms(const int minNumCliffs) const;

  // Returns the amount of time that @param numCliff or fewer cliffs have been
  // detected for. (Returns MAX_UINT for numCliffs == 4)
  ...
  // Returns the max number of cliffs observed while picked up (according to
  // RobotState.status) Returns 0 if not currently picked up.
  int GetMaxNumCliffsDetectedWhilePickedUp() const {
    return _maxNumCliffsDetectedWhilePickedUp;
  }
 ...
 private:
  ...
  // Minimum observed cliff sensor values (used to adaptively adjust detection
  // thresholds)
  CliffSensorDataArray _cliffMinObserved;

  // This is the allowed delta above the minimum-ever-seen cliff sensor value
  // below which cliff detection is triggered. In other words, if the
  // lowest-ever-seen value for a given cliff sensor is 90, and
  // _cliffDetectAllowedDelta is 40, then the cliff detection threshold will be
  // set at 130 (90+40). If the raw cliff value ever falls below the threshold,
  // cliff detection is triggered. This value can also be dynamically adjusted
  // if it is suspected that we are driving on a surface with poor IR
  // reflectivity (e.g. a carpet).
  ...
  // Raw cliff sensor data when the last RobotStopped event occurred due to a
  // cliff detection.
