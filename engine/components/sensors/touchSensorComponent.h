...
class BoxFilter {
 public:
  BoxFilter(int windowSize) : _buffer(), _winSize(windowSize), _sum(0) {}
...
class TouchBaselineCalibrator {
 public:
  ...
  bool IsChargerModeCheckRunning() const { return _chargerModeChanged; }
...
class TouchSensorComponent
 ...
 public:
  ...
  void UnitTestOnly_SetFilterParameters(FilterParameters params) {
    ...
    _detectOffsetFromBase =
        params.dynamicThreshMininumUndetectOffset + params.dynamicThreshGap;
 ...
 public:
  u16 GetLatestRawTouchValue() const { return _lastRawTouchValue; }

  bool IsCalibrated() const { return _baselineCalibrator.IsCalibrated(); }
 ...
 private:
  ...
  // TODO(Al): Could probably move the recording logic to IBehaviorPlaypen by
  ...
  // Pointer to a struct that should be populated with touch sensor data when
  // recording
  ...
  enum DevLogMode { PreTouch, PostTouch };
