...
//  Because ImuComponent inherits a bunch of stuff as a component, including
//  being non-copyable. This gives us a thin wrapper around the actual
//  historical info, which can be copied to, and queried by, the vision thread,
//  as part of its VisionPoseData input.
//
class ImuHistory {
 public:
  ImuHistory() = default;
 ...
 private:
  ...
  bool GetImuDataBeforeAndAfter(RobotTimeStamp_t t, IMUDataFrame& before,
                                IMUDataFrame& after) const;

  // Returns true if the any of the numToLookBack imu data before timestamp t
  // have rates that are greater than the given rates
  bool IsImuDataBeforeTimeGreaterThan(const RobotTimeStamp_t t,
                                      const int numToLookBack, const f32 rateX,
                                      const f32 rateY, const f32 rateZ) const;
...
//  See note above for ImuHistory as to why we have a separate component
//  wrapper.
//
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
inline bool ImuHistory::WasRotatingTooFast(
    RobotTimeStamp_t t, const f32 bodyTurnSpeedLimit_radPerSec,
    const f32 headTurnSpeedLimit_radPerSec,
    const int numImuDataToLookBack) const {
  return (WasHeadRotatingTooFast(t, headTurnSpeedLimit_radPerSec,
                                 numImuDataToLookBack) ||
          WasBodyRotatingTooFast(t, bodyTurnSpeedLimit_radPerSec,
                                 numImuDataToLookBack));
}
