...
AccountSettingsManager::AccountSettingsManager()
    : IDependencyManagedComponent(this,
                                  RobotComponentID::AccountSettingsManager) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AccountSettingsManager::InitDependent(
    Robot* robot, const RobotCompMap& dependentComponents) {
  _robot = robot;
  _jdocsManager = &_robot->GetComponent<JdocsManager>();

  _accountSettingsConfig =
      &_robot->GetContext()->GetDataLoader()->GetAccountSettingsConfig();
  ...
  const bool jdocNeedsCreation = _jdocsManager->JdocNeedsCreation(
      external_interface::JdocType::ACCOUNT_SETTINGS);
  _currentAccountSettings.clear();
  if (jdocNeedsCreation) {
    LOG_INFO(
        "AccountSettingsManager.InitDependent.NoAccountSettingsJdocFile",
        "Account settings jdoc file not found; one will be created shortly");
  } else {
    _currentAccountSettings = _jdocsManager->GetJdocBody(
        external_interface::JdocType::ACCOUNT_SETTINGS);
  ...
  for (Json::ValueConstIterator it = _accountSettingsConfig->begin();
       it != _accountSettingsConfig->end(); ++it) {
    if (!_currentAccountSettings.isMember(it.name())) {
      ...
      settingsDirty = true;
      LOG_INFO("AccountSettingsManager.InitDependent.AddDefaultItem",
  ...
  for (Json::ValueConstIterator it = _currentAccountSettings.begin();
       it != _currentAccountSettings.end(); ++it) {
  ...
  if (settingsDirty) {
    ...
    UpdateAccountSettingsJdoc(kSaveToCloudImmediately,
  ...
  _settingSetters[external_interface::AccountSetting::DATA_COLLECTION] = {
      nullptr, &AccountSettingsManager::ApplyAccountSettingDataCollection};
  _settingSetters[external_interface::AccountSetting::APP_LOCALE] = {
      &AccountSettingsManager::ValidateAccountSettingAppLocale, nullptr};

  _jdocsManager->RegisterOverwriteNotificationCallback(
      external_interface::JdocType::ACCOUNT_SETTINGS, [this]() {
        _currentAccountSettings = _jdocsManager->GetJdocBody(
            external_interface::JdocType::ACCOUNT_SETTINGS);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AccountSettingsManager::UpdateDependent(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool AccountSettingsManager::SetAccountSetting(
  ...
  if (!_currentAccountSettings.isMember(key)) {
    LOG_ERROR("AccountSettingsManager.SetAccountSetting.InvalidKey",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::string AccountSettingsManager::GetAccountSettingAsString(
  ...
  if (!_currentAccountSettings.isMember(keyString)) {
    LOG_ERROR("AccountSettingsManager.GetRobotSettingAsString.InvalidKey",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool AccountSettingsManager::GetAccountSettingAsBool(
    const external_interface::AccountSetting key) const {
  const std::string& keyString = AccountSetting_Name(key);
  if (!_currentAccountSettings.isMember(keyString)) {
    LOG_ERROR("AccountSettingsManager.GetRobotSettingAsBool.InvalidKey",
              "Invalid key %s", keyString.c_str());
    return false;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
uint32_t AccountSettingsManager::GetAccountSettingAsUInt(
    const external_interface::AccountSetting key) const {
  const std::string& keyString = AccountSetting_Name(key);
  if (!_currentAccountSettings.isMember(keyString)) {
    LOG_ERROR("AccountSettingsManager.GetRobotSettingAsUInt.InvalidKey",
              "Invalid key %s", keyString.c_str());
    return 0;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool AccountSettingsManager::DoesSettingUpdateCloudImmediately(
    const external_interface::AccountSetting key) const {
  const std::string& keyString = AccountSetting_Name(key);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool AccountSettingsManager::UpdateAccountSettingsJdoc(
  ...
  const bool success = _jdocsManager->UpdateJdoc(
      external_interface::JdocType::ACCOUNT_SETTINGS, &_currentAccountSettings,
...
void AccountSettingsManager::ApplyAllCurrentAccountSettings() {
  LOG_INFO("AccountSettingsManager.ApplyAllCurrentAccountSettings",
           "Applying all current account settings");
  for (Json::ValueConstIterator it = _currentAccountSettings.begin();
       it != _currentAccountSettings.end(); ++it) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool AccountSettingsManager::ApplyAccountSetting(
    const external_interface::AccountSetting setting) {
  ...
  if (it != _settingSetters.end()) {
    ...
    if (validationFunc != nullptr) {
      ...
      if (!success) {
        LOG_ERROR(
            "AccountSettingsManager.ApplyAccountSetting.ValidateFunctionFailed",
    ...
    if (applicationFunc != nullptr) {
      LOG_DEBUG("AccountSettingsManager.ApplyAccountSetting",
                "Applying Account Setting '%s'",
                AccountSetting_Name(setting).c_str());
      success = (this->*(applicationFunc))();

      if (!success) {
        LOG_ERROR(
            "AccountSettingsManager.ApplyAccountSetting.ApplyFunctionFailed",
            "Error attempting to apply %s setting",
            AccountSetting_Name(setting).c_str());
      }
    }
...
bool AccountSettingsManager::ApplyAccountSettingDataCollection() {
  static const std::string& key =
      AccountSetting_Name(external_interface::AccountSetting::DATA_COLLECTION);
  ...
  const auto msg = EngineToRobot::CreateupdatedSettings(UpdatedSettings(
      SettingBeingChanged::SETTING_ENABLE_DATA_COLLECTION, value, ""));
...
bool AccountSettingsManager::ValidateAccountSettingAppLocale() {
  static const std::string& key =
      AccountSetting_Name(external_interface::AccountSetting::APP_LOCALE);
