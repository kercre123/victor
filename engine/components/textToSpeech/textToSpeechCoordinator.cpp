...
namespace {
// The coordinator will wait kPlayTimeoutScalar times the expected duration
// before erroring out on a TTS utterance
constexpr float kPlayTimeoutScalar = 2.0f;

// Utterances should not take longer than this to generate
constexpr float kGenerationTimeout_s =
    20.0f;  // making this high until we figure out accurate generation times
}  // namespace

// Helper to map trigger types
static TextToSpeechTriggerMode GetTriggerMode(
    UtteranceTriggerType triggerType) {
  switch (triggerType) {
  ...
  DEV_ASSERT(false,
             "TextToSpeechCoordinator.GetTriggerMode.InvalidTriggerType");
...
TextToSpeechCoordinator::TextToSpeechCoordinator()
    : IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::TextToSpeechCoordinator) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TextToSpeechCoordinator::InitDependent(
    Vector::Robot* robot, const RobotCompMap& dependentComps) {
  ...
  auto callback =
      [this](const AnkiEvent<RobotInterface::RobotToEngine>& event) {
        ...
        UpdateUtteranceState(ttsEvent.ttsID, ttsEvent.ttsState,
                             ttsEvent.expectedDuration_ms);
      };

  auto* messageHandler = _robot->GetRobotMessageHandler();
  _signalHandle = messageHandler->Subscribe(
      RobotInterface::RobotToEngineTag::textToSpeechEvent, callback);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TextToSpeechCoordinator::UpdateDependent(
  ...
  for (auto it = _utteranceMap.cbegin(); it != _utteranceMap.cend();) {
    ...
    if (UtteranceState::Generating == utterance.state &&
        (currentTime_s - utterance.timeStartedGeneration_s) >
      ...
      LOG_ERROR("TextToSpeechCoordinator.Update.GenerationTimeout",
                ...
                utteranceID, kGenerationTimeout_s);
    ...
    if (UtteranceState::Playing == utterance.state) {
      ...
      float playingTimeout_s =
          utterance.expectedDuration_s * kPlayTimeoutScalar;
      if (playTime_s >= playingTimeout_s) {
        LOG_WARNING("TextToSpeechCoordinator.Update.UtterancePlayingTimeout",
                    "Utterance %d was expected to play for %f seconds, timed "
                    "out after %f",
                    utteranceID, utterance.expectedDuration_s, playTime_s);
        UpdateUtteranceState(utteranceID, TextToSpeechState::Invalid);
      }
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
const uint8_t TextToSpeechCoordinator::CreateUtterance(
    const std::string& utteranceString, const UtteranceTriggerType& triggerType,
    const AudioTtsProcessingStyle& style, const float durationScalar,
    const float pitchScalar, const UtteranceUpdatedCallback callback) {
  RobotInterface::TextToSpeechPrepare msg;
  const size_t maxTextLength = msg.text.max_size() - 1;

  // we store the string in a 1024 character array, so we need can't have a
  // string greater than this (including the null terminating character).
  if (utteranceString.length() > maxTextLength) {
    LOG_ERROR(
        "TextToSpeechCoordinator.CreateUtterance",
        "Utterance string cannot be longer than %d characters (yours is %d)",
        (int)maxTextLength, (int)utteranceString.length());
  ...
  std::memcpy(msg.text.data(), utteranceString.c_str(),
              utteranceString.size() + 1);
  ...
  // Update the state here so that the callback is run at least once before the
  // user checks the state of the utterance they've just created
...
const uint8_t TextToSpeechCoordinator::CreateUtterance(
    const std::string& text, const UtteranceTriggerType& triggerType,
    const AudioTtsProcessingStyle& style, UtteranceUpdatedCallback callback) {
  return CreateUtterance(text, triggerType, style, 1.f, 0.f, callback);
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
const UtteranceState TextToSpeechCoordinator::GetUtteranceState(
    const uint8_t utteranceID) const {
  auto it = _utteranceMap.find(utteranceID);
  if (it != _utteranceMap.end()) {
    return it->second.state;
  } else {
    LOG_ERROR("TextToSpeechCoordinator.UnknownUtteranceID",
              "State was requested for utterance %d for which there is no "
              "valid record",
              utteranceID);
...
const bool TextToSpeechCoordinator::PlayUtterance(const uint8_t utteranceID) {
  ...
  if (it == _utteranceMap.end()) {
    LOG_ERROR(
        "TextToSpeechCoordinator.PlayUtterance.UnknownUtterance",
  ...
  if (UtteranceTriggerType::Manual != triggerType &&
      UtteranceTriggerType::KeyFrame != triggerType) {
...
const bool TextToSpeechCoordinator::CancelUtterance(const uint8_t utteranceID) {
  ...
  if (it == _utteranceMap.end()) {
    LOG_ERROR(
        "TextToSpeechCoordinator.UnknownUtteranceID",
  ...
  // notes:
  //  + if cancelling when it's generating, thread will hang until generation is
  //  complete
  ...
  if ((UtteranceState::Invalid != utteranceState) &&
      (UtteranceState::Finished != utteranceState)) {
    LOG_INFO("TextToSpeechCoordinator.CancelUtteranceGeneration",
             "Cancel ttsID %d", utteranceID);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TextToSpeechCoordinator::UpdateUtteranceState(
    const uint8_t& ttsID, const TextToSpeechState& ttsState,
    const float& expectedDuration_ms) {
  auto it = _utteranceMap.find(ttsID);
  if (it == _utteranceMap.end()) {
    LOG_WARNING("TextToSpeechCoordinator.StateUpdate.Invalid",
                "Received state update for invalid utterance %d", ttsID);
    return;
  }
  ...
  switch (ttsState) {
    case TextToSpeechState::Invalid: {
      LOG_WARNING("TextToSpeechCoordinator.StateUpdate",
                  "ttsID %d operation failed", ttsID);
      utterance.state = UtteranceState::Invalid;
      break;
    }
    case TextToSpeechState::Preparing: {
      LOG_INFO("TextToSpeechCoordinator.StateUpdate",
               "ttsID %d is being prepared", ttsID);
      utterance.state = UtteranceState::Generating;
      break;
    }
    case TextToSpeechState::Playable: {
      LOG_INFO("TextToSpeechCoordinator.StateUpdate",
               "ttsID %d is now playable", ttsID);
    ...
    case TextToSpeechState::Prepared: {
      LOG_INFO("TextToSpeechCoordinator.StateUpdate", "ttsID %d is prepared",
               ttsID);
      utterance.state = UtteranceState::Ready;
      utterance.expectedDuration_s = Util::MilliSecToSec(expectedDuration_ms);
      break;
    ...
    case TextToSpeechState::Playing: {
      LOG_INFO("TextToSpeechCoordinator.StateUpdate", "ttsID %d is now playing",
               ttsID);
    ...
    case TextToSpeechState::Finished: {
      float timeSinceStartedPlaying_s =
          currentTime_s - utterance.timeStartedPlaying_s;
      LOG_INFO("TextToSpeechCoordinator.StateUpdate.FinishedPlaying",
               "ttsID %d has finished playing after %f seconds", ttsID,
