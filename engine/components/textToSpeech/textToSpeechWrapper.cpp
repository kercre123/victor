...
void TextToSpeechWrapper::OnUtteranceUpdated(const UtteranceState& state) {
  ...
  if (state != previousState) {
    _state = state;
    PRINT_DEBUG("Transition from %d state to %d state", (int)previousState,
                (int)_state);

    switch (_state) {
      case UtteranceState::Invalid: {
        // if we're now invalid and have a "ready callback", then we must have
        // failed during prep if we have a "finished callback", we must have
        // been playing, so call it
        if (_readyCallback) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool TextToSpeechWrapper::SetUtteranceText(
    const std::string& text, OnUtteranceReadyCallback readyCallback,
    OnUtteranceCompleteCallback completeCallback) {
  ...
  if (UtteranceState::Invalid == _state) {
    PRINT_DEBUG("Setting utterance text: %s",
                Util::HidePersonallyIdentifiableInfo(text.c_str()));

    using AudioTTSProcessStyle =
        AudioMetaData::SwitchState::Robot_Vic_External_Processing;
    _id = _ttsCoordinator->CreateUtterance(
        text, _triggerType, AudioTTSProcessStyle::Default_Processed,
        std::bind(&TextToSpeechWrapper::OnUtteranceUpdated, this,
                  std::placeholders::_1));
...
bool TextToSpeechWrapper::PlayUtterance(OnUtteranceCompleteCallback callback) {
  DEV_ASSERT(nullptr != _ttsCoordinator, "TextToSpeechWrapper.UnInitialized");
  DEV_ASSERT(UtteranceTriggerType::Manual == _triggerType,
             "TextToSpeechWrapper.InvalidTriggerType");
  ...
  if (IsReadyToPlay() && (kInvalidUtteranceID != _id)) {
    ...
    if (_ttsCoordinator->PlayUtterance(_id)) {
      // if you double set the finished callback, you've probably done something
      // wrong, however, things will still work fine
      ASSERT_NAMED_EVENT(!_finishedCallback,
                         "TextToSpeechWrapper.PlayUtterance",
                         "Specified OnUtteranceCompleteCallback when one was "
                         "already set from SetUtteranceText(...)");
...
void TextToSpeechWrapper::CancelUtterance() {
  ...
  if ((UtteranceState::Invalid != _state) &&
      (UtteranceState::Finished != _state) && (kInvalidUtteranceID != _id)) {
