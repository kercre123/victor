 ...
 *  + Essentially just tracks TextToSpeechCoordinator states for you and
 *simplifies things
 *  + Meant to be used in cases where BehaviorTextToSpeechLoop is too heavy
 *handed, or you're rolling your own anims
 *  * Only supports Robot_Vic_External_Processing::Default_Processed, but would
 *be easy to extend if the need arose
 *
 **********************************************************************************************************************/
...
class TextToSpeechWrapper {
 public:
  ...
  // only needs to be called once, subsequent calls to Reset() will maintain
  // initialization
  ...
  // returns true when the utterance is ready to be played, false if it fails
  // during generation
  using OnUtteranceReadyCallback = std::function<void(bool)>;
  // returns true when the utterance is finished playing, false if it fails
  // during playback
  ...
  // note: OnUtteranceCompleteCallback is generally only needed when trigger
  // type is "immediate", but you do you.
  // ** once you have begun utterance generation, it is up to you to ensure that
  // either the utterance plays through,
  //    or you cancel it, else the .wav file will orphaned and taking up memory
  bool SetUtteranceText(const std::string& text, OnUtteranceReadyCallback = {},
                        OnUtteranceCompleteCallback = {});
  ...
  // "not valid" can mean there was a failure, or you haven't started generating
  // the utterance yet
