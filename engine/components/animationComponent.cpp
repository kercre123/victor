...
void AnimationComponent::InitDependent(Vector::Robot* robot,
  ...
  _animationGroups = std::make_unique<AnimationGroupWrapper>(
      *(context->GetDataLoader()->GetAnimationGroups()));
  ...
  if (extInterface != nullptr) {
    ...
    helper.SubscribeGameToEngine<
        MessageGameToEngineTag::RequestAvailableAnimations>();
    helper.SubscribeGameToEngine<
        MessageGameToEngineTag::RequestAvailableAnimationGroups>();
    helper
        .SubscribeGameToEngine<MessageGameToEngineTag::DisplayProceduralFace>();
    helper.SubscribeGameToEngine<MessageGameToEngineTag::SetFaceHue>();
    helper.SubscribeGameToEngine<
        MessageGameToEngineTag::DisplayFaceImageBinaryChunk>();
    helper.SubscribeGameToEngine<
        MessageGameToEngineTag::DisplayFaceImageRGBChunk>();
  ...
  RobotInterface::MessageHandler* messageHandler =
      _robot->GetContext()->GetRobotManager()->GetMsgHandler();

  // Subscribe to RobotToEngine messages
  using localHandlerType = void (AnimationComponent::*)(
      const AnkiEvent<RobotInterface::RobotToEngine>&);
  // Create a helper lambda for subscribing to a tag with a local handler
  auto doRobotSubscribe = [this, messageHandler](
                              RobotInterface::RobotToEngineTag tagType,
                              localHandlerType handler) {
    GetSignalHandles().push_back(messageHandler->Subscribe(
        tagType, std::bind(handler, this, std::placeholders::_1)));
  };

  // bind to specific handlers
  doRobotSubscribe(RobotInterface::RobotToEngineTag::animAdded,
                   &AnimationComponent::HandleAnimAdded);
  doRobotSubscribe(RobotInterface::RobotToEngineTag::animStarted,
                   &AnimationComponent::HandleAnimStarted);
  doRobotSubscribe(RobotInterface::RobotToEngineTag::animEnded,
                   &AnimationComponent::HandleAnimEnded);
  doRobotSubscribe(RobotInterface::RobotToEngineTag::animEvent,
                   &AnimationComponent::HandleAnimationEvent);
  doRobotSubscribe(RobotInterface::RobotToEngineTag::animState,
                   &AnimationComponent::HandleAnimState);
}
...
void AnimationComponent::Init() {
  ...
  const bool success = _robot->GetContext()->GetDataPlatform()->readAsJson(
      Util::Data::Scope::Resources, manifestFile, jsonManifest);
  if (!success) {
  ...
  for (int i = 0; i < jsonManifest.size(); ++i) {
    ...
    _availableAnims[jsonAnim[kNameField].asCString()].length_ms =
        jsonAnim[kLengthField].asInt();
  }
  LOG_INFO("AnimationComponent.Init.ManifestRead", "%zu animations loaded",
           _availableAnims.size());

  if (ANKI_DEVELOPER_CODE) {
    // now that we loaded the animations, go check the animation whitelist and
    // make sure everything in there is a valid animation prefix
    const auto& whitelistedPrefixes =
        _robot->GetContext()
            ->GetDataLoader()
            ->GetAllWhitelistedChargerAnimationPrefixes();
    for (const auto& prefix : whitelistedPrefixes) {
      ...
      if (!hasMatchingAnim) {
        LOG_WARNING("AnimationComponent.AnimWhitelistInvalid",
                    "Anim whitelist in RobotDataLoader contains prefix '%s' "
                    "for which there is no valid clip name",
                    prefix.c_str());
      }
    }
  }
...
Result AnimationComponent::GetAnimationMetaInfo(
    const std::string& animName, AnimationMetaInfo& metaInfo) const {
...
// Doles animations (the max number that can be doled per tic) to game if
// requested
void AnimationComponent::DoleAvailableAnimations() {
  if (_isDolingAnims) {
    u32 numAnimsDoledThisTic = 0;

    auto it = _nextAnimToDole.empty() ? _availableAnims.begin()
                                      : _availableAnims.find(_nextAnimToDole);
    for (; it != _availableAnims.end() &&
           numAnimsDoledThisTic < kMaxNumAvailableAnimsToReportPerTic;
         ++it) {
    ...
    if (it == _availableAnims.end()) {
      ...
      _nextAnimToDole = "";
      _robot->Broadcast(ExternalInterface::MessageEngineToGame(
          ExternalInterface::EndOfMessage(
              ExternalInterface::MessageType::AnimationAvailable)));
    } else {
...
const std::string& AnimationComponent::GetAnimationNameFromGroup(
    const std::string& name, bool strictCooldown, int recursionCount) const {
  static const std::string empty("");

  if (!ANKI_VERIFY(recursionCount < kMaxAnimGroupRecursionDepth,
                   "AnimationComponent.GetAnimationNameFromGroup."
                   "ExceededMaxREcursionDepth",
                   "Recursion loop! Recursed %d times to group name '%s'",
                   recursionCount, name.c_str())) {
    // just give up after max recursion depth
    return empty;
  }

  const AnimationGroup* group =
      _animationGroups->_container.GetAnimationGroup(name);
  if (group != nullptr && !group->IsEmpty()) {
    const std::string& selectedAnim = group->GetAnimationName(
        _robot->GetMoodManager(), _animationGroups->_container,
    ...
    if (_animationGroups->_container.HasGroup(selectedAnim)) {
      LOG_INFO("GetAnimationName.SubGroupSelected",
               "Group %s returned sub-group %s, going deeper", name.c_str(),
               selectedAnim.c_str());
      return GetAnimationNameFromGroup(selectedAnim, strictCooldown,
                                       recursionCount + 1);
    } else {
      return selectedAnim;
    }
...
Result AnimationComponent::PlayAnimByName(const std::string& animName,
                                          int numLoops, bool interruptRunning,
                                          AnimationCompleteCallback callback,
                                          const u32 actionTag,
                                          float timeout_sec, u32 startAt_ms,
                                          bool renderInEyeHue) {
  if (!_isInitialized) {
  ...
  auto it = _availableAnims.find(animName);
  if (it == _availableAnims.end()) {
    LOG_WARNING("AnimationComponent.PlayAnimByName.AnimNotFound", "%s",
                animName.c_str());
    return RESULT_FAIL;
  }

  LOG_DEBUG("AnimationComponent.PlayAnimByName.PlayingAnim", "%s",
            it->first.c_str());
  ...
  //       Or should it queue on anim process side and optionally interrupt
  //       currently executing anim?
  ...
  if (_robot->SendRobotMessage<RobotInterface::PlayAnim>(
          numLoops, startAt_ms, currTag, renderInEyeHue, animName) ==
      RESULT_OK) {
    SetAnimationCallback(animName, callback, currTag, actionTag, numLoops,
                         timeout_sec);
  }
...
Result AnimationComponent::PlayAnimWithSpriteBoxRemaps(
    const std::string& animName, const RemapMap& remaps, bool interruptRunning,
    AnimationCompleteCallback callback,
  ...
  auto it = _availableAnims.find(animName);
  if (it == _availableAnims.end()) {
    LOG_WARNING("AnimationComponent.PlayAnimWithSpriteBoxRemaps.AnimNotFound",
                "%s", animName.c_str());
    return RESULT_FAIL;
  }
  ...
  if (IsPlayingAnimation() && !interruptRunning) {
    LOG_INFO(
        "AnimationComponent.PlayAnimWithSpriteBoxRemaps."
        "WontInterruptCurrentAnim",
        "");
    return RESULT_FAIL;
  }

  const Tag currTag = GetNextTag();
  const auto& spritePathMap =
      *_robot->GetContext()->GetDataLoader()->GetSpritePaths();
  ...
  if (remaps.size() > msg.spriteBoxRemaps.size()) {
    LOG_ERROR("AnimationComponent.PlayAnimWithSpriteBoxRemaps.MessageOverflow",
              ...
              remaps.size(), msg.spriteBoxRemaps.size());
    return RESULT_FAIL;
  }
  ...
  if (!lockFaceAtEndOfAnimTag.empty()) {
    auto lockTrackCallback = [this, lockFaceAtEndOfAnimTag](
                                 const AnimationComponent::AnimResult res,
                                 u32 streamTimeAnimEnded) {
      if (res == AnimResult::Completed) {
        _movementComponent->RecordTracksLocked((u8)AnimTrackFlag::FACE_TRACK,
                                               lockFaceAtEndOfAnimTag);
      }
    };
    SetAnimationCallback(animName, lockTrackCallback, currTag, 0, 0, 0);
  }

  return _robot->SendRobotMessage<RobotInterface::PlayAnimWithSpriteBoxRemaps>(
      msg);
}

Result AnimationComponent::PlayAnimWithSpriteBoxKeyFrames(
    const std::string& animName,
    const std::vector<Vision::SpriteBoxKeyFrame>& keyframes,
    bool interruptRunning, AnimationCompleteCallback callback) {
  if (!_isInitialized) {
    LOG_WARNING("AnimationComponent.PlayAnimWithSpriteBoxRemaps.Uninitialized",
                "");
    return RESULT_FAIL;
  ...
  // Empty animName will play a constructed animation containing ONLY the
  // provided SpriteBoxKeyFrames
  if (!animName.empty()) {
    // Check that animName is valid
    auto it = _availableAnims.find(animName);
    if (it == _availableAnims.end()) {
  ...
  if (keyframes.size() > msg.spriteBoxKeyFrames.size()) {
    LOG_ERROR(
        "AnimationComponent.PlayAnimWithSpriteBoxKeyFrames.MessageOverflow",
        "Attempted to send %zu KeyFrames, message can only carry %zu",
        keyframes.size(), msg.spriteBoxKeyFrames.size());
    return RESULT_FAIL;
  }
  ...
  std::memcpy(&msg.spriteBoxKeyFrames, keyframes.data(),
              keyframes.size() * sizeof(keyframes[0]));
  msg.animName = animName;
  ...
  return _robot
      ->SendRobotMessage<RobotInterface::PlayAnimWithSpriteBoxKeyFrames>(msg);
}

Result AnimationComponent::AddSpriteBoxKeyFramesToRunningAnim(
    const std::vector<Vision::SpriteBoxKeyFrame>& keyframes) {
  if (!_isInitialized) {
    LOG_WARNING("AnimationComponent.PlayAnimWithSpriteBoxRemaps.Uninitialized",
                "");
  ...
  if (keyframes.size() > msg.spriteBoxKeyFrames.size()) {
    LOG_ERROR(
        "AnimationComponent.AddSpriteBoxKeyFramesToRunningAnim.MessageOverflow",
        "Attempted to send %zu KeyFrames, message can only carry %zu",
        keyframes.size(), msg.spriteBoxKeyFrames.size());
    return RESULT_FAIL;
  }
...
AnimationComponent::Tag AnimationComponent::IsAnimPlaying(
    const std::string& animName) {
...
Result AnimationComponent::StopAnimByName(const std::string& animName) {
  ...
  if (it != _availableAnims.end()) {
    ...
    if (tag != kNotAnimatingTag) {
      LOG_DEBUG("AnimationComponent.StopAnimByName.AbortingAnim", "%s (%d)",
                animName.c_str(), tag);
      return _robot->SendRobotMessage<RobotInterface::AbortAnimation>(tag);
    } else {
      LOG_WARNING("AnimationComponent.StopAnimByName.AnimNotPlaying", "%s",
                  animName.c_str());
      return RESULT_OK;
    }
  } else {
    LOG_WARNING("AnimationComponent.StopAnimByName.InvalidName", "%s",
                animName.c_str());
    return RESULT_FAIL;
  }
}

void AnimationComponent::AlterStreamingAnimationAtTime(
    RobotInterface::EngineToRobot&& msg, TimeStamp_t relativeStreamTime_ms,
    bool applyBeforeTick, MovementComponent* devSafetyCheck) {
  ...
  switch (msg.GetTag()) {
    case RobotInterface::EngineToRobotTag::setFullAnimTrackLockState: {
      DEV_ASSERT(devSafetyCheck == _movementComponent,
                 "AnimationComponent.AlterStreamingAnimationAtTime."
                 "DirectlyQueueingMessageThatMustGoThroughMovementComponent");
      alterMsg.setFullAnimTrackLockState =
          std::move(msg.Get_setFullAnimTrackLockState());
      break;
    }
    ...
    default: {
      LOG_ERROR(
          ...
          "Message Type %s is not currently implemented - update clad and anim "
          "process to support",
...
Result AnimationComponent::DisplayFaceImageBinary(const Vision::Image& img,
                                                  u32 duration_ms,
  ...
  if (IsPlayingAnimation() && !interruptRunning) {
    LOG_INFO(
        "AnimationComponent.DisplayFaceImageBinary.WontInterruptCurrentAnim",
        "");
    return RESULT_FAIL;
  }
  ...
  if (!ANKI_VERIFY(imageIsValidSize,
                   ...
                   img.GetNumCols(), img.GetNumRows(), img.IsContinuous(),
  ...
  for (int halfIdx = 0; halfIdx < 2; ++halfIdx) {
    ...
    static_assert(
        8 * kFaceArraySize == kNumHalfImagePixels,
        "AnimationComponent.DisplayFaceImageBinary.WrongFaceDataSize");
...
Result AnimationComponent::DisplayFaceImage(const Vision::Image& img,
                                            u32 duration_ms,
                                            bool interruptRunning) {
  return DisplayFaceImageHelper<RobotInterface::DisplayFaceImageGrayscaleChunk>(
      img, duration_ms, interruptRunning);
}

Result AnimationComponent::DisplayFaceImage(
    const Vision::ImageRGB565& imgRGB565, u32 duration_ms,
    bool interruptRunning) {
  return DisplayFaceImageHelper<RobotInterface::DisplayFaceImageRGBChunk>(
      imgRGB565, duration_ms, interruptRunning);
}

Result AnimationComponent::DisplayFaceImage(const Vision::ImageRGB& img,
                                            u32 duration_ms,
                                            bool interruptRunning) {
  static Vision::ImageRGB565
      img565;  // static to avoid repeatedly allocating this once it's used
...
Result AnimationComponent::DisplayFaceImageHelper(const ImageType& img,
                                                  u32 duration_ms,
                                                  bool interruptRunning) {
  // Make sure types logically match (e.g. message type should not be grayscale
  // if image type is RGB)
  static_assert(
      (std::is_same<MessageType,
                    RobotInterface::DisplayFaceImageGrayscaleChunk>::value &&
       std::is_same<ImageType, Vision::Image>::value) ||
          (std::is_same<MessageType,
                        RobotInterface::DisplayFaceImageRGBChunk>::value &&
  ...
  ASSERT_NAMED(img.IsContinuous(),
               "AnimationComponent.DisplayFaceImage.NotContinuous");
  ASSERT_NAMED(img.GetNumRows() == FACE_DISPLAY_HEIGHT,
               "AnimationComponent.DisplayFaceImage.IncorrectImageHeight");
  ASSERT_NAMED(img.GetNumCols() == FACE_DISPLAY_WIDTH,
               "AnimationComponent.DisplayFaceImage.IncorrectImageWidth");
  ...
  // std::remove_pointer<decltype(startIt)>::type, typename
  // decltype(msg.faceData)::value_type>::value, "wrong type");
  ...
  DEV_ASSERT_MSG(
      chunkCount == kExpectedNumChunks,
      "AnimationComponent.DisplayFaceImageHelper.UnexpectedNumChunks", "%d",
      chunkCount);

  return RESULT_OK;
}

void AnimationComponent::SetAnimationCallback(
    const std::string& animName, AnimationCompleteCallback callback,
    const u32 currTag, const u32 actionTag, int numLoops, float timeout_sec,
  ...
  _callbackMap.emplace(
      std::piecewise_construct, std::forward_as_tuple(currTag),
      std::forward_as_tuple(animName, callback, actionTag, abortTime_sec,
                            callbackStillValidEvenIfTagIsNot));
}

void AnimationComponent::AddKeepFaceAliveDisableLock(
    const std::string& lockName) {
  _numKeepFaceAliveDisableLocks++;

  LOG_INFO("AnimationComponent.KeepFaceAlive.DisableLock.Locked",
           "Adding disable lock for '%s', count is %d locks", lockName.c_str(),
           _numKeepFaceAliveDisableLocks);
...
void AnimationComponent::RemoveKeepFaceAliveDisableLock(
    const std::string& lockName) {
  if (ANKI_VERIFY(_numKeepFaceAliveDisableLocks > 0,
    ...
    LOG_INFO("AnimationComponent.KeepFaceAlive.DisableLock.Removed",
             ...
             lockName.c_str(), _numKeepFaceAliveDisableLocks);
...
Result AnimationComponent::SendEnableKeepFaceAlive(bool enable,
  ...
  const Result res =
      _robot->SendRobotMessage<RobotInterface::EnableKeepFaceAlive>(
          disableTimeout_ms, enable);
...
Result AnimationComponent::AddOrUpdateEyeShift(
    const std::string& name, f32 xPix, f32 yPix, TimeStamp_t duration_ms,
    f32 xMax, f32 yMax, f32 lookUpMaxScale, f32 lookDownMinScale,
    f32 outerEyeScaleIncrease) {
  Result res = _robot->SendRobotMessage<RobotInterface::AddOrUpdateEyeShift>(
      xPix, yPix, duration_ms, xMax, yMax, lookUpMaxScale, lookDownMinScale,
      outerEyeScaleIncrease, name);
  if (res == RESULT_OK) {
...
Result AnimationComponent::RemoveEyeShift(const std::string& name,
                                          u32 disableTimeout_ms) {
  if (IsEyeShifting(name)) {
    Result res = _robot->SendRobotMessage<RobotInterface::RemoveEyeShift>(
        disableTimeout_ms, name);
    if (res == RESULT_OK) {
...
Result AnimationComponent::AddSquint(const std::string& name, f32 squintScaleX,
                                     f32 squintScaleY, f32 upperLidAngle) {
  Result res = _robot->SendRobotMessage<RobotInterface::AddSquint>(
      squintScaleX, squintScaleY, upperLidAngle, name);
  if (res == RESULT_OK) {
...
Result AnimationComponent::RemoveSquint(const std::string& name,
                                        u32 disableTimeout_ms) {
  if (IsEyeSquinting(name)) {
    Result res = _robot->SendRobotMessage<RobotInterface::RemoveSquint>(
        disableTimeout_ms, name);
    if (res == RESULT_OK) {
...
Result AnimationComponent::SetFaceSaturation(float level) {
  const Result res =
      _robot->SendRobotMessage<RobotInterface::SetFaceSaturation>(level);
  return res;
}

AnimationTag AnimationComponent::SetTriggerWordGetInCallback(
...
std::array<AnimationTag, 4> AnimationComponent::SetAlexaUXResponseCallback(
    std::function<void(unsigned int, bool)> callback) {
  _alexaResponseCallback = callback;
  const std::array<AnimationTag, 4> tags = {
      {_tagForAlexaListening, _tagForAlexaThinking, _tagForAlexaSpeaking,
       _tagForAlexaError}};
  return tags;
}
...
template <>
void AnimationComponent::HandleMessage(
    const ExternalInterface::RequestAvailableAnimations& msg) {
...
template <>
void AnimationComponent::HandleMessage(
    const ExternalInterface::RequestAvailableAnimationGroups& msg) {
  std::vector<std::string> animNames(_robot->GetContext()
                                         ->GetDataLoader()
                                         ->GetAnimationGroups()
                                         ->GetAnimationGroupNames());
  for (std::vector<std::string>::iterator i = animNames.begin();
       i != animNames.end(); ++i) {
    _robot->GetExternalInterface()
        ->BroadcastToGame<ExternalInterface::AnimationGroupAvailable>(*i);
  }
}

template <>
void AnimationComponent::HandleMessage(
    const ExternalInterface::DisplayProceduralFace& msg) {
  static_assert(std::tuple_size<decltype(msg.faceParams.leftEye)>::value ==
                    (size_t)ProceduralEyeParameter::NumParameters,
                "LeftEye parameter array is the wrong length");
  static_assert(std::tuple_size<decltype(msg.faceParams.rightEye)>::value ==
                    (size_t)ProceduralEyeParameter::NumParameters,
  ...
  if (IsPlayingAnimation() && !msg.interruptRunning) {
    LOG_INFO(
        "AnimationComponent.DisplayProceduralFace.WontInterruptCurrentAnim",
        "");
    return;
  }

  // Convert ExternalInterface version of DisplayProceduralFace to
  // RobotInterface version and send
  _robot->SendRobotMessage<RobotInterface::DisplayProceduralFace>(
      msg.faceParams, msg.duration_ms);
}

template <>
void AnimationComponent::HandleMessage(
    const ExternalInterface::SetFaceHue& msg) {
  _robot->SendRobotMessage<RobotInterface::SetFaceHue>(msg.hue);
}

template <>
void AnimationComponent::HandleMessage(
    const ExternalInterface::DisplayFaceImageBinaryChunk& msg) {
  if (!_isInitialized) {
    LOG_WARNING(
        "AnimationComponent.HandleDisplayFaceImageBinaryChunk.Uninitialized",
        "");
    return;
  }
  ...
  if (IsPlayingAnimation() && !msg.interruptRunning) {
    LOG_INFO(
        "AnimationComponent.HandleDisplayFaceImage.WontInterruptCurrentAnim",
        "");
    return;
  }

  // Convert ExternalInterface version of DisplayFaceImage to RobotInterface
  // version and send
  _robot->SendRobotMessage<RobotInterface::DisplayFaceImageBinaryChunk>(
      msg.duration_ms, msg.faceData, msg.imageId, msg.chunkIndex);
}

template <>
void AnimationComponent::HandleMessage(
    const ExternalInterface::DisplayFaceImageRGBChunk& msg) {
  if (!_isInitialized) {
  ...
  if ((_oledImageBuilder->GetRecievedChunkMask() ^ fullMask) == 0) {
    Vision::ImageRGB565 image(FACE_DISPLAY_HEIGHT, FACE_DISPLAY_WIDTH,
                              _oledImageBuilder->GetAllData());
...
template <>
void AnimationComponent::HandleMessage(
    const ExternalInterface::ReadAnimationFile& msg) {
  _robot->SendRobotMessage<RobotInterface::AddAnim>(msg.full_path);
}

// ================ Robot message handlers ======================

void AnimationComponent::HandleAnimAdded(
    const AnkiEvent<RobotInterface::RobotToEngine>& message) {
  const auto& payload = message.GetData().Get_animAdded();
  LOG_INFO("HandleAnimAdded", "name=%s length=%d", payload.animName.c_str(),
           payload.animLength);
  _availableAnims[payload.animName].length_ms = payload.animLength;
}

void AnimationComponent::HandleAnimStarted(
    const AnkiEvent<RobotInterface::RobotToEngine>& message) {
  const auto& payload = message.GetData().Get_animStarted();

  // the trigger word get-in is started in the anim process without our
  // knowledge, so if we see it being started, let's not complain about it since
  // it's expected note: we could have a "started" callback for this similar to
  // _triggerWordGetInCallbackFunction
  //       this way UserIntentComponent knows exactly when the trigger word anim
  //       starts instead of just assuming
  const bool isTriggerWordGetIn =
      (payload.tag == _tagForTriggerWordGetInCallbacks);
  ...
  if (it != _callbackMap.end() || isTriggerWordGetIn || isAlexa) {
    // LOG_INFO("AnimStarted.Tag", "name=%s, tag=%d", payload.animName.c_str(),
    // payload.tag);
  } else if (payload.animName != EnumToString(AnimConstants::PROCEDURAL_ANIM)) {
    LOG_WARNING("AnimationComponent.AnimStarted.UnexpectedTag",
                "name=%s, tag=%d", payload.animName.c_str(), payload.tag);
    return;
  }
  ...
  _robot->GetContext()->GetVizManager()->SendCurrentAnimation(_currAnimName,
                                                              _currAnimTag);
}

void AnimationComponent::HandleAnimEnded(
    const AnkiEvent<RobotInterface::RobotToEngine>& message) {
  ...
  while (it != _callbackMap.end()) {
    ...
    // LOG_INFO("AnimEnded.Tag", "name=%s, tag=%d", payload.animName.c_str(),
    // payload.tag);
    it->second.ExecuteCallback(
        payload.wasAborted ? AnimResult::Aborted : AnimResult::Completed,
  ...
  if (payload.tag == _tagForTriggerWordGetInCallbacks) {
    // this wont be in our _callbackMap, so for debug's sake let's print this
    // out LOG_INFO("AnimEnded.Tag", "name=%s, tag=%d",
    // payload.animName.c_str(), payload.tag);
    atLeastOneCallback = true;
  ...
  if (!atLeastOneCallback &&
    ...
    LOG_WARNING("AnimationComponent.AnimEnded.UnexpectedTag", "name=%s, tag=%d",
                payload.animName.c_str(), payload.tag);
    return;
  }
  ...
  DEV_ASSERT_MSG(_currAnimName.empty() || _currAnimName == payload.animName,
                 "AnimationComponent.AnimEnded.UnexpectedName",
                 "Got %s, expected %s", payload.animName.c_str(),
                 _currAnimName.c_str());
  DEV_ASSERT_MSG(
      _currAnimTag == kNotAnimatingTag || _currAnimTag == payload.tag,
      "AnimationComponent.AnimEnded.UnexpectedTag", "Got %d, expected %d",
      payload.tag, _currAnimTag);
...
void AnimationComponent::HandleAnimationEvent(
    const AnkiEvent<RobotInterface::RobotToEngine>& message) {
  ...
  if (it != _callbackMap.end()) {
    ...
    _robot->GetExternalInterface()
        ->BroadcastToGame<ExternalInterface::AnimationEvent>(std::move(msg));
  }
}

void AnimationComponent::HandleAnimState(
    const AnkiEvent<RobotInterface::RobotToEngine>& message) {
...
void AnimationComponent::AddAdditionalAnimationCallback(
    const std::string& name,
    AnimationComponent::AnimationCompleteCallback callback,
    bool callEvenIfAnimCanceled) {
  // If we should only call the callback while the anim is active we need to
  // look up the action tag, otherwise just leave it as zero
  u32 actionTag = 0;
  u32 currTag = 0;
  ...
  if (actionTag == 0) {
    LOG_ERROR(
        "AnimationComponent.AddAdditionalAnimationCallback.NoActionTagFound",
        "Attempting to add an additional callback to animation %s, but it was "
        "not found in the callback map",
        name.c_str());
    return;
  }
  ...
  SetAnimationCallback(name, callback, currTag, actionTag, numLoops,
                       timeout_sec, callEvenIfAnimCanceled);
}
...
bool AnimationComponent::TagIsAlexa(AnimationTag tag) const {
  const bool isAlexa =
      (tag == _tagForAlexaListening) || (tag == _tagForAlexaThinking) ||
      (tag == _tagForAlexaSpeaking) || (tag == _tagForAlexaError);
  return isAlexa;
}
