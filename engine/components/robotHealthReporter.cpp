...
// This path should point to a temporary filesystem that will be cleared by each
// reboot.
...
RobotHealthReporter::RobotHealthReporter()
    : IDependencyManagedComponent(this, RobotComponentID::RobotHealthReporter) {

}

void RobotHealthReporter::BootInfoCheck(const OSState* osState) {
  DEV_ASSERT(osState != nullptr,
             "RobotHealthReporter.BootInfoCheck.InvalidOSState");
  ...
  int i = 0;
  for (auto pos = kDiskInfoPaths.begin(); pos != kDiskInfoPaths.end();
       ++pos, ++i) {
    const auto& path = *pos;
    uint64_t avail_kB = 0;
...
void RobotHealthReporter::WifiInfoCheck(const OSState* osState) {
  ...
  if (_wifiInfo.alert != wifiInfo.alert) {
    DASMSG(robot_wifi_info, "robot.wifi_info",
           "Wifi status has crossed alert threshold");
...
void RobotHealthReporter::DiskInfoCheck(const OSState* osState) {
  DEV_ASSERT(osState != nullptr,
             "RobotHealthReporter.DiskInfoCheck.InvalidOSState");
  ...
  for (const auto& path : kDiskInfoPaths) {
    ...
    if (!osState->GetDiskInfo(path, info)) {
      LOG_ERROR("RobotHealthReporter.DiskInfoCheck.InvalidDiskInfo",
                "Unable to fetch disk info for %s", path.c_str());
      info.alert = OSState::Alert::Red;
    }

    // Has filesystem crossed an alert threshold?
    if (_diskInfo[path].alert != info.alert) {
      DASMSG(robot_disk_info, "robot.disk_info",
             "Disk status has crossed alert threshold");
...
void RobotHealthReporter::MemoryInfoCheck(const OSState* osState) {
  DEV_ASSERT(osState != nullptr,
             "RobotHealthReporter.MemoryInfoCheck.InvalidOSState");
  ...
  if (info.alert != _memoryAlert) {
    DASMSG(robot_memory_info, "robot.memory_info",
           "Memory pressure has crossed alert threshold");
    DASMSG_SET(s1, info.alert, "Alert level");
...
void RobotHealthReporter::CPUInfoCheck(const OSState* osState) {
  DEV_ASSERT(osState != nullptr,
             "RobotHealthReporter.CPUInfoCheck.InvalidOSState");
...
//
void RobotHealthReporter::InitDependent(
    Robot* robot, const DependencyManager& dependencyManager) {
  // Nothing to do here?
}
...
//
void RobotHealthReporter::UpdateDependent(
