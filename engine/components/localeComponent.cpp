...
LocaleComponent::LocaleComponent()
    : RobotComponent(this, RobotComponentID::LocaleComponent) {}
...
void LocaleComponent::InitDependent(Robot *, const RobotCompMap &dependencies) {
  ...
  DEV_ASSERT(context != nullptr,
             "LocaleComponent.InitDependent.InvalidContext");

  const auto *dataPlatform = context->GetDataPlatform();
  DEV_ASSERT(dataPlatform != nullptr,
             "LocaleComponent.InitDependent.InvalidDataPlatform");
  ...
  if (!SetLocale(localeString)) {
    LOG_WARNING("LocaleComponent.InitDependent.InvalidLocale",
                "Unable to set locale %s", localeString.c_str());
...
bool LocaleComponent::SetLocale(const std::string &locale) {
  ...
  if (locale == _locale) {
    LOG_DEBUG("LocaleComponent.SetLocale", "Already using locale %s",
              locale.c_str());
    return true;
  }
  ...
  if (!LoadDirectory(localePath)) {
    LOG_WARNING("LocaleComponent.SetLocale.InvalidLocale", "Unable to load %s",
                localePath.c_str());
    _locale.clear();
    return false;
  }
...
bool LocaleComponent::LoadFile(const std::string &path) {
  ...
  try {
    ...
    if (!reader.parse(document, root)) {
      ...
      LOG_WARNING("LocaleComponent.LoadFile", "Unable to parse %s (%s)",
                  path.c_str(), errors.c_str());
      return false;
    }

    if (!root.isObject()) {
      LOG_WARNING("LocaleComponent.LoadFile", "Invalid root object in %s",
                  path.c_str());
      return false;
    }
    ...
    for (const auto &memberName : memberNames) {
      ...
      if (!member.isObject()) {
        LOG_WARNING("LocaleComponent.LoadFile", "Invalid entry for key %s",
                    memberName.c_str());
        continue;
      }
      ...
      if (!translation.isString()) {
        LOG_WARNING("LocaleComponent.LoadFile",
                    "Invalid translation for key %s", memberName.c_str());
        continue;
      }
    ...
    LOG_ERROR("LocaleComponent.LoadFile", "Failed to load %s (%s)",
              path.c_str(), ex.what());
    return false;
  }
...
bool LocaleComponent::LoadDirectory(const std::string &path) {
  ...
  if (!Util::FileUtils::DirectoryExists(path)) {
    LOG_WARNING("LocaleComponent.LoadDirectory", "Invalid directory %s",
                path.c_str());
    return false;
  }
  ...
  for (const auto &file : files) {
    if (!LoadFile(file)) {
      LOG_WARNING("LocaleComponent.LoadDirectory", "Failed to load %s",
                  file.c_str());
      ok = false;
    }
  }
