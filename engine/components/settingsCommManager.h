...
// But if we registered reactors at 10Hz it would take about 13 years to exhaust
// a uint32.
...
class SettingsCommManager
 ...
 public:
  ...
  bool HandleRobotSettingChangeRequest(
      const external_interface::RobotSetting robotSetting,
      const Json::Value& settingJson, const bool updateSettingsJdoc = false);
  bool ToggleRobotSettingHelper(
      const external_interface::RobotSetting robotSetting);

  bool HandleAccountSettingChangeRequest(
      const external_interface::AccountSetting accountSetting,
      const Json::Value& settingJson, const bool updateSettingsJdoc = false);
  bool ToggleAccountSettingHelper(
      const external_interface::AccountSetting accountSetting);

  bool HandleUserEntitlementChangeRequest(
  ...
  bool ToggleUserEntitlementHelper(
      const external_interface::UserEntitlement userEntitlement);

  void RefreshConsoleVars();

  // for volume change notifications
  // we do notifications from settingsCommManager and not settingsManager
  // because we're interested in being notified of volume changes coming in from
  // the app and other sources that use that interface, not voice commands which
  // (through behaviorVolume) go directly to settingsManager
  SettingsReactorId RegisterVolumeChangeReactor(
      OnVolumeChangedCallback callback);
