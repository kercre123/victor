 ...
 * Date:   3/28/2016
 *
 * Description: Interface for storing and retrieving data from robot's
...
class NVStorageComponent : public IDependencyManagedComponent<RobotComponentID>,
 ...
 public:
  ...
  // TODO: For COZMO_V2 consider making Write(), Read(), and Erase() just return
  // NVResult
  ...
  using NVStorageWriteEraseCallback =
      std::function<void(NVStorage::NVResult res)>;
  bool Write(NVStorage::NVEntryTag tag, const u8* data, size_t size,
             NVStorageWriteEraseCallback callback = {});

  bool Write(NVStorage::NVEntryTag tag, const std::vector<u8>* data,
             NVStorageWriteEraseCallback callback = {});
  ...
  //
  // Returns true if request was successfully sent.
  using NVStorageReadCallback =
      std::function<void(u8* data, size_t size, NVStorage::NVResult res)>;
  bool Read(NVStorage::NVEntryTag tag, NVStorageReadCallback callback = {},
            std::vector<u8>* data = nullptr);

  // Returns the word-aligned size that nvStorage automaticaly rounds up to on
  // all saved content. e.g. If you write 10 bytes to tag 2 and then read back
  // tag 2, you will receive 12 bytes.
 ...
 private:
  ...
  using TagDataMap =
      std::unordered_map<NVStorage::NVEntryTag, std::vector<u8> >;
  ...
  bool Erase(NVStorage::NVEntryTag tag, NVStorageWriteEraseCallback callback,
             u32 eraseSize);

  // Whether or not we should be writing headers
  bool _writingFactory = false;

  // Only BehaviorFactoryTest should ever call this function since it is special
  // and is writing factory data
