...
struct PlanParameters {
  ...
  // these target poses are in terms of the "drive center" of the robot, which
  // may be different from the "pose" the robot is at when it gets there (e.g.
  // pose may be geometric center, instead of between front wheels)
  std::vector<Pose3d> targetPoses;

  // starting point for the plan
  Pose3d driveCenter;

  // the driver center pose and target poses all must share this origin for the
  // plan to be valid
  PoseOriginID_t commonOriginID = PoseOriginList::UnknownOriginID;
};

PlanParameters::PlanParameters(Vector::Robot* robot,
...
bool PlanParameters::IsEqual(const PlanParameters& otherPlan) const {
  if (otherPlan.commonOriginID != this->commonOriginID) {
    return false;
  }
  if (otherPlan.targetPoses.size() != this->targetPoses.size()) {
    return false;
  }
  if (!otherPlan.driveCenter.IsSameAs(this->driveCenter, .001, .001)) {
    return false;
  }

  for (int i = 0; i < this->targetPoses.size(); ++i) {
    if (!otherPlan.targetPoses[i].IsSameAs(this->targetPoses[i], .001, .001)) {
      return false;
    }
  }
...
PathComponent::PathComponent()
    : IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::PathPlanning),
...
PathComponent::~PathComponent() { Abort(); }

void PathComponent::InitDependent(Vector::Robot* robot,
  ...
  if (context) {
    ...
    if (nullptr != context->GetDataPlatform()) {
      ...
      // For unit tests, or cases where we don't have data, use the short
      // planner in its place
      LOG_WARNING("Robot.PathComponent.NoDataPlatform.WrongPlanner",
                  "Using short planner as the long planner, since we don't "
                  "have a data platform");
      _longPathPlanner = _shortPathPlanner;
    }
  } else {
    LOG_WARNING("Robot.PathComponent.NoContext",
                "No cozmo context, won't be fully functional");
  }

  auto eventLambda = [this](const AnkiEvent<RobotInterface::RobotToEngine>&
                                event) {
    RobotInterface::PathFollowingEvent payload =
        event.GetData().Get_pathFollowingEvent();

    LOG_DEBUG("PathComponent.ReceivedPathEvent",
              "ID:%d Event:%s Status:%s CancelID=%u SentID=%u RcvdID=%u",
              payload.pathID, EnumToString(payload.eventType),
              ERobotDriveToPoseStatusToString(_driveToPoseStatus),
              _lastCanceledPathID, _lastSentPathID, _lastRecvdPathID);

    // handle complete and interrupted paths in cases where we wait to cancel.
    // Returns true if this lambda handled the message, false otherwise
    auto handleCancelLambda = [this](const RobotInterface::PathFollowingEvent&
                                         payload) {
      if (IsWaitingToCancelPath()) {
        if (payload.pathID == _lastCanceledPathID) {
          // path has canceled, update state appropriately
          if (_driveToPoseStatus ==
              ERobotDriveToPoseStatus::WaitingToCancelPath) {
            SetDriveToPoseStatus(ERobotDriveToPoseStatus::Ready);
          } else {
            ANKI_VERIFY(
                _driveToPoseStatus ==
                    ERobotDriveToPoseStatus::WaitingToCancelPathAndSetFailure,
          ...
          // This is possible in cases of high latency. For example, engine
          // could send path 1, send a cancel for path 1 and a new path, path 2,
          // then abort path 2. After this, we might receive a message about
          // path 1 while waiting to cancel path 2
          LOG_WARNING(
              "PathComponent.PathEvent.CanceledPathDifferentPathComplete",
              "We are in status '%s' waiting for path %d to cancel, but got "
              "message that path %d is %s",
              ERobotDriveToPoseStatusToString(_driveToPoseStatus),
              _lastCanceledPathID, payload.pathID,
              PathEventTypeToString(payload.eventType));
        }
        // we handled the message here, so tell the caller not to continue to
        // process it
        return true;
      }

      if (_lastCanceledPathID > 0 && payload.pathID == _lastCanceledPathID) {
        // we got a complete or interrupted message, but we've already canceled
        // that path (and moved on to a new one, which is why our status isn't
        // Waiting and this case isn't handled above). In this case, ignore this
        // message (mark it as handled)
        return true;
      }
    ...
    switch (payload.eventType) {
      case PathEventType::PATH_STARTED: {
        if (IsWaitingToCancelPath() || payload.pathID == _lastCanceledPathID) {
          // If we are waiting to cancel _any_ path, ignore paths starting.
          // Also, separately check _lastCanceledPathID because we may have
          // moved on to a status for a new path while still expecting to
          // receive a cancel from an old path

          LOG_INFO(
              "PathComponent.PathEvent.OldPathStartedWhileWaitingForCancel",
              "The robot started path %d which is about to be canceled. Status "
              "is %s",
              payload.pathID,
              ERobotDriveToPoseStatusToString(_driveToPoseStatus));
        ...
        // In general, we should be coming from "Waiting to Begin" when we
        // receive Path Started. We could be "Following" already if path/segment
        // ID in RobotState already transitioned us, or waiting to cancel (which
        // is handled above, so not listed here)
        if (!ANKI_VERIFY(
        ...
        ANKI_VERIFY(payload.pathID == _lastSentPathID,
                    ...
                    "Last sent ID:%d, starting ID:%d", _lastSentPathID,
                    payload.pathID);
      ...
      case PathEventType::PATH_COMPLETED: {
        ...
        ANKI_VERIFY(payload.pathID == _lastSentPathID,
                    ...
                    "Last sent ID:%d, completing ID:%d", _lastSentPathID,
                    payload.pathID);

        // Note that OnPathComplete is safe to call if the path is already
        // complete (e.g. thanks to RobotState message updates)
        OnPathComplete();
      ...
      case PathEventType::PATH_INTERRUPTED: {
        ...
        ANKI_VERIFY(payload.pathID == _lastRecvdPathID,
                    ...
                    "Last received ID:%d, completing ID:%d", _lastRecvdPathID,
                    payload.pathID);

        // Only mark complete if this interruption is for the last path we
        // actually sent and not an earlier one, which we don't care about
        // anymore
  ...
  _pathEventHandle = _robot->GetRobotMessageHandler()->Subscribe(
      RobotInterface::RobotToEngineTag::pathFollowingEvent, eventLambda);
}
...
void PathComponent::OnPathComplete() {
  ...
  if ((_selectedPathPlanner != nullptr) &&
      (_selectedPathPlanner->CheckPlanningStatus() !=
       EPlannerStatus::Running)) {
    // we reached the end of the path and the planner isn't running, so there's
    // no chance the robot will make it to the replanning origin if it hasn't
    // already. Set this as false to allow failure if the robot isn't in place
    _waitingToMatchReplanOrigin = false;
  }
}

void PathComponent::UpdateCurrentPathSegment(s8 currPathSegment) {
  const bool robotReceivedLastPath =
  ...
  if (robotReceivedLastPath) {
    ...
    if (_driveToPoseStatus == ERobotDriveToPoseStatus::FollowingPath &&
        currPathSegment < 0 && _plannerActive) {
      LOG_INFO("PathComponent.UpdateRobotData.ComputingNewPlan",
               "Actively replanning and finished following path ID %d",
               _lastRecvdPathID);

      // we are actively replanning, but ran out of our old path. Now we are
      // waiting to compute a new plan
...
void PathComponent::UpdateDependent(const RobotCompMap& dependentComps) {
  if (false && _selectedPathPlanner != nullptr) {
    LOG_WARNING(
        "PathComponentStatus", "plannerStatus=%s, driveStatus=%s, stopped=%d",
  ...
  // Try restarting the planner if the robot is following a path to deal with
  // new obstacles or origin shifts. Also, to handle an edge case where the
  // robot stopped short of its expected start pose during replanning, try once
  // to restart the planner, which will decide if it's actually necessary or
  // possible to do so.
  const bool tooFarFromReplanStart =
      _waitingToMatchReplanOrigin && _hasStoppedBeforeExecuting &&
      (_selectedPathPlanner != nullptr) &&
      (_selectedPathPlanner->GetErrorType() ==
       EPlannerErrorType::TooFarFromPlan) &&
      (_driveToPoseStatus == ERobotDriveToPoseStatus::ComputingPath);
  if ((_driveToPoseStatus == ERobotDriveToPoseStatus::FollowingPath) ||
      tooFarFromReplanStart) {
    if (tooFarFromReplanStart) {
      _waitingToMatchReplanOrigin = false;
    }
    HandlePossibleOriginChanges();
  } else if (_waitingToMatchReplanOrigin) {
    // Maybe having driven a bit since the last UpdateDependent() means the
    // robot is closer to the start of its replanned path. Let it attempt to
    // start driving it again
    TryCompletingPath();
  }

  if (IsWaitingForRobotResponse()) {
    // we are waiting on the robot to for something, make sure it hasn't timed
    // out
    ...
    if (currTime_s > _lastMsgSendTime_s + kSendMsgFailedTimeout_s) {
      LOG_ERROR(
          "PathComponent.SentUnreceivedPath",
          "robot did not start executing path. Last send = %d, last recv = %d",
          _lastSentPathID, _lastRecvdPathID);
      AbortAndSetFailure();
    }
  }
}
...
void PathComponent::HandlePossibleOriginChanges() {
  ...
  const bool tooFar __attribute((unused)) =
      (_selectedPathPlanner->GetErrorType() ==
       EPlannerErrorType::TooFarFromPlan);
  DEV_ASSERT(
  ...
  // TODO: COZMO-1637: Once we figure this out, switch these verifies back to
  // dev_asserts for efficiency
  ANKI_VERIFY(
      _currPlanParams->commonOriginID != PoseOriginList::UnknownOriginID,
      "PathComponent.HandlePossibleOriginChanges.NullParamsOrigin", "");

  ANKI_VERIFY(_robot->GetPoseOriginList().ContainsOriginID(
                  _currPlanParams->commonOriginID),
              "PathComponent.Update.CommonOriginNotInRobotPoseOriginList",
              "ID:%d", _currPlanParams->commonOriginID);

  // haveOriginsChanged: Check if the robot's origin has changed since planning.
  // If no delocs, relocs, or rejiggers have happened, our stored commonOrigin
  // will be the robots world origin. Otherwise, we check for origin
  // rejiggering. If our stored origin was rejiggered, then it is no longer an
  // origin itself. If we can get it with respect to the new origin, we can
  // rejigger targetPoses and continue. One example where this happens would be
  // that the robot localizes to object 1 (origin A), get picked up and put down
  // (creating origin B), starts driving somewhere, and while driving sees
  // object 1, which causes a rejigger to origin A
  const bool haveOriginsChanged =
      (_currPlanParams->commonOriginID !=
       _robot->GetPoseOriginList().GetCurrentOriginID());

  const Pose3d& commonOrigin = _robot->GetPoseOriginList().GetOriginByID(
      _currPlanParams->commonOriginID);
  const bool canAdjustOrigin = _robot->IsPoseInWorldOrigin(commonOrigin);

  if (haveOriginsChanged && !canAdjustOrigin) {
    // the origins changed and we can't rejigger our goal to the new origin (we
    // probably delocalized), completely abort
    AbortAndSetFailure();
    LOG_INFO("PathComponent.Update.Replan.NotPossible",
             "Our goal is in another coordinate frame that we can't get wrt "
             "current, we can't replan");
  } else if (haveOriginsChanged && canAdjustOrigin) {
    RejiggerTargetsAndReplan();
  } else {
    // we didn't need to adjust origins, so delegate on Restart to decide if we
    // should replan
...
void PathComponent::RejiggerTargetsAndReplan() {
  ...
  LOG_INFO("PathComponent.Update.Replan.RejiggeringPlanner",
           "Our goal is in another coordinate frame, but we are updating to "
           "current frame");

  // our origin changed, but we can rejigger. Do so now, and re-plan to take any
  // new information from the rejigger into account
  bool rejiggerSuccess = true;
  for (auto& targetPose : _currPlanParams->targetPoses) {
    rejiggerSuccess &=
        targetPose.GetWithRespectTo(_robot->GetWorldOrigin(), targetPose);
  }
  ...
  if (ANKI_VERIFY(rejiggerSuccess, "PathComponent.Update.Rejigger",
    ...
    if (started) {
      ...
      SetDriveToPoseStatus(ERobotDriveToPoseStatus::ComputingPath);
      LOG_INFO(
          "PathComponent.Update.Rejigger.Fallback",
          "Planning to newly rejiggered poses failed, but fallback succeeded.");
    } else {
      LOG_WARNING("PathComponent.Update.Replan.RejiggeringPlanner",
  ...
  switch (_selectedPathPlanner->CheckPlanningStatus()) {
    case EPlannerStatus::Error: {
      if (ReplanWithFallbackPlanner()) {
        SetDriveToPoseStatus(ERobotDriveToPoseStatus::ComputingPath);
        LOG_INFO("PathComponent.Update.Planner.Error",
                 "Running planner returned error status, using fallback "
                 "planner instead");
      } else {
        // abort in case we are currently following an invalid path
        AbortAndSetFailure();
        LOG_INFO("PathComponent.Update.Planner.Error",
                 "Running planner returned error status");
      }
      break;
    }
    ...
    case EPlannerStatus::Running: {
      DEV_ASSERT(
          _driveToPoseStatus != ERobotDriveToPoseStatus::Failed &&
    ...
    case EPlannerStatus::CompleteNoPlan: {
      LOG_INFO("PathComponent.Update.Planner.CompleteNoPlan",
      ...
      if (_driveToPoseStatus == ERobotDriveToPoseStatus::FollowingPath ||
          ...
          _driveToPoseStatus ==
              ERobotDriveToPoseStatus::WaitingToCancelPathAndSetFailure) {
        // we must have replanned and discovered that we are at the goal, so
        // stop following the path
        LOG_INFO("PathComponent.Update.Planner.NoPlanWhileTraversing",
                 "Planner completed with empty plan while we were already "
                 "following a plan. clearing plan");
        Abort();
        // don't set failure here, since we have an empty plan, we have
...
void PathComponent::TryCompletingPath() {
  ...
  // NOTE: If object-carrying state changes between the time this function is
  // called
  //       and the time the path is executed, the robot will either drive too
  //       fast and potentially fall off cliffs or too slow and potentially look
  //       strange.
  PathMotionProfile cliffSafeMotionProfile =
      ClampToCliffSafeSpeed(*_pathMotionProfile);
  ...
  if (_selectedPathPlanner->HasCompletePath()) {
    ...
    // Overwrite the cached path in the planner instance with this postprocessed
    // path
    finalPath =
        IPathPlanner::ApplyMotionProfile(rawPath, cliffSafeMotionProfile);
  ...
  // the planner finished but returned no path... either the robot is at the
  // goal, some internal error occurred, or, if the robot was replanning, it
  // probably isn't yet close enough to that position to start following the
  // path. In the latter case, set a flag so that we can retry picking up this
  ...
  // collisions are always OK for empty paths, or if the selected planner
  // actually checked for collisions
  const bool collisionsAcceptable =
      _selectedPathPlanner->ChecksForCollisions() ||
      finalPath.GetNumSegments() == 0;

  // Some children of IPathPlanner may return a path that hasn't been checked
  // for obstacles. Here, check if the planner used to compute that path
  // considers obstacles. If it doesn't, check for an obstacle penalty. If there
  // is one, re-run with the lattice planner, which always considers obstacles
  // in its search.
  if ((!collisionsAcceptable) && (nullptr != _longPathPlanner)) {
    const float startPoseAngle_rad =
        driveCenterPose.GetRotationAngle<'Z'>().ToFloat();
    if (!_longPathPlanner->CheckIsPathSafe(finalPath, startPoseAngle_rad)) {
      // bad path. try with the fallback planner if possible

      if (ReplanWithFallbackPlanner()) {
        LOG_INFO("PathComponent.Update.Planner.Collisions",
                 "Planner returned a path with obstacles, using fallback "
                 "planner instead");
        return;
      } else {
        AbortAndSetFailure();
        LOG_INFO("PathComponent.Update.Planner.Collisions.ReplanFail",
                 "Planner returned a path with obstacles, can't get valid "
                 "fallback plan, failing");
        return;
      }
    }
  }
  ...
  if (finalPath.GetNumSegments() == 0) {
    if (_driveToPoseStatus == ERobotDriveToPoseStatus::FollowingPath ||
        _driveToPoseStatus == ERobotDriveToPoseStatus::WaitingToBeginPath) {
      // we must have replanned and discovered that we are at the goal, so stop
      // following the path
      LOG_INFO("PathComponent.Update.Planner.PlanComplete.EmptyPlan",
               "Planner completed with empty plan while we were already "
               "following a plan. clearing plan");
      ClearPath();
    } else {
      LOG_INFO("PathComponent.Update.Planner.CompleteWithPlan.EmptyPlan",
    ...
    if (res != RESULT_OK) {
      LOG_WARNING(
          "Robot.PathComponent.UnableToExecuteCompletedPath",
    ...
    if (_plannerSelectedPoseIndex) {
      // When someone called StartDrivingToPose with multiple possible poses,
      // they had an option to pass in a pointer to be set when we know which
      // pose was selected by the planner. If they did, set it now
...
void PathComponent::SelectPlannerHelper(const Pose3d& targetPose) {
  ...
  float distSquared = pow(target2d.GetX() - start2d.GetX(), 2) +
                      pow(target2d.GetY() - start2d.GetY(), 2);

  if (distSquared < std::pow(kMaxDistanceForShortPlanner_mm, 2)) {
    Radians finalAngleDelta =
        targetPose.GetRotationAngle<'Z'>() -
        _robot->GetDriveCenterPose().GetRotationAngle<'Z'>();
    const bool withinFinalAngleTolerance =
        finalAngleDelta.getAbsoluteVal().ToFloat() <=
        2 * PLANNER_MAINTAIN_ANGLE_THRESHOLD;

    Radians initialTurnAngle =
        atan2(
            target2d.GetY() - _robot->GetDriveCenterPose().GetTranslation().y(),
            target2d.GetX() -
                _robot->GetDriveCenterPose().GetTranslation().x()) -
        _robot->GetDriveCenterPose().GetRotationAngle<'Z'>();

    const bool initialTurnAngleLarge =
        initialTurnAngle.getAbsoluteVal().ToFloat() >
        0.5 * PLANNER_MAINTAIN_ANGLE_THRESHOLD;

    const bool farEnoughAwayForMinAngle =
        distSquared > std::pow(kMinDistanceForMinAnglePlanner_mm, 2);

    // if we would need to turn fairly far, but our current angle is fairly
    // close to the goal, use the planner which backs up first to minimize the
    // turn
    if (withinFinalAngleTolerance && initialTurnAngleLarge &&
        farEnoughAwayForMinAngle) {
      LOG_INFO("PathComponent.SelectPlanner.ShortMinAngle",
               "distance^2 is %f, angleDelta is %f, initialTurnAngle is %f, "
               "selecting short min_angle planner '%s'",
               distSquared, finalAngleDelta.getAbsoluteVal().ToFloat(),
               initialTurnAngle.getAbsoluteVal().ToFloat(),
               _shortMinAnglePathPlanner->GetName().c_str());
      _selectedPathPlanner = _shortMinAnglePathPlanner;
    } else {
      LOG_INFO("PathComponent.SelectPlanner.Short",
               "distance^2 is %f, angleDelta is %f, initialTurnAngle is %f, "
               "selecting short planner '%s'",
               distSquared, finalAngleDelta.getAbsoluteVal().ToFloat(),
               initialTurnAngle.getAbsoluteVal().ToFloat(),
    ...
    LOG_INFO("PathComponent.SelectPlanner.Long",
             "distance^2 is %f, selecting long planner '%s'", distSquared,
             _longPathPlanner->GetName().c_str());
    _selectedPathPlanner = _longPathPlanner;
  }

  Util::sInfo(
      "robot.planner_selected",
      {{DDATA, std::to_string(Util::numeric_cast<int>(std::round(distSquared)))
                   .c_str()}},
      _selectedPathPlanner->GetName().c_str());
...
void PathComponent::SelectPlanner() {
  if (!_currPlanParams->targetPoses.empty()) {
    // for now, just grab the closest pose and use that to select the planner.
    // Note that this pose might be different from the target pose the selected
    // planner actually plans to (because planning distance is not the same as
    // euclidean distance)
    Planning::GoalID closest = IPathPlanner::ComputeClosestGoalPose(
        _robot->GetDriveCenterPose(), _currPlanParams->targetPoses);
    SelectPlannerHelper(_currPlanParams->targetPoses[closest]);
  } else {
    LOG_WARNING("PathComponent.SelectPlanner.NoTargets",
                "can't select a planner with no target poses");
    _selectedPathPlanner.reset();
  }
}

void PathComponent::SetCustomMotionProfile(
    const PathMotionProfile& motionProfile) {
  if (HasCustomMotionProfile()) {
    LOG_WARNING("PathComponent.SetMotionProfile.Conflict",
                "Trying to set custom motion profile, but one is already set! "
                "Overriding");
  }

  // warning for is custom
  if (!motionProfile.isCustom) {
    LOG_WARNING("PathComponent.SetCustomMotionProfile.NotCustom",
                "Motion profile passed in didn't have it's isCustom flag set. "
...
void PathComponent::SetCustomMotionProfileForAction(
    const PathMotionProfile& motionProfile, IActionRunner* action) {
  SetCustomMotionProfile(motionProfile);

  DEV_ASSERT(action != nullptr,
             "PathComponent.SetCustomMotionProfileForAction.NullAction");
...
PathMotionProfile PathComponent::GetCustomMotionProfile() const {
  ANKI_VERIFY(
      _hasCustomMotionProfile,
      "PathComponent.GetCustomMotionProfile.NoCustomProfile",
      "Trying to get the custom profile, but none is set. This is a bug");
  return ClampToCliffSafeSpeed(*_pathMotionProfile);
}

Result PathComponent::ConfigureAndStartPlanner(
    const std::vector<Pose3d>& poses,
    std::shared_ptr<Planning::GoalID> selectedPoseIndexPtr) {
  if (poses.empty()) {
    LOG_WARNING("PathComponent.ConfigureAndStartPlanner.NoTargetPoses",
  ...
  if (_plannerActive) {
    if (!_currPlanParams->IsEqual(newPlanParams)) {
      ...
      // already started the planner, and will now start following the path on
      // update cycle after plan completes
      LOG_INFO(
          "PathComponent.ConfigureAndStartPlanner."
          "AlreadyStartedWithMatchingParams",
          "");
      return RESULT_OK;
    }
  }
  ...
  if (IsActive()) {
    // we may be following the same path, so only check the goals and originID
    // for equality
    newPlanParams.driveCenter = _currPlanParams->driveCenter;
    if (!_currPlanParams->IsEqual(newPlanParams)) {
      // stop doing what we are doing, so we can make a new plan
      LOG_INFO("PathComponent.ConfigureAndStartPlanner.AlreadyBusy",
      ...
      if (!_plannerActive && IsPlanReady() && _startFollowingPath) {
        ...
        LOG_INFO(
            "PathComponent.ConfigureAndStartPlanner."
            "NotReadyToExecuteFollowingPath",
            "%d %d %d", _plannerActive, IsPlanReady(), _startFollowingPath);
      }
      return RESULT_OK;
    }
  }
  ...
  if (!somePlannerStarted) {
    ...
    // otherwise we are computing. Note that some planners finish immediately,
    // in which case the status will be updated on the next Update tick
    SetDriveToPoseStatus(ERobotDriveToPoseStatus::ComputingPath);
  }
...
Result PathComponent::PrecomputePath(
    const std::vector<Pose3d>& poses,
    std::shared_ptr<Planning::GoalID> selectedPoseIndexPtr) {
...
Result PathComponent::StartDrivingToPose(
    const std::vector<Pose3d>& poses,
    std::shared_ptr<Planning::GoalID> selectedPoseIndexPtr) {
...
bool PathComponent::StartPlanner(const Pose3d& driveCenterPose) {
  if (!_selectedPathPlanner) {
    LOG_ERROR("PathComponent.StartPlanner.NoSelectedPlanner",
              "Must select planner before starting");
    return false;
  }

  EComputePathStatus status = _selectedPathPlanner->ComputePath(
      driveCenterPose, _currPlanParams->targetPoses);
  if (status == EComputePathStatus::Error) {
    auto fallbackSuccess = ReplanWithFallbackPlanner();
    LOG_INFO("PathComponent.StartPlanner.PlanningError.UseFallbackPlanner",
             "%d", fallbackSuccess);
...
bool PathComponent::ReplanWithFallbackPlanner() {
  ...
  const std::string& oldPlannerName =
      _selectedPathPlanner ? _selectedPathPlanner->GetName() : "NULL";
...
void PathComponent::RestartPlannerIfNeeded() {
  ...
  switch (_selectedPathPlanner->ComputeNewPathIfNeeded(
      _robot->GetDriveCenterPose(), forceReplan, _replanningCanChangeGoal)) {
    case EComputePathStatus::Error: {
      if (ReplanWithFallbackPlanner()) {
        LOG_INFO(
            "PathComponent.RestartIfNeeded.Error.Fallback",
            "computing a new path resulted in an error, switching to fallback");
      } else {
        LOG_INFO("PathComponent.RestartIfNeeded.Error.NoFallback",
                 "computing a new path resulted in an error but couldn't use "
                 "fallback. Failing");
        // abort in case we are currently following an invalid path
        AbortAndSetFailure();
      }
    ...
    case EComputePathStatus::Running: {
      LOG_DEBUG("PathComponent.Replan.Running",
                "ComputeNewPathIfNeeded running");
      ...
      if (currPath.GetNumSegments() > 0) {
        ...
        if (!_selectedPathPlanner->CheckIsPathSafe(currPath, startAngle,
                                                   validSubPath)) {
          _isReplanning = true;
          // entire path is not safe, set the current path to just the valid
          // portion
          if (_currPathSegment >= validSubPath.GetNumSegments()) {
            // the robot already drove the extent of the valid path, so force
            // stop
            LOG_INFO(
                "PathComponent.RestartPlannerIfNeeded."
                "PathUnsafeMidExecutionIssuingEstop",
                "segIdx=%d numValidSeg=%u", (int)_currPathSegment,
                validSubPath.GetNumSegments());
            ClearPath();
            _hasStoppedBeforeExecuting = true;
          } else {
            if (_pdo->GetLastDoledIdx() >= validSubPath.GetNumSegments()) {
              // we already sent the extent of the valid path, so trim it before
              // it's executed
              TrimRobotPathToLength(validSubPath.GetNumSegments());
            }
            LOG_INFO(
                "PathComponent.RestartPlannerIfNeeded.PathUnsafe."
                "TrimmingToSafeSubpath",
                "segIdx=%d numValidSeg=%u", (int)_currPathSegment,
                validSubPath.GetNumSegments());
            _pdo->ReplacePath(validSubPath);
            // redraw the path
            _robot->GetContext()->GetVizManager()->DrawPath(
                _robot->GetID(), _pdo->GetPath(), NamedColors::EXECUTED_PATH);
          }
        }
      }
      _plannerActive = true;
    ...
    case EComputePathStatus::NoPlanNeeded: {
      DEV_ASSERT(_driveToPoseStatus == ERobotDriveToPoseStatus::FollowingPath,
                 "Robot.PathComponent.RestarTPlannerIfNeeded.NoPlan."
                 "InconsistentState");
...
Result PathComponent::ClearPath() {
  ...
  _robot->GetContext()->GetVizManager()->ErasePath(_robot->GetID());
  if (_pdo) {
    LOG_DEBUG("PathComponent.ClearPath.ClearingPDO",
              "sent=%u rcvd=%u cancel=%u", _lastSentPathID, _lastRecvdPathID,
              _lastCanceledPathID);
    _pdo->ClearPath();
  }
  ...
  return _robot->SendMessage(
      RobotInterface::EngineToRobot(RobotInterface::ClearPath(0)));
}
...
Result PathComponent::TrimRobotPathToLength(uint8_t length) {
  ...
  if (_pdo != nullptr) {
    ...
    if (numSegments > length) {
      ...
      _lastMsgSendTime_s =
          BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
      result = _robot->SendMessage(RobotInterface::EngineToRobot(
          RobotInterface::TrimPath(0, numPopBack)));
    }
  }
...
bool PathComponent::IsPlanReady() const {
  bool ready = false;
  if (_selectedPathPlanner != nullptr) {
    ready = (_selectedPathPlanner->CheckPlanningStatus() ==
             EPlannerStatus::CompleteWithPlan);
  }
  return ready;
}
...
bool PathComponent::IsPathSafe(const Planning::Path& path,
                               const Pose3d* driveCenter) const {
  if (driveCenter == nullptr) {
    driveCenter = &_robot->GetDriveCenterPose();
  }

  // currently just check the long path planner, since it is the only one that
  // supports collision checking at the moment
  if (_longPathPlanner) {
    return _longPathPlanner->CheckIsPathSafe(
        path, driveCenter->GetRotationAngle<'Z'>().ToFloat());
  } else {
    LOG_WARNING(
        "PathComponent.IsPathSafe",
        "No planner exists that checks collisions, so all paths are safe");
    return true;
  }
}
...
Result PathComponent::ExecutePath(const Planning::Path& path) {
  ...
  if (path.GetNumSegments() == 0) {
    LOG_WARNING("PathComponent.ExecutePath.EmptyPath", "");
    ...
    if (lastResult == RESULT_OK) {
      ++_lastSentPathID;
      if (_pdo) {
        LOG_DEBUG("PathComponent.ExecutePath.SetPathPDO",
                  "sent=%u rcvd=%u cancel=%u", _lastSentPathID,
                  _lastRecvdPathID, _lastCanceledPathID);
        _pdo->SetPath(path);
      }

      LOG_INFO("PathComponent.SendExecutePath",
    ...
    _robot->GetContext()->GetVizManager()->DrawPath(_robot->GetID(), path,
                                                    NamedColors::EXECUTED_PATH);
  }
...
void PathComponent::ExecuteTestPath(const PathMotionProfile& motionProfile) {
  // NOTE: no need to use the custom motion profile here, we just manually pass
  // it in to the test path
  Planning::Path p;
...
void PathComponent::SetDriveToPoseStatus(ERobotDriveToPoseStatus newValue) {
  if (newValue != _driveToPoseStatus) {
    LOG_INFO("PathComponent.TransitionStatus", "%s -> %s",
             ERobotDriveToPoseStatusToString(_driveToPoseStatus),
...
void PathComponent::SetStartPath(bool autoStart) {
  const bool needsRestarting =
      (autoStart && !_startFollowingPath && IsPlanReady() && HasPathToFollow());
...
PathMotionProfile PathComponent::ClampToCliffSafeSpeed(
    const PathMotionProfile& motionProfile) const {
  PathMotionProfile cliffSafeMotionProfile = motionProfile;
  const bool isCarrying = _robot->GetCarryingComponent().IsCarryingObject();
  const float maxSpeed = isCarrying ? MAX_SAFE_WHILE_CARRYING_WHEEL_SPEED_MMPS
  ...
  auto ClampSpeedFunc = [maxSpeed, isCarrying](float orig_speed) {
    if (Util::InRange(orig_speed, -maxSpeed, maxSpeed)) {
      ...
      LOG_WARNING("PathComponent.ClampToCliffSafeSpeed.ClampingUnsafeSpeed",
                  "%f mm/s will be clamped to %f mm/s", orig_speed, maxSpeed);
    }
    return CLIP(orig_speed, -maxSpeed, maxSpeed);
  };

  cliffSafeMotionProfile.speed_mmps = ClampSpeedFunc(motionProfile.speed_mmps);
  cliffSafeMotionProfile.reverseSpeed_mmps =
      ClampSpeedFunc(motionProfile.reverseSpeed_mmps);
  cliffSafeMotionProfile.dockSpeed_mmps =
      ClampSpeedFunc(motionProfile.dockSpeed_mmps);
