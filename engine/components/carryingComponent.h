...
class CarryingComponent : public IDependencyManagedComponent<RobotComponentID>,
 ...
 public:
  ...
  // "attached" to the robot. Set deleteLocatedObject=true to delete the object
  // instead of leaving it at the pose
  Result SetCarriedObjectAsUnattached(bool deleteLocatedObjects = false);

  // TODO Define better API for (un)setting carried objects: they are confused
  // easily with SetCarriedObjectAsUnattached
  ...
  const Vision::Marker::Code GetCarryingMarkerCode() const {
    return _carryingMarkerCode;
  }
 ...
 private:
  ...
  Result SetObjectAsAttachedToLift(
