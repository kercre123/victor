 ...
 * Created: 7/11/18
 *
 * Description: Class which handles cube connection requests from the app/sdk
 *layer and subscribes to the CubeConnectionCoordinator as necessary.
 *
...
AppCubeConnectionSubscriber::AppCubeConnectionSubscriber()
    : IDependencyManagedComponent(
          this, RobotComponentID::AppCubeConnectionSubscriber) {
  _appToEngineTags = {{AppToEngineTag::kConnectCubeRequest,
...
void AppCubeConnectionSubscriber::GetInitDependencies(
    RobotCompIDSet& dependencies) const {
  dependencies.insert(RobotComponentID::CozmoContextWrapper);
}

void AppCubeConnectionSubscriber::InitDependent(
    Robot* robot, const RobotCompMap& dependentComps) {
  ...
  for (const auto& tag : _appToEngineTags) {
    _eventHandles.push_back(_gi->Subscribe(
        tag, std::bind(&AppCubeConnectionSubscriber::HandleAppRequest, this,
                       std::placeholders::_1)));
  }
}
...
void AppCubeConnectionSubscriber::ConnectedCallback(
    CubeConnectionType connectionType) {
  switch (connectionType) {
    case CubeConnectionType::Interactable: {
      LOG_INFO(
          "AppCubeConnectionSubscriber.ConnectedCallback."
          "ConnectionAttemptSuccess",
      ...
      const auto& activeId =
          _robot->GetCubeCommsComponent().GetConnectedCubeActiveId();
      const auto* object =
    ...
    case CubeConnectionType::Background: {
      LOG_INFO(
          "AppCubeConnectionSubscriber.ConnectedCallback.ConnectedBackground",
          "Cube was already connected in background. Waiting for transition to "
          "Interactable connection.");
      break;
    }
    default: {
      PRINT_NAMED_WARNING(
          "AppCubeConnectionSubscriber.ConnectedCallback.InvalidConnectionType",
...
void AppCubeConnectionSubscriber::ConnectionFailedCallback() {
  LOG_INFO(
      "AppCubeConnectionSubscriber.ConnectionFailedCallback."
...
void AppCubeConnectionSubscriber::HandleAppRequest(
  ...
  switch (event.GetData().GetTag()) {
    case GatewayWrapperTag::kConnectCubeRequest: {
      LOG_INFO("AppCubeConnectionSubscriber.HandleAppRequest.SubscribeRequest",
               "Received a request for cube connection from gateway. "
               "Subscribing to interactable connection "
               "for %f seconds",
    ...
    case GatewayWrapperTag::kDisconnectCubeRequest: {
      LOG_INFO(
          "AppCubeConnectionSubscriber.HandleAppRequest.UnsubscribeRequest",
          "App is done with the cube connection. Requesting to unsubscribe");
      if (!ccc.UnsubscribeFromCubeConnection(this)) {
        PRINT_NAMED_WARNING(
            "AppCubeConnectionSubscriber.HandleAppRequest.UnsubscribeFailed",
            "Failed to unsubscribe from our cube connection. Did we have one "
            "in the first place?");
      }
      break;
    }
    case GatewayWrapperTag::kFlashCubeLightsRequest: {
      LOG_INFO(
          "AppCubeConnectionSubscriber.HandleAppRequest.FlashCubeLightsRequest",
      ...
      if (ccc.IsConnectedToCube()) {
        ...
        PRINT_NAMED_WARNING(
            "AppCubeConnectionSubscriber.HandleAppRequest."
            "FlashCubeLightsRequest.NotConnected",
    ...
    case GatewayWrapperTag::kSetPreferredCubeRequest: {
      ...
      const auto& factoryId =
          event.GetData().set_preferred_cube_request().factory_id();
      _robot->GetCubeCommsComponent().SetPreferredCube(factoryId);
      break;
    }
    default:
      DEV_ASSERT(false,
                 "AppCubeConnectionSubscriber.HandleAppRequest.UnhandledTag");
      break;
  }
}
