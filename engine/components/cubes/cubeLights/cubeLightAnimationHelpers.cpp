...
bool ParseCubeAnimationFromJson(const std::string& animName,
                                ...
                                CubeLightAnimation::Animation& outAnimation) {
  if (!ANKI_VERIFY(jsonRoot.isArray(),
                   "CubeLightAnimation.ParseCubeAnimationFromJson."
                   "BaseElementIsNotAnArray",
                   "Cube animations are expected to be specified as an array "
                   "of light patterns")) {
    return false;
  }
  ...
  for (s32 patternNum = 0; patternNum < numPatterns; ++patternNum) {
    ...
    if (!lightPattern.isMember(kPatternKey) ||
      ...
      PRINT_NAMED_ERROR(
          "CubeLightAnimationContainer.ParseJsonToAnim.InvalidJson",
          "Missing member field in %s json file", animName.c_str());
      return false;
    }
    ...
    if (!res) {
      PRINT_NAMED_ERROR(
          "CubeLightAnimationContainer.ParseJsonToAnim.InvalidPattern",
          "Missing member field in one of the pattern definitions in %s json "
          "file",
          animName.c_str());
      return false;
    }
...
bool ParseJsonToPattern(const Json::Value& json, LightPattern& pattern) {
  ...
  bool res = true;
  res &= JsonTools::GetColorValuesToArrayOptional(json, kOnColorsKey,
                                                  values.onColors);
  res &= JsonTools::GetColorValuesToArrayOptional(json, kOffColorsKey,
                                                  values.offColors);
  ...
  res &= JsonTools::GetArrayOptional(json, kTransitionOnPeriodKey,
                                     values.transitionOnPeriod_ms);
  res &= JsonTools::GetArrayOptional(json, kTransitionOffPeriodKey,
                                     values.transitionOffPeriod_ms);
  res &= JsonTools::GetArrayOptional(json, kOffsetKey, values.offset);
  res &= json.isMember(kRotateKey);

  // If any of the member fields are missing from the pattern definition return
  // false
  if (!res) {
    return false;
  }
...
Json::Value AnimationToJSON(const std::string& animName,
  ...
  for (const auto& pattern : animation) {
    ...
    // actual pattern
    {
      ...
      patternJSON[kTransitionOnPeriodKey] =
          LEDArrayToJson(pattern.lights.transitionOnPeriod_ms);
      patternJSON[kTransitionOffPeriodKey] =
          LEDArrayToJson(pattern.lights.transitionOffPeriod_ms);
...
void LightPattern::Print() const {
  for (int i = 0; i < static_cast<int>(CubeConstants::NUM_CUBE_LEDS); i++) {
    PRINT_CH_DEBUG(
        "CubeLightComponent", "LightPattern.Print",
        "%s LED %u, onColor %u, offColor %u, onFrames %u, "
        "offFrames %u, transOnFrames %u, transOffFrames %u, offset %u",
        name.c_str(), i, lights.onColors[i], lights.offColors[i],
        lights.onPeriod_ms[i], lights.offPeriod_ms[i],
        lights.transitionOnPeriod_ms[i], lights.transitionOffPeriod_ms[i],
        lights.offset[i]);
  }
}

void RotateLightPatternCounterClockwise(LightPattern& pattern,
                                        uint8_t positionsToRotate) {
  if (positionsToRotate > kNumCubeLEDs) {
    PRINT_NAMED_ERROR(
        "CubeLightAnimationHelpers.RotateLightPatternClockwise."
        "InvalidRotationNumber",
        "Can only rotate LEDs %d positions, received request to rotate %d "
        "positions",
        kNumCubeLEDs, positionsToRotate);
    return;
  }
#define ROTATE_ARRAY(property)                                            \
  std::rotate(pattern.lights.property.begin(),                            \
              pattern.lights.property.begin() +                           \
                  (CubeLightAnimation::kNumCubeLEDs - positionsToRotate), \
...
void OverwriteLEDs(const LightPattern& source, LightPattern& toOverwrite,
  ...
  for (const auto& led : ledsToOverwrite) {
    ...
    toOverwrite.lights.transitionOnPeriod_ms[led] =
        source.lights.transitionOnPeriod_ms[led];
    toOverwrite.lights.transitionOffPeriod_ms[led] =
        source.lights.transitionOffPeriod_ms[led];
...
const LightPattern& GetLightsOffPattern() {
  static const LightPattern kLightsOffPattern = {
      .name = "LightsOffLights", .lights = GetLightsOffObjectLights()};
