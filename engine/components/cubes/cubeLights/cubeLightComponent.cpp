 ...
 *              A cube light animation consists of multiple patterns, where each
 *pattern has a duration and when the pattern has been playing for that duration
 *the next pattern gets played. An animation can be played on one of three
 *layers (User/Game, Engine, Default), which function as a priority system with
 *the User/Game layer having highest priority. Each layer can have multiple
 *animations "playing" in a stack so once the top level animation finishes the
 *next anim will resume playing assuming its total duration has not been
 *reached. This is to support blending animations together (playing an animation
 *on top of another)
 *
...
#define MS_TO_LED_FRAMES(ms)                                            \
  ((ms) == std::numeric_limits<u32>::max()                              \
       ? std::numeric_limits<u8>::max()                                 \
       : Util::numeric_cast<u8>(((ms) + CUBE_LED_FRAME_LENGTH_MS - 1) / \
                                CUBE_LED_FRAME_LENGTH_MS))

bool CubeLightAnimation::ObjectLights::operator==(
    const CubeLightAnimation::ObjectLights& other) const {
  return this->onColors == other.onColors &&
         ...
         this->offset == other.offset && this->rotate == other.rotate &&
         this->makeRelative == other.makeRelative &&
         this->relativePoint == other.relativePoint;
}

#if ANKI_DEV_CHEATS
void CubeLightComponent::SaveLightsToDisk(
    const std::string& fileName, const CubeLightAnimation::Animation& anim) {
  auto json = CubeLightAnimation::AnimationToJSON(fileName, anim);
  const Util::Data::DataPlatform* platform =
      _robot->GetContext()->GetDataPlatform();
  auto relFolderPath = "DevLights/";
  auto fullFolderPath =
      platform->pathToResource(Util::Data::Scope::Cache, relFolderPath);
  ...
  platform->writeAsJson(fullFolderPath + fileName, json);
}
#endif
...
void CubeLightComponent::InitDependent(Vector::Robot* robot,
                                       const RobotCompMap& dependentComps) {
  _robot = robot;
  _cubeLightAnimations = std::make_unique<CubeLightAnimationContainer>(
      _robot->GetContext()->GetDataLoader()->GetCubeLightAnimations());
  _triggerToAnimMap =
      robot->GetContext()->GetDataLoader()->GetCubeAnimationTriggerMap();
  // Subscribe to messages
  if (_robot->HasExternalInterface()) {
    auto helper = MakeAnkiEventUtil(*_robot->GetExternalInterface(), *this,
                                    _eventHandles);
    using namespace ExternalInterface;
    helper
        .SubscribeEngineToGame<MessageEngineToGameTag::ObjectConnectionState>();
    ...
    helper.SubscribeGameToEngine<
        MessageGameToEngineTag::SetAllActiveObjectLEDs>();
  ...
  if (robot->HasGatewayInterface()) {
    ...
    for (const auto& tag : _appToEngineTags) {
      _eventHandles.push_back(
          _gi->Subscribe(tag, std::bind(&CubeLightComponent::HandleAppRequest,
...
void CubeLightComponent::UpdateInternal(bool shouldPickNextAnim) {
  const EngineTimeStamp_t curTime =
  ...
  for (auto& objectInfo : _objectInfo) {
    ...
    // If the current animation never ends and it isn't being stopped don't do
    // anything
    if (objectAnim.timeCurPatternEnds == 0 && !objectAnim.stopNow) {
      continue;
    }
    // Otherwise if the current pattern in this animation should end or the
    // whole animation is being stopped
    else if (curTime >= objectAnim.timeCurPatternEnds || objectAnim.stopNow) {
      // The current pattern is ending so try to go to the next pattern in the
      // animation
      ++objectAnim.curPattern;

      // If there are no more patterns in this animation or the animation is
      // being stopped
      if (objectAnim.curPattern == objectAnim.endOfPattern ||
          objectAnim.stopNow) {
        auto removeAndCallback = [&objectAnim, &objectInfo, &layer]() {
          ...
          // Note: objectAnim is now invalid. It can NOT be reassigned or
          // accessed
          ...
          if (callback) {
            ...
            // If the action gets destroyed and the tag is no longer in use the
            // callback would be invalid
            if (actionTag == 0 || IActionRunner::IsTagInUse(actionTag)) {
              callback();
            }
          }
        };
        ...
        // Keep removing animations and calling callbacks while there are
        // animations to stop on this layer
        ...
        if (objectInfo.second.animationsOnLayer[layer].empty()) {
          ...
          if (!objectInfo.second.isOnlyGameLayerEnabled) {
            ...
            PRINT_CH_INFO("CubeLightComponent",
                          "CubeLightComponent.Update.NoAnimsLeftOnLayer",
                          "No more animations left on layer %s for object %u "
                          "going to %s layer",
                          LayerToString(prevLayer), objectInfo.first.GetValue(),
                          LayerToString(layer));

            // In some cases we don't want to PickNextAnimForDefaultLayer in
            // order to prevent lights from flickering when stopping and playing
            // anims
          ...
          // Otherwise only game layer is enable so just turn the lights off
            ...
            Result res = SetCubeLightAnimation(
                objectInfo.first,
                CubeLightAnimation::GetLightsOffObjectLights());
            if (res != RESULT_OK) {
        ...
        // There are still animations being played on this layer so switch to
        // the next anim Note: A new variable objectAnim2 is created instead of
        // reassigning objectAnim This is because objectAnim has become invalid
        // by the call to pop_back() which means trying to assign to it is not
        // valid
        //
        ...
        // x = l.back(); // this looks like it should make x reference "a"
        // however x is invalid from the pop_back and cannot be reassigned
        const auto& prevObjectAnim =
            objectInfo.second.animationsOnLayer[layer].back();
        PRINT_CH_INFO("CubeLightComponent",
                      "CubeLightComponent.Update.PlayingPrevious",
                      "Have previous anim %s on layer %s id %u",
                      prevObjectAnim.debugName.c_str(), LayerToString(layer),
                      objectInfo.first.GetValue());

        Result res = SetCubeLightAnimation(objectInfo.first,
                                           prevObjectAnim.curPattern->lights);
        if (res != RESULT_OK) {
          PRINT_NAMED_WARNING("CubeLightComponent.Update.SetLightsFailed", "");
        }
        continue;
      }
      // Otherwise, we are going to the next pattern in the animation so update
      // the time the pattern will end
      else {
...
bool CubeLightComponent::PlayLightAnimFromAction(
    const ObjectID& objectID, const CubeAnimationTrigger& animTrigger,
    AnimCompletedCallback callback, const u32& actionTag) {
  const AnimLayerEnum layer = AnimLayerEnum::User;
  const bool res =
      PlayLightAnimByTrigger(objectID, animTrigger, layer, callback);
  if (res) {
    // iter is guaranteed to be valid otherwise PlayLightAnim will have returned
    // false animationOnLayer[layer] is guaranteed to not be empty because
    // PlayLightAnim will return true when an animation is added to the layer
    auto iter = _objectInfo.find(objectID);

    DEV_ASSERT(iter != _objectInfo.end(),
               "CubeLightComponent.PlayLightAnimFromAction.IterNotValid");

    // Update the action tag the callback is from, the animation was just added
    // to the end of animationsOnLayer[layer] by the call to PlayLightAnim
    iter->second.animationsOnLayer[layer].back().actionTagCallbackIsFrom =
        actionTag;
  }
  return res;
}

CubeLightAnimation::Animation* CubeLightComponent::GetAnimation(
...
bool CubeLightComponent::PlayLightAnimByTrigger(
    const ObjectID& objectID, const CubeAnimationTrigger& animTrigger,
    AnimCompletedCallback callback, bool playDuringBackgroundConnection,
    bool hasModifier, const CubeLightAnimation::ObjectLights& modifier,
    const s32 durationModifier_ms) {
  if (_cubeConnectedInBackground && !playDuringBackgroundConnection) {
    return false;
  }
  return PlayLightAnimByTrigger(objectID, animTrigger, AnimLayerEnum::Engine,
                                callback, hasModifier, modifier,
                                durationModifier_ms);
}

bool CubeLightComponent::PlayLightAnimByTrigger(
    const ObjectID& objectID, const CubeAnimationTrigger& animTrigger,
    const AnimLayerEnum& layer, AnimCompletedCallback callback,
    bool hasModifier, const CubeLightAnimation::ObjectLights& modifier,
    const s32 durationModifier_ms) {
  // Get the name of the animation to play for this trigger
  const std::string& animName =
      _robot->GetContext()->GetDataLoader()->GetCubeAnimationForTrigger(
          animTrigger);
  ...
  const bool success =
      PlayLightAnimInternal(objectID, *anim, layer, callback, hasModifier,
                            modifier, durationModifier_ms, animName, handle);
  if (success) {
...
bool CubeLightComponent::PlayLightAnim(const ObjectID& objectID,
  ...
  return PlayLightAnimInternal(objectID, animation, AnimLayerEnum::Engine,
                               callback, false, {}, 0, debugName, outHandle);
}

bool CubeLightComponent::PlayLightAnimInternal(
    const ObjectID& objectID, CubeLightAnimation::Animation& animation,
    const AnimLayerEnum& layer, AnimCompletedCallback callback,
    bool hasModifier, const CubeLightAnimation::ObjectLights& modifier,
    const s32 durationModifier_ms, const std::string& debugName,
    AnimationHandle& outHandle) {
  auto iter = _objectInfo.find(objectID);
  if (iter == _objectInfo.end()) {
    PRINT_CH_INFO(
        "CubeLightComponent",
        "CubeLightComponent.PlayLightAnim.InvalidObjectID",
  ...
  const bool isCurAnimBlended =
      (isPlayingAnim && animationsOnLayer.back().isBlended);

  // If we are currently playing an animation and it can not be overridden do
  // nothing
  if (isPlayingAnim && !animationsOnLayer.back().canBeOverridden) {
    PRINT_CH_INFO("CubeLightComponent",
                  "CubeLightComponent.PlayLightAnim.CantBeOverridden",
                  "Current anim %s can not be overridden on object %u layer %s",
                  animationsOnLayer.back().debugName.c_str(),
                  objectID.GetValue(), LayerToString(layer));
    return false;
  }

  // If only the game layer is enabled and we are trying to play this animation
  // on a different layer
  if (objectInfo.isOnlyGameLayerEnabled && layer != AnimLayerEnum::User) {
    PRINT_CH_INFO("CubeLightComponent",
                  "CubeLightComponent.OnlyGameLayerEnabled",
                  "Only game layer is enabled, refusing to play anim %s on "
                  "object %u layer %s",
                  debugName.c_str(), objectID.GetValue(), LayerToString(layer));
    return false;
  }

  // If the game/user layer is not enabled and an animation is trying to be
  // played on the game/user layer don't play it
  if (!objectInfo.isOnlyGameLayerEnabled && layer == AnimLayerEnum::User) {
    PRINT_CH_INFO("CubeLightComponent",
                  "CubeLightComponent.PlayLightAnim.NotPlayingUserAnim",
  ...
  // Attempt to blend the animation with the current light pattern. If the
  // animation can not be blended with the current animation then blendedAnim
  // will be the same as either modifiedAnim or animIter->second
  CubeLightAnimation::Animation blendedAnim;
  const bool didBlend = BlendAnimWithCurLights(
      objectID, (hasModifier ? modifiedAnim : animation), blendedAnim);

  // If the current animation is already a blended animation and the animation
  // to play was able to be blended with the current animation then do nothing
  // TODO (Al): Maybe support this ability in the future. Animation duration of
  // blended animations gets complicated
  if (isCurAnimBlended && didBlend) {
    PRINT_CH_INFO(
        "CubeLightComponent",
        "CubeLightComponent.PlayLightAnim.AlreadyPlayingBlendedAnim",
  ...
  // Treat modified animations the same as blended animations (ie they live in
  // the blendedAnims array)
  if (!isCurAnimBlended && (didBlend || hasModifier)) {
    // Store the blendedAnim in the blendedAnims array so we can grab iterators
    // to it
    objectInfo.blendedAnims[layer] = blendedAnim;

    info.debugName =
        (isPlayingAnim ? animationsOnLayer.back().debugName + "+" : "") +
        debugName;
    ...
    // If this animation has no duration then clear the existing animations on
    // the layer
  ...
  // If the first pattern in the animation has a non zero duration then update
  // the time the pattern should end
  ...
  // Whether or not this animation canBeOverridden is defined by whether or not
  // the last pattern in the animation definition can be overridden
  ...
  if (layer <= objectInfo.curLayer) {
    PRINT_CH_INFO("CubeLightComponent",
                  "CubeLightComponent.PlayLightAnim.SettingLights",
                  "Playing animation %s on layer %s for object %u",
                  debugName.c_str(), LayerToString(layer), objectID.GetValue());

    // Update the current layer for this object
    objectInfo.curLayer = layer;

    Result res =
        SetCubeLightAnimation(objectID, newAnimOnLayer.curPattern->lights);
    if (res != RESULT_OK) {
      // Since no light pattern was set, setting the end time of the pattern to
      // zero will make it so that everytime it is updated nothing will happen
      // (previous pattern will probably be playing)
      newAnimOnLayer.timeCurPatternEnds = 0;
    }
  }
  // Otherwise the animation was still added to the layer but a higher priority
  // layer is currently playing an animation so we didn't actually set object
  // lights
  else {
    PRINT_CH_INFO(
        "CubeLightComponent", "CubeLightComponent.PlayLightAnim.LightsNotSet",
...
void CubeLightComponent::StopAllAnimsOnLayer(const AnimLayerEnum& layer,
                                             const ObjectID& objectID) {
  // If objectID is unknown (-1) then stop all animations for all objects on the
  // given layer
  if (objectID.IsUnknown()) {
    PRINT_CH_INFO("CubeLightComponent",
                  "CubeLightComponent.StopAllAnimsOnLayer.AllObjects",
    ...
    if (iter == _objectInfo.end()) {
      PRINT_CH_INFO(
          "CubeLightComponent",
          "CubeLightComponent.StopAllAnimsOnLayer.BadObject",
...
bool CubeLightComponent::StopLightAnimAndResumePrevious(
    const CubeAnimationTrigger& animTrigger, const ObjectID& objectID) {
  return StopLightAnimByTrigger(animTrigger, AnimLayerEnum::Engine, objectID);
}

bool CubeLightComponent::StopLightAnimAndResumePrevious(
    const AnimationHandle& handle, const ObjectID& objectID) {
  return StopLightAnimInternal(handle, AnimLayerEnum::Engine, objectID);
}

bool CubeLightComponent::StopLightAnimByTrigger(
    const CubeAnimationTrigger& animTrigger, const AnimLayerEnum& layer,
    const ObjectID& objectID, bool shouldPickNextAnim) {
  bool success = false;
...
bool CubeLightComponent::StopLightAnimInternal(
    const AnimationHandle& animationHandle, const AnimLayerEnum& layer,
    const ObjectID& objectID, bool shouldPickNextAnim) {
  auto helper = [this](const AnimationHandle& animationHandle,
                       const ObjectID& objectID, const AnimLayerEnum& layer,
                       bool& foundAnimWithTrigger, std::string& debugName) {
  ...
  if (objectID.IsUnknown()) {
    for (auto& iter : _objectInfo) {
      helper(animationHandle, iter.first, layer, foundAnimWithTrigger,
             debugName);
    }
  } else {
  ...
  if (foundAnimWithTrigger) {
    PRINT_CH_INFO("CubeLightComponent", "CubeLightComponent.StopLightAnim",
                  "Stopping %s on object %d on layer %s.", debugName.c_str(),
                  objectID.GetValue(), LayerToString(layer));
  }
  ...
  for (const auto& objectInfoPair : _objectInfo) {
    ...
    ss << "object=" << objectInfoPair.first.GetValue()
  ...
  PRINT_CH_DEBUG("CubeLightComponent", "CubeLightComponent.StopLignAnim.Result",
                 ...
                 debugName.c_str(), ss.str().c_str());

  return foundAnimWithTrigger;
}

void CubeLightComponent::ApplyAnimModifier(
  ...
  for (auto& pattern : modifiedAnim) {
    for (int i = 0; i < CubeLightAnimation::kNumCubeLEDs; ++i) {
      ...
      pattern.lights.transitionOnPeriod_ms[i] +=
          modifier.transitionOnPeriod_ms[i];
      pattern.lights.transitionOffPeriod_ms[i] +=
          modifier.transitionOffPeriod_ms[i];
...
bool CubeLightComponent::BlendAnimWithCurLights(
    const ObjectID& objectID, const CubeLightAnimation::Animation& anim,
    CubeLightAnimation::Animation& blendedAnim) {
  auto* activeObject = _robot->GetBlockWorld().GetConnectedBlockByID(objectID);
  if (activeObject == nullptr) {
    PRINT_CH_INFO("CubeLightComponent",
                  "CubeLightComponent.BlendAnimWithCurLights.NullActiveObject",
                  "No active object with id %u found in block world unable to "
                  "blend anims",
                  objectID.GetValue());
    return false;
  }
  ...
  // For every pattern in the anim replace the on/offColors that are completely
  // zero (completely zero meaning alpha is 0 as well as r,g,b are 0, normally
  // alpha should be 1) with whatever the corresponding color that is currently
...
bool CubeLightComponent::StopAndPlayLightAnim(
    const ObjectID& objectID, const CubeAnimationTrigger& animTriggerToStop,
    const CubeAnimationTrigger& animTriggerToPlay,
    AnimCompletedCallback callback, bool hasModifier,
    const CubeLightAnimation::ObjectLights& modifier) {
  DEV_ASSERT(
      !_objectInfo[objectID].animationsOnLayer[AnimLayerEnum::Engine].empty(),
      "CubeLightComponent.StopAndPlayLightAnim.NoAnimsCurrentlyOnEngineLayer");
  // Stop the anim and prevent the update call in StopLightAnim from picking a
  // next default anim This will prevent the lights from briefly flickering
  // between the calls to stop and play
  StopLightAnimByTrigger(animTriggerToStop, AnimLayerEnum::Engine, objectID,
                         false);
  bool playDuringBackgroundConnection = true;
  return PlayLightAnimByTrigger(objectID, animTriggerToPlay, callback,
                                playDuringBackgroundConnection, hasModifier,
                                modifier);
}

bool CubeLightComponent::StopAndPlayLightAnim(
    const ObjectID& objectID, AnimationHandle& handle,
    CubeLightAnimation::Animation& animToPlay, const std::string& debugName,
    AnimCompletedCallback callback, bool hasModifier,
    const CubeLightAnimation::ObjectLights& modifier) {
  DEV_ASSERT(
      !_objectInfo[objectID].animationsOnLayer[AnimLayerEnum::Engine].empty(),
  ...
  return PlayLightAnimInternal(objectID, animToPlay, AnimLayerEnum::Engine,
                               callback, hasModifier, modifier, 0, debugName,
                               handle);
}

// =============Connction & Status Light Controls==================

bool CubeLightComponent::PlayConnectionLights(const ObjectID& objectID,
                                              AnimCompletedCallback callback) {
  // Connect lights take precedence over other anims
  StopAllAnimsOnLayer(AnimLayerEnum::State);
  return PlayLightAnimByTrigger(objectID, CubeAnimationTrigger::WakeUp,
                                AnimLayerEnum::State, callback);
}

bool CubeLightComponent::PlayDisconnectionLights(
    const ObjectID& objectID, AnimCompletedCallback callback) {
  // Disconnect lights take precedence over other anims
  StopAllAnimsOnLayer(AnimLayerEnum::State);
  return PlayLightAnimByTrigger(objectID, CubeAnimationTrigger::ShutDown,
                                AnimLayerEnum::State, callback);
}

bool CubeLightComponent::CancelDisconnectionLights(const ObjectID& objectID) {
  return StopLightAnimByTrigger(CubeAnimationTrigger::ShutDown,
                                AnimLayerEnum::State, objectID);
}

bool CubeLightComponent::PlayTapResponseLights(const ObjectID& objectID,
                                               AnimCompletedCallback callback) {
  // Cube tap response lights should take precedence over other state anims
  StopAllAnimsOnLayer(AnimLayerEnum::State);
  return PlayLightAnimByTrigger(objectID,
                                CubeAnimationTrigger::TapResponsePulse,
                                AnimLayerEnum::State, callback);
}

bool CubeLightComponent::SetCubeBackgroundState(
    const bool connectedInBackground) {
...
void CubeLightComponent::PickNextAnimForDefaultLayer(const ObjectID& objectID) {
  ...
  // If status anims are disabled for "background" cube connections, play a
  // blank anim
  if (_cubeConnectedInBackground) {
    // TODO:(str) remove the "cube sleep" infrastructure?
    PlayLightAnimByTrigger(objectID, CubeAnimationTrigger::SleepNoFade,
                           AnimLayerEnum::State);
    return;
  }
...
void CubeLightComponent::EnableGameLayerOnly(const ObjectID& objectID,
                                             bool enable) {
  PRINT_CH_INFO(
      "CubeLightComponent", "CubeLightComponent.EnableGameLayerOnly",
      "%s game layer only for %s", (enable ? "Enabling" : "Disabling"),
      (objectID.IsUnknown()
           ? "all objects"
  ...
  if (objectID.IsUnknown()) {
    if (_onlyGameLayerEnabledForAll != enable) {
      if (enable) {
        SetCubeLightAnimation(objectID,
                              CubeLightAnimation::GetLightsOffObjectLights());
        for (auto& pair : _objectInfo) {
...
const char* CubeLightComponent::LayerToString(
    const AnimLayerEnum& layer) const {
  switch (layer) {
...
template <>
void CubeLightComponent::HandleMessage(
    const ExternalInterface::ObjectConnectionState& msg) {
...
template <>
void CubeLightComponent::HandleMessage(
    const ExternalInterface::PlayCubeAnim& msg) {
...
template <>
void CubeLightComponent::HandleMessage(
    const ExternalInterface::StopCubeAnim& msg) {
  ObjectID objectID = msg.objectID;
  if (objectID < 0) {
    objectID = _robot->GetBlockWorld().GetSelectedObject();
  }
...
template <>
void CubeLightComponent::HandleMessage(
    const ExternalInterface::SetAllActiveObjectLEDs& msg) {
...
template <>
void CubeLightComponent::HandleMessage(
    const ExternalInterface::SetActiveObjectLEDs& msg) {
  SetCubeLightAnimation(msg.objectID, msg.whichLEDs, msg.onColor, msg.offColor,
                        msg.onPeriod_ms, msg.offPeriod_ms,
                        msg.transitionOnPeriod_ms, msg.transitionOffPeriod_ms,
                        msg.turnOffUnspecifiedLEDs, msg.makeRelative,
                        {msg.relativeToX, msg.relativeToY}, msg.rotate);
}

template <>
void CubeLightComponent::HandleMessage(
    const ExternalInterface::RobotDelocalized& msg) {
  _robotDelocalized = true;

  // Set light states back to connected since Cozmo has lost track of all
  // objects (except for carried objects, because we still know where those are)
  for (auto& pair : _objectInfo) {
    const bool isCarryingCube =
        _robot->GetCarryingComponent().IsCarryingObject(pair.first);
    if (!isCarryingCube) {
      PlayLightAnimByTrigger(pair.first, CubeAnimationTrigger::Connected,
                             AnimLayerEnum::State);
    }
  }
}
...
template <>
void CubeLightComponent::HandleMessage(
    const ExternalInterface::EnableLightStates& msg) {
  // When enable == true in the EnableLightStates message all layers should be
  // enabled so we don't want to only enable the game layer hence the negation
  // of msg.enable This is so the EnableLightStates message did not need to
  // change
  EnableGameLayerOnly(msg.objectID, !msg.enable);
}

Result CubeLightComponent::SetCubeLightAnimation(
    const ObjectID& objectID, const CubeLightAnimation::ObjectLights& values) {
  Block* activeObject = nullptr;
  if (values.makeRelative == MakeRelativeMode::RELATIVE_LED_MODE_OFF) {
    // note this could be a checked_cast
    activeObject = dynamic_cast<Block*>(
        _robot->GetBlockWorld().GetConnectedBlockByID(objectID));
  } else {
    // note this could be a checked_cast
    activeObject = dynamic_cast<Block*>(
        _robot->GetBlockWorld().GetLocatedObjectByID(objectID));
  ...
  activeObject->SetLEDs(values.onColors, values.offColors, values.onPeriod_ms,
                        values.offPeriod_ms, values.transitionOnPeriod_ms,
                        values.transitionOffPeriod_ms, values.offset);
...
Result CubeLightComponent::SetCubeLightAnimation(
    const ObjectID& objectID, const WhichCubeLEDs whichLEDs, const u32 onColor,
    const u32 offColor, const u32 onPeriod_ms, const u32 offPeriod_ms,
    const u32 transitionOnPeriod_ms, const u32 transitionOffPeriod_ms,
    const bool turnOffUnspecifiedLEDs, const MakeRelativeMode makeRelative,
    const Point2f& relativeToPoint, const bool rotate) {
  Block* activeCube = nullptr;
  if (makeRelative == MakeRelativeMode::RELATIVE_LED_MODE_OFF) {
    // note this could be a checked_cast
    activeCube = dynamic_cast<Block*>(
        _robot->GetBlockWorld().GetConnectedBlockByID(objectID));
  } else {
    // note this could be a checked_cast
    activeCube = dynamic_cast<Block*>(
  ...
  // trying to do relative mode in an object that is not located in the current
  // origin, will fail, since its pose doesn't mean anything for relative
  // purposes
  if (activeCube == nullptr) {
    PRINT_CH_INFO("CubeLightComponent",
  ...
  // NOTE: if make relative mode is "off", this call doesn't do anything:
  rotatedWhichLEDs = activeCube->MakeWhichLEDsRelativeToXY(
      whichLEDs, relativeToPoint, makeRelative);

  activeCube->SetLEDs(rotatedWhichLEDs, onColor, offColor, onPeriod_ms,
                      offPeriod_ms, transitionOnPeriod_ms,
                      transitionOffPeriod_ms, 0, turnOffUnspecifiedLEDs);
...
Result CubeLightComponent::SetLights(const Block* object, const bool rotate) {
  ...
  for (int i = 0; i < CubeLightAnimation::kNumCubeLEDs; ++i) {
    ...
    cubeLights.lights[i].transitionOnFrames =
        MS_TO_LED_FRAMES(ledState.transitionOnPeriod_ms);
    cubeLights.lights[i].transitionOffFrames =
        MS_TO_LED_FRAMES(ledState.transitionOffPeriod_ms);
    ...
    if (DEBUG_LIGHTS) {
      PRINT_CH_DEBUG("CubeLightComponent", "CubeLightComponent.SetLights",
                     ...
                     "onFrames 0x%x (%ums), "
                     "offFrames 0x%x (%ums), transOnFrames 0x%x (%ums), "
                     "transOffFrames 0x%x (%ums), offset 0x%x (%ums)",
                     ...
                     cubeLights.lights[i].transitionOnFrames,
                     ledState.transitionOnPeriod_ms,
                     cubeLights.lights[i].transitionOffFrames,
                     ledState.transitionOffPeriod_ms,
  ...
  if (activeId != _robot->GetCubeCommsComponent().GetConnectedCubeActiveId()) {
    PRINT_NAMED_WARNING("CubeLightComponent.SetLights.ObjectNotConnected",
                        "Object with activeId %d is not connected", activeId);
  } else {
...
// TEMP (Kevin): WhiteBalancing is eventually to be done in body so just doing
// something simple here to get us by.
//               Basically if there is any red at all, then blue and green
//               channels are scaled down to 60%.
ColorRGBA CubeLightComponent::WhiteBalanceColor(
    const ColorRGBA& origColor) const {
...
void CubeLightComponent::HandleAppRequest(const AppToEngineEvent& event) {
  ...
  switch (event.GetData().GetTag()) {
    case GatewayWrapperTag::kSetCubeLightsRequest: {
      PRINT_NAMED_INFO(
          "CubeLightComponent.HandleAppRequest.SetCubeLightsRequest",
      ...
      std::copy(request.on_color().begin(), request.on_color().end(),
                onColor.begin());

      std::array<unsigned int, 4> offColor;
      std::copy(request.off_color().begin(), request.off_color().end(),
                offColor.begin());

      std::array<unsigned int, 4> onPeriodMs;
      std::copy(request.on_period_ms().begin(), request.on_period_ms().end(),
                onPeriodMs.begin());

      std::array<unsigned int, 4> offPeriodMs;
      std::copy(request.off_period_ms().begin(), request.off_period_ms().end(),
                offPeriodMs.begin());

      std::array<unsigned int, 4> transitionOnPeriodMs;
      std::copy(request.transition_on_period_ms().begin(),
                request.transition_on_period_ms().end(),
                transitionOnPeriodMs.begin());

      std::array<unsigned int, 4> transitionOffPeriodMs;
      std::copy(request.transition_off_period_ms().begin(),
                request.transition_off_period_ms().end(),
                transitionOffPeriodMs.begin());

      std::array<int, 4> offset;
      std::copy(request.offset().begin(), request.offset().end(),
                offset.begin());
      ...
      if (request.make_relative() == 0) {
        ...
        const CubeLightAnimation::ObjectLights lights{
            ...
            .makeRelative = (Anki::Vector::MakeRelativeMode)(
                (int)request.make_relative() - 1),
            .relativePoint = {request.relative_to_x(),
                              request.relative_to_y()}};
        ...
        // TODO: Fully integrate these light calls with the sdk behavior
        // VIC-4872
        PlayLightAnimInternal(request.object_id(), animation,
                              AnimLayerEnum::Engine, nullptr, false, {}, 0,
                              "GeneratedWrappedAnimation", handle);
      }
      break;
    }
