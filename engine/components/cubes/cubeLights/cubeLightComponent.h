 ...
 * Created: 12/2/2016
 *
 * Description: Manages playing cube light animations on objects as well as
 *backpack lights A cube light animation consists of multiple patterns, where
 *each pattern has a duration and when the pattern has been playing for that
 *duration the next pattern gets played. An animation can be played on one of
 *three layers (User/Game, Engine, Default), which function as a priority system
 *with the User/Game layer having highest priority. Each layer can have multiple
...
class CubeLightComponent : public IDependencyManagedComponent<RobotComponentID>,
 ...
 public:
  ...
  CubeLightAnimation::Animation* GetAnimation(
      const CubeAnimationTrigger& animTrigger);

  // Takes whatever animation is pointed to by the animTrigger and plays it on
  // the specified object Optional Arguments
  ...
  // - A duration modifier to increase the duration of all patterns in the
  // animation Plays the animation on the engine layer Returns true if the
  // animation was able to be played
  bool PlayLightAnimByTrigger(
      const ObjectID& objectID, const CubeAnimationTrigger& animTrigger,
      AnimCompletedCallback callback = {},
      bool playDuringBackgroundConnection = false, bool hasModifier = false,
  ...
  bool PlayLightAnim(const ObjectID& objectID,
                     ...
                     AnimCompletedCallback callback,
                     const std::string& debugName, AnimationHandle& outHandle);

  // Stops the specified animation pointed to by the animTrigger on an optional
  // object and resumes whatever animation was previously playing By default the
  // specified animation is stopped on all objects Returns true if the animation
  // was stopped
  ...
  // Stops the specified animation and immediately plays the animTriggerToPlay
  // without allowing whatever animation was previously playing from resuming
  // Returns true if the animation was able to be played
  bool StopAndPlayLightAnim(
      const ObjectID& objectID, const CubeAnimationTrigger& animTriggerToStop,
      const CubeAnimationTrigger& animTriggerToPlay,
      AnimCompletedCallback callback = {}, bool hasModifier = false,
      const CubeLightAnimation::ObjectLights& modifier = {});
  // handle will be used to stop an animation, and then set with the handle for
  // the new animToPlay
  bool StopAndPlayLightAnim(
  ...
  // NOTE: objectID's throughout this system will likely be unnecessary on
  // victor since he has only got one cube (just let the cubeCommsComponent
  // worry about it) but that's a thought for later...
  bool PlayConnectionLights(const ObjectID& objectID,
                            AnimCompletedCallback callback = {});
  bool PlayDisconnectionLights(const ObjectID& objectID,
                               AnimCompletedCallback callback = {});
  bool CancelDisconnectionLights(const ObjectID& objectID);
  bool PlayTapResponseLights(const ObjectID& objectID,
                             AnimCompletedCallback callback = {});
  ...
  // In these scenarios engine may need to check whether it can set lights or
  // not to ensure that a StopAndPlay was not cancelled before another one was
  // called which will hit an assert
  bool CanEngineSetLightsOnCube(const ObjectID& objectID);

 private:
  // Animations can be played on three different layers: User, Engine, or
  // Default User layer is for animations triggered by game Engine layer is for
  // animations triggered within engine Default layer is for the constantly
  // playing animations that change based on object posestate (WakeUp,
  // Connected, Visible, Carrying)
  enum AnimLayerEnum {
    User,    // Lights set from external sources like game
    Engine,  // Lights set from within engine (interacting, doubleTap)
    State,  // Light patterns that are always playing and connected to the state
            // of the cubes (visible, connected, wakeUp)
  ...
  struct CurrentAnimInfo {
    ...
    // Animation handle used internally/externally to reference playing
    // animations
    AnimationHandle animationHandle;

    // An iterator to the current light pattern within the CubeLightAnimation
    ...
    // The basestation time that the curPattern ends and the next pattern in the
    // CubeLightAnimation should start playing
    ...
    // If this is != 0 then this animation was played from the
    // TriggerCubeAnimationAction This is to prevent calling a callback that is
    // invalid due to the action being destroyed before the animation completes
    u32 actionTagCallbackIsFrom = 0;
  };
  // An array of all the animations that are playing on each layer (indexed by
  // layer) Multiple animations can be "playing" on a given layer (hence the
  // list of PatternPlayInfo) This is so that you can blend animations and
  // return to the previously playing animation
  using AnimLayer =
      std::array<std::list<CurrentAnimInfo>, AnimLayerEnum::Count>;
  ...
  struct ObjectInfo {
    ...
    // Array (indexed by layer) of lists of PatternPlayInfo (actively playing
    // animations)
    AnimLayer animationsOnLayer;

    // Array (indexed by layer) of dynamically created animations (blended anims
    // and modified anims)
    std::array<CubeLightAnimation::Animation, AnimLayerEnum::Count>
        blendedAnims;
  };
  ...
  // Default to true so that status lights are driven from the
  ...
  // Only TriggerCubeAnimationAction should call this PlayLightAnim function
  // because the action will play the animation on the user layer instead of the
  // engine layer
  ...
  // Plays an anim on an obect on the given layer. Optionally takes a callback
  // and/or a modifier to be applied to the animation Returns true if the
  // animation was able to be played
  bool PlayLightAnimByTrigger(
      const ObjectID& objectID, const CubeAnimationTrigger& animTrigger,
  ...
  // shouldPickNextAnimOnStop determines if we can immediately pick a default
  // anim to start playing on the State layer should there be no Engine anims
  // left Returns true if the animation was stopped
  ...
  void StopAllAnimsOnLayer(const AnimLayerEnum& layer,
                           const ObjectID& objectID = -1);

  Result SetCubeLightAnimation(const ObjectID& objectID,
                               const CubeLightAnimation::ObjectLights& lights);
  Result SetCubeLightAnimation(
      const ObjectID& objectID, const WhichCubeLEDs whichLEDs,
      const u32 onColor, const u32 offColor, const u32 onPeriod_ms,
      const u32 offPeriod_ms, const u32 transitionOnPeriod_ms,
      const u32 transitionOffPeriod_ms, const bool turnOffUnspecifiedLEDs,
      const MakeRelativeMode makeRelative, const Point2f& relativeToPoint,
      const bool rotate);

  // Applies the modifier to the animation by augmenting the light values in the
  // animation Will apply the modifier to all patterns in the animation Note:
  // Will not modify the duration of the animation
  void ApplyAnimModifier(const CubeLightAnimation::Animation& anim,
                         const CubeLightAnimation::ObjectLights& modifier,
                         CubeLightAnimation::Animation& modifiedAnim);

  // Attempts to blend the anim with the currently playing animation on the
  // object If the anim has any on/offColors that are 0 they get replaced by the
  // current on/offColor on the object Returns true if the animation was
  ...
  // Debug function to play subsequent animation triggers on an object with each
