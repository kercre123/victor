...
class MovementListener : public ICubeAccelListener {
 public:
  ...
  // The smaller maxToAdd is the lower the movementScore will be from table
  // shaking/random vibrations so those are easier to filter out (still reported
  // as movement so callback will be called). Does mean that it will take longer
  // for actual movements for the movementScore to reach maxAllowedScore The
  // larger the decay the quicker we stop reporting movement after stopping but
  // also reduces the ability to detect small movements
  MovementListener(const float hpFilterCoeff, const float maxMovementScoreToAdd,
 ...
 private:
  ...
  // Callback called when movement is detected. Passed in argument is the
