...
MovementStartStopListener::MovementStartStopListener(
    std::function<void(void)> startedMovingCallback,
  ...
  _movementListener = std::make_unique<MovementListener>(
      kHpFilterCoef, kMaxMovementScoreToAdd, kMovementScoreDecay,
      kMaxAllowedMovementScore, movementCallback);

  DEV_ASSERT(
      (_startedMovingThresh > _stoppedMovingThresh) &&
