...
class ShakeListener : public ICubeAccelListener {
 public:
  ...
  // highPassFilterCoeff = 0.5, highPassLowerThresh = 2.5, highPassUpperThresh
  // = 3.9 The callback will be called when shaking is detected. The callback
  // parameter is the amount of shaking (how violent the shaking is) The larger
  // hpUpperThresh is the larger/more violent the shake need to initially
  // trigger shake detection. The smaller hpLowerThresh is the smaller the shake
  // needed to continue to trigger shake detection once an initial shake has
  // been detected
 ...
 private:
  ...
  // Callback called when shaking is detected. Passed in argument is the
  ...
  // Used for the initial shake detection (typically larger than the lowerThresh
