...
void CubeAccelComponent::InitDependent(Vector::Robot* robot,
...
bool CubeAccelComponent::AddListener(
  ...
  if (obj == nullptr) {
    PRINT_NAMED_WARNING("CubeAccelComponent.AddListener.InvalidObject",
                        "Object id %d is not connected", objectID.GetValue());
    return false;
  }
  ...
  if (!successfullyInserted) {
    PRINT_NAMED_WARNING("CubeAccelComponent.AddListener.InsertFailed",
                        "Failed to insert listener for object %d. Does this "
                        "listener already exist in the set?",
                        objectID.GetValue());
  }
...
void CubeAccelComponent::HandleCubeAccelData(const ActiveID& activeID,
  ...
  if (object == nullptr) {
    DEV_ASSERT(false,
               "CubeAccelComponent.HandleCubeAccelData.NoConnectedObject");
    return;
  }
  ...
  for (const auto& accelReading : accelData.accelReadings) {
    ...
    auto rawAccelToMmps = [](const s16 rawAccel) {
      ...
      const float accelG =
          static_cast<float>(rawAccel) / std::numeric_limits<s16>::max() * 4.f;
...
template <>
void CubeAccelComponent::HandleMessage(
  ...
  if (msg.connected) {
    ...
    auto movedOrStoppedListener =
        std::make_shared<CubeAccelListeners::MovementStartStopListener>(
            startedMovingCallback, stoppedMovingCallback);
    ...
    auto upAxisListener =
        std::make_shared<CubeAccelListeners::UpAxisChangedListener>(
    ...
    if (_upAxisChangedListeners.find(objectId) !=
        _upAxisChangedListeners.end()) {
      _upAxisChangedListeners.erase(objectId);
    }
  }
}

void CubeAccelComponent::ObjectMovedOrStoppedCallback(const ObjectID objectId,
                                                      const bool isMoving) {
  ...
  if (nullptr == connectedObj) {
    ...
    DEV_ASSERT(
        connectedObj->IsActive(),
        "CubeAccelComponent.ObjectMovedOrStoppedCallback.NonActiveObject");

    LOG_INFO(
        "CubeAccelComponent.ObjectMovedOrStoppedCallback.ObjectMovedOrStopped",
    ...
    const bool shouldIgnoreMovement =
        _robot->GetBlockTapFilter().ShouldIgnoreMovementDueToDoubleTap(
            connectedObj->GetID());
    if (shouldIgnoreMovement && isMoving) {
      LOG_INFO(
          "CubeAccelComponent.ObjectMovedOrStoppedCallback.IgnoringMessage",
  ...
  const bool isCarryingObject =
      _robot->GetCarryingComponent().IsCarryingObject(objectId);
  if (locatedObject != nullptr) {
    // We expect carried objects to move, so don't mark them as
    // dirty/inaccurate. Their pose state should remain accurate/known because
    // they are attached to the lift. I'm leaving this a separate check from the
    // decision about broadcasting the movement, in case we want to easily
    // remove the checks above but keep this one.
  ...
  // Don't notify game about objects being carried that have moved, since we
  // expect them to move when the robot does.
  // TODO: Consider broadcasting carried object movement if the robot is _not_
  // moving
  //
  // Don't notify game about moving objects that are being docked with, because
  // we expect those to move if/when we bump them. But we still mark them as
  // dirty/inaccurate below because they have in fact moved and we wouldn't want
  // to localize with them.
  //
  // TODO: Consider not filtering these out and letting game ignore them somehow
  //       - Option 1: give game access to dockingID so it can do this same
  //       filtering
  //       - Option 2: add a "wasDocking" flag to the ObjectMoved/Stopped
  //       message
  //       - Option 3: add a new ObjectMovedWhileDocking message
  //

  const bool isDockingObject =
      (connectedObj->GetID() == _robot->GetDockingComponent().GetDockObject());
...
void CubeAccelComponent::ObjectUpAxisChangedCallback(const ObjectID objectId,
                                                     const UpAxis& upAxis) {
  LOG_INFO("CubeAccelComponent.ObjectUpAxisChangedCallback.UpAxisChanged",
           "ObjectID: %d, UpAxis: %s", objectId.GetValue(),
           EnumToString(upAxis));
