...
#define RGBA_TO_RGB_ARRAY(rgba)                            \
  {                                                        \
    {                                                      \
      static_cast<uint8_t>((rgba & 0xFF000000) >> 24),     \
...
LedAnimation::LedAnimation(const CubeLightState& lightState,
                           const int baseIndex) {
  ...
  if (lightState.onFrames == 0 && lightState.offFrames == 0 &&
      lightState.transitionOnFrames == 0 &&
      lightState.transitionOffFrames == 0) {
    DEV_ASSERT(_keyframes.empty(),
               "LedAnimation.LedAnimation.ShouldBeNoKeyframes");
    keyframe.color = RGBA_TO_RGB_ARRAY(lightState.onColor);
    keyframe.holdFrames = 0;
    keyframe.decayFrames = 0;
    _keyframes.push_back(keyframe);
  ...
  _keyframes.back().nextIndex = hasOffset ? 1 + baseIndex : 0 + baseIndex;
