...
class CubeCommsComponent : public IDependencyManagedComponent<RobotComponentID>,
 ...
 public:
  ...
  // Return the factory ID of the preferred cube, or an empty string if there is
  // none.
  ...
  // Returns the FactoryId of the currently-connected cube, or empty string if
  // there is no connected cube
  BleFactoryId GetConnectedCubeFactoryId() const;

  // Set whether or not to broadcast ObjectAvailable messages to game
  void SetBroadcastObjectAvailable(const bool enable = true) {
    _broadcastObjectAvailableMsg = enable;
  }

  // Return the BLE client's current cube connection state
  CubeConnectionState GetCubeConnectionState() const {
    return _cubeBleClient->GetCubeConnectionState();
  }

  // True if we are _fully_ connected to a cube over BLE
  bool IsConnectedToCube() const {
    return GetCubeConnectionState() == CubeConnectionState::Connected;
  }

  // True if a cube disconnection has been scheduled
  bool IsDisconnectScheduled() const {
    return _disconnectFromCubeTime_sec > 0.f;
  }
 ...
 private:
  ...
  BleFactoryId GetCurrentCube() const {
    return _cubeBleClient->GetCurrentCube();
  }
  ...
  void GenerateCubeLightMessages(
  ...
  void HandleCubeMessage(const BleFactoryId& factoryId,
                         const MessageCubeToEngine& msg);

  // Handler for when light cube BLE connection is established/unestablished
  void HandleConnectionStateChange(const BleFactoryId& factoryId,
  ...
  // Find a cube's ActiveID from its factoryId. Returns
  // ObservableObject::InvalidActiveID if not found
