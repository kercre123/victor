...
CubeCommsComponent::CubeCommsComponent()
  ...
  _cubeBleClient->RegisterObjectAvailableCallback(std::bind(
      &CubeCommsComponent::HandleObjectAvailable, this, std::placeholders::_1));
  _cubeBleClient->RegisterCubeMessageCallback(
      std::bind(&CubeCommsComponent::HandleCubeMessage, this,
                std::placeholders::_1, std::placeholders::_2));
  _cubeBleClient->RegisterCubeConnectionCallback(
      std::bind(&CubeCommsComponent::HandleConnectionStateChange, this,
                std::placeholders::_1, std::placeholders::_2));
  _cubeBleClient->RegisterScanFinishedCallback(
      std::bind(&CubeCommsComponent::HandleScanForCubesFinished, this));
  _cubeBleClient->RegisterConnectionFailedCallback(
      std::bind(&CubeCommsComponent::HandleConnectionFailed, this,
                std::placeholders::_1));
}

void CubeCommsComponent::InitDependent(Vector::Robot* robot,
  ...
  if (ANKI_VERIFY(_robot->HasExternalInterface(),
    ...
    auto callback = std::bind(&CubeCommsComponent::HandleGameEvents, this,
                              std::placeholders::_1);
    auto* extInterface = _robot->GetExternalInterface();
    _signalHandles.push_back(extInterface->Subscribe(
        ExternalInterface::MessageGameToEngineTag::ConnectToCube, callback));
    _signalHandles.push_back(extInterface->Subscribe(
        ExternalInterface::MessageGameToEngineTag::DisconnectFromCube,
        callback));
    _signalHandles.push_back(extInterface->Subscribe(
        ExternalInterface::MessageGameToEngineTag::ForgetPreferredCube,
        callback));
    _signalHandles.push_back(extInterface->Subscribe(
        ExternalInterface::MessageGameToEngineTag::SetPreferredCube, callback));
    _signalHandles.push_back(extInterface->Subscribe(
        ExternalInterface::MessageGameToEngineTag::SendAvailableObjects,
        callback));
  ...
  if (ANKI_VERIFY(_robot->HasGatewayInterface(),
    ...
    auto callback = std::bind(&CubeCommsComponent::HandleAppEvents, this,
                              std::placeholders::_1);
    _signalHandles.push_back(gatewayInterface->Subscribe(
        external_interface::GatewayWrapperTag::kCubesAvailableRequest,
        callback));
  }
  ...
  if (ANKI_VERIFY(platform != nullptr,
    ...
    const auto& cubeFirmwarePath = platform->pathToResource(
        Util::Data::Scope::Resources, kCubeFirmwarePath);
  ...
  if (_startScanWhenUnconnected) {
    if (GetCubeConnectionState() == CubeConnectionState::UnconnectedIdle) {
      LOG_INFO("CubeCommsComponent.UpdateDependent.StartingScan",
               ...
               "Starting a scan for cubes, which should result in a cube "
               "connection");
      _startScanWhenUnconnected = false;
      if (!StartScanningForCubes()) {
        PRINT_NAMED_WARNING(
            "CubeCommsComponent.UpdateDependent.FailedInitiateScan",
            "Failed to initiate scanning for cubes");
      }
    } else if (GetCubeConnectionState() !=
               CubeConnectionState::PendingDisconnect) {
      DEV_ASSERT_MSG(
          false, "CubeCommsComponent.UpdateDependent.UnexpectedConnectionState",
          "A scan is scheduled (_startScanWhenUnconnected == true), but we are "
          "not pending a disconnection. Connection state: %s",
...
bool CubeCommsComponent::RequestConnectToCube(
    const ConnectionCallback& connectedCallback) {
  // If a disconnection is scheduled, un-schedule it and return.
  if (_disconnectFromCubeTime_sec > 0.f) {
    _disconnectFromCubeTime_sec = -1.f;
    DEV_ASSERT(IsConnectedToCube(),
               "CubeCommsComponent.RequestConnectToCube."
               "ShouldBeConnectedIfDisconnectScheduled");
    LOG_INFO(
        "CubeCommsComponent.RequestConnectToCube.CancellingScheduledDisconnect",
        "We are already connected to this cube, but a disconnection was "
        "scheduled. Cancelling the scheduled disconnection.");
  ...
  switch (currConnectionState) {
    ...
    case CubeConnectionState::PendingConnect: {
      PRINT_NAMED_WARNING(
          "CubeCommsComponent.RequestConnectToCube.AlreadyConnected",
          "Already connected or pending connection to cube %s (active ID %d). "
          "Connection state %s",
          GetCurrentCube().c_str(), GetConnectedCubeActiveId(),
          CubeConnectionStateToString(GetCubeConnectionState()));
      return false;
    }
    case CubeConnectionState::ScanningForCubes: {
      PRINT_NAMED_WARNING(
          "CubeCommsComponent.RequestConnectToCube.AlreadyScanning",
    ...
    case CubeConnectionState::UnconnectedIdle: {
      ...
      if (!StartScanningForCubes()) {
        PRINT_NAMED_WARNING(
            "CubeCommsComponent.RequestConnectToCube.FailedInitiateScan",
            "Failed to initiate scanning for cubes");
        return false;
      }
      break;
    ...
    case CubeConnectionState::PendingDisconnect: {
      LOG_INFO("CubeCommsComponent.RequestConnectToCube.ScanScheduled",
               "We are currently pending a disconnection from a cube, so we "
               "will attempt to scan/connect to "
    ...
    default: {
      PRINT_NAMED_WARNING(
          "CubeCommsComponent.RequestConnectToCube.InvalidConnectionState",
...
bool CubeCommsComponent::RequestDisconnectFromCube(
    const float gracePeriod_sec,
    const ConnectionCallback& disconnectedCallback) {
  if (GetCubeConnectionState() == CubeConnectionState::UnconnectedIdle) {
    PRINT_NAMED_WARNING(
        "CubeCommsComponent.RequestDisconnectFromCube.Unconnected",
        "Already unconnected!");
    return false;
  }
  ...
  if (gracePeriod_sec <= 0.f) {
    ...
    LOG_INFO("CubeCommsComponent.RequestDisconnectFromCube.DisconnectScheduled",
             "Disconnect scheduled to occur in %.2f seconds. Cube will remain "
             "connected in the interim.",
             gracePeriod_sec);
  }
...
void CubeCommsComponent::SetPreferredCube(const BleFactoryId& factoryId) {
  ...
  if (ANKI_VERIFY(platform != nullptr,
    ...
    const auto& preferredCubeFilePath = platform->pathToResource(
        Util::Data::Scope::Persistent, kPreferredCubeFilename);
    ...
    writer.Close();
    if (0 != rename(tmpPreferredCubeFilePath.c_str(),
                    preferredCubeFilePath.c_str())) {
...
bool CubeCommsComponent::SendCubeLights(const CubeLights& cubeLights) {
  ...
  GenerateCubeLightMessages(cubeLights, cubeLightSequence,
  ...
  for (auto& keyframeMsg : cubeLightKeyframeChunks) {
    ...
    if (!SendCubeMessage(cubeKeyframeChunkMsg)) {
      PRINT_NAMED_WARNING(
          "CubeCommsComponent.SendCubeLights.FailedSendingChunk",
  ...
  if (!SendCubeMessage(cubeLightSequenceMsg)) {
    PRINT_NAMED_WARNING(
        "CubeCommsComponent.SendCubeLights.FailedSendingSequence",
...
bool CubeCommsComponent::StartScanningForCubes(
    const bool autoConnectAfterScan) {
  if (GetCubeConnectionState() == CubeConnectionState::ScanningForCubes) {
    PRINT_NAMED_WARNING(
        "CubeCommsComponent.StartScanningForCubes.AlreadyScanning",
  ...
  if (IsConnectedToCube() ||
    ...
    PRINT_NAMED_WARNING(
        "CubeCommsComponent.StartScanningForCubes.AlreadyConnected",
        "Already connected or pending connection to cube %s - not initiating "
        "scan (connection state %s)",
  ...
  LOG_INFO("CubeCommsComponent.StartScanningForCubes.StartScan",
           "Beginning scan for cubes (duration %.2f seconds). Will %sattempt "
           "to connect to a cube after scan.",
           kDefaultScanningTime_sec, _connectAfterScan ? "" : "NOT ");
...
void CubeCommsComponent::StopScanningForCubes() {
  if (GetCubeConnectionState() != CubeConnectionState::ScanningForCubes) {
    PRINT_NAMED_WARNING("CubeCommsComponent.StopScanningForCubes.NotScanning",
                        "StopScanning requested, but we are not currently "
                        "scanning for cubes! Connection state: %s",
                        CubeConnectionStateToString(GetCubeConnectionState()));
  }
...
void CubeCommsComponent::GenerateCubeLightMessages(
    const CubeLights& cubeLights, CubeLightSequence& cubeLightSequence,
    std::vector<CubeLightKeyframeChunk>& cubeLightKeyframeChunks) {
  DEV_ASSERT(cubeLightKeyframeChunks.empty(),
             "CubeCommsComponent.GenerateCubeLightMessages."
             "CubeLightKeyframeChunksNotEmpty");
  cubeLightKeyframeChunks.clear();

  DEV_ASSERT(!(cubeLights.playOnce && cubeLights.rotate),
             "CubeCommsComponent.GenerateCubeLightMessages."
             "CannotHaveBothPlayOnceAndRotation");
  ...
  DEV_ASSERT(animations.size() == kNumCubeLEDs,
             "CubeCommsComponent.GenerateCubeLightMessages.WrongNumAnimations");
  ...
  // essentially concatenates the keyframes of all 4 LED animations then
  // separates the entire list into chunks of 3 for sending over the wire.
...
void CubeCommsComponent::HandleGameEvents(
    const AnkiEvent<ExternalInterface::MessageGameToEngine>& event) {
  ...
  switch (tag) {
    ...
    case ExternalInterface::MessageGameToEngineTag::DisconnectFromCube:
      RequestDisconnectFromCube(
          event.GetData().Get_DisconnectFromCube().gracePeriod_sec);
      break;
    ...
    case ExternalInterface::MessageGameToEngineTag::SendAvailableObjects:
      _broadcastObjectAvailableMsg =
          event.GetData().Get_SendAvailableObjects().enable;
      break;
    default:
...
void CubeCommsComponent::HandleAppEvents(
    const AnkiEvent<external_interface::GatewayWrapper>& event) {
  if (event.GetData().GetTag() ==
      external_interface::GatewayWrapperTag::kCubesAvailableRequest) {
    if (_robot->HasGatewayInterface()) {
      ...
      // TODO(str): Hack to make the FinalCheck App screen detect cube
      // de-tabbing properly, see VIC-12123 If we haven't seen any cubes yet,
      // run a scan
      if (_cubeScanResults.empty() &&
          GetCubeConnectionState() == CubeConnectionState::UnconnectedIdle) {
...
void CubeCommsComponent::HandleObjectAvailable(
  ...
  DEV_ASSERT_MSG(msg.objectType == kValidCubeType,
                 ...
                 "%s is not a valid object type. We expect to hear only from "
                 "objects of type %s",
                 ObjectTypeToString(msg.objectType),
                 ObjectTypeToString(kValidCubeType));

  const bool cubeUnknown =
      (_cubeScanResults.find(msg.factory_id) == _cubeScanResults.end());
  ...
  if (msg.factory_id == _preferredCubeFactoryId) {
    LOG_INFO("CubeCommsComponent.HandleObjectAvailable.EndingScanEarly",
             "Ending cube scan early since we have heard from our preferred "
             "cube '%s'",
...
void CubeCommsComponent::HandleCubeMessage(const BleFactoryId& factoryId,
                                           const MessageCubeToEngine& msg) {
  ...
  switch (msg.GetTag()) {
    case MessageCubeToEngineTag::accelData: {
      _robot->GetCubeAccelComponent().HandleCubeAccelData(activeId,
                                                          msg.Get_accelData());
      break;
    }
    case MessageCubeToEngineTag::voltageData: {
      _robot->GetCubeBatteryComponent().HandleCubeVoltageData(
          factoryId, msg.Get_voltageData());
      break;
    }
    default: {
...
void CubeCommsComponent::HandleConnectionStateChange(
    const BleFactoryId& factoryId, const bool connected) {
  LOG_INFO("CubeCommsComponent.HandleConnectionStateChange.Recvd",
           "FactoryID %s, connected %d", factoryId.c_str(), connected);
...
void CubeCommsComponent::OnCubeConnected(const BleFactoryId& factoryId) {
  if (_factoryIdToActiveIdMap.find(factoryId) ==
      _factoryIdToActiveIdMap.end()) {
    // Must be the first time we're connecting to this cube. Generate a new
    // ActiveID.
  ...
  const ObjectID objID = _robot->GetBlockWorld().AddConnectedBlock(
      activeId, factoryId, kValidCubeType);
  if (objID.IsSet()) {
    LOG_INFO("CubeCommsComponent.OnCubeConnected.Connected",
             "Object %d (activeID %d, factoryID %s)", objID.GetValue(),
             activeId, factoryId.c_str());

    // TODO: arguably blockworld should do this, because when do we want to
    // remove/add objects and not notify? Send connection message to game
    using namespace ExternalInterface;
    _robot->Broadcast(MessageEngineToGame(ObjectConnectionState(
        objID.GetValue(), factoryId, kValidCubeType, true)));
  }
...
void CubeCommsComponent::OnCubeDisconnected(const BleFactoryId& factoryId) {
  ...
  // Remove active object from blockworld if it exists, and remove all instances
  // in all origins
  const ObjectID objID = _robot->GetBlockWorld().RemoveConnectedBlock(activeId);

  // TODO: arguably blockworld should do this, because when do we want to
  // remove/add objects and not notify?
  if (objID.IsSet()) {
    // Send connection message to game
    using namespace ExternalInterface;
    _robot->Broadcast(MessageEngineToGame(ObjectConnectionState(
        objID.GetValue(), factoryId, kValidCubeType, false)));
  }
...
void CubeCommsComponent::HandleScanForCubesFinished() {
  if (_cubeScanResults.empty()) {
    LOG_INFO("CubeCommsComponent.HandleScanForCubesFinished.NoCubesFound",
             "List of available cubes is empty - no advertising cubes were "
             "found during scanning. Sad!");
  ...
  LOG_INFO(
      "CubeCommsComponent.HandleScanForCubesFinished.ScanningForCubesEnded",
  ...
  DASMSG_SET(i1, _cubeScanResults.size(),
             "Number of advertising cubes found during scan");
  DASMSG_SEND();

  if (!_connectAfterScan) {
    LOG_INFO(
        "CubeCommsComponent.HandleScanForCubesFinished.IgnoringScanResults",
        "Scanning has completed but _connectAfterScan is false, so we will not "
        "attempt to connect to a cube");
    return;
  }

  // Discovery period has ended. Loop over list of available cubes and connect
  // to the preferred cube if possible, else the cube with highest RSSI.
  ...
  DEV_ASSERT(!cubeToConnectTo.empty(),
             "CubeCommsComponent.HandleScanForCubesFinished.NoCubeChosen");

  LOG_INFO("CubeCommsComponent.HandleScanForCubesFinished.AttemptingConnection",
           "Attempting to connect to cube with factoryID %s because %s. Signal "
           "strength %d.",
  ...
  if (!_cubeBleClient->RequestConnectToCube(cubeToConnectTo)) {
    PRINT_NAMED_WARNING(
        "CubeCommsComponent.HandleScanForCubesFinished."
        "FailedRequestingConnection",
...
void CubeCommsComponent::HandleConnectionFailed(const BleFactoryId& factoryId) {
  PRINT_NAMED_WARNING(
      "CubeCommsComponent.HandleConnectionFailed.ConnectionAttemptFailed",
      "Connection attempt to cube %s has failed", factoryId.c_str());
...
void CubeCommsComponent::ReadPreferredCubeFromFile() {
  ...
  if (ANKI_VERIFY(
          ...
          "")) {
    const auto& preferredCubeFile = platform->pathToResource(
        Util::Data::Scope::Persistent, kPreferredCubeFilename);
    ...
    if (Util::FileUtils::FileExists(preferredCubeFile)) {
      ...
      if (reader.parse(contents, root)) {
        parsingSuccessful = JsonTools::GetValueOptional(
            root, kBlockFacIdKey, _preferredCubeFactoryId);
      }
    }

    if (!parsingSuccessful) {
      PRINT_NAMED_WARNING(
          "CubeCommsComponent.ReadPreferredCubeFromFile."
          "FailedLoadingPreferredCubeFromFile",
    ...
    if (webService != nullptr) {
      auto onData =
          [this](const Json::Value& data,
            ...
            const bool shouldFlashCubeLights =
                flashCubeLights.isBool() && flashCubeLights.asBool();
            if (shouldFlashCubeLights && IsConnectedToCube()) {
              // flash lights on connected cube
              const auto& activeId = GetActiveId(GetCurrentCube());
              const auto* object =
                  _robot->GetBlockWorld().GetConnectedBlockByActiveID(activeId);
              if (object != nullptr) {
                _robot->GetCubeLightComponent().PlayLightAnimByTrigger(
                    object->GetID(), CubeAnimationTrigger::Flash);
              }
            }

            const auto& connectCube = data["connectCube"];
            const bool shouldConnectCube =
                connectCube.isBool() && connectCube.asBool();
            if (shouldConnectCube) {
              RequestConnectToCube();
            }

            const auto& disconnectCube = data["disconnectCube"];
            const bool shouldDisconnectCube =
                disconnectCube.isBool() && disconnectCube.asBool();
            if (shouldDisconnectCube) {
              RequestDisconnectFromCube(0.f);
            }

            const auto& forgetPreferredCube = data["forgetPreferredCube"];
            const bool shouldForgetCube =
                forgetPreferredCube.isBool() && forgetPreferredCube.asBool();
            if (shouldForgetCube) {
              ForgetPreferredCube();
            }
          };

      _signalHandles.emplace_back(
          webService->OnWebVizData(kWebVizModuleNameCubes)
              .ScopedSubscribe(onData));
    }
  }
}
...
void CubeCommsComponent::SendDataToWebViz() {
  ...
  if (context != nullptr) {
    if (auto webSender = WebService::WebVizSender::CreateWebVizSender(
            kWebVizModuleNameCubes, context->GetWebService())) {
      Json::Value commInfo = Json::objectValue;
      std::string connectionState =
          CubeConnectionStateToString(GetCubeConnectionState());
      if (IsConnectedToCube()) {
        const auto& batteryVoltsStr = std::to_string(
            _robot->GetCubeBatteryComponent().GetCubeBatteryVoltage(
                GetCurrentCube()));
        connectionState += " (battery: " + batteryVoltsStr + "V)";
      }
      commInfo["connectionState"] = connectionState;
      commInfo["connectedCube"] =
          IsConnectedToCube() ? GetCurrentCube() : "(none)";
      commInfo["preferredCube"] =
          _preferredCubeFactoryId.empty() ? "(none)" : _preferredCubeFactoryId;
