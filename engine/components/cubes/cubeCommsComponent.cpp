...
void CubeCommsComponent::Disconnect()
{
  for(const auto& iter : _availableCubes)
  {
    if(iter.second.connected)
    {
      _cubeBleClient->DisconnectFromCube(iter.second.factoryId);
    }
  }
}

bool CubeCommsComponent::IsConnectedToCube()
{
  for(const auto& iter : _availableCubes)
  {
    if(iter.second.connected)
...
{
  if(_connectionChangedCallback != nullptr)
  {
    _connectionChangedCallback(connected);
  }
