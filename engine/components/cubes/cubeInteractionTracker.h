 ...
 * Created: 2018-7-17
 *
 * Description: Robot component which monitors cube data and attempts to discern
 *when a user is physically interacting with a cube.
 *
...
struct TargetStatus {
  ...
  const ObservableObject*
      observableObject;  // valid only if located in blockWorld
  ...
  bool visibleRecently;  // true if our current target was seen less than
                         // kRecentlyVisibleLimit_s ago
  bool observedSinceMovedByRobot;
  bool movedThisFrame;
  bool movedRecently;  // true if our current target moved less than
                       // kRecentlyMovedLimit_s ago
...
class CubeInteractionTracker
 ...
 private:
  enum class ECITState { Idle, TrackingConnected, TrackingUnconnected };
  ...
  void HandleEngineEvents(
      const AnkiEvent<ExternalInterface::MessageEngineToGame>& event);
  ...
  // TODO:(str) Make this work for multiple visible targets (dev
  // environment/future proofing) std::vector<TargetStatus> _targets;
