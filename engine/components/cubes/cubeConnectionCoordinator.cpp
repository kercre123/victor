 ...
 * Created: 6/20/18
 *
 * Description: Component to which behaviors can submit cube connection
 *subscriptions and cube connection state queries. Based on subscriptions, opens
 *and closes cube connections and manages "fundamental" cube connection light
 *              animations
 *
...
// Local constants
namespace {
// How long should we wait without non-background subscriptions before switching
...
// How long should the CubeCommsComponent hold the connection after disconnect
// is requested
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
CubeConnectionCoordinator::SubscriberRecord::SubscriberRecord(
    ICubeConnectionSubscriber* const subscriberPtr, bool connectInBackground,
    float expireIn_s)
    : subscriber(subscriberPtr),
      isBackgroundConnection(connectInBackground),
      expirationTime_s(
          expireIn_s == kDontExpire
              ? kDontExpire
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeConnectionCoordinator::InitDependent(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeConnectionCoordinator::GetUpdateDependencies(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeConnectionCoordinator::UpdateDependent(
  ...
  switch (_coordinatorState) {
    ...
    case ECoordinatorState::ConnectedInteractable: {
      ...
      // If we've been without subscribers for long enough, convert to a
      // background connection
      if (_timeToEndStandby_s != 0 && _timeToEndStandby_s < currentTime_s) {
    ...
    case ECoordinatorState::ConnectedBackground: {
      ...
      if (_timeToDisconnect_s != 0 && _timeToDisconnect_s < currentTime_s) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeConnectionCoordinator::SubscribeToCubeConnection(
    ICubeConnectionSubscriber* const subscriber, const bool connectInBackground,
  ...
  if (!newRecord.isBackgroundConnection) {
    ...
    _timeToEndStandby_s = 0;
    if (ECoordinatorState::ConnectedSwitchingToBackground ==
        _coordinatorState) {
      _cancelSwitchToBackground = true;
    }
  }

  // If we're already connected, invoke appropriate callbacks now since they
  // won't be invoked by state transitions
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool CubeConnectionCoordinator::UnsubscribeFromCubeConnection(
    ICubeConnectionSubscriber* const subscriber) {
  SubscriptionIterator it;
  if (!FindRecordBySubscriber(subscriber, it)) {
    bool subscriberWasDumped = false;
    if (_connectionLostUnexpectedly) {
      // If we recently lost connection, allow unsubscribes from dumped
      // subscribers without errors
    ...
    if (!subscriberWasDumped) {
      PRINT_NAMED_WARNING(
          "CubeConnectionCoordinator.InvalidUnsubscribeRequest",
  ...
  if (!it->isBackgroundConnection) {
    if (--_nonBackgroundSubscriberCount == 0) {
      LOG_INFO("CubeConnectionCoordinator.NoInteractableSubscribers",
               "No remaining interactable subscribers, transitioning to "
               "background in %f seconds",
               kStandbyTimeout_s);
  ...
  if (_subscriptionRecords.empty()) {
    // This timeout gets reset in TransitionToConnectedBackground, but it should
    // be set from here so that we know when to disconnect if already in
    // ConnectedBackground state when our last subscriber drops
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeConnectionCoordinator::CheckForConnectionLoss(
    const RobotCompMap& dependentComps) {
  if (!dependentComps.GetComponent<CubeCommsComponent>().IsConnectedToCube()) {
    PRINT_NAMED_WARNING("CubeConnectionCoordinator.ConnectionDropped",
                        "Connection lost unexpectedly. Notifying subscribers "
                        "and dumping subscription record");

    std::set<ICubeConnectionSubscriber*> subscribersRemovedThisTick;
    for (auto& subscriberRecord : _subscriptionRecords) {
      // Track the subscribers getting dumped, the subscription record will be
      // wiped to avoid errant state
    ...
    float connectionDuration_s =
        BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() -
        _connectionStartTime_s;
    ...
    DASMSG_SET(
        i1, !_subscriptionRecords.empty(),
        "1 if there were any subscribers at disconnect, 0 or null otherwise");
    DASMSG_SET(i2, std::round(connectionDuration_s),
    ...
    dependentComps.GetComponent<CubeLightComponent>().SetCubeBackgroundState(
        connectedInBackground);

    SET_STATE(UnConnected);

    // do a separate pass through the list instead of the previous loop over
    // _subscriptionRecords, since that could be modified by
    // ConnectionLostCallback() if, e.g., the subscriber wants to reconnect
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeConnectionCoordinator::TransitionToConnectedInteractable(
    const RobotCompMap& dependentComps) {
  ...
  // Notify subscribers
  for (auto& subscriberRecord : _subscriptionRecords) {
    subscriberRecord.subscriber->ConnectedCallback(
        CubeConnectionType::Interactable);
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeConnectionCoordinator::TransitionToSwitchingToBackground(
    const RobotCompMap& dependentComps) {
  SET_STATE(ConnectedSwitchingToBackground);
  _timeToSwitchToBackground_s = 0;

  LOG_INFO(
      "CubeConnectionCoordinator.SwitchingToBackgroundConnection",
  ...
  auto animCompletedCallback = [this, &cubeLights]() {
    // It's possible a subscription came in while playing disconnect lights from
    // either the user or an internal action. If that's the case, don't modify
    // the state of the CubeConnectionCoordinator from the callback.
    if (!_cancelSwitchToBackground) {
      float currentTime_s =
          BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  ...
  if (nullptr != _connectedBlock) {
    if (!cubeLights.PlayDisconnectionLights(_connectedBlock->GetID(),
                                            animCompletedCallback)) {
      // If the lights didn't play, the callback will never come from the light
      // component. Invoke it now.
      animCompletedCallback();
    }
  } else {
    // We don't have a valid object to play lights on, the callback will never
    // come, invoke it now
    animCompletedCallback();
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeConnectionCoordinator::CancelSwitchToBackground(
    const RobotCompMap& dependentComps) {
  // Don't play connection lights or notify subscribers, since the cube
  // connection is still live
  SET_STATE(ConnectedInteractable);

  LOG_INFO("CubeConnectionCoordinator.SwitchToBackgroundInterrupted",
           "New interactable subscription received while switching to "
           "background, returning to interactable");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeConnectionCoordinator::TransitionToConnectedBackground(
    const RobotCompMap& dependentComps) {
  SET_STATE(ConnectedBackground);

  if (_subscriptionRecords.empty()) {
    // Reset the timeout in case the last unsubscribe was a while ago. This
    // ensures a minimum Background time of kBackgroundConnectionTimeout_s
    // before disconnecting
    _timeToDisconnect_s =
        BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() +
        kBackgroundConnectionTimeout_s;
  }

  // Notify subscribers
  for (auto& subscriberRecord : _subscriptionRecords) {
    subscriberRecord.subscriber->ConnectedCallback(
        CubeConnectionType::Background);
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeConnectionCoordinator::RequestConnection(
    const RobotCompMap& dependentComps) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeConnectionCoordinator::RequestDisconnect(
    const RobotCompMap& dependentComps) {
  ...
  DASMSG_SET(s1, _lastSubscriber,
             "Last subscriber before current connection closed");
  DASMSG_SET(i1, std::round(connectionDuration_s),
             "Connection duration in seconds");
  DASMSG_SEND();
  ...
  cubeComms.RequestDisconnectFromCube(kDisconnectGracePeriodSec,
                                      disconnectCallback);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeConnectionCoordinator::HandleConnectionAttemptResult(
    const RobotCompMap& dependentComps) {
  if (!_connectionAttemptSucceeded) {
    PRINT_NAMED_WARNING(
        "CubeConnectionCoordinator.ConnectionFailed",
        "CubeCommsComponent failed to establish a cube connection.");

    // The Connection attempt failed. Clear out the subscription record so that
    // further connection attempts are made only if subscribers drive them.
    std::set<ICubeConnectionSubscriber*> subscribersRemovedThisTick;
    for (auto& subscriberRecord : _subscriptionRecords) {
      subscribersRemovedThisTick.insert(subscriberRecord.subscriber);
    }
    ...
    // Notify former subscribers of the failed attempt after clearing the record
    // so that re-subscribes aren't immediately thrown out.
    for (auto* subscriber : subscribersRemovedThisTick) {
    ...
    ANKI_VERIFY(nullptr != _connectedBlock,
                ...
                "Block world did not have a connected active object for "
                "CubeComms connected cube activeID");
    if (_nonBackgroundSubscriberCount == 0) {
      TransitionToConnectedBackground(dependentComps);
    } else {
      TransitionToConnectedInteractable(dependentComps);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool CubeConnectionCoordinator::FindRecordBySubscriber(
    ICubeConnectionSubscriber* const subscriber,
    ...
    if (nullptr != webService) {
      auto onData =
          [this](const Json::Value& data,
            ...
            const auto& subscribeTempInteractable =
                data["subscribeTempInteractable"];
            if (subscribeTempInteractable.isBool()) {
              SubscribeToCubeConnection(&sStaticTestSubscriber2, false,
                                        kDebugTempSubscriptionTimeout_s);
            }
            ...
            const auto& subscribeTempBackground =
                data["subscribeTempBackground"];
            if (subscribeTempBackground.isBool()) {
              SubscribeToCubeConnection(&sStaticTestSubscriber4, true,
                                        kDebugTempSubscriptionTimeout_s);
            }
          };
...
void CubeConnectionCoordinator::SendDataToWebViz() {
  ...
  if (nullptr != _context) {
    auto* webService = _context->GetWebService();
    if (nullptr != webService &&
        webService->IsWebVizClientSubscribed(kWebVizModuleNameCubes)) {
      ...
      if ((ECoordinatorState::ConnectedInteractable == _coordinatorState) &&
          (_timeToEndStandby_s != 0)) {
        Json::Value blob;
        blob["SwitchingToBackgroundIn"] =
            (int)(_timeToEndStandby_s - currentTime_s);
        stateCountdown.append(blob);
      } else if ((ECoordinatorState::ConnectedBackground ==
                  _coordinatorState) &&
                 (_timeToDisconnect_s != 0)) {
        Json::Value blob;
      ...
      for (const auto& record : _subscriptionRecords) {
        ...
        subscriberDatum["SubscriberName"] =
            record.subscriber->GetCubeConnectionDebugName();
        subscriberDatum["SubscriptionType"] =
            record.isBackgroundConnection ? "Background" : "Interactable";
        if (record.expirationTime_s >= 0.0f) {
          subscriberDatum["ExpiresIn"] =
              (int)(record.expirationTime_s - currentTime_s);
        }
