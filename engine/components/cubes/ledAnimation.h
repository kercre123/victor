 ...
 * Created: 03/12/2018
 *
 * Description: Generates individual keyframe sequences from CubeLight
 *'animations'
 *
...
class LedAnimation {
 public:
  ...
  // Get the index of the starting keyframe of the animation (always the first
  // element)
  int GetStartingIndex() const { return _baseIndex; }

  // Get the index of the final keyframe of the animation (always the last
  // element)
  int GetFinalIndex() const {
    return _baseIndex + static_cast<int>(_keyframes.size() - 1);
  }

  // Get a reference to the underlying keyframes
  const std::vector<CubeLightKeyframe>& GetKeyframes() const {
    return _keyframes;
  }
