...
class CubeConnectionCoordinator
 ...
 public:
  ...
  // Note that subscriptions do not expire by default. Subscriptions with
  // expireIn_s == 0 expire immediately, so they will trigger connections if
  // unconnected, or reset appropriate timeouts if currently connected.
  ...
  bool UnsubscribeFromCubeConnection(
      ICubeConnectionSubscriber* const subscriber);

  bool IsConnectedToCube() {
    return (ECoordinatorState::ConnectedInteractable == _coordinatorState ||
            ECoordinatorState::ConnectedBackground == _coordinatorState ||
            ECoordinatorState::ConnectedSwitchingToBackground ==
                _coordinatorState);
  }

  bool IsConnectedInteractable() {
    return (ECoordinatorState::ConnectedInteractable == _coordinatorState ||
            ECoordinatorState::ConnectedSwitchingToBackground ==
                _coordinatorState);
  }

  bool IsConnectedBackground() {
    return ECoordinatorState::ConnectedBackground == _coordinatorState;
  }

  // Returns the ActiveID of the currently-connected cube from the
  // cubeCommsComponent, or ObservableObject::InvalidActiveID if there is no
  // connected cube
  ActiveID GetConnectedCubeActiveId() const { return _connectedCubeActiveID; }

  // Returns the ActiveObject corresponding to the _connectedCubeActiveID
  // returned from the CubeCommsComponent, or nullptr if there is no such object
 ...
 private:
  ...
  struct SubscriberRecord {
    ...
    // Define comparison to enable storage in std::set. Values will be ordered
    // smallest to largest expiration time, with permanent subscriptions stored
    // at the end. Items with expireIn_s == 0 will expire on the first update
    // after connection, or the next update if currently connected
  ...
  bool FindRecordBySubscriber(ICubeConnectionSubscriber* const subscriber,
