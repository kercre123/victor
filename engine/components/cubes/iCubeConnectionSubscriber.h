 ...
 * Created: 6/20/18
 *
 * Description: Interface class for use by the CubeConnectionCoordinator for
 *observer-pattern style tracking of subscribers. Classes which use the
 *CubeConnectionCoordinator to subscribe to cube connections should implement
 *this interface
 *
...
class ICubeConnectionSubscriber {
 public:
  ...
  // 2. An existing connection finishes converting either direction
  ...
  // Subscribers must implement this function for visibility, even if the
  // implementation is empty. The cubeConnectionCoordinator will notify
  // subscribers then dump all subscriptions in the event that the connection
  // drops unexpectedly. This prevents preservation of invalid state, but means
  // subscribers must re-subscribe to trigger a new connection attempt.
...
class TestCubeConnectionSubscriber : public ICubeConnectionSubscriber {
 public:
  ...
  virtual std::string GetCubeConnectionDebugName() const override {
    return ("TestSubscriber" + std::to_string(_id));
  };
  ...
  virtual void ConnectionFailedCallback() override {
    PRINT_NAMED_INFO("CubeConnectionCoordinatorTest.ConnectionFailed",
                     "Connection failed callback received");
  }
  virtual void ConnectionLostCallback() override {
    PRINT_NAMED_INFO("CubeConnectionCoordinatorTest.ConnectionLost",
