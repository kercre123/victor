...
#define SET_STATE(s)                                                       \
  do {                                                                     \
    ...
    _debugStateString = #s;                                                \
    SendDataToWebViz();                                                    \
    LOG_INFO("CubeInteractionTracker.SetState", "Tracker state set to %s", \
             #s);                                                          \
  } while (0);
...
static const float kTargetMovedFarDist_mm =
    100.0f;  // If we see the target move this far, its held.
...
static constexpr float kFramesToHeld_idle =
    1.5;  // Contiguous frames WITH detected movement to flip to HELD
static constexpr float kFramesToNotHeld_idle =
    30;  // Contiguous frames WITHOUT detected movement to flip to NOT HELD
// -- Visual Tracking --
...
// -- Computed Params --
static constexpr float kFilterIncrement_idle =
    kHeldProbabilityThreshold / kFramesToHeld_idle;
static constexpr float kFilterDecrement_idle =
    (1.0f - kNotHeldProbabilityThreshold) / kFramesToNotHeld_idle;
static constexpr float kFilterIncrement_tracking =
    kHeldProbabilityThreshold / kFramesToHeld_tracking;
static constexpr float kFilterDecrement_tracking =
    (1.0f - kNotHeldProbabilityThreshold) / kFramesToNotHeld_tracking;
static constexpr float kFilterIncrement_connected =
    kHeldProbabilityThreshold / kFramesToHeld_connected;
static constexpr float kFilterDecrement_connected =
    (1.0f - kNotHeldProbabilityThreshold) / kFramesToNotHeld_connected;
...
CubeInteractionTracker::CubeInteractionTracker()
    : IDependencyManagedComponent(this,
                                  RobotComponentID::CubeInteractionTracker),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeInteractionTracker::GetInitDependencies(
    RobotCompIDSet& dependencies) const {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeInteractionTracker::InitDependent(Robot* robot,
  ...
  if (ANKI_VERIFY(_robot->HasExternalInterface(),
                  ...
                  "")) {
    auto callback = std::bind(&CubeInteractionTracker::HandleEngineEvents, this,
                              std::placeholders::_1);
    auto* extInterface = _robot->GetExternalInterface();
    _signalHandles.push_back(extInterface->Subscribe(
        ExternalInterface::MessageEngineToGameTag::ObjectTapped, callback));
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeInteractionTracker::GetUpdateDependencies(
    RobotCompIDSet& dependencies) const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeInteractionTracker::UpdateDependent(
    const RobotCompMap& dependentComps) {
  ...
  switch (_trackerState) {
    ...
    case ECITState::TrackingConnected: {
      ...
      // Within connected tracking, DetectMarkers at high rate as we approach
      // thinking the user is holding the cube
      if (_trackingAtHighRate &&
          (_targetStatus.probabilityIsHeld < kConnectedLowRateConfidence)) {
        _trackingAtHighRate = false;
        auto& vsm = dependentComps.GetComponent<VisionScheduleMediator>();
        vsm.SetVisionModeSubscriptions(
            this, {{VisionMode::Markers, EVisionUpdateFrequency::High}});
      } else if (!_trackingAtHighRate && (_targetStatus.probabilityIsHeld >
                                          kConnectedHighRateConfidence)) {
      ...
      if (ccc.IsConnectedInteractable() && !_targetStatus.isHeld) {
        if (_tapDetectedDuringTick) {
          // Subscribe to the connection first so the CCC can work out state
          // properly in case we were in the process of disconnecting
  ...
  // Clear this var in case we don't want to be handling taps right now, so we
  // don't "handle" them later...
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeInteractionTracker::UpdateIsHeldEstimate(
    const RobotCompMap& dependentComps) {
  ...
  // If the cube moved far from where it was last seen, directly modify held
  // probability
  if (_targetStatus.movedFarThisFrame) {
    _targetStatus.probabilityIsHeld = Anki::Util::Max(
        kTargetMovedFarHeldConfidence, _targetStatus.probabilityIsHeld);
  } else {
    suspectHeldThisFrame = _targetStatus.movedThisFrame;

    // Update a bi-stable, filtered, multi-frame estimate of the probability
    // that the cube is currently being held
    float probHeldDelta =
        suspectHeldThisFrame ? _filterIncrement : -_filterDecrement;
    _targetStatus.probabilityIsHeld = Anki::Util::Clamp(
        _targetStatus.probabilityIsHeld + probHeldDelta, 0.0f, 1.0f);
  }

  if (!_targetStatus.isHeld &&
      (_targetStatus.probabilityIsHeld > kHeldProbabilityThreshold)) {
    ...
    // If the user is holding a connected cube, maintain the connection state
    // until they put it down
  ...
  } else if (_targetStatus.isHeld &&
             (_targetStatus.probabilityIsHeld < kNotHeldProbabilityThreshold)) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeInteractionTracker::TransitionToIdle(
    const RobotCompMap& dependentComps) {
  SET_STATE(Idle);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeInteractionTracker::TransitionToTrackingConnected(
    const RobotCompMap& dependentComps) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeInteractionTracker::TransitionToTrackingUnconnected(
    const RobotCompMap& dependentComps) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeInteractionTracker::UpdateTargetStatus(
    const RobotCompMap& dependentComps) {
...
bool CubeInteractionTracker::SelectTarget(const RobotCompMap& dependentComps) {
  ...
  if (ccc.IsConnectedToCube()) {
    ...
    ANKI_VERIFY(nullptr != _targetStatus.activeObject,
                ...
                "CubeConnectionCoordinator reported connected to cube, but had "
                "no valid active object");

    // Check whether we know where the target is, or just that its connected
    _targetStatus.observableObject =
        blockWorld.GetLocatedObjectByID(_targetStatus.activeObject->GetID());
  } else {
    // We're not connected...
    ...
    _targetStatus.observableObject =
        blockWorld.FindMostRecentlyObservedObject(*_targetFilter);
  }

  if ((nullptr == _targetStatus.activeObject) &&
      (nullptr == _targetStatus.observableObject)) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeInteractionTracker::UpdateTargetVisibility(
    const RobotCompMap& dependentcomps) {
  if (nullptr != _targetStatus.observableObject &&
      _targetStatus.observableObject->GetLastObservedTime() >=
    ...
    float timeSinceSeen_s =
        _currentTimeThisTick_s - _targetStatus.lastObservedTime_s;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeInteractionTracker::UpdateTargetMotion(
    const RobotCompMap& dependentComps) {
  ...
  if (carryingComp.IsCarryingObject(GetTargetID())) {
    ...
    if (_targetStatus.visibleThisFrame) {
      ...
      // Ignore the first position update after being carried by the robot as
      // far as being held goes. This allows the robot to update the pose of
      // something it just set down without triggering as held.
      if (_targetStatus.observedSinceMovedByRobot) {
        Pose3d relMovementPose;
        // if the old pose can't be compared, assume no movement. This should
        // cover delocalization.
        if (currentPose.GetWithRespectTo(_targetStatus.prevPose,
                                         relMovementPose)) {
          Radians angleThreshold;
          angleThreshold.setDegrees(kTargetUnmovedAngle_deg);
          if (!currentPose.IsSameAs(_targetStatus.prevPose,
                                    kTargetUnmovedDist_mm, angleThreshold)) {
            targetHasMoved = true;

            // Check if the target has "moved far" since last seen
            // If the target has moved far from where we last saw it, ramp up
            // the probability that its currently held by the user
            ...
            if (moved_mm >= kTargetMovedFarDist_mm) {
              ...
              LOG_INFO("CubeInteractionTracker.TargetMovedFar",
                       "Target moved %f mm since last seen. Assuming currently "
                       "held.",
                       moved_mm);
            }
          }
        ...
        // We've discarded one pose update since the robot moved the cube.
        // Resume normal tracking next frame.
  ...
  // This is mostly tracked for debug purposes, but could be used elsewhere if
  // interested
  if (_targetStatus.movedFarRecently && !_targetStatus.movedFarThisFrame) {
    float timeSinceMovedFar =
        _currentTimeThisTick_s - _targetStatus.lastMovedFarTime_s;
    _targetStatus.movedFarRecently =
        timeSinceMovedFar < kRecentlyMovedFarLimit_s;
  }
  ...
  if (targetHasMoved) {
    ...
    _targetStatus.movedThisFrame = false;
    float timeSinceMoved_s =
        _currentTimeThisTick_s - _targetStatus.lastMovedTime_s;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeInteractionTracker::UpdateTargetAngleFromRobotFwd(
    const RobotCompMap& dependentComps) {
  ...
  _targetStatus.observableObject->GetPose().GetWithRespectTo(_robot->GetPose(),
                                                             targetWRTRobot);
  Pose2d robotToTargetFlat(targetWRTRobot);
  Radians angleToTarget(
      atan2f(robotToTargetFlat.GetY(), robotToTargetFlat.GetX()));
  _targetStatus.angleFromRobotFwd_deg = angleToTarget.getDegrees();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeInteractionTracker::UpdateTargetDistance(
    const RobotCompMap& dependentComps) {
  _targetStatus.distMeasuredWithProx = false;

  if (!_targetStatus.visibleRecently ||
      (nullptr == _targetStatus.observableObject)) {
    // We could possibly update distance from Prox if it hasn't been very long
    // since we last saw the target. BUT... if we're connected and deloc, the
    // ObservableObject* will be null, so we have to watch out for that
    return;
  }
  ...
  if (kUseProxForDistance) {
    ...
    if (!_targetStatus.observableObject->GetPose().GetWithRespectTo(
            robotPose, cubePoseWrtRobot)) {
      targetPotentiallyObservable = false;
    }

    // Sensor beam goes along its x axis, so the distance away is simply the
    // pose's x value For the the cube to have some chance of being reliably
    // picked up by the prox sensor, some part of it should should cross the
    // sensor beam, so we can check the Y value of cubePoseWrtRobot
    const f32 cubeHalfWidth_mm =
        _targetStatus.observableObject->GetSize().y() * .5f;
    if (!IN_RANGE(cubePoseWrtRobot.GetTranslation().x(), 0.f,
                  kMaxProxTrackingRange_mm) ||
        abs(cubePoseWrtRobot.GetTranslation().y()) > cubeHalfWidth_mm) {
      targetPotentiallyObservable = false;
    }

    // If we can see the cube (or have seen it recently enough) to verify its in
    // prox range, use prox
  ...
  // If not using prox, we must have seen the target this frame to have any
  // useful information
  if (!usingProx && _targetStatus.visibleThisFrame) {
    ...
    _targetStatus.observableObject->GetClosestMarkerPose(robotPose, true,
                                                         closestMarkerPose);
    _targetStatus.distance_mm =
        Point2f(closestMarkerPose.GetTranslation()).Length();
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CubeInteractionTracker::HandleEngineEvents(
  ...
  switch (event.GetData().GetTag()) {
    ...
    default: {
      PRINT_NAMED_ERROR(
          "CubeInteractionTracker.HandleEngineEvents.UnhandledEvent",
  ...
  if (_robot != nullptr) {
    ...
    if (nullptr != context) {
      if (auto webSender = WebService::WebVizSender::CreateWebVizSender(
        ...
        citInfo["timeSinceHeld"] =
            _currentTimeThisTick_s - _targetStatus.lastHeldTime_s;
        citInfo["timeSinceMoved"] =
            _currentTimeThisTick_s - _targetStatus.lastMovedTime_s;
        citInfo["timeSinceObserved"] =
            _currentTimeThisTick_s - _targetStatus.lastObservedTime_s;
        citInfo["timeSinceTapped"] =
            _currentTimeThisTick_s - _targetStatus.lastTappedTime_s;
