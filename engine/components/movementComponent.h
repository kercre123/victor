...
class MovementComponent : public IDependencyManagedComponent<RobotComponentID>,
 ...
 public:
  ...
  virtual void AdditionalInitAccessibleComponents(
      std::set<RobotComponentID>& components) const override {
  ...
  // - Cozmo is turning one direction and you turn him faster so he overshoots
  // his turn angle
  // - Cozmo is stuck on an object and is unable to turn
  void CheckForUnexpectedMovement(const RobotState& robotState);

  bool IsUnexpectedMovementDetected() const {
    return _unexpectedMovement.IsDetected();
  }

  UnexpectedMovementSide GetUnexpectedMovementSide() const {
    return IsUnexpectedMovementDetected() ? _unexpectedMovementSide
                                          : UnexpectedMovementSide::UNKNOWN;
  }

  u8 GetUnexpectedMovementCount() const {
    return _unexpectedMovement.GetCount();
  }

  // True if any motor speed (head, left, or wheels) is non-zero in most recent
  // RobotState message
  ...
  // - If we fail finding the state in history, all methods return TRUE (to be
  // conservative)
  // - These are non-const because they can insert things into pose history due
  // to interpolation
  ...
  // Retrieves the timestamp at which one or more motors was last tracked as
  // moving. If no robot state has ever been received indicating movement, this
  // will return a timestamp of 0!
  RobotTimeStamp_t GetLastTimeWasMoving() const { return _lastTimeWasMoving; }
  RobotTimeStamp_t GetLastTimeHeadWasMoving() const {
    return _lastTimeHeadWasMoving;
  }
  RobotTimeStamp_t GetLastTimeLiftWasMoving() const {
    return _lastTimeLiftWasMoving;
  }
  RobotTimeStamp_t GetLastTimeWheelsWereMoving() const {
    return _lastTimeWheelsWereMoving;
  }

  // Convenience methods for checking head OR wheels, since either moves the
  // camera
  bool IsCameraMoving() const { return _isHeadMoving || _areWheelsMoving; }
  bool WasCameraMoving(
      RobotTimeStamp_t atTime);  // Slightly more efficient than calling
                                 // WasHeadMoving _and_ WereWheelsMoving
  ...
  // In order to unlock tracks, the unlock 'who' needs to match the 'who' that
  // did the locking
  void LockTracks(u8 tracks, const std::string& who,
                  const std::string& debugName);
  // Returns true if there are any locks on tracks after unlocking tracks locked
  // by 'who'
  bool UnlockTracks(u8 tracks, const std::string& who);

  // Converts int who to a string (used to easily allow actions to lock tracks
  // with their tag)
  void LockTracks(u8 tracks, const int who, const std::string& debugName) {
    LockTracks(tracks, std::to_string(who), debugName);
  }
  bool UnlockTracks(u8 tracks, const int who) {
    return UnlockTracks(tracks, std::to_string(who));
  }

  // Notify the movement component that updates should be applied up to and
  // including the specified relative stream time If shouldClearUnusedLocks is
  // true any locks not applied on this function call will be dropped
  void ApplyUpdatesForStreamTime(const TimeStamp_t relativeStreamTime_ms,
                                 const bool shouldClearUnusedLocks = true);

  // Occassionally we want to lock/unlock a track partway through an animation -
  // calling this function will cause the locking/unlocking operation to happen
  // at the timestamp specified in the currently streaming animation NOTE: While
  // the lock/unlock will be appropriately applied in the animation streamer,
  // there may be delays between when the face is locked as visible to the user
  // and when the functions that return locked tracks above are updated to
  // reflect the true robot state
  void LockTracksAtStreamTime(const u8 tracks,
                              const TimeStamp_t relativeStreamTime_ms,
                              const bool applyBeforeTick,
                              const std::string& who,
                              const std::string& debugName);
  void UnlockTracksAtStreamTime(const u8 tracks,
                                const TimeStamp_t relativeStreamTime_ms,
                                const bool applyBeforeTick,
                                const std::string& who);
  ...
  Result MoveLiftToAngle(const f32 angle_rad, const f32 max_speed_rad_per_sec,
  ...
  // When the command is received by the robot, it returns a MotorActionAck
  // message with the ID that can optionally be written to actionID_out.
  Result MoveLiftToHeight(const f32 height_mm, const f32 max_speed_rad_per_sec,
                          const f32 accel_rad_per_sec2,
                          const f32 duration_sec = 0.f,
                          MotorActionID* actionID_out = nullptr);
  ...
  // When the command is received by the robot, it returns a MotorActionAck
  // message with the ID that can optionally be written to actionID_out.
  Result MoveHeadToAngle(const f32 angle_rad, const f32 max_speed_rad_per_sec,
                         const f32 accel_rad_per_sec2,
                         const f32 duration_sec = 0.f,
                         MotorActionID* actionID_out = nullptr);
  ...
  // unless you have a need to run the wheels for an indeterminate amount of
  // time.
  Result DriveWheels(const f32 lWheel_speed_mmps, const f32 rWheel_speed_mmps,
  ...
  // message with the ID that can optionally be written to actionID_out.
  Result TurnInPlace(
      const f32 angle_rad,  // Absolute angle the robot should achieve at the
                            // end of the turn.
      const f32 max_speed_rad_per_sec,  // Maximum speed of the turn (sign
      ...
      const u16 num_half_revolutions,  // Used for turns greater than 180
                                       // degrees (e.g. "turn 720 degrees")
      bool use_shortest_direction,  // If true, robot should take the shortest
                                    // path to angle_rad and disregard the sign
                                    // of max_speed and num_half_revolutions. If
                                    // false, consider sign of max_speed and
                                    // num_half_revolutions to determine which
                                    // direction to turn and how far to turn
                                    // (e.g. for turns longer than 360 deg)
      MotorActionID* actionID_out = nullptr);

  // Flag the eye shift layer with the given name for removal next time head
  // moves. You may optionally specify the duration of the layer removal (i.e.
  // how long it takes to return to not making any face adjustment)
  void RemoveEyeShiftWhenHeadMoves(const std::string& name,
                                   TimeStamp_t duration_ms = 0);
  ...
  bool IsDirectDriving() const {
    return ((_drivingWheels || _drivingHead || _drivingLift));
  }

  u8 GetMaxUnexpectedMovementCount() const {
    return _unexpectedMovement.GetMaxCount();
  }

  // Call this if you would like to allow 'external' movement commands (e.g. raw
  // DriveWheels command). These should really only be allowed for things like
  // SDK and Webots. The requester name is used to track where the request came
  // from so that commands can be dis-allowed when no one needs them anymore.
  void AllowExternalMovementCommands(const bool enable,
                                     const std::string& requester);

  // Enable/disable detection of rotation without motors. This must be
  // explicitly enabled since it differs from the most common use case of this
  // component.
  void EnableUnexpectedRotationWithoutMotors(bool enabled) {
    _enableRotatedWithoutMotors = enabled;
  }
  bool IsUnexpectedRotationWithoutMotorsEnabled() const {
    return _enableRotatedWithoutMotors;
  }

  // Enable/disable detection of unexpected movement even when picked up, and
  // clamps the max angular velocity of any point-turn to the value specified by
  // kMaxHeldInPalmTurnSpeed_radps. Must be explicitly enabled by the
  // HeldInPalmTracker component since that component enables behaviors with
  // point-turn actions to run even when the robot is picked up.
  void EnableHeldInPalmMode(const bool enabled);
  bool IsHeldInPalmModeEnabled() const { return _heldInPalmModeEnabled; }

  f32 GetMaxTurnSpeedWhileHeldInPalm_radps() const {
    return kMaxHeldInPalmTurnSpeed_radps;
  }
  ...
  // detected.) This happens normally if the motors are not actively being
  // driven.
  bool AreEncodersDisabled() const { return _areEncodersDisabled; }

  // Whether or not the head was detected to have moved while the encoders were
  // "disabled" i.e. Calibration is necessary!
  bool IsHeadEncoderInvalid() const { return _isHeadEncoderInvalid; }

  // Whether or not the lift was detected to have moved while the encoders were
  // "disabled" i.e. Calibration is necessary!
  bool IsLiftEncoderInvalid() const { return _isLiftEncoderInvalid; }

 private:
  // Record a notification from the animation process that a set of tracks has
  // locked. This is a work-around for the huge mess that is BlackJack face
  // lock/unlocking and should NOT be used for anything else. Ideally it will be
  // removed soon during animStreamer refactoring.
  ...
  // Checks if the speed is near zero and if it is sets flag to false and
  // unlocks tracks otherwise it will set flag to true and lock the tracks if
  // they are not locked
  ...
  // Processes all currently locked tracks in addition to all delayed
  // locks/unlocks in order to get the full enumeration of tracks that will be
  // locked at a given point in time
  u8 GetTracksLockedAtRelativeStreamTime(
  ...
  // True if we are currently allowed to handle raw motion commands from the
  // outside world (e.g. while SDK or Webots is active)
  bool _allowExternalMovementCommands = false;

  // Keep track of who requested to allow external movement commands (e.g. "sdk"
  // or "ui")
  ...
  class UnexpectedMovement {
   ...
   public:
    ...
    void Increment(u8 countInc, f32 leftSpeed_mmps, f32 rightSpeed_mmps,
                   RobotTimeStamp_t currentTime);
    void Decrement();
    void Reset();
    void EnableHeldInPalmMode(const bool enable) {
      _heldInPalmModeEnabled = enable;
    }
  };

  UnexpectedMovement _unexpectedMovement;

  UnexpectedMovementSide _unexpectedMovementSide =
      UnexpectedMovementSide::UNKNOWN;
  ...
  // Flags for whether or not we are currently directly driving the following
  // motors
  ...
  struct DelayedTrackLockInfo {
    DelayedTrackLockInfo(u8 tracksToLock, const std::string& who)
        : tracksToLock(tracksToLock), who(who) {}
    u8 tracksToLock;
    std::string who;
  };

  std::array<std::multiset<LockInfo>, (size_t)AnimConstants::NUM_TRACKS>
      _trackLockCount;

  // maps relative stream timestamps to information about how to lock tracks
  // when that timestamp is reached
