...
class VisionComponent : public IDependencyManagedComponent<RobotComponentID>,
 ...
 public:
  ...
  virtual void GetUpdateDependencies(
    ...
    // Applies the scheduling consequences of the last frame's subscriptions
    // before ticking VisionComponent
    dependencies.insert(RobotComponentID::VisionScheduleMediator);
  };
  ...
  // In asynchronous mode, it will be processed as soon as VisionSystem thread
  // is ticked. Will Release and Invalidate buffer should this fail
  Result SetNextImage(Vision::ImageBuffer& buffer);

  // Pause/Unpause processing of images
  // Note: Will still capture images from CameraService but they will be
  // discarded
  ...
  // Looks through all results available from the VisionSystem and processes
  // them. This updates the Robot's BlockWorld and FaceWorld using those
  // results.
  ...
  Result SendCompressedImage(const Vision::CompressedImage& img,
  ...
  void GetMarkerDetectionTurnSpeedThresholds(
      f32& bodyTurnSpeedThresh_degPerSec,
  ...
  bool WillStoreNextImageForCameraCalibration() const {
    return _storeNextImageForCalibration;
  }
  size_t GetNumStoredCameraCalibrationImages() const;

  // Get jpeg compressed data of calibration images
  // dotsFoundMask has bits set to 1 wherever the corresponding calibration
  // image was found to contain a valid target
  std::list<std::vector<u8>> GetCalibrationImageJpegData(
      u8* dotsFoundMask = nullptr) const;

  // Get the specified calibration pose to the robot. 'whichPose' must be
  // [0,numCalibrationimages].
  ...
  void EnableComputingCameraCalibration(bool enable) {
    EnableMode(VisionMode::Calibration, enable);
  }
  ...
  // Called automatically when doFactoryDotTest=true. Exposed publicly for unit
  // test
  Result FindFactoryTestDotCentroids(
  ...
  // Links a name with a face ID and sets up the robot's ability to say that
  // name. Then merges the newly-named ID into mergeWithID, if set to a valid
  // existing ID (which should already be named!)
  ...
  // On success, a RobotLoadedKnownFace message with the new ID/name pairing is
  // broadcast.
  ...
  // Generally there will only be one entry, but its possible we may support
  // multiple enrollments with the same name.
  ...
  Result LoadFaceAlbumFromFile(
      const std::string& path);  // Broadcasts any loaded names and IDs
  Result LoadFaceAlbumFromFile(
      const std::string& path,
      std::list<Vision::LoadedKnownFace>&
          loadedFaces);    // Populates list, does not broadcast
  Result SaveFaceAlbum();  // use album path specified in vision_config.json
  Result LoadFaceAlbum();  // use album path specified in vision_config.json,
                           // broadcast loaded names and IDs

  // NOTE: if params.path is empty, it will default to <cachePath>/camera/images
  // (where cachePath comes from DataPlatform)
  ...
  void SetMirrorModeDisplayString(const std::string& str,
  ...
  // Enable/disable broadcasting proto image chunks to the SDK. Set resolution
  // of images broadcasted to the Viz.
  void EnableSendingSDKImageChunks(bool enableImageStreaming,
                                   bool enableHighResolutionImages);
  ...
  IlluminationState GetLastIlluminationState() const {
    return _lastIlluminationState;
  }
 ...
 protected:
  ...
  // variables which are updated in the main (engine) thread but checked on the
  // worker thread must be volatile to avoid potential caching issues
  ...
  // While _visionSystemInput.locked is true this is being processed by
  // VisionSystem and can not be modified by VisionComponent
  VisionSystemInput _visionSystemInput = {};

  // This mutex/condition_variable pair is used for the main thread to signal
  // the Processor thread that an image is ready to be processed (or we are
  // shutting down)
  ...
  f32 _markerDetectionBodyTurnSpeedThreshold_radPerSec =
      kDefaultBodySpeedThresh;
  f32 _markerDetectionHeadTurnSpeedThreshold_radPerSec =
      kDefaultHeadSpeedThresh;
  ...
  Vision::ImageQuality _lastBroadcastImageQuality =
      Vision::ImageQuality::Unchecked;
  ...
  Vision::ImageEncoding _desiredImageFormat =
      Vision::ImageEncoding::NoneImageEncoding;

  // State machine to make sure nothing is using the shared memory from the
  // camera system before we request a different camera capture format as well
  // as to wait until we get a frame from the camera after changing formats
  // before unpausing vision component
  ...
  std::map<f32, Homography>
      _groundPlaneHomographyLUT;  // keyed on head angle in radians

  // Factory centroid finder: returns the centroids of the 4 factory test dots,
  // computes camera pose w.r.t. the target and broadcasts a
  // RobotCompletedFactoryDotTest message. This runs on the main thread and
  // should only be used for factory tests. Is run automatically when
  // _doFactoryDotTest=true and sets it back to false when done.
  ...
  // Time at which we attempted to restart the camera as we had not received a
  // valid image for some amount of time
  EngineTimeStamp_t _restartingCameraTime_ms = 0;

  IlluminationState _lastIlluminationState = IlluminationState::Unknown;

#if REMOTE_CONSOLE_ENABLED
  // Array of pairs of ConsoleVars and their associated values used for toggling
  // VisionModes
  using VisionModeConsoleVarPair = std::pair<Util::ConsoleVar<bool>*, bool>;
  std::array<VisionModeConsoleVarPair, static_cast<u32>(VisionMode::Count)>
      _visionModeConsoleVars;
#endif

  void ReadVisionConfig(const Json::Value& config);
  void PopulateGroundPlaneHomographyLUT(
      f32 angleResolution_rad = DEG_TO_RAD(0.25f));
  ...
  void BroadcastLoadedNamesAndIDs(
...
inline void VisionComponent::Pause(bool isPaused) {
  ...
  PRINT_CH_INFO("VisionComponent", "VisionComponent.Pause.Set",
                "Setting Paused from %d to %d", _paused, isPaused);
  _paused = isPaused;
}
...
inline Vision::Camera& VisionComponent::GetCamera(void) { return *_camera; }

inline const std::shared_ptr<Vision::CameraCalibration>
VisionComponent::GetCameraCalibration() const {
...
inline void VisionComponent::SetMarkerDetectionTurnSpeedThresholds(
    f32 bodyTurnSpeedThresh_degPerSec, f32 headTurnSpeedThresh_degPerSec) {
  if (bodyTurnSpeedThresh_degPerSec < 0) {
    _markerDetectionBodyTurnSpeedThreshold_radPerSec = kDefaultBodySpeedThresh;
  } else {
    _markerDetectionBodyTurnSpeedThreshold_radPerSec =
        DEG_TO_RAD(bodyTurnSpeedThresh_degPerSec);
  }

  if (headTurnSpeedThresh_degPerSec < 0) {
    _markerDetectionHeadTurnSpeedThreshold_radPerSec = kDefaultHeadSpeedThresh;
  } else {
    _markerDetectionHeadTurnSpeedThreshold_radPerSec =
        DEG_TO_RAD(headTurnSpeedThresh_degPerSec);
  }
}

inline void VisionComponent::GetMarkerDetectionTurnSpeedThresholds(
    f32& bodyTurnSpeedThresh_degPerSec,
    f32& headTurnSpeedThresh_degPerSec) const {
  bodyTurnSpeedThresh_degPerSec =
      RAD_TO_DEG(_markerDetectionBodyTurnSpeedThreshold_radPerSec);
  headTurnSpeedThresh_degPerSec =
...
inline void VisionComponent::SetMirrorModeDisplayString(
    const std::string& str, const ColorRGBA& color) {
