...
const float kDasSendPeriod_sec =
    60.f * 60.f * 24.f;  // send to DAS every 24 hours
}  // namespace
...
BatteryStats::~BatteryStats() { LogToDas(); }

void BatteryStats::Update(const float batteryTemp_degC,
  ...
  if ((_lastSampleTime_sec == 0.f) ||
    ...
    *_temperatureStats_degC += batteryTemp_degC;
    // Ignore voltage readings near zero (processes may have started while
    // battery was 'disconnected' and thus reporting 0 volts)
...
void BatteryStats::UpdateEncoderStats(bool encodersDisabled, bool calmMode) {
  if (calmMode) {
    ...
    // Approximation! In calm mode, one RobotState message is received for every
    // 8 that normally would be so increase the total sample count since this is
    // our approximate measure of time.
...
void BatteryStats::LogToDas() {
  if (_temperatureStats_degC->GetNum() > 0) {
    DASMSG(battery_temperature_stats, "battery.temperature_stats",
           "Battery temperature statistics");
    DASMSG_SET(i1, _temperatureStats_degC->GetIntMin(),
               "Minimum battery temperature experienced (degC)");
    DASMSG_SET(i2, _temperatureStats_degC->GetIntMax(),
               "Maximum battery temperature experienced (degC)");
    DASMSG_SET(i3, _temperatureStats_degC->GetIntMean(),
               "Average battery temperature experienced (degC)");
  ...
  if (_voltageStats->GetNum() > 0) {
    ...
    DASMSG(battery_voltage_stats, "battery.voltage_stats",
           "Battery voltage statistics");
    DASMSG_SET(i1, std::round(1000.f * _voltageStats->GetMin()),
               "Minimum battery voltage experienced (mV)");
    DASMSG_SET(i2, std::round(1000.f * _voltageStats->GetMax()),
               "Maximum battery voltage experienced (mV)");
    DASMSG_SET(i3, std::round(1000.f * _voltageStats->GetMean()),
               "Average battery voltage experienced (mV)");
    DASMSG_SET(i4, _voltageStats->GetNum(), "Total number of samples");
    DASMSG_SEND();
  }

  if (_encoderStateSamples > 0) {
    DASMSG(battery_encoder_power_stats, "battery.encoder_power_stats",
           "Encoder power statistics");
    DASMSG_SET(i1, (100 * _encoderDisabledCount) / _encoderStateSamples,
               "Percentage of time encoders were disabled while off charger");
    DASMSG_SET(i2, (100 * _encoderCalmCount) / _encoderStateSamples,
               "Percentage of time encoders were disabled due to calm while "
               "off charger");
