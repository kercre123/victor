...
class BatteryComponent : public IDependencyManagedComponent<RobotComponentID>,
 ...
 public:
  ...
  // time that the battery is connected while on the charger should still be 30
  // min. (When disconnected, GetBatteryVoltsRaw() returns a low reading that
  // shouldn't be considered to represent that actual battery voltage.
  // GetBatteryVolts() always holds the last valid filtered voltage reading.)
  ...
  // NOTE: If battery is disconnected and IsCharging() == true, it means the
  // battery
  //       has actually suspended charging but will resume when the battery has
  //       cooled down.
  bool IsCharging() const { return _isCharging; }

  bool IsChargingStalledBecauseTooHot() const {
    return _isCharging && _battDisconnected;
  }
  ...
  // True if we think the robot is on a charger. This becomes true only when the
  // robot touches the charger contacts, and remains true until we think the
  // robot has driven off the charger. It will not become true based on
  // localization or observing the charger marker, only based on feeling the
  // charger. A robot on the charger contacts is always on the platform (NOTE:
  // even if it thinks it's in the air or on it's side)
  ...
  // Returns how long the provided state has been active. Returns 0 if not in
  // that state
  ...
  // Returns 0.f if battery is connected to charger circuit or if not on
  // charger. NOTE: If battery is disconnected and IsCharging() == true, it
  // means the battery
  //       has actually suspended charging but will resume when the battery has
  //       cooled down.
  float GetBatteryDisconnectedDurationSec() const;

  external_interface::GatewayWrapper GetBatteryState(
      const external_interface::BatteryStateRequest& request);

  uint8_t GetBatteryTemperature_C() const { return _battTemperature_C; }

  // If the robot is low battery, this will give a timer of how long it should
  // charge for in seconds. Once elapsed, or if the robot was not recently low
  // battery, it will return 0.0
 ...
 private:
  ...
  int GetBatteryVolts_mV() {
    return static_cast<int>(std::round(1000.f * _batteryVoltsFilt));
  }
  int GetBatteryVoltsRaw_mV() {
    return static_cast<int>(std::round(1000.f * _batteryVoltsRaw));
  }
