 ...
 * Created: 6/4/18
 *
 * Description: Controls the process of taking/storing photos and communicating
 *with the app
 *
...
// If enabled, disables all smoothing and sharpening, despite what is in Json
// config (but still uses undistortion)
CONSOLE_VAR(bool, kTakePhoto_UseRawPhotos, "Photography", false);

static int DeleteAllPhotosWebServerImpl(
...
static int DeleteAllPhotosWebServerHandler(struct mg_connection* conn,
                                           void* cbdata) {
  ...
  const int returnCode =
      ws->ProcessRequestExternal(conn, cbdata, DeleteAllPhotosWebServerImpl);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void PhotographyManager::InitDependent(Robot* robot,
  ...
  if (gi != nullptr) {
    auto commonCallback = std::bind(&PhotographyManager::HandleEvents, this,
                                    std::placeholders::_1);

    // Subscribe to desired simple events
    _signalHandles.push_back(
        gi->Subscribe(external_interface::GatewayWrapperTag::kPhotosInfoRequest,
                      commonCallback));
    _signalHandles.push_back(gi->Subscribe(
        external_interface::GatewayWrapperTag::kPhotoRequest, commonCallback));
    _signalHandles.push_back(
        gi->Subscribe(external_interface::GatewayWrapperTag::kThumbnailRequest,
                      commonCallback));
    _signalHandles.push_back(gi->Subscribe(
        external_interface::GatewayWrapperTag::kDeletePhotoRequest,
        commonCallback));
  }

  const auto& config =
      robot->GetContext()->GetDataLoader()->GetPhotographyConfig();
  kMaxSlots = JsonTools::GetValue<s32>(config["MaxSlots"]);
  kRemoveDistortion =
      JsonTools::ParseBool(config, "RemoveDistortion", kModuleName);
  kMedianFilterSize =
      JsonTools::ParseUint8(config, "MedianFilterSize", kModuleName);
  kSharpeningAmount =
      JsonTools::ParseFloat(config, "SharpeningAmount", kModuleName);
  ...
  DEV_ASSERT(_platform != nullptr,
             "PhotographyManager.InitDependent.DataPlatformIsNull");

  _savePath = _platform->pathToResource(Util::Data::Scope::Persistent,
                                        kPhotoManagerFolder);
  if (!Util::FileUtils::CreateDirectory(_savePath)) {
    LOG_ERROR("PhotographyManager.InitDependent.FailedToCreateFolder",
              "Failed to create folder %s", _savePath.c_str());
    return;
  }

  _fullPathPhotoInfoFile =
      Util::FileUtils::FullFilePath({_savePath, kPhotoManagerFilename});
  if (Util::FileUtils::FileExists(_fullPathPhotoInfoFile)) {
    if (!LoadPhotosFile()) {
      LOG_ERROR("PhotographyManager.InitDependent.FailedLoadingPhotosFile",
                "Error loading photos file");
    }
  } else {
    LOG_WARNING("PhotographyManager.InitDependent.NoPhotosFile",
                "Photos file not found; creating new one");
    SavePhotosFile();
  }

  const auto& webService = robot->GetContext()->GetWebService();
  webService->RegisterRequestHandler("/deleteallphotos",
                                     DeleteAllPhotosWebServerHandler, this);
}
...
Result PhotographyManager::EnablePhotoMode(bool enable) {
  DEV_ASSERT(nullptr != _visionComponent,
             "PhotographyManager.EnablePhotoMode.NullVisionComponent");

  if (!IsReadyToSwitchModes()) {
    // Special case: can't disable photo mode because we're waiting on something
    // else: queue to disable when ready
    if (!enable) {
      LOG_INFO("PhotographyManager.EnablePhotoMode.QueuingDisable",
               "Current State: %s", GetStateString().c_str());
  ...
  _state = (enable ? State::WaitingForPhotoModeEnable
                   : State::WaitingForPhotoModeDisable);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::string PhotographyManager::GetSavePath() const { return _savePath; }
...
PhotographyManager::PhotoHandle PhotographyManager::TakePhoto() {
  DEV_ASSERT(nullptr != _visionComponent,
             "PhotographManager.TakePhoto.NullVisionComponent");

  if (!IsReadyToTakePhoto()) {
    LOG_WARNING("PhotographyManager.TakePhoto.NotReady", "Current State: %s",
                GetStateString().c_str());
    return 0;
  }

  const uint8_t medianFilterSize =
      (kTakePhoto_UseRawPhotos ? 0 : kMedianFilterSize);
  const float sharpeningAmount =
      (kTakePhoto_UseRawPhotos ? 0.f : kSharpeningAmount);

  _visionComponent->SetSaveImageParameters(ImageSaverParams(
      GetSavePath(), ImageSendMode::SingleShot, kSaveQuality,
      GetBasename(_nextPhotoID), Vision::ImageCacheSize::Full, kThumbnailScale,
      1.f,  // saveScale
      kRemoveDistortion, medianFilterSize, sharpeningAmount));

  _lastRequestedPhotoHandle =
      (TimeStamp_t)_visionComponent->GetLastProcessedImageTimeStamp();
  _state = State::WaitingForTakePhoto;

  LOG_INFO("PhotographyManager.TakePhoto.SetSaveParams", "RequestedHandle: %zu",
           _lastRequestedPhotoHandle);
...
void PhotographyManager::SetLastPhotoTimeStamp(RobotTimeStamp_t timestamp) {
  static_assert(sizeof(PhotoHandle) >= sizeof(TimeStamp_t),
                "PhotoHandle alias must be able to accomodate TimeStamps");
  _lastSavedPhotoHandle = (TimeStamp_t)timestamp;

  LOG_INFO("PhotographyManager.SetLastPhotoTimeStamp.SettingHandle",
           "Saved Handle: %zu (Last Requested: %zu)", _lastSavedPhotoHandle,
           _lastRequestedPhotoHandle);

  if (WasPhotoTaken(_lastRequestedPhotoHandle)) {
    // Last TakePhoto call is completed, go back to InPhotoMode, meaning we are
    // ready to take more photos
    _state = State::InPhotoMode;

    // Record info about the photo in the 'photos info'
    // Note: When VIC-3649 "Add engine support for getting the current wall
    // time" is done, use it here?
    ...
    _photoInfos.push_back(PhotoInfo(_nextPhotoID, epochTimestamp,
                                    photoCopiedToApp, thumbCopiedToApp));
...
bool PhotographyManager::LoadPhotosFile() {
  ...
  if (!_platform->readAsJson(_fullPathPhotoInfoFile, data)) {
    LOG_ERROR("PhotographyManager.LoadPhotosFile.Failed", "Failed to read %s",
              _fullPathPhotoInfoFile.c_str());
    return false;
  }
  ...
  for (const auto& info : data[kPhotoInfosKey]) {
    ...
    const bool thumbCopied =
        info.isMember(kThumbCopiedKey) ? info[kThumbCopiedKey].asBool() : false;

    _photoInfos.push_back(
        PhotoInfo(id, dateTimeTaken, photoCopied, thumbCopied));
  }

  const auto numPhotos = _photoInfos.size();
  if (numPhotos > kMaxSlots) {
    LOG_WARNING("PhotographyManager.LoadPhotosFile.DeletingPhotos",
                "Removing some photos because there are now fewer slots "
                "(configuration change)");
    // Delete the oldest photo(s)
    for (int i = 0; i < numPhotos - kMaxSlots; i++) {
      // Delete the first photo in the list; DeletePhotoByID will compress the
      // list
...
void PhotographyManager::SavePhotosFile() const {
  ...
  if (!_platform->writeAsJson(_fullPathPhotoInfoFile, data)) {
    LOG_ERROR("PhotographyManager.SavePhotosFile.Failed",
              "Failed to write photos info file");
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool PhotographyManager::DeletePhotoByID(const int id,
                                         const bool savePhotosFile) {
  ...
  Util::FileUtils::DeleteFile(Util::FileUtils::FullFilePath(
      {_savePath, baseName + "." + GetPhotoExtension()}));
  Util::FileUtils::DeleteFile(Util::FileUtils::FullFilePath(
      {_savePath, baseName + "." + GetThumbExtension()}));
  ...
  LOG_INFO("PhotographyManager.DeletePhotoByID",
           "Photo with ID %i, at index %i, deleted", id, index);
  if (savePhotosFile) {
    SavePhotosFile();
  }
  return true;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void PhotographyManager::SendDASEvent(const bool succeeded,
                                      const std::string& failReason) {
  DASMSG(take_a_photo_end, "behavior.take_a_photo.end",
         "User attempted to take a photo or selfie");
  DASMSG_SET(s1, succeeded ? "Success" : "Fail",
             "Completion status (success or fail)");
  DASMSG_SET(s2, failReason, "The reason the photo attempt failed");
  DASMSG_SET(i1, _photoInfos.size(),
             "The number of photos stored, after the attempt");
  DASMSG_SEND();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool PhotographyManager::ImageHelper(const int id, const bool isThumbnail,
                                     std::string& fullpath) {
  ...
  fullpath = Util::FileUtils::FullFilePath(
      {_savePath,
       GetBasename(id) + "." +
           (isThumbnail ? GetThumbExtension() : GetPhotoExtension())});

  if (!Util::FileUtils::FileExists(fullpath)) {
    LOG_ERROR("PhotographyManager.ImageHelper", "Error finding file %s",
              fullpath.c_str());
    return false;
  }

  LOG_INFO("PhotographyManager.ImageHelper",
           "Getting %s with ID %i, at index %i, with path %s",
...
int PhotographyManager::PhotoIndexFromID(const int id) const {
  ...
  if (index >= _photoInfos.size()) {
    LOG_ERROR("PhotographyManager.PhotoIndexFromID", "Photo ID %i not found",
              id);
    return -1;
  }
  return index;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void PhotographyManager::HandleEvents(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void PhotographyManager::OnRequestPhotosInfo(
    const external_interface::PhotosInfoRequest& photosInfoRequest) {
  ...
  for (auto i = 0; i < _photoInfos.size(); i++) {
    ...
    external_interface::PhotoInfo* photoInfo =
        photosInfoResp->add_photo_infos();
  ...
  _gatewayInterface->Broadcast(
      ExternalMessageRouter::WrapResponse(photosInfoResp));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void PhotographyManager::OnRequestPhoto(
    const external_interface::PhotoRequest& photoRequest) {
  const auto photoId = photoRequest.photo_id();
  LOG_INFO("PhotographyManager.OnRequestPhoto", "Requesting photo with ID %u",
           photoId);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void PhotographyManager::OnRequestThumbnail(
    const external_interface::ThumbnailRequest& thumbnailRequest) {
  const auto photoId = thumbnailRequest.photo_id();
  LOG_INFO("PhotographyManager.OnRequestThumbnail",
           "Requesting thumbnail with ID %u", photoId);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void PhotographyManager::OnRequestDeletePhoto(
    const external_interface::DeletePhotoRequest& deletePhotoRequest) {
  const auto photoId = deletePhotoRequest.photo_id();
  LOG_INFO("PhotographyManager.OnRequestDeletePhoto",
           "Deleting photo with ID %u", photoId);
  static const bool saveChangeToDisk = true;
  const bool success = DeletePhotoByID(photoId, saveChangeToDisk);
  if (!success) {
    LOG_ERROR("PhotographyManager.OnRequestDeletePhoto",
              "Failed to delete photo with ID %u", photoId);
  }

  auto* deletePhotoResp = new external_interface::DeletePhotoResponse();
  deletePhotoResp->set_success(success);
  _gatewayInterface->Broadcast(
      ExternalMessageRouter::WrapResponse(deletePhotoResp));
}
