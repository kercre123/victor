...
// Giving this its own local define, in case we want to control it independently
// of DEV_CHEATS / SHIPPING, etc.
...
// Note: If COZMO-11199 is implemented (more user-friendly playing animations by
// name
...
static void PlayAnimationByName(ConsoleFunctionContextRef context) {
  if (_thisRobot != nullptr) {
    ...
    if (animName) {
      ...
      bool renderInEyeHue =
          ConsoleArg_GetOptional_Bool(context, "renderInEyeHue", true);
  ...
  if (_thisRobot != nullptr) {
    ...
    if (animFile) {
      const std::string animationFolder =
          _thisRobot->GetContextDataPlatform()->pathToResource(
              Anki::Util::Data::Scope::Resources, "/assets/animations/");
      ...
      if (robotDataLoader != nullptr) {
        ...
        if (animContainer != nullptr) {
          ...
          auto spriteSequenceContainer =
              _thisRobot->GetComponent<DataAccessorComponent>()
                  .GetSpriteSequenceContainer();
          std::atomic<float> loadingCompleteRatio(0);
          std::atomic<bool> abortLoad(false);
          CannedAnimationLoader animLoader(platform, spriteSequenceContainer,
                                           loadingCompleteRatio, abortLoad);

          animLoader.LoadAnimationIntoContainer(animationPath.c_str(),
                                                animContainer);
          LOG_INFO("Robot.AddAnimation", "Loaded animation from %s",
                   animationPath.c_str());
        }
      }
    }
...
CONSOLE_FUNC(PlayAnimationByName, "Animation", const char* animName,
             optional int numLoops, optional bool renderInEyeHue);
...
static void PrintBodyData(ConsoleFunctionContextRef context) {
  if (_thisRobot != nullptr) {
    ...
    const uint32_t period_tics =
        ConsoleArg_Get_UInt(context, "printPeriod_tics");
    ...
    _thisRobot->SendMessage(RobotInterface::EngineToRobot(
        RobotInterface::PrintBodyData(period_tics, motors, prox, battery)));
    LOG_INFO("Robot.PrintBodyData", "Period: %d tic, (m: %d, p: %d, b: %d)",
             period_tics, motors, prox, battery);
  }
}

CONSOLE_FUNC(PrintBodyData, "Syscon", uint32_t printPeriod_tics,
             optional bool motors, optional bool prox, optional bool battery);
...
// NOTE: Need to keep kVoiceStyles in sync with
// AudioMetaData::SwitchState::Robot_Vic_External_Processing in
...
void SayText(ConsoleFunctionContextRef context) {
  ...
  LOG_INFO("Robot.SayText",
           ...
           EnumToString(style), kDurationScalar, kPitchScalar);

  auto& ttsCoordinator = robot->GetTextToSpeechCoordinator();
  const auto triggerType = UtteranceTriggerType::Immediate;
  ttsCoordinator.CreateUtterance(textStr, triggerType, style, kDurationScalar,
                                 kPitchScalar);
}
...
// timeToConsiderOfftreads is tuned based on the fact that we have to wait half
// a second from the time the cliff sensor detects ground to when the robot
// state message updates to the fact that it is no longer picked up
static const TimeStamp_t kRobotTimeToConsiderOfftreads_ms = 250;
static const TimeStamp_t kRobotTimeToConsiderOfftreadsOnBack_ms =
    kRobotTimeToConsiderOfftreads_ms * 3;
...
static const float kPitchAngleOnFacePlantMin_sim_rads =
    DEG_TO_RAD(110.f);  // This has not been tested
static const float kPitchAngleOnFacePlantMax_sim_rads =
    DEG_TO_RAD(-80.f);  // This has not been tested
...
Robot::Robot(const RobotID_t robotID, CozmoContext* context)
  ...
  if (Util::FileUtils::FileExists(dataClearedFile)) {
    DASMSG(robot_cleared_user_data, "robot.cleared_user_data",
  ...
  float idleTime_sec;
  auto upTime_sec = static_cast<uint32_t>(
      OSState::getInstance()->GetUptimeAndIdleTime(idleTime_sec));
  ...
  // create all components
  {
    ...
    _components->AddDependentComponent(
        RobotComponentID::AppCubeConnectionSubscriber,
        new AppCubeConnectionSubscriber());
    _components->AddDependentComponent(RobotComponentID::CozmoContextWrapper,
                                       new ContextWrapper(context));
    _components->AddDependentComponent(RobotComponentID::BlockWorld,
                                       new BlockWorld());
    _components->AddDependentComponent(RobotComponentID::FaceWorld,
                                       new FaceWorld());
    _components->AddDependentComponent(RobotComponentID::PetWorld,
                                       new PetWorld());
    _components->AddDependentComponent(RobotComponentID::PublicStateBroadcaster,
                                       new PublicStateBroadcaster());
    _components->AddDependentComponent(RobotComponentID::EngineAudioClient,
                                       new Audio::EngineRobotAudioClient());
    _components->AddDependentComponent(RobotComponentID::PathPlanning,
                                       new PathComponent());
    _components->AddDependentComponent(
        RobotComponentID::DrivingAnimationHandler,
        new DrivingAnimationHandler());
    _components->AddDependentComponent(RobotComponentID::ActionList,
                                       new ActionList());
    _components->AddDependentComponent(RobotComponentID::Movement,
                                       new MovementComponent());
    _components->AddDependentComponent(RobotComponentID::Vision,
                                       new VisionComponent());
    _components->AddDependentComponent(RobotComponentID::VisionScheduleMediator,
                                       new VisionScheduleMediator());
    _components->AddDependentComponent(RobotComponentID::Map,
                                       new MapComponent());
    _components->AddDependentComponent(RobotComponentID::NVStorage,
                                       new NVStorageComponent());
    _components->AddDependentComponent(RobotComponentID::AIComponent,
                                       new AIComponent());
    _components->AddDependentComponent(RobotComponentID::CubeLights,
                                       new CubeLightComponent());
    _components->AddDependentComponent(RobotComponentID::BackpackLights,
                                       new BackpackLightComponent());
    _components->AddDependentComponent(RobotComponentID::CubeAccel,
                                       new CubeAccelComponent());
    _components->AddDependentComponent(RobotComponentID::CubeBattery,
                                       new CubeBatteryComponent());
    _components->AddDependentComponent(RobotComponentID::CubeComms,
                                       new CubeCommsComponent());
    _components->AddDependentComponent(
        RobotComponentID::CubeConnectionCoordinator,
        new CubeConnectionCoordinator());
    _components->AddDependentComponent(RobotComponentID::CubeInteractionTracker,
                                       new CubeInteractionTracker());
    _components->AddDependentComponent(RobotComponentID::GyroDriftDetector,
                                       new RobotGyroDriftDetector());
    _components->AddDependentComponent(RobotComponentID::HabitatDetector,
                                       new HabitatDetectorComponent());
    _components->AddDependentComponent(RobotComponentID::Docking,
                                       new DockingComponent());
    _components->AddDependentComponent(RobotComponentID::Carrying,
                                       new CarryingComponent());
    _components->AddDependentComponent(RobotComponentID::CliffSensor,
                                       new CliffSensorComponent());
    _components->AddDependentComponent(RobotComponentID::ProxSensor,
                                       new ProxSensorComponent());
    _components->AddDependentComponent(RobotComponentID::ImuSensor,
                                       new ImuComponent());
    _components->AddDependentComponent(RobotComponentID::RangeSensor,
                                       new RangeSensorComponent());
    _components->AddDependentComponent(RobotComponentID::TouchSensor,
                                       new TouchSensorComponent());
    _components->AddDependentComponent(RobotComponentID::Animation,
                                       new AnimationComponent());
    _components->AddDependentComponent(RobotComponentID::StateHistory,
                                       new RobotStateHistory());
    _components->AddDependentComponent(RobotComponentID::MoodManager,
                                       new MoodManager());
    _components->AddDependentComponent(RobotComponentID::StimulationFaceDisplay,
                                       new StimulationFaceDisplay());
    _components->AddDependentComponent(RobotComponentID::BlockTapFilter,
                                       new BlockTapFilterComponent());
    _components->AddDependentComponent(
        RobotComponentID::RobotToEngineImplMessaging,
        new RobotToEngineImplMessaging());
    _components->AddDependentComponent(RobotComponentID::MicComponent,
                                       new MicComponent());
    _components->AddDependentComponent(RobotComponentID::Battery,
                                       new BatteryComponent());
    _components->AddDependentComponent(RobotComponentID::FullRobotPose,
                                       new FullRobotPose());
    _components->AddDependentComponent(RobotComponentID::DataAccessor,
                                       new DataAccessorComponent());
    _components->AddDependentComponent(RobotComponentID::BeatDetector,
                                       new BeatDetectorComponent());
    _components->AddDependentComponent(
        RobotComponentID::TextToSpeechCoordinator,
        new TextToSpeechCoordinator());
    _components->AddDependentComponent(RobotComponentID::SDK,
                                       new SDKComponent());
    _components->AddDependentComponent(RobotComponentID::PhotographyManager,
                                       new PhotographyManager());
    _components->AddDependentComponent(RobotComponentID::PowerStateManager,
                                       new PowerStateManager());
    _components->AddDependentComponent(RobotComponentID::SettingsCommManager,
                                       new SettingsCommManager());
    _components->AddDependentComponent(RobotComponentID::RobotHealthReporter,
                                       new RobotHealthReporter());
    _components->AddDependentComponent(RobotComponentID::SettingsManager,
                                       new SettingsManager());
    _components->AddDependentComponent(RobotComponentID::RobotStatsTracker,
                                       new RobotStatsTracker());
    _components->AddDependentComponent(
        RobotComponentID::VariableSnapshotComponent,
        new VariableSnapshotComponent());
    _components->AddDependentComponent(RobotComponentID::JdocsManager,
                                       new JdocsManager());
    _components->AddDependentComponent(RobotComponentID::AccountSettingsManager,
                                       new AccountSettingsManager());
    _components->AddDependentComponent(
        RobotComponentID::UserEntitlementsManager,
        new UserEntitlementsManager());
    _components->AddDependentComponent(RobotComponentID::LocaleComponent,
                                       new LocaleComponent());
    _components->InitComponents(this);
  }

  GetComponent<FullRobotPose>().GetPose().SetName("Robot_" +
                                                  std::to_string(_ID));
  ...
  // It will also flag that a localization update is needed when it increments
  // the frameID so set the flag to false
  _frameId = 0;
  GetStateHistory()->Clear();
  _needToSendLocalizationUpdate = false;

  GetRobotToEngineImplMessaging().InitRobotMessageComponent(
      GetContext()->GetRobotManager()->GetMsgHandler(), this);

  // Setting camera pose according to current head angle.
  // (Not using SetHeadAngle() because _isHeadCalibrated is initially false
  // making the function do nothing.)
  GetVisionComponent().GetCamera().SetPose(
      GetCameraPose(GetComponent<FullRobotPose>().GetHeadAngle()));
...
Robot::~Robot() {
  ...
  // VIC-1961: Remove touch sensor component before aborting all since there's a
  // DEV_ASSERT crash and we need to write data out from the touch sensor
  // component out. This explicit destruction can be removed once the DEV_ASSERT
  // is fixed
  _components->RemoveComponent(RobotComponentID::TouchSensor);

  AbortAll();

  // Destroy our actionList before things like the path planner, since actions
  // often rely on those. ActionList must be cleared before it is destroyed
  // because pending actions may attempt to make use of the pointer.
  GetActionList().Clear();
...
bool Robot::CheckAndUpdateTreadsState(const RobotState& msg) {
  ...
  const EngineTimeStamp_t currentTimestamp =
  ...
  //// COZMO_UP_RIGHT
  const bool currOnTreads =
      std::abs(GetPitchAngle().ToDouble() - kPitchAngleOntreads_rads) <=
      kPitchAngleOntreadsTolerance_rads;

  //// COZMO_ON_BACK
  const float backAngle =
      IsPhysical() ? kPitchAngleOnBack_rads : kPitchAngleOnBack_sim_rads;
  const bool currOnBack = std::abs(GetPitchAngle().ToDouble() - backAngle) <=
                          DEG_TO_RAD(kPitchAngleOnBackTolerance_deg);
  //// COZMO_ON_SIDE
  const bool currOnSide =
      Util::IsNear(std::abs(_robotAccelFiltered.y), kOnSideAccel_mmps2,
                   kOnSideToleranceAccel_mmps2);
  const bool onRightSide = currOnSide && (_robotAccelFiltered.y > 0.f);
  //// COZMO_ON_FACE
  const float facePlantMinAngle = IsPhysical()
                                      ? kPitchAngleOnFacePlantMin_rads
                                      : kPitchAngleOnFacePlantMin_sim_rads;
  const float facePlantMaxAngle = IsPhysical()
                                      ? kPitchAngleOnFacePlantMax_rads
                                      : kPitchAngleOnFacePlantMax_sim_rads;

  const bool currFacePlant = GetPitchAngle() > facePlantMinAngle ||
                             GetPitchAngle() < facePlantMaxAngle;
  ...
  if (isFalling) {
    if (_awaitingConfirmationTreadState != OffTreadsState::Falling) {
      ...
      _timeOffTreadStateChanged_ms =
          currentTimestamp - kRobotTimeToConsiderOfftreads_ms;
    }
  } else if (currOnSide) {
    ...
    if (_awaitingConfirmationTreadState != OffTreadsState::OnBack) {
      ...
      // On Back is a special case as it is also an intermediate state for
      // coming from onFace -> onTreads. hence we wait a little longer than
      // usual(kRobotTimeToConsiderOfftreads_ms) to check if it's on back.
      _timeOffTreadStateChanged_ms =
          currentTimestamp + kRobotTimeToConsiderOfftreadsOnBack_ms;
    }
  ...
  if (_awaitingConfirmationTreadState == OffTreadsState::OnTreads &&
      isPickedUp == true) {
  ...
  // there is a delay for the cliff sensor to confirm the robot is no longer
  // picked up
  if (_awaitingConfirmationTreadState != OffTreadsState::OnTreads &&
      isPickedUp != true && !currOnBack && !currOnSide && !currFacePlant) {
    _awaitingConfirmationTreadState = OffTreadsState::OnTreads;
    // Allows this to be called instantly
    _timeOffTreadStateChanged_ms =
        currentTimestamp - kRobotTimeToConsiderOfftreads_ms;
  ...
  if (_timeOffTreadStateChanged_ms + kRobotTimeToConsiderOfftreads_ms <=
          currentTimestamp &&
    ...
    if (_awaitingConfirmationTreadState == OffTreadsState::Falling) {
      ...
      LOG_INFO("Robot.CheckAndUpdateTreadsState.FallingStarted", "t=%dms",
      ...
      // This is not an exact measurement of fall time since it includes some
      // detection delays on the robot side It may also include
      // kRobotTimeToConsiderOfftreads_ms depending on how the robot lands
      LOG_INFO("Robot.CheckAndUpdateTreadsState.FallingStopped",
               "t=%dms, duration=%dms", (TimeStamp_t)GetLastMsgTimestamp(),
               (TimeStamp_t)(GetLastMsgTimestamp() - _fallingStartedTime_ms));
      _fallingStartedTime_ms = 0;
    }

    _offTreadsState = _awaitingConfirmationTreadState;
    Broadcast(ExternalInterface::MessageEngineToGame(
        ExternalInterface::RobotOffTreadsStateChanged(_offTreadsState)));

    LOG_INFO("Robot.OfftreadsState.TreadStateChanged",
             "TreadState changed to:%s", EnumToString(_offTreadsState));
    ...
    if (_offTreadsState == OffTreadsState::OnTreads) {
      ...
      // If we are not localized and there is nothing else left in the world (in
      // any origin) that we could localize to, then go ahead and mark us as
      // localized (via odometry alone)
      if (!IsLocalized() && !GetBlockWorld().AnyRemainingLocalizableObjects(
                                PoseOriginList::UnknownOriginID)) {
        LOG_INFO("Robot.UpdateOfftreadsState.NoMoreRemainingLocalizableObjects",
                 "Marking previously-unlocalized robot as localized to "
                 "odometry because "
      ...
      // are no longer carrying that something and don't know where it is
      // anymore
  ...
  //////////////////////////////////
  // Check if robot is stuck in-air for a long time, but is likely not being
  // held. Might be indicative of a vibrating surface or overly sensitive
  // conditions for staying in the picked up state.
  ...
  if (!reportedInAirTooLong) {
    ...
    // still being held.
    const bool robotAngleChanged =
        (lastStableRobotAngle_rad - msg.pose.angle).getAbsoluteVal().ToFloat() >
        kRobotAngleChangedThresh_rad;
    if ((_offTreadsState == OffTreadsState::InAir) && robotAngleChanged) {
      inAirTooLongReportTime_ms =
          currentTimestamp + kInAirTooLongTimeReportTime_ms;
    ...
    if ((inAirTooLongReportTime_ms > 0) &&
      ...
      DASMSG(robot_too_long_in_air, "robot.too_long_in_air",
             "Robot has been in InAir picked up state for too long. Vibrating "
             "surface?");
      DASMSG_SEND();
...
Util::RandomGenerator& Robot::GetRNG() { return *GetContext()->GetRandom(); }
...
void Robot::Delocalize(bool isCarryingObject) {
  ...
  // TODO rsam:
  // origins are no longer destroyed to prevent children from having to rejigger
  // as cubes do. This however has the problem of leaving zombie origins, and
  // having systems never deleting dead poses that can never be transformed
  // withRespectTo a current origin. The origins growing themselves is not a big
  // problem since they are merely a Pose3d instance. However systems that keep
  // Poses around because they have a valid origin could potentially be a
  // problem. This would have to be profiled to identify those systems, so not
  // really worth adding here a warning for "number of zombies is too big"
  // without actually keeping track of how many children they hold, or for how
  // long. Eg: zombies with no children could auto-delete themselves, but is the
  // cost of bookkeeping bigger than what we are currently losing to zombies?
  // That's the question to profile
  ...
  // Create a new pose frame so that we can't get pose history entries with the
  // same pose frame that have different origins (Not 100% sure this is totally
  // necessary but seems like the cleaner / safer thing to do.)
  ...
  if (_syncRobotAcked) {
    // Need to update the robot's pose history with our new origin and pose
    // frame IDs
    LOG_INFO("Robot.Delocalize.SendingNewOriginID",
             "Sending new localization update at t=%u, with pose frame %u and "
             "origin ID=%u",
             (TimeStamp_t)GetLastMsgTimestamp(), GetPoseFrameID(),
             worldOrigin.GetID());
    SendAbsLocalizationUpdate(GetComponent<FullRobotPose>().GetPose(),
                              GetLastMsgTimestamp(), GetPoseFrameID());
  }

  // Update VizText
  GetContext()->GetVizManager()->SetText(TextLabelType::LOCALIZED_TO,
                                         NamedColors::YELLOW,
                                         "LocalizedTo: <nothing>");
  GetContext()->GetVizManager()->SetText(
      TextLabelType::WORLD_ORIGIN, NamedColors::YELLOW, "WorldOrigin[%lu]: %s",
      GetPoseOriginList().GetSize(), worldOrigin.GetName().c_str());
  ...
  // Have to do this _after_ clearing the pose confirmer because
  // UpdateObjectOrigin adds the carried objects to the pose confirmer in their
  // newly updated pose, but _before_ deleting zombie objects (since dirty
  // carried objects may get deleted)
  if (GetCarryingComponent().IsCarryingObject()) {
    // Carried objects are in the pose chain of the robot, whose origin has now
    // changed. Thus the carried object's actual origin no longer matches the
    // way they are stored in BlockWorld.
    const auto& objectID = GetCarryingComponent().GetCarryingObjectID();
    const Result result =
        GetBlockWorld().UpdateObjectOrigin(objectID, oldOriginID);
    if (RESULT_OK != result) {
      LOG_WARNING("Robot.Delocalize.UpdateObjectOriginFailed", "Object %d",
                  objectID.GetValue());
    }
  }
  ...
  // send message to game. At the moment I implement this so that Webots can
  // update the render, but potentially any system can listen to this
  Broadcast(ExternalInterface::MessageEngineToGame(
      ExternalInterface::RobotDelocalized()));

  DASMSG(robot_delocalized, "robot.delocalized",
         "The robot has delocalized. This event occurs any time the robot "
         "delocalizes.");
...
Result Robot::SetLocalizedTo(const ObservableObject* object) {
  ...
  if (_offTreadsState != OffTreadsState::OnTreads) {
    LOG_ERROR("Robot.SetLocalizedTo.OffTreads",
              "Cannot localize while off treads");
    return RESULT_FAIL;
  }

  if (object->GetID().IsUnknown()) {
    LOG_ERROR("Robot.SetLocalizedTo.IdNotSet",
  ...
  for (const auto& marker : object->GetMarkers()) {
    if (marker.GetLastObservedTime() >= mostRecentObsTime) {
      ...
      if (false == marker.GetPose().GetWithRespectTo(
                       GetVisionComponent().GetCamera().GetPose(),
                       markerPoseWrtCamera)) {
        LOG_ERROR("Robot.SetLocalizedTo.MarkerOriginProblem",
                  "Could not get pose of marker w.r.t. robot camera");
        return RESULT_FAIL;
      }
      const f32 distToMarkerSq =
          markerPoseWrtCamera.GetTranslation().LengthSq();
      if (_localizedMarkerDistToCameraSq < 0.f ||
          distToMarkerSq < _localizedMarkerDistToCameraSq) {
  ...
  // Update VizText
  GetContext()->GetVizManager()->SetText(
      TextLabelType::LOCALIZED_TO, NamedColors::YELLOW, "LocalizedTo: %s_%d",
      ObjectTypeToString(object->GetType()), _localizedToID.GetValue());
  GetContext()->GetVizManager()->SetText(
      TextLabelType::WORLD_ORIGIN, NamedColors::YELLOW, "WorldOrigin[%lu]: %s",
      GetPoseOriginList().GetSize(), GetWorldOrigin().GetName().c_str());
...
void UpdateFaceImageRGBExample(Robot& robot) {
  ...
  // The duration of the image should ideally be some multiple of
  // ANIM_TIME_STEP_MS, especially if you're playing a bunch of images in
  // sequence, otherwise the speed of the animation may not be as expected.
...
Result Robot::UpdateFullRobotState(const RobotState& msg) {
  ...
  _robotAccelMagnitude =
      sqrtf(_robotAccel.x * _robotAccel.x + _robotAccel.y * _robotAccel.y +
            _robotAccel.z * _robotAccel.z);

  const float kAccelMagFilterConstant = 0.95f;  // between 0 and 1
  _robotAccelMagnitudeFiltered =
      (kAccelMagFilterConstant * _robotAccelMagnitudeFiltered) +
  ...
  const bool isDelocalizing = wasTreadsStateUpdated &&
                              (prevOffTreadsState == OffTreadsState::OnTreads ||
                               _offTreadsState == OffTreadsState::OnTreads);

  if (isDelocalizing && (prevOffTreadsState == OffTreadsState::OnTreads)) {
    // Robot is delocalized, and not because it was put back down. Tell the map
    // component to send relevant info about the previous map. This is commanded
    // here instead of map component's CreateLocalizedMemoryMap so that the map
    // info from when the robot is in the air is not sent when the robot is put
    // back down, since MapComponent doesn't (need to) track OffTreadsState
    GetMapComponent().SendDASInfoAboutCurrentMap();
  }

  if (wasTreadsStateUpdated) {
    DASMSG(robot_offtreadsstatechanged, "robot.offtreadsstatechanged",
           "The robot off treads state changed");
    DASMSG_SET(s1, OffTreadsStateToString(_offTreadsState), "New state");
    DASMSG_SET(s2, OffTreadsStateToString(prevOffTreadsState),
               "Previous state");
    DASMSG_SEND();
  }

  // this flag can have a small delay with respect to when we actually picked up
  // the block, since Engine notifies the robot, and the robot updates on the
  // next state update. But that delay guarantees that the robot knows what we
  // think it's true, rather than mixing timestamps of when it started carrying
  // vs when the robot knows that it was carrying
  const bool isCarryingObject = IS_STATUS_FLAG_SET(IS_CARRYING_BLOCK);
  // robot->SetCarryingBlock( isCarryingObject ); // Still needed?
  GetDockingComponent().SetPickingOrPlacing(
      IS_STATUS_FLAG_SET(IS_PICKING_OR_PLACING));
  ...
  _hasMovedSinceLocalization |= (isHeadMoving || areWheelsMoving ||
                                 _offTreadsState != OffTreadsState::OnTreads);
  ...
  if (isDelocalizing) {
    ...
    DEV_ASSERT(msg.pose_frame_id <= GetPoseFrameID(),
               "Robot.UpdateFullRobotState.FrameFromFuture");
    ...
    if (!GetPoseOriginList().ContainsOriginID(msg.pose_origin_id)) {
      LOG_WARNING("Robot.UpdateFullRobotState.BadOriginID",
                  "Received RobotState with originID=%u, only %zu pose origins "
                  "available",
    ...
    const Pose3d& origin =
        GetPoseOriginList().GetOriginByID(msg.pose_origin_id);

    // Initialize new pose to be within the reported origin
    Pose3d newPose(msg.pose.angle, Z_AXIS_3D(),
                   {msg.pose.x, msg.pose.y, msg.pose.z}, origin);

    // It's possible the pose origin to which this update refers has since been
    // rejiggered and is now the child of another origin. To add to history
    // below, we must first flatten it. We do all this before "fixing" pose_z
    // because pose_z will be w.r.t. robot origin so we want newPose to already
    // be as well.
    ...
    if (msg.pose_frame_id == GetPoseFrameID()) {
      ...
      lastResult = GetStateHistory()->GetLastStateWithFrameID(msg.pose_frame_id,
                                                              histState);
      if (lastResult != RESULT_OK) {
        // Don't print warning if frame_id 0 because this can sometimes happen
        // on startup
    ...
    newPose.SetTranslation(
        {newPose.GetTranslation().x(), newPose.GetTranslation().y(), pose_z});

    // Add to history
    const HistRobotState histState(
        newPose, msg, GetProxSensorComponent().GetLatestProxData());
    lastResult = GetStateHistory()->AddRawOdomState(msg.timestamp, histState);

    if (lastResult != RESULT_OK) {
      LOG_WARNING("Robot.UpdateFullRobotState.AddPoseError",
                  "AddRawOdomStateToHistory failed for timestamp=%d",
                  msg.timestamp);
      return lastResult;
    }
    ...
    if (frameIsCurrent) {
      ...
      // COZMO-5850 (Al) This is to catch the issue where our frameID is
      // incremented but fails to send to the robot due to some origin issue.
      // Somehow the robot's pose becomes an origin and doesn't exist in the
      // PoseOriginList. The frameID mismatch then causes all sorts of issues in
      // things (ie VisionSystem won't process the next image). Delocalizing
      // will fix the mismatch by creating a new origin and sending a
      // localization update
      static const u32 kNumTicksWithMismatchedFrameIDs =
          100;  // 3 seconds (called each RobotState msg)

      ++_numMismatchedFrameIDs;

      if (_numMismatchedFrameIDs > kNumTicksWithMismatchedFrameIDs) {
        LOG_ERROR(
            "Robot.UpdateFullRobotState.MismatchedFrameIDs",
            "Robot[%u] and engine[%u] frameIDs are mismatched, delocalizing",
            msg.pose_frame_id, GetPoseFrameID());
  ...
  // Update processed proxSensorData in history after ProxSensorComponent was
  // updated
  GetStateHistory()->UpdateProxSensorData(
      msg.timestamp, GetProxSensorComponent().GetLatestProxData());
...
#pragma clang diagnostic pop
  ...
  /*
    ...
    _ID, GetComponent<FullRobotPose>().GetPose().GetTranslation().x(),
    GetComponent<FullRobotPose>().GetPose().GetTranslation().y(),
    GetComponent<FullRobotPose>().GetPose().GetTranslation().z(),
  ...
  */
  ...
  // TODO: Should this just be a different message? Or one that includes the
  // state message from the robot?
  RobotState stateMsg(msg);

  const u16 imageFramePeriod_ms =
      Util::numeric_cast<u16>(GetVisionComponent().GetFramePeriod_ms());
  const u16 imageProcPeriod_ms =
      Util::numeric_cast<u16>(GetVisionComponent().GetProcessingPeriod_ms());

  // Send state to visualizer for displaying
  VizInterface::RobotStateMessage vizState(
      stateMsg, _robotImuTemperature_degC,
      GetCliffSensorComponent().GetCliffDetectThresholds(), imageFramePeriod_ms,
      imageProcPeriod_ms, GetMoveComponent().GetLockedTracks(),
      GetAnimationComponent().GetAnimState_TracksInUse(),
      GetBatteryComponent().GetBatteryVolts(), _offTreadsState,
...
bool Robot::HasReceivedRobotState() const { return _newStateMsgAvailable; }

Result Robot::GetHistoricalCamera(RobotTimeStamp_t t_request,
...
Pose3d Robot::GetHistoricalCameraPose(const HistRobotState& histState,
...
u32 Robot::GetDisplayWidthInPixels() const { return FACE_DISPLAY_WIDTH; }

u32 Robot::GetDisplayHeightInPixels() const { return FACE_DISPLAY_HEIGHT; }

Vision::Camera Robot::GetHistoricalCamera(const HistRobotState& histState,
                                          RobotTimeStamp_t t) const {
...
Result Robot::Update() {
  ...
  if (!_gotStateMsgAfterRobotSync) {
    LOG_DEBUG("Robot.Update",
              "Waiting for first full robot state to be handled");
    return RESULT_OK;
  }
  ...
  if (IsPhysical()) {
    GetContext()->GetVizManager()->DrawRobot(
        robotPoseWrtOrigin, GetComponent<FullRobotPose>().GetHeadAngle(),
        GetComponent<FullRobotPose>().GetLiftAngle());
  }
  ...
  Quad3f quadOnGround3d(Point3f(quadOnGround2d[TopLeft].x(),
                                quadOnGround2d[TopLeft].y(), zHeight),
                        Point3f(quadOnGround2d[BottomLeft].x(),
                                quadOnGround2d[BottomLeft].y(), zHeight),
                        Point3f(quadOnGround2d[TopRight].x(),
                                quadOnGround2d[TopRight].y(), zHeight),
                        Point3f(quadOnGround2d[BottomRight].x(),
                                quadOnGround2d[BottomRight].y(), zHeight));

  GetContext()->GetVizManager()->DrawRobotBoundingBox(
      GetID(), quadOnGround3d, ROBOT_BOUNDING_QUAD_COLOR);
  ...
  GetContext()->GetVizManager()->DrawCuboid(999, {ROBOT_BOUNDING_X,
  ROBOT_BOUNDING_Y, ROBOT_BOUNDING_Z}, vizPose, ROBOT_BOUNDING_QUAD_COLOR);
  */
  ...
  if (kDebugPossibleBlockInteraction) {
    ...
    GetBlockWorld().FindLocatedMatchingObjects(
        filter,
        matchingObjects);  // note this doesn't retrieve unknowns anymore
    for (const auto obj : matchingObjects) {
      ...
      LOG_DEBUG("Robot.ObjectInteractionState",
                "block:%d poseState:%8s moving?%d RestingFlat?%d carried?%d "
                "poseWRT?%d"
                " z=%6.2f UpAxis:%s CanStack?%d CanPickUp?%d FromGround?%d",
                obj->GetID().GetValue(), PoseStateToString(obj->GetPoseState()),
                obj->IsMoving(), obj->IsRestingFlat(),
                (GetCarryingComponent().IsCarryingObject() &&
                 GetCarryingComponent().GetCarryingObjectID() == obj->GetID()),
                gotRelPose, relPose.GetTranslation().z(), axisStr,
  ...
  if (!_sentEngineLoadedMsg) {
    ...
    auto battery_mV =
        static_cast<uint32_t>(GetBatteryComponent().GetBatteryVolts() * 1000);

    LOG_INFO("Robot.Update.EngineFullyLoaded", "OnCharger: %d, Battery_mV: %d",
             onCharger, battery_mV);

    DASMSG(robot_engine_ready, "robot.engine_ready",
           "All robot processes are ready");
...
Result Robot::SetNewPose(const Pose3d& newPose) {
  ...
  return AddVisionOnlyStateToHistory(
      timeStamp, GetComponent<FullRobotPose>().GetPose(),
      GetComponent<FullRobotPose>().GetHeadAngle(),
      GetComponent<FullRobotPose>().GetLiftAngle());
}
...
void Robot::SetPose(const Pose3d& newPose) {
  ...
  ComputeDriveCenterPose(GetComponent<FullRobotPose>().GetPose(),
                         _driveCenterPose);
  ...
  if (_isHeadCalibrated) {
    ...
    if (clippedHeadAngle != angle) {
      LOG_WARNING("Robot.GetCameraHeadPose.HeadAngleOOB",
                  "Angle %.3frad / %.1f", angle, RAD_TO_DEG(angle));
    }
  }

  // note: moving the motor inside bounds shouldn't erase previous state
  _isHeadMotorOutOfBounds |=
      angle < (MIN_HEAD_ANGLE - HEAD_ANGLE_LIMIT_MARGIN) ||
...
bool Robot::IsHeadCalibrated() const { return _isHeadCalibrated; }

bool Robot::IsLiftCalibrated() const { return _isLiftCalibrated; }
...
void Robot::SetLiftAngle(const f32& angle) {
  // note: moving the motor inside bounds shouldn't erase previous state
  _isLiftMotorOutOfBounds |=
      angle < (MIN_LIFT_ANGLE - LIFT_ANGLE_LIMIT_MARGIN) ||
      angle > (MAX_LIFT_ANGLE + LIFT_ANGLE_LIMIT_MARGIN);

  // TODO: Add lift angle limits?
  GetComponent<FullRobotPose>().SetLiftAngle(angle);

  Robot::ComputeLiftPose(GetComponent<FullRobotPose>().GetLiftAngle(),
                         GetComponent<FullRobotPose>().GetLiftPose());

  DEV_ASSERT(GetComponent<FullRobotPose>().GetLiftPose().IsChildOf(
                 GetComponent<FullRobotPose>().GetLiftBasePose()),
             "Robot.SetLiftAngle.InvalidPose");
}
...
bool Robot::WasObjectTappedRecently(const ObjectID& objectID) const {
  return GetComponent<BlockTapFilterComponent>()
      .ShouldIgnoreMovementDueToDoubleTap(objectID);
}

TimeStamp_t Robot::GetTimeSincePowerButtonPressed_ms() const {
  // this is a time difference, so could be any type, but to avoid someone
  // thinking that all power button times are robot times, this returns an
  // engine timestmap to match _timePowerButtonPressed_ms
  return _timePowerButtonPressed_ms == 0
             ? 0
             : TimeStamp_t(
                   BaseStationTimer::getInstance()->GetCurrentTimeStamp() -
                   _timePowerButtonPressed_ms);
...
void Robot::HandlePokeEvent() {
  ...
  LOG_INFO("Robot.HandlePokeEvent", "Last poke event timestamp recorded as %u",
           static_cast<u32>(_timeLastPoked));
}

EngineTimeStamp_t Robot::GetTimeSinceLastPoke_ms() const {
  // If the robot has never reported being poked, then set the time difference
  // to the maximum allowable value
  return _timeLastPoked == 0
             ? std::numeric_limits<EngineTimeStamp_t>::max()
             : TimeStamp_t(
                   BaseStationTimer::getInstance()->GetCurrentTimeStamp() -
                   _timeLastPoked);
}
...
Result Robot::LocalizeToObject(const ObservableObject* seenObject,
  ...
  if (nullptr == seenObject) {
    if (false == GetPose().GetWithRespectTo(existingObject->GetPose(),
                                            robotPoseWrtObject)) {
    ...
    if ((lastResult = GetStateHistory()->GetComputedStateAt(
             seenObject->GetLastObservedTime(), &histStatePtr,
             &histStateKey)) != RESULT_OK) {
      LOG_ERROR("Robot.LocalizeToObject.CouldNotFindHistoricalPose", "Time %d",
                seenObject->GetLastObservedTime());
      return lastResult;
    }

    // The computed historical pose is always stored w.r.t. the robot's world
    // origin and parent chains are lost. Re-connect here so that
    // GetWithRespectTo will work correctly
    ...
    if (robotPoseAtObsTime.GetWithRespectTo(seenObject->GetPose(),
                                            robotPoseWrtObject) == false) {
  ...
  // robotPoseWrtMat.SetName(std::string("Robot_") +
  ...
  if (IsLocalized()) {
    ...
    static const f32 zUpdateWeight =
        0.1f;  // weight of new value (previous gets weight of 1 - this)
  ...
  if (nullptr != seenObject) {
    lastResult =
        AddVisionOnlyStateToHistory(seenObject->GetLastObservedTime(),
                                    robotPoseWrtOrigin, headAngle, liftAngle);
    if (lastResult != RESULT_OK) {
  ...
  if (!existingObject->GetPose().HasSameRootAs(origOrigin)) {
    ...
    const PoseOriginID_t origOriginID =
        GetPoseOriginList().GetCurrentOriginID();
    ...
    Result result =
        _poseOrigins->Rejigger(robotPoseWrtObject.FindRoot(), transform);
    if (ANKI_VERIFY(RESULT_OK == result,
                    "Robot.LocalizeToObject.RejiggerFailed", "")) {
      const PoseOriginID_t newOriginID =
          GetPoseOriginList().GetCurrentOriginID();
      ...
      // Note that map component must be updated before blockworld in case
      // blockworld tries to insert a new object into the map.
      ...
      // after updating all block world objects, flatten out origins to remove
      // grandparents
  ...
  if (nullptr != histStatePtr) {
    ...
    histStatePtr->SetPose(GetPoseFrameID(), robotPoseWrtOrigin, headAngle,
                          liftAngle);
  }
  ...
  if (histStatePtr->WasPickedUp() ||
      (_offTreadsState != OffTreadsState::OnTreads)) {
    LOG_INFO("Robot.LocalizeToObject.OffTreads",
             "Not localizing to object since we are not on treads");
    return RESULT_OK;
  }
  ...
  /*
    ...
    "Using %s object %d to localize robot %d at (%.3f,%.3f,%.3f),
    %.1fdeg@(%.2f,%.2f,%.2f), frameID=%d\n",
  ...
  */
  ...
  // Don't actually send the update here, because it's possible that we are
  // going to call LocalizeToObject more than once in this tick, which could
  // cause the pose frame ID to update multiple times, replacing what's stored
  // for this timestamp in pose history. We don't want to send a pose frame to
  // the robot that's about to be replaced in history and never used again
  // (causing errors in looking for it when we later receive a RobotState
  // message with that invalidated frameID), so just set this flag to do the
  // localization update once out in Update().
...
Result Robot::SetPoseOnCharger() {
  ...
  Charger* charger =
      dynamic_cast<Charger*>(GetBlockWorld().GetLocatedObjectByID(_chargerID));
  if (charger == nullptr) {
    LOG_WARNING("Robot.SetPoseOnCharger.NoChargerWithID",
                "Robot %d has docked to charger, but Charger object with ID=%d "
  ...
  Result lastResult =
      SetNewPose(charger->GetRobotDockedPose().GetWithRespectToRoot());
  if (lastResult != RESULT_OK) {
    LOG_WARNING("Robot.SetPoseOnCharger.SetNewPose",
  ...
  LOG_INFO("Robot.SetPoseOnCharger.SetPose",
           "Robot %d now on charger %d, at (%.1f,%.1f,%.1f) @ %.1fdeg, "
           "timeStamp = %d",
           _ID, charger->GetID().GetValue(),
           GetComponent<FullRobotPose>().GetPose().GetTranslation().x(),
           GetComponent<FullRobotPose>().GetPose().GetTranslation().y(),
           GetComponent<FullRobotPose>().GetPose().GetTranslation().z(),
...
Result Robot::SetPosePostRollOffCharger() {
  ...
  Result lastResult =
      SetNewPose(charger->GetRobotPostRollOffPose().GetWithRespectToRoot());
  if (lastResult != RESULT_OK) {
    LOG_WARNING("Robot.SetPosePostRollOffCharger.SetNewPose",
                "Failed to set new pose");
    return lastResult;
  }

  LOG_INFO("Robot.SetPosePostRollOffCharger.NewRobotPose",
           "Updated robot pose to be in front of the charger, as if it had "
           "just rolled off.");
  return RESULT_OK;
}
...
Result Robot::SendMessage(const RobotInterface::EngineToRobot& msg,
                          bool reliable, bool hot) const {
  static Util::MessageProfiler msgProfiler("Robot::SendMessage");

  Result sendResult =
      GetContext()->GetRobotManager()->GetMsgHandler()->SendMessage(
          msg, reliable, hot);
  if (sendResult == RESULT_OK) {
    msgProfiler.Update((int)msg.GetTag(), msg.Size());
  } else {
    const char* msgTypeName = EngineToRobotTagToString(msg.GetTag());
    LOG_WARNING("Robot.SendMessage",
                "Robot %d failed to send a message type %s", _ID, msgTypeName);
...
Result Robot::SendSyncRobot() const {
  Result result =
      SendMessage(RobotInterface::EngineToRobot(RobotInterface::SyncRobot()));
...
Result Robot::SendAbsLocalizationUpdate(const Pose3d& pose,
  ...
  // Send flattened poses to the robot, because when we get them back in
  // odometry updates with origin IDs, we can only hook them back up directly to
  // the corresponding pose origin (we can't know the chain that led there
  // anymore)

  const Pose3d& poseWrtOrigin = pose.GetWithRespectToRoot();
  const Pose3d& origin =
      poseWrtOrigin.GetParent();  // poseWrtOrigin's parent is, by definition,
                                  // the root / an origin
  DEV_ASSERT(origin.IsRoot(), "Robot.SendAbsLocalizationUpdate.OriginNotRoot");
  DEV_ASSERT(pose.HasSameRootAs(origin),
             "Robot.SendAbsLocalizationUpdate.ParentOriginMismatch");
  ...
  return SendMessage(
      RobotInterface::EngineToRobot(RobotInterface::AbsoluteLocalizationUpdate(
          (TimeStamp_t)t, frameId, originID, poseWrtOrigin.GetTranslation().x(),
...
Result Robot::SendAbsLocalizationUpdate() const {
  ...
  RobotTimeStamp_t t;
  HistRobotState histState;
  if (GetStateHistory()->GetLatestVisionOnlyState(t, histState) ==
      RESULT_FAIL) {
    LOG_WARNING("Robot.SendAbsLocUpdate.NoVizPoseFound", "");
    return RESULT_FAIL;
  }

  return SendAbsLocalizationUpdate(histState.GetPose().GetWithRespectToRoot(),
                                   t, histState.GetFrameId());
}
...
IExternalInterface* Robot::GetExternalInterface() const {
  DEV_ASSERT(GetContext()->GetExternalInterface() != nullptr,
             "Robot.ExternalInterface.nullptr");
...
IGatewayInterface* Robot::GetGatewayInterface() const {
  DEV_ASSERT(GetContext()->GetGatewayInterface() != nullptr,
             "Robot.GatewayInterface.nullptr");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Message handlers subscribed to in
// RobotToEngineImplMessaging::InitRobotMessageComponent
...
void Robot::HandleMessage(const ExternalInterface::RequestRobotSettings& msg) {
  ...
  if (cameraCalibration == nullptr) {
    ...
    cameraCalibration =
        std::make_shared<Vision::CameraCalibration>(0, 0, 1.f, 1.f, 0.f, 0.f);
  }

  ExternalInterface::CameraConfig cameraConfig(
      cameraCalibration->GetFocalLength_x(),
      cameraCalibration->GetFocalLength_y(), cameraCalibration->GetCenter_x(),
      ...
      visionComponent.GetMinCameraGain(), visionComponent.GetMaxCameraGain());
...
/*
  const Pose3d Robot::ProxDetectTransform[] = { Pose3d(0, Z_AXIS_3D(), Vec3f(50,
  25, 0)), Pose3d(0, Z_AXIS_3D(), Vec3f(50, 0, 0)), Pose3d(0, Z_AXIS_3D(),
  Vec3f(50, -25, 0)) };
*/
...
Quad2f Robot::GetBoundingQuadXY(const Pose3d& atPose, const f32 padding_mm) {
  ...
  if (padding_mm != 0.f) {
    Quad2f paddingQuad({padding_mm, -padding_mm}, {padding_mm, padding_mm},
                       {-padding_mm, -padding_mm}, {-padding_mm, padding_mm});
...
f32 Robot::GetHeight() const {
  return std::max(ROBOT_BOUNDING_Z,
                  GetLiftHeight() + LIFT_HEIGHT_ABOVE_GRIPPER);
}

f32 Robot::GetLiftHeight() const {
  return ConvertLiftAngleToLiftHeightMM(
      GetComponent<FullRobotPose>().GetLiftAngle());
}

Transform3d Robot::GetLiftTransformWrtCamera(const f32 atLiftAngle,
...
OffTreadsState Robot::GetOffTreadsState() const { return _offTreadsState; }
...
Result Robot::AddVisionOnlyStateToHistory(const RobotTimeStamp_t t,
  ...
  // Set needToSendLocalizationUpdate to true so we send an update on the next
  // tick
  _needToSendLocalizationUpdate = true;
...
Result Robot::GetComputedStateAt(const RobotTimeStamp_t t_request,
                                 Pose3d& pose) const {
  HistStateKey histStateKey;
  const HistRobotState* histStatePtr = nullptr;
  Result lastResult = GetStateHistory()->GetComputedStateAt(
      t_request, &histStatePtr, &histStateKey);
  if (lastResult == RESULT_OK) {
...
bool Robot::UpdateCurrPoseFromHistory() {
  ...
  RobotTimeStamp_t t;
  HistRobotState histState;
  if (GetStateHistory()->ComputeStateAt(GetStateHistory()->GetNewestTimeStamp(),
                                        t, histState) == RESULT_OK) {
    const Pose3d& worldOrigin = GetWorldOrigin();
    Pose3d newPose;
    if ((histState.GetPose().GetWithRespectTo(worldOrigin, newPose)) == false) {
      // This is not necessarily an error anymore: it's possible we've received
      // an odometry update from the robot w.r.t. an old origin (before being
      // delocalized), in which case we can't use it to update the current pose
      // of the robot in its new frame.
      LOG_INFO("Robot.UpdateCurrPoseFromHistory.GetWrtParentFailed",
               "Could not update robot %d's current pose using historical pose "
               "w.r.t. %s because we are now in frame %s.",
...
Result Robot::AbortAnimation() { return SendAbortAnimation(); }

Result Robot::SendAbortAnimation() {
  return SendMessage(
      RobotInterface::EngineToRobot(RobotInterface::AbortAnimation()));
}

void Robot::ComputeDriveCenterPose(const Pose3d& robotPose,
                                   Pose3d& driveCenterPose) const {
  MoveRobotPoseForward(robotPose, GetDriveCenterOffset(), driveCenterPose);
}

void Robot::ComputeOriginPose(const Pose3d& driveCenterPose,
                              Pose3d& robotPose) const {
  MoveRobotPoseForward(driveCenterPose, -GetDriveCenterOffset(), robotPose);
}

void Robot::MoveRobotPoseForward(const Pose3d& startPose, f32 distance,
                                 Pose3d& movedPose) {
  movedPose = startPose;
...
// @TODO: Refactor internal components to use proto state, and remove clad state
// utilities/messages VIC-4998
ExternalInterface::RobotState Robot::GetRobotState() const {
  ...
  msg.lastImageTimeStamp =
      (TimeStamp_t)GetVisionComponent().GetLastProcessedImageTimeStamp();
...
external_interface::RobotState* Robot::GenerateRobotStateProto() const {
  ...
  auto* dstPoseStruct = new external_interface::PoseStruct(
      srcPoseStruct.x, srcPoseStruct.y, srcPoseStruct.z, srcPoseStruct.q0,
      srcPoseStruct.q1, srcPoseStruct.q2, srcPoseStruct.q3,
  ...
  auto* dstAccelStruct = new external_interface::AccelData(
      srcAccelStruct.x, srcAccelStruct.y, srcAccelStruct.z);
  msg->set_allocated_accel(dstAccelStruct);

  const auto& srcGyroStruct = GetHeadGyroData();
  auto* dstGyroStruct = new external_interface::GyroData(
      srcGyroStruct.x, srcGyroStruct.y, srcGyroStruct.z);
  ...
  msg->set_last_image_time_stamp(
      (TimeStamp_t)GetVisionComponent().GetLastProcessedImageTimeStamp());

  const auto& srcProxData = GetProxSensorComponent().GetLatestProxData();
  auto* dstProxData = new external_interface::ProxData(
      srcProxData.distance_mm, srcProxData.signalQuality,
      srcProxData.unobstructed, srcProxData.foundObject,
...
RobotState Robot::GetDefaultRobotState() {
  const auto kDefaultStatus =
      (Util::EnumToUnderlying(RobotStatusFlag::HEAD_IN_POS) |
       Util::EnumToUnderlying(RobotStatusFlag::LIFT_IN_POS));

  const RobotPose kDefaultPose(0.f, 0.f, 0.f, 0.f, 0.f, 0.f);

  std::array<uint16_t, Util::EnumToUnderlying(CliffSensor::CLIFF_COUNT)>
      defaultCliffRawVals;
  defaultCliffRawVals.fill(std::numeric_limits<uint16_t>::max());

  std::array<IMUDataFrame, IMUConstants::IMU_FRAMES_PER_ROBOT_STATE>
      defaultImuDataFrames;
  defaultImuDataFrames.fill(IMUDataFrame{0, GyroData{0, 0, 0}});

  const RobotState state(
      1,             // uint32_t timestamp, (Robot does not report at t=0
      ...
      std::move(defaultImuDataFrames),  // std::array<Anki::Vector::IMUDataFrame,
                                        // 6> imuData,
...
Result Robot::ComputeHeadAngleToSeePose(const Pose3d& pose, Radians& headAngle,
                                        f32 yTolFrac) const {
  Pose3d poseWrtNeck;
  const bool success = pose.GetWithRespectTo(
      GetComponent<FullRobotPose>().GetNeckPose(), poseWrtNeck);
  if (!success) {
  ...
  const Point3f pointWrtNeck(
      Point2f(poseWrtNeck.GetTranslation())
          .Length(),  // Drop z and get length in XY plane
      0.f, poseWrtNeck.GetTranslation().z());
...
#define DEBUG_HEAD_ANGLE_ITERATIONS 0
  while (iteration++ < kMaxIterations) {
    if (DEBUG_HEAD_ANGLE_ITERATIONS) {
      LOG_DEBUG("ComputeHeadAngle", "%d: %.1fdeg", iteration,
                RAD_TO_DEG(searchAngle_rad));
    }
    ...
    // Note: not using camera's Project3dPoint() method because it does special
    // handling
    //  for points not in the image limits, which we don't want here. We also
    //  don't need to add ycen, because we'll just subtract it right back off to
    //  see how far from centered we are. And we're only using y, so we'll just
    ...
    const f32 y =
        calib->GetFocalLength_y() * (pointWrtCam.y() / pointWrtCam.z());

    // See if the projection is close enough to center
    if (Util::IsFltLE(std::abs(y), kYTol)) {
      if (DEBUG_HEAD_ANGLE_ITERATIONS) {
        LOG_DEBUG("ComputeHeadAngle", "CONVERGED: %.1fdeg",
                  RAD_TO_DEG(searchAngle_rad));
      }
...
Result Robot::ComputeTurnTowardsImagePointAngles(
  ...
  if (RESULT_OK != result) {
    LOG_WARNING(
        "Robot.ComputeTurnTowardsImagePointAngles.ComputeHistPoseFailed",
        "t=%u", (TimeStamp_t)timestamp);
    absPanAngle = GetPose().GetRotation().GetAngleAroundZaxis();
    absTiltAngle = GetComponent<FullRobotPose>().GetHeadAngle();
    return result;
  }

  absTiltAngle = std::atan2f(-pt.y(), calib->GetFocalLength_y()) +
                 histState.GetHeadAngle_rad();
  absPanAngle = std::atan2f(-pt.x(), calib->GetFocalLength_x()) +
                histState.GetPose().GetRotation().GetAngleAroundZaxis();

  return RESULT_OK;
}

void Robot::DevReplaceAIComponent(AIComponent* aiComponent, bool shouldManage) {
  IDependencyManagedComponent<Anki::Vector::RobotComponentID>* explicitUpcast =
      aiComponent;
  _components->DevReplaceDependentComponent(RobotComponentID::AIComponent,
                                            explicitUpcast, shouldManage);
}
...
#define HANDLE_RESULT(res, nextState)                                     \
  {                                                                       \
    if (res != ToFSensor::CommandResult::Success) {                       \
      PRINT_NAMED_ERROR("Robot.UpdateToFStartupChecks.Fail", "State: %u", \
                        state);                                           \
  ...
  // This will handle failing should we not get any valid ROIs or for some
  // reason one of the command callbacks is not called (never seen it happen but
  // who knows...)
...
bool Robot::UpdateGyroCalibChecks(Result& res) {
  ...
  // should be displaying the low battery image to encourage user to put the
  // robot down. Note that by the time that the sync has been sent, the face has
  // already been blank for around 7 seconds.
  ...
  if (!displayedImage && _syncRobotSentTime_sec > 0 &&
    ...
    // but we haven't received syncTime yet likely because the gyro hasn't
    // calibrated
    GetAnimationComponent().Init();

    static const std::string kGyroNotCalibratedImg =
        "config/sprites/independentSprites/battery_low.png";
    const std::string imgPath = GetContextDataPlatform()->pathToResource(
        Anki::Util::Data::Scope::Resources, kGyroNotCalibratedImg);
    Vision::ImageRGB img;
    img.Load(imgPath);
    // Display the image indefinitely or atleast until something else is
    // displayed
    GetAnimationComponent().DisplayFaceImage(img, 0, true);
    // Move the head to look up to show the image clearly
    GetMoveComponent().MoveHeadToAngle(MAX_HEAD_ANGLE, MAX_HEAD_SPEED_RAD_PER_S,
                                       MAX_HEAD_ACCEL_RAD_PER_S2, 1.f);
    displayedImage = true;
  }
...
void Robot::Shutdown(ShutdownReason reason) {
  if (_toldToShutdown) {
    LOG_WARNING("Robot.Shutdown.AlreadyShuttingDown", "Ignoring new reason %s",
                EnumToString(reason));
    return;
  }
