 ...
 * Date:   06/20/16
 *
 * Description: Implements CustomObject which is an object type that is created
 *from external sources, such as via the SDK They can optionally be created with
 *markers associated with them so they are observable in the world.
 *
 *
...
class CustomObject : public ObservableObject {
 public:
  // NOTE: You cannot directly construct a CustomObject, but instead must use
  // one of these static Create methods

  // Creates a fully custom box with a specific marker on each side
  static CustomObject* CreateBox(
  ...
  // Create a box with no markers (not actually observable, but can be treated
  // as a fixed obstacle)
  static CustomObject* CreateFixedObstacle(f32 xSize_mm, f32 ySize_mm,
                                           f32 zSize_mm);

  virtual ~CustomObject();

  static Vision::MarkerType GetVisionMarkerType(
 ...
 private:
  ...
  constexpr static const f32 kWallThickness_mm =
      10.f;  // x dimension of "walls"

  CustomObject(ObjectType objectType, CustomObjectMarker markerFront,
               CustomObjectMarker markerBack, CustomObjectMarker markerTop,
               CustomObjectMarker markerBottom, CustomObjectMarker markerLeft,
               CustomObjectMarker markerRight, f32 xSize_mm, f32 ySize_mm,
               f32 zSize_mm, f32 markerWidth_mm, f32 markerHeight_mm,
               bool isUnique, CustomShape shape);
...
inline CustomObject* CustomObject::CloneType() const {
  return new CustomObject(
      this->GetType(), _markersByFace[FrontFace], _markersByFace[BackFace],
      _markersByFace[TopFace], _markersByFace[BottomFace],
      _markersByFace[LeftFace], _markersByFace[RightFace], _size.x(), _size.y(),
      _size.z(), _markerSize.x(), _markerSize.y(), _isUnique, _customShape);
}
