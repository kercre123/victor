 ...
 * Created: 2017-09-11
 *
 * Description: Component for consuming new sensor data and processing the
 *information into the appropriate map objects
 *
...
// of DEV_CHEATS / NDEBUG, etc.
#define ENABLE_DRAWING ANKI_DEV_CHEATS

namespace Anki {
namespace Vector {

// how often we request redrawing maps. Added because I think clad is getting
// overloaded with the amount of quads
CONSOLE_VAR(float, kMapRenderRate_sec, "MapComponent", 0.25f);

// kObjectRotationChangeToReport_deg: if the rotation of an object changes by
// this much, memory map will be notified
CONSOLE_VAR(float, kObjectRotationChangeToReport_deg, "MapComponent", 10.0f);
// kObjectPositionChangeToReport_mm: if the position of an object changes by
// this much, memory map will be notified
CONSOLE_VAR(float, kObjectPositionChangeToReport_mm, "MapComponent", 5.0f);

// kNeverMergeOldMaps: if set to false, we only relocalize if the robot is in
// the same world origin as the previous map
CONSOLE_VAR(bool, kMergeOldMaps, "MapComponent", false);

// kRobotRotationChangeToReport_deg: if the rotation of the robot changes by
// this much, memory map will be notified
CONSOLE_VAR(float, kRobotRotationChangeToReport_deg, "MapComponent", 20.0f);
// kRobotPositionChangeToReport_mm: if the position of the robot changes by this
// much, memory map will be notified
...
CONSOLE_VAR(float, kCliffTimeout_ms, "MapComponent",
            1200.f * 1000);  // 20 minutes

// the length and half width of two triangles used in FlagProxObstaclesUsingPose
// (see method)
...
CONSOLE_VAR(float, kHoughAngleResolution_deg,
            "MapComponent.VisualEdgeDetection", 2.0);
CONSOLE_VAR(int, kHoughAccumThreshold, "MapComponent.VisualEdgeDetection", 20);
CONSOLE_VAR(float, kHoughMinLineLength_mm, "MapComponent.VisualEdgeDetection",
            40.0);
CONSOLE_VAR(float, kHoughMaxLineGap_mm, "MapComponent.VisualEdgeDetection",
            10.0);
CONSOLE_VAR(float, kEdgeLineLengthToInsert_mm,
            "MapComponent.VisualEdgeDetection", 200.f);
CONSOLE_VAR(float, kVisionCliffPadding_mm, "MapComponent.VisualEdgeDetection",
            20.f);

CONSOLE_VAR(int, kMaxPixelsUsedForHoughTransform,
            "MapComponent.VisualEdgeDetection", 160000);  // 400 x 400 max size

namespace {

// return the content type we would set in the memory type for each object type
MemoryMapTypes::EContentType ObjectTypeToMemoryMapContentType(ObjectType type,
                                                              bool isAdding) {
  using ContentType = MemoryMapTypes::EContentType;
  ContentType retType = ContentType::Unknown;
  if (IsBlockType(type, false) || IsCustomType(type, false) ||
      IsChargerType(type, false)) {
    retType = isAdding ? ContentType::ObstacleObservable
                       : ContentType::ClearOfObstacle;
  }
...
decltype(auto) GetChargerRegion(const Pose3d& poseWRTRoot) {
  // grab the cannonical corners and then apply the transformation. If we use
  // `GetBoundingQuadXY`, we no longer know where the "back" is. Unfortunately,
  // order matters here, and for corners on the ground plane, the order is (from
  // charger.cpp):
  ...
  // points for calculating the collision area of a charger, which is different
  // from the physical bounding box
  ...
  const Point2f exteriorBL =
      poseWRTRoot * (corners[0] + kExteriorChargerOffsetBL);
  const Point2f exteriorFL =
      poseWRTRoot * (corners[1] + kExteriorChargerOffsetFL);
  const Point2f exteriorFR =
      poseWRTRoot * (corners[2] + kExteriorChargerOffsetFR);
  const Point2f exteriorBR =
      poseWRTRoot * (corners[3] + kExteriorChargerOffsetBR);
  const Point2f interiorBL =
      poseWRTRoot * (corners[0] + kInteriorChargerOffsetBL);
  const Point2f interiorFL =
      poseWRTRoot * (corners[1] + kInteriorChargerOffsetFL);
  const Point2f interiorFR =
      poseWRTRoot * (corners[2] + kInteriorChargerOffsetFR);
  const Point2f interiorBR =
      poseWRTRoot * (corners[3] + kInteriorChargerOffsetBR);

  // only want to flag the back and sides of the charger, so define each side as
  // a separate trapezoid as seen in the diagram above
  return MakeUnion2f(
...
decltype(auto) GetHabitatRegion(const Pose3d& poseWRTRoot) {
  ...
  // points for calculating the collision area of a habitat, relative to charger
  // pose
  ...
  // a separate trapezoid as seen in the diagram above
  return MakeUnion2f(
      FastPolygon({actualExteriorBack, actualExteriorLeft, actualInteriorLeft,
                   actualInteriorBack}),  // back-left
      FastPolygon({actualExteriorBack, actualExteriorRight, actualInteriorRight,
                   actualInteriorBack}),  // back-right
      FastPolygon({actualExteriorFront, actualExteriorLeft, actualInteriorLeft,
                   actualInteriorFront}),  // front-left
      FastPolygon({actualExteriorFront, actualExteriorRight,
                   actualInteriorRight, actualInteriorFront}));  // front-right
}
...
void DevProcessOneFrameForVisionEdges(ConsoleFunctionContextRef context) {
  if (consoleRobot != nullptr) {
    consoleRobot->GetActionList().QueueAction(
        QueueActionPosition::NOW,
        new WaitForImagesAction(1, VisionMode::OverheadEdges));
  }
}
...
void DevDrawCliffPoses(ConsoleFunctionContextRef context) {
  if (consoleRobot != nullptr) {
    ...
    MemoryMapTypes::NodePredicate isDropSensorCliff =
        [](MemoryMapTypes::MemoryMapDataConstPtr nodeData) -> bool {
      const bool isValidCliff =
          nodeData->type == MemoryMapTypes::EContentType::Cliff;
      return isValidCliff;
    };
    ...
    for (auto& node : cliffNodes) {
      Pose3d pose =
          MemoryMapData::MemoryMapDataCast<MemoryMapData_Cliff>(node)->pose;
      // set z-height above the map when rendering
      Point3f renderPoint(pose.GetTranslation().x(), pose.GetTranslation().y(),
                          3.f);
...
CONSOLE_FUNC(DevProcessOneFrameForVisionEdges,
             "MapComponent.VisualEdgeDetection");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void MapComponent::InitDependent(Vector::Robot* robot,
  ...
  if (_robot->HasExternalInterface()) {
    ...
    helper.SubscribeGameToEngine<
        MessageGameToEngineTag::SetMemoryMapRenderEnabled>();
    helper.SubscribeGameToEngine<
        MessageGameToEngineTag::SetMemoryMapBroadcastFrequency_sec>();
  }
    ...
    if (webService != nullptr) {
      auto onData =
          [this](const Json::Value& data,
                 const std::function<void(const Json::Value&)>& sendFunc) {
            _webMessageDirty = true;
          };
      _eventHandles.emplace_back(
          webService->OnWebVizData(kWebVizModuleName).ScopedSubscribe(onData));
    }
  }
}
...
// - - - - - - - - - - - - - - - - - - - -
template <>
void MapComponent::HandleMessage(
    const ExternalInterface::SetMemoryMapRenderEnabled& msg) {
  SetRenderEnabled(msg.enabled);
};

template <>
void MapComponent::HandleMessage(
    const ExternalInterface::SetMemoryMapBroadcastFrequency_sec& msg) {
...
void MapComponent::UpdateDependent(const RobotCompMap& dependentComps) {
  ...
  if (currentNavMemoryMap) {
    ...
    const f32 currentTime_s =
        BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();

    const bool shouldSendViz =
        (ENABLE_DRAWING && _vizMessageDirty && _isRenderEnabled) ||
        _webMessageDirty;
    const bool shouldSendSDK =
        _gameMessageDirty && (_broadcastRate_sec >= 0.0f);
    ...
    if (shouldSendViz) {
      ...
      if (doViz) {
        ...
        nextDrawTime_s +=
            ((int)(currentTime_s - nextDrawTime_s) / kMapRenderRate_sec + 1) *
            kMapRenderRate_sec;
    ...
    if (shouldSendSDK) {
      ...
      if (FLT_LE(nextBroadcastTime_s, currentTime_s)) {
        ...
        // Reset the timer but don't accumulate error
        nextBroadcastTime_s +=
            ((int)(currentTime_s - nextBroadcastTime_s) / _broadcastRate_sec +
             1) *
            _broadcastRate_sec;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void MapComponent::UpdateMapOrigins(PoseOriginID_t oldOriginID,
                                    PoseOriginID_t newOriginID) {
  // oldOrigin is the pointer/id of the map we were just building, and it's
  // going away. It's the current map newOrigin is the pointer/id of the map
  // that is staying, it's the one we rejiggered to, and we haven't changed in a
  // while
  auto oldMapIter = _navMaps.find(oldOriginID);
  auto newMapIter = _navMaps.find(newOriginID);

  const Pose3d& oldOrigin =
      _robot->GetPoseOriginList().GetOriginByID(oldOriginID);
  const Pose3d& newOrigin =
      _robot->GetPoseOriginList().GetOriginByID(newOriginID);
  ...
  ANKI_VERIFY(oldOriginID == _currentMapOriginID,
              ...
              "rejiggering map %d, but currentID = %d", oldOriginID,
              _currentMapOriginID);

  // maps have changed, so make sure to clear all the renders
  ClearRender();

  // before we merge the object information from the memory maps, apply
  // rejiggering also to their reported poses
  ...
  newMapIter->second.activationTime_ms =
  ...
  if (oldMapIter != _navMaps.end()) {
    ...
    UpdateBroadcastFlags(
        newMapIter->second.map->Merge(*(oldMapIter->second.map), oldWrtNew));

    newMapIter->second.activeDuration_ms +=
        oldMapIter->second.activeDuration_ms;
...
void MapComponent::UpdateRobotPose() {
  ...
  const Point3f distThreshold(kRobotPositionChangeToReport_mm,
  ...
  const bool isFarFromPrev =
      !isPrevSet || (!robotPoseWrtOrigin.IsSameAs(
                        _reportedRobotPose, distThreshold, angleThreshold));
  ...
  if (addAgain) {
    ...
    InsertData(Poly2f(robotSensorQuad),
               MemoryMapData(EContentType::ClearOfCliff, currentTimestamp));

    const Quad2f& robotQuad = _robot->GetBoundingQuadXY(robotPoseWrtOrigin);

    // regular clear of obstacle
    InsertData(Poly2f(robotQuad),
               MemoryMapData(EContentType::ClearOfObstacle, currentTimestamp));

    _robot->GetAIComponent().GetComponent<AIWhiteboard>().ProcessClearQuad(
        robotQuad);
...
void MapComponent::TimeoutObjects() {
  ...
  if (currentNavMemoryMap) {
    ...
    const RobotTimeStamp_t unrecognizedTooOld =
        (currentTime <= kUnrecognizedTimeout_ms)
            ? 0
            : currentTime - kUnrecognizedTimeout_ms;
    const RobotTimeStamp_t visionTooOld = (currentTime <= kVisionTimeout_ms)
                                              ? 0
                                              : currentTime - kVisionTimeout_ms;
    const RobotTimeStamp_t proxTooOld =
        (currentTime <= kProxTimeout_ms) ? 0 : currentTime - kProxTimeout_ms;
    const RobotTimeStamp_t cliffTooOld =
        (currentTime <= kCliffTimeout_ms) ? 0 : currentTime - kCliffTimeout_ms;

    NodeTransformFunction timeoutObjects =
        [unrecognizedTooOld, visionTooOld, proxTooOld,
         cliffTooOld](MemoryMapDataPtr data) -> MemoryMapDataPtr {
      ...
      if ((EContentType::Cliff == nodeType && lastObs <= cliffTooOld) ||
          (EContentType::ObstacleUnrecognized == nodeType &&
           lastObs <= unrecognizedTooOld) ||
          (EContentType::InterestingEdge == nodeType &&
           lastObs <= visionTooOld) ||
          (EContentType::NotInterestingEdge == nodeType &&
           lastObs <= visionTooOld) ||
...
void MapComponent::FlagGroundPlaneROIInterestingEdgesAsUncertain() {
  ...
  DEV_ASSERT(
      currentNavMemoryMap,
      "MapComponent.FlagGroundPlaneROIInterestingEdgesAsUncertain.NullMap");

  NodeTransformFunction transform =
      [](MemoryMapDataPtr oldData) -> MemoryMapDataPtr {
...
void MapComponent::FlagQuadAsNotInterestingEdges(const Quad2f& quadWRTOrigin) {
  InsertData(Poly2f(quadWRTOrigin),
             MemoryMapData(EContentType::NotInterestingEdge,
                           _robot->GetLastImageTimeStamp()));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void MapComponent::FlagInterestingEdgesAsUseless() {
  // flag all content as Unknown: ideally we would add a new type
  // (SmallInterestingEdge), so that we know we detected something, but we
  // discarded it because it didn't have enough info; however that increases
  // complexity when raycasting, finding boundaries, readding edges, etc. By
  // flagging Unknown we simply say "there was something here, but we are not
  // sure what it was", which can be good to re-explore the area

  auto currentNavMemoryMap = GetCurrentMemoryMap();
  DEV_ASSERT(currentNavMemoryMap,
             "MapComponent.FlagInterestingEdgesAsUseless.NullMap");
...
void MapComponent::FlagProxObstaclesUsingPose() {
  ...
  // coverage of the prox sensor, but scaled differently. Any prox obstacle that
  // it covers will be marked as explored. Since it looks better if the robot
  // comes close to an object in order to mark it as explored, the poly isn't as
  // tall as the prox sensor's reach.

  const auto& rot = pose.GetRotation();
  Vec3f offset1(kProxExploredTriangleLength_mm,
                kProxExploredTriangleHalfWidth_mm, 0);
  Vec3f offset2(kProxExploredTriangleLength_mm,
                -kProxExploredTriangleHalfWidth_mm, 0);
  const Point2f p1 = pose.GetTranslation();
  const Point2f p2 =
      (pose.GetTransform() * Transform3d(rot, offset1)).GetTranslation();
  const Point2f p3 =
      (pose.GetTransform() * Transform3d(rot, offset2)).GetTranslation();
  const FastPolygon triangleExplored({p1, p2, p3});

  // mark any prox obstacle in triangleExplored as explored
  NodeTransformFunction exploredFunc =
      [](MemoryMapDataPtr data) -> MemoryMapDataPtr {
    if (data->type == EContentType::ObstacleProx) {
      auto castPtr =
          MemoryMapData::MemoryMapDataCast<MemoryMapData_ProxObstacle>(data);
      castPtr->MarkExplored();
    }
    return data;
  };
  const bool addedExplored =
      currentNavMemoryMap->TransformContent(exploredFunc, triangleExplored);
...
bool MapComponent::FlagProxObstaclesTouchingExplored() {
  ...
  // mark any NOT_EXPLORED ObstacleProx that is touching an EXPLORED
  // ObstacleProx quad as EXPLORED
  NodePredicate innerCheckFunc = [](MemoryMapDataConstPtr inside) {
    if (inside->type == EContentType::ObstacleProx) {
      auto castInside =
          MemoryMapData::MemoryMapDataCast<const MemoryMapData_ProxObstacle>(
              inside);
  ...
  MemoryMapData_ProxObstacle toAdd(MemoryMapData_ProxObstacle::EXPLORED,
                                   {0.0f, 0.0f, 0.0f},
                                   _robot->GetLastImageTimeStamp());
  const bool changedBorder = currentNavMemoryMap->FillBorder(
      innerCheckFunc, outerCheckFunc, toAdd.Clone());
...
void MapComponent::CreateLocalizedMemoryMap(PoseOriginID_t worldOriginID) {
  ...
  // Since we are going to create a new memory map, check if any of the existing
  // ones have become a zombie This could happen if either the current map never
  // saw a localizable object, or if objects in previous maps have been moved or
  // deactivated, which invalidates them as localizable
  ...
  while (iter != _navMaps.end()) {
    ...
    const bool isZombie =
        _robot->GetBlockWorld().IsZombiePoseOrigin(iter->first) ||
        !kMergeOldMaps;
    if (isZombie) {
      LOG_INFO("MapComponent.memory_map.deleting_zombie_map", "%d",
               worldOriginID);
      // also remove the reported poses in this origin for every object (fixes a
      // leak, and better tracks where objects are)
      for (auto& posesForObjectIt : _reportedPoses) {
        OriginToPoseInMapInfo& posesPerOriginForObject =
            posesForObjectIt.second;
        const PoseOriginID_t zombieOriginID = iter->first;
        posesPerOriginForObject.erase(zombieOriginID);
      }
      iter = _navMaps.erase(iter);
    } else {
      LOG_INFO("MapComponent.memory_map.keeping_alive_map", "%d",
               worldOriginID);
      ++iter;
    }
  }

  // if the origin is null, we would never merge the map, which could leak if a
  // new one was created do not support this by not creating one at all if the
  // origin is null
  if (PoseOriginList::UnknownOriginID != worldOriginID) {
    const EngineTimeStamp_t currTimeStamp_ms =
        BaseStationTimer::getInstance()->GetCurrentTimeStamp();
    if (_currentMapOriginID != PoseOriginList::UnknownOriginID) {
      ...
      if (it != _navMaps.end()) {
        ...
        it->second.activeDuration_ms +=
            TimeStamp_t(currTimeStamp_ms - it->second.activationTime_ms);
      }
    }

    // create a new memory map in the given origin
    LOG_INFO("MapComponent.CreateLocalizedMemoryMap",
             "Setting current origin to %i", worldOriginID);
...
const size_t kReservedBytes =
    1 + 2;  // Message overhead for:  Tag, and vector size
...
const size_t kQuadsPerMessage =
    kMaxBufferForQuads / sizeof(QuadInfoVector::value_type);
const size_t kFullQuadsPerMessage =
    kMaxBufferForQuads / sizeof(QuadInfoFullVector::value_type);

static_assert(kQuadsPerMessage > 0,
              "MapComponent.Broadcast.InvalidQuadsPerMessage");
static_assert(kFullQuadsPerMessage > 0,
              "MapComponent.Broadcast.InvalidFullQuadsPerMessage");
}  // namespace
...
void MapComponent::BroadcastMapToViz(const MapBroadcastData& mapData) const {
  ...
  _robot->Broadcast(MessageViz(
      MemoryMapMessageVizBegin(_currentMapOriginID, mapData.mapInfo)));
  // chunk the quad messages
  for (u32 seqNum = 0;
       seqNum * kFullQuadsPerMessage < mapData.quadInfoFull.size(); seqNum++) {
    auto start = seqNum * kFullQuadsPerMessage;
    auto end =
        std::min(mapData.quadInfoFull.size(), start + kFullQuadsPerMessage);
    _robot->Broadcast(MessageViz(MemoryMapMessageViz(
        _currentMapOriginID,
        QuadInfoFullVector(mapData.quadInfoFull.begin() + start,
                           mapData.quadInfoFull.begin() + end))));
  }
...
void MapComponent::BroadcastMapToWeb(const MapBroadcastData& mapData) const {
  auto* webService = _robot->GetContext()->GetWebService();
  if (webService == nullptr ||
      !webService->IsWebVizClientSubscribed(kWebVizModuleName)) {
    return;
  }
  ...
  // chunk the quad messages
  for (u32 seqNum = 0; seqNum * kQuadsPerMessage < mapData.quadInfo.size();
       seqNum++) {
    ...
    for (auto it = mapData.quadInfo.begin() + start;
         it != mapData.quadInfo.begin() + end; ++it) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void MapComponent::BroadcastMapToSDK(
    const MemoryMapTypes::MapBroadcastData& mapData) const {
  using namespace ExternalInterface;

  // Send the begin message
  _robot->Broadcast(MessageEngineToGame(MemoryMapMessageBegin(
      _currentMapOriginID, mapData.mapInfo.rootDepth,
      mapData.mapInfo.rootSize_mm, mapData.mapInfo.rootCenterX,
      mapData.mapInfo.rootCenterY)));
  ...
  for (u32 seqNum = 0; seqNum * kQuadsPerMessage < mapData.quadInfo.size();
    ...
    auto start = seqNum * kQuadsPerMessage;
    auto end = std::min(mapData.quadInfo.size(), start + kQuadsPerMessage);
    _robot->Broadcast(MessageEngineToGame(MemoryMapMessage(QuadInfoVector(
        mapData.quadInfo.begin() + start, mapData.quadInfo.begin() + end))));
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
void MapComponent::SendDASInfoAboutMap(
    const PoseOriginID_t& mapOriginID) const {
  if (mapOriginID != PoseOriginList::UnknownOriginID) {
    ...
    if (ANKI_VERIFY((it != _navMaps.end()) && (it->second.map != nullptr),
                    ...
                    mapOriginID)) {
      const float explored_mm2 =
          1e6f * it->second.map->GetExploredRegionAreaM2();
      const float collision_mm2 = it->second.map->GetArea(
          [](const auto& data) { return data->IsCollisionType(); });
      TimeStamp_t activeDuration_ms = 0;
      if (mapOriginID == _currentMapOriginID) {
        // still active, so need to append the time since activated to the
        // duration
        ...
        activeDuration_ms = it->second.activeDuration_ms +
                            TimeStamp_t(currTime_ms - activationTime);
      } else {
        // inactive. just use the cached activation time
        activeDuration_ms = it->second.activeDuration_ms;
      }

      DASMSG(robot_delocalized_map_info, "robot.delocalized_map_info",
             "When the robot is delocalized, this contains information about "
             "the nav map. This occurs when the robot "
             "delocalizes due to being picked up");
      DASMSG_SET(i1, (int64_t)explored_mm2, "Total surface area known (mm2)");
      DASMSG_SET(
          i2, (int64_t)collision_mm2,
          "Total surface area that is an obstacle (mm2), a subset of i1");
      DASMSG_SET(
          i3, (int64_t)activeDuration_ms,
          "Duration (ms) of the map, perhaps after multiple delocalizations");
      DASMSG_SEND();
    }
  }
}
...
std::shared_ptr<INavMap> MapComponent::GetCurrentMemoryMapHelper() const {
  ...
  const bool validOrigin =
      (PoseOriginList::UnknownOriginID == _currentMapOriginID) ||
      (_robot->GetPoseOriginList().GetCurrentOriginID() == _currentMapOriginID);

  ANKI_VERIFY(validOrigin, "MemoryMap.GetCurrentMap.BadOrigin",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void MapComponent::UpdateObjectPose(const ObservableObject& object,
                                    const Pose3d* oldPose,
                                    PoseState oldPoseState) {
  // TODO (VIC-13789): Clean this method up (e.g., no need to pass a pointer to
  // oldPose)

  const ObjectID& objectID = object.GetID();
  DEV_ASSERT(objectID.IsSet(),
             "MapComponent.OnObjectPoseChanged.InvalidObjectID");
  ...
  if (!oldValid && newValid) {
    ...
    // updating the pose of an object, decide if we update the report. As an
    // optimization, we don't update it if the poses are close enough
    const int objectIdInt = objectID.GetValue();
    const OriginToPoseInMapInfo& reportedPosesForObject =
        _reportedPoses[objectIdInt];
    const PoseOrigin& curOrigin = object.GetPose().FindRoot();
    const PoseOriginID_t curOriginID = curOrigin.GetID();
    DEV_ASSERT_MSG(
        _robot->GetPoseOriginList().ContainsOriginID(curOriginID),
        "MapComponent.OnObjectPoseChanged.ObjectOriginNotInOriginList", "ID:%d",
        curOriginID);
    const auto poseInNewOriginIter = reportedPosesForObject.find(curOriginID);

    if (poseInNewOriginIter != reportedPosesForObject.end()) {
      // note that for distThreshold, since Z affects whether we add to the
      // memory map, distThreshold should be smaller than the threshold to not
      // report
      DEV_ASSERT(kObjectPositionChangeToReport_mm <
                     object.GetDimInParentFrame<'Z'>() * 0.5f,
                 "OnObjectPoseChanged.ChangeThresholdTooBig");
      const float distThreshold = kObjectPositionChangeToReport_mm;
      const Radians angleThreshold(
          DEG_TO_RAD(kObjectRotationChangeToReport_deg));

      // compare new pose with previous entry and decide if isFarFromPrev
      const PoseInMapInfo& info = poseInNewOriginIter->second;
      const bool isFarFromPrev =
          (!info.isInMap ||
           (!object.GetPose().IsSameAs(info.pose, Point3f(distThreshold),
                                       angleThreshold)));
    ...
    // deleting an object, remove its report using oldOrigin (the origin it was
    // removed from)
    ...
    PRINT_NAMED_ERROR("MapComponent.OnObjectPoseChanged.BothStatesAreInvalid",
                      "Object %d changing from Invalid to Invalid",
                      objectID.GetValue());
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
void MapComponent::AddObservableObject(const ObservableObject& object,
                                       const Pose3d& newPose) {
  const auto& objectType = object.GetType();
  const MemoryMapTypes::EContentType addType =
      ObjectTypeToMemoryMapContentType(objectType, true);
  if (addType == MemoryMapTypes::EContentType::Unknown) {
    ...
    PRINT_CH_INFO("MapComponent",
                  "MapComponent.AddObservableObject.InvalidAddType",
  ...
  if (memoryMap) {
    // in order to properly handle stacks, do not add the quad to the memory map
    // for objects that are not on the floor
    Pose3d objWrtRobot;
    if (newPose.GetWithRespectTo(_robot->GetPose(), objWrtRobot)) {
      const bool isFloating =
          object.IsPoseTooHigh(objWrtRobot, 1.f, STACKED_HEIGHT_TOL_MM, 0.f);
      if (isFloating) {
        // store in as a reported pose, but set as not in map (the pose value is
        // not relevant)
        ...
        if (IsChargerType(objectType, false)) {
          const bool inHabitat =
              (_robot->GetHabitatDetectorComponent().GetHabitatBeliefState() ==
               HabitatBeliefState::InHabitat);
          MemoryMapData_ObservableObject data(object, boundingPoly,
                                              _robot->GetLastImageTimeStamp());

          if (inHabitat) {
            const auto region = MakeUnion2f(GetChargerRegion(newPoseWrtOrigin),
                                            GetHabitatRegion(newPoseWrtOrigin));
          ...
          // eventually we will want to store multiple ID's to the node data in
          // the case for multiple blocks however, we have no mechanism for
          // merging data, so for now we just replace with the new id
          MemoryMapData_ObservableObject data(object, boundingPoly,
                                              _robot->GetLastImageTimeStamp());
          InsertData(boundingPoly, data);
        } else {
          PRINT_NAMED_WARNING(
              "MapComponent.AddObservableObject.AddedNonObservableType",
              "AddObservableObject was called to add a non observable object");
          InsertData(boundingPoly,
                     MemoryMapData(addType, _robot->GetLastImageTimeStamp()));
        }

        // store in as a reported pose
        _reportedPoses[objectId][originID] =
            PoseInMapInfo(newPoseWrtOrigin, true);
      }
    } else {
      // should not happen, so warn about it
      PRINT_NAMED_WARNING(
          "MapComponent.AddObservableObject.InvalidPose",
          "Could not get object's new pose wrt robot. Won't add to map");
    }
  } else {
    // if the map was removed (for zombies), we shouldn't be asking to add an
    // object to it
    PRINT_NAMED_ERROR(
        "MapComponent.AddObservableObject.NoMapForOrigin",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
void MapComponent::RemoveObservableObject(const ObservableObject& object,
                                          PoseOriginID_t originID) {
  using namespace MemoryMapTypes;

  const auto& objectType = object.GetType();
  const MemoryMapTypes::EContentType removalType =
      ObjectTypeToMemoryMapContentType(objectType, false);
  if (removalType == EContentType::Unknown) {
    // this is not ok, this object type can be added but can't be removed from
    // the map
    PRINT_NAMED_WARNING(
        "MapComponent.RemoveObservableObject.InvalidRemovalType",
  ...
  if (matchPair != _navMaps.end()) {
    ...
    NodeTransformFunction transform = [id, &clearData](MemoryMapDataPtr data) {
      if (data->type == EContentType::ObstacleObservable) {
        // eventually we will want to store multiple ID's to the node data in
        // the case for multiple blocks however, we have no mechanism for
        // merging data, so for now we are just completely replacing the
        // NodeContent if the ID matches.
        if (MemoryMapData::MemoryMapDataCast<
                const MemoryMapData_ObservableObject>(data)
                ->id == id) {
          return clearData;
        }
      }
    ...
    // if the map was removed (for zombies), we shouldn't be asking to remove an
    // object from it
    DEV_ASSERT(matchPair == _navMaps.end(),
               "MapComponent.RemoveObservableObject.NoMapForOrigin");
  }
}
...
void MapComponent::MarkObjectUnobserved(const ObservableObject& object) {
  ...
  if (currentNavMemoryMap) {
    const ObjectID id = object.GetID();
    PRINT_CH_INFO("MapComponent", "MapComponent.MarkObjectUnobserved",
                  "Marking observable object %d as unobserved", (int)id);

    NodeTransformFunction transform = [id](MemoryMapDataPtr data) {
      if (data->type == EContentType::ObstacleObservable) {
        auto objectData =
            MemoryMapData::MemoryMapDataCast<MemoryMapData_ObservableObject>(
                data);
        if (objectData->id == id) {
          objectData->MarkUnobserved();
        }
      }
      return data;
    };
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
void MapComponent::UpdateOriginsOfObjects(PoseOriginID_t curOriginID,
                                          PoseOriginID_t relocalizedOriginID) {
  // Origins have changed, and some maps may be merged, so make sure to clear
  // everything
  ClearRender();

  // for every object in the current map, we have a decision to make. We are
  // going to bring that memory map into what is becoming the current one. That
  // means also bringing the last reported pose of every object onto the new
  // map. The current map is obviously more up to date than the map we merge
  // into, since the map we merge into is map we identified a while ago. This
  // means that if an object moved and we now know where it is, the good pose is
  // in the currentMap, not in the mapWeMergeInto. So, for every object in the
  // currentMap we are going to remove their pose from the mapWeMergeInto. This
  // will make the map we merge into gain the new info, at the same time that we
  // remove info known to not be the most accurate
  ...
  for (auto& pairIdToPoseInfoByOrigin : _reportedPoses) {
    ...
    const ObservableObject* object =
        _robot->GetBlockWorld().GetLocatedObjectByID(
            pairIdToPoseInfoByOrigin.first);
    if (nullptr == object) {
      PRINT_CH_INFO(
          "MapComponent", "MapComponent.UpdateOriginsOfObjects.NotAnObject",
          "Could not find object ID '%d' in MapComponent updating their quads",
          pairIdToPoseInfoByOrigin.first);
      continue;
    }

    // find the reported pose for this object in the current origin
    OriginToPoseInMapInfo& poseInfoByOriginForObj =
        pairIdToPoseInfoByOrigin.second;
    const auto& matchInCurOrigin = poseInfoByOriginForObj.find(curOriginID);
    const bool isObjectReportedInCurrent =
        (matchInCurOrigin != poseInfoByOriginForObj.end());
    if (isObjectReportedInCurrent) {
      // we have an entry in the current origin. We don't care if `isInMap` is
      // true or false. If it's true it means we have a better position
      // available in this frame, if it's false it means we saw the object in
      // this frame, but somehow it became unknown. If it became unknown, the
      // position it had in the origin we are relocalizing to is old and not to
      // be trusted. This is the reason why we don't erase reported poses, but
      // rather flag them as !isInMap. Additionally we don't have to worry about
      // the container we are iterating changing, since iterators are not
      // affected by changing a boolean, but are if we erased from it.
      RemoveObservableObject(*object, relocalizedOriginID);

      // we are bringing over the current info into the relocalized origin,
      // update the reported pose in the relocalized origin to be that of the
      // newest information
      poseInfoByOriginForObj[relocalizedOriginID].isInMap =
          matchInCurOrigin->second.isInMap;
      if (matchInCurOrigin->second.isInMap) {
        // bring over the pose if it's in map (otherwise we don't care about the
        // pose) when we bring it, flatten out to the relocalized origin
        DEV_ASSERT(_robot->GetPoseOriginList()
                       .GetOriginByID(relocalizedOriginID)
                       .HasSameRootAs(matchInCurOrigin->second.pose),
                   "MapComponent.UpdateOriginsOfObjects.PoseDidNotHookGrandpa");
        poseInfoByOriginForObj[relocalizedOriginID].pose =
            matchInCurOrigin->second.pose.GetWithRespectToRoot();
      }
      // also, erase the current origin from the reported poses of this object,
      // since we will never use it after this Note this should not alter the
      // iterators we are using for _reportedPoses
      poseInfoByOriginForObj.erase(curOriginID);
    } else {
      // we don't have this object in the current memory map. The info from this
      // object if at all is in the previous origin (then one we are
      // relocalizing to), or another origin not related to these two, do
...
void MapComponent::ClearRobotToMarkers(const ObservableObject* object) {
  ...
  DEV_ASSERT(
      object->GetPose().IsChildOf(_robot->GetWorldOrigin()),
  ...
  // only clear to the markers, since for custom object types, the object might
  // be significantly larger than the marker
  for (const auto& observedMarkerIt : observedMarkers) {
    // NOTE: (mrw) We are making assumptions here that the marker is both normal
    // to the map plane, and is oriented
    //       to a 90° angle (up/down/left/right). Additionally, this will clear
    //       all the way to the marker, so even if the object's physical
    //       properties extend in front of the marker, we might be overwritting
    //       that region with `ClearOfObstacle` state. This is particularly
    //       noticeable for the charger, but at the time of writing this, it is
    //       not interfering with any docking behavior.
    const Quad3f& markerCorners = observedMarkerIt->Get3dCorners(
        observedMarkerIt->GetPose().GetWithRespectToRoot());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
void MapComponent::ClearRobotToEdge(const Point2f& p, const Point2f& q,
                                    const RobotTimeStamp_t t) {
  auto currentMap = GetCurrentMemoryMap();
  if (currentMap) {
    // NOTE: (MRW) currently using robot pose center, though to be correct we
    // should use the center of the
    ...
    const Point2f r1 =
        (_robot->GetPose().GetTransform() * Transform3d(rot, rayOffset1))
            .GetTranslation();
    const Point2f r2 =
        (_robot->GetPose().GetTransform() * Transform3d(rot, rayOffset2))
            .GetTranslation();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
void MapComponent::ClearRegion(const BoundedConvexSet2f& region,
                               const RobotTimeStamp_t t) {
  auto currentMap = GetCurrentMemoryMap();
  if (currentMap) {
    MemoryMapDataPtr clearData =
        MemoryMapData(INavMap::EContentType::ClearOfObstacle, t).Clone();
    NodeTransformFunction trfm = [&clearData](MemoryMapDataPtr currentData) {
      if (currentData->type == EContentType::ObstacleProx) {
        auto castPtr =
            MemoryMapData::MemoryMapDataCast<MemoryMapData_ProxObstacle>(
                currentData);
        castPtr->MarkClear();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
void MapComponent::AddProxData(const BoundedConvexSet2f& region,
                               const MemoryMapData& data) {
  auto currentMap = GetCurrentMemoryMap();
  if (currentMap) {
    // Make sure we enable collision types before inserting
    auto newData = MemoryMapData::MemoryMapDataCast<MemoryMapData_ProxObstacle>(
        data.Clone());
    newData->SetCollidable(_enableProxCollisions);

    NodeTransformFunction trfm =
        [&newData](MemoryMapDataPtr currentData) -> MemoryMapDataPtr {
      if (currentData->type == EContentType::ObstacleProx) {
        auto castPtr =
            MemoryMapData::MemoryMapDataCast<MemoryMapData_ProxObstacle>(
                currentData);
...
void MapComponent::SetUseProxObstaclesInPlanning(bool enable) {
  ...
  auto currentNavMemoryMap = GetCurrentMemoryMap();
  if (currentNavMemoryMap) {
    PRINT_CH_INFO("MapComponent", "MapComponent.SetUseProxObstaclesInPlanning",
                  "Setting prox obstacles as %s collidable",
                  enable ? "" : "NOT");
    NodeTransformFunction enableProx = [enable](MemoryMapDataPtr data) {
      if (EContentType::ObstacleProx == data->type) {
        MemoryMapData::MemoryMapDataCast<MemoryMapData_ProxObstacle>(data)
            ->SetCollidable(enable);
      }
      return data;
    };
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
void MapComponent::InsertData(const Poly2f& polyWRTOrigin,
                              const MemoryMapData& data) {
  return InsertData(MemoryMapTypes::MemoryMapRegion(FastPolygon(polyWRTOrigin)),
                    data);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
void MapComponent::InsertData(const MemoryMapTypes::MemoryMapRegion& region,
                              const MemoryMapData& data) {
  auto currentMap = GetCurrentMemoryMap();
  if (currentMap) {
...
bool MapComponent::CheckForCollisions(const BoundedConvexSet2f& region) const {
  ...
  if (currentMap) {
    return currentMap->AnyOf(
        region, [](const auto& data) { return data->IsCollisionType(); });
  }
  return false;
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
bool MapComponent::CheckForCollisions(
    const BoundedConvexSet2f& region,
    const MemoryMapTypes::NodePredicate& pred) const {
  const auto currentMap = GetCurrentMemoryMap();
  if (currentMap) {
...
float MapComponent::GetCollisionArea(const BoundedConvexSet2f& region) const {
  const auto currentMap = GetCurrentMemoryMap();
  if (currentMap) {
    return currentMap->GetArea(
        [](const auto& data) { return data->IsCollisionType(); }, region);
  }
  return 0.f;
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
Result MapComponent::ProcessVisionOverheadEdges(
    const OverheadEdgeFrame& frameInfo) {
  ...
  if (frameInfo.groundPlaneValid) {
    ...
    // ground plane was invalid (atm we don't use this). It's probably only
    // useful if we are debug-rendering the ground plane
    _robot->GetContext()->GetVizManager()->EraseSegments(
        "MapComponent.AddVisionOverheadEdges");
  }
  return ret;
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
void MapComponent::AddDetectedObstacles(
    const OverheadEdgeFrame& edgeObstacles) {
  // TODO: Do something different with these vs. "interesting" overhead edges?
  if (edgeObstacles.groundPlaneValid &&
      !edgeObstacles.chains.GetVector().empty()) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
Result MapComponent::FindSensorDetectedCliffs(
    std::vector<MemoryMapTypes::MemoryMapDataConstPtr>& cliffNodes) const {
  cliffNodes.clear();
  const auto currentMap = GetCurrentMemoryMap();
  if (!currentMap) {
    return RESULT_FAIL;
  ...
  NodePredicate isDropSensorCliff = [](MemoryMapDataConstPtr nodeData) -> bool {
    ...
    if (isValidCliff) {
      auto nCliff =
          MemoryMapData::MemoryMapDataCast<MemoryMapData_Cliff>(nodeData);
  ...
  std::for_each(cliffNodeSet.begin(), cliffNodeSet.end(),
                [&](const auto& dataPtr) { cliffNodes.push_back(dataPtr); });
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
Result MapComponent::AddVisionOverheadEdges(
    const OverheadEdgeFrame& frameInfo) {
  const auto currentMap = GetCurrentMemoryMap();
  if (!currentMap) {
    return RESULT_OK;
  ...
  const auto& res = _robot->GetStateHistory()->ComputeStateAt(
      frameInfo.timestamp, histTimestamp, histState, useInterp);
  if (res != RESULT_OK) {
  ...
  if (result != RESULT_OK) {
    PRINT_CH_INFO(
        "MapComponent",
        "MapComponent.AddVisionOverheadEdges.UnableToRetreiveCliffCenters", "");
    return result;
  }
  ...
  */
  ...
  NodePredicate isCollisionType = [](const auto& data) {
    return (data->type == EContentType::ObstacleProx) ||
  ...
  std::vector<bool> collisionCheckResults = currentMap->AnyOf(
      robotPose.GetTranslation(), imagePoints, isCollisionType);
  ...
  if (validPoints.size() >= kHoughAccumThreshold && cliffNodes.size() > 0) {
    ...
    // TODO(agm) currently we set the newest cliff as the "target" cliff to
    // extend
    //           it would be nice if we could decide on the cliff to extend more
    //           intelligently based on what we are currently observing. This
    //           assumes that the edge processing is always called in response
    //           to discovering a new cliff from the drop sensor.
    auto newestNodeIter = std::max_element(
        cliffNodes.cbegin(), cliffNodes.cend(),
    ...
    if (newestNodeIter != cliffNodes.cend()) {
      auto newCliffNode =
          MemoryMapData::MemoryMapDataCast<const MemoryMapData_Cliff>(
              *newestNodeIter);
      Pose3d refinedCliffPose;
      const bool result = RefineNewCliffPose(validPoints, newCliffNode,
                                             oldCliffNodes, refinedCliffPose);
      if (result) {
        auto ptr = std::const_pointer_cast<MemoryMapData_Cliff>(
            newCliffNode.GetSharedPtr());
        ...
        // special transform function to insert visual cliffs, without
        // overwriting sensor-detected cliffs (merges both sources of info the
        // same node)
        NodeTransformFunction transformVisionCliffs =
            [&cliffDataVisPtr](MemoryMapDataPtr currNode) -> MemoryMapDataPtr {
          if (currNode->type == EContentType::Cliff) {
            // a node can be from the cliff sensor AND from vision
            auto currCliff =
                MemoryMapData::MemoryMapDataCast<MemoryMapData_Cliff>(currNode);
        ...
        currentMap->Insert(
            FastPolygon({
                refinedCliffPose2d * Point2f(-kVisionCliffPadding_mm,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
bool MapComponent::RefineNewCliffPose(
  ...
  const Pose3d& newCliffPose =
      MemoryMapData::MemoryMapDataCast<const MemoryMapData_Cliff>(newCliffNode)
          ->pose;
  const Point2f& newCliffCenter = newCliffPose.GetTranslation();
  std::vector<Point2f> oldCliffCenters;
  std::transform(
      oldCliffNodes.cbegin(), oldCliffNodes.cend(),
      std::back_inserter(oldCliffCenters),
      [](MemoryMapTypes::MemoryMapDataConstPtr iter) -> Point2f {
        return MemoryMapData::MemoryMapDataCast<const MemoryMapData_Cliff>(iter)
            ->pose.GetTranslation();
      });

  DEV_ASSERT(std::all_of(oldCliffNodes.cbegin(), oldCliffNodes.cend(),
                         [](const MemoryMapDataConstPtr& ptr) {
                           return ptr->type == EContentType::Cliff;
                         }),
             "MapComponent.RefineNewCliffPose.MemoryMapDataTypesNotCliff");
  DEV_ASSERT(
      points.size() > 1,
      "MapComponent.RefineNewCliffPose.NotEnoughPointsToExtractLineFrom");
  ...
  if (rows == 0 || cols == 0) {
    PRINT_NAMED_WARNING(
        "MapComponent.RefineNewCliffPose.BinaryImageHasZeroRowCol", "");
    return false;
  }

  if (rows * cols > kMaxPixelsUsedForHoughTransform) {
    PRINT_NAMED_WARNING("MapComponent.RefineNewCliffPose.BinaryImageTooLarge",
                        "dims=(%d,%d)", rows, cols);
    return false;
  }
  ...
  // => 10mm gap between edge-feature points is used to discard highly
  // fragmented
  //    edge detections (e.g. highly irregular patterned textures)
  std::vector<cv::Vec4i> linesInImg;
  cv::HoughLinesP(binImg, linesInImg, 1, DEG_TO_RAD(kHoughAngleResolution_deg),
                  kHoughAccumThreshold, kHoughMinLineLength_mm,
                  kHoughMaxLineGap_mm);

  if (linesInImg.size() == 0) {
    PRINT_CH_INFO("MapComponent",
                  "MapComponent.RefineNewCliffPose.NoLinesFoundInBinaryImage",
                  "%zd count of edge points", points.size());
    return false;
  }
  ...
  const auto toCartesian =
      [&](const cv::Vec4i& seg) -> std::pair<Point2f, Point2f> {
    auto x1 = seg[0] + xMin;
    auto y1 = seg[1] + yMin;
    auto x2 = seg[2] + xMin;
    auto y2 = seg[3] + yMin;
  ...
  std::for_each(linesInImg.begin(), linesInImg.end(),
                [&](const cv::Vec4i& lineInImg) {
                  linesInCartes.push_back(toCartesian(lineInImg));
                });

  // helper lambda -- perpendicular distance Squared to a line from point
  const auto& perpDistSqToLine =
      [](const std::pair<Point2f, Point2f>& endPoints,
         const Point2f& testPoint) -> float {
    ...
    auto& x0 = testPoint.x();
    auto& y0 = testPoint.y();
    return pow((y2 - y1) * x0 - (x2 - x1) * y0 + x2 * y1 - y2 * x1, 2) /
  ...
  if (lineIdx < linesInCartes.size()) {
    ...
    // we want to translate the cliff center pose to lie on the detected edge
    // line and reorient the pose s.t. y-axis is along the edge, x-axis points
    ...
    */
    ...
    auto projCliffCenter =
        p1 + lineUnitVec * Anki::DotProduct(lineUnitVec, newCliffCenter - p1);
    auto projYAxis =
        p1 + lineUnitVec * Anki::DotProduct(lineUnitVec,
                                            (Pose2d)newCliffPose * Y_AXIS_2D() -
                                                p1);  // on the edgeLine
    auto correctedYAxis = projYAxis - projCliffCenter;
    auto correctedXAxis =
        Point2f(correctedYAxis.y(), -correctedYAxis.x());  // y^ cross z^ = x^
    Radians cliffAngleWrtWorld =
        std::atan2(correctedXAxis.y(), correctedXAxis.x());
    refinedCliffPose =
        Pose3d(cliffAngleWrtWorld, Z_AXIS_3D(),
               Point3f(projCliffCenter.x(), projCliffCenter.y(), 0.f));
    refinedCliffPose.SetParent(_robot->GetWorldOrigin());
    return true;
  } else {
    PRINT_CH_INFO("MapComponent",
                  "MapComponent.RefineNewCliffPose.NoAcceptableLinesFound",
                  "%zd candidate lines", linesInCartes.size());
  }
  return false;
}
