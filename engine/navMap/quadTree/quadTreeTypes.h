...
// wrapper for a tuple that allows implicit casting to any of its contained
// types
...
class SmartTuple : public std::tuple<Ts...> {
 private:
  ...
  struct is_unique_set<Head, Tail...>
      : std::integral_constant<bool, !has_type<Head, Tail...>::value &&
                                         is_unique_set<Tail...>::value> {};

 public:
  // make sure SmartTuple only contains unique types
  static_assert(is_unique_set<Ts...>::value,
                "SmartTuple cannot have duplicate types");

  SmartTuple() : std::tuple<Ts...>() {}
  SmartTuple(const SmartTuple<Ts...>& n) : std::tuple<Ts...>(n) {}

  // NOTE: use `enable_if_t` here, otherwise we will try to cast to find
  // `SmartTuple<Ts...>` inside `std::tuple<Ts...>`
  //       and get `tuple:1017:5: error: static_assert failed "type not found in
  //       type list"`
  template <typename U, typename = std::enable_if_t<has_type<U, Ts...>::value>>
  SmartTuple(const U& u) : std::tuple<Ts...>() {
    std::get<U>(*this) = u;
  }
...
// wrapper class for specifying the interface between QT actions and geometry
// methods
class FoldableRegion {
 public:
  FoldableRegion(const BoundedConvexSet2f& set)
    ...
    // NOTE: lambda incurs some ~5% perforamnce overhead from the std::function
    // interface,
    //       so use std::bind where possible to reduce function deref overhead.
    //       lambda implementations are included in comments for clarity on what
    //       the bind method should evaluate to

    Contains = std::bind(&BoundedConvexSet2f::Contains, &set, _1);
    // ~ [&set](const Point2f& p) { return set.Contains(p); };
    ContainsQuad = std::bind(&BoundedConvexSet2f::ContainsAll, &set,
                             std::bind(&AxisAlignedQuad::GetVertices, _1));
    // ~ [&set](const AxisAlignedQuad& q) { return
    // set.ContainsAll(q.GetVertices()); };
  ...
  FoldableRegion(const PointSetUnion2f<T, U>& set)
    ...
    // ~ [&set](const Point2f& p) { return set.Contains(p); };
    ContainsQuad =
        std::bind(&PointSetUnion2f<T, U>::ContainsHyperCube, &set, _1);
...
constexpr QuadTreeTypes::EQuadrant Vec2Quadrant(const Vec2f& dir) {
  if ((dir.x() < 0.f) && (dir.y() < 0.f)) {
    return EQuadrant::MinusXMinusY;
  } else if ((dir.x() > 0.f) && (dir.y() < 0.f)) {
    return EQuadrant::PlusXMinusY;
  } else if ((dir.x() < 0.f) && (dir.y() > 0.f)) {
    return EQuadrant::MinusXPlusY;
  } else if ((dir.x() > 0.f) && (dir.y() > 0.f)) {
    return EQuadrant::PlusXPlusY;
  }

  // NOTE: when we have exact matches for 0.f, discriminate the difference
  // between -0.f and 0.f.
  //       This preserves the property that checking a vector reflected through
  //       the origin results in a quadrant reflected through the origin (this
  //       property is not true for vertical and horizontal lines when using
  //       float comparison operations, since -0.f == 0.f by definition)
  else if (dir.x() == 0.f) {
    return (dir.y() >= 0.f) ? EQuadrant::MinusXPlusY : EQuadrant::PlusXMinusY;
  } else if (dir.y() == 0.f) {
    return (dir.x() >= 0.f) ? EQuadrant::PlusXPlusY : EQuadrant::MinusXMinusY;
  }
...
inline constexpr QuadTreeTypes::EQuadrant GetQuadrantInDirection(
    EQuadrant from, EDirection dir) {
  // bit position 0 is the Y coordinate, bit position 1 is x, so toggle the
  // appropriate bit with an XOR
  switch (dir) {
    case EDirection::PlusX:
    case EDirection::MinusX: {
      return (EQuadrant)((std::underlying_type<EQuadrant>::type)from ^ 2);
    };
    case EDirection::PlusY:
    case EDirection::MinusY: {
      return (EQuadrant)((std::underlying_type<EQuadrant>::type)from ^ 1);
    };
  }
}

inline constexpr QuadTreeTypes::EDirection GetOppositeDirection(
    EDirection dir) {
  // directions are defined in CW order, so just move two positions CW and grab
  // the last two bits
  const EDirection ret =
      (EDirection)(((std::underlying_type<EDirection>::type)dir + 2) & 0b11);
  return ret;
}

inline constexpr bool IsSibling(EQuadrant from, EDirection dir) {
  // bit position 0 is the Y coordinate, bit position 1 is x, so just compare
  // state of these bits for each direction
  switch (dir) {
    case EDirection::PlusX: {
      return ((std::underlying_type<EQuadrant>::type)from & 0b10);
    }
    case EDirection::MinusX: {
      return !((std::underlying_type<EQuadrant>::type)from & 0b10);
    }
    case EDirection::PlusY: {
      return ((std::underlying_type<EQuadrant>::type)from & 0b01);
    }
    case EDirection::MinusY: {
      return !((std::underlying_type<EQuadrant>::type)from & 0b01);
    }
...
// computes the node address relative to the root of a tree whose origin is
// (0,0) assumes maximum reachable depth is desired
NodeAddress GetAddressForNodeCenter(const Point2i& nodeCenter,
                                    const uint8_t& depth);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
inline Point2i GetIntegralCoordinateOfNode(const Point2f& point,
  ...
  // as a result, the integral coordinates line up better when we first
  // transform s.t. one of the four nearest nodes to the tree center becomes
  // (0,0)
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
inline Point2f GetCartesianCoordinateOfNode(const Point2i& point,
                                            const Point2f& center,
                                            const f32 precision,
                                            const uint8_t height) {
