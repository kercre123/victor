 ...
 * Date:   01/13/2016
 *
 * Description: Class for processing a quadTree. It relies on the quadTree and
 *quadTreeNodes to share the proper information for the Processor.
 *
...
CONSOLE_VAR(bool, kRenderSeeds, "QuadTreeProcessor",
            false);  // renders seeds differently for debugging purposes
CONSOLE_VAR(bool, kRenderBordersFrom, "QuadTreeProcessor",
            false);  // renders detected borders (origin quad)
CONSOLE_VAR(bool, kRenderBordersToDot, "QuadTreeProcessor",
            false);  // renders detected borders (border center) as dots
CONSOLE_VAR(bool, kRenderBordersToQuad, "QuadTreeProcessor",
            false);  // renders detected borders (destination quad)
CONSOLE_VAR(bool, kRenderBorder3DLines, "QuadTreeProcessor",
            false);  // renders borders returned as 3D lines (instead of quads)
CONSOLE_VAR(float, kRenderZOffset, "QuadTreeProcessor",
            20.0f);  // adds Z offset to all quads
CONSOLE_VAR(bool, kDebugFindBorders, "QuadTreeProcessor",
            false);  // prints debug information in console

#define DEBUG_FIND_BORDER(format, ...)                                        \
  if (kDebugFindBorders) {                                                    \
    do {                                                                      \
      ::Anki::Util::sChanneledInfoF(LOG_CHANNEL, "NMQTProcessor", {}, format, \
                                    ##__VA_ARGS__);                           \
    } while (0);                                                              \
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void QuadTreeProcessor::OnNodeContentChanged(const QuadTreeNode* node,
                                             const NodeContent& oldContent) {
  using namespace MemoryMapTypes;
  const EContentType oldType =
      static_cast<const MemoryMapDataPtr&>(oldContent)->type;
  const EContentType newType =
      static_cast<const MemoryMapDataPtr&>(node->GetData())->type;
  ...
  // update exploration area based on the content type
  {
    ...
    const bool needsToRemove =
        !wasEmpty &&
        (node->IsSubdivided() || (newType == EContentType::Unknown));
    const bool needsToAdd = wasEmpty && !(node->IsSubdivided() ||
...
void QuadTreeProcessor::OnNodeDestroyed(const QuadTreeNode* node) {
  ...
  {
    const bool shouldBeCountedOld =
        (oldContent == EContentType::InterestingEdge);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::vector<bool> QuadTreeProcessor::AnyOfRays(
    const Point2f& start, const std::vector<Point2f>& ends,
  ...
  const auto startBres = QuadTreeTypes::GetIntegralCoordinateOfNode(
      start, _quadTree->GetCenter(), _quadTree->GetContentPrecisionMM(),
      _quadTree->GetMaxHeight());
  const auto maxTreeHeight = _quadTree->GetMaxHeight();

  for (int rayIdx = 0; rayIdx < ends.size(); ++rayIdx) {
    BresenhamLinePixelIterator bresIter(
        startBres,
        QuadTreeTypes::GetIntegralCoordinateOfNode(
            ends[rayIdx], _quadTree->GetCenter(),
            _quadTree->GetContentPrecisionMM(), _quadTree->GetMaxHeight()));
    while (!bresIter.Done()) {
      ...
      if (got == localCache.end()) {
        ...
        ((const QuadTree*)_quadTree)
            ->Fold(getNode,
                   GetAddressForNodeCenter(rasterPoint, maxTreeHeight));
        const bool result =
            qnode &&
            pred(static_cast<const MemoryMapDataPtr&>(qnode->GetData()));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
QuadTreeProcessor::NodeSet QuadTreeProcessor::GetNodesToFill(
    const NodePredicate& innerPred, const NodePredicate& outerPred) {
  NodeSet output;
    ...
    for (const auto& node : keyValuePair.second) {
      ...
      if (innerPred(static_cast<const MemoryMapDataPtr&>(node->GetData()))) {
        ...
        for (const auto& neighbor : node->GetNeighbors()) {
          if (outerPred(
                  static_cast<const MemoryMapDataPtr&>(neighbor->GetData()))) {
  ...
  while (!unexpandedNodes.empty()) {
    ...
    for (const auto& neighbor : node->GetNeighbors()) {
      if (innerPred(
              static_cast<const MemoryMapDataPtr&>(neighbor->GetData())) &&
          (output.find(neighbor) == output.end())) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool QuadTreeProcessor::FillBorder(const NodePredicate& innerPred,
                                   const NodePredicate& outerPred,
                                   const MemoryMapDataPtr& data) {
  bool changed = false;
  for (const auto& node : GetNodesToFill(innerPred, outerPred)) {
    changed |= _quadTree->Transform(node->GetAddress(),
                                    [&data](auto) { return data; });
  }
...
bool QuadTreeProcessor::HasContentType(EContentType nodeType) const {
  DEV_ASSERT_MSG(IsCached(nodeType), "QuadTreeProcessor.HasContentType",
                 "%s is not cached", EContentTypeToString(nodeType));

  // check if any nodes for that type are cached currently
  auto nodeSetMatch = _nodeSets.find(nodeType);
  const bool hasAny =
      (nodeSetMatch != _nodeSets.end()) && !nodeSetMatch->second.empty();
  return hasAny;
}
