...
class QuadTreeNode : private Util::noncopyable {
 ...
 public:
  ...
  // run the provided accumulator function recursively over the tree for all
  // nodes intersecting with region (if provided). NOTE: any recursive call
  // through the QTN should be implemented by fold so all collision checks
  // happen in a consistant manner
  void Fold(const FoldFunctorConst& accumulator,
            const FoldableRegion& region = RealNumbers2f(),
            FoldDirection dir = FoldDirection::BreadthFirst) const;
  void Fold(const FoldFunctorConst& accumulator, const NodeAddress& addr,
            FoldDirection dir = FoldDirection::BreadthFirst) const;
 ...
 protected:
  ...
  // Leave the constructor as a protected member so only the root node or other
  // Quad tree nodes can create new nodes it will allow subdivision as long as
  // level is greater than 0
  QuadTreeNode(const QuadTreeNode* parent = nullptr,
               EQuadrant quadrant = EQuadrant::Root);
  ...
  // checks if all children are the same type, if so it removes the children and
  // merges back to a single parent
  void TryAutoMerge();
  ...
  // swaps children and content with 'otherNode', updating the children's parent
  // pointer
  void SwapChildrenAndContent(QuadTreeNode* otherNode);

  // run the provided accumulator function recursively over the tree for all
  // nodes intersecting with region (if provided). NOTE: mutable recursive calls
  // should remain private to ensure tree invariants are held
  void Fold(FoldFunctor& accumulator,
            const FoldableRegion& region = RealNumbers2f(),
            FoldDirection dir = FoldDirection::BreadthFirst);
  void Fold(FoldFunctor& accumulator, const NodeAddress& addr,
            FoldDirection dir = FoldDirection::BreadthFirst);
  ...
  // iterate until we reach the nodes that have a border in the given direction,
  // and add them to the vector NOTE: this method is expected to NOT clear the
  // vector before adding descendants
  void AddSmallestDescendants(
      EDirection direction,
      std::vector<const QuadTreeNode*>& descendants) const;
  ...
  // callbacks to notify external system if an element has changed or been
  // destroyed
