 ...
 * Note nodes can work with a processor to speed up algorithms and searches,
 *however this implementation supports working with one processor only for any
 *given node. Do not use more than one processor instance for nodes, or
...
static_assert(!std::is_copy_assignable<QuadTreeNode>::value,
              "QuadTreeNode was designed non-copyable");
static_assert(!std::is_copy_constructible<QuadTreeNode>::value,
              "QuadTreeNode was designed non-copyable");
static_assert(!std::is_move_assignable<QuadTreeNode>::value,
              "QuadTreeNode was designed non-movable");
static_assert(!std::is_move_constructible<QuadTreeNode>::value,
              "QuadTreeNode was designed non-movable");
...
QuadTreeNode::QuadTreeNode(const QuadTreeNode* parent, EQuadrant quadrant)
    : _boundingBox({0, 0}, {0, 0}), _parent(parent), _quadrant(quadrant) {
  if (_parent) {
    ...
    _boundingBox =
        AxisAlignedQuad(_center - Point2f(halfLen), _center + Point2f(halfLen));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
QuadTreeNode::~QuadTreeNode() { _destructorCallback(this); }
...
bool QuadTreeNode::Subdivide() {
  if ((_maxHeight == 0) || IsSubdivided()) {
    return false;
  }

  // create new children, push our data to them, then clear our own data
  _childrenPtr.emplace_back(
      new QuadTreeNode(this, EQuadrant::PlusXPlusY));  // up L
  _childrenPtr.emplace_back(
      new QuadTreeNode(this, EQuadrant::PlusXMinusY));  // up R
  _childrenPtr.emplace_back(
      new QuadTreeNode(this, EQuadrant::MinusXPlusY));  // lo L
  _childrenPtr.emplace_back(
      new QuadTreeNode(this, EQuadrant::MinusXMinusY));  // lo R
...
void QuadTreeNode::TryAutoMerge() {
  ...
  // check if all children classified the same content (assumes node content
  // equality is transitive)
  for (size_t i = 0; i < _childrenPtr.size() - 1; ++i) {
    allChildrenEqual &=
        (_childrenPtr[i]->GetData() == _childrenPtr[i + 1]->GetData());
  }
...
void QuadTreeNode::SwapChildrenAndContent(QuadTreeNode* otherNode) {
  ...
  // swap contents by use of copy, since changes have to be notified to the
  // processor
...
const QuadTreeNode* QuadTreeNode::GetChild(EQuadrant quadrant) const {
  const QuadTreeNode* ret =
      (_childrenPtr.empty())
          ? (nullptr)
          : (_childrenPtr[(std::underlying_type<EQuadrant>::type)quadrant]
                 .get());
  return ret;
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void QuadTreeNode::AddSmallestDescendants(
    EDirection direction, std::vector<const QuadTreeNode*>& descendants) const {
  if (!IsSubdivided()) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
const QuadTreeNode* QuadTreeNode::FindSingleNeighbor(
  ...
  if (parentNeighbor) {
    ...
    // if directNeighbor exits, then use that, otherwise return our parents
    // neighbor
...
std::vector<const QuadTreeNode*> QuadTreeNode::GetNeighbors() const {
  ...
  if (plusX) {
    plusX->AddSmallestDescendants(EDirection::MinusX, neighbors);
  }
  if (minusX) {
    minusX->AddSmallestDescendants(EDirection::PlusX, neighbors);
  }
  if (minusY) {
    minusY->AddSmallestDescendants(EDirection::PlusY, neighbors);
  }
  if (plusY) {
    plusY->AddSmallestDescendants(EDirection::MinusY, neighbors);
  }
...
void QuadTreeNode::Fold(FoldFunctor& accumulator, const NodeAddress& addr,
                        FoldDirection dir) {
  if (FoldDirection::BreadthFirst == dir) {
    accumulator(*this);
  }
  ...
  if (FoldDirection::DepthFirst == dir) {
    accumulator(*this);
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void QuadTreeNode::Fold(const FoldFunctorConst& accumulator,
                        const NodeAddress& addr, FoldDirection dir) const {
  if (FoldDirection::BreadthFirst == dir) {
    accumulator(*this);
  }
...
// - - - - - - - - - - - - - - - - - - - -
/*
  For calls that are constrained by some convex region, first we can potentially
  avoid excess collision checks if the current node is fully contained by the
  Fold Region. In the example below, nodes 1 through 6 need intersection checks,
  but nodes A through D do not since their parent is fully contained by Fold
  Region
...
namespace {
inline u8 GetChildFilterMask(const Point2f& center,
                             const AxisAlignedQuad& aabb) {
  u8 childFilter = 0b1111;  // Bit field represents quadrants (-x, -y), (-x,
                            // +y), (+x, -y), (+x, +y)

  if (aabb.GetMinVertex().x() > center.x()) {
    childFilter &= 0b0011;
  }  // only +x (top) nodes
  if (aabb.GetMaxVertex().x() < center.x()) {
    childFilter &= 0b1100;
  }  // only -x (bot) nodes
  if (aabb.GetMinVertex().y() > center.y()) {
    childFilter &= 0b0101;
  }  // only +y (left) nodes
  if (aabb.GetMaxVertex().y() < center.y()) {
    childFilter &= 0b1010;
  }  // only -y (right) nodes
...
void QuadTreeNode::Fold(FoldFunctor& accumulator, const FoldableRegion& region,
                        FoldDirection dir) {
  ...
  if (region.ContainsQuad(_boundingBox)) {
    ...
    u8 childFilter = IsSubdivided()
                         ? GetChildFilterMask(_center, region.GetBoundingBox())
                         : 0;
    for (const auto& cPtr : _childrenPtr) {
      if (childFilter & 0x1) {
...
void QuadTreeNode::Fold(const FoldFunctorConst& accumulator,
                        const FoldableRegion& region, FoldDirection dir) const {
  // node and region are disjoint
  if (!region.IntersectsQuad(_boundingBox)) {
    return;
  }
