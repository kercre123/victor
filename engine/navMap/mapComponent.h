...
class MapComponent : public IDependencyManagedComponent<RobotComponentID>,
 ...
 public:
  ...
  void UpdateObjectPose(const ObservableObject& object, const Pose3d* oldPose,
  ...
  // flag all interesting edges in front of the robot (using ground plane ROI)
  // as uncertain, meaning we want the robot to grab new edges since we don't
  // trust the ones we currently have in front of us
  void FlagGroundPlaneROIInterestingEdgesAsUncertain();

  // flags any interesting edges in the given quad as not interesting anymore.
  // Quad should be passed wrt current origin
  ...
  void InsertData(const MemoryMapTypes::MemoryMapRegion& region,
  ...
  // moves the frontier of explored prox obstacles into touching unexplored prox
  // ostacles
  ...
  // clear the space in the memory map between the robot and observed markers
  // for the given object, because if we saw the marker, it means there's
  // nothing between us and the marker. The observed markers are obtained
  // querying the current marker observation time
  void ClearRobotToMarkers(const ObservableObject* object);

  // clear the space between the robot and the line segment defined by points p
  // and q. The base of the region is a line segment of fixed length that is
  // perpendicular to the robot direction
  void ClearRobotToEdge(const Point2f& p, const Point2f& q,
  ...
  // return true of the specified region contains any objects of known collision
  // types
  bool CheckForCollisions(const BoundedConvexSet2f& region) const;
  bool CheckForCollisions(const BoundedConvexSet2f& region,
                          const MemoryMapTypes::NodePredicate& pred) const;

  // returns the accumulated area of cells in mm^2 in the current map that
  // satisfy the predicate (and region, if supplied)
  ...
  void SendDASInfoAboutCurrentMap() const {
    SendDASInfoAboutMap(_currentMapOriginID);
  }
  ...
  std::shared_ptr<const INavMap> GetCurrentMemoryMap() const {
    return GetCurrentMemoryMapHelper();
  }
  std::shared_ptr<INavMap> GetCurrentMemoryMap() {
    return GetCurrentMemoryMapHelper();
  }
 ...
 private:
  ...
  // updates the objects reported in curOrigin that are moving to the
  // relocalizedOrigin by virtue of rejiggering
  void UpdateOriginsOfObjects(PoseOriginID_t curOriginID,
                              PoseOriginID_t relocalizedOriginID);

  // add/remove the given object to/from the memory map
  void AddObservableObject(const ObservableObject& object,
                           const Pose3d& newPose);
  void RemoveObservableObject(const ObservableObject& object,
                              PoseOriginID_t originID);

  // search the navMap and remove any nodes that have exceeded their timeout
  // period
  void TimeoutObjects();
  ...
  //       cliff object, which has one pose (determined at the time it was
  //       sensed)
  Result FindSensorDetectedCliffs(
  ...
  struct PoseInMapInfo {
    ...
    Pose3d pose;
    bool isInMap;  // if true the pose was sent to the map, if false the pose
  ...
  // use multiple dirty flags to broadcast to different channels in case they
  // have different broadcast rates
  ...
  // config variable for conditionally enabling/disabling prox obstacles in
  // planning
