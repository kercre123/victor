...
enum class EContentType : uint8_t {
  ...
  NotInterestingEdge,    // a border/edge detected by the camera that we have
                         // already explored and it's not interesting anymore
...
using MemoryMapDataList =
    std::unordered_set<MemoryMapDataPtr, MemoryMapDataHasher<MemoryMapData>>;
using MemoryMapDataConstList =
    std::unordered_set<MemoryMapDataConstPtr,
                       MemoryMapDataHasher<const MemoryMapData>>;
...
using QuadInfoFullVector =
    std::vector<ExternalInterface::MemoryMapQuadInfoFull>;
...
// returns false if the base constructor for MemoryMapData can be used with
// content type, and true if a derived class constructor must be called, forcing
// additional data to be provided on instantiation
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Array of content that provides an API
// with compilation checks for algorithms that require combinations of content
// types. It's for example used to make sure that you define a value for all
// content types, rather than including only those you want to be true.
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -

using FullContentArray =
    Util::FullEnumToValueArrayChecker::FullEnumToValueArray<EContentType, bool>;
using Util::FullEnumToValueArrayChecker::
    IsSequentialArray;  // import IsSequentialArray to this namespace

// variable type in which we can pack EContentType as flags. Check
// ENodeContentTypeToFlag
using EContentTypePackedType = uint32_t;

// Converts EContentType values into flag bits. This is handy because I want to
// store EContentType in the smallest type possible since we have a lot of quad
// nodes, but I want to pass groups as bit flags in one packed variable
