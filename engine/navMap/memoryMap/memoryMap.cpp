 ...
 * Date:   12/09/2015
 *
 * Description: Map of the space navigated by the robot with some memory
...
#define MONITOR_PERFORMANCE(eval)                                \
  (kMapPerformanceTestsEnabled)                                  \
      ? PerformanceMonitor([&]() { return eval; },               \
                           __FILE__ ":" + std::string(__func__)) \
      : eval
...
static void UpdatePerformanceRecord(const double& time_us,
  ...
  if (record.samples > kMapPerformanceTestsSampleWindow) {
    ...
    record.avgTime_us +=
        (time_us - record.avgTime_us) / kMapPerformanceTestsSampleWindow;
  } else {
  ...
  if ((++record.samples & kMapPerformanceTestsSampleWindow - 1) == 0) {
    DEV_ASSERT((kMapPerformanceTestsSampleWindow &
                (kMapPerformanceTestsSampleWindow - 1)) == 0,
               "Performance sample window not a power of 2");
    PRINT_NAMED_INFO("PerformanceMonitor", "Average time for '%s' is %f us",
                     recordName.c_str(), record.avgTime_us);
  }
}

// handle non-void methods
template <typename T>
static auto PerformanceMonitor(
    T f, const std::string& method,
    typename std::enable_if<!std::is_void<decltype(f())>::value>::type* =
        nullptr) -> decltype(f()) {
...
ColorRGBA GetNodeVizColor(MemoryMapDataPtr node) {
  ...
  if (node->type == EContentType::ObstacleProx && kRenderProxBeliefs) {
    u8 val = MemoryMapData::MemoryMapDataCast<MemoryMapData_ProxObstacle>(node)
                 ->GetObstacleConfidence();
  ...
  Anki::ColorRGBA color =
      Anki::NamedColors::WHITE;  // default that any other rule overrides

  using namespace ExternalInterface;
  switch (node->GetExternalContentType()) {
    case ENodeContentTypeEnum::Unknown: {
      color = Anki::NamedColors::DARKGRAY;
      color.SetAlpha(0.2f);
      break;
    }
    case ENodeContentTypeEnum::ClearOfObstacle: {
      color = Anki::NamedColors::GREEN;
      color.SetAlpha(0.5f);
      break;
    }
    case ENodeContentTypeEnum::ClearOfCliff: {
      color = Anki::NamedColors::DARKGREEN;
      color.SetAlpha(0.8f);
      break;
    }
    case ENodeContentTypeEnum::ObstacleCube: {
      color = Anki::NamedColors::RED;
      color.SetAlpha(0.5f);
      break;
    }
    case ENodeContentTypeEnum::ObstacleUnrecognized: {
    ...
    case ENodeContentTypeEnum::Cliff: {
      const auto& cliffData =
          MemoryMapData::MemoryMapDataCast<MemoryMapData_Cliff>(node);
    ...
    case ENodeContentTypeEnum::InterestingEdge: {
      color = Anki::NamedColors::MAGENTA;
      color.SetAlpha(0.5f);
      break;
    }
    case ENodeContentTypeEnum::NotInterestingEdge: {
      color = Anki::NamedColors::PINK;
      color.SetAlpha(0.8f);
      break;
    }
    case ENodeContentTypeEnum::ObstacleProx: {
      color = (Anki::NamedColors::CYAN * scale) +
              (Anki::NamedColors::GREEN * (1 - scale));
      color.SetAlpha(0.5f + .5 * scale);
      break;
    }
    case ENodeContentTypeEnum::ObstacleProxExplored: {
      color = (Anki::NamedColors::BLUE * scale) +
              (Anki::NamedColors::GREEN * (1 - scale));
      color.SetAlpha(0.5f + .5 * scale);
      break;
    }
...
MemoryMap::MemoryMap()
    : _processor(),
      _quadTree(std::bind(&QuadTreeProcessor::OnNodeDestroyed, &_processor,
                          std::placeholders::_1),
                std::bind(&QuadTreeProcessor::OnNodeContentChanged, &_processor,
                          std::placeholders::_1, std::placeholders::_2)) {
  _processor.SetRoot(&_quadTree);
}

MemoryMap::~MemoryMap() { _processor.SetRoot(nullptr); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool MemoryMap::Merge(const INavMap& other, const Pose3d& transform) {
  DEV_ASSERT(dynamic_cast<const MemoryMap*>(&other),
             "MemoryMap.Merge.UnsupportedClass");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool MemoryMap::FillBorder(const NodePredicate& innerPred,
                           const NodePredicate& outerPred,
                           const MemoryMapDataPtr& newData) {
  // ask the processor to do it
  std::unique_lock<std::shared_timed_mutex> lock(_writeAccess);
  return MONITOR_PERFORMANCE(
      _processor.FillBorder(innerPred, outerPred, newData));
}
...
bool MemoryMap::AnyOf(const MemoryMapRegion& r, const NodePredicate& f) const {
  ...
  _quadTree.Fold(
      [&](const auto& node) {
        retv |= f(static_cast<const MemoryMapDataPtr&>(node.GetData()));
      },
      r);
  return retv;
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::vector<bool> MemoryMap::AnyOf(const Point2f& start,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
float MemoryMap::GetArea(const NodePredicate& pred,
                         const MemoryMapRegion& region) const {
  float retv = 0.f;
  std::shared_lock<std::shared_timed_mutex> lock(_writeAccess);
  _quadTree.Fold(
      [&](const auto& node) {
...
bool MemoryMap::Insert(const MemoryMapRegion& r, const MemoryMapData& data) {
  ...
  NodeTransformFunction trfm = [&dataPtr](const MemoryMapDataPtr& currentData) {
    ...
    return currentData->CanOverrideSelfWithContent(dataPtr) ? dataPtr
                                                            : currentData;
  };
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool MemoryMap::Insert(const MemoryMapRegion& r,
...
void MemoryMap::GetBroadcastInfo(MemoryMapTypes::MapBroadcastData& info) const {
  ...
  QuadTreeTypes::FoldFunctorConst accumulator = [&info, this](
    ...
    if (node.IsRootNode()) {
      ...
      info.mapInfo = ExternalInterface::MemoryMapInfo(
          node.GetMaxHeight(), node.GetSideLen(), node.GetCenter().x(),
          node.GetCenter().y(), 1.f, instanceId.str());
    }
    ...
    if (!node.IsSubdivided()) {
      ...
      info.quadInfo.emplace_back(nodeData->GetExternalContentType(),
                                 node.GetMaxHeight(), vizColor);

      info.quadInfoFull.emplace_back(vizColor, node.GetCenter().x(),
                                     node.GetCenter().y(), node.GetSideLen());
    }
  };
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void MemoryMap::FindContentIf(const NodePredicate& pred,
                              MemoryMapDataConstList& output,
                              const MemoryMapRegion& region) const {
  QuadTreeTypes::FoldFunctorConst accumulator =
      [&output, &pred](const QuadTreeNode& node) {
