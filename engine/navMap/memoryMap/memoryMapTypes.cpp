...
bool ExpectsAdditionalData(EContentType type) {
  DEV_ASSERT(
      type != EContentType::_Count,
      "MemoryMapTypes.ExpectsAdditionalData.UsingControlTypeIsNotAllowed");
...
const char* EContentTypeToString(EContentType contentType) {
  switch (contentType) {
    ...
    case EContentType::ClearOfCliff:
      return "ClearOfCliff";
    case EContentType::ObstacleObservable:
      return "ObstacleObservable";
    case EContentType::ObstacleProx:
      return "ObstacleProx";
    case EContentType::ObstacleUnrecognized:
      return "ObstacleUnrecognized";
    case EContentType::Cliff:
      return "Cliff";
    case EContentType::InterestingEdge:
      return "InterestingEdge";
    case EContentType::NotInterestingEdge:
      return "NotInterestingEdge";
    case EContentType::_Count:
      return "ERROR_COUNT_SHOULD_NOT_BE_USED";
  }
  return "ERROR";
}
...
EContentTypePackedType EContentTypeToFlag(EContentType contentType) {
  ...
  DEV_ASSERT(contentTypeValue < sizeof(EContentTypePackedType) * 8,
             "ENodeContentTypeToFlag.InvalidContentType");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool IsInEContentTypePackedType(EContentType contentType,
                                EContentTypePackedType contentPackedTypes) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
EContentTypePackedType ConvertContentArrayToFlags(
    const MemoryMapTypes::FullContentArray& array) {
  using namespace MemoryMapTypes;
  using namespace QuadTreeTypes;

  DEV_ASSERT(IsSequentialArray(array),
             "MemoryMapTreeTypes.ConvertContentArrayToFlags.InvalidArray");

  EContentTypePackedType contentTypeFlags = 0;
  for (const auto& entry : array) {
    if (entry.Value()) {
      const EContentTypePackedType contentTypeFlag =
          EContentTypeToFlag(entry.EnumValue());
