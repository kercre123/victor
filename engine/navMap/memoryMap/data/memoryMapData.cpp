...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool MemoryMapData::CanOverrideSelfWithContent(
    MemoryMapDataConstPtr newContent) const {
  ...
  if (newContentType == EContentType::Cliff) {
    ...
    // ClearOfObstacle currently comes from vision or prox sensor having a
    // direct line of sight to some object, so it can't clear obsstacles it cant
    // see (cliffs and unrecognized). Additionally, ClearOfCliff is currently a
    // superset of Clear of Obstacle, so trust ClearOfCliff flags.
    const bool isTotalClear =
    ...
    // InterestingEdge can only override basic node types, because it would
    // cause data loss otherwise. For example, we don't want to override a
    // recognized marked cube or a cliff with their own border
    ...
    if (dataType == EContentType::ObstacleProx) {
      DEV_ASSERT(
          dynamic_cast<const MemoryMapData_ProxObstacle*>(this) != nullptr,
          "MemoryMapData.CanOverride.InvalidCast");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
ExternalInterface::ENodeContentTypeEnum MemoryMapData::GetExternalContentType()
    const {
  ...
  switch (type) {
    case EContentType::Unknown: {
      externalContentType = ENodeContentTypeEnum::Unknown;
      break;
    }
    case EContentType::ClearOfObstacle: {
      externalContentType = ENodeContentTypeEnum::ClearOfObstacle;
      break;
    }
    case EContentType::ClearOfCliff: {
      externalContentType = ENodeContentTypeEnum::ClearOfCliff;
      break;
    }
    case EContentType::ObstacleUnrecognized: {
      externalContentType = ENodeContentTypeEnum::ObstacleUnrecognized;
      break;
    }
    case EContentType::InterestingEdge: {
      externalContentType = ENodeContentTypeEnum::InterestingEdge;
      break;
    }
    case EContentType::NotInterestingEdge: {
      externalContentType = ENodeContentTypeEnum::NotInterestingEdge;
      break;
    }
    // handled elsewhere
