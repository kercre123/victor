...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
MemoryMapData_ProxObstacle::MemoryMapData_ProxObstacle(ExploredType explored,
                                                       const Pose2d& pose,
                                                       RobotTimeStamp_t t)
...
bool MemoryMapData_ProxObstacle::Equals(const MemoryMapData* other) const {
  ...
  const MemoryMapData_ProxObstacle* castPtr =
      static_cast<const MemoryMapData_ProxObstacle*>(other);
  const bool isNearLocation =
      IsNearlyEqual(_pose.GetTranslation(), castPtr->_pose.GetTranslation(),
                    20.f);  // close enough to initial observed pose
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
ExternalInterface::ENodeContentTypeEnum
MemoryMapData_ProxObstacle::GetExternalContentType() const {
  return _explored
             ? ExternalInterface::ENodeContentTypeEnum::ObstacleProxExplored
             : ExternalInterface::ENodeContentTypeEnum::ObstacleProx;
}
