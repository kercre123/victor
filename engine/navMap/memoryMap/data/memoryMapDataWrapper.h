 ...
 * Date:   15/02/2018
 *
 * Description: Wrapper for shared pointer to prevent instantiating with null
 *pointers, bad casting operations, and enforce deep comparisons when using ==
 *and != operators.
 *
...
template <class T,
          typename = std::enable_if<std::is_base_of<MemoryMapData, T>::value>>
class MemoryMapDataWrapper {
  friend class MemoryMapData;

 public:
  // make sure the copy constructor is only enabled if copying from an inherited
  // to a base class
  template <typename S = T>
  MemoryMapDataWrapper(
      const MemoryMapDataWrapper<S>& other,
  ...
  bool operator==(const MemoryMapDataWrapper<T>& other) const {
    return dataPtr->Equals(other.dataPtr.get());
  }
  bool operator!=(const MemoryMapDataWrapper<T>& other) const {
    return !(dataPtr->Equals(other.dataPtr.get()));
  }

  MemoryMapDataWrapper<T>& operator=(const MemoryMapDataWrapper& other) {
    dataPtr = other.GetSharedPtr();
    return *this;
  }
