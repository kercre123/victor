...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
MemoryMapData_Cliff::MemoryMapData_Cliff(const Pose3d& cliffPose,
                                         RobotTimeStamp_t t)
...
bool MemoryMapData_Cliff::Equals(const MemoryMapData* other) const {
  ...
  const MemoryMapData_Cliff* castPtr =
      static_cast<const MemoryMapData_Cliff*>(other);

  if (isFromVision == castPtr->isFromVision &&
      isFromCliffSensor == castPtr->isFromCliffSensor) {
    if (isFromCliffSensor) {  // && castPtr->isFromCliffSensor
      const bool isNearLocation =
          IsNearlyEqual(pose.GetTranslation(), castPtr->pose.GetTranslation());
      const bool isNearRotation = IsNearlyEqual(
          pose.GetRotation(), castPtr->pose.GetRotation(), kRotationTolerance);
...
MemoryMapData_Cliff::GetExternalContentType() const {
