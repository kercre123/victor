 ...
 * Date:   08/02/2016
 *
 * Description: Base for data structs that will be held in every node depending
...
class MemoryMapData {
 ...
 public:
  ...
  MemoryMapData(EContentType type = EContentType::Unknown,
                RobotTimeStamp_t time = 0)
      : MemoryMapData(type, time, false) {}
  ...
  virtual ExternalInterface::ENodeContentTypeEnum GetExternalContentType()
      const;

  // return true if this type collides with the robot
  virtual bool IsCollisionType() const {
    return ((type == EContentType::ObstacleUnrecognized) ||
            (type == EContentType::Cliff));
  }

  // compare to MemoryMapData and return bool if the data stored is the same
  virtual bool Equals(const MemoryMapData* other) const {
  ...
  // returns true if this node can be replaced by the given content type. Some
  // content type replacement rules depend on whether the quad center is fully
  // contained within the insertion polygon (centerContainedByROI)
  ...
  template <class T>
  static MemoryMapDataWrapper<const T> MemoryMapDataCast(
      MemoryMapDataConstPtr ptr);

 protected:
  MemoryMapData(MemoryMapTypes::EContentType type, RobotTimeStamp_t time,
                bool calledFromDerived);
...
inline MemoryMapData::MemoryMapData(MemoryMapTypes::EContentType type,
                                    RobotTimeStamp_t time,
                                    bool expectsAdditionalData)
    : type(type), firstObserved_ms(time), lastObserved_ms(time) {
  // need to make sure we dont ever create a MemoryMapData without providing all
  // information required. This locks us from creating something like
  // MemoryMapData_ObservableObject without the ID, for instance.
  DEV_ASSERT(ExpectsAdditionalData(type) == expectsAdditionalData,
             "MemoryMapData.ImproperConstructorCalled");
}
...
template <class T>
inline MemoryMapDataWrapper<T> MemoryMapData::MemoryMapDataCast(
    MemoryMapDataPtr ptr) {
  ...
  DEV_ASSERT(std::dynamic_pointer_cast<T>(ptr.GetSharedPtr()),
             "MemoryMapDataCast.BadQuadDataDynCast");
  return MemoryMapDataWrapper<T>(
      std::static_pointer_cast<T>(ptr.GetSharedPtr()));
}

template <class T>
inline MemoryMapDataWrapper<const T> MemoryMapData::MemoryMapDataCast(
    MemoryMapDataConstPtr ptr) {
  assert(T::HandlesType(ptr->type));
  assert(std::dynamic_pointer_cast<const T>(ptr.GetSharedPtr()));
  return MemoryMapDataWrapper<const T>(
      std::static_pointer_cast<const T>(ptr.GetSharedPtr()));
}
