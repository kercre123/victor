...
class MemoryMapData_ProxObstacle : public MemoryMapData {
 public:
  enum ExploredType { NOT_EXPLORED = 0, EXPLORED };

  // constructor
  MemoryMapData_ProxObstacle(ExploredType explored, const Pose2d& pose,
                             RobotTimeStamp_t t);
  ...
  virtual bool IsCollisionType() const override {
    return IsConfirmedObstacle();
  }
  ...
  // NOTE: at the time of adding the `_belief` value, there was still pending
  // tuning as to performance for
  //       removing objects. Once weights and thresholds have been verified, we
  //       should probably a more formal coded relationship to the paramters if
  //       possible.
  ...
  // - - - - - - - - - - - - - - - - - - - - Attributes
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // - - - - - - - - - - - - - - - - - - - - If you add attributes, make sure
  // you add them to ::Equals and ::Clone (if required)

  // Important: this is available in all NOT_EXPLORED obstacles and only some
  // EXPLORED. We lose these params when flood filling from EXPLORED to
  // NOT_EXPLORED, although that's not ideal. todo: fix this (FillBorder)
 private:
  ...
  bool _collidable;  // if the robot should consider this object as a collision
                     // type
};
