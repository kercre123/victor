 ...
 * Created: 1/22/2014
 *
 * Description: Interface class that creates multiple TCP or UDP clients to
 *connect and communicate with advertising devices.
 *
...
static double GetCurrentTimeInSeconds() {
  // Note: BaseStationTimer returns 0.0 when not started, so we have to use
  // universal time here
...
ConnectedDeviceInfo::~ConnectedDeviceInfo() { DestroyClients(); }

void ConnectedDeviceInfo::ConnectToClients(UdpClient* inClient,
                                           UdpClient* outClient) {
  ...
  // Pretend we just received something, so the timeout countdown starts from
  // now
...
MultiClientComms::MultiClientComms() : isInitialized_(false) {}

Result MultiClientComms::Init(const char* advertisingHostIP,
                              int advertisingPort,
                              unsigned int maxSentBytesPerTic) {
  if (isInitialized_) {
    PRINT_NAMED_WARNING(
        "MultiClientComms.Init.AlreadyInitialized",
  ...
  if (false ==
    ...
    PRINT_NAMED_ERROR("MultiClientComms.Init.FailedToConnect",
                      "Failed to connect to advertising host at %s "
                      "on port %d",
...
MultiClientComms::~MultiClientComms() { DisconnectAllDevices(); }

// Returns true if we are ready to use TCP
bool MultiClientComms::IsInitialized() { return true; }

ssize_t MultiClientComms::Send(const Comms::MsgPacket& p) {
  // TODO: Instead of sending immediately, maybe we should queue them and send
  // them all at once to more closely emulate BTLE.

#if (DO_SIM_COMMS_LATENCY)
  /*
  // If no send latency, just send now
  if (SIM_SEND_LATENCY_SEC == 0.0) {
    if ((maxSentBytesPerTic_ > 0) && (bytesSentThisUpdateCycle_ + p.dataLen >
  maxSentBytesPerTic_)) { #if(DEBUG_COMMS)
      PRINT_NAMED_INFO("MultiClientComms.MaxSendLimitReached", "queueing
  message"); #endif } else { bytesSentThisUpdateCycle_ += p.dataLen; return
  RealSend(p);
    }
  }
  */
  // Otherwise add to send queue
  sendMsgPackets_[p.destId].emplace_back(
...
ssize_t MultiClientComms::RealSend(const Comms::MsgPacket& p) {
  ...
  if (it != connectedDevices_.end()) {
    ...
    if (sent < 0) {
      PRINT_NAMED_WARNING("MultiClientComms.RealSend.SendFailed",
                          "destId: %d, socket %d, sent = %zd (errno = %d '%s')",
                          p.destId, udpClient->GetSocketFd(), sent, errno,
                          strerror(errno));
    }

    return sent;
  } else {
    PRINT_NAMED_WARNING("MultiClientComms.RealSend.NotConnected", "destId: %d",
                        p.destId);
    return -1;
  }
}
...
void MultiClientComms::Update(bool send_queued_msgs) {
  ...
  const size_t kMinAdMsgSize =
      sizeof(EMessageTag) +
      advMsg.Size();  // Size of message with an empty ip string

  ssize_t bytes_recvd = 0;
  do {
    uint8_t messageData[64];
    bytes_recvd =
        advertisingChannelClient_.Recv((char*)messageData, sizeof(messageData));
    if (bytes_recvd >= kMinAdMsgSize) {
      ...
      if (messageTag == kAdvertisementMsgTag) {
        ...
        const size_t innerMessageSize =
            (size_t)bytes_recvd - sizeof(EMessageTag);

        const size_t bytesUnpacked =
        ...
        if (bytesUnpacked == innerMessageSize) {
          ...
          if (it != connectedDevices_.end()) {
            ...
            // disconnect the old connection and allow it to full connect on the
            // next re-send
            ConnectedDeviceInfo& deviceInfo = it->second;
            const double initialConnectionTime =
                deviceInfo.GetInitialConnectionTime();
            const double timeConnected_s =
                currentTime_s - initialConnectionTime;
            const double kMinConnectedTimeBeforeNewConnect_s = 5.0;
            if (timeConnected_s > kMinConnectedTimeBeforeNewConnect_s) {
              PRINT_NAMED_INFO(
                  "MultiClientComms.Update.DisconnectOldConnection",
                  "Advert for device %d connected for %.1f seconds, assume new "
                  "connection attempt",
                  advMsg.id, timeConnected_s);
              DisconnectDeviceByID(advMsg.id);
            }
          } else {
            if (DEBUG_COMMS && (advertisingDevices_.find(advMsg.id) ==
                                advertisingDevices_.end())) {
              PRINT_NAMED_INFO("MultiClientComms.Update.NewDevice",
                               "Detected advertising device %d on host %s at "
                               "ports ToEng=%d, FromEng=%d",
                               advMsg.id, advMsg.ip.c_str(),
                               (int)advMsg.toEnginePort,
                               (int)advMsg.fromEnginePort);
            }
          ...
          PRINT_NAMED_WARNING("MultiClientComms.Update.ErrorUnpackingAdMsg",
                              "Unpacked %zu bytes, expected %zu", bytesUnpacked,
                              innerMessageSize);
        }
      }
    }
  ...
  while (it != advertisingDevices_.end()) {
    if (currentTime_s - it->second.lastSeenTime_s >
        ROBOT_ADVERTISING_TIMEOUT_S) {
#if (DEBUG_COMMS)
      PRINT_NAMED_INFO("MultiClientComms.Update.TimeoutDevice",
                       "Removing device %d from advertising list. (Last seen: "
                       "%f, curr time: %f)",
                       it->second.devInfo.id, it->second.lastSeenTime,
                       currTime);
#endif
  ...
  for (iter = recvdMsgPackets_.begin(); iter != recvdMsgPackets_.end();
       ++iter) {
  ...
  // printf("TIME %f: Total: %d, rel: %d\n", currTime, recvdMsgPackets_.size(),
  // numRecvRdyMsgs_);

  // Send messages that are scheduled to be sent, up to the outgoing bytes
  // limit.
  if (send_queued_msgs) {
    bytesSentThisUpdateCycle_ = 0;
    std::map<int, PacketQueue_t>::iterator sendQueueIt =
        sendMsgPackets_.begin();
    while (sendQueueIt != sendMsgPackets_.end()) {
      PacketQueue_t* pQueue = &(sendQueueIt->second);
      while (!pQueue->empty()) {
        if (pQueue->front().first <= currentTime_s) {
          if ((maxSentBytesPerTic_ > 0) &&
              (bytesSentThisUpdateCycle_ + pQueue->front().second.dataLen >
               maxSentBytesPerTic_)) {
#if (DEBUG_COMMS)
            PRINT_NAMED_INFO("MultiClientComms.MaxSendLimitReached",
                             "%d messages left in queue to send later",
                             (int)pQueue->size() - 1);
#endif
            break;
          }
...
void MultiClientComms::ReadAllMsgPackets() {
  ...
  while (it != connectedDevices_.end()) {
    ...
    while (1)  // Keep reading socket until no bytes available
    {
      ...
      const ssize_t bytes_recvd =
          udpClient->Recv((char*)recvBuf, kMaxRecvBufSize);
      ...
      if (bytes_recvd < 0) {
        // Disconnect client
        PRINT_NAMED_INFO("MultiClientComms.ReadAllMsgPackets",
                         "Recv failed. Disconnecting client");
...

      {
        ...
        {
          PRINT_NAMED_ERROR("MultiClientComms.ReadTruncated",
                            "Read %zd, buffer size only %zu", bytes_recvd,
                            kMaxRecvBufSize);
        }
        ...
        recvdMsgPackets_.emplace_back(
            std::piecewise_construct, std::forward_as_tuple(recvTime),
            std::forward_as_tuple((s32)(it->first), (s32)-1,
                                  Util::numeric_cast<u16>(bytes_recvd), recvBuf,
                                  currentTime_s));
      }
...
bool MultiClientComms::ConnectToDeviceByID(int devID) {
  ...
  if (it != advertisingDevices_.end()) {
    ...
    if (outClient->Connect(adMsg.ip.c_str(), adMsg.fromEnginePort)) {
      ...
      if (adMsg.fromEnginePort != adMsg.toEnginePort) {
        ...
        if (!inClient->Connect(adMsg.ip.c_str(), adMsg.toEnginePort)) {
          ...
          PRINT_NAMED_WARNING(
              "MultiClientComms.ConnectToDeviceByID.InFailed",
              "Connection attempt to device %d at %s:%d (ToEngine) failed",
      ...
      if (inClient) {
#if (DEBUG_COMMS)
        PRINT_NAMED_INFO("MultiClientComms.ConnectToDeviceByID",
                         "Connected to device %d at %s:%d/%d", adMsg.id,
                         adMsg.ip.c_str(), adMsg.toEnginePort,
                         adMsg.fromEnginePort);
#endif
    ...
    PRINT_NAMED_WARNING(
        "MultiClientComms.ConnectToDeviceByID.OutFailed",
        "Connection attempt to device %d at %s:%d (FromEngine) failed",
...
u32 MultiClientComms::ConnectToAllDevices() {
  for (advertisingDevicesIt_t it = advertisingDevices_.begin();
       it != advertisingDevices_.end(); it++) {
...
void MultiClientComms::DisconnectAllDevices() {
  for (connectedDevicesIt_t it = connectedDevices_.begin();
       it != connectedDevices_.end();) {
    it->second.DestroyClients();
...
u32 MultiClientComms::GetNumActiveConnectedDevices(double maxIdleTime_s) const {
  ...
  for (auto& kv : connectedDevices_) {
    ...
    const double secondsSinceLastRecv =
        currentTime_s - deviceInfo.GetLastRecvTime();
