 ...
 * Created: 8/1/2016
 *
 * Description: Monitors initial events after robot connection to determine
 *result to report
 *
...
class RobotInitialConnection : private Util::SignalHolder {
 public:
  ...
  // returns true if robot was in the process of connecting and we broadcasted a
  // connection failed message
  bool HandleDisconnect(RobotConnectionResult connectionResult);

  // returns if we should filter out (not deliver) a given message type to/from
  // this robot if firmware doesn't match, we'll do this to almost every message
