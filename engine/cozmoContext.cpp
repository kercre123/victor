...
void CozmoContext::SetSdkStatus(SdkStatusType statusType,
...
void CozmoContext::SetLocale(const std::string& localeString) {
  ...
  if (!localeString.empty()) {
    ...
    //    auto * audioController = (_audioServer ?
    //    _audioServer->GetAudioController() : NULL); auto *
    //    cozmoAudioController =
    //    dynamic_cast<CozmoAudioController*>(audioController); if (nullptr !=
    //    cozmoAudioController) {
...
bool CozmoContext::IsEngineThread() const {
  return Util::AreCpuThreadIdsEqual(_threadIdHolder->_id,
