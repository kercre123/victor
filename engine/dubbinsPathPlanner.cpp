...
EComputePathStatus DubbinsPlanner::ComputePath(const Pose3d& startPose,
  ...
  f32 startAngle = startPose.GetRotationAngle<'Z'>()
                       .ToFloat();  // Assuming robot is not tilted
  ...
  const float dotProductThreshold =
      0.0152f;  // 1.f - std::cos(DEG_TO_RAD(10)); // within 10 degrees
  if (!rotAngle.IsNear(0.f, DEG_TO_RAD(10.f)) &&
      !NEAR(std::abs(dotProduct), 1.f, dotProductThreshold)) {
    PRINT_NAMED_ERROR("PathPlanner.GetPlan.NonZAxisRot_start",
                      "GetPlan() does not support rotations around anything "
                      "other than z-axis (%f %f %f)",
  ...
  f32 targetAngle = targetPose.GetRotationAngle<'Z'>()
                        .ToFloat();  // Assuming robot is not tilted
  ...
  if (Planning::GenerateDubinsPath(
          _path, startPt.x(), startPt.y(), startAngle, targetPt.x(),
          targetPt.y(), targetAngle,
          std::min(DUBINS_START_RADIUS_MM, dubinsRadius),
          std::min(DUBINS_END_RADIUS_MM, dubinsRadius),
          DUBINS_TARGET_SPEED_MMPS, DUBINS_ACCEL_MMPS2,
          DUBINS_DECEL_MMPS2) == 0) {
    PRINT_CH_INFO("Planner", "GetPlan.NoPathFound",
                  "Could not generate Dubins path (startPose %f %f %f, "
                  "targetPose %f %f %f)",
                  startPt.x(), startPt.y(), startAngle, targetPt.x(),
                  targetPt.y(), targetAngle);
    return EComputePathStatus::Error;
  } else {
    _hasValidPath = true;
