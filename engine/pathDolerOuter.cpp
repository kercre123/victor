...
void PathDolerOuter::ReplacePath(const Planning::Path& newPath) {
  // TODO: (mrw) we probably want to check that the already doled segments are
  // equal for safety
  path_ = newPath;
...
void PathDolerOuter::Dole(size_t numToDole) {
  DEV_ASSERT(msgHandler_ != nullptr,
             "PathDolerOuter.Dole.InvalidMessageHandler");
  ...
  LOG_DEBUG("PathDolerOuter.Dole",
            "Should dole from %d to %zu (totalSegments = %zu)",
            lastDoledSegmentIdx_ + 1, endIdx, pathSizeOnBasestation_);

  for (size_t i = (size_t)lastDoledSegmentIdx_ + 1; i <= endIdx; ++i) {
    // Get reference to current segment
    const auto& segment =
        path_.GetSegmentConstRef(Util::numeric_cast<uint8_t>(i));

    LOG_DEBUG("PathDolerOuter.Dole", "Doling out basestation idx %zu : %s", i,
    ...
    switch (segment.GetType()) {
      case Planning::PST_LINE: {
        ...
        if (msgHandler_->SendMessage(
                RobotInterface::EngineToRobot(std::move(m))) == RESULT_FAIL) {
...
void PathDolerOuter::Update(const s8 currPathIdx) {
  // If there is a free slot on the robot and there are segments left to dole,
  // then dole
  const int numFreeSlots =
      MAX_NUM_PATH_SEGMENTS_ROBOT - (lastDoledSegmentIdx_ - currPathIdx) - 1;

  if ((numFreeSlots > 0) && (pathSizeOnBasestation_ > 0) &&
      (lastDoledSegmentIdx_ < pathSizeOnBasestation_ - 1)) {
    Dole(numFreeSlots);
  }
}
