...
const Pose2d kBlockPreDockPoseOffset = {0, 0,
                                        DEFAULT_MIN_PREDOCK_POSE_DISTANCE_MM};

// Static helper for looking up block properties by type
const Block::BlockInfoTableEntry_t& Block::LookupBlockInfo(
...
#include "engine/BlockDefinitions.h"
  ...
  DEV_ASSERT(entry != BlockInfoLUT.end(),
...
void Block::AddFace(const FaceName whichFace, const Vision::MarkerType& code,
                    const float markerSize_mm, const u8 dockOrientations,
  ...
  DEV_ASSERT(halfDepth > 0.f && halfHeight > 0.f && halfWidth > 0.f,
             "Block.AddFace.InvalidHalfSize");

  // The poses here are based on the Marker's canonical pose being in the
  // X-Z plane
  // NOTE: these poses intentionally have no parent. That is handled by
  // AddMarker below.
  switch (whichFace) {
    case FRONT_FACE:
    ...
    case TOP_FACE:
      ...
      facePose = Pose3d(2.09439510f, {-0.57735027f, 0.57735027f, -0.57735027f},
                        {0.f, 0.f, halfHeight});
      break;
...
void Block::GeneratePreActionPoses(
      ...
      switch (type) {
        case PreActionPose::ActionType::DOCKING: {
          if (face.dockOrientations & (1 << rot)) {
            Pose3d preDockPose(
                M_PI_2 + kBlockPreDockPoseOffset.GetAngle().ToFloat(),
                Z_AXIS_3D(),
                {kBlockPreDockPoseOffset.GetX(),
                 -kBlockPreDockPoseOffset.GetY(), -halfHeight},
                marker.GetPose());

            preDockPose.RotateBy(Rvec);

            preActionPoses.emplace_back(PreActionPose::DOCKING, &marker,
                                        preDockPose,
        ...
        case PreActionPose::ActionType::FLIPPING: {
          // Flip preActionPoses are at the corners of the block so need to
          // divided by root2 to get x and y dist
          const float flipPreActionPoseDist =
              FLIP_PREDOCK_POSE_DISTAMCE_MM / 1.414f;

          if (face.dockOrientations & (1 << rot)) {
            Pose3d preDockPose(
                M_PI_2 + M_PI_4 + kBlockPreDockPoseOffset.GetAngle().ToFloat(),
                Z_AXIS_3D(),
                {flipPreActionPoseDist + halfWidth, -flipPreActionPoseDist,
                 -halfHeight},
                marker.GetPose());

            preDockPose.RotateBy(Rvec);

            preActionPoses.emplace_back(PreActionPose::FLIPPING, &marker,
                                        preDockPose, 0);
          }
          break;
        }
        case PreActionPose::ActionType::PLACE_ON_GROUND: {
          const f32 DefaultPrePlaceOnGroundDistance =
              ORIGIN_TO_LIFT_FRONT_FACE_DIST_MM - DRIVE_CENTER_OFFSET;

          // Add a pre-placeOnGround pose to each face, where the robot will be
          // sitting relative to the face when we put down the block -- one for
          // each orientation of the block.
          Pose3d prePlaceOnGroundPose(
              M_PI_2, Z_AXIS_3D(),
          ...
          preActionPoses.emplace_back(PreActionPose::PLACE_ON_GROUND, &marker,
                                      prePlaceOnGroundPose, 0);
          break;
        }
        case PreActionPose::ActionType::PLACE_RELATIVE: {
          // Add a pre-placeRelative pose to each face, where the robot should
          // be before it approaches the block in order to place a carried
          // object on top of or in front of it.
          Pose3d prePlaceRelativePose(
              M_PI_2, Z_AXIS_3D(),
              Point3f{0.f, -PLACE_RELATIVE_MIN_PREDOCK_POSE_DISTANCE_MM,
                      -halfHeight},
              marker.GetPose());

          prePlaceRelativePose.RotateBy(Rvec);

          preActionPoses.emplace_back(
              PreActionPose::PLACE_RELATIVE, &marker, prePlaceRelativePose,
        ...
        case PreActionPose::ActionType::ROLLING: {
          if (face.rollOrientations & (1 << rot)) {
            ...
            preActionPoses.emplace_back(PreActionPose::ROLLING, &marker,
                                        preDockPose,
                                        DEFAULT_PREDOCK_POSE_LINE_LENGTH_MM);
          }
          break;
...
Block::Block(const ObjectType& type, const ActiveID& activeID,
  ...
  DEV_ASSERT(IsBlockType(type, false), "Block.InvalidType");
  ...
  for (const auto& face : LookupBlockInfo(_type).faces) {
    AddFace(face.whichFace, face.code, face.size, face.dockOrientations,
            face.rollOrientations);
  }

  // Every block should at least have a front face defined in the
  // BlockDefinitions file
  DEV_ASSERT(markersByFace_[FRONT_FACE] != NULL,
             "Block.Constructor.InvalidFrontFace");

  // Skip the check for ghost objects, which are a special case (they have 6
  // unknown markers)
  if (ANKI_DEVELOPER_CODE && (type != ObjectType::Block_LIGHTCUBE_GHOST)) {
    ...
    DEV_ASSERT(uniqueCodes.size() == markerList.size(),
               "Block.Constructor.InvalidMarkerList");
  }
...
const std::vector<Point3f>& Block::GetCanonicalCorners() const {
  static const std::vector<Point3f> CanonicalCorners = {
      {Point3f(-0.5f, -0.5f, 0.5f), Point3f(0.5f, -0.5f, 0.5f),
       Point3f(-0.5f, -0.5f, -0.5f), Point3f(0.5f, -0.5f, -0.5f),
       Point3f(-0.5f, 0.5f, 0.5f), Point3f(0.5f, 0.5f, 0.5f),
       Point3f(-0.5f, 0.5f, -0.5f), Point3f(0.5f, 0.5f, -0.5f)}};
...
void Block::GetCorners(const Pose3d& atPose,
...
// to the block's size
Quad2f Block::GetBoundingQuadXY(const Pose3d& atPose,
  ...
  // Re-center
  Point2f center(atPoseWrtOrigin.GetTranslation().x(),
                 atPoseWrtOrigin.GetTranslation().y());
  boundingQuad += center;
...
Block::FaceName& operator++(Block::FaceName& fname) {
  fname = (fname < Block::NUM_FACES)
              ? static_cast<Block::FaceName>(static_cast<int>(fname) + 1)
              : Block::NUM_FACES;
  return fname;
}
...
Vision::KnownMarker const& Block::GetMarker(FaceName onFace) const {
  static const Block::FaceName OppositeFaceLUT[Block::NUM_FACES] = {
      Block::BACK_FACE, Block::RIGHT_FACE,  Block::FRONT_FACE,
      Block::LEFT_FACE, Block::BOTTOM_FACE, Block::TOP_FACE};
  ...
  if (markerPtr == NULL) {
    ...
    } else if ((markerPtr = markersByFace_
                    [OppositeFaceLUT[onFace] /*GetOppositeFace(onFace)*/]) ==
               NULL) {
...
const Vision::KnownMarker& Block::GetTopMarker(
    Pose3d& topMarkerPoseWrtOrigin) const {
  ...
  for (auto marker = _markers.begin(); marker != _markers.end(); ++marker) {
    ...
    const f32 currentDotProd =
        DotProduct(marker->ComputeNormal(poseWrtOrigin), Z_AXIS_3D());
...
void Block::Visualize(const ColorRGBA& color) const {
  ...
  _vizHandle =
      _vizManager->DrawCuboid(GetID().GetValue(), _size, vizPose, color);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void Block::SetLEDs(const WhichCubeLEDs whichLEDs, const ColorRGBA& onColor,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void Block::MakeStateRelativeToXY(const Point2f& xyPosition,
                                  MakeRelativeMode mode) {
  ...
  switch (mode) {
    ...
    default:
      LOG_ERROR("Block.MakeStateRelativeToXY",
                "Unrecognized relative LED mode %s.",
                MakeRelativeModeToString(mode));
      return;
  }
  ...
  switch (referenceLED) {
    ...
    default:
      LOG_ERROR("Block.MakeStateRelativeToXY", "Unexpected reference LED %d.",
                static_cast<int>(referenceLED));
      return;
  }
...
WhichCubeLEDs Block::MakeWhichLEDsRelativeToXY(const WhichCubeLEDs whichLEDs,
  ...
  switch (mode) {
    ...
    default:
      LOG_ERROR("Block.MakeStateRelativeToXY",
                "Unrecognized relateive LED mode %s.",
                MakeRelativeModeToString(mode));
      return whichLEDs;
  }
  ...
  switch (referenceLED) {
    ...
    case WhichCubeLEDs::BACK:
      // Rotate two slots (either direction)
      // TODO: Do this in one shot
      return RotateWhichLEDsAroundTopFace(
          RotateWhichLEDsAroundTopFace(whichLEDs, true), true);
...
WhichCubeLEDs Block::GetCornerClosestToXY(const Point2f& xyPosition) const {
  ...
  LOG_INFO("Block.GetCornerClosestToXY",
           "Block %d's TopMarker is = %s, angle = %.3f deg", GetID().GetValue(),
  ...
  // assert(angle >= -M_PI && angle <= M_PI); // No longer needed: Radians class
  // handles this
...
WhichCubeLEDs Block::GetFaceClosestToXY(const Point2f& xyPosition) const {
  ...
  const Vec2f v(xyPosition.x() - topMarkerCenter.x(),
                xyPosition.y() - topMarkerCenter.y());
...
inline const u8* GetRotationLUT(bool clockwise) {
  static const u8 cwRotatedPosition[Block::NUM_LEDS] = {3, 0, 1, 2};
  static const u8 ccwRotatedPosition[Block::NUM_LEDS] = {1, 2, 3, 0};

  // Choose the appropriate LUT
  const u8* rotatedPosition =
      (clockwise ? cwRotatedPosition : ccwRotatedPosition);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
WhichCubeLEDs Block::RotateWhichLEDsAroundTopFace(WhichCubeLEDs whichLEDs,
                                                  bool clockwise) {
  const u8* rotatedPosition = GetRotationLUT(clockwise);
  ...
  for (u8 iLED = 0; iLED < NUM_LEDS; ++iLED) {
    ...
    rotatedWhichLEDs |= ((currentBit & (u8)whichLEDs) > 0)
                        << rotatedPosition[iLED];
