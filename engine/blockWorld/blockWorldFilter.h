 ...
 *    $LastChangedRevision$
 *
 * Description: A helper class for filtering searches through objects in
 *BlockWorld.
 *
...
class BlockWorldFilter {
 public:
  ...
  bool ConsiderOrigin(PoseOriginID_t objectOriginID,
                      PoseOriginID_t robotOriginID) const;
  bool ConsiderType(ObjectType type) const;
  bool ConsiderObject(const ObservableObject* object)
      const;  // Checks ID and runs FilterFcn(object)
  ...
  void AddFilterFcn(
      const FilterFcn& filterFcn);  // add to list of filters (all must pass)
  ...
  enum class OriginMode : uint8_t {
    InRobotFrame,     // Only objects in the current robot coordinate frame are
                      // returned (Default)
    NotInRobotFrame,  // Only objects *not* in the current robot coordinate
                      // frame are returned
 ...
 protected:
  ...
  template <class T>
  static bool ConsiderHelper(const std::set<T>& ignoreSet,
                             const std::set<T>& allowSet, T x);

};  // class BlockWorldFilter

#pragma mark - Inlined Implementations

inline void BlockWorldFilter::SetIgnoreOrigins(
    std::set<PoseOriginID_t>&& origins) {
...
inline void BlockWorldFilter::SetAllowedOrigins(
    std::set<PoseOriginID_t>&& origins) {
...
template <class T>
inline bool BlockWorldFilter::ConsiderHelper(const std::set<T>& ignoreSet,
                                             const std::set<T>& allowSet, T x) {
...
inline bool BlockWorldFilter::ConsiderOrigin(PoseOriginID_t objectOrigin,
                                             PoseOriginID_t robotOrigin) const {
  switch (_originMode) {
...
inline bool BlockWorldFilter::ConsiderObject(
    const ObservableObject* object) const {
  DEV_ASSERT(nullptr != object, "BlockWorldFilter.ConsiderObject.NullObject");

  const bool considerObj =
      ConsiderHelper(_ignoreIDs, _allowedIDs, object->GetID());
  if (considerObj) {
...
inline bool BlockWorldFilter::PoseStateKnownFilter(
    const ObservableObject* object) {
  DEV_ASSERT(nullptr != object,
             "BlockWorldFilter.PoseStateKnownFilter.NullObject");
  return object->IsPoseStateKnown();
}

inline bool BlockWorldFilter::ActiveObjectsFilter(
    const ObservableObject* object) {
  DEV_ASSERT(nullptr != object,
             "BlockWorldFilter.ActiveObjectsFilter.NullObject");
  return object->IsActive();
}

inline bool BlockWorldFilter::UniqueObjectsFilter(
    const ObservableObject* object) {
  DEV_ASSERT(nullptr != object,
             "BlockWorldFilter.UniqueObjectsFilter.NullObject");
  return object->IsUnique();
}

inline bool BlockWorldFilter::IsLightCubeFilter(
    const ObservableObject* object) {
  DEV_ASSERT(nullptr != object,
             "BlockWorldFilter.IsLightCubeFilter.NullObject");
  return IsValidLightCube(object->GetType(), false);
}

inline bool BlockWorldFilter::IsCustomObjectFilter(
    const ObservableObject* object) {
  DEV_ASSERT(nullptr != object,
             "BlockWorldFilter.IsCustomObjectFilter.NullObject");
