 ...
 * Created: 10/1/2013
 *
 * Description: Implements a container for tracking the state of all objects in
 *Cozmo's world.
 *
...
BlockWorld::BlockWorld()
    : UnreliableComponent<BCComponentID>(this, BCComponentID::BlockWorld),
      IDependencyManagedComponent<RobotComponentID>(
          this, RobotComponentID::BlockWorld) {}  // BlockWorld() Constructor

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void BlockWorld::InitDependent(Robot* robot,
...
#ifdef SIMULATOR
  // VIC-12886 These object types are only used in Webots tests (not in the real
  // world), so only define them if this is sim. The physical robot can
  // sometimes hallucinate these objects, which causes issues.
...
void BlockWorld::UpdateDependent(const RobotCompMap& dependentComps) {
  // Check for any objects that overlap with the robot's position, and mark them
  // dirty
...
void BlockWorld::SetupEventHandlers(IExternalInterface& externalInterface) {
  ...
  helper
      .SubscribeGameToEngine<MessageGameToEngineTag::DeleteAllCustomObjects>();
  helper.SubscribeGameToEngine<
      MessageGameToEngineTag::UndefineAllCustomMarkerObjects>();
  helper.SubscribeGameToEngine<
      MessageGameToEngineTag::DeleteCustomMarkerObjects>();
  helper.SubscribeGameToEngine<
      MessageGameToEngineTag::DeleteFixedCustomObjects>();
  helper.SubscribeGameToEngine<MessageGameToEngineTag::SelectNextObject>();
  helper
      .SubscribeGameToEngine<MessageGameToEngineTag::CreateFixedCustomObject>();
...
BlockWorld::~BlockWorld() {}  // ~BlockWorld() Destructor

Result BlockWorld::DefineObject(
    std::unique_ptr<const ObservableObject>&& object) {
  ...
  if (RESULT_OK == addResult) {
    PRINT_CH_DEBUG("BlockWorld",
                   "BlockWorld.DefineObject.AddedObjectDefinition",
                   "Defined %s in Object Library", EnumToString(objType));

    if (redefiningExistingType) {
      PRINT_NAMED_WARNING(
          "BlockWorld.DefineObject.RemovingObjectsWithPreviousDefinition",
...
template <>
void BlockWorld::HandleMessage(
    const ExternalInterface::DeleteFixedCustomObjects& msg) {
  BlockWorldFilter filter;
  filter.SetOriginMode(BlockWorldFilter::OriginMode::InAnyFrame);
  ...
  _robot->GetContext()
      ...
      ->BroadcastToGame<ExternalInterface::RobotDeletedFixedCustomObjects>();
}

template <>
void BlockWorld::HandleMessage(
    const ExternalInterface::DeleteCustomMarkerObjects& msg) {
  BlockWorldFilter filter;
  filter.SetOriginMode(BlockWorldFilter::OriginMode::InAnyFrame);
  filter.AddFilterFcn(&BlockWorldFilter::IsCustomObjectFilter);
  filter.AddIgnoreType(
      ObjectType::CustomFixedObstacle);  // everything custom _except_ fixed
                                         // obstacles
  DeleteLocatedObjects(filter);
  _robot->GetContext()
      ->GetExternalInterface()
      ->BroadcastToGame<ExternalInterface::RobotDeletedCustomMarkerObjects>();
}

template <>
void BlockWorld::HandleMessage(
    const ExternalInterface::DeleteAllCustomObjects& msg) {
  BlockWorldFilter filter;
  filter.SetOriginMode(BlockWorldFilter::OriginMode::InAnyFrame);
  filter.AddFilterFcn(&BlockWorldFilter::IsCustomObjectFilter);
  DeleteLocatedObjects(filter);
  _robot->GetContext()
      ->GetExternalInterface()
      ->BroadcastToGame<ExternalInterface::RobotDeletedAllCustomObjects>();
};

template <>
void BlockWorld::HandleMessage(
    const ExternalInterface::UndefineAllCustomMarkerObjects& msg) {
  ...
  // Remove the definition of anything that uses any Custom marker from the
  // ObsObjLibrary
  static_assert(Util::EnumToUnderlying(CustomObjectMarker::Circles2) == 0,
                "Assuming first CustomObjectMarker is Circles2");

  s32 numRemoved = 0;
  for (auto customMarker = CustomObjectMarker::Circles2;
       customMarker < CustomObjectMarker::Count; ++customMarker) {
    const Vision::MarkerType markerType =
        CustomObject::GetVisionMarkerType(customMarker);
  ...
  PRINT_CH_INFO("BlockWorld",
                "BlockWorld.HandleMessage.UndefineAllCustomObjects",
...
template <>
void BlockWorld::HandleMessage(
    const ExternalInterface::CreateFixedCustomObject& msg) {
  Pose3d newObjectPose(msg.pose, _robot->GetPoseOriginList());

  ObjectID id = BlockWorld::CreateFixedCustomObject(newObjectPose, msg.xSize_mm,
                                                    msg.ySize_mm, msg.zSize_mm);

  _robot->GetContext()
      ->GetExternalInterface()
      ->BroadcastToGame<ExternalInterface::CreatedFixedCustomObject>(id);
};
...
void BlockWorld::HandleMessage(const ExternalInterface::DefineCustomBox& msg) {
  ...
  CustomObject* customBox = CustomObject::CreateBox(
      msg.customType, msg.markerFront, msg.markerBack, msg.markerTop,
      msg.markerBottom, msg.markerLeft, msg.markerRight, msg.xSize_mm,
      msg.ySize_mm, msg.zSize_mm, msg.markerWidth_mm, msg.markerHeight_mm,
      msg.isUnique);

  if (nullptr != customBox) {
    const Result defineResult =
        DefineObject(std::unique_ptr<CustomObject>(customBox));
    success = (defineResult == RESULT_OK);
  }

  _robot->GetContext()
      ->GetExternalInterface()
      ->BroadcastToGame<ExternalInterface::DefinedCustomObject>(success);
};
...
void BlockWorld::HandleMessage(const ExternalInterface::DefineCustomCube& msg) {
  ...
  CustomObject* customCube = CustomObject::CreateCube(
      msg.customType, msg.marker, msg.size_mm, msg.markerWidth_mm,
      msg.markerHeight_mm, msg.isUnique);

  if (nullptr != customCube) {
    const Result defineResult =
        DefineObject(std::unique_ptr<CustomObject>(customCube));
    success = (defineResult == RESULT_OK);
  }
...
void BlockWorld::HandleMessage(const ExternalInterface::DefineCustomWall& msg) {
  ...
  CustomObject* customWall = CustomObject::CreateWall(
      msg.customType, msg.marker, msg.width_mm, msg.height_mm,
      msg.markerWidth_mm, msg.markerHeight_mm, msg.isUnique);
  if (nullptr != customWall) {
    const Result defineResult =
        DefineObject(std::unique_ptr<CustomObject>(customWall));
    success = (defineResult == RESULT_OK);
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
ObservableObject* BlockWorld::FindLocatedObjectHelper(
    const BlockWorldFilter& filter, const ModifierFcn& modifierFcn,
    bool returnFirstFound) const {
  ObservableObject* matchingObject = nullptr;

  const auto& currRobotOriginId =
  ...
  for (const auto& objectsByOrigin : _locatedObjects) {
    ...
    for (const auto& object : objectsByOrigin.second) {
      if (nullptr == object) {
        LOG_ERROR("BlockWorld.FindLocatedObjectHelper.NullObject", "origin %d",
                  originID);
        continue;
      }
...
Block* BlockWorld::FindConnectedObjectHelper(const BlockWorldFilter& filter,
  ...
  for (auto& connectedObject : _connectedObjects) {
    ...
    const bool objectMatches =
        filter.ConsiderType(connectedObject->GetType()) &&
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
ObservableObject* BlockWorld::GetLocatedObjectByIdHelper(
    const ObjectID& objectID) const {
  // Find the object with the given ID with any pose state, in the current world
  // origin
  BlockWorldFilter filter;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Block* BlockWorld::GetConnectedBlockByActiveIdHelper(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
ObservableObject* BlockWorld::FindLocatedObjectClosestToHelper(
    const Pose3d& pose, const Vec3f& distThreshold,
    const BlockWorldFilter& filterIn) const {
  // TODO: Keep some kind of OctTree data structure to make these queries
  // faster?

  // Note: This function only considers the magnitude of distThreshold, not the
  // individual elements (see VIC-12526)
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
ObservableObject* BlockWorld::FindLocatedClosestMatchingObjectHelper(
    const ObservableObject& object, const Vec3f& distThreshold,
    const Radians& angleThreshold, const BlockWorldFilter& filterIn) const {
  ...
  filter.AddFilterFcn(
      [&object, &closestDist, &closestAngle](const ObservableObject* current) {
        Vec3f Tdiff;
        Radians angleDiff;
        if (current->IsSameAs(object, closestDist, closestAngle, Tdiff,
                              angleDiff)) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
ObservableObject* BlockWorld::FindLocatedClosestMatchingTypeHelper(
  ...
  BlockWorldFilter filter(filterIn);
  filter.AddFilterFcn([withType, &pose, &closestDist,
                       &closestAngle](const ObservableObject* current) {
    Vec3f Tdiff;
    Radians angleDiff;
    if (current->GetType() == withType &&
        current->GetPose().IsSameAs(pose, closestDist, closestAngle, Tdiff,
                                    angleDiff)) {
      closestDist = Tdiff.GetAbs();
      closestAngle = angleDiff.getAbsoluteVal();
      return true;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
BlockWorld::ObjectsContainer_t::const_iterator
BlockWorld::FindInContainerWithID(const ObjectsContainer_t& container,
                                  const ObjectID& objectID) const {
  return std::find_if(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result BlockWorld::BroadcastObjectObservation(
  ...
  _robot->GetVisionComponent().GetCamera().ProjectObject(
      *observedObject, projectedCorners, observationDistance);
  ...
  if (observedObject->IsActive()) {
    if (IsValidLightCube(observedObject->GetType(), false)) {
      ...
      if (activeCube == nullptr) {
        PRINT_NAMED_ERROR(
            "BlockWorld.BroadcastObjectObservation.NullActiveCube",
            "ObservedObject %d with IsActive()==true could not be cast to "
            "ActiveCube.",
  ...
  RobotObservedObject observation(
      observedObject->GetLastObservedTime(),
      LegacyGetObjectFamily(observedObject), observedObject->GetType(),
      observedObject->GetID(),
      CladRect(boundingBox.GetX(), boundingBox.GetY(), boundingBox.GetWidth(),
               boundingBox.GetHeight()),
      observedObject->GetPose().ToPoseStruct3d(_robot->GetPoseOriginList()),
      topMarkerOrientation.ToFloat(), observedObject->IsActive());
...
void BlockWorld::BroadcastLocatedObjectStates() {
  ...
  filter.SetFilterFcn([this, &objectStates](const ObservableObject* obj) {
    ...
    LocatedObjectState objectState(
        obj->GetID(), obj->GetLastObservedTime(), obj->GetType(),
        obj->GetPose().ToPoseStruct3d(_robot->GetPoseOriginList()),
        obj->GetPoseState(), isConnected);

    objectStates.objects.push_back(std::move(objectState));
    return true;
  });

  // Iterate over all objects and add them to the available objects list if they
  // pass the filter
...
void BlockWorld::BroadcastConnectedObjects() {
  ...
  filter.SetFilterFcn([&objectStates](const ObservableObject* obj) {
    ConnectedObjectState objectState(obj->GetID(), obj->GetType());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result BlockWorld::UpdateObjectOrigin(const ObjectID& objectID,
                                      const PoseOriginID_t oldOriginID) {
  ...
  if (objectIt == objectsInOldOrigin.end()) {
    LOG_INFO("BlockWorld.UpdateObjectOrigin.ObjectNotFound",
             "Object %d not found in origin %s", objectID.GetValue(),
  ...
  if (!object->GetPose().HasSameRootAs(oldOrigin)) {
    ...
    LOG_INFO("BlockWorld.UpdateObjectOrigin.ObjectFound",
             "Updating ObjectID %d from origin %s to %s", objectID.GetValue(),
    ...
    // Add to object's current origin (if it's there already, issue a warning
    // and remove the duplicate first)
    auto& objectsInNewOrigin = _locatedObjects[newOriginID];
    auto existingIt =
        FindInContainerWithID(objectsInNewOrigin, object->GetID());
    if (existingIt != objectsInNewOrigin.end()) {
      LOG_WARNING(
          "BlockWorld.UpdateObjectOrigin.ObjectAlreadyInNewOrigin",
...
Result BlockWorld::UpdateObjectOrigins(PoseOriginID_t oldOriginID,
  ...
  DEV_ASSERT_MSG(_robot->GetPoseOriginList().ContainsOriginID(oldOriginID),
                 "BlockWorld.UpdateObjectOrigins.BadOldOriginID", "ID:%d",
                 oldOriginID);

  DEV_ASSERT_MSG(_robot->GetPoseOriginList().ContainsOriginID(newOriginID),
                 "BlockWorld.UpdateObjectOrigins.BadNewOriginID", "ID:%d",
                 newOriginID);
  ...
  ModifierFcn originUpdater = [&oldOrigin, &newOrigin, newOriginID, &result,
                               this](ObservableObject* oldObject) {
    Pose3d newPose;

    if (_robot->GetCarryingComponent().IsCarryingObject(oldObject->GetID())) {
      // Special case: don't use the pose w.r.t. the origin b/c carried objects'
      // parent is the lift. The robot is already in the new frame by the time
      // this called, so we don't need to adjust anything
      DEV_ASSERT(
          _robot->GetPoseOriginList().GetCurrentOriginID() == newOriginID,
          "BlockWorld.UpdateObjectOrigins.RobotNotInNewOrigin");
      DEV_ASSERT(
          oldObject->GetPose().GetRootID() == newOriginID,
          "BlockWorld.UpdateObjectOrigins.OldCarriedObjectNotInNewOrigin");
      newPose = oldObject->GetPose();
    } else if (false ==
               oldObject->GetPose().GetWithRespectTo(newOrigin, newPose)) {
    ...
    // Look for a matching object in the new origin. Should have same type. If
    // unique, should also have same ID, or if not unique, the poses should
    // match.
    ...
    if (oldObject->IsUnique()) {
      ...
      newObject = FindLocatedObjectClosestTo(
          oldObject->GetPose(), oldObject->GetSameDistanceTolerance(),
          filterNew);
    }
    ...
    if (nullptr == newObject) {
      PRINT_CH_INFO(
          "BlockWorld", "BlockWorld.UpdateObjectOrigins.NoMatchFound",
          "No match found for %s %d, adding new at T=(%.1f,%.1f,%.1f)",
          EnumToString(oldObject->GetType()), oldObject->GetID().GetValue(),
      ...
      PRINT_CH_INFO(
          "BlockWorld", "BlockWorld.UpdateObjectOrigins.ObjectOriginChanged",
          "Updating %s %d's origin from %s to %s (matched by %s to ID:%d). "
          "T_old=(%.1f,%.1f,%.1f), T_new=(%.1f,%.1f,%.1f)",
          EnumToString(oldObject->GetType()), oldObject->GetID().GetValue(),
          oldOrigin.GetName().c_str(), newOrigin.GetName().c_str(),
          oldObject->IsUnique() ? "type" : "pose",
          newObject->GetID().GetValue(), T_old.x(), T_old.y(), T_old.z(),
          T_new.x(), T_new.y(), T_new.z());

      // we also want to keep the MOST recent objectID, rather than the one we
      // used to have for this object, because if clients are bookkeeping IDs,
      // the know about the new one (for example, if an action is already going
      // to pick up that objectID, it should not change by virtue of
      // rejiggering) Note: despite the name, oldObject is the most recent
      // instance of this match. Thanks, Andrew.
      newObject->CopyID(oldObject);
    }

    // Use all of oldObject's time bookkeeping, then update the pose and pose
    // state
    newObject->SetObservationTimes(oldObject);
    newObject->SetPose(newPose, oldObject->GetLastPoseUpdateDistance(),
                       oldObject->GetPoseState());
    ...
    if (addNewObject) {
      ...
      PRINT_CH_INFO("BlockWorld",
                    "BlockWorld.UpdateObjectOrigins.NoMatchingObjectInNewFrame",
                    "Adding %s object with ID %d to new origin %s",
                    EnumToString(newObject->GetType()),
                    newObject->GetID().GetValue(), newOrigin.GetName().c_str());
    }
  };
  ...
  if (RESULT_OK == result) {
    // Erase all the objects in the old frame now that their counterparts in the
    // new frame have had their poses updated rsam: Note we don't have to call
    // Delete since we don't clear or notify. There is no way that we could be
    // deleting any objects in this origin during rejigger, since we bring
    // objects to the previously known map or override their pose. For that
    // reason, directly remove the origin rather than calling
    // DeleteLocatedObjectsByOrigin Note that we decide to not notify of objects
    // that merge (passive matched by pose), because the old ID in the old
    // origin is not in the current one.
...
void BlockWorld::DeleteZombieOrigins() {
  for (auto originIt = _locatedObjects.begin();
       originIt != _locatedObjects.end();) {
    if (IsZombiePoseOrigin(originIt->first)) {
      LOG_INFO("BlockWorld.DeleteZombieOrigins.DeletingOrigin",
               "Deleting origin %d (which contained %zu objects) because it "
               "was zombie",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
Result BlockWorld::ProcessVisualObservations(
    const std::vector<std::shared_ptr<ObservableObject>>& objectsSeenRaw,
    const RobotTimeStamp_t atTimestamp) {
  // if there are no objects, then exit early. This might happen if we see an
  // SDK marker but have not created a custom object for it.
  if (objectsSeenRaw.empty()) {
    return RESULT_OK;
  }

  // We cannot trust observations of objects if we were off treads, so no need
  // to continue
  ...
  const bool wasCameraMoving =
      (_robot->GetMoveComponent().IsCameraMoving() ||
       _robot->GetMoveComponent().WasCameraMoving(atTimestamp));
  const bool canRobotLocalize = (_robot->GetLocalizedTo().IsUnknown() ||
  ...
  // VIC-14462: we no longer relocalize to objects in other origins due to
  // rejiggering bugs, and the map timing out anyway
  if (canRobotLocalize && (existingCharger != nullptr)) {
    // We already have a charger in the current origin - is it close enough to
    // its last pose to localize to it?
    const bool localizeToCharger = existingCharger->GetPose().IsSameAs(
        observedCharger->GetPose(), existingCharger->GetSameDistanceTolerance(),
    ...
    if (localizeToCharger) {
      ...
      for (auto& obj : objectsSeen) {
        obj->SetPose(newObjSeenIt->GetWithRespectToRoot(),
                     obj->GetLastPoseUpdateDistance(), obj->GetPoseState());
        ++newObjSeenIt;
      }
    }
  }
  ...
  if (canRobotLocalize && (existingCharger == nullptr)) {
    ...
    // NOTE: this just sets the "localizedTo" fields, and shouldn't update the
    // robot pose since the pose transformation with itself is the identity
    // transformation
    result =
        _robot->LocalizeToObject(observedCharger.get(), observedCharger.get());
  }

  // For any objects whose poses were just updated, broadcast information about
  // them now. Note that this list could be different from the objectsSeen list,
  // since we may have decided to ignore an object observation for some reason
  // (e.g. robot was moving too fast)
  ...
  for (auto* object : updatedNowObjects) {
    ...
    // If we are observing an object that we are supposed to be carrying, then
    // tell the robot we are no longer carrying it
    if (_robot->GetCarryingComponent().IsCarryingObject(object->GetID())) {
      LOG_INFO("BlockWorld.ProcessVisualObservations.SeeingCarriedObject",
               "We have observed object %d, so we must not be carrying it "
               "anymore. Unsetting as carried object.",
    ...
    _robot->GetMapComponent().UpdateObjectPose(*object, &oldPoseCopy,
                                               PoseState::Known);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void BlockWorld::UpdateKnownObjects(
    const std::vector<std::shared_ptr<ObservableObject>>& objectsSeen,
    const RobotTimeStamp_t atTimestamp, const bool ignoreCharger) {
  // Go through each observation and, if possible, associate it to an
  // already-known object. If the observation does not match any existing known
  // objects, we generate a new objectID for it and add it to the list of known
  // objects.
  for (const auto& objSeen : objectsSeen) {
    DEV_ASSERT(!objSeen->GetID().IsSet(),
               "BlockWorld.UpdateKnownObjects.SeenObjectAlreadyHasID");
    ...
    const bool isSelectedObject =
        ...
        ((_robot->GetDockingComponent().GetDockObject() ==
          matchingObject->GetID()) ||
         (_robot->GetCarryingComponent().IsCarryingObject(
             matchingObject->GetID())));

    // If we haven't found a match in the current origin, then continue looking
    // in other origins (for unique objects)
    if ((matchingObject == nullptr) && isUnique) {
      filter.SetOriginMode(BlockWorldFilter::OriginMode::InAnyFrame);
      matchingObject = FindLocatedMatchingObject(filter);
    }

    // Was the camera moving? If so, we must skip this observation _unless_ this
    // is the dock object or carry object. Might be sufficient to check for
    // movement at historical time, but to be conservative (and account for
    // timestamping inaccuracies?) we will also check _current_ moving status.
    const bool wasCameraMoving =
        (_robot->GetMoveComponent().IsCameraMoving() ||
         _robot->GetMoveComponent().WasCameraMoving(atTimestamp));
    const bool ignoreChargerAndIsCharger =
        (ignoreCharger && IsChargerType(objSeen->GetType(), false));
    ...
    if (matchingObject != nullptr) {
      ...
      const PoseID_t matchingObjectOrigin =
          matchingObject->GetPose().GetRootID();
      ...
      matchingObject->SetPose(objSeen->GetPose(), distToObjSeen,
                              PoseState::Known);

      // If we matched an object from a previous origin, we need to move it into
      // the current origin
      if (matchingObjectOrigin != _robot->GetWorldOriginID()) {
        UpdateObjectOrigin(matchingObject->GetID(), matchingObjectOrigin);
      }
    } else {
      // Did not find _any_ match for this object among located objects. If this
      // is an active object, maybe there is a known connected instance (e.g.,
      // robot has connected to a cube but has not yet visually observed it)
      // from which we can grab the objectID.
      if (objSeen->IsActive()) {
        BlockWorldFilter connectedFilter;
        connectedFilter.SetAllowedTypes({objSeen->GetType()});
        const auto* connectedBlock =
            FindConnectedMatchingBlock(connectedFilter);
        if (connectedBlock != nullptr) {
          DEV_ASSERT(connectedBlock->GetID().IsSet(),
                     "BlockWorld.UpdateKnownObjects.ConnectedObjectHasNoId");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
std::vector<std::shared_ptr<ObservableObject>>
BlockWorld::FilterRawObservedObjects(
    const std::vector<std::shared_ptr<ObservableObject>>& objectsSeenRaw) {
  ...
  objectsSeenFilt.erase(
      std::remove_if(objectsSeenFilt.begin(), objectsSeenFilt.end(),
                     [](const std::shared_ptr<ObservableObject>& obj) {
                       return obj->GetLastPoseUpdateDistance() >
                              obj->GetMaxObservationDistance_mm();
                     }),
      objectsSeenFilt.end());

  // Ignore duplicate 'unique' objects. For example, chargers are supposed to be
  // 'unique' objects, meaning we can only ever know about one of them at a
  // time. However, it is still possible to see two of them in the same image.
  // We only want to keep the closest one for consistency.
  //
  // First, sort the container by distance so that we can use std::unique to do
  // the work for us.

  std::sort(objectsSeenFilt.begin(), objectsSeenFilt.end(),
            [](const std::shared_ptr<ObservableObject>& objA,
               const std::shared_ptr<ObservableObject>& objB) {
              return objA->GetLastPoseUpdateDistance() <
                     objB->GetLastPoseUpdateDistance();
            });
...
void BlockWorld::CheckForUnobservedObjects(RobotTimeStamp_t atTimestamp) {
  ...
  if (_robot->GetOffTreadsState() != OffTreadsState::OnTreads ||
      ...
      _robot->GetImuComponent().GetImuHistory().WasRotatingTooFast(
          atTimestamp)) {
    return;
  }

  auto originIter =
      _locatedObjects.find(_robot->GetPoseOriginList().GetCurrentOriginID());
  if (originIter == _locatedObjects.end()) {
    // No objects relative to this origin: Nothing to do
    return;
  }

  // Create a list of unobserved object IDs (IDs since we can remove several of
  ...
  for (const auto& object : originIter->second) {
    ...
    const bool isUnobserved =
        ((lastObservedTime < atTimestamp) &&
         (_robot->GetCarryingComponent().GetCarryingObjectID() !=
          object->GetID()) &&
  ...
  DEV_ASSERT(camera.IsCalibrated(),
             "BlockWorld.CheckForUnobservedObjects.CameraNotCalibrated");
  for (const auto& objectID : unobservedObjectIDs) {
    // if the object doesn't exist anymore, it was deleted by another one, for
    // example through a stack or if doesn't have markers (like unexpected move
    // objects), skip
    ...
    if (unobservedObject->GetPose().GetWithRespectTo(camera.GetPose(),
                                                     objectPoseWrtCamera)) {
      // should have markers
      const auto& markerList = unobservedObject->GetMarkers();
      if (!markerList.empty()) {
        const float observationDistance =
            unobservedObject->GetMaxObservationDistance_mm();
        ...
        // distFactor = (1-distNorm) + 1; 1-distNorm to invert normalization, +1
        // because we want 100% at distNorm=1
        const f32 distToObjInvFactor =
            2 - (objectPoseWrtCamera.GetTranslation().Length() /
                 observationDistance);
        float xPadding = focalLenX * markerSize.x() * distToObjInvFactor /
                         observationDistance;
        float yPadding = focalLenY * markerSize.y() * distToObjInvFactor /
                         observationDistance;
        ...
        PRINT_NAMED_ERROR("BlockWorld.CheckForUnobservedObjects.NoMarkers",
                          "Object %d (Type:%s)", objectID.GetValue(),
                          EnumToString(unobservedObject->GetType()));
        continue;
      }
    } else {
      PRINT_NAMED_ERROR(
          "BlockWorld.CheckForUnobservedObjects.ObjectNotInCameraPoseOrigin",
          "Object %d (PosePath:%s)", objectID.GetValue(),
          unobservedObject->GetPose().GetNamedPathToRoot(false).c_str());
      continue;
    }

    // We want to remove objects that should have been visible from the current
    // pose, but were not observed for some reason. There are two scenarios:
    //   - If the object's pose is marked 'dirty' and we didn't see it, we
    //   immediately remove it.
    //   - If the object's pose is 'known', we only remove the object if we saw
    //   _another_ object behind it (proving
    //     that it really must not be there).
    //
    // Note: The return value of IsVisibleFrom() can be a source of confusion.
    // See VIC-13732 for details.
    bool hasNothingBehind = false;
    const bool shouldBeVisible = unobservedObject->IsVisibleFrom(
        camera, MAX_MARKER_NORMAL_ANGLE_FOR_SHOULD_BE_VISIBLE_CHECK_RAD,
        MIN_MARKER_SIZE_FOR_SHOULD_BE_VISIBLE_CHECK_PIX, xBorderPad, yBorderPad,
        hasNothingBehind);

    const bool isDirtyPoseState =
        (PoseState::Dirty == unobservedObject->GetPoseState());
    ...
    if (removeObject) {
      LOG_INFO("BlockWorld.CheckForUnobservedObjects.MarkingUnobservedObject",
               ...
               unobservedObject->GetID().GetValue(), shouldBeVisible,
               hasNothingBehind, isDirtyPoseState);
...
ObjectID BlockWorld::CreateFixedCustomObject(const Pose3d& p,
                                             const f32 xSize_mm,
                                             const f32 ySize_mm,
                                             const f32 zSize_mm) {
  // Create an instance of the custom obstacle
  CustomObject* customObstacle =
      CustomObject::CreateFixedObstacle(xSize_mm, ySize_mm, zSize_mm);
  ...
  // Initialize with Known pose so it won't delete immediately because it isn't
  // re-seen
  ...
  DEV_ASSERT(!customObject->GetID().IsSet(),
             "BlockWorld.CreateFixedCustomObject.NewObjectHasID");
  customObject->SetID();
...
ObjectID BlockWorld::AddConnectedBlock(const ActiveID& activeID,
  ...
  // only connected objects should be added through this method, so a required
  // activeID is a must
  DEV_ASSERT(activeID != ObservableObject::InvalidActiveID,
             "BlockWorld.AddConnectedBlock.CantAddInvalidActiveID");
  ...
  if (nullptr != conObjWithActiveID) {
    // Verify here that factoryID and objectType match, and if they do, simply
    // ignore the message, since we already have a valid instance
    const bool isSameObject =
        (factoryID == conObjWithActiveID->GetFactoryID()) &&
    ...
    // if it's not the same, then what the hell, we are currently using that
    // activeID for other object!
    LOG_ERROR("BlockWorld.AddConnectedBlock.ConflictingActiveID",
              "ActiveID:%d found when we tried to add that activeID as "
              "connected object. Removing previous.",
              activeID);
  ...
  const auto* const conObjectWithFactoryID =
      FindConnectedObjectHelper(filter, nullptr, true);
  ANKI_VERIFY(nullptr == conObjectWithFactoryID,
              "BlockWorld.AddConnectedBlock.FactoryIDAlreadyUsed", "%s",
              factoryID.c_str());

  // This is the new object we are going to create. We can't insert it in
  // _connectedObjects until we know the objectID, so we create it first, and
  // then we look for unconnected matches (we have seen the object but we had
  // not connected to it.) If we find one, we will inherit the objectID from
  // that match; if we don't find a match, we will assign it a new objectID.
  // Then we can add to the container of _connectedObjects.
  ...
  if (nullptr == newActiveObjectPtr) {
    // failed to create the object (that function should print the error, exit
    // here with unSet ID)
    return ObjectID();
  }
  ...
  // Is there an active object with the same activeID and type that already
  // exists?
  BlockWorldFilter filterByActiveID;
  filterByActiveID.SetOriginMode(BlockWorldFilter::OriginMode::InAnyFrame);
  filterByActiveID.AddFilterFcn([activeID](const ObservableObject* object) {
    return object->GetActiveID() == activeID;
  });
  ...
  if (matchingObjects.empty()) {
    // If no match found, find one of the same type with an invalid activeID and
    // assume that's the one we are connecting to
    ...
    if (!objectsOfSameType.empty()) {
      ...
      for (auto& sameTypeObject : objectsOfSameType) {
        if (matchObjectID.IsSet()) {
          ...
          DEV_ASSERT(matchObjectID == sameTypeObject->GetID(),
                     "BlockWorld.AddConnectedBlock.NotSameObjectID");
        } else {
          // set once
        ...
        if (sameTypeObject->GetActiveID() ==
            ObservableObject::InvalidActiveID) {
          ...
          LOG_INFO(
              "BlockWorld.AddConnectedBlock.FoundMatchingObjectWithNoActiveID",
              "objectID %d, activeID %d, type %s",
              sameTypeObject->GetID().GetValue(), sameTypeObject->GetActiveID(),
              EnumToString(objType));
        } else {
          ...
          if (sameTypeObject->GetFactoryID() != factoryID) {
            ...
            LOG_INFO(
                ...
                "ActiveID %d (factoryID %s) is same type as another existing "
                "object (objectID %d, activeID %d, factoryID %s, type %s) "
                "updating ids to match",
                activeID, factoryID.c_str(), sameTypeObject->GetID().GetValue(),
                sameTypeObject->GetActiveID(),
                sameTypeObject->GetFactoryID().c_str(), EnumToString(objType));

            // if we have a new factoryID, override the old instances with the
            // new one we connected to
            if (!factoryID.empty()) {
              sameTypeObject->SetActiveID(activeID);
              sameTypeObject->SetFactoryID(factoryID);
            }
            ...
            LOG_INFO(
                "BlockWorld.AddConnectedBlock."
                "FoundIdenticalObjectOnDifferentSlot",
                "Updating activeID of block with factoryID %s from %d to %d",
                sameTypeObject->GetFactoryID().c_str(),
                sameTypeObject->GetActiveID(), activeID);
    ...
    // We can't find more than one object of the same type in a single origin.
    // Otherwise something went really bad
    DEV_ASSERT(matchingObjects.size() <= 1,
               "BlockWorld.AddConnectedBlock.TooManyMatchingObjects");

    // NOTE: [MAM] This error has not happened for some time, so I removed a lot
    // of logic from this else block. Find it again here if it is needed:
    // https://github.com/anki/victor/blob/319a692ed2fc7cd85aa9009e415809cce827689a/engine/blockWorld/blockWorld.cpp#L1699
    // We should not find any objects in any origins that have this activeID.
    // Otherwise that means they have not disconnected properly. If there's a
    // timing issue with connecting an object to an activeID before
    // disconnecting a previous object, we would like to know, so we can act
    // accordingly. Add this error here to detect that situation.
    LOG_ERROR("BlockWorld.AddConnectedBlock.ConflictingActiveID",
              "Objects with ActiveID:%d were found when we tried to add that "
              "activeID as connected object.",
              activeID);
  }

  // at this point the new active connected object has a valid objectID, we can
  // finally add it to the world
  DEV_ASSERT(newActiveObjectPtr->GetID().IsSet(),
             "BlockWorld.AddConnectedBlock.ObjectIDWasNeverSet");
...
ObjectID BlockWorld::RemoveConnectedBlock(const ActiveID& activeID) {
    ...
    if (objPtr == nullptr) {
      ...
      if (removedObjectID.IsSet()) {
        LOG_ERROR("BlockWorld.RemoveConnectedBlock.DuplicateEntry",
                  "Duplicate entry found in _connectedObjects for object with "
                  "activeID %d. "
                  "Existing object ID %d, this object ID %d. Removing this "
                  "entry as well",
                  activeID, removedObjectID.GetValue(),
                  objPtr->GetID().GetValue());
      }
  ...
  if (removedObjectID.IsSet()) {
    ...
    matchingIDInAnyOrigin.SetOriginMode(
        BlockWorldFilter::OriginMode::InAnyFrame);
    ...
    ModifierFcn clearActiveID = [](ObservableObject* object) {
      ...
      object->SetFactoryID(
          ObservableObject::InvalidFactoryID);  // should not be needed. Should
                                                // we keep it?
    };
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void BlockWorld::AddLocatedObject(
    const std::shared_ptr<ObservableObject>& object) {
  DEV_ASSERT(object->HasValidPose(),
             "BlockWorld.AddLocatedObject.NotAValidPoseState");
  DEV_ASSERT(object->GetID().IsSet(),
             "BlockWorld.AddLocatedObject.ObjectIDNotSet");
  ...
  // rsam: I would like to do this in a cleaner way, maybe just refactoring the
  // code, but here seems fishy design-wise
  {
    ...
    // find by ObjectID. The objectID should match, since observations search
    // for objectID even in connected
  ...
  // not asserting in case SDK tries to do this, but do not add it to the
  // BlockWorld
  ...
  objectsInThisOrigin.push_back(
      object);  // store the new object, this increments refcount
  ...
  PRINT_CH_INFO("BlockWorld", "BlockWorld.AddLocatedObject",
                "Adding new %s%s object and ID=%d ActID=%d FacID=%s at (%.1f, "
                "%.1f, %.1f), in frame %s.",
                object->IsActive() ? "active " : "",
                EnumToString(object->GetType()), object->GetID().GetValue(),
                object->GetActiveID(), object->GetFactoryID().c_str(),
  ...
  // fire DAS event
  DASMSG(robot.object_located, "robot.object_located",
         "First time object has been seen in this origin");
  ...
  // make sure that everyone gets notified that there's a new object in town, I
  // mean in this origin
  {
    const Pose3d* oldPosePtr = nullptr;
    const PoseState oldPoseState = PoseState::Invalid;
    _robot->GetMapComponent().UpdateObjectPose(*object, oldPosePtr,
                                               oldPoseState);
  }
}
...
void BlockWorld::SetRobotOnChargerContacts() {
  ...
  if (charger != nullptr) {
    ...
    // Don't have a match in this origin, so create a new instance. If we have a
    // match in _another_ origin, copy its ID and delete it. Note: We could
    // localize to the existing charger here, but if we've gotten to this point
    // it likely means someone has picked up the robot and placed it on the
    // charger. If that's the case, we make the assumption that the world has
    // changed enough that we should just start anew rather than use an old
    // origin.
    ...
    newCharger->SetPose(poseWrtOrigin, poseWrtRobot.GetTranslation().Length(),
                        PoseState::Known);
    newCharger->SetLastObservedTime(
        (TimeStamp_t)_robot->GetLastImageTimeStamp());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
Result BlockWorld::SetObjectPose(const ObjectID& objId, const Pose3d& newPose,
  ...
  // Even if makeWrtOrigin is false, we still want to ensure that the given pose
  // is in the same origin as the robot's world origin.
  ...
  object->SetPose(newObjectPose, object->GetLastPoseUpdateDistance(),
                  poseState);

  // Inform map component of the updated pose
  _robot->GetMapComponent().UpdateObjectPose(*object, &object->GetPose(),
                                             object->GetPoseState());
...
void BlockWorld::MarkObjectDirty(ObservableObject* object) {
  DEV_ASSERT(object->HasValidPose(),
             "BlockWorld.MarkObjectDirty.CantChangePoseStateOfInvalidObjects");

  if (_robot->GetCarryingComponent().IsCarryingObject(object->GetID())) {
    LOG_WARNING("BlockWorld.MarkObjectDirty.CarryingObject",
                "Not marking carried object as dirty");
    return;
  }
...
void BlockWorld::OnRobotDelocalized(PoseOriginID_t newWorldOriginID) {
  // Since we are no longer relocalizing between deloc events, clear the current
  // set of objects
...
void BlockWorld::SanityCheckBookkeeping() const {
  ...
  for (auto const& objectsByOrigin : _locatedObjects) {
    ...
    for (auto const& object : objects) {
      ...
      if (object->IsUnique()) {
        ANKI_VERIFY(
            knownTypes.find(objType) == knownTypes.end(),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
Result BlockWorld::UpdateObservedMarkers(
    const std::list<Vision::ObservedMarker>& currentObsMarkers) {
  ANKI_CPU_PROFILE("BlockWorld::UpdateObservedMarkers");

  if (!currentObsMarkers.empty()) {
    const RobotTimeStamp_t atTimestamp =
        currentObsMarkers.front().GetTimeStamp();
    ...
    if (ANKI_DEVELOPER_CODE) {
      for (auto const& marker : currentObsMarkers) {
        if (marker.GetTimeStamp() != atTimestamp) {
          PRINT_NAMED_ERROR(
              "BlockWorld.UpdateObservedMarkers.MisMatchedTimestamps",
              "Expected t=%u, Got t=%u", (TimeStamp_t)atTimestamp,
              marker.GetTimeStamp());
          return RESULT_FAIL;
        }
      }
...
#define DISPLAY_ALL_OCCLUDERS 0
  if (DISPLAY_ALL_OCCLUDERS) {
    ...
    for (auto const& rect : occluders) {
      std::vector<cv::Point2i> points{rect.GetTopLeft().get_CvPoint_(),
                                      rect.GetTopRight().get_CvPoint_(),
                                      rect.GetBottomRight().get_CvPoint_(),
                                      rect.GetBottomLeft().get_CvPoint_()};
...
void BlockWorld::CheckForRobotObjectCollisions() {
  ...
  intersectingObjectFilter.SetOriginMode(
      BlockWorldFilter::OriginMode::InRobotFrame);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - -
bool BlockWorld::IntersectsRobotBoundingBox(
  ...
  const bool isDockingWithObject =
      _robot->GetDockingComponent().GetDockObject() == objectID;
  ...
  if (false == object->GetPose().GetWithRespectTo(_robot->GetWorldOrigin(),
                                                  objectPoseWrtRobotOrigin)) {
    LOG_WARNING(
        "BlockWorld.IntersectsRobotBoundingBox.BadOrigin",
  ...
  const Quad2f robotBBox =
      _robot->GetBoundingQuadXY(_robot->GetPose().GetWithRespectToRoot(),
...
void BlockWorld::ClearLocatedObjectHelper(ObservableObject* object) {
  ...
  if (_robot->GetLocalizedTo() == object->GetID()) {
    PRINT_CH_INFO(
        "BlockWorld", "BlockWorld.ClearObjectHelper.LocalizeRobotToNothing",
  ...
  if (_robot->GetCarryingComponent().GetCarryingObjectID() == object->GetID()) {
    PRINT_CH_INFO(
        "BlockWorld", "BlockWorld.ClearObjectHelper.ClearingCarriedObject",
  ...
  if (_selectedObjectID == object->GetID()) {
    PRINT_CH_INFO(
        "BlockWorld", "BlockWorld.ClearObjectHelper.ClearingSelectedObject",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void BlockWorld::FindConnectedMatchingBlocks(
    const BlockWorldFilter& filter, std::vector<const Block*>& result) const {
  // slight abuse of the FindObjectHelper, I just use it for filtering, then I
  // add everything that passes the filter to the result vector
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void BlockWorld::FindConnectedMatchingBlocks(const BlockWorldFilter& filter,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void BlockWorld::FindLocatedMatchingObjects(
    const BlockWorldFilter& filter, std::vector<ObservableObject*>& result) {
  // slight abuse of the FindLocatedObjectHelper, I just use it for filtering,
  // then I add everything that passes the filter to the result vector
  ModifierFcn addToResult = [&result](ObservableObject* candidateObject) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
const ObservableObject* BlockWorld::FindMostRecentlyObservedObject(
    const BlockWorldFilter& filterIn) const {
...
static inline BlockWorldFilter GetIntersectingObjectsFilter(
    const Quad2f& quad, f32 padding_mm, const BlockWorldFilter& filterIn) {
  BlockWorldFilter filter(filterIn);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void BlockWorld::FindLocatedIntersectingObjects(
  ...
  Quad2f quadSeen =
      objectSeen->GetBoundingQuadXY(objectSeen->GetPose(), padding_mm);
  FindLocatedMatchingObjects(
      GetIntersectingObjectsFilter(quadSeen, padding_mm, filter),
...
void BlockWorld::FindLocatedIntersectingObjects(
    const ObservableObject* objectSeen,
    std::vector<ObservableObject*>& intersectingExistingObjects, f32 padding_mm,
    const BlockWorldFilter& filter) {
  Quad2f quadSeen =
      objectSeen->GetBoundingQuadXY(objectSeen->GetPose(), padding_mm);
  FindLocatedMatchingObjects(
...
void BlockWorld::FindLocatedIntersectingObjects(
    const Quad2f& quad,
    std::vector<const ObservableObject*>& intersectingExistingObjects,
    f32 padding_mm, const BlockWorldFilter& filterIn) const {
  FindLocatedMatchingObjects(
      GetIntersectingObjectsFilter(quad, padding_mm, filterIn),
      intersectingExistingObjects);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void BlockWorld::GetLocatedObjectBoundingBoxesXY(
  ...
  filter.AddFilterFcn([minHeight, maxHeight, padding,
                       &rectangles](const ObservableObject* object) {
    const Point3f rotatedSize(object->GetPose().GetRotation() *
                              object->GetSize());
    const f32 objectCenter =
        object->GetPose().GetWithRespectToRoot().GetTranslation().z();

    const f32 objectTop = objectCenter + (0.5f * rotatedSize.z());
    const f32 objectBottom = objectCenter - (0.5f * rotatedSize.z());

    const bool bothAbove =
        (objectTop >= maxHeight) && (objectBottom >= maxHeight);
    const bool bothBelow =
        (objectTop <= minHeight) && (objectBottom <= minHeight);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void BlockWorld::GetObstacles(
    std::vector<std::pair<Quad2f, ObjectID>>& boundingBoxes,
    const f32 padding) const {
  BlockWorldFilter filter;
  ...
  // Figure out height filters in world coordinates (because
  // GetLocatedObjectBoundingBoxesXY() uses heights of objects in world
  // coordinates)
  ...
  GetLocatedObjectBoundingBoxesXY(minHeight, maxHeight, padding, boundingBoxes,
                                  filter);
}
...
bool BlockWorld::IsZombiePoseOrigin(PoseOriginID_t originID) const {
  ...
  DEV_ASSERT(_robot->GetPoseOriginList().ContainsOriginID(originID),
             "BlockWorld.IsZombiePoseOrigin.InvalidOriginID");

  // current world is not a zombie
  const bool isCurrent =
      (originID == _robot->GetPoseOriginList().GetCurrentOriginID());
  if (isCurrent) {
    return false;
  }
...
bool BlockWorld::AnyRemainingLocalizableObjects(PoseOriginID_t originID) const {
  // Filter out anything that can't be used for localization (i.e. only allow
  // charger)
  BlockWorldFilter filter;
  filter.SetAllowedTypes({ObjectType::Charger_Basic});

  // Allow all origins if UnknownOriginID was passed in, otherwise allow only
  ...
  const bool localizableObjectFound =
      (nullptr != FindLocatedObjectHelper(filter, nullptr, true));
...
void BlockWorld::DeleteLocatedObjects(const BlockWorldFilter& filter) {
  ...
  struct DeletedObjectInfo {
    DeletedObjectInfo(const Pose3d& oldPose, const PoseState oldPoseState,
                      const ObservableObject* objCopy)
        : _oldPose(oldPose),
          _oldPoseState(oldPoseState),
          _objectCopy(objCopy) {}
  ...
  for (auto& originPair : _locatedObjects) {
    ...
    if (filter.ConsiderOrigin(
            crntOriginID, _robot->GetPoseOriginList().GetCurrentOriginID())) {
      auto& objectContainer = originPair.second;
      for (auto objectIter = objectContainer.begin();
           objectIter != objectContainer.end();) {
        auto* object = objectIter->get();
        if (object == nullptr) {
          LOG_ERROR("BlockWorld.DeleteLocatedObjects.NullObject", "origin %d",
                    crntOriginID);
          ...
          if (isCurrentOrigin) {
            ...
            {
              DEV_ASSERT(object->HasValidPose(),
                         "BlockWorld.DeleteLocatedObjects.InvalidPoseState");
              ObservableObject* objCopy = object->CloneType();
              objCopy->CopyID(object);
              if (objCopy->IsActive()) {
                objCopy->SetActiveID(
                    object->GetActiveID());  // manually having to copy all IDs
                                             // is fishy design
                objCopy->SetFactoryID(object->GetFactoryID());
              }
              objectsToBroadcast.emplace_back(object->GetPose(),
                                              object->GetPoseState(), objCopy);
            }
          }
  ...
  for (auto& objectDeletedInfo : objectsToBroadcast) {
    ...
    // cache values
    const ObjectID& deletedID =
        objectDeletedInfo._objectCopy
            ->GetID();  // note this won't be valid after delete
    ...
    DEV_ASSERT(!objectDeletedInfo._objectCopy->HasValidPose(),
               "BlockWorld.DeleteLocatedObjects.CopyInheritedPose");
    _robot->GetMapComponent().UpdateObjectPose(*objectDeletedInfo._objectCopy,
                                               oldPosePtr, oldPoseState);
...
void BlockWorld::DeselectCurrentObject() {
  if (_selectedObjectID.IsSet()) {
    if (ENABLE_DRAWING) {
      // Erase the visualization of the selected object's preaction poses/lines.
      // Note we do this across all frames in case the selected object is in a
      // different origin and we have delocalized
      BlockWorldFilter filter;
      filter.SetOriginMode(BlockWorldFilter::OriginMode::InAnyFrame);
...
bool BlockWorld::SelectObject(const ObjectID& objectID) {
  ...
  if (newSelection != nullptr) {
    // Unselect current object of interest, if it still exists (Note that it may
    // just get reselected here, but I don't think we care.)
    DeselectCurrentObject();
...
void BlockWorld::CycleSelectedObject() {
  ...
  for (auto const& obj : allObjects) {
    ...
    if (object != nullptr &&
      ...
      if (currSelectedObjectFound) {
        ...
        // PRINT_INFO("new block found: id %d  type %d", block.first,
        // blockType.first);
        break;
      } else if (object->GetID() == _selectedObjectID) {
        currSelectedObjectFound = true;
        if (ENABLE_DRAWING) {
          // Erase the visualization of the current selection so we can draw
          // only the the new one (even if we end up just re-drawing this one)
          object->EraseVisualization();
        }
        // PRINT_INFO("curr block found: id %d  type %d", block.first,
        // blockType.first);
      }
    }
  ...
  if (_selectedObjectID.IsSet()) {
    ...
    PRINT_CH_DEBUG("BlockWorld", "BlockWorld.CycleSelectedObject.NoObject",
                   "No object of interest found");
  }
...
void BlockWorld::DrawAllObjects() const {
  ...
  auto webSender = WebService::WebVizSender::CreateWebVizSender(
      "navmap", _robot->GetContext()->GetWebService());
  ...
  ModifierFcn visualizeHelper = [this, &locObject,
                                 webSender](ObservableObject* object) {
    if (object->GetID() == _selectedObjectID) {
      ...
      const ActionableObject* selectedObject =
          dynamic_cast<const ActionableObject*>(object);
  ...
  // don't fill type unless there's some actual data (to avoid unnecessary
  // sends)
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
ObjectFamily BlockWorld::LegacyGetObjectFamily(
