 ...
 * Created: 07/30/15
 *
 * Description: Manager for events to be used across the engine; responsible for
 *keeping track of events that can trigger, registered listeners, and
 *dispatching events when they occur.
 *
...
class AnkiEventMgrBase : private Util::noncopyable {
 public:
  ...
  virtual void UnsubscribeAll() { _eventHandlerMap.clear(); }
...
// Original templated AnkiEventMgr type (with a default type for the signal
// structure)
template <typename DataType,
          typename SignalStruct = EventHandlerSignal<DataType> >
class AnkiEventMgr : public AnkiEventMgrBase<DataType, SignalStruct> {
 public:
  ...
  Signal::SmartHandle Subscribe(const uint32_t type,
                                SubscriberFunction<DataType> function) {
    return this->_eventHandlerMap[type].ScopedSubscribe(function);
  }

  void SubscribeForever(const uint32_t type,
                        SubscriberFunction<DataType> function) {
...
/**
 *      Specialization takes in another param: mailbox id. Mailbox allows us to
 * listen for events coming from specific device, or going to specific device.
 * For example, if you want to subscribe to "battery_status" message coming only
 * from device "3".
 *
 */
...
template <typename DataType>
using MailboxSignalMap =
    std::unordered_map<uint32_t, EventHandlerSignal<DataType> >;

// Here's the partially specialized type's declaration/definition
template <typename DataType>
class AnkiEventMgr<DataType, MailboxSignalMap<DataType> >
    : public AnkiEventMgrBase<DataType, MailboxSignalMap<DataType> > {
 public:
  ...
  // Allows subscribing to events by type with the passed in function
  Signal::SmartHandle Subscribe(const uint32_t mailbox, const uint32_t type,
                                SubscriberFunction<DataType> function) {
    return this->_eventHandlerMap[type][mailbox].ScopedSubscribe(function);
  }

  void SubscribeForever(const uint32_t mailbox, const uint32_t type,
                        SubscriberFunction<DataType> function) {
