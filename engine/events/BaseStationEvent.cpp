...
void IBaseStationEventListener::RegisterForAllEvents(void) {
  BaseStationEventDispatcher::Instance().RegisterEventListener(BSETYPE_ALL,
                                                               this);
}

//------------------------------------------------------------------------------------------------------------------------------
void IBaseStationEventListener::UnregisterForAllEvents(void) {
  BaseStationEventDispatcher::Instance().UnregisterEventListener(BSETYPE_ALL,
                                                                 this);
}
...
//------------------------------------------------------------------------------------------------------------------------------
void BaseStationEventDispatcher::RegisterEventListener(
    BaseStationEventType type, IBaseStationEventListener* observer) {
  assert((type >= 0) && (type <= BSETYPE_ALL));
  if (type != BSETYPE_ALL) {
    ObserverList& list = observers_[type];
    ObserverList::iterator match =
        std::find(list.begin(), list.end(), observer);
    if (match == list.end()) {
...
//------------------------------------------------------------------------------------------------------------------------------
void BaseStationEventDispatcher::UnregisterEventListener(
    BaseStationEventType type, IBaseStationEventListener* observer) {
  assert((type >= 0) && (type <= BSETYPE_ALL));
  if (type != BSETYPE_ALL) {
    ObserverList& list = observers_[type];
...
//------------------------------------------------------------------------------------------------------------------------------
void BaseStationEventDispatcher::EventRaised(
    IBaseStationEventInterface* event, BaseStationEventDelivery delivery) {
  NotifyEventListeners(event);
  delete event;
}

//------------------------------------------------------------------------------------------------------------------------------
void BaseStationEventDispatcher::NotifyEventListeners(
    IBaseStationEventInterface* event) {
