...
%                 filt = fspecial('gaussian',[21,21],5);
    ...
    if runStereoBmComputation && length(images) == 2
        tic
        disparity = stereo.compute(images{1}, images{2});
        toc
        ...
        disparityToShowBoth = [images{1}, disparityToShow];
        colorDisparityToShowBoth = [repmat(images{1},[1,1,3]), colorDisparityToShow];
