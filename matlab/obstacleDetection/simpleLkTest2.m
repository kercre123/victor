...
function simpleLkTest2(varargin)
    ...
    while true
        ...
        if runStereoBmComputation && length(images) == 2
            ...
            if imageScale == 2
                leftIm = uint8((double(images{1}(1:2:end,1:2:end)) + double(images{1}(2:2:end,1:2:end)) + double(images{1}(1:2:end,2:2:end)) + double(images{1}(2:2:end,2:2:end))) / 4);
                rightIm = uint8((double(images{2}(1:2:end,1:2:end)) + double(images{2}(2:2:end,1:2:end)) + double(images{2}(1:2:end,2:2:end)) + double(images{2}(2:2:end,2:2:end))) / 4);
                stereoColormap = stereoDisparityColors32;
            elseif imageScale < 1
                keyboard
            else
                leftIm = imresize(images{1}, imageSize);
                rightIm = imresize(images{2}, imageSize);
            ...
            if interpolateMissing
                ...
                for y = 1:size(disparity,1)
                    %                     firstX = find(disparity(y,:) > 0, 1, 'first');
                    %                     lastX = find(disparity(y,:) > 0, 1, 'last');
                    %                     disparity(y, 1:(firstX-1)) = disparity(y,firstX);
                    %                     disparity(y, (lastX+1):end) = disparity(y,lastX);
                    %                     for x = (firstX+1):(lastX-1)
                    %                     end % for x = (firstX+1):(lastX-1)
