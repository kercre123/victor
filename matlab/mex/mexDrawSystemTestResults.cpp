...
void DrawOneMarker(cv::Mat &drawnImage, const Quadrilateral<f32> &corners,
                   const char *name, const f64 showImageDetectionsScale,
                   const cv::Scalar &quadColor, const cv::Scalar &topBarColor);

void DrawOneQuad(cv::Mat &drawnImage, const Quadrilateral<f32> &quad,
                 const cv::Scalar &color, const s32 lineWidth,
                 const f64 showImageDetectionsScale);

void DrawQuads(cv::Mat &drawnImage,
               const FixedLengthList<Quadrilateral<f32> > &detectedQuads,
               const FixedLengthList<s32> &detectedQuads_types,
               const f64 showImageDetectionsScale);

void DrawMarkers(cv::Mat &drawnImage,
...
void DrawOneMarker(cv::Mat &drawnImage, const Quadrilateral<f32> &corners,
  ...
  Point<f32> minCorners[2] = {Point<f32>(FLT_MAX, FLT_MAX),
                              Point<f32>(FLT_MAX, FLT_MAX)};
  ...
  cv::line(
      drawnImage,
      cv::Point(saturate_cast<s32>(showImageDetectionsScale * corners[0].x),
                saturate_cast<s32>(showImageDetectionsScale * corners[0].y)),
      cv::Point(saturate_cast<s32>(showImageDetectionsScale * corners[2].x),
                saturate_cast<s32>(showImageDetectionsScale * corners[2].y)),
      topBarColor, 3);
  ...
  cv::putText(drawnImage, name,
              cv::Point(saturate_cast<s32>(midX * showImageDetectionsScale + 5),
                        saturate_cast<s32>(midY * showImageDetectionsScale)),
              cv::FONT_HERSHEY_COMPLEX_SMALL, 0.25 * showImageDetectionsScale,
              quadColor, 1, 8);
}
...
void DrawOneQuad(cv::Mat &drawnImage, const Quadrilateral<f32> &quad,
  ...
  for (s32 i = 0; i < numLines; i++) {
    ...
    cv::line(drawnImage,
             cv::Point(
                 saturate_cast<s32>(showImageDetectionsScale * quad[index0].x),
                 saturate_cast<s32>(showImageDetectionsScale * quad[index0].y)),
             cv::Point(
                 saturate_cast<s32>(showImageDetectionsScale * quad[index1].x),
                 saturate_cast<s32>(showImageDetectionsScale * quad[index1].y)),
             color, lineWidth);
  }
}
...
void DrawQuads(cv::Mat &drawnImage,
  ...
  for (s32 iQuad = 0; iQuad < numQuads; iQuad++) {
    ...
    DrawOneQuad(drawnImage, curQuad, cv::Scalar(255, 255, 255), 3,
                showImageDetectionsScale);
    DrawOneQuad(drawnImage, curQuad, cv::Scalar(0, 0, 0), 1,
                showImageDetectionsScale);

    if (detectedQuads_types[iQuad] > 0) {
      // Find the leftmost corner, with a slight preference to choose the upper
      // one
      ...
      for (s32 i = 0; i < 4; i++) {
        if (curQuad.corners[i].x < (minPoint.x + 3) &&
            curQuad.corners[i].y < minPoint.y) {
          minPoint = curQuad.corners[i];
        }
      }
      ...
      if (detectedQuads_types[iQuad] < 100000) {
        ...
        cv::Point scaledMinPoint(
            saturate_cast<s32>(showImageDetectionsScale * minPoint.x),
            saturate_cast<s32>(showImageDetectionsScale * minPoint.y));

        CvPutTextWithBackground(
            drawnImage, validText, scaledMinPoint,
            cv::FONT_HERSHEY_COMPLEX_SMALL, 0.5 * showImageDetectionsScale,
            cv::Scalar(255, 255, 255), cv::Scalar(0, 0, 0), 1, 8, true);
      }  // if(detectedQuads_types[iQuad] < 100000)
    }    // if(detectedQuads_types[iQuad] > 0)
  }      // for(s32 iQuad=0; iQuad<numQuads; iQuad++)
}  // void DrawQuads(cv::Mat &drawnImage, const
   // FixedLengthList<Quadrilateral<f32> > &detectedQuads, const
   // FixedLengthList<s32> &detectedQuads_types)
...
void DrawMarkers(cv::Mat &drawnImage,
  ...
  for (s32 iMarker = 0; iMarker < numMarkers; iMarker++) {
    ...
    if (detectedMarkers_types[iMarker] == 1 ||
        detectedMarkers_types[iMarker] == 2) {
    ...
    DrawOneMarker(drawnImage, detectedMarkers[iMarker],
                  detectedMarkers_names[iMarker], showImageDetectionsScale,
                  quadColor, topBarColor);
  }  // for(s32 iMarker=0; iMarker<numMarkers; iMarker++)
}

void DrawTextResults(cv::Mat &drawnImage, const ToShowResults &toShowResults,
                     const f64 showImageDetectionsScale) {
  ...
  snprintf(resultsText, 1024, "Test %d %d %s", toShowResults.iTest,
           toShowResults.iPose, toShowResults.testFunctionName);

  cv::putText(drawnImage, resultsText, cv::Point(0, curY),
              cv::FONT_HERSHEY_COMPLEX_SMALL, textFontSize,
              cv::Scalar(255, 255, 255), 1, 8);

  curY += textHeightInPixels;

  snprintf(resultsText, 1024, "Dist:%dmm angle:%d expos:%0.1f light:%d",
           saturate_cast<s32>(toShowResults.scene_Distance),
           saturate_cast<s32>(toShowResults.scene_angle),
           toShowResults.scene_CameraExposure,
           saturate_cast<s32>(toShowResults.scene_light));
  ...
  snprintf(resultsText, 1024, "markers:%d/%d/%d/%d/%d",
           toShowResults.numCorrect_positionLabelRotation,
           toShowResults.numCorrect_positionLabel,
           toShowResults.numCorrect_position, toShowResults.numQuadsDetected,
           toShowResults.numQuadsNotIgnored);
  ...
  snprintf(resultsText, 1024, "spurrious:%d",
           toShowResults.numSpurriousDetections);
...
cv::Mat DrawSystemTestResults(
    ...
    const FixedLengthList<s32> &detectedMarkers_types,
    const FixedLengthList<char *> &detectedMarkers_names,
    const f64 showImageDetectionsScale, const ToShowResults &toShowResults) {
  ...
  cv::Mat drawnImage(saturate_cast<s32>(showImageDetectionsScale *
  ...
  DrawQuads(drawnImage, detectedQuads, detectedQuads_types,
            showImageDetectionsScale);

  DrawMarkers(drawnImage, detectedMarkers, detectedMarkers_types,
              detectedMarkers_names, showImageDetectionsScale);
...
ToShowResults ParseToShowResults(const mxArray *in) {
  ...
  results.numCorrect_positionLabelRotation =
      saturate_cast<s32>(mxGetScalar(GetCell(in, 7)));
  results.numCorrect_positionLabel =
      saturate_cast<s32>(mxGetScalar(GetCell(in, 8)));
  ...
  results.numSpurriousDetections =
      saturate_cast<s32>(mxGetScalar(GetCell(in, 12)));
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  AnkiConditionalErrorAndReturn(
      nrhs == 9 && nlhs >= 0 && nlhs <= 1, "mexDrawSystemTestResults",
      "Call this function as follows:"
      "<drawnImage> = mexDrawSystemTestResults(uint8(detectedQuads), "
      "detectedQuads_types, detectedMarkers, detectedMarkers_types, "
      "detectedMarkers_names, showImageDetectionsScale, outputFilenameImage, "
      "toShowResults);");

  MemoryStack memory(mxMalloc(bufferSize), bufferSize);
  AnkiConditionalErrorAndReturn(memory.IsValid(), "mexDrawSystemTestResults",
                                "Memory could not be allocated");

  Array<u8> inputImage = mxArrayToArray<u8>(prhs[0], memory);
  Array<Array<f64> > detectedQuadsRaw =
      mxCellArrayToArray<f64>(prhs[1], memory);
  Array<s32> detectedQuads_typesRaw = mxArrayToArray<s32>(prhs[2], memory);
  Array<Array<f64> > detectedMarkersRaw =
      mxCellArrayToArray<f64>(prhs[3], memory);
  Array<s32> detectedMarkers_typesRaw = mxArrayToArray<s32>(prhs[4], memory);
  Array<char *> detectedMarkers_namesRaw =
      mxCellArrayToStringArray(prhs[5], memory);
  ...
  FixedLengthList<Quadrilateral<f32> > detectedQuads =
      FixedLengthList<Quadrilateral<f32> >(numQuads, memory,
                                           Flags::Buffer(true, false, true));
  FixedLengthList<s32> detectedQuads_types =
      FixedLengthList<s32>(numQuads, memory, Flags::Buffer(true, false, true));
  FixedLengthList<Quadrilateral<f32> > detectedMarkers =
      FixedLengthList<Quadrilateral<f32> >(numMarkers, memory,
                                           Flags::Buffer(true, false, true));
  FixedLengthList<s32> detectedMarkers_types = FixedLengthList<s32>(
      numMarkers, memory, Flags::Buffer(true, false, true));
  FixedLengthList<char *> detectedMarkers_names(
      numMarkers, memory, Flags::Buffer(true, false, true));
  ...
  for (s32 iQuad = 0; iQuad < numQuads; iQuad++) {
    ...
    for (s32 iCorner = 0; iCorner < 4; iCorner++) {
      detectedQuads[iQuad].corners[iCorner].x =
          static_cast<f32>(curQuadRaw[iCorner][0] - 1);
      detectedQuads[iQuad].corners[iCorner].y =
          static_cast<f32>(curQuadRaw[iCorner][1] - 1);
    }
  ...
  for (s32 iMarker = 0; iMarker < numMarkers; iMarker++) {
    ...
    for (s32 iCorner = 0; iCorner < 4; iCorner++) {
      detectedMarkers[iMarker].corners[iCorner].x =
          static_cast<f32>(curMarkerRaw[iCorner][0] - 1);
      detectedMarkers[iMarker].corners[iCorner].y =
          static_cast<f32>(curMarkerRaw[iCorner][1] - 1);
    }
  ...
  cv::Mat drawnImage = DrawSystemTestResults(
      inputImage, detectedQuads, detectedQuads_types, detectedMarkers,
      detectedMarkers_types, detectedMarkers_names, showImageDetectionsScale,
      toShowResults);
  ...
  if (nlhs > 0) {
    plhs[0] =
        Anki::imageArrayToMxArray<u8>(drawnImage.data, drawnImage.rows,
                                      drawnImage.cols, drawnImage.channels());
  }
