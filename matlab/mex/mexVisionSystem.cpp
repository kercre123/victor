...
Result Update(const mxArray* mxImg) {
  ...
  // NOTE: Just putting in bogus focal length parameters here since all we
  // really
  //  care about for detection is the image size
  Vision::CameraCalibration camCalib(img.GetNumRows(), img.GetNumCols(), 1.f,
                                     1.f, img.GetNumCols() / 2,
                                     img.GetNumRows() / 2);
