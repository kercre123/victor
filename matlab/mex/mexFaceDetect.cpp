...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  const std::string faceFilename = QUOTE(OPENCV_ROOT_PATH) "/data/lbpcascades/lbpcascade_frontalface.xml";
  const std::string eyeFilename =QUOTE(OPENCV_ROOT_PATH) "/data/haarcascades/haarcascade_eye_tree_eyeglasses.xml";

  if(face_cascade == NULL) {
    face_cascade = new cv::CascadeClassifier();

    if(! face_cascade->load(faceFilename))
    {
  ...
  if(eyes_cascade == NULL) {
    ...
    if(! eyes_cascade->load(eyeFilename))
    {
