...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (!(nlhs == 3 && nrhs >= 1 && nrhs <= 6)) {
    mexErrMsgTxt(
        "Usage: [faces,eyes,hands] = mexFaceDetect(im, <faceCascadeFilename>, "
        "<eyeCascadeFilename>, <handCascadeFilename>, <scaleFactor>, "
        "<minNeighbors>);\nTo use the default cascade for face or eyes, use "
        "filename ''. To not use the eye cascade, use filename 'none'.\n");
    return;
  }

  std::string newFaceCascadeFilename =
      QUOTE(OPENCV_ROOT_PATH) "/data/lbpcascades/lbpcascade_frontalface.xml";
  std::string newEyeCascadeFilename = QUOTE(
      OPENCV_ROOT_PATH) "/data/haarcascades/"
                        "haarcascade_eye_tree_eyeglasses.xml";
  std::string newHandCascadeFilename =
      QUOTE(OPENCV_ROOT_PATH) "/data/haarcascades/hands/hand.xml";
  ...
  if (nrhs >= 5) {
    const double tmpScaleFactor =
        Anki::Embedded::saturate_cast<f64>(mxGetScalar(prhs[4]));

    if (tmpScaleFactor >= 1.0) scaleFactor = tmpScaleFactor;
  }

  if (nrhs >= 6) {
    const int tmpMinNeighbors =
        Anki::Embedded::saturate_cast<s32>(mxGetScalar(prhs[5]));

    if (tmpMinNeighbors >= 1.0) minNeighbors = tmpMinNeighbors;
  }
  ...
  face_cascade->detectMultiScale(
      grayscaleFrame, faces, scaleFactor, minNeighbors,
  ...
  plhs[1] = mxCreateDoubleMatrix(
      storedEyes.size(), 5, mxREAL);  // [faceIndex, left, top, width, height]
