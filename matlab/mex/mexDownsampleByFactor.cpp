...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  ConditionalErrorAndReturn(img.get_rawDataPointer() != 0, "mexDownsampleByFactor", "Could not allocate Matrix img");
        
  Matrix<u8> imgDownsampled = AllocateMatrixFromHeap<u8>(img.get_size(0)/downsampleFactor, img.get_size(1)/downsampleFactor);
  ConditionalErrorAndReturn(imgDownsampled.get_rawDataPointer() != 0, "mexDownsampleByFactor", "Could not allocate Matrix imgDownsampled");
