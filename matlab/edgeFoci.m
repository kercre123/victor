function [h,m] = edgeFoci(img, sigma)
...
sigma_adj = max(0, sqrt( (alpha*sigma)^2 - sigma_0^2 )); % for blurring
sigma_u = sqrt((beta*sigma)^2 - (alpha*sigma)^2);
...
% Normalized edges
fbar = separable_filter(f, gaussian_kernel(alpha*sigma*sqrt(lambda^2-1)));
fnorm = f ./ max(fbar, 10/sigma);

% Separable filters
[g,dg,dg2] = gaussian_kernel(sigma_u);
...
for i = 1:numTheta
  ...
  xoffset = sigma*cos(theta_i);
  yoffset = sigma*sin(theta_i);
...
if DebugDisplay
  figure(2)
  subplot 221, hold off, imagesc(img), axis image, hold on
  plot(mx, my, 'r.', 'MarkerSize', 10);
  subplot 222, hold off, imagesc(f), axis image, hold on
  mask = f > .05;
  quiver(xgrid(mask), ygrid(mask), cos(theta(mask)), sin(theta(mask)));
  subplot 223, imagesc(fnorm), axis image
