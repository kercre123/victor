...
/* This is for Visual Studio, so this header works as distributed in the binary
 * downloads */
 ...
 * you will only get audio in one of these samplings. \param list Store a
 * pointer to the sample rates array there. \param number Store the number of
 * sample rates there. */
 ...
 * mpg123_open_feed(). \param mh handle \param path filesystem path \return
 ...
 * and feeding the input bitstream. \param mh handle \return MPG123_OK on
 * success
 */
 ...
 */
MPG123_EXPORT int mpg123_read(mpg123_handle *mh, unsigned char *outmemory,
 ...
 * steps or one call this one here. \param mh handle \param inmemory input
 * buffer \param inmemsize number of input bytes \param outmemory output buffer
...
MPG123_EXPORT int mpg123_decode(mpg123_handle *mh,
                                ...
                                unsigned char *outmemory, size_t outmemsize,
                                size_t *done);
 ...
 * the specific functions. If gapless code is in effect, the positions are
 * adjusted to compensate the skipped padding/delay - meaning, you should not
 * care about that at all and just use the position defined for the samples you
 * get out of the decoder;-) The general usage is modelled after stdlib's
 * ftell() and fseek(). Especially, the whence parameter for the seek functions
 * has the same meaning as the one for fseek() and needs the same constants from
 * stdlib.h:
 ...
 * samples, too (or the samples played twice only once)! Short: When you care
 * about the sample position, don't mess with those parameters;-) Also, seeking
 * is not guaranteed to work for all streams (underlying stream may not support
 * it). And yet another caveat: If the stream is concatenated out of differing
 * pieces (Frankenstein stream), seeking may suffer, too.
 *
 * @{
 */
 ...
 * of input data will be from input_offset byte position. \param mh handle
 *  \param sampleoff offset in PCM samples
 *  \param whence one of SEEK_SET, SEEK_CUR or SEEK_END
 ...
 * for both. \param band The equaliser band to change (from 0 to 31) \param val
 * The (linear) adjustment factor. \return MPG123_OK on success
 */
 ...
 *  \param mh handle
 *  \param channel Can be MPG123_LEFT, MPG123_RIGHT or MPG123_LEFT|MPG123_RIGHT
 * for (arithmetic mean of) both. \param band The equaliser band to change (from
 * 0 to 31) \return The (linear) adjustment factor (zero for pad parameters) */
 ...
 * mpg123_frameinfo structure. \param mh handle \param mi address of existing
 * frameinfo structure to write to \return MPG123_OK on success
 */
 ...
 *  Even with the fill property, the character count is not obvious as there
 * could be multiple trailing null bytes. \param sb string handle \param utf8 a
 * flag to tell if the string is in utf8 encoding \return character count
 */
...
enum mpg123_text_encoding {
  ...
  mpg123_text_utf16 =
      5 /**< Some UTF-16 encoding. The last of a set of leading BOMs (byte order
         ...
         * code points. If you want to decode little endian data
...
MPG123_EXPORT enum mpg123_text_encoding mpg123_enc_from_id3(
 ...
 * \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_replace_buffer(mpg123_handle *mh, unsigned char *data,
                                        size_t size);
...
MPG123_EXPORT int mpg123_replace_reader_handle(
    mpg123_handle *mh, ssize_t (*r_read)(void *, void *, size_t),
