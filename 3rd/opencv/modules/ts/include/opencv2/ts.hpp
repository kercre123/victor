...
#define PARAM_TEST_CASE(name, ...) \
  struct name : testing::TestWithParam<testing::tuple<__VA_ARGS__> >
...
class SkipTestException : public cv::Exception {
 public:
  ...
  SkipTestException(const cv::String& message) : dummy(0) {
    this->msg = message;
  }
};
...
inline double randReal(RNG& rng) { return (double)rng; }
...
void add(const Mat& a, double alpha, const Mat& b, double beta, Scalar gamma,
         Mat& c, int ctype, bool calcAbs = false);
void multiply(const Mat& a, const Mat& b, Mat& c, double alpha = 1);
void divide(const Mat& a, const Mat& b, Mat& c, double alpha = 1);

void convert(const Mat& src, cv::OutputArray dst, int dtype, double alpha = 1,
             double beta = 0);
void copy(const Mat& src, Mat& dst, const Mat& mask = Mat(),
...
void erode(const Mat& src, Mat& dst, const Mat& _kernel,
           Point anchor = Point(-1, -1), int borderType = 0,
           const Scalar& borderValue = Scalar());
void dilate(const Mat& src, Mat& dst, const Mat& _kernel,
            Point anchor = Point(-1, -1), int borderType = 0,
            const Scalar& borderValue = Scalar());
...
void initUndistortMap(const Mat& a, const Mat& k, Size sz, Mat& mapx,
                      Mat& mapy);

void minMaxLoc(const Mat& src, double* minval, double* maxval,
               vector<int>* minloc, vector<int>* maxloc,
               const Mat& mask = Mat());
...
bool cmpUlps(const Mat& data, const Mat& refdata, int expMaxDiff,
             double* realMaxDiff, vector<int>* idx);
...
// a wrapper for the previous function. in case of error prints the message to
// log file.
int cmpEps2(TS* ts, const Mat& data, const Mat& refdata,
            double success_err_level, bool element_wise_relative_error,
            const char* desc);
...
class BaseTest {
 public:
  ...
  // returns true if and only if the different test cases do not depend on each
  // other (so that test system could get right to a problematic test case)
 ...
 protected:
  ...
  virtual int update_progress(int progress, int test_case_idx, int count,
                              double dt);
...
class TS {
 public:
  ...
  // low-level printing functions that are used by individual tests and by the
  // system itself
  ...
  virtual void update_context(BaseTest* test, int test_case_idx,
  ...
  enum FailureCode {
    ...
    // the tested function returned invalid object, e.g. matrix, containing
    // NaNs, structure with NULL or out-of-range fields (while it should not)
    ...
    // the tested function returned valid object (a single number or numerical
    // array), but it differs too much from the original (or produced by the
    // test) object
    FAIL_BAD_ACCURACY = -12,

    // the tested function hung. Sometimes, it can be determined by unexpectedly
    // long processing time (in this case there should be possibility to
    // interrupt such a function
    FAIL_HANG = -13,

    // unexpected response on passing bad arguments to the tested function
    // (the function crashed, proceed successfully (while it should not), or
    // returned error code that is different from what is expected)
    FAIL_BAD_ARG_CHECK = -14,

    // the test data (in whole or for the particular test case) is invalid
    FAIL_INVALID_TEST_DATA = -15,

    // the test has been skipped because it is not in the selected subset of the
    // tests to run, because it has been run already within the same run with
    // the same parameters, or because of some other reason and this is not
    // considered as an error. Normally TS::run() (or overridden method in the
    // derived class) takes care of what needs to be run, so this code should
    // not occur.
    SKIPPED = 1
  };
  ...
  TS::FailureCode get_err_code() {
    return TS::FailureCode(current_test_info.code);
  }
 ...
 protected:
  // these are allocated within a test to try to keep them valid in case of
  // stack corruption
  RNG rng;
...
/*****************************************************************************************\
*            Subclass of BaseTest for testing functions that process dense
arrays           *
\*****************************************************************************************/
...
class ArrayTest : public BaseTest {
 ...
 protected:
  ...
  virtual void get_test_array_types_and_sizes(int test_case_idx,
                                              vector<vector<Size> >& sizes,
                                              vector<vector<int> >& types);
  virtual void fill_array(int test_case_idx, int i, int j, Mat& arr);
  virtual void get_minmax_bounds(int i, int j, int type, Scalar& low,
                                 Scalar& high);
  ...
  int run_test_case(int expected_code, const string& _descr, F f) {
    ...
    try {
      ...
      if (e.code != expected_code) {
        ts->printf(TS::LOG,
                   "%s (test case #%d): the error code %d is different from "
                   "the expected %d\n",
...
/*! @brief Try to find requested data file
  ...
  2. One of these:
     ...
     b. OpenCV testdata at install location: CMAKE_INSTALL_PREFIX +
 ...
 */
std::string findDataFile(const std::string& relative_path,
                         bool required = true);
...
#define CV_TEST_MAIN(resourcesubdir, ...) \
  CV_TEST_MAIN_EX(resourcesubdir, NOOP, __VA_ARGS__)
...
class ifstream : public stringstream {
 ...
 public:
  ifstream(const char* filename, ios_base::openmode mode = ios_base::in)
    ...
    if (mode & ios_base::binary) modeStr += "b";
...
class ofstream : public stringstream {
 ...
 public:
  ...
  void open(const char* filename, ios_base::openmode mode = ios_base::out) {
    string modeStr("w+");
    if (mode & ios_base::trunc) modeStr = "w";
    if (mode & ios_base::binary) modeStr += "b";
    f = fopen(filename, modeStr.c_str());
  ...
  void close() {
    if (f) {
      fwrite(reinterpret_cast<const char*>(this->str().c_str()),
             this->str().size(), 1, f);
      fclose(f);
    }
    f = NULL;
