...
#define LOGD(...)                                                   \
  ((void)__android_log_print(ANDROID_LOG_DEBUG, PERF_TESTS_LOG_TAG, \
                             __VA_ARGS__))
#define LOGI(...) \
  ((void)__android_log_print(ANDROID_LOG_INFO, PERF_TESTS_LOG_TAG, __VA_ARGS__))
#define LOGW(...) \
  ((void)__android_log_print(ANDROID_LOG_WARN, PERF_TESTS_LOG_TAG, __VA_ARGS__))
#define LOGE(...)                                                   \
  ((void)__android_log_print(ANDROID_LOG_ERROR, PERF_TESTS_LOG_TAG, \
                             __VA_ARGS__))
#else
#define LOGD(_str, ...)          \
  do {                           \
...
#define SZ_ALL_VGA \
  ::testing::Values(::perf::szQVGA, ::perf::szVGA, ::perf::szSVGA)
#define SZ_ALL_GA                                                  \
  ::testing::Values(::perf::szQVGA, ::perf::szVGA, ::perf::szSVGA, \
                    ::perf::szXGA, ::perf::szSXGA)
#define SZ_ALL_HD                                                 \
  ::testing::Values(::perf::sznHD, ::perf::szqHD, ::perf::sz720p, \
                    ::perf::sz1080p)
#define SZ_ALL_SMALL                                                         \
...
#define SZ_ALL                                                     \
  ::testing::Values(::perf::szQVGA, ::perf::szVGA, ::perf::szSVGA, \
                    ::perf::szXGA, ::perf::szSXGA, ::perf::sznHD,  \
                    ::perf::szqHD, ::perf::sz720p, ::perf::sz1080p)
#define SZ_TYPICAL \
  ::testing::Values(::perf::szVGA, ::perf::szqHD, ::perf::sz720p, ::perf::szODD)

#define TYPICAL_MAT_SIZES \
  ::perf::szVGA, ::perf::sz720p, ::perf::sz1080p, ::perf::szODD
#define TYPICAL_MAT_TYPES CV_8UC1, CV_8UC4, CV_32FC1
#define TYPICAL_MATS                                   \
  testing::Combine(testing::Values(TYPICAL_MAT_SIZES), \
                   testing::Values(TYPICAL_MAT_TYPES))
#define TYPICAL_MATS_C1                                \
  testing::Combine(testing::Values(TYPICAL_MAT_SIZES), \
                   testing::Values(CV_8UC1, CV_32FC1))
#define TYPICAL_MATS_C4 \
...
/*****************************************************************************************\
*     CV_ENUM and CV_FLAGS - macro to create printable wrappers for defines and
enums     *
\*****************************************************************************************/
...
#define CV_ENUM(class_name, ...)                                               \
  ...
  static inline void PrintTo(const class_name& t, std::ostream* os) {          \
    t.PrintTo(os);                                                             \
  }                                                                            \
  }
...
CV_ENUM(MatDepth, CV_8U, CV_8S, CV_16U, CV_16S, CV_32S, CV_32F, CV_64F,
        CV_USRTYPE1)

/*****************************************************************************************\
*                 Regression control utility for performance testing *
\*****************************************************************************************/
enum ERROR_TYPE { ERROR_ABSOLUTE = 0, ERROR_RELATIVE = 1 };

class Regression {
 public:
  static Regression& add(TestBase* test, const std::string& name,
                         cv::InputArray array, double eps = DBL_EPSILON,
                         ERROR_TYPE err = ERROR_ABSOLUTE);
  static Regression& addMoments(TestBase* test, const std::string& name,
                                const cv::Moments& array,
                                double eps = DBL_EPSILON,
                                ERROR_TYPE err = ERROR_ABSOLUTE);
  static Regression& addKeypoints(TestBase* test, const std::string& name,
                                  const std::vector<cv::KeyPoint>& array,
                                  double eps = DBL_EPSILON,
                                  ERROR_TYPE err = ERROR_ABSOLUTE);
  static Regression& addMatches(TestBase* test, const std::string& name,
                                const std::vector<cv::DMatch>& array,
                                double eps = DBL_EPSILON,
                                ERROR_TYPE err = ERROR_ABSOLUTE);
  static void Init(const std::string& testSuitName,
                   const std::string& ext = ".xml");

  Regression& operator()(const std::string& name, cv::InputArray array,
                         double eps = DBL_EPSILON,
                         ERROR_TYPE err = ERROR_ABSOLUTE);
 ...
 private:
  ...
  cv::RNG regRNG;  // own random numbers generator to make collection and
                   // verification work identical
  ...
  void verify(cv::FileNode node, cv::InputArray array, double eps,
              ERROR_TYPE err);
  void verify(cv::FileNode node, cv::Mat actual, double eps,
              std::string argname, ERROR_TYPE err);
};

#define SANITY_CHECK(array, ...) \
  ::perf::Regression::add(this, #array, array, ##__VA_ARGS__)
#define SANITY_CHECK_MOMENTS(array, ...) \
  ::perf::Regression::addMoments(this, #array, array, ##__VA_ARGS__)
#define SANITY_CHECK_KEYPOINTS(array, ...) \
  ::perf::Regression::addKeypoints(this, #array, array, ##__VA_ARGS__)
#define SANITY_CHECK_MATCHES(array, ...) \
  ::perf::Regression::addMatches(this, #array, array, ##__VA_ARGS__)
...
typedef struct performance_metrics {
  ...
  enum {
    ...
    TERM_EXCEPTION = 3,
    TERM_SKIP_TEST =
        4,  // there are some limitations and test should be skipped
    TERM_UNKNOWN = -1
  };
...
class TestBase : public ::testing::Test {
 public:
  ...
  static void Init(const std::vector<std::string>& availableImpls, int argc,
  ...
  static enum PERF_STRATEGY setModulePerformanceStrategy(
      enum PERF_STRATEGY strategy);

  class PerfSkipTestException : public cv::Exception {
   public:
    int dummy;  // workaround for MacOSX Xcode 7.3 bug (don't make class
                // "empty")
 ...
 protected:
  ...
  enum WarmUpType { WARMUP_READ, WARMUP_WRITE, WARMUP_RNG, WARMUP_NONE };
 ...
 private:
  ...
  static void declareArray(SizeVector& sizes, cv::InputOutputArray a,
                           WarmUpType wtype);

  class _declareHelper {
   public:
    _declareHelper& in(cv::InputOutputArray a1, WarmUpType wtype = WARMUP_READ);
    _declareHelper& in(cv::InputOutputArray a1, cv::InputOutputArray a2,
                       WarmUpType wtype = WARMUP_READ);
    _declareHelper& in(cv::InputOutputArray a1, cv::InputOutputArray a2,
                       cv::InputOutputArray a3, WarmUpType wtype = WARMUP_READ);
    _declareHelper& in(cv::InputOutputArray a1, cv::InputOutputArray a2,
                       cv::InputOutputArray a3, cv::InputOutputArray a4,
                       WarmUpType wtype = WARMUP_READ);

    _declareHelper& out(cv::InputOutputArray a1,
                        WarmUpType wtype = WARMUP_WRITE);
    _declareHelper& out(cv::InputOutputArray a1, cv::InputOutputArray a2,
                        WarmUpType wtype = WARMUP_WRITE);
    _declareHelper& out(cv::InputOutputArray a1, cv::InputOutputArray a2,
...
template <typename T>
class TestBaseWithParam : public TestBase,
...
#define PERF_TEST(test_case_name, test_name)                                  \
  namespace PERF_PROXY_NAMESPACE_NAME_(test_case_name, test_name) {           \
    class TestBase { /*compile error for this class means that you are trying \
                        to use perf::TestBase as a fixture*/                  \
    };                                                                        \
    ...
    TEST_F(test_case_name, test_name) {                                       \
      CV__PERF_TEST_BODY_IMPL(#test_case_name "_" #test_name);                \
    }                                                                         \
  }                                                                           \
  void PERF_PROXY_NAMESPACE_NAME_(test_case_name,                             \
                                  test_name)::test_case_name::PerfTestBody()
...
#define PERF_TEST_F(fixture, testname)                                        \
  namespace PERF_PROXY_NAMESPACE_NAME_(fixture, testname) {                   \
    ...
    TEST_F(fixture, testname) {                                               \
      CV__PERF_TEST_BODY_IMPL(#fixture "_" #testname);                        \
    }                                                                         \
  }                                                                           \
...
//
// @Note PERF_TEST_P() below violates behavior of original Google Tests - there
// is no tests instantiation in original TEST_P() This macro is intended for
// usage with separate INSTANTIATE_TEST_CASE_P macro
#define PERF_TEST_P_(test_case_name, test_name) \
  CV__TEST_P(test_case_name, test_name, PerfTestBody, CV__PERF_TEST_BODY_IMPL)

// Defines a parametrized performance test.
//
// @Note Original TEST_P() macro doesn't instantiate tests with parameters. To
// keep original usage use PERF_TEST_P_() macro
//
// The first parameter is the name of the test fixture class, which
// also doubles as the test case name.  The second parameter is the
// name of the test within the test case.
...
//
//   PERF_TEST_P(FooTest, DoTestingRight, ::testing::Values(::perf::szVGA,
//   ::perf::sz720p) {
...
#define PERF_TEST_P(fixture, name, params)                     \
  ...
  CV__TEST_P(fixture##_##name, name, PerfTestBodyDummy,        \
             CV__PERF_TEST_BODY_IMPL) {}                       \
...
#else
#define __CV_TEST_EXEC_ARGS(...) __VA_ARGS__;
#endif
#endif
...
#define CV_PERF_TEST_MAIN_INTERNALS(modulename, impls, ...)                    \
  ...
  {                                                                            \
    ...
    ::perf::TestBase::Init(                                                    \
        std::vector<std::string>(impls, impls + sizeof impls / sizeof *impls), \
        argc, argv);                                                           \
...
// impls must be an array, not a pointer; "plain" should always be one of the
// implementations
...
//! deprecated
#define TEST_CYCLE_N(n) \
  for (declare.iterations(n); next() && startTimer(); stopTimer())
//! deprecated
#define TEST_CYCLE() for (; next() && startTimer(); stopTimer())
//! deprecated
#define TEST_CYCLE_MULTIRUN(runsNum)                               \
  for (declare.runs(runsNum); next() && startTimer(); stopTimer()) \
