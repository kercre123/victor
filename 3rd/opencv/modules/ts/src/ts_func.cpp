...
const char* getTypeName(int type) {
  static const char* type_names[] = {"8u",  "8s",  "16u", "16s",
                                     "32s", "32f", "64f", "ptr"};
...
int typeByName(const char* name) {
  ...
  for (i = 0; i < CV_DEPTH_MAX; i++)
    if (strcmp(name, getTypeName(i)) == 0) return i;
  return -1;
}
...
Size randomSize(RNG& rng, double maxSizeLog) {
  ...
  if ((unsigned)rng % 2 != 0) std::swap(width_log, height_log);
  Size sz;
...
void randomSize(RNG& rng, int minDims, int maxDims, double maxSizeLog,
                vector<int>& sz) {
...
Mat randomMat(RNG& rng, Size size, int type, double minVal, double maxVal,
              bool useRoi) {
  Size size0 = size;
  if (useRoi) {
  ...
  if (size0 == size) return m;
  return m(Rect((size0.width - size.width) / 2,
                (size0.height - size.height) / 2, size.width, size.height));
}

Mat randomMat(RNG& rng, const vector<int>& size, int type, double minVal,
              double maxVal, bool useRoi) {
  ...
  for (i = 0; i < dims; i++) {
    ...
    if (useRoi) {
      ...
      r[i] =
          Range((size0[i] - size[i]) / 2, (size0[i] - size[i]) / 2 + size[i]);
    }
...
void add(const Mat& _a, double alpha, const Mat& _b, double beta, Scalar gamma,
         Mat& c, int ctype, bool calcAbs) {
  Mat a = _a, b = _b;
  if (a.empty() || alpha == 0) {
    // both alpha and beta can be 0, but at least one of a and b must be
    // non-empty array, otherwise we do not know the size of the output (and may
    // be type of the output, when ctype<0)
  ...
  int cn = a.channels();
  int total = (int)planes[0].total(),
      maxsize = std::min(12 * 12 * std::max(12 / cn, 1), total);

  CV_Assert(planes[0].rows == 1);
  buf[0].create(1, maxsize, CV_64FC(cn));
  if (!b.empty()) buf[1].create(1, maxsize, CV_64FC(cn));
  ...
  for (i = 0; i < nplanes; i++, ++it) {
    for (int j = 0; j < total; j += maxsize) {
      ...
      if (b.empty()) {
        ...
        for (k = 0; k < n; k++) aptr[k] += bptr[k] + gptr[k];
      }
      if (calcAbs)
        for (k = 0; k < n; k++) aptr[k] = fabs(aptr[k]);
...
inline void convert_(const _Tp1* src, _Tp2* dst, size_t total, double alpha,
                     double beta) {
  size_t i;
  if (alpha == 1 && beta == 0)
    for (i = 0; i < total; i++) dst[i] = saturate_cast<_Tp2>(src[i]);
  else if (beta == 0)
    for (i = 0; i < total; i++) dst[i] = saturate_cast<_Tp2>(src[i] * alpha);
  else
    for (i = 0; i < total; i++)
...
inline void convertTo(const _Tp* src, void* dst, int dtype, size_t total,
...
void convert(const Mat& src, cv::OutputArray _dst, int dtype, double alpha,
             double beta) {
...
void copy(const Mat& src, Mat& dst, const Mat& mask, bool invertMask) {
  ...
  NAryMatIterator it(arrays, planes);
  size_t j, k, elemSize = src.elemSize(), maskElemSize = mask.elemSize(),
               total = planes[0].total();
  size_t i, nplanes = it.nplanes;
  ...
  for (i = 0; i < nplanes; i++, ++it) {
    ...
    for (j = 0; j < total;
         j++, sptr += elemSize, dptr += elemSize, mptr += maskElemSize) {
      if (mcn == 1) {
        if ((mptr[0] != 0) ^ invertMask)
          for (k = 0; k < elemSize; k++) dptr[k] = sptr[k];
      } else {
        for (int c = 0; c < mcn; c++)
...
void set(Mat& dst, const Scalar& gamma, const Mat& mask) {
  ...
  if (mask.empty()) {
    ...
    size_t i, nplanes = it.nplanes;
    size_t j, k, elemSize = dst.elemSize(),
                 planeSize = plane.total() * elemSize;

    for (k = 1; k < elemSize; k++)
      if (gptr[k] != gptr[0]) break;
    ...
    for (i = 0; i < nplanes; i++, ++it) {
      ...
      if (uniform)
        ...
        for (j = 0; j < planeSize; j += elemSize, dptr += elemSize)
          for (k = 0; k < elemSize; k++) dptr[k] = gptr[k];
      } else
  ...
  NAryMatIterator it(arrays, planes);
  size_t j, k, elemSize = dst.elemSize(), maskElemSize = mask.elemSize(),
               total = planes[0].total();
  size_t i, nplanes = it.nplanes;
...
void insert(const Mat& src, Mat& dst, int coi) {
  CV_Assert(dst.size == src.size && src.depth() == dst.depth() && 0 <= coi &&
            coi < dst.channels());
  ...
  NAryMatIterator it(arrays, planes);
  size_t i, nplanes = it.nplanes;
  size_t j, k, size0 = src.elemSize(), size1 = dst.elemSize(),
               total = planes[0].total();
  ...
  for (i = 0; i < nplanes; i++, ++it) {
    ...
    for (j = 0; j < total; j++, sptr += size0, dptr += size1) {
      for (k = 0; k < size0; k++) dptr[k] = sptr[k];
    }
  }
}
...
void extract(const Mat& src, Mat& dst, int coi) {
  ...
  for (i = 0; i < nplanes; i++, ++it) {
    ...
    for (j = 0; j < total; j++, sptr += size0, dptr += size1) {
      for (k = 0; k < size1; k++) dptr[k] = sptr[k];
    }
  }
}
...
void transpose(const Mat& src, Mat& dst) {
  ...
  for (i = 0; i < dst.rows; i++) {
    ...
    for (j = 0; j < dst.cols; j++, sptr += src.step[0], dptr += esz) {
      for (k = 0; k < esz; k++) dptr[k] = sptr[k];
    }
  }
}

template <typename _Tp>
static void randUniInt_(RNG& rng, _Tp* data, size_t total, int cn,
                        const Scalar& scale, const Scalar& delta) {
...
template <typename _Tp>
static void randUniFlt_(RNG& rng, _Tp* data, size_t total, int cn,
                        const Scalar& scale, const Scalar& delta) {
  for (size_t i = 0; i < total; i += cn)
    for (int k = 0; k < cn; k++) {
...
template <typename _Tp>
static void erode_(const Mat& src, Mat& dst, const vector<int>& ofsvec) {
  ...
  for (int y = 0; y < dst.rows; y++) {
    ...
    for (int x = 0; x < width; x++) {
      ...
      for (int i = 1; i < n; i++) result = std::min(result, sptr[x + ofs[i]]);
      dptr[x] = result;
    }
  }
}
...
template <typename _Tp>
static void dilate_(const Mat& src, Mat& dst, const vector<int>& ofsvec) {
  int width = dst.cols * src.channels(), n = (int)ofsvec.size();
  const int* ofs = &ofsvec[0];
  ...
  for (int y = 0; y < dst.rows; y++) {
    ...
    for (int x = 0; x < width; x++) {
      _Tp result = sptr[x + ofs[0]];
      for (int i = 1; i < n; i++) result = std::max(result, sptr[x + ofs[i]]);
      dptr[x] = result;
    }
  }
}
...
void erode(const Mat& _src, Mat& dst, const Mat& _kernel, Point anchor,
  ...
  if (anchor == Point(-1, -1)) anchor = Point(kernel.cols / 2, kernel.rows / 2);
  if (borderType == BORDER_CONSTANT) borderValue = getMaxVal(src.depth());
  ...
  for (int i = 0; i < kernel.rows; i++)
    for (int j = 0; j < kernel.cols; j++)
      if (kernel.at<uchar>(i, j) != 0) ofs.push_back(i * step + j * cn);
  if (ofs.empty()) ofs.push_back(anchor.y * step + anchor.x * cn);
...
void dilate(const Mat& _src, Mat& dst, const Mat& _kernel, Point anchor,
  ...
  if (anchor == Point(-1, -1)) anchor = Point(kernel.cols / 2, kernel.rows / 2);
  if (borderType == BORDER_CONSTANT) borderValue = getMinVal(src.depth());
  copyMakeBorder(_src, src, anchor.y, kernel.rows - anchor.y - 1, anchor.x,
                 kernel.cols - anchor.x - 1, borderType, borderValue);
  dst.create(_src.size(), src.type());
...
template <typename _Tp>
static void filter2D_(const Mat& src, Mat& dst, const vector<int>& ofsvec,
                      const vector<double>& coeffvec) {
  const int* ofs = &ofsvec[0];
  ...
  for (int y = 0; y < dst.rows; y++) {
    ...
    for (int x = 0; x < width; x++) {
      double s = 0;
      for (int i = 0; i < ncoeffs; i++) s += sptr[x + ofs[i]] * coeff[i];
      dptr[x] = s;
    }
  }
}
...
void filter2D(const Mat& _src, Mat& dst, int ddepth, const Mat& kernel,
  ...
  int step = (int)(src.step / src.elemSize1()), cn = src.channels();
  for (int i = 0; i < kernel.rows; i++)
    for (int j = 0; j < kernel.cols; j++) ofs.push_back(i * step + j * cn);
...
void copyMakeBorder(const Mat& src, Mat& dst, int top, int bottom, int left,
                    int right, int borderType, const Scalar& borderValue) {
  ...
  if (borderType == BORDER_CONSTANT) {
    ...
    uchar* val = &valvec[0];
    scalarToRawData(borderValue, val, src.type(),
                    (src.cols + left + right) * src.channels());
    ...
    for (i = 0; i < src.rows; i++) {
      ...
      for (j = 0; j < left; j++) dptr[j - left] = val[j];
      if (dptr != sptr)
        for (j = 0; j < width; j++) dptr[j] = sptr[j];
      for (j = 0; j < right; j++) dptr[j + width] = val[j];
    }

    for (i = 0; i < top; i++) {
      uchar* dptr = dst.ptr(i);
      for (j = 0; j < width1; j++) dptr[j] = val[j];
    }
    ...
    for (i = 0; i < left; i++) {
      ...
      for (k = 0; k < esz; k++) ltab[i * esz + k] = j + k;
    }
    for (i = 0; i < right; i++) {
      j = borderInterpolate(src.cols + i, src.cols, borderType) * esz;
      for (k = 0; k < esz; k++) rtab[i * esz + k] = j + k;
    }
    ...
    for (i = 0; i < src.rows; i++) {
      ...
      for (j = 0; j < left; j++) dptr[j] = sptr[ltab[j]];
      if (dptr + left != sptr) {
        for (j = 0; j < width; j++) dptr[j + left] = sptr[j];
      }
      for (j = 0; j < right; j++) dptr[j + left + width] = sptr[rtab[j]];
    }
    ...
    for (i = 0; i < top; i++) {
      ...
      for (k = 0; k < width1; k++) dptr[k] = sptr[k];
    }
...
template <typename _Tp>
static void minMaxLoc_(const _Tp* src, size_t total, size_t startidx,
                       double* _minval, double* _maxval, size_t* _minpos,
                       size_t* _maxpos, const uchar* mask) {
  _Tp maxval = saturate_cast<_Tp>(*_maxval),
      minval = saturate_cast<_Tp>(*_minval);
...
static void setpos(const Mat& mtx, vector<int>& pos, size_t idx) {
  ...
  if (idx > 0) {
    ...
    for (int i = mtx.dims - 1; i >= 0; i--) pos[i] = -1;
  }
}

void minMaxLoc(const Mat& src, double* _minval, double* _maxval,
               vector<int>* _minloc, vector<int>* _maxloc, const Mat& mask) {
  ...
  for (i = 0; i < nplanes; i++, ++it, startidx += total) {
    ...
    switch (depth) {
      case CV_8U:
        minMaxLoc_((const uchar*)sptr, total, startidx, &minval, &maxval,
                   &minidx, &maxidx, mptr);
        break;
      case CV_8S:
        minMaxLoc_((const schar*)sptr, total, startidx, &minval, &maxval,
                   &minidx, &maxidx, mptr);
        break;
      case CV_16U:
        minMaxLoc_((const ushort*)sptr, total, startidx, &minval, &maxval,
                   &minidx, &maxidx, mptr);
        break;
      case CV_16S:
        minMaxLoc_((const short*)sptr, total, startidx, &minval, &maxval,
                   &minidx, &maxidx, mptr);
        break;
      case CV_32S:
        minMaxLoc_((const int*)sptr, total, startidx, &minval, &maxval, &minidx,
                   &maxidx, mptr);
        break;
      case CV_32F:
        minMaxLoc_((const float*)sptr, total, startidx, &minval, &maxval,
                   &minidx, &maxidx, mptr);
        break;
      case CV_64F:
        minMaxLoc_((const double*)sptr, total, startidx, &minval, &maxval,
                   &minidx, &maxidx, mptr);
        break;
      default:
        CV_Assert(0);
  ...
  if (_maxloc) setpos(src, *_maxloc, maxidx);
  if (_minloc) setpos(src, *_minloc, minidx);
}

static int normHamming(const uchar* src, size_t total, int cellSize) {
  int result = 0;
  ...
  for (size_t i = 0; i < total; i++) {
    unsigned a = src[i];
    for (; a != 0; a >>= cellSize) result += (a & mask) != 0;
  }
  return result;
}

template <typename _Tp>
static double norm_(const _Tp* src, size_t total, int cn, int normType,
                    double startval, const uchar* mask) {
  size_t i;
  ...
  if (normType == NORM_INF) {
    if (!mask)
      for (i = 0; i < total; i++)
        result = std::max(
            result, (double)std::abs(
                        0 + src[i]));  // trick with 0 used to quiet gcc warning
    else
      for (int c = 0; c < cn; c++) {
        for (i = 0; i < total; i++)
          if (mask[i])
    ...
    if (!mask)
      for (i = 0; i < total; i++) result += std::abs(0 + src[i]);
    else
      for (int c = 0; c < cn; c++) {
        for (i = 0; i < total; i++)
          if (mask[i]) result += std::abs(0 + src[i * cn + c]);
      }
  } else {
    if (!mask)
      for (i = 0; i < total; i++) {
...
template <typename _Tp>
static double norm_(const _Tp* src1, const _Tp* src2, size_t total, int cn,
                    int normType, double startval, const uchar* mask) {
  size_t i;
  double result = startval;
  if (!mask) total *= cn;
    ...
    if (!mask)
      ...
      for (int c = 0; c < cn; c++) {
        for (i = 0; i < total; i++)
          if (mask[i])
            result = std::max(
                result, (double)std::abs(src1[i * cn + c] - src2[i * cn + c]));
      }
  } else if (normType == NORM_L1) {
    if (!mask)
      for (i = 0; i < total; i++) result += std::abs(src1[i] - src2[i]);
    else
      for (int c = 0; c < cn; c++) {
        for (i = 0; i < total; i++)
          if (mask[i]) result += std::abs(src1[i * cn + c] - src2[i * cn + c]);
      }
  } else {
    if (!mask)
      for (i = 0; i < total; i++) {
...
double norm(InputArray _src, int normType, InputArray _mask) {
  ...
  if (normType0 == NORM_L2) result = sqrt(result);
  return result;
}

double norm(InputArray _src1, InputArray _src2, int normType,
            InputArray _mask) {
  ...
  if (normType == NORM_HAMMING || normType == NORM_HAMMING2) {
    ...
    if (!mask.empty()) bitwise_and(temp, mask, temp);
  ...
  for (i = 0; i < nplanes; i++, ++it) {
    ...
    switch (depth) {
      case CV_8U:
        result = norm_((const uchar*)sptr1, (const uchar*)sptr2, total, cn,
                       normType, result, mptr);
        break;
      case CV_8S:
        result = norm_((const schar*)sptr1, (const schar*)sptr2, total, cn,
                       normType, result, mptr);
        break;
      case CV_16U:
        result = norm_((const ushort*)sptr1, (const ushort*)sptr2, total, cn,
                       normType, result, mptr);
        break;
      case CV_16S:
        result = norm_((const short*)sptr1, (const short*)sptr2, total, cn,
                       normType, result, mptr);
        break;
      case CV_32S:
        result = norm_((const int*)sptr1, (const int*)sptr2, total, cn,
                       normType, result, mptr);
        break;
      case CV_32F:
        result = norm_((const float*)sptr1, (const float*)sptr2, total, cn,
                       normType, result, mptr);
        break;
      case CV_64F:
        result = norm_((const double*)sptr1, (const double*)sptr2, total, cn,
                       normType, result, mptr);
        break;
      default:
        CV_Error(Error::StsUnsupportedFormat, "");
  ...
  if (normType0 == NORM_L2) result = sqrt(result);
  return isRelative ? result / (cvtest::norm(src2, normType) + DBL_EPSILON)
                    : result;
}

double PSNR(InputArray _src1, InputArray _src2) {
  CV_Assert(_src1.depth() == CV_8U);
  double diff = std::sqrt(cvtest::norm(_src1, _src2, NORM_L2SQR) /
                          (_src1.total() * _src1.channels()));
  return 20 * log10(255. / (diff + DBL_EPSILON));
}

template <typename _Tp>
static double crossCorr_(const _Tp* src1, const _Tp* src2, size_t total) {
  double result = 0;
  for (size_t i = 0; i < total; i++) result += (double)src1[i] * src2[i];
  return result;
}
...
static void logicOp_(const uchar* src1, const uchar* src2, uchar* dst,
                     size_t total, char c) {
  size_t i;
  if (c == '&')
    for (i = 0; i < total; i++) dst[i] = src1[i] & src2[i];
  else if (c == '|')
    for (i = 0; i < total; i++) dst[i] = src1[i] | src2[i];
  else
    for (i = 0; i < total; i++) dst[i] = src1[i] ^ src2[i];
...
static void logicOpS_(const uchar* src, const uchar* scalar, uchar* dst,
                      size_t total, char c) {
  ...
  if (c == '&')
    for (i = 0; i < total; i += blockSize, dst += blockSize, src += blockSize) {
      ...
      for (j = 0; j < sz; j++) dst[j] = src[j] & scalar[j];
    }
  else if (c == '|')
    for (i = 0; i < total; i += blockSize, dst += blockSize, src += blockSize) {
      size_t sz = MIN(total - i, blockSize);
...
template <typename _Tp>
static void compare_(const _Tp* src1, const _Tp* src2, uchar* dst, size_t total,
                     int cmpop) {
  size_t i;
  switch (cmpop) {
    case CMP_LT:
      for (i = 0; i < total; i++) dst[i] = src1[i] < src2[i] ? 255 : 0;
      break;
    case CMP_LE:
      for (i = 0; i < total; i++) dst[i] = src1[i] <= src2[i] ? 255 : 0;
      break;
...
template <typename _Tp, typename _WTp>
static void compareS_(const _Tp* src1, _WTp value, uchar* dst, size_t total,
                      int cmpop) {
  size_t i;
  switch (cmpop) {
    case CMP_LT:
      for (i = 0; i < total; i++) dst[i] = src1[i] < value ? 255 : 0;
      break;
    case CMP_LE:
      for (i = 0; i < total; i++) dst[i] = src1[i] <= value ? 255 : 0;
      break;
  ...
  for (i = 0; i < nplanes; i++, ++it) {
    ...
    switch (depth) {
      ...
      case CV_8S:
        ...
        break;
      case CV_16U:
        compare_((const ushort*)sptr1, (const ushort*)sptr2, dptr, total,
                 cmpop);
        break;
      case CV_16S:
      ...
      case CV_32F:
        ...
        break;
      case CV_64F:
        compare_((const double*)sptr1, (const double*)sptr2, dptr, total,
                 cmpop);
        break;
      default:
        CV_Error(Error::StsUnsupportedFormat, "");
...
template <typename _Tp>
double cmpUlpsInt_(const _Tp* src1, const _Tp* src2, size_t total, int imaxdiff,
  ...
  for (i = 0; i < total; i++) {
    ...
    if (realmaxdiff < diff) {
      realmaxdiff = diff;
      if (diff > imaxdiff && idx == 0) idx = i + startidx;
    }
  }
  return realmaxdiff;
}
...
template <>
double cmpUlpsInt_<int>(const int* src1, const int* src2, size_t total,
                        int imaxdiff, size_t startidx, size_t& idx) {
  size_t i;
...
static double cmpUlpsFlt_(const int* src1, const int* src2, size_t total,
                          int imaxdiff, size_t startidx, size_t& idx) {
...
static double cmpUlpsFlt_(const int64* src1, const int64* src2, size_t total,
                          int imaxdiff, size_t startidx, size_t& idx) {
...
bool cmpUlps(const Mat& src1, const Mat& src2, int imaxDiff,
             double* _realmaxdiff, vector<int>* loc) {
  CV_Assert(src1.type() == src2.type() && src1.size == src2.size);
  const Mat* arrays[] = {&src1, &src2, 0};
  Mat planes[2];
  NAryMatIterator it(arrays, planes);
  ...
  if (_realmaxdiff) *_realmaxdiff = 0;
  ...
  for (i = 0; i < nplanes; i++, ++it, startidx += total) {
    ...
    switch (depth) {
      case CV_8U:
        realmaxdiff = cmpUlpsInt_((const uchar*)sptr1, (const uchar*)sptr2,
                                  total, imaxDiff, startidx, idx);
        break;
      case CV_8S:
        realmaxdiff = cmpUlpsInt_((const schar*)sptr1, (const schar*)sptr2,
                                  total, imaxDiff, startidx, idx);
        break;
      case CV_16U:
        realmaxdiff = cmpUlpsInt_((const ushort*)sptr1, (const ushort*)sptr2,
                                  total, imaxDiff, startidx, idx);
        break;
      case CV_16S:
        realmaxdiff = cmpUlpsInt_((const short*)sptr1, (const short*)sptr2,
                                  total, imaxDiff, startidx, idx);
        break;
      case CV_32S:
        realmaxdiff = cmpUlpsInt_((const int*)sptr1, (const int*)sptr2, total,
                                  imaxDiff, startidx, idx);
        break;
      case CV_32F:
        realmaxdiff = cmpUlpsFlt_((const int*)sptr1, (const int*)sptr2, total,
                                  imaxDiff, startidx, idx);
        break;
      case CV_64F:
        realmaxdiff = cmpUlpsFlt_((const int64*)sptr1, (const int64*)sptr2,
                                  total, imaxDiff, startidx, idx);
        break;
      default:
        CV_Error(Error::StsUnsupportedFormat, "");
    ...
    if (_realmaxdiff) *_realmaxdiff = std::max(*_realmaxdiff, realmaxdiff);
  }
  if (idx > 0 && loc) setpos(src1, *loc, idx);
  return idx == 0;
}

template <typename _Tp>
static void checkInt_(const _Tp* a, size_t total, int imin, int imax,
                      size_t startidx, size_t& idx) {
  for (size_t i = 0; i < total; i++) {
    int val = a[i];
...
template <typename _Tp>
static void checkFlt_(const _Tp* a, size_t total, double fmin, double fmax,
                      size_t startidx, size_t& idx) {
  for (size_t i = 0; i < total; i++) {
    double val = a[i];
...
int check(const Mat& a, double fmin, double fmax, vector<int>* _idx) {
  ...
  if (idx != 0 && _idx) setpos(a, *_idx, idx);
  return idx == 0 ? 0 : -1;
}
...
#define CMP_EPS_INVALID_REF_DATA \
  -3  // there is NaN or Inf value in reference data

// compares two arrays. max_diff is the maximum actual difference,
// success_err_level is maximum allowed difference, idx is the index of the
// first element for which difference is >success_err_level (or index of element
...
int cmpEps(const Mat& arr, const Mat& refarr, double* _realmaxdiff,
  ...
  for (i = 0; i < nplanes; i++, ++it, startidx += total) {
    ...
    switch (depth) {
      ...
      case CV_32F:
        for (j = 0; j < total; j++) {
          ...
          double threshold;
          if (((int*)sptr1)[j] == ((int*)sptr2)[j]) continue;
      ...
      case CV_64F:
        for (j = 0; j < total; j++) {
          ...
          double threshold;
          if (((int64*)sptr1)[j] == ((int64*)sptr2)[j]) continue;
          if (cvIsNaN(a_val) || cvIsInf(a_val)) {
            result = CMP_EPS_INVALID_TEST_DATA;
            idx = startidx + j;
            break;
    ...
    if (_realmaxdiff) *_realmaxdiff = MAX(*_realmaxdiff, realmaxdiff);
    if (idx != 0) break;
  }

  if (result == 0 && idx != 0) result = CMP_EPS_BIG_DIFF;

  if (result < -1 && _realmaxdiff) *_realmaxdiff = exp(1000.);
  if (idx > 0 && _idx) setpos(arr, *_idx, idx);
...
int cmpEps2(TS* ts, const Mat& a, const Mat& b, double success_err_level,
  ...
  vector<int> idx;
  int code =
      cmpEps(a, b, &diff, success_err_level, &idx, element_wise_relative_error);
...
template <typename _Tp>
static void GEMM_(const _Tp* a_data0, int a_step, int a_delta,
                  const _Tp* b_data0, int b_step, int b_delta,
                  const _Tp* c_data0, int c_step, int c_delta, _Tp* d_data,
                  int d_step, int d_rows, int d_cols, int a_cols, int cn,
                  double alpha, double beta) {
  for (int i = 0; i < d_rows;
       i++, d_data += d_step, c_data0 += c_step, a_data0 += a_step) {
    for (int j = 0; j < d_cols; j++) {
      ...
      if (cn == 1) {
        ...
        for (int k = 0; k < a_cols; k++) {
          s_re +=
              ((double)a_data[0]) * b_data[0] - ((double)a_data[1]) * b_data[1];
          s_im +=
              ((double)a_data[0]) * b_data[1] + ((double)a_data[1]) * b_data[0];
          a_data += a_delta;
          b_data += b_step;
        }
...
void gemm(const Mat& _a, const Mat& _b, double alpha, const Mat& _c,
          double beta, Mat& d, int flags) {
  Mat a = _a, b = _b, c = _c;

  if (a.data == d.data) a = a.clone();

  if (b.data == d.data) b = b.clone();

  if (!c.empty() && c.data == d.data && (flags & cv::GEMM_3_T)) c = c.clone();
  ...
  if (!c.empty()) {
    ...
    CV_Assert(c.dims == 2 && c.type() == a.type() && c_rows == a_rows &&
              c_cols == b_cols);
  }
...
template <typename _Tp>
static void transform_(const _Tp* sptr, _Tp* dptr, size_t total, int scn,
                       int dcn, const double* mat) {
  for (size_t i = 0; i < total; i++, sptr += scn, dptr += dcn) {
    for (int j = 0; j < dcn; j++) {
      double s = mat[j * (scn + 1) + scn];
      for (int k = 0; k < scn; k++) s += mat[j * (scn + 1) + k] * sptr[k];
      dptr[j] = saturate_cast<_Tp>(s);
    }
  }
}

void transform(const Mat& src, Mat& dst, const Mat& transmat,
               const Mat& _shift) {
  double mat[20];
  ...
  CV_Assert(scn <= 4 && dcn <= 4 && (mattype == CV_32F || mattype == CV_64F) &&
            (!haveShift || (shift.type() == mattype &&
  ...
  if (mattype == CV_32F) {
    for (int i = 0; i < transmat.rows; i++) {
      ...
      if (haveShift) mat[i * (scn + 1) + scn] = shift.at<float>(i);
    }
  } else {
    for (int i = 0; i < transmat.rows; i++) {
      mat[i * (scn + 1) + scn] = 0.;
      ...
      if (haveShift) mat[i * (scn + 1) + scn] = shift.at<double>(i);
    }
  }
...
template <typename _Tp>
static void minmax_(const _Tp* src1, const _Tp* src2, _Tp* dst, size_t total,
                    char op) {
  if (op == 'M')
    for (size_t i = 0; i < total; i++) dst[i] = std::max(src1[i], src2[i]);
  else
    for (size_t i = 0; i < total; i++) dst[i] = std::min(src1[i], src2[i]);
}
...
static void minmax(const Mat& src1, const Mat& src2, Mat& dst, char op) {
  ...
  for (i = 0; i < nplanes; i++, ++it) {
    ...
    switch (depth) {
      case CV_8U:
        minmax_((const uchar*)sptr1, (const uchar*)sptr2, (uchar*)dptr, total,
                op);
        break;
      case CV_8S:
        minmax_((const schar*)sptr1, (const schar*)sptr2, (schar*)dptr, total,
                op);
        break;
      case CV_16U:
        minmax_((const ushort*)sptr1, (const ushort*)sptr2, (ushort*)dptr,
                total, op);
        break;
      case CV_16S:
        minmax_((const short*)sptr1, (const short*)sptr2, (short*)dptr, total,
                op);
        break;
      case CV_32S:
        ...
        break;
      case CV_32F:
        minmax_((const float*)sptr1, (const float*)sptr2, (float*)dptr, total,
                op);
        break;
      case CV_64F:
        minmax_((const double*)sptr1, (const double*)sptr2, (double*)dptr,
                total, op);
        break;
      default:
        CV_Error(Error::StsUnsupportedFormat, "");
...
template <typename _Tp>
static void minmax_(const _Tp* src1, _Tp val, _Tp* dst, size_t total, char op) {
  if (op == 'M')
    for (size_t i = 0; i < total; i++) dst[i] = std::max(src1[i], val);
  else
    for (size_t i = 0; i < total; i++) dst[i] = std::min(src1[i], val);
}
...
static void minmax(const Mat& src1, double val, Mat& dst, char op) {
  ...
  for (i = 0; i < nplanes; i++, ++it) {
    ...
    switch (depth) {
      case CV_8U:
        minmax_((const uchar*)sptr1, saturate_cast<uchar>(ival), (uchar*)dptr,
                total, op);
        break;
      case CV_8S:
        minmax_((const schar*)sptr1, saturate_cast<schar>(ival), (schar*)dptr,
                total, op);
        break;
      case CV_16U:
        minmax_((const ushort*)sptr1, saturate_cast<ushort>(ival),
                (ushort*)dptr, total, op);
        break;
      case CV_16S:
        minmax_((const short*)sptr1, saturate_cast<short>(ival), (short*)dptr,
                total, op);
        break;
      case CV_32S:
        minmax_((const int*)sptr1, saturate_cast<int>(ival), (int*)dptr, total,
                op);
        break;
      case CV_32F:
        minmax_((const float*)sptr1, saturate_cast<float>(val), (float*)dptr,
                total, op);
        break;
      case CV_64F:
        minmax_((const double*)sptr1, saturate_cast<double>(val), (double*)dptr,
                total, op);
        break;
      default:
        CV_Error(Error::StsUnsupportedFormat, "");
...
void min(const Mat& src1, double val, Mat& dst) { minmax(src1, val, dst, 'm'); }

void max(const Mat& src1, double val, Mat& dst) { minmax(src1, val, dst, 'M'); }

template <typename _Tp>
static void muldiv_(const _Tp* src1, const _Tp* src2, _Tp* dst, size_t total,
                    double scale, char op) {
  if (op == '*')
    for (size_t i = 0; i < total; i++)
...
static void muldiv(const Mat& src1, const Mat& src2, Mat& dst, double scale,
                   char op) {
  dst.create(src2.dims, src2.size, src2.type());
  CV_Assert(src1.empty() ||
            (src1.type() == src2.type() && src1.size == src2.size));
  const Mat* arrays[] = {&src1, &src2, &dst, 0};
  Mat planes[3];
  ...
  for (i = 0; i < nplanes; i++, ++it) {
    ...
    switch (depth) {
      case CV_8U:
        muldiv_((const uchar*)sptr1, (const uchar*)sptr2, (uchar*)dptr, total,
                scale, op);
        break;
      case CV_8S:
        muldiv_((const schar*)sptr1, (const schar*)sptr2, (schar*)dptr, total,
                scale, op);
        break;
      case CV_16U:
        muldiv_((const ushort*)sptr1, (const ushort*)sptr2, (ushort*)dptr,
                total, scale, op);
        break;
      case CV_16S:
        muldiv_((const short*)sptr1, (const short*)sptr2, (short*)dptr, total,
                scale, op);
        break;
      case CV_32S:
        muldiv_((const int*)sptr1, (const int*)sptr2, (int*)dptr, total, scale,
                op);
        break;
      case CV_32F:
        muldiv_((const float*)sptr1, (const float*)sptr2, (float*)dptr, total,
                scale, op);
        break;
      case CV_64F:
        muldiv_((const double*)sptr1, (const double*)sptr2, (double*)dptr,
                total, scale, op);
        break;
      default:
        CV_Error(Error::StsUnsupportedFormat, "");
...
template <typename _Tp>
static void mean_(const _Tp* src, const uchar* mask, size_t total, int cn,
                  Scalar& sum, int& nz) {
  if (!mask) {
    nz += (int)total;
    total *= cn;
    for (size_t i = 0; i < total; i += cn) {
      for (int c = 0; c < cn; c++) sum[c] += src[i + c];
    }
  } else {
    for (size_t i = 0; i < total; i++)
      if (mask[i]) {
        nz++;
        for (int c = 0; c < cn; c++) sum[c] += src[i * cn + c];
      }
  }
}
...
static void calcSobelKernel1D(int order, int _aperture_size, int size,
                              vector<int>& kernel) {
  ...
  if (_aperture_size < 0) {
    ...
    assert(size == 3);
    for (i = 0; i < size; i++) kernel[i] = scharr[order * 3 + i];
    return;
  }

  for (i = 1; i <= size; i++) kernel[i] = 0;
  kernel[0] = 1;
...
Mat calcSobelKernel2D(int dx, int dy, int _aperture_size, int origin) {
  CV_Assert((_aperture_size == -1 ||
             (_aperture_size >= 1 && _aperture_size % 2 == 1)) &&
            dx >= 0 && dy >= 0 && dx + dy <= 3);
  Size ksize = _aperture_size == -1
                   ? Size(3, 3)
                   : _aperture_size > 1 ? Size(_aperture_size, _aperture_size)
                                        : dx > 0 ? Size(3, 1) : Size(1, 3);
  ...
  for (int i = 0; i < kernel.rows; i++) {
    ...
    for (int j = 0; j < kernel.cols; j++) kernel.at<float>(i, j) = kx[j] * ay;
  }
...
void initUndistortMap(const Mat& _a0, const Mat& _k0, Size sz, Mat& _mapx,
                      Mat& _mapy) {
...
std::ostream& operator<<(std::ostream& out, const MatInfo& m) {
  if (!m.m || m.m->empty())
    ...
    static const char* depthstr[] = {"8u",  "8s",  "16u", "16s",
                                     "32s", "32f", "64f", "?"};
    out << depthstr[m.m->depth()] << "C" << m.m->channels() << " " << m.m->dims
        << "-dim (";
...
static Mat getSubArray(const Mat& m, int border, vector<int>& ofs0,
                       vector<int>& ofs) {
...
template <typename _Tp, typename _WTp>
static void writeElems(std::ostream& out, const void* data, int nelems,
                       int starpos) {
...
struct MatPart {
  MatPart(const Mat& _m, const vector<int>* _loc) : m(&_m), loc(_loc) {}
  const Mat* m;
...
static std::ostream& operator<<(std::ostream& out, const MatPart& m) {
  ...
  if (!m.loc)
    ...
    for (i = 0; i < m.m->rows; i++) {
      writeElems(out, m.m->ptr(i), width, depth,
                 i == (*m.loc)[0] ? (*m.loc)[1] : -1);
      out << (i < m.m->rows - 1 ? ";\n" : "");
    }
  }
...
::testing::AssertionResult MatComparator::operator()(const char* expr1,
                                                     const char* expr2,
                                                     const Mat& m1,
                                                     const Mat& m2) {
  if (m1.type() != m2.type() || m1.size != m2.size)
    return ::testing::AssertionFailure()
           << "The reference and the actual output arrays have different type "
              "or size:\n"
           << expr1 << " ~ " << MatInfo(m1) << "\n"
           << expr2 << " ~ " << MatInfo(m2) << "\n";
  ...
  if (code >= 0) return ::testing::AssertionSuccess();
  ...
  return ::testing::AssertionFailure()
         << "too big relative difference (" << realmaxdiff << " > " << maxdiff
         << ") between " << MatInfo(m1) << " '" << expr1 << "' and '" << expr2
         << "' at " << Mat(loc0).t() << ".\n"
         << "- " << expr1 << ":\n"
...
void printVersionInfo(bool useStdOut) {
  ...
  if (parallel_framework) {
    ::testing::Test::RecordProperty("cv_parallel_framework",
                                    parallel_framework);
    if (useStdOut)
  ...
  if (checkHardwareSupport(CV_CPU_AVX_512IFMA512))
    cpu_features += " avx-512ifma512";
#endif
#if CV_AVX_512PF
...
#ifdef HAVE_TEGRA_OPTIMIZATION
  const char* tegra_optimization =
      tegra::useTegra() && tegra::isDeviceSupported() ? "enabled" : "disabled";
  ::testing::Test::RecordProperty("cv_tegra_optimization", tegra_optimization);
  if (useStdOut)
    std::cout << "Tegra optimization: " << tegra_optimization << std::endl;
#endif
  ...
  if (useStdOut)
    std::cout << "Intel(R) IPP optimization: " << ipp_optimization << std::endl;

  cv::String ippVer = cv::ipp::getIppVersion();
  ::testing::Test::RecordProperty("cv_ipp_version", ippVer);
  if (useStdOut)
    std::cout << "Intel(R) IPP version: " << ippVer.c_str() << std::endl;
#endif
}

void threshold(const Mat& _src, Mat& _dst, double thresh, double maxval,
               int thresh_type) {
  int i, j;
...
static void _minMaxIdx(const float* src, const uchar* mask, double* _minVal,
                       double* _maxVal, size_t* _minIdx, size_t* _maxIdx,
  ...
  if (_minIdx) *_minIdx = minIdx;
  if (_maxIdx) *_maxIdx = maxIdx;
  if (_minVal) *_minVal = minVal;
  if (_maxVal) *_maxVal = maxVal;
}

void minMaxIdx(InputArray _img, double* minVal, double* maxVal, Point* minLoc,
               Point* maxLoc, InputArray _mask) {
  Mat img = _img.getMat();
  Mat mask = _mask.getMat();
  CV_Assert(img.dims <= 2);

  _minMaxIdx((const float*)img.data, mask.data, minVal, maxVal, (size_t*)minLoc,
             (size_t*)maxLoc, (int)img.total(), 1);
