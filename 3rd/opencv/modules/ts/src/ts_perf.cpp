...
static void setCurrentThreadAffinityMask(int mask) {
  ...
  if (syscallres) {
    int err = errno;
    CV_UNUSED(err);
    LOGE("Error in the syscall setaffinity: mask=%d=0x%x err=%d=0x%x", mask,
         mask, err, err);
  }
}
#endif
...
class PerfEnvironment : public ::testing::Environment {
 public:
  void TearDown() { cv::setNumThreads(-1); }
};
...
static void randu(cv::Mat& m) {
  ...
  if (m.depth() < CV_32F) {
    int minmax[] = {0, 256};
    cv::Mat mr = cv::Mat(m.rows, (int)(m.cols * m.elemSize()), CV_8U, m.ptr(),
                         m.step[0]);
    cv::randu(mr, cv::Mat(1, 1, CV_32S, minmax),
              cv::Mat(1, 1, CV_32S, minmax + 1));
  } else if (m.depth() == CV_32F) {
    // float minmax[] = {-FLT_MAX, FLT_MAX};
    float minmax[] = {-bigValue, bigValue};
    cv::Mat mr = m.reshape(1);
    cv::randu(mr, cv::Mat(1, 1, CV_32F, minmax),
              cv::Mat(1, 1, CV_32F, minmax + 1));
  } else {
    // double minmax[] = {-DBL_MAX, DBL_MAX};
    double minmax[] = {-bigValue, bigValue};
    cv::Mat mr = m.reshape(1);
    cv::randu(mr, cv::Mat(1, 1, CV_64F, minmax),
              cv::Mat(1, 1, CV_64F, minmax + 1));
  }
}
...
Regression& Regression::add(TestBase* test, const std::string& name,
                            cv::InputArray array, double eps, ERROR_TYPE err) {
  if (test) test->setVerified();
  return instance()(name, array, eps, err);
}

Regression& Regression::addMoments(TestBase* test, const std::string& name,
                                   const cv::Moments& array, double eps,
                                   ERROR_TYPE err) {
...
Regression& Regression::addKeypoints(TestBase* test, const std::string& name,
                                     const std::vector<cv::KeyPoint>& array,
                                     double eps, ERROR_TYPE err) {
  int len = (int)array.size();
  cv::Mat pt(len, 1, CV_32FC2, len ? (void*)&array[0].pt : 0,
             sizeof(cv::KeyPoint));
  cv::Mat size(len, 1, CV_32FC1, len ? (void*)&array[0].size : 0,
               sizeof(cv::KeyPoint));
  cv::Mat angle(len, 1, CV_32FC1, len ? (void*)&array[0].angle : 0,
                sizeof(cv::KeyPoint));
  cv::Mat response(len, 1, CV_32FC1, len ? (void*)&array[0].response : 0,
                   sizeof(cv::KeyPoint));
  cv::Mat octave(len, 1, CV_32SC1, len ? (void*)&array[0].octave : 0,
                 sizeof(cv::KeyPoint));
  cv::Mat class_id(len, 1, CV_32SC1, len ? (void*)&array[0].class_id : 0,
                   sizeof(cv::KeyPoint));

  return Regression::add(test, name + "-pt", pt, eps, ERROR_ABSOLUTE)(
      name + "-size", size, eps, ERROR_ABSOLUTE)(name + "-angle", angle, eps,
                                                 ERROR_ABSOLUTE)(
      name + "-response", response, eps, err)(name + "-octave", octave, eps,
                                              ERROR_ABSOLUTE)(
      name + "-class_id", class_id, eps, ERROR_ABSOLUTE);
}

Regression& Regression::addMatches(TestBase* test, const std::string& name,
                                   const std::vector<cv::DMatch>& array,
                                   double eps, ERROR_TYPE err) {
  int len = (int)array.size();
  cv::Mat queryIdx(len, 1, CV_32SC1, len ? (void*)&array[0].queryIdx : 0,
                   sizeof(cv::DMatch));
  cv::Mat trainIdx(len, 1, CV_32SC1, len ? (void*)&array[0].trainIdx : 0,
                   sizeof(cv::DMatch));
  cv::Mat imgIdx(len, 1, CV_32SC1, len ? (void*)&array[0].imgIdx : 0,
                 sizeof(cv::DMatch));
  cv::Mat distance(len, 1, CV_32FC1, len ? (void*)&array[0].distance : 0,
                   sizeof(cv::DMatch));

  return Regression::add(test, name + "-queryIdx", queryIdx, DBL_EPSILON,
                         ERROR_ABSOLUTE)(name + "-trainIdx", trainIdx,
                                         DBL_EPSILON, ERROR_ABSOLUTE)(
...
void Regression::init(const std::string& testSuitName, const std::string& ext) {
  ...
  if (data_path_dir) {
    ...
    std::string path_base =
        (data_path_dir[0] == 0 ? std::string(".")
                               : std::string(data_path_dir)) +
  ...
  try {
    if (storageIn.open(storageInPath, cv::FileStorage::READ)) {
      ...
      if (storageInPath.length() > 3 &&
          storageInPath.substr(storageInPath.length() - 3) == ".gz")
        storageOutPath += "_new";
      storageOutPath += ext;
    }
  } catch (cv::Exception&) {
  ...
  if (!storageIn.isOpened()) storageOutPath = storageInPath;
}

Regression::Regression()
    : regRNG(cv::getTickCount())  // this rng should be really random
{}

Regression::~Regression() {
  if (storageIn.isOpened()) storageIn.release();
  if (storageOut.isOpened()) {
    if (!currentTestNodeName.empty()) storageOut << "}";
    storageOut.release();
  }
}
...
std::string Regression::getCurrentTestNodeName() {
  ...
  if (test_info == 0) return "undefined";

  std::string nodename =
      std::string(test_info->test_case_name()) + "--" + test_info->name();
  size_t idx = nodename.find_first_of('/');
  if (idx != std::string::npos) nodename.erase(idx);

  const char* type_param = test_info->type_param();
  if (type_param != 0) (nodename += "--") += type_param;

  const char* value_param = test_info->value_param();
  if (value_param != 0) (nodename += "--") += value_param;

  for (size_t i = 0; i < nodename.length(); ++i)
    if (!isalnum(nodename[i]) && '_' != nodename[i]) nodename[i] = '-';

  return nodename;
}

bool Regression::isVector(cv::InputArray a) {
  return a.kind() == cv::_InputArray::STD_VECTOR_MAT ||
         a.kind() == cv::_InputArray::STD_VECTOR_VECTOR ||
...
double Regression::getElem(cv::Mat& m, int y, int x, int cn) {
  switch (m.depth()) {
    case CV_8U:
      return *(m.ptr<unsigned char>(y, x) + cn);
    case CV_8S:
      return *(m.ptr<signed char>(y, x) + cn);
    case CV_16U:
      return *(m.ptr<unsigned short>(y, x) + cn);
    case CV_16S:
      return *(m.ptr<signed short>(y, x) + cn);
    case CV_32S:
      return *(m.ptr<signed int>(y, x) + cn);
    case CV_32F:
      return *(m.ptr<float>(y, x) + cn);
    case CV_64F:
      return *(m.ptr<double>(y, x) + cn);
    default:
      return 0;
  }
}
...
void Regression::write(cv::Mat m) {
  ...
  write() << "last"
          << "{"
          << "x" << m.size.p[1] - 1 << "y" << m.size.p[0] - 1 << "val"
          << getElem(m, m.size.p[0] - 1, m.size.p[1] - 1, m.channels() - 1)
          << "}";
...
void Regression::verify(cv::FileNode node, cv::Mat actual, double eps,
                        std::string argname, ERROR_TYPE err) {
  ...
  double actual_last = getElem(actual, actual.size.p[0] - 1,
                               actual.size.p[1] - 1, actual.channels() - 1);
  ...
  ASSERT_NEAR(expect_rng1, actual_rng1, eps)
      << argname << " has unexpected value of the [" << x1 << ":" << y1 << ":"
      << cn1 << "] element" << std::endl;
  ...
  ASSERT_NEAR(expect_rng2, actual_rng2, eps)
      << argname << " has unexpected value of the [" << x2 << ":" << y2 << ":"
      << cn2 << "] element" << std::endl;
}
...
static int countViolations(const cv::Mat& expected, const cv::Mat& actual,
                           const cv::Mat& diff, double eps,
                           double* max_violation = 0, double* max_allowed = 0) {
  cv::Mat diff64f;
...
void Regression::verify(cv::FileNode node, cv::InputArray array, double eps,
  ...
  ASSERT_EQ(expected_kind, array.kind())
      << "  Argument \"" << node.name() << "\" has unexpected kind";
  ASSERT_EQ(expected_type, array.type())
      << "  Argument \"" << node.name() << "\" has unexpected type";

  cv::FileNode valnode = node["val"];
  if (isVector(array)) {
    int expected_length = (int)node["len"];
    ASSERT_EQ(expected_length, (int)array.total())
        << "  Vector \"" << node.name() << "\" has unexpected length";
    int idx = node["idx"];
      ...
      if (expected.empty()) {
        ...
        if (err == ERROR_ABSOLUTE) {
          if (!cv::checkRange(diff, true, 0, 0, eps)) {
            ...
            FAIL() << "  Absolute difference (=" << max
                   << ") between argument \"" << node.name() << "[" << idx
                   << "]\" and expected value is greater than " << eps;
          }
        } else if (err == ERROR_RELATIVE) {
          double maxv, maxa;
          int violations =
              countViolations(expected, actual, diff, eps, &maxv, &maxa);
          if (violations > 0) {
            if (expected.total() * expected.channels() < 12)
              std::cout << " Expected: " << std::endl
                        << expected << std::endl
            ...
            FAIL() << "  Relative difference (" << maxv << " of " << maxa
                   << " allowed) between argument \"" << node.name() << "["
                   << idx << "]\" and expected value is greater than " << eps
                   << " in " << violations << " points";
          }
        }
      }
    }
    ...
    if (valnode.isNone()) {
      ...
      if (expected.empty()) {
        ASSERT_TRUE(actual.empty()) << "  expected empty " << node.name();
      } else {
        ASSERT_EQ(expected.size(), actual.size())
        ...
        if (err == ERROR_ABSOLUTE) {
          if (!cv::checkRange(diff, true, 0, 0, eps)) {
            ...
            FAIL() << "  Difference (=" << max << ") between argument1 \""
                   << node.name() << "\" and expected value is greater than "
                   << eps;
          }
        } else if (err == ERROR_RELATIVE) {
          double maxv, maxa;
          ...
          if (violations > 0) {
            ...
            FAIL() << "  Relative difference (" << maxv << " of " << maxa
                   << " allowed) between argument \"" << node.name()
                   << "\" and expected value is greater than " << eps << " in "
                   << violations << " points";
          }
        }
      }
...
Regression& Regression::operator()(const std::string& name,
                                   cv::InputArray array, double eps,
                                   ERROR_TYPE err) {
  ...
  if (!array.empty() && array.depth() == CV_USRTYPE1) {
    ADD_FAILURE() << "  Can not check regression for CV_USRTYPE1 data type for "
                  << name;
    return *this;
  }
  ...
  if (n.isNone()) {
    if (param_write_sanity) {
      if (nodename != currentTestNodeName) {
        if (!currentTestNodeName.empty()) write() << "}";
        currentTestNodeName = nodename;

        write() << nodename << "{";
      }
      // TODO: verify that name is alphanumeric, current error message is
      // useless
      write() << name << "{";
      write(array);
      write() << "}";
    } else if (param_verify_sanity) {
      ADD_FAILURE() << "  No regression data for " << name
                    << " argument, test node: " << nodename;
    }
  } else {
...
\*****************************************************************************************/
performance_metrics::performance_metrics() { clear(); }
...
static void loadPerfValidationResults(const std::string& fileName) {
  ...
  while (!infile.eof()) {
    ...
    float value = 0;
    if (!(infile >> value)) {
      if (infile.eof()) break;  // it is OK
      std::cout << "ERROR: Can't load performance validation results from "
                << fileName << "!" << std::endl;
      return;
    }
    infile.ignore(1);
    ...
    if (!name.empty() &&
        name[name.size() - 1] == '\r')  // CRLF processing on Linux
      name.resize(name.size() - 1);
    perf_validation_results[name] = value;
  }
  std::cout << "Performance validation results loaded from " << fileName << " ("
            << perf_validation_results.size() << " entries)" << std::endl;
}
...
static void savePerfValidationResults() {
  if (!perf_validation_results_outfile.empty()) {
    std::ofstream outfile(
        (perf_validation_results_directory + perf_validation_results_outfile)
            .c_str());
    std::map<std::string, float>::const_iterator i;
    for (i = perf_validation_results.begin();
         i != perf_validation_results.end(); ++i) {
      outfile << i->second << ';';
      outfile << i->first << std::endl;
    }
    outfile.close();
    std::cout << "Performance validation results saved ("
              << perf_validation_results.size() << " entries)" << std::endl;
  }
}
...
class PerfValidationEnvironment : public ::testing::Environment {
 public:
  ...
  virtual void TearDown() { savePerfValidationResults(); }
};

#ifdef ENABLE_INSTRUMENTATION
static void printShift(cv::instr::InstrNode* pNode,
                       cv::instr::InstrNode* pRoot) {
  // Print empty line for a big tree nodes
  if (pNode->m_pParent) {
    int parendIdx = pNode->m_pParent->findChild(pNode);
    if (parendIdx > 0 &&
        pNode->m_pParent->m_childs[parendIdx - 1]->m_childs.size()) {
  ...
  for (int i = (int)cache.size() - 1; i >= 0; i--) {
    if (cache[i]->m_pParent) {
      if (cache[i]->m_pParent->findChild(cache[i]) ==
          (int)cache[i]->m_pParent->m_childs.size() - 1)
        printf("    ");
      else
        printf("|   ");
    }
...
static double calcLocalWeight(cv::instr::InstrNode* pNode) {
  if (pNode->m_pParent && pNode->m_pParent->m_pParent)
    return ((double)pNode->m_payload.m_ticksTotal * 100 /
            pNode->m_pParent->m_payload.m_ticksTotal);
  else
    return 100;
}
...
static double calcGlobalWeight(cv::instr::InstrNode* pNode) {
  ...
  return ((double)pNode->m_payload.m_ticksTotal * 100 /
          (double)globNode->m_payload.m_ticksTotal);
}

static void printNodeRec(cv::instr::InstrNode* pNode,
                         cv::instr::InstrNode* pRoot) {
  printf("%s", (pNode->m_payload.m_funName.substr(0, 40) +
                ((pNode->m_payload.m_funName.size() > 40) ? "..." : ""))
                   .c_str());

  // Write instrumentation flags
  if (pNode->m_payload.m_instrType != cv::instr::TYPE_GENERAL ||
      pNode->m_payload.m_implType != cv::instr::IMPL_PLAIN) {
    printf("<");
  ...
  if (pNode->m_pParent) {
    printf(" - TC:%d C:%d", pNode->m_payload.m_threads,
           pNode->m_payload.m_counter);
    printf(" T:%.2fms", pNode->m_payload.getTotalMs());
    if (pNode->m_pParent->m_pParent)
      printf(" L:%.0f%% G:%.0f%%", calcLocalWeight(pNode),
             calcGlobalWeight(pNode));
  }
  printf("\n");

  {
    ...
    for (size_t i = 1; i < pNode->m_childs.size(); i++) {
      if (pNode->m_childs[i - 1]->m_payload.m_funName ==
          pNode->m_childs[i]->m_payload.m_funName)
        continue;
      for (size_t j = i + 1; j < pNode->m_childs.size(); j++) {
        if (pNode->m_childs[i - 1]->m_payload.m_funName ==
            pNode->m_childs[j]->m_payload.m_funName) {
...
static uint64 getNodeTimeRec(cv::instr::InstrNode* pNode, cv::instr::TYPE type,
                             cv::instr::IMPL impl) {
  uint64 ticks = 0;

  if (pNode->m_pParent && (type < 0 || pNode->m_payload.m_instrType == type) &&
      pNode->m_payload.m_implType == impl) {
...
#ifdef HAVE_IPP
  printf("\nIPP weight: %.1f%%", ((double)getImplTime(cv::instr::IMPL_IPP) *
                                  100 / (double)getTotalTime()));
#endif
#ifdef HAVE_OPENCL
  printf("\nOPENCL weight: %.1f%%",
         ((double)getImplTime(cv::instr::IMPL_OPENCL) * 100 /
          (double)getTotalTime()));
#endif
  printf("\n[/TRACE    ]\n");
  fflush(stdout);
...
void TestBase::Init(const std::vector<std::string>& availableImpls, int argc,
  ...
  const std::string command_line_keys =
      ...
      "{   perf_min_samples            |10       |minimal required numer of "
      "samples}"
      "{   perf_force_samples          |100      |force set maximum number of "
      "samples for all tests}"
      "{   perf_seed                   |809564   |seed for random numbers "
      "generator}"
      "{   perf_threads                |-1       |the number of worker "
      "threads, if parallel execution is enabled}"
      "{   perf_write_sanity           |false    |create new records for "
      "sanity checks}"
      "{   perf_verify_sanity          |false    |fail tests having no "
      "regression data for sanity checks}"
      "{   perf_impl                   |" +
      available_impls[0] +
      "|the implementation variant of functions under test}"
      "{   perf_list_impls             |false    |list available "
      "implementation variants and exit}"
      "{   perf_run_cpu                |false    |deprecated, equivalent to "
      "--perf_impl=plain}"
      "{   perf_strategy               |default  |specifies performance "
      "measuring strategy: default, base or simple (weak restrictions)}"
      "{   perf_read_validation_results |        |specifies file name with "
      "performance results from previous run}"
      "{   perf_write_validation_results |       |specifies file name to write "
      "performance validation results}"
#ifdef __ANDROID__
      "{   perf_time_limit             |6.0      |default time limit for a "
      "single test (in seconds)}"
      "{   perf_affinity_mask          |0        |set affinity mask for the "
      "main thread}"
      "{   perf_log_power_checkpoints  |         |additional xml logging for "
      "power measurement}"
#else
      "{   perf_time_limit             |3.0      |default time limit for a "
      "single test (in seconds)}"
#endif
      "{   perf_max_deviation          |1.0      |}"
#ifdef HAVE_IPP
      "{   perf_ipp_check              |false    |check whether IPP works "
      "without failures}"
#endif
#ifdef CV_COLLECT_IMPL_DATA
      "{   perf_collect_impl           |false    |collect info about executed "
      "implementations}"
#endif
#ifdef ENABLE_INSTRUMENTATION
      "{   perf_instrument             |0        |instrument code to collect "
      "implementations trace: 1 - perform instrumentation; 2 - separate "
      "functions with the same name }"
#endif
      "{   help h                      |false    |print help info}"
#ifdef HAVE_CUDA
      "{   perf_cuda_device            |0        |run CUDA test suite onto "
      "specific CUDA capable device}"
      "{   perf_cuda_info_only         |false    |print an information about "
      "system and an available CUDA devices and then exit.}"
#endif
  ...
  param_impl = args.get<bool>("perf_run_cpu")
                   ? "plain"
                   : args.get<std::string>("perf_impl");
  ...
  param_max_outliers =
      std::min(100., std::max(0., args.get<double>("perf_max_outliers")));
...
#ifdef HAVE_IPP
  test_ipp_check = !args.get<bool>("perf_ipp_check")
                       ? getenv("OPENCV_IPP_CHECK") != NULL
                       : true;
#endif
  ...
  if (std::find(available_impls.begin(), available_impls.end(), param_impl) ==
      available_impls.end()) {
  ...
  if (param_instrument > 0) {
    if (param_instrument == 2)
      cv::instr::setFlags(cv::instr::getFlags() |
  ...
  param_cuda_device =
      std::max(0, std::min(cv::cuda::getCudaEnabledDeviceCount(),
                           args.get<int>("perf_cuda_device")));

  if (param_impl == "cuda") {
    cv::cuda::DeviceInfo info(param_cuda_device);
    if (!info.isCompatible()) {
      printf(
          "[----------]\n[ FAILURE  ] \tDevice %s is NOT compatible with "
          "current CUDA module build.\n[----------]\n",
          info.name()),
          fflush(stdout);
      exit(-1);
    }
...
#endif
    if (path) perf_validation_results_directory = path;
  }

  std::string fileName_perf_validation_results_src =
      args.get<std::string>("perf_read_validation_results");
  if (!fileName_perf_validation_results_src.empty()) {
    perf_validation_enabled = true;
    loadPerfValidationResults(perf_validation_results_directory +
                              fileName_perf_validation_results_src);
  }

  perf_validation_results_outfile =
      args.get<std::string>("perf_write_validation_results");
  if (!perf_validation_results_outfile.empty()) {
    perf_validation_enabled = true;
  ...
  timeLimitDefault = param_time_limit == 0.0
                         ? 1
                         : (int64)(param_time_limit * cv::getTickFrequency());
  iterationsLimitDefault =
      param_force_samples == 0 ? (unsigned)(-1) : param_force_samples;
...
std::string TestBase::getSelectedImpl() { return param_impl; }

enum PERF_STRATEGY TestBase::setModulePerformanceStrategy(
    enum PERF_STRATEGY strategy) {
...
enum PERF_STRATEGY TestBase::getCurrentModulePerformanceStrategy() {
  return strategyForce == PERF_STRATEGY_DEFAULT ? strategyModule
                                                : strategyForce;
}
...
void TestBase::declareArray(SizeVector& sizes, cv::InputOutputArray a,
                            WarmUpType wtype) {
  if (!a.empty()) {
    sizes.push_back(std::pair<int, cv::Size>(getSizeInBytes(a), getSize(a)));
    warmup(a, wtype);
  } else if (a.kind() != cv::_InputArray::NONE)
    ADD_FAILURE() << "  Uninitialized input/output parameters are not allowed "
...
void TestBase::warmup(cv::InputOutputArray a, WarmUpType wtype) {
  ...
  if (a.empty())
    ...
    return;
  } else if (a.kind() != cv::_InputArray::STD_VECTOR_MAT &&
             a.kind() != cv::_InputArray::STD_VECTOR_VECTOR)
    warmup_impl(a.getMat(), wtype);
  else {
    size_t total = a.total();
    for (size_t i = 0; i < total; ++i) warmup_impl(a.getMat((int)i), wtype);
  }
}
...
int TestBase::getSizeInBytes(cv::InputArray a) {
  ...
  if (a.kind() != cv::_InputArray::STD_VECTOR_MAT &&
      a.kind() != cv::_InputArray::STD_VECTOR_VECTOR)
...
PERF_STRATEGY TestBase::getCurrentPerformanceStrategy() const {
  if (strategyForce == PERF_STRATEGY_DEFAULT)
    return (testStrategy == PERF_STRATEGY_DEFAULT) ? strategyModule
                                                   : testStrategy;
  else
...
bool TestBase::next() {
  ...
  cv::theRNG().state =
      param_seed;  // this rng should generate same numbers for each run
  ++currentIter;
  ...
  do {
    ...
    if (getCurrentPerformanceStrategy() == PERF_STRATEGY_BASE) {
      ...
      if (fabs(metrics.mean) > 1e-6)
        has_next =
            metrics.stddev > perf_stability_criteria * fabs(metrics.mean);
      else
        has_next = true;
    }
  } while (false);
  ...
  if (perf_validation_enabled && !has_next) {
    ...
    if (!perf_validation_results.empty() && !name.empty()) {
      std::map<std::string, float>::iterator i =
          perf_validation_results.find(name);
      bool isSame = false;
      bool found = false;
      bool grow = false;
      if (i != perf_validation_results.end()) {
        ...
        isSame = fabs(median_ms - prev_result) <=
                 perf_validation_criteria * fabs(median_ms);
        if (!isSame) {
      ...
      if (!isSame) {
        if (perfValidationStage < 2) {
          if (perfValidationStage == 0 && currentIter <= minIters * 3 &&
              currentIter < nIters) {
            ...
            perfValidationStage++;
                     ...
                     (grow || metrics.stddev > perf_stability_criteria *
                                                   fabs(metrics.mean))) {
            CV_TRACE_REGION("idle_delay");
            printf(
                "Performance is unstable, it may be a result of overheat "
                "problems\n");
...
#else
            WaitForSingleObjectEx(GetCurrentThread(),
                                  perf_validation_idle_delay_ms, FALSE);
#endif
#else
  ...
  if (log_power_checkpoints) {
    ...
    unsigned long long t1 =
        tim.tv_sec * 1000LLU + (unsigned long long)(tim.tv_usec / 1000.f);

    if (currentIter == 1)
      RecordProperty("test_start", cv::format("%llu", t1).c_str());
    if (!has_next)
      RecordProperty("test_complete", cv::format("%llu", t1).c_str());
  }
#endif
...
unsigned int TestBase::getTotalInputSize() const {
  unsigned int res = 0;
  for (SizeVector::const_iterator i = inputData.begin(); i != inputData.end();
       ++i)
    res += i->first;
  return res;
}
...
unsigned int TestBase::getTotalOutputSize() const {
  unsigned int res = 0;
  for (SizeVector::const_iterator i = outputData.begin(); i != outputData.end();
       ++i)
    res += i->first;
  return res;
}
...
bool TestBase::startTimer() {
  ...
  if (currentIter == 0) {
    cv::instr::setFlags(
        cv::instr::getFlags() |
        cv::instr::FLAGS_MAPPING);  // enable mapping for the first run
    cv::instr::resetTrace();
  }
#endif
...
void TestBase::stopTimer() {
  ...
  cv::instr::setFlags(
      cv::instr::getFlags() &
      ~cv::instr::FLAGS_MAPPING);  // disable mapping to decrease overhead for
                                   // +1 run
#endif
}
...
performance_metrics& TestBase::calcMetrics() {
  ...
  if (getCurrentPerformanceStrategy() == PERF_STRATEGY_BASE) {
    ...
    metrics.outliers =
        static_cast<int>(times.size() * param_max_outliers / 100);
    for (unsigned int i = 0; i < metrics.outliers; i++) --end;
  } else {
    assert(false);
  }
...
void TestBase::validateMetrics() {
  ...
  ASSERT_GE(m.samples, 1u)
      << "  No time measurements was performed.\nstartTimer() and stopTimer() "
         "commands are required for performance tests.";

  if (getCurrentPerformanceStrategy() == PERF_STRATEGY_BASE) {
    EXPECT_GE(m.samples, param_min_samples)
        << "  Only a few samples are collected.\nPlease increase number of "
           "iterations or/and time limit to get reliable performance "
           "measurements.";

    if (m.gstddev > DBL_EPSILON) {
      EXPECT_GT(/*m.gmean * */ 1.,
                /*m.gmean * */ 2 * sinh(m.gstddev * param_max_deviation))
          << "  Test results are not reliable ((mean-sigma,mean+sigma) "
             "deviation interval is greater than measured time interval).";
    }

    EXPECT_LE(
        m.outliers,
        std::max((unsigned int)cvCeil(m.samples * param_max_outliers / 100.),
                 1u))
    ...
    printf(
        "[ PERFSTAT ]    (samples = %d, mean = %.2f, median = %.2f, stddev = "
        "%.2f (%.1f%%))\n",
...
void TestBase::reportMetrics(bool toJUnitXML) {
  ...
  CV_TRACE_ARG_VALUE(median, "mean_ms",
                     (double)(m.mean * 1000.0f / metrics.frequency));
  CV_TRACE_ARG_VALUE(median, "median_ms",
                     (double)(m.median * 1000.0f / metrics.frequency));
  CV_TRACE_ARG_VALUE(stddev, "stddev_ms",
                     (double)(m.stddev * 1000.0f / metrics.frequency));
  CV_TRACE_ARG_VALUE(stddev_percents, "stddev_percents",
                     (double)(m.stddev / (double)m.mean * 100.0f));
  ...
  if (m.terminationReason == performance_metrics::TERM_SKIP_TEST) {
    ...
    if (cv::instr::useInstrumentation()) {
      ...
      RecordProperty(
          "total_ipp_weight",
          cv::format("%.1f", ((double)getImplTime(cv::instr::IMPL_IPP) * 100 /
                              (double)getTotalTime())));
      RecordProperty(
          "total_opencl_weight",
          cv::format("%.1f", ((double)getImplTime(cv::instr::IMPL_OPENCL) *
                              100 / (double)getTotalTime())));
      cv::instr::resetTrace();
    }
#endif
    ...
    switch (m.terminationReason) {
      ...
      case performance_metrics::TERM_INTERRUPT:
        LOGD(
            "termination reason:  aborted by the performance testing "
            "framework");
        break;
    ...
    if (nIters == (unsigned int)-1 ||
        m.terminationReason == performance_metrics::TERM_ITERATIONS)
    ...
    if (m.samples > 0) {
      ...
      LOGD("median    =%11.0f = %.2fms", m.median,
           m.median * 1e3 / m.frequency);
      LOGD("gmean     =%11.0f = %.2fms", m.gmean, m.gmean * 1e3 / m.frequency);
      LOGD("gstddev   =%11.8f = %.2fms for 97%% dispersion interval", m.gstddev,
           m.gmean * 2 * sinh(m.gstddev * 3) * 1e3 / m.frequency);
      LOGD("mean      =%11.0f = %.2fms", m.mean, m.mean * 1e3 / m.frequency);
      LOGD("stddev    =%11.0f = %.2fms", m.stddev,
           m.stddev * 1e3 / m.frequency);
    }
  }
}
...
#ifdef __ANDROID__
  if (param_affinity_mask) setCurrentThreadAffinityMask(param_affinity_mask);
#endif
...
void TestBase::TearDown() {
  if (metrics.terminationReason == performance_metrics::TERM_SKIP_TEST) {
    ...
    if (!HasFailure() && !verified)
      ADD_FAILURE() << "The test has no sanity checks. There should be at "
                       "least one check at the end of performance test.";
...
#ifdef ENABLE_INSTRUMENTATION
      if (cv::instr::useInstrumentation()) InstumentData::printTree();
#endif
      return;
    }
  }
  ...
  if (param_collect_impl) {
    ...
    if (implConf.ipp_mt) {
      if (implConf.icv) {
        printf("ICV_MT ");
        std::vector<cv::String> fun =
            implConf.GetCallsForImpl(CV_IMPL_IPP | CV_IMPL_MT);
        printf("(");
        for (int i = 0; i < fun.size(); i++) {
          printf("%s ", fun[i].c_str());
        }
        printf(") ");
      }
      if (implConf.ipp) {
      ...
      if (implConf.icv) {
        printf("ICV ");
        std::vector<cv::String> fun = implConf.GetCallsForImpl(CV_IMPL_IPP);
        printf("(");
        for (int i = 0; i < fun.size(); i++) {
          printf("%s ", fun[i].c_str());
        }
    ...
    if (implConf.ocl) {
      printf("OCL ");
      std::vector<cv::String> fun = implConf.GetCallsForImpl(CV_IMPL_OCL);
      printf("(");
      for (int i = 0; i < fun.size(); i++) {
        printf("%s ", fun[i].c_str());
      }
    ...
    if (!(implConf.ipp_mt || implConf.icv || implConf.ipp || implConf.ocl ||
          implConf.plain))
      printf("ERROR ");
    printf("\n");
    fflush(stdout);
...
std::string TestBase::getDataPath(const std::string& relativePath) {
  ...
  if (data_path_dir) {
    int len = (int)strlen(data_path_dir) - 1;
    if (len < 0) len = 0;
    path = (data_path_dir[0] == 0 ? std::string(".")
                                  : std::string(data_path_dir)) +
           (data_path_dir[len] == '/' || data_path_dir[len] == '\\'
                ? ""
                : path_separator);
...
void TestBase::RunPerfTestBody() {
  try {
#ifdef CV_COLLECT_IMPL_DATA
    if (param_collect_impl) implConf.Reset();
#endif
    this->PerfTestBody();
#ifdef CV_COLLECT_IMPL_DATA
    if (param_collect_impl) implConf.GetImpl();
#endif
    ...
    metrics.terminationReason = performance_metrics::TERM_EXCEPTION;
#ifdef HAVE_CUDA
    if (e.code == cv::Error::GpuApiCallError) cv::cuda::resetDevice();
#endif
    FAIL() << "Expected: PerfTestBody() doesn't throw an exception.\n  Actual: "
              "it throws cv::Exception:\n  "
           << e.what();
...
TestBase::_declareHelper& TestBase::_declareHelper::runs(
    unsigned int runsNumber) {
  test->runsPerIteration = runsNumber;
  return *this;
}

TestBase::_declareHelper& TestBase::_declareHelper::in(cv::InputOutputArray a1,
                                                       WarmUpType wtype) {
...
TestBase::_declareHelper& TestBase::_declareHelper::out(cv::InputOutputArray a1,
                                                        WarmUpType wtype) {
  if (!test->times.empty()) return *this;
...
TestBase::_declareHelper& TestBase::_declareHelper::strategy(
    enum PERF_STRATEGY s) {
  test->testStrategy = s;
  return *this;
}
...
void perf::sort(std::vector<cv::KeyPoint>& pts,
  ...
  for (int i = 0; i < desc.rows; ++i) idxs[i] = i;
...
*                                  ::perf::GpuPerf
\*****************************************************************************************/
bool perf::GpuPerf::targetDevice() { return param_impl == "cuda"; }
