...
template <>
inline void big_any_policy<flann_algorithm_t>::print(std::ostream& out,
                                                     void* const* src) {
  out << int(*reinterpret_cast<flann_algorithm_t const*>(*src));
}

template <>
inline void big_any_policy<cv::String>::print(std::ostream& out,
                                              void* const* src) {
...
struct any {
 ...
 public:
  ...
  /// Assignment operator. Template-based version above doesn't work as
  /// expected. We need regular assignment operator here.
  any& operator=(const any& x) { return assign(x); }
  ...
  bool empty() const { return policy->type() == typeid(anyimpl::empty_any); }
  ...
  const std::type_info& type() const { return policy->type(); }
