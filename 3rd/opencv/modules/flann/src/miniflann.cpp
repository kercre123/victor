...
IndexParams::IndexParams() { params = new ::cvflann::IndexParams(); }

template <typename T>
T getParam(const IndexParams& _p, const String& key,
           const T& defaultVal = T()) {
  ::cvflann::IndexParams& p = get_params(_p);
  ::cvflann::IndexParams::const_iterator it = p.find(key);
  if (it == p.end()) return defaultVal;
...
String IndexParams::getString(const String& key,
...
void IndexParams::getAll(std::vector<String>& names, std::vector<int>& types,
  ...
  for (; it != it_end; ++it) {
    ...
    try {
      cvflann::flann_algorithm_t val =
          it->second.cast<cvflann::flann_algorithm_t>();
...
CompositeIndexParams::CompositeIndexParams(int trees, int branching,
                                           int iterations,
...
AutotunedIndexParams::AutotunedIndexParams(float target_precision,
                                           float build_weight,
                                           float memory_weight,
...
HierarchicalClusteringIndexParams::HierarchicalClusteringIndexParams(
    int branching, flann_centers_init_t centers_init, int trees,
    int leaf_size) {
  ::cvflann::IndexParams& p = get_params(*this);
...
LshIndexParams::LshIndexParams(int table_number, int key_size,
...
void buildIndex_(void*& index, const Mat& data, const IndexParams& params,
  ...
  ::cvflann::Matrix<ElementType> dataset((ElementType*)data.data, data.rows,
                                         data.cols);
...
void buildIndex(void*& index, const Mat& data, const IndexParams& params,
                const Distance& dist = Distance()) {
...
Index::Index(InputArray _data, const IndexParams& params,
             flann_distance_t _distType) {
  index = 0;
  featureType = CV_32F;
  algo = FLANN_INDEX_LINEAR;
  distType = FLANN_DIST_L2;
...
void Index::build(InputArray _data, const IndexParams& params,
                  flann_distance_t _distType) {
  ...
  switch (distType) {
    ...
    case FLANN_DIST_HIST_INTERSECT:
      buildIndex< ::cvflann::HistIntersectionDistance<float> >(index, data,
                                                               params);
      break;
...
void runKnnSearch_(void* index, const Mat& query, Mat& indices, Mat& dists,
  ...
  CV_Assert(query.type() == type && indices.type() == CV_32S &&
            dists.type() == dtype);
  CV_Assert(query.isContinuous() && indices.isContinuous() &&
            dists.isContinuous());

  ::cvflann::Matrix<ElementType> _query((ElementType*)query.data, query.rows,
                                        query.cols);
  ::cvflann::Matrix<int> _indices(indices.ptr<int>(), indices.rows,
                                  indices.cols);
  ::cvflann::Matrix<DistanceType> _dists(dists.ptr<DistanceType>(), dists.rows,
                                         dists.cols);

  ((IndexType*)index)
      ->knnSearch(_query, _indices, _dists, knn,
...
void runKnnSearch(void* index, const Mat& query, Mat& indices, Mat& dists,
                  int knn, const SearchParams& params) {
  runKnnSearch_<Distance, ::cvflann::Index<Distance> >(index, query, indices,
                                                       dists, knn, params);
}
...
int runRadiusSearch_(void* index, const Mat& query, Mat& indices, Mat& dists,
  ...
  return ((IndexType*)index)
      ->radiusSearch(_query, _indices, _dists, saturate_cast<float>(radius),
                     (const ::cvflann::SearchParams&)get_params(params));
}
...
int runRadiusSearch(void* index, const Mat& query, Mat& indices, Mat& dists,
                    double radius, const SearchParams& params) {
  return runRadiusSearch_<Distance, ::cvflann::Index<Distance> >(
      index, query, indices, dists, radius, params);
}

static void createIndicesDists(OutputArray _indices, OutputArray _dists,
                               Mat& indices, Mat& dists, int rows, int minCols,
                               int maxCols, int dtype) {
  if (_indices.needed()) {
    indices = _indices.getMat();
    if (!indices.isContinuous() || indices.type() != CV_32S ||
        indices.rows != rows || indices.cols < minCols ||
        indices.cols > maxCols) {
      if (!indices.isContinuous()) _indices.release();
...
void Index::knnSearch(InputArray _query, OutputArray _indices,
  ...
  createIndicesDists(_indices, _dists, indices, dists, query.rows, knn, knn,
                     dtype);
  ...
  switch (distType) {
    case FLANN_DIST_HAMMING:
      ...
      break;
    case FLANN_DIST_L2:
      runKnnSearch< ::cvflann::L2<float> >(index, query, indices, dists, knn,
                                           params);
      break;
    case FLANN_DIST_L1:
      runKnnSearch< ::cvflann::L1<float> >(index, query, indices, dists, knn,
                                           params);
      break;
#if MINIFLANN_SUPPORT_EXOTIC_DISTANCE_TYPES
    case FLANN_DIST_MAX:
      runKnnSearch< ::cvflann::MaxDistance<float> >(index, query, indices,
                                                    dists, knn, params);
      break;
    case FLANN_DIST_HIST_INTERSECT:
      runKnnSearch< ::cvflann::HistIntersectionDistance<float> >(
          index, query, indices, dists, knn, params);
      break;
    case FLANN_DIST_HELLINGER:
      runKnnSearch< ::cvflann::HellingerDistance<float> >(index, query, indices,
                                                          dists, knn, params);
      break;
    case FLANN_DIST_CHI_SQUARE:
      runKnnSearch< ::cvflann::ChiSquareDistance<float> >(index, query, indices,
                                                          dists, knn, params);
      break;
    case FLANN_DIST_KL:
      runKnnSearch< ::cvflann::KL_Divergence<float> >(index, query, indices,
                                                      dists, knn, params);
      break;
#endif
    default:
...
int Index::radiusSearch(InputArray _query, OutputArray _indices,
  ...
  createIndicesDists(_indices, _dists, indices, dists, query.rows, maxResults,
                     INT_MAX, dtype);

  if (algo == FLANN_INDEX_LSH)
    CV_Error(Error::StsNotImplemented,
             "LSH index does not support radiusSearch operation");

  switch (distType) {
    case FLANN_DIST_HAMMING:
      return runRadiusSearch<HammingDistance>(index, query, indices, dists,
                                              radius, params);

    case FLANN_DIST_L2:
      return runRadiusSearch< ::cvflann::L2<float> >(index, query, indices,
                                                     dists, radius, params);
    case FLANN_DIST_L1:
      return runRadiusSearch< ::cvflann::L1<float> >(index, query, indices,
                                                     dists, radius, params);
#if MINIFLANN_SUPPORT_EXOTIC_DISTANCE_TYPES
    case FLANN_DIST_MAX:
      return runRadiusSearch< ::cvflann::MaxDistance<float> >(
          index, query, indices, dists, radius, params);
    case FLANN_DIST_HIST_INTERSECT:
      return runRadiusSearch< ::cvflann::HistIntersectionDistance<float> >(
          index, query, indices, dists, radius, params);
    case FLANN_DIST_HELLINGER:
      return runRadiusSearch< ::cvflann::HellingerDistance<float> >(
          index, query, indices, dists, radius, params);
    case FLANN_DIST_CHI_SQUARE:
      return runRadiusSearch< ::cvflann::ChiSquareDistance<float> >(
          index, query, indices, dists, radius, params);
    case FLANN_DIST_KL:
      return runRadiusSearch< ::cvflann::KL_Divergence<float> >(
          index, query, indices, dists, radius, params);
#endif
    default:
      CV_Error(Error::StsBadArg, "Unknown/unsupported distance type");
...
void saveIndex_(const Index* index0, const void* index, FILE* fout) {
  ...
  // so make sure we always write integers (which are 4-byte values in any
  // modern C compiler)
...
void Index::save(const String& filename) const {
  ...
  switch (distType) {
    ...
    case FLANN_DIST_MAX:
      ...
      break;
    case FLANN_DIST_HIST_INTERSECT:
      saveIndex< ::cvflann::HistIntersectionDistance<float> >(this, index,
                                                              fout);
      break;
    case FLANN_DIST_HELLINGER:
...
template <typename Distance, typename IndexType>
bool loadIndex_(Index* index0, void*& index, const Mat& data, FILE* fin,
                const Distance& dist = Distance()) {
  typedef typename Distance::ElementType ElementType;
...
template <typename Distance>
bool loadIndex(Index* index0, void*& index, const Mat& data, FILE* fin,
               const Distance& dist = Distance()) {
  return loadIndex_<Distance, ::cvflann::Index<Distance> >(index0, index, data,
                                                           fin, dist);
}
...
bool Index::load(InputArray _data, const String& filename) {
  ...
  featureType =
      header.data_type == FLANN_UINT8
          ? CV_8U
          : header.data_type == FLANN_INT8
                ? CV_8S
                : header.data_type == FLANN_UINT16
                      ? CV_16U
                      : header.data_type == FLANN_INT16
                            ? CV_16S
                            : header.data_type == FLANN_INT32
                                  ? CV_32S
                                  : header.data_type == FLANN_FLOAT32
                                        ? CV_32F
                                        : header.data_type == FLANN_FLOAT64
                                              ? CV_64F
                                              : -1;
  ...
  if ((int)header.rows != data.rows || (int)header.cols != data.cols ||
      featureType != data.type()) {
    fprintf(stderr,
            "Reading FLANN index error: the saved data size (%d, %d) or type "
            "(%d) is different from the passed one (%d, %d), %d\n",
            (int)header.rows, (int)header.cols, featureType, data.rows,
            data.cols, data.type());
    fclose(fin);
    return false;
  }
  ...
  if (!((distType == FLANN_DIST_HAMMING && featureType == CV_8U) ||
    ...
    fprintf(stderr,
            "Reading FLANN index error: unsupported feature type %d for the "
            "index type %d\n",
            featureType, algo);
    fclose(fin);
    return false;
