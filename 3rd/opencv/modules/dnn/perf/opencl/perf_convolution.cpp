...
typedef tuple<Size, InpShapeNumOut, GroupSize, StrideSize>
    ConvParam;  // kernel_size, inp shape, groups, stride
typedef TestBaseWithParam<ConvParam> ConvolutionPerfTest;

static inline MatShape blobShape(int count, int nplanes, int height,
                                 int width) {
  int data[] = {count, nplanes, height, width};
  return MatShape(data, data + 4);
}

OCL_PERF_TEST_P(ConvolutionPerfTest, perf,
                Combine(Values(Size(1, 1), Size(3, 3), Size(5, 5),
                               Size(11, 11)),
                        Values(make_pair(blobShape(1, 4, 224, 224), 64),
                               make_pair(blobShape(1, 64, 112, 122), 128),
                               make_pair(blobShape(1, 256, 28, 28), 512)),
                        GroupSize::all(), StrideSize::all())) {
  RNG rng(0);
  ...
  Ptr<Layer> layer =
      cv::dnn::LayerFactory::createLayerInstance("Convolution", lp);
  ...
  for (int i = 0; i < internals.size(); i++) {
    ...
    if (total(internals[i])) internalBlobs.back().create(internals[i], CV_32F);
  }
  ...
  declare.in(inpBlob2D, wgtBlob2D, WARMUP_RNG)
      .out(outBlob2D)
      .tbb_threads(cv::getNumThreads());

  // warmup
  layer->forward(inpBlobs, outBlobs, internalBlobs);

  TEST_CYCLE() { layer->forward(inpBlobs, outBlobs, internalBlobs); }
