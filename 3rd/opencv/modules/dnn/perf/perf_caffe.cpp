...
// sudo make install
//
// In case of problems with cublas_v2.h at
// include/caffe/util/device_alternate.hpp: add line #define CPU_ONLY before the
// first line #ifdef CPU_ONLY  // CPU-only Caffe.
...
static caffe::Net<float>* initNet(std::string proto, std::string weights) {
  ...
  caffe::Net<float>* net = new caffe::Net<float>(
      proto, caffe::TEST, caffe::Caffe::GetDefaultDevice());
#else
  ...
  Mat inputMat(input->height(), input->width(), CV_32FC3,
               (char*)input->cpu_data());
...
PERF_TEST(AlexNet_caffe, CaffePerfTest) {
  caffe::Net<float>* net =
      initNet("dnn/bvlc_alexnet.prototxt", "dnn/bvlc_alexnet.caffemodel");
...
PERF_TEST(GoogLeNet_caffe, CaffePerfTest) {
  caffe::Net<float>* net =
      initNet("dnn/bvlc_googlenet.prototxt", "dnn/bvlc_googlenet.caffemodel");
  TEST_CYCLE() net->Forward();
  SANITY_CHECK_NOTHING();
}
...
PERF_TEST(SqueezeNet_v1_1_caffe, CaffePerfTest) {
  caffe::Net<float>* net =
      initNet("dnn/squeezenet_v1.1.prototxt", "dnn/squeezenet_v1.1.caffemodel");
  TEST_CYCLE() net->Forward();
  SANITY_CHECK_NOTHING();
}
