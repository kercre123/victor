...
class DNNTestNetwork
    : public ::perf::TestBaseWithParam<tuple<DNNBackend, DNNTarget> > {
 public:
  dnn::Backend backend;
  dnn::Target target;
  ...
  void processNet(std::string weights, std::string proto,
                  std::string halide_scheduler, const Mat& input,
    ...
    if (!proto.empty()) proto = findDataFile(proto, false);
    if (backend == DNN_BACKEND_HALIDE) {
      if (halide_scheduler == "disabled")
        throw ::SkipTestException("Halide test is disabled");
      if (!halide_scheduler.empty())
        halide_scheduler =
            findDataFile(std::string("dnn/halide_scheduler_") +
                             (target == DNN_TARGET_OPENCL ? "opencl_" : "") +
                             halide_scheduler,
                         true);
    }
    if (framework == "caffe") {
    ...
    std::cout << "    Weights(parameters): " << divUp(weightsMemory, 1u << 20)
              << " Mb" << std::endl;
...
PERF_TEST_P_(DNNTestNetwork, ResNet50) {
  processNet("dnn/ResNet-50-model.caffemodel", "dnn/ResNet-50-deploy.prototxt",
             "resnet_50.yml", Mat(cv::Size(224, 224), CV_32FC3), "prob",
             "caffe");
}

PERF_TEST_P_(DNNTestNetwork, SqueezeNet_v1_1) {
  processNet("dnn/squeezenet_v1.1.caffemodel", "dnn/squeezenet_v1.1.prototxt",
             "squeezenet_v1_1.yml", Mat(cv::Size(227, 227), CV_32FC3), "prob",
             "caffe");
}

PERF_TEST_P_(DNNTestNetwork, Inception_5h) {
  processNet("dnn/tensorflow_inception_graph.pb", "", "inception_5h.yml",
...
PERF_TEST_P_(DNNTestNetwork, SSD) {
  processNet("dnn/VGG_ILSVRC2016_SSD_300x300_iter_440000.caffemodel",
             "dnn/ssd_vgg16.prototxt", "disabled",
...
PERF_TEST_P_(DNNTestNetwork, MobileNet_SSD_Caffe) {
  processNet("dnn/MobileNetSSD_deploy.caffemodel",
...
INSTANTIATE_TEST_CASE_P(/*nothing*/, DNNTestNetwork,
                        testing::Combine(::testing::Values(TEST_DNN_BACKEND),
                                         DNNTarget::all()));
