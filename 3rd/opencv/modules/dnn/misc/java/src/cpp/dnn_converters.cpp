...
std::vector<MatShape> List_to_vector_MatShape(JNIEnv* env, jobject list) {
  ...
  for (jint i = 0; i < len; i++) {
    jobject element =
        static_cast<jobject>(env->CallObjectMethod(list, m_get, i));
...
jobject vector_Ptr_Layer_to_List(JNIEnv* env,
  ...
  for (std::vector<cv::Ptr<cv::dnn::Layer> >::iterator it = vs.begin();
       it != vs.end(); ++it) {
...
std::vector<cv::Ptr<cv::dnn::Layer> > List_to_vector_Ptr_Layer(JNIEnv* env,
  ...
  for (jint i = 0; i < len; i++) {
    jobject element =
        static_cast<jobject>(env->CallObjectMethod(list, m_get, i));
    cv::Ptr<cv::dnn::Layer>* layer_ptr =
        (cv::Ptr<cv::dnn::Layer>*)GETNATIVEOBJ(env, jLayerClass, element);
