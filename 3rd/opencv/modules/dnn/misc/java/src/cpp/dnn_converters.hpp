...
#define LAYER(ENV)                                                       \
  static_cast<jclass>(ENV->NewGlobalRef(ENV->FindClass("org/opencv/dnn/" \
                                                       "Layer")))
...
jobject vector_Ptr_Layer_to_List(JNIEnv* env,
                                 std::vector<cv::Ptr<cv::dnn::Layer> >& vs);

std::vector<cv::Ptr<cv::dnn::Layer> > List_to_vector_Ptr_Layer(JNIEnv* env,
                                                               jobject list);
