...
TEST(Test_TensorFlow, read_inception) {
  ...
  {
    const string model =
        findDataFile("dnn/tensorflow_inception_graph.pb", false);
...
static void runTensorFlowNet(const std::string& prefix, bool hasText = false,
  ...
  if (memoryLoad) {
    ...
    string dataConfig;
    if (hasText) ASSERT_TRUE(readFileInMemory(netConfig, dataConfig));
...
TEST(Test_TensorFlow, eltwise_add_mul) { runTensorFlowNet("eltwise_add_mul"); }

TEST(Test_TensorFlow, pad_and_concat) { runTensorFlowNet("pad_and_concat"); }
...
TEST(Test_TensorFlow, deconvolution) { runTensorFlowNet("deconvolution"); }
...
TEST(Test_TensorFlow, MobileNet_SSD) {
  ...
  std::string netConfig =
      findDataFile("dnn/ssd_mobilenet_v1_coco.pbtxt", false);
  std::string imgPath = findDataFile("dnn/street.png", false);

  Mat inp;
  resize(imread(imgPath), inp, Size(300, 300));
  inp = blobFromImage(inp, 1.0f / 127.5, Size(), Scalar(127.5, 127.5, 127.5),
                      true);
  ...
  for (int i = 0; i < outNames.size(); ++i) {
    std::string path = findDataFile(
        "dnn/tensorflow/ssd_mobilenet_v1_coco." + outNames[i] + ".npy", false);
...
TEST(Test_TensorFlow, lstm) { runTensorFlowNet("lstm", true); }
