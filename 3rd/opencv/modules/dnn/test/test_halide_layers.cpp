...
TEST(Padding_Halide, Accuracy) {
  ...
  for (int t = 0; t < kNumRuns; ++t) {
    for (int i = 0; i < paddings.size(); ++i) paddings[i] = rand() % 5;

    LayerParams lp;
    lp.set("paddings",
           DictValue::arrayInt<int*>(&paddings[0], paddings.size()));
    lp.type = "Padding";
    lp.name = "testLayer";
...
// Convolution
////////////////////////////////////////////////////////////////////////////////
typedef TestWithParam<tuple<Vec3i, Size, Size, Size, Size, Size, bool> >
    Convolution;
TEST_P(Convolution, Accuracy) {
  ...
  Mat weights({outChannels, inChannels / group, kernel.height, kernel.width},
              CV_32F);
...
INSTANTIATE_TEST_CASE_P(Layer_Test_Halide, Convolution,
                        Combine(
...
// Deconvolution
////////////////////////////////////////////////////////////////////////////////
typedef TestWithParam<tuple<Vec3i, Size, Size, Size, Size, Vec4i, bool> >
    Deconvolution;
TEST_P(Deconvolution, Accuracy) {
  ...
  Mat weights({inChannels, outChannels / group, kernel.height, kernel.width},
              CV_32F);
  randu(weights, -1.0f, 1.0f);
...
INSTANTIATE_TEST_CASE_P(Layer_Test_Halide, Deconvolution,
                        Combine(
                            /*in channels, out channels, group*/
                            ...
                            /*dilation*/ Values(Size(1, 1), Size(2, 2)),
                            /*stride, adj. pad*/
                            Values(Vec4i(1, 1, 0, 0), Vec4i(2, 2, 1, 0),
                                   Vec4i(1, 2, 0, 1)),
                            /*has bias*/ Bool()));
...
TEST_P(BatchNorm, Accuracy) {
  ...
  lp.blobs.reserve(4);
  for (int i = 0; i < 3; ++i) lp.blobs.push_back(Mat({kNumChannels}, CV_32F));
  if (hasBias || hasWeights) lp.blobs.push_back(Mat({kNumChannels}, CV_32F));

  for (Mat& m : lp.blobs) randu(m, 0.0f, 1.0f);

  testInPlaceActivation(lp);
}

INSTANTIATE_TEST_CASE_P(Layer_Test_Halide, BatchNorm,
                        Combine(
                            /*has weights*/ Bool(),
                            /*has bias*/ Bool(),
...
INSTANTIATE_TEST_CASE_P(Layer_Test_Halide, ReLU,
                        Values(
...
INSTANTIATE_TEST_CASE_P(Layer_Test_Halide, NoParamActivation,
                        Values(
...
INSTANTIATE_TEST_CASE_P(Layer_Test_Halide, Power,
                        ...
                        Values(Vec3f(0.9f, 1.0f, 1.1f), Vec3f(0.9f, 1.1f, 1.0f),
                               ...
                               Vec3f(1.1f, 0.9f, 1.0f),
                               Vec3f(1.1f, 1.0f, 0.9f)));
...
TEST_P(Eltwise, Accuracy) {
  ...
  if (op == "sum" && weighted) {
    ...
    eltwiseParam.set("coeff",
                     DictValue::arrayReal<float*>(&coeff[0], coeff.size()));
  }
  eltwiseParam.type = "Eltwise";
  eltwiseParam.name = "testLayer";
  int eltwiseId =
      net.addLayer(eltwiseParam.name, eltwiseParam.type, eltwiseParam);
...
INSTANTIATE_TEST_CASE_P(Layer_Test_Halide, Eltwise,
                        Combine(
                            /*input size*/ Values(Vec3i(1, 4, 5),
                                                  Vec3i(2, 8, 6)),
