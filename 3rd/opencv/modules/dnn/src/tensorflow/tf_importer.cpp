...
// This values are used to indicate layer output's data layout where it's
// possible.
enum DataLayout { DATA_LAYOUT_NHWC, DATA_LAYOUT_NCHW, DATA_LAYOUT_UNKNOWN };
...
void blobShapeFromTensor(const tensorflow::TensorProto& tensor,
                         MatShape& shape) {
  shape.clear();
  if (tensor.has_tensor_shape()) {
    ...
    if (n) {
      ...
      for (i = 0; i < n; i++) shape[i] = (int)_shape.dim(i).size();
    } else
...
static Mat getTensorContent(const tensorflow::TensorProto& tensor) {
  ...
  switch (tensor.dtype()) {
    case tensorflow::DT_FLOAT: {
      if (!content.empty())
        return Mat(1, content.size() / sizeof(float), CV_32FC1,
                   (void*)content.c_str())
            .clone();
      else {
    ...
    case tensorflow::DT_DOUBLE: {
      if (!content.empty())
        return Mat(1, content.size() / sizeof(double), CV_64FC1,
                   (void*)content.c_str())
            .clone();
      else {
    ...
    case tensorflow::DT_INT32: {
      if (!content.empty())
        return Mat(1, content.size() / sizeof(int32_t), CV_32SC1,
                   (void*)content.c_str())
            .clone();
      else {
    ...
    case tensorflow::DT_HALF: {
      ...
      if (!content.empty()) {
        ...
        halfs = Mat(1, content.size() / kHalfSize, CV_16UC1,
                    (void*)content.c_str());
      } else {
...
void parseTensor(const tensorflow::TensorProto& tensor, Mat& dstBlob) {
  ...
  if (dims == 4) {
    int num = shape[0], channels = shape[1], height = shape[2],
        width = shape[3];
      ...
      for (int i_c = 0; i_c < shape[1]; i_c++) {
        for (int i_h = 0; i_h < shape[2]; i_h++) {
          for (int i_w = 0; i_w < shape[3]; i_w++) {
            int dst_i = channels * height * width * i_n + height * width * i_c +
                        width * i_h + i_w;
            int src_i = channels * height * width * i_n + i_c +
                        channels * width * i_h + channels * i_w;
    ...
    for (int i = 0; i < size; i++) dstData[i] = data[i];
  }
}
...
void printList(const tensorflow::AttrValue::ListValue& val) {
  std::cout << "(";
  for (int i = 0; i < val.i_size(); i++) std::cout << " " << val.i(i);
  std::cout << " )";
}
...
void printTensor(const tensorflow::TensorProto& tensor) {
  ...
  if (tensor.tensor_content().empty()) return;

  switch (tensor.dtype()) {
    case tensorflow::DT_FLOAT: {
      const float* data =
          reinterpret_cast<const float*>(tensor.tensor_content().c_str());
      int size = tensor.tensor_content().size() / sizeof(float);
      for (int i = 0; i < std::min(10, size); i++) std::cout << " " << data[i];
      if (size > 10) std::cout << " ... " << size - 10 << " more";
      break;
    }
    case tensorflow::DT_INT32: {
      const int* data =
          reinterpret_cast<const int*>(tensor.tensor_content().c_str());
...
const tensorflow::AttrValue& getLayerAttr(const tensorflow::NodeDef& layer,
...
void setStrides(LayerParams& layerParams, const tensorflow::NodeDef& layer) {
  if (hasLayerAttr(layer, "strides")) {
    ...
    if (val.list().i_size() != 4 || val.list().i(0) != 1 ||
        val.list().i(3) != 1)
...
StrIntVector getNextLayers(const tensorflow::GraphDef& net,
                           const String& layer_name, const String& type = "") {
  StrIntVector layers;
...
void ExcludeLayer(tensorflow::GraphDef& net, const int layer_index,
                  const int input_blob_index, bool remove_from_net = true) {
  ...
  if (remove_from_net) net.mutable_node()->DeleteSubrange(layer_index, 1);
}
...
class TFImporter {
 ...
 private:
  ...
  void connect(const std::map<String, int>& layers_name_id_map, Net& network,
               const Pin& outPin, const int input_layer_id,
               const int input_blob_id);
  void connectToAllBlobs(const std::map<String, int>& layer_id, Net& network,
                         const Pin& outPin, const int input_layer_id,
                         const int input_blobs_count);
  const tensorflow::TensorProto& getConstBlob(
      const tensorflow::NodeDef& layer, std::map<String, int> const_layers,
  ...
  // Optional text definition of TensorFlow graph. More flexible than binary
  // format and may be used to build the network using binary format only as a
  // weights storage. This approach is similar to Caffe's `.prorotxt` and
  // `.caffemodel`.
  tensorflow::GraphDef netTxt;
};

TFImporter::TFImporter(const char* model, const char* config) {
  if (model && model[0]) ReadTFNetParamsFromBinaryFileOrDie(model, &netBin);
  if (config && config[0]) ReadTFNetParamsFromTextFileOrDie(config, &netTxt);
}
...
void TFImporter::kernelFromTensor(const tensorflow::TensorProto& tensor,
                                  Mat& dstBlob) {
  MatShape shape;
  blobShapeFromTensor(tensor, shape);
  int dims = (int)shape.size();
    ...
    for (int i_ic = 0; i_ic < input_c; i_ic++) {
      for (int i_h = 0; i_h < height; i_h++) {
        for (int i_w = 0; i_w < width; i_w++) {
          int dst_i = input_c * height * width * i_oc + height * width * i_ic +
                      width * i_h + i_w;
          int src_i = out_c * input_c * width * i_h + out_c * input_c * i_w +
                      out_c * i_ic + i_oc;
          CV_Assert(dst_i < total);
          CV_Assert(src_i < total);
          dstData[dst_i] = data[src_i];
...
void TFImporter::connect(const std::map<String, int>& layers_name_id_map,
                         Net& network, const Pin& outPin,
                         const int input_layer_id, const int input_blob_id) {
  std::map<String, int>::const_iterator it =
      layers_name_id_map.find(outPin.name);
...
void TFImporter::connectToAllBlobs(const std::map<String, int>& layer_id,
                                   Net& network, const Pin& outPin,
                                   const int input_layer_id,
                                   const int input_blobs_count) {
  for (int input_blob_id = 0; input_blob_id < input_blobs_count;
       input_blob_id++)
    connect(layer_id, network, outPin, input_layer_id, input_blob_id);
}

const tensorflow::TensorProto& TFImporter::getConstBlob(
    const tensorflow::NodeDef& layer, std::map<String, int> const_layers,
  ...
  if (nodeIdx < netBin.node_size() &&
      netBin.node(nodeIdx).name() == kernel_inp.name) {
...
static void addConstNodes(tensorflow::GraphDef& net,
                          std::map<String, int>& const_layers,
  ...
  for (int li = 0; li < net.node_size(); li++) {
    ...
    if (type == "Dequantize") {
      ...
      //   attr { key: "mode" value { s: "MIN_FIRST" } } (quantization
      //   technique)
      ...
      CV_Assert(qMin.total() == 1, qMin.type() == CV_32FC1, qMax.total() == 1,
                qMax.type() == CV_32FC1);
      ...
      tensor->set_tensor_content(content.data,
                                 content.total() * content.elemSize1());
...
// this layer's output has this data layout too. Returns DATA_LAYOUT_UNKNOWN
// otherwise.
static int predictOutputDataLayout(const tensorflow::NodeDef& layer,
...
void TFImporter::populateNet(Net dstNet) {
  ...
  for (int li = 0; li < layersSize; li++) {
    ...
    if (layers_to_ignore.find(name) != layers_to_ignore.end()) continue;
    ...
    if (type == "Conv2D" || type == "SpaceToBatchND" ||
      ...
      if (type == "SpaceToBatchND") {
        ...
        CV_Assert(dilation.size() == 2 &&
                  dilation.get<int>(0) == dilation.get<int>(1));
      ...
      if (hasLayerAttr(layer, "data_format")) {
        ...
        if (format == "NHWC")
          ...
          CV_Error(Error::StsParseError,
                   "Unknown data_format value: " + format);
      } else
        data_layouts[name] = DATA_LAYOUT_NHWC;
      ...
      blobFromTensor(getConstBlob(layer, value_id, -1, &kernel_blob_index),
                     layerParams.blobs[0]);

      if (kernel_blob_index == 1) {  // In this case output is computed by x*W
                                     // formula - W should be transposed
      ...
      if (newShape.size() != 4 &&
          data_layouts[layer.input(0)] == DATA_LAYOUT_NHWC) {
        LayerParams permLP;
      ...
      if (perm.total() == 4) {
        ...
        if (data_layouts[layer.input(0)] == DATA_LAYOUT_NHWC) {
          if (permData[0] == 0 && permData[1] == 3 && permData[2] == 1 &&
              permData[3] == 2) {
            // in TensorFlow: NHWC->NCHW
            // in OpenCV: NCHW->NCHW
            data_layouts[name] = DATA_LAYOUT_NCHW;
          } else if (permData[0] == 0 && permData[1] == 1 && permData[2] == 2 &&
                     permData[3] == 3) {
          ...
          if (permData[0] == 0 && permData[1] == 2 && permData[2] == 3 &&
              permData[3] == 1) {
        ...
        layerParams.set("order",
                        DictValue::arrayInt<int*>(permData, perm.total()));
      ...
      connectToAllBlobs(layer_id, dstNet, parsePin(layer.input(0)), id,
                        layer.input_size());
      ...
      // TODO: determing axis index remapping by input dimensions order of input
      // blob
      // TODO: slicing input may be Const op
      // TODO: slicing kernels for convolutions - in current implenmentation it
      // is impossible
      ...
      if (haveConst) {
        ...
        } else  // is a vector
        {
          ...
          if (!next_layers.empty()) {
            ...
            int weights_layer_index = next_layers[0].second;
            blobFromTensor(
                getConstBlob(net.node(weights_layer_index), value_id),
      ...
      layerParams.set("paddings", DictValue::arrayInt<int*>((int*)paddings.data,
                                                            paddings.total()));
      ...
      if (hasLayerAttr(layer, "align_corners"))
        layerParams.set("align_corners",
                        getLayerAttr(layer, "align_corners").b());
      ...
      if (hasLayerAttr(layer, "variance")) {
        Mat variance =
            getTensorContent(getLayerAttr(layer, "variance").tensor());
        layerParams.set("variance",
                        DictValue::arrayReal<float*>((float*)variance.data,
                                                     variance.total()));
      }
      if (hasLayerAttr(layer, "aspect_ratio")) {
        Mat aspectRatios =
            getTensorContent(getLayerAttr(layer, "aspect_ratio").tensor());
        layerParams.set("aspect_ratio",
                        DictValue::arrayReal<float*>((float*)aspectRatios.data,
                                                     aspectRatios.total()));
      }
      if (hasLayerAttr(layer, "scales")) {
        Mat scales = getTensorContent(getLayerAttr(layer, "scales").tensor());
        layerParams.set("scales", DictValue::arrayReal<float*>(
                                      (float*)scales.data, scales.total()));
      }
      ...
      if (hasLayerAttr(layer, "share_location"))
        layerParams.set("share_location",
                        getLayerAttr(layer, "share_location").b());
      if (hasLayerAttr(layer, "background_label_id"))
        layerParams.set("background_label_id",
                        getLayerAttr(layer, "background_label_id").i());
      if (hasLayerAttr(layer, "nms_threshold"))
        layerParams.set("nms_threshold",
                        getLayerAttr(layer, "nms_threshold").f());
      ...
      if (hasLayerAttr(layer, "confidence_threshold"))
        layerParams.set("confidence_threshold",
                        getLayerAttr(layer, "confidence_threshold").f());
      if (hasLayerAttr(layer, "loc_pred_transposed"))
        layerParams.set("loc_pred_transposed",
      ...
      CV_Error_(Error::StsError,
                ("Unknown layer type %s in op %s", type.c_str(), name.c_str()));
    }
  }
}
