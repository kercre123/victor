...
static size_t fread__(void *ptr, size_t size, size_t nitems, FILE *stream) {
  ...
  while (!feof(stream) && !ferror(stream) && (nread < nitems))
    nread +=
        fread((char *)ptr + nread * size, size,
              std::min<size_t>(2147483648UL / size, nitems - nread), stream);
  return nread;
}
#else
...
static void THDiskFile_reverseMemory(void *dst, const void *src, long blockSize,
                                     long numBlocks) {
  if (blockSize != 1) {
...
/*READ_WRITE_METHODS(long, Long,
                   int ret = fscanf(dfself->handle, "%ld", &data[i]); if(ret <=
   0) break; else nread++, int ret = fprintf(dfself->handle, "%ld", data[i]);
   if(ret <= 0) break; else nwrite++)*/
...
static long THDiskFile_readLong(THFile *self, int64 *data, long n) {
  ...
  if (dfself->file.isBinary) {
    ...
    } else /* if(dfself->longSize == 8) */
    {
      ...
      if (!buffer) THError("can not allocate buffer");
...
static long THDiskFile_readString(THFile *self, const char *format,
                                  char **str_) {
  THDiskFile *dfself = (THDiskFile *)(self);
  THArgCheck(dfself->handle != NULL, 1, "attempt to use a closed file");
  THArgCheck(dfself->file.isReadable, 1,
...
#define TBRS_BSZ 1024L
  ...
  if (format[1] == 'a') {
    ...
    for (;;) {
      ...
      if (p == NULL) THError("read error: failed to allocate buffer");
...
THFile *THDiskFile_new(const std::string &name, const char *mode, int isQuiet) {
...
#ifdef _MSC_VER
  if (fopen_s(&handle, name.c_str(), "rb") != 0) handle = NULL;
#else
  ...
  if (!handle) {
    if (isQuiet)
      ...
      THError("cannot open <%s> in mode %c%c", name.c_str(),
              (isReadable ? 'r' : ' '), (isWritable ? 'w' : ' '));
  }

  self = (THDiskFile *)THAlloc(sizeof(THDiskFile));
  if (!self) THError("cannot allocate memory for self");
