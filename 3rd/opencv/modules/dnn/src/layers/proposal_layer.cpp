 ...
 public:
  ProposalLayerImpl(const LayerParams& params) {
...

    {
      ...
      lp.set("height",
             DictValue::arrayReal<float*>(&heights[0], heights.size()));
    ...
    }
    {
      ...
      lp.set("background_label_id", 1);  // We won't pass background scores so
                                         // set it out of range [0, num_classes)
  ...
  bool getMemoryShapes(const std::vector<MatShape>& inputs,
    ...
    priorBoxLayer->getMemoryShapes(layerInputs, 1, layerOutputs,
                                   layerInternals);
    ...
    scoresPermute->getMemoryShapes(layerInputs, 1, layerOutputs,
                                   layerInternals);
    CV_Assert(layerOutputs.size() == 1);
    CV_Assert(layerInternals.empty());
    internals.push_back(layerOutputs[0]);
    ...
    deltasPermute->getMemoryShapes(layerInputs, 1, layerOutputs,
                                   layerInternals);
    CV_Assert(layerOutputs.size() == 1);
    CV_Assert(layerInternals.empty());
    internals.push_back(layerOutputs[0]);
  ...
  void finalize(const std::vector<Mat*>& inputs, std::vector<Mat>& outputs) {
    ...
    layerOutputs.assign(1, Mat(shape(bboxDeltas->size[0], bboxDeltas->size[2],
                                     bboxDeltas->size[3], bboxDeltas->size[1]),
                               CV_32FC1));
  ...
  void forward(std::vector<Mat*>& inputs, std::vector<Mat>& outputs,
    ...
    fakeImageBlob.create(shape(1, 1, imInfo.at<float>(0), imInfo.at<float>(1)),
                         CV_8UC1);
