...
class PaddingLayerImpl : public PaddingLayer {
 public:
  ...
  bool getMemoryShapes(const std::vector<MatShape> &inputs,
    ...
    CV_Assert(inputDims == -1 || inpShape.size() == inputDims ||
              inpShape.size() > paddings.size());

    outputs.resize(1, inpShape);
    int offset = (inputDims == -1 ? 0 : (inpShape.size() > inputDims ? 1 : 0));
    for (int i = 0; i < paddings.size(); ++i) {
      outputs[0][offset + i] =
          inpShape[offset + i] + paddings[i].first + paddings[i].second;
    }
    return false;
  }
  ...
  void forward(std::vector<Mat *> &inputs, std::vector<Mat> &outputs,
    ...
    if (paddingType == "constant") {
      ...
      if (inputs[0]->size[0] != outputs[0].size[0] ||
          inputs[0]->size[1] != outputs[0].size[1])
        CV_Error(Error::StsNotImplemented,
                 "Only spatial reflection padding is supported.");
      ...
      CV_Assert(padTop < inpHeight, padBottom < inpHeight, padLeft < inpWidth,
                padRight < inpWidth);

      for (size_t n = 0; n < inputs[0]->size[0]; ++n) {
        for (size_t ch = 0; ch < inputs[0]->size[1]; ++ch) {
          copyMakeBorder(getPlane(*inputs[0], n, ch),
                         getPlane(outputs[0], n, ch), padTop, padBottom,
                         padLeft, padRight, BORDER_REFLECT_101);
        }
      }
    } else
      CV_Error(Error::StsNotImplemented,
               "Unknown padding type: " + paddingType);
  }
  ...
  virtual Ptr<BackendNode> initHalide(
    ...
    Halide::Func padded = Halide::BoundaryConditions::constant_exterior(
        inputBuffer, paddingValue);
