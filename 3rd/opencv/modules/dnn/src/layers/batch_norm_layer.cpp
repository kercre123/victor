...
class BatchNormLayerImpl : public BatchNormLayer {
 public:
  ...
  BatchNormLayerImpl(const LayerParams& params) {
    ...
    CV_Assert(blobs[1].total() == n && blobs[0].isContinuous() &&
              blobs[1].isContinuous() && blobs[0].type() == CV_32F &&
              blobs[1].type() == CV_32F);
    ...
    if (!hasWeights && !hasBias) {
      ...
      if (varMeanScale != 0) varMeanScale = 1 / varMeanScale;
    }
    ...
    if (hasWeights) {
      ...
      CV_Assert(w.isContinuous() && w.type() == CV_32F &&
                w.total() == (size_t)n);
    }
    ...
    if (hasBias) {
      ...
      CV_Assert(b.isContinuous() && b.type() == CV_32F &&
                b.total() == (size_t)n);
    }
    ...
    const float* weightsData =
        hasWeights ? blobs[weightsBlobIndex].ptr<float>() : 0;
    ...
    for (size_t i = 0; i < n; ++i) {
      float w = (hasWeights ? weightsData[i] : 1.0f) /
                sqrt(stdData[i] * varMeanScale + epsilon);
      dstWeightsData[i] = w;
      dstBiasData[i] =
          (hasBias ? biasData[i] : 0.0f) - w * meanData[i] * varMeanScale;
    }
  }
