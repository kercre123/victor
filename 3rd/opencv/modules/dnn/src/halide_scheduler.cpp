...
static void applyComputeRoot(const FileNode& directive, Halide::Func& func) {
  ...
  if (compute_root) func.compute_root();
}
  ...
  for (const auto& directive : directives) {
    if (directive.name() == "split")
      ...
      CV_Error(
          ...
          "Scheduling directive " + directive.name() + " is not implemented.");
  }
}
...
HalideScheduler::HalideScheduler(const std::string& configFile) {
  if (!configFile.empty()) fs = FileStorage(configFile, FileStorage::READ);
}
...
bool HalideScheduler::process(Ptr<BackendNode>& node) {
  ...
  if (scheduleNode.empty())
    CV_Error(cv::Error::StsParseError,
             "Scheduling file should has scheduling node");
  ...
  std::vector<Halide::Func>& funcs =
      node.dynamicCast<HalideBackendNode>()->funcs;
