...
std::vector<Halide::Buffer<> > halideBuffers(
...
class HalideBackendNode : public BackendNode {
 public:
  ...
  // root layer but replace top by fused one (i.e. conv+padding to
  // relu+padding).
  HalideBackendNode(const Ptr<HalideBackendNode>& base,
...
// Compile Halide pipeline to specific target. Use outputs to set bounds of
// functions.
void compileHalide(const std::vector<Mat>& outputs, Ptr<BackendNode>& node,
                   int targetId);
