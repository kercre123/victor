...
PERF_TEST_P(FeaturesFinderVec, ParallelFeaturesFinder, NUMBER_IMAGES) {
  ...
  TEST_CYCLE() { (*featuresFinder)(imgs, features); }
...
PERF_TEST_P(match, bestOf2Nearest, TEST_DETECTORS) {
  ...
  float scale1 =
      (float)std::min(1.0, sqrt(WORK_MEGAPIX * 1e6 / img1_full.total()));
  float scale2 =
      (float)std::min(1.0, sqrt(WORK_MEGAPIX * 1e6 / img2_full.total()));
...
PERF_TEST_P(matchVector, bestOf2NearestVectorFeatures,
            testing::Combine(TEST_DETECTORS, testing::Values(2, 4, 8))) {
  Mat img1, img1_full = imread(getDataPath("stitching/boat1.jpg"));
  Mat img2, img2_full = imread(getDataPath("stitching/boat2.jpg"));
  float scale1 =
      (float)std::min(1.0, sqrt(WORK_MEGAPIX * 1e6 / img1_full.total()));
  ...
  for (size_t i = 0; i < pairwise_matches.size(); ++i) {
    if (pairwise_matches[i].src_img_idx < 0) continue;
...
PERF_TEST_P(match, affineBestOf2Nearest, TEST_DETECTORS) {
  ...
  if (GetParam() == "surf") {
    finder = makePtr<detail::SurfFeaturesFinder>();
    matcher = makePtr<detail::AffineBestOf2NearestMatcher>(
        false, false, SURF_MATCH_CONFIDENCE);
  } else if (GetParam() == "orb") {
    finder = makePtr<detail::OrbFeaturesFinder>();
    matcher = makePtr<detail::AffineBestOf2NearestMatcher>(
        false, false, ORB_MATCH_CONFIDENCE);
  } else {
    FAIL() << "Unknown 2D features type: " << GetParam();
  }
  ...
  // last row should be precisely (0, 0, 1) as it is just added for
...
PERF_TEST_P(matchVector, affineBestOf2NearestVectorFeatures,
            testing::Combine(TEST_DETECTORS, testing::Values(2, 4, 8))) {
  Mat img1, img1_full = imread(getDataPath("stitching/s1.jpg"));
  Mat img2, img2_full = imread(getDataPath("stitching/s2.jpg"));
  float scale1 =
