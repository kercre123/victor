...
OCL_PERF_TEST(Photo, DenoisingGrayscale) {
  Mat _original =
      imread(getDataPath("cv/denoising/lena_noised_gaussian_sigma=10.png"),
             IMREAD_GRAYSCALE);
...
OCL_PERF_TEST(Photo, DISABLED_DenoisingGrayscaleMulti) {
  ...
  for (int i = 0; i < imgs_count; i++) {
    string original_path =
        format("cv/denoising/lena_noised_gaussian_sigma=20_multi_%d.png", i);
  ...
  OCL_TEST_CYCLE()
  cv::fastNlMeansDenoisingMulti(original, result, imgs_count / 2, imgs_count,
                                15);
...
OCL_PERF_TEST(Photo, DISABLED_DenoisingColoredMulti) {
  ...
  OCL_TEST_CYCLE()
  cv::fastNlMeansDenoisingColoredMulti(original, result, imgs_count / 2,
                                       imgs_count, 10, 15);
