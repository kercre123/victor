...
PARAM_TEST_CASE(FastNlMeansDenoisingTestBase, Channels, int, bool, bool) {
  ...
  virtual void SetUp() {
    ...
    h.resize(cn);
    for (int i = 0; i < cn; i++) h[i] = 3.0f + 0.5f * i;
  }
...
OCL_TEST_P(FastNlMeansDenoising, Mat) {
  for (int j = 0; j < test_loop_times; j++) {
    ...
    OCL_OFF(cv::fastNlMeansDenoising(
        src_roi, dst_roi, std::vector<float>(1, h[0]), templateWindowSize,
        searchWindowSize, normType));
    OCL_ON(cv::fastNlMeansDenoising(
        usrc_roi, udst_roi, std::vector<float>(1, h[0]), templateWindowSize,
        searchWindowSize, normType));
...
OCL_TEST_P(FastNlMeansDenoising_hsep, Mat) {
  for (int j = 0; j < test_loop_times; j++) {
    ...
    OCL_OFF(cv::fastNlMeansDenoising(src_roi, dst_roi, h, templateWindowSize,
                                     searchWindowSize, normType));
    OCL_ON(cv::fastNlMeansDenoising(usrc_roi, udst_roi, h, templateWindowSize,
                                    searchWindowSize, normType));
...
OCL_TEST_P(FastNlMeansDenoisingColored, Mat) {
  for (int j = 0; j < test_loop_times; j++) {
    ...
    OCL_OFF(cv::fastNlMeansDenoisingColored(
        src_roi, dst_roi, h[0], h[0], templateWindowSize, searchWindowSize));
    OCL_ON(cv::fastNlMeansDenoisingColored(
...
OCL_INSTANTIATE_TEST_CASE_P(Photo, FastNlMeansDenoising,
                            Combine(Values(1, 2, 3, 4),
                                    Values((int)NORM_L2, (int)NORM_L1), Bool(),
                                    Values(true)));
...
OCL_INSTANTIATE_TEST_CASE_P(Photo, FastNlMeansDenoisingColored,
                            Combine(Values(3, 4), Values((int)NORM_L2), Bool(),
                                    Values(false)));
