...
PERF_TEST_P(Size_MatType, pyrDown,
            testing::Combine(
                testing::Values(sz1080p, sz720p, szVGA, szQVGA, szODD),
                testing::Values(CV_8UC1, CV_8UC3, CV_8UC4, CV_16SC1, CV_16SC3,
                                CV_16SC4, CV_32FC1, CV_32FC3, CV_32FC4))) {
  Size sz = get<0>(GetParam());
  int matType = get<1>(GetParam());
  const double eps = CV_MAT_DEPTH(matType) <= CV_32S ? 1 : 1e-5;
  perf::ERROR_TYPE error_type =
      CV_MAT_DEPTH(matType) <= CV_32S ? ERROR_ABSOLUTE : ERROR_RELATIVE;
...
PERF_TEST_P(Size_MatType, pyrDown_ovx,
            testing::Combine(
                testing::Values(sz1080p, sz720p, szVGA, szQVGA, szODD),
                testing::Values(CV_8UC1, CV_8UC3, CV_8UC4, CV_16SC1, CV_16SC3,
                                CV_16SC4, CV_32FC1, CV_32FC3, CV_32FC4))) {
...
PERF_TEST_P(Size_MatType, pyrUp,
            testing::Combine(testing::Values(sz720p, szVGA, szQVGA, szODD),
...
PERF_TEST_P(Size_MatType, buildPyramid,
            testing::Combine(testing::Values(sz1080p, sz720p, szVGA, szQVGA,
                                             szODD),
                             testing::Values(CV_8UC1, CV_8UC3, CV_8UC4,
                                             CV_32FC1, CV_32FC3, CV_32FC4))) {
  Size sz = get<0>(GetParam());
