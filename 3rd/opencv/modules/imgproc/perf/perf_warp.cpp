...
typedef TestBaseWithParam<tr1::tuple<Size, InterType, BorderMode> >
    TestWarpAffine;
typedef TestBaseWithParam<tr1::tuple<Size, InterType, BorderMode> >
    TestWarpPerspective;
typedef TestBaseWithParam<tr1::tuple<Size, InterType, BorderMode, MatType> >
    TestWarpPerspectiveNear_t;
typedef TestBaseWithParam<
    tr1::tuple<MatType, Size, InterType, BorderMode, RemapMode> >
    TestRemap;

void update_map(const Mat& src, Mat& map_x, Mat& map_y, const int remapMode);

PERF_TEST_P(TestWarpAffine, WarpAffine,
            Combine(Values(szVGA, sz720p, sz1080p), InterType::all(),
                    BorderMode::all())) {
  ...
  Mat warpMat =
      getRotationMatrix2D(Point2f(src.cols / 2.f, src.rows / 2.f), 30., 2.2);
  declare.in(src).out(dst);

  TEST_CYCLE()
  warpAffine(src, dst, warpMat, sz, interType, borderMode, borderColor);
...
PERF_TEST_P(TestWarpPerspective, WarpPerspective,
  ...
  TEST_CYCLE()
  warpPerspective(src, dst, warpMat, sz, interType, borderMode, borderColor);
...
PERF_TEST_P(TestWarpPerspectiveNear_t, WarpPerspectiveNear,
            Combine(Values(Size(640, 480), Size(1920, 1080), Size(2592, 1944)),
                    InterType::all(), BorderMode::all(),
  ...
  Mat srcVertices = (Mat_<Vec2f>(1, 4) << Vec2f(0, 0),
                     ...
                     Vec2f(static_cast<float>(size.width - 1),
  ...
  Mat dstVertices = (Mat_<Vec2f>(1, 4) << Vec2f(0, static_cast<float>(shift)),
                     ...
                     Vec2f(static_cast<float>(size.width - shift),
                           static_cast<float>(size.height - shift)),
                     Vec2f(static_cast<float>(shift / 2),
                           static_cast<float>(size.height - 1)));
  ...
  TEST_CYCLE() {
    warpPerspective(src, dst, warpMat, size, interType, borderMode,
                    borderColor);
  }
...
PERF_TEST_P(TestRemap, remap,
            Combine(Values(TYPICAL_MAT_TYPES), Values(szVGA, sz720p, sz1080p),
                    InterType::all(), BorderMode::all(), RemapMode::all())) {
