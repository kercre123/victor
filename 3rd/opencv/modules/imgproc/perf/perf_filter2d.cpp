...
PERF_TEST_P(TestFilter2d, Filter2d,
            Combine(Values(Size(320, 240), sz1080p), Values(3, 5),
                    BorderMode::all())) {
  Size sz;
...
PERF_TEST_P(TestFilter2d, Filter2d_ovx,
  ...
  TEST_CYCLE()
  filter2D(src, dst, CV_16SC1, kernel, Point(kSize / 2, kSize / 2), 0.,
           borderMode);
...
PERF_TEST_P(Image_KernelSize, GaborFilter2d,
            Combine(Values("stitching/a1.png", "cv/shared/pic5.png"),
                    Values(16, 32, 64))) {
  ...
  double gamma = 0.5;
  Mat gaborKernel =
      getGaborKernel(Size(kernelSize, kernelSize), sigma, theta, lambda, gamma);
  Mat filteredImage;

  declare.in(sourceImage);

  TEST_CYCLE() { filter2D(sourceImage, filteredImage, CV_32F, gaborKernel); }
