...
typedef TestBaseWithParam<tr1::tuple<Size, MatType, MatType, InterType> >
    TestRemap;

PERF_TEST_P(TestRemap, Remap,
            Combine(Values(szVGA, sz1080p),
                    Values(CV_16UC1, CV_16SC1, CV_32FC1),
                    Values(CV_16SC2, CV_32FC1, CV_32FC2), InterType::all())) {
  Size sz;
  ...
  for (int j = 0; j < map1.rows; ++j)
    for (int i = 0; i < map1.cols; ++i) switch (map1_type) {
        case CV_32FC1:
