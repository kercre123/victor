...
double testUtil::performTest(const cv::videostab::MotionModel model, int size) {
  cv::Ptr<cv::videostab::MotionEstimatorRansacL2> estimator =
      cv::makePtr<cv::videostab::MotionEstimatorRansacL2>(model);

  estimator->setRansacParams(cv::videostab::RansacParams(
      size, 3.f * testUtil::sigma /*3 sigma rule*/, 0.5f, 0.5f));
  ...
  for (int attempt = 0; attempt < testUtil::testRun; attempt++) {
    ...
    bool isOK = false;
    const cv::Mat estTransform =
        estimator->estimate(src.reshape(2), dst.reshape(2), &isOK);
...
TEST(Regression, MM_TRANSLATION_AND_SCALE) {
  EXPECT_LT(testUtil::performTest(cv::videostab::MM_TRANSLATION_AND_SCALE, 3),
            7.f);
}
