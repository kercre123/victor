...
void solveQuartic(const double *factors, double *realRoots) {
  ...
  double r4 = (256 * a0 * a4_3 - 3 * (a3_2 * a3_2) - 64 * a1 * a3 * a4_2 +
               16 * a2a4 * a3_2) /
              (256 * (a4_3 * a4));
  ...
  complex<double> sqrt_2m =
      sqrt(static_cast<complex<double> >(-2 * p4 / 3 + t));
  double B_4A = -a3 / (4 * a4);
...
void polishQuarticRoots(const double *coeffs, double *roots) {
  ...
  for (int i = 0; i < iterations; ++i) {
    for (int j = 0; j < 4; ++j) {
      double error =
          (((coeffs[0] * roots[j] + coeffs[1]) * roots[j] + coeffs[2]) *
               roots[j] +
           coeffs[3]) *
              roots[j] +
          coeffs[4];
      double derivative =
          ((4 * coeffs[0] * roots[j] + 3 * coeffs[1]) * roots[j] +
           2 * coeffs[2]) *
              roots[j] +
          coeffs[3];
...
int ap3p::computePoses(const double featureVectors[3][3],
                       ...
                       double solutionsR[4][3][3], double solutionsT[4][3]) {
  // world point vectors
...
bool ap3p::solve(cv::Mat &R, cv::Mat &tvec, const cv::Mat &opoints,
...
int ap3p::solve(std::vector<cv::Mat> &Rs, std::vector<cv::Mat> &tvecs,
...
bool ap3p::solve(double R[3][3], double t[3], double mu0, double mv0, double X0,
...
int ap3p::solve(double R[4][3][3], double t[4][3], double mu0, double mv0,
  ...
  double featureVectors[3][3] = {
      {mu0, mu1, mu2}, {mv0, mv1, mv2}, {mk0, mk1, mk2}};
