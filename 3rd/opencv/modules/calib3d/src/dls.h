...
class dls {
 ...
 private:
  ...
  void compute_eigenvec(const cv::Mat& Mtilde, cv::Mat& eigenval_real,
                        cv::Mat& eigenval_imag, cv::Mat& eigenvec_real,
                        cv::Mat& eigenvec_imag);
  void fill_coeff(const cv::Mat* D);

  // useful functions
  cv::Mat cayley_LS_M(const std::vector<double>& a,
...
class EigenvalueDecomposition {
 ...
 public:
  ...
  EigenvalueDecomposition(InputArray src) { compute(src); }
