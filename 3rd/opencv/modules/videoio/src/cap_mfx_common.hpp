...
inline std::ostream &operator<<(std::ostream &out, const mfxInfoMFX &info) {
  out << "InfoMFX:" << std::endl
      << "| Codec: " << FourCC(info.CodecId) << " / " << info.CodecProfile
      << " / " << info.CodecLevel << std::endl
...
inline std::ostream &operator<<(std::ostream &out, const mfxFrameInfo &info) {
  out << "FrameInfo: " << std::endl
      ...
      << "| ROI: "
      << "(" << info.CropX << ";" << info.CropY << ") " << info.CropW << "x"
      << info.CropH << std::endl
      << "| BitDepth(L/C): " << info.BitDepthLuma << " / "
      << info.BitDepthChroma << std::endl
...
class Plugin {
 public:
  static Plugin *loadEncoderPlugin(MFXVideoSession &session, mfxU32 codecId) {
    static const mfxPluginUID hevc_enc_uid = {
        0x6f, 0xad, 0xc7, 0x91, 0xa0, 0xc2, 0xeb, 0x47,
        0x9a, 0xb6, 0xdc, 0xd5, 0xea, 0x9d, 0xa3, 0x47};
    if (codecId == MFX_CODEC_HEVC) return new Plugin(session, hevc_enc_uid);
    return 0;
  }
  static Plugin *loadDecoderPlugin(MFXVideoSession &session, mfxU32 codecId) {
    static const mfxPluginUID hevc_dec_uid = {
        0x33, 0xa6, 0x1c, 0x0b, 0x4c, 0x27, 0x45, 0x4c,
        0xa8, 0xd8, 0x5d, 0xde, 0x75, 0x7c, 0x6f, 0x8e};
    if (codecId == MFX_CODEC_HEVC) return new Plugin(session, hevc_dec_uid);
    return 0;
  }
  ~Plugin() {
    if (isGood()) MFXVideoUSER_UnLoad(session, &uid);
  }
 ...
 private:
  Plugin(MFXVideoSession &_session, mfxPluginUID _uid)
      : session(_session), uid(_uid) {
...
class SurfacePool {
 public:
  SurfacePool(ushort width_, ushort height_, ushort count,
              const mfxFrameInfo &frameInfo, uchar bpp = 12);
  ~SurfacePool();
  ...
  static SurfacePool *create(T *instance, mfxVideoParam &params) {
    ...
    if (res < MFX_ERR_NONE) return 0;
    return new SurfacePool(request.Info.Width, request.Info.Height,
                           request.NumFrameSuggested, params.mfx.FrameInfo);
  }
