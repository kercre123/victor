...
VideoCapture_IntelMFX::VideoCapture_IntelMFX(const cv::String &filename)
  ...
  DBG(cout << "DecodeHeader: " << res << endl
           << params.mfx << params.mfx.FrameInfo << endl);
  if (res < MFX_ERR_NONE) {
...
bool VideoCapture_IntelMFX::grabFrame() {
  ...
  while (true) {
    ...
    outSurface = 0;
    res = decoder->DecodeFrameAsync(bs->drain ? 0 : &bs->stream, workSurface,
                                    (mfxFrameSurface1 **)&outSurface, &sync);
    if (res == MFX_ERR_NONE) {
...
bool VideoCapture_IntelMFX::retrieveFrame(int, OutputArray out) {
  ...
  hal::cvtTwoPlaneYUVtoBGR(data.Y, data.UV, data.Pitch, res.data, res.step,
                           cols, rows, 3, false, 0);
