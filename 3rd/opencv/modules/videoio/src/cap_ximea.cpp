...
CvCapture* cvCreateCameraCapture_XIMEA(const char* serialNumber) {
  ...
  if (capture->open(serialNumber)) return capture;
...
IplImage* CvCaptureCAM_XIMEA::retrieveFrame(int) {
  ...
  switch (image.frm) {
    ...
    case XI_MONO16:
    case XI_RAW16:
      memcpy(frame->imageData, image.bp,
             image.width * image.height * sizeof(WORD));
      break;
    case XI_RGB24:
    case XI_RGB_PLANAR:
...
void CvCaptureCAM_XIMEA::resetCvImage() {
  ...
  if (do_reset) {
    ...
    switch (image.frm) {
      ...
      case XI_RAW16:
        frame =
            cvCreateImage(cvSize(image.width, image.height), IPL_DEPTH_16U, 1);
        break;
      case XI_RGB24:
      case XI_RGB_PLANAR:
        frame =
      ...
      case XI_RGB32:
        frame =
            cvCreateImage(cvSize(image.width, image.height), IPL_DEPTH_8U, 4);
        break;
      default:
        errMsg("CvCaptureCAM_XIMEA::resetCvImage ERROR: Unknown format.",
               XI_NOT_SUPPORTED_DATA_FORMAT);
...
int CvCaptureCAM_XIMEA::ocvParamtoXimeaParam(int property_id) const {
  ...
  switch (property_id) {
    ...
    case CV_CAP_PROP_POS_FRAMES:
      ...
      stat = xiSetParamInt(hmv, XI_PRM_COUNTER_SELECTOR,
                           XI_CNT_SEL_TRANSPORT_TRANSFERRED_FRAMES);
...
bool CvCaptureCAM_XIMEA::setProperty(int property_id, double value) {
  ...
  if (doAcqReset) {
    ...
    if (stat != XI_OK) setProp_result = false;
  }
...
void CvCaptureCAM_XIMEA::errMsg(const char* msg, int errNum) const {
  ...
  switch (errNum) {
    ...
    case XI_FREE_BANDWIDTH:
      error_message = "Freeing bandwith error";
      break;
    case XI_READBLK:
    ...
    case XI_CANT_PROCESS:
      error_message = "Data cant be processed";
      break;
    ...
    case XI_ACQUISITION_STOPED_WERR:
      error_message = "Acquisition has been stoped with error.";
      break;
    ...
    case XI_NOT_ENOUGH_PRIVILEGES:
      error_message = "Application doesnt enough privileges(one or more app";
      break;
    ...
    case XI_DEVICE_HAS_BEEN_RESETED:
      error_message = "Device has been reseted, abnormal initial state";
      break;
    ...
    case XI_COULDNT_INIT_PROCESSOR:
      error_message = "Couldnt initialize processor.";
      break;
    case XI_NOT_INITIALIZED:
      error_message =
          "The object/module/procedure/process being referred to has not been "
          "started.";
      break;
