...
bool DeviceHandler::init(MFXVideoSession &session) {
  ...
  DBG(cout << "MFX QueryVersion: " << res << " => " << ver.Major << "."
           << ver.Minor << endl);

  if (res != MFX_ERR_NONE) return false;
...
VAHandle::VAHandle() {
  ...
  if (file < 0) CV_Error(Error::StsError, "Can't open file: " + filename);
...
bool VAHandle::initDeviceSession(MFXVideoSession &session) {
  ...
  DBG(cout << "vaInitialize: " << va_res << endl
           << majorVer << '.' << minorVer << endl);
  if (va_res == VA_STATUS_SUCCESS) {
    mfxStatus mfx_res = session.SetHandle(
        static_cast<mfxHandleType>(MFX_HANDLE_VA_DISPLAY), display);
...
SurfacePool::SurfacePool(ushort width_, ushort height_, ushort count,
                         const mfxFrameInfo &frameInfo, uchar bpp)
  ...
  for (int i = 0; i < count; ++i) {
    ...
    DBG(cout << "allocate surface " << (void *)&surface
             << ", Y = " << (void *)dataPtr << " (" << width << "x" << height
             << ")" << endl);
  }
  DBG(cout << "Allocated: " << endl
           << "- surface data: " << buffers.size() << " bytes" << endl
           << "- surface headers: "
           << surfaces.size() * sizeof(mfxFrameSurface1) << " bytes" << endl);
}

SurfacePool::~SurfacePool() {}

mfxFrameSurface1 *SurfacePool::getFreeSurface() {
  for (std::vector<mfxFrameSurface1>::iterator i = surfaces.begin();
       i != surfaces.end(); ++i)
    if (!i->Data.Locked) return &(*i);
  return 0;
}

//==================================================================================================

ReadBitstream::ReadBitstream(const char *filename, size_t maxSize)
    : drain(false) {
...
bool ReadBitstream::read() {
  ...
  stream.DataOffset = 0;
  input.read((char *)(stream.Data + stream.DataLength),
...
WriteBitstream::WriteBitstream(const char *filename, size_t maxSize) {
  ...
  stream.MaxLength = (mfxU32)maxSize;
  stream.Data = new mfxU8[stream.MaxLength];
  DBG(cout << "BS Allocate " << maxSize << " bytes ("
           << ((float)maxSize / (1 << 20)) << " Mb)" << endl);
  CV_Assert(stream.Data);
}
