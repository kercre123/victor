...
class DECLSPEC_UUID("4F8A1939-2FD3-46DB-AE70-DB7E0DD79B73")
    DECLSPEC_NOVTABLE ICustomStreamSink : public IUnknown {
 public:
...
// Type: IUnknown*
DEFINE_GUID(MF_MEDIASINK_SAMPLEGRABBERCALLBACK, 0x26957aa7, 0xaff4, 0x464c,
            0xbb, 0x8b, 0x7, 0xba, 0x65, 0xce, 0x11, 0xdf);

// {4BD133CC-EB9B-496E-8865-0813BFBC6FAA}
DEFINE_GUID(MF_STREAMSINK_ID, 0x4bd133cc, 0xeb9b, 0x496e, 0x88, 0x65, 0x8, 0x13,
            0xbf, 0xbc, 0x6f, 0xaa);

// {C9E22A8C-6A50-4D78-9183-0834A02A3780}
DEFINE_GUID(MF_STREAMSINK_MEDIASINKINTERFACE, 0xc9e22a8c, 0x6a50, 0x4d78, 0x91,
            0x83, 0x8, 0x34, 0xa0, 0x2a, 0x37, 0x80);

// {DABD13AB-26B7-47C2-97C1-4B04C187B838}
DEFINE_GUID(MF_MEDIASINK_PREFERREDTYPE, 0xdabd13ab, 0x26b7, 0x47c2, 0x97, 0xc1,
            0x4b, 0x4, 0xc1, 0x87, 0xb8, 0x38);
...
MAKE_MAP(MediaEventType)
MediaEventTypeMap(MediaEventTypePairs,
                  MediaEventTypePairs + sizeof(MediaEventTypePairs) /
                                            sizeof(MediaEventTypePairs[0]));

MAKE_ENUM(MFSTREAMSINK_MARKER_TYPE)
...
StreamSinkMarkerTypeMap(StreamSinkMarkerTypePairs,
                        StreamSinkMarkerTypePairs +
                            sizeof(StreamSinkMarkerTypePairs) /
                                sizeof(StreamSinkMarkerTypePairs[0]));

#ifdef WINRT

#ifdef __cplusplus_winrt
#define _ContextCallback Concurrency::details::_ContextCallback
#define BEGIN_CALL_IN_CONTEXT(hr, var, ...) \
  hr = S_OK;                                \
    var._CallInContext([__VA_ARGS__]() {
#define END_CALL_IN_CONTEXT(hr)                                   \
...
#define _DeviceInformationCollection \
...
#define _MediaCaptureInitializationSettings \
...
#define _MediaEncodingProperties \
  Windows::Media::MediaProperties::IMediaEncodingProperties
#define _VideoEncodingProperties \
...
#define _DeviceInformationStatics \
  Windows::Devices::Enumeration::DeviceInformation
#define _MediaEncodingProfile \
...
#define END_CREATE_ASYNC(hr)                                      \
  if (FAILED(hr)) throw Platform::Exception::CreateException(hr); \
  }))
#define DEFINE_TASK Concurrency::task
#define CREATE_TASK Concurrency::create_task
#define CREATE_OR_CONTINUE_TASK(_task, rettype, func) \
  _task = (_task == Concurrency::task<rettype>())     \
...
#define MAKE_READONLY_PROPERTY(Type, PropName, PropValue) \
...
#define GET_WRL_OBJ_FROM_REF(objtype, obj, orig, hr) \
  objtype ^ obj = orig;                              \
  hr = S_OK;
#define GET_WRL_OBJ_FROM_OBJ(objtype, obj, orig, hr) \
  objtype ^ obj = safe_cast<objtype ^>(orig);        \
  hr = S_OK;
#define WRL_ENUM_GET(obj, prefix, prop) obj::##prop
#define WRL_PROP_GET(obj, prop, arg, hr) \
  arg = obj->##prop;                     \
  hr = S_OK;
#define WRL_PROP_PUT(obj, prop, arg, hr) \
  obj->##prop = arg;                     \
  hr = S_OK;
#define WRL_METHOD_BASE(obj, method, ret, hr) \
  ret = obj->##method();                      \
  hr = S_OK;
#define WRL_METHOD(obj, method, ret, hr, ...) \
  ret = obj->##method(__VA_ARGS__);           \
  hr = S_OK;
#define WRL_METHOD_NORET_BASE(obj, method, hr) \
  obj->##method();                             \
  hr = S_OK;
#define WRL_METHOD_NORET(obj, method, hr, ...) \
...
#define ACTIVATE_OBJ(rtclass, objtype, obj, hr)  \
  MAKE_WRL_OBJ(objtype) obj = ref new objtype(); \
  hr = S_OK;
#define ACTIVATE_STATIC_OBJ(rtclass, objtype, obj, hr) \
  objtype obj;                                         \
  hr = S_OK;
#else
...
#define _MediaCaptureVideoPreview \
  ABI::Windows::Media::Capture::IMediaCaptureVideoPreview
#define _MediaCaptureInitializationSettings \
  ABI::Windows::Media::Capture::IMediaCaptureInitializationSettings
#define _VideoDeviceController \
  ABI::Windows::Media::Devices::IVideoDeviceController
#define _MediaDeviceController \
  ABI::Windows::Media::Devices::IMediaDeviceController
#define _MediaEncodingProperties \
  ABI::Windows::Media::MediaProperties::IMediaEncodingProperties
#define _VideoEncodingProperties \
  ABI::Windows::Media::MediaProperties::IVideoEncodingProperties
...
#define _MediaEncodingProfile \
  ABI::Windows::Media::MediaProperties::IMediaEncodingProfile
...
#define CREATE_OR_CONTINUE_TASK(_task, rettype, func)                    \
  _task = (_task == Concurrency_winrt::task<rettype>())                  \
              ? Concurrency_winrt::create_task<rettype>(func)            \
              : _task.then([func](rettype, rettype *retVal) -> HRESULT { \
                  return func(retVal);                                   \
                });
...
#define MAKE_PROPERTY(Type, PropName, PropValue) \
  STDMETHODIMP get_##PropName(Type *pVal) {      \
    if (pVal) {                                  \
      *pVal = PropValue;                         \
    } else {                                     \
      return E_INVALIDARG;                       \
  ...
  STDMETHODIMP put_##PropName(Type Val) {        \
    PropValue = Val;                             \
    return S_OK;                                 \
  }
#define MAKE_PROPERTY_STRING(Type, PropName, PropValue)       \
  STDMETHODIMP get_##PropName(Type *pVal) {                   \
    if (pVal) {                                               \
      return ::WindowsDuplicateString(PropValue.Get(), pVal); \
    } else {                                                  \
      return E_INVALIDARG;                                    \
    }                                                         \
  }                                                           \
...
#define RELEASE_AGILE_WRL(x) \
  if (x) {                   \
    (x)->Release();          \
    x = nullptr;             \
  }
#define RELEASE_WRL(x) \
  if (x) {             \
    (x)->Release();    \
    x = nullptr;       \
  }
#define GET_WRL_OBJ_FROM_REF(objtype, obj, orig, hr) \
  Microsoft::WRL::ComPtr<objtype> obj;               \
...
#define ACTIVATE_LOCAL_OBJ(objtype, ...) \
  Microsoft::WRL::Make<objtype>(__VA_ARGS__)
#define ACTIVATE_EVENT_HANDLER(objtype, ...) \
  Microsoft::WRL::Callback<objtype>(__VA_ARGS__).Get()
#define ACTIVATE_OBJ(rtclass, objtype, obj, hr)                    \
  MAKE_WRL_OBJ(objtype) obj;                                       \
  {                                                                \
    Microsoft::WRL::ComPtr<IActivationFactory> objFactory;         \
    hr = Windows::Foundation::GetActivationFactory(                \
        Microsoft::WRL::Wrappers::HStringReference(rtclass).Get(), \
...
class ComPtr {
 public:
  ComPtr() throw() {}
  ComPtr(T* lp) throw() { p = lp; }
  ComPtr(_In_ const ComPtr<T>& lp) throw() { p = lp.p; }
  virtual ~ComPtr() {}
  ...
  bool operator!() const throw() { return p.operator==(NULL); }
  bool operator==(_In_opt_ T* pT) const throw() { return p.operator==(pT); }
  bool operator!=(_In_opt_ T* pT) const throw() { return p.operator!=(pT); }
  operator bool() { return p.operator!=(NULL); }

  T* const* GetAddressOf() const throw() { return &p; }

  T** GetAddressOf() throw() { return &p; }
  ...
  void Attach(_In_opt_ T* p2) throw() { p.Attach(p2); }
  // Detach the interface (does not Release)
  T* Detach() throw() { return p.Detach(); }
  ...
  HRESULT As(_Out_ ComPtr<U>* lp) const throw() {
    return p->QueryInterface(
        __uuidof(U), reinterpret_cast<void**>(lp->ReleaseAndGetAddressOf()));
  }
...
class CBaseAttributes : public TBase {
 protected:
  ...
  // Initializes the object by creating the standard Media Foundation attribute
  // store.
 ...
 public:
  ...
  STDMETHODIMP Compare(IMFAttributes *pTheirs,
                       MF_ATTRIBUTES_MATCH_TYPE MatchType, BOOL *pbResult) {
    assert(_spAttributes);
  ...
  HRESULT DeserializeFromStream(DWORD dwOptions, IStream *pStm) {
    assert(_spAttributes);
    return MFDeserializeAttributesFromStream(_spAttributes.Get(), dwOptions,
                                             pStm);
  }
  ...
  HRESULT GetRatio(REFGUID guidKey, UINT32 *pnNumerator,
                   UINT32 *punDenominator) {
    assert(_spAttributes);
    return MFGetAttributeRatio(_spAttributes.Get(), guidKey, pnNumerator,
                               punDenominator);
  }

  HRESULT SetRatio(REFGUID guidKey, UINT32 unNumerator, UINT32 unDenominator) {
    assert(_spAttributes);
    return MFSetAttributeRatio(_spAttributes.Get(), guidKey, unNumerator,
                               unDenominator);
  }

  // Gets an attribute whose value represents the size of something (eg a video
  // frame).
  HRESULT GetSize(REFGUID guidKey, UINT32 *punWidth, UINT32 *punHeight) {
    assert(_spAttributes);
    return MFGetAttributeSize(_spAttributes.Get(), guidKey, punWidth,
                              punHeight);
  }

  // Sets an attribute whose value represents the size of something (eg a video
  // frame).
...
#endif
{
 public:
  // IUnknown methods
#if defined(_MSC_VER) && _MSC_VER >= 1700  // '_Outptr_result_nullonfailure_'
                                           // SAL is avaialable since VS 2012
  STDMETHOD(QueryInterface)
  (REFIID riid, _Outptr_result_nullonfailure_ void **ppv)
#else
  STDMETHOD(QueryInterface)
  ...
  StreamSink()
      : m_IsShutdown(false),
        ...
        m_fWaitingForFirstSample(false),
  ...
  HRESULT Initialize() {
    ...
    if (SUCCEEDED(hr)) {
      ...
      hr = CBaseAttributes<>::GetUnknown(MF_STREAMSINK_MEDIASINKINTERFACE,
                                         __uuidof(IMFMediaSink),
                                         (LPVOID *)pMedSink.GetAddressOf());
  ...
  HRESULT Start(MFTIME start) {
    ...
    if (m_state != State_TypeNotSet) {
      ...
      m_fWaitingForFirstSample =
          SUCCEEDED(m_spCurrentType->GetMajorType(&guiMajorType)) &&
          (guiMajorType == MFMediaType_Video);
  ...
  HRESULT Shutdown() {
    ...
    HRESULT hr = S_OK;
    assert(!m_IsShutdown);
    hr = m_pParent->GetUnknown(MF_MEDIASINK_SAMPLEGRABBERCALLBACK,
                               IID_IMFSampleGrabberSinkCallback,
                               (LPVOID *)pSampleCallback.GetAddressOf());
    if (SUCCEEDED(hr)) {
  ...
  HRESULT STDMETHODCALLTYPE ProcessSample(IMFSample *pSample) {
    ...
    if (SUCCEEDED(hr)) hr = CheckShutdown();
    if (SUCCEEDED(hr)) {
      ...
      if (SUCCEEDED(hr)) {
        hr = pSampleCallback->OnProcessSample(
            guidMajorType, 0, hnsTime, hnsDuration, pSrc, cbCurrentLength);
        pInput->Unlock();
      }
      if (SUCCEEDED(hr)) {
  ...
  HRESULT STDMETHODCALLTYPE PlaceMarker(
    ...
    HRESULT hr = S_OK;
    if (m_state == State_TypeNotSet) hr = MF_E_NOT_INITIALIZED;
    ...
    DebugPrintOut(L"StreamSink::PlaceMarker: HRESULT=%i %s\n", hr,
                  StreamSinkMarkerTypeMap.at(eMarkerType).c_str());
    return hr;
  }
  ...
  HRESULT STDMETHODCALLTYPE GetEvent(DWORD dwFlags, IMFMediaEvent **ppEvent) {
    ...
    if (SUCCEEDED(hr) && SUCCEEDED((*ppEvent)->GetType(&meType)) &&
        meType == MEStreamSinkStopped) {
    }
    HRESULT hrStatus = S_OK;
    if (SUCCEEDED(hr)) hr = (*ppEvent)->GetStatus(&hrStatus);
    if (SUCCEEDED(hr))
      DebugPrintOut(L"StreamSink::GetEvent: HRESULT=%i %s\n", hrStatus,
                    MediaEventTypeMap.at(meType).c_str());
    else
  ...
  HRESULT STDMETHODCALLTYPE QueueEvent(MediaEventType met,
    ...
    if (SUCCEEDED(hr)) {
      hr = m_spEventQueue->QueueEventParamVar(met, guidExtendedType, hrStatus,
                                              pvValue);
    }

    LeaveCriticalSection(&m_critSec);
    DebugPrintOut(L"StreamSink::QueueEvent: HRESULT=%i %s\n", hrStatus,
                  MediaEventTypeMap.at(met).c_str());
  ...
  STDMETHODIMP IsMediaTypeSupported(
    ...
    if (ppMediaType && SUCCEEDED(hr)) {
      ...
      if (SUCCEEDED(hr)) {
        hr = m_pParent->GetUnknown(MF_MEDIASINK_PREFERREDTYPE,
                                   __uuidof(IMFMediaType), (LPVOID *)&pType);
      }
      if (SUCCEEDED(hr)) {
        hr = pType->LockStore();
      }
      ...
      if (SUCCEEDED(hr)) {
        ...
        for (uiCount = 0; SUCCEEDED(hr) && uiCount < uiTotal; uiCount++) {
          ...
          if (SUCCEEDED(hr) &&
              (guid == MF_MT_FRAME_SIZE || guid == MF_MT_MAJOR_TYPE ||
               guid == MF_MT_PIXEL_ASPECT_RATIO ||
               guid == MF_MT_ALL_SAMPLES_INDEPENDENT ||
               guid == MF_MT_INTERLACE_MODE || guid == MF_MT_SUBTYPE)) {
  ...
  STDMETHODIMP GetMediaTypeByIndex(
    ...
    if (dwIndex > 0) {
      ...
      // return preferred type based on media capture library 6 elements
      // preferred preview type hr = m_spCurrentType.CopyTo(ppType);
      if (SUCCEEDED(hr)) {
        _ComPtr<IMFMediaType> pType;
  ...
  STDMETHODIMP SetCurrentMediaType(IMFMediaType *pMediaType) {
    ...
    if (SUCCEEDED(hr)) {
      ...
      if (SUCCEEDED(hr) && guid == MFMediaType_Video) {
        hr = MFGetAttributeSize(m_spCurrentType.Get(), MF_MT_FRAME_SIZE,
...
//  Clear(FN fn) takes a functor object that releases the objects, if they need
//  cleanup.
...
// The ComPtrList class template derives from List<> and implements a list of
// COM pointers.
...
class List {
 protected:
  ...
  struct Node {
    ...
    Node(T item) : prev(nullptr), next(nullptr) { this->item = item; }
 ...
 protected:
  ...
  virtual HRESULT RemoveItem(Node *pNode, T *ppItem) {
    ...
    assert(pNode !=
           &m_anchor);  // We should never try to remove the anchor node.
    if (pNode == &m_anchor) {
      return E_INVALIDARG;
    }
 ...
 public:
  ...
  // ppItem can be nullptr if you don't want the item back. (But the method does
...
// succeed but return a nullptr pointer. By default, the list does not allow
// nullptr pointers.
...
/* Be sure to declare webcam device capability in manifest
  For better media capture support, add the following snippet with correct
  module name to the project manifest (videoio needs DLL activation class
  factoryentry points): <Extensions> <Extension
  Category="windows.activatableClass.inProcessServer"> <InProcessServer>
        <Path>modulename</Path>
        <ActivatableClass ActivatableClassId="cv.MediaSink"
  ThreadingModel="both" />
      </InProcessServer>
    </Extension>
  </Extensions>*/

extern const __declspec(selectany) WCHAR RuntimeClass_CV_MediaSink[] =
    L"cv.MediaSink";
...
class MediaSink :
    ...
    public Microsoft::WRL::RuntimeClass<
        ...
        IMFMediaSink, IMFClockStateSink, Microsoft::WRL::FtmBase,
        CBaseAttributes<>>
#else
    public IMFMediaSink,
...
{
#ifdef WINRT
  InspectableClass(RuntimeClass_CV_MediaSink, BaseTrust) public :
#else
 public:
  ULONG STDMETHODCALLTYPE AddRef() { return InterlockedIncrement(&m_cRef); }
  ULONG STDMETHODCALLTYPE Release() {
...
#endif
      MediaSink()
      : m_IsShutdown(false), m_llStartTime(0) {
    CBaseAttributes<>::Initialize(0U);
    InitializeCriticalSectionEx(&m_critSec, 3000, 0);
  ...
  STDMETHODIMP SetProperties(
    ...
    if (pConfiguration) {
      ...
      Microsoft::WRL::ComPtr<
          ABI::Windows::Foundation::Collections::IMap<HSTRING, IInspectable *>>
          spSetting;
      Microsoft::WRL::ComPtr<ABI::Windows::Foundation::IPropertyValue>
          spPropVal;
      Microsoft::WRL::ComPtr<
          ABI::Windows::Media::MediaProperties::IMediaEncodingProperties>
          pMedEncProps;
      UINT32 uiType =
          ABI::Windows::Media::Capture::MediaStreamType_VideoPreview;
      ...
      if (SUCCEEDED(hr)) {
        hr = spSetting->Lookup(Microsoft::WRL::Wrappers::HStringReference(
                                   MF_PROP_SAMPLEGRABBERCALLBACK)
                                   .Get(),
                               spInsp.ReleaseAndGetAddressOf());
        if (FAILED(hr)) {
          hr = E_INVALIDARG;
        }
        if (SUCCEEDED(hr)) {
      ...
      if (SUCCEEDED(hr)) {
        hr = spSetting->Lookup(
            Microsoft::WRL::Wrappers::HStringReference(MF_PROP_VIDTYPE).Get(),
            spInsp.ReleaseAndGetAddressOf());
        if (FAILED(hr)) {
          hr = E_INVALIDARG;
        }
      ...
      if (SUCCEEDED(hr)) {
        hr = spSetting->Lookup(
            Microsoft::WRL::Wrappers::HStringReference(MF_PROP_VIDENCPROPS)
                .Get(),
            spInsp.ReleaseAndGetAddressOf());
        if (FAILED(hr)) {
          hr = E_INVALIDARG;
      ...
      if (SUCCEEDED(hr)) {
        hr = SetMediaStreamProperties(
            (ABI::Windows::Media::Capture::MediaStreamType)uiType,
            pMedEncProps.Get());
      }
    }
  ...
  static HRESULT ConvertPropertiesToMediaType(
    ...
    *ppMT = nullptr;
    ...
    while (hasCurrent) {
      Microsoft::WRL::ComPtr<ABI::Windows::Foundation::Collections::
    ...
    if (SUCCEEDED(hr)) {
      ...
      if (SUCCEEDED(hr)) {
        if (guiMajorType != MFMediaType_Video &&
            guiMajorType != MFMediaType_Audio) {
          hr = E_UNEXPECTED;
        }
      }
    }
  ...
  HRESULT SetMediaStreamProperties(
      ABI::Windows::Media::Capture::MediaStreamType MediaStreamType,
      _In_opt_ ABI::Windows::Media::MediaProperties::IMediaEncodingProperties
    ...
    if (MediaStreamType !=
            ABI::Windows::Media::Capture::MediaStreamType_VideoPreview &&
        MediaStreamType !=
            ABI::Windows::Media::Capture::MediaStreamType_VideoRecord &&
        MediaStreamType !=
            ABI::Windows::Media::Capture::MediaStreamType_Audio) {
      return E_INVALIDARG;
    }
    ...
    if (mediaEncodingProperties != nullptr) {
      ...
      if (SUCCEEDED(hr)) {
        hr = AddStreamSink(GetStreamId(MediaStreamType), nullptr,
  ...
  HRESULT STDMETHODCALLTYPE GetCharacteristics(
    ...
    if (SUCCEEDED(hr = CheckShutdown())) {
      // if had an activation object for the sink, shut down would be managed
      // and MF_STREAM_SINK_SUPPORTS_ROTATION appears to be setable to TRUE
      *pdwCharacteristics =
          MEDIASINK_FIXED_STREAMS;  // | MEDIASINK_REQUIRE_REFERENCE_MEDIATYPE;
    }
    LeaveCriticalSection(&m_critSec);
  ...
  HRESULT STDMETHODCALLTYPE AddStreamSink(DWORD dwStreamSinkIdentifier,
    ...
    if (SUCCEEDED(hr)) {
      ...
      if (SUCCEEDED(hr)) hr = pStream.As<IMFStreamSink>(&spMFStream);
#else
      StreamSink* pSink = new StreamSink();
      if (pSink) {
        hr = pSink->QueryInterface(IID_IMFStreamSink,
                                   (void**)spMFStream.GetAddressOf());
        if (SUCCEEDED(hr)) {
    ...
    if (SUCCEEDED(hr)) {
      ...
      if (SUCCEEDED(hr)) {
        hr = pAttr->SetUnknown(MF_STREAMSINK_MEDIASINKINTERFACE,
                               (IMFMediaSink *)this);
      }
    }
    if (SUCCEEDED(hr)) {
...
#endif
      if (SUCCEEDED(hr)) hr = spCustomSink->Shutdown();
    }
    LeaveCriticalSection(&m_critSec);
  ...
  HRESULT STDMETHODCALLTYPE GetStreamSinkByIndex(DWORD dwIndex,
                                                 IMFStreamSink **ppStreamSink) {
    ...
    if (SUCCEEDED(hr)) {
      ...
      for (; pos != endPos && dwCurrent < dwIndex;
           pos = m_streams.Next(pos), ++dwCurrent) {
  ...
  HRESULT STDMETHODCALLTYPE GetStreamSinkById(DWORD dwStreamSinkIdentifier,
                                              IMFStreamSink **ppStreamSink) {
    if (ppStreamSink == NULL) {
      return E_INVALIDARG;
    }
  ...
  SetPresentationClock(IMFPresentationClock *pPresentationClock) {
    EnterCriticalSection(&m_critSec);
  ...
  HRESULT STDMETHODCALLTYPE
  GetPresentationClock(IMFPresentationClock **ppPresentationClock) {
  ...
  HRESULT STDMETHODCALLTYPE Shutdown(void) {
    ...
    if (SUCCEEDED(hr)) {
      ...
      _ComPtr<IMFMediaType> pType;
      hr = CBaseAttributes<>::GetUnknown(MF_MEDIASINK_PREFERREDTYPE,
                                         __uuidof(IMFMediaType),
                                         (LPVOID *)pType.GetAddressOf());
      if (SUCCEEDED(hr)) {
  ...
  class StartFunc {
   public:
    StartFunc(LONGLONG llStartTime) : _llStartTime(llStartTime) {}
  ...
  // IMFClockStateSink
  HRESULT STDMETHODCALLTYPE OnClockStart(MFTIME hnsSystemTime,
  ...
  HRESULT STDMETHODCALLTYPE OnClockStop(MFTIME hnsSystemTime) {
    ...
    if (SUCCEEDED(hr))
      hr = GetUnknown(MF_MEDIASINK_SAMPLEGRABBERCALLBACK,
                      ...
                      (LPVOID *)pSampleCallback.GetAddressOf());
    LeaveCriticalSection(&m_critSec);
    if (SUCCEEDED(hr)) hr = pSampleCallback->OnClockStop(hnsSystemTime);
  ...
  HRESULT STDMETHODCALLTYPE OnClockPause(MFTIME hnsSystemTime) {
    ...
    hr = GetUnknown(MF_MEDIASINK_SAMPLEGRABBERCALLBACK,
                    IID_IMFSampleGrabberSinkCallback,
                    (LPVOID *)pSampleCallback.GetAddressOf());
    if (SUCCEEDED(hr)) hr = pSampleCallback->OnClockPause(hnsSystemTime);
  ...
  HRESULT STDMETHODCALLTYPE OnClockRestart(MFTIME hnsSystemTime) {
    ...
    hr = GetUnknown(MF_MEDIASINK_SAMPLEGRABBERCALLBACK,
                    IID_IMFSampleGrabberSinkCallback,
                    (LPVOID *)pSampleCallback.GetAddressOf());
    if (SUCCEEDED(hr)) hr = pSampleCallback->OnClockRestart(hnsSystemTime);
