...
VideoWriter_IntelMFX::VideoWriter_IntelMFX(const String &filename, int _fourcc,
                                           double fps, Size frameSize_, bool)
    : session(0),
      plugin(0),
      deviceHandler(0),
      bs(0),
  ...
  if (plugin && !plugin->isGood()) {
    MSG(cerr << "MFX: LoadPlugin failed for codec: " << codecId << " ("
             << FourCC(_fourcc) << ")" << endl);
    return;
  }
  ...
  params.mfx.TargetKbps =
      (mfxU16)cvRound(frameSize.area() * fps / 500);  // TODO: set in options
  ...
  DBG(cout << "MFX Query: " << res << endl
           << params.mfx << params.mfx.FrameInfo);
  if (res < MFX_ERR_NONE) {
  ...
  // Open output bitstream
  {
    ...
    bs =
        new WriteBitstream(filename.c_str(), par.mfx.BufferSizeInKB * 1024 * 2);
    if (!bs->isOpened()) {
...
bool VideoWriter_IntelMFX::write_one(cv::InputArray bgr) {
  ...
  if (!bgr.empty() &&
      (bgr.dims() != 2 || bgr.type() != CV_8UC3 || bgr.size() != frameSize)) {
    MSG(cerr << "MFX: invalid frame passed to encoder: "
             << "dims/depth/cn=" << bgr.dims() << "/" << bgr.depth() << "/"
             << bgr.channels() << ", size=" << bgr.size() << endl);
    return false;
  }
  if (!bgr.empty()) {
    ...
    Mat src = bgr.getMat();
    hal::cvtBGRtoTwoPlaneYUV(src.data, src.step, workSurface->Data.Y,
                             workSurface->Data.UV, workSurface->Data.Pitch,
  ...
  while (true) {
    ...
    if (res == MFX_ERR_NONE) {
      res = session->SyncOperation(
          sync, 1000);  // 1 sec, TODO: provide interface to modify timeout
      if (res == MFX_ERR_NONE) {
        // ready to write
        if (!bs->write()) {
...
Ptr<VideoWriter_IntelMFX> VideoWriter_IntelMFX::create(const String &filename,
                                                       int _fourcc, double fps,
                                                       Size frameSize,
                                                       bool isColor) {
  if (codecIdByFourCC(_fourcc) > 0) {
    Ptr<VideoWriter_IntelMFX> a = makePtr<VideoWriter_IntelMFX>(
        filename, _fourcc, fps, frameSize, isColor);
    if (a->isOpened()) return a;
  }
