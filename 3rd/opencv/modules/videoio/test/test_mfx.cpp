...
TEST(Videoio_MFX, write_invalid) {
  ...
  VideoWriter writer;
  bool res = true;
  ASSERT_NO_THROW(res = writer.open(filename, CAP_INTEL_MFX,
                                    VideoWriter::fourcc('H', '2', '6', '4'), 1,
                                    Size(641, 480), true));
  EXPECT_FALSE(res);
  ...
  EXPECT_FALSE(writer.isOpened());
  ASSERT_NO_THROW(res = writer.open(filename, CAP_INTEL_MFX,
                                    VideoWriter::fourcc('A', 'B', 'C', 'D'), 1,
                                    Size(640, 480), true));
  EXPECT_FALSE(res);
  EXPECT_FALSE(writer.isOpened());
  ASSERT_NO_THROW(res = writer.open(String(), CAP_INTEL_MFX,
                                    VideoWriter::fourcc('H', '2', '6', '4'), 1,
                                    Size(640, 480), true));
  EXPECT_FALSE(res);
  EXPECT_FALSE(writer.isOpened());
  ASSERT_ANY_THROW(res = writer.open(filename, CAP_INTEL_MFX,
                                     VideoWriter::fourcc('H', '2', '6', '4'), 0,
                                     Size(640, 480), true));
  EXPECT_FALSE(res);
  EXPECT_FALSE(writer.isOpened());
...
INSTANTIATE_TEST_CASE_P(
    videoio, Videoio_MFX,
    testing::Combine(testing::Values(Size(640, 480), Size(638, 478),
                                     Size(636, 476), Size(1920, 1080)),
