...
class CascadeClassifierImpl : public BaseCascadeClassifier {
 ...
 protected:
  ...
  bool detectSingleScale(InputArray image, Size processingRectSize, int yStep,
                         double factor, std::vector<Rect>& candidates,
                         std::vector<int>& rejectLevels,
                         std::vector<double>& levelWeights, Size sumSize0,
                         bool outputRejectLevels = false);
#ifdef HAVE_OPENCL
  bool ocl_detectMultiScaleNoGrouping(const std::vector<float>& scales,
                                      std::vector<Rect>& candidates);
#endif
  void detectMultiScaleNoGrouping(InputArray image,
                                  std::vector<Rect>& candidates,
                                  std::vector<int>& rejectLevels,
                                  std::vector<double>& levelWeights,
                                  double scaleFactor, Size minObjectSize,
                                  Size maxObjectSize,
                                  bool outputRejectLevels = false);
  ...
  enum { BOOST = 0 };
  enum {
    DO_CANNY_PRUNING = CASCADE_DO_CANNY_PRUNING,
  ...
  friend int predictOrdered(CascadeClassifierImpl& cascade,
                            Ptr<FeatureEvaluator>& featureEvaluator,
                            double& weight);

  template <class FEval>
  friend int predictCategorical(CascadeClassifierImpl& cascade,
                                Ptr<FeatureEvaluator>& featureEvaluator,
                                double& weight);

  template <class FEval>
  friend int predictOrderedStump(CascadeClassifierImpl& cascade,
                                 Ptr<FeatureEvaluator>& featureEvaluator,
                                 double& weight);

  template <class FEval>
  friend int predictCategoricalStump(CascadeClassifierImpl& cascade,
                                     Ptr<FeatureEvaluator>& featureEvaluator,
                                     double& weight);

  int runAt(Ptr<FeatureEvaluator>& feval, Point pt, int scaleIdx,
            double& weight);
...
#define CV_SUM_PTRS(p0, p1, p2, p3, sum, rect, step)                          \
  /* (x, y) */                                                                \
  (p0) = sum + (rect).x + (step) * (rect).y, /* (x + w, y) */                 \
      (p1) =                                                                  \
          sum + (rect).x + (rect).width + (step) * (rect).y, /* (x, y + h) */ \
      (p2) = sum + (rect).x +                                                 \
             (step) * ((rect).y + (rect).height), /* (x + w, y + h) */        \
      (p3) =                                                                  \
          sum + (rect).x + (rect).width + (step) * ((rect).y + (rect).height)

#define CV_TILTED_PTRS(p0, p1, p2, p3, tilted, rect, step)                    \
  /* (x, y) */                                                                \
  (p0) = tilted + (rect).x + (step) * (rect).y, /* (x - h, y + h) */          \
      (p1) = tilted + (rect).x - (rect).height +                              \
             (step) * ((rect).y + (rect).height), /* (x + w, y + w) */        \
      (p2) = tilted + (rect).x + (rect).width +                               \
             (step) * ((rect).y + (rect).width), /* (x + w - h, y + w + h) */ \
...
#define CALC_SUM(rect, offset) \
  CALC_SUM_((rect)[0], (rect)[1], (rect)[2], (rect)[3], offset)
...
#define CALC_SUM_OFS(rect, ptr) \
  CALC_SUM_OFS_((rect)[0], (rect)[1], (rect)[2], (rect)[3], ptr)
...
class HaarEvaluator : public FeatureEvaluator {
 public:
  ...
  virtual float calcOrd(int featureIdx) const { return (*this)(featureIdx); }
...
inline HaarEvaluator::OptFeature ::OptFeature() {
  ...
  ofs[0][0] = ofs[0][1] = ofs[0][2] = ofs[0][3] = ofs[1][0] = ofs[1][1] =
      ofs[1][2] = ofs[1][3] = ofs[2][0] = ofs[2][1] = ofs[2][2] = ofs[2][3] = 0;
}
...
inline float HaarEvaluator::OptFeature ::calc(const int* ptr) const {
  ...
  if (weight[2] != 0.0f) ret += weight[2] * CALC_SUM_OFS(ofs[2], ptr);
...
inline LBPEvaluator::Feature ::Feature() { rect = Rect(); }

inline LBPEvaluator::OptFeature ::OptFeature() {
  for (int i = 0; i < 16; i++) ofs[i] = 0;
}

inline int LBPEvaluator::OptFeature ::calc(const int* p) const {
  int cval = CALC_SUM_OFS_(ofs[5], ofs[6], ofs[9], ofs[10], p);

  return (CALC_SUM_OFS_(ofs[0], ofs[1], ofs[4], ofs[5], p) >= cval ? 128
                                                                   : 0) |  // 0
         (CALC_SUM_OFS_(ofs[1], ofs[2], ofs[5], ofs[6], p) >= cval ? 64
                                                                   : 0) |  // 1
         (CALC_SUM_OFS_(ofs[2], ofs[3], ofs[6], ofs[7], p) >= cval ? 32
                                                                   : 0) |  // 2
         (CALC_SUM_OFS_(ofs[6], ofs[7], ofs[10], ofs[11], p) >= cval
              ? 16
              : 0) |  // 5
         (CALC_SUM_OFS_(ofs[10], ofs[11], ofs[14], ofs[15], p) >= cval
              ? 8
              : 0) |  // 8
         (CALC_SUM_OFS_(ofs[9], ofs[10], ofs[13], ofs[14], p) >= cval
              ? 4
              : 0) |  // 7
         (CALC_SUM_OFS_(ofs[8], ofs[9], ofs[12], ofs[13], p) >= cval
              ? 2
              : 0) |  // 6
...
inline int predictOrdered(CascadeClassifierImpl& cascade,
                          Ptr<FeatureEvaluator>& _featureEvaluator,
                          double& sum) {
  ...
  CascadeClassifierImpl::Data::DTree* cascadeWeaks =
      &cascade.data.classifiers[0];
  ...
  for (int si = 0; si < nstages; si++) {
    ...
    if (sum < stage.threshold) return -si;
  }
  return 1;
}
...
inline int predictOrderedStump(CascadeClassifierImpl& cascade,
  ...
  FEval& featureEvaluator = (FEval&)*_featureEvaluator;
  const CascadeClassifierImpl::Data::Stump* cascadeStumps =
      &cascade.data.stumps[0];
  const CascadeClassifierImpl::Data::Stage* cascadeStages =
      &cascade.data.stages[0];
