...
class NumpyAllocator : public MatAllocator {
 public:
  ...
  UMatData* allocate(int dims0, const int* sizes, int type, void* data,
    ...
    if (data != 0) {
      // issue #6969: CV_Error(Error::StsAssert, "The data should normally be
      // NULL!"); probably this is safe to do in such extreme case
...
static bool pyopencv_to(PyObject* o, Mat& m, const ArgInfo info) {
  ...
  if (needcopy) {
    if (info.outputarg) {
      failmsg(
          "Layout of the output array %s is incompatible with cv::Mat "
...
static int UMatWrapper_init(cv2_UMatWrapperObject* self, PyObject* args,
                            PyObject* kwds) {
  self->um = NULL;
  {
    // constructor ()
  ...
  }
  {
    ...
    int rows, cols, type;
    if (PyArg_ParseTupleAndKeywords(args, kwds, "iii", (char**)kwlist, &rows,
                                    &cols, &type)) {
  ...
  }
  {
    ...
    if (PyArg_ParseTupleAndKeywords(args, kwds, "O(ii)|(ii)", (char**)kwlist,
                                    &obj, &y0, &y1, &x0, &x1) &&
        PyObject_IsUMat(obj)) {
...
// UMatWrapper.get() - returns numpy array by transferring UMat data to Mat and
// than wrapping it to numpy array (using numpy allocator - and so without
// unnecessary copy)
...
static PyObject* UMatWrapper_handle(cv2_UMatWrapperObject* self, PyObject* args,
                                    PyObject* kwds) {
...
// UMatWrapper.isContinuous() - returns true if the matrix is a submatrix of
// another matrix
...
static PyMethodDef UMatWrapper_methods[] = {
    {"get", (PyCFunction)UMatWrapper_get, METH_NOARGS, "Returns numpy array"},
...
struct pyopencvVecConverter {
  ...
  static PyObject* from(const std::vector<_Tp>& value) {
    if (value.empty()) return PyTuple_New(0);
    int type = traits::Type<_Tp>::value;
    int depth = CV_MAT_DEPTH(type), channels = CV_MAT_CN(type);
...
template <typename _Tp>
static inline bool pyopencv_to_generic_vec(PyObject* obj,
  ...
  for (i = 0; i < n; i++) {
    PyObject* item = items[i];
    if (!pyopencv_to(item, value[i], info)) break;
  }
  Py_DECREF(seq);
  return i == n;
}
...
template <typename _Tp>
static inline PyObject* pyopencv_from_generic_vec(
    const std::vector<_Tp>& value) {
...
template <typename _Tp, typename _Tr>
struct pyopencvVecConverter<std::pair<_Tp, _Tr> > {
  static bool to(PyObject* obj, std::vector<std::pair<_Tp, _Tr> >& value,
                 const ArgInfo info) {
...
template <typename _Tp>
struct pyopencvVecConverter<std::vector<_Tp> > {
  static bool to(PyObject* obj, std::vector<std::vector<_Tp> >& value,
                 const ArgInfo info) {
    return pyopencv_to_generic_vec(obj, value, info);
  }
...
template <>
struct pyopencvVecConverter<Mat> {
...
template <>
struct pyopencvVecConverter<KeyPoint> {
  static bool to(PyObject* obj, std::vector<KeyPoint>& value,
                 const ArgInfo info) {
    return pyopencv_to_generic_vec(obj, value, info);
  }
...
template <>
struct pyopencvVecConverter<DMatch> {
  static bool to(PyObject* obj, std::vector<DMatch>& value,
                 const ArgInfo info) {
    return pyopencv_to_generic_vec(obj, value, info);
  }
...
template <>
struct pyopencvVecConverter<String> {
  static bool to(PyObject* obj, std::vector<String>& value,
                 const ArgInfo info) {
    return pyopencv_to_generic_vec(obj, value, info);
  }
...
bool pyopencv_to(PyObject* obj, TermCriteria& dst, const char* name) {
  (void)name;
  if (!obj) return true;
  return PyArg_ParseTuple(obj, "iid", &dst.type, &dst.maxCount, &dst.epsilon) >
         0;
}
...
bool pyopencv_to(PyObject* obj, RotatedRect& dst, const char* name) {
  (void)name;
  if (!obj) return true;
  return PyArg_ParseTuple(obj, "(ff)(ff)f", &dst.center.x, &dst.center.y,
                          &dst.size.width, &dst.size.height, &dst.angle) > 0;
}
...
PyObject* pyopencv_from(const Moments& m) {
  return Py_BuildValue(
      "{s:d,s:d,s:d,s:d,s:d,s:d,s:d,s:d,s:d,s:d,s:d,s:d,s:d,s:d,s:d,s:d,s:d,s:"
      "d,s:d,s:d,s:d,s:d,s:d,s:d}",
      "m00", m.m00, "m10", m.m10, "m01", m.m01, "m20", m.m20, "m11", m.m11,
      "m02", m.m02, "m30", m.m30, "m21", m.m21, "m12", m.m12, "m03", m.m03,
      "mu20", m.mu20, "mu11", m.mu11, "mu02", m.mu02, "mu30", m.mu30, "mu21",
      m.mu21, "mu12", m.mu12, "mu03", m.mu03, "nu20", m.nu20, "nu11", m.nu11,
      "nu02", m.nu02, "nu30", m.nu30, "nu21", m.nu21, "nu12", m.nu12, "nu03",
      m.nu03);
}
...
static void OnMouse(int event, int x, int y, int flags, void* param) {
  ...
  PyObject* args =
      Py_BuildValue("iiiiO", event, x, y, flags, PyTuple_GetItem(o, 1));
...
static PyObject* pycvSetMouseCallback(PyObject*, PyObject* args, PyObject* kw) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kw, "sO|O", (char**)keywords, &name,
                                   &on_mouse, &param))
    return NULL;
  ...
  ERRWRAP2(
      setMouseCallback(name, OnMouse, Py_BuildValue("OO", on_mouse, param)));
  Py_RETURN_NONE;
}
#endif
...
static PyObject* pycvCreateTrackbar(PyObject*, PyObject* args) {
  ...
  if (!PyArg_ParseTuple(args, "ssiiO", &trackbar_name, &window_name, value,
                        &count, &on_change))
    return NULL;
  ...
  ERRWRAP2(createTrackbar(trackbar_name, window_name, value, count, OnChange,
                          Py_BuildValue("OO", on_change, Py_None)));
  Py_RETURN_NONE;
}
...
static PyObject* pycvCreateButton(PyObject*, PyObject* args, PyObject* kw) {
  const char* keywords[] = {"buttonName", "onChange",           "userData",
                            "buttonType", "initialButtonState", NULL};
  PyObject* on_change;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kw, "sO|Oii", (char**)keywords,
                                   &button_name, &on_change, &userdata,
  ...
  ERRWRAP2(createButton(button_name, OnButtonChange,
                        Py_BuildValue("OO", on_change, userdata), button_type,
...
static int convert_to_char(PyObject* o, char* dst,
...
#define MKTYPE2(NAME)           \
  ...
  if (!to_ok(&pyopencv_##NAME##_Type)) return NULL;
#else
#define MKTYPE2(NAME)           \
  pyopencv_##NAME##_specials(); \
  if (!to_ok(&pyopencv_##NAME##_Type)) return
#endif
...
#ifdef HAVE_OPENCV_HIGHGUI
    {"createTrackbar", pycvCreateTrackbar, METH_VARARGS,
     "createTrackbar(trackbarName, windowName, value, count, onChange) -> "
     "None"},
    {"createButton", (PyCFunction)pycvCreateButton,
     METH_VARARGS | METH_KEYWORDS,
     "createButton(buttonName, onChange [, userData, buttonType, "
     "initialButtonState]) -> None"},
    {"setMouseCallback", (PyCFunction)pycvSetMouseCallback,
     METH_VARARGS | METH_KEYWORDS,
...
static struct PyModuleDef cv2_moduledef = {
    PyModuleDef_HEAD_INIT, MODULESTR, "Python wrapper for OpenCV.",
...
#if PY_MAJOR_VERSION >= 3
#define PUBLISH_OBJECT(name, type) \
  Py_INCREF(&type);                \
  PyModule_AddObject(m, name, (PyObject*)&type);
#else
// Unrolled Py_INCREF(&type) without (PyObject*) cast
// due to "warning: dereferencing type-punned pointer will break strict-aliasing
// rules"
#define PUBLISH_OBJECT(name, type)                          \
  _Py_INC_REFTOTAL _Py_REF_DEBUG_COMMA(&type)->ob_refcnt++; \
  PyModule_AddObject(m, name, (PyObject*)&type);
#endif
