...
class ROISelector {
 public:
  Rect select(const String &windowName, Mat img, bool showCrossair = true,
    ...
    while (!(key == 32 || key == 27 || key == 13)) {
      ...
      rectangle(selectorParams.image, selectorParams.box, Scalar(255, 0, 0), 2,
                1);
  ...
  void select(const String &windowName, Mat img,
    ...
    for (;;) {
      ...
      if (key == 27) break;
      if (temp.width > 0 && temp.height > 0) boundingBoxes.push_back(temp);
    }
  }
        ...
        if (selectorParams.isDrawing) {
          if (selectorParams.drawFromCenter) {
            ...
            selectorParams.box.x =
                std::min(std::max(selectorParams.center.x -
                                      selectorParams.box.width / 2.0,
                                  0.),
                         (double)imageSize.width);
            selectorParams.box.y =
                std::min(std::max(selectorParams.center.y -
                                      selectorParams.box.height / 2.0,
                                  0.),
                         (double)imageSize.height);
          } else {
            selectorParams.box.width = std::max(
                std::min(x - selectorParams.box.x,
                         (double)imageSize.width - selectorParams.box.x),
                -selectorParams.box.x);
            selectorParams.box.height = std::max(
                std::min(y - selectorParams.box.y,
                         (double)imageSize.height - selectorParams.box.y),
                -selectorParams.box.y);
          }
        }
        break;
...
Rect cv::selectROI(InputArray img, bool showCrosshair, bool fromCenter) {
  ...
  return selector.select("ROI selector", img.getMat(), showCrosshair,
                         fromCenter);
}

Rect cv::selectROI(const String &windowName, InputArray img, bool showCrosshair,
                   bool fromCenter) {
  ROISelector selector;
...
void cv::selectROIs(const String &windowName, InputArray img,
                    std::vector<Rect> &boundingBox, bool showCrosshair,
                    bool fromCenter) {
  ROISelector selector;
  selector.select(windowName, img.getMat(), boundingBox, showCrosshair,
                  fromCenter);
}
