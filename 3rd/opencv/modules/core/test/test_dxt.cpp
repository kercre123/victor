...
static void DFT_1D(const Mat& _src, Mat& _dst, int flags,
                   const Mat& _wave = Mat()) {
  ...
  if (wave.empty()) wave = initDFTWave(n, (flags & DFT_INVERSE) != 0);

  const Complexd* w = wave.ptr<Complexd>();
  if (!_src.isContinuous()) srcstep = _src.step;
  if (!_dst.isContinuous()) dststep = _dst.step;
...
static Mat initDCTWave(int n, bool inv) {
  ...
  for (k = 0; k < n; k++) wave.at<double>(0, k) = scale;
  scale *= sqrt(2.);
  for (i = 1; i < n; i++)
    for (k = 0; k < n; k++)
      wave.at<double>(i, k) = scale * cos(angle * i * (2 * k + 1));

  if (inv) cv::transpose(wave, wave);

  return wave;
}

static void DCT_1D(const Mat& _src, Mat& _dst, int flags,
                   const Mat& _wave = Mat()) {
  _dst.create(_src.size(), _src.type());
  ...
  if (wave.empty()) wave = initDCTWave(n, (flags & DFT_INVERSE) != 0);
  w = wave.ptr<double>();

  if (!_src.isContinuous()) srcstep = (int)(_src.step / _src.elemSize());
  if (!_dst.isContinuous()) dststep = (int)(_dst.step / _dst.elemSize());
  ...
  if (_src.type() == CV_32FC1) {
    ...
    for (i = 0; i < n; i++, dst += dststep) {
      ...
      for (j = 0; j < n; j++, src += srcstep) sum += src[0] * w[j];
      w += n;
      dst[0] = (float)sum;
    }
...
static void convertFromCCS(const Mat& _src0, const Mat& _src1, Mat& _dst,
                           int flags) {
  if (_dst.rows > 1 && (_dst.cols > 1 || (flags & DFT_ROWS))) {
    ...
    if (!_src0.isContinuous()) srcstep = (int)(_src0.step / _src0.elemSize1());
  ...
  for (i = 0; i < dst.rows; i++) {
    if (depth == CV_32F) {
      ...
      if (!(flags & CV_DXT_MUL_CONJ))
        for (j = 0; j < cols; j += 2) {
          double re =
              (double)a[j] * (double)b[j] - (double)a[j + 1] * (double)b[j + 1];
          double im =
              (double)a[j + 1] * (double)b[j] + (double)a[j] * (double)b[j + 1];
...
class CxCore_DXTBaseTest : public cvtest::ArrayTest {
 ...
 protected:
  ...
  bool temp_dst;       // use temporary destination (for real->ccs DFT and ccs
                       // MulSpectrums)
};

CxCore_DXTBaseTest::CxCore_DXTBaseTest(bool _allow_complex, bool _allow_odd,
                                       bool _spectrum_mode)
    : Base(),
      flags(0),
      allow_complex(_allow_complex),
      allow_odd(_allow_odd),
      spectrum_mode(_spectrum_mode),
      inplace(false),
      temp_dst(false) {
  test_array[INPUT].push_back(NULL);
  if (spectrum_mode) test_array[INPUT].push_back(NULL);
...
void CxCore_DXTBaseTest::get_test_array_types_and_sizes(
  ...
  flags =
      bits & (CV_DXT_INVERSE | CV_DXT_SCALE | CV_DXT_ROWS | CV_DXT_MUL_CONJ);
  if (spectrum_mode) flags &= ~CV_DXT_INVERSE;
  types[TEMP][0] = types[TEMP][1] = types[INPUT][0] = types[OUTPUT][0] =
  ...
  if (spectrum_mode) {
    ...
    if (flags & CV_DXT_INVERSE) {
      ...
      if (allow_complex) types[OUTPUT][0] = depth + 8;
  ...
  inplace = false;
  if (spectrum_mode || (!temp_dst && types[INPUT][0] == types[OUTPUT][0]) ||
...
double CxCore_DXTBaseTest::get_success_error_level(int test_case_idx, int i,
                                                   int j) {
...
void CxCore_DFTTest::prepare_to_validation(int /*test_case_idx*/) {
  ...
  if (src_cn != 2 || dst_cn != 2) {
    ...
    if (!(flags & CV_DXT_INVERSE)) {
      ...
      cvtest::convertFromCCS(cvdft_dst, cvdft_dst, test_mat[OUTPUT][0], flags);
  ...
  if (tmp_dst != &dst) cvtest::extract(*tmp_dst, dst, 0);
}
...
CxCore_MulSpectrumsTest::CxCore_MulSpectrumsTest()
    : CxCore_DXTBaseTest(true, true, true) {}

double CxCore_MulSpectrumsTest::get_success_error_level(int test_case_idx,
                                                        int i, int j) {
  (void)test_case_idx;
  CV_Assert(i == OUTPUT);
  CV_Assert(j == 0);
...
TEST(Core_DCT, accuracy) {
  CxCore_DCTTest test;
  test.safe_run();
}
TEST(Core_DFT, accuracy) {
  CxCore_DFTTest test;
  test.safe_run();
}
TEST(Core_MulSpectrums, accuracy) {
  CxCore_MulSpectrumsTest test;
  test.safe_run();
}
 ...
 protected:
  void run(int) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      Mat src8u(m, n, depth), src(m, n, depth),
          dst(m, n, CV_MAKETYPE(depth, 2));
...
TEST(Core_DFT, complex_output) {
  Core_DFTComplexOutputTest test;
  test.safe_run();
}
  ...
  void run(int) {
    for (int i = 0; i < 3; ++i) {
      ...
      for (int j = 0; j < 100; ++j) {
        ...
        if (out.channels() != cn_out || two.channels() != cn_in ||
            cvtest::norm(one, two, NORM_INF) > 1e-5) {
          cout << "Test #" << j + 1 << " - "
               << "elements: " << m << " x " << n << ", "
               << "channels: " << one.channels() << " (" << cn_in << ")"
               << " -> " << out.channels() << " (" << cn_out << ")"
               << " -> " << two.channels() << " (" << cn_in << ")" << endl;
          cout << "signal:\n" << one << endl << endl;
          cout << "spectrum:\n" << out << endl << endl;
          cout << "inverse:\n" << two << endl << endl;
...
TEST(Core_DFT, reverse) {
  Core_DXTReverseTest test(Core_DXTReverseTest::ModeDFT);
  test.safe_run();
}
TEST(Core_DCT, reverse) {
  Core_DXTReverseTest test(Core_DXTReverseTest::ModeDCT);
  test.safe_run();
}
