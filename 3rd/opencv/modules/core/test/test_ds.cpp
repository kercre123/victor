...
static CvTsSimpleSeq* cvTsCreateSimpleSeq(int max_count, int elem_size) {
  CvTsSimpleSeq* seq =
      (CvTsSimpleSeq*)cvAlloc(sizeof(*seq) + max_count * elem_size);
...
static void cvTsReleaseSimpleSeq(CvTsSimpleSeq** seq) { cvFree(seq); }
...
static void cvTsClearSimpleSeq(CvTsSimpleSeq* seq) { seq->count = 0; }

static void cvTsSimpleSeqShiftAndCopy(CvTsSimpleSeq* seq, int from_idx,
                                      int to_idx, void* elem = 0) {
  ...
  if (elem)
    memcpy(seq->array + from_idx * elem_size, elem,
           (to_idx - from_idx) * elem_size);
}
...
static void cvTsSimpleSeqInvert(CvTsSimpleSeq* seq) {
  ...
  for (i = 0; i < len / 2; i++) {
    ...
    for (k = 0; k < elem_size; k++) CV_SWAP(a[k], b[k], t);
  }
}
...
static CvTsSimpleSet* cvTsCreateSimpleSet(int max_count, int elem_size) {
  CvTsSimpleSet* set_header = (CvTsSimpleSet*)cvAlloc(
      sizeof(*set_header) + max_count * (elem_size + 1 + sizeof(int)));
...
static void cvTsSimpleSetRemove(CvTsSimpleSet* set_header, int index) {
  assert(set_header->free_count < set_header->max_count && 0 <= index &&
         index < set_header->max_count);
...
static CvTsSimpleGraph* cvTsCreateSimpleGraph(int max_vtx_count, int vtx_size,
  ...
  graph = (CvTsSimpleGraph*)cvAlloc(
      sizeof(*graph) + max_vtx_count * max_vtx_count * (edge_size + 1));
...
static void cvTsSimpleGraphAddEdge(CvTsSimpleGraph* graph, int idx1, int idx2,
                                   void* edge) {
...
static void cvTsSimpleGraphRemoveEdge(CvTsSimpleGraph* graph, int idx1,
                                      int idx2) {
  int i, t, n = graph->oriented ? 1 : 2;
...
static char* cvTsSimpleGraphFindEdge(CvTsSimpleGraph* graph, int idx1,
                                     int idx2) {
  if (cvTsSimpleGraphFindVertex(graph, idx1) &&
      cvTsSimpleGraphFindVertex(graph, idx2)) {
    char* edge = graph->matrix +
                 (idx1 * graph->vtx->max_count + idx2) * graph->edge_size;
...
class Core_DynStructBaseTest : public cvtest::BaseTest {
 ...
 protected:
  ...
  void run_func(void);
  void set_error_context(const char* condition, const char* err_msg,
...
Core_DynStructBaseTest::~Core_DynStructBaseTest() { clear(); }

void Core_DynStructBaseTest::run_func() {}

bool Core_DynStructBaseTest::can_do_fast_forward() { return false; }

void Core_DynStructBaseTest::clear() { cvtest::BaseTest::clear(); }
...
int Core_DynStructBaseTest::read_params(CvFileStorage* fs) {
  ...
  max_struct_size =
      cvReadInt(find_param(fs, "max_struct_size"), max_struct_size);
  ...
  min_log_storage_block_size = cvReadInt(
      find_param(fs, "min_log_storage_block_size"), min_log_storage_block_size);
  max_log_storage_block_size = cvReadInt(
      find_param(fs, "max_log_storage_block_size"), max_log_storage_block_size);
  min_log_elem_size =
      cvReadInt(find_param(fs, "min_log_elem_size"), min_log_elem_size);
  max_log_elem_size =
      cvReadInt(find_param(fs, "max_log_elem_size"), max_log_elem_size);
  ...
  min_log_storage_block_size =
      cvtest::clipInt(min_log_storage_block_size, 7, 20);
...
void Core_DynStructBaseTest::update_progressbar() {
  ...
  t = cv::getTickCount();
  test_progress =
      update_progress(test_progress, 0, 0, (double)(t - start_time) / cpu_freq);
}
...
int Core_DynStructBaseTest::test_seq_block_consistence(int _struct_idx,
                                                       CvSeq* seq, int total) {
  int sum = 0;
  ...
  if (seq->first) {
    ...
    for (;;) {
      CV_TS_SEQ_CHECK_CONDITION(
          sum == block->start_index - delta_idx && block->count > 0 &&
              block->prev == prev_block && prev_block->next == block,
    ...
    CV_TS_SEQ_CHECK_CONDITION(
        block->prev->count * seq->elem_size + block->prev->data <=
            seq->block_max,
...
Core_SeqBaseTest::~Core_SeqBaseTest() { clear(); }
...
int Core_SeqBaseTest::test_multi_create() {
  ...
  for (int i = 0; i < struct_count; i++) {
    ...
    t = cvtest::randReal(rng) * (max_log_elem_size - min_log_elem_size) +
        min_log_elem_size;
  ...
  for (cur_count = struct_count; cur_count > 0; cur_count--) {
    for (;;) {
      ...
      if (pos[struct_idx] < 0) {
        ...
        hdr_size =
            MIN(hdr_size, (int)(storage->block_size - sizeof(CvMemBlock)));
      ...
      if (pos[struct_idx] == sseq->count) {
        ...
        /* del index */
        for (; k < cur_count - 1; k++) index[k] = index[k + 1];
        break;
      }
...
int Core_SeqBaseTest::test_get_seq_elem(int _struct_idx, int iters) {
  ...
  if (sseq->count == 0) return 0;

  for (int i = 0; i < iters; i++) {
    int idx = cvtest::randInt(rng) % (sseq->count * 3) - sseq->count * 3 / 2;
    int idx0 = (unsigned)idx < (unsigned)(sseq->count)
                   ? idx
                   : idx < 0 ? idx + sseq->count : idx - sseq->count;
...
int Core_SeqBaseTest::test_get_seq_reading(int _struct_idx, int iters) {
  ...
  if (total == 0) {
    CV_TS_SEQ_CHECK_CONDITION(reader.ptr == 0,
                              "Empty sequence reader pointer is not NULL");
    return 0;
  }
  ...
  for (iter = 0; iter < iters; iter++) {
    ...
    if (op >= max_val - 2) {
      ...
      if (!bad_range) {
        ...
        CV_TS_SEQ_CHECK_CONDITION(pos == cvGetSeqReaderPos(&reader),
                                  "reader doesn't stay at the current position "
                                  "after wrong positioning");
      }
    } else {
      ...
      CV_TS_SEQ_CHECK_CONDITION(
          memcmp(elem, cvTsSimpleSeqElem(sseq, pos), sseq->elem_size) == 0,
  ...
  for (iter = 0; iter < iters; iter++) {
    ...
    switch (op) {
      ...
      case 2:  // push/pushfront/insert
        if (sseq->count == sseq->max_count) break;
        ...
        CV_TS_SEQ_CHECK_CONDITION(
            elem2 != 0, "The inserted element could not be retrieved");
        CV_TS_SEQ_CHECK_CONDITION(
            seq->total == sseq->count &&
      ...
      case 5:  // pop/popfront/remove
        ...
        if (sseq->count > 0) {
          ...
          CV_TS_SEQ_CHECK_CONDITION(
              elem2 != 0, "GetSeqElem fails after removing the element");

          CV_TS_SEQ_CHECK_CONDITION(
              memcmp(elem2, cvTsSimpleSeqElem(sseq, pos - (pos == sseq->count)),
                     elem_size) == 0,
              "The first shifted element is not correct after removing another "
              "element");
        } else {
          CV_TS_SEQ_CHECK_CONDITION(
              seq->first == 0,
      ...
      case 8:  // push [front] multi/insert slice
        ...
        pos = whence < 0
                  ...
                  : whence > 0
                        ? sseq->count
                        : (int)(cvtest::randInt(rng) % (sseq->count + 1));
        if (whence != 0) {
          ...
          cvMakeSeqHeaderForArray(CV_SEQ_KIND_GENERIC, sizeof(CvSeq),
                                  sseq->elem_size, elem, count, &header,
                                  &block);
        ...
        if (sseq->count > 0) {
          ...
          pos = count > 0 ? (int)(cvtest::randInt(rng) % count + pos)
                          : MAX(pos - 1, 0);
          elem2 = cvGetSeqElem(seq, pos);
          CV_TS_SEQ_CHECK_CONDITION(
              elem2 != 0, "multi push operation doesn't add elements");
          CV_TS_SEQ_CHECK_CONDITION(
              seq->total == sseq->count &&
                  memcmp(elem2, cvTsSimpleSeqElem(sseq, pos), elem_size) == 0,
      ...
      case 11:  // pop [front] multi
        ...
        whence = op - 10;
        pos = whence < 0 ? 0
                         : whence > 0 ? sseq->count - count
                                      : (int)(cvtest::randInt(rng) %
                                              (sseq->count - count + 1));
        ...
        if (whence != 0) {
          ...
          if (count > 0) {
            CV_TS_SEQ_CHECK_CONDITION(
                memcmp(elem, cvTsSimpleSeqElem(sseq, pos), elem_size) == 0,
                "The first (in the sequence order) removed element is wrong "
                "after popmulti");
          }
        } else {
      ...
      case 12:  // seqslice
      {
        ...
        CvSeq* seq_slice =
            cvSeqSlice(seq, cvSlice(pos, pos + count), storage, copy_data);
        ...
        if (count > 0) {
          ...
          CV_TS_SEQ_CHECK_CONDITION(
              elem2 && memcmp(elem2, cvTsSimpleSeqElem(sseq, pos + test_idx),
                              elem_size) == 0,
    ...
    if (test_seq_block_consistence(struct_idx, seq, sseq->count) < 0) return -1;

    if (test_get_seq_elem(struct_idx, 7) < 0) return -1;
...
void Core_SeqBaseTest::run(int) {
  try {
    ...
    for (gen = 0; gen < generations; gen++) {
      ...
      if (!storage) {
        t = cvtest::randReal(rng) *
                (max_log_storage_block_size - min_log_storage_block_size) +
            min_log_storage_block_size;
      ...
      for (i = 0; i < struct_count; i++) {
        if (test_seq_block_consistence(
                i, (CvSeq*)cxcore_struct[i],
                ((CvTsSimpleSeq*)simple_struct[i])->count) < 0)
          return;

        if (test_get_seq_elem(i, MAX(iterations / 3, 7)) < 0) return;

        if (test_get_seq_reading(i, MAX(iterations / 3, 7)) < 0) return;
        update_progressbar();
      }

      if (test_seq_ops(iterations) < 0) return;
    ...
    for (gen = 0; gen < generations; gen++) {
      ...
      for (iter = 0; iter < iterations / 10; iter++) {
        ...
        for (i = 0; i < struct_count; i++) {
          ...
          if (test_seq_block_consistence(i, seq, sseq->count) < 0) return;

          if (sseq->count > 0 && cvtest::randInt(rng) % 2 == 0) {
            slice.end_index = cvtest::randInt(rng) % sseq->count + 1;
            slice.start_index =
                cvtest::randInt(rng) % (sseq->count - slice.end_index + 1);
          ...
          for (k = 0; k < (sseq->count > 0 ? 10 : 0); k++) {
            ...
            elem2 = cvSeqSearch(seq, elem0, k % 2 ? icvCmpSeqElems : 0, 0, &idx,
                                seq);

            CV_TS_SEQ_CHECK_CONDITION(
                elem != 0 && memcmp(elem0, elem, seq->elem_size) == 0,
                "cvSeqInvert gives incorrect result");
            CV_TS_SEQ_CHECK_CONDITION(
                elem2 != 0 && memcmp(elem0, elem2, seq->elem_size) == 0 &&
...
Core_SetTest::~Core_SetTest() { clear(); }
...
int Core_SetTest::test_set_ops(int iters) {
  ...
  for (iter = 0; iter < iters; iter++) {
    ...
    if (iter > iters / 10 && cvtest::randInt(rng) % 200 == 0)  // clear set
    {
      ...
      CV_TS_SEQ_CHECK_CONDITION(
          cvset->active_count == 0 && cvset->total == 0 && cvset->first == 0 &&
              cvset->free_elems == 0 &&
    ...
    } else if (op == 0 || op == 1)  // add element
    {
      ...
      CV_TS_SEQ_CHECK_CONDITION(
          CV_IS_SET_ELEM(elem3) && idx == idx0 && elem3 == elem2 &&
              (!pass_data || memcmp((char*)elem3 + sizeof(int), elem_data,
                                    pure_elem_size) == 0),
          "The added element is not correct");

      CV_TS_SEQ_CHECK_CONDITION(
          (!first_free || elem3 == first_free) &&
    ...
    } else if (op == 2 || op == 3)  // remove element
    {
      ...
      if (sset->free_count == sset->max_count || idx >= sset->count) continue;

      elem_data = cvTsSimpleSetFind(sset, idx);
      if (elem_data == 0) continue;

      elem = cvGetSetElem(cvset, idx);
      CV_TS_SEQ_CHECK_CONDITION(
          CV_IS_SET_ELEM(elem) && elem->flags == idx &&
      ...
      CV_TS_SEQ_CHECK_CONDITION(
          !CV_IS_SET_ELEM(elem) && !cvGetSetElem(cvset, idx) &&
              (elem->flags & CV_SET_ELEM_IDX_MASK) == idx,
          "cvSetRemove[ByPtr] didn't release the element properly");

      CV_TS_SEQ_CHECK_CONDITION(
          elem->next_free == first_free && cvset->free_elems == elem &&
              cvset->active_count == prev_count - 1,
    ...
    CV_TS_SEQ_CHECK_CONDITION(
        cvset->active_count == sset->max_count - sset->free_count &&
            cvset->total >= cvset->active_count &&
            (cvset->total == 0 || cvset->total >= prev_total),
        "The total number of cvset elements is not correct");

    // CvSet and simple set do not necessary have the same "total" (active &
    // free) number, so pass "set->total" to skip that check
  ...
  try {
    ...
    for (gen = 0; gen < generations; gen++) {
      ...
      for (int i = 0; i < struct_count; i++) {
        ...
        elem_size =
            MIN(elem_size, (int)(storage->block_size - sizeof(void*) -
                                 sizeof(CvMemBlock) - sizeof(CvSeqBlock)));
        pure_elem_size =
            MIN(pure_elem_size, elem_size - (int)sizeof(CvSetElem));
      ...
      if (test_set_ops(iterations * 100) < 0) return;
...
Core_GraphTest::~Core_GraphTest() { clear(); }
...
int Core_GraphTest::test_graph_ops(int iters) {
  ...
  for (iter = 0; iter < iters; iter++) {
    ...
    int prev_vtx_total = graph->total, prev_edge_total = graph->edges->total,
    ...
    {
      int prev_vtx_count2 = graph->total,
          prev_edge_count2 = graph->edges->total;

      cvClearGraph(graph);
      cvTsClearSimpleGraph(sgraph);

      CV_TS_SEQ_CHECK_CONDITION(
          graph->active_count == 0 && graph->total == 0 && graph->first == 0 &&
              graph->free_elems == 0 &&
              (graph->free_blocks != 0 || prev_vtx_count2 == 0),
          "The graph is not empty after clearing");

      CV_TS_SEQ_CHECK_CONDITION(
          edges->active_count == 0 && edges->total == 0 && edges->first == 0 &&
              edges->free_elems == 0 &&
              (edges->free_blocks != 0 || prev_edge_count2 == 0),
          "The graph is not empty after clearing");
    } else if (op == 0)  // add vertex
    {
      if (sgraph->vtx->free_count == 0) continue;
      ...
      CV_TS_SEQ_CHECK_CONDITION(
          (CV_IS_SET_ELEM(vtx3) && vtx3->flags == idx && vtx3->first == 0) ||
              (idx == idx0 && vtx3 == vtx2 &&
      ...
      CV_TS_SEQ_CHECK_CONDITION(
          (!first_free || first_free == (CvSetElem*)vtx3) &&
    ...
    } else if (op == 1)  // remove vertex
    {
      ...
      if (sgraph->vtx->free_count == sgraph->vtx->max_count ||
          idx >= sgraph->vtx->count)
        continue;

      vtx_data = cvTsSimpleGraphFindVertex(sgraph, idx);
      if (vtx_data == 0) continue;
      ...
      CV_TS_SEQ_CHECK_CONDITION(
          CV_IS_SET_ELEM(vtx) && vtx->flags == idx &&
              (pure_vtx_size == 0 ||
               memcmp(vtx + 1, vtx_data, pure_vtx_size) == 0),
    ...
    } else if (op == 2)  // add edge
    {
      ...
      if (sgraph->vtx->free_count >= sgraph->vtx->max_count - 1) continue;
      ...
      CV_TS_SEQ_CHECK_CONDITION(vtx != 0 && vtx2 != 0 &&
                                    vtx->flags == v_idx[0] &&
                                    vtx2->flags == v_idx[1],
                                "Some of the vertices are missing");

      if (cvtest::randInt(rng) % 2) {
        v_prev_degree[0] = cvGraphVtxDegreeByPtr(graph, vtx);
        v_prev_degree[1] = cvGraphVtxDegreeByPtr(graph, vtx2);
        res =
            cvGraphAddEdgeByPtr(graph, vtx, vtx2, pass_data ? edge : 0, &edge2);
        ...
        res = cvGraphAddEdge(graph, v_idx[0], v_idx[1], pass_data ? edge : 0,
                             &edge2);
      ...
      CV_TS_SEQ_CHECK_CONDITION(
          res == 1 && edge2 != 0 && CV_IS_SET_ELEM(edge2) &&
              ((edge2->vtx[0] == vtx && edge2->vtx[1] == vtx2) ||
               (!CV_IS_GRAPH_ORIENTED(graph) && edge2->vtx[0] == vtx2 &&
                edge2->vtx[1] == vtx)) &&
              (!pass_data || pure_edge_size == 0 ||
               memcmp(edge2 + 1, edge + 1, pure_edge_size) == 0),
          "The edge has been added incorrectly");

      if (!pass_data) {
        if (pure_edge_size > 0) memcpy(edge2 + 1, edge + 1, pure_edge_size);
        edge2->weight = edge->weight;
      }

      CV_TS_SEQ_CHECK_CONDITION(
          v_degree[0] == v_prev_degree[0] + 1 &&
      ...
      CV_TS_SEQ_CHECK_CONDITION(
          (!first_free || first_free == (CvSetElem*)edge2) &&
    ...
    } else if (op == 3)  // find & remove edge
    {
      ...
      CV_TS_SEQ_CHECK_CONDITION(
          edge != 0 && edge->weight == v_idx[0] + v_idx[1] &&
              ((edge->vtx[0] == vtx && edge->vtx[1] == vtx2) ||
               (!CV_IS_GRAPH_ORIENTED(graph) && edge->vtx[1] == vtx &&
                edge->vtx[0] == vtx2)) &&
              (pure_edge_size == 0 ||
               memcmp(edge + 1, edge_data, pure_edge_size) == 0),
      ...
      CV_TS_SEQ_CHECK_CONDITION(
          graph->edges->free_elems == (CvSetElem*)edge &&
    ...
    CV_TS_SEQ_CHECK_CONDITION(
        graph->active_count ==
                sgraph->vtx->max_count - sgraph->vtx->free_count &&
            graph->total >= graph->active_count &&
            (graph->total == 0 || graph->total >= prev_vtx_total),
        "The total number of graph vertices is not correct");

    CV_TS_SEQ_CHECK_CONDITION(
        graph->edges->total >= graph->edges->active_count &&
            (graph->edges->total == 0 ||
             graph->edges->total >= prev_edge_total),
        "The total number of graph vertices is not correct");

    // CvGraph and simple graph do not necessary have the same "total" (active &
    // free) number, so pass "graph->total" (or "graph->edges->total") to skip
    // that check
    test_seq_block_consistence(struct_idx, (CvSeq*)graph, graph->total);
    test_seq_block_consistence(struct_idx, (CvSeq*)graph->edges,
                               graph->edges->total);
    update_progressbar();
  }
    ...
    for (gen = 0; gen < generations; gen++) {
      ...
      for (i = 0; i < struct_count; i++) {
        ...
        for (k = 0; k < 2; k++) {
          t = cvtest::randReal(rng) * (max_log_elem_size - min_log_elem_size) +
              min_log_elem_size;
          int pe = cvRound(exp(t * CV_LOG2)) -
                   1;  // pure_elem_size==0 does also make sense
        ...
        cvTsReleaseSimpleGraph((CvTsSimpleGraph**)&simple_struct[i]);
        simple_struct[i] =
            cvTsCreateSimpleGraph(max_struct_size / 4, pure_elem_size[0],
                                  pure_elem_size[1], is_oriented);
        cxcore_struct[i] =
            cvCreateGraph(is_oriented ? CV_ORIENTED_GRAPH : CV_GRAPH,
                          sizeof(CvGraph), elem_size[0], elem_size[1], storage);
      }

      if (test_graph_ops(iterations * 10) < 0) return;
...
int Core_GraphScanTest::create_random_graph(int _struct_idx) {
  ...
  cxcore_struct[_struct_idx] = graph =
      cvCreateGraph(is_oriented ? CV_ORIENTED_GRAPH : CV_GRAPH, sizeof(CvGraph),
                    sizeof(CvGraphVtx), sizeof(CvGraphEdge), storage);

  for (i = 0; i < vtx_count; i++) cvGraphAddVtx(graph);
  ...
  for (i = 0; i < edge_count; i++) {
    ...
    if (j != k) cvGraphAddEdge(graph, j, k);
  }

  assert(graph->active_count == vtx_count &&
         graph->edges->active_count <= edge_count);
...
void Core_GraphScanTest::run(int) {
  ...
  try {
    ...
    for (gen = 0; gen < generations; gen++) {
      ...
      storage_blocksize =
          MAX(storage_blocksize,
              (int)(sizeof(CvGraph) + sizeof(CvMemBlock) + sizeof(CvSeqBlock)));
      storage_blocksize = MAX(
          storage_blocksize,
          (int)(sizeof(CvGraphEdge) + sizeof(CvMemBlock) + sizeof(CvSeqBlock)));
      storage_blocksize = MAX(
          storage_blocksize,
          (int)(sizeof(CvGraphVtx) + sizeof(CvMemBlock) + sizeof(CvSeqBlock)));
      storage.reset(cvCreateMemStorage(storage_blocksize));

      if (gen == 0) {
        // special regression test for one sample graph.
        // !!! ATTENTION !!! The test relies on the particular order of the
        // inserted edges (LIFO: the edge inserted last goes first in the list
        // of incident edges). if it is changed, the test will have to be
        // modified.

        int vtx_count = -1, edge_count = 0,
            edges[][3] = {{0, 4, 'f'}, {0, 1, 't'}, {1, 4, 't'}, {1, 2, 't'},
                          {2, 3, 't'}, {4, 3, 'c'}, {3, 1, 'b'}, {5, 7, 't'},
                          {7, 5, 'b'}, {5, 6, 't'}, {6, 0, 'c'}, {7, 6, 'c'},
                          {6, 4, 'c'}, {-1, -1, 0}};

        CvGraph* graph =
            cvCreateGraph(CV_ORIENTED_GRAPH, sizeof(CvGraph),
                          sizeof(CvGraphVtx), sizeof(CvGraphEdge), storage);
        ...
        for (;;) {
          ...
          switch (code) {
            ...
            default:
              CV_TS_SEQ_CHECK_CONDITION(
                  0, "Invalid code appeared during graph scan");
          }
      ...
      for (iter = 0; iter < iterations; iter++) {
        ...
        for (i = 0; i < 2; i++) {
          CvGraphVtx* start_vtx =
              cvtest::randInt(rng) % 2 || graph->active_count == 0
                  ? 0
                  : cvGetGraphVtx(graph,
                                  cvtest::randInt(rng) % graph->active_count);
          ...
          for (;;) {
            ...
            if (code == CV_GRAPH_OVER)
              ...
              CV_TS_SEQ_CHECK_CONDITION(
                  edge_idx < graph->edges->active_count &&
              ...
              CV_TS_SEQ_CHECK_CONDITION(
                  vtx_idx < graph->active_count && vtx_mask[vtx_idx] == 0,
          ...
          CV_TS_SEQ_CHECK_CONDITION(
              cvtest::norm(Mat(vtx_mask), CV_L1) == graph->active_count &&
                  cvtest::norm(Mat(edge_mask), CV_L1) ==
...
TEST(Core_DS_Seq, basic_operations) {
  Core_SeqBaseTest test;
  test.safe_run();
}
TEST(Core_DS_Seq, sort_invert) {
  Core_SeqSortInvTest test;
  test.safe_run();
}
TEST(Core_DS_Set, basic_operations) {
  Core_SetTest test;
  test.safe_run();
}
TEST(Core_DS_Graph, basic_operations) {
  Core_GraphTest test;
  test.safe_run();
}
TEST(Core_DS_Graph, scan) {
  Core_GraphScanTest test;
  test.safe_run();
}
