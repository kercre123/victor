...
/// !!! NOTE !!! These tests happily avoid overflow cases & out-of-range
/// arguments so that output arrays contain neigher Inf's nor Nan's. Handling
/// such cases would require special modification of check function
/// (validate_test_results) => TBD.
/// Also, need some logarithmic-scale generation of input data. Right now it is
/// done (in some tests) by generating min/max boundaries for random data in
/// logarimithic scale, but within the same test case all the input array
...
double Core_MathTest::get_success_error_level(int /*test_case_idx*/, int i,
                                              int j) {
  return test_mat[i][j].depth() == CV_32F ? FLT_EPSILON * 128
                                          : DBL_EPSILON * 1024;
}

void Core_MathTest::get_test_array_types_and_sizes(
    int test_case_idx, vector<vector<Size> >& sizes,
    vector<vector<int> >& types) {
  RNG& rng = ts->get_rng();
  ...
  for (i = 0; i < test_array.size(); i++) {
    ...
    for (j = 0; j < count; j++) types[i][j] = type;
  }
...
Core_PowTest::Core_PowTest() { power = 0; }
...
void Core_PowTest::get_test_array_types_and_sizes(int test_case_idx,
  ...
  if (depth < CV_32F || cvtest::randInt(rng) % 8 == 0)
    ...
    power = i == 16 ? 1. / 3
                    : i == 15 ? 0.5
                              : i == 14 ? -0.5 : cvtest::randReal(rng) * 10 - 5;
  }
...
void Core_PowTest::get_minmax_bounds(int /*i*/, int /*j*/, int type,
...
void Core_PowTest::run_func() {
  if (!test_nd) {
    if (fabs(power - 1. / 3) <= DBL_EPSILON &&
        test_mat[INPUT][0].depth() == CV_32F) {
      ...
      case CV_8U:
        if (ipower < 0)
          for (j = 0; j < ncols; j++) {
            ...
            ((uchar*)b_data)[j] = (uchar)(
                val == 0 ? 255
                         : val == 1 ? 1 : val == 2 && ipower == -1 ? 1 : 0);
          }
        else
          for (j = 0; j < ncols; j++) {
            int val = ((uchar*)a_data)[j];
...
class Core_MatrixTest : public cvtest::ArrayTest {
 public:
  typedef cvtest::ArrayTest Base;
  Core_MatrixTest(int in_count, int out_count, bool allow_int,
                  bool scalar_output, int max_cn);
...
Core_MatrixTest::Core_MatrixTest(int in_count, int out_count, bool _allow_int,
                                 bool _scalar_output, int _max_cn)
    : allow_int(_allow_int), scalar_output(_scalar_output), max_cn(_max_cn) {
  int i;
  for (i = 0; i < in_count; i++) test_array[INPUT].push_back(NULL);
...
void Core_MatrixTest::get_test_array_types_and_sizes(
    int test_case_idx, vector<vector<Size> >& sizes,
    vector<vector<int> >& types) {
  RNG& rng = ts->get_rng();
  ...
  for (i = 0; i < test_array.size(); i++) {
    ...
    for (j = 0; j < count; j++) {
      types[i][j] = type;
      if (flag) sizes[i][j] = Size(4, 1);
    }
  }
}

double Core_MatrixTest::get_success_error_level(int test_case_idx, int i,
                                                int j) {
  int input_depth = test_mat[INPUT][0].depth();
  double input_precision =
      input_depth < CV_32F ? 0 : input_depth == CV_32F ? 5e-5 : 5e-10;
...
Core_DotProductTest::Core_DotProductTest()
    : Core_MatrixTest(2, 1, true, true, 4) {}

void Core_DotProductTest::run_func() {
  test_mat[OUTPUT][0].at<Scalar>(0, 0) =
      Scalar(cvDotProduct(test_array[INPUT][0], test_array[INPUT][1]));
}

void Core_DotProductTest::prepare_to_validation(int) {
  test_mat[REF_OUTPUT][0].at<Scalar>(0, 0) =
      Scalar(cvtest::crossCorr(test_mat[INPUT][0], test_mat[INPUT][1]));
}
...
Core_CrossProductTest::Core_CrossProductTest()
    : Core_MatrixTest(2, 1, false, false, 1) {}

void Core_CrossProductTest::get_test_array_types_and_sizes(
    int, vector<vector<Size> >& sizes, vector<vector<int> >& types) {
  RNG& rng = ts->get_rng();
  int depth = cvtest::randInt(rng) % 2 + CV_32F;
  int cn = cvtest::randInt(rng) & 1 ? 3 : 1, type = CV_MAKETYPE(depth, cn);
  CvSize sz;

  types[INPUT][0] = types[INPUT][1] = types[OUTPUT][0] = types[REF_OUTPUT][0] =
      type;
  ...
  sizes[INPUT][0] = sizes[INPUT][1] = sizes[OUTPUT][0] = sizes[REF_OUTPUT][0] =
      sz;
}

void Core_CrossProductTest::run_func() {
  cvCrossProduct(test_array[INPUT][0], test_array[INPUT][1],
                 test_array[OUTPUT][0]);
}
...
class Core_GEMMTest : public Core_MatrixTest {
 ...
 protected:
  void get_test_array_types_and_sizes(int test_case_idx,
                                      vector<vector<Size> >& sizes,
                                      vector<vector<int> >& types);
  void get_minmax_bounds(int /*i*/, int /*j*/, int /*type*/, Scalar& low,
                         Scalar& high);
...
void Core_GEMMTest::get_test_array_types_and_sizes(
    int test_case_idx, vector<vector<Size> >& sizes,
    vector<vector<int> >& types) {
  RNG& rng = ts->get_rng();
  Size sizeA;
...
void Core_GEMMTest::get_minmax_bounds(int /*i*/, int /*j*/, int /*type*/,
                                      Scalar& low, Scalar& high) {
  low = Scalar::all(-10.);
  high = Scalar::all(10.);
}
...
void Core_GEMMTest::prepare_to_validation(int) {
  cvtest::gemm(test_mat[INPUT][0], test_mat[INPUT][1], alpha,
               test_array[INPUT][4] ? test_mat[INPUT][4] : Mat(), beta,
               test_mat[REF_OUTPUT][0], tabc_flag);
}
...
Core_MulTransposedTest::Core_MulTransposedTest()
    : Core_MatrixTest(2, 1, false, false, 1) {
  test_case_count = 100;
  order = 0;
...
void Core_MulTransposedTest::get_test_array_types_and_sizes(
    int test_case_idx, vector<vector<Size> >& sizes,
    vector<vector<int> >& types) {
  RNG& rng = ts->get_rng();
  ...
  src_type =
      src_type == 0
          ...
          : src_type == 1
                ? CV_16U
                : src_type == 2 ? CV_16S : src_type == 3 ? CV_32F : CV_64F;
  ...
  if (bits & 1)
    ...
    if (bits & 2) sizes[INPUT][1].height = 1;
    if (bits & 4) sizes[INPUT][1].width = 1;
  }

  sizes[TEMP][0] = sizes[INPUT][0];
  types[INPUT][0] = src_type;
  types[OUTPUT][0] = types[REF_OUTPUT][0] = types[INPUT][1] = types[TEMP][0] =
      dst_type;

  order = (bits & 8) != 0;
  sizes[OUTPUT][0].width = sizes[OUTPUT][0].height =
      order == 0 ? sizes[INPUT][0].height : sizes[INPUT][0].width;
  sizes[REF_OUTPUT][0] = sizes[OUTPUT][0];
}

void Core_MulTransposedTest::get_minmax_bounds(int /*i*/, int /*j*/,
                                               int /*type*/, Scalar& low,
                                               Scalar& high) {
  low = cvScalarAll(-10.);
  high = cvScalarAll(10.);
...
void Core_MulTransposedTest::run_func() {
  cvMulTransposed(test_array[INPUT][0], test_array[OUTPUT][0], order,
                  test_array[INPUT][1]);
}
...
void Core_MulTransposedTest::prepare_to_validation(int) {
  ...
  cvtest::gemm(temp, temp, 1., Mat(), 0, test_mat[REF_OUTPUT][0],
               order == 0 ? GEMM_2_T : GEMM_1_T);
}
...
Core_TransformTest::Core_TransformTest()
    : Core_MatrixTest(3, 1, true, false, 4) {
  scale = 1;
  diagMtx = false;
}

void Core_TransformTest::get_test_array_types_and_sizes(
    int test_case_idx, vector<vector<Size> >& sizes,
    vector<vector<int> >& types) {
  RNG& rng = ts->get_rng();
  int bits = cvtest::randInt(rng);
  ...
  mattype = depth < CV_32S
                ? CV_32F
                : depth == CV_64F ? CV_64F : bits & 1 ? CV_32F : CV_64F;
...
double Core_TransformTest::get_success_error_level(int test_case_idx, int i,
                                                   int j) {
  ...
  return depth <= CV_8S
             ...
             : depth <= CV_32S
                   ? 9
                   : Base::get_success_error_level(test_case_idx, i, j);
}

void Core_TransformTest::run_func() {
  CvMat _m = test_mat[INPUT][1], _shift = test_mat[INPUT][2];
  cvTransform(test_array[INPUT][0], test_array[OUTPUT][0], &_m,
              _shift.data.ptr ? &_shift : 0);
}
...
void Core_TransformTest::prepare_to_validation(int) {
  ...
  cvtest::transform(test_mat[INPUT][0], test_mat[REF_OUTPUT][0], transmat,
                    shift);
}
...
void Core_TransformLargeTest::get_test_array_types_and_sizes(
    int test_case_idx, vector<vector<Size> >& sizes,
    vector<vector<int> >& types) {
  RNG& rng = ts->get_rng();
  int bits = cvtest::randInt(rng);
...
Core_PerspectiveTransformTest::Core_PerspectiveTransformTest()
    : Core_MatrixTest(2, 1, false, false, 2) {}

void Core_PerspectiveTransformTest::get_test_array_types_and_sizes(
    int test_case_idx, vector<vector<Size> >& sizes,
    vector<vector<int> >& types) {
  RNG& rng = ts->get_rng();
  int bits = cvtest::randInt(rng);
...
double Core_PerspectiveTransformTest::get_success_error_level(int test_case_idx,
                                                              int i, int j) {
  int depth = test_mat[INPUT][0].depth();
  return depth == CV_32F
             ? 1e-4
             : depth == CV_64F ? 1e-8
                               : Core_MatrixTest::get_success_error_level(
                                     test_case_idx, i, j);
}
...
static void cvTsPerspectiveTransform(const CvArr* _src, CvArr* _dst,
  ...
  if (mat_depth == CV_32F) {
    for (i = 0; i < transmat->rows; i++)
      for (j = 0; j < cols; j++)
        mat[i * cols + j] =
            ((float*)(transmat->data.ptr + transmat->step * i))[j];
  } else {
    assert(mat_depth == CV_64F);
    for (i = 0; i < transmat->rows; i++)
      for (j = 0; j < cols; j++)
        mat[i * cols + j] =
            ((double*)(transmat->data.ptr + transmat->step * i))[j];
  }
  ...
  for (i = 0; i < a->rows; i++) {
    ...
    switch (depth) {
      case CV_32F:
        for (j = 0; j < cols; j++) buf[j] = ((float*)src)[j];
        break;
      case CV_64F:
        for (j = 0; j < cols; j++) buf[j] = ((double*)src)[j];
        break;
      default:
        assert(0);
    }
    ...
    switch (cn) {
      ...
      case 3:
        for (j = 0; j < cols; j += 3) {
          double t0 = buf[j] * mat[0] + buf[j + 1] * mat[1] +
                      buf[j + 2] * mat[2] + mat[3];
          double t1 = buf[j] * mat[4] + buf[j + 1] * mat[5] +
                      buf[j + 2] * mat[6] + mat[7];
          double t2 = buf[j] * mat[8] + buf[j + 1] * mat[9] +
                      buf[j + 2] * mat[10] + mat[11];
          double w = buf[j] * mat[12] + buf[j + 1] * mat[13] +
                     buf[j + 2] * mat[14] + mat[15];
          w = w ? 1. / w : 0;
          buf[j] = t0 * w;
          buf[j + 1] = t1 * w;
          buf[j + 2] = t2 * w;
    ...
    switch (depth) {
      case CV_32F:
        for (j = 0; j < cols; j++) ((float*)dst)[j] = (float)buf[j];
        break;
      case CV_64F:
        for (j = 0; j < cols; j++) ((double*)dst)[j] = buf[j];
        break;
      default:
        assert(0);
    }
...
void Core_PerspectiveTransformTest::prepare_to_validation(int) {
  ...
  cvTsPerspectiveTransform(test_array[INPUT][0], test_array[REF_OUTPUT][0],
                           &transmat);
}
...
Core_MahalanobisTest::Core_MahalanobisTest()
    : Core_MatrixTest(3, 1, false, true, 1) {
  test_case_count = 100;
  test_array[TEMP].push_back(NULL);
  test_array[TEMP].push_back(NULL);
  test_array[TEMP].push_back(NULL);
...
void Core_MahalanobisTest::get_test_array_types_and_sizes(
    int test_case_idx, vector<vector<Size> >& sizes,
    vector<vector<int> >& types) {
  RNG& rng = ts->get_rng();
  Core_MatrixTest::get_test_array_types_and_sizes(test_case_idx, sizes, types);
  ...
  sizes[INPUT][2].width = sizes[INPUT][2].height =
      sizes[INPUT][0].width + sizes[INPUT][0].height - 1;
...
int Core_MahalanobisTest::prepare_test_case(int test_case_idx) {
  int code = Base::prepare_test_case(test_case_idx);
  if (code > 0) {
    // make sure that the inverted "covariation" matrix is symmetrix and
    // positively defined.
    cvtest::gemm(test_mat[INPUT][2], test_mat[INPUT][2], 1., Mat(), 0.,
                 test_mat[TEMP][2], GEMM_2_T);
...
void Core_MahalanobisTest::run_func() {
  test_mat[OUTPUT][0].at<Scalar>(0, 0) = cvRealScalar(cvMahalanobis(
      test_array[INPUT][0], test_array[INPUT][1], test_array[INPUT][2]));
}

void Core_MahalanobisTest::prepare_to_validation(int) {
  cvtest::add(test_mat[INPUT][0], 1., test_mat[INPUT][1], -1., Scalar::all(0),
              test_mat[TEMP][0], test_mat[TEMP][0].type());
  if (test_mat[INPUT][0].rows == 1)
    cvtest::gemm(test_mat[TEMP][0], test_mat[INPUT][2], 1., Mat(), 0.,
                 test_mat[TEMP][1], 0);
  else
    cvtest::gemm(test_mat[INPUT][2], test_mat[TEMP][0], 1., Mat(), 0.,
                 test_mat[TEMP][1], 0);

  test_mat[REF_OUTPUT][0].at<Scalar>(0, 0) = cvRealScalar(
      sqrt(cvtest::crossCorr(test_mat[TEMP][0], test_mat[TEMP][1])));
}
...
Core_CovarMatrixTest::Core_CovarMatrixTest()
    : Core_MatrixTest(1, 1, true, false, 1),
      flags(0),
      t_flag(0),
      len(0),
      count(0),
...
void Core_CovarMatrixTest::get_test_array_types_and_sizes(
    int test_case_idx, vector<vector<Size> >& sizes,
    vector<vector<int> >& types) {
  RNG& rng = ts->get_rng();
  int bits = cvtest::randInt(rng);
  ...
  flags = bits &
          (CV_COVAR_NORMAL | CV_COVAR_USE_AVG | CV_COVAR_SCALE | CV_COVAR_ROWS);
  ...
  if (single_matrix && t_flag) flags = (flags & ~CV_COVAR_ROWS) | CV_COVAR_COLS;
  ...
  sizes[OUTPUT][0] = sizes[REF_OUTPUT][0] =
      flags & CV_COVAR_NORMAL ? Size(len, len) : Size(count, count);
  sizes[INPUT_OUTPUT][0] = sizes[REF_INPUT_OUTPUT][0] =
      !t_flag ? Size(len, 1) : Size(1, len);
  sizes[TEMP][0] = sizes[INPUT][0];

  types[INPUT_OUTPUT][0] = types[REF_INPUT_OUTPUT][0] = types[OUTPUT][0] =
      types[REF_OUTPUT][0] = types[TEMP][0] =
          CV_MAT_DEPTH(types[INPUT][0]) == CV_64F || (bits & 512) ? CV_64F
                                                                  : CV_32F;

  are_images = (bits & 1024) != 0;
  for (i = 0; i < (single_matrix ? 1 : count); i++) temp_hdrs.push_back(NULL);
}
...
void Core_CovarMatrixTest::run_func() {
  cvCalcCovarMatrix((const void**)&temp_hdrs[0], count, test_array[OUTPUT][0],
                    test_array[INPUT_OUTPUT][0], flags);
}
...
void Core_CovarMatrixTest::prepare_to_validation(int) {
  ...
  cvtest::add(test_mat[INPUT][0], 1, temp0, -1, Scalar::all(0), temp0,
              temp0.type());

  cvtest::gemm(
      temp0, temp0, scale, Mat(), 0., test_mat[REF_OUTPUT][0],
...
void Core_DetTest::get_test_array_types_and_sizes(int test_case_idx,
                                                  vector<vector<Size> >& sizes,
                                                  vector<vector<int> >& types) {
  Base::get_test_array_types_and_sizes(test_case_idx, sizes, types);
...
void Core_DetTest::get_minmax_bounds(int /*i*/, int /*j*/, int /*type*/,
                                     Scalar& low, Scalar& high) {
  low = cvScalarAll(-2.);
  high = cvScalarAll(2.);
}

double Core_DetTest::get_success_error_level(int /*test_case_idx*/, int /*i*/,
                                             int /*j*/) {
  return CV_MAT_DEPTH(cvGetElemType(test_array[INPUT][0])) == CV_32F ? 1e-2
                                                                     : 1e-5;
}

int Core_DetTest::prepare_test_case(int test_case_idx) {
  int code = Core_MatrixTest::prepare_test_case(test_case_idx);
  if (code > 0) cvTsFloodWithZeros(test_mat[INPUT][0], ts->get_rng());
...
void Core_DetTest::run_func() {
  test_mat[OUTPUT][0].at<Scalar>(0, 0) =
      cvRealScalar(cvDet(test_array[INPUT][0]));
}

// LU method that chooses the optimal in a column pivot element
static double cvTsLU(CvMat* a, CvMat* b = NULL, CvMat* x = NULL,
                     int* rank = 0) {
  int i, j, k, N = a->rows, N1 = a->cols, Nm = MIN(N, N1),
               step = a->step / sizeof(double);
  ...
  double t, det = 1.;
  assert(CV_MAT_TYPE(a->type) == CV_64FC1 && (!b || CV_ARE_TYPES_EQ(a, b)) &&
         (!x || CV_ARE_TYPES_EQ(a, x)));
  ...
  for (i = 0; i < Nm; i++) {
    ...
    if (k != i) {
      for (j = i; j < N1; j++) CV_SWAP(a0[i * step + j], a0[k * step + j], t);
    ...
    for (j = i + 1; j < N; j++, a2 += step, b2 += b_step) {
      t = a2[i] / a1[i];
      for (k = i + 1; k < N1; k++) a2[k] -= t * a1[k];

      for (k = 0; k < M; k++) b2[k] -= t * b1[k];
    }
  ...
  if (x) {
    ...
    for (i = N - 1; i >= 0; i--) {
      ...
      for (j = 0; j < M; j++) {
        t = b1[j];
        for (k = i + 1; k < N1; k++) t -= a1[k] * x0[k * x_step + j];
...
void Core_InvertTest::get_test_array_types_and_sizes(
    int test_case_idx, vector<vector<Size> >& sizes,
    vector<vector<int> >& types) {
  RNG& rng = ts->get_rng();
  int bits = cvtest::randInt(rng);
...
double Core_InvertTest::get_success_error_level(int /*test_case_idx*/, int,
                                                int) {
  return CV_MAT_DEPTH(cvGetElemType(test_array[OUTPUT][0])) == CV_32F ? 1e-2
                                                                      : 1e-6;
}
...
int Core_InvertTest::prepare_test_case(int test_case_idx) {
  ...
  if (code > 0) {
    ...
    if (method == CV_CHOLESKY) {
      cvtest::gemm(test_mat[INPUT][0], test_mat[INPUT][0], 1., Mat(), 0.,
                   test_mat[TEMP][0], CV_GEMM_B_T);
...
void Core_InvertTest::get_minmax_bounds(int /*i*/, int /*j*/, int /*type*/,
                                        Scalar& low, Scalar& high) {
  low = cvScalarAll(-1.);
  high = cvScalarAll(1.);
}
...
static double cvTsSVDet(CvMat* mat, double* ratio) {
  ...
  if (type == CV_32FC1) {
    for (i = 0; i < nm; i++) det *= w->data.fl[i];
    *ratio = w->data.fl[nm - 1] < FLT_EPSILON
                 ? 0
                 : w->data.fl[nm - 1] / w->data.fl[0];
  } else {
    for (i = 0; i < nm; i++) det *= w->data.db[i];
    *ratio = w->data.db[nm - 1] < FLT_EPSILON
                 ? 0
                 : w->data.db[nm - 1] / w->data.db[0];
  }
...
void Core_InvertTest::prepare_to_validation(int) {
  ...
  double threshold =
      (input.depth() == CV_32F ? FLT_EPSILON : DBL_EPSILON) * 1000;

  cvtest::convert(input, temp1, temp1.type());

  if (det < threshold ||
      ((method == CV_LU || method == CV_CHOLESKY) &&
       (result == 0 || ratio < threshold)) ||
...
Core_SolveTest::Core_SolveTest()
    : Core_MatrixTest(2, 1, false, false, 1), method(0), rank(0), result(0.) {
  test_case_count = 100;
  max_log_array_size = 7;
  test_array[TEMP].push_back(NULL);
  test_array[TEMP].push_back(NULL);
...
void Core_SolveTest::get_test_array_types_and_sizes(
    int test_case_idx, vector<vector<Size> >& sizes,
    vector<vector<int> >& types) {
  RNG& rng = ts->get_rng();
  int bits = cvtest::randInt(rng);
  ...
  if (in_sz.width > in_sz.height) in_sz = cvSize(in_sz.height, in_sz.width);
  Base::get_test_array_types_and_sizes(test_case_idx, sizes, types);
  sizes[INPUT][0] = in_sz;
  int min_size = MIN(sizes[INPUT][0].width, sizes[INPUT][0].height);
  ...
  types[TEMP][1] = CV_64FC1;
  sizes[OUTPUT][0] = sizes[REF_OUTPUT][0] =
      Size(sizes[INPUT][1].width, min_size);
}
...
void Core_SolveTest::get_minmax_bounds(int /*i*/, int /*j*/, int /*type*/,
                                       Scalar& low, Scalar& high) {
  low = cvScalarAll(-1.);
  high = cvScalarAll(1.);
}

double Core_SolveTest::get_success_error_level(int /*test_case_idx*/, int,
                                               int) {
  return CV_MAT_DEPTH(cvGetElemType(test_array[OUTPUT][0])) == CV_32F ? 5e-2
                                                                      : 1e-8;
}

void Core_SolveTest::run_func() {
  result = cvSolve(test_array[INPUT][0], test_array[INPUT][1],
                   test_array[TEMP][0], method);
}
...
void Core_SolveTest::prepare_to_validation(int) {
  ...
  if (method == CV_LU) {
    ...
    double threshold =
        (input.type() == CV_32F ? FLT_EPSILON : DBL_EPSILON) * 1000;
    CvMat _input = input;
    double ratio = 0, det = cvTsSVDet(&_input, &ratio);
  ...
  Mat* pdst =
      input.rows <= input.cols ? &test_mat[OUTPUT][0] : &test_mat[INPUT][1];

  cvtest::gemm(input, test_mat[TEMP][0], 1., test_mat[INPUT][1], -1., *pdst, 0);
  if (pdst != &dst) cvtest::gemm(input, *pdst, 1., Mat(), 0., dst, CV_GEMM_A_T);
  dst0 = Scalar::all(0);
}
...
void Core_SVDTest::get_test_array_types_and_sizes(int test_case_idx,
                                                  vector<vector<Size> >& sizes,
                                                  vector<vector<int> >& types) {
  RNG& rng = ts->get_rng();
  int bits = cvtest::randInt(rng);
  ...
  if (symmetric) sizes[INPUT][0] = Size(min_size, min_size);
  ...
  types[TEMP][0] = types[TEMP][1] = types[TEMP][2] = types[TEMP][3] =
      types[INPUT][0];
  ...
  sizes[OUTPUT][1] =
      !have_u ? Size(0, 0) : compact ? Size(min_size, min_size) : Size(m, m);
  sizes[OUTPUT][2] =
      !have_v ? Size(0, 0) : compact ? Size(min_size, min_size) : Size(n, n);
...
void Core_SVDTest::get_minmax_bounds(int /*i*/, int /*j*/, int /*type*/,
                                     Scalar& low, Scalar& high) {
  low = cvScalarAll(-2.);
  high = cvScalarAll(2.);
}
...
double Core_SVDTest::get_success_error_level(int test_case_idx, int i, int j) {
  ...
  double input_precision =
      input_depth < CV_32F ? 0 : input_depth == CV_32F ? 1e-5 : 5e-11;
  double output_precision = Base::get_success_error_level(test_case_idx, i, j);
  return MAX(input_precision, output_precision);
}
...
void Core_SVDTest::run_func() {
  ...
  if (!src) src = test_array[INPUT][0];
  cvSVD(src, test_array[TEMP][0], test_array[TEMP][1], test_array[TEMP][2],
        flags);
}
...
void Core_SVDTest::prepare_to_validation(int /*test_case_idx*/) {
  ...
  if (have_u) {
    ...
    cvtest::gemm(*src, *src, 1., Mat(), 0., *dst,
                 src->rows == dst->rows ? CV_GEMM_B_T : CV_GEMM_A_T);
  ...
  for (i = 0; i < min_size; i++) {
    ...
    test_mat[OUTPUT][3].at<uchar>(i) =
        aii >= 0 && normval < threshold && (i == 0 || aii <= prev);
    prev = aii;
  }
  ...
  if (have_u && have_v) {
    ...
    if (m >= n) {
      cvtest::gemm(test_mat[TEMP][1], *w, 1., Mat(), 0.,
      ...
      cvtest::gemm(*w, test_mat[TEMP][2], 1., Mat(), 0.,
                   test_mat[REF_OUTPUT][0],
...
Core_SVBkSbTest::Core_SVBkSbTest()
    : Core_MatrixTest(2, 1, false, false, 1),
      flags(0),
      have_b(false),
      symmetric(false),
...
void Core_SVBkSbTest::get_test_array_types_and_sizes(
    int test_case_idx, vector<vector<Size> >& sizes,
    vector<vector<int> >& types) {
  RNG& rng = ts->get_rng();
  int bits = cvtest::randInt(rng);
...
int Core_SVBkSbTest::prepare_test_case(int test_case_idx) {
  ...
  if (code > 0) {
    ...
    CvMat _input = input;
    cvSVD(&_input, test_array[TEMP][0], test_array[TEMP][1],
          test_array[TEMP][2], flags);
  }

  return code;
}

void Core_SVBkSbTest::get_minmax_bounds(int /*i*/, int /*j*/, int /*type*/,
                                        Scalar& low, Scalar& high) {
  low = cvScalarAll(-2.);
  high = cvScalarAll(2.);
}

double Core_SVBkSbTest::get_success_error_level(int /*test_case_idx*/,
                                                int /*i*/, int /*j*/) {
  return CV_MAT_DEPTH(cvGetElemType(test_array[INPUT][0])) == CV_32F ? 1e-3
                                                                     : 1e-7;
}
...
void Core_SVBkSbTest::prepare_to_validation(int) {
  ...
  double threshold =
      cv::sum(w)[0] *
      (DBL_EPSILON * 2);  //(is_float ? FLT_EPSILON*10 : DBL_EPSILON*2);
  ...
  if (is_float) {
    ...
    if (!b.empty()) test_mat[INPUT][1].convertTo(b, CV_64F);
  }

  Mat t0, t1;

  if (!b.empty())
    cvtest::gemm(u, b, 1., Mat(), 0., t0,
                 !(flags & CV_SVD_U_T) ? CV_GEMM_A_T : 0);
...
struct pred_complex {
  bool operator()(const complex_type& lhs, const complex_type& rhs) const {
    return fabs(lhs.real() - rhs.real()) > fabs(rhs.real()) * FLT_EPSILON
               ? lhs.real() < rhs.real()
               : lhs.imag() < rhs.imag();
  }
};
...
void Core_SolvePolyTest::run(int) {
  ...
  for (int idx = 0, max_idx = 1000, progress = 0; idx < max_idx; ++idx) {
    ...
    for (int maxiter = 100; !pass && maxiter < 10000;
         maxiter *= 2, cubic_case = (cubic_case + 1) % 2) {
      for (int j = 0; j < n + 1; ++j) a[j] = c[j].real();
      ...
      for (int j = 0; j < n; ++j) ar[j] = complex_type(u[j * 2], u[j * 2 + 1]);
      ...
      if (n == 3) {
        ...
        cvFlip(&amat, &amat, 0);
        if (nr2 > 0) std::sort(ar2.begin(), ar2.begin() + nr2, pred_double());
        ar2.resize(nr2);

        int nr1 = 0;
        for (int j = 0; j < n; j++)
          if (fabs(r[j].imag()) < DBL_EPSILON) ar1[nr1++] = r[j].real();
      ...
      for (int j = 0; j < n; ++j) {
        ...
        div += sqrt(pow(r[j].real() - ar[j].real(), 2) +
    ...
    pass = pass && r_5623.at<double>(0) == 0 && r_5623.at<double>(1) == 0 &&
           r_5623.at<double>(2) == 0;
...
class Core_PhaseTest : public cvtest::BaseTest {
 ...
 protected:
  virtual void run(int) {
    ...
    Mat x(axisCount + 1, dim, t), y(axisCount + 1, dim, t);
    ...
    if (outOfRangeCount > 0) {
      ts->printf(cvtest::TS::LOG,
                 "There are result angles that are out of range [0, 360] (part "
                 "of them is %f)\n",
    ...
    size_t errDegCount =
        diff.total() -
        countNonZero((diff < maxAngleDiff) | ((360 - diff) < maxAngleDiff));
    if (errDegCount > 0) {
      ts->printf(cvtest::TS::LOG,
                 "There are incorrect result angles (in degrees) (part of them "
                 "is %f)\n",
...
template <typename T>
class Core_CheckRange : public testing::Test {};
...
TYPED_TEST_P(Core_CheckRange, Negative3CN) {
  ...
  TypeParam data[] = {5,  6, 7, 10, 11, 12, 13, 14, 15, 10, 11, 12, 10, 11,
                      12, 2, 5, 6,  8,  8,  8,  12, 12, 12, 14, 14, 14};
  cv::Mat src =
      cv::Mat(3, 3, CV_MAKETYPE(cv::DataDepth<TypeParam>::value, 3), data);
...
REGISTER_TYPED_TEST_CASE_P(Core_CheckRange, Negative, Negative3CN, Positive,
                           Bounds, Zero, One);

typedef ::testing::Types<signed char, unsigned char, signed short,
...
TEST(Core_Invert, small) {
  cv::Mat a = (cv::Mat_<float>(3, 3) << 2.42104644730331, 1.81444796521479,
               -3.98072565304758, 0, 7.08389214348967e-3, 5.55326770986007e-3,
               0, 0, 7.44556154284261e-3);
  // cv::randu(a, -1, 1);
...
TEST(Core_CovarMatrix, accuracy) {
  Core_CovarMatrixTest test;
  test.safe_run();
}
TEST(Core_CrossProduct, accuracy) {
  Core_CrossProductTest test;
  test.safe_run();
}
TEST(Core_Determinant, accuracy) {
  Core_DetTest test;
  test.safe_run();
}
TEST(Core_DotProduct, accuracy) {
  Core_DotProductTest test;
  test.safe_run();
}
TEST(Core_GEMM, accuracy) {
  Core_GEMMTest test;
  test.safe_run();
}
TEST(Core_Invert, accuracy) {
  Core_InvertTest test;
  test.safe_run();
}
TEST(Core_Mahalanobis, accuracy) {
  Core_MahalanobisTest test;
  test.safe_run();
}
TEST(Core_MulTransposed, accuracy) {
  Core_MulTransposedTest test;
  test.safe_run();
}
TEST(Core_Transform, accuracy) {
  Core_TransformTest test;
  test.safe_run();
}
TEST(Core_TransformLarge, accuracy) {
  Core_TransformLargeTest test;
  test.safe_run();
}
TEST(Core_PerspectiveTransform, accuracy) {
  Core_PerspectiveTransformTest test;
  test.safe_run();
}
TEST(Core_Pow, accuracy) {
  Core_PowTest test;
  test.safe_run();
}
TEST(Core_SolveLinearSystem, accuracy) {
  Core_SolveTest test;
  test.safe_run();
}
TEST(Core_SVD, accuracy) {
  Core_SVDTest test;
  test.safe_run();
}
TEST(Core_SVBkSb, accuracy) {
  Core_SVBkSbTest test;
  test.safe_run();
}
TEST(Core_Trace, accuracy) {
  Core_TraceTest test;
  test.safe_run();
}
TEST(Core_SolvePoly, accuracy) {
  Core_SolvePolyTest test;
  test.safe_run();
}
TEST(Core_Phase, accuracy32f) {
  Core_PhaseTest test(CV_32FC1);
  test.safe_run();
}
TEST(Core_Phase, accuracy64f) {
  Core_PhaseTest test(CV_64FC1);
  test.safe_run();
}

TEST(Core_SVD, flt) {
  float a[] = {1.23377746e+011f,  -7.05490125e+010f, -4.18380882e+010f,
               -11693456.f,       -39091328.f,       77492224.f,
               -7.05490125e+010f, 2.36211143e+011f,  -3.51093473e+010f,
               70773408.f,        -4.83386156e+005f, -129560368.f,
               -4.18380882e+010f, -3.51093473e+010f, 9.25311222e+010f,
               -49052424.f,       43922752.f,        12176842.f,
               -11693456.f,       70773408.f,        -49052424.f,
               8.40836094e+004f,  5.17475293e+003f,  -1.16122949e+004f,
               -39091328.f,       -4.83386156e+005f, 43922752.f,
               5.17475293e+003f,  5.16047969e+004f,  5.68887842e+003f,
               77492224.f,        -129560368.f,      12176842.f,
               -1.16122949e+004f, 5.68887842e+003f,  1.28060578e+005f};

  float b[] = {283751232.f,      2.61604198e+009f,  -745033216.f,
               2.31125625e+005f, -4.52429188e+005f, -1.37596525e+006f};
...

    {
      ...
      for (iter = 0; iter < maxIter; iter++) {
        ...
        if (inVariant == VECTOR) {
          ...
          for (i = 0; i < N; i++) data[i] = data0[rng.uniform(0, N0)];

          kmeans(
              data, K, labels,
              TermCriteria(TermCriteria::MAX_ITER + TermCriteria::EPS, 30, 0),
          ...
          switch (inVariant) {
            ...
            case MAT_N_1_CDIM:
              ...
              for (i = 0; i < N; i++)
                memcpy(data.ptr(i), data0.ptr(rng.uniform(0, N0)),
                       dims * sizeof(float));
              break;
            ...
            case MAT_1_N_CDIM:
              ...
              for (i = 0; i < N; i++)
                memcpy(data.ptr() + i * dims * sizeof(float),
                       data0.ptr(rng.uniform(0, N0)), dims * sizeof(float));
              break;
        ...
        for (i = 0; i < K; i++) ASSERT_GT(hist.at<int>(i), 0);
      }
    }
  }
};

TEST(Core_KMeans, singular) {
  CV_KMeansSingularTest test;
  test.safe_run(MAT_N_DIM_C1);
}

CV_ENUM(KMeansInputVariant, MAT_N_DIM_C1, MAT_N_1_CDIM, MAT_1_N_CDIM,
        MAT_N_DIM_C1_NONCONT, MAT_N_1_CDIM_NONCONT, VECTOR)
...
INSTANTIATE_TEST_CASE_P(AllVariants, Core_KMeans_InputVariants,
                        KMeansInputVariant::all());

TEST(Core_KMeans, compactness) {
  const int N = 1024;
  const int attempts = 4;
  const TermCriteria crit =
      TermCriteria(TermCriteria::COUNT, 5, 0);  // low number of iterations
  ...
  for (int K = 1; K <= N; K *= 2) {
    ...
    Mat labels, centers;
    double compactness =
        kmeans(data, K, labels, crit, attempts, KMEANS_PP_CENTERS, centers);
...
TEST(Core_Pow, special) {
  for (int i = 0; i < 100; i++) {
    ...
    // generate power from [-n, n] interval with 1/8 step - enough to check
    // various cases.
    const int max_pf = 3;
    ...
    for (int j = 0; j < n; j++) {
      double val =
          type == CV_32F ? (double)mtx.at<float>(j) : mtx.at<double>(j);
      double r =
          type == CV_32F ? (double)result.at<float>(j) : result.at<double>(j);
      double r0;
      if (power == 0.)
        r0 = 1;
      else if (is_ipower) {
        r0 = 1;
        for (int k = 0; k < std::abs(ipower); k++) r0 *= val;
        if (ipower < 0) r0 = 1. / r0;
      } else
...
TEST(Core_Cholesky, accuracy64f) {
  ...
  *dev.ptr<double>() = 10.0;
  ...
  for (int i = 0; i < A.rows; i++)
    for (int j = i + 1; j < A.cols; j++) A.at<double>(i, j) = 0.0;
...
TEST(Core_QR_Solver, accuracy64f) {
  ...
  *dev.ptr<double>() = 10.0;
  ...
  EXPECT_LE(norm(A.t() * (A * solutionQR), A.t() * B, CV_RELATIVE_L2),
            FLT_EPSILON);
  ...
  rng.fill(A, RNG::NORMAL, mean, dev);
  rng.fill(B, RNG::NORMAL, mean, dev);
  for (int i = 0; i < A.cols; i++) A.at<double>(0, i) = A.at<double>(1, i);
...
softdouble naiveExp(softdouble x) {
  ...
  if (exponent >= 0) {
    ...
    exponent = -exponent;
    for (int j = 0; j < exponent; j++) sum = sqrt(sum);
    return sum;
  }
}
...
TEST(Core_SoftFloat, exp32) {
  ...
  for (int i = 0; i < 50000; i++) {
    ...
    if (softfloat(x.f) > ln_max) x.f = rng.uniform(0.0f, (float)ln_max);
...
TEST(Core_SoftFloat, exp64) {
  ...
  for (int i = 0; i < 50000; i++) {
    ...
    uint64 mantissa =
        (((long long int)((unsigned int)(rng)) << 32) | (unsigned int)(rng)) &
        ((1LL << 52) - 1);
    x.u = (sign << 63) | (exponent << 52) | mantissa;
    if (softdouble(x.f) > ln_max) x.f = rng.uniform(0.0, (double)ln_max);
...
TEST(Core_SoftFloat, sincos64) {
  static const softdouble two = softdouble(2), three = softdouble(3),
                          half = softdouble::one() / two,
                          zero = softdouble::zero(), one = softdouble::one(),
                          pi = softdouble::pi(), piby2 = pi / two,
                          eps = softdouble::eps(), sin45 = sqrt(two) / two,
                          sin60 = sqrt(three) / two;
  ...
  for (size_t i = 0; i < nStdValues; i++) {
    for (size_t k = 0; k < stdAngles.size() / 3; k++) {
      softdouble x = stdAngles[k * 3] +
                     pi * softdouble(2 * ((int)i - (int)nStdValues / 2));
  ...
  exponents.push_back(256);
  exponents.push_back(-256);
  exponents.push_back(512);
  exponents.push_back(-512);
  exponents.push_back(1022);
  exponents.push_back(-1022);
  ...
  for (size_t i = 0; i < inputs.size(); i++) {
    ...
    softdouble sx = sin(x);
    softdouble cx = cos(x);
    ASSERT_FALSE(sx.isInf());
    ASSERT_FALSE(cx.isInf());
    ASSERT_FALSE(sx.isNaN());
    ASSERT_FALSE(cx.isNaN());
    ASSERT_LE(abs(sx), one);
...
TEST(Core_SoftFloat, CvRound) {
  struct {
    ...
    int32_t out32;
      ...
      {0x7ff0000000000001U, INT64_MAX,
       INT32_MAX},  // nan(casts to maximum value)
      {0xfff0000000000001U, INT64_MAX,
       INT32_MAX},  // nan(casts to maximum value)
      {0x7ffa5a5a5a5a5a5aU, INT64_MAX,
       INT32_MAX},  // nan(casts to maximum value)
      {0xfffa5a5a5a5a5a5aU, INT64_MAX,
       INT32_MAX},  // nan(casts to maximum value)
      {0x7fe123456789abcdU, INT64_MAX,
       INT32_MAX},  // 9.627645455595956656406699747E307
      {0xffe123456789abcdU, INT64_MIN,
       INT32_MIN},  // -9.627645455595956656406699747E307
      ...
      {0x41efffffffffffffU, 0x100000000,
       INT32_MAX},  // 4294967295.99999952316284179688
      {0xc1efffffffffffffU, -0x100000000,
       INT32_MIN},  // -4294967295.99999952316284179688
      {0x41effffffff00000U, 0x100000000,
       INT32_MAX},  // (2^32-1) + 0.5 = 4294967295.5
      {0xc1effffffff00000U, -0x100000000,
       INT32_MIN},  // -(2^32-1) - 0.5 = -4294967295.5
      ...
      {0xc1e0000000000000U, -0x80000000ll,
       -0x7fffffff - 1},  // -2^31 = -2147483648
      {0x41dfffffffffffffU, 0x80000000ll,
       INT32_MAX},  // 2147483647.99999976158142089844
      {0xc1dfffffffffffffU, -0x80000000ll,
       -0x7fffffff - 1},  // -2147483647.99999976158142089844

      {0x41dffffffff00000U, 0x80000000ll, INT32_MAX},  // (2^31-1) + 0.75
      {0xc1dffffffff00000U, -0x80000000ll,
       -0x7fffffff - 1},                               // -(2^31-1) - 0.75
      {0x41dfffffffe00001U, 0x80000000ll, INT32_MAX},  // (2^31-1) + 0.5 + 2^-22
      {0xc1dfffffffe00001U, -0x80000000ll,
       -0x7fffffff - 1},  // -(2^31-1) - 0.5 - 2^-22
      ...
      {0xc1dfffffffdfffffU, -0x7fffffff,
       -0x7fffffff},                                  // -(2^31-1) - 0.5 + 2^-22
  ...
  for (int i = 0, maxi = sizeof(_values) / sizeof(_values[0]); i < maxi; i++) {
    ...
    EXPECT_EQ((uint64_t)(values[i].out64),
              saturate_cast<uint64_t>(values[i].inVal));
    EXPECT_EQ(values[i].out32, cvRound(values[i].inVal));
    EXPECT_EQ(values[i].out32, saturate_cast<int32_t>(values[i].inVal));
    EXPECT_EQ((uint32_t)(values[i].out32),
              saturate_cast<uint32_t>(values[i].inVal));
  }
}
