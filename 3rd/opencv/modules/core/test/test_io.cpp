...
static SparseMat cvTsGetRandomSparseMat(int dims, const int* sz, int type,
  ...
  for (i = 0; i < nzcount; i++) {
    int idx[CV_MAX_DIM];
    for (j = 0; j < dims; j++) idx[j] = cvtest::randInt(rng) % sz[j];
...
static bool cvTsCheckSparse(const CvSparseMat* m1, const CvSparseMat* m2,
                            double eps) {
  ...
  for (node1 = cvInitSparseMatIterator(m1, &it1); node1 != 0;
       node1 = cvGetNextSparseNode(&it1)) {
    ...
    if (depth == CV_8U || depth == CV_8S) {
      ...
      if (*(ushort*)v1 != *(ushort*)v2) return false;
    } else if (depth == CV_32S) {
      if (*(int*)v1 != *(int*)v2) return false;
    } else if (depth == CV_32F) {
      if (fabs(*(float*)v1 - *(float*)v2) > eps * (fabs(*(float*)v2) + 1))
        return false;
    } else if (fabs(*(double*)v1 - *(double*)v2) >
               eps * (fabs(*(double*)v2) + 1))
      return false;
  }
...
class Core_IOTest : public cvtest::BaseTest {
 ...
 protected:
  void run(int) {
    double ranges[][2] = {{0, 256},        {-128, 128},         {0, 65536},
                          {-32768, 32768}, {-1000000, 1000000}, {-10, 10},
                          {-10, 10}};
    RNG& rng = ts->get_rng();
    RNG rng0;
    int progress = 0;
    ...
    for (int idx = 0; idx < test_case_count; idx++) {
      ...
      FileStorage fs(filename,
                     FileStorage::WRITE + (mem ? FileStorage::MEMORY : 0));

      int test_int = (int)cvtest::randInt(rng);
      double test_real = (cvtest::randInt(rng) % 2 ? 1 : -1) *
                         exp(cvtest::randReal(rng) * 18 - 9);
      string test_string =
          "vw "
          "wv23424rt\"&amp;&lt;&gt;&amp;&apos;@#$@$%$%&%IJUKYILFD@#$@%$&*&() ";
      ...
      Mat test_mat(cvtest::randInt(rng) % 30 + 1, cvtest::randInt(rng) % 30 + 1,
                   CV_MAKETYPE(depth, cn));

      rng0.fill(test_mat, CV_RAND_UNI, Scalar::all(ranges[depth][0]),
                Scalar::all(ranges[depth][1]));
      if (depth >= CV_32F) {
      ...
      CvGraph* graph =
          cvCreateGraph(CV_ORIENTED_GRAPH, sizeof(CvGraph), sizeof(CvGraphVtx),
                        sizeof(CvGraphEdge), storage);
      int edges[][2] = {{0, 1}, {1, 2}, {2, 0}, {0, 3}, {3, 4}, {4, 1}};
      int i, vcount = 5, ecount = 6;
      for (i = 0; i < vcount; i++) cvGraphAddVtx(graph);
      for (i = 0; i < ecount; i++) {
        CvGraphEdge* edge;
      ...
      rng0.fill(test_mat_nd, CV_RAND_UNI, Scalar::all(ranges[depth][0]),
                Scalar::all(ranges[depth][1]));
      if (depth >= CV_32F) {
        exp(test_mat_nd, test_mat_nd);
        MatND test_mat_scale(test_mat_nd.dims, test_mat_nd.size,
                             test_mat_nd.type());
        rng0.fill(test_mat_scale, CV_RAND_UNI, Scalar::all(-1), Scalar::all(1));
      ...
      fs << "test_int" << test_int << "test_real" << test_real << "test_string"
         << test_string;
      ...
      fs << "test_list"
         << "[" << 0.0000000000001 << 2 << CV_PI << -3435345
         << "2-502 2-029 3egegeg"
         << "{:"
      ...
      fs << "test_map"
         << "{"
         << "x" << 1 << "y" << 2 << "width" << 100 << "height" << 200 << "lbp"
         << "[:";
      ...
      if (!m || !CV_IS_MAT(m) || m->rows != test_mat.rows ||
          m->cols != test_mat.cols ||
          cvtest::cmpEps(cv::cvarrToMat(&stub1), cv::cvarrToMat(&_test_stub1),
                         &max_diff, 0, &pt, true) < 0) {
        ts->printf(
            cvtest::TS::LOG,
            "the read matrix is not correct: (%.20g vs %.20g) at (%d,%d)\n",
            cvGetReal2D(&stub1, pt[0], pt[1]),
            cvGetReal2D(&_test_stub1, pt[0], pt[1]), pt[0], pt[1]);
        ts->set_failed_test_info(cvtest::TS::FAIL_INVALID_OUTPUT);
        return;
      }
      if (m && CV_IS_MAT(m)) cvReleaseMat(&m);
      ...
      if (!CV_ARE_TYPES_EQ(&stub, &_test_stub) ||
        ...
        ts->printf(cvtest::TS::LOG,
                   "readObj method: the read nd matrix is not correct: (%.20g "
                   "vs %.20g) vs at (%d,%d)\n",
                   cvGetReal2D(&stub1, pt[0], pt[1]),
                   cvGetReal2D(&_test_stub1, pt[0], pt[1]), pt[0], pt[1]);
        ts->set_failed_test_info(cvtest::TS::FAIL_INVALID_OUTPUT);
      ...
      if (!CV_ARE_TYPES_EQ(&stub, &_test_stub) ||
          ...
          cvtest::cmpEps(cv::cvarrToMat(&stub1), cv::cvarrToMat(&_test_stub1),
                         &max_diff, 0, &pt, true) < 0) {
        ts->printf(cvtest::TS::LOG,
                   "C++ method: the read nd matrix is not correct: (%.20g vs "
                   "%.20g) vs at (%d,%d)\n",
                   cvGetReal2D(&stub1, pt[0], pt[1]),
                   cvGetReal2D(&_test_stub1, pt[1], pt[0]), pt[0], pt[1]);
        ts->set_failed_test_info(cvtest::TS::FAIL_INVALID_OUTPUT);
        return;
      }
      ...
      if (tl.type() != FileNode::SEQ || tl.size() != 6 ||
          ...
          (int)tl[1] != 2 || fabs((double)tl[2] - CV_PI) >= DBL_EPSILON ||
          (int)tl[3] != -3435345 || (String)tl[4] != "2-502 2-029 3egegeg" ||
          tl[5].type() != FileNode::MAP || tl[5].size() != 3 ||
          (int)tl[5]["month"] != 12 || (int)tl[5]["day"] != 31 ||
          (int)tl[5]["year"] != 1969) {
      ...
      if (tm.type() != FileNode::MAP || tm.size() != 5 || real_x != 1 ||
          real_y != 2 || real_width != 100 || real_height != 200 ||
          tm_lbp.type() != FileNode::SEQ || tm_lbp.size() != 8 ||
          real_lbp_val != 0xb6) {
      ...
      if (graph2->active_count != vcount || graph3->active_count != vcount ||
          graph2->edges->active_count != ecount ||
          graph3->edges->active_count != ecount) {
        ts->printf(cvtest::TS::LOG,
                   "the cloned or read graph have wrong number of vertices or "
                   "edges\n");
        ts->set_failed_test_info(cvtest::TS::FAIL_INVALID_OUTPUT);
        return;
      }
      ...
      for (i = 0; i < ecount; i++) {
        ...
        if (!edge2 || edge2->weight != (float)(i + 1) || !edge3 ||
            edge3->weight != (float)(i + 1)) {
          ts->printf(cvtest::TS::LOG,
      ...
      fs.release();
      if (!mem) remove(filename.c_str());
    }
  }
};

TEST(Core_InputOutput, write_read_consistency) {
  Core_IOTest test;
  test.safe_run();
}
...
static inline void read(
    const FileNode& node, UserDefinedType& value,
    const UserDefinedType& default_value = UserDefinedType()) {
  if (node.empty()) {
    value = default_value;
  } else {
    node["a"] >> value.a;
...
class CV_MiscIOTest : public cvtest::BaseTest {
 ...
 protected:
  void run(int) {
    const char* suffix[3] = {".yml", ".xml", ".json"};
...
TEST(Core_InputOutput, FileStorageKey) {
  cv::FileStorage f("dummy.yml",
                    cv::FileStorage::WRITE | cv::FileStorage::MEMORY);

  EXPECT_NO_THROW(f << "key1"
                    << "value1");
  EXPECT_NO_THROW(f << "_key2"
                    << "value2");
  EXPECT_NO_THROW(f << "key_3"
                    << "value3");
...
TEST(Core_InputOutput, FileStorageSpaces) {
  ...
  cv::FileStorage f2(f.releaseAndGetString(),
...
struct data_t {
  ...
  /*0x18*/ d d2;
  /*0x20*/ i i4;
  i required_alignment_field_for_linux32;
  /*
   * OpenCV persistence.cpp stuff expects: sizeof(data_t) = alignSize(36,
   * sizeof(largest type = double)) = 40 Some compilers on some archs returns
   * sizeof(data_t) = 36 due struct packaging UB
   */

  static inline const char* signature() {
    if (sizeof(data_t) != 40) {
      printf(
          "sizeof(data_t)=%d, u1=%p u2=%p i1=%p i2=%p i3=%p d1=%p d2=%p "
          "i4=%p\n",
          (int)sizeof(data_t), &(((data_t*)0)->u1), &(((data_t*)0)->u2),
          &(((data_t*)0)->i1), &(((data_t*)0)->i2), &(((data_t*)0)->i3),
          &(((data_t*)0)->d1), &(((data_t*)0)->d2), &(((data_t*)0)->i4));
    }
...
TEST(Core_InputOutput, filestorage_base64_basic) {
  ...
  char const* filenames[] = {"core_io_base64_basic_test.yml",
                             "core_io_base64_basic_test.xml",
                             "core_io_base64_basic_test.json", 0};
  ...
  for (char const** ptr = filenames; *ptr; ptr++) {
    ...
    { /* init */
      ...
      const cv::Range ranges[] = {cv::Range(0, 2), cv::Range(0, 2),
                                  cv::Range(1, 2), cv::Range(0, 2)};
      _nd_out = _4d(ranges);
    ...
    { /* write */
      ...
      for (int i = 0; i < 10; i++)
        cvWriteRawDataBase64(*fs, rawdata.data() + i * 100, 100,
                             data_t::signature());
      cvEndWriteStruct(*fs);
    ...
    { /* read */
      ...
      cvReadRawData(*fs, fs["rawdata"].node, rawdata.data(),
                    data_t::signature());
...
TEST(Core_InputOutput, filestorage_base64_valid_call) {
  ...
  char const* filenames[] = {"core_io_base64_other_test.yml",
                             ...
                             0};
  char const* real_name[] = {"core_io_base64_other_test.yml",
                             "core_io_base64_other_test.xml",
                             "core_io_base64_other_test.json",
  ...
  for (char const** ptr = filenames; *ptr; ptr++) {
    ...
    EXPECT_NO_THROW({
      ...
      for (int i = 0; i < 10; i++)
        cvWriteRawData(*fs, rawdata.data(), static_cast<int>(rawdata.size()),
                       "i");
      cvEndWriteStruct(*fs);
...
TEST(Core_InputOutput, filestorage_base64_invalid_call) {
  ...
  char const* filenames[] = {"core_io_base64_other_test.yml",
                             "core_io_base64_other_test.xml",
                             "core_io_base64_other_test.json", 0};
...
TEST(Core_InputOutput, filestorage_keypoints_vec_vec_io) {
  ...
  FileStorage writer(
      "", FileStorage::WRITE + FileStorage::MEMORY + FileStorage::FORMAT_XML);
...
TEST(Core_InputOutput, FileStorage_DMatch) {
  ...
  cv::FileStorage fs_read(fs_result,
                          cv::FileStorage::READ | cv::FileStorage::MEMORY);
...
TEST(Core_InputOutput, FileStorage_KeyPoint) {
  cv::FileStorage fs("keypoint.xml",
                     cv::FileStorage::WRITE | cv::FileStorage::MEMORY);
