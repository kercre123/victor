...
class Core_RandTest : public cvtest::BaseTest {
 ...
 protected:
  void run(int);
  bool check_pdf(const Mat& hist, double scale, int dist_type, double& refval,
                 double& realval);
};
...
static double chi2_p95(int n) {
  ...
  if (n <= 30) return chi2_tab95[n - 1];
  return n + sqrt((double)2 * n) * xp + 0.6666666666666 * (xp * xp - 1);
}

bool Core_RandTest::check_pdf(const Mat& hist, double scale, int dist_type,
  ...
  if (dist_type == CV_RAND_UNI) {
    ...
    sum2 = 1. / sum2;
    for (i = 0; i < hsz; i++) H0[i] = (float)(H0[i] * sum2);
  }
  ...
  for (i = 0; i < hsz; i++) {
    double a = H0[i];
    double b = H[i] * scale;
    if (a > DBL_EPSILON) chi2 += (a - b) * (a - b) / (a + b);
  }
  realval = chi2;
...
void Core_RandTest::run(int) {
  static int _ranges[][2] = {
      {0, 256},        {-128, 128},         {0, 65536},
      {-32768, 32768}, {-1000000, 1000000}, {-1000, 1000},
      {-1000, 1000}};
  ...
  for (int idx = 0; idx < test_case_count; idx++) {
    ...
    for (c = 0; c < cn; c++) {
      int a, b, hsz;
      if (dist_type == CV_RAND_UNI) {
        a = (int)(cvtest::randInt(rng) %
                  (_ranges[depth][1] - _ranges[depth][0])) +
            _ranges[depth][0];
        do {
          b = (int)(cvtest::randInt(rng) %
                    (_ranges[depth][1] - _ranges[depth][0])) +
              _ranges[depth][0];
        } while (abs(a - b) <= 1);
        if (a > b) std::swap(a, b);
    ...
    for (k = 0; k <= maxk; k++) {
      ...
      for (slice = 0; slice < maxSlice; slice++, sz += dsz) {
        dsz = slice + 1 < maxSlice ? (int)(cvtest::randInt(rng) % (SZ - sz + 1))
                                   : SZ - sz;
    ...
    if (maxk >= 1 && cvtest::norm(arr[0], arr[1], NORM_INF) > eps) {
      ts->printf(cvtest::TS::LOG,
                 "RNG output depends on the array lengths (some generated "
                 "numbers get lost?)");
    ...
    for (c = 0; c < cn; c++) {
      ...
      if (dist_type == CV_RAND_UNI && W[c] != SZ) {
        ts->printf(cvtest::TS::LOG,
                   "Uniform RNG gave values out of the range [%g,%g) on "
                   "channel %d/%d\n",
                   A[c], B[c], c, cn);
        ts->set_failed_test_info(cvtest::TS::FAIL_INVALID_OUTPUT);
        return;
      ...
      if (dist_type == CV_RAND_NORMAL && W[c] < SZ * .90) {
        ts->printf(cvtest::TS::LOG,
                   "Normal RNG gave too many values out of the range "
                   "(%g+4*%g,%g+4*%g) on channel %d/%d\n",
    ...
    if (do_sphere_test) {
      ...
      int sdim = SDIM % 2;
      double V0 = sdim + 1;
      for (sdim += 2; sdim <= SDIM; sdim += 2) V0 *= 2 * CV_PI / sdim;
    ...
    for (int i = 0; i < a.rows; i++)
      for (int j = 0; j < a.cols; j++) {
        ...
        if (v == 0)
          ...
          nx++;
        if (vf < FLT_MAX * -0.999f)
          nfmin++;
        else if (vf > FLT_MAX * 0.999f)
          nfmax++;
        else
          nfx++;
      }
...
TEST(Core_Rand, range) {
  Core_RandRangeTest test;
  test.safe_run();
}
...
TEST(Core_RNG_MT19937, regression) {
  ...
  int expected[length] = {177, 158, 180, 177, 160, 179, 143, 162, 177, 144, 170,
                          174, 165, 168, 168, 156, 177, 157, 159, 169, 177, 182,
                          166, 154, 144, 180, 168, 152, 170, 187, 160, 145, 139,
                          164, 157, 179, 148, 183, 159, 160, 196, 184, 149, 142,
                          162, 148, 163, 152, 168, 173, 160, 181, 172, 181, 155,
                          153, 158, 171, 138, 150, 150};
...
TEST(Core_Rand, Regression_Stack_Corruption) {
  ...
  size_t offset = 0;
  cv::Mat_<cv::Point2d> x(2, 3, (cv::Point2d*)(buffer + offset));
  offset += x.total() * x.elemSize();
  double& param1 = *(double*)(buffer + offset);
  offset += sizeof(double);
  double& param2 = *(double*)(buffer + offset);
  offset += sizeof(double);
  param1 = -9;
  param2 = 2;
...
class RandRowFillParallelLoopBody : public cv::ParallelLoopBody {
 public:
  ...
  void operator()(const cv::Range& r) const {
    ...
    // theRNG() state is changed here (but state collision has low probability,
