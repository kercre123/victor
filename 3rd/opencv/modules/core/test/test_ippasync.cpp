...
PARAM_TEST_CASE(IPPAsyncShared, Channels, hppAccelType) {
  ...
  void generateTestData() {
    ...
    sts = hppQueryMatrixAllocParams(accel, (hpp32u)(matrix_Size.width * cn),
                                    (hpp32u)matrix_Size.height,
...
INSTANTIATE_TEST_CASE_P(IppATest, IPPAsyncShared,
                        Combine(Values(1, 2, 3, 4),
                                Values(HPP_ACCEL_TYPE_CPU,
                                       HPP_ACCEL_TYPE_GPU)));

INSTANTIATE_TEST_CASE_P(
    IppATest, IPPAsync,
    Combine(Values(CV_8U, CV_16U, CV_16S, CV_32F), Values(1, 2, 3, 4),
