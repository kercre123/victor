...
static void testOpenCLKernel(cv::ocl::Kernel& k) {
  ...
  int64 kernel_time =
      k.args(cv::ocl::KernelArg::ReadOnlyNoSize(
                 src),  // size is not used (similar to 'dst' size)
             cv::ocl::KernelArg::WriteOnly(dst), (int)5)
...
TEST(OpenCL, support_binary_programs) {
  ...
  {  // Generate program binary from OpenCL C source
    static const char* opencl_kernel_src =
        "__kernel void test_kernel(__global const uchar* src, int src_step, "
        "int src_offset,\n"
        "                          __global uchar* dst, int dst_step, int "
        "dst_offset, int dst_rows, int dst_cols,\n"
        "                          int c)\n"
        "{\n"
    ...
    std::cout << "Program binary size: " << program_binary_code.size()
              << " bytes" << std::endl;
  }
  ...
  {  // Load program from binary (without sources)
    ...
    cv::ocl::ProgramSource src = cv::ocl::ProgramSource::fromBinary(
        module_name, "simple_binary", (uchar*)&program_binary_code[0],
        program_binary_code.size(), "");
    cv::String errmsg;
    cv::ocl::Program program(src, "", errmsg);
    ASSERT_TRUE(program.ptr() != NULL);
...
TEST(OpenCL, support_SPIR_programs) {
  ...
  if (!device.isExtensionSupported("cl_khr_spir")) {
    throw cvtest::SkipTestException(
        "'cl_khr_spir' extension is not supported by OpenCL device");
  }
  std::vector<char> program_binary_code;
  ...
  {  // Load program from SPIR format
    ASSERT_FALSE(program_binary_code.empty());
    cv::ocl::ProgramSource src = cv::ocl::ProgramSource::fromSPIR(
        module_name, "simple_spir", (uchar*)&program_binary_code[0],
        program_binary_code.size(), "");
    cv::String errmsg;
    cv::ocl::Program program(src, "", errmsg);
    ASSERT_TRUE(program.ptr() != NULL);
