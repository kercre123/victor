...
TEST(Image2D, createAlias) {
  if (cv::ocl::haveOpenCL()) {
    ...
    if (1 < major || (1 == major && 2 <= minor)) {
      ...
      EXPECT_NO_THROW(isFormatSupported =
                          cv::ocl::Image2D::isFormatSupported(CV_8U, 1, false));
      ...
      if (isFormatSupported && canCreateAlias) {
        ...
        std::cout
            << "Impossible to create alias for selected image. Test skipped."
            << std::endl;
    }
  } else
    std::cout << "OpenCL runtime not found. Test skipped" << std::endl;
...
TEST(Image2D, turnOffOpenCL) {
  if (cv::ocl::haveOpenCL()) {
    ...
    EXPECT_NO_THROW(isFormatSupported =
                        cv::ocl::Image2D::isFormatSupported(CV_8U, 1, true));
