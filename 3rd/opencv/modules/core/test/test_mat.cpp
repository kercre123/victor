...
class Core_ReduceTest : public cvtest::BaseTest {
 ...
 protected:
  void run(int);
  int checkOp(const Mat& src, int dstType, int opType, const Mat& opRes,
              int dim);
...
template <class Type>
void testReduce(const Mat& src, Mat& sum, Mat& avg, Mat& max, Mat& min,
                int dim) {
...
int Core_ReduceTest::checkOp(const Mat& src, int dstType, int opType,
                             const Mat& opRes, int dim) {
  int srcType = src.type();
  bool support = false;
  if (opType == CV_REDUCE_SUM || opType == CV_REDUCE_AVG) {
    if (srcType == CV_8U &&
        (dstType == CV_32S || dstType == CV_32F || dstType == CV_64F))
      support = true;
    ...
    if (srcType == CV_32F && (dstType == CV_32F || dstType == CV_64F))
      support = true;
    if (srcType == CV_64F && dstType == CV_64F) support = true;
  } else if (opType == CV_REDUCE_MAX) {
    if (srcType == CV_8U && dstType == CV_8U) support = true;
    if (srcType == CV_32F && dstType == CV_32F) support = true;
    if (srcType == CV_64F && dstType == CV_64F) support = true;
  ...
  if (!support) return cvtest::TS::OK;
  ...
  if (check) {
    char msg[100];
    const char* opTypeStr = opType == CV_REDUCE_SUM
                                ? "CV_REDUCE_SUM"
                                : opType == CV_REDUCE_AVG
                                      ? "CV_REDUCE_AVG"
                                      : opType == CV_REDUCE_MAX
                                            ? "CV_REDUCE_MAX"
                                            : opType == CV_REDUCE_MIN
                                                  ? "CV_REDUCE_MIN"
...
TEST(Core_PCA, accuracy) {
...
#ifdef CHECK_C
  Mat prjTestPoints, backPrjTestPoints, cPoints = rPoints.t(),
                                        cTestPoints = rTestPoints.t();
  CvMat _points, _testPoints, _avg, _eval, _evec, _prjTestPoints,
      _backPrjTestPoints;
#endif
  ...
  for (int i = 0; i < subEvec.rows; i++) {
    ...
    // eigenvectors have normalized length, but both directions v and -v are
    // valid
    ...
    if (err > evecEps) {
      ...
      EXPECT_LE(err, evecEps)
          << "pca.eigenvectors is incorrect (CV_PCA_DATA_AS_ROW) at " << i
          << " "
          << cv::format("max diff is %g at (i=%d, j=%d) (%g vs %g)\n", mval,
                        mloc.y, mloc.x,
                        rPCA.eigenvectors.at<float>(mloc.y, mloc.x),
  ...
  err = cvtest::norm(cv::abs(ocvPrjTestPoints), cv::abs(rPrjTestPoints.t()),
                     NORM_L2 | NORM_RELATIVE);
  ASSERT_LE(err, diffPrjEps)
      << "bad accuracy of project() (CV_PCA_DATA_AS_COL)";
  err = cvtest::norm(cPCA.backProject(ocvPrjTestPoints), rBackPrjTestPoints.t(),
                     NORM_L2 | NORM_RELATIVE);
  ASSERT_LE(err, diffBackPrjEps)
  ...
  if (cPCA.eigenvectors.rows > maxComponents)
    err = cvtest::norm(cv::abs(rvPrjTestPoints.rowRange(0, maxComponents)),
                       cv::abs(rPrjTestPoints.t()), NORM_L2 | NORM_RELATIVE);
  else
    err = cvtest::norm(
        cv::abs(rvPrjTestPoints),
        cv::abs(rPrjTestPoints.colRange(0, cPCA.eigenvectors.rows).t()),
        NORM_L2 | NORM_RELATIVE);

  ASSERT_LE(err, diffPrjEps)
      << "bad accuracy of project() (CV_PCA_DATA_AS_COL); retainedVariance="
      << retainedVariance;
  err = cvtest::norm(cPCA.backProject(rvPrjTestPoints), rBackPrjTestPoints.t(),
                     NORM_L2 | NORM_RELATIVE);
  ASSERT_LE(err, diffBackPrjEps)
  ...
  err = cvtest::norm(backPrjTestPoints, rBackPrjTestPoints,
                     NORM_L2 | NORM_RELATIVE);
  ASSERT_LE(err, diffBackPrjEps)
  ...
  evec = evec.t();
  _evec = evec;
  prjTestPoints = prjTestPoints.t();
  _prjTestPoints = prjTestPoints;
  backPrjTestPoints = backPrjTestPoints.t();
  _backPrjTestPoints = backPrjTestPoints;
  ...
  err = cvtest::norm(cv::abs(prjTestPoints), cv::abs(rPrjTestPoints.t()),
                     NORM_L2 | NORM_RELATIVE);
  ASSERT_LE(err, diffPrjEps)
      << "bad accuracy of cvProjectPCA() (CV_PCA_DATA_AS_COL)";
  err = cvtest::norm(backPrjTestPoints, rBackPrjTestPoints.t(),
                     NORM_L2 | NORM_RELATIVE);
  ASSERT_LE(err, diffBackPrjEps)
  ...
  lPCA.read(fs.root());
  err = cvtest::norm(rPCA.eigenvectors, lPCA.eigenvectors,
                     NORM_L2 | NORM_RELATIVE);
  EXPECT_LE(err, 0) << "bad accuracy of write/load functions (YML)";
  err =
      cvtest::norm(rPCA.eigenvalues, lPCA.eigenvalues, NORM_L2 | NORM_RELATIVE);
  EXPECT_LE(err, 0) << "bad accuracy of write/load functions (YML)";
...
static double getValue(SparseMat& M, const int* idx, RNG& rng) {
  ...
  const uchar* ptr = d == 2 ? M.ptr(idx[0], idx[1], false, phv)
                            : d == 3 ? M.ptr(idx[0], idx[1], idx[2], false, phv)
                                     : M.ptr(idx, false, phv);
  return !ptr ? 0
              : M.type() == CV_32F ? *(float*)ptr
                                   : M.type() == CV_64F ? *(double*)ptr : 0;
}
...
static double getValue(const CvSparseMat* M, const int* idx) {
  ...
  return !ptr ? 0
              : type == CV_32F ? *(float*)ptr
                               : type == CV_64F ? *(double*)ptr : 0;
}
...
static void eraseValue(CvSparseMat* M, const int* idx) { cvClearND(M, idx); }
...
void Core_ArrayOpTest::run(int /* start_from */) {
  ...
  // dense matrix operations
  {
    ...
    if (A.at<float>(idx0[0], idx0[1], idx0[2]) != val0 ||
      ...
      ts->printf(cvtest::TS::LOG,
                 "one of cvSetReal3D, cvSetRealND, cvSet3D, cvSetND "
  ...
  // test cv::Mat::forEach
  {
    ...
    for (size_t i = 0; i < sizeof(dims) / sizeof(dims[0]); ++i) {
      total2 +=
          ((dims[i] - 1) * dims[i] / 2) * dims[0] * dims[1] * dims[2] / dims[i];
    }
    if (total != total2) {
      ts->printf(cvtest::TS::LOG,
  ...
  // See https://github.com/opencv/opencv/issues/8447
  {
        ...
        for (int i2 = 0; i2 < dims[2]; ++i2) {
          for (int i3 = 0; i3 < dims[3]; ++i3) {
            for (int i4 = 0; i4 < dims[4]; ++i4) {
              ...
              if (pixel[0] != i0 || pixel[1] != i1 || pixel[2] != i2 ||
                  pixel[3] != i3 || pixel[4] != i4) {
                if (!error_reported) {
                  ts->printf(cvtest::TS::LOG,
                             "forEach is not correct.\n"
                             "First error detected at position (%d, %d, %d, "
                             "%d, %d), got value (%d, %d, %d, %d, %d).\n",
                             i0, i1, i2, i3, i4, pixel[0], pixel[1], pixel[2],
                             pixel[3], pixel[4]);
                  error_reported = true;
                }
                errcount++;
    ...
    uint64 total2 = 0;
    for (size_t i = 0; i < sizeof(dims) / sizeof(dims[0]); ++i) {
      total2 += ((dims[i] - 1) * dims[i] / 2) * dims[0] * dims[1] * dims[2] *
                dims[3] * dims[4] / dims[i];
    }
    if (total != total2) {
      ts->printf(cvtest::TS::LOG,
                 "forEach is not correct because total is invalid.\n");
  ...
  for (int si = 0; si < 10; si++) {
    ...
    for (i = 0; i < nz0; i++) {
      for (;;) {
        for (k = 0; k < dims; k++) idx[k] = (unsigned)rng % size[k];
        sidx = idx2string(idx, dims);
        if (M0.count(sidx) == 0) break;
      }
      all_idxs.push_back(sidx);
      M0[sidx] = all_vals[i];
      if (all_vals[i] == min_val) min_sidx = sidx;
      if (all_vals[i] == max_val) max_sidx = sidx;
      setValue(M, idx, all_vals[i], rng);
      double v = getValue(M, idx, rng);
      if (v != all_vals[i]) {
        ts->printf(cvtest::TS::LOG,
                   "%d. immediately after SparseMat[%s]=%.20g the current "
                   "value is %.20g\n",
    ...
    if (nz1 != nz0 || nz2 != nz0) {
      ...
      ts->printf(
          cvtest::TS::LOG,
          "%d: The number of non-zero elements before/after converting to/from "
          "dense matrix is not correct: %d/%d (while it should be %d)\n",
          si, nz1, nz2, nz0);
      break;
    }
    ...
    if (fabs(norm0 - _norm0) > fabs(_norm0) * eps ||
      ...
      errcount++;
      ts->printf(cvtest::TS::LOG,
                 "%d: The norms are different: %.20g/%.20g/%.20g vs "
                 "%.20g/%.20g/%.20g\n",
    ...
    for (i = 0; i < n; i++) {
      ...
      if (val1 != val0 || val2 != val0 ||
          fabs(val3 - val0 * 2) > fabs(val0 * 2) * FLT_EPSILON) {
        errcount++;
        ts->printf(cvtest::TS::LOG,
    ...
    for (i = 0; i < n; i++) {
      ...
      if (val1 != 0 || val2 != 0) {
        errcount++;
        ts->printf(cvtest::TS::LOG,
                   "SparseMat: after deleting M[%s], it is =%g/%g (while it "
                   "should be 0)\n",
    ...
    if (nz != 0) {
      errcount++;
      ts->printf(cvtest::TS::LOG,
                 "The number of non-zero elements after removing all the "
                 "elements = %d (while it should be 0)\n",
                 nz);
      break;
    }
    ...
    if (val1 != min_val || val2 != max_val || s1 != min_sidx ||
        s2 != max_sidx) {
      errcount++;
      ts->printf(
          cvtest::TS::LOG,
          "%d. Sparse: The value and positions of minimum/maximum elements are "
          "different from the reference values and positions:\n\t"
          "(%g, %g, %s, %s) vs (%g, %g, %s, %s)\n",
          si, val1, val2, s1.c_str(), s2.c_str(), min_val, max_val,
          min_sidx.c_str(), max_sidx.c_str());
      break;
    }
    ...
    if ((min_val < 0 && (val1 != min_val || s1 != min_sidx)) ||
      ...
      ts->printf(
          cvtest::TS::LOG,
          "%d. Dense: The value and positions of minimum/maximum elements are "
          "different from the reference values and positions:\n\t"
          "(%g, %g, %s, %s) vs (%g, %g, %s, %s)\n",
          si, val1, val2, s1.c_str(), s2.c_str(), min_val, max_val,
          min_sidx.c_str(), max_sidx.c_str());
  ...
  ts->set_failed_test_info(errcount == 0 ? cvtest::TS::OK
    ...
    for (int x = 0; x < m.cols; x++) {
      ...
      for (size_t i = 0; i < mv.size(); i++) {
        ...
        for (size_t li = 0; li < mvChannel; li++)
          if (mElem[loc + li] != mvElem[li]) diffElemCount++;
        loc += mvChannel;
      }
...
static int calcDiffElemCount(const vector<Mat>& mv, const Mat& m) {
  int depth = m.depth();
  switch (depth) {
    case CV_8U:
...
class Core_MergeSplitBaseTest : public cvtest::BaseTest {
 protected:
  virtual int run_case(int depth, size_t channels, const Size& size,
                       RNG& rng) = 0;
  ...
  virtual void run(int) {
    ...
    RNG& rng = theRNG();
    Size mSize(rng.uniform(minMSize, maxMSize),
               rng.uniform(minMSize, maxMSize));
  ...
  virtual int run_case(int depth, size_t matCount, const Size& size, RNG& rng) {
    ...
    if (dst.size() != size) {
      ts->printf(cvtest::TS::LOG,
                 ...
                 commonLog.str().c_str(), dst.rows, dst.cols, size.height,
                 size.width);
      return cvtest::TS::FAIL_INVALID_OUTPUT;
    }
    ...
    if (diffElemCount > 0) {
      ts->printf(
          ...
          commonLog.str().c_str(),
          static_cast<float>(diffElemCount) / (channels * size.area()));
      return cvtest::TS::FAIL_INVALID_OUTPUT;
    }
    ...
    for (size_t i = 0; i < dst.size(); i++) {
      if (dst[i].size() != size) {
        ts->printf(
            ...
            commonLog.str().c_str(), i, dst[i].rows, dst[i].cols, size.height,
            size.width);
        return cvtest::TS::FAIL_INVALID_OUTPUT;
      }
...
TEST(Core_Reduce, accuracy) {
  Core_ReduceTest test;
  test.safe_run();
}
TEST(Core_Array, basic_operations) {
  Core_ArrayOpTest test;
  test.safe_run();
}

TEST(Core_Merge, shape_operations) {
  Core_MergeTest test;
  test.safe_run();
}
TEST(Core_Split, shape_operations) {
  Core_SplitTest test;
  test.safe_run();
}
...
TEST(Core_Mat, reshape_1942) {
  cv::Mat A = (cv::Mat_<float>(2, 3) << 3.4884074, 1.4159607, 0.78737736,
               2.3456569, -0.88010466, 0.3009364);
  int cn = 0;
  ASSERT_NO_THROW(cv::Mat_<float> M = A.reshape(3); cn = M.channels(););
  ASSERT_EQ(1, cn);
}

static void check_ndim_shape(const cv::Mat& mat, int cn, int ndims,
                             const int* sizes) {
...
TEST(Core_Mat, push_back) {
  ...
  Mat c = (Mat_<float>(2, 2) << -0.88010466f, 0.3009364f, 2.22399974f,
           -5.45933905f);
...
TEST(Core_Mat, copyNx1ToVector) {
  ...
  ASSERT_PRED_FORMAT2(cvtest::MatComparator(0, 0), ref_dst8,
                      cv::Mat_<uchar>(dst8));

  src.convertTo(ref_dst16, CV_16U);
  src.convertTo(dst16, CV_16U);

  ASSERT_PRED_FORMAT2(cvtest::MatComparator(0, 0), ref_dst16,
                      cv::Mat_<ushort>(dst16));
}
...
TEST(Core_SparseMat, footprint) {
  ...
  double dataSize0 = ((double)m.hdr->pool.size() +
                      (double)m.hdr->hashtab.size() * sizeof(size_t)) *
                     1e-6;
  ...
  double dataSize1 = ((double)m.hdr->pool.size() +
                      (double)m.hdr->hashtab.size() * sizeof(size_t)) *
                     1e-6;
...
TEST(Mat, push_back_vector) {
  ...
  for (int i = 0; i < 5; ++i) result.push_back(cv::Mat(vec1).reshape(1, 1));
...
TEST(Mat_, template_based_ptr) {
  int dim[4] = {2, 2, 1, 2};
  Mat_<float> mat = (Mat_<float>(4, dim) << 11.0f, 22.0f, 33.0f, 44.0f, 55.0f,
                     66.0f, 77.0f, 88.0f);
  int idx[4] = {1, 0, 0, 1};
