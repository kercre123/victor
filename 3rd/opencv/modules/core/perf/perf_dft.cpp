...
#define MAT_SIZES_DFT \
  cv::Size(320, 480), cv::Size(800, 600), cv::Size(1280, 1024), sz1080p, sz2K
CV_ENUM(FlagsType, 0, DFT_INVERSE, DFT_SCALE, DFT_COMPLEX_OUTPUT, DFT_ROWS,
        DFT_INVERSE | DFT_COMPLEX_OUTPUT)
#define TEST_MATS_DFT                                                \
  testing::Combine(testing::Values(MAT_SIZES_DFT),                   \
                   testing::Values(MAT_TYPES_DFT), FlagsType::all(), \
                   testing::Values(true, false))

typedef std::tr1::tuple<Size, MatType, FlagsType, bool>
    Size_MatType_FlagsType_NzeroRows_t;
typedef perf::TestBaseWithParam<Size_MatType_FlagsType_NzeroRows_t>
...
PERF_TEST_P(Size_MatType_FlagsType_NzeroRows, dft, TEST_MATS_DFT) {
  ...
  if (isNzeroRows) nonzero_rows = sz.height / 2;
...
PERF_TEST_P(Size_MatType_Flag, dct,
            testing::Combine(
  ...
  declare.in(src, WARMUP_RNG).out(dst).time(60);
