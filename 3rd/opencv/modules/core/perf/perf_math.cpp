...
PERF_TEST_P(VectorLength, phase32f,
            testing::Values(128, 1000, 128 * 1024, 512 * 1024, 1024 * 1024)) {
...
PERF_TEST_P(VectorLength, phase64f,
            testing::Values(128, 1000, 128 * 1024, 512 * 1024, 1024 * 1024)) {
  size_t length = GetParam();
...
PERF_TEST_P(MaxDim_MaxPoints, kmeans,
  ...
  for (i = 0; i < N; i++) data0.row(rng.uniform(0, N0)).copyTo(data.row(i));
  ...
  cv::sort(clusterPointsNumber, sortedClusterPointsNumber,
