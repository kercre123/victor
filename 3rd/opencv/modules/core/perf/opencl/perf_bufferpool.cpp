...
struct BufferPoolState {
  ...
  BufferPoolState(BufferPoolController* c, bool enable) : controller_(c) {
    if (!cv::ocl::useOpenCL()) {
  ...
  ~BufferPoolState() { controller_->setMaxReservedSize(oldMaxReservedSize_); }
};

typedef TestBaseWithParam<bool> BufferPoolFixture;

OCL_PERF_TEST_P(BufferPoolFixture, BufferPool_UMatCreation100, Bool()) {
  BufferPoolState s(cv::ocl::getOpenCLAllocator()->getBufferPoolController(),
                    GetParam());
