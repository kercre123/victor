...
#include <va/va.h>
#else  // HAVE_VA
#define NO_VA_SUPPORT_ERROR               \
  CV_ErrorNoReturn(cv::Error::StsBadFunc, \
                   "OpenCV was build without VA support (libva)")
#endif  // HAVE_VA
...
static clGetDeviceIDsFromVA_APIMediaAdapterINTEL_fn
    clGetDeviceIDsFromVA_APIMediaAdapterINTEL = NULL;
static clCreateFromVA_APIMediaSurfaceINTEL_fn
    clCreateFromVA_APIMediaSurfaceINTEL = NULL;
static clEnqueueAcquireVA_APIMediaSurfacesINTEL_fn
    clEnqueueAcquireVA_APIMediaSurfacesINTEL = NULL;
static clEnqueueReleaseVA_APIMediaSurfacesINTEL_fn
    clEnqueueReleaseVA_APIMediaSurfacesINTEL = NULL;
...
Context& initializeContextFromVA(VADisplay display, bool tryInterop) {
  ...
  if (tryInterop) {
    ...
    if (status != CL_SUCCESS)
      CV_Error(cv::Error::OpenCLInitError,
    ...
    // For CL-VA interop, we must find platform/device with
    // "cl_intel_va_api_media_sharing" extension. With standard initialization
    // procedure, we should examine platform extension string for that. But in
    // practice, the platform ext string doesn't contain it, while device ext
    // string does. Follow Intel procedure (see tutorial), we should obtain
    // device IDs by extension call. Note that we must obtain function pointers
    // using specific platform ID, and can't provide pointers in advance. So, we
    // iterate and select the first platform, for which we got non-NULL
    // pointers, device, and CL context.
    ...
    for (int i = 0; i < (int)numPlatforms; ++i) {
      ...
      clCreateFromVA_APIMediaSurfaceINTEL =
          (clCreateFromVA_APIMediaSurfaceINTEL_fn)
              clGetExtensionFunctionAddressForPlatform(
                  platforms[i], "clCreateFromVA_APIMediaSurfaceINTEL");
      clEnqueueAcquireVA_APIMediaSurfacesINTEL =
          (clEnqueueAcquireVA_APIMediaSurfacesINTEL_fn)
              clGetExtensionFunctionAddressForPlatform(
                  platforms[i], "clEnqueueAcquireVA_APIMediaSurfacesINTEL");
      clEnqueueReleaseVA_APIMediaSurfacesINTEL =
          (clEnqueueReleaseVA_APIMediaSurfacesINTEL_fn)
              clGetExtensionFunctionAddressForPlatform(
                  platforms[i], "clEnqueueReleaseVA_APIMediaSurfacesINTEL");
      ...
      status = clGetDeviceIDsFromVA_APIMediaAdapterINTEL(
          platforms[i], CL_VA_API_DISPLAY_INTEL, display,
      ...
      context =
          clCreateContext(props, numDevices, &device, NULL, NULL, &status);
      if (status != CL_SUCCESS) {
...
#if defined(HAVE_VA_INTEL) && defined(HAVE_OPENCL)
static bool ocl_convert_nv12_to_bgr(cl_mem clImageY, cl_mem clImageUV,
                                    cl_mem clBuffer, int step, int cols,
                                    int rows) {
  ocl::Kernel k;
...
static bool ocl_convert_bgr_to_nv12(cl_mem clBuffer, int step, int cols,
                                    int rows, cl_mem clImageY,
                                    cl_mem clImageUV) {
  ocl::Kernel k;
...
static void copy_convert_nv12_to_bgr(const VAImage& image,
                                     const unsigned char* buffer, Mat& bgr) {
  const float d1 = 16.0f;
  const float d2 = 128.0f;

  static const float coeffs[5] = {1.163999557f, 2.017999649f, -0.390999794f,
...
static void copy_convert_bgr_to_nv12(const VAImage& image, const Mat& bgr,
                                     unsigned char* buffer) {
  const float d1 = 16.0f;
  const float d2 = 128.0f;

  static const float coeffs[8] = {0.256999969f,   0.50399971f,    0.09799957f,
                                  -0.1479988098f, -0.2909994125f, 0.438999176f,
                                  -0.3679990768f, -0.0709991455f};
...
void convertToVASurface(VADisplay display, InputArray src, VASurfaceID surface,
                        Size size) {
  (void)display;
  (void)src;
  (void)surface;
  ...
  if (contextInitialized) {
    ...
    cl_mem clImageY = clCreateFromVA_APIMediaSurfaceINTEL(
        context, CL_MEM_WRITE_ONLY, &surface, 0, &status);
    if (status != CL_SUCCESS)
      CV_Error(cv::Error::OpenCLApiCallError,
               "OpenCL: clCreateFromVA_APIMediaSurfaceINTEL failed (Y plane)");
    cl_mem clImageUV = clCreateFromVA_APIMediaSurfaceINTEL(
        context, CL_MEM_WRITE_ONLY, &surface, 1, &status);
    if (status != CL_SUCCESS)
      CV_Error(cv::Error::OpenCLApiCallError,
    ...
    status =
        clEnqueueAcquireVA_APIMediaSurfacesINTEL(q, 2, images, 0, NULL, NULL);
    if (status != CL_SUCCESS)
      CV_Error(cv::Error::OpenCLApiCallError,
               "OpenCL: clEnqueueAcquireVA_APIMediaSurfacesINTEL failed");
    if (!ocl::ocl_convert_bgr_to_nv12(clBuffer, (int)u.step[0], u.cols, u.rows,
                                      clImageY, clImageUV))
      CV_Error(cv::Error::OpenCLApiCallError,
...
void convertFromVASurface(VADisplay display, VASurfaceID surface, Size size,
                          OutputArray dst) {
  (void)display;
  (void)surface;
  (void)dst;
  ...
  // TODO Need to specify ACCESS_WRITE here somehow to prevent useless data
  // copying!
  ...
  if (contextInitialized) {
    ...
    cl_mem clImageY = clCreateFromVA_APIMediaSurfaceINTEL(
        context, CL_MEM_READ_ONLY, &surface, 0, &status);
    if (status != CL_SUCCESS)
      CV_Error(cv::Error::OpenCLApiCallError,
               "OpenCL: clCreateFromVA_APIMediaSurfaceINTEL failed (Y plane)");
    cl_mem clImageUV = clCreateFromVA_APIMediaSurfaceINTEL(
        context, CL_MEM_READ_ONLY, &surface, 1, &status);
    if (status != CL_SUCCESS)
      CV_Error(cv::Error::OpenCLApiCallError,
               "OpenCL: clCreateFromVA_APIMediaSurfaceINTEL failed (UV plane)");
    ...
    if (status != CL_SUCCESS)
      CV_Error(cv::Error::OpenCLApiCallError,
               "OpenCL: clEnqueueAcquireVA_APIMediaSurfacesINTEL failed");
    if (!ocl::ocl_convert_nv12_to_bgr(clImageY, clImageUV, clBuffer,
                                      (int)u.step[0], u.cols, u.rows))
      CV_Error(cv::Error::OpenCLApiCallError,
