...
bool createDirectories(const cv::String& path_) {
  ...
  if (pos == cv::String::npos) pos = path.rfind('\\');
...
struct FileLock::Impl {
  Impl(const char* fname) {
    ...
    do {
      handle =
          ::CreateFileA(fname, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE,
      ...
      if (INVALID_HANDLE_VALUE == handle) {
        if (ERROR_SHARING_VIOLATION == GetLastError()) {
          ...
          CV_ErrorNoReturn_(Error::StsAssert,
                            ("Can't open lock file: %s", fname));
        }
      }
      break;
    } while (numRetries > 0);
  ...
  bool lock() {
    ...
    return !!::LockFileEx(handle, LOCKFILE_EXCLUSIVE_LOCK, 0, MAXDWORD,
                          MAXDWORD, &overlapped);
  }
  bool unlock() {
    OVERLAPPED overlapped;
...
cv::String getCacheDirectory(const char* sub_directory_name,
  ...
  if (cache_path.empty()) {
    ...
    if (default_cache_path.empty()) {
      ...
      if (xdg_cache_env && xdg_cache_env[0] &&
          utils::fs::isDirectory(xdg_cache_env)) {
    ...
    if (!default_cache_path.empty()) {
      if (utils::fs::isDirectory(default_cache_path)) {
        default_cache_path = utils::fs::join(
            default_cache_path, utils::fs::join("opencv", CV_VERSION));
        ...
        if (!utils::fs::createDirectories(default_cache_path)) {
          CV_LOG_DEBUG(NULL, "Can't create OpenCV cache sub-directory: "
                                 << default_cache_path);
        } else {
      ...
      CV_LOG_DEBUG(NULL,
                   "OpenCV has no support to discover default cache directory "
                   "on the current platform");
    }
  } else {
    if (cache_path == "disabled") return cache_path;
    if (!isDirectory(cache_path)) {
      CV_LOG_WARNING(NULL,
                     "Specified non-existed directory, creating OpenCV "
                     "sub-directory for caching purposes: "
                         << cache_path);
      if (!createDirectories(cache_path)) {
        CV_LOG_ERROR(NULL,
                     "Can't create OpenCV cache sub-directory: " << cache_path);
        cache_path.clear();
      }
    }
  }
