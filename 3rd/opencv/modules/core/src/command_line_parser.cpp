...
struct CommandLineParser::Impl {
  ...
  std::vector<String> split_range_string(const String& str, char fs,
                                         char ss) const;
  std::vector<String> split_string(const String& str, char symbol = ' ',
                                   bool create_empty_item = false) const;
...
static const char* get_type_name(int type) {
  if (type == Param::INT) return "int";
  if (type == Param::BOOLEAN) return "bool";
  if (type == Param::UNSIGNED_INT) return "unsigned";
  if (type == Param::UINT64) return "unsigned long long";
  if (type == Param::FLOAT) return "float";
  if (type == Param::REAL) return "double";
  if (type == Param::STRING) return "string";
  return "unknown";
}
...
static void from_str(const String& str, int type, void* dst) {
  ...
  if (ss.fail()) {
    CV_Error_(Error::StsBadArg, ("can not convert: [%s] to [%s]", str.c_str(),
                                 get_type_name(type)));
  }
}

void CommandLineParser::getByName(const String& name, bool space_delete,
                                  int type, void* dst) const {
  CV_TRY {
      ...
      for (size_t j = 0; j < impl->data[i].keys.size(); j++) {
        if (name == impl->data[i].keys[j]) {
          ...
          if ((v.empty() && type != Param::STRING) || v == noneValue) {
            impl->error = true;
            impl->error_message =
                impl->error_message + "Missing parameter: '" + name + "'\n";
            return;
          }
  ...
  CV_CATCH(Exception, e) {
    impl->error = true;
    impl->error_message =
        impl->error_message + "Parameter '" + name + "': " + e.err + "\n";
    return;
  }
  CV_Error_(Error::StsBadArg, ("undeclared key '%s' requested", name.c_str()));
}

void CommandLineParser::getByIndex(int index, bool space_delete, int type,
                                   void* dst) const {
  CV_TRY {
    for (size_t i = 0; i < impl->data.size(); i++) {
      if (impl->data[i].number == index) {
        ...
        if ((v.empty() && type != Param::STRING) || v == noneValue) {
          impl->error = true;
          impl->error_message =
              impl->error_message + format("Missing parameter #%d\n", index);
          return;
        }
        from_str(v, type, dst);
        return;
  ...
  CV_CATCH(Exception, e) {
    impl->error = true;
    impl->error_message =
        impl->error_message + format("Parameter #%d: ", index) + e.err + "\n";
    return;
  }
  CV_Error_(Error::StsBadArg, ("undeclared position %d requested", index));
}

static bool cmp_params(const CommandLineParserParams& p1,
                       const CommandLineParserParams& p2) {
...
CommandLineParser::CommandLineParser(int argc, const char* const argv[],
  ...
  if (pos_s == String::npos) {
    ...
    impl->app_name =
        String(argv[0]).substr(pos_s + 1, String(argv[0]).length() - pos_s);
  }
...
CommandLineParser& CommandLineParser::operator=(
...
void CommandLineParser::Impl::apply_params(const String& key,
                                           const String& value) {
...
void CommandLineParser::printMessage() const {
  if (impl->about_message != "") printf("%s\n", impl->about_message.c_str());
  ...
  for (size_t i = 0; i < impl->data.size(); i++) {
    if (impl->data[i].number > -1) {
      String name =
          impl->data[i].keys[0].substr(1, impl->data[i].keys[0].length() - 1);
      printf("%s ", name.c_str());
    }
  }
...
std::vector<String> CommandLineParser::Impl::split_range_string(
    const String& _str, char fs, char ss) const {
  String str = _str;
    ...
    if (str[0] == fs) {
      if (begin == true) {
        CV_THROW(cv::Exception(CV_StsParseError,
                               String("error in split_range_string(") + str +
                                   String(", ") + String(1, fs) + String(", ") +
                                   String(1, ss) + String(")"),
                               "", __FILE__, __LINE__));
      }
      begin = true;
...
std::vector<String> CommandLineParser::Impl::split_string(
    const String& _str, char symbol, bool create_empty_item) const {
  String str = _str;
  std::vector<String> vec;
  String word = "";
