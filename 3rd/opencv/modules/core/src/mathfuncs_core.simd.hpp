...
void fastAtan32f(const float *Y, const float *X, float *angle, int len,
                 bool angleInDegrees);
void fastAtan64f(const double *Y, const double *X, double *angle, int len,
                 bool angleInDegrees);
void fastAtan2(const float *Y, const float *X, float *angle, int len,
               bool angleInDegrees);
...
struct v_atan {
  ...
  v_atan(const T &scale) : s(Trait::all(scale)) {
...
struct v_atan<double> {
  ...
  inline int operator()(int len, const double *Y, const double *X,
                        double *angle) {
    int i = 0;
...
template <typename T>
static inline void atanImpl(const T *Y, const T *X, T *angle, int len,
                            bool angleInDegrees) {
  int i = 0;
...
void invSqrt32f(const float *src, float *dst, int len) {
  ...
  for (; i < len; i++) dst[i] = 1 / std::sqrt(src[i]);
}
...
void sqrt32f(const float *src, float *dst, int len) {
  ...
  for (; i < len; i++) dst[i] = std::sqrt(src[i]);
}
...
void exp32f(const float *_x, float *y, int n) {
  ...
  static const float A4 = (float)(1.000000000000002438532970795181890933776 /
                                  EXPPOLY_32F_A0),
                     A3 = (float)(.6931471805521448196800669615864773144641 /
                                  EXPPOLY_32F_A0),
                     A2 = (float)(.2402265109513301490103372422686535526573 /
                                  EXPPOLY_32F_A0),
                     A1 = (float)(.5550339366753125211915322047004666939128e-1 /
                                  EXPPOLY_32F_A0);

#undef EXPPOLY
#define EXPPOLY(x) (((((x) + A1) * (x) + A2) * (x) + A3) * (x) + A4)
  ...
  if (n >= 8) {
    ...
    static const __m128 maxval4 =
        _mm_set1_ps((float)(exp_max_val / exp_prescale));
    static const __m128 minval4 =
        _mm_set1_ps((float)(-exp_max_val / exp_prescale));
    ...
    for (; i <= n - 8; i += 8) {
      ...
      __m256d xd0 = _mm256_cvtps_pd(
          _mm_min_ps(_mm_max_ps(_mm_loadu_ps(&x[i].f), minval4), maxval4));
      __m256d xd1 = _mm256_cvtps_pd(
          _mm_min_ps(_mm_max_ps(_mm_loadu_ps(&x[i + 4].f), minval4), maxval4));
      ...
      __m256 xf =
          _mm256_insertf128_ps(_mm256_castps128_ps256(_mm256_cvtpd_ps(xd0)),
                               _mm256_cvtpd_ps(xd1), 1);

      xf = _mm256_mul_ps(xf, postscale8);

      xi0 = _mm_packs_epi32(xi0, xi1);

      _mm_store_si128((__m128i *)tab_idx,
                      _mm_and_si128(xi0, _mm_set1_epi16(EXPTAB_MASK)));

      xi0 =
          _mm_add_epi16(_mm_srai_epi16(xi0, EXPTAB_SCALE), _mm_set1_epi16(127));
      ...
      __m256d yd0 = _mm256_set_pd(expTab[tab_idx[3]], expTab[tab_idx[2]],
                                  expTab[tab_idx[1]], expTab[tab_idx[0]]);
      __m256d yd1 = _mm256_set_pd(expTab[tab_idx[7]], expTab[tab_idx[6]],
                                  expTab[tab_idx[5]], expTab[tab_idx[4]]);

      // gcc does not support _mm256_set_m128
      //__m256 yf = _mm256_set_m128(_mm256_cvtpd_ps(yd1), _mm256_cvtpd_ps(yd0));
      __m256 yf =
          _mm256_insertf128_ps(_mm256_castps128_ps256(_mm256_cvtpd_ps(yd0)),
                               _mm256_cvtpd_ps(yd1), 1);

      //_mm256_set_m128i(xi1, xi0)
      __m256i temp = _mm256_castps_si256(
          _mm256_insertf128_ps(_mm256_castps128_ps256(_mm_castsi128_ps(xi0)),
                               _mm_castsi128_ps(xi1), 1));
  ...
  if (n >= 8) {
    ...
    for (; i <= n - 8; i += 8) {
      ...
      __m128d yd0 = _mm_unpacklo_pd(_mm_load_sd(expTab + tab_idx[0]),
                                    _mm_load_sd(expTab + tab_idx[1]));
      __m128d yd1 = _mm_unpacklo_pd(_mm_load_sd(expTab + tab_idx[2]),
                                    _mm_load_sd(expTab + tab_idx[3]));
      __m128d yd2 = _mm_unpacklo_pd(_mm_load_sd(expTab + tab_idx[4]),
                                    _mm_load_sd(expTab + tab_idx[5]));
      __m128d yd3 = _mm_unpacklo_pd(_mm_load_sd(expTab + tab_idx[6]),
                                    _mm_load_sd(expTab + tab_idx[7]));
...
#define EXPPOLY(x) \
  ...
  for (; i <= n - 4; i += 4) {
    ...
    _mm_store_si128((__m128i *)tab_idx,
                    _mm_and_si128(xi0, _mm_set1_epi32(EXPTAB_MASK)));

    xi0 =
        _mm_add_epi32(_mm_srai_epi32(xi0, EXPTAB_SCALE), _mm_set1_epi32(1023));
    ...
    __m128d yf0 = _mm_unpacklo_pd(_mm_load_sd(expTab + tab_idx[0]),
                                  _mm_load_sd(expTab + tab_idx[1]));
    __m128d yf1 = _mm_unpacklo_pd(_mm_load_sd(expTab + tab_idx[2]),
                                  _mm_load_sd(expTab + tab_idx[3]));
  ...
  for (; i <= n - 4; i += 4) {
    ...
    t = (int)(x[i].i >> 52);
    if ((t & 2047) > 1023 + 10) x0 = t < 0 ? -exp_max_val : exp_max_val;

    t = (int)(x[i + 1].i >> 52);
    if ((t & 2047) > 1023 + 10) x1 = t < 0 ? -exp_max_val : exp_max_val;

    t = (int)(x[i + 2].i >> 52);
    if ((t & 2047) > 1023 + 10) x2 = t < 0 ? -exp_max_val : exp_max_val;

    t = (int)(x[i + 3].i >> 52);
    if ((t & 2047) > 1023 + 10) x3 = t < 0 ? -exp_max_val : exp_max_val;
...
void log32f(const float *_x, float *y, int n) {
  ...
  static const float A0 = 0.3333333333333333333333333f, A1 = -0.5f, A2 = 1.f;
...
#define LOGPOLY(x) (((A0 * (x) + A1) * (x) + A2) * (x))
  ...
  for (; i <= n - 4; i += 4) {
    ...
    __m128i yi0 = _mm_sub_epi32(
        _mm_and_si128(_mm_srli_epi32(h0, 23), _mm_set1_epi32(255)),
        _mm_set1_epi32(127));
    __m128d yd0 = _mm_mul_pd(_mm_cvtepi32_pd(yi0), ln2_2);
    __m128d yd1 =
        _mm_mul_pd(_mm_cvtepi32_pd(_mm_unpackhi_epi64(yi0, yi0)), ln2_2);

    __m128i xi0 =
        _mm_or_si128(_mm_and_si128(h0, _mm_set1_epi32(LOGTAB_MASK2_32F)),
                     _mm_set1_epi32(127 << 23));

    h0 = _mm_and_si128(_mm_srli_epi32(h0, 23 - LOGTAB_SCALE - 1),
                       _mm_set1_epi32(LOGTAB_MASK * 2));
...
void log64f(const double *x, double *y, int n) {
  ...
  static const double
      A7 = 1.0,
      A6 = -0.5, A5 = 0.333333333333333314829616256247390992939472198486328125,
      A4 = -0.25, A3 = 0.2,
...
#define LOGPOLY(x, k)                            \
  ...
  static const __m128i log_and_mask2 =
      _mm_set_epi32(LOGTAB_MASK2, 0xffffffff, LOGTAB_MASK2, 0xffffffff);
  static const __m128i log_or_mask2 =
      _mm_set_epi32(1023 << 20, 0, 1023 << 20, 0);
  ...
  for (; i <= n - 4; i += 4) {
    ...
    __m128d xd0 = _mm_castsi128_pd(
        _mm_or_si128(_mm_and_si128(h0, log_and_mask2), log_or_mask2));
    __m128d xd1 = _mm_castsi128_pd(
        _mm_or_si128(_mm_and_si128(h1, log_and_mask2), log_or_mask2));

    h0 = _mm_unpackhi_epi32(_mm_unpacklo_epi32(h0, h1),
                            _mm_unpackhi_epi32(h0, h1));

    __m128i yi0 = _mm_sub_epi32(
        _mm_and_si128(_mm_srli_epi32(h0, 20), _mm_set1_epi32(2047)),
        _mm_set1_epi32(1023));
    __m128d yd0 = _mm_mul_pd(_mm_cvtepi32_pd(yi0), ln2_2);
    __m128d yd1 =
        _mm_mul_pd(_mm_cvtepi32_pd(_mm_unpackhi_epi64(yi0, yi0)), ln2_2);

    h0 = _mm_and_si128(_mm_srli_epi32(h0, 20 - LOGTAB_SCALE - 1),
                       _mm_set1_epi32(LOGTAB_MASK * 2));
    _mm_store_si128((__m128i *)idx, h0);
    h0 = _mm_cmpeq_epi32(h0, _mm_set1_epi32(510));
    ...
    xd0 = _mm_add_pd(
        xd0, _mm_and_pd(_mm_castsi128_pd(_mm_unpacklo_epi32(h0, h0)), shift2));
    xd1 = _mm_add_pd(
        xd1, _mm_and_pd(_mm_castsi128_pd(_mm_unpackhi_epi32(h0, h0)), shift2));
...
float fastAtan2(float y, float x) { return atanImpl<float>(y, x); }
