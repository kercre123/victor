...
struct RegionStatisticsStatus {
  ...
  void propagateFrom(const RegionStatisticsStatus& src) {
    _skipDepth = -1;
    if (src._skipDepth >= 0) enableSkipMode(0);
#ifdef HAVE_IPP
...
static inline std::ostream& operator<<(std::ostream& out,
                                       const RegionStatisticsStatus& s) {
  out << "ignore={";
  if (s._skipDepth >= 0) out << " SKIP=" << s._skipDepth;
#ifdef HAVE_IPP
  if (s.ignoreDepthImplIPP) out << " IPP=" << s.ignoreDepthImplIPP;
#endif
#ifdef HAVE_OPENCL
  if (s.ignoreDepthImplOpenCL) out << " OpenCL=" << s.ignoreDepthImplOpenCL;
#endif
#ifdef HAVE_OPENVX
  if (s.ignoreDepthImplOpenVX) out << " OpenVX=" << s.ignoreDepthImplOpenVX;
#endif
  out << "}";
  return out;
}
...
struct TraceManagerThreadLocal {
  ...
  struct StackEntry {
    ...
    StackEntry(Region* region_, const Region::LocationStaticStorage* location_,
               int64 beginTimestamp_)
        : region(region_),
          location(location_),
...
inline Region* getCurrentActiveRegion() {
  return getTraceManager().tls.get()->getCurrentActiveRegion();
}
inline Region* getCurrentRegion() {
  return getTraceManager().tls.get()->stackTopRegion();
}

void parallelForSetRootRegion(const Region& rootRegion,
                              const TraceManagerThreadLocal& root_ctx);
...
struct Region::LocationExtraData {
  ...
  static Region::LocationExtraData* init(
      const Region::LocationStaticStorage& location);
};
