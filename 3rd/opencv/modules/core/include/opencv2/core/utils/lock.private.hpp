...
class shared_lock_guard {
 public:
  ...
  explicit inline shared_lock_guard(Mutex& m) : mutex_(&m) {
    mutex_->lock_shared();
  }
...
/** @brief An optional simple scoped lock (RAII-style locking for
...
/** @brief An optional shared scoped lock (RAII-style locking for shared/reader
 * access).
 *
 * Doesn't lock if mutex pointer is NULL.
 *
...
class optional_shared_lock_guard {
 ...
 private:
  ...
  optional_shared_lock_guard& operator=(
      const optional_shared_lock_guard&);  // disabled
};
