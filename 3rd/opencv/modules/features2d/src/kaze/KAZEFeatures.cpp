...
void KAZEFeatures::Allocate_Memory_Evolution(void) {
  ...
  for (int i = 0; i <= options_.omax - 1; i++) {
    for (int j = 0; j <= options_.nsublevels - 1; j++) {
      ...
      aux.esigma =
          options_.soffset *
          pow((float)2.0f, (float)(j) / (float)(options_.nsublevels) + i);
...
int KAZEFeatures::Create_Nonlinear_Scale_Space(const Mat& img) {
  ...
  gaussian_2D_convolution(evolution_[0].Lt, evolution_[0].Lt, 0, 0,
                          options_.soffset);
  gaussian_2D_convolution(evolution_[0].Lt, evolution_[0].Lsmooth, 0, 0,
  ...
  for (size_t i = 1; i < evolution_.size(); i++) {
    ...
    gaussian_2D_convolution(evolution_[i - 1].Lt, evolution_[i].Lsmooth, 0, 0,
                            options_.sderivatives);

    // Compute the Gaussian derivatives Lx and Ly
    Scharr(evolution_[i].Lsmooth, evolution_[i].Lx, CV_32F, 1, 0, 1, 0,
           BORDER_DEFAULT);
    Scharr(evolution_[i].Lsmooth, evolution_[i].Ly, CV_32F, 0, 1, 1, 0,
           BORDER_DEFAULT);
    ...
    if (options_.diffusivity == KAZE::DIFF_PM_G1)
      ...
      weickert_diffusivity(evolution_[i].Lx, evolution_[i].Ly, Lflow,
                           options_.kcontrast);
...
void KAZEFeatures::Compute_KContrast(const Mat& img, const float& kpercentile) {
  options_.kcontrast = compute_k_percentile(
      img, kpercentile, options_.sderivatives, options_.kcontrast_bins, 0, 0);
}
...
class MultiscaleDerivativesKAZEInvoker : public ParallelLoopBody {
 public:
  explicit MultiscaleDerivativesKAZEInvoker(std::vector<TEvolution>& ev)
      : evolution_(&ev) {}

  void operator()(const Range& range) const {
    std::vector<TEvolution>& evolution = *evolution_;
    for (int i = range.start; i < range.end; i++) {
      compute_scharr_derivatives(evolution[i].Lsmooth, evolution[i].Lx, 1, 0,
                                 evolution[i].sigma_size);
      compute_scharr_derivatives(evolution[i].Lsmooth, evolution[i].Ly, 0, 1,
                                 evolution[i].sigma_size);
      compute_scharr_derivatives(evolution[i].Lx, evolution[i].Lxx, 1, 0,
                                 evolution[i].sigma_size);
      compute_scharr_derivatives(evolution[i].Ly, evolution[i].Lyy, 0, 1,
                                 evolution[i].sigma_size);
      compute_scharr_derivatives(evolution[i].Lx, evolution[i].Lxy, 0, 1,
                                 evolution[i].sigma_size);

      evolution[i].Lx = evolution[i].Lx * ((evolution[i].sigma_size));
      evolution[i].Ly = evolution[i].Ly * ((evolution[i].sigma_size));
      evolution[i].Lxx = evolution[i].Lxx * ((evolution[i].sigma_size) *
                                             (evolution[i].sigma_size));
      evolution[i].Lxy = evolution[i].Lxy * ((evolution[i].sigma_size) *
                                             (evolution[i].sigma_size));
      evolution[i].Lyy = evolution[i].Lyy * ((evolution[i].sigma_size) *
                                             (evolution[i].sigma_size));
    }
  }
...
/* ************************************************************************* */
/**
 * @brief This method computes the multiscale derivatives for the nonlinear
 * scale space
 */
...
class FindExtremumKAZEInvoker : public ParallelLoopBody {
 public:
  explicit FindExtremumKAZEInvoker(
      std::vector<TEvolution>& ev,
      std::vector<std::vector<KeyPoint> >& kpts_par, const KAZEOptions& options)
        ...
        for (int jx = 1; jx < options_.img_width - 1; jx++) {
          ...
          if (value > options_.dthreshold) {
            if (value >= *(evolution[i].Ldet.ptr<float>(ix) + jx - 1)) {
              ...
              if (check_maximum_neighbourhood(evolution[i].Ldet, 1, value, ix,
                                              jx, 1)) {
                // Now check on the lower scale
                if (check_maximum_neighbourhood(evolution[i - 1].Ldet, 1, value,
                                                ix, jx, 0)) {
                  // Now check on the upper scale
                  if (check_maximum_neighbourhood(evolution[i + 1].Ldet, 1,
                                                  value, ix, jx, 0))
                    is_extremum = true;
                }
              }
...
/* ************************************************************************* */
/**
 * @brief This method performs the detection of keypoints by using the
 * normalized score of the Hessian determinant through the nonlinear scale space
 * @param kpts Vector of keypoints
...
void KAZEFeatures::Do_Subpixel_Refinement(std::vector<KeyPoint>& kpts) {
  ...
  for (size_t i = 0; i < kpts_.size(); i++) {
    ...
    *(b.ptr<float>(2)) = -Ds;
    ...
    if (fabs(*(dst.ptr<float>(0))) <= 1.0f &&
        fabs(*(dst.ptr<float>(1))) <= 1.0f &&
        fabs(*(dst.ptr<float>(2))) <= 1.0f) {
      kpts_[i].pt.x += *(dst.ptr<float>(0));
      kpts_[i].pt.y += *(dst.ptr<float>(1));
      dsc = kpts_[i].octave + (kpts_[i].angle + *(dst.ptr<float>(2))) /
                                  ((float)(options_.nsublevels));
...
class KAZE_Descriptor_Invoker : public ParallelLoopBody {
 public:
  KAZE_Descriptor_Invoker(std::vector<KeyPoint>& kpts, Mat& desc,
...
void KAZEFeatures::Feature_Description(std::vector<KeyPoint>& kpts, Mat& desc) {
  ...
  parallel_for_(Range(0, (int)kpts.size()),
                KAZE_Descriptor_Invoker(kpts, desc, evolution_, options_));
}
 ...
 * 2006
 */
void KAZEFeatures::Compute_Main_Orientation(
    KeyPoint& kpt, const std::vector<TEvolution>& evolution_,
    const KAZEOptions& options) {
  ...
  for (int i = -6; i <= 6; ++i) {
    for (int j = -6; j <= 6; ++j) {
      if (i * i + j * j < 36) {
        ...
        if (iy >= 0 && iy < options.img_height && ix >= 0 &&
            ix < options.img_width) {
  ...
  for (ang1 = 0; ang1 < 2.0f * CV_PI; ang1 += 0.15f) {
    ang2 = (ang1 + (float)(CV_PI / 3.0) > (float)(2.0 * CV_PI)
 ...
 */
void KAZE_Descriptor_Invoker::Get_KAZE_Upright_Descriptor_64(
    ...
    while (j < pattern_size) {
      ...
      for (int k = i; k < i + 9; k++) {
        for (int l = j; l < j + 9; l++) {
          ...
          checkDescriptorLimits(x1, y1, options_.img_width,
                                options_.img_height);

          y2 = (int)(sample_y + 0.5f);
          x2 = (int)(sample_x + 0.5f);

          checkDescriptorLimits(x2, y2, options_.img_width,
                                options_.img_height);
 ...
 * Realtime Feature Detection and Matching, ECCV 2008
 */
void KAZE_Descriptor_Invoker::Get_KAZE_Descriptor_64(const KeyPoint& kpt,
                                                     float* desc) const {
  float dx = 0.0, dy = 0.0, mdx = 0.0, mdy = 0.0, gauss_s1 = 0.0,
        gauss_s2 = 0.0;
...
/* ************************************************************************* */
/**
 * @brief This method computes the extended upright descriptor (not rotation
 * invariant) of the provided keypoint
 * @param kpt Input keypoint
 * @param desc Descriptor vector
 * @note Rectangular grid of 24 s x 24 s. Descriptor Length 128. The descriptor
 * is inspired from Agrawal et al., CenSurE: Center Surround Extremas for
 * Realtime Feature Detection and Matching, ECCV 2008
 */
void KAZE_Descriptor_Invoker::Get_KAZE_Upright_Descriptor_128(
    const KeyPoint& kpt, float* desc) const {
...
/* ************************************************************************* */
/**
 * @brief This method computes the extended G-SURF descriptor of the provided
 * keypoint given the main orientation of the keypoint
 * @param kpt Input keypoint
 * @param desc Descriptor vector
 * @note Rectangular grid of 24 s x 24 s. Descriptor Length 128. The descriptor
 * is inspired from Agrawal et al., CenSurE: Center Surround Extremas for
 * Realtime Feature Detection and Matching, ECCV 2008
 */
void KAZE_Descriptor_Invoker::Get_KAZE_Descriptor_128(const KeyPoint& kpt,
                                                      float* desc) const {
  float gauss_s1 = 0.0, gauss_s2 = 0.0;
  float rx = 0.0, ry = 0.0, rrx = 0.0, rry = 0.0, len = 0.0, xf = 0.0, yf = 0.0,
        ys = 0.0, xs = 0.0;
