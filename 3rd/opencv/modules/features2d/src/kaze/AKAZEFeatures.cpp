 ...
 */
static inline void nld_step_scalar_one_lane(const Mat& Lt, const Mat& Lf,
                                            Mat& Lstep, float step_size,
...
class NonLinearScalarDiffusionStep : public ParallelLoopBody {
 public:
  ...
  void operator()(const Range& range) const {
    nld_step_scalar_one_lane(*Lt_, *Lf_, *Lstep_, step_size_, range.start,
                             range.end);
  }
...
#ifdef HAVE_OPENCL
static inline bool ocl_non_linear_diffusion_step(InputArray Lt_, InputArray Lf_,
                                                 OutputArray Lstep_,
                                                 float step_size) {
  if (!Lt_.isContinuous()) return false;
  ...
  return ker
      .args(ocl::KernelArg::ReadOnly(Lt), ocl::KernelArg::PtrReadOnly(Lf),
            ocl::KernelArg::PtrWriteOnly(Lstep), step_size)
      .run(2, globalSize, 0, true);
}
#endif  // HAVE_OPENCL

static inline void non_linear_diffusion_step(InputArray Lt_, InputArray Lf_,
                                             OutputArray Lstep_,
                                             float step_size) {
  CV_INSTRUMENT_REGION()
  ...
  parallel_for_(Range(0, Lt.rows),
                NonLinearScalarDiffusionStep(Lt, Lf, Lstep, step_size));
}
 ...
 */
static inline float compute_kcontrast(InputArray Lx_, InputArray Ly_,
                                      float perc, int nbins) {
  CV_INSTRUMENT_REGION()
...
#ifdef HAVE_OPENCL
static inline bool ocl_pm_g2(InputArray Lx_, InputArray Ly_, OutputArray Lflow_,
                             float kcontrast) {
  UMat Lx = Lx_.getUMat();
  UMat Ly = Ly_.getUMat();
...
static inline void compute_diffusivity(InputArray Lx, InputArray Ly,
                                       OutputArray Lflow, float kcontrast,
                                       int diffusivity) {
  CV_INSTRUMENT_REGION()
 ...
 * @param image Input image for which the nonlinear scale space needs to be
 * created
 */
template <typename MatType>
static inline void create_nonlinear_scale_space(
    InputArray image, const AKAZEOptions& options,
  ...
  GaussianBlur(img, evolution[0].Lsmooth, Size(ksize, ksize), options.soffset,
               options.soffset, BORDER_REPLICATE);
  ...
  float kcontrast = compute_kcontrast(Lx, Ly, options.kcontrast_percentile,
                                      options.kcontrast_nbins);
...
static inline void convertScalePyramid(
    const std::vector<Evolution<MatTypeSrc> >& src,
...
#ifdef HAVE_OPENCL
static inline bool ocl_compute_determinant(InputArray Lxx_, InputArray Lxy_,
                                           InputArray Lyy_, OutputArray Ldet_,
                                           float sigma) {
  ...
  return ker
      .args(ocl::KernelArg::PtrReadOnly(Lxx), ocl::KernelArg::PtrReadOnly(Lxy),
 ...
 */
static inline void compute_determinant(InputArray Lxx_, InputArray Lxy_,
                                       InputArray Lyy_, OutputArray Ldet_,
                                       float sigma) {
  CV_INSTRUMENT_REGION()
  ...
  CV_OCL_RUN(Lxx_.isUMat() && Ldet_.isUMat(),
             ocl_compute_determinant(Lxx_, Lxy_, Lyy_, Ldet_, sigma));

  // output determinant
  Mat Lxx = Lxx_.getMat(), Lxy = Lxy_.getMat(), Lyy = Lyy_.getMat(),
      Ldet = Ldet_.getMat();
 ...
 public:
  ...
  void operator()(const Range& range) const {
    ...
    for (int i = range.start; i < range.end; i++) {
      ...
      // free Lsmooth to same some space in the pyramid, it is not needed
      // anymore
      e.Lsmooth.release();

      // compute determinant scaled by sigma
      float sigma_size_quat =
          (float)(e.sigma_size * e.sigma_size * e.sigma_size * e.sigma_size);
 ...
 */
static inline void Compute_Determinant_Hessian_Response(
    UMatPyramid& evolution) {
  CV_INSTRUMENT_REGION()
...
static inline void Compute_Determinant_Hessian_Response(Pyramid& evolution) {
  ...
  parallel_for_(Range(0, (int)evolution.size()),
                DeterminantHessianResponse<Mat>(evolution));
}
 ...
 */
static inline bool find_neighbor_point(const int x, const int y,
                                       const Mat& mask, const int search_radius,
                                       int& idx) {
...
class FindKeypointsSameScale : public ParallelLoopBody {
 public:
  explicit FindKeypointsSameScale(const Pyramid& ev, std::vector<Mat>& kpts,
                                  float dthreshold)
 ...
 * @param keypoints_by_layers Output vectors of detected keypoints; one vector
...
void AKAZEFeatures::Find_Scale_Space_Extrema(
  ...
  parallel_for_(Range(0, (int)evolution_.size()),
                FindKeypointsSameScale(evolution_, keypoints_by_layers,
                                       options_.dthreshold));
  ...
  for (size_t i = 1; i < keypoints_by_layers.size(); i++) {
    ...
    const int diff_ratio =
        (int)evolution_[i].octave_ratio / (int)evolution_[i - 1].octave_ratio;
    const int search_radius = evolution_[i].sigma_size *
                              diff_ratio;  // size of keypoint in this level
    ...
    for (int y = 0; y < keypoints.rows; y++) {
      for (int x = 0; x < keypoints.cols; x++, j++) {
        ...
        if (find_neighbor_point(p_x, p_y, keypoints_by_layers[i - 1],
                                search_radius, idx)) {
  ...
  for (int i = (int)keypoints_by_layers.size() - 2; i >= 0; i--) {
    ...
    const int diff_ratio =
        (int)evolution_[i + 1].octave_ratio / (int)evolution_[i].octave_ratio;
    const int search_radius =
        evolution_[i + 1].sigma_size;  // size of keypoints in upper level
 ...
 * @param keypoints_by_layers Input vectors of detected keypoints, sorted by
 * evolution levels
...
class SURF_Descriptor_Upright_64_Invoker : public ParallelLoopBody {
 public:
  SURF_Descriptor_Upright_64_Invoker(std::vector<KeyPoint>& kpts, Mat& desc,
  ...
  void operator()(const Range& range) const {
    for (int i = range.start; i < range.end; i++) {
      Get_SURF_Descriptor_Upright_64(
          (*keypoints_)[i], descriptors_->ptr<float>(i), descriptors_->cols);
    }
  }

  void Get_SURF_Descriptor_Upright_64(const KeyPoint& kpt, float* desc,
                                      int desc_size) const;
...
class SURF_Descriptor_64_Invoker : public ParallelLoopBody {
 public:
  SURF_Descriptor_64_Invoker(std::vector<KeyPoint>& kpts, Mat& desc,
                             Pyramid& evolution)
      : keypoints_(&kpts), descriptors_(&desc), evolution_(&evolution) {}
  ...
  void Get_SURF_Descriptor_64(const KeyPoint& kpt, float* desc,
                              int desc_size) const;
...
class MSURF_Upright_Descriptor_64_Invoker : public ParallelLoopBody {
 public:
  MSURF_Upright_Descriptor_64_Invoker(std::vector<KeyPoint>& kpts, Mat& desc,
                                      Pyramid& evolution)
      : keypoints_(&kpts), descriptors_(&desc), evolution_(&evolution) {}
  ...
  void Get_MSURF_Upright_Descriptor_64(const KeyPoint& kpt, float* desc,
                                       int desc_size) const;
...
class MSURF_Descriptor_64_Invoker : public ParallelLoopBody {
 public:
  MSURF_Descriptor_64_Invoker(std::vector<KeyPoint>& kpts, Mat& desc,
                              Pyramid& evolution)
      : keypoints_(&kpts), descriptors_(&desc), evolution_(&evolution) {}
  ...
  void Get_MSURF_Descriptor_64(const KeyPoint& kpt, float* desc,
                               int desc_size) const;
...
class Upright_MLDB_Full_Descriptor_Invoker : public ParallelLoopBody {
 public:
  Upright_MLDB_Full_Descriptor_Invoker(std::vector<KeyPoint>& kpts, Mat& desc,
                                       Pyramid& evolution,
...
class Upright_MLDB_Descriptor_Subset_Invoker : public ParallelLoopBody {
 public:
  Upright_MLDB_Descriptor_Subset_Invoker(std::vector<KeyPoint>& kpts, Mat& desc,
                                         Pyramid& evolution,
                                         AKAZEOptions& options,
 ...
 private:
  ...
  Mat descriptorSamples_;  // List of positions in the grids to sample LDB bits
                           // from.
  Mat descriptorBits_;
};
...
class MLDB_Full_Descriptor_Invoker : public ParallelLoopBody {
 public:
  MLDB_Full_Descriptor_Invoker(std::vector<KeyPoint>& kpts, Mat& desc,
                               Pyramid& evolution, AKAZEOptions& options)
      : keypoints_(&kpts),
        descriptors_(&desc),
        evolution_(&evolution),
        options_(&options) {}
  ...
  void Get_MLDB_Full_Descriptor(const KeyPoint& kpt, unsigned char* desc,
                                int desc_size) const;
...
class MLDB_Descriptor_Subset_Invoker : public ParallelLoopBody {
 public:
  MLDB_Descriptor_Subset_Invoker(std::vector<KeyPoint>& kpts, Mat& desc,
                                 Pyramid& evolution, AKAZEOptions& options,
                                 Mat descriptorSamples, Mat descriptorBits)
      : keypoints_(&kpts),
        descriptors_(&desc),
        evolution_(&evolution),
        options_(&options),
  ...
  void Get_MLDB_Descriptor_Subset(const KeyPoint& kpt, unsigned char* desc,
                                  int desc_size) const;
 ...
 */
void AKAZEFeatures::Compute_Descriptors(std::vector<KeyPoint>& kpts,
  ...
  switch (options_.descriptor) {
                                          ...
                                          // to rotation
    {
      parallel_for_(
          ...
          MSURF_Upright_Descriptor_64_Invoker(kpts, desc, evolution_));
    } break;
    case AKAZE::DESCRIPTOR_KAZE: {
      parallel_for_(Range(0, (int)kpts.size()),
                    MSURF_Descriptor_64_Invoker(kpts, desc, evolution_));
    } break;
 ...
 * 2006
 */
static inline void Compute_Main_Orientation(KeyPoint& kpt,
                                            const Pyramid& evolution) {
...
void AKAZEFeatures::Compute_Keypoints_Orientation(
    std::vector<KeyPoint>& kpts) const {
  CV_INSTRUMENT_REGION()

  parallel_for_(Range(0, (int)kpts.size()),
                ComputeKeypointOrientation(kpts, evolution_));
}
 ...
 */
void MSURF_Upright_Descriptor_64_Invoker::Get_MSURF_Upright_Descriptor_64(
    const KeyPoint& kpt, float* desc, int desc_size) const {
  const int dsize = 64;
 ...
 * Realtime Feature Detection and Matching, ECCV 2008
 */
void MSURF_Descriptor_64_Invoker::Get_MSURF_Descriptor_64(const KeyPoint& kpt,
                                                          float* desc,
                                                          int desc_size) const {
  const int dsize = 64;
  CV_Assert(desc_size == dsize);
 ...
 * @param desc Descriptor vector
 */
void Upright_MLDB_Full_Descriptor_Invoker::Get_Upright_MLDB_Full_Descriptor(
    const KeyPoint& kpt, unsigned char* desc, int desc_size) const {
  ...
  const int sample_step[3] = {pattern_size, divUp(pattern_size * 2, 3),
...
void MLDB_Full_Descriptor_Invoker::MLDB_Fill_Values(
    float* values, int sample_step, const int level, float xf, float yf,
    float co, float si, float scale) const {
  const Pyramid& evolution = *evolution_;
...
void MLDB_Full_Descriptor_Invoker::MLDB_Binary_Comparisons(float* values,
                                                           unsigned char* desc,
                                                           int count,
                                                           int& dpos) const {
  int chan = options_->descriptor_channels;
 ...
 * @param desc Descriptor vector
 */
void MLDB_Full_Descriptor_Invoker::Get_MLDB_Full_Descriptor(
    const KeyPoint& kpt, unsigned char* desc, int desc_size) const {
  const int max_channels = 3;
  ...
  for (int lvl = 0; lvl < 3; lvl++) {
    ...
    MLDB_Fill_Values(values, sample_step[lvl], kpt.class_id, xf, yf, co, si,
                     scale);
 ...
 * @param desc Descriptor vector
 */
void MLDB_Descriptor_Subset_Invoker::Get_MLDB_Descriptor_Subset(
    const KeyPoint& kpt, unsigned char* desc, int desc_size) const {
  float rx = 0.f, ry = 0.f;
  ...
  CV_Assert((pattern_size & 1) == 0);
  const int sample_steps[3] = {pattern_size, divUp(pattern_size * 2, 3),
                               divUp(pattern_size, 2)};
 ...
 * @param desc Descriptor vector
 */
void Upright_MLDB_Descriptor_Subset_Invoker::Get_Upright_MLDB_Descriptor_Subset(
    const KeyPoint& kpt, unsigned char* desc, int desc_size) const {
  float di = 0.0f, dx = 0.0f, dy = 0.0f;
...
void generateDescriptorSubsample(Mat& sampleList, Mat& comparisons, int nbits,
  ...
  CV_Assert(nbits <= ssz &&
            "Descriptor size can't be bigger than full descriptor (486 = 162*3 "
            "- 3 channels)");
