 ...
 */
void gaussian_2D_convolution(const cv::Mat& src, cv::Mat& dst, int ksize_x,
 ...
 */
void image_derivatives_scharr(const cv::Mat& src, cv::Mat& dst, int xorder,
                              int yorder) {
...
void pm_g2(InputArray _Lx, InputArray _Ly, OutputArray _dst, float k) {
  ...
  for (int y = 0; y < sz.height; y++) {
    ...
    for (int x = 0; x < sz.width; x++) {
      dst_row[x] =
          1.0f /
          (1.0f + ((Lx_row[x] * Lx_row[x] + Ly_row[x] * Ly_row[x]) * k2inv));
    }
  }
}
/* ************************************************************************* */
 ...
 */
void weickert_diffusivity(InputArray _Lx, InputArray _Ly, OutputArray _dst,
                          float k) {
  _dst.create(_Lx.size(), _Lx.type());
  Mat Lx = _Lx.getMat();
  Mat Ly = _Ly.getMat();
 ...
 * Proceedings of Algorithmy 2000
 */
void charbonnier_diffusivity(InputArray _Lx, InputArray _Ly, OutputArray _dst,
                             float k) {
  _dst.create(_Lx.size(), _Lx.type());
  Mat Lx = _Lx.getMat();
  Mat Ly = _Ly.getMat();
  ...
  for (int y = 0; y < sz.height; y++) {
    ...
    for (int x = 0; x < sz.width; x++) {
      float den =
          sqrt(1.0f + inv_k * (Lx_row[x] * Lx_row[x] + Ly_row[x] * Ly_row[x]));
      dst_row[x] = 1.0f / den;
    }
  }
}
 ...
 * factor given an input image, the percentile (0-1), the gradient scale and the
 ...
 */
void compute_scharr_derivatives(const cv::Mat& src, cv::Mat& dst, int xorder,
                                int yorder, int scale) {
  Mat kx, ky;
...
class Nld_Step_Scalar_Invoker : public cv::ParallelLoopBody {
 public:
  Nld_Step_Scalar_Invoker(cv::Mat& Ld, const cv::Mat& c, cv::Mat& Lstep,
                          float _stepsize)
  ...
  void operator()(const cv::Range& range) const {
    ...
    for (int i = range.start; i < range.end; i++) {
      ...
      for (int j = 1; j < Lstep.cols - 1; j++) {
        float xpos =
            (c_curr[j] + c_curr[j + 1]) * (ld_curr[j + 1] - ld_curr[j]);
        float xneg =
            (c_curr[j - 1] + c_curr[j]) * (ld_curr[j] - ld_curr[j - 1]);
 ...
 */
void nld_step_scalar(cv::Mat& Ld, const cv::Mat& c, cv::Mat& Lstep,
                     float stepsize) {
  CV_INSTRUMENT_REGION()

  cv::parallel_for_(cv::Range(1, Lstep.rows - 1),
                    Nld_Step_Scalar_Invoker(Ld, c, Lstep, stepsize),
