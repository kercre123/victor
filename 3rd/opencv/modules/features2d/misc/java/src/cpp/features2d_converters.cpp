...
void vector_KeyPoint_to_Mat(std::vector<KeyPoint>& v_kp, Mat& mat) {
  ...
  for (int i = 0; i < count; i++) {
    KeyPoint kp = v_kp[i];
    mat.at<Vec<float, 7> >(i, 0) =
        Vec<float, 7>(kp.pt.x, kp.pt.y, kp.size, kp.angle, kp.response,
...
void vector_DMatch_to_Mat(std::vector<DMatch>& v_dm, Mat& mat) {
  ...
  for (int i = 0; i < count; i++) {
    DMatch dm = v_dm[i];
    mat.at<Vec<float, 4> >(i, 0) = Vec<float, 4>(
        (float)dm.queryIdx, (float)dm.trainIdx, (float)dm.imgIdx, dm.distance);
  }
}

void Mat_to_vector_vector_KeyPoint(Mat& mat,
                                   std::vector<std::vector<KeyPoint> >& vv_kp) {
  std::vector<Mat> vm;
  vm.reserve(mat.rows);
...
void vector_vector_KeyPoint_to_Mat(std::vector<std::vector<KeyPoint> >& vv_kp,
                                   Mat& mat) {
  std::vector<Mat> vm;
...
void Mat_to_vector_vector_DMatch(Mat& mat,
                                 std::vector<std::vector<DMatch> >& vv_dm) {
  std::vector<Mat> vm;
  vm.reserve(mat.rows);
  Mat_to_vector_Mat(mat, vm);
  for (size_t i = 0; i < vm.size(); i++) {
...
void vector_vector_DMatch_to_Mat(std::vector<std::vector<DMatch> >& vv_dm,
                                 Mat& mat) {
  std::vector<Mat> vm;
