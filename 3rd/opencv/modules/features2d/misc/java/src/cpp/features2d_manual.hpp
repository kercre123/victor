...
class CV_EXPORTS_AS(FeatureDetector) javaFeatureDetector {
 public:
  CV_WRAP void detect(const Mat& image, CV_OUT std::vector<KeyPoint>& keypoints,
  ...
  CV_WRAP bool empty() const { return wrapped->empty(); }
  ...
  /**
   * supported: FAST STAR SIFT SURF ORB MSER GFTT HARRIS BRISK AKAZE Grid(XXXX)
   * Pyramid(XXXX) Dynamic(XXXX) not supported: SimpleBlob, Dense
   * @deprecated
   */
...
class CV_EXPORTS_AS(DescriptorExtractor) javaDescriptorExtractor {
 public:
  ...
  CV_WRAP void compute(const std::vector<Mat>& images,
                       CV_IN_OUT std::vector<std::vector<KeyPoint> >& keypoints,
                       CV_OUT std::vector<Mat>& descriptors) const {
    return wrapped->compute(images, keypoints, descriptors);
  }

  CV_WRAP int descriptorSize() const { return wrapped->descriptorSize(); }

  CV_WRAP int descriptorType() const { return wrapped->descriptorType(); }
  ...
  javaDescriptorExtractor(Ptr<DescriptorExtractor> _wrapped)
      : wrapped(_wrapped) {}
