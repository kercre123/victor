...
typedef std::tr1::tuple<std::string, Ptr<FeatureDetector>, float, float>
    String_FeatureDetector_Float_Float_t;
const static std::string IMAGE_TSUKUBA = "features2d/tsukuba.png";
const static std::string IMAGE_BIKES =
...
static void matchKeyPoints(const vector<KeyPoint>& keypoints0, const Mat& H,
  ...
  for (int i0 = 0; i0 < static_cast<int>(keypoints0.size()); i0++) {
    ...
    for (size_t i1 = 0; i1 < keypoints1.size(); i1++) {
      if (nearestPointIndex >= 0 && usedMask[i1]) continue;
    ...
    if (nearestPointIndex >= 0) usedMask[nearestPointIndex] = 1;
  }
}

class DetectorInvariance
    : public TestWithParam<String_FeatureDetector_Float_Float_t> {
 protected:
  virtual void SetUp() {
    // Read test data
    const std::string filename =
        cvtest::TS::ptr()->get_data_path() + get<0>(GetParam());
...
TEST_P(DetectorRotationInvariance, rotation) {
  ...
  mask0(Rect(borderSize, borderSize, mask0.cols - 2 * borderSize,
             mask0.rows - 2 * borderSize))
      .setTo(Scalar(255));
  ...
  for (int angle = 0; angle < maxAngle; angle += angleStep) {
    Mat H =
        rotateImage(image0, mask0, static_cast<float>(angle), image1, mask1);
    ...
    for (size_t m = 0; m < matches.size(); m++) {
      if (matches[m].distance < minIntersectRatio) continue;
      ...
      ASSERT_FALSE(angle0 == -1 || angle1 == -1)
          << "Given FeatureDetector is not rotation invariant, it can not be "
             "tested here.";
      ...
      if (rotAngle0 >= 360.f) rotAngle0 -= 360.f;

      float angleDiff =
          std::max(rotAngle0, angle1) - std::min(rotAngle0, angle1);
      ...
      if (isAngleCorrect) angleInliersCount++;
    }

    float keyPointMatchesRatio =
        static_cast<float>(keyPointMatchesCount) / keypoints0.size();
    EXPECT_GE(keyPointMatchesRatio, minKeyPointMatchesRatio)
        << "angle: " << angle;

    if (keyPointMatchesCount) {
      float angleInliersRatio =
          static_cast<float>(angleInliersCount) / keyPointMatchesCount;
      EXPECT_GE(angleInliersRatio, minInliersRatio) << "angle: " << angle;
    }
#if SHOW_DEBUG_LOG
    std::cout << "angle = " << angle << ", keypoints = " << keypoints1.size()
              << ", keyPointMatchesRatio = " << keyPointMatchesRatio
              << ", angleInliersRatio = "
              << (keyPointMatchesCount
                      ? (static_cast<float>(angleInliersCount) /
                         keyPointMatchesCount)
                      : 0)
              << std::endl;
#endif
...
TEST_P(DetectorScaleInvariance, scale) {
  ...
  for (int scaleIdx = 1; scaleIdx <= 3; scaleIdx++) {
    ...
    EXPECT_LE(keypoints1.size(), keypoints0.size())
    ...
    float keyPointMatchesRatio =
        static_cast<float>(keyPointMatchesCount) / keypoints1.size();
    EXPECT_GE(keyPointMatchesRatio, minKeyPointMatchesRatio);

    if (keyPointMatchesCount) {
      float scaleInliersRatio =
          static_cast<float>(scaleInliersCount) / keyPointMatchesCount;
    ...
    std::cout << "scale = " << scale
              << ", keyPointMatchesRatio = " << keyPointMatchesRatio
              << ", scaleInliersRatio = "
              << (keyPointMatchesCount ? static_cast<float>(scaleInliersCount) /
                                             keyPointMatchesCount
                                       : 0)
              << std::endl;
#endif
...
INSTANTIATE_TEST_CASE_P(AKAZE_DESCRIPTOR_KAZE, DetectorRotationInvariance,
                        Value(IMAGE_TSUKUBA,
                              AKAZE::create(AKAZE::DESCRIPTOR_KAZE), 0.5f,
                              0.71f));
...
INSTANTIATE_TEST_CASE_P(AKAZE_DESCRIPTOR_KAZE, DetectorScaleInvariance,
                        Value(IMAGE_BIKES,
                              AKAZE::create(AKAZE::DESCRIPTOR_KAZE), 0.08f,
                              0.49f));
