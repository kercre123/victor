...
typedef std::tr1::tuple<std::string, Ptr<FeatureDetector>,
                        Ptr<DescriptorExtractor>, float>
...
#define Value(...) \
  Values(String_FeatureDetector_DescriptorExtractor_Float_t(__VA_ARGS__))

static void rotateKeyPoints(const vector<KeyPoint>& src, const Mat& H,
                            float angle, vector<KeyPoint>& dst) {
  // suppose that H is rotation given from rotateImage() and angle has value
  // passed to rotateImage()
  ...
  for (size_t i = 0; i < dst.size(); i++) {
    ...
    if (dstAngle >= 360.f) dstAngle -= 360.f;
    dst[i].angle = dstAngle;
  }
}

class DescriptorInvariance
    : public TestWithParam<String_FeatureDetector_DescriptorExtractor_Float_t> {
 protected:
  virtual void SetUp() {
    // Read test data
...
TEST_P(DescriptorRotationInvariance, rotation) {
  ...
  for (int angle = 0; angle < maxAngle; angle += angleStep) {
    ...
    float descInliersRatio =
        static_cast<float>(descInliersCount) / keypoints0.size();
    EXPECT_GE(descInliersRatio, minInliersRatio);
#if SHOW_DEBUG_LOG
    std::cout << "angle = " << angle << ", inliers = " << descInliersCount
              << ", descInliersRatio = "
              << static_cast<float>(descInliersCount) / keypoints0.size()
              << std::endl;
#endif
...
TEST_P(DescriptorScaleInvariance, scale) {
  ...
  for (int scaleIdx = 1; scaleIdx <= 3; scaleIdx++) {
    ...
    EXPECT_GE(descInliersRatio, minInliersRatio);
#if SHOW_DEBUG_LOG
    std::cout << "scale = " << scale << ", inliers = " << descInliersCount
              << ", descInliersRatio = "
              << static_cast<float>(descInliersCount) / keypoints0.size()
              << std::endl;
#endif
...
INSTANTIATE_TEST_CASE_P(AKAZE, DescriptorScaleInvariance,
                        Value(IMAGE_BIKES, AKAZE::create(), AKAZE::create(),
                              0.6f));

INSTANTIATE_TEST_CASE_P(AKAZE_DESCRIPTOR_KAZE, DescriptorScaleInvariance,
                        Value(IMAGE_BIKES,
                              AKAZE::create(AKAZE::DESCRIPTOR_KAZE),
                              AKAZE::create(AKAZE::DESCRIPTOR_KAZE), 0.55f));
