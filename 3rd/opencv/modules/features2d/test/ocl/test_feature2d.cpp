...
PARAM_TEST_CASE(Feature2DFixture, Ptr<Feature2D>, std::string) {
  ...
  virtual void SetUp() {
    ...
    // note: we use keypoints from CPU for GPU too, to test descriptors
    // separately
...
OCL_INSTANTIATE_TEST_CASE_P(AKAZE, Feature2DFixture,
                            testing::Combine(testing::Values(AKAZE::create()),
                                             TEST_IMAGES));

OCL_INSTANTIATE_TEST_CASE_P(
    AKAZE_DESCRIPTOR_KAZE, Feature2DFixture,
    testing::Combine(testing::Values(AKAZE::create(AKAZE::DESCRIPTOR_KAZE)),
                     TEST_IMAGES));
