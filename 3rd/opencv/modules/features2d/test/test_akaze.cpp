...
TEST(Features2d_AKAZE, detect_and_compute_split) {
  ...
  Ptr<Feature2D> ext = AKAZE::create(AKAZE::DESCRIPTOR_MLDB, 0, 3, 0.001f, 1, 1,
                                     KAZE::DIFF_PM_G2);
...
/**
 * This test is here to guard propagation of NaNs that happens on this image.
 * NaNs are guarded by debug asserts in AKAZE, which should fire for you if you
 * are lucky.
 *
 * This test also reveals problems with uninitialized memory that happens only
 * on this image. This is very hard to hit and depends a lot on particular
 * allocator. Run this test in valgrind and check for uninitialized values if
 * you think you are hitting this problem again.
 */
