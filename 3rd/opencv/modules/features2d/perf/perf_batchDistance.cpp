...
typedef perf::TestBaseWithParam<Norm_Destination_CrossCheck_t>
...
PERF_TEST_P(Norm_Destination_CrossCheck, batchDistance_8U,
  ...
  TEST_CYCLE() {
    batchDistance(queryDescriptors, trainDescriptors, dist, destinationType,
                  (isCrossCheck) ? ndix : noArray(), normType, knn, Mat(), 0,
                  isCrossCheck);
  }
...
PERF_TEST_P(Norm_CrossCheck, batchDistance_Dest_32S,
            testing::Combine(testing::Values((int)NORM_HAMMING,
                                             (int)NORM_HAMMING2),
                             testing::Bool())) {
  NormType normType = get<0>(GetParam());
  ...
  TEST_CYCLE() {
    batchDistance(queryDescriptors, trainDescriptors, dist, CV_32S,
                  (isCrossCheck) ? ndix : noArray(), normType, knn, Mat(), 0,
                  isCrossCheck);
  }
...
PERF_TEST_P(Source_CrossCheck, batchDistance_L2,
            testing::Combine(testing::Values(CV_8U, CV_32F), testing::Bool())) {
  ...
  declare.time(50);
  TEST_CYCLE() {
    batchDistance(queryDescriptors, trainDescriptors, dist, CV_32F,
                  (isCrossCheck) ? ndix : noArray(), NORM_L2, knn, Mat(), 0,
                  isCrossCheck);
  }
...
void generateData(Mat& query, Mat& train, const int sourceType) {
  const int dim = 500;
  const int queryDescCount = 300;  // must be even number because we split train
                                   // data in some cases in two
