...
#define CV_LOG_FATAL(tag, ...)                                  \
  for (;;) {                                                    \
    if (cv::utils::logging::getLogLevel() <                     \
        cv::utils::logging::LOG_LEVEL_FATAL)                    \
      break;                                                    \
    std::stringstream ss;                                       \
    ss << __VA_ARGS__;                                          \
    cv::utils::logging::internal::writeLogMessage(              \
        cv::utils::logging::LOG_LEVEL_FATAL, ss.str().c_str()); \
    break;                                                      \
  }
#define CV_LOG_ERROR(tag, ...)                                  \
  for (;;) {                                                    \
    ...
    ss << __VA_ARGS__;                                          \
    cv::utils::logging::internal::writeLogMessage(              \
        cv::utils::logging::LOG_LEVEL_ERROR, ss.str().c_str()); \
    break;                                                      \
  }
#define CV_LOG_WARNING(tag, ...)                                  \
  for (;;) {                                                      \
    ...
    ss << __VA_ARGS__;                                            \
    cv::utils::logging::internal::writeLogMessage(                \
        cv::utils::logging::LOG_LEVEL_WARNING, ss.str().c_str()); \
    break;                                                        \
  }
...
#define CV_LOG_INFO(tag, ...)                                  \
  for (;;) {                                                   \
    ...
    ss << __VA_ARGS__;                                         \
    cv::utils::logging::internal::writeLogMessage(             \
        cv::utils::logging::LOG_LEVEL_INFO, ss.str().c_str()); \
    break;                                                     \
  }
#endif
...
#define CV_LOG_DEBUG(tag, ...)                                  \
  for (;;) {                                                    \
    ...
    ss << __VA_ARGS__;                                          \
    cv::utils::logging::internal::writeLogMessage(              \
        cv::utils::logging::LOG_LEVEL_DEBUG, ss.str().c_str()); \
    break;                                                      \
  }
#endif
...
#define CV_LOG_VERBOSE(tag, v, ...)                               \
  for (;;) {                                                      \
    if (cv::utils::logging::getLogLevel() <                       \
      ...
      break;                                                      \
    std::stringstream ss;                                         \
    ss << "[VERB" << v << ":" << cv::utils::getThreadID() << "] " \
       << __VA_ARGS__;                                            \
    cv::utils::logging::internal::writeLogMessage(                \
        cv::utils::logging::LOG_LEVEL_VERBOSE, ss.str().c_str()); \
    break;                                                        \
  }
#endif
