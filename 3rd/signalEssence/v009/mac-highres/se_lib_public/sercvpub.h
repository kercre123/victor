...
typedef struct {
  ...
  int32 BypassEntireRcvPath;  // one bypass to rule them all (overrides
                              // individual bypasses)

  int32 BypassDcRemove;
  int32 BypassNoiseReduction;
  int32 BypassPreClAgc;  // 0=normal, 1=bypass
  int32 BypassLrHpf;     // Bypass Level Responsive High Pass Filter, 0=normal,
                         // 1=bypass
  int32 BypassEQ;
...
typedef struct {
  ...
  int32 RoutNumOutputs;  // total number of output channels =  (#input channels)
                         // * (#crossover channels)
} SeRcvPublicObject_t;
...
/*=======================================================================
   ...
   Returns:         void
=======================================================================*/
void SeRcvSetConfigDefaults(SeRcvConfig_t *p, float32 blockTimeS,
                            float32 sampleRateHz_Rin, float32 sampleRateHz_Rcv,
                            float32 SampleRateHz_Rout);

void SeRcvInit(SeRcvConfig_t *pConfig, SeRcvPublicObject_t *pp);
void SeRcvDestroy(SeRcvPublicObject_t *pp);

void SeProcessRcv(SeRcvPublicObject_t *pp,
                  SeRcvParamsFromTxPath_t *paramsFromTxPath,
                  const int16 *rinPtr, int16 *routPtr);
