...
void MultiAecSetDefaultConfig(MultiAecConfig_t *pConfig, uint16 blockSize,
                              uint16 numMics, uint16 sampleRateHz);

//
// get implementation specific aec config object;
// you need to recast the returned pointer into the appropriate
// AEC config type
void *MultiAecGetConfig(MultiAecConfig_t *pConfig,
                        MetaAecImplementation_t type);
