...
typedef struct {
  ...
  uint16
      BeamIndex[MAX_SEARCH_BEAMS]
               [MAX_BEAM_WIDTH_INDEXES];  // shouldn't this use MAX_SEARCH_BEAMS
                                          // instead of MAX_SOLUTIONS
  uint16
      NumInSet[MAX_SEARCH_BEAMS]
              [MAX_BEAM_WIDTH_INDEXES];  // shouldn't this use MAX_SEARCH_BEAMS
                                         // instead of MAX_SOLUTIONS
...
typedef struct {
  ...
  int16 NoCompDelay;
  uint16 NumSubbands;
  uint16 NumNoiseRefCandidates;  // presume the same number per subband -- not
                                 // currently used
  uint16 NumSolutionsInclNr[MAX_SUBBANDS_EVER];  // number of all beams desired
                                                 // and NR
  uint16 NumSolnCandidates[MAX_SUBBANDS_EVER];   // number of all beams, not
                                                 // including NR
  ...
  int16 MicCombineIndexTable[MAX_SUBBANDS_EVER][MAX_SOLUTIONS]
                            [MAX_MIC_CONTRIBS];
  int16 MicCombineDelayTable[MAX_SUBBANDS_EVER][MAX_SOLUTIONS]
                            [MAX_MIC_CONTRIBS];
...
typedef struct {
  ...
  // The enable solution array is the base at init time.   This gives and easy
  // way to restrict beams at the start, while another array in private is kept
  // to enable and disable beams depending on the direction-finding results.
