...
typedef struct {
  ...
  BeamCoordinate_t
      Coordinate;  // centimeters - uint16.  XYZ position of beam location
  float
      NearestDistanceCmFlt;  // centimeters - distance from talker position to
                             // the closest microphone to aid with AGC on large
                             // arrays. (See ComputeSoutGain, 4th parameter)
} FdBeamSpecObj_t;
...
typedef struct {
  ...
  uint16 TimeResolution_usec;  // effectively a request, see the main struct for
                               // the state
  ...
  float32 PowerCompareValue;  // sets floor of fuzzy lookup for power level
                              // confidence
  //    float32             PowerCompareScaleFactor; // correct PowerCmpareValue
  //    for sin gain
  ...
  float32 BeamMeritDeductFactor;  // 1.0 or greater, amount to subtract from the
                                  // rest of the merits for the ratio compare
