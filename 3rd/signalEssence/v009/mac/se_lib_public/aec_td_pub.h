...
typedef struct {
  ...
  uint32
      pMagChanModelRO[2];  // sum magnitude channel model (two channel models)
  ...
  uint16 latestStepSizeModifierRO;  // stepSizeModifier specified in latest
                                    // update call
  ...
  int32*(
      ppChanModel32RW[2]);  // ppChanModel32 is an array[2] of pointers to int32
  ...
  //
  // how many times has AecTdPickBestChanModel picked the non-active channel
  // model?
...
void AecTdSetDefaultConfig(AecTdConfig_t* pConfig, int32 blockSize,
                           int32 lenChanModel, int32 sampleRateHz);
