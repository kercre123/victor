...
SEDiagEnumDescriptor_t FallbackFlagDescriptor[] = {
    ...
    DIAG_ENUM_DECLARE(    FBF_FORCE_ECHO_CANCEL_WITH_NR, "select echo-cancelling beam with noise reference"),
...
void PolicyInit(void)
{
    ...
    id = SEDiagGetIndex("senr_bypass_all");
    SeAssert(id >= 0);
    sp->SEDiagId_SENR_Bypass_All = id;
...
void PolicyDoActions( void *p)
{
    ...
    if( (sp->KeyPhraseCountDown > 0) || sp->OverrideWithPointingLocationFlag || (sp->FallbackFlag!=FBF_AUTO_SELECT) )    
    {
        ...
        else if((sp->FallbackFlag == FBF_FORCE_ECHO_CANCEL)||(sp->FallbackFlag == FBF_FORCE_ECHO_CANCEL_WITH_NR))  // check if Echo Cancelling Beam
    ...
    // Currently, it is most important to treat the echo as the noise reference
    // There is a "beam" composed of two microphones that forms the noise reference signal.
    // These two microphones are closest to the loudspeaker.
    // Conceptually, it may be possible to establish a different noise reference toward the rear, but
    // for Anki is has been determined that echo is the biggest problem.
    // Currently, there is only a speech reco mode.  There is no telecom mode.

    // if EC NR mode then set both the echo cancelling beam and enable the noise reference noise reduction

    if (sp->FallbackFlag == FBF_FORCE_ECHO_CANCEL_WITH_NR)
    {
        SEDiagSetInt16(sp->SEDiagId_SENR_Bypass_All, 0);
    }
    else
    {
        SEDiagSetInt16(sp->SEDiagId_SENR_Bypass_All, 1);
    }
}
...
void PolicySetFallbackEchoCancellingWithNoiseReference(void)
{
    RobotAudioPolicy_t *sp;
    sp = &RobotAudioPolicyObj;

    // disable override
    sp->FallbackFlag = FBF_FORCE_ECHO_CANCEL_WITH_NR ;
}
