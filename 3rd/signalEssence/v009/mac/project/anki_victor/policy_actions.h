...
typedef struct {
  ...
  int32 OverrideWithPointingLocationFlag;  // use relative Phi to set spatial
                                           // filter if set
  ...
  int32 KeyPhraseIndexMailbox;  // set to -1 if SE determined, to valid index if
                                // host determined
  ...
  float WeightedDiffRotate[MAX_POSITION_READINGS];  // polar coordinates of new
                                                    // data
