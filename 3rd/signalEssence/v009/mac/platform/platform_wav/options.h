...
typedef struct {
  char *filename;
  float *
      data;  // this is a 2-d array of impulse response data.  The data order
             // interleved, so when operated on, must use a stride of 'channels'
  sf_count_t frames;  // the number of frames in the data struct.
                      // frames*channels = total data length of data
...
typedef struct {
  wav_file_t sin[MAX_SIN_FILES];    // the sin input files.  each one is a
                                    // multi-channel .wav file.
  ir_file_t sin_ir[MAX_SIN_FILES];  // the impulse responses.  One for each sin
                                    // must be defined.
  fir_bank_t
      sin_firs[MAX_SIN_FILES];  // each sin gets an entire bank of fir filters.
...
int wav_file_read_block_short(wav_file_t *wav, fir_bank_t *fir_bank,
                              int16 *output, int channels, int frames);
