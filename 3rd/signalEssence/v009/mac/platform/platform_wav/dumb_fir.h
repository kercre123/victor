...
/** Initialize the dumb fir filter.  When done with the dumb_fir_t, be sure to
 * call dumb_fir_free on it. You can specify taps = NULL and ntaps == 0 in order
 * to specify a unity impulse response (i.e. copy in to out).
...
/** Run the dumb fir filter.  Each call takes ninput samples and generates
 * ninput output sampels.
 ...
 */
void fir_bank_run(fir_bank_t *bank, int16 *input, int16 *output, int channels,
