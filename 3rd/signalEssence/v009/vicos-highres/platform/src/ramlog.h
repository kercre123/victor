...
typedef struct {
  ...
  int has_looped;  // if has_looped, then all items are valid.  else, only
                   // elements up to current are valie.
  sem_t sem;
} ramlog_t;
 ...
 *
 */
typedef void (*ramlog_callback)(long long unsigned int time_ns,
                                ramlog_data_t *data);
