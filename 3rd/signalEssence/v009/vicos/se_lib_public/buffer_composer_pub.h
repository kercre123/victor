...
typedef struct {
  ...
  // Channel mapping:
  // The Nth source and channel index specifies what samples are to be copied
  // into the output buffer. e.g.  0: (SENR_OUT, 0_
...
void SoutSetDefaultConfig(BufferCompConfig_t *pConfig, int blockSize,
                          float32 sampleRateHz);
void SoutSetNumChans(BufferCompConfig_t *pConfig, int numChans);
void SoutSetChanSource(BufferCompConfig_t *pConfig, int outChanIndex,
                       BufferCompSrc_t source, int sourceIndex);
