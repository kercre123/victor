...
#define MAX_NUM_SIN_FILES \
  7  // arbitrary number, but used only by old-style declarations
typedef struct {
  int filesAreOpen;
  int useMMIF;
  ...
  int *pSrcFileChanIndexRin;  // e.g. [4,5] -> wav file channel 4 -> rin0, chan
                              // 5-> rin1
  int blockSizeRin;
...
int BrGetSampleBlocks(BlockReader_t *p, int16 *pRinSamps, int16 *pRefInSamps,
                      int16 *pSinSamps);
