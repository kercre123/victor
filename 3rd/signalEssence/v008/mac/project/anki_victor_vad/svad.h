...
typedef struct {
  ...
  int Activity;
  float PowerTrk;
  float ActivityRatio;  // ratio between power merit and max(noiseFloor,
                        // absThreshold)
...
//
int DoSVad(SVadObject_t *sp,
           float nfestConfidence,  // noise floor measurement confidence, set to
                                   // zero if gear noise, 1.0 otherwise
