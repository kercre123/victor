...
typedef enum {
  ...
  //
  TP_SERCV_JUMP_TO_CROSSOVER,    // A specfial return code that tells the SE_RCV
                                 // path to jump directly to the crossover and
                                 // skip other processing.
  TP_SERCV_JUMP_OVER_CROSSOVER,  // A specfial return code that tells the SE_RCV
                                 // path to jump directly PAST the crossover and
                                 // skip other processing.
  TP_LAST
} TapPointResult_t;

typedef TapPointResult_t (*TapPointFunc_t)(void *pInOut, int32 numChannels,
                                           int32 numElemPerChan,
                                           size_t sizeOfElem, void *arg);
...
TapPointResult_t TapPointCall(TapPoint_t tapPoint, void *pInOut,
                              int32 numChannels, int32 numElemPerChan,
                              size_t sizeOfElem);
