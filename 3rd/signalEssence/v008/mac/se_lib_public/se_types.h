...
#if (defined(SE_PLTFRM_ANSI) || defined(SE_PLTFRM_X86) || \
     defined(SE_PLTFRM_X86_64) || defined(SE_PLTFRM_ARM))
...
typedef int32 printf_d; /* This is a %d type.  On C55, %d must be int16, on most
                           everything else, it's an int32 */
typedef long int printf_ld;
typedef int32 enum_t; /** This is a weird type -- it's an int the same size as
                         an enum so that we can pass enums into the SEDiag as
                         ints and use a casting that matches properly. */
...
typedef float float32;
typedef int16 printf_d; /* This is a %d type.  On C55, %d must be int16, on most
                           everything else, it's an int32 */
typedef int32 printf_ld;
typedef int16 enum_t; /** This is a weird type -- it's an int the same size as
                         an enum so that we can pass enums into the SEDiag as
                         ints and use a casting that matches properly. */
...
#define ARRAY_SIZE(static_array) \
...
// WARN_UNUSED
// put WARN_UNUSED before a function name to issue warnings under GCC compile
// time when a functions return value is ignored. int WARN_UNUSED foo(void) {}
#if defined(__GNUC__)
// note that clang/LLVM also defines __GNUC__
// to see all predefined macros:  clang -dM -E -x c /dev/null | less
// warning  see http://clang.llvm.org/compatibility.html#inline regarding inline
