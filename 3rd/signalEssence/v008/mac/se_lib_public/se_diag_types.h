...
typedef enum {
  ...
  SE_DIAG_BASIC, /** A Parameter/diag that pretty much everybody will want to
                    look at and use. */
  SE_DIAG_INTERMEDIATE, /** Parameter/diag that is a little less common to
                           touch, may be touched by customers. */
  SE_DIAG_ADVANCED,     /** The diag is unlikely to be touched */
  SE_DIAG_DEBUG, /** This diag is really only for debugging, though we may want
                    to set or examine it. */
  SE_DIAG_LEVEL_LAST
} SEDiagLevel_t;
...
#define DIAG_ENUM_END \
  { 0, 0, 0 }
#define DIAG_ENUM_DECLARE(ENUM, DESCR) \
  { ENUM, #ENUM, DESCR }

struct SEDiag_t;
/* The type of the callback function  */
typedef void (*SEDiagCallback_t)(struct SEDiag_t *diag, void *value,
                                 int elem_index);
...
typedef struct SEDiag_t {
  ...
  /** The callback is used during write operations.  If this callback field is
   * non-null
   *  the function is called with a pointer to this diag struct, as well as the
   * user argument. */
