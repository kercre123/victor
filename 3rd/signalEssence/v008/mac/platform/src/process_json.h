...
{
    "set" : [{ "at_block": <at_block_spec>, "diags":
<dict_of_diags_and_values_to_set>} <more set blocks allowed...>], "dump": [{
"at_block": <at_block_spec>, "diags": <list_of_diags_to_dump>} <more dump blocks
allowed...>], "dump_filename" : "some_filename.json"
}
    <at_block_spec>  : "*"          	     <--- every block
                     : [0, 10, 30]  	     <--- before blocks 0, 10, 30
                     : "end"        	     <--- after last block
                     : [0, 10, 30, "end"] <--- before blocks 0, 10, 30, and
after the last. <dict_of_diags_and_values_to_set> : : { "my_diag_1" : 32 } : {
"my_diag_2" : [3, 4, 6.3], <list_of_diags_to_dump> : : [ "my_diag_1",
"my_diag_2", ... ]
...
So, a typical file might look like this:
{
    ...
    "dump"          : [ { "at_block" : "*",
                          "diags"    : [ "current_beam", "erle" ] <--- dump
every block these diags.
                        },
                        { "at_block" : "end",
                          "digas"    : [ "erle" ]
                        }
...
typedef void (*dump_file_start_t)(
    struct process_file_dumper *self, process_json_t *pj,
    const char
        *dump_filename);  // called once at beginning processing (init time)
typedef void (*dump_file_end_t)(
    struct process_file_dumper *self,
    process_json_t *pj);  // called once at end of processing    (destruct time)
typedef void (*dump_file_begin_block_t)(
    struct process_file_dumper *self, process_json_t *pj, int block,
    int is_last_block);  // called once for each block, at start of block
                         // processing.
typedef void (*dump_file_end_block_t)(
    struct process_file_dumper *self, process_json_t *pj, int block,
    int is_last_block);  // called once for each block, at end of block
                         // processing.
typedef void (*dump_file_sediag_start_t)(
    struct process_file_dumper *self, process_json_t *pj,
    const char *name);  // called once for each diag, for each block
typedef void (*dump_file_sediag_item_t)(
    struct process_file_dumper *self, process_json_t *pj,
    const char *valuestring,
    int is_last);  // called once for each diag, for each block
typedef void (*dump_file_sediag_end_t)(
    struct process_file_dumper *self, process_json_t *pj,
    int is_last_diag_this_block);  // called once for each diag, for each block
...
// process_json
//  call this once at startup.  This will slurp up the json file and check it
//  out.
