...
void send_packet(void)
  ...
  *packed_CRC_position=
  ...
  cf_SendData((ubyte *)&outgoing_response,outgoing_response.data_length+4);
  }
//============================================================================
//                              check_for_packet()
//
...
ubyte check_for_packet(void)
  ...
  if(MAX_DATA_LENGTH<(incoming_command.data_length=cf_PeekByte()))
    {
    //Throw out one byte of garbage. Next pass through should re-sync.
    cf_GetByte();
    return(0);
  ...
  if((int)cf_PeekBytesAvail()<(incoming_command.data_length+2))
  ...
  for(i=0;i<incoming_command.data_length;i++)
    incoming_command.data[i]=cf_PeekByte();
  //Now move over the CRC.
  incoming_command.CRC.as_bytes[0]=cf_PeekByte();
  incoming_command.CRC.as_bytes[1]=cf_PeekByte();
  //Now check the CRC.
