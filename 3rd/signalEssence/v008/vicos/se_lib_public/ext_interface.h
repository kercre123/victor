...
typedef int16 (*VDotProductQ15_i16_i16_t)(const int16 *xptr, const int16 *hptr,
                                          uint16 N);

typedef void (*ComputeRFir_t)(const int16 *pInput, const int16 *pCoef,
                              int16 *pOutput, uint16 ntapsminus1,
                              uint16 blocksize, uint16 decimation_factor);

typedef void (*AecConvolution_t)(const int16 *coeffs, const int16 *input,
                                 int32 *output, int rshifts, int lenChanModel,
                                 int blockSize, int lshifts);

typedef void (*Interp1toNCore_t)(uint16 inBlockSize, uint16 interpFactor,
                                 int16 **ppInSamps, int16 *pOut0,
                                 const int16 *coefAddr,
                                 uint16 numCoefsPerPhase);

typedef void (*VCMul_i16_t)(const int16 *xp, const int16 *yp, int16 *dest,
...
struct RFirF_t;
struct RFirF_ops_t {
  void (*init)(struct RFirF_t *rfirf, int coef_len, int blocksize,
               int deciFactor, const float32 *pBackwardsH);
