...
/* (C) Copyright 2009 Signal Essence; All Rights Reserved
  ...
  Description:
      ...
      if output is not obvious the the data type is denoted by adding a suffix
with _

      Nameing:
      <V><S><operation><operand_<data type operand>_<data type operand>_<data
type result>

      V -- if vector operation
      S -- if 2nd operand is a scalar, otherwise it is implied the 2nd operand
is V

      If all the operands are the same type then only one data type need be
listed. If two operands are the same and the result is different, then list the
operand type and then the result type. If the operands are different types, then
list all types in order of usage.
...
/*-----------------------------------------------------------------------
  ...
  Description:  Vector Add to a Vector.  Computes the element by element
                sum of two vectors.
-------------------------------------------------------------------------*/
typedef void (*VAdd_i16_t)(const int16 *src1, const int16 *src2, int16 *out,
                           uint16 N);
...
/*----------------------------------------------------------------------=
  ...
  Description:  Vector Add.  Adds two longword vectors together, element by
element.
...
/*=======================================================================
  ...
  Returns: void
=======================================================================*/
void VComplexPoweri16_i32(const int16 *inptr,  // complex vector
...
void VCMul_i16_Win32(const int16 *xptr,  // complex vector
...
/*----------------------------------------------------------------------
    ...
    Description:
      ...
      Multiplies each 16-bit integer element of a complex vector by a Q15
vector, complex element by real element to produce a 32-bit result and add that
      to an existing 32-bit result.

    Returns: void
-------------------------------------------------------------------------*/
typedef void (*VCRMulAdd_i16_i32_t)(
    const int16 *cPtr,      // complex input
...
/*=======================================================================
  ...
  Description:
      Geneates a complex tone from a phasor and initial state.
          This is useful for creating linear delay in the frequency domain, or
for creating a complex tone in the time domain.
...
typedef void (*VGenComplexTone_i16_t)(
    ...
    int16 *destPtr, uint16 numComplexOut);
...
/*----------------------------------------------------------------------
    ...
    Description:
       If an element is below the minLimitValue then it is set to the
minLimitValue

    Returns: void
-------------------------------------------------------------------------*/

void VLimitMin_i32(const int32 *src, int32 minLimitValue, int32 *dest,
                   uint16 N);

void VLimitMin_i16(const int16 *src, int16 minLimitValue, int16 *dest,
                   uint16 N);
...
/*----------------------------------------------------------------------
    ...
    Description:
      ...
      Takes the maximum value between each element in two vectors of 16 bit
words. output = max( src1, srcr2 );

    Returns: void
-------------------------------------------------------------------------*/
void VMax_i16(const int16 *src1Ptr, const int16 *src2Ptr, int16 *outPtr,
              uint16 N);
...
void VMax_i32(const int32 *src1Ptr, const int32 *src2Ptr, int32 *outPtr,
              uint16 N);
...
void VMin_i16(const int16 *src1Ptr, const int16 *src2Ptr, int16 *outPtr,
              uint16 N);

void VMinWithIndex_i16(const int16 *pV0, const int16 *pV1, int16 *pMinIndex,
                       int16 *pMinVal, uint16 N);

void VMin3WayWithIndex_i16(const int16 *pV0, const int16 *pV1, const int16 *pV2,
                           int16 *pMinIndex, int16 *pMinVal, uint16 N);
...
void VMin_i32(const int32 *src1Ptr, const int32 *src2Ptr, int32 *outPtr,
              uint16 N);
...
/*----------------------------------------------------------------------
    ...
    Description:
        Multiples a vector by a scaler and multiplies another vector by a scalar
and adds them to get a 32 bit result. C = A*a + B*b The scalars are Q15 values.

    Returns: void
-------------------------------------------------------------------------*/
void VMMQ15_Add_i32(const int32 *xptr, int16 a, const int32 *yptr, int16 b,
                    int32 *outPtr, uint16 N);
...
/*----------------------------------------------------------------------
    ...
    Returns: void
-------------------------------------------------------------------------*/
void VMul_i32_q15_i32(const int32 *src, const int16 *q15Ptr, int32 *dest,
                      int32 N);
void VMul_i32_q12_i32(const int32 *src, const int16 *q12Ptr, int32 *dest,
                      uint16 N);
void VMul_i32_q10_i32(const int32 *src, const int16 *q12Ptr, int32 *dest,
                      uint16 N);
...
/*----------------------------------------------------------------------
    ...
    Description:
      ...
      Vector subtract with saturation.  Computes the element by element
difference of of a two vectors.
...
typedef void (*VSub_i16_t)(const int16 *aptr, const int16 *bptr, int16 *dest,
                           uint16 N);
...
/*-------------------------------------------------------------------------
    ...
    Returns: void
-------------------------------------------------------------------------*/
void VTrackUpAveDown_i32(int32 *statePtr, const int32 *inPtr, int16 alpha,
                         int N);
...
/*-------------------------------------------------------------------------
    ...
    Description:
        ...
        Moves a vector to the right by moving element by element starting with
the last element.  Then retrieves data from a "previous" buffer to fill the area
in front. Finally, it saves

    Returns: void
-------------------------------------------------------------------------*/

void VDelaySave_i16(const int16 *src, int16 *dest, int16 *delayBufPtr,
                    int16 blockSize, int16 delaySize);
...
-------------------------------------------------------------------------*/
typedef uint32 (*VPower_i16_i32_t)(const int16 *inptr, uint16 rshifts,
                                   uint16 N);
...
typedef uint32 (*VPowerWithExponent_i16_i32_t)(const int16 *inptr,
                                               uint16 *expPtr, uint16 N);
...
-------------------------------------------------------------------------*/
typedef void (*VLeftShifts_i16_t)(const int16 *src, int16 numLefts, int16 *dest,
                                  uint16 N);
...
/*-------------------------------------------------------------------------
    ...
    Notes:
-------------------------------------------------------------------------*/
typedef void (*VShiftRtRnd_i32_i16_t)(const int32 *lsrc, int16 *sdest,
                                      uint16 shifts, uint16 N);
...
/*-------------------------------------------------------------------------
    ...
    Notes:
-------------------------------------------------------------------------*/
typedef void (*VShiftLtRnd_i16_i32_t)(const int16 *ssrc, int32 *ldest,
                                      uint16 shifts, uint16 N);
...
/*-------------------------------------------------------------------------
    ...
    Description:
      ...
      Vector Multiply by a ramping scalar.  Multiply all the elements of a
vector by a single scalar that is incremented every sample

    Returns: the last value of the scalar

    Notes:
-------------------------------------------------------------------------*/
int16 VMulQ15Ramp_i16(const int16 *src, int16 q15val, int16 delta, int16 *dest,
                      uint16 N);
...
/*-------------------------------------------------------------------------
    ...
    Notes:
-------------------------------------------------------------------------*/
void VMulSmoothed_i16(const int16 *src, int16 *statePtr, const int16 *alpha,
                      int16 target, int16 *dest, uint16 N);
...
/*-------------------------------------------------------------------------
    ...
    Description:
      ...
      Element-by-element vector multiply.  Multiply each of the elements of a
vector by another vector value in Q15 or Q12 format
...
-------------------------------------------------------------------------*/
typedef void (*VMul_i16_q15_i16_t)(const int16 *aptr, const int16 *bptr,
                                   int16 *dest, uint16 N);
extern VMul_i16_q15_i16_t VMul_i16_q15_i16;

// for testing
void VMul_i16_q15_i16_ansi(const int16 *src, const int16 *q15, int16 *dest,
                           uint16 N);
void VMul_i16_q12_i16(const int16 *aptr, const int16 *bptr, int16 *dest,
                      int32 N);
...
-----------------------------------------------------------------------*/
int16 VDotProductQ15_i16_i16_ansi(const int16 *xptr, const int16 *hptr,
                                  uint16 N);

typedef void (*VDotProductsLeftShift_q15_i16_t)(
    const int16 *inPtr,        // input
...
//
// for testing
void VDotProductsLeftShift_q15_i16_ansi(
    const int16 *inPtr,        // input
    const int16 *coefPtr,      // other input
    uint16 leftShifts,         // allow upscaling
    int16 *outPtr,             // output vector
...
void VWindowWithPreviousBlock_i16(
    const int16 *wfPtr,   // windowing function, size of window
...
typedef void (*VScale_i32_q15_i32_t)(const int32 *inPtr, int16 q15Scalar,
...
typedef void (*VScale_i16_q12_i16_t)(const int16 *src, int16 q12val,
                                     int16 *dest, int N);
extern VScale_i16_q12_i16_t VScale_i16_q12_i16;

typedef void (*VScale_i16_q15_i32_t)(const int16 *src, int16 q15val,
                                     int32 *dest, int N);
...
typedef void (*VScale_q31_q15_q15_t)(const int32 *px, int16 k, int16 *py,
                                     int N);
extern VScale_q31_q15_q15_t VScale_q31_q15_q15;

typedef void (*VScale_q31_q14_q15_t)(const int32 *px, int16 k_q14, int16 *py,
                                     int N);
...
/* ======================================================
 ...
 =========================================================*/
void VScaleAdd_i16_q15_i32_i32(const int16 *src, int16 q15val,
                               const int32 *i32src, int32 *dest, uint16 N);

void VScaleAdd_i16_q12_i32_i32(const int16 *src, int16 q12val,
                               const int32 *i32src, int32 *dest, uint16 N);

void VScaleAdd_i16_i16_i32_i32(const int16 *src, int16 i16val,
                               const int32 *i32src, int32 *dest, uint16 N);
void VScaleAdd_i16_q12_i16_i16(const int16 *s1p,
                               int16 i16val,  // Q12
                               const int16 *s2p, int16 *dest, uint16 N);

typedef void (*VScaleAdd_i16_q15_i16_i16_t)(const int16 *s1p,
                                            int16 i16val,  // Q15
                                            const int16 *s2p, int16 *dest,
                                            uint16 N);
