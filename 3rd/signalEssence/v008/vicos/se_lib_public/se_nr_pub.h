...
typedef enum {
  ...
  SENR_DISABLE_NOISE_REDUCTION,  // set final suppression gain to unity
                                 // but retain the overlap-add
                                 // analysis-synthesis rigamorole
...
typedef struct {
  ...
  uint16 UseNoiseRefSignal;   // 1 = use noise reference signal in calculations;
                              // 0 = ignore (treat as zero)
  uint16 UseNoiseFloorEstim;  // 1 = use estimated noise floor; 0 = ignore
                              // (treat as 0)
  ...
  int32 NumOptChans;  // number of opt solution/noise reference channels
...
void SenrSetDefaultConfig(SenrConfig_t *cp, int32 blockSize,
