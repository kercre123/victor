...
typedef struct {
  ...
  uint32
      NUnderflowReadsPostInit;  // number of underflow reads (post init write)
  int32 NWriteIncrReadDecr;     // counter:  write increments, read decrements
} SampleDelayQueue_t;

/* Function Prototypes */

void InitSampleDelayQueue(SampleDelayQueue_t *sp, int16 *bufAddr,
                          uint32 bufferSize,  // size of the allocated array
                          uint32 initialDelay, char *pSeDiagPrefix);

uint32 GetNumSamplesDelay(SampleDelayQueue_t *rp);

// void WriteSampleDelayQueue(
void SampleDelayWrite(SampleDelayQueue_t *rp, const int16 *inbufptr,
                      uint32 numSampsIn);

// void ReadSampleDelayQueue(
int SampleDelayPeek(SampleDelayQueue_t *rp, int16 *outbufptr,
                    uint32 numSamples);

int SampleDelayDequeue(SampleDelayQueue_t *sp, int16 *outBufPtr,
                       uint32 numSamples);
