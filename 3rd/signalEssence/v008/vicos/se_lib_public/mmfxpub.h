...
typedef struct {
  ...
  uint16 BypassSpatialFilterMicIndex;  // if spatial filter bypassed, select
                                       // this mic index
...
typedef struct {
  ...
  SystemTestsConfig_t SystemTestsConfig;  // narrowband noise generator sweep
                                          // parameters, sine generator
...
/* Function Prototypes */
void MMFxSetDefaultConfig(MMFxConfig_t *cfgp, int32 numMics, float32 blockTimeS,
                          float32 sampleRateHz_Send, float32 sampleRateHz_Sin,
                          float32 sampleRateHz_Sout, float32 sampleRateHz_Refin,
                          float32 sampleRateHz_Rcv, float32 sampleRateHz_Rin,
...
=======================================================================*/
int16 MMFxInit(MMFxPublicObject_t *pubp, MMFxConfig_t *cfgp);

//
// free/reset resources allocated during MMFxInit
void MMFxDestroy(MMFxPublicObject_t *pubp);

void MMFxProcessMicrophones(MMFxPublicObject_t *pp, const int16 *sinPtr,
                            const int16 *refPtr, int16 *outPtr);
...
/* ===========================================================================
    ...
    the Config data structure.   For the most part, this is a one-to-one copy,
but this could allow for additional interpretation, say from a floating point
...
==================================================================================
*/
void SpatialFilterSetDefaultConfig(SpatialFilterConfig_t *cp, int16 blockSize,
                                   int16 sampleRate_kHz);

void CopyLocationToBeamMapping(LocationToBeamMapping_t *srcp,
                               LocationToBeamMapping_t *destp);

void CopySpatialFilterSpec(SpatialFilterSpec_t *srcp,
                           SpatialFilterSpec_t *destp);

void MMFxDisableBeamSearchLocation(MMFxPublicObject_t *pMMFxPubObj,
                                   uint16 index);
void MMFxEnableBeamSearchLocation(MMFxPublicObject_t *pMMFxPubObj,
                                  uint16 index);
void MMFxDisableBeamSearchLocationList(MMFxPublicObject_t *pMMFxPubObj,
                                       const uint16 *disableList, int N);
void MMFxEnableBeamSearchLocationList(MMFxPublicObject_t *pMMFxPubObj,
                                      const uint16 *enableList, int N);
void MMFxEnableAllBeamSearchLocations(MMFxPublicObject_t *pMMFxPubObj);

void MMFxSetForcedSpatialBeamAllSubbands(MMFxPublicObject_t *pMMFxPubObj,
                                         uint16 beamIndex);
void MMFxClearForcedSpatialBeam(MMFxPublicObject_t *pMMFxPubObj);
void MMIfSetAutoLocationSearch(void);
void MMFxSetForcedNrSpatialBeamAllSubbands(MMFxPublicObject_t *pMMFxPubObj,
                                           uint16 beamIndex);
void MMFxClearForcedNrSpatialBeam(MMFxPublicObject_t *pMMFxPubObj);

void MMIfSetupScratchMemory(MMFxConfig_t *pMMFxConfig, char *pScratchH,
                            size_t lenH, char *pScratchX, size_t lenX,
                            char *pScratchS, size_t lenS);

void MMFxReconfigNoiseReduction(MMFxPublicObject_t *pMMFxPubObj,
                                const SenrConfig_t *pConfig);
