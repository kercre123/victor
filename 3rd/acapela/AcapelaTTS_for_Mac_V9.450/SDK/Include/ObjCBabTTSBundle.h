...
#define BABTTS_LIB OBJCBABTTSBUNDLE_ACATTS_LIB_PREFIX OBJCBABTTSBUNDLE_ACATTS_LIB_NAKEDNAME
#endif

#if !defined(BABTTS_LIBNAME)
#define BABTTS_LIBNAME OBJCBABTTSBUNDLE_ACATTS_LIB_NAKEDNAME
#endif
...
- (id)initWithBundleID:(long int)bundleID {
  char szTemp[512];
  Dl_info infoDylib;
  const char *libraryPathStr = [libraryPath cStringUsingEncoding:NSMacOSRomanStringEncoding];
  ...
  if (NULL != BabTtsBundleDLL) {
    PrivBabTTS_SetLicense =
        (pPrivBabTTS_SetLicense)dlsym(BabTtsBundleDLL, (const char *)"PrivBabTTS_SetLicense");

    if (NULL != PrivBabTTS_SetLicense) {
      NSLog(@"LoadSymbol of BabTtsBundleDLL OK");
      return self;
    } else {
      NSLog(@"Wrong version of AcaTTS: PrivBabTTS_SetLicense (%p)", PrivBabTTS_SetLicense);
