...
#define BABTTS_LIBNAME _T(BABTTSBUNDLE_ACATTS_LIB_NAKEDNAME)
#endif
...
class CTTSBundle {
 ...
 protected:
  ...
  bool SetLicense(const char* lpszLicense) {
    if (NULL == PrivBabTTS_SetLicense || NULL == lpszLicense) return false;
  ...
  void Init(BundleID bundleid, LPCTSTR lpszLib) {
    ...
    PrivBabTTS_SetLicense = (pPrivBabTTS_SetLicense)LOADSYMBOL(
        BabTtsBundleDLL, (const char*)DECL_PrivBabTTS_SetLicense);
