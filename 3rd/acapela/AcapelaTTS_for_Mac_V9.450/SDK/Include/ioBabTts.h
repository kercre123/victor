...
#if defined(WIN32)
#error \
    "WINAPI is not defined. Did you mess your header files order or forget to include windows.h or stdafx.h?"
#else
#define WINAPI
#endif
#endif
...
typedef enum {
  ...
  BABTTS_EVENT_SENTENCE = SETBIT(
      1),  // The phoneme is a sentence boundary (generally the first silence)
  BABTTS_EVENT_USERMRK = SETBIT(2),  // The phoneme position is synchronized
                                     // with the SAPI \Mrk=XXXX\ tag (User tag)
  BABTTS_EVENT_TEXTBEGIN = SETBIT(3),  // First phoneme of a text
  BABTTS_EVENT_VOLUME = SETBIT(
      4),  // Actually not used (Must correspond to the SAPI \VOL=XXX\ tag)
  BABTTS_EVENT_TEXTEND = SETBIT(6),  // Last phoneme of a text
  BABTTS_EVENT_TEXTPROCESSED =
      SETBIT(8),  // The text has been processed, duration is now known.
...
typedef enum {
  ...
  E_BABTTS_DICT_BADVERSION =
      -26,  // The dictionary version is too old ... Use the conversion tool
  E_BABTTS_LIBNOTINITIALIZED = -25,
  E_BABTTS_NOTVALIDLICENSE =
      -24,  // The license key is not valid for the requested function
  ...
  E_BABTTS_NOREGISTRY =
      -9,  // The required registry keys are not valid / do not exist
  ...
  E_BABTTS_ISPLAYING =
      -3,  // Already in play mode or currently processing text (obsolete)
  ...
  W_BABTTS_NOTFULLYPROCESSED =
      2,  // The processing was not fully processed. Need to call the function
          // one more time to complete the process.
...
typedef BabTtsError(WINAPI *BabTTSProc)(LPBABTTS lpBabTTS, DWORD dwMsg,
...
typedef struct {
  unsigned char
      bMouthHeight;  // Height of the mouth or lips. This is a linear range from
                     // 0xFF for the maximum possible height for the mouth to
                     // 0x00 for the minimum height (that is, the mouth or lips
                     // are closed).
  unsigned char bMouthWidth;  // Width of the mouth or lips. This is a linear
                              // range from 0xFF for the maximum possible width
                              // for the face to 0x00 for the minimum width
                              // (that is, the mouth or lips are puckered).
  unsigned char
      bMouthUpturn;  // Extent to which the mouth turns up at the corners (that
                     // is, how much it smiles). This is a linear range from
                     // 0xFF for the maximum upturn (that is, the mouth is fully
                     // smiling) to 0x00 if the corners of the mouth turn down.
                     // If this member is 0x80, the mouth is neutral.
  unsigned char
      bJawOpen;  // Angle to which the jaw is open. This is a linear range from
                 // 0xFF for completely open to 0x00 for completely closed.
  unsigned char
      bTeethUpperVisible;  // Extent to which the upper teeth are visible. This
                           // is a linear range from 0xFF for the maximum extent
                           // (that is, the upper teeth and gums are completely
                           // exposed) to 0x00 for the minimum (the upper teeth
                           // are completely hidden). If this member is 0x80,
                           // only the teeth are visible.
  unsigned char
      bTeethLowerVisible;  // Extent to which the lower teeth are visible. This
                           // is a linear range from 0xFF for the maximum extent
                           // (that is, the lower teeth and gums are completely
                           // exposed) to 0x00 for the minimum (the lower teeth
                           // are completely hidden.) If this member is 0x80,
                           // only the teeth are visible.
  unsigned char
      bTonguePosn;  // Tongue position. This a linear range from 0xFF if the
                    // tongue is against the upper teeth, to 0x00 if it is
                    // relaxed. If this member is 0x80, the tongue is visible.
  unsigned char
      bLipTension;  // Lip tension. This is a linear range from 0xFF if the lips
                    // are very tense to 0x00 if they are completely relaxed.
} BABTTSMOUTH;
...
#define BABTTSDICTPROC_ACTION_CONTINUEANDOVW \
  BABTTSDICTPROC_ACTION_CONTINUEANDSKIP
#define BABTTSDICTPROC_ACTION_CONTINUEANDKEEP \
  BABTTSDICTPROC_ACTION_CONTINUEANDSTOP
...
/* Prototype of the call-back function.
   ...
   the `BabTTS_SetSettings()` function with the `BABTTS_PARAM_CBINSTANCE`
   parameter (see your SDK Developper Guide). */
typedef DWORD(WINAPI *BabTTSDictProc)(void *lpUserData, DWORD dwCode,
