...
typedef struct BABILE_sEventCallBackStruct /* [OUT, given by TTS] !! scratch
                                              memory */
{
  BB_U32 size; /* sizeof the structure itself (should be >= to
                  BABILE_sEventCallBackStruct) */
  BB_S32
      event_type; /* CallBack type (?_MARK_TYPE_WORD, ?_MARK_TYPE_SENTENCE...)*/
  void* object;   /* pointer to an user defined object */
  BB_S32 pos; /* (start) position of the event (in chars) in the current text
                 buffer */
...
typedef BB_S32 (*BB_pMarkCallback)(
    BABILE_EventCallBackStruct* /*[OUT, scratch]*/);
...
typedef struct BABILE_sMemParam {
  ...
  BB_DbLs* synthLS;
  BB_DbLs* nlpeLS;
  void* u32MarkCallbackInstance; /* Allows to store an user defined object
                                    pointer inside BABILE object, the pointer is
                                    then returned by the callback */
