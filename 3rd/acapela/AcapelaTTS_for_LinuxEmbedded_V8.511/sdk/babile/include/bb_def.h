...
#define BB_GCC_ATTR_FMT(archetype, stridx, firstargidx) \
  BB_GCC_ATTR(__format__(archetype, stridx, firstargidx))
#define BB_GCC_ATTR_FMT_THIS(archetype, stridx, firstargidx) \
  BB_GCC_ATTR_FMT(archetype, (stridx + 1), (firstargidx + 1))
#define BB_GCC_ATTR_FMT_PRINTF(stridx, firstargidx) \
  BB_GCC_ATTR_FMT(_BB_GCC_ATTR_FMT_ARCHETYPE_PRINTF, stridx, firstargidx)
#define BB_GCC_ATTR_FMT_PRINTF_THIS(stridx, firstargidx) \
  BB_GCC_ATTR_FMT_THIS(_BB_GCC_ATTR_FMT_ARCHETYPE_PRINTF, stridx, firstargidx)
#define BB_GCC_ATTR_FMT_VPRINTF(stridx) \
  BB_GCC_ATTR_FMT(_BB_GCC_ATTR_FMT_ARCHETYPE_PRINTF, stridx, 0)
#define BB_GCC_ATTR_FMT_VPRINTF_THIS(stridx) \
  BB_GCC_ATTR_FMT(_BB_GCC_ATTR_FMT_ARCHETYPE_PRINTF, (stridx + 1), 0)
#define BB_GCC_ATTR_FMT_SCANF(stridx, firstargidx) \
  BB_GCC_ATTR_FMT(_BB_GCC_ATTR_FMT_ARCHETYPE_SCANF, stridx, firstargidx)
#define BB_GCC_ATTR_FMT_SCANF_THIS(stridx, firstargidx) \
  BB_GCC_ATTR_FMT_THIS(_BB_GCC_ATTR_FMT_ARCHETYPE_SCANF, stridx, firstargidx)
#define BB_GCC_ATTR_FMT_STRFTIME(stridx, firstargidx) \
  BB_GCC_ATTR_FMT(_BB_GCC_ATTR_FMT_ARCHETYPE_STRFTIME, stridx, firstargidx)
#define BB_GCC_ATTR_FMT_STRFTIME_THIS(stridx, firstargidx) \
  BB_GCC_ATTR_FMT_THIS(_BB_GCC_ATTR_FMT_ARCHETYPE_STRFTIME, stridx, firstargidx)
...
#if defined(__SYMBIAN32__) /* Take care that __SYMBIAN32__ and WIN32 may be \
                              defined together */
...
#if defined(__IAR_EWB__) ||                                     \
    (/* GCC (MinGW) and Clang for Win32 */ (defined(WIN32) ||   \
                                            defined(_WIN32)) && \
...
#endif
/*-----------------------------------------------------------------------------------*/
/* BB_malloc, BB_free, BB_alloca, fast_clock, BB_INLINE, FILE functions
 * definition */
#ifdef BB_CPLUSPLUS
extern "C" {
#endif
...
#endif /* HAVE_ALLOCA_H */
#if 0  /* IT WORKS BETTER (faster) WITH C ANSI functions than native SYmbian \
          ones!!!!*/
...
#define DBASPI_FSEEK(A, B, C)                                             \
  (C == SEEK_SET                                                          \
       ? dbHandle->setOffset = B                                          \
       : (C == SEEK_CUR                                                   \
              ? dbHandle->setOffset = dbHandle->current + B               \
              : (C == SEEK_END ? dbHandle->setOffset = dbHandle->size + B \
                               : 0)))
...
#pragma warning(push)
#pragma warning(disable : 4001) /* Avoid the warning in STD headers!!! (VS8) : \
                                   warning C4001: nonstandard extension        \
...
#elif defined(WIN32) || defined(_WIN32) || defined(TMS470Rxx_SIM) ||       \
    defined(DARWIN_PPC) || defined(DARWIN_I386) || defined(LINUX_INTEL) || \
    defined(SUN) || defined(CPU_RISC_ARM7) || defined(_WIN32_WCE) ||       \
    defined(__SYMBIAN32__) || defined(PALMOSARM) || defined(OS_VXWORKS) || \
    defined(__EPSON_C33__) || defined(ADS12) || defined(__HEW_IDE__) ||    \
...
#define BB_MAU16C8 \
  BB_BIT3 /* 16-bits width MAU compacted as 8-bits virtual addressing */
#define BB_MAU32C8 \
  BB_BIT4 /* 32-bits width MAU compacted as 8-bits virtual addressing */
#define BB_MAU32C16 \
  BB_BIT5 /* 32-bits width MAU compacted as 16-bits virtual addressing */
...
/* BB_NUMELMINARRAY(array)
   ...
   **NOT SUITABLE** for pointers nor arrays decayed as pointers.
   ...
   const BB_NF32 array2[] = { 1.5f, 3.1415f, 0.5f };
        ...
        Note: This macro accepts to compute the size of a pointer in some
   cases... it's unsafe but there's no way to safe-guard against this. Static
   analysis of the source code might help here. For more info, look here:
...
#define BB_NUMELMINARRAY(array)         \
  ((sizeof(array) / sizeof(*(array))) / \
   ((size_t)(!(sizeof(array) % sizeof(*(array))))))
#endif
...
#define BB_ASSERT_IMPL_PRINTMSG_(condStr, file, line)                          \
  {                                                                            \
    static char ASSERT_FAIL_buffer_[160u];                                     \
                                                                               \
    setvbuf(stderr, ASSERT_FAIL_buffer_, _IONBF, sizeof(ASSERT_FAIL_buffer_)); \
    fprintf(stderr,                                                            \
            "*** ASSERTION FAILURE: `%s` is false in file %s at "              \
            "line %u\n",                                                       \
            (condStr), (file), (line));                                        \
  }
...
#define BB_ASSERT_IMPL_MSGBOX_(condStr, file, line)                    \
  {                                                                    \
    static char ASSERT_FAIL_buffer_[160u];                             \
                                                                       \
    _snprintf(ASSERT_FAIL_buffer_, sizeof(ASSERT_FAIL_buffer_),        \
              "`%s` is "                                               \
              "false in file %s at line %u\n",                         \
              (condStr), (file), (line));                              \
    MessageBoxA(0u, ASSERT_FAIL_buffer_, "ASSERTION FAILURE",          \
...
/* BB_ASSERT(cond)
   ...
   from such builds. */
#if defined(_DEBUG) || defined(BB_CFG_DEBUG) || defined(BB_CFG_INTERNALRELEASE)
#define BB_ASSERT(cond) BB_ASSERT_IMPL_((cond), __FILE__, __LINE__);
#else
...
#define BB_UNREACHABLE_PRINTMSG_(file, line)                          \
  {                                                                   \
    static char UNREACH_POINT_buffer_[160u];                          \
                                                                      \
    setvbuf(stderr, UNREACH_POINT_buffer_, _IONBF,                    \
            sizeof(UNREACH_POINT_buffer_));                           \
    fprintf(stderr,                                                   \
            "*** UNREACHABLE POINT: Reached an unreachable point in " \
            "file %s at line %u\n",                                   \
            (file), (line));                                          \
    fflush(stderr);                                                   \
  }
...
#define BB_UNREACHABLE_MSGBOX_(file, line)                                    \
  {                                                                           \
    static char UNREACH_POINT_buffer_[160u];                                  \
                                                                              \
    _snprintf(UNREACH_POINT_buffer_, sizeof(UNREACH_POINT_buffer_),           \
              "Reached an unreachable point in file %s at line %u\n", (file), \
              (line));                                                        \
    MessageBoxA(0u, UNREACH_POINT_buffer_, "UNREACHABLE POINT",               \
                (MB_OK | MB_ICONERROR | MB_DEFBUTTON1 | MB_TASKMODAL |        \
                 MB_SETFOREGROUND));                                          \
  }
#else
...
#define BB_STATIC_ASSERT(condition, message)                        \
  {                                                                 \
    typedef char BB_STATIC_ASSERTION_TEST_FAILED__                  \
        line_##__LINE__##__##BB_PP_COUNTER()[(condition) ? 1 : -1]; \
  }
...
/* BB_SAFE_CONST_EXPR(expr)
   ...
   */
#define BB_SAFE_CONST_EXPR(expr) (BB_UNUSED_EXPRRES(expr), expr)
