...
#define BBSELECTOR_ROPI_NOVIRTUAL_ONLYOGG_0 /* Override of C++ virtuals \
                                               function to use only OGG */
#endif
...
#define READFILEDICO       /* Allows reading text dictionary files (dictl, \
                              dicm,dictman) */
#ifdef MALLOCED
#undef EMBEDDED
#endif
 ...
 * ============================================*/
/* all off by default!!! */
#define DEBUG_DLSTTS_0  /* should be moved to a NLPE configuration (it is not \
                           global) */
#define SAVEPHOONFILE_0 /* Hack in NLPE + babile_demo to outoout phonetics -> \
                           should become obsolette (console_babile)*/
...
#define OGG_DEBUG_MALLOC_0 /* Log mallocs / free (with EMBEDDED - for ogg - \
                              tremor) */
...
#define NO_MESSAGES /* removes messages */ /* handling of PRINT_MESSAGE,     \
                                              WARNING_MESSAGE, FATAL_MESSAGE \
                                            */
...
#if defined(WIN64) || defined(_WIN64) || defined(__LP64__) || \
    defined(_LP64) /* Not ported to 64 bits */
#undef BBNLP_COMPILEFLAG_TTFENABLE
#undef BBSELECTOR_COMPILEFLAG_OPUSENABLE
#endif
...
#if (defined(BBSELECTOR_ROPI_NOVIRTUAL_ONLYOGG)) /* Polymorphism is not \
                                                    allowed in this mode */
#if ((defined(BBSELECTOR_COMPILEFLAG_HNMENABLE) && \
      BBSELECTOR_COMPILEFLAG_HNMENABLE == 1) ||    \
     (defined(BBSELECTOR_COMPILEFLAG_WAVENABLE) && \
      BBSELECTOR_COMPILEFLAG_WAVENABLE == 1))
#error
#endif
#endif
...
#define BB_CONSOLE /* CONSOLE ( _CONSOLE) application OR (_WINDOWS + \
                      BABILSTATIC OR POCKETPC)*/
#endif
