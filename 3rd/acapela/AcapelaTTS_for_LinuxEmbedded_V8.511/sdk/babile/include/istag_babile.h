...
/* Example 1:	allows to call setSettings when the text is parsed by the NLP
   engine. Caution, this callBack is called when the text is parsed, so the
   setting is applied at this time. */
char const* const BABILE_tagStrings[] = {
    ...
    "mbv",
    /*BABIL_PARM_MBR_VOICEFREQ 56 */ /* Voice Frequency */
    "",                              /*						57 */
    "",                              /*						58 */
    "",                              /*						59 */
...
static BB_U32 BABILE_NLPE_tagCallFct(
    BABILE_Obj* obj, TagStr* tagstr) { /* obj has to be BABILE_Obj */
  BB_U32 store = 1;
  BB_TCHAR* name;
  BB_U8 i = 0;
  if (!tagstr || !tagstr->in_name || !tagstr->in_val || !obj ||
      tagstr->in_size < sizeof(TagStr))
    return store;
  ...
  while (BABILE_tagStrings[i] &&
         strcmp((const char*)name, BABILE_tagStrings[i]))
    i++;
...
static BB_U32 ALIAS_tagCallFct(
    void* obj, TagStr* tagstr) { /* obj has to be a SimpleDictionary */
  BB_U32 store = 1;
  BB_TCHAR *tag, *ist;
  ...
  if (!strcmp((const char*)tagstr->in_name, "alias")) {
    ...
    if (dicSimpleCb((SimpleDictionary*)obj, (char*)tagstr->in_val, (char**)&tag,
                    NULL, -1) >= 0) { /* replaces and store the sel=Unit tag */
      if ((ist = (BB_TCHAR*)strchr((const char*)tag, '=')) != NULL) {
        tagstr->out_newval = &ist[1];
        tagstr->out_newname = newName;
        len = ((BB_UPTR)ist - (BB_UPTR)tag) > sizeof(newName)
                  ? (int)sizeof(newName)
                  : (int)((BB_UPTR)ist - (BB_UPTR)tag);
    ...
    } else { /* else remove the tag */
      store = 0;
    }
  }
  return store;
}
static BB_U32 EXPAND_tagCallFct(
    void* obj, TagStr* tagstr) { /* obj has to be a SimpleDictionary */
  BB_U32 store = 1;
  BB_TCHAR* tag;
  static BB_TCHAR newName[12];
...
static BB_U32 MY_tagCallFct(void* obj, TagStr* tagstr) {
  ...
  if (tagstr && tagstr->in_size >= sizeof(TagStr) && obj) {
    ...
    } else if (!strcmp((const char*)tagstr->in_name, "expand") && o->dict) {
      return EXPAND_tagCallFct(o->dict, tagstr);
    } else
#endif /* _ISTAG_BABILE_USE_ALLIAS_ */
#ifdef __ACA_BB_TAGHNDL_H__
        if (!_stricmp((const char*)tagstr->in_name, "vct") ||
            !_stricmp((const char*)tagstr->in_name, "vol") ||
            !_stricmp((const char*)tagstr->in_name,
                      "rst")) { /* delays tag handling to synchronise it with
                                   output samples */
