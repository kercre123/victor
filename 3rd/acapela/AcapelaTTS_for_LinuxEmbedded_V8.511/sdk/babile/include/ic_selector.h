...
static const unsigned long BUFFER_FORCE =
    010000000000; /* Force the selected mode (otherwise it selects a beter one)
                   */
static const unsigned long DUMMY_LOAD =
    020000000000; /* ALLOCATE NOTHING AT ALL (used to check needed memory) */
static const unsigned long MAX_NB_PRESEL_MASK =
    000000000777; /* MAX NB Presel used to fix maximum Runtime Memory (1 to 511)
                   */
static const unsigned long DEFAULT_BUFFER_LEVEL =
    (BUFFER_UNITACOUSTIC_ALL | BUFFER_UNITSOFFSETS_ALL | BUFFER_BERSTREAM_ALL |
     MAX_NB_PRESEL_MASK | BUFFER_FORCE | BBSELECTOR_DEFAULT_MAX_NB_PRESEL);
/* better:
 * 50|BUFFER_UNITACOUSTIC_ALL|BUFFER_UNITSOFFSETS_SMALL|BUFFER_BERSTREAM_ALL */
#else
/*const uint32 BUFFER_UNITACOUSTIC_SMALL=	001000000000; Not available*/
...
#define DUMMY_LOAD \
  020000000000 /* ALLOCATE NOTHING AT ALL (used to check needed memory) */
#define MAX_NB_PRESEL_MASK                                                    \
  000000000777 /* MAX NB Presel used to fix maximum Runtime Memory (1 to 511) \
                */
#define DEFAULT_BUFFER_LEVEL                                                  \
  (BUFFER_UNITACOUSTIC_ALL | BUFFER_UNITSOFFSETS_ALL | BUFFER_BERSTREAM_ALL | \
   MAX_NB_PRESEL_MASK | BUFFER_FORCE | BBSELECTOR_DEFAULT_MAX_NB_PRESEL)
/* better:
 * 50|BUFFER_UNITACOUSTIC_ALL|BUFFER_UNITSOFFSETS_SMALL|BUFFER_BERSTREAM_ALL */
...
#define BBSELECTOR_SYNCONDURATION \
  0x0010 /* pass duration information for animated face! */
#define BBSELECTOR_SYNCONDIPHONES 0x0020 /* sync on diphones */
#define BBSELECTOR_PRESEL_MODE \
  0x2000 /* Debug mode !!! Output preselected units*/
#define BBSELECTOR_INDEX_MODE 0x8000 /* Debug mode !!! Output selected units*/
#define BBSELECTOR_SYNCSENT_FLAG                                               \
  0x4000 /* status FLAG used to remember that the SYNC point was already       \
            reached                                                            \
                                                 => set is reached, unset just \
            after SYNC point */
/* ******************* */
/* Callback constants */
 ...
 */
#define BBSELECTOR_SYNCDURCALL \
  7 /* return phoneme durations at once for the breath group */
...
#define BBSELECTOR_PARM_VOICESHAPE \
  7 /* Removed -> now external module (BABILE) */
#define BBSELECTOR_PARM_TIMERATIO 8 /* = 10000/BBSELECTOR_PARM_SPEEDRATIO */
#define BBSELECTOR_PARM_SELBREAK                                         \
  9 /* 0 -> No sub breath groups; >0 -> minimal length of the sub-breath \
       groups */
#define BBSELECTOR_PARM_UNITSVALIDITYCALLBACK 10
#define BBSELECTOR_PARM_SELECTUNIT \
  11 /* (Harmoniser) select this unit id for future use */
#define BBSELECTOR_PARM_MAKENUUT                                     \
  12 /* (Harmoniser) generates NUUT and WAV using selected units (by \
        BBSELECTOR_PARM_SELECTUNIT)*/
#define BBSELECTOR_PARM_RESETSELECTEDUNITS \
  13 /* (Harmoniser) reset selected units (by BBSELECTOR_PARM_SELECTUNIT) */
#define BBSELECTOR_PARM_RESERVED4 14 /* (Harmoniser) not used yet*/
#define BBSELECTOR_PARM_SPANAROUNDVOICEPITCHDELTA                          \
  15 /* (Picola optimisation)(special, hidden) Allow to limit pitch lookup \
        arround theoritical phonemùe pitch span can be positive or negative.                                              \
        lookup span is pitch delta (of phoneme) + span	*/
