static void BABILEDEMO_changeInitConfig(
    BABILE_MemParam* initializationParameters, const char* const loadconfig) {
  const char* const* c;
  const char* x = NULL;
  int i, trigger = 0;
  c = BABILE_sMBRConfigStrings;
  for (i = 0; c[i] != NULL && (*(c[i]) == 0 || !strstr(loadconfig, c[i]));
       i++) {
  }
  ...
  i = 0;
  do {
    for (; c[i] != NULL && (*(c[i]) == 0 || !(x = strstr(loadconfig, c[i])));
         i++) {
    }
    if (c[i] && x) {
      int v;
...
void ChangeSetting(BABILE_Obj* bab, BB_S16 param, char* av, FILE* FILEOUT) {
  ...
  if (0 > BABILE_getSetting(bab, param, &val)) {
    ...
    if (param < sizeof(BABILE_settingStrings) / sizeof(char*))
      fprintf(FILEOUT, "%s " BB_S32_PFMT " ", BABILE_settingStrings[param],
              val);
    val = atoi(av);
...
static void BABILEDEMO_changeSettings(FILE* FILEOUT, void* bab, int argpos,
                                      int argc, char** argv) {
  while (argpos < argc - 1) /* Changing parameters */
  {
    if (strcmp(argv[argpos], "VOICECTRL") == 0) {
      ...
      if (BABILE_settingStrings[j] == NULL &&
          !strcmp(argv[argpos], "DEFAULT")) {
