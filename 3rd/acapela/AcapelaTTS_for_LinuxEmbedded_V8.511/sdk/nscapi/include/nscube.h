...
#ifdef __cplusplus
#define NSCUBE_API extern "C" __attribute__((visibility("default")))
#else
#define NSCUBE_API extern __attribute__((visibility("default")))
#endif
...
#define MANDARINCHINESE 2052      /* mandarin chinese		    */
...
#define FINNISHSWEDISH 2077       /* finland swedish		    */
#define FINLANDSWEDISH 2077       /* finland swedish		    */
#define CATALAN 1027              /* catalan					*/
#define USSPANISH 21514           /* us spanish 				*/
#define SCANIAN 62493             /* scanian swedish		    */
#define GOTHENBURGSWEDISH 58397   /* gothenburg swedish       */
#define AUSTRALIAN 3081           /* australian english       */
#define AUSTRIANGERMAN 3079       /* austrian german          */
#define SAUDIARABIANENGLISH 49161 /* saudi arabia english     */
#define SCOTTISHENGLISH 1537      /* scottish english         */
...
#define NSC_ERR_INVALID_POINTER -24
#define NSC_INVALID_PARAM -25
#define NSC_INVALID_LICENSE -26
...
typedef struct {
  ...
  unsigned short uiIpa_Phoneme[MAX_IPA_PAR_PHO]; /* IPA phoneme
                                                  */
  unsigned int
      uiDuration;           /* phoneme duration in ms           */
  unsigned int uiByteCount; /* position in bytes in whole audio signal	*/
...
/* Prototype of the call back function used to retreive the sound data from the
 * TTS */
...
NSCUBE_API nscRESULT WINAPI nscCreateServerContext(int nAddressFamily,
                                                   int nPort,
                                                   const char *pSrvAddress,
                                                   nscHSRV *phSrv);

NSCUBE_API nscRESULT WINAPI nscCreateServerContextEx(int nAddressFamily,
                                                     int nCmdPort,
                                                     int nDataPort,
                                                     const char *pSrvAddress,
                                                     nscHSRV *phSrv);

NSCUBE_API nscRESULT WINAPI nscReleaseServerContext(nscHSRV hSrv);

NSCUBE_API nscRESULT WINAPI nscGetServerType(nscHSRV hSrv,
...
NSCUBE_API nscRESULT WINAPI nscFindFirstVoice(
    nscHSRV hSrv, const char *pVoiceName, int nSampleFreq, int nLanguage,
    int nGender, PNSC_FINDVOICE_DATA pFindVoiceData, nscHANDLE *phVoice);

NSCUBE_API nscRESULT WINAPI
nscFindNextVoice(nscHANDLE hFindVoice, PNSC_FINDVOICE_DATA pFindVoiceData);

NSCUBE_API nscRESULT WINAPI nscCloseFindVoice(nscHANDLE hFindVoice);

NSCUBE_API nscRESULT WINAPI nscCreateDispatcher(nscHANDLE *phDispatch);

NSCUBE_API nscRESULT WINAPI nscDeleteDispatcher(nscHANDLE phDispatch);

NSCUBE_API nscRESULT WINAPI nscInitChannel(nscHSRV hSrv, const char *pVoiceList,
                                           int nSampleFreq, int nCoding,
                                           nscHANDLE pDispatch,
                                           nscCHANID *pChId,
                                           const char *voiceMode = "");

NSCUBE_API nscRESULT WINAPI nscCloseChannel(nscHSRV hSrv, nscCHANID ChId);

NSCUBE_API nscRESULT WINAPI nscLockChannel(nscHSRV hSrv, nscCHANID ChId,
                                           nscHANDLE pDispatch,
                                           nscHANDLE *phTTS);

NSCUBE_API nscRESULT WINAPI nscUnlockChannel(nscHANDLE hTTS);

NSCUBE_API nscRESULT WINAPI nscNbVoice(nscHANDLE hTTS, int *pNbVoice);

NSCUBE_API nscRESULT WINAPI nscInfoVoice(nscHANDLE hTTS, int nVoice,
                                         PNSC_FINDVOICE_DATA pFindVoice);

NSCUBE_API nscRESULT WINAPI nscSwitchVoice(nscHANDLE hTTS, int nVoice);

NSCUBE_API nscRESULT WINAPI nscSwitchVoiceEx(nscHANDLE hTTS,
                                             const char *pszVoice);

NSCUBE_API nscRESULT WINAPI nscAddText(nscHANDLE hTTS, const char *pszInputTxt,
                                       void *pUserText);

NSCUBE_API nscRESULT WINAPI nscAddTextW(nscHANDLE hTTS,
                                        const wchar_t *pwszInputTxt,
                                        void *pUserText);

NSCUBE_API nscRESULT WINAPI nscAddTextEx(nscHANDLE hTTS,
                                         const char *pszInputEncoding,
                                         const void *pszInputText,
                                         const size_t sInputTextSize,
                                         void *pUserText);

NSCUBE_API nscRESULT WINAPI nscExecChannel(nscHANDLE hTTS,
                                           PNSC_EXEC_DATA pExecData);

NSCUBE_API nscRESULT WINAPI nscStartChannel(nscHANDLE hTTS,
                                            PNSC_EXEC_DATA pExecData);

NSCUBE_API nscRESULT WINAPI nscGetEvent(nscHANDLE hDispatch, int nTimeOut);

NSCUBE_API nscRESULT WINAPI nscProcessEvent(nscHANDLE hDispatch);

NSCUBE_API nscRESULT WINAPI nscGetandProcess(nscHANDLE hDispatch, int nTimeOut);

NSCUBE_API nscRESULT WINAPI nscExitChannel(nscHANDLE hTTS);

NSCUBE_API nscRESULT WINAPI nscPauseChannel(nscHANDLE hTTS);

NSCUBE_API nscRESULT WINAPI nscGetParamChannel(nscHANDLE hTTS,
                                               NSC_PRMTYPE_ENUM nParam,
                                               int *pnParamValue

);

NSCUBE_API nscRESULT WINAPI nscSetParamChannel(nscHANDLE hTTS,
                                               NSC_PRMTYPE_ENUM nParam,
                                               int nParamValue);

NSCUBE_API nscRESULT WINAPI nscGetParamChannelEx(nscHANDLE hTTS, int nType,
                                                 int nParam, int *pnParamValue

);

NSCUBE_API nscRESULT WINAPI nscSetParamChannelEx(nscHANDLE hTTS, int nType,
                                                 int nParam, int nParamValue);

NSCUBE_API nscRESULT WINAPI nscGetServerProt(nscHSRV hSrv,
                                             NSC_SRVPROT_ENUM *pServerProt);

NSCUBE_API nscRESULT WINAPI nscSetOemKEY(nscHSRV hSrv, char *pszKey);

NSCUBE_API nscRESULT WINAPI nscServerShutdown(nscHSRV hSrv, int nState);

NSCUBE_API nscRESULT WINAPI nscSetVoicesPaths(nscHSRV hSrv, char *paths);

NSCUBE_API nscRESULT WINAPI nscSetLog(nscHSRV hSrv, char *logPath,
                                      int logLevel);

NSCUBE_API char *WINAPI nscGetVersion(
