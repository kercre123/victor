...
/**
 * This class implements Settings Interface to manage Alexa Settings on the
 * product.
 *
 * This class writes the Setting change to database and notifies the observers
 * of the setting.
 * @see
...
class Settings
 ...
 public:
   ...
   * @param settingsStorage An interface to store, load, modify and delete
   * Settings.
   * @param globalSettingsObserver A set of SettingsGlobalObserver which are
   * notified when all the settings are changed.
   * @param dataManager A dataManager object that will track the
   * CustomerDataHandler.
   * @return An instance of Settings if construction is successful or nullptr if
   * construction fails.
   */
  static std::shared_ptr<Settings> create(
      ...
      std::unordered_set<std::shared_ptr<
          avsCommon::sdkInterfaces::GlobalSettingsObserverInterface>>
          observers,
   ...
   * @param settingObserver The observer to be added to @c
   * m_mapOfSettingsAttributes.
   */
   ...
   * @param key The name of the setting to which an observer is mapped to.
   * @param settingObserver The observer to be removed from @c
   * m_mapOfSettingsAttributes.
   */
   ...
   * @param settingsGlobalObserver The observer to be added to @c
   * m_globalSettingsObserver.
   */
  void addGlobalSettingsObserver(
      std::shared_ptr<avsCommon::sdkInterfaces::GlobalSettingsObserverInterface>
          globalSettingsObserver);

  /**
   * Remove the observer from the @c m_globalSettingsObserver.
   * @param settingsGlobalObserver The observer to be removed from @c
   * m_globalSettingsObserver.
   */
  ...
  /**
   * Function called by the application when a Setting is changed. It calls @c
   * executeChangeSetting via the executor.
   *
   * @param key The name of the setting which is changed.
   * @param value The new value of the setting.
   * @return A future which is @c true if the setting is changed successfully
   * else @c false.
   */
  std::future<bool> changeSetting(const std::string& key,
                                  const std::string& value);

  /**
   * Function which sends the default settings to AVS if the settings do not
   * already exist in the database. If the settings already exist, the event is
   * not sent.
   */
 ...
 private:
  ...
  struct SettingElements {
    ...
    std::unordered_set<std::shared_ptr<
        avsCommon::sdkInterfaces::SingleSettingObserverInterface>>
  ...
  /**
   * Function which implements the setting change. The function writes to the
   * database and notifies the observers of the setting.
   *
   * @param key The name of the setting which is changed.
   * @param value The new value of the setting.
   * @return A boolean which is @c true if the setting is changed successfully
   * else @c false.
   */
  ...
  /**
   * Variable which indicates whether to send the default Settings to AVS or
   * not. If the setting does not exist in the database, sendDefaultSetting is
   * set to true, and SettingsUpdated event is sent.
   */
