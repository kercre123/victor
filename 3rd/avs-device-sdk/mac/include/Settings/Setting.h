 ...
 *
 * This class provide methods to set value through local request (@c set method)
 * and through AVS request (@c setAvsChange). This setting should also be able
 * to persist the current value as well as revert to previous value if a
 * transaction fails.
 *
 * The setting can be configured to use multiple avs synchronization protocols.
 * See @c SettingProtocolInterface.
 *
...
class Setting : public SettingInterface<ValueT> {
 public:
   ...
   *
   * @param defaultValue The default value to be used if no value was found in
   * the storage.
   * @param settingProtocol The avs protocol used to persist and synchronize
   * values with avs.
   * @param applyValueFn Function responsible for validating and applying a new
   * setting value to the device.
  ...
  /**
   * Request to set the managed setting to the given @c value. This should be
   * used when the request came from AVS.
   *
 ...
 private:
   ...
   * Constructor.
   *
   * @param value The default value to be used if no value was found in the
   * storage.
   * @param applyValueFn Function responsible for applying a new setting value
   * on the device.
   * @param settingProtocol The avs protocol used to persist and synchronize
   * values with avs.
   */
  Setting(const ValueType& value,
          std::function<bool(const ValueType&)> applyValueFn,
          std::unique_ptr<SettingProtocolInterface> settingProtocol);

  /**
   * Implement the set value logic. This is a common path between avs / local
   * value change.
   *
   * @param value The target value.
   * @return A pair where @c first represents whether the value could be applied
   * and @c second is a string representation of @c m_value by the end of the
   * operation.
   */
  std::pair<bool, std::string> handleSetValue(const ValueType& value);

  /**
   * Restore function called during setting initialization. This should restore
   * the persisted value.
   *
  ...
  /// Function used to apply the new setting to the device. For example, for
  /// timezone, this function changes the device timezone.
  ...
  /// Old value used in case the protocol fails and the old value has to be
  /// restored.
...
std::shared_ptr<Setting<ValueT>> Setting<ValueT>::create(
  ...
  if (!settingProtocol) {
    avsCommon::utils::logger::acsdkError(
        LogEntry("Setting", "createFailed").d("reason", "nullSettingProtocol"));
    return nullptr;
  }

  auto setting = std::shared_ptr<Setting<ValueT>>(
      new Setting(defaultValue, applyValueFn, std::move(settingProtocol)));

  if (!setting->restore()) {
    avsCommon::utils::logger::acsdkError(
        LogEntry("Setting", "createFailed").d("reason", "restoreValueFailed"));
    return nullptr;
  }
...
template <typename ValueT>
std::pair<bool, std::string> Setting<ValueT>::handleSetValue(
    const ValueType& value) {
  bool ok;
  std::string valueStr;
  std::tie(ok, valueStr) = toSettingString<ValueT>(value);
  if (ok && (!this->m_applyFunction || this->m_applyFunction(value))) {
    avsCommon::utils::logger::acsdkInfo(
        LogEntry("Setting", __func__).d("value", valueStr));
    m_oldValue = this->m_value;
    this->m_value = value;
    return {true, valueStr};
  }
  avsCommon::utils::logger::acsdkError(
      LogEntry("Setting", "setValueFailed").d("reason", "applyFailed"));
...
bool Setting<ValueT>::setAvsChange(const ValueType& value) {
  ...
  auto notifyObservers = [this](SettingNotifications notification) {
...
bool Setting<ValueT>::restore() {
  auto applyChange =
      [this](const std::string& databaseValue) -> std::pair<bool, std::string> {
    ...
    if (databaseValue.empty()) {
      ...
      std::tie(convertOk, value) =
          fromSettingString<ValueT>(databaseValue, this->m_value);
    }
    std::lock_guard<std::mutex> lock{m_mutex};
...
Setting<ValueT>::Setting(
    const ValueType& value, std::function<bool(const ValueType&)> applyValueFn,
    std::unique_ptr<SettingProtocolInterface> settingProtocol)
