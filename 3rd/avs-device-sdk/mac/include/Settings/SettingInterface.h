...
/**
 * Interface for setting objects. Second template argument is used to restrict
 * usage to the types accepted.
 *
 * The setting implementation only support types that have the following
 * properties:
 * - copy assignable.
...
class SettingInterface {
 public:
  ...
  /**
   * Request to set the managed setting to the given @c value. Note that this is
   ...
   *
   * The observer will be executed every time the setting value changes or a
   * request fails.
   *
   * @param observer The connector to register for calling back whenever the
 ...
 protected:
  ...
  typename std::conditional<std::is_scalar<ValueType>::value,
                            std::atomic<ValueType>, GuardedValue>::type m_value;
};
...
template <typename ValueT>
bool SettingInterface<ValueT>::addObserver(
...
template <typename ValueT>
void SettingInterface<ValueT>::notifyObservers(
...
template <typename ValueT>
void SettingInterface<ValueT>::removeObserver(
    std::shared_ptr<ObserverType>& observer) {
  std::lock_guard<std::mutex>{m_observerMutex};
...
template <typename ValueT>
SettingInterface<ValueT>::SettingInterface(const ValueT& value)
    : m_value(value) {}
