...
/**
 * A basic class for performing basic SQLite database operations.  This the
 * boilerplate code used to manage the SQLiteDatabase.  This database is not
 * thread-safe, and must be protected before being used in a mutlithreaded
 * fashion.
 */
class SQLiteDatabase {
 public:
  ...
  class Transaction {
   ...
   private:
     ...
     * Constructor
     *
     * @param dbHandle weak pointer to a @c SQLiteDatabase instance that will
     * own the transaction.
     */
   ...
   *
   * @param filePath The location of the file that the SQLite DB will use as
   * it's backing storage when initialize or open are called.
   */
   ...
   * Destructor.
   *
   * On destruction, the DB is checked to see if it is closed.  It must be
   * closed before the SQLiteDatabase object is destroyed as there is no handle
   * to the DB to close it after this object is gone.
   */
  ~SQLiteDatabase();
   ...
   *
   * @return true, if the (empty) DB is created successfully.  If there is
   * already a DB at the path specified, this function fails and returns false.
   * It also returns false if the database is already opened or if there is an
   ...
   *
   * @return true, if the DB is opened successfully.  If there is no DB at the
   * path specified, this function fails and returns false.  It also returns
   * false if the database is already opened or if there is an internal failure
   * in creating the DB.
   */
  bool open();
   ...
   */
  std::unique_ptr<SQLiteStatement> createStatement(
 ...
 private:
   ...
   *
   * @return true if transaction has been successfully committed, false
   * otherwise.
   */
   ...
   * @return true if transaction has been successfully rolled back, false
   * otherwise.
   */
  ...
  /**
   * A shared_ptr to this that is used to manage viability of weak_ptrs to this.
   * This shared_ptr has a no-op deleter, and does not manage the lifecycle of
   * this instance.  Instead, ~SQLiteDatabase() resets this shared_ptr to signal
