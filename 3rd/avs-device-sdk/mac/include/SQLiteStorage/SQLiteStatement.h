...
/**
 * A utility class to simplify interaction with a SQLite statement.  In
 * particular, the resource management operations which are common to many
 * functions are captured in this class's constructor and destructor, as well as
 * making the underlying C-style interface more C++ friendly.
 *
 * The main operations of this class generally map to SQLite operations, so
 * please refer to the online documentation for SQLite for further guidance.
...
class SQLiteStatement {
 public:
  ...
  /**
   * Performs an iteration of the SQL query, which evaluates to either a single
   * row of the results, or completes the query.
   *
  ...
  /**
   * Resets a statement object to be re-executed with different bound
   * parameters.
   *
   ...
   *
   * @param index The SQL index (the left-most begins at 1, not 0) the value
   * should be bound to.
   ...
   * @return The name of the column.  If the index is out of bounds, the name
   * will be the empty string.
   */
  std::string getColumnName(int index) const;

  /**
   * Returns the text value of a particular column in the current row being
   * evaluated. NOTE: The left-most index for SQLite lookup operations begins at
   * 0.
   *
   * If the function is called with an out-of-bounds index, the result is
   * undefined. If the function is called with a valid index, but a different
   * type than specified in the table schema, then SQLite will perform an
   * implicit conversion as described by its online documentation here:
   *
   * https://sqlite.org/c3ref/column_blob.html
   *
   * If sufficient care is taken while using this class, then both errors above
   * can be avoided.
   *
   * @param index The index of the column being queried (left-most column is 0).
   * @return The text value of the column.  See function comment with respect to
   * error handling.
   */
  std::string getColumnText(int index) const;

  /**
   * Returns the integer value of a particular column in the current row being
   * evaluated. NOTE: The left-most index for SQLite lookup operations begins at
   * 0.
   *
   * If the function is called with an out-of-bounds index, the result is
   ...
   *
   * @param index The index of the column being queried (left-most column is 0).
   * @return The integer value of the column.  See function comment with respect
   * to error handling.
   */
  int getColumnInt(int index) const;

  /**
   * Returns the 64-bit integer value of a particular column in the current row
   * being evaluated. NOTE: The left-most index for SQLite lookup operations
   * begins at 0.
   *
   * If the function is called with an out-of-bounds index, the result is
   * undefined. If the function is called with a valid index, but a different
   ...
   *
   * @param index The index of the column being queried (left-most column is 0).
   * @return The 64-bit integer value of the column.  See function comment with
