...
/**
 * This class serves to instantiate each default component with of the SDK with
 * no specializations to provide an "out-of-box" component that users may
 * utilize for AVS interaction.
 */
class DefaultClient
 ...
 public:
  ...
  static const auto INVALID_INDEX =
      capabilityAgents::aip::AudioInputProcessor::INVALID_INDEX;

  /**
   * Creates and initializes a default AVS SDK client. To connect the client to
   * AVS, users should make a call to connect() after creation.
   *
   * @param deviceInfo DeviceInfo which reflects the device setup credentials.
   * @param customerDataManager CustomerDataManager instance to be used by
   * RegistrationManager and instances of all classes extending
   * CustomDataHandler.
   * @param externalMusicProviderMediaPlayers The map of <players, mediaPlayer>
   * to use to play content from each external music provider.
   * @param externalMusicProviderSpeakers The map of <players, speaker> to use
   * to track volume of each external music provider media player.
   * @param adapterCreationMap The map of <players, adapterCreationMethod> to
   * use when creating the adapters for the different music providers supported
   * by ExternalMediaPlayer.
   * @param speakMediaPlayer The media player to use to play Alexa speech from.
   * @param audioMediaPlayer The media player to use to play Alexa audio content
   * from.
   * @param alertsMediaPlayer The media player to use to play alerts from.
   * @param notificationsMediaPlayer The media player to play notification
   * indicators.
   ...
   * @param additionalSpeakers A list of additional speakers to receive volume
   * changes.
   * @param equalizerRuntimeSetup Equalizer component runtime setup
   * @param audioFactory The audioFactory is a component that provides unique
   * audio streams.
   * @param authDelegate The component that provides the client with valid LWA
   * authorization.
   * @param alertStorage The storage interface that will be used to store
   * alerts.
   * @param messageStorage The storage interface that will be used to store
   * certified sender messages.
   * @param notificationsStorage The storage interface that will be used to
   * store notification indicators.
   * @param settingsStorage The storage interface that will be used to store
   * settings (DEPRECATED).
   * @param deviceSettingStorage The storage interface that will be used to
   * store device settings.
   * @param bluetoothStorage The storage interface that will be used to store
   * bluetooth data.
   * @param alexaDialogStateObservers Observers that can be used to be notified
   * of Alexa dialog related UX state changes.
   * @param connectionObservers Observers that can be used to be notified of
   * connection status changes.
   * @param isGuiSupported Whether the device supports GUI.
   * @param capabilitiesDelegate The component that provides the client with the
   * ability to send messages to the Capabilities API.
   * @param contextManager The @c ContextManager which will provide the context
   * for various components.
   * @param transportFactory The object passed in here will be used whenever a
   * new transport object for AVS communication is needed.
   * @param firmwareVersion The firmware version to report to @c AVS or @c
   * INVALID_FIRMWARE_VERSION.
   * @param sendSoftwareInfoOnConnected Whether to send SoftwareInfo upon
   * connecting to @c AVS.
   * @param softwareInfoSenderObserver Object to receive notifications about
   * sending SoftwareInfo.
   * @return A @c std::unique_ptr to a DefaultClient if all went well or @c
   * nullptr otherwise.
   *
   * TODO: Allow the user to pass in a MediaPlayer factory rather than each
   * media player individually.
   */
  static std::unique_ptr<DefaultClient> create(
      ...
      const capabilityAgents::externalMediaPlayer::ExternalMediaPlayer::
          AdapterCreationMap& adapterCreationMap,
      std::shared_ptr<avsCommon::utils::mediaPlayer::MediaPlayerInterface>
          speakMediaPlayer,
      std::shared_ptr<avsCommon::utils::mediaPlayer::MediaPlayerInterface>
          audioMediaPlayer,
      ...
      std::shared_ptr<avsCommon::sdkInterfaces::phone::PhoneCallerInterface>
          phoneCaller,
#endif
      ...
      std::shared_ptr<capabilityAgents::settings::SettingsStorageInterface>
          settingsStorage,
      std::unique_ptr<settings::storage::DeviceSettingStorageInterface>
          deviceSettingStorage,
      std::shared_ptr<capabilityAgents::bluetooth::BluetoothStorageInterface>
          bluetoothStorage,
      ...
      std::shared_ptr<avsCommon::sdkInterfaces::ContextManagerInterface>
          contextManager,
      std::shared_ptr<alexaClientSDK::acl::TransportFactoryInterface>
          transportFactory,
      ...
      std::shared_ptr<
          avsCommon::sdkInterfaces::SoftwareInfoSenderObserverInterface>
  ...
  /**
   * Connects the client to AVS. After this call, users can observe the state of
   * the connection asynchronously by using a connectionObserver that was passed
   * in to the create() function.
   *
   * @param capabilitiesDelegate The component that provides the client with the
   * ability to send messages to the Capabilities API.
   * @param avsEndpoint An optional parameter to the AVS URL to connect to. If
   * empty the "endpoint" value of the "acl" configuration will be used.  If
   * there no such configuration value a default value will be used instead.
   */
  void connect(const std::shared_ptr<
                   avsCommon::sdkInterfaces::CapabilitiesDelegateInterface>&
                   capabilitiesDelegate,
               const std::string& avsEndpoint = "");

  /**
   * Disconnects the client from AVS if it is connected. After the call, users
   * can observer the state of the connection asynchronously by using a
   * connectionObserver that was passed in to the create() function.
   */
  void disconnect();

  /**
   * Stops the foreground activity if there is one. This acts as a "stop" button
   * that can be used to stop an ongoing activity. This call will block until
   * the foreground activity has stopped all user-observable activities.
   */
   ...
   *     @c removeObserver() from @c
   * DialogUXStateObserverInterface::onDialogUXStateChanged() will result in a
   *     deadlock.
   *
  ...
  /**
   * Adds an observer to be notified of @c AudioPlayer state changes. This can
   * be used to be be notified of the @c PlayerActivity of the @c AudioPlayer.
   *
   * @param observer The observer to add.
   */
  ...
  /**
   * Adds an observer to be notified when a TemplateRuntime directive is
   * received.
   *
   * @param observer The observer to add.
   */
  void addTemplateRuntimeObserver(
      std::shared_ptr<
          avsCommon::sdkInterfaces::TemplateRuntimeObserverInterface>
          observer);

  /**
   * Removes an observer to be notified when a TemplateRuntime directive is
   * received.
   *
   * @param observer The observer to remove.
   */
  ...
  /**
   * Notify the TemplateRuntime Capability Agent that the display card is
  ...
  void addSettingObserver(
      ...
      std::shared_ptr<avsCommon::sdkInterfaces::SingleSettingObserverInterface>
          observer);

  /**
   * Removes an observer to a single setting to be notified of that setting
   * change.
   *
   * @param key The name of the setting.
  ...
  void addExternalMediaPlayerObserver(
      std::shared_ptr<avsCommon::sdkInterfaces::externalMediaPlayer::
  ...
  /**
   * Sends the default settings set by the user to the AVS after connection is
   * established.
   */
  ...
  getPlaybackRouter() const;

  /**
   * Adds a SpeakerManagerObserver to be alerted when the volume and mute
   * changes.
   *
  ...
  /**
   * Removes a SpeakerManagerObserver from being alerted when the volume and
   * mute changes.
   *
   * @param observer The observer to be notified upon volume and mute changes.
   */
...
#ifdef ENABLE_REVOKE_AUTH
  /**
   * Adds a RevokeAuthorizationObserver to be alerted when a revoke
   * authorization request occurs.
   *
   * @param observer The observer to be notified of revoke authorization
   * requests.
   */
  ...
  /**
   * Removes a RevokeAuthorizationObserver from being alerted when a revoke
   * authorization request occurs.
   *
   * @param observer The observer to no longer be notified of revoke
   ...
   *
   * @note Be sure to release all references to the returned @c
   * EqualizerController before releasing the last reference to the @c
   * DefaultClient.
   ...
   */
  bool setFirmwareVersion(
      avsCommon::sdkInterfaces::softwareInfo::FirmwareVersion firmwareVersion);

  /**
   * Begins a wake word initiated Alexa interaction.
   *
   * @param wakeWordAudioProvider The audio provider containing the audio data
   * stream along with its metadata.
   ...
   * Alexa.
   * @return A future indicating whether the interaction was successfully
   * started.
   */
  std::future<bool> notifyOfWakeWord(
      capabilityAgents::aip::AudioProvider wakeWordAudioProvider,
      avsCommon::avs::AudioInputStream::Index beginIndex,
      avsCommon::avs::AudioInputStream::Index endIndex, std::string keyword,
      std::chrono::steady_clock::time_point startOfSpeechTimestamp,
      const capabilityAgents::aip::ESPData espData =
          capabilityAgents::aip::ESPData::getEmptyESPData(),
      std::shared_ptr<const std::vector<char>> KWDMetadata = nullptr);

  /**
   * Begins a tap to talk initiated Alexa interaction. Note that this can also
   * be used for wake word engines that don't support providing both a begin and
   * end index.
   *
   * @param tapToTalkAudioProvider The audio provider containing the audio data
   * stream along with its metadata.
   * @param beginIndex An optional parameter indicating where in the stream to
   * start reading from.
   * @param startOfSpeechTimestamp Moment in time when user started talking to
   ...
   *
   * @param holdToTalkAudioProvider The audio provider containing the audio data
   * stream along with its metadata.
   * @param startOfSpeechTimestamp Moment in time when user started talking to
   * Alexa. This parameter is optional and it is used to measure user perceived
   * latency.
  ...
  /**
   * Ends a hold to talk interaction by forcing the client to stop streaming
   * audio data to the cloud and ending any currently ongoing recognize
   * interactions.
   *
   * @return A future indicating whether audio streaming was successfully
   * stopped. This can be false if this was called in the wrong state.
   */
  std::future<bool> notifyOfHoldToTalkEnd();

  /**
   * Ends a tap to talk interaction by forcing the client to stop streaming
   * audio data to the cloud and ending any currently ongoing recognize
   * interactions.
   *
   * @return A future indicating whether audio streaming was successfully
  ...
  /**
   * Retrieves the device settings manager which can be used to access device
   * settings.
   *
 ...
 private:
   ...
   * RegistrationManager and instances of all classes extending
   * CustomDataHandler.
   * @param externalMusicProviderMediaPlayers The map of <PlayerId, mediaPlayer>
   * to use to play content from each external music provider.
   * @param externalMusicProviderSpeakers The map of <players, speaker> to use
   * to track volume of each external music provider media player.
   * @param adapterCreationMap The map of <players, adapterCreationMethod> to
   ...
   * changes.
   * @param equalizerRuntimeSetup Equalizer component runtime setup
   * @param audioFactory The audioFactory is a component the provides unique
   * audio streams.
   * @param authDelegate The component that provides the client with valid LWA
   * authorization.
   * @param alertStorage The storage interface that will be used to store
   ...
   * @param settingsStorage The storage interface that will be used to store
   * settings (DEPRECATED).
   * @param deviceSettingsStorage The storage interface that will be used to
   * store device settings.
   * @param bluetoothStorage The storage interface that will be used to store
   * bluetooth data.
   * @param alexaDialogStateObservers Observers that can be used to be notified
  ...
  std::shared_ptr<capabilityAgents::aip::AudioInputProcessor>
      m_audioInputProcessor;

  /// The speech synthesizer.
  std::shared_ptr<capabilityAgents::speechSynthesizer::SpeechSynthesizer>
      m_speechSynthesizer;
  ...
  std::shared_ptr<capabilityAgents::externalMediaPlayer::ExternalMediaPlayer>
      m_externalMediaPlayer;

  /// The alerts capability agent.
  std::shared_ptr<capabilityAgents::alerts::AlertsCapabilityAgent>
  ...
  std::shared_ptr<capabilityAgents::notifications::NotificationsCapabilityAgent>
      m_notificationsCapabilityAgent;

  /// The user inactivity monitor.
  std::shared_ptr<capabilityAgents::system::UserInactivityMonitor>
      m_userInactivityMonitor;

#ifdef ENABLE_PCC
  /// The phoneCallController capability agent.
  std::shared_ptr<capabilityAgents::phoneCallController::PhoneCallController>
  ...
  /// The playbackRouter.
  std::shared_ptr<capabilityAgents::playbackController::PlaybackRouter>
      m_playbackRouter;

  /// The playbackController capability agent.
  std::shared_ptr<capabilityAgents::playbackController::PlaybackController>
      m_playbackController;

  /// The settings object.
  std::shared_ptr<capabilityAgents::settings::Settings> m_settings;

  /// The speakerManager. Used for controlling the volume and mute settings of
  /// @c SpeakerInterface objects.
  std::shared_ptr<capabilityAgents::speakerManager::SpeakerManager>
      m_speakerManager;

  /// The TemplateRuntime capability agent.
  std::shared_ptr<capabilityAgents::templateRuntime::TemplateRuntime>
      m_templateRuntime;

  /// The MRM capability agent.
  std::shared_ptr<capabilityAgents::mrm::MRMCapabilityAgent>
      m_mrmCapabilityAgent;
  ...
  std::shared_ptr<capabilityAgents::equalizer::EqualizerCapabilityAgent>
      m_equalizerCapabilityAgent;
  ...
  std::shared_ptr<capabilityAgents::system::SoftwareInfoSender>
      m_softwareInfoSender;

#ifdef ENABLE_REVOKE_AUTH
  /// The System.RevokeAuthorizationHandler directive handler.
  std::shared_ptr<capabilityAgents::system::RevokeAuthorizationHandler>
      m_revokeAuthorizationHandler;
#endif
  ...
  /// Settings storage. This storage needs to be closed during default client
  /// destruction.
  std::shared_ptr<
      alexaClientSDK::settings::storage::DeviceSettingStorageInterface>
      m_deviceSettingStorage;
};
