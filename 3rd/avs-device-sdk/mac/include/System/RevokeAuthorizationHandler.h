...
/**
 * This class implements a @c CapabilityAgent that handles the @c
...
class RevokeAuthorizationHandler : public avsCommon::avs::CapabilityAgent {
 public:
  ...
  /**
   * Adds an observer to be notified when the System.RevokeAuthorization
   * Directive has been received.
   *
   * @param observer The observer to be notified when the
   * System.RevokeAuthorization Directive has been received.
   * @return true if the observer was added, false if invalid observer or
   * already an existing observer.
   */
  bool addObserver(
      std::shared_ptr<
   ...
   * when the System.RevokeAuthorization Directive has been received.
   *
   * @param observer The observer that should no longer be notified when the
   * System.RevokeAuthorization Directive has been sent.
   * @return true if the observer was removed, false if invalid observer or
   * wasn't a registered observer.
   */
  bool removeObserver(
      std::shared_ptr<
          avsCommon::sdkInterfaces::RevokeAuthorizationObserverInterface>
