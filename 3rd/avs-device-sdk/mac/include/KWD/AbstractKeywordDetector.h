...
class AbstractKeywordDetector {
 public:
  /**
   * Adds the specified observer to the list of observers to notify of key word
   * detection events.
   *
   * @param keyWordObserver The observer to add.
   */
  void addKeyWordObserver(
      std::shared_ptr<avsCommon::sdkInterfaces::KeyWordObserverInterface>
          keyWordObserver);

  /**
   * Removes the specified observer to the list of observers to notify of key
  ...
  void addKeyWordDetectorStateObserver(
      std::shared_ptr<
          ...
          keyWordDetectorStateObserver);

  /**
   * Removes the specified observer to the list of observers to notify of key
   * word detector state changes.
   *
 ...
 protected:
  ...
  AbstractKeywordDetector(
      std::unordered_set<
          std::shared_ptr<avsCommon::sdkInterfaces::KeyWordObserverInterface>>
          keyWordObservers = std::unordered_set<std::shared_ptr<
              avsCommon::sdkInterfaces::KeyWordObserverInterface>>(),
      std::unordered_set<std::shared_ptr<
          avsCommon::sdkInterfaces::KeyWordDetectorStateObserverInterface>>
          keyWordDetectorStateObservers = std::unordered_set<
              std::shared_ptr<avsCommon::sdkInterfaces::
   ...
   * @param beginIndex The absolute begin index of the first part of the keyword
   * found within the @c stream.
   * @param endIndex The absolute end index of the last part of the keyword
   * within the stream of the @c stream.
  ...
  /**
   * Notifies all keyword detector state observers of state changes in the
   * derived detector.
   *
  ...
  /**
   * Reads from the specified stream into the specified buffer and does the
   * appropriate error checking and observer notifications.
   *
   ...
   * @param[out] errorOccurred Lets caller know if there were any errors that
   * occurred with the read call.
  ...
  ssize_t readFromStream(
      ...
      std::shared_ptr<avsCommon::avs::AudioInputStream> stream, void* buf,
      size_t nWords, std::chrono::milliseconds timeout, bool* errorOccurred);
 ...
 private:
  /**
   * The observers to notify on key word detections. This should be locked with
   * m_keyWordObserversMutex prior to usage.
   */
  std::unordered_set<
      std::shared_ptr<avsCommon::sdkInterfaces::KeyWordObserverInterface>>
      m_keyWordObservers;
  ...
  /**
   * The observers to notify of state changes in the engine. This should be
   * locked with m_keyWordDetectorStateObserversMutex prior to usage.
   */
  std::unordered_set<std::shared_ptr<
      avsCommon::sdkInterfaces::KeyWordDetectorStateObserverInterface>>
      m_keyWordDetectorStateObservers;

  /// Lock to protect m_keyWordObservers when users wish to add or remove
  /// observers
  mutable std::mutex m_keyWordObserversMutex;

  /// Lock to protect m_keyWordDetectorStateObservers when users wish to add or
  /// remove observers
  mutable std::mutex m_keyWordDetectorStateObserversMutex;

  /**
   * The current state of the detector. This is stored so that we don't notify
   * observers of the same change in state multiple times.
   */
  avsCommon::sdkInterfaces::KeyWordDetectorStateObserverInterface::
      KeyWordDetectorState m_detectorState;
};
