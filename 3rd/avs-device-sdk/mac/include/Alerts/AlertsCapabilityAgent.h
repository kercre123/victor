...
class AlertsCapabilityAgent
 ...
 public:
   ...
   * Create function.
   *
   * @param messageSender An interface to which this object will send Events to
   * AVS.
   * @param connectionManager An @c AVSConnectionManagerInterface instance to
   * listen for connection status updates.
   * @param certifiedMessageSender An interface to which this object will send
   * guaranteed Events to AVS.
   * @param focusManager An interface with which this object will request and
   * release Alert channel focus.
   * @param speakerManager An interface to control volume of the Alerts.
   * @param contextManager An interface to which this object will send context
   * updates as alert states change.
   * @param exceptionEncounteredSender An interface which allows
   * ExceptionEncountered Events to be sent to AVS.
   ...
   * @param alertRenderer An alert renderer, which Alerts will use to generate
   * user-perceivable effects when active.
  ...
  static std::shared_ptr<AlertsCapabilityAgent> create(
      ...
      std::shared_ptr<
          avsCommon::sdkInterfaces::audio::AlertsAudioFactoryInterface>
          alertsAudioFactory,
  ...
  void onConnectionStatusChanged(const Status status,
                                 const ChangedReason reason) override;

  void onFocusChanged(avsCommon::avs::FocusState focusState) override;

  void onAlertStateChange(const std::string& token,
  ...
  /// @{
  void onFocusChanged(const std::string& channelName,
                      avsCommon::avs::FocusState newFocus) override;
  /// @}
  ...
  /**
   * A function that allows an application to clear all alerts from storage.
   * This may be useful for a scenario where a user logs out of a device, and
   * another user will log in.  As the first user logs out, their pending alerts
   * should not go off.
   */
  void removeAllAlerts();

  /**
   * This function provides a way for application code to request this object
   * stop any active alert as the result of a user action, such as pressing a
  ...
  /**
   * Refresh timers. This can be called if the system clock time changes to
   * reset the timers so that they go off at the correct time
   */
  void refreshTimers();
 ...
 private:
   ...
   * @param speakerManager An interface to control volume of the Alerts.
   * @param contextManager An interface to which this object will send context
   * updates as stored alerts change.
   * @param exceptionEncounteredSender An interface which allows
   * ExceptionEncountered messages to be sent to AVS.
   * @param alertStorage An interface to store, load, modify and delete Alerts.
   * @param alertsAudioFactory A provider of audio streams specific to Alerts.
   * @param alertRenderer An alert renderer, which Alerts will use to generate
   * user-perceivable effects when active.
   ...
   * single worker thread.  All other public functions in this class can be
   * called asynchronously, and pass data to the @c Executor thread through
   * parameters to lambda functions.  No additional synchronization is needed.
   *
   * The other private functions in this class are called from the execute*
   * functions, and so also don't require additional synchronization.
   */
  /// @{

  /**
   * An implementation of this directive handling function, which may be called
  ...
  /**
   * A handler function which will be called by our internal executor when the
   * connection status changes.
   *
   * @param status The connection status.
   * @param reason The reason the connection status changed.
   */
  void executeOnConnectionStatusChanged(const Status status,
  ...
  /**
   * A handler function which will be called by our internal executor when the
   * FocusManager reports channel focus changes.
   *
   * @param channelName Name of the channel whose focus is changed.
   * @param focusState The current focus of the channel.
   */
  void executeOnFocusManagerFocusChanged(const std::string& channelName,
                                         avsCommon::avs::FocusState focusState);

  /**
   * A handler function which will be called by our internal executor when the
   * SpeakerManager reports volume changes.
   */
  ...
  /**
   * A handler function which will be called by our internal executor when an
   ...
   */
  void executeOnAlertStateChange(const std::string& alertToken,
                                 AlertObserverInterface::State state,
                                 const std::string& reason);

  /**
   * A handler function which will be called by our internal executor to add an
   * alert observer.
   *
   * @param observer The observer to add.
   */
  ...
  /**
   * A handler function which will be called by our internal executor to remove
   * an alert observer.
   *
   * @param observer The observer to remove.
   */
  ...
  /**
   * A handler function which will be called by our internal executor to notify
   * observers of alert changes.
   *
   * @param alertToken The AVS token identifying the alert.
   * @param state The state of the alert.
   * @param reason The reason the the state changed, if applicable.
   */
  void executeNotifyObservers(const std::string& alertToken,
                              AlertObserverInterface::State state,
                              const std::string& reason = "");

  /**
   * A handler function which will be called by our internal executor to remove
   * all alerts currently being managed.
   */
  void executeRemoveAllAlerts();

  /**
   * A handler function which will be called by our internal executor to handle
   * a local stop.
   */
   ...
   * @param[out] alertToken The AVS alert token, required for us to notify AVS
   * whether the SetAlert failed or succeeded.
   * @return Whether the SetAlert processing was successful.
   */
  bool handleSetAlert(
      const std::shared_ptr<avsCommon::avs::AVSDirective>& directive,
      const rapidjson::Document& payload, std::string* alertToken);
   ...
   * @param payload The payload containing the alert data fields.
   * @param[out] alertToken The AVS alert token, required for us to notify AVS
   * whether the DeleteAlert failed or succeeded.
  ...
  /**
   * Utility function to send a single alert related Event to AVS. If
   * isCertified is set to true, then the Event will be guaranteed to be sent to
   * AVS at some point in the future, even if there is no currently active
   * connection.  If it is set to false, and there is no currently active
   * connection, the Event will not be sent.
   *
   * @param eventName The name of the Event to be sent.
   * @param alertToken The token of the Alert being sent to AVS within the
   * Event.
   * @param isCertified Whether the event must be guaranteed to be sent.  See
  ...
  /**
   * Utility function to send a multiple alerts related Event to AVS. If
   * isCertified is set to true, then the Event will be guaranteed to be sent to
   * AVS at some point in the future, even if there is no currently active
   * connection.  If it is set to false, and there is no currently active
   * connection, the Event will not be sent.
   *
   * @param eventName The name of the Event to be sent.
   * @param tokenList The list of Alert tokens being sent to AVS within the
   * Event.
   * @param isCertified Whether the event must be guaranteed to be sent.  See
   * function description for details.
   */
  void sendBulkEvent(const std::string& eventName,
                     const std::list<std::string>& tokenList,
                     bool isCertified = false);

  /**
   * The function sends Alerts.VolumeChanged event when forced or there is a
   * difference between current volume and the volume used to update AVS last
   * time.
   *
  ...
  /**
   * A utility function to update the focus manager with the current state of
   * all alerts.
   */
   ...
   * @return true on success, false otherwise
   */
  bool getSpeakerVolumeSettings(
      avsCommon::sdkInterfaces::SpeakerInterface::SpeakerSettings*
          speakerSettings);

  /**
   * Updates the volume of the next alert to sound. Only next alert is affected,
   * the volume of the currently sounding alert is not changed.
   *
   ...
   *
   * These variables are only accessed by the @c m_executor, with the exception
   * of initialization, and shutdown. The first thing shutdown does is shutdown
   * the @c m_executor, thus making this safe.
   */
  /// @{
  ...
  /// Set of observers to notify when an alert status changes.  @c m_mutex must
  /// be acquired before access.
  ...
  /// Our helper object that takes care of managing alert persistence and
  /// rendering.
  AlertScheduler m_alertScheduler;

  /// @}

  /// This member contains a factory to provide unique audio streams for the
  /// various alerts.
