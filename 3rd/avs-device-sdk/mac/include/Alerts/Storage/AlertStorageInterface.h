...
/**
 * An Interface class which defines APIs for interacting with a general
 * database, as well as for storing, loading, and modifying Alerts.
 */
class AlertStorageInterface {
 public:
   ...
   * The fields which are updated by this operation are the state and scheduled
   * times of the alert.  All other fields of an alert do not change over time,
   * and so will not be captured in the database when calling this function.
   *
  ...
  /**
   * Erases multiple alerts from the database. This function must be atomic, no
   * alert is to be deleted if there was an error deleting one.
   * @note It is not an error if an alert in the parameter list is not present
   * in the database.  This accommodates the race condition between an local
   * alert deletion and a request from AVS.
   *
   * @param alertList The list of alerts to be erased.
   * @return Whether all alerts were successfully erased.
   */
  virtual bool bulkErase(
      const std::list<std::shared_ptr<Alert>>& alertList) = 0;

  /**
   * A utility function to clear the database of all records.  Note that the
   * database will still exist, as will the tables.  Only the rows will be
   * erased.
   *
