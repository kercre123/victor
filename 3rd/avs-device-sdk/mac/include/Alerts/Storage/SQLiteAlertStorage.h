...
/**
 * An implementation that allows us to store alerts using SQLite. This class is
 * not thread safe.
 *
 * TODO: ACSDK-390 Investigate adding an abstraction layer between this class
 * and the AlertStorageInterface, where the middle layer is expressed purely in
 * SQL.
 */
class SQLiteAlertStorage : public AlertStorageInterface {
 public:
  /**
   * Factory method for creating a storage object for Alerts based on an SQLite
   * database.
   *
   * @param configurationRoot The global config object.
   * @param alertsAudioFactory A factory that can produce default alert sounds.
   * @return Pointer to the SQLiteAlertStorage object, nullptr if there's an
   * error creating it.
   */
  ...
  /**
   * An enum class to help debug database contents.  This type is used in the
   * printStats function below.
   */
  ...
  /**
   * A utility function to print the contents of the database to the SDK logger
   * output. This function is provided for debug use only.
   */
 ...
 private:
  ...
  /**
   * Utility function to migrate an existing V1 Alerts database file to the V2
   * format.
   *
   * The expectation for V2 is that a table with the name 'alerts_v2' exists.
   *
   * If this table does not exist, then this function will create it, and the
   * additional tables that V2 expects, and then load all alerts from the V1
   * table and save them into the V2 table.
   *
   * @return Whether the migration was successful.  Returns true by default if
   * the db is already V2.
   */
  bool migrateAlertsDbFromV1ToV2();

  /**
   * A utility function to help us load alerts from different versions of the
   * alerts table.  Currently, versions 1 and 2 are supported.
   *
