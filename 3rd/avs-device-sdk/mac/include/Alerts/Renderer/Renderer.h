...
class Renderer
 ...
 public:
   ...
   *
   * @param mediaPlayer the @c MediaPlayerInterface that the @c Renderer object
   * will interact with.
  ...
  void setObserver(
      std::shared_ptr<RendererObserverInterface> observer) override;

  void start(std::function<std::unique_ptr<std::istream>()> audioFactory,
             const std::vector<std::string>& urls = std::vector<std::string>(),
             int loopCount = 0,
             std::chrono::milliseconds loopPause = std::chrono::milliseconds{
                 0}) override;
 ...
 private:
   ...
   * Constructor.
   *
   * @param mediaPlayer The @c MediaPlayerInterface, which will render audio for
   * an alert.
   */
  Renderer(std::shared_ptr<avsCommon::utils::mediaPlayer::MediaPlayerInterface>
               mediaPlayer);
   ...
   *
   * @param audioFactory A function that produces a stream of audio that is used
   * for the default if nothing else is available.
   ...
   * @param loopPauseInMilliseconds The number of milliseconds to pause between
   * rendering url sequences.
   */
  void executeStart(std::function<std::unique_ptr<std::istream>()> audioFactory,
                    const std::vector<std::string>& urls, int loopCount,
  ...
  void executeOnPlaybackError(
      SourceId sourceId, const avsCommon::utils::mediaPlayer::ErrorType& type,
   ...
   */
  void notifyObserver(RendererObserverInterface::State state,
                      const std::string& message = "");

  /**
   * Utility function to set our internal sourceId to a non-assigned state,
   * which is best encapsulated in a function since it is a bit non-intuitive
   * per MediaPlayer's interface.
   */
  void resetSourceId();

  /**
   * Utility function that checks whether the default audio will be rendered or
   * will be taken from the URL sources
   *
   * @return @c true if default audio will be rendered.  Returns @c false
   * otherwise.
   */
  bool shouldPlayDefault();

  /**
   * Utility function that checks whether the audio source will be played
   * repeatedly by the media player
   *
   * @return @c true if media player will loop the audio source .  Returns @c
   * false otherwise.
   */
  bool shouldMediaPlayerRepeat();

  /**
   * Utility function that checks whether there is an audio source that will be
   * played next
   *
   * @return @c true if there is an audio source to play next.  Returns @c false
   * otherwise.
   */
   ...
   *
   * @return @c true if pause is needed after a plaback.  Returns @c false
   * otherwise.
   */
  bool shouldPause();

  /**
   * Utility function that checks whether the last played source is the last one
   * in the loop
   *
   * @return @c true if  last played source is the last one in the loop. Returns
   * @c false otherwise.
   */
   ...
   * to be played
   *
   * @return @c true if  last played source is the last one to be polayed.
  ...
  /**
   * Utility function to handle the rendering of the next audio asset, with
   * respect to @c m_remainingLoopCount and @c m_nextUrlIndexToRender.  If all
   * urls within a loop have completed, and there are further loops to render,
   * this function will also perform a sleep for the @c m_loopPause duration.
   *
   * @return @c true if there are more audio assets to render, and the next one
   * has been successfully sent to the @c m_mediaPlayer to be played.  Returns
   * @c false otherwise.
   */
  bool renderNextAudioAsset();

  /**
   * Utility function to handle all aspects of an error occurring.  The sourceId
   * is reset, the observer is notified and the observer is reset.
   *
  ...
  /// An optional sequence of urls to be rendered.  If the vector is empty,
  /// m_localAudioFilePath will be rendered instead.
  std::vector<std::string> m_urls;

  /// The number of streams that have been rendered during the processing of the
  /// current loop.
  ...
  /// The number of times @c m_urls should be rendered as specified in the
  /// directive
  ...
  /// A pointer to a stream factory to use as the default audio to use when the
  /// audio assets aren't available.
  std::function<std::unique_ptr<std::istream>()> m_defaultAudioFactory;

  /// A flag to capture if the renderer has been asked to stop by its owner. @c
  /// m_waitMutex must be locked when modifying this variable.
  bool m_isStopping;

  /// The condition variable used to wait for @c stop() or @c m_loopPause
  /// timeouts.
