 ...
 * must be called.
 * @note This class makes use of a global configuration to a database file,
 * meaning that it is best used as a singleton.
 */
class NotificationsCapabilityAgent
 ...
 public:
  ...
  /// @{
  void provideState(const avsCommon::avs::NamespaceAndName& stateProviderName,
  ...
  /// @{
  /**
   * This function sets off logic to handle a Notification having been
   * completely rendered.
   */
 ...
 private:
   ...
   * Sets m_isEnabled to true if there are NotificationIndicators stored in the
   * database. Creates the NotificationIndicator database if it doesn't exist,
   * else opens it.
   */
  bool init();

  /**
   * This method deserializes a @c Directive's payload into a @c
   * rapidjson::Document.
   *
   ...
   */
  bool parseDirectivePayload(std::shared_ptr<DirectiveInfo> info,
   ...
   *
   * These methods (and only these methods) are called by @c m_executor on a
   * single worker thread.  All other methods in this class can be called
   ...
   * This method checks the playAudioIndicator field of the
   * NotificationIndicator and, if it is set, changes the State to PLAYING,
   * updates m_assetId, and calls m_renderer->renderNotification().
   *
   * @param notificationIndicator The notificationIndicator to attempt to
   * render.
   */
  void executeRenderNotification(
      const NotificationIndicator& notificationIndicator);

  /**
   * Consolidates code for getting and setting IndicatorState.
   * This method gets the current IndicatorState and compares it with the
   * persistVisualIndicator field of the next NotificationIndicator. If they
   * differ, IndicatorState is updated and observers are notified.
   *
   * @param nextNotificationIndicator The NotificationIndicator to check the
   * current IndicatorState against.
   */
  void executePossibleIndicatorStateChange(
      const avsCommon::avs::IndicatorState& nextIndicatorState);

  /**
   * Sets the state of the NotificationsCapabilityAgent. All state changes
  ...
  /**
   * Provides relevant state. Called when m_isEnabled or the current
   * IndicatorState changes.
   *
   ...
   */
  void executeProvideState(bool sendToken = false,
  ...
  /**
   * Handles necessary start-up behavior such as the initial observer notify()
   * of the current indicator state and the playing of any remaining
   * NotificationIndicators.
   */
  void executeInit();

  /**
   * Delegates to the correct executePlayFinished<num>Queued() method based on
   * queue size.
   */
   ...
   *
   * These methods deal with different situations that may arise during the
   * lifetime of a NotificationsCapabilityAgent. They should all run on an
   * executor thread and account for any possible State.
   */
  /// @{

  /**
   * Situation: On start-up, the NotificationIndicator queue may not be empty.
   * The next NotificationIndicator should play and the IndicatorState should be
   * appropriately set.
   */
  void executeStartQueueNotEmpty();

  /**
   * Situation: A SetIndicator directive came down. This should be called from
   * handleSetIndicatorDirective().
   *
  ...
  /**
   * Situation: A ClearIndicator directive came down. This should be called from
   * handleClearIndicatorDirective().
   */
  void executeClearIndicator(std::shared_ptr<DirectiveInfo> info);

  /**
   * Situation: The renderer has completed rendering a Notification and the
   * queue is empty.
   */
  ...
  /**
   * Situation: The renderer has completed rendering a Notification and the
   * queue still has one NotificationIndicator in it. Because items are popped
   * from the queue only after processing, this means that the play that just
   * finished corresponds to the item still in the queue. It should be popped
   * with a state transition to IDLE.
   */
  void executePlayFinishedOneQueued();

  /**
   * Situation: The renderer has completed rendering a Notification and the
   * queue has more than one NotificationIndicator left in it.
   */
  ...
  /// Stores notification indicators in the order they are received and the
  ...
  std::shared_ptr<
      avsCommon::sdkInterfaces::audio::NotificationsAudioFactoryInterface>
      m_notificationsAudioFactory;

  /// Holds the current assetId to check against incoming SetIndicator
  /// directives.
  std::string m_currentAssetId;

  /// If this flag is true, there are pending notifications that the user has
  /// not opened. Changes to this member variable should be followed by
  ...
  /// A condition variable to notify doShutdown() that the state change to
