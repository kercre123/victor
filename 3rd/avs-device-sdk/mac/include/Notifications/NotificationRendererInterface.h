...
class NotificationRendererInterface {
 public:
  ...
  /**
   * Add an observer to receive a notifications about rendering notification
   * audio clips.
   *
   * @param observer The observer to call back.
   */
  virtual void addObserver(
      std::shared_ptr<NotificationRendererObserverInterface> observer) = 0;

  /**
   * Remove an observer from the set of observers to receive a notifications
   * about rendering notification audio clips.
   *
   * @param observer The observer to call back.
   */
  ...
  /**
   * Render (play) a notification audio clip.  If the asset at the specified url
   * cannot be played for some reason, the default notification audio clip
   * should be played, instead.  If renderNotification is called while another
   * rendering operation is in progress, this method fails and returns false.
   * @note: Calling this method from a NotificationRendererObserverInterface
   * callback will lead to a deadlock.
   *
   * @param audioFactory A function that produces an audio stream to play if the
   * audio specified by
  ...
  /**
   * Cancel any ongoing rendering of a notification audio clip.  Further render
   * requests will be refused until an observer callback is made to indicate
   * that rendering has finished (i.e. cancellation is complete).
   *
