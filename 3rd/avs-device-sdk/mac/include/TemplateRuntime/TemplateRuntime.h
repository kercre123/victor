...
/**
 * This class implements a @c CapabilityAgent that handles the AVS @c
 * TemplateRuntime API.  The
 * @c TemplateRuntime CA is responsible for handling the directives with the
 * TemplateRuntime namespace.  Due to the fact that the @c RenderPlayerInfo
 * directives are closely related to the @c AudioPlayer, the @c TemplateRuntime
 * CA is an observer to the AudioPlayer and will be synchronizing the @c
 * RenderPlayerInfo directives with the corresponding @c AudioItem being handled
 * in the @c AudioPlayer.
 *
 * The @c TemplateRuntime CA is also an observer to the @c DialogUXState to
 * determine the end of a interaction so that it would know when to clear a @c
 * RenderTemplate displayCard.
 *
 * The clients who are interested in any TemplateRuntime directives can
 * subscribe themselves as an observer, and the clients will be notified via the
...
class TemplateRuntime
 ...
 public:
   ...
   *
   * @param audioPlayerInterface The object to use for subscribing @c
   * TemplateRuntime as an observer of the @c AudioPlayer.
  ...
  static std::shared_ptr<TemplateRuntime> create(
      std::shared_ptr<avsCommon::sdkInterfaces::AudioPlayerInterface>
  ...
  /**
   * This function adds an observer to @c TemplateRuntime so that it will get
   * notified for renderTemplateCard or renderPlayerInfoCard.
   *
  ...
  /**
   * This function removes an observer from @c TemplateRuntime so that it will
   * no longer be notified of renderTemplateCard or renderPlayerInfoCard
   * callbacks.
   *
   * @param observer The @c TemplateRuntimeObserverInterface
   */
  ...
  /**
   * This function notifies the @c TemplateRuntime that a displayCard has been
   * cleared from the screen.  Upon getting this notification, the @c
   * TemplateRuntime will release the visual channel.
   */
 ...
 private:
  ...
  enum class State {
    ...
    /*
     * The @c TemplateRuntime has received a displayCard event is acquiring the
     * visual channel from @c FocusManager.
     */
    ACQUIRING,

    /*
     * The @c TemplateRuntime has focus, either background or foreground, of the
     * channel and has notified its observers of a displayCard. @TemplateRuntime
     * will remain in this state until there is a timeout, clearCard, or
     * focusChanged(NONE) event.
     */
    DISPLAYING,

    /*
     * The @c TemplateRuntime has received a timeout or a clearCard event and is
     * releasing the channel and has notified its observers to clear the
     * display.
     */
    RELEASING,

    /*
     * The @c TemplateRuntime has received a displayCard event during releasing
     * of the channel and is trying to acquire the visual channel again.
     */
    REACQUIRING
  };
  ...
  struct AudioItemPair {
     ...
     * @param renderPlayerInfoDirective The @c RenderPlayerInfo directive that
     * corresponds to the audioItemId.
     */
    AudioItemPair(
        std::string itemId,
        std::shared_ptr<
            alexaClientSDK::avsCommon::avs::CapabilityAgent::DirectiveInfo>
            renderPlayerInfoDirective)
   ...
   *
   * @param audioPlayerInterface The object to use for subscribing @c
   * TemplateRuntime as an observer of AudioPlayer.
   * @param exceptionSender The object to use for sending AVS Exception
   * messages.
   */
  TemplateRuntime(
  ...
  /**
   * This function handles any unknown directives received by @c TemplateRuntime
   * CA.
   *
   * @param info The @c DirectiveInfo containing the @c AVSDirective and the @c
   * DirectiveHandlerResultInterface.
  ...
  /**
   * This is an internal function that handles updating the @c
   * m_audioItemInExecution when the @c AudioPlayer notifies the @c
   * TemplateRuntime CA of any changes in the @c AudioPlayer audio state.  This
   * function is intended to be used in the context of @c m_executor worker
   * thread.
   *
   * @param state The @c PlayerActivity of the @c AudioPlayer.
   * @param context The @c Context of the @c AudioPlayer at the time of the
   * notification.
   */
  void executeAudioPlayerInfoUpdates(avsCommon::avs::PlayerActivity state,
                                     const Context& context);

  /**
   * This is an internal function that start or stop the @c m_clearDisplayTimer
   * based on the @c PlayerActivity reported by the @c AudioPlayer.
   *
   * @param state The @c PlayerActivity of the @c AudioPlayer.
   */
  ...
  /**
   * This function handles the notification of the renderPlayerInfoCard
   * callbacks to all the observers.  This function is intended to be used in
   * the context of @c m_executor worker thread.
   */
  void executeRenderPlayerInfoCallbacks(bool isClearCard);

  /**
   * This function handles the notification of the renderTemplateCard callbacks
   * to all the observers.  This function is intended to be used in the context
   * of @c m_executor worker thread.
   */
  void executeRenderTemplateCallbacks(bool isClearCard);

  /**
   * This is an internal function that is called when the state machine is ready
   * to notify the @TemplateRuntime observers to display a card.
   */
  void executeDisplayCard();

  /**
   * This is an internal function that is called when the state machine is ready
   * to notify the @TemplateRuntime observers to clear a card.
   */
   ...
   */
  /// @{
  /// A set of observers to be notified when a @c RenderTemplate or @c
  /// RenderPlayerInfo directive is received
  std::unordered_set<std::shared_ptr<
      avsCommon::sdkInterfaces::TemplateRuntimeObserverInterface>>
      m_observers;

  /*
   * This is used to store the current executing @c AudioItem based on the
   * callbacks from the AudioPlayerObserverInterface.
   */
  AudioItemPair m_audioItemInExecution;

  /*
   * This queue is for storing the @c RenderPlayerInfo directives when its
   * audioItemId does not match the audioItemId in execution in the @c
   * AudioPlayer.
   */
  std::queue<AudioItemPair> m_audioItems;

  /// This is to store the @c AudioPlayerInfo to be passed to the observers in
  /// the renderPlayerInfoCard callback.
  ...
  /*
   * This is an interface to the @c AudioPlayer.  The @c TemplateRuntime CA used
   * this interface to add and remove itself as an observer to the @c
   * AudioPlayer.  The interface is also used to query the latest offset of the
   * audio playback in the @c AudioPlayer.
   */
  std::shared_ptr<avsCommon::sdkInterfaces::AudioPlayerInterface>
      m_audioPlayerInterface;
  ...
  /// The timeout value in ms for clearing the diplay card when AudioPlay is
  /// FINSIHED
