...
class ExternalMediaPlayer
 ...
 public:
  ...
  using AdapterSpeakerMap = std::unordered_map<
      std::string, std::shared_ptr<avsCommon::sdkInterfaces::SpeakerInterface>>;

  // Signature of functions to create an ExternalMediaAdapter.
  using AdapterCreateFunction = std::shared_ptr<
      avsCommon::sdkInterfaces::externalMediaPlayer::
      ...
      std::shared_ptr<avsCommon::sdkInterfaces::ContextManagerInterface>
          contextManager,
      std::shared_ptr<avsCommon::sdkInterfaces::ExternalMediaPlayerInterface>
          externalMediaPlayer);

  // Map of adapter business names to their creation method.
  using AdapterCreationMap =
      std::unordered_map<std::string, AdapterCreateFunction>;

  /**
   * Creates a new @c ExternalMediaPlayer instance.
   *
   * @param mediaPlayers The map of <PlayerId, MediaPlayer> to be used to find
   * the mediaPlayer to use for this adapter.
   * @param speakers The map of <PlayerId, SpeakerInterface> to be used to find
   * the speaker to use for this adapter.
   * @param adapterCreationMap The map of <PlayerId, AdapterCreateFunction> to
   * be used to create the adapters.
   * @param speakerManager A @c SpeakerManagerInterface to perform volume
   * changes requested by adapters.
   * @param messageSender The object to use for sending events.
   * @param focusManager The object used to manage focus for the adapter managed
   * by the EMP.
  ...
  virtual void onTogglePressed(avsCommon::avs::PlaybackToggle toggle,
   ...
   */
  void addObserver(
      const std::shared_ptr<avsCommon::sdkInterfaces::externalMediaPlayer::
 ...
 private:
  ...
  /**
   * This method returns the ExternalMediaPlayer session state registered in the
   * ExternalMediaPlayer namespace.
   */
  std::string provideSessionState();

  /**
   * This method returns the Playback state registered in the
   * Alexa.PlaybackStateReporter state.
   */
   ...
   * @param focusManager The focus manager to be used by the adapter to
   * acquire/release channel.
   * @param contextManager The context manager of the ExternalMediaPlayer and
   * adapters.
   */
  void createAdapters(
      const AdapterMediaPlayerMap& mediaPlayers,
  ...
  void sendExceptionEncounteredAndReportFailed(
      std::shared_ptr<DirectiveInfo> info, const std::string& message,
  ...
  void executeProvideState(
      ...
      bool sendToken = false, unsigned int stateRequestToken = 0);
  /// @}
   ...
   * @param document The rapidjson document resulting from parsing the directive
   * in directiveInfo.
   * @return A shared-ptr to the ExternalMediaAdapterInterface on which the
   * actual adapter method has to be invoked.
   */
  std::shared_ptr<avsCommon::sdkInterfaces::externalMediaPlayer::
                      ExternalMediaAdapterInterface>
  preprocessDirective(std::shared_ptr<DirectiveInfo> info,
                      rapidjson::Document* document);
   ...
   */
  void notifyObservers(const std::string& playerId,
                       const avsCommon::sdkInterfaces::externalMediaPlayer::
                           ObservableSessionProperties* sessionProperties);
  ...
  void notifyObservers(
      ...
      const avsCommon::sdkInterfaces::externalMediaPlayer::
          ObservablePlaybackStateProperties* playbackProperties);
  ...
  static std::unordered_map<
      ...
      std::pair<avsCommon::sdkInterfaces::externalMediaPlayer::RequestType,
                ExternalMediaPlayer::DirectiveHandler>>
