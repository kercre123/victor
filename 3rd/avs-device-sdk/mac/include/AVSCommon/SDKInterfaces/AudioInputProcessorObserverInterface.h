...
class AudioInputProcessorObserverInterface {
 public:
  ...
  enum class State {
    /// In this state, the @c AudioInputProcessor is not waiting for or
    /// transmitting speech.
    IDLE,

    /// In this state, the @c AudioInputProcessor is waiting for a call to
    /// recognize().
    EXPECTING_SPEECH,
    ...
    /**
     * In this state, the @c AudioInputProcessor has finished streaming and is
     * waiting for completion of an Event. Note that @c recognize() calls are
     * not permitted while in the @c BUSY state.
     */
    BUSY
  };
  ...
  /**
   * This function is called when the state of the observed @c
   * AudioInputProcessor changes.  This function will block processing of audio
   * inputs, so implementations should return quickly.
   *
...
/**
 * Write an @c AudioInputProcessorObserverInterface::State value to an @c
 * ostream as a string.
 *
 * @param stream The stream to write the value to.
 * @param state The @c AudioInputProcessorObserverInterface::State value to
