...
/**
 * The SpeakerInterface is concerned with the control of volume and mute
 * settings of a speaker. The two settings are independent of each other, and
 * the respective APIs shall not affect the other setting in any way. Compound
 * behaviors (such as unmuting when volume is adjusted) will be handled at a
...
class SpeakerInterface {
 public:
   ...
   * The minimum volume level should correspond to no volume output, but this
   * setting should still be independent from mute. For speakers which do not
   * have independent mute and volume settings, the interface must be
   * implemented as if they did so. For example, when unmuting a speaker, the
   * volume setting should correspond to the level it was at before muting.
   */
   ...
   * will be [AVS_SET_VOLUME_MIN, AVS_SET_VOLUME_MAX], and implementers of the
   * interface must normalize the volume to fit the needs of their drivers.
   *
   ...
   * will be [AVS_ADJUST_VOLUME_MIN, AVS_ADJUST_VOLUME_MAX], and implementers of
   * the interface must normalize the volume to fit the needs of their drivers.
   *
   ...
   *
   * @param mute Represents whether the speaker should be muted (true) or
   * unmuted (false).
   * @return Whether the operation was successful.
   */
  ...
  /**
   * Return a @c SpeakerSettings object to indicate the current settings of the
   * speaker.
   *
  ...
  /**
   * Get the @c Type. The @c Type should remain static and not change with the
