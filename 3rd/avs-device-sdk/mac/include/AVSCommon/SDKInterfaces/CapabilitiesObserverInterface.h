...
/**
 * This interface is used to observe changes to the state of the
...
class CapabilitiesObserverInterface {
 public:
  ...
  /// The enum Error encodes possible errors which may occur when changing
  /// state.
  enum class Error {
    ...
    /// The request is missing a required parameter, has an invalid value, or is
    /// otherwise improperly formed.
    BAD_REQUEST
  };
   ...
   *
   * @note Implementations of this method must not call CapabilitiesDelegate
   * methods because the CapabilitiesDelegate may be in a 'locked' state at the
   * time this call is made. If you do, then you may end up with a deadlock.
   *
...
inline std::ostream& operator<<(
    std::ostream& stream, const CapabilitiesObserverInterface::State& state) {
  switch (state) {
 ...
 */
inline std::ostream& operator<<(
    std::ostream& stream, const CapabilitiesObserverInterface::Error& error) {
  switch (error) {
