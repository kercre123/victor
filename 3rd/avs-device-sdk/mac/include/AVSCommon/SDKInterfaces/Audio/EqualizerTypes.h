...
/**
 * Enum representing all the equalizer bands supported by SDK. Customer device
 * may support only a subset of these.
 */
...
/**
 * An iterable array that contains all the values of @c EqualizerBand in order
 * from lower frequencies to higher.
 */
const std::array<EqualizerBand, 3> EqualizerBandValues = {
    {EqualizerBand::BASS, EqualizerBand::MIDRANGE, EqualizerBand::TREBLE}};

/**
 * Enum representing all the equalizer modes supported by SDK. Customer device
 * may support only a subset of these. 'NONE' represents a special mode to
 * reflect custom band values with no named preset.
 */
...
const std::array<EqualizerMode, 6> EqualizerModeValues = {
    {EqualizerMode::NONE, EqualizerMode::MOVIE, EqualizerMode::MUSIC,
     EqualizerMode::NIGHT, EqualizerMode::SPORT, EqualizerMode::TV}};

/// A collection of bands with their level values. This is an alias for @c
/// EqualizerBand to band level (int) map.
using EqualizerBandLevelMap =
    std::unordered_map<EqualizerBand, int, utils::functional::EnumClassHash>;

/**
 * Structure to represent Current state of the equalizer.
 */
struct EqualizerState {
  /// Equalizer mode selected. Use @c EqualizerMode::NONE value to represent no
  /// specific mode.
  EqualizerMode mode;
...
inline bool operator==(const EqualizerState& state1,
 ...
 * @param band The @c EqualizerBand value to write to the @c ostream as a
 * string.
 ...
 *
 * @param stream The stream to write the value to.
 * @param mode The @c EqualizerMode value to write to the @c ostream as a
 * string.
 * @return The @c ostream that was passed in and written to.
 */
 ...
 */
inline utils::error::SuccessResult<EqualizerBand> stringToEqualizerBand(
    const std::string& stringValue) {
  if (stringValue == "BASS") {
    return utils::error::SuccessResult<EqualizerBand>::success(
        EqualizerBand::BASS);
  }
 ...
 *
 * @param stringValue a string containing the value to parse.
 * @param[out] mode Pointer to @c EqualizerMode to receive parsed result. If
 * function fails, the value is not changed.
 * @return true if parsing was successful, false otherwise.
 */
inline utils::error::SuccessResult<EqualizerMode> stringToEqualizerMode(
    const std::string& stringValue) {
  if (stringValue == "NONE") {
    return utils::error::SuccessResult<EqualizerMode>::success(
        EqualizerMode::NONE);
  }
