...
/**
 * A KeyWordObserverInterface is an interface class that clients can extend to
 * listen for keyword detections.
 */
class KeyWordObserverInterface {
 public:
  /// This represents when an index passed into the onKeyWordDetected() call
  /// should be ignored.
  ...
  /**
   * Used to notify the observer of keyword detections. Once called, the client
   * should return as soon as possible. Failure to do so might block the wake
   * word engine from processing audio data, depending on which wake word engine
   * is used. Any additional work that needs to be done should be done on a
   * separate thread or after returning.
   *
   * @param stream The stream in which the keyword was detected.
   * @param keyword The keyword detected.
   * @param beginIndex The optional absolute begin index of the first part of
   * the keyword found within the @c stream. If this is set to
   * UNSPECIFIED_INDEX, then it should be ignored.
   * @param endIndex The optional absolute end index of the last part of the
   * keyword within the stream of the @c stream. If this is set to
   * UNSPECIFIED_INDEX, then it should be ignored.
   * @param KWDMetadata Wake word engine metadata.
   */
  virtual void onKeyWordDetected(
      std::shared_ptr<avs::AudioInputStream> stream, std::string keyword,
