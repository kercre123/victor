...
class TimeUtils {
 public:
  ...
  /**
   * This function converts a string representing time, encoded in the ISO-8601
   * format, to what is commonly known as Unix time (epoch).
   *
   ...
   * @return Whether the conversion was successful.
   */
  bool convert8601TimeStringToUnix(const std::string& timeString,
                                   int64_t* unixTime);

  /**
   * Gets the current time in Unix epoch time, as a 64 bit integer.
   *
   * @param[out] currentTime The current time in Unix epoch time, as a 64 bit
   * integer.
  ...
  /**
   * Convert timepoint to a ISO 8601 RFC 3339 date-time string.  This follows
 ...
 private:
   ...
   *
   * In order to calculate the timezone offset, we call gmtime and localtime
   * giving the same arbitrary time point. Then, we convert them back to time_t
   * and calculate the conversion difference. The arbitrary time point is 24
   * hours past epoch, so we don't have to deal with negative time_t values.
   *
   * This function uses non-threadsafe time functions. Thus, it is important to
   * use the SafeCTimeAccess class.
   *
