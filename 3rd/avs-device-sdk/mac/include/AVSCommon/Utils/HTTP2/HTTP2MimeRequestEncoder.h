...
class HTTP2MimeRequestEncoder : public HTTP2RequestSourceInterface {
 ...
 private:
  ...
  /**
   * Copy a string in to the provided buffer, starting at the offset
   * @m_stringIndex into the buffer, truncating the copy if necessary to not
   * exceed the size of the buffer.  @c m_stringIndex will be updated to the
   * next byte after the copied string, and @c m_bytesCopied will have the count
  ...
  /**
   * Copy a string and a CRLF in to the provided buffer, starting at the offset
   * @m_stringIndex into the buffer, truncating the copy if necessary to not
   * exceed the size of the buffer.  @c m_stringIndex will be updated to the
   * next byte after the copied string and CRLF, and @c m_bytesCopied will have
   * the count of copied bytes added to it.
   *
   * @param bytes The buffer to copy the string to.
   * @param size The size of the buffer to copy the string to.
   * @param text The string to send.
  ...
  /**
   * Create a HTTP2SendDataResult with HTTP2SendStatus::CONTINUE and a size of
   * @c m_bytesCopied.
   *
   * @return An HTTP2SendDataResult with HTTP2SendStatus::CONTINUE and a size of
   * @c m_bytesCopied.
   */
  ...
  /// The boundary string with CRLF nd two-dash prefix to simplify emitting it
