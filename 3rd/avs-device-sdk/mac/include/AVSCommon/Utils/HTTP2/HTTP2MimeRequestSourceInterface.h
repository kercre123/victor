...
/**
 * Interface for providing data to be sent as part of a Mime encoded HTTP2
 * request.
 */
class HTTP2MimeRequestSourceInterface {
 public:
  ...
  /**
   * Get the header lines that should be output with the next mime part.  This
   * will be called once before
   * @c onSendMimePartData() is called for the first mime part and after each
   * call to @c onSendMimePartData() that returns @c HTTP2SendDataResult.status
   * == COMPLETE.
   *
   * @note Calls to this method may block network operations for the associated
   * instance of HTTP2ConnectionInterface, so they should return quickly.
   *
   * @return An @c HTTP2GetMimeHeadersResult specifying the status of the
   * operation and a vector of header lines if the status was CONTINUE.
   */
