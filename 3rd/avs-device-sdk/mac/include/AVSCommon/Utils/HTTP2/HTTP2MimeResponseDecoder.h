...
/**
 * Class that adapts between HTTPResponseSinkInterface and
 * HTTP2MimeResponseSinkInterface providing mime decoding services.
 */
class HTTP2MimeResponseDecoder : public HTTP2ResponseSinkInterface {
 public:
  ...
  HTTP2MimeResponseDecoder(
      std::shared_ptr<HTTP2MimeResponseSinkInterface> sink);
 ...
 private:
   ...
   * **/
  static void partBeginCallback(const MultipartHeaders& headers,
                                void* userData);
  ...
  size_t m_index;
  /// Number of characters left to check at the beginning of the stream for a
  /// leading CRLF sequence.
