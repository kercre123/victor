...
class HTTP2RequestConfig {
 public:
   ...
   */
  HTTP2RequestConfig(HTTP2RequestType type, const std::string& url,
                     const std::string& idPrefix);

  /**
   * Specify the maximum time, in milliseconds, for the connection phase to the
   * server to take.
   *
   * @param timeout The max amount of time, in milliseconds, for the connection
   * phase to the server to take.
   */
   ...
   *
   * @param timeout The max amount of time, in milliseconds, that the request is
   * allowed to take.
   */
  void setTransferTimeout(std::chrono::milliseconds timeout);

  /**
   * Specify the maximum time, in milliseconds, to wait between any read or
   * write operations for this request.
   *
   * @param timeout The max amount of time, in milliseconds, between any read or
   * write operations for this request.
   */
   ...
   *
   * @param The priority of the request. Higher values (max 255) specify higher
   * priority. By default requests are assigned a priority of 16.
  ...
  /**
   * If this request expects that transfer will happen intermittently, set this
   * property. (It is false by default.)
   */
  void setIntermittentTransferExpected();

  /**
   * Set stream identification prefix to use for file names if extended curl
   * logging is enabled. Request ID will be appended to it.
   * @param logicalStreamIdPrefix logical stream Id prefix to use in CURL log
   * file names.
   */
  ...
  /**
   * Get the maximum time, in milliseconds, for the connection phase to the
   * server to take.
   *
   * @return The max amount of time, in milliseconds, for the connection phase
   * to the server to take.
   */
   ...
   *
   * @return The max amount of time, in milliseconds, that the request is
   * allowed to take.
   */
  std::chrono::milliseconds getTransferTimeout() const;

  /**
   * Get the maximum time, in milliseconds, to wait between any read or write
   ...
   *
   * @return The priority of the request. Higher values (max 255) specify higher
   * priority. By default requests are assigned a priority of 16.
   * @see https://tools.ietf.org/html/rfc7540#section-5.3
   */
   ...
   * @return Whether this request expects that transfer will happen
   * intermittently.
   */
 ...
 private:
  ...
  /// Default priority for streams. @see
  /// https://tools.ietf.org/html/rfc7540#section-5.3
  static const uint8_t DEFAULT_PRIORITY = 16;

  /// The max amount of time, in milliseconds, for the connection phase to the
  /// server to take. Value of std::chrono::milliseconds::zero() means "not set"
  std::chrono::milliseconds m_connectionTimeout;

  /// The max amount of time, in milliseconds, that the request is allowed to
  /// take. Value of std::chrono::milliseconds::zero() means "not set"
  std::chrono::milliseconds m_transferTimeout;

  /// The max amount of time, in milliseconds, between any read or write
  /// operations for this request. Value of std::chrono::milliseconds::zero()
  /// means "not set"
  std::chrono::milliseconds m_activityTimeout;

  /// The priority of the request. Higher values (max 255) specify higher
  /// priority. By default requests are assigned a priority of 16.
  /// @see https://tools.ietf.org/html/rfc7540#section-5.3
  uint8_t m_priority;
  ...
  /// If true, the transfer thread may be put to sleep even when this request
  /// isn't paused.
...
inline HTTP2RequestConfig::HTTP2RequestConfig(HTTP2RequestType type,
...
inline void HTTP2RequestConfig::setConnectionTimeout(
    std::chrono::milliseconds timeout) {
  m_connectionTimeout = timeout;
}

inline void HTTP2RequestConfig::setTransferTimeout(
    std::chrono::milliseconds timeout) {
  m_transferTimeout = timeout;
}

inline void HTTP2RequestConfig::setActivityTimeout(
    std::chrono::milliseconds timeout) {
...
inline void HTTP2RequestConfig::setRequestSource(
    std::shared_ptr<HTTP2RequestSourceInterface> source) {
  m_source = std::move(source);
}

inline void HTTP2RequestConfig::setResponseSink(
    std::shared_ptr<HTTP2ResponseSinkInterface> sink) {
...
inline std::chrono::milliseconds HTTP2RequestConfig::getConnectionTimeout()
    const {
  return m_connectionTimeout;
}

inline std::chrono::milliseconds HTTP2RequestConfig::getTransferTimeout()
    const {
  return m_transferTimeout;
}

inline std::chrono::milliseconds HTTP2RequestConfig::getActivityTimeout()
    const {
...
inline std::shared_ptr<HTTP2RequestSourceInterface>
HTTP2RequestConfig::getSource() const {
  return m_source;
}

inline std::shared_ptr<HTTP2ResponseSinkInterface> HTTP2RequestConfig::getSink()
    const {
  return m_sink;
}
