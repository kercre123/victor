...
class LibcurlHTTP2Request
    : public alexaClientSDK::avsCommon::utils::http2::HTTP2RequestInterface {
 public:
  /**
   * Constructor.
   * @param config
  ...
  /**
   * If the response code has been received and not previously reported, notify
   * the HTTP2ResponseSinkInterface that the response code has been received.
   */
 ...
 private:
   ...
   * @param userData Context passed back from @c libcurl.  Should always be a
   * pointer to @c LibcurlHTTP2Request.
   * @return @c size if the operation was successful and @c size bytes were
   * received, @c CURL_WRITEFUNC_PAUSE if the bytes could not be processed
   * during the call and @c libcurl should call back later to try again, or some
   * other value (e.g. 0), to abort the request.
   */
   ...
   * @param userData Context passed back from @c libcurl.  Should always be a
   * pointer to @c LibcurlHTTP2Request.
   * @return @c size if the operation was successful, or any other value to
   * abort the request.
   */
   ...
   * @param userData  Context passed back from @c libcurl.  Should always be a
   * pointer to @c LibcurlHTTP2Request.
   * @return Zero to indicate the end of the data to send, 1 to size * nmemb to
   * indicate the number of bytes read in to @c data, or CURL_READFUNC_PAUSE to
   * indicate that no data was read but @c libcurl should call back later to try
   * again.
   */
  static size_t readCallback(char* data, size_t size, size_t nmemb,
                             void* userData);

  /**
   * Returns the HTTP response code to this request.
   *
   * @returns The HTTP response code if one has been received, 0 if not, and < 0
  ...
  /// Max time the stream may make no progress before @c hasProgressTimedOut()
  /// returns true.
