...
class LibcurlHTTP2Connection
    : public avsCommon::utils::http2::HTTP2ConnectionInterface {
 public:
  /**
   * Create an @c LibcurlHTTP2Connection.
   *
   * @return The new @c LibcurlHTTP2Connection or nullptr if the operation
   * fails.
   */
  ...
  createAndSendRequest(const http2::HTTP2RequestConfig& config) override;
 ...
 private:
  ...
  /** Set up the multi-handle.  Re-assign a new multi-handle, if m_multi is
   * already non-null.
   *
  ...
  /**
   * Main network loop. Repeatedly call curl_multi_perform in order to transfer
   * data on the incorporated streams.
   */
  void networkLoop();
  ...
  /**
   * Checks if any active streams have finished and reports the response code
   * and completion status for them.
   */
  void cleanupFinishedStreams();

  /** Checks for streams that have been cancelled, removes them, and reports
   * CANCELLED completion status. Checks for streams that have not progressed
   * within their timeout, removes them, and reports the response code (if any)
   * and completion status (TIMEOUT).
   */
  void cleanupCancelledAndStalledStreams();

  /**
   * Determine whether all non-intermittent streams are paused.  An intermittent
   * stream would be a persistent downchannel stream, for instace.
   ...
   *
   * @return next request, or nullPtr if queue is empty or the network loop is
   * stopping.
   */
  ...
  /// Represents a CURL multi handle.  Intended to only be accessed by the
  /// network loop thread.
  std::unique_ptr<avsCommon::utils::libcurlUtils::CurlMultiHandleWrapper>
      m_multi;

  /// Serializes concurrent access to the m_requestQueue and m_isStopping
  /// members.
  std::mutex m_mutex;

  /// Used to notify the network loop thread that there is at least one request
  /// queued or that the loop has been instructed to stop.
  std::condition_variable m_cv;

  /// The list of streams that either do not have HTTP response headers, or have
  /// outstanding response data. Only accessed from the network loop thread.
