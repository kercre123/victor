...
class LogEntry {
 public:
   ...
   * @return This instance to facilitate adding more information to this log
   * entry.
   */
  ...
  /**
   * Add data (hence the name 'd') in the form of a @c key, @c value pair to the
   * metadata of this log entry.
   *
   * @param key The key identifying the value to add to this LogEntry.
   * @param value The value to add to this LogEntry.
   * @return This instance to facilitate adding more information to this log
  ...
  /**
   * Add sensitive data in the form of a @c key, @c value pair to the metadata
   * of this log entry. Because the data is 'sensitive' it will only be emitted
   * in DEBUG builds.
   *
   * @param key The key identifying the value to add to this LogEntry.
   * @param value The value to add to this LogEntry.
  ...
  /**
   * Add an arbitrary message to the end of the text of this LogEntry.  Once
   * this has been called no other additions should be made to this LogEntry.
   *
   ...
   *
   * @return The rendered text of this LogEntry.  The returned buffer is only
   * guaranteed to be valid for the lifetime of this LogEntry, and only as long
   * as no further modifications are made to it.
   */
  const char* c_str() const;

 private:
  /// Add the appropriate prefix for a key,value pair that is about to be
  /// appended to the text of this LogEntry.
  void prefixKeyValuePair();

  /// Add the appropriate prefix for an arbitrary message that is about to be
  /// appended to the text of this LogEntry.
  void prefixMessage();
   ...
   * ...so we need to reserve ',', '=' and ':'.  We escape those vales with '\'
   * so we escape '\' as well.
   *
  ...
  /// Flag indicating (if true) that some metadata has already been appended to
  /// this LogEntry.
  bool m_hasMetadata;
...
// Define ACSDK_EMIT_SENSITIVE_LOGS if you want to include sensitive data in log
// output.
