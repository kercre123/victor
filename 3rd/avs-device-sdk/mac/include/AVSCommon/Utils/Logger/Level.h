 ...
 */
enum class Level {
  /// Most verbose debug log level. Compiled out when ACSDK_DEBUG_LOG_ENABLED is
  /// not defined.
  DEBUG9,

  /// Intermediate debug log level. Compiled out when ACSDK_DEBUG_LOG_ENABLED is
  /// not defined.
  DEBUG8,
  ...
  /// Least verbose debug log level. Compiled out when ACSDK_DEBUG_LOG_ENABLED
  /// is not defined.
  DEBUG0,
 ...
 * @return Returns the name of the Level. If the level is not recognized,
 * returns "UNKNOWN".
 */
std::string convertLevelToName(Level level);

/**
 * Get a character corresponding to a Level value. The characters returned are
 * unique per log level and are intended to be used to minimize the space taken
 * up by the level specifier in log lines.
 * @param level The Level for which to get the corresponding character.
 * @return The character corresponding to the level. If the @c level is not
 * recognized, returns 'U'.
 */
 ...
 * @return The @c Level corresponding to the specified name. If the @c name is
 * not recognized, returns @c Level::UNKNOWN.
 */
