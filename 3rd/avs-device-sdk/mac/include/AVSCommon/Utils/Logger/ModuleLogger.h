...
/**
 * @c Logger implementation providing per module configuration. Forwards logs to
 * another @c Logger.
 */
class ModuleLogger : public Logger,
 ...
 public:
   ...
   * Constructor.
   *
   * @param configKey The name of the root configuration key to inspect for a
   * "logLevel" string value. That string is used to specify the lowest log
   * severity level that this @c ModuleLogger should emit.
   */
  ...
  void emit(Level level, std::chrono::system_clock::time_point time,
            const char* threadId, const char* text) override;
 ...
 private:
  ...
  /**
   * Combine @c m_moduleLogLevel and @c m_sinkLogLevel to determine the
   * appropriate value for m_logLevel.
   */
  void updateLogLevel();
