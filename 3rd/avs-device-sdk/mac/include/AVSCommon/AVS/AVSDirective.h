...
class AVSDirective : public AVSMessage {
 public:
  /**
   * An enum to indicate the status of parsing an AVS Directive from a JSON
   ...
   * @param attachmentContextId The contextId required to get attachments from
   * the AttachmentManager.
   * @return A pair of an AVSDirective pointer and a parse status.  If the
   * AVSDirective is nullptr, the status will express the parse error.
   */
   ...
   * @param readerPolicy The policy with which to create the @c
   * AttachmentReader.
   * @return An attachment reader or @c nullptr if no attachment was found with
   * the given @c contentId.
   */
  std::unique_ptr<avsCommon::avs::attachment::AttachmentReader>
 ...
 private:
  ...
  std::shared_ptr<avsCommon::avs::attachment::AttachmentManagerInterface>
      m_attachmentManager;
  /// The contextId needed to acquire the right attachment from the
  /// attachmentManager.
 ...
 */
inline std::string avsDirectiveParseStatusToString(
    AVSDirective::ParseStatus status) {
  switch (status) {
 ...
 * @param status The @c ParseStatus value to write to the @c ostream as a
 * string.
