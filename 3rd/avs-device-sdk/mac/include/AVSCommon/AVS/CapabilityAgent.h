...
/**
 * @c CapabilityAgent implements methods which most capability agents will need,
 * namely:
 * @li @c DirectiveHandlerInterface,
 * @li Building the JSON event string given the name, payload and context,
 * @li A map of the message Id to @c AVSDirective and @c
 * DirectiveResultInterface. Derived capability agents may extend this class.
 * They may have to implement the following interfaces:
 ...
 * @li @c ContextRequesterInterface: To request context from the @c
 * ContextManager, as necessary.
 */
class CapabilityAgent : public sdkInterfaces::DirectiveHandlerInterface,
 ...
 public:
   ...
   *
   * The following four functions implement the @c DirectiveHandlerInterface.
   * Only the message Id is passed to the
   * @c handleDirective and @c cancelDirective functions, so we need to maintain
   * a mapping of the message Id to the
   * @c AVSDirective and @c DirectiveHandlerResultInterface, i.e., @c
   * DirectiveAndResultInterface. The
   * @c DirectiveHandlerInterface functions call the functions of the same name
   * with the
  ...
  void onContextFailure(
      const sdkInterfaces::ContextRequestError error) override;

  // <ANKI> (These used to be protected. Now they're public to get directive
  // info for debugging)
 ...
 protected:
   ...
   * @param exceptionEncounteredSender Object to use to send
   * ExceptionEncountered messages.
   */
  CapabilityAgent(
      const std::string& nameSpace,
      std::shared_ptr<sdkInterfaces::ExceptionEncounteredSenderInterface>
          exceptionEncounteredSender);

  /**
   * CapabilityAgent maintains a map from messageId to instances of
   * DirectiveInfo so that CapabilityAgents can track the processing of an @c
   * AVSDirective.
   */
  class DirectiveInfo {
   public:
     ...
     * Constructor.
     *
     * @param directiveIn The @c AVSDirective with which to populate this
     * DirectiveInfo.
     * @param resultIn The @c DirectiveHandlerResultInterface instance with
     * which to populate this DirectiveInfo.
     */
    DirectiveInfo(
  ...
  /**
   * Create a DirectiveInfo instance with which to track the handling of an @c
   * AVSDirective.
   * @note This method is virtual to allow derived CapabilityAgent's to extend
   * DirectiveInfo with additional information.
   *
   * @param directive The @c AVSDirective to be processed.
   * @param result The object with which to communicate the outcome of
   * processing the @c AVSDirective.
   * @return A DirectiveInfo instance with which to track the processing of @c
   * directive.
   */
   ...
   * DirectiveHandler a chance to prepare for handling of an @c AVSDirective. If
   * an error occurs during the pre-Handling phase and that error should cancel
   * the handling of subsequent
   * @c AVSDirectives with the same @c DialogRequestId, the @c DirectiveHandler
   * should call the @c setFailed method on the @c result instance passed in to
   * this call.
   *
   ...
   *
   * @param info The @c DirectiveInfo instance for the @c AVSDirective to
   * process.
   */
  // <ANKI> (made public)
  // virtual void preHandleDirective(std::shared_ptr<DirectiveInfo> info) = 0;
  ...
  /**
   * Handle the action specified by the @c AVSDirective in @c info. The handling
   * of subsequent directives with the same @c DialogRequestId may be blocked
   * until the @c DirectiveHandler calls the @c setSucceeded() method of the @c
   * DirectiveHandlingResult present in @c info. If handling of this directive
   * fails @c setFailed() should be called to indicate a failure.
   *
   * @note The implementation of this method MUST be thread-safe.
   * @note The implementation of this method MUST return quickly. Failure to do
   * so blocks the processing of subsequent @c AVSDirectives.
   ...
   * for the @c AVSDirective in
   * @info. Once this has been called the @c CapabilityAgent should not expect
   * to receive further calls regarding this directive.
   *
   * @note The implementation of this method MUST be thread-safe.
   * @note The implementation of this method MUST return quickly. Failure to do
   * so blocks the processing of subsequent @c AVSDirectives.
  ...
  /**
   * This function releases resources associated with the @c AVSDirective which
   * is no longer in use by a
   * @c CapabilityAgent.
   *
   * @note This function should be called from @c handleDirective() and @c
   * cancelDirective() implementations after the outcome of @c handleDirective()
   * or @c cancelDirective() has been reported.
   *
   ...
   * optional @c context. The header includes the namespace, name, message Id
   * and an optional @c dialogRequestId. The message Id required for the header
   * is a random string that is generated and added to the header.
   *
   * @param eventName The name of the event to be include in the header.
   * @param dialogRequestIdString The value associated with the
   * "dialogRequestId" key.
 ...
 private:
  ...
  std::unordered_map<std::string, std::shared_ptr<DirectiveInfo>>
      m_directiveInfoMap;
