...
class AbstractAVSConnectionManager
    : public sdkInterfaces::AVSConnectionManagerInterface {
 public:
  /// Type alias for brevity.
  using ConnectionStatusObserverInterface =
  ...
  AbstractAVSConnectionManager(
      std::unordered_set<std::shared_ptr<ConnectionStatusObserverInterface>>
          observers = std::unordered_set<
              std::shared_ptr<ConnectionStatusObserverInterface>>());
  ...
  void addConnectionStatusObserver(
      std::shared_ptr<ConnectionStatusObserverInterface> observer) override;
 ...
 protected:
  ...
  /**
   * Utility function to notify all observers of the current Connection Status
   * and Reason.
   */
  ...
  /// The reason we changed to the current connection status.  @c m_mutex must
  /// be acquired before access.
  ConnectionStatusObserverInterface::ChangedReason m_connectionChangedReason;

  /// Set of observers to notify when the connection status changes.  @c m_mutex
