...
class AttachmentReader {
 public:
  /**
   * An enum class to communicate the possible states following a @c read()
   * call.
   */
  enum class ReadStatus {
    /// Things are ok.
    OK,
    /// On a request for n bytes, less than n bytes were available on a
    /// non-blocking read.
    OK_WOULDBLOCK,
    /// On a request for n bytes, less than n bytes were available on a blocking
    /// read.
    OK_TIMEDOUT,
    /// The writer has overwritten the new data on reader's current position.
    /// Reader position is reset
    /// to current writer position.
    OK_OVERRUN_RESET,
    /// The underlying data representation is no longer readable.
    CLOSED,
    /// The writer has corrupted the reader data.  The attachment is no longer
    /// valid.
    ERROR_OVERRUN,
    /// The number of bytes in the request is smaller than the word-size of the
  ...
  /// An enum class to indicate when the @c read() function should stop
  /// returning data after a call to @c close().
  enum class ClosePoint {
    /// Stop returning data immediately after the @c close() call.
    IMMEDIATELY,
    /// Stop returning data when all of the data in the buffer at the time @c
    /// close() was called has been read.
   ...
   * @param[out] readStatus The out-parameter where the resulting state of the
   * read will be expressed.
   * @param timeoutMs The timeout for this read call in milliseconds.  This
   * value is only used for the @c BLOCKING reader policy.  If this parameter is
   * zero, there is no timeout and blocking reads will wait forever.
   * @return The number of bytes read as a result of this call.
   */
  virtual std::size_t read(
      void* buf, std::size_t numBytes, ReadStatus* readStatus,
   ...
   * @return @c true if the specified position points at unexpired data, or @c
   * false otherwise. Note that it is valid to seek into a future index that has
  ...
  /**
   * The close function.  An implementation will take care of any resource
   * management when a reader no longer needs to use an attachment.
   *
   * @param closePoint The point at which the reader should stop reading from
   * the attachment.
   */
  virtual void close(
      ClosePoint closePoint = ClosePoint::AFTER_DRAINING_CURRENT_BUFFER) = 0;
};
