...
/**
 * A macro to be used in operator << overload function to help with the
 * translation of @c Button to string.
 *
...
/**
 * A macro to be used in operator << overload function to help with the
 * translation of @c Toggle to string.
 *
...
namespace std {
/**
 * @ std::hash() specialization defined to allow @c PlaybackButton to be used as
 * a key in @c std::unordered_map.
 */
template <>
struct hash<alexaClientSDK::avsCommon::avs::PlaybackButton> {
  inline size_t operator()(
      const alexaClientSDK::avsCommon::avs::PlaybackButton& button) const {
    return std::hash<std::string>{}(PlaybackButtonToString(button));
  }
};
/**
 * @ std::hash() specialization defined to allow @c PlaybackToggle to be used as
 * a key in @c std::unordered_map.
 */
template <>
struct hash<alexaClientSDK::avsCommon::avs::PlaybackToggle> {
  inline size_t operator()(
      const alexaClientSDK::avsCommon::avs::PlaybackToggle& toggle) const {
