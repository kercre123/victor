...
class AudioPlayer
 ...
 public:
  ...
  void onTags(SourceId id,
              std::unique_ptr<const VectorOfTags> vectorOfTags) override;
  /// @}
 ...
 private:
   ...
   * @param exceptionSender The object to use for sending AVS Exception
   * messages.
   * @param playbackRouter The playback router used for switching playback
   * buttons handler to default.
   * @return A @c std::shared_ptr to the new @c AudioPlayer instance.
   */
  AudioPlayer(
      std::shared_ptr<avsCommon::utils::mediaPlayer::MediaPlayerInterface>
  ...
  /**
   * This function provides updated context information for @c AudioPlayer to @c
   * ContextManager.  This function is called when @c ContextManager calls @c
   * provideState(), and is also called internally by @c changeActivity().
   *
   ...
   *
   * @li If focus changes to @c FOREGROUND after a @c Play directive requested
   * focus, @c AudioPlayer will start playing.
   * @li If focus changes to @c BACKGROUND while playing (when another component
   * acquires focus on a higher-priority channel), @c AudioPlayer will pause
   * playback until it regains @c FOREGROUND focus.
   * @li If focus changes to @c FOREGROUND while paused, @c AudioPlayer will
   * resume playing.
  ...
  /**
   * Call an @c m_progressTimer method, keeping track of how many calls to
   * m_progressTimer are in progress.
   *
   ...
   *
   * @param Whether Whether @c m_progressTimer is between @c start() and @c
   * stop().
   */
  ...
  void executeOnTags(SourceId id,
  ...
  /**
   * This function stops playback of the current song, and optionally starts the
   * next queued song.
   *
   * @param startNextSong Indicates whether to start playing the next song after
  ...
  /**
   * This function changes the @c AudioPlayer state.  All state changes are made
  ...
  /**
   * Most of the @c AudioPlayer events use the same payload, and only vary in
   * their event name.  This utility function constructs and sends these generic
   * @c AudioPlayer events.
   *
   * @param eventName The name of the event to send.
   * @param offset The offset to send.  If this parameter is left with its
   * default (invalid) value, the current offset from MediaPlayer will be sent.
   */
  void sendEventWithTokenAndOffset(
      const std::string& eventName,
      std::chrono::milliseconds offset =
          avsCommon::utils::mediaPlayer::MEDIA_PLAYER_INVALID_OFFSET);
   ...
   *
   * @note @c MediaPlayer has a getOffset function which only works while
   * actively playing, but AudioPlayer needs to be able to report its offset at
   * any time, even when paused or stopped.  To address the gap, this function
   *     reports the live offset from @c MediaPlayer when it is playing, and
   * reports a cached offset when
  ...
  /// The @c PlaybackRouterInterface instance to use when @c AudioPlayer becomes
  /// active.
  std::shared_ptr<avsCommon::sdkInterfaces::PlaybackRouterInterface>
      m_playbackRouter;
   ...
   *
   * This variable is primarily an Executor Thread Variable (see below) in that
   * it is always written from the executor thread and can be safely read
   * without synchronization from the executor thread.  However, this is not
   * listed as an Executor Thread Variable with the others below because there
   * is one non-executor function which reads from this variable: @c
   * onFocusChanged().
   *
   * Focus change notifications are required to block until the focus change
   * completes, so @c onFocusChanged() blocks waiting for a state change.  This
   * is a read-only operation from outside the executor thread, so it doesn't
   * break thread-safety for reads inside the executor, but it does require that
   * these reads from outside the executor lock
   * @c m_currentActivityMutex, and that writes from inside the executor lock @c
   * m_currentActivityMutex and notify
   * @c m_currentActivityConditionVariable..
   */
  avsCommon::avs::PlayerActivity m_currentActivity;

  /// Protects writes to @c m_currentActivity and waiting on @c
  /// m_currentActivityConditionVariable.
  ...
  /// The initial offset for the currently (or most recent) playing @c
  /// AudioItem.
  ...
  /// When in the @c BUFFER_UNDERRUN state, this records the time at which the
  /// state was entered.
  ...
  /**
   * This keeps track of the current offset in the audio stream.  Reading the
   * offset from @c MediaPlayer is insufficient because @c MediaPlayer only
   * returns a valid offset when it is actively playing, but @c AudioPlayer must
   * return a valid offset when @c MediaPlayer is stopped.
   */
  std::chrono::milliseconds m_offset;

  /// A set of observers to be notified when there's a change in the audio
  /// state.
  std::unordered_set<
      std::shared_ptr<avsCommon::sdkInterfaces::AudioPlayerObserverInterface>>
      m_observers;
  ...
  /**
   * A flag which is set when calling @c MediaPlayerInterface::stop(), and used
   * by @c onPlaybackStopped() to decide whether to continue on to the next
   * queued item.
   */
  bool m_playNextItemAfterStopped;

  /**
   * A flag which is set when calling @c MediaPlayerInterface::stop(), and
   * cleared in @c executeOnPlaybackStopped(). This flag is used to tell if the
   * @c AudioPlayer is in the process of stopping playback.
   */
  bool m_isStopCalled;
