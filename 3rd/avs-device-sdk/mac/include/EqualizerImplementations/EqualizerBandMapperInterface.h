...
class EqualizerBandMapperInterface {
 public:
  ...
  /**
   * Maps AVS band levels to the target equalizer bands. Number of target bands
   * and mapping method are implementation specific. All supported bands should
   * be defined. It is up to the implementation to decide what levels to use for
   * missing bands.
   *
   * @param bandLevelMap Map of AVS bands and their levels.
   * @param setBandCallback Callback receiving the target band index and the
   * level to be applied. It is up to the user to set up implementation properly
   * to be able to map index to the band. No particular order of indices is
   * guaranteed. The signature of the callback is void callback(int bandIndex,
   * int level);
   */
