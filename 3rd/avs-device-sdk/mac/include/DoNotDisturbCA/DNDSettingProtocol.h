...
/**
 * Implement the logic of DoNotDisturb protocol where AVS is the source of
 * truth. This implementation corresponds to Alexa.DoNotDisturb v1.0 AVS
 * interface.
 */
class DNDSettingProtocol : public settings::SettingProtocolInterface {
 public:
   ...
   *
   * @param metadata The setting metadata used to generate a unique database
   * key.
   * @param eventSender Object used to send events to avs in order to report
   * changes to the device.
   * @param settingStorage The setting storage object.
   * @return A pointer to the new @c DNDSettingProtocol object if it succeeds;
   * @c nullptr otherwise.
   */
  ...
  bool avsChange(ApplyChangeFunction applyChange,
                 RevertChangeFunction revertChange,
                 SettingNotificationFunction notifyObservers) override;

  bool restoreValue(ApplyDbChangeFunction applyChange,
                    SettingNotificationFunction notifyObservers) override;
  ///@}
 ...
 private:
  ...
  /// Object used to send events to avs in order to report changes to the
  /// device.
