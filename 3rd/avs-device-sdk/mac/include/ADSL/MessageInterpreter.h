...
/**
 * Class that converts @c acl::Message to @c AVSDirective, and passes those
 * directives to a
...
class MessageInterpreter
 ...
 public:
   ...
   * Constructor.
   *
   * @param exceptionEncounteredSender The exceptions encountered messages
   * sender, which will allow us to send exception encountered back to AVS.
   * @param directiveSequencerInterface The DirectiveSequencerInterface
   * implementation, which will receive
   *        @c AVSDirectives.
   * @param attachmentManager The @c AttachmentManager which created @c
   * AVSDirectives will use to acquire Attachments.
   */
  MessageInterpreter(
      std::shared_ptr<
