 ...
 * lower level MRM functionality, such as device / platform, local network, time
 * synchronization, and audio playback.  The api provided here is minimal and
...
class MRMHandlerInterface : public avsCommon::utils::RequiresShutdown {
 public:
  ...
  /**
   * Function to handle if a speaker setting has changed.  MRM only needs to
   * know the type of the speaker.
   *
   * @param type The type of the speaker which has changed.
   */
  virtual void onSpeakerSettingsChanged(
      const avsCommon::sdkInterfaces::SpeakerInterface::Type& type) = 0;

  /**
   * Function to be called when a System.UserInactivityReportSent Event has been
   * sent to AVS.
   */
