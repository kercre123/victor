...
/**
 * Handle the HTTP2 request and response that establishes and services the
 * downchannel of an connection to AVS.
 */
class DownchannelHandler
 ...
 private:
   ...
   * @param authToken The token to use to authorize the request.
   */
  DownchannelHandler(std::shared_ptr<ExchangeHandlerContextInterface> context,
                     const std::string& authToken);

  /// @name HTTP2RequestSourceInterface methods
  /// @{
  std::vector<std::string> getRequestHeaderLines() override;
  avsCommon::utils::http2::HTTP2SendDataResult onSendData(char* bytes,
