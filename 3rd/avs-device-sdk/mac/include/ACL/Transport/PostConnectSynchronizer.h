...
class PostConnectSynchronizer
 ...
 public:
   ...
   *
   * @param The @c ContextManager which will provide the context for the @c
   * SynchronizeState event.
   * @return a shared pointer to PostConnectSynchronizer if all given parameters
   * are valid, otherwise return nullptr.
   */
 ...
 private:
  /**
   * Enum specifying the various states in a @c PostConnectSychronizer's
   * lifecycle.
   */
  enum class State {
    ...
    /// @c mainLoop() is running, no @c GetContext() request or message send is
    /// in progress.
    RUNNING,
   ...
   * for the @c SynchronizeState event.
   */
  PostConnectSynchronizer(
      std::shared_ptr<avsCommon::sdkInterfaces::ContextManagerInterface>
          contextManager);
  ...
  /**
   * Loop to fetch the context and send a post-connect message to AVS - or retry
   * if either fails.
   */
  void mainLoop();

  /**
   * Stop mainLoop().  This method blocks until mainLoop() exits or the
   * operation fails.
   *
  ...
  /// The context manager that provides application context in the post connect
  /// loop.
  std::shared_ptr<avsCommon::sdkInterfaces::ContextManagerInterface>
      m_contextManager;
