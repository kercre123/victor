 ...
 */
class HTTP2Transport : public std::enable_shared_from_this<HTTP2Transport>,
 ...
 public:
   ...
   * @param http2Connection Instance of HTTP2ConnectionInterface with which to
   * perform HTTP2 operations.
   * @param messageConsumer The MessageConsumerInterface to pass messages to.
   * @param attachmentManager The attachment manager that manages the
   * attachments.
   * @param transportObserver The observer of the new instance of
   * TransportInterface.
   * @param postConnectFactory The object used to create @c PostConnectInterface
   * instances.
   * @param configuration An optional configuration to specify HTTP2/2
   * connection settings.
  ...
  /**
   * Get the HTTP2ConnectionInterface instance being used by this
   * HTTP2Transport.
   *
   * @return The HTTP2ConnectionInterface instance being used by this
   * HTTP2Transport.
   */
  std::shared_ptr<avsCommon::utils::http2::HTTP2ConnectionInterface>
  getHTTP2Connection();
  ...
  /// @{
  void sendPostConnectMessage(
      std::shared_ptr<avsCommon::avs::MessageRequest> request) override;
  /// @}
  ...
  std::shared_ptr<avsCommon::utils::http2::HTTP2RequestInterface>
 ...
 private:
  ...
  enum class State {
    ...
    /// Performing operations that require a connection, but which must be done
    ...
    /// Tearing down the connection.  Possibly waiting for some streams to
    /// complete.
    DISCONNECTING,
   ...
   * @param transportObserver The observer of the new instance of
   * TransportInterface.
   * @param postConnect The object used to create PostConnectInterface
   * instances.
   ...
   * @param beforeConnected Whether or not to only allow enqueuing of messages
   * before connected.
   */
  void enqueueRequest(std::shared_ptr<avsCommon::avs::MessageRequest> request,
                      bool beforeConnected);

  /**
   * Handle sending @c MessageRequests and pings while in @c
   * State::POST_CONNECTING or @c State::CONNECTED.
   *
   * @param whileState Continue sending @c MessageRequests and pings while in
   * this state.
   * @return The current value of @c m_state.
   */
  ...
  /**
   * Notify observers that this @c HTTP2Transport has established a connection
   * with AVS.
   */
   ...
   */
  void notifyObserversOnDisconnect(
      avsCommon::sdkInterfaces::ConnectionStatusObserverInterface::ChangedReason
          reason);

  /**
   * Notify observers that this @c HTTP2Transport's connection was terminated by
   * AVS.
   */
  ...
  /// Observers of this class, to be notified on changes in connection and
  ...
  /// PostConnect object is used to perform activities required once a
