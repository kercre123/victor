...
/**
 * This an abstract base class which specifies the interface to manage an actual
 * connection over some medium to AVS.
 *
 * Implementations of this class are required to be thread-safe.
 */
class MessageRouter : public MessageRouterInterface,
 ...
 public:
  /**
   * Constructor.
   * @param authDelegate An implementation of an AuthDelegate, which will
   * provide valid access tokens with which the MessageRouter can authorize the
   * client to AVS.
   * @param attachmentManager The AttachmentManager, which allows ACL to write
   * attachments received from AVS.
   * @param transportFactory Factory used to create new transport objects.
   * @param avsEndpoint The endpoint to connect to AVS.  If empty the "endpoint"
   * value of the "acl" configuration will be used.  If there no such
   * configuration value a default value will be used instead.
   */
  ...
  void setObserver(
      std::shared_ptr<MessageRouterObserverInterface> observer) override;
  ...
  void onServerSideDisconnect(
      std::shared_ptr<TransportInterface> transport) override;

  void consumeMessage(const std::string& contextId,
 ...
 private:
   ...
   * Architectural note:
   *  @li A derived class cannot access the required observer method directly
   * due a friend relationship at the base class level. However this method
   * bridges the gap, and allows the observer's public interface to remain
   *    unchanged.
   *
   * @param status The current status of the connection.
   * @param reason The reason the connection status changed.
   ...
   */
  void notifyObserverOnReceive(const std::string& contextId,
                               const std::string& message);

  /**
   * Creates a new transport, and begins the connection process. The new
   * transport immediately becomes the active transport. @c m_connectionMutex
   * must be locked to call this method.
   */
  void createActiveTransportLocked();

  /**
   * Disconnects all transports. @c m_connectionMutex must be locked to call
   * this method.
   *
   * @param reason The reason the last transport was disconnected
   * @param lock Reference to the @c unique_lock that must be held when this
   * method is called. The lock may be released during the execution of this
   * method, but will be locked when this method exits.
   */
  ...
  /**
   * Reset m_activeTransport. First check if m_activeTransport is in
   * m_transports.  If not, issue a warning (because it should be) and queue the
   * safe release of our reference to the transport.
   * @c m_connectionMutex must be locked to call this method.
   */
  void safelyResetActiveTransportLocked();

  /**
   * Hold a reference to a transport until its shutdown() method (executed on
   * our executor) returns.
   *
  ...
  /// This mutex guards access to all connection related state, specifically the
  /// status and all transport interaction.
  std::mutex m_connectionMutex;

  /// The current connection status. Access serialized with @c
  /// m_connectionMutex.
  avsCommon::sdkInterfaces::ConnectionStatusObserverInterface::Status
      m_connectionStatus;

  /// The current connection reason. Access serialized with @c
  /// m_connectionMutex.
  avsCommon::sdkInterfaces::ConnectionStatusObserverInterface::ChangedReason
      m_connectionReason;

  /**
   * When the MessageRouter is enabled, any disconnect should automatically
   * trigger a reconnect with AVS. Access serialized with @c m_connectionMutex.
   */
  bool m_isEnabled;

  /// A vector of all transports which are not disconnected. Access serialized
  /// with @c m_connectionMutex.
  std::vector<std::shared_ptr<TransportInterface>> m_transports;

  /// The current active transport to send messages on. Access serialized with
  /// @c m_connectionMutex.
