...
/**
 * A Channel represents a focusable layer with a priority, allowing the observer
 * which has acquired the Channel to understand focus changes.
 */
class Channel {
 public:
  /*
   * This class contains the states of the @c Channel.  The states inside this
   * structure are intended to be shared via the @c ActivityTrackerInterface.
   */
  struct State {
    ...
    /*
     * The channel's name.  Although the name is not dynamic, it is useful for
     * identifying which channel the state belongs to.
     */
    std::string name;
  ...
  /**
   * Updates the focus and notifies the Channel's observer, if there is one, of
   * the focus change. This method does not return until the
   * ChannelObserverInterface##onFocusChanged() callback to the observer
   * returns. If the focus
  ...
  /**
   * Compares this Channel and another Channel and checks which is higher
   * priority. A Channel is considered higher priority than another Channel if
   * its m_priority is lower than the other Channel's.
   *
   ...
   * @return @c true if the observer currently owns the Channel and @c false
   * otherwise.
   */
  bool doesObserverOwnChannel(
      std::shared_ptr<avsCommon::sdkInterfaces::ChannelObserverInterface>
          observer) const;
