...
/**
 * The @c AudioActivityTracker implements the @c ActivityTrackerInterface and
 * gets notification from the @c FocusManager any activities in the audio
 * channels.  It also implements the @c StateProviderInterface and will provide
...
class AudioActivityTracker
 ...
 public:
   ...
   * context for events.
   * @return A @c std::shared_ptr to the new @c AudioActivityTracker instance,
   * or @c nullptr if the operation failed.
   */
  ...
  /// @{
  void notifyOfActivityUpdates(
      const std::vector<Channel::State>& channelStates) override;
  /// @}
 ...
 private:
  ...
  /**
   * This function processes the vector of @c Channel::State and stores them
   * inside @c m_channelsContext.
   *
   * @param channelStates The vector of @c Channel::State that has been updated
   * as notified by the FocusManager via the @c notifyOfActivityUpdates()
   * callback.
   */
  void executeNotifyOfActivityUpdates(
      const std::vector<Channel::State>& channelStates);

  /**
   * This function provides updated context information for AudioActivityTracker
   * to @c ContextManager.  This function is called when @c ContextManager calls
   * @c provideState().
   *
   * @param stateRequestToken The token @c ContextManager passed to the @c
   * provideState() call, which will be passed along to the
   * ContextManager::setState() call.
   */
   ...
   */
  const std::string& executeChannelNameInLowerCase(
   ...
   */
  /// @{
  /// A map to store the @c Channel::State to all the audio channels.  The key
  /// of this map is the name of the @c Channel.
  std::unordered_map<std::string, Channel::State> m_channelStates;

  /// A map to store the channel name in lower cases as required by the
  /// AudioActivity context.  The key of this map is the name of the @c Channel.
