...
/**
 * This playlist parser implements an iterative depth first search algorithm to
 * find audio urls inside a playlist url. Each call to next will perform the
 * search until it hits a leaf (a valid media). When the playlist is empty, it
 * will set the parse result to SUCCESS.
 *
 * This class is not thread safe except for method @c abort(). This method can
 * be called while another thread is running and it will cause ongoing and
 * future calls to @c next to fail. Calls to @c initializeParsing will reset
 * abort state.
 */
class IterativePlaylistParser : public avsCommon::utils::playlistParser::
                                    IterativePlaylistParserInterface {
 public:
  /**
   * Creates a new @c IterativePlaylistParser instance.
   *
   * @param contentFetcherFactory A factory that can create @c
   * HTTPContentFetcherInterfaces.
   * @return An @c std::unique_ptr to a new @c IterativePlaylistParser if
   * successful or @c nullptr otherwise.
   */
 ...
 private:
   ...
   * Constructor.
   *
   * @param contentFetcherFactory The object that will be used to create objects
   * with which to fetch content from urls.
   */
  ...
  bool getPlaylistContent(
      std::unique_ptr<avsCommon::sdkInterfaces::HTTPContentFetcherInterface>
          contentFetcher,
      std::string* content);
  ...
  /// A queue with items to either a playlist url to parse or media info to
  /// play. This object is used to save the traversal state.
