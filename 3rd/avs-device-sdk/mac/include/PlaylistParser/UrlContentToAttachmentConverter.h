...
/// Class that handles the streaming of urls containing media into @c
/// Attachments.
class UrlContentToAttachmentConverter
    : public avsCommon::utils::playlistParser::PlaylistParserObserverInterface,
      public avsCommon::utils::RequiresShutdown {
 public:
  /// Class to observe errors that arise from converting a URL to to an @c
  /// Attachment
  ...
  /**
   * Creates a converter object. Note that calling this function will commence
   * the parsing and streaming of the URL into the internal attachment. If a
   * desired start time is specified, this function will attempt to start
   * streaming at that offset, based on available metadata if the URL points to
   * a playlist file. If no such information is available, streaming will begin
   * from the beginning. It is up to the caller of this function to make a call
   * to
   * @c getStartStreamingPoint() to find out the actual offset from which
   * streaming began.
   *
   * @param contentFetcherFactory Used to create @c HTTPContentFetchers.
   * @param url The URL to stream from.
   * @param observer An observer to be notified of any errors that may happen
   * during streaming.
   * @param startTime The desired time to attempt to start streaming from. Note
   * that this will only succeed in cases where the URL points to a playlist
   * with metadata about individual chunks within it. If none are found,
   * streaming will begin from the beginning.
   * @return A @c std::shared_ptr to the new @c UrlContentToAttachmentConverter
   * object or @c nullptr on failure.
   *
   * @note This object is intended to be used once. Subsequent calls to @c
   * convertPlaylistToAttachment() will fail.
   */
  static std::shared_ptr<UrlContentToAttachmentConverter> create(
      ...
      const std::string& url, std::shared_ptr<ErrorObserverInterface> observer,
   ...
   *
   * @return A @c std::shared_ptr of an @c InProcessAttachment or @c nullptr on
   * failure.
   */
 ...
 private:
   ...
   * @param observer An observer to be notified of any errors that may happen
   * during streaming.
   * @param desiredStartTime The desired time to attempt to start streaming
   * from. Note that this will only succeed in cases where the URL points to a
   * playlist with metadata about individual chunks within it. If none are
   * found, streaming will begin from the beginning.
   */
  ...
  void onPlaylistEntryParsed(
      int requestId,
      avsCommon::utils::playlistParser::PlaylistEntry playlistEntry) override;
  ...
  /**
   * Downloads the content from the url, decrypts (if required) and writes it
   ...
   * @return @c true if the content was successfully streamed and written or @c
   * false otherwise.
   */
   ...
   * @return @c true if the content was successfully downloaded or @c false
   * otherwise.
   */
  bool download(const std::string& url, const std::vector<std::string>& headers,
   ...
   * @param url The URL to download.
   * @param headers HTTP headers to pass to server.
   * @param[out] content Sets the content of the pointer if download is
   * successful.
   * @return @c true if the content was successfully downloaded or @c false
   * otherwise.
   */
   ...
   */
  bool shouldDecrypt(const avsCommon::utils::playlistParser::EncryptionInfo&
   ...
   *
   * These member variables are only accessed by @c onPlaylistEntryParsed
   * callback functions that is called in a single thread in PlaylistParser or
   * after the @c m_playlistParser is shutdown, and do not require any
   * synchronization.
   */
  /// @{
  ...
  /// A counter to indicate the current running total of durations found in
  /// playlist entries.
