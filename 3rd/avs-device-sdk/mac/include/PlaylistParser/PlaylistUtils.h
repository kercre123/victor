...
/**
 * Parses an PLS playlist and returns the "children" URLs in the order they
 * appeared in the playlist.
 *
 * @param content The content to parse.
 * @return A vector of URLs in the order they appeared in the playlist.
 */
std::vector<std::string> parsePLSContent(const std::string& playlistURL,
                                         const std::string& content);

/**
 * Removes a carriage return from the end of a line. This is required to handle
 ...
 * @note This function should be used to retrieve content specifically from
 * playlist URLs. Attempting to use this on a media URL could be blocking
 * forever as the URL might point to a live stream.
 */
bool extractPlaylistContent(
    std::unique_ptr<avsCommon::utils::HTTPContent> httpContent,
    std::string* content);

/**
 * Determines whether the provided url is an absolute url as opposed to a
 * relative url. This is done by simply checking to see if the string contains
 * the substring "://".
 *
...
/**
 * Creates an absolute url, given a base url and a relative path from that url.
 * For example, if "www.awesomewebsite.com/music/test.m3u" was the base url and
 * the relative path was "music.mp3", "www.awesomewebsite.com/music/music.mp3"
 * would be returned.
 *
 ...
 */
bool getAbsoluteURLFromRelativePathToURL(std::string baseURL,
