...
/**
 * This class implementes a @c CapabilityAgent that handles the AVS @c Speaker
 * API.
 *
 * The @c SpeakerManager can handle multiple @c SpeakerInterface objects. @c
 * SpeakerInterface are grouped by their respective types, and the volume and
 * mute state will be consistent across each type. For example, to change the
 * volume of all speakers of a specific type:
 *
 * @code{.cpp}
 *     // Use local setVolume API.
 *     std::future<bool> result = speakerManager->setVolume(type,
 * AVS_SET_VOLUME_MAX);
 ...
 * @endcode
 *
 * Clients may extend the @c SpeakerInterface::Type enum if multiple independent
 * volume controls are needed.
 */
class SpeakerManager
 ...
 public:
  /**
   * Create an instance of @c SpeakerManager, and register the @c
   * SpeakerInterfaces that will be controlled by it. SpeakerInterfaces will be
   * grouped by @c SpeakerInterface::Type.
   *
   ...
   * @param exceptionEncounteredSender An @c ExceptionEncounteredSenderInterface
   * to send directive processing exceptions to AVS.
   */
  static std::shared_ptr<SpeakerManager> create(
      const std::vector<std::shared_ptr<
          avsCommon::sdkInterfaces::SpeakerInterface>>& speakers,
  ...
  std::future<bool> setVolume(
      avsCommon::sdkInterfaces::SpeakerInterface::Type type, int8_t volume,
      bool forceNoNotifications = false) override;
  std::future<bool> adjustVolume(
      avsCommon::sdkInterfaces::SpeakerInterface::Type type, int8_t delta,
      bool forceNoNotifications = false) override;
  std::future<bool> setMute(
      avsCommon::sdkInterfaces::SpeakerInterface::Type type, bool mute,
      bool forceNoNotifications = false) override;
  ...
  void removeSpeakerManagerObserver(
      std::shared_ptr<avsCommon::sdkInterfaces::SpeakerManagerObserverInterface>
          observer) override;
  void addSpeaker(std::shared_ptr<avsCommon::sdkInterfaces::SpeakerInterface>
                      speaker) override;
  /// @}
 ...
 private:
  ...
  SpeakerManager(
      const std::vector<std::shared_ptr<
          avsCommon::sdkInterfaces::SpeakerInterface>>& speakerInterfaces,
      std::shared_ptr<avsCommon::sdkInterfaces::ContextManagerInterface>
          contextManager,
      std::shared_ptr<avsCommon::sdkInterfaces::MessageSenderInterface>
          messageSender,
   ...
   */
  bool parseDirectivePayload(std::string payload,
  ...
  /**
   * Sends <Volume/Mute>Changed events to AVS. The events are identical except
   * for the name.
   *
  ...
  /**
   * Internal function to set the volume for a specific @c Type. This runs on a
   * worker thread. Upon success, a VolumeChanged event will be sent to AVS.
   *
   ...
   * @param forceNoNotifications This flag will ensure no event is sent and the
  ...
  /**
   * Internal function to adjust the volume for a specific @c Type. This runs on
   * a worker thread. Upon success, a VolumeChanged event will be sent to AVS.
   *
   * @param type The type of speaker to modify volume for.
  ...
  /**
   * Internal function to set the mute for a specific @c Type. This runs on a
   * worker thread. Upon success, a MuteChanged event will be sent to AVS.
   *
  ...
  /**
   * Internal function to send events and notify observers when settings have
   * changed. This runs on a worker thread.
   *
  ...
  void executeNotifySettingsChanged(
      const avsCommon::sdkInterfaces::SpeakerInterface::SpeakerSettings&
          settings,
      const std::string& eventName,
      const avsCommon::sdkInterfaces::SpeakerManagerObserverInterface::Source&
          source,
      const avsCommon::sdkInterfaces::SpeakerInterface::Type& type);

  /**
   * Internal function to notify the observer when a @c SpeakerSettings change
   * has occurred.
   *
  ...
  /**
   * Validates that all speakers with the given @c Type have the same @c
   * SpeakerSettings.
   *
  ...
  std::multimap<avsCommon::sdkInterfaces::SpeakerInterface::Type,
                std::shared_ptr<avsCommon::sdkInterfaces::SpeakerInterface>>
      m_speakerMap;

  /// The observers to be notified whenever any of the @c SpeakerSetting
  /// changing APIs are called.
