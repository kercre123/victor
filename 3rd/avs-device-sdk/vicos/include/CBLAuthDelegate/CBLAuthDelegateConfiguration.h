...
class CBLAuthDelegateConfiguration {
 public:
   ...
   *
   * @param configuration The ConfigurationNode containing the configuration
   * parameters for the new instance.
   * @param deviceInfo The deviceInfo instance.
   * @return the CBLAuthDelegateConfiguration instance if successful, else a
   * nullptr.
   */
  ...
  /**
   * Get the duration before access token expiration to start refreshing the
   * token.
   *
   * @return The duration before access token expiration to start refreshing the
   * token.
   */
  std::chrono::seconds getAccessTokenRefreshHeadStart() const;

  /**
   * Get the locale to use when prompting the user to authenticate the client to
   * access AVS.
   *
   * @return The locale to use when prompting the user to authenticate the
   * client to access AVS.
   */
  ...
  /**
   * Get (as text) the 'scope_data' JSON object to send to @c LWA when
   * requesting a code pair.
   *
   * @return The 'scope_data' JSON object to send to @c LWA when requesting a
   * code pair.
   */
 ...
 private:
   ...
   *
   * @param root The root level node of the raw configuration from which to
  ...
  /// How far ahead of auth token expiration to start making requests to refresh
  /// the auth token.
