...
/**
 * The interface to persistent storage needed by a CBLAuthDelegate (Code Based
 * Linking AuthDelegate).
 */
class CBLAuthDelegateStorageInterface {
 public:
   ...
   * If the file specified already exists, or if a database is already being
   * handled by this object, then this function returns false.
   *
   * @return @c true If the database is created ok, or @c false if either the
   * file exists or a database is already being handled by this object.
   */
  virtual bool createDatabase() = 0;

  /**
   * Open a database with the given filepath.  If this object is already
   * managing an open database, or the file does not exist, or there is a
   * problem opening the database, this function returns false.
   *
   * @return @c true If the database is opened ok, @c false if either the file
   * does not exist, if this object is already managing an open database, or if
  ...
  /**
   * A utility function to clear all records from storage.  Note that the
   * storage will still exist, it will just have not content.
   *
