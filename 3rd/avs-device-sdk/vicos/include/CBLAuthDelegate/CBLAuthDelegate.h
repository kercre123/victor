...
/**
 * CBLAuthDelegate provides an implementation of the @c AuthDelegateInterface
 ...
 */
class CBLAuthDelegate : public avsCommon::sdkInterfaces::AuthDelegateInterface,
 ...
 public:
   ...
   * @param customerDataManager The CustomerDataManager instance this instance
   * should register with.
   * @param storage The object used to persist the new CBLAuthDelegate's state.
   * @param authRequester Observer used to tell the user to browse to a URL and
   * enter a 'user code'.
   * @param httpPost Instance that implements HttpPostInterface. If nullptr, a
  ...
  static std::unique_ptr<CBLAuthDelegate> create(
      ...
      std::shared_ptr<avsCommon::utils::libcurlUtils::HttpPostInterface>
          httpPost = nullptr,
 ...
 private:
  /**
   * The Code-Based Linking authorization process has several stages.  This enum
   * is used to track which stage of the process this @c CBLAuthDelegate is in.
   */
  enum class FlowState {
    /// Initialization: if there is an existing refresh token, transition to @c
    /// REFRESHING_TOKEN, if not
    /// transition to @c REQUESTING_CODE_PAIR.
    STARTING,
    /// No valid refresh token, restart the authorization process by requesting
    /// a code pair from @c LWA,
    /// retrying if required. Once a valid code pair is acquired, ask the user
    /// to authorize by browsing
    /// to a verification URL (supplied by LWA with the code pair) and entering
    /// the user_code from the
    /// code pair.  Then transition to @c REQUESTING_TOKEN.
    REQUESTING_CODE_PAIR,
    /// Have received a code pair from @c LWA and are waiting for the user to
    /// authenticate and enter
    /// the user code. Wait for the user by polling @c LWA for an access token
    /// using the device code
    /// and user code. Waiting stops when an access token is received or the
    /// code pair expires. If an
    /// access token is acquired, transition to @c REFRESHING_TOKEN. If the code
    /// pair expires before
    /// an access token is acquired, transition back to @c REQUESTING_CODE_PAIR.
    REQUESTING_TOKEN,
    /// Have a refresh token, and may have a valid access token. Periodically
    /// refresh (or acquire) an
    /// access token so that if possible, a valid access token is always
    /// available. If the refresh
    /// token becomes invalid, transition back to the @c REQUESTING_CODE_PAIR
    /// state.
    REFRESHING_TOKEN,
    /// Either a shutdown has been triggered or an unrecoverable error has been
    /// encountered. Stop
   ...
   * should register with.
   * @param storage The object used to persist the new CBLAuthDelegate's state.
   * @param authRequester Observer used to tell the user to authenticate the
   * client for access to AVS by browsing to a URL (supplied by @c LWA) and
   * enter a 'user code' (also provided by @c LWA).
   * @param httpPost Instance that implements HttpPostInterface. Must not be @c
   * nullptr. The behavior for passing in @c nullptr is undefined.
   */
  CBLAuthDelegate(
      std::shared_ptr<registrationManager::CustomerDataManager>
          customerDataManager,
  ...
  /**
   * init() is used by create() to perform initialization after construction but
   * before returning the AuthDelegate instance so that clients only get access
   * to fully formed instances.
   *
   * @param configuration The ConfigurationNode containing the configuration
   * parameters for the new instance.
   * @param deviceInfo The deviceInfo instance.
   ...
   */
  avsCommon::sdkInterfaces::AuthObserverInterface::Error
   ...
   *
   * @param response The response to handle.
   * @param expiresImmediately Whether or not any access token received should
   * expire immediately. This is used to force immediate refresh of the initial
   * access token (acquired by @c handleRequestingToken()) to validate the
   * refresh token in combination with the client ID to avoid the confusing user
   * experience that results if the access token fail to refresh (typically an
   * hour) after initial authoriation.
   * @return The resulting status.
   */
  ...
  /// Point in time when the last received pair of @c device_code, @c user_code
  /// will expire.
  ...
  /// True if an authorization failure was reported for the current value of
  /// m_accessToken.
