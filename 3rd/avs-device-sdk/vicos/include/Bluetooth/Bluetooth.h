...
/**
 * The Bluetooth Capability Agent is responsible for implementing the Bluetooth
 * AVS interface. This consists of two areas of responsibilities:
 *
 * -# The connectivity of devices. This includes scanning, pairing and
 * connecting.
 ...
 *
 * The Bluetooth agent will handle directives from AVS and requests from peer
 * devices. Examples include pairing and connection requests, as we as media
 * playback requests. Some examples of this are:
 *
 * - "Alexa, connect".
 ...
 * - "Alexa next".
 *
 * Connectivity is defined as when two devices have paired and established
 * connections of all applicable services (A2DP, AVRCP, etc). Alexa does not
 * support multiple connected multimedia devices. If a device is currently
 * connected, attempting to connect a second device should force a disconnect on
 * the currently connected device.
 *
 * At this time, the agent does not enforce the disconnect of the currently
 * connected device. It is theoretically possible to connect two devices
 * simultaneously, but the behavior is undefined. It is advised to disconnect a
 * currently connected device before connecting a new one. Enforcement of this
 * will be available in an upcoming release.
 *
 * Interfaces in AVSCommon/SDKInterfaces/Bluetooth can be implemented for
 * customers who wish to use their own Bluetooth stack. The Bluetooth agent
 * operates based on events. Please refer to the BluetoothEvents.h file for a
 * list of events that must be sent.
 *
 * Supported Profiles
 *
 * Profiles listed under here refer to the Capability Agent's support of these
 * profiles in relation to AVS. This does not speak about support for them at
 * other layers (the stack, client applications, etc).
 *
...
class Bluetooth
 ...
 public:
   ...
   * @param exceptionEncounteredSender Responsible for sending exceptions to
   * AVS.
  ...
  static std::shared_ptr<Bluetooth> create(
      ...
      std::unique_ptr<
          avsCommon::sdkInterfaces::bluetooth::BluetoothDeviceManagerInterface>
          deviceManager,
 ...
 protected:
  ...
  /// @{
  void onEventFired(
      const avsCommon::utils::bluetooth::BluetoothEvent& event) override;
  /// @}
 ...
 private:
  ...
  // TODO ACSDK-1392: Optimize by updating the context only when there is a
  // delta.
  ...
  void sendExceptionEncountered(
      ...
      const std::string& message, avsCommon::avs::ExceptionErrorType type);
  ...
  /**
   * Connect with the device matching the given uuid. This will connect all
   * available services between the two devices.
   *
   * @param uuid The uuid associated with the device.
   */
  ...
  /**
   * Connect with the most recently connected device that supports the given
   * profile. The profile is only a selector; this will connect all available
   * services between the two devices. The version information is not used
   * currently.
   *
   * @param profileName The profile name to select devices by.
   * @param profileVersion The profile version to select devices by.
   */
  void executeConnectByProfile(const std::string& profileName,
                               const std::string& profileVersion);

  /**
   * Disconnect with the device matching the given uuid. This will disconnect
   * all available services between the two devices.
   *
   * @param uuid The uuid associated with the device.
   */
  ...
  /**
   * Helper function to abstract shared logic in
   * pairing/unpairing/connecting/disconnecting operations.
   *
   * @param device The device to operate on.
   * @param function The function to call (one of
  ...
  /**
   * Drain the command queue of @c AVRCPCommands. We use a queue so we can
   * process the commands after the Bluetooth agent has entered the foreground.
   */
  ...
  /**
   * This handles the details of sending the incoming A2DP stream into
   * MediaPlayer. A callback is set up to copy incoming buffers into an @c
   * AttachmentReader which can be consumed by the MediaPlayer.
   *
   * @param stream The incoming A2DP stream.
   */
  void setCurrentStream(
      std::shared_ptr<avsCommon::utils::bluetooth::FormattedAudioStreamAdapter>
          stream);
  ...
  void onFormattedAudioStreamAdapterData(
      avsCommon::utils::AudioFormat audioFormat, const unsigned char* buffer,
  ...
  retrieveDeviceByMac(const std::string& mac);
   ...
   */
  std::shared_ptr<avsCommon::sdkInterfaces::bluetooth::BluetoothDeviceInterface>
  retrieveDeviceByUuid(const std::string& uuid);

  /**
   * Retrieve the UUID by its MAC address. If no UUID is found, then one will be
   * generated and inserted.
   *
   * @param mac The MAC address of the associated UUID.
   * @param[out] uuid The UUID.
   *
   * @return Whether an UUID is successfully obtained either by retrieval or
   * generation.
   */
  bool retrieveUuid(const std::string& mac, std::string* uuid);

  /// Clears the databse of mac,uuid that are not known by the
  /// @BluetoothDeviceManager.
  void clearUnusedUuids();

  /**
   * Most events require the context, this method queues the event and requests
   * the context. Once the context is available in onContextAvailable, the event
   * will be dequeued and sent.
   *
   * @param eventName The name of the event.
   * @param eventPayload The payload of the event.
   */
  void executeQueueEventAndRequestContext(const std::string& eventName,
                                          const std::string& eventPayload);

  /**
   * Sends an event to alert AVS that the list of found and paired devices has
   * changed.
   *
  ...
  /// Sends a scanDevicesFailed event to alert AVS that attempting to scan for
  /// devices failed.
  void executeSendScanDevicesFailed();

  /// Sends an event to indicate the adapter successfully entered discoverable
  /// mode.
  ...
  /**
   * Sends an event to indicate that connecting with a device by profile
   * succeeded.
   *
   * @param device The device.
  ...
  void executeSendConnectByProfileSucceeded(
      std::shared_ptr<
          avsCommon::sdkInterfaces::bluetooth::BluetoothDeviceInterface>
          device,
      const std::string& profileName, avsCommon::avs::Requester requester);
   ...
   * @param requester Whether this was initiated by the CLOUD or DEVICE.
   */
  void executeSendConnectByProfileFailed(const std::string& profileName,
                                         avsCommon::avs::Requester requester);
  ...
  /// Sends an event to indicate we successfully sent an AVRCP play to the
  /// target.
  ...
  /// Sends an event to indicate we successfully sent an AVRCP pause to the
  /// target.
  ...
  /// Sends an event to indicate we successfully sent an AVRCP next to the
  /// target.
  ...
  /// Sends an event to indicate we successfully sent an AVRCP previous to the
  /// target.
  void executeSendMediaControlPreviousSucceeded();

  /// Sends an event to indicate we failed to send an AVRCP previous to the
  /// target.
  ...
  // TODO 1396: Combine @c m_streamingState and @c m_focusState into a single
  // state machine if possible.
  /**
   * The current @c StreamingState of the device. This represents the internal
   * media streaming state of the Bluetooth agent in relation to a connected
   * device.
   */
  ...
  avsCommon::utils::mediaPlayer::MediaPlayerObserverInterface::SourceId
      m_sourceId;

  /// The @c BluetoothDeviceManagerInterface instance responsible for device
  /// management.
  std::shared_ptr<
      avsCommon::sdkInterfaces::bluetooth::BluetoothDeviceManagerInterface>
      m_deviceManager;
  ...
  std::deque<avsCommon::sdkInterfaces::bluetooth::services::AVRCPCommand>
      m_cmdQueue;

  /// An event queue used to store events which need to be sent. The pair is
  ...
  /// The current activeDevice. This is the one that is connected and sending
  /// media via A2DP.
  std::shared_ptr<avsCommon::sdkInterfaces::bluetooth::BluetoothDeviceInterface>
      m_activeDevice;
  ...
  /// Used to persist data necessary for Bluetooth. This includes UUID, MAC, and
  /// connection order.
  ...
  std::shared_ptr<avsCommon::avs::attachment::InProcessAttachment>
      m_mediaAttachment;

  /// A writer to write the A2DP stream buffers into the InProcessAttachment.
  std::shared_ptr<avsCommon::avs::attachment::AttachmentWriter>
      m_mediaAttachmentWriter;

  /// An executor used for serializing requests on the Bluetooth agent's own
  /// thread of execution.
