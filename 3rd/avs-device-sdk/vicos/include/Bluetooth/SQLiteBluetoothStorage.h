...
class SQLiteBluetoothStorage : public BluetoothStorageInterface {
 public:
   ...
   *
   * @param configurationRoot A ConfigurationNode containing the location of the
   * .db file. Should take the form: "bluetooth" : { "databaseFilePath" :
   * "<filePath>" }
   */
  ...
  bool getMacToUuid(
      std::unordered_map<std::string, std::string>* macToUuid) override;
  bool getUuidToMac(
      std::unordered_map<std::string, std::string>* uuidToMac) override;
  bool getOrderedMac(bool ascending, std::list<std::string>* macs) override;
  bool insertByMac(const std::string& mac, const std::string& uuid,
 ...
 private:
  ...
  /**
   * Utility that extracts a row from the database using a given statement. The
   * lock must be obtained before calling this function to ensure a consistent
   * result. A step will be executed on the passed in statement, so this
  ...
  /**
   * Utility that extracts from the database all uuid and mac pairs. The pairs
   * are returned in a map, and the key is dependent on the @c keyPreference
   * param. The lock must be obtained before calling this function.
   *
   * @param keyPreference A preference of whether the uuid or the mac should be
   * the key.
  ...
  /**
   * Utility that gets a data element from the database based on the constraint
   * key. The constraint key must be unique. The lock must be obtained before
   ...
   * @return A bool indicating success.
   */
  bool getAssociatedDataLocked(const std::string& constraintKey,
