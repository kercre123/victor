...
/// A storage interface used for Bluetooth that should provide UUID to MAC
/// mappings and maintain insertion order.
class BluetoothStorageInterface {
 public:
   ...
   * @return A bool indicating success.
   */
  virtual bool getMacToUuid(
      std::unordered_map<std::string, std::string>* macToUuid) = 0;
   ...
   * @return A bool indicating success.
   */
  virtual bool getUuidToMac(
      std::unordered_map<std::string, std::string>* uuidToMac) = 0;
   ...
   * has the same MAC address, the operation should fail unless overwrite is
   * specified.
   *
   * @param mac The MAC address.
   * @param uuid The UUID.
   * @param overwrite Whether or not to overwrite an existing entry with the
   * same MAC address.
   * @return A bool indicating success.
   */
  virtual bool insertByMac(const std::string& mac, const std::string& uuid,
                           bool overwrite) = 0;

  /**
   * Remove the entry by the MAC address. The operation is considered successful
   * if the entry no longer exists after this call, including the case where the
   * entry did not exist prior.
   *
   * @param mac The MAC address.
   * @return A bool indicating success.
   */
