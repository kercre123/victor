...
/**
 * In-memory implementation for the equalizer configuration. The configuration
 * is set during the creation and is not changed afterwards. This class also
 * holds the hardcoded defaults used by SDK.
 */
class InMemoryEqualizerConfiguration
    : public avsCommon::sdkInterfaces::audio::EqualizerConfigurationInterface {
 public:
  /**
   * Destructor.
   */
  ...
  /**
   * Factory to create an instance of @c InMemoryEqualizerConfiguration from the
   * parameters provided.
   *
   ...
   * @param defaultState Default state of the equalizer used when there is no
   * state stored in a persistent storage or when a band level is being reset.
   * @return An instance of @c InMemoryEqualizerConfiguration on success, or @c
   * nullptr in case of invalid parameters or inconsistent configuration.
   */
  static std::shared_ptr<InMemoryEqualizerConfiguration> create(
      int minBandLevel, int maxBandLevel,
      const std::set<avsCommon::sdkInterfaces::audio::EqualizerBand>&
          bandsSupported,
      const std::set<avsCommon::sdkInterfaces::audio::EqualizerMode>&
          modesSupported,
  ...
  /**
   * A factory to create an instance of @c InMemoryEqualizerConfiguration using
   * the hardcoded defaults.
   *
   * @return An instance of @c InMemoryEqualizerConfiguration using the
   * hardcoded defaults.
   */
  ...
  std::set<avsCommon::sdkInterfaces::audio::EqualizerBand> getSupportedBands()
      const override;

  std::set<avsCommon::sdkInterfaces::audio::EqualizerMode> getSupportedModes()
      const override;
  ...
  avsCommon::sdkInterfaces::audio::EqualizerState getDefaultState()
      const override;

  bool isBandSupported(
      avsCommon::sdkInterfaces::audio::EqualizerBand band) const override;

  bool isModeSupported(
      avsCommon::sdkInterfaces::audio::EqualizerMode mode) const override;
  ///@}
 ...
 protected:
   ...
   * @param validateValues A flag indicating whether to check band levels for
   * the allowed range. This could be false for the case when allowed range is
   * inconsistent and there is no point in validating against it.
   * @return True if all bands are supported and (optionally) band levels are
   * withing the supported range, false otherwise.
   */
