...
/**
 * An implementation of @c EqualizerConfigurationInterface that uses SDK
...
class SDKConfigEqualizerConfiguration : public InMemoryEqualizerConfiguration {
 public:
  ...
  /**
   * Flag indicating whether we should treat a band as supported by default if
   * "bands" configuration branch exists in JSON configuration.
   */
  static const bool BAND_IS_SUPPORTED_IF_MISSING_IN_CONFIG = false;
  /**
   * Flag indicating whether we should treat a mode as supported by default if
   * "modes" configuration branch exists in JSON configuration.
   */
   ...
   *
   * @param configRoot @c ConfigurationNode containing the equalizer
   * capabilities.
   * @return A new instance of SDKConfigEqualizerConfiguration on success or @c
