...
/**
 * Class to control equalizer operations in the SDK. All the equalizer state
 * manipulations and corresponding notifications are performed within this
 * class.
 */
class EqualizerController {
 public:
   ...
   *
   * @param modeController Interface to handle mode changes. This parameter may
   * be null if no modes are supported in
   * @c configuration.
   * @param configuration Interface to provide equalizer capabilities and
   * configuration.
   * @param storage Interface to provide persistent storage for equalizer state.
   * @return A new instance of @c EqualizerController if all parameters are
   * valid, @c nullptr otherwise.
   */
  static std::shared_ptr<EqualizerController> create(
      std::shared_ptr<
          avsCommon::sdkInterfaces::audio::EqualizerModeControllerInterface>
          modeController,
      std::shared_ptr<
          avsCommon::sdkInterfaces::audio::EqualizerConfigurationInterface>
          configuration,
      std::shared_ptr<
          avsCommon::sdkInterfaces::audio::EqualizerStorageInterface>
          storage);
   ...
   * @return @c SuccessResult containing current level of the equalizer band in
   * dB on success and undefined value on failure.
   */
  avsCommon::utils::error::SuccessResult<int> getBandLevel(
      avsCommon::sdkInterfaces::audio::EqualizerBand band);

  /**
   * Get levels for multiple bands. Levels are in dB.
   *
   * @param bands A set of bands to get levels for.
   * @return @c SuccessResult containing a map requested bands and their current
   * levels on success and undefined value on failure. Levels are in dB.
   */
  avsCommon::utils::error::SuccessResult<
      avsCommon::sdkInterfaces::audio::EqualizerBandLevelMap>
  getBandLevels(std::set<avsCommon::sdkInterfaces::audio::EqualizerBand> bands);

  /**
   * Set new level for an equalizer band. Level is in dB.
   * This method is not reenterable, calling it from the thread of listener or
   * equalizer callback will cause deadlock.
   *
   * @param band Equalizer band to set level for.
   * @param level Level to set equalizer band to. Level is in dB.
   */
  void setBandLevel(avsCommon::sdkInterfaces::audio::EqualizerBand band,
                    int level);

  /**
   * Set levels for multiple equalizer bands. Levels are in dB.
   * This method is not reenterable, calling it from the thread of listener or
   * equalizer callback will cause deadlock. If unsupported bands/levels are
   * provided, method will try to use supported ones only and will truncate
   * levels if needed.
   *
   * @param bandLevelMap A map of equalizer bands to be changed and their new
   * levels. Levels are in dB.
   */
  void setBandLevels(
      const avsCommon::sdkInterfaces::audio::EqualizerBandLevelMap&
          bandLevelMap);

  /**
   * Adjust levels of multiple equalizer bands. Levels are in dB.
   * This method is not reenterable, calling it from the thread of listener or
   * equalizer callback will cause deadlock. If unsupported bands are provided,
   * method will try to use supported ones only. Adjustments leading to the
   * levels beyond the supported range will be truncated.
   *
   * @param bandAdjustmentMap A map of equalizer bands to be adjusted and
   * relative change to be applied. Negative values represent level decreasing.
   * Levels adjustment values are in dB.
   */
  void adjustBandLevels(
      const avsCommon::sdkInterfaces::audio::EqualizerBandLevelMap&
          bandAdjustmentMap);
   ...
   */
  void resetBands(
      const std::set<avsCommon::sdkInterfaces::audio::EqualizerBand>& bands);
   ...
   */
  void registerEqualizer(
      std::shared_ptr<avsCommon::sdkInterfaces::audio::EqualizerInterface>
          equalizer);
  ...
  void addListener(
      std::shared_ptr<
          avsCommon::sdkInterfaces::audio::EqualizerControllerListenerInterface>
          listener);
 ...
 private:
  ...
  /**
   * Performs the actual equalizer state change. Applies changes to all
   * equalizer implementations, then notifies all listeners of the changes
   * applied if any. Method assumes that m_stateMutex is locked.
   */
  void updateStateLocked();

  /**
   * Applies transformation operation over the current equalizer state's band
   * levels. Method assumes m_stateMutex to be locked.
   *
   * @param changesDataMap @c EqualizerBandLevelMap containing changes data for
   * required bands.
   * @param operation Modification operation to perform on current state's band
   * value and a change. Operation accepts original band level and data for the
   * change to be applied and returns modified value.
   */
  ...
  /**
   * Internal method to start equalizer band changes. Method assumes
   * m_stateMutex to be locked.
   *
   * @param bandLevelMap New levels for the equalizer bands.
   */
  void setBandLevelsLocked(
      const avsCommon::sdkInterfaces::audio::EqualizerBandLevelMap&
          bandLevelMap);
   ...
   *
   * @return State retrieved from the persistent storage or default state if no
   * or invalid state was restored.
   */
  avsCommon::sdkInterfaces::audio::EqualizerState loadState();

  /**
   * Truncate band level to fit the supported range according to the current
   * configuration.
   *
  ...
  std::shared_ptr<avsCommon::sdkInterfaces::audio::EqualizerStorageInterface>
      m_storage;
  ...
  /// A list of equalizer implementations that apply equalization to audio
  /// stream.
  std::list<
      std::shared_ptr<avsCommon::sdkInterfaces::audio::EqualizerInterface>>
      m_equalizers;
