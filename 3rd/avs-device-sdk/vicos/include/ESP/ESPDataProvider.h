...
/**
 * The ESPDataProvider is used to connect the sample app with the ESP library.
 * When enabled, the ESPDataProvider object feeds the ESP Library constantly
...
class ESPDataProvider : public ESPDataProviderInterface {
 public:
   ...
   *
   * @param audioProvider Should have the audio input stream used by the
   * wakeword engine and the input parameters.
   * @return A valid ESPDataProvider pointer if creation succeeds and a empty
   * pointer if it fails.
   */
  static std::unique_ptr<ESPDataProvider> create(
      const capabilityAgents::aip::AudioProvider& audioProvider);
 ...
 private:
  /**
   * ESP processing loop. This method will feed the ESP library with the audio
   * input until the ESPDataProvider is disabled.
   */
  void espLoop();
   ...
   *
   * @param reader Audio input stream that should be used to feed the ESP
   * library.
  ...
  /// Object used to calculate the frame energy. The access to this variable is
  /// guarded by @c m_mutex.
  ...
  /// Indicates if ESP data is provided or not. The access to this variable is
  /// guarded by @c m_mutex.
  bool m_isEnabled;
