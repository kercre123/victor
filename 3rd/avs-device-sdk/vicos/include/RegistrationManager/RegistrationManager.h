 ...
 *
 * When a user is logging out of the device, the registration manager will close
 * down the AVS connection, cancel ongoing directives and delete any customer
 * data saved in the device.
 */
class RegistrationManager {
 public:
   ...
   *
   * @param directiveSequencer Object used to clear directives during logout
   * process.
   * @param connectionManager Connection manager must be disabled during
   * customer logout.
   * @param dataManager Object that manages customer data, which must be cleared
   * during logout.
   */
  RegistrationManager(
  ...
  /**
   * Add a new registration observer object which will get notified after the
  ...
  /**
   * Remove the given observer object which will no longer get any registration
   * notification.
   *
 ...
 private:
  ...
  // Used to enable / disable connection during logout to avoid any
  // interruption.
  std::shared_ptr<avsCommon::sdkInterfaces::AVSConnectionManagerInterface>
      m_connectionManager;

  // Used to clear customer data to ensure that a future login will not have
  // access to previous customer data
  ...
  // Observers
  std::unordered_set<std::shared_ptr<RegistrationObserverInterface> >
      m_observers;
};
