...
/**
 * The Interaction Model Capability Agent provides a way for AVS cloud initiated
 * actions to be executed by the client.
 *
 * When AVS requires the client to execute an action, it will send an
 * InteractionModel.NewDialogRequest directive to set a @c dialogRequestId on
 * the @c DirectiveSequencer.
 *
 * Once @c dialogRequestId is set, the @c DirectiveSequencer can then expect
 * directives with the actions tagged with the dialogRequestId.
 */
class InteractionModelCapabilityAgent
 ...
 public:
   ...
   *
   * @param directiveSequencer The Directive Sequencer responsible for
   * processing AVS directives.
 ...
 private:
   ...
   * @param[out] errMessage Error message associated to failure to process the
   * directive
  ...
  bool handleDirectiveHelper(
      std::shared_ptr<avsCommon::avs::CapabilityAgent::DirectiveInfo> info,
      std::string* errMessage, avsCommon::avs::ExceptionErrorType* type);

  /// Set of capability configurations that will get published using
  /// Capabilities API
  std::unordered_set<std::shared_ptr<avsCommon::avs::CapabilityConfiguration>>
      m_capabilityConfigurations;

  /// Pointer to the Directive Sequencer responsible for processing AVS
  /// directives
  std::shared_ptr<avsCommon::sdkInterfaces::DirectiveSequencerInterface>
