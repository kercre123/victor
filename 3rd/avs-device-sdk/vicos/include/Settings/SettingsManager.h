...
class SettingsManager {
 public:
   ...
   *
   * Explanation:
   *   - tuple definition: This allow us to use @c std::get<> to get a specific
   * index of @c SettingsT.
  ...
  using SettingType = typename std::decay<decltype(
      *std::get<index>(std::tuple<SettingsT*...>()))>::type;
  ...
  /**
   * Enqueue request to set the given settings represented by @c id to the given
   * @c value.
   *
   * @tparam index The index of the target setting.
  ...
  /**
   * Get the current value of this setting. This will not take into
   * consideration pending @c set requests.
   *
   * @tparam index The index of the target setting.
   * @param defaultValue The value to be returned in case the setting does not
   * exist.
   * @return A pair representing whether the setting exist and what is its
   * value. If the setting doesn't exist, this function will return {false,
   * defaultValue}. If the setting exist, it will return {true, settingValue}.
   */
  template <size_t index>
  std::pair<bool, ValueType<index>> getValue(
      const ValueType<index>& defaultValue) const;
   ...
   * @return @c true if it succeeds; @c false if manager already has a setting
   * object from the same type.
   */
  template <size_t index>
...
template <size_t index>
std::pair<bool,
          typename SettingsManager<SettingsT...>::template ValueType<index>>
SettingsManager<SettingsT...>::getValue(
    const ValueType<index>& defaultValue) const {
  ...
  avsCommon::utils::logger::acsdkError(
      LogEntry("SettingManager", "getValueFailed")
...
template <typename... SettingsT>
template <size_t index>
SetSettingResult SettingsManager<SettingsT...>::setValue(
    const ValueType<index>& value) {
  std::lock_guard<std::mutex> lock{m_mutex};
  auto& setting = std::get<index>(m_settings);
  if (setting) {
  ...
  avsCommon::utils::logger::acsdkError(
      LogEntry("SettingManager", "setValueFailed")
          .d("reason", "invalidSetting")
          .d("settingIndex", index));
...
template <typename... SettingsT>
template <size_t index>
bool SettingsManager<SettingsT...>::addObserver(
    std::shared_ptr<ObserverType<index>> observer) {
  std::lock_guard<std::mutex> lock{m_mutex};
  auto& setting = std::get<index>(m_settings);
  if (setting && observer) {
  ...
  avsCommon::utils::logger::acsdkError(
      LogEntry("SettingManager", "addObserverFailed")
          .d("reason", "invalidSetting")
          .d("settingIndex", index));
  return false;
}
...
template <typename... SettingsT>
template <size_t index>
void SettingsManager<SettingsT...>::removeObserver(
    std::shared_ptr<ObserverType<index>> observer) {
  std::lock_guard<std::mutex> lock{m_mutex};
  auto& setting = std::get<index>(m_settings);
  if (setting && observer) {
  ...
  avsCommon::utils::logger::acsdkError(
      LogEntry("SettingManager", "removeObserverFailed")
          .d("reason", "invalidSetting")
          .d("settingIndex", index));
}
...
template <typename... SettingsT>
template <size_t index>
bool SettingsManager<SettingsT...>::addSetting(
    std::shared_ptr<SettingType<index>> newSetting) {
  std::lock_guard<std::mutex> lock{m_mutex};
  auto& setting = std::get<index>(m_settings);
  ...
  avsCommon::utils::logger::acsdkError(
      LogEntry("SettingManager", "addSettingFailed")
          .d("reason", "invalidSetting")
          .d("settingIndex", index));
  return false;
}
...
template <typename... SettingsT>
template <size_t index>
void SettingsManager<SettingsT...>::removeSetting(
    std::shared_ptr<SettingType<index>> oldSetting) {
  std::lock_guard<std::mutex> lock{m_mutex};
  auto& setting = std::get<index>(m_settings);
  if (setting == oldSetting) {
    ...
    avsCommon::utils::logger::acsdkError(
        LogEntry("SettingManager", "removeSettingFailed")
            .d("reason", "invalidSetting")
            .d("settingIndex", index));
  }
}
