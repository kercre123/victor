...
/**
 * The SettingCallbacks allow easy management of callbacks to monitor multiple
 * settings with one object only.
 *
 * This object can be used to register multiple callbacks that are member
 * functions, lambdas and static functions. E.g.:
 *
 * @code
 *
 * MyClass(std::shared_ptr<DeviceSettingManagers> manager) : m_callbacks
 * {manager} { m_callbacks.add<ASCENDING_ALARM>([](bool enable,
 * SettingNotifications notification){
 *         // do something...
 *     });
 *     m_callbacks.add<TIMEZONE>(staticFunction);
 *     m_callbacks.add<WAKEWORD_CONFIRMATION>(std::bind(this,
 * &MyClass::onWakewordConfirmation, std::placeholder::_1));
 * }
 *
 * ~MyClass() {
 ...
 * @endcode
 *
 * If you would like to listen notifications related to only one setting, see @c
 * SettingEventCallback class or extend the @c SettingObserverInterface class.
 *
...
class SettingCallbacks {
 public:
  ...
  template <size_t id>
  using FunctionType =
      std::function<void(const ValueType<id>&, SettingNotifications)>;
   ...
   * @return Pointer to new object if it succeeds to add all callbacks, @c
   * nullptr otherwise.
   */
  static std::shared_ptr<SettingCallbacks<ManagerT>> create(
  ...
  /**
   * Disconnects the observed setting from all observed settings. This function
   * also gets called by the destructor.
   *
   * @param manager The object that manages all settings.
 ...
 private:
  ...
  std::vector<std::shared_ptr<SettingCallbackAdapterInterface<ManagerT>>>
      m_callbacks;
};
...
template <typename ManagerT>
std::shared_ptr<SettingCallbacks<ManagerT>> SettingCallbacks<ManagerT>::create(
    std::shared_ptr<ManagerT> manager) {
  if (manager) {
    return std::shared_ptr<SettingCallbacks<ManagerT>>(
        new SettingCallbacks<ManagerT>(manager));
  }
  return nullptr;
}
...
template <typename ManagerT>
template <size_t id>
bool SettingCallbacks<ManagerT>::add(
    SettingCallbacks::FunctionType<id> callback) {
...
template <typename ManagerT>
SettingCallbacks<ManagerT>::SettingCallbacks(std::shared_ptr<ManagerT> manager)
    : m_manager{manager} {}
