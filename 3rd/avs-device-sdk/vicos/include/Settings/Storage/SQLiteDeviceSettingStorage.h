...
class SQLiteDeviceSettingStorage : public DeviceSettingStorageInterface {
 public:
   ...
   * SQLite database. Settings will be stored in the misc database.
   *
   * @param configurationRoot The global config object where the location of the
   * misc database can be found.
   * @return Pointer to the SQLiteDeviceSettingStorage object, nullptr if
   * there's an error creating it.
   */
  ...
  bool open() override;
  void close() override;
  bool storeSetting(const std::string& key, const std::string& value,
                    SettingStatus status) override;
  SettingStatusAndValue loadSetting(const std::string& key) override;
  bool deleteSetting(const std::string& key) override;
  bool updateSettingStatus(const std::string& key,
                           SettingStatus status) override;
  /// @}
 ...
 private:
   ...
   *
   * @return A boolean to specify whether the creation of the table was
   * successful.
   */
