...
/**
 * This class provides a guaranteed message delivery service to AVS.  Upon
 * calling the single api,
 * @c sendJSONMessage, this class will persist the message and continually
 * attempt sending it until it succeeds. The persistence will work across
 * application runs, dependent on the nature of the storage object provided.
 *
 * To avoid excessive memory usage, the maximum number of messages stored in
 * this way is configurable via the settings 'queueSizeWarnLimit' and
 * 'queueSizeHardLimit', under the configuration root 'certifiedSender'.
 *
 * Similarly, the file path for the database storage is configured under the
 * setting 'databaseFilePath'.
 *
 * This class maintains the ordering of messages passed to it.  For example, if
 * @c sendJSONMessage is invoked with messages A then B then C, then this class
 * guarantees that the messages will be sent to AVS in the same order - A then B
 * then C.
 */
class CertifiedSender
 ...
 public:
  /**
   * This function creates a new instance of a @c CertifiedSender. If it fails
   * for any reason, @c nullptr is returned.
   *
   * @param messageSender The entity which is able to send @c MessageRequests to
   * AVS.
   * @param connection The connection which may be observed to determine
   * connection status.
   * @param storage The object which manages persistent storage of messages to
   * be sent.
  ...
  /**
   * Function to request a message be sent to AVS.  Since this class is strictly
   * responsible for sending a message to AVS, the parameter is explicitly
   * described as being in JSON format.  This will be expected to take the form
   * of some kind of Event which AVS understands.  While the sending of the
   * message is entirely asynchronous, the future returned allows the caller to
   * know if the request was successfully persisted.  Once the message is
   * persisted, the caller can expect the message to be sent to AVS at some
   * point in the future by this class.
   *
 ...
 private:
  ...
  class CertifiedMessageRequest : public avsCommon::avs::MessageRequest {
   public:
    ...
    void sendCompleted(
        avsCommon::sdkInterfaces::MessageRequestObserverInterface::Status
            sendMessageStatus) override;

    /**
     * A blocking function which will return once the @c MessageSender has
     * completed processing the message.
     *
     * @return The status returned by the @c MessageSender once it's handled the
    ...
    /**
     * Utility function to return the database id associated with this @c
     * MessageRequest.
     *
   ...
   private:
    ...
    std::mutex m_mutex;
    /// The condition variable used when waiting for the @c MessageRequest to be
    /// processed.
   ...
   * @param dataManager A dataManager object that will track the
   * CustomerDataHandler.
   * @param queueSizeWarnLimit The number of items we can store for sending
   * without emitting a warning.
   * @param queueSizeHardLimit The maximum number of items we can store for
   * sending.
   */
  CertifiedSender(
      std::shared_ptr<avsCommon::sdkInterfaces::MessageSenderInterface>
  ...
  /// A queue size threshold, beyond which we will emit warnings if more items
  /// are added.
  ...
  std::mutex m_mutex;
  /// A condition variable with which to notify the worker thread that a new
  ...
  /// Executor to decouple the public-facing api from possibly inefficient
