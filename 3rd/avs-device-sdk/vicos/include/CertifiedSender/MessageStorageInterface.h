...
/**
 * An Interface class which defines APIs for interacting with a database for
 * storing text-based messages.
 *
 * An implementation of this interface must enforce ordering of the messages, so
 * that the ordering of items returned in the load() operation are the same as
 * the ordering of store() calls.
 *
...
class MessageStorageInterface {
 public:
  ...
  struct StoredMessage {
     ...
     * Constructor.
     *
     * @param id The id which the database implementation associates with the
     * message.
     * @param message The text message which has been stored in the database.
     */
    StoredMessage(int id, const std::string& message)
   ...
   * If a database is already being handled by this object, or there is are
   * other errors creating the database, this function returns false.
   *
   * @return @c true If the database is created ok, or @c false if a database is
   * already being handled by this object or there is an internal error creating
   * the database.
   */
   ...
   * @param[out] id The id associated with the stored messsage, if successfully
   * stored.
