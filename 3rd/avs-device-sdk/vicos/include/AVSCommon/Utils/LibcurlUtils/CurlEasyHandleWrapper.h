...
class CurlEasyHandleWrapper {
 public:
   ...
   * size_t callback(char* buffer, size_t blockSize, size_t numBlocks, void*
   * userData)
   ...
   * @param userData Some user data passed in with CURLOPT_XDATA (where X =
   * READ, WRITE, or HEADER)
   */
  using CurlCallback = size_t (*)(char* buffer, size_t blockSize,
                                  size_t numBlocks, void* userData);

  /**
   * Debug Callbacks to libcurl typically follow the below pattern.
   * size_t callback(CURL* handle, curl_infotype infoType, char *buffer, size_t
   * blockSize, void *userData)
   ...
   *
   * @param id name to use to identify this handle.  If none provided, an
   * automatically generated one will be assigned.
   */
  ...
  /*
   * Adds a POST Header to the list of the headers to add to the future POST
   * request
   *
   * @param header The HTTP header to add
   * @returns true if the addition was successful
  ...
  /**
   * Sets a timeout, in seconds, for how long the stream transfer is allowed to
   * take. If not set explicitly, there will be no timeout.
   *
   * @param timeoutSeconds The amount of time in seconds the stream can take.
   * Set to @c 0 to disable a timeout.
   ...
   *
   * @param data String buffer to the full data to send in a HTTP POST
   * operation.
  ...
  /**
   * Sets how long the stream should take, in seconds, to establish a
   * connection. If not set explicitly there is no timeout.
   *
   * @param timeoutSeconds The amount of time, in seconds, establishing a
   * connection should take. Set to @c 0 to disable a timeout.
 ...
 private:
   ...
   * @param user User pointer used to pass which HTTP2Stream is being reported
   * on.
   * @return Always returns zero.
   */
  static int debugFunction(CURL* handle, curl_infotype type, char* data,
