...
class HttpPost : public HttpPostInterface {
 public:
  /**
   * Create a new HttpPost instance, passing ownership of the new instance on to
   * the caller.
   *
   * @return Returns an std::unique_ptr to the new HttpPost instance, or @c
  ...
  long doPost(const std::string& url, const std::string& data,
              std::chrono::seconds timeout, std::string& body) override;

  HTTPResponse doPost(
      const std::string& url, const std::vector<std::string> headerLines,
      const std::vector<std::pair<std::string, std::string>>& data,
      std::chrono::seconds timeout) override;

  HTTPResponse doPost(const std::string& url,
 ...
 private:
   ...
   */
  static size_t staticWriteCallbackLocked(char* ptr, size_t size, size_t nmemb,
                                          void* userdata);
