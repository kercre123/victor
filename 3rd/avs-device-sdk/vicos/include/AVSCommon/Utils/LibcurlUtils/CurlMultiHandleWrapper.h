 ...
 * Wrapping enables:
 * - tracking handles to ensure that all handles are removed before
...
class CurlMultiHandleWrapper {
 public:
   ...
   *
   * @return The new CurlMultiHandleWrapper instance, or nullptr if the
   * operation fails.
   */
  ...
  /**
   * Read and/or write available data for the @c libcurl @c handles added to
   * this @c libcurl @c multi @c handle.
   *
   * @param[out] runningHandles Returns the number of handles for which
  ...
  /**
   * Wait for actions to perform on the @c libcurl @c handles added to this @c
   * libcurl @c multi @c handle.
   *
   * @param timeout How long to wait for actions to perform.
   * @param[out] countHandlesUpdated The number of handles for which actions are
  ...
  /**
   * Receive the next messages about the @c libcurl @c handles added to this @c
   * libcurl @c multi @c handle.
   *
   * @param[out] messagesInQueue The number of @c libcurl @c handles  for which
   * messages remain.
