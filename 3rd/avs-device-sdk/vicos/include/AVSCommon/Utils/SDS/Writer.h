...
/**
 * This is a nested class in @c SharedDataStream which provides an interface for
 * writing (producing) data to the stream.
 *
 * @note This class is primarily intended to be used from a single thread.  The
 * @c Writer as a whole is thread-safe in the sense that @c Writer and @c
 * Readers can all live in different threads, but individual member functions of
 * a
 * @c Writer instance should not be called from multiple threads except where
 * specifically noted in function documentation below.
 */
template <typename T>
class SharedDataStream<T>::Writer {
 public:
   ...
   *
   * @note Using a weakly-typed enum so errors can be compared to @c write()
   * return values without casting.
   */
  struct Error {
    enum {
      /// Returned when @c close() has been previously called on the @c Writer.
      CLOSED = 0,
      /// Returned when policy is @c Policy::ALL_OR_NOTHING and the @c write()
      /// would overwrrite unconsumed data.
      WOULDBLOCK = -1,
      /// Returned when a @c write() parameter is invalid.
      INVALID = -2,
      /// Returned when policy is @c Policy::BLOCKING and no space becomes
      /// available before the specified timeout.
      TIMEDOUT = -3,
    };
  };

  /**
   * Constructs a new @c Writer which produces data for the provided @c
   * SharedDataStream.
   *
   ...
   * @param timeout The maximum time to wait (if @c policy is @c BLOCKING) for
   * space to write into.  If this parameter is zero, there is no timeout and
   * blocking writes will wait forever.  If @c policy is not @C BLOCKING, this
   *     parameter is ignored.
   * @return The number of @c wordSize words copied, or zero if the stream has
   * closed, or a negative @c Error code if the stream is still open, but no
   * data could be written.
   *
   * @note A stream is closed for the @c Writer if @c Writer::close() has been
   * called.
   *
   * @warning If @c policy is @c BLOCKING and @c timeout is 0, this function
   * will only unblock when a @c Reader
   *     @c read()s some data or @c seek()s forward.  Applications which use
   * this combination of parameters must use
   *     @c Readers to drain some data from the @c SharedDataStream if they need
   * to unblock the @c Writer.
   */
  ssize_t write(
  ...
  /**
   * This function closes the @c Writer, such that @c Readers will return 0 when
   * they catch up with the @c Writer, and subsequent calls to @c write() will
   * return 0.
   */
  void close();

  /**
   * This function returns the word size (in bytes).  All @c SharedDataStream
   * operations that work with data or position in the stream are quantified in
   * words.
   *
   * @return The size (in bytes) of words for this @c Writer's @c
   * SharedDataStream.
   */
...
template <typename T>
SharedDataStream<T>::Writer::Writer(Policy policy,
                                    std::shared_ptr<BufferLayout> bufferLayout)
    : m_policy{policy}, m_bufferLayout{bufferLayout}, m_closed{false} {
  // Note - SharedDataStream::createWriter() holds writerEnableMutex while
...
ssize_t SharedDataStream<T>::Writer::write(const void* buf, size_t nWords,
  ...
  if (nullptr == buf) {
    logger::acsdkError(
        logger::LogEntry(TAG, "writeFailed").d("reason", "nullBuffer"));
    return Error::INVALID;
  }
  if (0 == nWords) {
    logger::acsdkError(
        logger::LogEntry(TAG, "writeFailed").d("reason", "zeroNumWords"));
    return Error::INVALID;
  }
  ...
  if (!header->isWriterEnabled) {
    logger::acsdkError(
        logger::LogEntry(TAG, "writeFailed").d("reason", "writerDisabled"));
    return Error::CLOSED;
  }

  auto wordsToCopy = nWords;
  auto buf8 = static_cast<const uint8_t*>(buf);
  std::unique_lock<Mutex> backwardSeekLock(header->backwardSeekMutex,
                                           std::defer_lock);
  Index writeEnd = header->writeStartCursor + nWords;

  switch (m_policy) {
    case Policy::NONBLOCKABLE:
      // For NONBLOCKABLE, we can truncate the write if it won't fit in the
      // buffer.
    ...
    case Policy::ALL_OR_NOTHING:
      // For ALL_OR_NOTHING, we can't overwrite readers, and we can't truncate,
      // but we might be able to discard bytes that overflow if
      // oldestUnconsumedCursor is in the future (e.g. there are readers waiting
      // for future data which has not been written yet).

      // Note - this check must be performed while locked to prevent a reader
      // from backwards-seeking into the write region between here and the
      // writeEndCursor update below.
      backwardSeekLock.lock();
      if ((writeEnd >= header->oldestUnconsumedCursor) &&
          ((writeEnd - header->oldestUnconsumedCursor) >
           m_bufferLayout->getDataSize())) {
        return Error::WOULDBLOCK;
      }
      break;
    case Policy::BLOCKING:
      // For BLOCKING, we need to wait until there is room for at least one
      // word.
      ...
      if (std::chrono::milliseconds::zero() == timeout) {
        header->spaceAvailableConditionVariable.wait(backwardSeekLock,
                                                     predicate);
      } else if (!header->spaceAvailableConditionVariable.wait_for(
                     backwardSeekLock, timeout, predicate)) {
  ...
  // We've updated our end cursor, so we no longer need to hold off backward
  // seeks.
  if (backwardSeekLock) {
  ...
  if (Policy::ALL_OR_NOTHING == m_policy) {
    // If we have more data than the SDS can hold and we're not going to be
    // overwriting oldestUnconsumedCursor, we can safely discard the initial
    // data and just leave the trailing data in the buffer.
  ...
  memcpy(m_bufferLayout->getData(header->writeStartCursor), buf8,
         beforeWrap * getWordSize());
  if (afterWrap > 0) {
    memcpy(m_bufferLayout->getData(header->writeStartCursor + beforeWrap),
           buf8 + beforeWrap * getWordSize(), afterWrap * getWordSize());
  }

  // Advance the write cursor.
  // Note: To prevent a race condition and ensure that readers which block on
  // dataAvailableConditionVariable don't miss a notify, we should always lock
  // the dataAvailableConditionVariable mutex while moving writeStartCursor.  As
  // an optimization, we skip that lock for NONBLOCKABLE writers under the
  // assumption that they will be writing continuously, so a missed notification
  // is not significant. Note: As a further optimization, the lock could be
  // omitted if no blocking readers are in use (ACSDK-251).
  ...
  // Notify the reader(s).
  // Note: as an optimization, we could skip this if there are no blocking
  // readers (ACSDK-251).
...
std::string SharedDataStream<T>::Writer::errorToString(Error error) {
  ...
  logger::acsdkError(logger::LogEntry(TAG, "errorToStringFailed")
                         .d("reason", "invalidError")
                         .d("error", error));
