...
/**
 * This is a nested class in SharedDataStream which provides an interface for
 * reading (consuming) data from a stream.
 *
 * @note This class is primarily intended to be used from a single thread.  The
 * @c Reader as a whole is thread-safe in the sense that @c Writer and @c
 * Readers can all live in different threads, but individual member functions of
 * a
 * @c Reader instance should not be called from multiple threads except where
...
class SharedDataStream<T>::Reader {
 public:
  ...
  /// Specifies a reference to measure @c seek()/@c tell()/@c close() offsets
  /// against.
   ...
   *
   * @note Using a weakly-typed enum so errors can be compared to @c read()
   * return values without casting.
   */
  struct Error {
    enum {
      /**
       * Returned when the stream is closed - either due to a @c Writer::close()
       * call and no remaining buffered data, or due to a @c Reader::close()
       * call which has reached its close index.
       */
      CLOSED = 0,
      /// Returned when the data requested has been overwritten and is invalid.
      OVERRUN = -1,
      /// Returned when policy is @c Policy::NONBLOCKING and no data is
      /// available.
      WOULDBLOCK = -2,
      /// Returned when policy is @c Policy::BLOCKING and no data becomes
  ...
  /**
   * Constructs a new @c Reader which consumes data from the provided @c
   * SharedDataStream.  The caller must hold
   ...
   * buffer.
   *
   * @param buf A buffer to copy the consumed data to.  This buffer must be
   * large enough to hold @c nWords
   ...
   * data.  If this parameter is zero, there is no timeout and blocking reads
   * will wait forever.  If @c policy is @c NONBLOCKING, this parameter is
   * ignored.
   * @return The number of @c wordSize words copied if data was consumed, or
   * zero if the stream has closed, or a negative @c Error code if the stream is
   * still open, but no data could be consumed.
   *
   * @note A stream is closed for the @c Reader if @c Reader::close() has been
   * called on it, or if @c Writer::close() has been called and the @c Reader
   * has consumed all remaining data left in the stream when the @c Writer
   *     closed.  In the special case of a new stream, where no @c Writer has
   * been created, the stream is not considered to be closed for the @c Reader;
   * attempts to @c read() will either block or return @c WOULDBLOCK, depending
   * on the @c Policy.
   */
  ssize_t read(
      void* buf, size_t nWords,
      std::chrono::milliseconds timeout = std::chrono::milliseconds(0));

  /**
   * This function moves the @c Reader to the specified location in the stream.
   * If successful, subsequent calls to
   * @c read() will start from the new location.  For this function to succeed,
   * the specified location *must* point at data which has not been pushed out
   * of the buffer; if the specified location points at old data which has
   * already been overwritten, the @c seek() call will fail.  If the specified
   * location points at future data which does not yet exist in the buffer, the
   * @c seek() call will succeed.  If the @c seek() call fails, the @c Reader
   * position will remain unchanged.
   *
   * @param offset The position (in @c wordSize words) in the stream, relative
   * to @c reference, to move the @c Reader to.
   * @param reference The position in the stream @c offset is applied to.
   * @return @c true if the specified position points at unconsumed data, else
   * @c false.
   */
   ...
   * relative to @c reference.
   *
   * @note For @c Reference::BEFORE_WRITER, if the read cursor points at a
   * location in the future (after the writer), then the reader is not before
   * the writer, so this function will return 0.
   */
  Index tell(Reference reference = Reference::ABSOLUTE) const;

  /**
   * This function sets the point at which the @c Reader's stream will close.
   * With the default parameters, this function will close t he stream
   * immediately, without reading any additional data.  To schedule the stream
   * to close once all the data which is currently in the buffer has been read,
   * call `close(0, Reference::BEFORE_WRITER)`. If another close point is
   * desired, it can be specified using a different
   * @c offset and/or @c reference.
   *
   * @param offset The position (in @c wordSize words) in the stream, relative
   * to @c reference, to close at.
   * @param reference The position in the stream the close point is measured
   * against.
   *
   * @note This function can be called from any thread or process, and it will
   * schedule the @c Reader to close, however it will *not* wake up a @c
   * BLOCKING @c Reader which is already blocked waiting for data.  In the case
   * of a blocked @c read(), that @c read() will return when it wakes up -
   * either due to a timeout, or due to a @c Writer adding data to the stream.
   */
  void close(Index offset = 0, Reference reference = Reference::AFTER_READER);

  /**
   * This function returns the id assigned to this @c Reader.  If a @c Reader
   * instance is not destroyed cleanly (e.g. a @c Reader from another process
   * that crashes), its id can be passed to @c SharedDataStream::reset() to free
   * up its resources.
   *
   ...
   * words.
   *
   * @return The size (in bytes) of words for this @c Reader's @c
   * SharedDataStream.
   */
 ...
 private:
  ...
  /**
   * The index in @c BufferLayout::getReaderCursorArray() and @c
   * BufferLayout::getReaderCloseIndexArray() assigned to this @c Reader.
   */
  uint8_t m_id;
  ...
  /// Pointer to this reader's close index in
  /// BufferLayout::getReaderCloseIndexArray().
...
template <typename T>
SharedDataStream<T>::Reader::Reader(Policy policy,
  ...
  // Note - SharedDataStream::createReader() holds readerEnableMutex while
  // calling this function. Read new data only. Note: It is important that new
  // readers start with their cursor at the writer.  This allows
  // updateOldestUnconsumedCursor() to be thread-safe without holding
  // readerEnableMutex.  See updateOldestUnconsumedCursor() comments for further
  // explanation.
...
SharedDataStream<T>::Reader::~Reader() {
  ...
  // introduce a race condition in updateOldestUnconsumedCursor().  See
...
template <typename T>
ssize_t SharedDataStream<T>::Reader::read(void* buf, size_t nWords,
                                          std::chrono::milliseconds timeout) {
  if (nullptr == buf) {
    logger::acsdkError(
        logger::LogEntry(TAG, "readFailed").d("reason", "nullBuffer"));
    return Error::INVALID;
  }

  if (0 == nWords) {
    logger::acsdkError(logger::LogEntry(TAG, "readFailed")
                           .d("reason", "invalidNumWords")
                           .d("numWords", nWords));
    return Error::INVALID;
  }
  ...
  if ((header->writeEndCursor >= *m_readerCursor) &&
      (header->writeEndCursor - *m_readerCursor) >
  ...
  if (0 == wordsAvailable) {
    if (header->writeEndCursor > 0 && !header->isWriterEnabled) {
      ...
      // Condition for returning from read: the Writer has been closed or there
      // is data to read
      auto predicate = [this, header] {
        return header->hasWriterBeenClosed ||
      ...
      } else if (!header->dataAvailableConditionVariable.wait_for(lock, timeout,
                                                                  predicate)) {
  ...
  memcpy(buf8, m_bufferLayout->getData(*m_readerCursor),
         beforeWrap * getWordSize());
  if (afterWrap > 0) {
    memcpy(buf8 + (beforeWrap * getWordSize()),
  ...
  *m_readerCursor += nWords;
  ...
  // Final check for overrun (do this before the updateOldestUnconsumedCursor()
  // call below for improved accuracy).
  bool overrun = ((header->writeEndCursor - *m_readerCursor) >
                  m_bufferLayout->getDataSize());
...
bool SharedDataStream<T>::Reader::seek(Index offset, Reference reference) {
  ...
  switch (reference) {
    ...
    case Reference::BEFORE_WRITER:
      if (offset > *writeStartCursor) {
        logger::acsdkError(
            logger::LogEntry(TAG, "seekFailed")
                .d("reason", "seekBeforeStreamStart")
  ...
  // Per documentation of updateOldestUnconsumedCursor(), don't try to seek
  // backwards while oldestConsumedCursor is being updated.
  ...
  // starting to overwrite us between here and the m_readerCursor update below.
  // If this is not a backward seek, then the mutex is not held.
  if (*writeEndCursor >= absolute &&
      *writeEndCursor - absolute > m_bufferLayout->getDataSize()) {
    logger::acsdkError(
        logger::LogEntry(TAG, "seekFailed").d("reason", "seekOverwrittenData"));
    return false;
  }
...
template <typename T>
typename SharedDataStream<T>::Index SharedDataStream<T>::Reader::tell(
  ...
  switch (reference) {
    ...
    case Reference::BEFORE_WRITER:
      return (*writeStartCursor >= *m_readerCursor)
                 ? *writeStartCursor - *m_readerCursor
                 : 0;
    case Reference::ABSOLUTE:
      return *m_readerCursor;
  }
  logger::acsdkError(
      logger::LogEntry(TAG, "tellFailed").d("reason", "invalidReference"));
...
void SharedDataStream<T>::Reader::close(Index offset, Reference reference) {
  ...
  switch (reference) {
    ...
    case Reference::BEFORE_WRITER:
      if (*writeStartCursor < offset) {
        logger::acsdkError(
            logger::LogEntry(TAG, "closeFailed")
  ...
  if (!validReference) {
    logger::acsdkError(
        logger::LogEntry(TAG, "closeFailed").d("reason", "invalidReference"));
  }
