...
enum class PlaylistParseResult {
  ...
  /// The playlist has been fully parsed successfully. This indicates that
  /// parsing of the playlist has completed.
  FINISHED,

  /**
   * The playlist parsing has encountered an error and will abort parsing. In
   * this case, the url in the callback will not be valid.
   */
  ERROR,
...
struct EncryptionInfo {
   ...
   */
  EncryptionInfo(Method method, std::string url,
...
struct PlaylistEntry {
   ...
   * @return PlaylistEntry with url, INVALID_DURATION and STILL_ONGOING
   * parseResult.
   */
  static PlaylistEntry createMediaInitInfo(
      std::string url, ByteRange byteRange = std::make_tuple(0, 0));
   ...
   * @param _encryptionInfo The encryption info of the media. Default value is
   * NONE.
   */
  PlaylistEntry(
      std::string _url, std::chrono::milliseconds _duration,
...
inline PlaylistEntry PlaylistEntry::createErrorEntry(const std::string& url) {
  return PlaylistEntry(url, std::chrono::milliseconds(-1),
                       PlaylistParseResult::ERROR);
}

inline PlaylistEntry PlaylistEntry::createMediaInitInfo(std::string url,
                                                        ByteRange byteRange) {
  auto duration = std::chrono::milliseconds(-1);
  return PlaylistEntry(url, duration, PlaylistParseResult::STILL_ONGOING,
                       Type::MEDIA_INIT_INFO, byteRange);
}

inline EncryptionInfo::EncryptionInfo() : method(Method::NONE) {}

inline EncryptionInfo::EncryptionInfo(Method method, std::string url,
                                      std::string initVector)
    : method(method), keyURL(url), initVector(initVector) {}

inline bool EncryptionInfo::isValid() const {
  return (Method::NONE == method) ||
         (Method::NONE != method && !keyURL.empty() && !initVector.empty());
}
...
inline PlaylistEntry::PlaylistEntry(
    std::string _url, std::chrono::milliseconds _duration,
    avsCommon::utils::playlistParser::PlaylistParseResult _parseResult,
    Type _type, ByteRange _byteRange, EncryptionInfo _encryptionInfo)
    : type(_type),
      url(_url),
      duration(_duration),
