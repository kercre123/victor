 ...
 *
 * E.g.: To build the following json:
 * {"param1":"value","param2":{"param2.1":100}}. Use:
 *
...
class JsonGenerator {
 public:
  ...
  /**
   * Checks whether the generator has been finalized (i.e., no changes can be
   * made to the current json).
   *
   ...
   * @return @c true if it succeeds to create a new object and @c false if it
   * fails.
   */
   ...
   * @return @c true if it succeeded to add the new member and @c false
   * otherwise.
   */
   ...
   */
  bool addRawJsonMember(const std::string& key, const std::string& json,
                        bool validate = true);

  /**
   * Return the string representation of the object.
   *
   * @param finalize If set to @c true the object will be finalized and the
   * string returned will be a complete json document. If @c false, the returned
   * string will represent the current state of the json generation which could
   * be partial.
   * @note Once the object has been finalized, no changes can be made to the
   * generator.
 ...
 private:
  ...
  /**
   * Method used to finalize the json. This will close all the open objects
