 ...
 *
 * The name ThreadMoniker is used instead of ThreadId to avoid confusion with
 * platform specific thread identifiers or the @c std::thread::id values
...
class ThreadMoniker {
 public:
  ...
  /**
   * Set the moniker for @c std::this_thread. This method should be called
   * before @c getThisThreadMoniker() in order to take effect.
   *
 ...
 private:
   ...
   * Constructor.
   *
   * @param moniker Optional moniker for this thread. If no moniker is provided,
   * a new moniker will be provided.
   */
   ...
   *
   * @param moniker Use this moniker to initialize the @c ThreadMoniker if it
   * doesn't exist already.
   * @return The moniker for the @c std::this_thread.
   */
  static inline const ThreadMoniker& getMonikerObject(
      const std::string& moniker = std::string());

  /**
   * Return the @c ThreadMoniker object for the current thread for OS that don't
   ...
   * @return The moniker for the @c std::this_thread.
   */
  static const ThreadMoniker& getMonikerObjectFromMap(
      const std::string& moniker = std::string());
...
const ThreadMoniker& ThreadMoniker::getMonikerObject(
    const std::string& moniker) {
#ifdef _WIN32
  return getMonikerObjectFromMap(moniker);
#else
  /// Per-thread static instance so that @c m_threadMoniker.m_moniker is @c
  /// std::this_thread's moniker.
