...
/**
 * This class encapsulates content received from HTTP request, specifically the
 * status code, the content-type, and the actual content of the response.
 */
class HTTPContent {
 public:
  /**
   ...
   * @param stream The Attachment from which to read the HTTP content from or @c
   * nullptr if no data was fetched.
   */
  HTTPContent(
  ...
  /**
   * This function checks if the status code is HTTP success or not.  This
   * function blocks until status code is set.
   *
  ...
  /**
   * This function returns the status code.  This function blocks until status
   * code is set.
   *
  ...
  /**
   * This function returns the content type.  This function blocks until content
   * type is set.
   *
 ...
 private:
  ...
  /// An @c Attachment from which to read the HTTP content from or @c nullptr if
  /// no data was fetched.
  std::shared_ptr<avsCommon::avs::attachment::InProcessAttachment> m_dataStream;
};

inline HTTPContent::HTTPContent(
    std::future<long> httpStatusCode, std::future<std::string> httpContentType,
    std::shared_ptr<avsCommon::avs::attachment::InProcessAttachment> stream)
...
inline bool HTTPContent::isReady(
...
inline std::shared_ptr<avsCommon::avs::attachment::InProcessAttachment>
HTTPContent::getDataStream() {
  return m_dataStream;
}
