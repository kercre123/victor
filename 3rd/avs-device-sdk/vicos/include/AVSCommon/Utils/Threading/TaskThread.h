 ...
 *
 * @note It's the caller responsibility to restart the @c TaskThread if
 * jobRunner returns false.
 */
class TaskThread {
 public:
  /**
   * Constructs a TaskThread to read from the given TaskQueue. This does not
   * start the thread.
   *
  ...
  /**
   * Start executing tasks from the given job runner. The task thread will keep
   * running until @c jobRunner returns @c false or @c start gets called again.
   *
   * @param jobRunner Function that should execute jobs. The function should
   * return @c true if there's more tasks to be executed.
   * @return @c true if it succeeds to start the new jobRunner thread; @c false
   * if it fails.
   */
 ...
 private:
   ...
   *
   * @param jobRunner Function that should execute the next job. The function
   * should return @c true if a new job still exists.
   */
  ...
  /// Flag used by the new thread to ensure that the old thread will exit once
