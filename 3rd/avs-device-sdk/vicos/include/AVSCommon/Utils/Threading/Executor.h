...
class Executor {
 public:
   ...
   *
   * @param delayExit The period of time that this executor will keep its thread
   * running while waiting for a new job. We use 1s by default.
   */
  Executor(const std::chrono::milliseconds& delayExit =
   ...
   * another function object) to be executed on an Executor thread. The future
   * must be checked for validity before waiting on it.
   *
  ...
  /**
   * Submits a callable type (function, lambda expression, bind expression, or
   * another function object) to the front of the internal queue to be executed
   * on an Executor thread. The future must be checked for validity before
   * waiting on it.
   *
   * @param task A callable type representing a task.
   * @param args The arguments to call the task with.
   ...
   */
  template <typename Task, typename... Args>
  auto submitToFront(Task task, Args&&... args)
      -> std::future<decltype(task(args...))>;
  ...
  /// Clears the executor of outstanding tasks and refuses any additional tasks
  /// to be submitted.
  void shutdown();
 ...
 private:
  ...
  /**
   * Checks if the job queue is empty and that no job is added in the grace
   * period determined by @c m_timeout.
   *
   * @return @c true if there's at least one job left in the queue; @c false if
   ...
   * tasks, this call will return an empty function.
   *
   * @returns A function that represents a new task. The function will be empty
   * if the queue has no job.
   */
  std::function<void()> pop();

  /**
   * Pushes a task on the the queue. If the queue is shutdown, the task will be
   * dropped, and an invalid future will be returned.
   *
   * @param front If @c true, push to the front of the queue, else push to the
   * back.
   * @param task A task to push to the front or back of the queue.
   * @param args The arguments to call the task with.
   * @returns A @c std::future to access the return value of the task. If the
   * queue is shutdown, the task will be dropped, and an invalid future will be
   * returned.
   */
  template <typename Task, typename... Args>
  auto pushTo(bool front, Task task, Args&&... args)
      -> std::future<decltype(task(args...))>;
  ...
  /// Period that this queue will wait for a new job until it releases the task
  /// thread.
  ...
  /// The thread to execute tasks on. The thread must be declared last to be
  /// destructed first.
  TaskThread m_taskThread;
};

template <typename Task, typename... Args>
auto Executor::submit(Task task, Args&&... args)
    -> std::future<decltype(task(args...))> {
  bool front = false;
...
template <typename Task, typename... Args>
auto Executor::submitToFront(Task task, Args&&... args)
    -> std::future<decltype(task(args...))> {
  bool front = true;
  return pushTo(front, std::forward<Task>(task), std::forward<Args>(args)...);
}
...
/**
 * Utility function which waits for a @c std::future to be fulfilled and forward
 * the result to a @c std::promise.
 *
 * @param promise The @c std::promise to fulfill when @c future is fulfilled.
 * @param future The @c std::future on which to wait for a result to forward to
 * @c promise.
 */
template <typename T>
inline static void forwardPromise(std::shared_ptr<std::promise<T>> promise,
                                  std::future<T>* future) {
 ...
 *
 * @param promise The @c std::promise to fulfill when @c future is fulfilled.
 * @param future The @c std::future on which to wait before fulfilling @c
 * promise.
 */
template <>
inline void forwardPromise<void>(std::shared_ptr<std::promise<void>> promise,
...
template <typename Task, typename... Args>
auto Executor::pushTo(bool front, Task task, Args&&... args)
    -> std::future<decltype(task(args...))> {
   ...
   * holding onto resources that need to be freed (through a std::shared_ptr for
   * example).  If the user needs to wait for those resources to be freed they
   * have no way of knowing how long to wait.  The translated_task lambda below
   * is a workaround for this limitation.  It executes the packaged task, then
   * disposes of it before passing the task's return value back to the future
  ...
  // Create a promise/future that we will fulfill when we have cleaned up the
  // task.
  auto cleanupPromise =
      std::make_shared<std::promise<decltype(task(args...))>>();
...

  {
    ...
    if (!m_shutdown) {
      ...
      m_queue.emplace(front ? m_queue.begin() : m_queue.end(),
                      std::move(translated_task));
    } else {
