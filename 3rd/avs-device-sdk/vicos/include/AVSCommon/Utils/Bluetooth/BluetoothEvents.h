...
class BluetoothEvent {
 public:
  ...
  /**
   * Get @c BluetoothDeviceInterface associated with the event. The value
   ...
   */
  std::shared_ptr<avsCommon::sdkInterfaces::bluetooth::services::AVRCPCommand>
  getAVRCPCommand() const;
 ...
 protected:
  ...
  BluetoothEvent(
      ...
      std::shared_ptr<
          avsCommon::sdkInterfaces::bluetooth::services::AVRCPCommand>
          avrcpCommand = nullptr);
...
inline BluetoothEvent::BluetoothEvent(
    ...
    std::shared_ptr<
        ...
        device,
    avsCommon::sdkInterfaces::bluetooth::DeviceState deviceState,
    MediaStreamingState mediaStreamingState, std::shared_ptr<A2DPRole> a2dpRole,
    std::shared_ptr<avsCommon::sdkInterfaces::bluetooth::services::AVRCPCommand>
        avrcpCommand)
    : m_type{type},
      m_device{device},
...
inline BluetoothEventType BluetoothEvent::getType() const { return m_type; }

inline std::shared_ptr<
    avsCommon::sdkInterfaces::bluetooth::BluetoothDeviceInterface>
BluetoothEvent::getDevice() const {
  return m_device;
}

inline avsCommon::sdkInterfaces::bluetooth::DeviceState
BluetoothEvent::getDeviceState() const {
...
inline DeviceDiscoveredEvent::DeviceDiscoveredEvent(
    const std::shared_ptr<
        avsCommon::sdkInterfaces::bluetooth::BluetoothDeviceInterface>& device)
    : BluetoothEvent(BluetoothEventType::DEVICE_DISCOVERED, device,
                     avsCommon::sdkInterfaces::bluetooth::DeviceState::IDLE,
                     MediaStreamingState::IDLE) {}

/**
 * Event indicating that a device is removed from the underlying stack, if
 * applicable. This signifies that the stack is no longer aware of the device.
 * For example, if the stack forgets an unpaired device. This must be sent with
 * a reference to the @c BluetoothDeviceInterface.
 */
...
/**
 * Event indicating that a device undergoes a state transition. The available
 * states are dictated by @c DeviceState. This must be sent with a reference to
 * the @c BluetoothDeviceInterface.
 */
...
/**
 * Event indicating that a device's streaming state has changed. This refers to
 * the A2DP profile. This must be sent on transitions between @c
 * MediaStreamingState.
 */
class MediaStreamingStateChangedEvent : public BluetoothEvent {
 public:
   ...
   * @param device The peer device that is connected and whose streaming status
   * changed
   */
  explicit MediaStreamingStateChangedEvent(
      MediaStreamingState newState, A2DPRole role,
      std::shared_ptr<
          avsCommon::sdkInterfaces::bluetooth::BluetoothDeviceInterface>
          device);
};
...
inline MediaStreamingStateChangedEvent::MediaStreamingStateChangedEvent(
    ...
    std::shared_ptr<
        avsCommon::sdkInterfaces::bluetooth::BluetoothDeviceInterface>
        device)
    : BluetoothEvent(BluetoothEventType::STREAMING_STATE_CHANGED, device,
                     avsCommon::sdkInterfaces::bluetooth::DeviceState::IDLE,
                     newState, std::make_shared<A2DPRole>(role)){};
...
inline AVRCPCommandReceivedEvent::AVRCPCommandReceivedEvent(
    avsCommon::sdkInterfaces::bluetooth::services::AVRCPCommand command)
    : BluetoothEvent(
          BluetoothEventType::AVRCP_COMMAND_RECEIVED, nullptr,
          avsCommon::sdkInterfaces::bluetooth::DeviceState::IDLE,
          MediaStreamingState::IDLE, nullptr,
          std::make_shared<
              avsCommon::sdkInterfaces::bluetooth::services::AVRCPCommand>(
              command)) {}
