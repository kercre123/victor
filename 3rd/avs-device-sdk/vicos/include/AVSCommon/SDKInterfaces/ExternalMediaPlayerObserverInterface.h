...
inline ObservableSessionProperties::ObservableSessionProperties()
...
inline bool operator==(
  ...
  return observableSessionPropertiesA.loggedIn ==
             observableSessionPropertiesB.loggedIn &&
         observableSessionPropertiesA.userName ==
             observableSessionPropertiesB.userName;
}
...
struct ObservablePlaybackStateProperties {
   ...
   * @param pTrackName
   */
  ObservablePlaybackStateProperties(const std::string& state,
...
inline ObservablePlaybackStateProperties::ObservablePlaybackStateProperties(
    const std::string& state, const std::string& trackName)
    : state{state}, trackName{trackName} {};

inline bool operator==(const ObservablePlaybackStateProperties& observableA,
                       const ObservablePlaybackStateProperties& observableB) {
  return observableA.state == observableB.state &&
         observableA.trackName == observableB.trackName;
}
 ...
 public:
  ...
  virtual void onLoginStateProvided(
      ...
      const avsCommon::sdkInterfaces::externalMediaPlayer::
          ObservableSessionProperties sessionStateProperties) = 0;
  ...
  virtual void onPlaybackStateProvided(
      const std::string& playerId,
      const avsCommon::sdkInterfaces::externalMediaPlayer::
          ObservablePlaybackStateProperties playbackStateProperties) = 0;
};
