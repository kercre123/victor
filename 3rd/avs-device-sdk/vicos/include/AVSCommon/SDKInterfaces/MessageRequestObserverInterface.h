...
/**
 * An interface class which allows a derived class to observe a MessageRequest
 * implementation.
 */
class MessageRequestObserverInterface {
 public:
  /**
   * This enum expresses the various end-states that a send request could arrive
   * at.
   */
  enum class Status {
    ...
    /// The send failed due to an internal error on the server which sends code
    /// 500.
    ...
    /// The send failed due to server canceling it before the transmission
    /// completed.
    CANCELED,
  ...
  virtual void onSendCompleted(
      MessageRequestObserverInterface::Status status) = 0;
...
/**
 * Write a @c MessageRequestObserverInterface::Status value to an @c ostream as
 * a string.
 *
...
inline std::ostream& operator<<(
    std::ostream& stream, MessageRequestObserverInterface::Status status) {
  switch (status) {
