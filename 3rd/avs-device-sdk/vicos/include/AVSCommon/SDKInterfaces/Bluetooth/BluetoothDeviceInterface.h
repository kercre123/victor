...
/**
 * Overload for the @c DeviceState enum. This will write the @c DeviceState as a
 * string to the provided stream.
 *
...
class BluetoothDeviceInterface {
 public:
  ...
  virtual std::vector<std::shared_ptr<services::SDPRecordInterface>>
  getSupportedServices() = 0;

  // TODO : Generic getService method.
  /// @return A pointer to an instance of the @c A2DPSourceInterface if
  /// supported, else a nullptr.
  virtual std::shared_ptr<services::A2DPSourceInterface> getA2DPSource() = 0;

  /// @return A pointer to an instance of the @c A2DPSinkInterface if supported,
  /// else a nullptr.
  virtual std::shared_ptr<services::A2DPSinkInterface> getA2DPSink() = 0;

  /// @return A pointer to an instance of the @c AVRCPTargetInterface if
  /// supported, else a nullptr.
  virtual std::shared_ptr<services::AVRCPTargetInterface> getAVRCPTarget() = 0;

  /// @return A pointer to an instance of the @c AVRCPControllerInterface if
  /// supported, else a nullptr.
  virtual std::shared_ptr<services::AVRCPControllerInterface>
  getAVRCPController() = 0;
};
