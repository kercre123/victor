...
/// Used to implement an instance of AVRCPController (Audio/Video Remote Control
/// Profile).
class AVRCPControllerInterface : public BluetoothServiceInterface {
 public:
  /**
   * The Service UUID.
   * 110e is the legacy UUID used as both the identifier for the AVRCP Profile
   * as well as the AVRCP Controller service before v1.3. 110f is the UUID used
   * for AVRCP Controller service in newer versions of AVRCP. However, the 110e
   * record must always be present, in later versions of AVRCP for backwards
   * compabitibility. We will use 110e as the identifying record.
   */
