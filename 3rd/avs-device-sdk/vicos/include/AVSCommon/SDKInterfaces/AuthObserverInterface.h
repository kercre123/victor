...
class AuthObserverInterface {
 public:
  ...
  enum class Error {
    ...
    INVALID_VALUE,
    /// The authorization code is invalid, expired, revoked, or was issued to a
    /// different client.
    ...
    /// Client should slow down in the rate of requests polling for an access
    /// token.
    SLOW_DOWN,
   ...
   *
   * @note Implementations of this method must not call AuthDelegate methods
   * because the AuthDelegate may be in a 'locked' state at the time this call
   * is made.
   *
