...
/**
 * This is a wrapper class which allows a client to send a Message to AVS, and
 * be notified when the attempt to send the Message was completed.
 */
class MessageRequest {
 public:
   ...
   * @param uriPathExtension An optional uri path extension which will be
   * appended to the base url of the AVS. endpoint.  If not specified, the
   * default AVS path extension should be used by the sender implementation.
   */
  ...
  /**
   * Adds an attachment reader to the message. The attachment data will be the
   * next part in the message to be sent to AVS.
   ...
   * @param attachmentReader The attachment data to be sent to AVS along with
   * the message.
   */
  void addAttachmentReader(
      const std::string& name,
  ...
  /**
   * This is called once the send request has completed.  The status parameter
  ...
  /**
   * This function will be called if AVS responds with an exception message to
   * this message request being sent.
   *
   ...
   */
  void addObserver(
      std::shared_ptr<avsCommon::sdkInterfaces::MessageRequestObserverInterface>
          observer);
  ...
  /**
   * A function to evaluate if the given status reflects receipt of the message
   * by the server.
   *
   * @param status The status being queried.
   * @return Whether the status reflects receipt of the message by the server.
   */
  static bool isServerStatus(
      sdkInterfaces::MessageRequestObserverInterface::Status status);
