...
/**
 * Class creates an ExceptionEncountered event and sends to AVS using @c
 * MessageSenderInterface.
 */
class ExceptionEncounteredSender
    : public avsCommon::sdkInterfaces::ExceptionEncounteredSenderInterface {
 public:
  /**
   * Creates a new @c ExceptionEncounteredSender instance.
   *
   * @param messageSender The object to use for sending events.
   * @return A @c std::unique_ptr to the new @c ExceptionEncounteredSender
   * instance.
   */
  ...
  /**
   * This function asks the @c ExceptionEncounteredSender to send a
   * ExceptionEncountered event to AVS when the client is unable to execute a
   * directive.
   *
   * @param unparsedDirective The directive which the client was unable to
   * execute, must be sent to AVS
   * @param error The @c ExceptionErrorType to be sent to AVS
   * @param errorDescription The error details to be sent for logging and
   * troubleshooting.
   */
  void sendExceptionEncountered(const std::string& unparsedDirective,
