 ...
 * following an in-process memory management model.
 *
...
class InProcessAttachmentReader : public AttachmentReader {
 public:
   ...
   * @param index If being constructed from an existing @c SharedDataStream, the
   * index indicates where to read from.
   * @param reference The position in the stream @c offset is applied to.  This
   * parameter defaults to 0, indicating no offset from the specified reference.
   * @param resetOnOverrun If overrun is detected on @c read, whether to close
   * the attachment (default behavior) or to reset the read position to where
   * current write position is (and skip all the bytes in between).
   * @return Returns a new InProcessAttachmentReader, or nullptr if the
   * operation failed.  This parameter defaults to @c ABSOLUTE, indicating
   * offset is relative to the very beginning of the Attachment.
   */
  static std::unique_ptr<InProcessAttachmentReader> create(
      SDSTypeReader::Policy policy, std::shared_ptr<SDSType> sds,
  ...
  void close(ClosePoint closePoint =
                 ClosePoint::AFTER_DRAINING_CURRENT_BUFFER) override;
 ...
 private:
  ...
  InProcessAttachmentReader(SDSTypeReader::Policy policy,
                            std::shared_ptr<SDSType> sds, bool resetOnOverrun);

  /// The underlying @c SharedDataStream reader.
  std::shared_ptr<SDSTypeReader> m_reader;

  // On @c read overrun, Whether to close the attachment, or reset it to catch
  // up with the write
