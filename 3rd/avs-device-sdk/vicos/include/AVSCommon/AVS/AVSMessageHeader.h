...
/**
 * The AVS message header, which contains the common fields required for an AVS
 * message.
 */
class AVSMessageHeader {
 public:
   ...
   * @param avsDialogRequestId The dialog request ID of an AVS message, which is
   * optional.
   */
  AVSMessageHeader(const std::string& avsNamespace, const std::string& avsName,
  ...
  /**
   * Returns the name in an AVS message, which describes the intent of the
   * message.
   *
   * @return The name.
   */
 ...
 private:
  ...
  /// Name within the namespace of the AVSMessage, which describes the intent of
  /// the message.
