...
class PlaylistParser
 ...
 public:
   ...
   * @return An @c std::unique_ptr to a new @c PlaylistParser if successful or
   * @c nullptr otherwise.
   */
  ...
  int parsePlaylist(
      std::string url,
      std::shared_ptr<
          avsCommon::utils::playlistParser::PlaylistParserObserverInterface>
          observer,
      std::vector<PlaylistType> playlistTypesToNotBeParsed =
          std::vector<PlaylistType>()) override;
 ...
 private:
  ...
  /**
   * Parses the playlist pointed to by the url specified in a depth first search
   * manner.
   *
