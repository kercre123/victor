...
struct PlayItem {
   ...
   * If PlayItem is information about media to play, then type is set to
   * MEDIA_INFO.
   */
...
struct M3UContent {
   ...
   */
  M3UContent(const std::vector<avsCommon::utils::playlistParser::PlaylistEntry>
                 entries,
             bool isLive);
...
/**
 * Parses an M3U playlist and returns the "children" URLs in the order they
 ...
 * @return @c M3UContent which contains parsed list of variant URLs (for master
 * playlist) OR list of @ Playlist (for media playlist).
 */
M3UContent parseM3UContent(const std::string& playlistURL,
...
avsCommon::utils::playlistParser::EncryptionInfo parseHLSEncryptionLine(
    const std::string& line, const std::string& baseURL);
 ...
 */
avsCommon::utils::playlistParser::ByteRange parseHLSByteRangeLine(
 ...
 */
avsCommon::utils::playlistParser::PlaylistEntry parseHLSMapLine(
    const std::string& line, const std::string& baseURL);
