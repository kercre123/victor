...
/**
 * CapabilitiesDelegate provides an implementation of the
 * CapabilitiesDelegateInterface. It allows clients to register capabilities
 * implemented by agents and publish them so that Alexa is aware of the device's
 * capabilities.
 */
class CapabilitiesDelegate
 ...
 public:
   ...
   *
   * @param authDelegate The auth delegate instance needed for
   * CapabilitiesDelegate.
   * @param miscStorage The miscDB instance needed for CapabilitiesDelegate.
   * @param httpPut The HTTP PUT handler instance needed for
   * CapabilitiesDelegate.
   ...
   * @return If successful, returns a new CapabilitiesDelegate, otherwise @c
   * nullptr.
   */
  static std::shared_ptr<CapabilitiesDelegate> create(
      ...
      const std::shared_ptr<
          avsCommon::sdkInterfaces::storage::MiscStorageInterface>& miscStorage,
      const std::shared_ptr<avsCommon::utils::libcurlUtils::HttpPutInterface>&
          httpPut,
  ...
  bool registerCapability(
      const std::shared_ptr<
          avsCommon::sdkInterfaces::CapabilityConfigurationInterface>&
  ...
  void addCapabilitiesObserver(
      std::shared_ptr<avsCommon::sdkInterfaces::CapabilitiesObserverInterface>
          observer) override;
  ...
  /// @{
  void onAuthStateChange(AuthObserverInterface::State newState,
                         AuthObserverInterface::Error newError) override;
  /// @}
 ...
 private:
   ...
   * CapabilitiesDelegate instance so that clients only get access to fully
   * formed instances.
   *
   * @param configurationRoot The global config object.
   * @return @c true if initialization is successful.
   */
  bool init(const avsCommon::utils::configuration::ConfigurationNode&
                configurationRoot);

  /**
   * Returns if a capability (given its configuration map) has been registered.
   * You need to have the capability registry mutex locked when calling this
   * function.
   *
   * @param capabilityMap The capability configuration map of a specific
   * capability.
   * @return true if a capability has been registered, else false.
   */
  bool isCapabilityRegisteredLocked(
  ...
  /**
   * Looks for an override message in the config first and returns that if
   * available. If not available, then it will construct a message from the
  ...
  /**
   * Gets an auth token from the authDelegate instance available to
   * CapabilitiesDelegate. This will wait till the auth token is available.
   *
  ...
  /**
   * Returns whether the data/metadata is different from the previous successful
   * message.
   *
   * @return true if the data/metadata is different from the previous successful
   * message, else false.
   */
  bool isCapabilitiesPublishDataDifferent();

  /**
   * Returns whether the Capabilities API message's capabilities is different
   * from the previous successful message.
   *
   * @return true if the Capabilities API message's capabilities is different
   * from the previous successful message, else false.
   */
   ...
   *
   * @return true if the Capabilities API data was successfully saved, else
   * false.
   */
   ...
   * function.
   *
   * @return The Capabilities API message constructed from the registered
   * capabilities.
   */
  std::string
  getCapabilitiesPublishMessageBodyFromRegisteredCapabilitiesLocked();

  /**
   * Construct a Capabilities API message from the override message provided in
   * the config.
   *
   * @return The Capabilities API message constructed from the override message
  ...
  /// Mutex used to serialize access to Capabilities state and Capabilities
  /// state observers.
  std::mutex m_capabilitiesMutex;

  /// Authorization state change observers. Access is synchronized with @c
  /// m_capabilitiesMutex.
  std::unordered_set<
      std::shared_ptr<avsCommon::sdkInterfaces::CapabilitiesObserverInterface>>
      m_capabilitiesObservers;

  /// Current state of CapabilitiesDelegate. Access is synchronized with @c
  /// m_capabilitiesMutex.
  avsCommon::sdkInterfaces::CapabilitiesObserverInterface::State
      m_capabilitiesState;

  /// Current CapabilitiesDelegate error. Access is synchronized with @c
  /// m_capabilitiesMutex.
  avsCommon::sdkInterfaces::CapabilitiesObserverInterface::Error
      m_capabilitiesError;
  ...
  /// A map of the capability key (consisting of the interface type and name)
  /// and the capability config map.
  std::unordered_map<std::string,
                     std::shared_ptr<avsCommon::avs::CapabilityConfiguration>>
      m_capabilityConfigs;

  /// Map of the capability key (consisting of the interface type and name) and
  /// the capability config map registered by capabilties.
  std::unordered_map<std::string,
                     std::shared_ptr<avsCommon::avs::CapabilityConfiguration>>
  ...
  /// Map of previous capabilities. This is a map of the capability key
  /// (consisting of the interface type and name) and the capability config map
  std::unordered_map<std::string,
                     std::shared_ptr<avsCommon::avs::CapabilityConfiguration>>
  ...
  /// Used to let CapabilitiesDelegate know that it no longer needs to wait to
  /// publish.
  std::condition_variable m_publishWaitDone;

  /// An internal executor that performs execution of callable objects passed to
  /// it sequentially but asynchronously.
