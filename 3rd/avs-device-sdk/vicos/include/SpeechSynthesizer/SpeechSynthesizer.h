...
class SpeechSynthesizer
 ...
 public:
  ...
  using SpeechSynthesizerObserverInterface =
      avsCommon::sdkInterfaces::SpeechSynthesizerObserverInterface;

  /**
   * Create a new @c SpeechSynthesizer instance.
   *
   * @param mediaPlayer The instance of the @c MediaPlayerInterface used to play
   * audio.
   * @param messageSender The instance of the @c MessageSenderInterface used to
   * send events to AVS.
   * @param focusManager The instance of the @c FocusManagerInterface used to
   * acquire focus of a channel.
   * @param contextManager The instance of the @c ContextObserverInterface to
   * use to set the context of the @c SpeechSynthesizer.
   * @param exceptionSender The instance of the @c
   * ExceptionEncounteredSenderInterface to use to notify AVS when a directive
   * cannot be processed.
   *
   * @return Returns a new @c SpeechSynthesizer, or @c nullptr if the operation
   * failed.
   */
   ...
   */
  void addObserver(
      std::shared_ptr<SpeechSynthesizerObserverInterface> observer);
   ...
   * SpeechSynthesizerObserverInterface::onStateChanged() will result in a
   * deadlock.
   *
  ...
  void onPlaybackError(SourceId id,
 ...
 private:
  ...
  class SpeakDirectiveInfo {
   public:
    ...
    /// A flag to indicate if the directive complete message has to be sent to
    /// the @c DirectiveSequencer.
    bool sendCompletedMessage;

    /// A flag to indicate if setFailed() has been sent to the @c
    /// DirectiveSequencer.
   ...
   * acquire focus of a channel.
   * @param contextManager The instance of the @c ContextObserverInterface to
   * use to set the context of the SpeechSynthesizer.
   * @param exceptionSender The instance of the @c
   * ExceptionEncounteredSenderInterface to use to notify AVS when a directive
   * cannot be processed.
   */
   ...
   * Adds the @c SpeechSynthesizer as a state provider with the @c
   * ContextManager.
   */
  void init();

  /**
   * Handle a SpeechSynthesizer.Speak directive (on the @c m_executor thread)
   * immediately. Starts playing the speech associated with a directive.
   *
   * @param info The directive to handle immediately. The result object is a @c
   * nullptr.
   */
  void executeHandleImmediately(std::shared_ptr<DirectiveInfo> info);

  /**
   * Pre-handle a SpeechSynthesizer.Speak directive (on the @c m_executor
   * thread). Starts any caching of attachment data for playing the speech
   * associated with the directive. This needs to be called after
   * @c DirectiveInfo, the @c AVSDirective and @c
   * DirectiveHandlerResultInterface have been checked to be valid.
   * @c DirectiveHandlerResultInterface is not needed for @c
   * executeHandleImmediately.
   *
   * @param speakInfo The directive to pre-handle and the associated data.
   */
  void executePreHandleAfterValidation(
      std::shared_ptr<SpeakDirectiveInfo> speakInfo);

  /**
   * Handle a SpeechSynthesizer.Speak directive (on the @c m_executor thread).
   * This starts a request for the foreground focus. This needs to be called
   * after @c DirectiveInfo, the @c AVSDirective and
   * @c DirectiveHandlerResultInterface have been checked to be valid. @c
   * DirectiveHandlerResultInterface is not needed for @c
   * executeHandleImmediately.
   *
   * @param speakInfo The directive to handle and the associated data.
   */
  void executeHandleAfterValidation(
      std::shared_ptr<SpeakDirectiveInfo> speakInfo);
   ...
   *
   * @param info The directive to preHandle and the result object with which to
  ...
  /**
   * Handle a SpeechSynthesizer.Speak directive (on the @c m_executor thread).
   * This starts a request for the foreground focus.
   *
   * @param info The directive to handle and the result object with which to
   * communicate the result.
   */
  void executeHandle(std::shared_ptr<DirectiveInfo> info);

  /**
   * Cancel execution of a SpeechSynthesizer.Speak directive (on the @c
   * m_executor thread).
   *
  ...
  /**
   * Execute a change of state (on the @c m_executor thread). If the @c
   * m_desiredState is @c PLAYING, playing the audio of the current directive is
   * started. If the @c m_desiredState is @c FINISHED this method triggers
   * termination of playing the audio.
   */
  void executeStateChange();

  /**
   * Request to provide an update of the SpeechSynthesizer's state to the
   * ContextManager (on the @c m_executor thread).
   *
  ...
  /**
   * Handle (on the @c m_executor thread) notification that speech playback has
   * started.
   */
  ...
  /**
   * Handle (on the @c m_executor thread) notification that speech playback
   * encountered an error.
   *
   * @param error Text describing the nature of the error.
   */
  void executePlaybackError(
      const avsCommon::utils::mediaPlayer::ErrorType& type, std::string error);
   ...
   * @param offsetInMilliseconds The current offset of text to speech in
   * milliseconds.
   * @return The JSON state string.
   */
  std::string buildState(std::string& token,
   ...
   * @c ContextManager with the new state and send an event with the updated
   * state to AVS where applicable.
   * @c m_mutex must be acquired before calling this function. All observers
   * will be notified of a new state regardless of @c provideState value.
   *
   * @param newState The new state of the @c SpeechSynthesizer.
   * @param provideState If true ContextManager will be updated with the new
   * state.
   */
  ...
  /**
   * Set the desired state the @c SpeechSynthesizer needs to transition to based
   * on the @c newFocus.
  ...
  /**
   * Reset @c m_currentInfo, cleaning up any @c SpeechSynthesizer resources and
   * removing from CapabilityAgent's map of active @c AVSDirectives.
   *
   * @param info The @c DirectiveInfo instance to make current (defaults to @c
   * nullptr).
   */
  void resetCurrentInfo(std::shared_ptr<SpeakDirectiveInfo> info = nullptr);

  /**
   * Send the handling completed notification and clean up the resources of @c
   * m_currentInfo.
   */
  void setHandlingCompleted();

  /**
   * Send the handling failed notification and clean up the resources of @c
   * m_currentInfo.
   *
   * @param description
   */
  ...
  void sendExceptionEncounteredAndReportFailed(
      ...
      avsCommon::avs::ExceptionErrorType type, const std::string& message);

  /**
   * Send ExceptionEncountered because a required property was not in the @c
   * AVSDirective's payload.
   *
   ...
   * @param checkResult Check if the @c DirectiveHandlerResultInterface is not a
   * @c nullptr in the @c DirectiveInfo.
  ...
  /**
   * Find the @c SpeakDirectiveInfo instance (if any) for the specified @c
   * messageId.
   *
   ...
   */
  std::shared_ptr<SpeakDirectiveInfo> getSpeakDirectiveInfo(
      const std::string& messageId);

  /**
   * Checks if the @c messageId is already present in the map. If its not
   * present, adds an entry to the map.
   * @param messageId The @c messageId value to add to the @c
   * m_speakDirectiveInfoMap
   * @param speakDirectiveInfo The @c SpeakDirectiveInfo corresponding to the @c
   * messageId to add.
   * @return @c true if @c messageId to @c SpeakDirectiveInfo mapping was added.
   * @c false if entry already exists for the @c messageId.
   */
  bool setSpeakDirectiveInfo(
      const std::string& messageId,
      std::shared_ptr<SpeechSynthesizer::SpeakDirectiveInfo>
          speakDirectiveInfo);

  /**
   * Adds the @c SpeakDirectiveInfo to the @c m_speakInfoQueue. If the entry is
   * being added to an empty queue, it initiates @c executeHandle on the @c
   * SpeakDirectiveInfo.
   * @param speakInfo The @c SpeakDirectiveInfo to add to the @c
   * m_speakInfoQueue.
   */
  void addToDirectiveQueue(std::shared_ptr<SpeakDirectiveInfo> speakInfo);

  /**
   * Removes the @c SpeakDirectiveInfo corresponding to the @c messageId from
   * the @c m_speakDirectiveInfoMap.
   *
  ...
  /**
   * Reset the @c m_mediaSourceId once the @c MediaPlayer finishes with playback
   * or when @c MediaPlayer returns an error.
   */
  void resetMediaSourceId();

  /**
   * Id to identify the specific source when making calls to
   * MediaPlayerInterface. If this is modified or retrieved from methods that
   * are not protected by the executor then additional locking will be needed.
   */
  ...
  /// The set of @c SpeechSynthesizerObserverInterface instances to notify of
  /// state changes.
  std::unordered_set<std::shared_ptr<SpeechSynthesizerObserverInterface>>
      m_observers;

  /**
   * The current state of the @c SpeechSynthesizer. @c m_mutex must be acquired
   * before reading or writing the
   * @c m_currentState.
   */
  SpeechSynthesizerObserverInterface::SpeechSynthesizerState m_currentState;

  /**
   * The state the @c SpeechSynthesizer must transition to. @c m_mutex must be
   * acquired before reading or writing the @c m_desiredState.
   */
  ...
  /// @c SpeakDirectiveInfo instance for the @c AVSDirective currently being
  /// handled.
  std::shared_ptr<SpeakDirectiveInfo> m_currentInfo;

  /// Mutex to serialize access to m_currentState, m_desiredState, and
  /// m_waitOnStateChange.
  std::mutex m_mutex;

  /// A flag to keep track of if @c SpeechSynthesizer has called @c Stop()
  /// already or not.
  bool m_isAlreadyStopping;

  /// Condition variable to wake @c onFocusChanged() once the state transition
  /// to desired state is complete.
  std::condition_variable m_waitOnStateChange;

  /// Map of message Id to @c SpeakDirectiveInfo.
  std::unordered_map<std::string, std::shared_ptr<SpeakDirectiveInfo>>
      m_speakDirectiveInfoMap;

  /**
   * Mutex to protect @c messageId to @c SpeakDirectiveInfo mapping. This mutex
   * must be acquired before accessing or modifying the m_speakDirectiveInfoMap
   */
   ...
   *
   * @note This declaration needs to come *after* any variables used by the
   * executor thread so that the thread shuts down before the variables are
   * destroyed.
   */
