...
class EqualizerCapabilityAgent
    : public std::enable_shared_from_this<EqualizerCapabilityAgent>,
      public avsCommon::sdkInterfaces::audio::
 ...
 public:
   ...
   *
   * @param equalizerController An SDK's component to control equalizer state
   ...
   * @return A new instance of @c EqualizerCapabilityAgent on success, @c
   * nullptr otherwise.
   */
  static std::shared_ptr<EqualizerCapabilityAgent> create(
      std::shared_ptr<alexaClientSDK::equalizer::EqualizerController>
          equalizerController,
 ...
 private:
  ...
  /**
   * Prepares EqualizerController Interface DCF configuration and keeps it
   * internally.
   */
  void generateCapabilityConfiguration();

  /**
   * Performs an attempt to fix the DCF configuration desynchronization. This is
   * introduced to handle the case when AVS EqualizerController API directive
   * contains a band or mode that is unsupported according to current
   * configuration. We assume two options:
   * - AVS bug
   * - AVS and SDK has different versions of the DCF configuration for whatever
   * reason. Here we tray to fix the second case.
   */
  ...
  /// An instance of @c CapabilitiesDelegate to invalidate DCF configuration if
  /// required.
  std::shared_ptr<avsCommon::sdkInterfaces::CapabilitiesDelegateInterface>
