...
class ExchangeHandlerContextInterface {
 public:
  ...
  /**
   * Notification that the downchannel has failed to be established or has
   * disconnected.
   */
  ...
  /**
   * Notification that sending a @c MessageRequest has failed or been
   * acknowledged by AVS (this is used to indicate it is okay to send the next
   * message).
   */
  ...
  /**
   * Notification that sending a ping to AVS has failed or been acknowledged by
   * AVS.
   *
   ...
   *
   * @param authToken The auth token used for the forbidden request or an empty
   ...
   * @return The new instance of HTTP2RequestInterface, or nullptr if the
   * operation failed.
   */
  virtual std::shared_ptr<avsCommon::utils::http2::HTTP2RequestInterface>
