 ...
 *
 * This includes forwarding json payloads to an @c MessageConsumer, attachments
 * to attachment writers, and capturing exceptions for non 2xx results.
 */
class MimeResponseSink
    : public avsCommon::utils::http2::HTTP2MimeResponseSinkInterface {
 public:
  /**
   * Constructor.
   *
   ...
   * @param attachmentContextId Id added to content IDs to assure global
   * uniqueness.
   */
  MimeResponseSink(
  ...
  bool onBeginMimePart(
      const std::multimap<std::string, std::string>& headers) override;
  avsCommon::utils::http2::HTTP2ReceiveDataStatus onReceiveMimeData(
      const char* bytes, size_t size) override;
  bool onEndMimePart() override;
  avsCommon::utils::http2::HTTP2ReceiveDataStatus onReceiveNonMimeData(
      const char* bytes, size_t size) override;
  void onResponseFinished(
      avsCommon::utils::http2::HTTP2ResponseFinishedStatus status) override;
  /// @}
 ...
 private:
   ...
   */
  avsCommon::utils::http2::HTTP2ReceiveDataStatus writeToAttachment(
  ...
  /**
   * The directive message being received from AVS by this stream.  It may be
   * built up over several calls if either the write quantums are small, or if
   * the message is long.
   */
  std::string m_directiveBeingReceived;

  /**
   * The attachment id of the attachment currently being processed.  This
   * variable is needed to prevent duplicate creation of @c Attachment objects
   * when data is re-driven.
   */
  std::string m_attachmentIdBeingReceived;

  /// The current AttachmentWriter.
  std::unique_ptr<avsCommon::avs::attachment::AttachmentWriter>
      m_attachmentWriter;

  /// Non-mime response body acculumulated for response codes other than
