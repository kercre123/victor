...
class DirectiveSequencer
 ...
 public:
   ...
   *
   * @param exceptionSender An instance of the @c
   * ExceptionEncounteredSenderInterface used to send ExceptionEncountered
   * messages to AVS for directives that are not handled.
   * @return Returns a new DirectiveSequencer, or nullptr if the operation
   * failed.
   */
 ...
 private:
   ...
   * m_mutex.
   *
   * @param lock A @c unique_lock on m_mutex, allowing this method to release
   * the lock around callbacks that need to be invoked.
   */
  void receiveDirectiveLocked(std::unique_lock<std::mutex>& lock);

  /// Serializes access to data members (besides m_directiveRouter and
  /// m_directiveProcessor).
  std::mutex m_mutex;
