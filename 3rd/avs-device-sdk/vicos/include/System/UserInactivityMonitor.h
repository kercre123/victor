...
/// This class implements a @c CapabilityAgent that handles the @c SetEndpoint
/// directive.
class UserInactivityMonitor
 ...
 private:
  ...
  /// Send inactivity report by comparing to the last time active. We will
  /// register this function with the timer.
  ...
  /**
   * Utility function to notify our inactivityObservers that the
  ...
  /// Mutex to synchronize access to @c m_lastTimeActive and @c
  /// m_inactivityObservers.
  std::mutex m_mutex;

  /**
   * Timer for sending events every hour.
   * Declared after @c m_mutex so m_eventTimer does not access @c m_mutex after
   * it has been destroyed.
   */
  avsCommon::utils::timing::Timer m_eventTimer;

  /// Flag for notifying that @c onUserActive was not able to update @c
  /// m_lastTimeActive.
  std::atomic_bool m_recentUpdateBlocked;

  /**
   * Time point to keep user inactivity. Access synchronized by @c m_timeMutex,
   * and blocks tracked by @c m_recentUpdateBlocked.
   */
  std::chrono::time_point<std::chrono::steady_clock> m_lastTimeActive;

  /// Observers to be notified when the System.UserInactivityReport Event has
  /// been sent.
