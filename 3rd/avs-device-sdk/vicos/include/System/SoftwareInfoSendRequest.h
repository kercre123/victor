 ...
 *
 * @note If the event fails with SERVER_INTERNAL_ERROR_V2, sending the event is
 * retried until it succeeds or the request is cancelled.
 */
class SoftwareInfoSendRequest
 ...
 public:
   ...
   * @return The newly created instance of InfoSendRequest, or nullptr if the
   * operation failed.
   */
 ...
 private:
  ...
  /**
   * A promise to fulfill with the send status when the send operation
   * completes.
  ...
  /**
   * Timers used to schedule retries when a send attempt results in the
   * SERVER_INTERNAL_ERROR_V2 status.
   * @note A @c Timer's task may not be used to @c re-start() its triggering @
   * Timer.  To get around this limitation, two timers are used here such that a
   * failed retry triggered by one of the @c Timers will use the other @c Timer
   * to specify the delay before the next retry.
   * TODO: ACSDK-841 Enhance the Timer class to allow re-start() of a timer from
   * the Timer's task
   * @note Access to this member is serialized with @c m_mutex.
   */
