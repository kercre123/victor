...
/**
 * @c SoftwareInfoSender is a @c CapabilityAgent that handles the @c
 * System.ReportSoftwareInfo directive and the sending of @c System.SoftwareInfo
 * events to AVS.
 *
 * @see
 * https://developer.amazon.com/docs/alexa-voice-service/system.html#reportsoftwareinfo-directive
 * @see
...
class SoftwareInfoSender
 ...
 public:
   ...
   *
   * @param firmwareVersion The firmware version to include in SoftwareInfo
   * events.
   * @param sendSoftwareInfoUponConnect Whether the SoftwareInfo event should be
   * sent upon connecting.
   * @param observer An object to receive notifications from this
   * SoftwareInfoSender.
   ...
   *
   * If the firmware version changes while the client is already connected to @c
   * AVS, it should call
   * @c setFirmwareVersion() immediately with the new version.  If the version
   * is new it will trigger sending a @c SoftwareInfo event to AVS.
   *
 ...
 private:
  ...
  /// Whether to send the SoftwareInfo event upon @c CONNECTED.  Access to this
  /// member is serialized by @c m_mutex.
  bool m_sendSoftwareInfoUponConnect;

  /// Object to receive notifications from this SoftwareInfoSender.
  std::shared_ptr<avsCommon::sdkInterfaces::SoftwareInfoSenderObserverInterface>
      m_observer;
  ...
  /// The last reported connection status. Access to this member is serialized
  /// by @c m_mutex.
  avsCommon::sdkInterfaces::ConnectionStatusObserverInterface::Status
      m_connectionStatus;

  /// @c SoftwareInfoSendRequest created in response to reaching the @c
  /// CONNECTED state or in response to a call to @c setFirmwareVersion().
  /// Access to this member is serialized by @c m_mutex.
  std::shared_ptr<capabilityAgents::system::SoftwareInfoSendRequest>
      m_clientInitiatedSendRequest;

  /// @c SoftwareInfoSendRequest created in response to receiving a
  /// System.ReportSoftwareInfo directive. Access to this member is serialized
  /// by @c m_mutex.
  std::shared_ptr<capabilityAgents::system::SoftwareInfoSendRequest>
      m_handleDirectiveSendRequest;
};
