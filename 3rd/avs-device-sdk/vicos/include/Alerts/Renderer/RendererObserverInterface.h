...
class RendererObserverInterface {
 public:
  ...
  enum class State {
    ...
    STARTED,
    /// The renderer has stopped rendering due to being stopped via a direct api
    /// call.
    STOPPED,
    /// The renderer has completed rendering, if the renderer was initiated with
    /// a finite loop count.
    COMPLETED,
   ...
   * @param reason The reason for the change of state, if required.  This is
   * typically set on an error.
   */
  virtual void onRendererStateChange(State state,
