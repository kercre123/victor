 ...
 *
 * This class is decoupled from the Renderer, which is set by its owning object.
 * This class encpasulates and translates all renderer states, so that an owning
 * object need only know if the alert object is 'active', for example, rather
 * than also query rendering state.  An alert object in an 'active' state
 * implies the user perceivable rendering is occurring (whether that means
 * audible, visual, or other perceivable stimulus).
 */
class Alert : public renderer::RendererObserverInterface,
 ...
 public:
  ...
  enum class State {
    ...
    UNSET,
    /// The alert is set and is expected to become active at some point in the
    /// future.
    SET,
    /// The alert is ready to activate, and is waiting for the channel to be
    /// acquired.
    READY,
    /// Rendering has been initiated, but is not yet perceivable from a user's
    /// point of view.
    ACTIVATING,
    /// Rendering has been initiated, and is perceivable from a user's point of
    /// view.
    ACTIVE,
  ...
  /**
   * A utility structure to encapsulate the data reflecting custom assets for an
   * alert.
   */
  struct AssetConfiguration {
    ...
    /**
     * A vector of the play order of the asset ids.  AVS sends this list in its
     * SetAlert Directive, and to render the alert, all assets mapping to these
     * ids must be played in sequence.
     */
    std::vector<std::string> assetPlayOrderItems;
    /// The background asset id, if specified by AVS.
    std::string backgroundAssetId;
    /// The pause time, in milliseconds, that should be taken between each loop
    /// of asset rendering.
    std::chrono::milliseconds loopPause;
  };

  /**
   * A struct to encapsulate an alert's static data. These data members are not
   * expected to change after initialization. The purpose of this struct is to
   * provide access to data members expected to be persisted on-device.
   */
  struct StaticData {
    /**
     * Constructor.
  ...
  /**
   * A struct to encapsulate an alert's dynamic data. These data members are
   * expected to change. The purpose of this struct is to provide access to data
   * members expected to be persisted on-device.
   */
  struct DynamicData {
    /**
     * Constructor.
     */
    DynamicData()
        : state{State::SET}, loopCount{0}, hasRenderingFailed{false} {}
  ...
  /**
   * Utility struct to allow us to share Context data that can be sent to AVS
   * representing this alert.
   */
  struct ContextInfo {
    /**
     * Constructor.
     ...
     * @param scheduledTime_ISO_8601 The time, in ISO-8601 format, when this
     * alert should activate.
     */
    ContextInfo(const std::string& token, const std::string& type,
   ...
   * @return The string conversion.
   */
  static std::string parseFromJsonStatusToString(
      Alert::ParseFromJsonStatus parseFromJsonStatus);

  /**
   * Constructor.
   */
  Alert(std::function<std::unique_ptr<std::istream>()> defaultAudioFactory,
        std::function<std::unique_ptr<std::istream>()> shortAudioFactory);

  /**
   * Returns a string to identify the type of the class.  Required for
   * persistent storage.
   *
  ...
  /**
   * A function that gets a factory to create a stream to the default audio for
   * an alert.
   *
  ...
  /**
   * A function that gets a factory to create a stream to the short audio for an
   * alert.
   *
  ...
  void onRendererStateChange(renderer::RendererObserverInterface::State state,
                             const std::string& reason) override;

  /**
   * Given a rapidjson pre-parsed Value, parse the fields required for a valid
   * alert.
   *
   * @param payload The pre-parsed rapidjson::Value.
   * @param[out] errorMessage An output parameter where a parse error message
   * may be stored.
   * @return The status of the parse.
   */
  ParseFromJsonStatus parseFromJson(const rapidjson::Value& payload,
  ...
  /**
   * Set an observer on the alert.  An alert may have only one observer -
   * repeated calls to this function will replace any previous value with the
   * new one.
   *
   ...
   *
   * @note Use @c snooze for active alarms. This method will fail since it does
   * not stop alarm rendering.
   * @note The caller should validate the new schedule which should not be more
   * than 30 minutes in the past.
   * @param newScheduledTime The new time for the alarm to occur, in ISO-8601
   * format.
   ...
   *
   * @param updatedScheduledTime The new time for the alarm to occur, in
   * ISO-8601 format.
   * @return @c true if it succeeds; @c false otherwise.
   */
  ...
  /**
   * Sets the state of this alert to active.  Only has effect if the Alert's
   * state is State::ACTIVATING.
   */
  bool setStateActive();
   ...
   * @param dynamicData A pointer to a DynamicData struct. If this is not
   * nullptr, it will be populated with this alert's DynamicData.
   * @param staticData A pointer to a StaticData struct. If this is not nullptr,
   * it will be populated with this alert's StaticData.
   */
   ...
   * @param dynamicData A pointer to a DynamicData struct. If this is not
   * nullptr, this alert's DynamicData will be set.
   * @param staticData A pointer to a StaticData struct. If this is not nullptr,
   * this alert's StaticData will be set.
   ...
   *
   * @param currentUnixTime The time with which to compare the activation time
   * of the alert.
   * @param timeLimitSeconds How long an alert may be late, and still considered
   * valid.
  ...
  /**
   * Get the time, in milliseconds, to be paused between custom-asset loop
   * rendering.
   *
   * @return The time, in milliseconds, to be paused between custom-asset loop
   * rendering.
   */
  ...
  /**
   * Returns the utility structure, containing the Context data associated with
   * this alert.
   *
   * @return The utility structure, containing the Context data associated with
   * this alert.
   */
 ...
 private:
  ...
  /**
   * A utility function to be invoked when the maximum time for an alert has
   * expired.
   */
  ...
  /// The timer to ensure this alert is not active longer than a maximum length
  /// of time.
...
/**
 * A utility struct which allows alert objects to be sorted uniquely by time in
 * STL containers such as sets.
 */
struct TimeComparator {
  /**
   * Alerts may have the same time stamp, so let's include the token to ensure
   * unique and consistent ordering.
   */
  bool operator()(const std::shared_ptr<Alert>& lhs,
                  const std::shared_ptr<Alert>& rhs) const {
 ...
 * @param state The @c Alert::State value to write to the @c ostream as a
 * string.
 ...
 *
 * @param stream The stream to write the value to.
 * @param reason The @c Alert::StopReason value to write to the @c ostream as a
 * string.
 * @return The @c ostream that was passed in and written to.
 */
inline std::ostream& operator<<(std::ostream& stream,
 ...
 *
 * @param stream The stream to write the value to.
 * @param status The @c Alert::ParseFromJsonStatus value to write to the @c
 * ostream as a string.
 * @return The @c ostream that was passed in and written to.
 */
inline std::ostream& operator<<(std::ostream& stream,
