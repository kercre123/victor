...
/**
 * This class handles the management of AVS alerts.  This is essentially a
 * time-ordered queue, where a timer is set for the alert which must activate
 * soonest.  As alerts are added or removed, this timer must be reset.
 */
class AlertScheduler : public AlertObserverInterface {
 public:
   ...
   * Constructor.
   *
   * @param alertStorage The storage object where alerts can be saved, modified
   * and deleted.
   * @param alertRenderer The object which will handle user-perceivable effects
   * upon alert activation.
   * @param alertPastDueTimeLimitSeconds The threshold in seconds, beyond which
   * alerts will be considered past-due and discarded.
   */
  AlertScheduler(std::shared_ptr<storage::AlertStorageInterface> alertStorage,
                 std::shared_ptr<renderer::RendererInterface> alertRenderer,
                 std::chrono::seconds alertPastDueTimeLimitSeconds);

  void onAlertStateChange(const std::string& alertToken, State state,
                          const std::string& reason = "") override;

  /**
   * Initialization.
   *
   * @note This function must be called before other use of an object this
   * class.
   *
   * @param observer An observer which we will notify of all alert state
   * changes.
  ...
  /**
   * Snooze an active alert to re-activate at a new specified time.  The alert,
   * if active, will be de-activated and re-scheduled for the new time.
   *
   ...
   */
  bool snoozeAlert(const std::string& alertToken,
  ...
  /**
   * Delete multiple alerts from the schedule by their tokens. All existing
   * alerts are deleted with all-or-none rule. In case of failure no actual
   * deletion is made. Missing alert is not treated as an error.
   *
   * @param tokenList List of tokens of the alerts to be deleted
   * @return true if all alerts has been deleted, false if any of the deletion
   * failed.
   */
 ...
 private:
  /**
   * A handler function which will be called by our internal executor when a
   * managed alert changes state.
   *
   * @param alertToken The AVS token identifying the alert.
   * @param state The state of the alert.
   * @param reason The reason the the state changed, if applicable.
   */
  void executeOnAlertStateChange(std::string alertToken, State state,
                                 std::string reason);

  /**
   * Update an alert with the new schedule. This function cannot update an
   * active alert (use snooze instead).
   *
   ...
   */
  bool updateAlert(const std::shared_ptr<Alert>& alert,
                   const std::string& newScheduledTime);

  /**
   * A utility function which wraps the executor submission to notify our
   * observer.
   *
   * @param alertToken The AVS token identifying the alert.
   * @param state The state of the alert.
   * @param reason The reason the the state changed, if applicable.
   */
  void notifyObserver(const std::string& alertToken,
   ...
   * @param reason The reason the the state changed, if applicable.
   */
  void executeNotifyObserver(std::string alertToken,
                             AlertObserverInterface::State state,
                             std::string reason = "");

  /**
   * Utility function to set the timer for the next scheduled alert.  This
   * function requires @c m_mutex be locked.
   */
  ...
  /**
   * Utility function to activate the next scheduled alert.  This function
   * requires @c m_mutex be locked.
   */
  ...
  /**
   * Utility function to query if a given alert is active.  This function
   * requires @c m_mutex be locked.
   *
  ...
  /**
   * A utility function to retreive a scheduled alert given its token.  This
   * function requires @c m_mutex be locked.
   *
  ...
  /**
   * A utility function to deactivate the currently active alert.  This function
   * requires @c m_mutex be locked.
   *
  ...
  /**
   * Our observer.  Once initialized, this is only accessed within executor
   * functions, so does not need mutex protection.
   */
  ...
  /// The maximum time-limit in seconds for which an alert will be valid beyond
  /// its scheduled time.
