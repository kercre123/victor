 ...
 *
 * When DoNotDisturb mode is on AVS blocks some interactions from reaching the
 * device so the customer won't be disturbed. Locally SDK provides only the way
 * to get immediate state of the DND mode, track its changes coming from any
 * source and update it making sure that it will be synchronized with AVS. No
 * other customer experience is affected.
 */
class DoNotDisturbCapabilityAgent
 ...
 public:
   ...
   *
   * @param customerDataManager Component to register Capability Agent as
   * customer data container (equalizer state).
   ...
   * @param settingsManager A settingsManager object that manages do not disturb
   * setting.
   * @param settingsStorage The storage interface that will be used to store
   * device settings.
   * @return A new instance of @c DoNotDisturbCapabilityAgent on success, @c
   * nullptr otherwise.
   */
 ...
 private:
   ...
   * @param value Valid JSON string representation of the boolean value. I.e.
  ...
  sendDNDEvent(const std::string& eventName, const std::string& value);
   ...
   * @return true if operation succeeds and could be reported as such to AVS,
   * false if an error occurred. False implies that exception has been reported
   * to AVS and directive is already processed.
   */
  ...
  /**
   * Flag indicating latest reported connection status. True if SDK is connected
   * to the AVS and ready, false otherwise.
   */
  bool m_isConnected;
  ...
  /// Flag indicating whether there were changes made to the DND status while
  /// being offline.
  std::atomic_bool m_hasOfflineChanges;

  /// An executor used for serializing requests on agent's own thread of
  /// execution.
