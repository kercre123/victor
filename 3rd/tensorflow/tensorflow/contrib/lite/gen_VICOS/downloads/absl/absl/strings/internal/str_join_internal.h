...
// iterator range, each separated by the given separator, into an output
// std::string, and formats each element using the provided Formatter object.
...
// std::string-like objects that do not need any additional formatting. This is
// to optimize the common case of joining, say, a std::vector<std::string> or a
// std::vector<absl::string_view>.
//
// This is an overload of the previous JoinAlgorithm() function. Here the
// Formatter argument is of type NoFormatter. Since NoFormatter is an internal
// type, this overload is only invoked when strings::Join() is called with a
// range of std::string-like objects (e.g., std::string, absl::string_view), and
// an explicit Formatter argument was NOT specified.
//
// The optimization is that the needed space will be reserved in the output
// std::string to avoid the need to resize while appending. To do this, the
// iterator range will be traversed twice: once to calculate the total needed
// size, and then again to copy the elements and delimiters to the output
// std::string.
