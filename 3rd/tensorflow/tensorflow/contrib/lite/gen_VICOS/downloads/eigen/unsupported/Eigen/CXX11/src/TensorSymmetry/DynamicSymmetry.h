...
class DynamicSGroup {
 public:
  ...
  template <typename Op, typename RV, typename Index, std::size_t N,
            typename... Args>
  inline RV apply(const std::array<Index, N>& idx, RV initial,
                  Args&&... args) const {
    eigen_assert(N >= m_numIndices &&
                 "Can only apply symmetry group to objects that have at least "
  ...
  inline RV apply(const std::vector<Index>& idx, RV initial,
                  Args&&... args) const {
    eigen_assert(idx.size() >= m_numIndices &&
                 "Can only apply symmetry group to objects that have at least "
                 "the required amount of indices.");
    for (std::size_t i = 0; i < size(); i++)
      initial = Op::run(h_permute(i, idx), m_elements[i].flags, initial,
  ...
  operator()(Tensor_& tensor, typename Tensor_::Index firstIndex,
             IndexTypes... otherIndices) const {
    static_assert(sizeof...(otherIndices) + 1 == Tensor_::NumIndices,
                  "Number of indices used to access a tensor coefficient must "
                  "be equal to the rank of the tensor.");
    return operator()(tensor,
                      std::array<typename Tensor_::Index, Tensor_::NumIndices>{
  ...
  operator()(Tensor_& tensor,
             std::array<typename Tensor_::Index, Tensor_::NumIndices> const&
                 indices) const {
    return internal::tensor_symmetry_value_setter<Tensor_, DynamicSGroup>(
        tensor, *this, indices);
  }
 ...
 private:
  ...
  struct Generator {
    ...
    int two;
    int flags;
    constexpr inline Generator(int one_, int two_, int flags_)
        : one(one_), two(two_), flags(flags_) {}
  };
  ...
  inline std::array<Index, N> h_permute(
      std::size_t which, const std::array<Index, N>& idx,
      internal::numeric_list<int, n...>) const {
    return std::array<Index, N>{
        {idx[n >= m_numIndices ? n : m_elements[which].representation[n]]...}};
  }

  template <typename Index>
  inline std::vector<Index> h_permute(std::size_t which,
                                      std::vector<Index> idx) const {
...
// dynamic symmetry group that auto-adds the template parameters in the
// constructor
...
class DynamicSGroupFromTemplateArgs : public DynamicSGroup {
 public:
  ...
  inline DynamicSGroupFromTemplateArgs(
      DynamicSGroupFromTemplateArgs const& other)
      : DynamicSGroup(other) {}
  inline DynamicSGroupFromTemplateArgs(DynamicSGroupFromTemplateArgs&& other)
      : DynamicSGroup(other) {}
  inline DynamicSGroupFromTemplateArgs<Gen...>& operator=(
      const DynamicSGroupFromTemplateArgs<Gen...>& o) {
    DynamicSGroup::operator=(o);
    return *this;
  }
  inline DynamicSGroupFromTemplateArgs<Gen...>& operator=(
      DynamicSGroupFromTemplateArgs<Gen...>&& o) {
    DynamicSGroup::operator=(o);
    return *this;
  }
...
inline DynamicSGroup::GroupElement DynamicSGroup::mul(GroupElement g1,
                                                      GroupElement g2) const {
...
inline void DynamicSGroup::updateGlobalFlags(int flagDiffOfSameGenerator) {
  switch (flagDiffOfSameGenerator) {
    ...
    case (NegationFlag | ConjugationFlag):
      // every element is it's own negative conjugate => whole tensor is
      // imaginary
      m_globalFlags |= GlobalImagFlag;
      break;
      /* NOTE:
       *   since GlobalZeroFlag == GlobalRealFlag | GlobalImagFlag, if one
       * generator causes the tensor to be real and the next one to be
       * imaginary, this will trivially give the correct result
       */
  }
}
