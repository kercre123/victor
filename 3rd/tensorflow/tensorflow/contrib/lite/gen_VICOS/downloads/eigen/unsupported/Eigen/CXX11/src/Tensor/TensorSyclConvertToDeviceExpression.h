...
#define TENSORMAPCONVERT(CVQual)                                           \
  ...
  struct ConvertToDeviceExpression<                                        \
      CVQual TensorMap<T, Options_, MakePointer_> > {                      \
...
/// specialisation of the \ref ConvertToDeviceExpression struct when the node
/// types are TensorForcedEvalOp and TensorLayoutSwapOp
#define KERNELBROKERCONVERTFORCEDEVALLAYOUTSWAPINDEXTUPLEOP(CVQual, ExprNode) \
  template <typename Expr>                                                    \
  struct ConvertToDeviceExpression<CVQual ExprNode<Expr> > {                  \
    typedef CVQual ExprNode<typename ConvertToDeviceExpression<Expr>::Type>   \
        Type;                                                                 \
  };
...
#define KERNELBROKERCONVERTTUPLEREDUCTION(CVQual)                   \
  template <typename OP, typename Dim, typename subExpr>            \
  struct ConvertToDeviceExpression<                                 \
      CVQual TensorTupleReducerOp<OP, Dim, subExpr> > {             \
    typedef CVQual TensorTupleReducerOp<                            \
        OP, Dim, typename ConvertToDeviceExpression<subExpr>::Type> \
        Type;                                                       \
  };
...
#define KERNELBROKERCONVERTSLICEOP(CVQual)                                 \
  ...
  struct ConvertToDeviceExpression<                                        \
      CVQual TensorSlicingOp<StartIndices, Sizes, XprType> > {             \
    typedef CVQual                                                         \
        TensorSlicingOp<StartIndices, Sizes,                               \
                        typename ConvertToDeviceExpression<XprType>::Type> \
            Type;                                                          \
  };
...
#define KERNELBROKERCONVERTERSLICESTRIDEOP(CVQual)                         \
  template <typename StartIndices, typename StopIndices, typename Strides, \
            typename XprType>                                              \
  struct ConvertToDeviceExpression<CVQual TensorStridingSlicingOp<         \
      StartIndices, StopIndices, Strides, XprType> > {                     \
    typedef CVQual TensorStridingSlicingOp<                                \
        StartIndices, StopIndices, Strides,                                \
        typename ConvertToDeviceExpression<XprType>::Type>                 \
        Type;                                                              \
  };
...
#define KERNELBROKERCONVERTCHIPPINGOP(CVQual)                               \
  ...
  struct ConvertToDeviceExpression<CVQual TensorChippingOp<DimId, Expr> > { \
    typedef CVQual TensorChippingOp<                                        \
        DimId, typename ConvertToDeviceExpression<Expr>::Type>              \
        Type;                                                               \
  };
...
#define KERNELBROKERCONVERTIMAGEPATCHOP(CVQual)                        \
  ...
  struct ConvertToDeviceExpression<                                    \
    ...
    typedef CVQual TensorImagePatchOp<                                 \
        Rows, Cols, typename ConvertToDeviceExpression<XprType>::Type> \
        Type;                                                          \
  };
...
/// specialisation of the \ref ConvertToDeviceExpression struct when the node
/// type is TensorVolumePatchOp
#define KERNELBROKERCONVERTVOLUMEPATCHOP(CVQual)                               \
  template <DenseIndex Plannes, DenseIndex Rows, DenseIndex Cols,              \
            typename XprType>                                                  \
  struct ConvertToDeviceExpression<                                            \
      CVQual TensorVolumePatchOp<Plannes, Rows, Cols, XprType> > {             \
    typedef CVQual                                                             \
        TensorVolumePatchOp<Plannes, Rows, Cols,                               \
                            typename ConvertToDeviceExpression<XprType>::Type> \
            Type;                                                              \
  };
