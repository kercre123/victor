...
#define TENSORMAPEXPR(CVQual)                                                  \
  template <typename T, int Options_, template <class> class MakePointer_,     \
            size_t N>                                                          \
  struct PlaceHolderExpression<CVQual TensorMap<T, Options_, MakePointer_>,    \
                               N> {                                            \
    typedef CVQual PlaceHolder<CVQual TensorMap<T, Options_, MakePointer_>, N> \
        Type;                                                                  \
  };
...
#define CUSTOMUNARYOPEVAL(CVQual)                                            \
  ...
  struct PlaceHolderExpression<                                              \
      CVQual TensorCustomUnaryOp<CustomUnaryFunc, XprType>, N> {             \
    typedef CVQual                                                           \
        PlaceHolder<CVQual TensorCustomUnaryOp<CustomUnaryFunc, XprType>, N> \
            Type;                                                            \
  };
...
#define CUSTOMBINARYOPEVAL(CVQual)                                             \
  template <typename CustomBinaryFunc, typename LhsXprType,                    \
            typename RhsXprType, size_t N>                                     \
  struct PlaceHolderExpression<                                                \
      CVQual TensorCustomBinaryOp<CustomBinaryFunc, LhsXprType, RhsXprType>,   \
      N> {                                                                     \
...
#define CHIPPINGOP(CVQual)                                                 \
  ...
  struct PlaceHolderExpression<CVQual TensorChippingOp<DimId, Expr>, N> {  \
    typedef CVQual                                                         \
        TensorChippingOp<DimId, typename CalculateIndex<N, Expr>::ArgType> \
            Type;                                                          \
  };
...
#define SYCLCONTRACTIONCONVOLUTIONPLH(CVQual, ExprNode)                  \
  template <typename Indices, typename LhsXprType, typename RhsXprType,  \
            size_t N>                                                    \
  struct PlaceHolderExpression<                                          \
      CVQual ExprNode<Indices, LhsXprType, RhsXprType>, N> {             \
    typedef CVQual                                                       \
        PlaceHolder<CVQual ExprNode<Indices, LhsXprType, RhsXprType>, N> \
            Type;                                                        \
  };
...
#define SLICEOPEXPR(CVQual)                                                    \
  ...
  struct PlaceHolderExpression<                                                \
      CVQual TensorSlicingOp<StartIndices, Sizes, XprType>, N> {               \
    typedef CVQual TensorSlicingOp<                                            \
        StartIndices, Sizes, typename CalculateIndex<N, XprType>::ArgType>     \
        Type;                                                                  \
  };
...
#define SYCLSLICESTRIDEOPPLH(CVQual)                                          \
  template <typename StartIndices, typename StopIndices, typename Strides,    \
            typename XprType, size_t N>                                       \
  struct PlaceHolderExpression<                                               \
      CVQual TensorStridingSlicingOp<StartIndices, StopIndices, Strides,      \
                                     XprType>,                                \
      N> {                                                                    \
    typedef CVQual                                                            \
        TensorStridingSlicingOp<StartIndices, StopIndices, Strides,           \
                                typename CalculateIndex<N, XprType>::ArgType> \
            Type;                                                             \
  };
...
#define SYCLTENSORIMAGEPATCHOP(CVQual)                                         \
  ...
  struct PlaceHolderExpression<CVQual TensorImagePatchOp<Rows, Cols, XprType>, \
                               N> {                                            \
    typedef CVQual TensorImagePatchOp<                                         \
        Rows, Cols, typename CalculateIndex<N, XprType>::ArgType>              \
        Type;                                                                  \
  };
...
#define SYCLTENSORVOLUMEPATCHOP(CVQual)                                   \
  template <DenseIndex Planes, DenseIndex Rows, DenseIndex Cols,          \
            typename XprType, size_t N>                                   \
  struct PlaceHolderExpression<                                           \
      CVQual TensorVolumePatchOp<Planes, Rows, Cols, XprType>, N> {       \
    typedef CVQual TensorVolumePatchOp<                                   \
        Planes, Rows, Cols, typename CalculateIndex<N, XprType>::ArgType> \
        Type;                                                             \
  };
