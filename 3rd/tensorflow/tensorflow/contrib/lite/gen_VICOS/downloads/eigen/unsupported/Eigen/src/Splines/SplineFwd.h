...
struct SplineTraits<Spline<_Scalar, _Dim, _Degree>, Dynamic> {
  ...
  enum {
    NumOfDerivativesAtCompileTime =
        OrderAtCompileTime /*!< The number of derivatives defined for the
                              current spline. */
  };

  enum {
    DerivativeMemoryLayout =
        Dimension == 1 ? RowMajor
                       : ColMajor /*!< The derivative type's memory layout. */
  };
   ...
   * derivatives. */
  typedef Array<Scalar, Dynamic, Dynamic, RowMajor,
                NumOfDerivativesAtCompileTime, OrderAtCompileTime>
      BasisDerivativeType;

  /** \brief The data type used to store the spline's derivative values. */
  typedef Array<Scalar, Dimension, Dynamic, DerivativeMemoryLayout, Dimension,
                NumOfDerivativesAtCompileTime>
      DerivativeType;
...
struct SplineTraits<Spline<_Scalar, _Dim, _Degree>, _DerivativeOrder>
  ...
  enum {
    NumOfDerivativesAtCompileTime =
        _DerivativeOrder == Dynamic
            ? Dynamic
            : _DerivativeOrder + 1 /*!< The number of derivatives defined for
                                      the current spline. */
  };
  ...
  /** \brief The data type used to store the values of the basis function
   * derivatives. */
  typedef Array<_Scalar, Dynamic, Dynamic, RowMajor,
                NumOfDerivativesAtCompileTime, OrderAtCompileTime>
      BasisDerivativeType;

  /** \brief The data type used to store the spline's derivative values. */
  typedef Array<_Scalar, _Dim, Dynamic, DerivativeMemoryLayout, _Dim,
                NumOfDerivativesAtCompileTime>
      DerivativeType;
};
