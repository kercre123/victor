 ...
 * and matname_SPD.mtx if the matrix is Symmetric and positive definite (or
 * Hermitian) The right hand side vectors are loaded as well, if they exist.
 * They should be named as matname_b.mtx.
 * Note that the right hand side for a SPD matrix is named as matname_SPD_b.mtx
 *
 * Sometimes a reference solution is available. In this case, it should be named
 * as matname_x.mtx
 *
 * Sample code
 * \code
...
class MatrixMarketIterator {
 ...
 public:
  ...
  inline VectorType& rhs() {
    ...
    std::string rhs_file;
    rhs_file =
        m_folder + "/" + m_matname + "_b.mtx";  // The pattern is matname_b.mtx
 ...
 protected:
  ...
  bool m_isvalid;
  bool m_matIsLoaded;  // Determine if the matrix has already been loaded from
                       // the file
