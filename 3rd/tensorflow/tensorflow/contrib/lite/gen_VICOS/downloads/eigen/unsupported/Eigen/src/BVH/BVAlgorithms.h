...
bool intersect_helper(const BVH &tree, Intersector &intersector,
...
template <typename Volume1, typename Object1, typename Object2,
...
template <typename Volume2, typename Object2, typename Object1,
          typename Intersector>
...
/**  Given a BVH, runs the query encapsulated by \a intersector.
  *  The Intersector type must provide the following members: \code
     bool intersectVolume(const BVH::Volume &volume) //returns true if volume
  intersects the query bool intersectObject(const BVH::Object &object) //returns
  true if the search should terminate immediately \endcode
  */
template <typename BVH, typename Intersector>
...
/**  Given two BVH's, runs the query on their Cartesian product encapsulated by
  \a intersector.
  *  The Intersector type must provide the following members: \code
     bool intersectVolumeVolume(const BVH1::Volume &v1, const BVH2::Volume &v2)
  //returns true if product of volumes intersects the query bool
  intersectVolumeObject(const BVH1::Volume &v1, const BVH2::Object &o2)
  //returns true if the volume-object product intersects the query bool
  intersectObjectVolume(const BVH1::Object &o1, const BVH2::Volume &v2)
  //returns true if the volume-object product intersects the query bool
  intersectObjectObject(const BVH1::Object &o1, const BVH2::Object &o2)
  //returns true if the search should terminate immediately \endcode
  */
template <typename BVH1, typename BVH2, typename Intersector>
void BVIntersect(
    const BVH1 &tree1, const BVH2 &tree2,
    Intersector &intersector)  // TODO: tandem descent when it makes sense
{
  ...
  typedef internal::intersector_helper1<typename BVH1::Volume,
                                        typename BVH1::Object,
                                        typename BVH2::Object, Intersector>
      Helper1;
  typedef internal::intersector_helper2<typename BVH2::Volume,
                                        typename BVH2::Object,
                                        typename BVH1::Object, Intersector>
      Helper2;
  ...
  std::vector<std::pair<Index1, Index2> > todo(
      1, std::make_pair(tree1.getRootIndex(), tree2.getRootIndex()));
  ...
  while (!todo.empty()) {
    ...
    for (; vBegin1 != vEnd1;
      ...
      for (vCur2 = vBegin2; vCur2 != vEnd2;
           ++vCur2) {  // go through child volumes of second tree
        if (intersector.intersectVolumeVolume(vol1, tree2.getVolume(*vCur2)))
          todo.push_back(std::make_pair(*vBegin1, *vCur2));
      }

      for (oCur2 = oBegin2; oCur2 != oEnd2;
           ++oCur2) {  // go through child objects of second tree
    ...
    for (; oBegin1 != oEnd1;
         ++oBegin1) {  // go through child objects of first tree
      for (vCur2 = vBegin2; vCur2 != vEnd2;
           ++vCur2) {  // go through child volumes of second tree
...
typename Minimizer::Scalar minimize_helper(const BVH &tree,
                                           Minimizer &minimizer,
                                           typename BVH::Index root,
...
/**  Given a BVH, runs the query encapsulated by \a minimizer.
  ...
  *  The Minimizer type must provide the following members: \code
     typedef Scalar //the numeric type of what is being minimized--not
  necessarily the Scalar type of the BVH (if it has one) Scalar
  minimumOnVolume(const BVH::Volume &volume) Scalar minimumOnObject(const
  BVH::Object &object) \endcode
  */
template <typename BVH, typename Minimizer>
...
/**  Given two BVH's, runs the query on their cartesian product encapsulated by
  ...
  *  The Minimizer type must provide the following members: \code
     typedef Scalar //the numeric type of what is being minimized--not
  necessarily the Scalar type of the BVH (if it has one) Scalar
  minimumOnVolumeVolume(const BVH1::Volume &v1, const BVH2::Volume &v2) Scalar
  minimumOnVolumeObject(const BVH1::Volume &v1, const BVH2::Object &o2) Scalar
...
typename Minimizer::Scalar BVMinimize(const BVH1 &tree1, const BVH2 &tree2,
                                      Minimizer &minimizer) {
  typedef typename Minimizer::Scalar Scalar;
  typedef typename BVH1::Index Index1;
  typedef typename BVH2::Index Index2;
  typedef internal::minimizer_helper1<typename BVH1::Volume,
                                      typename BVH1::Object,
                                      typename BVH2::Object, Minimizer>
      Helper1;
  typedef internal::minimizer_helper2<typename BVH2::Volume,
                                      typename BVH2::Object,
                                      typename BVH1::Object, Minimizer>
      Helper2;
  typedef std::pair<Scalar, std::pair<Index1, Index2> >
      QueueElement;  // first element is priority
  typedef typename BVH1::VolumeIterator VolIter1;
  typedef typename BVH1::ObjectIterator ObjIter1;
  typedef typename BVH2::VolumeIterator VolIter2;
  typedef typename BVH2::ObjectIterator ObjIter2;
    ...
    for (; oBegin1 != oEnd1;
      ...
      for (oCur2 = oBegin2; oCur2 != oEnd2;
        ...
        minimum = (std::min)(minimum,
                             minimizer.minimumOnObjectObject(*oBegin1, *oCur2));
      }
      ...
      for (vCur2 = vBegin2; vCur2 != vEnd2;
        ...
        minimum = (std::min)(
            minimum, internal::minimize_helper(tree2, helper, *vCur2, minimum));
      }
    }
    ...
    for (; vBegin1 != vEnd1;
      ...
      for (oCur2 = oBegin2; oCur2 != oEnd2;
        ...
        minimum = (std::min)(minimum, internal::minimize_helper(
                                          tree1, helper, *vBegin1, minimum));
      }

      for (vCur2 = vBegin2; vCur2 != vEnd2;
           ++vCur2) {  // go through child volumes of second tree
        Scalar val =
            minimizer.minimumOnVolumeVolume(vol1, tree2.getVolume(*vCur2));
        if (val < minimum)
