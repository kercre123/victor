...
class LevenbergMarquardt {
 ...
 public:
  ...
  LevenbergMarquardtSpace::Status lmder1(FVectorType &x,
  ...
  LevenbergMarquardtSpace::Status lmstr1(FVectorType &x,
                                         const Scalar tol = sqrt_epsilon());
...
LevenbergMarquardtSpace::Status LevenbergMarquardt<FunctorType, Scalar>::lmder1(
...
LevenbergMarquardt<FunctorType, Scalar>::minimizeInit(FVectorType &x) {
  ...
  eigen_assert(
      (!useExternalScaling || diag.size() == n) &&
  ...
  /*     check the input parameters for errors. */
  if (n <= 0 || m < n || parameters.ftol < 0. || parameters.xtol < 0. ||
      parameters.gtol < 0. || parameters.maxfev <= 0 || parameters.factor <= 0.)
    return LevenbergMarquardtSpace::ImproperInputParameters;
...
LevenbergMarquardt<FunctorType, Scalar>::minimizeOneStep(FVectorType &x) {
  ...
  if (fnorm != 0.)
    for (Index j = 0; j < n; ++j)
      if (wa2[permutation.indices()[j]] != 0.)
        gnorm = (std::max)(
            gnorm, abs(fjac.col(j).head(j + 1).dot(qtf.head(j + 1) / fnorm) /
  ...
  do {
    ...
    wa3 = fjac.template triangularView<Upper>() *
          (qrfac.colsPermutation().inverse() * wa1);
    ...
    if (abs(actred) <= parameters.ftol && prered <= parameters.ftol &&
    ...
    if (abs(actred) <= NumTraits<Scalar>::epsilon() &&
        prered <= NumTraits<Scalar>::epsilon() && Scalar(.5) * ratio <= 1.)
...
template <typename FunctorType, typename Scalar>
LevenbergMarquardtSpace::Status LevenbergMarquardt<FunctorType, Scalar>::lmstr1(
    FVectorType &x, const Scalar tol) {
  n = x.size();
  m = functor.values();
...
LevenbergMarquardt<FunctorType, Scalar>::minimizeOptimumStorageOneStep(
  ...
  if (sing) {
    ...
    for (Index ii = 0; ii < fjac.cols(); ii++)
      fjac.col(ii).segment(ii + 1, fjac.rows() - ii - 1) *=
          fjac(ii, ii);  // rescale vectors
  ...
  do {
    /* determine the levenberg-marquardt parameter. */
    internal::lmpar<Scalar>(fjac, permutation.indices(), diag, qtf, delta, par,
                            wa1);
    ...
    /* compute the scaled predicted reduction and */
    /* the scaled directional derivative. */
    wa3 = fjac.topLeftCorner(n, n).template triangularView<Upper>() *
          (permutation.inverse() * wa1);
    temp1 = numext::abs2(wa3.stableNorm() / fnorm);
    temp2 = numext::abs2(sqrt(par) * pnorm / fnorm);
    prered = temp1 + temp2 / Scalar(.5);
    dirder = -(temp1 + temp2);
...
template <typename FunctorType, typename Scalar>
LevenbergMarquardtSpace::Status LevenbergMarquardt<FunctorType, Scalar>::lmdif1(
