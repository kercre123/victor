  ...
  for (Index k = 0; k < k_size; k += 32) {
    ...
    if (!CHECK_LHS_BOUNDARY) {
      if ((threadIdx.y / 4 + k + 24) < k_size) {
        lhs_pf0 =
            lhs.template loadPacket<Unaligned>(lhs_vert, (threadIdx.y / 4 + k));
        lhs_pf1 = lhs.template loadPacket<Unaligned>(lhs_vert,
                                                     (threadIdx.y / 4 + k + 8));
        lhs_pf2 = lhs.template loadPacket<Unaligned>(
            lhs_vert, (threadIdx.y / 4 + k + 16));
        lhs_pf3 = lhs.template loadPacket<Unaligned>(
      ...
      if (lhs_vert + 3 < m_size) {
        if ((threadIdx.y / 4 + k + 24) < k_size) {
          lhs_pf0 = lhs.template loadPacket<Unaligned>(lhs_vert,
                                                       (threadIdx.y / 4 + k));
          lhs_pf1 = lhs.template loadPacket<Unaligned>(
