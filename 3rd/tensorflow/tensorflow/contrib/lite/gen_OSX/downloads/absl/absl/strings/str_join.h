...
// passing a range, a separator std::string to use between the elements joined,
// and an optional Formatter responsible for converting each argument in the
// range to a std::string. If omitted, a default `AlphaNumFormatter()` is called
// on the elements to be joined, using the same formatting that `absl::StrCat()`
// uses. This package defines a number of default formatters, and you can define
...
// argument as a std::string and appending it to a given output std::string.
