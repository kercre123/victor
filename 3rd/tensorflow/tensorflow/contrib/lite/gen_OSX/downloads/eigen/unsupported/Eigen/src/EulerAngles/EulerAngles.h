 ...
 * represented by Euler angles, but there is no single representation (e.g.
 ...
 *
 * When converting rotation to EulerAngles, this class convert it to specific
 * ranges When converting some rotation to EulerAngles, the rules for ranges are
 * as follow:
 * - If the rotation we converting from is an EulerAngles
 *  (even when it represented as RotationBase explicitly), angles ranges are
 * __undefined__.
 * - otherwise, alpha and gamma angles will be in the range [-PI, PI].<BR>
 *   As for Beta angle:
 *    - If the system is Tait-Bryan, the beta angle will be in the range [-PI/2,
 * PI/2].
 *    - otherwise:
 *      - If the beta axis is positive, the beta angle will be in the range [0,
 * PI]
 *      - If the beta axis is negative, the beta angle will be in the range
 * [-PI, 0]
 *
...
class EulerAngles : public RotationBase<EulerAngles<_Scalar, _System>, 3> {
 ...
 public:
  ...
  /** Constructs and initialize an EulerAngles from the array data {alpha, beta,
   * gamma} */
  explicit EulerAngles(const Scalar* data) : m_angles(data) {}

  /** Constructs and initializes an EulerAngles from either:
   *  - a 3x3 rotation matrix expression(i.e. pure orthogonal matrix with
   * determinant of +1),
   *  - a 3D vector expression representing Euler angles.
   *
   * \note If \p other is a 3x3 rotation matrix, the angles range rules will be
   * as follow:<BR> Alpha and gamma angles will be in the range [-PI, PI].<BR>
   *  As for Beta angle:
   *   - If the system is Tait-Bryan, the beta angle will be in the range
   * [-PI/2, PI/2].
   *   - otherwise:
   *     - If the beta axis is positive, the beta angle will be in the range [0,
   * PI]
   ...
   *
   * \note If \p rot is an EulerAngles (even when it represented as RotationBase
   * explicitly), angles ranges are __undefined__. Otherwise, alpha and gamma
   * angles will be in the range [-PI, PI].<BR> As for Beta angle:
   *   - If the system is Tait-Bryan, the beta angle will be in the range
   * [-PI/2, PI/2].
   *   - otherwise:
   *     - If the beta axis is positive, the beta angle will be in the range [0,
  ...
  EulerAngles& operator=(const MatrixBase<Derived>& other) {
    ...
    internal::eulerangles_assign_impl<System, Derived>::run(*this,
                                                            other.derived());
    return *this;
  }
