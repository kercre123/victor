...
template <typename Index, int Mode, typename LhsScalar, bool ConjLhs,
          typename RhsScalar, int StorageOrder>
struct band_solve_triangular_selector;

template <typename Index, int Mode, typename LhsScalar, bool ConjLhs,
          typename RhsScalar>
struct band_solve_triangular_selector<Index, Mode, LhsScalar, ConjLhs,
                                      RhsScalar, RowMajor> {
  ...
  static void run(Index size, Index k, const LhsScalar* _lhs, Index lhsStride,
                  RhsScalar* _other) {
          ...
          other.coeffRef(i, col) -=
              cjLhs.row(i)
                  ...
                  .cwiseProduct(other.col(col).segment(
                      IsLower ? i - actual_k : i + 1, actual_k))
                  .sum();
      ...
      for (int ii = 0; ii < size; ++ii) {
        ...
        if (actual_k > 0)
          other.col(col).segment(IsLower ? i + 1 : i - actual_k, actual_k) -=
              other.coeff(i, col) *
              cjLhs.col(i).segment(actual_start, actual_k);
      }
    }
  }
};
