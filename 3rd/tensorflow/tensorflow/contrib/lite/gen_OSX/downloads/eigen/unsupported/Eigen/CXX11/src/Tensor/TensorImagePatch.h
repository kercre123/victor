...
#ifdef EIGEN_USE_SYCL  // this is work around for sycl as Eigen could not use
                       // c++11 deligate constructor feature
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorImagePatchOp(
      const XprType& expr, DenseIndex patch_rows, DenseIndex patch_cols,
      DenseIndex row_strides, DenseIndex col_strides, DenseIndex in_row_strides,
      DenseIndex in_col_strides, DenseIndex row_inflate_strides,
      DenseIndex col_inflate_strides, bool padding_explicit,
      ...
      : m_xpr(expr),
        ...
        m_row_inflate_strides(row_inflate_strides),
        m_col_inflate_strides(col_inflate_strides),
        m_padding_explicit(padding_explicit),
        m_padding_top(padding_top),
        m_padding_bottom(padding_bottom),
        m_padding_left(padding_left),
        m_padding_right(padding_right),
...
struct TensorEvaluator<const TensorImagePatchOp<Rows, Cols, ArgType>, Device> {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const XprType& xpr() const {
    return m_op;
  }
#endif
