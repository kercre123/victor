...
// This FFT implementation was derived from kissfft
// http:sourceforge.net/projects/kissfft Copyright 2003-2009 Mark Borgerding
...
struct kiss_cpx_fft {
  ...
  inline void work(int stage, Complex *xout, const _Src *xin, size_t fstride,
                   size_t in_stride) {
  ...
  inline void bfly3(Complex *Fout, const size_t fstride, const size_t m) {
    ...
    do {
      ...
      scratch[0] *= epi3.imag();
      *Fout += scratch[3];
      Fout[m2] = Complex(Fout[m].real() + scratch[0].imag(),
  ...
  inline void bfly5(Complex *Fout, const size_t fstride, const size_t m) {
    ...
    for (u = 0; u < m; ++u) {
      ...
      *Fout0 += scratch[8];
      ...
      scratch[5] = scratch[0] + Complex((scratch[7].real() * ya.real()) +
                                            (scratch[8].real() * yb.real()),
                                        (scratch[7].imag() * ya.real()) +
      ...
      *Fout4 = scratch[5] + scratch[6];
      ...
      scratch[11] = scratch[0] + Complex((scratch[7].real() * yb.real()) +
                                             (scratch[8].real() * ya.real()),
                                         (scratch[7].imag() * yb.real()) +
...
struct kissfft_impl {
  ...
  inline void inv(Scalar *dst, const Complex *src, int nfft) {
    if (nfft & 3) {
      ...
      m_tmpBuf1[0] = Complex(src[0].real() + src[ncfft].real(),
                             src[0].real() - src[ncfft].real());
      ...
      get_plan(ncfft, true)
          .work(0, reinterpret_cast<Complex *>(dst), &m_tmpBuf1[0], 1, 1);
    }
  }
 ...
 protected:
  ...
  inline int PlanKey(int nfft, bool isinverse) const {
  ...
  inline Complex *real_twiddles(int ncfft2) {
    using std::acos;
    std::vector<Complex> &twidref =
        m_realTwiddles[ncfft2];  // creates new if not there
