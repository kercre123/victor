...
template <typename T>
inline T *vector_data(std::vector<T> &vector) {
...
template <typename T>
inline const T *vector_data(const std::vector<T> &vector) {
  return &(*vector.begin());
}
...
class numeric_limits<long long> {
 public:
  ...
  static long long max() {
    return static_cast<long long>((1ULL << ((sizeof(long long) << 3) - 1)) - 1);
  }
};
...
template <typename T>
using is_floating_point = std::tr1::is_floating_point<T>;
template <typename T>
...
bool operator==(const unique_ptr<T>& x, const D* y) {
  return static_cast<D*>(x.get()) == y;
}

template <class T>
bool operator==(const unique_ptr<T>& x, intptr_t y) {
