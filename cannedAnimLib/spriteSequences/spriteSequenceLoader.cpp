...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Vision::SpriteSequenceContainer* SpriteSequenceLoader::LoadSpriteSequences(
  ...
  using MyDispatchWorker =
      Util::DispatchWorker<3, Vision::SpriteCache*, std::string>;
  MyDispatchWorker::FunctionType workerFunction =
      std::bind(&SpriteSequenceLoader::LoadSequence, this,
                std::placeholders::_1, std::placeholders::_2);
  MyDispatchWorker worker(workerFunction);
  for (const auto& path : spriteSequenceDirs) {
    const std::string spriteSeqFolder =
        dataPlatform->pathToResource(resourceScope, path);

    // Get the list of all the directory names
    std::vector<std::string> spriteSequenceDirNames;
    Util::FileUtils::ListAllDirectories(spriteSeqFolder,
                                        spriteSequenceDirNames);

    // Go through the list of directories, removing disallowed names, updating
    // timestamps, and removing ones that don't need to be loaded
    ...
    while (nameIter != spriteSequenceDirNames.end()) {
      ...
      std::string fullDirPath =
          Util::FileUtils::FullFilePath({spriteSeqFolder, folderName});
  ...
  auto* container =
      new Vision::SpriteSequenceContainer(std::move(_spriteSequences));
...
void SpriteSequenceLoader::LoadSequence(Vision::SpriteCache* cache,
  ...
  // Even though files *might* be sorted alphabetically by the readdir call
  // inside FilesInDirectory, we can't rely on it so do it ourselves
  std::vector<std::string> fileNames =
      Util::FileUtils::FilesInDirectory(fullDirectoryPath);
  ...
  if (specIter == fileNames.end()) {
    ...
    const std::string fullFilename =
        Util::FileUtils::FullFilePath({fullDirectoryPath, *specIter});
    const bool success =
        Util::Data::DataPlatform::readAsJson(fullFilename, spec);
  ...
  _spriteSequences.emplace(Vision::SpritePathMap::GetAssetID(
                               Util::FileUtils::GetFileName(fullDirectoryPath)),
                           seq);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void SpriteSequenceLoader::LoadSequenceLegacy(
    Vision::SpriteCache* cache, const std::string& fullDirectoryPath,
    const std::vector<std::string>& relativeImgNames,
    Vision::SpriteSequence& outSeq) const {
  for (auto fileIter = relativeImgNames.begin();
       fileIter != relativeImgNames.end(); ++fileIter) {
    ...
    if (dotPos == std::string::npos) {
      ...
      PRINT_NAMED_ERROR(
          "SpriteSequenceLoader.LoadSequenceLegacy",
    ...
    if (frameNum < outSeq.GetNumFrames()) {
      PRINT_NAMED_ERROR("SpriteSequenceLoader.LoadSequenceLegacy",
                        "Image %s has frame number %d, but sequence already "
                        "has %d frames - skipping frame",
    ...
    const std::string fullFilePath =
        Util::FileUtils::FullFilePath({fullDirectoryPath, filename});
    Vision::HSImageHandle faceHueAndSaturation =
        ProceduralFace::GetHueSatWrapper();
    Vision::SpriteHandle handle;
    handle =
        cache->GetSpriteHandleForSpritePath(fullFilePath, faceHueAndSaturation);

    if (frameNum != outSeq.GetNumFrames()) {
      PRINT_NAMED_ERROR(
          "SpriteSequenceLoader.LoadSequenceLegacy.MissingFrameNumbers",
          ...
          outSeq.GetNumFrames(), filename.c_str());
      break;
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void SpriteSequenceLoader::LoadSequenceFromSpec(
    Vision::SpriteCache* cache, const std::string& fullDirectoryPath,
    const Json::Value& spec, const std::vector<std::string>& relativeImgNames,
    Vision::SpriteSequence& outSeq) const {
  {
    const auto loopStr = JsonTools::ParseString(
        spec, kLoopKey,
        "SpriteSequenceLoader.LoadSequenceFromSpec.NoLoopString");
    Vision::SpriteSequence::LoopConfig loopConfig =
        Vision::SpriteSequence::LoopConfigFromString(loopStr);
  ...
  if (ANKI_VERIFY(segmentArray.isArray(),
    ...
    for (const auto& segment : segmentArray) {
      if (segment[kSegmentTypeKey] != "straightThrough") {
        PRINT_NAMED_ERROR(
            "SpriteSequenceLoader.LoadSequenceFromSpec.InvalidSegmentType",
      ...
      if (ANKI_VERIFY(
        ...
        for (const auto& fileNameJSON : orderedFileArray) {
          ...
          auto iter = std::find(relativeImgNames.begin(),
                                relativeImgNames.end(), fileName);
          if (iter != relativeImgNames.end()) {
            ...
            Vision::HSImageHandle faceHueAndSaturation =
                ProceduralFace::GetHueSatWrapper();
            Vision::SpriteHandle handle;
            handle = cache->GetSpriteHandleForSpritePath(fullFilePath,
                                                         faceHueAndSaturation);
            outSeq.AddFrame(handle);
          } else {
            PRINT_NAMED_ERROR(
                "SpriteSequenceLoader.LoadSequenceFromSpec.FileNotInFolder",
                "Could not find file %s in folder %s", fileName.c_str(),
                fullDirectoryPath.c_str());
          }
        }
      }
    }
