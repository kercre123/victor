...
class ScanlineDistorter {
 public:
  ...
  // Given the fractional vertical eye position, returns the number of pixels of
  // horizontal distortion to apply.
  s32 GetEyeDistortionAmount(f32 eyeFrac) const;

  // Given the "warp" matrix which positions/scales the eye in the face, draws
  // the corresponding "off" noise into the image.
  template <typename T>
  void AddOffNoise(const Matrix_3x3f& warpMatrix, const s32 eyeHeight,
                   const s32 eyeWidth, T& faceImg) const {
    for (const auto& pt : _offNoisePoints) {
      ...
      const s32 row = Util::Clamp((s32)std::round(noisePt.y()), 0,
                                  faceImg.GetNumRows() - 1);
      const s32 col = Util::Clamp((s32)std::round(noisePt.x()), 0,
                                  faceImg.GetNumCols() - 1);

      if (kProcFaceScanline_OffNoiseMaxWidth > 1) {
        const s32 width =
            GetRNG().RandIntInRange(1, kProcFaceScanline_OffNoiseMaxWidth);
  ...
  // Gets sequence of distortions using the ScanlineDistorter in the given
  // faceData. Call until it returns false, which indicates there are no more
  // distortion frames and the face is back in its original state. The output
  // "offset" indicates the desired timing since the previous state.
  static bool GetNextDistortionFrame(const f32 degree, ProceduralFace& faceData,
                                     TimeStamp_t& timeInc);
 ...
 private:
  ...
  enum class Shape : u8 {
    ...
    Triangle,  // 3 control points, top/bottom shift one way, middle shifts
               // other
  ...
  struct ControlPoint {
    f32 vertical_frac;  // vertical position within the eye, relative to eye
                        // height
    s32 direction;
    s32 amount_pix;
