 ...
 * Created: 5/15/2017
 *
 * Description: Responsible for holding distortion parameters for a
 *ProceduralFace
 *
...
// Fraction of (nominal) eye area to be off (note: does not consider "Width"
// parameter below)
CONSOLE_VAR_RANGED(f32, kProcFaceScanline_OffNoiseProb, CONSOLE_GROUP_NAME,
                   0.1f, 0.f, 1.f);

// Max width of each "off" noise bar
CONSOLE_VAR(s32, kProcFaceScanline_OffNoiseMaxWidth, CONSOLE_GROUP_NAME, 3);

// Max amount to randomly shift control-point distortion shifts left and right,
// per scanline
...
ScanlineDistorter::ScanlineDistorter(s32 maxAmount_pix, f32 noiseProb) {
  ...
  switch (_shape) {
    case Shape::Skew: {
      ...
      _controlPoints.emplace_back(0.f, -direction,
                                  GetRNG().RandIntInRange(1, maxAmount_pix));
      _controlPoints.emplace_back(1.f, direction,
                                  GetRNG().RandIntInRange(1, maxAmount_pix));
      break;
    }

    case Shape::Triangle: {
      // Three control points: top, bottom, and roughly in the middle (mid moves
      // opposite dir as top/bottom)
      _controlPoints.emplace_back(0.0f, -direction,
                                  GetRNG().RandIntInRange(1, maxAmount_pix));
      const f32 midPoint = GetRNG().RandDblInRange(0.35, 0.65);
      _controlPoints.emplace_back(midPoint, direction,
                                  GetRNG().RandIntInRange(1, maxAmount_pix));
      _controlPoints.emplace_back(1.0f, -direction,
                                  GetRNG().RandIntInRange(1, maxAmount_pix));
      break;
    }
    ...
    case Shape::S_Curve: {
      ...
      _controlPoints.emplace_back(upperMidPoint, direction,
                                  GetRNG().RandIntInRange(1, maxAmount_pix));
      const f32 lowerMidPoint = GetRNG().RandDblInRange(0.65, 0.85);
      _controlPoints.emplace_back(lowerMidPoint, -direction,
                                  GetRNG().RandIntInRange(1, maxAmount_pix));
      _controlPoints.emplace_back(1.0f, direction,
                                  GetRNG().RandIntInRange(1, maxAmount_pix));
      break;
    ...
    case Shape::Count: {
      PRINT_NAMED_ERROR("ProceduralFace.InitScanlineDistortion.BadShape",
                        "Count is not valid shape");
      return;
    }
  }

  if (Util::IsFltGTZero(noiseProb)) {
    const f32 eyeArea = (f32)(ProceduralFace::NominalEyeHeight *
                              ProceduralFace::NominalEyeWidth);
...
void ScanlineDistorter::Update(s32 maxAmount_pix) {
  // Shift the control points relative to the direction they are already headed
  // (based on direction) (Positive means keep moving in the same direction,
  // Negative means the opposite direciton)
  for (auto& controlPt : _controlPoints) {
    const s32 direction =
        (maxAmount_pix < 0 ? -controlPt.direction : controlPt.direction);
    const s32 shift =
        direction * GetRNG().RandIntInRange(1, std::abs(maxAmount_pix));
...
s32 ScanlineDistorter::GetEyeDistortionAmount(f32 eyeFrac) const {
  DEV_ASSERT(
      _controlPoints.size() > 1,
      "ScanlineDistorter.GetEyeScanlineDistortion.NotEnoughControlPoints");

  s32 distortionAmount_pix = 0;
  for (s32 iControlPt = 0; iControlPt < _controlPoints.size() - 1;
       ++iControlPt) {
    ...
    if (eyeFrac >= controlPt1.vertical_frac &&
        eyeFrac < controlPt2.vertical_frac) {
      // Linearly interpolate distortion amount based on position between y1 and
      // y2
      const f32 w = (eyeFrac - controlPt1.vertical_frac) /
                    (controlPt2.vertical_frac - controlPt1.vertical_frac);
      DEV_ASSERT(Util::InRange(w, 0.f, 1.f),
                 "ProceduralFace.GetScanlineDistortion.BadWeight");
      distortionAmount_pix = std::round((1.f - w) * (f32)controlPt1.amount_pix +
                                        w * (f32)controlPt2.amount_pix);

      if (kProcFaceScanline_MaxShiftNoise > 0) {
        distortionAmount_pix += GetRNG().RandIntInRange(
            -kProcFaceScanline_MaxShiftNoise, kProcFaceScanline_MaxShiftNoise);
      }
    }
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
bool ScanlineDistorter::GetNextDistortionFrame(const f32 degree,
                                               ProceduralFace& faceData,
                                               TimeStamp_t& timeInc) {
  struct DistortParams {
    f32 probNoDistortionAfter;  // Probability that after this frame we show the
                                // undistorted face for a single frame
    s32 amount_pix;
  };

  // Note:
  // (1) "amount" is cumulative!
  // (2) if spacing is greater than one keyframe time, the undistorted face will
  // be shown until next distortion
  static const std::vector<DistortParams> distortionAmounts{
      {.probNoDistortionAfter = 0.f, .amount_pix = 1},
      {.probNoDistortionAfter = 0.f, .amount_pix = 1},
      {.probNoDistortionAfter = 0.75f,
       .amount_pix = 2},  // Will flash undistorted for a frame after this
      {.probNoDistortionAfter = 0.f, .amount_pix = 1},
      ...
      {.probNoDistortionAfter = 0.75f,
       .amount_pix = -5},  // Will flash undistorted for a frame after this
  ...
  if (distortionIter == distortionAmounts.end()) {
    // Reset for next time and leave faceData with no distortion. Let caller
    // know there's nothing else coming.
    ...
    if (distortionIter == distortionAmounts.begin()) {
      ...
      faceData.InitScanlineDistorter(amount_pix,
                                     kProcFaceScanline_OffNoiseProb);
    } else {
      DEV_ASSERT(
          nullptr != faceData.GetScanlineDistorter(),
