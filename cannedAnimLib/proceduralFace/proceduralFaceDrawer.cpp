...
// switched from (x) to std::round(x) as it was noticed edges were jittering
// when the roundness of corners was animated (VIC-3930). Keeping as a macro
// in-case there's a faster way to round()/cast().
...
CONSOLE_VAR_ENUM(u8, kProcFace_LineType, CONSOLE_GROUP, 1,
                 "Line_4,Line_8,Line_AA");  // Only affects OpenCV drawing, not
                                            // post-smoothing
CONSOLE_VAR_ENUM(
    u8, kProcFace_InterpolationType, CONSOLE_GROUP, 1,
...
CONSOLE_VAR_RANGED(f32, kProcFace_EyeLightnessMultiplier, CONSOLE_GROUP, 1.f,
                   0.f, 2.f);

CONSOLE_VAR(bool, kProcFace_HotspotRender, CONSOLE_GROUP, true);  // Render glow
CONSOLE_VAR_RANGED(f32, kProcFace_HotspotFalloff, CONSOLE_GROUP, 0.48f, 0.05f,
                   1.f);

CONSOLE_VAR(bool, kProcFace_EnableAntiAliasing, CONSOLE_GROUP, true);
CONSOLE_VAR_RANGED(s32, kProcFace_AntiAliasingSize, CONSOLE_GROUP, 3, 0,
                   15);  // full image antialiasing (3 will use NEON)
CONSOLE_VAR_ENUM(uint8_t, kProcFace_AntiAliasingFilter, CONSOLE_GROUP,
                 (uint8_t)Filter::BoxFilter, "None,Box,Gaussian");
CONSOLE_VAR_RANGED(f32, kProcFace_AntiAliasingSigmaFraction, CONSOLE_GROUP,
                   0.5f, 0.0f, 1.0f);

#if PROCEDURALFACE_GLOW_FEATURE
CONSOLE_VAR_RANGED(f32, kProcFace_GlowSizeMultiplier, CONSOLE_GROUP, 1.f, 0.f,
                   1.f);
CONSOLE_VAR_RANGED(f32, kProcFace_GlowLightnessMultiplier, CONSOLE_GROUP, 1.f,
                   0.f, 10.f);
CONSOLE_VAR_ENUM(uint8_t, kProcFace_GlowFilter, CONSOLE_GROUP,
                 (uint8_t)Filter::BoxFilter,
                 "None,Box,Gaussian,Box (NEON code; size 3)");
#endif

#if PROCEDURALFACE_SCANLINE_FEATURE
CONSOLE_VAR(bool, kProcFace_Scanlines, CONSOLE_GROUP, false);
CONSOLE_VAR_RANGED(ProceduralFace::Value, kProcFace_DefaultScanlineOpacity,
                   CONSOLE_GROUP, 1.f, 0.f, 1.f);
#endif

#if PROCEDURALFACE_NOISE_FEATURE
static const s32 kNumNoiseImages = 7;

CONSOLE_VAR_RANGED(s32, kProcFace_NoiseNumFrames, CONSOLE_GROUP, 5, 0,
                   kNumNoiseImages);
CONSOLE_VAR_RANGED(f32, kProcFace_NoiseMinLightness, CONSOLE_GROUP, 0.92f, 0.f,
                   2.f);  // replaces kProcFace_NoiseFraction
CONSOLE_VAR_RANGED(f32, kProcFace_NoiseMaxLightness, CONSOLE_GROUP, 1.14f, 0.f,
                   2.f);  // replaces kProcFace_NoiseFraction
...
Matrix_3x3f ProceduralFaceDrawer::GetTransformationMatrix(
    f32 angleDeg, f32 scaleX, f32 scaleY, f32 tX, f32 tY, f32 x0, f32 y0) {
  //
  // Create a 2x3 warp matrix which incorporates scale, rotation, and
  // translation
  //    W = R * [scale_x    0   ] * [x - x0] + [x0] + [tx]
  //            [   0    scale_y]   [y - y0] + [y0] + [ty]
  //
  // So a given point gets scaled (first!) and then rotated around the given
  // center (x0,y0)and then translated by (tx,ty).
  //
  ...
  Matrix_3x3f W{alpha_x, beta_y,  (1.f - alpha_x) * x0 - beta_y * y0 + tX,
...
#define NUM_FAST_EXP_TERMS 2
  ...
  if (x < -(f32)(2 * NUM_FAST_EXP_TERMS)) {
    // Things get numerically unstable for very negative inputs x. Value is
    // basically zero anyway.
    return 0.f;
  }
#if NUM_FAST_EXP_TERMS == 2
  x = 1.f + (x * 0.25f);  // Constant here is 1/(2^N)
  x *= x;
  x *= x;  // Number of multiplies here is also N
...
inline Array2d<u8> CreateNoiseImage(const Util::RandomGenerator& rng) {
  ...
  for (s32 j = 0; j < noiseImg.GetNumElements(); ++j) {
    // Storing noise in the range [0 255] instead of [0 2.0] both to reduce
    // memory usage as well as improving NEON performance by not having to
    // convert between float and u8
    noiseImg_i[j] = Util::numeric_cast_clamped<u8>(
        rng.RandDblInRange(kProcFace_NoiseMinLightness,
...
const Array2d<u8>& ProceduralFaceDrawer::GetNoiseImage(
    const Util::RandomGenerator& rng) {
  // NOTE: Since this is called separately for each eye, this looks better if we
  ...
  kProcFace_NoiseMinLightness = Anki::Util::Clamp(
      kProcFace_NoiseMinLightness, 0.f, kProcFace_NoiseMaxLightness);
  kProcFace_NoiseMaxLightness = Anki::Util::Clamp(
      kProcFace_NoiseMaxLightness, kProcFace_NoiseMinLightness, 2.f);
  ...
  if (kProcFace_NoiseMinLightness_old != kProcFace_NoiseMinLightness ||
...
void ProceduralFaceDrawer::ApplyAntiAliasing(Vision::Image& shape, float minX,
  ...
  if (kProcFace_AntiAliasingSize > 0) {
    ...
    switch (kProcFace_AntiAliasingFilter) {
      ...
      case (uint8_t)Filter::GaussianFilter: {
        cv::GaussianBlur(
            shape.get_CvMat_(), shape.get_CvMat_(),
            cv::Size(kProcFace_AntiAliasingSize, kProcFace_AntiAliasingSize),
            (f32)kProcFace_AntiAliasingSize *
...
void ProceduralFaceDrawer::DrawEye(const ProceduralFace& faceData,
                                   WhichEye whichEye,
                                   const Matrix_3x3f* W_facePtr,
                                   Vision::Image& faceImg,
  ...
  // the right eye. So
  const f32 upLeftRadX =
      faceData.GetParameter(whichEye, Parameter::UpperOuterRadiusX) *
      halfEyeWidth;
  const f32 upLeftRadY =
      faceData.GetParameter(whichEye, Parameter::UpperOuterRadiusY) *
      halfEyeHeight;
  const f32 lowLeftRadX =
      faceData.GetParameter(whichEye, Parameter::LowerOuterRadiusX) *
      halfEyeWidth;
  const f32 lowLeftRadY =
      faceData.GetParameter(whichEye, Parameter::LowerOuterRadiusY) *
      halfEyeHeight;

  const f32 upRightRadX =
      faceData.GetParameter(whichEye, Parameter::UpperInnerRadiusX) *
      halfEyeWidth;
  const f32 upRightRadY =
      faceData.GetParameter(whichEye, Parameter::UpperInnerRadiusY) *
      halfEyeHeight;
  const f32 lowRightRadX =
      faceData.GetParameter(whichEye, Parameter::LowerInnerRadiusX) *
      halfEyeWidth;
  const f32 lowRightRadY =
      faceData.GetParameter(whichEye, Parameter::LowerInnerRadiusY) *
      halfEyeHeight;
  ...
  // 1. Eye shape poly
  {
    ...
    if (upRightRadX > 0 && upRightRadY > 0) {
      cv::ellipse2Poly(cv::Point(ROUND(halfEyeWidth - upRightRadX),
                                 ROUND(-halfEyeHeight + upRightRadY)),
                       cv::Size(upRightRadX, upRightRadY), 0, 270, 360,
                       kProcFace_EllipseDelta, segment);
    ...
    if (lowRightRadX > 0 && lowRightRadY > 0) {
      cv::ellipse2Poly(cv::Point(ROUND(halfEyeWidth - lowRightRadX),
                                 ROUND(halfEyeHeight - lowRightRadY)),
                       cv::Size(lowRightRadX, lowRightRadY), 0, 0, 90,
                       kProcFace_EllipseDelta, segment);
      eyePoly.insert(eyePoly.end(), segment.begin(), segment.end());
    } else {
      eyePoly.emplace_back(halfEyeWidth, halfEyeHeight);
    ...
    if (lowLeftRadX > 0 && lowLeftRadY > 0) {
      cv::ellipse2Poly(cv::Point(ROUND(-halfEyeWidth + lowLeftRadX),
                                 ROUND(halfEyeHeight - lowLeftRadY)),
                       cv::Size(lowLeftRadX, lowLeftRadY), 0, 90, 180,
                       kProcFace_EllipseDelta, segment);
      eyePoly.insert(eyePoly.end(), segment.begin(), segment.end());
    } else {
      eyePoly.emplace_back(-halfEyeWidth, halfEyeHeight);
    ...
    if (upLeftRadX > 0 && upLeftRadY > 0) {
      cv::ellipse2Poly(cv::Point(ROUND(-halfEyeWidth + upLeftRadX),
                                 ROUND(-halfEyeHeight + upLeftRadY)),
                       cv::Size(upLeftRadX, upLeftRadY), 0, 180, 270,
                       kProcFace_EllipseDelta, segment);
      eyePoly.insert(eyePoly.end(), segment.begin(), segment.end());
    } else {
      eyePoly.emplace_back(-halfEyeWidth, -halfEyeHeight);
  ...
  // 2. Lower lid poly
  {
    ...
    const f32 lowerLidY =
        faceData.GetParameter(whichEye, Parameter::LowerLidY) *
        static_cast<f32>(eyeHeight);
    const f32 angleDeg =
        faceData.GetParameter(whichEye, Parameter::LowerLidAngle);
    const f32 angleRad = DEG_TO_RAD(angleDeg);
    const f32 yAngleAdj = -halfEyeWidth * std::tan(angleRad);
    lowerLidPoly = {
        {(s32)ROUND(halfEyeWidth + 1.f),
         (s32)ROUND(halfEyeHeight - lowerLidY -
                    yAngleAdj)},  // Upper right corner
        {(s32)ROUND(halfEyeWidth + 1.f),
         (s32)ROUND(halfEyeHeight + 1.f)},  // Lower right corner
        {(s32)ROUND(-halfEyeWidth - 1.f),
         (s32)ROUND(halfEyeHeight + 1.f)},  // Lower left corner
        {(s32)ROUND(-halfEyeWidth - 1.f),
         (s32)ROUND(halfEyeHeight - lowerLidY +
                    yAngleAdj)},  // Upper left corner
    };
    // Add bend:
    const f32 yRad = faceData.GetParameter(whichEye, Parameter::LowerLidBend) *
                     static_cast<f32>(eyeHeight);
    if (yRad != 0) {
      const f32 xRad = ROUND(halfEyeWidth / std::cos(angleRad));
      cv::ellipse2Poly(cv::Point(0, ROUND(halfEyeHeight - lowerLidY)),
                       cv::Size(xRad, yRad), angleDeg, 180, 360,
                       kProcFace_EllipseDelta, segment);
      DEV_ASSERT(std::abs(segment.front().x - lowerLidPoly.back().x) < 3 &&
                     std::abs(segment.front().y - lowerLidPoly.back().y) < 3,
                 "First curved lower lid segment point not close to last lid "
                 "poly point.");
      DEV_ASSERT(std::abs(segment.back().x - lowerLidPoly.front().x) < 3 &&
                     std::abs(segment.back().y - lowerLidPoly.front().y) < 3,
                 "Last curved lower lid segment point not close to first lid "
                 "poly point.");
  ...
  // 3. Upper lid poly
  {
    ...
    const f32 upperLidY =
        faceData.GetParameter(whichEye, Parameter::UpperLidY) *
        static_cast<f32>(eyeHeight);
    const f32 angleDeg =
        faceData.GetParameter(whichEye, Parameter::UpperLidAngle);
    const f32 angleRad = DEG_TO_RAD(angleDeg);
    const f32 yAngleAdj = -halfEyeWidth * std::tan(angleRad);
    upperLidPoly = {
        {(s32)ROUND(-halfEyeWidth - 1.f),
         (s32)ROUND(-halfEyeHeight + upperLidY +
                    yAngleAdj)},  // Lower left corner
        {(s32)ROUND(-halfEyeWidth - 1.f),
         (s32)ROUND(-halfEyeHeight - 1.f)},  // Upper left corner
        {(s32)ROUND(halfEyeWidth + 1.f),
         (s32)ROUND(-halfEyeHeight - 1.f)},  // Upper right corner
        {(s32)ROUND(halfEyeWidth + 1.f),
         (s32)ROUND(-halfEyeHeight + upperLidY -
                    yAngleAdj)},  // Lower right corner
    };
    // Add bend:
    const f32 yRad = faceData.GetParameter(whichEye, Parameter::UpperLidBend) *
                     static_cast<f32>(eyeHeight);
    if (yRad != 0) {
      const f32 xRad = ROUND(halfEyeWidth / std::cos(angleRad));
      cv::ellipse2Poly(cv::Point(0, ROUND(-halfEyeHeight + upperLidY)),
                       cv::Size(xRad, yRad), angleDeg, 0, 180,
                       kProcFace_EllipseDelta, segment);
      DEV_ASSERT(std::abs(segment.front().x - upperLidPoly.back().x) < 3 &&
                     std::abs(segment.front().y - upperLidPoly.back().y) < 3,
                 "First curved upper lid segment point not close to last lid "
                 "poly point");
      DEV_ASSERT(std::abs(segment.back().x - upperLidPoly.front().x) < 3 &&
                     std::abs(segment.back().y - upperLidPoly.front().y) < 3,
                 "Last curved upper lid segment point not close to first lid "
                 "poly point");
  ...
  Point<2, Value> eyeCenter =
      (whichEye == WhichEye::Left)
          ? Point<2, Value>(ProceduralFace::GetNominalLeftEyeX(),
                            ProceduralFace::GetNominalEyeY())
          : Point<2, Value>(ProceduralFace::GetNominalRightEyeX(),
                            ProceduralFace::GetNominalEyeY());
  ...
  // This warp is a combination of the eye-specific parameters and full-face
  // parameters
  auto W = GetTransformationMatrix(
      faceData.GetParameter(whichEye, Parameter::EyeAngle),
      faceData.GetParameter(whichEye, Parameter::EyeScaleX),
      faceData.GetParameter(whichEye, Parameter::EyeScaleY), eyeCenter.x(),
      eyeCenter.y());
...
#if PROCEDURALFACE_GLOW_FEATURE
  const Value glowFraction = Util::Min(
      1.f, Util::Max(-1.f,
                     kProcFace_GlowSizeMultiplier *
                         faceData.GetParameter(whichEye, Parameter::GlowSize)));
  const SmallMatrix<2, 3, f32> W_glow = GetTransformationMatrix(
      faceData.GetParameter(whichEye, Parameter::EyeAngle),
      (1 + glowFraction) *
          faceData.GetParameter(whichEye, Parameter::EyeScaleX),
      (1 + glowFraction) *
  ...
  // Initialize bounding box corners at their opposite extremes. We will figure
  // out their true locations as we loop over the eyePoly below.
  ...
  // Warp the poly and the glow. Use the warped glow (which is a larger shape)
  // to compute the overall eye bounding box
  for (auto& point : eyePoly) {
    const Point<3, f32> pointF32{
        static_cast<f32>(whichEye == WhichEye::Left ? point.x : -point.x),
        static_cast<f32>(point.y), 1.f};
...
#if PROCEDURALFACE_GLOW_FEATURE
    // Use glow warp to figure out larger bounding box which contains glow as
    // well
  ...
  // Make sure the upper left and bottom right points are in bounds (note that
  // we loop over pixels below *inclusive* of the bottom right point, so we use
  // HEIGHT/WIDTH-1)

  // Note: visual artifacts can be seen with the NEON pathway, only slightly on
  // the top/left border, which
  //       is fixed by extending the ROI by half the filter size (as expected).
  //       However, they are very clearly visible on the bottom/right and only
  //       disappear by extending the ROI by kProcFace_AntiAliasingSize.

  upperLeft.x() =
      std::max(0.f, upperLeft.x() - kProcFace_AntiAliasingSize * .5f);
  upperLeft.y() =
      std::max(0.f, upperLeft.y() - kProcFace_AntiAliasingSize * .5f);
  bottomRight.x() = std::min((f32)(ProceduralFace::WIDTH - 1),
                             bottomRight.x() + kProcFace_AntiAliasingSize);
  bottomRight.y() = std::min((f32)(ProceduralFace::HEIGHT - 1),
                             bottomRight.y() + kProcFace_AntiAliasingSize);

  // Create the bounding that we're returning from the upperLeft and bottomRight
  // points
  ...
  // only render if the eyes are large enough, scale is handled in the calling
  // function
  if (eyeWidth > 0 && eyeHeight > 0) {
    // The hotspot center params leave the hot spot at the eye center if zero.
    // If non-zero, they shift left/right/up/down where a magnitude of 1.0 moves
    // the center to the extreme edge of the eye shape
    const Point2f hotSpotCenter =
        W *
        Point3f(0.5f * eyeWidth *
                    faceData.GetParameter(whichEye, Parameter::HotSpotCenterX),
                0.5f * eyeHeight *
                    faceData.GetParameter(whichEye, Parameter::HotSpotCenterY),
                1.f);

    // Inner Glow = the brighter glow at the center of the eye that falls off
    // radially towards the edge of the eye Outer Glow = the "halo" effect
    // around the outside of the eye shape Add inner glow to the eye shape,
    // before we compute the outer glow, so that boundaries conditions match.
    if (kProcFace_HotspotRender) {
      ...
      // Compute the 2x2 inverse covariance matrix for the hotspot Gaussian
      // falloff, incorporating the scale and rotation from the eye and face
      // warp (so it moves and rotates with the eyes)
      ...
      DEV_ASSERT_MSG(
          upperLeft.y() >= 0 && bottomRight.y() < _eyeShape.GetNumRows(),
          "ProceduralFaceDrawer.DrawEye.BadRow", "%f %f", upperLeft.y(),
          bottomRight.y());
      DEV_ASSERT_MSG(
          upperLeft.x() >= 0 && bottomRight.x() < _eyeShape.GetNumCols(),
          "ProceduralFaceDrawer.DrawEye.BadCol", "%f %f", upperLeft.x(),
          bottomRight.x());
      ...
      for (s32 i = upperLeft.y(); i <= bottomRight.y(); ++i) {
        ...
        for (s32 j = upperLeft.x(); j <= bottomRight.x(); ++j) {
          ...
          if (insideEye) {
            ...
            DEV_ASSERT_MSG(
                Util::InRange(falloff, 0.f, 1.f),
                "ProceduralFaceDrawer.DrawEye.BadInnerGlowFalloffValue", "%f",
                falloff);

            eyeValue = NUMERIC_CAST_CLAMPED(
                ROUND(static_cast<f32>(eyeValue) * falloff));
          }
        }
      }
    }
...
#if PROCEDURALFACE_GLOW_FEATURE
    Rectangle<s32> eyeBoundingBoxS32(upperLeft.CastTo<s32>(),
                                     bottomRight.CastTo<s32>());
    ...
    const Value glowLightness =
        kProcFace_GlowLightnessMultiplier *
        faceData.GetParameter(whichEye, Parameter::GlowLightness);
    if (Util::IsFltGTZero(glowLightness) && Util::IsFltGTZero(glowFraction)) {
      ...
      switch (kProcFace_GlowFilter) {
        case (uint8_t)Filter::BoxFilter:
          cv::boxFilter(eyeShapeROI.get_CvMat_(), glowImgROI.get_CvMat_(), -1,
                        cv::Size(glowSizeX, glowSizeY));
          break;
        case (uint8_t)Filter::GaussianFilter:
          cv::GaussianBlur(eyeShapeROI.get_CvMat_(), glowImgROI.get_CvMat_(),
                           cv::Size(glowSizeX, glowSizeY), (f32)glowSizeX,
                           (f32)glowSizeY);
          break;
      ...
      if (kProcFace_EnableAntiAliasing) {
        // Antialiasing (AFTER glow because it changes eyeShape, which we use to
        // compute the glow above)

        // Optimization: if we're applying interpolation and transforming the
        // face, skip antialiasing?
        ApplyAntiAliasing(_eyeShape, upperLeft.x(), upperLeft.y(),
                          bottomRight.x(), bottomRight.y());
      }

      const f32 eyeLightness =
          faceData.GetParameter(whichEye, Parameter::Lightness);
      DEV_ASSERT(Util::InRange(eyeLightness, -1.f, 1.f),
                 "ProceduralFaceDrawer.DrawEye.InvalidLightness");

      // Draw the eye into the face image, adding outer glow, noise, and
      // stylized scanlines
      {
        ANKI_CPU_PROFILE("DrawEyePixels");

        DEV_ASSERT_MSG(
            upperLeft.y() >= 0 && bottomRight.y() < faceImg.GetNumRows(),
            "ProceduralFaceDrawer.DrawEye.BadRow", "%f %f", upperLeft.y(),
            bottomRight.y());
        DEV_ASSERT_MSG(
            upperLeft.x() >= 0 && bottomRight.x() < faceImg.GetNumCols(),
            "ProceduralFaceDrawer.DrawEye.BadCol", "%f %f", upperLeft.x(),
            bottomRight.x());
        for (s32 i = upperLeft.y(); i <= bottomRight.y(); ++i) {
          ...
          const bool somethingToDraw = (eyeValue > 0);
...
#if PROCEDURALFACE_GLOW_FEATURE
              // Combine everything together: inner glow falloff, and the glow
              // value. Note that the value in glowImg/eyeShape is already
              // [0,255]
              newValue = std::max(newValue, eyeValue);

              const bool isPartOfEye =
                  (eyeValue >= glowValue);  // (and not part of glow)
              if (isPartOfEye) {
            ...
            newValue *= kProcFace_EyeLightnessMultiplier;
              ...
              // Note: If we're drawing the right eye, there may already be
              // something in the image
              //       from when we drew the left eye (e.g. with large glow), so
              //       use max
              faceImg_i[j] =
                  std::max(faceImg_i[j], NUMERIC_CAST_CLAMPED(ROUND(newValue)));
            }
          }
        }
      }
  ...
  void ProceduralFaceDrawer::DrawFace(const ProceduralFace& faceData,
    ...
    bool dirty =
        false;  // set to true to force all stages to render, previous pipeline
    dirty = DrawEyes(faceData, dirty);
    dirty = ApplyScanlines(_faceCache.img8[_faceCache.finalFace],
                           faceData.GetScanlineOpacity(), dirty);
    dirty = DistortScanlines(faceData, dirty);
    dirty = ApplyNoise(rng, dirty);
    dirty = ConvertColorspace(faceData, output, dirty);
  }  // DrawFace()

  bool ProceduralFaceDrawer::DrawEyes(const ProceduralFace& faceData,
                                      bool dirty) {
    ANKI_CPU_PROFILE("DrawEyes");

    if (!dirty) {
      if (_faceCache.faceData.GetParameters(WhichEye::Left) !=
              faceData.GetParameters(WhichEye::Left) ||
          _faceCache.faceData.GetParameters(WhichEye::Right) !=
              faceData.GetParameters(WhichEye::Right) ||
          ...
          !Util::IsFltNear(_faceCache.faceData.GetSaturation(),
                           faceData.GetSaturation())) {
    ...
    if (dirty) {
      ...
      _faceCache.faceData.SetParameters(WhichEye::Left,
                                        faceData.GetParameters(WhichEye::Left));
      _faceCache.faceData.SetParameters(
          WhichEye::Right, faceData.GetParameters(WhichEye::Right));
      ...
      DEV_ASSERT(_faceCache.finalFace < _faceCache.kSize,
                 "ProceduralFaceDrawer.DistortScanlines.FaceCacheTooSmall");
      _faceCache.img8[_faceCache.eyes].Allocate(
          ProceduralFace::HEIGHT,
          ProceduralFace::WIDTH);  // Will do nothing if already the right size
      _faceCache.img8[_faceCache.eyes].FillWith(0);

      // Create a full-face warp matrix if needed and provide it to the
      // eye-rendering call
      const Matrix_3x3f* W_facePtr = nullptr;
      Matrix_3x3f W_face;
      const bool hasFaceTransform =
          (!Util::IsNearZero(_faceCache.faceData.GetFaceAngle()) ||
           !(_faceCache.faceData.GetFacePosition() == 0) ||
           !(_faceCache.faceData.GetFaceScale() == 1.f));
      if (hasFaceTransform) {
        W_face = GetTransformationMatrix(
            faceData.GetFaceAngle(), faceData.GetFaceScale().x(),
            faceData.GetFaceScale().y(), faceData.GetFacePosition().x(),
      ...
      DrawEye(_faceCache.faceData, WhichEye::Left, W_facePtr,
              _faceCache.img8[_faceCache.eyes], _leftBBox);
      DrawEye(_faceCache.faceData, WhichEye::Right, W_facePtr,
              _faceCache.img8[_faceCache.eyes], _rightBBox);

      const std::array<Quad2f, 2> leftRightQuads{
          {Quad2f(_leftBBox), Quad2f(_rightBBox)}};
  ...
  bool ProceduralFaceDrawer::DistortScanlines(const ProceduralFace& faceData,
                                              bool dirty) {
    ...
    auto scanlineDistorter = faceData.GetScanlineDistorter();
    if (nullptr != scanlineDistorter) {
      // Any scanline distorter affects the output image, assigned a new element
      // in the face cache and make all later stages needed updates

      // Note: scanline distortion has changed from modifying the input image to
      // generating a new
      //       output image, this allows eye rendering and face transforms to be
      //       cached and the scanline distortion applied to the original output
      //       rather than a face that has already had scanline distortion
      //       applied.
      dirty = true;

      _faceCache.finalFace = _faceCache.distortedFace = _faceCache.eyes + 1;
      DEV_ASSERT(_faceCache.finalFace < _faceCache.kSize,
                 "ProceduralFaceDrawer, face cache too small.");
      _faceCache.img8[_faceCache.distortedFace].Allocate(ProceduralFace::HEIGHT,
                                                         ProceduralFace::WIDTH);
      ...
      // No scanline distortion, pass forward the cached face transform as
      // output
  ...
  bool ProceduralFaceDrawer::ApplyNoise(const Util::RandomGenerator& rng,
                                        bool dirty) {
    if (PROCEDURALFACE_NOISE_FEATURE && (kProcFace_NoiseNumFrames > 0)) {
      ...
      DEV_ASSERT(_faceCache.finalFace < _faceCache.kSize,
                 "ProceduralFaceDrawer, face cache too small.");
      _faceCache.img8[_faceCache.finalFace].Allocate(ProceduralFace::HEIGHT,
                                                     ProceduralFace::WIDTH);
      if (dirty) {
      ...
      // If the eyes haven't changed, the face hasn't changed and no-scanline
      // distorter then there is no work to do until the noise stage

      // TODO: https://ankiinc.atlassian.net/browse/VIC-3647 Apply noise to eyes
      // individually rather than entire face ^ might not be desirable with the
      // neon version. May incur cache penalty when pulling images from memory
      ...
      for (s32 i = _faceRowMin; i <= _faceRowMax; ++i) {
        ...
        const u8* eyeShape_i =
            _faceCache.img8[_faceCache.distortedFace].GetRow(i);
        ...
        for (; j <= _faceColMax - (kNumElementsProcessed - 1);
             j += kNumElementsProcessed) {
        ...
        for (; j <= _faceColMax; ++j) {
          *faceImg_i = Util::numeric_cast_clamped<u8>(
              (static_cast<u16>(*eyeShape_i) * static_cast<u16>(*noiseImg_i)) >>
              7);
          ++noiseImg_i;
          ++eyeShape_i;
          ++faceImg_i;
  ...
  bool ProceduralFaceDrawer::ConvertColorspace(
      const ProceduralFace& faceData, Vision::ImageRGB565& output, bool dirty) {
    ...
    if (dirty) {
      ...
      DEV_ASSERT(Util::InRange(hueFactor, 0.f, 1.f),
                 "ProceduralFaceDrawer.DrawEye.InvalidHue");
...
#endif
      DEV_ASSERT(Util::InRange(satFactor, -1.f, 1.f),
                 "ProceduralFaceDrawer.DrawEye.InvalidSaturation");
      const u8 drawSat = ROUND(255.f * satFactor);

      // ... otherwise convert final image, limited by bounding box, to RGB565
      Rectangle<s32> eyesROI(_faceColMin, _faceRowMin,
                             _faceColMax - _faceColMin + 1,
                             _faceRowMax - _faceRowMin + 1);
      Vision::ImageRGB565 roi = output.GetROI(eyesROI);
      _faceCache.img8[_faceCache.finalFace].GetROI(eyesROI).ConvertV2RGB565(
          drawHue, drawSat, roi);
    }
  ...
  bool ProceduralFaceDrawer::GetNextBlinkFrame(ProceduralFace & faceData,
    ...
    static const std::vector<BlinkParams> blinkParams{
        ...
        {.height = .15f,
         .width = 2.f,
         .t = 33,
         .blinkState = BlinkState::JustOpened},
        {.height = .7f,
         .width = 1.2f,
         .t = 33,
    ...
    if (paramIter == blinkParams.end()) {
      ...
      for (auto whichEye : {WhichEye::Left, WhichEye::Right}) {
        faceData.SetParameter(
            whichEye, Parameter::EyeScaleX,
            originalFace.GetParameter(whichEye, Parameter::EyeScaleX) *
                paramIter->width);
        faceData.SetParameter(
            whichEye, Parameter::EyeScaleY,
            originalFace.GetParameter(whichEye, Parameter::EyeScaleY) *
                paramIter->height);
      }
      ...
      switch (paramIter->blinkState) {
        case BlinkState::Closed: {
          ...
          const ProceduralFace::Value blinkHeight =
              (originalFace.GetParameter(WhichEye::Left,
                                         Parameter::EyeCenterY) +
               originalFace.GetParameter(WhichEye::Right,
                                         Parameter::EyeCenterY)) /
              2;
        ...
        case BlinkState::JustOpened: {
          ...
          for (auto whichEye : {WhichEye::Left, WhichEye::Right}) {
            faceData.SetParameter(
                whichEye, Parameter::EyeCenterY,
                originalFace.GetParameter(whichEye, Parameter::EyeCenterY));
            for (auto lidParam : lidParams) {
              faceData.SetParameter(
                  whichEye, lidParam,
                  originalFace.GetParameter(whichEye, lidParam));
            }
          }
          break;
        }
  ...
  bool ProceduralFaceDrawer::ApplyScanlines(Vision::ImageRGB & imageHsv,
                                            const float opacity, bool dirty) {
#if PROCEDURALFACE_SCANLINE_FEATURE
    if (kProcFace_Scanlines) {
      ...
      if (applyScanlines) {
        DEV_ASSERT(Util::InRange(opacity, 0.f, 1.f),
                   "ProceduralFaceDrawer.ApplyScanlines.InvalidOpacity");
  ...
  bool ProceduralFaceDrawer::ApplyScanlines(Vision::Image & image8,
                                            const float opacity, bool dirty) {
#if PROCEDURALFACE_SCANLINE_FEATURE
    if (kProcFace_Scanlines) {
      ANKI_CPU_PROFILE("ApplyScanlines");
