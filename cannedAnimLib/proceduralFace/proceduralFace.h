...
#define PROCEDURALFACE_NOISE_FEATURE \
  1  // feature capable and enabled as num frames = 5
#define PROCEDURALFACE_ANIMATED_SATURATION \
  0  // disable saturation in canned animations
#define PROCEDURALFACE_PROCEDURAL_SATURATION \
  1  // only take saturation from the C++ API
#define PROCEDURALFACE_GLOW_FEATURE 0
#define PROCEDURALFACE_SCANLINE_FEATURE \
  0  // feature capable but disabled by default as kProcFace_Scanlines = false
...
class ProceduralFace {
 public:
  ...
  // These values are based off of V1 parameters but scaled up by a ratio of V2
  // dimensions : V1 dimensions (roughly 1.43x) V1 width: 128   New: 184
  // => 1.43x increase V1 height: 64   New:  96  => 1.5x  increase
  ...
  using EyeParamArray =
      std::array<Value, static_cast<size_t>(Parameter::NumParameters)>;

  // Note: SCREEN Left and Right, not Cozmo's left and right!!!!
  enum WhichEye { Left, Right };
  ...
  // Reset parameters to their nominal values. If !withBlankFace, uses the face
  // passed to SetResetData. If withBlankFace, uses the face passed to
  // SetBlankFaceData
  ...
  void SetFromValues(const std::vector<f32>& leftEyeData,
                     const std::vector<f32>& rightEyeData, f32 faceAngle_deg,
                     f32 faceCenterX, f32 faceCenterY, f32 faceScaleX,
  ...
  // Get/Set the overall angle of the whole face (still using parameter on
  // interval [-1,1]
  ...
  // Get a pointer that encapsulates the procedural face's hue and saturation
  // images
  ...
  // Get ScanlineDistortion component. Returns nullptr if there is no scanline
  // distortion (i.e. if InitScanlineDistortion has not been called)
  ScanlineDistorter* GetScanlineDistorter() { return _scanlineDistorter.get(); }
  const ScanlineDistorter* GetScanlineDistorter() const {
    return _scanlineDistorter.get();
  }
  ...
  void Interpolate(const ProceduralFace& face1, const ProceduralFace& face2,
                   float fraction, bool usePupilSaccades = false);
  ...
  //  - lookUpMaxScale controls how big the eyes get when looking up (negative
  //  y)
  //  - lookDownMinScale controls how small the eyes get when looking down
  //  (positive y)
  //  - outerEyeScaleIncrease controls the differentiation between inner/outer
  //  eye height
  //    when looking left or right
  void LookAt(f32 x, f32 y, f32 xmax, f32 ymax, f32 lookUpMaxScale = 1.1f,
              f32 lookDownMinScale = 0.85f, f32 outerEyeScaleIncrease = 0.1f);
  ...
  // Get the bounding edge of the current eyes in screen pixel space, at their
  // current size and position, without taking into account the current
  // FacePosition (a.k.a. face center) or face angle.
 ...
 private:
  ...
  void CombineEyeParams(EyeParamArray& eyeArray0,
  ...
  template <class T>
  void AddConsoleVar(T& var, const char* name, const char* group,
...
inline void ProceduralFace::SetParameter(WhichEye whichEye, Parameter param,
                                         Value value) {
  _eyeParams[whichEye][static_cast<size_t>(param)] =
      Clip(whichEye, param, value);
}

inline ProceduralFace::Value ProceduralFace::GetParameter(
    WhichEye whichEye, Parameter param) const {
  return _eyeParams[whichEye][static_cast<size_t>(param)];
}

inline const ProceduralFace::EyeParamArray& ProceduralFace::GetParameters(
    WhichEye whichEye) const {
  return _eyeParams[whichEye];
}

inline void ProceduralFace::SetParameters(WhichEye eye,
                                          const EyeParamArray& params) {
...
inline Point<2, ProceduralFace::Value> const& ProceduralFace::GetFacePosition()
    const {
  return _faceCenter;
}
...
inline Point<2, ProceduralFace::Value> const& ProceduralFace::GetFaceScale()
    const {
  return _faceScale;
}
...
inline void ProceduralFace::SetHue(Value hue) {
  ...
  GetHueImage().FillWith(
      static_cast<u8>(_hue * std::numeric_limits<u8>::max()));
}

inline ProceduralFace::Value ProceduralFace::GetHue() { return _hue; }

inline void ProceduralFace::ResetHueToDefault() { _hue = DefaultHue; }
...
inline Vision::Image& ProceduralFace::GetHueImage() { return _hueImage; }

inline Vision::Image* ProceduralFace::GetHueImagePtr() { return &_hueImage; }

inline Vision::Image& ProceduralFace::GetSaturationImage() { return _satImage; }
