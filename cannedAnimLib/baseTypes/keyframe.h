...
class IKeyFrame {
 public:
  ...
  // Returns the timestamp at which the keyframe has finished performing some
  // action on the robot NOTE: This is NOT the last timestamp when the keyframe
  // sends a message, but the last timestamp when whatever action the keyframe
  // started actually finishes E.G. The lift keyframe sends a single message,
  // but if the lift motion lasts 2000 ms, the keyframe should return 2000_ms
  // from this function so that if the track is a single keyframe long the
  // animation doesn't immediately complete and potentially stop the motion
  // early
  ...
  // is playing in
  void SetTriggerTime_ms(TimeStamp_t triggerTime_ms) {
    _triggerTime_ms = triggerTime_ms;
  }

  // Set all members from Json or FlatBuffers. Calls virtual
  // SetMembersFromJson() method so subclasses can specify how to populate their
  // members. Second argument is used to print nicer debug strings if something
  // goes wrong
  ...
  // if not available.
  virtual RobotInterface::EngineToRobot* GetStreamMessage(
      const TimeStamp_t timeSinceAnimStart_ms) const = 0;
#endif
 ...
 protected:
  ...
  virtual Result SetMembersFromJson(const Json::Value& jsonRoot,
                                    const std::string& animNameDebug = "") = 0;

  TimeStamp_t GetTimeSinceTrigger(
      const TimeStamp_t timeSinceAnimStart_ms) const {
    return (timeSinceAnimStart_ms > GetTriggerTime_ms())
               ? (timeSinceAnimStart_ms - GetTriggerTime_ms())
               : 0;
  }
  ...
  // The trigger time is protected instead of private so derived classes can
  // access it.
  TimeStamp_t _triggerTime_ms = 0;

 private:
  // A random number generator for all keyframes to share (for adding
  // variability)
  static Util::RandomGenerator sRNG;

};  // class IKeyFrame

inline Util::RandomGenerator& IKeyFrame::GetRNG() const { return sRNG; }
...
class HeadAngleKeyFrame : public IKeyFrame {
 public:
  HeadAngleKeyFrame() {}
  HeadAngleKeyFrame(s8 angle_deg, u8 angle_variability_deg,
  ...
  Result DefineFromFlatBuf(const CozmoAnim::HeadAngle* headAngleKeyframe,
                           const std::string& animNameDebug);

#if CAN_STREAM
  virtual RobotInterface::EngineToRobot* GetStreamMessage(
      const TimeStamp_t timeSinceAnimStart_ms) const override;
#endif
 ...
 protected:
  virtual Result SetMembersFromJson(
      const Json::Value& jsonRoot,
      const std::string& animNameDebug = "") override;
  virtual Result SetMembersFromFlatBuf(
      const CozmoAnim::HeadAngle* headAngleKeyframe,
      const std::string& animNameDebug = "");

  virtual TimeStamp_t GetKeyframeDuration_ms() const override {
    return _keyframeActiveDuration_ms;
  }
...
class LiftHeightKeyFrame : public IKeyFrame {
 public:
  LiftHeightKeyFrame() {}
  LiftHeightKeyFrame(u8 height_mm, u8 heightVariability_mm,
                     TimeStamp_t duration_ms);
  ...
  Result DefineFromFlatBuf(const CozmoAnim::LiftHeight* liftHeightKeyframe,
                           const std::string& animNameDebug);
 ...
 protected:
  virtual Result SetMembersFromJson(
      ...
      const std::string& animNameDebug = "") override;
  virtual Result SetMembersFromFlatBuf(
      const CozmoAnim::LiftHeight* liftHeightKeyframe,
      const std::string& animNameDebug = "");
...
class RobotAudioKeyFrame : public IKeyFrame {
 public:
  ...
  Result DefineFromFlatBuf(const CozmoAnim::RobotAudio* audioKeyframe,
                           const std::string& animNameDebug);
  ...
  const AudioRefList& GetAudioReferencesList() const {
    return _audioReferences;
  }
 ...
 protected:
  virtual Result SetMembersFromJson(
      const Json::Value& jsonRoot,
      const std::string& animNameDebug = "") override;
  Result SetMembersFromDeprecatedJson(const Json::Value& jsonRoot,
                                      const std::string& animNameDebug = "");
  virtual Result SetMembersFromFlatBuf(
      const CozmoAnim::RobotAudio* audioKeyframe,
      const std::string& animNameDebug = "");
...
class ProceduralFaceKeyFrame : public IKeyFrame {
 public:
  ProceduralFaceKeyFrame(TimeStamp_t triggerTime_ms = 0,
                         TimeStamp_t durationTime_ms = 0);
  ProceduralFaceKeyFrame(const ProceduralFace& face,
  ...
  Result DefineFromFlatBuf(const CozmoAnim::ProceduralFace* procFaceKeyframe,
                           const std::string& animNameDebug);

#if CAN_STREAM
  // Always returns nullptr. Use GetInterpolatedFace() to get the face stored in
  // this keyframe.
  virtual RobotInterface::EngineToRobot* GetStreamMessage(
      const TimeStamp_t timeSinceAnimStart_ms) const override {
    return nullptr;
  ...
  RobotInterface::EngineToRobot* GetInterpolatedStreamMessage(
      const ProceduralFaceKeyFrame& nextFrame);

  // Returns the interpolated face between the current keyframe and the next.
  // If the nextFrame is nullptr, then this frame's procedural face are
  // returned.
  ProceduralFace GetInterpolatedFace(const ProceduralFaceKeyFrame& nextFrame,
                                     const TimeStamp_t currentTime_ms);

  void SetKeyframeActiveDuration_ms(TimeStamp_t activeDuration_ms) {
    _keyframeActiveDuration_ms = activeDuration_ms;
  }
 ...
 protected:
  ...
  virtual TimeStamp_t GetKeyframeDuration_ms() const override {
    return _keyframeActiveDuration_ms == 0 ? ANIM_TIME_STEP_MS
                                           : _keyframeActiveDuration_ms;
  }
...
inline ProceduralFaceKeyFrame::ProceduralFaceKeyFrame(
    TimeStamp_t triggerTime, TimeStamp_t durationTime_ms) {
  SetTriggerTime_ms(triggerTime);
  _keyframeActiveDuration_ms = durationTime_ms;
}

inline ProceduralFaceKeyFrame::ProceduralFaceKeyFrame(
    const ProceduralFace& face, TimeStamp_t triggerTime,
...
class EventKeyFrame : public IKeyFrame {
 public:
  ...
  Result DefineFromFlatBuf(const CozmoAnim::Event* eventKeyframe,
                           const std::string& animNameDebug);
 ...
 protected:
  virtual Result SetMembersFromJson(
      const Json::Value& jsonRoot,
      const std::string& animNameDebug = "") override;
  virtual Result SetMembersFromFlatBuf(const CozmoAnim::Event* eventKeyframe,
                                       const std::string& animNameDebug = "");
...
class BackpackLightsKeyFrame : public IKeyFrame {
 public:
  ...
  Result DefineFromFlatBuf(CozmoAnim::BackpackLights* backpackKeyframe,
                           const std::string& animNameDebug);
 ...
 protected:
  virtual Result SetMembersFromJson(
      ...
      const std::string& animNameDebug = "") override;
  virtual Result SetMembersFromFlatBuf(
      CozmoAnim::BackpackLights* backpackKeyframe,
      const std::string& animNameDebug = "");
...
class BodyMotionKeyFrame : public IKeyFrame {
 public:
  ...
  Result DefineFromFlatBuf(const CozmoAnim::BodyMotion* bodyKeyframe,
                           const std::string& animNameDebug);
  ...
  Result ProcessRadiusString(const std::string& radiusStr,
                             const std::string& animNameDebug);
 ...
 protected:
  virtual Result SetMembersFromJson(
      ...
      const std::string& animNameDebug = "") override;
  virtual Result SetMembersFromFlatBuf(
      const CozmoAnim::BodyMotion* bodyKeyframe,
      const std::string& animNameDebug = "");
...
// A TurnToRecordedHeadingKeyFrame commands the robot to turn to the heading
// that was previously recorded by a RecordHeadingKeyFrame
class TurnToRecordedHeadingKeyFrame : public IKeyFrame {
 public:
  TurnToRecordedHeadingKeyFrame();
  TurnToRecordedHeadingKeyFrame(s16 offset_deg, s16 speed_degPerSec,
                                s16 accel_degPerSec2, s16 decel_degPerSec2,
                                u16 tolerance_deg, u16 numHalfRevs,
                                bool useShortestDir, s32 duration_ms);
