...
class Track {
 public:
  ...
  //
  // Default copy constructor and copy assignment operator do not do the right
  // thing for std::list iterators. When a std::list is copied, any associated
  // iterator must be updated to point to the same location in the new location.
  // The default member-wise copy produces an invalid iterator.
  //
  ...
  Track<FRAME_TYPE>(const Track<FRAME_TYPE>& other)
      : _frames(other._frames), _frameIter(_frames.begin()) {
  ...
  // Define from JSON. Second argument is used to print nicer debug strings if
  // something goes wrong.
  Result AddKeyFrameToBack(const Json::Value& jsonRoot,
                           const std::string& animNameDebug = "");

  // Define from FlatBuffers. Second argument is used to print nicer debug
  // strings if something goes wrong.
  // TODO: Reduce some code duplication in this method (COZMO-8766). Rather than
  // have all the different types
  //       for the different FlatBuffers-generated classes, can we use a single
  //       AddFlatBufKeyFrameToBack() method that has the type of the
  //       FlatBuffers class as an additional template argument?
  Result AddKeyFrameToBack(const CozmoAnim::LiftHeight* liftKeyframe,
                           const std::string& animNameDebug = "");
  Result AddKeyFrameToBack(const CozmoAnim::ProceduralFace* procFaceKeyframe,
                           const std::string& animNameDebug = "");
  Result AddKeyFrameToBack(const CozmoAnim::HeadAngle* headKeyframe,
                           const std::string& animNameDebug = "");
  Result AddKeyFrameToBack(const CozmoAnim::RobotAudio* audioKeyframe,
                           const std::string& animNameDebug = "");
  Result AddKeyFrameToBack(const CozmoAnim::Event* eventKeyframe,
                           const std::string& animNameDebug = "");
  Result AddKeyFrameToBack(const CozmoAnim::BodyMotion* bodyKeyframe,
                           const std::string& animNameDebug = "");
  Result AddKeyFrameToBack(
  ...
  // left in the track.
  RobotInterface::EngineToRobot* GetCurrentStreamingMessage(
      const TimeStamp_t relativeStreamingTime_ms) const;
  ...
  // Get pointer to next keyframe. Returns nullptr if the track is on the last
  // frame.
  const FRAME_TYPE* GetNextKeyFrame() const;

  // Get a pointer to the first KeyFrame in the track. Returns nullptr if track
  // is empty.
  const FRAME_TYPE* GetFirstKeyFrame() const;

  // Get a pointer to the last KeyFrame in the track. Returns nullptr if track
  // is empty.
  ...
  void AppendTrack(const Track& appendTrack,
                   const TimeStamp_t appendStartTime_ms);

  // Moves the frameIter forward to the keyframe that should be set for the
  // given time NOTE: This function only moves the track forward
 ...
 private:
  ...
  Result AddKeyFrameToBackHelper(const FRAME_TYPE& keyFrame,
                                 FRAME_TYPE*& prevKeyFrame);
  Result AddKeyFrameByTimeHelper(const FRAME_TYPE& keyFrame,
                                 FRAME_TYPE*& prevKeyFrame);
...
const FRAME_TYPE* Track<FRAME_TYPE>::GetNextKeyFrame() const {
  DEV_ASSERT(_frameIter != _frames.end(),
             "Frame iterator should not be at end");
...
Result Track<FRAME_TYPE>::AddKeyFrameToBackHelper(const FRAME_TYPE& keyFrame,
  ...
  if (_frames.size() > ConstMaxFramesPerTrack()) {
    PRINT_NAMED_WARNING("Animation.Track.AddKeyFrameToBack.TooManyFrames",
                        "There are already %zu frames in track of type %s. "
                        "Refusing to add more.",
...
Result Track<FRAME_TYPE>::AddKeyFrameByTimeHelper(const FRAME_TYPE& keyFrame,
  ...
  if (_frames.size() > ConstMaxFramesPerTrack()) {
    PRINT_NAMED_WARNING(
        "Animation.Track.AddKeyFrameByTime.TooManyFrames",
  ...
  while (framePlaceIter != _frames.end() &&
         framePlaceIter->GetTriggerTime_ms() <= desiredTrigger) {
...
template <typename FRAME_TYPE>
RobotInterface::EngineToRobot* Track<FRAME_TYPE>::GetCurrentStreamingMessage(
    const TimeStamp_t relativeStreamingTime_ms) const {
...
template <typename FRAME_TYPE>
Result Track<FRAME_TYPE>::AddKeyFrameToBack(
    const CozmoAnim::LiftHeight* liftKeyframe,
    const std::string& animNameDebug) {
  FRAME_TYPE newKeyFrame;
  Result lastResult =
      newKeyFrame.DefineFromFlatBuf(liftKeyframe, animNameDebug);
...
Result Track<FRAME_TYPE>::AddKeyFrameToBack(
  ...
  FRAME_TYPE newKeyFrame;
  Result lastResult =
      newKeyFrame.DefineFromFlatBuf(procFaceKeyframe, animNameDebug);
  if (RESULT_OK != lastResult) {
    return lastResult;
  }
...
template <typename FRAME_TYPE>
Result Track<FRAME_TYPE>::AddKeyFrameToBack(
    const CozmoAnim::HeadAngle* headKeyframe,
    const std::string& animNameDebug) {
  FRAME_TYPE newKeyFrame;
  Result lastResult =
      newKeyFrame.DefineFromFlatBuf(headKeyframe, animNameDebug);
  if (RESULT_OK != lastResult) {
    return lastResult;
  }
  return AddNewKeyFrameToBack(newKeyFrame);
...
Result Track<FRAME_TYPE>::AddKeyFrameToBack(
  ...
  FRAME_TYPE newKeyFrame;
  Result lastResult =
      newKeyFrame.DefineFromFlatBuf(audioKeyframe, animNameDebug);
  if (RESULT_OK != lastResult) {
    return lastResult;
  }
  return AddNewKeyFrameToBack(newKeyFrame);
...
Result Track<FRAME_TYPE>::AddKeyFrameToBack(
  ...
  FRAME_TYPE newKeyFrame;
  Result lastResult =
      newKeyFrame.DefineFromFlatBuf(eventKeyframe, animNameDebug);
  if (RESULT_OK != lastResult) {
    return lastResult;
  }
  return AddNewKeyFrameToBack(newKeyFrame);
...
Result Track<FRAME_TYPE>::AddKeyFrameToBack(
  ...
  FRAME_TYPE newKeyFrame;
  Result lastResult =
      newKeyFrame.DefineFromFlatBuf(bodyKeyframe, animNameDebug);
  if (RESULT_OK != lastResult) {
    return lastResult;
  }
  return AddNewKeyFrameToBack(newKeyFrame);
...
Result Track<FRAME_TYPE>::AddKeyFrameToBack(
  ...
  FRAME_TYPE newKeyFrame;
  Result lastResult =
      newKeyFrame.DefineFromFlatBuf(recordHeadingKeyframe, animNameDebug);
  if (RESULT_OK != lastResult) {
    return lastResult;
  }
  return AddNewKeyFrameToBack(newKeyFrame);
...
Result Track<FRAME_TYPE>::AddKeyFrameToBack(
    ...
    const std::string& animNameDebug) {
  FRAME_TYPE newKeyFrame;
  Result lastResult = newKeyFrame.DefineFromFlatBuf(
      turnToRecordedHeadingKeyframe, animNameDebug);
  if (RESULT_OK != lastResult) {
    return lastResult;
  }
  return AddNewKeyFrameToBack(newKeyFrame);
...
template <typename FRAME_TYPE>
Result Track<FRAME_TYPE>::AddKeyFrameToBack(const Json::Value& jsonRoot,
                                            const std::string& animNameDebug) {
  FRAME_TYPE newKeyFrame;
...
Result Track<FRAME_TYPE>::AddNewKeyFrameToBack(const FRAME_TYPE& newKeyFrame) {
  ...
  if (lastResult == RESULT_OK) {
    if (_frames.size() > 1) {
      ...
      if (_frames.back().GetTriggerTime_ms() <=
          nextToLastFrame->GetTriggerTime_ms()) {
        PRINT_NAMED_WARNING(
            "Animation.Track.AddKeyFrameToBack.BadTriggerTime",
            "New keyframe (t=%d) must be after the last keyframe (t=%d)",
            _frames.back().GetTriggerTime_ms(),
            nextToLastFrame->GetTriggerTime_ms());
...
template <class FRAME_TYPE>
void Track<FRAME_TYPE>::AppendTrack(const Track<FRAME_TYPE>& appendTrack,
                                    const TimeStamp_t appendStartTime_ms) {
    ...
    if (ANKI_DEV_CHEATS) {
      if (GetNextKeyFrame() != nullptr) {
        if ((_frameIter->GetTimestampActionComplete_ms() > toTime_ms) &&
          ...
          PRINT_NAMED_ERROR(
              "Track.AdvanceTrack.KeyframeStillActiveButTimeToPlayNextFrame",
