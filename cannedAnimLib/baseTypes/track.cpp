...
static void EnableStopMessageHelper(const BodyMotionKeyFrame& addedKeyFrame,
  ...
  if (nullptr != prevKeyFrame) {
    ...
    if (prevKeyFrame->GetTimestampActionComplete_ms() >
        addedKeyFrame.GetTriggerTime_ms() - ANIM_TIME_STEP_MS) {
      // PRINT_NAMED_DEBUG("Animations.EnableStopMessageHelper",
      //                  "Disabling stop message for body motion keyframe at
      //                  t=%d " "with duration=%d because of next keyframe at
      //                  t=%d", prevKeyFrame->GetTriggerTime_ms(),
...
template <>
Result Track<BodyMotionKeyFrame>::AddKeyFrameToBack(
    const BodyMotionKeyFrame& keyFrame) {
...
//       Shouldn't be necessary since this command doesn't make the robot move
//       indefinitely, plus it should really only be used at the end of an
//       animation where animationController will automatically command a stop
//       anyway since it treats the keyframe the same as a BodyMotionKeyFrame.

template <>
Result Track<BodyMotionKeyFrame>::AddKeyFrameByTime(
    const BodyMotionKeyFrame& keyFrame) {
  BodyMotionKeyFrame* prevKeyFrame = nullptr;
...
// report that it's finished before the backpack track has finished. For all
// other keyframes, there's no need to check duration because the lights will
// naturally stay in whatever state the keyframe leaves them until the next one
// changes them. So for any "previous" keyframe, we know there's another one
// coming and we can just set its duration to 0. This avoids introducing delay
// when a keyframe finishes at the same time the next one should trigger.
template <>
Result Track<BackpackLightsKeyFrame>::AddKeyFrameToBack(
    const BackpackLightsKeyFrame& keyFrame) {
...
template <>
Result Track<BackpackLightsKeyFrame>::AddKeyFrameByTime(
    const BackpackLightsKeyFrame& keyFrame) {
  BackpackLightsKeyFrame* prevKeyFrame = nullptr;
  Result result = AddKeyFrameByTimeHelper(keyFrame, prevKeyFrame);
...
template <>
Result Track<ProceduralFaceKeyFrame>::AddKeyFrameToBack(
    const ProceduralFaceKeyFrame& keyFrame) {
  ...
  if (allKeyframes.size() >= 2) {
    ...
    oldBackIter++;
    oldBackIter->SetKeyframeActiveDuration_ms(backIter->GetTriggerTime_ms() -
                                              oldBackIter->GetTriggerTime_ms());
  }
  return res;
}
  ...
  if (ANKI_DEV_CHEATS) {
    ...
    while (safetyCheckIter != allKeyframes.end()) {
      ...
      if (nextIter != allKeyframes.end()) {
        ANKI_VERIFY(
            safetyCheckIter->GetTimestampActionComplete_ms() ==
