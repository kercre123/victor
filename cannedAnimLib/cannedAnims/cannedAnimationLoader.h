...
class CannedAnimationLoader : private Util::noncopyable {
 public:
  ...
  CannedAnimationLoader(
      ...
      std::atomic<float>& loadingCompleteRatio, std::atomic<bool>& abortLoad)
      : _platform(platform),
        _spriteSequenceContainer(spriteSequenceContainer),
        _loadingCompleteRatio(loadingCompleteRatio),
        _abortLoad(abortLoad) {}

  void LoadAnimationsIntoContainer(const AnimDirInfo& info,
                                   CannedAnimationContainer* container);
  void LoadAnimationIntoContainer(const std::string& path,
                                  CannedAnimationContainer* container);
 ...
 private:
  ...
  Result DefineFromJson(const Json::Value& jsonRoot,
                        ...
                        CannedAnimationContainer* container);
  Result DefineFromFlatBuf(const CozmoAnim::AnimClip* animClip,
                           std::string& animName,
                           CannedAnimationContainer* container);
  Result SanityCheck(Result lastResult, Animation& animation,
