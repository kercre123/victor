...
void SetNewTapHeight(ConsoleFunctionContextRef context) {
  if (s_cubeAnimContainer != nullptr) {
    Animation* anim =
        s_cubeAnimContainer->GetAnimation(kCubeSpinnerAnimationName);
...
Animation* CannedAnimationContainer::GetAnimation(const std::string& name) {
  const Animation* animPtr =
      const_cast<const CannedAnimationContainer*>(this)->GetAnimation(name);
  return const_cast<Animation*>(animPtr);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
const Animation* CannedAnimationContainer::GetAnimation(
...
std::vector<std::string> CannedAnimationContainer::GetAnimationNames() {
  ...
  for (std::unordered_map<std::string, Animation>::iterator i =
           _animations.begin();
       i != _animations.end(); ++i) {
    v.push_back(i->first);
  }
