 ...
 *
 * Description:
 *    Class for storing SpriteBoxKeyFrames and constructing CompositeImages from
 *them
 *
...
constexpr TimeStamp_t kOverrideIndefinitely =
    std::numeric_limits<TimeStamp_t>::infinity();

// Fwd declare local helper
bool GetFrameFromSpriteSequenceHelper(
    const Vision::SpriteSequence& sequence, const uint16_t frameIdx,
    const Vision::SpriteSeqEndType& spriteSeqEndType,
    Vision::SpriteHandle& outSpriteHandle);

const Vision::SpriteBox kFullFaceSpriteBox{100.0f,
                                           0,
                                           0,
                                           FACE_DISPLAY_WIDTH,
                                           FACE_DISPLAY_HEIGHT,
...
Result SpriteBoxCompositor::AddKeyFrame(const CozmoAnim::SpriteBox* spriteBox) {
  ...
  newKeyFrame.assetID =
      Vision::SpritePathMap::GetAssetID(spriteBox->assetName()->str());
  newKeyFrame.spriteSeqEndType =
      Vision::SpriteSeqEndTypeFromString(spriteBox->spriteSeqEndType()->str());
  newKeyFrame.spriteBox.alpha = spriteBox->alpha();
  newKeyFrame.spriteBox.name =
      Vision::SpriteBoxNameFromString(spriteBox->spriteBoxName()->str());
  newKeyFrame.spriteBox.layer =
      Vision::LayerNameFromString(spriteBox->layer()->str());
  ...
  renderMethodString =
      ("CustomHue" == renderMethodString) ? "EyeColor" : renderMethodString;
  newKeyFrame.spriteBox.renderMethod =
      Vision::SpriteRenderMethodFromString(renderMethodString);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result SpriteBoxCompositor::AddKeyFrame(const Json::Value& json,
                                        const std::string& animName) {
  Vision::SpriteBoxKeyFrame newKeyFrame;
  newKeyFrame.triggerTime_ms =
      JsonTools::ParseInt32(json, kTriggerTimeKey, animName);
  newKeyFrame.assetID = Vision::SpritePathMap::GetAssetID(
      JsonTools::ParseString(json, kAssetNameKey, animName));
  newKeyFrame.spriteSeqEndType = Vision::SpriteSeqEndTypeFromString(
      JsonTools::ParseString(json, kSpriteSeqEndKey, animName));
  newKeyFrame.spriteBox.name = Vision::SpriteBoxNameFromString(
      JsonTools::ParseString(json, kSpriteBoxNameKey, animName));
  newKeyFrame.spriteBox.alpha =
      JsonTools::ParseInt32(json, kAlphaKey, animName);
  newKeyFrame.spriteBox.layer = Vision::LayerNameFromString(
      JsonTools::ParseString(json, kLayerNameKey, animName));
  ...
  newKeyFrame.spriteBox.width =
      JsonTools::ParseInt32(json, kWidthKey, animName);
  newKeyFrame.spriteBox.height =
      JsonTools::ParseInt32(json, kHeightKey, animName);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result SpriteBoxCompositor::AddFullFaceSpriteSeq(
    const CozmoAnim::FaceAnimation* faceAnimationKeyFrame,
    const Vision::SpriteSequenceContainer& spriteSeqContainer) {
  return AddFullFaceSpriteSeqInternal(
      Vision::SpritePathMap::GetAssetID(
          faceAnimationKeyFrame->animName()->str()),
      (u32)faceAnimationKeyFrame->triggerTime_ms(), spriteSeqContainer);
}
...
Result SpriteBoxCompositor::AddFullFaceSpriteSeq(
  ...
  return AddFullFaceSpriteSeqInternal(
      Vision::SpritePathMap::GetAssetID(
          JsonTools::ParseString(json, kFaceKeyFrameAssetNameKey, animName)),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result SpriteBoxCompositor::AddFullFaceSpriteSeqInternal(
    const Vision::SpritePathMap::AssetID assetID,
  ...
  // SpriteBoxKeyFrames don't have a notion of duration, so the length of the
  // animation is determined by the triggerTime of the last keyframe in the
  // SpriteBoxCompositor if there are no other keyframes. To make sure
  // animations with legacy keyframes play all the way through the Sequence we
  // have to deliberately add a clear keyframe at the end of it. The animation
  // team requested this design and knows to bracket animations with "end"
  // keyframes going forward.
  Vision::SpriteBoxKeyFrame clearKeyFrame(startKeyFrame);
  clearKeyFrame.assetID = Vision::SpritePathMap::kClearSpriteBoxID;
  clearKeyFrame.triggerTime_ms +=
      spriteSeqContainer.GetSpriteSequence(assetID)->GetNumFrames() *
      ANIM_TIME_STEP_MS;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result SpriteBoxCompositor::AddKeyFrameInternal(
    Vision::SpriteBoxKeyFrame&& spriteBoxKeyFrame) {
  ...
  if (_spriteBoxMap->end() == iter) {
    ...
    if (!track.InsertKeyFrame(std::move(spriteBoxKeyFrame))) {
      LOG_ERROR("SpriteBoxCompositor.AddKeyFrame.DuplicateKeyFrame",
                "Attempted to add overlapping keyframe for SpriteBoxName: %s "
                "at time: %d ms",
                EnumToString(name), triggerTime_ms);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void SpriteBoxCompositor::SetFaceImageOverride(
    const Vision::SpriteHandle& spriteHandle,
    const TimeStamp_t relativeStreamTime_ms, const TimeStamp_t duration_ms) {
  _faceImageOverride = spriteHandle;
  _faceImageOverrideEndTime_ms = duration_ms != 0
                                     ? (relativeStreamTime_ms + duration_ms)
                                     : kOverrideIndefinitely;
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void SpriteBoxCompositor::AddSpriteBoxRemap(
    const Vision::SpriteBoxName spriteBox,
  ...
  if (Vision::SpritePathMap::kClearSpriteBoxID == remappedAssetID) {
    LOG_ERROR("SpriteBoxCompositor.SetAssetRemap.InvalidRemap",
              "kClearSpriteBoxID should not be used in engine. Use "
              "kEmptySpriteBoxID instead.");
    return;
  }

  if (IsEmpty()) {
    LOG_ERROR(
        "SpriteBoxCompositor.AddSpriteBoxRemap.EmptyCompositor",
        "Attempted to add remap for SpriteBox %s with remapped AssetID %d",
        EnumToString(spriteBox), remappedAssetID);
    return;
  }
  ...
  if (_spriteBoxMap->end() == iter) {
    LOG_ERROR("SpriteBoxCompositor.AddSpriteBoxRemap.InvalidSpriteBox",
              "Attempted to add remap for invalid SpriteBox %s with remapped "
              "AssetID %d",
              EnumToString(spriteBox), remappedAssetID);
    return;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void SpriteBoxCompositor::CacheInternalSprites(
  ...
  LOG_WARNING("SpriteBoxCompositor.CacheInternalSprites.CachingNotSupported",
              "Caching of internal sprites from the SpriteBoxCompositor is "
              "currently unsupported");
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void SpriteBoxCompositor::AppendTracks(const SpriteBoxCompositor& other,
                                       const TimeStamp_t animOffset_ms) {
  if (nullptr != other._spriteBoxMap) {
    for (const auto& spriteBoxTrackIter : *other._spriteBoxMap) {
      for (const auto& spriteBoxKeyFrame :
           spriteBoxTrackIter.second.GetKeyFrames()) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void SpriteBoxCompositor::MoveToStart() { _advanceTime_ms = 0; }
...
bool SpriteBoxCompositor::HasFramesLeft() const {
  // If we've been given an override image, keep this animation running as
  ...
  // Keep tabs on this when working in AnimationStreamer. It currently has to
  // use '<=' in order to have the final frame displayed due to calling
  // locations in AnimationStreamer, but that feels odd since it ends up
  // requiring that we run one frame beyond the last frame of the animation
  // before AnimationStreamer cleans up the animation.
  return (_advanceTime_ms <= _lastKeyFrameTime_ms);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
TimeStamp_t SpriteBoxCompositor::CompareLastFrameTime(
    const TimeStamp_t lastFrameTime_ms) const {
  return (lastFrameTime_ms > _lastKeyFrameTime_ms) ? lastFrameTime_ms
                                                   : _lastKeyFrameTime_ms;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool SpriteBoxCompositor::PopulateCompositeImage(
    Vision::SpriteCache& spriteCache,
  ...
  for (auto& trackPair : *_spriteBoxMap) {
    ...
    if (_overrideAllSpritesToEyeHue) {
      currentKeyFrame.spriteBox.renderMethod =
          Vision::SpriteRenderMethod::EyeColor;
    }
    ...
    if (spriteSeqContainer.IsValidSpriteSequenceID(currentKeyFrame.assetID)) {
      const uint16_t frameIdx =
          (timeSinceAnimStart_ms - currentKeyFrame.triggerTime_ms) /
          ANIM_TIME_STEP_MS;
      const Vision::SpriteSequence& sequence =
          *spriteSeqContainer.GetSpriteSequence(currentKeyFrame.assetID);

      if (!GetFrameFromSpriteSequenceHelper(sequence, frameIdx,
                                            currentKeyFrame.spriteSeqEndType,
                                            spriteHandle)) {
      ...
      spriteHandle =
          spriteCache.GetSpriteHandleForAssetID(currentKeyFrame.assetID);
    }
...
namespace {
bool GetFrameFromSpriteSequenceHelper(
    const Vision::SpriteSequence& sequence, const uint16_t absFrameIdx,
    const Vision::SpriteSeqEndType& spriteSeqEndType,
    Vision::SpriteHandle& outSpriteHandle) {
...
void SpriteBoxTrack::ClearUpToTime(const TimeStamp_t toTime_ms) {
  ...
  while ((_track.end() != nextKeyFrameIter) &&
         (nextKeyFrameIter->triggerTime_ms <= toTime_ms)) {
    ++currentKeyFrameIter;
    ++nextKeyFrameIter;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool SpriteBoxTrack::GetCurrentKeyFrame(
    const TimeStamp_t timeSinceAnimStart_ms,
    Vision::SpriteBoxKeyFrame& outKeyFrame) {
  if (_track.empty() || (timeSinceAnimStart_ms < _firstKeyFrameTime_ms)) {
    // Nothing to draw yet
    return false;
  }

  // Use the last access as a search start point (if appropriate) to optimize
  // search for linear playback
  ...
  // Find the current set of keyframes
  while ((_track.end() != _nextKeyFrameIter) &&
         (_nextKeyFrameIter->triggerTime_ms <= timeSinceAnimStart_ms)) {
    ++_currentKeyFrameIter;
    ++_nextKeyFrameIter;
  }
  outKeyFrame = *_currentKeyFrameIter;

  // "Clear" keyframes override everything, including remaps and "Empty"s.
  // Render nothing for this keyframe
  ...
  const float interpRatio =
      ((float)(timeSinceAnimStart_ms - currentKeyFrame.triggerTime_ms) /
  ...
  if (currentKeyFrame.spriteBox.alpha != nextKeyFrame.spriteBox.alpha) {
    outKeyFrame.spriteBox.alpha =
        (1.0f - interpRatio) * currentKeyFrame.spriteBox.alpha +
        interpRatio * nextKeyFrame.spriteBox.alpha;
  }
  if (currentKeyFrame.spriteBox.xPos != nextKeyFrame.spriteBox.xPos) {
    outKeyFrame.spriteBox.xPos =
        (1.0f - interpRatio) * currentKeyFrame.spriteBox.xPos +
        interpRatio * nextKeyFrame.spriteBox.xPos;
  }
  if (currentKeyFrame.spriteBox.yPos != nextKeyFrame.spriteBox.yPos) {
    outKeyFrame.spriteBox.yPos =
        (1.0f - interpRatio) * currentKeyFrame.spriteBox.yPos +
        interpRatio * nextKeyFrame.spriteBox.yPos;
  }
