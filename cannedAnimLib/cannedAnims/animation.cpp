...
Animation::Animation(const std::string& name)
    : _name(name), _isInitialized(false) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool Animation::operator==(const Animation& other) const {
  return (_name == other._name) && (_headTrack == other._headTrack) &&
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result Animation::DefineFromFlatBuf(
    const std::string& name, const CozmoAnim::AnimClip* animClip,
  ...
  TODO: Update the processing of BackpackLights keyframes to NOT use an
        ...
        corresponding track AddKeyFrameToBack() method should be overloaded to
  take in a CozmoAnim::BackpackLights keyframe and process accordingly
  (COZMO-8766).
  ...
  */
  ...
  if (procFaceData != nullptr) {
    for (int pfIdx = 0; pfIdx < procFaceData->size(); pfIdx++) {
      const CozmoAnim::ProceduralFace* procFaceKeyframe =
          procFaceData->Get(pfIdx);
      const Result addResult =
          _proceduralFaceTrack.AddKeyFrameToBack(procFaceKeyframe, name);
      if (addResult != RESULT_OK) {
        LOG_ERROR("Animation.DefineFromFlatBuf.AddKeyFrameFailure",
  ...
  if (audioData != nullptr) {
    for (int audioIdx = 0; audioIdx < audioData->size(); audioIdx++) {
      ...
      const Result addResult =
          _robotAudioTrack.AddKeyFrameToBack(audioKeyframe, name);
      if (addResult != RESULT_OK) {
        LOG_ERROR("Animation.DefineFromFlatBuf.AddKeyFrameFailure",
  ...
  if (backpackData != nullptr) {
    for (int bpIdx = 0; bpIdx < backpackData->size(); bpIdx++) {
      // TODO: Update the processing of these keyframes to NOT use an
      // intermediate
      //       JSON data structure (Json::Value) for them. The corresponding
      //       track AddKeyFrameToBack() method should be overloaded to take in
      //       a CozmoAnim::BackpackLights keyframe and process accordingly
      //       (COZMO-8766).
      ...
      const Result addResult =
          _backpackLightsTrack.AddKeyFrameToBack(jsonFrame, name);
      if (addResult != RESULT_OK) {
        LOG_ERROR("Animation.DefineFromFlatBuf.AddKeyFrameFailure",
  ...
  if (nullptr != spriteBoxData) {
    for (int sbIdx = 0; sbIdx < spriteBoxData->size(); sbIdx++) {
      ...
      if (RESULT_OK != addResult) {
        LOG_ERROR("Animation.DefineFromFlatBuf.AddKeyFrameFailure",
                  "Adding SpriteBox frame %d failed", sbIdx);
        return addResult;
      }
    }
  }
  ...
  if (spriteSequenceData != nullptr) {
    for (int faIdx = 0; faIdx < spriteSequenceData->size(); faIdx++) {
      const CozmoAnim::FaceAnimation* faceAnimKeyframe =
          spriteSequenceData->Get(faIdx);
      const Result addResult = _spriteBoxCompositor.AddFullFaceSpriteSeq(
          faceAnimKeyframe, *seqContainer);
      if (RESULT_OK != addResult) {
        LOG_ERROR("Animation.DefineFromFlatBuf.AddKeyFrameFailure",
  ...
  if (eventData != nullptr) {
    for (int eIdx = 0; eIdx < eventData->size(); eIdx++) {
      ...
      const Result addResult =
          _eventTrack.AddKeyFrameToBack(eventKeyframe, name);
      if (addResult != RESULT_OK) {
        LOG_ERROR("Animation.DefineFromFlatBuf.AddKeyFrameFailure",
  ...
  if (bodyData != nullptr) {
    for (int bdyIdx = 0; bdyIdx < bodyData->size(); bdyIdx++) {
      ...
      const Result addResult =
          _bodyPosTrack.AddKeyFrameToBack(bodyKeyframe, name);
      if (addResult != RESULT_OK) {
        LOG_ERROR("Animation.DefineFromFlatBuf.AddKeyFrameFailure",
  ...
  if (recordHeadingData != nullptr) {
    for (int rhIdx = 0; rhIdx < recordHeadingData->size(); rhIdx++) {
      const CozmoAnim::RecordHeading* recordHeadingKeyframe =
          recordHeadingData->Get(rhIdx);
      const Result addResult =
          _recordHeadingTrack.AddKeyFrameToBack(recordHeadingKeyframe, name);
      if (addResult != RESULT_OK) {
        LOG_ERROR("Animation.DefineFromFlatBuf.AddKeyFrameFailure",
  ...
  if (turnToRecordedHeadingData != nullptr) {
    for (int rhIdx = 0; rhIdx < turnToRecordedHeadingData->size(); rhIdx++) {
      const CozmoAnim::TurnToRecordedHeading* turnToRecordedHeadingKeyframe =
          turnToRecordedHeadingData->Get(rhIdx);
      const Result addResult = _turnToRecordedHeadingTrack.AddKeyFrameToBack(
          turnToRecordedHeadingKeyframe, name);
      if (addResult != RESULT_OK) {
        LOG_ERROR("Animation.DefineFromFlatBuf.AddKeyFrameFailure",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result Animation::DefineFromJson(
    const std::string& name, const Json::Value& jsonRoot,
    Vision::SpriteSequenceContainer* seqContainer) {
  _name = name;
  ...
  for (s32 iFrame = 0; iFrame < numFrames; ++iFrame) {
    ...
    if (!jsonFrame.isObject()) {
      PRINT_NAMED_ERROR(
          "Animation.DefineFromJson.FrameMissing",
          "frame %d of '%s' animation is missing or incorrect type.", iFrame,
          _name.c_str());
      return RESULT_FAIL;
    }
    ...
    if (frameName == HeadAngleKeyFrame::GetClassName()) {
      ...
      addResult = _spriteBoxCompositor.AddFullFaceSpriteSeq(
          jsonFrame, *seqContainer, name);
      ...
      addResult =
          _turnToRecordedHeadingTrack.AddKeyFrameToBack(jsonFrame, name);
    } else if (frameName == ProceduralFaceKeyFrame::GetClassName()) {
      addResult = _proceduralFaceTrack.AddKeyFrameToBack(jsonFrame, name);
    } else {
      PRINT_NAMED_ERROR(
          "Animation.DefineFromJson.UnrecognizedFrameName",
          "Frame %d in '%s' animation has unrecognized name '%s'.", iFrame,
          _name.c_str(), frameName.c_str());
      return RESULT_FAIL;
    }

    if (addResult != RESULT_OK) {
      PRINT_NAMED_ERROR("Animation.DefineFromJson.AddKeyFrameFailure",
                        "Adding %s frame %d failed.", frameName.c_str(),
                        iFrame);
      return addResult;
    }
...
// Helper macro for running a given method of all tracks and combining the
// result in the specified way. To just call a method, use ";" for COMBINE_WITH,
// or use "&&" or "||" to combine into a single result.
#define ALL_TRACKS(__METHOD__, __COMBINE_WITH__, ...)                       \
  _headTrack.__METHOD__(__VA_ARGS__) __COMBINE_WITH__ _liftTrack            \
      .__METHOD__(__VA_ARGS__) __COMBINE_WITH__ _proceduralFaceTrack        \
      .__METHOD__(__VA_ARGS__) __COMBINE_WITH__ _eventTrack                 \
      .__METHOD__(__VA_ARGS__) __COMBINE_WITH__ _robotAudioTrack            \
      .__METHOD__(__VA_ARGS__) __COMBINE_WITH__ _backpackLightsTrack        \
      .__METHOD__(__VA_ARGS__) __COMBINE_WITH__ _bodyPosTrack               \
      .__METHOD__(__VA_ARGS__) __COMBINE_WITH__ _recordHeadingTrack         \
      .__METHOD__(__VA_ARGS__) __COMBINE_WITH__ _turnToRecordedHeadingTrack \
      .__METHOD__(__VA_ARGS__)                                              \
          __COMBINE_WITH__ _spriteBoxCompositor.__METHOD__(__VA_ARGS__)

//# define ALL_TRACKS(__METHOD__, __ARG__, __COMBINE_WITH__)
//ALL_TRACKS_WITH_ARG(__METHOD__, void, __COMBINE_WITH__)
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void Animation::ClearUpToCurrent() { ALL_TRACKS(ClearUpToCurrent, ;); }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool Animation::IsEmpty() const { return ALL_TRACKS(IsEmpty, &&); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool Animation::HasFramesLeft() const { return ALL_TRACKS(HasFramesLeft, ||); }
...
void Animation::AppendAnimation(const Animation& appendAnim) {
  ...
  _headTrack.AppendTrack(appendAnim.GetTrack<HeadAngleKeyFrame>(),
                         animOffest_ms);
  _liftTrack.AppendTrack(appendAnim.GetTrack<LiftHeightKeyFrame>(),
                         animOffest_ms);
  _proceduralFaceTrack.AppendTrack(
      appendAnim.GetTrack<ProceduralFaceKeyFrame>(), animOffest_ms);
  _eventTrack.AppendTrack(appendAnim.GetTrack<EventKeyFrame>(), animOffest_ms);
  _backpackLightsTrack.AppendTrack(
      appendAnim.GetTrack<BackpackLightsKeyFrame>(), animOffest_ms);
  _bodyPosTrack.AppendTrack(appendAnim.GetTrack<BodyMotionKeyFrame>(),
                            animOffest_ms);
  _recordHeadingTrack.AppendTrack(appendAnim.GetTrack<RecordHeadingKeyFrame>(),
                                  animOffest_ms);
  _turnToRecordedHeadingTrack.AppendTrack(
      appendAnim.GetTrack<TurnToRecordedHeadingKeyFrame>(), animOffest_ms);
  _robotAudioTrack.AppendTrack(appendAnim.GetTrack<RobotAudioKeyFrame>(),
                               animOffest_ms);
  _spriteBoxCompositor.AppendTracks(appendAnim.GetSpriteBoxCompositor(),
                                    animOffest_ms);
}
...
uint32_t Animation::GetLastKeyFrameTime_ms() const {
  ...
  lastFrameTime_ms =
      CompareLastFrameTime<RecordHeadingKeyFrame>(lastFrameTime_ms);
  lastFrameTime_ms =
      CompareLastFrameTime<TurnToRecordedHeadingKeyFrame>(lastFrameTime_ms);
  lastFrameTime_ms = CompareLastFrameTime<EventKeyFrame>(lastFrameTime_ms);
  lastFrameTime_ms =
      CompareLastFrameTime<BackpackLightsKeyFrame>(lastFrameTime_ms);
  lastFrameTime_ms =
      CompareLastFrameTime<ProceduralFaceKeyFrame>(lastFrameTime_ms);
  lastFrameTime_ms =
      _spriteBoxCompositor.CompareLastFrameTime(lastFrameTime_ms);
...
uint32_t Animation::GetLastKeyFrameEndTime_ms() const {
  ...
  lastFrameTime_ms =
      CompareLastFrameEndTime<RobotAudioKeyFrame>(lastFrameTime_ms);
  lastFrameTime_ms =
      CompareLastFrameEndTime<HeadAngleKeyFrame>(lastFrameTime_ms);
  lastFrameTime_ms =
      CompareLastFrameEndTime<LiftHeightKeyFrame>(lastFrameTime_ms);
  lastFrameTime_ms =
      CompareLastFrameEndTime<BodyMotionKeyFrame>(lastFrameTime_ms);
  lastFrameTime_ms =
      CompareLastFrameEndTime<RecordHeadingKeyFrame>(lastFrameTime_ms);
  lastFrameTime_ms =
      CompareLastFrameEndTime<TurnToRecordedHeadingKeyFrame>(lastFrameTime_ms);
  lastFrameTime_ms = CompareLastFrameEndTime<EventKeyFrame>(lastFrameTime_ms);
  lastFrameTime_ms =
      CompareLastFrameEndTime<BackpackLightsKeyFrame>(lastFrameTime_ms);
  lastFrameTime_ms =
      CompareLastFrameEndTime<ProceduralFaceKeyFrame>(lastFrameTime_ms);
  lastFrameTime_ms =
      _spriteBoxCompositor.CompareLastFrameTime(lastFrameTime_ms);
...
TimeStamp_t Animation::CompareLastFrameTime(
  ...
  if (!track.IsEmpty()) {
    ...
    return std::max(lastFrameTime_ms,
                    track.GetLastKeyFrame()->GetTriggerTime_ms());
  }
...
// - - - - - - - - - - - - - - - - - - - -
template <class KeyFrameType>
TimeStamp_t Animation::CompareLastFrameEndTime(
    const TimeStamp_t lastFrameTime_ms) const {
  const auto& track = GetTrack<KeyFrameType>();
  if (!track.IsEmpty()) {
    // Compare track's last key frame time and lastFrameTime_ms
    return std::max(lastFrameTime_ms,
                    track.GetLastKeyFrame()->GetTimestampActionComplete_ms());
  }
  // No key frames in track
  return lastFrameTime_ms;
}
