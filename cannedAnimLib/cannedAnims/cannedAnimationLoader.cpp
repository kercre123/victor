...
namespace {
// We report some loading data info so the UI can inform the user. Ratio of time
// taken per section is approximate, based on recent profiling. Some sections
// below are called out specifically, the rest makes up the remainder. These
// should add up to be less than or equal to 1.0!
static constexpr float kAnimationsLoadingRatio = 0.7f;
}  // namespace

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CannedAnimationLoader::LoadAnimationsIntoContainer(
    const AnimDirInfo& info, CannedAnimationContainer* container) {
  {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CannedAnimationLoader::LoadAnimationIntoContainer(
    const std::string& path, CannedAnimationContainer* container) {
  AnimDirInfo info;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
CannedAnimationLoader::AnimDirInfo CannedAnimationLoader::CollectAnimFiles(
  ...
  // animations
  {
    for (const auto& path : paths) {
      CannedAnimationLoader::WalkAnimationDir(
          _platform, path, info.animFileTimestamps,
  ...
  // print results
  LOG_INFO("CannedAnimationLoader.CollectAnimFiles.Results",
           "Found %zu animation files", info.jsonFiles.size());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CannedAnimationLoader::WalkAnimationDir(
    const Util::Data::DataPlatform* platform, const std::string& animationDir,
    AnimDirInfo::TimestampMap& timestamps,
    const std::function<void(const std::string&)>& walkFunc) {
  const std::string animationFolder =
      platform->pathToResource(Util::Data::Scope::Resources, animationDir);
  const std::vector<const char*> fileExts = {"json", "bin"};
  auto filePaths =
      Util::FileUtils::FilesInDirectory(animationFolder, true, fileExts, true);
  ...
  for (const auto& path : filePaths) {
    ...
    if (result == -1) {
      LOG_WARNING("CannedAnimationLoader.WalkAnimationDir",
                  "could not get mtime for %s", path.c_str());
      continue;
    }
    bool loadFile = false;
...
void CannedAnimationLoader::AddToLoadingRatio(float delta) {
  // Allows for a thread to repeatedly try to update the loading ratio until it
  // gets access
...
void CannedAnimationLoader::LoadAnimationsInternal(
  ...
  const double startTime =
      Util::Time::UniversalTime::GetCurrentTimeInMilliseconds();
#endif
  ...
  using MyDispatchWorker =
      Util::DispatchWorker<3, const std::string&, CannedAnimationContainer*>;
  MyDispatchWorker::FunctionType loadFileFunc =
      std::bind(&CannedAnimationLoader::LoadAnimationFile, this,
  ...
  for (int i = 0; i < size; i++) {
    ...
    // LOG_DEBUG("CannedAnimationLoader.LoadAnimations", "loaded regular anim %d
    // of %zu", i, size);
  }

  _perAnimationLoadingRatio =
      kAnimationsLoadingRatio * 1.0f / Util::numeric_cast<float>(size);
  myWorker.Process();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void CannedAnimationLoader::LoadAnimationFile(
    const std::string& path, CannedAnimationContainer* container) {
  if (_abortLoad.load(std::memory_order_relaxed)) {
    return;
  }

  // PRINT_CH_DEBUG("Animations",
  // "CannedAnimationLoader.LoadAnimationFile.LoadingAnimationsFromBinaryOrJson",
  ...
  if (binFile) {
    ...
    if (binFileContents.size() == 0) {
      LOG_ERROR("CannedAnimationLoader.LoadAnimationFile.BinaryDataEmpty",
                "Found no data in %s", path.c_str());
      return;
    }
    unsigned char* binData = binFileContents.data();
    if (nullptr == binData) {
      LOG_ERROR("CannedAnimationLoader.LoadAnimationFile.BinaryDataNull",
                "Found no data in %s", path.c_str());
      return;
    }
    auto animClips = CozmoAnim::GetAnimClips(binData);
    if (nullptr == animClips) {
      LOG_ERROR("CannedAnimationLoader.LoadAnimationFile.AnimClipsNull",
                "Found no animations in %s", path.c_str());
      return;
    }
    auto allClips = animClips->clips();
    if (nullptr == allClips) {
      LOG_ERROR("CannedAnimationLoader.LoadAnimationFile.AllClipsNull",
                "Found no animations in %s", path.c_str());
      return;
    }
    if (allClips->size() == 0) {
      LOG_ERROR("CannedAnimationLoader.LoadAnimationFile.AnimClipsEmpty",
                "Found no animations in %s", path.c_str());
      return;
    }
    ...
    for (int clipIdx = 0; clipIdx < allClips->size(); clipIdx++) {
      ...
      // PRINT_CH_DEBUG("Animations",
      // "CannedAnimationLoader.LoadAnimationFile.LoadingSpecificAnimFromBinary",
      //              "Loading '%s' from %s", animName, path.c_str());
      std::string strName = animName;

      // TODO: Should this mutex lock happen here or immediately before this for
      // loop (COZMO-8766)?
    ...
    if (success && !animDefs.empty()) {
      ...
      // TODO: This warning is useful, but it causes a crash when we use the
      // current mechanism for
      //       animators to preview their work in Maya on the robot. We plan on
      //       changing that preview-on-robot to use the SDK, so this warning
      //       should be tested and potentially enabled after that. See
      //       COZMO-9251 for some related info (nishkar, 2/2/2017).
      //
      ...
      //                      "Animation name '%s' does not seem to match
      //                      filename '%s'", animationId.c_str(),
      //                      path.c_str());
      //}
    }
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result CannedAnimationLoader::DefineFromFlatBuf(
    const CozmoAnim::AnimClip* animClip, std::string& animName,
    CannedAnimationContainer* container) {
  Animation animation(animName);

  Result lastResult =
      animation.DefineFromFlatBuf(animName, animClip, _spriteSequenceContainer);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result CannedAnimationLoader::DefineFromJson(
    const Json::Value& jsonRoot, std::string& animationName,
    CannedAnimationContainer* container) {
  ...
  */
  ...
  if (animationNames.empty()) {
    ...
    PRINT_NAMED_WARNING(
        "CannedAnimationLoader.DefineFromJson.TooManyAnims",
  ...
  PRINT_CH_DEBUG(LOG_CHANNEL, "CannedAnimationLoader::DefineFromJson",
                 "Loading '%s'", animationName.c_str());

  Animation animation(animationName);
  Result lastResult = animation.DefineFromJson(
      animationName, jsonRoot[animationName], _spriteSequenceContainer);
  ...
  if (res == Result::RESULT_OK) {
    ...
    if (outOverwriting) {
      PRINT_NAMED_WARNING(
          "CannedAnimationLoader.DefineFromJson.OverwritingExistingAnimation",
          "Container already had an animation named %s, overwriting",
          animationName.c_str());
    }
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result CannedAnimationLoader::SanityCheck(Result lastResult,
                                          Animation& animation,
  ...
  if (animation.GetName() != animationName) {
    PRINT_NAMED_ERROR("CannedAnimationContainer.DefineFromJson",
                      "Animation's internal name ('%s') doesn't match "
                      "container's name for it ('%s').",
                      animation.GetName().c_str(), animationName.c_str());
    return RESULT_FAIL;
  }
