...
class SpriteBoxCompositor {
 public:
  ...
  void AppendTracks(const SpriteBoxCompositor& other,
 ...
 private:
  ...
  SpriteBoxCompositor& operator=(
      SpriteBoxCompositor&& other);  // Move assignment
  SpriteBoxCompositor& operator=(
      const SpriteBoxCompositor& other);  // Copy assignment

  Result AddKeyFrameInternal(Vision::SpriteBoxKeyFrame&& spriteBox);

  Vision::SpriteBoxKeyFrame InterpolateKeyFrames(
      const Vision::SpriteBoxKeyFrame& thisKeyFrame,
  ...
  using SpriteBoxMap =
      std::unordered_map<Vision::SpriteBoxName, SpriteBoxTrack>;
  std::unique_ptr<SpriteBoxMap> _spriteBoxMap;
};

class SpriteBoxTrack {
 public:
  // Sort SpriteBoxKeyFrames by triggerTime_ms within a set. This also implies
  // that for a given SpriteBoxName, two KeyFrames are considered duplicates if
  // they have the same triggerTime_ms. Ergo, multiple keyframes for the same
  // SBName and trigger time are not allowed.
  struct SpriteBoxKeyFrameCompare {
    bool operator()(const Vision::SpriteBoxKeyFrame& lhs,
                    const Vision::SpriteBoxKeyFrame& rhs) const {
      return lhs.triggerTime_ms < rhs.triggerTime_ms;
    }
  };

  using SpriteBoxKeyFrameSet =
      std::set<Vision::SpriteBoxKeyFrame, SpriteBoxKeyFrameCompare>;
  ...
  bool GetCurrentKeyFrame(const TimeStamp_t timeSinceAnimStart_ms,
                          Vision::SpriteBoxKeyFrame& outKeyFrame);
  const SpriteBoxKeyFrameSet& GetKeyFrames() const { return _track; }

  void SetAssetRemap(const Vision::SpritePathMap::AssetID remappedAssetID) {
    _remappedAssetID = remappedAssetID;
  }
  void ClearAssetRemap() {
