...
class Animation {
 public:
  ...
  Result DefineFromFlatBuf(const std::string& name,
  ...
  void SetFaceImageOverride(const Vision::SpriteHandle& spriteHandle,
    ...
    return _spriteBoxCompositor.SetFaceImageOverride(
        spriteHandle, relativeStreamTime_ms, duration_ms);
  }
  ...
  void ClearOverrides() { _spriteBoxCompositor.ClearOverrides(); }

  void AddSpriteBoxRemap(const Vision::SpriteBoxName& spriteBoxName,
                         const Vision::SpritePathMap::AssetID remappedAssetID) {
    return _spriteBoxCompositor.AddSpriteBoxRemap(spriteBoxName,
                                                  remappedAssetID);
  }
  ...
  const Animations::Track<KeyFrameType>& GetTrack() const {
    // Normally I hate using const_cast, but GetTrack is a template function
    // where the actual implementation is to have a different specialization for
    // each type, in order to return the correct Track member.
    return const_cast<Animation*>(this)->GetTrack<KeyFrameType>();
  }

  const Animations::SpriteBoxCompositor& GetSpriteBoxCompositor() const {
    return _spriteBoxCompositor;
  }
  ...
  // If the animation has any sprites (the sprite sequence track) cache them for
  // the duration of the animation so that they're not being loaded from disk
  // during playback
  ...
  bool PopulateCompositeImage(
      Vision::SpriteCache& spriteCache,
      Vision::SpriteSequenceContainer& spriteSeqContainer,
      TimeStamp_t timeSinceAnimStart_ms, Vision::CompositeImage& outCompImg) {
    return _spriteBoxCompositor.PopulateCompositeImage(
        spriteCache, spriteSeqContainer, timeSinceAnimStart_ms, outCompImg);
  }
 ...
 private:
  ...
  // Compare if the track's last key frame time is gerater then the
  // lastFrameTime_ms argument Return the greater time
  template <class KeyFrameType>
  TimeStamp_t CompareLastFrameTime(const TimeStamp_t lastFrameTime_ms) const;

  // Compare if the track's last key frame + duration time is greater then the
  // lastFrameTime_ms argument Return the greater time
  template <class KeyFrameType>
...
Result Animation::AddKeyFrameToBack(const KeyFrameType& kf) {
  ...
  if ((oldKF != nullptr) && (newKF->GetTriggerTime_ms() == 0)) {
