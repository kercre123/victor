...
`./HOW-record.sh` to capture performance data.
...
```./HOW-report.sh``` to produce same output as ```./HOW-simpleperf.sh```

```./HOW-report.sh --sort tid``` to list threads, most expensive first
...
```./HOW-report.sh --tids 9660``` to view the results for a specific thread
...
[Inferno](../../lib/util/tools/simpleperf/doc/inferno.md) is a flamegraph generator, the width of a flamegraph represents 100% of the number of samples and the height is related to the number of functions on the stack when sampling occurred.

The flamegraph give you instant vision on the CPU cycles cost centers, specific offenders can be found by looking for plateaus, for example the most expensive task from the sample below is ```CLAHE_Interpolation_Body``` showing in orange with another plateau on the right with ```_raw_spin_unlock_irq```.

![How-inferno screen shot](images/HOW-inferno.png)

### [HOW-annotate.sh](../../project/victor/simpleperf/HOW-annotate.sh)

It is possible to drill down to the source line level with ```HOW-annotate.sh```.  All functions with samples will have the source code annotated where each line will have its relative percentage of samples reported.
...
-p        |--app                  |X  |Profile an Android app, given the package name. Like -p com.example.android.myapp.
