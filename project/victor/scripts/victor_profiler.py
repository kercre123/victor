...
#ANDROID_NDK_ROOT = os.path.join(os.environ['HOME'], '.anki', 'android', 'ndk-repository', 'android-ndk-r15b')
#SIMPLEPERF_ROOT = os.path.join(ANDROID_NDK_ROOT, 'simpleperf')
SIMPLEPERF_ROOT = os.path.join(os.path.dirname(__file__), '..', 'tools', 'simpleperf')
sys.path.append(SIMPLEPERF_ROOT)
...
class AppProfiler(object):
    ...
    def __init__(self, config):
        ...
        # victor:
        # app_package_name => "com.anki.cozmoengine"
        # app_process_name => (cozmoengined)
        # native_lib_dir => /data/data/com.anki.cozmoengine/lib (or build dir on local sys)

        config_names = ['app_process_name', 'device_native_lib_dir', 'native_bin_dir',
                        'app_package_name', 'native_lib_dir', 'apk_file_path',
                        'recompile_app', 'launch_activity', 'launch_inst_test',
                        'record_options', 'perf_data_path', 'adb_path', 'readelf_path',
                        'binary_cache_dir']
    ...
    def prepare_profiling(self):
        ...
        # Skip re-compiling java code
        # self._recompile_app()
        
        # _restart_app restarted an "App", not sure if we need to restart a process..
        # self._restart_app()

        self._get_app_environment()
        self._download_simpleperf()

        # shouldn't need to copy libs - just point simpleperf to native_lib_dir
    ...
    def start_and_wait_profiling(self):
        self.run_in_app_dir([
            ...
            str(self.app_pid), '--symfs', self.config['device_native_lib_dir']])
    ...
    def collect_profiling_data(self):
        ...
        if self.config['native_bin_dir']:
            config['symfs_dirs'].append(self.config['native_bin_dir'])
