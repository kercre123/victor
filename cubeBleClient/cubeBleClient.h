...
class CubeBleClient {
 public:
  ...
  // Webots processes must do this before creating CubeBleClient for the first
  // time. Unit test processes must call SetSupervisor(nullptr) to run without a
  // supervisor.
  static void SetSupervisor(webots::Supervisor* sup);
#endif

  using ObjectAvailableCallback =
      std::function<void(const ExternalInterface::ObjectAvailable&)>;
  using CubeMessageCallback =
      std::function<void(const BleFactoryId&, const MessageCubeToEngine&)>;
  using CubeConnectionCallback =
      std::function<void(const BleFactoryId&, const bool connected)>;
  using ScanFinishedCallback = std::function<void(void)>;
  using ConnectionFailedCallback = std::function<void(const BleFactoryId&)>;

  void RegisterObjectAvailableCallback(
      const ObjectAvailableCallback& callback) {
  ...
  void RegisterConnectionFailedCallback(
      const ConnectionFailedCallback& callback) {
 ...
 private:
  ...
  CubeConnectionState _cubeConnectionState =
      CubeConnectionState::UnconnectedIdle;
