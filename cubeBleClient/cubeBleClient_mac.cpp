 ...
 *
 * Description:
 *               Defines interface to simulated cubes (mac-specific
 *implementations)
 *
...
CubeBleClient::CubeBleClient() {
  ...
  if (nullptr != _engineSupervisor) {
    ...
    DEV_ASSERT(_discoveryReceiver != nullptr,
               "CubeBleClient.NullDiscoveryReceiver");
    ...
    DEV_ASSERT(numReceiversField != nullptr,
               "CubeBleClient.NullNumReceiversField");
    const int numCubeReceivers = numReceiversField->getSFInt32();

    for (int i = 0; i < numCubeReceivers; i++) {
      auto* receiver = _engineSupervisor->getReceiver("cubeCommsReceiver" +
                                                      std::to_string(i));
...
bool CubeBleClient::SendMessageInternal(const MessageEngineToCube& msg) {
  ...
  // return value of 1 indicates that the message was successfully queued (see
...
bool CubeBleClient::RequestConnectInternal(const BleFactoryId& factoryId) {
  ...
  for (auto* rec : _receivers) {
    ...
    const auto it = std::find_if(
        _cubeReceiverMap.begin(), _cubeReceiverMap.end(),
  ...
  DEV_ASSERT_MSG(_cubeReceiverMap.find(factoryId) != _cubeReceiverMap.end(),
                 ...
                 "Could not find a free receiver for cube with factory ID %s. "
                 "Connected to too many cubes?",
                 factoryId.c_str());
...
bool CubeBleClient::InitInternal() { return true; }
...
bool CubeBleClient::UpdateInternal() {
  ...
  if (_cubeConnectionState == CubeConnectionState::Connected &&
    ...
    PRINT_NAMED_WARNING(
        "CubeBleClient.Update.NotHearingFromCube",
  ...
  while (_discoveryReceiver->getQueueLength() > 0) {
    ...
    if (cubeMessage.GetTag() == MessageCubeToEngineTag::available) {
      ...
      const bool connectedToThisCube =
          (_currentCube == msg.factory_id) &&
          (_cubeConnectionState == CubeConnectionState::Connected);
