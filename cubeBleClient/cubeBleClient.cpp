...
void CubeBleClient::StopScanning() { StopScanInternal(); }

bool CubeBleClient::SendMessageToLightCube(const MessageEngineToCube& msg) {
  bool result = false;
  if (ANKI_VERIFY(_cubeConnectionState == CubeConnectionState::Connected &&
                      !_currentCube.empty(),
...
bool CubeBleClient::RequestDisconnectFromCube() {
  const bool connectedOrPending =
      ((_cubeConnectionState == CubeConnectionState::Connected) ||
       (_cubeConnectionState == CubeConnectionState::PendingConnect)) &&
      !_currentCube.empty();
  bool result = false;
  if (ANKI_VERIFY(connectedOrPending,
                  "CubeBleClient.RequestDisconnectFromCube."
                  "NotConnectedOrPendingConnect",
                  "Current connection state %s, current cube '%s'",
                  CubeConnectionStateToString(_cubeConnectionState),
                  _currentCube.c_str())) {
