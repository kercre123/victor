...
class BleClient : public Anki::BluetoothDaemon::IPCClient {
 public:
  ...
  using AdvertisementCallback =
      std::function<void(const std::string& addr, const int rssi)>;
  using ReceiveDataCallback = std::function<void(
      const std::string& addr, const std::vector<uint8_t>& data)>;
  ...
  // Receive data callback gets called whenever we receive a message from the
  // connected cube. Messages from other cubes are ignored (and should not
  // happen normally).
