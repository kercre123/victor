 ...
 * Created: 3/28/2018
 *
 * Description: ble Client for ankibluetoothd. This is essentially a wrapper
 *around IPCClient. Contains a thread which runs the ev loop for communicating
 *              with the server - keep this in mind when writing callbacks.
 *Callbacks will occur on the ev loop thread.
 *
...
BleClient::BleClient(struct ev_loop* loop)
  ...
  _asyncStartScanSignal.set<BleClient, &BleClient::AsyncStartScanCallback>(
      this);
  _connectionCheckTimer
      .set<BleClient, &BleClient::ServerConnectionCheckTimerCallback>(this);
...
void BleClient::Stop() {
  ...
  // Signal the ev loop thread to break out of its loop and wait for the thread
  // to complete
...
bool BleClient::Send(const std::vector<uint8_t>& msg) {
  ...
  SendMessage(_connectionId, uuid, reliable, msg);
  return true;
}
...
void BleClient::DisconnectFromCube() {
  if (!IsConnectedToServer()) {
    PRINT_NAMED_WARNING(
        "BleClient.DisconnectFromCube.NotConnectedToServer",
...
void BleClient::FlashCube() {
  const bool canFlashCube = IsConnectedToServer() && (_connectionId >= 0) &&
                            _pendingFirmwareCheckOrUpdate;
  if (!canFlashCube) {
    PRINT_NAMED_WARNING(
        "BleClient.FlashCube.CannotFlashCube",
        "Cannot flash the cube - invalid BleClient state. ConnectedToServer "
        "%d, ConnectedToCube %d, PendingFirmwareCheckOrUpdate %d",
        IsConnectedToServer(), _connectionId >= 0,
        (int)_pendingFirmwareCheckOrUpdate);
    return;
  }
  ...
  size_t offset = kFirmwareVersionStrLen;  // skip the first few bytes, which is
...
void BleClient::OnScanResults(
    int error, const std::vector<BluetoothDaemon::ScanResultRecord>& records) {
  if (error != 0) {
    PRINT_NAMED_WARNING("BleClient.OnScanResults.Error",
                        "OnScanResults reporting error %d", error);
    return;
  }
...
void BleClient::OnOutboundConnectionChange(
  ...
  PRINT_NAMED_INFO("BleClient.OnOutboundConnectionChange",
                   "addr %s, connected %d, connection_id %d, ", address.c_str(),
                   connected, connection_id);

  if (address != _cubeAddress) {
    PRINT_NAMED_WARNING(
        "BleClient.OnOutboundConnectionChange.IgnoringUnexpected",
        "Ignoring unexpected %s from address %s (connection_id %d). Expected "
        "address: %s",
        connected ? "connection" : "disconnection", address.c_str(),
        connection_id, _cubeAddress.c_str());
    return;
  }
  ...
  if (connected) {
    ...
    // We were trying to connect, but received a disconnect notice instead.
    // Let's try again.
...
void BleClient::OnCharacteristicReadResult(
    const int connection_id, const int error,
    const std::string& characteristic_uuid, const std::vector<uint8_t>& data) {
  if ((connection_id < 0) || (connection_id != _connectionId)) {
    return;
  }
  ...
  const bool isAppVersion = Util::StringCaseInsensitiveEquals(
      characteristic_uuid, Anki::kCubeAppVersion_128_BIT_UUID);
  ...
  if (isAppVersion && _pendingFirmwareCheckOrUpdate) {
    ...
    RequestConnectionParameterUpdate(
        _cubeAddress, kGattConnectionIntervalHighPriorityMinimum,
        kGattConnectionIntervalHighPriorityMaximum,
        kGattConnectionLatencyDefault, kGattConnectionTimeoutDefault);

    // Check the cube's firmware version against the on-disk version
    DEV_ASSERT(!_cubeFirmwareVersionOnDisk.empty(),
               "BleClient.OnCharacteristicReadResult.NoOnDiskFirmwareVersion");
    if (cubeFirmwareVersion == _cubeFirmwareVersionOnDisk) {
      ...
      PRINT_NAMED_INFO(
          "BleClient.OnCharacteristicReadResult.FirmwareVersionMismatch",
          "Flashing cube since its firmware version (%s) does not match that "
          "on disk (%s)",
          cubeFirmwareVersion.c_str(), _cubeFirmwareVersionOnDisk.c_str());
      DASMSG(cube_firmware_mismatch, "cube.firmware_mismatch",
             "Flashing cube since its firmware version does not match that on "
             "disk");
...
void BleClient::OnReceiveMessage(const int connection_id,
  ...
  if (Util::StringCaseInsensitiveEquals(characteristic_uuid,
                                        Anki::kCubeAppVersion_128_BIT_UUID)) {
    const auto& cubeFirmwareVersion = std::string(value.begin(), value.end());
    if (cubeFirmwareVersion == _cubeFirmwareVersionOnDisk) {
      PRINT_NAMED_INFO("BleClient.OnReceiveMessage.FlashingSuccess", "%s",
                       cubeFirmwareVersion.c_str());
      DASMSG(cube_firmware_flash_success, "cube.firmware_flash_success",
             "Flashing cube firmware succeeded");
      DASMSG_SET(s1, _cubeAddress, "Cube factory ID");
      DASMSG_SET(s2, cubeFirmwareVersion, "Cube firmware version");
      DASMSG_SEND();
    } else {
      PRINT_NAMED_WARNING("BleClient.OnReceiveMessage.FlashingFailure",
                          "got = %s exp = %s", cubeFirmwareVersion.c_str(),
                          _cubeFirmwareVersionOnDisk.c_str());
      DASMSG(cube_firmware_flash_fail, "cube.firmware_flash_fail",
             "Flashing cube firmware failed");
      DASMSG_SET(s1, _cubeAddress, "Cube factory ID");
      DASMSG_SET(s2, cubeFirmwareVersion, "Cube firmware version");
      DASMSG_SET(s3, _cubeFirmwareVersionOnDisk, "On disk firmware version");
      DASMSG_SEND();

      // Disconnect from the cube, since there is no use in keeping the
      // connection with incorrect cube firmware
      DisconnectFromCube();
    }

    // consider the firmware check complete now.
    _pendingFirmwareCheckOrUpdate = false;
  } else if (Util::StringCaseInsensitiveEquals(
                 characteristic_uuid, Anki::kCubeAppRead_128_BIT_UUID)) {
...
void BleClient::ServerConnectionCheckTimerCallback(ev::timer& timer,
                                                   int revents) {
  ...
  if (!isConnected) {
    if (wasConnected) {
      PRINT_NAMED_WARNING(
          "BleClient.ServerConnectionCheckTimerCallback.DisconnectedFromServer",
          "We've become disconnected from the BLE server - attempting to "
          "reconnect");
    ...
    if (!Connect()) {
      PRINT_NAMED_WARNING(
          "BleClient.ServerConnectionCheckTimerCallback.ConnectFailed",
          "Unable to connect to ble server - will retry");
    }
  } else if (!wasConnected && isConnected) {
    PRINT_NAMED_INFO(
        "BleClient.ServerConnectionCheckTimerCallback.ConnectedToServer",
        "Connected to the BLE server!");
  }

  // Fire up the timer again for the appropriate interval
  timer.start(isConnected ? kDisconnectionCheckTime_sec
...
bool BleClient::GetCubeFirmwareFromDisk(std::vector<uint8_t>& firmware) {
  if (!Util::FileUtils::FileExists(_cubeFirmwarePath)) {
    PRINT_NAMED_ERROR(
        "BleClient.GetCubeFirmwareFromDisk.MissingCubeFirmwareFile",
  ...
  if (firmware.size() < kFirmwareVersionStrLen) {
    PRINT_NAMED_ERROR(
        "BleClient.GetCubeFirmwareFromDisk.CubeFirmwareFileTooSmall",
  ...
  _cubeFirmwareVersionOnDisk =
      std::string(firmware.begin(), firmware.begin() + kFirmwareVersionStrLen);

  PRINT_NAMED_INFO(
      "BleClient.GetCubeFirmwareFromDisk.ReadCubeFirmwareFileVersion",
