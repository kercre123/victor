 ...
 *
 * Description:
 *               Defines interface to BLE central process which communicates
 *with cubes (vic-os specific implementations)
 *
...
// shared queue for buffering advertisement messages received on the client
// thread
struct CubeAdvertisementInfo {
  CubeAdvertisementInfo(const std::string& addr, const int rssi)
      : addr(addr), rssi(rssi) {}
  std::string addr;
  int rssi;
};
...
CubeBleClient::CubeBleClient() {
  ...
  _bleClient->RegisterAdvertisementCallback(
      [](const std::string& addr, const int rssi) {
        std::lock_guard<std::mutex> lock(_cubeAdvertisementBuffer_mutex);
        _cubeAdvertisementBuffer.emplace(addr, rssi);
      });

  _bleClient->RegisterReceiveDataCallback(
  ...
  _bleClient->RegisterScanFinishedCallback([]() { _scanningFinished = true; });
}
...
bool CubeBleClient::UpdateInternal() {
  ...
  if (!_bleClient->IsConnectedToServer() &&
    ...
    PRINT_NAMED_WARNING("CubeBleClient.UpdateInternal.NotConnectedToDaemon",
                        "We are not connected to the bluetooth daemon - "
                        "setting connection state to %s. "
  ...
  if (_cubeConnectionState == CubeConnectionState::PendingConnect) {
    ...
    const bool connectionAttemptTimedOut =
        (_connectionAttemptFailTime_sec > 0.f) &&
        (now_sec > _connectionAttemptFailTime_sec);
    const bool firmwareUpdateTimedOut = (_firmwareUpdateFailTime_sec > 0.f) &&
                                        (now_sec > _firmwareUpdateFailTime_sec);

    if (connectionAttemptTimedOut || firmwareUpdateTimedOut) {
      PRINT_NAMED_WARNING(
          "CubeBleClient.UpdateInternal.ConnectionTimeout",
          "%s has taken more than %.2f seconds - aborting.",
          connectionAttemptTimedOut ? "Connection attempt"
                                    : "Firmware check or update",
          connectionAttemptTimedOut ? kConnectionAttemptTimeout_sec
                                    : kFirmwareUpdateTimeout_sec);
      DASMSG(cube_connection_failed, "cube.connection_failed",
  ...
  auto onConnect = [this]() {
    PRINT_NAMED_INFO("CubeBleClient.UpdateInternal.ConnectedToCube",
                     "Connected to cube %s", _currentCube.c_str());
  ...
  auto onDisconnect = [this]() {
    PRINT_NAMED_INFO("CubeBleClient.UpdateInternal.DisconnectedFromCube",
                     "Disconnected from cube %s", _currentCube.c_str());
    DASMSG(cube_disconnected, "cube.disconnected",
           "We have disconnected from a cube");
    DASMSG_SET(s1, _currentCube, "Cube factory ID");
    DASMSG_SEND();
    _cubeConnectionState = CubeConnectionState::UnconnectedIdle;
    for (const auto& callback : _cubeConnectionCallbacks) {
  ...
  if (connectedToCube &&
    ...
    PRINT_NAMED_WARNING(
        "CubeBleClient.UpdateInternal.UnexpectedConnectOrDisconnect",
        "Received unexpected %s. Previous connection state: %s",
        connectedToCube ? "connection" : "disconnection",
        CubeConnectionStateToString(_cubeConnectionState));
    DASMSG(cube_unexpected_connect_disconnect,
           "cube.unexpected_connect_disconnect",
           "Unexpectedly connected or disconnected from a cube");
    DASMSG_SET(
        i1, connectedToCube,
        "1 if we have connected to a cube, 0 or null if we have disconnected");
    DASMSG_SET(s1, _currentCube, "Cube factory ID");
    DASMSG_SET(s2, CubeConnectionStateToString(_cubeConnectionState),
  ...
  while (!swapCubeAdvertisementBuffer.empty()) {
    ...
    msg.objectType =
        ObjectType::Block_LIGHTCUBE1;  // TODO - update this with the Victor
                                       // cube type once it's defined
    ...
    if (_cubeConnectionState == CubeConnectionState::ScanningForCubes) {
      ...
      PRINT_NAMED_WARNING("CubeBleClient.UpdateInternal.IgnoringAdvertisement",
                          "Ignoring cube advertisement message from %s since "
                          "we are not scanning for cubes. "
  ...
  while (!swapCubeMsgRecvBuffer.empty()) {
    ...
    if (_cubeConnectionState == CubeConnectionState::Connected) {
      ...
      PRINT_NAMED_WARNING(
          "CubeBleClient.UpdateInternal.IgnoringCubeMsg",
...
bool CubeBleClient::RequestConnectInternal(const BleFactoryId& factoryId) {
  ...
  DEV_ASSERT(_currentCube.empty(),
             "CubeBleClient.RequestConnectInternal.CubeAddressNotEmpty");

  _currentCube = factoryId;
  _cubeConnectionState = CubeConnectionState::PendingConnect;

  PRINT_NAMED_INFO("CubeBleClient.RequestConnectInternal.AttemptingToConnect",
                   "Attempting to connect to cube %s", _currentCube.c_str());

  DEV_ASSERT(_connectionAttemptFailTime_sec < 0.f,
             "CubeBleClient.RequestConnectInternal."
             "UnexpectedConnectionAttemptFailTime");
  const float now_sec =
...
bool CubeBleClient::RequestDisconnectInternal() {
  if (!_bleClient->IsConnectedToCube()) {
    PRINT_NAMED_WARNING("CubeBleClient.RequestDisconnectInternal.NotConnected",
                        "We are not connected to any cubes! Telling BleClient "
                        "to disconnect anyway to be safe. "
                        "Current connection state: %s. Setting connection "
