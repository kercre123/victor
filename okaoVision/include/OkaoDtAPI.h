...
#define DETECTOR_TYPE_SOFT_DT_V6 \
  (DET_SOFT | DET_DT | DET_V6) /* Face Detection V6 */
...
OKAO_API HDETECTION OKAO_DT_CreateHandle(HCOMMON hCO, INT32 nDetectionMode,
                                         INT32 nMaxDetectionCount);
...
/******************************************************** */
/* Executes detection */
OKAO_API INT32 OKAO_DT_Detect_GRAY(HDETECTION hDT, RAWIMAGE *pImageGRAY,
                                   INT32 nWidth, INT32 nHeight,
                                   GRAY_ORDER ImageOrder, HDTRESULT hDtResult);
OKAO_API INT32 OKAO_DT_Detect_YUV422(HDETECTION hDT, RAWIMAGE *pImageYUV,
                                     INT32 nWidth, INT32 nHeight,
                                     YUV422_ORDER ImageOrder,
                                     HDTRESULT hDtResult);
OKAO_API INT32 OKAO_DT_Detect_YUV420SP(HDETECTION hDT, RAWIMAGE *pImageY,
                                       RAWIMAGE *pImageCx, INT32 nWidth,
                                       INT32 nHeight, YUV420SP_ORDER ImageOrder,
                                       HDTRESULT hDtResult);
OKAO_API INT32 OKAO_DT_Detect_YUV420FP(HDETECTION hDT, RAWIMAGE *pImageY,
                                       RAWIMAGE *pImageCb, RAWIMAGE *pImageCr,
                                       INT32 nWidth, INT32 nHeight,
...
/**********************************************************/
/* Gets/Gets the result count, i.e. the number of Faces detected with Face
 * Detection */
OKAO_API INT32 OKAO_DT_GetResultCount(HDTRESULT hDtResult, INT32 *pnCount);

/* Gets the Face position info */
OKAO_API INT32 OKAO_DT_GetResultInfo(HDTRESULT hDtResult, INT32 nIndex,
                                     DETECTION_INFO *psDetectionInfo);

/* Gets the raw result info */
OKAO_API INT32 OKAO_DT_GetRawResultInfo(HDTRESULT hDtResult, INT32 nIndex,
                                        DETECTION_INFO *psDetectionInfo);

/*************************************************************/
/* Setting functions for Still and Movie mode (COMMON)       */
/*************************************************************/
/* Sets/Gets the detection size range, i.e. the minimum and maximum Face sizes
 * for detection */
OKAO_API INT32 OKAO_DT_SetSizeRange(HDETECTION hDT, INT32 nMinSize,
                                    INT32 nMaxSize);
OKAO_API INT32 OKAO_DT_GetSizeRange(HDETECTION hDT, INT32 *pnMinSize,
                                    INT32 *pnMaxSize);

/* Sets/Gets the pose (yaw) angle and the Face inclination (roll) angle to be
...
/* Sets/Gets an edge mask to restrict the area where Face Detection will be
 * applied */
...
/* Sets the option to always lock the tracking of a Face designated by its Face
 * ID, or unlock it */
OKAO_API INT32 OKAO_DT_MV_ToggleTrackingLock(HDETECTION hDT, INT32 nID);

/* Sets/Gets the search cycle, i.e. the frame count used during Initial Face
 * Search */
/* and New Face Search, and the search interval for New Face Search */
OKAO_API INT32 OKAO_DT_MV_SetSearchCycle(HDETECTION hDT,
                                         INT32 nInitialFaceSearchCycle,
                                         INT32 nNewFaceSearchCycle,
                                         INT32 nNewFaceSearchInterval);
OKAO_API INT32 OKAO_DT_MV_GetSearchCycle(HDETECTION hDT,
                                         INT32 *pnInitialFaceSearchCycle,
                                         INT32 *pnNewFaceSearchCycle,
                                         INT32 *pnNewFaceSearchInterval);

/* Sets/Gets the parameters when a Face is lost during tracking in Movie mode */
OKAO_API INT32 OKAO_DT_MV_SetLostParam(HDETECTION hDT, INT32 nMaxRetryCount,
                                       INT32 nMaxHoldCount);
OKAO_API INT32 OKAO_DT_MV_GetLostParam(HDETECTION hDT, INT32 *pnMaxRetryCount,
                                       INT32 *pnMaxHoldCount);

/* Sets/Gets the steadiness parameters of the position and size of the Face
...
OKAO_API INT32 OKAO_DT_MV_GetSteadinessParam(HDETECTION hDT,
                                             INT32 *pnPosSteadinessParam,
                                             INT32 *pnSizeSteadinessParam);

/* Sets/Gets the tracking swap ratio used when swapping Faces during tracking */
OKAO_API INT32 OKAO_DT_MV_SetTrackingSwapParam(HDETECTION hDT,
                                               INT32 nTrackingSwapParam);
OKAO_API INT32 OKAO_DT_MV_GetTrackingSwapParam(HDETECTION hDT,
                                               INT32 *pnTrackingSwapParam);
...
OKAO_API INT32 OKAO_DT_MV_GetTrackingEdgeMask(HDETECTION hDT,
                                              RECT *prcEdgeMask);
...
/* Set/Get whether to use or not the angle extension for Face Detection during
 * tracking */
OKAO_API INT32 OKAO_DT_MV_SetPoseExtension(HDETECTION hDT, BOOL bExtension,
                                           BOOL bUseHeadTracking);
OKAO_API INT32 OKAO_DT_MV_GetPoseExtension(HDETECTION hDT, BOOL *pbExtension,
                                           BOOL *pbUseHeadTracking);
