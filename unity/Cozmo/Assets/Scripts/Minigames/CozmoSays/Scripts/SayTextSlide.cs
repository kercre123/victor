...
public class SayTextSlide : MonoBehaviour {
  ...
  private char HandleInputValidation(string input, int charIndex, char addedChar) {
    return Cozmo.UI.CozmoInputFilter.IsValidInput(addedChar, allowPunctuation: true, allowDigits: false) ? addedChar : '\0';
  }
  ...
  private void SetSayTextReactionaryBehaviors(bool enable) {
    if (RobotEngineManager.Instance.CurrentRobot != null) {
      if (!enable) {
        RobotEngineManager.Instance.CurrentRobot.DisableReactionsWithLock(ReactionaryBehaviorEnableGroups.kSayTextSlideId, ReactionaryBehaviorEnableGroups.kSayTextSlideTriggers);
      }
      else {
        RobotEngineManager.Instance.CurrentRobot.RemoveDisableReactionsLock(ReactionaryBehaviorEnableGroups.kSayTextSlideId);
      }
    }
  }
    ...
    if (hasBadWords) {
      ...
      RobotActionUnion[] actions = {
        ...
        new RobotActionUnion().Initialize(new SayTextWithIntent().Initialize(
          ...
          true)),
        new RobotActionUnion().Initialize(new PlayAnimationTrigger().Initialize(1, getOutTrigger, true, false, false, false))
      };
