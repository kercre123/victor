    ...
    callback: function() {
      ...
      if (deleteList.length < 2 ) {
        ...
        Blockly.confirm(Blockly.Msg.DELETE_ALL_BLOCKS.
            ...
            function(ok) {
              window.player.play('click'); // *** ANKI CHANGE ***
              if (ok) {
                deleteNext();
              }
            });
 ...
 * // *** ANKI CHANGE ***
 *
 * This playAudio implementation is a replacement for the original playAudio method.
 * Original Scratch 3.0 code is below this method (see loadAudio, preloadAudio and
 * playAudio).
 *
 * The Scratch code uses the HTML5 Audio element, an older API. The replacement
 * code uses HTML5 Web Audio API.
 * 
 * See webAudioPlayer.js for more information.
 *
 * Original Scratch docs for playAudio:
...
Blockly.WorkspaceSvg.prototype.playAudio = function(name, opt_volume) {
  // TODO Support opt_volume?
  if (window.player) {
    window.player.play(name);
  }
};
