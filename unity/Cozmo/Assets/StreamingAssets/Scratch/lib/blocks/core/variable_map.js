      ...
      Blockly.confirm(confirmText,
          function(ok) {
            if (ok) {
              ...
              // *** ANKI CHANGE ***
              // With the Anki custom modal, the callback to Blockly.confirm is executed later than
              // with the standard JavaScript modal that Scratch uses. This difference in execution steps
              // causes refreshToolboxSelection_ not to get called up the stack in the Anki case.
              // Calling it here instead so that, in the case of variables being deleted, the variable being deleted
              // no longer shows in the flyout.
              map.workspace.refreshToolboxSelection_();
            }
          });
    } else {
