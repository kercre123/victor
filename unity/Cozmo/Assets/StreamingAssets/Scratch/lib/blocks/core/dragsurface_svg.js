...
Blockly.DragSurfaceSvg.prototype.translateAndScaleGroup = function(x, y, scale) {
  ...
  if (Blockly.is3dSupported()) {
    // To turn on is3dSupported() for Android, turn on hardware acceleration (which can
    // be done in the manifest file).
    //
    // '-webkit-transform' is required for Android 4.4. 'transform' does not work.
    transform = '-webkit-transform: translate3d(' + x + 'px, ' + y + 'px, 0px)' +
...
Blockly.DragSurfaceSvg.prototype.translateSurface = function(x, y) {
  ...
  if (Blockly.is3dSupported()) {
    ...
    //
    // '-webkit-transform' is required for Android 4.4. 'transform' does not work.
    transform = '-webkit-transform: translate3d(' + x + 'px, ' + y + 'px, 0px); display: block;';
