...
public class PingStatus : MonoBehaviour {
  ...
  void OnDestroy() {
    if (_coroutine != null) {
      Debug.Log("Stopping ping coroutine");
  ...
  private IEnumerator PingCoroutine() {
    Debug.Log("Pinging " + RobotEngineManager.kRobotIP);
    ...
    while (!_Ping.isDone && ((Time.time - startTime) < kKillPingTime)) {
      if ((Time.time - _LastLogTime) > kLogTime) {
        Debug.Log("Still pinging " + RobotEngineManager.kRobotIP);
        _LastLogTime = Time.time;
      }
      yield return null;
    }
    ...
    if (_Ping.isDone) {
      Debug.Log("Ping completed. Time is = " + _Ping.time);
    } else {
