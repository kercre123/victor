...
public class VirtualStick : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler {
  ...
  public float Horizontal {
    get { 
      ...
      if (Mathf.Abs(hor) > horClampThrow)
        hor = horClampThrow * Mathf.Sign(hor);
  ...
  void OnEnable() {
    ...
    if (hint != null)
      hint.gameObject.SetActive(!bg.gameObject.activeSelf);
  ...
  void ResizeToScreen() {
    ...
    if (stickWidthInches > 0f && stickHeightInches > 0f) {
      ...
      if (capTop != null)
        capTop.sizeDelta = size;
      if (capBottom != null)
        capBottom.sizeDelta = size;
      if (capLeft != null)
        capLeft.sizeDelta = size;
      if (capRight != null)
        capRight.sizeDelta = size;

      if (originMarker != null)
        originMarker.sizeDelta = size;
    ...
    if (capTop != null)
      capTop.anchoredPosition = Vector2.up * height * 0.5f;
    if (capBottom != null)
      capBottom.anchoredPosition = -Vector2.up * height * 0.5f;
    if (capLeft != null)
      capLeft.anchoredPosition = -Vector2.right * width * 0.5f;
    if (capRight != null)
      capRight.anchoredPosition = Vector2.right * width * 0.5f;

    if (originMarker != null)
      originMarker.anchoredPosition = Vector2.zero;
    ...
    if (deadZoneRect != null) {
      ...
      if (deadZoneRadial) {
        ...
        if (deadZone.x > 0f && !verticalOnly)
          size.x = deadZone.x * width;
        if (deadZone.y > 0f && !horizontalOnly)
  ...
  void RefreshStickImages() {
    if (stickImageThrown == null)
      return;
    if (stickImageNeutral == null)
      return;
    ...
    if (horizontalOnly || SideModeEngaged)
      thrown = throwVector.x != 0f;
    if (verticalOnly || VertModeEngaged)
      thrown = throwVector.y != 0f;
  ...
  void RefreshBGImages() {
    if (bgDefaultMode != null)
      bgDefaultMode.gameObject.SetActive(!UpModeEngaged && !DownModeEngaged && !SideModeEngaged && !VertModeEngaged);

    if (bgVertMode != null)
      bgVertMode.gameObject.SetActive(VertModeEngaged);
    if (bgUpMode != null)
      bgUpMode.gameObject.SetActive(UpModeEngaged);
    if (bgDownMode != null)
      bgDownMode.gameObject.SetActive(DownModeEngaged);
    if (bgSideMode != null)
      bgSideMode.gameObject.SetActive(SideModeEngaged);


    if (deadZoneHModeRect != null)
      deadZoneHModeRect.gameObject.SetActive(SideModeEngaged);
    if (deadZoneVModeRect != null)
      deadZoneVModeRect.gameObject.SetActive(VertModeEngaged);
  }
  ...
  void ConsiderSwipe(bool underway) {
    ...
    if (!allowAxisSwipes && !allowPreciseSwipes)
      return;
    ...
    if (allowPreciseSwipes && !horizontalOnly && !verticalOnly)
      return;
