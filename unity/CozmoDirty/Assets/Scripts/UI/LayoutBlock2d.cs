...
public class LayoutBlock2d : MonoBehaviour {
  ...
  public void SetLights(Color color1, Color color2, Color color3, Color color4) {
    if (images_LED.Length > 0)
      images_LED[0].color = color1;
    if (images_LED.Length > 1)
      images_LED[1].color = color2;
    if (images_LED.Length > 2)
      images_LED[2].color = color3;
    if (images_LED.Length > 3)
      images_LED[3].color = color4;
  }

  public void Validate(bool valid = true) {
    if (valid && !Validated) {
      if (validatedSound != null)
        AudioManager.PlayOneShot(validatedSound);
    }
