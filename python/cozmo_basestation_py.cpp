 ...
 * Created: 05/25/17
 *
 * Description: Cozmo basestation functionality that is exposed for use in
 *Python.
 *
...
ProceduralFace GetProcFaceFromVals(boost::python::list& leftEyeList,
                                   boost::python::list& rightEyeList,
                                   double faceAngle_deg, double faceCenterX,
                                   double faceCenterY, double faceScaleX,
                                   double faceScaleY) {
  ...
  procFace.SetFromValues(
      leftEyeVector, rightEyeVector, static_cast<f32>(faceAngle_deg),
...
class FaceImage {
 ...
 public:
  FaceImage(boost::python::list&, boost::python::list&, double, double, double,
            double, double);
  void Interpolate(boost::python::list&, boost::python::list&, double, double,
...
FaceImage::FaceImage(boost::python::list& leftEye,
                     boost::python::list& rightEye, double faceAngle_deg,
                     double faceCenterX, double faceCenterY, double faceScaleX,
                     double faceScaleY) {
  _procFace = GetProcFaceFromVals(leftEye, rightEye, faceAngle_deg, faceCenterX,
                                  faceCenterY, faceScaleX, faceScaleY);
  _faceImg = ProceduralFaceDrawer::DrawFace(_procFace);
}

void FaceImage::Interpolate(boost::python::list& leftEye,
                            boost::python::list& rightEye, double faceAngle_deg,
                            double faceCenterX, double faceCenterY,
                            double faceScaleX, double faceScaleY,
                            double blendFraction) {
  ProceduralFace interpolatedProcFace;
  ProceduralFace secondProcFace =
      GetProcFaceFromVals(leftEye, rightEye, faceAngle_deg, faceCenterX,
                          faceCenterY, faceScaleX, faceScaleY);
  interpolatedProcFace.Interpolate(_procFace, secondProcFace,
                                   static_cast<float>(blendFraction));
...
boost::python::tuple FaceImage::GetPixelList() {
  ...
  return boost::python::make_tuple(_faceImg.GetNumRows(), _faceImg.GetNumCols(),
                                   pixels);
}
...
boost::python::tuple FaceImage::GetPixelMemory() {
  ...
  DEV_ASSERT(_faceImg.IsContinuous(),
             "FaceImage.GetPixelMemory.FaceImageNotContinuous");
  const uint8_t* imageData = _faceImg.GetDataPointer();
  PyObject* pyObj = PyMemoryView_FromObject(
      PyBuffer_FromMemory(const_cast<uint8_t*>(imageData), size));
  boost::python::object memoryView(
...
BOOST_PYTHON_MODULE(cozmo_basestation_py) {
  ...
  // The methods that are exposed for use in Python use the
  // lowercase_with_underscore naming convention to adhere to the PEP-8 style
  // guide for Python code.

  class_<FaceImage>("FaceImage",
                    init<boost::python::list&, boost::python::list&, double,
                         double, double, double, double>())
