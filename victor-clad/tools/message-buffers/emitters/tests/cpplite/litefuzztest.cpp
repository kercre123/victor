...
int main(int argc, char **argv) {
  ...
  if (type_name != correct_type_name) {
    std::cerr << "State is for wrong type. Expected "
              << correct_type_name + ", but got " << type_name << std::endl;
    return 1;
  }
  ...
  std::generate_n(reinterpret_cast<uint8_t *>(&message), sizeof(MessageType),
                  std::rand);

  std::cout << "Unpacking " << correct_type_name << " from " << filename
            << "..." << std::endl;
  ...
  if (!message.IsValid()) {
    std::cerr << "Invalid message: failed to parse (and size is "
              << message.Size() << " vs expected " << dataSize << ")"
              << std::endl;
    return 1;
  }

  if (message.Size() != dataSize) {
    std::cerr << "Valid message, wrong size: size is " << message.Size()
              << " vs expected " << dataSize << std::endl;
    return 1;
  }
