...
void app_validate(const void* data) {
  const OTA_Validate* cert = (const OTA_Validate*) data;
  int size = cert->length;

  // Clean unverified space
  memset(&app_write_address[size], 0, sizeof(ApplicationMap) - size);

  // Size limit
  if (size < minimum_length || size > sizeof(ApplicationMap)) {
    return ;
  }
  ...
  uint32_t crc = ~0;
  {
    const uint8_t *buf = app_write_address;
...
{
  app_erase();  // Wipe any existing valid application
  const OTA_Packet* packet = (const OTA_Packet*) param->value;

  int length = param->length - sizeof(OTA_Packet);
  ...
  switch (packet->address) {
    case 0xFFFE:
      ...
      break ;
    case 0xFFFF:
      app_validate(param->value);
      break ;
    default:
      memcpy(&app_write_address[packet->address], packet->data, length);
      break ;
  }
}
