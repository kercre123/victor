...
echo Building...
ssh nathan@anki-vm-keil 'bash -lc "cd '"$HOSTNAME/$PRJ"';./convert_projx_to_uv4.sh";bash -lc "cd '"$HOSTNAME/$PRJ/syscon"';/cygdrive/c/Keil_v4/UV4/UV4.exe -j0 -b sysboot.uvproj -o build.log;cat build.log;rm build.log";bash -lc "cd '"$HOSTNAME/$PRJ/syscon"';export COZMO2_CERT="$HOSTNAME/$PRJ/syscon/tools/development.pem";/cygdrive/c/Keil_v4/UV4/UV4.exe -j0 -b syscon.uvproj -o build.log;cat build.log;rm build.log"'
echo Copying back...
rsync -rte "ssh" nathan@anki-vm-keil:$HOSTNAME/$PRJ ..
cd syscon
export COZMO2_CERT=./tools/development.pem
python3 ./tools/sign.py -b build/syscon.bin ./build/syscon/syscon.axf
cd ..
