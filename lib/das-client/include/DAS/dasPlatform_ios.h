...
class DASPlatform_IOS : public IDASPlatform {
 public:
  ...
  // app info
  virtual const char* GetAppVersion() const override {
    return _appVersion.c_str();
  }

  // device/OS info
  virtual const char* GetDeviceId() const override { return _deviceId.c_str(); }
  virtual const char* GetDeviceModel() const override {
    return _deviceModel.c_str();
  }
  virtual const char* GetOsVersion() const override {
    return _osVersion.c_str();
  }
  virtual const char* GetCombinedSystemVersion() const override {
    return _combinedSystemVersion.c_str();
  }
  virtual const char* GetPlatform() const override { return _platform.c_str(); }

  // storage/battery
  virtual const char* GetFreeDiskSpace() const override {
    return _freeDiskSpace.c_str();
  }
  virtual const char* GetTotalDiskSpace() const override {
    return _totalDiskSpace.c_str();
  }
  virtual const char* GetBatteryLevel() const override {
    return _batteryLevel.c_str();
  }
  virtual const char* GetBatteryState() const override {
    return _batteryState.c_str();
  }

  // other misc platform-specific info
  virtual const std::map<std::string, std::string>& GetMiscInfo()
      const override {
    return _miscInfo;
  }
  virtual const std::map<std::string, std::string>& GetMiscGlobals()
      const override {
    return _miscGlobals;
  }
