...
size_t curl_write_function(char *ptr, size_t size, size_t nmemb,
                           std::string *userdata) {
  userdata->append(ptr, size * nmemb);
  return size * nmemb;
}

bool dasPostToServer(const std::string &url, const std::string &postInfo,
  ...
  int rv =
      deflateInit2(&stream, zLevel, zMethod, zWindowSize, zMemLevel, zStrategy);
  if (rv != Z_OK) {
    LOGE("dasPostToServer.deflateInit2: Unable to initialize stream (error %d)",
         rv);
    return false;
  }
  ...
  if (rv != Z_OK) {
    LOGE(
        "dasPostToServer.deflateSetHeader: Unable to initialize header (error "
        "%d)",
        rv);
    return false;
  }
  ...
  if (curl) {
    ...
    const std::string &escapedBody =
        curl_easy_escape(curl, encodedBody.c_str(), encodedBody.size());
    ...
    postBody.append(
        "&MessageAttribute.3.Value.StringValue=application%2Fvnd.anki%2Bjson%"
        "3B+format%3Dnormal%3B+product%3Dvic");
