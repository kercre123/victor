...
DasAppender::DasAppender(const std::string& dasLogDir, const std::string& url,
                         uint32_t flush_interval, size_t maxLogLength,
                         size_t maxLogFiles,
  ...
  bool mkdirSuccess =
      (0 == mkdir(dasLogDir.c_str(), S_IRWXU)) || (EEXIST == errno);
  if (mkdirSuccess) {
    _logFileAppender = new Anki::Das::DasLogFileAppender(
        dasLogDir, maxLogLength, maxLogFiles, archiveFunction,
        unarchiveFunction, archiveFileExtension);
    _syncQueue.Wake([this]() {
      _logFileAppender->RolloverAllLogFiles();
      Flush();
    });
  } else {
    LOGD("ERROR! Couldn't create DAS log directory('%s'): %s",
         dasLogDir.c_str(), strerror(errno));
    // TODO: Try to send this log message directly to the endpoint over the
    // network
  }
}
...
void DasAppender::append(DASLogLevel level, const char* eventName,
                         const char* eventValue, ThreadId_t threadId,
  ...
  _syncQueue.Wake([this, eventDictionary = std::move(eventDictionary),
                   curSequence]() mutable {
    ...
    if (eventDictionary.end() ==
        eventDictionary.find(kMessageVersionGlobalKey)) {
...
void DasAppender::SetTimedFlush() {
  ...
  if (!_isWaitingForFlush && _flushIntervalSeconds != 0) {
    ...
    auto when = std::chrono::system_clock::now() +
                std::chrono::seconds(_flushIntervalSeconds);
    _syncQueue.WakeAfter([this]() { Flush(); }, when);
  }
}

void DasAppender::ForceFlush() { ForceFlushWithCallback({}); }
...
void DasAppender::Flush() {
  ...
  if (_logFileAppender != nullptr) {
    if (!_isUploadingPaused) {
      auto func = std::bind(&DasAppender::ConsumeALogFile, this,
...
bool DasAppender::ConsumeALogFile(const std::string& logFilePath, bool* stop) {
  ...
  if (logFileLength > kDasMaxAllowableLogLength) {
    ...
    DASError("dasappender.consumealogfile.filetoobig",
             "File %s too big (%zu bytes with max %zu). Pretending it was "
             "consumed so it will be deleted.",
  ...
  if (success) {
    _lastFlushFailed = false;
    DASEvent("dasappender.postdasdata.success", "%s %s", logFilePath.c_str(),
             postResponse.c_str());
  } else {
    *stop = true;
    _lastFlushFailed = true;
    if (postResponse.empty()) {
      DASEvent("dasappender.postdasdata.failurenoresponse", "%s",
               logFilePath.c_str());
    } else {
      DASEvent("dasappender.postdasdata.failurewithresponse", "%s %s",
