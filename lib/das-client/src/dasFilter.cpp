...
#pragma mark FilterNode
  class FilterNode {
   ...
   public:
    ...
    DASLogLevel GetMinLogLevel(
        std::vector<const std::string>::const_iterator& logger_cur,
      ...
      if (logger_cur == logger_end) {
        ...
        if (it == _children.end()) {
          ...
          return it->second.GetMinLogLevel(++logger_cur, logger_end);
        }
      }
    }
    ...
    void SetMinLogLevel(const DASLogLevel logLevel) { _logLevel = logLevel; }
...
DASFilter::DASFilter() : pimpl(new DasFilterImpl()) {}

DASFilter::~DASFilter() { delete pimpl; }
...
void DASFilter::SetMinLogLevel(const std::string& loggerName,
                               DASLogLevel level) {
