...
DasLogFileAppender::DasLogFileAppender(
    const std::string& logDirPath, size_t maxLogLength, size_t maxLogFiles,
...
void DasLogFileAppender::UpdateLogFilePath() {
  _currentLogFileName =
      MakeLogFilePath("", _currentLogFileNumber, kDasInProgressExtension);
}
...
void DasLogFileAppender::OpenHandleToCurrentLogFile() {
  _currentLogFileHandle.open(FullLogFilePath(),
                             std::ofstream::out | std::ofstream::app);
  if (!_currentLogFileHandle) {
    LOGD("Error getting handle for file %s: %s !!", FullLogFilePath().c_str(),
         strerror(errno));
  }
}
...
std::string DasLogFileAppender::MakeLogFilePath(
    const std::string& logDir, uint32_t logNumber,
  ...
  pathStream << std::setfill('0') << std::setw(4) << logNumber << "."
             << extension;
...
uint32_t DasLogFileAppender::NextAvailableLogFileNumber(
  ...
  while (!foundFileNumber) {
    ...
    std::string candidatePath =
        MakeLogFilePath(_logDirPath, fileNumber, kDasLogFileExtension);
    ...
    if (foundFileNumber && !_archiveFileExtension.empty()) {
      const auto fullArchiveExtension =
          std::string(kDasLogFileExtension + _archiveFileExtension);
      candidatePath =
          MakeLogFilePath(_logDirPath, fileNumber, fullArchiveExtension);
      if (AnkiUtil::FileExistsAtPath(candidatePath)) {
        foundFileNumber = false;
      }
    }
    ...
    if (foundFileNumber) {
      candidatePath =
          MakeLogFilePath(_logDirPath, fileNumber, kDasInProgressExtension);
      if (AnkiUtil::FileExistsAtPath(candidatePath)) {
        foundFileNumber = false;
      }
    }
    ...
    if (!foundFileNumber) {
      ...
      if (fileNumber == startNumber) {
        ...
        // FIXME: we should log the fact that we've rolled over, since it means
        // we're losing data.
        foundFileNumber = true;
      }
    }
...
std::string DasLogFileAppender::CurrentLogFilePath() {
  ...
  _loggingQueue.WakeSync(
      [this, &logFilePath] { logFilePath = PrvCurrentLogFilePath(); });
  return logFilePath;
}
...
void DasLogFileAppender::PrvFlush() { CurrentLogFileHandle().flush(); }
  ...
  if (logDir) {
    ...
    while ((dirent = readdir(logDir)) != nullptr) {
      ...
      if (DT_REG == dirent->d_type &&
          stringEndsWith(filename, kDasInProgressExtension)) {
...
void DasLogFileAppender::ConsumeLogFiles(
    DASLogFileConsumptionBlock ConsumptionBlock) {
  _loggingQueue.WakeSync(
      [this, ConsumptionBlock = std::move(ConsumptionBlock)] {
        DIR* logDir = opendir(_logDirPath.c_str());
        ...
        if (logDir) {
          ...
          while (!shouldStop && fileIter != fileList.end()) {
            ...
            ++fileIter;
            // If we have a callback set for unarchiving and this file matches,
            // unarchive it
            if (_unarchiveCallback &&
                stringEndsWith(filename, _archiveFileExtension)) {
            ...
            if (stringEndsWith(filename, kDasLogFileExtension)) {
              ...
              if (shouldDelete) {
                ...
                if (!success) {
                  LOGD("Error removing file '%s': %s", fullPath.c_str(),
                       strerror(errno));
                }
              }
