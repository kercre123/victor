...
TEST(DasAppenderTest, DasAppenderConstructor) {
  ...
  EXPECT_EQ(0, access(testDASDir.c_str(), F_OK))
      << testDASDir << " should have been created";
  delete testAppender;
...
TEST(DasAppenderTest, DasAppenderCleanupOldEmptyInProgress) {
  (void)rmrf(testDASDir.c_str());
  bool mkdirSuccess =
      (0 == mkdir(testDASDir.c_str(), S_IRWXU)) || (EEXIST == errno);
  EXPECT_TRUE(mkdirSuccess);
  std::string inProg2 = testDASDir + "/02." +
                        Anki::Das::DasLogFileAppender::kDasInProgressExtension;
  std::string inProg3 = testDASDir + "/03." +
                        Anki::Das::DasLogFileAppender::kDasInProgressExtension;
  EXPECT_EQ(0, truncate_to_zero(inProg2.c_str()));
  EXPECT_EQ(0, truncate_to_zero(inProg3.c_str()));
  EXPECT_EQ(0, access(inProg2.c_str(), F_OK))
      << inProg2 << " should have been created";
  EXPECT_EQ(0, access(inProg3.c_str(), F_OK))
      << inProg3 << " should have been created";
  Anki::Das::DasAppender* testAppender =
      new Anki::Das::DasAppender(testDASDir, testDASURL);
  EXPECT_EQ(0, access(inProg2.c_str(), F_OK))
      << inProg2 << " should not be deleted yet";
  EXPECT_EQ(0, access(inProg3.c_str(), F_OK))
      << inProg3 << " should not be deleted yet";
  testAppender->SetIsUploadingPaused(false);
  delete testAppender;
  testAppender = nullptr;
  EXPECT_EQ(-1, access(inProg2.c_str(), F_OK))
      << inProg2 << " should have been rolled over";
  EXPECT_EQ(-1, access(inProg3.c_str(), F_OK))
      << inProg3 << " should have been rolled over";
  (void)rmrf(testDASDir.c_str());
}
...
TEST(DasAppenderTest, DasAppenderFlushWithCallback) {
  ...
  testAppender->append(DASLogLevel_Info, "asdf", "asdf", 0, "asdf", "asdf", 0,
                       &tempMap, tempMap);
