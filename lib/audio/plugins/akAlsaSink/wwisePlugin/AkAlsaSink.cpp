...
BEGIN_MESSAGE_MAP(CAkAlsaSinkApp, CWinApp)
END_MESSAGE_MAP()

CAkAlsaSinkApp::CAkAlsaSinkApp()
{

}

// The one and only CAkAlsaSinkApp object
CAkAlsaSinkApp theApp;

// CAkAlsaSinkApp initialization
BOOL CAkAlsaSinkApp::InitInstance()
{
	CWinApp::InitInstance();
	// There is no Windows Alsa RunTime no need to register.
	return TRUE;
}

AK::Wwise::IPluginBase* __stdcall AkCreatePlugin(unsigned short in_usCompanyID, unsigned short in_usPluginID)
{
	return new AkAlsaSinkPlugin();
}
