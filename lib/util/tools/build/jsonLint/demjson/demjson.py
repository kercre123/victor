    ...
    def decode_number(self, s, i=0, imax=None):
        ...
        if s[j:j+3] == 'NaN':
            if self._allow_non_numbers:
                ...
                raise JSONDecodeError('NaN literals are not allowed in strict JSON', s[j:])
            ...
            if self._allow_non_numbers:
                ...
                raise JSONDecodeError('Infinity literals are not allowed in strict JSON', s[j:])
                ...
                if i < imax and (txt[i] == ',' or txt[i] == closer):
                    ...
                    if c == ',':
                        if not saw_value:
                            ...
                            if isdict:
                                raise JSONDecodeError('can not omit elements of an object (dictionary)', txt[i:])
