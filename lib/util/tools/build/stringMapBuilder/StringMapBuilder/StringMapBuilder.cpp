...
StringMapBuilder::MapChangeInfo& StringMapBuilder::MapChangeInfo::operator|=(
    const StringMapBuilder::MapChangeInfo& rhs) {
...
bool StringMapBuilder::MapChangeInfo::HadAnyErrors() const {
  ...
  return _missingKeysAreStillPresent || _foundDuplicateKeys ||
         _failedToPopulate;
}

void StringMapBuilder::AddMapForKey(const StringToInt& map, const string& key,
                                    ptree& target) {
  ptree data;
...
// Note: CompareMaps doesn't just "compare", it fills in ids in map using any
// ids found in stringMap.json
StringMapBuilder::MapChangeInfo StringMapBuilder::CompareMaps(
    StringToInt& map, string key, ptree config, bool removeMissingKeys) {
  StringMapBuilder::MapChangeInfo mapChangeInfo;
  ...
  if (optionalList) {
    ...
    BOOST_FOREACH (const boost::property_tree::ptree::value_type& v,
      ...
      // get item id
      string stringId =
          v.second.get(AnkiUtil::kP_STRING_ID, AnkiUtil::kP_UNDEFINED);
      int intId = v.second.get(AnkiUtil::kP_INT_ID, -1);

      // cout << "looking for " << key << " " << stringId << " - " << intId <<
      // "\n";
      // save max value
      if (maxIntId < intId) maxIntId = intId;

      StringToInt::iterator it = map.find(stringId);
      if (it != map.end()) {
        it->second = intId;
      } else {
        // a key in target is no longer present - e.g. a key has been removed,
        // if removeMissingKeys is true this old key is kept for backwards
        // compatability
  ...
  for (StringToInt::iterator it = map.begin(); it != map.end(); ++it) {
    if (it->second >= 0) {
      ...
      if (usedIds[it->second]) {
        ...
        cout << "WARNING: found duplicate id " << it->second << " on: " << key
             << "." << it->first << " - previously used on:\n";
        {
  ...
  for (StringToInt::iterator it = map.begin(); it != map.end(); it++) {
    if (it->second < 0) {
      ...
      cout << "assigned new id to " << key << " . " << it->first << " = "
           << it->second << "\n";
    }
  }
...
// if entry contains string data, or a list of string data, then those values
// are added to the map
bool StringMapBuilder::PopulateMapWithSearchKey(
    StringToInt& map, const ptree& config, const string& searchForKey,
    bool allowMultipleIdOcurrences) {
  bool ok = true;
  ...
  for (auto& entry : config) {
    ...
    if (!entry.second.data().empty() && key == searchForKey) {
      ...
      if (!allowMultipleIdOcurrences &&
          map.find(entry.second.data()) != map.end()) {
    ...
    if (!entry.second.empty()) {
      ...
      for (auto& subEntry : entry.second) {
        ...
        if (subEntry.first.empty() && subEntry.second.empty() &&
          ...
          if (key == searchForKey) {
            // Verify duplicated ids
            if (!allowMultipleIdOcurrences &&
                map.find(subEntry.second.data()) != map.end()) {
              cout << "WARNING: found duplicate id " << entry.second.data()
                   << endl;
              ok = false;
            }
      ...
      const bool okSubTree = PopulateMapWithSearchKey(
          map, entry.second, searchForKey, allowMultipleIdOcurrences);
      ok = ok && okSubTree;
    }
  }
...
bool StringMapBuilder::PopulateMap(StringToInt& map, const ptree& config,
                                   const string& listKey, const string& idKey,
                                   bool allowMultipleIdOcurrences) {
  bool ok = true;

  // verify that keys exists, so that ptree does not throw exception
  boost::optional<const ptree&> optionalList =
      config.get_child_optional(listKey);
  if (optionalList) {
    BOOST_FOREACH (const ptree::value_type& v, *optionalList) {
      string id;
      if (idKey.empty()) {
        // if missing, do not create an entry for the default id
        boost::optional<string> optionalValue =
            v.second.get_value_optional<string>();
        if (!optionalValue) {
          continue;
        }
        ...
        // if missing, do not create an entry for the default id
        boost::optional<string> optionalKey =
            v.second.get_optional<string>(idKey);
        if (!optionalKey) {
          continue;
        }
...
bool StringMapBuilder::PopulateNestedMap(StringToInt& map, const ptree& config,
                                         const string& listKey,
                                         const string& nestedListKey,
                                         const string& idKey,
                                         bool allowMultipleIdOcurrences) {
  bool ok = true;
  ...
  if (optionalList) {
    BOOST_FOREACH (const ptree::value_type& v, *optionalList) {
      boost::optional<const ptree&> optionalNestedList =
          v.second.get_child_optional(nestedListKey);
      if (optionalNestedList) {
        BOOST_FOREACH (const ptree::value_type& nestedV, *optionalNestedList) {
          boost::optional<string> optionalid =
              nestedV.second.get_optional<string>(idKey);
          if (optionalid && !optionalid.get().empty()) {
            // Verify duplicated ids
            if (!allowMultipleIdOcurrences &&
                map.find(optionalid.get()) != map.end()) {
              cout << "WARNING: found duplicate id " << optionalid.get()
                   << endl;
              ok = false;
            }

            map[optionalid.get()] = -1;
            // cout << "parsed " << listKey << " . " << nestedListKey << " . "
            // << optionalid.get() << "\n";
          }
        }
      }
...
bool StringMapBuilder::PopulateMapFromDefFile(StringToInt& map,
  ...
  if (myfile.is_open()) {
    while (std::getline(myfile, line)) {
      ...
      if (fieldIndex < words.size()) {
        // Verify duplicated ids
        if (!allowMultipleIdOcurrences &&
            map.find(words[fieldIndex]) != map.end()) {
...
bool StringMapBuilder::ReadPtreeFromFile(const string& fileName,
                                         ptree& config) {
  ...
  if (PRINT_DEBUG_FLAG) cout << "reading config json from " << fileName << "\n";
...
bool StringMapBuilder::ParseListKeyNode(ptree& ptreeNode,
                                        ListAndKey& listKeyNode) {
  // get nested list key
  boost::optional<string> optionalSearchForKey =
      ptreeNode.get_optional<string>("search_for_key");
  ...
  // get list key
  boost::optional<string> optionalListKey =
      ptreeNode.get_optional<string>("list_key");
  if (!optionalListKey) {
  ...
  // get nested list key
  boost::optional<string> optionalNestedListKey =
      ptreeNode.get_optional<string>("nested_list_key");

  // get id key
  boost::optional<string> optionalIdKey =
      ptreeNode.get_optional<string>("id_key");
  if (!optionalIdKey) {
...
bool StringMapBuilder::Build(const string& configRoot, const string& workRoot,
                             const string& configJson, const bool testOnly) {
  // parse config json
  ...
  boost::optional<string> optionalTargetFile =
      internalConfig.get_optional<string>("target");
  ...
  if (!ReadPtreeFromFile(configRoot + optionalTargetFile.get(),
                         targetStringMap)) {
    cout << "original string map file could not be opened [" << configRoot
         << optionalTargetFile.get() << "], creating from scratch\n";
  }

  StringMapBuilder::MapChangeInfo mapChangeInfo;
  ptree generatedMap;

  boost::optional<ptree&> optionalList =
      internalConfig.get_child_optional("sources");
  if (optionalList) {
    // for each source
    BOOST_FOREACH (ptree::value_type& listItem, *optionalList) {
      // get file, or files
      vector<string> files;
      boost::optional<string> optionalFileName =
          listItem.second.get_optional<string>("file");
      if (optionalFileName) {
        files.push_back(optionalFileName.get());
      } else {
        boost::optional<ptree&> optionalFileList =
            listItem.second.get_child_optional("files");
        if (optionalFileList) {
      ...
      if (files.size() == 0) {
        cout << "source did not contain any files to parse, make sure you "
      ...
      boost::optional<ptree&> optionalListAndKeyList =
          listItem.second.get_child_optional("list_and_keys");
      if (optionalListAndKeyList) {
        BOOST_FOREACH (ptree::value_type& listKeyNode,
                       *optionalListAndKeyList) {
      ...
      // get target key
      boost::optional<string> optionalTargetKey =
          listItem.second.get_optional<string>("string_map_target_key");
      ...
      bool allowMultipleIdOcurrences =
          listItem.second.get("allow_multiple_id_ocurrences", false);

      if (PRINT_DEBUG_FLAG) {
        cout << "working on: \n";
        cout << "string_map_target_key [" << optionalTargetKey.get() << "]\n";
        cout << "remove_missing_keys ["
             << (removeMissingKeys ? "true" : "false") << "]\n";
      ...
      for (unsigned int i = 0; i < files.size(); ++i) {
        if (files[i].rfind(".def") != std::string::npos) {
          ...
          string delims = "()";
          sourcesPopulateOk =
              PopulateMapFromDefFile(stringsToCheck, configRoot + files[i],
                                     delims, 1, allowMultipleIdOcurrences) &&
              sourcesPopulateOk;
          continue;
        }
        ptree someConfig;
        if (!ReadPtreeFromFile(configRoot + files[i], someConfig)) {
          // if we could not read a file do not even generate the
          // stringMap.json, it could mess up keys because the ids actually
          // failed to load
          cout << "could not open file: [" << configRoot << files[i]
               << "]. Aborting stringMap generation...\n";
          return false;
        }
        ...
        for (unsigned int j = 0; j < listDefinitions.size(); ++j) {
          if (!listDefinitions[j].searchForKey.empty()) {
            sourcesPopulateOk =
                PopulateMapWithSearchKey(stringsToCheck, someConfig,
                                         listDefinitions[j].searchForKey,
                                         allowMultipleIdOcurrences) &&
                sourcesPopulateOk;
          } else if (!listDefinitions[j].nestedListKey.empty()) {
            sourcesPopulateOk =
                PopulateNestedMap(
                    stringsToCheck, someConfig, listDefinitions[j].listKey,
                    listDefinitions[j].nestedListKey, listDefinitions[j].idKey,
                    allowMultipleIdOcurrences) &&
                sourcesPopulateOk;
          } else {
            sourcesPopulateOk = PopulateMap(stringsToCheck, someConfig,
                                            listDefinitions[j].listKey,
                                            listDefinitions[j].idKey,
                                            allowMultipleIdOcurrences) &&
                                sourcesPopulateOk;
          }
        }
      ...
      if (!sourcesPopulateOk) {
        ...
        cout << "Maps for target '" << optionalTargetKey.get()
             << "' failed to populate!\n\n";
      } else {
        // compare data
        mapChangeInfo |= CompareMaps(stringsToCheck, optionalTargetKey.get(),
                                     targetStringMap, removeMissingKeys);

        // generate new config file
        AddMapForKey(stringsToCheck, optionalTargetKey.get(), generatedMap);

        if (PRINT_DEBUG_FLAG) cout << "\n\n";
      }
    }
  } else {
    if (PRINT_DEBUG_FLAG)
  ...
  if (!mapChangeInfo.HadAnyErrors()) {
    if (mapChangeInfo.MadeAnyChanges()) {
      if (testOnly) {
        cout << "test mode - not writing config json to " << configRoot
             << optionalTargetFile.get() << "\n";
      } else {
        string fileName = configRoot + optionalTargetFile.get();

        cout << "writing config json to " << fileName << "\n";
        try {
          boost::property_tree::json_parser::write_json(fileName, generatedMap,
                                                        std::locale(), true);
        }
        // catch c++ exceptions
        catch (exception& e) {
