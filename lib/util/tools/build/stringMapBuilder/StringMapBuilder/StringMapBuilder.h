...
class StringMapBuilder {
 public:
  static bool Build(const string& configRoot, const string& workRoot,
 ...
 private:
  ...
  static void AddMapForKey(const StringToInt& map, const string& key,
                           ptree& target);
  static MapChangeInfo CompareMaps(StringToInt& map, string key, ptree config,
                                   bool removeMissingKeys);
  static bool PopulateMap(StringToInt& map, const ptree& config,
                          const string& listKey, const string& idKey,
                          bool allowMultipleIdOcurrences);
  static bool PopulateMapWithSearchKey(StringToInt& map, const ptree& config,
                                       const string& searchForKey,
                                       bool allowMultipleIdOcurrences);
  static bool PopulateNestedMap(StringToInt& map, const ptree& config,
                                const string& listKey,
                                const string& nestedListKey,
                                const string& idKey,
                                bool allowMultipleIdOcurrences);
  static bool PopulateMapFromDefFile(StringToInt& map, const string& fileName,
