 ...
 * DUK_USE_PANIC_HANDLER, DUK_PANIC() calls a helper which prints out the error
 * and causes a process exit.
 *
...
#define DUK_ERROR_REQUIRE_TYPE_INDEX(thr, index, expectname, lowmemstr) \
  do {                                                                  \
    duk_err_require_type_index((thr), DUK_FILE_MACRO,                   \
                               (duk_int_t)DUK_LINE_MACRO, (index),      \
                               (expectname));                           \
  } while (0)
...
#define DUK_ERROR_UNIMPLEMENTED_DEFMSG(thr)                  \
  do {                                                       \
    duk_err_unimplemented_defmsg((thr), DUK_FILE_MACRO,      \
...
#define DUK_ERROR_UNSUPPORTED_DEFMSG(thr)                  \
  do {                                                     \
    duk_err_unsupported_defmsg((thr), DUK_FILE_MACRO,      \
                               (duk_int_t)DUK_LINE_MACRO); \
  } while (0)
#endif
...
#define DUK_ERROR_API_INDEX(thr, index)                                 \
  do {                                                                  \
    duk_err_api_index((thr), DUK_FILE_MACRO, (duk_int_t)DUK_LINE_MACRO, \
                      (index));                                         \
  } while (0)
...
DUK_INTERNAL_DECL void duk_err_augment_error_create(
    duk_hthread *thr, duk_hthread *thr_callstack, const char *filename,
    duk_int_t line, duk_bool_t noblame_fileline);
#endif
...
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_require_type_index(
    ...
    duk_idx_t index, const char *expect_name));
#else
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_require_type_index(
    duk_hthread *thr, const char *filename, duk_int_t linenumber,
    duk_idx_t index, const char *expect_name));
#endif
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_api_index(duk_hthread *thr,
                                                      const char *filename,
...
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_range(duk_hthread *thr,
                                                  ...
                                                  duk_int_t linenumber,
                                                  const char *message));
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_unimplemented_defmsg(
    duk_hthread *thr, const char *filename, duk_int_t linenumber));
#if !defined(DUK_USE_BYTECODE_DUMP_SUPPORT)
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_unsupported_defmsg(
    duk_hthread *thr, const char *filename, duk_int_t linenumber));
#endif
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_internal_defmsg(
    duk_hthread *thr, const char *filename, duk_int_t linenumber));
...
DUK_NORETURN(DUK_INTERNAL_DECL void duk_default_fatal_handler(
    duk_context *ctx, duk_errcode_t code, const char *msg));

#if !defined(DUK_USE_PANIC_HANDLER)
DUK_NORETURN(DUK_INTERNAL_DECL void duk_default_panic_handler(
