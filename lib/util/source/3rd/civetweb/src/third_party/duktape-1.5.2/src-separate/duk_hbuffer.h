...
#define DUK_HBUFFER_FLAG_DYNAMIC                                              \
  DUK_HEAPHDR_USER_FLAG(0) /* buffer is behind a pointer, dynamic or external \
                            */
#define DUK_HBUFFER_FLAG_EXTERNAL \
  DUK_HEAPHDR_USER_FLAG(          \
      1) /* buffer pointer is to an externally allocated buffer */
...
#define DUK_HBUFFER_SET_SIZE(x, v)                                          \
  do {                                                                      \
    ...
    (x)->hdr.h_flags =                                                      \
        ((x)->hdr.h_flags & 0x0000ffffUL) | (((duk_uint32_t)duk__v) << 16); \
  } while (0)
...
#define DUK_HBUFFER_FIXED_SET_SIZE(x, v) \
...
#define DUK_HBUFFER_DYNAMIC_ADD_SIZE(x, dv) \
  DUK_HBUFFER_ADD_SIZE((duk_hbuffer *)(x), (dv))
#define DUK_HBUFFER_DYNAMIC_SUB_SIZE(x, dv) \
...
#define DUK_HBUFFER_FIXED_GET_DATA_PTR(heap, x) \
  ((duk_uint8_t *)(((duk_hbuffer_fixed *)(x)) + 1))

#if defined(DUK_USE_HEAPPTR16)
#define DUK_HBUFFER_DYNAMIC_GET_DATA_PTR(heap, x)    \
  ((void *)DUK_USE_HEAPPTR_DEC16((heap)->heap_udata, \
...
DUK_INTERNAL_DECL duk_hbuffer *duk_hbuffer_alloc(duk_heap *heap,
                                                 ...
                                                 void **out_bufdata);
DUK_INTERNAL_DECL void *duk_hbuffer_get_dynalloc_ptr(
