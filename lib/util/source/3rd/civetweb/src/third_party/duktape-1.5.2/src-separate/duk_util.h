...
#define DUK_BW_PUSH_AS_STRING(thr, bw_ctx)                                 \
  do {                                                                     \
    duk_push_lstring((duk_context *)(thr), (const char *)(bw_ctx)->p_base, \
...
#define DUK_BW_WRITE_RAW_HSTRING(thr, bw_ctx, val)                       \
  do {                                                                   \
    ...
    DUK_MEMCPY((void *)((bw_ctx)->p),                                    \
               (const void *)DUK_HSTRING_GET_DATA((val)), duk__val_len); \
    (bw_ctx)->p += duk__val_len;                                         \
  } while (0)
