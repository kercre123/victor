...
#define DUK_IVAL_ARITH_EXTRAOP \
  3 /* binary arithmetic using extraops; DUK_EXTRAOP_INSTOF etc */
...
/* Compiling state of one function, eventually converted to
...
struct duk_compiler_func {
  ...
  duk_reg_t temp_max; /* highest value of temp_reg (temp_max - 1 is highest used
                         reg) */
  ...
  duk_reg_t reg_stmt_value; /* register for writing value of 'non-empty'
                               statements (global or eval code), -1 is marker */
  ...
  duk_bool_t is_arguments_shadowed; /* argument/function declaration shadows
                                       'arguments' */
...
#define DUK_JS_COMPILE_FLAG_FUNCEXPR                                          \
  (1 << 2) /* source is a function expression (used for Function constructor) \
            */
