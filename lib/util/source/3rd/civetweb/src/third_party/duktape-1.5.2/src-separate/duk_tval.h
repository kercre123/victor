...
#define DUK__TVAL_SET_TAGGEDPOINTER(v, h, tag)                              \
  do {                                                                      \
...
#define DUK__TVAL_SET_TAGGEDPOINTER(v, h, tag)                             \
  do {                                                                     \
    (v)->ull[DUK_DBL_IDX_ULL0] =                                           \
        (((duk_uint64_t)(tag)) << 48) | ((duk_uint64_t)(duk_uint32_t)(h)); \
  } while (0)
#endif
...
#if defined(DUK_USE_64BIT_OPS)
/* Double casting for pointer to avoid gcc warning (cast from pointer to integer
 * of different size) */
#if defined(DUK_USE_DOUBLE_ME)
#define DUK__TVAL_SET_LIGHTFUNC(v, fp, flags)                                \
  do {                                                                       \
...
#define DUK__TVAL_SET_LIGHTFUNC(v, fp, flags)                                \
  do {                                                                       \
    (v)->ui[DUK_DBL_IDX_UI0] =                                               \
        (((duk_uint32_t)DUK_TAG_LIGHTFUNC) << 16) | ((duk_uint32_t)(flags)); \
...
#if defined(DUK_USE_DOUBLE_ME)
#define DUK__TVAL_SET_FASTINT(v, i)                                          \
  do {                                                                       \
    (v)->ui[DUK_DBL_IDX_UI0] = ((duk_uint32_t)DUK_TAG_FASTINT) << 16 |       \
                               (((duk_uint32_t)((i) >> 32)) & 0x0000ffffUL); \
    (v)->ui[DUK_DBL_IDX_UI1] = (duk_uint32_t)(i);                            \
  } while (0)
#define DUK__TVAL_SET_FASTINT_U32(v, i)                               \
  do {                                                                \
    (v)->ui[DUK_DBL_IDX_UI0] = ((duk_uint32_t)DUK_TAG_FASTINT) << 16; \
    (v)->ui[DUK_DBL_IDX_UI1] = (duk_uint32_t)(i);                     \
  } while (0)
...
#define DUK__TVAL_SET_FASTINT(v, i)                    \
  do {                                                 \
    (v)->ull[DUK_DBL_IDX_ULL0] =                       \
        (((duk_uint64_t)DUK_TAG_FASTINT) << 48) |      \
        (((duk_uint64_t)(i)) & 0x0000ffffffffffffULL); \
  } while (0)
#define DUK__TVAL_SET_FASTINT_U32(v, i)                              \
  do {                                                               \
    (v)->ull[DUK_DBL_IDX_ULL0] =                                     \
        (((duk_uint64_t)DUK_TAG_FASTINT) << 48) | (duk_uint64_t)(i); \
  } while (0)
#endif

#define DUK__TVAL_SET_FASTINT_I32(v, i)      \
  do {                                       \
...
#if defined(DUK_USE_DOUBLE_ME)
#define DUK__TVAL_GET_FASTINT(v)                                     \
  (((duk_int64_t)((((duk_uint64_t)(v)->ui[DUK_DBL_IDX_UI0]) << 32) | \
                  ((duk_uint64_t)(v)->ui[DUK_DBL_IDX_UI1])))         \
       << 16 >>                                                      \
   16)
#else
#define DUK__TVAL_GET_FASTINT(v) \
  ((((duk_int64_t)(v)->ull[DUK_DBL_IDX_ULL0]) << 16) >> 16)
#endif
...
#define DUK_TVAL_SET_UNDEFINED(v)                               \
  do {                                                          \
    (v)->us[DUK_DBL_IDX_US0] = (duk_uint16_t)DUK_TAG_UNDEFINED; \
  } while (0)
#define DUK_TVAL_SET_UNUSED(v)                               \
  do {                                                       \
    (v)->us[DUK_DBL_IDX_US0] = (duk_uint16_t)DUK_TAG_UNUSED; \
  } while (0)
#define DUK_TVAL_SET_NULL(v)                               \
  do {                                                     \
    (v)->us[DUK_DBL_IDX_US0] = (duk_uint16_t)DUK_TAG_NULL; \
  } while (0)

#define DUK_TVAL_SET_BOOLEAN(v, val) \
  DUK_DBLUNION_SET_HIGH32(           \
      (v), (((duk_uint32_t)DUK_TAG_BOOLEAN) << 16) | ((duk_uint32_t)(val)))
...
#define DUK_TVAL_SET_FASTINT(v, i) \
  DUK_TVAL_SET_DOUBLE((v), (duk_double_t)(i)) /* XXX: fast int-to-double */
#define DUK_TVAL_SET_FASTINT_I32(v, i) \
  DUK_TVAL_SET_DOUBLE((v), (duk_double_t)(i))
#define DUK_TVAL_SET_FASTINT_U32(v, i) \
  DUK_TVAL_SET_DOUBLE((v), (duk_double_t)(i))
...
#define DUK_TVAL_SET_LIGHTFUNC(v, fp, flags) \
  DUK__TVAL_SET_LIGHTFUNC((v), (fp), (flags))
#define DUK_TVAL_SET_STRING(v, h) \
  DUK__TVAL_SET_TAGGEDPOINTER((v), (h), DUK_TAG_STRING)
#define DUK_TVAL_SET_OBJECT(v, h) \
  DUK__TVAL_SET_TAGGEDPOINTER((v), (h), DUK_TAG_OBJECT)
#define DUK_TVAL_SET_BUFFER(v, h) \
  DUK__TVAL_SET_TAGGEDPOINTER((v), (h), DUK_TAG_BUFFER)
#define DUK_TVAL_SET_POINTER(v, p) \
  DUK__TVAL_SET_TAGGEDPOINTER((v), (p), DUK_TAG_POINTER)
...
#define DUK_TVAL_GET_DOUBLE(v) ((v)->d)
#endif
#define DUK_TVAL_GET_LIGHTFUNC(v, out_fp, out_flags)     \
  do {                                                   \
    (out_flags) = (v)->ui[DUK_DBL_IDX_UI0] & 0xffffUL;   \
    (out_fp) = (duk_c_function)(v)->ui[DUK_DBL_IDX_UI1]; \
  } while (0)
#define DUK_TVAL_GET_LIGHTFUNC_FUNCPTR(v) \
  ((duk_c_function)((v)->ui[DUK_DBL_IDX_UI1]))
#define DUK_TVAL_GET_LIGHTFUNC_FLAGS(v) \
  (((int)(v)->ui[DUK_DBL_IDX_UI0]) & 0xffffUL)
...
#define DUK_TVAL_IS_BOOLEAN_TRUE(v) \
  ((v)->ui[DUK_DBL_IDX_UI0] == DUK_XTAG_BOOLEAN_TRUE)
#define DUK_TVAL_IS_BOOLEAN_FALSE(v) \
  ((v)->ui[DUK_DBL_IDX_UI0] == DUK_XTAG_BOOLEAN_FALSE)
...
#define DUK_TVAL_GET_NUMBER(tv)                                         \
  (DUK_TVAL_IS_FASTINT((tv)) ? (duk_double_t)DUK_TVAL_GET_FASTINT((tv)) \
                             : DUK_TVAL_GET_DOUBLE((tv)))
#define DUK_TVAL_GET_NUMBER(tv) duk_tval_get_number_unpacked((tv))
#else
/* This seems reasonable overall. */
#define DUK_TVAL_GET_NUMBER(tv)                                           \
  (DUK_TVAL_IS_FASTINT((tv)) ? duk_tval_get_number_unpacked_fastint((tv)) \
                             : DUK_TVAL_GET_DOUBLE((tv)))
#endif
#else
...
#define DUK_TVAL_IS_BOOLEAN_FALSE(tv) \
  (((tv)->t == DUK_TAG_BOOLEAN) && ((tv)->v.i == 0))
#if defined(DUK_USE_FASTINT)
...
#define DUK_TVAL_IS_NUMBER(tv) \
  ((tv)->t == DUK__TAG_NUMBER || (tv)->t == DUK_TAG_FASTINT)
#else
...
DUK_INTERNAL_DECL void duk_tval_set_number_chkfast(duk_tval *tv,
                                                   duk_double_t x);
#endif
