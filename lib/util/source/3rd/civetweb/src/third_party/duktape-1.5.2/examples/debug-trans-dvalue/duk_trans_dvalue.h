...
#define DUK_DVALUE_OBJECT \
  15 /* i: class number, buf: pointer data, len: pointer length */
#define DUK_DVALUE_POINTER 16 /* buf: pointer data, len: pointer length */
#define DUK_DVALUE_LIGHTFUNC \
  17 /* i: lightfunc flags, buf: pointer data, len: pointer length */
...
struct duk_trans_dvalue_ctx {
  ...
  duk_trans_buffer
      send_buf; /* sending towards Duktape (duktape read callback) */
  duk_trans_buffer
      recv_buf; /* receiving from Duktape (duktape write callback) */
  int handshake_done;
...
duk_size_t duk_trans_dvalue_read_cb(void *udata, char *buffer,
                                    duk_size_t length);
