...
#define DUK_CALL_FLAG_IGNORE_RECLIMIT                                  \
  (1 << 0) /* duk_handle_call_xxx: call ignores C recursion limit (for \
              errhandler calls) */
#define DUK_CALL_FLAG_CONSTRUCTOR_CALL                                   \
  (1 << 1) /* duk_handle_call_xxx: constructor call (i.e. called as 'new \
              Foo()') */
#define DUK_CALL_FLAG_IS_RESUME \
  (1 << 2) /* duk_handle_ecma_call_setup: setup for a resume() */
#define DUK_CALL_FLAG_IS_TAILCALL \
  (1 << 3) /* duk_handle_ecma_call_setup: setup for a tail call */
...
#define DUK_EQUALS_FLAG_SAMEVALUE \
  (1 << 0) /* use SameValue instead of non-strict equality */
#define DUK_EQUALS_FLAG_STRICT \
  (1 << 1) /* use strict equality instead of non-strict equality */
...
DUK_INTERNAL_DECL duk_small_int_t duk_js_to_arrayindex_raw_string(
    const duk_uint8_t *str, duk_uint32_t blen, duk_uarridx_t *out_idx);
DUK_INTERNAL_DECL duk_uarridx_t
duk_js_to_arrayindex_string_helper(duk_hstring *h);
...
DUK_INTERNAL_DECL duk_bool_t duk_js_compare_helper(duk_hthread *thr,
                                                   ...
                                                   duk_tval *tv_y,
                                                   duk_small_int_t flags);
DUK_INTERNAL_DECL duk_bool_t duk_js_instanceof(duk_hthread *thr, duk_tval *tv_x,
                                               duk_tval *tv_y);
...
DUK_INTERNAL_DECL duk_bool_t duk_js_getvar_activation(duk_hthread *thr,
                                                      ...
                                                      duk_hstring *name,
                                                      duk_bool_t throw_flag);
DUK_INTERNAL_DECL void duk_js_putvar_envrec(duk_hthread *thr, duk_hobject *env,
                                            duk_hstring *name, duk_tval *val,
                                            duk_bool_t strict);
...
DUK_INTERNAL_DECL duk_bool_t duk_js_delvar_activation(duk_hthread *thr,
                                                      duk_activation *act,
                                                      duk_hstring *name);
DUK_INTERNAL_DECL duk_bool_t duk_js_declvar_activation(
    duk_hthread *thr, duk_activation *act, duk_hstring *name, duk_tval *val,
    duk_small_int_t prop_flags, duk_bool_t is_func_decl);
DUK_INTERNAL_DECL void duk_js_init_activation_environment_records_delayed(
    duk_hthread *thr, duk_activation *act);
DUK_INTERNAL_DECL void duk_js_close_environment_record(duk_hthread *thr,
                                                       duk_hobject *env,
                                                       duk_hobject *func,
                                                       duk_size_t regbase);
DUK_INTERNAL_DECL duk_hobject *duk_create_activation_environment_record(
    duk_hthread *thr, duk_hobject *func, duk_size_t idx_bottom);
DUK_INTERNAL_DECL
void duk_js_push_closure(duk_hthread *thr, duk_hcompiledfunction *fun_temp,
                         duk_hobject *outer_var_env, duk_hobject *outer_lex_env,
                         duk_bool_t add_auto_proto);

/* call handling */
DUK_INTERNAL_DECL duk_int_t duk_handle_call_protected(
    duk_hthread *thr, duk_idx_t num_stack_args, duk_small_uint_t call_flags);
...
DUK_INTERNAL_DECL duk_bool_t duk_handle_ecma_call_setup(
    duk_hthread *thr, duk_idx_t num_stack_args, duk_small_uint_t call_flags);
