...
#define DUK_VALSTACK_INITIAL_SIZE                                      \
  128 /* roughly 1.0 kiB -> but rounds up to DUK_VALSTACK_GROW_STEP in \
         practice */
...
#define DUK_ACT_FLAG_TAILCALLED \
  (1 << 1) /* activation has tail called one or more times */
#define DUK_ACT_FLAG_CONSTRUCT \
  (1 << 2) /* function executes as a constructor (called via "new") */
#define DUK_ACT_FLAG_PREVENT_YIELD \
  (1 << 3) /* activation prevents yield (native call or "new") */
#define DUK_ACT_FLAG_DIRECT_EVAL (1 << 4) /* activation is a direct eval call \
                                           */
#define DUK_ACT_FLAG_BREAKPOINT_ACTIVE \
  (1 << 5) /* activation has active breakpoint(s) */
...
#define DUK_TB_FLAG_NOBLAME_FILELINE \
  (1 << 0) /* don't report __FILE__ / __LINE__ as fileName/lineNumber */
...
#define DUK_CAT_FLAG_CATCH_BINDING_ENABLED \
  (1 << 6) /* request to create catch binding */
#define DUK_CAT_FLAG_LEXENV_ACTIVE \
  (1 << 7) /* catch or with binding is currently active */
...
#define DUK_HTHREAD_STATE_RUNNING \
  2 /* thread currently running (only one at a time) */
#define DUK_HTHREAD_STATE_RESUMED \
  3 /* thread resumed another thread (active but not running) */
...
/* XXX: for a memory-code tradeoff, remove 'func' and make it's access either a
 * function or a macro.  This would make the activation 32 bytes long on 32-bit
 * platforms again.
 */

/* Note: it's nice if size is 2^N (at least for 32-bit platforms). */
struct duk_activation {
  duk_tval tv_func; /* borrowed: full duk_tval for function being executed; for
                       lightfuncs */
  duk_hobject
      *func; /* borrowed: function being executed; for bound function calls,
                this is the final, real function, NULL for lightfuncs */
  duk_hobject
      *var_env; /* current variable environment (may be NULL if delayed) */
  duk_hobject
      *lex_env; /* current lexical environment (may be NULL if delayed) */
...
struct duk_catcher {
  duk_hstring *h_varname; /* borrowed reference to catch variable name (or NULL
                             if none) */
  ...
  duk_size_t
      idx_base; /* idx_base and idx_base+1 get completion value and type */
...
struct duk_hthread {
...
#if !defined(DUK_USE_PREFER_SIZE)
  duk_size_t valstack_size; /* cached: valstack_end - valstack (in entries, not
                               bytes) */
#endif
  ...
  duk_size_t callstack_preventcount; /* number of activation records in
                                        callstack preventing a yield */
...
DUK_INTERNAL_DECL duk_activation *duk_hthread_get_current_activation(
    duk_hthread *thr);
DUK_INTERNAL_DECL void *duk_hthread_get_valstack_ptr(
    duk_heap *heap, void *ud); /* indirect allocs */
DUK_INTERNAL_DECL void *duk_hthread_get_callstack_ptr(
    duk_heap *heap, void *ud); /* indirect allocs */
DUK_INTERNAL_DECL void *duk_hthread_get_catchstack_ptr(
    duk_heap *heap, void *ud); /* indirect allocs */

#if defined(DUK_USE_DEBUGGER_SUPPORT)
DUK_INTERNAL_DECL duk_uint_fast32_t
duk_hthread_get_act_curr_pc(duk_hthread *thr, duk_activation *act);
#endif
DUK_INTERNAL_DECL duk_uint_fast32_t
duk_hthread_get_act_prev_pc(duk_hthread *thr, duk_activation *act);
