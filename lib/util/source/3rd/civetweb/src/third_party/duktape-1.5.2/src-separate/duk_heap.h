...
#define DUK_HEAP_FLAG_MARKANDSWEEP_RUNNING \
  (1 << 0) /* mark-and-sweep is currently running */
#define DUK_HEAP_FLAG_MARKANDSWEEP_RECLIMIT_REACHED                         \
  (1 << 1) /* mark-and-sweep marking reached a recursion limit and must use \
              multi-pass marking */
#define DUK_HEAP_FLAG_REFZERO_FREE_RUNNING \
  (1 << 2) /* refcount code is processing refzero list */
#define DUK_HEAP_FLAG_ERRHANDLER_RUNNING                                   \
  (1 << 3) /* an error handler (user callback to augment/replace error) is \
              running */
#define DUK_HEAP_FLAG_INTERRUPT_RUNNING \
  (1 << 4) /* executor interrupt running (used to avoid nested interrupts) */
#define DUK_HEAP_FLAG_FINALIZER_NORESCUE \
  (1 << 5) /* heap destruction ongoing, finalizer rescue no longer possible */
...
#define DUK_LJ_TYPE_CONTINUE                                                   \
  5 /* value1 -> label number, pseudo-type to indicate a continue continuation \
       (for ENDFIN) */
...
#define DUK_MS_FLAG_NO_STRINGTABLE_RESIZE                                \
  (1 << 1) /* don't resize stringtable (but may sweep it); needed during \
              stringtable resize */
#define DUK_MS_FLAG_NO_OBJECT_COMPACTION                                      \
  (1 << 2) /* don't compact objects; needed during object property allocation \
              resize */
#define DUK_MS_FLAG_NO_FINALIZERS                                              \
  (1 << 3) /* don't run finalizers; leave finalizable objects in finalize_list \
              for next round */
#define DUK_MS_FLAG_SKIP_FINALIZERS                                   \
  (1 << 4) /* don't run finalizers; queue finalizable objects back to \
              heap_allocated */
...
#define DUK_HEAP_CLEAR_STEP_STATE(heap)         \
  do {                                          \
...
#define DUK_HEAP_SET_PAUSED(heap)      \
  do {                                 \
    (heap)->dbg_paused = 1;            \
    (heap)->dbg_state_dirty = 1;       \
    DUK_HEAP_CLEAR_STEP_STATE((heap)); \
  } while (0)
#define DUK_HEAP_CLEAR_PAUSED(heap)    \
  do {                                 \
    (heap)->dbg_paused = 0;            \
    (heap)->dbg_state_dirty = 1;       \
    DUK_HEAP_CLEAR_STEP_STATE((heap)); \
...
struct duk_heap {
  ...
  duk_bool_t dbg_paused;     /* currently paused: talk with debug client until
                                step/resume */
  duk_bool_t
  ...
  duk_bool_t dbg_detaching; /* debugger detaching; used to avoid calling detach
                               handler recursively */
  duk_small_uint_t
      dbg_step_type; /* step type: none, step into, step over, step out */
  duk_hthread
      *dbg_step_thread; /* borrowed; NULL if no step state (NULLed in unwind) */
  ...
  duk_uint32_t dbg_exec_counter; /* cumulative opcode execution count (overflows
                                    are OK) */
  duk_uint32_t dbg_last_counter; /* value of dbg_exec_counter when we last did a
                                    Date-based check */
...
DUK_INTERNAL_DECL void duk_heap_remove_any_from_heap_allocated(
    duk_heap *heap, duk_heaphdr *hdr);
#endif
#if defined(DUK_USE_INTERRUPT_COUNTER)
...
DUK_INTERNAL_DECL duk_hstring *duk_heap_string_intern(duk_heap *heap,
                                                      ...
                                                      duk_uint32_t blen);
DUK_INTERNAL_DECL duk_hstring *duk_heap_string_intern_checked(
...
DUK_INTERNAL_DECL duk_hstring *duk_heap_string_intern_u32(duk_heap *heap,
                                                          duk_uint32_t val);
DUK_INTERNAL_DECL duk_hstring *duk_heap_string_intern_u32_checked(
