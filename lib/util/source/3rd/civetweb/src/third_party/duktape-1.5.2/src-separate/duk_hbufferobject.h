...
#define DUK_ASSERT_HBUFFEROBJECT_VALID(h)                                      \
  do {                                                                         \
    ...
    DUK_ASSERT(                                                                \
        ((h)->shift == 0 && (h)->elem_type == DUK_HBUFFEROBJECT_ELEM_UINT8) || \
        ((h)->shift == 0 &&                                                    \
         (h)->elem_type == DUK_HBUFFEROBJECT_ELEM_UINT8CLAMPED) ||             \
        ((h)->shift == 0 && (h)->elem_type == DUK_HBUFFEROBJECT_ELEM_INT8) ||  \
        ((h)->shift == 1 &&                                                    \
         (h)->elem_type == DUK_HBUFFEROBJECT_ELEM_UINT16) ||                   \
        ((h)->shift == 1 && (h)->elem_type == DUK_HBUFFEROBJECT_ELEM_INT16) || \
        ((h)->shift == 2 &&                                                    \
         (h)->elem_type == DUK_HBUFFEROBJECT_ELEM_UINT32) ||                   \
        ((h)->shift == 2 && (h)->elem_type == DUK_HBUFFEROBJECT_ELEM_INT32) || \
        ((h)->shift == 2 &&                                                    \
         (h)->elem_type == DUK_HBUFFEROBJECT_ELEM_FLOAT32) ||                  \
        ((h)->shift == 3 &&                                                    \
         (h)->elem_type == DUK_HBUFFEROBJECT_ELEM_FLOAT64));                   \
...
#define DUK_HBUFFEROBJECT_CLAMP_BYTELENGTH(h, len) \
  (DUK_ASSERT_EXPR((h) != NULL), duk_hbufferobject_clamp_bytelength((h), (len)))
...
#endif
DUK_INTERNAL_DECL void duk_hbufferobject_push_validated_read(
    duk_context *ctx, duk_hbufferobject *h_bufobj, duk_uint8_t *p,
    duk_small_uint_t elem_size);
DUK_INTERNAL_DECL void duk_hbufferobject_validated_write(
    duk_context *ctx, duk_hbufferobject *h_bufobj, duk_uint8_t *p,
    duk_small_uint_t elem_size);
