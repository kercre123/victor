...
#define DUK__DEBUG_LOG(lev, ...)                        \
  duk_debug_log((duk_small_int_t)(lev), DUK_FILE_MACRO, \
                (duk_int_t)DUK_LINE_MACRO, DUK_FUNC_MACRO, __VA_ARGS__);
...
#define DUK_DPRINT                   \
  DUK__DEBUG_STASH(DUK_LEVEL_DEBUG), \
      (void)duk_debug_log /* args go here in parens */
#else
...
#ifdef DUK_USE_DDPRINT
#define DUK_DDPRINT                   \
  DUK__DEBUG_STASH(DUK_LEVEL_DDEBUG), \
      (void)duk_debug_log /* args go here in parens */
#else
...
#ifdef DUK_USE_DDDPRINT
#define DUK_DDDPRINT                   \
  DUK__DEBUG_STASH(DUK_LEVEL_DDDEBUG), \
      (void)duk_debug_log /* args go here in parens */
#else
...
#endif
DUK_INTERNAL_DECL void duk_debug_format_funcptr(char *buf, duk_size_t buf_size,
                                                duk_uint8_t *fptr,
                                                duk_size_t fptr_size);

#ifdef DUK_USE_VARIADIC_MACROS
DUK_INTERNAL_DECL void duk_debug_log(duk_small_int_t level, const char *file,
                                     duk_int_t line, const char *func,
                                     const char *fmt, ...);
#else /* DUK_USE_VARIADIC_MACROS */
...
DUK_INTERNAL_DECL void duk_fb_put_bytes(duk_fixedbuffer *fb,
                                        const duk_uint8_t *buffer,
                                        duk_size_t length);
...
DUK_INTERNAL_DECL void duk_fb_sprintf(duk_fixedbuffer *fb, const char *fmt,
                                      ...);
DUK_INTERNAL_DECL void duk_fb_put_funcptr(duk_fixedbuffer *fb,
                                          duk_uint8_t *fptr,
                                          duk_size_t fptr_size);
