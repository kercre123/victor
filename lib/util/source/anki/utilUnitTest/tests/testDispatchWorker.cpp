...
TEST(DispatchWorker, ProcessNoArgs) {
  ...
  // Zero extra worker threads, no jobs
  {
    ...
    MyDispatchWorker::FunctionType workerFunction = [&myInt]() {
      std::atomic_fetch_add(&myInt, 1);
    };
...
TEST(DispatchWorker, ProcessWithArgs) {
  ...
  // Many extra worker threads, many jobs with args
  {
    ...
    MyDispatchWorker::FunctionType workerFunction = [&myInt](int int1, int int2,
                                                             int int3) {
  ...
  // Some extra worker threads, const vector reference argument
  {
    ...
    MyDispatchWorker::FunctionType workerFunction =
        [&myInt](const std::vector<int>& myVector) {
  ...
  {
#ifndef LINUX  // std::is_copy_constructible<std::tuple<std::unique_ptr<...>>>
               // is a build failure
    std::atomic_int myInt(0);
    using MyDispatchWorker =
        DispatchWorker<10, std::unique_ptr<std::vector<int>>>;
    MyDispatchWorker::FunctionType workerFunction =
        [&myInt](std::unique_ptr<std::vector<int>> myUnique) {
