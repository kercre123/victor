...
NoEq operator==(const T&, const Arg&);

struct NoPlus {};
template <typename T, typename Arg>
NoPlus operator+(const T&, const Arg&);
...
struct PlusExists {
  enum {
    value = !std::is_same<decltype(*(T*)(0) + *(Arg*)(0)), NoPlus>::value
  };
};
}  // namespace
...
TEST(TestStronglyTyped, CheckStronglyTyped) {
  ...
  static_assert(!std::is_convertible<TypeA, TypeB>::value,
                "Should not be able to convert between the two");
  static_assert(!std::is_convertible<TypeB, TypeA>::value,
                "Should not be able to convert between the two");
  static_assert(std::is_convertible<Underlying, TypeA>::value,
                "Should be able to convert into strongly typed");
  static_assert(std::is_convertible<Underlying, TypeB>::value,
                "Should be able to convert into strongly typed");
  static_assert(!std::is_convertible<TypeA, Underlying>::value,
                "Should be not able to convert out of strongly typed");
  static_assert(!std::is_convertible<TypeB, Underlying>::value,
                "Should be not able to convert out of strongly typed");
