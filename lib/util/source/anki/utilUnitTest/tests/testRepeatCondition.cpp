...
TEST_F(RepeatConditionTest, NoRepeatTest) {
  ...
  CombinationCondition* combo =
      new CombinationCondition(CombinationConditionOperationNot, condition);

  std::vector<std::string> eventNames;
  eventNames.push_back("game.event");
  NoticeAction* action = new NoticeAction(false, 0, "", "results.notice.title",
                                          "results.notice.description", "", "");
  QuestRule* rule = new QuestRule(
      "test.repeat.none", eventNames, "test.repeat.none.title",
      "test.repeat.none.description", "", "", "", nullptr, combo, action);
...
TEST_F(RepeatConditionTest, RepeatTest) {
  ...
  NoticeAction* action = new NoticeAction(false, 0, "", "results.notice.title",
                                          "results.notice.description", "", "");
  QuestRule* rule = new QuestRule(
      "test.repeat.one", eventNames, "test.repeat.one.title",
      "test.repeat.one.description", "", "", "", nullptr, combo, action);
  rule->SetRepeatable(true);
  questEngine.AddRule(rule);
