...
TEST_F(StringUtilsTest, UTF8BadByteStrings) {
  // Values above 0x7f that do not signify the start of a multibyte sequence are
  // invalid
...
TEST_F(StringUtilsTest, UTF8Surrogates) {
  // 0x1f601 should be encoded as F0 9F 98 81 in UTF-8, however some conversions
  // from UTF-16 will convert the UTF-16 surrogate pair (D83D DE01) to UTF-8
  // instead We will accept these, even though RFC 3629 declares them invalid
