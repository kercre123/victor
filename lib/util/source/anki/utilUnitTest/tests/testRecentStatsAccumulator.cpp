...
TEST(RecentStatsAccumulator, Empty) {
  // This is a clone of the StatsAccumulator version, to verify this behaves the
  // same way
...
TEST(RecentStatsAccumulator, MultipleResets) {
  ...
  // (b) always give data from >0 samples, and generally >= N/2 samples (as long
  // as >= N/2 samples have been added)
  ...
  // next add will flush 1 tracker and move to the other (which should have ~1/2
  // as many values in it)
  sa += 0.0;
