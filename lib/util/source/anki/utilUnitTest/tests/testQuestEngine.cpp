...
TEST_F(QuestEngineTest, SaveAndLoad) {
  ...
  CountThresholdCondition* limitThree =
      new CountThresholdCondition(CountThresholdOperatorEquals, 2, "game.win");
  std::vector<std::string> eventNames;
  eventNames.push_back("game.win");
  NoticeAction* triggeredAction =
      new NoticeAction(false, 0, "", "results.notice.title",
                       "results.notice.description", "", "");
  QuestRule* rule =
      new QuestRule("test.save.load", eventNames, "test.save.load.title",
                    "test.save.load.description", "", "", "", nullptr,
                    limitThree, triggeredAction);
...
TEST_F(QuestEngineTest, Clear) {
  ...
  NoticeAction* triggeredAction =
      new NoticeAction(false, 0, "", "results.notice.title",
                       "results.notice.description", "", "");
  CountThresholdCondition* limitThree =
      new CountThresholdCondition(CountThresholdOperatorEquals, 3, triggerKey);
  QuestRule* rule = new QuestRule("test.clear", eventNames, "test.clear.title",
                                  "test.clear.description", "", "", "", nullptr,
                                  limitThree, triggeredAction);
  questEngine->AddRule(rule);
...
TEST_F(QuestEngineTest, CountThresholdRule) {
  ...
  CountThresholdCondition* limitThree =
      new CountThresholdCondition(CountThresholdOperatorEquals, 3, triggerKey);
  QuestRule* rule = new QuestRule("test.count", eventNames, "test.count.title",
                                  "test.count.description", "", "", "", nullptr,
                                  limitThree, triggeredAction);
  questEngine->AddRule(rule);
...
TEST_F(QuestEngineTest, CombinationRule) {
  ...
  CountThresholdCondition* winCondition = new CountThresholdCondition(
      CountThresholdOperatorGreaterThanEqual, 1, winKey);

  CountThresholdCondition* starCondition =
      new CountThresholdCondition(CountThresholdOperatorEquals, 1, starKey);

  CombinationCondition* oneWinThreeStars = new CombinationCondition(
      CombinationConditionOperationAnd, winCondition, starCondition);

  NoticeAction* triggeredAction =
      new NoticeAction(false, 0, "", "results.notice.title",
                       "results.notice.description", "", "");
  QuestRule* rule = new QuestRule(
      "test.combination.rule", eventNames, "test.combination.rule.title",
      "test.combination.rule.description", "", "", "", nullptr,
      oneWinThreeStars, triggeredAction);
  questEngine->AddRule(rule);
...
TEST_F(QuestEngineTest, DuplicateEvents) {
  ...
  CountThresholdCondition* gameStart =
      new CountThresholdCondition(CountThresholdOperatorEquals, 1, startKey);
  ...
  QuestRule* resetStartOnStart = new QuestRule(
      "test.duplicate.one", startEvents, "test.duplicate.one.title",
      "test.duplicate.one.description", "", "", "", nullptr, gameStart,
      startAction);
  questEngine->AddRule(resetStartOnStart);

  std::vector<std::string> winEvents;
  winEvents.push_back(winKey);
  CountThresholdCondition* oneWin =
      new CountThresholdCondition(CountThresholdOperatorEquals, 1, winKey);
  NoticeAction* triggeredAction =
      new NoticeAction(false, 0, "", "results.notice.title",
                       "results.notice.description", "", "");
  QuestRule* rule =
      new QuestRule("test.duplicate.two", winEvents, "test.duplicate.two.title",
                    "test.duplicate.two.description", "", "", "", nullptr,
                    oneWin, triggeredAction);
  questEngine->AddRule(rule);
