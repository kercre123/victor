...
TEST_F(CombinationConditionTest, AndTest) {
  ...
  CountThresholdCondition* countOne = new CountThresholdCondition(
      CountThresholdOperatorEquals, 1, "game.event");
  DateCondition* dateTuesday = new DateCondition();
  dateTuesday->SetDayOfWeek(2);

  CombinationCondition* combo = new CombinationCondition(
      CombinationConditionOperationAnd, countOne, dateTuesday);
  ...
  QuestRule* rule = new QuestRule(
      "test.combination.and", eventNames, "test.combination.and.title",
      "test.combination.and.description", "", "", "", nullptr, combo, action);
...
TEST_F(CombinationConditionTest, OrTest) {
  ...
  CombinationCondition* combo = new CombinationCondition(
      CombinationConditionOperationOr, countOne, dateTuesday);
  ...
  NoticeAction* action = new NoticeAction(false, 0, "", "results.notice.title",
                                          "results.notice.description", "", "");
  QuestRule* rule = new QuestRule(
      "test.combination.or", eventNames, "test.combination.or.title",
      "test.combination.or.description", "", "", "", nullptr, combo, action);
  questEngine->AddRule(rule);
