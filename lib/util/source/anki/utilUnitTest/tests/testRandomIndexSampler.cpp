...
bool EachElementAppearsExactlyOnce(const std::vector<int>& elements,
                                   int& badElem) {
...
bool EachElementHasSameProbability(
    const std::vector<std::vector<int>>& elementsList, float tolOverall,
    float tolPositional) {
  ...
  for (const auto& totalPair : totalCount) {
    varianceAll += (totalPair.second - avgTotalCount) *
                   (totalPair.second - avgTotalCount) / numElements;
  }
  ...
  if (requireInAnyPosition) {
    ...
    for (const auto& perPositionPair : countPerPosition) {
      ...
      auto& avg = avgPositionCount[perPositionPair.first.first];
      var += (perPositionPair.second - avg) * (perPositionPair.second - avg) /
             numElements;
    }
...
struct TestInfo {
  enum GetType { GET_NEXT, GET_ALL };
  GetType getType;
  RandomIndexSampler::OrderType orderType;
  bool isStaticShuffler = false;
};
// separate so the actual test can EXPECT_*
std::string RunTest(RandomGenerator& rng, TestInfo info, int populationSize,
                    int numSamples, int numVectors, float tolOverall,
                    float tolPositional, bool shouldMatchTol = true) {
  ...
  for (auto& vec : samples) {
    ...
    EXPECT_EQ((int)vec.size(), numSamples)
        << "Sampled vector has incorrect size";

    int badElem = 0;
    const bool onlyOnce = EachElementAppearsExactlyOnce(vec, badElem);
    if (!onlyOnce) {
      return std::string("element ") + std::to_string(badElem) +
             " didnt appear exactly once";
    }
  }
  ...
  if (tolOverall >= 0.0f || tolPositional >= 0.0f) {
    const bool sameP =
        EachElementHasSameProbability(samples, tolOverall, tolPositional);
...
TEST(RandomIndexSampler, RandomOrderGetAll) {
  ...
  EXPECT_TRUE(result3.empty() && "choosing 5 out of 10 should exceed variance ")
      << result3;
  std::string result4 = RunTest(rng, info, 10, 5, 5000, 0.05f, 0.1f);
  EXPECT_TRUE(result4.empty() &&
              "choosing 5 out of 10 with more samples should show convergence ")
      << result4;
}
...
TEST(RandomIndexSampler, RandomOrderStaticShuffler) {
  ...
  EXPECT_TRUE(result1.empty() && "choosing 10 out of 10 static shuffler")
      << result1;
  // tighten tol and it should exceed it
  std::string result2 = RunTest(rng, info, 10, 10, 1000, 0.0f, 0.1f, false);
  EXPECT_TRUE(result2.empty() &&
              "choosing 10 out of 10 static shuffler should exceed")
      << result2;
  // converges
  std::string result3 = RunTest(rng, info, 10, 10, 5000, 0.0f, 0.1f);
  EXPECT_TRUE(result3.empty() &&
              "choosing 10 out of 10 static shuffler should show convergence")
      << result3;
}
...
TEST(RandomIndexSampler, FloydGetAll) {
  ...
  // choosing 10 out of 10 (order will NOT be random, so just check that all
  // elements appear once)
  std::string result1 = RunTest(rng, info, 10, 10, 5000, dontCheck, dontCheck);
  EXPECT_TRUE(result1.empty() && "choosing 10 out of 10") << result1;

  // only choosing 5 out of 10, elements should appear randomly, but their
  // positions in the sampled vector wont be
  std::string result2 = RunTest(rng, info, 10, 9, 1000, 0.05f, dontCheck);
  EXPECT_TRUE(result2.empty() && "choosing 5 out of 10") << result2;
  std::string result3 =
      RunTest(rng, info, 10, 5, 1000, 0.01f, dontCheck, false);
  EXPECT_TRUE(result3.empty() && "choosing 5 out of 10 should exceed variance")
      << result3;
  std::string result4 = RunTest(rng, info, 10, 5, 8000, 0.01f, dontCheck);
  EXPECT_TRUE(result4.empty() && "choosing 5 out of 10 show convergence")
      << result4;
}
