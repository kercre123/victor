...
AnkiLab AnkiLabTest::CreateAnkiLabAB50() const {
  const std::string experimentsJsonContents =
      CreateExperimentAB50DataJsonContents();
...
TEST_F(AnkiLabTest, ExperimentHashBucket) {
  uint8_t bucket = CalculateExperimentHashBucket("report_test", "user_id_0");
  EXPECT_GE(bucket, 0);
...
TEST_F(AnkiLabTest, TestaudienceTagIntersections) {
  ...
  EXPECT_FALSE(
      AudienceListIsSubsetOfList(experimentKeys, mismatchedaudienceTags));

  // Experiment ids empty
  EXPECT_TRUE(
      AudienceListIsSubsetOfList(std::vector<std::string>{}, audienceTags));

  // audience ids empty
  EXPECT_FALSE(
      AudienceListIsSubsetOfList(experimentKeys, std::vector<std::string>{}));
}
...
TEST_F(AnkiLabTest, GetExperimentVariation) {
...

  {
    const ExperimentVariation* variation =
        GetExperimentVariation(experiment, 0);
  ...
  {
    const ExperimentVariation* variation =
        GetExperimentVariation(experiment, 49);
    EXPECT_EQ(nullptr, variation);
  }

  {
    const ExperimentVariation* variation =
        GetExperimentVariation(experiment, 99);
    EXPECT_EQ(nullptr, variation);
  }

  {
    const ExperimentVariation* variation =
        GetExperimentVariation(experiment, 50);
    EXPECT_NE(nullptr, variation);
...
TEST_F(AnkiLabTest, ActivateExperiment) {
...

  {
    ...
    AssignmentStatus status = lab.ActivateExperiment(
        "report_test", "user_id_8", std::vector<std::string>(0), variationKey);
...

  {
    std::string variationKey;
    AssignmentStatus status = lab.ActivateExperiment(
        "report_test", "user_id_0", std::vector<std::string>(0), variationKey);
    EXPECT_EQ(AssignmentStatus::Assigned, status);
...

  {
    ...
    AssignmentStatus status =
        lab.ActivateExperiment("report_test", "user_id_000000",
                               std::vector<std::string>(0), variationKey);
    EXPECT_EQ(AssignmentStatus::Unassigned, status);
  }
}

TEST_F(AnkiLabTest, ActivateExperimentAllControl) {
  // Ensure that an experiment with 0% treatment, 100% control is assigned
  // correctly
  AnkiLab lab = CreateAnkiLabAB50();
...

  {
    std::string variationKey;
    AssignmentStatus status =
        lab.ActivateExperiment("report_test_all_control", "user_id_8",
                               std::vector<std::string>(0), variationKey);
    EXPECT_EQ(AssignmentStatus::Assigned, status);
    EXPECT_EQ("control", variationKey);
  }
...
TEST_F(AnkiLabTest, ActivateExperimentAllTreatment) {
  // Ensure that an experiment with 0% control, 100% treatment is assigned
  // correctly
  AnkiLab lab = CreateAnkiLabAB50();

  const Experiment* experiment =
      lab.FindExperiment("report_test_all_treatment");
  EXPECT_NE(nullptr, experiment);
  {
    const ExperimentVariation* variation =
        GetExperimentVariation(experiment, 0);
...

  {
    std::string variationKey;
    AssignmentStatus status = lab.ActivateExperiment(
        "report_test_all_treatment", "user_id_8", {}, variationKey);
    EXPECT_EQ(AssignmentStatus::Assigned, status);
    EXPECT_EQ("treatment", variationKey);
  }
}
...
TEST_F(AnkiLabTest, AutoActivateExperiments) {
  ...
  size_t attemptCount =
      lab.AutoActivateExperimentsForUser("user_id_8", {"first_time_user"});
...
TEST_F(AnkiLabTest, ActivateExperimentWithAudienceFiltering) {
  {
    ...
    AssignmentStatus status = lab.ActivateExperiment(
        "report_test_first_time_user", "user_id_0", audienceTags, variationKey);
    EXPECT_EQ(AssignmentStatus::Assigned, status);
    EXPECT_EQ("treatment", variationKey);
  }

  {
    ...
    std::string variationKey;
    AssignmentStatus status =
        lab.ActivateExperiment("report_test_first_time_user", "user_id_0",
                               std::vector<std::string>(0), variationKey);
...

  {
    ...
    std::vector<std::string> audienceTags = {"country_de"};
    AssignmentStatus status = lab.ActivateExperiment(
        "report_test", "user_id_8", audienceTags, variationKey);
    EXPECT_EQ(AssignmentStatus::Assigned, status);
    EXPECT_EQ("treatment", variationKey);
  }
}
...
TEST_F(AnkiLabTest, ActivateExperimentIsRunning) {
  ...
  {
    uint32_t epochSec =
        Util::EpochSecFromIso8601UTCDateString("2017-04-01T00:00:00Z");

    std::string variationKey;
    AssignmentStatus status;
    status = lab.ActivateExperiment("report_test_times", "user_id_8",
                                    std::vector<std::string>(0), epochSec,
                                    variationKey);
    EXPECT_EQ(AssignmentStatus::Assigned, status);
    EXPECT_EQ("treatment", variationKey);
  }
...
TEST_F(AnkiLabTest, ActivateExperimentBeforeStarted) {
  ...
  {
    uint32_t epochSec =
        Util::EpochSecFromIso8601UTCDateString("2017-03-01T00:00:00Z");
...
TEST_F(AnkiLabTest, ActivateExperimentAfterStopped) {
  ...
  {
    uint32_t epochSec =
        Util::EpochSecFromIso8601UTCDateString("2017-05-01T00:00:00Z");
...
TEST_F(AnkiLabTest, ActivateExperimentWhilePaused) {
  ...
  {
    uint32_t epochSec =
        Util::EpochSecFromIso8601UTCDateString("2017-04-11T00:00:00Z");
...
TEST_F(AnkiLabTest, ActivateExperimentAfterResume) {
  ...
  {
    uint32_t epochSec =
        Util::EpochSecFromIso8601UTCDateString("2017-04-21T00:00:00Z");
...
TEST_F(AnkiLabTest, ForceActivateExperiment) {
  ...
  std::string variationKey;
  AssignmentStatus status;
  status = lab.ForceActivateExperiment("report_test", "user_id_8", "control");
...
TEST_F(AnkiLabTest, ForceActivateExperimentFailsForInvalidVariant) {
  ...
  std::string variationKey;
  AssignmentStatus status;
  status = lab.ForceActivateExperiment("report_test", "user_id_8",
                                       "not_a_valid_variant");
...
TEST_F(AnkiLabTest, ActivateExperimentOverrideAssignment) {
  ...
  bool willOverride =
      lab.OverrideExperimentVariation("report_test", "user_id_8", "control");
  ...
  {
    status = lab.ActivateExperiment("report_test", "user_id_8",
                                    std::vector<std::string>(0), variationKey);
...
TEST_F(AnkiLabTest, ActivateExperimentOverrideAssignmentRequiresValidCriteria) {
  ...
  bool willOverride = lab.OverrideExperimentVariation("report_test_times",
                                                      "user_id_8", "control");
  EXPECT_TRUE(willOverride);
...
TEST_F(AnkiLabTest, DisableTheLab) {
  ...
  // Verify that user is unassigned when lab is disabled
  {
    ...
    AssignmentStatus status =
        lab.ActivateExperiment("report_test", "user_id_8", {}, variationKey);
    EXPECT_EQ(AssignmentStatus::Unassigned, status);
  ...
  {
    AssignmentStatus status =
        lab.ForceActivateExperiment("report_test", "user_id_8", "treatment");
    EXPECT_EQ(AssignmentStatus::Unassigned, status);

    // Verify that experiment name and variation are evaluated even if lab is
    // disabled when attempting a force
    status = lab.ForceActivateExperiment("invalid", "user_id_8", "treatment");
    EXPECT_EQ(AssignmentStatus::ExperimentNotFound, status);
    status = lab.ForceActivateExperiment("report_test", "user_id_8", "invalid");
    EXPECT_EQ(AssignmentStatus::VariantNotFound, status);
  }
...
TEST_F(AnkiLabTest, HandleActiveAssignmentUpdate) {
  ...
  Signal::SmartHandle handle =
      lab.ActiveAssignmentsUpdatedSignal().ScopedSubscribe(
          [&assignmentsSize](
              const std::vector<AssignmentDef>& activeAssignments) {
...
TEST_F(AnkiLabTest, RestorePreviousAssignments) {
  ...
  // Simulate previously being in the "control" group for an experiment that
  // changed and put 100% of users in the "treatment" group.  Our user should
  // remain in "control"
  AssignmentStatus status = lab.RestoreActiveExperiment(
      "report_test_all_treatment", "user_id_8", "control");
  EXPECT_EQ(AssignmentStatus::Assigned, status);

  std::string variationKey;
  status = lab.ActivateExperiment("report_test_all_treatment", "user_id_8", {},
                                  variationKey);
  EXPECT_EQ(AssignmentStatus::Assigned, status);
  EXPECT_EQ("control", variationKey);
  ...
  uint32_t epochSec =
      Util::EpochSecFromIso8601UTCDateString("2017-05-01T00:00:00Z");
  status = lab.RestoreActiveExperiment("report_test_times", "user_id_8",
                                       "treatment", epochSec);
  EXPECT_EQ(AssignmentStatus::ExperimentNotRunning, status);
}
...
TEST_F(AnkiLabTest, RestoreAssignmentWithAudienceFiltering) {
  ...
  AssignmentStatus status;
  status =
      lab.ActivateExperiment(experimentKey, userId, audienceTags, variationKey);
  EXPECT_EQ(AssignmentStatus::AudienceMismatch, status);
  EXPECT_EQ("", variationKey);
  ...
  // A subsequent call to activate the experiment should succeed, even if the
