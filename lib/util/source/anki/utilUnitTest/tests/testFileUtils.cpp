...
TEST_F(FileUtilsTest, CreateRemoveDirectory) {
  ...
  Anki::Util::FileUtils::CreateDirectory(path + "/A/B/cc/myfile.txt", true,
                                         true);
  DirectoryShouldExist(path + "/A/B/cc");
...
TEST_F(FileUtilsTest, ListDirectory) {
  ...
  std::vector<std::string> files =
      Anki::Util::FileUtils::FilesInDirectory(path);
...
TEST_F(FileUtilsTest, ListDirectoryWithExtensionFilters) {
  ...
  auto FilesMatch = [](const std::set<std::string>& groundTruth,
  ...
  const std::set<std::string> fileNames{
      "jpegExtension.jpeg", "jpgLowerExtension.jpg", "jpgUpperExtension.JPG",
      "noExtension", "pngExtension.png"};
  ...
  foundFiles =
      Anki::Util::FileUtils::FilesInDirectory(path, false, {"jpg", "JPG"});
  ...
  foundFiles = Anki::Util::FileUtils::FilesInDirectory(
      path, false, {"jpg", "png", "jpeg", "JPG"});
  correctAnswer = {"jpegExtension.jpeg", "jpgLowerExtension.jpg",
                   "jpgUpperExtension.JPG", "pngExtension.png"};
  ASSERT_TRUE(FilesMatch(correctAnswer, foundFiles));

  foundFiles =
      Anki::Util::FileUtils::FilesInDirectory(path, false, {"jpg", "jpeg"});
  correctAnswer = {"jpegExtension.jpeg", "jpgLowerExtension.jpg"};
  ASSERT_TRUE(FilesMatch(correctAnswer, foundFiles));
...
TEST_F(FileUtilsTest, FileExists) {
  ...
  Anki::Util::FileUtils::DeleteFile(fileName);
  FileShouldNotExist(fileName);
  ASSERT_FALSE(Anki::Util::FileUtils::FileExists(fileName))
      << fileName << " should not exist";

  Anki::Util::FileUtils::WriteFile(fileName, "file contents");
  AssertFileExistsWithContents(fileName, "file contents");
  ASSERT_TRUE(Anki::Util::FileUtils::FileExists(fileName))
...
TEST_F(FileUtilsTest, RecurseGetFiles) {
  ...
  std::vector<std::string> foundFiles =
      Anki::Util::FileUtils::FilesInDirectory(path, true, nullptr, true);
...
#define FILESEP "/"
  ...
  const std::string groundTruth1("path" FILESEP "to" FILESEP "given" FILESEP
                                 "file.jpg");
  const std::string groundTruth1b(FILESEP "path" FILESEP "to" FILESEP
                                          "given" FILESEP "file.jpg");
  ...
  //
  std::string result =
      Anki::Util::FileUtils::FullFilePath({"path", "to", "given", "file.jpg"});
  ASSERT_EQ(result, groundTruth1);

  const std::string path1("path"), path2("to"), path3("given"),
      file("file.jpg");
  ...
  //
  result = Anki::Util::FileUtils::FullFilePath(
      {"path" FILESEP, FILESEP "to", "given", FILESEP "file.jpg"});
  ASSERT_EQ(result, groundTruth1);

  result = Anki::Util::FileUtils::FullFilePath(
      {FILESEP "path" FILESEP, FILESEP "to", FILESEP "given" FILESEP,
       FILESEP "file.jpg"});
  ...
  //
  result = Anki::Util::FileUtils::FullFilePath(
      {"path" FILESEP "to", "given" FILESEP "file.jpg"});
  ASSERT_EQ(result, groundTruth1);

  result =
      Anki::Util::FileUtils::FullFilePath({FILESEP "path" FILESEP "to" FILESEP,
                                           FILESEP "given" FILESEP "file.jpg"});
  ASSERT_EQ(result, groundTruth1b);
  ...
  result =
      Anki::Util::FileUtils::FullFilePath({"", "path" FILESEP, "", "file.jpg"});
  ASSERT_EQ(result, groundTruth2);
}
...
#define FILESEP "/"
  ...
  const std::string path1("path" FILESEP "to" FILESEP "given" FILESEP
                          "file.jpg");
  const std::string path2(FILESEP "path" FILESEP "to" FILESEP "given" FILESEP
                                  "file.jpg");
  ...
  // TODO: Instead of returning an empty string this should probably return
  // "file"
