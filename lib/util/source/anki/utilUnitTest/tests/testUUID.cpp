...
TEST_F(TestUUID, UUIDCompare) {
  UUIDBytes expectedUUID = {{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                             0x08, 0x09, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15}};

  UUIDBytes uuid0_10 = {{0x10, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
                         0x09, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15}};
  UUIDBytes uuid15_0 = {{0x0, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
                         0x09, 0x10, 0x11, 0x12, 0x13, 0x14, 0x0}};
  UUIDBytes uuid5_0 = {{0x0, 0x01, 0x02, 0x03, 0x04, 0x0, 0x06, 0x07, 0x08,
                        0x09, 0x10, 0x11, 0x12, 0x13, 0x14, 0x0}};

  ASSERT_EQ(UUIDCompare(&expectedUUID, &uuid0_10),
            expectedUUID.bytes[0] - uuid0_10.bytes[0])
      << "Should differ by 10 at bytes 0";
  ASSERT_EQ(UUIDCompare(&expectedUUID, &uuid15_0),
            expectedUUID.bytes[15] - uuid15_0.bytes[15])
      << "Should differ by 15 at byte 15";
  ASSERT_EQ(UUIDCompare(&expectedUUID, &uuid5_0),
            expectedUUID.bytes[5] - uuid5_0.bytes[5])
      << "Should differ by 5 at byte 5";
}

TEST_F(TestUUID, UUIDCopy) {
  UUIDBytes sourceBytes = {{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                            0x08, 0x09, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15}};
  ...
  ASSERT_TRUE(UUIDCompare(sourceUUID, UUIDcopy) == 0)
      << "Copy should be identical to source";
  ...
  ASSERT_EQ(empty, UUIDEmpty())
      << "UUIDEmpty() returns the same read-only pointer";
  ASSERT_EQ(UUIDCompare(UUIDEmpty(), empty), 0)
      << "UUIDCopy() cannot modify an empty UUID";
}

TEST_F(TestUUID, UUIDFromString) {
  UUIDBytes expectedUUID = {{0x87, 0x5e, 0x95, 0x53, 0x17, 0xd0, 0x44, 0xbc,
                             0x8c, 0x49, 0xda, 0x77, 0x74, 0xda, 0x76, 0xb7}};
  ...
  ASSERT_TRUE(UUIDCompare(&uuid, &expectedUUID) == 0)
      << "UUID bytes should be equal";
  ...
  ASSERT_EQ(UUIDCompare(&invalidUUID1, &expectedUUID),
            invalidUUID1.bytes[15] - expectedUUID.bytes[15]);
...
TEST_F(TestUUID, StringFromUUID) {
  UUIDBytes uuidBytes = {{0x87, 0x5e, 0x95, 0x53, 0x17, 0xd0, 0x44, 0xbc, 0x8c,
                          0x49, 0xda, 0x77, 0x74, 0xda, 0x76, 0xb7}};
  ...
  ASSERT_EQ(strncmp(uuidString, expectedUUIDString, strlen(expectedUUIDString)),
            0);
  ...
  UUIDBytes invalidUUIDBytes15 = {{0x87, 0x5e, 0x95, 0x53, 0x17, 0xd0, 0x44,
                                   0xbc, 0x8c, 0x49, 0xda, 0x77, 0x74, 0xda,
                                   0x76}};
  const char* invalidUUIDString15 = StringFromUUIDBytes(&invalidUUIDBytes15);
  ASSERT_NE(strncmp(invalidUUIDString15, expectedUUIDString,
                    strlen(expectedUUIDString)),
            0);
}
