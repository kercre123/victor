 ...
 *              UtilMsgPack/UtilMsgUnpack are the main funcitons used to
 *pack/unpack this data.
 *
 **/
...
/**
 *  Helper function for the testing of packing different types and their
 *upper/lower bounds.
 **/
template <typename T>
void TestTypePackingHelper(char packType, T minValue, T maxValue) {
  ...
  error =
      SafeUtilMsgPack(dstBuffer, bufferSize, &bytesWritten, packString,
                      srcBuffer[0], srcBuffer[1], srcBuffer[2], srcBuffer[3]);
  uint32_t expectedSize = SafeUtilGetMsgSize(
      packString, srcBuffer[0], srcBuffer[1], srcBuffer[2], srcBuffer[3]);
  ...
  error = SafeUtilMsgUnpack(srcBuffer, bufferSize, &bytesRead, packString,
...
void TestArrayPackingHelper(int size, const char* packString, T* srcBuffer) {
  // When we pack array size, we pack it as a char and therefore can't have more
  // than max char.
  ASSERT_LE(size, MAX_ELEMENTS);

  static const char BAD_VALUE =
      0xDE;  // Simple way to check for uninitialized values.
  static const int DST_BUFFER_SIZE =
      ((MAX_ELEMENTS * sizeof(T)) + 2);  // Add some padding for the size char
                                         // and for buffer overrun detection.
  ...
  // Testing UtilMsgPack
  {
    ...
    unsigned int bytesWritten = 0;
    const UtilMsgError error = SafeUtilMsgPack(
        dstBuffer, DST_BUFFER_SIZE, &bytesWritten, packString, size, srcBuffer);
    ...
    EXPECT_EQ((char)size,
              dstBuffer[0]);  // Check that the size was packed correctly.
    ...
    EXPECT_EQ(
        BAD_VALUE,
        dstBuffer[lastIndex]);  // Check that we didn't overrun the buffer.
  }
  ...
  // Testing UtilMsgUnpack
  {
    ...
    unsigned int bytesRead = 0;
    const UtilMsgError error =
        SafeUtilMsgUnpack(dstBuffer, DST_BUFFER_SIZE, &bytesRead, packString,
                          &unpackedSize, unpackedBuffer);
    EXPECT_EQ(UTILMSG_OK, error);
    ...
    EXPECT_EQ(BAD_VALUE,
              *(char*)lastElement);  // Check that we didn't overrun the buffer.
...
TEST(TestMsgPack, TestArrayPacking) {
  ...
  // Since gtest seems to automatically fail tests when a floating value is Nan,
  // we cannot simply use an array of random bytes. We'll need to create an
  // array of random legitimate float values instead.
  {
    Anki::Util::RandomGenerator rand(12345);
 ...
 **/
template <typename T>
void TestRandomPackingHelper(char*& inBuffer, T expected,
                             unsigned char expectedSize, T* expectedArray) {
...
/**
 *  Test packing of all supported types in a single pack/unpack using randomly
 *generated data.
 **/
TEST(TestMsgPack, TestRandomData) {
  ...
  const long long int rLong =
      (long long int)rand.RandIntInRange(INT32_MIN, INT32_MAX);
  ...
  // Add up how many bytes we need to allocate for our source/destination
  // buffers. 1 byte for each array size + 1 sizeof(T) for the random value + X
  // sizeof(T) for the array.
  ...
  // Test the packing of all of this random data.
  {
    ...
    unsigned int bytesWritten = 0;
    const UtilMsgError error =
        SafeUtilMsgPack(dstBuffer, numBytes, &bytesWritten, packString, rChar,
                        cArraySize, cArray, rShort, hArraySize, hArray, rInt,
                        iArraySize, iArray, rLong, lArraySize, lArray, rFloat,
                        fArraySize, fArray, rDouble, dArraySize, dArray);
    uint32_t expectedSize = SafeUtilGetMsgSize(
        packString, rChar, cArraySize, cArray, rShort, hArraySize, hArray, rInt,
        iArraySize, iArray, rLong, lArraySize, lArray, rFloat, fArraySize,
        fArray, rDouble, dArraySize, dArray);
  ...
  // Test the unpacking of all of this random data.
  {
    ...
    unsigned int bytesRead = 0;
    const UtilMsgError error = SafeUtilMsgUnpack(
        dstBuffer, numBytes, &bytesRead, packString, &cExpected, &cExpectedSize,
        cExpectedArray, &hExpected, &hExpectedSize, hExpectedArray, &iExpected,
        &iExpectedSize, iExpectedArray, &lExpected, &lExpectedSize,
        lExpectedArray, &fExpected, &fExpectedSize, fExpectedArray, &dExpected,
        &dExpectedSize, dExpectedArray);
...
/*
 *  Simple test to make sure the number of bytes written/read is being reported
 * accurately. The other unit tests also incorporate checking for bytes
 * written/read so I'm keeping this simple.
 */
TEST(TestMsgPack, TestPackingSize) {
  ...
  const int bytesExpected = sizeof(char) + sizeof(short) + sizeof(int) +
  ...
  error = SafeUtilMsgPack(dstBuffer, BUFFER_SIZE, &bytesWritten, "chilfd",
                          cValue, hValue, iValue, lValue, fValue, dValue);
  expectedSize = SafeUtilGetMsgSize("chilfd", cValue, hValue, iValue, lValue,
                                    fValue, dValue);
  EXPECT_EQ(expectedSize, bytesWritten);
  EXPECT_EQ(UTILMSG_OK, error);
  ...
  unsigned int bytesRead = 0;
  error =
      SafeUtilMsgUnpack(dstBuffer, BUFFER_SIZE, &bytesRead, "chilfd", &cValue,
                        &hValue, &iValue, &lValue, &fValue, &dValue);
  EXPECT_EQ(UTILMSG_OK, error);
  EXPECT_EQ(bytesExpected, bytesRead);

  // Check for a zero sized array.
  error =
      SafeUtilMsgPack(dstBuffer, BUFFER_SIZE, &bytesWritten, "ac", 0, cValue);
  ...
  error = SafeUtilMsgUnpack(dstBuffer, BUFFER_SIZE, &bytesRead, "ac", &size,
                            &cValue);
  EXPECT_EQ(UTILMSG_OK, error);
...
TEST(TestMsgPack, TestSafetyChecking) {
  ...
  // Negative array size ...
  error = SafeUtilMsgPack(dstBuffer, BUFFER_SIZE, &bytesPacked, "ac", -1,
                          srcBuffer);
  EXPECT_EQ(UTILMSG_OK, error);
  EXPECT_EQ(1, bytesPacked);
  error = SafeUtilMsgUnpack(dstBuffer, BUFFER_SIZE, &bytesRead, "ac",
                            &expectedSize, srcBuffer);
  EXPECT_EQ(UTILMSG_OK, error);
  EXPECT_EQ(0, expectedSize);

  // Too large array size ...
  error = SafeUtilMsgPack(dstBuffer, BUFFER_SIZE, &bytesPacked, "ac", 256,
                          srcBuffer);
  EXPECT_EQ(UTILMSG_OK, error);
  ...
  error = SafeUtilMsgPack(dstBuffer, 1, &bytesPacked, "cac", srcBuffer[0], 1,
                          srcBuffer);
  EXPECT_EQ(UTILMSG_BUFFEROVRN, error);
  EXPECT_EQ(1, bytesPacked);
  error = SafeUtilMsgUnpack(dstBuffer, 1, &bytesRead, "cac", &srcBuffer[0],
                            &expectedSize, srcBuffer);
  EXPECT_EQ(UTILMSG_BUFFEROVRN, error);
  EXPECT_EQ(1, bytesRead);

  // Buffer overrun: Writing/Reading char ...
  error = SafeUtilMsgPack(dstBuffer, 1, &bytesPacked, "acc", BUFFER_SIZE,
                          srcBuffer, srcBuffer[0]);
  EXPECT_EQ(UTILMSG_BUFFEROVRN, error);
  EXPECT_EQ(1, bytesPacked);
  error =
      SafeUtilMsgUnpack(dstBuffer, 1, &bytesRead, "cc", srcBuffer, srcBuffer);
  EXPECT_EQ(UTILMSG_BUFFEROVRN, error);
  EXPECT_EQ(1, bytesRead);
  EXPECT_EQ(dstBuffer[0], srcBuffer[0]);

  // Buffer overrun: Writing/Reading short ...
  error = SafeUtilMsgPack(dstBuffer, 2, &bytesPacked, "ahc", BUFFER_SIZE,
                          srcBuffer, srcBuffer[0]);
  EXPECT_EQ(UTILMSG_BUFFEROVRN, error);
  EXPECT_EQ(1, bytesPacked);
  error =
      SafeUtilMsgUnpack(dstBuffer, 2, &bytesRead, "hh", srcBuffer, srcBuffer);
  EXPECT_EQ(UTILMSG_BUFFEROVRN, error);
  EXPECT_EQ(2, bytesRead);
  EXPECT_EQ(*(short*)dstBuffer, *(short*)srcBuffer);

  // Buffer overrun: Writing/Reading int ...
  error = SafeUtilMsgPack(dstBuffer, 4, &bytesPacked, "aic", BUFFER_SIZE,
                          srcBuffer, srcBuffer[0]);
  EXPECT_EQ(UTILMSG_BUFFEROVRN, error);
  EXPECT_EQ(1, bytesPacked);
  error =
      SafeUtilMsgUnpack(dstBuffer, 4, &bytesRead, "ii", srcBuffer, srcBuffer);
  EXPECT_EQ(UTILMSG_BUFFEROVRN, error);
  EXPECT_EQ(4, bytesRead);
  EXPECT_EQ(*(int*)dstBuffer, *(int*)srcBuffer);

  // Buffer overrun: Writing/Reading long ...
  error = SafeUtilMsgPack(dstBuffer, 8, &bytesPacked, "alc", BUFFER_SIZE,
                          srcBuffer, srcBuffer[0]);
  EXPECT_EQ(UTILMSG_BUFFEROVRN, error);
  EXPECT_EQ(1, bytesPacked);
  error =
      SafeUtilMsgUnpack(dstBuffer, 8, &bytesRead, "ll", srcBuffer, srcBuffer);
  EXPECT_EQ(UTILMSG_BUFFEROVRN, error);
  EXPECT_EQ(8, bytesRead);
  EXPECT_EQ(*(long long int*)dstBuffer, *(long long int*)srcBuffer);

  // Buffer overrun: Writing/Reading float ...
  error = SafeUtilMsgPack(dstBuffer, 4, &bytesPacked, "afc", BUFFER_SIZE,
                          srcBuffer, srcBuffer[0]);
  EXPECT_EQ(UTILMSG_BUFFEROVRN, error);
  EXPECT_EQ(1, bytesPacked);
  error =
      SafeUtilMsgUnpack(dstBuffer, 4, &bytesRead, "ff", srcBuffer, srcBuffer);
  EXPECT_EQ(UTILMSG_BUFFEROVRN, error);
  EXPECT_EQ(4, bytesRead);
  EXPECT_EQ_ALLOW_NANS(*(float*)dstBuffer, *(float*)srcBuffer);

  // Buffer overrun: Writing/Reading double ...
  error = SafeUtilMsgPack(dstBuffer, 8, &bytesPacked, "adc", BUFFER_SIZE,
                          srcBuffer, srcBuffer[0]);
  EXPECT_EQ(UTILMSG_BUFFEROVRN, error);
  EXPECT_EQ(1, bytesPacked);
  error =
      SafeUtilMsgUnpack(dstBuffer, 8, &bytesRead, "dd", srcBuffer, srcBuffer);
  EXPECT_EQ(UTILMSG_BUFFEROVRN, error);
  EXPECT_EQ(8, bytesRead);
  ...
  EXPECT_EQ(0, bytesPacked);
  error = SafeUtilMsgUnpack(dstBuffer, BUFFER_SIZE, &bytesRead, "az",
                            &expectedSize, srcBuffer);
  EXPECT_EQ(UTILMSG_INVALIDARG, error);
  EXPECT_EQ(1, bytesRead);
}
