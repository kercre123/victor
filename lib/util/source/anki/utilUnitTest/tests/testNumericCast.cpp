...
void TestEqualDouble(double expectedResult, double actualResult) {
  // Due to floating-point precision loss, especially around conversion to/from
  // strings, we must compare using a relative epsilon value

  using namespace Anki::Util;

  const double diff = expectedResult - actualResult;
  // scale epsilon by smaller of the 2 absolute values - that way if both values
  // are similarly large we compare against a larger epsilon
...
void TestNumericCastAgaintExpected(FromType fromValue, ToType expectedResult,
                                   bool expectedIsValid) {
  EXPECT_EQ(expectedIsValid, Anki::Util::IsValidNumericCast<ToType>(fromValue));

  TestEqual(expectedResult,
            Anki::Util::numeric_cast_clamped<ToType>(fromValue));
...
#define GET_TYPENAME_SPECIALIZATION(type) \
  template <>                             \
...
static void PrintValue(FromType value, bool isForCodeGen = true) {
  ...
  if (isForCodeGen && (value < 0) && (int64_t(value) == kInt64Lowest)) {
    // workaround for compiler not liking the smallest i64 value if written
    // literally
    printf("kInt64Lowest");
    return;
  }
  ...
  if (isForCodeGen && (value > 0) &&
      (uint64_t(value) > uint64_t(std::numeric_limits<int64_t>::max()))) {
    printf("ull");
  }
}
...
void TestNumericCastValues(const FromType* fromValues,
                           const ToType* expectedToValues,
                           const bool* expectedValidValues, size_t numValues) {
  ...
  for (size_t vI = 0; vI < numValues; ++vI) {
    ...
    TestNumericCastAgaintExpected<ToType>(fromValue, expectedToValues[vI],
                                          expectedValidValues[vI]);
  }

  printf("Tested: %s to %s: %zu values, range: ", GetTypeName<FromType>(),
         GetTypeName<ToType>(), numValues);
...
void TestNumericCastFrom_int8_t() {
  // Test every possible value of an int8 being cast into every other numeric
  // type
  ...
  printf("Tested: int8 to all: %d values, range: %d..%d\n", numValues,
         FromTypeLimits::lowest(), lastI);
}

void TestNumericCastFrom_uint8_t() {
  // Test every possible value of a uint8 being cast into every other numeric
  // type
...
// Tables etc. for other tests are generated from code and places in a separate
// file
...
void CodeGenTestNumericCastTo(const FromType* fromValues, size_t numValues) {
  ...
  printf(
      ...
      "Anki::Util::array_size(expectedValues_%s), \"Array Sizes Differ!\");\n",
      toTypeName);
  printf(
      "    static_assert(Anki::Util::array_size(fromValues) == "
      "Anki::Util::array_size(expectedValid_%s),  \"Array Sizes Differ!\");\n",
      toTypeName);
  printf("\n");
  printf(
      "    TestNumericCastValues(fromValues, expectedValues_%s, "
      "expectedValid_%s, Anki::Util::array_size(fromValues));\n",
...
void CodeGenTestNumericCastAll() {
  printf(
      "// Auto-generated code from testNumericCast for generating tables of "
      "expected casts for each type->type for a representative set of "
      "values\n");
  printf(
      "// See CodeGenTestNumericCastAll(), enabled via '#define "
      "ENABLE_NUMERIC_CAST_CODE_GEN 1'\n");
  printf("\n\n");

  // Use values around each signed/unsigned boundary (i.e. around +/- 2^(8n-1)
  // and 2^(8n) for n=0,1,2,4,8) 2^7  = 128 2^8  = 256 2^15 = 32768 2^16 = 65536
  // 2^31 = 2147483648
  // 2^32 = 4294967296
  ...
  // int16 ->
  {
    ...
    FromType values[] = {-32768, -32767, -32766,        // -2^15
                         -257,   -256,   -255,   -254,  // -2^8
                         -129,   -128,   -127,   -126,  // -2^7
                         -1,     0,      1,             //  0
                         126,    127,    128,    129,   //  2^7
  ...
  // int32 ->
  {
    ...
    // Note: have to write "(-9223372036854775807)-1" instead of
    // -9223372036854775808 to appease the compiler...
    FromType values[] = {
...

  {
    ...
    FromType values[] = {0,
                         ...
                         9223372036854775809ull,  //  2^63
                         18446744073709551614ull,
                         18446744073709551615ull};  //  2^64
...

  {
    ...
    FromType values[] = {minPossibleValue,
                         -1.e38,
                         -1.e29,
                         -1.e20,
                         -1.e11,
