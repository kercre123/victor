...
TEST(CircularBuffer, LoopBufferPushBack) {
  ...
  // first prime the buffer (so we can easily test on previous values in the
  // next loop)
  for (int i = -9; i <= -6; ++i) {
  ...
  // now continue to add items verifying buffer order each time, enough times to
  // ensure several loops
  for (int i = -5; i <= 24; ++i) {
    testBuff.push_back(i);
...
TEST(CircularBuffer, LoopBufferPushBackArray) {
  ...
  for (size_t idx = 0; idx < testArray2Size; idx += loopSize) {
    ...
    for (size_t testDataOffset = 0; testDataOffset < loopSize;
         ++testDataOffset) {
  ...
  // Test completely filling the buffer 6x while poping data out in between
  // iterations
  for (size_t idx = 0; idx < testArray2Size; idx += loopSize) {
    testBuff.push_back(&testArray2[idx], loopSize);
    EXPECT_FALSE(testBuff.empty());
    ...
    for (size_t testDataOffset = 0; testDataOffset < loopSize - outArray2Size;
         ++testDataOffset) {
      EXPECT_EQ(testBuff[testDataOffset],
                testArray2[outArray2Size + idx + testDataOffset]);
    }
  }
...
TEST(CircularBuffer, MoveConstructor) {
  ...
  Anki::Util::CircularBuffer<int> testBuff2(
      std::move(testBuff));  // Move Constructor

  // testBuff should now be empty and have no storage allocated, testBuff2
  // should be equivalent to old testBuff
  ...
  // check we can reserve testBuff again and add to it without it affecting
  // testBuff2
