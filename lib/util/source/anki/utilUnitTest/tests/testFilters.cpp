...
class TestLowPassFilterSimple : public ::testing::Test {
 public:
  // Create a step input (0 for the first sample, 1 after that) and ensure the
  // filtered value rises in the expected way.
...
TEST_F(TestLowPassFilterSimple, TestFilterParamChanges) {
  ...
  // Construct filter with one sample period, but immediately change it just to
  // test that we can.
  Anki::Util::LowPassFilterSimple filter(kInitialSamplePeriod_sec,
                                         kInitialTimeConstant_sec);
