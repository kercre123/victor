...
double GetTimingModeScalar() {
  ...
  switch (UtilUnitTestShared::GetTimingMode()) {
    case UtilUnitTestShared::TimingMode::Valgrind:
      // Scale timeouts etc. up by X to account for valgrind running the tests
      // slower
      scalar = 10.0;
      break;
    default:
...
void InitOverdriveNetSettings() {
  ...
  ReliableConnection::SetMaxTimeSinceLastSend(
      ReliableConnection::GetTimeBetweenResendsInMS() - 1.0);
...
void InitNetworkSettings(int configNum) {
  // try slightly different settings per test to get coverage of different
  // setting combos too
...
class TestNetTransportDataReceiver
 ...
 public:
    ...
    if (size > 0) {
      ...
      if (buffer == INetTransportDataReceiver::OnConnectRequest) {
        ...
        printf("[%s] Recv OnDisconnected from '%s' (was index %d)\n", _name,
               sourceAddress.ToString().c_str(), existingConnection);
      } else {
        assert(0);
      }
    }
  ...
  void SetTransport(ReliableTransport* transport) { _transport = transport; }
 ...
 private:
  ...
  mutable std::mutex _mutex;  // so unit tests can safely access data whilst
                              // socket thread writes to this
};
...
struct TestClientWrapper {
  ...
  ~TestClientWrapper() {
    // Make sure _reliableTransport's thread stops running before anything else
    // is destroyed
    KillThreads();
    delete _udpTransport;
  }

  void KillThreads() { _reliableTransport.KillThread(); }
...
struct TestClientWrappers3 {
  ...
  ~TestClientWrappers3() { KillThreads(); }
...
bool WaitUntilDoneOrMaxTime(uint32_t sleepPerTickInMS, uint32_t maxDurationInMS,
                            std::function<bool()> isWorkDoneFunc) {
  ...
  // printf("Slept for %u ms - %s\n", timeSlept, isWorkDone ? "Done!" :
  // "TimedOut!");

  return isWorkDone;
}

// returns true if receivedBytes is identical to any ordering of all of the
// sentMessages
bool TestStringsReceivedOKInAnyOrder(
    const std::vector<uint8_t>& receivedBytes,
    const std::vector<const char*>& sentMessages) {
  size_t bytesSent = 0;
...

  {
    ...
    size_t receivedOffset =
        0;  // how far into receivedBytes that we've matched so far
    for (size_t i = 0; i < sentMessages.size(); ++i) {
      // try to match each message, each time using all unmatched sent messages
      // as candidates
      bool foundMatch = false;
      for (size_t j = 0; (!foundMatch && (j < messagesLeftToMatch.size()));
           ++j) {
        const char* messageSent = messagesLeftToMatch[j];
        const size_t messageSentLen = strlen(messageSent);

        int bufCompRes =
            memcmp(&receivedBytes[receivedOffset], messageSent, messageSentLen);
        if (bufCompRes == 0) {
          foundMatch = true;
...
void TestMultiClientMessageSend(TestClientWrapper& host1,
                                TestClientWrapper& client2,
                                TestClientWrapper& client3,
                                uint32_t maxTimeForTest) {
  ...
  WaitUntilDoneOrMaxTime(25, maxTimeForTest, [&] {
    return ((host1._dataReceiver.GetNumConnections() == 3) &&
  ...
  host1._reliableTransport.SendData(true, host1.GetTransportAddress(),
                                    (uint8_t*)k_TestMessage1,
                                    uint32_t(strlen(k_TestMessage1)));
  host1._reliableTransport.SendData(true, client2.GetTransportAddress(),
                                    (uint8_t*)k_TestMessage2,
                                    uint32_t(strlen(k_TestMessage2)));
  host1._reliableTransport.SendData(true, client3.GetTransportAddress(),
                                    (uint8_t*)k_TestMessage3,
                                    uint32_t(strlen(k_TestMessage3)));

  // Send messages from client2 to All
  client2._reliableTransport.SendData(true, host1.GetTransportAddress(),
                                      (uint8_t*)k_TestMessage4,
                                      uint32_t(strlen(k_TestMessage4)));
  client2._reliableTransport.SendData(true, client2.GetTransportAddress(),
                                      (uint8_t*)k_TestMessage5,
                                      uint32_t(strlen(k_TestMessage5)));
  client2._reliableTransport.SendData(true, client3.GetTransportAddress(),
                                      (uint8_t*)k_TestMessage6,
                                      uint32_t(strlen(k_TestMessage6)));

  // Send messages from client3 to All
  client3._reliableTransport.SendData(true, host1.GetTransportAddress(),
                                      (uint8_t*)k_TestMessage7,
                                      uint32_t(strlen(k_TestMessage7)));
  client3._reliableTransport.SendData(true, client2.GetTransportAddress(),
                                      (uint8_t*)k_TestMessage8,
                                      uint32_t(strlen(k_TestMessage8)));
  client3._reliableTransport.SendData(true, client3.GetTransportAddress(),
                                      (uint8_t*)k_TestMessage9,
                                      uint32_t(strlen(k_TestMessage9)));

  WaitUntilDoneOrMaxTime(25, maxTimeForTest, [&] {
    return ((host1._dataReceiver.GetNumMessagesReceived() >= 3) &&
...

  {
    ...
    bool res = TestStringsReceivedOKInAnyOrder(
        host1._dataReceiver.LockAndGetReceivedBytes(), sentMessages);
  ...
  }
  {
    ...
    bool res = TestStringsReceivedOKInAnyOrder(
        client2._dataReceiver.LockAndGetReceivedBytes(), sentMessages);
  ...
  }
  {
    ...
    bool res = TestStringsReceivedOKInAnyOrder(
        client3._dataReceiver.LockAndGetReceivedBytes(), sentMessages);
...
static void MissingPacketTest() {
  ...
  const uint32_t totalPacketsDropped =
  ...
  // not a test fail as such, but we want to make sure the deterministic random
  // values we're giving result in at least 1 dropped packet otherwise we
  // weren't really stressing anything
  EXPECT_GE(totalPacketsDropped, 1);  // make sure we're dropping enough packets
                                      // to make the test meaningful

  printf(
      "MissingPacketTest: Simulated %u packets dropped total. (host1: %u, "
      "client2: %u, client3: %u)\n",
      totalPacketsDropped, host1._emulatorSocket.GetNumPacketsDropped(),
      client2._emulatorSocket.GetNumPacketsDropped(),
      client3._emulatorSocket.GetNumPacketsDropped());

  const uint32_t outOfOrderHost1 =
      host1._reliableTransport.GetTransportStats().GetRecvStats().GetErrorCount(
          eME_OutOfOrder);
  const uint32_t outOfOrderClient2 =
      client2._reliableTransport.GetTransportStats()
          .GetRecvStats()
          .GetErrorCount(eME_OutOfOrder);
  const uint32_t outOfOrderClient3 =
      client3._reliableTransport.GetTransportStats()
          .GetRecvStats()
          .GetErrorCount(eME_OutOfOrder);
  const uint32_t totalOutOfOrder =
      outOfOrderHost1 + outOfOrderClient2 + outOfOrderClient3;

  printf(
      "MissingPacketTest: Simulated %u packets arrived out of order requiring "
      "resends. (host1: %u, client2: %u, client3: %u)\n",
...
static void ReorderedJitterTest() {
  ...
  // Simulate jitter with wide range of latency for each packet - will cause
  // packet reorder
  ...
  printf(
      "ReorderedJitterTest: Simulated %u packets dropped total. (host1: %u, "
      "client2: %u, client3: %u)\n",
      totalPacketsDropped, host1._emulatorSocket.GetNumPacketsDropped(),
      client2._emulatorSocket.GetNumPacketsDropped(),
      client3._emulatorSocket.GetNumPacketsDropped());
  ...
  printf(
      "ReorderedJitterTest: Simulated %u packets arrived out of order "
      "requiring resends. (host1: %u, client2: %u, client3: %u)\n",
      totalOutOfOrder, outOfOrderHost1, outOfOrderClient2, outOfOrderClient3);

  // not a test fail as such, but we want to make sure the deterministic random
  // values we're giving result in at least 1 out of order message otherwise we
  // weren't really stressing anything
...
static void HugePacketTest() {
  ...
  host1._reliableTransport.Connect(client2.GetTransportAddress());
  WaitUntilDoneOrMaxTime(25, k_MaxTimeForTest, [&] {
    return (host1._dataReceiver.GetNumConnections() == 1) &&
           (client2._dataReceiver.GetNumConnections() == 1);
  });

  const size_t k_NumUIntsToStore = 1024 * 2;  // 2K ints -> 8KB message
  uint32_t* hugeMessageBuffer = new uint32_t[k_NumUIntsToStore];
  const size_t k_SizeOfHugeMessageBuffer =
      sizeof(hugeMessageBuffer[0]) * k_NumUIntsToStore;

  for (uint32_t i = 0; i < k_NumUIntsToStore; ++i) {
    hugeMessageBuffer[i] = i;
  }

  host1._reliableTransport.SendData(
      true, client2.GetTransportAddress(),
      reinterpret_cast<uint8_t*>(hugeMessageBuffer), k_SizeOfHugeMessageBuffer);

  WaitUntilDoneOrMaxTime(25, k_MaxTimeForTest, [&] {
    return (client2._dataReceiver.GetNumBytesReceived() >=
            k_SizeOfHugeMessageBuffer);
  });

  EXPECT_EQ(client2._dataReceiver.GetNumBytesReceived(),
            k_SizeOfHugeMessageBuffer);

  if (client2._dataReceiver.GetNumBytesReceived() ==
      k_SizeOfHugeMessageBuffer) {
    const int bufCompRes =
        memcmp(&client2._dataReceiver.LockAndGetReceivedBytes()[0],
               hugeMessageBuffer, k_SizeOfHugeMessageBuffer);
    client2._dataReceiver.Unlock();
  ...
  printf(
      "HugePacketTest: Simulated %u packets dropped total. (host1: %u, "
      "client2: %u)\n",
      totalPacketsDropped, host1._emulatorSocket.GetNumPacketsDropped(),
      client2._emulatorSocket.GetNumPacketsDropped());
  ...
  printf(
      "HugePacketTest: Simulated %u packets arrived out of order requiring "
      "resends. (host1: %u, client2: %u)\n",
      totalOutOfOrder, outOfOrderHost1, outOfOrderClient2);

  // not a test fail as such, but we want to make sure the deterministic random
  // values we're giving result in at least 1 out of order message (from packet
  // lost or reorder) otherwise we weren't really stressing anything
  EXPECT_GE(totalOutOfOrder, 1);
}
...
static void MultipleLargePacketTest() {
  // Like HugePacketTest, but splits sends into multiple large packets that are
  // small enough to fit several (~5) in 1 UDP packet, but are large and
  // numerous enough that it will take multiple large UDP packets to fit all of
  // them. Verifies in-order ness of discrete messages and underlying reliable
  // code that puts as many reliable messages into 1 UDP packet as it can
  ...
  // Multiple smaller sends of k_BytesPerMessage to transmit data
  {
    ...
    const uint8_t* messageBytes =
        reinterpret_cast<const uint8_t*>(hugeMessageBuffer);
    while (numBytesSent < k_SizeOfHugeMessageBuffer) {
      const uint32_t bytesLeftToSend = Anki::Util::numeric_cast<uint32_t>(
          k_SizeOfHugeMessageBuffer - numBytesSent);
      const uint32_t bytesToSend = (bytesLeftToSend < k_BytesPerMessage)
                                       ? bytesLeftToSend
                                       : k_BytesPerMessage;
      host1._reliableTransport.SendData(true, client2.GetTransportAddress(),
                                        &messageBytes[numBytesSent],
                                        bytesToSend);
  ...
  printf(
      "MultipleLargePacketTest: Simulated %u packets dropped total. (host1: "
      "%u, client2: %u)\n",
      totalPacketsDropped, host1._emulatorSocket.GetNumPacketsDropped(),
      client2._emulatorSocket.GetNumPacketsDropped());
  ...
  printf(
      "MultipleLargePacketTest: Simulated %u packets arrived out of order "
      "requiring resends. (host1: %u, client2: %u)\n",
      totalOutOfOrder, outOfOrderHost1, outOfOrderClient2);
...
static void MixUnreliableAndReliableTest() {
  ...
  uint8_t smallUnreliableMessage[k_NumUnreliableBytes] = {'U', 'N', 'R',
                                                          'E', 'L', 'T'};
  ...
  const size_t k_BytesPerMessage = 250;
  // must send less unreliable bytes than 1 reliable message to ensure we don't
  // break the wait until all reliable have arrived
  const size_t maxUnreliableSends =
      (k_BytesPerMessage / k_NumUnreliableBytes) - 1;
  {
    size_t numBytesSent = 0;
    const uint8_t* messageBytes =
        reinterpret_cast<const uint8_t*>(hugeMessageBuffer);
    while (numBytesSent < k_SizeOfHugeMessageBuffer) {
      ...
      if (numUnreliableSends < maxUnreliableSends) {
        host1._reliableTransport.SendData(false, client2.GetTransportAddress(),
                                          smallUnreliableMessage,
                                          sizeof(smallUnreliableMessage));
        ++numUnreliableSends;
      }
    }
  }
  ...
  ASSERT_GT(client2._dataReceiver.GetNumBytesReceived(),
            k_SizeOfHugeMessageBuffer - 1);

  const size_t extraBytesReceived =
      client2._dataReceiver.GetNumBytesReceived() - k_SizeOfHugeMessageBuffer;
  const size_t numUnreliableReceived =
      extraBytesReceived / k_NumUnreliableBytes;

  // can't guarentee any unreliable messages are received given random packet
  // loss (especially with cozmo settings that tend to put them all in 1 packet)
  // ASSERT_GT( numUnreliableReceived, 0 ); // need at least some unreliable
  // messages to make it through...
  ASSERT_EQ(numUnreliableReceived * k_NumUnreliableBytes,
            extraBytesReceived);  // should be exact multiple

  if (client2._dataReceiver.GetNumBytesReceived() >=
      k_SizeOfHugeMessageBuffer) {
    const std::vector<uint8_t>& clientBytes =
        client2._dataReceiver.LockAndGetReceivedBytes();
  ...
  printf(
      "MixUnreliableAndReliableTest: Simulated %u packets dropped total. "
      "(host1: %u, client2: %u)\n",
      totalPacketsDropped, host1._emulatorSocket.GetNumPacketsDropped(),
      client2._emulatorSocket.GetNumPacketsDropped());
  ...
  printf(
      "MixUnreliableAndReliableTest: Simulated %u packets arrived out of order "
      "requiring resends. (host1: %u, client2: %u). %zu unreliable arrived out "
      "of %zu\n",
      totalOutOfOrder, outOfOrderHost1, outOfOrderClient2,
      numUnreliableReceived, numUnreliableSends);
