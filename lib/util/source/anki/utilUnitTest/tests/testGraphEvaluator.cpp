...
TEST(GraphEvaluator2d, TwoNodesUp) {
  // A 2-node graph should always return the y value for [0] or [1] if out of
  // bounds, and lerped otherwise
  ...
  Anki::Util::GraphEvaluator2d testGraph(
      {{kNodeValX1, kNodeValY1}, {kNodeValX2, kNodeValY2}});
  ...
  const float expectedSlope =
      (kNodeValY2 - kNodeValY1) / (kNodeValX2 - kNodeValX1);
...
TEST(GraphEvaluator2d, TwoNodesSameX) {
  // A 2-node graph should always return the y value for [0] or [1] if out of
  // bounds, and lerped otherwise where 2 neighboring nodes share the same x
  // value, the y value will never be lerped between them and will instead jump
  // from the 1st node to the 2nd once x becomes > than the x value.
  ...
  Anki::Util::GraphEvaluator2d testGraph(
      {{kNodeValX, kNodeValY1}, {kNodeValX, kNodeValY2}});
  ...
  EXPECT_FLOAT_EQ(xFound, 0.5f);
  EXPECT_EQ(
      testGraph.FindFirstX(15.3f, xFound),
      false);  // (both nodes are on same x so interval y values are ignored)
  EXPECT_EQ(testGraph.FindFirstX(60.3f, xFound), true);
  EXPECT_FLOAT_EQ(xFound, 0.5f);
  EXPECT_EQ(testGraph.FindFirstX(60.4f, xFound), false);

  EXPECT_FLOAT_EQ(testGraph.GetSlopeAt(kNodeValX),
                  std::numeric_limits<float>::max());
}

TEST(GraphEvaluator2d, EightNodes) {
  Anki::Util::GraphEvaluator2d testGraph({{-5.0f, -10.0f},
                                          {-3.0f, -2.0f},
                                          {-1.0f, 0.0f},
                                          {-1.0f, 100.0f},
                                          {1.0f, -100.0f},
...
TEST(GraphEvaluator2d, SlopeEdgeCase) {
  Anki::Util::GraphEvaluator2d testGraph(
      {{0.0f, 0.0f}, {1.0f, 1.0f}, {1.0f, 100.0f}});

  EXPECT_FLOAT_EQ(testGraph.GetSlopeAt(0.5f), 1.0f);
  EXPECT_FLOAT_EQ(testGraph.GetSlopeAt(1.0f),
                  1.0f);  // slope should not be infinite
...
TEST(GraphEvaluator2d, BadDataNodeOrder) {
  ...
  Anki::Util::GraphEvaluator2d testGraph(
      {{0.0f, 10.0f}, {-1.0f, 8.0f}, {1.0f, 1000.0f}}, false);
...
TEST(GraphEvaluator2d, MoveConstructor) {
  ...
  Anki::Util::GraphEvaluator2d testGraph2(
      std::move(testGraph));  // Move Constructor

  // testBuff is now probably empty (but not guarenteed), testBuff2 should be
  // equivalent to old testBuff

  // EXPECT_EQ(testGraph.GetNumNodes(), 0); // can't guarentee this as it
  // depends on std::vector implementation (but works in XCode and helps verify
  // that we're invoking the move version)
  ...
  // check we can reserve testGraph again and add to it without it affecting
  // testGraph2
...
TEST(GraphEvaluator2d, CopyConstructor) {
  ...
  // testBuff and testBuff2 should now be equivalent (although independent if
...
TEST(GraphEvaluator2d, RoundTripJson) {
  Anki::Util::GraphEvaluator2d testGraph({{-5.0f, 0.0f},
                                          {0.0f, -10.0f},
                                          {1.5f, 2.0f},
                                          {2.0f, 123.456f},
                                          {20.0f, 50.0f}});
...
TEST(GraphEvaluator2d, EqualityOperators) {
  ...
  Anki::Util::GraphEvaluator2d testGraph1(
      {{-5.0f, -10.0f}, {-3.0f, -2.0f}, {-1.0f, 0.0f}, {-1.0f, 100.0f}});
