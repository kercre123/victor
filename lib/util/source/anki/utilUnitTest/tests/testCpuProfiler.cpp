...
void SubFuncQuick() { ANKI_CPU_PROFILE("SubFuncQuick"); }
...
void FakeMainConfig(
    double maxTickTime = FLT_MAX,
    uint32_t logFrequency = Anki::Util::CpuThreadProfiler::kLogFrequencyNever) {
...
void FakeMainA() {
  ANKI_CPU_TICK("MainThreadA", FLT_MAX,
                Anki::Util::CpuThreadProfiler::kLogFrequencyNever);
  ANKI_CPU_PROFILE("FakeMainA");
  SubFuncA();
}

void FakeMainB() {
  ANKI_CPU_TICK("MainThreadB", FLT_MAX,
                Anki::Util::CpuThreadProfiler::kLogFrequencyNever);
...
void FakeMainC() {
  ANKI_CPU_TICK("MainThreadC", FLT_MAX,
                Anki::Util::CpuThreadProfiler::kLogFrequencyNever);
...
void FakeMainD() {
  ANKI_CPU_TICK("MainThreadD", FLT_MAX,
                Anki::Util::CpuThreadProfiler::kLogFrequencyNever);
...
const char* SafeGetSampleName(const Anki::Util::CpuThreadProfile& profile,
                              size_t index) {
...
TEST(CpuProfiler, OneTickProfile) {
  ...
  cpuProfiler.Reset();
  // Don't throw any samples out based on time (otherwise results depend on
  // machine speed)
  Anki::Util::CpuThreadProfiler::SetMinSampleDuration_ms(-0.01);

  FakeMainA();

  Anki::Util::CpuThreadProfiler* threadProfMain =
      cpuProfiler.GetThreadProfiler(Anki::Util::GetCurrentThreadId());

  ASSERT_NE(threadProfMain, nullptr);

  EXPECT_EQ(cpuProfiler.GetThreadProfilerByName("maiNtHReadA"),
            threadProfMain);  // Case insensitive lookup

  const Anki::Util::CpuThreadProfile& profile =
      threadProfMain->GetCurrentProfile();
...
TEST(CpuProfiler, OneSlowTickProfile) {
  ...
  FakeMainConfig(0.0f);  // It's guarenteed to take longer than 0ms, so this
                         // should trigger a slow log
...
TEST(CpuProfiler, MultiThreaded) {
  ...
  // Have to ask for thread ids immediately (before the thread ends and clears
  // them)
  ...
  Anki::Util::CpuThreadProfiler* threadProfA =
      cpuProfiler.GetThreadProfiler(threadA_id);
  Anki::Util::CpuThreadProfiler* threadProfB =
      cpuProfiler.GetThreadProfiler(threadB_id);
  Anki::Util::CpuThreadProfiler* threadProfC =
      cpuProfiler.GetThreadProfiler(threadC_id);
  Anki::Util::CpuThreadProfiler* threadProfD =
      cpuProfiler.GetThreadProfiler(threadD_id);
