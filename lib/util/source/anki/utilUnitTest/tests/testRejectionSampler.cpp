...
TEST(RejectionSamplerHelper, Test) {
  ...
  auto rejectZeroHandle =
      sampler.AddCondition([](const Type& sample) { return sample != 0; });
  ...
  // scoped condition
  {
    auto handle = sampler.AddScopedCondition(
        [](const Type& sample) { return sample != 1; });
    EXPECT_FALSE(sampler.Evaluate(rng, 0));
  ...
  auto rejectFiveHandle =
      sampler.AddCondition(std::make_shared<ConditionRejectOnly>(5));
  ...
  // scoped condition
  {
    ...
    auto rejectTwoHandle = sampler.AddScopedCondition(
        std::make_shared<ConditionRejectOnly>(2), rejectFiveHandle);
