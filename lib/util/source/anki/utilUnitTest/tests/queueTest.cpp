...
TEST(QueueTest, DISABLED_ScheduledCallbacks) {
  ...
  Anki::Util::TaskHandle handle = Anki::Util::Dispatch::ScheduleCallback(
      queue, std::chrono::milliseconds(500), [&a] { a++; });
...
TEST(QueueTest, DISABLED_RemoveCallback) {
  ...
  volatile int a = 0;
  Anki::Util::TaskHandle handle = Anki::Util::Dispatch::ScheduleCallback(
      queue, std::chrono::milliseconds(250), [&a] { a++; });
...
// VIC-492: Disabled because test does not pass reliably
//
// test that invalidating a handle prevents it from executing again, even if
// it's already queued to run on the main queue (like, it's already been moved
// from the deferred queue to the main queue because its time to execute passed
// while a task was running that invalidates the handle)
TEST(QueueTest, DISABLED_ReverseTaskHeartbeat) {
  class TestClass {
   public:
    TestClass(volatile int& counter)
        : _counter(counter), _pointer(new std::string("stuff")) {}
    ~TestClass() {
    ...
    void Update() {
      std::cout << "QueueTest.ReverseTaskHeartbeat: " << *_pointer;
      _counter++;
    }
  ...
  Queue* queue = Anki::Util::Dispatch::Create();
  obj->SetTaskHandle(Anki::Util::Dispatch::ScheduleCallback(
      queue, std::chrono::milliseconds(100), [obj] { obj->Update(); }));

  std::this_thread::sleep_for(std::chrono::milliseconds(120));
  EXPECT_EQ(1, counter);

  // now add a task that will sleep for long enough that the above task will be
  ...
  // sleep for long enough that if the repeated task is on the main queue, it
  // will execute and crash
