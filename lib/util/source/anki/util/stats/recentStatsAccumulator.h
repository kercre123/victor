 ...
 * Created: 06/25/15
 *
 * Description: wraps 2 offset statsAccumulators that are reset every N samples,
 *and offset by N/2 from each other so you can always calculate from N/2..N
 *recent samples
 *
...
class RecentStatsAccumulator {
 public:
  ...
  const StatsAccumulator& GetPrimaryAccumulator() const {
    return (_stats[1].GetNumDbl() > _stats[0].GetNumDbl()) ? _stats[1]
                                                           : _stats[0];
  }
