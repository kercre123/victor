...
CONSOLE_VAR_RANGED(float, kMessageProfilerDuration, "CpuProfiler", 0.f, 0.f,
                   60.f * 60.f);
...
MessageProfiler::MessageProfiler(const std::string& prefix)
    : m_prefix(prefix), m_start(clock()), m_started(false), m_failed(false) {
...
void MessageProfiler::Report() {
  ...
  for (int i = 0; i < kMaxMessages; ++i) {
    if (m_count[i] > 0) {
      LOG_INFO(
          m_prefix.c_str(),
          "Tag: 0x%02X %0.2f messages/s = %zu bytes/message (%0.2f bytes/sec)",
          i, m_count[i] / duration, m_size[i],
