...
static int ThreadPriorityToIntPriority(int policy,
  ...
  switch (threadPriority) {
    ...
    case ThreadPriority::Default:
      // We don't know what the default was, assume medium (we shouldn't usually
      // get here as we only set for non-default)
...
void SetThreadPriority(pthread_t inThread, ThreadPriority threadPriority) {
  ...
  if (ANKI_DEVELOPER_CODE) {
    ...
    if (res != 0) {
      PRINT_NAMED_ERROR("GetThreadPriority.Failed",
                        "Error: %s (res=%d) getting thread priority and policy",
                        std::strerror(errno), res);
    } else {
  ...
  if (res != 0) {
#if defined(ANDROID)
    // Currently, on Android, we expect pthread_setschedparam to fail with
    // EPERM. A regular (non-root) process cannot change thread priority with
    ...
    PRINT_NAMED_INFO("SetThreadPriority.Success",
                     "Changed thread policy:priority from %d:%d to %d:%d)",
                     oldPolicy, oldPriority, policy, priority);
  }
}
...
bool SetThreadName(std::thread::native_handle_type inThread,
                   const char* threadName) {
  if (threadName == nullptr) {
    return false;
  }

  // The thread name length is restricted to 16 characters,including the
  // terminating null byte ('\0'). A C string is as long as the number of
  // characters between the beginning of the string and the terminating null
  // character (without including the terminating null character itself).
  ASSERT_NAMED(strlen(threadName) < kMaxThreadNameLength,
               "SetThreadPriority.NameTooLong");

  // RETURN VALUE - On success, these functions return 0; on error, they return
  ...
  if (ANKI_VERIFY(pthread_self() == inThread,
                  "SetThreadPriority.NotCalledFromTargetThread",
...
bool SetThreadName(std::thread::native_handle_type inThread,
  ...
  return SetThreadName(inThread,
                       threadName.substr(0, kMaxThreadNameLength - 1).c_str());
}
