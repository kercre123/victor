...
class QueuedUDPPacket {
 public:
  ...
  QueuedUDPPacket();
  QueuedUDPPacket(ssize_t messageSize, const msghdr* messageHeader,
  ...
  void Set(ssize_t messageSize, const msghdr* messageHeader,
           QueuedUDPPacket::DeliveryTime deliveryTime);
...
class UDPSocketQueue {
 public:
  ...
  void QueueMessage(ssize_t messageSize, const msghdr* messageHeader,
                    QueuedUDPPacket::DeliveryTime deliveryTime);
...
class NetEmulatorUDPSocket : public IUDPSocket {
 public:
  ...
  explicit NetEmulatorUDPSocket(uint32_t randSeed = 0,
                                float inRandomPacketLossPercentage = -1.0f,
                                uint32_t minLatencyInMS = 0,
                                uint32_t maxLatencyInMS = 0);
  virtual ~NetEmulatorUDPSocket();

  virtual int OpenSocket(int protocolFamily, int socketType,
                         int protocol) override;
  virtual int BindSocket(int socketId, const sockaddr* socketAddress,
                         socklen_t socketAddressLength) override;
  virtual int GetSocketOpt(int socketId, int level, int optname, void* optVal,
                           socklen_t* optlen) override;
  virtual int SetSocketOpt(int socketId, int level, int optname,
                           const void* optVal, socklen_t optlen) override;
  virtual int CloseSocket(int socketId) override;
  virtual ssize_t SendTo(int socketId, const void* messageData,
                         size_t messageDataSize, int flags,
                         const sockaddr* destSockAddress,
                         socklen_t destSockAddressLength) override;
  virtual ssize_t ReceiveMessage(int socketId, msghdr* messageHeader,
                                 int flags) override;
 ...
 private:
  ...
  float _randomPacketLossPercentage;  // <0 = no random packet loss, 100 = all
                                      // packets lost
