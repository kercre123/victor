...
PendingMessage::~PendingMessage() { DestroyMessage(); }

void PendingMessage::Set(const SrcBufferSet& srcBuffers,
  ...
  // seqId == invalid implies unreliable message, this must be in sync with
  // reliability of type (otherwise multi-messages will incorrectly determine if
  // that message contributes to the seqId increment)
  assert((seqId == k_InvalidReliableSeqId) ==
         IsMessageTypeAlwaysSentUnreliably(messageType));

  ANKI_NET_MESSAGE_VERBOSE(
      ("ReliableMessage.Set", "", _message, _messageSize, &srcBuffers));
}
