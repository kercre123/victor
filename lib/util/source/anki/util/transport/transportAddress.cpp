...
TransportAddress::TransportAddress() : _type(Type::None) {}
...
TransportAddress::TransportAddress(const struct in6_addr& ipv6Address,
                                   uint16_t port) {
...
TransportAddress::TransportAddress(const std::string& addressAsString,
                                   int32_t port) {
...
void TransportAddress::SetIPv6Address(const struct in6_addr& ipv6Address,
                                      uint16_t port) {
  _type = Type::IPv6;
...
void TransportAddress::SetIPAddressFromString(const char* addressAsString,
                                              int32_t port) {
  if (port < 0 || port >= 0x10000) {
    PRINT_NAMED_WARNING("TransportAddress.PortOutOfRange",
                        "IP address %s specified with bad port %d",
                        addressAsString, port);
    _type = Type::None;
    return;
  }
  ...
  if (1 == rc) {
    ...
    PRINT_NAMED_WARNING("TransportAddress",
                        "inet_pton(AF_INET6, %s) returned errno = %d",
  ...
  if (1 == rc) {
    ...
    PRINT_NAMED_WARNING("TransportAddress",
                        "inet_pton(AF_INET, %s) returned errno = %d",
                        addressAsString, errno);
  }
  PRINT_NAMED_WARNING("TransportAddress", "inet_pton failed to parse %s",
                      addressAsString);
  _type = Type::None;
  return;
}

void TransportAddress::SetIPAddressFromString(
    const std::string& addressAsString, int32_t port) {
  SetIPAddressFromString(addressAsString.c_str(), port);
}
...
bool TransportAddress::IsIPAddress() const { return _type == Type::IP; }

bool TransportAddress::IsIPv6Address() const { return _type == Type::IPv6; }

bool TransportAddress::IsVirtualAddress() const {
  return _type == Type::Virtual;
}

bool TransportAddress::IsBLEAddress() const { return _type == Type::BLE; }
...
std::string TransportAddress::ToString() const {
  if (IsVirtualAddress()) {
    ...
    return std::string(
        std::to_string(ipBytes[0]) + "." + std::to_string(ipBytes[1]) + "." +
        std::to_string(ipBytes[2]) + "." + std::to_string(ipBytes[3]) + ":" +
        std::to_string(_address.ip.port));
  } else if (IsIPv6Address()) {
    char ipv6_buf[INET6_ADDRSTRLEN];
    const char* ipv6_str =
        inet_ntop(AF_INET6, &(_address.ipv6.addr), ipv6_buf, sizeof(ipv6_buf));
  ...
  switch (_type) {
    ...
    case Type::IP:
      return (_address.ip.addr == rhs._address.ip.addr &&
              _address.ip.port == _address.ip.port);
    case Type::IPv6:
      return (IN6_ARE_ADDR_EQUAL(&(_address.ipv6.addr),
                                 &(rhs._address.ipv6.addr)) &&
...
static inline int in6_addr_compare(const struct in6_addr* a,
                                   const struct in6_addr* b) {
  return memcmp(&a->s6_addr[0], &b->s6_addr[0], sizeof(struct in6_addr));
}

static inline bool in6_is_addr_less(const struct in6_addr* a,
                                    const struct in6_addr* b) {
...
bool TransportAddress::operator<(const TransportAddress& rhs) const {
  if (_type < rhs._type) {
    ...
    // Only compare the union elements relevant for the type - otherwise might
    // be invalid / uninitialized
    switch (_type) {
      case Type::None:
        return false;
      case Type::IP:
        return ((_address.ip.addr < rhs._address.ip.addr) ||
                ((_address.ip.addr == rhs._address.ip.addr) &&
                 (_address.ip.port < rhs._address.ip.port)));
      case Type::IPv6:
        return ((in6_is_addr_less(&(_address.ipv6.addr),
                                  &(rhs._address.ipv6.addr))) ||
                ((IN6_ARE_ADDR_EQUAL(&(_address.ipv6.addr),
                                     &(rhs._address.ipv6.addr)) &&
...
std::size_t TransportAddress::GetHash() const {
  ...
  switch (_type) {
    case Type::None:
      break;
    case Type::IP:
      result = (static_cast<std::uint64_t>(_address.ip.port) << 32) |
               _address.ip.addr;
      break;
    case Type::IPv6:
