...
CONSOLE_VAR_RANGED(uint32_t, kPrintNetworkStatsTimeSpacingMS, "Network", 1000,
                   0, 10000);

// ============================== Packet Headers
// ========================================

// Packet headers for ensuring that the messages are of the correct type. (Used
// for unreliable messages sent via this transport too)
static const uint8_t k_AnkiReliablePacketHeaderPrefix[3] = {
    'R', 'E', 01};  // i.e. "Reliable Transport Layer 1"

struct AnkiReliablePacketHeader {
  explicit AnkiReliablePacketHeader(uint8_t messageType,
  ...
  void Set(uint8_t messageType, ReliableSequenceId seqIdMin,
           ReliableSequenceId seqIdMax, ReliableSequenceId lastReceivedId) {
  ...
  bool IsReliable() const {
    return ((_seqIdMin != k_InvalidReliableSeqId) ||
            (_seqIdMax != k_InvalidReliableSeqId));
  }
...
static_assert(sizeof(AnkiReliablePacketHeader) == 10,
              "Expected size mismatch, check layout and padding");

#define RELIABLE_HEADER_DESCRIPTOR_TEXT "3cPrefix|Ty|SqMin|SqMax|SqRec|"
static const char* k_ReliableHeaderDescriptor =
    SIZED_SRC_BUFFER_DESCRIPTOR(RELIABLE_HEADER_DESCRIPTOR_TEXT);
...
ReliableTransport::ReliableTransport(IUnreliableTransport* unreliableTransport,
...
void ReliableTransport::KillThread() { _queue.reset(); }

uint32_t ReliableTransport::MaxTotalBytesPerMessage() const {
  return _unreliable->MaxTotalBytesPerMessage() -
         sizeof(AnkiReliablePacketHeader);
}

void ReliableTransport::ReSendReliableMessage(
    ReliableConnection* connectionInfo, const uint8_t* buffer,
    unsigned int bufferSize, EReliableMessageType messageType,
    ReliableSequenceId seqIdMin, ReliableSequenceId seqIdMax) {
  uint8_t headerBuffer[sizeof(AnkiReliablePacketHeader)];
  const uint32_t headerSize =
      BuildHeader(headerBuffer, sizeof(headerBuffer), messageType, seqIdMin,
                  seqIdMax, connectionInfo->GetLastInAckedMessageId());

  SrcBufferSet srcBuffers;
  srcBuffers.AddBuffer(
      SizedSrcBuffer(headerBuffer, headerSize, k_ReliableHeaderDescriptor));
  srcBuffers.AddBuffer(SizedSrcBuffer(
      buffer, bufferSize, SIZED_SRC_BUFFER_DESCRIPTOR("Message...")));
...
void ReliableTransport::QueueMessage(bool reliable,
                                     const TransportAddress& destAddress,
                                     const uint8_t* buffer,
                                     unsigned int bufferSize,
                                     EReliableMessageType messageType,
                                     bool flushPacket) {
  if (_runSynchronous) {
    SendMessage(reliable, destAddress, buffer, bufferSize, messageType,
                flushPacket);
  } else {
    ...
    QueueAction([this, reliable, destAddress, newBuffer, bufferSize,
                 messageType, flushPacket, queuedTime] {
      SendMessage(reliable, destAddress, newBuffer, bufferSize, messageType,
                  flushPacket, queuedTime);
      delete[] newBuffer;
    });
  }
}
...
void ReliableTransport::SendMessage(bool reliable,
                                    const TransportAddress& destAddress,
                                    const uint8_t* buffer,
                                    unsigned int bufferSize,
                                    EReliableMessageType messageType,
  ...
  ReliableConnection* connectionInfo =
      FindConnection(destAddress, isConnectRequest);
  if (nullptr == connectionInfo) {
    PRINT_NAMED_WARNING(
        "ReliableTransport.SendMessage.NullConnection",
        "Trying to send a normal message to an unconnected destination! %s",
        destAddress.ToString().c_str());
    return;
  }

  // See if we need to split the message into multiple smaller pieces and
  // reassemble on the other end

  const uint32_t maxPayloadPerMessage = MaxTotalBytesPerMessage();
  const uint32_t maxPayloadPerMultiPartMessage =
      maxPayloadPerMessage -
      k_MultiPartMessageHeaderSize;  // for multi part tracking

  const bool isMultiPartMessage = (bufferSize > maxPayloadPerMessage);
  uint32_t bufferSizeSent = 0;

  if (isMultiPartMessage && !reliable) {
    PRINT_NAMED_WARNING("ReliableTransport.SendMessage.UnreliableMultiPart",
                        "Had to split %u byte unreliable message (> %u bytes) "
                        "- forcing it to be reliable so it can be reassembled!",
                        bufferSize, maxPayloadPerMessage);
    reliable = true;
  }

  if (isMultiPartMessage) {
    assert((messageType == eRMT_SingleReliableMessage) ||
           (messageType == eRMT_SingleUnreliableMessage) ||
  ...
  const uint32_t numMessagesToSend =
      !isMultiPartMessage
          ? 1
          : ((bufferSize + (maxPayloadPerMultiPartMessage - 1)) /
             maxPayloadPerMultiPartMessage);
  assert(numMessagesToSend > 0);

  const bool unreliableAndSendImmediately =
      !reliable && sSendUnreliableMessagesImmediately;

  do {
    const uint8_t* messageBuffer =
        (buffer != nullptr) ? &buffer[bufferSizeSent] : nullptr;
    ...
    const ReliableSequenceId seqId =
        reliable ? connectionInfo->GetNextOutSequenceNumber()
                 : k_InvalidReliableSeqId;

    SrcBufferSet srcBuffers;
    if (unreliableAndSendImmediately) {
      // only need header if we're sending immediately (i.e. unreliably)
      uint32_t headerSize =
          BuildHeader(headerBuffer, sizeof(headerBuffer), messageType, seqId,
                      seqId, connectionInfo->GetLastInAckedMessageId());
      srcBuffers.AddBuffer(
          SizedSrcBuffer(headerBuffer, headerSize, k_ReliableHeaderDescriptor));
    }
    ...
    if (isMultiPartMessage) {
      ...
      srcBuffers.AddBuffer(
          SizedSrcBuffer(multiMessageHeader, sizeof(multiMessageHeader),
                         SIZED_SRC_BUFFER_DESCRIPTOR("Mu|Mu|")));
    }

    if (messageBufferSize > 0) {
      srcBuffers.AddBuffer(
          SizedSrcBuffer(messageBuffer, messageBufferSize,
                         SIZED_SRC_BUFFER_DESCRIPTOR("Message...")));
    }
    ...
    if (unreliableAndSendImmediately) {
      ...
      // Store message (incase it needs to re-send it later) (without header,
      // but with any multi part info)
  ...
  if (!unreliableAndSendImmediately && (sMaxPacketsToSendOnSendMessage > 0)) {
    // we queued up some messages - give connection a chance to send some of
    // them
    connectionInfo->SendOptimalUnAckedPackets(this,
                                              sMaxPacketsToSendOnSendMessage);
  }
}

void ReliableTransport::SendData(bool reliable,
                                 const TransportAddress& destAddress,
                                 const uint8_t* buffer, uint32_t bufferSize,
                                 bool flushPacket) {
  const EReliableMessageType messageType =
      reliable ? eRMT_SingleReliableMessage : eRMT_SingleUnreliableMessage;
  QueueMessage(reliable, destAddress, buffer, bufferSize, messageType,
               flushPacket);
}
...
unsigned int ReliableTransport::FillAddressFromAdvertisement(
    TransportAddress& address, const uint8_t* buffer, unsigned int size) {
  return _unreliable->FillAddressFromAdvertisement(address, buffer, size);
}

void ReliableTransport::HandleSubMessage(
    const uint8_t* innerMessage, uint32_t innerMessageSize, uint8_t messageType,
    ReliableSequenceId reliableSequenceId, ReliableConnection* connectionInfo,
    const TransportAddress& sourceAddress) {
  const bool isReliable = (reliableSequenceId != k_InvalidReliableSeqId);
  const bool isNextReliable =
      isReliable && connectionInfo->IsNextInSequenceId(reliableSequenceId);
  const bool ignoreMessage =
      isReliable && !isNextReliable;  // message is a repeat or out of order

  connectionInfo->AddRecvMessageStats(innerMessageSize, isReliable,
                                      !ignoreMessage);
  ...
  switch (messageType) {
    case eRMT_ConnectionRequest:
      if (_dataReceiver) {
        ...
        _dataReceiver->ReceiveData(INetTransportDataReceiver::OnConnectRequest,
                                   0, sourceAddress);
      }
      break;
    case eRMT_ConnectionResponse:
      if (_dataReceiver) {
        ...
        _dataReceiver->ReceiveData(INetTransportDataReceiver::OnConnected, 0,
                                   sourceAddress);
      }
      break;
    case eRMT_DisconnectRequest:
      if (_dataReceiver) {
        _dataReceiver->ReceiveData(INetTransportDataReceiver::OnDisconnected, 0,
                                   sourceAddress);
      }
    ...
    case eRMT_SingleUnreliableMessage:
      if (_dataReceiver) {
        _dataReceiver->ReceiveData(innerMessage, innerMessageSize,
                                   sourceAddress);
      }
      break;
    case eRMT_MultiPartMessage: {
      PendingMultiPartMessage& pendingMultiPart =
          connectionInfo->GetPendingMultiPartMessage();
      if (pendingMultiPart.AddMessagePart(innerMessage, innerMessageSize)) {
        // Pass the completed re-assembled message on, and remove it.

        ANKI_NET_MESSAGE_VERBOSE(("Multi Complete", "Message...",
                                  pendingMultiPart.GetBytes(),
                                  pendingMultiPart.GetSize()));

        if (_dataReceiver) {
          _dataReceiver->ReceiveData(pendingMultiPart.GetBytes(),
                                     pendingMultiPart.GetSize(), sourceAddress);
        }
    ...
    case eRMT_MultipleMixedMessages:
      assert(0);  // should be handled purely at ReceiveData, and never embedded
                  // as a sub-message
      break;
    case eRMT_ACK:
      // Ack id is already handled for all messages from the header earlier -
      // nothing left to do!
      break;
    case eRMT_Ping:
      ...
      break;
    default:
      PRINT_NAMED_ERROR("ReliableTransport.Sub.BadType",
                        "unknown reliable message type: %u", messageType);
      break;
  }
}

void ReliableTransport::ReceiveData(const uint8_t* buffer, unsigned int size,
                                    const TransportAddress& sourceAddress) {
  bool handledMessageType = false;

  // see if this is a message for our layer specifically
  if (size >= sizeof(AnkiReliablePacketHeader)) {
    const AnkiReliablePacketHeader* reliablePacketHeader =
        reinterpret_cast<const AnkiReliablePacketHeader*>(buffer);

    if (reliablePacketHeader->HasCorrectPrefix()) {
      ANKI_NET_MESSAGE_VERBOSE(
          ("Rel Recv", "|" RELIABLE_HEADER_DESCRIPTOR_TEXT "Message...", buffer,
           size));
      ...
      const ReliableSequenceId minSeqId =
          isReliable ? reliablePacketHeader->GetSequenceIdMin()
                     : k_InvalidReliableSeqId;
      const ReliableSequenceId maxSeqId =
          isReliable ? reliablePacketHeader->GetSequenceIdMax()
                     : k_InvalidReliableSeqId;

      uint32_t innerMessageSize = size - sizeof(AnkiReliablePacketHeader);
      const uint8_t* innerMessage =
          (innerMessageSize > 0) ? &buffer[sizeof(AnkiReliablePacketHeader)]
                                 : nullptr;

      const bool isMultipleMessages =
          IsMutlipleMessagesType((EReliableMessageType)messageType);

      bool isConnectRequest = messageType == eRMT_ConnectionRequest;
      if (!isConnectRequest) {
        // is it multiple messages where the first one is a connect request?
        isConnectRequest = isMultipleMessages && (innerMessage != nullptr) &&
                           (innerMessageSize > 0) &&
                           (*innerMessage == eRMT_ConnectionRequest);
      }

      // Only create a new connection if this is a connect request
      ReliableConnection* connectionInfo =
          FindConnection(sourceAddress, isConnectRequest);
      if (nullptr == connectionInfo) {
        PRINT_NAMED_WARNING(
            "ReliableTransport.Recv.Unconnected",
            "Received %s message from unconnected source! %s",
            ReliableMessageTypeToString((EReliableMessageType)messageType),
            sourceAddress.ToString().c_str());
        return;
      }
      ...
      if (connectionInfo->UpdateLastAckedMessage(
              reliablePacketHeader->GetSequenceIdLastReceived())) {
        // the list of reliable messages was updated - give connection a chance
        // to send latest reliable messages now
        if (sMaxPacketsToReSendOnAck > 0) {
          connectionInfo->SendOptimalUnAckedPackets(this,
                                                    sMaxPacketsToReSendOnAck);
        }
      }
      ...
      if (isReliable) {
        anyMessagesToReceive =
            connectionInfo->IsWaitingForAnyInRange(minSeqId, maxSeqId);

        if (!anyMessagesToReceive) {
          // If it's multiple mixed messages we still need to process any
          // unreliable messages within
          if (messageType == eRMT_MultipleMixedMessages) {
            anyMessagesToReceive = true;
          } else {
            _transportStats.AddRecvError(eME_OutOfOrder);
            ANKI_NET_PRINT_VERBOSE("ReliableTransport.Recv.OutOfOrder",
                                   "ReceiveData - Ignoring out of order "
                                   "messages %u..%u (waiting for %u)",
                                   minSeqId, maxSeqId,
                                   connectionInfo->GetNextInSequenceId());

#if !ENABLE_RC_PACKET_TIME_DIAGNOSTICS
            {
              // We only early-out if we're not interestd on the stats on each
              // message
              return;
            }
      ...
      if (isMultipleMessages) {
        ...
        while (bytesProcessed < innerMessageSize) {
          uint8_t subMessageType =
              *reinterpret_cast<const uint8_t*>(&innerMessage[bytesProcessed]);
          bytesProcessed += sizeof(subMessageType);
          // If we get a sub message that appears to have an invalid type, this
          // message is no longer valid. Toss it.
          if (!IsValidMessageType((EReliableMessageType)subMessageType)) {
            PRINT_NAMED_ERROR(
                "ReliableTransport.ReceiveData",
                "Invalid submessage type %u when processing multiple messages.",
                subMessageType);
            _transportStats.AddRecvError(eME_BadType);
            break;
          }
          uint16_t subMessageSize =
              *reinterpret_cast<const uint16_t*>(&innerMessage[bytesProcessed]);
          bytesProcessed += sizeof(subMessageSize);
          // If our sub message is too big, this message is no longer valid.
          // Toss it.
          if (bytesProcessed + subMessageSize > innerMessageSize) {
            PRINT_NAMED_ERROR(
                "ReliableTransport.ReceiveData",
                "Invalid submessage size %u when processing multiple messages.",
                subMessageSize);
          ...
          assert(nullptr !=
                 FindConnection(sourceAddress,
                                false));  // could theoretically change in loop,
                                          // but shouldn't in practice

          const bool isSubMessageReliable =
              isReliable && !IsMessageTypeAlwaysSentUnreliably(
                                (EReliableMessageType)subMessageType);
          const ReliableSequenceId subMessageSeqId =
              isSubMessageReliable ? reliableSequenceId
                                   : k_InvalidReliableSeqId;

          // Message

          const uint8_t* subMessageData =
              (subMessageSize == 0) ? nullptr : &innerMessage[bytesProcessed];
          bytesProcessed += subMessageSize;

          HandleSubMessage(subMessageData, subMessageSize, subMessageType,
                           subMessageSeqId, connectionInfo, sourceAddress);
        ...
        HandleSubMessage(innerMessage, innerMessageSize, messageType, minSeqId,
                         connectionInfo, sourceAddress);
      }
    } else {
      PRINT_NAMED_WARNING(
          "ReliableTransport.BadPrefix", "Header '%s' has wrong prefix!",
          ConvertMessageBufferToString(buffer, sizeof(AnkiReliablePacketHeader),
                                       Util::eBTTT_Ascii)
              .c_str());
    ...
    PRINT_NAMED_WARNING(
        "ReliableTransport.BadPrefix.TooSmall", "Header '%s' is too small!",
  ...
  if (!handledMessageType) {
    ...
    PRINT_NAMED_WARNING("ReliableTransport.Recv.Unhandled",
                        "Unknown/Unhandled type of message!");
...
uint32_t ReliableTransport::BuildHeader(uint8_t* outBuffer,
                                        uint32_t outBufferCapacity,
                                        uint8_t type,
                                        ReliableSequenceId seqIdMin,
                                        ReliableSequenceId seqIdMax,
                                        ReliableSequenceId lastReceivedId) {
  ASSERT_AND_RETURN_VALUE_IF_FAIL(
      outBufferCapacity >= sizeof(AnkiReliablePacketHeader), 0);

  AnkiReliablePacketHeader* packetHeader =
      reinterpret_cast<AnkiReliablePacketHeader*>(outBuffer);
...
void DumpUpdateStats(const Stats::StatsAccumulator& timesBetweenUpdates) {
  ...
  if (num > 0.0) {
    ...
    PRINT_CH_INFO(
        "Network", "ReliableTransport.UpdateStats",
        "UpdateTimes: %.1f samples, %.2f ms avg (%.2f sd) (%.2f..%.2f ms)", num,
        avg, sd, min, max);

  } else {
    PRINT_CH_INFO("Network", "ReliableTransport.UpdateStats",
                  "UpdateTimes: NO SAMPLES!");
  }
}
#endif  // ENABLE_RT_UPDATE_TIME_DIAGNOSTICS

void ReliableTransport::Update() {
  // lock on mutex to make sure nothing sends at the same time we receive data
  // or examine our connection data
  std::lock_guard<std::mutex> lock(_mutex);
  ...
  const NetTimeStamp updateStartTimeMs = GetCurrentNetTimeStamp();
  {
    if (_lastUpdateTime != kNetTimeStampZero) {
      ...
      // We expect ~2ms, but it's only a big issue if it seriously impacts our
      // ability to read and ack packets in a timely manner
      if (timeSinceLastUpdate > 15.0 && !_reliableConnectionMap.empty()) {
        const double timeSinceLastReport =
            updateStartTimeMs - _lastReportOfSlowUpdate;
        const double kMinTimeBetweenSlowSleepReports_ms = 60000.0;
        if (timeSinceLastReport > kMinTimeBetweenSlowSleepReports_ms) {
          Anki::Util::sChanneledInfoF(
              "Network", "reliable_transport.update.sleep.slow", {{DDATA, "2"}},
              "%.2f", timeSinceLastUpdate);
  ...
  for (ReliableConnectionMap::iterator it = _reliableConnectionMap.begin();
       it != _reliableConnectionMap.end();) {
    ReliableConnection* existingConnection = it->second;
    if (!existingConnection->Update(this)) {
      // Connection has timed out!
      // TODO - allow game to turn this off in lobby to workaround bug where BLE
      // scanning prevents all IP received on Android
      const bool disconnectOnTimeout = true;
      if (disconnectOnTimeout) {
        PRINT_CH_INFO("Network", "ReliableTransport.Update.ConnectionTimedOut",
                      "Disconnecting TimedOut Connection to '%s'!",
  ...
  if (kPrintNetworkStats) {
    ...
    if (std::chrono::duration_cast<std::chrono::milliseconds>(now -
                                                              sLastPrintTime)
            .count() >= Util::kPrintNetworkStatsTimeSpacingMS) {
      sLastPrintTime = now;
      Print();
    }
  }
...
#if ENABLE_RUN_TIME_PROFILING
  {
    ...
    if (_lastUpdateTime != kNetTimeStampZero) {
      const double timeSinceLastUpdate =
          currentTimePostUpdate - updateStartTimeMs;
      if (timeSinceLastUpdate > 2.0) {
        Anki::Util::sEventF("reliable_transport.update.run.slow",
...
void ReliableTransport::UpdateNetStats() {
  ...
  for (ReliableConnectionMap::const_iterator it =
           _reliableConnectionMap.begin();
       it != _reliableConnectionMap.end(); ++it) {
    ++numConnections;
    const ReliableConnection* existingConnection = it->second;

    {
      const Stats::StatsAccumulator& pingRoundTripStats =
          sTrackAckLatency ? existingConnection->GetAckRoundTripStats()
      ...
      if (pingRoundTripStats.GetNum() > 0) {
        ...
        latencyMin = (numPingStatsAdded == 0)
                         ? pingRoundTripStats.GetMin()
                         : Min(latencyMin, (float)pingRoundTripStats.GetMin());
        latencyMax = (numPingStatsAdded == 0)
                         ? pingRoundTripStats.GetMax()
                         : Max(latencyMax, (float)pingRoundTripStats.GetMax());
    ...
    {
      const Stats::StatsAccumulator& extQueuedTimeStats =
          existingConnection->GetExternalQueuedTimeStats();
      if (extQueuedTimeStats.GetNum() > 0) {
        extQueuedAvg_ms += extQueuedTimeStats.GetMean();
        extQueuedMin_ms =
            (numExtQueuedStatsAdded == 0)
                ? extQueuedTimeStats.GetMin()
                : Min(extQueuedMin_ms, (float)extQueuedTimeStats.GetMin());
        extQueuedMax_ms =
            (numExtQueuedStatsAdded == 0)
                ? extQueuedTimeStats.GetMax()
                : Max(extQueuedMax_ms, (float)extQueuedTimeStats.GetMax());
        ++numExtQueuedStatsAdded;
      }
    }

    {
      const Stats::StatsAccumulator& queuedTimeStats =
          existingConnection->GetQueuedTimeStats();
      if (queuedTimeStats.GetNum() > 0) {
        queuedAvg_ms += queuedTimeStats.GetMean();
        queuedMin_ms = (numQueuedStatsAdded == 0)
                           ? queuedTimeStats.GetMin()
                           : Min(queuedMin_ms, (float)queuedTimeStats.GetMin());
        queuedMax_ms = (numQueuedStatsAdded == 0)
                           ? queuedTimeStats.GetMax()
                           : Max(queuedMax_ms, (float)queuedTimeStats.GetMax());
        ++numQueuedStatsAdded;
      }
    }
  }

  // averaging the stats isn't ideal (especially SD...), but better than just
  // showing one value
...
#if ANKI_NET_MESSAGE_LOGGING_ENABLED
  PRINT_CH_INFO("Network", "ReliableTransport.Print", "NumConnections: %zu",
                _reliableConnectionMap.size());
...
ReliableConnection* ReliableTransport::FindConnection(
    const TransportAddress& sourceAddress, bool createIfNew) {
  ReliableConnectionMap::iterator it =
      _reliableConnectionMap.find(sourceAddress);
...
void ReliableTransport::DeleteConnection(
    const TransportAddress& sourceAddress) {
...
void ReliableTransport::ChangeSyncMode(bool isSync) {
  if (!_runSynchronous && isSync) {
    ...
    _updateHandle = Dispatch::ScheduleCallback(
        _queue.get(), std::chrono::milliseconds(2), [this] { Update(); });
  }
