 ...
 *              Fulfills same interface for a Posix/BSD style socket
 *implementation, but is faked internally to allow for easier testing of
...
FakeUDPPacketData::FakeUDPPacketData()
    : _message(), _socketAddressSrc(), _socketAddressDest() {}

FakeUDPPacketData::FakeUDPPacketData(const void* inBuffer, size_t inSize,
                                     const sockaddr* srcSockAddress,
                                     socklen_t srcSockAddressLength,
                                     const sockaddr* destSockAddress,
                                     socklen_t destSockAddressLength)
    : _message(), _socketAddressSrc(), _socketAddressDest() {
  Set(inBuffer, inSize, srcSockAddress, srcSockAddressLength, destSockAddress,
      destSockAddressLength);
}
...
void FakeUDPPacketData::Set(const void* inBuffer, size_t inSize,
  ...
  _socketAddressSrc.Set(
      srcSockAddress, Anki::Util::numeric_cast<uint32_t>(srcSockAddressLength));
  _socketAddressDest.Set(destSockAddress, Anki::Util::numeric_cast<uint32_t>(
                                              destSockAddressLength));
}
...
FakeUDPSocketInfo::~FakeUDPSocketInfo() { ClearQueues(); }
...
int FakeUDPSocketInfo::Open(int socketId, int protocolFamily, int socketType,
                            int protocol) {
  assert(!_isOpen);
...
int FakeUDPSocketInfo::Bind(const sockaddr* socketAddress,
  ...
  if (_isOpen) {
    if (_isBound) {
      ...
      const sockaddr_in* socketAddress_in =
...
int FakeUDPSocketInfo::GetSocketOpt(int level, int optname, void* optVal,
                                    socklen_t* optlen) {
  if (_isOpen) {
...
int FakeUDPSocketInfo::SetSocketOpt(int level, int optname, const void* optVal,
                                    socklen_t optlen) {
  if (_isOpen) {
    // TODO
    return 0;
...
ssize_t FakeUDPSocketInfo::SendTo(const void* messageData,
                                  ...
                                  socklen_t destSockAddressLength) {
  if (_isOpen) {
    _outMessageQueue.emplace_back(messageData, messageDataSize, srcSockAddress,
                                  srcSockAddressLength, destSockAddress,
                                  destSockAddressLength);
...
ssize_t FakeUDPSocketInfo::ReceiveMessage(msghdr* messageHeader, int flags) {
  if (_isOpen) {
    if (_inMessageQueue.size() > 0) {
      ...
      // Write out data, clamped to max available size in messageHeader if
      // applicable
...

      {
        ...
        while (bytesWritten < bytesToWrite) {
          ...
          const size_t bytesToWrite = (bytesLeftToWrite < ioVec.iov_len)
                                          ? bytesLeftToWrite
                                          : ioVec.iov_len;
      ...
      {
        const PacketByteArray& srcSocketAddress =
            readMessage.GetSocketAddressSrc();
        const uint32_t srcSocketAddressSize = srcSocketAddress.GetSize();
        const size_t bytesToWrite =
            (srcSocketAddressSize < messageHeader->msg_namelen)
                ? srcSocketAddressSize
                : messageHeader->msg_namelen;
        memcpy(messageHeader->msg_name, srcSocketAddress.GetBuffer(),
               bytesToWrite);
        messageHeader->msg_namelen =
            Anki::Util::numeric_cast<socklen_t>(bytesToWrite);
      }
...
void FakeUDPSocketInfo::PipeFrom(FakeUDPSocketInfo& inSocket,
                                 uint32_t localIpAddress) {
  for (MessageQueue::iterator it = inSocket._outMessageQueue.begin();
       it != inSocket._outMessageQueue.end();) {
    FakeUDPPacketData& packetData = *it;
    bool pipeThisMessage = false;

    {
      const sockaddr* socketAddressSrc = reinterpret_cast<const sockaddr*>(
          packetData.GetSocketAddressSrc().GetBuffer());
      const sockaddr* socketAddressDest = reinterpret_cast<const sockaddr*>(
          packetData.GetSocketAddressDest().GetBuffer());
      ...
      assert(packetData.GetSocketAddressDest().GetSize() ==
             sizeof(sockaddr_in));

      const sockaddr_in* socketAddressSrc_in =
          reinterpret_cast<const sockaddr_in*>(socketAddressSrc);
      const sockaddr_in* socketAddressDest_in =
          reinterpret_cast<const sockaddr_in*>(socketAddressDest);

      const uint32_t srcIp = socketAddressSrc_in->sin_addr.s_addr;
      const uint32_t destIp = socketAddressDest_in->sin_addr.s_addr;

      const bool isListeningToSrcAddress =
          ((_inAddress == htonl(INADDR_ANY)) || (_inAddress == srcIp));
      const bool isOnCorrectPort = (_port == socketAddressDest_in->sin_port);

      const bool dataIsForUs = isListeningToSrcAddress && isOnCorrectPort &&
                               (destIp == localIpAddress);
...
FakeUDPSocket::FakeUDPSocket(uint32_t ipAddress)
    : IUDPSocket(), _localIpAddress(ipAddress) {
  g_FakeUDPSocketManager.Add(this);
}

FakeUDPSocket::~FakeUDPSocket() { g_FakeUDPSocketManager.Remove(this); }

int FakeUDPSocket::OpenSocket(int protocolFamily, int socketType,
                              int protocol) {
  for (int i = 0; i < k_MaxSockets; ++i) {
    if (!_socketInfo[i].IsOpen()) {
      return _socketInfo[i].Open(IndexToSocketId(i), protocolFamily, socketType,
                                 protocol);
    }
  }
...
int FakeUDPSocket::BindSocket(int socketId, const sockaddr* socketAddress,
                              socklen_t socketAddressLength) {
...
int FakeUDPSocket::GetSocketOpt(int socketId, int level, int optname,
...
int FakeUDPSocket::SetSocketOpt(int socketId, int level, int optname,
...
ssize_t FakeUDPSocket::SendTo(int socketId, const void* messageData,
                              size_t messageDataSize, int flags,
                              const sockaddr* destSockAddress,
                              socklen_t destSockAddressLength) {
  std::lock_guard<std::mutex> lock(_mutex);
  ...
  if (fakeSocket != nullptr) {
    ...
    const socklen_t srcSockAddressLength =
        Anki::Util::numeric_cast<socklen_t>(sizeof(srcSockAddress));
    ...
    ssize_t sendResult = fakeSocket->SendTo(
        messageData, messageDataSize, flags,
        reinterpret_cast<const sockaddr*>(&srcSockAddress),
        srcSockAddressLength, destSockAddress, destSockAddressLength);
...
ssize_t FakeUDPSocket::ReceiveMessage(int socketId, msghdr* messageHeader,
                                      int flags) {
...
void FakeUDPSocket::PipeFrom(FakeUDPSocket* inSocket) {
  ...
  // Only lock inSocket if it's not this socket (this socket already locked via
  // PipeTo)
...
uint32_t FakeUDPSocket::GetLocalIpAddress() { return _localIpAddress; }
