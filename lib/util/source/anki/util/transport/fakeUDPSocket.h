...
class FakeUDPPacketData {
 public:
  ...
  void Set(const void* inBuffer, size_t inSize, const sockaddr* srcSockAddress,
           socklen_t srcSockAddressLength, const sockaddr* destSockAddress,
           socklen_t destSockAddressLength);
  ...
  const PacketByteArray& GetSocketAddressSrc() const {
    return _socketAddressSrc;
  }

  PacketByteArray& GetSocketAddressDest() { return _socketAddressDest; }
  const PacketByteArray& GetSocketAddressDest() const {
    return _socketAddressDest;
  }
...
class FakeUDPSocketInfo {
 public:
  ...
  int SetSocketOpt(int level, int optname, const void* optVal,
                   socklen_t optlen);
 ...
 private:
  ...
  uint32_t
      _inAddress;  // in network order (i.e. same as socket.sin_addr.s_addr)
...
class FakeUDPSocket : public IUDPSocket {
 public:
  ...
  static const uint32_t k_DefaultLocalIpAddress =
      (192) | (168 << 8) | (0 << 16) | (1 << 24);
 ...
 private:
  ...
  FakeUDPSocketInfo* GetSocketByIndex(int index) {
    return ((index >= 0) && (index < k_MaxSockets)) ? &_socketInfo[index]
                                                    : nullptr;
  }
