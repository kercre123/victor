 ...
 * Created: 1/21/15
 *
 * Description: Describes an interface to a network protocol that doesn't
...
class IUnreliableTransport {
 public:
  ...
  virtual void SendData(const TransportAddress& destAddress,
                        const SrcBufferSet& srcBuffers) = 0;
  ...
  void SetDataReceiver(INetTransportDataReceiver* dataReceiver) {
