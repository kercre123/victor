...
{ eRMT_Invalid = 0,
  ...
  eRMT_SingleUnreliableMessage,   // for unreliable messages stuffed into a
                                  // multi-message of other messages to minimize
                                  // packets sent
  eRMT_MultiPartMessage,          // big message, in n pieces
  eRMT_MultipleReliableMessages,  // several reliable messages grouped grouped
                                  // into 1 packet (increase chance of acking
                                  // all together)
  eRMT_MultipleUnreliableMessages,  // several unreliable messages grouped into
                                    // 1 packet (reduce number of packets sent)
  eRMT_MultipleMixedMessages,  // several (reliable and unreliable) messages
                               // grouped into 1 packet (combo of above)
  eRMT_ACK,
  eRMT_Ping,
  eRMT_Count } EReliableMessageType;

// ReliableTransport layer can still send things unreliably - some message types
