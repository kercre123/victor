...
class UDPTransport : public IUnreliableTransport {
 public:
  ...
  UDPTransport() : UDPTransport(AF_INET) {}
  UDPTransport(sa_family_t family);
  virtual ~UDPTransport();

  // from IUnreliableTransport
  virtual void SendData(const TransportAddress& destAddress,
                        const SrcBufferSet& srcBuffers) override;
  ...
  // ========== UDP Header settings - allows control per-app of things like
  // prefix, whether to add a CRC, etc. ==========
 ...
 private:
  ...
  ssize_t SendDataToSockAddress(const sockaddr_in& destSockAddress,
                                uint32_t destSockAddressLength,
                                const SrcBufferSet& srcBuffers) {
    return SendDataToSockAddress(
        reinterpret_cast<const sockaddr&>(destSockAddress),
        destSockAddressLength, srcBuffers);
  }
  ssize_t SendDataToSockAddress(const sockaddr_in6& destSockAddress,
                                uint32_t destSockAddressLength,
                                const SrcBufferSet& srcBuffers) {
    return SendDataToSockAddress(
        reinterpret_cast<const sockaddr&>(destSockAddress),
  ...
  ssize_t SendDataToSockAddress(const sockaddr& destSockAddress,
                                uint32_t destSockAddressLength,
                                const SrcBufferSet& srcBuffers);
  ...
  void HandleReceivedMessage(const uint8_t* buffer, uint32_t bufferLength,
