...
MessageStats::MessageStats(const char* inName) : _name(inName) { Reset(); }
...
void MessageStats::Print(const char* inName) const {
  if (_numMessages > 0) {
    ...
    PRINT_CH_INFO(
        "Network", "MessageStats",
        "[%s][%s] %u messages, %llu bytes, avg = %2.2f bytes (%u..%u)", inName,
        _name, _numMessages, _numBytes, averageBytes, _smallestMessage,
        _largestMessage);
    for (int i = 0; i < eME_Count; ++i) {
      if (_errorCounts[i] > 0) {
        PRINT_CH_INFO("Network", "MessageStats", "  ErrorCount %s = %u",
                      MessageErrorToString(EMessageError(i)), _errorCounts[i]);
      }
    }
  }
}
...
TransportStats::TransportStats(const char* inName)
    : _sentStats("Sent"), _recvStats("Recv"), _name(inName) {}
