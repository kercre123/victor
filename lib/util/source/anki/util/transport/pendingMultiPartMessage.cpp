...
PendingMultiPartMessage::PendingMultiPartMessage() { Clear(); }

PendingMultiPartMessage::~PendingMultiPartMessage() {}

bool PendingMultiPartMessage::AddMessagePart(const uint8_t* message,
                                             uint32_t messageSize) {
  if (messageSize > k_MultiPartMessageHeaderSize) {
    ANKI_NET_MESSAGE_VERBOSE(("HandleMultiPartMessage Recv",
                              "|Mu|Mu|Message...", message, messageSize));
    ...
    if (messageIndex == _nextExpectedPart) {
      ...
      ANKI_NET_PRINT_VERBOSE(
          "MultiPartMessage",
          "Message part %d of %d - adding %u bytes to %u existing",
...
const uint8_t* PendingMultiPartMessage::GetBytes() const { return &_bytes[0]; }
