 ...
 * Created: 02/04/15
 *
 * Description: A large "MultiPart" message that had to be split into pieces to
 *send and is still being assembled by this client
 *
...
const uint32_t k_MultiPartMessageHeaderSize =
    2;  // 2 bytes to indicate message X of Y
...
class PendingMultiPartMessage {
 public:
  ...
  bool IsWaitingForParts() const {
    return ((_lastExpectedPart > 0) &&
            (_nextExpectedPart <= _lastExpectedPart));
  }
