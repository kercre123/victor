 ...
 * Created: 02/15/15
 *
 * Description: Network Emulator UDP Socket, for testing and
 *emulation/simulation of different conditions Pipes everything to another
 *IUDPSocket interface (real or otherwise), and is only responsible for
 *modifying delivery time (including re-ordering) and random packet loss
 *
...
CONSOLE_VAR_RANGED(float, gUDPRandomPacketLossPercentage, "Network.Emulator",
                   -1.0f, -1.0f, 101.0f);
...
static bool RandPercentChanceOccured(float randPercentSuccess,
                                     float randValueIn0To100Range) {
...
QueuedUDPPacket::DeliveryTime
GetCurrentDeliveryTime()  // Milliseconds since first call to this function
{
  using namespace std::chrono;

  static high_resolution_clock::time_point firstEverTime =
      high_resolution_clock::now();
  high_resolution_clock::time_point currentTimeChrono =
      high_resolution_clock::now();

  const double numMilliSecondsSinceStart =
      duration_cast<milliseconds>(currentTimeChrono - firstEverTime).count();
  const QueuedUDPPacket::DeliveryTime currentDeliveryTime =
      Anki::Util::numeric_cast<QueuedUDPPacket::DeliveryTime>(
          numMilliSecondsSinceStart);
...
QueuedUDPPacket::QueuedUDPPacket() : _msgFlags(), _deliveryTime(0) {}

QueuedUDPPacket::QueuedUDPPacket(ssize_t messageSize,
                                 const msghdr* messageHeader,
                                 QueuedUDPPacket::DeliveryTime deliveryTime)
    : _msgFlags(), _deliveryTime(0) {
...
void QueuedUDPPacket::Set(ssize_t messageSize, const msghdr* messageHeader,
                          QueuedUDPPacket::DeliveryTime deliveryTime) {
  _msgName.Set(messageHeader->msg_name, messageHeader->msg_namelen);

  {
    // only 1 contiguous buffer support currently - if required add support for
    // appending them into stored buffer
    assert(messageHeader->msg_iovlen == 1);
    {
      iovec& ioVec = messageHeader->msg_iov[0];
      const uint32_t clampedSize =
          std::min(Anki::Util::numeric_cast<uint32_t>(messageSize),
                   Anki::Util::numeric_cast<uint32_t>(ioVec.iov_len));
      _msgIo.Set(ioVec.iov_base, clampedSize);
    }
  }

  _msgControl.Set(messageHeader->msg_control,
                  (uint32_t)messageHeader->msg_controllen);
...
UDPSocketQueue::UDPSocketQueue() : _socketId(-1), _isOpen(false) {}

UDPSocketQueue::~UDPSocketQueue() { ClearQueues(); }

void UDPSocketQueue::ClearQueues() { _inMessageQueue.clear(); }
...
void UDPSocketQueue::QueueMessage(ssize_t messageSize,
                                  const msghdr* messageHeader,
                                  QueuedUDPPacket::DeliveryTime deliveryTime) {
  if (_isOpen) {
...
ssize_t UDPSocketQueue::DequeueMessage(msghdr* messageHeader) {
  if (_isOpen) {
    ...
    for (PacketQueue::iterator it = _inMessageQueue.begin();
         it != _inMessageQueue.end();) {
      QueuedUDPPacket& queuedMessage = *it;
      const bool isReadyToDeliver =
          (GetCurrentDeliveryTime() >= queuedMessage.GetDeliveryTime());
...
NetEmulatorUDPSocket::NetEmulatorUDPSocket(uint32_t randSeed,
                                           float inRandomPacketLossPercentage,
                                           uint32_t minLatencyInMS,
                                           uint32_t maxLatencyInMS)
    : IUDPSocket(),
...
int NetEmulatorUDPSocket::OpenSocket(int protocolFamily, int socketType,
                                     int protocol) {
  UDPSocketQueue* socketQueue = FindFreeSocket();

  if (socketQueue != nullptr) {
    const int socketId =
        _udpSocketImpl->OpenSocket(protocolFamily, socketType, protocol);
    socketQueue->Open(socketId);
    return socketId;
  } else {
    PRINT_NAMED_ERROR(
        "NetEmulatorUDPSocket",
        "Error: No free emulator socket queues - all %d are in use!",
        kMaxSockets);
    errno = ENOBUFS;
    return -1;
  }
...
int NetEmulatorUDPSocket::BindSocket(int socketId,
                                     const sockaddr* socketAddress,
                                     socklen_t socketAddressLength) {
  return _udpSocketImpl->BindSocket(socketId, socketAddress,
                                    socketAddressLength);
}

int NetEmulatorUDPSocket::GetSocketOpt(int socketId, int level, int optname,
                                       void* optVal, socklen_t* optlen) {
  return _udpSocketImpl->GetSocketOpt(socketId, level, optname, optVal, optlen);
}

int NetEmulatorUDPSocket::SetSocketOpt(int socketId, int level, int optname,
...
ssize_t NetEmulatorUDPSocket::SendTo(int socketId, const void* messageData,
                                     size_t messageDataSize, int flags,
                                     const sockaddr* destSockAddress,
                                     socklen_t destSockAddressLength) {
  return _udpSocketImpl->SendTo(socketId, messageData, messageDataSize, flags,
                                destSockAddress, destSockAddressLength);
}

ssize_t NetEmulatorUDPSocket::ReceiveMessage(int socketId,
                                             msghdr* messageHeader, int flags) {
  const msghdr inMessageHeader = *messageHeader;

  UDPSocketQueue* socketQueue = FindSocketById(socketId);

  // Pump the wrapped socket - we deliberately empty it here (rather than 1 per
  // call) to ensure correct time stamps
  {
    ssize_t wrappedRecv = 0;
    do {
      wrappedRecv =
          _udpSocketImpl->ReceiveMessage(socketId, messageHeader, flags);
      if (wrappedRecv >= 0) {
        assert(socketQueue != nullptr);

        if (RandPercentChanceOccured(
                GetRandomPacketLossPercentage(),
                _randomGenerator.RandDblInRange(0.0f, 100.0f))) {
          ...
          QueuedUDPPacket::DeliveryTime deliveryTime =
              ...
              _randomGenerator.RandIntInRange(_minLatencyInMS, _maxLatencyInMS);
      ...
      *messageHeader =
          inMessageHeader;  // restore buffer sizes etc. for next read attempt
...
float NetEmulatorUDPSocket::GetRandomPacketLossPercentage() const {
  return (gUDPRandomPacketLossPercentage > _randomPacketLossPercentage)
             ? gUDPRandomPacketLossPercentage
             : _randomPacketLossPercentage;
}

void NetEmulatorUDPSocket::SetRandomPacketLossPercentage(
    float inRandomPacketLossPercentage) {
...
void NetEmulatorUDPSocket::SetLatencyRangeInMS(uint32_t minLatencyInMS,
                                               uint32_t maxLatencyInMS) {
