 ...
 * Created: 2/03/15
 *
 * Description: Manage a set of non contiguous src buffers that are all to be
 *merged in-order into one outbound message
 *
...
// Enable this to store descriptor text with each SizedSrcBuffer - used for
// debug printing messages to show field names
...
class SizedSrcBuffer {
 public:
  SizedSrcBuffer();
  SizedSrcBuffer(const uint8_t* inbuffer, uint32_t inbufferSize,
                 const char* inDescriptor = nullptr);
  ~SizedSrcBuffer() {}

  const uint8_t* GetBuffer() const { return _buffer; }

  const uint32_t GetSize() const { return _bufferSize; }
...
class SrcBufferSet {
 public:
  ...
  uint32_t GetCount() const { return _bufferCount; }
