...
static const size_t kMaxNetMessageSize =
    1472;  // Assuming a an MTU of 1500, minus (20+8) for the underlying IP and
           // UDP headers, this should be optimal for most networks/users

static const uint8_t kDefaultHeaderPrefix[4] = {
    'A', 'N', 'K', 02};  // Prefix is equivalent to 'A''N''K'02 in Little Endian
                         // - i.e. "ANKI Drive 2", i.e. Overdrive
...
class PosixUDPSocket : public IUDPSocket {
 public:
  ...
  virtual int CloseSocket(int socketId) override { return close(socketId); }
  ...
  virtual ssize_t SendTo(int socketId, const void* messageData,
    ...
    return sendto(socketId, messageData, messageDataSize, flags,
  ...
  virtual uint32_t GetLocalIpAddress() override {
    ...
    PRINT_CH_INFO("Network", "UDPTransport", "Our IP address: %s",
                  TransportAddress(ipAddress, 0).ToString().c_str());
    return ipAddress;
  }
  ...
  void SetIpRetriever(IIpRetriever* retriever) { _ipRetriever = retriever; }
...
void IpAddressToString(char* outBuffer, size_t outBufferLen,
                       uint32_t inIpAddress) {
  uint8_t* ipBytes = (uint8_t*)&inIpAddress;
  snprintf(outBuffer, outBufferLen, "%u.%u.%u.%u", ipBytes[0], ipBytes[1],
           ipBytes[2], ipBytes[3]);
}

HeaderCRCType ComputeCRC(HeaderCRCType inCrc, const void* inBuffer,
                         uint32_t inBufferSize) {
  return calculate_crc_ccitt(inCrc, reinterpret_cast<const uint8_t*>(inBuffer),
                             inBufferSize);
}
...
UDPTransport::HeaderPrefix::HeaderPrefix(const uint8_t* prefixBytes,
                                         uint32_t numBytes) {
  Set(prefixBytes, numBytes);
}

void UDPTransport::HeaderPrefix::Set(const uint8_t* prefixBytes,
                                     uint32_t numBytes) {
  if (numBytes > kMaxLength) {
    PRINT_NAMED_WARNING("UDPTransport.HeaderPrefix",
                        "prefix too long - truncating from %u to %u", numBytes,
                        kMaxLength);
    numBytes = kMaxLength;
  }
...
void UDPTransport::SetHeaderPrefix(const uint8_t* prefixBytes,
                                   uint32_t numBytes) {
...
size_t UDPTransport::GetMaxNetMessageSize() { return sMaxNetMessageSize; }
...
bool UDPTransport::DoesHeaderHaveCRC() { return sDoesHeaderHaveCRC; }

UDPTransport::HeaderPrefix UDPTransport::sHeaderPrefix(
    kDefaultHeaderPrefix, sizeof(kDefaultHeaderPrefix));
...
const char* GetAnkiPacketHeaderDescriptor() {
  ...
  if ((sDescriptorHasCRC != UDPTransport::DoesHeaderHaveCRC()) ||
    ...
    size_t descLen = 0;
    sprintf(&sDescriptor[descLen], "|%u ch Prefix...........................",
            UDPTransport::GetHeaderPrefix().GetLength());
 ...
 public:
  static const ifaddrs* GetIfAddrs(ifaddrs** const pointerToFree,
    ...
    if (errCode == 0) {
      for (ifaddrs* ifa = *pointerToFree; ifa != nullptr; ifa = ifa->ifa_next) {
        ...
        if (ifa->ifa_addr && ifa->ifa_name && (ifa->ifa_name[0] == 'e') &&
            (ifa->ifa_name[1] == 'n')) {
          if (ifa->ifa_addr->sa_family == family) {
            ...
            const bool isEn0Address =
                (ifa->ifa_name[2] == '0') && (ifa->ifa_name[3] == 0);
            if (!hasEn0Address) {
  ...
  virtual uint32_t GetIpAddress() override {
    ...
    if (localIpAddress == 0) {
      ...
      PRINT_NAMED_WARNING("GetLocalIpAddressFromIfAddr",
                          "No address found, defaulting to localhost!");
    }

    PRINT_CH_INFO("Network", "UDPTransport.GetIpAddress", "IP address = %s",
                  TransportAddress(localIpAddress, 0).ToString().c_str());
...
struct in6_addr GetLocalIpv6LinkLocalAddress(IUDPSocket* _udpSocketImpl) {
  struct in6_addr localIpv6Address =
      _udpSocketImpl->GetLocalIpv6LinkLocalAddress();

  if (IN6_IS_ADDR_UNSPECIFIED(&localIpv6Address)) {
    PRINT_NAMED_WARNING("GetLocalIpv6LinkLocalAddress",
                        "localIpv6Address is unspecified");
  }

  return localIpv6Address;
}

void UDPTransport::SetSocketImpl(IUDPSocket* udpSocketImpl,
                                 bool ownsSocketImpl) {
  FreeSocketImpl();
...
uint32_t UDPTransport::GetLocalIpAddress() {
  ...
  PRINT_CH_INFO("Network", "UDPTransport.GetLocalIpAddress",
                "Our IP address: %s",
                TransportAddress(ipAddress, 0).ToString().c_str());
  return ipAddress;
}
...
uint32_t UDPTransport::GetBroadcastAddressFromIfAddr() {
  ...
  if (ifa != nullptr) {
    subnetMask =
        PosixIpRetriever::GetSockAddress((const sockaddr_in*)ifa->ifa_netmask);
    ipAddress =
        PosixIpRetriever::GetSockAddress((const sockaddr_in*)ifa->ifa_addr);
  }
  freeifaddrs(ifAddrMemory);
  ...
  if (subnetMask == 0 || ipAddress == 0) {
    ...
    PRINT_NAMED_WARNING("GetBroadcastAddressFromIfAddr",
                        "No address found, defaulting to 255.255.255.255!");
  } else {
    broadcastAddress = (ipAddress & subnetMask) | (~subnetMask);
  }

  PRINT_CH_INFO("Network", "UDPTransport.GetBroadcastAddressFromIfAddr",
                "Broadcast address = %s",
                TransportAddress(broadcastAddress, 0).ToString().c_str());
...
// On Android, the Wifi interface should be wlan0, but maybe it is eth0
  ...
  // On iOS/Mac OSX and maybe the Android emulator, use the en0 interface for
  // Wifi
  if (!index) {
...
bool UDPTransport::OpenSocket(int port) {
  ...
  if (_family == AF_INET6) {
    ...
    if (ecode) {
      PRINT_NAMED_ERROR(
          "UDPTransport.BadDefaultIpv6Address",
          "Error: getaddrinfo for multicast address. ecode = %d '%s'", ecode,
          gai_strerror(ecode));
      return false;
    }
  }
  ...
  _socketId = _udpSocketImpl->OpenSocket(
      localAddr->ai_family, localAddr->ai_socktype, localAddr->ai_protocol);
  if (_socketId < 0) {
    PRINT_NAMED_ERROR(
        "UDPTransport.OpenSocketFailed",
        "Error: Unable to open socket - res = %d, errno = %d '%s'", _socketId,
        errno, strerror(errno));
    CloseSocket();
    return false;
  } else {
    PRINT_CH_INFO("Network", "UDPTransport.OpenSocket", "Opened Socket %d",
                  _socketId);
  }
  ...
  if (_family == AF_INET) {
    ...
    int setSockOptRes = -1;
    setSockOptRes = _udpSocketImpl->SetSocketOpt(
        _socketId, SOL_SOCKET, SO_BROADCAST, (void*)&sockOptionEnable,
        sizeof(sockOptionEnable));
    if (setSockOptRes < 0) {
      PRINT_NAMED_ERROR(
          "UDPTransport.SetBroadcastFailed",
    ...
    int hops = 1;
    if (_udpSocketImpl->SetSocketOpt(_socketId, IPPROTO_IPV6,
                                     IPV6_MULTICAST_HOPS, &hops,
                                     sizeof(hops)) != 0) {
      safe_freeaddrinfo(&mcastAddr);
      PRINT_NAMED_ERROR(
          "UDPTransport.SetIpv6MulticastHopsFailed",
          "SetSockOpt - IPV6_MULTICAST_HOPS failed, errno = %d '%s'", errno,
          strerror(errno));
      CloseSocket();
      return false;
    }
    ...
    if (_udpSocketImpl->SetSocketOpt(_socketId, IPPROTO_IPV6, IPV6_MULTICAST_IF,
      ...
      safe_freeaddrinfo(&localAddr);
      PRINT_NAMED_ERROR(
          "UDPTransport.SetIpv6MulticastIfFailed",
          "SetSockOpt - IPV6_MULTICAST_IF failed, errno = %d '%s'", errno,
          strerror(errno));
      CloseSocket();
      return false;
    }
  ...
  assert(_socketId >= 0);
  const int bindResult = _udpSocketImpl->BindSocket(
      _socketId, localAddr->ai_addr, localAddr->ai_addrlen);
  if (bindResult < 0) {
    if (EADDRINUSE == errno) {
      PRINT_NAMED_WARNING(
          "UDPTransport.OpenSocket.BindInUse",
          "Warning: Unable to bind to in-use socket, continuing as this is OK "
          "in case of running multiple instances on one machine.");
    } else {
      PRINT_NAMED_ERROR(
          "UDPTransport.OpenSocket.BindFailed",
          "Error: Unable to bind socket (res = %d), errno = %d '%s'",
  ...
  PRINT_CH_INFO("Network", "UDPTransport",
                "Socket %d open on port %d. Bind %ssuccessful", _socketId,
                _port, (bindResult < 0) ? "Un" : "");
  return true;
}
...
bool UDPTransport::CloseSocket() {
  ...
  if (closeResult < 0) {
    PRINT_NAMED_ERROR("UDPTransport.CloseSocket.Failed",
                      "Unable to close socket %d (res = %d), errno = %d '%s'",
                      _socketId, closeResult, errno, strerror(errno));
  } else {
    PRINT_CH_INFO("Network", "UDPTransport.CloseSocket.Success",
                  "Socket %d closed successfully", _socketId);
  }
...
void UDPTransport::ResetSocket() { _reset = true; }

uint32_t BuildPacketHeader(uint8_t* outBuffer, const SrcBufferSet& srcBuffers) {
  const UDPTransport::HeaderPrefix& headerPrefix =
      UDPTransport::GetHeaderPrefix();
...
uint32_t BuildPacket(uint8_t* outBuffer, uint32_t outBufferCapacity,
...
ssize_t UDPTransport::SendDataToSockAddress(const sockaddr& destSockAddress,
                                            uint32_t destSockAddressLengthIn,
                                            const SrcBufferSet& srcBuffers) {
  socklen_t destSockAddressLength = destSockAddressLengthIn;

  uint8_t bufferWithHeader[kMaxNetMessageSize];
  uint32_t bufferWithHeaderSize =
      BuildPacket(bufferWithHeader, sizeof(bufferWithHeader), srcBuffers);

  ANKI_NET_MESSAGE_VERBOSE(("UDP Send", GetAnkiPacketHeaderDescriptor(),
                            bufferWithHeader, bufferWithHeaderSize,
                            &srcBuffers));

  _transportStats.AddSentMessage(bufferWithHeaderSize);

  ssize_t sentBytes =
      _udpSocketImpl->SendTo(_socketId, bufferWithHeader, bufferWithHeaderSize,
                             0, &destSockAddress, destSockAddressLength);
  ...
  if (sentBytes != bufferWithHeaderSize) {
    if (sentBytes >= 0) {
      ...
      PRINT_NAMED_ERROR("UDPTransport.SentWrongNumBytes",
                        "sentBytes %zd != bufferSize %u", sentBytes,
...
void UDPTransport::SendData(const TransportAddress& destAddress,
                            const SrcBufferSet& srcBuffers) {
  ssize_t sendRes = -1;
  ...
  if (destAddress.IsIPAddress()) {
    ...
    sin.sin_port =
        htons(destAddress.GetIPPort());  // convert to network byte order (nbo)
    sin.sin_addr.s_addr =
        destAddress.GetIPAddress();  // ip address is already in nbo

    char ip_buf[INET_ADDRSTRLEN] = {0};
    const char* ip_str __attribute__((unused)) =
        inet_ntop(AF_INET, &sin.sin_addr.s_addr, ip_buf, sizeof(ip_buf));
    ANKI_NET_PRINT_VERBOSE("SendData", "Send to '%s:%d': %u bytes", ip_str,
                           ntohs(sin.sin_port),
                           srcBuffers.CalculateTotalSize());
    ...
    PRINT_NAMED_ERROR("UDPTransport.SendData.NonIpAddress",
                      "Error: UDP can only send to IP addresses!");
  }

  if (isValidIpAddress && (sendRes < 0)) {
    _transportStats.AddSendError(eME_SendFailed);

    // when send fails it usually fails for every send (i.e. every few ms), so
    // we throttle the warning to avoid spamming
    ...
    if (kEnableVerboseNetworkLogging ||
      ...
      PRINT_NAMED_WARNING(
          "UDPTransport.SendFailed",
          "sendto '%s' returned %zd, errno = %d '%s' (%u sends failed), now = "
          "%.1f",
          destAddress.ToString().c_str(), sendRes, errno, strerror(errno),
          _transportStats.GetSentStats().GetErrorCount(eME_SendFailed), now);
      _lastSendErrorPrinted = now;
    }
  }
}

void UDPTransport::HandleReceivedMessage(const uint8_t* buffer,
                                         uint32_t bufferLength,
                                         const TransportAddress& sourceAddress,
                                         bool wasTruncated) {
  ANKI_NET_MESSAGE_VERBOSE(
      ("UDP Recv", GetAnkiPacketHeaderDescriptor(), buffer, bufferLength));
  ...
  if (bufferLength < headerSize) {
    ...
    PRINT_NAMED_WARNING(
        "UDPTransport.BadPrefix.TooSmall", "Header '%s' is too small!",
        ConvertMessageBufferToString(buffer, bufferLength, Util::eBTTT_Ascii)
            .c_str());
  ...
  const bool hasCorrectHeaderPrefix =
      (memcmp(buffer, headerPrefix.GetBytes(), headerPrefix.GetLength()) == 0);
  if (!hasCorrectHeaderPrefix) {
    // Message wasn't from us, random internet traffic maybe
    PRINT_NAMED_WARNING(
        "UDPTransport.BadPrefix", "Header '%s' has wrong prefix!",
        ConvertMessageBufferToString(buffer, headerSize, Util::eBTTT_Ascii)
            .c_str());
  ...
  if (wasTruncated) {
    // This is bad as the message appeared to be from us (as it had the correct
    // prefix)
    PRINT_NAMED_WARNING("UDPTransport.Recv.Truncated",
                        "WARNING - Message buffer was too small to receive "
                        "entire message and was truncated - ignoring message!");
  ...
  if (DoesHeaderHaveCRC()) {
    ...
    memcpy(&headerCrc, &buffer[headerPrefix.GetLength()],
           sizeof(HeaderCRCType));

    HeaderCRCType crc = ComputeCRC(0, messageBuffer, messageBufferLength);
    if (crc != headerCrc) {
      _transportStats.AddRecvError(eME_BadCRC);
      PRINT_NAMED_WARNING(
          "UDPTransport.Recv.BadCRC",
          "WARNING - Message Has incorrect CRC (%u != %u) - ignoring message!",
          uint32_t(crc), uint32_t(headerCrc));
      return;
    }
  }

  if (_dataReceiver) {
    _dataReceiver->ReceiveData(messageBuffer, messageBufferLength,
                               sourceAddress);
  }
}
...
bool UDPTransport::TryToReadMessage() {
  ...
  ssize_t bytesReceived =
      _udpSocketImpl->ReceiveMessage(_socketId, &message, MSG_DONTWAIT);
  ...
  if (bytesReceived > 0) {
    ...
    HandleReceivedMessage(buffer, static_cast<uint32_t>(bytesReceived),
                          sourceTransportAddress, wasTruncated);

    return true;
  } else {
    if (EWOULDBLOCK !=
        errno)  // EWOULDBLOCK just means there's nothing in there
    {
      // This is a warning, not an error, because it sometimes happens in a
      // normal disconnection flow.
      PRINT_NAMED_WARNING(
          "UDPTransport.ReadFailed",
    ...
    if (ENOTCONN == errno) {
      ...
      if (!closeSocketSuccess) {
        PRINT_NAMED_ERROR(
            "UDPTransport.ReadFailed.CloseSocketFailed",
            "Error: Closing unconnected socket failed. errno = %d '%s'", errno,
            strerror(errno));
      }

      if (closeSocketSuccess && !OpenSocket(_port)) {
        PRINT_NAMED_ERROR(
            "UDPTransport.ReadFailed.ReopenSocketFailed",
            "Error: Reopening closed socket failed. errno = %d '%s'", errno,
            strerror(errno));
      }
    }
...
void UDPTransport::UpdateSocketImplForNetEmulation() {
  ...
  if (isNetEmulatorEnabled != wasNetEmulatorEnabled) {
    ...
    if (hadOpenSocket) {
      PRINT_NAMED_INFO("UDPTransport.UpdateSocketImplForNetEmulation",
                       "Switching from %s to %s socket impl, closing and "
                       "reopening socket %d",
                       wasNetEmulatorEnabled ? "NetEmu" : "Posix",
                       isNetEmulatorEnabled ? "NetEmu" : "Posix", _socketId);
      CloseSocket();
    }
...
void UDPTransport::FillAdvertisementBytes(AdvertisementBytes& bytes) {
  if (_family == AF_INET) {
    ...
    const uint8_t* const ipAddressAsBytes =
        reinterpret_cast<const uint8_t*>(&ipAddress);
    for (auto i = 0; i < sizeof(ipAddress); i++) {
      bytes.push_back(ipAddressAsBytes[i]);
    }
  } else if (_family == AF_INET6) {
    const struct in6_addr ipv6Address =
        GetLocalIpv6LinkLocalAddress(_udpSocketImpl);
...
unsigned int UDPTransport::FillAddressFromAdvertisement(
  ...
  if (_family == AF_INET) {
    ...
    if (size >= addressSize) {
      ...
      uint16_t port =
          ntohs(*reinterpret_cast<const uint16_t*>(&buffer[sizeof(ipAddress)]));
    ...
    if (size >= addressSize) {
      ...
      uint16_t port = ntohs(*reinterpret_cast<const uint16_t*>(
          &buffer[sizeof(ipv6Address.s6_addr)]));
