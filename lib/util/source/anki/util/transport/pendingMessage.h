 ...
 *              2)  reliable and hasn't been confirmed as successfully delivered
 *yet
 *
...
class PendingMessage {
 public:
  ...
  void Set(const SrcBufferSet& srcBuffers, EReliableMessageType messageType,
           ReliableSequenceId seqId, bool flushPacket,
           NetTimeStamp externalQueuedTime);
  ...
  EReliableMessageType GetType() const {
    return (EReliableMessageType)_messageType;
  }

  bool IsReliable() const {
    return (_sequenceNumber != k_InvalidReliableSeqId);
  }

  bool HasExternalQueuedTime() const {
    return (_externalQueuedTime != kNetTimeStampZero);
  }
  NetTimeStamp GetExternalQueuedDuration() const {
    return HasExternalQueuedTime() ? (_queuedTime - _externalQueuedTime)
                                   : kNetTimeStampZero;
  }
 ...
 private:
  ...
  NetTimeStamp
      _externalQueuedTime;  // Time that async request to queue was made
  NetTimeStamp
      _queuedTime;  // Time that we processed the request and queued the message
  NetTimeStamp
      _firstSentTime;  // Very first time that message was sent over a socket
  NetTimeStamp
      _lastSentTime;  // Most recent time that message was sent over a socket
  uint8_t* _message;
  uint32_t _messageSize;
  ReliableSequenceId _sequenceNumber;
  uint8_t _messageType;
  bool _flushPacket;  // is the message important enough to send even without a
                      // full packet of stuff
};
