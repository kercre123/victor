...
class JsonWriter {
 public:
  ...
  // This must be called before every list entry, including the first. Creates
  // an unnamed group for the next list item
  void NextListItem();

  // Instead of calling NextListItem(), use this to create a "naked" list entry,
  // which is just a value. Either pass in the value as a string, or use the
  // stream operator
  ...
  // Adds a simple key-value pair, using the underlying file stream operator.
  // two ways to do it. NOTE: the stream operator will always wrap values in
  // quotes! jsonWriter.AddEntry("key", "valueString");
  // jsonWriter.AddEntry("key", 3.1415);
  // jsonWriter.AddEntry("key")<<value;

  // NOTE: this is "Wrong" if you stream any non-string value. It will put it in
  // quotes, which works in ptree, but isn't valid json, so doesn't work in
  // jsoncpp

  // N.B. the value you print better not have any " in it!!!
  // TODO: could validate by inheriting from stream and returning this instead
  // of returning underlying stream
