...
JsonWriter::JsonWriter(const string& filename)
    : stream_(file_), file_(filename.c_str(), ios_base::out), closeFile_(true) {
  stream_ << "{\n";
  level_ = 1;
}

JsonWriter::JsonWriter(ostream& stream) : stream_(stream), closeFile_(false) {
  stream_ << "{\n";
  level_ = 1;
}
...
void JsonWriter::EndList() {
  // go backwards through the stack to figure out if this was a grouped list or
  // a raw list
...
void JsonWriter::End(char c) {
  ...
  if (stack_.empty()) {
    PRINT_NAMED_ERROR("jsonWriter.inconsistentStack",
                      "End called but stack was empty");
  ...
  if (stack_.back() != c) {
    PRINT_NAMED_ERROR("jsonWriter.inconsistentStack",
                      "Invalid stack value '%c' (%d), looking for '%c'",
                      stack_.back(), stack_.back(), c);
