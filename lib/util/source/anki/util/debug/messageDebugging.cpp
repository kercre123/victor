...
std::string ConvertMessageBufferToString(const uint8_t* buffer,
  ...
  // Calculate if we need to truncate the buffer size printed to fit within
  // maxStringLength

  const char* kTruncatedSuffix = "...";
  const bool showHexForUnprintable = (printType == eBTTT_Ascii);

  const uint32_t lettersPerByte =
      (addLeadingSpace || (printType == eBTTT_Decimal)) ? 3 : 2;
  const uint32_t fullStringLen = lettersPerByte * bufferLength;

  const bool isOutputTruncated = (fullStringLen > maxStringLength);
  const uint32_t clampedStringLen =
      isOutputTruncated ? maxStringLength : fullStringLen;

  uint32_t clampedBufferLen = bufferLength;
  if (isOutputTruncated) {
    assert(fullStringLen > clampedStringLen);
    const uint32_t extraLettersToSkip =
        (uint32_t)strlen(kTruncatedSuffix);  // need to remove additional bytes
                                             // to make room for suffix
    const uint32_t numLettersToSkip =
        (fullStringLen - clampedStringLen) + extraLettersToSkip;
    const uint32_t numBytesToSkip =
        (numLettersToSkip + (lettersPerByte - 1)) / lettersPerByte;
    clampedBufferLen =
        (bufferLength > numBytesToSkip) ? (bufferLength - numBytesToSkip) : 0;
  }
  ...
  for (uint32_t i = 0; i < clampedBufferLen; ++i) {
    ...
    const EBytesToTextType printTypeForChar =
        (showHexForUnprintable && !isPrintable) ? eBTTT_Hex : printType;
...
void LogVerboseMessage(const char* name, const char* columnLabels,
  ...
  std::string asciiString =
      ConvertMessageBufferToString(buffer, bufferLength, eBTTT_Ascii);
  std::string hexString =
      ConvertMessageBufferToString(buffer, bufferLength, eBTTT_Hex);
  ...
  while (numBytesPrinted < asciiString.size()) {
    ++line;
    printf("%02d %s   '%s'\n", line, name,
           asciiString.substr(numBytesPrinted, charsPerLine).c_str());
    printf("%02d %s 0x'%s'\n", line, name,
           hexString.substr(numBytesPrinted, charsPerLine).c_str());
