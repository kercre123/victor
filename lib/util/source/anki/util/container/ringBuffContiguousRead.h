 ...
 * Date:   Jun 9 2018
 *
 * Description: Ring buffer with contiguous READ memory of a guaranteed minimum
 *size. The size of the backing buffer is increased by the expected max read
 *size. This is intended for use with routines that need access to C arrays.
 *              Unlike CircularBuffer and FixedCircularBuffer, you will not be
 *able to add data until enough is consumed.
 *
...
class RingBuffContiguousRead {
 public:
  ...
  // If enough read data is available, moves the read cursor forward by len and
  ...
  // This class guarantees at least _maxReadSize contiguous elements on a read
  // as long as there are at least that many elements written. But the
  // contiguous region may be larger. Check the size here.
 ...
 private:
  ...
  inline size_t GetNumAvailable() const { return _capacity - GetNumUsed(); }
