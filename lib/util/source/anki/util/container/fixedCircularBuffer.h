 ...
 * Created: 11/29/17
 *
 * Description: Fixed memory and storage size (no dynamic allocations) circular
 *buffer.
 *
...
class FixedCircularBuffer {
 public:
  ...
  FixedCircularBuffer(FixedCircularBuffer&& rhs) noexcept {
    ...
    // we only need to reset the following to ensure the buffer is still in a
    // valid and reliable state
    rhs._firstIndex = 0;
    rhs._numEntries = 0;
  }
  ...
  bool operator==(
      const Anki::Util::FixedCircularBuffer<T, kCapacity>& rhs) const {
    if (size() != rhs.size()) {
      return false;
    }
  ...
  void push_back(const T* newEntryArray, size_t arraySize) {
    // If the incoming array is too big, skip forward over the entries that
  ...
  size_t front(T* outArray, size_t arraySize) {
    const size_t availableSize =
        (arraySize <= _numEntries) ? arraySize : _numEntries;
