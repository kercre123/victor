 ...
 * Created: 09/18/2018
 *
 * Description: A queue data structure that efficiently keeps track of the min
 *and max elements within it at all times. Complexity (where n is number of
 ...
 *
 *              Adapted from
 *https://people.cs.uct.ac.za/~ksmith/articles/sliding_window_minimum.html#sliding-window-minimum-algorithm.
 *
...
class MinMaxQueue {
 ...
 private:
  // The underlying container (we use a std::deque here instead of a std::queue
  // since deque has a clear() method)
  std::deque<T> _data;
