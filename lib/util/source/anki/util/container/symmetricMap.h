 ...
 * Created: 10/26/2018
 *
 * Description: Symmetric map for associating mappings between two values and
 *looking up either one by the other.
 *
 *              NOTE: Current implementation is not particularly memory
 *efficient and stores two of everything inserted. For small maps and simple
 *data types, this probably doesn't matter, but could be improved for more
 *general use. (VIC-11264)
 *
 *
...
class SymmetricMap<TypeA, TypeB, DiffTypes> {
 public:
  ...
  //  symMap.Erase(42) will remove both (42, "Meaning of Life") and (42,
  //  "Dolphins") symMap.Erase("Negative") will remove both (-1, "Negative) and
  //  (-2, "Negative")
  //
  void Erase(const TypeA& a);
  void Erase(const TypeB& b);

  // Find: Adds all elements associated with the given key to values. (NOTE:
  // does not clear values first!)
  //
  // Examples using instantiation above:
  //  symMap.Find(42, strs) will populate strs with {"Dolphins", "Meaning of
  //  Life"} symMap.Find("Nil", vals) will populate vals with 0
  ...
  // GetKeys: Adds all keys of the first or second type to given set. (NOTE:
  // does not clear keys first!)
  //
  // Examples using instantiation above:
  //  std::set<int> keys;
...
// - - - - - - - - - - - - - - - - - - - -
template <class TypeA, class TypeB>
SymmetricMap<TypeA, TypeB, DiffTypes>::SymmetricMap(
    const std::initializer_list<std::pair<TypeA, TypeB>>& args) {
...
// - - - - - - - - - - - - - - - - - - - -
template <class TypeA, class TypeB>
void SymmetricMap<TypeA, TypeB, DiffTypes>::GetKeys(
    std::set<TypeA>& keys) const {
...
// - - - - - - - - - - - - - - - - - - - -
template <class TypeA, class TypeB>
void SymmetricMap<TypeA, TypeB, DiffTypes>::Insert(const TypeA& a,
                                                   const TypeB& b) {
  _AtoB.emplace(a, b);
  _BtoA.emplace(b, a);
}
...
// - - - - - - - - - - - - - - - - - - - -
template <class TypeA, class TypeB>
void SymmetricMap<TypeA, TypeB, DiffTypes>::EraseHelper(const TypeA& a,
                                                        const TypeB& b) {
...
// - - - - - - - - - - - - - - - - - - - -
template <class TypeA, class TypeB>
void SymmetricMap<TypeA, TypeB, DiffTypes>::EraseHelper(const TypeB& b,
                                                        const TypeA& a) {
...
// - - - - - - - - - - - - - - - - - - - -
template <class TypeA, class TypeB>
void SymmetricMap<TypeA, TypeB, DiffTypes>::Find(
    const TypeA& key, std::set<TypeB>& values) const {
...
template <class TypeA, class TypeB>
void SymmetricMap<TypeA, TypeB, DiffTypes>::Find(
    const TypeB& key, std::set<TypeA>& values) const {
...
// - - - - - - - - - - - - - - - - - - - -
template <class TypeA, class TypeB>
SymmetricMap<TypeA, TypeB, SameTypes>::SymmetricMap(
    const std::initializer_list<std::pair<TypeA, TypeB>>& args) {
  for (const auto& pair : args) {
    Insert(pair.first, pair.second);
  }
...
// - - - - - - - - - - - - - - - - - - - -
template <class TypeA, class TypeB>
void SymmetricMap<TypeA, TypeB, SameTypes>::Insert(const TypeA& a,
                                                   const TypeB& b) {
  _map.emplace(a, b);
  _map.emplace(b, a);
}
...
// - - - - - - - - - - - - - - - - - - - -
template <class TypeA, class TypeB>
void SymmetricMap<TypeA, TypeB, SameTypes>::EraseHelper(const TypeA& a,
                                                        const TypeB& b) {
  auto range = _map.equal_range(a);
  auto iter = range.first;
  while (iter != range.second) {
...
// - - - - - - - - - - - - - - - - - - - -
template <class TypeA, class TypeB>
void SymmetricMap<TypeA, TypeB, SameTypes>::Find(
    const TypeA& key, std::set<TypeB>& values) const {
...
// - - - - - - - - - - - - - - - - - - - -
template <class TypeA, class TypeB>
void SymmetricMap<TypeA, TypeB, SameTypes>::GetKeys(
    std::set<TypeA>& keys) const {
