...
// required specialization for CircularBuffer<bool>, since the underlying
// container is specialized
template <>
struct CB_Typedef<bool> {
  using type = CircularBufferInternal<bool, std::vector<bool>::reference,
...
class CircularBufferInternal {
 public:
  ...
  CircularBufferInternal& operator=(const CircularBufferInternal& other) =
      default;
  ...
  CircularBufferInternal(CircularBufferInternal&& rhs) noexcept
    ...
    // rhs._buffer is likely now empty (size and capacity == 0), but this not
    // guarenteed by the C++ standard we only need to reset the following to
    // ensure the buffer is still in a valid and reliable state (empty with no
    // capacity)
    rhs._firstIndex = 0;
    rhs._numEntries = 0;
    rhs._capacity = 0;
  ...
  bool operator==(
      const Anki::Util::CircularBufferInternal<T, TRef, TCRef>& rhs) const {
    if (size() != rhs.size()) {
      return false;
    }
  ...
  void push_back(const T* newEntryArray, size_t arraySize) {
    ...
    if (newIndex + arraySize <= _capacity) {
      ...
      memcpy(&(_buffer[0]), &newEntryArray[firstSegSize],
             overflowSize * sizeof(T));
    }
  ...
  TCRef front() const { return (*this)[0]; }

  TRef front() { return (*this)[0]; }
  ...
  TCRef back() const { return (*this)[size() - 1]; }

  TRef back() { return (*this)[size() - 1]; }
 ...
 private:
  ...
  size_t _firstIndex;
  size_t _numEntries;
  size_t _capacity;  // not necessarily the same as _buffer.capacity() (e.g. if
                     // reduced, vector doesn't have to shrink)
};
