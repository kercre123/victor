...
#define PTREE_GET_CHILD_MANDATORY(tree, key) \
  Anki::Util::PtreeTools::GetChildMandatory(tree, key, __FILE__, __LINE__)
#define PTREE_GET_MANDATORY(tree, key, val) \
  Anki::Util::PtreeTools::GetMandatory(tree, key, val, __FILE__, __LINE__)
...
const boost::property_tree::ptree& GetChildMandatory(
    const boost::property_tree::ptree& tree, const std::string& key,
    const std::string& file, int line);

// helper for mandatory child getter
template <class T>
void GetMandatory(const boost::property_tree::ptree& tree,
                  const std::string& key, T& value, const std::string& file,
                  int line) {
  ...
  if (opt) {
    ...
    PRINT_NAMED_ERROR("Ptree.MissingKey", "error: %s:%d: key '%s' not found",
                      file.c_str(), line, key.c_str());
