...
// performs a fast deep merge, iterating all nodes in 'what', overriding all
// nodes that are common and copying those that are not
void FastDeepOverride(boost::property_tree::ptree& where,
                      const boost::property_tree::ptree& what);

// allows extending from 'what' into 'where', giving precedence to 'where' when
// common keys are found instead of 'what', storing the result in 'where'. The
// optional skipKey is removed from 'what' even if is not present in this is
// equivalent to: 1) make a copy of where 2) make a copy of what 3) remove
// skipKey from what(copy) 4) FastDeepOverride where into what(copy), which ends
// up in where without an extra copy If you also make a copy of where in your
// code, you could optimize by making a copy of what instead and overriding it
// with where = FastDeepOverride(what, where, x) This version exists as opposed
// to FastDeepAddition because overrides allow A.B.C.D notation, whereas
// unprocessed trees do not support it with an extra step.
void OrderedDeepOverride(boost::property_tree::ptree& where,
                         const boost::property_tree::ptree& what,
...
ptree DeepMergeLimited(const ptree& first, const ptree& second,
                       const std::string& nestedNodeKey);
...
// These are old, slow versions. Keeping them in case new versions have bugs and
// we need to reproduce old behavior, but do not use them.
class PtreeKey;  // TODO this is probably deprecated(unused) once we remove
                 // deprecated methods
