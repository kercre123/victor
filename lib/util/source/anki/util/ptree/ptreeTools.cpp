...
// This class tracks extensions in a tree that is being processed. It holds an
// Id that is being extended somewhere, along with information about the places,
// the data defined in the id, dependencies, etc. for a faster preprocess
// algorithm.
class ExtensionId {
 public:
  // constructor
  ...
  void AddExtension(ptree* place) { _extensions.push_back(place); }

  // add another Id that needs to be resolved before we are
  void AddDependency(ExtensionId* dep) { _dependencies.insert(dep); }
  ...
  // calculates sorting value if not calculated yet, otherwise it returns the
  // current value
  ...
  // debug print
  void Print() const {
    printf("+ (%d) %s [%zd uses][%zd dependencies]\n", _orderValue, _id.c_str(),
           _extensions.size(), _dependencies.size());
 ...
 private:
  ...
  ptree* _ptr;
  // value used for comparisons, calculated before extensions are resolved so
  // that we can do it in one pass
  int _orderValue;
...
void ExtensionId::Apply() {
  ...
  for (auto& extIt : _extensions) {
    // apply their overrides on top of our tree(_ptr), setting the result on
    // their tree and ignoring our ids
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - This method finds all extensions/ids
// and builds the ExtensionId table required for fast processing
void FindExtensions(ptree& tree, ExtensionMap& idMap,
                    std::vector<ExtensionId*>& openIds) {
  ExtensionId* idEntry = nullptr;
  boost::optional<string> idMatch =
      tree.get_optional<string>(Anki::Util::kP_ID);
  if (idMatch) {
    ...
    if (idEntry->IsDefined()) {
      PRINT_NAMED_ERROR("PTreeTools.Preprocess",
                        "The id '%s' is present multiple times",
                        idMatch->c_str());
  ...
  boost::optional<string> extendMatch =
      tree.get_optional<string>(Anki::Util::kP_EXTENDS);
  if (extendMatch) {
...
namespace {
// This function recurses through the entire tree, printing anything that
// contains a __debug__ key, value is printed as a string
void PrintDebugTrees(ptree& tree) {
  boost::optional<std::string> debugFlag =
      tree.get_optional<std::string>("__debug__");
  if (debugFlag) {
...
// given tree would fail to process for any reason
bool IsValidTreeForProcess(const ptree& ptree) {
  const size_t extendsLen = strlen(Anki::Util::kP_EXTENDS);

  // iterate the whole tree and check that extends is not used within paths with
  // format A.B.*
  for (const auto& entry : ptree) {
    ...
    if (extendsIdx != std::string::npos &&
        (extendsIdx != 0 || key.size() != extendsLen)) {
      PRINT_NAMED_ERROR("PTreeTools",
                        "Extends can't be defined within dot format paths '%s'",
                        key.c_str());
      return false;
    }
...
void FastDeepOverride(ptree& where, const ptree& what) {
  ...
  for (auto& whatEntry : what) {
    ...
    // if what::key is empty, we consider the entry part of a list, so just put
    // it in where
    if (key.empty()) {
    ...
    // entry is a ptree
      ...
      if (whereValue) {
        const bool bothTrees =
            !whereValue->empty() && !whatEntry.second.empty();
        if (bothTrees) {
          // Some lists rely currently on merging, whereas we want to support
          // some that override. If so, we need to check here if this list (or
          // ptree) is supposed to override instead of merge.
          ...
          if (thisKeyListShouldOverride) {
            // if what::value and where::value are lists, override completely
            // instead of merging
          ...
          // if what::value is a ptree, put the whole tree as a child of the
          // current key
          where.put_child(key, whatEntry.second);
        } else {
        ...
        // if 'where' does not have this key at this level, put what::value
        // under where::key
        where.put_child(key, whatEntry.second);
      }
    }
...
ptree DeepMergeLimited(const ptree& first, const ptree& second,
                       const string& nestedNodeKey) {
  // join ptrees and store
  // ------ >>>> Here be dragons
  ptree newData(first);
  boost::optional<ptree&> nestedNode =
      newData.get_child_optional(nestedNodeKey);
  for (ptree::const_iterator it = second.begin(); it != second.end(); ++it) {
    if (it->first.compare(nestedNodeKey) == 0 && nestedNode) {
      ...
      for (ptree::const_iterator nestedIt = it->second.begin();
           nestedIt != it->second.end(); ++nestedIt)
...
void FastPreprocess(ptree& tree) {
  ...
  {
    DEV_ASSERT(IsValidTreeForProcess(tree),
               "Can't process given tree. Check previous log for info");
  }
#endif
...
#if ANKI_DEVELOPER_CODE
  {
    ...
    if (!openIds.empty()) {
      PRINT_NAMED_ERROR(
          "PtreeTools.FastPreprocess",
          "Not all open ids were resolved, this is a programmer error.");
    ...
    for (const auto& extensionIt : extensions) {
      ...
      if (!extensionIt.second.IsDefined()) {
        PRINT_NAMED_ERROR(
            "PtreeTools.FastPreprocess",
            "Id '%s' is not defined, but we want to extend from it",
            extensionIt.first.c_str());
        throw std::runtime_error("PtreeTools.FastPreprocess");
      }

      // guarantee no-loops
      const bool dependsOnSelf =
          extensionIt.second.DependsOn(extensionIt.first);
      if (dependsOnSelf) {
        PRINT_NAMED_ERROR("PtreeTools.FastPreprocess",
                          "Id '%s' depends on itself",
                          extensionIt.first.c_str());
        throw std::runtime_error("PtreeTools.FastPreprocess");
      }
    }
  ...
  // 1) store in vector the Ids that are actually extended (the rest may be used
  // for something else)
  ...
  // 2) sort entries by dependencies (this guarantees we can replace in order
  // without conflicts)
...
namespace {
static bool is_not_alnum(char c) { return !(isalnum(c) || c == '_'); }
}  // namespace
...
const pair<string, pair<ptree, bool> > newMapEntry(const string& s,
                                                   const ptree& p, bool b) {
    ...
    for (const ptree::value_type& child : current) {
      ...
      if (child.second.size() == 0 || child.first.empty()) {
        ...
        if (child.first.empty()) {
          ...
          string s;
          if (keychain != "") s = keychain;

          // get other node
          boost::optional<ptree&> mergeListOptional =
              ptMerged.get_child_optional(s);
          if (mergeListOptional) {
            // cout << " extending list at " << s << " : " <<
            // child.second.data() << "\n";
            mergeListOptional->push_back(make_pair("", child.second));
          } else {
            // cout << " creating a list at " << s << " : " <<
            // child.second.data() << "\n";
            ptree mergeList;
      ...
      // Subtree
        ...
        // Put keys (identifiers of subtrees) and all of its parents (where
        // present) aside for later iteration. Keys on first level have no
        // parents
        if (keychain != "")
...
void Preprocess_Deprecated(ptree& tree) {
  // hack for replay / basestation-main
  if (tree.get<bool>(Anki::Util::kP_PREPROCESSED, false)) return;
  ...
  BOUNDED_WHILE(50000, !firstPass.done()) {
    ...
    boost::optional<string> extends =
        firstPass.topTree().get_optional<string>(Anki::Util::kP_EXTENDS);
    if (extends) {
      numRefs++;
  ...
  PRINT_NAMED_DEBUG("Preprocess_Deprecated",
                    "There are %d references and %zd ids", numRefs,
                    idMap.size());
  ...
  BOUNDED_WHILE(100, numIDRefs > 0) {
    PRINT_NAMED_INFO("PTreeTools.PreprocessIteration", "phase 2 numIDRefs: %d",
                     numIDRefs);
    ...
    for (internal::IDMap::iterator it = idMap.begin(); it != end; ++it) {
      ...
      BOUNDED_WHILE(1000, !trav.done()) {
        boost::optional<string> extends =
            trav.topTree().get_optional<string>(Anki::Util::kP_EXTENDS);
        if (extends) {
          numIDRefs++;
          ...
          if (targetIt != end) {
            ...
            if (target.second) {
              ...
              for (const ptree::value_type& v : child) {
                boost::optional<ptree&> other =
                    target.first.get_child_optional(v.first);
            ...
            PRINT_NAMED_ERROR("PtreeTools.InvalidConfig",
                              "id '%s' not found in the map", extends->c_str());
          }
        }
        trav.next();
      }
  ...
  // and finally go back through the ptree and merge in the now fully resolved
  // ids
  multimap<string, PtreeKey>::iterator endKey = extendsKeys.end();
  for (multimap<string, PtreeKey>::iterator it = extendsKeys.begin();
       it != endKey; ++it) {
    internal::IDMap::iterator idIter = idMap.find(it->first);
    if (idIter == idMap.end()) {
      PRINT_NAMED_ERROR("PtreeTools.InternalError",
                        "Key '%s' not found in id map", it->first.c_str());
      continue;
    }

    if (!idIter->second.second) {
      PRINT_NAMED_ERROR("Ptree.CouldNotResolveKey",
                        "could not fully resolve id '%s'", it->first.c_str());
...
ptree& GetChild_Deprecated(ptree& tree, const PtreeKey& key) {
  ...
  for (unsigned int i = 0; i < key.size(); ++i) {
    ...
    if (key[i].second >= 0) {
      std::pair<ptree::assoc_iterator, ptree::assoc_iterator> bounds =
          curr->equal_range("");
      ...
      if (dist > 0) {
        ...
        if (key[i].second >= dist) {
          PRINT_NAMED_ERROR("PTreeTools.GetChild.IndexOutOfBounds",
                            "Index of %d exceeds distance of %ld",
                            key[i].second, dist);
