...
struct UnsignedIntTranslator {
  ...
  boost::optional<Json::UInt64> get_value(const std::string& str) {
    ...
    if (!str.empty() && std::regex_match(str, int_pattern)) {
      return boost::optional<Json::UInt64>(
          numeric_cast<Json::UInt64>(std::stoull(str)));
    }
...
struct HexStringTranslator {
  // checks if a string fits the 0x... hex pattern, and if so lets it through as
  // a string (no conversion)
...
static inline bool _PTreeConvertValue(const boost::property_tree::ptree& input,
                                      Json::Value& output) {
...
bool PTreeToJSON(const boost::property_tree::ptree& input,
                 Json::Value& output) {
  if (input.empty()) {
...
//  please only use this if it is too much work to convert a class already
//  depending on ptree
bool JSONToPTree(const Json::Value& input,
