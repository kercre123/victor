...
class CladEnumToStringMap {
 public:
  bool Load(const Anki::Util::Data::DataPlatform* data, const std::string& path,
  ...
  // Caching the lookup map will allocate additional memory for the class but
  // make future lookups faster Returns true if outKey is set, false if lookup
  // failed
  bool GetKeyForValue(const std::string& value, CladEnum& outKey,
                      bool shouldCacheLookupMap = false);

  bool GetKeyForValueConst(const std::string& value, CladEnum& outKey) const;
...
// - - - - - - - - - - - - - - - - - - - -
template <class CladEnum>
bool CladEnumToStringMap<CladEnum>::Load(
    const Anki::Util::Data::DataPlatform* data, const std::string& path,
    const std::string& fileNameKey, bool isReverseLookupAllowed) {
  if (nullptr == data) {
    return false;
  }
  ...
  const std::string fullPath =
      data->pathToResource(Util::Data::Scope::Resources, path);

  const bool fileExists = Anki::Util::FileUtils::FileExists(fullPath);
  ANKI_VERIFY(fileExists,
              "CladEnumToStringMap.Load.FileDoesNotExistAtExpectedPath",
    ...
    if (success) {
      ...
      for (int i = 0; i < numPairs; ++i) {
        ...
        if (ANKI_VERIFY(memberNames.size() == 2,
          ...
          std::string filePath = JsonTools::ParseString(
              singleEvent, fileNameKey.c_str(), debugName + fileNameKey);
          const std::string enumAsString = JsonTools::ParseString(
              singleEvent, kCladEventKey, debugName + kCladEventKey);
          CladEnum enumValue;
    ...
    for (const auto& pair : _cladMap) {
      ...
      if (pair.first != outKey) {
        PRINT_NAMED_ERROR("CladEnumToStringMap.Load.ReverseLookupFailure",
                          ...
                          pair.second.c_str(), EnumToString(outKey),
                          EnumToString(pair.first));
      }
    }
  }
...
// - - - - - - - - - - - - - - - - - - - -
template <class CladEnum>
bool CladEnumToStringMap<CladEnum>::GetKeyForValue(const std::string& value,
                                                   CladEnum& outKey,
                                                   bool shouldCacheLookupMap) {
  if (!_isReverseLookupAllowed) {
    return false;
  }
...
// - - - - - - - - - - - - - - - - - - - -
template <class CladEnum>
bool CladEnumToStringMap<CladEnum>::GetKeyForValueConst(
    const std::string& value, CladEnum& outKey) const {
  if (!_isReverseLookupAllowed) {
    return false;
  }
...
// - - - - - - - - - - - - - - - - - - - -
template <class CladEnum>
void CladEnumToStringMap<CladEnum>::UpdateValue(CladEnum ev,
