...
bool ParseCladEnumFromJSON(const Json::Value& json, CladEnum& value,
                           ...
                           bool shouldAssertIfMissing = true) {
  if (!json.isString()) {
    const char* debugMsg =
        (debugName + ".GetCladEnumFromJSON.ParseString.ValueNotString").c_str();
    DEV_ASSERT(!shouldAssertIfMissing, debugMsg);
    return false;
  }
  std::string str = json.asString();
  // Note - this functionality can only be used in engine until VIC-2545 is
  // imlemented
  const bool foundValue = Vector::EnumFromString(str.c_str(), value);
  if (!foundValue) {
    const char* debugMsg =
        (debugName + "GetCladEnumFromJSON.ParseString.InvalidValue").c_str();
    DEV_ASSERT(!shouldAssertIfMissing, debugMsg);
  }
...
template <class CladEnum>
bool GetCladEnumFromJSON(const Json::Value& config, const std::string& key,
                         CladEnum& value, const std::string& debugName,
                         bool shouldAssertIfMissing = true) {
  const Json::Value& child = config[key];
  if (child.isNull()) {
    const char* debugMsg =
        (debugName + ".GetCladEnumFromJSON.ParseString.KeyMissing").c_str();
