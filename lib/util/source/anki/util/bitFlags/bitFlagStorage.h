 ...
 * Created: 02/02/2016
 *
 * Description: Templated container for storing N uintX_t structures and
 *supporting just enough arithmetic operators so that it can be used inside
 *BitFlags<T> This allows unlimited number of bits/flags to be stored in
 *BitFlags instead of just the uint64 limit
 *
...
struct BitFlagStorage {
 public:
  static_assert(!std::numeric_limits<InnerVarType>::is_signed,
                "Only supports unsigned inner types");

  BitFlagStorage() {
    // default constructor does nothing - exactly the same as the underlying
    // types
  }
  ...
  BitFlagStorage operator<<(uint32_t shiftAmmount) const {
    ...
    const uint32_t numVarsOffset =
        shiftAmmount / numBitsPerVar;  // 0 for < 1 entire var
    const uint32_t offsetInVar =
        shiftAmmount - (numVarsOffset * numBitsPerVar);  // 0..(numBitsPerVar-1)
  ...
  bool operator<(const BitFlagStorage& rhs) const { return (Compare(rhs) < 0); }
  ...
  // ============================== Equivalent to InnerType expanded to
  // BitFlagStorage with all but [0]=InnerType and [>0]=0
  ...
  bool operator<(const InnerVarType& rhs) const { return (Compare(rhs) < 0); }
 ...
 private:
  InnerVarType
      _storage[kNumVars];  // from Least to Most signifcant. [0] contains LSB
};
