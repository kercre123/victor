 ...
 * Created: 10/20/2014
 *
 * Description: Templated bit flags container - helper for working with bit
 *flags inside a given type (e.g. uint8/16/32/64)
 *
...
class BitFlags {
 public:
  using StorageType = T;
  using BitShiftType =
      uint32_t;  // allows up to 2^32 bits - e.g. up to a 2 GB variable...

  BitFlags() : _bitFlags(0u) {}

  BitFlags(const T& val) : _bitFlags(val) {}
  ...
  inline bool IsBitFlagSet(ET bitFlag) const {
    const T bitMask =
        GetBitMask(Anki::Util::numeric_cast<BitShiftType>(bitFlag));
...
// For > 64bits use BitFlagStorage for a structure of multiple built in uint
// types that supports bit shifts and masks or BitFlags<BitFlagStorage<uint64_t,
// 3>, MyEnum> myBitFlags;
