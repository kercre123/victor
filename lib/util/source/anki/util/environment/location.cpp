...
static const std::string kCachedLocationTmpFileName =
    "lastCachedLocation.json.tmp";
...
static bool ParseLocationFromJson(const Json::Value& jsonLocation,
                                  Location& location) {
  Json::Value countryObj =
      jsonLocation.get("country", Json::Value(Json::objectValue));
  std::string isoCodeStr =
      countryObj.get("iso_code", Json::Value(Json::stringValue)).asString();
...
static bool ParseLocationFromJsonString(const std::string& jsonString,
                                        Location& location) {
  Json::Reader reader;
...
static bool ParseLocationFromJsonFile(const std::string& filePath,
                                      Location& location) {
...
void Location::StartProvider(const Location::ProviderConfig& config) {
  ...
  sCachedLocationFilePath = FileUtils::FullFilePath(
      {sProviderConfig.workingDir, kCachedLocationFileName});
  if (config.expireCache) {
    sHaveCachedLocation = false;
    FileUtils::DeleteFile(sCachedLocationFilePath);
  } else {
    sHaveCachedLocation =
        ParseLocationFromJsonFile(sCachedLocationFilePath, sCachedLocation);
  ...
  HttpRequestCallback callback =
      [](const HttpRequest& request, const int responseCode,
        ...
        if (!isHttpSuccessCode(responseCode)) {
          Util::sChanneledInfoF(LOG_CHANNEL,
                                "util.location.fetch_failed.bad_http_response",
        ...
        if (!parsingSuccess) {
          PRINT_NAMED_INFO("util.location.failed_to_parse", "%s",
                           request.uri.c_str());
  ...
  if (!sProviderConfig.url.empty() &&
      !sProviderConfig.authHeaderValue.empty()) {
    HttpRequest httpRequest;
    httpRequest.method = HttpMethodGet;
    httpRequest.uri = sProviderConfig.url;
    httpRequest.storageFilePath = FileUtils::FullFilePath(
        {sProviderConfig.workingDir, kCachedLocationTmpFileName});
