...
Locale GetCurrentLocaleAndroid(void* jniEnv) {
  ...
  jmethodID getDefaultMethodID = env->GetStaticMethodID(
      localeClassHandle.get(), "getDefault", "()Ljava/util/Locale;");
  jmethodID getLanguageMethodID = env->GetMethodID(
      localeClassHandle.get(), "getLanguage", "()Ljava/lang/String;");
  jmethodID getCountryMethodID = env->GetMethodID(
      localeClassHandle.get(), "getCountry", "()Ljava/lang/String;");
  if (!getDefaultMethodID || !getLanguageMethodID || !getCountryMethodID) {
    return Locale::kDefaultLocale;
  }

  JObjectHandle localeObjHandle{
      env->CallStaticObjectMethod(localeClassHandle.get(), getDefaultMethodID,
                                  NULL),
      env};
  ...
  std::string languageString = JNIUtils::getStringFromObjectMethod(
      env, localeObjHandle.get(), getLanguageMethodID);
  std::string countryString = JNIUtils::getStringFromObjectMethod(
      env, localeObjHandle.get(), getCountryMethodID);
...
Locale GetCurrentLocaleAndroid() {
  if (ANKI_USE_JNI) {
    ...
    PRINT_NAMED_WARNING("GetCurrentLocaleAndroid.jni_disabled", "%s",
                        "JNI locale not available");
    return Locale::kDefaultLocale;
  }
}
