...
class PerfMetric {
 public:
  ...
  bool ParseCommands(const std::string& queryStr,
                     const bool queueForExecution = true);
 ...
 protected:
  ...
  void DumpFiles();
  void DumpLine(const DumpType dumpType, int dumpBufferOffset, FILE* fd,
                std::string* resultStr) const;
  ...
  void OnShutdown();
  void InitInternal(Util::Data::DataPlatform* dataPlatform,
                    WebService::WebService* webService);
  void UpdateWaitMode();
  void RemoveOldFiles() const;
  bool FrameBufferEmpty() const {
    return _nextFrameIndex == 0 && !_bufferFilled;
  }
  ...
  struct PerfMetricCommand {
    ...
    PerfMetricCommand(CommandType cmd, DumpType dumpType, bool dumpAll) {
      _command = cmd;
      _dumpType = dumpType, _dumpAll = dumpAll;
    }
  };
