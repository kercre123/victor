 ...
 * Created: 11/03/2017
 *
 * Description: Lightweight performance metric recording: base class
 *implementation
 *
...
static int PerfMetricWebServerHandler(struct mg_connection* conn,
                                      void* cbdata) {
  ...
  const int returnCode = ws->ProcessRequestExternal(
      conn, cbdata, PerfMetricWebServerImpl, commands);
...
void PerfMetric::InitInternal(Util::Data::DataPlatform* dataPlatform,
  ...
  _webService->RegisterRequestHandler("/perfmetric", PerfMetricWebServerHandler,
                                      this);
#endif
}
...
void PerfMetric::Status(std::string* resultStr) const {
  if (resultStr != nullptr) {
    ...
    *resultStr += ("," + std::to_string(numFrames) + "," +
                   std::to_string(kNumFramesInBuffer) + "\n");
  }
}

void PerfMetric::Start() {
  if (_isRecording) {
    LOG_INFO("PerfMetric.Start",
...
void PerfMetric::Dump(const DumpType dumpType, const bool dumpAll,
  ...
  if (dumpAll) {
    if ((dumpType == DT_RESPONSE_STRING) && (resultStr != nullptr)) {
      // When dumping all to HTTP response string, also send back the current
      // frame buffer index: First, return the new frame buffer index (what we
      // are catching up to)
      *resultStr +=
          ("Next frame buffer index:" + std::to_string(_nextFrameIndex) + "\n");
    }
  ...
  for (int frameIndex = 0; frameIndex < numFrames; frameIndex++) {
    ...
    const float tickSleepOver_ms =
        frame._tickSleepActual_ms - frame._tickSleepIntended_ms;
...
#define LINE_DATA_VARS                                          \
  frameTimeString.c_str(), frameIndex, frame._tickExecution_ms, \
      frame._tickTotal_ms, frame._tickSleepIntended_ms,         \
      frame._tickSleepActual_ms, tickSleepOver_ms

      static const char* kFormatLine = "%s %5i %8.3f %8.3f %8.3f %8.3f %8.3f";
      static const char* kFormatLineCSV = "%s,%i,%.3f,%.3f,%.3f,%.3f,%.3f";

      int strSize =
          snprintf(_dumpBuffer, kSizeDumpBuffer,
    ...
    timeAtStartOfFrame =
        IncrementFrameTime(timeAtStartOfFrame, frame._tickTotal_ms);
  ...
  for (int lineIndex = 0; lineIndex < kNumLinesInSummary; lineIndex++) {
    static const char* kSummaryLineFormat =
        "%18s %8.3f %8.3f %8.3f %8.3f %8.3f";
    static const char* kSummaryLineCSVFormat =
        ",%s,%8.3f,%8.3f,%8.3f,%8.3f,%8.3f";

#define SUMMARY_LINE_VARS(StatCall)                           \
  accTickDuration.StatCall(), accTickTotal.StatCall(),        \
      accSleepIntended.StatCall(), accSleepActual.StatCall(), \
      accSleepOver.StatCall()

    int strSize = 0;
#define OUTPUT_SUMMARY_LINE(StatDescription, StatCall)                      \
  strSize += snprintf(                                                      \
      &_dumpBuffer[strSize], kSizeDumpBuffer - strSize,                     \
      dumpType == DT_FILE_CSV ? kSummaryLineCSVFormat : kSummaryLineFormat, \
      StatDescription, SUMMARY_LINE_VARS(StatCall));
    switch (lineIndex) {
      case 0:
        OUTPUT_SUMMARY_LINE("Min:", GetMin);
        break;
      case 1:
        OUTPUT_SUMMARY_LINE("Max:", GetMax);
        break;
      case 2:
        OUTPUT_SUMMARY_LINE("Mean:", GetMean);
        break;
      case 3:
        OUTPUT_SUMMARY_LINE("Std:", GetStd);
        break;
    }
...
void PerfMetric::DumpFramesSince(const int firstFrameBufferIndex,
  ...
  if (firstFrameBufferIndex < 0) {
    ...
    if (firstFrameBufferIndex < _nextFrameIndex) {
      ...
      DEV_ASSERT(_bufferFilled,
                 "first frame requested is beyond buffer that's been recorded");
      numFrames =
          kNumFramesInBuffer - (firstFrameBufferIndex - _nextFrameIndex);
    }
  }

  // First, return the new frame buffer index (what we are catching up to), and
  // the number of frames to follow
  *resultStr += (std::to_string(_nextFrameIndex) + "," +
...
#define SINCE_LINE_DATA_VARS                                                 \
  frame._tickExecution_ms, frame._tickTotal_ms, frame._tickSleepIntended_ms, \
      frame._tickSleepActual_ms, tickSleepOver_ms

    // Note that we omit the first two fields normally sent (log time and frame
    // buffer index)
    static const char* kFormatLineCSV = ",,%.3f,%.3f,%.3f,%.3f,%.3f";

    int strSize = snprintf(_dumpBuffer, kSizeDumpBuffer, kFormatLineCSV,
                           SINCE_LINE_DATA_VARS);

    // Append additional data from derived class
    static const bool kGraphableDataOnly =
        true;  // To exclude behavior strings in engine, e.g.
    strSize += AppendFrameData(DT_FILE_CSV, frameBufferIndex, strSize,
                               kGraphableDataOnly);
...
void PerfMetric::DumpFiles() {
  ...
  sstr << _fileDir << "/" << _logBaseFileName
...
void PerfMetric::DumpLine(const DumpType dumpType, int dumpBufferOffset,
                          FILE* fd, std::string* resultStr) const {
  switch (dumpType) {
    case DT_LOG: {
#ifdef SIMULATOR
      // In webots, the log system adds a newline for us, unlike in VicOS, so we
      // strip it off here rather than have code below to append a newline in
      // the VicOS version
...
void PerfMetric::RemoveOldFiles() const {
  ...
  const int numFilesToRemove =
      static_cast<int>(fileList.size()) - kMaxNumFilesToKeep;
...
float PerfMetric::IncrementFrameTime(float msSinceMidnight,
                                     const float msToAdd) const {
...
void PerfMetric::WaitSeconds(const float seconds) {
  if (_waitMode) {
    LOG_INFO("PerfMetric.WaitSeconds",
             "Wait for seconds requested but already in wait mode");
  }
  _waitMode = true;
  _waitTicksRemaining = 0;
  _waitTimeToExpire =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() + seconds;
  LOG_INFO("PerfMetric.WaitSeconds", "Waiting for %f seconds", seconds);
}

void PerfMetric::WaitTicks(const int ticks) {
  if (_waitMode) {
    LOG_INFO("PerfMetric.WaitTicks",
             "Wait for ticks requested but already in wait mode");
  }
  _waitMode = true;
...
bool PerfMetric::ParseCommands(const std::string& queryStr,
  ...
  while (!queryString.empty()) {
    ...
    if (current == "status") {
      ...
      if (current.substr(0, cmdKeywordWaitSeconds.size()) ==
          cmdKeywordWaitSeconds) {
        std::string argumentValue =
            current.substr(cmdKeywordWaitSeconds.size());
        PerfMetricCommand cmd(WAIT_SECONDS);
        try {
          cmd._waitSeconds = std::stof(argumentValue);
        } catch (std::exception) {
          LOG_INFO("PerfMetric.ParseCommands",
                   "Error parsing float argument in perfmetric command: %s",
                   current.c_str());
          return 0;
        }
        cmds.push_back(cmd);
      } else if (current.substr(0, cmdKeywordWaitTicks.size()) ==
                 cmdKeywordWaitTicks) {
        ...
        try {
          ...
          LOG_INFO("PerfMetric.ParseCommands",
                   "Error parsing int argument in perfmetric command: %s",
                   current.c_str());
          return false;
        }
        cmds.push_back(cmd);
      } else if (current.substr(0, cmdKeywordDumpResponseSince.size()) ==
                 cmdKeywordDumpResponseSince) {
        std::string argumentValue =
            current.substr(cmdKeywordDumpResponseSince.size());
        ...
        LOG_INFO("PerfMetric.ParseCommands",
                 "Error parsing perfmetric command: %s", current.c_str());
        return false;
      }
    }
  }
