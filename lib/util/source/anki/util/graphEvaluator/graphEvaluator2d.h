...
class GraphEvaluator2d {
 public:
  ...
  explicit GraphEvaluator2d(const std::vector<Node>& inNodes,
                            bool errorOnFailure = true);

  // ensure noexcept default move and copy assignment-operators/constructors are
  // created
  ...
  // Get slope at x. If x is within the range of data, the return value is the
  // slope at x- (from the left) to match the implementation of EvaluateY. If x
  // is outside of the range, slope is defined to be 0. (If there are only two
  // nodes with the same x value, this returns FLT_MAX.)
  float GetSlopeAt(float x) const;

  // Reverse lookup to try and find an X value that would produce Y, returns
  // success (on success outX has the x result)
