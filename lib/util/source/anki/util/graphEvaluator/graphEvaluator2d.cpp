 ...
 * Created: 10/13/15
 *
 * Description: A 2d graph of nodes, lerps values between nodes, clamps values
 *at ends of the range
 *
...
bool GraphEvaluator2d::Node::operator==(
    const GraphEvaluator2d::Node& rhs) const {
...
GraphEvaluator2d::GraphEvaluator2d(const std::vector<Node>& inNodes,
                                   bool errorOnFailure) {
  AddNodes(inNodes, errorOnFailure);
}

void GraphEvaluator2d::Clear() { _nodes.clear(); }
...
bool GraphEvaluator2d::AddNodes(const std::vector<Node>& inNodes,
                                bool errorOnFailure) {
  ...
  if (errorOnFailure && !addedAllNodesOK) {
    PRINT_NAMED_ERROR("GraphEvaluator2d.AddNodes.ErrorAdding",
                      "Expected to have %zu nodes after adding %zu, but has "
                      "%zu nodes instead!",
...
bool GraphEvaluator2d::AddNode(float x, float y, bool errorOnFailure) {
  ...
  if (numNodes > 0) {
    ...
    if (x < _nodes[lastNodeIdx]._x) {
      if (errorOnFailure) {
        PRINT_NAMED_ERROR(
            "GraphEvaluator2d.AddNode.OORange",
            "new node (%f, %f) has x < than last node %zu = (%f, %f)", x, y,
            lastNodeIdx, _nodes[lastNodeIdx]._x, _nodes[lastNodeIdx]._y);
      }
      return false;
    }
  }
...
float GraphEvaluator2d::GetSlopeAt(float x) const {
  ...
  // calc and cache the second to last slope, in case the last segment is
  // vertical. If there are only two nodes sharing the same x value, this
  // returns FLT_MAX.
  ...
  for (size_t i = 1; i < numNodes; ++i) {
    ...
    if (notVertical && ((x <= nextNode->_x) || lastSegment)) {
      ...
      // this slope will not get returned unless the last segment is vertical,
      // which means that the second to last segment should not be vertical
      // (because 3 nodes with the same x value is prohibited)
    }
  ...
  // the last segment was vertical, so either there's only one vertical segment
  // and this returns FLT_MAX, or there are multiple segments and this returns
  // the second to last segment's slope.
...
bool GraphEvaluator2d::FindFirstX(float y, float& outX) const {
  ...
  // See if almost exact match on 1st node (span won't find if it's single node
  // graph or only a NEAR match outside of span's range)
  ...
  for (size_t i = 1; i < numNodes; ++i) {
    ...
    if (((y >= prevNode->_y) && (y <= nextNode->_y)) ||
      ...
      // X result exists in this span, but if there's no x separation then
      // technically that result isn't possible (there's an immediate step up)
...
bool GraphEvaluator2d::ReadFromJson(const Json::Value& inJson) {
  ...
  if (nodes.isNull()) {
    PRINT_NAMED_WARNING("GraphEvaluator2d.ReadFromJson.NoNodes",
                        "Missing entry for '%s' key", kNodesKey);
    return false;
  }
  ...
  for (uint32_t i = 0; i < numNodes; ++i) {
    ...
    if (node.isNull()) {
      PRINT_NAMED_WARNING("GraphEvaluator2d.ReadFromJson.BadNode",
                          "Node %u failed to read", i);
      return false;
    }
    ...
    if (nodeX.isNull()) {
      PRINT_NAMED_WARNING("GraphEvaluator2d.ReadFromJson.BadX",
                          "Node %u failed to read '%s'", i, kNodeXKey);
      return false;
    }
    if (nodeY.isNull()) {
      PRINT_NAMED_WARNING("GraphEvaluator2d.ReadFromJson.BadY",
                          "Node %u failed to read '%s'", i, kNodeYKey);
      return false;
    }
