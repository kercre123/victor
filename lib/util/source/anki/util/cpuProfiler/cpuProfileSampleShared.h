 ...
 * Created: 07/30/16
 *
 * Description: Data shared between all instances of a single profile (across
 *calls and ticks) Currently just the name of the sample Can easily be extended
 *to track stats like min/avg/max time, num calls (per-tick or ever) but as that
 *can also be collected externally we skip it here for now to minimize skewing
 *performance
 *
...
class CpuSampleStats {
 public:
  ...
  const Stats::StatsAccumulator& GetRecentStats() const {
    return (_recentStats[0].GetNumDbl() > _recentStats[1].GetNumDbl())
               ? _recentStats[0]
               : _recentStats[1];
  }
  const Stats::StatsAccumulator& GetLifetimeStats() const {
    return _lifetimeStats;
  }

 private:
  Stats::StatsAccumulator _lifetimeStats;
  // For recent stats we reset the larger of the 2 stats every N ticks, so after
  // the first N ticks we always have (N..2N) recent samples in GetRecentStats()
  Stats::StatsAccumulator _recentStats[2];
};

class CpuSamplePerThreadData {
 public:
  CpuSamplePerThreadData() : _isCalledFromThread(false) {}
  ...
  void SetIsCalledFromThread(bool newVal = true) {
    _isCalledFromThread = newVal;
  }
...
class CpuProfileSampleShared {
 public:
  explicit CpuProfileSampleShared(const char* name) : _name(name) {}
  ...
  void ResetForThread(uint32_t threadIndex) {
    GetThreadData(threadIndex).Reset();
  }

  CpuSampleStats& GetSampleStats(uint32_t threadIndex) {
    return GetThreadData(threadIndex).GetSampleStats();
  }
  const CpuSampleStats& GetSampleStats(uint32_t threadIndex) const {
    return GetThreadData(threadIndex).GetSampleStats();
  }
  const Stats::StatsAccumulator& GetRecentStats(uint32_t threadIndex) const {
    return GetSampleStats(threadIndex).GetRecentStats();
  }
  const Stats::StatsAccumulator& GetLifetimeStats(uint32_t threadIndex) const {
    return GetSampleStats(threadIndex).GetLifetimeStats();
  }

  bool IsCalledFromThread(uint32_t threadIndex) const {
    return GetThreadData(threadIndex).IsCalledFromThread();
  }
  void SetIsCalledFromThread(uint32_t threadIndex, bool newVal = true) {
    GetThreadData(threadIndex).SetIsCalledFromThread(newVal);
  }
 ...
 private:
  ...
  CpuSamplePerThreadData& GetThreadData(uint32_t threadIndex) {
    return const_cast<CpuSamplePerThreadData&>(
        const_cast<const CpuProfileSampleShared*>(this)->GetThreadData(
            threadIndex));
  }
