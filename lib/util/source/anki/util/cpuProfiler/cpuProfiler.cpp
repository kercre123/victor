...
// a) We assume that no calls to profile happen before static initialization, so
// the global will be constructed in time b) We avoid any race condition from
// multiple threads calling GetInstance() before the instance if finished
// construction
CpuProfiler gProfilerInstance;
}  // end anonymous namespace

CpuProfiler& CpuProfiler::GetInstance() { return gProfilerInstance; }
...
CpuThreadProfiler* CpuProfiler::GetThreadProfilerByName(
...
void CpuProfiler::CheckAndUpdateProfiler(CpuThreadProfiler& profiler,
                                         double maxTickTime_ms,
                                         uint32_t logFreq) const {
  // Skip checking for stale settings as these can be changed via console
  // variables
  profiler.SetMaxTickTime_ms(maxTickTime_ms);
  profiler.SetLogFrequency(logFreq);
}

CpuThreadProfiler* CpuProfiler::GetOrAddThreadProfiler(CpuThreadId threadId,
                                                       const char* threadName,
                                                       double maxTickTime_ms,
                                                       uint32_t logFreq) {
  ...
  if (existingProfiler) {
    if (existingProfiler->HasBeenDeleted()) {
      ...
      PRINT_CH_INFO("CpuProfiler", "CpuProfiler.ThreadReused",
                    "Thread %u: '%s' being reused",
                    existingProfiler->GetThreadIndex(), threadName);
      return existingProfiler;
    } else {
      PRINT_NAMED_WARNING(
          "CpuProfiler.ThreadNameClash",
          "Multiple threads named '%s' - will not profile duplicate threads!",
          threadName);
      return nullptr;
    }
  }
  ...
  if (newIndex < kCpuProfilerMaxThreads) {
    ...
    threadProfiler->Init(threadId, newIndex, threadName, maxTickTime_ms,
                         logFreq, _creationTimePoint);

    PRINT_CH_INFO("CpuProfiler", "CpuProfiler.ThreadAdded",
                  "Thread %u: '%s' added", threadProfiler->GetThreadIndex(),
                  threadName);
...
static CpuThreadProfiler* GetThreadByNameOrIndex(const char* threadName) {
  ...
  CpuThreadProfiler* threadProfiler =
      cpuProfiler.GetThreadProfilerByName(threadName);
  if (!threadProfiler) {
    // See if threadName is an index instead
    int threadIndex = atoi(threadName);
    if ((threadIndex != 0) || (strcmp(threadName, "0") == 0)) {
      threadProfiler =
          CpuProfiler::GetInstance().GetThreadProfilerByIndex(threadIndex);
    }
  }
...
static void SetThreadMaxTickTime(ConsoleFunctionContextRef context) {
  ...
  const double maxTickTime_ms =
      ConsoleArg_Get_Double(context, "maxTickTime_ms");

  CpuThreadProfiler* profiler = GetThreadByNameOrIndex(threadName);

  if (profiler) {
    context->channel->WriteLog(
        "Changing Thread '%s' max tick time from %f to %f ms", threadName,
        profiler->GetMaxTickTime_ms(), maxTickTime_ms);

    profiler->SetMaxTickTime_ms(maxTickTime_ms);
  } else {
    context->channel->WriteLog(
        "Thread '%s' not found - cannot set tick time to %f ms", threadName,
        maxTickTime_ms);
  }
}
CONSOLE_FUNC(SetThreadMaxTickTime, kCpuProfilerSection, const char* threadName,
             double maxTickTime_ms);
...
static void SetThreadLogFreq(ConsoleFunctionContextRef context) {
  ...
  if (profiler) {
    context->channel->WriteLog(
        "Changing Thread '%s' log frequency from %u to %u",
        profiler->GetThreadName(), profiler->GetLogFrequency(), logFreq);

    profiler->SetLogFrequency(logFreq);
  } else {
    context->channel->WriteLog(
        "Thread '%s' not found - cannot set log freq to %u", threadName,
        logFreq);
  }
}
CONSOLE_FUNC(SetThreadLogFreq, kCpuProfilerSection, const char* threadName,
             uint32_t logFreq);
...
static void LogProfile(ConsoleFunctionContextRef context) {
  ...
  if (profiler) {
    context->channel->WriteLog("Requesting Profile for thread '%s'",
                               profiler->GetThreadName());
    profiler->RequestProfile();
  } else {
    context->channel->WriteLog("Thread '%s' not found - cannot show profile",
                               threadName);
  }
}
...
static void ListProfiledThreads(ConsoleFunctionContextRef context) {
  ...
  for (uint32_t i = 0; i < profilerCount; ++i) {
    const CpuThreadProfiler* threadProfiler =
        cpuProfiler.GetThreadProfilerByIndex(i);
    if (threadProfiler) {
      context->channel->WriteLog(
          "Thread %u: '%s', maxTick: %f, LogFreq: %u%s", i,
          threadProfiler->GetThreadName(), threadProfiler->GetMaxTickTime_ms(),
