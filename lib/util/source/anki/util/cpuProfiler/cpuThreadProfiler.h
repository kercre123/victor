...
#define ANKI_CPU_PROFILER_WARN_ON_TINY_SAMPLES \
  0  // Enable to track down functions that are too quick to be worth profiling
#define ANKI_CPU_PROFILER_WARN_ON_SKIPPED_SAMPLES \
  0  // Enable to track down functions that aren't being profiled
...
class CpuThreadProfiler {
 public:
  ...
  void Init(CpuThreadId threadId, uint32_t threadIndex, const char* threadName,
            double maxTickTime_ms, uint32_t logFrequency,
            const CpuProfileClock::time_point& baseTimePoint,
            uint32_t sampleCount = 512);

  static void SetChromeTracingFile(const char* tracingFile);
  static void SendToWebVizCallback(
      const std::function<void(const Json::Value&)>& callback);
  void ReuseThread(CpuThreadId threadId);

  void SortProfile() {
    _currentProfile.SortSamples();
  }  // optional, needed for LogProfile() to work correctly
  ...
  const CpuThreadProfile& GetCurrentProfile() const {
    return _currentProfile;
  }  // NOTE: Unsafe to call from another thread whilst this is being updated

  bool IsSameThread(CpuThreadId threadId) const {
    return AreCpuThreadIdsEqual(_threadId, threadId);
  }
  ...
  double GetTimeSinceBase_ms(
      const CpuProfileClock::time_point& timePoint) const {
  ...
  static void SetMinSampleDuration_ms(double newVal) {
    sMinSampleDuration_ms = newVal;
  }
 ...
 private:
  void WarnOfSkippedProfileSample(const CpuProfileSampleShared& sharedData,
                                  double duration_ms);
  void WarnOfTinyProfileSample(const CpuProfileSampleShared& sharedData,
                               double duration_ms);
  ...
  CpuProfileClock::time_point
      _baseTimePoint;  // a common time point shared by all threads, before any
                       // thread started
  double _maxTickTime_ms;  // Max tick time before forcing tick to be logged
  uint32_t
      _threadIndex;  // index of this thread in the profiler (for simple TLS)
