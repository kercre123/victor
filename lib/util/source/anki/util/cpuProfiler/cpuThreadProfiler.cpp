...
CONSOLE_VAR_ENUM(int, kProfilerLogOutput, "CpuProfiler", 0,
                 "Console,Chrome Tracing,WebViz");

double CpuThreadProfiler::sMinSampleDuration_ms =
    0.01;  // ignore trivial samples below this

FILE* CpuThreadProfiler::_chromeTracingFile = nullptr;
std::function<void(const Json::Value&)> CpuThreadProfiler::_webServiceCallback =
    nullptr;
...
void CpuThreadProfiler::Init(CpuThreadId threadId, uint32_t threadIndex,
                             const char* threadName, double maxTickTime_ms,
                             uint32_t logFrequency,
...
void CpuThreadProfiler::SetChromeTracingFile(const char* tracingFile) {
  ...
  if (_chromeTracingFile) {
    ...
    fprintf(_chromeTracingFile, "\"ts_epoch\": %lld,",
            std::chrono::duration_cast<std::chrono::milliseconds>(
                CpuProfileClock::now().time_since_epoch())
                .count());
...
void CpuThreadProfiler::SendToWebVizCallback(
...
void CpuThreadProfiler::StartTick() { _currentProfile.StartTick(); }
...
void CpuThreadProfiler::EndTick() {
  ...
  const bool shouldLogTickFreq =
      (_logFrequency != kLogFrequencyNever) &&
      ((_logFrequency == 0) ||
       ((_currentProfile.GetTickNum() % _logFrequency) == 0));

  if (_profileRequested || shouldLogTickFreq ||
      (isSlowTick && kProfilerLogSlowTicks)) {
    if (isSlowTick) {
      PRINT_CH_INFO("CpuProfiler", "SlowTick",
                    "Thread %u '%s' Took %.3f ms (> %.3f ms)", GetThreadIndex(),
                    GetThreadName(), tickDuration, _maxTickTime_ms);
    }
    SortAndLogProfile();
    LogAllCalledSamples();
    ...
    // PRINT_CH_INFO("CpuProfiler", "NotLogging", "Thread %u '%s' tick %u Took
    // %.3f ms",
    //              GetThreadIndex(), GetThreadName(),
    //              _currentProfile.GetTickNum(),
...
#if ANKI_CPU_PROFILE_TRACK_SAMPLE_STATS
  {
    ...
    // This allows us to only track the most recent N..2N values without having
    // to maintain a rolling buffer

    const uint32_t kMaxTicksForRecentStats = 100;
    const uint32_t smallestTickCountIndex =
        _currentProfile.GetIndexForSmallestRecentTickCount();
    const uint32_t smallestTickCount =
        _currentProfile.GetRecentTickCount(smallestTickCountIndex);

    if (smallestTickCount > kMaxTicksForRecentStats) {
      const uint32_t largestTickCountIndex =
          (smallestTickCountIndex == 0) ? 1 : 0;

      // Reset the largest set
      // (the smallest set will now be 0, and it will take
      // kMaxTicksForRecentStats until we reset again)
...
void CpuThreadProfiler::LogProfile() const {
  if (kProfilerLogOutput == (int)Output::Console) {
    PRINT_CH_INFO("CpuProfiler", "LogProfile",
                  "Thread %u '%s' tick %u [%.3f ms from start %.3f, end %.3f]",
                  GetThreadIndex(), GetThreadName(),
                  _currentProfile.GetTickNum(),
                  _currentProfile.GetTickDuration(),
...
void CpuThreadProfiler::LogAllCalledSamples() const {
  if (kProfilerLogOutput == (int)Output::Console) {
    PRINT_CH_INFO("CpuProfiler", "LogAllCalledSamples",
                  "Thread %u '%s' tick %u", GetThreadIndex(), GetThreadName(),
                  _currentProfile.GetTickNum());

    _currentProfile.LogAllCalledSamples(_threadIndex, _samplesCalledFromThread);
  } else if (kProfilerLogOutput == (int)Output::WebViz) {
    _currentProfile.PublishToWebService(_webServiceCallback, GetThreadName(),
                                        _threadIndex, _samplesCalledFromThread);
  }
}

void CpuThreadProfiler::WarnOfTinyProfileSample(
    const CpuProfileSampleShared& sharedData, double duration_ms) {
  PRINT_NAMED_WARNING("CpuThreadProfiler.IgnoreTinySample",
                      "Ignoring Tiny Sample '%s' took %f ms (<= %f)",
                      sharedData.GetName(), duration_ms, sMinSampleDuration_ms);
}

void CpuThreadProfiler::WarnOfSkippedProfileSample(
    const CpuProfileSampleShared& sharedData, double duration_ms) {
  PRINT_NAMED_WARNING("CpuThreadProfiler.SkippedSample",
                      "Skipped Sample '%s' took %f ms%s (%zu/%zu samples)",
                      sharedData.GetName(), duration_ms,
                      _currentProfile.HasStarted() ? "" : " TICK NOT STARTED!",
                      _currentProfile.GetSampleCount(),
                      _currentProfile.GetSampleCapacity());
}
