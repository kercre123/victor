...
const char* depthToPrefix(int depth) {
  static const char* kPrefixSpaces =
      "................................................................";  // Some
                                                                           // loggers
                                                                           // (e.g.
                                                                           // Webots)
  ...
  for (const CpuProfileSample& sample : _samples) {
    while (sampleStackDepth > 0) {
      ...
      if (latestSample->GetEndTimePoint() <= sample.GetStartTimePoint()) {
        // new sample starts after latestSample finished - pop latestSample off
        // the stack
        --sampleStackDepth;
      } else {
    ...
    {
      const CpuSampleStats& sampleStats =
          sample.GetSharedData().GetSampleStats(threadIndex);
      const Stats::StatsAccumulator& lifeTimeStats =
          sampleStats.GetLifetimeStats();
      const Stats::StatsAccumulator& recentStats = sampleStats.GetRecentStats();

      PRINT_CH_INFO(
          "CpuProfiler", "CPS",
          "%s%s: %.3fms [Start:%.3f, End:%.3f] (recent: %.3f..%.3f avg: %.3f "
          "num: %.0f) (lifetime: %.3f..%.3f avg: %.3f num: %.0f)",
          depthToPrefix((int)sampleStackDepth), sample.GetName(),
          ...
          recentStats.GetMinSafe(), recentStats.GetMaxSafe(),
          recentStats.GetMean(), recentStats.GetNumDbl(),
          lifeTimeStats.GetMinSafe(), lifeTimeStats.GetMaxSafe(),
          lifeTimeStats.GetMean(), lifeTimeStats.GetNumDbl());
    }
...
float CalcAvgMsPerTick(const Stats::StatsAccumulator& stats,
                       float oneOverTickCount) {
  const float callsPerTick = stats.GetNumDbl() * oneOverTickCount;
  const float avgMsPerTick = callsPerTick * stats.GetMean();
  return avgMsPerTick;
}

void CpuThreadProfile::LogAllCalledSamples(
  ...
  const float oneOverRecentTickCount =
      (recentTickCount > 0) ? (1.0f / float(recentTickCount)) : 1.0f;

  std::sort(
      sortedSamples.begin(), sortedSamples.end(),
      [threadIndex, oneOverRecentTickCount](const CpuProfileSampleShared* lhs,
                                            const CpuProfileSampleShared* rhs) {
        // Sort from highest average-per-tick to lowest
        const float avgMsPerTickL = CalcAvgMsPerTick(
            lhs->GetRecentStats(threadIndex), oneOverRecentTickCount);
        const float avgMsPerTickR = CalcAvgMsPerTick(
            rhs->GetRecentStats(threadIndex), oneOverRecentTickCount);
  ...
  for (const CpuProfileSampleShared* sample : sortedSamples) {
    ...
    PRINT_CH_INFO(
        "CpuProfiler", "CpuProfileList",
        "[%.3f ms per tick] %s: (recent: %.3f..%.3f avg: %.3f num: %.0f) "
        "(lifetime: %.3f..%.3f avg: %.3f num: %.0f) (%.2f calls per tick)",
        avgMsPerTick, sample->GetName(), recentStats.GetMinSafe(),
        recentStats.GetMaxSafe(), recentStats.GetMean(),
        recentStats.GetNumDbl(), lifeTimeStats.GetMinSafe(),
        lifeTimeStats.GetMaxSafe(), lifeTimeStats.GetMean(),
        lifeTimeStats.GetNumDbl(), callsPerTick);
  }
}

void CpuThreadProfile::SaveChromeTracingProfile(FILE* fp,
  ...
  for (const CpuProfileSample& sample : _samples) {
    while (sampleStackDepth > 0) {
      ...
      if (latestSample->GetEndTimePoint() <= sample.GetStartTimePoint()) {
        ...
        // the stack
        fprintf(fp,
                "{\"name\": \"%s\", \"cat\": \"c++\", \"ph\": \"E\", \"ts\": "
                "%lld, \"pid\": %d, \"tid\": %llu},\n",
                latestSample->GetName(),
                std::chrono::duration_cast<std::chrono::milliseconds>(
                    latestSample->GetEndTimePoint().time_since_epoch())
                    .count(),
                pid, tid);
        --sampleStackDepth;
      } else {
    ...
    {
      fprintf(fp,
              "{\"name\": \"%s\", \"cat\": \"c++\", \"ph\": \"B\", \"ts\": "
              "%lld, \"pid\": %d, \"tid\": %llu},\n",
              sample.GetName(),
              std::chrono::duration_cast<std::chrono::milliseconds>(
                  sample.GetStartTimePoint().time_since_epoch())
                  .count(),
              pid, tid);
    }
    fflush(fp);
...
void CpuThreadProfile::PublishToWebService(
