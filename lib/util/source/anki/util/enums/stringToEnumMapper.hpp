 ...
 * Created: 11/09/15
 *
 * Description: Template helper for doing relatively fast case insensitive
 *string-to-enum lookups
 *
 *              NOTE: Should only be included in CPP files - only needs
 *instantiating once per type
 *
...
// ================================================================================
// Template Classes to simplify creation of string-to-enum lookups (case
// insensitive and case sensitive)
//
// Example usage:
//
...
//
// 2) In cpp: implement using StringToEnumMapperI<T> or StringToEnumMapper<T>
// (for case insensitive or not):
//
...
//      (the requirement for no 2 entries to have the exact same representation
//      is already enforced by the compiler)
//
// ================================================================================
 ...
 public:
  StringToEnumMapperI() {
    ...
    for (T i = T(0); i < T::Count; ++i) {
      // Add all entries to map in lower case - allows all lookups to be case
      // insensitive
      ...
      _stringToEnumMap.insert(
          typename StringToEnumMap::value_type(lowerCaseString, i));

#if ANKI_DEVELOPER_CODE
      if (!newEntry.second) {
        const T existingMapping = newEntry.first->second;
        PRINT_NAMED_ERROR(
            "StringToEnumMapperI.DuplicateNames",
            "Entries %d and %d have same lower-case representation '%s'",
            (int)existingMapping, (int)i, lowerCaseString.c_str());
        assert(0);
      }
#endif
    }
  ...
  T GetTypeFromString(const char* inString) const {
    ...
    PRINT_NAMED_WARNING("StringToEnumMapperI.GetTypeFromString.NotFound",
...
// Case sensitive string mapper (avoids string->lower conversion on setup and
// lookup)
template <typename T>
class StringToEnumMapper {
 public:
  ...
  T GetTypeFromString(const char* inString,
                      bool assertOnInvalidEnum = false) const {
    const auto& it = _stringToEnumMap.find(inString);
    if (it != _stringToEnumMap.end()) {
      return it->second;
    }
    ...
    PRINT_NAMED_WARNING("StringToEnumMapper.GetTypeFromString.NotFound",
                        "No match found for '%s'", inString);
