...
// This will cause us to keep a separate list of all of our strings in order to
// convert back from STRID to std::string. Although helpful for debug, we may
// not want this for release builds since it will take up a lot of memory.
#define ALLOW_STRID_TO_SRING_MAPPING (1)

// This is how many strings we will pre-allocate for.  We can still grow after
// this, but this will allow us to limit how many re-hashing we need to do
// during the lifecycle of our game.
