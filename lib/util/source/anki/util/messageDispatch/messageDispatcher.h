...
class MessageDispatcher {
 public:
  ...
  virtual ~MessageDispatcher() { RemoveAllSubscribers(); }

  void Subscribe(MessageReceiver_t* receiver, MessageTag_t playerMessageType,
                 const MessageResponder_t& responder) {
  ...
  void Unsubscribe(const MessageReceiver_t* receiver, MessageTag_t messageTag) {
    ...
    if (foundIt != _receivers.end()) {
      foundIt->second.erase(
          std::remove_if(foundIt->second.begin(), foundIt->second.end(),
                         [receiver](ReceiverTuple& tuple) {
                           return tuple.receiver == receiver;
                         }),
          foundIt->second.end());
    }
  }
 ...
 private:
  struct ReceiverTuple {
    ReceiverTuple(MessageReceiver_t* receiver,
                  const MessageResponder_t& responder)
