...
// Return the json string as a map of <string, string>. This will fail if the
// json has non string values or is nested
...
// Warning:  This is homegrown code for use by
// PlayerAdvertisement::GetAdvertisingString. It has not been broadly tested.
// For wider use, unit tests are strongly recommended.
...
std::string StringJoin(const std::vector<std::string>& strings,
                       char delim = ',');
...
// posix Epoch time in struct form (time since Jan 1, 1970, midnight UTC) based
// on the passed in date string format If the date format is not valid, returns
// UINT32_MAX (distant future).
bool EpochFromDateString(const std::string& dateString,
                         const std::string& formatString, struct tm& outEpoch);

// @return posix Epoch time in seconds (time since Jan 1, 1970, midnight UTC)
// based on the specified date string in ISO8601 format: `YYYY-MM-DDTHH:MM:SSZ`.
