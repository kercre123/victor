...
typedef enum {
  ...
  UTILMSG_BUFFEROVRN,  // We would have read/written past the end of the input
                       // buffer
...
// 'h' for short (2 bytes)
// 'c' for char (1 byte)
UtilMsgError SafeUtilMsgPack(void *dst, uint32_t dstBytesLen,
                             uint32_t *oNumBytesPacked, const char *packStr,
                             ...);
...
// 'h' for short (2 bytes)
// 'c' for char (1 byte)
UtilMsgError SafeUtilMsgUnpack(const void *src, uint32_t srcBytesLen,
                               uint32_t *oNumBytesUnpacked, const char *packStr,
                               ...);
