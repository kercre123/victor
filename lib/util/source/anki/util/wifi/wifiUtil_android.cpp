...
void BindTransportToNetwork(Anki::Util::UDPTransport* transport) {
  if (transport->IsConnected()) {
    ...
    PRINT_NAMED_INFO("WifiUtil.BindTransport", "reset socket %d",
                     transport->GetSocketId());
  }
}
...
std::string WifiUtil::GetSSID() { return _ssid; }

std::string WifiUtil::GetConnectionStatus() { return _status; }
...
void JNICALL Java_com_anki_util_WifiUtil_NativeScanCallback(JNIEnv* env,
                                                            jobject clazz,
                                                            jobjectArray ssids,
                                                            jintArray levels) {}

void JNICALL Java_com_anki_util_WifiUtil_NativeStatusCallback(JNIEnv* env,
                                                              jobject clazz,
                                                              jstring ssid,
                                                              jstring status) {
...
void JNICALL Java_com_anki_util_WifiUtil_NativeBindNetworkCallback(
    JNIEnv* env, jobject thiz, jlong netId) {
  // todo: use NDK R13 functionality to bind to netId?
  _signal.emit();
}

void JNICALL Java_com_anki_util_WifiUtil_NativeBindLollipopCallback(
