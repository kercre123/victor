...
#define JNI_INIT_WARNING(message, args...) \
  __android_log_print(ANDROID_LOG_WARN, "AndroidRuntime", message, ##args)
...
// Keeping these hidden in the definitions file in an anonymous namespace for
// simplicity
namespace {
...
jobjectArray JNIUtils::convertStringMapToJObjectArray(
  ...
  jobjectArray arr =
      env->NewObjectArray((jsize)(stringMap.size() * 2), clazz.get(), 0);
  int k = 0;
  for (std::map<std::string, std::string>::const_iterator it =
           stringMap.begin();
       it != stringMap.end(); ++it, k += 2) {
...
bool JNIUtils::getStaticBooleanFieldFromClass(JNIEnv* env, const jclass clazz,
...
int JNIUtils::getStaticIntFieldFromClass(JNIEnv* env, const jclass clazz,
                                         const char* fieldName) {
  int value = -1;
...
std::string JNIUtils::getStaticStringFieldFromClass(JNIEnv* env,
                                                    const jclass clazz,
                                                    const char* fieldName) {
  if (nullptr == env) {
    return std::string();
  ...
  jfieldID fieldId =
      env->GetStaticFieldID(clazz, fieldName, "Ljava/lang/String;");
  JStringHandle javaString{(jstring)env->GetStaticObjectField(clazz, fieldId),
                           env};

  return getStringFromJString(env, javaString.get());
}

std::string JNIUtils::getStringFromStaticObjectMethod(JNIEnv* env,
                                                      const jclass clazz,
                                                      const char* name,
                                                      const char* sig) {
  if (nullptr == env) {
...
std::string JNIUtils::getStringFromStaticObjectMethod(JNIEnv* env,
  ...
  JStringHandle javaString{(jstring)env->CallStaticObjectMethod(clazz, method),
                           env};
...
jobject JNIUtils::getUnityActivity(JNIEnv* env) {
  ...
  JClassHandle unityPlayer{env->FindClass("com/unity3d/player/UnityPlayer"),
                           env};
  ...
  jfieldID activityField = env->GetStaticFieldID(
      unityPlayer.get(), "currentActivity", "Landroid/app/Activity;");
  if (nullptr == activityField) {
    return nullptr;
  }
  jobject activity =
      env->GetStaticObjectField(unityPlayer.get(), activityField);
...
bool JNIUtils::AcquireClassLoaderWithClass(const char* randomProjectClass) {
  ...
  if (nullptr == randomClass) {
    JNI_INIT_WARNING(
        "JNIUtils.AcquireClassLoaderWithClass.RandomProjectClassNotFound %s",
        randomProjectClass);
    return false;
  }

  // Note that the "getClassLoader" method is on the "Class" class type, hence
  // this step.
  JClassHandle classClass{env->GetObjectClass(randomClass.get()), env};
  if (nullptr == classClass) {
    JNI_INIT_WARNING(
        "JNIUtils.AcquireClassLoaderWithClass.GetClassClassFailed");
    return false;
  }
  ...
  if (nullptr == classLoaderClass) {
    JNI_INIT_WARNING(
        "JNIUtils.AcquireClassLoaderWithClass.ClassLoaderClassNotFound");
    return false;
  }

  jmethodID getClassLoaderMethod = env->GetMethodID(
      classClass.get(), "getClassLoader", "()Ljava/lang/ClassLoader;");

  // Now we have all the things we need to get and store off the ClassLoader
  // reference and method to find classes with it
  _sClassLoader = JGlobalObject{
      env->CallObjectMethod(randomClass.get(), getClassLoaderMethod), env};
  _sFindClassMethod = env->GetMethodID(classLoaderClass.get(), "findClass",
...
class JNIEnvWrapperImpl : public JNIEnvWrapper {
 public:
  JNIEnvWrapperImpl(JavaVM* jvm)
    ...
    if (!ANKI_USE_JNI) {
      PRINT_NAMED_WARNING("JNIEnvWrapperImpl.Init",
                          "JVM not enabled for platform");
      return;
    }
  ...
  virtual jclass FindClassInProject(const char* className) override {
    if (nullptr == _env) {
      PRINT_NAMED_ERROR("JNIEnvWrapperImpl.FindClassInProject",
                        "Missing JNIEnv");
      return nullptr;
    }
