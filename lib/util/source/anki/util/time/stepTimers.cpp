 ...
 * Created: 07/15/14
 *
 * Description: Set of functions and structs to add 'Steps' whose time will be
 *measured and can be printed for performance evaluations.
 *
...
struct Step {
  ...
  inline double GetSplitTime() const {
    const double ret = std::chrono::duration_cast<std::chrono::milliseconds>(
                           _chronoSplit - _chronoStart)
                           .count();
    return ret;
  }
  ...
  double Print(const std::string& prefix, int levels) const {
    ...
    if (levels >= 0 && !_subs.empty()) {
      ...
      // This prints how long the substeps took, and the time parent spent not
  ...
  unsigned int PrintPercent(const std::string& prefix, double parentTime,
                            unsigned int maxWidth, bool dryRun) const {
    ...
    unsigned int width =
        numeric_cast<unsigned int>(prefix.length() + _name.length() + 3);
    ...
    if (percent > expandThreshold) {
      if (!dryRun) {
        PRINT_NAMED_INFO("StepTimer",
                         ...
                         prefix.c_str(), _name.c_str(), spacing.c_str(), time,
                         GetSplitProcessTime(), time - childTime);
      }
      ...
      for (const auto& sIt : _subs) {
        unsigned int childWidth =
            sIt.PrintPercent("  " + prefix, time, maxWidth, dryRun);
      ...
      if (!dryRun) {
        ...
        // if this is the main update loop try block, it should never be less
        // than 10% of the time
        if (_name == "BS::Update::try" && parentTime > 10.0) {
          PRINT_NAMED_WARNING("StepTimer.SlowTryBlock",
                              "Try block took %.2f milliseconds (%.2f proc), "
                              "parent time was %.2f",
                              time, GetSplitProcessTime(), parentTime);
        }
      }
      return width;
    }
  ...
  void DeepSort() {
    ...
    std::sort(_subs.begin(), _subs.end(), [](const Step& lhs, const Step& rhs) {
...
void PushTimedStep(const char* name) {
  ...
  if (!pthread_equal(sMainThreadId, pthread_self())) return;
...
void PopTimedStep(const char* name) {
  ...
  while (sStepStack.size() >
         1)  // 1 because we want ROOT to stay there  if it's there
  {
