...
double StopWatch::Stop() {
  ...
  if (count_ > 60) {
    ...
    if ((averageLimit_ > 0.5 && diffMs > averageLimit_) ||
      ...
      aboveAverageCount_++;
      aboveAverageAverage_ =
          aboveAverageAccumulator_ / (double)aboveAverageCount_;
      if (diffMs > max_) max_ = diffMs;
    }
  }
...
#define SEND_STOPWATCH_STATS(eventNameThird, stat)           \
  {                                                          \
    string eventName("StopWatch." + id_ + eventNameThird);   \
    PRINT_NAMED_INFO(eventName.c_str(), "%f", (double)stat); \
  }

  SEND_STOPWATCH_STATS(".f_average", average_);
  SEND_STOPWATCH_STATS(".i_tickCount", count_);
  SEND_STOPWATCH_STATS(".f_aboveAveragePercent",
                       (double)aboveAverageCount_ / (double)count_);
