...
class QuestRule {
 public:
  QuestRule(const std::string& ruleId,
            const std::vector<std::string>& eventNames,
            const std::string& titleKey, const std::string& descriptionKey,
            const std::string& iconImage, const std::string& cellType,
            const std::string& link, AbstractCondition* availableCondition,
            AbstractCondition* triggerCondition, AbstractAction* action);
  ~QuestRule();
