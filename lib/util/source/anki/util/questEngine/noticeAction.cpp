...
NoticeAction::NoticeAction(const bool force, const uint8_t priority,
...
void NoticeAction::PerformAction(QuestEngine& questEngine) {
  time_t now = time(0);
  QuestNotice questNotice(_force, _priority, now, "", _target, _titleKey,
                          _descriptionKey, _buttonKey, _navigationAction);
