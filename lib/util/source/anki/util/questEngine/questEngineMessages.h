...
// Full command line: ../../../tools/message-buffers/emitters/CPP_emitter.py -C
// . -o .. ./util/questEngine/questEngineMessages.clad
...
struct NoticePair {
  ...
  explicit NoticePair(bool force, uint8_t priority, double noticeTime,
...
struct EventPair {
  ...
  explicit EventPair(const std::string& eventName, uint32_t count)
      : eventName(eventName), count(count) {}
...
struct TriggerLog {
  ...
  explicit TriggerLog(const std::string& ruleId,
      ...
      : ruleId(ruleId), triggerTimes(triggerTimes) {}
...
struct QuestStore {
  ...
  explicit QuestStore(
      const std::vector<Anki::Util::QuestEngine::EventPair>& eventStats,
      const std::vector<Anki::Util::QuestEngine::NoticePair>& notices,
      const std::vector<Anki::Util::QuestEngine::TriggerLog>& ruleTriggers)
      : eventStats(eventStats), notices(notices), ruleTriggers(ruleTriggers) {}
