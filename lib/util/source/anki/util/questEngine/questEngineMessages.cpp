...
bool NoticePair::operator==(const NoticePair& other) const {
  return (this->force == other.force && this->priority == other.priority &&
          this->noticeTime == other.noticeTime && this->guid == other.guid &&
          this->target == other.target && this->titleKey == other.titleKey &&
...
EventPair::EventPair(const CLAD::SafeMessageBuffer& buffer) { Unpack(buffer); }
...
bool EventPair::operator==(const EventPair& other) const {
  return (this->eventName == other.eventName && this->count == other.count);
}
...
size_t QuestStore::Unpack(const CLAD::SafeMessageBuffer& buffer) {
  buffer.ReadCompoundTypeVArray<Anki::Util::QuestEngine::EventPair, uint16_t>(
      this->eventStats);
  buffer.ReadCompoundTypeVArray<Anki::Util::QuestEngine::NoticePair, uint16_t>(
      this->notices);
  buffer.ReadCompoundTypeVArray<Anki::Util::QuestEngine::TriggerLog, uint16_t>(
      this->ruleTriggers);
  return buffer.GetBytesRead();
}
