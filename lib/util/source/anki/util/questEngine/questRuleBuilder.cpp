 ...
 * Created: 05/11/15
 *
 * Description: Builder class responsible for parsing JSON rules definition and
 *configuring quest engine
 *
 *
...
void QuestRuleBuilder::AddActionHandler(const std::string& actionType,
                                        const ActionHandler handler) {
  _actionHandlers[actionType] = handler;
}

void QuestRuleBuilder::AddConditionHandler(const std::string& conditionType,
                                           const ConditionHandler handler) {
  _conditionHandlers[conditionType] = handler;
}

bool QuestRuleBuilder::LoadDefinitions(const std::string& fileName,
  ...
  if (parsingSuccessful) {
    ...
    for (Json::ValueIterator it = rulesArray.begin(); it != rulesArray.end();
         ++it) {
      Json::Value item = (*it);
...
QuestRule* QuestRuleBuilder::BuildRule(const Json::Value& item) {
  ...
  if (action != nullptr && !ruleId.empty() && !eventNames.empty()) {
    rule = new QuestRule(ruleId, eventNames, titleKey, descriptionKey,
                         iconImage, cellType, link, availabilityCondition,
...
AbstractCondition* QuestRuleBuilder::BuildCondition(const Json::Value& item) {
  const std::string& conditionType =
      item.get(kQuestRuleConditionTypeKey, "").asString();
...
AbstractAction* QuestRuleBuilder::BuildAction(const Json::Value& item) {
  const std::string& actionType =
      item.get(kQuestRuleActionTypeKey, "").asString();
...
void QuestRuleBuilder::LoadDefaultActionHandlers() {
  const ActionHandler noticeHandler = [](const Json::Value& item) {
    ...
    const uint8_t priority =
        attributes.get(kQuestRuleNoticePriorityKey, 0).asUInt();
    const std::string& target =
        attributes.get(kQuestRuleNoticeTargetKey, "").asString();
    const std::string& titleKey =
        attributes.get(kQuestRuleNoticeTitleKey, "").asString();
    const std::string& descriptionKey =
        attributes.get(kQuestRuleNoticeDescriptionKey, "").asString();
    const std::string& buttonKey =
        attributes.get(kQuestRuleNoticeButtonKey, "").asString();
    const std::string& navigationAction =
        attributes.get(kQuestRuleNoticeNavigationActionKey, "").asString();
    NoticeAction* noticeAction =
        new NoticeAction(force, priority, target, titleKey, descriptionKey,
                         buttonKey, navigationAction);
    return noticeAction;
  };
  ...
  const ActionHandler resetHandler = [](const Json::Value& item) {
    ...
    const std::string& eventName =
        attributes.get(kQuestRuleActionAttributeEventName, "").asString();
...
void QuestRuleBuilder::LoadDefaultConditionHandlers() {
  const ConditionHandler combinationHandler = [this](const Json::Value& item) {
    ...
    const std::string& combinationOperation =
        attributes.get("operation", "").asString();
  ...
  const ConditionHandler countThresholdHandler = [](const Json::Value& item) {
    ...
    const std::string& operatorStr =
        attributes.get(kQuestRuleAttributeOperatorKey, "").asString();
    uint16_t targetValue =
        (uint16_t)attributes.get(kQuestRuleAttributeTargetValueKey, 0).asUInt();
    const std::string& triggerKey =
        attributes.get(kQuestRuleAttributeTriggerKeyKey, "").asString();
    ...
    CountThresholdCondition* countThresholdCondition =
        new CountThresholdCondition(countOperator, targetValue, triggerKey);
