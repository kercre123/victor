...
class QuestEngine : public noncopyable {
 public:
  ...
  RuleAvailableSignal* GetRuleAvailableSignal() const {
    return _availableSignal;
  };

  RuleTriggeredSignal* GetRuleTriggeredSignal() const {
    return _triggeredSignal;
  };

 private:
  void AppendTriggerHistory(const std::string& ruleId,
