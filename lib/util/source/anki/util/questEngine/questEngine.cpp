...
void QuestEngine::IncrementEvent(const std::string& eventName,
...
void QuestEngine::Load() {
  ...
  if (success && body.size() > 0) {
    ...
    if (marker != kPersistentFileMarker) {
      ...
      _countMap.clear();
      for (auto eventIterator = store.eventStats.begin();
           eventIterator != store.eventStats.end(); ++eventIterator) {
        const std::string& eventName = eventIterator->eventName;
        _countMap[eventName] = eventIterator->count;
      }

      _notices.clear();
      for (auto noticeIterator = store.notices.begin();
           noticeIterator != store.notices.end(); ++noticeIterator) {
        ...
        QuestNotice questNotice(noticeForce, noticePriority, noticeTime, guid,
                                target, titleKey, descriptionKey, buttonKey,
                                navigationAction);
        _notices.insert(questNotice);
      }

      _triggerHistoryMap.clear();
      for (auto triggerIterator = store.ruleTriggers.begin();
           triggerIterator != store.ruleTriggers.end(); ++triggerIterator) {
        const std::string& ruleId = triggerIterator->ruleId;
        const std::vector<double>& log = triggerIterator->triggerTimes;
        for (auto historyIterator = log.begin(); historyIterator != log.end();
             ++historyIterator) {
...
void QuestEngine::Save() {
  QuestStore store;
  for (auto eventIterator = _countMap.begin(); eventIterator != _countMap.end();
       ++eventIterator) {
    store.eventStats.emplace_back(eventIterator->first, eventIterator->second);
  }
  for (auto noticeIterator = _notices.begin(); noticeIterator != _notices.end();
       ++noticeIterator) {
    const QuestNotice& questNotice = *noticeIterator;
    store.notices.emplace_back(
        questNotice.IsForce(), questNotice.GetPriority(),
        questNotice.GetNoticeTime(), questNotice.GetGuid(),
        questNotice.GetTarget(), questNotice.GetTitleKey(),
        questNotice.GetDescriptionKey(), questNotice.GetButtonKey(),
        questNotice.GetNavigationAction());
  }
  for (auto triggerIterator = _triggerHistoryMap.begin();
       triggerIterator != _triggerHistoryMap.end(); ++triggerIterator) {
    TriggerLog log;
    log.ruleId = triggerIterator->first;
    const TriggerHistory& history = triggerIterator->second;
    for (auto historyIterator = history.begin();
         historyIterator != history.end(); ++historyIterator) {
...
bool QuestEngine::AddRule(QuestRule* rule) {
  ...
  if (!eventNames.empty() && !ruleId.empty() &&
      _ruleIds.find(ruleId) == _ruleIds.end()) {
    _rules.push_back(rule);
    _ruleIds.insert(ruleId);
    for (auto eventNamesIterator = eventNames.begin();
         eventNamesIterator != eventNames.end(); ++eventNamesIterator) {
...
const std::vector<Util::QuestEngine::QuestRule*> QuestEngine::GetAvailableRules(
    std::tm& time) {
...
void QuestEngine::AppendTriggerHistory(const std::string& ruleId,
                                       const time_t triggerTime) {
