...
bool FileUtils::CreateDirectory(const std::string& path,
                                const bool stripFilename, bool dashP,
                                const mode_t mode) {
  std::string workPath;
  if (stripFilename) {
...
std::vector<std::string> FileUtils::FilesInDirectory(const std::string& path,
  ...
  return FileUtils::FilesInDirectory(path, useFullPath, withExtensions,
                                     recurse);
}

std::vector<std::string> FileUtils::FilesInDirectory(
    const std::string& path, bool useFullPath,
    const std::vector<const char*>& withExtensions, bool recurse) {
  // We always want to useFullPath when looking for files recursively
  useFullPath |= recurse;

  std::vector<std::string> files;

  std::function<bool(const char* fname)> FilterFilename = [](const char*) {
    return true;
  };
  ...
  if (listingDir) {
    ...
    while ((info = readdir(listingDir))) {
      ...
      } else if (recurse && info->d_type == DT_DIR &&
                 strcmp(info->d_name, ".") != 0 &&
                 strcmp(info->d_name, "..") != 0) {
        // Note we simply pass true as the recurse and useFullPath values here,
        // since we already know we want to recurse
        auto recurseResults = FilesInDirectory(
            FullFilePath({path, info->d_name}), true, withExtensions, true);
...
std::string FileUtils::ReadFile(const std::string& fileName) {
  ...
  if (fileIn.is_open()) {
    std::string body((std::istreambuf_iterator<char>(fileIn)),
...
bool FileUtils::WriteFile(const std::string& fileName,
                          const std::vector<uint8_t>& body, bool append) {
  bool success = false;
...
bool FileUtils::WriteFileAtomic(const std::string& fileName,
...
bool FileUtils::WriteFileAtomic(const std::string& fileName,
  ...
  bool success = WriteFile(tmpFileName, body) &&
                 (0 == rename(tmpFileName.c_str(), fileName.c_str()));
  if (!success) {
    DeleteFile(tmpFileName);
  }
  return success;
...
static bool CopyOrMoveFile(const std::string& dest,
  ...
  if (!maxBytesToCopyFromEnd && deleteSource) {
    // If we are moving the file on the same mounted filesystem it should
    // succeed
    ...
    // A simple rename won't work.  The source and destination are on two
    // different mounted filesystems.  A copy is required.
  }
...
bool FileUtils::CopyFile(const std::string& dest,
                         const std::string& srcFileName,
                         const int maxBytesToCopyFromEnd) {
  return CopyOrMoveFile(dest, srcFileName, maxBytesToCopyFromEnd,
                        false /* deleteSource */);
}

bool FileUtils::MoveFile(const std::string& dest,
                         const std::string& srcFileName) {
...
void FileUtils::ListAllDirectories(const std::string& path,
...
bool FileUtils::FilenameHasSuffix(const char* inFilename,
                                  const char* inSuffix) {
...
long FileUtils::GetFileLastModificationTime(const std::string& path) {
  ...
  if (stat(path.c_str(), &info) == 0) {
    ...
    // but the older version used here has backwards compatibility on both
    // systems, as long as we don't need nanosecond resolution...which we don't.
