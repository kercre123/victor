...
// This alternative error logging define imported from our DAS library
// implementation
#define LOGD(...)                 \
  {                               \
...
RollingFileLogger::RollingFileLogger(Dispatch::Queue* queue,
...
RollingFileLogger::RollingFileLogger(Dispatch::create_queue_t,
                                     const std::string& baseDirectory,
                                     const std::string& extension,
                                     std::size_t maxFileSize)
...
void RollingFileLogger::Write(std::string message) {
  ExecuteBlock(
      [this, message = std::move(message)] { WriteInternal(message); });
}
...
void RollingFileLogger::WriteInternal(const std::string& message) {
  ...
  if (!_currentLogFileHandle.is_open() ||
      (messageSize + _numBytesWritten) > _maxFileSize) {
    RollLogFile();
    _numBytesWritten = 0;
  }
...
void RollingFileLogger::RollLogFile() {
  ...
  _currentLogFileHandle.open(nextFilename,
                             std::ofstream::out | std::ofstream::app);

  if (!_currentLogFileHandle) {
    LOGD("Error getting handle for file %s: %s !!", nextFilename.c_str(),
         strerror(errno));
  } else {
...
std::string RollingFileLogger::GetDateTimeString(
    const ClockType::time_point& time) {
  std::ostringstream stringStream;
  auto currTime_t = GetTimeT(time);
  auto numSecs =
      std::chrono::duration_cast<std::chrono::seconds>(time.time_since_epoch());
  auto millisLeft = std::chrono::duration_cast<std::chrono::milliseconds>(
      (time - numSecs).time_since_epoch());
...
RollingFileLogger::GetSystemClockTimePoint(const ClockType::time_point& time) {
  static const std::chrono::system_clock::time_point systemClockNow =
      std::chrono::system_clock::now();
  static const ClockType::time_point clockTypeNow = ClockType::now();

  const auto timeDiff =
      std::chrono::duration_cast<std::chrono::system_clock::duration>(
          time.time_since_epoch() - clockTypeNow.time_since_epoch());
...
void RollingFileLogger::Flush() {
  if (nullptr != _dispatchQueue) {
    Dispatch::Sync(_dispatchQueue, [this] { FlushInternal(); });
  } else {
    FlushInternal();
  }
}
