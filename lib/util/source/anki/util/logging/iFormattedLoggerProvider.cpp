...
static void thread_id_init() { pthread_key_create(&thread_id_key, nullptr); }
#endif
...
void IFormattedLoggerProvider::SetMinLogLevel(LogLevel logLevel) {
  DEV_ASSERT(logLevel < _logLevelEnabledFlags.size(),
             "IFormattedLoggerProvider.SetMinLogLevel.InvalidLogLevel");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void IFormattedLoggerProvider::SetLogLevelEnabled(LogLevel logLevel,
                                                  bool enabled) {
  DEV_ASSERT(logLevel < _logLevelEnabledFlags.size(),
             "IFormattedLoggerProvider.SetLogLevelEnabled.InvalidLogLevel");
  _logLevelEnabledFlags[logLevel] = enabled;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void IFormattedLoggerProvider::ParseLogLevelSettings(
    const Json::Value& config) {
  // parse config
  if (!config.isNull()) {
    for (const auto& logLevelInfo : config[kLevelListKey]) {
      // parse channel name
      DEV_ASSERT(logLevelInfo[kLevelNameKey].isString(),
                 "IFormattedLoggerProvider.ParseLogLevelSettings.BadName");
      ...
      if (logLevelVal < _LOG_LEVEL_COUNT) {
        // parse value
        DEV_ASSERT(
            logLevelInfo[kLevelEnabledKey].isBool(),
            "IFormattedLoggerProvider.ParseLogLevelSettings.BadEnableFlag");
...
bool IFormattedLoggerProvider::IsLogLevelEnabled(LogLevel logLevel) const {
  DEV_ASSERT(logLevel < _logLevelEnabledFlags.size(),
             "IFormattedLoggerProvider.IsLogLevelEnabled.InvalidLogLevel");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void IFormattedLoggerProvider::FormatAndLog(
    IFormattedLoggerProvider::LogLevel logLevel, const char* eventName,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void IFormattedLoggerProvider::FormatAndLogChanneled(
  ...
  uint32_t thread_id = numeric_cast_clamped<uint32_t>(
      (uintptr_t)pthread_getspecific(thread_id_key));
  if (0 == thread_id) {
  ...
  const bool printDasExtras =
      !PRINT_DAS_EXTRAS_FOR_EVENT_LEVEL_ONLY ||
  ...
  if (printDasExtras) {
    for (const auto& keyValuePair : keyValues) {
      snprintf(logString, kMaxStringBufferSize, "%s[%s: %s] ", logString,
  ...
  ASSERT_NAMED(
      eventName != nullptr,
      "IFormattedLoggerProvider.FormatAndLogChanneled logging null eventName");
  ASSERT_NAMED(
      eventValue != nullptr,
      "IFormattedLoggerProvider.FormatAndLogChanneled logging null eventValue");
