...
class VictorLogger : public ILoggerProvider, public IEventProvider {
 public:
  ...
  virtual void PrintLogE(const char* name, const KVPairVector& keyvals,
                         const char* strval) {
    LogError(ANDROID_LOG_ERROR, name, keyvals, strval);
  }

  virtual void PrintLogW(const char* name, const KVPairVector& keyvals,
                         const char* strval) {
    Log(ANDROID_LOG_WARN, name, keyvals, strval);
  }

  virtual void PrintLogI(const char* channel, const char* name,
                         const KVPairVector& keyvals, const char* strval) {
    Log(ANDROID_LOG_INFO, channel, name, keyvals, strval);
  }

  virtual void PrintLogD(const char* channel, const char* name,
                         const KVPairVector& keyvals, const char* strval) {
    Log(ANDROID_LOG_DEBUG, channel, name, keyvals, strval);
  }

  virtual void PrintEvent(const char* name, const KVPairVector& keyvals,
                          const char* strval) {
 ...
 private:
  ...
  void Log(android_LogPriority prio, const char* name,
           const KVPairVector& keyvals, const char* strval);

  void Log(android_LogPriority prio, const char* channel, const char* name,
           const KVPairVector& keyvals, const char* strval);

  void LogEvent(android_LogPriority prio, const char* name,
                const KVPairVector& keyvals);

  void LogError(android_LogPriority prio, const char* name,
                const KVPairVector& keyvals, const char* strval);
