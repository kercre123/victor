...
static std::string Demangle(const std::string& backtraceFrame) {
  ...
  // -4 because the valid statuses from __cxa_demangle() is 0, -1, -2, -3. If we
  // got -4 after calling __cxa_demangle() something very wrong happened.
  int status = -4;
  ...
  if (splitFrame.size() > 3) {
    ...
    // "0   webotsCtrlGameEngine                0x000000010cc84894
    // _ZN4Anki4Util14sDumpCallstackERKNSt3__112basic_stringIcNS1_11char_traitsIcEENS1_9allocatorIcEEEE
    // + 52" The mangled symbol is the third element from the back, when split
    // by ' '
    //
    // Use a tempPtr here in case __cxa_demangle cannot demangle the given
    // string and returns a nullptr.
    char* tempPtr =
        abi::__cxa_demangle(splitFrame.end()[-3].c_str(), 0, 0, &status);

    switch (status) {
      case 0:
        assert(tempPtr != nullptr);
        // Everything is fine, proceed to change the mangled name into the
        // demangled version.
      ...
      default: {
        ...
        PRINT_NAMED_WARNING(
            "Callstack.Demangle",
            "Couldn't demangle the symbol, __cxa_demangle returned status = %i",
            status);
        break;
      }
    }
    ...
    PRINT_NAMED_WARNING(
        "Callstack.Demangle",
        "Something is wrong with the format of the backtrace frame. It should "
        "look "
        "something like "
        "'0   webotsCtrlGameEngine                0x000000010cc84894 "
        "_ZN4Anki4Util14sDumpCallstackERKNSt3__112basic_stringIcNS1_11char_"
        "traitsIcEENS1_9allocatorIcEEEE + 52'");
  }
...
static std::string Demangle(const char* frame) {
  ...
  //   /anki/lib/libankiutil.so(_ZN4Anki4Util14sDumpCallstackERKNSt3__112basic_stringIcNS1_11char_traitsIcEENS1_9allocatorIcEEEE+0x23)
  //   [0xb03d4920] /anki/bin/vic-anim(+0x3dcc2) [0x7f592cc2]
  // or generically
...
void sDumpCallstack(const std::string& name) {
#ifdef USE_BACKTRACE
  {
    ...
    for (int i = 0; i < frames; ++i) {
      PRINT_CH_INFO("Unfiltered", name.c_str(), "%s",
