...
class IFormattedLoggerProvider : public ILoggerProvider {
 public:
  ...
  inline void PrintEvent(
      ...
      const std::vector<std::pair<const char*, const char*>>& keyValues,
      const char* eventValue) override {
    if (!IsLogLevelEnabled(LOG_LEVEL_EVENT)) {
      return;
    }
    FormatAndLog(LOG_LEVEL_EVENT, eventName, keyValues, eventValue);
  };
  inline void PrintLogI(
      const char* channelName, const char* eventName,
      const std::vector<std::pair<const char*, const char*>>& keyValues,
      const char* eventValue) override {
    if (!IsLogLevelEnabled(LOG_LEVEL_INFO)) {
      return;
    }
    FormatAndLogChanneled(LOG_LEVEL_INFO, channelName, eventName, keyValues,
                          eventValue);
  };
  inline void PrintLogD(
      const char* channelName, const char* eventName,
      const std::vector<std::pair<const char*, const char*>>& keyValues,
      const char* eventValue) override {
    ...
    FormatAndLogChanneled(LOG_LEVEL_DEBUG, channelName, eventName, keyValues,
                          eventValue);
  }

  // sets the minimum log level that is enabled. Levels above this one will also
  // be enabled
  ...
  virtual void Log(ILoggerProvider::LogLevel logLevel,
 ...
 private:
  ...
  void FormatAndLog(
      ILoggerProvider::LogLevel logLevel, const char* eventName,
      const std::vector<std::pair<const char*, const char*>>& keyValues,
      const char* eventValue);
  void FormatAndLogChanneled(
      ILoggerProvider::LogLevel logLevel, const char* channel,
      const char* eventName,
      const std::vector<std::pair<const char*, const char*>>& keyValues,
      const char* eventValue);
