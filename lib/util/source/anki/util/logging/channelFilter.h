...
struct ChannelVar : public ConsoleVar<bool> {
  ChannelVar(const std::string& name, bool defaultEnable,
      ...
      : ConsoleVar<bool>(enable, name.c_str(), "Channels",
                         unregisterInDestructor),
        enable(defaultEnable) {
    // We must re-set the default value here, after the call to ConsoleVar
    // constructor, because that constructor uses a reference to "enable", but
    // enable isn't set until after that constructor call.  VIC-13609
