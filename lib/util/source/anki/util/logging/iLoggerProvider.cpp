...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ILoggerProvider::SetFilter(
    const std::shared_ptr<const IChannelFilter>& infoFilter) {
  _infoFilter = infoFilter;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ILoggerProvider::PrintChanneledLogI(
...
void ILoggerProvider::PrintChanneledLogD(
    const char* channel, const char* eventName,
    const std::vector<std::pair<const char*, const char*>>& keyValues,
    const char* eventValue) {
  // if no filter is set or the channel is enabled
