...
// Log an error event
__attribute__((__used__)) void sLogError(const DasMsg& dasMessage);

// Log a warning event
__attribute__((__used__)) void sLogWarning(const DasMsg& dasMessage);

// Log an info event
__attribute__((__used__)) void sLogInfo(const DasMsg& dasMessage);

// Log a debug event
__attribute__((__used__)) void sLogDebug(const DasMsg& dasMessage);
...
// If this value changes, or event parameters change, DASManager must be updated
// to match. This is declared as a macro, not a constexpr, so it can be expanded
// by doxygen.
//
...
// If event name or event parameters change, DASManager must be updated to
// match. Names are defined as a macro, not a constexpr, so they can be expanded
// by doxygen.
//
...
// These events are used to indicate start/stop of association with an Anki
// profile ID. If event name or event parameters change, DASManager must be
// updated to match. Names are defined as a macro, not a constexpr, so they can
// be expanded by doxygen.
//
...
//
// Overwriting fields that have already been set (e.g. calling DASMSG_SET(i1,
// ...) twice) is not allowed.
//
// If a string field can contain JSON syntax characters such as double quotes
// (") or backslash (\), it should be escaped with DASMSG_ESCAPE(). Most string
// fields are JSON-safe so we do not do this automatically.
//
#ifndef DOXYGEN

#define DASMSG(ezRef, eventName, documentation) \
  {                                             \
    Anki::Util::DasMsg __DAS_msg(eventName);
#define DASMSG_SET(dasEntry, value, comment)                               \
  __DAS_msg.dasEntry = Anki::Util::DasItem(value);                         \
  /* define an empty struct (to detect if this item has been set twice) */ \
  struct _already_set_##dasEntry {};
#define DASMSG_SEND()              \
  Anki::Util::sLogInfo(__DAS_msg); \
  }
#define DASMSG_SEND_WARNING()         \
  Anki::Util::sLogWarning(__DAS_msg); \
  }
#define DASMSG_SEND_ERROR()         \
  Anki::Util::sLogError(__DAS_msg); \
  }
#define DASMSG_SEND_DEBUG()         \
  Anki::Util::sLogDebug(__DAS_msg); \
  }
