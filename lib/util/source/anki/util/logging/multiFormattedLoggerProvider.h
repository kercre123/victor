...
class MultiFormattedLoggerProvider : public MultiLoggerProvider {
 public:
  inline MultiFormattedLoggerProvider(
      const std::vector<IFormattedLoggerProvider*>& inVec)
      : MultiLoggerProvider({}) {
  ...
  MultiFormattedLoggerProvider& operator=(const MultiFormattedLoggerProvider&) =
      delete;
  ...
  inline void Log(ILoggerProvider::LogLevel logLevel,
                  const std::string& logMessage) {
    for (ILoggerProvider* provider : _providers) {
      IFormattedLoggerProvider* formatted_provider =
          dynamic_cast<IFormattedLoggerProvider*>(provider);
