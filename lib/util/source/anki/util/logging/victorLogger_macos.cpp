...
void VictorLogger::Log(const char * prio,
  const char * channel,
  const char * name,
  ...
  const char * strval)
{
  ...
  printf("%s %s [@%s] %s: %s\n", prio, _tag.c_str(), channel, name, strval);
}
  ...
  const char * strval)
{
  ...
  printf( "%s %s %s: %s\n", prio, _tag.c_str(), name, strval);
}

void VictorLogger::LogEvent(const char * prio,
  const char * name,
  const KVPairVector & keyvals)
{
  ...
  printf("%s %s @%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%llu\n",
                      prio, _tag.c_str(), name, str1, str2, str3, str4, int1, int2, int3, int4, uptime_ms);
...
//
inline const char * GetLogPrio(LogLevel level)
{
  const char * prio = nullptr;
  switch (level)
  {
  ...
  DEV_ASSERT(prio != nullptr, "VictorLogger.GetLogPrio.UnknownLogLevel");
  return prio;
}
...
void VictorLogger::LogEvent(LogLevel level, const DasMsg & dasMsg)
{
  ...
  printf("%s %s @%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%llu\n",
                      prio, _tag.c_str(), dasMsg.event.c_str(), dasMsg.s1.c_str(), dasMsg.s2.c_str(), dasMsg.s3.c_str(), dasMsg.s4.c_str(),
