 ...
 *should stack them into one set of functions that use LogLevel as a param,
...
#else
static inline bool DropBreadcrumb(bool result, const char*, int) {
  return result;
}
#endif
...
// Only useful if access to the global error flag involves some locking
// mechanism
void sPushErrG(bool value);  // Sets errG to value and locks access by other
                             // threads until PopErrG is called
void sPopErrG();  // Restores errG to its value before calling sPushErrG and
                  // enables access to errG by other threads

// Global flag to control break-on-error behavior
extern bool _errBreakOnError;

// If true, access to the global error flag uses a mutex device. Changing this
// value while logging could lead to a mutex lock, so don't.
extern bool _lockErrG;

//
// "Event level" logging is no longer a thing. Do not use it.
// Messages intended for DAS should use the explicit DASMSG interface declared
// by util/logging/DAS.h.
//
__attribute__((__deprecated__)) __attribute__((__used__)) void sEventF(
    const char* name, const KVPairVector& keyvals, const char* format, ...)
    __attribute__((format(printf, 3, 4)));

__attribute__((__deprecated__)) __attribute__((__used__)) void sEventV(
    const char* name, const KVPairVector& keyvals, const char* format,
    va_list args) __attribute__((format(printf, 3, 0)));

__attribute__((__used__)) void sEvent(const char* name,
                                      const KVPairVector& keyvals,
                                      const char* strval);

__attribute__((__used__)) void sErrorF(const char* name,
                                       const KVPairVector& keyvals,
                                       const char* format, ...)
    __attribute__((format(printf, 3, 4)));

__attribute__((__used__)) void sErrorV(const char* name,
                                       const KVPairVector& keyvals,
                                       const char* format, va_list args)
    __attribute__((format(printf, 3, 0)));

__attribute__((__used__)) void sError(const char* name,
                                      const KVPairVector& keyvals,
                                      const char* strval);

__attribute__((__used__)) void sWarningF(const char* name,
                                         const KVPairVector& keyvals,
                                         const char* format, ...)
    __attribute__((format(printf, 3, 4)));

__attribute__((__used__)) void sWarningV(const char* name,
                                         const KVPairVector& keyvals,
                                         const char* format, va_list args)
    __attribute__((format(printf, 3, 0)));

__attribute__((__used__)) void sWarning(const char* name,
                                        const KVPairVector& keyvals,
                                        const char* strval);

__attribute__((__used__)) void sInfoF(const char* name,
                                      const KVPairVector& keyvals,
                                      const char* format, ...)
    __attribute__((format(printf, 3, 4)));

__attribute__((__used__)) void sInfoV(const char* name,
                                      const KVPairVector& keyvals,
                                      const char* format, va_list args)
    __attribute__((format(printf, 3, 0)));

__attribute__((__used__)) void sInfo(const char* name,
                                     const KVPairVector& keyvals,
                                     const char* strval);

__attribute__((__used__)) void sChanneledInfoF(const char* channel,
                                               const char* name,
                                               const KVPairVector& keyvals,
                                               const char* format, ...)
    __attribute__((format(printf, 4, 5)));

__attribute__((__used__)) void sChanneledInfoV(const char* channel,
                                               const char* name,
                                               const KVPairVector& keyvals,
                                               const char* format, va_list args)
    __attribute__((format(printf, 4, 0)));

__attribute__((__used__)) void sChanneledInfo(const char* channel,
                                              const char* name,
                                              const KVPairVector& keyvals,
                                              const char* strval);

__attribute__((__used__)) void sChanneledDebugF(const char* channel,
                                                const char* name,
                                                const KVPairVector& keyvals,
                                                const char* format, ...)
    __attribute__((format(printf, 4, 5)));

__attribute__((__used__)) void sChanneledDebugV(
    const char* channel, const char* name, const KVPairVector& keyvals,
    const char* format, va_list args) __attribute__((format(printf, 4, 0)));

__attribute__((__used__)) void sChanneledDebug(const char* channel,
                                               const char* name,
                                               const KVPairVector& keyvals,
                                               const char* strval);
...
__attribute__((__used__)) bool sVerifyFailedReturnFalse(const char* file,
                                                        int line,
                                                        const char* name,
                                                        const char* format, ...)
    __attribute__((format(printf, 4, 5)));
...
#define PRINT_NAMED_ERROR(name, format, ...)                          \
  do {                                                                \
    if (ANKITRACE_ENABLED) {                                          \
      ...
      snprintf(PRINT_LOG_VAR_logString, MAX_LOG_STRING_LEN, format,   \
               ##__VA_ARGS__);                                        \
...
#define PRINT_NAMED_INFO(name, format, ...)                                 \
  do {                                                                      \
    if (ANKITRACE_ENABLED) {                                                \
      ...
      ::Anki::Util::sChanneledInfoF(name, name, {}, "%s",                   \
                                    PRINT_LOG_VAR_logString);               \
    } else {                                                                \
...
#define PRINT_NAMED_DEBUG(name, format, ...)                                 \
  do {                                                                       \
    if (ANKITRACE_ENABLED) {                                                 \
      ...
      ::Anki::Util::sChanneledDebugF(name, name, {}, "%s",                   \
                                     PRINT_LOG_VAR_logString);               \
    } else {                                                                 \
...
// Similar to DEV_ASSERT (below) but enabled in both debug and release (and
// shipping) builds.
//
// If the conditional expression (expr) is true, ANKI_VERIFY returns true.
// If the conditional expression (expr) is false, ANKI_VERIFY logs an error
// message, dumps the callstack, and returns false.
//
// The conditional expression (expr) will be evaluated only once. Arguments
// required to produce the formatted string for the log will only be evaluated
// if expr==false.
//
// Example 1:
// Use
...
//   }
//
// Note that "&& false" is used to inform static analysis that the "verify
// failed" branch always returns false. This prevents analyzers from generating
// bogus warnings caused by impossible code paths.
//
#define ANKI_VERIFY(expr, name, format, ...)                                \
  (expr ? ::Anki::Util::sVerifySucceededReturnTrue(__FILE__, __LINE__)      \
        : (::Anki::Util::sVerifyFailedReturnFalse(__FILE__, __LINE__, name, \
                                                  "VERIFY(%s): " format,    \
                                                  #expr, ##__VA_ARGS__) &&  \
           false))
...
//
#define PRINT_CH_INFO(channel, name, format, ...)                              \
  do {                                                                         \
    if (ANKITRACE_ENABLED) {                                                   \
      char PRINT_LOG_VAR_logString[MAX_LOG_STRING_LEN];                        \
      snprintf(PRINT_LOG_VAR_logString, MAX_LOG_STRING_LEN, format,            \
               ##__VA_ARGS__);                                                 \
      tracelog(TRACE_INFO, "%s %s %s", channel, name,                          \
               PRINT_LOG_VAR_logString);                                       \
      ::Anki::Util::sChanneledInfoF(channel, name, {}, "%s",                   \
                                    PRINT_LOG_VAR_logString);                  \
    } else {                                                                   \
...
#define PRINT_CH_DEBUG(channel, name, format, ...)                  \
  do {                                                              \
    if (ANKITRACE_ENABLED) {                                        \
      ...
      snprintf(PRINT_LOG_VAR_logString, MAX_LOG_STRING_LEN, format, \
               ##__VA_ARGS__);                                      \
      tracelog(TRACE_DEBUG, "%s %s %s", channel, name,              \
               PRINT_LOG_VAR_logString);                            \
      ::Anki::Util::sChanneledDebugF(channel, name, {}, "%s",       \
                                     PRINT_LOG_VAR_logString);      \
    } else {                                                        \
      ::Anki::Util::sChanneledDebugF(channel, name, {}, format,     \
                                     ##__VA_ARGS__);                \
    }                                                               \
    ::Anki::Util::DropBreadcrumb(false, __FILE__, __LINE__);        \
  } while (0)
...
#define PRINT_PERIODIC_CH_INFO(period, channel, name, format, ...)           \
  if (ANKITRACE_ENABLED) {                                                   \
    ...
    snprintf(PRINT_LOG_VAR_logString, MAX_LOG_STRING_LEN, format,            \
             ##__VA_ARGS__);                                                 \
    tracelog(TRACE_INFO, "%d %s %s %s", period, channel, name,               \
             PRINT_LOG_VAR_logString);                                       \
    PRINT_PERIODIC_CH_HELPER(sChanneledInfoF, period, channel, name, "%s",   \
                             PRINT_LOG_VAR_logString)                        \
  } else {                                                                   \
    PRINT_PERIODIC_CH_HELPER(sChanneledInfoF, period, channel, name, format, \
                             ##__VA_ARGS__)                                  \
  }
...
#define PRINT_PERIODIC_CH_DEBUG(period, channel, name, format, ...)           \
  if (ANKITRACE_ENABLED) {                                                    \
    ...
    snprintf(PRINT_LOG_VAR_logString, MAX_LOG_STRING_LEN, format,             \
             ##__VA_ARGS__);                                                  \
    tracelog(TRACE_DEBUG, "%d %s %s %s", period, channel, name,               \
             PRINT_LOG_VAR_logString);                                        \
    PRINT_PERIODIC_CH_HELPER(sChanneledDebugF, period, channel, name, "%s",   \
                             PRINT_LOG_VAR_logString)                         \
  } else {                                                                    \
    PRINT_PERIODIC_CH_HELPER(sChanneledDebugF, period, channel, name, format, \
                             ##__VA_ARGS__)                                   \
  }

// Streams
#define PRINT_STREAM_ERROR(name, args)                       \
  do {                                                       \
    std::stringstream ss;                                    \
    ss << args;                                              \
...
#define PRINT_STREAM_WARNING(name, args)                      \
  do {                                                        \
    std::stringstream ss;                                     \
    ss << args;                                               \
...
#define PRINT_STREAM_INFO(name, args)                               \
  do {                                                              \
    std::stringstream ss;                                           \
    ss << args;                                                     \
...
#if ALLOW_DEBUG_LOGGING
#define PRINT_STREAM_DEBUG(name, args)                               \
  do {                                                               \
    std::stringstream ss;                                            \
    ss << args;                                                      \
...
#define PRINT_AUTOSTREAM_DEBUG(args)               \
  do {                                             \
    char nameBuf[256];                             \
    GENERATE_EVENT_NAME(nameBuf, sizeof(nameBuf)); \
    PRINT_STREAM_DEBUG(nameBuf, args);             \
  } while (0)
#else
#define PRINT_AUTOSTREAM_DEBUG(args)
#endif
...
#define GENERATE_EVENT_NAME(nameBuf, nameBufLen)                          \
  {                                                                       \
    snprintf(nameBuf, nameBufLen, "%s.%s.%d", (SHORT_FILE), __FUNCTION__, \
             __LINE__);                                                   \
  }
...
#define ASSERT_NAMED_AND_RETURN_FALSE_IF_FAIL(exp, name)     \
  do {                                                       \
    ::Anki::Util::DropBreadcrumb(false, __FILE__, __LINE__); \
    if (!(exp)) {                                            \
...
#define ASSERT_NAMED_EVENT(expr, name, format, ...)                            \
  do {                                                                         \
    ::Anki::Util::DropBreadcrumb(expr ? true : false, __FILE__, __LINE__);     \
    if (!(expr)) {                                                             \
...
#define ASSERT_NAMED_EVENT_AND_RETURN_FALSE_IF_FAIL(exp, name, format, ...)   \
  do {                                                                        \
...
//
// Code blocks that are only used for developer assertions should be guarded
// with #if ANKI_DEV_ASSERT_ENABLED. Variables that are only used for developer
// assertions should be guarded with DEV_ASSERT_ONLY.
...
//
// Code within "if false" will be analyzed by compiler, so variables are counted
// as "used", but the entire block will be discarded by the optimizer because it
// can't be executed.
//
#define DEV_ASSERT_MSG(expr, name, format, ...)                               \
  do {                                                                        \
...
#define LOG_WARNING(name, fmt, ...) \
  PRINT_NAMED_WARNING(name, fmt, ##__VA_ARGS__)
#define LOG_INFO(name, fmt, ...) \
  PRINT_CH_INFO(LOG_CHANNEL, name, fmt, ##__VA_ARGS__)
#define LOG_DEBUG(name, fmt, ...) \
  PRINT_CH_DEBUG(LOG_CHANNEL, name, fmt, ##__VA_ARGS__)
...
//
#define LOG_CH_INFO(ch, name, fmt, ...) \
  PRINT_CH_INFO(ch, name, fmt, ##__VA_ARGS__)
#define LOG_CH_DEBUG(ch, name, fmt, ...) \
  PRINT_CH_DEBUG(ch, name, fmt, ##__VA_ARGS__)
