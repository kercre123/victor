...
PrintfLoggerProvider::PrintfLoggerProvider() : _minToStderrLevel(1000) {
  SetMinLogLevel(LOG_LEVEL_INFO);
}

PrintfLoggerProvider::PrintfLoggerProvider(
    ILoggerProvider::LogLevel minToStderrLogLevel, bool colorizeStderrOutput)
    : _minToStderrLevel(minToStderrLogLevel),
      _colorizeStderrOutput(colorizeStderrOutput) {
  SetMinLogLevel(LOG_LEVEL_INFO);
}

void PrintfLoggerProvider::Log(ILoggerProvider::LogLevel logLevel,
  ...
  if (colorizeOutput) {
    // Prepend/append color escape codes. Need to move the newline to the end if
    // it exists (due to Webots behavior)
    fprintf(outStream, ANSI_COLOR_RED "%s" ANSI_COLOR_RESET "\n",
            (message.back() == '\n')
                ? message.substr(0, message.length() - 1).c_str()
                : message.c_str());
  } else {
