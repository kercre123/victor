...
VictorLogger::VictorLogger(const std::string& tag) : _tag(tag) {}

void VictorLogger::Log(android_LogPriority prio, const char* channel,
                       const char* name, const KVPairVector& keyvals,
                       const char* strval) {
  ...
  __android_log_print(prio, _tag.c_str(), "[@%s] %s: %s", channel, name,
                      strval);
}

void VictorLogger::Log(android_LogPriority prio, const char* name,
                       const KVPairVector& keyvals, const char* strval) {
...
void VictorLogger::LogEvent(android_LogPriority prio, const char* name,
  ...
  // Note that key names must be an EXACT MATCH for values declared by
  // Anki::Util::DAS namespace.
  //
  ...
  //
  // We use a fixed format string for performance, but we need to update the
  // format string if event format ever changes.
  static_assert(Anki::Util::DAS::EVENT_MARKER == '@',
                "DAS event marker does not match declarations");
  static_assert(Anki::Util::DAS::FIELD_MARKER == '\x1F',
                "DAS field marker does not match declarations");
  ...
  __android_log_print(
      prio, _tag.c_str(),
      "@%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%llu", name, str1,
      str2, str3, str4, int1, int2, int3, int4, uptime_ms);
}

void VictorLogger::LogError(android_LogPriority prio, const char* name,
                            const KVPairVector& keyvals, const char* strval) {
  ...
  // Append any existing keyvals to the end of kv.  If keyvals has a value for
  // s1 or s2, they will supersede what is in kv when processed by LogEvent
...
inline android_LogPriority GetLogPrio(LogLevel level) {
  ...
  DEV_ASSERT(prio != ANDROID_LOG_UNKNOWN,
             "VictorLogger.GetLogPrio.UnknownLogLevel");
  return prio;
}
...
void VictorLogger::LogEvent(LogLevel level, const DasMsg& dasMsg) {
  ...
  __android_log_print(
      ...
      dasMsg.event.c_str(), dasMsg.s1.c_str(), dasMsg.s2.c_str(),
      dasMsg.s3.c_str(), dasMsg.s4.c_str(), dasMsg.i1.c_str(),
      dasMsg.i2.c_str(), dasMsg.i3.c_str(), dasMsg.i4.c_str(), uptime_ms);
}
