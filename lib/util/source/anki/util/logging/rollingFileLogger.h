...
class RollingFileLogger : noncopyable {
 public:
  ...
  RollingFileLogger(Dispatch::Queue* queue, const std::string& baseDirectory,
                    const std::string& extension = kDefaultFileExtension,
                    std::size_t maxFileSize = kDefaultMaxFileSize);
  // create a new queue that will be owned by this file logger
  RollingFileLogger(Dispatch::create_queue_t, const std::string& baseDirectory,
                    const std::string& extension = kDefaultFileExtension,
                    std::size_t maxFileSize = kDefaultMaxFileSize);
  ...
  static std::chrono::system_clock::time_point GetSystemClockTimePoint(
