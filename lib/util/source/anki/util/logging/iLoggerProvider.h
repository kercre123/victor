 ...
 * Created: 4/21/2015
 *
 * Description: Interface for classes that provide a different method of logging
...
class ILoggerProvider {
 public:
  ...
  // set filter after creation. Note this provider will keep a shared_ptr to the
  // filter
  ...
  // - - - - - - - - - - - - - - - - - - - - Unfiltered logs
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // - - - - - - - - - - - - - - - - - - - - Channel filters don't apply to
  // these levels
  ...
  virtual void PrintLogE(
      const char* eventName,
      const std::vector<std::pair<const char*, const char*>>& keyValues,
      const char* eventValue) = 0;
  virtual void PrintLogW(
      const char* eventName,
      const std::vector<std::pair<const char*, const char*>>& keyValues,
      const char* eventValue) = 0;
  ...
  // - - - - - - - - - - - - - - - - - - - - Filtered logs
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // - - - - - - - - - - - - - - - - - - - - Channel filters apply to these
  // levels. This method will filter messages and delegate to subclasses through
  // protected API the messages that pass the filter

  // Delegates on PrintLogI to print infos that pass channel filtering
  void PrintChanneledLogI(
      const char* channel, const char* eventName,
      const std::vector<std::pair<const char*, const char*>>& keyValues,
      const char* eventValue);

  void PrintChanneledLogD(
      const char* channel, const char* eventName,
      const std::vector<std::pair<const char*, const char*>>& keyValues,
      const char* eventValue);
 ...
 protected:
  ...
  // - - - - - - - - - - - - - - - - - - - - Internal API
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // - - - - - - - - - - - - - - - - - - - - Required in subclasses to print
  // messages that pass their filter

  virtual void PrintLogI(
      const char* channel, const char* eventName,
      const std::vector<std::pair<const char*, const char*>>& keyValues,
      const char* eventValue) = 0;

  virtual void PrintLogD(
      const char* channel, const char* eventName,
      const std::vector<std::pair<const char*, const char*>>& keyValues,
      const char* eventValue) = 0;
