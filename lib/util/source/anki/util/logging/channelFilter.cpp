 ...
 * Created: 05/19/2015
 *
 * Description: Class that provides a channel filter (for logs) that can load
 * from json and provides console vars to enable/disable log channels at
 * runtime.
 *
...
void ChannelFilter::Initialize(const Json::Value& config) {
  ...
  if (!config.isNull()) {
    for (const auto& channel : config[kChannelListKey]) {
      // parse channel name
      DEV_ASSERT(channel[kChannelNameKey].isString(),
                 "ChannelFilter.Initialize.BadName");
      const std::string& channelName = channel[kChannelNameKey].asString();

      // parse value
      DEV_ASSERT(channel[kChannelEnabledKey].isBool(),
                 "ChannelFilter.Initialize.BadEnableFlag");
      const bool channelEnabled = channel[kChannelEnabledKey].asBool();
      static const bool kUnregisterInDestructor = true;
      _channelEnableList.emplace(
          channelName,
          new ChannelVar(channelName, channelEnabled, kUnregisterInDestructor));
    }
  }
  ...
  // Print which channels are enabled
  {
    ...
    LOG_INFO("ChannelFilter.Channels", ": Enabled [%s]; Disabled [%s]",
             enabledStr.str().c_str(), disabledStr.str().c_str());
  }
...
void ChannelFilter::EnableChannel(const std::string& channelName) {
  ...
  if (it != _channelEnableList.end()) {
    ...
    _channelEnableList.emplace(
        channelName,
        new ChannelVar(channelName, true, kUnregisterInDestructor));
  }
}
...
void ChannelFilter::DisableChannel(const std::string& channelName) {
  ...
  if (it != _channelEnableList.end()) {
    ...
    _channelEnableList.emplace(
        channelName,
        new ChannelVar(channelName, false, kUnregisterInDestructor));
  }
}
