...
const char* GetLogLevelString(LogLevel level) {
  ...
  DEV_ASSERT_MSG(false, "GetLogLevelString.InvalidLogLevel",
                 "%d is not a valid level", level);
...
LogLevel GetLogLevelValue(const std::string& levelStr) {
  ...
  DEV_ASSERT_MSG(false, "GetLogLevelValue.InvalidLogLevel",
                 "'%s' is not a valid level", levelStr.c_str());
