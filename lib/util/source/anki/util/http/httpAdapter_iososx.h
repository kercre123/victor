...
@interface HttpAdapterURLSessionDelegate
    : NSObject <NSURLSessionDownloadDelegate, NSURLSessionTaskDelegate, NSURLSessionDelegate> {
...
class HttpAdapter : public IHttpAdapter {
 public:
  ...
  void StartRequest(HttpRequest request, Dispatch::Queue* queue, HttpRequestCallback callback,
                    HttpRequestDownloadProgressCallback progressCallback = nullptr);

  void ExecuteCallback(const uint64_t hash, const int responseCode,
                       std::map<std::string, std::string>& responseHeaders,
                       std::vector<uint8_t>& responseBody) { /* unused do nothing */
  }

  void ExecuteDownloadProgressCallback(const uint64_t hash, const int64_t bytesWritten,
                                       const int64_t totalBytesWritten,
                                       const int64_t totalBytesExpectedToWrite) { /* unused */
  }

  void DidCompleteWithError(NSURLSessionTask* task, NSError* error);

  void DidFinishDownloadingToURL(NSURLSessionDownloadTask* downloadTask, NSURL* location);

  void DidWriteData(NSURLSessionDownloadTask* downloadTask, int64_t bytesWritten,
 ...
 private:
  struct HttpRequestState {
    HttpRequestState() : HttpRequestState(HttpRequest(), nullptr, nullptr, nullptr, nullptr) {}
    HttpRequestState(HttpRequest request, Dispatch::Queue* queue, HttpRequestCallback callback,
                     HttpRequestDownloadProgressCallback progressCallback, NSURL* destinationUrl)
        : request(request),
          queue(queue),
          callback(callback),
  ...
  void ProcessGetRequest(NSString* httpMethod, HttpRequest& request, Dispatch::Queue* queue,
                         HttpRequestCallback callback,
                         HttpRequestDownloadProgressCallback progressCallback);

  void ProcessUploadRequest(NSString* httpMethod, HttpRequest& request, Dispatch::Queue* queue,
                            HttpRequestCallback callback);
  ...
  void SetResponseHeaders(NSURLResponse* response,
                          std::map<std::string, std::string>& responseHeaders);

  void ConvertParamsToBodyString(const HttpRequest& request, NSMutableString* bodyString);

  void DoCallback(Dispatch::Queue* queue, HttpRequestCallback callback, HttpRequest request,
