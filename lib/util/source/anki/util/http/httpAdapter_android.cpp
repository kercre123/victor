...
CONSOLE_VAR_RANGED(int, kHttpRequestTimeOutMSec, "Settings.Debug", 10000, 0,
                   100000);
...
HttpAdapter::HttpAdapter(Util::Dispatch::Queue* queue /* = nullptr */)
    : _internalQueue(queue), _adapterState(IHttpAdapter::AdapterState::DOWN) {
  if (!ANKI_USE_JNI) {
  ...
  if (nullptr != env) {
    JClassHandle httpClass{env->FindClass("com/anki/util/http/HttpAdapter"),
                           env};
...

    {
      ...
      jmethodID instanceMethodID = env->GetStaticMethodID(
          httpClass.get(), "getInstance", "()Lcom/anki/util/http/HttpAdapter;");
      JObjectHandle objInstance{
          env->CallStaticObjectMethod(httpClass.get(), instanceMethodID), env};
    ...
    _startRequestMethodID =
        env->GetMethodID(httpClass.get(), "startRequest",
                         ...
                         "String;[BILjava/lang/String;I)V");
...
HttpAdapter::~HttpAdapter() { ShutdownAdapter(); }
...
void HttpAdapter::StartRequest(
  ...
  if (_internalQueue != nullptr) {
    Dispatch::Async(_internalQueue,
                    [this, request = std::move(request), queue,
                     callback = std::move(callback),
                     progressCallback = std::move(progressCallback)]() mutable {
                      StartRequestInternal(std::move(request), queue,
                                           std::move(callback),
                                           std::move(progressCallback));
                    });
  } else {
    StartRequestInternal(std::move(request), queue, std::move(callback),
                         std::move(progressCallback));
  }
}

void HttpAdapter::StartRequestInternal(
    HttpRequest&& request, Util::Dispatch::Queue* queue,
    HttpRequestCallback callback,
    HttpRequestDownloadProgressCallback progressCallback) {
  if (!IsAdapterUp()) {
  ...
  JObjectArrayHandle headers{JNIUtils::convertStringMapToJObjectArray(
                                 env, request.headers, stringRefs),
                             env};
  JObjectArrayHandle params{
      JNIUtils::convertStringMapToJObjectArray(env, request.params, stringRefs),
      env};
  ...
  env->SetByteArrayRegion(jbodyArray.get(), 0, bodySize,
                          (const jbyte*)(request.body.data()));

  JStringHandle storageFilePath{
      env->NewStringUTF(request.storageFilePath.c_str()), env};

  PRINT_CH_INFO("HTTP", "http_adapter.request", "%s :: %s",
                HttpMethodToString(request.method), request.uri.c_str());

  uint64_t hash;
  {
    std::lock_guard<std::mutex> lock(_callbackMutex);
    hash = _sHashCounter.fetch_add(1);
    _requestResponseMap.emplace(
        hash, HttpRequestResponseType{
  ...
  env->CallVoidMethod(_javaObject.get(), _startRequestMethodID, hash, uri.get(),
                      headers.get(), params.get(), jbodyArray.get(), httpMethod,
                      storageFilePath.get(), requestTimeout);
...
void HttpAdapter::GetHttpRequestResponseTypeFromHash(
    const uint64_t hash, HttpRequestResponseType& response) {
  std::lock_guard<std::mutex> lock(_callbackMutex);
  response = _requestResponseMap.at(hash);
}

void HttpAdapter::ExecuteCallback(
  ...
  PRINT_NAMED_DEBUG("http_adapter.execute_callback", "%llx %d %d", hash,
                    responseCode, responseBody.size());
  ...
  PRINT_CH_INFO("HTTP", "http_adapter.response", "%d :: %s", responseCode,
                response.request.uri.c_str());

  Dispatch::Async(
      response.queue, [this, hash, response = std::move(response), responseCode,
                       responseBody = std::move(responseBody),
                       responseHeaders = std::move(responseHeaders)] {
        if (!IsAdapterUp()) {
          return;
        }
        response.callback(response.request, responseCode, responseHeaders,
                          responseBody);
...
void HttpAdapter::ExecuteDownloadProgressCallback(
  ...
  if (response.progressCallback) {
    Dispatch::Async(response.queue, [this, response = std::move(response),
...
void JNICALL Java_com_anki_util_http_HttpAdapter_NativeHttpRequestCallback(
    JNIEnv* env, jobject thiz, jlong hash, jint responseCode,
    jobjectArray responseHeaders, jbyteArray responseBody) {
  ...
  if (bytes != NULL) {
    ...
    for (jsize i = 0; i < stringCount; i += 2) {
      Anki::Util::JStringHandle jKey{
          (jstring)env->GetObjectArrayElement(responseHeaders, i), env};
      Anki::Util::JStringHandle jValue{
          (jstring)env->GetObjectArrayElement(responseHeaders, i + 1), env};
    ...
    if (instanceIter != requestInstances.end() &&
        instanceIter->second->IsAdapterUp()) {
...
void JNICALL
Java_com_anki_util_http_HttpAdapter_NativeHttpRequestDownloadProgressCallback(
    JNIEnv* env, jobject thiz, jlong hash, jlong bytesWritten,
    jlong totalBytesWritten, jlong totalBytesExpectedToWrite) {
  auto instanceIter = requestInstances.find(hash);
  if (instanceIter != requestInstances.end() &&
      instanceIter->second->IsAdapterUp()) {
    instanceIter->second->ExecuteDownloadProgressCallback(
        hash, bytesWritten, totalBytesWritten, totalBytesExpectedToWrite);
  }
}
}
