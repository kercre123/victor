...
void After(Queue* queue, std::chrono::milliseconds delayMSec,
           const Block& block, const char* name = nullptr);

// Push a block onto the queue that will execute repeatedly with the specified
// period Returns a smart handle that will remove the callback from the queue on
// destruction; if you do not save this handle, your callback will be
// immediately removed and never run
Anki::Util::TaskHandle ScheduleCallback(
    Queue* queue, std::chrono::milliseconds repeatPeriodMSec,
    const Block& block, const char* name = nullptr);

// Queue handle: wrapping of a queue owned by a class that will stop and release
// itself on destruction

// Empty struct tag to pass into constructor to indicate a queue should be
...
class QueueHandle {
 public:
  ...
  explicit QueueHandle(create_queue_t, const char* name = nullptr,
  ...
  // move operations
  QueueHandle(QueueHandle&& other) : _handle(nullptr) {
