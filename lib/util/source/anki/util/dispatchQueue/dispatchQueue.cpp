...
class Queue {
 public:
  ...
  Queue(const char* name,
      ...
      : taskExecutor(name, threadPriority){};
  virtual ~Queue(){};
  ...
  virtual void DispatchAfter(std::chrono::milliseconds durationMSec,
                             const Block& block, const char* name) {
  ...
  virtual Anki::Util::TaskHandle ScheduleCallback(
      std::chrono::milliseconds repeatPeriodMSec, const Block& block,
      const char* name) {
    return taskExecutor.WakeAfterRepeat(block, repeatPeriodMSec, name);
  }

  virtual void StopExecution() { taskExecutor.StopExecution(); }
};
...
class ImmediateQueue : public Queue {
 ...
 public:
  ImmediateQueue() : Queue(nullptr), _running(true){};
  ~ImmediateQueue(){};
  ...
  void DispatchAfter(std::chrono::milliseconds durationMSec, const Block& block,
                     const char* name) override {
    if (_running) block();
    //    std::chrono::time_point<std::chrono::steady_clock> when =
    //    std::chrono::time_point<std::chrono::steady_clock>::min(); when +=
    //    durationMSec; taskExecutor.WakeAfter(block, when);
  }

  class InvalidTaskHandle
      : public Anki::Util::TaskHandleContainer::ITaskHandle {
    void Invalidate() override {}
    virtual ~InvalidTaskHandle() {}
  };

  Anki::Util::TaskHandle ScheduleCallback(
      std::chrono::milliseconds repeatPeriodMSec, const Block& block,
      const char* name) override {
    block();  // I guess the concept of repetition is kinda weird for the
              // immediate queue...
    return Anki::Util::TaskHandle(
        new Anki::Util::TaskHandleContainer(*(new InvalidTaskHandle())));
  }

  void StopExecution() override { _running = false; };
};
...
void After(Queue* queue, std::chrono::milliseconds delayMSec,
           const Block& block, const char* name) {
  queue->DispatchAfter(delayMSec, block, name);
}

Anki::Util::TaskHandle ScheduleCallback(
    Queue* queue, std::chrono::milliseconds repeatPeriodMSec,
    const Block& block, const char* name) {
