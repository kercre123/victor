...
class ExternalPointerHandle {
 public:
  ...
  class Owner {
   public:
    ...
    T* get() const { return static_cast<T*>(masterPtr.get()); }

    T* operator->() const { return get(); }

    T& operator*() const { return *get(); }

    explicit operator bool() const { return (bool)masterPtr; }

    void reset() { masterPtr.reset(); }

    void reset(T* ptr) { masterPtr.reset(ptr); }
  ...
  ExternalPointerHandle(const ExternalPointerHandle<X, B>& other)
      : handle(other.handle) {}
  ...
  template <typename TOwner,
            typename = typename std::enable_if<
                ...
                std::is_same<TOwner, typename ExternalPointerHandle<
                                         typename TOwner::ptr_type,
                                         typename TOwner::base_type>::Owner>::
                    value>::type>
  ExternalPointerHandle(const TOwner& other) : handle(other.getHandle()) {}

 private:
  explicit ExternalPointerHandle(const std::weak_ptr<B>& weakPtr)
      : handle(weakPtr) {}
 ...
 public:
  ...
  T* get() const { return static_cast<T*>(handle.lock().get()); }

  T* operator->() const { return get(); }

  T& operator*() const { return *get(); }

  explicit operator bool() const { return !handle.expired(); }
