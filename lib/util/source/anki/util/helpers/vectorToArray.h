...
template <class T>
uint32_t VectorPtrToArray(const std::vector<const T *> &vectorOfPointers,
                          void **buffer, uint32_t size) {
  size_t count = 0;
  for (; count < size && count < vectorOfPointers.size(); ++count) {
    buffer[count] =
        const_cast<void *>(static_cast<const void *>(vectorOfPointers[count]));
...
uint32_t VectorValueToArray(const std::vector<T> &vectorOfValues, Y *buffer,
                            uint32_t size) {
  size_t count = 0;
