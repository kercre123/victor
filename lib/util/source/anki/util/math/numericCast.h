 ...
 * Created: 30/06/15
 *
 * Description: numeric_cast<> for asserting that a value won't under/over-flow
 *into the new type resolves to a constexpr static_cast<> in NDEBUG
 *              numeric_cast_clamped<> clamps the value to the new types
 *lowest..max in all build configs both utilize a bunch of helper functions
 *
...
DoesFirstTypeHaveGTERange()  // GTE = Greater Than or Equal
{
  typedef std::numeric_limits<FirstType> FirstTypeLimits;
  typedef std::numeric_limits<SecondType> SecondTypeLimits;
...
inline constexpr bool IsValidNumericCastBothSigned(FromType fromVal) {
  ...
  return DoesFirstTypeHaveGTERange<ToType, FromType>()
             ? true
             :  // going to same/larger type - must fit
             ((fromVal >=
               (FromType)
                   ToTypeLimits::lowest()) &&  // check within lowest..max
                                               // (compare as larger FromType)
...
inline constexpr bool IsValidNumericCastFromSigned(FromType fromVal) {
  ...
  return (fromVal < FromType(0))
             ? false
             :  // -ve signed value can't fit in any unsigned type
             DoesFirstTypeHaveGTERange<ToType, FromType>()
                 ? true  // going to same/larger type - must fit
                 : (fromVal <=
                    (FromType)
                        ToTypeLimits::max());  // check within max (compare as
                                               // larger FromType)
}
...
inline constexpr bool IsValidNumericCastToSigned(FromType fromVal) {
  ...
  return DoesFirstTypeHaveGTERange<ToType, FromType>()
             ...
             :  // going to same/larger type - must fit
             (fromVal <=
              (FromType)ToTypeLimits::max());  // check within max (compare as
                                               // larger FromType)
}
...
inline constexpr
  ...
  static_assert((FromTypeLimits::lowest() != FromTypeLimits::max()),
                "numeric_limits improperly defined for FromType - if it's an "
                "enum then cast to int first");
  static_assert((ToTypeLimits::lowest() != ToTypeLimits::max()),
                "numeric_limits improperly defined for ToType   - if it's an "
                "enum then cast to int first");

  return (FromTypeLimits::is_signed && ToTypeLimits::is_signed)
             ? IsValidNumericCastBothSigned<ToType, FromType>(fromVal)
             : (FromTypeLimits::is_signed)
                   ? IsValidNumericCastFromSigned<ToType, FromType>(fromVal)
                   : (ToTypeLimits::is_signed)
                         ? IsValidNumericCastToSigned<ToType, FromType>(fromVal)
                         : IsValidNumericCastBothUnsigned<ToType, FromType>(
                               fromVal);
}

// auto-cast enum to underlying type and call IsValidNumericCast for
// underlying_type
...
inline constexpr
  ...
  return IsValidNumericCast<ToType>(
      static_cast<typename std::underlying_type<FromTypeEnum>::type>(
          fromValEnum));
}
...
template <typename ToType, typename FromType>
inline constexpr ToType Helper_numeric_cast_clampedBothSigned(
    FromType fromVal) {
  typedef std::numeric_limits<ToType> ToTypeLimits;  //   signed

  return DoesFirstTypeHaveGTERange<ToType, FromType>()
             ? static_cast<ToType>(fromVal)
             :  // going to same/larger type - must fit
             (fromVal > (FromType)ToTypeLimits::max())
                 ? ToTypeLimits::max()
                 :  // clamp to max    (cast to larger FromType type for
                    // comparison)
                 (fromVal < (FromType)ToTypeLimits::lowest())
                     ? ToTypeLimits::lowest()  // clamp to lowest (cast to
                                               // larger FromType type for
                                               // comparison)
                     : static_cast<ToType>(fromVal);  // value in range
}

template <typename ToType, typename FromType>
inline constexpr ToType Helper_numeric_cast_clampedFromSigned(
    FromType fromVal) {
  typedef std::numeric_limits<ToType> ToTypeLimits;  // unsigned
  ...
  return (fromVal < FromType(0))
             ...
             (DoesFirstTypeHaveGTRange<FromType, ToType>() &&
              (fromVal > (FromType)ToTypeLimits::max()))
                 ? ToTypeLimits::max()  // clamp to max (cast to larger FromType
                                        // type for comparison)
                 : static_cast<ToType>(fromVal);  // value in range
}
...
inline constexpr ToType Helper_numeric_cast_clampedToSigned(FromType fromVal) {
  ...
  return (DoesFirstTypeHaveGTRange<FromType, ToType>() &&
          (fromVal > (FromType)ToTypeLimits::max()))
             ? ToTypeLimits::max()  // clamp to max (cast to larger FromType
                                    // type for comparison)
             : static_cast<ToType>(fromVal);  // value in range
...
template <typename ToType, typename FromType>
inline constexpr ToType Helper_numeric_cast_clampedBothUnsigned(
    FromType fromVal) {
  typedef std::numeric_limits<ToType> ToTypeLimits;  // unsigned
...
inline constexpr
  ...
  return (FromTypeLimits::is_signed && ToTypeLimits::is_signed)
             ...
             : FromTypeLimits::is_signed
                   ? Helper_numeric_cast_clampedFromSigned<ToType, FromType>(
                         fromVal)
                   : ToTypeLimits::is_signed
                         ? Helper_numeric_cast_clampedToSigned<ToType,
                                                               FromType>(
                               fromVal)
                         : Helper_numeric_cast_clampedBothUnsigned<ToType,
                                                                   FromType>(
                               fromVal);
}
...
inline constexpr
  ...
  return numeric_cast_clamped<ToType>(
      static_cast<typename std::underlying_type<FromTypeEnum>::type>(
          fromValEnum));
}
