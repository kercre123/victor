...
constexpr float FLOATING_POINT_COMPARISON_TOLERANCE_FLT =
    (float)FLOATING_POINT_COMPARISON_TOLERANCE;
...
inline constexpr bool IsNear(
    float x, float y,
    float epsilon = Anki::Util::FLOATING_POINT_COMPARISON_TOLERANCE_FLT) {
...
inline constexpr float ConvertMToMM(float inVal) { return inVal * 1000.0f; }

inline constexpr double ConvertMToMM(double inVal) { return inVal * 1000.0; }

inline constexpr float ConvertMMToM(float inVal) { return inVal * 0.001f; }

inline constexpr double ConvertMMToM(double inVal) { return inVal * 0.001; }
...
inline constexpr float SecToNanoSec(float sec) { return sec * 1000000000.0f; }

inline constexpr double SecToNanoSec(double sec) { return sec * 1000000000.0; }
...
inline constexpr float SecToMilliSec(float sec) { return sec * 1000.0f; }

inline constexpr double SecToMilliSec(double sec) { return sec * 1000.0; }
...
template <class T>
inline ANKI_NON_DEVELOPER_CONSTEXPR T Clamp(const T& val, const T& min,
                                            const T& max) {
...
inline float WrapToMax(float inVal, float maxValue) {
  ...
  // fmodf of a -ve number gives a result <0 (and >-maxValue), add one maxValue
  // to get it in the +ve range
  const float zeroToMaxValue =
      (modValue < 0.0f) ? (modValue + maxValue) : modValue;
  return zeroToMaxValue;
}

// ClampedUnsignedAddition - (unsigned types only) clamp at max value instead of
// overflowing
template <typename T>
inline constexpr T ClampedUnsignedAddition(T a, T b) {
  typedef std::numeric_limits<T> TypeLimits;
  static_assert((TypeLimits::lowest() != TypeLimits::max()),
                "Invalid numeric_limits for this type!");
  static_assert(!TypeLimits::is_signed, "Use ClampedSignedAddition!");

  return (a < (TypeLimits::max() - b)) ? (a + b) : TypeLimits::max();
}

// ClampedSignedAddition - (signed types only) clamp at min or max value instead
// of under/overflowing
template <typename T>
inline constexpr T ClampedSignedAddition(T a, T b) {
  // Note: if a and b are of opposite sign then the addition cannot overlow by
  // definition
  ...
  return ((a > 0) && (b > 0))
             ? ((a < (TypeLimits::max() - b)) ? (a + b) : TypeLimits::max())
             : ((a < 0) && (b < 0))
                   ? ((a > (TypeLimits::lowest() - b)) ? (a + b)
                                                       : TypeLimits::lowest())
                   : (a + b);
}

// ClampedAddition - uses helpers to determine signed or unsigned version at
// compile time need indirection via std::true/false_type to prevent compiler
// trying to build in both branches and static asserting
template <typename T>
inline constexpr T ClampedAdditionHelper(T a, T b,
                                         /*is_signed =*/std::false_type) {
  return ClampedUnsignedAddition<T>(a, b);
}

template <typename T>
inline constexpr T ClampedAdditionHelper(T a, T b,
                                         /*is_signed =*/std::true_type) {
...
// We want to have one math lib so the C++ version includes the C version but we
// want to make sure we use the inline templated version if only this file was
// include. The __cplusplus macro check doesn't work since the compiler is still
// C++ but this file might be externed "C"
#undef FLT_NEAR
...
// TODO( COZMO-7843): Get rid of this and use the templated version above
// eventually. Will need to cast args appropriately throughout the code though.
#undef MAX
...
#define CLIP(val, lo, hi)                                                      \
  ((((((val) > (lo)) ? (val) : (lo)) < (hi)) ? (((val) > (lo)) ? (val) : (lo)) \
                                             : (hi)))
