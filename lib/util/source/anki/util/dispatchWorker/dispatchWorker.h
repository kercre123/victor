 ...
 * Created: 8/30/16
 *
 * Description: Helper class for parallelizing tasks. Use by declaring a version
 * of the template with the desired number of threads and argument types to be
 * used by the task.
 *
 *
 * Example:
 ...
 * MyDispatchWorker::FunctionType loadFileFunc = [this] (const std::string&
 * filePath)
 * {
...
// template arguments are: number of threads, followed by list of arg types that
// the processing function expects in its tuple
...
class DispatchWorker {
 ...
 private:
  ...
  // Use the provided function to process the arguments specified by the input
  // iterators
  void DoThreadWork(typename ArgumentVector::iterator start,
                    typename ArgumentVector::iterator end);

  // Tuple unpacking - from
  ...
  void Invoke(FunctionArgType& params, seq<S...>) {
    _function(
        std::forward<typename std::tuple_element<S, FunctionArgType>::type>(
            std::get<S>(params))...);
  }
...
void DispatchWorker<TCount, Args...>::Process() {
  ...
  const std::size_t numTotalThreads =
      TCount + 1;  // Reserve one slot for the calling thread
  ...
  for (std::size_t i = 0; i < TCount; i++) {
    ...
    _workerThreads[i] = std::thread(&DispatchWorker::DoThreadWork, this,
                                    curIter, curIter + sizeForThread);
...
template <std::size_t TCount, typename... Args>
void DispatchWorker<TCount, Args...>::DoThreadWork(
    typename ArgumentVector::iterator start,
    typename ArgumentVector::iterator end) {
