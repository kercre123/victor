...
static void EnableAnkiLab(const bool enable) {
  if (sOpRunner) {
    sOpRunner([enable](AnkiLab* lab) { lab->Enable(enable); });
  }
}
...
namespace {
static DisableABTestingConsoleVar cvar__kForceDisableABTesting(
    kForceDisableABTesting, "kForceDisableABTesting", "A/B Testing", false);
...
void ForceAssignExperiment(ConsoleFunctionContextRef context) {
  ...
  context->channel->WriteLog("ForceAssignExperiment %s => %s", experimentKey,
                             variationKey);
}
CONSOLE_FUNC(ForceAssignExperiment, "A/B Testing", const char* experimentKey,
             const char* variationKey);
}  // namespace
#endif
...
void AddABTestingForcedAssignment(const std::string& experimentKey,
  ...
  if (!sOpRunner) {
    // After the engine is created and is executing its LoadData routine, it
    // will read the contents of this map and use it to force assignments.
...
void HandleABTestingForceURL(const std::string& urlQueryParams) {
  const std::vector<std::string> keyValStrings =
      StringSplit(urlQueryParams, '&');
