 ...
 * Created: 7/28/2017
 *
 * Description: External C interface for outside services to interact with
 *AnkiLab Needs outside initialization to inject ability to access an AnkiLab
 *instance
 *
 *
...
// - a LabOpRunner - a functor that, given a LabOperation to run, will execute
// it and pass it an
...
void InitializeABInterface(const LabOpRunner& opRunner,
                           const GetUserIdFunction& userIdRetriever);
...
ANKI_EXPORT const std::map<std::string, std::string>&
GetABTestingForcedAssignments();
