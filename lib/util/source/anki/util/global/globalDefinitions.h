 ...
 * Created: 07/16/2014
 *
 * Description: Global definitions to be included in all or a large amount of
 *files. Keep content to a minimum, please.
 *
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Use ANKI_DEVELOPER_CODE to strip down
// code that should only be available to developers in debug, but not for
// testing in release, nor available to players.
// Use ANKI_DEV_CHEATS to strip down code that should be available to developers
// in debug and testing in release, but not available to players.
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
#if defined(NDEBUG)
...
#define ANKI_DEV_CHEATS \
  1  // This is the expected behavior on "release" builds, overridden with
     // command-line defines for shipping builds
#endif
...
//
// Use ANKI_PRIVACY_GUARD and HidePersonallyIdentifiableInfo() for anything that
// should not be present in shipping mode, such as players' names in logs. NOTE:
// This is a separate flag from DEV_CHEATS to make it easier to find use cases
// in the code and in case we want to set it differently via other build flags
...
//
// Privacy guard must be enabled with -DANKI_PRIVACY_GUARD=1 for shipping
// builds.
//
...
#define ANKI_NON_DEVELOPER_CONSTEXPR \
  constexpr  // Function can be constexpr as e.g. Dev asserts are disabled
#endif

namespace Anki {
namespace Util {

// Simply returns given string unless ANKI_PRIVACY_GUARD is enabled, in which
// case "<PII>" is returned.
