 ...
 *
 *  Provides base component classes for a version of entity/component model in
 *which the components are dependent on each other and require a specific
...
class IDependencyManagedComponent {
 protected:
  ...
  // from void* to base vs derived classes (particularly when multiple
  // inheritance is involved) this class stores two versions of the ptr passed
  // in 1) the void* ptr comes directly from the templated type T and is
  // returned directly through a static cast to T in GetComponent - so long as
  // the base class is used in both cases all memory should be fine 2) A ptr to
  // the up-cast IDependencyManagedComponent. For simplicity this is always
  // stored as a shared ptr and a null deleter is passed in if the class has
  // specified it doesn't want its memory managed. Having an explicit up cast
  // means a) a compile time failure if the component being passed in is not a
  // valid dependency managed component and b) no memory/cast issues when
  // accessing IDependenceManagedComponent functions
  template <typename T>
  IDependencyManagedComponent(T* derivedPtr, EnumType componentEnum)
      : _derivedPtr(derivedPtr), _type(componentEnum) {
 ...
 public:
  ...
  // Additional components will be passed into init but may not have been
  // initialized
  virtual void AdditionalInitAccessibleComponents(
      std::set<EnumType>& components) const {};

  // TMP - add in robot to make transition to new system easier for comps that
  // previously received robot - theoretically only dependent components should
  ...
  // Additional components will be passed into update but may not have been
  // updated
  ...
  T& GetComponent() const {
    ...
    ANKI_VERIFY(
        IsComponentValid(),
        "DependencyManagedComponentWrapper.GetComponent.ValueIsNotValid", "");
  ...
  T* GetComponentPtr() const {
    ...
    ANKI_VERIFY(enumID == _type,
                "DependencyManagedComponentWrapper.GetComponentPtr."
                "CastingToIncorrectType",
                "");
    ANKI_VERIFY(
        IsComponentValid(),
        "DependencyManagedComponentWrapper.GetComponentPtr.ValueIsNotValid",
        "");
    auto* castPtr = static_cast<T*>(_derivedPtr);
    return castPtr;
  }
  ...
  bool IsComponentValid() const {
    return (_derivedPtr != nullptr) && IsComponentValidInternal();
  }
...
// Often used for components that exist within multiple dependency managed
// entities If the top level entity determines update/init order, the nested
// level would be an unreliable component
template <typename EnumType>
class UnreliableComponent : public IDependencyManagedComponent<EnumType> {
 ...
 public:
  ...
  virtual void GetInitDependencies(
      std::set<EnumType>& dependencies) const override final {}
  virtual void AdditionalInitAccessibleComponents(
      std::set<EnumType>& components) const override final{};
  virtual void InitDependent(
      Vector::Robot* robot,
