...
// - - - - - - - - - - - - - - - - - - - -
template <typename EnumType, typename ValueType>
void Entity<EnumType, ValueType>::AddComponent(EnumType enumID,
                                               ValueType component) {
  auto pair = _components.insert(std::make_pair(enumID, component));
  ANKI_VERIFY(pair.second, "Entity.AddComponent.FailedToInsert",
              "Failed to insert enum");
}
...
ValueType& Entity<EnumType, ValueType>::GetComponent(EnumType enumID) {
  ...
  ANKI_VERIFY(iter != _components.end(), "Entity.GetComponent.InvalidGet",
              "Verify Failed for enum");
  return iter->second;
}
...
// - - - - - - - - - - - - - - - - - - - -
template <typename EnumType, typename ValueType, EnumType CountField>
EntityFullEnumeration<EnumType, ValueType, CountField>::EntityFullEnumeration(
    const FullEntityArray& array) {
  ANKI_VERIFY(IsSequentialArray(array),
              "EntityFullEnumeration.Constructor.ArrayNotSequential", "");
  for (auto& entry : array) {
    Entity<EnumType, ValueType>::_components.insert(
        std::make_pair(entry.EnumValue(), entry.Value()));
  }
}
