...
// if DEBUG_PACKING_PRINT : packing functions will print binary operations for
// visual debugging
...
bool PackMaskedIds(const unsigned int* idArray, size_t idCount,
                   unsigned char* outBuffer, size_t outBufferSize,
                   const unsigned int bitsPerId,
                   unsigned int& requiredBytesToSend) {
  ...
  requiredBytesToSend =
      (((unsigned int)idCount * bitsPerId) + bufferTypeInBits - 1) /
      bufferTypeInBits;
  ...
  if (outBufferSize < requiredBytesToSend) {
    PRINT_NAMED_ERROR(
        "PackMaskedId",
        "outBuffer (%zd bytes) does not have enough bytes for the given id "
        "count %zd * %u = %zd bits = %u bytes. Programmer error",
        outBufferSize, idCount, bitsPerId, idCount * bitsPerId,
        requiredBytesToSend);
  ...
  const unsigned int maskSizeInBytes =
      sizeof(unsigned int);  // we only need to know the size here
  const unsigned int maskSizeInBits = maskSizeInBytes * 8;
  if (bufferTypeInBits > (maskSizeInBits - bitsPerId)) {
    // We can only code Ids that start at indices [0, maskTypeInBits-bitsPerId].
    // If the bufferTypeInBits is bigger than (maskTypeInBits - bitsPerId), some
    // Ids require the mask to split between buffer units. eg: 8 bits array, 8
    // bits mask, 3 bitsPerId. The fourth item would require a mask that can
    // split 2 bits from 1 byte, and 1 bit from the next one To solve this
    // easily, change the mask to 16 bits.
    PRINT_NAMED_ERROR(
        "PackMaskedId",
        "Mask of '%u' is too big for buffer and mask type. Programmer error.",
        bitsPerId);
    assert(false);  // this is a programmer error, assert
    return false;
  }

  // since masks require bigger buffers than necessary, provide such support
  // here
  ...
  size_t requiredBytesForMask =
      requiredBytesToSend +
      (maskSizeInBytes -
       1);  // the mask writes N bytes beyond the actual required
  ...
  // for programmer
  PRINT_NAMED_INFO("PackMaskedIds",
                   "To pack %zu Ids with %u bits per Id:", idCount, bitsPerId);
  PRINT_NAMED_INFO("PackMaskedIds", "IdCount requires %u bytes to be sent",
                   requiredBytesToSend);
  PRINT_NAMED_INFO("PackMaskedIds",
                   "MaskSize requires %zu bytes from the buffer",
                   requiredBytesForMask);
  PRINT_NAMED_INFO("PackMaskedIds", "%s (since outBuffer provided %zu)",
                   outBufferSize != outBufferPtrSize
                       ? "Temp buffer is required for mask:"
                       : "Temp buffer is not required:",
                   outBufferSize);

  // reset buffer
  memset(outBufferPtr, 0x00, outBufferPtrSize);

  const unsigned int firstBitOfMask =
      (maskSizeInBits - bitsPerId);  // when shifted
  for (unsigned int idIdx = 0; idIdx < idCount; ++idIdx) {
    ...
    assert((charPosition * sizeof(unsigned char) + sizeof(idHashedValue)) <=
           outBufferPtrSize * sizeof(unsigned char));
    assert((charPosition * sizeof(unsigned char) + sizeof(idValueAtBuffer)) <=
           requiredBytesForMask * sizeof(unsigned char));

    // hash this id
    unsigned int idValue = idArray[idIdx];
    if (idValue > (1 << bitsPerId)) {
      // this value doesn't fit in the number of bits!
      PRINT_NAMED_ERROR("PackMaskedId",
                        "Id '%u' is too big for the number of bits used to "
                        "encode it '%u'. Programmer error.",
                        idValue, bitsPerId);
      assert(false);  // this is a programmer error, assert
      return false;
    }
    // idMaskedValue = ( idValue & idMask ); // not necessary, since the number
    // is smaller than idMask
    idHashedValue = (idValue << (firstBitOfMask - charOffset));

    // htonl/ntohl is required or bits overlap with current algorithm (because
    // of shifting applied)
...
#pragma GCC diagnostic pop
    ...
    // first, read the current value in the buffer because we are encoding the
    // current id with some others
    memcpy(&idValueAtBuffer, &outBufferPtr[charPosition],
           sizeof(idValueAtBuffer));

    // now build the result that would go into the buffer
    idValueAtBuffer =
        idValueAtBuffer | idHashedValue;  // OR is fine because array was zeroed

    // and copy back into the buffer
    memcpy(&outBufferPtr[charPosition], &idValueAtBuffer,
           sizeof(idValueAtBuffer));
...
bool UnpackMaskedIds(const unsigned char* inBuffer, size_t inBufferSize,
                     unsigned int* outIdArray, size_t outIdCount,
                     const unsigned int bitsPerId) {
  // supporting only byte access buffers
  const unsigned int bufferTypeInBits = 8;
  ...
  const unsigned int requiredBytesToRead =
      (((unsigned int)outIdCount * bitsPerId) + bufferTypeInBits - 1) /
      bufferTypeInBits;
  if (requiredBytesToRead > inBufferSize) {
    PRINT_NAMED_ERROR(
        "UnpackMaskedIds",
        "inBuffer (%zu bytes) does not have enough bytes for the given id "
        "count %zu * %u = %zu bits = %u bytes. Programmer error",
        inBufferSize, outIdCount, bitsPerId, outIdCount * bitsPerId,
        requiredBytesToRead);
    assert(false);  // this is a programmer error, assert
    return false;
  }
  ...
  unsigned int requiredBytesForMask =
      requiredBytesToRead +
      (sizeof(idMask) -
       1);  // the mask writes N bytes beyond the actual required
  ...
  // for programmer
  PRINT_NAMED_INFO("UnpackMaskedIds",
                   "To unpack %zu Ids with %u bits per Id:", outIdCount,
                   bitsPerId);
  PRINT_NAMED_INFO("UnpackMaskedIds", "IdCount requires %u bytes to be read",
                   requiredBytesToRead);
  PRINT_NAMED_INFO("UnpackMaskedIds",
                   "MaskSize requires %u bytes from the buffer",
                   requiredBytesForMask);
  PRINT_NAMED_INFO("UnpackMaskedIds", "%s (since inBuffer provided %zu)",
                   inBufferSize != inBufferPtrSize
                       ? "Temp buffer is required for mask:"
                       : "Temp buffer is not required:",
                   inBufferSize);
  ...
  for (unsigned int idIdx = 0; idIdx < outIdCount; ++idIdx) {
    ...
    // otherwise we read/write beyond buffer
    assert((charPosition * sizeof(unsigned char) + sizeof(idValueAtBuffer)) <=
           inBufferPtrSize * sizeof(unsigned char));
    assert((charPosition * sizeof(unsigned char) + sizeof(idValueAtBuffer)) <=
           requiredBytesForMask * sizeof(unsigned char));

    // read the value from the buffer
    memcpy(&idValueAtBuffer, &inBufferPtr[charPosition],
           sizeof(idValueAtBuffer));
