...
IConsoleFunction::IConsoleFunction(
    ...
    : function_(std::move(function)), id_(keystring), category_(categoryName) {
...
IConsoleFunction::IConsoleFunction(const std::string& keystring,
    ...
    : IConsoleFunction(
          keystring,
          [function](ConsoleFunctionContextRef c) {
            if (function != nullptr) {
              function(c);
            }
          },
...
//------------------------------------------------------------------------------------------------------------------------------
// This takes a whitespace separated list of arguments and fills in our args_
// vector.
IConsoleFunction::ERROR_CODE IConsoleFunction::ParseText(
  ...
  while (!done) {
    const size_t length =
        ((end != std::string::npos) ? (end - begin) : (int)std::string::npos);
...
//------------------------------------------------------------------------------------------------------------------------------
// This reads in the argument signature for the function and constructs
// placeholders in the args_ vector.
...
//------------------------------------------------------------------------------------------------------------------------------
// Parse the argument declaration text and try to deduce the type and name of
// the variable. Not the most efficient function around, but it's only run at
// registration time and isn't a performance concern. The user can specify if
// the argument is optional or not, meaning it doesn't need to be sent to the
// function. eg. The string "bool aBool" will create and argument of type bool
// with the name aBool.
void IConsoleFunction::ParseArgDeclaration(const char* arg) {
  ...
  // User can specify if this arg is optional or not by preceding the arg with
  // "optional"
  bool optional = false;
  ...
  if ((begin == std::string::npos) || (end == std::string::npos) ||
      (begin >= end)) {
    assert(false);  // Invalid argument ... check signature_ to see where you
                    // went wrong
    return;
  }

  // Kill all leading and trailing white space.  Use a temp string to work
  // around malloc error that causes crash when using Xcode 9.1 + iOS SDK + -Os
  // optimization.
  ...
  if (end == std::string::npos) {
    assert(false);  // No argument/type name was specified ... check signature_
  ...
  if (type != INVALID_TYPE) {
    ...
    assert(false);  // Invalid argument type ... check signature_ to see where
                    // you went wrong
  }
}
...
#define CONSOLE_ARG_TYPE(__argname__, __argtype__, __argtokens__) __argtokens__,

ConsoleArgType IConsoleFunction::GetArgTypeFromString(
    const std::string& argstring) {
  // This map holds all of the accepted string tokens that we're matching per
  // type.
...
#define CONSOLE_ARG_TYPE(__argname__, __argtype__, ...)    \
  case ConsoleArgType_##__argname__:                       \
    arg = (new ConsoleArg<__argtype__>(name, isOptional)); \
    break;

IConsoleFunctionArg* IConsoleFunction::CreateConsoleArg(ConsoleArgType type,
                                                        StringID name,
                                                        bool isOptional) {
...
// case ConsoleArg_TYPE: arg     = ( new ConsoleArg<TYPE>::ConsoleArg( name,
// isOptional ) ); break;
#include "util/console/consoleSupportedTypes.def"
...
//------------------------------------------------------------------------------------------------------------------------------
const IConsoleFunctionArg* IConsoleFunction::FindArgument(
