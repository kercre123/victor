...
// Helper function - if a var is named with Hungarian notation - e.g. g_VarName
// or k_VarName - just use VarName for the console
static const char* SkipHungarianNotation(const char* inVarName) {
  if (inVarName && (inVarName[0] != 0)) {
    if (inVarName[1] == '_') {
      ...
      return &inVarName[2];
    } else if (((inVarName[0] == 'g') || (inVarName[0] == 'k')) &&
               isupper(int(inVarName[1]))) {
...
//------------------------------------------------------------------------------------------------------------------------------
// NOTE: static initialized console variables should have unregisterInDestructor
// false, this prevents actions during the static
//       destructors dynamically initialzed console variables, e.g. those
//       instantiated after main has started should have dynamic false which
//       will cause unregistration during destruction.
...
std::string ConsoleVar<signed char>::ToString(signed char value) const {
  // specialization required as otherwise this will be displayed as the ascii
  // equivalent
  char result[8];
