 ...
 *
 *  Description:
 *  - ConsoleFunction is a function that is registered with the ConsoleInterface
 *so that they can be called remotely via a console/terminal.
 *
 *******************************************************************************************************************************/
...
class IConsoleFunction {
 ...
 public:
  ...
  IConsoleFunction(const std::string& keystring, ConsoleFunc function,
                   const char* categoryName, const std::string& args);
  IConsoleFunction(const std::string& keystring, ConsoleStdFunc&& function,
                   const char* categoryName, const std::string& args);
  ~IConsoleFunction();
  void operator()(const ConsoleFunctionContext& context) const {
    if (function_ != nullptr) {
      function_((ConsoleFunctionContextRef)&context);
    }
  }
  ...
  const std::string GetFullPathString() const {
    return (std::string(category_) + "." + id_);
  }
 ...
 protected:
  ...
  IConsoleFunctionArg* CreateConsoleArg(ConsoleArgType type, StringID name,
                                        bool isOptional);
