 ...
 *
 *  Description:
 *  - The Console System allows users to register variables so that they can be
 *edited remotely via console/terminal
 *  - Users can also register functions with the system to be called remotely
...
class ConsoleSystem {
 ...
 public:
  ...
  template <typename T>
  void Register(T& value, const std::string& keystring,
                const std::string& category);
  void Register(const std::string& keystring, IConsoleVariable* variable);
  void Register(const std::string& keystring, ConsoleFunc function,
                const char* categoryName, const std::string& args);
  ...
  template <typename T>
  bool GetArgumentValue(const ConsoleFunctionContext& functiondata,
                        const char* argument, T& value);
 ...
 public:
  ...
  bool ParseConsoleFunctionCall(IConsoleFunction* functor, const char* token,
                                IConsoleChannel& channel);

 private:
  bool ParseConsoleVariableText(char* token, IConsoleChannel& channel,
                                bool readonly);
 ...
 public:
  ...
  const FunctionDatabase& GetFunctionDatabase() const {
    return consolefunctions_;
  }
...
//------------------------------------------------------------------------------------------------------------------------------
template <typename T>
inline void ConsoleSystem::Register(T& value, const std::string& keystring,
                                    const std::string& category) {
  // The IConsoleVariable will register itself within its constructor.
  IConsoleVariable* var =
      new ConsoleVar<T>(value, keystring.c_str(), category.c_str(), false);
  allocatedVariables_.push_back(var);
}

//------------------------------------------------------------------------------------------------------------------------------
template <typename T>
inline bool ConsoleSystem::GetArgumentValue(
    const ConsoleFunctionContext& functiondata, const char* argument,
    T& value) {
...
#define ANKI_CONSOLE_SYSTEM_INIT(iniPath) \
  Anki::Util::ConsoleSystem::Instance().FinishInitialization(iniPath);
#else
#define ANKI_CONSOLE_SYSTEM_INIT(iniPath)
#endif
...
ANKI_EXPORT uint8_t NativeAnkiUtilConsoleGetVar(
    int varIndex, int* ioNameLength, const char** outName,
    int* ioCategoryNameLength, const char** outCategoryName,
    double* outMinValue, double* outMaxValue, uint8_t* outIsToggleable,
    uint8_t* outIsIntType, uint8_t* outIsSigned) ANKI_VISIBLE;

// Value Getters
ANKI_EXPORT uint32_t NativeAnkiUtilConsoleGetVarValueString(
    const char* varName, char* outBuffer, uint32_t outBufferLen) ANKI_VISIBLE;
ANKI_EXPORT double NativeAnkiUtilConsoleGetVarValueAsDouble(const char* varName)
    ANKI_VISIBLE;
ANKI_EXPORT int64_t NativeAnkiUtilConsoleGetVarValueAsInt64(const char* varName)
    ANKI_VISIBLE;
ANKI_EXPORT uint64_t
NativeAnkiUtilConsoleGetVarValueAsUInt64(const char* varName) ANKI_VISIBLE;

// Value Setters
ANKI_EXPORT void NativeAnkiUtilConsoleToggleValue(const char* varName)
    ANKI_VISIBLE;
ANKI_EXPORT void NativeAnkiUtilConsoleSetValueWithString(
    const char* varName, const char* inString) ANKI_VISIBLE;

// Value Defaults
ANKI_EXPORT void NativeAnkiUtilConsoleResetAllToDefault() ANKI_VISIBLE;
ANKI_EXPORT void NativeAnkiUtilConsoleResetValueToDefault(const char* varName)
    ANKI_VISIBLE;
ANKI_EXPORT uint32_t NativeAnkiUtilConsoleIsDefaultValue(const char* varName)
    ANKI_VISIBLE;
...
ANKI_EXPORT uint32_t NativeAnkiUtilConsoleGetMaxFunctionSignatureLen()
    ANKI_VISIBLE;

// Function descriptions
ANKI_EXPORT uint8_t NativeAnkiUtilConsoleGetFunction(
    int varIndex, int* outNameLength, const char** outName,
    int* outCategoryLength, const char** outCategory, int* outSignatureLength,
    const char** outSignature) ANKI_VISIBLE;
// Call function
ANKI_EXPORT uint32_t NativeAnkiUtilConsoleCallFunction(
    const char* funcName, const char* funcArgs, uint32_t outTextLength,
    char* outText) ANKI_VISIBLE;
