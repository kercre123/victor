 ...
 *
 *  Description:
 *  - A Console Variable is a variable the can be registered with the
 *ConsoleInterface and be edited at runtime via a remote console/terminal.
...
class ConsoleVar : public IConsoleVariable {
 public:
  ...
  ConsoleVar(T& value, const char* id, const char* category, const T& minValue,
             const T& maxValue, bool unregisterInDestructor)
      : IConsoleVariable(id, category, unregisterInDestructor),
        _value(value),
        _minValue(minValue),
        _maxValue(maxValue),
  ...
  ConsoleVar(T& value, const char* id, const char* category, const char* values,
             bool unregisterInDestructor)
      : IConsoleVariable(id, category, unregisterInDestructor),
        _value(value),
        _minValue(0),
  ...
  virtual std::string GetDefaultAsString() const override {
    return ToString(_defaultValue);
  }

  virtual double GetAsDouble() const override {
    return numeric_cast_clamped<double>(_value);
  }
  virtual int64_t GetAsInt64() const override {
    return numeric_cast_clamped<int64_t>(_value);
  }
  virtual uint64_t GetAsUInt64() const override {
    return numeric_cast_clamped<uint64_t>(_value);
  }

  virtual double GetMinAsDouble() const override {
    return numeric_cast_clamped<double>(_minValue);
  }
  virtual double GetMaxAsDouble() const override {
    return numeric_cast_clamped<double>(_maxValue);
  }
  virtual int64_t GetMinAsInt64() const override {
    return numeric_cast_clamped<int64_t>(_minValue);
  }
  virtual int64_t GetMaxAsInt64() const override {
    return numeric_cast_clamped<int64_t>(_maxValue);
  }
  virtual uint64_t GetMinAsUInt64() const override {
    return numeric_cast_clamped<uint64_t>(_minValue);
  }
  virtual uint64_t GetMaxAsUInt64() const override {
    return numeric_cast_clamped<uint64_t>(_maxValue);
  }
  ...
  virtual const std::vector<std::string>& EnumValues() const override {
    return _enumValues;
  }
...
inline bool ConsoleVar<T>::ParseText(const char* text) {
  const std::vector<std::string>::iterator found =
      std::find(_enumValues.begin(), _enumValues.end(), text);
...
// must be declared specializations here (implemented in cpp) otherwise compiler
// will just use the defaults

//==============================================================================================================================
// Specializations for signed/unsigned char as stringstream treats them as ASCII
// otherwise
