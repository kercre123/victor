...
IConsoleFunctionArg::IConsoleFunctionArg(StringID name, bool optional)
    : name_(name), isOptional_(optional), isSet_(false) {}
...
//------------------------------------------------------------------------------------------------------------------------------
// Specialization for a char/int8_t ... istringstream will try to read it as a
// character, instead of an 8-bit integer
template <>
...
//------------------------------------------------------------------------------------------------------------------------------
// Specialization for unsigned char/uint8_t ... istringstream will try to read
// it as a character, instead of an 8-bit integer
template <>
...
#define CONSOLE_ARG_TYPE(__argname__, __argtype__, ...)        \
  template <>                                                  \
  ConsoleArgType ConsoleArg<__argtype__>::GetArgType() const { \
    return ConsoleArgType_##__argname__;                       \
  }
...
//------------------------------------------------------------------------------------------------------------------------------
// Explicit template instantiation ... these are the only types that we accept
// as console arguments.
