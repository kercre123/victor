...
AudioRecognizerProcessor::AudioRecognizerProcessor(
    const std::string& savedAudioDir) {
  if (ANKI_DEV_CHEATS) {
    _savedAudioDir = savedAudioDir;
  }
}

AudioRecognizerProcessor::~AudioRecognizerProcessor() { Stop(); }
...
void AudioRecognizerProcessor::SetSpeechRecognizer(SpeechRecognizer* newRecog) {
  ...
  if (_recognizer) {
    _recognizer->SetCallback(
        std::bind(&AudioRecognizerProcessor::AddRecognizerResult, this,
                  std::placeholders::_1));
  }
}

void AudioRecognizerProcessor::SetAudioInputSource(
  ...
  if (_audioInputSource) {
    _audioInputSource->SetCallback(
        std::bind(&AudioRecognizerProcessor::AudioSamplesCallback, this,
...
void AudioRecognizerProcessor::AudioSamplesCallback(const AudioSample* buffer,
                                                    uint32_t numSamples) {
  {
    std::lock_guard<std::mutex> lock(_componentsMutex);
    if (_recognizer) {
  ...
  if (ANKI_DEV_CHEATS && _minSamplesToCapture > 0) {
    ...
    if (_minSamplesToCapture <= numSamples) {
      ...
      auto curTimeString = std::to_string(
          std::chrono::duration_cast<std::chrono::milliseconds>(
              std::chrono::system_clock::now().time_since_epoch())
              .count());
      if (!_savedAudioDir.empty() &&
          Util::FileUtils::DirectoryDoesNotExist(_savedAudioDir)) {
        Util::FileUtils::CreateDirectory(_savedAudioDir);
      }
      const auto& saveFilePath = Util::FileUtils::FullFilePath(
          {_savedAudioDir, curTimeString + ".wav"});
      if (!WaveFile::SaveFile(saveFilePath, _savedAudio)) {
        PRINT_NAMED_ERROR(
            "AudioRecognizerProcessor.AudioSamplesCallback.SaveAudioClipFail",
            "Audio clip saving failed at path %s", saveFilePath.c_str());
      }
      _savedAudio.clear();
    } else {
...
void AudioRecognizerProcessor::AddRecognizerResult(
