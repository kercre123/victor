 ...
 * Created: 12/06/16
 *
 * Description: Simple interface that various speech recognizer implementations
 * can extend.
 *
...
class SpeechRecognizer {
 public:
  ...
  using SpeechCallback =
      std::function<void(const SpeechRecognizerCallbackInfo& info)>;

  void SetCallback(SpeechCallback callback = SpeechCallback{}) {
    _speechCallback = callback;
  }
  void Start();
  void Stop();

  virtual void Update(const AudioSample* audioData,
                      unsigned int audioDataLen) = 0;
