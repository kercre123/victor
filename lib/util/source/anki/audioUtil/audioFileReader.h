 ...
 * Created: 11/22/16
 *
 * Description: Platform independent interface for accessing a compressed audio
 * file's data in a "standard" format (16 bit signed PCM at 16kHz).
 *
...
class AudioFileReader : public IAudioInputSource {
 public:
  void SetSamplesPerChunk(uint32_t samplesPerChunk) {
  ...
  void DeliverAudio(bool doRealTime, bool addBeginSilence = false,
                    bool addEndSilence = false);
 ...
 private:
  ...
  bool GetNativeFileData(NativeAudioFileData& fileData,
