 ...
 * Created: 1/20/17
 *
 * Description: Component that uses native audio capture and feeds it to a
 * specified speech recognizer, then holds onto the results to be accessed by
 * another system.
 *
...
class AudioRecognizerProcessor {
 public:
  ...
  AudioRecognizerProcessor& operator=(AudioRecognizerProcessor&& other) =
      delete;
  AudioRecognizerProcessor(const AudioRecognizerProcessor& other) = delete;
  AudioRecognizerProcessor& operator=(const AudioRecognizerProcessor& other) =
      delete;
