...
#define AUDIOCAPTURE_ERROR(message, args...) \
  __android_log_print(ANDROID_LOG_ERROR, "AudioCaptureSystem", message, ##args)

namespace {
Anki::AudioUtil::AudioCaptureSystem::RequestCapturePermissionCallback
...
struct AudioCaptureSystemData {
  ...
  AudioChunk _inputBuffers[2] = {AudioChunk(kSamplesPerChunk),
                                 AudioChunk(kSamplesPerChunk)};
  ...
  void HandleCallback() {
    if (_recording) {
      ...
      // Note that because we're just alternating between buffers here, the data
      // passed to the callback below needs to be copied out (or processed in
      // place) before we fill up the alternate buffer and switch back.
      _enqueuedBuffer = (0 == _enqueuedBuffer) ? 1 : 0;
      (*_recorderBufferQueue)
          ->Enqueue(_recorderBufferQueue, _inputBuffers[_enqueuedBuffer].data(),
                    kBytesPerChunk);
...
static void HandleCallbackEntry(SLAndroidSimpleBufferQueueItf bufferQueue,
                                void* inUserData) {
  AudioCaptureSystemData* data =
      static_cast<AudioCaptureSystemData*>(inUserData);
  if (data) {
    data->HandleCallback();
  }
}
...
AudioCaptureSystem::AudioCaptureSystem(uint32_t samplesPerChunk,
                                       uint32_t sampleRate)
    : _samplesPerChunk(samplesPerChunk), _sampleRate_hz(sampleRate) {
...
void AudioCaptureSystem::Init() {
  ...
  setupSteps.emplace_back("realize engine", [this]() {
    return (*_impl->_engineObject)
        ->Realize(_impl->_engineObject, SL_BOOLEAN_FALSE);
  });

  // get the engine interface, which is needed in order to create other objects
  setupSteps.emplace_back("get engine interface", [this]() {
    return (*_impl->_engineObject)
        ->GetInterface(_impl->_engineObject, SL_IID_ENGINE,
                       &(_impl->_engineEngine));
  });
  ...
  setupSteps.emplace_back("create audio recorder", [this]() {
    ...
    return (*_impl->_engineEngine)
        ->CreateAudioRecorder(_impl->_engineEngine, &(_impl->_recorderObject),
                              &audioSrc, &audioSnk, 1, id, req);
  });

  // realize the audio recorder
  setupSteps.emplace_back("realize audio recorder", [this]() {
    SLresult result = (*_impl->_recorderObject)
                          ->Realize(_impl->_recorderObject, SL_BOOLEAN_FALSE);
    if (SL_RESULT_CONTENT_UNSUPPORTED == result) {
      AUDIOCAPTURE_ERROR(
          "Realize() recorder object resulted in "
          "SL_RESULT_CONTENT_UNSUPPORTED. Has android.permission.RECORD_AUDIO "
          "been requested?\n");
    }
    return result;
  });
  ...
  setupSteps.emplace_back("get recorder interface", [this]() {
    return (*_impl->_recorderObject)
        ->GetInterface(_impl->_recorderObject, SL_IID_RECORD,
                       &(_impl->_recorderRecord));
  });

  // get the buffer queue interface
  setupSteps.emplace_back("get bufferqueue interface", [this]() {
    return (*_impl->_recorderObject)
        ->GetInterface(_impl->_recorderObject, SL_IID_ANDROIDSIMPLEBUFFERQUEUE,
                       &(_impl->_recorderBufferQueue));
  });

  // register callback on the buffer queue
  setupSteps.emplace_back("register callback", [this]() {
    return (*_impl->_recorderBufferQueue)
        ->RegisterCallback(_impl->_recorderBufferQueue, HandleCallbackEntry,
                           _impl.get());
  });

  // Go through all the steps. If one fails, cleanup and abandon setup (this
  // replaces old goto functionality)
  for (auto& step : setupSteps) {
    SLresult result = step.second();
    if (SL_RESULT_SUCCESS != result) {
      AUDIOCAPTURE_ERROR(
          "ERROR: Problem initializing Android mic capture on step: %s. Code: "
          "%d\n",
...
AudioCaptureSystem::PermissionState AudioCaptureSystem::GetPermissionState(
    bool isRepeatRequest) const {
  ...
  Util::JClassHandle captureClass{
      envWrapper->FindClassInProject("com/anki/audioUtil/AudioCaptureSystem"),
      env};
  ...
  jmethodID hasPermissionMethodID =
      env->GetStaticMethodID(captureClass.get(), "hasCapturePermission", "()Z");
  jboolean permissionGranted =
      env->CallStaticBooleanMethod(captureClass.get(), hasPermissionMethodID);

  if (!permissionGranted) {
    jmethodID showRationaleMethodID = env->GetStaticMethodID(
        captureClass.get(), "shouldShowRationale", "()Z");
    jboolean shouldShowRationale =
        env->CallStaticBooleanMethod(captureClass.get(), showRationaleMethodID);

    // If we were denied permission we can check whether the OS says we should
    // give a rationale. If the OS says don't bother with the rationale and we
    // have requested permission sometime before, we know the user has selected
    // "don't show again", and won't be getting more prompts to allow mic
    // access. That means we need to tell them to go to their settings to enable
    // the access.
...
void AudioCaptureSystem::RequestCapturePermission(
    RequestCapturePermissionCallback resultCallback) const {
  auto envWrapper = Util::JNIUtils::getJNIEnvWrapper();
  JNIEnv* env = envWrapper->GetEnv();
  ...
  if (nullptr == captureClass) {
    PRINT_NAMED_ERROR(
        "AudioCaptureSystem.RequestCapturePermission.ClassNotFound",
        "Unable to find com.anki.audioUtil.AudioCaptureSystem");
    return;
  }
  ...
  jmethodID methodID = env->GetStaticMethodID(
      captureClass.get(), "requestCapturePermission", "()V");
  env->CallStaticVoidMethod(captureClass.get(), methodID);
}

AudioCaptureSystem::~AudioCaptureSystem() { StopRecording(); }
...
void AudioCaptureSystem::StartRecording() {
  if (_impl && !_impl->_recording) {
    ...
    (*_impl->_recorderRecord)
        ->SetRecordState(_impl->_recorderRecord, SL_RECORDSTATE_RECORDING);
    _impl->_enqueuedBuffer = 0;
    (*_impl->_recorderBufferQueue)
        ->Enqueue(_impl->_recorderBufferQueue,
                  _impl->_inputBuffers[_impl->_enqueuedBuffer].data(),
                  kBytesPerChunk);
  }
}
...
void AudioCaptureSystem::StopRecording() {
  if (_impl && _impl->_recording) {
    ...
    (*_impl->_recorderRecord)
        ->SetRecordState(_impl->_recorderRecord, SL_RECORDSTATE_STOPPED);
...
void JNICALL
Java_com_anki_audioUtil_AudioCaptureSystem_NativeRequestCapturePermissionCallback(
    JNIEnv* env, jobject clazz, jboolean permissionGranted) {
  std::lock_guard<std::mutex> lock(_requestPermCallbackLock);
