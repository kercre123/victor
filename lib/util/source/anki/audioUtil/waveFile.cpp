...
struct WaveFileHeaderData {
  ...
  static constexpr uint32_t _subchunk1Size =
      16;  // For PCM this omits a couple optional pieces
  static constexpr uint16_t _audioFormat = 1;

  static constexpr uint16_t _bitsPerSample =
      16;  // This is out of order in header for reuse

  static const std::array<uint8_t, 4> _subchunk2ID;
  static constexpr uint32_t _headerDataSize =
      _chunkID.size() + sizeof(uint32_t) + _format.size() +
  ...
  static std::vector<uint8_t> GetHeaderData(uint32_t numSamples,
                                            uint16_t numChannels,
                                            uint32_t sampleRate_hz) {
    const auto& subchunk2Size = Util::numeric_cast<uint32_t>(
        numSamples * numChannels * (_bitsPerSample / 8));

    const auto& chunkSize = Util::numeric_cast<uint32_t>(
        _format.size() +
        (_subchunk1ID.size() + sizeof(_subchunk1Size) + _subchunk1Size) +
    ...
    // Subchunk1ID
    headerData.insert(headerData.end(), _subchunk1ID.begin(),
                      _subchunk1ID.end());
    ...
    // ByteRate
    const auto& byteRate = Util::numeric_cast<uint32_t>(
        sampleRate_hz * numChannels * (_bitsPerSample / 8));
    PackData(byteRate, headerData);

    // BlockAlign
    const auto& blockAlign =
        Util::numeric_cast<uint16_t>(numChannels * (_bitsPerSample / 8));
    ...
    // Subchunk2ID
    headerData.insert(headerData.end(), _subchunk2ID.begin(),
                      _subchunk2ID.end());
    ...
    if (!ANKI_VERIFY(headerData.size() == _headerDataSize,
                     ...
                     "Expected size: %d Got size: %zu", _headerDataSize,
                     headerData.size())) {
...
// In C++17 These could even be defined as constexpr, but we're only on C++14
// currently
const std::array<uint8_t, 4> WaveFileHeaderData::_chunkID = {
    {'R', 'I', 'F', 'F'}};
const std::array<uint8_t, 4> WaveFileHeaderData::_format{{'W', 'A', 'V', 'E'}};
const std::array<uint8_t, 4> WaveFileHeaderData::_subchunk1ID{
    {'f', 'm', 't', ' '}};
const std::array<uint8_t, 4> WaveFileHeaderData::_subchunk2ID{
    {'d', 'a', 't', 'a'}};

static bool WriteToFile(const std::string& fileName, const uint8_t* dataBegin,
                        size_t numBytes, bool append) {
  bool success = false;
  ...
  if (fileOut.is_open()) {
    std::copy(dataBegin, dataBegin + numBytes,
...
bool WaveFile::SaveFile(const std::string& filename,
                        const AudioChunkList& chunkList, uint16_t numChannels,
                        uint32_t sampleRate_hz) {
  if (numChannels == 0) {
    // just no
  ...
  std::vector<uint8_t> headerData =
      WaveFileHeaderData::GetHeaderData(numSamples, numChannels, sampleRate_hz);

  // Put a max here in case we have a *lot* of data we're saving, so that we
  // write it out in pieces rather than use up more memory. Note this buffer
  // needs to at least be large enough to hold the header or the largest size of
  // an audio chunk, whichever is greater.
  ...
  for (const auto& chunk : chunkList) {
    ...
    if (bytesReady > 0 && bytesReady + nextChunkByteSize > kByteBufferMaxSize) {
    ...
    const auto* castedChunkBegin =
        reinterpret_cast<const uint8_t*>(&*chunk.begin());
    const auto* castedChunkEnd =
        reinterpret_cast<const uint8_t*>(&*chunk.end());
...
AudioChunkList WaveFile::ReadFile(const std::string& filename,
                                  std::size_t desiredSamplesPerChunk) {
  auto binaryData = Util::FileUtils::ReadFileAsBinary(filename);

  if (binaryData.size() <= WaveFileHeaderData::_headerDataSize) {
    PRINT_NAMED_WARNING("WaveFile.ReadFile.HeaderDataMissing",
                        "File only %zu bytes", binaryData.size());
    return AudioChunkList();
  }

  uint16_t numChannels = *reinterpret_cast<uint16_t*>(
      binaryData.data() + WaveFileHeaderData::_numChannelsOffset);
  if (numChannels == 0) {
    PRINT_NAMED_WARNING("WaveFile.ReadFile.NoChannels",
                        "Wave file header claims 0 channels. Aborting");
    return AudioChunkList();
  }
  ...
  const auto bytesPerChunk =
      desiredSamplesPerChunk * numChannels * sizeof(AudioSample);
  while (currentData + bytesPerChunk <= endDataMarker) {
    ...
    std::copy(dataStart, dataStart + (desiredSamplesPerChunk * numChannels),
              nextChunk.data());
  ...
  if (currentData < endDataMarker &&
      ((endDataMarker - currentData) % sizeof(AudioSample)) != 0) {
    auto extraBytesLen = (endDataMarker - currentData) % sizeof(AudioSample);
    endDataMarker -= extraBytesLen;
    PRINT_NAMED_WARNING("WaveFile.ReadFile.NotEvenlyDivisible",
                        "Clipping %zu bytes", extraBytesLen);
  }
  ...
  if (currentData < endDataMarker) {
    ...
    audioData.push_back(std::move(nextChunk));
    PRINT_NAMED_WARNING("WaveFile.ReadFile.SmallFinalChunk",
                        "Final chunk only %zu samples", extraSamples);
  }
