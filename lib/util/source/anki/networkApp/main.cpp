...
bool ParseArgs(int argc, const char* argv[], ConfigSettings& configSettings) {
  ...
  while (argIndex < argc) {
    ...
    if (argName && (argName[0] == '-')) {
      if (strcmp(&argName[1], "ip") == 0) {
        ...
        if (argIndex < argc) {
          ...
          printf("Send Mode %s - will send %d messages\n",
                 (configSettings._numSends > 0) ? "On" : "Off",
                 configSettings._numSends);
          handledArg = true;
        } else {
      ...
      } else if ((strcmp(&argName[1], "v") == 0) ||
                 (strcmp(&argName[1], "verbose") == 0)) {
...
static const char* k_MessagePreamble =
    "MESSAGE_10_ABCDEF_20_GHIJKL_30_MNOPQR_40_num_";

class DummyNetTransportDataReceiver
    : public Anki::Util::INetTransportDataReceiver {
 public:
  DummyNetTransportDataReceiver() : _numRecv(0), _numBytesRecv(0) {}

  virtual ~DummyNetTransportDataReceiver() {}

  virtual void ReceiveData(
      const uint8_t* buffer, uint32_t size,
      const Anki::Util::TransportAddress& sourceAddress) override {
    ++_numRecv;
    _numBytesRecv += size;
...
int main(int argc, const char* argv[]) {
  ...
  printfLoggerProvider.SetMinLogLevel(
      Anki::Util::ILoggerProvider::LOG_LEVEL_DEBUG);
  gLoggerProvider = &printfLoggerProvider;

  if (!ParseArgs(argc, argv, gConfigSettings)) {
    printf(
        "Usage: %s [-ip IP_ADDRESS] [-port PORT_NUMBER] [-send NUM_MESSAGES] "
        "[-freq TICKS_PER_MSG] [-rel 1/0] [-fakeUDP 1/0] [-fakeRandomLoss "
        "xx.x%%][-sleep MicroSec] [-v]\n",
        argv[0]);
    return EXIT_SUCCESS;
  }
  ...
  netEmulatorUDPSocket.SetRandomPacketLossPercentage(
      gConfigSettings._fakeUDPPacketLossPercentage);
  ...
  int tickCount = (gConfigSettings._freqSends > k_MaxTicksTilFirstSend)
                      ? (gConfigSettings._freqSends - k_MaxTicksTilFirstSend)
                      : 0;
  int numSent = 0;
...

  {
    ...
    destAddress.SetIPAddressFromString(gConfigSettings._ipAddress.c_str(),
                                       activePort);

    sendIpAddress = destAddress.GetIPAddress();
    printf("[NetworkTestApp] sendIpAddress = 0x%08x from '%s'\n", sendIpAddress,
           gConfigSettings._ipAddress.c_str());
    ...
    if (gConfigSettings._numSends > 0) {
      ...
      if (tickCount > gConfigSettings._freqSends) {
        ...
        if (numSent < gConfigSettings._numSends) {
          ...
          reliableTransport.SendData(gConfigSettings._sendReliable, destAddress,
                                     (uint8_t*)buffer,
