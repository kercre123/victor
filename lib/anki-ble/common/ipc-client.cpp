...
IPCClient::IPCClient(struct ev_loop* loop)
    : IPCEndpoint(loop), connect_watcher_(nullptr) {
#ifdef NDEBUG
...
void IPCClient::OnConnect() {
  ...
  delete connect_watcher_;
  connect_watcher_ = nullptr;
  AddPeerByFD(sockfd_);
}

void IPCClient::OnReceiveIPCMessage(const int sockfd, const IPCMessageType type,
  ...
  switch (type) {
    ...
    case IPCMessageType::OnPeripheralStateUpdate: {
      ...
      OnPeripheralStateUpdateArgs* args =
          (OnPeripheralStateUpdateArgs*)data.data();
      OnPeripheralStateUpdate(args->advertising, args->connection_id,
                              args->connected, args->congested);
    } break;
    case IPCMessageType::OnInboundConnectionChange: {
      logv("ipc-client: OnInboundConnectionChange");
      OnInboundConnectionChangeArgs* args =
          (OnInboundConnectionChangeArgs*)data.data();
    ...
    case IPCMessageType::OnReceiveMessage: {
      ...
      OnReceiveMessage(args->connection_id, characteristic_uuid, value);
    } break;
    case IPCMessageType::OnCharacteristicReadResult: {
      ...
      OnCharacteristicReadResultArgs* args =
          (OnCharacteristicReadResultArgs*)data.data();
      std::string characteristic_uuid(args->characteristic_uuid);
      std::vector<uint8_t> value(args->value, args->value + args->length);
      OnCharacteristicReadResult(args->connection_id, args->error,
    ...
    case IPCMessageType::OnDescriptorReadResult: {
      ...
      OnDescriptorReadResultArgs* args =
          (OnDescriptorReadResultArgs*)data.data();
      std::string characteristic_uuid(args->characteristic_uuid);
      std::string descriptor_uuid(args->descriptor_uuid);
      std::vector<uint8_t> value(args->value, args->value + args->length);
      OnDescriptorReadResult(args->connection_id, args->error,
                             characteristic_uuid, descriptor_uuid, value);
    } break;
    ...
    case IPCMessageType::OnOutboundConnectionChange: {
      ...
      OnOutboundConnectionChangeArgs* args =
          (OnOutboundConnectionChangeArgs*)data.data();
      ...
      OnOutboundConnectionChange(std::string(args->address), args->connected,
                                 args->connection_id, records);
    } break;
...
void IPCClient::SendMessage(const int connection_id,
  ...
  (void)strlcpy(args->characteristic_uuid, characteristic_uuid.c_str(),
  ...
  SendIPCMessageToServer(IPCMessageType::SendMessage, args_length,
                         (uint8_t*)args);
  free(args);
}
...
void IPCClient::ReadDescriptor(const int connection_id,
  ...
  (void)strlcpy(args.characteristic_uuid, characteristic_uuid.c_str(),
                sizeof(args.characteristic_uuid));
  (void)strlcpy(args.descriptor_uuid, descriptor_uuid.c_str(),
                sizeof(args.descriptor_uuid));
  SendIPCMessageToServer(IPCMessageType::DescriptorReadRequest, sizeof(args),
                         (uint8_t*)&args);
}
...
void IPCClient::Disconnect(const int connection_id) {
  DisconnectArgs args;
  args.connection_id = connection_id;
  SendIPCMessageToServer(IPCMessageType::Disconnect, sizeof(args),
                         (uint8_t*)&args);
}
...
void IPCClient::StartAdvertising(const BLEAdvertiseSettings& settings) {
  ...
  const std::vector<const BLEAdvertiseData*> ble_ad_data = {
      &(settings.GetAdvertisement()), &(settings.GetScanResponse())};
  std::vector<AdvertisingData*> ad_data = {&(args.advertisement),
                                           &(args.scan_response)};
  ...
  for (int i = 0; i < ble_ad_data.size(); i++) {
    ...
    if (dst->manufacturer_data_len > 0) {
      ...
      (void)memcpy(dst->manufacturer_data, manufacturer_data.data(),
                   dst->manufacturer_data_len);
    }
    ...
    if (dst->service_data_len > 0) {
      ...
      (void)memcpy(dst->service_data, service_data.data(),
                   dst->service_data_len);
    }
    dst->have_service_uuid = !src->GetServiceUUID().empty();
    if (dst->have_service_uuid) {
      strlcpy(dst->service_uuid, src->GetServiceUUID().c_str(),
              sizeof(dst->service_uuid));
    }
    dst->min_interval = src->GetMinInterval();
    dst->max_interval = src->GetMaxInterval();
  }

  SendIPCMessageToServer(IPCMessageType::StartAdvertising, sizeof(args),
                         (uint8_t*)&args);
}
...
void IPCClient::StartScan(const std::string& serviceUUID) {
  ...
  (void)strlcpy(args.service_uuid, serviceUUID.c_str(),
                sizeof(args.service_uuid));
  SendIPCMessageToServer(IPCMessageType::StartScan, sizeof(args),
                         (uint8_t*)&args);
}
...
void IPCClient::ConnectToPeripheral(const std::string& address) {
  ...
  SendIPCMessageToServer(IPCMessageType::ConnectToPeripheral, sizeof(args),
                         (uint8_t*)&args);
}
...
void IPCClient::DisconnectByAddress(const std::string& address) {
  ...
  (void)strlcpy(args.address, address.c_str(), sizeof(args.address));
  SendIPCMessageToServer(IPCMessageType::DisconnectByAddress, sizeof(args),
                         (uint8_t*)&args);
}
...
void IPCClient::SetAdapterName(const std::string& name) {
  ...
  SendIPCMessageToServer(IPCMessageType::SetAdapterName, sizeof(args),
                         (uint8_t*)&args);
}

void IPCClient::StopScan() { SendIPCMessageToServer(IPCMessageType::StopScan); }
