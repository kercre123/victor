...
IPCEndpoint::IPCEndpoint(struct ev_loop* loop) : loop_(loop) {
  CreateSocket();
...
void IPCEndpoint::ReceiveMessage(PeerState* p) {
  ...
  do {
    ...
    if (bytesRead > 0) {
      ...
      while (incoming_data.size() >= sizeof(*message)) {
        ...
        if (0 !=
            memcmp(message->magic, kIPCMessageMagic, sizeof(message->magic))) {
          loge(
              "ipc-endpoint: recv'd bad magic '%c%c%c%c' (%02X %02X %02X %02X)"
              ", expected '%c%c%c%c'",
              ...
              message->magic[0], message->magic[1], message->magic[2],
              message->magic[3], kIPCMessageMagic[0], kIPCMessageMagic[1],
              kIPCMessageMagic[2], kIPCMessageMagic[3]);

          OnPeerClose(p->GetFD());
          return;
        }
        if (message->version != kIPCMessageVersion) {
          loge("ipc-endpoint: recv'd version %u, expected %u", message->version,
               kIPCMessageVersion);
          OnPeerClose(p->GetFD());
          return;
        }
  ...
  if (bytesRead == -1) {
    ...
    loge("ipc-endpoint: recv. p->GetFD() = %d, errno = %d (%s)", p->GetFD(),
...
void IPCEndpoint::AddPeerByFD(const int fd) {
  ...
  read_write_watcher->set<IPCEndpoint, &IPCEndpoint::ReadWriteWatcherCallback>(
      this);
...
bool IPCEndpoint::SendMessageToPeer(const int fd, const IPCMessageType type,
                                    uint32_t length, uint8_t* val) {
  auto search = peers_.find(fd);
  if (search == peers_.end()) {
    return false;
  ...
  std::vector<uint8_t> packed_message(raw_pointer,
                                      raw_pointer + sizeof(message));
  if (length > 0) {
...
void IPCEndpoint::SendQueuedMessagesToPeer(const int sockfd) {
  ...
  while (!search->second->IsQueueEmpty()) {
    const std::vector<uint8_t>& packed_message =
        search->second->GetMessageAtFrontOfQueue();
    ssize_t bytesSent = send(sockfd, packed_message.data(),
                             packed_message.size(), MSG_NOSIGNAL);
    if (-1 == bytesSent) {
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        logv("ipc-endpoint: send would block");
        break;
      }
      logd("ipc-endpoint: send failed. errno = %s (%d)", strerror(errno),
           errno);
      OnPeerClose(sockfd);
      break;
    } else if (bytesSent < packed_message.size()) {
      loge("ipc-endpoint: send sent %zd bytes, expecting %zu bytes", bytesSent,
           packed_message.size());
      OnPeerClose(sockfd);
      break;
    } else {
...
IPCEndpoint::PeerState::~PeerState() {
  delete read_write_watcher_;
  read_write_watcher_ = nullptr;
}

int IPCEndpoint::PeerState::GetFD() const { return read_write_watcher_->fd; }
...
void IPCEndpoint::PeerState::AddMessageToQueue(
...
ScanResultRecord::ScanResultRecord(const std::string& address, const int rssi,
      ...
      switch (type) {
        ...
        case kADTypeCompleteListOf16bitServiceClassUUIDs: {
          ...
          while ((num_service_uuids < 4) && ((offset + 2) <= sz)) {
            std::vector<uint8_t> v(data.begin() + offset,
                                   data.begin() + offset + 2);
            ...
            (void)strlcpy(service_uuids[num_service_uuids], full_uuid.c_str(),
        ...
        case kADTypeCompleteListOf128bitServiceClassUUIDs: {
          ...
          while ((num_service_uuids < 4) && ((offset + 16) <= sz)) {
            ...
            (void)strlcpy(service_uuids[num_service_uuids], uuidString.c_str(),
                          sizeof(service_uuids[num_service_uuids]));
            num_service_uuids++;
            offset += 16;
          }
        ...
        case kADTypeCompleteLocalName: {
          ...
          (void)strlcpy(local_name, complete_local_name.c_str(),
                        sizeof(local_name));
        } break;
        case kADTypeManufacturerSpecificData: {
          manufacturer_data_len = length - 1;
          memcpy(manufacturer_data, data.data(),
  ...
  memcpy(advertisement_data, adv_data.data(),
