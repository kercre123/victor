...
static char nibbleToHexChar(uint8_t nib) { return "0123456789ABCDEF"[nib]; }
...
std::string byteVectorToHexString(const std::vector<uint8_t>& byteVector,
                                  const int numSpaces, const bool lowercase) {
  ...
  for (auto b : byteVector) {
    ...
    uint8_t low = b & 0xf;
    char hexDigitHigh =
        lowercase ? nibbleToHexCharLowerCase(high) : nibbleToHexChar(high);
    char hexDigitLow =
        lowercase ? nibbleToHexCharLowerCase(low) : nibbleToHexChar(low);
