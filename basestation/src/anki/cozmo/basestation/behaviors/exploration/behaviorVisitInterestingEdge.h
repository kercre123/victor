...
protected:
  ...
  IActionRunner* CreateLowLiftAndLowHeadActions(Robot& robot);
  
  // wait for new edges (while playing other actions if needed)
  void StartWaitingForEdges(Robot& robot);
  ...
  void StopSquintLoop(Robot& robot);
  ...
  void TransitionToS1_MoveToVantagePoint(Robot& robot, uint8_t attemptsDone);
  // S2: move slowly and try to gather more accurate information of where the border is
  void TransitionToS2_GatherAccurateEdge(Robot& robot);
  // S3: observe the border (play anim) from a safe close distance
  void TransitionToS3_ObserveFromClose(Robot& robot);
...
private:
  ...
  void PickGoals(const Robot& robot, BorderRegionScoreVector& validGoals) const;
  ...
  void GenerateVantagePoints(const Robot& robot, const BorderRegionScore& goal, const Vec3f& lookAtPoint, VantagePointVector& outVantagePoints) const;
  ...
  //
  static void FlagVisitedQuadAsNotInteresting(Robot& robot, float halfWidthAtRobot_mm, float farPlaneDistFromRobot_mm, float halfWidthAtFarPlane_mm);
  ...
  BaseClass::Status StateUpdate_GatheringAccurateEdge(Robot& robot);
