...
IBehavior* SimpleBehaviorChooser::ChooseNextBehavior(const Robot& robot, const MoodManager& moodManager, double currentTime_sec) const
{
  ...
  {
    float score = behavior->EvaluateScore(robot, moodManager, currentTime_sec);
    if (score > 0.0f)
    {
