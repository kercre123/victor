                      ...
                      const std::string& filename, const char *dispName, const std::set<std::string>& namesPresent)
{
  ...
  EncodedImage encodedImg(img);
  lastResult = robot.GetVisionComponent().SetNextImage(encodedImg);
  ASSERT_EQ(RESULT_OK, lastResult);
  ...
  if(SHOW_IMAGES)
  {
    ...
    auto faceIDs = robot.GetFaceWorld().GetKnownFaceIDsObservedSince(timestamp);
