...
#define CLST_UNREFERENCED_PARAM(param) param;
...
/** Returns the time elapsed since the given #start timestamp, measured in
 * seconds. */
OSAL_API double clst_clock_diff(long64 start);

OSAL_API void clst_usleep(unsigned useconds);

// See https://llvm.org/svn/llvm-project/llvm/trunk/lib/System/Win32/Process.inc
// and
...
OSAL_API void clst_get_filelist(char *find_path, char *file_extension,
                                filelist *list_head);
...
*/
OSAL_API int clst_thread_create(void **thread_handle, int stack_size,
                                clst_thread_func_type func, void *args);
