 ...
 */
typedef void (*clst_test_preamble_fn)(const clst_test_env env,
                                      void** user_data);

/** Test proper. This is where the test is actually run, and this is the only
 * function whose execution time is measured. Note that this function may be
 * executed multiple times per test case, while the preamble and postamble are
 * only executed once.
 */
typedef int (*clst_test_proper_fn)(const clst_test_env env, void* user_data,
                                   double* elapsed_time);
 ...
 * This function is called only once per test case.
 */
typedef void (*clst_test_postamble_fn)(const clst_test_env env,
                                       void* user_data);
/*@}*/
 ...
 */
struct _clst_test {
  /** Test identifier. It must be unique and reasonably short (32 characters
   * max.) */
  const char* test_name;
...
/** clst_sigfig prints value into str, with num_sig_figs number of significant
   figures, such that if num_sig_figs == 4: 54623.871 -> "54620" 54.623871 ->
   "54.62" 0.0546238 -> "0.05462"
  */
CLST_API int clst_sigfig(char* str, double value, int num_sig_figs);

/** All tests must immediately call this function in their main() routine to
 * transfer the control to the CLST framework. */
CLST_API int clst_main(int argc, char* argv[], clst_suite_type suite_type,
                       const char* suite_name, const char* suite_description,
                       int num_tests, const clst_test* test);

/* *** Utility macros and functions that the tests can use *** */

CLST_API cl_int clst_basic_test_setup(const clst_test_env env, int num_sources,
                                      const char** sources, const char* options,
                                      clst_basic_test_state* state,
                                      cl_context_properties priority);

CLST_API void clst_basic_test_release(const clst_test_env env,
                                      clst_basic_test_state* state);
 ...
 */
CLST_API int clst_test_error(const clst_test_env env, const char* filename,
                             unsigned line, cl_int errcode);

/** Notify function that can be used when the CL context is created for a test.
 * Use the pointer to the test environment as the 'user_data' argument to
 * clCreateContext.
 */
CLST_API void CL_CALLBACK clst_notify(const char* msg, const void* data,
                                      size_t data_size, void* _env);

/** Resets the state of the random number generator indirectly through a seed.
 * Use judiciously! Setting up the prng is generally the responsibility of the
 * clstlib, not the responsibility of the test!
 */
CLST_API void clst_srand(cl_ulong seed);

/** Resets the state of the random number generator directly to the given value.
 * Use judiciously! Setting up the prng is generally the responsibility of the
 * clstlib, not the responsibility of the test!
 */
 ...
 * Seeding the prng is generally the responsibility of the clstlib, not the
