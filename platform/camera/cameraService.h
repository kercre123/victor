...
class CameraService {
 public:
  ...
  //       Everyone else should be getting CameraCalibration data from
  //       NVStorageComponent!
  const CameraCalibration* GetHeadCamInfo();

  // Assign Webots supervisor
  // Webots processes must do this before creating AndroidHAL for the first
  ...
  // Releases lock on buffer for specified frameID acquired by calling
  // CameraGetFrame.
