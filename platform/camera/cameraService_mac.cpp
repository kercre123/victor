...
const u32 VISION_TIME_STEP =
    65;  // This should be a multiple of the world's basic time step!
...
const f32 kDistCoeffNoiseFrac =
    0.0f;  // fraction of the true value to use for uniformly distributed noise
           // (0 to disable)

// This buffers Webots camera images from the recent past, so that engine can
// request an image from a specific timestamp (in the past). The buffer contains
// pairs, where the first element is timestamp of image capture, and the second
// element is the RGB image itself.
constexpr size_t nBufferEntries = 3;  // 3 images = 195 ms
Util::FixedCircularBuffer<std::pair<TimeStamp_t, std::vector<u8>>,
                          nBufferEntries>
    webotsImageBuffer_;
...
// Apply lens distortion to the RGB image in frame, using the information from
// headCamInfo
...
CameraService::CameraService()
  ...
  imageBuffer_.resize(CAMERA_SENSOR_RESOLUTION_WIDTH *
                      CAMERA_SENSOR_RESOLUTION_HEIGHT * 3);

  if (nullptr != _engineSupervisor) {
    // Is the step time defined in the world file >= than the robot time? It
    // should be!
    DEV_ASSERT(ROBOT_TIME_STEP_MS >= _engineSupervisor->getBasicTimeStep(),
               "cameraService_mac.UnexpectedTimeStep");

    if (VISION_TIME_STEP %
            static_cast<u32>(_engineSupervisor->getBasicTimeStep()) !=
        0) {
      PRINT_NAMED_WARNING("cameraService_mac.InvalidVisionTimeStep",
                          "VISION_TIME_STEP (%d) must be a multiple of the "
                          "world's basic timestep (%.0f).",
                          VISION_TIME_STEP,
                          _engineSupervisor->getBasicTimeStep());
      return;
    }
    ...
    if (nullptr != headCam_) {
      ...
      // TODO: Not sure from Cyberbotics support message whether this should
      // include "+ VISION_TIME_STEP" or not...
      cameraStartTime_ms_ = GetTimeStamp();  // + VISION_TIME_STEP;
      lastImageCapturedTime_ms_ = 0;

      // Make the CozmoVizDisplay (which includes the nav map, etc.) invisible
      // to the camera. Note that the call to setVisibility() requires a pointer
      // to the camera NODE, _not_ the camera device (which is of type
      // webots::Camera*). There seems to be no good way to get the underlying
      // node pointer of the camera, so we have to do this somewhat hacky
      // iteration over all of the nodes in the world to find the camera node.
      const auto& vizNodes = WebotsHelpers::GetMatchingSceneTreeNodes(
          *_engineSupervisor, "CozmoVizDisplay");
...
Result CameraService::Update() { return RESULT_OK; }
...
void FillCameraInfo(const webots::Camera* camera, CameraCalibration& info) {
  ...
  if (kUseLensDistortion) {
    ...
    if (Util::IsFltGTZero(kDistCoeffNoiseFrac)) {
      ...
      for (s32 i = 0; i < 5; ++i) {
        info.distCoeffs[i] *= rng.RandDblInRange(1.f - kDistCoeffNoiseFrac,
                                                 1.f + kDistCoeffNoiseFrac);
      }
    }
  }
}  // FillCameraInfo
...
void ApplyLensDistortion(u8* frame, const CameraCalibration& headCamInfo) {
  // Apply radial/lens distortion. Note that cv::remap uses in inverse lookup to
  // find where the pixels in the output (distorted) image came from in the
  // source. So we have to compute the inverse distortion here. We do that using
  // cv::undistortPoints to create the necessary x/y maps for remap:
  ...
  if (x_undistorted.empty()) {
    ...
    const std::vector<f32> distCoeffs{kRadialDistCoeff1, kRadialDistCoeff2,
                                      kTangentialDistCoeff1,
                                      kTangentialDistCoeff2, kRadialDistCoeff3};
    const cv::Matx<f32, 3, 3> cameraMatrix(
        headCamInfo.focalLength_x, 0.f, headCamInfo.center_x, 0.f,
        headCamInfo.focalLength_y, headCamInfo.center_y, 0.f, 0.f, 1.f);

    cv::undistortPoints(points, points, cameraMatrix, distCoeffs, cv::noArray(),
                        cameraMatrix);
...
void CameraService::CameraSetCaptureSnapshot(bool start) { return; }

Result CameraService::InitCamera() { return RESULT_OK; }

Result CameraService::DeleteCamera() { return RESULT_OK; }

void CameraService::UnpauseForCameraSetting() { return; }
...
// Returns true and popuates buffer if we have an available image from at or
// before atTimestamp_ms.
bool CameraService::CameraGetFrame(u32 atTimestamp_ms,
  ...
  // This computation is based on Cyberbotics support's explanation for how to
  // compute the actual capture time of the current available image from the
  // simulated camera
  const TimeStamp_t currentImageTime_ms =
      (std::floor((currentTime_ms - cameraStartTime_ms_) / VISION_TIME_STEP) *
           VISION_TIME_STEP +
       cameraStartTime_ms_);

  // Do we have a 'new' image from webots?
  if (lastImageCapturedTime_ms_ != currentImageTime_ms) {
    // A 'new' image is available. Push the current webots image into the buffer
    // of available webots images
    auto& thisImage = webotsImageBuffer_.push_back();
    thisImage.first = currentImageTime_ms;
    auto& imageVec = thisImage.second;
    imageVec.resize(CAMERA_SENSOR_RESOLUTION_WIDTH *
                    CAMERA_SENSOR_RESOLUTION_HEIGHT * 3);

    const u8* image = headCam_->getImage();
    DEV_ASSERT(image != NULL,
               "cameraService_mac.CameraGetFrame.NullImagePointer");
    DEV_ASSERT_MSG(headCam_->getWidth() == headCamInfo_.ncols,
                   "cameraService_mac.CameraGetFrame.MismatchedImageWidths",
                   "HeadCamInfo:%d HeadCamWidth:%d", headCamInfo_.ncols,
                   headCam_->getWidth());

    // Copy from the webots 'image' into imageVec, converting from BGRA to RGB
    // along the way
  ...
  if ((atTimestamp_ms != 0) && (atTimestamp_ms < earliestImageTimestamp)) {
    return false;
  }

  u32 outputTimestamp = 0;
  // If atTimestamp_ms is zero, this indicates that the caller simply wants the
  ...
  if (atTimestamp_ms == 0) {
    ...
    // Find the image in the webots image buffer that is before or equal to
    // atTimestamp_ms, popping older images from the buffer along the way.
  ...
  rgb_ = Vision::ImageRGB(headCamInfo_.nrows, headCamInfo_.ncols,
                          imageBuffer_.data());

  rgb_.Resize(2.f, Vision::ResizeMethod::NearestNeighbor);

  buffer = Vision::ImageBuffer(
      const_cast<u8*>(reinterpret_cast<const u8*>(rgb_.GetDataPointer())),
      CAMERA_SENSOR_RESOLUTION_HEIGHT, CAMERA_SENSOR_RESOLUTION_WIDTH,
      Vision::ImageEncoding::RawRGB, outputTimestamp, _imageFrameID);
