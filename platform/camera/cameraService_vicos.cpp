 ...
 *
 * Description:
 *               Defines interface to a camera system provided by the
 *OS/platform
 *
 ...
 * Removes instance
 */
void CameraService::removeInstance() { Util::SafeDelete(_instance); };
...
CameraService::~CameraService() { DeleteCamera(); }

void CameraService::RegisterOnCameraRestartCallback(
...
bool IsCameraReady() {
  return (_camera != NULL && _powerState == CameraPowerState::Running);
}
...
Result CameraService::InitCamera() {
  ...
  if (status == ANKI_CAMERA_STATUS_RUNNING &&
    ...
    LOG_WARNING("CameraService.InitCamera.CameraStillRunning",
                "Camera is in state %d, power state %d", status, _powerState);
  ...
  if (rc != 0) {
    LOG_ERROR("CameraService.InitCamera.CameraInitFailed",
              "camera_init error %d", rc);
    _powerState = CameraPowerState::Off;
    return RESULT_FAIL;
  }
  ...
  if (rc != 0) {
    LOG_ERROR("CameraService.InitCamera.CameraStartFailed",
              "camera_start error %d", rc);
    _powerState = CameraPowerState::Off;
    return RESULT_FAIL;
  }
...
Result CameraService::DeleteCamera() {
  ...
  if (_camera == NULL || _powerState == CameraPowerState::Off) {
  ...
  if (res != 0) {
    LOG_ERROR("CameraService.DeleteCamera.CameraStopFailed",
              "camera_stop error %d", res);
    _powerState = CameraPowerState::Running;
    return RESULT_FAIL;
  }
  ...
  if (res != 0) {
    LOG_ERROR("CameraService.DeleteCamera.CameraReleaseFailed",
              "camera_release error %d", res);
    _powerState = CameraPowerState::Running;
    return RESULT_FAIL;
  }
...
Result CameraService::Update() {
  ...
  if (status != ANKI_CAMERA_STATUS_RUNNING) {
    ...
    if (status == ANKI_CAMERA_STATUS_OFFLINE) {
      LOG_INFO("CameraService.Update.Offline", "Camera is offline, re-initing");
    ...
    if ((rc == 0) && (status == ANKI_CAMERA_STATUS_IDLE)) {
      LOG_INFO("CameraService.Update.Idle", "Camera is idle, restarting");
...
void CameraService::CameraSetWhiteBalanceParameters(f32 r_gain, f32 g_gain,
                                                    f32 b_gain) {
  if (!IsCameraReady()) {
    return;
  }
  ...
  if (_waitingForFormatChange) {
    PRINT_NAMED_INFO(
        "CameraService.CameraSetWhiteBalanceParameters.FormatChanging",
...
void CameraService::CameraSetCaptureFormat(Vision::ImageEncoding format) {
  ...
  switch (format) {
    ...
    default:
      PRINT_NAMED_WARNING(
          "CameraService.CameraSetCaptureFormat.UnsupportedFormat", "%s",
          EnumToString(format));
      return;
  }
  ...
  PRINT_NAMED_INFO("CameraService.CameraSetCaptureFormat.SetFormat", "%s",
                   EnumToString(format));
...
void CameraService::CameraSetCaptureSnapshot(bool start) {
  ...
  PRINT_NAMED_INFO("CameraService.CameraSetCaptureSnapshot", "%s snapshot mode",
                   (start ? "Starting" : "Stopping"));
  camera_set_capture_snapshot(_camera, start);
}

bool CameraService::CameraGetFrame(u32 atTimestamp_ms,
  ...
  if (atTimestamp_ms != 0) {
    ...
    desiredImageTimestamp_ns =
        ((((int64_t)atTimestamp_ms - (int64_t)now_ms)) * 1000000LL) + now_ns;
  }

  int rc =
      camera_frame_acquire(_camera, desiredImageTimestamp_ns, &capture_frame);
  if (rc != 0) {
    return false;
  }
  ...
  if (capture_frame->timestamp != 0) {
    ...
    const TimeStamp_t frame_time_ms =
        now_ms - static_cast<TimeStamp_t>(offset_ns / 1000000LL);
  ...
  buffer = Vision::ImageBuffer(capture_frame->data, capture_frame->height,
                               capture_frame->width, _curFormat, timestamp,
                               _imageFrameID);
