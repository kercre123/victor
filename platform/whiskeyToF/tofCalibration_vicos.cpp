...
void set_calibration_save_path(const std::string& path) { _savePath = path; }

// Path is expected to end in "/"
int save_calibration_to_disk(void* calib, ssize_t size, const std::string& path,
                             const std::string& filename) {
  PRINT_NAMED_INFO("save_calibration_to_disk", "saving to %s%s", path.c_str(),
                   filename.c_str());
  ...
  if (f != nullptr) {
    ...
    if (rc != count) {
      PRINT_NAMED_ERROR("save_calibration_to_disk",
                        "fwrite to %s failed with %d", buf, ferror(f));
      rc = -1;
    } else {
      rc = 0;
    }
    ...
    PRINT_NAMED_ERROR("save_calibration_to_disk",
                      "failed to open file %s with %d", buf, errno);
  }
  return rc;
}
...
int load_calibration_from_disk(void* calib, ssize_t size,
  ...
  if (f != nullptr) {
    ...
    if (rc != count) {
      PRINT_NAMED_ERROR("load_calibration_from_disk", "fread failed %d",
                        ferror(f));
      rc = -1;
    } else {
      rc = 0;
    ...
    PRINT_NAMED_ERROR("load_calibration_from_disk", "failed to open %s with %d",
                      path.c_str(), errno);
  }
...
int load_calibration(VL53L1_Dev_t* dev) {
  ...
  int rc =
      load_calibration_from_disk(&calib, sizeof(calib), "/factory/tof.bin");
  if (rc < 0) {
    PRINT_NAMED_INFO("load_calibration", "Attempting to load old calib format");

    rc = load_calibration_from_disk(&calib, sizeof(calib),
                                    "/factory/tof_right.bin");
    if (rc < 0) {
      PRINT_NAMED_ERROR("load_calibration",
                        "Failed to load old tof calibration");
    }
  }
  ...
  if (rc == 0) {
    // DVT1 zone calibration was saved as the "stmvl531_zone_calibration_data_t"
    // structure from the kernel driver. This structure contains a u32 id field
    // before the VL53L1_ZoneCalibrationData_t. So we need to recreate the
    // layout of that struct in order to properly load the saved calibration
    // data.
    struct {
      uint32_t garbage;
    ...
    rc = load_calibration_from_disk(&buf, sizeof(buf),
                                    "/factory/tofZone_right.bin");
    if (rc == 0) {
      calibZone = buf.data;
    }
  } else {
    PRINT_NAMED_INFO("load_calibration", "Loading zone data");
    rc = load_calibration_from_disk(&calibZone, sizeof(calibZone),
                                    "/factory/tofZone.bin");
  }

  if (rc < 0) {
    PRINT_NAMED_ERROR("load_calibration",
                      "Failed to load tof zone calibration");
  } else {
...
int perform_xtalk_calibration(VL53L1_Dev_t* dev) {
  ...
  return VL53L1_PerformXTalkCalibration(
      dev, VL53L1_XTALKCALIBRATIONMODE_SINGLE_TARGET);
}
...
int run_xtalk_calibration(VL53L1_Dev_t* dev) {
  ...
  // If there was no crosstalk calibration needed then zero-out xtalk
  // calibration data before setting it
  if (noXtalk) {
...
// --------------------Offset Calibration--------------------
int perform_offset_calibration(VL53L1_Dev_t* dev, uint32_t dist_mm,
                               float reflectance) {
  VL53L1_Error rc = VL53L1_SetOffsetCalibrationMode(
      dev, VL53L1_OFFSETCALIBRATIONMODE_MULTI_ZONE);
  return_if_error(
      rc, "perform_offset_calibration: SetOffsetCalibrationMode failed");

  return VL53L1_PerformOffsetCalibration(
      dev, dist_mm, (FixPoint1616_t)(reflectance * (2 << 16)));
}

int run_offset_calibration(VL53L1_Dev_t* dev, uint32_t distanceToTarget_mm,
  ...
  rc = save_calibration_to_disk(calib);
  return_if_error(rc,
                  "run_offset_calibration: Save calibration to disk failed");

  rc = VL53L1_SetCalibrationData(dev, &calib);
  return_if_error(rc, "run_offset_calibration: Set calibration data failed");

  // Offset calibration populates the ZoneCalibrationData struct when configured
  // for multizones so we also need to save that calibration data
  VL53L1_ZoneCalibrationData_t calibZone;
  memset(&calibZone, 0, sizeof(calibZone));
  ...
  return_if_error(
      rc, "run_offset_calibration: Save zone calibration to disk failed");

  rc = VL53L1_SetZoneCalibrationData(dev, &calibZone);
  return_if_error(rc,
                  "run_offset_calibration: Set zone calibration data failed");

  return rc;
}

int perform_calibration(VL53L1_Dev_t* dev, uint32_t dist_mm,
                        float reflectance) {
