...
int open_dev(VL53L1_Dev_t* dev) {
  ...
  status = VL53L1_platform_init(dev, 0x29, 1, /* comms_type  I2C*/
  ...
  LOG_INFO("ToF.open_dev", "Name: %s Type: %s ID: %s Ver: %d.%d",
...
int setup_roi_grid(VL53L1_Dev_t* dev, const int rows, const int cols) {
  ...
  if (rows > MAX_ROWS) {
    LOG_ERROR("ToF.setup_roi_grid", "Cannot set %d rows (max %d)", rows,
              MAX_ROWS);
    return -1;
  }
  ...
  if (cols > MAX_COLS) {
    LOG_ERROR("ToF.setup_roi_grid", "Cannot set %d cols (max %d)", cols,
              MAX_ROWS);
    return -1;
  }
