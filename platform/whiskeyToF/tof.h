...
class ToFSensor {
 public:
  ...
  // hasDataUpdatedSinceLastCall indicates if the reading has changed since the
  // last time this function was called Data is only updated while ranging is
  // enabled
  ...
  // percentage There are 3 calibration steps, each has certain requirements
  ...
  // These are just suggested setups, other setups will work for crosstalk and
  // offset calibration.
  // - Crosstalk
  //     Idea is you just want to capture photons coming back due to the
  //     coverglass which is why you don't want to be looking at a target as the
  //     photons reflecting off the target will overwhelm the photons from the
  //     coverglass. It is possible to use a target with low reflectance at a
  //     closer distance instead of no target. To figure out proper setup, do
  //     offset calibration first and then try different targets and distances
  //     until you are 10% under ranging (distance returned is 10% of what it
  //     should be). The under ranging is due to the photons coming from the
  //     coverglass as opposed to the target.
  // - Offset
  //     Lots of different setups available by varying distance and target
  //     reflectance. Need a distance and target such that you get enough
  //     photons to not be affected by the coverglass but not so many that you
  //     saturate the sensor. Medium reflectance at somewhere between 100mm and
  //     400mm should be good. To go closer < 100mm, you would need lower
  //     reflectance like 5%.
