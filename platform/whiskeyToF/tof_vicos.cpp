...
ToFSensor::CommandResult run_calibration(uint32_t distanceToTarget_mm,
  ...
  if (rc < 0) {
    PRINT_NAMED_ERROR("ToFSensor.PerformCalibration.RightFailed",
                      "Failed to calibrate right sensor %d", rc);
  }

  _isCalibrating = false;

  ToFSensor::CommandResult res =
      (rc < 0 ? ToFSensor::CommandResult::CalibrateFailed
...
void ParseData(VL53L1_MultiRangingData_t& mz_data, RangeDataRaw& rangeData) {
  ...
  // Populate the list of "readings" for this ROIs RangingData for each object
  // detected
  if (mz_data.NumberOfObjectsFound > 0) {
    ...
    for (int r = 0; r < mz_data.NumberOfObjectsFound; r++) {
      ...
      reading.signalRate_mcps =
          CONVERT_1616_TO_FLOAT(mz_data.RangeData[r].SignalRateRtnMegaCps);
      reading.ambientRate_mcps =
          CONVERT_1616_TO_FLOAT(mz_data.RangeData[r].AmbientRateRtnMegaCps);
      reading.sigma_mm =
          CONVERT_1616_TO_FLOAT(mz_data.RangeData[r].SigmaMilliMeter);
      reading.rawRange_mm = mz_data.RangeData[r].RangeMilliMeter;

      // For all valid detected objects in this ROI keep track of which one was
      // the closest and use that for the overall processedRange_mm
...
// 1) Sometimes when starting ranging, we only ever get back invalid range
// readings.
//    You can stop and start ranging again and sometimes the sensor will
//    recover. It is also possible to go from valid readings to invalid readings
//    after calling stop and start.
// 2) It appears that calibration or some initial setting is changing when the
// sensor
//    is stopped/started. If you calibrate and then look at the readings, they
//    look very good. They are accurate within a couple of millimeters. However,
//    if you stop then start ranging again, all of the readings will have a
//    ~30mm offset. Most of calibration is still good as the readings are
//    indifferent towards the target/material.
...
void ProcessLoop() {
  while (!_stopProcessing) {
    ...
    if (_commandQueue.size() > 0) {
      ...
      switch (cmd.first) {
        ...
        case Command::SetupSensors: {
          ...
          if (_dev.platform_data.i2c_file_handle <= 0) {
            ...
            if (rc < 0) {
              ...
              PRINT_NAMED_ERROR("ToF.ProcessLoop.SetupSensors",
          ...
          if (rc < 0) {
            ...
            PRINT_NAMED_ERROR("ToF.ProcessLoop.SetupFailed",
                              "Failed to setup sensor");
          }
        ...
        case Command::PerformCalibration: {
          ...
          _rangingEnabled = false;
          res = run_calibration(_distanceToCalibTarget_mm,
    ...
    if (_rangingEnabled) {
      ...
      //   for(int j = 0; j < 4; j++)
      //   {
      //     if(data.data[i*4 + j].numObjects > 0 && data.data[i*4 +
      //     j].readings[0].status == 0)
      //     {
      //       ss << std::setw(7) << (uint32_t)(data.data[i*4 +
      //       j].processedRange_mm); lastValid.data[i*4 + j] = data.data[i*4 +
      //       j];
      //     }
      //     else
      //     {
      //       ss << std::setw(7) << (uint32_t)(lastValid.data[i*4 +
      //       j].processedRange_mm);
      //     }
      //   }
      //   ss << "\n";
    ...
    // Ranging is not enabled so sleep
      ...
      std::this_thread::sleep_for(
          std::chrono::milliseconds(BS_TIME_STEP_MS / 2));
    }
  }
...
bool ToFSensor::IsRanging() const { return _rangingEnabled; }

bool ToFSensor::IsCalibrating() const { return _isCalibrating; }
...
Result ToFSensor::Update() { return RESULT_OK; }
