...
void ToFSensor::removeInstance() { Util::SafeDelete(_instance); }
...
ToFSensor::ToFSensor() {
  if (nullptr != _engineSupervisor) {
    DEV_ASSERT(ROBOT_TIME_STEP_MS >= _engineSupervisor->getBasicTimeStep(),
               "tof_mac.UnexpectedTimeStep");

    leftSensor_ = _engineSupervisor->getRangeFinder("leftRangeSensor");
    leftSensor_->enable(ROBOT_TIME_STEP_MS);

    // Make the CozmoVizDisplay (which includes the nav map, etc.) invisible to
    // the Rangefinder. Note that the call to setVisibility() requires a pointer
    // to the Rangefinder NODE, _not_ the Rangefinder device (which is of type
    // webots::Rangefinder*). There seems to be no good way to get the
    // underlying node pointer of the Rangefinder, so we have to do this
...
bool ToFSensor::IsRanging() const { return true; }

bool ToFSensor::IsValidRoiStatus(uint8_t status) const { return true; }
...
bool ToFSensor::IsCalibrating() const { return false; }

void ToFSensor::SetLogPath(const std::string& path) { return; }
