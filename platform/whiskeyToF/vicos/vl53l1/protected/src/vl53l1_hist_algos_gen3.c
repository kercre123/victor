	...
	VL53L1_range_results_t                 *presults)
{
	...
	VL53L1_hist_calc_zero_distance_phase(&(palgo->VL53L1_p_010));
	...
	VL53L1_hist_remove_ambient_bins(&(palgo->VL53L1_p_010));
	...
	for (p = 0; p < palgo->VL53L1_p_051; p++) {

		ppulse_data = &(palgo->VL53L1_p_002[p]);
		...
		if (status == VL53L1_ERROR_NONE) {
			status =
				VL53L1_f_023(
					...
					&(palgo->VL53L1_p_010),
					palgo,
					palgo->VL53L1_p_010.VL53L1_p_004,
					&(palgo->VL53L1_p_052));
		}
	...
	for (p = 0; p < palgo->VL53L1_p_051; p++) {
		...
		if (status == VL53L1_ERROR_NONE)
			status =
			VL53L1_f_011(
			palgo->VL53L1_p_010.vcsel_width,
			palgo->VL53L1_p_010.VL53L1_p_019,
			palgo->VL53L1_p_010.total_periods_elapsed,
			palgo->VL53L1_p_010.result__dss_actual_effective_spads,
			prange_data);
	...
	VL53L1_hist_gen3_algo_private_data_t  *palgo)
{
	...
	for (lb = pdata->VL53L1_p_015; lb <= pdata->VL53L1_p_016; lb++) {
		...
		palgo->VL53L1_p_048[i]  = 0;
		palgo->VL53L1_p_008[i] = 0;

		for (w = 0; w < (pdata->VL53L1_p_055 << 1); w++) {





			j = lb + w + palgo->VL53L1_p_031;
			j = j - pdata->VL53L1_p_055;
			j = j % palgo->VL53L1_p_031;
			...
			if (i < ppulse->VL53L1_p_024 && j <
					ppulse->VL53L1_p_024) {
				if (w < pdata->VL53L1_p_055)
					palgo->VL53L1_p_048[i] +=
						ppulse->bin_data[j];
				else
					palgo->VL53L1_p_048[i] -=
						ppulse->bin_data[j];
	...
	VL53L1_hist_gen3_algo_private_data_t  *palgo)
{
	...
	for (lb = pdata->VL53L1_p_015; lb < pdata->VL53L1_p_016; lb++) {
		...
		if (i < palgo->VL53L1_p_024 &&
			...
			if (palgo->VL53L1_p_048[i] <= 0 &&
				palgo->VL53L1_p_048[j] > 0) {





				bin_x_delta = palgo->VL53L1_p_048[j] -
						palgo->VL53L1_p_048[i];
	...
	uint32_t *pmean_phase)
{
	...
	mean_phase = mean_phase % ((int32_t)VL53L1_p_031 * 2048);
