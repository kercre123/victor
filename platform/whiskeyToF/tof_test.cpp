...
int main(int argc, char** argv) {
  ...
  if (argc > 1) {
    if (argv[1][0] == 'c') {
      ...
      PRINT_NAMED_INFO("ToFTest", "Calibrating at %u with reflectance %f", dist,
                       reflectance);
      ...
      for (int j = 0; j < 4; j++) {
        ...
        if (data.data[i * 4 + j].numObjects > 0) {
          ...
          if (data.data[i * 4 + j].readings[0].status == 0) {
            ss << std::setw(7)
               << (uint32_t)(data.data[i * 4 + j].processedRange_mm);
            lastValid.data[i * 4 + j] = data.data[i * 4 + j];
          } else {
            ss << std::setw(7)
               << (uint32_t)(lastValid.data[i * 4 + j].processedRange_mm);
          }
        } else {
          ss << std::setw(7)
             << (uint32_t)(lastValid.data[i * 4 + j].processedRange_mm);
