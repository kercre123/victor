...
std::vector<std::string> split(const std::string& value, const std::string& delimiter)
{
    ...
    size_t start = 0; 
    size_t curr = 0;
    while ((curr = value.find(delimiter, start)) != std::string::npos){
        tokens.push_back(value.substr(start, curr - start));
        start = curr + delimiter.length();
    }
    return tokens;
