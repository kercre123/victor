...
void Usage() { fprintf(stderr, "Usage: %s [--help] file\n", PROCNAME); }
...
int main(int argc, const char* argv[]) {
  ...
  if (!ok) {
    LOG_ERROR("VicLogKernelPanic.CopyFile", "Failed to copy panic log %s",
              logpath.c_str());
  ...
  DASMSG(robot_kernel_panic, "robot.kernel.panic",
         "Robot has rebooted after kernel panic");
