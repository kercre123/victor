...
static void AgentCallback(GDBusConnection* connection, const gchar* sender,
...
void OnTechnologyChanged(GDBusConnection* connection, const gchar* sender_name,
                         const gchar* object_path, const gchar* interface_name,
  ...
  duration_s = std::difftime(
      t, (connected ? sLastWifiDisconnect_tm : sLastWifiConnect_tm));

  DASMSG(wifi_connection_status, event, "WiFi connection status changed.");
...
void Initialize(std::shared_ptr<TaskExecutor> taskExecutor) {
  ...
  static GThread* thread2 =
      g_thread_new("init_thread", ConnectionThread, nullptr);
  ...
  guint handle = g_dbus_connection_signal_subscribe(
      gdbusConn, "net.connman", "net.connman.Technology",
...
void ScanCallback(GObject* source_object, GAsyncResult* result,
                  gpointer user_data) {
  sTaskExecutor->Wake([]() { sWifiScanCompleteSignal.emit(); });
}

WifiScanErrorCode ScanForWiFiAccessPoints(
...
WifiScanErrorCode GetWiFiServices(std::vector<WiFiScanResult>& results,
  ...
  if (scan) {
    ...
    tech_proxy = conn_man_bus_technology_proxy_new_for_bus_sync(
        G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, "net.connman",
        "/net/connman/technology/wifi", nullptr, &error);
    if (error) {
    ...
    success =
        conn_man_bus_technology_call_scan_sync(tech_proxy, nullptr, &error);
    g_object_unref(tech_proxy);
    if (error) {
      loge("error asking connman to scan for wifi access points [%s]",
           error->message);
  ...
  manager_proxy = conn_man_bus_manager_proxy_new_for_bus_sync(
      G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, "net.connman", "/", nullptr,
      &error);
  if (error) {
  ...
  success = conn_man_bus_manager_call_get_services_sync(
      manager_proxy, &services, nullptr, &error);
  ...
  for (gsize i = 0; i < g_variant_n_children(services); i++) {
    WiFiScanResult result{
        WiFiAuth::AUTH_NONE_OPEN, false, false, 0, "", false, false};
    ...
    for (gsize j = 0; j < g_variant_n_children(attrs); j++) {
      ...
      if (g_str_equal(key, "Ethernet")) {
        for (gsize k = 0; k < g_variant_n_children(val); k++) {
          ...
          GVariant* ethernet_key_v =
              g_variant_get_child_value(ethernet_attr, 0);
          GVariant* ethernet_val_v =
              g_variant_get_child_value(ethernet_attr, 1);
          GVariant* ethernet_val = g_variant_get_variant(ethernet_val_v);
          const char* ethernet_key =
              g_variant_get_string(ethernet_key_v, nullptr);
          if (g_str_equal(ethernet_key, "Interface")) {
            if (g_str_equal(g_variant_get_string(ethernet_val, nullptr),
                            WIFI_DEVICE)) {
              iface_is_wlan0 = true;
            } else {
      ...
      if (g_str_equal(key, "Security")) {
        for (gsize k = 0; k < g_variant_n_children(val); k++) {
          ...
          const char* security_val_str =
              g_variant_get_string(security_val, nullptr);
...
void ScanForWiFiAccessPointsAsync() {
  ...
  conn_man_bus_technology_call_scan(tech_proxy, nullptr, ScanCallback, nullptr);
}
...
void ConnectCallback(GObject* source_object, GAsyncResult* result,
                     gpointer user_data) {
  g_mutex_lock(&connectMutex);
  struct ConnectInfo* data = (ConnectInfo*)user_data;

  conn_man_bus_service_call_connect_finish(data->service, result, &data->error);
  ...
  if (!strcmp(method_name, "RequestInput")) {
    ...
    if (wpaConnectInfo->name) {
      ...
      g_variant_builder_add(dict_builder, "{sv}", "Name",
                            g_variant_new_string(wpaConnectInfo->name));
    }
    if (wpaConnectInfo->ssid) {
      logi("%s: found 'SSID'", __func__);
      g_variant_builder_add(dict_builder, "{sv}", "SSID",
                            g_variant_new_bytestring(wpaConnectInfo->ssid));
    }
    if (wpaConnectInfo->passphrase) {
      logi("%s: found 'Passphrase'", __func__);
      g_variant_builder_add(dict_builder, "{sv}", "Passphrase",
                            g_variant_new_string(wpaConnectInfo->passphrase));
    }
  ...
  if (!strcmp(method_name, "ReportError")) {
    ...
    if (++wpaConnectInfo->retryCount < MAX_NUM_ATTEMPTS) {
      ...
      g_dbus_method_invocation_return_dbus_error(
          invocation, "net.connman.Agent.Error.Retry", "");
    }
  }
}
...
bool RegisterAgent(struct WPAConnectInfo* wpaConnectInfo) {
  ...
  manager = conn_man_bus_manager_proxy_new_sync(
      gdbusConn, G_DBUS_PROXY_FLAGS_NONE, "net.connman", "/", nullptr, &error);
  if (error) {
  ...
  guint agentId = 0;
  agentId = g_dbus_connection_register_object(
      gdbusConn, agentPath, introspection_data->interfaces[0], &agentVtable,
      wpaConnectInfo, nullptr, &error);
  if (agentId == 0 || error != NULL) {
    loge("Error registering agent object");
    return false;
  }

  if (!conn_man_bus_manager_call_register_agent_sync(manager, agentPath,
                                                     nullptr, &error)) {
...
bool RemoveWifiService(std::string ssid) {
  ...
  success = conn_man_bus_service_call_remove_sync(service, nullptr, &error);
...
ConnectWifiResult ConnectWiFiBySsid(std::string ssid, std::string pw,
  ...
  if (currentServiceVariant != nullptr) {
    // we have a connected service and it *isn't* the one we are currently
    // connected to
...
ConnManBusService* GetServiceForPath(std::string objectPath) {
  ...
  ConnManBusService* service = conn_man_bus_service_proxy_new_for_bus_sync(
      G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, "net.connman",
      objectPath.c_str(), nullptr, &error);

  if (service == nullptr || error != nullptr) {
    Log::Write("Could not find service for object path: %s",
               objectPath.c_str());
  }
...
ConnectWifiResult ConnectToWifiService(ConnManBusService* service) {
  ...
  conn_man_bus_service_call_connect(service, nullptr, ConnectCallback,
                                    (gpointer)&data);
...
bool DisconnectFromWifiService(ConnManBusService* service) {
  ...
  GError* error = nullptr;
  bool success =
      conn_man_bus_service_call_disconnect_sync(service, nullptr, &error);
...
WiFiState GetWiFiState() {
  ...
  GVariant* services = nullptr;
  bool success = conn_man_bus_manager_call_get_services_sync(
      manager_proxy, &services, nullptr, &error);
  g_object_unref(manager_proxy);
  if (error) {
    loge("Error getting services from connman");
...
bool IsAccessPointMode() {
  ...
  bool success = (bool)conn_man_bus_technology_call_get_properties_sync(
      tech_proxy, &properties, nullptr, &error);
...
bool EnableAccessPointMode(std::string ssid, std::string pw) {
  ...
  conn_man_bus_technology_call_set_property_sync(
      tech_proxy, "TetheringIdentifier", ssid_v, nullptr, &error);
  ...
  conn_man_bus_technology_call_set_property_sync(
      tech_proxy, "TetheringPassphrase", pw_v, nullptr, &error);
  ...
  conn_man_bus_technology_call_set_property_sync(tech_proxy, "Tethering",
                                                 enable_v, nullptr, &error);
...
void RecoverNetworkServices() {
  DASMSG(recover_network_services, "wifi.recover_network_services",
         "Attempt to recover network services");
  DASMSG_SEND();

  ExecCommandInBackground(
      {"sudo", "/bin/systemctl", "restart", "wpa_supplicant", "connman"},
      nullptr);
}
...
void WpaSupplicantScan() {
  ...
  FiW1Wpa_supplicant1* wpa_sup = fi_w1_wpa_supplicant1_proxy_new_sync(
      gdbusConn, G_DBUS_PROXY_FLAGS_NONE, "fi.w1.wpa_supplicant1",
      "/fi/w1/wpa_supplicant1", nullptr, nullptr);

  gchar* interface_path;
  gboolean success = fi_w1_wpa_supplicant1_call_get_interface_sync(
      wpa_sup, "wlan0", &interface_path, nullptr, nullptr);

  FiW1Wpa_supplicant1Outerface* interfacePath =
      fi_w1_wpa_supplicant1_outerface_proxy_new_sync(
          gdbusConn, G_DBUS_PROXY_FLAGS_NONE, "fi.w1.wpa_supplicant1",
          interface_path, nullptr, nullptr);
  ...
  gboolean scanSuccess = fi_w1_wpa_supplicant1_outerface_call_scan_sync(
      interfacePath, dict, nullptr, nullptr);

  Log::Write("Dbus-WpaSupplicant interface path [%d][%d][%s]", (int)scanSuccess,
             (int)success, interface_path);
}
...
WiFiIpFlags GetIpAddress(uint8_t* ipv4_32bits, uint8_t* ipv6_128bits) {
  ...
  while (current != nullptr) {
    ...
    if ((family == AF_INET || family == AF_INET6) &&
        (strcmp(current->ifa_name, interface) == 0)) {
      if (family == AF_INET) {
        // Handle IPv4
