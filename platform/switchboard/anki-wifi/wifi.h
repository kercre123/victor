...
ConnectWifiResult ConnectWiFiBySsid(std::string ssid, std::string pw,
                                    uint8_t auth, bool hidden,
                                    GAsyncReadyCallback cb, gpointer userData);
WifiScanErrorCode ScanForWiFiAccessPoints(std::vector<WiFiScanResult>& results);
WifiScanErrorCode GetWiFiServices(std::vector<WiFiScanResult>& results,
                                  bool scan);
void ScanForWiFiAccessPointsAsync();
std::vector<uint8_t> PackWiFiScanResults(
    const std::vector<WiFiScanResult>& results);
