...
extern "C" void core_common_on_exit(void) { lcd_shutdown(); }
...
void DrawFaultCode(uint16_t fault, bool willRestart) {
  ...
  img.DrawTextCenteredHorizontally(
      faultString, kNormalFont, 1.5f, 2, NamedColors::WHITE,
  ...
  img.DrawTextCenteredHorizontally(text, kNormalFont, scale, kTextThickness,
                                   NamedColors::WHITE,
                                   FACE_DISPLAY_HEIGHT - size.y(), kDrawTwice);
...
bool DrawStartPairingScreen(const std::string& robotName) {
  ...
  img.DrawTextCenteredHorizontally(robotName, kNormalFont, kRobotNameScale,
                                   kTextThickness, kWhiteColor,
                                   kRobotNameVerticalPosition, kDrawTwice);

  cv::Size textSize;
  float scale = 0;
  Vision::Image::MakeTextFillImageWidth(kAppURL, kNormalFont, kTextThickness,
                                        img.GetNumCols(), textSize, scale);
  img.DrawTextCenteredHorizontally(
      kAppURL, kNormalFont, scale, kTextThickness, kWhiteColor,
      (FACE_DISPLAY_HEIGHT + textSize.height) / 2, true);
...
void DrawShowPinScreen(const std::string& robotName, const std::string& pin) {
  ...
  Vision::ImageRGB key = Vision::ImageRGB(Vision::Image(
      FACE_DISPLAY_HEIGHT, FACE_DISPLAY_WIDTH, pairing_icon_key_gray));
  ...
  img.DrawTextCenteredHorizontally(pin, kNormalFont, 0.8f, kTextThickness,
                                   kWhiteColor, FACE_DISPLAY_HEIGHT - 5,
                                   kDrawTwice);
