...
class ExternalCommsCladHandlerV4 {
 public:
  using RtsConnectionSignal = Signal::Signal<void(
  ...
  RtsConnectionSignal& OnReceiveRtsAck() { return _receiveRtsAck; }
  ...
  // RtsSsh
  RtsConnectionSignal& OnReceiveRtsSsh() { return _DEV_ReceiveSshKey; }
  ...
  Anki::Vector::ExternalComms::ExternalComms ReceiveExternalCommsMsg(
    ...
    if (unpackSize != length) {
      // bugs
      Log::Write(
          "externalCommsCladHandler - Somehow our bytes didn't unpack to the "
          "proper size.");
    }

    if (extComms.GetTag() ==
        Anki::Vector::ExternalComms::ExternalCommsTag::RtsConnection) {
      Anki::Vector::ExternalComms::RtsConnection rstContainer =
          extComms.Get_RtsConnection();
      Anki::Vector::ExternalComms::RtsConnection_4 rtsMsg =
          rstContainer.Get_RtsConnection_4();
  ...
  static std::vector<uint8_t> SendExternalCommsMsg(
    ...
    if (packedSize != msg.Size()) {
      // bugs
      Log::Write(
          "externalCommsCladHandler - Somehow our bytes didn't pack to the "
          "proper size.");
    }
