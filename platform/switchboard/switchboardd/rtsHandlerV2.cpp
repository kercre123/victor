...
RtsHandlerV2::RtsHandlerV2(
  ...
  _onReceivePlainTextHandle =
      _stream->OnReceivedPlainTextEvent().ScopedSubscribe(
          std::bind(&RtsHandlerV2::HandleMessageReceived, this,
  ...
  _internetTimerSignal.SubscribeForever(
      std::bind(&RtsHandlerV2::HandleInternetTimerTick, this));
  ...
  ev_timer_init(&_handleInternet.timer, &RtsHandlerV2::sEvTimerHandler,
...
void RtsHandlerV2::StopPairing() { Reset(true); }

void RtsHandlerV2::ForceDisconnect() { SendRtsMessage<RtsForceDisconnect>(); }
...
void RtsHandlerV2::HandleRtsConnResponse(
    const Anki::Vector::ExternalComms::RtsConnection_2& msg) {
  ...
  if (_state == RtsPairingPhase::AwaitingPublicKey) {
    ...
    if (connResponse.connectionType ==
      ...
      if (_hasOwner && cloudAuth) {
        //
        // Because RTSv3 requires cloud authorization if there is already an
        // owner, and because RTSv2 does not support that functionality, we must
        // require all initial pairs to be done with RTSv3+ if the robot already
        // has a cloud account owner.
        //
        Log::Write(
            "Client tried to initial pair with V2 of protocol. Disconnecting.");
        Reset(true);
...
void RtsHandlerV2::HandleRtsChallengeMessage(
    const Vector::ExternalComms::RtsConnection_2& msg) {
...
void RtsHandlerV2::HandleRtsWifiConnectRequest(
    const Vector::ExternalComms::RtsConnection_2& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV2::HandleRtsWifiIpRequest(
    const Vector::ExternalComms::RtsConnection_2& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV2::HandleRtsStatusRequest(
    const Vector::ExternalComms::RtsConnection_2& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV2::HandleRtsWifiScanRequest(
    const Vector::ExternalComms::RtsConnection_2& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV2::HandleRtsOtaUpdateRequest(
    const Vector::ExternalComms::RtsConnection_2& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV2::HandleRtsOtaCancelRequest(
    const Vector::ExternalComms::RtsConnection_2& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV2::HandleRtsWifiAccessPointRequest(
    const Vector::ExternalComms::RtsConnection_2& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV2::HandleRtsForceDisconnect(
    const Vector::ExternalComms::RtsConnection_2& msg) {
  if (!(HasState(RtsCommsType::Encrypted) ||
...
void RtsHandlerV2::HandleRtsLogRequest(
    const Vector::ExternalComms::RtsConnection_2& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV2::HandleRtsCancelPairing(
    const Vector::ExternalComms::RtsConnection_2& msg) {
  Log::Write("Stopping pairing due to client request.");
  StopPairing();
}

void RtsHandlerV2::HandleRtsAck(
    const Vector::ExternalComms::RtsConnection_2& msg) {
  ...
  if (_state == RtsPairingPhase::AwaitingNonceAck &&
      ack.rtsConnectionTag == (uint8_t)Anki::Vector::ExternalComms::
                                  RtsConnection_2Tag::RtsNonceMessage) {
    HandleNonceAck();
  } else {
    // ignore msg
    IncrementAbnormalityCount();
...
void RtsHandlerV2::HandleInitialPair(uint8_t* publicKey,
  ...
  memcpy(&client.sessionRx, _keyExchange->GetDecryptKey(),
         sizeof(client.sessionRx));
  memcpy(&client.sessionTx, _keyExchange->GetEncryptKey(),
         sizeof(client.sessionTx));
...
void RtsHandlerV2::HandleChallengeResponse(uint8_t* pingChallengeAnswer,
                                           uint32_t length) {
  bool success = false;
...
void RtsHandlerV2::SendStatusResponse() {
  ...
  SendRtsMessage<RtsStatusResponse_2>(state.ssid, state.connState, isApMode,
...
void RtsHandlerV2::SendWifiAccessPointResponse(bool success, std::string ssid,
                                               std::string pw) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV2::SendWifiScanResult() {
  ...
  for (int i = 0; i < wifiResults.size(); i++) {
    Anki::Vector::ExternalComms::RtsWifiScanResult_2 result =
        Anki::Vector::ExternalComms::RtsWifiScanResult_2(
            wifiResults[i].auth, wifiResults[i].signal_level,
            wifiResults[i].ssid, wifiResults[i].hidden);
...
void RtsHandlerV2::SendOtaProgress(int status, uint64_t progress,
...
void RtsHandlerV2::sEvTimerHandler(struct ev_loop* loop, struct ev_timer* w,
                                   int revents) {
  std::ostringstream ss;
