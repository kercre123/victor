...
void Daemon::Start() {
  ...
  ev_timer_init(&_ankibtdTimer, HandleAnkibtdTimer, kRetryInterval_s,
                kRetryInterval_s);

  // Initialize Ota Timer
  _handleOtaTimer.signal = &_otaUpdateTimerSignal;
  _otaUpdateTimerSignal.SubscribeForever(
      std::bind(&Daemon::HandleOtaUpdateProgress, this));
  ev_timer_init(&_handleOtaTimer.timer, &Daemon::sEvTimerHandler,
                kOtaUpdateInterval_s, kOtaUpdateInterval_s);

  // Initialize Pairing Timer
  _pairingTimer.signal = &_pairingPreConnectionSignal;
  _pairingPreConnectionSignal.SubscribeForever(
      std::bind(&Daemon::HandlePairingTimeout, this));
  ev_timer_init(&_pairingTimer.timer, &Daemon::sEvTimerHandler,
                kPairingPreConnectionTimeout_s, 0);

  // Initialize wifi listeners
  Anki::Wifi::Initialize(_taskExecutor);
  _wifiWatcher = std::make_shared<WifiWatcher>(_loop);
  _wifiChangedHandle = Anki::Wifi::GetWifiChangedSignal().ScopedSubscribe(
      std::bind(&Daemon::OnWifiChanged, this, std::placeholders::_1,
                std::placeholders::_2));
...
void Daemon::Stop() {
  ...
  if (_engineMessagingClient != nullptr) {
    ...
    _engineMessagingClient->ShowPairingStatus(
...
void Daemon::LogWifiState() {
  ...
  bool connected =
      (wifiState.connState == Anki::Wifi::WiFiConnState::CONNECTED) ||
...
void Daemon::InitializeEngineComms() {
  ...
  _engineMessagingClient->OnReceivePairingStatus().SubscribeForever(
      std::bind(&Daemon::OnPairingStatus, this, std::placeholders::_1));
  _engineTimer.data = this;
  ev_timer_init(&_engineTimer, HandleEngineTimer, kRetryInterval_s,
                kRetryInterval_s);
  ev_timer_start(_loop, &_engineTimer);
}

void Daemon::InitializeGatewayComms() {
  _gatewayMessagingServer = std::make_shared<GatewayMessagingServer>(
      _loop, _taskExecutor, _tokenClient, _connectionIdManager);
...
void Daemon::InitializeCloudComms() {
  ...
  ev_timer_init(&_tokenTimer, HandleTokenTimer, kRetryInterval_s,
                kRetryInterval_s);
...
bool Daemon::TryConnectToEngineServer() {
  ...
  if (connected) {
    ...
    if (++_connectionFailureCounter >= kFailureCountToLog) {
      Log::Write(
          "Failed to Initialize EngineMessagingClient ... trying again.");
...
bool Daemon::TryConnectToTokenServer() {
  ...
  if (connected) {
    ...
    (void)_tokenClient->SendJwtRequest(
        [this](Anki::Vector::TokenError error, std::string jwt) {
...
void Daemon::InitializeBleComms() {
  ...
  if (_bleClient.get() == nullptr) {
    ...
    _bleOnConnectedHandle = _bleClient->OnConnectedEvent().ScopedSubscribe(
        std::bind(&Daemon::OnConnected, this, std::placeholders::_1,
                  std::placeholders::_2));
    _bleOnDisconnectedHandle =
        _bleClient->OnDisconnectedEvent().ScopedSubscribe(
            std::bind(&Daemon::OnDisconnected, this, std::placeholders::_1,
                      std::placeholders::_2));
    _bleOnIpcPeerDisconnectedHandle =
        _bleClient->OnIpcDisconnection().ScopedSubscribe(
            std::bind(&Daemon::OnBleIpcDisconnected, this));
...
void Daemon::UpdateAdvertisement(bool pairing) {
  ...
  if (AutoTest::IsAutoTestBot()) {
    if (!pairing) {
      Log::Write(
          "automation: UpdatingAdvertisement - overriding pairing state. "
...
void Daemon::OnConnected(int connId, INetworkStream* stream) {
  ...
  _taskExecutor->Wake([connId, stream, this]() {
    ...
    if (_securePairing == nullptr) {
      _securePairing = std::make_unique<Anki::Switchboard::RtsComms>(
          stream, _loop, _engineMessagingClient, _gatewayMessagingServer,
          _tokenClient, _connectionIdManager, _wifiWatcher, _taskExecutor,
          _isPairing, _isOtaUpdating, _hasCloudOwner);
      _pinHandle = _securePairing->OnUpdatedPinEvent().ScopedSubscribe(
          std::bind(&Daemon::OnPinUpdated, this, std::placeholders::_1));
      _otaHandle = _securePairing->OnOtaUpdateRequestEvent().ScopedSubscribe(
          std::bind(&Daemon::OnOtaUpdatedRequest, this, std::placeholders::_1));
      _endHandle = _securePairing->OnStopPairingEvent().ScopedSubscribe(
          std::bind(&Daemon::OnEndPairing, this));
      _completedPairingHandle =
          _securePairing->OnCompletedPairingEvent().ScopedSubscribe(
              std::bind(&Daemon::OnCompletedPairing, this));
    }
...
void Daemon::OnEndPairing() {
  ...
  if (_bleClient != nullptr) {
    Log::Write("Pairing ended: Disconnecting from BLE Central [%d]",
               _connectionId);
    _bleClient->Disconnect(_connectionId);
  }
...
void Daemon::OnOtaUpdatedRequest(std::string url) {
  ...
  if (access(kUpdateEngineServicePath.c_str(), F_OK) == -1) {
    ExecCommandInBackground(
        {kUpdateEngineExecPath, url},
...
void Daemon::StartPairing() {
  ...
  Log::Write(
      "[PT] Starting pairing timer... pairing will timeout in %d seconds.",
      kPairingPreConnectionTimeout_s);
}

void Daemon::OnPairingStatus(
  ...
  switch (tag) {
    ...
    case Anki::Vector::ExternalInterface::MessageEngineToGameTag::
      ...
      _engineMessagingClient->HandleWifiConnectRequest(
          std::string((char*)&payload.ssid), std::string((char*)&payload.pwd),
...
void Daemon::HandleEngineTimer(struct ev_loop* loop, struct ev_timer* w,
                               int revents) {
...
void Daemon::HandleAnkibtdTimer(struct ev_loop* loop, struct ev_timer* w,
                                int revents) {
  Daemon* daemon = (Daemon*)w->data;
...
void Daemon::HandleTokenTimer(struct ev_loop* loop, struct ev_timer* w,
                              int revents) {
  Daemon* daemon = (Daemon*)w->data;
...
void Daemon::sEvTimerHandler(struct ev_loop* loop, struct ev_timer* w,
                             int revents) {
...
static void SignalCallback(struct ev_loop* loop, struct ev_signal* w,
                           int revents) {
