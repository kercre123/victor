...
GatewayMessagingServer::GatewayMessagingServer(
    struct ev_loop* evloop, std::shared_ptr<TaskExecutor> taskExecutor,
...
GatewayMessagingServer::~GatewayMessagingServer() { (void)Disconnect(); }
...
bool GatewayMessagingServer::Init() {
  ...
  ev_timer_init(&_handleGatewayMessageTimer.timer,
                ...
                kGatewayMessageFrequency_s, kGatewayMessageFrequency_s);
...
std::shared_ptr<SafeHandle> GatewayMessagingServer::SendConnectionIdRequest(
  ...
  _connectionIdRequestHandlesQueue.push(
      std::weak_ptr<SafeHandle>(sharedHandle));
...
std::shared_ptr<SafeHandle>
GatewayMessagingServer::SendClientGuidRefreshRequest(
    ClientGuidRefreshRequestCallback callback) {
  std::shared_ptr<SafeHandle> sharedHandle = std::make_shared<SafeHandle>();

  _refreshClientGuidRequestCallbackQueue.push(callback);
  _refreshClientGuidRequestHandlesQueue.push(
      std::weak_ptr<SafeHandle>(sharedHandle));
...
std::shared_ptr<SafeHandle> GatewayMessagingServer::SendSdkProxyRequest(
    std::string clientGuid, std::string id, std::string path, std::string json,
    SdkProxyRequestCallback callback) {
  std::shared_ptr<SafeHandle> sharedHandle = std::make_shared<SafeHandle>();

  _sdkProxyRequestCallbackQueue.insert({id, callback});
  _sdkProxyRequestHandlesQueue.insert(
      {id, std::weak_ptr<SafeHandle>(sharedHandle)});
...
void GatewayMessagingServer::HandleAuthRequest(
    const SwitchboardRequest& message) {
  ...
  tokenClient->SendJwtRequest([this, sessionToken, clientName, appId,
                               tokenClient](Anki::Vector::TokenError error,
                                            std::string jwtToken) {
    ...
    switch (error) {
      ...
      case Anki::Vector::TokenError::NoError: {
        ...
        isPrimary = false;
        tokenClient->SendSecondaryAuthRequest(
            sessionToken, clientName, appId,
            [this, isPrimary](Anki::Vector::TokenError authError,
                              std::string appToken, std::string authJwtToken) {
      ...
      case Anki::Vector::TokenError::InvalidToken: {
        ...
        isPrimary = false;
        tokenClient->SendReassociateAuthRequest(
            sessionToken, clientName, appId,
            [this, isPrimary](Anki::Vector::TokenError authError,
                              std::string appToken, std::string authJwtToken) {
              Log::Write("CloudRequest Auth Response Handler");
      ...
      default: {
        ...
        SendMessage(
            SwitchboardResponse(Anki::Vector::AuthResponse("", "", error)));
      } break;
    }
  });
}

void GatewayMessagingServer::HandleConnectionIdRequest(
    const SwitchboardRequest& message) {
  _taskExecutor->Wake([this, message]() {
    ...
    SwitchboardResponse res = SwitchboardResponse(
        ExternalConnectionResponse(isConnected, connectionId));
    SendMessage(res);
  });
}

void GatewayMessagingServer::HandleConnectionIdResponse(
    const SwitchboardRequest& message) {
  _taskExecutor->Wake([this, message]() {
    // Receive connection id and status from Gateway
    ExternalConnectionResponse response =
        message.Get_ExternalConnectionResponse();
...
void GatewayMessagingServer::HandleClientGuidRefreshResponse(
    const SwitchboardRequest& message) {
  _taskExecutor->Wake([this, message]() {
    // Receive client guid refresh from Gateway
    ClientGuidRefreshRequestCallback cb =
        _refreshClientGuidRequestCallbackQueue.front();
    _refreshClientGuidRequestCallbackQueue.pop();
    std::weak_ptr<SafeHandle> handle =
        _refreshClientGuidRequestHandlesQueue.front();
...
void GatewayMessagingServer::HandleSdkProxyResponse(
    const SwitchboardRequest& message) {
  _taskExecutor->Wake([this, message]() {
    ...
    if (callbackIter != _sdkProxyRequestCallbackQueue.end() &&
      ...
      if (!handleIter->second.expired()) {
        callbackIter->second(response.messageId, response.statusCode,
      ...
      Log::Write(
          "GatewayMessageServer received Sdk Proxy Response from gateway with "
          "unknown id.");
    }
  });
}

void GatewayMessagingServer::ProcessCloudAuthResponse(
    bool isPrimary, Anki::Vector::TokenError authError, std::string appToken,
    std::string authJwtToken) {
  // Send SwitchboardResponse
  SwitchboardResponse res =
      SwitchboardResponse(Anki::Vector::AuthResponse(appToken, "", authError));
  SendMessage(res);
}

void GatewayMessagingServer::sEvGatewayMessageHandler(struct ev_loop* loop,
                                                      struct ev_timer* w,
                                                      int revents) {
  struct ev_GatewayMessageTimerStruct* wData =
      (struct ev_GatewayMessageTimerStruct*)w;

  GatewayMessagingServer* self = wData->messagingServer;
  int recvSize;

  while ((recvSize =
              wData->server->Recv((char*)sMessageData, sizeof(sMessageData))) >
    ...
    if (msgSize > (sizeof(sMessageData) - sizeof(msgSize))) {
      // if the size is greater than the buffer we support, than don't try to
      // read
      Log::Write(
          "GatewayMessagingServer received message from vic-gateway that "
          "didn't fit into our buffer.");
      continue;
    }
