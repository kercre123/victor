 ...
 * Date:   02/22/2019
 *
 * Description: Interface between the switchboard components and a controlling
 *entity used to receive command requests and pairing status updates, as well as
 *display information on the robot's face.
 *
...
class ISwitchboardCommandClient {
 public:
  ...
  virtual void SendMessage(
      const Anki::Vector::ExternalInterface::MessageGameToEngine& message) = 0;
  virtual void SetPairingPin(std::string pin) = 0;
  virtual void SendBLEConnectionStatus(bool connected) = 0;
  virtual void ShowPairingStatus(
      Anki::Vector::SwitchboardInterface::ConnectionStatus status) = 0;
