...
RtsHandlerV3::RtsHandlerV3(
  ...
  _onReceivePlainTextHandle =
      _stream->OnReceivedPlainTextEvent().ScopedSubscribe(
          std::bind(&RtsHandlerV3::HandleMessageReceived, this,
  ...
  _internetTimerSignal.SubscribeForever(
      std::bind(&RtsHandlerV3::HandleInternetTimerTick, this));
  ...
  ev_timer_init(&_handleInternet.timer, &RtsHandlerV3::sEvTimerHandler,
...
void RtsHandlerV3::StopPairing() { Reset(true); }

void RtsHandlerV3::ForceDisconnect() { SendRtsMessage<RtsForceDisconnect>(); }
...
void RtsHandlerV3::HandleRtsConnResponse(
...
void RtsHandlerV3::HandleRtsChallengeMessage(
...
void RtsHandlerV3::HandleRtsWifiConnectRequest(
    const Vector::ExternalComms::RtsConnection_3& msg) {
  if (!IsAuthenticated()) {
    return;
  }
...
void RtsHandlerV3::HandleRtsWifiIpRequest(
    const Vector::ExternalComms::RtsConnection_3& msg) {
  if (!IsAuthenticated()) {
    return;
  }
...
void RtsHandlerV3::HandleRtsStatusRequest(
    const Vector::ExternalComms::RtsConnection_3& msg) {
  if (!IsAuthenticated()) {
    return;
  }
...
void RtsHandlerV3::HandleRtsWifiScanRequest(
    const Vector::ExternalComms::RtsConnection_3& msg) {
  if (!IsAuthenticated()) {
    return;
  }
...
void RtsHandlerV3::HandleRtsWifiForgetRequest(
    const Vector::ExternalComms::RtsConnection_3& msg) {
  if (!IsAuthenticated()) {
    return;
  }
...
void RtsHandlerV3::HandleRtsOtaUpdateRequest(
    const Vector::ExternalComms::RtsConnection_3& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV3::HandleRtsOtaCancelRequest(
    const Vector::ExternalComms::RtsConnection_3& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV3::HandleRtsWifiAccessPointRequest(
    const Vector::ExternalComms::RtsConnection_3& msg) {
  if (!IsAuthenticated()) {
    return;
  }
...
void RtsHandlerV3::ProcessCloudAuthResponse(bool isPrimary,
...
void RtsHandlerV3::HandleRtsCloudSessionRequest(
    const Vector::ExternalComms::RtsConnection_3& msg) {
      ...
      [this, sessionToken](Anki::Vector::TokenError error,
        ...
        switch (error) {
          ...
          case Anki::Vector::TokenError::InvalidToken: {
            ...
            bool success = false;
            SendRtsMessage<RtsCloudSessionResponse>(
                success, RtsCloudStatus::InvalidSessionToken, "");
          } break;
          case Anki::Vector::TokenError::Connection:
          default: {
...
void RtsHandlerV3::HandleRtsForceDisconnect(
    const Vector::ExternalComms::RtsConnection_3& msg) {
  if (!(HasState(RtsCommsType::Encrypted) ||
...
void RtsHandlerV3::HandleRtsLogRequest(
    const Vector::ExternalComms::RtsConnection_3& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV3::HandleRtsCancelPairing(
    const Vector::ExternalComms::RtsConnection_3& msg) {
  Log::Write("Stopping pairing due to client request.");
  StopPairing();
}

void RtsHandlerV3::HandleRtsAck(
    const Vector::ExternalComms::RtsConnection_3& msg) {
  ...
  if (_state == RtsPairingPhase::AwaitingNonceAck &&
      ack.rtsConnectionTag == (uint8_t)Anki::Vector::ExternalComms::
                                  RtsConnection_3Tag::RtsNonceMessage) {
    HandleNonceAck();
  } else {
    // ignore msg
    IncrementAbnormalityCount();
...
void RtsHandlerV3::HandleInitialPair(uint8_t* publicKey,
...
void RtsHandlerV3::HandleChallengeResponse(uint8_t* pingChallengeAnswer,
                                           uint32_t length) {
  bool success = false;
...
void RtsHandlerV3::SendStatusResponse() {
  ...
  SendRtsMessage<RtsStatusResponse_3>(state.ssid, state.connState, isApMode,
                                      bleState, batteryState, buildNoString,
                                      _isOtaUpdating, _hasOwner);

  Log::Write("Send status response.");
}

void RtsHandlerV3::SendWifiAccessPointResponse(bool success, std::string ssid,
                                               std::string pw) {
  if (!IsAuthenticated()) {
    return;
  }
...
void RtsHandlerV3::SendOtaProgress(int status, uint64_t progress,
...
void RtsHandlerV3::sEvTimerHandler(struct ev_loop* loop, struct ev_timer* w,
                                   int revents) {
  std::ostringstream ss;
