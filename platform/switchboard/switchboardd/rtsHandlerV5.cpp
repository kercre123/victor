...
RtsHandlerV5::RtsHandlerV5(
    INetworkStream* stream, struct ev_loop* evloop,
  ...
  _onReceivePlainTextHandle =
      _stream->OnReceivedPlainTextEvent().ScopedSubscribe(
          std::bind(&RtsHandlerV5::HandleMessageReceived, this,
                    std::placeholders::_1, std::placeholders::_2));

  _onReceiveEncryptedHandle =
      _stream->OnReceivedEncryptedEvent().ScopedSubscribe(
          std::bind(&RtsHandlerV5::HandleMessageReceived, this,
                    std::placeholders::_1, std::placeholders::_2));

  _onFailedDecryptionHandle =
      _stream->OnFailedDecryptionEvent().ScopedSubscribe(
          std::bind(&RtsHandlerV5::HandleDecryptionFailed, this));

  // Register with private events
  _internetTimerSignal.SubscribeForever(
      std::bind(&RtsHandlerV5::HandleInternetTimerTick, this));
  ...
  ev_timer_init(&_handleInternet.timer, &RtsHandlerV5::sEvTimerHandler,
                kWifiConnectInterval_s, kWifiConnectInterval_s);
...
void RtsHandlerV5::StopPairing() { Reset(true); }

void RtsHandlerV5::ForceDisconnect() { SendRtsMessage<RtsForceDisconnect>(); }

void RtsHandlerV5::SubscribeToCladMessages() {
  _rtsConnResponseHandle =
      _cladHandler->OnReceiveRtsConnResponse().ScopedSubscribe(std::bind(
          &RtsHandlerV5::HandleRtsConnResponse, this, std::placeholders::_1));
  _rtsChallengeMessageHandle =
      _cladHandler->OnReceiveRtsChallengeMessage().ScopedSubscribe(
          std::bind(&RtsHandlerV5::HandleRtsChallengeMessage, this,
                    std::placeholders::_1));
  _rtsWifiConnectRequestHandle =
      _cladHandler->OnReceiveRtsWifiConnectRequest().ScopedSubscribe(
          std::bind(&RtsHandlerV5::HandleRtsWifiConnectRequest, this,
                    std::placeholders::_1));
  _rtsWifiIpRequestHandle =
      _cladHandler->OnReceiveRtsWifiIpRequest().ScopedSubscribe(std::bind(
          &RtsHandlerV5::HandleRtsWifiIpRequest, this, std::placeholders::_1));
  _rtsRtsStatusRequestHandle =
      _cladHandler->OnReceiveRtsStatusRequest().ScopedSubscribe(std::bind(
          &RtsHandlerV5::HandleRtsStatusRequest, this, std::placeholders::_1));
  _rtsWifiScanRequestHandle =
      _cladHandler->OnReceiveRtsWifiScanRequest().ScopedSubscribe(
          std::bind(&RtsHandlerV5::HandleRtsWifiScanRequest, this,
                    std::placeholders::_1));
  _rtsWifiForgetRequestHandle =
      _cladHandler->OnReceiveRtsWifiForgetRequest().ScopedSubscribe(
          std::bind(&RtsHandlerV5::HandleRtsWifiForgetRequest, this,
                    std::placeholders::_1));
  _rtsOtaUpdateRequestHandle =
      _cladHandler->OnReceiveRtsOtaUpdateRequest().ScopedSubscribe(
          std::bind(&RtsHandlerV5::HandleRtsOtaUpdateRequest, this,
                    std::placeholders::_1));
  _rtsOtaCancelRequestHandle =
      _cladHandler->OnReceiveRtsOtaCancelRequest().ScopedSubscribe(
          std::bind(&RtsHandlerV5::HandleRtsOtaCancelRequest, this,
                    std::placeholders::_1));
  _rtsWifiAccessPointRequestHandle =
      _cladHandler->OnReceiveRtsWifiAccessPointRequest().ScopedSubscribe(
          std::bind(&RtsHandlerV5::HandleRtsWifiAccessPointRequest, this,
                    std::placeholders::_1));
  _rtsCancelPairingHandle =
      _cladHandler->OnReceiveCancelPairingRequest().ScopedSubscribe(std::bind(
          &RtsHandlerV5::HandleRtsCancelPairing, this, std::placeholders::_1));
  _rtsLogRequestHandle =
      _cladHandler->OnReceiveRtsLogRequest().ScopedSubscribe(std::bind(
          &RtsHandlerV5::HandleRtsLogRequest, this, std::placeholders::_1));
  _rtsCloudSessionHandle =
      _cladHandler->OnReceiveRtsCloudSessionRequest().ScopedSubscribe(
          std::bind(&RtsHandlerV5::HandleRtsCloudSessionRequest, this,
                    std::placeholders::_1));
  _rtsAppConnectionIdHandle =
      _cladHandler->OnReceiveRtsAppConnectionIdRequest().ScopedSubscribe(
          std::bind(&RtsHandlerV5::HandleRtsAppConnectionIdRequest, this,
                    std::placeholders::_1));
  _rtsForceDisconnectHandle =
      _cladHandler->OnReceiveRtsForceDisconnect().ScopedSubscribe(
          std::bind(&RtsHandlerV5::HandleRtsForceDisconnect, this,
                    std::placeholders::_1));
  _rtsAckHandle = _cladHandler->OnReceiveRtsAck().ScopedSubscribe(
      std::bind(&RtsHandlerV5::HandleRtsAck, this, std::placeholders::_1));
  _rtsSdkProxyHandle = _cladHandler->OnReceiveRtsSdkRequest().ScopedSubscribe(
      std::bind(&RtsHandlerV5::HandleRtsSdkProxyRequest, this,
                std::placeholders::_1));
}
...
// Must be cloud authed for first time pair.
  ...
  if (_isFirstTimePair && _hasOwner) {
    ...
    if (!_hasCloudAuthed) {
      SendRtsMessage<RtsResponse>(RtsResponseCode::NotCloudAuthorized,
                                  "Not cloud authorized.");
    }
  ...
  _rtsKeys.clients.erase(
      std::remove_if(_rtsKeys.clients.begin(), _rtsKeys.clients.end(),
                     [this](RtsClientData c) {
                       Log::Write(
                           "Deleting previously saved keys for same client.");
                       return memcmp(&c.publicKey, &_clientSession.publicKey,
                                     sizeof(_clientSession.publicKey)) == 0;
                     }),
...
void RtsHandlerV5::HandleRtsConnResponse(
    const Anki::Vector::ExternalComms::RtsConnection_5& msg) {
  if (!HasState(RtsCommsType::Unencrypted)) {
    return;
  }

  if (_state == RtsPairingPhase::AwaitingPublicKey) {
    Anki::Vector::ExternalComms::RtsConnResponse connResponse =
        msg.Get_RtsConnResponse();

    if (connResponse.connectionType ==
        Anki::Vector::ExternalComms::RtsConnType::FirstTimePair) {
      if (_isPairing && !_isOtaUpdating) {
        HandleInitialPair((uint8_t*)connResponse.publicKey.data(),
      ...
      for (int i = 0; i < _rtsKeys.clients.size(); i++) {
        if (memcmp((uint8_t*)connResponse.publicKey.data(),
                   (uint8_t*)&(_rtsKeys.clients[i].publicKey),
                   crypto_kx_PUBLICKEYBYTES) == 0) {
          hasClient = true;
          _stream->SetCryptoKeys(_rtsKeys.clients[i].sessionTx,
...
void RtsHandlerV5::HandleRtsChallengeMessage(
    const Vector::ExternalComms::RtsConnection_5& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }

  if (_state == RtsPairingPhase::AwaitingChallengeResponse) {
    Anki::Vector::ExternalComms::RtsChallengeMessage challengeMessage =
        msg.Get_RtsChallengeMessage();

    HandleChallengeResponse((uint8_t*)&challengeMessage.number,
                            sizeof(challengeMessage.number));
  } else {
    // ignore msg
    IncrementAbnormalityCount();
...
void RtsHandlerV5::HandleRtsWifiConnectRequest(
    const Vector::ExternalComms::RtsConnection_5& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
  ...
  if (_state == RtsPairingPhase::ConfirmedSharedSecret) {
    Anki::Vector::ExternalComms::RtsWifiConnectRequest wifiConnectMessage =
        msg.Get_RtsWifiConnectRequest();

    Log::Write("Trying to connect to wifi network.");

    _wifiConnectTimeout_s =
        std::max(kWifiConnectMinTimeout_s, wifiConnectMessage.timeout);

    UpdateFace(
        Anki::Vector::SwitchboardInterface::ConnectionStatus::SETTING_WIFI);
    ...
    Wifi::ConnectWifiResult connected = Wifi::ConnectWiFiBySsid(
        wifiConnectMessage.wifiSsidHex, wifiConnectMessage.password,
        wifiConnectMessage.authType, (bool)wifiConnectMessage.hidden, nullptr,
        nullptr);
...
void RtsHandlerV5::HandleRtsWifiIpRequest(
    const Vector::ExternalComms::RtsConnection_5& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV5::HandleRtsStatusRequest(
    const Vector::ExternalComms::RtsConnection_5& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV5::HandleRtsWifiScanRequest(
    const Vector::ExternalComms::RtsConnection_5& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV5::HandleRtsWifiForgetRequest(
    const Vector::ExternalComms::RtsConnection_5& msg) {
  if (!IsAuthenticated()) {
    return;
  }
  ...
  if (_state == RtsPairingPhase::ConfirmedSharedSecret) {
    // Get message
    Anki::Vector::ExternalComms::RtsWifiForgetRequest forgetMsg =
        msg.Get_RtsWifiForgetRequest();
...
void RtsHandlerV5::HandleRtsOtaUpdateRequest(
    const Vector::ExternalComms::RtsConnection_5& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
  ...
  if (_state == RtsPairingPhase::ConfirmedSharedSecret && !_isOtaUpdating) {
    Anki::Vector::ExternalComms::RtsOtaUpdateRequest otaMessage =
        msg.Get_RtsOtaUpdateRequest();
...
void RtsHandlerV5::HandleRtsOtaCancelRequest(
    const Vector::ExternalComms::RtsConnection_5& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
  ...
  if (_state == RtsPairingPhase::ConfirmedSharedSecret && _isOtaUpdating) {
    (void)ExecCommand(
        {"sudo", "/bin/systemctl", "stop", "update-engine.service"});
...
void RtsHandlerV5::HandleRtsWifiAccessPointRequest(
    const Vector::ExternalComms::RtsConnection_5& msg) {
  if (!IsAuthenticated()) {
    return;
  }
  ...
  if (_state == RtsPairingPhase::ConfirmedSharedSecret) {
    Anki::Vector::ExternalComms::RtsWifiAccessPointRequest accessPointMessage =
        msg.Get_RtsWifiAccessPointRequest();
...
void RtsHandlerV5::ProcessCloudAuthResponse(bool isPrimary,
                                            Anki::Vector::TokenError authError,
                                            std::string appToken,
                                            std::string authJwtToken) {
  ...
  switch (authError) {
    case Anki::Vector::TokenError::NoError:
      ...
      status = isPrimary ? RtsCloudStatus::AuthorizedAsPrimary
                         : RtsCloudStatus::AuthorizedAsSecondary;
      ...
      if (_engineClient != nullptr) {
        ...
        const auto msg = Anki::Vector::ExternalInterface::MessageGameToEngine::
            CreateUserLoggedIn({});
  ...
  if (_gatewayServer != nullptr) {
    std::shared_ptr<SafeHandle> handle =
        _gatewayServer->SendClientGuidRefreshRequest([this, authError, status,
                                                      appToken](bool success) {
          // Send HandleRtsResponse
          SendRtsMessage<RtsCloudSessionResponse>(
              authError == Anki::Vector::TokenError::NoError, status, appToken);
        });
...
void RtsHandlerV5::HandleRtsCloudSessionRequest(
    const Vector::ExternalComms::RtsConnection_5& msg) {
  ...
  if (_tokenClient == nullptr) {
    SendRtsMessage<RtsResponse>(RtsResponseCode::UnsupportedRequest,
  ...
  if ((wifiState.connState != Anki::Wifi::WiFiConnState::CONNECTED) &&
    ...
    bool success = false;
    SendRtsMessage<RtsCloudSessionResponse>(
        success, RtsCloudStatus::ConnectionError, "");
    return;
  }

  std::weak_ptr<TokenResponseHandle> tokenHandle = _tokenClient->SendJwtRequest(
      [this, sessionToken, clientName, appId](Anki::Vector::TokenError error,
                                              std::string jwtToken) {
        ...
        switch (error) {
          case Anki::Vector::TokenError::NullToken: {
            ...
            std::weak_ptr<TokenResponseHandle> authHandle =
                _tokenClient->SendAuthRequest(
                    sessionToken, clientName, appId,
                    [this, isPrimary](Anki::Vector::TokenError authError,
                                      std::string appToken,
                                      std::string authJwtToken) {
                      ProcessCloudAuthResponse(isPrimary, authError, appToken,
                                               authJwtToken);
                    });
          ...
          case Anki::Vector::TokenError::NoError: {
            ...
            isPrimary = false;
            std::weak_ptr<TokenResponseHandle> authHandle =
                _tokenClient->SendSecondaryAuthRequest(
                    sessionToken, clientName, appId,
                    [this, isPrimary](Anki::Vector::TokenError authError,
                                      std::string appToken,
                                      std::string authJwtToken) {
          ...
          case Anki::Vector::TokenError::InvalidToken: {
            ...
            Log::Error(
                "Received invalid token for JwtRequest, trying to reassociate");
            isPrimary = false;
            std::weak_ptr<TokenResponseHandle> authHandle =
                _tokenClient->SendReassociateAuthRequest(
                    sessionToken, clientName, appId,
                    [this, isPrimary](Anki::Vector::TokenError authError,
                                      std::string appToken,
                                      std::string authJwtToken) {
...
void RtsHandlerV5::HandleRtsAppConnectionIdRequest(
    const Vector::ExternalComms::RtsConnection_5& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV5::HandleRtsSdkProxyRequest(
    const Vector::ExternalComms::RtsConnection_5& msg) {
  if (!IsAuthenticated()) {
    return;
  }
  ...
  if (_gatewayServer != nullptr) {
    ...
    std::string internalMessageId =
        _keyExchange->GeneratePin(kSdkRequestIdSize);
    _sdkRequestIds.insert({internalMessageId, sdkMsg.messageId});

    // send sdk proxy request to gateway
    std::shared_ptr<SafeHandle> handle = _gatewayServer->SendSdkProxyRequest(
        sdkMsg.clientGuid, internalMessageId, sdkMsg.urlPath, sdkMsg.json,
        [this](std::string id, uint16_t statusCode, std::string contentType,
               std::string content) {
          ...
          if (idIter != _sdkRequestIds.end()) {
            ...
            SendRtsMessage<RtsSdkProxyResponse>(idIter->second, statusCode,
                                                contentType, content);
            _sdkRequestIds.erase(idIter);
          } else {
            // id is not in dictionary
            DASMSG(
                sdk_proxy_response_unknown_id,
                "rtshandler.sdkproxyresponse.unknownid",
                "Received unknown message id for SdkProxyRequest in response.");
            DASMSG_SET(s1, id, "Unknown message id");
            DASMSG_SEND();

            DEV_ASSERT(false,
                       "RtsHandler received sdk proxy response from gateway "
                       "messaging server with unknown id");
          }
        });
...
void RtsHandlerV5::HandleRtsForceDisconnect(
    const Vector::ExternalComms::RtsConnection_5& msg) {
  if (!(HasState(RtsCommsType::Encrypted) ||
...
void RtsHandlerV5::HandleRtsLogRequest(
    const Vector::ExternalComms::RtsConnection_5& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV5::HandleRtsCancelPairing(
    const Vector::ExternalComms::RtsConnection_5& msg) {
  Log::Write("Stopping pairing due to client request.");
  StopPairing();
}

void RtsHandlerV5::HandleRtsAck(
    const Vector::ExternalComms::RtsConnection_5& msg) {
  Anki::Vector::ExternalComms::RtsAck ack = msg.Get_RtsAck();
  if (_state == RtsPairingPhase::AwaitingNonceAck &&
      ack.rtsConnectionTag == (uint8_t)Anki::Vector::ExternalComms::
                                  RtsConnection_5Tag::RtsNonceMessage) {
    HandleNonceAck();
  } else {
    // ignore msg
    IncrementAbnormalityCount();
...
void RtsHandlerV5::HandleInitialPair(uint8_t* publicKey,
  ...
  _stream->SetCryptoKeys(_keyExchange->GetEncryptKey(),
                         _keyExchange->GetDecryptKey());

  // Save keys to file
  // For now only save one client

  memcpy(&_clientSession.publicKey, publicKey,
         sizeof(_clientSession.publicKey));
  memcpy(&_clientSession.sessionRx, _keyExchange->GetDecryptKey(),
         sizeof(_clientSession.sessionRx));
  memcpy(&_clientSession.sessionTx, _keyExchange->GetEncryptKey(),
         sizeof(_clientSession.sessionTx));
...
void RtsHandlerV5::HandleChallengeResponse(uint8_t* pingChallengeAnswer,
                                           uint32_t length) {
  bool success = false;
...
void RtsHandlerV5::SendNonce() {
  ...
  std::array<uint8_t, crypto_aead_xchacha20poly1305_ietf_NPUBBYTES>
      toRobotNonceArray;
  memcpy(std::begin(toRobotNonceArray), toRobotNonce,
         crypto_aead_xchacha20poly1305_ietf_NPUBBYTES);

  std::array<uint8_t, crypto_aead_xchacha20poly1305_ietf_NPUBBYTES>
      toDeviceNonceArray;
  memcpy(std::begin(toDeviceNonceArray), toDeviceNonce,
         crypto_aead_xchacha20poly1305_ietf_NPUBBYTES);
...
void RtsHandlerV5::SendStatusResponse() {
  ...
  Wifi::WiFiState state = Wifi::GetWiFiState();
  uint8_t bleState =
      1;  // for now, if we are sending this message, we are connected
  uint8_t batteryState =
      0;  // for now, ignore this field until we have a way to get that info
  ...
  SendRtsMessage<RtsStatusResponse_5>(
      state.ssid, state.connState, isApMode, bleState, batteryState,
      buildNoString, esnString, _isOtaUpdating, _hasOwner,
      _hasCloudAuthed || (!_isFirstTimePair && _hasOwner));

  Log::Write("Send status response.");
}

void RtsHandlerV5::SendWifiAccessPointResponse(bool success, std::string ssid,
                                               std::string pw) {
  if (!IsAuthenticated()) {
    return;
  }
...
void RtsHandlerV5::SendWifiScanResult() {
  ...
  for (int i = 0; i < wifiResults.size(); i++) {
    ...
    Anki::Vector::ExternalComms::RtsWifiScanResult_3 result =
        Anki::Vector::ExternalComms::RtsWifiScanResult_3(
            wifiResults[i].auth, wifiResults[i].signal_level,
            wifiResults[i].ssid, wifiResults[i].hidden, provisioned);
...
void RtsHandlerV5::SendWifiConnectResult(Wifi::ConnectWifiResult result) {
  ...
  SendRtsMessage<RtsWifiConnectResponse_3>(wifiState.ssid, wifiState.connState,
                                           (uint8_t)result);
}
...
void RtsHandlerV5::SendFile(uint32_t fileId, std::vector<uint8_t> fileBytes) {
  ...
  uint8_t status = 0;  // not sure if we need status byte, but reserving so I
                       // don't regret ~PRA
  ...
  while ((ssize_t)remaining > 0) {
    ...
    SendRtsMessage<RtsFileDownload>(status, fileId, bytesWritten + msgSize,
                                    fileSizeBytes, dataChunk);
...
void RtsHandlerV5::SendOtaProgress(int status, uint64_t progress,
...
void RtsHandlerV5::UpdateFace(
    Anki::Vector::SwitchboardInterface::ConnectionStatus state) {
  if (_engineClient == nullptr) {
  ...
  if ((state ==
       Anki::Vector::SwitchboardInterface::ConnectionStatus::UPDATING_OS) ||
      (state ==
       Anki::Vector::SwitchboardInterface::ConnectionStatus::SETTING_WIFI)) {
    return;
  }
...
void RtsHandlerV5::sEvTimerHandler(struct ev_loop* loop, struct ev_timer* w,
                                   int revents) {
  std::ostringstream ss;
