...
RtsHandlerV4::RtsHandlerV4(
  ...
  _onReceivePlainTextHandle =
      _stream->OnReceivedPlainTextEvent().ScopedSubscribe(
          std::bind(&RtsHandlerV4::HandleMessageReceived, this,
  ...
  _internetTimerSignal.SubscribeForever(
      std::bind(&RtsHandlerV4::HandleInternetTimerTick, this));
  ...
  ev_timer_init(&_handleInternet.timer, &RtsHandlerV4::sEvTimerHandler,
...
void RtsHandlerV4::StopPairing() { Reset(true); }

void RtsHandlerV4::ForceDisconnect() { SendRtsMessage<RtsForceDisconnect>(); }
...
void RtsHandlerV4::HandleRtsConnResponse(
    const Anki::Vector::ExternalComms::RtsConnection_4& msg) {
...
void RtsHandlerV4::HandleRtsChallengeMessage(
...
void RtsHandlerV4::HandleRtsWifiConnectRequest(
    const Vector::ExternalComms::RtsConnection_4& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV4::HandleRtsWifiIpRequest(
    const Vector::ExternalComms::RtsConnection_4& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV4::HandleRtsStatusRequest(
    const Vector::ExternalComms::RtsConnection_4& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV4::HandleRtsWifiScanRequest(
    const Vector::ExternalComms::RtsConnection_4& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV4::HandleRtsWifiForgetRequest(
    const Vector::ExternalComms::RtsConnection_4& msg) {
  if (!IsAuthenticated()) {
    return;
  }
...
void RtsHandlerV4::HandleRtsOtaUpdateRequest(
    const Vector::ExternalComms::RtsConnection_4& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV4::HandleRtsOtaCancelRequest(
    const Vector::ExternalComms::RtsConnection_4& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV4::HandleRtsWifiAccessPointRequest(
    const Vector::ExternalComms::RtsConnection_4& msg) {
  if (!IsAuthenticated()) {
    return;
  }
...
void RtsHandlerV4::ProcessCloudAuthResponse(bool isPrimary,
...
void RtsHandlerV4::HandleRtsCloudSessionRequest(
    const Vector::ExternalComms::RtsConnection_4& msg) {
  ...
  std::weak_ptr<TokenResponseHandle> tokenHandle = _tokenClient->SendJwtRequest(
      [this, sessionToken](Anki::Vector::TokenError error,
                           std::string jwtToken) {
...
void RtsHandlerV4::HandleRtsAppConnectionIdRequest(
    const Vector::ExternalComms::RtsConnection_4& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV4::HandleRtsForceDisconnect(
    const Vector::ExternalComms::RtsConnection_4& msg) {
  if (!(HasState(RtsCommsType::Encrypted) ||
...
void RtsHandlerV4::HandleRtsLogRequest(
    const Vector::ExternalComms::RtsConnection_4& msg) {
  if (!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV4::HandleRtsCancelPairing(
    const Vector::ExternalComms::RtsConnection_4& msg) {
  Log::Write("Stopping pairing due to client request.");
  StopPairing();
}

void RtsHandlerV4::HandleRtsAck(
    const Vector::ExternalComms::RtsConnection_4& msg) {
  ...
  if (_state == RtsPairingPhase::AwaitingNonceAck &&
      ack.rtsConnectionTag == (uint8_t)Anki::Vector::ExternalComms::
                                  RtsConnection_4Tag::RtsNonceMessage) {
    HandleNonceAck();
  } else {
    // ignore msg
    IncrementAbnormalityCount();
...
void RtsHandlerV4::HandleInitialPair(uint8_t* publicKey,
...
void RtsHandlerV4::HandleChallengeResponse(uint8_t* pingChallengeAnswer,
                                           uint32_t length) {
  bool success = false;
...
void RtsHandlerV4::SendStatusResponse() {
  ...
  SendRtsMessage<RtsStatusResponse_4>(state.ssid, state.connState, isApMode,
                                      bleState, batteryState, buildNoString,
                                      esnString, _isOtaUpdating, _hasOwner);

  Log::Write("Send status response.");
}

void RtsHandlerV4::SendWifiAccessPointResponse(bool success, std::string ssid,
                                               std::string pw) {
  if (!IsAuthenticated()) {
    return;
  }
...
void RtsHandlerV4::SendOtaProgress(int status, uint64_t progress,
...
void RtsHandlerV4::sEvTimerHandler(struct ev_loop* loop, struct ev_timer* w,
                                   int revents) {
  std::ostringstream ss;
