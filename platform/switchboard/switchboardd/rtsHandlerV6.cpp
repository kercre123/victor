/**
 * File: RtsHandlerV6.cpp
 *
 * Author: paluri
 * Created: 04/17/2019
 *
 * Description: V6 of BLE Protocol
 *
...
#include "switchboardd/rtsHandlerV6.h"
...
long long RtsHandlerV6::sTimeStarted;

RtsHandlerV6::RtsHandlerV6(INetworkStream* stream, 
...
_wifiConnectTimeout_s(15)
{
  ...
  _onReceivePlainTextHandle = _stream->OnReceivedPlainTextEvent().ScopedSubscribe(
    std::bind(&RtsHandlerV6::HandleMessageReceived,
  ...
  _onFailedDecryptionHandle = _stream->OnFailedDecryptionEvent().ScopedSubscribe(
    std::bind(&RtsHandlerV6::HandleDecryptionFailed, this));

  // Register with private events
  _internetTimerSignal.SubscribeForever(std::bind(&RtsHandlerV6::HandleInternetTimerTick, this));
  ...
  ev_timer_init(&_handleInternet.timer, &RtsHandlerV6::sEvTimerHandler, kWifiConnectInterval_s, kWifiConnectInterval_s);
  
  Log::Write("RtsComms V6 starting up.");
}

RtsHandlerV6::~RtsHandlerV6() {
...
void RtsHandlerV6::Reset(bool forced) {
...
void RtsHandlerV6::StopPairing() {
  Reset(true);
}

void RtsHandlerV6::ForceDisconnect() {
  SendRtsMessage<RtsForceDisconnect>();
}

void RtsHandlerV6::SubscribeToCladMessages() {
  _rtsConnResponseHandle = _cladHandler->OnReceiveRtsConnResponse().ScopedSubscribe(std::bind(&RtsHandlerV6::HandleRtsConnResponse, this, std::placeholders::_1));
  _rtsChallengeMessageHandle = _cladHandler->OnReceiveRtsChallengeMessage().ScopedSubscribe(std::bind(&RtsHandlerV6::HandleRtsChallengeMessage, this, std::placeholders::_1));
  _rtsWifiConnectRequestHandle = _cladHandler->OnReceiveRtsWifiConnectRequest().ScopedSubscribe(std::bind(&RtsHandlerV6::HandleRtsWifiConnectRequest, this, std::placeholders::_1));
  _rtsWifiIpRequestHandle = _cladHandler->OnReceiveRtsWifiIpRequest().ScopedSubscribe(std::bind(&RtsHandlerV6::HandleRtsWifiIpRequest, this, std::placeholders::_1));
  _rtsRtsStatusRequestHandle = _cladHandler->OnReceiveRtsStatusRequest().ScopedSubscribe(std::bind(&RtsHandlerV6::HandleRtsStatusRequest, this, std::placeholders::_1));
  _rtsWifiScanRequestHandle = _cladHandler->OnReceiveRtsWifiScanRequest().ScopedSubscribe(std::bind(&RtsHandlerV6::HandleRtsWifiScanRequest, this, std::placeholders::_1));
  _rtsWifiForgetRequestHandle = _cladHandler->OnReceiveRtsWifiForgetRequest().ScopedSubscribe(std::bind(&RtsHandlerV6::HandleRtsWifiForgetRequest, this, std::placeholders::_1));
  _rtsOtaUpdateRequestHandle = _cladHandler->OnReceiveRtsOtaUpdateRequest().ScopedSubscribe(std::bind(&RtsHandlerV6::HandleRtsOtaUpdateRequest, this, std::placeholders::_1));
  _rtsOtaCancelRequestHandle = _cladHandler->OnReceiveRtsOtaCancelRequest().ScopedSubscribe(std::bind(&RtsHandlerV6::HandleRtsOtaCancelRequest, this, std::placeholders::_1));
  _rtsWifiAccessPointRequestHandle = _cladHandler->OnReceiveRtsWifiAccessPointRequest().ScopedSubscribe(std::bind(&RtsHandlerV6::HandleRtsWifiAccessPointRequest, this, std::placeholders::_1));
  _rtsCancelPairingHandle = _cladHandler->OnReceiveCancelPairingRequest().ScopedSubscribe(std::bind(&RtsHandlerV6::HandleRtsCancelPairing, this, std::placeholders::_1));
  _rtsLogRequestHandle = _cladHandler->OnReceiveRtsLogRequest().ScopedSubscribe(std::bind(&RtsHandlerV6::HandleRtsLogRequest, this, std::placeholders::_1));
  _rtsCloudSessionHandle = _cladHandler->OnReceiveRtsCloudSessionRequest().ScopedSubscribe(std::bind(&RtsHandlerV6::HandleRtsCloudSessionRequest, this, std::placeholders::_1));
  _rtsAppConnectionIdHandle = _cladHandler->OnReceiveRtsAppConnectionIdRequest().ScopedSubscribe(std::bind(&RtsHandlerV6::HandleRtsAppConnectionIdRequest, this, std::placeholders::_1));
  _rtsForceDisconnectHandle = _cladHandler->OnReceiveRtsForceDisconnect().ScopedSubscribe(std::bind(&RtsHandlerV6::HandleRtsForceDisconnect, this, std::placeholders::_1)); 
...
void RtsHandlerV6::SaveSessionKeys() {
...
void RtsHandlerV6::HandleRtsConnResponse(const Anki::Vector::ExternalComms::RtsConnection_6& msg) {
...
void RtsHandlerV6::HandleRtsChallengeMessage(const Vector::ExternalComms::RtsConnection_6& msg) {
  if(!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV6::HandleRtsWifiConnectRequest(const Vector::ExternalComms::RtsConnection_6& msg) {
  if(!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV6::HandleRtsWifiIpRequest(const Vector::ExternalComms::RtsConnection_6& msg) {
  if(!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV6::HandleRtsStatusRequest(const Vector::ExternalComms::RtsConnection_6& msg) {
  if(!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV6::HandleRtsWifiScanRequest(const Vector::ExternalComms::RtsConnection_6& msg) {
  if(!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV6::HandleRtsWifiForgetRequest(const Vector::ExternalComms::RtsConnection_6& msg) {
  if(!IsAuthenticated()) {
    return;
  }
...
void RtsHandlerV6::HandleRtsOtaUpdateRequest(const Vector::ExternalComms::RtsConnection_6& msg) {
  if(!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV6::HandleRtsOtaCancelRequest(const Vector::ExternalComms::RtsConnection_6& msg) {
  if(!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV6::HandleRtsWifiAccessPointRequest(const Vector::ExternalComms::RtsConnection_6& msg) {
  if(!IsAuthenticated()) {
    return;
  }
...
void RtsHandlerV6::ProcessCloudAuthResponse(bool isPrimary, Anki::Vector::TokenError authError, std::string appToken, std::string authJwtToken) {
...
void RtsHandlerV6::HandleRtsCloudSessionRequest(const Vector::ExternalComms::RtsConnection_6& msg) {
...
void RtsHandlerV6::HandleRtsAppConnectionIdRequest(const Vector::ExternalComms::RtsConnection_6& msg) {
  if(!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV6::HandleRtsSdkProxyRequest(const Vector::ExternalComms::RtsConnection_6& msg) {
  if(!IsAuthenticated()) {
    return;
  }
...
void RtsHandlerV6::HandleRtsLogRequest(const Vector::ExternalComms::RtsConnection_6& msg) {
  if(!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV6::HandleRtsCancelPairing(const Vector::ExternalComms::RtsConnection_6& msg) {
  Log::Write("Stopping pairing due to client request.");
  StopPairing();
}

void RtsHandlerV6::HandleRtsAck(const Vector::ExternalComms::RtsConnection_6& msg) {
  Anki::Vector::ExternalComms::RtsAck ack = msg.Get_RtsAck();
  if(_state == RtsPairingPhase::AwaitingNonceAck &&
    ack.rtsConnectionTag == (uint8_t)Anki::Vector::ExternalComms::RtsConnection_6Tag::RtsNonceMessage) {
    HandleNonceAck();
  } else {
    // ignore msg
    IncrementAbnormalityCount();
...
void RtsHandlerV6::HandleInitialPair(uint8_t* publicKey, uint32_t publicKeyLength) {
...
void RtsHandlerV6::HandleDecryptionFailed() {
  Log::Write("Decryption failed...");
  Reset();
}

void RtsHandlerV6::HandleNonceAck() {
...
void RtsHandlerV6::HandleChallengeResponse(uint8_t* pingChallengeAnswer, uint32_t length) {
  bool success = false;
...
void RtsHandlerV6::SendPublicKey() {
  if(!HasState(RtsCommsType::Unencrypted)) {
    return;
  }
  ...
  std::array<uint8_t, 2> bluetoothSigId;
  
  std::copy((uint8_t*)Anki::kAnkiBluetoothSIGCompanyIdentifier.data(),
            (uint8_t*)(Anki::kAnkiBluetoothSIGCompanyIdentifier.data() + Anki::kAnkiBluetoothSIGCompanyIdentifier.size()),
            bluetoothSigId.begin());

  SendRtsMessage<RtsConnRequest_6>(publicKeyArray, _isPairing, bluetoothSigId, Anki::kVictorProductIdentifier);

  // Save public key to file
  Log::Write("Sending public key to client.");
}

void RtsHandlerV6::SendNonce() {
  if(!HasState(RtsCommsType::Unencrypted)) {
    return;
  }
...
void RtsHandlerV6::SendChallenge() {
  if(!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV6::SendChallengeSuccess() {
  if(!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV6::SendStatusResponse() {
  if(!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV6::SendWifiAccessPointResponse(bool success, std::string ssid, std::string pw) {
  if(!IsAuthenticated()) {
    return;
  }
...
void RtsHandlerV6::SendWifiScanResult() {
  if(!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV6::SendWifiConnectResult(Wifi::ConnectWifiResult result) {
  if(!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV6::SendFile(uint32_t fileId, std::vector<uint8_t> fileBytes) {
  if(!HasState(RtsCommsType::Encrypted)) {
    return;
  }
...
void RtsHandlerV6::SendCancelPairing() {
  // Send challenge and update state
  SendRtsMessage<RtsCancelPairing>();
  Log::Write("Canceling pairing.");
}

void RtsHandlerV6::SendOtaProgress(int status, uint64_t progress, uint64_t expectedTotal) {
  if(!HasState(RtsCommsType::Encrypted)) {
    return;
  }
  // Send Ota Progress
...
void RtsHandlerV6::HandleMessageReceived(uint8_t* bytes, uint32_t length) {
...
void RtsHandlerV6::HandleTimeout() {
  if(_state != RtsPairingPhase::ConfirmedSharedSecret) {
...
void RtsHandlerV6::IncrementChallengeCount() {
...
void RtsHandlerV6::IncrementAbnormalityCount() {
...
void RtsHandlerV6::HandleInternetTimerTick() {
  _inetTimerCount++;
...
void RtsHandlerV6::UpdateFace(Anki::Vector::SwitchboardInterface::ConnectionStatus state) {
  if(_engineClient == nullptr) {
...
void RtsHandlerV6::sEvTimerHandler(struct ev_loop* loop, struct ev_timer* w, int revents)
{
  std::ostringstream ss;
