...
class KeyExchange {
 public:
  KeyExchange(uint8_t numPinDigits) : _numPinDigits(numPinDigits) {}

  // Getters
  uint8_t* GetEncryptKey() { return _encryptKey; }

  uint8_t* GetDecryptKey() { return _decryptKey; }

  uint8_t* GetPublicKey() { return _publicKey; }

  uint8_t* GetPrivateKey() { return _secretKey; }
  ...
  uint8_t* GetPinLengthPtr() { return &_numPinDigits; }

  uint8_t* GetToRobotNonce() { return _initialToRobotNonce; }

  uint8_t* GetToDeviceNonce() { return _initialToDeviceNonce; }

  uint8_t* GetVerificationHash() {
    crypto_generichash(_hashedKey, crypto_kx_SESSIONKEYBYTES, _encryptKey,
                       crypto_kx_SESSIONKEYBYTES, nullptr, 0);
