...
RtsComms::RtsComms(INetworkStream* stream, struct ev_loop* evloop,
  ...
  // pairing timeout
  _onPairingTimeoutReceived = _pairingTimeoutSignal.ScopedSubscribe(
      std::bind(&RtsComms::HandleTimeout, this));
  _handleTimeoutTimer.signal = &_pairingTimeoutSignal;
  ev_timer_init(&_handleTimeoutTimer.timer, &RtsComms::sEvTimerHandler,
                kPairingTimeout_s, kPairingTimeout_s);
}
...
void RtsComms::Init() {
  ...
  _onReceivePlainTextHandle =
      _stream->OnReceivedPlainTextEvent().ScopedSubscribe(
          std::bind(&RtsComms::HandleMessageReceived, this,
...
void RtsComms::SendOtaProgress(int status, uint64_t progress,
...
void RtsComms::HandleReset(bool forced) {
  //
  // 'Wake' is necessary to prevent RtsComms from being destroyed in its own
  // scope the SafeHandle weak_ptr allows us to know if RtsComms was destroyed
  ...
  _taskExecutor->Wake([this, weakSafeHandle, forced]() {
    ...
    if (forced) {
      ...
      Log::Write(
          "SecurePairing ending due to multiple failures. Requires external "
          "restart.");
      ev_timer_stop(_loop, &_handleTimeoutTimer.timer);
...
void RtsComms::HandleMessageReceived(uint8_t* bytes, uint32_t length) {
  _taskExecutor->WakeSync([this, bytes, length]() {
    ...
    if (_state == RtsPairingPhase::AwaitingHandshake) {
        ...
        if (handleHandshake) {
          ...
          switch (_rtsVersion) {
            case PairingProtocolVersion::CURRENT: {
              _rtsHandler = (IRtsHandler*)new RtsHandlerV5(
                  _stream, _loop, _engineClient, _tokenClient, _gatewayServer,
                  _connectionIdManager, _taskExecutor, _wifiWatcher, _isPairing,
                  _isOtaUpdating, _hasCloudOwner);

              RtsHandlerV5* _v5 = static_cast<RtsHandlerV5*>(_rtsHandler);
              _pinHandle = _v5->OnUpdatedPinEvent().ScopedSubscribe(
                  [this](std::string s) {
                    _pin = s;
                    this->OnUpdatedPinEvent().emit(s);
                  });
              _otaHandle = _v5->OnOtaUpdateRequestEvent().ScopedSubscribe(
                  [this](std::string s) {
                    this->OnOtaUpdateRequestEvent().emit(s);
                  });
              _endHandle = _v5->OnStopPairingEvent().ScopedSubscribe(
                  [this]() { this->OnStopPairingEvent().emit(); });
              _completedPairingHandle =
                  _v5->OnCompletedPairingEvent().ScopedSubscribe(
                      [this]() { this->OnCompletedPairingEvent().emit(); });
              _resetHandle = _v5->OnResetEvent().ScopedSubscribe(std::bind(
                  &RtsComms::HandleReset, this, std::placeholders::_1));

              break;
            }
            case PairingProtocolVersion::V4: {
              _rtsHandler = (IRtsHandler*)new RtsHandlerV4(
                  _stream, _loop, _engineClient, _tokenClient, _gatewayServer,
                  _connectionIdManager, _taskExecutor, _wifiWatcher, _isPairing,
                  _isOtaUpdating, _hasCloudOwner);

              RtsHandlerV4* _v4 = static_cast<RtsHandlerV4*>(_rtsHandler);
              _pinHandle = _v4->OnUpdatedPinEvent().ScopedSubscribe(
                  [this](std::string s) {
                    _pin = s;
                    this->OnUpdatedPinEvent().emit(s);
                  });
              _otaHandle = _v4->OnOtaUpdateRequestEvent().ScopedSubscribe(
                  [this](std::string s) {
                    this->OnOtaUpdateRequestEvent().emit(s);
                  });
              _endHandle = _v4->OnStopPairingEvent().ScopedSubscribe(
                  [this]() { this->OnStopPairingEvent().emit(); });
              _completedPairingHandle =
                  _v4->OnCompletedPairingEvent().ScopedSubscribe(
                      [this]() { this->OnCompletedPairingEvent().emit(); });
              _resetHandle = _v4->OnResetEvent().ScopedSubscribe(std::bind(
                  &RtsComms::HandleReset, this, std::placeholders::_1));
            ...
            case PairingProtocolVersion::FACTORY: {
              _rtsHandler = (IRtsHandler*)new RtsHandlerV2(
                  _stream, _loop, _engineClient, _tokenClient, _taskExecutor,
                  _wifiWatcher, _isPairing, _isOtaUpdating, _hasCloudOwner);

              RtsHandlerV2* _v2 = static_cast<RtsHandlerV2*>(_rtsHandler);
              _pinHandle = _v2->OnUpdatedPinEvent().ScopedSubscribe(
                  [this](std::string s) {
                    _pin = s;
                    this->OnUpdatedPinEvent().emit(s);
                  });
              _otaHandle = _v2->OnOtaUpdateRequestEvent().ScopedSubscribe(
                  [this](std::string s) {
                    this->OnOtaUpdateRequestEvent().emit(s);
                  });
              _endHandle = _v2->OnStopPairingEvent().ScopedSubscribe(
                  [this]() { this->OnStopPairingEvent().emit(); });
              _completedPairingHandle =
                  _v2->OnCompletedPairingEvent().ScopedSubscribe(
                      [this]() { this->OnCompletedPairingEvent().emit(); });
              _resetHandle = _v2->OnResetEvent().ScopedSubscribe(std::bind(
                  &RtsComms::HandleReset, this, std::placeholders::_1));
              break;
            }
            ...
            default: {
              ...
              Log::Write(
                  "Error: handleHandshake is true, but version is not "
                  "handled.");
              StopPairing();
              return;
            }
          ...
          Log::Write(
              "Unable to process handshake. Something very bad happened.");
          StopPairing();
        }
      } else {
        // ignore msg
      ...
      StopPairing();
      Log::Write(
          "Internal state machine error. Assuming raw message, but state is "
          "not initial [%d].",
          (int)_state);
    }
  });
...
bool RtsComms::HandleHandshake(uint16_t version) {
  ...
  if ((version == PairingProtocolVersion::CURRENT) ||
    ...
    Log::Write("Client reported incompatible version [%d]. Our version is [%d]",
               version, PairingProtocolVersion::CURRENT);
    return false;
  }
