...
TokenClient::TokenClient(struct ev_loop* evloop,
    ...
    : _loop(evloop), _taskExecutor(taskExecutor) {}

// TokenClient::~TokenClient() {}

bool TokenClient::Init() {
  ev_timer_init(&_handleTokenMessageTimer.timer,
                &TokenClient::sEvTokenMessageHandler, kMessageFrequency_s,
                kMessageFrequency_s);
  _handleTokenMessageTimer.client = &_client;
  _handleTokenMessageTimer.signal = &_tokenMessageSignal;

  _tokenResponseHandle = _tokenMessageSignal.ScopedSubscribe(std::bind(
      &TokenClient::HandleTokenResponse, this, std::placeholders::_1));
...
std::shared_ptr<TokenResponseHandle> TokenClient::SendAuthRequest(
    std::string sessionToken, std::string clientName, std::string appId,
    AuthRequestCallback callback) {
  std::shared_ptr<TokenResponseHandle> handle =
      std::make_shared<TokenResponseHandle>();

  _taskExecutor->Wake(
      [this, handle, callback, sessionToken, clientName, appId]() {
        // add callback to queue
        _authCallbacks.push(callback);
        _authHandles.push(handle);

        Anki::Vector::TokenRequest tokenRequest = Anki::Vector::TokenRequest(
            Anki::Vector::AuthRequest(sessionToken, clientName, appId));
...
std::shared_ptr<TokenResponseHandle> TokenClient::SendSecondaryAuthRequest(
    std::string sessionToken, std::string clientName, std::string appId,
    AuthRequestCallback callback) {
  std::shared_ptr<TokenResponseHandle> handle =
      std::make_shared<TokenResponseHandle>();

  _taskExecutor->Wake([this, handle, callback, sessionToken, clientName,
                       appId]() {
    // add callback to queue
    _authCallbacks.push(callback);
    _authHandles.push(handle);
    ...
    Anki::Vector::TokenRequest tokenRequest = Anki::Vector::TokenRequest(
        Anki::Vector::SecondaryAuthRequest(sessionToken, clientName, appId));
...
std::shared_ptr<TokenResponseHandle> TokenClient::SendReassociateAuthRequest(
    std::string sessionToken, std::string clientName, std::string appId,
    AuthRequestCallback callback) {
  std::shared_ptr<TokenResponseHandle> handle =
      std::make_shared<TokenResponseHandle>();
  ...
  _taskExecutor->Wake(
      [this, handle, callback, sessionToken, clientName, appId]() {
        ...
        Anki::Vector::TokenRequest tokenRequest = Anki::Vector::TokenRequest(
            Anki::Vector::ReassociateRequest(sessionToken, clientName, appId));
...
std::shared_ptr<TokenResponseHandle> TokenClient::SendJwtRequest(
...
void TokenClient::sEvTokenMessageHandler(struct ev_loop* loop,
                                         struct ev_timer* w, int revents) {
  struct ev_TokenMessageTimerStruct* wData =
      (struct ev_TokenMessageTimerStruct*)w;

  int recvSize;

  while ((recvSize = wData->client->Recv((char*)sMessageData,
                                         sizeof(sMessageData))) > 0) {
    ...
    if (unpackedSize != (size_t)recvSize) {
      Log::Error(
          "Received message from token server but had mismatch size when "
          "unpacked.");
      continue;
    }
