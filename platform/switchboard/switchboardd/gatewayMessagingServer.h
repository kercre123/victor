...
class GatewayMessagingServer {
  ...
  typedef std::function<void(std::string, uint16_t, std::string, std::string)>
      SdkProxyRequestCallback;

 public:
  using GatewayMessageSignal = Signal::Signal<void(SwitchboardRequest)>;
  explicit GatewayMessagingServer(
      struct ev_loop* loop, std::shared_ptr<TaskExecutor> taskExecutor,
  ...
  std::shared_ptr<SafeHandle> SendConnectionIdRequest(
      ConnectionIdRequestCallback callback);
  std::shared_ptr<SafeHandle> SendClientGuidRefreshRequest(
      ClientGuidRefreshRequestCallback callback);
  std::shared_ptr<SafeHandle> SendSdkProxyRequest(
      std::string clientGuid, std::string id, std::string path,
      std::string json, SdkProxyRequestCallback callback);
  ...
  static void sEvGatewayMessageHandler(struct ev_loop* loop, struct ev_timer* w,
                                       int revents);
 ...
 private:
  ...
  std::queue<ClientGuidRefreshRequestCallback>
      _refreshClientGuidRequestCallbackQueue;
  ...
  std::unordered_map<std::string, SdkProxyRequestCallback>
      _sdkProxyRequestCallbackQueue;
  std::unordered_map<std::string, std::weak_ptr<SafeHandle>>
      _sdkProxyRequestHandlesQueue;
