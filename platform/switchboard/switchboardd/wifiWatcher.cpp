...
WifiWatcher::WifiWatcher(struct ev_loop* loop) : _loop(loop), _enabled(true) {
  _timer.self = this;
  ev_timer_init(&_timer.timer, WatcherTick, 0, kWifiTick_s);
  ev_timer_start(_loop, &_timer.timer);
}

WifiWatcher::~WifiWatcher() { ev_timer_stop(_loop, &_timer.timer); }

void WifiWatcher::Enable() { _enabled = true; }

void WifiWatcher::Disable() { _enabled = false; }
...
void WifiWatcher::ConnectIfNoWifi() {
  ...
  if (Wifi::IsAccessPointMode()) {
    Log::Write(
        "WifiWatcher: access point mode enabled, doing nothing this tick.");
    return;
  }
  ...
  // Scan async ... this won't benefit us this tick, but it will populate the
  // list for next time!
  Anki::Wifi::ScanForWiFiAccessPointsAsync();

  std::vector<Anki::Wifi::WiFiScanResult> results;
  bool shouldScan = false;
  Anki::Wifi::WifiScanErrorCode code =
      Anki::Wifi::GetWiFiServices(results, shouldScan);
  ...
  for (const Anki::Wifi::WiFiScanResult& result : results) {
    if (result.provisioned) {
      Anki::Wifi::ConnectWifiResult connectResult =
          Anki::Wifi::ConnectWiFiBySsid(
              result.ssid,
              "",  // don't need pw since already provisioned
              (uint8_t)result.auth, result.hidden, nullptr, nullptr);
      ...
      if (connectResult == Anki::Wifi::ConnectWifiResult::CONNECT_SUCCESS) {
        ...
        Log::Write(
            "WifiWatcher: Switchboard autoconnected to wifi successfully.");
      } else {
...
void WifiWatcher::WatcherTick(struct ev_loop* loop, struct ev_timer* w,
                              int revents) {
