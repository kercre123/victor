...
using TaggedImageBuffer =
    TaggedItem<std::shared_ptr<Anki::Vision::ImageBuffer>>;
using TaggedImage = TaggedItem<std::shared_ptr<Anki::Vision::Image>>;
using TrackedFacesAndImagePair =
    std::pair<std::shared_ptr<std::list<Anki::Vision::TrackedFace>>,
...
public:
  ...
  CameraNode(const std::string &node_name)
      : Node(node_name),
        camera_service_(Anki::Vector::CameraService::getInstance()) {}

  bool Initialize() {
    OutputPort<TaggedImageBuffer> *output =
        Node::CreateOutputPort<TaggedImageBuffer>(OUTPUT_CAMERA);
    ...
    camera_task_ = [output, this]() {
      ...
      bool got_frame = camera_service_->CameraGetFrame(
          camera_service_->GetTimeStamp(), *shared_buffer);
      if (!got_frame) {
        Threadpool::Inst().EnqueueTask(camera_task_);
        return;
      }
      TaggedImageBuffer tagged_image_buffer(shared_buffer);
      CreateAndLogEvent(tagged_image_buffer, {GetUUID()},
                        DataEvent::Type::CREATION);
  ...
  void Start() { Threadpool::Inst().EnqueueTask(camera_task_); }
...
public:
  ...
  bool Initialize() {
    InputPort<TaggedImageBuffer> *input =
        Node::CreateInputPort<TaggedImageBuffer>(1, INPUT_BUFFER);
    OutputPort<TaggedImage> *output =
        Node::CreateOutputPort<TaggedImage>(OUTPUT_IMAGE);

    InputPort<TaggedImageBuffer>::EventCallback cb =
        [input, output, this](InputPort<TaggedImageBuffer> *) {
          ...
          Task get_image_task = [image_buffer, output, this]() {
            ...
            bool convert_to_gray = image_buffer.item->GetGray(
                *image.item, Anki::Vision::ImageCacheSize::Half);
            if (!convert_to_gray) {
...
public:
  ...
  static constexpr const char *OUTPUT_TRACKED_FACES_AND_IMAGE =
      "tracked_faces_and_image";
  FaceTrackerNode(const std::string &node_name) : Node(node_name) {
    ...
    face_tracker_.reset(
        new Anki::Vision::FaceTracker(camera_, data_path, jsonData));
    face_tracker_->EnableRecognition(false);
  }

  bool Initialize() {
    InputPort<TaggedImage> *input =
        Node::CreateInputPort<TaggedImage>(1, INPUT_IMAGE);
    OutputPort<TaggedTrackedFacesAndImage> *output =
        Node::CreateOutputPort<TaggedTrackedFacesAndImage>(
            OUTPUT_TRACKED_FACES_AND_IMAGE);

    InputPort<TaggedImage>::EventCallback cb = [input, output, this](
                                                   InputPort<TaggedImage> *) {
      Task get_faces_task = [input, output, this]() {
        ...
        faces_and_image.item.first =
            std::make_shared<std::list<Anki::Vision::TrackedFace>>();
        faces_and_image.item.second = image.item;
        std::list<Anki::Vision::UpdatedFaceID> updated_ids_list;
        Anki::Vision::DebugImageList<Anki::Vision::CompressedImage>
            debug_images;
        Anki::Result result = face_tracker_->Update(
            *faces_and_image.item.second, 1.f, *faces_and_image.item.first,
...
public:
  static constexpr const char *INPUT_TRACKED_FACES_AND_IMAGE =
      "tracked_faces_and_image";
  FacePublishNode(const std::string &node_name) : Node(node_name) {
    pub_handle_.reset(new ARF::PubHandle<std::string>(
        node_handle_.RegisterPublisher<std::string>("rgb")));
  }

  void Initialize() {
    InputPort<TaggedTrackedFacesAndImage> *input =
        Node::CreateInputPort<TaggedTrackedFacesAndImage>(
            5, INPUT_TRACKED_FACES_AND_IMAGE);

    InputPort<TaggedTrackedFacesAndImage>::EventCallback cb =
        [input, this](InputPort<TaggedTrackedFacesAndImage> *) {
          ...
          Task publish_faces_task = [tagged_tracked_faces_and_image, this]() {
            ...
            ARF::WallTime wall_time =
                ARF::float_to_time<ARF::WallTime>(image.GetTimestamp());
            ARF::MonotonicTime mono_time =
                ARF::float_to_time<ARF::MonotonicTime>(image.GetTimestamp());
            ARF::ConvertWallTimeToProto(
                wall_time,
                arf_message.mutable_header()->mutable_callback_time());
            ...
            arf_proto::Image *arf_image =
                tracked_faces_and_image->mutable_image();
            arf_image->set_rows(image.GetNumRows());
            arf_image->set_cols(image.GetNumCols());
            arf_image->set_encoding("gray");
            arf_image->set_data(image.GetDataPointer(),
                                image.GetNumRows() * image.GetNumCols());

            for (const auto &face : face_list) {
              arf_proto::TrackedFace *face_proto =
                  tracked_faces_and_image->add_face();
...
int main(int argc, char **argv) {
  ...
  ARF::connect_ports(
      camera_node.RetrieveOutputPort<TaggedImageBuffer>(
          CameraNode::OUTPUT_CAMERA),
      image_node.RetrieveInputPort<TaggedImageBuffer>(ImageNode::INPUT_BUFFER));
  ARF::connect_ports(
      image_node.RetrieveOutputPort<TaggedImage>(ImageNode::OUTPUT_IMAGE),
      face_tracker_node.RetrieveInputPort<TaggedImage>(
          FaceTrackerNode::INPUT_IMAGE));
  ARF::connect_ports(
      face_tracker_node.RetrieveOutputPort<TaggedTrackedFacesAndImage>(
          FaceTrackerNode::OUTPUT_TRACKED_FACES_AND_IMAGE),
      face_publish_node.RetrieveInputPort<TaggedTrackedFacesAndImage>(
          FacePublishNode::INPUT_TRACKED_FACES_AND_IMAGE));

  Threadpool::Inst().Initialize(
      5); // Num cores plus one extra potentially blocked in face tracker.
