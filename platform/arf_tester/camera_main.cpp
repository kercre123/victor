...
int main(int argc, char **argv) {
  ...
  ARF::NodeHandle n;
  ARF::PubHandle<std::string> pub_handle =
      n.RegisterPublisher<std::string>("rgb");

  Anki::Vector::CameraService *camera_service =
      Anki::Vector::CameraService::getInstance();
  ...
  const std::string data_path =
      "_build/mac/Debug/data/assets/cozmo_resources/config/engine/vision/";
  const std::string jsonFilename =
      "_build/mac/Debug/data/assets/cozmo_resources/config/engine/"
      "vision_config.json";
#else
  const std::string data_path =
  ...
  while (g_run) {
    ...
    bool got_frame = camera_service->CameraGetFrame(
        camera_service->GetTimeStamp(), image_buffer);
    if (!got_frame) {
    ...
    bool convert_to_gray =
        image_buffer.GetGray(image, Anki::Vision::ImageCacheSize::Half);
    if (!convert_to_gray) {
    ...
    Anki::Result result = face_tracker.Update(image, 1.f, face_list,
    ...
    bool convert_to_rgb =
        image_buffer.GetRGB(image_rgb, Anki::Vision::ImageCacheSize::Half);
    if (!convert_to_rgb) {
    ...
    ARF::WallTime wall_time =
        ARF::float_to_time<ARF::WallTime>(image_rgb.GetTimestamp());
    ARF::MonotonicTime mono_time =
        ARF::float_to_time<ARF::MonotonicTime>(image_rgb.GetTimestamp());
    ...
    arf_proto::TrackedFacesAndImage *tracked_faces_and_image =
        arf_message.mutable_tracked_faces_and_image();
    ...
    arf_image->set_data(image_rgb.GetDataPointer(),
                        3 * image_rgb.GetNumRows() * image_rgb.GetNumCols());
