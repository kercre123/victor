...
Result RobotLogUploader::Connect() {
  ...
  if (!ok) {
    LOG_ERROR("RobotLogUploader.Connect", "Unable to connect from %s to %s",
              _clientPath.c_str(), _serverPath.c_str());
...
Result RobotLogUploader::Send(const LogCollectorRequest &request) {
  ...
  if (sent <= 0) {
    LOG_ERROR("RobotLogUploader.Send",
              "Failed to send log collector request (%zd/%zu)", sent, size);
    return RESULT_FAIL;
  }
  return RESULT_OK;
}
...
Result RobotLogUploader::Receive(LogCollectorResponse &response) {
  ...
  if (received <= 0) {
    LOG_ERROR("RobotLogUploader.Receive",
              "Failed to receive log collector response (%zd/%zu)", received,
              sizeof(buffer));
    return RESULT_FAIL;
  }
...
Result RobotLogUploader::Upload(const std::string &path, std::string &url) {
  ...
  if (result != RESULT_OK) {
    LOG_ERROR("RobotLogUploader.Upload",
              "Unable to connect to log collector service");
    return result;
  }
  ...
  if (result != RESULT_OK) {
    LOG_ERROR("RobotLogUploader.Upload",
              "Unable to send upload request (error %d)", result);
    Disconnect();
    return result;
  }
  ...
  while (!ready) {
    ...
    if (n <= 0 && errno == EINTR) {
      LOG_WARNING("RobotLogUploader.Upload", "select interrupted (errno %d)",
                  errno);
      continue;
    }
    if (n <= 0) {
  ...
  if (result != RESULT_OK) {
    LOG_ERROR("RobotLogUploader.Upload",
              "Unable to receive log collector response (error %d)", result);
    return result;
  }
  ...
  switch (tag) {
    ...
    case LogCollectorResponseTag::err: {
      ...
      LOG_ERROR("RobotLogUploader.Upload", "Log collector upload error %s (%d)",
                EnumToString(err), (int)err);
      result = RESULT_FAIL;
      break;
    }
    default: {
