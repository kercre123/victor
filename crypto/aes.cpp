...
// GF(2^8) Note that i starts at 1, not 0).
...
void aes_cfb_encode(const void* key, void* iv, const uint8_t* data,
...
void aes_cfb_decode(const void* key, const void* iv, const uint8_t* data,
                    uint8_t* output, int length, void* aes_out) {
  uint8_t cleartext[AES_KEY_LENGTH];
  uint8_t ciphertext[AES_KEY_LENGTH];
