...
// Copied from https://github.com/googlesamples/android-ndk/tree/camera-sample/camera/common/utils
...
bool NativeCamera::FindSupportedCaptureResolution(ANativeWindow *display,
                                ImageResolution_Android* resRec) {
    ...
    if (foundIt) {
        resRec->width = foundRes.org_width();
        resRec->height = foundRes.org_height();
    } else {
        ...
        for (int tagIdx=0; tagIdx < count; ++tagIdx) {
            if (ACAMERA_LENS_FACING == tags[tagIdx]) {
                ...
                if (cam.facing_ == ACAMERA_LENS_FACING_FRONT) {
                  LOGI("Found front-facing camera");
...
void NativeCamera::OnSessionStateChange(ACameraCaptureSession* ses,
    ...
    if (state == CaptureSessionState::READY) {
        ...
        //TODO: move it into display, and schedule another capture
        return;
    }
