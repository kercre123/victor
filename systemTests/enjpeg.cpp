 ...
 * The header-generating code was found in a public domain JavaScript JPEG
 * encoder
 *
...
static const u8 ZIG_ZAG[] = {0,  1,  5,  6,  14, 15, 27, 28, 2,  4,  7,  13, 16,
...
static const unsigned short HTDC[256][2] = {
...
// Since we only process one row at a time, this is the state we must keep
// row-to-row
static int m_width;   // Width of the image
static int m_lastDC;  // Last DC value
static u8* m_out;     // Pointer to the output byte buffer
static int m_bitBuf,
    m_bitCnt;  // Current (unflushed) bit buffer and count of bits

static float m_quantY[64],
    m_quantUV[64];  // Quantization tables, including AAN scalers
...
static void inline writeBits(const unsigned short bits[2]) {
  // JPEG encoding is big-endian - so bits are placed starting at MSB and moving
  // toward LSB
  m_bitCnt -= bits[1];
...
static inline void putByte(u8 c) { *m_out++ = c; }

// This single-precision implementation of the AAN 1-D DCT comes from the IJG
// source
static inline void fpDCT(float i0, float i1, float i2, float i3, float i4,
                         float i5, float i6, float i7, float& d0, float& d1,
                         float& d2, float& d3, float& d4, float& d5, float& d6,
                         float& d7) {
  float tmp0 = i0 + i7;
  float tmp7 = i0 - i7;
  float tmp1 = i1 + i6;
...
static void doYBlock(u8* image) {
  ...
  for (int i = 0; i < 64; i += 8) {
    fpDCT(image[0], image[1 * PIXWIDTH], image[2 * PIXWIDTH],
          image[3 * PIXWIDTH], image[4 * PIXWIDTH], image[5 * PIXWIDTH],
          image[6 * PIXWIDTH], image[7 * PIXWIDTH], coeff[i], coeff[i + 1],
          coeff[i + 2], coeff[i + 3], coeff[i + 4], coeff[i + 5], coeff[i + 6],
          coeff[i + 7]);
  ...
  // DCT columns
  for (int i = 0; i < 8; i++)
    fpDCT(coeff[i], coeff[i + 8], coeff[i + 16], coeff[i + 24], coeff[i + 32],
          coeff[i + 40], coeff[i + 48], coeff[i + 56], coeff[i], coeff[i + 8],
          coeff[i + 16], coeff[i + 24], coeff[i + 32], coeff[i + 40],
          coeff[i + 48], coeff[i + 56]);
  ...
  for (int i = 0; i < 64; ++i) {
    ...
    // On M4 FPU conversions round toward zero, which is not as good as round
    // toward nearest
    // XXX: It's possible to make the FPU round toward nearest by messing with
    // flags and using VCVTR instruction..
    quant[ZIG_ZAG[i]] = ((int)(v) + 128) >>
                        8;  // Rounding is based on 256.0x factor in m_quantY
...
int JPEGCompress(u8* out, u8* in) {
  ...
  for (int x = 0; x < m_width; x += 8) doYBlock(in + x * PIXWIDTH);
...
int JPEGStart(u8* out, int width, int height, int quality) {
  ...
  static const float AAN_SCALERS[] = {
  ...
  for (int row = 0, k = 0; row < 8; ++row) {
    for (int col = 0; col < 8; ++col, ++k) {
      // XXX: This 256.0 (instead of 1.0) is used by a cheesy integer rouding
      // trick in m_quantY
      m_quantY[k] =
          256.0f / (YTable[ZIG_ZAG[k]] * AAN_SCALERS[row] * AAN_SCALERS[col]);
      DEBUG("%4.0f(%7.5f) ", (double)YTable[k], (double)m_quantY[k]);
      m_quantUV[k] =
          256.0f / (UVTable[ZIG_ZAG[k]] * AAN_SCALERS[row] * AAN_SCALERS[col]);
    }
    DEBUG("\n");
  }
  ...
  static const unsigned char head0[] = {
      0xFF, 0xD8, 0xFF, 0xE0,       0, 2 + 14,     // SOI, APP0, APP0 len (2+14
                                                   // bytes)
      'J',  'F',  'I',  'F',        0, 1,      1,  // Version 1.1
      0,    0,    1,    0,          1, 0,      0,  // No DPI info, no thumbnail
      0xFF, 0xDB, 0,    2 + 65 * 1,  // DQT (define quantization table) of
                                     // length 2+(1+64)*(number of QTs)
      0,                             // First QT follows
  };
  ...
  const int dhtlen = 2 + 1 + sizeof(std_dc_luminance_nrcodes) - 1 +
                     sizeof(std_dc_luminance_values) + 1 +
  ...
  const unsigned char head1[] = {
      ...
      8, height >> 8, height & 0xFF, width >> 8,
      width & 0xFF,  // 8-bit image, widthxheight
      ...
      0xFF, 0xC4, dhtlen >> 8,
      dhtlen & 255,  // DHT (define huffman table) based on dhtlen
  ...
  // putBlock(std_dc_chrominance_nrcodes+1,
  // sizeof(std_dc_chrominance_nrcodes)-1); putBlock(std_dc_chrominance_values,
  // sizeof(std_dc_chrominance_values)); putByte(0x11); // DC part of second HT
  // follows putBlock(std_ac_chrominance_nrcodes+1,
  // sizeof(std_ac_chrominance_nrcodes)-1); putBlock(std_ac_chrominance_values,
  // sizeof(std_ac_chrominance_values));
  static const unsigned char head2[] = {
      0xFF, 0xDA, 0,
      6 + 2 * 1,  // Start of scan, length = 6 + 2 * number of components
  ...
  // This removes a running DC bias caused by our use of unsigned pixels (JPEG
  // specifies signed pixels)
  m_lastDC = 8 * 128 / YTable[0];  // 64 pixels are 128 counts brighter than
                                   // expected, scaled down by DC quant
