...
int main(int argc, char** argv) {
  ...
  if (quality < 1 || quality > 100) bye("quality must be between 1 and 100");
  ...
  if (!fp) bye("Could not open input for reading");
  if (graysize != fread(in, 1, graysize + 1, fp))
    bye("Input file should be exactly width*height bytes, in ImageMagick .gray "
        "format (8bpp grayscale)");
  fclose(fp);
  ...
  fp = fopen(argv[2], "wb");
  if (!fp) bye("Could not open output for writing");
