...
BackpackLightAnimationContainer::BackpackLightAnimationContainer(
  ...
  for (const auto& pair : initializationMap) {
    ...
    if (BackpackLightAnimation::DefineFromJSON(pair.second, animation)) {
      ...
      auto animName = Util::FileUtils::GetFileName(
          pair.first, mustHaveExtension, removeExtension);
      AddAnimation(animName, std::move(animation));
    } else {
      PRINT_NAMED_ERROR(
          "BackpackLightAnimationContainer.Constructor.FailedToParseJSON",
          "Failed to parse JSON for file %s", pair.first.c_str());
    }
  }
}
...
T JsonColorValueToArray(const Json::Value& value) {
  ...
  for (u8 i = 0; i < (int)arr.size(); ++i) {
    ColorRGBA color(value[i][0].asFloat(), value[i][1].asFloat(),
                    value[i][2].asFloat(), value[i][3].asFloat());
    arr[i] = color.AsRGBA();
  }
  return arr;
}

void BackpackLightAnimationContainer::AddAnimation(
    const std::string& animationName,
    const BackpackLightAnimation::BackpackAnimation&& anim) {
  _animations.emplace(animationName, std::move(anim));
}

const BackpackLightAnimation::BackpackAnimation*
BackpackLightAnimationContainer::GetAnimation(const std::string& name) const {
  BackpackLightAnimation::BackpackAnimation* animPtr = nullptr;
  auto retVal = _animations.find(name);
  if (retVal == _animations.end()) {
    PRINT_NAMED_ERROR(
        "BackpackLightAnimationContainer.GetAnimation_Const.InvalidName",
        "Animation requested for unknown animation '%s'.", name.c_str());
  } else {
    animPtr =
        const_cast<BackpackLightAnimation::BackpackAnimation*>(&retVal->second);
  }
