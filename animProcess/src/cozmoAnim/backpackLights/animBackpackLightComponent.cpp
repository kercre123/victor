...
CONSOLE_VAR(u32, kOfflineTimeBeforeLights_ms, "Backpacklights",
            (1000 * 60 * 2));
...
BackpackLightComponent::BackpackLightComponent(const AnimContext* context)
    : _context(context), _offlineAtTime_ms(0) {
  ...
  _context->GetMicDataSystem()->AddTriggerWordDetectedCallback(
      [this](bool willStream) {
  ...
  _context->GetMicDataSystem()->AddStreamUpdatedCallback(
      [this](bool streamStart) {
...
void BackpackLightComponent::Init() {
  ...
  _backpackTriggerToNameMap =
      _context->GetDataLoader()->GetBackpackAnimationTriggerMap();
}

void BackpackLightComponent::UpdateCriticalBackpackLightConfig(
    bool isCloudStreamOpen, bool isMicMuted, bool isNotificationPending) {
  const AnimTimeStamp_t curTime_ms =
  ...
  if (isCloudStreamOpen) {
    ...
    // N        | Y            | N (happens after charging for too long
    // (>25min)) N        | N            | Y (no charging taking place)
  ...
  // If we have been offline for long enough
           ...
           ((TimeStamp_t)curTime_ms - _offlineAtTime_ms >
    ...
    // if the engine is playing a "behavior light", then we want to slide that
    // priority in right here turn off the critical lights since the engine
    // light will take priority over everything after this point once it stops,
    // critical lights will be re-started
    trigger = BackpackAnimationTrigger::Off;
  } else if (isNotificationPending) {
    trigger = BackpackAnimationTrigger::AlexaNotification;
  }
  // If we are on the charger and charging
  else if (_isOnChargerContacts && _isBatteryCharging && !_isBatteryFull &&
  ...
  if (trigger != _internalCriticalLightsTrigger) {
    ...
    if (anim == nullptr) {
      PRINT_NAMED_WARNING(
          "BackpackLightComponent.UpdateChargingLightConfig.NullAnim",
          "Got null anim for trigger %s", EnumToString(trigger));
      return;
    }

    PRINT_CH_INFO("BackpackLightComponent",
                  "BackpackLightComponent.UpdateCriticalLightConfig", "%s",
    ...
    if (trigger != BackpackAnimationTrigger::Off) {
      StartBackpackAnimationInternal(
          *anim, Util::EnumToUnderlying(BackpackLightSourcePrivate::Critical),
...
void BackpackLightComponent::Update() {
  ...
  // Consider stream to be open when the trigger word is detected or we are
  // actually streaming. Trigger word stays detected until the stream state is
  // updated
  const bool isCloudStreamOpen =
      (_willStreamOpen || _isStreaming || _alexaStreaming);
...
// behavior lights are triggered from the engine and show the state for an
// active behavior. we want these specific behavior lights to take precedence
// over some of the critical lights, but the way the system was setup, all
// critical lights take precedence over all engine lights.  This is a little
// workaround for that so we can determine if a higher priority "behavior light"
// (which is triggered from the engine) should take precedence over the current
// critical light ... see UpdateCriticalBackpackLightConfig(...) for how this is
// used
// ** a more robust solution will follow when we refactor the (many) light
// components once anim/engine processes are merged **
bool BackpackLightComponent::IsBehaviorBackpackLightActive() const {
  bool isAnyBehaviorLightActive = false;

  // _mostRecentTrigger tracks the last trigger that was requested from the
  // engine
...
void BackpackLightComponent::SetBackpackAnimation(
  ...
  StartBackpackAnimationInternal(
      lights, Util::EnumToUnderlying(BackpackLightSourcePrivate::Engine),
      _engineLightConfig);
}
...
void BackpackLightComponent::SetBackpackAnimation(
  ...
  if (anim == nullptr) {
    PRINT_NAMED_ERROR(
        "BackpackLightComponent.SetBackpackAnimation.NoAnimForTrigger",
  ...
  StartBackpackAnimationInternal(
      *anim, Util::EnumToUnderlying(BackpackLightSourcePrivate::Engine),
      _engineLightConfig);
}

void BackpackLightComponent::StartBackpackAnimationInternal(
    const BackpackLightAnimation::BackpackAnimation& lights,
...
bool BackpackLightComponent::StopBackpackAnimationInternal(
  ...
  if (!lightDataLocator._mapIter->second.empty()) {
    ...
    PRINT_NAMED_WARNING(
        "BackpackLightComponent.StopBackpackAnimationInternal.NoLocators",
...
Result BackpackLightComponent::SendBackpackLights(
    const BackpackLightAnimation::BackpackAnimation& lights) {
...
Result BackpackLightComponent::SendBackpackLights(
  ...
  if (anim == nullptr) {
    PRINT_NAMED_ERROR(
        "BackpackLightComponent.SendBackpackLights.NoAnimForTrigger",
        "Could not find animation for trigger %s name %s",
        EnumToString(trigger), animName.c_str());
    return RESULT_FAIL;
  }
...
std::vector<BackpackLightSourceType>
BackpackLightComponent::GetLightSourcePriority() {
  ...
  static_assert(
      numElements == Util::EnumToUnderlying(BackpackLightSourcePrivate::Count),
      "BackpackLightSource priority list does not contain an entry for each "
      "type of BackpackLightSource.");
...
void BackpackLightComponent::SetPairingLight(bool isOn) {
  _systemLightState =
      (isOn ? SystemLightState::Pairing : SystemLightState::Off);
}

void BackpackLightComponent::UpdateSystemLightState(bool isCloudStreamOpen) {
  if (_systemLightState == SystemLightState::Off && _selfTestRunning) {
  ...
  if (_systemLightState == SystemLightState::Off && isCloudStreamOpen) {
  ...
  if (prevState != _systemLightState) {
    ...
    switch (_systemLightState) {
      case SystemLightState::Invalid: {
        DEV_ASSERT(false,
                   "BackpackLightComponent.UpdateSystemLightState.Invalid");
        return;
      } break;
    ...
    const auto msg =
        RobotInterface::EngineToRobot(RobotInterface::SetSystemLight({light}));
    AnimComms::SendPacketToRobot((char*)msg.GetBuffer(), msg.Size());
  }
}
...
void BackpackLightComponent::UpdateBatteryStatus(
    const RobotInterface::BatteryStatus& msg) {
