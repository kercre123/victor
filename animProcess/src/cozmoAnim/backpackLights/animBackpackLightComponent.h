 ...
 *              Critical backpack lights take precedence over lights set by
 *external (engine) sources Current priority order is Low Battery, Offline,
 *Streaming, Charging, then lights sent by engine
 *
...
class BackpackLightComponent : private Util::noncopyable {
 public:
  ...
  // General purpose call to set backpack lights. The light pattern will persist
  // until this function is called again. Plays the lights on the Engine
  // priority level
  void SetBackpackAnimation(
  ...
  void SetAlexaNotification(bool hasNotification) {
  ...
  // Update battery status as we need to know when to play charging/low battery
  // lights Priority of battery related lights Low Battery > Charging > Fully
  // Charged (Off)
  void UpdateBatteryStatus(const RobotInterface::BatteryStatus& msg);

 private:
  // Start playing lights from the given source
  // lightLocator_out is updated to refer to the newly set lights and can be
  // used to stop the animation
  void StartBackpackAnimationInternal(
      const BackpackLightAnimation::BackpackAnimation& lights,
  ...
  bool StopBackpackAnimationInternal(
      const BackpackLightDataLocator& lightDataLocator);

  // Constructs and sends the actual backpack light message to the robot
  Result SendBackpackLights(
      const BackpackLightAnimation::BackpackAnimation& lights);
  ...
  // is there a current request for a light that's associated with an active
  // behavior?
  ...
  Util::CladEnumToStringMap<BackpackAnimationTrigger>*
      _backpackTriggerToNameMap = nullptr;
  ...
  // Note: this variable does NOT track the current trigger playing, it tracks
  // internal state for UpdateChargingLightConfig and should not be used for any
  // other decision making
  BackpackAnimationTrigger _internalCriticalLightsTrigger =
  ...
  bool _alexaStreaming =
      false;  // separate state in case we decide to change lights
