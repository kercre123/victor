...
// Some of our Util functions use clad features that are only generated for
// regular Cpp not CppLite such as EnumFromString so we need to write our own
...
// Public facing types that can be used when setting a backpack light
// configuration Should anything within animation process need to start setting
// backpack lights a new source should be added here and in
// GetLightSourcePriority()
enum class BackpackLightSource : BackpackLightSourceType { Count };
