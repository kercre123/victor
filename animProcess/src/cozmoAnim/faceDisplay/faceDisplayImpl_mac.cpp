...
FaceDisplayImpl::FaceDisplayImpl() {
  ...
  //      DEV_ASSERT(animSupervisor != nullptr,
  //      "animSupervisor.NullWebotsSupervisor");

  // Is the step time defined in the world file >= than the robot time? It
  // should be!
  //      DEV_ASSERT(TIME_STEP >= animSupervisor->getBasicTimeStep(),
  //      "animSupervisor.UnexpectedTimeStep");
...
void FaceDisplayImpl::FaceDraw(const u16* frame) {
  // Convert an RGB565 color into a 32-bit BGRA color image (i.e. 0xBBGGRRAA)
  // which webots expects
  ...
  // Send the entire image to webots (by using the webots::Display 'clipboard'
  // functionality), paste it from the 'clipboard' to the main display, then
  // delete it. (see
  // https://www.cyberbotics.com/doc/reference/display#display-functions)
  auto imgRef = face_->imageNew(FACE_DISPLAY_WIDTH, FACE_DISPLAY_HEIGHT,
                                faceImg_, webots::Display::ARGB);
