 ...
 * Created: 03/02/2018
 *
 * Description: Face screen object for Customer Support Info / Debug screens
 * that defines menu text, menu item selection behavior, and screen timeouts.
 *              Except for menu text, which is drawn with DrawMenu() to the
 * given image, other screen content is draw separately and is handled primarily
...
FaceInfoScreen::FaceInfoScreen(ScreenName name, ScreenName buttonGotoScreen)
...
void FaceInfoScreen::EnterScreen() {
  ...
  // Only set timeout if the timeout screen is different from the current
  // screen.
...
void FaceInfoScreen::AppendMenuItem(const std::string& text,
                                    ScreenName gotoScreen) {
...
bool FaceInfoScreen::HasMenu() const { return !_menu.empty(); }
...
ScreenName FaceInfoScreen::ConfirmMenuItemAndGetNextScreen() {
  DEV_ASSERT(HasMenu(),
             "FaceInfoScreen.ConfirmMenuItemAndGetNextScreen.MustHaveMenu");
