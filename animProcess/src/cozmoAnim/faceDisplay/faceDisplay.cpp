...
// Whether or not we need to manually stop the boot animation process,
// vic-bootAnim
...
CONSOLE_VAR_ENUM(u8, kDrawFace_Logging, ANKI_CPU_CONSOLEVARGROUP, 0,
...
void SetFaceBrightness(ConsoleFunctionContextRef context) {
  ...
  if (val >= 0 && val <= 20) {
    ...
    LOG_WARNING("FaceDisplay.SetFaceBrightness.Invalid",
                "Brightness value %d is invalid, refusing to set", val);
  }
}
...
FaceDisplay::FaceDisplay()
    : _stopDrawFace(false), _readyFace(false), _stopBootAnim(false) {
...
void FaceDisplay::DrawToFaceDebug(const Vision::ImageRGB565& img) {
  // We want to allow FaceInfoScreenManager to draw in the None screen in
  // particular in order to clear since there are no eyes to clear it for us.
  if (!FACTORY_TEST &&
      !FaceInfoScreenManager::getInstance()->IsActivelyDrawingToScreen()) {
    return;
  }
...
void FaceDisplay::DrawFaceLoop() {
  ...
  while (!_stopDrawFace) {
    // Note that this CPU profiler tag is less useful now that we are waiting on
    // a condition variable in this loop
    ANKI_CPU_TICK("FaceDisplay::DrawFaceLoop", maxDrawTime_ms,
                  Util::CpuProfiler::CpuProfilerLoggingTime(kDrawFace_Logging));
    ...
    if (_faceDrawNextImg != nullptr) {
      ...
      // Grab a reference to the image we're going to draw so we can release the
      // mutex
      ...
      //       impact to performance, both visually on the robot and dropped
      //       frames in the .gif file
      ...
      if (_displayImpl == nullptr || !_stopBootAnim) {
        // If we haven't created the display implementation instance, or we're
        // still waiting for the boot animation to complete, sleep for a bit and
        // then check again
...
void FaceDisplay::StopBootAnim() {
  if (!_stopBootAnim) {
    ...
    ExecCommandInBackground({"systemctl", "stop", "vic-bootAnim"}, [this](
                                                                       int rc) {
      if (rc != 0) {
        ...
        ExecCommandInBackground(
            {"systemctl", "kill", "-s", "9", "vic-bootAnim"}, [this](int rc) {
              // Killing didn't work for some reason so error and show fault
              // code
              if (rc != 0) {
