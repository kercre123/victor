...
class FaceInfoScreen {
 public:
  ...
  // buttonGotoScreen: Name of the screen to go to when backpack button is
  // pressed. (May be overridden in ProcessMenuNavigation())
  FaceInfoScreen(ScreenName screenName, ScreenName buttonGotoScreen);

  // Convenience ctor that allows you to specify simple non-menu text at the top
  // of the screen
  FaceInfoScreen(ScreenName screenName, ScreenName buttonGotoScreen,
  ...
  // Ordering of the items is determined by the order in which you call this
  // function
  ...
  // Resets cursor position, computes timeout, and executes custom enter/exit
  // functions.
  void EnterScreen();
  void ExitScreen();
  ...
  // Returns true if this screen has a menu item that was added via
  // AppendMenuItem
  bool HasMenu() const;
