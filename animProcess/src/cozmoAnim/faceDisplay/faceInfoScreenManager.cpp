...
std::atomic<CloudMic::ConnectionCode> _networkStatus{
    CloudMic::ConnectionCode::Connectivity};
...
CONSOLE_VAR_RANGED(f32, kToggleMuteTimeout_s, "FaceInfoScreenManager", 1.2f,
                   0.001f, 3.0f);
CONSOLE_VAR_RANGED(f32, kAlexaNotificationTimeout_s, "FaceInfoScreenManager",
                   2.0f, 0.001f, 3.0f);

// How long the button needs to be pressed for before it should trigger shutdown
// animation
CONSOLE_VAR(u32, kButtonPressDurationForShutdown_ms, "FaceInfoScreenManager",
            500);
#if ANKI_DEV_CHEATS
// Fake one of several types of button presses. This value will get reset
// immediately, so to run it again from the web interface, first set it to NoOp
CONSOLE_VAR_ENUM(int, kFakeButtonPressType, "FaceInfoScreenManager", 0,
                 "NoOp,singlePressDetected,doublePressDetected");
#endif
}  // namespace
...
void FaceInfoScreenManager::Init(Anim::AnimContext* context,
...
#define ADD_SCREEN(name, gotoScreen)                                     \
  _screenMap.emplace(                                                    \
      std::piecewise_construct, std::forward_as_tuple(ScreenName::name), \
...
#define ADD_SCREEN_WITH_TEXT(name, gotoScreen, staticText)                   \
  {                                                                          \
    ...
    _screenMap.emplace(std::piecewise_construct,                             \
                       ...
                       std::forward_as_tuple(ScreenName::name,               \
                                             ScreenName::gotoScreen, temp)); \
  }

#define ADD_MENU_ITEM(screen, itemText, gotoScreen) \
  GetScreen(ScreenName::screen)                     \
      ->AppendMenuItem(itemText, ScreenName::gotoScreen);
...
#define SET_TIMEOUT(screen, timeout_sec, timeoutScreen) \
  GetScreen(ScreenName::screen)                         \
      ->SetTimeout(timeout_sec, ScreenName::timeoutScreen);
...
#define SET_EXIT_ACTION(screen, lambda) \
  ...
  // * Camera
  const bool hideSpecialDebugScreens =
      (FACTORY_TEST && Factory::GetEMR()->fields.PLAYPEN_PASSED_FLAG) ||
      !ANKI_DEV_CHEATS;  // TODO: Use this line in master
  // const bool hideSpecialDebugScreens = (FACTORY_TEST &&
  // Factory::GetEMR()->fields.PLAYPEN_PASSED_FLAG);                        //
  // Use this line in factory branch
  ...
  // === Custom screen ===
  // Give the customText screen an exit action of resetting its timeout back to
  // default, since elsewhere we modify it when using it
  auto customTextEnterFcn = [this]() { DrawCustomText(); };
  ...
  // === Main screen ===
  auto mainEnterFcn = [this]() { DrawMain(); };
  ...
  FaceInfoScreen::MenuItemAction confirmClearUserData = [this]() {
    // Write this file to indicate that the data partition should be wiped on
    // reboot
  ...
  // === Network screen ===
  auto networkEnterFcn = [this]() { DrawNetwork(); };
  ...
  DISABLE_TIMEOUT(MirrorMode);  // Let toggling the associated VisionMode handle
                                // turning this on/off

  // === AlexaPairing ===
  auto alexaEnterAction = [this]() { DrawAlexaFace(); };
  ...
  DISABLE_TIMEOUT(
      AlexaPairing);  // let the authorization process handle timeout
  ...
  // === Toggling mute ===
  auto toggleMuteEnterAction = [this]() { DrawMuteAnimation(); };
  ...
  // === AlexaNotification ===
  auto alexaNotification = [this]() { DrawAlexaNotification(); };
  SET_ENTER_ACTION(AlexaNotification, alexaNotification);
  SET_TIMEOUT(AlexaNotification, kAlexaNotificationTimeout_s, None);

  // === Camera Motor Test ===
  // Add menu item to camera screen to start a test mode where the motors run
  // back and forth and camera images are streamed to the face
  ...
  if (OSState::getInstance()->IsInRecoveryMode()) {
    LOG_WARNING("FaceInfoScreenManager.Init.RecoveryModeFileFound",
...
FaceInfoScreen* FaceInfoScreenManager::GetScreen(ScreenName name) {
  ...
  DEV_ASSERT(it != _screenMap.end(),
             "FaceInfoScreenManager.GetScreen.ScreenNotFound");

  return &(it->second);
}

void FaceInfoScreenManager::SetNetworkStatus(
    const CloudMic::ConnectionCode& code) {
  _networkStatus = code;
...
void FaceInfoScreenManager::SetShouldDrawFAC(bool draw) {
  // TODO(Al): Remove once BC is written to persistent storage and it is easy to
  // revert robots to factory firmware to rerun them through playpen
  if (!FACTORY_TEST) {
    return;
  }
...
// Returns true if the screen is of the type during which the lift should be
// disabled and engine behaviors disabled.
...
void FaceInfoScreenManager::SetScreen(ScreenName screen) {
  ...
  DEV_ASSERT(_currScreen != nullptr,
             "FaceInfoScreenManager.SetScreen.NullCurrScreen");
  ...
  // Tell engine if the screen changes so behaviors can be appropriately
  // enabled/disabled
  ...
  if ((currScreenIsDebug != prevScreenIsDebug) ||
    ...
    // leaving the mute screen via single press may coincide with the start of a
    // wake word trigger, so don't clear it
  ...
  LOG_INFO("FaceInfoScreenManager.SetScreen.EnteringScreen", "%hhu",
           GetCurrScreenName());
  _currScreen->EnterScreen();

  if (!IsAlexaScreen(GetCurrScreenName())) {
    // when exiting alexa screens (say, into pairing), cancel any pending alexa
    // authorization
...
void FaceInfoScreenManager::DrawFAC() {
  DrawTextOnScreen(
      {"FAC"}, NamedColors::BLACK,
      (Factory::GetEMR()->fields.PLAYPEN_PASSED_FLAG ? NamedColors::GREEN
                                                     : NamedColors::RED),
      {0, FACE_DISPLAY_HEIGHT - 10}, 10, 3.f);
}

void FaceInfoScreenManager::UpdateFAC() {
  static bool prevPlaypenPassedFlag =
      Factory::GetEMR()->fields.PLAYPEN_PASSED_FLAG;
  const bool curPlaypenPassedFlag =
      Factory::GetEMR()->fields.PLAYPEN_PASSED_FLAG;
...
void FaceInfoScreenManager::DrawConfidenceClock(
    const RobotInterface::MicDirection& micData, float bufferFullPercent,
    uint32_t secondsRemaining, bool triggerRecognized) {
  // since we're always sending this data to the server, let's compute the max
  // confidence values each time and send the pre-computed values to the web
  // server so that if any of these default values change the server gets them
  // too
  ...
  // Calculate the current scale for the bars to use, based on filtered and
  ...
  if (nullptr != _webService) {
    ...
    if (_webService->IsWebVizClientSubscribed(kWebVizModuleName)) {
      ...
      if (currentTime > nextWebServerUpdateTime) {
        ...
        // 'direction' is the strongest direction, whereas 'selectedDirection'
        // is what's being used (locked-in)
        ...
        const auto& latestBeat =
            _context->GetMicDataSystem()->GetLatestBeatInfo();
  ...
  if (secondsRemaining > 0) {
    ...
    std::copy(drawText.c_str(), drawText.c_str() + drawText.length(),
              &(drawTextData.text[0]));
  ...
  DEV_ASSERT(_scratchDrawingImg != nullptr,
             "FaceInfoScreenManager::DrawConfidenceClock.InvalidScratchImage");
  ...
  constexpr int innerRadA_px =
      (int)(angleFactorA * (float)innerRadius_px);  // 3
  constexpr int innerRadB_px =
      (int)(angleFactorB * (float)innerRadius_px);  // 2
  constexpr int barWidthA_px =
      (int)(angleFactorA * (float)barWidth_px * 0.5f);  // 1
  constexpr int barWidthB_px =
      (int)(angleFactorB * (float)barWidth_px * 0.5f);  // 0
  constexpr int halfBarWidth_px = (int)((float)barWidth_px * 0.5f);

  // Multiplying factors (cos/sin) for the clock directions.
  // NOTE: Needs to have the 13th value so the unknown direction dot can display
  // properly
  ...
  // Precalculated offsets for the center of the base of each of the direction
  // bars, to be added to the center point of the clock
  ...
  // Precalculated offsets for the lower left and lower right points of the
  // direction bars (relative to a bar pointing at 12 o'clock on the drawn face,
  // aka 6 o'clock relative to victor).
  static const std::array<std::array<Point2i, 2>, 12> barWidthFactor = {{
      {{{halfBarWidth_px, 0},
        {-halfBarWidth_px, 0}}},  // 12 o'clock -  point down
      {{{barWidthA_px, barWidthB_px},
        {-barWidthA_px, -barWidthB_px}}},  // 1 o'clock
      {{{barWidthB_px, barWidthA_px},
        {-barWidthB_px, -barWidthA_px}}},  // 2 o'clock
      {{{0, halfBarWidth_px},
        {0, -halfBarWidth_px}}},  // 3 o'clock - point left
      {{{-barWidthB_px, barWidthA_px},
        {barWidthB_px, -barWidthA_px}}},  // 4 o'clock
      {{{-barWidthA_px, barWidthB_px},
        {barWidthA_px, -barWidthB_px}}},                // 5 o'clock
      {{{-halfBarWidth_px, 0}, {halfBarWidth_px, 0}}},  // 6 o'clock - point up
      {{{-barWidthA_px, -barWidthB_px},
        {barWidthA_px, barWidthB_px}}},  // 7 o'clock
      {{{-barWidthB_px, -barWidthA_px},
        {barWidthB_px, barWidthA_px}}},  // 8 o'clock
      {{{0, -halfBarWidth_px},
        {0, halfBarWidth_px}}},  // 9 o'clock - point right
      {{{barWidthB_px, -barWidthA_px},
        {-barWidthB_px, barWidthA_px}}},  // 10 o'clock
      {{{barWidthA_px, -barWidthB_px},
        {-barWidthA_px, barWidthB_px}}}  // 11 o'clock
  }};

  // Draw the outer circle
  drawImg.DrawCircle({(float)center_px.x(), (float)center_px.y()},
                     NamedColors::BLUE, circleRadius_px, 2);
  ...
  for (int i = 0; i < 12; ++i) {
    ...
    drawImg.DrawFilledConvexPolygon(
        {{baseX + barWidthFactor[i][0].x(), baseY + barWidthFactor[i][0].y()},
         {baseX + barWidthFactor[i][0].x() + lenX,
          baseY + barWidthFactor[i][0].y() + lenY},
         {baseX + barWidthFactor[i][1].x() + lenX,
          baseY + barWidthFactor[i][1].y() + lenY},
  ...
  drawImg.DrawFilledCircle(
      {(float)(center_px.x() + (int)(barLenFactor[winningIndex].x() *
                                     (float)(circleRadius_px + 1.f))),
       (float)(center_px.y() + (int)(barLenFactor[winningIndex].y() *
                                     (float)(circleRadius_px + 1.f)))},
      NamedColors::RED, 5);
  ...
  if (triggerRecognized) {
    drawImg.DrawFilledConvexPolygon(
        {{FACE_DISPLAY_WIDTH - triggerDispWidth_px,
          FACE_DISPLAY_HEIGHT - activeCircleRad_px * 2 -
              triggerOffsetFromActiveCircle_px},
         {FACE_DISPLAY_WIDTH - triggerDispWidth_px,
          FACE_DISPLAY_HEIGHT - activeCircleRad_px * 2 -
              triggerOffsetFromActiveCircle_px + triggerDispHeight},
         {FACE_DISPLAY_WIDTH, FACE_DISPLAY_HEIGHT - activeCircleRad_px * 2 -
                                  triggerOffsetFromActiveCircle_px +
                                  triggerDispHeight / 2}},
        NamedColors::GREEN);
  }
  ...
  drawImg.DrawFilledConvexPolygon(
      {{buffFullBarWidth_px, FACE_DISPLAY_HEIGHT - endOfBarHeight_px},
       ...
       {buffFullBarWidth_px + endOfBarWidth_px,
        FACE_DISPLAY_HEIGHT - endOfBarHeight_px}},
      NamedColors::RED);
  ...
  drawImg.DrawFilledConvexPolygon(
      {{0, FACE_DISPLAY_HEIGHT - endOfBarHeight_px + buffFullBarHeight_px / 2},
       ...
       {(int)(bufferFullPercent * (float)buffFullBarWidth_px),
        FACE_DISPLAY_HEIGHT - buffFullBarHeight_px / 2},
       {(int)(bufferFullPercent * (float)buffFullBarWidth_px),
        FACE_DISPLAY_HEIGHT - endOfBarHeight_px + buffFullBarHeight_px / 2}},
      NamedColors::RED);
  ...
  _scratchDrawingImg->DrawText(textLoc, delayStr, NamedColors::WHITE,
                               textScale);
...
void FaceInfoScreenManager::CheckForButtonEvent(const bool buttonPressed,
  ...
  const bool mightBeDoublePress =
      (lastPressTime_ms > 0) &&
      (curTime_ms - lastPressTime_ms < kDoublePressWindow_ms);
  ...
  const bool shouldTriggerShutdown =
      buttonPressed && (lastPressTime_ms > 0) &&
...
void FaceInfoScreenManager::ProcessMenuNavigation(const RobotState& state) {
  const bool buttonIsPressed = static_cast<bool>(
      state.status & (uint32_t)RobotStatusFlag::IS_BUTTON_PRESSED);
  bool buttonPressedEvent;
  bool buttonReleasedEvent;
  bool singlePressDetected;
  bool doublePressDetected;
  CheckForButtonEvent(buttonIsPressed, buttonPressedEvent, buttonReleasedEvent,
                      singlePressDetected, doublePressDetected);

  const bool isOnCharger = static_cast<bool>(
      state.status & (uint32_t)RobotStatusFlag::IS_ON_CHARGER);
  ...
  if (singlePressDetected && _engineLoaded) {
    if (IsAlexaScreen(currScreenName)) {
      ...
      LOG_INFO("FaceInfoScreenManager.ProcessMenuNavigation.GotSinglePress",
  ...
  if (doublePressDetected && isOnCharger &&
    ...
    if (FORCE_TRANSITION_TO_PAIRING) {
      LOG_WARNING(
          "FaceInfoScreenManager.ProcessMenuNavigation.ForcedPairing",
          "Remove FORCE_TRANSITION_TO_PAIRING when switchboard is working");
      SetScreen(ScreenName::Pairing);
    }
  } else if (doublePressDetected &&
             !isOnCharger &&  // while user-facing instructions may say "pick up
                              // the robot and double press," it's really just
                              // off charger
             _engineLoaded && CanEnterPairingFromScreen(currScreenName)) {
    ToggleMute("DOUBLE_PRESS");
  }

  // Check for button press to go to next debug screen
  if (buttonReleasedEvent) {
    if (_debugInfoScreensUnlocked && (currScreenName != ScreenName::None &&
  ...
  if (_currScreen->HasMenu() || currScreenName == ScreenName::Pairing) {
    ...
    if (!_liftTriggerReady && (liftRange_rad > kMenuLiftRange_rad)) {
      _liftTriggerReady = true;
               ...
               (Util::Abs(liftAngle - _liftLowestAngle_rad) <
                kMenuAngularTriggerThresh_rad)) {
      ...
      if (_currScreen->HasMenu()) {
        ...
        LOG_INFO("FaceInfoScreenManager.ProcessMenuNavigation.ExitPairing",
        ...
        // Note: The debug info screens will only be reported unlocked here if
        // they
        //       were unlocked the previous time the customer care screen was
        //       entered.
        DASMSG(robot_cc_screen_enter, "robot.cc_screen_enter",
               "Entered customer care screen");
        DASMSG_SET(i1, _debugInfoScreensUnlocked ? 1 : 0,
                   "Debug info screens unlocked");
        DASMSG_SEND();
      }
    }
  } else {
  ...
  // Process head motion for going from Main screen to "hidden" debug info
  // screens
  if (currScreenName == ScreenName::Main) {
    ...
    if (!_headTriggerReady && (headRange_rad > kMenuHeadRange_rad)) {
      _headTriggerReady = true;
               ...
               (Util::Abs(headAngle - _headLowestAngle_rad) <
                kMenuAngularTriggerThresh_rad)) {
      // Menu item confirmed. Go to first debug info screen
      _headTriggerReady = false;
      _debugInfoScreensUnlocked = true;
      LOG_INFO(
          "FaceInfoScreenManager.ProcessMenuNavigation.DebugScreensUnlocked",
          "");
      DrawScratch();
    }
  }
...
void FaceInfoScreenManager::DrawMain() {
  ...
  const std::string hwVer =
      "HW: " + std::to_string(Factory::GetEMR()->fields.HW_VER);
  ...
  // ESN/serialNo and the HW version are drawn on the same line with serialNo
  // default left aligned and HW version right aligned.
...
void FaceInfoScreenManager::DrawSensorInfo(const RobotState& state) {
  char temp[32] = "";
  sprintf(temp, "SYS: %s", _sysconVersion.c_str());
  const std::string syscon = temp;

  sprintf(temp, "CLF: %4u %4u %4u %4u", state.cliffDataRaw[0],
          state.cliffDataRaw[1], state.cliffDataRaw[2], state.cliffDataRaw[3]);
  const std::string cliffs = temp;

  std::string prox1, prox2;
  if (!IsWhiskey()) {
    sprintf(temp, "DIST:   %3umm", state.proxData.distance_mm);
    prox1 = temp;

    sprintf(temp, "        (%2.1f %2.1f %3.f)", state.proxData.signalIntensity,
            state.proxData.ambientIntensity, state.proxData.spadCount);
    prox2 = temp;
  }

  sprintf(temp, "TOUCH: %u", state.backpackTouchSensorRaw);
  const std::string touch = temp;

#define IS_STATUS_FLAG_SET(x) \
  ((state.status & (uint32_t)RobotStatusFlag::x) != 0)
  ...
  sprintf(temp, "BATT:  %0.2fV   %s%s%s%s%s", state.batteryVoltage,
          batteryDisconnected ? "D" : " ", batteryCharging ? "C" : " ",
          batteryHot ? "H" : " ", batteryLow ? "L" : " ",
          shutdownImminent ? "S" : " ");
  const std::string batt = temp;

  sprintf(temp, "CHGR:  %0.2fV", state.chargerVoltage);
  const std::string charger = temp;

  sprintf(temp, "TEMP:  %uC (H) / %uC (B)",
          OSState::getInstance()->GetTemperature_C(), state.battTemp_C);
  ...
  if (IsWhiskey()) {
    ...
    DrawTextOnScreen(
        {syscon, cliffs, prox1, prox2, touch, batt, charger, tempC});
  }
}

void FaceInfoScreenManager::DrawIMUInfo(const RobotState& state) {
  char temp[32] = "";
  sprintf(temp, "%*.0f %*.2f", 8, state.accel.x, 8, state.gyro.x);
  const std::string accelGyroX = temp;

  sprintf(temp, "%*.2f %*.2f", 8, state.accel.y, 8, state.gyro.y);
  const std::string accelGyroY = temp;

  sprintf(temp, "%*.2f %*.2f", 8, state.accel.z, 8, state.gyro.z);
...
void FaceInfoScreenManager::DrawMicInfo(
    const RobotInterface::MicData& micData) {
  if (GetCurrScreenName() != ScreenName::MicInfo) {
    return;
  }

  // Get the intensity of the first sample in each channel and print them to a
  // debug string. (Should we instead use the max intensity of the first n
  //samples per channel?)
  char temp[32] = "";
  sprintf(temp, "%d", micData.data[0]);
  const std::string micData0 = temp;

  sprintf(temp, "%d", micData.data[MicData::kSamplesPerBlockPerChannel]);
  const std::string micData1 = temp;

  sprintf(temp, "%d", micData.data[MicData::kSamplesPerBlockPerChannel * 2]);
  const std::string micData2 = temp;

  sprintf(temp, "%d", micData.data[MicData::kSamplesPerBlockPerChannel * 3]);
  const std::string micData3 = temp;

  DrawTextOnScreen({"MICS", micData0, micData1, micData2, micData3});
}

void FaceInfoScreenManager::SetCustomText(
    const RobotInterface::DrawTextOnScreen& text) {
  _customText = text;
...
void FaceInfoScreenManager::DrawCustomText() {
  DrawTextOnScreen({std::string(_customText.text, _customText.text_length)},
                   ColorRGBA(_customText.textColor.r, _customText.textColor.g,
                             _customText.textColor.b),
                   ColorRGBA(_customText.bgColor.r, _customText.bgColor.g,
...
void FaceInfoScreenManager::DrawAlexaFace() {
  ...
  alexaIcon.Load(_context->GetDataLoader()->GetSpritePaths()->GetAssetPath(
      kAlexaIconSpriteName));
  ...
  switch (_currScreen->GetName()) {
    case ScreenName::AlexaPairing: {
      // we have confirmed it's ok to hardcode this url, but if it's been set
      // for us already, use that
      const std::string& url =
          _alexaUrl.empty() ? "amazon.com/code" : _alexaUrl;
    ...
    default: {
      ANKI_VERIFY(false && "Unexpected alexa face",
                  "FaceInfoScreenManager.DrawAlexaFace.Unexpected", "");
      break;
    }
  }
  ...
  for (const auto& line : textVec) {
    ...
    _scratchDrawingImg->DrawTextCenteredHorizontally(
        line.text, CV_FONT_NORMAL, kDefaultTextScale * line.scale,
        kTextLineThickness, kTextColor, textLocationY, false);
  }
...
void FaceInfoScreenManager::DrawMuteAnimation() {
  ...
  // The value of muted was set prior to this method call, so indicates a
  // transition _to_ that state, so play the on/off or off/on anim to reflect
  // that
  const std::string animName =
      muted ? "anim_micstate_micoff_01" : "anim_micstate_micon_01";
...
// Draws each element of the textVec on a separate line (spacing determined by
// textSpacing_pix) in textColor with a background of bgColor.
void FaceInfoScreenManager::DrawTextOnScreen(
    const std::vector<std::string>& textVec, const ColorRGBA& textColor,
    const ColorRGBA& bgColor, const Point2f& loc, u32 textSpacing_pix,
    f32 textScale) {
  ...
  for (const auto& text : textVec) {
    _scratchDrawingImg->DrawText({textLocX, textLocY}, text.c_str(), textColor,
                                 textScale, textLineThickness);
...
void FaceInfoScreenManager::DrawTextOnScreen(const ColoredTextLines& lines,
  ...
  for (const auto& line : lines) {
    ...
    for (const auto& coloredText : line) {
      ...
      auto bbox = Vision::Image::GetTextSize(coloredText.text.c_str(),
                                             textScale, textLineThickness);
      if (coloredText.leftAlign) {
        textOffsetX += bbox.x();
      } else {
        // Right align text, need to account for the width of the text as
        // DrawText expects the bottom left corner location
        textLocX = FACE_DISPLAY_WIDTH - bbox.x() - textOffsetXRight;
        textOffsetXRight += bbox.x();
      }

      _scratchDrawingImg->DrawText({textLocX, textLocY},
                                   coloredText.text.c_str(), coloredText.color,
                                   textScale, textLineThickness);
    }
    textLocY += textSpacing_pix;
  }
...
void FaceInfoScreenManager::DrawToF(const RangeDataDisplay& data) {
  ...
  for (const auto& rangeData : data.data) {
    ...
    const Rectangle<f32> rect(x, y, gridWidth - 1,
                              gridHeight - 1);  // -1 for 1 pixel borders

    // Assuming max range is 1m
    f32 temp = std::max(rangeData.processedRange_mm,
                        0.000001f);  // Prevent divide by zero
    ...
    // Draw three things in each cell, distance (top left), status (top right),
    // and signal quality (bottom left)
    Point2f loc(x, y + 8);  // Draw text 8 pixels below top cell border
    const u8 textColor =
        (color > 128
             ? 255
             : 0);  // Make text color opposite of background for readability
    img.DrawText(loc, std::to_string((u32)(rangeData.processedRange_mm)),
                 {textColor, textColor, textColor}, kTextScale, false,
                 kTextThickness);

    // Range status is drawn a fixed amount from range distance (close to top
    // right corner of cell)
    const f32 xPos = loc.x() + (u32)(2.75f * (f32)kDefaultTextSpacing_pix);
    img.DrawText({xPos, loc.y()}, std::to_string(status),
                 {textColor, textColor, textColor}, kTextScale, false,
                 kTextThickness);

    const int yOffset = Vision::Image::GetTextSize(
                            std::to_string((u32)(rangeData.processedRange_mm)),
                            kTextScale, kTextThickness)
                            .y();
    const f32 yPos =
        loc.y() + yOffset + 1;  // +1 for extra spacing between text lines
    char t[8];
    sprintf(t, "%2.1f", signalQuality);
    img.DrawText({loc.x(), yPos}, std::string(t),
                 {textColor, textColor, textColor}, kTextScale, false,
                 kTextThickness);
  }
...
void FaceInfoScreenManager::EnablePairingScreen(bool enable) {
  if (enable && GetCurrScreenName() != ScreenName::Pairing) {
    ...
    // TODO: it's possible that the user entered the app pairing screen during
    // Alexa pairing, in which case the face should return to the Alexa screen
    // when app pairing is complete
    SetScreen(ScreenName::None);
  }
}

void FaceInfoScreenManager::EnableAlexaScreen(ScreenName screenName,
                                              const std::string& code,
                                              const std::string& url) {
  const bool validNewScreen =
      IsAlexaScreen(screenName) || (screenName == ScreenName::None);
  if (!ANKI_VERIFY(validNewScreen,
                   "FaceInfoScreenManager.EnableAlexaPairingScreen.Invalid",
  ...
  if ((screenName == ScreenName::AlexaPairing) &&
      (GetCurrScreenName() != ScreenName::AlexaPairing)) {
    _alexaCode = code;
    _alexaUrl = url;

    LOG_INFO("FaceInfoScreenManager.EnableAlexaPairingScreen.Code", "");

    DASMSG(pairing_code_displayed, "alexa.pairing_code_displayed",
           "A code to pair with AVS has been displayed");
    DASMSG_SEND();

    SetScreen(ScreenName::AlexaPairing);
  } else if ((screenName == ScreenName::AlexaPairingSuccess) &&
             (currScreen != ScreenName::AlexaPairingSuccess)) {
    LOG_INFO("FaceInfoScreenManager.EnableAlexaPairingScreen.Success", "");
    SetScreen(ScreenName::AlexaPairingSuccess);
  } else if ((screenName == ScreenName::AlexaPairingFailed) &&
             (currScreen != ScreenName::AlexaPairingFailed)) {
    LOG_INFO("FaceInfoScreenManager.EnableAlexaPairingScreen.Failed", "");
    SetScreen(ScreenName::AlexaPairingFailed);
  } else if ((screenName == ScreenName::AlexaPairingExpired) &&
             (currScreen != ScreenName::AlexaPairingExpired)) {
...
void FaceInfoScreenManager::ToggleMute(const std::string& reason) {
  ...
  if (_context->GetMicDataSystem()->IsMicMuted()) {
    DASMSG(microphone_off_message, "robot.microphone_off",
           "Microphone disabled (muted)");
    DASMSG_SET(s1, reason, "reason (how it was toggled)");
    DASMSG_SEND();
  } else {
    DASMSG(microphone_on_message, "robot.microphone_on",
  ...
  if ((_currScreen != nullptr) &&
      (_currScreen->GetName() == ScreenName::ToggleMute)) {
...
void FaceInfoScreenManager::DrawScratch() {
  ...
  const bool drawDebugScreensEnabledPixel =
      _debugInfoScreensUnlocked && GetCurrScreenName() == ScreenName::Main;
...
bool FaceInfoScreenManager::CanEnterPairingFromScreen(
    const ScreenName& screenName) const {
  switch (screenName) {
    case ScreenName::None:
    case ScreenName::FAC:
    case ScreenName::CustomText:
...
bool FaceInfoScreenManager::ScreenNeedsWait(
    const ScreenName& screenName) const {
  switch (screenName) {
    case ScreenName::AlexaPairing:
    case ScreenName::AlexaPairingSuccess:
...
void FaceInfoScreenManager::ExitCCScreen(
    Anim::AnimationStreamer* animStreamer) {
  const ScreenName curScreen =
      FaceInfoScreenManager::getInstance()->GetCurrScreenName();
  if (curScreen == ScreenName::SelfTestRunning) {
