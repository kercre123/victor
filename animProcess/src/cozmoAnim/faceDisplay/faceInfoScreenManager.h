 ...
 * Created: 12/19/2017
 *
 * Description: Handles navigation and drawing of the Customer Support Menu /
 * Debug info screens.
 *
 * Usage: Add drawing functionality as needed from various components.
 *        Add a corresponding ScreenName in faceInfoScreenTypes.h.
 *        In the new drawing functionality, return early if the ScreenName does
 * not match appropriately.
 *
...
class FaceInfoScreenManager
    : public Util::DynamicSingleton<FaceInfoScreenManager> {
  ANKIUTIL_FRIEND_SINGLETON(
      FaceInfoScreenManager);  // Allows base class singleton access
 ...
 public:
  ...
  // When enabled, switches to a screen showing the alexa pairing code, and
  // optionally the URL, depending on how the auth process originated (app or
  // voice command)
  void EnableAlexaScreen(ScreenName screenName, const std::string& code,
  ...
  // These functions update the screen only if they are relevant to the current
  // screen
  void DrawConfidenceClock(const RobotInterface::MicDirection& micData,
                           float bufferFullPercent, uint32_t secondsRemaining,
  ...
  void SetCalmPowerModeOnReturnToNone(
      const RobotInterface::CalmPowerMode& msg) {
    _calmModeMsgOnNone = msg;
  }
  ...
  void SetSysconVersion(const std::string& version) {
    _sysconVersion = version;
  }
 ...
 private:
  ...
  // Beyond return pressed and released events it also detects when a single
  // button press is detected vs. a double button press. Note that a
  // doublePressDetected does not coincide with two singlePressDetected's.
  void CheckForButtonEvent(const bool buttonPressed, bool& buttonPressedEvent,
                           bool& buttonReleasedEvent, bool& singlePressDetected,
                           bool& doublePressDetected);

  // Returns true if screenName is one of the screens that allow the user to
  // enter pairing when double pressing the backpack and on the charger
  ...
  // Returns true if screenName is a screen that should cause the behavior
  // system to Wait. Note that Pairing is handled another way, so is not
  // included here.
  ...
  struct ColoredText {
    ColoredText(const std::string& text,
                ...
                bool leftAlign = true)
        : text(text), color(color), leftAlign(leftAlign) {}
