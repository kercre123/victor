...
namespace {
const char* kPathToExternalIndependentSprites =
    "assets/sprites/independentSprites/";
const char* kPathToEngineIndependentSprites =
...
void RobotDataLoader::LoadConfigData() {
  // Text-to-speech config
  {
    ...
    const bool success = _platform->readAsJson(Util::Data::Scope::Resources,
  ...
  // Mic data config
  {
    const std::string& triggerConfigFile =
        "config/micData/micTriggerConfig.json";
    const bool success = _platform->readAsJson(
        Util::Data::Scope::Resources, triggerConfigFile, _micTriggerConfig);
    if (!success) {
...
void RobotDataLoader::LoadNonConfigData() {
  ...
  //  2) SpriteSequences use sprite map to load sequenceName -> all images in
  //  sequence directory 3) Canned animations use SpriteSequences for their
  //  FaceAnimation keyframe
  LoadIndependentSpritePaths();
  {
    std::vector<std::string> spriteSequenceDirs = {
        kPathToExternalSpriteSequences, kPathToEngineSpriteSequences};
    SpriteSequenceLoader seqLoader;
    auto* sContainer =
        seqLoader.LoadSpriteSequences(_platform, _spritePathMap.get(),
  ...
  // After we've finished loading Sprites and SpriteSequences, retroactively
  // verify any AssetID's requested before/during loading
  ...
  // Backpack light animations
  {
    ...
    const auto& fileInfo =
        animLoader.CollectAnimFiles({"config/engine/lights/backpackLights"});
...
void RobotDataLoader::LoadAnimationFile(const std::string& path) {
  ...
  if (anim == nullptr) {
    LOG_ERROR("RobotDataLoader.LoadAnimationFile", "Failed to load %s from %s",
              animName.c_str(), path.c_str());
    return;
  }
...
void RobotDataLoader::LoadIndependentSpritePaths() {
  // Get all independent sprites
  {
    ...
    for (const auto& path : spritePaths) {
      const std::string fullPathFolder =
          _platform->pathToResource(Util::Data::Scope::Resources, path);

      auto fullImagePaths = Util::FileUtils::FilesInDirectory(
          fullPathFolder, useFullPath, extensions, recurse);
      for (auto& fullImagePath : fullImagePaths) {
        const std::string fileName =
            Util::FileUtils::GetFileName(fullImagePath, true, true);
...
void RobotDataLoader::NotifyAnimAdded(const std::string& animName,
                                      uint32_t animLength) {
  AnimationAdded msg;
...
void RobotDataLoader::SetupProceduralAnimation() {
  // TODO: kevink - This should probably live somewhere else but since robot
  // data loader currently maintains control of both canned animations and
  // sprite sequences this is the best spot to put it for the time being
...
void RobotDataLoader::LoadBackpackAnimationTriggerMap() {
  _backpackAnimationTriggerMap->Load(
      _platform, "assets/cladToFileMaps/BackpackAnimationTriggerMap.json",
      "AnimName");
}

void RobotDataLoader::LoadBackpackLightAnimations(
    const CannedAnimationLoader::AnimDirInfo& fileInfo) {
  ...
  MyDispatchWorker::FunctionType loadFileFunc =
      std::bind(&RobotDataLoader::LoadBackpackLightAnimationFile, this,
                std::placeholders::_1);
  ...
  PRINT_CH_INFO("Animations",
                "RobotDataLoader.LoadBackpackLightAnimations.LoadTime",
