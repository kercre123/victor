...
PerfMetricAnim::PerfMetricAnim(const Anim::AnimContext* context)
  ...
  _headingLine1 =
      "                       Anim     Anim    Sleep    Sleep     Over      "
      "RtA   AtR   EtA   AtE  Anim Layer";
  _headingLine2 =
      "                   Duration     Freq Intended   Actual    Sleep    "
      "Count Count Count Count  Time Count";
  _headingLine2Extra = "";
  _headingLine1CSV = ",,Anim,Anim,Sleep,Sleep,Over,RtA,AtR,EtA,AtE,Anim,Layer";
  _headingLine2CSV =
      ",,Duration,Freq,Intended,Actual,Sleep,Count,Count,Count,Count,Time,"
      "Count";
...
void PerfMetricAnim::Init(Util::Data::DataPlatform* dataPlatform,
...
void PerfMetricAnim::Update(const float tickDuration_ms,
  ...
  if (_isRecording) {
    ...
    if (_bufferFilled) {
      // Move the 'first frame start time' up by the frame's time we're about to
      // overwrite
      _firstFrameTime =
          IncrementFrameTime(_firstFrameTime, frame._tickTotal_ms);
    }
    ...
    frame._relativeStreamTime_ms =
        _animationStreamer->GetRelativeStreamTime_ms();
...
const PerfMetric::FrameMetric& PerfMetricAnim::UpdateDumpAccumulators(
...
#define ANIM_LINE_DATA_VARS                                             \
  ...
  const int lenOut = snprintf(
      &_dumpBuffer[dumpBufferOffset], kSizeDumpBuffer - dumpBufferOffset,
...
int PerfMetricAnim::AppendSummaryData(const DumpType dumpType,
                                      const int dumpBufferOffset,
                                      const int lineIndex) {
  DEV_ASSERT_MSG(lineIndex < kNumLinesInSummary,
                 "PerfMetricAnim.AppendSummaryData",
...
#define APPEND_SUMMARY_LINE(StatCall)                                       \
  ...
  switch (lineIndex) {
    case 0:
      APPEND_SUMMARY_LINE(GetMin);
      break;
    case 1:
      APPEND_SUMMARY_LINE(GetMax);
      break;
    case 2:
      APPEND_SUMMARY_LINE(GetMean);
      break;
    case 3:
      APPEND_SUMMARY_LINE(GetStd);
      break;
  }
  return lenOut;
}
