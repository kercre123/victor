 ...
 * Created: Dec 5 2018
 *
 * Description: Holds weak_ptrs, and when requested, checks if the object is
 * still valid
 *
...
class DevShutdownChecker {
 public:
  ...
  void AddObject(const std::string& name, const std::shared_ptr<T>& obj) {
    ...
    ObjectInfo info;
    info.func = [wptr = std::move(weakObj)]() { return wptr.use_count(); };
    info.name = name;
    _objectInfos.push_back(std::move(info));
  }

  // Print a log line for any object that persists at the time this is called.
  // Pass removeIfDestroyed false if it should only be printed once, even if the
  // object persists for a second call to PrintRemaining
