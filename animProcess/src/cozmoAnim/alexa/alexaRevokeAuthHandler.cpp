...
// Since this file an an analog to RevokeAuthroizationHandler.cpp, some portions
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::shared_ptr<AlexaRevokeAuthHandler> AlexaRevokeAuthHandler::create(
    std::shared_ptr<ExceptionEncounteredSenderInterface>
        exceptionEncounteredSender) {
  ACSDK_DEBUG5(LX(__func__));
  if (!exceptionEncounteredSender) {
    ACSDK_ERROR(
        LX("createFailed").d("reason", "nullExceptionEncounteredSender"));
    return nullptr;
  }
  return std::shared_ptr<AlexaRevokeAuthHandler>(
      new AlexaRevokeAuthHandler(exceptionEncounteredSender));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
AlexaRevokeAuthHandler::AlexaRevokeAuthHandler(
    std::shared_ptr<ExceptionEncounteredSenderInterface>
        exceptionEncounteredSender)
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
avsCommon::avs::DirectiveHandlerConfiguration
AlexaRevokeAuthHandler::getConfiguration() const {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaRevokeAuthHandler::preHandleDirective(
    std::shared_ptr<avsCommon::avs::CapabilityAgent::DirectiveInfo> info) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaRevokeAuthHandler::handleDirectiveImmediately(
    std::shared_ptr<AVSDirective> directive) {
  handleDirective(
      std::make_shared<avsCommon::avs::CapabilityAgent::DirectiveInfo>(
          directive, nullptr));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaRevokeAuthHandler::handleDirective(
    std::shared_ptr<avsCommon::avs::CapabilityAgent::DirectiveInfo> info) {
  if (!info || !info->directive) {
    ACSDK_ERROR(LX("handleDirectiveFailed")
                    .d("reason", "nullDirectiveOrDirectiveInfo"));
    return;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaRevokeAuthHandler::cancelDirective(
    std::shared_ptr<avsCommon::avs::CapabilityAgent::DirectiveInfo> info) {
  if (!info || !info->directive) {
    removeDirectiveGracefully(info, true, "nullDirective");
    ACSDK_ERROR(LX("cancelDirectiveFailed")
                    .d("reason", "nullDirectiveOrDirectiveInfo"));
    return;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaRevokeAuthHandler::removeDirectiveGracefully(
    std::shared_ptr<avsCommon::avs::CapabilityAgent::DirectiveInfo> info,
    bool isFailure, const std::string& report) {
  if (info) {
    if (info->result) {
      if (isFailure) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool AlexaRevokeAuthHandler::addObserver(
    std::shared_ptr<RevokeAuthorizationObserverInterface> observer) {
  ACSDK_DEBUG5(LX(__func__));
  if (!observer) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool AlexaRevokeAuthHandler::removeObserver(
    std::shared_ptr<RevokeAuthorizationObserverInterface> observer) {
  if (!observer) {
