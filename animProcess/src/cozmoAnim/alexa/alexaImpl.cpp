...
namespace {
#define CRITICAL_SDK(event) \
  ACSDK_CRITICAL(avsCommon::utils::logger::LogEntry(__FILE__, event))
...
// TODO:(bn) FIXME: kAlexaImplWatchdogFailTime_s must be greater than
// kAlexaIdleDelay_s and kAlexaMaxIdleDelay_s. Should add an assert

// If the DialogUXState is set to Idle, but a directive was received to Play,
// then wait this long before setting to idle
CONSOLE_VAR_RANGED(float, kAlexaIdleDelay_s, "Alexa", 2.0f, 0.0f, 10.0f);
// If the DialogUXState was set to Idle, but the last Play directive was older
// than this amount, then switch to Idle
...
// enable to save a copy of the mic data out to the cache folder when we detect
// an "alexa" wake word NOTE: only valid and read during Init, so make sure to
// save the console var to enable it (or edit the value here)
CONSOLE_VAR(bool, kDumpAlexaTriggerAudio, "Alexa.Init", false);

// every this many seconds (in basestation time), grab the wall time (system
// clock) and see if it looks like it may have jumped. If so, refresh the alexa
// timers
CONSOLE_VAR(float, kAlexaHackCheckForSystemClockSyncPeriod_s, "Alexa", 5.0f);

// dont bubble up server errors unless the user has interacted within the last 1
// min (1 minute because sometimes it can take 30 secs for the sdk to respond
// with an error)
...
void AddFakePlayingSource(ConsoleFunctionContextRef context) {
  ...
  if (sAddFakeSourcePlaying != 0) {
    LOG_ERROR("AlexaImpl.ConsoleVar.AddFakePlayingSource.AlreadyPending",
              "Trying to set fake source for adding to %d, but %llu is already "
              "pending",
              source, sAddFakeSourcePlaying);
  } else {
...
void RemoveFakePlayingSource(ConsoleFunctionContextRef context) {
  ...
  if (sRemoveFakeSourcePlaying != 0) {
    LOG_ERROR("AlexaImpl.ConsoleVar.RemoveFakePlayingSource.AlreadyPending",
              "Trying to set fake source for removal to %d, but %llu is "
              "already pending",
              source, sRemoveFakeSourcePlaying);
  } else {
...
std::string AuthErrorToString(
    const avsCommon::sdkInterfaces::AuthObserverInterface::Error& error) {
  std::stringstream ss;
  ss << error;
  return ss.str();
}
...
std::string LocaleToString(const Util::Locale& locale) {
  // possible values: de-DE, en-AU, en-CA, en-GB, en-IN, en-US, es-ES, es-MX,
  // fr-FR, ja-JP (
  // https://developer.amazon.com/docs/alexa-voice-service/settings.html )
  // obtain the en-COUNTRY locale string from the passed-in locale's country.
  // Note that if a user goes into their alexa app and changes locale to
  // something like fr-CA, then goes into chewie and selects CA, we send to
  // amazon en-CA, and they'll have to go back into the alexa app if they need
  // to reselect french.
...
AlexaImpl::AlexaImpl()
  ...
  static_assert(
      std::is_same<
          SourceId,
          avsCommon::utils::mediaPlayer::MediaPlayerInterface::SourceId>::value,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaImpl::Init(const Anim::AnimContext* context,
                     InitCompleteCallback&& completionCallback) {
  _context = context;
  _initCompleteCallback = std::move(completionCallback);

  _lastWallTime = WallTime::getInstance()->GetApproximateTime();
  _lastWallTimeCheck_s =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();

  const auto* dataPlatform = _context->GetDataPlatform();

  _alexaPersistentFolder =
      dataPlatform->pathToResource(Util::Data::Scope::Persistent, kAlexaFolder);
  _alexaPersistentFolder =
      Util::FileUtils::AddTrailingFileSeparator(_alexaPersistentFolder);
  if (!_alexaPersistentFolder.empty() &&
      Util::FileUtils::DirectoryDoesNotExist(_alexaPersistentFolder)) {
    Util::FileUtils::CreateDirectory(_alexaPersistentFolder);
  }

  _alexaCacheFolder =
      dataPlatform->pathToResource(Util::Data::Scope::Cache, kAlexaFolder);
  _alexaCacheFolder =
      Util::FileUtils::AddTrailingFileSeparator(_alexaCacheFolder);
  if (!_alexaCacheFolder.empty() &&
      Util::FileUtils::DirectoryDoesNotExist(_alexaCacheFolder)) {
...
void AlexaImpl::InitThread() {
  ...
  if (!avsCommon::avs::initialization::AlexaClientSDKInit::initialize(
          configs)) {
    FailInitialization("Failed to initialize SDK!");
    return;
  }

  // The observer logs important changes in a variety of sdk components, and
  // calls back to AlexaImpl for things we register to. It's not possible to do
  // away with this object and instead pass shared_from_this{AlexaImpl} to
  // methods, even if AlexaImpl derived from the same base classes as
  // AlexaObserver, because Alexa needs to be able to destroy AlexaImpl. Use of
  // shared_from_this would mean that AlexaImpl and its children would form a
  // group of self-referencing shared_ptrs that could never be deleted. Instead,
  // AlexaImpl can be held as a unique_ptr in Alexa, but it means that we need
  // this additional object AlexaObserver.
  _observer = std::make_shared<AlexaObserver>();
  _observer->Init(
      std::bind(&AlexaImpl::OnDialogUXStateChanged, this,
                std::placeholders::_1),
      std::bind(&AlexaImpl::OnRequestAuthorization, this, std::placeholders::_1,
                std::placeholders::_2),
      std::bind(&AlexaImpl::OnAuthStateChange, this, std::placeholders::_1,
                std::placeholders::_2),
      std::bind(&AlexaImpl::OnSourcePlaybackChange, this, std::placeholders::_1,
                std::placeholders::_2),
      std::bind(&AlexaImpl::OnInternetConnectionChanged, this,
                std::placeholders::_1),
      std::bind(&AlexaImpl::OnAVSConnectionChanged, this, std::placeholders::_1,
                std::placeholders::_2),
      std::bind(&AlexaImpl::OnSendComplete, this, std::placeholders::_1),
      std::bind(&AlexaImpl::OnSDKLogout, this),
      std::bind(&AlexaImpl::OnNotificationsIndicator, this,
                std::placeholders::_1),
      std::bind(&AlexaImpl::OnAlertState, this, std::placeholders::_1,
                std::placeholders::_2),
      std::bind(&AlexaImpl::OnPlayerActivity, this, std::placeholders::_1));

  const auto& rootConfig =
      avsCommon::utils::configuration::ConfigurationNode::getRoot();

  // Creating customerDataManager which will be used by the registrationManager
  // and all classes that extend CustomerDataHandler
  // TODO (VIC-9837) see what else this baby can do
  auto customerDataManager =
  ...
  std::shared_ptr<avsCommon::utils::DeviceInfo> deviceInfo =
      avsCommon::utils::DeviceInfo::create(rootConfig);
  if (!deviceInfo) {
    FailInitialization("Creation of DeviceInfo failed!");
    return;
  } else {
    DASMSG(avs_device_info, "alexa.device_info",
           "we created an alexa object with the given device info");
  ...
  auto authDelegateStorage =
      authorization::cblAuthDelegate::SQLiteCBLAuthDelegateStorage::create(
          rootConfig);
  std::shared_ptr<avsCommon::sdkInterfaces::AuthDelegateInterface>
      authDelegate = authorization::cblAuthDelegate::CBLAuthDelegate::create(
          rootConfig, customerDataManager, std::move(authDelegateStorage),
          _observer, nullptr, deviceInfo);

  // Creating the misc DB object to be used by various components.
  std::shared_ptr<storage::sqliteStorage::SQLiteMiscStorage> miscStorage =
      storage::sqliteStorage::SQLiteMiscStorage::create(rootConfig);
  if (!miscStorage) {
  ...
  // Creating the CapabilitiesDelegate - This component provides the client with
  // the ability to send messages to the Capabilities API.
  _capabilitiesDelegate = capabilitiesDelegate::CapabilitiesDelegate::create(
      authDelegate, miscStorage, httpPut, customerDataManager, rootConfig,
      deviceInfo);
  if (!_capabilitiesDelegate) {
  ...
  auto messageStorage =
      certifiedSender::SQLiteMessageStorage::create(rootConfig);

  auto notificationsStorage =
      capabilityAgents::notifications::SQLiteNotificationsStorage::create(
          rootConfig);

  // settings (DEPRECATED)
  auto settingsStorage =
      capabilityAgents::settings::SQLiteSettingStorage::create(rootConfig);
  // settings
  auto deviceSettingsStorage =
      settings::storage::SQLiteDeviceSettingStorage::create(rootConfig);

  // Creating the alert storage object to be used for rendering and storing
  // alerts.
  auto audioFactory = std::make_shared<AlexaAudioFactory>();
  auto alertStorage =
      capabilityAgents::alerts::storage::SQLiteAlertStorage::create(
          rootConfig, audioFactory->alerts());
  // setup media player interfaces
  auto httpContentFetcherFactory = std::make_shared<
      avsCommon::utils::libcurlUtils::HTTPContentFetcherFactory>();
  _ttsMediaPlayer = std::make_shared<AlexaMediaPlayer>(
      AlexaMediaPlayer::Type::TTS, httpContentFetcherFactory);
  _ttsMediaPlayer->Init(_context);
  _alertsMediaPlayer = std::make_shared<AlexaMediaPlayer>(
      AlexaMediaPlayer::Type::Alerts, httpContentFetcherFactory);
  _alertsMediaPlayer->Init(_context);
  _audioMediaPlayer = std::make_shared<AlexaMediaPlayer>(
      AlexaMediaPlayer::Type::Audio, httpContentFetcherFactory);
  _audioMediaPlayer->Init(_context);
  _notificationsMediaPlayer = std::make_shared<AlexaMediaPlayer>(
      AlexaMediaPlayer::Type::Notifications, httpContentFetcherFactory);
  ...
  // Create an internet connection monitor that will notify the _observer when
  // internet connectivity changes. Note that this seems to poll every
  // DEFAULT_TEST_PERIOD=5 mins, which isn't enough to depend on for error
  // states. However, it does provide an accurate internet status on init
  auto internetConnectionMonitor =
      avsCommon::utils::network::InternetConnectionMonitor::create(
  ...
  const avsCommon::sdkInterfaces::softwareInfo::FirmwareVersion
      firmwareVersion = GetFirmwareVersion();

  _client = AlexaClient::Create(
      deviceInfo, customerDataManager, authDelegate, std::move(messageStorage),
      std::move(alertStorage), std::move(notificationsStorage),
      std::move(settingsStorage), std::move(deviceSettingsStorage),
      audioFactory, {_observer}, {_observer}, {_observer},
      _capabilitiesDelegate, _ttsMediaPlayer, _alertsMediaPlayer,
      _audioMediaPlayer, _notificationsMediaPlayer,
      std::static_pointer_cast<avsCommon::sdkInterfaces::SpeakerInterface>(
          _ttsMediaPlayer),
      std::static_pointer_cast<avsCommon::sdkInterfaces::SpeakerInterface>(
          _alertsMediaPlayer),
      std::static_pointer_cast<avsCommon::sdkInterfaces::SpeakerInterface>(
      ...
      std::move(internetConnectionMonitor), firmwareVersion);
  ...
  _client->SetDirectiveCallback(
      std::bind(&AlexaImpl::OnDirective, this, std::placeholders::_1,
  ...
  auto revokeObserver = std::make_shared<AlexaRevokeAuthObserver>(
      _client->GetRegistrationManager());
  _client->AddRevokeAuthorizationObserver(revokeObserver);

  _settingsCallbacks =
      settings::SettingCallbacks<settings::DeviceSettingsManager>::create(
          _client->GetSettingsManager());
  if (ANKI_VERIFY(_settingsCallbacks != nullptr,
                  "AlexaImpl.InitThead.NoCallbacks",
                  "Could not create settings callbacks")) {
    _settingsCallbacks->add<settings::DeviceSettingsIndex::DO_NOT_DISTURB>(
        [](bool enable, settings::SettingNotifications notifications) {
          // todo: handle do not disturb setting
          LOG_INFO("AlexaImpl.InitThead.SettingsCallback",
                   "Set DO_NOT_DISTURB=%d", enable);
        });
  }

  // Creating the buffer (Shared Data Stream) that will hold user audio data.
  // This is the main input into the SDK.
  size_t bufferSize = avsCommon::avs::AudioInputStream::calculateBufferSize(
      kBufferSize, kWordSize, kMaxReaders);
  auto buffer =
      std::make_shared<avsCommon::avs::AudioInputStream::Buffer>(bufferSize);
  ...
  // Creating each of the audio providers. An audio provider is a simple package
  // of data consisting of the stream of audio data, as well as metadata about
  // the stream. For each of the three audio providers created here, the same
...

  {
    ...
    _tapToTalkAudioProvider =
        std::make_shared<capabilityAgents::aip::AudioProvider>(
            sharedDataStream, compatibleAudioFormat, kNearField,
            tapAlwaysReadable, tapCanOverride, tapCanBeOverridden);
  }

  {
    ...
    _wakeWordAudioProvider =
        std::make_shared<capabilityAgents::aip::AudioProvider>(
            sharedDataStream, compatibleAudioFormat, kNearField,
            wakeAlwaysReadable, wakeCanOverride, wakeCanBeOverridden);
    ...
    std::shared_ptr<esp::ESPDataModifierInterface> espModifier =
        dummyEspProvider;
    // This observer is notified any time a keyword is detected and notifies the
    // AlexaClient to start recognizing.
    _keywordObserver = std::make_shared<AlexaKeywordObserver>(
        _client, *_wakeWordAudioProvider, espProvider);
  }
  ...
  if (ANKI_DEV_CHEATS && kDumpAlexaTriggerAudio) {
    ...
    std::shared_ptr<avsCommon::avs::AudioInputStream> debugDataStream =
        avsCommon::avs::AudioInputStream::create(debugBuffer, kWordSize,
                                                 kMaxReaders);
  ...
  // try connecting... this is already async but let's trigger it from this
...
void AlexaImpl::Update() {
  ...
  // NOTE: these (intentionally) don't call CheckForUXStateChange, so they may
  // not have immediate impact until something else happens
  ...
  if (kAlexaHackCheckForSystemClockSyncPeriod_s > 0.0f &&
      currTime_s >=
          _lastWallTimeCheck_s + kAlexaHackCheckForSystemClockSyncPeriod_s) {
    // HACK: NTP can cause the time to jump into the future, and the alexa
    // client's timers won't properly ring if this happens after initialization.
    // OSState::IsWallTimeSynced() exists to try to detect this, but appears to
    // be unreliable (see JIRA VIC-4527), so use this hack instead.
    ...
    if (std::abs(deltaSeconds - kAlexaHackCheckForSystemClockSyncPeriod_s) >
        kTimeToConsiderJump_s) {
      LOG_WARNING("AlexaImpl.Update.TimeJumpDetected.ResetTimers",
                  "Detected a time jump of %lld seconds (in %f BS seconds), "
                  "refreshing timers",
                  deltaSeconds, kAlexaHackCheckForSystemClockSyncPeriod_s);
  ...
  if (_timeToSetIdle_s >= 0.0f && currTime_s >= _timeToSetIdle_s &&
      _playingSources.empty()) {
    if (_dialogState != DialogUXState::IDLE) {
      _dialogState = DialogUXState::IDLE;
      LOG_INFO("AlexaImpl.Update.SetDialogStateIdle",
               "the timer to set idle elapsed, updating dialog state to idle");
    }
    checkedUXState = true;
    CheckForUXStateChange();
  }

  if ((_nextUXStateCheckTime_s > 0.0f) &&
      (currTime_s >= _nextUXStateCheckTime_s)) {
...
void AlexaImpl::CheckStateWatchdog() {
  ...
  if (currTime_s > _lastWatchdogCheckTime_s + kAlexaImplWatchdogCheckTime_s) {
    if (_uxState == AlexaUXState::Speaking) {
      // attempt to detect "forever face" bugs that could happen if we get out
      // of sync with one of the speakers. If we think we're speaking, but
      // nothing is playing, and this state persists for a while, then we are
      // likely "stuck"

      // NOTE: there is no code here to check if alerts are active (foreground
      // or background). This is intentional, if the alerts are active we hold
      // the state speaking, but we do that because we expect the Alerts player
      // to start making noise momentarily

      bool anyActivePlayers = false;
      for (const auto& playerPtr :
           {_ttsMediaPlayer, _alertsMediaPlayer, _audioMediaPlayer,
            _notificationsMediaPlayer}) {
      ...
      if (haveStuckTimer && anyActivePlayers) {
        // we thought we might be stuck previously, but now we know we aren't,
        // yay!
        LOG_INFO("AlexaImpl.CheckStateWatchdog.NotStuck",
                 "previously thought we might be stuck, but a player is active "
                 "so we aren't stuck (here at least)");
        // reset stuck timer
        _possibleStuckStateStartTime_s = -1.0f;
      } else if (haveStuckTimer && !anyActivePlayers) {
        // we still appear to be stuck
        const bool stuckForTooLong =
            (currTime_s >=
             _possibleStuckStateStartTime_s + kAlexaImplWatchdogFailTime_s);
        if (stuckForTooLong) {
        ...
        // else, we're stuck but it hasn't been long enough yet so wait a little
        // longer
      } else if (!haveStuckTimer && !anyActivePlayers) {
        // uh oh, we are speaking but there are no players active. This could be
        // OK if we are just holding speaking for a few seconds.
        ...
        if (_possibleStuckStateStartTime_s < 0.0f) {
          ...
          LOG_INFO("AlexaImpl.CheckStateWatchdog.WatchdogActive",
                   "At t=%f detected that no players are active, so we might "
                   "be stuck (but probably not)",
                   currTime_s);
        }
      } else {
        // LOG_INFO( "AlexaImpl.CheckStateWatchdog.HonkeyDorey", "ALL GOOD
        // active=%d", anyActivePlayers );
      }
    } else {
      if (_possibleStuckStateStartTime_s > 0.0f) {
...
void AlexaImpl::AttemptToFixStuckInSpeakingBug() {
  LOG_ERROR("AlexaImpl.WatchdogFailure.AttempToFix",
            "Attempting to fix watchdog state failure");
  ...
  const bool recovered = ANKI_VERIFY(
      _uxState == AlexaUXState::Idle, "AlexaImple.WatchdogFailFail",
  ...
  DASMSG_SET(s1, recovered ? "SUCCESS" : "FAILURE",
             "Success or failure of recovery");
  DASMSG_SET(s2, EnumToString(_uxState),
             "UX state after recovery (should be Idle if success)");
  DASMSG_SET(i1, sourceCounts,
             "Number of sources that were supposedly playing");
  ...
  DASMSG_SET(i4, alertWasBackground,
             "Was alert counted as active in the background?");
  DASMSG_SEND();

  if (!recovered) {
    // uh oh, something went _really_ wrong, let's clear the dialog state, force
    // the ux state and send it off so we at least might be able to get vector
    // back
    _dialogState = avsCommon::sdkInterfaces::DialogUXStateObserverInterface::
        DialogUXState::IDLE;
    _uxState = AlexaUXState::Idle;
    _audioActive = false;
...
void AlexaImpl::UpdateAsyncInit() {
  ...
  if ((_initState == InitState::ThreadComplete) ||
      (_initState == InitState::ThreadFailed)) {
    // loading is now done
    _initThread.join();
...
void AlexaImpl::SetLocale(const Util::Locale& locale) {
  if (ANKI_VERIFY(_client != nullptr, "AlexaImpl.SetLocale.NotInitialized",
    ...
    // Only notify the client if the user selected a country we expect (i.e.,
    // those listed in chewie)
...
void AlexaImpl::Logout() {
  ...
  if (!logoutCalled) {
    DASMSG(user_sign_out_result, "alexa.user_sign_out_result",
           "User signed out of AVS SDK");
    DASMSG_SET(
        s1, "FAILURE",
        "SUCCESS or FAILURE (note that this instance always sends failure)");
    DASMSG_SET(s2, "INTERNAL_ERROR",
               "error type (only INTERNAL_ERROR currently exists, meaning "
               "could not call logout)");
    DASMSG_SEND();
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaImpl::AddMicrophoneSamples(
    const AudioUtil::AudioSample* const samples, size_t nSamples) {
...
std::vector<std::shared_ptr<std::istream>> AlexaImpl::GetConfigs() const {
  ...
  configJsonStreams.push_back(
      std::shared_ptr<std::istringstream>(new std::istringstream(configJson)));
  return configJsonStreams;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaImpl::OnDirective(const std::string& directive,
                            const std::string& payload,
                            const std::string& fullUnparsed) {
  if (directive == "Play" || directive == "Speak" || directive == "SetAlert" ||
      directive == "DeleteAlert") {
    _lastPlayDirective_s =
        BaseStationTimer::getInstance()->GetCurrentTimeInSeconds();
  }
  // TODO: is there a Stop directive we can listed to? that should force the
  // Idle ux state or at least reset idle timers "StopCapture" might be it.
  // needs further investigation to see how it overlaps with music audio
  ...
  if (kLogAlexaDirectives) {
    ...
    if (success) {
      ...
      Util::FileUtils::WriteFile(_alexaCacheFolder + kDirectivesFile,
                                 std::to_string(bsTime) + " " + directive +
                                     ": " + json.toStyledString(),
                                 append);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaImpl::OnAuthStateChange(
  ...
  LOG_INFO("AlexaImpl.OnAuthStateChange",
           "authStateChanged newState=%d, error=%d", (int)newState, (int)error);

  auto sendResultDas =
      [](bool success,
         avsCommon::sdkInterfaces::AuthObserverInterface::Error error) {
        DASMSG(user_sign_in_result, "alexa.user_sign_in_result",
               "Result of initial user sign in attempt");
        DASMSG_SET(s1, success ? "SUCCESS" : "FAILURE", "SUCCESS or FAILURE");
        DASMSG_SET(s2, AuthErrorToString(error).c_str(),
                   "Error reason (from AVS SDK)");
        DASMSG_SEND();
      };

  // if not SUCCESS or AUTHORIZATION_PENDING, fail. AUTHORIZATION_PENDING seems
  // like a valid error to me: "Waiting for user to authorize the specified code
  // pair."
  if ((error != Error::SUCCESS) && (error != Error::AUTHORIZATION_PENDING)) {
    LOG_WARNING("AlexaImpl.onAuthStateChange.Error",
                "Alexa authorization experiences error (%d)", (int)error);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaImpl::OnRequestAuthorization(const std::string& url,
...
void AlexaImpl::OnDialogUXStateChanged(DialogUXState state) {
  LOG_INFO(
      "AlexaImpl.OnDialogUXStateChanged", "from '%s' to '%s'",
      avsCommon::sdkInterfaces::DialogUXStateObserverInterface::stateToString(
          _dialogState)
          .c_str(),
      avsCommon::sdkInterfaces::DialogUXStateObserverInterface::stateToString(
          state)
...
void AlexaImpl::CheckForUXStateChange() {
  ...
  // When playing sequential audio (e.g. flash news briefing) there is often a
  // very short gap between one source ending and the next starting. So, if one
  // is playing, consider it playing, but also consider it playing if it stopped
  // recently
  const bool audioPlaying =
      _audioActive || (currTime_s - _audioActiveLastChangeTime_s <=
                       kTimeToHoldSpeakingBetweenAudioPlays_s);

  // Consider a timer / alarm active if it's really active or if it's in the
  // background, since background timers will jump to foreground as soon as
  // nothing higher priority (i.e. Dialog) is playing
  const bool alertActive = _alertActive || _backgroundAlertActive;

  const bool anyPlaying =
      !_playingSources.empty() || alertActive || audioPlaying;

  // reset idle timer
  _timeToSetIdle_s = -1.0f;

  switch (_dialogState) {
    case avsCommon::sdkInterfaces::DialogUXStateObserverInterface::
        DialogUXState::FINISHED:
    case avsCommon::sdkInterfaces::DialogUXStateObserverInterface::
        DialogUXState::IDLE: {
      // if nothing is playing, it could be because something is about to play.
      // We only know because a directive for "Play" or "Speak" has arrived. If
      // one has arrived recently, don't switch to idle yet, and instead set a
      // timer so that it will switch to idle only if no other state change
      // occurs. Sometimes, if the delay is long enough, the robot will
      // momentarily break to normal behavior before returning to alexa. Maybe
      // we need a base delay. Note that we also consider this delay if an
      // alerts directive is received, because we somtimes receive the alerts
      // directive before the corresponding TTS (e.g. "Alarm set for 5pm").
      if (anyPlaying) {
        _uxState = AlexaUXState::Speaking;
      } else if (_lastPlayDirective_s < 0.0f ||
                 (currTime_s > _lastPlayDirective_s + kAlexaMaxIdleDelay_s)) {
        // "Play" wasn't received within kAlexaMaxIdleDelay_s seconds ago. Set
        // to Idle
        _uxState = AlexaUXState::Idle;
  ...
  if ((oldState != _uxState) && (_onAlexaUXStateChanged != nullptr)) {
    DEV_ASSERT(_uxState != AlexaUXState::Error,
               "AlexaImpl.CheckForUXStateChange.NoError");
    _onAlexaUXStateChanged(_uxState);
  }
}
...
void AlexaImpl::OnSourcePlaybackChange(SourceId id, bool playing) {
  // TODO:(bn) this could cause "forever face" bugs if it gets out of sync with
  // the actual media player state. This state duplication should be resolved
  // (both are in anim, no need for it)

  if (playing) {
    auto res = _playingSources.insert(id);
    // not fatal, but it means we don't understand the media player lifecycle
    // correctly
    if (ANKI_VERIFY(res.second, "AlexaImpl.OnSourcePlaybackChange.Exists",
                    "Source %llu was already playing", id)) {
      CheckForUXStateChange();
    }
  } else {
    ...
    if (it != _playingSources.end()) {
      ...
      LOG_WARNING("AlexaImpl.OnSourcePlaybackChange.NotFound",
                  "Source %llu not found", id);
    }
  }
  ...
  LOG_INFO("AlexaImpl.OnSourcePlaybackChange",
           "Source %llu set to %s, %zu are now playing: %s", id,
           playing ? "playing" : "not playing", _playingSources.size(),
           ss.str().c_str());
#endif
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaImpl::OnAVSConnectionChanged(
  ...
  LOG_INFO("AlexaImpl.OnAVSConnectionChanged", "status=%d, reason=%d", status,
           reason);
  if (status == avsCommon::sdkInterfaces::ConnectionStatusObserverInterface::
                    Status::CONNECTED) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaImpl::OnSendComplete(
    avsCommon::sdkInterfaces::MessageRequestObserverInterface::Status status) {
  LOG_INFO("AlexaImpl.OnSendComplete", "status '%s'",
           MessageStatusToString(status).c_str());

  DASMSG(send_complete, "alexa.response_to_request",
         "SDK responded to a sent message");
  DASMSG_SET(s1, MessageStatusToString(status).c_str(),
             "AVS-provided message status (e.g. SUCCESS, TIMEDOUT, ...)");
  DASMSG_SEND();

  // for most cases, only consider a non-success as an error if the user
  // interacted recently, because sometimes the sdk sends things behind the
  // scenes, and we dont want the robot to suddenly start speaking.

  using Status =
      avsCommon::sdkInterfaces::MessageRequestObserverInterface::Status;
  switch (status) {
    case Status::PENDING:  // The message has not yet been processed for
                           // sending.
    case Status::SUCCESS:  // The message was successfully sent.
    case Status::SUCCESS_NO_CONTENT:  // The message was successfully sent but
                                      // the HTTPReponse had no content.
      // don't "unset" the error here, since we don't know what the successful
      // message accomplished. it could still be in an error state.
      break;

    case Status::NOT_CONNECTED:  // The send failed because AVS was not
                                 // connected.
    case Status::TIMEDOUT:  // The send failed because of timeout waiting for
                            // AVS response.
    {
    ...
    case Status::CANCELED:  // The send failed due to server canceling it before
    ...
    case Status::NOT_SYNCHRONIZED:  // The send failed because AVS is not
                                    // synchronized.
    case Status::PROTOCOL_ERROR:    // The send failed due to an underlying
                                    // protocol error.
    case Status::INTERNAL_ERROR:    // The send failed due to an internal error
                                    // within ACL.
    case Status::SERVER_INTERNAL_ERROR_V2:  // The send failed due to an
                                            // internal error on the server
                                            // which sends code 500.
    case Status::REFUSED:      // The send failed due to server refusing the
                               // request.
    case Status::THROTTLED:    // The send failed due to excessive load on the
                               // server.
    case Status::BAD_REQUEST:  // The send failed due to invalid request sent by
                               // the user.
    case Status::SERVER_OTHER_ERROR:  // The send failed due to unknown server
                                      // error.
    {
      // sending the last request ended in failure. show the error face and play
      // audio
      if (InteractedRecently()) {
    ...
    case Status::INVALID_AUTH:  // The access credentials provided to ACL were
                                // invalid.
    {
      // sending the last request failed because auth was revoke. show the error
      // face and play audio
      if (InteractedRecently()) {
...
void AlexaImpl::OnSDKLogout() {
  ...
  // NOTE:(bn) This doesn't get called if we log out, nor if you de-register
  // from Amazon, not sure when it would get called
  DASMSG(user_sign_out_result, "alexa.user_remote_sign_out",
...
void AlexaImpl::OnAlertState(
  ...
  for (auto& alertPair : _alertStates) {
    ...
    switch (alertPair.second) {
      ...
      case State::FOCUS_ENTERED_FOREGROUND:  // The alert has entered the
                                             // foreground.
        canBeCancelled = true;
        break;

      case State::FOCUS_ENTERED_BACKGROUND:  // The alert has entered the
                                             // background.
        inBackground = true;

        // fall through
      case State::READY:      // The alert is ready to start, and is waiting for
                              // channel focus.
      case State::STOPPED:    // The alert has stopped due to user or system
                              // intervention.
      case State::SNOOZED:    // The alert has snoozed.
      case State::COMPLETED:  // The alert has completed on its own.
      case State::PAST_DUE:   // The alert has been determined to be past-due,
  ...
  LOG_INFO(
      "AlexaImpl.OnAlertState",
      "alert '%s' changed to state '%s', _alertActive=%d, "
      "_backgroundAlertActive=%d, %zu alerts tracked",
      alertID.c_str(),
      capabilityAgents::alerts::AlertObserverInterface::stateToString(state)
          .c_str(),
      _alertActive, _backgroundAlertActive, _alertStates.size());
  if (oldAlertActive != _alertActive) {
    // note: this is ok to only have two options, not three (e.g., "unknown")
    // since _alertActive is initialized as false, in which case if the initial
    // assignment to _alertActive is false, we don't need to call
    // CheckForUXStateChange anyway
    CheckForUXStateChange();
  }
  ...
  switch (state) {
    ...
    case State::PAST_DUE:   // The alert has been determined to be past-due, and
                            ...
                            // intervention.
    {
      ...
      ANKI_VERIFY(numErased == 1, "AlexaImpl.OnAlertState.BadAlertRemove",
                  "Erase of alert %s removed %zu elements", alertID.c_str(),
                  numErased);
      break;
    }
  }
...
void AlexaImpl::OnPlayerActivity(avsCommon::avs::PlayerActivity state) {
  ...
  if (playing != _audioActive) {
    ...
    CheckForUXStateChange();
    // call CheckForUXStateChange() again once
    // kTimeToHoldSpeakingBetweenAudioPlays_s expires todo: merge
    // _nextUXStateCheckTime_s logic into _timeToSetIdle_s
    _nextUXStateCheckTime_s =
        std::max(_nextUXStateCheckTime_s,
                 currTime_s + kTimeToHoldSpeakingBetweenAudioPlays_s + 0.1f);
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaImpl::SetAuthState(AlexaAuthState state, const std::string& url,
                             const std::string& code, bool errFlag) {
  // always send WaitingForCode in case url or code changed
  const bool changed =
      (_authState != state) || (state == AlexaAuthState::WaitingForCode);
  _authState = state;
...
void AlexaImpl::NotifyOfTapToTalk() {
  ...
  if (_client != nullptr) {
    if (!_isTapOccurring) {
      ...
      // check info known about connection before trying. these often don't get
      // updated until sending fails
      if (!_client->IsAVSConnected()) {
        SetNetworkConnectionError();
      } else {
        if (!_alertActive) {
          ANKI_VERIFY(
              _client->NotifyOfTapToTalk(*_tapToTalkAudioProvider).get(),
...
void AlexaImpl::NotifyOfWakeWord(uint64_t fromIndex, uint64_t toIndex) {
  ...
  if (_debugMicrophone) {
    ...
    const std::string filename = _alexaCacheFolder + "audioInput_" +
                                 std::to_string(sAudioDumpIdx++) + ".pcm";
    ...
    if (fileOut.is_open()) {
      ...
      // "absolute" index is total bytes ever, not the current size of the ring
      // buffer
      ...
      LOG_INFO("AlexaImpl.WroteAudioInput",
               ...
               filename.c_str(), debugStartIdx, debugEndIdx);
    }
  }
#endif

  // this can generate SdsReader:seekFailed:reason=seekOverwrittenData if the
  // time indices contain overwritten data
  _keywordObserver->onKeyWordDetected(_wakeWordAudioProvider->stream, "ALEXA",
                                      fromIndex, toIndex, nullptr);
}
...
void AlexaImpl::RemoveCallbacksForShutdown() {
  ...
  if (_initThread.joinable()) {
    // shut down init thread in prep for destruction since destruction could
    // occur on a thread
    _initThread.join();
    _initThread = {};
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
avsCommon::sdkInterfaces::softwareInfo::FirmwareVersion
AlexaImpl::GetFirmwareVersion() const {
  int major = 0;
  int minor = 0;
  int incremental = 0;
  int build = 0;
  OSState::getInstance()->GetOSBuildVersion(major, minor, incremental, build);
  // reversible, simpler than crc, but obviously might not be unique and may
  // overflow
  std::string concatVersion = std::to_string(major) + std::to_string(minor) +
  ...
  avsCommon::sdkInterfaces::softwareInfo::FirmwareVersion version =
      std::atoi(concatVersion.c_str());
  return version;
}
...
void AlexaImpl::SetNetworkConnectionError() {
  if (_internetConnected) {
    // the AVS internet connection check is slow, so let's double check by
    ...
    LOG_INFO("AlexaImpl.SetNetworkConnectionError.IPCheck", "ip addr: '%s'",
             ip.c_str());

    if (!hasIP) {
      // the internet connection check is pretty slow, and we don't have an IP,
      // so we definitely don't have internet. Update the status here
      OnInternetConnectionChanged(hasIP);
    }
  }
  // note that we might have a (local) IP address, but no internet, so don't set
  // us as connected based on the IP check

  // check if it's because of the internet (this flag only gets updated every 5
  // mins, so it gets checked second)
  if (_internetConnected) {
    ...
    LOG_INFO(
        "AlexaImpl.SetNetworkConnectionError.Unconnected.NoInitialConnection",
...
void AlexaImpl::SetNetworkError(AlexaNetworkErrorType errorType) {
  // NOTE: while a network error is a user-facing error, and it _should_ change
  // the AlexaUXState, AlexaImpl is agnostic that that ux state. Instead,
  // AlexaUXState::Error is considered only by the parent, Alexa. This is
  // because we need to play audio for the duration of that state, and AlexaImpl
  // may get destroyed if the error involves authentication.
...
bool AlexaImpl::InteractedRecently() const {
  ...
  const uint64_t currTime =
      duration_cast<milliseconds>(steady_clock::now().time_since_epoch())
          .count();
  const bool recent = (currTime <= kMillisConsideredRecentInteraction +
                                       _lastInteractionTime_ms);
  return recent;
}
...
void AlexaImpl::FailInitialization(const std::string& reason) {
  ...
  DASMSG(alexa_initialization_fail, "alexa.initialization_failed",
         "we failed to fully initialize the sdk");
