 ...
 * Created: Oct 19 2018
 *
 * Description: Observes many SDK components, runs callbacks, and optionally
...
// Since this file an an analog to UIManager.cpp, some portions were copied.
...
#define CONSOLE_LOG(x) \
  LOG_INFO("AlexaObserver.Log", "%s", std::string{x}.c_str())
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaObserver::Init(
    const OnDialogUXStateChangedFunc& onDialogUXStateChanged,
    ...
    const OnSendCompleted& onSendCompleted, const OnLogout& onLogout,
...
void AlexaObserver::Update() {
  // puts callables on the main thread. the sdk uses its Executor to run things
  // sequentially, but it spins up a bunch of threads
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaObserver::onConnectionStatusChanged(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaObserver::onSettingChanged(const std::string& key,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaObserver::onSpeakerSettingsChanged(
  ...
  auto func = [source, type, settings]() {
    ...
    oss << "SOURCE:" << source << " TYPE:" << type
        << " VOLUME:" << static_cast<int>(settings.volume)
        << " MUTE:" << settings.mute;
    CONSOLE_LOG(oss.str());
  };
  AddToQueue(std::move(func));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaObserver::onAlertStateChange(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaObserver::onPlayerActivityChanged(
    alexaClientSDK::avsCommon::avs::PlayerActivity state,
    const alexaClientSDK::avsCommon::sdkInterfaces::
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaObserver::onRequestAuthorization(const std::string& url,
  ...
  auto func = [this, url, code]() {
    ...
    oss << "To authorize, browse to: '" << url
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaObserver::onAuthStateChange(AuthObserverInterface::State newState,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaObserver::onPlaybackError(
    SourceId id, const avsCommon::utils::mediaPlayer::ErrorType& type,
    std::string error) {
  LOG_WARNING("AlexaObserver.onPlaybackError", "Error '%s': %s",
              errorTypeToString(type).c_str(), error.c_str());
  // now would be an ideal time to play "Sorry, music and radio playback are not
  // supported on this device," but we don't have that clip. TODO: obtain this
  // clip
  auto func = [this, id]() {
    if (_onSourcePlaybackChange != nullptr) {
      _onSourcePlaybackChange(id, false);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaObserver::onSendCompleted(
...
void AlexaObserver::onExceptionReceived(const std::string& exceptionMessage) {
  ...
  //         "description": "Unable to authenticate the request. Please provide
  //         a valid authorization token."
  //     }
  //   }

  LOG_WARNING("AlexaObserver.onExceptionReceived", "AVS SDK exception: %s",
  ...
  DASMSG(alexa_exception_msg, "alexa.exception",
         "AVS SDK received an exception");
  ...
  if (_connectionStatus ==
    ...
    switch (_dialogState) {
      ...
      case DialogUXState::SPEAKING:
        ...
        return;
      // This is an intermediate state after a SPEAK directive is completed. In
      // the case of a speech burst the next SPEAK could kick in or if its the
      // last SPEAK directive ALEXA moves to the IDLE state. So we do nothing
      // for this state.
