 ...
 * Created: Nov 4 2018
 *
 * Description: Subclass of the MessageRouter that accepts
 * MessageRequestObserverInterface observers. We need this to receive
 * onSendCompleted, which provides error status on failed attempts to send data
 * to AVS.
 *
 *
...
// NOTE: due to me being a lazy developer, this means it _wont_ get sent up, and
// won't actually work, but you can use this flag to get a dump of the file to
// check wakeword timing. See kDumpAlexaTriggerAudio in alexaImpl.cpp for a
// better implementation. This one is only useful if you want to test the exact
// bytes that would be sent up (but then they don't get sent up....)
CONSOLE_VAR(bool, kStealAlexaWakewordAudio, "Alexa.Messaging", false);

// hack to work around not having a data platform at this level. Note that this
// only works on robot
std::string GetAlexaCacheFolder() {
  ...
  if (alexaCacheFolder.empty()) {
    ...
    if (!alexaCacheFolder.empty() &&
        Util::FileUtils::DirectoryDoesNotExist(alexaCacheFolder)) {
    ...
    LOG_INFO("AlexaMessageRouter.LogMessages",
             "Logging outgoing messages to '%s'",
...
void WriteMessageDebugFile(const std::string& message, bool isSend) {
  ...
  const bool ok = Util::FileUtils::WriteFile(
      alexaCacheFolder + kMessageOutputFile,
      std::to_string(bsTime) + " " + messageType + ": " + messageStr.c_str(),
      append);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
AlexaMessageRouter::AlexaMessageRouter(
    const std::unordered_set<std::shared_ptr<
        avsCommon::sdkInterfaces::MessageRequestObserverInterface>>& observers,
    ...
    : MessageRouter{authDelegate, attachmentManager, transportFactory,
                    avsEndpoint},
      _observers{observers} {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaMessageRouter::sendMessage(
  ...
  if (kLogAlexaMessages) {
    ...
    LOG_DEBUG("AlexaMessageRouter.SendMessage",
              "sending message to AVS (%d attachments)", numAttachments);
      ...
      for (int attachment = 0; attachment < numAttachments; ++attachment) {
        ...
        if (!reader) {
          LOG_ERROR("AlexaMessageRouter.CouldNotGetAttachmentReader",
                    "null attachment reader for attachment id %d", attachment);
          continue;
        }
        ...
        const std::string filename = alexaCacheFolder + "attachment_" +
                                     std::to_string(sAttachmentIdx) + ".pcm";
        ...
        if (fileOut.is_open()) {
          ...
          size_t read = 1;
          while (read > 0) {
            using AVSReadStatus = alexaClientSDK::avsCommon::avs::attachment::
                AttachmentReader::ReadStatus;
          ...
          // TODO:(bn) this doesn't work at all, not sure if there's a better
          // way to do this so for now I'm using a different approach in
          // alexaImpl.cpp to non-destructively save this data

          LOG_INFO(
              "AlexaMessageRouter.WroteAttachment",
              "Wrote attachment id '%d' to file '%s' for reader with name '%s'",
              attachment, filename.c_str(), reader->name.c_str());
        }
      }
    }
  }
...
void AlexaMessageRouter::consumeMessage(const std::string& contextId,
                                        const std::string& message) {
  LOG_DEBUG("AlexaMessageRouter.ConsumeMessage",
            "consuming message cid '%s', message len %zu", contextId.c_str(),
            message.size());
