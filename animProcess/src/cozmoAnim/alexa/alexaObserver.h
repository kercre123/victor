...
// Since this file an an analog to UIManager.h, some portions were copied.
...
class AlexaObserver
    : public alexaClientSDK::avsCommon::sdkInterfaces::
      ...
      public alexaClientSDK::authorization::cblAuthDelegate::
          CBLAuthRequesterInterface,
      public alexaClientSDK::avsCommon::utils::mediaPlayer::
...

{
 public:
  ...
  using OnRequestAuthorizationFunc =
      std::function<void(const std::string&, const std::string&)>;
  using OnAuthStateChangeFunc = std::function<void(
  ...
  using OnSourcePlaybackChange =
      std::function<void(alexaClientSDK::avsCommon::utils::mediaPlayer::
                             MediaPlayerInterface::SourceId,
                         bool playing)>;
  using OnInternetConnectionChanged = std::function<void(bool connected)>;
  using OnAVSConnectionChanged = std::function<void(
  ...
  using OnSendCompleted =
      std::function<void(alexaClientSDK::avsCommon::sdkInterfaces::
                             MessageRequestObserverInterface::Status)>;
  using OnLogout = std::function<void(void)>;
  using OnNotificationIndicator =
      std::function<void(alexaClientSDK::avsCommon::avs::IndicatorState)>;
  using OnAlertState = std::function<void(
      const std::string&,
      alexaClientSDK::capabilityAgents::alerts::AlertObserverInterface::State)>;
  using OnPlayerActivity =
      std::function<void(alexaClientSDK::avsCommon::avs::PlayerActivity)>;
 ...
 protected:
  ...
  virtual void onConnectionStatusChanged(
      const alexaClientSDK::avsCommon::sdkInterfaces::
          ConnectionStatusObserverInterface::Status status,
      const alexaClientSDK::avsCommon::sdkInterfaces::
          ConnectionStatusObserverInterface::ChangedReason reason) override;
  ...
  virtual void onSpeakerSettingsChanged(
      ...
      const alexaClientSDK::avsCommon::sdkInterfaces::SpeakerInterface::Type&
          type,
      const alexaClientSDK::avsCommon::sdkInterfaces::SpeakerInterface::
          SpeakerSettings& settings) override;

  virtual void onRequestAuthorization(const std::string& url,
                                      const std::string& code) override;
  virtual void onCheckingForAuthorization() override;

  virtual void onAuthStateChange(
      alexaClientSDK::avsCommon::sdkInterfaces::AuthObserverInterface::State
          newState,
      alexaClientSDK::avsCommon::sdkInterfaces::AuthObserverInterface::Error
          newError) override;
  ...
  virtual void onSendCompleted(
      alexaClientSDK::avsCommon::sdkInterfaces::
          MessageRequestObserverInterface::Status status) override;
  virtual void onExceptionReceived(
      const std::string& exceptionMessage) override;

  // notifications
  virtual void onSetIndicator(
      alexaClientSDK::avsCommon::avs::IndicatorState state) override;
  ...
  // audio player
  virtual void onPlayerActivityChanged(
      alexaClientSDK::avsCommon::avs::PlayerActivity state,
      const alexaClientSDK::avsCommon::sdkInterfaces::
          AudioPlayerObserverInterface::Context& context) override;
 ...
 private:
  ...
  // Prints the current state of Alexa after checking what the appropriate
  // message to display is based on the current component states. This should
  // only be used within the internal executor.
  void PrintState();
  ...
  alexaClientSDK::avsCommon::sdkInterfaces::CapabilitiesObserverInterface::State
      _capabilitiesState;

  // The error associated with the CapabilitiesDelegate state.
  alexaClientSDK::avsCommon::sdkInterfaces::CapabilitiesObserverInterface::Error
      _capabilitiesError;
  ...
  // Counter used to make repeated messages about checking for authorization
