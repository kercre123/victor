...
class AlexaImpl : private Util::noncopyable {
 public:
  ...
  // Starts an async init thread that when complete runs a callback on the same
  // thread as callers to Update()
  ...
  using OnAlexaAuthStateChanged = std::function<void(
      AlexaAuthState, const std::string&, const std::string&, bool)>;
  void SetOnAlexaAuthStateChanged(const OnAlexaAuthStateChanged& callback) {
    _onAlexaAuthStateChanged = callback;
  }

  // will never call back with ux state Error. see comment in method
  // SetNetworkError.
  using OnAlexaUXStateChanged = std::function<void(AlexaUXState)>;
  void SetOnAlexaUXStateChanged(const OnAlexaUXStateChanged& callback) {
    _onAlexaUXStateChanged = callback;
  }
  ...
  void SetOnNetworkError(const OnNetworkError& callback) {
    _onNetworkError = callback;
  }

  using OnNotificationsChanged = std::function<void(bool hasNotification)>;
  void SetOnNotificationsChanged(const OnNotificationsChanged& callback) {
    _onNotificationsChanged = callback;
  }

  // Removes all callbacks passed above, and also stops internally processing
  // callbacks from the sdk in prep for being destroyed
  ...
  using SourceId = uint64_t;  // matches SDK's MediaPlayerInterface::SourceId,
                              // static asserted in cpp

 private:
  using DialogUXState = alexaClientSDK::avsCommon::sdkInterfaces::
  ...
  void OnSendComplete(alexaClientSDK::avsCommon::sdkInterfaces::
                          MessageRequestObserverInterface::Status status);
  void OnSDKLogout();
  void OnNotificationsIndicator(
      alexaClientSDK::avsCommon::avs::IndicatorState state);
  void OnAlertState(
      const std::string& alertID,
      alexaClientSDK::capabilityAgents::alerts::AlertObserverInterface::State
          state);
  void OnPlayerActivity(alexaClientSDK::avsCommon::avs::PlayerActivity state);

  // call every tick from update, occasionally this will perform some checks to
  // see if it looks like we are stuck in a UX state bug (e.g. "forever face")
  ...
  // if the sdk has been able to ping a specific amazon.com endpoint (not the
  // AVS endpoint)
  ...
  // if non-negative, the update loop with automatically set the ux state to
  // Idle at this time
  ...
  // hack to check if time is synced. As of this moment,
  // OSState::IsWallTimeSynced() is not reliable and fast on vicos.... so just
  // track if the system clock jumps and if so, refresh the timers
  ...
  std::shared_ptr<alexaClientSDK::capabilitiesDelegate::CapabilitiesDelegate>
      _capabilitiesDelegate;

  std::shared_ptr<alexaClientSDK::settings::SettingCallbacks<
      alexaClientSDK::settings::DeviceSettingsManager>>
      _settingsCallbacks;
  ...
  std::shared_ptr<alexaClientSDK::capabilityAgents::aip::AudioProvider>
  ...
  // copy that can be used to dump audio to file. Only used if a console var is
  // set
  ...
  // handles state of the loading thread. some of this could be done with
  // futures but meh
