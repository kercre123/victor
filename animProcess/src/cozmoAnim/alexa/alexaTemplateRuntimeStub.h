 ...
 * Created: 2018-11-13
 *
 * Description: A DEV-ONLY wrapper for template rendering, for debugging and
 *testing what kinds of data we get if we had a screen to render Alexa content
 *on. Results only show up in the directives file for now
 *
...
#if ANKI_DEV_CHEATS
// disable this entire class if dev cheats aren't on (don't want to ship it by
// accident)
...
class AlexaTemplateRuntimeStub
 ...
 private:
  ...
  const alexaClientSDK::avsCommon::avs::NamespaceAndName TEMPLATE{
      NAMESPACE, RENDER_TEMPLATE};
  const alexaClientSDK::avsCommon::avs::NamespaceAndName PLAYER_INFO{
      NAMESPACE, RENDER_PLAYER_INFO};
  const std::string TEMPLATERUNTIME_CAPABILITY_INTERFACE_TYPE =
      "AlexaInterface";
  const std::string TEMPLATERUNTIME_CAPABILITY_INTERFACE_NAME =
      "TemplateRuntime";
  const std::string TEMPLATERUNTIME_CAPABILITY_INTERFACE_VERSION = "1.1";

 public:
  explicit AlexaTemplateRuntimeStub(
  ...
  getConfiguration() const override {
    ...
    auto visualNonBlockingPolicy =
        alexaClientSDK::avsCommon::avs::BlockingPolicy(
            alexaClientSDK::avsCommon::avs::BlockingPolicy::MEDIUM_VISUAL,
            false);
