...
class AlexaPlaybackRecognizerComponent {
 ...
 private:
  ...
  static constexpr size_t kRecognizerBufferSize =
      160;  // 10 ms of single channel audio

  // Buffer constants
  // Add extra buffer for overflow
  static constexpr uint32_t kAudioEngOutBufferSize =
      SinkPluginTypes::kAkAlsaSinkBufferCount + 1;
  // Need lowest common denominator of 512 (resample buffer size) and 160 (10 ms
  // of data for recognizer) We always want to use continuous memory when
  // writing or reading
  static constexpr size_t kResamplerCircularBufferSize =
      kResamplerBufferSize * 5;

  // Handle to objects
  const Anim::AnimContext* _context =
      nullptr;  // This a pointer to a unique_ptr()'s instance
  SpeechRecognizerPryonLite* _recognizer =
      nullptr;  // This a pointer to a unique_ptr()'s instance
  ...
  CircularBuffer<SinkPluginTypes::AudioChunk, kAudioEngOutBufferSize>
      _audioEngOutBuffer;
  CircularBuffer<SinkPluginTypes::AudioChunk*, kAudioEngOutBufferSize>
      _validOutBufferPtrs;
  CircularBuffer<SinkPluginTypes::AkSinkAudioSample_t,
                 kResamplerCircularBufferSize>
      _resampledAudioBuffer;
  ...
  void SinkPluginCallback(const SinkPluginTypes::AudioChunk& chunk,
                          bool hasData);
