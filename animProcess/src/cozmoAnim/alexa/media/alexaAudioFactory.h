...
// the main factory actually contains multiple factories for different types of
// audio. For now we are just customizing Alerts, but if we wanted to customize
// the others we could do so here
...
class AlexaAudioFactory : public AlexaSDKAudio::AudioFactoryInterface {
 public:
  ...
  std::shared_ptr<AlexaSDKAudio::AlertsAudioFactoryInterface> alerts()
      const override;
 ...
 private:
  std::shared_ptr<
      alexaClientSDK::applicationUtilities::resources::audio::AudioFactory>
      _defaultFactory;
...
class AlexaCustomAlertsAudioFactory
    : public AlexaSDKAudio::AlertsAudioFactoryInterface {
 public:
