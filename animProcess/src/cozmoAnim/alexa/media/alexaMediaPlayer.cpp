...
/***************************************************************************************************
NOTE:  This is shitty prototype code. I'm putting it in master so that Alexa
will start speaking sooner.
TODO (VIC-9853): re-implement this properly. I think it should more closely
resemble MediaPlayer.h/cpp, and there should not be a thread::sleep
*************************************************************************************************/
...
using SourceId = alexaClientSDK::avsCommon::utils::mediaPlayer::
    MediaPlayerInterface::SourceId;
std::atomic<SourceId> AlexaMediaPlayer::_nextAvailableSourceID{
    1};  // 0 is invalid

using PluginId_t =
    Anki::AudioEngine::PlugIns::StreamingWavePortalPlugIn::PluginId_t;
...
// assume the input stream is an unsupported format and abort streaming after
// this many bytes are processed. this is arbitrary based on how much header
// info you'd actually expect in an mp3. Sometimes sources may provide large
// headers with their copyright blurbs, but probably not even this much. Note
// that if the stream ends prior to downloading this many bytes, we already exit
// cleanly.
...
const std::unordered_map<AlexaMediaPlayer::Type, AudioInfo> sAudioInfo{
    {AlexaMediaPlayer::Type::TTS,
     {AudioEngine::ToAudioGameObject(AudioMetaData::GameObjectType::AlexaVoice),
      kPluginIdTTS,
      alexaClientSDK::avsCommon::sdkInterfaces::SpeakerInterface::Type::
          AVS_SPEAKER_VOLUME,
      "TTS",
      AudioMetaData::GameEvent::GenericEvent::
    ...
    {AlexaMediaPlayer::Type::Alerts,
     {AudioEngine::ToAudioGameObject(
          AudioMetaData::GameObjectType::AlexaAlerts),
      kPluginIdAlerts,
      alexaClientSDK::avsCommon::sdkInterfaces::SpeakerInterface::Type::
          AVS_ALERTS_VOLUME,
      "Alerts",
    ...
    {AlexaMediaPlayer::Type::Notifications,
     {AudioEngine::ToAudioGameObject(
          AudioMetaData::GameObjectType::AlexaNotifications),
...
#define LOG(x, ...)                                                           \
  LOG_INFO("AlexaMediaPlayer.SpeakerInfo", "%s: " x, _audioInfo.name.c_str(), \
           ##__VA_ARGS__)
#if ANKI_DEV_CHEATS
...
AlexaMediaPlayer::AlexaMediaPlayer(
    ...
    : avsCommon::utils::RequiresShutdown{"AlexaMediaPlayer_" +
                                         sAudioInfo.at(type).name},
      _type(type),
      _state(State::Idle),
      _stateBeforePause(State::Idle),
      _playableClip(AudioEngine::kInvalidAudioEventId),
      _mp3Buffer(new Util::RingBuffContiguousRead<uint8_t>{kAudioBufferSize,
                                                           kMaxReadSize}),
      _dataValidity{DataValidity::Unknown},
      _dispatchQueue(Util::Dispatch::Create(
          ("APlayer_" + sAudioInfo.at(type).name).c_str())),
...
void AlexaMediaPlayer::Init(const Anim::AnimContext* context) {
  ...
  DEV_ASSERT(nullptr != context->GetAudioController(),
             "AudioMediaPlayer.InvalidAudioController");

  _audioController = context->GetAudioController();
  DEV_ASSERT_MSG(_audioController != nullptr,
                 "AudioMediaPlayer.Init.InvalidAudioController", "");

  mpg123_init();
  int initRet;
  _decoderHandle = mpg123_new(NULL, &initRet);
  if (_decoderHandle == nullptr) {
    LOG_ERROR("AlexaMediaPlayer.Init.DecoderFail",
              "Unable to create mpg123 handle: %s",
              mpg123_plain_strerror(initRet));
    return;
  }
  ...
  // Note: call mpg123_param( _decoderHandle, MPG123_VERBOSE, 2, 0) to enable
  // verbose logging
  ...
  if (platform != nullptr) {
    _cacheSaveFolder =
        platform->pathToResource(Util::Data::Scope::Cache, "alexa");
    _persistentSaveFolder =
        platform->pathToResource(Util::Data::Scope::Persistent, "alexa");

    _cacheSaveFolder =
        Util::FileUtils::AddTrailingFileSeparator(_cacheSaveFolder);
    _persistentSaveFolder =
        Util::FileUtils::AddTrailingFileSeparator(_cacheSaveFolder);

    if (!_cacheSaveFolder.empty() &&
        Util::FileUtils::DirectoryDoesNotExist(_cacheSaveFolder)) {
      Util::FileUtils::CreateDirectory(_cacheSaveFolder);
    }
    DEV_ASSERT(Util::FileUtils::DirectoryExists(_persistentSaveFolder),
               "AudioMediaPlayer.Init.NoPersistentSaveFolder");

#if ANKI_DEV_CHEATS
    for (int i = 1; i < 100; ++i) {
      std::string ss = _cacheSaveFolder + "speaker_" + _audioInfo.name +
                       std::to_string(i) + ".mp3";
      if (Util::FileUtils::FileExists(ss)) {
        Util::FileUtils::DeleteFile(ss);
        Util::FileUtils::DeleteFile(_cacheSaveFolder + "speaker_" +
                                    _audioInfo.name + std::to_string(i) +
                                    ".pcm");
      }
    }
#endif
  ...
  _audioPlaybackFinishedPtr = std::make_shared<AudioCallbackType>(
      [this](SourceId id) { CallOnPlaybackFinished(id); });
  _audioPlaybackErrorPtr = std::make_shared<AudioCallbackType>(
      [this](SourceId id) { CallOnPlaybackError(id); });
}
...
void AlexaMediaPlayer::Update() {
  ...
  if (_state == State::Playable) {
    ...
    callbackContext->SetEventCallbackFunc(
        [weakOnFinished, id = _playingSource](
            const AudioCallbackContext* thisContext,
            const AudioCallbackInfo& callbackInfo) {
          // if the user logs out, this callback could fire after
          // AlexaMediaPlayer is destoryed. check that here.
    ...
    const auto playingID = _audioController->PostAudioEvent(
        eventID, _audioInfo.gameObject, callbackContext);
    if (AudioEngine::kInvalidAudioPlayingId == playingID) {
      LOG_ERROR("AlexaMediaPlayer.Update.CouldNotPlay",
                "Speaker '%s' could not play", _audioInfo.name.c_str());
    }
    ...
    if (ANKI_VERIFY(_playableClip != AudioEngine::kInvalidAudioEventId,
      ...
      callbackContext->SetCallbackFlags(AudioCallbackFlag::Complete);
      callbackContext->SetExecuteAsync(false);  // run on main thread
      std::weak_ptr<AudioCallbackType> weakOnFinished =
          _audioPlaybackFinishedPtr;
      ...
      callbackContext->SetEventCallbackFunc(
          [weakOnFinished, weakOnError, id = _playingSource, invalidData](
              const AudioCallbackContext* thisContext,
              const AudioCallbackInfo& callbackInfo) {
            // if the user logs out, this event callback could fire after
            // AlexaMediaPlayer is destroyed. check that here.
            if (invalidData) {
            ...
            // not sure if weakOnFinished (CallOnPlaybackFinished) needs to be
            // called if weakOnError was also called
            auto callback = weakOnFinished.lock();
            if (callback && *callback) {
              (*callback)(id);
            }
      ...
      const auto playingID = _audioController->PostAudioEvent(
          _playableClip, _audioInfo.gameObject, callbackContext);
      if (AudioEngine::kInvalidAudioPlayingId == playingID) {
      ...
      _clipStartTime_ms =
          BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() / 1000.0f;
    } else {
...
void AlexaMediaPlayer::CallOnPlaybackFinished(SourceId id, bool runOnCaller) {
  auto func = [this, id]() {
    if (_playingSource == id) {
      ...
      LOG("CallOnPlaybackFinished %llu, but %llu is playing, so keep state as "
          "%s",
          id, _playingSource, StateToString());
      LogPlayingSourceMismatchEvent("CallOnPlaybackFinished", id,
                                    _playingSource);
    }
...
void AlexaMediaPlayer::CallOnPlaybackError(SourceId id) {
  _executor.submit([this, id]() {
    if (_playingSource == id) {
      ...
      LOG("CallOnPlaybackError %llu, but source %llu is playing, so keep state "
          "as %s",
          id, _playingSource, StateToString());
      LogPlayingSourceMismatchEvent("CallOnPlaybackError", id, _playingSource);
    }
    static const std::string kPlaybackError =
        "The device had trouble with playback";
    std::lock_guard<std::recursive_mutex> lg{_observerMutex};
    for (auto& observer : _observers) {
      observer->onPlaybackError(id,
                                avsCommon::utils::mediaPlayer::ErrorType::
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
SourceId AlexaMediaPlayer::GetNewSourceID() { return _nextAvailableSourceID++; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
SourceId AlexaMediaPlayer::setSource(
    std::shared_ptr<avsCommon::avs::attachment::AttachmentReader>
        attachmentReader,
    const avsCommon::utils::AudioFormat* format) {
  const SourceId sourceID = GetNewSourceID();
  LOG("set source %llu with attachment reader (%zu previous readers)", sourceID,
      _readers.size());
  ...
  if (format != nullptr) {
    std::stringstream ss;
    ss << "encoding=" << format->encoding
       << ", endianness=" << format->endianness;
    LOG("format: %s, sampleRateHz=%u, sampleSize=%u, numChannels=%u, "
        "dataSigned=%d, interleaved=%d",
        ss.str().c_str(), format->sampleRateHz, format->sampleSizeInBits,
        format->numChannels, format->dataSigned,
        format->layout == AudioFormat::Layout::INTERLEAVED);
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
SourceId AlexaMediaPlayer::setSource(const std::string& url,
  ...
  // Note: calling UrlContentToAttachmentConverter::create() can lead to an SDK
  // thread to call this class's onError(), so clear the relevant flag here
  // instead of at the beginning of the play loop
  _errorDownloading = false;
  _urlConverter = playlistParser::UrlContentToAttachmentConverter::create(
      _contentFetcherFactory, url, shared_from_this(), offset);
  if (!_urlConverter) {
    return 0;
  }
  ...
  std::shared_ptr<avsCommon::avs::attachment::AttachmentReader> reader =
      attachment->createReader(
          avsCommon::utils::sds::ReaderPolicy::NONBLOCKING);
  if (!reader) {
    return 0;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
SourceId AlexaMediaPlayer::setSource(std::shared_ptr<std::istream> stream,
                                     bool repeat) {
  const SourceId sourceID = GetNewSourceID();
  LOG("set source %llu with stream, repeat %s (%zu previous readers)", sourceID,
      repeat ? "true" : "false", _readers.size());
...
void AlexaMediaPlayer::OnNewSourceSet() {
  // setting a new source, so if we aren't currently idle then we need to stop
  // the previous source
  const State state = _state;
  if (state != State::Idle) {
    LOG("Stop playing source %llu from state %s because new source is set",
        _playingSource, StateToString(state));

    DASMSG(alexa_multi_source, "alexa.stop_previous_media",
           "The AVS SDK told us to play another piece of media without "
           "stopping the last (this is a health / debugging issue)");
...
bool AlexaMediaPlayer::play(SourceId id) {
  ...
  Util::Dispatch::Async(_dispatchQueue, [id, this]() {
    ...
    for (int tries = 0;
         tries < kMaxPlayTries && !_shuttingDown && id == _nextSourceToPlay;
         ++tries) {
      if (ExchangeState(State::Idle, State::Preparing)) {
        ...
        lk.unlock();
        LOG("play: state is not idle, sleeping to let it go idle (try %d)",
            tries);
        // TODO:(bn) condition variable or something, this doesn't seem to
        // happen often though
    ...
    if (_state != State::Preparing || _shuttingDown ||
        id != _nextSourceToPlay) {
      LOG("Failed to set state to preparing, state is %s, giving up on playing "
          "source %llu",
          StateToString(), id);
      return;
    }

    // at this point, we hold the play loop mutex, and we are the _only_ playing
    // async job for _this_ media player
    ...
    // We don't want to run this on the same core as the Audio Engine to reduce
    // stuttering
    cpu_set_t cpu_set;
    CPU_ZERO(&cpu_set);
    CPU_SET(0, &cpu_set);
    ...
    if (_decoderHandle == nullptr) {
      LOG_ERROR("AlexaMediaPlayer.Init.DecoderFeedFail",
                "Unable to reset decoder feed");
      return;
    }

    // new waveData instance to hold data passed to wwise
    _waveData =
        AudioEngine::PlugIns::StreamingWavePortalPlugIn::CreateDataInstance();

    // reset plugin
    {
      auto* pluginInterface = _audioController->GetPluginInterface();
      DEV_ASSERT(nullptr != pluginInterface,
                 "AlexaMediaPlayer.play.InvalidPluginInterface");
    ...
    // buffer to read mp3 data from source. this is an unnecessary copy if
    // RingBuffContiguousRead had a raw writable pointer
    std::array<uint8_t, kDownloadSize> input;

    int numBlocks = 0;
    const int kMaxBlocks = 10000;  // number of times we can try to check a
                                   // blocking reader before giving up
    ...
    while (stateOk() && !_shuttingDown) {
      ...
      while (!_shuttingDown) {
        const size_t spaceForWrite =
            _mp3Buffer->Capacity() - _mp3Buffer->Size();
        const auto numUnreadInStream = reader->GetNumUnreadBytes();
        // it's possible that size==0 if the download hasn't started. ignore
        // size for now, and try reading below, and use the readStatus of that
        // to determine what to do
        const size_t toReadFromStream =
            (numUnreadInStream == 0)
                ? input.size()
                : std::min(numUnreadInStream, input.size());
        if (spaceForWrite < toReadFromStream) {
          break;
        }
        const auto bytesRead =
            reader->Read(input.data(), toReadFromStream, readStatus);
        if (bytesRead > 0) {
          // this won't add if there aren't bytesRead available space, hence the
          // compairson to spaceForWrite above
          _mp3Buffer->AddData(input.data(), (int)bytesRead);
        }
        if (readStatus != AlexaReader::Status::Ok) {
          // LOG( "source %llu readStatus %s", id,
          // AlexaReader::StatusToString(readStatus) );
          break;
        }
      }

      statusOK = (readStatus == AlexaReader::Status::Ok) ||
                 (readStatus == AlexaReader::Status::WouldBlock);
      ...
      // decode everything in the buffer thus far, leaving only something that
      // perhaps wasnt a full mp3 frame
      const int timeDecoded_ms = Decode(_waveData, flush);
      _offset_ms += timeDecoded_ms;

      // sleep to avoid downloading data faster than it is played. todo: not
      // this...
      if (!flush && timeDecoded_ms > 0 && !_shuttingDown) {
        const auto samplesInBuffer = _waveData->GetNumberOfFramesReceived() -
                                     _waveData->GetNumberOfFramesPlayed();
        size_t sleepFor_ms;
        if (samplesInBuffer > _idealBufferSampleSize) {
          // Sleep until we have about _idealBufferSampleSize of samples left in
          // buffer
          sleepFor_ms = ((samplesInBuffer - _idealBufferSampleSize) * 1000) /
                        _waveData->GetSampleRate();
          ...
          sleepFor_ms =
              Util::Clamp(timeDecoded_ms, 25,
                          static_cast<const int>(timeDecoded_ms * 0.5f));
        }
      ...
      if (_dataValidity == DataValidity::Unknown) {
        if (_attemptedDecodeBytes > kMaxInvalidBytesBeforeAbort) {
          LOG_WARNING("AlexaMediaPlayer.Play.InvalidData.DecodeError",
                      "%s: source %llu Attempted to decode %zu bytes (> limit "
                      "of %zu), setting data invalid",
                      _audioInfo.name.c_str(), id, _attemptedDecodeBytes,
          ...
          LOG_WARNING("AlexaMediaPlayer.Play.InvalidData.FrameError",
                      "%s: source %llu Attempted to decode %d frames (> limit "
                      "of %zu), setting data invalid",
                      _audioInfo.name.c_str(), id, _invalidFrames,
      ...
      if (flush || invalidData || _shuttingDown || _errorDownloading) {
        LOG("done producing data on waveData %p for source %llu (flush:%d "
            "invalid:%d 404:%d)",
            _waveData.get(), id, flush, invalidData, (int)_errorDownloading);
    ...
    if (!invalidData && (_state == State::Preparing) &&
        (_dataValidity == DataValidity::Valid)) {
      // if the above loop exits and the decoder has prepared valid data (it
      // decoded at least 1 sample), then switch the state to Playable. normally
      // this happens when enough samples have been decoded, but sometimes
      // downloading finishes before that
      ExchangeState(State::Preparing, State::Playable);
    } else if (invalidData) {
      // if the decoder loop was aborted because of invalid data, switch to
      // playing a clip instead. this play-thread method will end, and Update()
      // will check the new state (ClipPlayable) and switch to that. we should
      // only be in State::Preparing at this stage, since data validity is tied
      // to not being able to decode a single byte
      ASSERT_NAMED_EVENT(_state == State::Preparing,
                         "AlexaMediaPlayer.play.InvalidButNotPreparing",
                         "state='%s'", StateToString());
      _dataValidity = DataValidity::Invalid;
      using namespace AudioEngine;
      using GenericEvent = AudioMetaData::GameEvent::GenericEvent;
      _playableClip = ToAudioEventId(
          GenericEvent::
              Play__Robot_Vic_Alexa__Avs_System_Prompt_Error_Cannot_Play_Song);
      SetState(State::ClipPlayable);
      // to be determined is whether the sdk will do something unexpected if we
      // close the readers below when switching to playing a clip.
      if (_errorDownloading) {
        DASMSG(alexa_unavailable_audio, "alexa.unavailable_audio",
               "404 error when trying to download audio");
        DASMSG_SEND();
      } else {
        DASMSG(alexa_unsupported_audio_format, "alexa.unsupported_audio_format",
...
bool AlexaMediaPlayer::StopInternal(SourceId id, bool runOnCaller) {
  ...
  auto onStop = [id, this]() {
    {
      // first attempt to grab the playing lock to avoid race conditions with
      // the ::play Async call
      std::unique_lock<std::mutex> lk{_playLoopMutex, std::try_to_lock};

      // if we got the lock, then the play thread isn't running. If not, then it
      // _is_ running with the lock, which means that it should stop on it's own

      // if we are playing or are about to play the given source, stop it
      if (_playingSource == id || _nextSourceToPlay == id) {
        SetState(State::Idle);
        const bool stoppingPlaying =
            (_playingSource == id) && (_nextSourceToPlay <= id);
        const bool stoppingBeforePlay =
            (id > _playingSource) && (_nextSourceToPlay == id);
        if (stoppingBeforePlay || stoppingPlaying) {
          _nextSourceToPlay = 0;
        }
        _offset_ms = 0;
      } else {
        LOG("AlexaMediaPlayer.StopInternal.OnStop.OtherSourcePlaying Stopping "
            "source %llu, but %llu is playing, so leaving state as '%s'",
            id, _playingSource, StateToString());
        LogPlayingSourceMismatchEvent("StopInternal", id, _playingSource);
      }

      // let the play mutex guard go out of scope here. NOTE: we may not
      // actually hold it since we used a try_to_lock
    }
...
bool AlexaMediaPlayer::pause(SourceId id) {
  if (_playingSource != id && _nextSourceToPlay != id) {
    LOG("pause source %llu FAIL, _playingSource %llu, _nextSourceToPlay %llu",
        id, _playingSource, _nextSourceToPlay);
  ...
  _executor.submit([id, this]() {
    // it's possible this could run a few iterations if the play thread is
    // changing the state, but should be exceedingly rare
    for (int tries = 0; tries < kMaxPlayTries; ++tries) {
      const State state = _state;
      _stateBeforePause = state;
      if (ExchangeState(_stateBeforePause, State::Paused)) {
        // exchange success!
        LOG("pause source %llu from state '%s'", id,
            StateToString(_stateBeforePause));
        break;
      } else {
#if ANKI_DEV_CHEATS
        LOG("pause source %llu try %d failed, _stateBeforePause %s, _state %s",
            id, tries, StateToString(_stateBeforePause), StateToString(_state));
#endif
      }
    }
...
bool AlexaMediaPlayer::resume(SourceId id) {
  if (_playingSource != id) {
    LOG("resume source %llu FAIL because playing source is %llu", id,
        _playingSource);
  ...
  if (_state != State::Paused) {
    LOG("resume source %llu FAIL because _state '%s' != State::Paused", id,
        StateToString());
    return false;
  }
  ...
  _executor.submit([id, this]() {
    ...
    if (!exchangeOK) {
      LOG("resume source %llu ERROR, state should have been paused, now is %s",
          id, StateToString());
      return;
    }
...
std::chrono::milliseconds AlexaMediaPlayer::getOffset(SourceId id) {
  if (id == _playingSource) {
    ...
    LOG_ERROR("AlexaMediaPlayer.GetOffset.SourceMismatch",
              "requesting source for id %llu, but last playing is %llu", id,
              _playingSource);
...
uint64_t AlexaMediaPlayer::getNumBytesBuffered() {
  ...
  // This should return "the number of bytes queued up in the media player
  // buffers." _attemptedDecodeBytes is the number of bytes downloaded, but we
  // still need the number of bytes played (likely bytes actually played, not
  // just sent to wwise). I have no idea how the return value of this method is
  // used.
  return 0;
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaMediaPlayer::setObserver(
    std::shared_ptr<avsCommon::utils::mediaPlayer::MediaPlayerObserverInterface>
        playerObserver) {
  std::lock_guard<std::recursive_mutex> lg{_observerMutex};
  if (playerObserver == nullptr) {
    // this seems to happen _after_ we doShutDown of some media players. Ideally
    // we would only remove the observer that the caller originally added, but
    // that's not the API.
    _observers.clear();
  } else {
...
void AlexaMediaPlayer::SetPlayerVolume() const {
  const float playerVolume =
      _settings.mute ? 0.0f : (_settings.volume / 100.0f);

  const auto parameterId =
      AudioEngine::ToAudioParameterId(_audioInfo.volumeParameter);
  const auto parameterValue = AudioEngine::ToAudioRTPCValue(playerVolume);
  _audioController->SetParameter(parameterId, parameterValue,
...
int AlexaMediaPlayer::Decode(const StreamingWaveDataPtr& data, bool flush) {
  ...
  // it may also make sense to exit this loop if not flush and Size() <
  // kMaxReadSize
  size_t buffSize =
      _mp3Buffer->GetContiguousSize();  // guaranteed to be at least min{Size(),
                                        // kMaxReadSize}
  ...
  // mpg123 keeps its own buffer, so even if it needs more data, it will hold on
  // to a partial frame, and we can advance the read cursor here
  bool success = _mp3Buffer->AdvanceCursor((int)buffSize);
  if (!success) {
    LOG_ERROR("AlexaMediaPlayer.Decode.ReadPtr",
              "Could not move read pointer by %zu (size=%zu)", buffSize,
              _mp3Buffer->Size());
  }
  ...
  if (decodeRes == MPG123_ERR) {
    LOG_ERROR("AlexaMediaPlayer.Decode.FeedError", "decoder error: %s",
              mpg123_strerror(_decoderHandle));
  } else if (decodeRes == MPG123_NEED_MORE) {
    return 0;
  }

  decodeRes = mpg123_framebyframe_next(_decoderHandle);
  while (!_shuttingDown && (decodeRes != MPG123_ERR) &&
         (decodeRes != MPG123_NEED_MORE)) {
    if (decodeRes == MPG123_NEW_FORMAT) {
      int enc = 0;
      mpg123_getformat(_decoderHandle, &_mediaInfo.sampleRate,
                       &_mediaInfo.channels, &enc);

      _idealBufferSampleSize =
          static_cast<size_t>(_mediaInfo.sampleRate * kIdealBufferSize_sec);
      _minPlaybackBufferSize =
          static_cast<size_t>(_mediaInfo.sampleRate * kMinBufferSize_sec);
      LOG("DECODING: channels=%d, hz=%ld, encoding=%d, "
          "_minPlaybackBufferSize=%zu, _idealBufferSampleSize=%zu",
          _mediaInfo.channels, _mediaInfo.sampleRate, enc,
    ...
    if (decodeRes == MPG123_OK) {
      ...
      if (header != 0) {
        // this decoder does a poor job at differentiating between MP3 and AAC.
        // Check the "reserved" fields
      ...
      if (validHeader && (bodySize > 0) && (body != nullptr)) {
        ...
        size_t frameBytes = 0;
        decodeRes = mpg123_framebyframe_decode(_decoderHandle, &offset,
                                               &decodedAudio, &frameBytes);
        if (decodedSize + frameBytes >= outBuffSize) {
          // there's no room in our output buffer. pass it on and start over
          if (decodedSize > 0) {
            DEV_ASSERT((decodedSize & 1) == 0,
                       "Should have decoded full shorts");
            const size_t shortBytes =
                decodedSize /
                sizeof(short);  // decoder uses bytes, we use shorts
            decoded_ms += CopyToWaveData(shortBytes, data, flush);
          }
          decodedSize = 0;
        }
        // there should be space now
        DEV_ASSERT(decodedSize + frameBytes < outBuffSize,
                   "AlexaMediaPlayer.Decode.NoOutputSpace");
        std::copy(decodedAudio, decodedAudio + frameBytes,
                  outBuff + decodedSize);
        decodedSize += frameBytes;
      }

    } else if (decodeRes == MPG123_ERR) {
      LOG_ERROR("AlexaMediaPlayer.Decode.DataError", "decoder error: %s",
                mpg123_strerror(_decoderHandle));
    }

    decodeRes = mpg123_framebyframe_next(_decoderHandle);
    if (decodeRes == MPG123_ERR) {
      LOG_ERROR("AlexaMediaPlayer.Decode.NextError", "decoder error: %s",
                mpg123_strerror(_decoderHandle));
    }
  }
  ...
  if (decodedSize > 0 || flush) {
    ...
    const size_t shortBytes =
        decodedSize / sizeof(short);  // decoder uses bytes, we use shorts
    decoded_ms += CopyToWaveData(shortBytes, data, flush);
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
float AlexaMediaPlayer::CopyToWaveData(size_t samples,
                                       const StreamingWaveDataPtr& waveData,
                                       bool flush) {
  using namespace AudioEngine;
  ...
  if (channels == 2) {
    // audio engine only supports mono. loop through samples and rewrite in the
    // same buffer
    int j = 0;
  ...
  StandardWaveDataContainer waveContainer{
      static_cast<uint32_t>(_mediaInfo.sampleRate), channels, samples};
  ...
  if ((_state == State::Preparing) &&
      ((numFrames >= _minPlaybackBufferSize) || flush)) {
    // LOG("Now Playable, numFrames %d minFrames %zu flush %d", numFrames,
    // _minPlaybackBufferSize, flush );
    ExchangeState(State::Preparing, State::Playable);
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
const char* const AlexaMediaPlayer::StateToString(
    const AlexaMediaPlayer::State& state) {
  switch (state) {
    case State::Idle:
      return "Idle";
    case State::Preparing:
...
void AlexaMediaPlayer::SavePCM(short* buff, size_t size) const {
  ...
  static int pcmfd = -1;
  if (pcmfd < 0) {
    const auto path = _cacheSaveFolder + "speaker_" + _audioInfo.name +
                      std::to_string(_playingSource) + ".pcm";
...
void AlexaMediaPlayer::SaveMP3(const unsigned char* buff, size_t size) const {
  ...
  if (mp3fd < 0) {
    const auto path = _cacheSaveFolder + "speaker_" + _audioInfo.name +
                      std::to_string(_playingSource) + ".mp3";
...
std::string AlexaMediaPlayer::GetSettingsFilename() const {
  return _persistentSaveFolder + "speaker_" + _audioInfo.name +
         "_settings.json";
}
...
void AlexaMediaPlayer::SaveSettings() const {
  if (!_persistentSaveFolder.empty()) {
    ...
    LOG("saved settings: volume [%d], mute [%d]", _settings.volume,
        _settings.mute);
  }
}
...
void AlexaMediaPlayer::LoadSettings() {
  if (!_persistentSaveFolder.empty()) {
    ...
    if (!fileContents.empty() && reader.parse(fileContents, metadata)) {
      ...
      LOG("loaded settings: volume [%d], mute [%d]", _settings.volume,
          _settings.mute);
    }
  }
}
...
void AlexaMediaPlayer::onError() {
  // Usually this means the _urlConverter experienced a 404 error when trying to
  // make an attachment reader
  LOG("An error occurred in the streaming of content");

  // Instead of dealing the the play/stop thread loops, just set a flag that is
  // handled in the play loop. The API here unfortunately doesn't tell us what
  // sourceID we're dealing with, so we don't know whether the error is with the
  // current play loop or another. We also don't know if the play loop will have
  // started before or after this flag is set. Set the flag and hope for the
  // best. TODO: delete this whole class
...
void AlexaMediaPlayer::doShutdown() {
  _shuttingDown = true;
  // wait for the DispatchQueue play loop to complete. The thread is still
  // running, but is cleaned up during destruction
  std::unique_lock<std::mutex> lk{_playLoopMutex};
  ...
  if (_state != State::Idle) {
    // if something is playing, notify observers of a stop, otherwise there ends
    // up being a circular ref fiasco among shared_ptrs in AlexaClient
  ...
  // _observers must be cleared here, otherwise there ends up being a circular
  // ref fiasco among shared_ptrs in AlexaClient and AlexaImpl, even if nothing
  // is playing. But since _observers is referenced in the _executor, first stop
  // _executor, then clear _observers.
  _executor.shutdown();
  {
    std::lock_guard<std::recursive_mutex> lg{_observerMutex};
    _observers.clear();
...
// - - - - - - - - - - - - - - - - - - - -
alexaClientSDK::avsCommon::sdkInterfaces::SpeakerInterface::Type
AlexaMediaPlayer::getSpeakerType() {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool AlexaMediaPlayer::ExchangeState(State expectedCurrentState,
                                     State desiredState) {
  const bool exchanged =
      _state.compare_exchange_strong(expectedCurrentState, desiredState);
  if (exchanged) {
    LOG("State: %s (exchange success from %s)", StateToString(),
        StateToString(expectedCurrentState));
  } else {
    LOG("State remains %s because exchange failed (desired was %s)!",
        StateToString(), StateToString(desiredState));
  }
  return exchanged;
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaMediaPlayer::LogPlayingSourceMismatchEvent(const std::string& func,
                                                     SourceId id,
                                                     SourceId playingId) {
  int sourceDelta = (int)id - (int)playingId;

  DASMSG(source_mismatch_event, "alexa.source_mismatch",
         "Media player source mismatch. This is a health / debugging event "
         "that is expected to happen sometimes");
  DASMSG_SET(s1, _audioInfo.name, "Audio speaker channel (e.g. TTS, Alerts)");
  DASMSG_SET(s2, func, "Calling funciton / identifier");
  DASMSG_SET(s3, StateToString(), "Current audio processing state");
  DASMSG_SET(i1, sourceDelta,
             "Delta between source being operated on and the playing source");
  DASMSG_SEND();
}
...
bool AlexaMediaPlayer::IsActive() {
  ...
  // now to detect if we _might become_ active, first try to grab the playing
  // lock
  std::unique_lock<std::mutex> lk{_playLoopMutex, std::try_to_lock};
