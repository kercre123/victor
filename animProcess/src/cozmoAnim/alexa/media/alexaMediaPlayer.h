...
class AlexaMediaPlayer
    : public alexaClientSDK::avsCommon::utils::mediaPlayer::
      ...
      public alexaClientSDK::playlistParser::UrlContentToAttachmentConverter::
 ...
 public:
  enum class Type : uint8_t { TTS, Alerts, Audio, Notifications };
  ...
  virtual SourceId setSource(
      ...
      const alexaClientSDK::avsCommon::utils::AudioFormat* format =
          nullptr) override;
  virtual SourceId setSource(const std::string& url,
  ...
  virtual void setObserver(
      std::shared_ptr<alexaClientSDK::avsCommon::utils::mediaPlayer::
  ...
  virtual alexaClientSDK::avsCommon::sdkInterfaces::SpeakerInterface::Type
  getSpeakerType() override;
  virtual void onError()
      override;  // a UrlContentToAttachmentConverter experienced an error
 ...
 private:
  ...
  // copied size shorts in _decodedPcm into waveData, returning the number of
  // milliseconds
  float CopyToWaveData(size_t size, const StreamingWaveDataPtr& waveData,
                       bool flush);
  ...
  void LogPlayingSourceMismatchEvent(const std::string& func, SourceId id,
                                     SourceId playingId);
  ...
  // return true if set, only set state if it is currently desired (atomically),
  // otherwise returns false and doesn't impact _state
  ...
  /// The set of MediaPlayerObserverInterface instances to notify of state
  /// changes
  std::unordered_set<
      std::shared_ptr<alexaClientSDK::avsCommon::utils::mediaPlayer::
                          MediaPlayerObserverInterface>>
  ...
  std::shared_ptr<alexaClientSDK::avsCommon::sdkInterfaces::
                      HTTPContentFetcherInterfaceFactoryInterface>
      _contentFetcherFactory;

  // Used to stream urls into attachments
  std::shared_ptr<
      alexaClientSDK::playlistParser::UrlContentToAttachmentConverter>
      _urlConverter;
  ...
  // guards access to _observers. not sure of order of sdk calls, so recursive
  // just in case
  std::recursive_mutex _observerMutex;

  // todo: these three wouldn't be necessary if we had a thread instead of a
  // dispatch queue. We basically need to join() the play/decoding thread during
  // doShutdown(). In the meantime, we wait for a condition variable
