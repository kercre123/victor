 ...
 * Created: 02/01/19
 *
 * Description: Run "Alexa" speech recognizer on the audio engine's master
 * output. The recognizer is provided by the SpeechRecognizerSystem and ran on a
 * worker thread. The SpeechRecognizerSystem responds when there is a trigger
 * detected in the playback stream.
 *
...
AlexaPlaybackRecognizerComponent::AlexaPlaybackRecognizerComponent(
    ...
    : _context(context),
      ...
      _processAudioThread(std::thread(
          &AlexaPlaybackRecognizerComponent::ProcessAudioLoop, this)) {}
...
bool AlexaPlaybackRecognizerComponent::Init() {
  ...
  if (!_speechRecSys._alexaPlaybackTrigger) {
    LOG_ERROR(
        "AlexaPlaybackRecognizerComponent._speechRecSys._alexaPlaybackTrigger."
        "IsNull",
        "");
    return false;
  }
  ...
  // Setup resampler
  int error = 0;
  _resamplerState = speex_resampler_init(
      SinkPluginTypes::kAkAlsaSinkChannelCount,  // num channels
      ...
      &error);
  if (error != 0) {
    LOG_WARNING("AlexaPlaybackRecognizerComponent",
                "speex_resampler_init error %d", error);
    return false;
  }

  // Setup sink plugin callback
  _sinkPlugin->SetPlaybackBufferCallback(
      std::bind(&AlexaPlaybackRecognizerComponent::SinkPluginCallback, this,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaPlaybackRecognizerComponent::SinkPluginCallback(
    const SinkPluginTypes::AudioChunk& chunk, bool hasData) {
  ...
  }
  {
    ...
    if (_audioEngOutBuffer.size() == _audioEngOutBuffer.capacity()) {
      ...
      LOG_WARNING("AlexaPlaybackRecognizerComponent.SinkPluginCallback",
                  "_audioEngOutputBuffer is FULL!!");
    }
  ...
  while (!_processThreadStop) {
    ...
    // Wait for audio engine thread to push data into buffer. Then get it out!
    {
      ...
      const auto waitFunc = [this] {
        return !_audioEngOutBuffer.empty() || _pendingLocaleUpdate ||
               _processThreadStop;
      };
      ...
      // Note: It is possible that the pointer's data is overridden. We are okay
      // with this, the buffer is static memory,
      //       the buffer is large enough to hold as much data we are concerned
      //       about and if the buffer is that far behind the data is useless to
      //       us.
    ...
    if (_pendingLocaleUpdate && _isInitialized) {
      ...
      ASSERT_NAMED(playbackTrigger != nullptr,
                   "AlexaPlaybackRecognizerComponent.ProcessAudioLoop."
                   "playbackTrigger.IsNull");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaPlaybackRecognizerComponent::ProcessAudio(
    const SinkPluginTypes::AudioChunk* inData) {
  ...
  if (!_resampledAudioBuffer.push_back(outData, numSamplesWritten)) {
    // Thish should never happen because the previous call to ProcessAudio()
    // should have made plenty of room
    LOG_WARNING("AlexaPlaybackRecognizerComponent.ProcessAudioLoop",
                "_resampledAudioBuffer doesn't have enough continous memory "
                "for push_back()!!!");
    ...
    if (!_resampledAudioBuffer.push_back(outData, numSamplesWritten)) {
      LOG_ERROR(
          "AlexaPlaybackRecognizerComponent.ProcessAudioLoop._"
          "resampledAudioBuffer.push_back.secondAttempt.Fail",
          "");
      return;
    }
  }

  speex_resampler_process_interleaved_int(_resamplerState, *inData,
                                          &numSamplesProcessed, outData,
                                          &numSamplesWritten);

  if (numSamplesWritten > 0) {
    while ((_resampledAudioBuffer.size() / kRecognizerBufferSize) > 0) {
      _recognizer->Update(&_resampledAudioBuffer.front(),
                          kRecognizerBufferSize);
