...
class Alexa {
 public:
  ...
  // cancels any pending authorization started by the user (not an auto-auth
  // during reboot). Reason is included in a DAS message
  void CancelPendingAlexaAuth(const std::string& reason);

  void OnEngineLoaded();

  // Adds samples to the mic stream buffer. Should be ok to call on another
  // thread
  void AddMicrophoneSamples(const AudioUtil::AudioSample* const samples,
  ...
  // Get the number of audio samples that have been added to Alexa "Microphone"
  // component
  uint64_t GetMicrophoneSampleIndex() const;

  // Whether there is a session that is active or in the process of
  // initializing. Assumes that the existence of the impl is still tied to
  // opt-in state (which may change)
 ...
 protected:
  // explicitly declare noncopyable (Util::noncopyable doesn't play well with
  // movable)
  Alexa(const Alexa& other) = delete;
  Alexa& operator=(const Alexa& other) = delete;

 private:
  // decides whether to create/destroy the impl. If !active, then deleteUserData
  // decides whether user data will be cleared
  void SetAlexaActive(bool active, bool deleteUserData = false);

  // called when SDK auth state changes
  void OnAlexaAuthChanged(AlexaAuthState state, const std::string& url,
                          const std::string& code, bool errFlag);

  // called when SDK dialog or media player state changes. Note this will never
  // send Error. The state should be switch to Error only when
  // OnAlexaNetworkError is called
  void OnAlexaUXStateChanged(AlexaUXState newState);

  // called when there is a user-facing error. This will set the ux state to
  ...
  // called when SDK changes whether an indicator for notifications should be
  // shown
  ...
  bool HasInitializedImpl()
      const;  // done loading sdk but maybe not done connecting yet

  // sets this class's _authState and messages engine if it changes
  void SetAuthState(AlexaAuthState state, const std::string& url = "",
  ...
  // Sets the face info screen to screenName, possibly with a url (intentional
  // copy) and code
  void SetAlexaFace(ScreenName screenName, std::string url = "",
                    const std::string& code = "") const;

  // helpers for the file that indicates whether the last robot run ended during
  ...
  // Use new create AudioCallbackContext instance, hand off ownership when
  // passing in to method
  void PlayAudioEvent(
      AudioEngine::AudioEventId eventId,
      AudioEngine::AudioCallbackContext* callback = nullptr) const;
  ...
  AlexaUXState _pendingUXState;  // during AlexaUXState::Error, this is pending
                                 // to be re-assigned to _uxState

  // If non-negative, turn on the wakeword at this time, even if not connected,
  // so we can play error states
  float _timeEnableWakeWord_s = -1.0f;
  // If non-negative, this is the time that the AlexaUXState::Error ends,
  // restoring _pendingUXState
  ...
  // whether a message was received from engine saying to opt in. this gets
  // reset after auth completes
  bool _authStartedByUser = false;
  // if during an authentication the state was ever WaitingForCode, this is the
  // most recent code
  ...
  // guards access to _impl when releasing it on main thread, and when used in
  // NotifyOfWakeWord and AddMicrophoneSamples, which can be called off thread
