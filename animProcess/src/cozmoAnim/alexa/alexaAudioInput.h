...
class AlexaAudioInput : public alexaClientSDK::applicationUtilities::resources::
                            audio::MicrophoneInterface {
 public:
  static std::unique_ptr<AlexaAudioInput> Create(
      std::shared_ptr<alexaClientSDK::avsCommon::avs::AudioInputStream> stream);

  // Adds data to the stream buffer held by alexa. Should be ok to call on
  // another thread
  void AddSamples(const AudioUtil::AudioSample* data, size_t size);

  uint64_t GetTotalNumSamples() const { return _totalNumSamples; }

  // Stops streaming from the microphone. Returns whether the stop was
  // successful. (note: the micDataSystem job is still there, but we just drop
  // all incoming data)
  virtual bool stopStreamingMicrophoneData() override;

  // Starts streaming from the microphone. returns whether the start was
  // successful. (note: the micDataSystem job was added before this was called,
  // but we just drop all incoming data)
  virtual bool startStreamingMicrophoneData() override;

  // For debug purposes, expose the reader for this mic data. Note that reading
  // from this reader will drain it (so for debugging purposes, we make a copy
  // of this whole object)
  std::unique_ptr<alexaClientSDK::avsCommon::avs::AudioInputStream::Reader>
  GetReader();

 private:
  explicit AlexaAudioInput(
      std::shared_ptr<alexaClientSDK::avsCommon::avs::AudioInputStream> stream);
  ...
  std::unique_ptr<alexaClientSDK::avsCommon::avs::AudioInputStream::Writer>
      _writer;
