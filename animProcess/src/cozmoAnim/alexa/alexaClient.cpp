 ...
 * Created: Oct 19 2018
 *
 * Description: Client for amazon voice services. This is an analog to the SDK
 * example DefaultClient that works with other Anki analogs like speakers/mics.
 *
 *
...
// Since this file an an analog to DefaultClient.cpp, some portions were copied.
// Here's the SDK license
/*
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::unique_ptr<AlexaClient> AlexaClient::Create(
    ...
    std::shared_ptr<capabilityAgents::alerts::storage::AlertStorageInterface>
        alertStorage,
    std::shared_ptr<
        capabilityAgents::notifications::NotificationsStorageInterface>
        notificationsStorage,
    std::shared_ptr<capabilityAgents::settings::SettingsStorageInterface>
        settingsStorage,
    ...
    std::unordered_set<std::shared_ptr<
        avsCommon::sdkInterfaces::DialogUXStateObserverInterface>>
        alexaDialogStateObservers,
    std::unordered_set<std::shared_ptr<
        avsCommon::sdkInterfaces::ConnectionStatusObserverInterface>>
        connectionObservers,
    std::unordered_set<std::shared_ptr<
        avsCommon::sdkInterfaces::MessageRequestObserverInterface>>
    ...
    std::shared_ptr<avsCommon::utils::network::InternetConnectionMonitor>
  ...
  if (!client->Init(
          deviceInfo, customerDataManager, authDelegate, messageStorage,
          alertStorage, notificationsStorage, settingsStorage,
          deviceSettingStorage, audioFactory, alexaDialogStateObservers,
          connectionObservers, messageRequestObservers, capabilitiesDelegate,
          ttsMediaPlayer, alertsMediaPlayer, audioMediaPlayer,
          notificationsMediaPlayer, ttsSpeaker, alertsSpeaker, audioSpeaker,
          notificationsSpeaker, internetConnectionMonitor, firmwareVersion)) {
    return nullptr;
  }
...
bool AlexaClient::Init(
  ...
  _dialogUXStateAggregator =
      std::make_shared<avsCommon::avs::DialogUXStateAggregator>();
  ...
  if (!_deviceSettingStorage->open()) {
    ACSDK_ERROR(
        LX("initializeFailed").d("reason", "deviceSettingStorageOpenFailed"));
    return false;
  }
  ...
  if (!_deviceSettingsManager) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "createDeviceSettingsManagerFailed"));
    return false;
  }

  // Creating the Attachment Manager - This component deals with managing
  // attachments and allows for readers and writers to be created to handle the
  // attachment.
  auto attachmentManager =
      std::make_shared<avsCommon::avs::attachment::AttachmentManager>(
          avsCommon::avs::attachment::AttachmentManager::AttachmentType::
              IN_PROCESS);

  // Creating the Context Manager - This component manages the context of each
  // of the components to update to AVS. It is required for each of the
  // capability agents so that they may provide their state just before any
  // event is fired off.
  ...
  auto postConnectSynchronizerFactory =
      acl::PostConnectSynchronizerFactory::create(contextManager);

  // Create a factory to create objects that establish a connection with AVS.
  auto transportFactory = std::make_shared<acl::HTTP2TransportFactory>(
      std::make_shared<
          avsCommon::utils::libcurlUtils::LibcurlHTTP2ConnectionFactory>(),
      postConnectSynchronizerFactory);

  // Creating the message router - This component actually maintains the
  // connection to AVS over HTTP2. It is created using the auth delegate, which
  // provides authorization to connect to AVS, and the attachment manager, which
  // helps ACL write attachments received from AVS.
  _messageRouter = std::make_shared<AlexaMessageRouter>(
      messageRequestObservers, authDelegate, attachmentManager,
      transportFactory);

  // Creating the connection manager - This component is the overarching
  // connection manager that glues together all the other networking components
  // into one easy-to-use component.
  _connectionManager = acl::AVSConnectionManager::create(
      _messageRouter, false, connectionObservers, {_dialogUXStateAggregator});
  if (!_connectionManager) {
    ACSDK_ERROR(
        LX("initializeFailed").d("reason", "unableToCreateConnectionManager"));
    return false;
  }

  // Creating our certified sender - this component guarantees that messages
  // given to it (expected to be JSON formatted AVS Events) will be sent to AVS.
  // This nicely decouples strict message sending from components which require
  // an Event be sent, even in conditions when there is no active AVS
  // connection.
  _certifiedSender = certifiedSender::CertifiedSender::create(
      _connectionManager, _connectionManager, messageStorage,
      customerDataManager);
  if (!_certifiedSender) {
    ACSDK_ERROR(
        LX("initializeFailed").d("reason", "unableToCreateCertifiedSender"));
    return false;
  }

  // Creating the Exception Sender - This component helps the SDK send
  // exceptions when it is unable to handle a directive sent by AVS. For that
  // reason, the Directive Sequencer and each Capability Agent will need this
  // component.
  _exceptionSender =
      avsCommon::avs::ExceptionEncounteredSender::create(_connectionManager);
  if (!_exceptionSender) {
    ACSDK_ERROR(
        LX("initializeFailed").d("reason", "unableToCreateExceptionSender"));
    return false;
  }

  // Creating the Directive Sequencer - This is the component that deals with
  // the sequencing and ordering of directives sent from AVS and forwarding them
  // along to the appropriate Capability Agent that deals with directives in
  // that Namespace/Name.
  _directiveSequencer = adsl::DirectiveSequencer::create(_exceptionSender);
  if (!_directiveSequencer) {
    ACSDK_ERROR(
        LX("initializeFailed").d("reason", "unableToCreateDirectiveSequencer"));
    return false;
  }

  auto messageInterpreter = std::make_shared<adsl::MessageInterpreter>(
      _exceptionSender, _directiveSequencer, attachmentManager);
  ...
  // implementing any customer registration operation such as login and logout
  _registrationManager =
      std::make_shared<registrationManager::RegistrationManager>(
          _directiveSequencer, _connectionManager, customerDataManager);
  ...
  // layered audio focus across various components. It handles granting access
  // to Channels as well as pushing different "Channels" to foreground,
  // background, or no focus based on which other Channels are active and the
  // priorities of those Channels. Each Capability Agent will require the Focus
  // Manager in order to request access to the Channel it wishes to play on.
  _audioFocusManager = std::make_shared<afml::FocusManager>(
      afml::FocusManager::getDefaultAudioChannels(), _audioActivityTracker);

  // Creating the User Inactivity Monitor - This component is responsibly for
  // updating AVS of user inactivity as described in the System Interface of
  // AVS.
  _userInactivityMonitor =
      capabilityAgents::system::UserInactivityMonitor::create(
          _connectionManager, _exceptionSender);
  if (!_userInactivityMonitor) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToCreateUserInactivityMonitor"));
    return false;
  }
  ...
  _audioInputProcessor = capabilityAgents::aip::AudioInputProcessor::create(
      _directiveSequencer, _connectionManager, contextManager,
      _audioFocusManager, _dialogUXStateAggregator, _exceptionSender,
      _userInactivityMonitor);
  if (!_audioInputProcessor) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToCreateAudioInputProcessor"));
    return false;
  }
  ...
  // that implements the SpeechSynthesizer interface of AVS.
  _speechSynthesizer =
      capabilityAgents::speechSynthesizer::SpeechSynthesizer::create(
          ttsMediaPlayer, _connectionManager, _audioFocusManager,
          contextManager, _exceptionSender, _dialogUXStateAggregator);
  if (!_speechSynthesizer) {
    ACSDK_ERROR(
        LX("initializeFailed").d("reason", "unableToCreateSpeechSynthesizer"));
    return false;
  }
  ...
  // Capability Agent that implements the PlaybackController interface of AVS.
  _playbackController =
      capabilityAgents::playbackController::PlaybackController::create(
          contextManager, _connectionManager);
  if (!_playbackController) {
    ACSDK_ERROR(
        LX("initializeFailed").d("reason", "unableToCreatePlaybackController"));
    return false;
  }

  // Creating the PlaybackRouter - This component routes a playback button press
  // to the active handler. The default handler is PlaybackController.
  _playbackRouter =
      capabilityAgents::playbackController::PlaybackRouter::create(
          _playbackController);
  if (!_playbackRouter) {
    ACSDK_ERROR(
        LX("initializeFailed").d("reason", "unableToCreatePlaybackRouter"));
    return false;
  }

  // Creating the Audio Player - This component is the Capability Agent that
  // implements the AudioPlayer interface of AVS.
  _audioPlayer = capabilityAgents::audioPlayer::AudioPlayer::create(
      audioMediaPlayer, _connectionManager, _audioFocusManager, contextManager,
      _exceptionSender, _playbackRouter);
  if (!_audioPlayer) {
    ACSDK_ERROR(
        LX("initializeFailed").d("reason", "unableToCreateAudioPlayer"));
    return false;
  }

  std::vector<std::shared_ptr<avsCommon::sdkInterfaces::SpeakerInterface>>
      allSpeakers = {
          ttsSpeaker, alertsSpeaker, audioSpeaker, notificationsSpeaker,
          // unused speaker types supported by SDK: bluetoothSpeaker,
          // ringtoneSpeaker
      };

  // Creating the SpeakerManager Capability Agent - This component is the
  // Capability Agent that implements the Speaker interface of AVS.
  _speakerManager = capabilityAgents::speakerManager::SpeakerManager::create(
      allSpeakers, contextManager, _connectionManager, _exceptionSender);
  if (!_speakerManager) {
    ACSDK_ERROR(
        LX("initializeFailed").d("reason", "unableToCreateSpeakerManager"));
    return false;
  }

  _alertsCapabilityAgent =
      capabilityAgents::alerts::AlertsCapabilityAgent::create(
          _connectionManager, _connectionManager, _certifiedSender,
          _audioFocusManager, _speakerManager, contextManager, _exceptionSender,
          alertStorage, audioFactory->alerts(),
          capabilityAgents::alerts::renderer::Renderer::create(
              alertsMediaPlayer),
          customerDataManager);
  if (!_alertsCapabilityAgent) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToCreateAlertsCapabilityAgent"));
    return false;
  }
  ...
  // Capability Agent that implements the Notifications interface of AVS.
  _notificationsCapabilityAgent =
      capabilityAgents::notifications::NotificationsCapabilityAgent::create(
          notificationsStorage,
          capabilityAgents::notifications::NotificationRenderer::create(
              notificationsMediaPlayer),
          contextManager, _exceptionSender, audioFactory->notifications(),
          customerDataManager);

  if (!_notificationsCapabilityAgent) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToCreateNotificationsCapabilityAgent"));
    return false;
  }

  // VIC-11806: notifications cleared when victor was turned off never recieve
  // the SetIndicator(OFF) callback so
  //            the notification light remains on when you boot up.  Clearing
  //            all saved notification data at startup, since amazon re-sends
  //            them upon connection
  // note: this seems really wrong and is a hack to pass cert :(
  _notificationsCapabilityAgent->clearData();

  // The Interaction Model Capability Agent provides a way for AVS cloud
  // initiated actions to be executed by the client.
  _interactionCapabilityAgent =
      capabilityAgents::interactionModel::InteractionModelCapabilityAgent::
          create(_directiveSequencer, _exceptionSender);
  if (!_interactionCapabilityAgent) {
    ACSDK_ERROR(
        LX("initializeFailed")
            .d("reason", "unableToCreateInteractionModelCapabilityAgent"));
    return false;
  }

  std::shared_ptr<capabilityAgents::settings::SettingsUpdatedEventSender>
      settingsUpdatedEventSender =
          capabilityAgents::settings::SettingsUpdatedEventSender::create(
              _certifiedSender);
  if (!settingsUpdatedEventSender) {
    ACSDK_ERROR(
        LX("initializeFailed").d("reason", "unableToCreateSettingsObserver"));
    return false;
  }

  // Creating the Setting object - This component implements the Setting
  // interface of AVS.
  _settings = capabilityAgents::settings::Settings::create(
      settingsStorage, {settingsUpdatedEventSender}, customerDataManager);
  if (!_settings) {
    ACSDK_ERROR(
        LX("initializeFailed").d("reason", "unableToCreateSettingsObject"));
    return false;
  }

  // Creating the Endpoint Handler - This component is responsible for handling
  // directives from AVS instructing the client to change the endpoint to
  // connect to.
  auto endpointHandler = capabilityAgents::system::EndpointHandler::create(
      _connectionManager, _exceptionSender);
  if (!endpointHandler) {
    ACSDK_ERROR(
        LX("initializeFailed").d("reason", "unableToCreateEndpointHandler"));
    return false;
  }

  // Creating the SystemCapabilityProvider - This component is responsible for
  // publishing information about the System capability agent.
  auto systemCapabilityProvider =
      capabilityAgents::system::SystemCapabilityProvider::create();
  if (!systemCapabilityProvider) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToCreateSystemCapabilityProvider"));
    return false;
  }

  // Creating the RevokeAuthorizationHandler - This component is responsible for
  // handling RevokeAuthorization directives from AVS to notify the client to
  // clear out authorization and re-enter the registration flow.
  _revokeAuthorizationHandler =
      AlexaRevokeAuthHandler::create(_exceptionSender);
  if (!_revokeAuthorizationHandler) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToCreateRevokeAuthorizationHandler"));
    return false;
  }

  if (avsCommon::sdkInterfaces::softwareInfo::isValidFirmwareVersion(
          firmwareVersion)) {
    const bool sendSoftwareInfoOnConnected = true;
    _softwareInfoSender = capabilityAgents::system::SoftwareInfoSender::create(
        firmwareVersion, sendSoftwareInfoOnConnected,
        nullptr,  // softwareInfoSenderObserver
        _connectionManager, _connectionManager, _exceptionSender);
    if (!_softwareInfoSender) {
      ACSDK_ERROR(LX("initializeFailed")
                      .d("reason", "unableToCreateSoftwareInfoSender"));
      return false;
    }
  } else {
    LOG_WARNING("AlexaClient.Init.InvalidFirmwareVersion",
                "SDK rejected firmware version %d", firmwareVersion);
    return false;
  }
  ...
  if (kDEV_ONLY_EnableAlexaTemplateRendererStub) {
    _templateRuntime =
        std::make_shared<AlexaTemplateRuntimeStub>(_exceptionSender);
  }
#endif

// Creating the DoNotDisturb Capability Agent
//_dndCapabilityAgent
//  = capabilityAgents::doNotDisturb::DoNotDisturbCapabilityAgent::create(
//  customerDataManager,
//                                                                         _exceptionSender,
//                                                                         _connectionManager,
...
//                                                                         );
// if( !_dndCapabilityAgent ) {
//  ACSDK_ERROR(LX("initializeFailed").d("reason",
//  "unableToCreateDNDCapabilityAgent")); return false;
//}
// AddConnectionObserver( _dndCapabilityAgent );

// Register directives

// This creates a directive wrapper that lets us snoop on directives json sent
// to capabilities The nameSpace is the namespace constant defined at the top of
// each capabilities' cpp file.
// TODO: expose this through a change to the SDK.
#define MAKE_WRAPPER(nameSpace, handler)                       \
  std::make_shared<AlexaCapabilityWrapper>(nameSpace, handler, \
                                           _exceptionSender, _onDirectiveFunc)

  if (!_directiveSequencer->addDirectiveHandler(
          MAKE_WRAPPER("SpeechSynthesizer", _speechSynthesizer))) {
    ACSDK_ERROR(LX("initializeFailed")
  ...
  if (!_directiveSequencer->addDirectiveHandler(
          MAKE_WRAPPER("SpeechRecognizer", _audioInputProcessor))) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToRegisterDirectiveHandler")
  ...
  // Note: I've never actually seen this one receive the "RevokeAuthorization"
  // directive we need
  if (!_directiveSequencer->addDirectiveHandler(
          MAKE_WRAPPER("System", _revokeAuthorizationHandler))) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToRegisterDirectiveHandler")
  ...
  if (!_directiveSequencer->addDirectiveHandler(
          MAKE_WRAPPER("System", _userInactivityMonitor))) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToRegisterDirectiveHandler")
  ...
  if (!_directiveSequencer->addDirectiveHandler(
          MAKE_WRAPPER("Alerts", _alertsCapabilityAgent))) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToRegisterDirectiveHandler")
  ...
  if (!_directiveSequencer->addDirectiveHandler(
          MAKE_WRAPPER("System", endpointHandler))) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToRegisterDirectiveHandler")
  ...
  if (!_directiveSequencer->addDirectiveHandler(
          MAKE_WRAPPER("AudioPlayer", _audioPlayer))) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToRegisterDirectiveHandler")
  ...
  if (!_directiveSequencer->addDirectiveHandler(
          MAKE_WRAPPER("Speaker", _speakerManager))) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToRegisterDirectiveHandler")
  ...
  if (!_directiveSequencer->addDirectiveHandler(
          MAKE_WRAPPER("Notifications", _notificationsCapabilityAgent))) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToRegisterDirectiveHandler")
  ...
  if (!_directiveSequencer->addDirectiveHandler(
          MAKE_WRAPPER("InteractionModel", _interactionCapabilityAgent))) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToRegisterDirectiveHandler")
  ...
  if (kDEV_ONLY_EnableAlexaTemplateRendererStub) {
    if (!_directiveSequencer->addDirectiveHandler(
            MAKE_WRAPPER("TemplateRuntime", _templateRuntime))) {
      ACSDK_ERROR(LX("initializeFailed")
                      .d("reason", "unableToRegisterDirectiveHandler")
  ...
  // if( !_directiveSequencer->addDirectiveHandler(
  // MAKE_WRAPPER("Alexa.DoNotDisturb", _dndCapabilityAgent) ) ) {
  //  ACSDK_ERROR(LX("initializeFailed")
  //              .d("reason", "unableToRegisterDirectiveHandler")
  ...
  if (!capabilitiesDelegate->registerCapability(_audioPlayer)) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToRegisterCapability")
                    .d("capabilitiesDelegate", "AudioPlayer"));
    return false;
  }

  if (!capabilitiesDelegate->registerCapability(
          _notificationsCapabilityAgent)) {
    ACSDK_ERROR(LX("initializeFailed")
                    .d("reason", "unableToRegisterCapability")
  ...
  auto sendDirective = [&](ConsoleFunctionContextRef context) {
    ...
    if (_directiveSequencer) {
      // if whatever responds to the directive needs to parse an attachment,
      // that will be undefined behavior. for other things like alerts, this
      // should work. Set LogAlexaDirectives to true, note the full directive
      // printed to logs, and then modify it as needed and re-send it here
      const std::string attachmentContextId;
      auto directive = avsCommon::avs::AVSDirective::create(
          unparsedDirective, attachmentManager, attachmentContextId);
      _directiveSequencer->onDirective(std::move(directive.first));
    }
  };
  sConsoleFuncs.emplace_front("SendAlexaDirective", std::move(sendDirective),
                              "Alexa", "const char* directive");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaClient::Connect(
    const std::shared_ptr<
        avsCommon::sdkInterfaces::CapabilitiesDelegateInterface>&
        capabilitiesDelegate) {
  // try connecting
  if (capabilitiesDelegate != nullptr) {
    capabilitiesDelegate->publishCapabilitiesAsyncWithRetries();
  } else {
    LOG_ERROR("AlexaClient.Connect.NoCapabilities",
              "no capabilities delegate, can't register capabilities");
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaClient::ChangeSetting(const std::string& key,
...
std::future<bool> AlexaClient::NotifyOfTapToTalk(
  ...
  return _audioInputProcessor->recognize(tapToTalkAudioProvider,
                                         capabilityAgents::aip::Initiator::TAP,
                                         startOfSpeechTimestamp, beginIndex);
}
...
std::future<bool> AlexaClient::NotifyOfWakeWord(
  ...
  LOG_INFO("AlexaClient.NotifyOfWakeWord",
           "keyword='%s', beginIDx=%llu, endIdx=%llu, connected=%d",
           keyword.c_str(), beginIndex, endIndex,
  ...
  return _audioInputProcessor->recognize(
      wakeWordAudioProvider, capabilityAgents::aip::Initiator::WAKEWORD,
      startOfSpeechTimestamp, beginIndex, endIndex, keyword, espData,
      KWDMetadata);
}
...
void AlexaClient::AddConnectionObserver(
    std::shared_ptr<avsCommon::sdkInterfaces::ConnectionStatusObserverInterface>
        observer) {
  if (ANKI_VERIFY(_connectionManager != nullptr,
                  "AlexaClient.AddConnectionObserver.Null",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaClient::RemoveConnectionObserver(
    std::shared_ptr<avsCommon::sdkInterfaces::ConnectionStatusObserverInterface>
        observer) {
  if (ANKI_VERIFY(_connectionManager != nullptr,
                  "AlexaClient.RemoveConnectionObserver.Null",
                  "Connection manager is null")) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaClient::onCapabilitiesStateChange(
...
void AlexaClient::StopAlerts() {
  if (ANKI_VERIFY(_alertsCapabilityAgent != nullptr,
                  "AlexaClient.StopAlerts.Null", "Alerts capability is null")) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaClient::AddSpeakerManagerObserver(
    std::shared_ptr<avsCommon::sdkInterfaces::SpeakerManagerObserverInterface>
        observer) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaClient::AddAlexaDialogStateObserver(
    std::shared_ptr<avsCommon::sdkInterfaces::DialogUXStateObserverInterface>
        observer) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaClient::AddInternetConnectionObserver(
    std::shared_ptr<
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaClient::AddRevokeAuthorizationObserver(
    std::shared_ptr<
        avsCommon::sdkInterfaces::RevokeAuthorizationObserverInterface>
        observer) {
  if (!_revokeAuthorizationHandler) {
    ACSDK_ERROR(LX("addRevokeAuthorizationObserver")
                    .d("reason", "revokeAuthorizationNotSupported"));
    return;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaClient::AddNotificationsObserver(
    std::shared_ptr<avsCommon::sdkInterfaces::NotificationsObserverInterface>
        observer) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaClient::AddAlertObserver(
    std::shared_ptr<capabilityAgents::alerts::AlertObserverInterface>
        observer) {
  if (ANKI_VERIFY(_alertsCapabilityAgent != nullptr,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AlexaClient::AddAudioPlayerObserver(
    std::shared_ptr<
        alexaClientSDK::avsCommon::sdkInterfaces::AudioPlayerObserverInterface>
        observer) {
...
AlexaClient::GetRegistrationManager() {
...
AlexaClient::GetSettingsManager() {
