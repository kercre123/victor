...
class RobotDataLoader : private Util::noncopyable {
 public:
  ...
  const Vision::SpritePathMap* GetSpritePaths() const {
    assert(_spritePathMap != nullptr);
    return _spritePathMap.get();
  }
  ...
  using BackpackAnimationTriggerMap =
      Util::CladEnumToStringMap<BackpackAnimationTrigger>;
  using FileJsonMap = std::unordered_map<std::string, const Json::Value>;
  const FileJsonMap& GetBackpackLightAnimations() const {
    return _backpackLightAnimations;
  }
  BackpackAnimationTriggerMap* GetBackpackAnimationTriggerMap() {
    return _backpackAnimationTriggerMap.get();
  }
