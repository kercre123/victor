 ...
 * Created: 12/27/2018
 *
 * Description: SpeechRecognizer implementation for Amazon's Pryon Lite. The cpp
 * defines the impl struct that is only declared in the header, in order to
...
class SpeechRecognizerPryonLite : public AudioUtil::SpeechRecognizer {
 public:
  ...
  SpeechRecognizerPryonLite& operator=(const SpeechRecognizerPryonLite& other) =
      delete;

  // Create recognizer with model file
  // Can recall method to change model
  // Note: When changing the model file the recognizer will be destroyed and
  // re-created.
  bool InitRecognizer(const std::string& modelFilePath, bool useVad);

  // Stream audio data to recognizer
  // Note: Stream all data to recognizer to keep stream's time in sync with
  // Alexa component, there is an internal VAD to improve recognizer
  // performance. When SpeechRecognizer is dissabled silence is streamed into
  // recognizer.
  virtual void Update(const AudioUtil::AudioSample* audioData,
                      unsigned int audioDataLen) override;

  // Set detection threshold for all keywords (this function can be called any
  // time after decoder initialization) Valid values 1-1000, 1 = lowest
  // threshold, most detections, 1000 = highest threshold, fewest detections.
  ...
  void SetAlexaMicrophoneOffset(uint64_t offset) {
    _alexaMicrophoneOffset = offset;
  }
 ...
 private:
  ...
  static bool LoadPryonModel(const std::string& filePath,
                             SpeechRecognizerPryonLiteData& data);
