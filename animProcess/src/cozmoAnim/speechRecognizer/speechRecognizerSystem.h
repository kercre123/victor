...
class SpeechRecognizerSystem {
 public:
  ...
  SpeechRecognizerSystem& operator=(const SpeechRecognizerSystem& other) =
      delete;

  using TriggerWordDetectedCallback =
      std::function<void(const AudioUtil::SpeechRecognizerCallbackInfo& info)>;
  using AlexaTriggerWordDetectedCallback = std::function<void(
  ...
  // set whether the notch detector should be active (for alexa keyword only).
  // When active, alexa triggers get dropped if we detect a notch.
  void ToggleNotchDetector(bool active);

  // add 'raw' audio samples
  void UpdateNotch(const AudioUtil::AudioSample* audioChunk,
                   unsigned int audioDataLen);

  // Update recognizer audio
  // NOTE: Always call from the same thread
  void Update(const AudioUtil::AudioSample* audioData,
  ...
  bool UpdateTriggerForLocale(
      const Util::Locale& newLocale,
      RecognizerTypeFlag recognizerFlags = RecognizerTypeFlag::All);

  // Alexa Methods
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // - - - - - - - - - - - - - - - - - - - - Alexa has been set active set
  // current locale and callback for Alexa trigger recognitions
  void ActivateAlexa(const Util::Locale& locale,
                     AlexaTriggerWordDetectedCallback callback);
 ...
 private:
  ...
  std::unique_ptr<AlexaPlaybackRecognizerComponent>
      _alexaPlaybackRecognizerComponent;
  ...
  // Alexa Methods
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // - - - - - - - - - - - - - - - - - - - - Init Alexa trigger detector Note:
  // This is done after Alex user has been authicated
  ...
  // Return true when locale file was found and is different then current locale
  // NOTE: This only sets the _nextTriggerPaths the locale will be updated in
  // the Update() call
  template <class SpeechRecognizerType>
  bool UpdateTriggerForLocale(
  ...
  // This should only be called from the Update() methods, needs to be performed
  // on the same thread
  void ApplyLocaleUpdate();

  template <class SpeechRecognizerType>
  void ApplySpeechRecognizerLoacleUpdate(
      TriggerContext<SpeechRecognizerType>& aTrigger);

  bool UpdateRecognizerModel(TriggerContext<SpeechRecognizerTHF>& aTrigger);
  bool UpdateRecognizerModel(
      TriggerContext<SpeechRecognizerPryonLite>& aTrigger);

  // Console Var methods
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // - - - - - - - - - - - - - - - - - - - - NOTE: These methods provide no
  // functionality when ANKI_DEV_CHEATS is turned off
  void SetupConsoleFuncs();

  template <class SpeechRecognizerType>
  std::string UpdateRecognizerHelper(
