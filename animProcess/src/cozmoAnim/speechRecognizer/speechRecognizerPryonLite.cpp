...
// TODO: Waiting for Amazon to provide libs for Mac platform, until then compile
// out recognizer for other platforms
...
CONSOLE_VAR_RANGED(int, kDefaultDetectThreshold, "SpeechRecognizer.Alexa", 250,
                   0, 1000);
...
SpeechRecognizerPryonLite::~SpeechRecognizerPryonLite() { Cleanup(); }

SpeechRecognizerPryonLite::SpeechRecognizerPryonLite(
    SpeechRecognizerPryonLite&& other)
    : SpeechRecognizer(std::move(other)) {
  SwapAllData(other);
}

SpeechRecognizerPryonLite& SpeechRecognizerPryonLite::operator=(
    SpeechRecognizerPryonLite&& other) {
...
bool SpeechRecognizerPryonLite::InitRecognizer(const std::string& modlePath,
  ...
  PryonLiteError err = PryonLite_GetModelAttributes(
      _impl->config.model, _impl->config.sizeofModel, &modelAttributes);
  if (err != PRYON_LITE_ERROR_OK) {
    LOG_ERROR("SpeechRecognizerPryonLite.Init.PryonLite_GetModelAttributes",
              "PryonLite error %d", err);
    Cleanup();
    return false;
  }
  ...
  err = PryonLiteDecoder_Initialize(&_impl->config, &sessionInfo,
                                    &_impl->decoder);
  if (err != PRYON_LITE_ERROR_OK) {
    LOG_ERROR("SpeechRecognizerPryonLite.Init.PryonLiteDecoder_Initialize",
              "PryonLite error %d", err);
    Cleanup();
    return false;
  }
  ...
  if (!success) {
    LOG_ERROR("SpeechRecognizerPryonLite.Init.SetDetectionThreshold",
              "PryonLite error %d", err);
    Cleanup();
    return false;
  }
...
void SpeechRecognizerPryonLite::Update(const AudioUtil::AudioSample* audioData,
                                       unsigned int audioDataLen) {
#if PRYON_ENABLED
  // Silence Audio data
  static const AudioUtil::AudioSample silenceData[160] = {0};

  const AudioUtil::AudioSample* data =
      _impl->disabled ? silenceData : audioData;
  // pass samples to decoder
  PryonLiteError err =
      PryonLiteDecoder_PushAudioSamples(_impl->decoder, data, audioDataLen);
  if (err != PRYON_LITE_ERROR_OK) {
    LOG_ERROR(
        "SpeechRecognizerPryonLite.Update.PryonLiteDecoder_PushAudioSamples",
        "PryonLite error %d", err);
  }
#endif
}
...
bool SpeechRecognizerPryonLite::SetDetectionThreshold(int threshold) {
  ...
  if (!PryonLiteDecoder_IsDecoderInitialized(_impl->decoder)) {
    LOG_INFO("SpeechRecognizerPryonLite.SetDetectionThreshold.NotInitialized",
             "Detect threshold will be set on init");
    return true;
  }
  _impl->config.detectThreshold = _detectThreshold;
  const PryonLiteError err = PryonLiteDecoder_SetDetectionThreshold(
      _impl->decoder, "ALEXA", _impl->config.detectThreshold);
  if (err != PRYON_LITE_ERROR_OK) {
    LOG_ERROR("SpeechRecognizerPryonLite.SetDetectionThreshold",
              "PryonLite error %d", err);
    return false;
  }
  return true;
...
bool SpeechRecognizerPryonLite::IsReady() const {
  ...
  // since we keep the wake word engine samples coupled to those samples passed
  // to alexa's mic input, pretend this wake word engine is initialized on mac
  // so that alexa receives any mic input at all
  return true;
#else
  return false;
#endif
...
bool SpeechRecognizerPryonLite::LoadPryonModel(
    const std::string& filePath, SpeechRecognizerPryonLiteData& data) {
#if PRYON_ENABLED
  if (data.config.model != nullptr || data.config.sizeofModel > 0) {
    LOG_ERROR("SpeechRecognizerPryonLite.LoadPryonModel",
  ...
  if (fileStream) {
    ...
    if (fileStream) {
      ...
      LOG_ERROR("SpeechRecognizerPryonLite.LoadPryonModel.ReadFile",
                "Error read %td of %zu bytes", fileStream.gcount(), length);
...
void SpeechRecognizerPryonLite::DetectionCallback(
    PryonLiteDecoderHandle handle, const PryonLiteResult* result) {
#if PRYON_ENABLED
  const auto* recContext =
      static_cast<SpeechRecognizerPryonLite*>(result->userData);
  if (recContext->_impl->disabled) {
    return;
  }

  const auto beginSampleIdx =
      result->beginSampleIndex + recContext->_alexaMicrophoneOffset;
  const auto endSampleIdx =
      result->endSampleIndex + recContext->_alexaMicrophoneOffset;

  const AudioUtil::SpeechRecognizerCallbackInfo info{
      .result = (result->keyword ? result->keyword : ""),
      .startTime_ms =
          static_cast<int>(beginSampleIdx / 16),  // 16 samples per ms
...
void SpeechRecognizerPryonLite::VadCallback(PryonLiteDecoderHandle handle,
                                            const PryonLiteVadEvent* vadEvent) {
#if PRYON_ENABLED
  auto* recContext =
      static_cast<SpeechRecognizerPryonLite*>(vadEvent->userData);
...
void SpeechRecognizerPryonLite::HandleInitFail(const std::string& failMessage) {
  PRINT_NAMED_ERROR("SpeechRecognizerPryonLite.Init.Fail", "%s",
                    failMessage.c_str());
  Cleanup();
}
