 ...
 * Author: Lee Crippen
 * Created: 12/12/2016
 * Updated: 10/29/2017 Simple version rename to differentiate from legacy
 * implementation.
 *
 * Description: SpeechRecognizer implementation for Sensory TrulyHandsFree. The
 * cpp defines the impl struct that is only declared in the header, in order to
 * encapsulate accessing outside headers to only be in this file.
 *
...
namespace {
// THF keyword for "none of the above" that can be used in non-phrasespotted
// grammars and search lists
...
struct SpeechRecognizerTHF::SpeechRecognizerTHFData {
  ...
  // We intentionally don't store off and reuse the pronun object. Attempting to
  // do so during testing resulted in crashes when calling into
  // thfSearchCreateFromGrammar and passing in a common pronun object. The safe
  // way to use the pronun object appears to be creating, using, and then
  // destroying it each time a search object is to be created.
...
SpeechRecognizerTHF::~SpeechRecognizerTHF() { Cleanup(); }
...
SpeechRecognizerTHF& SpeechRecognizerTHF::operator=(
    SpeechRecognizerTHF&& other) {
...
const RecogDataSP
SpeechRecognizerTHF::SpeechRecognizerTHFData::RetrieveDataForIndex(
  ...
  // Intentionally make a local copy of the shared ptr with the current recog
  // data
...
bool SpeechRecognizerTHF::Init(const std::string& pronunPath) {
  ...
  if (nullptr == createdSession) {
    ...
    const char* err = thfGetLastError(NULL)
                          ? thfGetLastError(NULL)
...
bool SpeechRecognizerTHF::AddRecognitionDataFromFile(
    IndexType index, const std::string& nnFilePath,
    const std::string& searchFilePath, bool isPhraseSpotted,
  ...
  auto cleanupAfterFailure = [&](const std::string& failMessage) {
    LOG_ERROR("SpeechRecognizerTHF.AddRecognitionDataFromFile.Fail", "%s %s",
              failMessage.c_str(), thfGetLastError(_impl->thfSession));
  ...
  if (InvalidIndex == index) {
    cleanupAfterFailure(
        std::string(
            "Specified index matches InvalidIndex and cannot be used: ") +
        std::to_string(index));
    return false;
  }
  ...
  auto indexIter = _impl->thfAllRecogs.find(index);
  if (indexIter != _impl->thfAllRecogs.end()) {
    cleanupAfterFailure(std::string("Recognizer already added at index ") +
                        std::to_string(index));
    return false;
  }

  // The code examples had a buffer size as double the standard chunk size, so
  // we'll do the same
  auto bufferSizeInSamples =
      Util::numeric_cast<unsigned short>(AudioUtil::kSamplesPerChunk * 2);

  /* Create recognizer */
  auto doSpeechDetect = isPhraseSpotted ? NO_SDET : SDET;
  createdRecognizer =
      thfRecogCreateFromFile(_impl->thfSession, nnFilePath.c_str(),
                             bufferSizeInSamples, -1, doSpeechDetect);
  ...
  createdSearch =
      thfSearchCreateFromFile(_impl->thfSession, createdRecognizer,
                              searchFilePath.c_str(), numBestResultsToReturn);
  if (nullptr == createdSearch) {
    const char* err = thfGetLastError(_impl->thfSession);
    std::string errorMessage =
        std::string("ERROR thfSearchCreateFromFile ") + err;
    cleanupAfterFailure(errorMessage);
    return false;
  }

  /* Initialize recognizer */
  if (!thfRecogInit(_impl->thfSession, createdRecognizer, createdSearch,
                    RECOG_KEEP_NONE)) {
  ...
  size_t sampleRate_hz =
      thfRecogGetSampleRate(_impl->thfSession, createdRecognizer);
  if (!ANKI_VERIFY(sampleRate_hz != 0, "SpeechRecognizerTHF.Init.NoSampleRate",
  ...
  if (allowsFollowupRecog) {
    if (!isPhraseSpotted) {
      cleanupAfterFailure(
          "Tried to set up phrase following with non-phrasespotting "
          "recognizers, which is not allowed.");
      return false;
    }

    constexpr float overlapTime_ms = 1000.f;
    if (!thfPhrasespotConfigSet(_impl->thfSession, createdRecognizer,
                                createdSearch, PS_SEQ_BUFFER, overlapTime_ms)) {
      cleanupAfterFailure("ERROR thfPhrasespotConfigSet PS_SEQ_BUFFER");
      return false;
    }
  }

  // This delay is recommended when using more complex command sets, but ours is
  // simple for the moment. Keeping this here for now just for reference. if
  // (isPhraseSpotted)
  // {
  //   if (!thfPhrasespotConfigSet(_impl->thfSession, createdRecognizer,
  //   createdSearch, PS_DELAY, 90))
  //   {
  ...
  _impl->thfAllRecogs[index] = MakeRecogDataSP(
      createdRecognizer, createdSearch, isPhraseSpotted, allowsFollowupRecog);
...
bool SpeechRecognizerTHF::RecogStatusIsEndCondition(uint16_t status) {
  switch (status) {
    case RECOG_SILENCE:  // Timed out waiting for start of speech (end
                         // condition).
    case RECOG_DONE:    // End of utterance detected (end condition).
    case RECOG_MAXREC:  // Timed out waiting for end of utterance (end
                        // condition).
    case RECOG_IGNORE:  // Speech detector triggered but failed the minduration
                        // test. Probably not speech (end condition).
...
void SpeechRecognizerTHF::Update(const AudioUtil::AudioSample* audioData,
  ...
  // If the recognizer has changed since last update, we need to potentially
  // reset and store it again
  ...
  if (_impl->reset || currentRecognizer != _impl->lastUsedRecognizer) {
    ...
    if (_impl->reset || _impl->lastUsedRecognizer) {
      if (thfRecogReset(_impl->thfSession, currentRecognizer)) {
        ...
        LOG_ERROR("SpeechRecognizerTHF.Update.thfRecogReset.Fail", "%s",
                  thfGetLastError(_impl->thfSession));
      }
    }
  ...
  auto recogPipeMode =
      currentRecogSP->IsPhraseSpotted() ? RECOG_ONLY : SDET_RECOG;
  unsigned short status = RECOG_SILENCE;
  if (!thfRecogPipe(_impl->thfSession, currentRecognizer, audioDataLen,
                    (short*)audioData, recogPipeMode, &status)) {
    LOG_ERROR("SpeechRecognizerTHF.Update.thfRecogPipe.Fail", "%s",
              thfGetLastError(_impl->thfSession));
    return;
  }
  ...
  if (!sPhraseForceHeard.empty() || RecogStatusIsEndCondition(status)) {
    ...
    if (sPhraseForceHeard.empty()) {
      if (!thfRecogResult(_impl->thfSession, currentRecognizer, &score,
                          &foundStringRaw, &wordAlign, NULL, NULL, NULL, NULL,
                          NULL)) {
        LOG_ERROR("SpeechRecognizerTHF.Update.thfRecogResult.Fail", "%s",
                  thfGetLastError(_impl->thfSession));
      }
    } else {
    ...
    if (foundStringRaw != nullptr && foundStringRaw[0] != '\0' &&
        kNotaString.compare(foundStringRaw) != 0) {
      // Get results for callback struct
      std::string foundString{foundStringRaw};
      std::replace(foundString.begin(), foundString.end(), '_', ' ');
      AudioUtil::SpeechRecognizerCallbackInfo info{.result = foundString,
                                                   .startTime_ms = 0,
                                                   .endTime_ms = 0,
                                                   .score = score};
      ...
      if (wordAlign != nullptr) {
        ...
        if (split.size() >= 2) {
          ...
          info.startSampleIndex = (info.startTime_ms * _impl->sampleRate_kHz) +
                                  _impl->lastResetSampleIndex;
          info.endSampleIndex = (info.endTime_ms * _impl->sampleRate_kHz) +
                                _impl->lastResetSampleIndex;
        }
      }

      DoCallback(info);
      LOG_INFO("SpeechRecognizerTHF.Update", "Recognizer -  %s",
               info.Description().c_str());
    }

    // If the current recognizer allows a followup recognizer to immediately
    // take over
    if (status == RECOG_DONE && currentRecogSP->AllowsFollowupRecog()) {
      // Verify whether we actually have a followup recognizer set
      const RecogDataSP nextRecogSP =
          _impl->RetrieveDataForIndex(_impl->thfFollowupRecog);
      if (nextRecogSP) {
        // Actually do the switch over to the new recognizer (as long as this
        // phrase wasn't forced), which copies some buffered audio data
        if (!sPhraseForceHeard.empty() ||
            thfRecogPrepSeq(_impl->thfSession, nextRecogSP->GetRecognizer(),
                            currentRecognizer)) {
          std::lock_guard<std::recursive_mutex>(_impl->recogMutex);
          ...
          LOG_ERROR("SpeechRecognizerTHF.Update.thfRecogPrepSeq.Fail", "%s",
                    thfGetLastError(_impl->thfSession));
        }
      }
    }
...
void SpeechRecognizerTHF::StopInternal() { _impl->disabled = true; }

void SpeechRecognizerTHF::Reset() { _impl->reset = true; }
