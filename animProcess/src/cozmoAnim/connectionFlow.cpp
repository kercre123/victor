...
bool DrawStartPairingScreen(Anim::AnimationStreamer* animStreamer) {
  ...
  img->DrawTextCenteredHorizontally(robotName, CV_FONT_NORMAL, kRobotNameScale,
                                    1, kColor, 15, false);

  cv::Size textSize;
  float scale = 0;
  Vision::Image::MakeTextFillImageWidth(kURL, CV_FONT_NORMAL, 1,
                                        img->GetNumCols(), textSize, scale);
  img->DrawTextCenteredHorizontally(kURL, CV_FONT_NORMAL, scale, 1, kColor,
                                    (FACE_DISPLAY_HEIGHT + textSize.height) / 2,
                                    true);
...
void DrawShowPinScreen(Anim::AnimationStreamer* animStreamer,
                       const Anim::AnimContext* context,
                       const std::string& pin) {
  s_enteredAnyScreen = true;

  Vision::ImageRGB key;
  key.Load(context->GetDataLoader()->GetSpritePaths()->GetAssetPath(
      kShowPinScreenSpriteName));
  ...
  img->DrawTextCenteredHorizontally(OSState::getInstance()->GetRobotName(),
                                    CV_FONT_NORMAL, kRobotNameScale, 1, kColor,
                                    15, false);

  img->DrawTextCenteredHorizontally(pin, CV_FONT_NORMAL, 0.8f, 1, kColor,
                                    FACE_DISPLAY_HEIGHT - 5, false);
...
void DrawWifiScreen(Anim::AnimationStreamer* animStreamer) {
  ...
  animStreamer->SetStreamingAnimation("anim_pairing_icon_wifi", 0, 0, 0,
                                      shouldInterrupt);
}
...
void DrawUpdatingOSScreen(Anim::AnimationStreamer* animStreamer) {
  ...
  const bool shouldInterrupt = true;
  animStreamer->SetStreamingAnimation("anim_pairing_icon_update", 0, 0, 0,
                                      shouldInterrupt);
}
...
void DrawUpdatingOSErrorScreen(Anim::AnimationStreamer* animStreamer) {
  ...
  const bool shouldInterrupt = true;
  animStreamer->SetStreamingAnimation("anim_pairing_icon_update_error", 0, 0, 0,
                                      shouldInterrupt);
}
...
void DrawWaitingForAppScreen(Anim::AnimationStreamer* animStreamer) {
  ...
  const bool shouldInterrupt = true;
  animStreamer->SetStreamingAnimation("anim_pairing_icon_awaitingapp", 0, 0, 0,
                                      shouldInterrupt);
}

void SetBLEPin(uint32_t pin) { _pin = pin; }
...
void UpdateConnectionFlow(const SwitchboardInterface::SetConnectionStatus& msg,
  ...
  PRINT_NAMED_INFO("ConnectionFlow.UpdateConnectionFlow.NewStatus", "%s",
                   EnumToString(msg.status));

  // isPairing is a proxy for "switchboard is doing something and needs to
  // display something on face"
  ...
  const bool shouldControlFace =
      isPairing || (msg.status == ConnectionStatus::SHOW_URL_FACE);

  // Enable pairing screen if status is anything besides NONE, COUNT, and
  // END_PAIRING Should do nothing if called multiple times with same argument
  // such as when transitioning from START_PAIRING to SHOW_PRE_PIN
  FaceInfoScreenManager::getInstance()->EnablePairingScreen(isPairing);

  // Disable face keepalive, but don't re-enable it when ending pairing. The
  // engine will send a message when it's ready to re-enable it, since it needs
  // time to send its first animation upon resuming
  if (shouldControlFace) {
    animStreamer->Abort();
    animStreamer->EnableKeepFaceAlive(false, 0);

    // Always look up since we're displaying something that user will want to
    // see
