...
Anki::Util::Data::DataPlatform* createPlatform() {
  ...
  if (config.isMember("DataPlatformPersistentPath")) {
    ...
    LOG_ERROR(
        "cozmoAnimMain.createPlatform.DataPlatformPersistentPathUndefined", "");
  }
...
int main(void) {
  ...
  // - console filter for logs
  {
    ...
    const std::string& consoleFilterConfigPath =
        "config/engine/console_filter_config.json";
    if (!dataPlatform->readAsJson(Anki::Util::Data::Scope::Resources,
                                  consoleFilterConfigPath,
                                  consoleFilterConfig)) {
      LOG_ERROR("CozmoAnimMain.main", "Failed to parse json file '%s'",
  ...
  ANKI_CONSOLE_SYSTEM_INIT(dataPlatform
                               ->pathToResource(Anki::Util::Data::Scope::Cache,
                                                "consoleVarsAnim.ini")
                               .c_str());
  ...
  if (RESULT_OK != result) {
    LOG_ERROR("CozmoAnimMain.main.InitFailed", "Unable to initialize (exit %d)",
              result);
    delete animEngine;
  ...
  while (!gShutdown) {
    ...
    const BaseStationTime_t curTime_ns = Util::numeric_cast<BaseStationTime_t>(
        Util::SecToNanoSec(curTime_s.count()));

    result = animEngine->Update(curTime_ns);
    if (RESULT_OK != result) {
      LOG_WARNING("CozmoAnimMain.main.UpdateFailed",
                  "Unable to update (result %d)", result);
    ...
    const auto remaining_us = duration_cast<microseconds>(
        targetEndFrameTime - tickAfterAnimExecution);
    const auto tickDuration_us =
        duration_cast<microseconds>(tickAfterAnimExecution - tickStart);
    ...
    if (remaining_us < microseconds(-ANIM_OVERTIME_WARNING_THRESH_US)) {
      LOG_WARNING("CozmoAnimMain.overtime",
                  "Update() (%dms max) is behind by %.3fms", ANIM_TIME_STEP_MS,
    ...
    // See if we've fallen very far behind (this happens e.g. after a 5-second
    // blocking load operation); if so, compensate by catching the target frame
    // end time up somewhat. This is so that we don't spend the next SEVERAL
    // frames catching up.
    ...
    const auto sleepTimeActual_us =
        duration_cast<microseconds>(tickStart - tickAfterAnimExecution);
    animEngine->RegisterTickPerformance(
