...
class BeatDetector {
 public:
  ...
  bool AddSamples(const AudioUtil::AudioSample* const samples,
 ...
 private:
...
#else
  // The downsampling process in syscon results in an actual sample rate of
  // 15625 Hz
  ...
  // Stages audio data to be piped into the aubio detector at the correct chunk
  // size. Use twice the capacity we actually need just to be safe.
  Util::FixedCircularBuffer<AudioUtil::AudioSample,
                            2 * (kAubioTempoHopSize +
                                 MicData::kSamplesPerBlockPerChannel)>
      _aubioInputBuffer;
};
