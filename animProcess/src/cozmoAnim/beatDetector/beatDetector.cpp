...
namespace {
// This scale factor gets applied to the output tempo estimate of the aubio
// tempo detector. It has been seen that the tempo detector usually reports a
// tempo ~1.2% higher than the actual tempo when using the _processed_ audio
// stream, and about 1.7% higher than actual when using the _raw_ audio stream
// (see kBeatDetectorUseProcessedAudio).
...
BeatInfo kInvalidBeatInfo{
    .tempo_bpm = -1.f, .confidence = -1.f, .time_sec = -1.f};
}  // namespace

BeatDetector::BeatDetector() : _latestBeat(kInvalidBeatInfo) { Start(); }

BeatDetector::~BeatDetector() { Stop(); }

bool BeatDetector::AddSamples(const AudioUtil::AudioSample* const samples,
  ...
  DEV_ASSERT(_aubioTempoDetector != nullptr,
             "BeatDetector.AddSamples.NullAubioTempoObject");

  // Place new data into the staging buffer
  DEV_ASSERT(
      _aubioInputBuffer.capacity() - _aubioInputBuffer.size() >= nSamples,
      "BeatDetector.AddSamples.AubioInputBufferIsFull");
  _aubioInputBuffer.push_back(samples, nSamples);

  // Feed the aubio tempo detector correct-sized chunks (size
  // kAubioTempoHopSize)
  ...
  while (_aubioInputBuffer.size() >= kAubioTempoHopSize) {
    for (int i = 0; i < kAubioTempoHopSize; i++) {
      ...
      auto sample = static_cast<smpl_t>(_aubioInputBuffer.front()) /
                    (std::numeric_limits<AudioUtil::AudioSample>::max() + 1);
    ...
    if (isBeat) {
      ...
      const auto conf = aubio_tempo_get_confidence(_aubioTempoDetector);
      {
        ...
        _latestBeat.time_sec =
            now_sec;  // NOTE: this is approximate. Should really do math with
...
bool BeatDetector::IsRunning() { return (_aubioTempoDetector != nullptr); }
...
void BeatDetector::Start() {
  ...
  _aubioTempoDetector =
      new_aubio_tempo(kAubioTempoMethod, kAubioTempoBufSize, kAubioTempoHopSize,
                      kAubioTempoSampleRate);
