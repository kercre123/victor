 ...
 * Created: 09/12/17
 *
 * Description: Animation Audio Client is the interface to perform animation
 * audio specific tasks. Provided a RobotAudioKeyFrame to handle the necessary
 * audio functionality for that frame. It also provides an interface to abort
 * animation audio and update (a.k.a. “tick”) the Audio Engine each frame.
 *
...
class AnimationAudioClient {
 public:
  ...
  void PlayAudioKeyFrame(const RobotAudioKeyFrame& keyFrame,
                         Util::RandomGenerator* randomGen);
 ...
 private:
  ...
  // Key frame types
  void HandleAudioRef(const AudioKeyFrameType::AudioEventGroupRef& eventRef,
                      Util::RandomGenerator* randomGen = nullptr);
  ...
  void CozmoEventCallback(const uint8_t ttsID,
