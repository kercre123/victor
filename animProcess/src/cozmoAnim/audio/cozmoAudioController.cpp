...
const std::set<APT> kConsumableParameters = {APT::Robot_Vic_Meter_Bus_Sfx,
                                             APT::Robot_Vic_Meter_Bus_Tts,
                                             APT::Robot_Vic_Meter_Bus_Vo};
...
static void AudioEngineLogCallback(uint32_t, const char*, ErrorLevel,
                                   AudioPlayingId, AudioGameObject);
#endif
...
CONSOLE_VAR_RANGED(uint8_t, kWriteAudioProfilerMaxLogCount, CONSOLE_PATH, 3, 1,
                   5);
CONSOLE_VAR_RANGED(uint8_t, kWriteAudioOutputMaxLogCount, CONSOLE_PATH, 1, 1,
                   5);
...
void DeleteAudioProfilerCaptures(ConsoleFunctionContextRef context) {
  const auto files = Util::FileUtils::FilesInDirectory(
      sWritePath, true, kProfilerCaptureFileExtension.c_str());
...
void DeleteAudioOutputCaptures(ConsoleFunctionContextRef context) {
  const auto files = Util::FileUtils::FilesInDirectory(
      sWritePath, true, kAudioOutputCaptureFileExtension.c_str());
  for (const auto& file : files) {
    Util::FileUtils::DeleteFile(file);
  }
}
...
void TestAudio_PinkNoise(ConsoleFunctionContextRef context) {
  if (sThis != nullptr) {
    sThis->PostAudioEvent(
        ToAudioEventId(
            AudioMetaData::GameEvent::GenericEvent::Play__Dev_Robot__Pink_1Sec),
...
void PostAudioEvent(ConsoleFunctionContextRef context) {
  if (sThis != nullptr) {
    ...
    const auto defaultGameObj =
        static_cast<uint64_t>(AudioMetaData::GameObjectType::Default);
    const uint64_t gameObjectId =
        ConsoleArg_GetOptional_UInt64(context, "gameObjectId", defaultGameObj);
...
void SetAudioSwitchState(ConsoleFunctionContextRef context) {
  if (sThis != nullptr) {
    ...
    const char* state = ConsoleArg_Get_String(context, "state");
    const uint64_t gameObjectId =
        ConsoleArg_Get_UInt64(context, "gameObjectId");
    sThis->SetSwitchState(
        AudioEngineController::GetAudioIdFromString(switchGroup),
        AudioEngineController::GetAudioIdFromString(state), gameObjectId);
  }
}
...
void SetAudioParameter(ConsoleFunctionContextRef context) {
  if (sThis != nullptr) {
    ...
    uint64_t gameObjectId = ConsoleArg_GetOptional_UInt64(
        context, "gameObjectId", kInvalidAudioGameObject);
    sThis->SetParameter(AudioEngineController::GetAudioIdFromString(parameter),
                        value, gameObjectId);
  }
}
...
CONSOLE_FUNC(PostAudioEvent, CONSOLE_PATH, const char* event,
             optional uint64 gameObjectId);
CONSOLE_FUNC(SetAudioState, CONSOLE_PATH, const char* stateGroup,
             const char* state);
CONSOLE_FUNC(SetAudioSwitchState, CONSOLE_PATH, const char* switchGroup,
             const char* state, uint64 gameObjectId);
CONSOLE_FUNC(SetAudioParameter, CONSOLE_PATH, const char* parameter,
             float value, optional uint64 gameObjectId);
...
CozmoAudioController::CozmoAudioController(const Anim::AnimContext* context)
  ...
  SetupConfig config{};
  {
    DEV_ASSERT(nullptr != _animContext,
               "CozmoAudioController.CozmoAudioController.AnimContext.IsNull");

    const Util::Data::DataPlatform* dataPlatform =
        _animContext->GetDataPlatform();
    const std::string assetPath =
        dataPlatform->pathToResource(Util::Data::Scope::Resources, "sound");
    sWritePath =
        dataPlatform->pathToResource(Util::Data::Scope::Cache, "sound");
    PRINT_CH_INFO("Audio", "CozmoAudioController.CozmoAudioController",
                  "AssetPath '%s'", assetPath.c_str());
    PRINT_CH_INFO("Audio", "CozmoAudioController.CozmoAudioController",
                  "WritePath '%s'", sWritePath.c_str());
    // If assets don't exist don't init the Audio engine
    const bool assetsExist = Util::FileUtils::DirectoryExists(assetPath);
    if (!assetsExist) {
      PRINT_NAMED_ERROR("CozmoAudioController.CozmoAudioController",
                        "Audio Assets do NOT exist - Ignore if Unit Test");
      return;
    }
    ...
    // Locale-specific adjustments are made by setting
    // GameState::External_Language below.
    ...
    // If we're using the audio engine, assert that it was successfully
    // initialized.
    DEV_ASSERT(IsInitialized(),
               "CozmoAudioController.Initialize Audio Engine fail");
  }

  // The audio engine was initialized correctly, so now let's setup everything
  // else
  if (IsInitialized()) {
    ...
    GetPluginInterface()->SetupAkAlsaSinkPlugIn(
        config.threadLowEngine.affinityMask);
#endif
    ...
    SetDefaultListeners(
        {ToAudioGameObject(AudioMetaData::GameObjectType::Victor_Listener)});
    SetupConsumableAudioParameters();

    using namespace AudioMetaData::SwitchState;

    SetSwitchState(
        ToAudioSwitchGroupId(SwitchGroupType::Robot_Vic_External_Input_Source),
        ToAudioSwitchStateId(
            (GenericSwitch)
                Robot_Vic_External_Input_Source::Streaming_Wave_Portal),
  ...
  PRINT_NAMED_WARNING(
      "CozmoAudioController",
      "Audio Engine is disabled - EXCLUDE_ANKI_AUDIO_LIBS flag is set");
...
bool CozmoAudioController::WriteProfilerCapture(bool write) {
  ...
  if (write) {
    // Remove old captures
    const auto maxLogCount =
        Console::kWriteAudioProfilerMaxLogCount - 1;  // Make room for new file
    ...
    uniqueName = kProfilerCaptureFileName + '_' + dateTimeStr + '.' +
                 kProfilerCaptureFileExtension;
  }
...
bool CozmoAudioController::WriteAudioOutputCapture(bool write) {
  ...
  if (write) {
    // Remove old captures
    const auto maxLogCount =
        Console::kWriteAudioOutputMaxLogCount - 1;  // Make room for new file
    RemoveCaptureFiles(sWritePath, kAudioOutputCaptureFileExtension,
                       maxLogCount);
    // Create new log file name
    const auto dateTimeStr = CreateFormattedUtcDateTimeString();
    uniqueName = kAudioOutputCaptureFileName + '_' + dateTimeStr + '.' +
                 kAudioOutputCaptureFileExtension;
  }
...
void CozmoAudioController::RemoveCaptureFiles(const std::string& dirPath,
  ...
  const auto files =
      FileUtils::FilesInDirectory(dirPath, true, fileExtension.c_str());
  if (files.size() > maxCount) {
    // Need to delete files
    using fileTime_t = unsigned long;
    std::map<fileTime_t, const std::string&, std::greater<fileTime_t> >
        fileTimeMap;
    // Get file creation time
    for (const auto& file : files) {
      struct stat info;
      if (stat(file.c_str(), &info) != 0) {
        PRINT_NAMED_WARNING("CozmoAudioController.RemoveCaptureFiles",
                            "Unable to get file info '%s'", file.c_str());
    ...
    if (fileTimeMap.size() > maxCount) {
      ...
      std::advance(deleteIt,
                   maxCount);  // Go to the first element we want to delete
...
std::string CozmoAudioController::CreateFormattedUtcDateTimeString() {
  ...
  if (nullptr == time) {
    PRINT_NAMED_ERROR(
        "CozmoAudioController.CreateFormattedUtcDateTimeString.UTC.Invalid",
        "gmtime returned null. Error: %s", strerror(errno));
    return "";
  }
  // Creat Data String "00-00-00_00-00-00_XXX"
  std::stringstream ss;
  ss << std::setfill('0') << std::setw(2) << (time->tm_mon + 1) << '-'
     << std::setw(2) << time->tm_mday << '-' << std::setw(2)
     << (time->tm_year - 100) << '_' << std::setw(2) << time->tm_hour << '-'
     << std::setw(2) << time->tm_min << '-' << time->tm_sec << '_'
     << time->tm_zone;
  return ss.str();
}
...
bool CozmoAudioController::ActivateParameterValueUpdates(bool activate) {
  ...
  if (activate) {
    ...
    AudioEngineCallbackFunc callbackFunc =
        [this](AudioEngineCallbackId callbackId,
          ...
          for (auto& paramKVP : _consumableParameterValues) {
            ...
            bool success = this->GetParameterValue(
                ToAudioParameterId(paramKVP.first), kInvalidAudioGameObject,
                kInvalidAudioPlayingId, val, type);
            if (success) {
              paramKVP.second = val;
            }
          }
        };
    _parameterUpdateCallbackId = RegisterGlobalCallback(
        AudioEngine::AudioEngineCallbackFlag::EndFrameRender,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool CozmoAudioController::GetActivatedParameterValue(
...
void CozmoAudioController::RegisterCladGameObjectsWithAudioController() {
  ...
  for (uint32_t aGameObj =
           static_cast<AudioGameObject>(GameObjectType::Default);
       aGameObj < static_cast<uint32_t>(GameObjectType::End); ++aGameObj) {
    // Register GameObjectType
    bool success = RegisterGameObject(
        static_cast<AudioGameObject>(aGameObj),
        std::string(EnumToString(static_cast<GameObjectType>(aGameObj))));
    if (!success) {
      PRINT_NAMED_ERROR(
          "CozmoAudioController.RegisterCladGameObjectsWithAudioController",
          "Registering GameObjectId: %ul - %s was unsuccessful", aGameObj,
          EnumToString(static_cast<GameObjectType>(aGameObj)));
    }
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
AudioPlayingId CozmoAudioController::PostAudioEvent(
    const std::string& eventName, AudioGameObject gameObjectId,
    AudioCallbackContext* callbackContext) {
  AudioPlayingId ret = AudioEngineController::PostAudioEvent(
      eventName, gameObjectId, callbackContext);
  ...
  if (ANKI_DEV_CHEATS) {
    ...
    if ((webservice != nullptr) &&
      ...
      toSend["gameObjectId"] =
          ToString(static_cast<AudioMetaData::GameObjectType>(gameObjectId));
      toSend["hasCallback"] = (callbackContext != nullptr);
      // Note: this hypothetically could flood wifi, but only if the webviz tab
      // is open. Ideally there would be an update call in this class to flush
      // accumulated events. We can add one if this ends up being problematic,
      // possibly one that is called from the webviz update loop. Same goes for
      // all the other methods below
...
// - - - - - - - - - - - - - - - - - - - -
AudioPlayingId CozmoAudioController::PostAudioEvent(
    AudioEventId eventId, AudioGameObject gameObjectId,
    AudioCallbackContext* callbackContext) {
  AudioPlayingId ret = AudioEngineController::PostAudioEvent(
      eventId, gameObjectId, callbackContext);
  ...
  if (ANKI_DEV_CHEATS) {
    ...
    if ((webservice != nullptr) &&
      ...
      toSend["type"] = "PostAudioEvent";
      toSend["time"] = BaseStationTimer::getInstance()->GetCurrentTimeStamp();
      toSend["eventName"] = ToString(
          static_cast<AudioMetaData::GameEvent::GenericEvent>(eventId));
      toSend["gameObjectId"] =
          ToString(static_cast<AudioMetaData::GameObjectType>(gameObjectId));
      toSend["hasCallback"] = (callbackContext != nullptr);
      webservice->SendToWebViz(kWebVizModuleName, toSend);
...
bool CozmoAudioController::SetState(AudioStateGroupId stateGroupId,
  ...
  if (ANKI_DEV_CHEATS) {
    ...
    if ((webservice != nullptr) &&
      ...
      toSend["time"] = BaseStationTimer::getInstance()->GetCurrentTimeStamp();
      toSend["stateGroupId"] = ToString(
          static_cast<AudioMetaData::GameState::StateGroupType>(stateGroupId));
...
bool CozmoAudioController::SetSwitchState(AudioSwitchGroupId switchGroupId,
  ...
  bool ret = AudioEngineController::SetSwitchState(switchGroupId, switchStateId,
                                                   gameObjectId);
  ...
  if (ANKI_DEV_CHEATS) {
    ...
    if ((webservice != nullptr) &&
      ...
      toSend["switchGroupId"] =
          ToString(static_cast<AudioMetaData::SwitchState::SwitchGroupType>(
              switchGroupId));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Setup Ak Logging callback
void AudioEngineLogCallback(uint32_t akErrorCode, const char* errorMessage,
                            AudioEngine::ErrorLevel errorLevel,
                            AudioEngine::AudioPlayingId playingId,
                            AudioEngine::AudioGameObject gameObjectId) {
  std::ostringstream logStream;
  logStream << "ErrorCode: " << akErrorCode << " Message: '"
            << ((nullptr != errorMessage) ? errorMessage : "")
            << "' LevelBitFlag: " << (uint32_t)errorLevel
            << " PlayingId: " << playingId << " GameObjId: " << gameObjectId;

  if (((uint32_t)errorLevel & (uint32_t)ErrorLevel::Message) ==
      (uint32_t)ErrorLevel::Message) {
    PRINT_CH_INFO(CozmoAudioController::kLogChannelName,
                  "CozmoAudioController.AudioEngineLog", "%s",
                  logStream.str().c_str());
  }

  if (((uint32_t)errorLevel & (uint32_t)ErrorLevel::Error) ==
      (uint32_t)ErrorLevel::Error) {
    PRINT_NAMED_WARNING("CozmoAudioController.AudioEngineError", "%s",
                        logStream.str().c_str());
  }
}
#endif
