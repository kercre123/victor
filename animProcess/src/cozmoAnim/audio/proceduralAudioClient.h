...
class ProceduralAudioClient {
 ...
 private:
  ...
  enum class FrameState { Stopped, PendingStart, Started };
  ...
  void UpdateTreadState(const AudioProceduralFrame& previousFrame,
                        const AudioProceduralFrame& currentFrame);
  void UpdateHeadState(const AudioProceduralFrame& previousFrame,
                       const AudioProceduralFrame& currentFrame);
  void UpdateLiftState(const AudioProceduralFrame& previousFrame,
                       const AudioProceduralFrame& currentFrame);

  // Helper method to identify movement state changes
  // return true of inOut_currentState was updated
  bool FrameStateUpdate(bool inCoolDown, bool isMoving,
                        FrameState& inOut_currentState, bool& out_SetRtpc);
};
