...
class SdkAudioComponent {
 public:
  ...
  //
  // CLAD message handlers are called on the main thread to handle incoming
  // requests.
  //
 ...
 private:
  ...
  using StreamingWaveDataPtr =
      std::shared_ptr<AudioEngine::StreamingWaveDataInstance>;
  ...
  bool SendAnimToEngine(SDKAudioStreamingState audioState,
                        uint32_t audioSent = 0, uint32_t audioPlayed = 0);
};
