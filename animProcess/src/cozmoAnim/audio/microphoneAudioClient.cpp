...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
MicrophoneAudioClient::~MicrophoneAudioClient() { _audioController = nullptr; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void MicrophoneAudioClient::ProcessMessage(
    const RobotInterface::MicDirection& msg) {
  if (nullptr != _audioController) {
    const float noiseFloor = log10(msg.latestNoiseFloor);
    float normNoiseFloor =
        MAX((noiseFloor - kNoiseFloorMin), 0.0f);  // Shift floor down
    normNoiseFloor = MIN((normNoiseFloor / kNoiseFloorRange),
                         1.0f);  // Normalize and limit ceiling

    using AudioParameter = AudioMetaData::GameParameter::ParameterType;
    const auto param = AudioEngine::ToAudioParameterId(
        AudioParameter::Robot_Vic_Environment_Ambient_Volume);
    _audioController->SetParameter(
        param, normNoiseFloor,
