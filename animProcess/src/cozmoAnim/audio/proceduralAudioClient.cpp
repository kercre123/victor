 ...
 * Created: 03/15/18
 *
 * Description: Procedural Audio Client handles robot state driven audio
 *features. By intercepting robot to engine messages the audio client can track
 *robot state and events to perform audio tasks.
 *
...
#define TREAD_LOG(msg, ...)                                          \
  PRINT_NAMED_WARNING("ProceduralAudioClient.UpdateTreadState", msg, \
                      ##__VA_ARGS__)
#else
#define TREAD_LOG(msg, ...)
#endif
...
#if ENABLE_HEAD_LOG
#define HEAD_LOG(msg, ...)                                          \
  PRINT_NAMED_WARNING("ProceduralAudioClient.UpdateHeadState", msg, \
                      ##__VA_ARGS__)
#else
#define HEAD_LOG(msg, ...)
#endif
...
#if ENABLE_LIFT_LOG
#define LIFT_LOG(msg, ...)                                          \
  PRINT_NAMED_WARNING("ProceduralAudioClient.UpdateLiftState", msg, \
                      ##__VA_ARGS__)
#else
#define LIFT_LOG(msg, ...)
#endif
...
ProceduralAudioClient::ProceduralAudioClient(
    CozmoAudioController* audioController)
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ProceduralAudioClient::ProcessMessage(
  ...
  switch (msg.tag) {
    ...
    case RobotToEngine::Tag_syncRobotAck: {
      // Wait for syncRobotAck to indicate the robot's motors are settled and
      // ready to start making noise NOTE: This may need improvments, perhaps
      // use motor calibration messages
      _isActive = true;
      break;
    }
    default:
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ProceduralAudioClient::HandleStateMessage(
    const RobotInterface::RobotToEngine& msg) {
...
void ProceduralAudioClient::UpdateTreadState(
  ...
  // If the spinSpeedRTPC is > 0 audio engine interprets it as a point turn,
  // otherwise it uses tread speed
  static const float kDefaultSpinRtpc = -0.01f;
  auto treadEvent = GE::Invalid;
  bool setRTPC = false;
  const bool inCoolDown =
      (currentFrame.GetTimeStamp_ms() < _treadCoolDownExpiresTimeStamp_ms);
  const bool stateUpdated = FrameStateUpdate(
      inCoolDown, currentFrame.IsTreadMoving(), _treadFrameState, setRTPC);
  if (stateUpdated) {
    switch (_treadFrameState) {
      case FrameState::Stopped: {
        treadEvent = GE::Stop__Robot_Vic_Sfx__Tread_Loop_Stop;
        _treadCoolDownExpiresTimeStamp_ms =
            currentFrame.GetTimeStamp_ms() + kTreadCoolDown_ms;
        TREAD_LOG("Stop");
        break;
      }
  ...
  if (setRTPC) {
    ...
    const float maxWheelSpeed =
        Util::Max(currentFrame.GetLeftTreadSpeed_mmps(),
                  currentFrame.GetRightTreadSpeed_mmps());
    const float minWheelSpeed =
        Util::Min(currentFrame.GetLeftTreadSpeed_mmps(),
                  currentFrame.GetRightTreadSpeed_mmps());

    if ((turnSpeedAbs > maxWheelSpeed) &&
        (turnSpeedAbs > Util::Abs(minWheelSpeed))) {
      // Point Turn
      spinSpeedRTPC = currentFrame.GetNormalizedTurnSpeed();
    }
    _audioController->SetParameter(
        ToAudioParameterId(GP::Robot_Vic_Tread_Speed),
        currentFrame.GetNormalizedTreadSpeed(), kProceduralGameObj);
    _audioController->SetParameter(
        ToAudioParameterId(GP::Robot_Vic_Tread_Accelerate),
        currentFrame.GetNormalizedTreadAcceleration(), kProceduralGameObj);
    _audioController->SetParameter(
        ToAudioParameterId(GP::Robot_Vic_Tread_Spin_Speed), spinSpeedRTPC,
        kProceduralGameObj);
    TREAD_LOG("FRAME: %s", currentFrame.CvsLogFrameData().c_str());
  }

  if (GE::Invalid != treadEvent) {
    _audioController->PostAudioEvent(AudioEngine::ToAudioEventId(treadEvent),
                                     kProceduralGameObj);
    // After stop event reset RTPCs
    if (GE::Stop__Robot_Vic_Sfx__Tread_Loop_Stop == treadEvent) {
      _audioController->SetParameter(
          ToAudioParameterId(GP::Robot_Vic_Tread_Speed), 0.0f,
          kProceduralGameObj);
      _audioController->SetParameter(
          ToAudioParameterId(GP::Robot_Vic_Tread_Accelerate), 0.0f,
          kProceduralGameObj);
      _audioController->SetParameter(
          ToAudioParameterId(GP::Robot_Vic_Tread_Spin_Speed), kDefaultSpinRtpc,
          kProceduralGameObj);
    }
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ProceduralAudioClient::UpdateHeadState(
    const AudioProceduralFrame& previousFrame,
    const AudioProceduralFrame& currentFrame) {
  if (!kEnableHeadProceduralMovement) {
    return;
  }
  ...
  bool setRTPC = false;
  const bool inCoolDown =
      (currentFrame.GetTimeStamp_ms() < _headCoolDownExpiresTimeStamp_ms);
  const bool stateUpdated = FrameStateUpdate(
      inCoolDown, currentFrame.IsHeadMoving(), _headFrameState, setRTPC);
  if (stateUpdated) {
    switch (_headFrameState) {
      case FrameState::Stopped: {
        headEvent = GE::Stop__Robot_Vic_Sfx__Head_Loop_Stop;
        _headCoolDownExpiresTimeStamp_ms =
            currentFrame.GetTimeStamp_ms() + kHeadCoolDown_ms;
        HEAD_LOG("Stop");
        break;
      }
  ...
  if (setRTPC) {
    _audioController->SetParameter(ToAudioParameterId(GP::Robot_Vic_Head_Speed),
                                   ...
                                   kProceduralGameObj);
    _audioController->SetParameter(
        ToAudioParameterId(GP::Robot_Vic_Head_Accelerate),
        currentFrame.GetNormalizedHeadAcceleration(), kProceduralGameObj);
    HEAD_LOG("FRAME: %s", currentFrame.CvsLogFrameData().c_str());
  }

  if (GE::Invalid != headEvent) {
    _audioController->PostAudioEvent(ToAudioEventId(headEvent),
                                     kProceduralGameObj);
    // After stop event reset RTPCs
    if (GE::Stop__Robot_Vic_Sfx__Head_Loop_Stop == headEvent) {
      _audioController->SetParameter(
          ToAudioParameterId(GP::Robot_Vic_Head_Speed), 0.0f,
          kProceduralGameObj);
      _audioController->SetParameter(
          ToAudioParameterId(GP::Robot_Vic_Head_Accelerate), 0.0f,
          kProceduralGameObj);
    }
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void ProceduralAudioClient::UpdateLiftState(
    const AudioProceduralFrame& previousFrame,
    const AudioProceduralFrame& currentFrame) {
  if (!kEnableLiftProceduralMovement) {
    return;
  }
  ...
  bool setRTPC = false;
  const bool inCoolDown =
      (currentFrame.GetTimeStamp_ms() < _liftCoolDownExpiresTimeStamp_ms);
  const bool stateUpdated = FrameStateUpdate(
      inCoolDown, currentFrame.IsLiftMoving(), _liftFrameState, setRTPC);
  if (stateUpdated) {
    switch (_liftFrameState) {
      case FrameState::Stopped: {
        liftEvent = GE::Stop__Robot_Vic_Sfx__Lift_Loop_Stop;
        _liftCoolDownExpiresTimeStamp_ms =
            currentFrame.GetTimeStamp_ms() + kLiftCoolDown_ms;
        LIFT_LOG("Stop");
        break;
      }
  ...
  if (setRTPC) {
    _audioController->SetParameter(ToAudioParameterId(GP::Robot_Vic_Lift_Speed),
                                   ...
                                   kProceduralGameObj);
    _audioController->SetParameter(
        ToAudioParameterId(GP::Robot_Vic_Lift_Accelerate),
        currentFrame.GetNormalizedLiftAcceleration(), kProceduralGameObj);
    LIFT_LOG("FRAME: %s", currentFrame.CvsLogFrameData().c_str());
  }

  if (GE::Invalid != liftEvent) {
    _audioController->PostAudioEvent(ToAudioEventId(liftEvent),
                                     kProceduralGameObj);
    // After stop event reset RTPCs
    if (GE::Stop__Robot_Vic_Sfx__Lift_Loop_Stop == liftEvent) {
      _audioController->SetParameter(
          ToAudioParameterId(GP::Robot_Vic_Lift_Speed), 0.0f,
          kProceduralGameObj);
      _audioController->SetParameter(
          ToAudioParameterId(GP::Robot_Vic_Lift_Accelerate), 0.0f,
          kProceduralGameObj);
    }
  }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool ProceduralAudioClient::FrameStateUpdate(bool inCoolDown, bool isMoving,
