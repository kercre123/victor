...
AudioPlaybackSystem::AudioPlaybackSystem(const Anim::AnimContext* context)
    : _animContext(context), _isJobLoading(false) {}
...
void AudioPlaybackSystem::Update(BaseStationTime_t currTime_nanosec) {
  ...
  // if we're not working on a current job and we have more in the queue, start
  // up the next job
  if (!_currentJob) {
    StartNextJobInQueue();
  }
...
void AudioPlaybackSystem::PlaybackAudio(const std::string& path) {
  if (IsValidFile(path)) {
    ...
    // note: we can only play one audio clip at a time, so the audio jobs will
    // queue until it reaches the front
    //       of the line.
  }
}
...
bool AudioPlaybackSystem::IsValidFile(const std::string& path) const {
  ...
  // shouldn't be a problem, but if we ever hit this we'll need to find another
  // solution
  DEV_ASSERT(path.length() <= 255,
             "AudioPlaybackSystem path is too long for AnimToEngine message");
...
void AudioPlaybackSystem::StartNextJobInQueue() {
  ...
  if (!_jobQueue.empty()) {
    // note: currently this wont stop the current job from playing as it's
    // playing on it's own thread.
    //       maybe we want the ability to stop a playing job, but for now
    //       ignoring it
    _currentJob.reset(_jobQueue.front());
    _jobQueue.pop();

    // this is just a single-run thread with no loop, so simply detach and let
    // it do it's thing might be worth looking into having it on a timeout to
    // ensure it doesn't stall for some reason, but I'm not worried about that
    // at this time.
    _isJobLoading = true;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AudioPlaybackSystem::LoadAudioPlaybackData(
    std::shared_ptr<AudioPlaybackJob> audiojob) {
  // note: this function is called from it's own thread

  // a job is considered complete as soon as it loads the audio data
  // when the job is complete, it's up to the AudioPlaybackSystem to deal with
  // playback
...
void AudioPlaybackSystem::BeginAudioPlayback() {
  ...
  DEV_ASSERT_MSG(nullptr != data, "AudioPlaybackSystem",
                 "Data was not properly loaded");

  CozmoAudioController* audioController = _animContext->GetAudioController();
  PlugIns::AnkiPluginInterface* const plugin =
      audioController->GetPluginInterface();
  DEV_ASSERT_MSG(nullptr != plugin, "AudioPlaybackSystem",
  ...
  DEV_ASSERT_MSG(plugin->WavePortalHasAudioDataInfo(), "AudioPlaybackSystem",
                 "Data NOT transferred to audio plugin");
  ...
  using namespace Anki::AudioMetaData;
  {
    ...
    // now post this message to the audio engine which tells it to play the
    // chunk of memeory we just passed to the plugin
    const AudioEventId audioId = ToAudioEventId(
        GameEvent::GenericEvent::Play__Robot_Vic__External_Voice_Message);
    const AudioGameObject audioGameObject =
        ToAudioGameObject(GameObjectType::VoiceRecording);
