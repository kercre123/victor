...
#define AUDIO_DEBUG_LOG(name, format, args...) \
  PRINT_CH_DEBUG("Audio", name, format, ##args)
#else
#define AUDIO_DEBUG_LOG(name, format, args...)
#endif
...
static const AudioGameObject kAnimGameObj =
    ToAudioGameObject(GameObjectType::Animation);
...
void AnimationAudioClient::PlayAudioKeyFrame(const RobotAudioKeyFrame& keyFrame,
  ...
  // NOTE: The order is corrected when loading the animation. (see keyframe.cpp
  // RobotAudioKeyFrame methods) NOTE: If the same State, Switch or Parameter is
  // set more then once on a single frame, last one in wins!
  const RobotAudioKeyFrame::AudioRefList& audioRefs =
      keyFrame.GetAudioReferencesList();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AnimationAudioClient::HandleAudioRef(const AudioEventGroupRef& eventRef,
                                          Util::RandomGenerator* randomGen) {
  const AudioEventGroupRef::EventDef* anEvent =
      eventRef.RetrieveEvent(true, randomGen);
  if (nullptr == anEvent) {
  ...
  if (playId != kInvalidAudioPlayingId) {
    ...
    SetCozmoEventParameter(playId, GameParameter::ParameterType::Event_Volume,
                           anEvent->Volume);
  }
...
void AnimationAudioClient::HandleAudioRef(const AudioStateRef& stateRef) {
  _audioController->SetState(ToAudioStateGroupId(stateRef.StateGroup),
                             ToAudioStateId(stateRef.State));
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AnimationAudioClient::HandleAudioRef(
    const AudioParameterRef& parameterRef) {
...
void AnimationAudioClient::AbortAnimation() {
  ...
  const auto event = ToAudioEventId(AudioMetaData::GameEvent::GenericEvent::
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
AudioEngine::AudioPlayingId AnimationAudioClient::PostCozmoEvent(
  ...
  const auto callbackFunc = std::bind(&AnimationAudioClient::CozmoEventCallback,
                                      this, ttsID, std::placeholders::_1);
  ...
  const AudioEngine::AudioPlayingId playId = _audioController->PostAudioEvent(
      audioEventId, ToAudioGameObject(gameObject), audioCallbackContext);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool AnimationAudioClient::SetCozmoEventParameter(
  ...
  return _audioController->SetParameterWithPlayingId(
      ToAudioParameterId(parameter), value, playId);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AnimationAudioClient::CozmoEventCallback(
    const uint8_t ttsID, const AudioEngine::AudioCallbackInfo& callbackInfo) {
  switch (callbackInfo.callbackType) {
    case AudioEngine::AudioCallbackType::Complete: {
      const auto& info =
          static_cast<const AudioCompletionCallbackInfo&>(callbackInfo);
      AUDIO_DEBUG_LOG("AnimationAudioClient.PostCozmoEvent.Callback", "%s",
    ...
    case AudioCallbackType::Error: {
      const auto& info =
          static_cast<const AudioErrorCallbackInfo&>(callbackInfo);
      PRINT_NAMED_WARNING("AnimationAudioClient.PostCozmoEvent.CallbackError",
                          "%s", info.GetDescription().c_str());
      RemoveActiveEvent(info.playId);
      if (ttsID != TextToSpeechComponent::kInvalidTTSID) {
        if (_ttsComponent != nullptr) {
    ...
    case AudioEngine::AudioCallbackType::Duration: {
      const auto& info =
          static_cast<const AudioDurationCallbackInfo&>(callbackInfo);
      PRINT_NAMED_WARNING(
          "AnimationAudioClient.PostCozmoEvent.CallbackUnexpected", "%s",
          info.GetDescription().c_str());
      break;
    }
    ...
    case AudioEngine::AudioCallbackType::Marker: {
      const auto& info =
          static_cast<const AudioMarkerCallbackInfo&>(callbackInfo);
      PRINT_NAMED_WARNING(
          "AnimationAudioClient.PostCozmoEvent.CallbackUnexpected", "%s",
          info.GetDescription().c_str());
      break;
    ...
    case AudioEngine::AudioCallbackType::Invalid: {
      PRINT_NAMED_WARNING("AnimationAudioClient.PostCozmoEvent.CallbackInvalid",
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AnimationAudioClient::RemoveActiveEvent(
    AudioEngine::AudioPlayingId playId) {
  std::lock_guard<std::mutex> lock(_lock);
