 ...
 *
 * Description: Component wrapper to generate, cache and use wave data from an
 * SDK message.
 *
...
namespace {
constexpr Anki::AudioMetaData::GameObjectType kSdkGameObject =
    Anki::AudioMetaData::GameObjectType::TextToSpeech;
constexpr Anki::AudioEngine::PlugIns::StreamingWavePortalPlugIn::PluginId_t
    kSdkPluginId = 100;
}  // namespace
...
SdkAudioComponent::SdkAudioComponent(const Anim::AnimContext* context) {
  ...
  DEV_ASSERT(nullptr != context->GetAudioController(),
             "SdkAudioComponent.InvalidAudioController");

  _audioController = context->GetAudioController();
  _audioPrepared = false;
  _audioPosted = false;
  _audioPlaybackFinishedPtr =
      std::make_shared<AudioCallbackType>([this]() { OnAudioCompleted(); });
}
...
//
// Called to handle incoming ExternalAudioComplete message when client signals
// end of streaming
//
void SdkAudioComponent::HandleMessage(
...
//
// Called to handle incoming ExternalAudioPrepare messages to start audio
// streaming process
//
void SdkAudioComponent::HandleMessage(
  ...
  if (!PrepareAudioEngine(msg)) {
    LOG_DEBUG("SdkAudioComponent.HandleMessage.ExternalAudioPrepare",
              "Unable to prepare audio engine for streaming");
...
//
// Called to handle incoming ExternalAudioChunk message for streaming audio data
// chunks
//
void SdkAudioComponent::HandleMessage(
    const RobotInterface::ExternalAudioChunk& msg) {
  if (!_audioPrepared) {
    LOG_DEBUG("SdkAudioComponent.HandleMessage.ExternalAudioChunk",
              "Dropping chunks due to cancellation");
    return;
  }
  ...
  const float totalAudioReceived_s =
      _totalAudioFramesReceived / (float)_audioRate;
  LOG_DEBUG("SdkAudioComponent.HandleMessage.ExternalAudioChunk",
            "Received (sec) %f", totalAudioReceived_s);
  if (!_audioPosted && totalAudioReceived_s > AUDIO_TO_BEGIN_PLAYING_SEC) {
    ...
    if (!PostAudioEvent()) {
      LOG_ERROR("SdkAudioComponent.HandleMessage.ExternalAudioChunk",
                "Unable to post audio event Audio Streaming");
...
bool SdkAudioComponent::SetPlayerVolume(float volume) const {
  if (!ANKI_VERIFY(
          volume <= 1.0f && volume >= 0.0f, "SdkAudioComponent.SetPlayerVolume",
          "InvalidVolumeLevel should be between 0.0 and 1.0 inclusive")) {
    return false;
  }
  const auto parameterId = AudioEngine::ToAudioParameterId(
      AudioMetaData::GameParameter::ParameterType::Robot_Vic_Sdk_Volume);
...
//
bool SdkAudioComponent::PrepareAudioEngine(
    const RobotInterface::ExternalAudioPrepare& msg) {
  if (_audioPrepared) {
  ...
  if (!ANKI_VERIFY(msg.audio_rate >= 8000 && msg.audio_rate <= 16025,
                   "SdkAudioComponent::PrepareAudioEngine",
                   "Invalid audio rate %u", msg.audio_rate)) {
    return false;
  }
  ...
  DEV_ASSERT(nullptr != pluginInterface,
             "SdkAudioComponent.PrepareAudioEngine.InvalidPluginInterface");
  ...
  if (!result) {
    LOG_ERROR("SdkAudioComponent::PrepareAudioEngine",
              "Failed to add data instance");
    return false;
  }
...
//
bool SdkAudioComponent::AddAudioChunk(
    const RobotInterface::ExternalAudioChunk& msg) {
  // check for dangerous buffer expansion
  auto* pluginInterface = _audioController->GetPluginInterface();
  auto* plugin = pluginInterface->GetStreamingWavePortalPlugIn();
  const auto played =
      plugin->GetDataInstance(kSdkPluginId)->GetNumberOfFramesPlayed();
  const auto received =
      plugin->GetDataInstance(kSdkPluginId)->GetNumberOfFramesReceived();
  LOG_DEBUG("SdkAudioComponent::AddAudioChunk", "Played %u Received %u", played,
            received);
  SendAnimToEngine(SDKAudioStreamingState::ChunkAdded, received, played);

  if (received - played > MAX_BUFFERED_AUDIO) {
    LOG_ERROR("SdkAudioComponent::AddAudioChunk",
              "Buffer overflow %u played %u received", played, received);
  ...
  const uint16_t wave_data_size =
      input_chunk_size / 2;  // size is in bytes, but buffer samples are 16 bit
  Anki::AudioEngine::StandardWaveDataContainer waveContainer(_audioRate, 1,
                                                             wave_data_size);
  waveContainer.CopyLittleEndianWaveData(
      (const unsigned char*)msg.audio_chunk_data, wave_data_size);
  const bool result =
      _waveData->AppendStandardWaveData(std::move(waveContainer));
...
bool SdkAudioComponent::PostAudioEvent() {
  ...
  // Set callback flags
  audioCallbackContext->SetCallbackFlags(
      AudioEngine::AudioCallbackFlag::Complete);
  ...
  audioCallbackContext->SetEventCallbackFunc(
      [weakOnFinished](const AudioCallbackContext* thisContext,
                       const AudioCallbackInfo& callbackInfo) {
        // if the user logs out, this callback could fire after
        // sdkAudioComponent is destroyed. check that here.
  ...
  const auto eventID = AudioEngine::ToAudioEventId(
      AudioEvent::Play__Robot_Vic__External_Sdk_Playback_01);
  const auto gameObject =
      static_cast<AudioEngine::AudioGameObject>(kSdkGameObject);
  ...
  if (AudioEngine::kInvalidAudioPlayingId == playingID) {
    LOG_ERROR("SdkAudioComponent.PostAudioEvent", "Failed to post eventID %u",
              eventID);
    return false;
  }
  LOG_DEBUG("SdkAudioComponent.PostAudioEvent",
            "Posted audio eventID %u playingID %d", eventID, playingID);
  return true;
}
...
void SdkAudioComponent::StopActiveAudio() {
  ...
  using AudioEvent = AudioMetaData::GameEvent::GenericEvent;
  const auto eventID = AudioEngine::ToAudioEventId(
      AudioEvent::Stop__Robot_Vic__External_Sdk_Playback_01);
...
void SdkAudioComponent::ClearActiveAudio() {
  ...
  auto* plugin =
      _audioController->GetPluginInterface()->GetStreamingWavePortalPlugIn();
  if (plugin != nullptr) {
    plugin->ClearAudioData(kSdkPluginId);
  }
}
...
//
// Handle a callback from the AudioEngine indicating that the SDK Audio has
// finished sending audio
//
void SdkAudioComponent::OnAudioCompleted() {
  LOG_DEBUG("SdkAudioComponent.OnAudioCompleted",
            "AudioStreaming completion callback received");
...
//
bool SdkAudioComponent::SendAnimToEngine(SDKAudioStreamingState audioState,
                                         uint32_t audioSent,
                                         uint32_t audioPlayed) {
  LOG_DEBUG("sdkAudioComponent.SendAnimToEngine",
            "audioState %d audioSent %d audioPlayed %d", (int)audioState,
            audioSent, audioPlayed);
