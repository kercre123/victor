...
class AudioProceduralFrame {
 public:
  ...
  // Acceleration considers the direction of the movement and returns positive
  // for increasing speeds, negative for decreasing speeds
  float GetNormalizedTreadAcceleration() const;

  // Spin speed can be double the max tread speed because it is the difference
  // of the 2 treads, however we want more resolution for slower speeds so we
  // cap the speed; Turn speed is always positive
 ...
 private:
  ...
  // The methods below need to be called after setting all the primary frame's
  // values
