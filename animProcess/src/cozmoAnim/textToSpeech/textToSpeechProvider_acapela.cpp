...
//
static inline unsigned char transformer(unsigned char c) { return (c ^ 0xAC); }
...
// XOR is symmetric so doing it twice returns the original value.
//
static int reveal(int val) { return (val ^ 0xACADADA); }

//
// Helper functions used for encoding.  These aren't needed at runtime.
//
#ifdef notdef
static std::string conceal(const std::string& val) { return reveal(val); }
#endif

#ifdef notdef
static int conceal(int val) { return reveal(val); }
#endif
...
float AcapelaTTS::GetSpeechRate(int speed, float durationScalar) {
  ...
  durationScalar =
      Anki::Util::Clamp(durationScalar, kDurationScalarMin, kDurationScalarMax);
...
float AcapelaTTS::GetAdjustedPitch(int basePitch, float pitchScalar) {
  ...
  pitchScalar =
      Anki::Util::Clamp(pitchScalar, kPitchScalarMin, kPitchScalarMax);
...
//
// Note that license string includes embedded quote and newline characters which
// must be preserved as part of the string.
//
