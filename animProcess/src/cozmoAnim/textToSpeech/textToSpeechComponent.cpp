 ...
 *
 * Description: Component wrapper to generate, cache and use wave data from a
 * given string and style. This class provides a platform-independent interface
 * to separate engine & audio libraries from details of a specific
...
constexpr Anki::AudioMetaData::GameObjectType kTTSGameObject =
...
TextToSpeechComponent::TextToSpeechComponent(const Anim::AnimContext* context)
  ...
  DEV_ASSERT(nullptr != context->GetAudioController(),
             "TextToSpeechComponent.InvalidAudioController");

  _audioController = context->GetAudioController();

  const Json::Value& tts_config =
      context->GetDataLoader()->GetTextToSpeechConfig();
  _pvdr =
      std::make_unique<TextToSpeech::TextToSpeechProvider>(context, tts_config);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - - Return a SWAG estimate of duration
// for a given text. Estimates are generous to avoid premature timeout.
f32 TextToSpeechComponent::GetEstimatedDuration_ms(const std::string& text) {
  return (text.size() * 1000.f / 4.0f);
}

f32 TextToSpeechComponent::GetDuration_ms(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result TextToSpeechComponent::CreateSpeech(
    const TTSID_t ttsID, const TextToSpeechTriggerMode triggerMode,
    const std::string& text, const AudioTtsProcessingStyle style,
    const float durationScalar, const float pitchScalar) {
  ...
  // Set trailing silence pause to 10 ms and add punctuation to the end of the
  // string
  ttsStr += " \\pau=10\\";
...

  {
    ...
    if (!it.second) {
      LOG_ERROR("TextToSpeechComponent.CreateSpeech",
                "ttsID %d already in cache", ttsID);
  ...
  Util::Dispatch::Async(_dispatchQueue, [this, ttsID, ttsStr, durationScalar,
                                         pitchScalar, waveData] {
    ...
    Result result =
        GetFirstAudioData(ttsStr, durationScalar, pitchScalar, waveData, done);
    if (RESULT_OK != result) {
      LOG_ERROR("TextToSpeechComponent.CreateSpeech",
                "Unable to get first audio data (error %d)", result);
...

    {
      ...
      if (!bundle) {
        LOG_DEBUG("TextToSpeechComponent.CreateSpeech",
                  "TTSID %d has been cancelled", ttsID);
        return;
      }
      if (!sentPlayable &&
          waveData->GetNumberOfFramesReceived() >= kMinPlayableFrames) {
        LOG_DEBUG("TextToSpeechComponent.CreateSpeech",
                  "TTSID %d audio is ready to play", ttsID);
        const f32 duration_ms =
    ...
    while (result == RESULT_OK && !done) {
      ...
      if (RESULT_OK != result) {
        LOG_ERROR("TextToSpeechComponent.CreateSpeech",
                  "Unable to get next audio data (error %d)", result);
        PushEvent({ttsID, TextToSpeechState::Invalid, 0.f});
        return;
      }
      {
    ...
    // Finalize data instance
    {
      ...
      LOG_DEBUG("TextToSpeechComponent.CreateSpeech",
                "TTSID %d audio is complete", ttsID);
...
bool TextToSpeechComponent::PrepareAudioEngine(const TTSID_t ttsID,
  ...
  if (nullptr == ttsBundle) {
    LOG_ERROR("TextToSpeechComponent.PrepareAudioEngine", "ttsID %u not found",
              ttsID);
    return false;
  }
  ...
  if (AudioCreationState::None == state) {
    LOG_WARNING("TextToSpeechComponent.PrepareAudioEngine.NoAudio",
                "ttsID %d audio not found", ttsID);
    return false;
  }

  if (AudioCreationState::Preparing == state) {
    LOG_WARNING("TextToSpeechComponent.PrepareAudioEngine.AudioPreparing",
                "ttsID %d audio not ready", ttsID);
    return false;
  }

  StreamingWaveDataPtr waveData = ttsBundle->waveData;
  if (!waveData) {
    LOG_ERROR("TextToSpeechComponent.PrepareAudioEngine.InvalidWaveData",
              "ttsID %d has no audio data", ttsID);
    return false;
  }
  ...
  DEV_ASSERT(nullptr != pluginInterface,
             "TextToSpeechComponent.PrepareAudioEngine.InvalidPluginInterface");
...
void TextToSpeechComponent::CleanupAudioEngine(const TTSID_t ttsID) {
  LOG_INFO("TextToSpeechComponent.CleanupAudioEngine", "Clean up ttsID %d",
           ttsID);
...
static void AppendAudioData(
    const std::shared_ptr<AudioEngine::StreamingWaveDataInstance>& waveData,
    const TextToSpeech::TextToSpeechProviderData& ttsData, bool done) {
  ...
  if (ttsData.GetNumSamples() > 0) {
    ...
    StandardWaveDataContainer waveContainer(sample_rate, num_channels,
                                            num_samples);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result TextToSpeechComponent::GetFirstAudioData(
    const std::string& text, float durationScalar, float pitchScalar,
    const StreamingWaveDataPtr& data, bool& done) {
  TextToSpeech::TextToSpeechProviderData ttsData;
  const Result result = _pvdr->GetFirstAudioData(text, durationScalar,
                                                 pitchScalar, ttsData, done);

  if (RESULT_OK != result) {
    LOG_ERROR("TextToSpeechComponent.GetFirstAudioData",
              "Unable to get first audio data (error %d)", result);
    return result;
  }
...
Result TextToSpeechComponent::GetNextAudioData(const StreamingWaveDataPtr& data,
                                               bool& done) {
  TextToSpeech::TextToSpeechProviderData ttsData;
  const Result result = _pvdr->GetNextAudioData(ttsData, done);

  if (RESULT_OK != result) {
    LOG_ERROR("TextToSpeechComponent.GetNextAudioData",
              "Unable to get next audio data (error %d)", result);
    return result;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
TextToSpeechComponent::BundlePtr TextToSpeechComponent::GetBundle(
    const TTSID_t ttsID) {
...
//
void TextToSpeechComponent::SetAudioProcessingStyle(
    AudioTtsProcessingStyle style) {
  const auto switchGroup = AudioMetaData::SwitchState::SwitchGroupType::
      Robot_Vic_External_Processing;
...
static bool SendAnimToEngine(uint8_t ttsID, TextToSpeechState state,
  ...
  LOG_DEBUG("TextToSpeechComponent.SendAnimToEngine", "ttsID %hhu state %hhu",
            ttsID, state);
  TextToSpeechEvent evt;
  evt.ttsID = ttsID;
  evt.ttsState = state;
  evt.expectedDuration_ms =
      expectedDuration;  // Used only on TextToSpeechState::Delivered messages
...
bool TextToSpeechComponent::PostAudioEvent(uint8_t ttsID) {
  const auto callbackFunc =
      std::bind(&TextToSpeechComponent::OnUtteranceCompleted, this, ttsID);
  ...
  const auto eventID = AudioEngine::ToAudioEventId(
      AudioEvent::Play__Robot_Vic__External_Voice_Text);
  ...
  if (AudioEngine::kInvalidAudioPlayingId == playingID) {
    LOG_ERROR("TextToSpeechComponent.PostAudioEvent",
              "Failed to post eventID %u for ttsID %d", eventID, ttsID);
    return false;
  }

  LOG_DEBUG("TextToSpeechComponent.PostAudioEvent",
            "eventID %u ttsID %d playingID %d", eventID, ttsID, playingID);
...
//
// Handle a callback from the AudioEngine indicating that the TtS Utterance has
// finished playing
//
void TextToSpeechComponent::OnUtteranceCompleted(uint8_t ttsID) {
  _activeTTSID = kInvalidTTSID;

  LOG_DEBUG("TextToSpeechComponent.UtteranceCompleted",
            "Completion callback received for ttsID %hhu", ttsID);
...
//
void TextToSpeechComponent::HandleMessage(
  ...
  LOG_DEBUG("TextToSpeechComponent.TextToSpeechPrepare",
            "ttsID %d triggerMode %s style %s durationScalar %.2f pitchScalar "
            "%.2f text %s",
            ttsID, EnumToString(triggerMode), EnumToString(style),
            durationScalar, pitchScalar,
            HidePersonallyIdentifiableInfo(text.c_str()));

  // Enqueue request on worker thread
  const Result result = CreateSpeech(ttsID, triggerMode, text, style,
                                     durationScalar, pitchScalar);
  if (RESULT_OK != result) {
    LOG_ERROR("TextToSpeechComponent.TextToSpeechPrepare",
              "Unable to create ttsID %d (result %d)", ttsID, result);
    SendAnimToEngine(ttsID, TextToSpeechState::Invalid);
    return;
  }

  // Execution continues in Update() when worker thread posts state change back
  // to main thread
}
...
//
void TextToSpeechComponent::HandleMessage(
    const RobotInterface::TextToSpeechPlay& msg) {
  const auto ttsID = msg.ttsID;
  ...
  const auto bundle = GetBundle(ttsID);
  if (!bundle) {
    LOG_ERROR("TextToSpeechComponent.TextToSpeechPlay", "ttsID %d not found",
              ttsID);
    SendAnimToEngine(ttsID, TextToSpeechState::Invalid);
    return;
  }
  ...
  if (triggerMode != TextToSpeechTriggerMode::Manual &&
      triggerMode != TextToSpeechTriggerMode::Keyframe) {
    LOG_ERROR("TextToSpeechComponent.TextToSpeechPlay",
              "ttsID %d has unplayable trigger mode %s", ttsID,
              EnumToString(bundle->triggerMode));
    SendAnimToEngine(ttsID, TextToSpeechState::Invalid);
    ClearOperationData(ttsID);
    return;
  }
  ...
  if (!PrepareAudioEngine(ttsID, duration_ms)) {
    LOG_ERROR("TextToSpeechComponent.TextToSpeechDeliver",
              "Unable to prepare audio engine for ttsID %d", ttsID);
    SendAnimToEngine(ttsID, TextToSpeechState::Invalid);
    ClearOperationData(ttsID);
    return;
  }

  LOG_INFO("TextToSpeechComponent.TextToSpeechPlay",
           "ttsID %d will play for %.2f ms", ttsID, duration_ms);

  // Post audio event? For manual triggers, post event now and notify engine
  // that playback is in progress. For keyframe events, event will be posted by
  // AnimationAudioClient and engine will be notified by callback to
  // OnAudioPlaying.
  if (triggerMode == TextToSpeechTriggerMode::Manual) {
    if (!PostAudioEvent(ttsID)) {
      LOG_ERROR("TextToSpeechComponent.TextToSpeechPlay",
                "Unable to post audio event for ttsID %d", ttsID);
      SendAnimToEngine(ttsID, TextToSpeechState::Invalid);
...
//
void TextToSpeechComponent::HandleMessage(
    const RobotInterface::TextToSpeechCancel& msg) {
  const auto ttsID = msg.ttsID;

  LOG_DEBUG("TextToSpeechComponent.HandleMessage.TextToSpeechCancel",
            "ttsID %d", ttsID);
...
void TextToSpeechComponent::OnStatePreparing(const TTSID_t ttsID) {
  ...
  const auto bundle = GetBundle(ttsID);
  if (!bundle) {
    LOG_DEBUG("TextToSpeechComponent.OnStatePreparing",
              "ttsID %d has been cancelled", ttsID);
    return;
  }
...
// Called by Update() in response to event from worker thread
//
void TextToSpeechComponent::OnStatePlayable(const TTSID_t ttsID,
                                            f32 duration_ms) {
  LOG_DEBUG("TextToSpeechComponent.OnStatePlayable", "ttsID %d duration %.2f",
            ttsID, duration_ms);

  const auto bundle = GetBundle(ttsID);
  if (!bundle) {
    LOG_DEBUG("TextToSpeechComponent.OnStatePlayable",
              "ttsID %d has been cancelled", ttsID);
    return;
  }
  ...
  if (bundle->triggerMode == TextToSpeechTriggerMode::Immediate) {
    if (!PrepareAudioEngine(ttsID, duration_ms)) {
      LOG_ERROR("TextToSpeechComponent.OnStatePlayable",
                "Unable to prepare audio for ttsID %d", ttsID);
      SendAnimToEngine(ttsID, TextToSpeechState::Invalid);
      ClearOperationData(ttsID);
      return;
    }
    ...
    LOG_INFO("TextToSpeech.OnStatePlayable",
             "ttsID %d will play for at least %.2f ms", ttsID, duration_ms);
    SendAnimToEngine(ttsID, TextToSpeechState::Playing, duration_ms);
  }
}
...
void TextToSpeechComponent::OnStatePrepared(const TTSID_t ttsID,
                                            f32 duration_ms) {
  LOG_DEBUG("TextToSpeechComponent.OnStatePrepared", "ttsID %d duration %.2f",
            ttsID, duration_ms);

  const auto bundle = GetBundle(ttsID);
  if (!bundle) {
    LOG_DEBUG("TextToSpeechComponent.OnStatePrepared",
              "ttsID %d has been cancelled", ttsID);
    return;
  }
...
//
// Called by main thread (once per tick) to handle events posted by worker
// thread
//
void TextToSpeechComponent::Update() {
  ...
  while (PopEvent(event)) {
    ...
    LOG_DEBUG("TextToSpeechComponent.Update",
              "Event ttsID %d state %hhu duration %f", ttsID, ttsState,
              duration_ms);
    ...
    switch (ttsState) {
      ...
      default:
        //
        // We don't expect any other events from the worker thread.  Transition
        // to Delivering/Delivered are handled by the main thread.
        //
        LOG_ERROR("TextToSpeechComponent.Update.UnexpectedState",
                  "Event ttsID %d unexpected state %hhu", ttsID, ttsState);
        break;
    }
  }
}
...
void TextToSpeechComponent::SetLocale(const std::string& locale) {
  //
  // Perform callback on worker thread so locale is changed in sync with TTS
  // processing. Any TTS operations queued before SetLocale() will be processed
  // with old locale. Any TTS operations queued after SetLocale() will be
  // processed with new locale.
  //
  LOG_DEBUG("TextToSpeechComponent.SetLocale", "Set locale to %s",
            locale.c_str());

  const auto& task = [this, locale = std::string(locale)] {
    DEV_ASSERT(_pvdr != nullptr,
               "TextToSpeechComponent.SetLocale.InvalidProvider");
    LOG_DEBUG("TextToSpeechComponent.SetLocale", "Setting locale to %s",
              locale.c_str());
    const Result result = _pvdr->SetLocale(locale);
    if (result != RESULT_OK) {
      LOG_ERROR("TextToSpeechComponent.SetLocale",
                "Unable to set locale to %s (error %d)", locale.c_str(),
                result);
    }
  };
...
void TextToSpeechComponent::OnAudioPlaying(const TTSID_t ttsID) {
  LOG_DEBUG("TextToSpeechComponent.OnAudioPlaying", "Now playing ttsID %d",
            ttsID);
  auto bundle = GetBundle(ttsID);
  if (!bundle) {
    LOG_ERROR("TextToSpeechComponent.OnAudioPlaying", "ttsID %d not found",
              ttsID);
    return;
  }
...
void TextToSpeechComponent::OnAudioComplete(const TTSID_t ttsID) {
  LOG_DEBUG("TextToSpeechComponent.OnAudioComplete",
            "Finished playing ttsID %d", ttsID);
  auto bundle = GetBundle(ttsID);
  if (!bundle) {
    LOG_ERROR("TextToSpeechComponent.OnAudioComplete", "ttsID %d not found",
              ttsID);
    return;
  }
...
void TextToSpeechComponent::OnAudioError(const TTSID_t ttsID) {
  LOG_DEBUG("TextToSpeechComponent.OnAudioError", "Error playing ttsID %d ",
            ttsID);
  auto bundle = GetBundle(ttsID);
  if (!bundle) {
    LOG_ERROR("TextToSpeechComponent.OnAudioError", "ttsID %d not found",
              ttsID);
    return;
  }
