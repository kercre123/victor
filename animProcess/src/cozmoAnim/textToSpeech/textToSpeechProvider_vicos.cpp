...
TextToSpeechProviderImpl::TextToSpeechProviderImpl(
    const Anim::AnimContext *context, const Json::Value &tts_platform_config) {
  ...
  DEV_ASSERT(context->GetRandom() != nullptr,
             "TextToSpeechProviderImpl.InvalidRNG");
  ...
  if (nullptr == dataPlatform) {
    ...
    LOG_WARNING("TextToSpeechProviderImpl.InvalidDataPlatform",
                "Missing data platform");
    return;
  }
...
TextToSpeechProviderImpl::~TextToSpeechProviderImpl() { Cleanup(); }
...
Result TextToSpeechProviderImpl::Initialize(const std::string &locale) {
  LOG_DEBUG("TextToSpeechProvider.Initialize", "Initialize locale %s",
            locale.c_str());

  if (locale == _locale) {
    LOG_DEBUG("TextToSpeechProvider.Initialize", "Already using locale %s",
              locale.c_str());
    return RESULT_OK;
  }
  ...
  std::string language =
      Anki::Util::Locale::LocaleFromString(locale).GetLanguageString();
  if (language.empty()) {
    LOG_ERROR("TextToSpeechProvider.Initialize",
              "Unable to get language from locale %s", locale.c_str());
    language = "en";
  }

  // Initialize configuration
  _tts_config = std::make_unique<TextToSpeechProviderConfig>(
      language, _tts_platform_config);
  ...
  _BAB_LangDba = BABILE_loadIniFile(iniFile.c_str(), &nlpeLS, &synthLS,
                                    &nlpModule, &synthAvail, &synthModule,
                                    &defaultText, loadParams.c_str());

  if (nullptr == _BAB_LangDba) {
    LOG_WARNING("TextToSpeechProvider.Initialize.LoadIniFile",
                "Failed to load ini file %s", iniFile.c_str());
  ...
  //
  // Ask Babile SDK how many memory segments it needs to track, then allocate a
  // tracker of appropriate size
  //
  ...
  if (nullptr == _BAB_Obj) {
    LOG_WARNING("TextToSpeechProvider.Initialize.Init",
                "Failed to initialize TTS library");
...

  {
    ...
    BABILE_getVersionEx(_BAB_Obj, (BB_TCHAR *)version,
                        sizeof(version) / sizeof(char));
    BABILE_getSetting(_BAB_Obj, BABIL_PARM_VOICEFREQ,
                      (BB_SPTR *)&_BAB_voicefreq);
    BABILE_getSetting(_BAB_Obj, BABIL_PARM_SAMPLESIZE,
                      (BB_SPTR *)&_BAB_samplesize);
    LOG_INFO("TextToSpeechProvider.Initialize.VersionEx",
             "TTS library version %s (%s) freq=%ld samplesize=%ld",
             BABILE_getVersion(), version, _BAB_voicefreq, _BAB_samplesize);
  }

  bbError = BABILE_setSetting(_BAB_Obj, BABIL_PARM_SPEED, speed);
  if (BB_OK != bbError) {
    LOG_WARNING("TextToSpeechProvider.Initialize.SetSpeed",
                "Unable to set speed %d (error %ld)", speed, bbError);
  }

  bbError = BABILE_setSetting(_BAB_Obj, BABIL_PARM_SEL_VOICESHAPE, shaping);
  if (BB_OK != bbError) {
    LOG_WARNING("TextToSpeechProvider.Initialize.SetVoiceShape",
                "Unable to set voice shape %d (error %ld)", shaping, bbError);
  }

  bbError = BABILE_setSetting(_BAB_Obj, BABIL_PARM_PITCH, pitch);
  if (BB_OK != bbError) {
    LOG_WARNING("TextToSpeechProvider.Initialize.SetPitch",
                "Unable to set pitch %d (error %ld)", pitch, bbError);
  }
  ...
  LOG_INFO("TextToSpeechProviderImpl.Initialize",
           "Initialized locale %s language %s", _locale.c_str(),
           _language.c_str());
...
Result TextToSpeechProviderImpl::GetFirstAudioData(
    const std::string &text, float durationScalar, float pitchScalar,
    TextToSpeechProviderData &data, bool &done) {
  LOG_INFO("TextToSpeechProvider.GetFirstAudioData",
           "text=%s duration=%.2f pitch=%.2f",
           Anki::Util::HidePersonallyIdentifiableInfo(text.c_str()),
           durationScalar, pitchScalar);

  if (nullptr == _BAB_Obj) {
    LOG_ERROR("TextToSpeechProvider.GetFirstAudioData",
              "TTS SDK not initialized");
    return RESULT_FAIL_INVALID_OBJECT;
  }
  ...
  const auto adjustedSpeed =
      AcapelaTTS::GetSpeechRate(baseSpeed, durationScalar);
  ...
  const auto adjustedPitch =
      AcapelaTTS::GetAdjustedPitch(basePitch, pitchScalar);
  ...
  if (BB_OK != bbError) {
    LOG_WARNING("TextToSpeechProvider.GetFirstAudioData",
                "Unable to reset TTS (error %ld)", bbError);
  }
...
#define SETPARAM(param, val)                                                   \
  {                                                                            \
    ...
    if (BB_OK != bbError) {                                                    \
      LOG_WARNING("TextToSpeechProvider.SetParam",                             \
                  "Unable to set %s to %d (error %ld)", #param, val, bbError); \
    }                                                                          \
  }
...
Result TextToSpeechProviderImpl::GetNextAudioData(
    TextToSpeechProviderData &data, bool &done) {
  // If we are draining the TTS buffer, pass nullptr, else pass pointer to
  // remaining text
  ...
  const BB_S32 charRead =
      BABILE_readText(_BAB_Obj, str, samples, numWanted, &numSamples);

  LOG_DEBUG("TextToSpeechProvider.GetNextAudioData",
            "charRead=%ld numSamples=%lu", charRead, numSamples);

  if (charRead < 0) {
    LOG_ERROR("TextToSpeechProvider.GetNextAudioData", "charRead=%ld",
              charRead);
