...
class TextToSpeechComponent {
 public:
  ...
  //
  // Update method is called once per tick on main thread. This method responds
  // to events posted by worker thread and performs tasks in synchronization
  // with the rest of the animation engine.
  //
  void Update();
 ...
 private:
  ...
  Result GetFirstAudioData(const std::string& text, float durationScalar,
                           float pitchScalar, const StreamingWaveDataPtr& data,
                           bool& done);
  ...
  // Asynchronous create the wave data for the given text and style, to be
  // played later Use GetOperationState() to check if wave data is Ready Return
  ...
  // out_duration_ms provides approximate duration of event before processing in
  // audio engine Return false if the audio has NOT been created or is not yet
  // ready. Output parameters will NOT be valid.
  ...
  // These methods are called by Update() on the main thread, in response to
  // events posted by the worker thread.
  //
