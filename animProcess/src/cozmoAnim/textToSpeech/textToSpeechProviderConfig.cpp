...
//
// Programmatic defaults. These values are used when unless overridden by
// configuration.
//
...
CONSOLE_VAR_RANGED(u32, kLeadingSilence_ms, CONSOLE_GROUP,
                   TTS_LEADINGSILENCE_MS, 0, 5000);
CONSOLE_VAR_RANGED(u32, kTrailingSilence_ms, CONSOLE_GROUP,
                   TTS_TRAILINGSILENCE_MS, 0, 5000);
CONSOLE_VAR_RANGED(u32, kPausePunctuation_ms, CONSOLE_GROUP,
                   TTS_PAUSEPUNCTUATION_MS, 50, 4000);
CONSOLE_VAR_RANGED(u32, kPauseSemicolon_ms, CONSOLE_GROUP,
                   TTS_PAUSESEMICOLON_MS, 50, 4000);
CONSOLE_VAR_RANGED(u32, kPauseComma_ms, CONSOLE_GROUP, TTS_PAUSECOMMA_MS, 50,
                   4000);
CONSOLE_VAR_RANGED(u32, kPauseBracket_ms, CONSOLE_GROUP, TTS_PAUSEBRACKET_MS,
                   50, 4000);
CONSOLE_VAR_RANGED(u32, kPauseSpelling_ms, CONSOLE_GROUP, TTS_PAUSESPELLING_MS,
                   50, 4000);
...
TextToSpeechProviderConfig::TextToSpeechProviderConfig(
    const std::string& language, const Json::Value& platform_config) {
...
int TextToSpeechProviderConfig::GetSpeed(Anki::Util::RandomGenerator* rng,
  ...
  for (const auto& trait : _speedTraits) {
    if (trait.textLengthMin <= textLength &&
        textLength <= trait.textLengthMax) {
