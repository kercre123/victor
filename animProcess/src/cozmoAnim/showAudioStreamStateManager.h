...
class ShowAudioStreamStateManager {
 public:
  ...
  // Most functions here need to be thread safe due to them being called from
  // trigger word detected callbacks which happen on a separate thread

  void SetTriggerWordResponse(
      const RobotInterface::SetTriggerWordResponse& msg);

  void SetAlexaUXResponses(const RobotInterface::SetAlexaUXResponses& msg);

  // Start the robot's response to the trigger in order to indicate that the
  // robot may be streaming audio The GetInAnimation is optional, the earcon and
  // backpack lights are not
  ...
  void SetPendingTriggerResponseWithoutGetIn(
      OnTriggerAudioCompleteCallback = {});

  // Indicates whether or not the audio stream state manager will be able to
  // indicate to the user that streaming may be happening - if this returns
  // false
  bool HasValidTriggerResponse();

  // Indicates whether voice data should be streamed to the cloud after the
  // trigger response has indicated to the user that streaming may be happening
  ...
  // with the exception of HasAnyAlexaResponse, alexa methods should be called
  // on the main thread. This is only because the current Alexa implementation
  // fits this constraint, so I'm assuming it here.
  ...
  void SetFrozenOnCharger(bool frozenOnCharger) {
    _frozenOnCharger = frozenOnCharger;
  }
 ...
 private:
  ...
  // Trigger word responses are triggered via callbacks from the trigger word
  // detector thread so we need to be thread safe and have pending responses to
  // be executed on the main thread in Update
  ...
  struct AlexaInfo {
    AlexaUXState state;  // a transition from Idle to this state will trigger
                         // the below response
