...
#include "cozmoAnim/doom/doomdata.h"

#include "cozmoAnim/doom/m_bbox.h"

#include "cozmoAnim/doom/v_video.h"
#include "util/logging/logging.h"
#define printferror(...) PRINT_NAMED_ERROR("DOOM", __VA_ARGS__)
...
{ 
	...
	|| (unsigned)scrn>4)
    {
      printferror( "Patch at %d,%d exceeds LFB\n", x,y );
      // No I_Error abort - what is up with TNT.WAD?
      printferror( "V_DrawPatch: bad patch (ignored)\n");
      return;
    }
#endif 
...
{ 
	...
	|| (unsigned)scrn>4)
    {
      printferror( "Patch origin %d,%d exceeds LFB\n", x,y );
    ...
    for ( ; col<w ; x++, col++, desttop++) 
    { 
	column = (column_t *)((unsigned char *)patch + patch->columnofs[w-1-col]); 
