...
{
    int i = voices;
    printf("All notes off on channel %d\n", c);
    while (i--)
        if (voice[i].status == VOICE_ON &&
            voice[i].channel == c)
        {
...
MidiSong *Timidity_LoadSongMem(unsigned char* buffer, size_t length)
{
    ...
    song = (MidiSong *)safe_malloc(sizeof(*song), 'K');
    ...
    if (!song->events) {
        Real_Tim_Free(song);
        song = NULL;
        printf( "Song had null events! Returning NULL.\n");
    }
    return(song);
}
