...
void skip(std::ifstream * fp, size_t len)
{
    ...
    while (len>0)
    {
        ...
        if (c>1024) c = 1024;
        len -= c;
        long before = static_cast<long>(fp->tellg());
        fp->read(tmp, c);
...
void *safe_malloc(size_t count, char debug)
{
    void *p;
    if (count > (1 << 21))
    {
        printf(
            "Strange, '%c' feels like allocating %d bytes. This must be a bug.\n",
            debug,
            static_cast<int>(count));
    }
    else if ((p = Real_Tim_Malloc(static_cast<int>(count))))
        return p;
    else
...
{
    PathList *plp = (PathList*)safe_malloc(sizeof(PathList), 'C');
    strcpy((plp->path = (char *)safe_malloc(strlen(s) + 1,'D')), s);
    plp->next = pathlist;
    pathlist = plp;
}
