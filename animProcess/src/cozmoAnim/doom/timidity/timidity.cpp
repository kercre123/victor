...
#include "cozmoAnim/doom/timidity/controls.h"
#include "cozmoAnim/doom/timidity/timidity.h"
...
static int read_config_file(const char *name)
{
	...
	while ( fp.getline( tokTmp, 1024 ) )
	{
		...
		else if (!strcmp(w[0], "drumset"))
		{
			...
			if (!drumset[i])
			{
				drumset[i]=(ToneBank*)safe_malloc(sizeof(ToneBank),'Q');
		...
		else if (!strcmp(w[0], "bank"))
		{
			...
			if (!tonebank[i])
			{
				tonebank[i]=(ToneBank*)safe_malloc(sizeof(ToneBank),'R');
			...
			strcpy((bank->tone[i].name=(char*)safe_malloc(strlen(w[1])+1,'S')),w[1]);
...
int Timidity_Init(int rate, int format, int channels, int samples, const char* config)
{
	...
	resample_buffer = (sample_t*)safe_malloc(AUDIO_BUFFER_SIZE*sizeof(sample_t),'T');
	common_buffer = (int32_t*)safe_malloc(AUDIO_BUFFER_SIZE*2*sizeof(int32_t),'U');
