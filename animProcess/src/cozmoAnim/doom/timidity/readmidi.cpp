...
#include "cozmoAnim/doom/timidity/readmidi.h"
...
static  int32_t read_local(void* buffer, size_t len, size_t count)
{
    if (fp.is_open() && len > 0) 
    {
        ...
        fp.read((char*)buffer,len*count);
        return static_cast<int32_t>(fp.tellg() - before);
    }
...
static void skip_local(size_t len)
{
    if (fp) 
    {
        ...
        if (len + local_buffer_cur > local_buffer_length) {
            printf( "skip_local failed on memory buffer\n");
        }
        else {
...
static void compute_sample_increment(int32_t tempo, int32_t divisions)
{
    ...
    printf( "Samples per delta-t: %d (correction %d)\n",
...
{
    signed char *s = (signed char *)safe_malloc(len + 1, 'L');
...
#define MIDIEVENT(at,t,ch,pa,pb) \
	newEventList=(MidiEventList*)safe_malloc(sizeof(MidiEventList),'M'); \
    ...
    for (;;)
    {
        ...
        else if (me == 0xFF) /* Meta event */
        {
            ...
            if (type>0 && type<16)
            {
                ...
                dumpstring(len, label[(type>7) ? 0 : type].c_str());
            }
            else
                switch (type)
                {
...
static MidiEvent *groom_list(int32_t divisions, int32_t *eventsp, int32_t *samplesp)
{
    ...
    groomed_list = lp = (MidiEvent*)safe_malloc(sizeof(MidiEvent) * (event_count + 1), 'N');
    meep = evlist;
...
MidiEvent *read_midi_buffer(unsigned char* buffer, size_t length, int32_t *count, int32_t *sp)
{
    ...
    evlist = (MidiEventList *)safe_malloc(sizeof(MidiEventList),'O');
    evlist->event.time = 0;
