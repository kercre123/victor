...
#include "cozmoAnim/doom/timidity/config.h"
#include "cozmoAnim/doom/timidity/common.h"
#include "cozmoAnim/doom/timidity/instrum.h"
#include "cozmoAnim/doom/timidity/playmidi.h"
#include "cozmoAnim/doom/timidity/output.h"
#include "cozmoAnim/doom/timidity/resample.h"
#include "cozmoAnim/doom/tables.h"
#include "cozmoAnim/doom/timidity/filter.h"

#include "cozmoAnim/doom/ResourcePath.h"
    ...
    int strip_tail)
{
  //int readCount = 0;
    ...
    ip = (Instrument *)safe_malloc(sizeof(Instrument),'F');
    ip->samples =  file[198];
    ip->sample = (Sample *)safe_malloc(sizeof(Sample) * ip->samples, 'G');
    ...
    for (i = 0; i < ip->samples; i++)
    {
        ...
        auto READ_LONG = [&](int32_t& thing)
        {
           ...
           // printf("BEFORE %d AFTER %d", tmplong, thing);
        };
      
      std::string ss;
      for( int mi2 = mi; mi2 < mi + 16; ++mi2 ) {
        if( file[mi2] == 0 ) {
          ss += "\\0";
        } else {
          ss += file[mi2];
      ...
      //printf("filesize %u, CHARS %s", file.size(), ss.c_str());
     ...
     // printf("HERE on %d READ %d", readCount++, sp->data_length);
...
//        printf("Size %d\n", sp->data_length);
        sp->data = (sample_t*)safe_malloc(sp->data_length, 'H');
        ...
        if (!(sp->modes & MODES_16BIT)) /* convert to 16-bit data */
        {
            ...
            uint16_t *tmp, *anew;
          //printf("SizeA %d", sp->data_length * 2);
            tmp = anew = (uint16_t*)safe_malloc(sp->data_length * 2, 'I');
            while (i--)
                *tmp++ = (uint16_t)(*cp++) << 8;
            cp = (uint8_t *)(sp->data);
