...
#include "cozmoAnim/doom/d_englsh.h"
...
static sf::Vector2i m_paninc; // how far the window pans each tic (map coords)
...
static sf::Vector2i markpoints[AM_NUMMARKPOINTS]; // where the points are
...
bool AM_Responder( sf::Event& ev )
{
    ...
    if (!automapactive)
    {
		if (ev.type == sf::Event::KeyPressed && ev.key.code == sf::Event::Key::Tab)
		{
		    AM_Start ();
		    viewactive = false;
		    rc = true;
    ...
    else if (ev.type == sf::Event::KeyPressed)
    {
		...
		switch(ev.key.code)
		{
		case sf::Event::Key::Right: // pan right
		    if (!followplayer) m_paninc.x = FTOM(F_PANINC);
		    else rc = false;
		    break;
		  case sf::Event::Key::Left: // pan left
		    if (!followplayer) m_paninc.x = -FTOM(F_PANINC);
		    else rc = false;
		    break;
		  case sf::Event::Key::Up: // pan up
		    if (!followplayer) m_paninc.y = FTOM(F_PANINC);
		    else rc = false;
		    break;
		  case sf::Event::Key::Down: // pan down
		    if (!followplayer) m_paninc.y = -FTOM(F_PANINC);
		    else rc = false;
		    break;
		  case sf::Event::Key::Subtract: // zoom out
		    mtof_zoommul = M_ZOOMOUT;
		    ftom_zoommul = M_ZOOMIN;
		    break;
		  case sf::Event::Key::Equal: // zoom in
		    mtof_zoommul = M_ZOOMIN;
		    ftom_zoommul = M_ZOOMOUT;
		    break;
		  case sf::Event::Key::Tab:
		    bigstate = 0;
		    viewactive = true;
		    AM_Stop ();
		    break;
		  case sf::Event::Key::Num0:
		    bigstate = !bigstate;
		    if (bigstate)
		    {
				AM_saveScaleAndLoc();
    ...
    else if (ev.type == sf::Event::KeyReleased)
    {
		...
		switch (ev.key.code)
		{
		case sf::Event::Key::Right:
		    if (!followplayer) m_paninc.x = 0;
		    break;
		  case sf::Event::Key::Left:
		    if (!followplayer) m_paninc.x = 0;
		    break;
		  case sf::Event::Key::Up:
		    if (!followplayer) m_paninc.y = 0;
		    break;
		  case sf::Event::Key::Down:
		    if (!followplayer) m_paninc.y = 0;
		    break;
		  case sf::Event::Key::Subtract:
		  case sf::Event::Key::Equal:
		    mtof_zoommul = FRACUNIT;
		    ftom_zoommul = FRACUNIT;
		    break;
		  default:
