...
#include "cozmoAnim/doom/i_video.h"

#include "cozmoAnim/doom/f_finale.h"
#include "cozmoAnim/doom/m_argv.h"
#include "cozmoAnim/doom/m_misc.h"
#include "cozmoAnim/doom/m_menu.h"
#include "cozmoAnim/doom/m_random.h"
#include "cozmoAnim/doom/i_system.h"
#include "cozmoAnim/doom/i_sound.h"

#include "cozmoAnim/doom/p_setup.h"
#include "cozmoAnim/doom/p_saveg.h"
#include "cozmoAnim/doom/p_tick.h"

#include "cozmoAnim/doom/d_main.h"

#include "cozmoAnim/doom/wi_stuff.h"
#include "cozmoAnim/doom/hu_stuff.h"
#include "cozmoAnim/doom/st_stuff.h"
#include "cozmoAnim/doom/am_map.h"

// Needs access to LFB.
#include "cozmoAnim/doom/v_video.h"

#include "cozmoAnim/doom/w_wad.h"

#include "cozmoAnim/doom/p_local.h"

#include "cozmoAnim/doom/i_sound.h"

// Data.
#include "cozmoAnim/doom/dstrings.h"
#include "cozmoAnim/doom/sounds.h"

// SKY handling - still the wrong place.
#include "cozmoAnim/doom/r_data.h"
#include "cozmoAnim/doom/r_sky.h"
#include "cozmoAnim/doom/g_game.h"
...
{ 
    ...
    speed = gamekeydown[sf::Event::Key::LShift] || gamekeydown[sf::Event::Key::RShift];// || joybuttons[joybspeed];
	...
	|| gamekeydown[sf::Event::Key::Right]
	|| gamekeydown[sf::Event::Key::Left])
		turnheld += ticdup; 
    else 
		turnheld = 0; 
	...
	if (gamekeydown[sf::Event::Key::Right] || rightStickXMove > joyDeadZone)
	    cmd->angleturn -= angleturn[tspeed]; 
	if (gamekeydown[sf::Event::Key::Left] || rightStickXMove < -joyDeadZone)
	    cmd->angleturn += angleturn[tspeed]; 
 
    if (gamekeydown[sf::Event::Key::Up] || gamekeydown[sf::Event::Key::W] || leftStickYMove < -joyDeadZone)
		forward += forwardmove[speed]; 
    if (gamekeydown[sf::Event::Key::Down] || gamekeydown[sf::Event::Key::S] || leftStickYMove > joyDeadZone)
		forward -= forwardmove[speed]; 
	if (gamekeydown[sf::Event::Key::D] || leftStickXMove > joyDeadZone)
		side += sidemove[speed];
	if (gamekeydown[sf::Event::Key::A] || leftStickXMove < -joyDeadZone)
		side -= sidemove[speed];
    
    // buttons
    cmd->chatchar = HU_dequeueChatChar(); 
 
    if (gamekeydown[sf::Event::Key::LControl] || mousebuttons[mousebfire]
	|| TriggerMove < -joyDeadZone) 
		cmd->buttons |= BT_ATTACK; 
 
    if (gamekeydown[sf::Event::Key::Space] )
    { 
		cmd->buttons |= BT_USE;
    ...
    static std::array<sf::Event::Key, NUMWEAPONS+1> weaponButtons = {{
        sf::Event::Key::Num0,
        sf::Event::Key::Num1,
        sf::Event::Key::Num2,
        sf::Event::Key::Num3,
...
{ 
	...
	&& ev->key.code == sf::Event::Key::F12 && (singledemo || !deathmatch) )
    {
		// spy mode 
		do 
		{ 
	...
	case sf::Event::KeyPressed:
		//check for full 
		if (ev->key.code == sf::Event::Key::F &&
			ev->key.control)
		{
			toggleFullscreen();
		}
		else if (ev->key.code == sf::Event::Key::P)
		{ 
		    sendpause = true; 
		    return true; 
		}
...
void G_WriteDemoTiccmd (ticcmd_t* cmd) 
{ 
    if (gamekeydown[sf::Event::Key::Q])           // press q to end demo recording
		G_CheckDemoStatus (); 
...
{ 
	...
	{
		PRINT_NAMED_ERROR("DOOM","Demo is from a different game version!\n");
		gameaction = ga_nothing;
		return;
	}
