...
CONSOLE_VAR_RANGED(float, kAnimEngine_TimeMax_ms, ANKI_CPU_CONSOLEVARGROUP, 33,
                   2, 33);
CONSOLE_VAR_ENUM(u8, kAnimEngine_TimeLogging, ANKI_CPU_CONSOLEVARGROUP, 0,
...
AnimEngine::AnimEngine(Util::Data::DataPlatform* dataPlatform)
  ...
  Anki::Util::CpuThreadProfiler::SetChromeTracingFile(
      dataPlatform
          ->pathToResource(Util::Data::Scope::Cache, "vic-anim-tracing.json")
          .c_str());
  ...
  _microphoneAudioClient.reset(
      new Audio::MicrophoneAudioClient(_context->GetAudioController()));
...
Result AnimEngine::Init() {
  ...
  // animation streamer must be initialized after loading non config data
  // (otherwise there are no animations loaded)
  _animationStreamer->Init(_ttsComponent.get());
  _context->GetBackpackLightComponent()->Init();

  // Create and set up EngineRobotAudioInput to receive Engine->Robot messages
  // and broadcast Robot->Engine
  ...
  _streamingAnimationModifier = std::make_unique<StreamingAnimationModifier>(
      _animationStreamer.get(), audioInput, _ttsComponent.get());

  // set up audio stream state manager
  {
    _context->GetShowAudioStreamStateManager()->SetAnimationStreamer(
        _animationStreamer.get());
  }

  AnimProcessMessages::Init(this, _animationStreamer.get(),
                            _streamingAnimationModifier.get(), audioInput,
                            _context.get());

  _context->GetWebService()->Start(
      _context->GetDataPlatform(),
      _context->GetDataLoader()->GetWebServerAnimConfig());
  FaceInfoScreenManager::getInstance()->Init(_context.get(),
                                             _animationStreamer.get());
  ...
  Result cvResult =
      SetNumOpencvThreads(NUM_ANIM_OPENCV_THREADS, "AnimEngine.Init");
...
Result AnimEngine::Update(const BaseStationTime_t currTime_nanosec) {
  ANKI_CPU_TICK(
      "AnimEngine::Update", kAnimEngine_TimeMax_ms,
      Util::CpuProfiler::CpuProfilerLoggingTime(kAnimEngine_TimeLogging));
  if (!_isInitialized) {
    LOG_ERROR("AnimEngine.Update",
              "Cannot update AnimEngine before it is initialized.");
    return RESULT_FAIL;
  }

  //
  // Declare some invariants. These components are always present after
  ...
  DEV_ASSERT(_streamingAnimationModifier,
             "AnimEngine.Update.InvalidStreamingAnimationModifier");
  ...
  if (RESULT_OK != result) {
    LOG_WARNING("AnimEngine.Update", "Unable to process messages (result %d)",
                result);
    return result;
  }
  ...
  // Update animations
  _streamingAnimationModifier->ApplyAlterationsBeforeUpdate(
      _animationStreamer.get());

  _animationStreamer->Update();

  _streamingAnimationModifier->ApplyAlterationsAfterUpdate(
      _animationStreamer.get());

  // Update audio controller
  if (_audioControllerPtr != nullptr) {
    // Update mic info in Audio Engine
    const auto& micDirectionMsg =
        _context->GetMicDataSystem()->GetLatestMicDirectionMsg();
...
void AnimEngine::RegisterTickPerformance(
...
void AnimEngine::HandleMessage(const RobotInterface::TextToSpeechPrepare& msg) {
  DEV_ASSERT(_ttsComponent,
             "AnimEngine.TextToSpeechPrepare.InvalidTTSComponent");
...
void AnimEngine::HandleMessage(const RobotInterface::TextToSpeechCancel& msg) {
  DEV_ASSERT(_ttsComponent,
             "AnimEngine.TextToSpeechCancel.InvalidTTSComponent");
  _ttsComponent->HandleMessage(msg);
}
...
void AnimEngine::HandleMessage(const RobotInterface::ExternalAudioChunk& msg) {
  DEV_ASSERT(_sdkAudioComponent,
             "AnimEngine.ExternalAudioChunk.InvalidSDKAudioComponent");
  _sdkAudioComponent->HandleMessage(msg);
}

void AnimEngine::HandleMessage(
    const RobotInterface::ExternalAudioComplete& msg) {
  DEV_ASSERT(_sdkAudioComponent,
             "AnimEngine.ExternalAudioComplete.InvalidSDKAudioComponent");
  _sdkAudioComponent->HandleMessage(msg);
}

void AnimEngine::HandleMessage(const RobotInterface::ExternalAudioCancel& msg) {
  DEV_ASSERT(_sdkAudioComponent,
             "AnimEngine.ExternalAudioCancel.InvalidSDKAudioComponent");
  _sdkAudioComponent->HandleMessage(msg);
}
...
void AnimEngine::HandleMessage(
  ...
  DEV_ASSERT(_sdkAudioComponent,
             "AnimEngine.ExternalAudioPrepare.InvalidSDKAudioComponent");
  _sdkAudioComponent->HandleMessage(msg);
}
