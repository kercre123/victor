...
class MicTriggerConfig {
 public:
  ...
  enum class ModelType { size_1mb, size_500kb, size_250kb, Count };
  ...
  struct TriggerDataPaths {
    ...
    bool operator==(const TriggerDataPaths& other) const {
      return _dataDir == other._dataDir && _netFile == other._netFile &&
             _searchFile == other._searchFile;
    }
    bool operator!=(const TriggerDataPaths& other) const {
      return !(*this == other);
    }
    ...
    bool IsValid() const {
      return !_dataDir.empty() && !_netFile.empty() && !_searchFile.empty();
    }

    std::string GenerateNetFilePath(const std::string& prefixPath = "") const;

    std::string GenerateSearchFilePath(
        const std::string& prefixPath = "") const;
  };

  // Note 'Count' and '-1' values indicate to use default
  TriggerDataPaths GetTriggerModelDataPaths(
      Util::Locale locale, ModelType modelType = ModelType::Count,
