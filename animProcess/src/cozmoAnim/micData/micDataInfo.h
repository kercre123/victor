...
class MicDataInfo {
 public:
  ...
  static constexpr uint32_t kMinAudioSizeToSave_ms =
      kTriggerOverlapSize_ms + 100;
  ...
  static constexpr uint32_t kMaxRecordTime_ms =
  ...
  // Note: Must set the fade in duration before CollectProcessedAudio() is
  // called, default duration is 0 (no fade)
  void SetAudioFadeInTime(uint32_t fadeInTime_ms);

  void CollectRawAudio(const AudioUtil::AudioSample* audioChunk, size_t size);
  void CollectProcessedAudio(const AudioUtil::AudioSample* audioChunk,
                             size_t size);
 ...
 private:
  ...
  // These members are accessed via multiple threads when the job is running, so
  // they use a mutex
