 ...
 * Created: 9/27/2017
 *
 * Description: Handles processing the mic samples from the robot process:
 * combining the channels, and extracting direction data.
 *
...
// Time necessary for the VAD logic to wait when there's no activity, before we
// begin skipping processing for performance. Note that this probably needs to
// at least be as long as the trigger, which is ~ 500-750ms.
CONSOLE_VAR_RANGED(uint32_t, kMicData_QuietTimeCooldown_ms, CONSOLE_GROUP, 1000,
                   500, 10000);

#if ANKI_DEV_CHEATS

CONSOLE_VAR(bool, kMicData_SaveRawFullIntent, CONSOLE_GROUP, false);
CONSOLE_VAR(bool, kMicData_SaveRawFullIntent_WakeWordless, CONSOLE_GROUP,
            false);
...
CONSOLE_VAR_ENUM(uint8_t, kDevForceProcessState, CONSOLE_GROUP,
                 _currentDevForcedProcesState,
                 "NormalOperation,None,NoProcessingSingleMic,"
                 "SigEsBeamformingOff,SigEsBeamformingOn");
...
const MicProcessingState kDefaultProcessingState =
    MicProcessingState::SigEsBeamformingOff;
const MicProcessingState kLowPowerModeProcessingState =
    MicProcessingState::NoProcessingSingleMic;
...
CONSOLE_VAR_RANGED(float, maxTriggerProcTime_ms, ANKI_CPU_CONSOLEVARGROUP, 10,
                   10, 32);
CONSOLE_VAR_ENUM(u8, kMicDataProcessorRaw_Logging, ANKI_CPU_CONSOLEVARGROUP, 0,
                 Util::CpuProfiler::CpuProfilerLogging());
CONSOLE_VAR_ENUM(u8, kMicDataProcessorTrigger_Logging, ANKI_CPU_CONSOLEVARGROUP,
                 0, Util::CpuProfiler::CpuProfilerLogging());
#endif

constexpr auto kCladMicDataTypeSize = sizeof(RobotInterface::MicData::data) /
                                      sizeof(RobotInterface::MicData::data[0]);
static_assert(
    kCladMicDataTypeSize == kIncomingAudioChunkSize,
...
MicDataProcessor::MicDataProcessor(const Anim::AnimContext* context,
  ...
  _selectedSearchBeamConfidence =
      SEDiagGetIndex("search_result_best_beam_confidence");
...
void MicDataProcessor::Init() {
  ASSERT_NAMED(_micDataSystem != nullptr,
               "MicDataProcessor.Init._micDataSystem.IsNull");
  ASSERT_NAMED(
      _micDataSystem->GetSpeechRecognizerSystem() != nullptr,
  ...
  _processTriggerThread =
      std::thread(&MicDataProcessor::ProcessTriggerLoop, this);
}
...
void MicDataProcessor::InitVAD() {
  ...
  /* set up VAD */
  SVadSetDefaultConfig(_sVadConfig.get(), kSamplesPerBlockPerChannel,
                       (float)AudioUtil::kSampleRate_hz);
  _sVadConfig->AbsThreshold = 250.0f;  // was 400
  _sVadConfig->HangoverCountDownStart =
      10;  // was 25, make 25 blocks (1/4 second) to see it actually end a
           // couple times
  SVadInit(_sVadObject.get(), _sVadConfig.get());
}

void MicDataProcessor::VoiceTriggerWordDetection(
...
void MicDataProcessor::FakeTriggerWordDetection(bool fromMute) {
  const AudioUtil::SpeechRecognizerCallbackInfo info{
      .result = "", .startTime_ms = 0, .endTime_ms = 0, .score = 0.0f};
  const auto source = fromMute ? TriggerWordDetectSource::ButtonFromMute
                               : TriggerWordDetectSource::Button;
  TriggerWordDetectCallback(source, info);
}

void MicDataProcessor::GetLatestMicDirectionData(
    MicDirectionData& out_lastSample,
...
void MicDataProcessor::TriggerWordDetectCallback(
    TriggerWordDetectSource source,
    const AudioUtil::SpeechRecognizerCallbackInfo& info) {
  ShowAudioStreamStateManager* showStreamState =
      _context->GetShowAudioStreamStateManager();
  // Ignore extra triggers during streaming
  if (_micDataSystem->HasStreamingJob() ||
      !showStreamState->HasValidTriggerResponse()) {
    return;
  }

  // By the time the earcon completes, engine may have changed its response, so
  // assume the decision to stream should be based on the engine-requested state
  // at the time of the trigger word callback. Ugh. Anyway, we should cache
  // shouldStream and use it in earConCallback
  ...
  auto earConCallback = [this, shouldStream](bool success) {
    // If we didn't succeed, it means that we didn't have a wake word response
    // setup
    if (success) {
      RobotTimeStamp_t mostRecentTimestamp =
          CreateTriggerWordDetectedJobs(shouldStream);
      LOG_INFO("MicDataProcessor.TWCallback", "Timestamp %d",
               (TimeStamp_t)mostRecentTimestamp);
    } else {
      // since we're not opening up a stream, we need to reset the streaming
      // light since it get's turned on when we hear the trigger word
      _micDataSystem->SetWillStream(false);
      LOG_WARNING("MicDataProcessor.TWCallback",
  ...
  const bool buttonPress =
      (source == TriggerWordDetectSource::Button) || muteButton;
  if (muteButton) {
    // don't play the get-in if this trigger word started from mute, because the
    // mute animation should be playing
  ...
  const bool willStreamAudio =
      showStreamState->ShouldStreamAfterTriggerWordResponse() &&
  ...
  auto engineMessage = std::make_unique<RobotInterface::RobotToEngine>(
      std::move(twDetectedMessage));
  _micDataSystem->SendMessageToEngine(std::move(engineMessage));

  // Tell signal essence software to lock in on the current direction if it's
  // known NOTE: This is disabled for now as we've gotten better accuracy with
  // the direction of the intent that happens after this point, so it is
  // currently not desireable to lock in the supposed trigger direction, as that
  // direction can be incorrect due to motor noise, speaker noise, etc. The code
  // is left here with this comment to make resurrecting this functionality in
  // the SE software easier in the future, if desired. if (currentDirection !=
  // kDirectionUnknown)
  // {
  ...
  LOG_INFO("MicDataProcessor.TWCallback", "Direction index %d",
           currentDirection);
}

RobotTimeStamp_t MicDataProcessor::CreateStreamJob(
    CloudMic::StreamType streamType, uint32_t overlapLength_ms) {
  // Setup Job
  auto newJob = std::make_shared<MicDataInfo>();
  newJob->_writeLocationDir =
      Util::FileUtils::FullFilePath({_writeLocationDir, "triggeredCapture"});
  ...
  newJob->_audioSaveCallback = std::bind(&MicDataSystem::AudioSaveCallback,
                                         _micDataSystem, std::placeholders::_1);
#endif
  ...
  // The immediate buffer is bigger than just the overlap time (time right after
  // trigger end but before trigger was recognized), so that the immediate
  // buffer also contains the trigger itself. So here we set our start index to
  // only capture that in-between time, and push it into the streaming job for
  // intent matching
  ...
  if (overlapLength_ms > 0) {
    ...
    size_t triggerOverlapStartIdx =
        (maxIndex > overlapCount) ? (maxIndex - overlapCount) : 0;
  ...
  RobotTimeStamp_t mostRecentTimestamp =
      _immediateAudioBuffer[_procAudioRawComplete - 1].timestamp;
...
RobotTimeStamp_t MicDataProcessor::CreateTriggerWordDetectedJobs(
  ...
  if (shouldStream) {
    // First we create the job responsible for streaming the intent after the
    // trigger
    mostRecentTimestamp =
        CreateStreamJob(CloudMic::StreamType::Normal, kTriggerOverlapSize_ms);
  } else {
    LOG_INFO("MicDataProcessor.CreateTriggerWordDetectedJobs.NoStreaming",
             "Not adding streaming jobs because disabled");
  }

  // Now we set up the optional job for recording _just_ the trigger that was
  // just recognized
  ...
  if (saveTriggerOnly) {
    ...
    triggerJob->_writeLocationDir =
        Util::FileUtils::FullFilePath({_writeLocationDir, "triggersOnly"});
    ...
    triggerJob->_audioSaveCallback =
        std::bind(&MicDataSystem::AudioSaveCallback, _micDataSystem,
                  std::placeholders::_1);
...
void MicDataProcessor::ProcessRawAudio(RobotTimeStamp_t timestamp,
                                       ...
                                       uint32_t robotStatus, float robotAngle) {
  ANKI_CPU_PROFILE("MicDataProcessor::ProcessRawAudio");
  TimedMicData* nextSampleSpot = nullptr;
  {
    // Note we don't bother to free any slots here that have been consumed (by
    // comparing size to _procAudioXferCount) because it's unnecessary with the
    // circular buffer.

    std::unique_lock<std::mutex> lock(_procAudioXferMutex);
    auto xferAvailableCheck = [this]() {
      return _processThreadStop ||
             _procAudioXferCount < _immediateAudioBuffer.capacity();
    };
  ...
  MicDirectionData directionResult = ProcessMicrophonesSE(
      audioChunk, nextSample.audioBlock.data(), robotStatus, robotAngle);

  // Feed the samples to the beat detector. Optionally either use a raw single
  // channel (the first quarter of the un-interleaved audio block) or the
  // processed audio block
  auto* audioSource = kBeatDetectorUseProcessedAudio
                          ? nextSample.audioBlock.data()
                          : audioChunk;

  UpdateBeatDetector(audioSource, kSamplesPerBlockPerChannel);

  // Now we're done filling out this slot, update the count so it can be
  // consumed
  {
    std::lock_guard<std::mutex> lock(_procAudioXferMutex);
    ++_procAudioXferCount;
  ...
  std::copy(directionResult.confidenceList.begin(),
            directionResult.confidenceList.end(), newMessage.confidenceList);

  auto engineMessage =
      std::make_unique<RobotInterface::RobotToEngine>(std::move(newMessage));
  _micDataSystem->SendMessageToEngine(std::move(engineMessage));
}

MicDirectionData MicDataProcessor::ProcessMicrophonesSE(
    const AudioUtil::AudioSample* audioChunk, AudioUtil::AudioSample* bufferOut,
    uint32_t robotStatus, float robotAngle) {
  std::lock_guard<std::mutex> lock(_seInteractMutex);
  PolicySetAbsoluteOrientation(robotAngle);
  // Note that currently we are only monitoring the moving flag. We _could_ also
  // discard mic data when the robot is picked up, but that is being evaluated
  // with design before implementation, see VIC-1219
  const bool robotIsMoving =
      static_cast<bool>(robotStatus & (uint32_t)RobotStatusFlag::IS_MOVING);
  const bool robotStoppedMoving = !robotIsMoving && _robotWasMoving;
  _isInLowPowerMode = static_cast<bool>(
      robotStatus & (uint32_t)RobotStatusFlag::CALM_POWER_MODE);
  _robotWasMoving = robotIsMoving;

  // Check if we are playing audio through the speaker. Add a small delay after
  // the speaker stops 'playing', since it's possible that the speaker is still
  // actually playing stuff for a small time after this starts to return false.
  ...
  const bool hasRobotNoise =
      (robotIsMoving ||
       (_isSpeakerActive && kMicData_SpeakerNoiseDisablesMics));

  if (robotStoppedMoving || speakerStoppedPlaying) {
    // When the robot has stopped moving (and the gears are no longer making
    // noise) or the speaker has just stopped playing audio, we reset the mic
    // direction confidence values to be based on non-noisy data
    MMIfResetLocationSearch();
  }

  // We only care about checking one channel, and since the channel data is
  // uninterleaved when passed in here, we simply give the start of the buffer
  // as the input to run the vad detection on
  ...
  int activityFlag = 0;
  {
    ...
    // Note while we _can_ pass a confidence value here adjusted while the robot
    // is moving, we'd rather err on the side of always thinking we hear a voice
    // when the robot moves or plays audio from the speaker, so we maximize our
    // chances of hearing any triggers over the noise. So when the robot is
    // moving, ignore the VAD, and instead just set activity to true.
    const float vadConfidence = 1.0f;
    activityFlag =
        DoSVad(_sVadObject.get(),  // object
               vadConfidence,  // confidence it is okay to measure noise floor,
                               // i.e. no known activity like gear noise
    ...
    // Keep a counter from the last active vad flag. When it hits 0 don't bother
    // doing the trigger recognition, then reset the counter when the flag is
    // active again
  ...
  if (!_isInLowPowerMode) {
    ...
    processingState = hasRobotNoise ? ProcessingState::SigEsBeamformingOff
                                    : ProcessingState::SigEsBeamformingOn;
  } else {
  ...
  // Allow overriding (for testing) to force enable or disable mic data
  // processing & force processing state
  ...
  if ((kDevForceProcessState > 0) ||
      (kDevForceProcessState != _currentDevForcedProcesState)) {
  ...
  switch (_activeProcState) {
    ...
    case ProcessingState::SigEsBeamformingOn: {
      ...
      static const std::array<AudioUtil::AudioSample,
                              kSamplesPerBlockPerChannel * kNumInputChannels>
          dummySpeakerOut{};
      {
  ...
  // When we know the robot is making noise (via moving or speaker is playing)
  // or not using a Signal Essense processing we clear direction data
  if (hasRobotNoise || !directionIsAvailable) {
    ...
    const auto* confListSrc =
        reinterpret_cast<const float*>(searchConfState->u.vp);
    // NOTE currently SE only calculates the 12 main directions (not "unknown"
    // or directly above the mics) so we only copy the 12 main directions
    std::copy(confListSrc, confListSrc + kLastValidIndex + 1,
              result.confidenceList.begin());
  }
  return result;
}
...
void MicDataProcessor::ProcessRawLoop() {
  ...
  if (error != 0) {
    LOG_ERROR("MicDataProcessor.ProcessRawLoop", "SetAffinityMaskError %d",
              error);
  }
#endif

  static constexpr uint32_t expectedAudioDropsPerAnimLoop = 7;
  static const uint32_t maxProcTime_ms =
      expectedAudioDropsPerAnimLoop * maxProcessingTimePerDrop_ms;
  const auto maxProcTime = std::chrono::milliseconds(maxProcTime_ms);
  while (!_processThreadStop) {
    ANKI_CPU_TICK("MicDataProcessorRaw", maxProcTime_ms,
                  Util::CpuProfiler::CpuProfilerLoggingTime(
    ...
    while (rawAudioToProcess.size() > 0) {
      ...
      // Copy the current set of jobs we have for recording audio, so the list
      // can be added to while processing continues
      std::deque<std::shared_ptr<MicDataInfo>> jobs =
          _micDataSystem->GetMicDataJobs();
      ...
      // Factory test doesn't need to do any mic processing, it just uses raw
      // data
      if (!FACTORY_TEST) {
        // Process the audio into a single channel, and collect it if desired
        (void)ProcessRawAudio(nextData.timestamp, audioChunk,
...
void MicDataProcessor::ProcessTriggerLoop() {
  ...
  int error =
      pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpu_set);
  if (error != 0) {
    LOG_ERROR("MicDataProcessor.ProcessTriggerLoop", "SetAffinityMaskError %d",
              error);
  }
#endif

  while (!_processThreadStop) {
    ANKI_CPU_TICK("MicDataProcessorTrigger", maxTriggerProcTime_ms,
                  Util::CpuProfiler::CpuProfilerLoggingTime(
    ...
    TimedMicData* readyDataSpot = nullptr;
    {
      ...
      std::unique_lock<std::mutex> lock(_procAudioXferMutex);
      auto dataReadyCheck = [this]() {
        return _processThreadStop || _procAudioXferCount > 0;
      };
      ...
      // Grab a handle to the next available data that's been processed out of
      // raw but not yet "transferred" to the trigger word recognition
      // processing
      readyDataSpot =
          &_immediateAudioBuffer[_procAudioRawComplete - _procAudioXferCount];
    }
    ...
    // Run the trigger detection, which will use the callback defined above
    {
      ...
      // Note we skip it if there is no activity as of the latest processed
      // audioblock
      _speechRecognizerSystem->Update(
          processedAudio.data(), (unsigned int)processedAudio.size(),
...
void MicDataProcessor::UpdateBeatDetector(
    const AudioUtil::AudioSample* const samples, const uint32_t nSamples) {
  ...
  if (_isInLowPowerMode) {
    ...
    if (beatDetected) {
      auto beatMessage =
          RobotInterface::BeatDetectorState{_beatDetector->GetLatestBeat()};
      auto engineMessage = std::make_unique<RobotInterface::RobotToEngine>(
          std::move(beatMessage));
      _micDataSystem->SendMessageToEngine(std::move(engineMessage));
    }
  }
}

void MicDataProcessor::ProcessMicDataPayload(
    const RobotInterface::MicData& payload) {
  // Store off this next job
  std::lock_guard<std::mutex> lock(_rawMicDataMutex);
  if (!_muteMics) {
    // Use whichever buffer is currently _not_ being processed
    auto& bufferToUse = (_rawAudioProcessingIndex == 1) ? _rawAudioBuffers[0]
                                                        : _rawAudioBuffers[1];
...
float MicDataProcessor::GetIncomingMicDataPercentUsed() {
  ...
  const auto updatedFullness =
      ((float)bufferInUse.size()) / ((float)bufferInUse.capacity());
  // Cache the current fullness for this buffer and use the greater of the two
  // buffer fullnesses This way the "fullness" returned is less variable and
  // better covers the worst case
  _rawAudioBufferFullness[inUseIndex] = updatedFullness;
  return MAX(_rawAudioBufferFullness[0], _rawAudioBufferFullness[1]);
}

void MicDataProcessor::SetActiveMicDataProcessingState(
    MicDataProcessor::ProcessingState state) {
  ...
  if (state != _activeProcState) {
    if (ENABLE_MIC_PROCESSING_STATE_UPDATE_LOG) {
      LOG_INFO("MicDataProcessor.SetActiveMicDataProcessingState",
               "Current state '%s' new state '%s'",
               GetProcessingStateName(_activeProcState),
               GetProcessingStateName(state));
    }
    ...
    switch (state) {
      ...
      case ProcessingState::SigEsBeamformingOff:
      case ProcessingState::SigEsBeamformingOn: {
        const bool shouldUseFallbackPolicy =
            (state == ProcessingState::SigEsBeamformingOff);
        const FallbackFlag_t policySetting =
            shouldUseFallbackPolicy ? kEcho_Cancel_Flag : FBF_AUTO_SELECT;
...
const char* MicDataProcessor::GetProcessingStateName(
    MicDataProcessor::ProcessingState state) const {
  switch (state) {
    case ProcessingState::None:
      return "None";
    case ProcessingState::NoProcessingSingleMic:
