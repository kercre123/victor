...
namespace {
const std::string kModelTypeStrings[] = {"size_1mb", "size_500kb", "size_250kb",
                                         "INVALID"};
constexpr auto kNumModelTypeStrings =
    sizeof(kModelTypeStrings) / sizeof(kModelTypeStrings[0]);
static_assert(
    kNumModelTypeStrings == ((int)MicTriggerConfig::ModelType::Count) + 1,
...
bool MicTriggerConfig::Init(const std::string& triggerKey,
                            const Json::Value& initData) {
  _localeTriggerDataMap.clear();

  // Verify this is a object of triggers
  if (!initData.isObject()) {
    LOG_ERROR("MicTriggerConfig.Init.JsonData",
              "Mic init data is not an object.");
    return false;
  }
  ...
  if (!triggerData.isArray()) {
    LOG_ERROR("MicTriggerConfig.Init.JsonData",
              "Mic trigger data is not an array.");
    return false;
  }
  ...
  for (const Json::Value& localeData : triggerData) {
    if (!localeData.isObject()) {
      LOG_ERROR("MicTriggerConfig.Init.JsonData",
                "Locale config data is not an object.");
      continue;
    }

    // Get the Locale type
    if (!localeData.isMember(kLocaleKey) ||
        !localeData[kLocaleKey].isString()) {
    ...
    const auto& nextLocale =
        Util::Locale::LocaleFromString(localeData[kLocaleKey].asString());
    ...
    if (!localeData.isMember(kDefaultModelTypeKey) ||
        !localeData[kDefaultModelTypeKey].isString()) {
      LOG_ERROR("MicTriggerConfig.LocaleDefaultModelType",
                "Locale data item does not contain default model type.\n%s",
                Json::StyledWriter().write(localeData).c_str());
      continue;
    }
    const auto& defaultModelType =
        ModelTypeFromString(localeData[kDefaultModelTypeKey].asString());

    // Load the model data list, make sure there's at least one entry
    if (!localeData.isMember(kModelListKey) ||
        !localeData[kModelListKey].isArray()) {
...
MicTriggerConfig::ModelDataMap MicTriggerConfig::InitModelData(
    const Json::Value& modelDataList) {
  // Verify this is a list of model data
  if (!modelDataList.isArray()) {
    LOG_ERROR("MicTriggerConfig.InitModelData.JsonData",
              "Model data is not an array.");
    return ModelDataMap{};
  }
  ...
  for (int i = 0; i < modelDataList.size(); ++i) {
    ...
    if (!modelData.isObject()) {
      LOG_ERROR("MicTriggerConfig.InitModelData.JsonData",
                "Model data is not an object.");
      continue;
    }

    // Verify the model type
    if (!modelData.isMember(kModelTypeKey) ||
        !modelData[kModelTypeKey].isString()) {
    ...
    auto nextModelType =
        ModelTypeFromString(modelData[kModelTypeKey].asString());
    if (nextModelType == ModelType::Count ||
        newModelDataMap.find(nextModelType) != newModelDataMap.end()) {
      LOG_ERROR("MicTriggerConfig.InitModelData.ModelType",
    ...
    if (!modelData.isMember(kDataDirectoryKey) ||
        !modelData[kDataDirectoryKey].isString()) {
    ...
    if (!modelData.isMember(kNetFileNameKey) ||
        !modelData[kNetFileNameKey].isString()) {
    ...
    if (!modelData.isMember(kDefaultSearchFileIndexKey) ||
        !modelData[kDefaultSearchFileIndexKey].isInt()) {
      LOG_ERROR("MicTriggerConfig.InitModelData.DefaultSearchFileIndex",
                "Model data item does not contain DefaultSearchFileIndex.\n%s",
                Json::StyledWriter().write(modelData).c_str());
      continue;
    }
    const auto defaultSearchFileIndex =
        modelData[kDefaultSearchFileIndexKey].asInt();

    // Verify the search file list is specified
    if (!modelData.isMember(kSearchFileListKey) ||
        !modelData[kSearchFileListKey].isArray()) {
    ...
    for (int i = 0; i < searchFileList.size(); ++i) {
      ...
      if (!searchFileData.isObject()) {
        LOG_ERROR("MicTriggerConfig.InitModelData.SearchFileData",
                  "SearchFile data is not an object.");
        continue;
      }

      // Verify the search file index
      if (!searchFileData.isMember(kSearchFileIndexKey) ||
          !searchFileData[kSearchFileIndexKey].isIntegral()) {
      ...
      if (!searchFileData.isMember(kSearchFileNameKey) ||
          !searchFileData[kSearchFileNameKey].isString()) {
        LOG_ERROR("MicTriggerConfig.InitModelData.SearchFileName",
                  "Search file data item does not contain file name.");
        continue;
      }
      const auto& searchFileName =
          searchFileData[kSearchFileNameKey].asString();
...
MicTriggerConfig::ModelType MicTriggerConfig::ModelTypeFromString(
...
MicTriggerConfig::TriggerDataPaths MicTriggerConfig::GetTriggerModelDataPaths(
    Util::Locale locale, MicTriggerConfig::ModelType modelType,
  ...
  modelType = (modelType != ModelType::Count)
                  ? modelType
                  : localeDataIter->second._defaultModelType;
  ...
  searchFileIndex = (searchFileIndex != -1)
                        ? searchFileIndex
                        : modelDataIter->second._defaultSearchFileIndex;
...
std::vector<std::string> MicTriggerConfig::GetAllTriggerModelFiles() const {
  ...
  for (const auto& localeDataKV : _localeTriggerDataMap) {
    for (const auto& modelDataKV : localeDataKV.second._modelDataMap) {
      auto netFilePath = Util::FileUtils::FullFilePath(
          {modelDataKV.second._dataDir, modelDataKV.second._netFile});
      triggerDataList.push_back(std::move(netFilePath));
      for (const auto& searchFileKV : modelDataKV.second._searchFileMap) {
        auto searchFilePath = Util::FileUtils::FullFilePath(
            {modelDataKV.second._dataDir, searchFileKV.second});
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::string MicTriggerConfig::TriggerDataPaths::GenerateNetFilePath(
    const std::string& prefixPath) const {
  return Util::FileUtils::FullFilePath({prefixPath, _dataDir, _netFile});
}

std::string MicTriggerConfig::TriggerDataPaths::GenerateSearchFilePath(
    const std::string& prefixPath) const {
