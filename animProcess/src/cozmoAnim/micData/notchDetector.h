 ...
 *
 * Description: Checks recently added power spectrums for a noticable notch
 * around a specific band. The power is computed periodically during AddSamples
 * (if requested), and HasNotch() will analyze average power using some ad-hoc
 * rules. Use of the Sliding DFT algorithm might be useful here. In some quick
 * benchmarks, the library used in audioFFT.h was still faster than my quick n
 * dirty sliding DFT, but I've since changed the window size and period.
 *
