 ...
 *              This is not a templated class (on, say, the window length) so
 * that the library methods are not in the header
 *
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
AudioFFT::AudioFFT(unsigned int N) : _N{N}, _buff{N, N}, _windowCoeffs(N, 0.0) {
  Reset();
...
std::vector<AudioFFT::DataType> AudioFFT::GetPower() {
  ...
  ret.push_back((_outData[0] * _outData[0] + _outData[1] * _outData[1]) *
                normFactor);
  for (int i = 2; i < _N; i += 2) {
    const DataType mag =
        _outData[i] * _outData[i] + _outData[i + 1] * _outData[i + 1];
...
void AudioFFT::DoDFT() {
  ...
  // pffft docs say: "If 'work' is NULL, then stack will be used instead (this
  // is probably the best strategy for small FFTs, say for N < 16384)."
  float* work = nullptr;
  PFFFT::pffft_transform_ordered((PFFFT::PFFFT_Setup*)_plan, _inData, _outData,
                                 work, PFFFT::PFFFT_FORWARD);
}
