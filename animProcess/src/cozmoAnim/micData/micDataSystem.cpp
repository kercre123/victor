...
#if ANKI_DEV_CHEATS
CONSOLE_VAR_RANGED(u32, kMicData_ClipRecordTime_ms, CONSOLE_GROUP, 4000, 500,
                   15000);
...
static_assert(
    std::is_same<std::remove_reference<decltype(
                     RobotInterface::MicDirection::confidenceList[0])>::type,
...
constexpr auto kMicDirectionConfListSize =
    sizeof(RobotInterface::MicDirection::confidenceList);
constexpr auto kMicDirectionConfListItemSize =
    sizeof(RobotInterface::MicDirection::confidenceList[0]);
static_assert(
    kMicDirectionConfListSize / kMicDirectionConfListItemSize ==
        decltype(MicDirectionData::confidenceList)().size(),
    "Expecting length of RobotInterface::MicDirection::confidenceList to match "
    "MicDirectionData::confidenceList");

void MicDataSystem::SetupConsoleFuncs() {
#if ANKI_DEV_CHEATS
  const auto enableTriggerHistoryFunc =
      [this](ConsoleFunctionContextRef context) {
        const bool enable = ConsoleArg_Get_Bool(context, "enable");
        EnableTriggerHistory(enable);
        context->channel->WriteLog("EnableRecentTriggers %s",
                                   enable ? "enabled" : "disable");
      };
  _devConsoleFuncs.emplace_front("EnableTriggerResults",
                                 std::move(enableTriggerHistoryFunc),
  ...
  const auto clearMicDataFunc = [this](ConsoleFunctionContextRef context) {
    if (!_writeLocationDir.empty()) {
      ...
      context->channel->WriteLog("Removed directory '%s'",
                                 _writeLocationDir.c_str());
    }
  };
  _devConsoleFuncs.emplace_front("ClearMicData", std::move(clearMicDataFunc),
                                 CONSOLE_GROUP ".zHiddenForSafety", "");
#endif
}
...
MicDataSystem::MicDataSystem(Util::Data::DataPlatform* dataPlatform,
  ...
  const std::string& dataWriteLocation =
      dataPlatform->pathToResource(Util::Data::Scope::Cache, "micdata");
  const std::string& triggerDataDir =
      dataPlatform->pathToResource(Util::Data::Scope::Resources, "assets");
  _writeLocationDir = dataWriteLocation;
  _micDataProcessor.reset(
      new MicDataProcessor(_context, this, dataWriteLocation));
  _speechRecognizerSystem.reset(
      new SpeechRecognizerSystem(_context, this, triggerDataDir));

  _persistentFolder = Util::FileUtils::AddTrailingFileSeparator(
  ...
  const std::string sockName = std::string{MIC_SERVER_BASE_PATH} +
                               (robotID == 0 ? "" : std::to_string(robotID));
  _udpServer->SetBindClients(false);
  const bool udpSuccess = _udpServer->StartListening(sockName);
  ANKI_VERIFY(udpSuccess, "MicDataSystem.Constructor.UdpStartListening",
              "Failed to start listening on socket %s", sockName.c_str());
}
...
void MicDataSystem::Init(const Anim::RobotDataLoader& dataLoader) {
  ...
  SpeechRecognizerSystem::TriggerWordDetectedCallback callback =
      [this](const AudioUtil::SpeechRecognizerCallbackInfo& info) {
        ...
        if (_alexaState == AlexaSimpleState::Active) {
          // Don't run "hey vector" when alexa is in the middle of an
          // interaction, or if the mic is muted
          return;
        }

        // saying "hey vector" should exit certain alexa debug screens and
        // cancel auth. FaceInfoScreen isn't currently set up to handle threads,
        // so set a flag that is handled in Update()
  ...
  if (webService) {
    AddSignalHandle(webService->OnWebVizSubscribed(kSpeechRecognizerWebvizName)
                        .ScopedSubscribe([this](const auto& sendFunc) {
                          SendRecentTriggerDetectionToWebViz(sendFunc);
                        }));
  }
  SetupConsoleFuncs();
#endif
...
MicDataSystem::~MicDataSystem() {
  // Tear down the mic data processor explicitly first, because it uses
  // functionality owned by MicDataSystem
  _micDataProcessor.reset();

  _udpServer->StopListening();
}

void MicDataSystem::ProcessMicDataPayload(
    const RobotInterface::MicData& payload) {
  _micDataProcessor->ProcessMicDataPayload(payload);
}

void MicDataSystem::RecordRawAudio(uint32_t duration_ms,
                                   const std::string& path, bool runFFT) {
  RecordAudioInternal(duration_ms, path, MicDataType::Raw, runFFT);
}

void MicDataSystem::RecordProcessedAudio(uint32_t duration_ms,
...
void MicDataSystem::StartWakeWordlessStreaming(CloudMic::StreamType type,
  ...
  if (HasStreamingJob()) {
    // We "fake" having a streaming job in order to achieve the "feel" of a
    // minimum streaming time for UX reasons (I think?). This means that
    // HasStreamingJob() may actually lie, so if we have a job, but have
    // completed streaming, and the engine is requesting a wakewordless stream
    // (e.g. knowledge graph), we should clear it now. This is a workaround to
    // fix VIC-13402 (a blocker for R1.4.1)

    // TODO:(bn) VIC-13438 this is tech debt and should be cleaned up

    if (_currentlyStreaming && _streamingComplete) {
      LOG_INFO(
          "MicDataSystem.StartWakeWordlessStreaming.OverlappingWithFakeStream."
          "Opening",
          "Request came in overlapping with a 'fake' extended request, so "
          "cancel it before starting a new one");
      ClearCurrentStreamingJob();
    } else {
      LOG_WARNING(
          "MicDataSystem.StartWakeWordlessStreaming.OverlappingStreamRequests",
          "Received StartWakeWorldlessStreaming message from engine, but "
          "micDataSystem is already streaming (not faking to extend the "
          "stream)");
      return;
    }
  }

  // we want to start the stream AFTER the audio is complete so that it is not
  ...
  auto callback = [this, type](bool success) {
    ...
    // setup
    if (success) {
      // it would be highly unlikely that we started another streaming job while
      // waiting for the earcon, but doesn't hurt to check
      if (!HasStreamingJob()) {
        ...
        LOG_WARNING(
            ...
            "OverlappingStreamRequests",
            "Started streaming job while waiting for "
            "StartTriggerResponseWithoutGetIn callback");
        SetWillStream(false);
      }
    } else {
      LOG_WARNING("MicDataSystem.StartWakeWordlessStreaming.CantStreamToCloud",
                  "Wakewordless streaming request received, but incapable of "
                  "opening the cloud stream, so ignoring request");
      SetWillStream(false);
    }
  };
...
void MicDataSystem::FakeTriggerWordDetection() {
  // completely ignore _micMuted and IsButtonPressAlexa() and stop alerts no
  // matter what
  ...
  if (_micMuted) {
    ...
    // This is an annoying code path since FaceInfoScreenManager::ToggleMute
    // calls back into MicDataSystem. But FaceInfoScreenManager is already set
    // up to check for various button clicks...
  ...
  if (IsButtonPressAlexa()) {
    ...
    // This next check is probably not necessary, but for symmetry, the hey
    // vector button press shouldn't trigger if alexa is in the middle of an
    // interaction
    if (_alexaState != AlexaSimpleState::Active) {
...
void MicDataSystem::RecordAudioInternal(uint32_t duration_ms,
                                        const std::string& path,
                                        MicDataType type, bool runFFT) {
  MicDataInfo* newJob = new MicDataInfo{};

  // If the input path has a file separator, remove the filename at the end and
  // use as the write directory
  if (path.find('/') != std::string::npos) {
    std::string nameBase = Util::FileUtils::GetFileName(path);
    newJob->_writeLocationDir =
        path.substr(0, path.length() - nameBase.length());
    newJob->_writeNameBase = nameBase;
  } else {
    // otherwise used the saved off write directory, and just the input path as
    // the name base
    newJob->_writeLocationDir = _writeLocationDir;
    newJob->_writeNameBase = path;
  }
  newJob->_audioSaveCallback =
      std::bind(&MicDataSystem::AudioSaveCallback, this, std::placeholders::_1);
  ...
  if (runFFT) {
    ...
    newJob->_rawAudioFFTCallback =
        [weakdata = std::move(weakptr)](std::vector<uint32>&& result) {
...
void MicDataSystem::Update(BaseStationTime_t currTime_nanosec) {
  ...
  const ssize_t bytesReceived =
      _udpServer->Recv((char*)receiveArray, kMaxReceiveBytes);
  if (bytesReceived > 0) {
    ...
    switch (msg.GetTag()) {
      ...
      case CloudMic::MessageTag::testStarted: {
        ...
        // TODO:(bn) check stream state here? Currently just assuming streaming
        // is on
      ...
      case CloudMic::MessageTag::connectionResult: {
        LOG_INFO("MicDataSystem.Update.RecvCloudProcess.connectionResult", "%s",
                 msg.Get_connectionResult().status.c_str());
        FaceInfoScreenManager::getInstance()->SetNetworkStatus(
            msg.Get_connectionResult().code);

        // Send the results back to engine
        ReportCloudConnectivity msgToEngine;
        msgToEngine.code =
            static_cast<ConnectionCode>(msg.Get_connectionResult().code);
        msgToEngine.numPackets = msg.Get_connectionResult().numPackets;
        msgToEngine.expectedPackets =
            msg.Get_connectionResult().expectedPackets;
        RobotInterface::SendAnimToEngine(msgToEngine);
        break;
      }

      default:
        LOG_INFO("MicDataSystem.Update.RecvCloudProcess.UnexpectedSignal",
  ...
  if (_saveJob != nullptr) {
    if (_saveJob->CheckDone()) {
      ...
      recordingSecondsRemaining =
          (_saveJob->GetTimeToRecord_ms() - _saveJob->GetTimeRecorded_ms()) /
          1000;
    }
  }
  ...
  if (_forceRecordClip && nullptr == _saveJob) {
    MicDataInfo* newJob = new MicDataInfo{};
    newJob->_writeLocationDir =
        Util::FileUtils::FullFilePath({_writeLocationDir, "debugCapture"});
  ...
  // Minimum length of time to display the "trigger heard" symbol on the mic
  // data debug screen (is extended by streaming)
  ...
  // lock the job mutex
  {
    ...
    if (!_currentlyStreaming && HasStreamingJob()
      ...
      if (_udpServer->HasClient()) {
        ...
        // even though this isn't necessarily the exact frame the backpack
        // lights begin (since that's done in a different thread), it doesn't
        // make a noticeable difference since this is an arbitrary number and
        ...
        auto hw =
            CloudMic::Hotword{CloudMic::StreamType::Normal, _locale.ToString(),
        ...
        ClearCurrentStreamingJob();
        LOG_INFO("MicDataSystem.Update.StreamingStartIgnored",
                 "Ignoring stream start as no clients connected.");
      }
    }

    //  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -
    //  -  -  -  -  -  -  -  -  -  -  -  -  -  -
    // ... this block is where we actually do the streaming of the mic data up
    // to the cloud ...
    ...
    if (_currentlyStreaming) {
      ...
      if (!_streamingComplete) {
        static constexpr size_t kMaxRecordNumChunks =
            (kStreamingTimeout_ms / kTimePerChunk_ms) + 1;
        ...
        if (receivedStopMessage || didTimeout) {
          ...
          LOG_INFO("MicDataSystem.Update.StreamingEnd", "%zu ms",
                   _streamingAudioIndex * kTimePerChunk_ms);
#if ANKI_DEV_CHEATS
...
#endif
          {
            // Copy any new data that has been pushed onto the currently
            // streaming job
            AudioUtil::AudioChunkList newAudio =
                _currentStreamingJob->GetProcessedAudio(_streamingAudioIndex);
      ...
      // we want to extend the streaming state so that it at least appears to be
      // streaming for a minimum duration here we hold onto the streaming job
      // until we've reached that minimum duration note: the streaming job will
      // not actually be recording, we're simply holding it so we don't start a
      // new job

      // TODO:(bn) VIC-13438 this is tech debt and has caused bugs and confusion
      // and should be cleaned up
      if (_streamingComplete) {
        // our stream is complete, so clear out the current stream as long as
        // our minimum streaming time has elapsed
        uint32_t minStreamingDuration_ms =
            _context->GetShowAudioStreamStateManager()
                ->GetMinStreamingDuration();
        const BaseStationTime_t minStreamDuration_ns =
            minStreamingDuration_ms * 1000 * 1000;
        const BaseStationTime_t minStreamEnd_ns =
            _streamBeginTime_ns + minStreamDuration_ns;
        if ((currTime_nanosec >= minStreamEnd_ns) || realStreamHasFinished) {
          LOG_INFO("MicDataSystem.Update.StreamingComplete.ClearJob",
                   "Clearing streaming job now that enough time has elapsed");
          ClearCurrentStreamingJob();
        }
      }
    }
...
#if ANKI_DEV_CHEATS
  // Store off a copy of (one of) the micDirectionData from this update for
  // debug drawing
  ...
  for (const auto& msg : stolenMessages) {
    if (msg->tag == RobotInterface::RobotToEngine::Tag_triggerWordDetected) {
      ...
      RobotInterface::SendAnimToEngine(msg->micDirection);
               ...
               RobotInterface::RobotToEngine::Tag_beatDetectorState) {
      RobotInterface::SendAnimToEngine(msg->beatDetectorState);
    } else {
      DEV_ASSERT_MSG(false, "MicDataSystem.Update.UnhandledOutgoingMessageType",
                     "%s",
                     RobotInterface::RobotToEngine::TagToString(msg->tag));
    }
  }
  ...
  if (updatedMicDirection || recordingSecondsRemaining != 0) {
    FaceInfoScreenManager::getInstance()->DrawConfidenceClock(
        _latestMicDirectionMsg, rawBufferFullness, recordingSecondsRemaining,
  ...
  if (_abortAlexaScreenDueToHeyVector) {
    ...
    Alexa* alexa = _context->GetAlexa();
    if (alexa != nullptr) {
      // sign out before we change the info screen so the reason is more
      // descriptive
    ...
    if (_context != nullptr) {
      ...
      if (audioController != nullptr) {
        ...
        if (pluginInterface != nullptr) {
          _speakerLatency_ms =
              pluginInterface->AkAlsaSinkGetSpeakerLatency_ms();
          if (_speakerLatency_ms != 0) {
...
void MicDataSystem::SendMessageToEngine(
...
void MicDataSystem::AudioSaveCallback(const std::string& dest) {
  if (_udpServer->HasClient()) {
    SendUdpMessage(
        CloudMic::Message::CreatedebugFile(CloudMic::Filename{dest}));
  }
  ...
  // let the world know our recording is now complete
  {
    ...
    // shouldn't be a problem, but if we ever hit this we'll need to find
    // another solution
    DEV_ASSERT(dest.length() <= 255,
               "MicDataSystem::AudioSaveCallback path is too long for "
               "MicRecordingComplete message");
    ...
    const auto callback =
        [this](const AudioUtil::SpeechRecognizerCallbackInfo& info,
          ...
          if (ignore || HasStreamingJob()) {
            ...
            // 3. ignore flag is ture, either playback recognizer triggered
            // positive or there is a "notch"
            return;
          }
          Alexa* alexa = _context->GetAlexa();
          ShowAudioStreamStateManager* showStreamState =
...
void MicDataSystem::ToggleMicMute() {
  // TODO (VIC-11587): we could save some CPU if the wake words recognizers are
  // actually disabled here. For now, we don't feed the raw audio buffer when
  // receiving messages from robot process. Which stops running the mic
  // processor and recognizers methods, therefore, saving CPU. However, mic
  // threads are still runing.
  _micMuted = !_micMuted;
  ...
  if (audioController != nullptr) {
    ...
    const auto eventID = ToAudioEventId(
        _micMuted
  ...
  // Note that Alexa also has a method to stopStreamingMicrophoneData, but
  // without the wakeword, the samples go no where. Also check if it saves CPU
  // to drop samples. Note that the time indices for the wake word bookends
  // might be wrong afterwards.
...
bool MicDataSystem::IsButtonPressAlexa() const {
  // Instead of only using _buttonPressIsAlexa, also check whether alexa has
  // been opted in. If the user sets the button to alexa, clears user data,
  // reverts to factory and then OTAs to latest, Alexa's init sequence will
  // message engine that alexa is disabled, which sets the button functionality
  // back to hey vector. But if jdocs settings are pulled _after_ that, it can
  // switch back to alexa. For now, we check here instead of having engine's
  // SettingsManager check the AlexaComponent's auth state, since that is tied
  // to the order of messages received from anim and so would need to track more
  // state. As a result, the user's button setting will still be set to alexa,
  // even if alexa is disabled. However, currently the app doesnt show this
  // setting if alexa is disabled, to it will be functionally equivalent to the
  // user.
  // TODO (VIC-12527): handle this in engine instead (or in addition to here,
  // since this extra check doesnt actually hurt if the app doesn't show the
  // setting and no other anim components are listening to
  // SetButtonWakeWordIsAlexa)
  Alexa* alexa = _context->GetAlexa();
...
void MicDataSystem::SendTriggerDetectionToWebViz(
  ...
  if (_context != nullptr) {
    ...
    if (webService != nullptr &&
        webService->IsWebVizClientSubscribed(kSpeechRecognizerWebvizName)) {
      const Json::Value& webVizData =
          _devEnableTriggerHistory ? _devTriggerResults.back() : data;
...
void MicDataSystem::SendRecentTriggerDetectionToWebViz(
...
void MicDataSystem::SendRecognizerDasLog(
    const AudioUtil::SpeechRecognizerCallbackInfo& info,
    const char* stateStr) const {
  MicData::MicDirectionData directionData;
  MicData::DirectionIndex dominantDirection;
  _micDataProcessor->GetLatestMicDirectionData(directionData,
                                               dominantDirection);
  DASMSG(speech_recognized, "mic_data_system.speech_trigger_recognized",
         "Voice trigger recognized");
  DASMSG_SET(s1, info.result.c_str(), "Recognized result");
  DASMSG_SET(s2, (stateStr != nullptr) ? stateStr : "",
             "Current Alexa UX State");
  DASMSG_SET(s3, std::to_string(info.score).c_str(), "Recognizer Score");
  DASMSG_SET(i1, dominantDirection,
             "Dominant Direction Index [0, 11], 12 is Unknown Direction");
  DASMSG_SET(i2, directionData.selectedDirection,
             "Selected Direction Index [0, 11], 12 is Unknown Direction");
