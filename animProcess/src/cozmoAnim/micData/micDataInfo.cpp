...
void MicDataInfo::CollectRawAudio(const AudioUtil::AudioSample* audioChunk,
                                  size_t size) {
  ...
  if (_typesToCollect.IsBitFlagSet(MicDataType::Raw)) {
    ...
    // Re-interlace the audio data, for the sake of the 4-channel .wav that'll
    // be written out.
    for (size_t sample = 0; sample < kSamplesPerBlockPerChannel; ++sample) {
      for (size_t channel = 0; channel < kNumInputChannels; ++channel) {
        newChunk[kNumInputChannels * sample + channel] =
            audioChunk[channel * kSamplesPerBlockPerChannel + sample];
      }
    }
...
void MicDataInfo::CollectProcessedAudio(
    const AudioUtil::AudioSample* audioChunk, size_t size) {
  std::lock_guard<std::mutex> lock(_dataMutex);
  if (_typesToCollect.IsBitFlagSet(MicDataType::Processed)) {
    ...
    if (_fadeInSamples > 0) {
      ...
      for (; (sampleIdx < size) && (_fadeInScalar < 1.0f); ++sampleIdx) {
        newChunk[sampleIdx] = static_cast<AudioUtil::AudioSample>(
            audioChunk[sampleIdx] * _fadeInScalar);
        _fadeInScalar += _fadeInStepSize;
        --_fadeInSamples;
      }
      // Copy remaining samples
      std::copy(audioChunk + sampleIdx, audioChunk + size,
                newChunk.begin() + sampleIdx);
    } else {
      // Copy entire chunk
...
void MicDataInfo::SetAudioFadeInTime(uint32_t fadeInTime_ms) {
  if (!_processedAudioData.empty()) {
    LOG_WARNING(
        "MicDataInfo.SetAudioFadeInTime",
...
void MicDataInfo::SaveCollectedAudio(const std::string& dataDirectory,
  ...
  // Check against a min recording length. If we're not recording raw and our
  // recorded processed time is too short, we're going to abandon saving it.
  if (_rawAudioData.empty() && (_processedAudioData.size() * kTimePerChunk_ms) <
                                   kMinAudioSizeToSave_ms) {
    return;
  }

  if (!nameToRemove.empty()) {
    Util::FileUtils::RemoveDirectory(
        Util::FileUtils::FullFilePath({dataDirectory, nameToRemove}));
  }

  const std::string& newDirPath =
      Util::FileUtils::FullFilePath({dataDirectory, nameToUse});
  bool createdNewDir = false;
  const std::string& writeLocationBase =
      Util::FileUtils::FullFilePath({newDirPath, nameToUse});
      ...
      auto saveRawWave = [dest = std::move(dest),
        ...
        if (doFFTProcess) {
          ...
          if (!Util::IsNearZero(length_s)) {
            ...
            LOG_INFO("MicDataInfo.FFTResultFromRaw", "%d %d %d %d", result[0],
...
// Since the local time on the robot is not reliable (especially over multiple
// reboots) we use a 2 part numerical naming convention to identify the oldest
// file and overwrite it. The first number indicates which iteration of the file
// it is (number of times overwritten) and the second number indicates the
// sequence of that file. This way a simple sort reveals the oldest file next in
// line to be overwritten. This also selects the original file that needs to be
// overwritten, and sets the name of that file to output.
std::string MicDataInfo::ChooseNextFileNameBase(std::string& out_dirToDelete) {
  ...
  while (listIter != dirNames.end()) {
    ...
    if (listIter->compare(0, kMicCapturePrefix.length(), kMicCapturePrefix) !=
        0) {
  ...
  // If number of entries is less than max, pick the name
  // miccapture_0000_(count())
  if (dirNames.size() < _numMaxFiles) {
    std::ostringstream newNameStream;
    newNameStream << kMicCapturePrefix << "0000_" << std::setfill('0')
                  << std::setw(4) << dirNames.size();
  ...
  const auto seqStrBegin =
      iterStrBegin + kNumberDigitsLength + 1;  // add one for the underscore
  ...
  std::ostringstream newNameStream;
  newNameStream << kMicCapturePrefix << std::setfill('0')
                << std::setw(kNumberDigitsLength) << (iterationNum + 1) << "_"
                << seqStr;
  return newNameStream.str();
}

std::vector<uint32_t> MicDataInfo::GetFFTResultFromRaw(
    const AudioUtil::AudioChunkList& data, float length_s) {
  ...
  for (auto i = 0; i < kNumInputChannels; ++i) {
    ...
    // Only look at the first half since the second half is just the inverse of
    // the first Only look at every other element to save processing time
