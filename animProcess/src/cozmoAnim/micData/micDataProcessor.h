...
class MicDataProcessor {
 public:
  MicDataProcessor(const Anim::AnimContext* context,
  ...
  BeatDetector& GetBeatDetector() {
    assert(nullptr != _beatDetector);
    return *_beatDetector.get();
  }

  // Create and start stream audio data job
  // Note: Overlap size is only as large as the audio buffer, see
  // kTriggerAudioLengthShipping_ms
  RobotTimeStamp_t CreateStreamJob(
      CloudMic::StreamType streamType = CloudMic::StreamType::Normal,
      uint32_t overlapLength_ms = 0);

  void VoiceTriggerWordDetection(
      const AudioUtil::SpeechRecognizerCallbackInfo& info);

  void FakeTriggerWordDetection(bool fromMute = false);

  void GetLatestMicDirectionData(MicDirectionData& out_lastSample,
 ...
 private:
  ...
  static constexpr uint32_t kRawAudioBufferSize =
      kRawAudioPerBuffer_ms / kTimePerChunk_ms;
  float _rawAudioBufferFullness[2] = {0.f, 0.f};
  // We have 2 fixed buffers for incoming raw audio that we alternate between,
  // so that the processing thread can work on one set of data while the main
  // thread can copy new data into the other set.
  Util::FixedCircularBuffer<RobotInterface::MicData, kRawAudioBufferSize>
      _rawAudioBuffers[2];
...
#if ANKI_DEV_CHEATS
  static constexpr uint32_t kTriggerAudioLength_ms =
      kTriggerAudioLengthDebug_ms;
#else
  static constexpr uint32_t kTriggerAudioLength_ms =
      kTriggerAudioLengthShipping_ms;
#endif

  // Internal buffer used to add to the streaming audio once a trigger is
  // detected
  static constexpr uint32_t kImmediateBufferSize =
      kTriggerAudioLength_ms / kTimePerChunk_ms;
  struct TimedMicData {
    std::array<AudioUtil::AudioSample, kSamplesPerBlockPerChannel> audioBlock;
    RobotTimeStamp_t timestamp;
  };
  Util::FixedCircularBuffer<TimedMicData, kImmediateBufferSize>
      _immediateAudioBuffer;

  using RawAudioChunk =
      std::array<AudioUtil::AudioSample, kIncomingAudioChunkSize>;
  static constexpr uint32_t kImmediateBufferRawSize =
      kTriggerAudioLength_ms / kTimePerChunk_ms;
  ...
  void TriggerWordDetectCallback(
  ...
  MicDirectionData ProcessMicrophonesSE(
      const AudioUtil::AudioSample* audioChunk,
      AudioUtil::AudioSample* bufferOut, uint32_t robotStatus,
      float robotAngle);

  void ProcessRawLoop();
  void ProcessTriggerLoop();

  void UpdateBeatDetector(const AudioUtil::AudioSample* const samples,
