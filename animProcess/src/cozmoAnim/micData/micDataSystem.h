 ...
 * Created: 03/26/2018
 *
 * Description: Handles Updates to mic data processing, streaming collection
 * jobs, and generally acts as messaging/access hub.
 *
...
class MicDataSystem : private Util::SignalHolder,
 ...
 public:
  ...
  MicData::MicDataProcessor* GetMicDataProcessor() const {
    return _micDataProcessor.get();
  }
  SpeechRecognizerSystem* GetSpeechRecognizerSystem() const {
    return _speechRecognizerSystem.get();
  }

  void ProcessMicDataPayload(const RobotInterface::MicData& payload);
  void RecordRawAudio(uint32_t duration_ms, const std::string& path,
                      bool runFFT);
  ...
  void SendMessageToEngine(
      std::unique_ptr<RobotInterface::RobotToEngine> msgPtr);

  void AddMicDataJob(std::shared_ptr<MicDataInfo> newJob,
                     bool isStreamingJob = false);
  ...
  const Anki::Vector::RobotInterface::MicDirection& GetLatestMicDirectionMsg()
      const {
    return _latestMicDirectionMsg;
  }
  ...
  void SetEnableDataCollectionSettings(bool isEnable) {
    _enableDataCollection = isEnable;
  }

  // simulated streaming is when we make everything look like we're streaming
  // normally, but we're not actually sending any data to the cloud; this lasts
  // for a set duration
  bool ShouldSimulateStreaming() const;

  // let's anybody who registered a callback with
  // AddTriggerWordDetectedCallback(...) know that we've heard the trigger word
  // and are either about to start streaming, or not (either on purpose, or it
  // was cancelled/error)
 ...
 private:
  ...
  // if hey vector is spoken, we'll need to abort the alexa pairing screen if
  // it's active. The overly verbose name is becuase we hardcode the "reason"
  // that we are leaving the pairing screen based on the assumption that this is
  // triggered via a "hey vector" wakeword
