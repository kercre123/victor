 ...
 *
 * Description:
 *   1) Receives messages from engine that should be applied to the animation
 *streamer at a specific timestep in animation playback 2) Monitors the
 *animation streamer's playback time and applies messages appropriately
 *
...
StreamingAnimationModifier::StreamingAnimationModifier(
    AnimationStreamer* streamer, Audio::EngineRobotAudioInput* audioInput,
    TextToSpeechComponent* ttsComponent) {
  auto newAnimationCallback = [this]() { _streamTimeToMessageMap.clear(); };
...
void StreamingAnimationModifier::ApplyMessagesHelper(
    AnimationStreamer* streamer, TimeStamp_t streamTime_ms) {
...
void StreamingAnimationModifier::ApplyAlterationsBeforeUpdate(
    AnimationStreamer* streamer) {
  const TimeStamp_t streamTime_ms = streamer->GetRelativeStreamTime_ms();
  ApplyMessagesHelper(streamer, streamTime_ms);
}

void StreamingAnimationModifier::ApplyAlterationsAfterUpdate(
    AnimationStreamer* streamer) {
  const TimeStamp_t streamTime_ms = streamer->GetRelativeStreamTime_ms();
...
void StreamingAnimationModifier::HandleMessage(
  ...
  if ((relativeStreamTime_ms % ANIM_TIME_STEP_MS) != 0) {
    ...
    PRINT_NAMED_WARNING(
        "StreamingAnimationModifier.DelayPending.InvalidDelay",
  ...
  // If this message should be applied at the end of the tick increase its time
  // by 1 This keeps lookups efficient without creating a new map to track this
  // data
  ...
  switch (static_cast<RobotInterface::EngineToRobotTag>(msg.internalTag)) {
    case RobotInterface::EngineToRobotTag::setFullAnimTrackLockState: {
      RobotInterface::EngineToRobot alterationMessage(
          std::move(msg.setFullAnimTrackLockState));
    ...
    case RobotInterface::EngineToRobotTag::postAudioEvent: {
      if (ANKI_DEV_CHEATS) {
        ANKI_VERIFY(
            msg.postAudioEvent.callbackId == 0,
            "StreamingAnimationModifier.HandleMessage.InvalidCallbackID",
            "Callbacks are not currently supported for altering the streaming "
            "animation");
        ANKI_VERIFY(msg.postAudioEvent.gameObject ==
                        Anki::AudioMetaData::GameObjectType::Animation,
                    "StreamingAnimationModifier.HandleMessage.PostAudioEvent."
                    "ImproperGameObject",
                    "All game objects sent through alter streaming animation "
                    "must have object type Animation");
      }

      RobotInterface::EngineToRobot alterationMessage(
          std::move(msg.postAudioEvent));
      AddToMapStreamMap(relativeStreamTime_ms, std::move(alterationMessage));
      break;
    }
    case RobotInterface::EngineToRobotTag::textToSpeechPlay: {
      RobotInterface::EngineToRobot alterationMessage(
          std::move(msg.textToSpeechPlay));
      AddToMapStreamMap(relativeStreamTime_ms, std::move(alterationMessage));
      break;
    }
    ...
    default: {
      PRINT_NAMED_ERROR(
          ...
          "Message Type %d is not currently implemented - update clad and and "
          "anim process to support",
          msg.internalTag);
      break;
    }
  }
...
void StreamingAnimationModifier::ApplyMessageToStreamer(
    AnimationStreamer* streamer, const RobotInterface::EngineToRobot& msg) {
  switch (msg.tag) {
    case (
        uint32_t)RobotInterface::EngineToRobotTag::setFullAnimTrackLockState: {
    ...
    default: {
      PRINT_NAMED_ERROR(
          "StreamingAnimationModifier.ApplyMessageToStreamer.NoImplementation",
          "Attempted to apply message tag of type %d to streamer, but no "
...
void StreamingAnimationModifier::AddToMapStreamMap(
    TimeStamp_t relativeStreamTime_ms, RobotInterface::EngineToRobot&& msg) {
