...
// (seems more related to the other ProceduralFace console vars, so putting it
// in that group instead)
CONSOLE_VAR_ENUM(int, kProcFace_Display, CONSOLE_GROUP, 0,
                 "Normal,Test,FullWhite,Override individually,Override "
                 "together");  // Override procedural face with ConsoleVars
                               // edited version
#if PROCEDURALFACE_NOISE_FEATURE
CONSOLE_VAR_EXTERN(s32, kProcFace_NoiseNumFrames);
#endif
CONSOLE_VAR_ENUM(int, kProcFace_GammaType, CONSOLE_GROUP, 0,
                 "None,FromLinear,ToLinear,AddGamma,RemoveGamma,Custom");
...
static ProceduralFace
    s_faceDataOverride;  // incoming values from console var system
static ProceduralFace
    s_faceDataBaseline;  // baseline to compare against, differences mean
                         // override the incoming animation
static bool s_faceDataReset = false;

uint16_t AnimationStreamer::_numLayersRendered = 0;

#if ANKI_DEV_CHEATS
static const Anim::AnimContext*
    s_context;  // copy of AnimContext in first constructed AnimationStreamer,
                // needed for GetDataPlatform
static bool s_faceDataOverrideRegistered = false;
static uint8_t s_gammaLUT[3][256];  // RGB x 256 entries

void ResetFace(ConsoleFunctionContextRef context) { s_faceDataReset = true; }

CONSOLE_FUNC(ResetFace, CONSOLE_GROUP);

static void LoadFaceGammaLUT(ConsoleFunctionContextRef context) {
  const std::string filename =
      ConsoleArg_GetOptional_String(context, "filename", "screenshot.tga");

  const Util::Data::DataPlatform* dataPlatform = s_context->GetDataPlatform();
  const std::string cacheFilename =
      dataPlatform->pathToResource(Util::Data::Scope::Cache, filename);
  ...
  if (result == RESULT_OK) {
    ...
    if (width != 256 || height != 1) {
      const std::string html = std::string("<html>\n") + filename +
                               " must be either a 256x1 image file\n" +
                               "</html>\n";
      context->channel->WriteLog("%s", html.c_str());
    } else {
      for (int channel = 0; channel < 3; ++channel) {
        // greyscale: offset = 0 always, RGB and RGBA: offset is channel, A is
        // ignored
    ...
    // see https://ankiinc.atlassian.net/browse/VIC-1646 to productize .tga
    // loading
    std::vector<uint8_t> tga =
        Anki::Util::FileUtils::ReadFileAsBinary(cacheFilename);
    if (tga.size() < 18) {
      const std::string html = std::string("<html>\n") + filename +
                               " is not a .tga file\n" + "</html>\n";
      context->channel->WriteLog("%s", html.c_str());
      ...
      if (tga[2] != 2 && tga[2] != 3) {
        const std::string html =
            std::string("<html>\n") + filename +
            " is not an uncompressed, true-color or grayscale .tga file\n" +
            "</html>\n";
        context->channel->WriteLog("%s", html.c_str());
      } else if (width != 256 || height != 1) {
        const std::string html = std::string("<html>\n") + filename +
                                 " must be a 256x1 .tga file\n" + "</html>\n";
        context->channel->WriteLog("%s", html.c_str());
      } else {
        for (int channel = 0; channel < 3; ++channel) {
          // greyscale: offset = 0 always, RGB and RGBA: offset is channel, A is
...
static const u32 kMinTimeBetweenLastNonProcFaceAndNextProcFace_ms =
    2 * ANIM_TIME_STEP_MS;

// Default time to wait before forcing KeepFaceAlive() after the latest stream
// has stopped
const f32 kDefaultLongEnoughSinceLastStreamTimeout_s = 0.5f;

bool kIsInManualUpdateMode = false;
u32 kCurrentManualFrameNumber = 0;
CONSOLE_VAR(bool, kShouldDisplayKeyframeNumber, "ManualAnimationPlayback",
            false);
...
//             reasons and sporadic idle throttling is not worth alerting the
//             dev about.
...
//////////
/// Manual Playback Console Vars - allow user to play back/hold single frames
/// within an animation
/////////
...
std::string DevGetFaceImagFolder() {
  ...
  auto folder =
      platform->pathToResource(Util::Data::Scope::Cache, faceImgsFolder);
...
void CaptureFaceImage(ConsoleFunctionContextRef context) {
  if (sDevBufferFacePtr != nullptr) {
    ...
    const std::string filename =
        (folder + animName + "_" + std::to_string(kCurrentManualFrameNumber) +
         ".png");
...
// Allows easy disabling of KeepFaceAlive using the console system (i.e.,
// without a message interface) This is useful for the animators to disable
// KeepFaceAlive while testing eye shapes, etc.
...
static void CaptureFace(ConsoleFunctionContextRef context) {
  ...
  if (s_framesToCapture == 0) {
    ...
    const Util::Data::DataPlatform* dataPlatform = s_context->GetDataPlatform();
    const std::string cacheFilename =
        dataPlatform->pathToResource(Util::Data::Scope::Cache, s_frameFilename);

    if (s_frameFilename.find(".gif") != std::string::npos) {
      s_gifVersion = 1;
      s_gif1 = jo_gif_start(cacheFilename.c_str(), FACE_DISPLAY_WIDTH,
                            FACE_DISPLAY_HEIGHT, 0, 256);
      ...
      s_gifVersion = 2;
      if (GifBegin(&s_gif2, cacheFilename.c_str(), FACE_DISPLAY_WIDTH,
                   FACE_DISPLAY_HEIGHT, 0, 8)) {
        s_framesToCapture = numFrames;
      }
    ...
    if (s_framesToCapture > 0) {
      ...
      html = std::string("<html>\nCapturing frames as <a href=\"/cache/") +
             s_frameFilename + "\">" + s_frameFilename + "\n" + "</html>\n";
    } else {
      html =
          std::string("<html>\nError: unable to open file <a href=\"/cache/") +
          s_frameFilename + "\">" + s_frameFilename + "\n" + "</html>\n";
    }
  } else {
    html = std::string("Capture already in progress as <a href=\"/cache/") +
           s_frameFilename + "\">" + s_frameFilename + "\n" + "</html>\n";
  }
...
CONSOLE_FUNC(CaptureFace, "Face", optional const char* filename,
             optional int numFrames);
#endif  // ANKI_DEV_CHEATS
...
AnimationStreamer::AnimationStreamer(const Anim::AnimContext* context)
    : _context(context),
      ...
      _tracksInUse(0),
      _animAudioClient(
          new Audio::AnimationAudioClient(context->GetAudioController())),
      _proceduralAudioClient(
          new Audio::ProceduralAudioClient(context->GetAudioController())),
      _longEnoughSinceLastStreamTimeout_s(
          kDefaultLongEnoughSinceLastStreamTimeout_s),
...
Result AnimationStreamer::Init(TextToSpeechComponent* ttsComponent) {
  ...
  DEV_ASSERT(nullptr != _context->GetDataLoader(),
             "AnimationStreamer.Init.NullRobotDataLoader");
  const std::string neutralFaceAnimName = "anim_neutral_eyes_01";
  _neutralFaceAnimation =
      _context->GetDataLoader()->GetCannedAnimation(neutralFaceAnimName);
  if (nullptr != _neutralFaceAnimation) {
    auto frame = _neutralFaceAnimation->GetTrack<ProceduralFaceKeyFrame>()
                     .GetFirstKeyFrame();
  ...
  // Do this after the ProceduralFace class has set to use the right neutral
  // face
  ...
  // Start with a blank face (face scale == 0) until the engine has initialized
  // and sent an animation
  {
    ProceduralFace blankFace;
    const f32 zeroScale = 0.0f;
    const std::vector<f32> arbitraryEyes(
        (int)ProceduralEyeParameter::NumParameters, 0.5f);
    blankFace.SetFromValues(arbitraryEyes, arbitraryEyes, 0.0f, 0.0f, 0.0f,
                            zeroScale, zeroScale, 0.0f);
...
Result AnimationStreamer::SetStreamingAnimation(
    const std::string& name, Tag tag, u32 numLoops, u32 startAt_ms,
  ...
  return SetStreamingAnimation(anim, tag, numLoops, startAt_ms,
                               interruptRunning, overrideAllSpritesToEyeHue,
                               false);
}

void AnimationStreamer::SetPendingStreamingAnimation(const std::string& name,
                                                     u32 numLoops) {
...
Result AnimationStreamer::SetStreamingAnimation(
    Animation* anim, Tag tag, u32 numLoops, u32 startAt_ms,
    bool interruptRunning, bool overrideAllSpritesToEyeHue, bool isInternalAnim,
    bool shouldClearProceduralAnim) {
  if (DEBUG_ANIMATION_STREAMING) {
    LOG_DEBUG(
  ...
  if (wasStreamingSomething) {
    if (nullptr != anim && !interruptRunning) {
      LOG_INFO("AnimationStreamer.SetStreamingAnimation.NotInterrupting",
               ...
               _streamingAnimation->GetName().c_str(), anim->GetName().c_str());
      return RESULT_FAIL;
    }
...
Result AnimationStreamer::SetProceduralFace(const ProceduralFace& face,
                                            u32 duration_ms) {
  DEV_ASSERT(nullptr != _proceduralAnimation,
             "AnimationStreamer.SetProceduralFace.NullProceduralAnimation");

  // Always add one keyframe
  ProceduralFaceKeyFrame keyframe(face);
  Result result = _proceduralAnimation->AddKeyFrameToBack(keyframe);

  // Add a second one later to interpolate to, if duration is longer than one
  // keyframe
  ...
  if (!(ANKI_VERIFY(RESULT_OK == result,
                    "AnimationStreamer.SetProceduralFace.FailedToCreateAnim",
                    ""))) {
    return result;
  }

  // ProceduralFace is always played as an "internal" animation since it's not
  // considered a regular animation by the engine so we don't need to send
  // AnimStarted and AnimEnded messages for it.
  result = SetStreamingAnimation(_proceduralAnimation, 0);

  return result;
}

void AnimationStreamer::SendAnimationMessages(
...
#define DEBUG_STREAM_KEYFRAME_MESSAGE(__KF_NAME__)
  ...
  if (SendIfTrackUnlocked(stateToSend.moveHeadMessage,
                          AnimTrackFlag::HEAD_TRACK)) {
    DEBUG_STREAM_KEYFRAME_MESSAGE("HeadAngle");
  }

  if (SendIfTrackUnlocked(stateToSend.moveLiftMessage,
                          AnimTrackFlag::LIFT_TRACK)) {
    DEBUG_STREAM_KEYFRAME_MESSAGE("LiftHeight");
  }

  if (SendIfTrackUnlocked(stateToSend.bodyMotionMessage,
                          AnimTrackFlag::BODY_TRACK)) {
    DEBUG_STREAM_KEYFRAME_MESSAGE("BodyMotion");
  }

  if (SendIfTrackUnlocked(stateToSend.recHeadMessage,
                          AnimTrackFlag::BODY_TRACK)) {
    DEBUG_STREAM_KEYFRAME_MESSAGE("RecordHeading");
  }

  if (SendIfTrackUnlocked(stateToSend.turnToRecHeadMessage,
                          AnimTrackFlag::BODY_TRACK)) {
    DEBUG_STREAM_KEYFRAME_MESSAGE("TurnToRecordedHeading");
  }

  if (SendIfTrackUnlocked(stateToSend.backpackLightsMessage,
                          AnimTrackFlag::BACKPACK_LIGHTS_TRACK)) {
    EnableBackpackAnimationLayer(true);
  }

  if (stateToSend.audioKeyFrameMessage != nullptr) {
    _animAudioClient->PlayAudioKeyFrame(*stateToSend.audioKeyFrameMessage,
                                        _context->GetRandom());
...
void AnimationStreamer::Process_displayFaceImageChunk(
    const RobotInterface::DisplayFaceImageBinaryChunk& msg) {
  // Since binary images and grayscale images both use the same underlying
  // image, ensure that only one type is being sent at a time
  DEV_ASSERT(_faceImageGrayscaleChunksReceivedBitMask == 0,
             "AnimationStreamer.Process_displayFaceImageChunk."
             "AlreadyReceivingGrayscaleImage");

  // Expand the bit-packed msg.faceData (every bit == 1 pixel) to byte array
  // (every byte == 1 pixel)
  static const u32 kExpectedNumPixels = FACE_DISPLAY_NUM_PIXELS / 2;
  static const u32 kDataLength = sizeof(msg.faceData);
  static_assert(8 * kDataLength == kExpectedNumPixels,
                "Mismatched face image and bit image sizes");

  if (msg.imageId != _faceImageId) {
    if (_faceImageChunksReceivedBitMask != 0) {
      LOG_WARNING(
          "AnimationStreamer.Process_displayFaceImageChunk.UnfinishedFace",
          "Overwriting ID %d with ID %d", _faceImageId, msg.imageId);
    }
...
void AnimationStreamer::Process_displayFaceImageChunk(
    const RobotInterface::DisplayFaceImageGrayscaleChunk& msg) {
  // Since binary images and grayscale images both use the same underlying
  // image, ensure that only one type is being sent at a time
  DEV_ASSERT(_faceImageChunksReceivedBitMask == 0,
             "AnimationStreamer.Process_displayFaceImageChunk."
             "AlreadyReceivingBinaryImage");

  if (msg.imageId != _faceImageGrayscaleId) {
    if (_faceImageGrayscaleChunksReceivedBitMask != 0) {
      LOG_WARNING(
          "AnimationStreamer.Process_displayFaceImageGrayscaleChunk."
          "UnfinishedFace",
          "Overwriting ID %d with ID %d", _faceImageGrayscaleId, msg.imageId);
    }
  ...
  static const u16 kMaxNumPixelsPerChunk =
      sizeof(msg.faceData) / sizeof(msg.faceData[0]);
  const auto numPixels = std::min(msg.numPixels, kMaxNumPixelsPerChunk);
  uint8_t* imageData_i = _faceImageGrayscale.GetDataPointer();
  std::copy_n(msg.faceData, numPixels,
              imageData_i + (msg.chunkIndex * kMaxNumPixelsPerChunk));

  if (_faceImageGrayscaleChunksReceivedBitMask ==
      kAllFaceImageGrayscaleChunksReceivedMask) {
    auto* img = new Vision::ImageRGBA(FACE_DISPLAY_HEIGHT, FACE_DISPLAY_WIDTH);
    img->SetFromGray(_faceImageGrayscale);
    auto handle = std::make_shared<Vision::SpriteWrapper>(img);
...
void AnimationStreamer::Process_displayFaceImageChunk(
    const RobotInterface::DisplayFaceImageRGBChunk& msg) {
  if (msg.imageId != _faceImageRGBId) {
    if (_faceImageRGBChunksReceivedBitMask != 0) {
      LOG_WARNING(
          "AnimationStreamer.Process_displayFaceImageRGBChunk.UnfinishedFace",
          "Overwriting ID %d with ID %d", _faceImageRGBId, msg.imageId);
    }
  ...
  const auto numPixels = std::min(msg.numPixels, kMaxNumPixelsPerChunk);
  std::copy_n(msg.faceData, numPixels,
              _faceImageRGB565.GetRawDataPointer() +
                  (msg.chunkIndex * kMaxNumPixelsPerChunk));

  if (_faceImageRGBChunksReceivedBitMask ==
      kAllFaceImageRGBChunksReceivedMask) {
    auto* img = new Vision::ImageRGBA(FACE_DISPLAY_HEIGHT, FACE_DISPLAY_WIDTH);
...
void AnimationStreamer::Process_playAnimWithSpriteBoxRemaps(
    const RobotInterface::PlayAnimWithSpriteBoxRemaps& msg) {
  ...
  // CopyIntoProceduralAnimation call will delete *_streamingAnimation without
  // assigning it to nullptr. This assignment prevents associated undefined
  // behavior
  _streamingAnimation = _neutralFaceAnimation;
  const std::string animName(msg.animName, msg.animName_length);
  CopyIntoProceduralAnimation(
      _context->GetDataLoader()->GetCannedAnimation(animName));
  for (int i = 0; i < msg.numRemaps; ++i) {
    _proceduralAnimation->AddSpriteBoxRemap(
        msg.spriteBoxRemaps[i].spriteBoxName,
        msg.spriteBoxRemaps[i].remappedAssetID);
  }
  SetStreamingAnimation(_proceduralAnimation, msg.tag, numLoops, startAtTime_ms,
                        interruptRunning, overrideAllSpritesToEyeHue,
                        isInternalAnim);
...
void AnimationStreamer::Process_addSpriteBoxKeyFrames(
    const RobotInterface::AddSpriteBoxKeyFrames& msg) {
  if (_streamingAnimation != _proceduralAnimation) {
    LOG_ERROR("AnimationStreamer.AddSpriteBoxKeyFrames.NoAnimation",
              "Attempted to add keyframes to running anim, but no "
              "_proceduralAnim was running");
    return;
  }
...
Result AnimationStreamer::SetFaceImage(Vision::SpriteHandle spriteHandle,
  ...
  DEV_ASSERT(nullptr != _proceduralAnimation,
             "AnimationStreamer.SetFaceImage.NullProceduralAnimation");

  _proceduralAnimation->SetFaceImageOverride(
      spriteHandle, _relativeStreamTime_ms, duration_ms);
  if (_streamingAnimation != _proceduralAnimation) {
    return SetStreamingAnimation(_proceduralAnimation, 0, 0, 0, true,
...
void AnimationStreamer::Abort(Tag tag, bool shouldClearProceduralAnim) {
  ...
  if (nullptr != _streamingAnimation &&
    ...
    LOG_INFO("AnimationStreamer.Abort",
             "Tag=%d %s hasFramesLeft=%d startSent=%d endSent=%d", _tag,
             _streamingAnimation->GetName().c_str(),
             _streamingAnimation->HasFramesLeft(), _startOfAnimationSent,
             _endOfAnimationSent);
    ...
    // If we get to KeepFaceAlive with this flag set, we'll stream neutral face
    // for safety.
    _wasAnimationInterruptedWithNothing = true;
  }
...
Result AnimationStreamer::InitStreamingAnimation(
    Tag withTag, u32 startAt_ms, bool overrideAllSpritesToEyeHue) {
  ...
  if (lastResult == RESULT_OK) {
    ...
    // If we are initializing ANY animation at all, we don't want keepalive's
    // mucking with the eye display state. If we eventually decide we want to
    // have an animation screen run with keepalive eyes, this will need to be
    // addressed across the entire keepalive system
    _proceduralTrackComponent->RemoveKeepFaceAlive(_relativeStreamTime_ms,
                                                   (3 * ANIM_TIME_STEP_MS));

    if (!s_enableKeepFaceAlive) {
      // If the animation doesn't have a procedural face track, and Face
      // Keep-alive is false (i.e., the "last procedural face should persist")
      // then the last procedural face should be blank. Set the last procedural
      // face to blank
...
bool AnimationStreamer::SendIfTrackUnlocked(RobotInterface::EngineToRobot*& msg,
                                            AnimTrackFlag track) {
  bool res = false;
  if (msg != nullptr) {
    if (!IsTrackLocked(_lockedTracks, (u8)track)) {
      switch (track) {
        ...
        case AnimTrackFlag::BODY_TRACK:
          ...
          break;
        default:
          // Audio, face, and event frames are handled separately since they
          // don't actually result in a EngineToRobot message
          LOG_WARNING("AnimationStreamer.SendIfTrackUnlocked.InvalidTrack",
                      "%s", EnumToString(track));
          break;
      }
    }
    Util::SafeDelete(msg);
...
void AnimationStreamer::GetStreamableFace(const Anim::AnimContext* context,
                                          const ProceduralFace& procFace,
                                          Vision::ImageRGB565& outImage) {
  ...
  if (kProcFace_Display == (int)FaceDisplayType::Test) {
    ...
    for (int i = 0; i < FACE_DISPLAY_HEIGHT / 3; ++i) {
      ...
      Vision::PixelRGB565* green_i =
          outImage.GetRow(i + FACE_DISPLAY_HEIGHT / 3);
      Vision::PixelRGB565* blue_i =
          outImage.GetRow(i + 2 * FACE_DISPLAY_HEIGHT / 3);
      for (int j = 0; j < FACE_DISPLAY_WIDTH; ++j) {
        const u8 value = Util::numeric_cast_clamped<u8>(
            std::round((f32)j / (f32)FACE_DISPLAY_WIDTH * 255.f));
    ...
    DEV_ASSERT(context != nullptr,
               "AnimationStreamer.BufferFaceToSend.NoContext");
    DEV_ASSERT(context->GetRandom() != nullptr,
               "AnimationStreamer.BufferFaceToSend.NoRNGinContext");
    ...
    if (s_faceDataReset) {
      ...
      NativeAnkiUtilConsoleResetValueToDefault(
          "ProcFace_EyeLightnessMultiplier");
    ...
    if (kProcFace_Display == (int)FaceDisplayType::OverrideIndividually ||
        kProcFace_Display == (int)FaceDisplayType::OverrideTogether) {
      // compare override face data with baseline, if different update the
      // rendered face
      ...
      if (kProcFace_Display == (int)FaceDisplayType::OverrideTogether) {
        s_faceDataOverride.SetParameters(
            ProceduralFace::WhichEye::Right,
            s_faceDataOverride.GetParameters(ProceduralFace::WhichEye::Left));
      }
      for (auto whichEye :
           {ProceduralFace::WhichEye::Left, ProceduralFace::WhichEye::Right}) {
        for (std::underlying_type<ProceduralFace::Parameter>::type iParam = 0;
             iParam <
             Util::EnumToUnderlying(ProceduralFace::Parameter::NumParameters);
             ++iParam) {
          if (s_faceDataOverride.GetParameter(
                  whichEye, (ProceduralFace::Parameter)iParam) !=
              s_faceDataBaseline.GetParameter(
                  whichEye, (ProceduralFace::Parameter)iParam)) {
            newProcFace.SetParameter(
                whichEye, (ProceduralFace::Parameter)iParam,
                s_faceDataOverride.GetParameter(
                    whichEye, (ProceduralFace::Parameter)iParam));
          }
        }
      }
      ...
      if (s_faceDataOverride.GetFaceAngle() !=
          s_faceDataBaseline.GetFaceAngle()) {
        newProcFace.SetFaceAngle(s_faceDataOverride.GetFaceAngle());
      }
      if (s_faceDataOverride.GetFaceScale()[0] !=
              s_faceDataBaseline.GetFaceScale()[0] ||
          s_faceDataOverride.GetFaceScale()[1] !=
              s_faceDataBaseline.GetFaceScale()[1]) {
        newProcFace.SetFaceScale(s_faceDataOverride.GetFaceScale());
      }
      if (s_faceDataOverride.GetFacePosition()[0] !=
              s_faceDataBaseline.GetFacePosition()[0] ||
          s_faceDataOverride.GetFacePosition()[1] !=
              s_faceDataBaseline.GetFacePosition()[1]) {
        newProcFace.SetFacePosition(s_faceDataOverride.GetFacePosition());
      }
      if (s_faceDataOverride.GetScanlineOpacity() !=
          s_faceDataBaseline.GetScanlineOpacity()) {
        newProcFace.SetScanlineOpacity(s_faceDataOverride.GetScanlineOpacity());
      }

      ProceduralFaceDrawer::DrawFace(newProcFace, *context->GetRandom(),
                                     outImage);
    } else {
...
void AnimationStreamer::UpdateCaptureFace(
    const Vision::ImageRGB565& faceImg565) {
  if (s_framesToCapture > 0) {
    clock_t end = clock();
    ...
    if (s_tga != NULL) {
      fwrite(frame.GetDataPointer(), sizeof(uint8_t),
             FACE_DISPLAY_WIDTH * FACE_DISPLAY_HEIGHT * 4, s_tga);
    } else {
      if (s_gifVersion == 1) {
        jo_gif_frame(&s_gif1, (uint8_t*)frame.GetDataPointer(), 4, false);
      } else {
        GifWriteFrame(&s_gif2, (uint8_t*)frame.GetDataPointer(),
                      FACE_DISPLAY_WIDTH, FACE_DISPLAY_HEIGHT, elapsed * 100);
      }
    }
...
void AnimationStreamer::BufferFaceToSend(Vision::ImageRGB565& faceImg565) {
  DEV_ASSERT_MSG(faceImg565.GetNumCols() == FACE_DISPLAY_WIDTH &&
                 ...
                 "Got %d x %d. Expected %d x %d", faceImg565.GetNumCols(),
                 faceImg565.GetNumRows(), FACE_DISPLAY_WIDTH,
                 FACE_DISPLAY_HEIGHT);
  ...
  if ((kProcFace_GammaType != kProcFace_GammaType_old) ||
      (kProcFace_Gamma_old != kProcFace_Gamma)) {
    switch (kProcFace_GammaType) {
      case (int)FaceGammaType::FromLinear:
        for (int i = 0; i < 256; i++) {
          s_gammaLUT[0][i] = s_gammaLUT[1][i] = s_gammaLUT[2][i] =
              cv::saturate_cast<uchar>(fromLinear(i / 255.f) * 255.0f);
        }
        break;
      case (int)FaceGammaType::ToLinear:
        for (int i = 0; i < 256; i++) {
          s_gammaLUT[0][i] = s_gammaLUT[1][i] = s_gammaLUT[2][i] =
              cv::saturate_cast<uchar>(toLinear(i / 255.f) * 255.0f);
        }
        break;
        ...
        for (int i = 0; i < 256; i++) {
          s_gammaLUT[0][i] = s_gammaLUT[1][i] = s_gammaLUT[2][i] =
              cv::saturate_cast<uchar>(
                  powf((float)(i / 255.f), 1.f / kProcFace_Gamma) * 255.0f);
        }
        break;
      case (int)FaceGammaType::RemoveGamma:
        for (int i = 0; i < 256; i++) {
          s_gammaLUT[0][i] = s_gammaLUT[1][i] = s_gammaLUT[2][i] =
              cv::saturate_cast<uchar>(
                  powf((float)(i / 255.f), kProcFace_Gamma) * 255.0f);
        }
        break;
    }
  ...
  if (kProcFace_GammaType != (int)FaceGammaType::None) {
    ...
    for (int i = 0; i < nrows; ++i) {
      ...
      for (int j = 0; j < ncols; ++j) {
        img_i[j].SetValue(Vision::PixelRGB565(s_gammaLUT[0][img_i[j].r()],
                                              s_gammaLUT[1][img_i[j].g()],
                                              s_gammaLUT[2][img_i[j].b()])
                              .GetValue());
      }
    }
  }
  ...
  if (kDisplayMemoryPressure) {
    ...
    if (info.alert > OSState::Alert::None) {
      const ColorRGBA& memAlertColor =
          (info.alert >= OSState::Alert::Red ? NamedColors::RED
                                             : NamedColors::YELLOW);
  ...
  if (SHOULD_SEND_DISPLAYED_FACE_TO_ENGINE) {
    ...
    ASSERT_NAMED(faceImg565.IsContinuous(),
                 "AnimationComponent.DisplayFaceImage.NotContinuous");
    ...
    static const int kExpectedNumChunks = static_cast<int>(
        std::ceilf((f32)FACE_DISPLAY_NUM_PIXELS / kMaxPixelsPerMsg));
    DEV_ASSERT_MSG(chunkCount == kExpectedNumChunks,
                   "AnimationComponent.DisplayFaceImage.UnexpectedNumChunks",
                   "%d", chunkCount);
  }
  ...
  if (kShouldDisplayPlaybackTime) {
    ...
    // Estimate if animation process is running slowly and display this on the
    // screen
    const AnimTimeStamp_t estimatedRealTime =
        BaseStationTimer::getInstance()->GetCurrentTimeStamp() - _startTime_ms;
    ...
    if (timeDrift > (2 * ANIM_TIME_STEP_MS)) {
      ...
      playbackTime += ("/" + std::to_string((TimeStamp_t)realSecs) + ":" +
                       std::to_string((TimeStamp_t)realMS));
    }
...
Result AnimationStreamer::SendStartOfAnimation() {
  DEV_ASSERT(!_startOfAnimationSent,
             "AnimationStreamer.SendStartOfAnimation.AlreadySent");
  DEV_ASSERT(_streamingAnimation != nullptr,
             "AnimationStreamer.SendStartOfAnimation.NullAnim");
  const std::string& streamingAnimName = _streamingAnimation->GetName();

  if (DEBUG_ANIMATION_STREAMING) {
    LOG_DEBUG("AnimationStreamer.SendStartOfAnimation",
              "Tag=%d, Name=%s, loopCtr=%d", _tag, streamingAnimName.c_str(),
              _loopCtr);
  }

  if (_loopCtr == 0) {
    // Don't actually send start message for proceduralFace or neutralFace anims
    // since they weren't requested by engine
    if (!_playingInternalAnim) {
      AnimationStarted startMsg;
      memcpy(startMsg.animName, streamingAnimName.c_str(),
             streamingAnimName.length());
...
//       Need to call this after triggerTime+durationTime of last keyframe has
//       expired.
Result AnimationStreamer::SendEndOfAnimation(bool abortingAnim) {
  DEV_ASSERT(
      _startOfAnimationSent && !_endOfAnimationSent,
      "AnimationStreamer.SendEndOfAnimation.StartNotSentOrEndAlreadySent");
  DEV_ASSERT(_streamingAnimation != nullptr,
             "AnimationStreamer.SendStartOfAnimation.NullAnim");
  const std::string& streamingAnimName = _streamingAnimation->GetName();

  if (DEBUG_ANIMATION_STREAMING) {
    LOG_INFO("AnimationStreamer.SendEndOfAnimation",
             "Tag=%d, Name=%s, t=%dms, loopCtr=%d, numLoops=%d", _tag,
             streamingAnimName.c_str(), _relativeStreamTime_ms, _loopCtr,
             _numLoops);
  }

  if (abortingAnim || (_loopCtr == _numLoops - 1)) {
    // Don't actually send end message for proceduralFace or neutralFace anims
    // since they weren't requested by engine
    if (!_playingInternalAnim) {
      AnimationEnded endMsg;
      memcpy(endMsg.animName, streamingAnimName.c_str(),
             streamingAnimName.length());
  ...
  // Every time we end an animation we should also re-enable BPL_USER layer on
  // robot
  EnableBackpackAnimationLayer(false);
...
// This side-loop of rendering logic is a workaround used only if there is no
// running animation. It should be possible to remove this given a more linear,
// single-image rendering pipeline.
Result AnimationStreamer::ExtractMessagesFromProceduralTracks(
    AnimationMessageWrapper& stateToSend) {
  ...
  if (_proceduralTrackComponent->HaveLayersToSend()) {
    ...
    const bool isFaceTrackAlreadyLocked =
        IsTrackLocked(lockedTracks, (u8)AnimTrackFlag::FACE_TRACK);
    const bool isTimeForProceduralFace =
        BaseStationTimer::getInstance()->GetCurrentTimeStamp() >=
        _nextProceduralFaceAllowedTime_ms;
    if (!isFaceTrackAlreadyLocked && !isTimeForProceduralFace) {
      lockedTracks |= (u8)AnimTrackFlag::FACE_TRACK;
    }

    ExtractMessagesRelatedToProceduralTrackComponent(
        _context, nullptr, _proceduralTrackComponent.get(), lockedTracks,
        _relativeStreamTime_ms, false, stateToSend);
  }

  return lastResult;
}  // ExtractMessagesFromProceduralTracks()

Result AnimationStreamer::ExtractMessagesFromStreamingAnim(
    AnimationMessageWrapper& stateToSend) {
  ...
  if (!_streamingAnimation->IsInitialized()) {
    LOG_ERROR(
        "Animation.Update",
        "%s: Animation must be initialized before it can be played/updated.",
        _streamingAnimation != nullptr ? _streamingAnimation->GetName().c_str()
                                       : "<NULL>");
    return RESULT_FAIL;
  }
  ...
  // Tracks which have no procedural alterations - grab any messages directly
  {
    ...
    auto& recordHeadingTrack =
        _streamingAnimation->GetTrack<RecordHeadingKeyFrame>();
    auto& turnToRecordedHeadingTrack =
        _streamingAnimation->GetTrack<TurnToRecordedHeadingKeyFrame>();

    stateToSend.moveHeadMessage =
        headTrack.GetCurrentStreamingMessage(_relativeStreamTime_ms);
    stateToSend.moveLiftMessage =
        liftTrack.GetCurrentStreamingMessage(_relativeStreamTime_ms);
    stateToSend.bodyMotionMessage =
        bodyTrack.GetCurrentStreamingMessage(_relativeStreamTime_ms);
    stateToSend.recHeadMessage =
        recordHeadingTrack.GetCurrentStreamingMessage(_relativeStreamTime_ms);
    stateToSend.turnToRecHeadMessage =
        turnToRecordedHeadingTrack.GetCurrentStreamingMessage(
            _relativeStreamTime_ms);
    ...
    if (eventTrack.HasFramesLeft() &&
      ...
      const AnimTimeStamp_t currTime_ms =
          BaseStationTimer::getInstance()->GetCurrentTimeStamp();
  ...
  // TODO(str): VIC-13519 Linearize Face Rendering
  // The remainder of this logic is not
  // "ExtractingMessagesFromStreamingAnimation"... this is confusing. This logic
  // could/should be moved up to Update so that building the desired animation
  // is a visibly linear process instead of burying procedural content under
  // canned animation oriented function calls. Apply any track layers to the
  // animation
  static const bool kStoreFace = true;
  ExtractMessagesRelatedToProceduralTrackComponent(
      _context, _streamingAnimation, _proceduralTrackComponent.get(),
      _lockedTracks, _relativeStreamTime_ms, kStoreFace, stateToSend);

  return lastResult;
}  // ExtractMessagesFromStreamingAnim()

Result AnimationStreamer::ExtractMessagesRelatedToProceduralTrackComponent(
    const Anim::AnimContext* context, Animation* anim,
    TrackLayerComponent* trackComp, const u8 tracksCurrentlyLocked,
    const TimeStamp_t timeSinceAnimStart_ms, const bool storeFace,
    AnimationMessageWrapper& stateToSend) {
  ANKI_CPU_PROFILE(
      "AnimationStreamer::ExtractMessagesRelatedToProceduralTrackComponent");

  TrackLayerComponent::LayeredKeyFrames layeredKeyFrames;
  trackComp->ApplyLayersToAnim(anim, timeSinceAnimStart_ms, layeredKeyFrames,
                               storeFace);

  if (layeredKeyFrames.haveBackpackKeyFrame &&
      !IsTrackLocked(tracksCurrentlyLocked,
                     (u8)AnimTrackFlag::BACKPACK_LIGHTS_TRACK)) {
    stateToSend.backpackLightsMessage =
        layeredKeyFrames.backpackKeyFrame.GetStreamMessage(
  ...
  if (layeredKeyFrames.haveAudioKeyFrame &&
    ...
    stateToSend.audioKeyFrameMessage =
        new RobotAudioKeyFrame(layeredKeyFrames.audioKeyFrame);
  }

  // TODO(str): VIC-13519 Linearize Face Rendering
  // Again... it makes precious little sense to be handling all this face
  // rendering logic at the bottom of a huge, apparently unrelated call stack.
  // Takes forever to discover this stuff
  // ----- Face Rendering Code -----

  bool haveEyesToRender =
  ...
  // when it goes off. There should be a configuration option to set, but for
  // now we do this hack for 1.7.0
  if (anim != nullptr && anim->GetName() == "anim_timersup_beep_01") {
    haveEyesToRender = false;
  }

  if (haveEyesToRender) {
    GetStreamableFace(context, layeredKeyFrames.faceKeyFrame.GetFace(),
                      stateToSend.faceImg);
  ...
  if (anim != nullptr) {
    ...
    bool renderFromCompImage = anim->PopulateCompositeImage(
        *context->GetDataLoader()->GetSpriteCache(),
    ...
    if (renderFromCompImage) {
      if (haveEyesToRender) {
        // TODO(str): VIC-13519 Linearize Face Rendering
        // We should just always be overlaying the procedural face onto the comp
        // image above after rendering the layers below the face layer, then
        // finish rendering the rest of the layers in the image. Right now we
        // waste a lot of optimization drawing the face image then copying it
        // into the CompImage, then pixel-by-pixel blitting it as a full-screen
        // overlay from within the CompImage.
    ...
    if (renderFromCompImage || haveEyesToRender) {
      const AnimTimeStamp_t currTime_ms =
          BaseStationTimer::getInstance()->GetCurrentTimeStamp();
      _nextProceduralFaceAllowedTime_ms =
          currTime_ms + kMinTimeBetweenLastNonProcFaceAndNextProcFace_ms;
    }
  }
...
Result AnimationStreamer::ExtractAnimationMessages(
    AnimationMessageWrapper& stateToSend) {
  Result lastResult = RESULT_OK;
    ...
    if (IsStreamingAnimFinished()) {
      ...
      if (_numLoops == 0 || _loopCtr < _numLoops) {
        if (DEBUG_ANIMATION_STREAMING) {
          LOG_INFO("AnimationStreamer.Update.Looping",
                   ...
                   _loopCtr, _numLoops, _streamingAnimation->GetName().c_str());
        }
        ...
        // To avoid streaming faceLayers set true and start streaming animation
        // next Update() tick.
        streamUpdated = true;
      } else {
        if (DEBUG_ANIMATION_STREAMING) {
      ...
      if (!_streamingAnimation->HasFramesLeft() &&
        ...
        if (_animAudioClient->HasActiveEvents()) {
          LOG_WARNING(
              "AnimationStreamer.ExtractMessagesFromStreamingAnim."
              "EndOfAnimation.ActiveAudioEvent",
...
void AnimationStreamer::SetKeepAliveIfAppropriate() {
  // Always keep face alive, unless we have a streaming animation, since we rely
  // on it to do all face updating and we don't want to step on it's
  // hand-designed toes. Wait a 1/2 second before running after we finish the
  // last streaming animation to help reduce stepping on the next animation's
  // toes when we have things sequenced. NOTE: lastStreamTime>0 check so that we
  // don't start keeping face alive before
  ...
  const bool longEnoughSinceStream =
      (BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() -
       _lastAnimationStreamTime) > _longEnoughSinceLastStreamTimeout_s;
  if (!haveStreamingAnimation && haveStreamedAnything &&
      longEnoughSinceStream) {
    if (!FACTORY_TEST) {
      if (s_enableKeepFaceAlive) {
        // If we were interrupted from streaming an animation and we've met all
        // the conditions to even be in this function, then we should make sure
        // we've got neutral face back on the screen
...
Result AnimationStreamer::Update() {
...

  {
    ...
    if (!_pendingAnimation.empty()) {
      SetStreamingAnimation(_pendingAnimation, /*tag*/ 1, _pendingNumLoops, 0,
  ...
  if (!kIsInManualUpdateMode) {
    // Check to see if we're not streaming anything and a keep alive should take
    // over
    ...
    // TODO(str): VIC-13519 Linearize Face Rendering
    // IMHO Interpolation doesn't belong here. We already have a concept of
    // tracks for specialized KeyFrame handling, the tracks should be
    // responsible for returning appropriate content given a relativeStreamTime
    // and interpolate internally. AnimationStreamer should not be concerned.
    static const bool kStoreFace = true;
    ExtractMessagesRelatedToProceduralTrackComponent(
        _context, _streamingAnimation, _proceduralTrackComponent.get(),
        _lockedTracks, _relativeStreamTime_ms, kStoreFace, messageWrapper);

    // AnimationInterpolator is unimplemented and does nothing at all in this
    // call.
    AnimationInterpolator::GetInterpolationMessages(
        _streamingAnimation, kCurrentManualFrameNumber, messageWrapper);
  ...
  // A workaround to remove tracks that escaped through the engine process'
  // track locking. This currently happens only for composite weather animations
  // and wake word animations, both of which bypass action system's track
  // locking.
  if (_streamingAnimation != nullptr) {
...
void AnimationStreamer::EnableKeepFaceAlive(bool enable,
                                            u32 disableTimeout_ms) {
  if (s_enableKeepFaceAlive && !enable) {
    ...
    if (_wasAnimationInterruptedWithNothing) {
      // The last animation ended without a replacement, but neutral eyes
      // weren't inserted because keepalive was disabled. Now that they're
      // re-enabled, set the neutral eyes.
      SetStreamingAnimation(_neutralFaceAnimation, kNotAnimatingTag);
      _wasAnimationInterruptedWithNothing = false;
    }
  }
...
bool AnimationStreamer::IsStreamingAnimFinished() const {
  return _endOfAnimationSent && (_streamingAnimation != nullptr) &&
...
void AnimationStreamer::ProcessAddOrUpdateEyeShift(
    const RobotInterface::AddOrUpdateEyeShift& msg) {
  const std::string layerName(msg.name, msg.name_length);
  _proceduralTrackComponent->AddOrUpdateEyeShift(
      layerName, msg.xPix, msg.yPix, msg.duration_ms, _relativeStreamTime_ms,
      msg.xMax, msg.yMax, msg.lookUpMaxScale, msg.lookDownMinScale,
      msg.outerEyeScaleIncrease);
}

void AnimationStreamer::ProcessRemoveEyeShift(
    const RobotInterface::RemoveEyeShift& msg) {
  const std::string layerName(msg.name, msg.name_length);
  _proceduralTrackComponent->RemoveEyeShift(layerName, _relativeStreamTime_ms,
                                            msg.disableTimeout_ms);
}

void AnimationStreamer::ProcessAddSquint(const RobotInterface::AddSquint& msg) {
  const std::string layerName(msg.name, msg.name_length);
  _proceduralTrackComponent->AddSquint(layerName, msg.squintScaleX,
                                       msg.squintScaleY, msg.upperLidAngle,
                                       _relativeStreamTime_ms);
}

void AnimationStreamer::ProcessRemoveSquint(
    const RobotInterface::RemoveSquint& msg) {
  const std::string layerName(msg.name, msg.name_length);
  _proceduralTrackComponent->RemoveSquint(layerName, _relativeStreamTime_ms,
                                          msg.disableTimeout_ms);
}
...
void AnimationStreamer::StopTracksInUse(bool aborting) {
  if (!aborting) {
    // The anim has terminated normally so just let head and lift settle to
    // final positions
    _tracksInUse &=
        ~((u8)AnimTrackFlag::LIFT_TRACK | (u8)AnimTrackFlag::HEAD_TRACK);
  }
...
// TODO(str): VIC-13519 Linearize Face Rendering
// This function should be replaced by directly rendering the procedural face.
// Treating it like a full-screen image here after its been built is very
// wasteful
void AnimationStreamer::InsertStreamableFaceIntoCompImg(
    Vision::ImageRGB565& streamableFace, Vision::CompositeImage& image) {
  ANKI_CPU_PROFILE("AnimationStreamer::InsertStreamableFaceIntoCompImg");
  auto* rgbaImg = new Vision::ImageRGBA(streamableFace.GetNumRows(),
                                        streamableFace.GetNumCols());
...
void AnimationStreamer::InvalidateBannedTracks(
    const std::string& animName,
    AnimationMessageWrapper& messageWrapper) const {
  const bool needToCheckWhitelist =
      _onCharger && ((_lockedTracks & (u8)AnimTrackFlag::BODY_TRACK) == 0);

  // note: this duplicates engine's animation_whitelist.json, but hopefully
  // InvalidateBannedTracks is removed soon
  //
  // Note: Any animation name that begins with or is equal to the provided
  // string will be considered. E.g. if "anim_test_" appears in the list, it
  // will match "anim_test_01", "anim_test_02", "anim_test_drive_01", etc.
  static const std::vector<std::string> whitelisted = {
      "anim_chargerdocking_comeoff_", "anim_chargerdocking_settle_",
      "anim_movement_forward_",       "anim_onboarding_driveoff_charger_",
  ...
  if (_onCharger && _frozenOnCharger) {
    // When on charger, don't move or play audio! (This could be alexa acoustic
    // test mode) Don't lock tracks so as to not disturb any other
    // functionality, simply drop the messages on the floor.
    ...
    if (ANKI_DEV_CHEATS) {
      // A list of known issues where animations are used without locking tracks
      // on the charger
      static const std::set<std::string> knownIssues = {
          "anim_lookatphone_loop_01", "anim_onboarding_wakeword_getin_01",
          "anim_avs_l_getin_03",      "anim_avs_suddenspeak_03",
          "PROCEDURAL_ANIM",
      };
      if (knownIssues.find(animName) == knownIssues.end()) {
        LOG_WARNING("AnimationStreamer.InvalidateBannedTracks.UnknownIssue",
                    "Animation '%s' did not have its body track locked when on "
                    "the charger",
                    animName.c_str());
      }
    }
