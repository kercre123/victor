 ...
 * Created: 06/28/2017
 *
 * Description: Component which manages creating various procedural animations
 *by using the trackLayerManagers to generate keyframes and add them to track
 *layers Currently there are only three trackLayerManagers face, backpack, and
 *audio
 *
 *
...
class TrackLayerComponent {
 public:
  ...
  // Put keep face alive into "focused" mode, which reduces the jumpiness and
  // eye darts to make the robot appear more focused and looking straight ahead,
  // but without "dead" eyes.
  ...
  void AddSquint(const std::string& name, f32 squintScaleX, f32 squintScaleY,
                 f32 upperLidAngle, TimeStamp_t streamTime_ms);

  // Removes specified squint after duration_ms has passed
  void RemoveSquint(const std::string& name, TimeStamp_t streamTime_ms,
  ...
  void AddOrUpdateEyeShift(const std::string& name, f32 xPix, f32 yPix,
                           TimeStamp_t duration_ms, TimeStamp_t streamTime_ms,
  ...
  void RemoveEyeShift(const std::string& name, TimeStamp_t streamTime_ms,
                      TimeStamp_t duration_ms = 0);
 ...
 private:
  // The KeepFaceAlive system consists of multiple Modifiers applied to the face
  // by the AnimationStreamer when no animation is controlling the face -- to
  // keep the robot looking "alive". For example, blinks are one Modifier, and
  // eye darts are another. Multiple Modifiers exist on separate layers and can
  // be run at the same time. A priority flag could be added in the future to
  // handle more complicated cases.

  struct KeepAliveModifier {
    using ActivityPerformFunc =
        std::function<bool(const TimeStamp_t streamTime_ms)>;
    ...
    KeepAliveModifier(
        const std::string& name, ActivityPerformFunc performFunc,
    ...
    void UpdateNextPerformanceTime() {
      nextPerformanceTime_ms = getNextPerformanceTimeFunc();
    }
  };
