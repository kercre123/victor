...
// MATH: (BufferSize / SampleRate) * NumberOfBuffers ==> 1024/48000*4 =
// 0.0853333 sec => 85 ms After doing slow motion recordings we found that there
// was an additional 115 ms of latency ==> total latency = 200 ms
CONSOLE_VAR_RANGED(u32, kAudioAnimationOffset_ms, "Audio.AnimationStreamer",
                   200, 0, 300);
#else
...
void TrackLayerComponent::Update() {
  ...
  /*
  auto& desiredGlitchComponent =
  _context->GetNeedsManager()->GetDesiredFaceDistortionComponent(); const float
  desiredGlitchDegree = desiredGlitchComponent.GetCurrentDesiredDistortion();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TrackLayerComponent::ApplyLayersToAnim(
    Animation* anim, const TimeStamp_t timeSinceAnimStart_ms,
    LayeredKeyFrames& layeredKeyframes, bool storeFace) const {
  ...
  ApplyFaceLayersToAnim(anim, timeSinceAnimStart_ms, layeredKeyframes,
                        storeFace);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TrackLayerComponent::KeepFaceAlive(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TrackLayerComponent::RemoveKeepFaceAlive(TimeStamp_t streamTime_ms,
                                              TimeStamp_t duration_ms) {
  _audioLayerManager->RemovePersistentLayer(kEyeDartLayerName, streamTime_ms,
                                            duration_ms);
  _faceLayerManager->RemovePersistentLayer(kEyeDartLayerName, streamTime_ms,
                                           duration_ms);

  _audioLayerManager->RemovePersistentLayer(kEyeBlinkLayerName, streamTime_ms,
                                            duration_ms);
  _faceLayerManager->RemovePersistentLayer(kEyeBlinkLayerName, streamTime_ms,
                                           duration_ms);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TrackLayerComponent::AddSquint(const std::string& name, f32 squintScaleX,
                                    f32 squintScaleY, f32 upperLidAngle,
                                    TimeStamp_t streamTime_ms) {
  Animations::Track<ProceduralFaceKeyFrame> faceTrack;
  _faceLayerManager->GenerateSquint(squintScaleX, squintScaleY, upperLidAngle,
                                    faceTrack, streamTime_ms);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TrackLayerComponent::AddOrUpdateEyeShift(
    const std::string& name, f32 xPix, f32 yPix, TimeStamp_t duration_ms,
    TimeStamp_t streamTime_ms, f32 xMax, f32 yMax, f32 lookUpMaxScale,
    f32 lookDownMinScale, f32 outerEyeScaleIncrease) {
  ProceduralFaceKeyFrame eyeShift;
  _faceLayerManager->GenerateEyeShift(xPix, yPix, xMax, yMax, lookUpMaxScale,
                                      lookDownMinScale, outerEyeScaleIncrease,
                                      (streamTime_ms + duration_ms), eyeShift);
  ...
  if (!_faceLayerManager->HasLayer(name)) {
    ...
    if (duration_ms > 0) {
      ...
      faceTrack.AddKeyFrameToBack(
          ProceduralFaceKeyFrame(streamTime_ms, duration_ms));
    }
...
void TrackLayerComponent::AddGlitch(f32 glitchDegree) {
  ...
  PRINT_CH_DEBUG("Animations", "TrackLayerComponent.AddGlitch", "Degree %.2f",
                 glitchDegree);
  Animations::Track<ProceduralFaceKeyFrame> faceTrack;
  const u32 numFrames =
      _faceLayerManager->GenerateFaceDistortion(glitchDegree, faceTrack);
...
void TrackLayerComponent::SetupKeepFaceAliveActivities() {
  ...
  auto eyeBlinkPerform = [this](const TimeStamp_t streamTime_ms) {
    ...
    Result result = _faceLayerManager->AddBlinkToFaceTrack(
        kEyeBlinkLayerName, streamTime_ms, eventList);
    if (RESULT_OK == result) {
      _audioLayerManager->AddEyeBlinkToAudioTrack(kEyeBlinkLayerName, eventList,
                                                  streamTime_ms);
    } else {
      PRINT_NAMED_WARNING(
          "TrackLayerComponent.SetupKeepFaceAliveActivities.eyeBlinkPerform",
  ...
  _keepAliveModifiers.emplace_back(kEyeBlinkLayerName, eyeBlinkPerform,
                                   eyeBlinkTimeFunc, true);

  // Eye Dart
  auto eyeDartPerform = [this](const TimeStamp_t streamTime_ms) {
    TimeStamp_t interpolationTime_ms = 0;
    Result result = _faceLayerManager->AddEyeDartToFaceTrack(
        kEyeDartLayerName, _isKeepFaceAliveFocused, streamTime_ms,
        interpolationTime_ms);
    if (RESULT_OK == result) {
      _audioLayerManager->AddEyeDartToAudioTrack(
          kEyeDartLayerName, interpolationTime_ms, streamTime_ms);
    } else {
      PRINT_NAMED_WARNING(
          "TrackLayerComponent.SetupKeepFaceAliveActivities.eyeDartPerform",
          "AddBlinkToFaceTrack.Failed");
    }
    return (RESULT_OK == result);
  };
  ...
  _keepAliveModifiers.emplace_back(kEyeDartLayerName, eyeDartPerform,
                                   eyeDartTimeFunc, true);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TrackLayerComponent::ApplyAudioLayersToAnim(
    Animation* anim, const TimeStamp_t timeSinceAnimStart_ms,
    LayeredKeyFrames& layeredKeyFrames) const {
  // VIC-4224: Due to audio engine playback latency the animation audio
  // keyframes are not in sync with the rest of the animation tracks while
  // playing. Therefore we have introduced a variable to offset that latency by
  // playing audio keyframes earlier so they better sync with the animation.
  const TimeStamp_t audioOffsetTime_ms =
      timeSinceAnimStart_ms + kAudioAnimationOffset_ms;
  ...
  if (_audioLayerManager->HaveLayersToSend()) {
    ...
    layeredKeyFrames.haveAudioKeyFrame |=
        _audioLayerManager->ApplyLayersToFrame(
            layeredKeyFrames.audioKeyFrame, timeSinceAnimStart_ms, applyFunc);
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TrackLayerComponent::ApplyBackpackLayersToAnim(
    Animation* anim, const TimeStamp_t timeSinceAnimStart_ms,
    LayeredKeyFrames& layeredKeyFrames) const {
  ...
  // If the backpackLayerManager has layers then we need to combine them
  // together (with the keyframe from the anim)
  if (_backpackLayerManager->HaveLayersToSend()) {
    auto applyFunc =
        [](const Animations::Track<BackpackLightsKeyFrame>& layerTrack,
           const TimeStamp_t timeSinceAnimStart_ms,
           BackpackLightsKeyFrame& outFrame) {
          // Get the current keyframe from the layer's track
          if (layerTrack.CurrentFrameIsValid(timeSinceAnimStart_ms)) {
            // TODO: Blend frame and outFrame?
            // Would need to account for whether or not the anim has a backpack
            // keyframe
    ...
    const bool haveBackpack = _backpackLayerManager->ApplyLayersToFrame(
        layeredKeyFrames.backpackKeyFrame, timeSinceAnimStart_ms, applyFunc);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TrackLayerComponent::ApplyFaceLayersToAnim(
    Animation* anim, const TimeStamp_t timeSinceAnimStart_ms,
    LayeredKeyFrames& layeredKeyFrames, bool storeFace) const {
  ...
  if (anim != nullptr) {
    // Face keyframe from animation should replace whatever is in
    // layeredKeyframes.faceKeyframe
    const bool kShouldReplace = true;
    const bool faceUpdated = _faceLayerManager->GetFaceHelper(
        anim->GetTrack<ProceduralFaceKeyFrame>(), timeSinceAnimStart_ms,
        layeredKeyFrames.faceKeyFrame, kShouldReplace);
    layeredKeyFrames.haveFaceKeyFrame = faceUpdated;

    // Update _lastProceduralFace if the face was updated and we should store
    // the face
  ...
  if (_faceLayerManager->HaveLayersToSend()) {
    auto applyFunc = [this](
                         const Animations::Track<ProceduralFaceKeyFrame>& track,
                         const TimeStamp_t timeSinceAnimStart_ms,
                         ProceduralFaceKeyFrame& outFrame) {
      // Procedural layers should not replace what is already in outFrame, they
      // need to be combined with it
      const bool kShouldReplace = false;
      return _faceLayerManager->GetFaceHelper(track, timeSinceAnimStart_ms,
                                              outFrame, kShouldReplace);
    };

    layeredKeyFrames.haveFaceKeyFrame |= _faceLayerManager->ApplyLayersToFrame(
        layeredKeyFrames.faceKeyFrame, timeSinceAnimStart_ms, applyFunc);
  }
}
