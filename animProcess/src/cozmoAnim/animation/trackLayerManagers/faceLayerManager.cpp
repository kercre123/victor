 ...
 *              Handles procedural face layering, which includes things like
 *KeepAlive, look-ats while turning, blinks, and repair glitches.
 *
...
// Only used for setting console var ranges below, so not used in shipping
// builds (thus ANKI_DEV_CHEATS)
constexpr int kMaxDartDist =
    std::max(FACE_DISPLAY_WIDTH, FACE_DISPLAY_HEIGHT) / 2;
#endif

// Global keep-alive eye dart params (spacing, distance, scaling)
CONSOLE_VAR_RANGED(s32, kKeepAliveEyeDart_SpacingMinTime_ms, CONSOLE_GROUP_NAME,
                   1000, 0, 10000);
CONSOLE_VAR_RANGED(s32, kKeepAliveEyeDart_SpacingMaxTime_ms, CONSOLE_GROUP_NAME,
                   2250, 0, 10000);
CONSOLE_VAR_RANGED(s32, kKeepAliveEyeDart_MaxDistFromCenter_pix,
                   CONSOLE_GROUP_NAME, 15, 0, kMaxDartDist);
CONSOLE_VAR_RANGED(s32, kKeepAliveEyeDart_MaxDistFromCenterFocused_pix,
                   CONSOLE_GROUP_NAME, 1, 0, kMaxDartDist);
CONSOLE_VAR_RANGED(f32, kKeepAliveEyeDart_UpMaxScale, CONSOLE_GROUP_NAME, 1.05f,
                   1.f, 1.2f);
CONSOLE_VAR_RANGED(f32, kKeepAliveEyeDart_DownMinScale, CONSOLE_GROUP_NAME,
                   0.9f, 0.5f, 1.f);
CONSOLE_VAR_RANGED(f32, kKeepAliveEyeDart_OuterEyeScaleIncrease,
                   CONSOLE_GROUP_NAME, 0.03f, 0.f, 0.2f);
CONSOLE_VAR_RANGED(f32, kKeepAliveEyeDart_ShiftLagFraction, CONSOLE_GROUP_NAME,
                   0.4f, 0.f, 1.f);
CONSOLE_VAR_RANGED(f32, kKeepAliveEyeDart_HotSpotPositionMultiplier,
                   CONSOLE_GROUP_NAME, 1.5f, 0.5f, 10.f);

// Medium distance eye dart params (when dart's length is larger than threshold)
// These darts have a single interpolation frame (with associated dart distance
// and squash fractions)
constexpr int kMediumDartDefaultThresh_pix = 5;
CONSOLE_VAR_RANGED(int, kKeepAliveEyeDart_MediumDistanceThresh_pix,
                   CONSOLE_GROUP_NAME, kMediumDartDefaultThresh_pix, 0,
                   kMaxDartDist);
CONSOLE_VAR_RANGED(f32, kKeepAliveEyeDart_MediumShiftFraction,
                   CONSOLE_GROUP_NAME, 0.2f, 0.f, 1.f);
CONSOLE_VAR_RANGED(f32, kKeepAliveEyeDart_MediumSquashFraction,
                   CONSOLE_GROUP_NAME, 0.85f, 0.5f, 1.f);

// Long distance eye dart params (when dart's length is larger than threshold)
// These darts have two interpolation frames (with associated dart distance and
// squash fractions)
constexpr int kLongDartDefaultThresh_pix = 10;
CONSOLE_VAR_RANGED(int, kKeepAliveEyeDart_LongDistanceThresh_pix,
                   CONSOLE_GROUP_NAME, kLongDartDefaultThresh_pix, 0,
                   kMaxDartDist);
CONSOLE_VAR_RANGED(f32, kKeepAliveEyeDart_LongShiftFraction1,
                   CONSOLE_GROUP_NAME, 0.2f, 0.f, 1.f);
CONSOLE_VAR_RANGED(f32, kKeepAliveEyeDart_LongShiftFraction2,
                   CONSOLE_GROUP_NAME, 0.4f, 0.f, 1.f);
CONSOLE_VAR_RANGED(f32, kKeepAliveEyeDart_LongSquashFraction1,
                   CONSOLE_GROUP_NAME, 0.7f, 0.5f, 1.f);
CONSOLE_VAR_RANGED(f32, kKeepAliveEyeDart_LongSquashFraction2,
                   CONSOLE_GROUP_NAME, 0.85f, 0.5f, 1.f);
...
// Blink params:
CONSOLE_VAR(f32, kMaxBlinkSpacingTimeForScreenProtection_ms, CONSOLE_GROUP_NAME,
            30000);
CONSOLE_VAR_RANGED(s32, kKeepAliveBlink_SpacingMinTime_ms, CONSOLE_GROUP_NAME,
                   3000, 0, 30000);
CONSOLE_VAR_RANGED(s32, kKeepAliveBlink_SpacingMaxTime_ms, CONSOLE_GROUP_NAME,
                   10000, 0, 30000);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool FaceLayerManager::GetFaceHelper(
    const Animations::Track<ProceduralFaceKeyFrame>& track,
    TimeStamp_t timeSinceAnimStart_ms, ProceduralFaceKeyFrame& procFace,
    ...
    if (currentKeyFrame.IsTimeToPlay(timeSinceAnimStart_ms)) {
      ...
      if (nextFrame != nullptr) {
        if (nextFrame->IsTimeToPlay(timeSinceAnimStart_ms)) {
          // If it's time to play the next frame and the current frame at the
          // same time, something's wrong!
          PRINT_NAMED_WARNING("FaceLayerManager.GetFaceHelper.FramesTooClose",
                              "currentFrameTriggerTime: %d ms, "
                              "nextFrameTriggerTime: %d, StreamTime: %d",
                              currentKeyFrame.GetTriggerTime_ms(),
                              nextFrame->GetTriggerTime_ms(),
                              timeSinceAnimStart_ms);
        } else {
          /*
           // If we're within one sample period following the currFrame, just
           play the current frame if (currStreamTime -
           currentKeyFrame.GetTriggerTime_ms() < ANIM_TIME_STEP_MS) {
           interpolatedParams = currentKeyFrame.GetFace().GetParams();
           paramsSet = true;
           }
           // We're on the way to the next frame, but not too close to it:
           interpolate. else if (nextFrame->GetTriggerTime_ms() - currStreamTime
           >= ANIM_TIME_STEP_MS) {
           */
          interpolatedFace = currentKeyFrame.GetInterpolatedFace(
              *nextFrame, timeSinceAnimStart_ms);
          paramsSet = true;
          //}
        }
      } else {
      ...
      if (paramsSet) {
        ...
        if (shouldReplace) {
          ...
          const_cast<ProceduralFace&>(procFace.GetFace())
              .Combine(interpolatedFace);
        }
      }
    }  // if(nextFrame != nullptr
  }    // if(track.HasFramesLeft())
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void FaceLayerManager::GenerateEyeShift(f32 xPix, f32 yPix, f32 xMax, f32 yMax,
                                        f32 lookUpMaxScale,
  ...
  procFace.LookAt(xPix, yPix, std::max(xMin, ProceduralFace::WIDTH - xMax),
                  std::max(yMin, ProceduralFace::HEIGHT - yMax), lookUpMaxScale,
...
int GetNumEyeDartInterpFrames(const Point2f& dartVector) {
  ...
  if (ANKI_DEV_CHEATS && (kKeepAliveEyeDart_MediumDistanceThresh_pix >=
                          kKeepAliveEyeDart_LongDistanceThresh_pix)) {
    LOG_WARNING(
        "FaceLayerManager.GetNumEyeDartInterpFrames.BadThresholds",
  ...
  if (dartDistanceSq > (kKeepAliveEyeDart_LongDistanceThresh_pix *
                        kKeepAliveEyeDart_LongDistanceThresh_pix)) {
    numInterpFrames = 2;
  } else if (dartDistanceSq > (kKeepAliveEyeDart_MediumDistanceThresh_pix *
                               kKeepAliveEyeDart_MediumDistanceThresh_pix)) {
    numInterpFrames = 1;
  }
...
struct DartParam {
  ...
  f32 verticalSquash;  // Vertical squash adds a sort of mini-blink to the dart
                       // (per animators), so no horizontal
};
...
static inline DartParam InterpDartParam(const Point2f& lastDartPosition,
                                        ...
                                        f32 xshiftFraction,
                                        const f32 squashFraction) {
  // Compute interpolated dart position. To produce less linear motion, we move
  // the X and Y coordinates by differing amounts, depending on direction, and
  // not in uniform steps to the final position. This gives the feel of both an
  // ease-in and an arc shape to the path.
  ...
  // Hot spot moves in the same direction as the dart vector but is simply
  // relative amounts (not absolute positions)
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void FaceLayerManager::GenerateKeepAliveEyeDart(
    const std::string& layerName, bool hasDartLayer, const f32 maxDist_pix,
  ...
  DEV_ASSERT(Util::IsFltGTZero(maxDist_pix),
             "FaceLayerManager.GenerateKeepAliveEyDart.ZeroDistance");

  const Point2f dartFinalPosition(
  ...
  const f32 hotSpotScale =
      kKeepAliveEyeDart_HotSpotPositionMultiplier / maxDist_pix;
  const Point2f dartFinalHotspot(
      Util::Clamp(dartFinalPosition.x() * hotSpotScale, -1.f, 1.f),
  ...
  switch (numInterpFrames) {
    ...
    case 1: {
      ...
      dartParams.emplace_back(
          InterpDartParam(_lastDartPosition, dartVector, dartFinalHotspot,
    ...
    case 2: {
      ...
      dartParams.emplace_back(InterpDartParam(
          dartParams.back().position, dartVector, dartFinalHotspot,
    ...
    default:
      DEV_ASSERT_MSG(
          false,
          "FaceLayerManager.GenerateKeepAliveEyeDart.InvalidNumInterpFrames",
  ...
  for (const auto& dartParam : dartParams) {
    ...
    dartFace.SetParameterBothEyes(ProceduralFace::Parameter::HotSpotCenterX,
                                  dartParam.hotspotPosition.x());
    dartFace.SetParameterBothEyes(ProceduralFace::Parameter::HotSpotCenterY,
                                  dartParam.hotspotPosition.y());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void FaceLayerManager::GenerateBlink(
  ...
  do {
    moreBlinkFrames =
        ProceduralFaceDrawer::GetNextBlinkFrame(blinkFace, blinkState, timeInc);
    track.AddKeyFrameToBack(
        ProceduralFaceKeyFrame(blinkFace, totalOffset, timeInc));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result FaceLayerManager::AddBlinkToFaceTrack(
    const std::string& layerName, const TimeStamp_t timeSinceKeepAliveStart_ms,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result FaceLayerManager::AddEyeDartToFaceTrack(
    const std::string& layerName, const bool isFocused,
    const TimeStamp_t timeSinceKeepAliveStart_ms,
    TimeStamp_t& out_interpolationTime_ms) {
  const f32 maxDist =
      (isFocused ? kKeepAliveEyeDart_MaxDistFromCenterFocused_pix
  ...
  if (Util::IsFltGTZero(maxDist)) {
    ...
    const bool noOtherFaceLayers =
        (numLayers == 0 || (numLayers == 1 && hasDartLayer));

    // If there's no other face layer active right now, do the dart. Otherwise,
    // skip it
    if (noOtherFaceLayers) {
      GenerateKeepAliveEyeDart(layerName, hasDartLayer, maxDist,
                               timeSinceKeepAliveStart_ms);
    }
  }
...
s32 FaceLayerManager::GetNextEyeDartTime_ms() const {
  return GetRNG().RandIntInRange(kKeepAliveEyeDart_SpacingMinTime_ms,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
u32 FaceLayerManager::GenerateFaceDistortion(
    float distortionDegree,
  ...
  do {
    TimeStamp_t timeInc;
    moreDistortionFrames = ScanlineDistorter::GetNextDistortionFrame(
        distortionDegree, repairFace, timeInc);
    totalOffset += timeInc;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void FaceLayerManager::GenerateSquint(
    f32 squintScaleX, f32 squintScaleY, f32 upperLidAngle,
    Animations::Track<ProceduralFaceKeyFrame>& track,
  ...
  squintFace.SetParameterBothEyes(ProceduralFace::Parameter::EyeScaleY,
                                  DockSquintScaleY);
  squintFace.SetParameterBothEyes(ProceduralFace::Parameter::EyeScaleX,
                                  DockSquintScaleX);
  squintFace.SetParameterBothEyes(ProceduralFace::Parameter::UpperLidAngle,
                                  -10.0f);
  // need start at t=0 (a.k.a. timeSinceKeepAliveStart_ms) to get interpolation
  track.AddKeyFrameToBack(
      ProceduralFaceKeyFrame(timeSinceKeepAliveStart_ms, interpolationTime_ms));
  track.AddKeyFrameToBack(ProceduralFaceKeyFrame(
      squintFace, (timeSinceKeepAliveStart_ms + interpolationTime_ms)));
}
