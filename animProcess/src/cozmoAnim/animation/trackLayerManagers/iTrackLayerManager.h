...
class ITrackLayerManager {
 public:
  ...
  using ApplyLayerFunc = std::function<bool(
      const Animations::Track<FRAME_TYPE>&, const TimeStamp_t, FRAME_TYPE&)>;

  // Updates frame by applying all layers to it using applyLayerFunc which
  // should define how to combine the current keyframe in the layer's track with
  // another keyframe Both applyFunc and this function should return whether or
  // not the frame was updated Note: applyLayerFunc is responsible for moving to
  // the next keyframe of a layer's track
