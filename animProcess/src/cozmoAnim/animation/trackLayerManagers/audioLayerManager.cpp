...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result AudioLayerManager::AddEyeBlinkToAudioTrack(
    const std::string& layerName, const BlinkEventList& eventList,
  ...
  const auto eventIt = std::find_if(
      eventList.begin(), eventList.end(), [](const BlinkEvent& event) {
  ...
  if (eventIt != eventList.end()) {
    ...
    eventGroup.AddEvent(
        GameEvent::GenericEvent::Play__Robot_Vic_Sfx__Scrn_Procedural_Blink,
        1.0f, 1.0f);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result AudioLayerManager::AddEyeDartToAudioTrack(
    const std::string& layerName, const TimeStamp_t interpolationTime_ms,
    const TimeStamp_t timeSinceAnimStart_ms) {
  ...
  Animations::Track<RobotAudioKeyFrame> audioTrack;
  // Add parameter
  auto paramRef = AudioParameterRef(
      GameParameter::ParameterType::Robot_Vic_Screen_Shift_Interpolation_Time,
      interpolationTime_ms, 0.0f, AudioEngine::Multiplexer::CurveType::Linear,
      kProceduralGameObject);
  frame.AddAudioRef(std::move(paramRef));
  // Add Event Group
  AudioEventGroupRef eventGroup(kProceduralGameObject);
  eventGroup.AddEvent(
      GameEvent::GenericEvent::Play__Robot_Vic_Sfx__Scrn_Procedural_Shift, 1.0f,
      1.0f);
  frame.AddAudioRef(std::move(eventGroup));
  frame.SetTriggerTime_ms(
      interpolationTime_ms);  // Always start with begining of movement
  audioTrack.AddKeyFrameToBack(frame);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
Result AudioLayerManager::AddEyeSquintToAudioTrack(
    const std::string& layerName, const TimeStamp_t timeSinceAnimStart_ms) {
  ...
  AudioEventGroupRef eventGroup(kProceduralGameObject);
  eventGroup.AddEvent(
      GameEvent::GenericEvent::Play__Robot_Vic_Sfx__Scrn_Procedural_Squint,
      1.0f, 1.0f);
  frame.AddAudioRef(std::move(eventGroup));
  frame.SetTriggerTime_ms(
      timeSinceAnimStart_ms);  // Always start with begining of movement
  audioTrack.AddKeyFrameToBack(frame);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void AudioLayerManager::GenerateGlitchAudio(
    u32 numFramesToGen, Animations::Track<RobotAudioKeyFrame>& outTrack) const {
  if (!_enabled) {
    return;
  }
  ...
  for(int i = 0; i < numFramesToGen; i++)
  {
    ...
    for(int i = 0; i < sample.sample.size(); ++i)
    {
      ...
      prevGlitchAudioSampleVal = (prevGlitchAudioSampleVal + (0.02 * rand))
  / 1.02; sample.sample[i] = ((prevGlitchAudioSampleVal * 3.5) + 1) * 128;
    }
