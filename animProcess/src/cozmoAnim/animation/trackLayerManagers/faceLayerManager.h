...
class FaceLayerManager : public ITrackLayerManager<ProceduralFaceKeyFrame> {
 public:
  ...
  // Eye shifts keyframes are generated with a relative start time - they should
  // then be updated to reflect their true playback time within a track
  void GenerateEyeShift(f32 xPix, f32 yPix, f32 xMax, f32 yMax,
                        f32 lookUpMaxScale, f32 lookDownMinScale,
                        f32 outerEyeScaleIncrease, TimeStamp_t duration_ms,
  ...
  // When isFocused=true, eye darts will be much smaller in order to keep the
  // eyes moving but still looking forward
  ...
  // Generates a track of all keyframes necessary to make the face have
  // distortion Returns how many keyframes were generated
  u32 GenerateFaceDistortion(
