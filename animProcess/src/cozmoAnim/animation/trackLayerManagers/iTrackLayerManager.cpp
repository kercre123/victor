...
ITrackLayerManager<FRAME_TYPE>::ITrackLayerManager(
    const Util::RandomGenerator& rng)
    : _rng(rng) {}

template <class FRAME_TYPE>
bool ITrackLayerManager<FRAME_TYPE>::ApplyLayersToFrame(
    FRAME_TYPE& frame, const TimeStamp_t timeSinceAnimStart_ms,
  ...
  for (auto layerIter = _layers.begin(); layerIter != _layers.end();
       ++layerIter) {
...
template <class FRAME_TYPE>
Result ITrackLayerManager<FRAME_TYPE>::AddLayer(
    const std::string& name, const Animations::Track<FRAME_TYPE>& track) {
...
template <class FRAME_TYPE>
void ITrackLayerManager<FRAME_TYPE>::AddPersistentLayer(
    const std::string& name, const Animations::Track<FRAME_TYPE>& track) {
  if (_layers.find(name) != _layers.end()) {
    PRINT_NAMED_WARNING(
        "TrackLayerManager.AddPersistentLayer.LayerAlreadyExists", "");
  }
...
template <class FRAME_TYPE>
void ITrackLayerManager<FRAME_TYPE>::AddToPersistentLayer(
    const std::string& layerName, FRAME_TYPE& keyframe) {
...
template <class FRAME_TYPE>
void ITrackLayerManager<FRAME_TYPE>::RemovePersistentLayer(
    const std::string& layerName, TimeStamp_t streamTime_ms,
    TimeStamp_t duration_ms) {
  auto layerIter = _layers.find(layerName);
  if (layerIter != _layers.end()) {
    auto& layerName = layerIter->first;
    LOG_INFO("ITrackLayerManager.RemovePersistentLayer",
             "%s, (Layers remaining=%lu)", layerName.c_str(),
             (unsigned long)_layers.size() - 1);
  ...
  if (_layers.empty()) {
    ...
    for (const auto& layer : _layers) {
      if (!layer.second.isPersistent || !layer.second.sentOnce) {
        ...
        // that has not been sent in its entirety at least once: return that
        // there are still layers to send
        return true;
      }
    }
    // All layers are persistent ones that have been sent, so no need to keep
    // sending them by themselves. They only need to be re-applied while there's
    // something else being sent
    return false;
  }
}

template <class FRAME_TYPE>
bool ITrackLayerManager<FRAME_TYPE>::HasLayer(
    const std::string& layerName) const {
  return _layers.find(layerName) != _layers.end();
}

template <class FRAME_TYPE>
void ITrackLayerManager<FRAME_TYPE>::AdvanceTracks(
    const TimeStamp_t toTime_ms) {
    ...
    if (!layer.track.HasFramesLeft()) {
      ...
      if (layer.isPersistent) {
        if (layer.track.IsEmpty()) {
          LOG_WARNING(
              "AnimationStreamer.UpdateFace.EmptyPersistentLayer",
          ...
          if (DEBUG_FACE_LAYERING) {
            LOG_DEBUG("AnimationStreamer.UpdateFace.HoldingLayer",
                      "Holding last frame of face layer %s", layerName.c_str());
          }
...
template <class FRAME_TYPE>
void ITrackLayerManager<FRAME_TYPE>::ValidateTrack(
    const Animations::Track<FRAME_TYPE>& track) {
  if (ANKI_DEV_CHEATS) {
    // Ensure tracks don't overlap
    for (const auto& keyframe : track.GetCopyOfKeyframes()) {
      ANKI_VERIFY(keyframe.GetTriggerTime_ms() !=
                      keyframe.GetTimestampActionComplete_ms(),
...
template <>
void ITrackLayerManager<ProceduralFaceKeyFrame>::ValidateTrack(
    const Animations::Track<ProceduralFaceKeyFrame>& track) {
  if (ANKI_DEV_CHEATS) {
    // Ensure tracks don't overlap
    auto keyframes = track.GetCopyOfKeyframes();
    for (auto keyframeIter = keyframes.begin(); keyframeIter != keyframes.end();
         keyframeIter++) {
      ANKI_VERIFY(keyframeIter->GetTriggerTime_ms() !=
                      keyframeIter->GetTimestampActionComplete_ms(),
                  "ITrackLayerManager.ValidateTrack.KeyframeWithNoLength",
                  "All keyframes must have a duration");
      ...
      if (nextIter != keyframes.end()) {
        ANKI_VERIFY(
            keyframeIter->GetTimestampActionComplete_ms() ==
                nextIter->GetTriggerTime_ms(),
            "ITrackLayerManager.ValidateTrack.ProceduralKeyframeTimeMismatch",
            "Previous keyframe ends at %u, but next frame does not trigger "
            "until %u, interpolation will break",
            keyframeIter->GetTimestampActionComplete_ms(),
            nextIter->GetTriggerTime_ms());
      }
    }
...
template class ITrackLayerManager<
    BackpackLightsKeyFrame>;  // BackpackLayerManager
