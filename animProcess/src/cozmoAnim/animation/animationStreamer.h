...
class AnimationStreamer {
 public:
  ...
  // TODO: This could be removed in favor of just referring to ::Anki::Cozmo,
  // but avoiding touching too much code now.
  ...
  //
  // If interruptRunning == true, any currently-streaming animation will be
  // aborted. Actual streaming occurs on calls to Update().
  //
  ...
  // animation playing
  //
  // If overrideAllSpritesToEyeHue is true the SpriteBoxKeyFrames will be
  // treated as grayscale and rendered in the robot's eye hue - if it's false
  // the keyframes will be rendered as RGB images
  Result SetStreamingAnimation(const std::string& name, Tag tag,
                               u32 numLoops = 1, u32 startAt_ms = 0,
                               bool interruptRunning = true,
                               bool overrideAllSpritesToEyeHue = false);

  // Subset of the function above that is applied in the ::Update function and
  // called from PlayAnimation
  ...
  void Process_displayFaceImageChunk(
  ...
  Result SetFaceImage(Vision::SpriteHandle spriteHandle,
                      bool overrideAllSpritesToEyeHue, u32 duration_ms);

  Audio::ProceduralAudioClient* GetProceduralAudioClient() const {
    return _proceduralAudioClient.get();
  }

  // If any animation is set for streaming and isn't done yet, stream it.
  Result Update();

  // If tag == kNotAnimatingTag, it stops whatever animation may currently be
  // playing. Otherwise, it stops the currently running animation only if it
  ...
  // Functions passed in here will be called each time a new animation is set to
  // streaming
  void AddNewAnimationCallback(NewAnimationCallback callback) {
    _newAnimationCallbacks.push_back(callback);
  }

  // Returns the time in ms that the animation streamer will use to get
  // animation frames NOTE: This value generally updated at the end of the
  // Update tick, so checks before streamer update will tell you the stream time
  // used this tick, checks after will show the value for the next call to
  // update
  TimeStamp_t GetRelativeStreamTime_ms() const {
    return _relativeStreamTime_ms;
  }

  // Set/Reset the amount of time to wait before forcing KeepFaceAlive() after
  ...
  TrackLayerComponent* GetProceduralTrackComponent() {
    return _proceduralTrackComponent.get();
  }
  const TrackLayerComponent* GetProceduralTrackComponent() const {
    return _proceduralTrackComponent.get();
  }
  ...
  void SetLockedTracks(u8 whichTracks) {
    if (whichTracks & (u8)AnimTrackFlag::BACKPACK_LIGHTS_TRACK) {
      PRINT_NAMED_ERROR("AnimationStreamer.SetLockedTracks.BackpackLightTrack",
                        "Backpack light track is always locked, why are you "
                        "trying to lock it");
    }
...
// Always keep the backpack light track locked in shipping
  ...
  void RedirectFaceImagesToDebugScreen(bool redirect) {
    _redirectFaceImagesToDebugScreen = redirect;
  }

  void SetOnCharger(bool onCharger);

  // When on the charger, the robot won't play any motion or audio frames,
  // irrespective of locked tracks
  void SetFrozenOnCharger(bool enabled);

  // Procedural Eye
  void ProcessAddOrUpdateEyeShift(
 ...
 private:
  ...
  // There are a few special cases where time should not be incremented for a
  // tick e.g. looping animations which are initialized one tick, but don't get
  // their first update call until the next tick
  ...
  // Time to wait before forcing KeepFaceAlive() after the latest stream has
  // stopped
  ...
  // Binary images
  u32 _faceImageId = 0;  // Used only for tracking chunks of the same image as
                         // they are received
  u8 _faceImageChunksReceivedBitMask = 0;
  const u8 kAllFaceImageChunksReceivedMask =
      0x3;  // 2 bits for 2 expected chunks

  // Grayscale images
  u32 _faceImageGrayscaleId = 0;  // Used only for tracking chunks of the same
                                  // image as they are received
  u32 _faceImageGrayscaleChunksReceivedBitMask = 0;
  const u32 kAllFaceImageGrayscaleChunksReceivedMask =
      0x7fff;  // 15 bits for 15 expected chunks (FACE_DISPLAY_NUM_PIXELS / 1200
               // pixels_per_msg ~= 15)

  // RGB images
  Vision::ImageRGB565 _faceImageRGB565;
  u32 _faceImageRGBId = 0;  // Used only for tracking chunks of the same image
                            // as they are received
  u32 _faceImageRGBChunksReceivedBitMask = 0;
  const u32 kAllFaceImageRGBChunksReceivedMask =
      0x3fffffff;  // 30 bits for 30 expected chunks (FACE_DISPLAY_NUM_PIXELS /
                   // 600 pixels_per_msg ~= 30)
  ...
  Result SetStreamingAnimation(Animation* anim, Tag tag, u32 numLoops = 1,
                               u32 startAt_ms = 0, bool interruptRunning = true,
                               bool overrideAllSpritesToEyeHue = false,
  ...
  Result InitStreamingAnimation(Tag withTag, u32 startAt_ms = 0,
                                bool overrideAllSpritesToEyeHue = false);
  ...
  // Used to stream _just_ the stuff left in the various layers (all procedural
  // stuff)
  Result ExtractMessagesFromProceduralTracks(
      AnimationMessageWrapper& stateToSend);

  // Combine the tracks inside of the specified animations with the tracks in
  // the track layer component specified, and then assign the output to
  // stateToSend
  Result ExtractMessagesRelatedToProceduralTrackComponent(
  ...
  bool ShouldProcessAnimationFrame(Animation* anim, TimeStamp_t startTime_ms,
  ...
  // If abortingAnim == true, then the message is sent even if all loops were
  // not completed.
  ...
  static void InsertStreamableFaceIntoCompImg(
  ...
  static void GetStreamableFace(const Anim::AnimContext* context,
  ...
  // Sends msg to appropriate destination as long as the specified track is
  // unlocked
  bool SendIfTrackUnlocked(RobotInterface::EngineToRobot*& msg,
                           AnimTrackFlag track);
