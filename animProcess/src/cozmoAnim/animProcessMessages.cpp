...
Anki::Vector::Anim::StreamingAnimationModifier* _streamingAnimationModifier =
    nullptr;
...
static void ListAnimations(ConsoleFunctionContextRef context) {
  ...
  std::vector<std::string> names =
      _context->GetDataLoader()->GetAnimationNames();
...
static void PlayAnimation(ConsoleFunctionContextRef context) {
  ...
  if (name) {
    ...
    if (!_dispatchQueue) {
      _dispatchQueue = Anki::Util::Dispatch::Create(
          "AddAnimation", Anki::Util::ThreadPriority::Low);
    }
    ...
    snprintf(numLoopsStr, sizeof(numLoopsStr), "%d",
             (numLoops > 9999) ? 9999 : numLoops);
    std::string text =
        std::string("Playing ") + name + " " + numLoopsStr + " times<br>";
...
static void AddAnimation(ConsoleFunctionContextRef context) {
  ...
  if (path) {
    const std::string animationFolder =
        _context->GetDataPlatform()->pathToResource(
...
static void ShowCurrentAnimation(ConsoleFunctionContextRef context) {
  const std::string currentAnimation =
      _animStreamer->GetStreamingAnimationName();
  context->channel->WriteLog("<html>\n");
...
CONSOLE_FUNC(PlayAnimation, "Animations", const char* name,
             optional int numLoops);
...
void Process_checkCloudConnectivity(
    const Anki::Vector::RobotInterface::CheckCloudConnectivity& msg) {
  auto* micDataSystem = _context->GetMicDataSystem();
  if (micDataSystem != nullptr) {
...
void Process_setFullAnimTrackLockState(
    const Anki::Vector::RobotInterface::SetFullAnimTrackLockState& msg) {
  // LOG_DEBUG("AnimProcessMessages.Process_setFullAnimTrackLockState", "0x%x",
  // msg.whichTracks);
...
void Process_addAnim(const Anki::Vector::RobotInterface::AddAnim& msg) {
  ...
  LOG_INFO("AnimProcessMessages.Process_addAnim", "Animation File: %s",
           path.c_str());
...
void Process_playAnim(const Anki::Vector::RobotInterface::PlayAnim& msg) {
  ...
  LOG_INFO("AnimProcessMessages.Process_playAnim", "Anim: %s, Tag: %d",
  ...
  _animStreamer->SetStreamingAnimation(animName, msg.tag, msg.numLoops,
                                       msg.startAt_ms, interruptRunning,
                                       overrideAllSpritesToEyeHue);
}

void Process_abortAnimation(
    const Anki::Vector::RobotInterface::AbortAnimation& msg) {
  LOG_INFO("AnimProcessMessages.Process_abortAnimation", "Tag: %d", msg.tag);
  _animStreamer->Abort(msg.tag);
}

void Process_displayProceduralFace(
    const Anki::Vector::RobotInterface::DisplayProceduralFace& msg) {
...
void Process_setFaceSaturation(
    const Anki::Vector::RobotInterface::SetFaceSaturation& msg) {
  ProceduralFace::SetSaturation(msg.saturation);
}

void Process_displayFaceImageBinaryChunk(
    const Anki::Vector::RobotInterface::DisplayFaceImageBinaryChunk& msg) {
  _animStreamer->Process_displayFaceImageChunk(msg);
}

void Process_displayFaceImageGrayscaleChunk(
    const Anki::Vector::RobotInterface::DisplayFaceImageGrayscaleChunk& msg) {
  _animStreamer->Process_displayFaceImageChunk(msg);
}

void Process_displayFaceImageRGBChunk(
    const Anki::Vector::RobotInterface::DisplayFaceImageRGBChunk& msg) {
  _animStreamer->Process_displayFaceImageChunk(msg);
}

void Process_playAnimWithSpriteBoxRemaps(
    const Anki::Vector::RobotInterface::PlayAnimWithSpriteBoxRemaps& msg) {
  _animStreamer->Process_playAnimWithSpriteBoxRemaps(msg);
}

void Process_playAnimWithSpriteBoxKeyFrames(
    const Anki::Vector::RobotInterface::PlayAnimWithSpriteBoxKeyFrames& msg) {
  _animStreamer->Process_playAnimWithSpriteBoxKeyFrames(msg);
}

void Process_addSpriteBoxKeyFrames(
    const Anki::Vector::RobotInterface::AddSpriteBoxKeyFrames& msg) {
  _animStreamer->Process_addSpriteBoxKeyFrames(msg);
}

void Process_enableKeepFaceAlive(
    const Anki::Vector::RobotInterface::EnableKeepFaceAlive& msg) {
  _animStreamer->EnableKeepFaceAlive(msg.enable, msg.disableTimeout_ms);
}

void Process_setKeepFaceAliveFocus(
    const Anki::Vector::RobotInterface::SetKeepFaceAliveFocus& msg) {
  _animStreamer->SetKeepFaceAliveFocus(msg.enable);
}

void Process_addOrUpdateEyeShift(
    const Anki::Vector::RobotInterface::AddOrUpdateEyeShift& msg) {
  _animStreamer->ProcessAddOrUpdateEyeShift(msg);
}

void Process_removeEyeShift(
    const Anki::Vector::RobotInterface::RemoveEyeShift& msg) {
...
void Process_removeSquint(
    const Anki::Vector::RobotInterface::RemoveSquint& msg) {
  _animStreamer->ProcessRemoveSquint(msg);
}

void Process_postAudioEvent(
    const Anki::AudioEngine::Multiplexer::PostAudioEvent& msg) {
  _engAudioInput->HandleMessage(msg);
}

void Process_stopAllAudioEvents(
    const Anki::AudioEngine::Multiplexer::StopAllAudioEvents& msg) {
  _engAudioInput->HandleMessage(msg);
}

void Process_postAudioGameState(
    const Anki::AudioEngine::Multiplexer::PostAudioGameState& msg) {
  _engAudioInput->HandleMessage(msg);
}

void Process_postAudioSwitchState(
    const Anki::AudioEngine::Multiplexer::PostAudioSwitchState& msg) {
  _engAudioInput->HandleMessage(msg);
}

void Process_postAudioParameter(
    const Anki::AudioEngine::Multiplexer::PostAudioParameter& msg) {
  _engAudioInput->HandleMessage(msg);
}

void Process_setDebugConsoleVarMessage(
    const Anki::Vector::RobotInterface::SetDebugConsoleVarMessage& msg) {
  // We are using messages generated by the CppLite emitter here, which does not
  // support variable length arrays. CLAD also doesn't have a char, so the
  // "strings" in this message are actually arrays of uint8's. Thus we need to
  // do this reinterpret cast here. In some future world, ideally we avoid all
  // this and use, for example, a web interface to set/access console vars,
  // instead of passing around via CLAD messages.
  ...
  Anki::Util::IConsoleVariable* consoleVar =
      Anki::Util::ConsoleSystem::Instance().FindVariable(varName);
  if (consoleVar && consoleVar->ParseText(tryValue)) {
    // SendVerifyDebugConsoleVarMessage(_externalInterface, varName,
    // consoleVar->ToString().c_str(), consoleVar, true);
    LOG_INFO("AnimProcessMessages.Process_setDebugConsoleVarMessage.Success",
             "'%s' set to '%s'", varName, tryValue);
  } else {
    LOG_WARNING("AnimProcessMessages.Process_setDebugConsoleVarMessage.Fail",
                "Error setting '%s' to '%s'", varName, tryValue);
    //      SendVerifyDebugConsoleVarMessage(_externalInterface,
    //      msg.varName.c_str(),
    //                                       consoleVar ? "Error: Failed to
    //                                       Parse" : "Error: No such variable",
    //                                       consoleVar, false);
  }
}

void Process_startRecordingMicsRaw(
    const Anki::Vector::RobotInterface::StartRecordingMicsRaw& msg) {
  auto* micDataSystem = _context->GetMicDataSystem();
  if (micDataSystem == nullptr) {
    return;
  }

  micDataSystem->RecordRawAudio(
      msg.duration_ms, std::string(msg.path, msg.path_length), msg.runFFT);
}

void Process_startRecordingMicsProcessed(
    const Anki::Vector::RobotInterface::StartRecordingMicsProcessed& msg) {
  auto* micDataSystem = _context->GetMicDataSystem();
  if (micDataSystem == nullptr) {
    return;
  }
  ...
  micDataSystem->RecordProcessedAudio(msg.duration_ms,
                                      std::string(msg.path, msg.path_length));
}

void Process_startWakeWordlessStreaming(
    const Anki::Vector::RobotInterface::StartWakeWordlessStreaming& msg) {
  auto* micDataSystem = _context->GetMicDataSystem();
  if (micDataSystem == nullptr) {
    return;
  }
  ...
  micDataSystem->StartWakeWordlessStreaming(
      static_cast<CloudMic::StreamType>(msg.streamType),
      msg.playGetInFromAnimProcess);
}

void Process_setTriggerWordResponse(
    const Anki::Vector::RobotInterface::SetTriggerWordResponse& msg) {
...
void Process_setAlexaUXResponses(
    const Anki::Vector::RobotInterface::SetAlexaUXResponses& msg) {
  auto* showStreamStateManager = _context->GetShowAudioStreamStateManager();
  if (showStreamStateManager != nullptr) {
...
void Process_resetBeatDetector(
    const Anki::Vector::RobotInterface::ResetBeatDetector& msg) {
  auto* micDataSystem = _context->GetMicDataSystem();
  if (micDataSystem != nullptr) {
...
void Process_setAlexaUsage(
    const Anki::Vector::RobotInterface::SetAlexaUsage& msg) {
...
void Process_setButtonWakeWord(
    const Anki::Vector::RobotInterface::SetButtonWakeWord& msg) {
  auto* micDataSystem = _context->GetMicDataSystem();
  if (micDataSystem != nullptr) {
...
void Process_setLCDBrightnessLevel(
    const Anki::Vector::RobotInterface::SetLCDBrightnessLevel& msg) {
  FaceDisplay::getInstance()->SetFaceBrightness(msg.level);
}

void Process_playbackAudioStart(
    const Anki::Vector::RobotInterface::StartPlaybackAudio& msg) {
...
void Process_drawTextOnScreen(
    const Anki::Vector::RobotInterface::DrawTextOnScreen& msg) {
  FaceInfoScreenManager::getInstance()->SetCustomText(msg);
}

void Process_runDebugConsoleFuncMessage(
    const Anki::Vector::RobotInterface::RunDebugConsoleFuncMessage& msg) {
  // We are using messages generated by the CppLite emitter here, which does not
  // support variable length arrays. CLAD also doesn't have a char, so the
  // "strings" in this message are actually arrays of uint8's. Thus we need to
  // do this reinterpret cast here. In some future world, ideally we avoid all
  ...
  // TODO: Ideally, we'd send back a verify message that we (failed to) set this
  Anki::Util::IConsoleFunction* consoleFunc =
      Anki::Util::ConsoleSystem::Instance().FindFunction(funcName);
  if (consoleFunc) {
    enum { kBufferSize = 512 };
    char buffer[kBufferSize];
    const uint32_t res = NativeAnkiUtilConsoleCallFunction(funcName, funcArgs,
                                                           kBufferSize, buffer);
    LOG_INFO("AnimProcessMessages.Process_runDebugConsoleFuncMessage",
             "%s '%s' set to '%s'", (res != 0) ? "Success" : "Failure",
             funcName, funcArgs);
  } else {
    LOG_WARNING(
        "AnimProcessMessages.Process_runDebugConsoleFuncMessage.NoConsoleFunc",
        "No Func named '%s'", funcName);
  }
}
...
void Process_externalAudioPrepare(
    const RobotInterface::ExternalAudioPrepare& msg) {
  _animEngine->HandleMessage(msg);
}

void Process_externalAudioComplete(
    const RobotInterface::ExternalAudioComplete& msg) {
  _animEngine->HandleMessage(msg);
}

void Process_externalAudioCancel(
    const RobotInterface::ExternalAudioCancel& msg) {
  _animEngine->HandleMessage(msg);
}

void Process_textToSpeechPrepare(
    const RobotInterface::TextToSpeechPrepare& msg) {
  _animEngine->HandleMessage(msg);
}
...
void Process_setConnectionStatus(
    const Anki::Vector::SwitchboardInterface::SetConnectionStatus& msg) {
...
void Process_setBLEPin(
    const Anki::Vector::SwitchboardInterface::SetBLEPin& msg) {
  SetBLEPin(msg.pin);
}

void Process_rangeDataToDisplay(
    const Anki::Vector::RobotInterface::RangeDataToDisplay& msg) {
  FaceInfoScreenManager::getInstance()->DrawToF(msg.data);
}

void Process_sendBLEConnectionStatus(
    const Anki::Vector::SwitchboardInterface::SendBLEConnectionStatus& msg) {
  // todo
}

void Process_alterStreamingAnimation(
    const RobotInterface::AlterStreamingAnimationAtTime& msg) {
  _streamingAnimationModifier->HandleMessage(msg);
}

void Process_setLocale(const RobotInterface::SetLocale& msg) {
  DEV_ASSERT(_animEngine != nullptr,
             "AnimProcessMessages.SetLocale.InvalidEngine");
  _animEngine->HandleMessage(msg);
}
...
void Process_acousticTestEnabled(
    const Anki::Vector::RobotInterface::AcousticTestEnabled& msg) {
...
void Process_triggerBackpackAnimation(
    const RobotInterface::TriggerBackpackAnimation& msg) {
...
void Process_enableMirrorModeScreen(
    const RobotInterface::EnableMirrorModeScreen& msg) {
...
void Process_updatedSettings(const RobotInterface::UpdatedSettings& msg) {
  ...
  switch (msg.settingBeingChanged) {
    case SettingBeingChanged::SETTING_ENABLE_DATA_COLLECTION:
      _context->GetMicDataSystem()->SetEnableDataCollectionSettings(
          msg.enableDataCollection);
      break;
...
void Process_fakeWakeWordFromExternalInterface(
    const RobotInterface::FakeWakeWordFromExternalInterface& msg) {
  _context->GetMicDataSystem()->FakeTriggerWordDetection();
}

void AnimProcessMessages::ProcessMessageFromEngine(
  ...
  switch (msg.tag) {
    ...
    case RobotInterface::EngineToRobot::Tag_calmPowerMode: {
      ...
      // Only relay the power mode to robot process if not already in
      // pairing/debug screen.
      FaceInfoScreenManager::getInstance()->SetCalmPowerModeOnReturnToNone(
          msg.calmPowerMode);
      forwardToRobot =
          FaceInfoScreenManager::getInstance()->GetCurrScreenName() ==
          ScreenName::None;
      break;
    }
    case RobotInterface::EngineToRobot::Tag_setBackpackLights: {
      // Intercept the SetBackpackLights message from engine
      _context->GetBackpackLightComponent()->SetBackpackAnimation(
          {msg.setBackpackLights});
      break;
    }
...
static void HandleRobotStateUpdate(const Anki::Vector::RobotState& robotState) {
  _pendingRobotDisconnectTime_sec =
      BaseStationTimer::getInstance()->GetCurrentTimeInSeconds() +
      kNoRobotStateDisconnectTimeout_sec;
  ...
  auto* micDataSystem = _context->GetMicDataSystem();
  if (micDataSystem != nullptr) {
    const bool isMicFace =
        FaceInfoScreenManager::getInstance()->GetCurrScreenName() ==
        ScreenName::MicDirectionClock;
    if (isMicFace) {
      const auto liftHeight_mm =
          ConvertLiftAngleToLiftHeightMM(robotState.liftAngle);
...
void AnimProcessMessages::ProcessMessageFromRobot(
    const RobotInterface::RobotToEngine& msg) {
  ...
  switch (tag) {
    ...
    case RobotInterface::RobotToEngine::Tag_prepForShutdown: {
      PRINT_NAMED_INFO("AnimProcessMessages.ProcessMessageFromRobot.Shutdown",
                       "");
      // Need to wait a couple ticks before actually shutting down so that this
      // message can be forwarded up to engine
    ...
    case RobotInterface::RobotToEngine::Tag_state: {
      ...
      const bool onChargerContacts =
          (msg.state.status & (uint32_t)RobotStatusFlag::IS_ON_CHARGER);
    ...
    case RobotInterface::RobotToEngine::Tag_robotStopped: {
      LOG_INFO("AnimProcessMessages.ProcessMessageFromRobot.RobotStopped",
               "Abort animation");
      _animStreamer->Abort();
    } break;
...
Result AnimProcessMessages::Init(
  ...
  // Preconditions
  DEV_ASSERT(nullptr != animEngine,
             "AnimProcessMessages.Init.InvalidAnimEngine");
  DEV_ASSERT(nullptr != animStreamer,
             "AnimProcessMessages.Init.InvalidAnimStreamer");
  DEV_ASSERT(nullptr != audioInput,
             "AnimProcessMessages.Init.InvalidAudioInput");
...
Result AnimProcessMessages::MonitorConnectionState(
    BaseStationTime_t currTime_nanosec) {
  ...
  static const BaseStationTime_t kDisconnectedTimeout_ns =
      Util::SecToNanoSec(5.f);
  ...
  if (!wasConnected && isConnected) {
    LOG_INFO("AnimProcessMessages.MonitorConnectionState",
             "Robot now available");
    ...
    PRINT_NAMED_WARNING(
        "AnimProcessMessages.MonitorConnectionState.DisconnectedFromEngine",
        "We have become disconnected from engine process. Displaying a fault "
        "code in %.1f seconds.",
...
Result AnimProcessMessages::Update(BaseStationTime_t currTime_nanosec) {
  ...
  if (!AnimComms::IsConnectedToRobot()) {
    ...
    LOG_WARNING("AnimProcessMessages.Update.RobotStateTimeout",
                "Disconnecting robot");
    AnimComms::DisconnectRobot();
  ...
  // Process messages from engine
  {
    ...
    while ((dataLen = AnimComms::GetNextPacketFromEngine(
                pktBuffer_, MAX_PACKET_BUFFER_SIZE)) > 0) {
      ...
      if (msg.Size() != dataLen) {
        LOG_WARNING("AnimProcessMessages.Update.EngineToRobot.InvalidSize",
                    "Invalid message size from engine (%d != %d)", msg.Size(),
                    dataLen);
        continue;
      }
      if (!msg.IsValid()) {
        LOG_WARNING("AnimProcessMessages.Update.EngineToRobot.InvalidData",
                    "Invalid message from engine");
        continue;
      }
  ...
  // Process messages from robot
  {
    ...
    while ((dataLen = AnimComms::GetNextPacketFromRobot(
                pktBuffer_, MAX_PACKET_BUFFER_SIZE)) > 0) {
      ...
      if (msg.Size() != dataLen) {
        LOG_WARNING("AnimProcessMessages.Update.RobotToEngine.InvalidSize",
                    "Invalid message size from robot (%d != %d)", msg.Size(),
                    dataLen);
        continue;
      }
      if (!msg.IsValid()) {
        LOG_WARNING("AnimProcessMessages.Update.RobotToEngine.InvalidData",
                    "Invalid message from robot");
        continue;
      }
...
#else
  FaceInfoScreenManager::getInstance()->SetShouldDrawFAC(
      !Factory::GetEMR()->fields.PACKED_OUT_FLAG);
#endif
#endif
  ...
  if (!_bootAnimStopped &&
      _context->GetMicDataSystem()->HasConnectionToCloud() && _engineLoaded) {
...
bool AnimProcessMessages::SendAnimToRobot(
    const RobotInterface::EngineToRobot& msg) {
  static Util::MessageProfiler msgProfiler(
      "AnimProcessMessages::SendAnimToRobot");
  LOG_TRACE("AnimProcessMessages.SendAnimToRobot", "Send tag %d size %u",
            msg.tag, msg.Size());
...
bool AnimProcessMessages::SendAnimToEngine(
    const RobotInterface::RobotToEngine& msg) {
  static Util::MessageProfiler msgProfiler(
      "AnimProcessMessages::SendAnimToEngine");

  LOG_TRACE("AnimProcessMessages.SendAnimToEngine", "Send tag %d size %u",
            msg.tag, msg.Size());
