 ...
 * Created: 7/30/2017
 *
 * Description: Create sockets and manages low-level data transfer to engine and
 *robot processes
 *
...
void UpdateSocketBufferStats(SocketBufferStats& stats, int socket) {
  if (socket >= 0) {
    ...
    DEV_ASSERT(incoming >= 0,
               "AnimComms.UpdateSocketBufferStats.InvalidIncoming");
    if (incoming >= 0) {
      stats._incoming->Record(incoming);
    }
    const auto outgoing = Anki::Messaging::GetOutgoingSize(socket);
    DEV_ASSERT(outgoing >= 0,
               "AnimComms.UpdateSocketBufferStats.InvalidOutgoing");
    if (outgoing >= 0) {
...
void ReportSocketBufferStats(const std::string& name,
  ...
  LOG_INFO("AnimComms.ReportSocketBufferStats", "%s = %lld/%lld/%lld",
           name.c_str(), min, mean, max);
}

void ReportSocketBufferStats(const std::string& name,
                             const SocketBufferStats& stats) {
...
Result InitRobotComms() {
  ...
  const std::string& client_path =
      std::string(ANIM_ROBOT_CLIENT_PATH) + std::to_string(robotID);
  const std::string& server_path =
      std::string(ANIM_ROBOT_SERVER_PATH) + std::to_string(robotID);

  LOG_INFO("AnimComms.InitRobotComms", "Connect from %s to %s",
...
Result InitEngineComms() {
  const RobotID_t robotID = OSState::getInstance()->GetRobotID();
  const std::string& server_path =
      std::string(ENGINE_ANIM_SERVER_PATH) + std::to_string(robotID);

  LOG_INFO("AnimComms.InitEngineComms", "Start listening at %s",
           server_path.c_str());

  if (!_engineComms.StartListening(server_path)) {
    LOG_ERROR("AnimComms.InitEngineComms", "Unable to listen at %s",
              server_path.c_str());
    return RESULT_FAIL_IO;
  }
...
Result InitComms() {
  ...
  if (RESULT_OK != result) {
    LOG_ERROR("AnimComms.InitComms", "Unable to init robot comms (result %d)",
              result);
    return result;
  }
  ...
  result = InitEngineComms();
  if (RESULT_OK != result) {
    LOG_ERROR("AnimComms.InitComms", "Unable to init engine comms (result %d)",
              result);
    return result;
  }
...
bool IsConnectedToRobot(void) { return _robotComms.IsConnected(); }

bool IsConnectedToEngine(void) { return _engineComms.HasClient(); }
...
bool SendPacketToEngine(const void* buffer, const u32 length) {
  ...
  if (bytesSent < (ssize_t)length) {
    LOG_ERROR("AnimComms.SendPacketToEngine.FailedSend",
              "Failed to send msg contents (%zd of %d bytes sent)", bytesSent,
              length);
    DisconnectEngine();
    return false;
  }
...
u32 GetNextPacketFromEngine(u8* buffer, u32 max_length) {
  ...
  if (dataLen < 0) {
    ...
    LOG_ERROR("GetNextPacketFromEngine.FailedRecv",
              "Failed to receive from engine");
    DisconnectEngine();
    return 0;
  }
  return (u32)dataLen;
...
bool SendPacketToRobot(const void* buffer, const u32 length) {
  ...
  if (bytesSent < (ssize_t)length) {
    LOG_ERROR("SendPacketToRobot.FailedSend",
              "Failed to send msg contents (%zd bytes sent)", bytesSent);
    DisconnectRobot();
    return false;
  }
...
u32 GetNextPacketFromRobot(u8* buffer, u32 max_length) {
  ...
  if (dataLen < 0) {
    // Something went wrong
    LOG_ERROR("GetNextPacketFromRobot.FailedRecv",
              "Failed to receive from robot");
    DisconnectRobot();
    return 0;
  }
