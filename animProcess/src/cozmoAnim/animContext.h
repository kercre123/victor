 ...
 * Created: 1/29/2016
 *
 * Description: Holds references to components and systems that are used often
 *by all different parts of code, where it is unclear who the appropriate owner
 *of that system would be. NOT intended to be a container to hold ALL systems
 *and components, which would simply be lazy.
 *
...
class AnimContext : private Util::noncopyable {
 ...
 public:
  ...
  Audio::CozmoAudioController* GetAudioController()
      const;  // Can return nullptr
  AudioMultiplexer* GetAudioMultiplexer() const { return _audioMux.get(); }
  MicData::MicDataSystem* GetMicDataSystem() const {
    return _micDataSystem.get();
  }
  ShowAudioStreamStateManager* GetShowAudioStreamStateManager() const {
    return _showStreamStateManager.get();
  }
  WebService::WebService* GetWebService() const { return _webService.get(); }
  Audio::AudioPlaybackSystem* GetAudioPlaybackSystem() const {
    return _audioPlayer.get();
  }
  Alexa* GetAlexa() const { return _alexa.get(); }
  BackpackLightComponent* GetBackpackLightComponent() const {
    return _backpackLightComponent.get();
  }
