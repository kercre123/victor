 ...
 *              Leaves the screen on when stopped under the assumption whatever
 *is stopping it will continue to use the screen
 *
...
namespace {
// Path to the boot animation file generated by `python3
// /robot/test/gif_to_raw.py <gif of boot animation>` It is a binary file
// containing all the frames in the animation in rgb565 format
static const char* _animPath =
...
static void animate(void* frame) {
  lcd_draw_frame2((uint16_t*)frame,
                  FRAME_WIDTH * FRAME_HEIGHT * sizeof(uint16_t));
}

static void handler(int signum) { gShutdown = true; }
...
int main(int argc, char** argv) {
  ...
  while (!gShutdown) {
    ...
    const auto dif = std::chrono::duration_cast<std::chrono::milliseconds>(
        endTime - startTime);
    timeCount += dif.count();
