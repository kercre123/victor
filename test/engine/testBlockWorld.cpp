...
class BlockWorldTest : public testing::Test {
 ...
 private:
  ...
  void FakeRobotState(uint32_t atTimestamp, bool isHeadMoving = false) {
    ...
    if (isHeadMoving) {
      stateMsg.status &=
          ~Anki::Util::EnumToUnderlying(RobotStatusFlag::HEAD_IN_POS);
    }
  ...
  void SetCameraCalibration() {
    // Note: These values are approximate Cozmo calibration values, so they may
    // not match the actual robot's calibration
  ...
  // Observe the given marker code at the given timestamp. If no 'corners' are
  // specified, there is no guarantee where the marker is in the frame - this
  // just simply 'observes' a marker in some pose.
  void ObserveMarker(const Vision::Marker::Code code,
                     const uint32_t atTimestamp,
                     const Quad2f corners = Quad2f{
                         Point2f(67, 117), Point2f(70, 185), Point2f(136, 116),
                         Point2f(137, 184)}) {
    // Fake an observation
    VisionProcessingResult procResult;
    procResult.timestamp = (TimeStamp_t)atTimestamp;
    procResult.observedMarkers.clear();
    procResult.observedMarkers.push_back(
        Vision::ObservedMarker((TimeStamp_t)atTimestamp, code, corners,
...
TEST_F(BlockWorldTest, Create) { ASSERT_TRUE(_robot != nullptr); }

TEST_F(BlockWorldTest, ObserveMarker) {
  // Observe a marker of an object, and ensure that we create an instance of
  // that object in Blockworld
  ...
  const Vision::Marker::Code code =
      cube.GetMarker(Block::FaceName::FRONT_FACE).GetCode();
  ...
  ASSERT_EQ(object->GetActiveID(), -1)
      << "Object should have an invalid activeID, since it's unconnected";
  ASSERT_EQ(_robot->GetWorldOriginID(), object->GetPose().GetRootID())
      << "Object should be in robot's world origin";
  ASSERT_EQ(PoseState::Known, object->GetPoseState());
}

TEST_F(BlockWorldTest, ObserveConnectedBlock) {
  // Pretend to connect to a block, disconnect from it, connect to it again,
  // then observe that block later and ensure that the object IDs are properly
  // matched.
  ...
  ObjectID connectedObjectId =
      blockWorld.AddConnectedBlock(activeId, factoryId, objectType);
  ...
  // Now pretend to observe an object of the same type. It should inherit the
  // ObjectID of the connected block.
  ...
  // Pretend to disconnect from the block, and ensure we remove the connected
  // instance and keep the located instance
  const auto removedObjID =
      _robot->GetBlockWorld().RemoveConnectedBlock(activeId);
...
TEST_F(BlockWorldTest, ConnectToObservedBlock) {
  // Pretend to observe a block then connect to it and ensure that the object
  // IDs are properly matched.
  ...
  // Pretend to 'connect' to a block of the same type. The connected object
  // should inherit the objectID of the observed block.
  ActiveID activeId = 1;
  FactoryID factoryId = "aa:bb:cc:dd:ee:ff";
...
TEST_F(BlockWorldTest, DISABLED_LocalizeToCharger) {
  ...
  // Robot should be localized now in the original origin, since we've observed
  // the charger
  ...
  ASSERT_NE(firstOriginId, _robot->GetWorldOriginID())
      << "Should be in a new origin since we've delocalized";

  object = blockWorld.FindLocatedMatchingObject(filter);
  ASSERT_EQ(nullptr, object);

  // Now observe the charger again. Should re-localize to the charger in the
  // first origin.
  fakeTimestamp_ms += 10;
  FakeRobotState(fakeTimestamp_ms);
  ...
  ASSERT_TRUE(_robot->IsLocalized());
  ASSERT_EQ(firstOriginId, _robot->GetWorldOriginID())
      << "Should have localized in the first origin, since that's where we saw "
         "the charger first";
...
TEST_F(BlockWorldTest, ObservationDistance) {
  ...
  const Quad2f farCorners{Point2f(67, 117), Point2f(70, 135), Point2f(82, 116),
                          Point2f(84, 134)};
  ...
  // Now observe the charger in a new location (but still close enough). This
  // should move the charger's pose (not the robot's pose), since the robot has
  // not moved.
  ...
  const Quad2f closeCorners2 = {Point2f(57, 107), Point2f(60, 175),
                                Point2f(126, 106), Point2f(127, 174)};

  ObserveMarker(code, fakeTimestamp_ms, closeCorners2);

  ASSERT_FALSE(
      firstChargerPose.IsSameAs(object->GetPose(), 5.f, DEG_TO_RAD(5.f)));
