...
TEST(LineSegment, TestIntersection) {
...

  {
    ...
    bool res = l1.IntersectsAt(l2, intersection);
    ASSERT_TRUE(res);
    EXPECT_EQ(Anki::Point2f(9.492753623188406, 5.753623188405797),
              intersection);
  }
...
TEST(LineSegment, TestInHalfPlane) {
  // check if in positive x halfplane
  {
    ...
    Anki::Line2f xAxisMinus({0.f, -1.f},
                            0);  // -y = 0 (reverses sign of dot products)
...
TEST(AffineHyperplane, InHalfPlane) {
  // All tests for linear 4-dimensional hyperplane: x1 + x2 + x3 + x4 = 0
  {
    ...
    // InHalfPlane is strictly Ax + b > 0, so cannot be in the halfplane defined
    // by itself
...
TEST(Ball, Contains) {
  {
    ...
    for (int i = 0; i < 1000; ++i) {
      Anki::Point3f p;
      for (int j = 0; j < 3; ++j) {
        p[j] = gen.RandDbl() - gen.RandDbl();
      }

      // VIC-2171: Unit test must apply same logic for tolerance as method being
      // tested
...
TEST(Ball, InHalfPlane) {
  // test axis lines
  {
    ...
    Anki::Line2f xAxis({0.f, 1.f}, 0);  //  y = 0
    Anki::Line2f xAxisMinus({0.f, -1.0},
                            0);  // -y = 0 (reverses sign of dot products)
