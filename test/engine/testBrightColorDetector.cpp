...
TEST(BrightColorDetector, WhiteBalance)
{
  ...
  ImageRGB inputImage, outputImage, colorAdjustedImage;
  Image grayImage;
  ...
  for (const auto& filename : filenames){
    ...
    ASSERT_EQ(inputImage.Load(filename), Anki::Result::RESULT_OK);
    inputImage.CopyTo(outputImage);
    inputImage.CopyTo(colorAdjustedImage);
    grayImage = inputImage.ToGray();
    ...
    // Rotate the image color
    {
      ...
      if (!Anki::Util::IsNear(measured.dot(neutral),1)){
        ...
        cv::Mat_<PixelRGB>& image = colorAdjustedImage.get_CvMat_();
        for (s32 row = 0; row < colorAdjustedImage.GetNumRows(); ++row){
          for (s32 col = 0; col < colorAdjustedImage.GetNumCols(); ++col){
    ...
    colorAdjustedImage.Display("Color Adjusted Image", 0);
    ...
    result = brightColorDetector.Detect(colorAdjustedImage, modifiedImageSalientPoints);

    ASSERT_EQ(result, Anki::Result::RESULT_OK);

    drawSalientPoints(originalImageSalientPoints, outputImage);
    outputImage.Display("Output Image", 0);
    drawSalientPoints(modifiedImageSalientPoints, colorAdjustedImage);
    colorAdjustedImage.Display("Color Adjusted Output Image", 0);
