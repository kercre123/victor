...
TEST(CubeLedAnimation, TestAllFields) {
  ...
  for (auto baseIndex : baseIndices) {
    ...
    // Starting index should be baseIndex + 0, and end index should be baseIndex
    // + 2
    EXPECT_EQ(baseIndex + 0, ledAnimation.GetStartingIndex());
    EXPECT_EQ(baseIndex + 2, ledAnimation.GetFinalIndex());

    // Keyframe 0 should be offColor, with holdFrames = 0, decayFrames =
    // transitionOnFrames, nextIndex = baseIndex + 1
    ...
    // Keyframe 1 should be onColor, with holdFrames = onFrames, decayFrames =
    // transitionOffFrames, nextIndex = baseIndex + 2
    ...
    // Keyframe 2 should be offColor, with holdFrames = offTime, decayFrames =
    // 0, nextIndex = baseIndex
...
TEST(CubeLedAnimation, TestOffset) {
  ...
  for (auto baseIndex : baseIndices) {
    ...
    // Keyframe 0 should be black, with holdFrames = offset, decayFrames = 0,
...
TEST(CubeLedAnimation, TestPlayOnce) {
  ...
  for (auto baseIndex : baseIndices) {
    ...
    // Keyframe 2 should be offColor, with holdFrames = 0, decayFrames = 0,
    // nextIndex = baseIndex
    EXPECT_EQ(0x11, keyframes[2].color[0]);
    EXPECT_EQ(0x22, keyframes[2].color[1]);
    EXPECT_EQ(0x33, keyframes[2].color[2]);
...
TEST(CubeLedAnimation, TestLinkToOther) {
  ...
  EXPECT_EQ(keyframesFirst[first.GetFinalIndex()].nextIndex,
            second.GetStartingIndex());
}
