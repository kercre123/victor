 ...
 * Created: 2018-03-12
 *
 * Description: Unit tests for cube messaging - compares CLAD messages with cube
...
TEST(CubeMessages, CompareTags) {
  // Ensure that the CLAD message tag type matches the CubeCommand (which is the
  // command type byte in firmware)
  bool typesMatch =
      std::is_same<std::underlying_type<MessageEngineToCubeTag>::type,
                   CubeCommand>::value;
  typesMatch &= std::is_same<std::underlying_type<MessageCubeToEngineTag>::type,
                             CubeCommand>::value;
  ASSERT_TRUE(typesMatch);

  // Compare message tags
  EXPECT_EQ(static_cast<uint8_t>(MessageEngineToCubeTag::lightSequence),
            COMMAND_LIGHT_INDEX);
  EXPECT_EQ(static_cast<uint8_t>(MessageEngineToCubeTag::lightKeyframes),
            COMMAND_LIGHT_KEYFRAMES);
  EXPECT_EQ(static_cast<uint8_t>(MessageCubeToEngineTag::accelData),
            COMMAND_ACCEL_DATA);
  EXPECT_EQ(static_cast<uint8_t>(MessageCubeToEngineTag::voltageData),
            COMMAND_VOLTAGE_DATA);
}
