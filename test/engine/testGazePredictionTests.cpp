...
TEST(EyeContact, GazeEstimationInterface) {
  ...
  const std::string pathToImages =
      cozmoContext->GetDataPlatform()->pathToResource(
          Util::Data::Scope::Resources, "test/gazeEstimationTests");
  ...
  for (auto const& filename : imageFiles) {
    ...
    lastResult =
        faceTracker.Update(image, cropFactor, faces, updatedIDs, debugImages);
...
TEST(EyeContact, EyeContactInterface) {
  ...
  const std::string base = cozmoContext->GetDataPlatform()->pathToResource(
      Util::Data::Scope::Resources, "test/gazeEstimationTests");
  std::string inputPath =
      Util::FileUtils::FullFilePath({base, "gazePoints.json"});
  std::ifstream gazeDataFile(inputPath, std::ifstream::binary);
  gazeDataFile >> gazeData;

  std::map<uint, std::pair<int, int>> gazePoints;
  for (Json::Value::iterator it = gazeData.begin(); it != gazeData.end();
       it++) {
