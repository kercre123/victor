...
#define WAIT_KEY \
  (-1)  // with <0 no image will be shown, but all the computations to classify
        // it will be done

extern Anki::Vector::CozmoContext* cozmoContext;

void VisualizeClassifierOnImages(
    const std::string& pathToImages,
    const Anki::Vector::RawPixelsClassifier& clf,
    const Anki::Vector::IFeaturesExtractor& extractor =
        Anki::Vector::SinglePixelFeaturesExtraction()) {
  ...
  for (const auto& filename : imageFiles) {
    std::cout << "Classifying image " << ++imageno << " over "
              << imageFiles.size() << " total" << std::endl;
...
Anki::Vector::VisionPoseData PopulateVisionPoseData(float angle) {
  Anki::Pose3d::AllowUnownedParents(true);

  Anki::Vector::Robot robot(0, cozmoContext);
  Anki::Vector::VisionComponent& component = robot.GetVisionComponent();
  ...
  const bool groundPlaneVisible =
      component.LookupGroundPlaneHomography(angle, groundPlaneHomography);

  Anki::Vector::VisionPoseData poseData;
...
Anki::Vector::VisionPoseData PopulateVisionPoseData() {
  Anki::Vector::VisionPoseData poseData;
  poseData.groundPlaneVisible = true;
  Anki::Matrix_3x3f H{284.671, -297.243, 5046.35,     24.9759, 4.61332e-05,
                      12049.2, 0.890988, 5.96046e-08, 15.7945};
...
void checkClassifier(Anki::Vector::RawPixelsClassifier& clf,
  ...
  if (maxTotalError > 0) {
    ...
    std::vector<std::vector<Anki::Vector::RawPixelsClassifier::FeatureType>>
        pixels;
    Anki::Vector::CVMatToVector<float>(trainingSamples, pixels);
    ...
    EXPECT_PRED_FORMAT2(::testing::FloatLE, resultErrorTrainingSet,
                        maxTotalError);
    std::cout << "Error on whole training set: " << resultErrorTrainingSet
              << std::endl;
  }
  ...
  if (maxPositivesError > 0) {
    ...
    // The local friendly lambda. There's a lot going on here just because
    // trainingLabels is a cv::Mat_ with a template type. Getting the type out
    // of the Mat_ is not easy though, hence the dance with decltype
    auto fillPixels = [&trainingSamples =
                           static_cast<const cv::Mat&>(trainingSamples)](
                          cv::Mat& positiveY,
                          std::vector<std::vector<
                              Anki::Vector::RawPixelsClassifier::FeatureType>>&
                              pixels,
                          const auto& trainingLabels) {
      using T = decltype(trainingLabels(0));  // either float or int
      auto elementY = trainingLabels.begin();
      for (uint rowNumber = 0; elementY != trainingLabels.end();
           rowNumber++, elementY++) {
        ...
        if (value != 0) {  // if it's positive class, add the sample to pixels
          ...
          Anki::Vector::CVMatToVector<float>(trainingSamples.row(rowNumber),
                                             rowPixels);
...
/*
 * Tests that training works with a specific error when not using class
 * weighting
 */
TEST(SurfaceClassifier, DISABLED_LRClassifier_TestWholeErrorNoWeighting) {
  ...
  Anki::Vector::LRRawPixelsClassifier clf(config, cozmoContext);

  const std::string path = cozmoContext->GetDataPlatform()->pathToResource(
      Anki::Util::Data::Scope::Resources, "test/overheadMap/run7");
 ...
 */
TEST(SurfaceClassifier,
     DISABLED_LRClassifier_TestPositiveClassOnlyWithWeights) {
  Json::Value config;
  // populate the Json file
  {
    config["NumClusters"] = 5;
  ...
  // There will be a high error on the whole training set, but low on the
  // positives only
...
TEST(SurfaceClassifier, DISABLED_LRClassifier_TestManualLabels) {
  ...
  const std::string path = cozmoContext->GetDataPlatform()->pathToResource(
      Anki::Util::Data::Scope::Resources, "test/overheadMap/manual_labels");
  ...
  if (DISPLAY_IMAGES) {
    VisualizeClassifierOnImages(
        "/Users/lorenzori/tmp/overhead_data/real/run8/images", clf);
  }
}
...
TEST(SurfaceClassifier, DISABLED_THClassifier_TestRealRun) {
  ...
  Anki::Vector::THRawPixelsClassifier clf(config, cozmoContext);
...
TEST(SurfaceClassifier, DISABLED_DTClassifier_TestManualLabels) {
  ...
  Anki::Vector::DTRawPixelsClassifier clf(config, cozmoContext);
...
TEST(SurfaceClassifier, DISABLED_DTClassifier_TestSerialization) {
  ...
  const std::string path = cozmoContext->GetDataPlatform()->pathToResource(
      Anki::Util::Data::Scope::Resources, "test/overheadMap/realImagesDesk");
  ...
  const std::string serializeFileName =
      Anki::Util::FileUtils::FullFilePath({path, "deskClassifier.yaml"});
  bool result = clf.Serialize(serializeFileName.c_str());
  ASSERT_TRUE(result);

  // deserialize the classifier
  Anki::Vector::DTRawPixelsClassifier clf2(cozmoContext);
  ...
  if (DISPLAY_IMAGES) {
    VisualizeClassifierOnImages(
        "/Users/lorenzori/tmp/images_training/real_images/single_shots_desk/"
        "desk",
        clf2);
  }
}
...
TEST(SurfaceClassifier, DISABLED_DTClassifier_TestMeanData) {
  ...
  if (DISPLAY_IMAGES) {
    const Anki::Vector::MeanFeaturesExtractor extractor =
        Anki::Vector::MeanFeaturesExtractor(1);
    VisualizeClassifierOnImages(
        "/Users/lorenzori/tmp/images_training/real_images/single_shots_desk/"
        "desk",
        clf, extractor);
...
TEST(SurfaceClassifier, DISABLED_GroundClassifier_NoiseRemoval) {
  Json::Value config;
  {
    ...
    config["FileOrDirName"] =
        "config/engine/vision/groundClassifier/deskClassifier.yaml";
  }

  Anki::Vector::GroundPlaneClassifier groundPlaneClassifier(config,
                                                            cozmoContext);
  ...
  //  "/Users/lorenzori/tmp/images_training/real_images/selective_annotation";

  std::vector<std::string> imageFiles =
      Anki::Util::FileUtils::FilesInDirectory(path, true, "jpg");
  const Anki::Vector::VisionPoseData poseData =
      PopulateVisionPoseData(Anki::Util::DegToRad(-30));
  Anki::Vector::MeanFeaturesExtractor extractor(1);

  for (const auto& imagepath : imageFiles) {
    PRINT_CH_INFO("VisionSystem", "Test.GroundPlaneClassifier.LoadImage",
                  "Loading image %s", imagepath.c_str());

    Anki::Vision::ImageRGB testImg;
    ASSERT_EQ(testImg.Load(imagepath), Anki::RESULT_OK);

    Anki::Result result = groundPlaneClassifier.Update(
        testImg, poseData, debugImageList, outEdges);
    ...
    if (DISPLAY_IMAGES) {
      ...
      VisualizeClassifierOnImages(
          imagepath, groundPlaneClassifier.GetClassifier(), extractor);
    }
  }
}
...
TEST(SurfaceClassifier, DISABLED_DTClassifier_TestReflectiveDesk) {
  ...
  const std::string path = cozmoContext->GetDataPlatform()->pathToResource(
      Anki::Util::Data::Scope::Resources, "test/overheadMap/reflectiveDesk");
  ...
  // serialize the classifier
  const std::string serializeFileName = Anki::Util::FileUtils::FullFilePath(
      {path, "reflectiveMazeClassifier.yaml"});
  bool result = clf.Serialize(serializeFileName.c_str());
  ASSERT_TRUE(result);
  ...
  if (DISPLAY_IMAGES) {
    auto extractor = Anki::Vector::MeanFeaturesExtractor(3);
    VisualizeClassifierOnImages(
        "/Users/lorenzori/tmp/images_training/real_images/reflectiveMaze", clf2,
        extractor);
  }
}
