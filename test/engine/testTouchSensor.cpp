...
class TestTouchSensorMessageMonitor {
 public:
  TestTouchSensorMessageMonitor(const CozmoContext* context)
      : _touchButtonEventCount(0), _lastTouchButtonEventPressed(false) {
    ...
    auto countTouchButtonEventCb =
        [this](const AnkiEvent<MessageEngineToGame>& event) {
          this->CountTouchButtonEvent(
              event.GetData().Get_TouchButtonEvent().isPressed);
        };

    _signalHandles.push_back(extInterface->Subscribe(
        MessageEngineToGameTag::TouchButtonEvent, countTouchButtonEventCb));
  }
...
class TouchSensorTest : public testing::Test {
 public:
  ...
  void LoadAnnotationsFromFile(
      const std::string& basePath, const std::string& configFile,
    ...
    const std::string fileContents{
        Util::FileUtils::ReadFile(basePath + configFile)};
    ...
    for (const auto& testConfig : configs) {
      const std::string rawFilename =
          JsonTools::ParseString(testConfig, "filename", "");
      ...
      PRINT_NAMED_INFO("TestTouchSensor", "Read %zd values from file (%s)",
                       fileTouchSensorReadings.size(), rawFilepath.c_str());
      ...
      for (const auto& intervalDesc : intervalsJson) {
        ...
        if (tag.compare("idle") == 0) {
          ...
          PRINT_NAMED_ERROR("TestTouchSensor", "Not a valid tag type (%s)",
                            tag.c_str());
          ASSERT_TRUE(false);
        }
      }
...
TEST_F(TouchSensorTest, DISABLED_CalibrateAndClassifyMultirobotTests) {
  ...
  const std::string basePath = cozmoContext->GetDataPlatform()->pathToResource(
      Util::Data::Scope::Resources, kTestResourcesDir);
  ...
  for (int i = 0; i < readingsList.size(); ++i) {
    ...
    std::function<void(void)> testSoftCase = [&]() {
      // const float percentStrokeDetected =
      // (float)_msgMonitor->GetCountTouchButtonEvent()/(anot.softCount);
      // EXPECT_GE(percentStrokeDetected,
      // 0.125-Util::FLOATING_POINT_COMPARISON_TOLERANCE_FLT);
      EXPECT_TRUE(testTouchSensorComponent.IsCalibrated());
    };
...
//
// Outputs:
// + a csv with a line item for each trial (a trial consists of robot+playpen
// attempt)
...
//    and then looks for false positives in a differently conditioned
//    touch-sensor stream (on the same robot).
//
...
TEST_F(TouchSensorTest, DISABLED_AnalyzePlayPenTestsDec17) {
  ...
  // const std::string mfgDataDir =
  // "/Users/arjunm/Code/data/touch_sensor/playpen_dvt1/copy";
  ...
  const auto convertJsonToVector =
      [](const Json::Value& json) -> TouchSensorStream {
    TouchSensorStream ss;

    for (Json::Value::const_iterator it = json.begin(); it != json.end();
         ++it) {
  ...
  std::vector<std::string> jsonFileList =
      Util::FileUtils::FilesInDirectory(mfgDataDir, true, "json", true);

  // remove the following named ones: they were identified in a prior pass as
  // invalid
  {
    ...
    for (auto s : invalidFiles) {
      auto got = std::find_if(
          jsonFileList.begin(), jsonFileList.end(),
  ...
  PRINT_NAMED_INFO("AnalyzePlayPenTests", "Found %zd jsons",
                   jsonFileList.size());
  ...
  for (const auto& jsonFilename : jsonFileList) {
    ...
    allTouchSensorTrialData.push_back(PlaypenTrialEntry{
        .readingsByScenario = {convertJsonToVector(offChrgStat),
                               convertJsonToVector(onChrgStat),
                               convertJsonToVector(offChrgMobl)}});
  }

  PRINT_NAMED_INFO("AnalyzePlayPenTests", "Parsed %zd touch sensor streams",
  ...
  for (const auto& trial : allTouchSensorTrialData) {
    for (uint8_t i = 0; i < EScenarioType::Count; ++i) {
      ...
      DEV_ASSERT(readings.size() >= kMinNumReadingsToCalibrate,
                 "NotEnoughReadingsToCalibrate");
  ...
  const auto scenToString = [](const EScenarioType t) -> std::string {
    const std::string map[EScenarioType::Count] = {"OffChrgSt", "OnChrgSt",
                                                   "OffChrgMob"};
    return map[t];
  };
  ...
  for (int i = 0; i < testTable.size(); ++i) {
    ...
    for (const auto& trial : allTouchSensorTrialData) {
      total++;
      const auto& calibrationReadings =
          trial.readingsByScenario[testTable[i].calibCond];
      const auto& testingReadings =
          trial.readingsByScenario[testTable[i].testCond];
      ...
      // (1) calibrate
      DEV_ASSERT(calibrationReadings.size() >= kMinNumReadingsToCalibrate,
                 "NotEnoughReadingsToCalibrate");
      for (int i = 0; i < kMinNumReadingsToCalibrate; ++i) {
    ...
    const auto pct = [](size_t n, size_t t) -> float { return 100.0f * n / t; };
    ...
    PRINT_NAMED_INFO(
        ...
        scenToString(testTable[i].testCond).c_str(), calibRate, numDidCalib,
        total, noFalsePosRate, numDidCalib - numFalsePositives, numDidCalib);
  }
}
...
TEST_F(TouchSensorTest, DISABLED_AnalyzeManualDataCollection) {
  ...
  const std::string dataDir =
      "/Users/arjunm/Code/data/touch_sensor/manual_dvt1/touchDataCollection/"
      "rename";
  ...
  std::vector<std::string> lines =
      Anki::Util::StringSplit(answerKeyContents, '\n');
  ...
  PRINT_NAMED_INFO("AnalyzeManualDataCollection",
                   "Loaded Answer Key (%zd entries)", answerKey.size());
  ...
  const std::vector<std::string> robotIDs = {
      "1d182069", "1e99ea7b", "1ed88710", "1ed8889d",
      "1f19f8bf", "50156d80", "d3ef4932",
  };

  const std::vector<std::string> motorStates = {"Idle", "Moving"};

  const std::vector<std::string> chargerStates = {"OffCharger", "OnCharger"};

  const std::vector<std::string> touchTypes = {"No_Touch", "Sustained_Contact",
                                               "Petting_Soft", "Petting_Hard"};
  ...
  struct Score {
    ...
    Score operator+(const Score& rhs) const {
      return Score(total + rhs.total, numCalib + rhs.numCalib,
  ...
  // helper lambda
  const auto LoadReadingsFromFile =
    ...
    std::for_each(
        ...
        [&ret](const std::string& s) -> void { ret.push_back(std::stoi(s)); });
    return ret;
  };

  RobotCompMap dummyCompMap;

  typedef function<Score(const vector<int>&, const vector<int>&, const int,
                         const float)>
      TestFuncType;

  TestFuncType MainCalibAndTestScheme =
      [this, &dummyCompMap](const std::vector<int>& calib,
                            const std::vector<int>& test,
                            const int expectedTouchCount,
                            const float minPercentCorrect) -> Score {
    _msgMonitor->ResetCountTouchButtonEvent();
    TouchSensorComponent tscomp;
    tscomp.InitDependent(_robot, dummyCompMap);
    Score result;

    DEV_ASSERT(calib.size() >= kMinNumReadingsToCalibrate,
               "NotEnoughReadingsToCalibrate");
    for (int i = 0; i < kMinNumReadingsToCalibrate; ++i) {
    ...
    if (tscomp.IsCalibrated()) {
      ...
      float percentDetected =
          float(result.numTouchDetected) / expectedTouchCount;
  ...
  const auto getFileKey = [](const string& robot, const string& motorS,
  ...
  for (const auto& cfg : sameMotorChargerStateSkews) {
    ...
    for (const auto& touchType :
         vector<string>{"Petting_Soft", "Petting_Hard"}) {
      ...
      for (const auto& robot : robotIDs) {
        ...
        // calibrate
        std::string calibKey =
            getFileKey(robot, motorState, chargerState, "No_Touch");
        std::vector<int> calib = LoadReadingsFromFile(dataDir + "/" + calibKey);

        // test and score
        std::string testKey =
            getFileKey(robot, motorState, chargerState, touchType);
        ...
        output << "\t" << std::setw(10) << robot << printScore(score_rob)
               << endl;
      }
  ...
  for (const auto& skew : diffMotorChargerStateSkews) {
    ...
    output << "Calib Condition: " << motorStateCalib << " " << chargerStateCalib
           << endl;
    output << "Test Condition: " << motorStateTest << " " << chargerStateTest
           << endl;
    ...
    for (const auto& robot : robotIDs) {
      output << "\tRobot " << robot << " ";
      std::vector<int> calib = LoadReadingsFromFile(
          dataDir + "/" +
          getFileKey(robot, motorStateCalib, chargerStateCalib, "No_Touch"));
      const auto testKey =
          getFileKey(robot, motorStateTest, chargerStateTest, "No_Touch");
      std::vector<int> test = LoadReadingsFromFile(dataDir + "/" + testKey);
...
TEST_F(TouchSensorTest, DISABLED_TouchSensorTuningPVT) {
  ...
  // helper lambda
  const auto LoadReadingsFromFile =
      [](const std::string& filename) -> std::vector<uint16_t> {
    std::string contents{Anki::Util::FileUtils::ReadFile(filename)};
    std::vector<std::string> items = Anki::Util::StringSplit(contents, '\n');
    std::vector<uint16_t> ret;
    std::for_each(items.begin(), items.end(),
                  [&ret](const std::string& s) -> void {
                    ret.push_back((uint16_t)std::stoi(s));
                  });
    return ret;
  };
  ...
  //       - compute the result metrics (accuracy, false positives, extra
  //       detects)
  ...
  std::string workspace =
      "/Users/arjun/Code/data/touch_sensor/pvt_dvt4p5/workspace";
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      for (int fod = 2; fod <= 6; ++fod) {
        for (int fou = 2; fou <= 6; ++fou) {
          for (int mfou = 6; mfou <= 14; ++mfou) {
            for (int mfod = 15; mfod <= 15;
                 mfod += 5) {  // manual testing reveals this is irrelevant
              for (int mccu = 6; mccu <= 6;
                   ++mccu) {  // manual testing reveals this is irrelevant
                skewTable.push_back(TouchSensorComponent::FilterParameters{
                    box, gap, fod, fou, mfod, mfou, mccu});
              }
            }
          }
        }
  ...
  //  });
  PRINT_NAMED_INFO("TouchSensorTuningPVT", "%zd skews created",
                   skewTable.size());
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  std::vector<std::string> robots = {"e1005b", "e20077", "e2014d",
                                     "e200d4", "e20155", "e200ce"};
  ...
  for (auto& robot : robots) {
    SensorReadings sr;
    sr.idleOff_noTouch = LoadReadingsFromFile(workspace + "/" + robot +
                                              "_Idle_OffCharger_No_Touch");
    //    sr.idleOn_noTouch = LoadReadingsFromFile(workspace + "/" + robot +
    //    "_Idle_OnCharger_No_Touch");
    sr.idleOff_pettingSoft = LoadReadingsFromFile(
        workspace + "/" + robot + "_Idle_OffCharger_Petting_Soft");
    //    sr.idleOn_pettingSoft = LoadReadingsFromFile(workspace + "/" + robot +
    //    "_Idle_OnCharger_Petting_Soft");
  ...
  typedef std::function<TestResultScore(
      const SensorReadings&,
      const TouchSensorComponent::FilterParameters& skew)>
      TestFuncType;

  TestFuncType TestMain =
      [this, &dummyCompMap](const SensorReadings& sensorReadings,
                            const TouchSensorComponent::FilterParameters& skew)
      -> TestResultScore {
    _msgMonitor->ResetCountTouchButtonEvent();
    TouchSensorComponent tscomp;
    ...
    const auto applyReadings = [&](const std::vector<uint16_t>& readings) {
      ...
      for (int i = 40; i < last; i++) {  // skip the first 40 readings (noisy
                                         // because of moving on-off charger)
        Anki::Vector::RobotState msg = _robot->GetDefaultRobotState();
    ...
    if (tscomp.IsCalibrated()) {
      ...
      result.numTouchesGot_OffCharger =
          (int)_msgMonitor->GetCountTouchButtonEvent();

      //      _msgMonitor->ResetCountTouchButtonEvent();
      //      applyReadings(sensorReadings.idleOn_pettingSoft);
      //      result.numTouchesGot_OnCharger =
      //      (int)_msgMonitor->GetCountTouchButtonEvent();
  ...
  for (auto& skew : skewTable) {
    ...
    for (auto& readings : readingSet) {
      ...
      if (ret.numTouchesGot_OffCharger >= numTrueTouchesPerInstance) {
        aggExtraDetects +=
            ret.numTouchesGot_OffCharger - numTrueTouchesPerInstance;
      } else {
        aggPctCorrect *=
            float(ret.numTouchesGot_OffCharger) / numTrueTouchesPerInstance;
      }

      //      if(ret.numTouchesGot_OnCharger >= numTrueTouchesPerInstance) {
      //        aggExtraDetects += ret.numTouchesGot_OnCharger -
      //        numTrueTouchesPerInstance;
      //      } else {
      //        aggPctCorrect *=
      //        float(ret.numTouchesGot_OnCharger)/numTrueTouchesPerInstance;
      //      }
    }

    //    printf("Skew: %d %d %d %d %d %d %d =
    //    %d/%4.2f/%d/%d\n",skew.boxFilterSize, skew.dynamicThreshGap,
    //    skew.dynamicThreshDetectFollowOffset,
    //    skew.dynamicThreshUndetectFollowOffset,
    //    skew.dynamicThreshMaximumDetectOffset,
    //    skew.dynamicThreshMininumUndetectOffset,
    //    skew.minConsecCountToUpdateThresholds, allCalib, aggPctCorrect,
    //    aggNumFalsePos, aggExtraDetects);
    ...
    auto durSinceProgressUpdate =
        float(clock() - last_progress_update_time) / CLOCKS_PER_SEC;
