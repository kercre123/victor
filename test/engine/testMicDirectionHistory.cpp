...
TEST(SUITE, MicDirectionHistory_RecentDirection_simple) {
  ...
  newHistory.AddDirectionSample(10, isVadActive, directionFirst, 200,
                                directionFirst);
  ...
  newHistory.AddDirectionSample(20, isVadActive, directionFirst, 200,
                                directionFirst);
  ASSERT_EQ(directionFirst, newHistory.GetRecentDirection(0));
  ASSERT_EQ(directionFirst, newHistory.GetRecentDirection(maxTimeLength_ms));

  // Verify that a higher more recent string of results is detected
  newHistory.AddDirectionSample(30, isVadActive, directionSecond, 200,
                                directionSecond);
  newHistory.AddDirectionSample(40, isVadActive, directionSecond, 200,
                                directionSecond);
  newHistory.AddDirectionSample(50, isVadActive, directionSecond, 200,
                                directionSecond);
  ...
  newHistory.AddDirectionSample(60, isVadActive, directionFirst, 200,
                                directionFirst);
  newHistory.AddDirectionSample(70, isVadActive, directionFirst, 200,
                                directionFirst);
  ASSERT_EQ(directionFirst, newHistory.GetRecentDirection(70));
  ASSERT_EQ(directionFirst, newHistory.GetRecentDirection(maxTimeLength_ms));

  // Verify that looking at just some recent history (not all time) gets
  // expected result
  ASSERT_EQ(directionSecond, newHistory.GetRecentDirection(50));
...
TEST(SUITE, MicDirectionHistory_RecentDirection_complex) {
  ...
  for (uint32_t i = 0; i < historyLen; ++i) {
    if ((t % 20) == 0) {
      newHistory.AddDirectionSample(t, isVadActive, directionFirst, 200,
                                    directionFirst);
    } else {
      newHistory.AddDirectionSample(t, isVadActive, directionSecond, 200,
                                    directionFirst);
    }
    nextT();
  }

  static_assert((MicDirectionHistory::kMicDirectionHistoryLen % 2) == 0,
                "MicDirectionHistory length expected to be even");

  // Since we have an even number of slots, this should end with an equal count
  // of the two input directions, and the first of the two to be checked should
  // be returned
  ASSERT_EQ(directionFirst, newHistory.GetRecentDirection(maxTimeLength_ms));

  // Now lets add in a new direction with a count just greater than the others,
  // and verify it's picked
  for (uint32_t i = 0; i < (historyLen / 2) + 1; ++i) {
    newHistory.AddDirectionSample(nextT(), isVadActive, directionThird, 200,
                                  directionThird);
  }
...
TEST(SUITE, MicDirectionHistory_GetDirectionAtTime) {
  ...
  for (uint32_t i = 0; i < 10; ++i) {
    newHistory.AddDirectionSample(nextT(), isVadActive, directionFirst, 200,
                                  directionFirst);
  }  // ts 10 : 100
  for (uint32_t i = 0; i < 20; ++i) {
    newHistory.AddDirectionSample(nextT(), isVadActive, directionSecond, 200,
                                  directionSecond);
  }  // ts 110 : 300
  ...
  ASSERT_EQ(directionFirst,
            newHistory.GetDirectionAtTime(maxTimeLength_ms, maxTimeLength_ms));

  for (uint32_t i = 0; i < 100; ++i) {
    newHistory.AddDirectionSample(nextT(), isVadActive, directionThird, 200,
                                  directionThird);
  }  // ts 610 : 1600
  ASSERT_EQ(directionThird,
            newHistory.GetDirectionAtTime(maxTimeLength_ms, maxTimeLength_ms));
...
TEST(SUITE, MicDirectionHistory_GetHistoryAtTime) {
  ...
  // History ending at a particular point with a duration included in that node
  // returns only that node
  ...
  // History ending in a node with duration continuing into previous node
  // returns both
