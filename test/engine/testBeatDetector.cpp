...
TEST_F(BeatDetectorComponentTest, OnBeatCallback) {
  int nCallbacks = 0;
  auto callback = [&nCallbacks]() { ++nCallbacks; };

  const auto callbackId =
      _beatDetectorComponent->RegisterOnBeatCallback(callback);
  ...
  const bool result =
      _beatDetectorComponent->UnregisterOnBeatCallback(callbackId);
...
TEST_F(BeatDetectorComponentTest, DISABLED_PossibleBeatDetection) {
  ...
  ASSERT_FALSE(_beatDetectorComponent->IsPossibleBeatDetected())
      << "Unexpected possible beat detected";
  ASSERT_FALSE(_beatDetectorComponent->IsBeatDetected())
      << "Unexpected beat detected";

  // Note: Using UniversalTime is a bit hairy here, since it will use the system
  // time. But within a reasonable margin this should be fine.
  const float now_sec =
  ...
  int nFakeBeats = 0;
  for (float beatTime = now_sec - startTimeAgo_sec; beatTime < now_sec;
       beatTime += 60.f / kFakeTempo_bpm) {
    const auto& fakeBeat = _beatDetectorComponent->TEST_fakeLowConfidenceBeat(
        kFakeTempo_bpm, beatTime);
    _beatDetectorComponent->OnBeat(fakeBeat);
    ++nFakeBeats;
  }

  // Updating the beat detector component should cull the list to a more recent
  // window
  Update(_beatDetectorComponent);

  ASSERT_LT(_beatDetectorComponent->_recentBeats.size(), nFakeBeats)
      << "Expected beats to be culled from list";
  ...
  // If we replace the latest beat to have a high confidence, we should have a
  // possible beat
  auto& lastBeat = _beatDetectorComponent->_recentBeats.back();
  lastBeat = _beatDetectorComponent->TEST_fakeHighConfidenceBeat(
      lastBeat.tempo_bpm, lastBeat.time_sec);

  ASSERT_TRUE(_beatDetectorComponent->IsPossibleBeatDetected())
      << "Expected possible beat detected";
  ASSERT_FALSE(_beatDetectorComponent->IsBeatDetected())
      << "Unexpected beat detected";

  // If the latest beat has a _very_ high confidence, we should also have a
  // normal beat detected
  lastBeat.confidence = 10000.f;

  ASSERT_TRUE(_beatDetectorComponent->IsPossibleBeatDetected())
      << "Expected possible beat detected";
  ASSERT_TRUE(_beatDetectorComponent->IsBeatDetected())
      << "Expected beat detected";

  // If we modify the latest beat with a different tempo, no beat should be
  // detected
...
TEST_F(BeatDetectorComponentTest, DISABLED_BeatDetection) {
  ...
  for (float beatTime = now_sec - startTimeAgo_sec; beatTime < now_sec;
       beatTime += 60.f / kFakeTempo_bpm) {
    const auto& fakeBeat = _beatDetectorComponent->TEST_fakeHighConfidenceBeat(
        kFakeTempo_bpm, beatTime);
    _beatDetectorComponent->OnBeat(fakeBeat);
    ++nFakeBeats;
  }
