...
// Increase this (e.g. to max int) in order to run longer tests using all the
// available video frames The limit is here to keep the test time shorter for
// general use.
...
static void Recognize(Robot& robot, RobotTimeStamp_t timestamp,
                      RobotState& stateMsg, Vision::Image& img,
                      const std::string& filename, const char* dispName,
                      const std::set<std::string>& namesPresent) {
  ...
  if (SHOW_IMAGES) {
    ...
    for (auto& face : faces) {
      ...
      for (s32 iFeature = 0; iFeature < (s32)Vision::TrackedFace::NumFeatures;
        ...
        const Vision::TrackedFace::Feature& feature =
            face.GetFeature(featureName);

        for (size_t crntPoint = 0, nextPoint = 1; nextPoint < feature.size();
             ++crntPoint, ++nextPoint) {
          dispImg.DrawLine(feature[crntPoint], feature[nextPoint], drawColor,
                           1);
        }
      }
    }

    dispImg.DrawText(Point2f(1.f, dispImg.GetNumRows() - 1),
                     std::to_string(img.GetTimestamp()), NamedColors::RED, 0.6f,
                     true);
...
Result Enroll(Robot& robot, RobotTimeStamp_t& fakeTime, RobotState& stateMsg,
              Vision::Image& img, const std::string& dataPath,
  ...
  auto enrollFiles = Util::FileUtils::FilesInDirectory(
      Util::FileUtils::FullFilePath({dataPath, userDir, enrollSubDir}), true,
      imageFileExtensions);
  ...
  const TimeStamp_t kEnrollTimeInc =
      1500;  // Pretend there's this much time b/w each image
  ...
  for (s32 iFile = 0; iFile < numFiles; ++iFile, fakeTime += kEnrollTimeInc) {
    ...
    Recognize(robot, fakeTime, stateMsg, img, enrollFile, "EnrollImage",
              {userDir});
    ...
    if (observedFaceIDs.size() > 1) {
      PRINT_NAMED_WARNING(
          "TestFaceRecognition.Enroll.SkippingEnrollImageWithMultipleFaces",
    ...
    if (observedID >= 0) {
      if (Vision::UnknownFaceID == enrollmentID) {
        ...
        robot.GetVisionComponent().SetFaceEnrollmentMode(
            enrollmentID, (int)Vision::FaceRecognitionConstants::
                              MaxNumEnrollDataPerAlbumEntry);

      } else {
        auto enrollingFace = robot.GetFaceWorld().GetFace(enrollmentID);
        if (nullptr != enrollingFace &&
            enrollingFace->GetNumEnrollments() > 0) {
          EXPECT_EQ((int)Vision::FaceRecognitionConstants::
                        MaxNumEnrollDataPerAlbumEntry,
                    enrollingFace->GetNumEnrollments());

          robot.GetVisionComponent().AssignNameToFace(enrollmentID, userDir);
          PRINT_NAMED_INFO(
              "FaceRecognition.VideoRecognitionAndTracking.AddingName",
...
Result ShowBlankFrames(s32 N, Robot& robot, RobotTimeStamp_t& fakeTime,
                       RobotState& stateMsg, Vision::Image& img,
                       const char* dispName) {
  Result lastResult = RESULT_OK;
  ...
  for (s32 iBlank = 0; iBlank < N; ++iBlank, fakeTime += kBlankTimeInc) {
    Recognize(robot, fakeTime, stateMsg, img, "", dispName,
              std::set<std::string>());
    ...
    if (iBlank >= 2) {
      auto observedFaceIDs =
          robot.GetFaceWorld().GetFaceIDs(img.GetTimestamp());
      if (!observedFaceIDs.empty()) {
...
TEST(FaceRecognition, VideoRecognitionAndTracking) {
  ...
  if (false == Vision::FaceTracker::IsRecognitionSupported()) {
    PRINT_NAMED_ERROR(
        "FaceRecognition.VideoRecognitionAndTracking.RecognitionNotSupported",
        "Skipping test because the compiled face tracker does not support "
        "recognition");
    return;
  }

  const std::string dataPath = cozmoContext->GetDataPlatform()->pathToResource(
      Util::Data::Scope::Resources, "test/faceRecVideoTests/");
  ...
  std::vector<TestDirData> testDirData = {
      ...
      //{.dirName = "girl3",        .isForTraining = true,    .names =
      //{"girl3"}},
  ...
  Json::Value& config =
      cozmoContext->GetDataLoader()->GetRobotVisionConfigUpdatableRef();
  ...
  dependentComps.AddDependentComponent(RobotComponentID::CozmoContextWrapper,
                                       new ContextWrapper(cozmoContext));
  for (s32 iReload = 0; iReload < 2; ++iReload) {
    ...
    robot.GetVisionScheduleMediator().DevOnly_SelfSubscribeVisionMode(
        {VisionMode::Faces});
    ...
    if (iReload == 0) {
      ...
      for (auto const& test : testDirData) {
        if (test.isForTraining) {
          ...
          lastResult =
              Enroll(robot, fakeTime, stateMsg, img, dataPath, test.dirName);
          ASSERT_EQ(RESULT_OK, lastResult);

          lastResult = ShowBlankFrames(NumBlankFramesBetweenPeople, robot,
                                       fakeTime, stateMsg, img, "EnrollImage");
    ...
    if (iReload > 0) {
      ...
      Result loadResult = robot.GetVisionComponent().LoadFaceAlbumFromFile(
          "testAlbum", loadedFaces);
    ...
    for (auto& test : testDirData) {
      ...
      while (Util::FileUtils::DirectoryExists(clipPath)) {
        ...
        for (s32 iFile = 0; iFile < numFiles;
             ++iFile, fakeTime += kTestTimeInc) {
          const std::string& testFile = testFiles[iFile];

          Recognize(robot, fakeTime, stateMsg, img, testFile, "TestImage",
                    test.names);
          stats.totalFrames++;
        ...
        const f32 detectionPercent =
            (f32)(stats.facesDetected * 100) /
            (f32)(stats.totalFrames * test.names.size());
        const f32 recogPercent =
            (f32)(stats.facesRecognized * 100) / (f32)stats.facesDetected;
        const f32 falsePosPercent =
            (f32)(stats.falsePositives * 100) / (f32)stats.facesDetected;

        PRINT_NAMED_INFO(
            "FaceRecognition.VideoRecognitionAndTracking.Stats",
            "%s: %d images, %d faces detected (%.1f%%), %d faces recognized "
            "(%.1f%%), %d false positives (%.1f%%), %d total IDs assigned",
            testDir, stats.totalFrames, stats.facesDetected, detectionPercent,
            stats.facesRecognized, recogPercent, stats.falsePositives,
            falsePosPercent, stats.numFaceIDs);
        ...
        // Show the system blank frames before next video so it stops tracking
        lastResult = ShowBlankFrames(NumBlankFramesBetweenPeople, robot,
                                     fakeTime, stateMsg, img, "TestImage");
        EXPECT_EQ(RESULT_OK, lastResult);

        ++clipCtr;
        clipPath = Util::FileUtils::FullFilePath(
            {dataPath, testDir, "clip" + std::to_string(clipCtr)});
      }  // while clipDir exists

      PRINT_NAMED_INFO("FaceRecognition.VideoRecognitionAndTracking.ClipStats",
                       "Found face in %d of %d clips (%.1f%%)", clipsSucceeded,
                       clipCtr - 1,
                       100.f * (f32)clipsSucceeded / (f32)(clipCtr - 1));
    ...
    Result saveResult =
        robot.GetVisionComponent().SaveFaceAlbumToFile("testAlbum");
