 ...
 * Created: 6/17/16
 *
 * Description: Configure an audio animation scenario by inserting each audio
 *event. Generate Animation & RobotAudioBuffer for given config.
 *
...
class AnimationAudioTestConfig {
 public:
  ...
  struct TestAudioEvent {
    Anki::AudioMetaData::GameEvent::GenericEvent event =
        Anki::AudioMetaData::GameEvent::GenericEvent::Invalid;
    uint32_t startTime_ms = 0;
    uint32_t duration_ms = 0;
    TestAudioEvent(Anki::AudioMetaData::GameEvent::GenericEvent event,
                   uint32_t startTime_ms, uint32_t duration_ms)
        : event(event), startTime_ms(startTime_ms), duration_ms(duration_ms) {}

    bool InAnimationTimeRange(uint32_t animationTime_ms) {
      return (animationTime_ms >= startTime_ms) &&
             (animationTime_ms < GetCompletionTime_ms());
    }

    uint32_t GetCompletionTime_ms() const { return startTime_ms + duration_ms; }

    // TODO: Add methods to offset actual buffer creation to simulate the buffer
    // not perfectly lining up with desired
    //       situation.
  };
  ...
  std::vector<TestAudioEvent> FrameAudioEvents(const int32_t frameStartTime_ms,
                                               const int32_t frameEndTime_ms);

  std::vector<TestAudioEvent> GetCurrentPlayingEvents(
      const int32_t frameStartTime_ms, const int32_t frameEndTime_ms,
      const uint32_t framedrift_ms);
  ...
  void LoadStreamingAnimation(
      Anki::Vector::RobotAnimation::StreamingAnimationTest&
          out_streamingAnimation);
