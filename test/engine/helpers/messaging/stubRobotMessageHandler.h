...
class StubMessageHandler : public RobotInterface::MessageHandler {
 public:
  virtual void Init(const Json::Value& config, RobotManager* robotMgr,
                    const CozmoContext* context) override {
    // do nothing
  }

  virtual Result ProcessMessages() override { return RESULT_OK; }

  virtual Result SendMessage(const RobotInterface::EngineToRobot& msg,
                             bool reliable = true, bool hot = false) override {
    _msgsToRobot.push_back(msg);
    // always succeed
    return Result::RESULT_OK;
  }

  // return true if there's an execute path message in the outgoing queue, and
  // set the path id in arguments
  ...
  const std::vector<RobotInterface::EngineToRobot>& GetMsgsToRobot() const {
    return _msgsToRobot;
  }

  // Message storage grows without bounds, so it's a good idea to clear out from
  // time to time. This functions will clear all saved messages from engine to
  // robot
  void ClearMsgsToRobot() { _msgsToRobot.clear(); }

 private:
  // Holds all messages sent from engine to the robot.
  // NOTE: this grows without bounds, so if you have a long-running test, you
  // should clear it periodically
