...
// TODO refactor this because it's also in blockworld unit tests. This should be
// a helper
ObservableObject* CreateObjectLocatedAtOrigin(Robot& robot,
  ...
  objectPtr->InitPose(
      originPose, Anki::PoseState::Known);  // posestate could be something else
  ...
  ANKI_VERIFY(objectInWorld == objectPtr,
              "CreateObjectLocatedAtOrigin.NotSameObject", "");
  ANKI_VERIFY(nullptr != objectInWorld,
...
TEST(ObjectInteractionInfoCache, BestObjectConsistency) {
  ...
  ObservableObject* object1 =
      CreateObjectLocatedAtOrigin(robot, ObjectType::Block_LIGHTCUBE1);
  ASSERT_TRUE(nullptr != object1);
  objID1 = object1->GetID();

  ObservableObject* object2 =
      CreateObjectLocatedAtOrigin(robot, ObjectType::Block_LIGHTCUBE2);
  ASSERT_TRUE(nullptr != object2);
  objID2 = object2->GetID();

  const int xOffsetObj1 = 100;

  // put the first cube close to the robot, and put the second cube much further
  // away
  {
    const Pose3d obj1Pose(0.0f, Z_AXIS_3D(), {xOffsetObj1, 0, 0},
                          robot.GetPose());
    auto result =
        robot.GetBlockWorld().SetObjectPose(objID1, obj1Pose, PoseState::Known);
    ASSERT_EQ(RESULT_OK, result);
  }
  {
    const Pose3d obj2Pose(0.0f, Z_AXIS_3D(), {xOffsetObj1 * 100, 0, 0},
                          robot.GetPose());
    auto result =
        robot.GetBlockWorld().SetObjectPose(objID2, obj2Pose, PoseState::Known);
    ASSERT_EQ(RESULT_OK, result);
  }

  static const float arbitrary_time_increase = 100000000.0f;
  static float incrementEngineTime_ns =
  ...
  // Use pickup object - details should not be important for this test, we're
  // looking to test default expected behavior
  ObjectInteractionIntention intent =
      ObjectInteractionIntention::PickUpObjectNoAxisCheck;
  ObjectInteractionInfoCache& objCache =
      robot.GetAIComponent().GetComponent<ObjectInteractionInfoCache>();
  ...
  {
    const Pose3d obj2Pose(0.0f, Z_AXIS_3D(), {xOffsetObj1 / 2, 0, 0},
                          robot.GetPose());
    auto result =
        robot.GetBlockWorld().SetObjectPose(objID2, obj2Pose, PoseState::Known);
    ASSERT_EQ(RESULT_OK, result);
