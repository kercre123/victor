...
TEST(AnimationTest, AnimationLoading) {
  ...
  const auto fullPath =
      cozmoContext->GetDataPlatform()->GetResourcePath(kPathToDevData);
  ...
  auto spriteSequenceContainer =
      robot.GetComponent<DataAccessorComponent>().GetSpriteSequenceContainer();
  ...
  // Load JSON animations
  {
    ...
    auto jsonFilePaths = Util::FileUtils::FilesInDirectory(
        fullPath, useFullPath, jsonExt, shouldRecurse);
  ...
  CannedAnimationContainer binaryAnimContainer;
  {
    ...
    auto binaryFilePaths = Util::FileUtils::FilesInDirectory(
        fullPath, useFullPath, binExt, shouldRecurse);
  ...
  if (!sameSize) {
    PRINT_NAMED_WARNING(
        "AnimationTest.AnimationLoading.SizesDoNotMatch",
    ...
    for (const auto& name : jsonAnimNames) {
      const auto it =
          std::find(binaryAnimNames.begin(), binaryAnimNames.end(), name);
    ...
    for (const auto& name : binaryAnimNames) {
      const auto it =
          std::find(jsonAnimNames.begin(), jsonAnimNames.end(), name);
  ...
  for (const auto& name : binaryAnimNames) {
    ...
    if (!animExists) {
      PRINT_NAMED_WARNING(
          "AnimationTest.AnimationLoading.MissingJSONAnim - may be an issue "
          "copying assets for test",
          "JSON container does not have animation %s", name.c_str());
      continue;
    }
...
// Ensure that copying animations/keyframes doesn't result in any lost
// information
