...
// Single origin for all the poses here to use, which will not destruct before
// anything that uses it
const Anki::Pose3d origin(0, Anki::Z_AXIS_3D(), {0, 0, 0}, "Origin");

const Anki::Vector::ProxSensorData proxSensorValid = {
    .distance_mm = 100, .signalQuality = 10, .isLiftInFOV = false};

const Anki::Vector::ProxSensorData proxSensorNotValid = {
    .distance_mm = 100, .signalQuality = 10, .isLiftInFOV = true};

const uint8_t frontCliffDetectedFlags =
    (1 << Anki::Util::EnumToUnderlying(Anki::Vector::CliffSensor::CLIFF_FL)) |
...
TEST(RobotStateHistory, AddGetPose) {
  ...
  auto WasStateCarrying = [](const RobotState &state) -> bool {
    return (state.status &
            Util::EnumToUnderlying(RobotStatusFlag::IS_CARRYING_BLOCK));
  };
  ...
  ASSERT_TRUE(histState.GetProxSensorData().foundObject ==
              histState2.GetProxSensorData().foundObject);
  for (int i = 0; i < Util::EnumToUnderlying(CliffSensor::CLIFF_COUNT); ++i) {
    CliffSensor sensor = static_cast<CliffSensor>(i);
    ASSERT_TRUE(histState.WasCliffDetected(sensor) ==
                histState2.WasCliffDetected(sensor));
  }
...
TEST(RobotStateHistory, GroundTruthPose) {
  ...
  const Pose3d p3(M_PI_2_F - 0.25 * M_PI_F, Z_AXIS_3D(),
                  Vec3f(1 - sqrtf(2), 2, 0), origin);
  ...
  */
  ASSERT_TRUE(
      histState.GetPose().IsSameAs(p3, DIST_EQ_THRESH, ANGLE_EQ_THRESH));
  ...
  */
  ...
  // 3) Now inserting the same ground truth pose again but with a higher frame
  // id
  histState.SetPose(frameID + 1, p1, h1, l1);
  hist.AddVisionOnlyState(t2, histState);

  // Requested pose at t3 should be pose p1 modified by the pose diff between p2
  // and p3
  ASSERT_TRUE(hist.ComputeStateAt(t3, t, histState) == RESULT_OK);

  ASSERT_TRUE(histState.GetPose().IsSameAs(p1_by_p2Top3, DIST_EQ_THRESH,
                                           ANGLE_EQ_THRESH));
...
TEST(RobotStateHistory, CullToWindowSizeTest) {
  ...
  for (TimeStamp_t t = 0; t < 6000; t += 100) {
    ...
    // Don't add any visStates so as to test possible bad erase conditions in
    // CullToWindowSize()
  ...
  printf(
      "CullToWindowSizeTest: raw %zu, vis %zu, comp %zu, keyByTs %zu, tsByKey "
      "%zu\n",
      hist._states.size(), hist._visStates.size(), hist._computedStates.size(),
      hist._keyByTsMap.size(), hist._tsByKeyMap.size());
