...
TEST(VisionSystem, DISABLED_CameraCalibrationTarget_InvertedBox) {
  ...
  Anki::Vector::VisionSystem* visionSystem =
      new Anki::Vector::VisionSystem(cozmoContext);
  Anki::Result result =
      visionSystem->Init(cozmoContext->GetDataLoader()->GetRobotVisionConfig());
  ASSERT_EQ(Anki::Result::RESULT_OK, result);

  // Don't really need a valid camera calibration, so just pass a dummy one in
  // to make vision system happy. All that matters is the image dimensions be
  // correct.
  std::shared_ptr<Anki::Vision::CameraCalibration> calib(
      new Anki::Vision::CameraCalibration(360, 640, 290, 290, 320, 180, 0.f));
  ...
  std::string testImgPath = cozmoContext->GetDataPlatform()->pathToResource(
  ...
  // When this test was originally written, marker detector would run on the
  // same sized image that ImageCache was reset with. This is no longer the
  // case. Now ImageCache is reset with a Full sized image and marker detector
  // always runs at half the original image's resolution so we need to scale
  // these images by 2.
  ...
  ASSERT_EQ(
      processingResult.cameraCalibration.front().GetFocalLength_x() != 1.f,
      true);

  const std::vector<f32> distortionCoeffs = {
      {-0.06456808353003426, -0.2702951616534774, 0.001409446798192025,
       0.001778340478064528, 0.1779613197923669, 0, 0, 0}};

  const Anki::Vision::CameraCalibration expectedCalibration(
      360, 640, 372.328857, 368.344482, 306.370270, 185.576843, 0,
      distortionCoeffs);

  const auto& computedCalibration = processingResult.cameraCalibration.front();

  ASSERT_NEAR(computedCalibration.GetCenter_x(),
              expectedCalibration.GetCenter_x(),
              Anki::Util::FLOATING_POINT_COMPARISON_TOLERANCE_FLT);

  ASSERT_NEAR(computedCalibration.GetCenter_y(),
              expectedCalibration.GetCenter_y(),
              Anki::Util::FLOATING_POINT_COMPARISON_TOLERANCE_FLT);

  ASSERT_NEAR(computedCalibration.GetFocalLength_x(),
              expectedCalibration.GetFocalLength_x(),
              Anki::Util::FLOATING_POINT_COMPARISON_TOLERANCE_FLT);

  ASSERT_NEAR(computedCalibration.GetFocalLength_y(),
              expectedCalibration.GetFocalLength_y(),
              Anki::Util::FLOATING_POINT_COMPARISON_TOLERANCE_FLT);
  ...
  ASSERT_NEAR(computedCalibration.GetDistortionCoeffs().size(),
              expectedCalibration.GetDistortionCoeffs().size(), 0);

  for (int i = 0; i < expectedCalibration.GetDistortionCoeffs().size(); ++i) {
    ASSERT_NEAR(computedCalibration.GetDistortionCoeffs()[i],
                expectedCalibration.GetDistortionCoeffs()[i],
                Anki::Util::FLOATING_POINT_COMPARISON_TOLERANCE_FLT);
  }
...
TEST(VisionSystem, ImageCompositing_MarkerDetection) {
  ...
  const u32 imageReadyPeriod = JsonTools::ParseUInt32(
      cfg["ImageCompositing"], "imageReadyPeriod", kDebugName);
  const u32 imageResetPeriod =
      imageReadyPeriod *
      JsonTools::ParseUInt32(cfg["ImageCompositing"],
                             "numImageReadyCyclesBeforeReset", kDebugName);
  ASSERT_EQ(RESULT_OK, result);

  // Don't really need a valid camera calibration, so just pass a dummy one in
  // to make vision system happy.
  auto calib = std::make_shared<Vision::CameraCalibration>(
      360, 640, 290.f, 290.f, 160.f, 120.f, 0.f);
  result = visionSystem.UpdateCameraCalibration(calib);
  ASSERT_EQ(RESULT_OK, result);

  const std::string testImageDir =
  ...
  const auto isChargerDetected =
      [](const std::list<Vision::ObservedMarker>& markers) -> bool {
    return !markers.empty() && strncmp("MARKER_CHARGER_HOME",
                                       markers.front().GetCodeName(), 19) == 0;
  };

  // Each test case is a collection of images at a given ambient lighting level
  // (given by 2nd folder name),
  //  and a given LCD brightness (given by the 3rd folder name).
  // For example:
  //  "light1p75" is at light level 1.75 (using an arbitrary scale marked on a
  //  hardware dimmer switch). "1" refers to LCD brightness level =20. Other
  //  values this can be are "0" which maps to LCD=1
  std::vector<std::string> testCaseSubDirs = {
      "light1p75/1",

      "light2p0/0",  "light2p0/1",

      "light2p25/0", "light2p25/1",

      "light2p5/0",  "light2p5/1",
  };

  for (const auto& subdir : testCaseSubDirs) {
    const std::vector<std::string> testFiles =
        Util::FileUtils::FilesInDirectory(
            Util::FileUtils::FullFilePath({testImageDir, subdir}), false,
            ".jpg");
    PRINT_CH_DEBUG("VisionSystem", kDebugName, "Loaded %zu images (%s)",
                   testFiles.size(), subdir.c_str());

    DEV_ASSERT_MSG((testFiles.size() >= imageResetPeriod), kDebugName,
                   "Not enough images for one reset period.");
    ...
    for (size_t index = 0; index < testFiles.size(); ++index) {
      ...
      Vision::Image img;
      result = img.Load(
          Util::FileUtils::FullFilePath({testImageDir, subdir, filename}));
      ASSERT_EQ(RESULT_OK, result);
      imageCache.Reset(img);
      ...
      input.modesToProcess.Insert(
          Anki::Vector::VisionMode::Markers_FastRotation);
      ...
      robotState.cameraPose.SetParent(
          robotState.histState
              .GetPose());  // just so we don't trigger an assert
      ...
      countCompositeImagesProcessed += processingResult.modesProcessed.Contains(
          Vector::VisionMode::Markers_Composite);
      if (index == 0) {  // first image
        // Ensure no images are found in a base image (thus requiring
        // compositing to detect a marker)
...
#define DEBUG_DISPLAY DISABLED
  ...
  // NOTE: We don't just use a MarkerDetector here because the VisionSystem also
  // does CLAHE preprocessing which
  ...
  // correct.
  auto calib = std::make_shared<Vision::CameraCalibration>(
      240, 320, 290.f, 290.f, 160.f, 120.f, 0.f);
  result = visionSystem.UpdateCameraCalibration(calib);
  ASSERT_EQ(RESULT_OK, result);
  ...
  const std::string testImageDir =
      cozmoContext->GetDataPlatform()->pathToResource(
          Util::Data::Scope::Resources, "test/markerDetectionTests");

  struct TestDefinition {
    std::string subDir;
    f32 expectedFailureRate;
    std::function<bool(const std::list<Vision::ObservedMarker>&,
                       const std::string& filename)>
        didSucceedFcn;
  };

  // Helper to check success by verifying exactly one marker was detected and
  // its name matches the filename (ignoring trailing number)
  auto matchFilenameFcn = [](const std::list<Vision::ObservedMarker>& markers,
  ...
  std::vector<TestDefinition> testDefinitions = {
      TestDefinition{
          ...
          .didSucceedFcn = [](const std::list<Vision::ObservedMarker>& markers,
      ...
      TestDefinition{
          ...
          .didSucceedFcn = [](const std::list<Vision::ObservedMarker>& markers,
            ...
            return (markers.size() == 1) &&
                   (strncmp("MARKER_CHARGER_HOME",
                            markers.front().GetCodeName(), 19) == 0);
          }},

  };

  const std::vector<std::string> markerDirs = {"MARKER_CHARGER_HOME",
  ...
  for (auto const& markerDir : markerDirs) {
    testDefinitions.push_back(TestDefinition{
        ...
        .didSucceedFcn = [&markerDir](
                             const std::list<Vision::ObservedMarker>& markers,
                             const std::string& filename) -> bool {
          return (markers.size() == 1) &&
                 (markerDir.compare(0, markerDir.length(),
                                    markers.front().GetCodeName()) == 0);
        }});
  }
  ...
  for (auto& testDefinition : testDefinitions) {
    ...
    std::vector<std::string> testFiles = Util::FileUtils::FilesInDirectory(
        Util::FileUtils::FullFilePath({testImageDir, subDir}), false, ".jpg");
    const std::vector<std::string> testFiles_png =
        Util::FileUtils::FilesInDirectory(
            Util::FileUtils::FullFilePath({testImageDir, subDir}), false,
            ".png");
    std::copy(testFiles_png.begin(), testFiles_png.end(),
              std::back_inserter(testFiles));
    ...
    for (auto& filename : testFiles) {
      ...
      // When this test was originally written, marker detector would run on the
      // same sized image that ImageCache was reset with. This is no longer the
      // case. Now ImageCache is reset with a Full sized image and marker
      // detector always runs at half the original image's resolution so we need
      // to scale these images by 2. Note: This resizes with Cubic instead of
      // Linear because the tests fail when the images are resized with Linear
      ...
      // For now, the measure of "success" for an image is if we detected at
      // least one marker in it. We are not checking whether the marker's type
      // or position is correct.
      const bool success = testDefinition.didSucceedFcn(
          processingResult.observedMarkers, filename);
      if (!success) {
        ++numFailures;
      }

      if (DEBUG_DISPLAY != DISABLED &&
          ((DEBUG_DISPLAY != ENABLE_ON_FAIL) || !success)) {
        ...
        for (auto const& marker : processingResult.observedMarkers) {
          ...
          dispImg.DrawText(textPoint, markerName, NamedColors::YELLOW, 0.5f,
                           true);
        }
        auto meanVal = cv::mean(img.get_CvMat_());
        dispImg.DrawText(Point2f(1, 9),
                         "mean: " + std::to_string((u8)meanVal[0]),
                         NamedColors::RED, 0.4f, true);
    ...
    PRINT_NAMED_INFO("VisionSystem.MarkerDetectionTests",
                     "%s: %.1f%% failures (%u of %zu)", subDir.c_str(),
...
TEST(VisionSystem, ImageQuality) {
  ...
  const std::string testImageDir =
      cozmoContext->GetDataPlatform()->pathToResource(
          Util::Data::Scope::Resources, "test/imageQualityTests");

  const std::vector<std::string> testSubDirs = {"Good", "TooBright", "TooDark"};
  ...
  const std::vector<Test> tests = {
      {.subDir = "Good", .exposureTime_ms = 31, .exposureGain = 2.f},
      {
          .subDir = "TooBright",
  ...
  for (auto& test : tests) {
    const std::string fullFileName =
        Util::FileUtils::FullFilePath({testImageDir, test.subDir});
    const std::vector<std::string> testFiles =
        Util::FileUtils::FilesInDirectory(fullFileName, false, ".jpg");

    for (auto& filename : testFiles) {
      // Fake the exposure parameters so that we are always against the extremes
      // in order to trigger TooDark and TooBright
      const Vector::VisionSystem::GammaCurve gammaCurve{};
      result = visionSystem.SetCameraExposureParams(
          test.exposureTime_ms, test.exposureGain, gammaCurve);
      ASSERT_EQ(RESULT_OK, result);

      Vision::ImageRGB img;
      result = img.Load(
          Util::FileUtils::FullFilePath({testImageDir, test.subDir, filename}));
      ASSERT_EQ(RESULT_OK, result);
      ...
      PRINT_NAMED_INFO("VisionSystem.ImageQuality", "%s = %s", filename.c_str(),
                       EnumToString(processingResult.imageQuality));
...
GTEST_TEST(LaserPointDetector, LaserDetect) {
  ...
  // This unit test does not test the "real" Detect function, only the one that
  // doesn't use the robot pose. This function is only used by this test. The
  // important change here is to get the robot's head angle and the
  // corresponding homography and test the full Detect metod.
...
#define DEBUG_LASER_DISPLAY false
  ...
  auto doTest = [](const std::string& imageName, int expectedPoints) {
    PRINT_NAMED_INFO("LaserPointDetector.LaserDetect", "Testing image %s",
                     imageName.c_str());
  ...
  // True positives
  {
    ...
    const std::vector<std::string>& files =
        Util::FileUtils::FilesInDirectory(testImageDir, true, ".jpg", false);
...
GTEST_TEST(NeuralNets, InitFromConfig) {
  ...
  const Json::Value& config =
      cozmoContext->GetDataLoader()->GetRobotVisionConfig();
  ...
  const Json::Value& allModelsConfig =
      neuralNetConfig[NeuralNets::JsonKeys::Models];

  const std::string visionConfigPath =
      Util::FileUtils::FullFilePath({"config", "engine", "vision"});
  const std::string dataPath = cozmoContext->GetDataPlatform()->pathToResource(
      Util::Data::Scope::Resources, visionConfigPath);
  const std::string cachePath = cozmoContext->GetDataPlatform()->pathToResource(
  ...
  // Make sure "NeuralNetRunner" load succeeds for each model, given all the
  // current params in vision_config.json
  ...
  for (const auto& modelConfig : allModelsConfig) {
    ...
    const Result loadRunnerResult =
        neuralNetRunner.Init(dnnCachePath, modelConfig);
    ASSERT_EQ(RESULT_OK, loadRunnerResult);

    // Check that the model file exists and is a .tflite model (unless this is
    // an "offboard" model definition)
    ASSERT_TRUE(modelConfig.isMember(NeuralNets::JsonKeys::ModelType));
    const std::string modelTypeStr =
        modelConfig[NeuralNets::JsonKeys::ModelType].asString();
    if (modelTypeStr != NeuralNets::JsonKeys::OffboardModelType) {
      ...
      const std::string modelFileName =
          modelConfig[NeuralNets::JsonKeys::GraphFile].asString();
      ...
      const std::string extension =
          modelFileName.substr(extIndex, std::string::npos);
      ASSERT_EQ(".tflite",
                extension);  // TODO: make this depend on which neural net
                             // platform we're building with?

      // Make sure model file exists
      const std::string fullModelPath =
          Util::FileUtils::FullFilePath({modelPath, modelFileName});
...
GTEST_TEST(VisionModeSet, BasicFunctionality) {
  ...
  VisionModeSet set3{VisionMode::Markers, VisionMode::Motion,
                     VisionMode::Faces};
  ...
  const std::list<VisionMode> listOfModes{VisionMode::Faces, VisionMode::Motion,
                                          VisionMode::Illumination};

  set1.Insert(listOfModes);
  ASSERT_EQ(listOfModes.size(), set1.size());
  std::for_each(listOfModes.begin(), listOfModes.end(),
                [&set1](VisionMode mode) { ASSERT_TRUE(set1.Contains(mode)); });
  ...
  // NOTE: This will not compile because a container of VisionMode types is
  // static_asserted
