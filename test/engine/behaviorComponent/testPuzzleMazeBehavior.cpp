 ...
 * Created: 2017-11-28
 *
 * Description: Unit tests and simulator for how fast cozmo runs the maze with
 *different settings.
 *
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - -
void CreatePuzzleMazeBehavior(
    Robot& robot, ICozmoBehaviorPtr& puzzleMazeBehavior,
    BehaviorExternalInterface& behaviorExternalInterface,
    const Anki::Util::Data::DataPlatform& dataPlatform) {
  ASSERT_TRUE(puzzleMazeBehavior == nullptr)
      << "test bug: should not have behavior yet";

  Json::Value jsonRoot;
  dataPlatform.readAsJson(Util::Data::Scope::Resources,
                          "config/engine/behaviorComponent/behaviors/freeplay/"
                          "userInteractive/puzzleMaze.json",
                          jsonRoot);
...
float GetTimeForCurrentPuzzle(
    BehaviorPuzzleMaze* puzzleMazePtr, AIComponent& aiComponent, Robot& robot,
    BehaviorExternalInterface& behaviorExternalInterface) {
  ...
  // Expect every puzzle is solvable in under max iterations. This is the
  // actually reasonable unit tests part.
...
void ConvertSecondsToMinutes(const float totalSeconds, int& outMinutes,
                             int& outSeconds) {
...
TEST(PuzzleMazeBehavior, DISABLED_BalanceTool) {
  // This isn't really a unit test, we just needed a way to show time it took to
  // solve puzzles. Just run through the test N times and print the average of
  // each puzzle.
  ...
  Anki::Util::Data::DataPlatform dataPlatform(persistentPath, cachePath,
                                              resourcePath);
  ...
  CreatePuzzleMazeBehavior(robot, puzzleMazeBehavior, behaviorExternalInterface,
                           dataPlatform);

  BehaviorPuzzleMaze* puzzleMazePtr =
      (BehaviorPuzzleMaze*)puzzleMazeBehavior.get();
  ...
  for (int i = 0; i < numPuzzles; ++i) {
    ...
    for (int j = 0; j < maxRuns; ++j) {
      float time = GetTimeForCurrentPuzzle(puzzleMazePtr, aiComponent, robot,
  ...
  PRINT_NAMED_INFO("****puzzle.info", "Data after %d Runs Each *******",
                   maxRuns);
  for (auto& puzzles : mazeData) {
    ...
    PRINT_NAMED_INFO("******puzzle.info", "id: %s : %zu x %zu *******",
                     puzzleid, puzzles.second.width, puzzles.second.height);
    PRINT_NAMED_INFO("puzzle.info", "id: %s : avg Time: %d minutes %d seconds",
                     puzzleid, minutes, seconds);
    ConvertSecondsToMinutes(puzzles.second.minTime, minutes, seconds);
    PRINT_NAMED_INFO("puzzle.info", "id: %s : min Time: %d minutes %d seconds",
                     puzzleid, minutes, seconds);
    ConvertSecondsToMinutes(puzzles.second.maxTime, minutes, seconds);
    PRINT_NAMED_INFO("puzzle.info", "id: %s : max Time: %d minutes %d seconds",
                     puzzleid, minutes, seconds);
  }
}
