...
TEST(BehaviorInterface, Create) {
  ...
  Json::Value emptyConfig = ICozmoBehavior::CreateDefaultBehaviorConfig(
      BehaviorClass::Wait, BehaviorID::Anonymous);
  TestBehavior emptyBase(emptyConfig);
  {
    ...
    testBehaviorFramework.InitializeStandardBehaviorComponent(
        &emptyBase, nullptr, true, container);
  }

  BehaviorExternalInterface& behaviorExternalInterface =
      testBehaviorFramework.GetBehaviorExternalInterface();

  Json::Value empty =
      ICozmoBehavior::CreateDefaultBehaviorConfig(emptyClass, emptyID);
...
TEST(BehaviorInterface, HandleMessages) {
  ...
  TestBehavior b(empty);
  Json::Value empty2 =
      ICozmoBehavior::CreateDefaultBehaviorConfig(emptyClass, emptyID2);
  TestBehavior b2(empty2);

  {
    BehaviorContainer* bc = new BehaviorContainer(emptyBehaviorMap);
    testBehaviorFramework.InitializeStandardBehaviorComponent(&b, nullptr, true,
                                                              bc);
  ...
  // Tick the behavior component to send message to behavior
  {
    ...
    testBehaviorFramework.GetBehaviorComponent().UpdateDependent(
        dependentComps);
  }
...
TEST(BehaviorInterface, OutsideAction) {
  ...
  WaitForLambdaAction* action =
      new WaitForLambdaAction([&done](Robot& r) { return done; });
...
TEST(BehaviorInterface, DelegateIfInControlFailures) {
  ...
  IncrementBaseStationTimerTicks();
  DoBehaviorComponentTicks(robot, b,
                           testBehaviorFramework.GetBehaviorComponent(), 3);
  ...
  // action hasn't updated yet, so it's done. Should still fail to start a new
  // action
  errGGotSet = CallWithoutError([&robot, &done, &b]() {
    EXPECT_FALSE(b.CallDelegateIfInControl(robot, done));
  });
...
TEST(BehaviorInterface, DelegateIfInControlCallbacks) {
  ...
  bool ret = b.CallDelegateIfInControlExternalCallback1(
      robot, done,
  ...
  ret = b.CallDelegateIfInControlExternalCallback2(
      robot, done,
      [&callbackCalled](ActionResult res) { callbackCalled = true; });
  EXPECT_TRUE(ret);
...
TEST(BehaviorInterface, DelegateIfInControlWhenNotRunning) {
  ...
  bool ret = b.CallDelegateIfInControlExternalCallback2(
      robot, done1,
      [&callbackCalled1](ActionResult res) { callbackCalled1 = true; });

  EXPECT_FALSE(ret)
      << "should fail to start acting since the behavior isn't running";
  ...
  ret = b.CallDelegateIfInControlExternalCallback2(
      robot, done2,
      [&callbackCalled2](ActionResult res) { callbackCalled2 = true; });
  ...
  EXPECT_TRUE(robot.GetActionList().IsEmpty())
      << "action should have been canceled by stop";

  EXPECT_FALSE(callbackCalled1);
  EXPECT_FALSE(callbackCalled2)
      << "callback shouldn't happen if behavior isn't running";
}
...
TEST(BehaviorInterface, StopActingWithoutCallback) {
  ...
  EXPECT_FALSE(callbackCalled1)
      << "DelegateIfInControl callback should not have run";
...
class TestInitBehavior : public ICozmoBehavior {
 public:
  TestInitBehavior(const Json::Value& config) : ICozmoBehavior(config) {}
  ...
  virtual void OnBehaviorActivated() override {
    _inited = true;
    WaitForLambdaAction* action =
        new WaitForLambdaAction([this](Robot& r) { return _stopAction; });
  ...
  virtual void OnBehaviorDeactivated() override { _stopped = true; }
};
...
TEST(BehaviorInterface, DelegateIfInControlInsideInit) {
  ...
  EXPECT_FALSE(robot.GetActionList().IsEmpty())
      << "action should be started by Init";
...
TEST(BehaviorInterface, BehaviorRespondsToUserIntents) {
  ...
  for (int i = 0; i <= 11; ++i) {
    ...
    } else if (i >= 1 && i <= 3) {  // 1-3 are behavior waiting for tags (one
                                    // tag, the other tag, a wrong tag)
      config["responseType"] = "user intent tags";
    } else if (i >= 4 &&
               i <= 6) {  // 4-6 are lambdas (lambda with correct data, lambda
                          // with wrong data, a wrong tag)
      config["responseType"] = "user intent lambdas";
    } else if (i >= 7 &&
               i <= 9) {  // 7-9 are intents (intent with correct data, intent
                          // with wrong/empty data, wrong tag)
      config["responseType"] = "user intent intent Victor";
    } else if (i == 10 ||
               i == 11) {  // 10-11 are intents with null string (intent with
                           // null string, intent with nonnull string)
    ...
    EXPECT_TRUE(b.WantsToBeActivatedBehavior());  // it's the ICozmoBehavior
                                                  // we're testing here, not b

    switch (i) {
      case 0:  // behavior isnt listening to intents, so it should always be
               // true
      {
        passedIntent.Set_test_name(UserIntent_Test_Name{""});
        EXPECT_TRUE(b.WantsToBeActivated());
        uic.DevSetUserIntentPending(std::move(passedIntent),
                                    UserIntentSource::Voice);
        EXPECT_TRUE(b.WantsToBeActivatedInternal());
        uic.DropUserIntent(USER_INTENT(test_name));
      } break;
      case 1:  // behavior is waiting for test_user_intent_1; also, it should
               // activate the intent
      {
        passedIntent.Set_test_user_intent_1({});
        EXPECT_FALSE(b.WantsToBeActivated());
        uic.DevSetUserIntentPending(passedIntent.GetTag(),
                                    UserIntentSource::Voice);
        EXPECT_TRUE(b.WantsToBeActivated());
        b.OnActivated();
      ...
      case 2:  // behavior is waiting for test_name; also, it should activate
               // the intent
      ...
      case 5:  // behavior is waiting for test_name with "Victor" but we give it
               // Cozmo
      case 8:  // behavior is waiting for test_name with "Victor" but we give it
               // Cozmo
      case 11:  // behavior is waiting for test_name with "" but we give it
                // Cozmo
      {
