...
TEST(DelegationTree, FullTreeWalkthrough) {
  ...
  auto& delegationComponent =
      testFramework.GetBehaviorExternalInterface().GetDelegationComponent();
...
TEST(DelegationTree, DumpBehaviorTransitionsToFile) {
  ...
  const auto* dataLoader =
      testFramework.GetRobot().GetContext()->GetDataLoader();
  ASSERT_NE(dataLoader, nullptr)
      << "Cannot test behaviors if no data loader exists";
  ...
  for (const auto& behPair : behaviorMap) {
    std::string id =
        Anki::Vector::BehaviorTypesWrapper::BehaviorIDToString(behPair.first);
...
struct DelegationTreeNode {
  ...
  DelegationTreeNode(IBehavior* behavior,
      ...
      : indentString("  "), entryString("* "), continueString("| ") {
    this->behavior = behavior;
    this->children = children;
    this->parent = parent;
  }

  void FormatAsList(std::stringstream& ss, std::string prefix = "",
                    bool isLastOfDepth = true) {
    // depth-first through the tree, maintaining a tab depth, output behavior
    // names note that this will be called recursively!

    std::string outID =
        behavior->GetDebugLabel();  // TODO: can trim unique-making digits as
                                    // above if desired
    // let's see if we can output the behavior's class name.
    // IBehavior doesn't let us do that, but ICozmoBehavior does...
    // this is dangerous, of course, but in practice we can be reasonably sure
    // we have ICozmoBehaviors
    ICozmoBehavior* cozmoBehavior = static_cast<ICozmoBehavior*>(behavior);
    BehaviorClass behaviorClass = cozmoBehavior->GetClass();
    std::string behaviorClassStr =
    ...
    // TODO: is there a clever way to do a "min rows for connector" like Brad
    // does is the Python version? (the brute force version is to do a pass
    // through the tree to determine the size of each node) number of children
    // as cheap proxy for size
    ...
    // for the last one we tell it it's last of depth, so it doesn't do a
    // continuation marker
  ...
  void FormatAsHTML(std::stringstream& ss) {
    ...
    if (children.size() < 1) {
      ...
      // otherwise a details structure, with line as summary and list of
      // children in details
      ss << "<details>\n";
...
std::shared_ptr<DelegationTreeNode> DelegationTreeFromMap(
    std::map<IBehavior*, std::set<IBehavior*>> delegateMap, IBehavior* base,
    DelegationTreeNode* parent = nullptr) {
  std::shared_ptr<DelegationTreeNode> root(new DelegationTreeNode(
      base, std::vector<std::shared_ptr<DelegationTreeNode>>(), parent));

  // figure out if this is a dispatcher that needs its delegates in the correct
  // order behaviorContainter's FindBehaviorByIDAndDownCast only works for the
  // ultimate class, I believe--can't check in the middle of the class
  // hierarchy. We'll try dynamic_cast
  IBehaviorDispatcher* base_dispatcher =
      dynamic_cast<IBehaviorDispatcher*>(base);
  // check if that worked
  if (base_dispatcher != nullptr) {
    // this is a dispatcher

    // NOTE this method is protected; the "#define protected public" hack at the
    // top of this file is necessary for this to work
    std::vector<ICozmoBehaviorPtr> delegates_vcp =
        base_dispatcher->GetAllPossibleDispatches();

    for (ICozmoBehaviorPtr b : delegates_vcp) {
      root->children.push_back(
          DelegationTreeFromMap(delegateMap, b.get(), root.get()));
    }
  } else {
    ...
    for (IBehavior* b : delegates_sbs) {
      root->children.push_back(
          DelegationTreeFromMap(delegateMap, b, root.get()));
    }
  }
...
TEST(DelegationTree, CreateBehaviorTreeHTML) {
  // This test outputs an HTML representation of the Behavior Tree, with type
  // annotations and correct ordering for priority dispatchers and the like.
  ...
  // after the full tree walk, delegate map will be populated. just process
  // that.
  std::shared_ptr<DelegationTreeNode> delegationTreePtr =
      DelegationTreeFromMap(delegateMap, base, nullptr);
...
TEST(DelegationTree, CheckActiveFeatures) {
  // this test checks that active features are correctly defined, and also dumps
  // the active feature per behavior branch to a file, if specified in the
  // environment

  ////////////////////////////////////////////////////////////////////////////////

  // Add any active feature definitions here which exist but aren't yet used in
  // the main behavior tree (e.g. because they are still under development).
  // This must be removed once they are used
  std::set<ActiveFeature> unusedActiveFeatures = {
      ActiveFeature::Frustrated,      // not used
      ActiveFeature::Onboarding,      // exists but in a different stack. todo:
  ...
  ASSERT_EQ((int)ActiveFeature::NoFeature, 0)
      << "unit test is broken without this";
  ...
  // simple voice responses can specify active features outside of the normal
  // behavior tree, so add those here
  tbf.IterateSimpleVoiceResponse(
      [&usedFeatures](const MetaUserIntent_SimpleVoiceResponse& response) {
  ...
  auto evaluateTree = [&ss, &tbf, &usedFeatures, &uic](bool isLeaf) {
    ...
    auto& afc =
        tbf.GetBehaviorComponent().GetComponent<ActiveFeatureComponent>();
    ...
    // all leaf behaviors must have some feature specified, or explicitly
    // specify NoFeature. Additionally, if a behavior explicitly specifies
    // NoFeature, then it shouldn't delegate to any behavior with a feature
    // lower in the stack.
    ...
    if (isLeaf) {
      const auto& behaviorIterator =
          tbf.GetBehaviorComponent().GetComponent<ActiveBehaviorIterator>();
      bool hasExplicitNoFeature = false;
      bool hasAnyFeature = false;

      auto checkFeatureCallback = [&hasExplicitNoFeature, &hasAnyFeature](
                                      const ICozmoBehavior& behavior) {
        ...
        if (behavior.GetAssociatedActiveFeature(feature)) {
          if (feature == ActiveFeature::NoFeature) {
            ...
            EXPECT_FALSE(hasExplicitNoFeature)
                << "Behavior stack specified no feature, but at behavior "
                << behavior.GetDebugLabel() << " has active feature "
      ...
      EXPECT_TRUE(hasExplicitNoFeature || hasAnyFeature)
          << "must specify some feature in each stack, or manually specify "
             "NoFeature"
          << std::endl
          << "behavior stack: "
          << BehaviorStack::StackToBehaviorString(currentStack) << std::endl;

      if (hasExplicitNoFeature && !hasAnyFeature) {
        EXPECT_EQ(afcFeature, ActiveFeature::NoFeature)
            << "stack specifies no feature, but component has feature "
            << ActiveFeatureToString(afcFeature) << std::endl
            << "behavior stack: "
            << BehaviorStack::StackToBehaviorString(currentStack) << std::endl;
      } else {
        EXPECT_NE(afcFeature, ActiveFeature::NoFeature)
            << "stack specifies a feature, but component has feature "
            << ActiveFeatureToString(afcFeature) << std::endl
            << "behavior stack: "
            << BehaviorStack::StackToBehaviorString(currentStack) << std::endl;
      }
  ...
  for (uint32_t featureInt = 1; featureInt < ActiveFeatureNumEntries;
       ++featureInt) {
    ...
    if (unusedActiveFeatures.find(af) == unusedActiveFeatures.end()) {
      EXPECT_TRUE(usedFeatures.find(af) != usedFeatures.end())
          << "Tree did not expose feature " << ActiveFeatureToString(af);
    }
  }

  // make sure a lazy developer didn't forget to remove an unused feature once
  // it's actually used
  for (const auto& af : unusedActiveFeatures) {
    EXPECT_TRUE(usedFeatures.find(af) == usedFeatures.end())
        << "Please remove '" << ActiveFeatureToString(af)
        << "' from the unused features list (since it's used)";
  }
}

TEST(DelegationTree, PrepareToBeForceActivated) {
  // doesn't actually test anything. makes a list of those behaviors whose
  // PrepareTobeForceActivated arent sufficient, and they still don't want to be
  // activated
  ...
  auto addToList = [](IBehavior* delegate, ListType& list) {
    ...
    auto& classes =
        list[BehaviorTypesWrapper::BehaviorClassToString(castPtr->GetClass())];
  ...
  delegateMap.insert(std::make_pair(bottomOfStack, tmpDelegates));
  auto normalCallback = []() {};
  testFramework.FullTreeWalk(delegateMap, normalCallback,
                             evalPriorToDelegation);
  ...
  std::cout
      << "The following behavior classes (and instances) can be force started:"
      << std::endl;
  printList(workingBehaviors);
  std::cout << std::endl;

  std::cout << "The following behavior classes (and instances) can NOT be "
               "force started:"
            << std::endl;
