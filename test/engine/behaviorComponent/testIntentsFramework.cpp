...
namespace {
// Used to assert that intents are handled quickly enough. Should match the
// value set in userIntentComponent.cpp
static const size_t kMaxTicksToClear = 3;
}  // namespace

std::unordered_map<std::string, TestIntentsFramework::IntentInfo>
    TestIntentsFramework::_completedUserIntents;
...
bool TestIntentsFramework::TestUserIntentTransition(
    TestBehaviorFramework& tbf, const std::vector<IBehavior*>& initialStack,
    UserIntent intentToSend, BehaviorID expectedIntentHandlerID,
  ...
  uic.DevSetUserIntentPending(std::move(intentToSend),
                              UserIntentSource::Unknown);
  // TODO:(bn) add support for intent sources

  // Tick the behavior component until the intent is consumed, so that the
  // behavior can respond to the intent
  AICompMap emptyMap;
  uint8_t tics = 0;
  ...
  if (!stack.empty()) {
    if (onlyCheckInStack) {
      for (const auto* stackElem : stack) {
        const auto* castTopOfStack =
            dynamic_cast<const ICozmoBehavior*>(stackElem);
      ...
      const auto* castTopOfStack =
          dynamic_cast<const ICozmoBehavior*>(topOfStack);
...
bool TestIntentsFramework::TryParseCloudIntent(TestBehaviorFramework& tbf,
  ...
  if (intent != nullptr) {
    // find out what intent it is (complicated bc we purposefully don't expose a
    // GetIntent() without knowing the intent)
    using Type = std::underlying_type<UserIntentTag>::type;
    for (Type idx = 0; idx != static_cast<Type>(UserIntentTag::test_SEPARATOR);
         ++idx) {
...
const std::string& TestIntentsFramework::GetLabelForIntent(
    const UserIntent& intent) const {
  const Json::Value& intentJson = intent.GetJSON();
  // an intent maps to a label if all json keys in _completedUserIntents are
  // found in intentJson
  for (const auto& entry : _completedUserIntents) {
    ...
    for (const auto& key : entry.second.def.getMemberNames()) {
      ...
      if (intentValue.isNull() ||
          (intentValue.asString() != entry.second.def[key].asString())) {
        match = false;
        break;
      }
    }
...
std::set<std::string> TestIntentsFramework::GetCompletedLabels(
    EHandledLocation location) const {
  std::set<std::string> ret;
  for (const auto& entry : _completedUserIntents) {
    if ((location == EHandledLocation::Anywhere) ||
...
UserIntent TestIntentsFramework::GetCompletedIntent(
...
void TestIntentsFramework::LoadCompletedIntents() {
  ...
  const bool success = cozmoContext->GetDataPlatform()->readAsJson(
      Util::Data::Scope::Resources, jsonFilename, json);
  ...
  auto saveToMap = [&](const std::string& key, EHandledLocation loc) {
    ...
    for (const auto& label : blob.getMemberNames()) {
      const auto it = _completedUserIntents.find(label);
      ASSERT_NAMED(it == _completedUserIntents.end(),
                   ("Label already exists: " + label).c_str());
