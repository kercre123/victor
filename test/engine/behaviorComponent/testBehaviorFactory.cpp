...
// verifies that behavior matches expected data based on the Json above and
// factory contains it correctly
void VerifyBehavior(const ICozmoBehaviorPtr inBehavior,
                    const BehaviorContainer& behaviorContainer,
                    size_t expectedBehaviorCount) {
...
TEST(BehaviorFactory, CreateAndDestroyBehaviors) {
  ...
  BehaviorContainer& behaviorContainer =
      testBehaviorFramework.GetBehaviorContainer();

  const size_t kBaseBehaviorCount =
      behaviorContainer.GetBehaviorMap()
          .size();  // some behaviors are added by default so likely >0

  Json::Value testBehaviorJson;
  Json::Reader reader;
  const bool parsedOK =
      reader.parse(kTestBehaviorJson, testBehaviorJson, false);
  ASSERT_TRUE(parsedOK);

  EXPECT_EQ(behaviorContainer.FindBehaviorByID(expectedID),
            nullptr);  // this behavior shouldn't exist by default

  BehaviorExternalInterface& behaviorExternalInterface =
      testBehaviorFramework.GetBehaviorExternalInterface();

  const bool createdOK =
      behaviorContainer.CreateAndStoreBehavior(testBehaviorJson);
  ASSERT_TRUE(createdOK);
  ICozmoBehaviorPtr newBehavior =
      behaviorContainer.FindBehaviorByID(BEHAVIOR_ID(Wait_TestInjectable));
