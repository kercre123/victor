...
TEST(BeiConditions, TestBEIConditionFactory) {
...

  {
    ...
    auto returned =
        BEIConditionFactory::CreateBEICondition(cond1Config, "test");
...

  {
    ...
    auto returned =
        BEIConditionFactory::CreateBEICondition(cond2Config, "test");
    ASSERT_TRUE(returned != nullptr);
  ...
  {
    auto handle1 =
        BEIConditionFactory::InjectCustomBEICondition("cond1", cond1);
    handles.push_back(handle1);
    handles.emplace_back(
        BEIConditionFactory::InjectCustomBEICondition("cond2", cond2));
  }
...
TEST(BeiConditions, Emotion) {
  ...
  InitBEIPartial({{BEIComponentID::MoodManager, &moodManager},
                  {BEIComponentID::RobotInfo, &info}},
                 bei);
...
TEST(BeiConditions, SimpleMood) {
  ...
  // so change stimulation with these to be in the mid-range of each definition
  // to reduce the chance they need changing again
  ...
  tickMoodManager(1);
  EXPECT_TRUE(
      condTrans->AreConditionsMet(bei));  // med->high, correct transition
  moodManager.SetEmotion(EmotionType::Stimulated, medStim);
  tickMoodManager(1);
  EXPECT_FALSE(
      condTrans->AreConditionsMet(bei));  // high->med, wrong transition
  ...
  tickMoodManager(1);
  EXPECT_FALSE(
      condTransFromAny->AreConditionsMet(bei));  // med->low, wrong transition
  moodManager.SetEmotion(EmotionType::Stimulated, highStim);
  tickMoodManager(1);
  EXPECT_TRUE(condTransFromAny->AreConditionsMet(
      bei));  // low->high, correct transition
  moodManager.SetEmotion(EmotionType::Stimulated, medStim);
  tickMoodManager(1);
  moodManager.SetEmotion(EmotionType::Stimulated, highStim);
  tickMoodManager(1);
  EXPECT_TRUE(condTransFromAny->AreConditionsMet(
      bei));  // med->high also a correct transition
  ...
  moodManager.SetEmotion(EmotionType::Confident, notFrustrated);
  tickMoodManager(1);
  EXPECT_TRUE(condTransToAny->AreConditionsMet(
      bei));  // frustrated->highstim, correct transition
  moodManager.SetEmotion(EmotionType::Confident,
                         frustrated);  // highstim->frustrated, wrong transition
  tickMoodManager(1);
  moodManager.SetEmotion(EmotionType::Stimulated, lowStim);
  tickMoodManager(1);
  EXPECT_TRUE(condTransToAny->AreConditionsMet(
      bei));  // frustrated->lowstim, correct transition
}
...
TEST(BeiConditions, Timer) {
  ...
  BaseStationTimer::getInstance()->UpdateTime(
      Util::SecToNanoSec(resetTime_s + 1.0f));
  EXPECT_FALSE(cond->AreConditionsMet(bei));

  BaseStationTimer::getInstance()->UpdateTime(
      Util::SecToNanoSec(resetTime_s + 29.0f));
  EXPECT_FALSE(cond->AreConditionsMet(bei));

  BaseStationTimer::getInstance()->UpdateTime(
      Util::SecToNanoSec(resetTime_s + 30.01f));
  EXPECT_TRUE(cond->AreConditionsMet(bei));

  BaseStationTimer::getInstance()->UpdateTime(
      Util::SecToNanoSec(resetTime_s + 34.7f));
  EXPECT_TRUE(cond->AreConditionsMet(bei));

  BaseStationTimer::getInstance()->UpdateTime(
      Util::SecToNanoSec(resetTime_s + 40.0f));
  EXPECT_FALSE(cond->AreConditionsMet(bei));

  BaseStationTimer::getInstance()->UpdateTime(
      Util::SecToNanoSec(resetTime_s + 80.0f));
  EXPECT_FALSE(cond->AreConditionsMet(bei));
...
TEST(BeiConditions, CompoundAnd) {
  ...
  auto cond =
      ConditionCompound::CreateAndCondition({subCond1, subCond2}, "unit_test");
...
TEST(BeiConditions, CompoundOr) {
  ...
  auto cond =
      ConditionCompound::CreateOrCondition({subCond1, subCond2}, "unit_test");
...
TEST(BeiConditions, RobotInHabitat) {
  ...
  bei.GetHabitatDetectorComponent().ForceSetHabitatBeliefState(
      HabitatBeliefState::InHabitat, "UnitTest");
  EXPECT_TRUE(cond->AreConditionsMet(bei));

  bei.GetHabitatDetectorComponent().ForceSetHabitatBeliefState(
      HabitatBeliefState::NotInHabitat, "UnitTest");
  EXPECT_FALSE(cond->AreConditionsMet(bei));

  bei.GetHabitatDetectorComponent().ForceSetHabitatBeliefState(
      HabitatBeliefState::Unknown, "UnitTest");
  EXPECT_FALSE(cond->AreConditionsMet(bei));
}
...
TEST(BeiConditions, UserIntentPending) {
  ...
  // (5) test_name           matches the tag and lambda must eval (name must be
  // Victor)
  ...
  uic.SetUserIntentPending(USER_INTENT(test_user_intent_1),
                           UserIntentSource::Voice);  // right intent
  EXPECT_TRUE(cond->AreConditionsMet(bei));
  EXPECT_TRUE(cond->AreConditionsMet(bei));
  ...
  uic.SetUserIntentPending(
      UserIntent::Createtest_timeWithUnits(std::move(timeWithUnits)),
  ...
  uic.SetUserIntentPending(
      UserIntent::Createset_timer(std::move(timeInSeconds1)),
      UserIntentSource::Voice);
  ...
  EXPECT_FALSE(cond->AreConditionsMet(bei));
  uic.SetUserIntentPending(
      UserIntent::Createset_timer(std::move(timeInSeconds2)),
      UserIntentSource::Voice);
  EXPECT_TRUE(cond->AreConditionsMet(bei));  // correct intent
  EXPECT_EQ(cond->GetUserIntentTagSelected(), USER_INTENT(set_timer));
  uic.DropUserIntent(USER_INTENT(set_timer));
  ...
  UserIntent_Test_TimeWithUnits timeWithUnits1{60,
                                               UserIntent_Test_Time_Units::m};
  UserIntent_Test_TimeWithUnits timeWithUnits2{20,
                                               UserIntent_Test_Time_Units::m};
  uic.SetUserIntentPending(
      UserIntent::Createtest_timeWithUnits(std::move(timeWithUnits1)),
      UserIntentSource::Voice);
  EXPECT_TRUE(cond->AreConditionsMet(bei));  // correct intent
  ...
  EXPECT_FALSE(cond->AreConditionsMet(bei));
  uic.SetUserIntentPending(
      UserIntent::Createtest_timeWithUnits(std::move(timeWithUnits2)),
      UserIntentSource::Voice);
  ...
  uic.SetUserIntentPending(
      std::move(name5),
      UserIntentSource::Voice);  // right intent with right data
  EXPECT_TRUE(cond->AreConditionsMet(bei));
  EXPECT_EQ(cond->GetUserIntentTagSelected(), USER_INTENT(test_name));
  uic.DropUserIntent(USER_INTENT(test_name));
}
...
TEST(BeiConditions, BehaviorTimer) {
  ...
  // prior to expiration
  BaseStationTimer::getInstance()->UpdateTime(
      Util::SecToNanoSec(resetTime1 + 14.9999));
  EXPECT_FALSE(cond->AreConditionsMet(bei));

  // just past expiration
  BaseStationTimer::getInstance()->UpdateTime(
      Util::SecToNanoSec(resetTime1 + 15.0001));
  EXPECT_TRUE(cond->AreConditionsMet(bei));
  ...
  BaseStationTimer::getInstance()->UpdateTime(
      Util::SecToNanoSec(resetTime2 + 14.9999));
  EXPECT_FALSE(cond->AreConditionsMet(bei));

  BaseStationTimer::getInstance()->UpdateTime(
      Util::SecToNanoSec(resetTime2 + 15.0001));
  EXPECT_TRUE(cond->AreConditionsMet(bei));
...
TEST(BeiConditions, FeatureGate) {
  ...
  bei.GetRobotInfo().GetContext()->GetFeatureGate()->SetFeatureEnabled(feature,
                                                                       true);
  EXPECT_TRUE(cond->AreConditionsMet(bei));
...
TEST(BeiConditions, ObjectKnown) {
  // tests that the conditions are true only when the object is known, the
  // maxAge_ms params match the object age
  const ObjectType testType =
      ObjectType::Block_LIGHTCUBE1;  // should match the below json
  ...
  ObservableObject* object1 =
      CubePlacementHelper::CreateObjectLocatedAtOrigin(robot, testType);
  ...
  auto result = robot.GetBlockWorld().SetObjectPose(object1->GetID(), obj1Pose,
                                                    PoseState::Known);
...
TEST(BeiConditions, FaceKnown) {
  ...
  // create conditions
  IBEIConditionPtr condDefault;
  CreateBEI(jsonDefault, condDefault);
  IBEIConditionPtr condMaxDist400mm;
  CreateBEI(jsonMaxDist400mm, condMaxDist400mm);
  IBEIConditionPtr condMaxAge10sec;
  CreateBEI(jsonMaxAge10sec, condMaxAge10sec);
  IBEIConditionPtr condMustBeNamed;
  CreateBEI(jsonMustBeNamed, condMustBeNamed);

  // init and activate
  condDefault->Init(bei);
  condDefault->SetActive(bei, true);
  condMaxDist400mm->Init(bei);
  condMaxDist400mm->SetActive(bei, true);
  condMaxAge10sec->Init(bei);
  condMaxAge10sec->SetActive(bei, true);
  condMustBeNamed->Init(bei);
  condMustBeNamed->SetActive(bei, true);
