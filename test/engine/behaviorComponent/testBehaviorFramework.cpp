...
const std::string kNamedBehaviorStackFileName =
    "test/aiTests/namedBehaviorStacks.json";
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void InitBEIPartial(const BEIComponentMap& map,
                    BehaviorExternalInterface& bei) {
  bei.Init(
      GetFromMap<AIComponent>(map, BEIComponentID::AIComponent),
      ...
      GetFromMap<BehaviorTimerManager>(map,
                                       BEIComponentID::BehaviorTimerManager),
      GetFromMap<BlockWorld>(map, BEIComponentID::BlockWorld),
      GetFromMap<BackpackLightComponent>(
          map, BEIComponentID::BackpackLightComponent),
      GetFromMap<CubeAccelComponent>(map, BEIComponentID::CubeAccel),
      GetFromMap<CubeCommsComponent>(map, BEIComponentID::CubeComms),
      GetFromMap<CubeConnectionCoordinator>(
          map, BEIComponentID::CubeConnectionCoordinator),
      GetFromMap<CubeInteractionTracker>(
          map, BEIComponentID::CubeInteractionTracker),
      ...
      GetFromMap<HabitatDetectorComponent>(map,
                                           BEIComponentID::HabitatDetector),
      ...
      GetFromMap<PublicStateBroadcaster>(
          map, BEIComponentID::PublicStateBroadcaster),
      GetFromMap<SDKComponent>(map, BEIComponentID::SDK),
      GetFromMap<Audio::EngineRobotAudioClient>(
          map, BEIComponentID::RobotAudioClient),
      GetFromMap<BEIRobotInfo>(map, BEIComponentID::RobotInfo),
      GetFromMap<DataAccessorComponent>(map, BEIComponentID::DataAccessor),
      GetFromMap<TextToSpeechCoordinator>(
          map, BEIComponentID::TextToSpeechCoordinator),
      GetFromMap<TouchSensorComponent>(map, BEIComponentID::TouchSensor),
      GetFromMap<VariableSnapshotComponent>(
          map, BEIComponentID::VariableSnapshotComponent),
      GetFromMap<VisionComponent>(map, BEIComponentID::Vision),
      GetFromMap<VisionScheduleMediator>(
          map, BEIComponentID::VisionScheduleMediator),
...
TestBehaviorFramework::TestBehaviorFramework(
    int robotID, Anki::Vector::CozmoContext* context)
    : _behaviorExternalInterface(nullptr), _behaviorSystemManager(nullptr) {
  if (context == nullptr) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TestBehaviorFramework::InitializeStandardBehaviorComponent(
...
void TestBehaviorFramework::InitializeStandardBehaviorComponent(
  ...
  // swap out the robot's ai component for a custom one
  {
    ...
    entity->AddDependentComponent(BCComponentID::AIComponent,
                                  _robot->GetComponentPtr<AIComponent>(),
                                  false);
    auto bootLoader = new BehaviorsBootLoader(baseBehavior, {});
    entity->AddDependentComponent(BCComponentID::BehaviorsBootLoader,
                                  bootLoader);
    entity->AddDependentComponent(BCComponentID::BehaviorContainer,
                                  _behaviorContainer.get(), false);

    BehaviorComponent::GenerateManagedComponents(*_robot, entity);

    DependencyManagedEntity<RobotComponentID> dependencies;
    dependencies.AddDependentComponent(RobotComponentID::MicComponent,
                                       _robot->GetComponentPtr<MicComponent>(),
                                       false);
    dependencies.AddDependentComponent(
        RobotComponentID::Vision, _robot->GetComponentPtr<VisionComponent>(),
        false);
    dependencies.AddDependentComponent(RobotComponentID::FaceWorld,
                                       _robot->GetComponentPtr<FaceWorld>(),
                                       false);
    _robot->GetAIComponent().InitDependent(_robot.get(), dependencies);
    _behaviorComponent =
        _robot->GetAIComponent().GetComponentPtr<BehaviorComponent>();
...
void TestBehaviorFramework::SetDefaultBaseBehavior() {
  ...
  // Load base behavior in from data
  {
    ...
    const Json::Value& behaviorSystemConfig =
        (dataLoader != nullptr) ? dataLoader->GetVictorFreeplayBehaviorConfig()
                                : blankActivitiesConfig;

    BehaviorID baseBehaviorID = BEHAVIOR_ID(Anonymous);
    const bool validBehavior = BehaviorIDFromString(
        behaviorSystemConfig["normalBaseBehavior"].asString(), baseBehaviorID);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TestBehaviorFramework::ReplaceBehaviorStack(
    std::vector<IBehavior*> newStack) {
  ...
  if (newStack.size() > 0) {
    ...
    bsm._behaviorStack->InitBehaviorStack(
        *baseBehaviorIter,
  ...
  for (auto& delegate : newStack) {
    ...
    auto& delegationComponent =
        GetBehaviorExternalInterface().GetDelegationComponent();
...
void TestBehaviorFramework::AddDelegateToStack(IBehavior* delegate) {
  ...
  // cancel all delegates (including actions) because the behaviors OnActivated
  // may have delegated to something
  auto& delegationComponent =
      GetBehaviorExternalInterface().GetDelegationComponent();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TestBehaviorFramework::ApplyAdditionalRequirementsBeforeDelegation(
    IBehavior* delegate) {
  ...
  if (delCozPtr != nullptr) {
    ...
    // Apply dev requirements from devDelegationRequirements before test
    // requirements
    ApplyForceDelegationRequirements(delCozPtr, bei);

    // move on to test requirements
    if (!delCozPtr->WantsToBeActivated()) {
#define ADD_CASE_FOR_BEHAVIOR(x)                                         \
  case BehaviorClass::x: {                                               \
    ApplyTestDelegationRequirements<BehaviorClass::x>(delCozPtr, *this); \
  } break
...
#define ADD_CASE_FOR_BEHAVIOR(x)                                      \
  case BehaviorID::x: {                                               \
    ApplyTestDelegationRequirements<BehaviorID::x>(delCozPtr, *this); \
  } break
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TestBehaviorFramework::RemoveAdditionalDelegationRequirements(
    IBehavior* delegate) {
  std::set<IBehavior*> linkedScope;
  delegate->GetLinkedActivatableScopeBehaviors(linkedScope);
  for (auto* linkedDelegate : linkedScope) {
  ...
  ICozmoBehavior* delCozPtr = dynamic_cast<ICozmoBehavior*>(delegate);
  if (delCozPtr != nullptr) {
    // search first by behaviorID, and if that doesn't match, search by debug
    // label
    auto it =
        _cachedDelegationReqs.find(BehaviorIDToString(delCozPtr->GetID()));
...
#define ADD_CASE_FOR_BEHAVIOR(x)                                      \
  case BehaviorID::x: {                                               \
    RemoveTestDelegationRequirements<BehaviorID::x>(delCozPtr, *this, \
                                                    it->second);      \
  } break
      switch (delCozPtr->GetID()) {
        ADD_CASE_FOR_BEHAVIOR(DriveOffChargerFace);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
std::vector<IBehavior*> TestBehaviorFramework::GetNamedBehaviorStack(
  ...
  ASSERT_NAMED(
      _namedBehaviorStacks.find(stackName) != _namedBehaviorStacks.end(),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TestBehaviorFramework::SetBehaviorStackByName(
    const std::string& stackName) {
  if (_namedBehaviorStacks.empty()) {
    LoadNamedBehaviorStacks();
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TestBehaviorFramework::FullTreeWalk(
    ...
    if (iter != delegateMap.end()) {
      for (auto& delegate : iter->second) {
        ...
        if (nextDelegates == delegateMap.end()) {
          ...
          nextDelegates =
              delegateMap
                  .insert(std::make_pair(delegate, std::move(tmpDelegates)))
                  .first;
        }

        ASSERT_NAMED(nextDelegates != delegateMap.end(),
                     "TestBehaviorFramework.TreeWalk.InvalidDelegates");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool TestBehaviorFramework::CanStackOccurDuringFreeplay(
  ...
  DEV_ASSERT(1 == currentStack.size(),
             "CanStackOccurDuringFreeplay.SizeMismatch");
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void InjectBehaviorIntoStack(ICozmoBehavior& injectBehavior,
  ...
  if (ANKI_VERIFY(!behaviorStack.empty(),
                  "InjectBehaviorIntoStack.NoBaseBehavior", "")) {
...
void InjectValidDelegateIntoBSM(TestBehaviorFramework& testFramework,
  ...
  if (iter == bsm._behaviorStack->_stackMetadataMap.end()) {
    ...
    DEV_ASSERT(bsm._behaviorStack->_behaviorStack.empty(),
               "TestBehaviorFramework.InjectValidDelegate."
               "StackNotEmptyButDelegatMapMiss");
    bsm._behaviorStack->_stackMetadataMap[delegator] =
        BehaviorStack::StackMetadataEntry(
            delegator,
            static_cast<int>(bsm._behaviorStack->_behaviorStack.size()));
    bsm._behaviorStack->_stackMetadataMap[delegator].delegates.insert(
        delegated);
  } else {
...
void InjectAndDelegate(TestBehaviorFramework& testFramework,
                       IBehavior* delegator, IBehavior* delegated) {
  InjectValidDelegateIntoBSM(testFramework, delegator, delegated);
  auto& bsm = testFramework.GetBehaviorSystemManager();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TestSuperPoweredBehavior::GetAllDelegates(
...
void TestBehavior::InitBehavior() {
  ...
  auto robotExternalInterface = robotInfo.HasExternalInterface()
                                    ? robotInfo.GetExternalInterface()
                                    : nullptr;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TestBehavior::OnBehaviorActivated() { _inited = true; }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TestBehavior::OnBehaviorDeactivated() { _stopped = true; }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TestBehavior::HandleWhileInScopeButNotActivated(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TestBehavior::Foo() { _calledVoidFunc++; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TestBehavior::Bar() { _calledRobotFunc++; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool TestBehavior::CallDelegateIfInControl(Robot& robot,
                                           bool& actionCompleteRef) {
  WaitForLambdaAction* action = new WaitForLambdaAction(
      [&actionCompleteRef](Robot& r) { return actionCompleteRef; });
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool TestBehavior::CallDelegateIfInControlExternalCallback1(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool TestBehavior::CallDelegateIfInControlExternalCallback2(
    Robot& robot, bool& actionCompleteRef,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool TestBehavior::CallDelegateIfInControlInternalCallbackVoid(
    Robot& robot, bool& actionCompleteRef) {
  WaitForLambdaAction* action = new WaitForLambdaAction(
      [&actionCompleteRef](Robot& r) { return actionCompleteRef; });
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
bool TestBehavior::CallDelegateIfInControlInternalCallbackRobot(
    Robot& robot, bool& actionCompleteRef) {
  WaitForLambdaAction* action = new WaitForLambdaAction(
      [&actionCompleteRef](Robot& r) { return actionCompleteRef; });
...
void TestBehaviorFramework::LoadNamedBehaviorStacks() {
  ASSERT_NAMED(_namedBehaviorStacks.empty(),
               "Named Behavior Stacks should be loaded only once");

  Json::Value stackList;
  const bool readSuccess = cozmoContext->GetDataPlatform()->readAsJson(
      Util::Data::Scope::Resources, kNamedBehaviorStackFileName, stackList);
  ASSERT_NAMED(readSuccess, ("TestBehaviorFramework error reading file: " +
  ...
  for (const auto& behaviorStackName : stackList.getMemberNames()) {
    ...
    for (auto& behaviorName : jsonStack) {
      ...
      ASSERT_NAMED(
          EnumFromString(behaviorName.asString(), behaviorId),
          ("no behavior exists of name: " + behaviorName.asString()).c_str());

      behaviorStack.push_back(
          GetBehaviorContainer().FindBehaviorByID(behaviorId).get());
    }
    const auto& it = _namedBehaviorStacks.find(behaviorStackName);
    ASSERT_NAMED(
        it == _namedBehaviorStacks.end(),
        ("Name already exists for behavior stack: " + behaviorStackName)
            .c_str());
    ASSERT_NAMED(
        stackIsForTestOnly || CanStackOccurDuringFreeplay(behaviorStack),
        ("Named Behavior Stack: " + behaviorStackName +
         " is not achievable during freeplay")
            .c_str());
...
void TestBehaviorFramework::AddFakeFirstFace() {
  ...
  if (_robot->GetStateHistory()->GetNumRawStates() == 0) {
    // faceworld needs an update to robot state before adding a face (this
    // should only happen in unit tests)
    RobotState robotState = Robot::GetDefaultRobotState();
    _robot->GetStateHistory()->AddRawOdomState(
        faceWorld->_lastObservedFaceTimeStamp,
        HistRobotState(_robot->GetPose(), robotState, {}));

    // a face only gets added if the robot isn't moving much. to check this,
    // there must be some IMU values
    TimeStamp_t ts = (TimeStamp_t)faceWorld->_lastObservedFaceTimeStamp;
    _robot->GetImuComponent().AddData(
        IMUDataFrame{ts, GyroData{0.0f, 0.0f, 0.0f}});
    _robot->GetImuComponent().AddData(
        IMUDataFrame{ts, GyroData{0.0f, 0.0f, 0.0f}});
    _robot->GetImuComponent().AddData(
        IMUDataFrame{ts, GyroData{0.0f, 0.0f, 0.0f}});
  ...
  // Update() gets called more than once per tick already whenever multiple
  // robot messages are received, so calling it again shouldn't be any more of
  // an issue in dev than it is in prod
  faceWorld->Update(faceList);

  DEV_ASSERT_MSG(faceWorld->HasAnyFaces(),
                 "TestBehaviorFramework.DevAddFakeFace.NoFaces",
                 "Method failed to add a face");
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - -
void TestBehaviorFramework::AddFakeFirstObject(ObjectType objectType,
                                               Pose3d* pose) {
  auto* blockWorld = _robot->GetComponentPtr<BlockWorld>();
  BlockWorldFilter filter;
  filter.AddAllowedType(objectType);
  const bool anyObject =
      (nullptr != blockWorld->FindLocatedMatchingObject(filter));
  if (anyObject) {
    return;
  }
  ...
  ANKI_VERIFY(nullptr != objectPtr,
              "TestBehaviorFramework.AddFakeFirstObject.CreatedNull", "");
  ANKI_VERIFY(!objectPtr->GetID().IsSet(),
              "TestBehaviorFramework.AddFakeFirstObject.IDSet", "");
  ANKI_VERIFY(!objectPtr->HasValidPose(),
              "TestBehaviorFramework.AddFakeFirstObject.HasValidPose", "");
  objectPtr->SetID();
  objectPtr->InitPose(*pose, Anki::PoseState::Known);
  blockWorld->AddLocatedObject(std::shared_ptr<ObservableObject>(objectPtr));

  ANKI_VERIFY(blockWorld->FindLocatedMatchingObject(filter) != nullptr,
              "TestBehaviorFramework.AddFakeFirstObject.Fail", "");
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - - - - - - - - - - - - - - - -
void TestBehaviorFramework::IterateSimpleVoiceResponse(
    std::function<void(const MetaUserIntent_SimpleVoiceResponse&)> lambda) {
  if (ANKI_VERIFY(
          _aiComponent != nullptr,
          "TestBehaviorFramework.IterateSimpleVoiceResponse.NullAIComponent",
          "")) {
    auto& uic = _aiComponent->GetComponent<BehaviorComponent>()
