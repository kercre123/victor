...
class TestBehaviorHighLevelAI {
 public:
  explicit TestBehaviorHighLevelAI(
  ...
  bool AreStatesRespondingTo(UserIntentTag intentTag, const UserIntent* intent,
    ...
    for (const auto& transitionPair : _transitionPairs) {
      ...
      for (auto transPtr : transitionPair.second) {
        // check compound conditions to see if they contain UserIntentPending
        // conditions
        auto type = transPtr->GetConditionType();
        if (type == BEIConditionType::Compound) {
          auto condition =
              std::dynamic_pointer_cast<ConditionCompound>(transPtr);
          EXPECT_TRUE(condition != nullptr);
          const auto& operands = condition->TESTONLY_GetOperands({});
          for (auto& operand : operands) {
            if (operand->GetConditionType() ==
                BEIConditionType::UserIntentPending) {
        ...
        if (type == BEIConditionType::UserIntentPending) {
          auto condition =
              std::dynamic_pointer_cast<ConditionUserIntentPending>(transPtr);
          EXPECT_TRUE(condition != nullptr);
  ...
  static bool IsStateRunning(std::shared_ptr<BehaviorHighLevelAI> beh,
...
struct CompletedEntry {
  CompletedEntry(const std::string& s, const std::vector<std::string>& ex)
      : jsonStr(s), exceptions(ex) {}
  std::string jsonStr;
  Json::Value json;
...
TEST(BehaviorHighLevelAI, UserIntentsHandled) {
  // this test: for each cloud intent and app intent, and for each state in HL
  // AI, check that the corresponding user intent is handled, unless there's an
  // explicit exception here. This cares only about the user intents for which
  // there are cloud/app intents because that probably means they were at least
  // partially hooked up. Eventually these will be replaced by DialogFlow data.
  // For now, when you finish a new intent related to HL AI, add it to the list
  // below as a cloud intent, and this test will compare HLAI, the list below,
  // and user_intent_map for any discrepancies.
  ...
  std::unordered_map<std::string, std::pair<UserIntent, ExceptionsList>>
      completedIntents;
  auto labels = tih.GetCompletedLabels(
      TestIntentsFramework::EHandledLocation::HighLevelAI);
  for (const auto& label : labels) {
    const auto it = exceptionsList.find(label);
    ASSERT_FALSE(it == exceptionsList.end())
        << "Completed intent " << label << " not listed in exceptionsList";
    const auto& exceptions = it->second;
    completedIntents[label] =
        std::make_pair(tih.GetCompletedIntent(label), exceptions);
  }
  ...
  BC.FindBehaviorByIDAndDowncast(BEHAVIOR_ID(HighLevelAI),
                                 BEHAVIOR_CLASS(HighLevelAI), behavior);
  ...
  for (const auto& completedEntry : completedIntents) {
    ...
    const UserIntent* intentPtr =
        (intent.GetJSON().size() == 1) ? nullptr : &intent;

    std::vector<std::string> statesNotResponding;
    const bool anyResponding = testHLAI.AreStatesRespondingTo(
        intentTag, intentPtr, statesNotResponding);
    EXPECT_TRUE(anyResponding)
        << "Completed list contains " << label << "("
        << UserIntentTagToString(intentTag) << ") but is not handled in HLAI";

    // states that arent responding should be listed as exceptions
    for (const auto& actualException : statesNotResponding) {
      const auto itFound = std::find(expectedExceptions.begin(),
                                     expectedExceptions.end(), actualException);
      EXPECT_TRUE(itFound != expectedExceptions.end())
          << "State " << actualException << " does not respond to "
          << UserIntentTagToString(intentTag) << " (" << label << ")";
      // you should add a transition or add this state as an exception in this
      // test (see completedList)
    }
    // and the inverse: exceptions should not have states that are responding
    for (const auto& expectedException : expectedExceptions) {
      const auto itFound =
          std::find(statesNotResponding.begin(), statesNotResponding.end(),
                    expectedException);
      EXPECT_TRUE(itFound != statesNotResponding.end())
          << "State " << expectedException << " responds to "
          << UserIntentTagToString(intentTag) << " (" << label
          << ") but is listed as an exception";
      // you should update this test to remove the exception (see
      // exceptionsList, above)
    }
  }
  ...
  for (const auto& exceptionEntry : exceptionsList) {
    ...
    const auto it =
        std::find_if(completedIntents.begin(), completedIntents.end(),
                     [&](const auto& entry) { return entry.first == label; });
    EXPECT_TRUE(it != completedIntents.end())
        << "In addition to adding exceptions to the above list, add to "
           "completedUserIntents.json";
  }
}

TEST(BehaviorHighLevelAI, PostBehaviorSuggestionsConsidered) {
  // tests that when voice commands that offer post-behavior suggestions end,
  // and then HLAI resumes, that the state is set properly. Typically
  // post-behavior suggestions are non-binding, but we want this ones to occur
  // so they are enforced through this test.

  // a list of the named completedUserIntents(.json), the behavior that responds
  // to it, and the HLAI state that should follow
  std::vector<std::tuple<std::string, BehaviorID, std::string>> expected = {
      ...
      //}, // removed in
      //https://github.com/anki/voice-intent-resolution-config/pull/33/files
      {"roll_cube", BEHAVIOR_ID(RollCubeVoiceCommand), "Observing"},
      {"meet_victor", BEHAVIOR_ID(MeetVictor), "Socializing"},
      //{ "imperative_come", BEHAVIOR_ID(ComeHereVoiceCommand),   "Socializing"
      //}, // todo: come here needs a look for face in order to activate
  };
  ...
  for (const auto& tup : expected) {
    ...
    if (intentTag == USER_INTENT(meet_victor)) {
      intent.Set_meet_victor(UserIntent_MeetVictor{
          "Special name for unit tests to end enrollment"});
    }
    ...
    const auto itFound =
        std::find_if(stack.begin(), stack.end(),
                     [&behaviorID, &behaviorToCancel](auto* behavior) {
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      const auto itFound =
          std::find_if(stack.begin(), stack.end(), [](auto* behavior) {
            auto* castBeh = dynamic_cast<ICozmoBehavior*>(behavior);
    ...
    BC.FindBehaviorByIDAndDowncast(BEHAVIOR_ID(HighLevelAI),
                                   BEHAVIOR_CLASS(HighLevelAI), behHLAI);
    const bool newStateRunning =
        TestBehaviorHighLevelAI::IsStateRunning(behHLAI, nextState);
    EXPECT_TRUE(newStateRunning)
        << nextState << " not running after " << intentName << " ended";
  }
}
