 ...
 * Created: Mar 20 2018
 *
 * Description: Framework to help with testing user/cloud/app intents that we
 *consider "done"
 *
...
class TestIntentsFramework {
 public:
  ...
  // behavior taking control of the stack. True if it takes control, false
  // otherwise. Pass in:
  ...
  // returns true if the sample cloud intent can be parsed into a user intent.
  // If requireMatch, then that user intent can NOT be unmatched_intent. Also
  // sets a non-null intent ptr with the matching intent
  ...
  // Get a set of labels we consider "complete." This is a set instead of
  // something unordered bc it's useful to compare sorted sets. Provide an
  // optional location to get labels based on where the user intent is expected
  // to be handled
  std::set<std::string> GetCompletedLabels(
      EHandledLocation location = EHandledLocation::Anywhere) const;
