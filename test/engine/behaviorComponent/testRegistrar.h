 ...
 * Created: 2018 mar 22
 *
 * Description: Registers unit tests, so that you can test that tests were
 *written
 *
...
struct TestRegistrar {
 public:
  ...
  static bool IsRegistered(const std::string& testID) {
    const bool ret = RegisteredTestsInternal().find(testID) !=
                     RegisteredTestsInternal().end();
    return ret;
  }

  static const std::set<std::string>& RegisteredTests() {
    return RegisteredTestsInternal();
  }
...
#define MAKE_REGISTRAR(T)                                   \
  struct T : public TestRegistrar<T> {                      \
