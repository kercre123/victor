 ...
 * Created: 2018 Mar 20
 *
 * Description: tests that intents from various sources are parsed into valid
 *user intents
 *
...
TEST(UserIntentsParsing, CloudSampleFileParses) {
  ...
  if (true) {
    // this unit test only runs as a required build step of the
    // voice-intent-resolution-config, which pulls victor master, sets
    // ANKI_TEST_INTENT_SAMPLE_FILE, and runs the mac build
    ...
    PRINT_NAMED_INFO("UserIntentsParsing.TestIntentSampleFile.ConfigSize",
                     "Size of JSON root = %d", config.size());
    ...
    const bool parsedOK = cozmoContext->GetDataPlatform()->readAsJson(
        Util::Data::Scope::Resources, jsonFilename, config);
    EXPECT_TRUE(parsedOK);
  }
  ...
  for (const auto& sample : config) {
    ...
    if (intent.GetTag() != UserIntentTag::unmatched_intent) {
      ...
      if (label.empty()) {
        ...
        PRINT_NAMED_INFO("UserIntentsParsing.TestIntentSampleFile.SampleLabel",
                         "The label for %s is %s", ss.str().c_str(),
                         label.c_str());
  ...
  while (itSamples != sampleLabels.end()) {
    PRINT_NAMED_INFO("UserIntentsParsing.CloudSampleFileParses.SampleLabels",
                     "[ %s ]", itSamples->c_str());
    ++itSamples;
  }
  auto itCompleted = completedLabels.begin();
  while (itCompleted != completedLabels.end()) {
    PRINT_NAMED_INFO("UserIntentsParsing.CloudSampleFileParses.CompletedLabels",
                     "[ %s ]", itCompleted->c_str());
    ++itCompleted;
  }
  ...
  while (itSamples != sampleLabels.end() &&
         itCompleted != completedLabels.end()) {
    while (itSamples != sampleLabels.end()) {
  ...
  EXPECT_FALSE(itSamples == sampleLabels.end() &&
               itCompleted != completedLabels.end())
      << "Could not find " << *itCompleted << " and maybe more ";

  // it should be ok if user_intent_map has a cloud intent that isnt completed,
  // but it should exist in dialogflow samples; otherwise it should be
  // considered garbage.
  UserIntentMap intentMap(cozmoContext->GetDataLoader()->GetUserIntentConfig(),
                          cozmoContext);
  std::vector<std::string> cloudIntentsList =
      intentMap.DevGetCloudIntentsList();
  for (const auto& cloudName : cloudIntentsList) {
    // if this cloud intent has "test_parsing" set to false in user_intent_map,
    // then skip it and do not check if it exists in Dialogflow sample file
    ...
    //                 "Looking for Dialogflow sample that matches '%s'",
    //                 cloudName.c_str());
    bool found = false;
    ...
    EXPECT_TRUE(found) << "Could not find user_intent_map cloud intent "
                       << cloudName << " in sample file";
  }
}

TEST(UserIntentsParsing, CompletedInCloudList) {
  // tests that every completed intent has a match for both app and cloud in
  // user_intent_map.
  ...
  for (const auto& completedLabel : completedLabels) {
    ...
    auto itCloud = std::find_if(
        cloudIntentsList.begin(), cloudIntentsList.end(), [&](const auto& x) {
          return (intentMap.GetUserIntentFromCloudIntent(x) == intentTag);
        });
    EXPECT_TRUE(itCloud != cloudIntentsList.end())
        << "Could not find " << UserIntentTagToString(intentTag)
        << " in cloud intent list";

    // app intents haven't been fully realized;
    // commenting out this test until app intents are actually required
    // auto itApp = std::find_if( appIntentsList.begin(), appIntentsList.end(),
    // [&](const auto& x ) {
    //  return (intentMap.GetUserIntentFromAppIntent( x ) == intentTag);
    //});
    // EXPECT_TRUE( itApp != appIntentsList.end() ) << "Could not find " <<
    // UserIntentTagToString(intentTag) << " in app intent list";
  }
}
