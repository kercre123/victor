...
TEST(TestRobotPosSampler, DistributionsCorrect) {
  // verifies that the (x,y) output of SamplePointInAnnulus is uniform by
  // binning into a grid and checking its sample statistics.
  ...
  // now check every square bin that is fully contained by the ring has the same
  // approx count
  auto getCOV = [&](const Grid& grid, float& prob) {
    ...
    for (int i = 0; i < numBins; ++i) {
      ...
      for (int j = 0; j < numBins; ++j) {
        ...
        const float r2Sq =
            (x1 - centerX) * (x1 - centerX) + (y2 - centerY) * (y2 - centerY);
        const float r3Sq =
            (x2 - centerX) * (x2 - centerX) + (y1 - centerY) * (y1 - centerY);
        const float r4Sq =
  ...
  // test that "throwing a dart probability" matches. we need a finer grid here
  // based on what's being tested, but we don't want to use a finer grid above
  // since it needs way more samples
  Grid fineGrid(100, std::vector<int>(100, 0));
  sampleMorePoints(fineGrid, 1000);
  float pHit = 0.0f;
  getCOV(fineGrid, pHit);
  float pExpected = M_PI_F * (maxR * maxR - minR * minR) /
                    (4 * maxR * maxR);  // ratio of areas
...
TEST(TestRobotPosSampler, RejectIfInRange) {
  ...
  RobotPointSamplerHelper::RejectIfInRange rejectIfInRange(minDist_mm,
                                                           maxDist_mm);
  ...
  for (const auto otherPt : otherPts) {
    std::generate_n(std::back_inserter(inRangeSamples), nSamples, [&]() {
      return otherPt + RobotPointSamplerHelper::SamplePointInAnnulus(
                           rng, minDist_mm, maxDist_mm);
    });
    std::generate_n(std::back_inserter(notInRangeSamples), nSamples, [&]() {
      return otherPt +
             RobotPointSamplerHelper::SamplePointInAnnulus(rng, 0.f, 0.5f);
    });
    std::generate_n(std::back_inserter(notInRangeSamples), nSamples, [&]() {
      return otherPt +
             RobotPointSamplerHelper::SamplePointInAnnulus(rng, 1.f, 4.f);
    });
  }

  // none of the "other points" themselves should not be in range since we
  // specified a nonzero minDist_mm
  notInRangeSamples.insert(notInRangeSamples.end(), otherPts.begin(),
                           otherPts.end());

  for (const auto& pt : inRangeSamples) {
    EXPECT_FALSE(rejectIfInRange(pt))
        << "Sample point " << pt
        << "unexpectedly returned true for rejectIfInRange";
  }
  for (const auto& pt : notInRangeSamples) {
    EXPECT_TRUE(rejectIfInRange(pt))
        << "Sample point " << pt
        << "unexpectedly returned false for rejectIfInRange";
  }
}
...
TEST(TestRobotPosSampler, RejectIfNotInRange) {
  ...
  RobotPointSamplerHelper::RejectIfNotInRange rejectIfNotInRange(minDist_mm,
                                                                 maxDist_mm);
  ...
  std::generate_n(std::back_inserter(notInRangeSamples), nSamples, [&]() {
    return otherPt +
           RobotPointSamplerHelper::SamplePointInAnnulus(rng, 1.f, 10.f);
  });

  // the "other point" itself should not be in range since we specified a
  // nonzero minDist_mm
  notInRangeSamples.push_back(otherPt);

  for (const auto& pt : inRangeSamples) {
    EXPECT_TRUE(rejectIfNotInRange(pt))
        << "Sample point " << pt
        << "unexpectedly returned false for rejectIfNotInRange";
  }
  for (const auto& pt : notInRangeSamples) {
    EXPECT_FALSE(rejectIfNotInRange(pt))
        << "Sample point " << pt
        << "unexpectedly returned true for rejectIfNotInRange";
  }
}
...
TEST(TestRobotPosSampler, ChargerNotInView) {
  ...
  // Generate some test samples which are definitely in range or out of range
  // The charger's origin is at the front of the charger 'lip', and the x axis
  // points into the charger (toward the marker). The z axis points up.
  //
  // This means that points that are in front of the charger should have
  ...
  std::generate_n(std::back_inserter(inViewSamples), nSamples, [&]() {
    Point2f pt;
    do {
      pt = RobotPointSamplerHelper::SamplePointInAnnulus(rng, minDist_mm,
                                                         maxDist_mm);
    } while (pt.x() >= 0);
    return pt;  // pt.x() should be negative here, which should satisfy the
                // condition
  });
  std::generate_n(std::back_inserter(notInViewSamples), nSamples, [&]() {
    ...
    return pt;  // pt.x() should be positive here, which should fail to satisfy
                // the condition
  });

  for (const auto& pt : inViewSamples) {
    EXPECT_TRUE(rejectIfChargerOutOfView(pt))
        << "Sample point " << pt
        << "unexpectedly returned false for rejectIfChargerOutOfView";
  }
  for (const auto& pt : notInViewSamples) {
    EXPECT_FALSE(rejectIfChargerOutOfView(pt))
        << "Sample point " << pt
        << "unexpectedly returned true for rejectIfChargerOutOfView";
  }
