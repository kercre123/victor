...
Anki::Vector::CozmoContext* cozmoContext =
    nullptr;  // This is externed and used by tests

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - - Test listener that fails the test if it ends with the global error
// flag set to true
 ...
 private:
  ...
  virtual void OnTestEnd(const ::testing::TestInfo& test_info) {
    if (Anki::Util::sGetErrG()) {
      // unset it if this test started without it set. The only way errG will
      // start off set is if another thread sets it, in which case we preserve
      // errG so that the return value of main() will be nonzero
      if (!_errG) {
...
TEST(DataPlatform, ReadWrite) {
  ...
  const bool readSuccess = cozmoContext->GetDataPlatform()->readAsJson(
      Anki::Util::Data::Scope::Resources, "config/engine/configuration.json",
      config);
  EXPECT_TRUE(readSuccess);

  config["blah"] = 7;
  const bool writeSuccess = cozmoContext->GetDataPlatform()->writeAsJson(
      Anki::Util::Data::Scope::Cache, "someRandomFolder/A/writeTest.json",
      config);
  EXPECT_TRUE(writeSuccess);

  std::string someRandomFolder =
      cozmoContext->GetDataPlatform()->pathToResource(
          Anki::Util::Data::Scope::Cache, "someRandomFolder");
  Anki::Util::FileUtils::RemoveDirectory(someRandomFolder);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - - - - -
TEST(Cozmo, SimpleCozmoTest) { ASSERT_TRUE(true); }
...
TEST(BlockWorld, AddAndRemoveObject) {
  ...
  Vision::Marker::Code testCode =
      testCube.GetMarker(Block::FaceName::FRONT_FACE).GetCode();
  ...
  // Calibration values from Sept 1, 2015 - on 4.1 robot headboard with SSID
  // 3a97
  ...
  */
  ...
  auto camCalib = std::make_shared<Vision::CameraCalibration>(
      HEAD_CAM_CALIB_HEIGHT, HEAD_CAM_CALIB_WIDTH,
  ...
  // stretch observed bottom side slightly to make computed pose definitely not
  // flat (so it doesn't get clamped)
  ...
  corners[Quad::BottomLeft] = {xcen - markerHalfSize * kNotFlatFraction,
                               ycen + markerHalfSize};
  corners[Quad::TopRight] = {xcen + markerHalfSize, ycen - markerHalfSize};
  corners[Quad::BottomRight] = {xcen + markerHalfSize * kNotFlatFraction,
                                ycen + markerHalfSize};
  Vision::ObservedMarker marker(stateMsgTimestamp, testCode, corners,
                                robot.GetVisionComponent().GetCamera());

  // Enable "vision while moving" so that we don't have to deal with trying to
  // compute angular velocities, since we don't have real state history to do
  // so.
  robot.GetVisionComponent().EnableVisionWhileRotatingFast(true);

  // Queue the marker in VisionComponent, which will in turn queue it for
  // processing by BlockWorld
  ...
  // There should be an object of the right type, with the right ID in
  // BlockWorld
  {
    BlockWorldFilter filter;
    std::vector<ObservableObject*> objects;
    ...
    // Projected corners of the marker should match the corners of the fake
    // marker we queued above
    std::vector<const Vision::KnownMarker*> observedMarkers;
    object->GetObservedMarkers(observedMarkers, 1);
    ASSERT_EQ(1, observedMarkers.size());  // Should only have seen one

    Pose3d markerPoseWrtCamera;
    const bool poseResult = observedMarkers[0]->GetPose().GetWithRespectTo(
        robot.GetVisionComponent().GetCamera().GetPose(), markerPoseWrtCamera);
    ASSERT_TRUE(poseResult);

    auto obsCorners3d = observedMarkers[0]->Get3dCorners(markerPoseWrtCamera);
    Quad2f obsCorners;
    robot.GetVisionComponent().GetCamera().Project3dPoints(obsCorners3d,
                                                           obsCorners);
    const f32 kCornerTol_pix = 0.75f;
    const bool cornersMatch =
        IsNearlyEqual(obsCorners, corners, kCornerTol_pix);
  ...
  {
    // should be unknown now. In the new code Unknown objects are removed from
    // their origin
    ObservableObject* object = blockWorld.GetLocatedObjectByID(firstObjID);
  ...
  for (s32 i = 0; i < kNumObservations; ++i) {
    // Tick the robot, which will tick the BlockWorld, which will use the queued
    // marker
    stateMsgTimestamp += 10;
    lastResult = robot.UpdateFullRobotState(fakeRobotState(stateMsgTimestamp));
    ASSERT_EQ(RESULT_OK, lastResult);
    Vision::ObservedMarker markerFar(stateMsgTimestamp, testCode, corners,
                                     robot.GetVisionComponent().GetCamera());
  ...
  {
    // Now try clearing the object, and make sure we can't still get it using
    // the second ID
    ObservableObject* objectBeforeClear =
        blockWorld.GetLocatedObjectByID(secondObjID);
    ASSERT_NE(objectBeforeClear, nullptr);

    blockWorld.ClearLocatedObject(objectBeforeClear);

    ObservableObject* objectAfterClear =
        blockWorld.GetLocatedObjectByID(secondObjID);
    ...
    ObservableObject* objectAfterDelete =
        blockWorld.GetLocatedObjectByID(secondObjID);
...
static Anki::Result ObserveMarkerHelper(
    const s32 kNumObservations,
    std::list<std::pair<Anki::Vision::Marker::Code, Anki::Quad2f>>&&
        codesAndCorners,
    Anki::RobotTimeStamp_t& fakeTime, Anki::Vector::Robot& robot,
  ...
  for (s32 i = 0; i < kNumObservations; ++i, fakeTime += 10) {
    ...
    if (RESULT_OK != lastResult) {
      PRINT_NAMED_ERROR("ObservedMarkerHelper.UpdateFullRobotStateFailed",
                        "i=%d fakeTime=%u", i, (TimeStamp_t)fakeTime);
      return lastResult;
    }
    ...
    for (auto& codeAndCorners : codesAndCorners) {
      procResult.observedMarkers.push_back(Vision::ObservedMarker(
          (TimeStamp_t)fakeTime, codeAndCorners.first, codeAndCorners.second,
          robot.GetVisionComponent().GetCamera()));
    }
    ...
    if (RESULT_OK != lastResult) {
      PRINT_NAMED_ERROR("ObservedMarkerHelper.UpdateVisionMarkersFailed",
                        "i=%d fakeTime=%u", i, (TimeStamp_t)fakeTime);
      return lastResult;
    }
  }
...
static Anki::Result FakeRobotMovement(Anki::Vector::Robot& robot,
  ...
  stateMsg.timestamp = (TimeStamp_t)fakeTime;
  stateMsg.status |=
      (u16)RobotStatusFlag::ARE_WHEELS_MOVING;  // Set moving flag
  Result lastResult = robot.UpdateFullRobotState(stateMsg);
  stateMsg.status &=
      ~(u16)RobotStatusFlag::ARE_WHEELS_MOVING;  // Unset moving flag
  fakeTime += 10;
...
TEST(BlockWorld, UpdateObjectOrigins) {
  ...
  const Vision::Marker::Code farCode =
      farCube.GetMarker(Block::FaceName::FRONT_FACE).GetCode();
  const Vision::Marker::Code closeCode =
      closeCube.GetMarker(Block::FaceName::FRONT_FACE).GetCode();

  const Quad2f farCorners{Point2f(213, 111), Point2f(214, 158),
                          Point2f(260, 111), Point2f(258, 158)};

  const Quad2f closeCorners{Point2f(67, 117), Point2f(70, 185),
                            Point2f(136, 116), Point2f(137, 184)};

  const ObjectID farID = robot.GetBlockWorld().AddConnectedBlock(
      0, "AA:AA:AA:AA:AA:AA", ObjectType::Block_LIGHTCUBE1);
  ASSERT_TRUE(farID.IsSet());

  const ObjectID closeID = robot.GetBlockWorld().AddConnectedBlock(
      1, "BB:BB:BB:BB:BB:BB", ObjectType::Block_LIGHTCUBE2);
  ...
  const ObservableObject* matchingObject =
      robot.GetBlockWorld().FindLocatedMatchingObject(filter);
  ...
  lastResult =
      ObserveMarkerHelper(kNumObservations, {{closeCode, closeCorners}},
                          fakeTime, robot, stateMsg, procResult);
  ASSERT_EQ(RESULT_OK, lastResult);
  ...
  lastResult = ObserveMarkerHelper(
      kNumObservations, {{farCode, farCorners}, {closeCode, closeCorners}},
      fakeTime, robot, stateMsg, procResult);
  ASSERT_EQ(RESULT_OK, lastResult);
  ...
  // Seeing both objects again, now that we've moved, should relocalize (and not
  // crash!)
  lastResult =
      ObserveMarkerHelper(1, {{farCode, farCorners}, {closeCode, closeCorners}},
                          fakeTime, robot, stateMsg, procResult);
  ASSERT_EQ(RESULT_OK, lastResult);
...
TEST(FactoryTest, IdealCameraPose) {
  ...
  auto calib = std::make_shared<Vision::CameraCalibration>(240, 320, 290.f,
                                                           290.f, 160.f, 120.f);
  const f32 kDotSpacingX_mm = 40.f;
  const f32 kDotSpacingY_mm = 27.f;
  const f32 kTargetDist_mm = 60.f;
  const Quad3f kTargetDotCenters(
  ...
  for (f32 pitch_deg : kPitchAngle_deg) {
    for (f32 yaw_deg : kYawAngle_deg) {
      for (f32 roll_deg : kRollAngle_deg) {
        ...
        robot.GetVisionComponent().GetCamera().Project3dPoints(targetDotCenters,
                                                               imgQuad);

        Result result =
            robot.GetVisionComponent().ComputeCameraPoseVsIdeal(imgQuad, pose);
...

        {
          ...
          EXPECT_NEAR(pose.GetRotation().GetAngleAroundXaxis().getDegrees(),
                      pitch_deg, kAngleTol_deg);
          EXPECT_NEAR(pose.GetRotation().GetAngleAroundYaxis().getDegrees(),
                      yaw_deg, kAngleTol_deg);
          EXPECT_NEAR(pose.GetRotation().GetAngleAroundZaxis().getDegrees(),
                      roll_deg, kAngleTol_deg);
        }

        PRINT_NAMED_INFO(
            "FactoryTest.IdealCameraPose.IdealHeadPose",
            "position=(%.1f,%.1f,%.1f)mm Roll=%.1fdeg (vs %.1f) Pitch=%.1fdeg "
            "(vs %.1f), Yaw=%.1fdeg (vs %.1f)",
        ...
        PRINT_NAMED_INFO(
            "FactoryTest.IdealCameraPose.ImperfectCalib",
            "position=(%.1f,%.1f,%.1f)mm Roll=%.1fdeg (vs %.1f) Pitch=%.1fdeg "
            "(vs %.1f), Yaw=%.1fdeg (vs %.1f)",
            pose.GetTranslation().x(), pose.GetTranslation().y(),
            pose.GetTranslation().z(),
...
TEST(FactoryTest, FindDotsInImages) {
  ...
  std::vector<TestData> tests = {
      {
          ...
          .calib = std::make_shared<Vision::CameraCalibration>(
              240, 320, 290.f, 290.f, 160.f, 120.f),
      },
      {
  ...
  for (auto& test : tests) {
    const std::string testFilename =
        cozmoContext->GetDataPlatform()->pathToResource(
            Util::Data::Scope::Resources, test.filename);
    ...
    lastResult =
        robot.GetVisionComponent().FindFactoryTestDotCentroids(image, msg);
    ASSERT_EQ(lastResult, RESULT_OK);
    ...
    for (s32 iDot = 0; iDot < 4; ++iDot) {
      centroidStr += "(" + std::to_string(msg.dotCenX_pix[iDot]) + "," +
                     std::to_string(msg.dotCenY_pix[iDot]) + ") ";
    }
    ...
    PRINT_NAMED_INFO("FactoryTest.FindDotsInImages.HeadPose",
                     "'%s'[%s]: position=(%.1f,%.1f,%.1f)mm Roll=%.1fdeg "
                     "Pitch=%.1fdeg, Yaw=%.1fdeg",
...
TEST(ActionableObject, PreActionPoseCaching) {
  ...
  ASSERT_EQ(cube.GetCurrentPreActionPoses(preActionPoses, robotPose,
                                          {PreActionPose::ActionType::DOCKING}),
            true);

  std::vector<PreActionPose> cachedPreActionPoses;
  // Should be using the cached preAction poses
  ASSERT_EQ(cube.GetCurrentPreActionPoses(cachedPreActionPoses, robotPose,
                                          {PreActionPose::ActionType::DOCKING}),
            false);

  std::vector<PreActionPose> flippingPoses;
  // Make sure that only DOCKING preAction poses were cached, all other
  // ActionTypes should still need to be generated
  ASSERT_EQ(
      cube.GetCurrentPreActionPoses(flippingPoses, robotPose,
                                    {PreActionPose::ActionType::FLIPPING}),
      true);
  ...
  for (int i = 0; i < preActionPoses.size(); ++i) {
    ASSERT_EQ(preActionPoses[i].GetPose().IsSameAs(
                  cachedPreActionPoses[i].GetPose(), 0, 0),
              true);
  }
  ...
  // Should need to regenerate preAction poses since our cached poses were
  // cleared
  ASSERT_EQ(cube.GetCurrentPreActionPoses(preActionPoses, robotPose,
                                          {PreActionPose::ActionType::DOCKING}),
            true);
...
TEST(BlockWorld, ObjectRobotCollisionCheck) {
  ...
  ObservableObject* object = CubePlacementHelper::CreateObjectLocatedAtOrigin(
      robot, ObjectType::Block_LIGHTCUBE1);
  ...
  // Put the robot somewhere "random" (to avoid possible special case of
  // reasoning near origin)
  const Pose3d robotPose(M_PI_4_F, Z_AXIS_3D(), {123.f, 456.f, 0.f},
                         robot.GetWorldOrigin());
  robot.SetPose(robotPose);

  // Helper lambda for doing test with object at different heights
  auto CheckPoseStateAtHeight = [&robot, object,
                                 &fakeTime](f32 objHeight) -> PoseState {
    // Object is always colocated with robot, but at different heights
    const Point2f robotPosition(robot.GetPose().GetTranslation());
    Pose3d pose(0.f, Z_AXIS_3D(),
                {robotPosition.x(), robotPosition.y(), objHeight});
    ...
    // Set object's new pose with fake observations
    {
      ...
      auto result = robot.GetBlockWorld().SetObjectPose(object->GetID(), pose,
                                                        PoseState::Known);
      if (!ANKI_VERIFY(result == RESULT_OK,
                       "BlockWorld.ObjectRobotCollisionCheck.HelperLambda."
                       "FailedUpdateBlockworld",
                       "")) {
  ...
  ASSERT_EQ(PoseState::Dirty,
            CheckPoseStateAtHeight(object->GetSize().z() * 0.5f));

  // Object directly above robot should leave object Known
  ASSERT_EQ(PoseState::Known,
            CheckPoseStateAtHeight(ROBOT_BOUNDING_Z + object->GetSize().z()));

  // Object center below ground but still sticking up above ground should be
  // Dirty
  ASSERT_EQ(PoseState::Dirty,
            CheckPoseStateAtHeight(-0.5f * object->GetSize().z() + 5.f));

  // Object completely below ground under the robot should remain Known
  ASSERT_EQ(PoseState::Known,
            CheckPoseStateAtHeight(-1.5 * object->GetSize().z()));
...
TEST(Localization, UnexpectedMovement) {
  ...
  const Quad2f closeCorners1{Point2f(67, 117), Point2f(70, 185),
                             Point2f(136, 116), Point2f(137, 184)};
  ...
  auto FakeUnexpectedMovement = [](RobotState& stateMsg, Robot& robot,
                                   RobotTimeStamp_t& fakeTime) -> Result {
  ...
  // Fake unexpected movement until we are one state message away from
  // triggering it
  for (int i = 0; i < robot.GetMoveComponent().GetMaxUnexpectedMovementCount();
       ++i) {
  ...
  // Fake one more unexpected movement which used to cause unexpected movement
  // to fully trigger setting the robot's pose to a pose in history (when the
  // unexpected movement started). This would be from an old origin, before the
  // delocalize, causing the robot's pose's origin to be different from the
  // robot's world origin
  FakeUnexpectedMovement(stateMsg, robot, fakeTime);
...
int main(int argc, char** argv) {
  ...
  // Since some tests are testing for changes to the global error flag in a MT
  // environment, block access to the flag in some circumstances
  Anki::Util::_lockErrG = true;

  // LEAKING HERE
  Anki::Util::PrintfLoggerProvider* loggerProvider =
      new Anki::Util::PrintfLoggerProvider();
  ...
  if (configRoot.empty() || workRoot.empty()) {
    ...
    /*  // still troubleshooting different run time environments,
        // need to find a way to detect where the resources folder is located on
    disk.
  ...
  // LEAKING HERE
  Anki::Util::Data::DataPlatform* dataPlatform =
      new Anki::Util::Data::DataPlatform(persistentPath, cachePath,
                                         resourcePath);
  UiMessageHandler handler(0);
  ProtoMessageHandler protoHandler;
  cozmoContext =
      new Anki::Vector::CozmoContext(dataPlatform, &handler, &protoHandler);
  ...
  // std::string cache =
  // dataPlatform->pathToResource(Anki::Vector::Data::Scope::Cache, "");
  // Anki::Util::FileUtils::RemoveDirectory(cache);
  // std::string files =
  // dataPlatform->pathToResource(Anki::Vector::Data::Scope::Output, "");
  ...
  // Tests must not set the global error flag (via calls to PRINT_NAMED_ERROR or
  // ANKI_VERIFY). ListenerFailOnErrFlag checks for this, but unsets the global
  // error flag so that not all tests look like they're failing. This final case
  // for testHadError will catch any threads that set the global error flag
