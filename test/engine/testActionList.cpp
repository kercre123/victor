...
constexpr uint8_t track1 = (u8)AnimTrackFlag::BODY_TRACK |
                           (u8)AnimTrackFlag::HEAD_TRACK |
                           (u8)AnimTrackFlag::LIFT_TRACK;
constexpr uint8_t track2 =
    (u8)AnimTrackFlag::BACKPACK_LIGHTS_TRACK | (u8)AnimTrackFlag::AUDIO_TRACK;
constexpr uint8_t track3 =
    (u8)AnimTrackFlag::FACE_TRACK | (u8)AnimTrackFlag::EVENT_TRACK;
...
class TestAction : public IAction {
 public:
  TestAction(Robot& r, const std::string& name, RobotActionType type,
             u8 tracks = 0);
...
TestAction::TestAction(Robot& r, const std::string& name, RobotActionType type,
                       u8 tracks)
...
class TestInterruptAction : public IAction {
 public:
  TestInterruptAction(Robot& r, const std::string& name, RobotActionType type,
                      u8 tracks = 0);
...
TestInterruptAction::TestInterruptAction(Robot& r, const std::string& name,
                                         RobotActionType type, u8 tracks)
    : IAction(name, type, tracks) {
  _complete = false;
  SetRobot(&r);
}
...
class TestCompoundActionSequential : public CompoundActionSequential {
 public:
  TestCompoundActionSequential(
      Robot& r, const std::initializer_list<IActionRunner*>& actions,
      const std::string& name);
  virtual ~TestCompoundActionSequential() { actionsDestroyed.push_back(_name); }
  virtual std::list<std::shared_ptr<IActionRunner>>& GetActions() {
    return _actions;
  }
...
TestCompoundActionSequential::TestCompoundActionSequential(
    Robot& r, const std::initializer_list<IActionRunner*>& actions,
    const std::string& name)
...
TestCompoundActionParallel::TestCompoundActionParallel(
    Robot& r, const std::initializer_list<IActionRunner*>& actions,
    const std::string& name)
...
class TestActionWithinAction : public IAction {
 public:
  TestActionWithinAction(Robot& r, const std::string& name,
                         RobotActionType type);
  virtual ~TestActionWithinAction() {
    _compoundAction.PrepForCompletion();  // does nothing if already called
...
TestActionWithinAction::TestActionWithinAction(Robot& r,
                                               const std::string& name,
                                               RobotActionType type)
    : IAction(name, type, track1 | track2), _compoundAction(r, {}, "Comp1") {
  _complete = false;
  SetRobot(&r);
}

ActionResult TestActionWithinAction::Init() {
  _compoundAction.AddAction(
      new TestAction(GetRobot(), "Test1", RobotActionType::WAIT, track1));
  _compoundAction.AddAction(
      new TestAction(GetRobot(), "Test2", RobotActionType::WAIT, track2));
...
class TestActionThatCancels : public IAction {
 public:
  TestActionThatCancels(Robot& r, const std::string& name, RobotActionType type,
                        u8 tracks = 0);
...
TestActionThatCancels::TestActionThatCancels(Robot& r, const std::string& name,
                                             RobotActionType type, u8 tracks)
    : IAction(name, type, tracks) {
  SetRobot(&r);
}
...
TEST(QueueAction, SingleActionFinishes) {
  ...
  TestAction* testAction =
      new TestAction(r, "Test", RobotActionType::WAIT, track1);
...
TEST(QueueAction, ThreeActionsFinish) {
  ...
  TestAction* testAction1 =
      new TestAction(r, "Test1", RobotActionType::WAIT, track1);
  TestAction* testAction2 =
      new TestAction(r, "Test2", RobotActionType::WAIT, track2);
  TestAction* testAction3 =
      new TestAction(r, "Test3", RobotActionType::WAIT, track3);
...
// Tests queueing three actions, cancelling the second and letting the rest
// complete normally
TEST(QueueAction, ThreeActionsCancelSecond) {
  ...
  TestAction* testAction1 =
      new TestAction(r, "Test1", RobotActionType::WAIT, track1);
  TestAction* testAction2 =
      new TestAction(r, "Test2", RobotActionType::TURN_IN_PLACE, track2);
  TestAction* testAction3 =
      new TestAction(r, "Test3", RobotActionType::TRACK_OBJECT, track3);
...
// Tests queueing two actions where the first one, which hasn't started running,
// is interruptable so queueing the second action doesn't interrupt the first
// but instead adds it to the front of the queue.
TEST(QueueAction, InterruptActionNotRunning) {
  Robot r(0, cozmoContext);
  auto* testAction = new TestAction(r, "Test1", RobotActionType::WAIT, track1);
  auto* testInterruptAction = new TestInterruptAction(
      r, "Test2", RobotActionType::TURN_IN_PLACE, track2);
...
// Tests queueing two actions where the first one, which has started running, is
// interruptable so queueing the second action interrupts it. The second action
// completes and the interrupted action resumes and completes.
...
// Tests queueing a sequential compound action made of two actions and both
// actions complete at the same time
TEST(QueueAction, CompoundActionSequentialSingle) {
  ...
  auto* compoundAction =
      new TestCompoundActionSequential(r, {testAction1, testAction2}, "Comp1");
...
// Tests queueing a sequential compound action made of two actions and they
...
// Tests queueing a sequential compound action that initially has two actions
// and a third action is added to the compound action after an update
  ...
  watcher.RegisterActionEndedCallbackForAllActions(
      [&completedTags](
        ...
        EXPECT_TRUE(inserted)
            << "set already contained tag " << completion.idTag;
      });
...
// Tests queueing an empty sequential compound action and adding three
// additional actions to it at various times
...
// Tests queueing a parallel compound action with two actions that complete
// immediately
TEST(QueueAction, ParallelActionImmediateComplete) {
  ...
  auto* testAction2 = new TestAction(r, "Test2", RobotActionType::WAIT, track2);
  auto* compoundAction =
      new TestCompoundActionParallel(r, {testAction1, testAction2}, "Comp1");
...
// Tests queueing a parallel compound action with two actions where one
...
TEST(QueueAction, QueueNowAndClearRemaining) {
  ...
  actionList.QueueAction(QueueActionPosition::NOW_AND_CLEAR_REMAINING,
                         testAction3);
  CheckTracksUnlocked(r, track1);
...
TEST(QueueAction, QueueActionWithinAction) {
  Robot r(0, cozmoContext);
  auto* testActionWithinAction = new TestActionWithinAction(
      r, "TestActionWithinAction", RobotActionType::UNKNOWN);
  auto& actionList = r.GetActionList();
  ...
  // Sets the subActions of testActionWithinAction to be complete so that
  // testActionWithinAction can complete
  ...
  // TestActionWithinAction has a compound action with 2 actions for a total of
  // 4 actions In this case the parent action TestActionWithinAction succeeds
  // before the sub actions officially succeed (TestActionWithinAction is
  // checked if it is done before the sub actions) so it completes which
  // destroys the sub action and its actions, then itself
...
TEST(QueueAction, ActionFailureRetry) {
  ...
  // testAction1 completed and unlocked its tracks and testAction2 had to retry
  // so it also unlocked its tracks
  CheckTracksUnlocked(r, track1 | track2);
...
TEST(QueueAction, QueueDuplicate) {
  ...
  EXPECT_EQ(actionList.QueueAction(QueueActionPosition::AT_END, testAction1),
            Anki::RESULT_FAIL);
  EXPECT_EQ(actionList.QueueAction(QueueActionPosition::NEXT, testAction1),
            Anki::RESULT_FAIL);
  EXPECT_EQ(actionList.QueueAction(QueueActionPosition::NOW, testAction1),
            Anki::RESULT_FAIL);
  EXPECT_EQ(
      actionList.QueueAction(QueueActionPosition::NOW_AND_RESUME, testAction1),
      Anki::RESULT_FAIL);
  EXPECT_EQ(actionList.QueueAction(QueueActionPosition::NOW_AND_CLEAR_REMAINING,
                                   testAction1),
            Anki::RESULT_FAIL);
  EXPECT_EQ(
      actionList.QueueAction(QueueActionPosition::IN_PARALLEL, testAction1),
...
TEST(QueueAction, CancelFromDestructor) {
  ...
  auto* actionThatCancels = new TestActionThatCancels(
      r, "TestActionThatCancels", RobotActionType::WAIT);
...
TEST(QueueAction, CancelAlreadyCancelledFromDestructor) {
  ...
  auto* actionThatCancels = new TestActionThatCancels(
      r, "TestActionThatCancels", RobotActionType::WAIT);
  actionThatCancels->_tagsToCancelOnDestroy.push_back(
      actionThatCancels->GetTag());
  ...
  // Cancel by type
  TestActionThatCancels* actionThatCancels2 = new TestActionThatCancels(
      r, "TestActionThatCancels", RobotActionType::WAIT);
  actionThatCancels2->_tagsToCancelOnDestroy.push_back(
      actionThatCancels2->GetTag());
  ...
  // Cancel by clearing
  TestActionThatCancels* actionThatCancels3 = new TestActionThatCancels(
      r, "TestActionThatCancels", RobotActionType::WAIT);
  actionThatCancels3->_tagsToCancelOnDestroy.push_back(
      actionThatCancels3->GetTag());
...
TEST(QueueAction, ComplexNestedAction) {
  ...
  {
    // This Test1 Track locking intentionally results in Test2 not being able to
    // begin due to Test1 locking all tracks, which is verified below
    TestAction* testAction1 =
        new TestAction(r, "Test1", RobotActionType::WAIT, track1);
    TestAction* testAction2 =
        new TestAction(r, "Test2", RobotActionType::WAIT, track1);
    TestAction* testAction3 =
        new TestAction(r, "Test3", RobotActionType::WAIT, track3);
    TestAction* testAction4 =
        new TestAction(r, "Test4", RobotActionType::WAIT, track1);
    TestAction* testAction5 =
        new TestAction(r, "Test5", RobotActionType::WAIT, track2);
    TestCompoundActionSequential* compoundAction1 =
        new TestCompoundActionSequential(r, {}, "Comp1");
    TestCompoundActionSequential* compoundAction2 =
        new TestCompoundActionSequential(r, {}, "Comp2");
    TestCompoundActionParallel* compoundAction3 =
        new TestCompoundActionParallel(r, {}, "Comp3");
    TestCompoundActionParallel* compoundAction4 =
        new TestCompoundActionParallel(r, {}, "Comp4");
    ...
    testRef2 =
        compoundAction3->AddAction(testAction2, ignoreThisActionSuccessResult);
  ...
  std::set<u32> allTags = {testRef1.lock()->GetTag(), testRef2.lock()->GetTag(),
                           testRef3.lock()->GetTag(), testRef4.lock()->GetTag(),
                           testRef5.lock()->GetTag(), compRef1->GetTag(),
                           compRef2.lock()->GetTag(), compRef3.lock()->GetTag(),
  ...
  CheckActionDestroyedUnordered({"Test1", "Test2", "Test3", "Test4", "Test5",
...
TEST(ActionTag, ConflictingUnityTags) {
  ...
  const bool errGGotSet =
      CallWithoutError([&]() { EXPECT_FALSE(testAction2->SetTag(100)); });
  EXPECT_TRUE(errGGotSet);

  EXPECT_NE(testAction1->GetTag(), testAction2->GetTag());
  EXPECT_EQ(testAction2->GetTag(),
            origTag2);  // Tag will not change because of conflict

  // these are technically not started, but we assert if the action is destroyed
  // for any other state than NOT_STARTED. Here, they're BAD_TAG. But prep
  // anyway to avoid the assert
  testAction1->PrepForCompletion();
  testAction2->PrepForCompletion();
...
TEST(ActionTag, ConflictingNormalTags) {
  ...
  // cant set a tag that's in use
  bool errGGotSet =
      CallWithoutError([&]() { EXPECT_FALSE(testAction1->SetTag(origTag2)); });
  EXPECT_TRUE(errGGotSet);

  EXPECT_NE(testAction1->GetTag(), testAction2->GetTag());
  EXPECT_EQ(testAction1->GetTag(),
            origTag1);  // Tag will not change because of conflict

  // and in the other direction: cant set a tag that's in use
  errGGotSet =
      CallWithoutError([&]() { EXPECT_FALSE(testAction2->SetTag(origTag1)); });
  EXPECT_TRUE(errGGotSet);
...
TEST(ActionTag, ConflictAutoGeneratedTag) {
  ...
  // First auto-generated tag will conflict with the set tag of testAction1 so
  // testAction2's tag will be 2 greater than testAction1's instead of 1 greater
  TestAction* testAction2;
  bool errGGotSet = CallWithoutError([&]() {
    testAction2 = new TestAction(r, "Test2", RobotActionType::WAIT);
  });
  EXPECT_TRUE(errGGotSet);
...
TEST(ActionWatcher, AddAndRemoveCallbacks) {
  ...
  ActionEndedCallback cb1 =
      [&callbackResults](
          const ExternalInterface::RobotCompletedAction& completion) {
        callbackResults[1] = {completion.idTag, completion.actionType,
                              completion.result};
      };

  ActionEndedCallback cb2 =
      [&callbackResults](
          const ExternalInterface::RobotCompletedAction& completion) {
        callbackResults[2] = {completion.idTag, completion.actionType,
                              completion.result};
      };
  ...
  ActionEndedCallbackID cbID1 =
      watcher.RegisterActionEndedCallbackForAllActions(cb1);
  ...
  TestAction* testAction1 =
      new TestAction(robot, "Test1", RobotActionType::WAIT, track1);
  ...
  ActionEndedCallbackID cbID2 =
      watcher.RegisterActionEndedCallbackForAllActions(cb2);
  EXPECT_NE(cbID1, cbID2) << "ids should be unique";
  callbackResults.clear();

  TestAction* testAction2 =
      new TestAction(robot, "Test2", RobotActionType::WAIT, track1);
  ...
  TestAction* testAction3 =
      new TestAction(robot, "Test3", RobotActionType::WAIT, track1);
  ...
  TestAction* testAction4 =
      new TestAction(robot, "Test4", RobotActionType::WAIT, track1);
