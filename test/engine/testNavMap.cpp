...
TEST(TestNavMap, FillBorder) {
  // tests the content transformations performed by fillborder, but not the
  // locations of the quads. four region sizes:
  ...
  // we will fill from clear space to prox obstacles and replace it with a
  // cliff. Only the littleProx1 should be replaced, not littleProx2
  ...
  MemoryMapData_ProxObstacle proxObstacle(MemoryMapData_ProxObstacle::EXPLORED,
                                          {0.0f, 0.0f, 0.0f}, 0);
  ...
  MemoryMapData_ProxObstacle proxObstacle2(MemoryMapData_ProxObstacle::EXPLORED,
                                           {0.0f, 0.0f, 0.0f}, 0);
  ...
  const EContentTypePackedType nodeNeighborsToFillFrom =
      ConvertContentArrayToFlags(clearOfObstacleTypes);

  MemoryMapData_Cliff toReplace(Pose3d{""}, 0);
  NodePredicate innerType = [](const auto& inside) {
    return inside->type == EContentType::ObstacleProx;
  };
  NodePredicate outerType = [&](const auto& outside) {
    return IsInEContentTypePackedType(outside->type, nodeNeighborsToFillFrom);
  };
