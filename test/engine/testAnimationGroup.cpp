...
void DeserializeAnimationGroupContainerFromJson(
    AnimationGroupContainer& container, const std::string& name,
...
TEST(AnimationGroupContainer, AnimationGroupContainerDeserialization) {
  ...
  DeserializeAnimationGroupContainerFromJson(container, "b",
                                             kOneAnimationDefaultMoodJson);

  EXPECT_EQ(2, container.GetAnimationGroupNames().size());

  DeserializeAnimationGroupContainerFromJson(container, "c",
                                             kOneAnimationHighStimJson);

  EXPECT_EQ(3, container.GetAnimationGroupNames().size());

  DeserializeAnimationGroupContainerFromJson(container, "d",
                                             kTwoAnimationsDefaultMoodsJson);

  EXPECT_EQ(4, container.GetAnimationGroupNames().size());

  DeserializeAnimationGroupContainerFromJson(
      container, "e", kTwoAnimationsHighStimDefaultMoodsJson);
  ...
  const bool err =
      CallWithoutError([&]() { group = container.GetAnimationGroup("a"); });
  EXPECT_TRUE(err);
...
TEST(AnimationGroup, GetAnimationNameBeforeCooldownSingle) {
  ...
  Anki::BaseStationTimer::getInstance()->UpdateTime(
      Anki::Util::SecToNanoSec(0.f));
  moodManager.UpdateDependent(dependencies);

  AnimationGroup group =
      DeserializeAnimationGroupFromJson(kOneAnimationDefaultMoodCooldownJson);
  ...
  Anki::BaseStationTimer::getInstance()->UpdateTime(
      Anki::Util::SecToNanoSec(5.f));
  moodManager.UpdateDependent(dependencies);

  const std::string& name2 =
      group.GetAnimationName(moodManager, groupContainer);
...
TEST(AnimationGroup, GetAnimationNameBeforeCooldownMultiple) {
  ...
  AnimationGroup group = DeserializeAnimationGroupFromJson(
      kTwoAnimationsDefaultMoodWithCooldownJson);

  std::string name;

  // either animation could be selected, so keep going until we get major win
  // (because that one has a cool down)
  int tries = 0;
...
TEST(AnimationGroup, GetDefaultAnimationNameUnweighted) {
  ...
  AnimationGroup group =
      DeserializeAnimationGroupFromJson(kOneAnimationDefaultMoodUnweightedJson);
...
// is astronomical.
void TestTwoAnimations100Times(const char* json, const MoodManager& moodManager,
                               AnimationGroupContainer& groupContainer,
                               bool& foundMajorWin,
                               bool& foundMajorWinBeatBox) {
...
TEST(AnimationGroup, GetEitherAnimationNameOfTwo) {
  ...
  TestTwoAnimations100Times(kTwoAnimationsDefaultMoodsJson, moodManager,
                            groupContainer, foundMajorWin,
                            foundMajorWinBeatBox);
...
TEST(AnimationGroup, GetNeitherAnimationNameOfTwo) {
  ...
  bool foundMajorWin = false, foundMajorWinBeatBox = false;
  const bool err = CallWithoutError([&]() {
    TestTwoAnimations100Times(kTwoAnimationsHighFrustratedMoodsJson,
                              moodManager, groupContainer, foundMajorWin,
                              foundMajorWinBeatBox);
  });
  EXPECT_TRUE(err);
...
TEST(AnimationGroup, GetFirstAnimationNameOfTwo) {
  ...
  bool foundMajorWin = false, foundMajorWinBeatBox = false;
  TestTwoAnimations100Times(kTwoAnimationsHighStimDefaultMoodsJson, moodManager,
                            groupContainer, foundMajorWin,
                            foundMajorWinBeatBox);
...
TEST(AnimationGroup, AllCLADTriggersHaveGroups) {
  ...
  for (size_t i = 0; i < AnimationTriggerNumEntries - 1; ++i) {
    ...
    EXPECT_TRUE(data->HasAnimationForTrigger(trigger))
        << "Could not find anim group for trigger "
