...
def ExportAnkiAnim(item):
    ...
    # Lock the other layers so that we don't try to edit everything.
    ...
    if rootLayer:
        ...
        for layer in layers:
            layer_lock_state[layer] = cmds.animLayer(layer, query=True, lock=True)
            cmds.animLayer(layer, edit=True, selected=False, lock=True)
        cmds.animLayer("BakeResults", edit=True, selected=True, solo=True, lock=False)
    ...
    #Reset anim layer state back to what it was before.
    for layer_name in layer_lock_state:
        cmds.animLayer(layer_name, edit=True, lock=layer_lock_state[layer_name])
