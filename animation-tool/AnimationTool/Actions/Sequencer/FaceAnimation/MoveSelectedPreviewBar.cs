...
namespace AnimationTool.FaceAnimation
{
    public class MoveSelectedPreviewBar : Action
    {
        ...
        public bool Do()
        {
            ...
            if(left)
            {
                double destination = Math.Round(previewBar.YValues[0] - MoveSelectedDataPoints.DELTA_X, 3);

                if (destination < Math.Round(dataPoint.YValues[0] - MoveSelectedDataPoints.DELTA_X, 3))
                {
                    return false;
                }
            ...
            else if (right)
            {
                ...
                if (destination > Math.Round(dataPoint.YValues[1], 3))
                {
                    return false;
                }
