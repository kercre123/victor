...
Anki::Switchboard::BleMessageProtocol::BleMessageProtocol(size_t size)
    : _maxSize(size), _receiveState(kMsgStart) {
  if (_maxSize < 1) {
    // todo: handle
  }
}

void Anki::Switchboard::BleMessageProtocol::ReceiveRawBuffer(uint8_t* buffer,
                                                             size_t size) {
  if (size < 1) {
    return;
  }
  ...
  switch (mulipartState) {
    ...
    case kMsgEnd: {
      ...
      // Notify
      _receiveMessageSignal.emit(reinterpret_cast<uint8_t*>(_buffer.data()),
                                 _buffer.size());
...
void Anki::Switchboard::BleMessageProtocol::SendMessage(uint8_t* buffer,
                                                        size_t size) {
