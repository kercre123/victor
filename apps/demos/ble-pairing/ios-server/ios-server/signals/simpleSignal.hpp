...
class ProtoSignal<R(Args...), Collector>
 ...
 private:
  ...
  class ProtoHandle : public HandleBase {
   public:
    ...
    ProtoHandle(ProtoSignal<R(Args...), Collector> *signalOwner,
                size_t signalId, std::shared_ptr<void> heartbeat)
        : _subject(signalOwner), _link(signalId), _heartbeatRef(heartbeat) {}
    virtual ~ProtoHandle() {}
  ...
  struct SignalLink {
    ...
    explicit SignalLink(const CbFunction &cbf)
        : next(NULL), prev(NULL), function(cbf), ref_count(1) {}
  ...
  std::shared_ptr<void>
      heartbeat_;  // for observers to tell if this signal is still alive
  ...
  /// Function to remove a signal handler through its connection ID, returns if
  /// a handler was removed.
  bool Unsubscribe(size_t connection) {
 ...
 public:
  ...
  ProtoSignal(const CbFunction &method)
      : callback_ring_(NULL),
        heartbeat_((void *)0x12345678,
                   [](void *) {})  // pointer isn't actually valid, so give
                                   // deleter that won't delete anything
  {
    if (method != NULL) {
      ensure_ring();
  ...
  /// If you call this function and don't store the return value, you're almost
  /// certainly doing it wrong. This handle can also be manually unsubscribed by
  /// assigning it nullptr.
  SmartHandle ScopedSubscribe(const CbFunction &cb)
      __attribute__((warn_unused_result)) {
    ensure_ring();
    return SmartHandle(new ScopedHandleContainer(
        new ProtoHandle(this, callback_ring_->add_before(cb), heartbeat_)));
  }

  /// Add callback to this signal dispatcher. If the signal dispatcher ever
  /// emit()s after your callback is no longer valid, very bad things will
  /// happen. ScopedSubscribe (above) is recommended.
