...
class BleMessageProtocol {
 public:
  ...
  BleMessageSignal& OnReceiveMessageEvent() { return _receiveMessageSignal; }

  BleMessageSignal& OnSendRawBufferEvent() { return _sendRawBufferSignal; }
 ...
 private:
  ...
  void Append(uint8_t* buffer, size_t size) {
    std::copy((buffer + 1), (buffer + 1) + size - 1,
