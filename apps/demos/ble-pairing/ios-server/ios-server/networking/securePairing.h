...
class SecurePairing {
 public:
  // Types
  using ReceivedWifiCredentialsSignal =
      Signal::Signal<void(std::string, std::string)>;
  ...
  // PIN Update Event
  UpdatedPinSignal& OnUpdatedPinEvent() { return _updatedPinSignal; }
 ...
 private:
  ...
  // Template Methods
  template <class T>
  typename std::enable_if<std::is_base_of<Anki::Switchboard::Message, T>::value,
                          int>::type
