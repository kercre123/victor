...
class WifiServer {
  using ClientConnectedSignal =
      Signal::Signal<void(Anki::Switchboard::WiFiNetworkStream*)>;
  using AcceptedSocketSignal =
      Signal::Signal<void(struct sockaddr* client_address, int size)>;
 ...
 private:
  ...
  static void AcceptCallback(struct ev_loop* loop, struct ev_io* watcher,
                             int revents);
};
}  // namespace Switchboard
}  // namespace Anki
