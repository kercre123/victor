...
class BLENetworkStream : public INetworkStream {
 public:
  BLENetworkStream(CBPeripheralManager* peripheral,
                   CBMutableCharacteristic* writeCharacteristic,
                   CBMutableCharacteristic* encryptedWriteCharacteristic) {
    _peripheralManager = peripheral;
    _writeCharacteristic = writeCharacteristic;
    _encryptedWriteCharacteristic = encryptedWriteCharacteristic;

    _bleMessageProtocolEncrypted =
        std::make_unique<BleMessageProtocol>(kMaxPacketSize);
    _bleMessageProtocolPlainText =
        std::make_unique<BleMessageProtocol>(kMaxPacketSize);

    _onSendRawHandle =
        _bleMessageProtocolEncrypted->OnSendRawBufferEvent().ScopedSubscribe(
            std::bind(&BLENetworkStream::HandleSendRawEncrypted, this,
                      std::placeholders::_1, std::placeholders::_2));
    _onReceiveHandle =
        _bleMessageProtocolEncrypted->OnReceiveMessageEvent().ScopedSubscribe(
            std::bind(&BLENetworkStream::HandleReceiveEncrypted, this,
                      std::placeholders::_1, std::placeholders::_2));

    _onSendRawPlainHandle =
        _bleMessageProtocolPlainText->OnSendRawBufferEvent().ScopedSubscribe(
            std::bind(&BLENetworkStream::HandleSendRawPlainText, this,
                      std::placeholders::_1, std::placeholders::_2));
    _onReceivePlainHandle =
        _bleMessageProtocolPlainText->OnReceiveMessageEvent().ScopedSubscribe(
            std::bind(&BLENetworkStream::HandleReceivePlainText, this,
                      std::placeholders::_1, std::placeholders::_2));
  }

  using SendSignal = Signal::Signal<void(uint8_t*, int, bool)>;

  SendSignal& OnSendEvent() { return _sendSignal; }
