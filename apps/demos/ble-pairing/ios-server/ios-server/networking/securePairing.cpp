...
Anki::Switchboard::SecurePairing::SecurePairing(INetworkStream* stream,
  ...
  _onReceivePlainTextHandle =
      _stream->OnReceivedPlainTextEvent().ScopedSubscribe(
          std::bind(&SecurePairing::HandleMessageReceived, this,
                    std::placeholders::_1, std::placeholders::_2));

  _onReceiveEncryptedHandle =
      _stream->OnReceivedEncryptedEvent().ScopedSubscribe(
          std::bind(&SecurePairing::HandleEncryptedMessageReceived, this,
                    std::placeholders::_1, std::placeholders::_2));
  ...
  _pairingTimeoutSignal.SubscribeForever(
      std::bind(&SecurePairing::HandleTimeout, this));
  ...
  ev_timer_init(&_handleTimeoutTimer.timer, &SecurePairing::sEvTimerHandler,
...
void Anki::Switchboard::SecurePairing::HandleInitialPair(
    uint8_t* publicKey, uint32_t publicKeyLength) {
...
void Anki::Switchboard::SecurePairing::HandleChallengeResponse(
    uint8_t* pingChallengeAnswer, uint32_t length) {
  bool success = false;
...
void Anki::Switchboard::SecurePairing::HandleMessageReceived(uint8_t* bytes,
                                                             uint32_t length) {
  _taskExecutor->WakeSync([this, bytes, length]() {
    ...
    Anki::Switchboard::SetupMessage messageType =
        (Anki::Switchboard::SetupMessage)bytes[0];
    ...
    switch (messageType) {
      case Anki::Switchboard::SetupMessage::MSG_HANDSHAKE: {
        if (_state == PairingState::Initial ||
          ...
          HandshakeMessage* handshakeMessage =
              Message::CastFromBuffer<HandshakeMessage>((char*)bytes);
      ...
      case Anki::Switchboard::SetupMessage::MSG_REQUEST_INITIAL_PAIR: {
        if (_state == PairingState::AwaitingPublicKey) {
          PublicKeyMessage* pKeyMsg =
              Message::CastFromBuffer<PublicKeyMessage>((char*)bytes);

          HandleInitialPair((uint8_t*)pKeyMsg->publicKey,
...
void Anki::Switchboard::SecurePairing::HandleEncryptedMessageReceived(
  ...
  _taskExecutor->WakeSync([this, bytes, length]() {
    ...
    // first byte has type
    Anki::Switchboard::SecureMessage messageType =
        (Anki::Switchboard::SecureMessage)bytes[0];
...
Anki::Switchboard::SecurePairing::SendPlainText(const T& message) {
  uint32_t length;
  uint8_t* buffer = (uint8_t*)Anki::Switchboard::Message::CastToBuffer<T>(
      (T*)&message, &length);
...
void Anki::Switchboard::SecurePairing::sEvTimerHandler(struct ev_loop* loop,
                                                       struct ev_timer* w,
                                                       int revents) {
  std::ostringstream ss;
