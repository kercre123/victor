...
class INetworkStream {
 public:
  virtual int SendPlainText(uint8_t* bytes, int length)
      __attribute__((warn_unused_result)) = 0;
  virtual int SendEncrypted(uint8_t* bytes, int length)
      __attribute__((warn_unused_result)) = 0;
 ...
 protected:
  int Encrypt(uint8_t* buffer, int length, uint8_t* output,
    ...
    int result = crypto_aead_xchacha20poly1305_ietf_encrypt(
        output, outputLength, buffer, length, nullptr, 0, nullptr,
        _EncryptNonce, _EncryptKey);
 ...
 private:
  ...
  int Decrypt(uint8_t* buffer, int length, uint8_t* output,
    ...
    int result = crypto_aead_xchacha20poly1305_ietf_decrypt(
        output, outputLength, nullptr, buffer, length, nullptr, 0,
        _DecryptNonce, _DecryptKey);
