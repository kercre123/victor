...
Anki::Util::Data::DataPlatform* createPlatform(
    const std::string& persistentPath, const std::string& cachePath,
  ...
  return new Anki::Util::Data::DataPlatform(persistentPath, cachePath,
                                            resourcesPath);
}
...
Anki::Util::Data::DataPlatform* createPlatform() {
  ...
  if (strlen(config_file_path)) {
    ...
    if (!reader.parse(jsonContents, config)) {
      PRINT_STREAM_ERROR(
          "victorWebServerMain.createPlatform.JsonConfigParseError",
          "json configuration parsing error: "
  ...
  if (config.isMember("DataPlatformPersistentPath")) {
    ...
    LOG_ERROR(
        "victorWebServerMain.createPlatform."
        "DataPlatformPersistentPathUndefined",
        "");
  }
  ...
  if (config.isMember("DataPlatformCachePath")) {
    ...
    LOG_ERROR(
        "victorWebServerMain.createPlatform.DataPlatformCachePathUndefined",
        "");
  }
  ...
  if (config.isMember("DataPlatformResourcesPath")) {
    ...
    LOG_ERROR(
        "victorWebServerMain.createPlatform.DataPlatformResourcesPathUndefined",
        "");
  }
...
int main(void) {
  ...
  Json::Value wsConfig;
  static const std::string& wsConfigPath =
      "webserver/webServerConfig_standalone.json";
  const bool success = dataPlatform->readAsJson(Util::Data::Scope::Resources,
                                                wsConfigPath, wsConfig);
  if (!success) {
