...
class WebService {
 public:
  ...
  void SendToWebSockets(const std::string& moduleName,
                        const Json::Value& data) const;

  inline void SendToWebViz(const std::string& moduleName,
                           const Json::Value& data) const {
    SendToWebSockets(moduleName, data);
  }

  // returns true if a client has subscribed to a given module name (or any
  // module if empty)
  bool IsWebVizClientSubscribed(const std::string& moduleName = {}) const;

  // subscribe to when a client connects and notifies the webservice that they
  // want data for moduleName
  ...
  OnWebVizSubscribedType& OnWebVizSubscribed(const std::string& moduleName) {
    return _webVizSubscribedSignals[moduleName];
  }

  // subscribe to when a client (who is listening to moduleName) sends data back
  // to the webservice
  using OnWebVizDataType =
      Signal::Signal<void(const Json::Value&, const SendToClientFunc&)>;
  OnWebVizDataType& OnWebVizData(const std::string& moduleName) {
    return _webVizDataSignals[moduleName];
  }

  // temp signals for the app sending data and requesting data
  using OnAppToEngineOnDataType =
      Signal::Signal<std::string(const std::string&)>;
  ...
  OnAppToEngineRequestDataType& OnAppToEngineRequestData() {
    return _appToEngineRequestData;
  }

  const std::string& getConsoleVarsTemplate();

  enum class WhichWebServer { Standalone = 0, Engine = 1, Anim = 2 };
  ...
  struct Request {
    Request(RequestType rt, const std::string& param1,
            const std::string& param2);
    Request(RequestType rt, const std::string& param1,
            const std::string& param2, const std::string& param3,
            ExternalCallback extCallback, void* cbdata);
    ...
    bool _done;  // Result has been used and now it's OK for main thread to
                 // delete this item
  ...
  void RegisterRequestHandler(std::string uri, mg_request_handler handler,
                              void* cbdata);
 ...
 private:
  ...
  static int HandleWebSocketsConnect(const struct mg_connection* conn,
                                     void* cbparams);
  static int HandleWebSocketsData(struct mg_connection* conn, int bits,
                                  char* data, size_t dataLen, void* cbparams);
  static void HandleWebSocketsClose(const struct mg_connection* conn,
                                    void* cbparams);
  ...
  void SendToWebSocket(struct mg_connection* conn,
                       const Json::Value& data) const;
  ...
  std::unordered_map<std::string, OnWebVizSubscribedType>
      _webVizSubscribedSignals;
