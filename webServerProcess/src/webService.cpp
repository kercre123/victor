...
enum { WebSocketsTypeText = 0x1, WebSocketsTypeCloseConnection = 0x8 };
...
class ExternalOnlyConsoleChannel : public Anki::Util::IConsoleChannel {
 public:
  ExternalOnlyConsoleChannel(char* outText, uint32_t outTextLength)
      : _outText(outText), _outTextLength(outTextLength), _outTextPos(0) {
  ...
  int WriteLogv(const char* format, va_list args) override {
    // Print to a temporary buffer first so we can use that for any required
    // logs
    const int printRetVal =
        vsnprintf(_tempBuffer, kTempBufferSize, format, args);
    ...
    if (printRetVal > 0) {
      if ((_outText != nullptr) && (_outTextLength > _outTextPos)) {
        ...
        const int outPrintRetVal = snprintf(&_outText[_outTextPos],
                                            remainingRoom, "%s\n", _tempBuffer);
        // note outPrintRetVal can be >remainingRoom (snprintf returns size
        // required, not size used) so this can make _outTextPos >
        // _outTextLength, but this is ok as we only use it when <
        // _outTextLength
  ...
  bool IsTTYLoggingEnabled() const override { return true; }
...
static int LogHandler(struct mg_connection* conn, void* cbdata) {
#if USE_DAS
...
static int ProcessRequest(
    struct mg_connection* conn, WebService::WebService::RequestType requestType,
    const std::string& param1, const std::string& param2,
    const std::string& param3 = "", bool waitAndSendResponse = true,
    WebService::WebService::ExternalCallback extCallback = nullptr,
    void* cbdata = nullptr) {
  if (s_ShuttingDown) {
    return 1;
  }

  WebService::WebService::Request* requestPtr =
      new WebService::WebService::Request(requestType, param1, param2, param3,
                                          extCallback, cbdata);

  struct mg_context* ctx = mg_get_context(conn);
  WebService::WebService* that =
      static_cast<WebService::WebService*>(mg_get_user_data(ctx));
  ...
  if (waitAndSendResponse) {
...

    {
      ...
      requestPtr->_readyCondition.wait_for(
          lk, seconds(kTimeoutDuration_s),
    ...
    if (!requestPtr->_resultReady) {
      ...
      requestPtr->_result =
          "Timed out after " + std::to_string(kTimeoutDuration_s) + " seconds";
    }
...
static int ConsoleVarsUI(struct mg_connection* conn, void* cbdata) {
  ...
  const int returnCode = ProcessRequest(
      conn, WebService::WebService::RequestType::RT_ConsoleVarsUI, category,
      standalone);

  return returnCode;
}

static int ConsoleVarSet(struct mg_connection* conn, void* cbdata) {
  const mg_request_info* info = mg_get_request_info(conn);
  ...
  const int returnCode = ProcessRequest(
      conn, WebService::WebService::RequestType::RT_ConsoleVarSet, query, "");

  return returnCode;
}

static int ConsoleVarGet(struct mg_connection* conn, void* cbdata) {
  const mg_request_info* info = mg_get_request_info(conn);
  ...
  const int returnCode = ProcessRequest(
      conn, WebService::WebService::RequestType::RT_ConsoleVarGet, key, "");

  return returnCode;
}

static int ConsoleVarList(struct mg_connection* conn, void* cbdata) {
  const mg_request_info* info = mg_get_request_info(conn);
  ...
  const int returnCode = ProcessRequest(
      conn, WebService::WebService::RequestType::RT_ConsoleVarList, key, "");

  return returnCode;
}

static int ConsoleFuncList(struct mg_connection* conn, void* cbdata) {
  const mg_request_info* info = mg_get_request_info(conn);
  ...
  const int returnCode = ProcessRequest(
      conn, WebService::WebService::RequestType::RT_ConsoleFuncList, key, "");

  return returnCode;
}

static int ConsoleFuncCall(struct mg_connection* conn, void* cbdata) {
  const mg_request_info* info = mg_get_request_info(conn);
  ...
  if (func.substr(0, 5) == "func=") {
    ...
    if (amp == std::string::npos) {
      ...
      // unescape '+' => ' ',  except if it follows a '\', in which case keep
      // the '+' and drop the '\'
    ...
    returnCode = ProcessRequest(
        conn, WebService::WebService::RequestType::RT_ConsoleFuncCall, func,
        args);
  }
...
static int ProcessRequestFromQueryString(
    struct mg_connection* conn, void* cbdata,
    WebService::WebService::RequestType type) {
  const mg_request_info* info = mg_get_request_info(conn);
  std::string request;
  if (info->content_length > 0) {
    char buf[info->content_length + 1];
...
static int TempAppToEngine(struct mg_connection* conn, void* cbdata) {
  return ProcessRequestFromQueryString(
      conn, cbdata, WebService::WebService::RequestType::RT_TempAppToEngine);
}

static int TempEngineToApp(struct mg_connection* conn, void* cbdata) {
  return ProcessRequestFromQueryString(
      conn, cbdata, WebService::WebService::RequestType::RT_TempEngineToApp);
}

WebService::WebService::Request::Request(
    RequestType rt, const std::string& param1, const std::string& param2,
...
static int dasinfo(struct mg_connection* conn, void* cbdata) {
  mg_printf(conn,
            "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nConnection: "
            "close\r\n\r\n");

// NOTE:  For some reason, android builds of the webserver are not getting
// USE_DAS defined properly
#if USE_DAS

  std::string dasString =
      "DAS: " + std::string(DASGetLogDir()) + " DASDisableNetworkReason:";
...
#endif
  const std::string& allowPerfPage =
      that->GetConfig()["allowPerfPage"].asString();
  const std::string& whichWebServer =
      std::to_string(that->GetConfig()["whichWebServer"].asInt());
  const std::string& allowConsoleVarsPage =
      that->GetConfig()["allowConsoleVarsPage"].asString();
  const std::string& allowPerfMetricPage =
      that->GetConfig()["allowPerfMetricPage"].asString();
  const int tickBudget_ms = that->GetConfig()["tickBudget_ms"].asInt();

  mg_printf(conn, "%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%d\n", title0.c_str(),
            title1.c_str(), startPage.c_str(), webotsSim.c_str(),
            allowPerfPage.c_str(), whichWebServer.c_str(),
            allowConsoleVarsPage.c_str(), allowPerfMetricPage.c_str(),
            tickBudget_ms);
  return 1;
}
...
static int GetMainRobotInfo(struct mg_connection* conn, void* cbdata) {
  ...
  mg_printf(conn, "%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n", serialNo.c_str(),
            ip.c_str(), buildConfig.c_str(), procVersion.c_str(),
            procCmdLine.c_str(), robotName.c_str(), osBuildVersion.c_str(),
            sha.c_str(), MACAddress.c_str(), SSID.c_str());
  return 1;
}
...
static int GetPerfStats(struct mg_connection* conn, void* cbdata) {
  ...
  if (active[kStat_MemoryInfo1] || active[kStat_MemoryInfo2]) {
    ...
    if (active[kStat_MemoryInfo1]) {
      // Memory use 1
      stat_mem1 = std::to_string(info.totalMem_kB) + "," +
                  std::to_string(info.freeMem_kB);
    }
    if (active[kStat_MemoryInfo2]) {
      // Memory use 2
      stat_mem2 = std::to_string(info.totalMem_kB) + "," +
                  std::to_string(info.availMem_kB);
    }
  }

  std::vector<std::string> stat_cpuStat;
  if (active[kStat_OverallCpu] || active[kStat_Cpu0] || active[kStat_Cpu1] ||
  ...
  if (active[kStat_UserDiskSpace]) {
    ...
    if (success) {
      stat_userDiskSpace =
          std::to_string(info.total_kB) + "," + std::to_string(info.avail_kB);
    } else {
  ...
  LOG_INFO("WebService.Perf", "GetPerfStats took %lld microseconds to read",
           elapsed_us);
  ...
  mg_printf(conn, "%s\n%s\n%s\n%s\n%s\n%s\n%s\n", stat_cpuFreq.c_str(),
            stat_temperature.c_str(), stat_uptime.c_str(),
            stat_idleTime.c_str(), stat_rtc.c_str(), stat_mem1.c_str(),
            stat_mem2.c_str());
  mg_printf(conn, "%s\n%s\n%s\n%s\n%s\n%s\n", stat_cpuStat[0].c_str(),
            stat_cpuStat[1].c_str(), stat_cpuStat[2].c_str(),
            stat_cpuStat[3].c_str(), stat_cpuStat[4].c_str(),
...
static int SystemCtl(struct mg_connection* conn, void* cbdata) {
  ...
  if (query.substr(0, 5) == "proc=") {
    ...
    if (amp != std::string::npos) {
      ...
      LOG_INFO("WebService.Systemctl", "SystemCtl took %lld microseconds",
               elapsed_us);
    }
  }
  mg_printf(conn,
...
static int GetProcessStatus(struct mg_connection* conn, void* cbdata) {
  ...
  if (query.substr(0, 5) == "proc=") {
    struct mg_context* ctx = mg_get_context(conn);
    Anki::Vector::WebService::WebService* that =
        static_cast<Anki::Vector::WebService::WebService*>(
    ...
    static const long long kTimeoutDuration_s = 10;
    {
      ...
      s_ProcessStatusCondition.wait_for(lk, seconds(kTimeoutDuration_s), [] {
        return !s_WaitingForProcessStatus;
      });

      if (s_WaitingForProcessStatus) {
        LOG_INFO("WebService.GetProcessStatus",
                 "GetProcessStatus timed out after %lld seconds",
                 kTimeoutDuration_s);
      }
  ...
  const auto now = steady_clock::now();
  const auto elapsed_us = duration_cast<microseconds>(now - startTime).count();
  LOG_INFO("WebService.GetProcessStatus.Time",
           "GetProcessStatus took %lld microseconds", elapsed_us);
...
WebService::WebService() : _ctx(nullptr) {}
...
void WebService::Start(Anki::Util::Data::DataPlatform* platform,
  ...
  const std::string webserverPath =
      platform->pathToResource(Util::Data::Scope::Resources, "webserver");

  std::string rewrite;
  rewrite += "/persistent=" +
             platform->pathToResource(Util::Data::Scope::Persistent, "");
  rewrite += ",";
  rewrite += "/resources=" +
             platform->pathToResource(Util::Data::Scope::Resources, "");
  rewrite += ",";
  rewrite += "/cache=" + platform->pathToResource(Util::Data::Scope::Cache, "");
  rewrite += ",";
  rewrite += "/currentgamelog=" +
             platform->pathToResource(Util::Data::Scope::CurrentGameLog, "");
#if USE_DAS
  rewrite += ",";
  ...
  const char* options[] = {"document_root", webserverPath.c_str(),
                           "listening_ports", portNumString.c_str(),
                           "num_threads", "4", "url_rewrite_patterns",
                           rewrite.c_str(), "static_file_max_age", "0",
                           // https://ankiinc.atlassian.net/browse/VIC-1554
  ...
  mg_set_websocket_handler(_ctx, "/socket", HandleWebSocketsConnect,
                           HandleWebSocketsReady, HandleWebSocketsData,
                           HandleWebSocketsClose, 0);
  ...
  const std::string& consoleVarsTemplate = platform->pathToResource(
      Util::Data::Scope::Resources, "webserver/consolevarsui.html");
  _consoleVarsUIHTMLTemplate =
      Anki::Util::StringFromContentsOfFile(consoleVarsTemplate);
...
void WebService::Update() {
  ...
  // First pass:  Delete any completely-finished requests from the list (and
  ...
  Anki::Util::ConsoleSystem& consoleSystem =
    ...
    if (!requestPtr->_resultReady) {
      switch (requestPtr->_requestType) {
        ...
        case RT_ConsoleVarGet: {
          ...
          if (!key.empty()) {
            const Anki::Util::IConsoleVariable* consoleVar =
                consoleSystem.FindVariable(key.c_str());
            if (consoleVar) {
            ...
            if (key.substr(0, 4) == "key=") {
              ...
              if (consoleVar) {
                if (consoleVar->ParseText(value.c_str())) {
                  // success
                  LOG_INFO("WebService", "CONSOLE_VAR %s %s", key.c_str(),
                           value.c_str());
                  requestPtr->_result += consoleVar->ToString() + "<br>";
                } else {
                  requestPtr->_result +=
                      "Error setting variable " + key + "=" + value + "<br>";
                }
              } else {
        ...
        case RT_ConsoleVarList: {
          ...
          for (Anki::Util::ConsoleSystem::VariableDatabase::const_iterator it =
                   varDatabase.begin();
               it != varDatabase.end(); ++it) {
            std::string label = it->second->GetID();
            if (keyLen == 0 || Anki::Util::StringCaseInsensitiveEquals(
                                   label.substr(0, keyLen), key)) {
        ...
        case RT_ConsoleFuncList: {
          ...
          for (Anki::Util::ConsoleSystem::FunctionDatabase::const_iterator it =
                   funcDatabase.begin();
        ...
        case RT_ConsoleFuncCall: {
          ...
          Anki::Util::IConsoleFunction* consoleFunc =
              consoleSystem.FindFunction(func.c_str());
          if (consoleFunc) {
            ...
            bool success = consoleSystem.ParseConsoleFunctionCall(
                consoleFunc, args.c_str(), consoleChannel);
            if (success) {
              LOG_INFO("WebService.FuncCallSuccess",
                       "CONSOLE_FUNC %s %s success", func.c_str(),
                       args.c_str());
              requestPtr->_result += outText;
            } else {
              LOG_INFO("WebService.FuncCallFailure",
                       "CONSOLE_FUNC %s %s failed %s", func.c_str(),
                       args.c_str(), outText);
              requestPtr->_result += outText;
            }
          } else {
            LOG_INFO("WebService.FuncCallNotFound",
                     "CONSOLE_FUNC %s %s not found", func.c_str(),
                     args.c_str());
          }
        } break;
        case RT_External: {
          // Call out to the external update handler
          DEV_ASSERT(requestPtr->_externalCallback != nullptr,
                     "Expecting valid externalCallback pointer");
        ...
        case RT_WebsocketOnData: {
          ...
          auto sendToClient = [idx, moduleName,
                               this](const Json::Value& toSend) {
            std::lock_guard<std::mutex> lock(s_wsConnectionsMutex);
            if ((idx < _webSocketConnections.size()) &&
                (_webSocketConnections[idx].subscribedModules.count(
                     moduleName) > 0)) {
              Json::Value payload;
          ...
          requestPtr->_done =
              true;  // no one cares about the result, just cleanup immediately
        } break;
      }
...
#ifdef VICOS
    // shutdown nicely on the robot but let the OS handle it for the simulator,
    // mg_stop triggers the thread sanitizer and execution stops here, by
    // removing this line in SIMULATOR builds it allows the thread sanitizier to
...
void WebService::RegisterRequestHandler(std::string uri,
...
int WebService::ProcessRequestExternal(struct mg_connection* conn, void* cbdata,
  ...
  return ProcessRequest(conn, WebService::WebService::RequestType::RT_External,
                        param1, param2, param3, waitAndSendResponse,
                        extCallback, cbdata);
}
...
void WebService::GenerateConsoleVarsUI(std::string& page,
  ...
  for (Anki::Util::ConsoleSystem::VariableDatabase::const_iterator it =
    ...
    if (it->second->IsToggleable()) {
      ...
      category_html[cat] += "                    <label for=\"" + label +
                            "\">" + label + "</label>\n";
      const std::string checked = (it->second->GetAsInt64()) ? " checked" : "";
      category_html[cat] +=
          "                    <input type=\"checkbox\" name=\"" + label +
          "\" id=\"" + label + "\" onclick=\"onCheckboxClickHandler(this)\"" +
          checked + ">\n";
      category_html[cat] += "                </div>\n";
      ...
      category_html[cat] += "                    <label for=\"" + label +
                            "\">" + label + "</label>\n";
      category_html[cat] += "                    <select name=\"" + label +
                            "\" id=\"" + label + "\" class=\"listbox\">\n";
      const auto& values = it->second->EnumValues();
      s64 currentValue = it->second->GetAsInt64();
      for (const auto& item : values) {
        const std::string selected =
            (currentValue == 0) ? "selected=\"selected\"" : "";
        --currentValue;
        category_html[cat] += "                        <option " + selected +
                              ">" + item + "</option>\n";
      }
      ...
      if (it->second->IsIntegerType()) {
        if (it->second->IsSignedType()) {
          snprintf(sliderRange, sizeof(sliderRange),
                   "data-value=\"%lld\" data-begin=\"%lld\" data-end=\"%lld\" "
                   "data-scale=\"1\"",
                   it->second->GetAsInt64(), it->second->GetMinAsInt64(),
                   it->second->GetMaxAsInt64());

          snprintf(inputRange, sizeof(inputRange), "min=\"%lld\" max=\"%lld\"",
                   it->second->GetMinAsInt64(), it->second->GetMaxAsInt64());
        } else {
          snprintf(sliderRange, sizeof(sliderRange),
                   "data-value=\"%llu\" data-begin=\"%llu\" data-end=\"%llu\" "
                   "data-scale=\"1\"",
                   it->second->GetAsUInt64(), it->second->GetMinAsUInt64(),
                   it->second->GetMaxAsUInt64());

          snprintf(inputRange, sizeof(inputRange), "min=\"%llu\" max=\"%llu\"",
                   it->second->GetMinAsUInt64(), it->second->GetMaxAsUInt64());
        }
      } else {
        snprintf(sliderRange, sizeof(sliderRange),
                 "data-value=\"%g\" data-begin=\"%g\" data-end=\"%g\" "
                 "data-scale=\"100.0\"",
                 it->second->GetAsDouble(), it->second->GetMinAsDouble(),
                 it->second->GetMaxAsDouble());

        snprintf(inputRange, sizeof(inputRange), "min=\"%g\" max=\"%g\"",
                 it->second->GetMinAsDouble(), it->second->GetMaxAsDouble());
      }

      category_html[cat] += "                <div>\n";
      category_html[cat] += "                  <label for=\"" + label +
                            "_amount\">" + label + ":</label>\n";
      category_html[cat] += "                  <div id=\"" + label +
                            "\" class=\"slider\" " + sliderRange +
                            " style=\"width: 100px; margin: 0.25em;\"></div>\n";
      category_html[cat] +=
          "                  <input type=\"text\" id=\"" + label +
          "_amount\" class=\"amount\" " + inputRange +
          " style=\"margin: 0.25em; border:1; font-weight:bold;\">\n";
  ...
  for (Anki::Util::ConsoleSystem::FunctionDatabase::const_iterator it =
    ...
    if (sig.empty()) {
      ...
      category_html[cat] += "                <div>\n";
      category_html[cat] += "                  <a id=\"tt\" title=\"(" + sig +
                            ")\"><label for=\"" + label + "_function\">" +
                            label + ":</label></a>\n";
      category_html[cat] += "                  <input type=\"text\" id=\"" +
                            label +
                            "_args\" value=\"\" style=\"margin: 0.25em; "
                            "border:1; font-weight:bold;\">\n";
      category_html[cat] += "                  <input type=\"submit\" id=\"" +
                            label +
                            "_function\" value=\"Call\" class=\"function\">\n";
      category_html[cat] += "                </div><br>\n";
    }
  }
  ...
  html += "    <ul>\n";
  for (std::map<std::string, std::string>::const_iterator it =
           category_merged_html.begin();
       it != category_merged_html.end(); ++it) {
    html += "        <li><a href=\"#tabs-" + sanitize_tag(it->first) + "\">" +
            it->first + "</a></li>\n";
  }
  html += "    </ul>\n";
...
void WebService::SendToWebSockets(const std::string& moduleName,
                                  const Json::Value& data) const {
  std::lock_guard<std::mutex> lock(s_wsConnectionsMutex);
  Json::Value payload;
  bool hasAssigned =
      false;  // don't copy payload unless there is >= 1 client for this module
  for (const auto& connData : _webSocketConnections) {
    if (connData.subscribedModules.find(moduleName) !=
        connData.subscribedModules.end()) {
      if (!hasAssigned) {
        payload["module"] = moduleName;
        payload["data"] = data;
        hasAssigned = true;
...
bool WebService::IsWebVizClientSubscribed(const std::string& moduleName) const {
  ...
  for (const auto& connData : _webSocketConnections) {
    if ((connData.subscribedModules.find(moduleName) !=
         connData.subscribedModules.end()) ||
        (moduleName.empty() &&
         !connData.subscribedModules.empty()))  // any module subscribed
    {
      return true;
    }
  }
...
int WebService::HandleWebSocketsConnect(const struct mg_connection* conn,
                                        void* cbparams) {
  return 0;  // proceed with connection
}

void WebService::HandleWebSocketsReady(struct mg_connection* conn,
                                       void* cbparams) {
  struct mg_context* ctx = mg_get_context(conn);
  Anki::Vector::WebService::WebService* that =
      static_cast<Anki::Vector::WebService::WebService*>(mg_get_user_data(ctx));
  DEV_ASSERT(that != nullptr, "Expecting valid webservice this pointer");
  that->OnOpenWebSocket(conn);
}

int WebService::HandleWebSocketsData(struct mg_connection* conn, int bits,
  ...
  switch (opcode) {
    ...
    case WebSocketsTypeCloseConnection: {
      // agree to close connection, but don't do anything here until the close
      // event fires
      ret = 0;
    } break;
...
void WebService::HandleWebSocketsClose(const struct mg_connection* conn,
                                       void* cbparams) {
  struct mg_context* ctx = mg_get_context(conn);
  Anki::Vector::WebService::WebService* that =
      static_cast<Anki::Vector::WebService::WebService*>(mg_get_user_data(ctx));
...
void WebService::SendToWebSocket(struct mg_connection* conn,
                                 const Json::Value& data) const {
...
void WebService::OnReceiveWebSocket(struct mg_connection* conn,
                                    const Json::Value& data) {
  std::lock_guard<std::mutex> lock(s_wsConnectionsMutex);

  // find connection
  auto it = std::find_if(
      _webSocketConnections.begin(), _webSocketConnections.end(),
      [&conn](const auto& perConnData) { return perConnData.conn == conn; });
  ...
  if (it != _webSocketConnections.end()) {
    if (!data["type"].isNull() && !data["module"].isNull()) {
      ...
      if (data["type"].asString() == "subscribe") {
        ...
        ProcessRequest(
            conn, WebService::WebService::RequestType::RT_WebsocketOnSubscribe,
            moduleName, std::to_string(idx), "", waitAndSendResponse);
      } else if (data["type"].asString() == "unsubscribe") {
        it->subscribedModules.erase(moduleName);
      } else if ((data["type"].asString() == "data") &&
                 !data["data"].isNull()) {
        const bool waitAndSendResponse = false;
        std::stringstream ss;
        ss << data["data"];
        ProcessRequest(
            conn, WebService::WebService::RequestType::RT_WebsocketOnData,
            moduleName, std::to_string(idx), ss.str(), waitAndSendResponse);
      }
    ...
    std::stringstream ss;
    ss << data;
    LOG_ERROR("Webservice.OnReceiveWebSocket", "No connection for data %s",
              ss.str().c_str());
  }
}
...
bool WebService::IsWebVizClientSubscribed(
...
int WebService::ProcessRequestExternal(struct mg_connection* /*conn*/,
                                       void* /*cbdata*/,
                                       ExternalCallback /*extCallback*/,
                                       const std::string& /*param1*/,
                                       const std::string& /*param2*/,
